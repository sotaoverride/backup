...
void gt_ggc_mx_lang_tree_node(void* x_p)
{
    ...
    while (ggc_test_and_set_mark(xlimit))
        xlimit =
            (CODE_CONTAINS_STRUCT(TREE_CODE(&(*xlimit).generic), TS_TYPE_COMMON)
                 ? ((union lang_tree_node*)(*xlimit)
                        .generic.type_common.next_variant)
                 : CODE_CONTAINS_STRUCT(TREE_CODE(&(*xlimit).generic),
                                        TS_COMMON)
                       ? ((union lang_tree_node*)(*xlimit).generic.common.chain)
                       : NULL);
    while (x != xlimit)
    {
        ...
        x = (CODE_CONTAINS_STRUCT(TREE_CODE(&(*x).generic), TS_TYPE_COMMON)
                 ? ((union lang_tree_node*)(*x)
                        .generic.type_common.next_variant)
                 : CODE_CONTAINS_STRUCT(TREE_CODE(&(*x).generic), TS_COMMON)
                       ? ((union lang_tree_node*)(*x).generic.common.chain)
                       : NULL);
    }
}
