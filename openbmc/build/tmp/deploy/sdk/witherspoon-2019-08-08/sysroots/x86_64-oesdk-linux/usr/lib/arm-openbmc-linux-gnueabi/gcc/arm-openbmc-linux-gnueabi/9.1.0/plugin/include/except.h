...
enum eh_region_type
{
    ...
    /* TRY regions implement catching an exception.  The list of types
       associated with the attached catch handlers is examined in order by the
       runtime and control is transferred to the appropriate handler.  Note that
       a NULL type list is a catch-all handler, and that it will catch *all*
       exceptions including those originating from a different language.  */
    ERT_TRY,
...
struct GTY(()) eh_region_d
{
    ...
    union eh_region_u
    {
        ...
        struct eh_region_u_must_not_throw
        {
           ...
           reachable from within the function, rather than implementable from
           the runtime. The normal way for this to happen is for there to be a
           CLEANUP region contained within this MUST_NOT_THROW region.  Note
           that if the runtime handles the MUST_NOT_THROW region, we have no
           control over what termination function is called; it will be decided
           by the personality function in effect for this CIE.  */
            tree failure_decl;
...
extern hash_map<void*, void*>* duplicate_eh_regions(struct function*, eh_region,
                                                    int,
