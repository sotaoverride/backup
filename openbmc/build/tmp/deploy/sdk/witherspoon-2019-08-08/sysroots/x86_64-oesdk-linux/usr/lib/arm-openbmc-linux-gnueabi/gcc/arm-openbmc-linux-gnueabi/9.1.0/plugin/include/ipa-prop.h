...
struct GTY(()) ipa_pass_through_data
{
    ...
    /* Operation that is performed on the argument before it is passed on.
       ...
       and operand field from this structure is the second one.  */
...
struct GTY(()) ipa_jump_func
{
    ...
    /* Information about value range, containing valid data only when vr_known
       is true.  The pointed to structure is shared betweed different jump
    ...
    /* Represents a value of a jump function.  pass_through is used only in jump
       ...
       jump functions and member_cst holds constant c++ member functions.  */
...
inline ipa_node_params::ipa_node_params() :
    ...
    known_contexts(vNULL), analysis_done(0), node_enqueued(0),
    do_clone_for_all_contexts(0), is_all_contexts_clone(0), node_dead(0),
    node_within_scc(0), node_calling_single_call(0), versionable(0)
{
}
...
struct ipa_func_body_info
{
    ...
    /* Number of statements we are still allowed to walked by when analyzing
       this function.  */
...
static inline void ipa_set_param_used(struct ipa_node_params* info, int i,
                                      bool val)
{
    gcc_checking_assert(info->descriptors);
...
static inline void ipa_set_controlled_uses(struct ipa_node_params* info, int i,
                                           int val)
{
    gcc_checking_assert(info->descriptors);
...
class GTY((user)) ipa_node_params_t : public function_summary<ipa_node_params*>
{
  public:
    ...
    virtual void duplicate(cgraph_node* node, cgraph_node* node2,
                           ipa_node_params* data, ipa_node_params* data2);
};
...
tree ipa_get_indirect_edge_target(struct cgraph_edge* ie, vec<tree>,
                                  ...
                                  vec<ipa_agg_jump_function_p>, bool*);
...
ipa_polymorphic_call_context
    ipa_context_from_jfunc(ipa_node_params*, cgraph_edge*, int, ipa_jump_func*);
