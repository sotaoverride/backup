...
enum availability
{
    ...
    /* Function body/variable initializer is known and all it's uses are
       explicitly visible within current unit (ie it's address is never taken
           ...
           chain_prev("%h.previous"))) symtab_node
{
  public:
    ...
    bool call_for_symbol_and_aliases(bool (*callback)(symtab_node*, void*),
                                     void* data, bool include_overwrite);
    ...
    inline bool can_be_discarded_p(void)
    {
        return (DECL_EXTERNAL(decl) ||
                ((get_comdat_group() || DECL_COMMON(decl) ||
    ...
    /* When true the alias is translated into its target symbol either by GCC
       ...
       Currently transparent aliases come in three different flavors
         - aliases having the same assembler name as their target (aka
       duplicated declarations). In this case the assembler names compare via
    ...
    /* Set when symbol needs to be streamed into LTO bytecode for LTO, or in
       case of offloading, for separate compilation for a different target.  */
    ...
    /* Linked list of symbols with the same asm name.  There may be multiple
       ...
       Because inline clones are kept in the assembler name has, they also
  ...
  protected:
    ...
    /* Remove node from symbol table.  This function is not used directly, but
       via cgraph/varpool node removal routines.  */
...
struct GTY((tag("SYMTAB_FUNCTION"))) cgraph_node : public symtab_node
{
  public:
    ...
    /* Walk the alias chain to return the function cgraph_node is alias of.
       ...
       when determining the availability.  */
    cgraph_node*
        function_or_virtual_thunk_symbol(enum availability* avail = NULL,
                                         struct symtab_node* ref = NULL);
    ...
    /* Create node representing clone of N executed COUNT times.  Decrease
       ...
       be the outline function the new one is (even indirectly) inlined to. All
       hooks will see this in node's global.inlined_to, when invoked. Can be
       NULL if the node is not inlined.  SUFFIX is string that is appended to
       the original name.  */
    ...
    /* Create callgraph node clone with new declaration.  The actual body will
       be copied later at compilation stage.  The name of the new clone will be
    ...
    cgraph_edge* create_edge(cgraph_node* callee, gcall* call_stmt,
                             profile_count count);
    ...
    void create_edge_including_clones(cgraph_node* callee, gimple* old_stmt,
                                      gcall* stmt, profile_count count,
    ...
    /* Call callback on cgraph_node, thunks and aliases associated to NODE.
       ...
       skipped.  */
    bool call_for_symbol_thunks_and_aliases(
        bool (*callback)(cgraph_node* node, void* data), void* data,
        bool include_overwritable, bool exclude_virtual_thunks = false);
    ...
    /* Return true if cgraph_node can be made local for API change.
       ...
       at the expense of possible code size growth if function is used in
    ...
    /* Return true when function cgraph_node can be expected to be removed
       ...
       This function behaves as cgraph_only_called_directly_p because
       eliminating all uses of COMDAT function does not make it necessarily
       disappear from the program unless we are compiling whole program or we do
       LTO.  In this case we know we win since dynamic linking will not really
       discard the linkonce section.
  ...
  private:
    ...
    bool call_for_symbol_and_aliases_1(bool (*callback)(cgraph_node*, void*),
                                       void* data, bool include_overwritable);
};
...
class GTY(()) ipa_polymorphic_call_context
{
  public:
    ...
    /* Produce context specifying all derrived types of OTR_TYPE.  If OTR_TYPE
       is NULL, the context is set to dummy "I know nothing" setting.  */
            ...
            for_user)) cgraph_edge
{
    ...
    /* Speculative call edge turned out to be direct call to CALLE_DECL.
       ...
       call. It is up to caller to redirect the call as appropriate. */
...
class GTY((tag("SYMTAB_VARIABLE"))) varpool_node : public symtab_node
{
  public:
    ...
    /* For given variable pool node, walk the alias chain to return the function
       ...
       when determining the availability.  */
    inline varpool_node*
        ultimate_alias_target(availability* availability = NULL,
                              symtab_node* ref = NULL);
...
template <>
template <>
inline bool is_a_helper<cgraph_node*>::test(symtab_node* p)
{
...
template <>
template <>
inline bool is_a_helper<varpool_node*>::test(symtab_node* p)
{
...
class GTY((tag("SYMTAB"))) symbol_table
{
  public:
    ...
    /* C++ frontend produce same body aliases all over the place, even before
       PCH gets streamed out. It relies on us linking the aliases with their
       function in order to do the fixups, but ipa-ref is not PCH safe.
       Consequentely we first produce aliases without links, but once C++ FE is
       sure he won't sream PCH we build the links via this function.  */
    ...
    /* Once all functions from compilation unit are in memory, produce all
       clones and update all calls.  We might also do this on demand if we don't
       want to bring all functions to memory prior compilation, but current
       WHOPR implementation does that and it is bit easier to keep everything
       right in this order.  */
...
void tree_function_versioning(tree, tree, vec<ipa_replace_map*, va_gc>*, bool,
                              bitmap, bool, bitmap, basic_block);
...
inline bool resolution_used_from_other_file_p(
    enum ld_plugin_symbol_resolution resolution)
{
    return (
        resolution == LDPR_PREVAILING_DEF || resolution == LDPR_PREEMPTED_REG ||
...
#define FOR_EACH_VARIABLE(node)                                                \
    for ((node) = symtab->first_variable(); (node);                            \
...
{
    return (!thunk.thunk_p && (thunk.fixed_offset || thunk.virtual_offset_p ||
...
inline bool cgraph_node::only_called_directly_or_aliased_p(void)
{
    ...
    return (!force_output && !address_taken && !ifunc_resolver &&
            !used_from_other_partition && !DECL_VIRTUAL_P(decl) &&
            !DECL_STATIC_CONSTRUCTOR(decl) && !DECL_STATIC_DESTRUCTOR(decl) &&
            !used_from_object_file_p() && !externally_visible);
}
...
inline bool varpool_node::can_remove_if_no_refs_p(void)
{
    if (DECL_EXTERNAL(decl))
        return true;
    return (
        !force_output && !used_from_other_partition &&
        ((DECL_COMDAT(decl) && !forced_by_abi && !used_from_object_file_p()) ||
...
{
    return (definition && !externally_visible && !used_from_other_partition &&
            !force_output);
}
...
inline bool symtab_node::call_for_symbol_and_aliases(
    bool (*callback)(symtab_node*, void*), void* data,
...
inline bool cgraph_node::call_for_symbol_and_aliases(
    bool (*callback)(cgraph_node*, void*), void* data,
    bool include_overwritable)
{
    if (include_overwritable || get_availability() > AVAIL_INTERPOSABLE)
    {
...
inline bool varpool_node::call_for_symbol_and_aliases(
    bool (*callback)(varpool_node*, void*), void* data,
    bool include_overwritable)
{
    if (include_overwritable || get_availability() > AVAIL_INTERPOSABLE)
    {
