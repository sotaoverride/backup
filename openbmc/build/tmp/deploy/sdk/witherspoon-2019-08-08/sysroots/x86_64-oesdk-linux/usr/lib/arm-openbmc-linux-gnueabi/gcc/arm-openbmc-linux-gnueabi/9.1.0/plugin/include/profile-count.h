...
#else
    if (a < ((uint64_t)1 << 31) && b < ((uint64_t)1 << 31) &&
        c < ((uint64_t)1 << 31))
    {
        *res = (a * b + (c / 2)) / c;
        return true;
...
/* Data type to hold probabilities.  It implements fixed point arithmetics
   ...
   probabilities that has not been determined yet (for example bacause of
...
class GTY((user)) profile_probability
{
  ...
  public:
    ...
    /* Return VAL1/VAL2.  */
    static profile_probability probability_in_gcov_type(gcov_type val1,
                                                        gcov_type val2)
    {
        profile_probability ret;
    ...
    profile_probability operator/(const profile_probability& other) const
    {
        ...
        if (m_val >= other.m_val)
        {
            ret.m_val = max_probability;
            ret.m_quality =
                MIN(MIN(m_quality, other.m_quality), profile_guessed);
            return ret;
        }
        else if (!m_val)
            ret.m_val = 0;
        else
        {
            ...
            ret.m_val =
                MIN(RDIV((uint64_t)m_val * max_probability, other.m_val),
                    max_probability);
        }
        ret.m_quality = MIN(MIN(m_quality, other.m_quality), profile_adjusted);
        return ret;
    ...
    /* Return true when the probability of edge is reliable.
       ...
       to 50%) than the reality so it is bad idea to use it to drive
       optimization such as those disabling dynamic branch prediction for well
       predictable branches.

       There are two exceptions - edges leading to noreturn edges and edges
       predicted by number of iterations heuristics are predicted well.  This
       macro should be able to distinguish those, but at the moment it simply
       check for noreturn heuristic that is only one giving probability over 99%
       or bellow 1%.  In future we might want to propagate reliability
...
#undef UINT64_BIT_FIELD_ALIGN
  ...
  public:
    ...
    profile_count max(profile_count other) const
    {
        ...
        gcc_checking_assert(compatible_p(other));
        if (m_val < other.m_val ||
            (m_val == other.m_val && m_quality < other.m_quality))
            return other;
        return *this;
    }
    ...
    profile_count apply_probability(profile_probability prob) const
    {
        ...
        safe_scale_64bit(m_val, prob.m_val,
                         profile_probability::max_probability, &tmp);
        ret.m_val = tmp;
    ...
    profile_count apply_scale(profile_count num, profile_count den) const
    {
        ...
        ret.m_quality =
            MIN(MIN(MIN(m_quality, profile_adjusted), num.m_quality),
                den.m_quality);
    ...
    profile_probability probability_in(const profile_count overall) const
    {
        ...
        if (overall.m_val < m_val)
        {
            ...
            ret.m_val = RDIV(m_val * profile_probability::max_probability,
                             overall.m_val);
        ret.m_quality =
            MIN(MAX(MIN(m_quality, overall.m_quality), profile_guessed),
                profile_adjusted);
        return ret;
    }
