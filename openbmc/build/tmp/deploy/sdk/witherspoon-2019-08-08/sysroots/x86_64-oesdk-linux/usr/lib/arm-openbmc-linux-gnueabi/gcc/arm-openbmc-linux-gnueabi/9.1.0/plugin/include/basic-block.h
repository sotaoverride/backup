...
typedef int
    __assert_gimple_bb_smaller_rtl_bb[(int)sizeof(struct rtl_bb_info) -
...
#define BB_SET_PARTITION(bb, part)                                             \
    ...
    {                                                                          \
        basic_block bb_ = (bb);                                                \
        bb_->flags =                                                           \
            ((bb_->flags & ~(BB_HOT_PARTITION | BB_COLD_PARTITION)) | (part)); \
    } while (0)
...
#define FALLTHRU_EDGE(bb)                                                      \
    (EDGE_SUCC((bb), 0)->flags & EDGE_FALLTHRU ? EDGE_SUCC((bb), 0)            \
                                               : EDGE_SUCC((bb), 1))

/* BB is assumed to contain conditional jump.  Return the branch edge.  */
#define BRANCH_EDGE(bb)                                                        \
    (EDGE_SUCC((bb), 0)->flags & EDGE_FALLTHRU ? EDGE_SUCC((bb), 1)            \
                                               : EDGE_SUCC((bb), 0))
...
#define FOR_EACH_EDGE(EDGE, ITER, EDGE_VEC)                                    \
    for ((ITER) = ei_start((EDGE_VEC)); ei_cond((ITER), &(EDGE));              \
         ei_next(&(ITER)))
