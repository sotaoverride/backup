...
/* Usage of TREE_LANG_FLAG_?:
  ...
  VIEW_CONVERT_EXPR) AGGR_INIT_ZERO_FIRST (in AGGR_INIT_EXPR)
  ...
  TEMPLATE_DECL) DECL_LOCAL_FUNCTION_P (in FUNCTION_DECL) DECL_MUTABLE_P (in
  FIELD_DECL) DECL_DEPENDENT_P (in USING_DECL) LABEL_DECL_BREAK (in LABEL_DECL)
...
union GTY(
    (desc("cp_tree_node_structure (&%h)"),
     chain_next("(union lang_tree_node *) c_tree_chain_next (&%h.generic)")))
    lang_tree_node
{
    ...
    struct tree_static_assert
        GTY((tag("TS_CP_STATIC_ASSERT"))) static_assertion;
    ...
    struct tree_constraint_info
        GTY((tag("TS_CP_CONSTRAINT_INFO"))) constraint_info;
    struct tree_userdef_literal
        GTY((tag("TS_CP_USERDEF_LITERAL"))) userdef_literal;
};
...
#define DECL_FRIEND_CONTEXT(NODE)                                              \
    ((DECL_DECLARES_FUNCTION_P(NODE) && DECL_FRIEND_P(NODE) &&                 \
      !DECL_FUNCTION_MEMBER_P(NODE))                                           \
         ? LANG_DECL_FN_CHECK(NODE)->context                                   \
         : NULL_TREE)
...
#define TYPE_ALIAS_TEMPLATE_INFO(NODE)                                         \
    (DECL_LANG_SPECIFIC(TYPE_NAME(NODE)) ? DECL_TEMPLATE_INFO(TYPE_NAME(NODE)) \
                                         : NULL_TREE)
...
#define TYPE_TEMPLATE_INFO_MAYBE_ALIAS(NODE)                                   \
    (TYPE_ALIAS_P(NODE) ? TYPE_ALIAS_TEMPLATE_INFO(NODE)                       \
...
#define REF_PARENTHESIZED_P(NODE)                                              \
    TREE_LANG_FLAG_2(TREE_CHECK4((NODE), COMPONENT_REF, INDIRECT_REF,          \
                                 SCOPE_REF, VIEW_CONVERT_EXPR))
...
inline void init_aggr_init_expr_arg_iterator(tree exp,
...
inline tree first_aggr_init_expr_arg(tree exp,
                                     aggr_init_expr_arg_iterator* iter)
{
...
#define SCALAR_TYPE_P(TYPE)                                                    \
    (TYPE_PTRDATAMEM_P(TYPE) || TREE_CODE(TYPE) == ENUMERAL_TYPE ||            \
     ARITHMETIC_TYPE_P(TYPE) || TYPE_PTR_P(TYPE) || TYPE_PTRMEMFUNC_P(TYPE) || \
     NULLPTR_TYPE_P(TYPE))
...
#define EMPTY_CONSTRUCTOR_P(NODE)                                              \
    (TREE_CODE(NODE) == CONSTRUCTOR &&                                         \
     vec_safe_is_empty(CONSTRUCTOR_ELTS(NODE)) && !TREE_HAS_CONSTRUCTOR(NODE))
...
#define TYPE_OBJ_P(NODE)                                                       \
    (!TYPE_REF_P(NODE) && !VOID_TYPE_P(NODE) &&                                \
...
#define CP_OMP_CLAUSE_INFO(NODE)                                               \
    TREE_TYPE(                                                                 \
        OMP_CLAUSE_RANGE_CHECK(NODE, OMP_CLAUSE_PRIVATE, OMP_CLAUSE_LINEAR))
...
extern tree build_conditional_expr(const op_location_t&, tree, tree, tree,
                                   tsubst_flags_t);
...
extern tree build_operator_new_call(tree, vec<tree, va_gc>**, tree*, tree*,
                                    tree, tree, tree*, tsubst_flags_t);
extern tree build_new_method_call(tree, tree, vec<tree, va_gc>**, tree, int,
                                  tree*, tsubst_flags_t);
extern tree build_special_member_call(tree, tree, vec<tree, va_gc>**, tree, int,
                                      tsubst_flags_t);
extern tree build_new_op(const op_location_t&, enum tree_code, int, tree, tree,
...
extern struct conversion* good_conversion(tree, tree, tree, int,
                                          tsubst_flags_t);
extern location_t get_fndecl_argument_location(tree, int);
extern void complain_about_bad_argument(location_t arg_loc, tree from_type,
                                        tree to_type, tree fndecl, int parmnum);
...
extern bool enforce_access(tree, tree, tree, tsubst_flags_t,
...
extern tree start_decl(const cp_declarator*, cp_decl_specifier_seq*, int, tree,
                       tree, tree*);
...
extern tree grokfield(const cp_declarator*, cp_decl_specifier_seq*, tree, bool,
                      tree, tree);
...
extern tree mark_use(tree expr, bool rvalue_p, bool read_p,
                     location_t = UNKNOWN_LOCATION, bool reject_builtin = true);
...
extern tree build_new(vec<tree, va_gc>**, tree, tree, vec<tree, va_gc>**, int,
                      tsubst_flags_t);
...
extern tree implicitly_declare_fn(special_function_kind, tree, bool, tree,
                                  tree);
...
extern tree cp_convert_range_for(tree, tree, tree, tree, unsigned int, bool,
                                 unsigned short);
extern void cp_convert_omp_range_for(tree&, vec<tree, va_gc>*, tree&, tree&,
                                     tree&, tree&, tree&, tree&);
...
extern tree do_auto_deduction(tree, tree, tree,
                              ...
                              auto_deduction_context = adc_unspecified,
                              tree = NULL_TREE, int = LOOKUP_NORMAL);
...
extern tree fn_type_unification(tree, tree, tree, const tree*, unsigned int,
...
extern bool maybe_instantiate_noexcept(tree,
                                       tsubst_flags_t = tf_warning_or_error);
...
extern vec<qualified_typedef_usage_t, va_gc>*
    get_types_needing_access_check(tree);
extern int template_class_depth(tree);
extern int is_specialization_of(tree, tree);
extern bool is_specialization_of_friend(tree, tree);
extern tree get_pattern_parm(tree, tree);
extern int comp_template_args(tree, tree, tree* = NULL, tree* = NULL,
                              bool = false);
...
extern tree lookup_member(tree, tree, int, bool, tsubst_flags_t,
                          access_failure_info* afi = NULL);
...
extern tree dfs_walk_all(tree, tree (*)(tree, void*), tree (*)(tree, void*),
                         void*);
extern tree dfs_walk_once(tree, tree (*)(tree, void*), tree (*)(tree, void*),
                          void*);
...
extern cp_expr perform_koenig_lookup(cp_expr, vec<tree, va_gc>*,
                                     tsubst_flags_t);
extern tree finish_call_expr(tree, vec<tree, va_gc>**, bool, bool,
                             tsubst_flags_t);
extern tree
    lookup_and_finish_template_variable(tree, tree,
                                        tsubst_flags_t = tf_warning_or_error);
...
extern cp_expr finish_id_expression(tree, tree, tree, cp_id_kind*, bool, bool,
                                    bool*, bool, bool, bool, bool, const char**,
                                    location_t);
...
extern tree finish_qualified_id_expr(tree, tree, bool, bool, bool, bool,
                                     tsubst_flags_t);
...
extern tree finish_omp_for(location_t, enum tree_code, tree, tree, tree, tree,
                           tree, tree, tree, vec<tree>*, tree);
extern tree finish_omp_for_block(tree, tree);
extern void finish_omp_atomic(location_t, enum tree_code, enum tree_code, tree,
                              tree, tree, tree, tree, tree,
                              enum omp_memory_order);
extern void finish_omp_barrier(void);
extern void finish_omp_depobj(location_t, tree, enum omp_clause_depend_kind,
                              tree);
...
extern void cxx_print_error_function(diagnostic_context*, const char*,
...
extern tree finish_class_member_access_expr(cp_expr, tree, bool,
                                            tsubst_flags_t);
extern tree build_x_indirect_ref(location_t, tree, ref_operator,
                                 tsubst_flags_t);
...
extern tree cp_build_function_call_vec(tree, vec<tree, va_gc>**,
                                       tsubst_flags_t);
extern tree build_x_binary_op(const op_location_t&, enum tree_code, tree,
                              enum tree_code, tree, enum tree_code, tree*,
                              tsubst_flags_t);
extern tree build_x_array_ref(location_t, tree, tree, tsubst_flags_t);
extern tree build_x_unary_op(location_t, enum tree_code, cp_expr,
                             tsubst_flags_t);
...
extern tree build_x_compound_expr_from_list(tree, expr_list_kind,
                                            tsubst_flags_t);
extern tree build_x_compound_expr_from_vec(vec<tree, va_gc>*, const char*,
                                           tsubst_flags_t);
...
extern cp_expr build_x_modify_expr(location_t, tree, enum tree_code, tree,
                                   tsubst_flags_t);
extern tree cp_build_modify_expr(location_t, tree, enum tree_code, tree,
                                 tsubst_flags_t);
extern tree convert_for_initialization(tree, tree, tree, int, impl_conv_rhs,
...
extern tree composite_pointer_type(tree, tree, tree, tree,
                                   composite_pointer_operation, tsubst_flags_t);
extern tree merge_types(tree, tree);
extern tree strip_array_domain(tree);
extern tree check_return_expr(tree, bool*);
extern tree cp_build_binary_op(const op_location_t&, enum tree_code, tree, tree,
                               tsubst_flags_t);
...
typedef void cp_binding_oracle_function(enum cp_oracle_request,
                                        tree identifier);
...
inline bool named_decl_hash::equal(const value_type existing,
