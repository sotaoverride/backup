...
struct lang_hooks_for_types
{
    ...
    /* If non-NULL, return TYPE1 with any language-specific modifiers copied
       from TYPE2.  */
    ...
    /* Return TRUE if TYPE implements a fixed point type and fills in
       information for the debugger about scale factor, etc.  */
...
struct lang_hooks
{
    ...
    /* This compares two types for equivalence ("compatible" in C-based
       languages). This routine should only return 1 if it is sure.  It should
       not be used in contexts where erroneously returning 0 causes problems. */
    ...
    /* Like builtin_function, but make sure the scope is the external scope.
       ...
       that defines the builtin is declared via function specific target
       options, which can save memory for machines like the x86_64 that have
       multiple ISAs.  If this points to the same function as builtin_function,
       the
