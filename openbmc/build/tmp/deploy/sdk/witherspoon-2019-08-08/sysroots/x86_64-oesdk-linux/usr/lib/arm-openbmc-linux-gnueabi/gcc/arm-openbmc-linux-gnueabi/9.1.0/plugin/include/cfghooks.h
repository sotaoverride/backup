...
struct cfg_hooks
{
    ...
    /* Redirect edge E to the given basic block B and update underlying program
       representation.  Returns edge representing redirected branch (that may
       not be equivalent to E in the case of duplicate edges being removed) or
       NULL if edge is not easily redirectable for whatever reason.  */
    ...
    /* Add fake edges to the function exit for any non constant and non noreturn
       ...
       BLOCKS or to the whole CFG if BLOCKS is zero.  Return the number of
       blocks that were split.

       The goal is to expose cases in which entering a basic block does not
       imply that all subsequent instructions must be executed.  */
    ...
    /* A hook for duplicating loop in CFG, currently this is used
       in loop versioning.  */
    bool (*cfg_hook_duplicate_loop_to_header_edge)(struct loop*, edge, unsigned,
    ...
    void (*lv_adjust_loop_header_phi)(basic_block, basic_block, basic_block,
                                      edge);
...
extern void copy_bbs(basic_block*, unsigned, basic_block*, edge*, unsigned,
                     edge*, struct loop*, basic_block, bool);
