...
template <typename T>
inline machine_mode opt_mode<T>::else_blk() const
{
...
template <typename T>
inline T opt_mode<T>::require() const
{
...
template <typename T>
template <typename U>
inline bool opt_mode<T>::exists(U* mode) const
{
...
template <typename T>
inline bool is_a(machine_mode m)
{
    return T::includes_p(m);
}

template <typename T, typename U>
inline bool is_a(const opt_mode<U>& m)
{
...
template <typename T, typename U>
inline T as_a(const opt_mode<U>& m)
{
...
template <typename T>
inline opt_mode<T> dyn_cast(machine_mode m)
{
    if (T::includes_p(m))
...
template <typename T, typename U>
inline opt_mode<T> dyn_cast(const opt_mode<U>& m)
{
...
template <typename T, typename U>
inline bool is_a(machine_mode m, U* result)
{
    if (T::includes_p(m))
    {
...
{
#if GCC_VERSION >= 4001
    return (__builtin_constant_p(mode) ? mode_size_inline(mode)
                                       : mode_size[mode]);
#else
...
{
#if GCC_VERSION >= 4001
    return (__builtin_constant_p(mode) ? mode_unit_size_inline(mode)
                                       : mode_unit_size[mode]);
#else
...
{
#if GCC_VERSION >= 4001
    return (__builtin_constant_p(mode) ? mode_nunits_inline(mode)
                                       : mode_nunits[mode]);
#else
...
{
  public:
    bit_field_mode_iterator(HOST_WIDE_INT, HOST_WIDE_INT, poly_int64,
                            poly_int64, unsigned int, bool);
...
template <typename T>
inline T get_narrowest_mode(T mode)
{
...
template <typename T>
inline bool is_int_mode(machine_mode mode, T* int_mode)
{
...
template <typename T>
inline bool is_float_mode(machine_mode mode, T* float_mode)
{
...
template <typename T>
inline bool is_complex_int_mode(machine_mode mode, T* cmode)
{
...
template <typename T>
inline bool is_complex_float_mode(machine_mode mode, T* cmode)
{
...
template <typename T>
inline void start(opt_mode<T>* iter, enum mode_class mclass)
{
...
template <typename T>
inline bool iterate_p(opt_mode<T>* iter)
{
    return iter->exists();
}
...
template <typename T>
inline void get_wider(opt_mode<T>* iter)
{
...
template <typename T>
inline void get_known_wider(T* iter)
{
    *iter = GET_MODE_WIDER_MODE(*iter).require();
}
...
template <typename T>
inline void get_2xwider(opt_mode<T>* iter)
{
...
#define FOR_EACH_MODE(ITERATOR, START, END)                                    \
    for ((ITERATOR) = (START); (ITERATOR) != (END);                            \
