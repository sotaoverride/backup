...
extern expanded_location expand_location_to_spelling_point(
    location_t, enum location_aspect aspect = LOCATION_ASPECT_CARET);
...
/* Return true if LOCATION is the locus of a token that comes from
   ...
   from_macro_expansion_at in its treatment of macro arguments, for which this
   returns false.  */
...
extern location_t make_location(location_t caret, location_t start,
                                location_t finish);
...
class GTY(()) string_concat_db
{
  public:
    ...
    bool get_string_concatenation(location_t loc, int* out_num,
                                  location_t** out_locs);
