...
void gt_ggc_mx_string_pool_data(void* x_p)
{
                ...
                {
                    {
                        union tree_node* const x1 =
                            ((*x).entries[i0])
                                ? HT_IDENT_TO_GCC_IDENT(((*x).entries[i0]))
                                : NULL;
...
void gt_pch_nx_string_pool_data(void* x_p)
{
    struct string_pool_data* const x = (struct string_pool_data*)x_p;
    ...
    {
        {
            ...
            if ((*x).entries != NULL)
            {
                ...
                gt_pch_note_object((*x).entries, x,
                                   gt_pch_p_16string_pool_data);
            }
        }
    }
}

void gt_pch_p_16string_pool_data(ATTRIBUTE_UNUSED void* this_obj, void* x_p,
            ...
            {
                {
                    ...
                    if ((void*)((*x).entries) == this_obj)
                        op(&(x1), cookie);
                    (*x).entries[i0] =
                        (x1) ? GCC_IDENT_TO_HT_IDENT((x1)) : NULL;
                }
            }
            if ((void*)(x) == this_obj)
                op(&((*x).entries), cookie);
...
EXPORTED_CONST struct ggc_root_tab gt_ggc_r_gt_stringpool_h[] = {
    {&spd, 1, sizeof(spd), &gt_ggc_mx_string_pool_data,
