...
/* Returns the last map that was allocated in the line table SET.
   ...
   otherwise.*/
inline line_map* LINEMAPS_LAST_ALLOCATED_MAP(const line_maps* set,
                                             bool map_kind)
{
...
inline line_map_ordinary* LINEMAPS_ORDINARY_MAP_AT(const line_maps* set,
                                                   int index)
{
...
extern location_t linemap_line_start(struct line_maps* set,
                                     linenum_type to_line,
                                     unsigned int max_column_hint);
...
extern const struct line_map* linemap_add(struct line_maps*, enum lc_reason,
                                          unsigned int sysp,
                                          const char* to_file,
                                          linenum_type to_line);
...
inline linenum_type SOURCE_LINE(const line_map_ordinary* ord_map,
                                location_t loc)
{
...
inline linenum_type SOURCE_COLUMN(const line_map_ordinary* ord_map,
                                  location_t loc)
{
    return ((loc - ord_map->start_location) &
...
/* Encode and return a source location from a given line and
   column.  */
location_t linemap_position_for_line_and_column(line_maps* set,
                                                const line_map_ordinary*,
...
/* Encode and return a location_t starting from location LOC and
   ...
   virtual locations.  */
location_t linemap_position_for_loc_and_offset(struct line_maps* set,
                                               location_t loc,
                                               unsigned int offset);
...
/* Return a positive value if PRE denotes the location of a token that
   ...
   otherwise.  */
int linemap_compare_locations(struct line_maps* set, location_t pre,
                              location_t post);

/* Return TRUE if LOC_A denotes the location a token that comes
   topogically before the token denoted by location LOC_B, or if they
   are equal.  */
inline bool linemap_location_before_p(struct line_maps* set, location_t loc_a,
                                      location_t loc_b)
{
...
/* If LOC is the virtual location of a token coming from the expansion
   ...
   *MAP is set to the map of the returned location if the later is
   different from LOC.  */
location_t
    linemap_unwind_to_first_non_reserved_loc(struct line_maps*, location_t loc,
...
bool linemap_get_file_highest_location(struct line_maps* set,
                                       const char* file_name, location_t* loc);
