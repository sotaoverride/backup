...
class GTY((user)) hash_map
{
  ...
  public:
    ...
    explicit hash_map(
        const hash_map& h, bool ggc = false,
        bool gather_mem_stats = GATHER_STATISTICS CXX_MEM_STAT_INFO) :
    ...
    template <typename Arg,
              bool (*f)(const typename Traits::key_type&, const Value&, Arg)>
    ...
    template <typename Arg,
              bool (*f)(const typename Traits::key_type&, Value*, Arg)>
    void traverse(Arg a) const
    {
        for (typename hash_table<hash_entry>::iterator iter = m_table.begin();
             iter != m_table.end(); ++iter)
    ...
    class iterator
    {
      public:
        explicit iterator(
            const typename hash_table<hash_entry>::iterator& iter) :
            m_iter(iter)
        {
        }
...
static inline void gt_ggc_mx(hash_map<K, V, H>* h)
{
    gt_ggc_mx(&h->m_table);
}

template <typename K, typename V, typename H>
static inline void gt_pch_nx(hash_map<K, V, H>* h)
{
    gt_pch_nx(&h->m_table);
}
...
template <typename K, typename V, typename H>
static inline void gt_pch_nx(hash_map<K, V, H>* h, gt_pointer_operator op,
                             void* cookie)
{
