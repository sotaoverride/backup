...
#define DF_REF_REAL_REG(REF)                                                   \
    (GET_CODE((REF)->base.reg) == SUBREG ? SUBREG_REG((REF)->base.reg)         \
                                         : ((REF)->base.reg))
#define DF_REF_REGNO(REF) ((REF)->base.regno)
#define DF_REF_REAL_LOC(REF)                                                   \
    (GET_CODE(*((REF)->regular_ref.loc)) == SUBREG                             \
         ? &SUBREG_REG(*((REF)->regular_ref.loc))                              \
         : ((REF)->regular_ref.loc))
#define DF_REF_REG(REF) ((REF)->base.reg)
#define DF_REF_LOC(REF)                                                        \
    (DF_REF_CLASS(REF) == DF_REF_REGULAR ? (REF)->regular_ref.loc : NULL)
#define DF_REF_BB(REF)                                                         \
    (DF_REF_IS_ARTIFICIAL(REF) ? (REF)->artificial_ref.bb                      \
...
#define DF_REF_IS_REG_MARKED(REF)                                              \
    (DF_REF_FLAGS_IS_SET((REF), DF_REF_REG_MARKER))
...
#define DF_INSN_UID_SAFE_GET(UID)                                              \
    (((unsigned)(UID) < DF_INSN_SIZE()) ? DF_INSN_UID_GET(UID) : NULL)
...
extern bool can_move_insns_across(rtx_insn*, rtx_insn*, rtx_insn*, rtx_insn*,
                                  basic_block, regset, regset, rtx_insn**);
