...
#define FOR_EACH_SSA_TREE_OPERAND(TREEVAR, STMT, ITER, FLAGS)                  \
    for (TREEVAR = op_iter_init_tree(&(ITER), STMT, FLAGS);                    \
         !op_iter_done(&(ITER)); (void)(TREEVAR = op_iter_next_tree(&(ITER))))
...
static inline void op_iter_init(ssa_op_iter* ptr, gimple* stmt, int flags)
{
    ...
    ptr->uses =
        (flags & (SSA_OP_USE | SSA_OP_VUSE)) ? gimple_use_ops(stmt) : NULL;
...
static inline use_operand_p op_iter_init_phiuse(ssa_op_iter* ptr, gphi* phi,
                                                int flags)
{
...
static inline def_operand_p op_iter_init_phidef(ssa_op_iter* ptr, gphi* phi,
                                                int flags)
{
