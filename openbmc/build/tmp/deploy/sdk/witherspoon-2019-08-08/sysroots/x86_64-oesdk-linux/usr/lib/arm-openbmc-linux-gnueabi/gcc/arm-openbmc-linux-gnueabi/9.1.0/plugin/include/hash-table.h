...
template <typename Type>
inline Type* xcallocator<Type>::data_alloc(size_t count)
{
...
template <typename Type>
inline void xcallocator<Type>::data_free(Type* memory)
{
    return ::free(memory);
}
...
extern unsigned int
    hash_table_higher_prime_index(unsigned long n) ATTRIBUTE_PURE;
...
class hash_table
{
  ...
  public:
    explicit hash_table(
        size_t, bool ggc = false, bool gather_mem_stats = GATHER_STATISTICS,
        mem_alloc_origin origin = HASH_TABLE_ORIGIN CXX_MEM_STAT_INFO);
    explicit hash_table(
        const hash_table&, bool ggc = false,
...
hash_table<Descriptor, Lazy, Allocator>::hash_table(
    size_t size, bool ggc, bool gather_mem_stats ATTRIBUTE_UNUSED,
    mem_alloc_origin origin MEM_STAT_DECL) :
    m_n_elements(0),
    m_n_deleted(0), m_searches(0), m_collisions(0), m_ggc(ggc)
#if GATHER_STATISTICS
    ,
...
template <typename Descriptor, bool Lazy,
          template <typename Type> class Allocator>
hash_table<Descriptor, Lazy, Allocator>::hash_table(
    const hash_table& h, bool ggc, bool gather_mem_stats ATTRIBUTE_UNUSED,
    mem_alloc_origin origin MEM_STAT_DECL) :
    m_n_elements(h.m_n_elements),
    m_n_deleted(h.m_n_deleted), m_searches(0), m_collisions(0), m_ggc(ggc)
#if GATHER_STATISTICS
    ,
    m_gather_mem_stats(gather_mem_stats)
#endif
...
inline typename hash_table<Descriptor, Lazy, Allocator>::value_type*
    hash_table<Descriptor, Lazy, Allocator>::alloc_entries(
        size_t n MEM_STAT_DECL) const
{
    value_type* nentries;

    if (m_gather_mem_stats)
        hash_table_usage().register_instance_overhead(sizeof(value_type) * n,
                                                      this);
...
typename hash_table<Descriptor, Lazy, Allocator>::value_type*
    hash_table<Descriptor, Lazy, Allocator>::find_empty_slot_for_expand(
        hashval_t hash)
{
...
template <typename Descriptor, bool Lazy,
          template <typename Type> class Allocator>
typename hash_table<Descriptor, Lazy, Allocator>::value_type&
    hash_table<Descriptor, Lazy, Allocator>::find_with_hash(
...
template <typename Descriptor, bool Lazy,
          template <typename Type> class Allocator>
void hash_table<Descriptor, Lazy, Allocator>::remove_elt_with_hash(
    const compare_type& comparable, hashval_t hash)
{
...
template <typename Descriptor, bool Lazy,
          template <typename Type> class Allocator>
template <typename Argument,
          int (*Callback)(typename hash_table<Descriptor, Lazy,
                                              Allocator>::value_type* slot,
                          Argument argument)>
...
#define FOR_EACH_HASH_TABLE_ELEMENT(HTAB, RESULT, TYPE, ITER)                  \
    for ((ITER) = (HTAB).begin();                                              \
         (ITER) != (HTAB).end() ? (RESULT = *(ITER), true) : false; ++(ITER))

/* ggc walking routines.  */

template <typename E>
static inline void gt_ggc_mx(hash_table<E>* h)
{
...
template <typename D>
static inline void hashtab_entry_note_pointers(void* obj, void* h,
...
template <typename D>
static void gt_pch_nx(hash_table<D>* h)
{
    bool success =
...
template <typename D>
static inline void gt_pch_nx(hash_table<D>* h, gt_pointer_operator op,
                             void* cookie)
{
...
template <typename H>
inline void gt_cleare_cache(hash_table<H>* h)
{
