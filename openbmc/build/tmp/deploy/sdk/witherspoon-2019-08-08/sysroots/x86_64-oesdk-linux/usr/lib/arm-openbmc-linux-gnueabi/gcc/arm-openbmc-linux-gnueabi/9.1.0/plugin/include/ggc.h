...
extern void*
    ggc_internal_cleared_alloc(size_t, void (*)(void*), size_t,
                               size_t CXX_MEM_STAT_INFO) ATTRIBUTE_MALLOC;
...
template <typename T>
inline T* ggc_alloc(ALONE_CXX_MEM_STAT_INFO)
{
    if (need_finalization_p<T>())
        ...
        return static_cast<T*>(
            ggc_internal_alloc(sizeof(T), NULL, 0, 1 PASS_MEM_STAT));
}

template <typename T>
inline T* ggc_cleared_alloc(ALONE_CXX_MEM_STAT_INFO)
{
    if (need_finalization_p<T>())
        return static_cast<T*>(ggc_internal_cleared_alloc(
            sizeof(T), finalize<T>, 0, 1 PASS_MEM_STAT));
    else
        return static_cast<T*>(
...
template <typename T>
inline T* ggc_vec_alloc(size_t c CXX_MEM_STAT_INFO)
{
    if (need_finalization_p<T>())
...
template <typename T>
inline T* ggc_cleared_vec_alloc(size_t c CXX_MEM_STAT_INFO)
{
    if (need_finalization_p<T>())
        return static_cast<T*>(ggc_internal_cleared_alloc(
            c * sizeof(T), finalize<T>, sizeof(T), c PASS_MEM_STAT));
    else
...
extern const char* ggc_alloc_string(const char* contents,
                                    int length CXX_MEM_STAT_INFO);
