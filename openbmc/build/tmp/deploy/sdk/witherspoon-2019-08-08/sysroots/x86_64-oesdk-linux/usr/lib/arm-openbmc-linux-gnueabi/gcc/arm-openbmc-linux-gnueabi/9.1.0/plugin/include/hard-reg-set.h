...
static inline bool hard_reg_set_subset_p(const HARD_REG_SET x,
                                         const HARD_REG_SET y)
{
    return (x & ~y) == HARD_CONST(0);
}

static inline bool hard_reg_set_equal_p(const HARD_REG_SET x,
                                        const HARD_REG_SET y)
{
    return x == y;
}

static inline bool hard_reg_set_intersect_p(const HARD_REG_SET x,
                                            const HARD_REG_SET y)
{
    return (x & y) != HARD_CONST(0);
}
...
static inline void hard_reg_set_iter_init(hard_reg_set_iterator* iter,
                                          HARD_REG_SET set, unsigned min,
                                          unsigned* regno)
{
...
static inline bool hard_reg_set_iter_set(hard_reg_set_iterator* iter,
                                         unsigned* regno)
{
    while (1)
    {
...
static inline void hard_reg_set_iter_next(hard_reg_set_iterator* iter,
                                          unsigned* regno)
{
    iter->bits >>= 1;
    *regno += 1;
