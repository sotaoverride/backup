...
struct pool_usage : public mem_usage
{
    ...
    pool_usage(size_t allocated, size_t times, size_t peak, size_t instances,
        ...
        m_element_size(element_size), m_pool_name(pool_name)
    {
    }
    ...
    pool_usage operator+(const pool_usage& second)
    {
        return pool_usage(m_allocated + second.m_allocated,
                          ...
                          m_instances + second.m_instances, m_element_size,
                          m_pool_name);
    }
...
template <typename TBlockAllocator>
inline void base_pool_allocator<TBlockAllocator>::initialize()
{
...
template <typename TBlockAllocator>
inline void base_pool_allocator<TBlockAllocator>::release()
{
    if (!m_initialized)
        return;
...
template <typename TBlockAllocator>
inline void* base_pool_allocator<TBlockAllocator>::allocate()
{
    if (!m_initialized)
        initialize();
...
template <typename T>
inline void* operator new(size_t, object_allocator<T>& a)
{
