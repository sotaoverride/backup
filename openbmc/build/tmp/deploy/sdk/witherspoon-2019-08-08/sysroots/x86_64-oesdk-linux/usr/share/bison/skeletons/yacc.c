...
#define YYCOPY_NEEDED 1]])[
     ...
     (!defined __cplusplus || (] b4_locations_if([[defined] b4_api_PREFIX      \
                                                  [LTYPE_IS_TRIVIAL &&         \
...
#define YY_LAC_DISCARD(Event)                                                  \
    ...
    {                                                                          \
        ...
        {                                                                      \
            if (yydebug)                                                       \
                YYFPRINTF(stderr,                                              \
                          "LAC: initial context discarded due to " Event       \
                          "\n");                                               \
        ...
        while (1)
        {
            ...
            }
            {
                yytype_int16 yystate;
                {
                    ...
                    yystate = ((yytype_int16)(0 <= yyi && yyi <= YYLAST &&
                                                      yycheck[yyi] == *yyesp
                                                  ? yytable[yyi]
                                                  : yydefgoto[yylhs]));
                }
                ...
                YYSIZE_T *yyes_capacity, ]])[yytype_int16 *yyssp, int yytoken)
{
        ...
        /* There are many possibilities here to consider:
           ...
           - Don't assume there isn't a lookahead just because this state is a
             ...
             action, or user semantic action that manipulated
           yychar.]b4_lac_if([[ In the first two cases, it might appear that the
           current syntax error should have been detected in the previous state
           when yy_lac was invoked.  However, at that time, there might have
           been a different syntax error that discarded a different initial
           context during error recovery, leaving behind the current
           lookahead.]], [[
            ...
            if (yyx != YYTERROR && yyx != YYUNDEFTOK)
              {
                    {
                        int yy_lac_status =
                            yy_lac(yyesa, yyes, yyes_capacity, yyssp, yyx);
                        if (yy_lac_status == 2)
                            return 2;
                        if (yy_lac_status == 1)
                            continue;
    ...
    case N:                                                                    \
        ...
        break
                default: /* Avoid compiler warnings. */
                    ...
                    YYCASE_(4, YY_("syntax error, unexpected %s, expecting %s "
                                   "or %s or %s"));
                    YYCASE_(5, YY_("syntax error, unexpected %s, expecting %s "
                                   "or %s or %s or %s"));
#undef YYCASE_
    }
                   ...
                   [[[yypstate *yyps]], [[yyps]]])[
{
            ...
            {
#ifndef yyoverflow
                /* If the stack was reallocated but the parse did not complete,
                   then the stack still needs to be freed.  */
