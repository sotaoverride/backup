...
struct bitmap_usage : public mem_usage
{
    ...
    bitmap_usage operator+(const bitmap_usage& second)
    {
        return bitmap_usage(m_allocated + second.m_allocated,
                            m_times + second.m_times, m_peak + second.m_peak,
    ...
    inline void dump(mem_location* loc, mem_usage& total) const
    {
        ...
        fprintf(stderr,
                ...
                get_percent(m_times, total.m_times), SIZE_AMOUNT(m_nsearches),
                SIZE_AMOUNT(m_search_iter), loc->m_ggc ? "ggc" : "heap");
    ...
    static inline void dump_header(const char* name)
    {
        fprintf(stderr, "%-48s %11s%16s%17s%12s%12s%10s\n", name, "Leak",
                "Peak", "Times", "N searches", "Search iter", "Type");
    }
...
/* DST = A | (B & ~C).  Return true if DST changes.  */
extern bool bitmap_ior_and_compl(bitmap DST, const_bitmap A, const_bitmap B,
                                 const_bitmap C);
...
static inline void bitmap_initialize(bitmap head,
...
static inline void bmp_iter_set_init(bitmap_iterator* bi, const_bitmap map,
...
static inline void bmp_iter_and_init(bitmap_iterator* bi, const_bitmap map1,
...
static inline void bmp_iter_and_compl_init(bitmap_iterator* bi,
...
static inline bool bmp_iter_set(bitmap_iterator* bi, unsigned* bit_no)
{
    ...
    *bit_no = ((*bit_no + BITMAP_WORD_BITS - 1) / BITMAP_WORD_BITS *
               BITMAP_WORD_BITS);
    bi->word_no++;
...
static inline bool bmp_iter_and(bitmap_iterator* bi, unsigned* bit_no)
{
    ...
    *bit_no = ((*bit_no + BITMAP_WORD_BITS - 1) / BITMAP_WORD_BITS *
    ...
    while (1)
    {
        ...
        {
            bi->bits =
                bi->elt1->bits[bi->word_no] & bi->elt2->bits[bi->word_no];
            if (bi->bits)
                goto next_bit;
            *bit_no += BITMAP_WORD_BITS;
            bi->word_no++;
