...
#define WI_UNARY_RESULT_VAR(RESULT, VAL, T, X)                                 \
    WI_UNARY_RESULT(T)                                                         \
    RESULT = wi::int_traits<WI_UNARY_RESULT(T)>::get_binary_result(X, X);      \
    HOST_WIDE_INT* VAL = RESULT.write_val()
...
struct binary_traits<T1, T2, FLEXIBLE_PRECISION, CONST_PRECISION>
{
    /* Spelled out explicitly (rather than through FIXED_WIDE_INT)
       so as not to confuse gengtype.  */
    typedef generic_wide_int<fixed_wide_int_storage<int_traits<T2>::precision>>
        result_type;
    typedef result_type operator_result;
    typedef bool predicate_result;
...
struct binary_traits<T1, T2, CONST_PRECISION, FLEXIBLE_PRECISION>
{
    /* Spelled out explicitly (rather than through FIXED_WIDE_INT)
       so as not to confuse gengtype.  */
    typedef generic_wide_int<fixed_wide_int_storage<int_traits<T1>::precision>>
        result_type;
    typedef result_type operator_result;
    typedef bool predicate_result;
    typedef result_type signed_shift_result_type;
...
inline ::wi::storage_ref::storage_ref(const HOST_WIDE_INT* val_in,
    ...
    val(val_in),
    len(len_in), precision(precision_in)
{
}
...
template <typename storage>
inline HOST_WIDE_INT generic_wide_int<storage>::to_shwi() const
{
    if (is_sign_extended)
        return this->get_val()[0];
    else
...
template <typename storage>
inline unsigned HOST_WIDE_INT generic_wide_int<storage>::to_uhwi() const
{
...
/* TODO: The compiler is half converted from using HOST_WIDE_INT to
   ...
   unconverted code.  */
template <typename storage>
inline HOST_WIDE_INT generic_wide_int<storage>::to_short_addr() const
{
    return this->get_val()[0];
}
...
template <typename storage>
inline HOST_WIDE_INT generic_wide_int<storage>::sign_mask() const
{
...
/* Return the signed value of the least-significant explicitly-encoded
   block.  */
template <typename storage>
inline HOST_WIDE_INT generic_wide_int<storage>::slow() const
{
    return this->get_val()[0];
}
...
/* Return the signed value of the most-significant explicitly-encoded
   block.  */
template <typename storage>
inline HOST_WIDE_INT generic_wide_int<storage>::shigh() const
{
...
template <typename storage>
inline unsigned HOST_WIDE_INT generic_wide_int<storage>::ulow() const
{
    return this->get_val()[0];
}
...
/* Return the unsigned value of the most-significant
   explicitly-encoded block.  */
template <typename storage>
inline unsigned HOST_WIDE_INT generic_wide_int<storage>::uhigh() const
{
    return this->get_val()[this->get_len() - 1];
}
...
template <typename storage>
template <typename T>
inline generic_wide_int<storage>& generic_wide_int<storage>::
    operator=(const T& x)
{
    storage::operator=(x);
    return *this;
...
template <typename storage>
inline unsigned int wi::int_traits<generic_wide_int<storage>>::get_precision(
...
template <typename storage>
inline wi::storage_ref wi::int_traits<generic_wide_int<storage>>::decompose(
    HOST_WIDE_INT*, unsigned int precision, const generic_wide_int<storage>& x)
{
...
template <bool SE, bool HDP>
inline wide_int_ref_storage<SE, HDP>::wide_int_ref_storage(
    const wi::storage_ref& x) :
    storage_ref(x)
{
}
...
template <typename T>
inline wide_int_ref_storage<SE, HDP>::wide_int_ref_storage(
    const T& x, unsigned int precision) :
...
namespace wi
{
unsigned int force_to_size(HOST_WIDE_INT*, const HOST_WIDE_INT*, unsigned int,
                           unsigned int, unsigned int, signop sgn);
unsigned int from_array(HOST_WIDE_INT*, const HOST_WIDE_INT*, unsigned int,
...
class GTY(()) wide_int_storage
{
  ...
  public:
    ...
    static wide_int from_array(const HOST_WIDE_INT*, unsigned int, unsigned int,
                               bool = true);
...
template <typename T>
inline wide_int_storage& wide_int_storage::operator=(const T& x)
{
    {
        STATIC_ASSERT(!wi::int_traits<T>::host_dependent_precision);
    }
...
inline void wide_int_storage::set_len(unsigned int l, bool is_sign_extended)
{
    ...
    if (!is_sign_extended && len * HOST_BITS_PER_WIDE_INT > precision)
        val[len - 1] =
            sext_hwi(val[len - 1], precision % HOST_BITS_PER_WIDE_INT);
}

/* Treat X as having signedness SGN and convert it to a PRECISION-bit
   number.  */
inline wide_int wide_int_storage::from(const wide_int_ref& x,
...
inline wide_int wide_int_storage::from_array(const HOST_WIDE_INT* val,
                                             unsigned int len,
                                             unsigned int precision,
                                             bool need_canon_p)
{
    wide_int result = wide_int::create(precision);
    result.set_len(
        wi::from_array(result.write_val(), val, len, precision, need_canon_p));
    return result;
}
...
template <typename T1, typename T2>
inline wide_int wi::int_traits<wide_int_storage>::get_binary_result(const T1& x,
                                                                    const T2& y)
{
...
class GTY(()) fixed_wide_int_storage
{
  private:
    HOST_WIDE_INT
    val[(N + HOST_BITS_PER_WIDE_INT + 1) / HOST_BITS_PER_WIDE_INT];
    unsigned int len;
...
template <int N>
inline unsigned int fixed_wide_int_storage<N>::get_precision() const
{
    return N;
}

template <int N>
inline const HOST_WIDE_INT* fixed_wide_int_storage<N>::get_val() const
{
    return val;
}

template <int N>
inline unsigned int fixed_wide_int_storage<N>::get_len() const
{
    return len;
}

template <int N>
inline HOST_WIDE_INT* fixed_wide_int_storage<N>::write_val()
{
    return val;
}

template <int N>
inline void fixed_wide_int_storage<N>::set_len(unsigned int l, bool)
{
    len = l;
...
template <int N>
inline FIXED_WIDE_INT(N) fixed_wide_int_storage<N>::from(const wide_int_ref& x,
                                                         signop sgn)
{
...
/* Create a FIXED_WIDE_INT (N) from the explicit block encoding given by
   ...
   trailing blocks.  */
template <int N>
inline FIXED_WIDE_INT(N) fixed_wide_int_storage<N>::from_array(
    const HOST_WIDE_INT* val, unsigned int len, bool need_canon_p)
{
    FIXED_WIDE_INT(N) result;
    result.set_len(
        wi::from_array(result.write_val(), val, len, N, need_canon_p));
    return result;
}

template <int N>
template <typename T1, typename T2>
inline FIXED_WIDE_INT(
    N) wi::int_traits<fixed_wide_int_storage<N>>::get_binary_result(const T1&,
                                                                    const T2&)
{
...
inline trailing_wide_int_storage::trailing_wide_int_storage(
    unsigned int precision, unsigned char* len, HOST_WIDE_INT* val) :
...
inline void trailing_wide_int_storage::set_len(unsigned int len,
                                               bool is_sign_extended)
{
    *m_len = len;
    if (!is_sign_extended && len * HOST_BITS_PER_WIDE_INT > m_precision)
        m_val[len - 1] =
            sext_hwi(m_val[len - 1], m_precision % HOST_BITS_PER_WIDE_INT);
}
...
/* Initialize the structure and record that all elements have precision
   PRECISION.  */
template <int N>
inline void trailing_wide_ints<N>::set_precision(unsigned int precision)
{
    m_precision = precision;
    m_max_len =
...
template <int N>
inline trailing_wide_int trailing_wide_ints<N>::operator[](unsigned int index)
{
...
template <int N>
inline typename trailing_wide_ints<N>::const_reference trailing_wide_ints<N>::
    operator[](unsigned int index) const
{
    return wi::storage_ref(&m_val[index * m_max_len], m_len[index],
                           m_precision);
}
...
template <int N>
inline size_t trailing_wide_ints<N>::extra_size(unsigned int precision)
{
...
template <typename T, bool signed_p>
inline wi::storage_ref wi::primitive_int_traits<T, signed_p>::decompose(
    HOST_WIDE_INT* scratch, unsigned int precision, T x)
{
    scratch[0] = x;
...
template <>
struct int_traits<int> : public primitive_int_traits<int, true>
{
};
...
template <>
struct int_traits<long> : public primitive_int_traits<long, true>
{
};
...
template <>
struct int_traits<long long> : public primitive_int_traits<long long, true>
{
};
...
inline wi::hwi_with_prec wi::uhwi(unsigned HOST_WIDE_INT val,
                                  unsigned int precision)
{
...
template <typename T>
inline wi::hwi_with_prec wi::ints_for<T, wi::VAR_PRECISION>::zero(const T& x)
{
...
inline wi::storage_ref wi::int_traits<wi::hwi_with_prec>::decompose(
    HOST_WIDE_INT* scratch, unsigned int precision, const wi::hwi_with_prec& x)
{
...
namespace wi
{
bool eq_p_large(const HOST_WIDE_INT*, unsigned int, const HOST_WIDE_INT*,
...
unsigned int zext_large(HOST_WIDE_INT*, const HOST_WIDE_INT*, unsigned int,
                        unsigned int, unsigned int);
unsigned int set_bit_large(HOST_WIDE_INT*, const HOST_WIDE_INT*, unsigned int,
                           unsigned int, unsigned int);
unsigned int lshift_large(HOST_WIDE_INT*, const HOST_WIDE_INT*, unsigned int,
                          unsigned int, unsigned int);
unsigned int lrshift_large(HOST_WIDE_INT*, const HOST_WIDE_INT*, unsigned int,
                           unsigned int, unsigned int, unsigned int);
unsigned int arshift_large(HOST_WIDE_INT*, const HOST_WIDE_INT*, unsigned int,
                           unsigned int, unsigned int, unsigned int);
...
template <typename T>
inline unsigned int wi::get_precision(const T& x)
{
...
template <typename T1, typename T2>
inline void wi::copy(T1& x, const T2& y)
{
...
template <typename T>
inline bool wi::fits_shwi_p(const T& x)
{
    WIDE_INT_REF_FOR(T) xi(x);
    return xi.len == 1;
}
...
/* Return true if X fits in an unsigned HOST_WIDE_INT with no loss of
   precision.  */
template <typename T>
inline bool wi::fits_uhwi_p(const T& x)
{
    WIDE_INT_REF_FOR(T) xi(x);
...
template <typename T>
inline bool wi::neg_p(const T& x, signop sgn)
{
    WIDE_INT_REF_FOR(T) xi(x);
    if (sgn == UNSIGNED)
        return false;
...
template <typename T>
inline HOST_WIDE_INT wi::sign_mask(const T& x)
{
    WIDE_INT_REF_FOR(T) xi(x);
    return xi.sign_mask();
}
...
template <typename T1, typename T2>
inline bool wi::eq_p(const T1& x, const T2& y)
{
...
/* Return true if X != Y.  X and Y must be binary-compatible.  */
template <typename T1, typename T2>
inline bool wi::ne_p(const T1& x, const T2& y)
{
    return !eq_p(x, y);
}
...
template <typename T1, typename T2>
inline bool wi::lts_p(const T1& x, const T2& y)
{
    unsigned int precision = get_binary_precision(x, y);
    WIDE_INT_REF_FOR(T1) xi(x, precision);
    WIDE_INT_REF_FOR(T2) yi(y, precision);
...
template <typename T1, typename T2>
inline bool wi::ltu_p(const T1& x, const T2& y)
{
    unsigned int precision = get_binary_precision(x, y);
    WIDE_INT_REF_FOR(T1) xi(x, precision);
    WIDE_INT_REF_FOR(T2) yi(y, precision);
...
template <typename T1, typename T2>
inline bool wi::lt_p(const T1& x, const T2& y, signop sgn)
{
    if (sgn == SIGNED)
        return lts_p(x, y);
    else
...
/* Return true if X <= Y when both are treated as signed values.  */
template <typename T1, typename T2>
inline bool wi::les_p(const T1& x, const T2& y)
{
    return !lts_p(y, x);
}
...
/* Return true if X <= Y when both are treated as unsigned values.  */
template <typename T1, typename T2>
inline bool wi::leu_p(const T1& x, const T2& y)
{
    return !ltu_p(y, x);
}
...
/* Return true if X <= Y.  Signedness of X and Y is indicated by SGN.  */
template <typename T1, typename T2>
inline bool wi::le_p(const T1& x, const T2& y, signop sgn)
{
    if (sgn == SIGNED)
        return les_p(x, y);
    else
...
/* Return true if X > Y when both are treated as signed values.  */
template <typename T1, typename T2>
inline bool wi::gts_p(const T1& x, const T2& y)
{
    return lts_p(y, x);
}
...
/* Return true if X > Y when both are treated as unsigned values.  */
template <typename T1, typename T2>
inline bool wi::gtu_p(const T1& x, const T2& y)
{
    return ltu_p(y, x);
}
...
/* Return true if X > Y.  Signedness of X and Y is indicated by SGN.  */
template <typename T1, typename T2>
inline bool wi::gt_p(const T1& x, const T2& y, signop sgn)
{
    if (sgn == SIGNED)
        return gts_p(x, y);
    else
...
/* Return true if X >= Y when both are treated as signed values.  */
template <typename T1, typename T2>
inline bool wi::ges_p(const T1& x, const T2& y)
{
    return !lts_p(x, y);
}
...
/* Return true if X >= Y when both are treated as unsigned values.  */
template <typename T1, typename T2>
inline bool wi::geu_p(const T1& x, const T2& y)
{
    return !ltu_p(x, y);
}
...
/* Return true if X >= Y.  Signedness of X and Y is indicated by SGN.  */
template <typename T1, typename T2>
inline bool wi::ge_p(const T1& x, const T2& y, signop sgn)
{
    if (sgn == SIGNED)
        return ges_p(x, y);
    else
...
/* Return -1 if X < Y, 0 if X == Y and 1 if X > Y.  Treat both X and Y
   as signed values.  */
template <typename T1, typename T2>
inline int wi::cmps(const T1& x, const T2& y)
{
    unsigned int precision = get_binary_precision(x, y);
    WIDE_INT_REF_FOR(T1) xi(x, precision);
    WIDE_INT_REF_FOR(T2) yi(y, precision);
...
/* Return -1 if X < Y, 0 if X == Y and 1 if X > Y.  Treat both X and Y
   as unsigned values.  */
template <typename T1, typename T2>
inline int wi::cmpu(const T1& x, const T2& y)
{
    unsigned int precision = get_binary_precision(x, y);
    WIDE_INT_REF_FOR(T1) xi(x, precision);
    WIDE_INT_REF_FOR(T2) yi(y, precision);
...
template <typename T1, typename T2>
inline int wi::cmp(const T1& x, const T2& y, signop sgn)
{
    if (sgn == SIGNED)
        return cmps(x, y);
    else
...
/* Return ~x.  */
template <typename T>
inline WI_UNARY_RESULT(T) wi::bit_not(const T& x)
{
...
/* Return -x.  */
template <typename T>
inline WI_UNARY_RESULT(T) wi::neg(const T& x)
{
    return sub(0, x);
}
...
/* Return -x.  Indicate in *OVERFLOW if performing the negation would
   cause an overflow.  */
template <typename T>
inline WI_UNARY_RESULT(T) wi::neg(const T& x, overflow_type* overflow)
{
...
template <typename T>
inline WI_UNARY_RESULT(T) wi::abs(const T& x)
{
...
template <typename T>
inline WI_UNARY_RESULT(T) wi::sext(const T& x, unsigned int offset)
{
    WI_UNARY_RESULT_VAR(result, val, T, x);
...
template <typename T>
inline WI_UNARY_RESULT(T) wi::zext(const T& x, unsigned int offset)
{
    WI_UNARY_RESULT_VAR(result, val, T, x);
    unsigned int precision = get_precision(result);
    WIDE_INT_REF_FOR(T) xi(x, precision);
...
/* Return the result of extending the low OFFSET bits of X according to
   signedness SGN.  */
template <typename T>
inline WI_UNARY_RESULT(T) wi::ext(const T& x, unsigned int offset, signop sgn)
{
...
template <typename T>
inline WI_UNARY_RESULT(T) wi::set_bit(const T& x, unsigned int bit)
{
    WI_UNARY_RESULT_VAR(result, val, T, x);
    unsigned int precision = get_precision(result);
    WIDE_INT_REF_FOR(T) xi(x, precision);
...
inline WI_BINARY_RESULT(T1, T2) wi::bit_and(const T1& x, const T2& y)
{
    ...
    if (__builtin_expect(xi.len + yi.len == 2, true))
    {
        ...
        result.set_len(
            and_large(val, xi.val, xi.len, yi.val, yi.len, precision),
            is_sign_extended);
    return result;
}
...
inline WI_BINARY_RESULT(T1, T2) wi::bit_and_not(const T1& x, const T2& y)
{
    ...
    if (__builtin_expect(xi.len + yi.len == 2, true))
    {
        ...
        result.set_len(
            and_not_large(val, xi.val, xi.len, yi.val, yi.len, precision),
            is_sign_extended);
    return result;
}
...
inline WI_BINARY_RESULT(T1, T2) wi::bit_or_not(const T1& x, const T2& y)
{
    ...
    if (__builtin_expect(xi.len + yi.len == 2, true))
    {
        ...
        result.set_len(
            or_not_large(val, xi.val, xi.len, yi.val, yi.len, precision),
            is_sign_extended);
    return result;
}
...
inline WI_BINARY_RESULT(T1, T2) wi::bit_xor(const T1& x, const T2& y)
{
    ...
    if (__builtin_expect(xi.len + yi.len == 2, true))
    {
        ...
        result.set_len(
            xor_large(val, xi.val, xi.len, yi.val, yi.len, precision),
            is_sign_extended);
    return result;
}
...
inline WI_BINARY_RESULT(T1, T2) wi::sub(const T1& x, const T2& y)
{
             ...
             __builtin_expect(xi.len + yi.len == 2, true))
    {
        ...
        val[1] = (HOST_WIDE_INT)resultl < 0 ? 0 : -1;
        result.set_len(
            1 + (((resultl ^ xl) & (xl ^ yl)) >> (HOST_BITS_PER_WIDE_INT - 1)));
    }
    else
...
template <typename T1, typename T2>
inline WI_BINARY_RESULT(T1, T2) wi::smul(const T1& x, const T2& y,
                                         overflow_type* overflow)
{
...
template <typename T1, typename T2>
inline WI_BINARY_RESULT(T1, T2) wi::umul(const T1& x, const T2& y,
                                         overflow_type* overflow)
{
...
template <typename T1, typename T2>
inline WI_BINARY_RESULT(T1, T2) wi::mul_high(const T1& x, const T2& y,
                                             signop sgn)
{
    WI_BINARY_RESULT_VAR(result, val, T1, x, T2, y);
    unsigned int precision = get_precision(result);
...
/* Return X / Y, rouding towards 0.  Treat X and Y as having the
   ...
   overflows.  */
template <typename T1, typename T2>
inline WI_BINARY_RESULT(T1, T2) wi::div_trunc(const T1& x, const T2& y,
                                              signop sgn,
                                              overflow_type* overflow)
{
    ...
    quotient.set_len(divmod_internal(quotient_val, 0, 0, xi.val, xi.len,
                                     precision, yi.val, yi.len, yi.precision,
                                     sgn, overflow));
    return quotient;
}
...
/* Return X / Y, rouding towards -inf.  Treat X and Y as having the
   ...
   overflows.  */
template <typename T1, typename T2>
inline WI_BINARY_RESULT(T1, T2) wi::div_floor(const T1& x, const T2& y,
                                              signop sgn,
                                              overflow_type* overflow)
{
    WI_BINARY_RESULT_VAR(quotient, quotient_val, T1, x, T2, y);
    ...
    quotient.set_len(divmod_internal(
        quotient_val, &remainder_len, remainder_val, xi.val, xi.len, precision,
...
/* Return X / Y, rouding towards +inf.  Treat X and Y as having the
   ...
   overflows.  */
template <typename T1, typename T2>
inline WI_BINARY_RESULT(T1, T2) wi::div_ceil(const T1& x, const T2& y,
                                             signop sgn,
                                             overflow_type* overflow)
{
    WI_BINARY_RESULT_VAR(quotient, quotient_val, T1, x, T2, y);
...
template <typename T1, typename T2>
inline WI_BINARY_RESULT(T1, T2) wi::div_round(const T1& x, const T2& y,
                                              signop sgn,
                                              overflow_type* overflow)
{
    WI_BINARY_RESULT_VAR(quotient, quotient_val, T1, x, T2, y);
...
template <typename T1, typename T2>
inline WI_BINARY_RESULT(T1, T2) wi::divmod_trunc(const T1& x, const T2& y,
                                                 signop sgn,
                                                 WI_BINARY_RESULT(T1, T2) *
                                                     remainder_ptr)
{
...
template <typename T1, typename T2>
inline WI_BINARY_RESULT(T1, T2) wi::mod_trunc(const T1& x, const T2& y,
                                              signop sgn,
                                              overflow_type* overflow)
{
    WI_BINARY_RESULT_VAR(remainder, remainder_val, T1, x, T2, y);
...
/* Compute X / Y, rouding towards -inf, and return the remainder.
   ...
   in *OVERFLOW if the division overflows.  */
template <typename T1, typename T2>
inline WI_BINARY_RESULT(T1, T2) wi::mod_floor(const T1& x, const T2& y,
                                              signop sgn,
                                              overflow_type* overflow)
{
    WI_BINARY_RESULT_VAR(quotient, quotient_val, T1, x, T2, y);
...
/* Compute X / Y, rouding towards +inf, and return the remainder.
   ...
   in *OVERFLOW if the division overflows.  */
template <typename T1, typename T2>
inline WI_BINARY_RESULT(T1, T2) wi::mod_ceil(const T1& x, const T2& y,
                                             signop sgn,
                                             overflow_type* overflow)
{
    WI_BINARY_RESULT_VAR(quotient, quotient_val, T1, x, T2, y);
...
template <typename T1, typename T2>
inline WI_BINARY_RESULT(T1, T2) wi::mod_round(const T1& x, const T2& y,
                                              signop sgn,
                                              overflow_type* overflow)
{
    WI_BINARY_RESULT_VAR(quotient, quotient_val, T1, x, T2, y);
...
/* Return X << Y.  Return 0 if Y is greater than or equal to
   the precision of X.  */
template <typename T1, typename T2>
inline WI_UNARY_RESULT(T1) wi::lshift(const T1& x, const T2& y)
{
       ...
       and sub-HWI integers inline.  */
        if (STATIC_CONSTANT_P(xi.precision > HOST_BITS_PER_WIDE_INT)
                ? (STATIC_CONSTANT_P(shift < HOST_BITS_PER_WIDE_INT - 1) &&
                   ...
                   xi.val[0] <=
                       (HOST_WIDE_INT)(
                           (unsigned HOST_WIDE_INT)HOST_WIDE_INT_MAX >> shift))
...
template <typename T1, typename T2>
inline WI_UNARY_RESULT(T1) wi::lrshift(const T1& x, const T2& y)
{
    WI_UNARY_RESULT_VAR(result, val, T1, x);
    ...
    else
    {
       ...
       and sub-HWI integers inline.  */
        if (STATIC_CONSTANT_P(xi.precision > HOST_BITS_PER_WIDE_INT)
                ? (shift < HOST_BITS_PER_WIDE_INT && xi.len == 1 &&
                   xi.val[0] >= 0)
...
template <typename T1, typename T2>
inline WI_UNARY_RESULT(T1) wi::arshift(const T1& x, const T2& y)
{
    WI_UNARY_RESULT_VAR(result, val, T1, x);
    /* Do things in the precision of the input rather than the output,
       since the result can be no larger than that.  */
...
template <typename T>
inline unsigned HOST_WIDE_INT wi::extract_uhwi(const T& x, unsigned int bitpos,
                                               unsigned int width)
{
...
template <typename T>
inline unsigned int wi::min_precision(const T& x, signop sgn)
{
    if (sgn == SIGNED)
...
template <typename T>
void gt_ggc_mx(generic_wide_int<T>*)
{
}

template <typename T>
void gt_pch_nx(generic_wide_int<T>*)
{
}

template <typename T>
void gt_pch_nx(generic_wide_int<T>*, void (*)(void*, void*), void*)
{
}
...
unsigned int shifted_mask(HOST_WIDE_INT*, unsigned int, unsigned int, bool,
                          unsigned int);
unsigned int from_array(HOST_WIDE_INT*, const HOST_WIDE_INT*, unsigned int,
                        unsigned int, bool);
} // namespace wi
...
inline wide_int wi::mask(unsigned int width, bool negate_p,
                         unsigned int precision)
{
    wide_int result = wide_int::create(precision);
...
inline wide_int wi::shifted_mask(unsigned int start, unsigned int width,
                                 bool negate_p, unsigned int precision)
{
    wide_int result = wide_int::create(precision);
    result.set_len(
        shifted_mask(result.write_val(), start, width, negate_p, precision));
    return result;
}
...
/* Return an integer of type T in which the low WIDTH bits are set
   and the other bits are clear, or the inverse if NEGATE_P.  */
template <typename T>
inline T wi::mask(unsigned int width, bool negate_p)
{
...
/* Return an integer of type T in which bit BIT is set and all the
   others are clear.  */
template <typename T>
inline T wi::set_bit_in_zero(unsigned int bit)
{
