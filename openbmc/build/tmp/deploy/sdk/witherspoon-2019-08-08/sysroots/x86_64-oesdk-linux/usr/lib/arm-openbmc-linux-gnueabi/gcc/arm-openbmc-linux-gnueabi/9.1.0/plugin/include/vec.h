...
inline unsigned vec_prefix::calculate_allocation(vec_prefix* pfx,
...
struct va_heap
{
    ...
    template <typename T>
    static void reserve(vec<T, va_heap, vl_embed>*&, unsigned,
...
struct va_gc
{
    ...
    static void reserve(vec<T, A, vl_embed>*&, unsigned,
                        bool CXX_MEM_STAT_INFO);
...
template <typename T, typename A>
inline void va_gc::release(vec<T, A, vl_embed>*& v)
{
    if (v)
        ::ggc_free(v);
    v = NULL;
...
template <typename T, typename A>
void va_gc::reserve(vec<T, A, vl_embed>*& v, unsigned reserve,
                    bool exact MEM_STAT_DECL)
{
    unsigned alloc =
        vec_prefix::calculate_allocation(v ? &v->m_vecpfx : 0, reserve, exact);
...
template <typename T>
inline void vec_default_construct(T* dst, unsigned n)
{
...
template <typename T>
inline void vec_copy_construct(T* dst, const T* src, unsigned n)
{
    for (; n; ++dst, ++src, --n)
...
template <typename T, typename A>
inline unsigned vec_safe_length(const vec<T, A, vl_embed>* v)
{
    return v ? v->length() : 0;
}
...
template <typename T, typename A>
inline T* vec_safe_address(vec<T, A, vl_embed>* v)
{
...
template <typename T, typename A>
inline bool vec_safe_is_empty(vec<T, A, vl_embed>* v)
{
...
template <typename T, typename A>
inline bool vec_safe_reserve_exact(vec<T, A, vl_embed>*& v,
...
template <typename T, typename A>
inline void vec_alloc(vec<T, A, vl_embed>*& v,
                      unsigned nelems CXX_MEM_STAT_INFO)
{
    v = NULL;
...
template <typename T, typename A>
inline void vec_free(vec<T, A, vl_embed>*& v)
{
    A::release(v);
}
...
template <typename T, typename A>
inline void vec_safe_grow(vec<T, A, vl_embed>*& v,
...
template <typename T, typename A>
inline void vec_safe_grow_cleared(vec<T, A, vl_embed>*& v,
                                  unsigned len CXX_MEM_STAT_INFO)
{
    unsigned oldlen = vec_safe_length(v);
...
template <typename T>
inline void vec_safe_grow_cleared(vec<T, va_heap, vl_ptr>*& v,
                                  unsigned len CXX_MEM_STAT_INFO)
{
...
template <typename T, typename A>
inline void vec_safe_insert(vec<T, A, vl_embed>*& v, unsigned ix,
                            ...
                            const vec<T, A, vl_embed>* src CXX_MEM_STAT_INFO)
{
    ...
    if (src_len)
    {
        vec_safe_reserve_exact(dst,
                               vec_safe_length(dst) + src_len PASS_MEM_STAT);
        dst->splice(*src);
    }
}
...
template <typename T, typename A>
inline const T& vec<T, A, vl_embed>::operator[](unsigned ix) const
{
...
template <typename T, typename A>
inline T& vec<T, A, vl_embed>::operator[](unsigned ix)
{
    gcc_checking_assert(ix < m_vecpfx.m_num);
    return m_vecdata[ix];
}
...
template <typename T, typename A>
inline T& vec<T, A, vl_embed>::last(void)
{
...
template <typename T, typename A>
inline bool vec<T, A, vl_embed>::space(unsigned nelems) const
{
...
template <typename T, typename A>
inline T* vec<T, A, vl_embed>::quick_push(const T& obj)
{
...
template <typename T, typename A>
inline T& vec<T, A, vl_embed>::pop(void)
{
...
template <typename T, typename A>
inline void vec<T, A, vl_embed>::truncate(unsigned size)
{
...
template <typename T, typename A>
inline void vec<T, A, vl_embed>::ordered_remove(unsigned ix)
{
    gcc_checking_assert(ix < length());
    T* slot = &m_vecdata[ix];
...
#define VEC_ORDERED_REMOVE_IF_FROM_TO(vec, read_index, write_index, elem_ptr,  \
                                      start, end, cond)                        \
    {                                                                          \
...
#define VEC_ORDERED_REMOVE_IF(vec, read_index, write_index, elem_ptr, cond)    \
    VEC_ORDERED_REMOVE_IF_FROM_TO((vec), read_index, write_index, elem_ptr, 0, \
                                  (vec).length(), (cond))
...
template <typename T, typename A>
inline void vec<T, A, vl_embed>::unordered_remove(unsigned ix)
{
    gcc_checking_assert(ix < length());
...
template <typename T, typename A>
inline T* vec<T, A, vl_embed>::bsearch(const void* key,
...
template <typename T, typename A>
inline bool vec<T, A, vl_embed>::contains(const T& search) const
{
...
template <typename T, typename A>
inline size_t vec<T, A, vl_embed>::embedded_size(unsigned alloc)
{
...
template <typename T, typename A>
inline void vec<T, A, vl_embed>::embedded_init(unsigned alloc, unsigned num,
                                               unsigned aut)
{
...
template <typename T, typename A>
inline void vec<T, A, vl_embed>::quick_grow(unsigned len)
{
...
template <typename T, typename A>
inline void vec<T, A, vl_embed>::quick_grow_cleared(unsigned len)
{
...
template <typename T>
void gt_ggc_mx(vec<T, va_gc_atomic, vl_embed>* v ATTRIBUTE_UNUSED)
{
...
template <typename T>
inline void vec_alloc(vec<T>*& v, unsigned nelems CXX_MEM_STAT_INFO)
{
    v = new vec<T>;
...
template <typename T>
inline void vec_check_alloc(vec<T, va_heap>*& vec,
                            unsigned nelems CXX_MEM_STAT_INFO)
{
    if (!vec)
...
template <typename T>
inline bool vec<T, va_heap, vl_ptr>::iterate(unsigned ix, T* ptr) const
{
    if (m_vec)
...
#define FOR_EACH_VEC_ELT_REVERSE(V, I, P)                                      \
    for (I = (V).length() - 1; (V).iterate((I), &(P)); (I)--)
...
template <typename T>
inline bool vec<T, va_heap, vl_ptr>::reserve(unsigned nelems,
                                             bool exact MEM_STAT_DECL)
{
    if (space(nelems))
        return false;
    ...
    /* For now play a game with va_heap::reserve to hide our auto storage if
       any, this is necessary because it doesn't have enough information to know
       the embedded vector is in auto storage, and so should not be freed.  */
...
template <typename T>
inline void vec<T, va_heap, vl_ptr>::create(unsigned nelems MEM_STAT_DECL)
{
    m_vec = NULL;
    if (nelems > 0)
...
template <typename T>
inline void vec<T, va_heap, vl_ptr>::release(void)
{
    if (!m_vec)
        return;
...
template <typename T>
inline void vec<T, va_heap, vl_ptr>::safe_splice(
    const vec<T, va_heap, vl_ptr>& src MEM_STAT_DECL)
{
    if (src.length())
    {
...
template <typename T>
inline T* vec<T, va_heap, vl_ptr>::quick_push(const T& obj)
{
...
template <typename T>
inline T* vec<T, va_heap, vl_ptr>::safe_push(const T& obj MEM_STAT_DECL)
{
...
template <typename T>
inline T& vec<T, va_heap, vl_ptr>::pop(void)
{
    return m_vec->pop();
}
...
template <typename T>
inline void vec<T, va_heap, vl_ptr>::truncate(unsigned size)
{
    if (m_vec)
        m_vec->truncate(size);
    else
...
template <typename T>
inline void vec<T, va_heap, vl_ptr>::safe_grow(unsigned len MEM_STAT_DECL)
{
    unsigned oldlen = length();
...
template <typename T>
inline void vec<T, va_heap, vl_ptr>::quick_grow(unsigned len)
{
...
template <typename T>
inline void vec<T, va_heap, vl_ptr>::quick_grow_cleared(unsigned len)
{
    gcc_checking_assert(m_vec);
...
template <typename T>
inline void vec<T, va_heap, vl_ptr>::quick_insert(unsigned ix, const T& obj)
{
...
template <typename T>
inline void vec<T, va_heap, vl_ptr>::safe_insert(unsigned ix,
                                                 const T& obj MEM_STAT_DECL)
{
    reserve(1, false PASS_MEM_STAT);
    quick_insert(ix, obj);
}
...
template <typename T>
inline void vec<T, va_heap, vl_ptr>::ordered_remove(unsigned ix)
{
...
template <typename T>
inline void vec<T, va_heap, vl_ptr>::unordered_remove(unsigned ix)
{
...
template <typename T>
inline void vec<T, va_heap, vl_ptr>::block_remove(unsigned ix, unsigned len)
{
...
template <typename T>
inline T* vec<T, va_heap, vl_ptr>::bsearch(const void* key,
...
template <typename T>
inline unsigned vec<T, va_heap, vl_ptr>::lower_bound(
    T obj, bool (*lessthan)(const T&, const T&)) const
{
...
template <typename T>
inline bool vec<T, va_heap, vl_ptr>::contains(const T& search) const
{
...
template <typename T>
inline void vec<T, va_heap, vl_ptr>::reverse(void)
{
    unsigned l = length();
    T* ptr = address();
...
template <typename T>
inline bool vec<T, va_heap, vl_ptr>::using_auto_storage() const
{
...
template <typename T>
inline void release_vec_vec(vec<vec<T>>& vec)
{
