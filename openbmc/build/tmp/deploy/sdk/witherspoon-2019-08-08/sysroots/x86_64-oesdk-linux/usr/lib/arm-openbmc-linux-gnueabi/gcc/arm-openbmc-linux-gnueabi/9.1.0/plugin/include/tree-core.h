...
enum omp_clause_defaultmap_kind
{
    ...
    OMP_CLAUSE_DEFAULTMAP_TOFROM =
        4 * (OMP_CLAUSE_DEFAULTMAP_CATEGORY_MASK + 1),
...
#define NUM_FLOATN_NX_TYPES                                                    \
    (TI_FLOATN_NX_TYPE_LAST - TI_FLOATN_NX_TYPE_FIRST + 1)

    /* Put the complex types after their component types, so that in
       (sequential) tree streaming we can assert that their component types have
       already been handled (see tree-streamer.c:record_common_node).  */
...
struct GTY(()) tree_base
{
    ...
    union
    {
        ...
        struct
        {
            ...
            /* This field is only used with TREE_TYPE nodes; the only reason it
           is present in tree_base instead of tree_type is to save space.  The
           size of the field must be large enough to hold addr_space_t values.
         */
...
struct GTY(()) tree_omp_clause
{
    ...
    tree
        GTY((length("omp_clause_num_ops[OMP_CLAUSE_CODE ((tree)&%h)]"))) ops[1];
};
...
struct GTY((chain_next("%h.next"),
            chain_prev("%h.prev"))) tree_statement_list_node
{
           ...
           variable_size)) tree_node
{
    ...
    struct tree_type_non_common
        GTY((tag("TS_TYPE_NON_COMMON"))) type_non_common;
...
struct attribute_spec
{
    ...
    tree (*handler)(tree* node, tree name, tree args, int flags,
                    bool* no_add_attrs);
