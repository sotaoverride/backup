...
#define OBJC_IS_CXX_KEYWORD(rid)                                               \
    (rid == RID_CLASS || rid == RID_SYNCHRONIZED || rid == RID_PUBLIC ||       \
     rid == RID_PROTECTED || rid == RID_PRIVATE || rid == RID_TRY ||           \
     rid == RID_THROW || rid == RID_CATCH)
...
#define abi_version_crosses(N)                                                 \
    (abi_version_at_least(N) !=                                                \
     (warn_abi_version == 0 || warn_abi_version >= (N)))
...
extern void check_function_arguments_recurse(void (*)(void*, tree,
...
extern tree c_common_fixed_point_type_for_size(unsigned int, unsigned int, int,
                                               int);
...
extern tree shorten_binary_op(tree result_type, tree op0, tree op1,
                              bool bitwise);
...
extern bool vector_types_convertible_p(const_tree t1, const_tree t2,
                                       bool emit_lax_note);
extern tree c_build_vec_perm_expr(location_t, tree, tree, tree, bool = true);
extern tree c_build_vec_convert(location_t, tree, location_t, tree,
                                bool = true);
...
extern bool c_cpp_diagnostic(cpp_reader*, enum cpp_diagnostic_level,
                             ...
                             const char*, va_list*) ATTRIBUTE_GCC_DIAG(5, 0);
...
extern tree c_finish_omp_for(location_t, enum tree_code, tree, tree, tree, tree,
                             tree, tree, tree, bool);
...
extern void invalid_array_size_error(location_t, cst_size_error, const_tree,
                                     const_tree);
...
extern void warn_tautological_cmp(const op_location_t&, enum tree_code, tree,
                                  tree);
...
extern void warn_about_parentheses(location_t, enum tree_code, enum tree_code,
                                   tree, enum tree_code, tree);
extern void warn_for_unused_label(tree label);
extern void warn_for_div_by_zero(location_t, tree divisor);
extern void warn_for_memset(location_t, tree, tree, int);
extern void warn_for_sign_compare(location_t, tree orig_op0, tree orig_op1,
                                  tree op0, tree op1, tree result_type,
...
extern void warn_for_multistatement_macros(location_t, location_t, location_t,
                                           enum rid);
...
extern enum flt_eval_method excess_precision_mode_join(enum flt_eval_method,
