...
inline typename tools::promote_args<T>::type
    round(const T& v, const Policy& pol, const mpl::false_)
{
    BOOST_MATH_STD_USING
    typedef typename tools::promote_args<T>::type result_type;
    if (!(boost::math::isfinite)(v))
        return policies::raise_rounding_error("boost::math::round<%1%>(%1%)", 0,
...
template <class T, class Policy>
inline typename tools::promote_args<T>::type round(const T& v, const Policy&,
                                                   const mpl::true_)
{
    return v;
}
...
inline int iround(const T& v, const Policy& pol)
{
    ...
    if ((r > (std::numeric_limits<int>::max)()) ||
        ...
        return static_cast<int>(policies::raise_rounding_error(
            "boost::math::iround<%1%>(%1%)", 0, v, 0, pol));
...
inline long lround(const T& v, const Policy& pol)
{
    ...
    if ((r > (std::numeric_limits<long>::max)()) ||
        ...
        return static_cast<long int>(policies::raise_rounding_error(
            "boost::math::lround<%1%>(%1%)", 0, v, 0L, pol));
