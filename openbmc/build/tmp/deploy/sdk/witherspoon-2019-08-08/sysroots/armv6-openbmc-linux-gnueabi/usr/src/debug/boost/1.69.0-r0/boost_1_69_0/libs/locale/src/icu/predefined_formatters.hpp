...
class icu_formatters_cache : public std::locale::facet
{
  public:
    ...
    icu_formatters_cache(icu::Locale const& locale) : locale_(locale)
    {
        ...
        static const icu::DateFormat::EStyle styles[4] = {
            icu::DateFormat::kShort, icu::DateFormat::kMedium,
            icu::DateFormat::kLong, icu::DateFormat::kFull};

        for (int i = 0; i < 4; i++)
        {
            hold_ptr<icu::DateFormat> fmt(
                icu::DateFormat::createDateInstance(styles[i], locale));
            icu::SimpleDateFormat* sfmt =
                dynamic_cast<icu::SimpleDateFormat*>(fmt.get());
            if (sfmt)
            {
        ...
        {
            hold_ptr<icu::DateFormat> fmt(
                icu::DateFormat::createTimeInstance(styles[i], locale));
            icu::SimpleDateFormat* sfmt =
                dynamic_cast<icu::SimpleDateFormat*>(fmt.get());
            if (sfmt)
            {
        ...
        for (int i = 0; i < 4; i++)
        {
            ...
            {
                hold_ptr<icu::DateFormat> fmt(
                    icu::DateFormat::createDateTimeInstance(styles[i],
                                                            styles[j], locale));
                icu::SimpleDateFormat* sfmt =
                    dynamic_cast<icu::SimpleDateFormat*>(fmt.get());
                if (sfmt)
    ...
    icu::NumberFormat* number_format(fmt_type type) const
    {
        ...
        switch (type)
        {
            ...
            case fmt_sci:
                ap.reset(
                    icu::NumberFormat::createScientificInstance(locale_, err));
                break;
#if U_ICU_VERSION_MAJOR_NUM * 100 + U_ICU_VERSION_MINOR_NUM >= 402
#if U_ICU_VERSION_MAJOR_NUM * 100 + U_ICU_VERSION_MINOR_NUM >= 408
            case fmt_curr_nat:
                ap.reset(icu::NumberFormat::createInstance(locale_,
                                                           UNUM_CURRENCY, err));
                break;
            case fmt_curr_iso:
                ap.reset(icu::NumberFormat::createInstance(
                    locale_, UNUM_CURRENCY_ISO, err));
                break;
#else
            case fmt_curr_nat:
                ap.reset(icu::NumberFormat::createInstance(
                    locale_, icu::NumberFormat::kCurrencyStyle, err));
                break;
            case fmt_curr_iso:
                ap.reset(icu::NumberFormat::createInstance(
                    locale_, icu::NumberFormat::kIsoCurrencyStyle, err));
                break;
#endif
#else
            case fmt_curr_nat:
            case fmt_curr_iso:
                ap.reset(
                    icu::NumberFormat::createCurrencyInstance(locale_, err));
                break;
#endif
            case fmt_per:
                ap.reset(
                    icu::NumberFormat::createPercentInstance(locale_, err));
                break;
            case fmt_spell:
                ap.reset(new icu::RuleBasedNumberFormat(icu::URBNF_SPELLOUT,
                                                        locale_, err));
                break;
            case fmt_ord:
                ap.reset(new icu::RuleBasedNumberFormat(icu::URBNF_ORDINAL,
                                                        locale_, err));
                break;
            default:
    ...
    icu::SimpleDateFormat* date_formatter() const
    {
        ...
        hold_ptr<icu::DateFormat> fmt(icu::DateFormat::createDateTimeInstance(
            icu::DateFormat::kMedium, icu::DateFormat::kMedium, locale_));
  ...
  private:
    mutable boost::thread_specific_ptr<icu::NumberFormat>
        number_format_[fmt_count];
