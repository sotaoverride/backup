...
static const gchar* gvdb_table_item_get_key(GvdbTable* file,
                                            ...
                                            gsize* size)
{
    ...
    *size = guint16_from_le(item->key_size);
    ...
    if
        G_UNLIKELY(start > end || end > file->size)
    return NULL;

    return file->data + start;
}

static gconstpointer gvdb_table_dereference(GvdbTable* file,
                                            const struct gvdb_pointer* pointer,
                                            gint alignment, gsize* size)
{
    guint32 start, end;
    ...
    if
        G_UNLIKELY(start > end || end > file->size || start & (alignment - 1))
    return NULL;
...
static void gvdb_table_setup_root(GvdbTable* file,
                                  const struct gvdb_pointer* pointer)
{
    ...
    if
        G_UNLIKELY(header == NULL || size < sizeof *header)
    return;
    ...
    if
        G_UNLIKELY(n_bloom_words * sizeof(guint32_le) > size)
    return;
    ...
    if
        G_UNLIKELY(n_buckets > G_MAXUINT / sizeof(guint32_le) ||
    ...
    if
        G_UNLIKELY(size % sizeof(struct gvdb_hash_item))
    return;
 ...
 **/
GvdbTable* gvdb_table_new_from_bytes(GBytes* bytes, gboolean trusted,
                                     GError** error)
{
 ...
 * Returns: a new #GvdbTable
 **/
GvdbTable* gvdb_table_new(const gchar* filename, gboolean trusted,
                          GError** error)
{
    GMappedFile* mapped;
    GvdbTable* table;
...
static gboolean gvdb_table_check_name(GvdbTable* file,
                                      struct gvdb_hash_item* item,
                                      const gchar* key, guint key_length)
{
    ...
    if
        G_UNLIKELY(this_key == NULL || this_size > key_length)
    return FALSE;

    key_length -= this_size;

    if
        G_UNLIKELY(memcmp(this_key, key + key_length, this_size) != 0)
    return FALSE;
    ...
    if
        G_LIKELY(parent < file->n_hash_items && this_size > 0)
    return gvdb_table_check_name(file, &file->hash_items[parent], key,
                                 key_length);
...
static const struct gvdb_hash_item*
    gvdb_table_lookup(GvdbTable* file, const gchar* key, gchar type)
{
    ...
    if
        G_UNLIKELY(file->n_buckets == 0 || file->n_hash_items == 0)
    return NULL;
    ...
    if (bucket == file->n_buckets - 1 ||
        (lastno = guint32_from_le(file->hash_buckets[bucket + 1])) >
            file->n_hash_items)
        lastno = file->n_hash_items;

    while
        G_LIKELY(itemno < lastno)
        {
            struct gvdb_hash_item* item = &file->hash_items[itemno];

            if (hash_value == guint32_from_le(item->hash_value))
                if
                    G_LIKELY(gvdb_table_check_name(file, item, key, key_length))
            if
                G_LIKELY(item->type == type)
            return item;
...
static gboolean gvdb_table_list_from_item(GvdbTable* table,
                                          const struct gvdb_hash_item* item,
 ...
 *
 * Returns: (array length=length): a %NULL-terminated list of strings, of length
 *@length
 **/
gchar** gvdb_table_get_names(GvdbTable* table, gsize* length)
{
    gchar** names;
    gint n_names;
    gint filled;
     ...
     */
    if
        G_UNLIKELY(total != n_names)
        {
            GPtrArray* fixed_names;

            fixed_names =
                g_ptr_array_sized_new(n_names + 1 /* NULL terminator */);
            for (i = 0; i < n_names; i++)
                if (names[i] != NULL)
...
static GVariant* gvdb_table_value_from_item(GvdbTable* table,
                                            const struct gvdb_hash_item* item)
{
    ...
    if
        G_UNLIKELY(data == NULL)
    return NULL;

    bytes = g_bytes_new_from_bytes(table->bytes, ((gchar*)data) - table->data,
                                   size);
    variant =
        g_variant_new_from_bytes(G_VARIANT_TYPE_VARIANT, bytes, table->trusted);
 ...
 * Frees @file.
 **/
void gvdb_table_free(GvdbTable* file)
{
    g_bytes_unref(file->bytes);
    g_slice_free(GvdbTable, file);
}
