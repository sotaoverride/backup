...
netsnmp_feature_child_of(snmp_agent, libnetsnmpagent) netsnmp_feature_child_of(
    agent_debugging_utilities, libnetsnmpagent)

    netsnmp_feature_child_of(
        allocate_globalcacheid,
        snmp_agent) netsnmp_feature_child_of(free_agent_snmp_session_by_session,
                                             snmp_agent)
        netsnmp_feature_child_of(
            check_all_requests_error,
            snmp_agent) netsnmp_feature_child_of(check_requests_error,
                                                 snmp_agent)
            netsnmp_feature_child_of(
                request_set_error_idx,
                snmp_agent) netsnmp_feature_child_of(set_agent_uptime,
                                                     snmp_agent)
                netsnmp_feature_child_of(agent_check_and_process, snmp_agent)

                    netsnmp_feature_child_of(dump_sess_list,
                                             agent_debugging_utilities)

                        netsnmp_feature_child_of(agent_remove_list_data,
                                                 netsnmp_unused)
                            netsnmp_feature_child_of(set_all_requests_error,
                                                     netsnmp_unused)
                                netsnmp_feature_child_of(addrcache_age,
                                                         netsnmp_unused)
                                    netsnmp_feature_child_of(
                                        delete_subtree_cache, netsnmp_unused)

#ifndef NETSNMP_NO_PDU_STATS

                                        static netsnmp_container* _pdu_stats =
                                            NULL;
...
static void _pdu_stats_init(void)
{
    static int done = 0;
    ...
    _pdu_stats_threshold = netsnmp_ds_get_int(
        NETSNMP_DS_APPLICATION_ID, NETSNMP_DS_AGENT_PDU_STATS_THRESHOLD);
...
static void _pdu_stats_shutdown(void)
{
...
static void _dump_pdu_stats(void)
{
    int x = 0;
    struct tm* tm;
    char timestr[40];
    ...
    for (; x < CONTAINER_SIZE(_pdu_stats); ++x)
    {
        ...
        else if (strftime(timestr, sizeof(timestr), "%m/%d/%Y %H:%M:%S", tm) ==
                 0)
            sprintf(timestr, "UNKNOWN");

        DEBUGMSGT_NC(("9:stats:pdu", "[%d] %ld ms, %s\n", x,
                      entry->processing_time, timestr));
        response = entry->pdu;
        ...
        else
        {
            ...
            if (response->errindex != 0)
            {
                ...
                if (vars)
                {
                    DEBUGMSGOID(("9:stats:pdu", vars->name, vars->name_length));
                }
                DEBUGMSG_NC(("9:stats:pdu", "\n"));
            }
        }
...
NETSNMP_INLINE void netsnmp_agent_add_list_data(netsnmp_agent_request_info* ari,
...
agent_set_cache* save_set_cache(netsnmp_agent_session* asp)
{
    ...
    ptr->saved_vars =
        asp->pdu->variables; /* requests contains pointers to variables */
...
NETSNMP_STATIC_INLINE void _reorder_getbulk(netsnmp_agent_session* asp)
{
    ...
    for (i = 0; i < repeats; i++)
    {
        ...
        {
            all_eoMib = 1;
            for (j = 1, prev = asp->bulkcache[i]; j < r;
...
NETSNMP_STATIC_INLINE void _fix_endofmibview(netsnmp_agent_session* asp)
{
    ...
    for (vb = asp->pdu->variables, ovb = asp->orig_pdu->variables; vb && ovb;
         vb = vb->next_variable, ovb = ovb->next_variable)
    {
...
int agent_check_and_process(int block)
{
    ...
    if (count > 0)
    {
         ...
         */
#ifndef NETSNMP_FEATURE_REMOVE_FD_EVENT_MANAGER
        netsnmp_dispatch_external_events2(&count, &readfds, &writefds,
                                          &exceptfds);
#endif /* NETSNMP_FEATURE_REMOVE_FD_EVENT_MANAGER */
...
int netsnmp_addrcache_add(const char* addr)
{
    ...
    if ((-1 == rc) && (NULL != addr))
    {
        ...
        { /* Otherwise, replace oldest entry */
...
int netsnmp_agent_check_packet(netsnmp_session* session,
                               ...
                               void* transport_data, int transport_data_length)
{
    ...
    name = netsnmp_ds_get_string(NETSNMP_DS_LIBRARY_ID, NETSNMP_DS_LIB_APPTYPE);
     ...
     */
    if (not_log_connection == SNMPERR_GENERR)
        not_log_connection = 0;
#endif
    ...
    else
    {
        ...
        if (0 == strncmp(addr_string, "callback", 8))
            ;
        else if (hosts_ctl(name, STRING_UNKNOWN, STRING_UNKNOWN,
                           STRING_UNKNOWN))
        {
            if (!not_log_connection)
            {
...
int netsnmp_register_agent_nsap(netsnmp_transport* t)
{
    ...
    s->flags =
        netsnmp_ds_get_int(NETSNMP_DS_APPLICATION_ID, NETSNMP_DS_AGENT_FLAGS);
    ...
    sp = snmp_add(s, t, netsnmp_agent_check_packet, netsnmp_agent_check_parse);
    if (sp == NULL)
    {
        SNMP_FREE(s);
        SNMP_FREE(n);
...
int init_master_agent(void)
{
    ...
    if (cptr)
    {
        ...
        if (!buf)
        {
            snmp_log(LOG_ERR, "Error processing transport \"%s\"\n", cptr);
            return 1;
        }
    }
    else
...
netsnmp_agent_session* init_agent_snmp_session(netsnmp_session* session,
                                               netsnmp_pdu* pdu)
{
    netsnmp_agent_session* asp =
        (netsnmp_agent_session*)calloc(1, sizeof(netsnmp_agent_session));
    ...
    DEBUGMSGTL(
        ("verbose:asp", "asp %p reqinfo %p created\n", asp, asp->reqinfo));
...
int netsnmp_remove_delegated_requests_for_session(netsnmp_session* sess)
{
    ...
    {
        DEBUGMSGTL(("snmp_agent",
                    "removed %d delegated request(s) for session "
                    "%8p\n",
                    total_count, sess));
 ...
 */
int netsnmp_pdu_stats_process(netsnmp_agent_session* asp)
{
...
int netsnmp_wrap_up_request(netsnmp_agent_session* asp, int status)
{
    ...
    if (asp->pdu)
    {
        ...
        if (status == SNMP_ERR_NOERROR)
        {
            ...
            status = _build_initial_pdu_packet(
                slp, asp->pdu, SNMP_MSG_GETBULK == asp->orig_pdu->command);
...
#ifndef NETSNMP_NO_WRITE_SUPPORT
                (asp->pdu->command == SNMP_MSG_SET ? STAT_SNMPINTOTALSETVARS
...
void netsnmp_free_agent_snmp_session_by_session(
    netsnmp_session* sess, void (*free_request)(netsnmp_request_list*))
{
...
int handle_snmp_packet(int op, netsnmp_session* session, int reqid,
                                 ...
                                 netsnmp_subtree* tp)
{
    ...
    if (tp && (asp->pdu->command == SNMP_MSG_GETNEXT ||
               asp->pdu->command == SNMP_MSG_GETBULK))
    {
        int result;
        int prefix_len;

        prefix_len = netsnmp_oid_find_prefix(tp->start_a, tp->start_len,
                                             tp->end_a, tp->end_len);
        if (prefix_len < 1)
        {
        ...
        while (result == VACM_NOTINVIEW)
        {
            ...
            if (tp)
            {
                ...
                {
                    result = netsnmp_acm_check_subtree(asp->pdu, tp->start_a,
                                                       prefix_len);
                }
            }
            else
    ...
    else
    {
...
#endif /* NETSNMP_NO_WRITE_SUPPORT */
            DEBUGMSGTL(("verbose:asp",
                        "asp %p reqinfo %p assigned to request\n", asp,
                        asp->reqinfo));
        ...
        if (tp->global_cacheid)
        {
             ...
             */
            if (asp->cache_store &&
                -1 != (cacheid = netsnmp_get_local_cachid(asp->cache_store,
                                                          tp->global_cacheid)))
            {
            }
            else
...
#define CACHE_GROW_SIZE 16
                ...
                asp->treecache = (netsnmp_tree_cache*)realloc(
                    asp->treecache,
                    sizeof(netsnmp_tree_cache) * asp->treecache_len);
    ...
    for (i = 0; i <= asp->treecache_num; i++)
    {
             ...
             request = request->next)
        {
            ...
            for (j = request->repeat, vb = request->requestvb_start;
                 vb && j > -1; j--, vb = vb->next_variable)
            {
                if (vb->type != ASN_NULL && vb->type != ASN_PRIV_RETRY)
                { /* not yet processed */
                    view = in_a_view(vb->name, &vb->name_length, asp->pdu,
                                     vb->type);
...
int netsnmp_create_subtree_cache(netsnmp_agent_session* asp)
{
    ...
    if (asp->treecache == NULL && asp->treecache_len == 0)
    {
        ...
        asp->treecache = (netsnmp_tree_cache*)calloc(
            asp->treecache_len, sizeof(netsnmp_tree_cache));
        if (asp->treecache == NULL)
            return SNMP_ERR_GENERR;
    }
    asp->treecache_num = -1;
    ...
    if (asp->pdu->command == SNMP_MSG_GETBULK)
    {
        ...
        {
            int maxbulk = netsnmp_ds_get_int(
                NETSNMP_DS_APPLICATION_ID, NETSNMP_DS_AGENT_MAX_GETBULKREPEATS);
            int maxresponses =
            ...
            if (maxresponses > (asp->pdu->msgMaxSize / avgvarbind))
            {
                ...
                DEBUGMSGTL(("snmp_agent:bulk",
                            "lowering maxresponse to %d based pdusession "
            ...
            asp->bulkcache = (netsnmp_variable_list**)malloc(
    ...
    for (varbind_ptr = asp->pdu->variables; varbind_ptr; varbind_ptr = vbsave)
    {
                ...
                if (asp->pdu->errindex > 0)
                {
                    ...
                    for (i = 1; i < asp->pdu->errindex; i++)
                    {
                        ...
                        if (!vbptr->next_variable)
                        {
                             ...
                             */
                            DEBUGMSGTL(
                                ("snmp_agent", "NextVar malloc failed\n"));
                        }
                        else
                        {
        ...
        {
            request =
                netsnmp_add_varbind_to_cache(asp, vbcount, varbind_ptr, tp);
...
int netsnmp_reassign_requests(netsnmp_agent_session* asp)
{
     ...
     * malloc new space
     */
    asp->treecache = (netsnmp_tree_cache*)calloc(asp->treecache_len,
...
void netsnmp_delete_subtree_cache(netsnmp_agent_session* asp)
{
    while (asp->treecache_num >= 0)
    {
        ...
        netsnmp_delete_request_infos(
            asp->treecache[asp->treecache_num].requests_begin);
        asp->treecache_num--;
    }
}
...
int netsnmp_check_requests_status(netsnmp_agent_session* asp,
                                  netsnmp_request_info* requests,
                                  int look_for_specific)
{
    /*
...
int netsnmp_check_all_requests_status(netsnmp_agent_session* asp,
                                      int look_for_specific)
{
    int i;
    for (i = 0; i <= asp->treecache_num; i++)
...
{
    int i, retstatus = SNMP_ERR_NOERROR, status = SNMP_ERR_NOERROR,
           final_status = SNMP_ERR_NOERROR;
    ...
    for (i = 0; i <= asp->treecache_num; i++)
    {
        ...
        switch (asp->mode)
        {
            ...
            case MODE_SET_COMMIT:
                retstatus = netsnmp_check_requests_status(
                    asp, asp->treecache[i].requests_begin,
                    SNMP_ERR_COMMITFAILED);
                break;

            case MODE_SET_UNDO:
                retstatus = netsnmp_check_requests_status(
                    asp, asp->treecache[i].requests_begin, SNMP_ERR_UNDOFAILED);
                break;
#endif /* NETSNMP_NO_WRITE_SUPPORT */

            default:
                retstatus = netsnmp_check_requests_status(
                    asp, asp->treecache[i].requests_begin, 0);
                break;
        }
...
void netsnmp_check_outstanding_agent_requests(void)
{
    ...
    while (netsnmp_agent_queued_list)
    {
            ...
            (agent_delegated_list))
        {
            ...
            netsnmp_processing_set = netsnmp_agent_queued_list;
            DEBUGMSGTL(("snmp_agent",
                        "SET request remains queued while "
        ...
        DEBUGMSGTL(
            ("snmp_agent", "processing queued request, asp = %8p\n", asp));
...
int check_delayed_request(netsnmp_agent_session* asp)
{
    ...
    DEBUGMSGTL(
        ("snmp_agent", "processing delegated request, asp = %8p\n", asp));
    ...
    switch (asp->mode)
    {
        ...
        case SNMP_MSG_GETNEXT:
            ...
            {
                DEBUGMSGTL(
                    ("snmp_agent", "canceling next walk for asp %p\n", asp));
                break;
            }
...
int check_getnext_results(netsnmp_agent_session* asp)
{
    ...
    for (i = 0; i <= old_treecache_num; i++)
    {
             ...
             request = request->next)
        {
             ...
             */
            if (snmp_oid_compare(
                    request->requestvb->name, request->requestvb->name_length,
                    request->range_end, request->range_end_len) >= 0)
            {
                /*
                 ...
                 */
                snmp_set_var_objid(request->requestvb, request->range_end,
...
int handle_getnext_loop(netsnmp_agent_session* asp)
{
    ...
    while (netsnmp_running)
    {
        ...
        count = rough_size = 0;
        DEBUGMSGTL(
            ("results:intermediate", "getnext results, before next pass:\n"));
             ...
             var_ptr = var_ptr->next_variable)
        {
            ...
            {
                DEBUGMSGTL(
                    ("results", "estimating packet too big; stop gathering\n"));
                asp->pdu->flags |=
                    UCD_MSG_FLAG_BULK_TOOBIG | UCD_MSG_FLAG_FORWARD_ENCODE;
...
int netsnmp_handle_request(netsnmp_agent_session* asp, int status)
{
    ...
    if (_pdu_stats_max > 0)
    {
        ...
        if (NULL == start)
        {
            ...
            data_list =
                netsnmp_create_data_list("netsnmp_pdu_stats", start, free);
            if (NULL == data_list)
            {
                free(start);
    ...
    switch (asp->pdu->command)
    {
        ...
        case SNMP_MSG_INTERNAL_SET_RESERVE1:
            ...
            if (asp->vbcount) /* efence doesn't like 0 size allocs */
                asp->requests = (netsnmp_request_info*)calloc(
                    asp->vbcount, sizeof(netsnmp_request_info));
            /*
             * collect varbinds
             */
        ...
        case SNMP_ENDOFMIBVIEW:
            ...
            switch (mode)
            {
                ...
                case MODE_GETBULK:
                     ...
                     *
                    snmp_log(LOG_ERR, "Illegal error_value %d for mode %d
                    ignored\n", error_value, mode); return SNMPERR_VALUE;
                     */
                                  ...
                                  int error_value, int idx)
{
    ...
    return _request_set_error(req, request->agent_req_info->mode, error_value);
}
