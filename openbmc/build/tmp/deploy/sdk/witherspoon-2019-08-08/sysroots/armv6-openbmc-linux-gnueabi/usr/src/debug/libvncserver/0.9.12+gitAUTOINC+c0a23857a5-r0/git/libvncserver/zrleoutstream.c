...
rfbBool zrleOutStreamFlush(zrleOutStream* os)
{
    ...
    while (os->zs.avail_in != 0)
    {
        do
        {
...
#ifdef ZRLE_DEBUG
            rfbLog("zrleOutStreamFlush: calling deflate, avail_in %d, "
                   "avail_out %d\n",
            ...
            {
                rfbLog(
                    "zrleOutStreamFlush: deflate failed with error code %d\n",
                    ret);
                return FALSE;
            }
...
static int zrleOutStreamOverrun(zrleOutStream* os, int size)
{
    ...
    while (os->in.end - os->in.ptr < size && os->in.ptr > os->in.start)
    {
        ...
        do
        {
...
#ifdef ZRLE_DEBUG
            rfbLog("zrleOutStreamOverrun: calling deflate, avail_in %d, "
                   "avail_out %d\n",
                   os->zs.avail_in, os->zs.avail_out);
#endif
            ...
            {
                rfbLog(
                    "zrleOutStreamOverrun: deflate failed with error code %d\n",
                    ret);
                return 0;
            }
        ...
        else
        {
             ...
             * start of the buffer.
             */
            rfbLog("zrleOutStreamOverrun: out buf not full, but in data not "
                   "consumed\n");
