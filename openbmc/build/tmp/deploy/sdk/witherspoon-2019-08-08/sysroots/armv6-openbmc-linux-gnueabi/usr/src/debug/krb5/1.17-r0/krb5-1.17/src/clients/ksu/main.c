...
#define LOG_NDELAY 0
           ...
           ((option = getopt(pargc, pargv, "n:c:r:a:zZDfpkql:e:")) != -1))
    {
        switch (option)
        {
            ...
            case 'n':
                ...
                {
                    com_err(prog_name, retval, _("when parsing name %s"),
                            optarg);
                    errflg++;
                }
                given_princ = TRUE;
            ...
            case 'c':
                if (cc_source_tag == NULL)
                {
                    ...
                    {
                        fprintf(stderr,
                                _("malformed credential cache name %s\n"),
                                cc_source_tag);
                        errflg++;
                    }
                }
    ...
    if ((retval = get_best_princ_for_target(
             ksu_context, source_uid, target_uid, source_user, target_user,
             cc_source, options, cmd, localhostname, &client, &hp)))
    {
    ...
    if (geteuid() != source_uid)
    {
        ...
        {
            com_err(prog_name, errno,
                    _("while returning to source uid after "
    ...
    if (auth_debug)
    {
        ...
        {
            fprintf(stderr, "GET_best_princ_for_target result-best principal ");
    ...
    if (source_uid && (source_uid != target_uid))
    {
        ...
        auth_val =
            krb5_auth_check(ksu_context, client, localhostname, options,
                            target_user, cc_tmp, &path_passwd, target_uid);
        ...
        syslog(LOG_NOTICE, "'%s %s' authenticated %s for %s%s", prog_name,
               target_user, client_name, source_user, ontty());

        /* Run authorization as target.*/
        if (krb5_seteuid(target_uid))
        {
            com_err(prog_name, errno,
                    _("while switching to target for "
                      "authorization check"));
            exit(1);
        }

        if ((retval = krb5_authorization(ksu_context, client, target_user, cmd,
        ...
        {
            com_err(prog_name, errno,
                    _("while switching back from target "
        ...
        if (authorization_val == TRUE)
        {
            ...
            if (cmd)
            {
                ...
                syslog(LOG_NOTICE,
                       "Account %s: authorization for %s for execution of %s "
                       "successful",
        ...
        else
        {
            if (cmd)
            {
                ...
                fprintf(stderr,
                        _("Account %s: authorization for %s for "
    ...
    if (stored)
    {
        retval = krb5_ccache_copy(ksu_context, cc_tmp, client, cc_target, FALSE,
                                  client, &stored);
        if (retval)
        {
            com_err(prog_name, retval, _("while copying cache %s to %s"),
    ...
    if (cmd)
    {
        ...
        {
            fprintf(stderr,
                    _("Internal error: command %s did not get "
                      "resolved\n"),
                    cmd);
            exit(1);
        }
 ...
 * message on failure. */
static krb5_error_code set_ccname_env(krb5_context ksu_context,
                                      krb5_ccache ccache)
{
 ...
 * message on failure.
 */
static krb5_error_code get_configured_defccname(krb5_context context,
                                                char** target_out)
{
    krb5_error_code retval;
    const char* defname;
 ...
 * message on failure. */
static krb5_error_code resolve_target_cache(krb5_context context,
                                            krb5_principal princ,
                                            krb5_ccache* ccache_out,
                                            krb5_boolean* ccache_reused)
{
    ...
    if (!switchable)
    {
        ...
        do
        {
            ...
            if (asprintf(&ccname, "%s.%s", target, sym) < 0)
            {
                ...
                free(sym);
                com_err(prog_name, retval,
                        _("while allocating memory for the "
                          "target ccache name"));
                goto cleanup;
            }
            free(sym);
...
static int set_env_var(name, value) char* name;
char* value;
{
    char* env_var_buf;
...
static void sweep_up(context, cc) krb5_context context;
krb5_ccache cc;
{
    krb5_error_code retval;
...
krb5_error_code get_params(optindex, pargc, pargv, params) int* optindex;
int pargc;
char** pargv;
char*** params;
{
...
krb5_error_code ksu_tgtname(context, server, client,
                            tgtprinc) krb5_context context;
const krb5_data *server, *client;
krb5_principal* tgtprinc;
{
    return krb5_build_principal_ext(
        context, tgtprinc, client->length, client->data, KRB5_TGS_NAME_SIZE,
        KRB5_TGS_NAME, server->length, server->data, 0);
}
