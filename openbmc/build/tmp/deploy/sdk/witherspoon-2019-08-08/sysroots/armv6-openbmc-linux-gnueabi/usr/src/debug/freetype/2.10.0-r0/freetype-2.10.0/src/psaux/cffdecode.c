...
static FT_Error cff_operator_seac(CFF_Decoder* decoder, FT_Pos asb, FT_Pos adx,
                                  FT_Pos ady, FT_Int bchar, FT_Int achar)
{
    FT_Error error;
    ...
    error = decoder->get_glyph_callback(face, (FT_UInt)bchar_index, &charstring,
                                        &charstring_len);
    if (!error)
    {
    ...
    error = decoder->get_glyph_callback(face, (FT_UInt)achar_index, &charstring,
                                        &charstring_len);
    if (!error)
    {
        /* the seac operator must not be nested */
...
cff_decoder_parse_charstrings(CFF_Decoder* decoder, FT_Byte* charstring_base,
                              FT_ULong charstring_len, FT_Bool in_dict)
{
    ...
    FT_Pos x, y;
    FT_Fixed* stack;
    FT_Int charstring_type = decoder->cff->top_font.font_dict.charstring_type;
    FT_UShort num_designs = decoder->cff->top_font.font_dict.num_designs;
    FT_UShort num_axes = decoder->cff->top_font.font_dict.num_axes;
        ...
        if (v >= 32 || v == 28)
        {
            ...
            else
            {
                ...
                val = (FT_Int32)(((FT_UInt32)ip[0] << 24) |
                                 ((FT_UInt32)ip[1] << 16) |
                                 ((FT_UInt32)ip[2] << 8) | (FT_UInt32)ip[3]);
                ip += 4;
        ...
        else
        {
            ...
            if (req_args & CFF_COUNT_CHECK_WIDTH)
            {
                if (num_args > 0 && decoder->read_width)
                {
                    ...
                    /* difference against `nominal_width'.  Else, the number
                     * here  */
                    /* is truly a width, not a difference against
                     * `nominal_width'. */
                    ...
                    switch (op)
                    {
                        ...
                        case cff_op_endchar:
                            /* If there is a width specified for endchar, we
                             * either have */
                    ...
                    {
                        decoder->glyph_width =
                            decoder->nominal_width + (stack[0] >> 16);
            ...
            switch (op)
            {
                ...
                case cff_op_cntrmask:
                    ...
                    {
                        if (hinter)
                            hinter->stems(hinter->hints, 0, num_args / 2,
                                          args - (num_args & ~1));
                ...
                case cff_op_vlineto:
                {
                    ...
                    FT_TRACE4(("%s\n",
                               op == cff_op_hlineto ? " hlineto" : " vlineto"));
                ...
                case cff_op_hflex1:
                {
                    ...
                    /* adding five more points: 4 control points, 1 on-curve
                     * point */
                    /* -- make sure we have enough space for the start point if
                     * it */
                    /* needs to be added */
                    if (cff_builder_start_point(builder, x, y) ||
                ...
                case cff_op_hflex:
                {
                    ...
                    /* adding six more points; 4 control points, 2 on-curve
                     * points */
                    if (cff_builder_start_point(builder, x, y) ||
                        cff_check_points(builder, 6))
                        goto Fail;
                ...
                case cff_op_flex1:
                {
                    ...
                    /* XXX: figure out whether this is supposed to be a
                     * horizontal */
                    /*      or vertical flex; the Type 2 specification is
                     * vague... */
                ...
                case cff_op_flex:
                {
                    ...
                    for (count = 6; count > 0; count--)
                    {
                        ...
                        cff_builder_add_point(
                            builder, x, y, FT_BOOL(count == 4 || count == 1));
                        args += 2;
                    }
                ...
                case cff_op_seac:
                    ...
                    error = cff_operator_seac(decoder, args[0], args[1],
                                              args[2], (FT_Int)(args[3] >> 16),
                                              (FT_Int)(args[4] >> 16));
                ...
                case cff_op_endchar:
                    ...
                    if (num_args >= 4)
                    {
                        /* Save glyph width so that the subglyphs don't
                         * overwrite it. */
                        FT_Pos glyph_width = decoder->glyph_width;

                        error = cff_operator_seac(
                            decoder, 0L, args[-4], args[-3],
                            (FT_Int)(args[-2] >> 16), (FT_Int)(args[-1] >> 16));
                ...
                {
                    FT_UInt32* randval =
                        in_dict ? &decoder->cff->top_font.random
                ...
                case cff_op_sqrt:
                    ...
                    else if (args[0] > 0)
                    {
                        ...
                        {
                            new_root =
                                (root + FT_DivFix(args[0], root) + 1) >> 1;
                            if (new_root == root)
                                break;
                            root = new_root;
                        }
                ...
                case cff_op_load:
                    ...
                    /* in version 16-March-2000                               */
                    {
                        ...
                        /* since we currently don't handle interpolation of
                         * multiple */
                        /* master fonts, we store a vector [1 0 0 ...] in the */
                        /* temporary storage array regardless of the Registry
                         * index  */
                        if (reg_idx >= 0 && reg_idx <= 2 && idx >= 0 &&
                            idx < CFF_MAX_TRANS_ELEMENTS && count >= 0 &&
                            count <= num_axes)
                        {
                            FT_Int end, i;
                ...
                {
                    FT_UInt idx =
                        (FT_UInt)((args[0] >> 16) + decoder->locals_bias);
                ...
                {
                    FT_UInt idx =
                        (FT_UInt)((args[0] >> 16) + decoder->globals_bias);
 ...
 */
FT_LOCAL_DEF(void)
cff_decoder_init(CFF_Decoder* decoder, TT_Face face, CFF_Size size,
                 ...
                 CFF_Decoder_Free_Glyph_Callback free_callback)
{
    ...
    decoder->globals_bias = cff_compute_bias(
        cff->top_font.font_dict.charstring_type, decoder->num_globals);
...
cff_decoder_prepare(CFF_Decoder* decoder, CFF_Size size, FT_UInt glyph_index)
{
    ...
    decoder->locals_bias = cff_compute_bias(
        decoder->cff->top_font.font_dict.charstring_type, decoder->num_locals);
