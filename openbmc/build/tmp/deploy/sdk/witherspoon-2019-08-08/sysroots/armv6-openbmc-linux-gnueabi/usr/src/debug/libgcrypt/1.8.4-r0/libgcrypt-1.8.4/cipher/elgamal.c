...
void _gcry_register_pk_elg_progress(void (*cb)(void*, const char*, int, int,
                                               int),
                                    void* cb_data)
{
    progress_cb = cb;
 ...
 */
static gcry_err_code_t generate(ELG_secret_key* sk, unsigned int nbits,
...
static gcry_err_code_t generate_using_x(ELG_secret_key* sk, unsigned int nbits,
                                        gcry_mpi_t x, gcry_mpi_t** ret_factors)
{
    gcry_err_code_t rc;
    gcry_mpi_t p;      /* The prime.  */
...
static gpg_err_code_t elg_generate(const gcry_sexp_t genparms,
                                   gcry_sexp_t* r_skey)
{
    gpg_err_code_t rc;
    unsigned int nbits;
    ...
    rc = sexp_build(r_skey, NULL,
                    ...
                    "  (elg(p%m)(g%m)(y%m)(x%m)))"
                    " %S)",
                    sk.p, sk.g, sk.y, sk.p, sk.g, sk.y, sk.x, misc_info);
...
static gcry_err_code_t elg_check_secret_key(gcry_sexp_t keyparms)
{
    gcry_err_code_t rc;
    ELG_secret_key sk = {NULL, NULL, NULL, NULL};

    rc = sexp_extract_param(keyparms, NULL, "pgyx", &sk.p, &sk.g, &sk.y, &sk.x,
                            NULL);
    if (rc)
        goto leave;
...
static gcry_err_code_t elg_encrypt(gcry_sexp_t* r_ciph, gcry_sexp_t s_data,
                                   gcry_sexp_t keyparms)
{
    ...
    /* Extract the key.  */
    rc = sexp_extract_param(keyparms, NULL, "pgy", &pk.p, &pk.g, &pk.y, NULL);
    if (rc)
        goto leave;
    if (DBG_CIPHER)
    {
...
static gcry_err_code_t elg_decrypt(gcry_sexp_t* r_plain, gcry_sexp_t s_data,
                                   gcry_sexp_t keyparms)
{
    gpg_err_code_t rc;
    struct pk_encoding_ctx ctx;
...
static gcry_err_code_t elg_sign(gcry_sexp_t* r_sig, gcry_sexp_t s_data,
                                gcry_sexp_t keyparms)
{
    gcry_err_code_t rc;
    struct pk_encoding_ctx ctx;
...
static gcry_err_code_t elg_verify(gcry_sexp_t s_sig, gcry_sexp_t s_data,
                                  gcry_sexp_t s_keyparms)
{
    ...
    /* Extract the key.  */
    rc = sexp_extract_param(s_keyparms, NULL, "pgy", &pk.p, &pk.g, &pk.y, NULL);
    if (rc)
        goto leave;
    if (DBG_CIPHER)
    {
...
gcry_pk_spec_t _gcry_pubkey_spec_elg = {
    GCRY_PK_ELG, {0, 0},        (GCRY_PK_USAGE_SIGN | GCRY_PK_USAGE_ENCR),
    "ELG",       elg_names,     "pgy",
    "pgyx",      "ab",          "rs",
    "pgy",       elg_generate,  elg_check_secret_key,
    elg_encrypt, elg_decrypt,   elg_sign,
    elg_verify,  elg_get_nbits,
};
