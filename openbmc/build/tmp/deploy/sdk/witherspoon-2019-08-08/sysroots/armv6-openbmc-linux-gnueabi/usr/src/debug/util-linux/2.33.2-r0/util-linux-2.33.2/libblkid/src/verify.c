...
blkid_dev blkid_verify(blkid_cache cache, blkid_dev dev)
{
    ...
    open_err:
        if ((errno == EPERM) || (errno == EACCES) || (errno == ENOENT))
        {
            ...
            DBG(PROBE,
                ul_debug("returning unverified data for %s", dev->bid_name));
            return dev;
        }
        blkid_free_dev(dev);
        return NULL;
    ...
    if (now >= dev->bid_time &&
        ...
        st.st_mtime <= dev->bid_time &&
#endif
        (diff < BLKID_PROBE_MIN || (dev->bid_flags & BLKID_BID_FL_VERIFIED &&
                                    diff < BLKID_PROBE_INTERVAL)))
        return dev;

#ifndef HAVE_STRUCT_STAT_ST_MTIM_TV_NSEC
    DBG(PROBE,
        ul_debug("need to revalidate %s (cache time %lu, stat time %lu,\t"
    ...
    DBG(PROBE,
        ul_debug(
            "need to revalidate %s (cache time %lu.%lu, stat time %lu.%lu,\t"
            "time since last check %lu)",
            dev->bid_name, (unsigned long)dev->bid_time,
            (unsigned long)dev->bid_utime, (unsigned long)st.st_mtime,
            (unsigned long)st.st_mtim.tv_nsec / 1000, (unsigned long)diff));
#endif
    ...
    blkid_probe_set_superblocks_flags(
        cache->probe, BLKID_SUBLKS_LABEL | BLKID_SUBLKS_UUID |
    ...
    if (dev)
    {
        ...
        DBG(PROBE, ul_debug("%s: devno 0x%04llx, type %s", dev->bid_name,
                            (long long)st.st_rdev, dev->bid_type));
    }
