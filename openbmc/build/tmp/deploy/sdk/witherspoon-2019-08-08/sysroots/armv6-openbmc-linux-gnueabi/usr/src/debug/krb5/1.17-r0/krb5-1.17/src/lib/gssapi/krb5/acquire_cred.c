...
#endif
static void (*pLeash_AcquireInitialTicketsIfNeeded)(krb5_context,
                                                    krb5_principal, char*,
                                                    int) = NULL;
...
static krb5_error_code check_keytab(krb5_context context, krb5_keytab kt,
...
static OM_uint32 acquire_accept_cred(krb5_context context,
...
#ifdef USE_LEASH
static krb5_error_code get_ccache_leash(krb5_context context,
                                        ...
                                        krb5_ccache* ccache_out)
{
    ...
    *ccache_out = NULL;
    ...
    if (hLeashDLL == INVALID_HANDLE_VALUE)
    {
        ...
        if (hLeashDLL != INVALID_HANDLE_VALUE)
        {
            (FARPROC) pLeash_AcquireInitialTicketsIfNeeded = GetProcAddress(
                hLeashDLL, "not_an_API_Leash_AcquireInitialTicketsIfNeeded");
        }
    }
                                      ...
                                      const krb5_data* value)
{
    ...
    if (config_princ->length != 2)
        return 0;
    if (data_eq_string(config_princ->data[1],
                       KRB5_CC_CONF_PROXY_IMPERSONATOR) &&
 ...
 * tickets for cred. */
static krb5_boolean can_get_initial_creds(krb5_context context,
                                          krb5_gss_cred_id_rec* cred)
{
    krb5_error_code code;
    ...
    code = krb5_kt_get_entry(context, cred->client_keytab, cred->name->princ, 0,
                             0, &entry);
    if (code)
    {
...
static krb5_error_code scan_ccache(krb5_context context,
                                   krb5_gss_cred_id_rec* cred)
{
    krb5_error_code code;
    ...
    code = krb5_build_principal_ext(
        context, &tgt_princ, realm->length, realm->data, KRB5_TGS_NAME_SIZE,
        KRB5_TGS_NAME, realm->length, realm->data, 0);
    if (code)
        return code;
...
static krb5_error_code get_cache_for_name(krb5_context context,
                                          krb5_gss_cred_id_rec* cred)
{
    krb5_error_code code;
...
static void set_refresh_time(krb5_context context, krb5_ccache ccache,
...
static krb5_error_code get_initial_cred(krb5_context context,
                                        krb5_gss_cred_id_rec* cred)
{
    krb5_error_code code;
    ...
    if (cred->password != NULL)
    {
        code = krb5_get_init_creds_password(context, &creds, cred->name->princ,
                                            cred->password, NULL, NULL, 0, NULL,
                                            opt);
    }
...
static OM_uint32
    acquire_init_cred(krb5_context context, OM_uint32* minor_status,
                      ...
                      krb5_keytab client_keytab, krb5_gss_cred_id_rec* cred)
{
    krb5_error_code code;
    ...
    if (GSS_ERROR(kg_sync_ccache_name(context, minor_status)))
        return GSS_S_FAILURE;
    if (GSS_ERROR(kg_caller_provided_ccache_name(minor_status, &caller_ccname)))
        return GSS_S_FAILURE;
...
static OM_uint32 acquire_cred_context(
    krb5_context context, OM_uint32* minor_status, gss_name_t desired_name,
    gss_buffer_t password, OM_uint32 time_req, gss_cred_usage_t cred_usage,
    krb5_ccache ccache, krb5_keytab client_keytab, krb5_keytab keytab,
    const char* rcname, krb5_boolean iakerb, gss_cred_id_t* output_cred_handle,
    OM_uint32* time_rec)
{
    ...
    else
    {
        ...
        if (time_rec)
        {
            ...
            if (GSS_ERROR(ret))
                goto error_out;
            *time_rec =
                ts_after(cred->expire, now) ? ts_delta(cred->expire, now) : 0;
...
static OM_uint32 acquire_cred(OM_uint32* minor_status, gss_name_t desired_name,
                              ...
                              OM_uint32* time_rec)
{
    ...
    ret = acquire_cred_context(context, minor_status, desired_name, password,
                               time_req, cred_usage, ccache, NULL, keytab, NULL,
                          ...
                          gss_cred_id_t cred_handle, gss_name_t target_name)
{
    ...
    *minor_status = 0;
    ...
    if (tname != NULL)
    {
        ...
        code =
            krb5_cc_select(context, tname->princ, &cred->ccache, &client_princ);
                                  ...
                                  const gss_buffer_t value)
{
    ...
    code =
        acquire_cred(minor_status, desired_name, NULL, GSS_C_INDEFINITE, usage,
                     req->id, req->keytab, FALSE, cred_handle, &time_rec);
    if (req->keytab_principal != NULL)
        k5_mutex_destroy(&name.lock);
    return code;
}

OM_uint32 KRB5_CALLCONV krb5_gss_acquire_cred_from(
    ...
    gss_OID_set* actual_mechs, OM_uint32* time_rec)
{
    ...
    ret = acquire_cred_context(context, minor_status, desired_name, NULL,
                               ...
                               keytab, rcname, 0, output_cred_handle, time_rec);
