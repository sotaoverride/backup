...
DEFINE_CONFIG_PARSE_ENUM(config_parse_wol, wol, WakeOnLan,
                         "Failed to parse WakeOnLan setting");

static const char* const port_table[] = {
    [NET_DEV_PORT_TP] = "tp",   [NET_DEV_PORT_AUI] = "aui",
    [NET_DEV_PORT_MII] = "mii", [NET_DEV_PORT_FIBRE] = "fibre",
...
assert_cc((ELEMENTSOF(ethtool_link_mode_bit_table) - 1) / 32 <
          ELEMENTSOF(((struct link_config){}).advertise));

DEFINE_STRING_TABLE_LOOKUP(ethtool_link_mode_bit,
...
{
    struct ethtool_drvinfo ecmd = {.cmd = ETHTOOL_GDRVINFO};
    struct ifreq ifr = {.ifr_data = (void*)&ecmd};
    char* d;
    int r;
    ...
    if (*fd < 0)
    {
        ...
        if (r < 0)
            return log_warning_errno(
                r, "link_config: could not connect to ethtool: %m");
    }
...
int ethtool_set_speed(int* fd, const char* ifname, unsigned speed,
                      Duplex duplex)
{
...
{
    struct ethtool_wolinfo ecmd = {.cmd = ETHTOOL_GWOL};
    struct ifreq ifr = {.ifr_data = (void*)&ecmd};
    bool need_update = false;
    int r;
...
static int get_stringset(int fd, struct ifreq* ifr, int stringset_id,
...
static int find_feature_index(struct ethtool_gstrings* strings,
                              const char* feature)
{
    unsigned i;
...
int ethtool_set_features(int* fd, const char* ifname, int* features)
{
    ...
    if (r < 0)
        return log_warning_errno(
            r, "link_config: could not get ethtool features for %s", ifname);

    sfeatures = alloca0(sizeof(struct ethtool_sfeatures) +
                        DIV_ROUND_UP(strings->len, 32U) *
                            sizeof(sfeatures->features[0]));
    ...
    for (i = 0; i < _NET_DEV_FEAT_MAX; i++)
    {
        ...
        if (features[i] != -1)
        {
            ...
            if (r < 0)
            {
                log_warning_errno(r, "link_config: could not find feature: %s",
    ...
    if (r < 0)
        return log_warning_errno(
            r, "link_config: could not set ethtool features for %s", ifname);

    return 0;
}

static int get_glinksettings(int fd, struct ifreq* ifr,
                             struct ethtool_link_usettings** g)
{
    struct ecmd
    {
    ...
    /* The interaction user/kernel via the new API requires a small
       ...
       0). When kernel and user agree, kernel returns valid info in all fields
       (ie. link mode length > 0 and cmd is ETHTOOL_GLINKSETTINGS). Based on
    ...
    */
    ...
    offset = 0;
    memcpy(u->link_modes.supported, &ecmd.link_mode_data[offset],
           4 * ecmd.req.link_mode_masks_nwords);

    offset += ecmd.req.link_mode_masks_nwords;
    memcpy(u->link_modes.advertising, &ecmd.link_mode_data[offset],
           4 * ecmd.req.link_mode_masks_nwords);

    offset += ecmd.req.link_mode_masks_nwords;
    memcpy(u->link_modes.lp_advertising, &ecmd.link_mode_data[offset],
           4 * ecmd.req.link_mode_masks_nwords);
...
static int get_gset(int fd, struct ifreq* ifr,
                    struct ethtool_link_usettings** u)
{
...
static int set_slinksettings(int fd, struct ifreq* ifr,
                             const struct ethtool_link_usettings* u)
{
    struct
    {
        struct ethtool_link_settings req;
    ...
    offset = 0;
    memcpy(&ecmd.link_mode_data[offset], u->link_modes.supported,
           4 * ecmd.req.link_mode_masks_nwords);

    offset += ecmd.req.link_mode_masks_nwords;
    memcpy(&ecmd.link_mode_data[offset], u->link_modes.advertising,
           4 * ecmd.req.link_mode_masks_nwords);

    offset += ecmd.req.link_mode_masks_nwords;
    memcpy(&ecmd.link_mode_data[offset], u->link_modes.lp_advertising,
           4 * ecmd.req.link_mode_masks_nwords);
...
static int set_sset(int fd, struct ifreq* ifr,
                    const struct ethtool_link_usettings* u)
{
    struct ethtool_cmd ecmd = {
        .cmd = ETHTOOL_SSET,
 ...
 * enabled then they are read-only. If the link is up they represent the
 * negotiated link mode; if the link is down, the speed is 0, %SPEED_UNKNOWN or
 * the highest enabled speed and @duplex is %DUPLEX_UNKNOWN or the best enabled
 * duplex mode.
 */
int ethtool_set_glinksettings(int* fd, const char* ifname,
                              struct link_config* link)
{
    ...
    {
        log_info("link_config: autonegotiation is unset or enabled, the speed "
    ...
    if (r < 0)
    {
        ...
        if (r < 0)
            return log_warning_errno(
                r, "link_config: Cannot get device settings for %s : %m",
                ifname);
    }
    ...
    if (!eqzero(link->advertise))
    {
        ...
        memcpy(&u->link_modes.advertising, link->advertise,
    ...
    if (r < 0)
        return log_warning_errno(
            r, "link_config: Cannot set device settings for %s : %m", ifname);

    return r;
}

int config_parse_channel(const char* unit, const char* filename, unsigned line,
...
{
    struct ethtool_channels ecmd = {.cmd = ETHTOOL_GCHANNELS};
    struct ifreq ifr = {.ifr_data = (void*)&ecmd};
...
int config_parse_advertise(const char* unit, const char* filename,
                           ...
                           const char* rvalue, void* data, void* userdata)
{
    ...
    for (p = rvalue;;)
    {
        ...
        /* We reuse the kernel provided enum which does not contain negative
         * value. So, the cast below is mandatory. Otherwise, the check below
         * always passes and access an invalid address. */
        if ((int)mode < 0)
        {
