...
static int kmod_config_add_command(struct kmod_config* config,
                                   const char* modname, const char* command,
...
static int kmod_config_add_softdep(struct kmod_config* config,
                                   const char* modname, const char* line)
{
    struct kmod_list* list;
                                  ...
                                  char* param, char* value)
{
    ...
    else
    {
        ...
        {
            ERR(config->ctx,
                "Ignoring bad option on kernel command line while parsing "
                "module name: '%s'\n",
                modname);
        }
                                  ...
                                  const char* path, const char* fn)
{
    ...
    if (len < 6 ||
        (!streq(&fn[len - 5], ".conf") && !streq(&fn[len - 6], ".alias")))
        return true;
...
static int conf_files_insert_sorted(struct kmod_ctx* ctx,
                                    struct kmod_list** list, const char* path,
                                    const char* name)
{
                    ...
                    const char* const* config_paths)
{
    ...
    conf_files_insert_sorted(ctx, &list, kmod_get_dirname(ctx),
                             "modules.softdep");
 ...
 */
KMOD_EXPORT struct kmod_config_iter*
