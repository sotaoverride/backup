...
#define PRIVRW (NETSNMP_SNMPV2ANY | 0x5000)
    ...
    REGISTER_MIB("mibII/vacm:sec2group", vacm_sec2group, variable1,
                 vacm_sec2group_oid);
    REGISTER_MIB("mibII/vacm:access", vacm_access, variable1, vacm_access_oid);
...
u_char* var_vacm_sec2group(struct variable* vp, oid* name, size_t* length,
...
u_char* var_vacm_access(struct variable* vp, oid* name, size_t* length,
...
u_char* var_vacm_view(struct variable* vp, oid* name, size_t* length, int exact,
        ...
        if (exact)
        {
            ...
            *cp = 0;
            ...
            if ((op + len) != (name + *length))
                return NULL; /* Declared length doesn't match what we actually
                                got */
            op1 = &(subtree[1]);
            while (len-- > 0)
            {
...
#ifndef NETSNMP_NO_WRITE_SUPPORT
int sec2group_parse_oid(oid* oidIndex, size_t oidLen, int* model,
...
struct vacm_groupEntry* sec2group_parse_groupEntry(oid* name, size_t name_len)
{
    ...
    if (sec2group_parse_oid(&name[SEC2GROUP_MIB_LENGTH],
                            name_len - SEC2GROUP_MIB_LENGTH, &model,
                            (u_char**)&newName, &nameLen))
        return NULL;
...
int write_vacmGroupName(int action, u_char* var_val, u_char var_val_type,
...
int write_vacmSecurityToGroupStorageType(int action, u_char* var_val,
                                         u_char var_val_type,
...
int write_vacmSecurityToGroupStatus(int action, u_char* var_val,
...
int access_parse_oid(oid* oidIndex, size_t oidLen, unsigned char** groupName,
                     size_t* groupNameLen, unsigned char** contextPrefix,
                     size_t* contextPrefixLen, int* model, int* level)
{
...
int write_vacmAccessStatus(int action, u_char* var_val, u_char var_val_type,
                           size_t var_val_len, u_char* statP, oid* name,
                           size_t name_len)
{
    static long long_ret;
    ...
    if (action == RESERVE1)
    {
        ...
        if (access_parse_oid(
                ...
                (u_char**)&newGroupName, &groupNameLen,
                (u_char**)&newContextPrefix, &contextPrefixLen, &model, &level))
        {
            return SNMP_ERR_INCONSISTENTNAME;
        }
        ...
        else
        {
            ...
            if (long_ret == RS_CREATEANDGO || long_ret == RS_CREATEANDWAIT)
            {
                if ((aptr = vacm_createAccessEntry(
                         newGroupName, newContextPrefix, model, level)) == NULL)
                {
                    free(newGroupName);
                    free(newContextPrefix);
                    return SNMP_ERR_GENERR;
    ...
    else if (action == COMMIT)
    {
        ...
        if (aptr)
        {
            if (long_ret == RS_DESTROY)
            {
                vacm_destroyAccessEntry(newGroupName, newContextPrefix, model,
                                        level);
            }
        }
        free(newGroupName);
...
int write_vacmAccessStorageType(int action, u_char* var_val,
                                u_char var_val_type, size_t var_val_len,
                                u_char* statP, oid* name, size_t name_len)
{
    /*
...
int write_vacmAccessContextMatch(int action, u_char* var_val,
                                 u_char var_val_type, size_t var_val_len,
                                 u_char* statP, oid* name, size_t name_len)
{
    /*
...
int write_vacmAccessReadViewName(int action, u_char* var_val,
                                 u_char var_val_type, size_t var_val_len,
                                 u_char* statP, oid* name, size_t name_len)
{
    static unsigned char string[VACMSTRINGLEN];
...
int write_vacmAccessWriteViewName(int action, u_char* var_val,
                                  u_char var_val_type, size_t var_val_len,
                                  u_char* statP, oid* name, size_t name_len)
{
    static unsigned char string[VACMSTRINGLEN];
...
int write_vacmAccessNotifyViewName(int action, u_char* var_val,
                                   u_char var_val_type, size_t var_val_len,
                                   u_char* statP, oid* name, size_t name_len)
{
    static unsigned char string[VACMSTRINGLEN];
...
int view_parse_oid(oid* oidIndex, size_t oidLen, unsigned char** viewName,
                   size_t* viewNameLen, oid** subtree, size_t* subtreeLen)
{
...
int write_vacmViewStatus(int action, u_char* var_val, u_char var_val_type,
                         size_t var_val_len, u_char* statP, oid* name,
                         size_t name_len)
{
    static long long_ret;
...
int write_vacmViewStorageType(int action, u_char* var_val, u_char var_val_type,
                              size_t var_val_len, u_char* statP, oid* name,
                              size_t name_len)
{
...
int write_vacmViewMask(int action, u_char* var_val, u_char var_val_type,
                       size_t var_val_len, u_char* statP, oid* name,
                       size_t name_len)
{
    static unsigned char string[VACMSTRINGLEN];
...
int write_vacmViewType(int action, u_char* var_val, u_char var_val_type,
                       size_t var_val_len, u_char* statP, oid* name,
                       size_t name_len)
{
    long newValue = *((long*)var_val);
...
int write_vacmViewSpinLock(int action, u_char* var_val, u_char var_val_type,
                           size_t var_val_len, u_char* statP, oid* name,
                           size_t name_len)
{
    static long long_ret;
    ...
    if (action == RESERVE1)
    {
        ...
        {
            DEBUGMSGTL(
                ("mibII/vacm_vars", "write to vacmViewSpinLock: bad length\n"));
            return SNMP_ERR_WRONGLENGTH;
        }
        long_ret = *((long*)var_val);
