...
static const char* successNotes[CMD_NUM] = {
    "Sec2group successfully created.",  "Sec2group successfully deleted.",
    "Access successfully created.",     "Access successfully deleted.",
    "View successfully created.",       "View successfully deleted.",
    "AuthAccess successfully created.", "AuthAccess successfully deleted."};
...
static oid
    vacmGroupName[MAX_OID_LEN] = {1, 3, 6, 1, 6, 3, 16, 1, 2, 1, 3},
    ...
    vacmViewTreeFamilyStatus[MAX_OID_LEN] = {1,  3, 6, 1, 6, 3,
                                             16, 1, 5, 2, 1, 6}
...
void usage(void)
{
    ...
    fprintf(stderr, "\nsnmpvacm commands:\n");
    fprintf(stderr, "        createAccess     GROUPNAME [CONTEXTPREFIX] "
                    "SECURITYMODEL SECURITYLEVEL CONTEXTMATCH READVIEWNAME "
                    "WRITEVIEWNAME NOTIFYVIEWNAME\n");
    fprintf(stderr, "        deleteAccess     GROUPNAME [CONTEXTPREFIX] "
    ...
    fprintf(stderr,
            "        createAuth       GROUPNAME [CONTEXTPREFIX] SECURITYMODEL "
            "SECURITYLEVEL AUTHTYPE CONTEXTMATCH VIEWNAME\n");
    fprintf(stderr, "        deleteAuth       GROUPNAME [CONTEXTPREFIX] "
                    "SECURITYMODEL SECURITYLEVEL AUTHTYPE\n");
}

void auth_oid(oid* it, size_t* len, const char* groupName, const char* prefix,
              int model, int level, const char* authtype)
{
    int i;
...
void access_oid(oid* it, size_t* len, const char* groupName, const char* prefix,
                int model, int level)
{
    int i;
...
void sec2group_oid(oid* it, size_t* len, int model, const char* name)
{
    int i;
...
int main(int argc, char* argv[])
{
     ...
     */
    {
        ...
        snmp_pdu_add_variable(pdu, vacmGroupName, name_length, ASN_OCTET_STR,
                              (u_char*)argv[arg + 2], strlen(argv[arg + 2]));
    }
     ...
     */
    {
        ...
        access_oid(vacmAccessStatus, &name_length, groupName, prefix, secModel,
                   secLevel);
        longvar = RS_DESTROY;
        snmp_pdu_add_variable(pdu, vacmAccessStatus, name_length, ASN_INTEGER,
     ...
     *
     * createAccess  GROUPNAME [CONTEXTPREFIX] SECURITYMODEL SECURITYLEVEL
     * CONTEXTMATCH READVIEWNAME WRITEVIEWNAME NOTIFYVIEWNAME
     *
     */
    {
        if (++arg + 7 > argc)
        ...
        access_oid(vacmAccessWriteViewName, &name_length, groupName, prefix,
                   secModel, secLevel);
        snmp_pdu_add_variable(pdu, vacmAccessWriteViewName, name_length,
                              ASN_OCTET_STR, (u_char*)argv[arg + 5],
                              strlen(argv[arg + 5]));

        access_oid(vacmAccessNotifyViewName, &name_length, groupName, prefix,
                   secModel, secLevel);
     ...
     *
     * deleteAuth  GROUPNAME [CONTEXTPREFIX] SECURITYMODEL SECURITYLEVEL
     * AUTHTYPE
     *
     */
    {
        ...
        authtype = argv[arg + 3];
        auth_oid(nsVacmRowStatus, &name_length, groupName, prefix, secModel,
                 secLevel, authtype);
        longvar = RS_DESTROY;
        snmp_pdu_add_variable(pdu, nsVacmRowStatus, name_length, ASN_INTEGER,
                              (u_char*)&longvar, sizeof(longvar));
    }
     ...
     *
     * createAuth  GROUPNAME [CONTEXTPREFIX] SECURITYMODEL SECURITYLEVEL
     * AUTHTYPE CONTEXTMATCH VIEWNAME
     *
     */
    {
        if (++arg + 6 > argc)
        ...
        auth_oid(nsVacmContextPfx, &name_length, groupName, prefix, secModel,
                 secLevel, authtype);
        ...
        snmp_pdu_add_variable(pdu, nsVacmContextPfx, name_length, ASN_INTEGER,
                              (u_char*)&contextMatch, sizeof(contextMatch));

        auth_oid(nsVacmViewName, &name_length, groupName, prefix, secModel,
                 secLevel, authtype);
        snmp_pdu_add_variable(pdu, nsVacmViewName, name_length, ASN_OCTET_STR,
                              (u_char*)argv[arg + 5], strlen(argv[arg + 5]));
    }
    else
    {
