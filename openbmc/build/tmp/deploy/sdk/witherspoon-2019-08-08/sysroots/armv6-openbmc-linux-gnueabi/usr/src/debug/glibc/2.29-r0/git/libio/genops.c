...
libc_hidden_def(__overflow)
    ...
    static int save_for_backup(FILE* fp, char* end_p)
{
    ...
    if (needed_size > current_Bsize)
    {
        ...
        if (least_mark < 0)
        {
            __mempcpy(__mempcpy(new_buffer + avail,
                                fp->_IO_save_end + least_mark, -least_mark),
                      fp->_IO_read_base, end_p - fp->_IO_read_base);
        }
        else
            memcpy(new_buffer + avail, fp->_IO_read_base + least_mark,
                   needed_size);
        free(fp->_IO_save_base);
    ...
    else
    {
        ...
        if (least_mark < 0)
        {
            memmove(fp->_IO_save_base + avail, fp->_IO_save_end + least_mark,
                    -least_mark);
            memcpy(fp->_IO_save_base + avail - least_mark, fp->_IO_read_base,
                   end_p - fp->_IO_read_base);
        }
        else if (needed_size > 0)
            memcpy(fp->_IO_save_base + avail, fp->_IO_read_base + least_mark,
                   needed_size);
    }
...
libc_hidden_def(_IO_doallocbuf)
    ...
    int _IO_default_underflow(FILE* fp)
{
    return EOF;
}
...
libc_hidden_def(_IO_sgetn)
    ...
    size_t _IO_default_xsgetn(FILE* fp, void* data, size_t n)
{
    size_t more = n;
    char* s = (char*)data;
    for (;;)
...
libc_hidden_def(_IO_default_doallocate)
    ...
    void _IO_init_internal(FILE* fp, int flags)
{
...
void _IO_no_init(FILE* fp, int flags, int orientation, struct _IO_wide_data* wd,
...
libc_hidden_def(_IO_default_finish)
    ...
    off64_t _IO_default_seekoff(FILE* fp, off64_t offset, int dir, int mode)
{
    return _IO_pos_BAD;
}
...
libc_hidden_def(_IO_unsave_markers)
    ...
    int _IO_default_pbackfail(FILE* fp, int c)
{
...
libc_hidden_def(_IO_default_pbackfail)
    ...
    off64_t _IO_default_seek(FILE* fp, off64_t offset, int dir)
{
    return _IO_pos_BAD;
}
...
libc_hidden_def(_IO_iter_end)
    ...
    _IO_ITER _IO_iter_next(_IO_ITER iter)
{
    return iter->_chain;
}
libc_hidden_def(_IO_iter_next)

    FILE* _IO_iter_file(_IO_ITER iter)
{
    return iter;
}
