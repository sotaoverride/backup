...
void perl_matcher<BidiIterator, Allocator, traits>::construct_init(
    const basic_regex<char_type, traits>& e, match_flag_type f)
{
    typedef typename regex_iterator_traits<BidiIterator>::iterator_category
        category;
    typedef
        typename basic_regex<char_type, traits>::flag_type expression_flag_type;
    ...
    if (!(m_match_flags & (match_perl | match_posix)))
    {
        ...
        else if ((re_f & (regbase::main_option_type | regbase::emacs_ex)) ==
                 (regbase::basic_syntax_group | regbase::emacs_ex))
            m_match_flags |= match_perl;
        else if ((re_f & (regbase::main_option_type | regbase::literal)) ==
                 (regbase::literal))
            m_match_flags |= match_perl;
        else
...
template <class BidiIterator, class Allocator, class traits>
void perl_matcher<BidiIterator, Allocator, traits>::estimate_max_state_count(
...
template <class BidiIterator, class Allocator, class traits>
inline void
    perl_matcher<BidiIterator, Allocator, traits>::estimate_max_state_count(
        void*)
{
...
inline bool perl_matcher<BidiIterator, Allocator, traits>::match()
{
    ...
    return protected_call(
        &perl_matcher<BidiIterator, Allocator, traits>::match_imp);
#else
    return match_imp();
#endif
}
...
bool perl_matcher<BidiIterator, Allocator, traits>::match_imp()
{
    ...
    try
    {
        ...
        m_presult->set_size((m_match_flags & match_nosubs)
                                ? 1u
                                : static_cast<typename results_type::size_type>(
                                      1u + re.mark_count()),
                            search_base, last);
...
#ifdef BOOST_REGEX_HAS_MS_STACK_GUARD
    return protected_call(
        &perl_matcher<BidiIterator, Allocator, traits>::find_imp);
#else
    return find_imp();
#endif
}
...
bool perl_matcher<BidiIterator, Allocator, traits>::find_imp()
{
    ...
    try
    {
        ...
        else
        {
            ...
            // increment our start position, otherwise we go into an infinite
            // loop:
            if (((m_match_flags & match_not_null) == 0) &&
                (m_result.length() == 0))
            {
                if (position == last)
                    return false;
        ...
        if (m_match_flags & match_posix)
        {
            m_result.set_size(static_cast<typename results_type::size_type>(
                                  1u + re.mark_count()),
                              base, last);
        ...
        unsigned type =
            (m_match_flags & match_continuous)
...
bool perl_matcher<BidiIterator, Allocator, traits>::match_prefix()
{
    ...
    restart = position;
    match_all_states();
    if (!m_has_found_match && m_has_partial_match &&
        (m_match_flags & match_partial))
    {
    ...
    if (m_has_found_match && (match_extra & m_match_flags))
    {
        ...
        {
            typename sub_match<BidiIterator>::capture_sequence_type& seq =
                ((*m_presult)[i]).get_captures();
...
bool perl_matcher<BidiIterator, Allocator, traits>::match_literal()
{
    ...
    {
        if ((position == last) ||
            (traits_inst.translate(*position, icase) != what[i]))
            return false;
    }
...
bool perl_matcher<BidiIterator, Allocator, traits>::match_start_line()
{
    ...
    if (position != last)
    {
        if (is_separator(*t) && !((*t == static_cast<char_type>('\r')) &&
                                  (*position == static_cast<char_type>('\n'))))
        {
            pstate = pstate->next.p;
            return true;
        }
...
bool perl_matcher<BidiIterator, Allocator, traits>::match_wild()
{
    if (position == last)
        return false;
    if (is_separator(*position) &&
        ((match_any_mask & static_cast<const re_dot*>(pstate)->mask) == 0))
        return false;
...
{
    if (position == last)
        return false; // can't be starting a word if we're already at the end of
                      // input
...
bool perl_matcher<BidiIterator, Allocator, traits>::match_backref()
{
    ...
    while (i != j)
    {
        if ((position == last) || (traits_inst.translate(*position, icase) !=
                                   traits_inst.translate(*i, icase)))
            return false;
        ++i;
        ++position;
    }
...
bool perl_matcher<BidiIterator, Allocator, traits>::match_long_set()
{
    ...
    BidiIterator t = re_is_set_member(
        position, last,
        static_cast<const re_set_long<char_class_type>*>(pstate), re.get_data(),
        icase);
    if (t != position)
    {
        pstate = pstate->next.p;
...
bool perl_matcher<BidiIterator, Allocator, traits>::match_set()
{
    if (position == last)
        return false;
    if (static_cast<const re_set*>(pstate)->_map[static_cast<unsigned char>(
...
bool perl_matcher<BidiIterator, Allocator, traits>::match_combining()
{
    ...
    ++position;
    while ((position != last) &&
           is_combining(traits_inst.translate(*position, icase)))
        ++position;
    pstate = pstate->next.p;
    return true;
}
...
#pragma warning(disable : 4127)
    ...
    {
        std::ptrdiff_t maxlen =
            ::boost::BOOST_REGEX_DETAIL_NS::distance(backstop, position);
    ...
    perl_matcher<BidiIterator, Allocator, traits>::match_assert_backref()
{
    ...
    else if (index > 0)
    {
        ...
        if (index >= 10000)
        {
            named_subexpressions::range_type r =
                re.get_data().equal_range(index);
            while (r.first != r.second)
            {
    ...
    else
    {
        ...
        // If index == 0 then check for any recursion at all, otherwise for
        // recursion to -index-1.
        int idx = -(index + 1);
        if (idx >= 10000)
        {
            named_subexpressions::range_type r = re.get_data().equal_range(idx);
            int stack_index =
                recursion_stack.empty() ? -1 : recursion_stack.back().idx;
            while (r.first != r.second)
            {
        ...
        {
            result = !recursion_stack.empty() &&
                     ((recursion_stack.back().idx == idx) || (index == 0));
        }
        pstate = pstate->next.p;
    }
    return result;
...
#pragma warning(disable : 4127)
    ...
    {
        while ((position != last) &&
               traits_inst.isctype(*position, m_word_mask))
            ++position;
        while ((position != last) &&
               !traits_inst.isctype(*position, m_word_mask))
            ++position;
