...
OM_uint32 KRB5_CALLCONV gss_inquire_cred(minor_status, cred_handle, name,
...

{
    ...
    if (name != NULL || lifetime != NULL || cred_usage != NULL)
    {
        ...
        if (name)
        {
            ...
            status = gssint_convert_name_to_union_name(&temp_minor_status, mech,
                                                       mech_name, name);
            if (status != GSS_S_COMPLETE)
            {
    ...
    if (mechanisms != NULL)
    {
        ...
        else
        {
            ...
            status =
                gss_add_oid_set_member(minor_status, &mech->mech_type, &mechs);
            if (GSS_ERROR(status))
                goto error;
        }
        *mechanisms = mechs;
...
gss_cred_usage_t* cred_usage;
{
    ...
    status = mech->gss_inquire_cred_by_mech(
        minor_status, mech_cred, public_mech, name ? &internal_name : NULL,
