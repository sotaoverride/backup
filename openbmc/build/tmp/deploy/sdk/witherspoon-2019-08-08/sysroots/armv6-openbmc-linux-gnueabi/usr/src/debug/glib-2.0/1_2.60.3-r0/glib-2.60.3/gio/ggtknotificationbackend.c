...
#define G_GTK_NOTIFICATION_BACKEND(o)                                          \
    (G_TYPE_CHECK_INSTANCE_CAST((o), G_TYPE_GTK_NOTIFICATION_BACKEND,          \
...
G_DEFINE_TYPE_WITH_CODE(
    GGtkNotificationBackend, g_gtk_notification_backend,
...
static gboolean g_gtk_notification_backend_is_supported(void)
{
    ...
    reply = g_dbus_connection_call_sync(
        ...
        g_variant_new("(s)", "org.gtk.Notifications"), G_VARIANT_TYPE("(s)"),
...
static void g_gtk_notification_backend_send_notification(
    GNotificationBackend* backend, const gchar* id, GNotification* notification)
{
    GVariant* params;

    params = g_variant_new(
        "(ss@a{sv})", g_application_get_application_id(backend->application),
        id, g_notification_serialize(notification));

    g_dbus_connection_call(backend->dbus_connection, "org.gtk.Notifications",
                           "/org/gtk/Notifications", "org.gtk.Notifications",
                           "AddNotification", params, G_VARIANT_TYPE_UNIT,
                           G_DBUS_CALL_FLAGS_NONE, -1, NULL, NULL, NULL);
}

static void g_gtk_notification_backend_withdraw_notification(
    GNotificationBackend* backend, const gchar* id)
{
    GVariant* params;

    params = g_variant_new(
        "(ss)", g_application_get_application_id(backend->application), id);
...
static void g_gtk_notification_backend_init(GGtkNotificationBackend* backend)
{
}
    ...
    g_gtk_notification_backend_class_init(GGtkNotificationBackendClass* class)
{
    ...
    backend_class->send_notification =
        g_gtk_notification_backend_send_notification;
    backend_class->withdraw_notification =
        g_gtk_notification_backend_withdraw_notification;
}
