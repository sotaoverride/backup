...
class object_cache
{
  public:
    ...
    static boost::shared_ptr<Object const> get(const Key& k,
                                               size_type l_max_cache_size);

  private:
    static boost::shared_ptr<Object const> do_get(const Key& k,
                                                  size_type l_max_cache_size);
...
boost::shared_ptr<Object const>
    object_cache<Key, Object>::get(const Key& k, size_type l_max_cache_size)
{
#ifdef BOOST_HAS_THREADS
...
#if defined(BOOST_NO_UNREACHABLE_RETURN_DETECTION) ||                          \
...
template <class Key, class Object>
boost::shared_ptr<Object const>
    object_cache<Key, Object>::do_get(const Key& k, size_type l_max_cache_size)
{
    ...
    if (s > l_max_cache_size)
    {
        ...
        BOOST_ASSERT(s_data.index[k]->first.get() == result.get());
        BOOST_ASSERT(&(s_data.index.find(k)->first) ==
