...
static void show_progress(goffset current_num_bytes, goffset total_num_bytes,
                          gpointer user_data)
{
    gint64 tv;
    ...
    g_print("\r\033[K");
    g_print(_("Transferred %s out of %s (%s/s)"), current_size, total_size,
            rate);
...
int handle_copy(int argc, char* argv[], gboolean do_help)
{
    ...
    g_option_context_set_summary(
        context, _("Copy one or more files from SOURCE to DESTINATION."));
    ...
    if (!dest_is_dir && argc > 3)
    {
        ...
        message = g_strdup_printf(_("Destination %s is not a directory"),
                                  argv[argc - 1]);
    ...
    for (i = 1; i < argc - 1; i++)
    {
        ...
        if (!g_file_copy(source, target, flags, NULL,
                         progress ? show_progress : NULL, NULL, &error))
        {
            if (interactive &&
                g_error_matches(error, G_IO_ERROR, G_IO_ERROR_EXISTS))
            {
                    ...
                    (line[0] == 'y' || line[0] == 'Y'))
                {
                    ...
                    start_time = g_get_monotonic_time();
                    if (!g_file_copy(source, target, flags, NULL,
                                     progress ? show_progress : NULL, NULL,
                                     &error))
                        goto copy_failed;
                }
            }
