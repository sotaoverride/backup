...
krb5_boolean fowner(fp, uid) FILE* fp;
uid_t uid;
{
    struct stat sbuf;
...
krb5_error_code krb5_authorization(context, principal, luser, cmd, ok, out_fcmd)
    /* IN */
...
char** out_fcmd;
{
    ...
    *ok = FALSE;
    ...
    if (k5login_flag && k5users_flag)
    {
        ...
        if (!(krb5_aname_to_localname(context, principal, strlen(princname),
                                      kuser)) &&
...
krb5_error_code k5login_lookup(fp, princname, found) FILE* fp;
char* princname;
krb5_boolean* found;
{
...
***********************************************************/
krb5_error_code k5users_lookup(fp, princname, cmd, found, out_fcmd) FILE* fp;
char* princname;
char* cmd;
krb5_boolean* found;
char** out_fcmd;
...
krb5_boolean fcmd_resolve(fcmd, out_fcmd, out_err) char* fcmd;
char*** out_fcmd;
char** out_err;
{
    char* err;
    ...
    else
    {
        ...
        {
            asprintf(&err,
                     _("Error: bad entry - %s in %s file, must be "
...
#ifndef CMD_PATH
        asprintf(&err,
                 _("Error: bad entry - %s in %s file, since %s is just "
        ...
        *out_err = err;
        ...
        {
            asprintf(&err,
                     _("Error: bad entry - %s in %s file, CMD_PATH "
                       "contains no paths \n"),
                     fcmd, KRB5_USERS_NAME);
            *out_err = err;
            return FALSE;
        ...
        do
        {
            ...
            { /* must be full path */
                asprintf(&err,
                         _("Error: bad path %s in CMD_PATH for %s must "
                           "start with '/' \n"),
                         tc, KRB5_USERS_NAME);
                *out_err = err;
                return FALSE;
...
krb5_boolean cmd_single(cmd) char* cmd;
{
...
int cmd_arr_cmp_postfix(fcmd_arr, cmd) char** fcmd_arr;
char* cmd;
{
    char* temp_fcmd;
    char* ptr;
...
int cmd_arr_cmp(fcmd_arr, cmd) char** fcmd_arr;
char* cmd;
{
    int result = 1;
    int i = 0;
...
krb5_boolean find_first_cmd_that_exists(fcmd_arr, cmd_out,
                                        err_out) char** fcmd_arr;
char** cmd_out;
char** err_out;
{
    struct stat st_temp;
...
int match_commands(fcmd, cmd, match, cmd_out, err_out) char* fcmd;
char* cmd;
krb5_boolean* match;
char** cmd_out;
char** err_out;
...
char* get_first_token(line, lnext) char* line;
char** lnext;
{
...
char* get_next_token(lnext) char** lnext;
{
    char *lptr, *out_ptr;
...
static void auth_cleanup(users_fp, login_fp, princname) FILE* users_fp;
FILE* login_fp;
char* princname;
{
...
void init_auth_names(pw_dir) char* pw_dir;
{
    const char* sep;
    int r1, r2;

    sep = ((strlen(pw_dir) == 1) && (*pw_dir == '/')) ? "" : "/";
    r1 = snprintf(k5login_path, sizeof(k5login_path), "%s%s%s", pw_dir, sep,
                  KRB5_LOGIN_NAME);
    r2 = snprintf(k5users_path, sizeof(k5users_path), "%s%s%s", pw_dir, sep,
                  KRB5_USERS_NAME);
    if (SNPRINTF_OVERFLOW(r1, sizeof(k5login_path)) ||
        SNPRINTF_OVERFLOW(r2, sizeof(k5users_path)))
    {
        fprintf(stderr,
                _("home directory name `%s' too long, can't search "
                  "for .k5login\n"),
                pw_dir);
        exit(1);
    }
