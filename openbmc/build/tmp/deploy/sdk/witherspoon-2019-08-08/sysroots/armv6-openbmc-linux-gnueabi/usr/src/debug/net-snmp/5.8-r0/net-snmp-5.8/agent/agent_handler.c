 ...
 *  @note This ends up calling netsnmp_create_handler(name,
...
netsnmp_handler_registration* netsnmp_handler_registration_create(
    const char* name, netsnmp_mib_handler* handler, const oid* reg_oid,
    ...
    const oid* reg_oid, size_t reg_oid_len, int modes)
{
    ...
    if (handler)
    {
        rv = netsnmp_handler_registration_create(name, handler, reg_oid,
                                                 reg_oid_len, modes);
        if (!rv)
...
int netsnmp_register_handler(netsnmp_handler_registration* reginfo)
{
    ...
    {
        DEBUGMSGTL(
            ("handler::register", "Registering %s (", reginfo->handlerName));
        ...
        {
            DEBUGMSGOID(
                ("handler::register", reginfo->rootoid, reginfo->rootoid_len));
        }
        else
        {
    ...
    if (0 == reginfo->modes)
    {
        ...
        snmp_log(LOG_WARNING,
                 "no registration modes specified for %s. "
                 "Defaulting to 0x%x\n",
    ...
    return netsnmp_register_mib(
        reginfo->handlerName, NULL, 0, 0, reginfo->rootoid,
        reginfo->rootoid_len, reginfo->priority, reginfo->range_subid,
        reginfo->range_ubound, NULL, reginfo->contextName, reginfo->timeout,
        flags, reginfo, 1);
}
...
int netsnmp_register_handler_nocallback(netsnmp_handler_registration* reginfo)
{
    ...
    return netsnmp_register_mib(
        reginfo->handler->handler_name, NULL, 0, 0, reginfo->rootoid,
        reginfo->rootoid_len, reginfo->priority, reginfo->range_subid,
        reginfo->range_ubound, NULL, reginfo->contextName, reginfo->timeout, 0,
        reginfo, 0);
}
 ...
 *  @return Returns SNMPERR_SUCCESS or SNMP_ERR_* error code.
 */
NETSNMP_INLINE int netsnmp_call_handler(netsnmp_mib_handler* next_handler,
...
NETSNMP_INLINE int netsnmp_call_next_handler(
    netsnmp_mib_handler* current, netsnmp_handler_registration* reginfo,
    ...
    netsnmp_handler_registration_dup(netsnmp_handler_registration* reginfo)
{
    ...
    r = (netsnmp_handler_registration*)calloc(
                                      ...
                                      netsnmp_mib_handler* handler)
{
    netsnmp_add_list_data(
        &handler_reg,
        netsnmp_create_data_list(name, (void*)handler, handler_free_callback));
 ...
 */
void netsnmp_inject_handler_into_subtree(netsnmp_subtree* tp, const char* name,
                                         netsnmp_mib_handler* handler,
                                         const char* before_what)
{
    ...
    for (tptr = tp; tptr != NULL; tptr = tptr->next)
    {
        ...
        if (strcmp(tptr->label_a, name) == 0)
        {
            ...
            netsnmp_inject_handler_before(tptr->reginfo,
                                          _clone_handler(handler), before_what);
        }
        else if (tptr->reginfo != NULL && tptr->reginfo->handlerName != NULL &&
            ...
            for (mh = tptr->reginfo->handler; mh != NULL; mh = mh->next)
            {
                if (mh->handler_name && strcmp(mh->handler_name, name) == 0)
                {
                    ...
                    netsnmp_inject_handler_before(
                        tptr->reginfo, _clone_handler(handler), before_what);
                    break;
                }
                else
                {
...
void parse_injectHandler_conf(const char* token, char* cptr)
{
    ...
    handler = (netsnmp_mib_handler*)netsnmp_get_list_data(handler_reg,
                                                          handler_to_insert);
    if (!handler)
    {
 ...
 */
static int handler_mark_inject_handler_done(int majorID, int minorID,
...
void netsnmp_init_handler_conf(void)
{
    snmpd_register_config_handler(
        "injectHandler", parse_injectHandler_conf, NULL,
    ...
    se_add_pair_to_slist("babystep_mode", strdup("undo_set"),
                         MODE_BSTEP_UNDO_SET);
    se_add_pair_to_slist("babystep_mode", strdup("commit"), MODE_BSTEP_COMMIT);
    ...
    se_add_pair_to_slist("handler_can_mode", strdup("GET/GETNEXT"),
                         HANDLER_CAN_GETANDGETNEXT);
    se_add_pair_to_slist("handler_can_mode", strdup("SET"), HANDLER_CAN_SET);
