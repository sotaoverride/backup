...
#define error_errno(_offset, error, _fmt, ...)                                 \
    ...
    {                                                                          \
        flush_progress();                                                      \
        log_error_errno(error, OFSfmt ": " _fmt, (uint64_t)_offset,            \
                        ##__VA_ARGS__);                                        \
    } while (0)

static int journal_file_object_verify(JournalFile* f, uint64_t offset,
                                      Object* o)
{
    uint64_t i;
    ...
    {
        error(offset, "Found compressed object that isn't of type DATA, which "
                      "is not allowed.");
        return -EBADMSG;
    }
    ...
    switch (o->object.type)
    {
        ...
        case OBJECT_DATA:
        {
            ...
            {
                error(offset, "Bad n_entries: %" PRIu64,
            ...
            if (le64toh(o->object.size) - offsetof(DataObject, payload) <= 0)
            {
                error(offset, "Bad object size (<= %zu): %" PRIu64,
                      offsetof(DataObject, payload), le64toh(o->object.size));
                return -EBADMSG;
            }
            ...
            if (compression)
            {
                ...
                r = decompress_blob(compression, o->data.payload,
                ...
                h2 =
                    hash64(o->data.payload, le64toh(o->object.size) -
                                                offsetof(Object, data.payload));

            if (h1 != h2)
            {
                error(offset, "Invalid hash (%08" PRIx64 " vs. %08" PRIx64, h1,
                      h2);
                return -EBADMSG;
            }
        ...
        case OBJECT_FIELD:
            ...
            {
                error(offset, "Bad field size (<= %zu): %" PRIu64,
                      offsetof(FieldObject, payload), le64toh(o->object.size));
                return -EBADMSG;
            }
        ...
        case OBJECT_ENTRY:
            if ((le64toh(o->object.size) - offsetof(EntryObject, items)) %
                ...
                0)
            {
                error(offset, "Bad entry size (<= %zu): %" PRIu64,
                      offsetof(EntryObject, items), le64toh(o->object.size));
                return -EBADMSG;
            }
            ...
            if ((le64toh(o->object.size) - offsetof(EntryObject, items)) /
                ...
                0)
            {
                error(offset, "Invalid number items in entry: %" PRIu64,
            ...
            {
                error(offset, "Invalid entry seqnum: %" PRIx64,
            ...
            {
                error(offset, "Invalid entry realtime timestamp: %" PRIu64,
            ...
            {
                error(offset, "Invalid entry monotonic timestamp: %" PRIu64,
            ...
            for (i = 0; i < journal_file_entry_n_items(o); i++)
            {
                ...
                {
                    error(offset,
                          "Invalid entry item (%" PRIu64 "/%" PRIu64
                          " offset: " OFSfmt,
        ...
        case OBJECT_FIELD_HASH_TABLE:
            if ((le64toh(o->object.size) - offsetof(HashTableObject, items)) %
                ...
                (le64toh(o->object.size) - offsetof(HashTableObject, items)) /
                    ...
                    0)
            {
                error(offset, "Invalid %s hash table size: %" PRIu64,
                      o->object.type == OBJECT_DATA_HASH_TABLE ? "data"
                                                               : "field",
                      le64toh(o->object.size));
                return -EBADMSG;
            }
        ...
        case OBJECT_ENTRY_ARRAY:
            if ((le64toh(o->object.size) - offsetof(EntryArrayObject, items)) %
                ...
                (le64toh(o->object.size) - offsetof(EntryArrayObject, items)) /
                    ...
                    0)
            {
                error(offset, "Invalid object entry array size: %" PRIu64,
                      le64toh(o->object.size));
                return -EBADMSG;
            }
            ...
            if (!VALID64(le64toh(o->entry_array.next_entry_array_offset)))
            {
                error(offset,
                      ...
                      "next_entry_array_offset: " OFSfmt,
            ...
            for (i = 0; i < journal_file_entry_array_n_items(o); i++)
                ...
                {
                    error(offset,
                          "Invalid object entry array item (%" PRIu64
                          "/%" PRIu64 "): " OFSfmt,
        ...
        case OBJECT_TAG:
            ...
            {
                error(offset, "Invalid object tag size: %" PRIu64,
                      le64toh(o->object.size));
                return -EBADMSG;
            }
            ...
            {
                error(offset, "Invalid object tag epoch: %" PRIu64,
                      le64toh(o->tag.epoch));
                return -EBADMSG;
            }
...
static int contains_uint64(MMapCache* m, MMapFileDescriptor* f, uint64_t n,
                           uint64_t p)
{
    uint64_t a, b;
    int r;
    ...
    while (a < b)
    {
        ...
        r = mmap_cache_get(m, f, PROT_READ | PROT_WRITE, 0, false,
                           c * sizeof(uint64_t), sizeof(uint64_t), NULL,
                           (void**)&z, NULL);
        if (r < 0)
            return r;
...
static int entry_points_to_data(JournalFile* f,
                                ...
                                uint64_t n_entries, uint64_t entry_p,
                                uint64_t data_p)
{
    ...
    {
        error(data_p, "Data object references invalid entry at " OFSfmt,
              entry_p);
        return -EBADMSG;
    }
    ...
    {
        error(entry_p,
              "Data object at " OFSfmt " not referenced by linked entry",
              data_p);
        return -EBADMSG;
    }
...
static int verify_data(JournalFile* f, Object* o, uint64_t p,
                       ...
                       uint64_t n_entry_arrays)
{
    ...
    while (i < n)
    {
        ...
        {
            error(p,
                  "Array chain has cycle (jumps back from " OFSfmt " to " OFSfmt
                  ")",
                  a, next);
            return -EBADMSG;
        }
...
static int verify_hash_table(JournalFile* f, MMapFileDescriptor* cache_data_fd,
                             uint64_t n_data,
                             MMapFileDescriptor* cache_entry_fd,
                             uint64_t n_entries,
                             MMapFileDescriptor* cache_entry_array_fd,
                             uint64_t n_entry_arrays, usec_t* last_usec,
                             bool show_progress)
{
...
static int verify_entry(JournalFile* f, Object* o, uint64_t p,
...
static int verify_entry_array(JournalFile* f, MMapFileDescriptor* cache_data_fd,
                              uint64_t n_data,
                              MMapFileDescriptor* cache_entry_fd,
                              uint64_t n_entries,
                              MMapFileDescriptor* cache_entry_array_fd,
                              ...
                              bool show_progress)
{
    ...
    while (i < n)
    {
        ...
        for (j = 0; i < n && j < m; i++, j++)
        {
            ...
            if (p <= last)
            {
                error(a, "Entry array not sorted at %" PRIu64 " of %" PRIu64, i,
                      n);
                return -EBADMSG;
            }
            last = p;
            ...
            {
                error(a, "Invalid array entry at %" PRIu64 " of %" PRIu64, i,
                      n);
                return -EBADMSG;
            }
...
int journal_file_verify(JournalFile* f, const char* key,
                        usec_t* first_contained, usec_t* last_validated,
                        usec_t* last_contained, bool show_progress)
{
    int r;
    Object* o;
    uint64_t p = 0, last_epoch = 0, last_tag_realtime = 0,
             last_sealed_realtime = 0;

    uint64_t entry_seqnum = 0, entry_monotonic = 0, entry_realtime = 0;
    sd_id128_t entry_boot_id;
    bool entry_seqnum_set = false, entry_monotonic_set = false,
         entry_realtime_set = false, found_main_entry_array = false;
    uint64_t n_weird = 0, n_objects = 0, n_entries = 0, n_data = 0,
             n_fields = 0, n_data_hash_tables = 0, n_field_hash_tables = 0,
             n_entry_arrays = 0, n_tags = 0;
    usec_t last_usec = 0;
    int data_fd = -1, entry_fd = -1, entry_array_fd = -1;
    MMapFileDescriptor *cache_data_fd = NULL, *cache_entry_fd = NULL,
                       *cache_entry_array_fd = NULL;
    unsigned i;
    ...
    for (;;)
    {
        ...
        if ((o->object.flags & OBJECT_COMPRESSED_XZ) &&
            !JOURNAL_HEADER_COMPRESSED_XZ(f->header))
        {
        ...
        if ((o->object.flags & OBJECT_COMPRESSED_LZ4) &&
            !JOURNAL_HEADER_COMPRESSED_LZ4(f->header))
        {
        ...
        switch (o->object.type)
        {
            ...
            case OBJECT_DATA_HASH_TABLE:
                ...
                if (le64toh(f->header->data_hash_table_offset) !=
                        p + offsetof(HashTableObject, items) ||
                    le64toh(f->header->data_hash_table_size) !=
                        le64toh(o->object.size) -
                            offsetof(HashTableObject, items))
                {
            ...
            case OBJECT_TAG:
                ...
                if (f->seal)
                {
                    ...
                    debug(p, "Checking tag %" PRIu64 "...",
                          le64toh(o->tag.seqnum));

                    rt = f->fss_start_usec +
                         le64toh(o->tag.epoch) * f->fss_interval_usec;
                    if (entry_realtime_set &&
                        entry_realtime >= rt + f->fss_interval_usec)
                    {
                        error(p, "tag/entry realtime timestamp out of "
                                 "synchronization");
                        r = -EBADMSG;
                        goto fail;
                    }
                    ...
                    while (q <= p)
                    {
                        r = journal_file_move_to_object(f, OBJECT_UNUSED, q,
                                                        &o);
                        if (r < 0)
                            goto fail;

                        r = journal_file_hmac_put_object(f, OBJECT_UNUSED, o,
                                                         q);
                        if (r < 0)
                            goto fail;
                    ...
                    if (memcmp(o->tag.tag, gcry_md_read(f->hmac, 0),
                               TAG_LENGTH) != 0)
                    {
    ...
    if (entry_realtime_set &&
        entry_realtime != le64toh(f->header->tail_entry_realtime))
    {
    ...
    r = verify_entry_array(f, cache_data_fd, n_data, cache_entry_fd, n_entries,
                           cache_entry_array_fd, n_entry_arrays, &last_usec,
                           show_progress);
    if (r < 0)
        goto fail;

    r = verify_hash_table(f, cache_data_fd, n_data, cache_entry_fd, n_entries,
                          cache_entry_array_fd, n_entry_arrays, &last_usec,
                          show_progress);
    if (r < 0)
        goto fail;
...
fail:
    ...
    log_error("File corruption detected at %s:" OFSfmt
              " (of %llu bytes, %" PRIu64 "%%).",
              f->path, p, (unsigned long long)f->last_stat.st_size,
