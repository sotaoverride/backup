...
/* nspawn is listening on the socket at the path in the constant
 * nspawn_notify_socket_path nspawn_notify_socket_path is relative to the
 * container the init process in the container pid can send messages to nspawn
...
static char* arg_machine =
    NULL; /* The name used by the host to refer to this */
...
static uint64_t arg_caps_retain =
    (1ULL << CAP_AUDIT_CONTROL) | (1ULL << CAP_AUDIT_WRITE) |
    (1ULL << CAP_CHOWN) | (1ULL << CAP_DAC_OVERRIDE) |
    (1ULL << CAP_DAC_READ_SEARCH) | (1ULL << CAP_FOWNER) |
    (1ULL << CAP_FSETID) | (1ULL << CAP_IPC_OWNER) | (1ULL << CAP_KILL) |
    (1ULL << CAP_LEASE) | (1ULL << CAP_LINUX_IMMUTABLE) | (1ULL << CAP_MKNOD) |
    (1ULL << CAP_NET_BIND_SERVICE) | (1ULL << CAP_NET_BROADCAST) |
    (1ULL << CAP_NET_RAW) | (1ULL << CAP_SETFCAP) | (1ULL << CAP_SETGID) |
    (1ULL << CAP_SETPCAP) | (1ULL << CAP_SETUID) | (1ULL << CAP_SYS_ADMIN) |
    (1ULL << CAP_SYS_BOOT) | (1ULL << CAP_SYS_CHROOT) | (1ULL << CAP_SYS_NICE) |
    (1ULL << CAP_SYS_PTRACE) | (1ULL << CAP_SYS_RESOURCE) |
...
static unsigned long arg_clone_ns_flags =
    CLONE_NEWIPC | CLONE_NEWPID | CLONE_NEWUTS;
static MountSettingsMask arg_mount_settings =
    MOUNT_APPLY_APIVFS_RO | MOUNT_APPLY_TMPFS_TMP;
...
static unsigned arg_console_width = (unsigned)-1,
                arg_console_height = (unsigned)-1;
...
static int help(void)
{
    ...
    printf(
        "%1$s [OPTIONS...] [PATH] [ARGUMENTS...]\n\n"
        "Spawn a command or OS in a light-weight container.\n\n"
        "  -h --help                 Show this help\n"
        "     --version              Print version string\n"
        "  -q --quiet                Do not show status information\n"
        "     --no-pager             Do not pipe output into a pager\n"
        "     --settings=BOOLEAN     Load additional settings from .nspawn "
        "file\n\n"
        "%3$sImage:%4$s\n"
        "  -D --directory=PATH       Root directory for the container\n"
        "     --template=PATH        Initialize root directory from template "
        "directory,\n"
        "                            if missing\n"
        "  -x --ephemeral            Run container with snapshot of root "
        "directory, and\n"
        "                            remove it after exit\n"
        "  -i --image=PATH           Root file system disk image (or device "
        "node) for\n"
        "                            the container\n"
        "     --oci-bundle=PATH      OCI bundle directory\n"
        "     --read-only            Mount the root directory read-only\n"
        "     --volatile[=MODE]      Run the system in volatile mode\n"
        "     --root-hash=HASH       Specify verity root hash for root disk "
        "image\n"
        "     --pivot-root=PATH[:PATH]\n"
        "                            Pivot root to given directory in the "
        "container\n\n"
        "%3$sExecution:%4$s\n"
        "  -a --as-pid2              Maintain a stub init as PID1, invoke "
        "binary as PID2\n"
        "  -b --boot                 Boot up full system (i.e. invoke init)\n"
        "     --chdir=PATH           Set working directory in the container\n"
        "  -E --setenv=NAME=VALUE    Pass an environment variable to PID 1\n"
        "  -u --user=USER            Run the command under specified user or "
        "UID\n"
        "     --kill-signal=SIGNAL   Select signal to use for shutting down "
        "PID 1\n"
        "     --notify-ready=BOOLEAN Receive notifications from the child init "
        "process\n\n"
        "%3$sSystem Identity:%4$s\n"
        "  -M --machine=NAME         Set the machine name for the container\n"
        "     --hostname=NAME        Override the hostname for the container\n"
        "     --uuid=UUID            Set a specific machine UUID for the "
        "container\n\n"
        "%3$sProperties:%4$s\n"
        "  -S --slice=SLICE          Place the container in the specified "
        "slice\n"
        ...
        "  -U --private-users=pick   Run within user namespace, autoselect "
        "UID/GID range\n"
        "     --private-users[=UIDBASE[:NUIDS]]\n"
        "                            Similar, but with user configured UID/GID "
        "range\n"
        "     --private-users-chown  Adjust OS tree ownership to private "
        "UID/GID range\n\n"
        "%3$sNetworking:%4$s\n"
        "     --private-network      Disable network in container\n"
        "     --network-interface=INTERFACE\n"
        "                            Assign an existing network interface to "
        "the\n"
        "                            container\n"
        "     --network-macvlan=INTERFACE\n"
        "                            Create a macvlan network interface based "
        "on an\n"
        "                            existing network interface to the "
        "container\n"
        "     --network-ipvlan=INTERFACE\n"
        "                            Create a ipvlan network interface based "
        "on an\n"
        "                            existing network interface to the "
        "container\n"
        "  -n --network-veth         Add a virtual Ethernet connection between "
        "host\n"
        "                            and container\n"
        "     --network-veth-extra=HOSTIF[:CONTAINERIF]\n"
        "                            Add an additional virtual Ethernet link "
        "between\n"
        "                            host and container\n"
        "     --network-bridge=INTERFACE\n"
        "                            Add a virtual Ethernet connection to the "
        "container\n"
        "                            and attach it to an existing bridge on "
        "the host\n"
        "     --network-zone=NAME    Similar, but attach the new interface to "
        "an\n"
        "                            an automatically managed bridge "
        "interface\n"
        "     --network-namespace-path=PATH\n"
        "                            Set network namespace to the one "
        "represented by\n"
        "                            the specified kernel namespace file node\n"
        "  -p --port=[PROTOCOL:]HOSTPORT[:CONTAINERPORT]\n"
        "                            Expose a container IP port on the host\n\n"
        "%3$sSecurity:%4$s\n"
        "     --capability=CAP       In addition to the default, retain "
        "specified\n"
        "                            capability\n"
        "     --drop-capability=CAP  Drop the specified capability from the "
        "default set\n"
        "     --no-new-privileges    Set PR_SET_NO_NEW_PRIVS flag for "
        "container payload\n"
        "     --system-call-filter=LIST|~LIST\n"
        "                            Permit/prohibit specific system calls\n"
        "  -Z --selinux-context=SECLABEL\n"
        "                            Set the SELinux security context to be "
        "used by\n"
        "                            processes in the container\n"
        "  -L --selinux-apifs-context=SECLABEL\n"
        "                            Set the SELinux security context to be "
        "used by\n"
        "                            API/tmpfs file systems in the "
        "container\n\n"
        "%3$sResources:%4$s\n"
        "     --rlimit=NAME=LIMIT    Set a resource limit for the payload\n"
        "     --oom-score-adjust=VALUE\n"
        "                            Adjust the OOM score value for the "
        "payload\n"
        "     --cpu-affinity=CPUS    Adjust the CPU affinity of the container\n"
        "     --personality=ARCH     Pick personality for this container\n\n"
        "%3$sIntegration:%4$s\n"
        "     --resolv-conf=MODE     Select mode of /etc/resolv.conf "
        "initialization\n"
        "     --timezone=MODE        Select mode of /etc/localtime "
        "initialization\n"
        "     --link-journal=MODE    Link up guest journal, one of no, auto, "
        "guest, \n"
        "                            host, try-guest, try-host\n"
        "  -j                        Equivalent to --link-journal=try-guest\n\n"
        "%3$sMounts:%4$s\n"
        "     --bind=PATH[:PATH[:OPTIONS]]\n"
        "                            Bind mount a file or directory from the "
        "host into\n"
        "                            the container\n"
        "     --bind-ro=PATH[:PATH[:OPTIONS]\n"
        "                            Similar, but creates a read-only bind "
        "mount\n"
        "     --inaccessible=PATH    Over-mount file node with inaccessible "
        "node to mask\n"
        "                            it\n"
        "     --tmpfs=PATH:[OPTIONS] Mount an empty tmpfs to the specified "
        "directory\n"
        ...
        "                            Similar, but creates a read-only overlay "
        "mount\n\n"
        "%3$sInput/Output:%4$s\n"
        "     --console=MODE         Select how stdin/stdout/stderr and "
        "/dev/console are\n"
            ...
            arg_userns_mode != USER_NAMESPACE_NO)
        {
            if (arg_userns_chown)
                return log_error_errno(SYNTHETIC_ERRNO(EINVAL),
                                       "--private-users-chown may not be "
                                       "combined with custom root mounts.");
            else if (arg_uid_shift == UID_INVALID)
                return log_error_errno(
                    SYNTHETIC_ERRNO(EINVAL),
                    "--private-users with automatic UID shift may not be "
                    "combined with custom root mounts.");
        }
    }
...
static int detect_unified_cgroup_hierarchy_from_environment(void)
{
    ...
    if (e)
    {
        ...
        if (r < 0)
            return log_error_errno(
                r, "Failed to parse $UNIFIED_CGROUP_HIERARCHY.");
        if (r > 0)
...
static int detect_unified_cgroup_hierarchy_from_image(const char* directory)
{
    ...
    /* Let's inherit the mode to use from the host system, but let's take into
     * consideration what systemd in the image actually supports. */
    r = cg_all_unified();
    if (r < 0)
        return log_error_errno(
            r, "Failed to determine whether we are in all unified mode.");
    if (r > 0)
    {
        /* Unified cgroup hierarchy support was added in 230. Unfortunately the
         * detection routine only detects 231, so we'll have a false negative
         * here for 230. */
        r = systemd_installation_has_version(directory, 230);
        if (r < 0)
            return log_error_errno(
                r, "Failed to determine systemd version in container: %m");
        if (r > 0)
            arg_unified_cgroup_hierarchy = CGROUP_UNIFIED_ALL;
        else
            arg_unified_cgroup_hierarchy = CGROUP_UNIFIED_NONE;
...
static void parse_share_ns_env(const char* name, unsigned long ns_flag)
{
    ...
    if (r < 0)
        log_warning_errno(
            r, "Failed to parse %s from environment, defaulting to false.",
            name);

    arg_clone_ns_flags =
        (arg_clone_ns_flags & ~ns_flag) | (r > 0 ? 0 : ns_flag);
...
static void parse_mount_settings_env(void)
{
    ...
    if (r >= 0)
        ...
        log_warning_errno(
            r, "Failed to parse $SYSTEMD_NSPAWN_TMPFS_TMP, ignoring: %m");
    ...
    if (r < 0)
    {
        log_warning_errno(r, "Failed to parse SYSTEMD_NSPAWN_API_VFS_WRITABLE "
                             "from environment, ignoring.");
        return;
    }
...
static int parse_argv(int argc, char* argv[])
{
    ...
    static const struct option options[] = {
        ...
        {"share-system", no_argument, NULL,
         ARG_SHARE_SYSTEM}, /* not documented */
        ...
        {"network-namespace-path", required_argument, NULL,
         ARG_NETWORK_NAMESPACE_PATH},
        ...
        {"private-users-chown", optional_argument, NULL,
         ARG_PRIVATE_USERS_CHOWN},
    ...
    while ((c = getopt_long(argc, argv, "+hD:u:abL:M:jS:Z:qi:xp:nUE:P", options,
                            NULL)) >= 0)
        switch (c)
        {
            ...
            case ARG_OCI_BUNDLE:
                r = parse_path_argument_and_warn(optarg, false,
                                                 &arg_oci_bundle);
                if (r < 0)
                    return r;
            ...
            case ARG_NETWORK_VETH_EXTRA:
                ...
                if (r < 0)
                    return log_error_errno(
                        r,
                        "Failed to parse --network-veth-extra= parameter: %s",
                        optarg);
            ...
            case ARG_DROP_CAPABILITY:
            {
                ...
                for (;;)
                {
                    ...
                    if (r < 0)
                        return log_error_errno(
                            r, "Failed to parse capability %s.", t);
                    if (r == 0)
                        break;
            ...
            case ARG_NO_NEW_PRIVILEGES:
                ...
                if (r < 0)
                    return log_error_errno(
                        r, "Failed to parse --no-new-privileges= argument: %s",
                        optarg);
            ...
            case ARG_LINK_JOURNAL:
                r = parse_link_journal(optarg, &arg_link_journal,
            ...
            case ARG_BIND:
            case ARG_BIND_RO:
                r = bind_mount_parse(&arg_custom_mounts, &arg_n_custom_mounts,
                                     optarg, c == ARG_BIND_RO);
                if (r < 0)
                    return log_error_errno(
                        r, "Failed to parse --bind(-ro)= argument %s: %m",
                        optarg);
            ...
            case ARG_TMPFS:
                r = tmpfs_mount_parse(&arg_custom_mounts, &arg_n_custom_mounts,
                                      optarg);
                if (r < 0)
                    return log_error_errno(
                        r, "Failed to parse --tmpfs= argument %s: %m", optarg);
            ...
            case ARG_OVERLAY:
            case ARG_OVERLAY_RO:
                r = overlay_mount_parse(&arg_custom_mounts,
                                        &arg_n_custom_mounts, optarg,
                                        c == ARG_OVERLAY_RO);
                if (r == -EADDRNOTAVAIL)
                    return log_error_errno(
                        r, "--overlay(-ro)= needs at least two colon-separated "
                           "directories specified.");
                if (r < 0)
                    return log_error_errno(
                        r, "Failed to parse --overlay(-ro)= argument %s: %m",
                        optarg);
            ...
            case ARG_INACCESSIBLE:
                r = inaccessible_mount_parse(&arg_custom_mounts,
                                             &arg_n_custom_mounts, optarg);
                if (r < 0)
                    return log_error_errno(
                        r, "Failed to parse --inaccessible= argument %s: %m",
                        optarg);
            ...
            case 'E':
            {
                ...
                if (!env_assignment_is_valid(optarg))
                    return log_error_errno(
                        SYNTHETIC_ERRNO(EINVAL),
                        "Environment variable assignment '%s' is not valid.",
                        optarg);
            ...
            case ARG_SHARE_SYSTEM:
                /* We don't officially support this anymore, except for compat
                 * reasons. People should use the $SYSTEMD_NSPAWN_SHARE_*
                 * environment variables instead. */
                log_warning("Please do not use --share-system anymore, use "
                            "$SYSTEMD_NSPAWN_SHARE_* instead.");
            ...
            case ARG_REGISTER:
                ...
                if (r < 0)
                {
                    log_error("Failed to parse --register= argument: %s",
                              optarg);
                    return r;
                }
            ...
            case ARG_VOLATILE:
                ...
                {
                    DUMP_STRING_TABLE(volatile_mode, VolatileMode,
                                      _VOLATILE_MODE_MAX);
                    return 0;
                }
                else
            ...
            case 'p':
                ...
                if (r == -EEXIST)
                    return log_error_errno(
                        r, "Duplicate port specification: %s", optarg);
                if (r < 0)
                    return log_error_errno(
                        r, "Failed to parse host port %s: %m", optarg);
            ...
            case ARG_PRIVATE_USERS:
            {
                ...
                else
                {
                    ...
                    /* anything else: User namespacing on, UID range is
                    ...
                    if (range)
                    {
                        ...
                        if (r < 0)
                            return log_error_errno(
                                r, "Failed to parse UID range \"%s\": %m",
                                range);
                    }
                    else
                        shift = optarg;
                    ...
                    if (r < 0)
                        return log_error_errno(
                            r, "Failed to parse UID \"%s\": %m", optarg);
            ...
            case ARG_SETTINGS:
                 ...
                 * yes              → read files, do not override cmdline, trust
                 * only subset override         → read files, override cmdline,
                 * trust only subset trusted          → read files, do not
                 * override cmdline, trust all
                 */
                ...
                if (r < 0)
                {
                    ...
                    else if (streq(optarg, "override"))
                    {
                        ...
                        return log_error_errno(
                            r, "Failed to parse --settings= argument: %s",
                            optarg);
                }
                else if (r > 0)
                {
            ...
            case ARG_CHDIR:
                if (!path_is_absolute(optarg))
                    return log_error_errno(
                        SYNTHETIC_ERRNO(EINVAL),
                        "Working directory %s is not an absolute path.",
                        optarg);
            ...
            case ARG_PIVOT_ROOT:
                r = pivot_root_parse(&arg_pivot_root_new, &arg_pivot_root_old,
                                     optarg);
                if (r < 0)
                    return log_error_errno(
                        r, "Failed to parse --pivot-root= argument %s: %m",
                        optarg);
            ...
            case ARG_NOTIFY_READY:
                ...
                if (r < 0)
                    return log_error_errno(
                        SYNTHETIC_ERRNO(EINVAL),
                        "%s is not a valid notify mode. Valid modes are: yes, "
                        "no, and ready.",
                        optarg);
                arg_notify_ready = r;
            ...
            case ARG_ROOT_HASH:
            {
                ...
                if (r < 0)
                    return log_error_errno(r, "Failed to parse root hash: %s",
                                           optarg);
            ...
            case ARG_SYSTEM_CALL_FILTER:
            {
                ...
                for (;;)
                {
                    ...
                    if (r < 0)
                        return log_error_errno(
                            r, "Failed to parse system call filter: %m");
            ...
            case ARG_RLIMIT:
            {
                ...
                if (r < 0)
                    return log_error_errno(
                        r, "Failed to parse resource limit: %s", eq + 1);
            ...
            case ARG_OOM_SCORE_ADJUST:
                ...
                if (r < 0)
                    return log_error_errno(
                        r, "Failed to parse --oom-score-adjust= parameter: %s",
                        optarg);
            ...
            case ARG_CPU_AFFINITY:
            {
                ...
                if (r < 0)
                    return log_error_errno(
                        r, "Failed to parse CPU affinity mask: %s", optarg);
            ...
            case ARG_RESOLV_CONF:
                if (streq(optarg, "help"))
                {
                    DUMP_STRING_TABLE(resolv_conf_mode, ResolvConfMode,
            ...
            case ARG_TIMEZONE:
                if (streq(optarg, "help"))
                {
                    DUMP_STRING_TABLE(timezone_mode, TimezoneMode,
                                      _TIMEZONE_MODE_MAX);
                    return 0;
                }
    ...
    if (arg_ephemeral && arg_template && !arg_directory)
        /* User asked for ephemeral execution but specified --template= instead
         * of --directory=. Semantically such an invocation makes some sense,
         * see https://github.com/systemd/systemd/issues/3667. Let's accept this
         * here, and silently make "--ephemeral --template=" equivalent to
         * "--ephemeral
         * --directory=". */
    ...
    arg_caps_retain =
        (arg_caps_retain | plus |
         (arg_private_network ? UINT64_C(1) << CAP_NET_ADMIN : 0)) &
        ~minus;
...
static int verify_arguments(void)
{
    ...
    if (arg_volatile_mode !=
        VOLATILE_NO) /* Make sure all file systems contained in the image are
                        mounted read-only if we are in volatile mode */
        arg_read_only = true;

    if (arg_keep_unit && arg_register && cg_pid_get_owner_uid(0, NULL) >= 0)
        /* Save the user from accidentally registering either
         * user-$SESSION.scope or user@.service.
         * The latter is not technically a user session, but we don't need to
         * labour the point. */
        return log_error_errno(SYNTHETIC_ERRNO(EINVAL),
                               "--keep-unit --register=yes may not be used "
                               "when invoked from a user session.");
    ...
    if (arg_userns_mode != USER_NAMESPACE_NO && !userns_supported())
        return log_error_errno(SYNTHETIC_ERRNO(EOPNOTSUPP),
                               "--private-users= is not supported, kernel "
                               "compiled without user namespace support.");
    ...
    /* We don't support --private-users-chown together with any of the volatile
     * modes since we couldn't change the read-only part of the tree (i.e. /usr)
     * anyway, or because it would trigger a massive copy-up (in case of
     * overlay) making the entire excercise pointless. */
    if (arg_userns_chown && arg_volatile_mode != VOLATILE_NO)
        return log_error_errno(
            SYNTHETIC_ERRNO(EINVAL),
            "--volatile= and --private-users-chown may not be combined.");

    /* If --network-namespace-path is given with any other network-related
     * option, we need to error out, to avoid conflicts between different
     * network options. */
    if (arg_network_namespace_path &&
        (arg_network_interfaces || arg_network_macvlan || arg_network_ipvlan ||
         arg_network_veth_extra || arg_network_bridge || arg_network_zone ||
         arg_network_veth || arg_private_network))
        return log_error_errno(SYNTHETIC_ERRNO(EINVAL),
                               "--network-namespace-path= cannot be combined "
    ...
    if (arg_userns_mode != USER_NAMESPACE_NO &&
        (arg_mount_settings & MOUNT_APPLY_APIVFS_NETNS) && !arg_private_network)
        return log_error_errno(
            SYNTHETIC_ERRNO(EINVAL),
            "Invalid namespacing settings. Mounting sysfs with --private-users "
...
static int userns_lchown(const char* p, uid_t uid, gid_t gid)
{
    ...
    if (gid != GID_INVALID)
    {
        ...
        if (gid < (gid_t)arg_uid_shift ||
            gid >= (gid_t)(arg_uid_shift + arg_uid_range))
            return -EOVERFLOW;
    }
...
static int userns_mkdir(const char* root, const char* path, mode_t mode,
                        uid_t uid, gid_t gid)
{
    const char* q;
    int r;
...
{
    return PATH_STARTSWITH_SET(path, "../usr/share/zoneinfo/",
                               "/usr/share/zoneinfo/");
}

static bool etc_writable(void)
{
    return !arg_read_only ||
           IN_SET(arg_volatile_mode, VOLATILE_YES, VOLATILE_OVERLAY);
}
...
static int setup_timezone(const char* dest)
{
    ...
    if (IN_SET(arg_timezone, TIMEZONE_AUTO, TIMEZONE_SYMLINK))
    {
        ...
        if (r == -ENOENT && arg_timezone == TIMEZONE_AUTO)
            m = etc_writable() ? TIMEZONE_DELETE : TIMEZONE_OFF;
                 ...
                 arg_timezone == TIMEZONE_AUTO) /* regular file? */
            m = etc_writable() ? TIMEZONE_COPY : TIMEZONE_BIND;
        else if (r < 0)
        {
            log_warning_errno(r,
                              "Failed to read host's /etc/localtime symlink, "
                              "not updating container timezone: %m");
            /* To handle warning, delete /etc/localtime and replace it with a
             * symbolic link to a time zone data file.
             *
             * Example:
    ...
    {
        log_warning_errno(
            r, "Failed to resolve /etc path in container, ignoring: %m");
        return 0;
    }
    ...
    switch (m)
    {
        ...
        case TIMEZONE_SYMLINK:
        {
            ...
            {
                log_warning(
                    "/etc/localtime does not point into /usr/share/zoneinfo/, "
            ...
            if (r >= 0 && streq_ptr(timezone_from_path(q), z))
                return 0; /* Already pointing to the right place? Then do
                             nothing .. */
            ...
            if (r < 0)
                log_debug_errno(
                    r,
                    "Timezone %s does not exist (or is not accessible) in "
                    "container, not creating symlink: %m",
                    z);
            else
            {
                if (unlink(where) < 0 && errno != ENOENT)
                {
                    log_full_errno(IN_SET(errno, EROFS, EACCES, EPERM)
                                       ? LOG_DEBUG
                                       : LOG_WARNING, /* Don't complain on
                                                         read-only images */
                                   errno,
                                   "Failed to remove existing timezone info %s "
                                   "in container, ignoring: %m",
                                   where);
                    return 0;
                }
                ...
                if (symlink(what, where) < 0)
                {
                    log_full_errno(IN_SET(errno, EROFS, EACCES, EPERM)
                                       ...
                                       : LOG_WARNING,
                                   errno,
                                   "Failed to correct timezone of container, "
                                   "ignoring: %m");
                    return 0;
                }
        ...
        case TIMEZONE_BIND:
        {
            ...
            if (found < 0)
            {
                log_warning_errno(found, "Failed to resolve /etc/localtime "
                                         "path in container, ignoring: %m");
                return 0;
            }
            ...
            r = mount_verbose(LOG_WARNING, "/etc/localtime", resolved, NULL,
                              MS_BIND, NULL);
            if (r >= 0)
                return mount_verbose(LOG_ERR, NULL, resolved, NULL,
        ...
        case TIMEZONE_COPY:
            ...
            r = copy_file_atomic("/etc/localtime", where, 0644, 0, 0,
                                 COPY_REFLINK | COPY_REPLACE);
            if (r < 0)
            {
                log_full_errno(
                    IN_SET(r, -EROFS, -EACCES, -EPERM) ? LOG_DEBUG
                                                       : LOG_WARNING,
                    r, "Failed to copy /etc/localtime to %s, ignoring: %m",
                    where);
                return 0;
            }
...
static int have_resolv_conf(const char* path)
{
    ...
    if (access(path, F_OK) < 0)
    {
        ...
        return log_debug_errno(
            errno, "Failed to determine whether '%s' is available: %m", path);
    }
...
static int resolved_listening(void)
{
    ...
    if (r < 0)
        return log_debug_errno(
            r, "Failed to check whether the 'org.freedesktop.resolve1' bus "
               "name is taken: %m");
    if (r == 0)
        return 0;

    r = sd_bus_get_property_string(
        bus, "org.freedesktop.resolve1", "/org/freedesktop/resolve1",
        "org.freedesktop.resolve1.Manager", "DNSStubListener", &error,
        &dns_stub_listener_mode);
    if (r < 0)
        return log_debug_errno(r,
                               "Failed to query DNSStubListener property: %s",
...
static int setup_resolv_conf(const char* dest)
{
    ...
    if (arg_resolv_conf == RESOLV_CONF_AUTO)
    {
        if (arg_private_network)
            m = RESOLV_CONF_OFF;
        else if (have_resolv_conf(STATIC_RESOLV_CONF) > 0 &&
                 resolved_listening() > 0)
            m = etc_writable() ? RESOLV_CONF_COPY_STATIC
    ...
    if (IN_SET(m, RESOLV_CONF_BIND_HOST, RESOLV_CONF_BIND_STATIC))
    {
        ...
        if (found < 0)
        {
            log_warning_errno(found, "Failed to resolve /etc/resolv.conf path "
                                     "in container, ignoring: %m");
            return 0;
        }
    ...
    if (r < 0)
    {
        /* If the file already exists as symlink, let's suppress the warning,
         * under the assumption that resolved or something similar runs inside
         * and the symlink points there.
         *
         * If the disk image is read-only, there's also no point in complaining.
         */
        log_full_errno(
            !IN_SET(RESOLV_CONF_COPY_HOST, RESOLV_CONF_COPY_STATIC) &&
                    IN_SET(r, -ELOOP, -EROFS, -EACCES, -EPERM)
                ? LOG_DEBUG
                : LOG_WARNING,
            r, "Failed to copy /etc/resolv.conf to %s, ignoring: %m", where);
        return 0;
    }
...
static int setup_boot_id(void)
{
    ...
    /* Generate a new randomized boot ID, so that each boot-up of the container
     * gets a new one */
    ...
    return mount_verbose(LOG_ERR, NULL, to, NULL,
...
{
    static const char devnodes[] = "null\0"
                                   "zero\0"
                                   "full\0"
                                   "random\0"
                                   "urandom\0"
    ...
    NULSTR_FOREACH(d, devnodes)
    {
        ...
        {
            _cleanup_free_ char *sl = NULL, *prefixed = NULL, *dn = NULL,
                                *t = NULL;
            ...
            if (mknod(to, st.st_mode, st.st_rdev) < 0)
            {
                ...
                if (errno == EEXIST)
                    log_notice("%s/dev is pre-mounted and pre-populated. If a "
                               "pre-mounted /dev is provided it needs to be an "
                ...
                if (r < 0)
                    return log_error_errno(
                        r, "Both mknod and bind mount (%s) failed: %m", to);
            }

            r = userns_lchown(to, 0, 0);
            if (r < 0)
                return log_error_errno(
                    r, "chown() of device node %s failed: %m", to);
            ...
            if (asprintf(&sl, "%s/%u:%u", dn, major(st.st_rdev),
                         minor(st.st_rdev)) < 0)
                return log_oom();
            ...
            if (symlink(t, prefixed) < 0)
                log_debug_errno(errno, "Failed to symlink '%s' to '%s': %m", t,
                                prefixed);
        }
    }
...
static int make_extra_nodes(const char* dest)
{
    ...
    for (i = 0; i < arg_n_extra_nodes; i++)
    {
        ...
        if (mknod(path, n->mode,
                  S_ISCHR(n->mode) || S_ISBLK(n->mode)
                      ? makedev(n->major, n->minor)
                      : 0) < 0)
            return log_error_errno(
                errno, "Failed to create device node '%s': %m", path);

        r = chmod_and_chown(path, n->mode, n->uid, n->gid);
        if (r < 0)
            return log_error_errno(
                r, "Failed to adjust device node ownership of '%s': %m", path);
    }
...
static int setup_pts(const char* dest)
{
    ...
    if (arg_selinux_apifs_context)
        (void)asprintf(&options,
                       ...
                       ",context=\"%s\"",
                       arg_uid_shift + TTY_GID, arg_selinux_apifs_context);
    else
#endif
        (void)asprintf(&options,
                       "newinstance,ptmxmode=0666,mode=620,gid=" GID_FMT,
    ...
    r = mount_verbose(LOG_ERR, "devpts", p, "devpts", MS_NOSUID | MS_NOEXEC,
                      options);
    if (r < 0)
        return r;
...
static int setup_keyring(void)
{
    ...
    /* Allocate a new session keyring for the container. This makes sure the
     * keyring of the session systemd-nspawn was invoked from doesn't leak into
     * the container. Note that by default we block keyctl() and request_key()
     * anyway via seccomp so doing this operation isn't strictly necessary, but
     * in case people explicitly whitelist
     * these system calls let's make sure we don't leak anything into the
     * container. */
...
static int setup_kmsg(int kmsg_socket)
{
    ...
    /* We create the kmsg FIFO as as temporary file in /run, but immediately
     * delete it after bind mounting it to /proc/kmsg. While FIFOs on the
     * reading side behave very similar to /proc/kmsg, their writing side
     * behaves differently from /dev/kmsg in that writing blocks when nothing is
     * reading. In order to avoid any problems with containers deadlocking due
     * to this we simply make /dev/kmsg unavailable to the container. */
    ...
    /* Store away the fd in the socket, so that it stays open as long as we run
     * the child */
...
static int on_address_change(sd_netlink* rtnl, sd_netlink_message* m,
                             void* userdata)
{
...
static int setup_journal(const char* directory)
{
    ...
    try
        = arg_link_journal_try || arg_link_journal == LINK_AUTO;
    ...
    {
        log_full(
            try ? LOG_WARNING : LOG_ERR,
            "Host and machine ids are equal (%s): refusing to link journals",
            sd_id128_to_string(arg_uuid, id));
        if (try)
            return 0;
        return -EEXIST;
    }
    ...
    FOREACH_STRING(dirname, "/var", "/var/log", "/var/log/journal")
    {
        ...
        if (r < 0)
        {
            ...
            log_full_errno(ignore ? LOG_DEBUG : LOG_ERR, r,
                           "Failed to create %s%s: %m", dirname,
                           ignore ? ", ignoring" : "");
            return ignore ? 0 : r;
        }
    }
    ...
    if (r >= 0)
    {
        if (IN_SET(arg_link_journal, LINK_GUEST, LINK_AUTO) && path_equal(d, q))
        {
    ...
    else if (r == -EINVAL)
    {
        ...
        if (arg_link_journal == LINK_GUEST && rmdir(p) < 0)
        {

            if (errno == ENOTDIR)
            {
                log_error("%s already exists and is neither a symlink nor a "
                          "directory",
                          p);
                return r;
            }
    ...
    if (arg_link_journal == LINK_GUEST)
    {
        ...
        if (symlink(q, p) < 0)
        {
            if (try)
            {
                ...
                return log_error_errno(errno, "Failed to symlink %s to %s: %m",
                                       q, p);
        }
    ...
    if (arg_link_journal == LINK_HOST)
    {
        ...
        if (r < 0 && r != -EEXIST)
        {
            ...
            {
                log_debug_errno(
                    r, "Failed to create %s, skipping journal setup: %m", p);
                return 0;
            }
            else
    ...
    if (r < 0)
        return log_error_errno(
            errno, "Failed to bind mount journal from host into guest: %m");
...
static int drop_capabilities(uid_t uid)
{
    ...
    /* Let's initialize all five capability sets to something valid. If the
     * quintet was configured via OCI use that, but fill in missing bits. If it
     * wasn't then derive the quintet in full from arg_caps_retain. */
...
static int reset_audit_loginuid(void)
{
    ...
    if (r < 0)
    {
        log_error_errno(r, "Failed to reset audit login UID. This probably "
                           "means that your kernel is too\n"
                           "old and you have audit enabled. Note that the "
                           "auditing subsystem is known to\n"
                           "be incompatible with containers on old kernels. "
                           "Please make sure to upgrade\n"
                           "your kernel or to off auditing with 'audit=0' on "
                           "the kernel command line before\n"
...
static int setup_propagate(const char* root)
{
    ...
    if (r < 0)
        return log_error_errno(
            r, "Failed to create /run/systemd/nspawn/incoming: %m");
    ...
    r = mount_verbose(LOG_ERR, NULL, q, NULL, MS_BIND | MS_REMOUNT | MS_RDONLY,
                      NULL);
    if (r < 0)
        return r;
...
static int setup_machine_id(const char* directory)
{
    ...
    /* If the UUID in the container is already set, then that's what counts, and
     * we use. If it isn't set, and the caller passed --uuid=, then we'll pass
     * it in the $container_uuid env var to PID 1 of the container. The
     * assumption is that PID 1 will then write it to /etc/machine-id to make it
     * persistent. If --uuid= is not passed we generate a random UUID, and pass
     * it via $container_uuid. In effect this means that /etc/machine-id in the
     * container and our idea of the container UUID will always be in sync (at
     * least if PID 1 in the container behaves nicely). */
    ...
    if (r < 0)
    {
        if (!IN_SET(r, -ENOENT, -ENOMEDIUM)) /* If the file is missing or empty,
                                                we don't mind */
            return log_error_errno(
                r, "Failed to read machine ID from container image: %m");
        ...
        if (sd_id128_is_null(arg_uuid))
        {
            ...
            if (r < 0)
                return log_error_errno(
                    r, "Failed to acquire randomized machine UUID: %m");
        }
    }
    else
    {
...
static int recursive_chown(const char* directory, uid_t shift, uid_t range)
{
    ...
    if (r == -EOPNOTSUPP)
        return log_error_errno(
            r, "Automatic UID/GID adjusting is only supported for UID/GID "
               "ranges starting at multiples of 2^16 with a range of 2^16.");
    if (r == -EBADE)
        return log_error_errno(
            r, "Upper 16 bits of root directory UID and GID do not match.");
    if (r < 0)
        return log_error_errno(r,
                               "Failed to adjust UID/GID shift of OS tree: %m");
    if (r == 0)
        log_debug("Root directory of image is already owned by the right "
                  "UID/GID range, skipping recursive chown operation.");
    else
...
static int wait_for_container(pid_t pid, ContainerStatus* container)
{
    ...
    switch (status.si_code)
    {
        ...
        case CLD_DUMPED:
            return log_error_errno(
                SYNTHETIC_ERRNO(EIO), "Container %s terminated by signal %s.",
                arg_machine, signal_to_string(status.si_status));

        default:
            return log_error_errno(SYNTHETIC_ERRNO(EIO),
                                   "Container %s failed due to unknown reason.",
                                   arg_machine);
    }
}

static int on_orderly_shutdown(sd_event_source* s,
                               ...
                               void* userdata)
{
    ...
    if (pid > 0)
    {
        ...
        {
            log_info("Trying to halt container. Send SIGTERM again to trigger "
...
static int on_sigchld(sd_event_source* s, const struct signalfd_siginfo* ssi,
                      void* userdata)
{
    pid_t pid;
...
static int on_request_stop(sd_bus_message* m, void* userdata,
                           sd_bus_error* error)
{
    pid_t pid;
    ...
    {
        log_info(
            "Container termination requested. Attempting to halt container.");
...
static int determine_names(void)
{
    ...
    if (!arg_image && !arg_directory)
    {
        if (arg_machine)
        {
            ...
            if (r == -ENOENT)
                return log_error_errno(r, "No image for machine '%s'.",
                                       arg_machine);
            if (r < 0)
                return log_error_errno(
                    r, "Failed to find image for machine '%s': %m",
                    arg_machine);
        ...
        else
        {
            ...
            if (r < 0)
                return log_error_errno(
                    r, "Failed to determine current directory: %m");
        }
    ...
    if (!arg_machine)
    {
        ...
        if (!machine_name_is_valid(arg_machine))
            return log_error_errno(SYNTHETIC_ERRNO(EINVAL),
                                   "Failed to determine machine name "
                                   "automatically, please use -M.");
    ...
    STRV_FOREACH_PAIR(k, v, arg_sysctl)
    {
        ...
        if (!good)
            return log_error_errno(SYNTHETIC_ERRNO(EPERM),
                                   "Refusing to write to sysctl '%s', as it is "
                                   "not safe in the selected namespaces.",
                                   *k);
...
static int inner_child(Barrier* barrier, const char* directory, bool secondary,
                       int kmsg_socket, int rtnl_socket, FDSet* fds)
{

    _cleanup_free_ char* home = NULL;
    char as_uuid[37];
    size_t n_env = 1;
    const char* envp[] = {"PATH=" DEFAULT_PATH_COMPAT,
                          NULL, /* container */
                          NULL, /* TERM */
                          NULL, /* HOME */
                          NULL, /* USER */
    ...
    /* This is the "inner" child process, i.e. the one forked off by the "outer"
     * child process, which is the one the container manager itself forked off.
     * At the time of clone() it gained its own CLONE_NEWNS, CLONE_NEWPID,
     * CLONE_NEWUTS, CLONE_NEWIPC, CLONE_NEWUSER namespaces. Note that it has
     * its own CLONE_NEWNS namespace, separate from the CLONE_NEWNS created for
     * the "outer" child, and also separate from the host's CLONE_NEWNS
     * namespace. The reason for having two levels of CLONE_NEWNS namespaces is
     * that the "inner" one is owned by the CLONE_NEWUSER namespace of the
     * container, while the "outer" one is owned by the host's CLONE_NEWUSER
     * namespace.
     *
     * Note at this point we have no CLONE_NEWNET namespace yet. We'll acquire
     * that one later through unshare(). See below. */
    ...
    r = mount_all(NULL, arg_mount_settings | MOUNT_IN_USERNS, arg_uid_shift,
    ...
    if (arg_use_cgns)
    {
        ...
        r = mount_cgroups("", arg_unified_cgroup_hierarchy,
                          arg_userns_mode != USER_NAMESPACE_NO, arg_uid_shift,
                          arg_uid_range, arg_selinux_apifs_context, true);
        if (r < 0)
            return r;
    }
    else
    ...
    r = mount_custom("/", arg_custom_mounts, arg_n_custom_mounts, false, 0, 0,
                     arg_selinux_apifs_context, true);
    if (r < 0)
        return r;
    ...
    if (arg_cpuset)
        if (sched_setaffinity(0, CPU_ALLOC_SIZE(arg_cpuset_ncpus), arg_cpuset) <
            0)
    ...
    r = setrlimit_closest_all((const struct rlimit* const*)arg_rlimit,
                              &which_failed);
    if (r < 0)
        return log_error_errno(r,
                               "Failed to apply resource limit RLIMIT_%s: %m",
    ...
    if (arg_selinux_context)
        if (setexeccon(arg_selinux_context) < 0)
            return log_error_errno(errno, "setexeccon(\"%s\") failed: %m",
                                   arg_selinux_context);
#endif

    /* Make sure we keep the caps across the uid/gid dropping, so that we can
     * retain some selected caps if we need to later on. */
    if (prctl(PR_SET_KEEPCAPS, 1) < 0)
        return log_error_errno(errno, "Failed to set PR_SET_KEEPCAPS: %m");

    if (uid_is_valid(arg_uid) || gid_is_valid(arg_gid))
        r = change_uid_gid_raw(arg_uid, arg_gid, arg_supplementary_gids,
    ...
    if (arg_user || !uid_is_valid(arg_uid) || arg_uid == 0)
        if (asprintf((char**)(envp + n_env++), "USER=%s", arg_user ?: "root") <
                0 ||
    ...
    if (asprintf((char**)(envp + n_env++), "container_uuid=%s",
                 id128_to_uuid_string(arg_uuid, as_uuid)) < 0)
        return log_oom();
    ...
    if (fdset_size(fds) > 0)
    {
        ...
        if (r < 0)
            return log_error_errno(
                r, "Failed to unset O_CLOEXEC for file descriptors.");

        if ((asprintf((char**)(envp + n_env++), "LISTEN_FDS=%u",
                      fdset_size(fds)) < 0) ||
            (asprintf((char**)(envp + n_env++), "LISTEN_PID=1") < 0))
            return log_oom();
    }
    if (asprintf((char**)(envp + n_env++), "NOTIFY_SOCKET=%s",
                 NSPAWN_NOTIFY_SOCKET_PATH) < 0)
        return log_oom();
    ...
    if (arg_chdir)
        if (chdir(arg_chdir) < 0)
            return log_error_errno(
                errno, "Failed to change to specified working directory %s: %m",
                arg_chdir);
    ...
    /* Now, explicitly close the log, so that we then can close all remaining
     * fds. Closing the log explicitly first has the benefit that the logging
     * subsystem knows about it, and is thus ready to be reopened should we need
     * it again. Note that the other fds closed here are at least the locking
     * and barrier fds. */
    log_close();
    ...
    else if (!strv_isempty(arg_parameters))
    {
        ...
        /* Use the user supplied search $PATH if there is one, or
         * DEFAULT_PATH_COMPAT if not to search the binary. */
    ...
    {
        if (!arg_chdir)
            /* If we cannot change the directory, we'll end up in /, that is
             * expected. */
...
static int setup_sd_notify_child(void)
{
    ...
    if (r < 0)
        return log_error_errno(errno, "bind(" NSPAWN_NOTIFY_SOCKET_PATH
                                      ") failed: %m");

    r = userns_lchown(NSPAWN_NOTIFY_SOCKET_PATH, 0, 0);
    if (r < 0)
        return log_error_errno(r, "Failed to chown " NSPAWN_NOTIFY_SOCKET_PATH
                                  ": %m");
...
static int outer_child(Barrier* barrier, const char* directory,
                       const char* console, DissectedImage* dissected_image,
                       bool secondary, int pid_socket, int uuid_socket,
                       int notify_socket, int kmsg_socket, int rtnl_socket,
                       int uid_shift_socket,
                       int unified_cgroup_hierarchy_socket, FDSet* fds,
                       int netns_fd)
{
    ...
    /* This is the "outer" child process, i.e the one forked off by the
     * container manager itself. It already has its own CLONE_NEWNS namespace
     * (which was created by the clone()). It still lives in the host's
     * CLONE_NEWPID, CLONE_NEWUTS, CLONE_NEWIPC, CLONE_NEWUSER and CLONE_NEWNET
     * namespaces. After it completed a number of
     * initializations a second child (the "inner" one) is forked off it, and it
     * exits. */
    ...
    if (arg_console_mode != CONSOLE_PIPE)
    {
        ...
        r = rearrange_stdio(
            terminal, terminal,
            terminal); /* invalidates 'terminal' on success and failure */
        if (r < 0)
            return log_error_errno(
                r, "Failed to move console to stdin/stdout/stderr: %m");
    }
    ...
    if (dissected_image)
    {
        /* If we are operating on a disk image, then mount its root directory
         * now, but leave out the rest. We can read the UID shift from it if we
         * need to. Further down we'll mount the rest, but then with the uid
         * shift known. That way we can mount VFAT file systems shifted to the
         * right place right away. This
    ...
    if (arg_userns_mode != USER_NAMESPACE_NO)
    {
        ...
        l = send(uid_shift_socket, &arg_uid_shift, sizeof(arg_uid_shift),
                 MSG_NOSIGNAL);
        if (l < 0)
        ...
        if (arg_userns_mode == USER_NAMESPACE_PICK)
        {
            /* When we are supposed to pick the UID shift, the parent will check
             * now whether the UID shift we just read from the image is
             * available. If yes, it will send the UID shift back to us, if
             * not it will pick a different one, and send it back to us. */

            l = recv(uid_shift_socket, &arg_uid_shift, sizeof(arg_uid_shift),
                     0);
            if (l < 0)
        ...
        log_full(arg_quiet ? LOG_DEBUG : LOG_INFO,
                 "Selected user namespace base " UID_FMT " and range " UID_FMT
                 ".",
    ...
    if (!dissected_image)
    {
        ...
        r = mount_verbose(LOG_ERR, directory, directory, NULL, MS_BIND | MS_REC,
                          NULL);
        if (r < 0)
            return r;
    }

    r = setup_pivot_root(directory, arg_pivot_root_new, arg_pivot_root_old);
    if (r < 0)
        return r;

    r = setup_volatile_mode(directory, arg_volatile_mode,
                            arg_userns_mode != USER_NAMESPACE_NO, arg_uid_shift,
    ...
    if (dissected_image)
    {
        /* Now we know the uid shift, let's now mount everything else that might
         * be in the image. */
        r = dissected_image_mount(
            dissected_image, directory, arg_uid_shift,
            DISSECT_IMAGE_MOUNT_NON_ROOT_ONLY | DISSECT_IMAGE_DISCARD_ON_LOOP |
                (arg_read_only ? DISSECT_IMAGE_READ_ONLY : 0));
        if (r < 0)
            return r;
    }
    ...
    {
        /* OK, we don't know yet which cgroup mode to use yet. Let's figure it
         * out, and tell the parent. */

        r = detect_unified_cgroup_hierarchy_from_image(directory);
        if (r < 0)
            return r;

        l = send(unified_cgroup_hierarchy_socket, &arg_unified_cgroup_hierarchy,
        ...
        unified_cgroup_hierarchy_socket =
    ...
    r = mount_all(directory, arg_mount_settings, arg_uid_shift,
                  arg_selinux_apifs_context);
    if (r < 0)
        return r;
    ...
    r = mount_custom(directory, arg_custom_mounts, arg_n_custom_mounts,
                     arg_userns_mode != USER_NAMESPACE_NO, arg_uid_shift,
                     arg_uid_range, arg_selinux_apifs_context, false);
    if (r < 0)
        return r;

    if (!arg_use_cgns)
    {
        r = mount_cgroups(directory, arg_unified_cgroup_hierarchy,
                          arg_userns_mode != USER_NAMESPACE_NO, arg_uid_shift,
                          arg_uid_range, arg_selinux_apifs_context, false);
        if (r < 0)
            return r;
    ...
    pid = raw_clone(SIGCHLD | CLONE_NEWNS | arg_clone_ns_flags |
    ...
    if (pid == 0)
    {
        ...
        r = inner_child(barrier, directory, secondary, kmsg_socket, rtnl_socket,
                        fds);
        if (r < 0)
            _exit(EXIT_FAILURE);
...
static int uid_shift_pick(uid_t* shift, LockFile* ret_lock_file)
{
    ...
    for (;;)
    {
        char lock_path[STRLEN("/run/systemd/nspawn-uid/") +
                       DECIMAL_STR_MAX(uid_t) + 1];
        _cleanup_(release_lock_file) LockFile lf = LOCK_FILE_INIT;

        if (--n_tries <= 0)
            return -EBUSY;

        if (candidate < CONTAINER_UID_BASE_MIN ||
            candidate > CONTAINER_UID_BASE_MAX)
            goto next;
        ...
        /* Make some superficial checks whether the range is currently known in
         * the user database */
    ...
    next:
        if (arg_machine && !tried_hashed)
        {
            ...
            candidate =
                (uid_t)siphash24(arg_machine, strlen(arg_machine), hash_key);
        ...
        candidate =
            (candidate % (CONTAINER_UID_BASE_MAX - CONTAINER_UID_BASE_MIN)) +
...
static int setup_uid_map(pid_t pid)
{
    char uid_map[STRLEN("/proc//uid_map") + DECIMAL_STR_MAX(uid_t) + 1],
        line[DECIMAL_STR_MAX(uid_t) * 3 + 3 + 1];
    int r;
...
static int nspawn_dispatch_notify_fd(sd_event_source* source, int fd,
...
static int setup_sd_notify_parent(sd_event* event, int fd,
                                  pid_t* inner_child_pid,
                                  sd_event_source** notify_event_source)
{
    int r;

    r = sd_event_add_io(event, notify_event_source, fd, EPOLLIN,
                        nspawn_dispatch_notify_fd, inner_child_pid);
    if (r < 0)
        return log_error_errno(r, "Failed to allocate notify event source: %m");

    (void)sd_event_source_set_description(*notify_event_source,
                                          "nspawn-notify");
...
static int merge_settings(Settings* settings, const char* path)
{
    ...
    /* Copy over bits from the settings, unless they have been explicitly masked
     * by command line switches. Note that this steals the fields of the
     * Settings* structure, and hence modifies it. */
    ...
    if ((arg_settings_mask & SETTING_DIRECTORY) == 0 && settings->root)
    {

        if (!arg_settings_trusted)
            log_warning(
                "Ignoring root directory setting, file %s is not trusted.",
                path);
        else
    ...
    if ((arg_settings_mask & SETTING_ENVIRONMENT) == 0 && settings->environment)
    ...
    if ((arg_settings_mask & SETTING_USER) == 0)
    {
        ...
        {
            free_and_replace(arg_supplementary_gids,
                             settings->supplementary_gids);
    ...
    if ((arg_settings_mask & SETTING_CAPABILITY) == 0)
    {
        ...
        /* Note that we copy both the simple plus/minus caps here, and the full
         * quintet from the Settings structure */
        ...
        if (!arg_settings_trusted && plus != 0)
        {
            if (settings->capability != 0)
                log_warning(
                    "Ignoring Capability= setting, file %s is not trusted.",
                    path);
        }
        else
            arg_caps_retain |= plus;
        ...
        if (capability_quintet_is_set(&settings->full_capabilities))
        {
            if (!arg_settings_trusted)
                log_warning(
                    "Ignoring capabilitiy settings, file %s is not trusted.",
                    path);
            else
        ...
        !sd_id128_is_null(settings->machine_id))
    {
        ...
        if (!arg_settings_trusted)
            log_warning("Ignoring MachineID= setting, file %s is not trusted.",
                        path);
        else
        ...
        settings->n_custom_mounts > 0)
    {
        ...
        if (!arg_settings_trusted)
            log_warning("Ignoring TemporaryFileSystem=, Bind= and "
                        "BindReadOnly= settings, file %s is not trusted.",
                        path);
        else
        {
    ...
    if ((arg_settings_mask & SETTING_NETWORK) == 0 &&
        (settings->private_network >= 0 || settings->network_veth >= 0 ||
         settings->network_bridge || settings->network_zone ||
         settings->network_interfaces || settings->network_macvlan ||
         settings->network_ipvlan || settings->network_veth_extra ||
         settings->network_namespace_path))
    {

        if (!arg_settings_trusted)
            log_warning("Ignoring network settings, file %s is not trusted.",
                        path);
        else
        {
        ...
        settings->expose_ports)
    {
        ...
        if (!arg_settings_trusted)
            log_warning("Ignoring Port= setting, file %s is not trusted.",
                        path);
        else
        {
        ...
        settings->userns_mode != _USER_NAMESPACE_MODE_INVALID)
    {
        ...
        if (!arg_settings_trusted)
            log_warning("Ignoring PrivateUsers= and PrivateUsersChown= "
                        "settings, file %s is not trusted.",
                        path);
        else
        {
    ...
    if ((arg_settings_mask & SETTING_SYSCALL_FILTER) == 0)
    {
        ...
        if (!arg_settings_trusted && !strv_isempty(settings->syscall_whitelist))
            log_warning(
                "Ignoring SystemCallFilter= settings, file %s is not trusted.",
                path);
        else
        {
            strv_free_and_replace(arg_syscall_whitelist,
                                  settings->syscall_whitelist);
            strv_free_and_replace(arg_syscall_blacklist,
                                  settings->syscall_blacklist);
        }

#if HAVE_SECCOMP
        if (!arg_settings_trusted && settings->seccomp)
            log_warning("Ignoring SECCOMP filter, file %s is not trusted.",
                        path);
        else
        {
    ...
    for (rl = 0; rl < _RLIMIT_MAX; rl++)
    {
        ...
        if (!arg_settings_trusted)
        {
            log_warning("Ignoring Limit%s= setting, file '%s' is not trusted.",
                        rlimit_to_string(rl), path);
            continue;
        }
    ...
    if ((arg_settings_mask & SETTING_HOSTNAME) == 0 && settings->hostname)
        ...
        settings->oom_score_adjust_set)
    {
        ...
        if (!arg_settings_trusted)
            log_warning(
                "Ignoring OOMScoreAdjust= setting, file '%s' is not trusted.",
                path);
        else
        {
    ...
    if ((arg_settings_mask & SETTING_CPU_AFFINITY) == 0 && settings->cpuset)
    {

        if (!arg_settings_trusted)
            log_warning(
                "Ignoring CPUAffinity= setting, file '%s' is not trusted.",
                path);
        else
        {
            if (arg_cpuset)
        ...
        settings->link_journal != _LINK_JOURNAL_INVALID)
    {
        ...
        if (!arg_settings_trusted)
            log_warning(
                "Ignoring journal link setting, file '%s' is not trusted.",
                path);
        else
        {
    ...
    if ((arg_settings_mask & SETTING_SLICE) == 0 && settings->slice)
    {

        if (!arg_settings_trusted)
            log_warning("Ignoring slice setting, file '%s' is not trusted.",
                        path);
        else
            free_and_replace(arg_slice, settings->slice);
    }

    if ((arg_settings_mask & SETTING_USE_CGNS) == 0 && settings->use_cgns >= 0)
    {

        if (!arg_settings_trusted)
            log_warning(
                "Ignoring cgroup namespace setting, file '%s' is not trusted.",
                path);
        else
        ...
        settings->clone_ns_flags != (unsigned long)-1)
    {
        ...
        if (!arg_settings_trusted)
            log_warning("Ignoring namespace setting, file '%s' is not trusted.",
                        path);
        else
        ...
        settings->console_mode >= 0)
    {
        ...
        if (!arg_settings_trusted)
            log_warning(
                "Ignoring console mode setting, file '%s' is not trusted.",
                path);
        else
            arg_console_mode = settings->console_mode;
    }

    /* The following properties can only be set through the OCI settings logic,
     * not from the command line, hence we don't consult arg_settings_mask for
     * them. */
...
static int run_container(int master, const char* console,
                         DissectedImage* dissected_image, bool secondary,
                         FDSet* fds, char veth_name[IFNAMSIZ],
                         bool* veth_created, union in_addr_union* exposed,
                         pid_t* pid, int* ret)
{
    ...
    _cleanup_(sd_event_source_unrefp) sd_event_source* notify_event_source =
        NULL;
    ...
    if (arg_userns_mode == USER_NAMESPACE_PICK)
    {
        /* When we shall pick the UID/GID range, let's first lock /etc/passwd,
         * so that we can safely check with getpwuid() if the specific user
         * already exists. Note that /etc might be read-only, in which case this
         * will fail with EROFS. But that's really OK, as in that case we can be
         * reasonably sure that no users are going to be added. Note that
         * getpwuid() checks are really just an extra safety net. We kinda
         * assume that the UID range we allocate from is really ours. */

        etc_passwd_lock = take_etc_passwd_lock(NULL);
        if (etc_passwd_lock < 0 && etc_passwd_lock != -EROFS)
            return log_error_errno(etc_passwd_lock,
    ...
    if (socketpair(AF_UNIX, SOCK_SEQPACKET | SOCK_CLOEXEC, 0, pid_socket_pair) <
        0)
    ...
    if (socketpair(AF_UNIX, SOCK_SEQPACKET | SOCK_CLOEXEC, 0,
                   notify_socket_pair) < 0)
        return log_error_errno(errno,
                               "Failed to create notify socket pair: %m");

    if (arg_userns_mode != USER_NAMESPACE_NO)
        if (socketpair(AF_UNIX, SOCK_SEQPACKET | SOCK_CLOEXEC, 0,
                       uid_shift_socket_pair) < 0)
            return log_error_errno(
                errno, "Failed to create uid shift socket pair: %m");

    if (arg_unified_cgroup_hierarchy == CGROUP_UNIFIED_UNKNOWN)
        if (socketpair(AF_UNIX, SOCK_SEQPACKET | SOCK_CLOEXEC, 0,
                       unified_cgroup_hierarchy_socket_pair) < 0)
            return log_error_errno(
                errno, "Failed to create unified cgroup socket pair: %m");

    /* Child can be killed before execv(), so handle SIGCHLD in order to
     * interrupt
     * parent's blocking calls and give it a chance to call wait() and
     * terminate. */
    ...
    {
        netns_fd =
            open(arg_network_namespace_path, O_RDONLY | O_NOCTTY | O_CLOEXEC);
        if (netns_fd < 0)
            return log_error_errno(errno, "Cannot open file %s: %m",
                                   arg_network_namespace_path);
        ...
        if (r == -EUCLEAN)
            log_debug_errno(
                r,
                "Cannot determine if passed network namespace path '%s' really "
                "refers to a network namespace, assuming it does.",
                arg_network_namespace_path);
        else if (r < 0)
            return log_error_errno(r, "Failed to check %s fs type: %m",
                                   arg_network_namespace_path);
        else if (r == 0)
            return log_error_errno(
                SYNTHETIC_ERRNO(EINVAL),
                "Path %s doesn't refer to a network namespace, refusing.",
                arg_network_namespace_path);
    }
    ...
    *pid = raw_clone(SIGCHLD | CLONE_NEWNS);
    if (*pid < 0)
        return log_error_errno(
            errno, "clone() failed%s: %m",
            errno == EINVAL
                ? ", do you have namespace support enabled in your kernel? "
                  "(You need UTS, IPC, PID and NET namespacing built in)"
                : "");
    ...
    if (*pid == 0)
    {
        ...
        unified_cgroup_hierarchy_socket_pair[0] =
            safe_close(unified_cgroup_hierarchy_socket_pair[0]);

        (void)reset_all_signal_handlers();
        (void)reset_signal_mask();

        r = outer_child(&barrier, arg_directory, console, dissected_image,
                        secondary, pid_socket_pair[1], uuid_socket_pair[1],
                        notify_socket_pair[1], kmsg_socket_pair[1],
                        rtnl_socket_pair[1], uid_shift_socket_pair[1],
                        unified_cgroup_hierarchy_socket_pair[1], fds, netns_fd);
        if (r < 0)
            _exit(EXIT_FAILURE);
    ...
    unified_cgroup_hierarchy_socket_pair[1] =
        safe_close(unified_cgroup_hierarchy_socket_pair[1]);

    if (arg_userns_mode != USER_NAMESPACE_NO)
    {
        /* The child just let us know the UID shift it might have read from the
         * image. */
        l = recv(uid_shift_socket_pair[0], &arg_uid_shift, sizeof arg_uid_shift,
                 0);
        if (l < 0)
        ...
        if (arg_userns_mode == USER_NAMESPACE_PICK)
        {
            /* If we are supposed to pick the UID shift, let's try to use the
             * shift read from the image, but if that's already in use, pick a
             * new one, and report back to the child, which one we now picked.
             */

            r = uid_shift_pick(&arg_uid_shift, &uid_shift_lock);
            if (r < 0)
                return log_error_errno(
                    r, "Failed to pick suitable UID/GID range: %m");

            l = send(uid_shift_socket_pair[0], &arg_uid_shift,
                     sizeof arg_uid_shift, MSG_NOSIGNAL);
            if (l < 0)
                return log_error_errno(errno, "Failed to send UID shift: %m");
            if (l != sizeof arg_uid_shift)
                return log_error_errno(SYNTHETIC_ERRNO(EIO),
    ...
    if (arg_unified_cgroup_hierarchy == CGROUP_UNIFIED_UNKNOWN)
    {
        /* The child let us know the support cgroup mode it might have read from
         * the image. */
        l = recv(unified_cgroup_hierarchy_socket_pair[0],
                 &arg_unified_cgroup_hierarchy,
                 sizeof(arg_unified_cgroup_hierarchy), 0);
        if (l < 0)
            return log_error_errno(errno, "Failed to read cgroup mode: %m");
        if (l != sizeof(arg_unified_cgroup_hierarchy))
            return log_error_errno(
                SYNTHETIC_ERRNO(EIO),
                "Short read while reading cgroup mode (%zu bytes).%s", l,
                l == 0 ? " The child is most likely dead." : "");
    }
    ...
    /* We also retrieve the socket used for notifications generated by outer
     * child */
    ...
    if (arg_private_network)
    {
        ...
        /* We created the primary and extra veth links now; let's remember this,
           so that we know to remove them later on. Note that we don't bother
           with removing veth links that were created here when their setup
           failed half-way, because in that case the kernel should be able to
           remove them on its own, since they cannot be referenced by anything
           yet. */
        *veth_created = true;
    ...
    if (arg_register || !arg_keep_unit)
    {
        ...
        if (r < 0)
            return log_error_errno(
                r, "Failed to disable close-on-exit behaviour: %m");
    }

    if (!arg_keep_unit)
    {
        /* When a new scope is created for this container, then we'll be
         * registered as its controller, in which case PID 1 will send us a
         * friendly RequestStop signal, when it is asked to terminate the
         * scope. Let's hook into that, and cleanly shut down the container, and
         * print a friendly message. */

        r = sd_bus_match_signal_async(bus, NULL, "org.freedesktop.systemd1",
                                      NULL, "org.freedesktop.systemd1.Scope",
                                      "RequestStop", on_request_stop, NULL,
                                      PID_TO_PTR(*pid));
        if (r < 0)
            return log_error_errno(r,
    ...
    if (arg_register)
    {
        r = register_machine(bus, arg_machine, *pid, arg_directory, arg_uuid,
                             ifi, arg_slice, arg_custom_mounts,
                             arg_n_custom_mounts, arg_kill_signal, arg_property,
                             arg_property_message, arg_keep_unit,
    ...
    {
        r = allocate_scope(bus, arg_machine, *pid, arg_slice, arg_custom_mounts,
                           arg_n_custom_mounts, arg_kill_signal, arg_property,
        ...
        log_notice("Machine and scope registration turned off, --slice= and "
                   "--property= settings will have no effect.");
    ...
    /* At this point we have made use of the UID we picked, and thus
     * nss-mymachines will make them appear in getpwuid(), thus we can release
     * the /etc/passwd lock. */
    ...
    if (arg_kill_signal > 0)
    {
        ...
        (void)sd_event_add_signal(event, NULL, SIGINT, on_orderly_shutdown,
                                  PID_TO_PTR(*pid));
        (void)sd_event_add_signal(event, NULL, SIGTERM, on_orderly_shutdown,
                                  PID_TO_PTR(*pid));
    }
    else
    {
    ...
    (void)sd_event_add_signal(event, NULL, SIGCHLD, on_sigchld,
                              PID_TO_PTR(*pid));

    if (arg_expose_ports)
    {
        r = expose_port_watch_rtnl(event, rtnl_socket_pair[0],
                                   on_address_change, exposed, &rtnl);
        if (r < 0)
            return r;
    ...
    if (IN_SET(arg_console_mode, CONSOLE_INTERACTIVE, CONSOLE_READ_ONLY))
    {
        ...
        if (arg_console_width != (unsigned)-1 ||
            arg_console_height != (unsigned)-1)
            (void)pty_forward_set_width_height(forward, arg_console_width,
                                               arg_console_height);
    }
    ...
    *pid = 0;
    ...
    if (r < 0)
        /* We failed to wait for the container, or the container exited
         * abnormally. */
        return r;
    if (r > 0 || container_status == CONTAINER_TERMINATED)
    {
        /* r > 0 → The container exited with a non-zero status.
         *         As a special case, we need to replace 133 with a different
         * value, because 133 is special-cased in the service file to reboot the
         * container. otherwise → The container exited with zero status and a
         * reboot was not requested.
         */
    ...
    if (arg_keep_unit)
    {
         ...
         * restarting the machine we want to restart the entire service, so
         * let's inform systemd about this with the special exit code 133. The
         * service file uses RestartForceExitStatus=133 so that this results in
         * a full nspawn restart. This is necessary since we might have cgroup
         * parameters set we want to have flushed out. */
...
{
    /* The default resource limits the kernel passes to PID 1, as per
     * kernel 4.16. Let's pass our container payload the same values as the
     * kernel originally passed to PID 1, in order to minimize differences
     * between host and container execution environments. */
    ...
    static const struct rlimit kernel_defaults[_RLIMIT_MAX] = {
        ...
        /* The kernel scales the default for RLIMIT_NPROC and RLIMIT_SIGPENDING
         * based on the system's amount of RAM. To provide best compatibility
         * we'll read these limits off PID 1 instead of hardcoding them here.
         * This is safe as we know that PID 1 doesn't change these two limits
         * and thus the original kernel's initialization should still be valid
         * during runtime — at least if PID 1 is systemd. Note that PID 1
         * changes a number of other resource limits during early initialization
         * which is why we
         * don't read the other limits from PID 1 but prefer the static table
         * above. */
    };
    ...
    for (rl = 0; rl < _RLIMIT_MAX; rl++)
    {
        ...
        if ((arg_settings_mask & (SETTING_RLIMIT_FIRST << rl)) == 0)
        {
            ...
            {
                /* For these two let's read the limits off PID 1. See above for
                 * an explanation. */

                if (prlimit(1, rl, NULL, &buffer) < 0)
                    return log_error_errno(
                        errno,
                        "Failed to read resource limit RLIMIT_%s of PID 1: %m",
                        rlimit_to_string(rl));
...
static int run(int argc, char* argv[])
{
    ...
    _cleanup_(release_lock_file) LockFile tree_global_lock = LOCK_FILE_INIT,
                                          tree_local_lock = LOCK_FILE_INIT;
    ...
    if (r < 0)
    {
        log_error_errno(r, "Failed to determine whether the unified cgroups "
                           "hierarchy is used: %m");
        goto finish;
    }
    ...
    /* Ignore SIGPIPE here, because we use splice() on the ptyfwd stuff and that
     * will generate SIGPIPE if the result is closed. Note that the container
     * payload child will reset signal mask+handler anyway,
     * so just turning this off here means we only turn it off in nspawn itself,
     * not any children. */
    ...
    if (arg_directory)
    {
        ...
        {
            log_error("Spawning container on root directory is not supported. "
                      "Consider using --ephemeral.");
            r = -EINVAL;
            goto finish;
        }
        ...
        if (arg_ephemeral)
        {
            ...
            {
                log_error_errno(r,
                                "Failed to determine whether directory %s is "
                                "mount point: %m",
                                arg_directory);
                goto finish;
            }
            ...
            {
                log_error_errno(
                    r, "Failed to generate name for directory snapshot: %m");
                goto finish;
            }

            r = image_path_lock(np,
                                (arg_read_only ? LOCK_SH : LOCK_EX) | LOCK_NB,
                                &tree_global_lock, &tree_local_lock);
            if (r < 0)
            {
            ...
            if (r < 0)
            {
                log_error_errno(r, "Failed to create snapshot %s from %s: %m",
                                np, arg_directory);
                goto finish;
            }
        ...
        else
        {
            r = chase_symlinks_and_update(&arg_directory,
                                          arg_template ? CHASE_NONEXISTENT : 0);
            if (r < 0)
                goto finish;

            r = image_path_lock(arg_directory,
                                (arg_read_only ? LOCK_SH : LOCK_EX) | LOCK_NB,
                                &tree_global_lock, &tree_local_lock);
            if (r == -EBUSY)
            {
                log_error_errno(r, "Directory tree %s is currently busy.",
                                arg_directory);
                goto finish;
            }
            if (r < 0)
            ...
            if (arg_template)
            {
                ...
                if (r == -EEXIST)
                    log_full(arg_quiet ? LOG_DEBUG : LOG_INFO,
                             "Directory %s already exists, not populating from "
                             "template %s.",
                             arg_directory, arg_template);
                else if (r < 0)
                {
                    log_error_errno(r,
                                    "Couldn't create snapshot %s from %s: %m",
                                    arg_directory, arg_template);
                    goto finish;
                }
                else
                    log_full(arg_quiet ? LOG_DEBUG : LOG_INFO,
                             "Populated %s from template %s.", arg_directory,
                             arg_template);
            }
        }
        ...
        else
        {
            ...
            if (laccess(q, F_OK) < 0)
            {
                log_error("Directory %s doesn't look like it has an OS tree. "
                          "Refusing.",
                          p);
                r = -EINVAL;
                goto finish;
    ...
    else
    {
        ...
        if (arg_ephemeral)
        {
            ...
            {
                log_error_errno(
                    r, "Failed to generate name for image snapshot: %m");
                goto finish;
            }
            ...
            r = copy_file(arg_image, np, O_EXCL, arg_read_only ? 0400 : 0600,
                          FS_NOCOW_FL, FS_NOCOW_FL, COPY_REFLINK | COPY_CRTIME);
            if (r < 0)
            {
        ...
        {
            r = image_path_lock(arg_image,
                                (arg_read_only ? LOCK_SH : LOCK_EX) | LOCK_NB,
                                &tree_global_lock, &tree_local_lock);
            if (r == -EBUSY)
            {
                r = log_error_errno(r, "Disk image %s is currently busy.",
                                    arg_image);
                goto finish;
            }
            if (r < 0)
            ...
            if (!arg_root_hash)
            {
                r = root_hash_load(arg_image, &arg_root_hash,
                                   &arg_root_hash_size);
                if (r < 0)
                {
                    log_error_errno(r,
                                    "Failed to load root hash file for %s: %m",
                                    arg_image);
                    goto finish;
                }
            }
        ...
        r = loop_device_make_by_path(arg_image,
                                     arg_read_only ? O_RDONLY : O_RDWR, &loop);
        if (r < 0)
        {
        ...
        r = dissect_image_and_warn(
            loop->fd, arg_image, arg_root_hash, arg_root_hash_size,
            DISSECT_IMAGE_REQUIRE_ROOT, &dissected_image);
        if (r == -ENOPKG)
        {
            /* dissected_image_and_warn() already printed a brief error message.
             * Extend on that with more details */
            log_notice(
                "Note that the disk image needs to\n"
                "    a) either contain only a single MBR partition of type "
                "0x83 that is marked bootable\n"
                "    b) or contain a single GPT partition of type "
        ...
        if (!arg_root_hash && dissected_image->can_verity)
            log_notice(
                "Note: image %s contains verity information, but no root hash "
                "specified! Proceeding without integrity checking.",
                arg_image);

        r = dissected_image_decrypt_interactively(
            dissected_image, NULL, arg_root_hash, arg_root_hash_size, 0,
            &decrypted_image);
        if (r < 0)
            goto finish;

        /* Now that we mounted the image, let's try to remove it again, if it is
         * ephemeral */
        if (remove_image && unlink(arg_image) >= 0)
            remove_image = false;
    }

    r = custom_mount_prepare_all(arg_directory, arg_custom_mounts,
                                 arg_n_custom_mounts);
    if (r < 0)
        goto finish;

    if (arg_console_mode < 0)
        arg_console_mode = isatty(STDIN_FILENO) > 0 && isatty(STDOUT_FILENO) > 0
                               ? CONSOLE_INTERACTIVE
                               : CONSOLE_READ_ONLY;

    if (arg_console_mode ==
        CONSOLE_PIPE) /* if we pass STDERR on to the container, don't add our
                         own logs into it too */
        arg_quiet = true;
    ...
    assert_se(sigprocmask_many(SIG_BLOCK, NULL, SIGCHLD, SIGWINCH, SIGTERM,
                               SIGINT, -1) >= 0);
    ...
    for (;;)
    {
        r = run_container(master, console, dissected_image, secondary, fds,
                          veth_name, &veth_created, &exposed, &pid, &ret);
        if (r <= 0)
            break;
    }

finish:
    (void)sd_notify(false, r == 0 && ret == EXIT_FORCE_RESTART
                               ? "STOPPING=1\nSTATUS=Restarting..."
                               : "STOPPING=1\nSTATUS=Terminating...");
    ...
    if (remove_directory && arg_directory)
    {
        ...
        k = rm_rf(arg_directory,
                  REMOVE_ROOT | REMOVE_PHYSICAL | REMOVE_SUBVOLUME);
        if (k < 0)
            log_warning_errno(k, "Cannot remove '%s', ignoring: %m",
                              arg_directory);
    }
    ...
    if (remove_image && arg_image)
    {
        if (unlink(arg_image) < 0)
            log_warning_errno(
                errno, "Can't remove image file '%s', ignoring: %m", arg_image);
    }
    ...
    if (remove_tmprootdir)
    {
        if (rmdir(tmprootdir) < 0)
            log_debug_errno(
                errno,
                "Can't remove temporary root directory '%s', ignoring: %m",
                tmprootdir);
    }
