...
static Client* client_get(const char* name, int create)
{
    ...
    AVAHI_LLIST_HEAD_INIT(SyncHostNameResolverInfo,
                          client->sync_host_name_resolvers);
    AVAHI_LLIST_HEAD_INIT(AsyncHostNameResolverInfo,
                          client->async_host_name_resolvers);
    AVAHI_LLIST_HEAD_INIT(SyncAddressResolverInfo,
                          client->sync_address_resolvers);
    AVAHI_LLIST_HEAD_INIT(AsyncAddressResolverInfo,
                          client->async_address_resolvers);
    AVAHI_LLIST_HEAD_INIT(DomainBrowserInfo, client->domain_browsers);
    AVAHI_LLIST_HEAD_INIT(ServiceTypeBrowserInfo,
                          client->service_type_browsers);
    AVAHI_LLIST_HEAD_INIT(ServiceBrowserInfo, client->service_browsers);
    AVAHI_LLIST_HEAD_INIT(SyncServiceResolverInfo,
                          client->sync_service_resolvers);
    AVAHI_LLIST_HEAD_INIT(AsyncServiceResolverInfo,
                          client->async_service_resolvers);
...
static void reconnect_callback(AvahiTimeout* t, AVAHI_GCC_UNUSED void* userdata)
{
    ...
    {
        struct timeval tv;
        avahi_log_debug(__FILE__ ": Connection failed, retrying in %ims...",
                        RECONNECT_MSEC);
...
static DBusHandlerResult
    msg_signal_filter_impl(AVAHI_GCC_UNUSED DBusConnection* c, DBusMessage* m,
                           AVAHI_GCC_UNUSED void* userdata)
{
    ...
    if (dbus_message_is_signal(m, DBUS_INTERFACE_LOCAL, "Disconnected"))
    {
        ...
        {
            avahi_log_warn(
                "Disconnected from D-Bus, trying to reconnect in %ims...",
                RECONNECT_MSEC);
            ...
            if (server->reconnect_timeout)
                server->poll_api->timeout_update(server->reconnect_timeout,
                                                 &tv);
            else
                server->reconnect_timeout = server->poll_api->timeout_new(
                    server->poll_api, &tv, reconnect_callback, NULL);
        }
        else
        {
    ...
    else if (dbus_message_is_signal(m, DBUS_INTERFACE_DBUS, "NameAcquired"))
    {
        ...
        if (!dbus_message_get_args(m, &error, DBUS_TYPE_STRING, &name,
                                   DBUS_TYPE_INVALID))
        {
...
static DBusHandlerResult msg_server_impl(DBusConnection* c, DBusMessage* m,
                                         AVAHI_GCC_UNUSED void* userdata)
{
    DBusError error;
    ...
    avahi_log_debug(__FILE__ ": interface=%s, path=%s, member=%s",
                    dbus_message_get_interface(m), dbus_message_get_path(m),
                    dbus_message_get_member(m));

    if (dbus_message_is_method_call(m, DBUS_INTERFACE_INTROSPECTABLE,
                                    "Introspect"))
        return avahi_dbus_handle_introspect(c, m,
                                            "org.freedesktop.Avahi.Server.xml");

    else if (dbus_message_is_method_call(m, AVAHI_DBUS_INTERFACE_SERVER,
                                         "GetHostName"))
    {
        ...
        return avahi_dbus_respond_string(
            c, m, avahi_server_get_host_name(avahi_server));
    }
    else if (dbus_message_is_method_call(m, AVAHI_DBUS_INTERFACE_SERVER,
                                         "SetHostName"))
    {
        ...
        if (avahi_server_set_host_name(avahi_server, name) < 0)
            return avahi_dbus_respond_error(
                                         ...
                                         "GetDomainName"))
    {
        ...
        return avahi_dbus_respond_string(
            c, m, avahi_server_get_domain_name(avahi_server));
    }
    else if (dbus_message_is_method_call(m, AVAHI_DBUS_INTERFACE_SERVER,
                                         "GetHostNameFqdn"))
    {
        ...
        return avahi_dbus_respond_string(
            c, m, avahi_server_get_host_name_fqdn(avahi_server));
    }
    else if (dbus_message_is_method_call(m, AVAHI_DBUS_INTERFACE_SERVER,
                                         "IsNSSSupportAvailable"))
    {
        ...
        {
            avahi_log_warn(
                "Error parsing Server::IsNSSSupportAvailable message");
            goto fail;
        }
                                         ...
                                         "GetLocalServiceCookie"))
    {
        ...
        {
            avahi_log_warn(
                "Error parsing Server::GetLocalServiceCookie message");
            goto fail;
        }

        return avahi_dbus_respond_uint32(
            c, m, avahi_server_get_local_service_cookie(avahi_server));
    }
    else if (dbus_message_is_method_call(m, AVAHI_DBUS_INTERFACE_SERVER,
                                         "GetNetworkInterfaceNameByIndex"))
    {
        ...
        if (!(dbus_message_get_args(m, &error, DBUS_TYPE_INT32, &idx,
                                    DBUS_TYPE_INVALID)))
        {
            avahi_log_warn(
                "Error parsing Server::GetNetworkInterfaceNameByIndex message");
            goto fail;
        }
                                         ...
                                         "GetNetworkInterfaceIndexByName"))
    {
        ...
        if (!(dbus_message_get_args(m, &error, DBUS_TYPE_STRING, &n,
                                    DBUS_TYPE_INVALID)) ||
            !n)
        {
            avahi_log_warn(
                "Error parsing Server::GetNetworkInterfaceIndexByName message");
            goto fail;
        }
                                         ...
                                         "GetAlternativeServiceName"))
    {
        ...
        {
            avahi_log_warn(
                "Error parsing Server::GetAlternativeServiceName message");
            goto fail;
        }
                                         ...
                                         "EntryGroupNew"))
    {
        ...
        static const DBusObjectPathVTable vtable = {
            NULL, avahi_dbus_msg_entry_group_impl, NULL, NULL, NULL, NULL};
        ...
        if (server->disable_user_service_publishing)
            return avahi_dbus_respond_error(c, m, AVAHI_ERR_NOT_PERMITTED,
                                            NULL);

        if (!(client = client_get(dbus_message_get_sender(m), TRUE)))
        {
            avahi_log_warn("Too many clients, client request failed.");
            return avahi_dbus_respond_error(c, m, AVAHI_ERR_TOO_MANY_CLIENTS,
                                            NULL);
        }
        ...
        {
            avahi_log_warn(
                "Too many objects for client '%s', client request failed.",
                client->name);
            return avahi_dbus_respond_error(c, m, AVAHI_ERR_TOO_MANY_OBJECTS,
                                            NULL);
        }
        ...
        i->path = NULL;
        i->n_entries = 0;
        AVAHI_LLIST_PREPEND(EntryGroupInfo, entry_groups, client->entry_groups,
                            i);
        client->n_objects++;

        if (!(i->entry_group = avahi_s_entry_group_new(
                  avahi_server, avahi_dbus_entry_group_callback, i)))
        {
        ...
        i->path =
            avahi_strdup_printf("/Client%u/EntryGroup%u", client->id, i->id);
                                         ...
                                         "ResolveHostName"))
    {
        ...
        if (!dbus_message_get_args(
                m, &error, DBUS_TYPE_INT32, &interface, DBUS_TYPE_INT32,
                &protocol, DBUS_TYPE_STRING, &name, DBUS_TYPE_INT32, &aprotocol,
        ...
        AVAHI_LLIST_PREPEND(SyncHostNameResolverInfo, sync_host_name_resolvers,
                            client->sync_host_name_resolvers, i);
        client->n_objects++;

        if (!(i->host_name_resolver = avahi_s_host_name_resolver_new(
                  avahi_server, (AvahiIfIndex)interface,
                  (AvahiProtocol)protocol, name, (AvahiProtocol)aprotocol,
                  (AvahiLookupFlags)flags,
                  avahi_dbus_sync_host_name_resolver_callback, i)))
        {
                                         ...
                                         "ResolveAddress"))
    {
        ...
        if (!dbus_message_get_args(m, &error, DBUS_TYPE_INT32, &interface,
                                   DBUS_TYPE_INT32, &protocol, DBUS_TYPE_STRING,
                                   &address, DBUS_TYPE_UINT32, &flags,
                                   DBUS_TYPE_INVALID) ||
            !address)
        {
        ...
        if (!avahi_address_parse(address, AVAHI_PROTO_UNSPEC, &a))
            return avahi_dbus_respond_error(c, m, AVAHI_ERR_INVALID_ADDRESS,
                                            NULL);
        ...
        i->client = client;
        i->message = dbus_message_ref(m);
        AVAHI_LLIST_PREPEND(SyncAddressResolverInfo, sync_address_resolvers,
                            client->sync_address_resolvers, i);
        client->n_objects++;

        if (!(i->address_resolver = avahi_s_address_resolver_new(
                  avahi_server, (AvahiIfIndex)interface,
                  (AvahiProtocol)protocol, &a, (AvahiLookupFlags)flags,
                  avahi_dbus_sync_address_resolver_callback, i)))
        {
                                         ...
                                         "DomainBrowserNew"))
    {
        ...
        DomainBrowserInfo* i;
        static const DBusObjectPathVTable vtable = {
            NULL, avahi_dbus_msg_domain_browser_impl, NULL, NULL, NULL, NULL};
        ...
        if (!dbus_message_get_args(
                m, &error, DBUS_TYPE_INT32, &interface, DBUS_TYPE_INT32,
                &protocol, DBUS_TYPE_STRING, &domain, DBUS_TYPE_INT32, &type,
                DBUS_TYPE_UINT32, &flags, DBUS_TYPE_INVALID) ||
            type < 0 || type >= AVAHI_DOMAIN_BROWSER_MAX)
        {
        ...
        i->client = client;
        i->path = NULL;
        AVAHI_LLIST_PREPEND(DomainBrowserInfo, domain_browsers,
                            client->domain_browsers, i);
        client->n_objects++;

        if (!(i->domain_browser = avahi_s_domain_browser_new(
                  avahi_server, (AvahiIfIndex)interface,
                  (AvahiProtocol)protocol, domain, (AvahiDomainBrowserType)type,
                  (AvahiLookupFlags)flags, avahi_dbus_domain_browser_callback,
                  i)))
        {
        ...
        i->path =
            avahi_strdup_printf("/Client%u/DomainBrowser%u", client->id, i->id);
        dbus_connection_register_object_path(c, i->path, &vtable, i);
        return avahi_dbus_respond_path(c, m, i->path);
    }
    else if (dbus_message_is_method_call(m, AVAHI_DBUS_INTERFACE_SERVER,
                                         "ServiceTypeBrowserNew"))
    {
        ...
        static const DBusObjectPathVTable vtable = {
            NULL, avahi_dbus_msg_service_type_browser_impl, NULL, NULL, NULL,
            NULL};
        int32_t interface, protocol;
        uint32_t flags;
        char* domain;
        ...
        if (!dbus_message_get_args(m, &error, DBUS_TYPE_INT32, &interface,
                                   DBUS_TYPE_INT32, &protocol, DBUS_TYPE_STRING,
                                   &domain, DBUS_TYPE_UINT32, &flags,
                                   DBUS_TYPE_INVALID))
        {
            avahi_log_warn(
                "Error parsing Server::ServiceTypeBrowserNew message");
            goto fail;
        }
        ...
        i->client = client;
        i->path = NULL;
        AVAHI_LLIST_PREPEND(ServiceTypeBrowserInfo, service_type_browsers,
                            client->service_type_browsers, i);
        client->n_objects++;

        if (!(i->service_type_browser = avahi_s_service_type_browser_new(
                  avahi_server, (AvahiIfIndex)interface,
                  (AvahiProtocol)protocol, domain, (AvahiLookupFlags)flags,
                  avahi_dbus_service_type_browser_callback, i)))
        {
        ...
        i->path = avahi_strdup_printf("/Client%u/ServiceTypeBrowser%u",
                                      client->id, i->id);
        dbus_connection_register_object_path(c, i->path, &vtable, i);
        return avahi_dbus_respond_path(c, m, i->path);
    }
                                         ...
                                         "ServiceBrowserNew"))
    {
        ...
        static const DBusObjectPathVTable vtable = {
            NULL, avahi_dbus_msg_service_browser_impl, NULL, NULL, NULL, NULL};
        int32_t interface, protocol;
        uint32_t flags;
        char *domain, *type;
        ...
        if (!dbus_message_get_args(
                m, &error, DBUS_TYPE_INT32, &interface, DBUS_TYPE_INT32,
                &protocol, DBUS_TYPE_STRING, &type, DBUS_TYPE_STRING, &domain,
                DBUS_TYPE_UINT32, &flags, DBUS_TYPE_INVALID) ||
            !type)
        {
        ...
        i->client = client;
        i->path = NULL;
        AVAHI_LLIST_PREPEND(ServiceBrowserInfo, service_browsers,
                            client->service_browsers, i);
        client->n_objects++;

        if (!(i->service_browser = avahi_s_service_browser_new(
                  avahi_server, (AvahiIfIndex)interface,
                  (AvahiProtocol)protocol, type, domain,
                  (AvahiLookupFlags)flags, avahi_dbus_service_browser_callback,
                  i)))
        {
        ...
        i->path = avahi_strdup_printf("/Client%u/ServiceBrowser%u", client->id,
                                      i->id);
        dbus_connection_register_object_path(c, i->path, &vtable, i);
        return avahi_dbus_respond_path(c, m, i->path);
    }
                                         ...
                                         "ResolveService"))
    {
        ...
        if (!dbus_message_get_args(
                m, &error, DBUS_TYPE_INT32, &interface, DBUS_TYPE_INT32,
                &protocol, DBUS_TYPE_STRING, &name, DBUS_TYPE_STRING, &type,
                DBUS_TYPE_STRING, &domain, DBUS_TYPE_INT32, &aprotocol,
                DBUS_TYPE_UINT32, &flags, DBUS_TYPE_INVALID) ||
            !type)
        {
        ...
        i->client = client;
        i->message = dbus_message_ref(m);
        AVAHI_LLIST_PREPEND(SyncServiceResolverInfo, sync_service_resolvers,
                            client->sync_service_resolvers, i);
        client->n_objects++;

        if (!(i->service_resolver = avahi_s_service_resolver_new(
                  avahi_server, (AvahiIfIndex)interface,
                  (AvahiProtocol)protocol, name, type, domain,
                  (AvahiProtocol)aprotocol, (AvahiLookupFlags)flags,
                  avahi_dbus_sync_service_resolver_callback, i)))
        {
                                         ...
                                         "ServiceResolverNew"))
    {
        ...
        static const DBusObjectPathVTable vtable = {
            NULL, avahi_dbus_msg_async_service_resolver_impl, NULL, NULL, NULL,
            NULL};
        ...
        if (!(client = client_get(dbus_message_get_sender(m), TRUE)))
        {
            avahi_log_warn(__FILE__
                           ": Too many clients, client request failed.");
            return avahi_dbus_respond_error(c, m, AVAHI_ERR_TOO_MANY_CLIENTS,
                                            NULL);
        }
        ...
        i->client = client;
        i->path = NULL;
        AVAHI_LLIST_PREPEND(AsyncServiceResolverInfo, async_service_resolvers,
                            client->async_service_resolvers, i);
        client->n_objects++;
        ...
        if (!(i->service_resolver = avahi_s_service_resolver_new(
                  ...
                  (AvahiProtocol)protocol, name, type, domain,
                  (AvahiProtocol)aprotocol, (AvahiLookupFlags)flags,
                  avahi_dbus_async_service_resolver_callback, i)))
        {
        ...
        /*         avahi_log_debug(__FILE__": [%s], new service resolver for
         * <%s.%s.%s>", i->path, name, type, domain); */

        i->path = avahi_strdup_printf("/Client%u/ServiceResolver%u", client->id,
                                      i->id);
        dbus_connection_register_object_path(c, i->path, &vtable, i);
        return avahi_dbus_respond_path(c, m, i->path);
    }
                                         ...
                                         "HostNameResolverNew"))
    {
        ...
        static const DBusObjectPathVTable vtable = {
            NULL, avahi_dbus_msg_async_host_name_resolver_impl,
            NULL, NULL,
            NULL, NULL};
        ...
        i->client = client;
        i->path = NULL;
        AVAHI_LLIST_PREPEND(AsyncHostNameResolverInfo,
                            async_host_name_resolvers,
                            client->async_host_name_resolvers, i);
        client->n_objects++;

        if (!(i->host_name_resolver = avahi_s_host_name_resolver_new(
                  avahi_server, (AvahiIfIndex)interface,
                  (AvahiProtocol)protocol, name, aprotocol,
                  (AvahiLookupFlags)flags,
                  avahi_dbus_async_host_name_resolver_callback, i)))
        {
        ...
        i->path = avahi_strdup_printf("/Client%u/HostNameResolver%u",
                                      client->id, i->id);
        dbus_connection_register_object_path(c, i->path, &vtable, i);
        return avahi_dbus_respond_path(c, m, i->path);
    }
                                         ...
                                         "AddressResolverNew"))
    {
        ...
        AvahiAddress a;
        static const DBusObjectPathVTable vtable = {
            NULL, avahi_dbus_msg_async_address_resolver_impl, NULL, NULL, NULL,
            NULL};
        ...
        i->client = client;
        i->path = NULL;
        AVAHI_LLIST_PREPEND(AsyncAddressResolverInfo, async_address_resolvers,
                            client->async_address_resolvers, i);
        client->n_objects++;

        if (!(i->address_resolver = avahi_s_address_resolver_new(
                  avahi_server, (AvahiIfIndex)interface,
                  (AvahiProtocol)protocol, &a, (AvahiLookupFlags)flags,
                  avahi_dbus_async_address_resolver_callback, i)))
        {
        ...
        i->path = avahi_strdup_printf("/Client%u/AddressResolver%u", client->id,
                                      i->id);
        dbus_connection_register_object_path(c, i->path, &vtable, i);
        return avahi_dbus_respond_path(c, m, i->path);
    }
                                         ...
                                         "RecordBrowserNew"))
    {
        ...
        RecordBrowserInfo* i;
        static const DBusObjectPathVTable vtable = {
            NULL, avahi_dbus_msg_record_browser_impl, NULL, NULL, NULL, NULL};
        int32_t interface, protocol;
        uint32_t flags;
        char* name;
        uint16_t type, clazz;
        ...
        if (!dbus_message_get_args(m, &error, DBUS_TYPE_INT32, &interface,
                                   DBUS_TYPE_INT32, &protocol, DBUS_TYPE_STRING,
                                   &name, DBUS_TYPE_UINT16, &clazz,
                                   DBUS_TYPE_UINT16, &type, DBUS_TYPE_UINT32,
        ...
        i->client = client;
        i->path = NULL;
        AVAHI_LLIST_PREPEND(RecordBrowserInfo, record_browsers,
                            client->record_browsers, i);
        client->n_objects++;

        key = avahi_key_new(name, clazz, type);
        assert(key);

        if (!(i->record_browser = avahi_s_record_browser_new(
                  avahi_server, (AvahiIfIndex)interface,
                  (AvahiProtocol)protocol, key, (AvahiLookupFlags)flags,
                  avahi_dbus_record_browser_callback, i)))
        {
            avahi_key_unref(key);
        ...
        i->path =
            avahi_strdup_printf("/Client%u/RecordBrowser%u", client->id, i->id);
        dbus_connection_register_object_path(c, i->path, &vtable, i);
        return avahi_dbus_respond_path(c, m, i->path);
    }

    avahi_log_warn("Missed message %s::%s()", dbus_message_get_interface(m),
                   dbus_message_get_member(m));
...
void dbus_protocol_server_state_changed(AvahiServerState state)
{
    ...
    m = dbus_message_new_signal(AVAHI_DBUS_PATH_SERVER,
                                AVAHI_DBUS_INTERFACE_SERVER, "StateChanged");
    ...
    dbus_message_append_args(m, DBUS_TYPE_INT32, &t, DBUS_TYPE_STRING, &e,
                             DBUS_TYPE_INVALID);
...
static int dbus_connect(void)
{
    ...
    static const DBusObjectPathVTable server_vtable = {
        NULL, msg_server_impl, NULL, NULL, NULL, NULL};
    ...
    if (dbus_bus_request_name(server->bus, AVAHI_DBUS_NAME,
    ...
    if (!(dbus_connection_register_object_path(
            server->bus, AVAHI_DBUS_PATH_SERVER, &server_vtable, NULL)))
    {
...
int dbus_protocol_setup(const AvahiPoll* poll_api,
                        ...
                        int _n_clients_max, int _n_objects_per_client_max,
                        int _n_entries_per_entry_group_max, int force)
{
    ...
    server->n_clients_max =
        _n_clients_max > 0 ? _n_clients_max : DEFAULT_CLIENTS_MAX;
    server->n_objects_per_client_max = _n_objects_per_client_max > 0
                                           ? _n_objects_per_client_max
                                           : DEFAULT_OBJECTS_PER_CLIENT_MAX;
    server->n_entries_per_entry_group_max =
        _n_entries_per_entry_group_max > 0
            ? _n_entries_per_entry_group_max
            : DEFAULT_ENTRIES_PER_ENTRY_GROUP_MAX;
    ...
    if (dbus_connect() < 0)
    {
        ...
        avahi_log_warn(
            "WARNING: Failed to contact D-Bus daemon, retrying in %ims.",
            RECONNECT_MSEC);
