...
void _xdg_mime_parent_read_from_file(XdgParentList* list, const char* file_name)
{
        ...
        *(sep++) = '\000';
        ...
        if (!entry)
        {
            if (list->n_mimes == alloc)
            {
                alloc <<= 1;
                list->parents =
                    realloc(list->parents, alloc * sizeof(XdgMimeParents));
            }
        ...
        else
        {
            entry->n_parents += 1;
            entry->parents =
                realloc(entry->parents, (entry->n_parents + 2) * sizeof(char*));
        }
    ...
    list->parents =
        realloc(list->parents, list->n_mimes * sizeof(XdgMimeParents));

    fclose(file);

    if (list->n_mimes > 1)
        qsort(list->parents, list->n_mimes, sizeof(XdgMimeParents),
              &parent_entry_cmp);
}
