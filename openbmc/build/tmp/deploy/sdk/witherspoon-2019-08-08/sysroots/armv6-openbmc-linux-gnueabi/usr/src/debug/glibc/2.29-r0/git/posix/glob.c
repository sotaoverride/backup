...
int GLOB_ATTRIBUTE __glob(const char* pattern, int flags,
                          int (*errfunc)(const char*, int), glob_t* pglob)
{
    const char* filename;
    char* dirname = NULL;
...
no_brace:
        ...
        else
        {
            ...
            else
            {
                ...
                if (unescape != NULL)
                {
                    ...
                    *((char*)mempcpy(newp, dirname + 1,
                                     end_name - dirname - 1)) = '\0';
                user_name = newp;
            }
            ...
            /* Look up specific user's home directory.  */
            {
                ...
                else
                {
                    if (flags & GLOB_TILDE_CHECK)
                    {
                        /* We have to regard it as an error if we cannot find
                           the home directory.  */
                        retval = GLOB_NOMATCH;
                        goto out;
                    }
    ...
    if (filename == NULL)
    {
        ...
        new_gl_pathv = realloc(pglob->gl_pathv, (newcount + 2) * sizeof(char*));
    ...
    else
    {
        ...
        if (dirname_modified)
            flags &= ~(GLOB_NOCHECK | GLOB_NOMAGIC);
        status =
            glob_in_dir(filename, dirname, flags, errfunc, pglob, alloca_used);
        if (status != 0)
        {
...
#define GLOBNAMES_MEMBERS(nnames)                                              \
    ...
    else if (meta == GLOBPAT_NONE)
    {
        ...
        mempcpy(mempcpy(mempcpy(fullname, directory, dirlen), "/", 1), pattern,
                patlen + 1);
