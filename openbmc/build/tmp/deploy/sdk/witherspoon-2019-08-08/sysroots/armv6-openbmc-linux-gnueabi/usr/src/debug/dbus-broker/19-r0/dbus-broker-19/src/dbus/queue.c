...
int iqueue_set_target(IQueue* iq, void* data, size_t n_data)
{
     ...
     * them on a peer.
     */
    r = user_charge(iq->user, &iq->pending.charge_data, NULL, USER_SLOT_BYTES,
                    n_data);
    if (r)
 ...
 */
int iqueue_get_cursor(IQueue* iq, void** bufferp, size_t** fromp, size_t* top,
                      FDList*** fdsp, UserCharge** charge_fdsp)
{
    void* p;
    int r;
     ...
     */
    memmove(iq->data, iq->data + iq->data_start, iq->data_end - iq->data_start);
    ...
    if (_c_unlikely_(iq->data_size <= iq->data_end))
    {
        ...
        r = user_charge(iq->user, &iq->charge_data, NULL, USER_SLOT_BYTES,
                        IQUEUE_LINE_MAX);
        if (r)
        {
            free(p);
     ...
     */
    if (iq->pending.n_data - iq->pending.n_copied >=
        iq->data_size - iq->data_end)
    {
    ...
    *bufferp = iq->data;
    *fromp = &iq->data_end;
    *top = (iq->data_size - iq->data_end) > IQUEUE_RECV_MAX
               ? iq->data_end + IQUEUE_RECV_MAX
               : iq->data_size;
    *fdsp = &iq->fds;
...
int iqueue_pop_line(IQueue* iq, const char** linep, size_t* np)
{
    ...
    for (; iq->data_cursor < iq->data_end; ++iq->data_cursor)
    {
         ...
         * length.
         */
        if (iq->data_cursor > 0 && iq->data[iq->data_cursor] == '\n' &&
