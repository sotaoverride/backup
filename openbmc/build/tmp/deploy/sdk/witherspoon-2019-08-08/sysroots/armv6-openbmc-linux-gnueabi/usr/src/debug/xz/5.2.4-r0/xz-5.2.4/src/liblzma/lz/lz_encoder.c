...
///
static lzma_ret fill_window(lzma_coder* coder, const lzma_allocator* allocator,
                            ...
                            lzma_action action)
{
    ...
    if (coder->next.code == NULL)
    {
        ...
        lzma_bufcpy(in, in_pos, in_size, coder->mf.buffer, &write_pos,
                    coder->mf.size);

        ret = action != LZMA_RUN && *in_pos == in_size ? LZMA_STREAM_END
                                                       : LZMA_OK;
    }
    else
    {
        ret = coder->next.code(coder->next.coder, allocator, in, in_pos,
                               in_size, coder->mf.buffer, &write_pos,
...
static lzma_ret lz_encode(void* coder_ptr, const lzma_allocator* allocator,
                          ...
                          lzma_action action)
{
    ...
    while (*out_pos < out_size && (*in_pos < in_size || action != LZMA_RUN))
    {
        ...
        if (coder->mf.action == LZMA_RUN &&
            ...
            return_if_error(
                fill_window(coder, allocator, in, in_pos, in_size, action));

        // Encode
        const lzma_ret ret =
            coder->lz.code(coder->lz.coder, &coder->mf, out, out_pos, out_size);
        if (ret != LZMA_OK)
        {
...
static bool lz_encoder_prepare(lzma_mf* mf, const lzma_allocator* allocator,
                               const lzma_lz_options* lz_options)
{
    ...
    if (lz_options->dict_size < LZMA_DICT_SIZE_MIN ||
        lz_options->dict_size > (UINT32_C(1) << 30) + (UINT32_C(1) << 29) ||
...
static bool lz_encoder_init(lzma_mf* mf, const lzma_allocator* allocator,
                            const lzma_lz_options* lz_options)
{
    ...
    if (mf->buffer == NULL)
    {
        ...
        mf->buffer = lzma_alloc(mf->size + LZMA_MEMCMPLEN_EXTRA, allocator);
        if (mf->buffer == NULL)
            return true;
    ...
    {
        mf->hash =
            lzma_alloc_zero(mf->hash_count * sizeof(uint32_t), allocator);
        mf->son = lzma_alloc(mf->sons_count * sizeof(uint32_t), allocator);
    ...
    if (lz_options->preset_dict != NULL && lz_options->preset_dict_size > 0)
    {
        ...
        memcpy(mf->buffer,
               lz_options->preset_dict + lz_options->preset_dict_size -
                   mf->write_pos,
               mf->write_pos);
...
static lzma_ret lz_encoder_update(
    ...
    const lzma_filter* reversed_filters)
{
    ...
    return_if_error(
        coder->lz.options_update(coder->lz.coder, reversed_filters));

    return lzma_next_filter_update(&coder->next, allocator,
                                   reversed_filters + 1);
}
...
extern lzma_ret lzma_lz_encoder_init(
    ...
    lzma_ret (*lz_init)(lzma_lz_encoder* lz, const lzma_allocator* allocator,
                        const void* options, lzma_lz_options* lz_options))
{
    ...
    return_if_error(
        lz_init(&coder->lz, allocator, filters[0].options, &lz_options));
...
extern LZMA_API(lzma_bool) lzma_mf_is_supported(lzma_match_finder mf)
{
    bool ret = false;
