...
#define ACT_STATE_RTRY 3 /* failure occured, trying to restablish ready state  \
                          */
#define ACT_STATE_SUSP                                                         \
    4 /* suspended due to failure (return fail until timeout expired) */
...
typedef struct actWrkrInfo
{
    action_t* pAction;
    void* actWrkrData;
    uint16_t uResumeOKinRow; /* number of times in a row that resume said OK
                    with an immediate failure following */
    ...
    union
    {
        ...
        {
            actWrkrIParams_t*
                iparams; /* dynamically sized array for transactional outputs */
            int currIParam;
...
struct wti_s
{
    ...
    int bIsRunning; /* is this thread currently running? (must be int for atomic
                       op!) */
    sbool bAlwaysRunning;     /* should this thread always run? */
    int* pbShutdownImmediate; /* end processing of this batch immediately if set
                                 to 1 */
    wtp_t* pWtp;   /* my worker thread pool (important if only the work thread
                      instance is passed! */
    ...
    pthread_cond_t pcondBusy;   /* condition to wake up the worker, protected by
                                   pmutUsr in wtp */
    DEF_ATOMIC_HELPER_MUT(mutIsRunning)
    struct
    {
        uint8_t
            script_errno; /* errno-type interface for RainerScript functions */
        uint8_t bPrevWasSuspended;
        uint8_t bDoAutoCommit; /* do a commit after each message
                                * this is usually set for batches with 0
                                * element, but may also be added as a
                                * user-selectable option (not implemented yet)
                                */
...
#define getActionStateByNbr(pWti, iActNbr)                                     \
    ((uint8_t)((pWti)->actWrkrInfo[(iActNbr)].flags.actState))
#define getActionState(pWti, pAction)                                          \
    (((uint8_t)(pWti)->actWrkrInfo[(pAction)->iActionNbr].flags.actState))
#define setActionState(pWti, pAction, newState)                                \
    ((pWti)->actWrkrInfo[(pAction)->iActionNbr].flags.actState = (newState))
#define getActionResumeInRow(pWti, pAction)                                    \
    (((pWti)->actWrkrInfo[(pAction)->iActionNbr].uResumeOKinRow))
#define setActionResumeInRow(pWti, pAction, val)                               \
    ((pWti)->actWrkrInfo[(pAction)->iActionNbr].uResumeOKinRow = (val))
#define incActionResumeInRow(pWti, pAction)                                    \
    ((pWti)->actWrkrInfo[(pAction)->iActionNbr].uResumeOKinRow++)
#define getActionNbrResRtry(pWti, pAction)                                     \
    (((pWti)->actWrkrInfo[(pAction)->iActionNbr].iNbrResRtry))
#define setActionNbrResRtry(pWti, pAction, val)                                \
    ((pWti)->actWrkrInfo[(pAction)->iActionNbr].iNbrResRtry = (val))
#define incActionNbrResRtry(pWti, pAction)                                     \
    ((pWti)->actWrkrInfo[(pAction)->iActionNbr].iNbrResRtry++)
#define wtiInitIParam(piparams)                                                \
    (memset((piparams), 0, sizeof(actWrkrIParams_t)))

#define wtiGetScriptErrno(pWti) ((pWti)->execState.script_errno)
#define wtiSetScriptErrno(pWti, newval)                                        \
