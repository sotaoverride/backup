...
static void run_emergency_cleanup(void)
{
    emergency_cleanup_item_t next;
    void (*f)(void);
...
char* _gpgrt_strconcat(const char* s1, ...)
{
    va_list arg_ptr;
    char* result;
...
char* _gpg_w32ce_strerror(int err)
{
    ...
    if (FormatMessageW(FORMAT_MESSAGE_FROM_SYSTEM, NULL, err,
                       MAKELANGID(LANG_NEUTRAL, SUBLANG_DEFAULT), tmpbuf,
                       STRBUFFER_SIZE - 1, NULL))
    {
        n = WideCharToMultiByte(CP_UTF8, 0, tmpbuf, -1, tls->strerror_buffer,
