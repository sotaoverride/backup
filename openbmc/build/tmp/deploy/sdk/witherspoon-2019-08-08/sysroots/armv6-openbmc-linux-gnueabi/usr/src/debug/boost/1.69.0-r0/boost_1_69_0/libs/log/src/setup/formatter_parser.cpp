...
BOOST_LOG_ANONYMOUS_NAMESPACE
{
    ...
    {
        //! Base class type
        typedef log::aux::lazy_singleton<formatters_repository<CharT>>
            base_type;
        ...
        typedef std::map<attribute_name, shared_ptr<formatter_factory_type>,
        ...
        formatter_factory_type& get_factory(attribute_name const& name) const
        {
            ...
            else
            {
                ...
                BOOST_LOG_THROW_DESCR(
                    setup_error,
                    "No formatter factory registered for attribute " +
                        name.string());
#endif
            }
        }
    ...
    class formatter_parser
    {
            ...
            while (p != end)
            {
                ...
                if (p != end)
                {
                    ...
                    if (*p == constants::char_paren_bracket_left)
                    {
                        ...
                        p = parse_args(constants::trim_spaces_left(++p, end),
                                       end);
                        p = constants::trim_spaces_left(p, end);
                        if (p == end)
                            BOOST_LOG_THROW_DESCR(
                                parse_error, "Invalid attribute placeholder in "
                                             "the formatter string");
                    }
      ...
      private:
        ...
        iterator_type parse_args(iterator_type begin, iterator_type end)
        {
            ...
            if (p == end)
                BOOST_LOG_THROW_DESCR(parse_error,
                                      "Invalid attribute placeholder arguments "
                                      "description in the formatter string");
            ...
            while (true)
            {
                ...
                if (!encoding::isalpha(*p))
                    BOOST_LOG_THROW_DESCR(
                        parse_error, "Placeholder argument name is invalid");
                for (++p; p != end; ++p)
                {
                    c = *p;
                ...
                if (p == end)
                    BOOST_LOG_THROW_DESCR(
                        parse_error, "Invalid attribute placeholder arguments "
                                     "description in the formatter string");
        ...
        void on_attribute_name(iterator_type begin, iterator_type end)
        {
            ...
            // For compatibility with Boost.Log v1 we recognize %_% as the
            // message attribute name
            const std::size_t len = end - begin;
            if (std::char_traits<char_type>::length(
                    constants::message_text_keyword()) == len &&
                std::char_traits<char_type>::compare(
                    constants::message_text_keyword(), begin, len) == 0)
            {
            ...
            else
            {
                m_AttrName = attribute_name(
                    log::aux::to_narrow(string_type(begin, end)));
            }
        }
        ...
        void push_attr()
        {
            ...
            else
            {
                ...
                formatters_repository<char_type> const& repo =
                    formatters_repository<char_type>::get();
                formatter_factory_type& factory = repo.get_factory(m_AttrName);
                append_formatter(
                    factory.create_formatter(m_AttrName, m_FactoryArgs));
            }
        ...
        void append_formatter(FormatterT fmt)
        {
            if (!!m_Formatter)
                m_Formatter =
                    boost::in_place(chained_formatter<char_type, FormatterT>(
                        boost::move(m_Formatter.get()), boost::move(fmt)));
            else
        ...
        BOOST_DELETED_FUNCTION(
            formatter_parser& operator=(formatter_parser const&))
    };
...
template <typename CharT>
BOOST_LOG_SETUP_API basic_formatter<CharT> parse_formatter(const CharT* begin,
                                                           const CharT* end)
{
    typedef CharT char_type;
    ...
    BOOST_LOG_EXPR_IF_MT(formatters_repository<CharT>& repo =
                             formatters_repository<CharT>::get();)
    BOOST_LOG_EXPR_IF_MT(
...
template BOOST_LOG_SETUP_API void register_formatter_factory<char>(
    attribute_name const& attr_name,
    shared_ptr<formatter_factory<char>> const& factory);
template BOOST_LOG_SETUP_API basic_formatter<char>
    parse_formatter<char>(const char* begin, const char* end);
#endif // BOOST_LOG_USE_CHAR

#ifdef BOOST_LOG_USE_WCHAR_T
template BOOST_LOG_SETUP_API void register_formatter_factory<wchar_t>(
    attribute_name const& attr_name,
    shared_ptr<formatter_factory<wchar_t>> const& factory);
template BOOST_LOG_SETUP_API basic_formatter<wchar_t>
