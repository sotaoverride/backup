 ...
 * character was read, the maximum line length was reached, or the end of the
 * stream was reached */
...
void stdout_stream_free(StdoutStream* s)
{
    ...
    if (s->server)
    {
        ...
        if (s->in_notify_queue)
            LIST_REMOVE(stdout_stream_notify_queue,
...
static int stdout_stream_save(StdoutStream* s)
{
    ...
    if (!s->state_file)
    {
        ...
        if (asprintf(&s->state_file, "/run/systemd/journal/streams/%lu:%lu",
                     (unsigned long)st.st_dev, (unsigned long)st.st_ino) < 0)
            return log_oom();
    }
    ...
    {
        LIST_PREPEND(stdout_stream_notify_queue,
                     s->server->stdout_streams_notify_queue, s);
        s->in_notify_queue = true;

        if (s->server->notify_event_source)
        {
            r = sd_event_source_set_enabled(s->server->notify_event_source,
                                            SD_EVENT_ON);
            if (r < 0)
                log_warning_errno(r,
...
fail:
    ...
    return log_error_errno(r, "Failed to save stream data %s: %m",
                           s->state_file);
}
                             ...
                             LineBreak line_break)
{
    ...
    if (s->context)
        (void)client_context_maybe_refresh(s->server, s->context, NULL, NULL, 0,
                                           NULL, USEC_INFINITY);
    else if (pid_is_valid(s->ucred.pid))
    {
        r = client_context_acquire(s->server, s->ucred.pid, &s->ucred, s->label,
                                   strlen_ptr(s->label), s->unit_id,
                                   &s->context);
        if (r < 0)
            log_warning_errno(r,
    ...
    if (s->forward_to_syslog || s->server->forward_to_syslog)
        server_forward_syslog(s->server, syslog_fixup_facility(priority),
                              s->identifier, p, &s->ucred, NULL);
    ...
    m = N_IOVEC_META_FIELDS + 7 +
        client_context_extra_fields_n_iovec(s->context);
    ...
    if (line_break != LINE_BREAK_NEWLINE)
    {
        ...
        /* If this log message was generated due to an uncommon line break then
         * mention this in the log entry */
    ...
    server_dispatch_message(s->server, iovec, n, m, s->context, NULL, priority,
                            0);
    return 0;
}
...
static int stdout_stream_line(StdoutStream* s, char* p, LineBreak line_break)
{
    ...
    /* line breaks by NUL, line max length or EOF are not permissible during the
     * negotiation part of the protocol */
    ...
    switch (s->state)
    {
        ...
        case STDOUT_STREAM_FORWARD_TO_CONSOLE:
            ...
            /* Try to save the stream, so that journald can be restarted and we
             * can recover */
                                 ...
                                 void* userdata)
{
    ...
    {
        log_error("Got invalid event from epoll for stdout stream: %" PRIx32,
                  revents);
        goto terminate;
    }

    /* If the buffer is full already (discounting the extra NUL we need), add
     * room for another 1K */
    ...
    /* Try to make use of the allocated buffer in full, but never read more than
     * the configured line size. Also, always leave room for a terminating NUL
...
int stdout_stream_install(Server* s, int fd, StdoutStream** ret)
{
    ...
    if (mac_selinux_use())
    {
        ...
        if (r < 0 && r != -EOPNOTSUPP)
            (void)log_warning_errno(
                r, "Failed to determine peer security context: %m");
    }
    ...
    if (r < 0)
        return log_error_errno(
            r, "Failed to adjust stdout event source priority: %m");
...
static int stdout_stream_new(sd_event_source* es, int listen_fd,
                             uint32_t revents, void* userdata)
{
    ...
    if (s->n_stdout_streams >= STDOUT_STREAMS_MAX)
    {
        ...
        /* By closing fd here we make sure that the client won't wait too long
         * for journald to gather all the data it adds to the error message to
         * find out that the connection has just been refused.
         */
        fd = safe_close(fd);
...
static int stdout_stream_load(StdoutStream* stream, const char* fname)
{
    ...
    r = parse_env_file(NULL, stream->state_file, "PRIORITY", &priority,
                       ...
                       &stream->identifier, "UNIT", &stream->unit_id,
                       "STREAM_ID", &stream_id);
    if (r < 0)
...
int server_restore_streams(Server* s, FDSet* fds)
{
    ...
    if (!d)
    {
        ...
        return log_warning_errno(
            errno, "Failed to enumerate /run/systemd/journal/streams: %m");
    }
    ...
    FOREACH_DIRENT(de, d, goto fail)
    {
        ...
        FDSET_FOREACH(fd, fds, i)
        {
            ...
            if (fstat(fd, &st) < 0)
                return log_error_errno(errno, "Failed to stat %s: %m",
                                       de->d_name);
...
int server_open_stdout_socket(Server* s)
{
    ...
    if (r < 0)
        return log_error_errno(
            r, "Failed to add stdout server fd to event source: %m");

    r = sd_event_source_set_priority(s->stdout_event_source,
                                     SD_EVENT_PRIORITY_NORMAL + 5);
    if (r < 0)
        return log_error_errno(
            r, "Failed to adjust priority of stdout server event source: %m");
