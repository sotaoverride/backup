...
typedef struct
{
    ...
    DBusAuthDataFunction
        server_data_func; /**< Function on server side for DATA */
    DBusAuthEncodeFunction
        server_encode_func; /**< Function on server side to encode */
    DBusAuthDecodeFunction
        server_decode_func; /**< Function on server side to decode */
    DBusAuthShutdownFunction
        server_shutdown_func; /**< Function on server side to shut down */
    DBusInitialResponseFunction
        client_initial_response_func; /**< Function on client side to handle
                                         initial response */
    DBusAuthDataFunction
        client_data_func; /**< Function on client side for DATA */
    DBusAuthEncodeFunction
        client_encode_func; /**< Function on client side for encode */
    DBusAuthDecodeFunction
        client_decode_func; /**< Function on client side for decode */
    DBusAuthShutdownFunction
        client_shutdown_func; /**< Function on client side for shutdown */
...
struct DBusAuth
{
    ...
    DBusCredentials*
        authorized_identity; /**< Credentials that are authorized */
    ...
    unsigned int needed_memory : 1; /**< We needed memory to continue since last
                                     ...
                                     */
    unsigned int
        already_got_mechanisms : 1; /**< Client already got mech list */
    unsigned int
        already_asked_for_initial_response : 1; /**< Already sent a blank
                                                   challenge to get an initial
                                                   response */
    unsigned int buffer_outstanding : 1;        /**< Buffer is "checked out" for
                                                   reading data into */

    unsigned int
        unix_fd_possible : 1; /**< This side could do unix fd passing */
    unsigned int
        unix_fd_negotiated : 1; /**< Unix fd was successfully negotiated */
};
...
typedef struct
{
    ...
    DBusList* mechs_to_try; /**< Mechanisms we got from the server that we're
                               going to try using */
...
static dbus_bool_t handle_server_state_waiting_for_begin(
    DBusAuth* auth, DBusAuthCommand command, const DBusString* args);
...
static const DBusAuthStateData client_state_need_send_auth = {"NeedSendAuth",
                                                              NULL};
...
static const DBusAuthStateData common_state_authenticated = {"Authenticated",
                                                             NULL};

static const DBusAuthStateData common_state_need_disconnect = {"NeedDisconnect",
                                                               NULL};
 ...
 */
static dbus_bool_t sha1_compute_hash(DBusAuth* auth, int cookie_id,
                                     ...
                                     DBusString* hash)
{
    ...
    if (!_dbus_keyring_get_hex_key(auth->keyring, cookie_id, &cookie))
        goto out_0;
    ...
    if (!_dbus_string_copy(server_challenge, 0, &to_hash,
                           _dbus_string_get_length(&to_hash)))
        goto out_1;

    if (!_dbus_string_append(&to_hash, ":"))
        goto out_1;

    if (!_dbus_string_copy(client_challenge, 0, &to_hash,
                           _dbus_string_get_length(&to_hash)))
        goto out_1;
    ...
    if (!_dbus_string_copy(&cookie, 0, &to_hash,
                           _dbus_string_get_length(&to_hash)))
        goto out_1;
...
static dbus_bool_t sha1_handle_first_client_response(DBusAuth* auth,
                                                     const DBusString* data)
{
     ...
     */
    if (auth->keyring && !_dbus_keyring_is_for_credentials(
                             auth->keyring, auth->desired_identity))
    {
    ...
    {
        auth->keyring = _dbus_keyring_new_for_credentials(
            auth->desired_identity, &auth->context, &error);
    ...
    if (!_dbus_string_copy(&auth->context, 0, &tmp2,
...
static dbus_bool_t sha1_handle_second_client_response(DBusAuth* auth,
                                                      const DBusString* data)
{
    ...
    if (!_dbus_string_copy_len(data, 0, i, &client_challenge, 0))
        goto out_2;

    _dbus_string_skip_blank(data, i, &i);

    if (!_dbus_string_copy_len(data, i, _dbus_string_get_length(data) - i,
                               &client_hash, 0))
        goto out_2;
    ...
    if (!sha1_compute_hash(auth, auth->cookie_id, &auth->challenge,
                           &client_challenge, &correct_hash))
        goto out_3;
...
static dbus_bool_t handle_server_data_cookie_sha1_mech(DBusAuth* auth,
                                                       const DBusString* data)
{
    if (auth->cookie_id < 0)
                                                    ...
                                                    DBusString* response)
{
    ...
    if (!_dbus_string_hex_encode(&username, 0, response,
...
static dbus_bool_t handle_client_data_cookie_sha1_mech(DBusAuth* auth,
                                                       const DBusString* data)
{
    ...
    if (!_dbus_string_copy_len(data, 0, i, &context, 0))
        goto out_1;
    ...
    if (!_dbus_string_copy_len(data, i, j - i, &cookie_id_str, 0))
        goto out_2;
    ...
    if (!_dbus_string_copy_len(data, i, j - i, &server_challenge, 0))
        goto out_3;
    ...
    {
        auth->keyring =
            _dbus_keyring_new_for_credentials(NULL, &context, &error);
    ...
    if (!sha1_compute_hash(auth, val, &server_challenge, &client_challenge,
                           &correct_hash))
        goto out_6;
...
static dbus_bool_t handle_server_data_external_mech(DBusAuth* auth,
                                                    const DBusString* data)
{
                                       ...
                                       auth->desired_identity))
    {
        ...
        if (!_dbus_credentials_add_credential(
                auth->authorized_identity, DBUS_CREDENTIAL_LINUX_SECURITY_LABEL,
                auth->credentials))
            return FALSE;
                                                 ...
                                                 DBusString* response)
{
    ...
    if (!_dbus_string_hex_encode(&plaintext, 0, response,
                                 _dbus_string_get_length(response)))
        goto failed;
...
static dbus_bool_t handle_client_data_external_mech(DBusAuth* auth,
                                                    const DBusString* data)
{
...
static dbus_bool_t handle_server_data_anonymous_mech(DBusAuth* auth,
                                                     const DBusString* data)
{
    if (_dbus_string_get_length(data) > 0)
    {
    ...
    /* We want to be anonymous (clear in case some other protocol got midway
     * through I guess) */
    _dbus_credentials_clear(auth->desired_identity);
                                                  ...
                                                  DBusString* response)
{
    ...
    if (!_dbus_string_append(&plaintext, "libdbus " DBUS_VERSION_STRING))
        goto failed;
...
static dbus_bool_t handle_client_data_anonymous_mech(DBusAuth* auth,
                                                     const DBusString* data)
{
 ...
 * Right now we have:
 *
 * - EXTERNAL checks socket credentials (or in the future, other info from the
 * OS)
...
static const DBusAuthMechanismHandler all_mechanisms[] = {
    {"EXTERNAL", handle_server_data_external_mech, NULL, NULL,
     handle_server_shutdown_external_mech,
     handle_client_initial_response_external_mech,
     handle_client_data_external_mech, NULL, NULL,
     handle_client_shutdown_external_mech},
    {"DBUS_COOKIE_SHA1", handle_server_data_cookie_sha1_mech, NULL, NULL,
     handle_server_shutdown_cookie_sha1_mech,
     handle_client_initial_response_cookie_sha1_mech,
     handle_client_data_cookie_sha1_mech, NULL, NULL,
     handle_client_shutdown_cookie_sha1_mech},
    {"ANONYMOUS", handle_server_data_anonymous_mech, NULL, NULL,
     handle_server_shutdown_anonymous_mech,
     handle_client_initial_response_anonymous_mech,
     handle_client_data_anonymous_mech, NULL, NULL,
     handle_client_shutdown_anonymous_mech},
    {NULL, NULL}};

static const DBusAuthMechanismHandler* find_mech(const DBusString* name,
                                                 char** allowed_mechs)
{
    int i;
    ...
    {
        if (_dbus_string_equal_c_str(name, all_mechanisms[i].mechanism))
                             ...
                             const DBusAuthMechanismHandler* mech)
{
    ...
    if (!_dbus_string_copy(&auth_command, 0, &auth->outgoing,
...
static dbus_bool_t send_rejected(DBusAuth* auth)
{
    ...
    if (!_dbus_string_append(&command, "REJECTED"))
        goto nomem;
    ...
    for (i = 0; all_mechanisms[i].mechanism != NULL; i++)
    {
        ...
        if (!_dbus_string_append(&command, all_mechanisms[i].mechanism))
            goto nomem;
    }
...
static dbus_bool_t send_ok(DBusAuth* auth)
{
    ...
    if (_dbus_string_append(&auth->outgoing, "OK ") &&
        _dbus_string_copy(&DBUS_AUTH_SERVER(auth)->guid, 0, &auth->outgoing,
                          _dbus_string_get_length(&auth->outgoing)) &&
...
static dbus_bool_t process_ok(DBusAuth* auth, const DBusString* args_from_ok)
{
    ...
    /* "args_from_ok" should be the GUID, whitespace already pulled off the
     * front */
    _dbus_assert(_dbus_string_get_length(
    ...
    {
        _dbus_verbose("Bad GUID from server, parsed %d bytes and had %d bytes "
                      "from server\n",
    ...
    if (!_dbus_string_copy(args_from_ok, 0,
                           &DBUS_AUTH_CLIENT(auth)->guid_from_server, 0))
    {
        _dbus_string_set_length(&DBUS_AUTH_CLIENT(auth)->guid_from_server, 0);
        return FALSE;
...
static dbus_bool_t process_data(DBusAuth* auth, const DBusString* args,
        ...
        case DBUS_AUTH_COMMAND_NEGOTIATE_UNIX_FD:
            if (auth->unix_fd_possible)
                ...
                return send_error(auth,
                                  "Unix FD passing not supported, not "
                                  "authenticated or otherwise not possible");
...
static dbus_bool_t record_mechanisms(DBusAuth* auth, const DBusString* args)
{
    ...
    while (next < len)
    {
        ...
        if (mech != NULL)
        {
            ...
            {
                _dbus_verbose("%s: Already tried mechanism %s; not adding to "
                              "list we will try\n",
                              DBUS_AUTH_NAME(auth), mech->mechanism);
            }
        }
        ...
        {
            _dbus_verbose("%s: Server offered mechanism \"%s\" that we don't "
                          "know how to use\n",
                          DBUS_AUTH_NAME(auth),
...
static DBusAuthCommand lookup_command_from_name(DBusString* command)
{
    ...
    {
        if (_dbus_string_equal_c_str(command, auth_command_names[i].name))
...
#define MAX_BUFFER (16 * _DBUS_ONE_KILOBYTE)
    ...
    do
    {
        ...
        {
            goto_state(auth, &common_state_need_disconnect);
            _dbus_verbose("%s: Disconnecting due to excessive data buffered in "
                          "auth phase\n",
                          DBUS_AUTH_NAME(auth));
            break;
        }
...
void _dbus_auth_bytes_sent(DBusAuth* auth, int bytes_sent)
{
    _dbus_verbose("%s: Sent %d bytes of: %s\n", DBUS_AUTH_NAME(auth),
                  bytes_sent, _dbus_string_get_const_data(&auth->outgoing));
 ...
 */
dbus_bool_t _dbus_auth_encode_data(DBusAuth* auth, const DBusString* plaintext,
                                   DBusString* encoded)
{
 ...
 */
dbus_bool_t _dbus_auth_decode_data(DBusAuth* auth, const DBusString* encoded,
                                   DBusString* plaintext)
{
    _dbus_assert(plaintext != encoded);
...
const char* _dbus_auth_get_guid_from_server(DBusAuth* auth)
{
    ...
    if (auth->state == &common_state_authenticated)
        return _dbus_string_get_const_data(
...
dbus_bool_t _dbus_auth_set_context(DBusAuth* auth, const DBusString* context)
{
    return _dbus_string_replace_len(
        context, 0, _dbus_string_get_length(context), &auth->context, 0,
