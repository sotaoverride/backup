...
static NORETURN void usage(int status)
{
    fprintf(
        ...
        flash_msg[DEFAULT_REQUEST], PROGRAM_NAME);
    exit(status);
}

static const char short_opts[] = "hiluV";
static const struct option long_opts[] = {
    {"help", no_argument, 0, 'h'},    {"islocked", no_argument, 0, 'i'},
    {"lock", no_argument, 0, 'l'},    {"unlock", no_argument, 0, 'u'},
...
int main(int argc, char* argv[])
{
    ...
    if (mtdLockInfo.start >= mtdInfo.size)
        errmsg_die("%#x is beyond device size %#x", mtdLockInfo.start,
                   mtdInfo.size);
