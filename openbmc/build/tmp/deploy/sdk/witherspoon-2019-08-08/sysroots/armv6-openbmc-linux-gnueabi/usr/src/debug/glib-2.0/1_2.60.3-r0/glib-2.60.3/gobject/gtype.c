...
#ifdef LOCK_DEBUG
#define G_READ_LOCK(rw_lock)                                                   \
    do                                                                         \
    {                                                                          \
        g_printerr(G_STRLOC ": readL++\n");                                    \
        g_rw_lock_reader_lock(rw_lock);                                        \
    } while (0)
#define G_READ_UNLOCK(rw_lock)                                                 \
    do                                                                         \
    {                                                                          \
        g_printerr(G_STRLOC ": readL--\n");                                    \
        g_rw_lock_reader_unlock(rw_lock);                                      \
    } while (0)
#define G_WRITE_LOCK(rw_lock)                                                  \
    do                                                                         \
    {                                                                          \
        g_printerr(G_STRLOC ": writeL++\n");                                   \
        g_rw_lock_writer_lock(rw_lock);                                        \
    } while (0)
#define G_WRITE_UNLOCK(rw_lock)                                                \
    do                                                                         \
    {                                                                          \
        g_printerr(G_STRLOC ": writeL--\n");                                   \
...
#define INVALID_RECURSION(func, arg, type_name)                                \
    ...
    {                                                                          \
        ...
        gpointer _arg = (gpointer)(arg);                                       \
        const gchar *_tname = (type_name), *_fname = (func);                   \
        if (_arg)                                                              \
    ...
    G_STMT_END
#define g_assert_type_system_initialized() g_assert(static_quark_type_flags)

#define TYPE_FUNDAMENTAL_FLAG_MASK                                             \
    (G_TYPE_FLAG_CLASSED | G_TYPE_FLAG_INSTANTIATABLE |                        \
     G_TYPE_FLAG_DERIVABLE | G_TYPE_FLAG_DEEP_DERIVABLE)
#define TYPE_FLAG_MASK (G_TYPE_FLAG_ABSTRACT | G_TYPE_FLAG_VALUE_ABSTRACT)
#define SIZEOF_FUNDAMENTAL_INFO                                                \
    ((gssize)MAX(MAX(sizeof(GTypeFundamentalInfo), sizeof(gpointer)),          \
                 sizeof(glong)))
...
static void type_data_make_W(TypeNode* node, const GTypeInfo* info,
                             const GTypeValueTable* value_table);
static inline void type_data_ref_Wm(TypeNode* node);
static inline void type_data_unref_U(TypeNode* node, gboolean uncached);
static void type_data_last_unref_Wm(TypeNode* node, gboolean uncached);
static inline gpointer type_get_qdata_L(TypeNode* node, GQuark quark);
static inline void type_set_qdata_W(TypeNode* node, GQuark quark,
                                    gpointer data);
static IFaceHolder* type_iface_peek_holder_L(TypeNode* iface,
                                             GType instance_type);
static gboolean type_iface_vtable_base_init_Wm(TypeNode* iface, TypeNode* node);
static void type_iface_vtable_iface_init_Wm(TypeNode* iface, TypeNode* node);
static gboolean type_node_is_a_L(TypeNode* node, TypeNode* iface_node);
...
#define CLASSED_NODE_IFACES_ENTRIES_LOCKED(node)                               \
    (G_ATOMIC_ARRAY_GET_LOCKED(CLASSED_NODE_IFACES_ENTRIES((node)),            \
                               IFaceEntries))
#define IFACE_NODE_N_PREREQUISITES(node) ((node)->n_prerequisites)
#define IFACE_NODE_PREREQUISITES(node) ((node)->prerequisites)
#define iface_node_get_holders_L(node)                                         \
    ((IFaceHolder*)type_get_qdata_L((node), static_quark_iface_holder))
#define iface_node_set_holders_W(node, holders)                                \
    (type_set_qdata_W((node), static_quark_iface_holder, (holders)))
#define iface_node_get_dependants_array_L(n)                                   \
    ((GType*)type_get_qdata_L((n), static_quark_dependants_array))
#define iface_node_set_dependants_array_W(n, d)                                \
    (type_set_qdata_W((n), static_quark_dependants_array, (d)))
...
#define IFACE_ENTRIES_N_ENTRIES(_entries)                                      \
    ((G_ATOMIC_ARRAY_DATA_SIZE((_entries)) - IFACE_ENTRIES_HEADER_SIZE) /      \
     sizeof(IFaceEntry))
...
static TypeNode* static_fundamental_type_nodes[(G_TYPE_FUNDAMENTAL_MAX >>
...
static TypeNode* type_node_any_new_W(TypeNode* pnode, GType ftype,
                                     const gchar* name, GTypePlugin* plugin,
                                     GTypeFundamentalFlags type_flags)
{
    ...
    node_size +=
        (sizeof(GType) *
         (1 + n_supers + 1)); /* self + ancestors + (0) for ->supers[] */
    ...
    if (!pnode)
    {
        ...
        node->is_instantiatable =
            (type_flags & G_TYPE_FLAG_INSTANTIATABLE) != 0;
    ...
    else
    {
        ...
        memcpy(node->supers + 1, pnode->supers,
               sizeof(GType) * (1 + pnode->n_supers + 1));
...
static TypeNode* type_node_fundamental_new_W(GType ftype, const gchar* name,
                                             GTypeFundamentalFlags type_flags)
{
...
static TypeNode* type_node_new_W(TypeNode* pnode, const gchar* name,
                                 GTypePlugin* plugin)

{
    ...
    return type_node_any_new_W(pnode, NODE_FUNDAMENTAL_TYPE(pnode), name,
                               plugin, 0);
}

static inline IFaceEntry* lookup_iface_entry_I(volatile IFaceEntries* entries,
                                               TypeNode* iface_node)
{
    guint8* offsets;
    guint offset_index;
    ...
    G_ATOMIC_ARRAY_DO_TRANSACTION(
        ...
        entry = NULL;
        offsets = transaction_data; offset_index = entries->offset_index;
        if (offsets != NULL &&
...
static inline IFaceEntry* type_lookup_iface_entry_L(TypeNode* node,
                                                    TypeNode* iface_node)
{
...
static inline gboolean type_lookup_iface_vtable_I(TypeNode* node,
                                                  TypeNode* iface_node,
                                                  gpointer* vtable_ptr)
{
    IFaceEntry* entry;
...
static inline gboolean type_lookup_prerequisite_L(TypeNode* iface,
...
static const gchar* type_descriptive_name_I(GType type)
{
    if (type)
    {
...
static gboolean check_plugin_U(GTypePlugin* plugin,
                               ...
                               const gchar* type_name)
{
    ...
    if (!plugin)
    {
        g_warning("plugin handle for type '%s' is NULL", type_name);
        return FALSE;
    }
    if (!G_IS_TYPE_PLUGIN(plugin))
    {
        g_warning("plugin pointer (%p) for type '%s' is invalid", plugin,
                  type_name);
        return FALSE;
    }
    if (need_complete_type_info &&
        !G_TYPE_PLUGIN_GET_CLASS(plugin)->complete_type_info)
    {
        g_warning(
            "plugin for type '%s' has no complete_type_info() implementation",
            type_name);
        return FALSE;
    }
    if (need_complete_interface_info &&
        !G_TYPE_PLUGIN_GET_CLASS(plugin)->complete_interface_info)
    {
        g_warning("plugin for type '%s' has no complete_interface_info() "
                  "implementation",
                  type_name);
        return FALSE;
    }
...
static gboolean check_type_name_I(const gchar* type_name)
{
    ...
    name_valid = (p[0] >= 'A' && p[0] <= 'Z') || (p[0] >= 'a' && p[0] <= 'z') ||
                 p[0] == '_';
...
static gboolean check_derivation_I(GType parent_type, const gchar* type_name)
{
    TypeNode* pnode;
    GTypeFundamentalInfo* finfo;
    ...
    if (!pnode)
    {
        g_warning("cannot derive type '%s' from invalid parent type '%s'",
                  type_name, type_descriptive_name_I(parent_type));
        return FALSE;
    }
...
static gboolean check_collect_format_I(const gchar* collect_format)
{
...
static gboolean check_value_table_I(const gchar* type_name,
                                    const GTypeValueTable* value_table)
{
    if (!value_table)
        return FALSE;
    else if (value_table->value_init == NULL)
    {
        if (value_table->value_free || value_table->value_copy ||
            value_table->value_peek_pointer || value_table->collect_format ||
            value_table->collect_value || value_table->lcopy_format ||
            value_table->lcopy_value)
    ...
    else /* value_table->value_init != NULL */
    {
        ...
        {
            g_warning("one of 'collect_format' and 'collect_value()' is "
                      "unspecified for type '%s'",
                      type_name);
            return FALSE;
        }
        ...
        {
            g_warning(
                "the '%s' specification for type '%s' is too long or invalid",
                "collect_format", type_name);
            return FALSE;
        }
        if ((value_table->lcopy_format || value_table->lcopy_value) &&
            (!value_table->lcopy_format || !value_table->lcopy_value))
        {
            g_warning("one of 'lcopy_format' and 'lcopy_value()' is "
                      "unspecified for type '%s'",
                      type_name);
            return FALSE;
        }
...
static gboolean check_type_info_I(TypeNode* pnode, GType ftype,
                                  const gchar* type_name, const GTypeInfo* info)
{
    GTypeFundamentalInfo* finfo =
    ...
    {
        if (pnode)
            g_warning("cannot instantiate '%s', derived from "
                      "non-instantiatable parent type '%s'",
                      type_name, NODE_NAME(pnode));
        else
            g_warning(
                "cannot instantiate '%s' as non-instantiatable fundamental",
                type_name);
        return FALSE;
    }
    ...
    {
        if (pnode)
            g_warning("cannot create class for '%s', derived from non-classed "
                      "parent type '%s'",
                      type_name, NODE_NAME(pnode));
        else
    ...
    {
        g_warning("specified interface size for type '%s' is smaller than "
                  "'GTypeInterface' size",
                  type_name);
        return FALSE;
    }
    ...
    if (finfo->type_flags & G_TYPE_FLAG_CLASSED)
    {
        ...
        {
            g_warning("specified class size for type '%s' is smaller than "
                      "'GTypeClass' size",
                      type_name);
            return FALSE;
        }
    ...
    if (finfo->type_flags & G_TYPE_FLAG_INSTANTIATABLE)
    {
        ...
        {
            g_warning("specified instance size for type '%s' is smaller than "
                      "'GTypeInstance' size",
                      type_name);
            return FALSE;
        }
...
static TypeNode* find_conforming_child_type_L(TypeNode* pnode, TypeNode* iface)
{
    ...
    for (i = 0; i < pnode->n_children && !node; i++)
        node = find_conforming_child_type_L(
            lookup_type_node_I(pnode->children[i]), iface);

    return node;
}

static gboolean check_add_interface_L(GType instance_type, GType iface_type)
{
    ...
    {
        g_warning(
            "cannot add interfaces to invalid (non-instantiatable) type '%s'",
    ...
    if (!iface || !NODE_IS_IFACE(iface))
    {
        g_warning("cannot add invalid (non-interface) type '%s' to type '%s'",
                  type_descriptive_name_I(iface_type), NODE_NAME(node));
        return FALSE;
    }
    if (node->data && node->data->class.class)
    {
        g_warning("attempting to add an interface (%s) to class (%s) after "
                  "class_init",
    ...
    {
        /* 2001/7/31:timj: erk, i guess this warning is junk as interface
         * derivation is flat */
        g_warning("cannot add sub-interface '%s' to type '%s' which does not "
                  "conform to super-interface '%s'",
    ...
    if (entry && entry->vtable == NULL &&
        !type_iface_peek_holder_L(iface, NODE_TYPE(node)))
    {
        /* ok, we do conform to this interface already, but the interface vtable
         * was not yet intialized, and we just conform to the interface because
         * it got added to one of our parents. so we allow overriding of holder
         * info here.
         */
        return TRUE;
    }
    /* check whether one of our children already conforms (or whether the
     * interface got added to this node already)
     */
    tnode = find_conforming_child_type_L(node, iface); /* tnode is_a node */
    if (tnode)
    {
        g_warning("cannot add interface type '%s' to type '%s', since type "
                  "'%s' already conforms to interface",
                  NODE_NAME(iface), NODE_NAME(node), NODE_NAME(tnode));
        return FALSE;
    }
    ...
    for (i = 0; i < IFACE_NODE_N_PREREQUISITES(iface); i++)
    {
        ...
        {
            g_warning("cannot add interface type '%s' to type '%s' which does "
                      "not conform to prerequisite '%s'",
                      NODE_NAME(iface), NODE_NAME(node), NODE_NAME(tnode));
            return FALSE;
        }
    }
...
static gboolean check_interface_info_I(TypeNode* iface, GType instance_type,
                                       const GInterfaceInfo* info)
{
    if ((info->interface_finalize || info->interface_data) &&
        !info->interface_init)
    {
        g_warning("interface type '%s' for type '%s' comes without initializer",
                  NODE_NAME(iface), type_descriptive_name_I(instance_type));
        return FALSE;
    }
                             ...
                             const GTypeValueTable* value_table)
{
    ...
    if (value_table)
    {
        /* need to setup vtable_size since we have to allocate it with data in
         * one chunk */
    ...
    if (node->is_instantiatable) /* careful, is_instantiatable is also
                                    is_classed */
    {
        TypeNode* pnode = lookup_type_node_I(NODE_PARENT_TYPE(node));
    ...
    else if (node->is_classed) /* only classed */
    {
        ...
        if (pnode)
            data->class.class_private_size =
                pnode->data->class.class_private_size;
    ...
    if (vtable_size)
    {
        ...
        /* we allocate the vtable and its strings together with the type data,
         * so children can take over their parent's vtable pointer, and we don't
    ...
    node->mutatable_check_cache =
        (node->data->common.value_table->value_init != NULL &&
...
static inline void type_data_ref_Wm(TypeNode* node)
{
    if (!node->data)
    {
        ...
        if (pnode)
        {
            ...
            if (node->data)
                INVALID_RECURSION("g_type_plugin_*", node->plugin,
                                  NODE_NAME(node));
        }
        ...
        g_type_plugin_complete_type_info(node->plugin, NODE_TYPE(node),
                                         &tmp_info, &tmp_value_table);
        G_WRITE_LOCK(&type_rw_lock);
        if (node->data)
            INVALID_RECURSION("g_type_plugin_*", node->plugin, NODE_NAME(node));

        check_type_info_I(pnode, NODE_FUNDAMENTAL_TYPE(node), NODE_NAME(node),
                          &tmp_info);
        type_data_make_W(node, &tmp_info,
                         check_value_table_I(NODE_NAME(node), &tmp_value_table)
                             ? &tmp_value_table
                             : NULL);
    }
    else
...
static inline gboolean type_data_ref_U(TypeNode* node)
{
    ...
    do
    {
        ...
        if (current < 1)
            return FALSE;
    } while (!g_atomic_int_compare_and_exchange((int*)&node->ref_count, current,
                                                current + 1));

    return TRUE;
}

static gboolean iface_node_has_available_offset_L(TypeNode* iface_node,
                                                  int offset, int for_index)
{
    guint8* offsets;
...
static int find_free_iface_offset_L(IFaceEntries* entries)
{
    IFaceEntry* entry;
    TypeNode* iface_node;
    int offset;
...
static void iface_node_set_offset_L(TypeNode* iface_node, int offset, int index)
{
    ...
    offsets = _g_atomic_array_copy(&iface_node->_prot.offsets, 0,
                                   new_size - old_size);
...
static void type_node_add_iface_entry_W(TypeNode* node, GType iface_type,
                                        IFaceEntry* parent_entry)
{
        ...
        for (i = 0; i < num_entries; i++)
        {
            ...
            if (entry->iface_type == iface_type)
            {
                ...
                else
                {
                     ...
                     * nothing todo, the entry and our children were already
                     * setup correctly
                     */
                }
                return;
    ...
    entries =
        _g_atomic_array_copy(CLASSED_NODE_IFACES_ENTRIES(node),
                             IFACE_ENTRIES_HEADER_SIZE, sizeof(IFaceEntry));
    num_entries = IFACE_ENTRIES_N_ENTRIES(entries);
    i = num_entries - 1;
    if (i == 0)
...
static void type_add_interface_Wm(TypeNode* node, TypeNode* iface,
                                  const GInterfaceInfo* info,
                                  GTypePlugin* plugin)
{
    ...
    g_assert(node->is_instantiatable && NODE_IS_IFACE(iface) &&
    ...
    for (i = 0; i < node->n_children; i++)
        type_node_add_iface_entry_W(lookup_type_node_I(node->children[i]),
                                    NODE_TYPE(iface), entry);
}

static void type_iface_add_prerequisite_W(TypeNode* iface,
                                          TypeNode* prerequisite_node)
{
    ...
    g_assert(NODE_IS_IFACE(iface) &&
             ...
             (prerequisite_node->is_instantiatable ||
    ...
    IFACE_NODE_PREREQUISITES(iface) =
        g_renew(GType, IFACE_NODE_PREREQUISITES(iface),
                IFACE_NODE_N_PREREQUISITES(iface));
    prerequisites = IFACE_NODE_PREREQUISITES(iface);
    memmove(prerequisites + i + 1, prerequisites + i,
            sizeof(prerequisites[0]) *
                (IFACE_NODE_N_PREREQUISITES(iface) - i - 1));
    ...
    for (i = 1; i <= n_dependants; i++)
        type_iface_add_prerequisite_W(lookup_type_node_I(dependants[i]),
                                      prerequisite_node);
}
                                       ...
                                       GType prerequisite_type)
{
    ...
    g_return_if_fail(G_TYPE_IS_INTERFACE(
        interface_type)); /* G_TYPE_IS_INTERFACE() is an external call: _U */
    ...
    {
        G_WRITE_UNLOCK(&type_rw_lock);
        g_warning("unable to add prerequisite '%s' to interface '%s' which is "
                  "already in use for '%s'",
                  type_descriptive_name_I(prerequisite_type),
                  type_descriptive_name_I(interface_type),
    ...
    if (prerequisite_node->is_instantiatable)
    {
        ...
        {
            TypeNode* prnode =
                lookup_type_node_I(IFACE_NODE_PREREQUISITES(iface)[i]);

            if (prnode->is_instantiatable)
            {
                G_WRITE_UNLOCK(&type_rw_lock);
                g_warning("adding prerequisite '%s' to interface '%s' "
        ...
        for (i = 0; i < prerequisite_node->n_supers + 1; i++)
            type_iface_add_prerequisite_W(
                iface, lookup_type_node_I(prerequisite_node->supers[i]));
        G_WRITE_UNLOCK(&type_rw_lock);
    }
    else if (NODE_IS_IFACE(prerequisite_node))
    {
        ...
        for (i = 0; i < IFACE_NODE_N_PREREQUISITES(prerequisite_node); i++)
            type_iface_add_prerequisite_W(iface,
                                          lookup_type_node_I(prerequisites[i]));
    ...
    {
        G_WRITE_UNLOCK(&type_rw_lock);
        g_warning("prerequisite '%s' for interface '%s' is neither "
                  "instantiatable nor interface",
                  type_descriptive_name_I(prerequisite_type),
                  type_descriptive_name_I(interface_type));
    }
}
...
static IFaceHolder* type_iface_retrieve_holder_info_Wm(TypeNode* iface,
                                                       GType instance_type,
                                                       gboolean need_info)
{
    ...
    if (iholder && !iholder->info && need_info)
    {
        ...
        if (iholder->info)
            INVALID_RECURSION("g_type_plugin_*", iface->plugin,
                              NODE_NAME(iface));
        ...
        g_type_plugin_complete_interface_info(iholder->plugin, instance_type,
                                              NODE_TYPE(iface), &tmp_info);
        G_WRITE_LOCK(&type_rw_lock);
        if (iholder->info)
...
static void type_iface_blow_holder_info_Wm(TypeNode* iface, GType instance_type)
{
...
GTypeInstance* g_type_create_instance(GType type)
{
    ...
    if (!node || !node->is_instantiatable)
    {
        g_error("cannot create new instance of invalid (non-instantiatable) "
                "type '%s'",
    ...
    if (private_size && RUNNING_ON_VALGRIND)
    {
        ...
        *(gpointer*)(allocated + private_size + ivar_size) =
            allocated + ALIGN_STRUCT(1);

        /* Tell valgrind that it should treat the object itself as such */
        VALGRIND_MALLOCLIKE_BLOCK(allocated + private_size,
                                  ivar_size + sizeof(gpointer), 0, TRUE);
        VALGRIND_MALLOCLIKE_BLOCK(allocated + ALIGN_STRUCT(1),
                                  private_size - ALIGN_STRUCT(1), 0, TRUE);
    }
    else
#endif
...
void g_type_free_instance(GTypeInstance* instance)
{
    ...
    if (!node || !node->is_instantiatable || !node->data ||
        node->data->class.class != (gpointer) class)
    {
        g_warning(
            "cannot free instance of invalid (non-instantiatable) type '%s'",
    ...
    {
        g_warning(
            "cannot free instance of abstract (non-instantiatable) type '%s'",
            NODE_NAME(node));
        return;
    }
        ...
        if (iface->data->iface.vtable_init_base || iface->data->iface.dflt_init)
        {
            ...
            if (iface->data->iface.dflt_init)
                iface->data->iface.dflt_init(
                    vtable, (gpointer)iface->data->iface.dflt_data);
            G_WRITE_LOCK(&type_rw_lock);
        }
    }
}
...
static gboolean type_iface_vtable_base_init_Wm(TypeNode* iface, TypeNode* node)
{
    ...
    /* type_iface_retrieve_holder_info_Wm() doesn't modify write lock for
     * returning NULL */
    ...
    g_assert(iface->data && entry && entry->vtable == NULL && iholder &&
             iholder->info);
...
static void type_iface_vtable_iface_init_Wm(TypeNode* iface, TypeNode* node)
{
    ...
    /* iholder->info should have been filled in by
    ...
    {
        GTypeInterfaceCheckFunc check_func =
            static_iface_check_funcs[i].check_func;
...
static gboolean type_iface_vtable_finalize_Wm(TypeNode* iface, TypeNode* node,
                                              GTypeInterface* vtable)
{
    IFaceEntry* entry = type_lookup_iface_entry_L(node, iface);
    IFaceHolder* iholder;
        ...
        iface->data->iface.vtable_finalize_base)
    {
        G_WRITE_UNLOCK(&type_rw_lock);
        if (iholder->info->interface_finalize)
            iholder->info->interface_finalize(vtable,
                                              iholder->info->interface_data);
...
static void type_class_init_Wm(TypeNode* node, GTypeClass* pclass)
{
     ...
     */
    g_assert(node->is_classed && node->data && node->data->class.class_size &&
             !node->data->class.class &&
             node->data->class.init_state == UNINITIALIZED);
    if (node->data->class.class_private_size)
        class = g_malloc0(ALIGN_STRUCT(node->data->class.class_size) +
                          node->data->class.class_private_size);
    else
    ...
    if (pclass)
    {
        ...
        memcpy(G_STRUCT_MEMBER_P(class,
                                 ALIGN_STRUCT(node->data->class.class_size)),
               G_STRUCT_MEMBER_P(pclass,
                                 ALIGN_STRUCT(pnode->data->class.class_size)),
               pnode->data->class.class_private_size);
    ...
    for (bnode = node; bnode;
        ...
        if (bnode->data->class.class_init_base)
            init_slist = g_slist_prepend(
                init_slist, (gpointer)bnode->data->class.class_init_base);
           ...
           i < IFACE_ENTRIES_N_ENTRIES(entries))
    {
        ...
        if (!type_iface_vtable_base_init_Wm(
                lookup_type_node_I(entry->iface_type), node))
        {
            guint j;
            IFaceEntries* pentries = CLASSED_NODE_IFACES_ENTRIES_LOCKED(pnode);

            /* need to get this interface from parent,
             * type_iface_vtable_base_init_Wm() doesn't modify write lock upon
             * FALSE, so entry is still valid;
             */
            g_assert(pnode != NULL);
    ...
    if (node->data->class.class_init)
        node->data->class.class_init(class,
                                     (gpointer)node->data->class.class_data);
    ...
    while ((entries = CLASSED_NODE_IFACES_ENTRIES_LOCKED(node)) != NULL)
    {
        ...
        type_iface_vtable_iface_init_Wm(lookup_type_node_I(entry->iface_type),
                                        node);

        /* As in the loop above, additional initialized entries might be
         * inserted if the write lock is released, but that's harmless because
         * the entries we need to initialize only move higher in the list.
         */
        i++;
    }
...
static void type_data_finalize_class_ifaces_Wm(TypeNode* node)
{
    ...
    g_assert(node->is_instantiatable && node->data && node->data->class.class &&
    ...
    for (i = 0; entries != NULL && i < IFACE_ENTRIES_N_ENTRIES(entries); i++)
    {
        ...
        if (entry->vtable)
        {
            if (type_iface_vtable_finalize_Wm(
                    lookup_type_node_I(entry->iface_type), node, entry->vtable))
            {
            ...
            {
                /* type_iface_vtable_finalize_Wm() doesn't modify write lock
                 * upon FALSE, iface vtable came from parent
                 */
                entry->vtable = NULL;
                entry->init_state = UNINITIALIZED;
            }
...
static void type_data_finalize_class_U(TypeNode* node, ClassData* cdata)
{
    ...
    for (bnode = lookup_type_node_I(NODE_PARENT_TYPE(node)); bnode;
         bnode = lookup_type_node_I(NODE_PARENT_TYPE(bnode)))
...
static void type_data_last_unref_Wm(TypeNode* node, gboolean uncached)
{
    ...
    if (node->is_classed && node->data && node->data->class.class &&
        static_n_class_cache_funcs && !uncached)
    {
        guint i;
        ...
        {
            GTypeClassCacheFunc cache_func =
                static_class_cache_funcs[i].cache_func;
            ...
            G_READ_LOCK(&type_rw_lock);
            if (!node->data || NODE_REFCOUNT(node) == 0)
                INVALID_RECURSION("GType class cache function ", cache_func,
                                  NODE_NAME(node));
            if (need_break)
                break;
        }
        ...
        else if (NODE_IS_IFACE(node) && tdata->iface.dflt_vtable)
        {
            ...
            if (tdata->iface.dflt_finalize || tdata->iface.vtable_finalize_base)
            {
                ...
                if (tdata->iface.dflt_finalize)
                    tdata->iface.dflt_finalize(
                        tdata->iface.dflt_vtable,
                        (gpointer)tdata->iface.dflt_data);
...
static inline void type_data_unref_U(TypeNode* node, gboolean uncached)
{
    ...
    do
    {
        ...
        if (current <= 1)
        {
            ...
            g_rec_mutex_lock(&class_init_rec_mutex); /* required locking order:
                                                        1) class_init_rec_mutex,
                                                        2) type_rw_lock */
            G_WRITE_LOCK(&type_rw_lock);
    ...
    for (i = 0; i < static_n_class_cache_funcs; i++)
            ...
            static_class_cache_funcs[i].cache_func == cache_func)
        {
            ...
            memmove(static_class_cache_funcs + i,
                    ...
                    sizeof(static_class_cache_funcs[0]) *
                        (static_n_class_cache_funcs - i));
            static_class_cache_funcs =
                g_renew(ClassCacheFunc, static_class_cache_funcs,
                        static_n_class_cache_funcs);
            found_it = TRUE;
            break;
        }
    ...
    for (i = 0; i < static_n_iface_check_funcs; i++)
            ...
            static_iface_check_funcs[i].check_func == check_func)
        {
            ...
            memmove(static_iface_check_funcs + i,
                    ...
                    sizeof(static_iface_check_funcs[0]) *
                        (static_n_iface_check_funcs - i));
            static_iface_check_funcs =
                g_renew(IFaceCheckFunc, static_iface_check_funcs,
                        static_n_iface_check_funcs);
            found_it = TRUE;
            break;
        }
 ...
 */
GType g_type_register_fundamental(GType type_id, const gchar* type_name,
                                  ...
                                  GTypeFlags flags)
{
    ...
    if (!check_type_name_I(type_name))
        return 0;
    if ((type_id & TYPE_ID_MASK) || type_id > G_TYPE_FUNDAMENTAL_MAX)
    {
        g_warning("attempt to register fundamental type '%s' with invalid type "
                  "id (%" G_GSIZE_FORMAT ")",
                  type_name, type_id);
        return 0;
    }
    ...
    {
        g_warning("cannot register instantiatable fundamental type '%s' as "
                  "non-classed",
                  type_name);
        return 0;
    }
    if (lookup_type_node_I(type_id))
    {
        g_warning("cannot register existing fundamental type '%s' (as '%s')",
                  type_descriptive_name_I(type_id), type_name);
        return 0;
    }
                             ...
                             const GTypeInfo* info, GTypeFlags flags)
{
    ...
    {
        g_warning("class finalizer specified for static type '%s'", type_name);
        return 0;
    }
 ...
 */
GType g_type_register_dynamic(GType parent_type, const gchar* type_name,
                                 ...
                                 const GInterfaceInfo* info)
{
    ...
    /* we only need to lock class_init_rec_mutex if instance_type already has
     * its class initialized, however this function is rarely enough called to
     * take the simple route and always acquire class_init_rec_mutex.
     */
    g_rec_mutex_lock(
        &class_init_rec_mutex); /* required locking order: 1)
                                   class_init_rec_mutex, 2) type_rw_lock */
    G_WRITE_LOCK(&type_rw_lock);
 ...
 */
void g_type_add_interface_dynamic(GType instance_type, GType interface_type,
                                  GTypePlugin* plugin)
{
    TypeNode* node;
    /* G_TYPE_IS_INSTANTIATABLE() is an external call: _U */
...
gpointer g_type_class_ref(GType type)
{
    ...
    {
        if (G_LIKELY(g_atomic_int_get(&node->data->class.init_state) ==
                     INITIALIZED))
...
gpointer g_type_default_interface_ref(GType g_type)
{
    ...
    {
        G_WRITE_UNLOCK(&type_rw_lock);
        g_warning("cannot retrieve default vtable for invalid or non-interface "
                  "type '%s'",
...
void g_type_default_interface_unref(gpointer g_iface)
{
    ...
    if (node && NODE_IS_IFACE(node))
        ...
        g_warning(
            "cannot unreference invalid interface default vtable for '%s'",
...
static inline gboolean
    type_node_check_conformities_UorL(TypeNode* node, TypeNode* iface_node,
                                      /*        support_inheritance */
                                      ...
                                      gboolean have_lock)
{
    ...
    support_interfaces = support_interfaces && node->is_instantiatable &&
...
static gboolean type_node_is_a_L(TypeNode* node, TypeNode* iface_node)
{
    return type_node_check_conformities_UorL(node, iface_node, TRUE, TRUE,
                                             TRUE);
}

static inline gboolean type_node_conforms_to_U(TypeNode* node,
                                               TypeNode* iface_node,
                                               gboolean support_interfaces,
                                               gboolean support_prerequisites)
{
    return type_node_check_conformities_UorL(
        node, iface_node, support_interfaces, support_prerequisites, FALSE);
}
 ...
 */
gboolean g_type_is_a(GType type, GType iface_type)
{
    ...
    node = lookup_type_node_I(type);
    iface_node = lookup_type_node_I(iface_type);
    is_a = node && iface_node &&
           type_node_conforms_to_U(node, iface_node, TRUE, TRUE);
...
static inline void type_set_qdata_W(TypeNode* node, GQuark quark, gpointer data)
{
    GData* gdata;
    QData* qdata;
    guint i;
...
static void type_add_flags_W(TypeNode* node, GTypeFlags flags)
{
    ...
    if ((flags & TYPE_FLAG_MASK) && node->is_classed && node->data &&
        node->data->class.class)
        g_warning("tagging type '%s' as abstract after class initialization",
                  NODE_NAME(node));
...
void g_type_query(GType type, GTypeQuery* query)
{
    ...
    if (node && node->is_classed && !node->plugin)
    {
        ...
        if (node->data) /* type is static or referenced */
        {
            ...
            query->instance_size = node->is_instantiatable
...
gboolean g_type_test_flags(GType type, guint flags)
{
    ...
    if (node)
    {
        ...
        {
            G_READ_LOCK(&type_rw_lock);
            tflags = (tflags & GPOINTER_TO_UINT(type_get_qdata_L(
                                   node, static_quark_type_flags))) == tflags;
            G_READ_UNLOCK(&type_rw_lock);
        }
        else
            tflags = TRUE;
                                         ...
                                         GType interface_type)
{
    ...
    g_return_val_if_fail(
        G_TYPE_IS_INTERFACE(interface_type),
        NULL); /* G_TYPE_IS_INTERFACE() is an external call: _U */
    ...
    g_warning(G_STRLOC ": attempt to look up plugin for invalid "
                       "instance/interface type pair.");
...
gboolean g_type_check_instance_is_a(GTypeInstance* type_instance,
                                    GType iface_type)
{
    TypeNode *node, *iface;
    gboolean check;
    ...
    iface = lookup_type_node_I(iface_type);
    check = node && node->is_instantiatable && iface &&
            type_node_conforms_to_U(node, iface, TRUE, FALSE);

    return check;
}

gboolean g_type_check_instance_is_fundamentally_a(GTypeInstance* type_instance,
...
gboolean g_type_check_class_is_a(GTypeClass* type_class, GType is_a_type)
{
    TypeNode *node, *iface;
    gboolean check;
    ...
    check = node && node->is_classed && iface &&
            type_node_conforms_to_U(node, iface, FALSE, FALSE);
    ...
    if (type_instance)
    {
        if (type_instance->g_class)
        {
            ...
            if (is_instantiatable)
                ...
                g_warning(
                    "invalid uninstantiatable type '%s' in cast to '%s'",
                    type_descriptive_name_I(type_instance->g_class->g_type),
                    type_descriptive_name_I(iface_type));
        }
        else
...
GTypeClass* g_type_check_class_cast(GTypeClass* type_class, GType is_a_type)
{
    if (type_class)
    {
        TypeNode *node, *iface;
...
static inline gboolean type_check_is_value_type_U(GType type)
{
...
restart_check:
    if (node)
    {
        if (node->data && NODE_REFCOUNT(node) > 0 &&
            ...
            tflags = GPOINTER_TO_UINT(
                type_get_qdata_L(node, static_quark_type_flags));
        else if (NODE_IS_IFACE(node))
        {
            guint i;
...
restart_table_peek:
    ...
    if (!has_refed_data)
        g_warning("can't peek value table for type '%s' which is not currently "
                  "referenced",
                  type_descriptive_name_I(type));
...
void _g_type_boxed_init(GType type, GBoxedCopyFunc copy_func,
                        GBoxedFreeFunc free_func)
{
    TypeNode* node = lookup_type_node_I(type);
...
void g_type_init_with_debug_flags(GTypeDebugFlags debug_flags)
{
    ...
    if (debug_flags)
        g_message("g_type_init_with_debug_flags() is no longer supported.  Use "
...
static void gobject_init(void)
{
    ...
    if (env_string != NULL)
    {
        ...
        _g_type_debug_flags = g_parse_debug_string(env_string, debug_keys,
     ...
     */
    node = type_node_fundamental_new_W(G_TYPE_NONE,
    ...
    node = type_node_fundamental_new_W(G_TYPE_INTERFACE,
                                       g_intern_static_string("GInterface"),
...
void g_type_class_add_private(gpointer g_class, gsize private_size)
{
    ...
    {
        g_warning("cannot add private field to invalid (non-instantiatable) "
                  "type '%s'",
                  type_descriptive_name_I(instance_type));
        return;
    }
    ...
    if (NODE_PARENT_TYPE(node))
    {
        ...
        if (node->data->instance.private_size !=
            pnode->data->instance.private_size)
        {
            g_warning("g_type_class_add_private() called multiple times for "
                      "the same type");
            return;
        }
    }
    ...
    private_size =
        ALIGN_STRUCT(node->data->instance.private_size + private_size);
...
gint g_type_add_instance_private(GType class_gtype, gsize private_size)
{
    ...
    {
        g_warning(
            "cannot use g_type_add_instance_private() with dynamic type '%s'",
            type_descriptive_name_I(class_gtype));
        return 0;
    }
     ...
     * hide it behind a macro. the function will return the private size,
     * instead of the offset, which will be stored inside a static variable
     * defined by the G_DEFINE_TYPE_EXTENDED macro. the G_DEFINE_TYPE_EXTENDED
     * macro will check the variable and call
     * g_type_class_add_instance_private(), which will use the data size and
     * actually register the private data, then return the computed offset of
     * the private data, which will be stored inside the static variable, so we
     * can use it to retrieve the pointer to the private data structure.
     *
                                        ...
                                        gint* private_size_or_offset)
{
    ...
    if (NODE_PARENT_TYPE(node))
    {
        ...
        if (node->data->instance.private_size !=
            pnode->data->instance.private_size)
        {
            g_warning("g_type_add_instance_private() called multiple times for "
                      "the same type");
            *private_size_or_offset = 0;
            return;
        }
    ...
    private_size = ALIGN_STRUCT(node->data->instance.private_size +
...
gint g_type_class_get_instance_private_offset(gpointer g_class)
{
    ...
    if (node->data->instance.private_size == parent_size)
        g_error("g_type_class_get_instance_private_offset() called on class %s "
                "but it has no private data",
...
void g_type_add_class_private(GType class_type, gsize private_size)
{
    ...
    if (NODE_PARENT_TYPE(node))
    {
        ...
        if (node->data->class.class_private_size !=
            pnode->data->class.class_private_size)
        {
            g_warning("g_type_add_class_private() called multiple times for "
                      "the same type");
            return;
        }
    }
...
gpointer g_type_class_get_private(GTypeClass* klass, GType private_type)
{
    ...
    if (G_UNLIKELY(!private_node ||
                   !NODE_IS_ANCESTOR(private_node, class_node)))
    {
    ...
    if (NODE_PARENT_TYPE(private_node))
    {
        ...
        if (G_UNLIKELY(private_node->data->class.class_private_size ==
                       parent_node->data->class.class_private_size))
        {
            g_warning("g_type_instance_get_class_private() requires a prior "
                      "call to g_type_add_class_private()");
            return NULL;
        }
