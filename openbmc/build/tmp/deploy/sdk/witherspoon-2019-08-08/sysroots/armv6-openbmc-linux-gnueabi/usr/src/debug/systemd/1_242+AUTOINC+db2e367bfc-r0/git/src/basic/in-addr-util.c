...
bool in4_addr_is_link_local(const struct in_addr* a)
{
    ...
    return (be32toh(a->s_addr) & UINT32_C(0xFFFF0000)) ==
           (UINT32_C(169) << 24 | UINT32_C(254) << 16);
}
...
bool in4_addr_is_non_local(const struct in_addr* a)
{
    ...
    return !in4_addr_is_null(a) && !in4_addr_is_localhost(a);
}
...
int in_addr_equal(int family, const union in_addr_union* a,
...
int in_addr_prefix_intersect(int family, const union in_addr_union* a,
                             unsigned aprefixlen, const union in_addr_union* b,
                             unsigned bprefixlen)
{
...
int in_addr_random_prefix(int family, union in_addr_union* u,
                          unsigned prefixlen_fixed_part, unsigned prefixlen)
{
    ...
    if (family == AF_INET)
    {
        ...
        c = be32toh(u->in.s_addr);
        c &= ((UINT32_C(1) << prefixlen_fixed_part) - 1)
             << (32 - prefixlen_fixed_part);

        random_bytes(&n, sizeof(n));
        n &= ((UINT32_C(1) << (prefixlen - prefixlen_fixed_part)) - 1)
             << (32 - prefixlen);
...
int in_addr_prefix_to_string(int family, const union in_addr_union* u,
                             unsigned prefixlen, char** ret)
{
    _cleanup_free_ char* x = NULL;
    char* p;
    size_t l;
...
int in_addr_ifindex_to_string(int family, const union in_addr_union* u,
                              int ifindex, char** ret)
{
    ...
    /* Much like in_addr_to_string(), but optionally appends the zone interface
     * index to the address, to properly handle IPv6 link-local addresses. */
...
int in_addr_ifindex_from_string_auto(const char* s, int* family,
                                     union in_addr_union* ret, int* ifindex)
{
    ...
    /* Similar to in_addr_from_string_auto() but also parses an optionally
     * appended IPv6 zone suffix ("scope id") if one is found. */
...
struct in_addr* in4_addr_prefixlen_to_netmask(struct in_addr* addr,
...
int in4_addr_default_prefixlen(const struct in_addr* addr,
                               unsigned char* prefixlen)
{
...
int in4_addr_default_subnet_mask(const struct in_addr* addr,
                                 struct in_addr* mask)
{
    unsigned char prefixlen;
    int r;
...
int in_addr_prefix_covers(int family, const union in_addr_union* prefix,
                          unsigned char prefixlen,
...
int in_addr_prefix_from_string(const char* p, int family,
...
static void in_addr_data_hash_func(const struct in_addr_data* a,
...
static int in_addr_data_compare_func(const struct in_addr_data* x,
                                     const struct in_addr_data* y)
{
    int r;
...
DEFINE_HASH_OPS(in_addr_data_hash_ops, struct in_addr_data,
                in_addr_data_hash_func, in_addr_data_compare_func);
