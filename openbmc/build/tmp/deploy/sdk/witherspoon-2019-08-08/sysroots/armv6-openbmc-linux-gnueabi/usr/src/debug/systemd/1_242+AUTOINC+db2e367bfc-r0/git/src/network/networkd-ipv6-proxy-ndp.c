...
static int ipv6_proxy_ndp_set(Link* link)
{
    ...
    r = sysctl_write_ip_property_boolean(AF_INET6, link->ifname, "proxy_ndp",
                                         v);
    if (r < 0)
...
static int ipv6_proxy_ndp_address_new_static(Network* network,
                                             IPv6ProxyNDPAddress** ret)
{
    _cleanup_(ipv6_proxy_ndp_address_freep)
        IPv6ProxyNDPAddress* ipv6_proxy_ndp_address = NULL;
    ...
    *ipv6_proxy_ndp_address = (IPv6ProxyNDPAddress){
    ...
    LIST_PREPEND(ipv6_proxy_ndp_addresses, network->ipv6_proxy_ndp_addresses,
...
static int set_ipv6_proxy_ndp_address_handler(sd_netlink* rtnl,
 ...
 */
int ipv6_proxy_ndp_address_configure(
    Link* link, IPv6ProxyNDPAddress* ipv6_proxy_ndp_address)
{
    ...
    r = sd_rtnl_message_new_neigh(rtnl, &req, RTM_NEWNEIGH, link->ifindex,
                                  AF_INET6);
    if (r < 0)
    ...
    r = sd_netlink_message_append_in6_addr(req, NDA_DST,
                                           &ipv6_proxy_ndp_address->in_addr);
    if (r < 0)
        return rtnl_log_create_error(r);
...
int ipv6_proxy_ndp_addresses_configure(Link* link)
{
    ...
    /* enable or disable proxy_ndp itself depending on whether
     * ipv6_proxy_ndp_addresses are set or not */
    r = ipv6_proxy_ndp_set(link);
    if (r != 0)
        return r;

    LIST_FOREACH(ipv6_proxy_ndp_addresses, ipv6_proxy_ndp_address,
                 link->network->ipv6_proxy_ndp_addresses)
    {
