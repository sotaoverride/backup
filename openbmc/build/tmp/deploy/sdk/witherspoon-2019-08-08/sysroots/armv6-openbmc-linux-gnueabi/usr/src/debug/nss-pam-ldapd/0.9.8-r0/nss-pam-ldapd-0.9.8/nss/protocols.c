...
{
    NSS_GETONE(NSLCD_ACTION_PROTOCOL_BYNAME, WRITE_STRING(fp, name),
...
nss_status_t NSS_NAME(getprotobynumber_r)(int number, struct protoent* result,
                                          ...
                                          int* errnop)
{
    NSS_GETONE(NSLCD_ACTION_PROTOCOL_BYNUMBER, WRITE_INT32(fp, number),
               read_protoent(fp, result, buffer, buflen, errnop));
}
...
static nss_status_t protocols_getprotobyname(nss_backend_t UNUSED(*be),
                                             void* args)
{
...
static nss_status_t protocols_getprotobynumber(nss_backend_t UNUSED(*be),
                                               void* args)
{
...
static nss_backend_op_t protocols_ops[] = {
    nss_ldap_destructor,      protocols_endprotoent,
    protocols_setprotoent,    protocols_getprotoent,
    protocols_getprotobyname, protocols_getprotobynumber};
