...
static int geneve_netdev_create_handler(sd_netlink* rtnl, sd_netlink_message* m,
                                        NetDev* netdev)
{
    int r;
    ...
    if (r == -EEXIST)
        log_netdev_info(netdev, "Geneve netdev exists, using existing without "
...
static int netdev_geneve_create(NetDev* netdev)
{
    ...
    if (in_addr_is_null(v->remote_family, &v->remote) == 0)
    {
        if (v->remote_family == AF_INET)
            r = sd_netlink_message_append_in_addr(m, IFLA_GENEVE_REMOTE,
                                                  &v->remote.in);
        else
            r = sd_netlink_message_append_in6_addr(m, IFLA_GENEVE_REMOTE6,
                                                   &v->remote.in6);
        if (r < 0)
            return log_netdev_error_errno(
                netdev, r,
                "Could not append IFLA_GENEVE_REMOTE/IFLA_GENEVE_REMOTE6 "
                "attribute: %m");
    }
    ...
    if (r < 0)
        return log_netdev_error_errno(
            netdev, r, "Could not append IFLA_GENEVE_TOS attribute: %m");
    ...
    r = sd_netlink_message_append_u8(m, IFLA_GENEVE_UDP_ZERO_CSUM6_TX,
                                     v->udp6zerocsumtx);
    if (r < 0)
        return log_netdev_error_errno(
            netdev, r,
    ...
    r = sd_netlink_message_append_u8(m, IFLA_GENEVE_UDP_ZERO_CSUM6_RX,
                                     v->udp6zerocsumrx);
    if (r < 0)
        return log_netdev_error_errno(
            netdev, r,
    ...
    {
        r = sd_netlink_message_append_u16(m, IFLA_GENEVE_PORT,
                                          htobe16(v->dest_port));
        if (r < 0)
            return log_netdev_error_errno(
    ...
    if (v->flow_label > 0)
    {
        r = sd_netlink_message_append_u32(m, IFLA_GENEVE_LABEL,
...
int config_parse_geneve_vni(const char* unit, const char* filename,
...
int config_parse_geneve_address(const char* unit, const char* filename,
                                unsigned line, const char* section,
                                unsigned section_line, const char* lvalue,
                                int ltype, const char* rvalue, void* data,
                                void* userdata)
...
int config_parse_geneve_flow_label(const char* unit, const char* filename,
                                   unsigned line, const char* section,
                                   unsigned section_line, const char* lvalue,
                                   int ltype, const char* rvalue, void* data,
                                   void* userdata)
{
    ...
    {
        log_syntax(unit, LOG_ERR, filename, line, r,
                   "Geneve flow label '%s' not valid. Flow label range should "
                   "be [0-1048575].",
                   rvalue);
        return 0;
    }
...
static int netdev_geneve_verify(NetDev* netdev, const char* filename)
{
    ...
    if (v->ttl == 0)
    {
        log_warning("Invalid Geneve TTL value '0' configured in '%s'. Ignoring",
                    filename);
        return -EINVAL;
    }
