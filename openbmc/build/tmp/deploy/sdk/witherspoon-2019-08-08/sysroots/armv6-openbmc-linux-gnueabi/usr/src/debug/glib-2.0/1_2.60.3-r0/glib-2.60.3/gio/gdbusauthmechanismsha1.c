...
G_DEFINE_TYPE_WITH_PRIVATE(GDBusAuthMechanismSha1, _g_dbus_auth_mechanism_sha1,
...
static void _g_dbus_auth_mechanism_sha1_finalize(GObject* object)
{
    ...
    if (G_OBJECT_CLASS(_g_dbus_auth_mechanism_sha1_parent_class)->finalize !=
        NULL)
        G_OBJECT_CLASS(_g_dbus_auth_mechanism_sha1_parent_class)
            ->finalize(object);
}
    ...
    _g_dbus_auth_mechanism_sha1_class_init(GDBusAuthMechanismSha1Class* klass)
{
    ...
    mechanism_class->server_get_reject_reason =
...
static gchar* ensure_keyring_directory(GError** error)
{
    ...
    {
        path = g_build_filename(g_get_home_dir(), ".dbus-keyrings", NULL);
    }

    if (g_file_test(path, G_FILE_TEST_EXISTS | G_FILE_TEST_IS_DIR))
    {
        if (g_getenv("G_DBUS_COOKIE_SHA1_KEYRING_DIR_IGNORE_PERMISSION") ==
            NULL)
        {
#ifdef G_OS_UNIX
            struct stat statbuf;
            ...
            if ((statbuf.st_mode & 0777) != 0700)
            {
                g_set_error(error, G_IO_ERROR, G_IO_ERROR_FAILED,
                            _("Permissions on directory “%s” are malformed. "
                              "Expected mode 0700, got 0%o"),
                            path, (guint)(statbuf.st_mode & 0777));
                g_free(path);
                path = NULL;
                goto out;
            }
...
#pragma GCC diagnostic pop
    ...
    if (g_mkdir(path, 0700) != 0)
    {
        ...
        g_set_error(error, G_IO_ERROR, g_io_error_from_errno(errsv),
                    _("Error creating directory “%s”: %s"), path,
                    g_strerror(errsv));
        g_free(path);
        path = NULL;
        goto out;
...
static gchar* keyring_lookup_entry(const gchar* cookie_context, gint cookie_id,
                                   GError** error)
{
    gchar* ret;
    gchar* keyring_dir;
    ...
    {
        g_prefix_error(error, _("Error opening keyring “%s” for reading: "),
                       path);
        goto out;
    }
    ...
    for (n = 0; lines[n] != NULL; n++)
    {
        ...
        {
            g_set_error(error, G_IO_ERROR, G_IO_ERROR_FAILED,
                        _("Line %d of the keyring at “%s” with content “%s” is "
                          "malformed"),
                        n + 1, path, line);
            g_strfreev(tokens);
            goto out;
        ...
        {
            g_set_error(error, G_IO_ERROR, G_IO_ERROR_FAILED,
                        _("First token of line %d of the keyring at “%s” with "
                          "content “%s” is malformed"),
                        n + 1, path, line);
            g_strfreev(tokens);
            goto out;
        }

        (void)g_ascii_strtoll(tokens[1], &endp,
                              10); /* do not care what the timestamp is */
        if (*endp != '\0')
        {
            g_set_error(error, G_IO_ERROR, G_IO_ERROR_FAILED,
                        _("Second token of line %d of the keyring at “%s” with "
                          "content “%s” is malformed"),
                        n + 1, path, line);
            g_strfreev(tokens);
            goto out;
...
/* function for logging important events that the system administrator should
 * take notice of */
G_GNUC_PRINTF(1, 2)
...
static gint keyring_acquire_lock(const gchar* path, GError** error)
{
     ...
     *         real locking implementations are still flaky on network
     * filesystems
     */
...
again:
    ...
    while (g_file_test(lock, G_FILE_TEST_EXISTS))
    {
        ...
        if (num_tries == 50)
        {
            /* ok, we slept 50*10ms = 0.5 seconds. Conclude that the lock file
             * must be stale (nuke the it from orbit)
             */
            if (g_unlink(lock) != 0)
            {
                errsv = errno;
                g_set_error(error, G_IO_ERROR, g_io_error_from_errno(errsv),
                            _("Error deleting stale lock file “%s”: %s"), lock,
                            g_strerror(errsv));
                goto out;
            }
...
#endif
        g_set_error(error, G_IO_ERROR, g_io_error_from_errno(errsv),
                    _("Error creating lock file “%s”: %s"), lock,
                    g_strerror(errsv));
        goto out;
    }
...
static gboolean keyring_release_lock(const gchar* path, gint lock_fd,
                                     GError** error)
{
    gchar* lock;
    gboolean ret;
    ...
    if (close(lock_fd) != 0)
    {
        int errsv = errno;
        g_set_error(error, G_IO_ERROR, g_io_error_from_errno(errsv),
                    _("Error closing (unlinked) lock file “%s”: %s"), lock,
                    g_strerror(errsv));
        goto out;
    }
    if (g_unlink(lock) != 0)
    {
        int errsv = errno;
        g_set_error(error, G_IO_ERROR, g_io_error_from_errno(errsv),
                    _("Error unlinking lock file “%s”: %s"), lock,
                    g_strerror(errsv));
        goto out;
    }
...
/* adds an entry to the keyring, taking care of locking and deleting
 * stale/future entries */
static gboolean keyring_generate_entry(const gchar* cookie_context,
                                       gint* out_id, gchar** out_cookie,
                                       GError** error)
{
    gboolean ret;
    gchar* keyring_dir;
    ...
    if (!g_file_get_contents(path, &contents, NULL, &local_error))
    {
        ...
        else
        {
            g_propagate_prefixed_error(
                error, local_error,
                _("Error opening keyring “%s” for writing: "), path);
            goto out;
        }
    }
    ...
    if (contents != NULL)
    {
        ...
        for (n = 0; lines[n] != NULL; n++)
        {
            ...
            {
                g_set_error(error, G_IO_ERROR, G_IO_ERROR_FAILED,
                            _("Line %d of the keyring at “%s” with content "
                              "“%s” is malformed"),
                            n + 1, path, line);
                g_strfreev(tokens);
                goto out;
            ...
            {
                g_set_error(error, G_IO_ERROR, G_IO_ERROR_FAILED,
                            _("First token of line %d of the keyring at “%s” "
                              "with content “%s” is malformed"),
                            n + 1, path, line);
                g_strfreev(tokens);
                goto out;
            }
            ...
            {
                g_set_error(error, G_IO_ERROR, G_IO_ERROR_FAILED,
                            _("Second token of line %d of the keyring at “%s” "
                              "with content “%s” is malformed"),
                            n + 1, path, line);
                g_strfreev(tokens);
                goto out;
            ...
            if (line_when > now)
            {
                /* Oddball case: entry is more recent than our current
                 * wall-clock time.. This is OK, it means that another server on
                 * another machine but with same $HOME wrote the entry.
                 *
                 * So discard the entry if it's more than 1 day in the future
                 * ("reasonable time in the future").
                 */
                if (line_when - now > 24 * 60 * 60)
                {
                    keep_entry = FALSE;
                    _log("Deleted SHA1 cookie from %" G_GUINT64_FORMAT
            ...
            {
                /* Discard entry if it's older than 15 minutes ("can be fairly
                 * short") */
            ...
            else
            {
                 ...
                 *
                 * (We need a bit of grace time compared to 15 minutes above..
                 * otherwise there's a race where we reuse the 14min59.9 secs
                 * old entry and a split-second later another server purges the
                 * now 15 minute old entry.)
                 */
    ...
    else
    {
        ...
        *out_cookie = _g_dbus_hexencode(raw_cookie, 32);
        ...
        g_string_append_printf(new_contents, "%d %" G_GINT64_FORMAT " %s\n",
                               *out_id, g_get_real_time() / G_USEC_PER_SEC,
                               *out_cookie);
        changed_file = TRUE;
    }

    /* and now actually write the cookie file if there are changes (this is
     * atomic) */
    if (changed_file)
    {
            ...
            if (error != NULL)
            {
                ...
                else
                {
                    g_prefix_error(error,
                                   _("(Additionally, releasing the lock for "
                                     "“%s” also failed: %s) "),
...
static gchar* generate_sha1(const gchar* server_challenge,
                            const gchar* client_challenge, const gchar* cookie)
{
    GString* str;
    gchar* sha1;
    ...
    mechanism_server_get_state(GDBusAuthMechanism* mechanism)
{
    ...
    g_return_val_if_fail(G_IS_DBUS_AUTH_MECHANISM_SHA1(mechanism),
                         G_DBUS_AUTH_MECHANISM_STATE_INVALID);
    g_return_val_if_fail(m->priv->is_server && !m->priv->is_client,
                         G_DBUS_AUTH_MECHANISM_STATE_INVALID);
                                          ...
                                          const gchar* data, gsize data_len)
{
    ...
    sha1 = generate_sha1(m->priv->server_challenge, client_challenge,
                         m->priv->cookie);
                                         ...
                                         gsize* out_data_len)
{
    ...
    g_return_val_if_fail(
        m->priv->state == G_DBUS_AUTH_MECHANISM_STATE_HAVE_DATA_TO_SEND, NULL);
    ...
    *out_data_len = 0;
    ...
    cookie_id = -1;
    error = NULL;
    if (!keyring_generate_entry(cookie_context, &cookie_id, &m->priv->cookie,
                                &error))
    {
    ...
    s = g_strdup_printf("%s %d %s", cookie_context, cookie_id,
...
static gchar* mechanism_server_get_reject_reason(GDBusAuthMechanism* mechanism)
{
    ...
    g_return_val_if_fail(G_IS_DBUS_AUTH_MECHANISM_SHA1(mechanism), NULL);
    g_return_val_if_fail(m->priv->is_server && !m->priv->is_client, NULL);
    g_return_val_if_fail(m->priv->state == G_DBUS_AUTH_MECHANISM_STATE_REJECTED,
                         NULL);
    ...
    mechanism_client_get_state(GDBusAuthMechanism* mechanism)
{
    ...
    g_return_val_if_fail(G_IS_DBUS_AUTH_MECHANISM_SHA1(mechanism),
                         G_DBUS_AUTH_MECHANISM_STATE_INVALID);
    g_return_val_if_fail(m->priv->is_client && !m->priv->is_server,
                         G_DBUS_AUTH_MECHANISM_STATE_INVALID);
