...
{
    lseek(ctrl,
          (descr->track * param.head + descr->head) * param.sect * SECTOR_SIZE,
          SEEK_SET);
}

static void format_disk(int ctrl, unsigned int track_from,
...
static void verify_disk(int ctrl, unsigned int track_from,
                        unsigned int track_to, unsigned int repair)
{
    unsigned char* data;
    struct format_descr current;
                ...
                if (read_bytes != track_size)
                {
                    ...
                    fprintf(stderr,
                            _("Problem reading track/head %u/%u,"
                              " expected %d, read %d\n"),
                            current.track, current.head, track_size,
                            read_bytes);
                    free(data);
                    exit(EXIT_FAILURE);
                }
...
int main(int argc, char** argv)
{
    ...
    while ((ch = getopt_long(argc, argv, "f:t:r:nVh", longopts, NULL)) != -1)
        switch (ch)
        {
            case 'f':
                track_from =
                    strtou32_or_err(optarg, _("invalid argument - from"));
                break;
            case 't':
            ...
            case 'r':
                repair =
                    strtou32_or_err(optarg, _("invalid argument - repair"));
                break;
            case 'n':
                verify = 0;
                break;
    ...
    printf(_("%s-sided, %d tracks, %d sec/track. Total capacity %d kB.\n"),
           (param.head == 2) ? _("Double") : _("Single"), param.track,
           param.sect, param.size >> 1);
