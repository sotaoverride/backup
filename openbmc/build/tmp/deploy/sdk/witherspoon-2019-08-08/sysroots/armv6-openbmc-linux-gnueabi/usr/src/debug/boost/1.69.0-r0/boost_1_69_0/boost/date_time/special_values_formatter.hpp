...
//! Class that provides generic formmatting ostream formatting for special
//! values
/*! This class provides for the formmating of special values to an output
 * stream. In particular, it produces strings for the values of negative and
 * positive infinity as well as not_a_date_time.
 *
 *  While not a facet, this class is used by the date and time facets for
 * formatting special value types.
 *
 */
...
class special_values_formatter
{
  public:
     ...
     *
     *  special_value_formatter svf(&special_value_names[0],
     *&special_value_names[3]);
     *@endcode
     */
                        ...
                        const boost::date_time::special_values& value) const
    {
        ...
        if (index < m_special_value_names.size())
        {
            std::copy(m_special_value_names[index].begin(),
                      m_special_value_names[index].end(), next);
        }
        return next;
    }
...
const typename special_values_formatter<CharT, OutItrT>::char_type
    special_values_formatter<CharT,
                             OutItrT>::default_special_value_names[3][17] = {
        {'n', 'o', 't', '-', 'a', '-', 'd', 'a', 't', 'e', '-', 't', 'i', 'm',
         'e'},
        {'-', 'i', 'n', 'f', 'i', 'n', 'i', 't', 'y'},
        {'+', 'i', 'n', 'f', 'i', 'n', 'i', 't', 'y'}};
