...
static errcode_t write_undo_indexes(struct undo_private_data* data, int flush)
{
    ...
    if (data->keys_in_block)
    {
        ...
        data->keyb->crc = 0;
        data->keyb->crc = ext2fs_cpu_to_le32(ext2fs_crc32c_le(
            ~0, (unsigned char*)data->keyb, data->tdb_data_size));
        dbg_printf("Writing keyblock to blk %llu\n", data->key_blk_num);
        retval = io_channel_write_blk64(data->undo_file, data->key_blk_num, 1,
                                        data->keyb);
        if (retval)
            return retval;
    ...
    hdr_crc =
        ext2fs_crc32c_le(~0, (unsigned char*)&data->hdr,
                         sizeof(data->hdr) - sizeof(data->hdr.header_crc));
    data->hdr.header_crc = ext2fs_cpu_to_le32(hdr_crc);
    retval = io_channel_write_blk64(data->undo_file, 0, -(int)sizeof(data->hdr),
                                    &data->hdr);
    if (retval)
        goto err_out;
...
static errcode_t undo_setup_tdb(struct undo_private_data* data)
{
    ...
    retval = ext2fs_alloc_generic_bmap(
        &data->fake_fs, EXT2_ET_MAGIC_BLOCK_BITMAP64, EXT2FS_BMAP64_RBTREE, 0,
        ~1ULL, ~1ULL, "undo block map", &data->written_block_map);
    if (retval)
        return retval;
    ...
    for (i = 0; i <= 2; i++)
    {
        retval = io_channel_read_blk64(data->undo_file, i, 1, data->keyb);
        if (retval)
            memset(data->keyb, 0, data->tdb_data_size);
        retval = io_channel_write_blk64(data->undo_file, i, 1, data->keyb);
        if (retval)
            return retval;
        retval = io_channel_flush(data->undo_file);
        if (retval)
...
static errcode_t undo_write_tdb(io_channel channel, unsigned long long block,
                                int count)

{
    ...
    while (block_num <= end_block)
    {
         ...
         */
        if (ext2fs_test_block_bitmap2(data->written_block_map, block_num))
        {
            /* Try the next block */
            block_num++;
            continue;
        ...
        retval =
            io_channel_read_blk64(data->real, backing_blk_num, sz, read_ptr);
        if (retval)
        {
        ...
        if (key != NULL &&
            (ext2fs_le64_to_cpu(key->fsblk) * channel->block_size +
             channel->block_size - 1 + keysz) /
                    channel->block_size ==
                backing_blk_num &&
            E2UNDO_MAX_EXTENT_BLOCKS * data->tdb_data_size > keysz + data_size)
        {
        ...
        dbg_printf("Writing block %llu to offset %llu size %d key %zu\n",
                   block_num, data->undo_blk_num, sz, data->num_keys - 1);
        retval = io_channel_write_blk64(data->undo_file, data->undo_blk_num, sz,
                                        read_ptr);
        if (retval)
        {
            free(read_ptr);
...
static errcode_t undo_io_read_error(io_channel channel ATTR((unused)),
                                    ...
                                    size_t size ATTR((unused)), int actual,
                            ...
                            io_channel channel)
{
    ...
    if (retval)
        return retval;
    retval =
        io_channel_read_blk64(undo_file, super_block, -SUPERBLOCK_SIZE, buf);
    if (retval)
        goto out;
                                      ...
                                      struct undo_private_data* data)
{
    ...
    retval = io_channel_read_blk64(data->undo_file, 0, -(int)sizeof(hdr), &hdr);
    if (retval)
        goto bad_file;

    if (memcmp(hdr.magic, E2UNDO_MAGIC, sizeof(hdr.magic)))
        goto bad_file;
    hdr_crc = ext2fs_crc32c_le(~0, (unsigned char*)&hdr,
                               sizeof(struct undo_header) - sizeof(__u32));
    ...
    if (blocksize > E2UNDO_MAX_BLOCK_SIZE ||
        blocksize < E2UNDO_MIN_BLOCK_SIZE || !blocksize || !fs_blocksize)
        goto bad_file;
    ...
    for (i = 0; i < num_keys; i += keys_per_block)
    {
        ...
        retval = io_channel_read_blk64(data->undo_file, lblk, 1, data->keyb);
        if (retval)
            goto bad_key_replay;
        ...
        crc = data->keyb->crc;
        data->keyb->crc = 0;
        key_crc = ext2fs_crc32c_le(~0, (unsigned char*)data->keyb, blocksize);
        ...
        for (j = 0, dkey = data->keyb->keys; j < max_j; j++, dkey++)
        {
            ...
            ext2fs_mark_block_bitmap_range2(data->written_block_map, undo_blk,
...
static errcode_t undo_open(const char* name, int flags, io_channel* channel)
{
    ...
    {
        retval = undo_io_backing_manager->open(name, flags, &data->real);
        if (retval)
            goto cleanup;
        ...
        if (data->tdb_file == NULL)
            goto cleanup;
        undo_fd = ext2fs_open_file(data->tdb_file, O_RDWR | O_CREAT, 0600);
        if (undo_fd < 0)
            goto cleanup;

        retval = undo_io_backing_manager->open(data->tdb_file, IO_FLAG_RW,
                                               &data->undo_file);
        if (retval)
            goto cleanup;
    }
                                 ...
                                 int size, const void* buf)
{
     ...
     */
    count =
        (size + (location % channel->block_size) + channel->block_size - 1) /
        channel->block_size;
