...
static void lldp_callback(sd_lldp* lldp, sd_lldp_event event,
                          sd_lldp_neighbor* n)
{
    assert(lldp);
...
static int lldp_make_space(sd_lldp* lldp, size_t extra)
{
    ...
    /* Remove all entries that are past their TTL, and more until at least the
     * specified number of extra entries are free. */
    ...
    for (;;)
    {
        ...
        if (hashmap_size(lldp->neighbor_by_id) >
            LESS_BY(lldp->neighbors_max, extra))
            goto remove_one;
...
static bool lldp_keep_neighbor(sd_lldp* lldp, sd_lldp_neighbor* n)
{
    ...
    /* Only add if the neighbor has a capability we are interested in. Note that
     * we also store all neighbors with no caps field set. */
...
static int lldp_add_neighbor(sd_lldp* lldp, sd_lldp_neighbor* n)
{
    ...
    if (old)
    {
        ...
        {
            /* Is this equal, then restart the TTL counter, but don't do
             * anything else. */
...
static int lldp_receive_datagram(sd_event_source* s, int fd, uint32_t revents,
                                 void* userdata)
{
    _cleanup_(sd_lldp_neighbor_unrefp) sd_lldp_neighbor* n = NULL;
...
_public_ int sd_lldp_start(sd_lldp* lldp)
{
    ...
    r = sd_event_add_io(lldp->event, &lldp->io_event_source, lldp->fd, EPOLLIN,
                        lldp_receive_datagram, lldp);
    if (r < 0)
        goto fail;

    r = sd_event_source_set_priority(lldp->io_event_source,
                                     lldp->event_priority);
    if (r < 0)
        goto fail;
...
_public_ int sd_lldp_attach_event(sd_lldp* lldp, sd_event* event,
                                  int64_t priority)
{
    int r;
...
_public_ int sd_lldp_set_callback(sd_lldp* lldp, sd_lldp_callback_t cb,
                                  void* userdata)
{
    assert_return(lldp, -EINVAL);
...
_public_ int sd_lldp_new(sd_lldp** ret)
{
    ...
    *lldp = (sd_lldp){
    ...
    r = prioq_ensure_allocated(&lldp->neighbor_by_expiry,
...
static int neighbor_compare_func(sd_lldp_neighbor* const* a,
                                 sd_lldp_neighbor* const* b)
{
...
static int lldp_start_timer(sd_lldp* lldp, sd_lldp_neighbor* neighbor)
{
    ...
    return event_reset_time(lldp->event, &lldp->timer_event_source,
                            clock_boottime_or_monotonic(), n->until, 0,
                            on_timer_event, lldp, lldp->event_priority,
                            "lldp-timer", true);
}
...
_public_ int sd_lldp_get_neighbors(sd_lldp* lldp, sd_lldp_neighbor*** ret)
{
    ...
    if (hashmap_isempty(lldp->neighbor_by_id))
    { /* Special shortcut */
        *ret = NULL;
        return 0;
    }
...
_public_ int sd_lldp_set_filter_address(sd_lldp* lldp,
                                        const struct ether_addr* addr)
{
    assert_return(lldp, -EINVAL);

    /* In order to deal nicely with bridges that send back our own packets,
     * allow one address to be filtered, so that our own can be filtered out
     * here. */
