 ...
 *
 * The implementation of SHA-1 (see
 * http://www.itl.nist.gov/fipspubs/fip180-1.htm). This SHA implementation was
...
static void sha_append(DBusSHAContext* context, const unsigned char* buffer,
                       unsigned int count)
{
    dbus_uint32_t tmp;
...
static dbus_bool_t check_sha_binary(const unsigned char* input, int input_len,
                                    const char* expected)
{
    ...
    {
        _dbus_warn("Expected hash %s got %s for SHA-1 sum", expected,
...
static dbus_bool_t decode_compact_string(const DBusString* line,
                                         DBusString* decoded)
{
    int n_bits;
    ...
    while (next < _dbus_string_get_length(line))
    {
        ...
        while (n_bits < total_bits)
        {
            ...
            {
                if (!_dbus_string_set_length(decoded,
                                             byte_containing_next_bit + 1))
    ...
    {
        fprintf(stderr,
                "Expected length %d bytes %d bits for compact string, got %d "
                "bytes\n",
...
static dbus_bool_t get_next_expected_result(DBusString* results,
                                            DBusString* result)
{
    DBusString line;
    dbus_bool_t retval;
    ...
    while (_dbus_string_pop_line(&tests, &line))
    {
        ...
        else if (_dbus_string_starts_with_c_str(&line, "H>"))
        {
            ...
            if (_dbus_string_find(&line, 0, "Type 3", NULL))
            {
                ...
                printf(" (ending tests due to Type 3 tests seen - this is "
                       "normal)\n");
                break;
            }
        }
        ...
        else
        {
            ...
            if (!_dbus_string_equal(&result, &expected))
            {
                fprintf(stderr, " for line %d got hash %s expected %s\n",
                        line_no, _dbus_string_get_const_data(&result),
...
#define CHECK(input, expected)                                                 \
    if (!check_sha_str(input, expected))                                       \
    return FALSE
    ...
    CHECK("12345678901234567890123456789012345678901234567890123456789012345678"
          "901234567890",
