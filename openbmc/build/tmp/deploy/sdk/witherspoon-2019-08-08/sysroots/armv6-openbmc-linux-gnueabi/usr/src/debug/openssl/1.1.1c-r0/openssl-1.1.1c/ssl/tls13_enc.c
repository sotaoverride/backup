                      ...
                      unsigned char* out, size_t outlen, int fatal)
{
    ...
    if (!WPACKET_init_static_len(&pkt, hkdflabel, sizeof(hkdflabel), 0) ||
        ...
        !WPACKET_memcpy(&pkt, label, labellen) || !WPACKET_close(&pkt) ||
                    ...
                    unsigned char* iv, size_t ivlen)
{
    ...
    return tls13_hkdf_expand(s, md, secret, ivlabel, sizeof(ivlabel) - 1, NULL,
                             0, iv, ivlen, 1);
}

int tls13_derive_finishedkey(SSL* s, const EVP_MD* md,
                             const unsigned char* secret, unsigned char* fin,
                             size_t finlen)
{
                              ...
                              unsigned char* out)
{
    ...
    else
    {
        ...
        if (!tls13_derive_finishedkey(s, ssl_handshake_md(s),
                                      s->client_app_traffic_secret, finsecret,
                                      hashlen))
            goto err;
...
static int derive_secret_key_and_iv(SSL* s, int sending, const EVP_MD* md,
                                    ...
                                    const unsigned char* label, size_t labellen,
                                    unsigned char* secret, unsigned char* iv,
...
int tls13_change_cipher_state(SSL* s, int which)
{
        ...
        ((which & SSL3_CC_SERVER) && (which & SSL3_CC_READ)))
    {
        if (which & SSL3_CC_EARLY)
        {
            ...
            if (!tls13_hkdf_expand(
                    s, md, insecret, early_exporter_master_secret,
                    sizeof(early_exporter_master_secret) - 1, hashval, hashlen,
                    s->early_exporter_master_secret, hashlen, 1))
            {
                SSLfatal(s, SSL_AD_INTERNAL_ERROR,
                         SSL_F_TLS13_CHANGE_CIPHER_STATE, ERR_R_INTERNAL_ERROR);
                goto err;
        ...
        else if (which & SSL3_CC_HANDSHAKE)
        {
            ...
            /*
             * The handshake hash used for the server read/client write
             * handshake traffic secret is the same as the hash for the server
    ...
    if (label == client_application_traffic)
    {
         ...
         */
        if (!tls13_hkdf_expand(
                s, ssl_handshake_md(s), insecret, resumption_master_secret,
                sizeof(resumption_master_secret) - 1, hashval, hashlen,
    ...
    if (label == server_application_traffic)
    {
        ...
        if (!tls13_hkdf_expand(s, ssl_handshake_md(s), insecret,
                               ...
                               sizeof(exporter_master_secret) - 1, hash,
                               hashlen, s->exporter_master_secret, hashlen, 1))
        {
            /* SSLfatal() already called */
            goto err;
        }
    ...
    if (finsecret != NULL &&
        !tls13_derive_finishedkey(s, ssl_handshake_md(s), secret, finsecret,
                                  finsecretlen))
    {
        /* SSLfatal() already called */
        goto err;
                                 ...
                                 size_t contextlen, int use_context)
{
    ...
    if (EVP_DigestInit_ex(ctx, md, NULL) <= 0 ||
        ...
        !tls13_hkdf_expand(s, md, s->exporter_master_secret,
                           (const unsigned char*)label, llen, data, datalen,
                           exportsecret, hashsize, 0) ||
        !tls13_hkdf_expand(s, md, exportsecret, exporterlabel,
                           sizeof(exporterlabel) - 1, hash, hashsize, out, olen,
                           0))
        goto err;
