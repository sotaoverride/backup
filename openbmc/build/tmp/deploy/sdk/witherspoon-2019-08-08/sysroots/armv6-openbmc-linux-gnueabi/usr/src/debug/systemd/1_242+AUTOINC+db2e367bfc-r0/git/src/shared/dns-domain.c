        ...
        if (*n == '\\')
        {
            ...
            else if (n[0] >= '0' && n[0] <= '9')
            {
                ...
                k = ((unsigned)(n[0] - '0') * 100) +
                    ((unsigned)(n[1] - '0') * 10) + ((unsigned)(n[2] - '0'));
...
/* @label_terminal: terminal character of a label, updated to point to the
 * terminal character of the previous label (always skipping one dot) or to NULL
 * if there are no more labels. */
int dns_label_unescape_suffix(const char* name, const char** label_terminal,
                              char* dest, size_t sz)
{
    const char* terminal;
    int r;
    ...
    /* Point name to the last label, and terminal to the preceding terminal
     * symbol (or make it a NULL pointer) */
    for (;;)
    {
        if (terminal < name)
        {
...
#if HAVE_LIBIDN
int dns_label_apply_idna(const char* encoded, size_t encoded_size,
                         char* decoded, size_t decoded_max)
{
...
int dns_label_undo_idna(const char* encoded, size_t encoded_size, char* decoded,
                        size_t decoded_max)
{
...
int dns_name_concat(const char* a, const char* b, DNSLabelFlags flags,
                    char** _ret)
{
    ...
    for (;;)
    {
        ...
        if (_ret)
        {
            ...
            r = dns_label_escape(label, r, ret + n + !first,
...
finish:
    ...
    if (_ret)
    {
        if (n == 0)
        {
            /* Nothing appended? If so, generate at least a single dot, to
             * indicate the DNS root domain */
...
void dns_name_hash_func(const char* p, struct siphash* state)
{
    ...
    for (;;)
    {
        ...
        siphash24_compress_byte(0, state); /* make sure foobar and foo.bar
...
DEFINE_HASH_OPS(dns_name_hash_ops, char, dns_name_hash_func,
...
int dns_name_change_suffix(const char* name, const char* old_suffix,
...
int dns_name_reverse(int family, const union in_addr_union* a, char** ret)
{
    ...
    if (family == AF_INET)
        ...
        r = asprintf(
            ...
            "%c.%c.%c.%c.%c.%c.%c.%c.%c.%c.ip6.arpa",
            hexchar(p[15] & 0xF), hexchar(p[15] >> 4), hexchar(p[14] & 0xF),
            hexchar(p[14] >> 4), hexchar(p[13] & 0xF), hexchar(p[13] >> 4),
            hexchar(p[12] & 0xF), hexchar(p[12] >> 4), hexchar(p[11] & 0xF),
            hexchar(p[11] >> 4), hexchar(p[10] & 0xF), hexchar(p[10] >> 4),
            hexchar(p[9] & 0xF), hexchar(p[9] >> 4), hexchar(p[8] & 0xF),
            hexchar(p[8] >> 4), hexchar(p[7] & 0xF), hexchar(p[7] >> 4),
            hexchar(p[6] & 0xF), hexchar(p[6] >> 4), hexchar(p[5] & 0xF),
            hexchar(p[5] >> 4), hexchar(p[4] & 0xF), hexchar(p[4] >> 4),
            hexchar(p[3] & 0xF), hexchar(p[3] >> 4), hexchar(p[2] & 0xF),
            hexchar(p[2] >> 4), hexchar(p[1] & 0xF), hexchar(p[1] >> 4),
            hexchar(p[0] & 0xF), hexchar(p[0] >> 4));
    else
        return -EAFNOSUPPORT;
    if (r < 0)
        return -ENOMEM;
...
int dns_name_address(const char* p, int* family, union in_addr_union* address)
{
    ...
    if (r > 0)
    {
        ...
        *family = AF_INET;
        address->in.s_addr =
            htobe32(((uint32_t)a[3] << 24) | ((uint32_t)a[2] << 16) |
    ...
    if (r > 0)
    {
        ...
        for (i = 0; i < ELEMENTSOF(a.s6_addr); i++)
        {
            ...
            a.s6_addr[ELEMENTSOF(a.s6_addr) - i - 1] =
                (uint8_t)y << 4 | (uint8_t)x;
        }
...
bool dnssd_srv_type_is_valid(const char* name)
{
    return dns_srv_type_is_valid(name) &&
           ((dns_name_endswith(name, "_tcp") > 0) ||
            (dns_name_endswith(name, "_udp") >
             0)); /* Specific to DNS-SD. RFC 6763, Section 7 */
}
...
int dns_service_join(const char* name, const char* type, const char* domain,
                     char** ret)
{
    char escaped[DNS_LABEL_ESCAPED_MAX];
...
int dns_service_split(const char* joined, char** _name, char** _type,
                      char** _domain)
{
...
int dns_name_apply_idna(const char* name, char** ret)
{
    ...
    if (r == IDN2_OK)
    {
        if (!startswith(name, "xn--"))
        {
            ...
            if (r != IDN2_OK)
            {
                log_debug("idn2_to_unicode_8z8z(\"%s\") failed: %d/%s", t, r,
                          idn2_strerror(r));
                return 0;
            }
            ...
            if (!streq_ptr(name, s))
            {
                log_debug("idn2 roundtrip failed: \"%s\" → \"%s\" → \"%s\", "
                          "ignoring.",
                          name, t, s);
                return 0;
            }
    ...
    log_debug("idn2_lookup_u8(\"%s\") failed: %d/%s", name, r,
              idn2_strerror(r));
    if (r == IDN2_2HYPHEN)
...
{
    /* Returns > 0 if the specified name is either a valid IP address formatted
     * as string or a valid DNS name */
