...
// This is a C++11 conforming version, place this first and use it wherever
// possible:
...
template <typename From, typename To,
          bool b = or_helper<boost::is_void<From>, boost::is_function<To>,
...
struct is_convertible_basic_impl
{
    ...
    static bool const value = sizeof(boost::detail::checker<To>::_m_check(
                                  static_cast<rvalue_type>(_m_from), 0)) ==
                              sizeof(::boost::type_traits::yes_type);
#else
...
// Unfortunately it doesn't quite pass all the tests for most compilers
// (sigh...) Enable this for your compiler if is_convertible_test.cpp will
// compile it...
//
...
struct is_convertible_basic_impl
{
    static ::boost::type_traits::no_type BOOST_TT_DECL
        _m_check(any_conversion...);
    ...
    BOOST_STATIC_CONSTANT(
        bool, value = sizeof(_m_check(static_cast<rvalue_type>(_m_from), 0)) ==
                      sizeof(::boost::type_traits::yes_type));
#else
    BOOST_STATIC_CONSTANT(bool, value = sizeof(_m_check(_m_from, 0)) ==
                                        sizeof(::boost::type_traits::yes_type));
#endif
};
...
struct is_convertible_basic_impl
{
    ...
    enum
    {
        value = sizeof(_m_check(static_cast<rvalue_type>(_m_from), 0, 0)) ==
                sizeof(::boost::type_traits::yes_type)
    };
#else
    enum
    {
        value = sizeof(_m_check(_m_from, 0, 0)) ==
                sizeof(::boost::type_traits::yes_type)
    };
#endif
};
...
#ifndef BOOST_NO_CXX11_RVALUE_REFERENCES
    BOOST_STATIC_CONSTANT(
        bool, value = sizeof(_m_check(static_cast<rvalue_type>(_m_from))) ==
                      sizeof(::boost::type_traits::yes_type));
#else
    BOOST_STATIC_CONSTANT(bool, value = sizeof(_m_check(_m_from)) ==
                                        sizeof(::boost::type_traits::yes_type));
#endif
};
...
struct is_convertible_basic_impl
    : is_convertible_basic_impl_aux<
          From, To,
          ::boost::is_function<
              typename ::boost::remove_reference<From>::type>::value>
{
};
...
struct is_convertible_basic_impl
{
    static ::boost::type_traits::no_type BOOST_TT_DECL _m_check(...);
    static ::boost::type_traits::yes_type BOOST_TT_DECL _m_check(To);
    typedef typename is_convertible_basic_impl_add_lvalue_reference<From>::type
        lvalue_type;
    static lvalue_type _m_from;
#ifdef BOOST_MSVC
#pragma warning(push)
    ...
    enum
    {
        value =
            (::boost::detail::is_convertible_basic_impl<From, To>::value &&
             !::boost::is_array<To>::value && !::boost::is_function<To>::value)
    };
};
...
struct is_convertible_impl
{
    BOOST_STATIC_CONSTANT(
        bool,
        value = (::boost::detail::is_convertible_basic_impl<From, To>::value &&
                 !::boost::is_array<To>::value &&
...
struct is_convertible_impl_dispatch_base
{
    ...
    typedef is_convertible_impl_select<
        ::boost::is_arithmetic<From>::value, ::boost::is_arithmetic<To>::value,
#if !defined(BOOST_NO_IS_ABSTRACT) && !defined(BOOST_TT_CXX11_IS_CONVERTIBLE)
        // We need to filter out abstract types, only if we don't have a
        // strictly conforming C++11 version:
...
struct is_convertible
    : public integral_constant<
          bool, ::boost::detail::is_convertible_impl_dispatch<From, To>::value>
{
    BOOST_STATIC_ASSERT_MSG(
        boost::is_complete<To>::value || boost::is_void<To>::value ||
            boost::is_array<To>::value,
        "Destination argument type to is_convertible must be a complete type");
    BOOST_STATIC_ASSERT_MSG(
        boost::is_complete<From>::value || boost::is_void<From>::value ||
            boost::is_array<From>::value,
...
template <class From, class To>
struct is_convertible
    : public integral_constant<bool, BOOST_IS_CONVERTIBLE(From, To)>
{
#if BOOST_WORKAROUND(BOOST_MSVC, <= 1900)
    BOOST_STATIC_ASSERT_MSG(
        boost::is_complete<From>::value || boost::is_void<From>::value ||
            boost::is_array<From>::value || boost::is_reference<From>::value,
        "From argument type to is_convertible must be a complete type");
#endif
