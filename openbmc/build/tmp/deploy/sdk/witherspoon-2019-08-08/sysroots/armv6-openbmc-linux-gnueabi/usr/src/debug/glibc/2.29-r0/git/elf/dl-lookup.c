...
static const ElfW(Sym) *
    check_match(const char* const undef_name, const ElfW(Sym) * const ref,
                const struct r_found_version* const version, const int flags,
                const int type_class, const ElfW(Sym) * const sym,
                const Elf_Symndx symidx, const char* const strtab,
                ...
                int* const num_versions)
{
    ...
    if (__glibc_unlikely((sym->st_value == 0 /* No value.  */
                          && sym->st_shndx != SHN_ABS && stt != STT_TLS) ||
...
static void do_lookup_unique(const char* undef_name, uint_fast32_t new_hash,
                             const struct link_map* map, struct sym_val* result,
                             int type_class, const ElfW(Sym) * sym,
                             const char* strtab, const ElfW(Sym) * ref,
                             const struct link_map* undef_map)
{
    ...
    struct unique_sym_table* tab = &GL(dl_ns)[map->l_ns]._ns_unique_sym_table;
...
#define INITIAL_NUNIQUE_SYM_TABLE 31
    ...
    {
        enter_unique_sym(entries, size, new_hash, strtab + sym->st_name, sym,
                         map);
...
static int __attribute_noinline__ do_lookup_x(
    const char* undef_name, uint_fast32_t new_hash, unsigned long int* old_hash,
    const ElfW(Sym) * ref, struct sym_val* result, struct r_scope_elem* scope,
    size_t i, const struct r_found_version* const version, int flags,
    struct link_map* skip, int type_class, struct link_map* undef_map)
{
    ...
    do
    {
        ...
        if (__glibc_likely(bitmask != NULL))
        {
            ...
            unsigned int hashbit2 =
                ((new_hash >> map->l_gnu_shift) & (__ELF_NATIVE_CLASS - 1));

            if (__glibc_unlikely((bitmask_word >> hashbit1) &
                                 (bitmask_word >> hashbit2) & 1))
            {
                Elf32_Word bucket =
                    map->l_gnu_buckets[new_hash % map->l_nbuckets];
                if (bucket != 0)
                {
        ...
        else
        {
            ...
            for (symidx = map->l_buckets[*old_hash % map->l_nbuckets];
                 symidx != STN_UNDEF; symidx = map->l_chain[symidx])
            {
                sym = check_match(undef_name, ref, version, flags, type_class,
                                  &symtab[symidx], symidx, strtab, map,
                                  &versioned_sym, &num_versions);
                if (sym != NULL)
                    goto found_it;
            }
       ...
       no possible ambiguities.  */
        ...
        found_it:
            ...
            if (ELF_RTYPE_CLASS_EXTERN_PROTECTED_DATA && undef_map == NULL &&
                ...
                type_class == ELF_RTYPE_CLASS_EXTERN_PROTECTED_DATA)
            {
                    ...
                    map->l_info[DT_RELASZ]->d_un.d_val != 0)
                {
                    ...
                    for (i = 0; i < rela_count; i++, rela++)
                        if (elf_machine_type_class(ELFW(R_TYPE)(
                                rela->r_info)) == ELF_RTYPE_CLASS_COPY)
                        {
            ...
            switch (ELFW(ST_BIND)(sym->st_info))
            {
                ...
                case STB_GNU_UNIQUE:;
                    do_lookup_unique(undef_name, new_hash, map, result,
                                     type_class, sym, strtab, ref, undef_map);
                    return 1;
    ...
    skip:
       ...
       and we have not found a weak entry, it is a bug.  */
        if (symidx == STN_UNDEF && version != NULL &&
...
static int add_dependency(struct link_map* undef_map, struct link_map* map,
                          int flags)
{
...
\nfile=%s [%lu];  needed by %s [%lu] (relocation dependency)\n\n",
                             ...
                             DSO_FILENAME(undef_map->l_name), undef_map->l_ns);
    }
    else
...
static void _dl_debug_bindings(const char* undef_name,
                             ...
                             struct link_map* skip_map)
{
    ...
    assert(version == NULL ||
           (flags & ~(DL_LOOKUP_ADD_DEPENDENCY | DL_LOOKUP_GSCOPE_LOCK)) == 0);
    ...
    if (__glibc_unlikely(current_value.s == NULL))
    {
            ...
            !(GLRO(dl_debug_mask) & DL_DEBUG_UNUSED))
        {
            ...
            struct dl_exception exception;
            /* XXX We cannot translate the message.  */
            _dl_exception_create_format(&exception,
                                        DSO_FILENAME(reference_name),
                                        "undefined symbol: %s%s%s", undef_name,
       ...
       return for the protected symbol.  */
        ...
        else
        {
            ...
            for (scope = symbol_scope; *scope != NULL; i = 0, ++scope)
                if (do_lookup_x(
                        undef_name, new_hash, &old_hash, *ref, &protected_value,
    ...
    if (__glibc_unlikely(current_value.m->l_type == lt_loaded)
       ...
       was just removed.  Try finding another definition.  */
        return _dl_lookup_symbol_x(
            undef_name, undef_map, ref,
            (flags & DL_LOOKUP_GSCOPE_LOCK) ? undef_map->l_scope : symbol_scope,
    ...
    if (__glibc_unlikely(GLRO(dl_debug_mask) &
        ...
        _dl_debug_bindings(undef_name, undef_map, ref, &current_value, version,
                           type_class, protected);
                               ...
                               int type_class, int protected)
{
    ...
    if (GLRO(dl_debug_mask) & DL_DEBUG_BINDINGS)
    {
        _dl_debug_printf("binding file %s [%lu] to %s [%lu]: %s symbol `%s'",
                         DSO_FILENAME(reference_name), undef_map->l_ns,
                         DSO_FILENAME(value->m->l_name), value->m->l_ns,
...
#error ELF_RTYPE_CLASS_COPY must be 0 or 2!
               ...
               library, it would be the one that is looked up.  */
                ...
                for (n++; n < scope->r_nlist; n++)
                    if (scope->r_list[n]->l_info[DT_SYMBOLIC] != NULL &&
                        do_lookup_x(
                            undef_name, new_hash, &old_hash, *ref, &val2,
                            &scope->r_list[n]->l_symbolic_searchlist, 0,
                            version, 0, NULL, type_class, undef_map) > 0)
                    {
                        conflict = 1;
                        val = val2;
                        break;
        ...
        if (value->s)
        {
            ...
            if (__glibc_unlikely(ELFW(ST_TYPE)(value->s->st_info) == STT_TLS))
        ...
        if (conflict || GLRO(dl_trace_prelink_map) == undef_map ||
            GLRO(dl_trace_prelink_map) == NULL || type_class >= 4)
        {
            _dl_printf(
                "%s 0x%0*Zx 0x%0*Zx -> 0x%0*Zx 0x%0*Zx ",
                conflict ? "conflict" : "lookup", (int)sizeof(ElfW(Addr)) * 2,
                (size_t)undef_map->l_map_start, (int)sizeof(ElfW(Addr)) * 2,
            ...
            if (conflict)
                _dl_printf("x 0x%0*Zx 0x%0*Zx ", (int)sizeof(ElfW(Addr)) * 2,
