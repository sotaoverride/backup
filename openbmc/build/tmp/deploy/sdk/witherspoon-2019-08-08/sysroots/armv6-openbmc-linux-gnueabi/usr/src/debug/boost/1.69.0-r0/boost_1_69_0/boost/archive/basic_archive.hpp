...
BOOST_ARCHIVE_DECL library_version_type BOOST_ARCHIVE_VERSION();
...
class class_id_type
{
  ...
  public:
    ...
    {
        //       BOOST_ASSERT(t_ <=
        //       boost::integer_traits<base_type>::const_max);
    }
...
class object_id_type
{
  ...
  public:
    ...
    explicit object_id_type(const std::size_t& t_) :
        t(static_cast<base_type>(t_))
    {
...
{
    bool t;
    explicit tracking_type(const bool t_ = false) : t(t_){};
    tracking_type(const tracking_type& t_) : t(t_.t)
    {
    }
    operator bool() const
    {
...
struct class_name_type : private boost::noncopyable
{
    ...
    explicit class_name_type(const char* key_) : t(const_cast<char*>(key_))
    {
    }
    explicit class_name_type(char* key_) : t(key_)
    {
    }
...
BOOST_ARCHIVE_DECL const char* BOOST_ARCHIVE_SIGNATURE();
...
BOOST_CLASS_IMPLEMENTATION(boost::archive::class_id_reference_type,
                           primitive_type)
BOOST_CLASS_IMPLEMENTATION(boost::archive::class_id_optional_type,
                           primitive_type)
...
BOOST_CLASS_IMPLEMENTATION(boost::archive::object_reference_type,
                           primitive_type)
