...
#define BLKID_VERSION "2.33.2"
#define BLKID_DATE "09-Apr-2019"
...
extern int blkid_dev_set_search(blkid_dev_iterate iter, const char* search_type,
...
extern char* blkid_devno_to_devname(dev_t devno)
    __ul_attribute__((warn_unused_result));
extern int blkid_devno_to_wholedisk(dev_t dev, char* diskname, size_t len,
                                    dev_t* diskdevno)
    __ul_attribute__((warn_unused_result));
...
extern int blkid_tag_next(blkid_tag_iterate iterate, const char** type,
                          const char** value);
extern void blkid_tag_iterate_end(blkid_tag_iterate iterate);
extern int blkid_dev_has_tag(blkid_dev dev, const char* type,
                             const char* value);

extern blkid_dev blkid_find_dev_with_tag(blkid_cache cache, const char* type,
                                         const char* value);
...
extern int blkid_probe_set_device(blkid_probe pr, int fd, blkid_loff_t off,
                                  blkid_loff_t size)
    __ul_attribute__((nonnull));
...
extern int blkid_superblocks_get_name(size_t idx, const char** name,
                                      int* usage);
...
extern int blkid_probe_filter_superblocks_type(blkid_probe pr, int flag,
                                               char* names[])
    __ul_attribute__((nonnull));
...
extern int blkid_probe_filter_superblocks_usage(blkid_probe pr, int flag,
                                                int usage)
    __ul_attribute__((nonnull));
...
extern int blkid_probe_invert_partitions_filter(blkid_probe pr)
    __ul_attribute__((nonnull));
extern int blkid_probe_filter_partitions_type(blkid_probe pr, int flag,
                                              char* names[])
    __ul_attribute__((nonnull));
