...
/* dbus-sysdeps-unix.c Wrappers around UNIX system/libc features (internal to
...
{
    static int const relevant_flag[] = {
        DBUS_FORCE_STDIN_NULL, DBUS_FORCE_STDOUT_NULL, DBUS_FORCE_STDERR_NULL};
...
static dbus_bool_t _dbus_open_socket(int* fd_p, int domain, int type,
                                     int protocol, DBusError* error)
{
#ifdef SOCK_CLOEXEC
    ...
    *fd_p = socket(domain, type | SOCK_CLOEXEC, protocol);
    ...
    else
    {
        dbus_set_error(error, _dbus_error_from_errno(errno),
                       "Failed to open socket: %s", _dbus_strerror(errno));
        return FALSE;
    }
}
 ...
 */
int _dbus_write_socket(DBusSocket fd, const DBusString* buffer, int start,
                       int len)
{
 ...
 * @param n_fds on input space in fds array, on output how many fds actually got
 * read
 * @returns number of bytes appended to string
 */
int _dbus_read_socket_with_unix_fds(DBusSocket fd, DBusString* buffer,
                                    int count, int* fds, unsigned int* n_fds)
{
...
again:
    ...
    if (bytes_read < 0)
    {
        ...
        {
            /* put length back (note that this doesn't actually realloc
             * anything) */
...
int _dbus_write_socket_with_unix_fds(DBusSocket fd, const DBusString* buffer,
                                     int start, int len, const int* fds,
                                     int n_fds)
{
...
#else
    return _dbus_write_socket_with_unix_fds_two(fd, buffer, start, len, NULL, 0,
                                                0, fds, n_fds);
#endif
}

int _dbus_write_socket_with_unix_fds_two(DBusSocket fd,
                                         const DBusString* buffer1, int start1,
                                         int len1, const DBusString* buffer2,
                                         int start2, int len2, const int* fds,
                                         int n_fds)
{
    ...
    return _dbus_write_socket_two(fd, buffer1, start1, len1, buffer2, start2,
                                  len2);
#else
 ...
 */
int _dbus_write_socket_two(DBusSocket fd, const DBusString* buffer1, int start1,
                           int len1, const DBusString* buffer2, int start2,
                           int len2)
{
#if HAVE_DECL_MSG_NOSIGNAL
 ...
 * @returns total bytes written from both buffers, or -1 on error
 */
int _dbus_write_two(int fd, const DBusString* buffer1, int start1, int len1,
                    const DBusString* buffer2, int start2, int len2)
{
    _dbus_assert(buffer1 != NULL);
    _dbus_assert(start1 >= 0);
    _dbus_assert(start2 >= 0);
...
#else  /* HAVE_WRITEV */
    {
        ...
        if (ret1 == len1 && buffer2 != NULL)
        {
            ...
            if (ret2 < 0)
                ret2 =
                    0; /* we can't report an error as the first write was OK */
 ...
 */
int _dbus_connect_unix_socket(const char* path, dbus_bool_t abstract,
                              DBusError* error)
{
    int fd;
    size_t path_len;
    ...
    if (connect(fd, (struct sockaddr*)&addr,
                _DBUS_STRUCT_OFFSET(struct sockaddr_un, sun_path) + path_len) <
        0)
    {
        dbus_set_error(error, _dbus_error_from_errno(errno),
                       "Failed to connect to socket %s: %s", path,
...
int _dbus_connect_exec(const char* path, char* const argv[], DBusError* error)
{
    ...
    {
        dbus_set_error(error, _dbus_error_from_errno(errno),
                       "Failed to fork() to call %s: %s", path,
                       _dbus_strerror(errno));
        close(fds[0]);
        close(fds[1]);
        return -1;
 ...
 */
int _dbus_listen_unix_socket(const char* path, dbus_bool_t abstract,
                             DBusError* error)
{
    int listen_fd;
    struct sockaddr_un addr;
    ...
    if (bind(listen_fd, (struct sockaddr*)&addr,
             _DBUS_STRUCT_OFFSET(struct sockaddr_un, sun_path) + path_len) < 0)
    {
        dbus_set_error(error, _dbus_error_from_errno(errno),
                       "Failed to bind socket \"%s\": %s", path,
                       _dbus_strerror(errno));
        _dbus_close(listen_fd, NULL);
        return -1;
    }
    ...
    {
        dbus_set_error(error, _dbus_error_from_errno(errno),
                       "Failed to listen on socket \"%s\": %s", path,
                       _dbus_strerror(errno));
        _dbus_close(listen_fd, NULL);
        return -1;
    }
 ...
 * @returns connection file descriptor or -1 on error
 */
DBusSocket _dbus_connect_tcp_socket(const char* host, const char* port,
                                    const char* family, DBusError* error)
{
    return _dbus_connect_tcp_socket_with_nonce(host, port, family,
                                               (const char*)NULL, error);
}
                                               ...
                                               DBusError* error)
{
    ...
    {
        dbus_set_error(error, _dbus_error_from_gai(res, errno),
                       "Failed to lookup host/port: \"%s:%s\": %s (%d)", host,
    ...
    if (fd.fd == -1)
    {
        dbus_set_error(error, _dbus_error_from_errno(saved_errno),
                       "Failed to connect to socket \"%s:%s\" %s", host, port,
 ...
 */
int _dbus_listen_tcp_socket(const char* host, const char* port,
                            const char* family, DBusString* retport,
                            DBusSocket** fds_p, DBusError* error)
{
    int saved_errno;
...
redo_lookup_with_port:
    ...
    while (tmp)
    {
        ...
        /* Nagle's algorithm imposes a huge delay on the initial messages
           going over TCP. */
        tcp_nodelay_on = 1;
        if (setsockopt(fd, IPPROTO_TCP, TCP_NODELAY, &tcp_nodelay_on,
        ...
        newlisten_fd =
            dbus_realloc(listen_fd, sizeof(DBusSocket) * (nlisten_fd + 1));
        if (!newlisten_fd)
        {
            _dbus_close(fd, NULL);
            dbus_set_error(error, DBUS_ERROR_NO_MEMORY,
        ...
        if (!_dbus_string_get_length(retport))
        {
            ...
            if (!port || !strcmp(port, "0"))
            {
                ...
                if (result == -1)
                {
                    saved_errno = errno;
                    dbus_set_error(
                        error, _dbus_error_from_errno(saved_errno),
                        "Failed to retrieve socket name for \"%s:%s\": %s",
                        host ? host : "*", port, _dbus_strerror(saved_errno));
                    goto failed;
                }

                if ((res = getnameinfo((struct sockaddr*)&addr, addrlen, NULL,
                                       0, portbuf, sizeof(portbuf),
    ...
    if (!nlisten_fd)
    {
        errno = EADDRINUSE;
        dbus_set_error(error, _dbus_error_from_errno(errno),
                       "Failed to bind socket \"%s:%s\": %s", host ? host : "*",
...
#if defined(HAVE_CMSGCRED)
    bytes_written = sendmsg(server_fd, &msg,
                            0
#if HAVE_DECL_MSG_NOSIGNAL
                                | MSG_NOSIGNAL
#endif
...
#endif
    {
        bytes_written = send(server_fd, buf, 1,
                             0
#if HAVE_DECL_MSG_NOSIGNAL
                                 | MSG_NOSIGNAL
#endif
                                            ...
                                            DBusCredentials* credentials)
{
    ...
    if (!_dbus_credentials_add_linux_security_label(
            credentials, _dbus_string_get_const_data(&buf)))
    {
        oom = TRUE;
        goto out;
    }
...
again:
...

    {
         ...
         * all, because this is much less fragile.
         */
#ifdef __OpenBSD__
        ...
        if (getsockopt(client_fd.fd, SOL_SOCKET, SO_PEERCRED, &cr, &cr_len) !=
            0)
        {
        ...
        {
            _dbus_verbose("Failed to getsockopt(SO_PEERCRED), returned %d "
                          "bytes, expected %d\n",
        ...
        {
            _dbus_verbose("Failed to getsockopt(LOCAL_PEEREID), returned %d "
                          "bytes, expected %d\n",
                          cr_len, (int)sizeof(cr));
        }
        else
        ...
        if (getpeerucred(client_fd.fd, &ucred) == 0)
        {
            ...
            else
            {
                ...
                else
                {
                    ...
                    {
                        _dbus_credentials_add_adt_audit_data(credentials, data,
                                                             size);
                        free(data);
                    }
                }
                                               ...
                                               DBusError* error)
{
    ...
    {
        dbus_set_error(error, _dbus_error_from_errno(errno), "%s",
                       _dbus_strerror(errno));
    ...
    {
        dbus_set_error(error, DBUS_ERROR_FAILED,
                       "%s directory is owned by user %lu, not %lu", directory,
                       (unsigned long)sb.st_uid, (unsigned long)geteuid());
        return FALSE;
    }
...
static dbus_bool_t fill_user_info_from_passwd(struct passwd* p,
                                              DBusUserInfo* info,
                                              DBusError* error)
{
...
static dbus_bool_t fill_user_info(DBusUserInfo* info, dbus_uid_t uid,
                                  const DBusString* username, DBusError* error)
{
...
#if defined(HAVE_POSIX_GETPWNAM_R) || defined(HAVE_NONPOSIX_GETPWNAM_R)
    {
        ...
        {
            dbus_set_error(
                error, _dbus_error_from_errno(errno),
...
#ifdef HAVE_GETGROUPLIST
    {
        ...
        if (getgrouplist(username_c, info->primary_gid, buf, &buf_count) < 0)
        {
            ...
            /* Presumed cause of negative return code: buf has insufficient
               ...
               case is to pass back the actual number of groups in buf_count,
               but on Mac OS X 10.5, buf_count is unhelpfully left alone. So as
               a hack, try to help out a bit by guessing a larger number of
               groups, within reason.. might still fail, of course, but we can
               at least print a more informative message.  I looked up the
               "right way" to do this by downloading Apple's own source code for
               the "id" command, and it turns out that they use an undocumented
               library function getgrouplist_2 (!) which is not declared in any
               header in /usr/include (!!). That did not seem like the way to go
               here.
            */
            ...
            errno = 0;
            if (getgrouplist(username_c, info->primary_gid, buf, &buf_count) <
                0)
            {
                if (errno == 0)
                {
                    _dbus_warn(
                        "It appears that username \"%s\" is in more than %d "
 ...
 * @returns #TRUE on success
 */
dbus_bool_t _dbus_user_info_fill(DBusUserInfo* info, const DBusString* username,
                                 DBusError* error)
{
 ...
 * @returns #TRUE on success
 */
dbus_bool_t _dbus_user_info_fill_uid(DBusUserInfo* info, dbus_uid_t uid,
                                     DBusError* error)
{
 ...
 * @returns #FALSE if no memory; does not properly roll back on failure, so only
 * some credentials may have been added
 */
dbus_bool_t
...
static dbus_bool_t _dbus_set_fd_nonblocking(int fd, DBusError* error)
{
    ...
    {
        dbus_set_error(error, _dbus_error_from_errno(errno),
                       "Failed to get flags from file descriptor %d: %s", fd,
                       _dbus_strerror(errno));
    ...
    if (fcntl(fd, F_SETFL, val | O_NONBLOCK) < 0)
    {
        dbus_set_error(
            error, _dbus_error_from_errno(errno),
            "Failed to set nonblocking flag of file descriptor %d: %s", fd,
            _dbus_strerror(errno));
...
#else
    fprintf(stderr, "  D-Bus not compiled with backtrace support so unable to "
                    "print a backtrace\n");
#endif
}
 ...
 */
dbus_bool_t _dbus_socketpair(DBusSocket* fd1, DBusSocket* fd2,
                             dbus_bool_t blocking, DBusError* error)
{
    ...
    if (!blocking && (!_dbus_set_fd_nonblocking(fds[0], NULL) ||
 ...
 * TMPDIR, TMP, and TEMP in that order. If none of those are set "/tmp" is
 * returned
 *
 ...
 */
static dbus_bool_t _read_subprocess_line_argv(const char* progpath,
...
#define WRITE_END 1
    ...
    {
        dbus_set_error(error, _dbus_error_from_errno(errno),
                       "Failed to create a pipe to call %s: %s", progpath,
                       _dbus_strerror(errno));
    ...
    {
        dbus_set_error(error, _dbus_error_from_errno(errno),
                       "Failed to fork() to call %s: %s", progpath,
                       _dbus_strerror(errno));
    ...
    if (pid == 0)
    {
        ...
        if (!_dbus_ensure_standard_fds(DBUS_FORCE_STDIN_NULL, &error_str))
        {
            ...
            if (write(errors_pipe[WRITE_END], error_str, strlen(error_str)) <
                    0 ||
 ...
 * @returns #TRUE on success, #FALSE if an error happened
 */
dbus_bool_t _dbus_get_autolaunch_address(const char* scope, DBusString* address,
                                         DBusError* error)
{
...
#ifdef DBUS_ENABLE_LAUNCHD
static dbus_bool_t _dbus_lookup_session_address_launchd(DBusString* address,
                                                        DBusError* error)
{
    ...
    valid_socket = _dbus_lookup_launchd_socket(
        &socket_path, "DBUS_LAUNCHD_SESSION_BUS_SOCKET", error);
...
dbus_bool_t _dbus_lookup_user_bus(dbus_bool_t* supported, DBusString* address,
                                  DBusError* error)
{
...
dbus_bool_t _dbus_lookup_session_address(dbus_bool_t* supported,
                                         DBusString* address, DBusError* error)
{
#ifdef DBUS_ENABLE_LAUNCHD
    *supported = TRUE;
                                                   ...
                                                   DBusCredentials* credentials)
{
    ...
    if (!_dbus_concat_dir_and_file(&homedir, &dotdir))
        goto failed;

    if (!_dbus_string_copy(&homedir, 0, directory,
                           _dbus_string_get_length(directory)))
    {
        goto failed;
    }
...
// PENDING(kdab) docs
dbus_bool_t _dbus_daemon_publish_session_bus_address(const char* addr,
                                                     const char* scope)
{
    return TRUE;
}
...
{
    /* Avoid the -Wlogical-op GCC warning, which can be triggered when EAGAIN
     * and EWOULDBLOCK are numerically equal, which is permitted as described by
     * errno(3).
     */
 ...
 * @param error return location for error code
 */
dbus_bool_t _dbus_append_address_from_socket(DBusSocket fd, DBusString* address,
                                             DBusError* error)
{
    union
    {
    ...
    switch (socket.sa.sa_family)
    {
        case AF_UNIX:
            ...
            break;
        case AF_INET:
            if (inet_ntop(AF_INET, &socket.ipv4.sin_addr, hostip,
                          sizeof(hostip)))
                if (_dbus_string_append_printf(
                        address, "tcp:family=ipv4,host=%s,port=%u", hostip,
        ...
        case AF_INET6:
            ...
            if (inet_ntop(AF_INET6, &socket.ipv6.sin6_addr, hostip,
                          sizeof(hostip)))
                if (_dbus_string_append_printf(address,
                                               "tcp:family=ipv6,port=%u,host=",
        ...
        default:
            dbus_set_error(
                error, _dbus_error_from_errno(EINVAL),
...
void _dbus_logv(DBusSystemLogSeverity severity, const char* msg, va_list args)
{
    ...
    {
        DBUS_VA_COPY(tmp, args);
        fprintf(stderr, "%s[" DBUS_PID_FORMAT "]: ", syslog_tag,
                _dbus_getpid());
