                     ...
                     unsigned int flags)
{
    ...
    *ptree = NULL;
     ...
     */
    if ((tree->levels = OPENSSL_zalloc(sizeof(*tree->levels) * (n + 1))) ==
        NULL)
    {
        OPENSSL_free(tree);
        X509V3err(X509V3_F_TREE_INIT, ERR_R_MALLOC_FAILURE);
...
static int tree_link_any(X509_POLICY_LEVEL* curr,
                         const X509_POLICY_CACHE* cache, X509_POLICY_TREE* tree)
{
    int i;
...
{
    if (*pnodes == NULL && (*pnodes = policy_node_cmp_new()) == NULL)
        return 0;
