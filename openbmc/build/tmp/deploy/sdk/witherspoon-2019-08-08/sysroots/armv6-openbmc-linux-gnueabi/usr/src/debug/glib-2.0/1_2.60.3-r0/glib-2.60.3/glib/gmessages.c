 ...
 * format.
 *
 * The convenience macros g_info(), g_message(), g_debug(), g_warning() and
 * g_error() will use the traditional g_log() API unless you define the symbol
...
_GLIB_EXTERN void myInvalidParameterHandler(const wchar_t* expression,
...
static void write_string(FILE* stream, const gchar* string)
{
    fputs(string, stream);
}

static void write_string_sized(FILE* stream, const gchar* string, gssize length)
{
...
static GLogFunc g_log_domain_get_handler_L(GLogDomain* domain,
                                           GLogLevelFlags log_level,
                                           gpointer* data)
{
    if (domain && log_level)
...
{
    return g_log_set_handler_full(log_domain, log_levels, log_func, user_data,
                                  NULL);
}
 ...
 * Since: 2.46
 */
guint g_log_set_handler_full(const gchar* log_domain, GLogLevelFlags log_levels,
                             GLogFunc log_func, gpointer user_data,
                             GDestroyNotify destroy)
{
...
static gchar* strdup_convert(const gchar* string, const gchar* charset)
{
    ...
    else
    {
        ...
        gchar* result = g_convert_with_fallback(string, -1, charset, "UTF-8",
                                                "?", NULL, NULL, &err);
        if (result)
            return result;
        else
...
static FILE* mklevel_prefix(gchar level_prefix[STRING_BUFFER_SIZE],
                            GLogLevelFlags log_level, gboolean use_color)
{
    ...
    switch (log_level & G_LOG_LEVEL_MASK)
    {
        ...
        default:
            if (log_level)
            {
                ...
                format_unsigned(level_prefix + 4, log_level & G_LOG_LEVEL_MASK,
                                16);
            }
            else
                strcat(level_prefix, "LOG");
            ...
            const gchar* format, va_list args)
{
    ...
    if (expected_messages)
    {
        ...
        {
            expected_messages =
                g_slist_delete_link(expected_messages, expected_messages);
        ...
        else if ((log_level & G_LOG_LEVEL_DEBUG) != G_LOG_LEVEL_DEBUG)
        {
            ...
            expected_message = g_strdup_printf(
                "Did not see expected message %s-%s: %s",
                expected->log_domain ? expected->log_domain : "**",
                level_prefix, expected->pattern);
            g_log_default_handler(G_LOG_DOMAIN, G_LOG_LEVEL_CRITICAL,
    ...
    for (i = g_bit_nth_msf(log_level, -1); i >= 0;
         i = g_bit_nth_msf(log_level, i))
    {
        ...
        if (log_level & test_level)
        {
            ...
            if (test_level & G_LOG_FLAG_RECURSION)
                ...
                log_func =
                    g_log_domain_get_handler_L(domain, test_level, &data);
            domain = NULL;
            g_mutex_unlock(&g_messages_lock);
            ...
            {
                masquerade_fatal =
                    fatal_log_func && !fatal_log_func(log_domain, test_level,
                                                      msg, fatal_log_data);
            }
            ...
            if ((test_level & G_LOG_FLAG_FATAL) && !masquerade_fatal)
            {
                ...
                {
                    gchar* locale_msg =
                        g_locale_from_utf8(fatal_msg_buf, -1, NULL, NULL, NULL);
...
/* We might be using tty emulators such as mintty, so try to detect it, if we
 * passed in a valid FD so we need to check the name of the pipe if _isatty (fd)
 * == 0
 */

static gboolean win32_is_pipe_tty(int fd)
{
    ...
    /* mintty uses a pipe, in the form of
 ...
 * be aborted by calling G_BREAKPOINT() at the end of this function. If the log
 * writer returns %G_LOG_WRITER_UNHANDLED (failure), no other fallback writers
 * will be tried. See the documentation for #GLogWriterFunc for information on
 * chaining writers.
 *
 ...
 *                   "MESSAGE", "This is a debug message about pointer %p and
 * integer %u.", some_pointer, some_integer);
 * ]|
 *
...
void g_log_structured(const gchar* log_domain, GLogLevelFlags log_level, ...)
{
    ...
    for (p = va_arg(args, gchar*), i = n_fields; strcmp(p, "MESSAGE") != 0;
...
void g_log_variant(const gchar* log_domain, GLogLevelFlags log_level,
                   GVariant* fields)
{
    GVariantIter iter;
    GVariant* value;
        ...
        else if (g_variant_is_of_type(value, G_VARIANT_TYPE_BYTESTRING))
        {
            ...
            else
            {
                _g_fprintf(
                    ...
                    " passed to g_log_variant(). Truncating to " G_STRINGIFY(
                        G_MAXSSIZE) " bytes.",
                    s);
    ...
    /* Log it. */
    g_log_structured_array(log_level, (GLogField*)fields_array->data,
                           fields_array->len);
 ...
 * Since: 2.50
 */
void g_log_structured_array(GLogLevelFlags log_level, const GLogField* fields,
                            gsize n_fields)
{
...
void g_log_structured_standard(const gchar* log_domain,
                               GLogLevelFlags log_level, const gchar* file,
                               const gchar* line, const gchar* func,
                               const gchar* message_format, ...)
{
    ...
    {
        fields[4].value = message_allocated =
            g_strdup_vprintf(message_format, args);
    }
 ...
 * Since: 2.50
 */
void g_log_set_writer_func(GLogWriterFunc func, gpointer user_data,
...
gboolean g_log_writer_supports_color(gint output_fd)
{
     ...
     * from Windows 10 onwards; bash on Windows has always supported ANSI
     * colors. The Windows 10 color support is supported on: -Output in the
     * cmd.exe, MSYS/Cygwin standard consoles. -Output in the cmd.exe,
     * MSYS/Cygwin piped to the less program. but not: -Output in Cygwin via
     * mintty (https://github.com/mintty/mintty/issues/482) -Color code output
     * when output redirected to file (i.e. program 2> some.txt)
     *
                                  ...
                                  gboolean use_color)
{
    ...
    g_string_append_printf(
        gstring, "%s%s.%03d%s: ", use_color ? "\033[34m" : "", time_buf,
        (gint)((now / 1000) % 1000), color_reset(use_color));
...
#if defined(__linux__) && !defined(__BIONIC__) && defined(HAVE_MKOSTEMP) &&    \
    defined(O_CLOEXEC)
#define ENABLE_JOURNAL_SENDV
#endif

#ifdef ENABLE_JOURNAL_SENDV
static int journal_sendv(struct iovec* iov, gsize iovlen)
{
    int buf_fd = -1;
    struct msghdr mh;
    ...
    if (g_strlcpy(sa.sun_path, "/run/systemd/journal/socket",
                                               ...
                                               gpointer user_data)
{
    ...
    out =
        g_log_writer_format_fields(log_level, fields, n_fields,
 ...
 */
static gboolean log_is_old_api(const GLogField* fields, gsize n_fields)
{
    return (n_fields >= 1 &&
...
{
    g_log(log_domain, G_LOG_LEVEL_CRITICAL, "%s: assertion '%s' failed",
 ...
 */
void g_warn_message(const char* domain, const char* file, int line,
                    const char* func, const char* warnexpr)
{
    char *s, lstr[32];
    g_snprintf(lstr, 32, "%d", line);
    if (warnexpr)
        s = g_strconcat("(", file, ":", lstr, "):", func, func[0] ? ":" : "",
...
void g_assert_warning(const char* log_domain, const char* file, const int line,
                      const char* pretty_function, const char* expression)
{
    if (expression)
        g_log(log_domain, G_LOG_LEVEL_ERROR,
              "file %s: line %d (%s): assertion failed: (%s)", file, line,
              pretty_function, expression);
    else
        g_log(log_domain, G_LOG_LEVEL_ERROR,
              "file %s: line %d (%s): should not be reached", file, line,
              pretty_function);
    _g_log_abort(FALSE);
    g_abort();
}
 ...
 * or stdout and if the log level is fatal it calls G_BREAKPOINT(). It
 * automatically prints a new-line character after the message, so one does not
 * need to be manually included in @message.
 *
