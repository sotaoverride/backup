...
static void name_ownership_link(NameOwnership* ownership, CRBNode* parent,
                                CRBNode** slot)
{
    assert(!c_rbnode_is_linked(&ownership->owner_node));

    c_rbtree_add(&ownership->owner->ownership_tree, parent, slot,
                 &ownership->owner_node);
}
...
static int name_ownership_new(NameOwnership** ownershipp, NameOwner* owner,
                              User* user, Name* name)
{
...
static int name_ownership_update(NameOwnership* ownership, uint32_t flags,
                                 NameChange* change)
{
...
static void name_link(Name* name, CRBNode* parent, CRBNode** slot)
{
    ...
    c_rbtree_add(&name->registry->name_tree, parent, slot,
                 &name->registry_node);
}
 ...
 */
int name_registry_ref_name(NameRegistry* registry, Name** namep,
                           const char* name_str)
{
    CRBNode **slot, *parent;
    int r;

    slot = c_rbtree_find_slot(&registry->name_tree, name_compare, name_str,
                              &parent);
    if (!slot)
    {
...
{
    return c_rbtree_find_entry(&registry->name_tree, name_compare, name_str,
                               Name, registry_node);
}
 ...
 */
int name_registry_request_name(NameRegistry* registry, NameOwner* owner,
                               User* user, const char* name_str, uint32_t flags,
                               NameChange* change)
{
    _c_cleanup_(name_unrefp) Name* name = NULL;
    ...
    slot = c_rbtree_find_slot(&owner->ownership_tree, name_ownership_compare,
                              name, &parent);
    if (!slot)
    {
 ...
 *         code on failure.
 */
int name_registry_release_name(NameRegistry* registry, NameOwner* owner,
                               const char* name_str, NameChange* change)
{
    NameOwnership* ownership;
    Name* name;
    ...
    ownership =
        c_rbtree_find_entry(&owner->ownership_tree, name_ownership_compare,
                            name, NameOwnership, owner_node);
    if (!ownership)
...
int name_snapshot_new(NameSnapshot** snapshotp, NameOwner* owner)
{
    ...
    c_rbtree_for_each_entry(ownership, &owner->ownership_tree,
                            owner_node)++ n_names;
