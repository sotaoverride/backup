...
static void audit_list_iter_init(struct audit_list_iter* iter)
{
...
static const char* audit_list_iter_next(struct audit_list_iter* iter)
{
...
static ElfW(Addr) _dl_start_final(void* arg, struct dl_start_final_info* info);
#endif
...
#define ELF_MACHINE_START_ADDRESS(map, start) (start)
        ...
        return ELF_MACHINE_START_ADDRESS(GL(dl_ns)[LM_ID_BASE]._ns_loaded,
                                         entry);
    }
}
...
static void map_doit(void* a)
{
    ...
    args->map = _dl_map_object(args->loader, args->str, type, 0, args->mode,
                               LM_ID_BASE);
}
...
static void dlmopen_doit(void* a)
{
    ...
    args->map = _dl_open(
        args->fname, (RTLD_LAZY | __RTLD_DLOPEN | __RTLD_AUDIT | __RTLD_SECURE),
        dl_main, LM_ID_NEWLM, _dl_argc, _dl_argv, __environ);
}
...
static inline struct link_map* find_needed(const char* name)
{
    struct r_scope_elem* scope =
        &GL(dl_ns)[LM_ID_BASE]._ns_loaded->l_searchlist;
...
static void* init_tls(void)
{
    ...
    /* Allocate.  */
    GL(dl_tls_dtv_slotinfo_list) = (struct dtv_slotinfo_list*)calloc(
        sizeof(struct dtv_slotinfo_list) + nelem * sizeof(struct dtv_slotinfo),
        1);
...
static unsigned int do_preload(const char* fname, struct link_map* main_map,
                               const char* where)
{
    const char* objname;
...
/* The LD_PRELOAD environment variable gives list of libraries
   ...
   performed.  */
unsigned int handle_ld_preload(const char* preloadlist,
        ...
        switch (ph->p_type)
        {
            ...
            case PT_INTERP:
                /* This "interpreter segment" was used by the program loader to
                   find the program interpreter, which is this program itself,
                   the dynamic linker.  We note what name finds us, so that a
                   future dlopen call or DT_NEEDED entry, for something that
                   wants to link against the dynamic linker as a shared library,
                   will know that the shared object is already loaded.  */
                _dl_rtld_libname.name =
                    ((const char*)main_map->l_addr + ph->p_vaddr);
                /* _dl_rtld_libname.next = NULL;	Already zero.  */
                GL(dl_rtld_map).l_libname = &_dl_rtld_libname;

                /* Ordinarilly, we would get additional names for the loader
                   from our DT_SONAME.  This can't happen if we were actually
                   linked as a static executable (detect this case when we have
                   no DYNAMIC). If so, assume the filename component of the
                   interpreter path to be our SONAME, and add it to our name
                   list.  */
            ...
            case PT_TLS:
                if (ph->p_memsz > 0)
                {
                    ...
                    if (ph->p_align == 0)
                        ...
                        main_map->l_tls_firstbyte_offset =
...
ERROR: ld.so: object '%s' cannot be loaded as audit interface: %s; ignored.\n",
            ...
            else
            {
                ...
                unsigned int lav;
                if (err_str == NULL && (laversion = largs.result) != NULL &&
       ...
       after relocation.  */
        ...
        if (GLRO(dl_debug_mask) & DL_DEBUG_PRELINK)
        {
            ...
            for (i = 0; i < scope->r_nlist; i++)
            {
                ...
                _dl_printf(
                    "\t%s => %s (0x%0*Zx, 0x%0*Zx)",
                    DSO_FILENAME(l->l_libname->name), DSO_FILENAME(l->l_name),
                    (int)sizeof l->l_map_start * 2, (size_t)l->l_map_start,
                    (int)sizeof l->l_addr * 2, (size_t)l->l_addr);
        ...
        else if (GLRO(dl_debug_mask) & DL_DEBUG_UNUSED)
        {
            ...
            struct relocate_args args = {
                .l = l,
                .reloc_mode =
                    ((GLRO(dl_lazy) ? RTLD_LAZY : 0) | __RTLD_NOIFUNC)};
        ...
        if (__builtin_expect(mode, trace) != trace)
            for (i = 1; i < (unsigned int)_dl_argc; ++i)
            {
                ...
                result = _dl_lookup_symbol_x(
                    _dl_argv[i], main_map, &ref, main_map->l_scope, NULL,
                    ELF_RTYPE_CLASS_PLT, DL_LOOKUP_ADD_DEPENDENCY, NULL);

                loadbase = LOOKUP_VALUE_ADDRESS(result, false);

                _dl_printf("%s found at 0x%0*Zd in object at 0x%0*Zd\n",
                           _dl_argv[i], (int)sizeof ref->st_value * 2,
                           (size_t)ref->st_value, (int)sizeof loadbase * 2,
                           (size_t)loadbase);
            }
        else
        {
...
#define VERNEEDTAG (DT_NUM + DT_THISPROCNUM + DT_VERSIONTAGIDX(DT_VERNEED))
                    ...
                    while (1)
                    {
                        ...
                        while (1)
                        {
                            ...
                            _dl_printf(
                                "\t\t%s (%s) %s=> %s\n", strtab + ent->vn_file,
                                strtab + aux->vna_name,
                                aux->vna_flags & VER_FLG_WEAK ? "[WEAK] " : "",
                                fname ?: "not found");
        ...
        !__builtin_expect(GLRO(dl_dynamic_weak), 0))
    {
        ...
        liblist =
            (ElfW(Lib)*)main_map->l_info[ADDRIDX(DT_GNU_LIBLIST)]->d_un.d_ptr;
        liblistend = (ElfW(
            Lib)*)((char*)liblist +
    ...
    if (prelinked)
    {
        if (main_map->l_info[ADDRIDX(DT_GNU_CONFLICT)] != NULL)
        {
            ...
            conflict = (ElfW(Rela)*)main_map->l_info[ADDRIDX(DT_GNU_CONFLICT)]
                           ->d_un.d_ptr;
            conflictend =
                (ElfW(Rela)*)((char*)conflict +
                              main_map->l_info[VALIDX(DT_GNU_CONFLICTSZ)]
                                  ->d_un.d_val);
    ...
    if (__glibc_unlikely(GLRO(dl_naudit) > 0))
    {
        ...
        if (head->l_auditing == 0)
        {
            ...
            {
                if (afct->activity != NULL)
                    afct->activity(&head->l_audit[cnt].cookie,
                                   LA_ACT_CONSISTENT);
        ...
        switch (len)
        {
            ...
            case 20:
                ...
                /* We might have some extra environment variable to handle. This
                   ...
                   in the switch statement here.  The code here assumes that
                   added environment variables have a different length.  */
