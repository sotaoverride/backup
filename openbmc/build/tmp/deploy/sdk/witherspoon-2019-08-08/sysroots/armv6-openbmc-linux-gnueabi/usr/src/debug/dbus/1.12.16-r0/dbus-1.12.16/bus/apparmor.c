 ...
 */
static BusAppArmorConfinement* bus_apparmor_confinement_new(char* label,
                                                            const char* mode)
{
    ...
    _bus_apparmor_detect_aa_dbus_support(dbus_bool_t* is_supported)
{
    ...
    *is_supported = FALSE;
    ...
    mask_file =
        open(_dbus_string_get_const_data(&aa_dbus), O_RDONLY | O_CLOEXEC);
    if (mask_file != -1)
    {
        *is_supported = TRUE;
        close(mask_file);
...
static void log_message(dbus_bool_t allow, const char* op, DBusString* data)
{
    ...
    if (audit_fd >= 0)
    {
        ...
        audit_log_user_avc_message(audit_fd, AUDIT_USER_AVC,
                                   _dbus_string_get_const_data(&avc), NULL,
                                   NULL, NULL, getuid());
        _dbus_string_free(&avc);
        return;
    }
...
static dbus_bool_t _dbus_append_pair_uint(DBusString* auxdata, const char* name,
                                          unsigned long value)
{
...
static dbus_bool_t _dbus_append_pair_str(DBusString* auxdata, const char* name,
                                         const char* value)
{
    return _dbus_string_append(auxdata, " ") &&
           _dbus_string_append(auxdata, name) &&
...
static dbus_bool_t build_common_query(DBusString* query, const char* con,
                                      const char* bustype)
{
    /**
...
static dbus_bool_t build_service_query(DBusString* query, const char* con,
                                       const char* bustype, const char* name)
{
    return build_common_query(query, con, bustype) && query_append(query, name);
}

static dbus_bool_t build_message_query(DBusString* query, const char* src_con,
                                       const char* bustype, const char* name,
                                       const char* dst_con, const char* path,
                                       const char* interface,
                                       const char* member)
{
    return build_common_query(query, src_con, bustype) &&
           query_append(query, name) && query_append(query, dst_con) &&
           query_append(query, path) && query_append(query, interface) &&
           query_append(query, member);
}

static dbus_bool_t build_eavesdrop_query(DBusString* query, const char* con,
                                         const char* bustype)
{
...
static void set_error_from_denied_message(
    DBusError* error, DBusConnection* sender,
    DBusConnection* proposed_recipient, dbus_bool_t requested_reply,
    const char* msgtype, const char* path, const char* interface,
    const char* member, const char* error_name, const char* destination)
{
    const char* proposed_recipient_loginfo;
    const char* unset = "(unset)";

    proposed_recipient_loginfo =
        proposed_recipient ? bus_connection_get_loginfo(proposed_recipient)
                           : "bus";
    ...
    dbus_set_error(error, DBUS_ERROR_ACCESS_DENIED,
                   ...
                   "destination=\"%s\" (%s)",
                   msgtype, bus_connection_get_name(sender),
                   bus_connection_get_loginfo(sender),
                   interface ? interface : unset, member ? member : unset,
                   error_name ? error_name : unset, requested_reply,
                   destination, proposed_recipient_loginfo);
}
#endif /* HAVE_APPARMOR */
...
void bus_apparmor_confinement_unref(BusAppArmorConfinement* confinement)
{
    ...
    {
        /**
         * Do not free confinement->mode, as libapparmor does a single malloc
         * for both confinement->label and confinement->mode.
         */
    ...
    if (aa_getpeercon(fd, &label, &mode) == -1)
    {
        if (errno == ENOMEM)
            ...
            dbus_set_error(error, _dbus_error_from_errno(errno),
                           "Failed to get AppArmor confinement information of "
                           "socket peer: %s",
                           _dbus_strerror(errno));
        return NULL;
    }
                                                ...
                                                DBusError* error)
{
    ...
    res = aa_query_label(AA_DBUS_BIND, _dbus_string_get_data(&qstr),
                         _dbus_string_get_length(&qstr), &allow, &audit);
    _dbus_string_free(&qstr);
    if (res == -1)
    {
        serrno = errno;
...
dbus_bool_t bus_apparmor_allows_send(
    DBusConnection* sender, DBusConnection* proposed_recipient,
    dbus_bool_t requested_reply, const char* bustype, int msgtype,
    const char* path, const char* interface, const char* member,
    const char* error_name, const char* destination, const char* source,
    BusActivationEntry* activation_entry, DBusError* error)
{
    ...
    {
        dst_label =
            bus_activation_entry_get_assumed_apparmor_label(activation_entry);
    }
    else
    {
        dst_con = bus_con;
     ...
     * And permission to send a message to Y grants permission to receive a
     * reply from Y. Note that this only applies to requested replies.
     * Unrequested replies still require a policy query.
     */
    if (requested_reply)
    {
        /* ignore requested reply messages and let dbus reply mapping handle
         * them as the send was already allowed
         */
        src_allow = TRUE;
        dst_allow = TRUE;
        goto out;
    ...
    else
    {
        ...
        res = aa_query_label(src_perm, _dbus_string_get_data(&qstr),
                             _dbus_string_get_length(&qstr), &src_allow,
                             &src_audit);
        _dbus_string_free(&qstr);
        if (res == -1)
        {
    ...
    else
    {
        ...
        res = aa_query_label(dst_perm, _dbus_string_get_data(&qstr),
                             _dbus_string_get_length(&qstr), &dst_allow,
                             &dst_audit);
        _dbus_string_free(&qstr);
        if (res == -1)
        {
    ...
    if (!src_allow || !dst_allow)
        set_error_from_denied_message(
            error, sender, proposed_recipient, requested_reply, msgtypestr,
            path, interface, member, error_name, destination);
     ...
     *      message. Unrequested replies will be silently discarded if the
     * sender does not have permission to send to the receiver or if the
     * receiver does not have permission to receive from the sender.
     */
...
audit:
    ...
    if (error_name &&
        !_dbus_append_pair_str(&auxdata, "error_name", error_name))
        goto oom;
    ...
    if (src_audit)
    {
        ...
        if (destination &&
            !_dbus_append_pair_str(&auxdata, "name", destination))
            goto oom;
        ...
        if (src_errno &&
            !_dbus_append_pair_str(&auxdata, "info", strerror(src_errno)))
            goto oom;
    ...
    if (dst_audit)
    {
        ...
        if (dst_label && !_dbus_append_pair_str(&auxdata, "label", dst_label))
            goto oom;
        ...
        if (dst_errno &&
            !_dbus_append_pair_str(&auxdata, "info", strerror(dst_errno)))
            goto oom;
                                              ...
                                              DBusError* error)
{
    ...
    res = aa_query_label(AA_DBUS_EAVESDROP, _dbus_string_get_data(&qstr),
                         _dbus_string_get_length(&qstr), &allow, &audit);
    _dbus_string_free(&qstr);
    if (res == -1)
    {
