...
td_err_e td_thr_get_info(const td_thrhandle_t* th, td_thrinfo_t* infop)
{
    ...
    else
    {
       ...
       multiple ps_pdread calls.  */
        ...
        err = DB_GET_FIELD_ADDRESS(tls, th->th_ta_p, th->th_unique, pthread,
                                   specific, 0);
        if (err != TD_OK)
            return err;
    ...
    infop->ti_pri =
        ((uintptr_t)schedpolicy == SCHED_OTHER ? 0 : (uintptr_t)schedprio);
    ...
    if (copy != NULL)
        err = DB_GET_FIELD_LOCAL(infop->ti_startfunc, th->th_ta_p, copy,
                                 pthread, start_routine, 0);
    if (copy != NULL && err == TD_OK)
    {
        ...
        if (err == TD_NOAPLIC)
            memset(&infop->ti_events.event_bits[idx], 0,
                   (TD_EVENTSIZE - idx) *
                       sizeof infop->ti_events.event_bits[0]);
    }
