...
struct date_time_period
{
    ...
    date_time_period operator-() const
    {
        return date_time_period(type, -value);
    }

    ///
    /// Constructor that creates date_time_period from period_type \a f and a
    /// value \a v -- default 1.
    ///
...
///
///  Get period_type for: Era i.e. AC, BC in Gregorian and Julian calendar,
///  range [0,1]
///
...
///
///  Get period_type for: Year, it is calendar specific, for example 2011 in
///  Gregorian calendar.
///
...
///
///  Get period_type for: Extended year for Gregorian/Julian calendars, where 1
///  BC == 0, 2 BC == -1.
///
...
///
///  Get period_type for: The month of year, calendar specific, in Gregorian
///  [0..11]
///
...
///
///  Get period_type for: The day of month, calendar specific, in Gregorian
///  [1..31]
///
...
///
///  Get period_type for: The number of day in year, starting from 1, in
///  Gregorian  [1..366]
///
...
/// the value to Sunday (1) would forward the date by 5 days forward and not
/// backward by two days as it could be expected if the numbers were taken as
/// is.
///
...
///
///  Get period_type for: Original number of the day of the week in month. For
///  example 1st Sunday,
...
///
///  Get period_type for: Local day of week, for example in France Monday is 1,
///  in US Sunday is 1, [1..7]
///
...
///
///  Get period_type for: First day of week, constant, for example Sunday in US
///  = 1, Monday in France = 2
///
...
///
///  Get date_time_period for: Era i.e. AC, BC in Gregorian and Julian calendar,
///  range [0,1]
///
...
///
///  Get date_time_period for: Year, it is calendar specific, for example 2011
...
///
///  Get date_time_period for: Extended year for Gregorian/Julian calendars,
///  where 1 BC == 0, 2 BC == -1.
///
...
///
///  Get date_time_period for: The month of year, calendar specific, in
///  Gregorian [0..11]
///
...
///
///  Get date_time_period for: The day of month, calendar specific, in Gregorian
///  [1..31]
///
...
///
///  Get date_time_period for: The number of day in year, starting from 1, in
///  Gregorian  [1..366]
///
...
///
///  Get date_time_period for: Local day of week, for example in France Monday
///  is 1, in US Sunday is 1, [1..7]
///
...
///
///  Get date_time_period for: First day of week, constant, for example Sunday
///  in US = 1, Monday in France = 2
///
...
///
inline date_time_period_set operator+(date_time_period_set const& a,
...
///
/// This information is not connected to specific date but generic to locale,
/// and timezone. It is used in obtaining general information about calendar and
/// is essential for creation of date_time objects.
///
class BOOST_LOCALE_DECL calendar
{
  public:
    ///
    /// Create calendar taking locale and timezone information from ios_base
    /// instance.
    ///
    /// \note throws std::bad_cast if ios does not have a locale with installed
    ...
    ///
    /// \note throws std::bad_cast if loc does not have \ref calendar_facet
    /// facet installed
    ///
    ...
    ///
    /// \note throws std::bad_cast if global locale does not have \ref
    /// calendar_facet facet installed
    ///
    ...
    ///
    /// Get greatest possible minimum value for period f, For example for
    /// period::day it is 1, but may be different for other calendars.
    ///
    int greatest_minimum(period::period_type f) const;
    ///
    /// Get maximum value for period f, For example for Gregorian calendar's
    /// maximum period::day it is 31.
    ///
    int maximum(period::period_type f) const;
    ///
    /// Get least maximum value for period f, For example for Gregorian
    /// calendar's maximum period::day it is 28.
    ///
    int least_maximum(period::period_type f) const;

    ///
    /// Get first day of week for specific calendar, for example for US it is 1
    /// - Sunday for France it is 2 - Monday
...
///
/// \brief this class represents a date time and allows to perform various
/// operation according to the locale settings.
///
/// This class allows to manipulate various aspects of dates and times easily
/// using arithmetic operations with periods.
///
/// General arithmetic functions:
///
/// - date_time + date_time_period_set = date_time: move time point forward by
/// specific periods like date_time + month;
/// - date_time - date_time_period_set = date_time: move time point backward by
/// specific periods like date_time - month;
/// - date_time << date_time_period_set  = date_time: roll time point forward by
/// specific periods with rolling to begin if overflows: like "2010-01-31" << 2*
/// day == "2010-01-02" instead of "2010-02-02"
/// - date_time >> date_time_period_set  = date_time: roll time point backward
/// by specific periods with rolling to end if overflows: like "2010-01-02" >>
/// 2* day == "2010-01-31" instead of "2009-12-30"
/// - date_time / period_type = int - current period value: like "2010-12-21" /
/// month == 12. "2010-12-21" / year = 2010
/// - (date_time - date_time) / period_type = int: distance between dates in
/// period_type. Like ("2010-12-01" - "2008-12-01") / month = 24.
///
...
class BOOST_LOCALE_DECL date_time
{
  public:
    ///
    /// Dafault constructor, uses default calendar initialized date_time object
    /// to current time.
    ///
    /// \note throws std::bad_cast if the global locale does not have \ref
    /// calendar_facet facet installed
    ///
    date_time();
    ///
    ...
    ///
    /// Create a date_time object using calendar \a cal and initializes it to
    /// current time.
    ///
    date_time(calendar const& cal);

    ///
    /// Create a date_time object using default calendar and define values given
    /// in \a set
    ///
    /// \note throws std::bad_cast if the global locale does not have \ref
    /// calendar_facet facet installed
    ...
    ///
    /// Create a date_time object using calendar \a cal and define values given
    /// in \a set
    ///
    ...
    /// Get POSIX time
    ///
    /// The POSIX time is number of seconds since January 1st, 1970 00:00 UTC,
    ...
    ///
    /// The POSIX time is number of seconds since January 1st, 1970 00:00 UTC,
    /// ignoring leap seconds. This time can be fetched from Operating system
    /// clock using C function time, gettimeofday and others.
    ///
    void time(double v);

    ///
    /// compare date_time in the timeline (ignores difference in calendar,
    /// timezone etc)
    ///
    ...
    ///
    /// calculate the distance from this date_time to \a other in terms of
    /// perios \a f
    ///
    ...
    int minimum(period::period_type f) const;
    ///
    /// Get minimal possible value for *this time point for a period \a f. For
    /// example in February maximum(day) may be 28 or 29, in January
    /// maximum(day)==31
    ///
    int maximum(period::period_type f) const;
...
///
/// This function uses locale, calendar and time zone of the target stream \a
/// in.
///
/// For example:
/// \code
                                         ...
                                         date_time const& t)
{
    ...
    if (display_flags == flags::date || display_flags == flags::time ||
        display_flags == flags::datetime || display_flags == flags::strftime)
    {
        out << time_point;
    }
    else
...
///
/// This function uses locale, calendar and time zone of the source stream \a
/// in.
///
template <typename CharType>
std::basic_istream<CharType>& operator>>(std::basic_istream<CharType>& in,
                                         date_time& t)
{
    double v;
...
///
/// It is generally used as syntactic sugar to calculate difference between two
/// dates.
///
/// Note: it stores references to the original objects, so it is not recommended
/// to be used outside of the equation you calculate the difference in.
///
class date_time_duration
{
  public:
    ///
    /// Create an object were \a first represents earlier point on time line and
    /// \a second is later point.
    ///
...
///
/// Calculates the difference between two dates, the left operand is a later
/// point on time line. Returns date_time_duration object.
///
inline date_time_duration operator-(date_time const& later,
...
{
///
///  Extract from date_time numerical value of Era i.e. AC, BC in Gregorian and
///  Julian calendar, range [0,1]
///
...
///
///  Extract from date_time numerical value of Year, it is calendar specific,
///  for example 2011 in Gregorian calendar.
///
...
///
///  Extract from date_time numerical value of Extended year for
///  Gregorian/Julian calendars, where 1 BC == 0, 2 BC == -1.
///
...
///
///  Extract from date_time numerical value of The month of year, calendar
///  specific, in Gregorian [0..11]
///
...
///
///  Extract from date_time numerical value of The day of month, calendar
...
///
///  Extract from date_time numerical value of The number of day in year,
...
///
///  Extract from date_time numerical value of Day of week, Sunday=1,
///  Monday=2,..., Saturday=7.
///
/// Note that updating this value respects local day of week, so for example,
/// If first day of week is Monday and the current day is Tuesday then setting
...
///
///  Extract from date_time numerical value of Original number of the day of the
///  week in month. For example 1st Sunday,
/// 2nd Sunday, etc. in Gregorian [1..5]
///
...
///
///  Extract from date_time numerical value of Local day of week, for example in
///  France Monday is 1, in US Sunday is 1, [1..7]
///
...
///
///  Extract from date_time numerical value of The week number within current
///  month
///
...
///
///  Extract from date_time numerical value of First day of week, constant, for
///  example Sunday in US = 1, Monday in France = 2
///
...
///
///  Extract from date_time_duration numerical value of duration in extended
///  years (for Gregorian/Julian calendars, where 1 BC == 0, 2 BC == -1).
///
...
///
///  Extract from date_time_duration numerical value of duration in days of
///  month
///
...
///
///  Extract from date_time_duration numerical value of duration in original
///  number of the day of the week in month
///
...
///
///  Extract from date_time_duration numerical value of duration in local day of
///  week
///
...
///
///  Extract from date_time_duration numerical value of duration in  12 clock
///  hours
///
...
///
///  Extract from date_time_duration numerical value of duration in  am or pm
///  markers
///
...
///
///  Extract from date_time_duration numerical value of duration in the week
///  number in the year
///
...
///
///  Extract from date_time_duration numerical value of duration in the first
///  day of week
///
...
///
/// Example of using date_time functions for generating calendar for current
/// year.
///
