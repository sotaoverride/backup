...
/* Signals {{{1 */
static gboolean g_settings_real_change_event(GSettings* settings,
                                             const GQuark* keys, gint n_keys)
{
    ...
    for (i = 0; i < n_keys; i++)
    {
        ...
        g_signal_emit(settings, g_settings_signals[SIGNAL_CHANGED], keys[i],
                      key);
    }
...
static gboolean g_settings_real_writable_change_event(GSettings* settings,
                                                      GQuark key)
{
...
static void settings_backend_changed(GObject* target, GSettingsBackend* backend,
                                     const gchar* key, gpointer origin_tag)
{
        ...
        g_settings_schema_has_key(settings->priv->schema, key + i))
    {
        ...
        g_signal_emit(settings, g_settings_signals[SIGNAL_CHANGE_EVENT], 0,
                      &quark, 1, &ignore_this);
    }
}

static void settings_backend_path_changed(GObject* target,
                                          GSettingsBackend* backend,
                                          const gchar* path,
                                          gpointer origin_tag)
{
...
static void settings_backend_keys_changed(GObject* target,
                                          GSettingsBackend* backend,
                                          const gchar* path,
                                          gpointer origin_tag,
                                          const gchar* const* items)
{
    ...
    for (i = 0; settings->priv->path[i] && settings->priv->path[i] == path[i];
         i++)
        ;
    ...
    if (path[i] == '\0')
    {
        ...
        if (l > 0)
            g_signal_emit(settings, g_settings_signals[SIGNAL_CHANGE_EVENT], 0,
                          quarks, l, &ignore_this);
    }
}

static void settings_backend_writable_changed(GObject* target,
                                              GSettingsBackend* backend,
                                              const gchar* key)
{
    GSettings* settings = G_SETTINGS(target);
    ...
    if (settings->priv->path[i] == '\0' &&
        ...
        g_signal_emit(settings,
                      g_settings_signals[SIGNAL_WRITABLE_CHANGE_EVENT], 0,
                      g_quark_from_string(key + i), &ignore_this);
}

static void settings_backend_path_writable_changed(GObject* target,
                                                   GSettingsBackend* backend,
                                                   const gchar* path)
{
    GSettings* settings = G_SETTINGS(target);
    ...
    if (g_str_has_prefix(settings->priv->path, path))
        g_signal_emit(settings,
                      g_settings_signals[SIGNAL_WRITABLE_CHANGE_EVENT], 0,
                      (GQuark)0, &ignore_this);
}

/* Properties, Construction, Destruction {{{1 */
static void g_settings_set_property(GObject* object, guint prop_id,
    ...
    switch (prop_id)
    {
        ...
        case PROP_SCHEMA_ID:
        {
            ...
            if (schema_id != NULL)
            {
                ...
                settings->priv->schema = g_settings_schema_source_lookup(
                    default_source, schema_id, TRUE);
...
static void g_settings_get_property(GObject* object, guint prop_id,
                                    GValue* value, GParamSpec* pspec)
{
    ...
    switch (prop_id)
    {
        ...
        case PROP_SCHEMA_ID:
            g_value_set_string(
                value, g_settings_schema_get_id(settings->priv->schema));
            break;
...
static const GSettingsListenerVTable listener_vtable = {
    settings_backend_changed, settings_backend_path_changed,
    settings_backend_keys_changed, settings_backend_writable_changed,
...
static void g_settings_constructed(GObject* object)
{
    ...
    if (settings->priv->path && schema_path &&
        strcmp(settings->priv->path, schema_path) != 0)
    ...
    g_settings_backend_watch(settings->priv->backend, &listener_vtable,
                             G_OBJECT(settings), settings->priv->main_context);
...
static void g_settings_class_init(GSettingsClass* class)
{
     ...
     */
    g_settings_signals[SIGNAL_CHANGED] = g_signal_new(
        I_("changed"), G_TYPE_SETTINGS, G_SIGNAL_RUN_LAST | G_SIGNAL_DETAILED,
        G_STRUCT_OFFSET(GSettingsClass, changed), NULL, NULL,
    ...
    g_settings_signals[SIGNAL_CHANGE_EVENT] =
        g_signal_new(I_("change-event"), G_TYPE_SETTINGS, G_SIGNAL_RUN_LAST,
    ...
    g_settings_signals[SIGNAL_WRITABLE_CHANGED] =
        g_signal_new(I_("writable-changed"), G_TYPE_SETTINGS,
                     ...
                     G_STRUCT_OFFSET(GSettingsClass, writable_changed), NULL,
     ...
     *          event. FALSE to propagate the event further.
     */
    g_settings_signals[SIGNAL_WRITABLE_CHANGE_EVENT] = g_signal_new(
        I_("writable-change-event"), G_TYPE_SETTINGS, G_SIGNAL_RUN_LAST,
    ...
    g_object_class_install_property(
        ...
        g_param_spec_object("backend", P_("GSettingsBackend"),
     ...
     */
    g_object_class_install_property(
        object_class, PROP_SCHEMA,
        g_param_spec_boxed("settings-schema", P_("schema"),
     ...
     */
    g_object_class_install_property(
        object_class, PROP_SCHEMA_ID,
        g_param_spec_string(
            "schema", P_("Schema name"),
            P_("The name of the schema for this settings object"), NULL,
            G_PARAM_CONSTRUCT_ONLY | G_PARAM_DEPRECATED | G_PARAM_READWRITE |
                G_PARAM_STATIC_STRINGS));
    ...
    g_object_class_install_property(
        ...
        g_param_spec_string(
            "path", P_("Base path"),
            P_("The path within the backend where the settings are"), NULL,
            G_PARAM_CONSTRUCT_ONLY | G_PARAM_READWRITE |
                G_PARAM_STATIC_STRINGS));
     ...
     */
    g_object_class_install_property(
        object_class, PROP_HAS_UNAPPLIED,
        g_param_spec_boolean(
            "has-unapplied", P_("Has unapplied changes"),
            P_("TRUE if there are outstanding changes to apply()"), FALSE,
     ...
     */
    g_object_class_install_property(
        object_class, PROP_DELAY_APPLY,
        g_param_spec_boolean(
            "delay-apply", P_("Delay-apply mode"),
            P_("Whether this settings object is in “delay-apply” mode"), FALSE,
            G_PARAM_READABLE | G_PARAM_STATIC_STRINGS));
}
...
GSettings* g_settings_new(const gchar* schema_id)
{
    ...
    return g_object_new(G_TYPE_SETTINGS, "schema-id", schema_id, NULL);
}

static gboolean path_is_valid(const gchar* path)
{
    if (!path)
        return FALSE;
...
GSettings* g_settings_new_with_path(const gchar* schema_id, const gchar* path)
{
    ...
    return g_object_new(G_TYPE_SETTINGS, "schema-id", schema_id, "path", path,
                        NULL);
}
                                       ...
                                       GSettingsBackend* backend)
{
    ...
    return g_object_new(G_TYPE_SETTINGS, "schema-id", schema_id, "backend",
                        backend, NULL);
}
                               ...
                               GSettingsBackend* backend, const gchar* path)
{
    ...
    g_return_val_if_fail(backend == NULL || G_IS_SETTINGS_BACKEND(backend),
                         NULL);
    g_return_val_if_fail(path == NULL || path_is_valid(path), NULL);

    return g_object_new(G_TYPE_SETTINGS, "settings-schema", schema, "backend",
                        backend, "path", path, NULL);
}
...
/* @value will be sunk */
static gboolean g_settings_write_to_backend(GSettings* settings,
...
static GVariant* g_settings_read_from_backend(GSettings* settings,
                                              GSettingsSchemaKey* key,
                                              ...
                                              gboolean default_value)
{
    ...
    path = g_strconcat(settings->priv->path, key->name, NULL);
    if (user_value_only)
        value = g_settings_backend_read_user_value(settings->priv->backend,
                                                   path, key->type);
    else
        value = g_settings_backend_read(settings->priv->backend, path,
...
gboolean g_settings_set_enum(GSettings* settings, const gchar* key, gint value)
{
    ...
    success =
        g_settings_write_to_backend(settings, &skey, g_steal_pointer(&variant));
    g_settings_schema_key_clear(&skey);
 ...
 * Returns: %TRUE, if the set succeeds
 **/
gboolean g_settings_set_flags(GSettings* settings, const gchar* key,
                              guint value)
{
    GSettingsSchemaKey skey;
    GVariant* variant;
 ...
 * Since: 2.26
 **/
gboolean g_settings_set_value(GSettings* settings, const gchar* key,
                              GVariant* value)
{
    GSettingsSchemaKey skey;
    gboolean success;
    ...
    {
        g_critical("g_settings_set_value: key '%s' in '%s' expects type '%s', "
                   "but a GVariant of type '%s' was given",
                    ...
                    ...)
{
    ...
    if (strchr(format, '&'))
    {
        g_critical("%s: the format string may not contain '&' (key '%s' from "
                   "schema '%s'). "
 ...
 * Since: 2.26
 */
gboolean g_settings_set(GSettings* settings, const gchar* key,
                        const gchar* format, ...)
{
    GVariant* value;
    va_list ap;
 ...
 **/
gpointer g_settings_get_mapped(GSettings* settings, const gchar* key,
 ...
 * Since: 2.26
 */
gboolean g_settings_set_string(GSettings* settings, const gchar* key,
                               const gchar* value)
{
 ...
 * Since: 2.50
 */
gboolean g_settings_set_int64(GSettings* settings, const gchar* key,
                              gint64 value)
{
 ...
 * Since: 2.50
 */
gboolean g_settings_set_uint64(GSettings* settings, const gchar* key,
                               guint64 value)
{
 ...
 * Since: 2.26
 */
gboolean g_settings_set_double(GSettings* settings, const gchar* key,
                               gdouble value)
{
 ...
 * Since: 2.26
 */
gboolean g_settings_set_boolean(GSettings* settings, const gchar* key,
                                gboolean value)
{
 ...
 * @settings: a #GSettings object
 * @key: the name of the key to set
 * @value: (nullable) (array zero-terminated=1): the value to set it to, or
 * %NULL
 *
 * Sets @key in @settings to @value.
 *
 ...
 * Since: 2.26
 */
gboolean g_settings_set_strv(GSettings* settings, const gchar* key,
...
void g_settings_delay(GSettings* settings)
{
    ...
    settings->priv->delayed = g_delayed_settings_backend_new(
        settings->priv->backend, settings, settings->priv->main_context);
...
GSettings* g_settings_get_child(GSettings* settings, const gchar* name)
{
    ...
    child_schema =
        g_settings_schema_get_string(settings->priv->schema, child_name);
    ...
    child = g_object_new(G_TYPE_SETTINGS, "backend", settings->priv->backend,
                         "schema-id", child_schema, "path", child_path, NULL);
    g_free(child_path);
    g_free(child_name);
 ...
 **/
gboolean g_settings_range_check(GSettings* settings, const gchar* key,
                                GVariant* value)
{
    GSettingsSchemaKey skey;
    gboolean good;
...
static void g_settings_binding_free(gpointer data)
{
    ...
    if (binding->key_handler_id)
        g_signal_handler_disconnect(binding->settings, binding->key_handler_id);
...
static void g_settings_binding_key_changed(GSettings* settings,
...
static void g_settings_binding_property_changed(GObject* object,
                                                ...
                                                gpointer user_data)
{
                                        ...
                                        binding->user_data)))
    {
        ...
        if (valid && !g_settings_schema_key_range_check(&binding->key, variant))
        {
            ...
            g_critical(
                "GObject property '%s' on a '%s' object is out of "
                "schema-specified range for key '%s' of '%s': %s",
                binding->property->name,
                g_type_name(binding->property->owner_type), binding->key.name,
                g_settings_schema_get_id(binding->key.schema), variant_str);
            g_free(variant_str);
            valid = FALSE;
        }
...
static gboolean g_settings_bind_invert_boolean_get_mapping(GValue* value,
                                                           GVariant* variant,
                                                           gpointer user_data)
{
...
static GVariant* g_settings_bind_invert_boolean_set_mapping(
 ...
 * Since: 2.26
 */
void g_settings_bind(GSettings* settings, const gchar* key, gpointer object,
                     const gchar* property, GSettingsBindFlags flags)
{
 ...
 * Since: 2.26
 */
void g_settings_bind_with_mapping(GSettings* settings, const gchar* key,
                                  ...
                                  gpointer user_data, GDestroyNotify destroy)
{
    ...
    {
        g_critical("g_settings_bind: no property '%s' on class '%s'", property,
        ...
        (binding->property->flags & G_PARAM_WRITABLE) == 0)
    {
        g_critical("g_settings_bind: property '%s' on class '%s' is not "
                   "writable",
                   binding->property->name, G_OBJECT_TYPE_NAME(object));
        return;
    }
                                               ...
                                               binding->key.type))
    {
        ...
        g_critical("g_settings_bind: property '%s' on class '%s' has type "
                   ...
                   "on schema '%s'",
                   binding->property->name, G_OBJECT_TYPE_NAME(object),
                   g_type_name(binding->property->value_type), type_string, key,
                   g_settings_schema_get_id(settings->priv->schema));
        g_free(type_string);
        return;
    }
        ...
        (~flags & G_SETTINGS_BIND_NO_SENSITIVITY))
    {
        ...
        if (sensitive && sensitive->value_type == G_TYPE_BOOLEAN &&
            ...
            g_settings_bind_writable(settings, binding->key.name, object,
                                     "sensitive", FALSE);
    }
    ...
    {
        detailed_signal =
            g_strdup_printf("notify::%s", binding->property->name);
        binding->property_handler_id = g_signal_connect(
            object, detailed_signal,
            G_CALLBACK(g_settings_binding_property_changed), binding);
        g_free(detailed_signal);

        if (~flags & G_SETTINGS_BIND_GET)
            g_settings_binding_property_changed(object, binding->property,
                                                binding);
    }
    ...
    if (flags & G_SETTINGS_BIND_GET)
    {
        if (~flags & G_SETTINGS_BIND_GET_NO_CHANGES)
        {
            ...
            binding->key_handler_id = g_signal_connect(
                settings, detailed_signal,
                G_CALLBACK(g_settings_binding_key_changed), binding);
            g_free(detailed_signal);
        }
    ...
    g_object_set_qdata_full(object, binding_quark, binding,
...
static void g_settings_binding_writable_changed(GSettings* settings,
                                                const gchar* key,
                                                gpointer user_data)
{
 ...
 * Since: 2.26
 */
void g_settings_bind_writable(GSettings* settings, const gchar* key,
                              gpointer object, const gchar* property,
                              gboolean inverted)
{
    ...
    {
        g_critical("g_settings_bind_writable: property '%s' on class '%s' is "
                   "not writable",
                   property, G_OBJECT_TYPE_NAME(object));
        return;
    }
    ...
    binding->handler_id = g_signal_connect(
        settings, detailed_signal,
        G_CALLBACK(g_settings_binding_writable_changed), binding);
    g_free(detailed_signal);
...
static const gchar* g_settings_action_get_name(GAction* action)
{
...
static const GVariantType* g_settings_action_get_parameter_type(GAction* action)
{
    GSettingsAction* gsa = (GSettingsAction*)action;
...
static gboolean g_settings_action_get_enabled(GAction* action)
{
    GSettingsAction* gsa = (GSettingsAction*)action;
...
static const GVariantType* g_settings_action_get_state_type(GAction* action)
{
    GSettingsAction* gsa = (GSettingsAction*)action;
...
static GVariant* g_settings_action_get_state(GAction* action)
{
    GSettingsAction* gsa = (GSettingsAction*)action;
    GVariant* value;

    value =
        g_settings_read_from_backend(gsa->settings, &gsa->key, FALSE, FALSE);
...
static GVariant* g_settings_action_get_state_hint(GAction* action)
{
    GSettingsAction* gsa = (GSettingsAction*)action;
...
static void g_settings_action_change_state(GAction* action, GVariant* value)
{
    GSettingsAction* gsa = (GSettingsAction*)action;

    if (g_settings_schema_key_type_check(&gsa->key, value) &&
        g_settings_schema_key_range_check(&gsa->key, value))
        g_settings_write_to_backend(gsa->settings, &gsa->key, value);
}

static void g_settings_action_activate(GAction* action, GVariant* parameter)
{
    GSettingsAction* gsa = (GSettingsAction*)action;
...
static void g_settings_action_get_property(GObject* object, guint prop_id,
                                           GValue* value, GParamSpec* pspec)
{
...
static void g_settings_action_finalize(GObject* object)
{
    ...
    G_OBJECT_CLASS(g_settings_action_parent_class)->finalize(object);
}

static void g_settings_action_init(GSettingsAction* gsa)
{
}
...
static void g_settings_action_class_init(GSettingsActionClass* class)
{
    ...
    g_object_class_override_property(class, ACTION_PROP_PARAMETER_TYPE,
                                     "parameter-type");
    g_object_class_override_property(class, ACTION_PROP_ENABLED, "enabled");
    g_object_class_override_property(class, ACTION_PROP_STATE_TYPE,
                                     "state-type");
    g_object_class_override_property(class, ACTION_PROP_STATE, "state");
}

static void g_settings_action_changed(GSettings* settings, const gchar* key,
                                      gpointer user_data)
{
    g_object_notify(user_data, "state");
}

static void g_settings_action_enabled_changed(GSettings* settings,
                                              const gchar* key,
                                              gpointer user_data)
{
...
GAction* g_settings_create_action(GSettings* settings, const gchar* key)
{
    ...
    g_signal_connect(settings, detailed_signal,
                     G_CALLBACK(g_settings_action_changed), gsa);
    g_free(detailed_signal);
    detailed_signal = g_strdup_printf("writable-changed::%s", key);
    g_signal_connect(settings, detailed_signal,
                     G_CALLBACK(g_settings_action_enabled_changed), gsa);
    g_free(detailed_signal);
