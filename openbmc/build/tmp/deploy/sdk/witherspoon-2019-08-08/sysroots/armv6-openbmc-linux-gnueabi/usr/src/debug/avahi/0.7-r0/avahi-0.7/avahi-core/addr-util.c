...
AvahiAddress* avahi_address_from_sockaddr(const struct sockaddr* sa,
                                          AvahiAddress* ret_addr)
{
    ...
    if (sa->sa_family == AF_INET)
        memcpy(&ret_addr->data.ipv4, &((const struct sockaddr_in*)sa)->sin_addr,
               sizeof(ret_addr->data.ipv4));
    else
        memcpy(&ret_addr->data.ipv6,
               &((const struct sockaddr_in6*)sa)->sin6_addr,
               sizeof(ret_addr->data.ipv6));
...
int avahi_address_is_ipv4_in_ipv6(const AvahiAddress* a)
{
    ...
    static const uint8_t ipv4_in_ipv6[] = {0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
    ...
    return memcmp(a->data.ipv6.address, ipv4_in_ipv6, sizeof(ipv4_in_ipv6)) ==
           0;
}
...
int avahi_address_is_link_local(const AvahiAddress* a)
{
    ...
    {
        unsigned n =
            (a->data.ipv6.address[0] << 8) | (a->data.ipv6.address[1] << 0);
