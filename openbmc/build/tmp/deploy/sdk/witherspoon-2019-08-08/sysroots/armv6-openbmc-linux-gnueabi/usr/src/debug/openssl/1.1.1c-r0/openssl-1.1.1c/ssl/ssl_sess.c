...
SSL_SESSION* ssl_session_dup(SSL_SESSION* src, int ticket)
{
    ...
    {
        dest->ext.supportedgroups = OPENSSL_memdup(
            src->ext.supportedgroups,
            src->ext.supportedgroups_len * sizeof(*src->ext.supportedgroups));
    ...
    if (src->ext.alpn_selected != NULL)
    {
        dest->ext.alpn_selected =
            OPENSSL_memdup(src->ext.alpn_selected, src->ext.alpn_selected_len);
 ...
 *     existing session if need be) and s->verify_result is set from the
 * session.
