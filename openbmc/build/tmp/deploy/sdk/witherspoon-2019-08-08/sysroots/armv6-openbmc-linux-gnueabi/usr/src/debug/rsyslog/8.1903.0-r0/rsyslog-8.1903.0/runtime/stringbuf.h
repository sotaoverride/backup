...
typedef struct cstr_s
{
    ...
    sbool isFinalized;
#endif
    uchar* pBuf;     /**< pointer to the string buffer, may be NULL if string is
                        empty */
...
#define cstrFinalize(pThis)                                                    \
    {                                                                          \
        if ((pThis)->iStrLen > 0)                                              \
            (pThis)->pBuf[(pThis)->iStrLen] =                                  \
                '\0'; /* space is always reserved for this */                  \
    }
#else
#define cstrFinalize(pThis)                                                    \
    {                                                                          \
 ...
 */
rsRetVal rsCStrAppendStrWithLen(cstr_t* pThis, const uchar* psz,
                                size_t iStrLen);

/**
 * Append a printf-style formated string to the buffer.
 *
 * \param fmt pointer to the format string (see man 3 printf for details). Must
 * not be NULL.
 */
