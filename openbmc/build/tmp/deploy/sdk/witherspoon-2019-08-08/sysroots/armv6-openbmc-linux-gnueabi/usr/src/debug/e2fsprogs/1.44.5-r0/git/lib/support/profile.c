...
#ifdef DEBUG_PROGRAM
static errcode_t profile_write_tree_file(struct profile_node* root,
                                         FILE* dstfile);

static errcode_t profile_write_tree_to_buffer(struct profile_node* root,
                                              char** buf);
#endif

static void profile_free_node(struct profile_node* relation);

static errcode_t profile_create_node(const char* name, const char* value,
                                     struct profile_node** ret_node);

#ifdef DEBUG_PROGRAM
static errcode_t profile_verify_node(struct profile_node* node);
#endif

static errcode_t profile_add_node(struct profile_node* section,
                                  const char* name, const char* value,
                                  struct profile_node** ret_node);

static errcode_t profile_find_node(struct profile_node* section,
                                   const char* name, const char* value,
                                   int section_flag, void** state,
                                   struct profile_node** node);

static errcode_t profile_node_iterator(void** iter_p,
                                       struct profile_node** ret_node,
                                       char** ret_name, char** ret_value);

static errcode_t profile_open_file(const char* file, prf_file_t* ret_prof);

static errcode_t profile_update_file(prf_file_t prf);

static void profile_free_file(prf_file_t profile);
...
errcode_t profile_init(const char* const* files, profile_t* ret_profile)
{
    const char* const* fs;
    profile_t profile;
    ...
    if (files)
    {
        for (fs = files; !PROFILE_LAST_FILESPEC(*fs); fs++)
        {
            ...
            else if ((retval != ENOTDIR) && strcmp(*fs, default_filename))
                goto errout;
...
errcode_t profile_set_default(profile_t profile, const char* def_string)
{
    ...
    while (*in)
    {
        ...
        errout:
            if (syntax_err_cb)
                (syntax_err_cb)(prf->filespec, retval, state.line_num);
            free(line);
            if (prf->root)
                profile_free_node(prf->root);
            return retval;
...
errcode_t profile_open_file(const char* filespec, prf_file_t* ret_prof)
{
    prf_file_t prf;
    errcode_t retval;
    char* home_env = 0;
...
static errcode_t parse_line(char* line, struct parse_state* state)
{
    ...
    if (ch == '[')
    {
        ...
        retval = profile_find_node(state->root_section, cp, 0, 1, &iter,
                                   &state->current_section);
        if (retval == PROF_NO_SECTION)
        {
            retval = profile_add_node(state->root_section, cp, 0,
                                      &state->current_section);
            if (retval)
                return retval;
        }
    ...
    *cp = '\0';
    ...
    if (do_subsection)
    {
        ...
        if (p)
            *p = '\0';
        retval = profile_add_node(state->current_section, tag, 0,
                                  &state->current_section);
        if (retval)
            return retval;
        if (p)
...
errcode_t profile_write_tree_to_buffer(struct profile_node* root, char** buf)
{
...
errcode_t profile_iterator_create(profile_t profile, const char* const* names,
                                ...
                                char** ret_name, char** ret_value)
{
     ...
     */
    if (iter->node &&
        (iter->file && iter->file->upd_serial != iter->file_serial))
    {
...
get_new_file:
    if (iter->node == 0)
    {
        if (iter->file == NULL || (iter->flags & PROFILE_ITER_FINAL_SEEN))
        {
    ...
    for (p = iter->node; p; p = p->next)
    {
        if (iter->name && strcmp(p->name, iter->name))
            continue;
        if ((iter->flags & PROFILE_ITER_SECTIONS_ONLY) && p->value)
            continue;
        if ((iter->flags & PROFILE_ITER_RELATIONS_ONLY) && !p->value)
            continue;
        if (skip_num > 0)
        {
            skip_num--;
...
errcode_t profile_get_uint(profile_t profile, const char* name,
                           const char* subname, const char* subsubname,
                           unsigned int def_val, unsigned int* ret_int)
{
    const char* value;
    errcode_t retval;
    char* end_value;
...
errcode_t profile_get_double(profile_t profile, const char* name,
                             const char* subname, const char* subsubname,
                             double def_val, double* ret_double)
{
    const char* value;
    errcode_t retval;
    char* end_value;
...
static const char* const conf_no[] = {
    "n", "no", "false", "nil", "0", "off", 0,
};

static errcode_t profile_parse_boolean(const char* s, int* ret_boolean)
{
    const char* const* p;
...
errcode_t profile_iterator(void** iter_p, char** ret_name, char** ret_value)
{
    char *name, *value;
    errcode_t retval;
...
static void do_cmd(profile_t profile, char** argv)
{
    ...
    else if (!strcmp(cmd, "query1"))
    {
        ...
        {
            fprintf(stderr, "Only 3 levels are allowed with query1\n");
            retval = EINVAL;
        }
        else
            retval =
                profile_get_value(profile, name, subname, subsubname, &value);
    ...
    {
        retval = profile_write_tree_file(profile->first_file->root, stdout);
#if 0
    ...
    {
        retval = profile_verify_node(profile->first_file->root);
#if 0
