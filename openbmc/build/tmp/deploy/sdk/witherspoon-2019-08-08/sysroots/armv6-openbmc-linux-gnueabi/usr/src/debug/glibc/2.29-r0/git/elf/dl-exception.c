...
static void __attribute__((noreturn)) length_mismatch(void)
{
                          ...
                          const char* errstring)
{
    ...
    if (errstring_copy != NULL)
    {
        ...
        exception->objname =
            memcpy(__mempcpy(errstring_copy, errstring, len_errstring), objname,
                   len_objname);
...
rtld_hidden_def(_dl_exception_create)
    ...
    void _dl_exception_create_format(struct dl_exception* exception,
                                     const char* objname, const char* fmt, ...)
{
    if (objname == NULL)
        objname = "";
    size_t len_objname = strlen(objname) + 1;
            ...
            if (*p == '%')
            {
                ...
                switch (*p)
                {
                    case 's':
                        ...
                        break;
                        /* Recognize the l modifier.  It is only important on
                       some platforms where long and int have a different size.
                       We can use the same code for size_t.  */
                    case 'l':
                    case 'z':
                        if (p[1] == 'x')
                        {
...
rtld_hidden_def(_dl_exception_create_format)
    ...
    void _dl_exception_free(struct dl_exception* exception)
{
