...
cid_load_glyph(T1_Decoder decoder, FT_UInt glyph_index)
{
    ...
    /* and charstring offset from the CIDMap.                */
    {
        ...
        if (fd_select >= (FT_ULong)cid->num_dicts || off2 > stream->size ||
            off1 > off2)
        {
        ...
        if (FT_ALLOC(charstring, glyph_length))
            goto Exit;
        if (FT_STREAM_READ_AT(cid->data_offset + off1, charstring,
                              glyph_length))
            goto Exit;
    }
    ...
    /* Now set up the subrs array and parse the charstrings. */
    {
        ...
        if (((PS_Driver)FT_FACE_DRIVER(face))->hinting_engine ==
            ...
            error = psaux->t1_decoder_funcs->parse_charstrings_old(
                decoder, charstring + cs_offset, glyph_length - cs_offset);
#else
        ...
        else
        {
            ...
            psaux->t1_make_subfont(FT_FACE(face), &dict->private_dict,
                                   &subfont);
            psdecoder.current_subfont = &subfont;

            error = psaux->t1_decoder_funcs->parse_charstrings(
                &psdecoder, charstring + cs_offset, glyph_length - cs_offset);
            ...
            if (FT_ERR_EQ(error, Glyph_Too_Big))
            {
                ...
                error = psaux->t1_decoder_funcs->parse_charstrings(
                    &psdecoder, charstring + cs_offset,
    ...
    if (!error && inc && inc->funcs->get_glyph_metrics)
    {
        ...
        error = inc->funcs->get_glyph_metrics(inc->object, glyph_index, FALSE,
                                              &metrics);
                    ...
                    FT_UInt glyph_index, FT_Int32 load_flags)
{
    ...
    error = psaux->t1_decoder_funcs->init(
        &decoder, cidglyph->face, cidsize, cidglyph, 0, /* glyph names -- XXX */
        0,                                              /* blend == 0 */
        hinting, FT_LOAD_TARGET_MODE(load_flags), cid_load_glyph);
    if (error)
        goto Exit;
    ...
    if (load_flags & FT_LOAD_NO_RECURSE)
    {
        ...
        cidglyph->metrics.horiAdvance = FIXED_TO_INT(decoder.builder.advance.x);
    ...
    else
    {
        ...
        metrics->horiAdvance = FIXED_TO_INT(decoder.builder.advance.x);
        cidglyph->linearHoriAdvance = FIXED_TO_INT(decoder.builder.advance.x);
        cidglyph->internal->glyph_transformed = 0;

        /* make up vertical ones */
        metrics->vertAdvance =
            (face->cid.font_bbox.yMax - face->cid.font_bbox.yMin) >> 16;
        ...
        {
            FT_Outline_Translate(&cidglyph->outline, font_offset.x,
                                 font_offset.y);
