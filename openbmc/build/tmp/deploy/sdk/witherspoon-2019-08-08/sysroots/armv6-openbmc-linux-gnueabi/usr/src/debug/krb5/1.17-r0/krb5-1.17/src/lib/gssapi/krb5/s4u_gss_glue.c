...
static int kg_is_initiator_cred(krb5_gss_cred_id_t cred)
{
    return (cred->usage == GSS_C_INITIATE || cred->usage == GSS_C_BOTH) &&
           (cred->ccache != NULL);
}

static OM_uint32 kg_impersonate_name(OM_uint32* minor_status,
                                     ...
                                     OM_uint32* time_rec, krb5_context context)
{
    ...
    *output_cred = NULL;
    ...
    {
        code = krb5_authdata_export_authdata(
            context, user->ad_context, AD_USAGE_TGS_REQ, &in_creds.authdata);
        if (code != 0)
        {
    ...
    code = krb5_get_credentials_for_user(
        context, KRB5_GC_CANONICALIZE | KRB5_GC_NO_STORE,
        impersonator_cred->ccache, &in_creds, NULL, &out_creds);
    if (code != 0)
    {
    ...
    major_status =
        kg_compose_deleg_cred(minor_status, impersonator_cred, out_creds,
                              time_req, output_cred, time_rec, context);
    ...
    OM_uint32* time_rec)
{
    ...
    *output_cred_handle = GSS_C_NO_CREDENTIAL;
    ...
    major_status =
        kg_cred_resolve(minor_status, context, impersonator_cred_handle, NULL);
    ...
    major_status = kg_impersonate_name(minor_status, imp_cred,
                                       (krb5_gss_name_t)desired_name, time_req,
                                       &cred, time_rec, context);
 ...
 */
static krb5_error_code make_proxy_cred(krb5_context context,
                                       krb5_gss_cred_id_t cred,
                                       krb5_gss_cred_id_t impersonator_cred)
{
    ...
    code = krb5_cc_copy_creds(context, impersonator_cred->ccache, cred->ccache);
    if (code)
        return code;
