...
static void check_flag(struct rtnl_link* link, struct nlattr* attrs[], int type,
                       int flag)
{
...
static int bridge_parse_af_full(struct rtnl_link* link,
                                struct nlattr* attr_full, void* data)
{
    struct bridge_data* bd = data;
                                ...
                                struct nl_dump_params* p, void* data)
{
    ...
    if (bd->ce_mask & BRIDGE_ATTR_FLAGS)
    {
        ...
        rtnl_link_bridge_flags2str(bd->b_flags & bd->b_flags_mask, buf,
                                   sizeof(buf));
        nl_dump(p, "%s", buf);
    }
...
#define BRIDGE_DIFF(ATTR, EXPR) ATTR_DIFF(attrs, BRIDGE_ATTR_##ATTR, a, b, EXPR)
    ...
    diff |=
        BRIDGE_DIFF(PORT_VLAN, memcmp(&a->vlan_info, &b->vlan_info,
    ...
    if (flags & LOOSE_COMPARISON)
        diff |= BRIDGE_DIFF(FLAGS, (a->b_flags ^ b->b_flags) & b->b_flags_mask);
    else
...
{
    return link->l_family == AF_BRIDGE && link->l_af_ops == &bridge_ops;
}
...
static const struct trans_tbl port_states[] = {
    __ADD(BR_STATE_DISABLED, disabled), __ADD(BR_STATE_LISTENING, listening),
    __ADD(BR_STATE_LEARNING, learning), __ADD(BR_STATE_FORWARDING, forwarding),
...
struct rtnl_link_bridge_vlan*
    rtnl_link_bridge_get_port_vlan(struct rtnl_link* link)
{
