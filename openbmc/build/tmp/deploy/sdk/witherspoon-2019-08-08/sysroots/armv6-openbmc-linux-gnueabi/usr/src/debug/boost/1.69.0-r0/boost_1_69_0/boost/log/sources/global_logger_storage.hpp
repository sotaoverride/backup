...
struct logger_holder_base
{
    ...
    logger_holder_base(const char* file, unsigned int line,
                       typeindex::type_index logger_type) BOOST_NOEXCEPT
...
struct logger_holder : public logger_holder_base
{
    //! The logger instance
    LoggerT m_Logger;
...
struct global_storage
{
    ...
    BOOST_LOG_API static shared_ptr<logger_holder_base>
        get_or_init(typeindex::type_index key, initializer_t initializer);
...
BOOST_LOG_API BOOST_LOG_NORETURN void
    throw_odr_violation(typeindex::type_index tag_type,
          ...
          shared_ptr<logger_holder<typename TagT::logger_type>>>
{
    ...
    {
        shared_ptr<logger_holder<logger_type>>& instance =
        ...
        shared_ptr<logger_holder_base> holder = global_storage::get_or_init(
            tag_type_index, &logger_singleton::construct_logger);
        const typeindex::type_index logger_type_index =
            typeindex::type_id<logger_type>();
        if (holder->m_LoggerType == logger_type_index)
        {
            // Note: dynamic_cast may fail here if logger_type is not visible
            // (for example, with Clang on Linux, if the original logger
            //       instance was initialized in a different DSO than where it's
            //       being queried). logger_holder visibility doesn't have
            //       effect since it is inhibited by the template parameter
            //       visibility.
            instance =
                boost::static_pointer_cast<logger_holder<logger_type>>(holder);
        }
        else
        {
            // In pure C++ this should never happen, since there cannot be two
            // different tag types that have equal type_infos. In real life it
            // can happen if the same-named tag is defined differently in two or
            // more dlls. This check is intended to detect such ODR violations.
            // However, there is no protection against different definitions of
            // the logger type itself.
            boost::log::sources::aux::throw_odr_violation(
                tag_type_index, logger_type_index, *holder);
        }
    }
...
#define BOOST_LOG_GLOBAL_LOGGER(tag_name, logger)                              \
    ...
    {                                                                          \
        ...
        {                                                                      \
            return ::boost::log::sources::aux::logger_singleton<               \
                tag_name>::get();                                              \
        }                                                                      \
    };
...
//! The macro defines a global logger initializer that will default-construct
//! the logger
...
//! The macro defines a global logger initializer that will construct the logger
...
//! The macro declares a global logger that will be constructed with the
//! specified arguments
