...
/** \file rr.h Functions and definitions for manipulating DNS resource record
 * (RR) data. */
...
{
    AVAHI_DNS_TYPE_ANY =
        0xFF, /**< Special query type for requesting all records */
...
enum
{
    ...
    AVAHI_DNS_CACHE_FLUSH =
        0x8000, /**< Not really a class but a bit which may be set in response
                   packets, see mDNS spec for more information */
    AVAHI_DNS_UNICAST_RESPONSE =
        0x8000 /**< Not really a class but a bit which may be set in query
                  packets, see mDNS spec for more information */
};

/** Encapsulates a DNS query key consisting of class, type and
    name. Use avahi_key_ref()/avahi_key_unref() for manipulating the
    reference counter. The structure is intended to be treated as "immutable",
   no changes should be imposed after creation */
typedef struct AvahiKey
{
    int ref;    /**< Reference counter */
    char* name; /**< Record name */
    uint16_t
        clazz; /**< Record class, one of the AVAHI_DNS_CLASS_xxx constants */
...
/** Create a new record object. Record data should be filled in right after
 * creation. The reference counter is set to 1. */
AvahiRecord* avahi_record_new(AvahiKey* k, uint32_t ttl);

/** Create a new record object. Record data should be filled in right after
 * creation. The reference counter is set to 1. */
AvahiRecord* avahi_record_new_full(const char* name, uint16_t clazz,
...
/** Parse a binary rdata object and fill it into *record. This function is
 * actually implemented in dns.c */
int avahi_rdata_parse(AvahiRecord* record, const void* rdata, size_t size);

/** Serialize an AvahiRecord object into binary rdata. This function is actually
 * implemented in dns.c */
