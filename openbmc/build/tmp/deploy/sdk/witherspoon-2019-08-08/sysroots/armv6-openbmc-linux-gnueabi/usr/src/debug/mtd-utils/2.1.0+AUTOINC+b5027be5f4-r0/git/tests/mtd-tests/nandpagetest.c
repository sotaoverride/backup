...
static NORETURN void usage(int status)
{
    fputs("Usage: " PROGRAM_NAME " [OPTIONS] <device>\n\n"
          ...
          "  -c, --count <num>  Number of erase blocks to use (at least 2, "
          "default all)\n"
...
static int write_eraseblock(int ebnum)
{
    ...
    return mtd_write(mtd_desc, &mtd, fd, ebnum, 0, writebuf, mtd.eb_size, NULL,
                     0, 0);
}
...
static int flush_data_rams(int eb0, int ebn)
{
    ...
    if (err)
        return err;
    err = mtd_read(&mtd, fd, ebn, mtd.eb_size - bufsize, twopages, bufsize);
    if (err)
        return err;
...
static int verify_eraseblock(int ebnum)
{
    ...
    if (addr <= addrn && !mtd_is_bad(&mtd, fd, ebnum + 1))
    {
        ...
        {
            fprintf(stderr, "cannot seek mtd%d to offset %lld", mtd.mtd_num,
                    (long long)addr);
            return -1;
        }
        ...
        for (rd = 0; rd < bufsize; rd += diff)
        {
            ...
            {
                fprintf(stderr,
                        "cannot read %d bytes from mtd%d "
                        "(eraseblock %d, offset %d)",
                        bufsize - rd, mtd.mtd_num, ebnum, (int)offset + rd);
                return -1;
            }
        }
...
static int erasetest(void)
{
    ...
    for (i = 0; i < pgsize; ++i)
        writebuf[i] = rand_r(&rnd_state);
    err = mtd_write(mtd_desc, &mtd, fd, ebnum, 0, writebuf, pgsize, NULL, 0, 0);
    if (err)
        return err;
...
out:
    ...
    if (flags & KEEP_CONTENTS)
    {
        ...
        for (i = 0; i < ebcnt; ++i)
        {
            ...
            err = mtd_write(mtd_desc, &mtd, fd, eb, 0, backupptr, mtd.eb_size,
                            NULL, 0, 0);
            if (err)
            {
