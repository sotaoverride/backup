...
class severity_level_holder
    : public boost::log::aux::lazy_singleton<
          severity_level_holder, boost::log::aux::thread_specific<uintmax_t*>>
{
};
...
{
    boost::log::aux::thread_specific<uintmax_t*>& tss =
        severity_level_holder::get();
    uintmax_t* p = tss.get();
    if (BOOST_UNLIKELY(!p))
    {
        ...
        boost::this_thread::at_thread_exit(
            boost::bind(checked_deleter<uintmax_t>(), p));
    }
    return *p;
}
