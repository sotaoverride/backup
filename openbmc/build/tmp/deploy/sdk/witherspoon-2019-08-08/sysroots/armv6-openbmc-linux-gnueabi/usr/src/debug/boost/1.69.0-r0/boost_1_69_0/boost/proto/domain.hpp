...
struct domain : Generator
{
    ...
    ///
    /// The <tt>as_expr\<\></tt> function object turns objects into Proto
    /// expressions, if they are not already, by making them Proto terminals
    /// held by value if possible. Objects that are already Proto expressions
    /// are left alone.
    ///
    /// If <tt>wants_basic_expr\<Generator\>::value</tt> is true, then let \c E
    /// be \c basic_expr; otherwise, let \t E be \c expr. Given an lvalue \c t
    /// of type \c T:
    ///
    ...
    ///
    /// If \c T is a Proto expression type and its generator type is different
    /// from \c Generator, the result is <tt>Generator()(t)</tt>.
    ///
    /// Otherwise, the result is \c t converted to an (un-const) rvalue.
    ///
    template <typename T, typename IsExpr = void,
              typename Callable = proto::callable>
    struct as_expr
        : detail::as_expr<T, typename detail::base_generator<Generator>::type,
    ...
    /// expression objects in this domain.
    ///
    /// The <tt>as_child\<\></tt> function object turns objects into Proto
    /// expressions, if they are not already, by making them Proto terminals
    /// held by reference. Objects that are already Proto expressions are simply
    ...
    template <typename T, typename IsExpr = void,
              typename Callable = proto::callable>
    struct as_child
        : detail::as_child<T, typename detail::base_generator<Generator>::type,
                           wants_basic_expr<Generator>::value>
    {
        BOOST_PROTO_CALLABLE()
    };
...
///
struct deduce_domain : domain<detail::not_a_generator, detail::not_a_grammar,
                              detail::not_a_domain>
{
};
