...
static OM_uint32 val_exp_sec_ctx_args(OM_uint32* minor_status,
...
OM_uint32 KRB5_CALLCONV gss_export_sec_context(minor_status, context_handle,
                                               interprocess_token)
...

{
    ...
    status =
        val_exp_sec_ctx_args(minor_status, context_handle, interprocess_token);
    ...
    status = mech->gss_export_sec_context(minor_status, &ctx->internal_ctx_id,
                                          &token);
    if (status != GSS_S_COMPLETE)
    {
