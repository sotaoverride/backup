...
static int loadave_store_config(int a, int b, void* c, void* d)
{
    char line[SNMP_MAXBUF_SMALL];
    if (laConfigSet > 0)
    {
        snprintf(line, SNMP_MAXBUF_SMALL, "pload %.02f %.02f %.02f", maxload[0],
                 maxload[1], maxload[2]);
...
void init_loadave(void)
{
     ...
     * information at
     */
    struct variable2 extensible_loadave_variables[] = {{MIBINDEX,
                                                        ASN_INTEGER,
     ...
     */
    oid loadave_variables_oid[] = {NETSNMP_UCDAVIS_MIB, NETSNMP_LOADAVEMIBNUM,
                                   1};
    ...
    snmpd_register_config_handler("pload", loadave_parse_config, NULL, NULL);
...
#define FIX_TO_DBL(_IN) (((double)_IN) / ((double)FSCALE))
    ...
    DEBUGMSGTL(("ucd-snmp/loadave", "irix6: %d %d %d\n", favenrun[0],
                favenrun[1], favenrun[2]));
#elif !defined(cygwin)
#if defined(NETSNMP_CAN_USE_NLIST) && defined(LOADAVE_SYMBOL)
    if (auto_nlist(LOADAVE_SYMBOL, (char*)r_ave, sizeof(double) * s_ave) == 0)
#endif
        return (-1);
#endif
    /*
...
static int write_laConfig(int action, u_char* var_val, u_char var_val_type,
                          ...
                          size_t name_len)
{
    ...
    switch (action)
    {
        case RESERVE1: /* Check values for acceptability */
            ...
            {
                DEBUGMSGTL(
                    ("ucd-snmp/loadave", "write to laConfig: bad length\n"));
...
u_char* var_extensible_loadave(struct variable* vp, oid* name, size_t* length,
