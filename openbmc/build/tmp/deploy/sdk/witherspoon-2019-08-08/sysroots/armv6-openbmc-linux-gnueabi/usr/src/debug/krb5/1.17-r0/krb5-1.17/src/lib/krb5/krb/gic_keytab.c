...
static krb5_error_code
    get_as_key_keytab(krb5_context context, krb5_principal client,
...
static krb5_error_code lookup_etypes_for_keytab(krb5_context context,
                                                krb5_keytab keytab,
...
static krb5_error_code sort_enctypes(krb5_enctype* req_list, int req_len,
                                     krb5_enctype* keytab_list)
{
    ...
    krb5_context context, krb5_init_creds_context ctx, krb5_keytab keytab)
{
    ...
    ret = sort_enctypes(ctx->request->ktype, ctx->request->nktypes, etype_list);
    free(etype_list);
    return ret;
}
                          ...
                          krb5_get_init_creds_opt* options, int* use_master)
{
    ...
    ret = krb5_init_creds_init(context, client, NULL, NULL, start_time, options,
                               &ctx);
    if (ret != 0)
        goto cleanup;
    ...
    krb5_kdc_rep** ret_as_reply)
{
    ...
    retval = k5_get_init_creds(
        ...
        opts, get_as_key_keytab, (void*)keytab, &use_master, ret_as_reply);
