...
static const GOptionEntry entries[] = {
    ...
    {"direct", 'D', 0, G_OPTION_ARG_FILENAME_ARRAY, &watch_direct,
     N_("Monitor a file directly (notices changes made via hardlinks)"),
     N_("LOCATION")},
    {"silent", 's', 0, G_OPTION_ARG_FILENAME_ARRAY, &watch_silent,
     N_("Monitors a file directly, but doesn’t report changes"),
     N_("LOCATION")},
    {"no-moves", 'n', 0, G_OPTION_ARG_NONE, &no_moves,
     N_("Report moves and renames as simple deleted/created events"), NULL},
    {"mounts", 'm', 0, G_OPTION_ARG_NONE, &mounts, N_("Watch for mount events"),
     NULL},
    {G_OPTION_REMAINING, 0, 0, G_OPTION_ARG_FILENAME_ARRAY, &watch_default},
    {NULL}};

static void watch_callback(GFileMonitor* monitor, GFile* child, GFile* other,
...
static gboolean add_watch(const gchar* cmdline, WatchType watch_type,
                          GFileMonitorFlags flags, gboolean connect_handler)
{
    ...
    if (connect_handler)
        g_signal_connect(monitor, "changed", G_CALLBACK(watch_callback),
                         g_strdup(cmdline));
...
int handle_monitor(int argc, gchar* argv[], gboolean do_help)
{
    ...
    g_option_context_set_summary(
        context, _("Monitor files or directories for changes."));
    ...
    if (!watch_dirs && !watch_files && !watch_direct && !watch_silent &&
        !watch_default)
    {
    ...
    if (watch_direct)
    {
        for (i = 0; watch_direct[i]; i++)
            if (!add_watch(watch_direct[i], WATCH_FILE,
                           flags | G_FILE_MONITOR_WATCH_HARD_LINKS, TRUE))
                return 1;
    }
    ...
    if (watch_silent)
    {
        for (i = 0; watch_silent[i]; i++)
            if (!add_watch(watch_silent[i], WATCH_FILE,
                           flags | G_FILE_MONITOR_WATCH_HARD_LINKS, FALSE))
                return 1;
    }
