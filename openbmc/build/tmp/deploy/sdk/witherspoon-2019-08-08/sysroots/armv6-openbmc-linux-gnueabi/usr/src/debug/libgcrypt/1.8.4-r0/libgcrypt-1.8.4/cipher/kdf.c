...
static gpg_err_code_t openpgp_s2k(const void* passphrase, size_t passphraselen,
...
gpg_err_code_t _gcry_kdf_pkdf2(const void* passphrase, size_t passphraselen,
                               ...
                               void* keybuffer)
{
    ...
    sbuf = (secmode ? xtrymalloc_secure(saltlen + 4 + hlen + hlen)
    ...
    ec = _gcry_md_open(
        &md, hashalgo,
        (GCRY_MD_FLAG_HMAC | (secmode ? GCRY_MD_FLAG_SECURE : 0)));
    if (ec)
    {
        xfree(sbuf);
        return ec;
    ...
    switch (algo)
    {
        ...
        case GCRY_KDF_ITERSALTED_S2K:
            if (!passphraselen)
                ...
                ec = openpgp_s2k(passphrase, passphraselen, algo, subalgo, salt,
        ...
        case GCRY_KDF_PBKDF2:
            if (!saltlen)
                ...
                ec = _gcry_kdf_pkdf2(passphrase, passphraselen, subalgo, salt,
                                     saltlen, iterations, keysize, keybuffer);
            break;
...
#if USE_SCRYPT
            ec =
                _gcry_kdf_scrypt(passphrase, passphraselen, algo, subalgo, salt,
                                 saltlen, iterations, keysize, keybuffer);
#else
            ec = GPG_ERR_UNSUPPORTED_ALGORITHM;
#endif /*USE_SCRYPT*/
