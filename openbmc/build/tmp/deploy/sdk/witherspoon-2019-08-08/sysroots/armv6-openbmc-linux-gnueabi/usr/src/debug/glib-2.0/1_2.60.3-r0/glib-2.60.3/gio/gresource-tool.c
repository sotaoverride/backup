...
static GResource* get_resource(const gchar* file)
{
    gchar* content;
    gsize size;
    GResource* resource;
...
static void list_resource(GResource* resource, const gchar* path,
                          const gchar* section, const gchar* prefix,
                          gboolean details)
{
    gchar** children;
    gsize size;
        ...
        if (g_resource_get_info(resource, child, 0, &size, &flags, NULL))
        {
            if (details)
                g_print("%s%s%6" G_GSIZE_FORMAT " %s %s\n", section,
                        section[0] ? " " : "", size,
                        (flags & G_RESOURCE_FLAGS_COMPRESSED) ? "c" : "u",
                        child);
            else
                g_print("%s\n", child);
        }
...
static void extract_resource(GResource* resource, const gchar* path)
{
    GBytes* bytes;
...
static Elf* get_elf(const gchar* file, gint* fd)
{
    Elf* elf;
...
typedef gboolean (*SectionCallback)(GElf_Shdr* shdr, const gchar* name,
                                    gpointer data);

static void elf_foreach_resource_section(Elf* elf, SectionCallback callback,
                                         gpointer data)
{
...
static GResource* resource_from_section(GElf_Shdr* shdr, int fd)
{
    ...
    contents = mmap(NULL, shdr->sh_size + page_offset, PROT_READ, MAP_PRIVATE,
...
static gboolean list_resources_cb(GElf_Shdr* shdr, const gchar* section,
                                  gpointer data)
{
    CallbackData* d = data;
    GResource* resource;
    ...
    list_resource(resource, "/", d->section ? "" : section, d->path,
                  d->details);
...
static void elf_list_resources(Elf* elf, int fd, const gchar* section,
                               const gchar* path, gboolean details)
{
    CallbackData data;
...
static gboolean extract_resource_cb(GElf_Shdr* shdr, const gchar* section,
                                    gpointer data)
{
    CallbackData* d = data;
    GResource* resource;
...
static void elf_extract_resource(Elf* elf, int fd, const gchar* section,
                                 const gchar* path)
{
    CallbackData data;
...
static gboolean print_section_name(GElf_Shdr* shdr, const gchar* name,
                                   gpointer data)
{
    g_print("%s\n", name);
    return TRUE;
...
static void cmd_sections(const gchar* file, const gchar* section,
                         const gchar* path, gboolean details)
{
    GResource* resource;
...
static void cmd_list(const gchar* file, const gchar* section, const gchar* path,
                     gboolean details)
{
    GResource* resource;
...
static void cmd_extract(const gchar* file, const gchar* section,
                        const gchar* path, gboolean details)
{
    GResource* resource;
...
static gint cmd_help(gboolean requested, const gchar* command)
{
    ...
    else
    {
        g_string_append_printf(string,
                               ...
                               synopsis[0] ? synopsis : "", description);
        ...
        if (strstr(synopsis, _("FILE")))
        {
            if (strcmp(command, "sections") == 0)
                g_string_append(string, _("  FILE      An elf file (a binary "
                                          "or a shared library)\n"));
            else
                g_string_append(
                    string,
        ...
        if (strstr(synopsis, _("[PATH]")))
            g_string_append(string, _("  PATH      An (optional) resource path "
                                      "(may be partial)\n"));
        else if (strstr(synopsis, _("PATH")))
            g_string_append(string, _("  PATH      A resource path\n"));
