    ...
    for (from = 0; from < ndx_count; from++)
    {
        ...
        if (inc_recurse)
        {
            ...
            else if (CVAL(node->data, 0) == 0)
            {
                ...
                if (flist)
                    flist->files[prev - flist->ndx_start]->flags &=
                        ~FLAG_HLINK_LAST;
                else
                {
        ...
        for (; from < ndx_count - 1; file = file_next, gnum = gnum_next, from++)
        { /*SHARED ITERATOR*/
...
static int maybe_hard_link(struct file_struct* file, int ndx, char* fname,
                           int statret, stat_x* sxp, const char* oldname,
                           STRUCT_STAT* old_stp, const char* realname,
                           int itemizing, enum logcode code)
{
                      ...
                      statret == 0 ? DEL_FOR_FILE : 0))
    {
        ...
        {
            itemize(fname, file, ndx, statret, sxp,
                    ITEM_LOCAL_CHANGE | ITEM_XNAME_FOLLOWS, 0, realname);
        }
 ...
 * cached name for it. */
static char* check_prior(struct file_struct* file, int gnum, int* prev_ndx_p,
                         struct file_list** flist_p)
{
    ...
    {
        struct file_list* flist;
        if (prev_ndx < 0 || (flist = flist_for_ndx(prev_ndx, NULL)) == NULL)
            break;
    ...
    if (inc_recurse && (node = hashtable_find(prior_hlinks, gnum, 0)) != NULL)
    {
...
int hard_link_check(struct file_struct* file, int ndx, char* fname, int statret,
                    stat_x* sxp, int itemizing, enum logcode code)
{
    STRUCT_STAT prev_st;
    ...
    if (!prev_name)
    {
        ...
        if (!(prev_file->flags & FLAG_HLINK_FIRST))
        {
            ...
            prev_name = realname =
                check_prior(prev_file, gnum, &prev_ndx, &flist);
        ...
        if (!prev_name)
        {
            ...
            {
                rprintf(FINFO,
                        "hlink for %d (%s,%d): found flist match (alt %d)\n",
    ...
    if (DEBUG_GTE(HLINK, 2))
    {
        rprintf(FINFO, "hlink for %d (%s,%d): leader is %d (%s)\n", ndx,
                f_name(file, NULL), gnum, prev_ndx, prev_name);
    }
                  ...
                  const char* oldname, int terse)
{
    if (do_link(oldname, fname) < 0)
    {
        ...
        if (terse)
        {
            ...
            code = FERROR_XFER;
        rsyserr(code, errno, "link %s => %s failed", full_fname(fname),
                oldname);
        return 0;
    }
                      ...
                      int alt_dest)
{
    ...
    if (alt_dest >= 0 && dry_run)
    {
        pathjoin(alt_name, MAXPATHLEN, basis_dir[alt_dest], f_name(file, NULL));
        our_name = alt_name;
    }
    else
        our_name = fname;
