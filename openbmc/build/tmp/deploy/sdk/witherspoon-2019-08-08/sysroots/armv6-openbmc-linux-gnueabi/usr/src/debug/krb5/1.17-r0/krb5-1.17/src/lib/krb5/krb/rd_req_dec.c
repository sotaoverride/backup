...
static krb5_error_code decrypt_authenticator(krb5_context, const krb5_ap_req*,
                                             krb5_authenticator**, int);
static krb5_error_code decode_etype_list(krb5_context context,
                                         const krb5_authenticator* authp,
                                         krb5_enctype** desired_etypes,
                                         int* desired_etypes_len);
static krb5_error_code
    negotiate_etype(krb5_context context, const krb5_enctype* desired_etypes,
                    int desired_etypes_len, int mandatory_etypes_index,
                    const krb5_enctype* permitted_etypes,
                    int permitted_etypes_len, krb5_enctype* negotiated_etype);

/* Unparse the specified server principal (which may be NULL) and the ticket
 * server principal. */
static krb5_error_code unparse_princs(krb5_context context,
                                      krb5_const_principal server,
                                      krb5_const_principal tkt_server,
                                      char** sname_out, char** tsname_out)
{
    krb5_error_code ret;
...
static krb5_error_code keytab_fetch_error(krb5_context context,
...
static krb5_error_code integrity_error(krb5_context context,
                                       krb5_const_principal server,
                                       krb5_const_principal tkt_server)
{
    krb5_error_code ret;
    ...
    k5_setmsg(context, ret,
              _("Cannot decrypt ticket for %s using keytab key for %s"), tsname,
              sname);
    krb5_free_unparsed_name(context, sname);
    krb5_free_unparsed_name(context, tsname);
    return ret;
...
static krb5_error_code nomatch_error(krb5_context context,
                                     krb5_const_principal server,
                                     krb5_const_principal tkt_server)
{
    krb5_error_code ret;
...
static krb5_error_code
    iteration_error(krb5_context context, krb5_const_principal server,
                    ...
                    krb5_boolean found_server_match,
                    krb5_boolean found_tkt_server, krb5_boolean found_kvno,
                    krb5_boolean found_higher_kvno, krb5_boolean found_enctype)
{
    krb5_error_code ret;
...
static inline krb5_boolean is_matching(krb5_context context,
                                       krb5_const_principal princ)
{
    if (princ == NULL)
        return TRUE;
...
static krb5_error_code try_one_entry(krb5_context context,
 ...
 */
static krb5_error_code decrypt_ticket(krb5_context context,
                                      const krb5_ap_req* req,
                                      krb5_const_principal server,
                                      krb5_keytab keytab,
                                      krb5_keyblock* keyblock_out)
{
    ...
    {
        return try_one_princ(context, req, server, keytab, TRUE, keyblock_out);
    }
                       ...
                       krb5_ticket** ticket, int check_valid_flag)
{
    ...
    { /* User to User authentication */
        if ((retval = krb5_decrypt_tkt_part(
                 context, &(*auth_context)->key->keyblock, req->ticket)))
            goto cleanup;
        if (check_valid_flag)
        {
    ...
    /* XXX this is an evil hack.  check_valid_flag is set iff the call
       ...
       key usage to use */
#ifndef LEAN_CLIENT
    if ((retval = decrypt_authenticator(
             context, req, &((*auth_context)->authentp), check_valid_flag)))
        goto cleanup;
#endif
    ...
    if ((*auth_context)->rcache)
    {
        ...
        {
            retval = krb5_rc_hash_message(
                context, &req->authenticator.ciphertext, &rep.msghash);
            if (!retval)
            {
    ...
    if ((retval =
             krb5_check_clockskew(context, (*auth_context)->authentp->ctime)))
        goto cleanup;
    ...
    if (check_valid_flag)
    {
        ...
        if ((retval = krb5_authdata_context_init(context,
            ...
            goto cleanup;
        if ((retval = krb5int_authdata_verify(
                 context, (*auth_context)->ad_context, AD_USAGE_MASK,
                 auth_context, &decrypt_key, req)))
            goto cleanup;
    }

    /* read RFC 4537 etype list from sender */
    retval = decode_etype_list(context, (*auth_context)->authentp,
                               &desired_etypes, &rfc4537_etypes_len);
    if (retval != 0)
        goto cleanup;
    ...
    if (desired_etypes == NULL)
        ...
        desired_etypes = (krb5_enctype*)realloc(
            desired_etypes, (rfc4537_etypes_len + 4) * sizeof(krb5_enctype));
    if (desired_etypes == NULL)
    {
        retval = ENOMEM;
        goto cleanup;
    ...
    retval = negotiate_etype(context, desired_etypes, desired_etypes_len,
    ...
    if ((*auth_context)->authentp->subkey)
    {
        ...
        if ((retval =
                 krb5_k_create_key(context, (*auth_context)->authentp->subkey,
                        ...
                        krb5_ticket** ticket)
{
    ...
    retval =
        rd_req_decoded_opt(context, auth_context, req, server, keytab,
                           ap_req_options, ticket, 1); /* check_valid_flag */
    return retval;
}

krb5_error_code krb5_rd_req_decoded_anyflag(
    krb5_context context, krb5_auth_context* auth_context,
    const krb5_ap_req* req, krb5_const_principal server, krb5_keytab keytab,
    krb5_flags* ap_req_options, krb5_ticket** ticket)
{
    krb5_error_code retval;
    retval = rd_req_decoded_opt(context, auth_context, req, server, keytab,
...
#ifndef LEAN_CLIENT
static krb5_error_code decrypt_authenticator(krb5_context context,
                                             ...
                                             int is_ap_req)
{
    ...
    if ((retval = krb5_c_decrypt(context, sesskey,
                                 ...
                                 0, &request->authenticator, &scratch)))
    {
        free(scratch.data);
        return (retval);
    }

#define clean_scratch()                                                        \
    {                                                                          \
    ...
    *negotiated_etype = ENCTYPE_NULL;
    ...
    for (i = mandatory_etypes_index; i < desired_etypes_len; i++)
    {
        ...
        if (permitted == FALSE)
        {
            ...
            if (krb5_enctype_to_string(desired_etypes[i], enctype_name,
        ...
        switch (authp->authorization_data[i]->ad_type)
        {
            case KRB5_AUTHDATA_IF_RELEVANT:
                code = krb5_decode_authdata_container(
                    context, KRB5_AUTHDATA_IF_RELEVANT,
                    authp->authorization_data[i], &ad_if_relevant);
                if (code != 0)
                    continue;

                for (j = 0; ad_if_relevant[j] != NULL; j++)
                {
                    if (ad_if_relevant[j]->ad_type ==
