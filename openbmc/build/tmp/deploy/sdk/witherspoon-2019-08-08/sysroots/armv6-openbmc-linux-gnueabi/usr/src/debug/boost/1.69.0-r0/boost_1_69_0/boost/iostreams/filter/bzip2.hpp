...
struct bzip2_params
{
    ...
    bzip2_params(bool small) : small(small), work_factor(0)
    {
    }
...
class bzip2_compressor_impl : public bzip2_base,
                              ...
                              bzip2_allocator<Alloc>
{
  ...
  private:
    void init();
    bool eof_; // Guard to make sure filter() isn't called after it returns
               // false.
};
...
//
template <typename Alloc = std::allocator<char>>
class bzip2_decompressor_impl : public bzip2_base,
#if BOOST_WORKAROUND(__BORLANDC__, < 0x600)
                                public
#endif
                                bzip2_allocator<Alloc>
    ...
    : symmetric_filter<detail::bzip2_compressor_impl<Alloc>, Alloc>
{
  ...
  public:
    ...
    basic_bzip2_compressor(
        const bzip2_params& = bzip2::default_block_size,
    ...
    : symmetric_filter<detail::bzip2_decompressor_impl<Alloc>, Alloc>
{
  ...
  public:
    ...
    basic_bzip2_decompressor(
        bool small = bzip2::default_small,
        std::streamsize buffer_size = default_device_buffer_size);
};
...
template <typename Alloc>
bool bzip2_compressor_impl<Alloc>::filter(const char*& src_begin,
                                          const char* src_end,
...
template <typename Alloc>
bool bzip2_decompressor_impl<Alloc>::filter(const char*& src_begin,
                                            const char* src_end,
                                            char*& dest_begin, char* dest_end,
                                            bool flush)
{
