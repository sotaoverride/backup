...
static void print_status_info(StatusInfo* i)
{
    ...
    if (!isempty(i->os_pretty_name))
    {
        ...
        if (i->home_url)
        {
            if (terminal_urlify(i->home_url, i->os_pretty_name, &formatted) >=
                0)
                t = formatted;
        }
...
static int show_one_name(sd_bus* bus, const char* attr)
{
    ...
    r = sd_bus_get_property(
        bus, "org.freedesktop.hostname1", "/org/freedesktop/hostname1",
        "org.freedesktop.hostname1", attr, &error, &reply, "s");
    if (r < 0)
        return log_error_errno(r, "Could not get property: %s",
...
static int show_all_names(sd_bus* bus, sd_bus_error* error)
{
    ...
    _cleanup_(sd_bus_message_unrefp) sd_bus_message *host_message = NULL,
                                                    *manager_message = NULL;
    int r;

    r = bus_map_all_properties(bus, "org.freedesktop.hostname1",
                               "/org/freedesktop/hostname1", hostname_map, 0,
                               error, &host_message, &info);
    if (r < 0)
        return r;

    r = bus_map_all_properties(bus, "org.freedesktop.systemd1",
                               "/org/freedesktop/systemd1", manager_map, 0,
                               error, &manager_message, &info);
...
static int show_status(int argc, char** argv, void* userdata)
{
    ...
    else
    {
        ...
        if (r < 0)
            return log_error_errno(r, "Failed to query system properties: %s",
                                   bus_error_message(&error, r));
...
static int set_simple_string(sd_bus* bus, const char* method, const char* value)
{
    ...
    r = sd_bus_call_method(bus, "org.freedesktop.hostname1",
                           "/org/freedesktop/hostname1",
                           "org.freedesktop.hostname1", method, &error, NULL,
                           "sb", value, arg_ask_password);
    if (r < 0)
        return log_error_errno(r, "Could not set property: %s",
                               bus_error_message(&error, -r));
...
static int set_hostname(int argc, char** argv, void* userdata)
{
    ...
    if (arg_pretty)
    {
        ...
        /* If the passed hostname is already valid, then assume the user doesn't
         * know anything about pretty hostnames, so let's unset the pretty
         * hostname, and just set the passed hostname as static/dynamic
         * hostname. */
        if (arg_static && hostname_is_valid(hostname, true))
            p = ""; /* No pretty hostname (as it is redundant), just a static
                       one */
        else
        ...
        /* Now that we set the pretty hostname, let's clean up the parameter and
         * use that as static hostname. If the hostname was already valid as
         * static hostname, this will only chop off the trailing dot if there is
         * one. If it was not valid, then it will be made fully valid by
         * truncating, dropping multiple dots, and dropping weird chars. Note
         * that we clean the name up only if we also are supposed to set the
         * pretty name. If the pretty name is not being set we assume the user
         * knows what he does and pass the name as-is. */
        h = strdup(hostname);
        if (!h)
            return log_oom();

        hostname = hostname_cleanup(
            h); /* Use the cleaned up name as static hostname */
    }
