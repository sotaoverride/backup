...
extern inline int __aio_create_helper_thread(pthread_t* threadp,
...
static struct requestlist* get_elem(void)
{
    ...
    if (freelist == NULL)
    {
        ...
        if (pool_size + 1 >= pool_max_size)
        {
            ...
            new_tab = (struct requestlist**)realloc(
                pool, new_max_size * sizeof(struct requestlist*));
...
struct requestlist* __aio_find_req(aiocb_union* elem)
{
...
struct requestlist* __aio_find_req_fd(int fildes)
{
    struct requestlist* runp = requests;
    ...
    return (runp != NULL && runp->aiocbp->aiocb.aio_fildes == fildes ? runp
                                                                     : NULL);
}
        ...
        else
        {
            ...
            if ((aiocbp->aiocb.aio_lio_opcode & 127) == LIO_READ)
            {
                if (sizeof(off_t) != sizeof(off64_t) &&
                    ...
                    aiocbp->aiocb.__return_value = TEMP_FAILURE_RETRY(
                        __pread64(fildes, (void*)aiocbp->aiocb64.aio_buf,
                                  aiocbp->aiocb64.aio_nbytes,
                                  aiocbp->aiocb64.aio_offset));
                else
                    aiocbp->aiocb.__return_value = TEMP_FAILURE_RETRY(
                        __libc_pread(fildes, (void*)aiocbp->aiocb.aio_buf,
                ...
                if (aiocbp->aiocb.__return_value == -1 && errno == ESPIPE)
                    /* The Linux kernel is different from others.  It returns
                       ...
                       read.  */
                    aiocbp->aiocb.__return_value = TEMP_FAILURE_RETRY(
                        read(fildes, (void*)aiocbp->aiocb64.aio_buf,
                             aiocbp->aiocb64.aio_nbytes));
            }
            else if ((aiocbp->aiocb.aio_lio_opcode & 127) == LIO_WRITE)
            {
                if (sizeof(off_t) != sizeof(off64_t) &&
                    aiocbp->aiocb.aio_lio_opcode & 128)
                    aiocbp->aiocb.__return_value = TEMP_FAILURE_RETRY(
                        __pwrite64(fildes, (const void*)aiocbp->aiocb64.aio_buf,
                                   aiocbp->aiocb64.aio_nbytes,
                                   aiocbp->aiocb64.aio_offset));
                else
                    aiocbp->aiocb.__return_value =
                        TEMP_FAILURE_RETRY(__libc_pwrite(
                            fildes, (const void*)aiocbp->aiocb.aio_buf,
                            aiocbp->aiocb.aio_nbytes,
                            aiocbp->aiocb.aio_offset));
                ...
                if (aiocbp->aiocb.__return_value == -1 && errno == ESPIPE)
                    /* The Linux kernel is different from others.  It returns
                       ...
                       write.  */
                    aiocbp->aiocb.__return_value = TEMP_FAILURE_RETRY(
                        write(fildes, (void*)aiocbp->aiocb64.aio_buf,
                              aiocbp->aiocb64.aio_nbytes));
            }
       ...
       something to arrive in it. */
        if (runp == NULL && optim.aio_idle_time >= 0)
        {
            ...
            pthread_cond_timedwait(&__aio_new_request_notification,
                                   &__aio_requests_mutex, &wakeup_time);
            --idle_thread_count;
            runp = runlist;
        }
