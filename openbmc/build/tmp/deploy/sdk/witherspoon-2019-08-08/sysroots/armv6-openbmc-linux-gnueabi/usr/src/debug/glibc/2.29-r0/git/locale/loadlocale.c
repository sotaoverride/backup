...
#define DEFINE_CATEGORY(category, category_name, items, a)                     \
    static const enum value_type _nl_value_type_##category[_NL_ITEM_INDEX(     \
        _NL_NUM_##category)] = {NO_PAREN items};
...
struct __locale_data* _nl_intern_locale_data(int category, const void* data,
                                             size_t datasize)
{
    const struct
    {
...
void _nl_load_locale(struct loaded_l10nfile* file, int category)
{
    ...
    if (__glibc_unlikely(S_ISDIR(st.st_mode)))
    {
        ...
        newp = (char*)alloca(filenamelen + 5 +
                             _nl_category_name_sizes[category] + 1);
        __mempcpy(
            __mempcpy(__mempcpy(newp, file->filename, filenamelen), "/SYS_", 5),
...
#define MAP_FILE 0
#endif
    filedata =
        __mmap((caddr_t)0, st.st_size, PROT_READ, MAP_FILE | MAP_COPY, fd, 0);
    ...
    newdata->name =
        NULL; /* This will be filled if necessary in findlocale.c. */
    newdata->alloc = alloc;

    file->data = newdata;
}

void _nl_unload_locale(struct __locale_data* locale)
{
