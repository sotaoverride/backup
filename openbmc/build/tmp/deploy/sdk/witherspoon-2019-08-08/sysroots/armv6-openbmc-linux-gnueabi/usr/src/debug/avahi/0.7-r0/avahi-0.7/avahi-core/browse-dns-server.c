...
static AvahiDNSServerInfo* get_server_info(AvahiSDNSServerBrowser* b,
                                           AvahiIfIndex interface,
                                           AvahiProtocol protocol,
                                           AvahiRecord* r)
{
    ...
    for (i = b->info; i; i = i->info_next)
        if (i->interface == interface && i->protocol == protocol &&
...
static void host_name_resolver_callback(
    AvahiSHostNameResolver* r, AVAHI_GCC_UNUSED AvahiIfIndex interface,
    AVAHI_GCC_UNUSED AvahiProtocol protocol, AvahiResolverEvent event,
    const char* host_name, const AvahiAddress* a, AvahiLookupResultFlags flags,
    void* userdata)
{
    ...
    switch (event)
    {
        case AVAHI_RESOLVER_FOUND:
        {
            ...
            i->browser->callback(i->browser, i->interface, i->protocol,
                                 AVAHI_BROWSER_NEW,
                                 i->srv_record->data.srv.name, &i->address,
                                 i->srv_record->data.srv.port, i->flags | flags,
                                 i->browser->userdata);
                            ...
                            void* userdata)
{
    ...
    /* Filter flags */
    flags &= AVAHI_LOOKUP_RESULT_CACHED | AVAHI_LOOKUP_RESULT_MULTICAST |
    ...
    switch (event)
    {
        case AVAHI_BROWSER_NEW:
        {
            ...
            i->host_name_resolver = avahi_s_host_name_resolver_new(
                b->server, interface, protocol, record->data.srv.name,
                b->aprotocol, b->user_flags, host_name_resolver_callback, i);
            i->flags = flags;
        ...
        case AVAHI_BROWSER_REMOVE:
        {
            ...
            if (!i->host_name_resolver)
                b->callback(b, interface, protocol, event,
                            i->srv_record->data.srv.name, &i->address,
                            i->srv_record->data.srv.port, i->flags | flags,
                            b->userdata);
        ...
        case AVAHI_BROWSER_CACHE_EXHAUSTED:
            ...
            b->callback(b, interface, protocol, event, NULL, NULL, 0, flags,
                        b->userdata);
...
AvahiSDNSServerBrowser* avahi_s_dns_server_browser_new(
    ...
    const char* domain, AvahiDNSServerType type, AvahiProtocol aprotocol,
    AvahiLookupFlags flags, AvahiSDNSServerBrowserCallback callback,
    void* userdata)
{

    static const char* const type_table[AVAHI_DNS_SERVER_MAX] = {
        "_domain._udp", "_dns-update._udp"};
    ...
    AVAHI_CHECK_VALIDITY_RETURN_NULL(server, type < AVAHI_DNS_SERVER_MAX,
                                     AVAHI_ERR_INVALID_FLAGS);

    if (!domain)
        domain = server->domain_name;

    if ((r = avahi_service_name_join(n, sizeof(n), NULL, type_table[type],
                                     domain)) < 0)
    {
    ...
    AVAHI_LLIST_PREPEND(AvahiSDNSServerBrowser, browser,
                        server->dns_server_browsers, b);
    ...
    if (!(b->record_browser =
              avahi_s_record_browser_new(server, interface, protocol, k, flags,
                                         record_browser_callback, b)))
        goto fail;
...
void avahi_s_dns_server_browser_free(AvahiSDNSServerBrowser* b)
{
    ...
    AVAHI_LLIST_REMOVE(AvahiSDNSServerBrowser, browser,
                       b->server->dns_server_browsers, b);
