...
int netlink_slot_allocate(sd_netlink* nl, bool floating, NetlinkSlotType type,
                          size_t extra, void* userdata, const char* description,
                          sd_netlink_slot** ret)
{
    ...
    switch (slot->type)
    {
        ...
        case NETLINK_REPLY_CALLBACK:
            ...
            if (slot->reply_callback.timeout != 0)
                prioq_remove(nl->reply_callbacks_prioq, &slot->reply_callback,
                             &slot->reply_callback.prioq_idx);

            break;
        case NETLINK_MATCH_CALLBACK:
            LIST_REMOVE(match_callbacks, nl->match_callbacks,
                        &slot->match_callback);
...
DEFINE_PUBLIC_TRIVIAL_REF_UNREF_FUNC(sd_netlink_slot, sd_netlink_slot,
                                     netlink_slot_free);
