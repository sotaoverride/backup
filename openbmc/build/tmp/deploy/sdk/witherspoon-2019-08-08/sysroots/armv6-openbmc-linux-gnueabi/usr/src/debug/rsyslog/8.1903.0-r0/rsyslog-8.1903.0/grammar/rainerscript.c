...
static struct cnfparamdescr incpdescr[] = {{"file", eCmdHdlrString, 0},
                                           {"text", eCmdHdlrString, 0},
                                           {"mode", eCmdHdlrGetWord, 0}};
static struct cnfparamblk incpblk = {
    CNFPARAMBLK_VERSION, sizeof(incpdescr) / sizeof(struct cnfparamdescr),
    incpdescr};
...
static rsRetVal DecodePropFilter(uchar* pline, struct cnfstmt* stmt)
{
    ...
    CHKiRet(msgPropDescrFill(&stmt->d.s_propfilt.prop,
    ...
    if (iRet != RS_RET_OK)
    {
        parser_errmsg("error %d compare operation property - ignoring selector",
                      iRet);
        FINALIZE;
    }
    ...
    else if (!rsCStrOffsetSzStrCmp(pCSCompOp, iOffset, (uchar*)"startswith",
                                   10))
    {
        stmt->d.s_propfilt.operation = FIOP_STARTSWITH;
    }
    else if (!rsCStrOffsetSzStrCmp(pCSCompOp, iOffset, (unsigned char*)"regex",
                                   5))
    {
        stmt->d.s_propfilt.operation = FIOP_REGEX;
    }
    else if (!rsCStrOffsetSzStrCmp(pCSCompOp, iOffset,
                                   (unsigned char*)"ereregex", 8))
    {
 ...
 * CMP_GE, CMP_NE.
 */
static void prifiltSetSeverity(struct funcData_prifilt* prifilt, int sev,
                               int mode)
{
    static int lessthanmasks[] = {0x00, 0x01, 0x03, 0x07, 0x0f,
                                  0x1f, 0x3f, 0x7f, 0xff};
    int i;
    for (i = 0; i < LOG_NFACILITIES + 1; ++i)
    {
 ...
 */
static void
    prifiltSetFacility(struct funcData_prifilt* __restrict__ const prifilt,
                       const int fac, const int mode)
{
    int i;
...
struct objlst* objlstAdd(struct objlst* root, struct cnfobj* o)
{
    ...
    { /* find last, linear search ok, as only during config phase */
 ...
 */
static struct nvlst* nvlstFindNameCStr(struct nvlst* lst,
                                       const char* const __restrict__ name)
{
...
static int doGetSize(struct nvlst* valnode, struct cnfparamdescr* param,
...
static int doGetBinary(struct nvlst* valnode, struct cnfparamdescr* param,
                       struct cnfparamvals* val)
{
    int r = 1;
    val->val.datatype = 'N';
...
static int doGetQueueType(struct nvlst* valnode, struct cnfparamdescr* param,
                          struct cnfparamvals* val)
{
    char* cstr;
    int r = 1;
    ...
    else
    {
        ...
        parser_errmsg("param '%s': unknown queue type: '%s'", param->name,
                      cstr);
        free(cstr);
        r = 0;
    }
 ...
 * starting with '0'.
 */
static int doGetFileCreateMode(struct nvlst* valnode,
...
static int doGetGID(struct nvlst* valnode, struct cnfparamdescr* param,
                    struct cnfparamvals* val)
{
    char* cstr;
    int r;
...
static int doGetUID(struct nvlst* valnode, struct cnfparamdescr* param,
                    struct cnfparamvals* val)
{
    char* cstr;
    int r;
 ...
 */
static int doGetInt(struct nvlst* valnode, struct cnfparamdescr* param,
                    struct cnfparamvals* val)
{
    long long n;
    int bSuccess;
...
static int doGetNonNegInt(struct nvlst* valnode, struct cnfparamdescr* param,
                          struct cnfparamvals* val)
{
    int bSuccess;
...
static int doGetPositiveInt(struct nvlst* valnode, struct cnfparamdescr* param,
                            struct cnfparamvals* val)
{
    int bSuccess;
...
static int doGetWord(struct nvlst* valnode, struct cnfparamdescr* param,
                     struct cnfparamvals* val)
{
    es_size_t i;
    int r = 1;
...
static int doGetArray(struct nvlst* valnode, struct cnfparamdescr* param,
                      struct cnfparamvals* val)
{
    int r = 1;
...
static int doGetChar(struct nvlst* valnode, struct cnfparamdescr* param,
                     struct cnfparamvals* val)
{
    int r = 1;
 ...
 */
static int nvlstGetParam(struct nvlst* valnode, struct cnfparamdescr* param,
                         struct cnfparamvals* val)
{
    uchar* cstr;
    int r;
 ...
 */
struct cnfparamvals* nvlstGetParams(struct nvlst* lst,
                                    struct cnfparamblk* params,
                                    struct cnfparamvals* vals)
{
    ...
    {
        bValsWasNULL = 1;
        if ((vals = calloc(params->nParams, sizeof(struct cnfparamvals))) ==
            NULL)
            return NULL;
    }
    else
...
void cnfparamsPrint(const struct cnfparamblk* params,
                    const struct cnfparamvals* vals)
{
    int i;
    char* cstr;
...
struct cnfexpr* cnfexprNew(unsigned nodetype, struct cnfexpr* l,
                           struct cnfexpr* r)
{
    struct cnfexpr* expr;
...
static int64_t str2num(es_str_t* s, int* bSuccess)
{
    ...
    {
        DBGPRINTF(
            "rainerscript: str2num: strlen == 0; invalid input (no string)\n");
        if (bSuccess != NULL)
        {
            *bSuccess = 1;
        }
 ...
 */
static es_str_t* var2String(struct svar* __restrict__ const r,
                            int* __restrict__ const bMustFree)
{
    es_str_t* estr;
    const char* cstr;
    rs_size_t lenstr;
...
uchar* var2CString(struct svar* __restrict__ const r,
                   int* __restrict__ const bMustFree)
{
    uchar* cstr;
    es_str_t* estr;
...
static rsRetVal doExtractFieldByChar(uchar* str, uchar delim,
                                     const int matchnbr, uchar** resstr)
{
    int iCurrFld;
    int allocLen;
    int iLen;
...
#ifdef VALGRIND
        allocLen += (3 - (iLen % 4));
        /*older versions of valgrind have a problem with strlen inspecting
         * 4-bytes at a time*/
#endif
...
static rsRetVal doExtractFieldByStr(uchar* str, char* delim,
                                    const rs_size_t lenDelim,
                                    const int matchnbr, uchar** resstr)
{
    int iCurrFld;
    int iLen;
    ...
    if (iCurrFld == matchnbr)
    {
        ...
        { /* found delmiter!  Note that pFldEnd *is* already on
...
static void doFunc_re_extract(struct cnffunc* func, struct svar* ret,
                              void* usrptr, wti_t* const pWti)
{
    size_t submatchnbr;
    short matchnbr;
    ...
    while (!bFound)
    {
        ...
        if (iREstat == 0)
        {
            ...
            {
                DBGPRINTF(
                    "oops ... start offset of successful regexec is -1\n");
                break;
            }
            if (iTry == matchnbr)
            {
            ...
            {
                DBGPRINTF("re_extract: regex found at offset %d, new offset "
                          "%d, tries %d\n",
    ...
    else
    {
        ...
        estr =
            es_newStrFromBuf(str + iOffs + pmatch[submatchnbr].rm_so, iLenBuf);
    }
 ...
 * TODO: think if we can keep our buffer; but that may not be trival thinking
...
static void ATTR_NONNULL()
    doFunc_parse_json(struct cnffunc* __restrict__ const func,
                      struct svar* __restrict__ const ret, void* const usrptr,
                      wti_t* const pWti)
{
                      ...
                      wti_t* __restrict__ const pWti)
{
    ...
    if (!success)
    {
        DBGPRINTF("rainerscript: random(max) didn't get a valid 'max' limit, "
                  "defaulting random-number "
                  "value to 0");
        retVal = 0;
        goto done;
    }
    if (max == 0)
    {
        DBGPRINTF("rainerscript: random(max) invalid, 'max' is zero, , "
                  "defaulting random-number value to 0");
        retVal = 0;
        goto done;
    }
    x = labs(randomNumber());
                                        ...
                                        wti_t* __restrict__ const pWti)
{
    ...
    {
        ret->d.n =
            es_str2num(((struct cnfstringval*)func->expr[0])->estr, NULL);
    }
    else
    {
        cnfexprEval(func->expr[0], &srcVal, usrptr, pWti);
                     ...
                     wti_t* __restrict__ const pWti)
{
    ...
    for (unsigned int i = 0; i < len; i++)
    {
        switch (str[i])
        {
            ...
            case '9':
                if (endblank == 1)
                {
                    DBGPRINTF("rainerscript: (ipv42num) error: wrong "
                              "IP-Address format "
                              "(invalid space(1))\n");
                    goto done;
                }
            ...
            case '.':
                ...
                break;
            default:
                DBGPRINTF("rainerscript: (ipv42num) error: wrong IP-Address "
                          "format (invalid charakter)\n");
                goto done;
        }
    }
    if (cyc != 3)
    ...
    value =
        num[0] * 256 * 256 * 256 + num[1] * 256 * 256 + num[2] * 256 + num[3];
done:
...
{ // TODO: generalize parameter getter? jgerhards, 2018-02-26
    ...
    ret->datatype = 'S';
    ret->d.estr =
        es_newStrFromSubStr(es, (es_size_t)start, (es_size_t)subStrLen);
    if (bMustFree)
        es_deleteStr(es);
    varFreeMembers(&srcVal[0]);
    varFreeMembers(&srcVal[1]);
                  ...
                  wti_t* __restrict__ const pWti)
{
    ...
    if (srcVal[1].datatype == 'S')
    {
        ...
        localRet =
            doExtractFieldByStr((uchar*)str, delimstr,
                                es_strlen(srcVal[1].d.estr), matchnbr, &resStr);
        free(delimstr);
    }
    else
    {
        ...
        localRet =
            doExtractFieldByChar((uchar*)str, (char)delim, matchnbr, &resStr);
    }
    if (localRet == RS_RET_OK)
    {
    ...
    {
        ret->d.estr =
            es_newStrFromCStr("***ERROR in field() FUNCTION***",
                   ...
                   wti_t* __restrict__ const pWti)
{
    ...
    {
        ret->d.estr =
            es_newStrFromCStr("TABLE-NOT-FOUND", sizeof("TABLE-NOT-FOUND") - 1);
        return;
    }
                       ...
                       wti_t* __restrict__ const pWti)
{
    ...
    if (sizeof(time_t) == sizeof(int))
    {
        if (unixtime < INT_MIN || unixtime > INT_MAX)
        {
            LogMsg(0, RS_RET_VAL_OUT_OF_RANGE, LOG_WARNING,
                   "Timestamp value %lld is out of range for this system "
                   "(time_t is "
                   "32bits)!\n",
                   unixtime);
            retval = 0;
    ...
    {
        if (!retval || datetime.formatUnixTimeFromTime_t(unixtime, formatstr,
                                                         result, resMax) == -1)
        {
                      ...
                      wti_t* __restrict__ const pWti)
{
    ...
    if (objUse(datetime, CORE_COMPONENT) == RS_RET_OK)
    {
        ...
        else if (datetime.ParseTIMESTAMP3164(
                     &s, (uchar**)&pszTS, &len, NO_PARSE3164_TZSTRING,
...
static int ATTR_NONNULL(1, 3, 4)
    doFunc_is_time(const char* __restrict__ const str,
                   ...
                   struct svar* __restrict__ const r, wti_t* pWti)
{
    ...
    if (objUse(datetime, CORE_COMPONENT) == RS_RET_OK)
    {
        ...
        for (int i = 0; i < numFormats; i++)
        {
            ...
            if (f == DATE_RFC3339)
            {
                if (datetime.ParseTIMESTAMP3339(&s, (uchar**)&pszTS, &len) ==
                    RS_RET_OK)
                {
            ...
            {
                if (datetime.ParseTIMESTAMP3164(
                        &s, (uchar**)&pszTS, &len, NO_PARSE3164_TZSTRING,
                        NO_PERMIT_YEAR_AFTER_TIME) == RS_RET_OK)
                {
            ...
            {
                DBGPRINTF(
                    "is_time: %s is not a valid date/time format specifier!\n",
                    fmt);
                break;
            }
        }
...
static void ATTR_NONNULL()
    doFunct_PreviousActionSuspended(struct cnffunc* const func
                     ...
                     wti_t* __restrict__ const pWti)
{
    ...
    len = snprintf(str, 16, "%d.%d.%d.%d", numip[3], numip[2], numip[1],
                   numip[0]);
done:
 ...
 */
static void ATTR_NONNULL()
    doFuncCall(struct cnffunc* __restrict__ const func,
               struct svar* __restrict__ const ret,
               void* __restrict__ const usrptr, wti_t* __restrict__ const pWti)
{
    ...
    if (func->fPtr == NULL)
    {
        ...
        LogError(0, RS_RET_INTERNAL_ERROR,
                 "rainerscript: internal error: NULL pointer for function "
                 "named '%s'\n",
                 fname);
        free(fname);
        ret->datatype = 'N';
                    ...
                    struct svar* __restrict__ const ret)
{
    ...
    if (var->prop.id == PROP_CEE || var->prop.id == PROP_LOCAL_VAR ||
        var->prop.id == PROP_GLOBAL_VAR)
    {
        localRet = msgGetJSONPropJSONorString((smsg_t*)usrptr, &var->prop,
                                              &json, &cstr);
        if (json != NULL)
        {
            assert(cstr == NULL);
            ...
            DBGPRINTF("rainerscript: (json) var %d:%s: '%s'\n", var->prop.id,
                      ...
                      (ret->d.json == NULL)
                          ? ""
                          : json_object_get_string(ret->d.json));
        }
        else
        { /* we have a string */
    ...
    {
        ret->datatype = 'S';
        pszProp = (uchar*)MsgGetProp((smsg_t*)usrptr, NULL, &var->prop,
                                     &propLen, &bMustBeFreed, NULL);
 ...
 */
static int evalStrArrayCmp(es_str_t* const estr_l,
                           ...
                           const int cmpop)
{
    ...
    {
        res =
            bsearch(&estr_l, ar->arr, ar->nmemb, sizeof(es_str_t*), qs_arrcmp);
        r = res != NULL;
    }
    ...
    else
    {
        for (i = 0; (r == 0) && (i < ar->nmemb); ++i)
        {
            ...
            {
                case CMP_STARTSWITH:
                    r = es_strncmp(estr_l, ar->arr[i], es_strlen(ar->arr[i])) ==
                        0;
                    break;
                case CMP_STARTSWITHI:
                    r = es_strncasecmp(estr_l, ar->arr[i],
                                       es_strlen(ar->arr[i])) == 0;
                    break;
                case CMP_CONTAINS:
                ...
                void* __restrict__ const usrptr, wti_t* __restrict__ const pWti)
{
    ...
    {
        /* note: comparison operations are extremely similar. The code can be
         * copyied, only places flagged with "CMP" need to be changed.
         */
        case CMP_EQ:
            /* this is optimized in regard to right param as a PoC for all
             * compOps So this is a NOT yet the copy template!
             */
            cnfexprEval(expr->l, &l, usrptr, pWti);
            ret->datatype = 'N';
            if (l.datatype == 'S')
            {
                if (expr->r->nodetype == 'S')
                {
                    ret->d.n = !es_strcmp(
                        l.d.estr,
                        ((struct cnfstringval*)expr->r)->estr); /*CMP*/
                }
                else if (expr->r->nodetype == 'A')
                {
                    ret->d.n = evalStrArrayCmp(
                        l.d.estr, (struct cnfarray*)expr->r, CMP_EQ);
                }
                else
                {
                    cnfexprEval(expr->r, &r, usrptr, pWti);
            ...
            else if (l.datatype == 'J')
            {
                ...
                {
                    ret->d.n = !es_strcmp(
                        estr_l, ((struct cnfstringval*)expr->r)->estr); /*CMP*/
                }
                else if (expr->r->nodetype == 'A')
                {
                    ret->d.n = evalStrArrayCmp(
                        estr_l, (struct cnfarray*)expr->r, CMP_EQ);
                }
                else
                {
                    cnfexprEval(expr->r, &r, usrptr, pWti);
        ...
        case CMP_NE:
            ...
            if (l.datatype == 'S')
            {
                ...
                {
                    ret->d.n = evalStrArrayCmp(
                        l.d.estr, (struct cnfarray*)expr->r, CMP_NE);
                }
                else
                {
                    if (r.datatype == 'S')
        ...
        case CMP_STARTSWITH:
            ...
            {
                ret->d.n = evalStrArrayCmp(estr_l, (struct cnfarray*)expr->r,
                                           CMP_STARTSWITH);
                bMustFree = 0;
            }
            else
...
static rsRetVal initFunc_dyn_stats(struct cnffunc* func)
{
    ...
    cstr =
        (uchar*)es_str2cstr(((struct cnfstringval*)func->expr[0])->estr, NULL);
...
static rsRetVal initFunc_re_match(struct cnffunc* func)
{
    ...
    {
        parser_errmsg(
            "param 2 of re_match/extract() must be a constant string");
        FINALIZE;
    }
...
static rsRetVal initFunc_exec_template(struct cnffunc* func)
{
    ...
    if (func->funcdata == NULL)
    {
        parser_errmsg("exec_template(): template '%s' could not be found",
                      tplName);
        FINALIZE;
    }
...
static rsRetVal resolveLookupTable(struct cnffunc* func)
{
    ...
    if (func->nParams == 0)
    { /*we assume first arg is lookup-table-name*/
        parser_errmsg("rsyslog logic error in line %d of file %s\n", __LINE__,
                      __FILE__);
        FINALIZE;
    }
    ...
    if (func->expr[0]->nodetype != 'S')
    {
        parser_errmsg("table name (param 1) of %s() must be a constant string",
                      fn_name);
        FINALIZE;
    }

    CHKmalloc(cstr = (uchar*)es_str2cstr(
                  ((struct cnfstringval*)func->expr[0])->estr, NULL));
    if ((func->funcdata = lookupFindTable(cstr)) == NULL)
    {
        parser_errmsg("lookup table '%s' not found (used in function: %s)",
                      cstr, fn_name);
        FINALIZE;
    }
...
static struct scriptFunct functions[] = {
    ...
    {"previous_action_suspended", 0, 0, doFunct_PreviousActionSuspended, NULL,
     NULL},
    {NULL, 0, 0, NULL, NULL, NULL} // last element to check end of array
};

static rscriptFuncPtr ATTR_NONNULL()
    extractFuncPtr(const struct scriptFunct* const funct,
                   const unsigned int nParams)
{
    ...
    if (funct->minParams == funct->maxParams)
    {
        ...
        {
            parser_errmsg(
                "number of parameters for %s() must be %hu but is %d.",
    ...
    else
    {
        ...
        {
            parser_errmsg(
                "number of parameters for %s() must be at least %hu but is %d.",
                funct->fname, funct->minParams, nParams);
        }
        else if (nParams > funct->maxParams)
        {
            parser_errmsg(
                "number of parameters for %s() must be at most %hu but is %d.",
                funct->fname, funct->maxParams, nParams);
        }
        else
        {
 ...
 * important.
 */
int cnfexprEvalBool(struct cnfexpr* __restrict__ const expr,
                    void* __restrict__ const usrptr, wti_t* const pWti)
{
    int convok;
    struct svar ret;
...
struct json_object*
    cnfexprEvalCollection(struct cnfexpr* __restrict__ const expr,
                          void* __restrict__ const usrptr, wti_t* const pWti)
{
    struct svar ret;
    void* retptr;
    ...
    {
        retptr =
            ret.d.json; /*caller is supposed to free the returned json-object*/
    }
    else
    {
        retptr = NULL;
...
struct cnfarray* cnfarrayAdd(struct cnfarray* __restrict__ const ar,
                             es_str_t* __restrict__ val)
{
    es_str_t** newptr;
    if ((newptr = realloc(ar->arr, (ar->nmemb + 1) * sizeof(es_str_t*))) ==
        NULL)
    {
...
struct cnfstmt* cnfstmtNewSet(char* var, struct cnfexpr* expr, int force_reset)
{
    ...
    if ((cnfstmt = cnfstmtNew(S_SET)) != NULL)
    {
        if (propNameToID((uchar*)var, &propid) == RS_RET_OK &&
            (propid == PROP_CEE || propid == PROP_LOCAL_VAR ||
...
struct cnfstmt* cnfstmtNewReloadLookupTable(struct cnffparamlst* fparams)
{
    ...
    if ((cnfstmt = cnfstmtNew(S_RELOAD_LOOKUP_TABLE)) != NULL)
    {
        ...
        cnfstmt->d.s_reload_lookup_table.table_name =
            cnfstmt->d.s_reload_lookup_table.stub_value = NULL;
        switch (nParams)
        {
            case 2:
                param = fparams->next;
                ...
                if ((cnfstmt->d.s_reload_lookup_table.stub_value = (uchar*)
                         es_str2cstr(((struct cnfstringval*)param->expr)->estr,
                                     NULL)) == NULL)
                {
                    parser_errmsg(
                        "statement ignored: reload_lookup_table statement "
                        "failed to allocate memory for lookup-table "
                        "stub-value\n");
                    failed = 1;
                }
                CASE_FALLTHROUGH
            case 1:
                ...
                if ((cnfstmt->d.s_reload_lookup_table.table_name = (uchar*)
                         es_str2cstr(((struct cnfstringval*)param->expr)->estr,
                                     NULL)) == NULL)
                {
                    parser_errmsg(
                ...
                break;
            default:
                parser_errmsg("statement ignored: "
                              "reload_lookup_table(table_name, optional:"
...
struct cnfstmt* cnfstmtNewAct(struct nvlst* lst)
{
    ...
    {
        parser_errmsg("warnings occured in file '%s' around line %d", cnfcurrfn,
                      yylineno);
    }
    else if (localRet != RS_RET_OK)
    {
        parser_errmsg("errors occured in file '%s' around line %d", cnfcurrfn,
                      yylineno);
        cnfstmt->nodetype = S_NOP; /* disable action! */
        goto done;
    }
    ...
    cnfexprOptimize_CMP_severity_facility(struct cnfexpr* expr)
{
    ...
    if (!strcmp("syslogseverity", ((struct cnfvar*)expr->l)->name))
    {
        if (expr->r->nodetype == 'N')
        {
            ...
            {
                DBGPRINTF(
                    "optimizer: change comparison OP to FUNC prifilt()\n");
                func =
                    cnffuncNew_prifilt(0); /* fac is irrelevant, set below... */
...
static struct cnfexpr* cnfexprOptimize_CMP_var(struct cnfexpr* expr)
{
    ...
    if (!strcmp("syslogfacility-text", ((struct cnfvar*)expr->l)->name))
    {
        ...
        {
            char* cstr =
                es_str2cstr(((struct cnfstringval*)expr->r)->estr, NULL);
...
static struct cnfexpr* cnfexprOptimize_AND_OR(struct cnfexpr* expr)
{
    ...
    if (expr->l->nodetype == 'F')
    {
        if (expr->r->nodetype == 'F')
        {
            ...
            if (funcl->fPtr == doFunct_Prifilt &&
                funcr->fPtr == doFunct_Prifilt)
            {
                DBGPRINTF("optimize combine AND/OR prifilt()\n");
                expr->l = NULL;
                prifiltCombine(funcl->funcdata, funcr->funcdata,
                               expr->nodetype);
                cnfexprDestruct(expr);
...
{
    DBGPRINTF(
        "optimizer: sorting array of %d members for CMP_EQ/NEQ comparison\n",
        arr->nmemb);
    ...
    switch (expr->nodetype)
    {
        ...
        case CMP_EQ:
            ...
            if (expr->l->nodetype == 'A')
            {
                ...
                { /* swap for simpler execution step */
                    exprswap = expr->l;
                    expr->l = expr->r;
                    expr->r = exprswap;
                }
...
{
    stmt->d.s_foreach.iter->collection =
        cnfexprOptimize(stmt->d.s_foreach.iter->collection);
    ...
    if (stmt->d.s_if.expr->nodetype == 'F')
    {
        ...
        if (func->fPtr == doFunct_Prifilt)
        {
            ...
            if (func->nParams == 0)
                ...
                stmt->printable = (uchar*)es_str2cstr(
                    ((struct cnfstringval*)func->expr[0])->estr, NULL);
            cnfexprDestruct(expr);
...
{
    if ((stmt->d.s_reload_lookup_table.table =
             lookupFindTable(stmt->d.s_reload_lookup_table.table_name)) == NULL)
    {
    ...
    for (stmt = root; stmt != NULL; stmt = stmt->next)
    {
        ...
        switch (stmt->nodetype)
        {
            ...
            case S_CALL:
                ...
                break;
            case S_CALL_INDIRECT:
                stmt->d.s_call_ind.expr =
                    cnfexprOptimize(stmt->d.s_call_ind.expr);
                break;
            case S_STOP:
                if (stmt->next != NULL)
                    parser_errmsg(
                        "STOP is followed by unreachable statements!\n");
                break;
            ...
            default:
                LogError(0, RS_RET_INTERNAL_ERROR,
                         "internal error: unknown stmt type %u during "
                         "optimizer run\n",
                         (unsigned)stmt->nodetype);
                break;
        }
...
struct cnffparamlst* cnffparamlstNew(struct cnfexpr* expr,
 ...
 */
static rscriptFuncPtr funcName2Ptr(char* const fname,
                                   const unsigned short nParams)
{
...
rsRetVal addMod2List(const int __attribute__((unused)) version,
...
/*version currently not used, might be needed later for versin check*/
{
    ...
    CHKmalloc(newNode =
                  (struct modListNode*)malloc(sizeof(struct modListNode)));
    newNode->version = 1;
    newNode->next = NULL;
    ...
    while (functArray[i].fname != NULL)
    {
        ...
        {
            parser_errmsg("function %s defined multiple times, second time "
                          "will be ignored",
                          functArray[i].fname);
        }
        i++;
...
struct cnffunc* cnffuncNew(es_str_t* fname, struct cnffparamlst* paramlst)
{
    ...
    for (param = paramlst; param != NULL; param = param->next)
        ++nParams;
    if ((func = malloc(sizeof(struct cnffunc) +
                       (nParams * sizeof(struct cnfexp*)))) != NULL)
    {
        func->nodetype = 'F';
        func->fname = fname;
...
struct cnffunc* cnffuncNew_prifilt(int fac)
{
    ...
    {
        if ((func->funcdata = calloc(1, sizeof(struct funcData_prifilt))) ==
            NULL)
        {
            free(func);
            return NULL;
...
int ATTR_NONNULL() cnfDoInclude(const char* const name, const int optional)
{
    ...
    {
        /* stat usually fails if we have a wildcard - so this does NOT indicate
         * error! */
...
/* Use GLOB_NOMAGIC to detect wildcards that match nothing. */
    ...
    for (i = cfgFiles.gl_pathc - 1; i >= 0; i--)
    {
        ...
        {
            DBGPRINTF(
                "warning: unable to process IncludeConfig directive '%s'\n",
                cfgFile);
        }
    }
    ...
    if (inc_file != NULL)
    {
        if (cnfDoInclude(inc_file, optional) != 0 && abort_if_missing)
        {
            fprintf(stderr,
                    ...
                    inc_file);
            exit(1); /* "good exit" - during config processing, requested by
                        user */
        }
    }
    else if (text != NULL)
...
void cnfparamvalsDestruct(const struct cnfparamvals* paramvals,
                          const struct cnfparamblk* blk)
{
    int i;
    if (paramvals == NULL)
        return;
    ...
    if (c[*iSrc] == '\\')
    {
        ...
        switch (c[*iSrc])
        {
            ...
            case '\\':
                ...
                break;
            case 'x':
                if ((*iSrc) + 2 >= len || !isxdigit(c[(*iSrc) + 1]) ||
                    !isxdigit(c[(*iSrc) + 2]))
                {
            ...
            case '6':
            case '7':
                if ((*iSrc) + 2 >= len || !isodigit(c[(*iSrc) + 1]) ||
                    !isodigit(c[(*iSrc) + 2]))
                {
                    /* error, incomplete escape, use as is */
                    c[iDst] = '\\';
