...
void fdset_close(FDSet* s)
{
    ...
    {
        /* Valgrind's fd might have ended up in this set here, due to
         * fdset_new_fill(). We'll ignore all failures here, so that the EBADFD
         * that valgrind will return us on close() doesn't influence us */

        /* When reloading duplicates of the private bus connection fds and
         * suchlike are closed here, which has no effect at all, since they are
         * only duplicates. So don't be surprised about these log messages. */
