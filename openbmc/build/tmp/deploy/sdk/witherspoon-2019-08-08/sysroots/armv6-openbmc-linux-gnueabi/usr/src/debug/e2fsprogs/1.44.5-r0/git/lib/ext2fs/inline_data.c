...
static errcode_t ext2fs_inline_data_ea_set(struct ext2_inline_data* data)
{
    ...
    retval =
        ext2fs_xattr_set(handle, "system.data", data->ea_data, data->ea_size);
err:
...
static errcode_t ext2fs_inline_data_ea_get(struct ext2_inline_data* data)
{
    ...
    retval = ext2fs_xattr_get(handle, "system.data", (void**)&data->ea_data,
                              &data->ea_size);
                                   ...
                                   void* priv_data)
{
    ...
    {
#ifdef WORDS_BIGENDIAN
        ctx->errcode = ext2fs_dirent_swab_out2(fs, ctx->buf, ctx->buflen, 0);
        if (ctx->errcode)
        {
            ret |= BLOCK_ABORT;
            goto out;
...
static errcode_t ext2fs_inline_data_dir_expand(ext2_filsys fs, ext2_ino_t ino,
                                               struct ext2_inode* inode,
...
#ifdef WORDS_BIGENDIAN
    retval =
        ext2fs_dirent_swab_in2(fs, buf + EXT4_INLINE_DATA_DOTDOT_SIZE, size, 0);
    if (retval)
        goto errout;
#endif
...
static errcode_t ext2fs_inline_data_file_expand(ext2_filsys fs, ext2_ino_t ino,
                                                struct ext2_inode* inode,
                                                char* buf, size_t size)
{
    ext2_file_t e2_file;
...
errcode_t ext2fs_inline_data_expand(ext2_filsys fs, ext2_ino_t ino)
{
    ...
    if (data.ea_size > 0)
    {
        memcpy(inline_buf + EXT4_MIN_INLINE_DATA_SIZE, data.ea_data,
               data.ea_size);
    }
    ...
    if (LINUX_S_ISDIR(inode.i_mode))
    {
        retval = ext2fs_inline_data_dir_expand(fs, ino, &inode, inline_buf,
                                               inline_size);
    }
    else
    {
        retval = ext2fs_inline_data_file_expand(fs, ino, &inode, inline_buf,
                                                inline_size);
    }
                                 ...
                                 size_t* size)
{
    ...
    if (data.ea_size > 0)
        memcpy((char*)buf + EXT4_MIN_INLINE_DATA_SIZE, data.ea_data,
               data.ea_size);
...
static errcode_t file_test(ext2_filsys fs)
{
    ...
    if (size != EXT4_MIN_INLINE_DATA_SIZE)
    {
        fprintf(stderr, "tst_inline_data: size of inline data is wrong\n");
        return 1;
    }
    ...
    if (retval)
    {
        com_err("file_test", retval, "while setting inline data %s", buf);
        goto err;
    }
    ...
    if (size != BUFF_SIZE)
    {
        fprintf(stderr, "tst_inline_data: size %lu != buflen %u\n", size,
                BUFF_SIZE);
        retval = 1;
        goto err;
    }
...
static errcode_t dir_test(ext2_filsys fs)
{
    ...
    if (retval)
    {
        com_err("dir_test", retval, "while creating %s dir", parent_name);
        return retval;
    }

    retval =
        ext2fs_lookup(fs, 11, parent_name, strlen(parent_name), 0, &parent);
    if (retval)
    {
        com_err("dir_test", retval, "while looking up %s dir", parent_name);
        return retval;
    }

    retval = ext2fs_lookup(fs, parent, dot_name, strlen(dot_name), 0, &tmp);
    if (retval)
    {
        com_err("dir_test", retval, "while looking up %s dir", parent_name);
        return retval;
    ...
    for (i = 0, dir = 13; i < 4; i++, dir++)
    {
        ...
        retval = ext2fs_lookup(fs, parent, dirname, strlen(dirname), 0, &tmp);
        if (retval)
        {
            com_err("dir_test", retval, "while looking up %s dir", parent_name);
            return retval;
...
int main(int argc, char* argv[])
{
    ...
    if (!fs->icache)
    {
        ...
        if (!fs->icache)
        {
            fprintf(stderr, "tst_inline_data: init inode cache failed\n");
            exit(1);
        }
