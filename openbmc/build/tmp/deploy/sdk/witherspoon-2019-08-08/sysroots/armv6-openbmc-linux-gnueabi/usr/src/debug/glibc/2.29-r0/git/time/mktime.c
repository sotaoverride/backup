...
static long_int const mktime_min =
    ((TYPE_SIGNED(time_t) && TYPE_MINIMUM(time_t) < TYPE_MINIMUM(long_int))
         ? TYPE_MINIMUM(long_int)
         : TYPE_MINIMUM(time_t));
static long_int const mktime_max =
    (TYPE_MAXIMUM(long_int) < TYPE_MAXIMUM(time_t) ? TYPE_MAXIMUM(long_int)
                                                   : TYPE_MAXIMUM(time_t));
...
/* Use CONVERT to convert T to a struct tm value in *TM.  T must be in
   ...
   failure.  */
static struct tm* convert_time(struct tm* (*convert)(const time_t*, struct tm*),
...
static struct tm* ranged_convert(struct tm* (*convert)(const time_t*,
                                                       struct tm*),
                                 long_int* t, struct tm* tp)
{
    long_int t1 =
        (*t < mktime_min ? mktime_min : *t <= mktime_max ? *t : mktime_max);
                         ...
                         mktime_offset_t* offset)
{
    ...
    long_int t0 =
        ydhms_diff(year, yday, hour, min, sec, EPOCH_YEAR - TM_YEAR_BASE, 0, 0,
                   0, negative_offset_guess);
            ...
            for (direction = -1; direction <= 1; direction += 2)
            {
                ...
                if (!INT_ADD_WRAPV(t, delta * direction, &ot))
                {
                    ...
                    if (!isdst_differ(isdst, otm.tm_isdst))
                    {
                        ...
                        long_int gt =
                            ot + tm_diff(year, yday, hour, min, sec, &otm);
...
offset_found:
    ...
    if (LEAP_SECONDS_POSSIBLE && sec_requested != tm.tm_sec)
    {
       ...
       value. Also, repair any damage from a false match due to a leap second.
     */
...
#ifdef _LIBC
    libc_hidden_def(mktime) libc_hidden_weak(timelocal)
#endif
