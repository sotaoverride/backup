...
static lzma_ret copy_or_code(lzma_simple_coder* coder,
                             ...
                             size_t out_size, lzma_action action)
{
    ...
    else
    {
        ...
        const lzma_ret ret =
            coder->next.code(coder->next.coder, allocator, in, in_pos, in_size,
                             out, out_pos, out_size, action);

        if (ret == LZMA_STREAM_END)
        {
            assert(!coder->is_encoder || action == LZMA_FINISH);
            coder->end_was_reached = true;
        }
        else if (ret != LZMA_OK)
        {
...
static size_t call_filter(lzma_simple_coder* coder, uint8_t* buffer,
                          size_t size)
{
    const size_t filtered = coder->filter(coder->simple, coder->now_pos,
                                          coder->is_encoder, buffer, size);
    coder->now_pos += filtered;
    return filtered;
}

static lzma_ret simple_code(void* coder_ptr, const lzma_allocator* allocator,
                            ...
                            lzma_action action)
{
    ...
    {
        lzma_bufcpy(coder->buffer, &coder->pos, coder->filtered, out, out_pos,
                    out_size);
    ...
    if (out_avail > buf_avail || buf_avail == 0)
    {
        ...
        *out_pos += buf_avail;
        ...
        {
            const lzma_ret ret =
                copy_or_code(coder, allocator, in, in_pos, in_size, out,
        ...
        const size_t filtered = call_filter(coder, out + out_start, size);
    ...
    {
        {
            const lzma_ret ret = copy_or_code(
                coder, allocator, in, in_pos, in_size, coder->buffer,
                &coder->size, coder->allocated, action);
            assert(ret != LZMA_STREAM_END);
            if (ret != LZMA_OK)
                return ret;
        }

        coder->filtered = call_filter(coder, coder->buffer, coder->size);
...
static lzma_ret simple_coder_update(
...
extern lzma_ret lzma_simple_coder_init(
    ...
    size_t (*filter)(void* simple, uint32_t now_pos, bool is_encoder,
                     uint8_t* buffer, size_t size),
    size_t simple_size, size_t unfiltered_max, uint32_t alignment,
    bool is_encoder)
{
    ...
    if (coder == NULL)
    {
        ...
        coder = lzma_alloc(sizeof(lzma_simple_coder) + 2 * unfiltered_max,
                           allocator);
        if (coder == NULL)
