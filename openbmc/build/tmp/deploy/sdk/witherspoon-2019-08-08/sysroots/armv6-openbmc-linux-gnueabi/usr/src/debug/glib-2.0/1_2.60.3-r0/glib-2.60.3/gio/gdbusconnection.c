 ...
 *
 *     fd 21:
 * dev=8:1,mode=0100644,ino=1171231,uid=0,gid=0,rdev=0:0,size=234,atime=1273070640,mtime=1267126160,ctime=1267126160
 *
...
struct _GDBusConnectionClass
{
    ...
    /*< public >*/
    /* Signals */
    void (*closed)(GDBusConnection* connection, gboolean remote_peer_vanished,
                   GError* error);
};
 ...
 */
static void call_destroy_notify(GMainContext* context, GDestroyNotify callback,
                                gpointer user_data)
{
    GSource* idle_source;
    ...
    g_source_set_callback(idle_source, call_destroy_notify_data_in_idle, data,
...
static gboolean _g_strv_has_string(const gchar* const* haystack,
                                   const gchar* needle)
{
    guint n;
    ...
    {                                                                          \
        ...
        {                                                                      \
            g_assertion_message(G_LOG_DOMAIN, __FILE__, __LINE__, G_STRFUNC,   \
                                "CONNECTION_ENSURE_LOCK: GDBusConnection "     \
                                "object lock is not locked");                  \
        }                                                                      \
    } while (FALSE)
#endif
...
struct _GDBusConnection
{
    ...
    /* A lock used in the init() method of the GInitable interface - see
     * comments in initable_init() for why a separate lock is needed.
     *
     ...
     * someone calls org.freedesktop.DBus.Peer.GetMachineId(). Protected by
     * @lock.
     */
    gchar* machine_id;
    ...
    /* FLAG_INITIALIZED is set exactly when initable_init() has finished
     * running. Inspect @initialization_error to see whether it succeeded or
     * failed.
     *
    ...
    GHashTable*
        map_rule_to_signal_data; /* match rule (gchar*)    -> SignalData */
    GHashTable*
        map_id_to_signal_data; /* id (guint)             -> SignalData */
    GHashTable*
        map_sender_unique_name_to_signal_data_array; /* unique sender (gchar*)
                                                        -> GPtrArray* of
                                                        SignalData */
    ...
    /* Map used for storing last used serials for each thread, protected by
     * @lock */
...
static void schedule_method_call(
    GDBusConnection* connection, GDBusMessage* message, guint registration_id,
    guint subtree_registration_id, const GDBusInterfaceInfo* interface_info,
    const GDBusMethodInfo* method_info, const GDBusPropertyInfo* property_info,
    GVariant* parameters, const GDBusInterfaceVTable* vtable,
 ...
 */
static gboolean check_initialized(GDBusConnection* connection)
{
 ...
 */
static gboolean check_unclosed(GDBusConnection* connection,
                               CheckUnclosedFlags check, GError** error)
{
...
static void g_dbus_connection_dispose(GObject* object)
{
    ...
    else
    {
        if (alive_connections != NULL)
            g_warn_if_fail(
                !g_hash_table_contains(alive_connections, connection));
    }
...
static void g_dbus_connection_get_property(GObject* object, guint prop_id,
                                           GValue* value, GParamSpec* pspec)
{
    ...
    switch (prop_id)
    {
        ...
        case PROP_EXIT_ON_CLOSE:
            g_value_set_boolean(
                value, g_dbus_connection_get_exit_on_close(connection));
            break;
...
/* called in any user thread, with the connection's lock not held */
static void g_dbus_connection_set_property(GObject* object, guint prop_id,
                                           const GValue* value,
                                           GParamSpec* pspec)
{
    GDBusConnection* connection = G_DBUS_CONNECTION(object);
    ...
    switch (prop_id)
    {
        ...
        case PROP_EXIT_ON_CLOSE:
            g_dbus_connection_set_exit_on_close(connection,
 ...
 */
static void g_dbus_connection_real_closed(GDBusConnection* connection,
                                          ...
                                          GError* error)
{
     ...
     */
    if (remote_peer_vanished && (flags & FLAG_EXIT_ON_CLOSE) != 0 &&
...
static void g_dbus_connection_class_init(GDBusConnectionClass* klass)
{
    ...
    g_object_class_install_property(
        ...
        g_param_spec_object("stream", P_("IO Stream"),
    ...
    g_object_class_install_property(
        ...
        g_param_spec_string(
            "address", P_("Address"),
            P_("D-Bus address specifying potential socket endpoints"), NULL,
    ...
    g_object_class_install_property(
        ...
        g_param_spec_flags("flags", P_("Flags"), P_("Flags"),
    ...
    g_object_class_install_property(
        ...
        g_param_spec_string("guid", P_("GUID"), P_("GUID of the server peer"),
                            NULL,
                            G_PARAM_READABLE | G_PARAM_WRITABLE |
                                G_PARAM_CONSTRUCT_ONLY | G_PARAM_STATIC_NAME |
                                G_PARAM_STATIC_BLURB | G_PARAM_STATIC_NICK));
     ...
     */
    g_object_class_install_property(
        gobject_class, PROP_UNIQUE_NAME,
        g_param_spec_string("unique-name", P_("unique-name"),
                            P_("Unique name of bus connection"), NULL,
     ...
     */
    g_object_class_install_property(
        gobject_class, PROP_CLOSED,
        g_param_spec_boolean("closed", P_("Closed"),
                             P_("Whether the connection is closed"), FALSE,
                             G_PARAM_READABLE | G_PARAM_STATIC_NAME |
                                 G_PARAM_STATIC_BLURB | G_PARAM_STATIC_NICK));
     ...
     */
    g_object_class_install_property(
        gobject_class, PROP_EXIT_ON_CLOSE,
        g_param_spec_boolean("exit-on-close", P_("Exit on close"),
                             P_("Whether the process is terminated when the "
     ...
     */
    g_object_class_install_property(
        gobject_class, PROP_CAPABILITY_FLAGS,
        g_param_spec_flags("capabilities", P_("Capabilities"),
                           P_("Capabilities"), G_TYPE_DBUS_CAPABILITY_FLAGS,
     ...
     *
     * A #GDBusAuthObserver object to assist in the authentication process or
     * %NULL.
     *
     * Since: 2.26
     */
    g_object_class_install_property(
        gobject_class, PROP_AUTHENTICATION_OBSERVER,
        g_param_spec_object(
            "authentication-observer", P_("Authentication Observer"),
    ...
    signals[CLOSED_SIGNAL] =
        g_signal_new(I_("closed"), G_TYPE_DBUS_CONNECTION, G_SIGNAL_RUN_LAST,
                     G_STRUCT_OFFSET(GDBusConnectionClass, closed), NULL, NULL,
...
static void g_dbus_connection_init(GDBusConnection* connection)
{
    ...
    connection->map_object_path_to_eo = g_hash_table_new_full(
        g_str_hash, g_str_equal, NULL, (GDestroyNotify)exported_object_free);
    ...
    connection->map_object_path_to_es = g_hash_table_new_full(
        g_str_hash, g_str_equal, NULL, (GDestroyNotify)exported_subtree_free);
    ...
    connection->main_context_at_construction =
...
{
    g_return_val_if_fail(G_IS_DBUS_CONNECTION(connection),
                         G_DBUS_CAPABILITY_FLAGS_NONE);
...
static void flush_in_thread_func(GTask* task, gpointer source_object,
                                 gpointer task_data, GCancellable* cancellable)
{
    GError* error = NULL;

    if (g_dbus_connection_flush_sync(source_object, cancellable, &error))
...
static gboolean emit_closed_in_idle(gpointer user_data)
{
    ...
    g_signal_emit(data->connection, signals[CLOSED_SIGNAL], 0,
                  data->remote_peer_vanished, data->error, &result);
    return FALSE;
}
 ...
 */
static void schedule_closed_unlocked(GDBusConnection* connection,
                                     gboolean remote_peer_vanished,
                                     GError* error)
{
    GSource* idle_source;
    ...
    idle_source = g_idle_source_new();
    g_source_set_priority(idle_source, G_PRIORITY_DEFAULT);
    g_source_set_callback(idle_source, emit_closed_in_idle, data,
...
static void sync_close_cb(GObject* source_object, GAsyncResult* res,
                          gpointer user_data)
{
...
guint32 g_dbus_connection_get_last_serial(GDBusConnection* connection)
{
    ...
    CONNECTION_LOCK(connection);
    ret = GPOINTER_TO_UINT(g_hash_table_lookup(
        connection->map_thread_to_last_serial, g_thread_self()));
    CONNECTION_UNLOCK(connection);
...
static gboolean g_dbus_connection_send_message_unlocked(
    GDBusConnection* connection, GDBusMessage* message,
    GDBusSendMessageFlags flags, volatile guint32* out_serial, GError** error)
{
    guchar* blob;
    gsize blob_size;
    ...
    if (!check_unclosed(connection,
                        (flags & SEND_MESSAGE_FLAGS_INITIALIZING)
                            ? MAY_BE_UNINITIALIZED
                            : 0,
                        error))
        goto out;

    blob = g_dbus_message_to_blob(message, &blob_size, connection->capabilities,
                                  error);
    if (blob == NULL)
        goto out;
  ...
  g_printerr ("----\n");
     ...
     */
    g_hash_table_replace(connection->map_thread_to_last_serial, g_thread_self(),
    ...
    _g_dbus_worker_send_message(connection->worker, message, (gchar*)blob,
                                blob_size);
                                        ...
                                        GError** error)
{
    ...
    g_return_val_if_fail(G_IS_DBUS_CONNECTION(connection), FALSE);
    g_return_val_if_fail(G_IS_DBUS_MESSAGE(message), FALSE);
    g_return_val_if_fail((flags & G_DBUS_SEND_MESSAGE_FLAGS_PRESERVE_SERIAL) ||
                             !g_dbus_message_get_locked(message),
                         FALSE);
    g_return_val_if_fail(error == NULL || *error == NULL, FALSE);

    CONNECTION_LOCK(connection);
    ret = g_dbus_connection_send_message_unlocked(connection, message, flags,
                                                  out_serial, error);
    CONNECTION_UNLOCK(connection);
    return ret;
}
...
static void send_message_with_reply_cleanup(GTask* task, gboolean remove)
{
    ...
    {
        gboolean removed =
            g_hash_table_remove(connection->map_method_serial_to_task,
...
static void send_message_data_deliver_reply_unlocked(GTask* task,
                                                     GDBusMessage* reply)
{
    SendMessageData* data = g_task_get_task_data(task);
...
static void send_message_data_deliver_error(GTask* task, GQuark domain,
...
/* Can be called from any thread with or without lock held */
static void send_message_with_reply_cancelled_cb(GCancellable* cancellable,
                                                 gpointer user_data)
{
    GTask* task = user_data;
    GSource* idle_source;
     ...
     */
    idle_source = g_idle_source_new();
    g_source_set_name(idle_source,
                      "[gio] send_message_with_reply_cancelled_idle_cb");
    g_task_attach_source(task, idle_source,
...
static void g_dbus_connection_send_message_with_reply_unlocked(
    GDBusConnection* connection, GDBusMessage* message,
    ...
    GAsyncReadyCallback callback, gpointer user_data)
{
    ...
    if (!g_dbus_connection_send_message_unlocked(connection, message, flags,
                                                 out_serial, &error))
    {
        g_task_return_error(task, error);
        g_object_unref(task);
    ...
    if (cancellable != NULL)
    {
        data->cancellable_handler_id = g_cancellable_connect(
            cancellable, G_CALLBACK(send_message_with_reply_cancelled_cb),
    ...
    g_hash_table_insert(connection->map_method_serial_to_task,
                        GUINT_TO_POINTER(*out_serial), g_steal_pointer(&task));
}
    ...
    GAsyncReadyCallback callback, gpointer user_data)
{
    ...
    g_return_if_fail((flags & G_DBUS_SEND_MESSAGE_FLAGS_PRESERVE_SERIAL) ||
                     !g_dbus_message_get_locked(message));
    g_return_if_fail(timeout_msec >= 0 || timeout_msec == -1);

    CONNECTION_LOCK(connection);
    g_dbus_connection_send_message_with_reply_unlocked(
        connection, message, flags, timeout_msec, out_serial, cancellable,
        callback, user_data);
    CONNECTION_UNLOCK(connection);
}
...
/* Called from a user thread, lock is not held */
static void send_message_with_reply_sync_cb(GDBusConnection* connection,
                                            GAsyncResult* res,
                                            gpointer user_data)
{
    ...
    volatile guint32* out_serial, GCancellable* cancellable, GError** error)
{
    ...
    g_dbus_connection_send_message_with_reply(
        connection, message, flags, timeout_msec, out_serial, cancellable,
        (GAsyncReadyCallback)send_message_with_reply_sync_cb, &data);
    g_main_loop_run(data.loop);
...
static FilterData** copy_filter_list(GPtrArray* filters)
{
    FilterData** copy;
    guint n;
...
/* requires CONNECTION_LOCK */
static void free_filter_list(FilterData** filters)
{
    guint n;
...
static void on_worker_message_received(GDBusWorker* worker,
                                       GDBusMessage* message,
                                       gpointer user_data)
{
    GDBusConnection* connection;
    ...
    // g_debug ("boo ref_count = %d %p %p", G_OBJECT (connection)->ref_count,
    ...
    for (n = 0; filters[n]; n++)
    {
        message = filters[n]->filter_function(connection, message, TRUE,
                                              filters[n]->user_data);
        if (message == NULL)
            break;
        g_dbus_message_lock(message);
    ...
    if (message != NULL)
    {
        ...
        if (message_type == G_DBUS_MESSAGE_TYPE_METHOD_RETURN ||
            message_type == G_DBUS_MESSAGE_TYPE_ERROR)
        {
            ...
            if (task != NULL)
            {
                ...
                // g_debug ("delivering reply/error for serial %d for %p",
                // reply_serial, connection);
                send_message_data_deliver_reply_unlocked(task, message);
            }
            else
            {
                // g_debug ("message reply/error for serial %d but no
                // SendMessageData found for %p", reply_serial, connection);
            }
            CONNECTION_UNLOCK(connection);
        }
...
static GDBusMessage* on_worker_message_about_to_be_sent(GDBusWorker* worker,
                                                        GDBusMessage* message,
                                                        gpointer user_data)
{
    GDBusConnection* connection;
    ...
    {
        g_dbus_message_lock(message);
        message = filters[n]->filter_function(connection, message, FALSE,
                                              filters[n]->user_data);
        if (message == NULL)
            break;
    }
...
static gboolean cancel_method_on_close(gpointer key, gpointer value,
                                       gpointer user_data)
{
    GTask* task = value;
    SendMessageData* data = g_task_get_task_data(task);
...
static void on_worker_closed(GDBusWorker* worker, gboolean remote_peer_vanished,
                             GError* error, gpointer user_data)
{
    ...
    {
        g_hash_table_foreach_remove(connection->map_method_serial_to_task,
...
static gboolean initable_init(GInitable* initable, GCancellable* cancellable,
                              GError** error)
{
    ...
    if (connection->address != NULL)
    {
             ...
             G_DBUS_CONNECTION_FLAGS_AUTHENTICATION_ALLOW_ANONYMOUS))
        {
            g_set_error_literal(&connection->initialization_error, G_IO_ERROR,
                                ...
                                _("Unsupported flags encountered when "
                                  "constructing a client-side connection"));
            goto out;
        }

        connection->stream = g_dbus_address_get_stream_sync(
            connection->address, NULL, /* TODO: out_guid */
    ...
    {
        g_assert(!(connection->flags &
                   G_DBUS_CONNECTION_FLAGS_AUTHENTICATION_CLIENT));
        g_assert(connection->guid != NULL);
        connection->auth = _g_dbus_auth_new(connection->stream);
        if (!_g_dbus_auth_run_server(
                connection->auth, connection->authentication_observer,
                connection->guid,
                (connection->flags &
                 G_DBUS_CONNECTION_FLAGS_AUTHENTICATION_ALLOW_ANONYMOUS),
                get_offered_capabilities_max(connection),
                &connection->capabilities, &connection->credentials,
                cancellable, &connection->initialization_error))
            goto out;
    }
    else if (connection->flags & G_DBUS_CONNECTION_FLAGS_AUTHENTICATION_CLIENT)
    {
        ...
        connection->guid = _g_dbus_auth_run_client(
            connection->auth, connection->authentication_observer,
            get_offered_capabilities_max(connection), &connection->capabilities,
            cancellable, &connection->initialization_error);
    ...
    {
        g_socket_set_blocking(g_socket_connection_get_socket(
                                  G_SOCKET_CONNECTION(connection->stream)),
                              FALSE);
    }
#endif
    ...
    connection->worker = _g_dbus_worker_new(
        connection->stream, connection->capabilities,
        ((connection->flags &
          G_DBUS_CONNECTION_FLAGS_DELAY_MESSAGE_PROCESSING) != 0),
        on_worker_message_received, on_worker_message_about_to_be_sent,
        on_worker_closed, connection);

    /* if a bus connection, call org.freedesktop.DBus.Hello - this is how we're
     * getting a name */
    if (connection->flags & G_DBUS_CONNECTION_FLAGS_MESSAGE_BUS_CONNECTION)
    {
            ...
            G_DBUS_CONNECTION_FLAGS_DELAY_MESSAGE_PROCESSING)
        {
            g_set_error_literal(&connection->initialization_error, G_IO_ERROR,
                                G_IO_ERROR_FAILED,
                                "Cannot use DELAY_MESSAGE_PROCESSING with "
                                "MESSAGE_BUS_CONNECTION");
            goto out;
        }

        hello_result = g_dbus_connection_call_sync(
            connection, "org.freedesktop.DBus", /* name */
            "/org/freedesktop/DBus",            /* path */
            "org.freedesktop.DBus",             /* interface */
            "Hello", NULL,                      /* parameters */
            G_VARIANT_TYPE("(s)"), CALL_FLAGS_INITIALIZING, -1,
...
out:
    if (!ret)
    {
        ...
        g_propagate_error(error,
                          g_error_copy(connection->initialization_error));
    }
 ...
 * Since: 2.26
 */
void g_dbus_connection_new(GIOStream* stream, const gchar* guid,
                           GDBusConnectionFlags flags,
                           ...
                           GAsyncReadyCallback callback, gpointer user_data)
{
    ...
    g_async_initable_new_async(G_TYPE_DBUS_CONNECTION, G_PRIORITY_DEFAULT,
                               cancellable, callback, user_data, "stream",
                               stream, "guid", guid, "flags", flags,
...
GDBusConnection* g_dbus_connection_new_finish(GAsyncResult* res, GError** error)
{
    ...
    object = g_async_initable_new_finish(G_ASYNC_INITABLE(source_object), res,
                                         error);
...
GDBusConnection* g_dbus_connection_new_sync(
    GIOStream* stream, const gchar* guid, GDBusConnectionFlags flags,
    GDBusAuthObserver* observer, GCancellable* cancellable, GError** error)
{
    _g_dbus_initialize();
    g_return_val_if_fail(G_IS_IO_STREAM(stream), NULL);
    g_return_val_if_fail(error == NULL || *error == NULL, NULL);
    return g_initable_new(G_TYPE_DBUS_CONNECTION, cancellable, error, "stream",
                          stream, "guid", guid, "flags", flags,
                          "authentication-observer", observer, NULL);
}
                                       ...
                                       gpointer user_data)
{
    ...
    g_async_initable_new_async(G_TYPE_DBUS_CONNECTION, G_PRIORITY_DEFAULT,
                               cancellable, callback, user_data, "address",
                               address, "flags", flags,
                               "authentication-observer", observer, NULL);
}
...
GDBusConnection* g_dbus_connection_new_for_address_sync(
    const gchar* address, GDBusConnectionFlags flags,
    GDBusAuthObserver* observer, GCancellable* cancellable, GError** error)
{
    _g_dbus_initialize();
    ...
    g_return_val_if_fail(error == NULL || *error == NULL, NULL);
    return g_initable_new(G_TYPE_DBUS_CONNECTION, cancellable, error, "address",
                          address, "flags", flags, "authentication-observer",
                          observer, NULL);
}
                                   ...
                                   GDestroyNotify user_data_free_func)
{
    ...
    data->id =
        g_atomic_int_add(&_global_filter_id, 1); /* TODO: overflow etc. */
    data->ref_count = 1;
...
static void purge_all_filters(GDBusConnection* connection)
{
    ...
    for (n = 0; n < connection->filters->len; n++)
    {
        ...
        call_destroy_notify(data->context, data->user_data_free_func,
                            data->user_data);
        g_main_context_unref(data->context);
        g_free(data);
    }
                                     ...
                                     guint filter_id)
{
    ...
    {
        g_warning(
            "g_dbus_connection_remove_filter: No filter found for filter_id %d",
            filter_id);
    }
}
...
typedef struct
{
    gchar* rule;
    gchar* sender;
    gchar* sender_unique_name; /* if sender is unique or org.freedesktop.DBus,
                                  then that name... otherwise blank */
...
static gchar* args_to_rule(const gchar* sender, const gchar* interface_name,
                           const gchar* member, const gchar* object_path,
                           const gchar* arg0, GDBusSignalFlags flags)
{
    GString* rule;
...
static void add_match_rule(GDBusConnection* connection, const gchar* match_rule)
{
    ...
    error = NULL;
    if (!g_dbus_connection_send_message_unlocked(
            connection, message, G_DBUS_SEND_MESSAGE_FLAGS_NONE, NULL, &error))
    {
        g_critical("Error while sending AddMatch() message: %s",
                   error->message);
        g_error_free(error);
    }
    g_object_unref(message);
...
/* Called in a user thread, lock is held */
static void remove_match_rule(GDBusConnection* connection,
                              const gchar* match_rule)
{
            ...
            connection, message, G_DBUS_SEND_MESSAGE_FLAGS_NONE, NULL, &error))
    {
         ...
         */
        g_critical("Error while sending RemoveMatch() message: %s",
                   error->message);
        g_error_free(error);
    }
    g_object_unref(message);
...
{
    return g_strcmp0(signal_data->sender_unique_name, "org.freedesktop.DBus") ==
               0 &&
           g_strcmp0(signal_data->interface_name, "org.freedesktop.DBus") ==
               0 &&
 ...
 *     signal
 * @callback: callback to invoke when there is a signal matching the requested
 * data
 ...
 *
 * Returns: a subscription identifier that can be used with
    ...
    gpointer user_data, GDestroyNotify user_data_free_func)
{
    ...
    /* Right now we abort if AddMatch() fails since it can only fail with the
     * bus being in an OOM condition. We might want to change that but that
     * would involve making g_dbus_connection_signal_subscribe() asynchronous
     * and having the call sites handle that. And there's really no sensible way
     * of handling this short of retrying to add the match rule... and then
     * there's the little thing that, hey, maybe there's a reason the bus in an
     * OOM condition.
     *
    ...
    g_return_val_if_fail(
        interface_name == NULL || g_dbus_is_interface_name(interface_name), 0);
    g_return_val_if_fail(member == NULL || g_dbus_is_member_name(member), 0);
    g_return_val_if_fail(
        object_path == NULL || g_variant_is_object_path(object_path), 0);
    g_return_val_if_fail(callback != NULL, 0);
    g_return_val_if_fail(check_initialized(connection), 0);
    g_return_val_if_fail(!((flags & G_DBUS_SIGNAL_FLAGS_MATCH_ARG0_PATH) &&
                           (flags & G_DBUS_SIGNAL_FLAGS_MATCH_ARG0_NAMESPACE)),
                         0);
    g_return_val_if_fail(
        !(arg0 == NULL && (flags & (G_DBUS_SIGNAL_FLAGS_MATCH_ARG0_PATH |
     ...
     */
    rule =
        args_to_rule(sender, interface_name, member, object_path, arg0, flags);

    if (sender != NULL && (g_dbus_is_unique_name(sender) ||
                           g_strcmp0(sender, "org.freedesktop.DBus") == 0))
    ...
    subscriber.id =
        g_atomic_int_add(&_global_subscriber_id, 1); /* TODO: overflow etc. */
    subscriber.context = g_main_context_ref_thread_default();

    /* see if we've already have this rule */
    signal_data =
        g_hash_table_lookup(connection->map_rule_to_signal_data, rule);
    if (signal_data != NULL)
    {
    ...
    signal_data->subscribers =
        g_array_new(FALSE, FALSE, sizeof(SignalSubscriber));
    g_array_append_val(signal_data->subscribers, subscriber);

    g_hash_table_insert(connection->map_rule_to_signal_data, signal_data->rule,
                        signal_data);

    /* Add the match rule to the bus...
     *
     * Avoid adding match rules for NameLost and NameAcquired messages - the bus
     * will always send such messages to us.
     */
    if (connection->flags & G_DBUS_CONNECTION_FLAGS_MESSAGE_BUS_CONNECTION)
    {
    ...
    if (signal_data_array == NULL)
    {
        ...
        g_hash_table_insert(
            connection->map_sender_unique_name_to_signal_data_array,
            g_strdup(signal_data->sender_unique_name), signal_data_array);
    }
    g_ptr_array_add(signal_data_array, signal_data);

out:
    g_hash_table_insert(connection->map_id_to_signal_data,
                        GUINT_TO_POINTER(subscriber.id), signal_data);
 ...
 */
static void unsubscribe_id_internal(GDBusConnection* connection,
                                    ...
                                    GArray* out_removed_subscribers)
{
    ...
    if (signal_data == NULL)
    {
        /* Don't warn here, we may have thrown all subscriptions out when the
    ...
    for (n = 0; n < signal_data->subscribers->len; n++)
    {
        ...
        subscriber =
            &(g_array_index(signal_data->subscribers, SignalSubscriber, n));
        ...
        if (signal_data->subscribers->len == 0)
        {
            g_warn_if_fail(g_hash_table_remove(
                connection->map_rule_to_signal_data, signal_data->rule));
            ...
            /* remove the match rule from the bus unless NameLost or
             * NameAcquired (see subscribe()) */
            if ((connection->flags &
                 G_DBUS_CONNECTION_FLAGS_MESSAGE_BUS_CONNECTION) &&
...
/* called on delivery thread (e.g. where g_dbus_connection_signal_subscribe()
 * was called) with no locks held
 */
static gboolean emit_signal_instance_in_idle_cb(gpointer data)
{
  ...
  g_print ("in emit_signal_instance_in_idle_cb (id=%d sender=%s path=%s interface=%s member=%s params=%s)\n",
    ...
    if (has_subscription)
        signal_instance->callback(
            signal_instance->connection, signal_instance->sender,
            signal_instance->path, signal_instance->interface,
            signal_instance->member, parameters, signal_instance->user_data);
...
static gboolean namespace_rule_matches(const gchar* namespace,
                                       const gchar* name)
{
    gint len_namespace;
    gint len_name;
...
static gboolean path_rule_matches(const gchar* path_a, const gchar* path_b)
{
    gint len_a, len_b;
...
static void schedule_callbacks(GDBusConnection* connection,
                               GPtrArray* signal_data_array,
                               GDBusMessage* message, const gchar* sender)
{
    guint n, m;
    const gchar* interface;
    const gchar* member;
  ...
  g_print ("In schedule_callbacks:\n"
    ...
    for (n = 0; n < signal_data_array->len; n++)
    {
        ...
        if (signal_data->interface_name != NULL &&
            g_strcmp0(signal_data->interface_name, interface) != 0)
            continue;

        if (signal_data->member != NULL &&
            g_strcmp0(signal_data->member, member) != 0)
            continue;

        if (signal_data->object_path != NULL &&
            g_strcmp0(signal_data->object_path, path) != 0)
            continue;
        ...
        for (m = 0; m < signal_data->subscribers->len; m++)
        {
            ...
            subscriber =
                &(g_array_index(signal_data->subscribers, SignalSubscriber, m));
            ...
            idle_source = g_idle_source_new();
            g_source_set_priority(idle_source, G_PRIORITY_DEFAULT);
            g_source_set_callback(idle_source, emit_signal_instance_in_idle_cb,
                                  signal_instance,
                               ...
                               GDBusMessage* message)
{
    ...
    {
        signal_data_array = g_hash_table_lookup(
            connection->map_sender_unique_name_to_signal_data_array, sender);
        if (signal_data_array != NULL)
 ...
 */
static gboolean has_object_been_unregistered(GDBusConnection* connection,
                                             ...
                                             guint subtree_registration_id)
{
    ...
    if (registration_id != 0 &&
        g_hash_table_lookup(connection->map_id_to_ei,
                            GUINT_TO_POINTER(registration_id)) == NULL)
    {
        ret = TRUE;
    }
    else if (subtree_registration_id != 0 &&
             g_hash_table_lookup(connection->map_id_to_es,
...
static gboolean invoke_get_property_in_idle_cb(gpointer _data)
{
    ...
    if (has_object_been_unregistered(data->connection, data->registration_id,
                                     data->subtree_registration_id))
    {
        reply = g_dbus_message_new_method_error(
            data->message, "org.freedesktop.DBus.Error.UnknownMethod",
            _("No such interface “org.freedesktop.DBus.Properties” on object "
              "at path %s"),
            g_dbus_message_get_path(data->message));
        g_dbus_connection_send_message(data->connection, reply,
                                       G_DBUS_SEND_MESSAGE_FLAGS_NONE, NULL,
                                       NULL);
    ...
    error = NULL;
    value = data->vtable->get_property(
        data->connection, g_dbus_message_get_sender(data->message),
        g_dbus_message_get_path(data->message), data->interface_info->name,
    ...
    else
    {
        ...
        reply = g_dbus_message_new_method_error_literal(
            data->message, dbus_error_name, error->message);
        g_dbus_connection_send_message(data->connection, reply,
                                       G_DBUS_SEND_MESSAGE_FLAGS_NONE, NULL,
                                       NULL);
...
static gboolean invoke_set_property_in_idle_cb(gpointer _data)
{
    ...
    g_variant_get(g_dbus_message_get_body(data->message), "(ssv)", NULL, NULL,
                  &value);

    if (!data->vtable->set_property(
            data->connection, g_dbus_message_get_sender(data->message),
            g_dbus_message_get_path(data->message), data->interface_info->name,
...
static gboolean validate_and_maybe_schedule_property_getset(
    GDBusConnection* connection, GDBusMessage* message, guint registration_id,
    guint subtree_registration_id, gboolean is_get,
    ...
    GMainContext* main_context, gpointer user_data)
{
    ...
    if (is_get)
        ...
        g_variant_get(g_dbus_message_get_body(message), "(&s&sv)",
                      &interface_name, &property_name, NULL);

    if (vtable == NULL)
        goto out;

    /* Check that the property exists - if not fail with
    ...
    property_info =
        g_dbus_interface_info_lookup_property(interface_info, property_name);
    if (property_info == NULL)
    {
        reply = g_dbus_message_new_method_error(
            message, "org.freedesktop.DBus.Error.InvalidArgs",
            _("No such property “%s”"), property_name);
        g_dbus_connection_send_message_unlocked(
            connection, reply, G_DBUS_SEND_MESSAGE_FLAGS_NONE, NULL, NULL);
        g_object_unref(reply);
        handled = TRUE;
        goto out;
    }
    ...
    if (is_get && !(property_info->flags & G_DBUS_PROPERTY_INFO_FLAGS_READABLE))
    {
        reply = g_dbus_message_new_method_error(
            message, "org.freedesktop.DBus.Error.InvalidArgs",
            _("Property “%s” is not readable"), property_name);
        g_dbus_connection_send_message_unlocked(
            connection, reply, G_DBUS_SEND_MESSAGE_FLAGS_NONE, NULL, NULL);
        g_object_unref(reply);
        handled = TRUE;
        goto out;
             ...
             !(property_info->flags & G_DBUS_PROPERTY_INFO_FLAGS_WRITABLE))
    {
        reply = g_dbus_message_new_method_error(
            message, "org.freedesktop.DBus.Error.InvalidArgs",
            _("Property “%s” is not writable"), property_name);
        g_dbus_connection_send_message_unlocked(
            connection, reply, G_DBUS_SEND_MESSAGE_FLAGS_NONE, NULL, NULL);
        g_object_unref(reply);
        handled = TRUE;
    ...
    if (!is_get)
    {
        ...
        if (g_strcmp0(g_variant_get_type_string(value),
                      property_info->signature) != 0)
        {
            reply = g_dbus_message_new_method_error(
                message, "org.freedesktop.DBus.Error.InvalidArgs",
    ...
    if (is_get)
    {
        ...
        {
            schedule_method_call(connection, message, registration_id,
                                 subtree_registration_id, interface_info, NULL,
                                 property_info,
                                 g_dbus_message_get_body(message), vtable,
                                 main_context, user_data);
            handled = TRUE;
            goto out;
        }
    }
    ...
    g_source_set_callback(idle_source,
                          is_get ? invoke_get_property_in_idle_cb
                                 : invoke_set_property_in_idle_cb,
                          property_data, (GDestroyNotify)property_data_free);
    if (is_get)
...
static gboolean handle_getset_property(GDBusConnection* connection,
                                       ExportedObject* eo,
                                       GDBusMessage* message, gboolean is_get)
{
    ...
    if (ei == NULL)
    {
        ...
        reply = g_dbus_message_new_method_error(
            message, "org.freedesktop.DBus.Error.InvalidArgs",
            _("No such interface “%s”"), interface_name);
        g_dbus_connection_send_message_unlocked(
            eo->connection, reply, G_DBUS_SEND_MESSAGE_FLAGS_NONE, NULL, NULL);
        g_object_unref(reply);
        handled = TRUE;
        goto out;
    }

    handled = validate_and_maybe_schedule_property_getset(
        eo->connection, message, ei->id, 0, is_get, ei->interface_info,
        ei->vtable, ei->context, ei->user_data);
out:
    return handled;
}
...
static gboolean invoke_get_all_properties_in_idle_cb(gpointer _data)
{
     ...
     *       returns an error. We need clarification in the D-Bus spec about
     * this.
     */
    g_variant_builder_init(&builder, G_VARIANT_TYPE("(a{sv})"));
    g_variant_builder_open(&builder, G_VARIANT_TYPE("a{sv}"));
    for (n = 0; data->interface_info->properties != NULL &&
                data->interface_info->properties[n] != NULL;
         n++)
    {
        const GDBusPropertyInfo* property_info =
            data->interface_info->properties[n];
        GVariant* value;
        ...
        value = data->vtable->get_property(
            data->connection, g_dbus_message_get_sender(data->message),
            g_dbus_message_get_path(data->message), data->interface_info->name,
            property_info->name, NULL, data->user_data);
    ...
    interface_has_readable_properties(GDBusInterfaceInfo* interface_info)
{
    ...
    for (i = 0; interface_info->properties[i]; i++)
        if (interface_info->properties[i]->flags &
...
/* called in any thread with connection's lock held */
static gboolean validate_and_maybe_schedule_property_get_all(
    GDBusConnection* connection, GDBusMessage* message, guint registration_id,
    guint subtree_registration_id, GDBusInterfaceInfo* interface_info,
    const GDBusInterfaceVTable* vtable, GMainContext* main_context,
    gpointer user_data)
{
    gboolean handled;
    GSource* idle_source;
        ...
        interface_has_readable_properties(interface_info))
    {
        schedule_method_call(connection, message, registration_id,
                             subtree_registration_id, interface_info, NULL,
                             NULL, g_dbus_message_get_body(message), vtable,
                             main_context, user_data);
        handled = TRUE;
        goto out;
    }
    ...
    idle_source = g_idle_source_new();
    g_source_set_priority(idle_source, G_PRIORITY_DEFAULT);
    g_source_set_callback(idle_source, invoke_get_all_properties_in_idle_cb,
...
/* called in GDBusWorker thread with connection's lock held */
static gboolean handle_get_all_properties(GDBusConnection* connection,
                                          ExportedObject* eo,
                                          GDBusMessage* message)
{
    ExportedInterface* ei;
    ...
    handled = validate_and_maybe_schedule_property_get_all(
        eo->connection, message, ei->id, 0, ei->interface_info, ei->vtable,
        ei->context, ei->user_data);
out:
    return handled;
}
...
static void maybe_add_path(const gchar* path, gsize path_len,
                           const gchar* object_path, GHashTable* set)
{
    if (g_str_has_prefix(object_path, path) && strlen(object_path) > path_len &&
        object_path[path_len - 1] == '/')
    {
        const gchar* begin;
        const gchar* end;
        gchar* s;
...
static gchar** g_dbus_connection_list_registered(GDBusConnection* connection,
                                                 const gchar* path)
{
    gchar** ret;
    CONNECTION_LOCK(connection);
...
/* called in GDBusWorker thread with connection's lock held */
static gboolean handle_introspect(GDBusConnection* connection,
                                  ExportedObject* eo, GDBusMessage* message)
{
    guint n;
    GString* s;
    GDBusMessage* reply;
    ...
    s = g_string_sized_new(
        sizeof(introspect_header) + sizeof(introspect_properties_interface) +
        sizeof(introspect_introspectable_interface) + sizeof(introspect_tail));
    ...
    registered =
        g_dbus_connection_list_registered_unlocked(connection, eo->object_path);
...
static gboolean call_in_idle_cb(gpointer user_data)
{
    ...
    registration_id = GPOINTER_TO_UINT(
        g_object_get_data(G_OBJECT(invocation), "g-dbus-registration-id"));
    subtree_registration_id = GPOINTER_TO_UINT(g_object_get_data(
        G_OBJECT(invocation), "g-dbus-subtree-registration-id"));

    if (has_object_been_unregistered(
            g_dbus_method_invocation_get_connection(invocation),
            registration_id, subtree_registration_id))
    {
        GDBusMessage* reply;
        reply = g_dbus_message_new_method_error(
        ...
        g_dbus_connection_send_message(
            g_dbus_method_invocation_get_connection(invocation), reply,
            G_DBUS_SEND_MESSAGE_FLAGS_NONE, NULL, NULL);
        g_object_unref(reply);
        goto out;
    }
    ...
    GMainContext* main_context, gpointer user_data)
{
    ...
    invocation = _g_dbus_method_invocation_new(
        g_dbus_message_get_sender(message), g_dbus_message_get_path(message),
        g_dbus_message_get_interface(message),
        g_dbus_message_get_member(message), method_info, property_info,
        connection, message, parameters, user_data);

    /* TODO: would be nicer with a real MethodData like we already
     * have PropertyData and PropertyGetAllData... */
    g_object_set_data(G_OBJECT(invocation), "g-dbus-interface-vtable",
                      (gpointer)vtable);
    g_object_set_data(G_OBJECT(invocation), "g-dbus-registration-id",
                      GUINT_TO_POINTER(registration_id));
    g_object_set_data(G_OBJECT(invocation), "g-dbus-subtree-registration-id",
                      GUINT_TO_POINTER(subtree_registration_id));

    idle_source = g_idle_source_new();
    g_source_set_priority(idle_source, G_PRIORITY_DEFAULT);
    g_source_set_callback(idle_source, call_in_idle_cb, invocation,
                          g_object_unref);
...
/* called in GDBusWorker thread with connection's lock held */
static gboolean validate_and_maybe_schedule_method_call(
    GDBusConnection* connection, GDBusMessage* message, guint registration_id,
    guint subtree_registration_id, GDBusInterfaceInfo* interface_info,
    const GDBusInterfaceVTable* vtable, GMainContext* main_context,
    gpointer user_data)
{
    ...
    /* TODO: the cost of this is O(n) - it might be worth caching the result */
    method_info = g_dbus_interface_info_lookup_method(
        interface_info, g_dbus_message_get_member(message));

    /* if the method doesn't exist, return the
     * org.freedesktop.DBus.Error.UnknownMethod error to the caller
     */
    if (method_info == NULL)
    {
        reply = g_dbus_message_new_method_error(
            message, "org.freedesktop.DBus.Error.UnknownMethod",
            _("No such method “%s”"), g_dbus_message_get_member(message));
        g_dbus_connection_send_message_unlocked(
            connection, reply, G_DBUS_SEND_MESSAGE_FLAGS_NONE, NULL, NULL);
        g_object_unref(reply);
        handled = TRUE;
    ...
    /* Check that the incoming args are of the right type - if they are not,
     * return the org.freedesktop.DBus.Error.InvalidArgs error to the caller
     */
    ...
    schedule_method_call(connection, message, registration_id,
                         subtree_registration_id, interface_info, method_info,
                         NULL, parameters, vtable, main_context, user_data);
    g_variant_unref(parameters);
    handled = TRUE;
...
/* called in GDBusWorker thread with connection's lock held */
static gboolean obj_message_func(GDBusConnection* connection,
                                 ExportedObject* eo, GDBusMessage* message)
{
    ...
    if (interface_name != NULL)
    {
        ...
        if (ei != NULL)
        {
            ...
            handled = validate_and_maybe_schedule_method_call(
                connection, message, ei->id, 0, ei->interface_info, ei->vtable,
                ei->context, ei->user_data);
            goto out;
        }
    }
    ...
    if (g_strcmp0(interface_name, "org.freedesktop.DBus.Introspectable") == 0 &&
        g_strcmp0(member, "Introspect") == 0 && g_strcmp0(signature, "") == 0)
    {
        handled = handle_introspect(connection, eo, message);
        goto out;
    }
    else if (g_strcmp0(interface_name, "org.freedesktop.DBus.Properties") ==
                 0 &&
             g_strcmp0(member, "Get") == 0 && g_strcmp0(signature, "ss") == 0)
    {
        ...
        goto out;
                 ...
                 0 &&
             g_strcmp0(member, "Set") == 0 && g_strcmp0(signature, "ssv") == 0)
    {
        ...
        goto out;
                 ...
                 0 &&
             g_strcmp0(member, "GetAll") == 0 && g_strcmp0(signature, "s") == 0)
    {
    ...
    gpointer user_data, GDestroyNotify user_data_free_func, GError** error)
{
    ...
    if (eo == NULL)
    {
        ...
        g_hash_table_insert(connection->map_object_path_to_eo, eo->object_path,
                            eo);
    }
    ...
    if (ei != NULL)
    {
        g_set_error(
            ...
            interface_info->name, object_path);
        goto out;
    }

    ei = g_new0(ExportedInterface, 1);
    ei->id =
        g_atomic_int_add(&_global_registration_id, 1); /* TODO: overflow etc. */
    ei->eo = eo;
    ...
    g_hash_table_insert(eo->map_if_name_to_ei, (gpointer)ei->interface_name,
                        ei);
                                             ...
                                             guint registration_id)
{
    ...
    g_warn_if_fail(g_hash_table_remove(connection->map_id_to_ei,
                                       GUINT_TO_POINTER(ei->id)));
    g_warn_if_fail(
        g_hash_table_remove(eo->map_if_name_to_ei, ei->interface_name));
                             ...
                             GClosure* set_property_closure)
{
    ...
    if (method_call_closure != NULL)
    {
        ...
        if (G_CLOSURE_NEEDS_MARSHAL(method_call_closure))
            g_closure_set_marshal(method_call_closure,
    ...
    if (get_property_closure != NULL)
    {
        ...
        if (G_CLOSURE_NEEDS_MARSHAL(get_property_closure))
            g_closure_set_marshal(get_property_closure,
                                  g_cclosure_marshal_generic);
    }
    ...
    if (set_property_closure != NULL)
    {
        ...
        if (G_CLOSURE_NEEDS_MARSHAL(set_property_closure))
            g_closure_set_marshal(set_property_closure,
                                  g_cclosure_marshal_generic);
    }
...
static void register_object_free_func(gpointer user_data)
{
...
static void register_with_closures_on_method_call(
    GDBusConnection* connection, const gchar* sender, const gchar* object_path,
    const gchar* interface_name, const gchar* method_name, GVariant* parameters,
    GDBusMethodInvocation* invocation, gpointer user_data)
{
    RegisterObjectData* data = user_data;
    GValue params[] = {G_VALUE_INIT, G_VALUE_INIT, G_VALUE_INIT, G_VALUE_INIT,
                       G_VALUE_INIT, G_VALUE_INIT, G_VALUE_INIT};
    ...
    g_closure_invoke(data->method_call_closure, NULL, G_N_ELEMENTS(params),
                     params, NULL);
...
static GVariant* register_with_closures_on_get_property(
    GDBusConnection* connection, const gchar* sender, const gchar* object_path,
    ...
    gpointer user_data)
{
    ...
    g_closure_invoke(data->get_property_closure, &result_value,
    ...
    if (!result)
        g_set_error(error, G_DBUS_ERROR, G_DBUS_ERROR_FAILED,
                    _("Unable to retrieve property %s.%s"), interface_name,
                    property_name);

    return result;
}

static gboolean register_with_closures_on_set_property(
    GDBusConnection* connection, const gchar* sender, const gchar* object_path,
    const gchar* interface_name, const gchar* property_name, GVariant* value,
    GError** error, gpointer user_data)
{
    RegisterObjectData* data = user_data;
    GValue params[] = {G_VALUE_INIT, G_VALUE_INIT, G_VALUE_INIT,
                       G_VALUE_INIT, G_VALUE_INIT, G_VALUE_INIT};
    GValue result_value = G_VALUE_INIT;
    gboolean result;
    ...
    g_closure_invoke(data->set_property_closure, &result_value,
                     G_N_ELEMENTS(params), params, NULL);
    ...
    if (!result)
        g_set_error(error, G_DBUS_ERROR, G_DBUS_ERROR_FAILED,
                    _("Unable to set property %s.%s"), interface_name,
                    property_name);
...
/**
 * g_dbus_connection_register_object_with_closures: (rename-to
 * g_dbus_connection_register_object)
 * @connection: A #GDBusConnection.
 * @object_path: The object path to register at.
 * @interface_info: Introspection data for the interface.
 * @method_call_closure: (nullable): #GClosure for handling incoming method
 * calls.
...
guint g_dbus_connection_register_object_with_closures(
    GDBusConnection* connection, const gchar* object_path,
    GDBusInterfaceInfo* interface_info, GClosure* method_call_closure,
    GClosure* get_property_closure, GClosure* set_property_closure,
    GError** error)
{
    RegisterObjectData* data;
    GDBusInterfaceVTable vtable = {
        method_call_closure != NULL ? register_with_closures_on_method_call
                                    : NULL,
        get_property_closure != NULL ? register_with_closures_on_get_property
                                     : NULL,
        set_property_closure != NULL ? register_with_closures_on_set_property
                                     : NULL};

    data = register_object_data_new(method_call_closure, get_property_closure,
                                    set_property_closure);

    return g_dbus_connection_register_object(connection, object_path,
                                             interface_info, &vtable, data,
                                             register_object_free_func, error);
}
                                       ...
                                       GVariant* parameters, GError** error)
{
    ...
    g_return_val_if_fail(G_IS_DBUS_CONNECTION(connection), FALSE);
    g_return_val_if_fail(destination_bus_name == NULL ||
                             g_dbus_is_name(destination_bus_name),
                         FALSE);
    g_return_val_if_fail(
        object_path != NULL && g_variant_is_object_path(object_path), FALSE);
    g_return_val_if_fail(interface_name != NULL &&
                             g_dbus_is_interface_name(interface_name),
                         FALSE);
    g_return_val_if_fail(
        signal_name != NULL && g_dbus_is_member_name(signal_name), FALSE);
    g_return_val_if_fail(
        parameters == NULL ||
    ...
    message =
        g_dbus_message_new_signal(object_path, interface_name, signal_name);
    ...
    ret = g_dbus_connection_send_message(
        connection, message, G_DBUS_SEND_MESSAGE_FLAGS_NONE, NULL, error);
    g_object_unref(message);
...
static GVariant* decode_method_reply(GDBusMessage* reply,
        ...
        case G_DBUS_MESSAGE_TYPE_METHOD_RETURN:
            ...
            if (!g_variant_is_of_type(result, reply_type))
            {
                ...
                g_set_error(
                    ...
                    method_name, g_variant_get_type_string(result),
                    type_string);
...
static void call_state_free(CallState* state)
{
...
static void g_dbus_connection_call_done(GObject* source, GAsyncResult* result,
                                        gpointer user_data)
{
    ...
    if (G_UNLIKELY(_g_dbus_debug_call()))
    {
        ...
        g_print("=============================================================="
                ...
                "      ",
                state->method_name, g_dbus_message_get_reply_serial(reply));
        if (reply != NULL)
        {
            g_print("SUCCESS\n");
        }
    ...
    if (reply != NULL)
        value = decode_method_reply(reply, state->method_name,
                                    state->reply_type, &state->fd_list, &error);
...
/* called in any thread, with the connection's lock not held */
static void g_dbus_connection_call_internal(
    ...
    GAsyncReadyCallback callback, gpointer user_data)
{
    ...
    if (callback != NULL)
    {
        ...
        g_dbus_connection_send_message_with_reply(
            connection, message, G_DBUS_SEND_MESSAGE_FLAGS_NONE, timeout_msec,
            &serial, cancellable, g_dbus_connection_call_done, task);
    }
    else
    {
        ...
        g_dbus_connection_send_message(
            connection, message, G_DBUS_SEND_MESSAGE_FLAGS_NONE, &serial, NULL);
    }
    ...
    if (G_UNLIKELY(_g_dbus_debug_call()))
    {
        ...
        g_print("=============================================================="
                ...
                bus_name != NULL ? bus_name : "(none)", serial);
        _g_dbus_debug_print_unlock();
    }
                                           ...
                                           GAsyncResult* res, GError** error)
{
    ...
    if (out_fd_list != NULL)
        *out_fd_list =
            state->fd_list != NULL ? g_object_ref(state->fd_list) : NULL;
    return ret;
}

/* called in any user thread, with the connection's lock not held */
static GVariant* g_dbus_connection_call_sync_internal(
    GDBusConnection* connection, const gchar* bus_name,
    const gchar* object_path, const gchar* interface_name,
    const gchar* method_name, GVariant* parameters,
    const GVariantType* reply_type, GDBusCallFlags flags, gint timeout_msec,
    GUnixFDList* fd_list, GUnixFDList** out_fd_list, GCancellable* cancellable,
    GError** error)
{
    GDBusMessage* message;
    GDBusMessage* reply;
    ...
    g_return_val_if_fail(
        object_path != NULL && g_variant_is_object_path(object_path), NULL);
    g_return_val_if_fail(interface_name != NULL &&
                             g_dbus_is_interface_name(interface_name),
                         NULL);
    g_return_val_if_fail(
        method_name != NULL && g_dbus_is_member_name(method_name), NULL);
    g_return_val_if_fail(timeout_msec >= 0 || timeout_msec == -1, NULL);
    g_return_val_if_fail(
        (parameters == NULL) ||
            g_variant_is_of_type(parameters, G_VARIANT_TYPE_TUPLE),
    ...
    reply = g_dbus_connection_send_message_with_reply_sync(
        connection, message, send_flags, timeout_msec,
    ...
    result =
        decode_method_reply(reply, method_name, reply_type, out_fd_list, error);
 ...
 * you give a @reply_type then any non-%NULL return value will be of this type.
...
void g_dbus_connection_call(GDBusConnection* connection, const gchar* bus_name,
                            ...
                            GAsyncReadyCallback callback, gpointer user_data)
{
    g_dbus_connection_call_internal(connection, bus_name, object_path,
                                    interface_name, method_name, parameters,
                                    reply_type, flags, timeout_msec, NULL,
...
GVariant* g_dbus_connection_call_sync(
    ...
    GCancellable* cancellable, GError** error)
{
    return g_dbus_connection_call_sync_internal(
        connection, bus_name, object_path, interface_name, method_name,
        parameters, reply_type, flags, timeout_msec, NULL, NULL, cancellable,
        error);
}
    ...
    GAsyncReadyCallback callback, gpointer user_data)
{
    g_dbus_connection_call_internal(connection, bus_name, object_path,
                                    interface_name, method_name, parameters,
                                    reply_type, flags, timeout_msec, fd_list,
                                    cancellable, callback, user_data);
}
...
GVariant* g_dbus_connection_call_with_unix_fd_list_finish(
    GDBusConnection* connection, GUnixFDList** out_fd_list, GAsyncResult* res,
    GError** error)
{
    return g_dbus_connection_call_finish_internal(connection, out_fd_list, res,
                                                  error);
}
    ...
    GError** error)
{
    return g_dbus_connection_call_sync_internal(
        connection, bus_name, object_path, interface_name, method_name,
        parameters, reply_type, flags, timeout_msec, fd_list, out_fd_list,
        cancellable, error);
}
 ...
 * the subtree
 */
static gboolean handle_subtree_introspect(GDBusConnection* connection,
                                          ExportedSubtree* es,
                                          GDBusMessage* message)
{
    GString* s;
     ...
     */
    children = es->vtable->enumerate(es->connection, sender, es->object_path,
                                     es->user_data);
    ...
    interfaces = es->vtable->introspect(es->connection, sender, es->object_path,
                                        requested_node, es->user_data);
    if (interfaces != NULL)
    {
    ...
    /* finally include nodes registered below us */
    subnode_paths = g_dbus_connection_list_registered(es->connection,
 ...
 * the subtree
 */
static gboolean handle_subtree_method_invocation(GDBusConnection* connection,
                                                 ExportedSubtree* es,
                                                 GDBusMessage* message)
{
    gboolean handled;
    ...
    if (g_strcmp0(interface_name, "org.freedesktop.DBus.Properties") == 0)
    {
        ...
        else if (g_strcmp0(member, "Set") == 0 &&
    ...
    if (!is_root)
    {
        ...
        if (~es->flags & G_DBUS_SUBTREE_FLAGS_DISPATCH_TO_UNENUMERATED_NODES)
        {
            ...
            children = es->vtable->enumerate(es->connection, sender,
                                             es->object_path, es->user_data);

            exists = _g_strv_has_string((const gchar* const*)children,
                                        requested_node);
            g_strfreev(children);
    ...
    interfaces =
        es->vtable->introspect(es->connection, sender, requested_object_path,
                               requested_node, es->user_data);
    ...
    if (interface_info != NULL)
    {
        ...
        interface_vtable = es->vtable->dispatch(
            es->connection, sender, es->object_path, interface_name,
            requested_node, &interface_user_data, es->user_data);
        if (interface_vtable == NULL)
            goto out;

        CONNECTION_LOCK(connection);
        handled = validate_and_maybe_schedule_method_call(
            es->connection, message, 0, es->id, interface_info,
            interface_vtable, es->context, interface_user_data);
        CONNECTION_UNLOCK(connection);
    }
    /* handle org.freedesktop.DBus.Properties interface if not explicitly
     * handled */
    else if (is_property_get || is_property_set || is_property_get_all)
    {
        if (is_property_get)
            ...
            g_variant_get(g_dbus_message_get_body(message), "(&s&sv)",
                          &interface_name, NULL, NULL);
        else if (is_property_get_all)
            g_variant_get(g_dbus_message_get_body(message), "(&s)",
                          &interface_name, NULL, NULL);
        else
            g_assert_not_reached();
        ...
        if (interface_info == NULL)
        {
            ...
            reply = g_dbus_message_new_method_error(
                message, "org.freedesktop.DBus.Error.InvalidArgs",
                _("No such interface “%s”"), interface_name);
            g_dbus_connection_send_message(es->connection, reply,
                                           G_DBUS_SEND_MESSAGE_FLAGS_NONE, NULL,
                                           NULL);
            g_object_unref(reply);
            handled = TRUE;
        ...
        if (interface_vtable == NULL)
        {
            g_warning("The subtree introspection function indicates that '%s' "
                      ...
                      "function on that interface gave us NULL",
                      interface_name);
            goto out;
        }
        ...
        if (is_property_get || is_property_set)
        {
            CONNECTION_LOCK(connection);
            handled = validate_and_maybe_schedule_property_getset(
                es->connection, message, 0, es->id, is_property_get,
                interface_info, interface_vtable, es->context,
                interface_user_data);
            CONNECTION_UNLOCK(connection);
        }
        else if (is_property_get_all)
...
/* called without lock held in the thread where the caller registered the
 * subtree */
static gboolean process_subtree_vtable_message_in_idle_cb(gpointer _data)
{
    ...
    if (g_strcmp0(g_dbus_message_get_interface(data->message),
                  "org.freedesktop.DBus.Introspectable") == 0 &&
        g_strcmp0(g_dbus_message_get_member(data->message), "Introspect") ==
            0 &&
    ...
    if (!handled)
    {
        ...
        g_dbus_connection_send_message(data->es->connection, reply,
                                       G_DBUS_SEND_MESSAGE_FLAGS_NONE, NULL,
                                       NULL);
        g_object_unref(reply);
    }
...
/* called in GDBusWorker thread with connection's lock held */
static gboolean subtree_message_func(GDBusConnection* connection,
                                     ExportedSubtree* es, GDBusMessage* message)
{
    GSource* idle_source;
    ...
    g_source_set_callback(idle_source,
                          process_subtree_vtable_message_in_idle_cb, data,
    ...
    /* since we own the entire subtree, handlers for objects not in the subtree
     * have been tried already by libdbus-1 - so we just need to ensure that
     * we're always going to reply to the message
     */
    return TRUE;
}
...
guint g_dbus_connection_register_subtree(
    GDBusConnection* connection, const gchar* object_path,
    const GDBusSubtreeVTable* vtable, GDBusSubtreeFlags flags,
    gpointer user_data, GDestroyNotify user_data_free_func, GError** error)
{
    guint ret;
    ExportedSubtree* es;
    ...
    if (es != NULL)
    {
        g_set_error(error, G_IO_ERROR, G_IO_ERROR_EXISTS,
                    _("A subtree is already exported for %s"), object_path);
        goto out;
    }
    ...
    es->flags = flags;
    es->id = g_atomic_int_add(&_global_subtree_registration_id,
                              1); /* TODO: overflow etc. */
                                              ...
                                              guint registration_id)
{
    ...
    g_warn_if_fail(g_hash_table_remove(connection->map_id_to_es,
                                       GUINT_TO_POINTER(es->id)));
    g_warn_if_fail(g_hash_table_remove(connection->map_object_path_to_es,
                                       es->object_path));
...
static void handle_generic_ping_unlocked(GDBusConnection* connection,
                                         const gchar* object_path,
                                         GDBusMessage* message)
{
    GDBusMessage* reply;
...
/* may be called in any thread, with connection's lock held */
static void handle_generic_get_machine_id_unlocked(GDBusConnection* connection,
                                                   const gchar* object_path,
                                                   GDBusMessage* message)
{
    GDBusMessage* reply;
    ...
    if (connection->machine_id == NULL)
    {
        ...
        {
            reply = g_dbus_message_new_method_error_literal(
                message, "org.freedesktop.DBus.Error.Failed", error->message);
            g_error_free(error);
        }
    }
    ...
    if (reply == NULL)
    {
        ...
        g_dbus_message_set_body(reply,
                                g_variant_new("(s)", connection->machine_id));
    }
    g_dbus_connection_send_message_unlocked(
        connection, reply, G_DBUS_SEND_MESSAGE_FLAGS_NONE, NULL, NULL);
    g_object_unref(reply);
...
/* may be called in any thread, with connection's lock held */
static void handle_generic_introspect_unlocked(GDBusConnection* connection,
                                               const gchar* object_path,
                                               GDBusMessage* message)
{
    guint n;
    ...
    registered =
        g_dbus_connection_list_registered_unlocked(connection, object_path);
    for (n = 0; registered != NULL && registered[n] != NULL; n++)
        g_string_append_printf(s, "  <node name=\"%s\"/>\n", registered[n]);
    g_strfreev(registered);
    g_string_append(s, "</node>\n");
                                        ...
                                        GDBusMessage* message)
{
        ...
        g_strcmp0(member, "Introspect") == 0 && g_strcmp0(signature, "") == 0)
    {
        ...
        handled = TRUE;
             ...
             g_strcmp0(member, "Ping") == 0 && g_strcmp0(signature, "") == 0)
    {
                                   ...
                                   GDBusMessage* message)
{
    ...
    if (G_UNLIKELY(_g_dbus_debug_incoming()))
    {
        ...
        g_print("=============================================================="
                ...
                g_dbus_message_get_sender(message) != NULL
    ...
    {
        es = g_hash_table_lookup(connection->map_object_path_to_es,
                                 subtree_path);
        if (es != NULL)
        {
            if (subtree_message_func(connection, es, message))
    ...
    /* if we end up here, the message has not been not handled - so return an
     * error saying this */
    reply = g_dbus_message_new_method_error(
        message, "org.freedesktop.DBus.Error.UnknownMethod",
        _("No such interface “%s” on object at path %s"), interface_name,
        object_path);
    g_dbus_connection_send_message_unlocked(
        connection, reply, G_DBUS_SEND_MESSAGE_FLAGS_NONE, NULL, NULL);
    g_object_unref(reply);
...
static GWeakRef* message_bus_get_singleton(GBusType bus_type, GError** error)
{
    GWeakRef* ret;
            ...
            else
            {
                if (starter_bus != NULL)
                {
                    g_set_error(error, G_IO_ERROR, G_IO_ERROR_INVALID_ARGUMENT,
                                _("Cannot determine bus address from "
                                  "DBUS_STARTER_BUS_TYPE environment variable"
                                  " — unknown value “%s”"),
                                starter_bus);
                }
                else
                {
                    g_set_error_literal(
                        error, G_IO_ERROR, G_IO_ERROR_INVALID_ARGUMENT,
                        _("Cannot determine bus address because the "
                          "DBUS_STARTER_BUS_TYPE environment "
                          "variable is not set"));
                }
            }
            break;
                                                     ...
                                                     GError** error)
{
    ...
    if (ret == NULL)
    {
        ...
        if (address == NULL)
            goto out;
        ret = g_object_new(G_TYPE_DBUS_CONNECTION, "address", address, "flags",
                           G_DBUS_CONNECTION_FLAGS_AUTHENTICATION_CLIENT |
                               G_DBUS_CONNECTION_FLAGS_MESSAGE_BUS_CONNECTION,
                           "exit-on-close", TRUE, NULL);
...
static void bus_get_async_initable_cb(GObject* source_object, GAsyncResult* res,
                                      gpointer user_data)
{
    GTask* task = user_data;
    GError* error = NULL;

    if (!g_async_initable_init_finish(G_ASYNC_INITABLE(source_object), res,
                                      &error))
    {
        g_assert(error != NULL);
        g_task_return_error(task, error);
 ...
 * Since: 2.26
 */
void g_bus_get(GBusType bus_type, GCancellable* cancellable,
               GAsyncReadyCallback callback, gpointer user_data)
{
    GDBusConnection* connection;
    GTask* task;
    ...
    else
    {
        g_async_initable_init_async(G_ASYNC_INITABLE(connection),
                                    ...
                                    bus_get_async_initable_cb, task);
    }
}
