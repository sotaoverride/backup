...
G_DEFINE_TYPE_WITH_CODE(GInetAddressMask, g_inet_address_mask, G_TYPE_OBJECT,
                        G_ADD_PRIVATE(GInetAddressMask) G_IMPLEMENT_INTERFACE(
                            G_TYPE_INITABLE,
...
static void g_inet_address_mask_set_property(GObject* object, guint prop_id,
                                             const GValue* value,
                                             GParamSpec* pspec)
{
...
static void g_inet_address_mask_get_property(GObject* object, guint prop_id,
    ...
    switch (prop_id)
    {
        case PROP_FAMILY:
            g_value_set_enum(value,
                             g_inet_address_get_family(mask->priv->addr));
            break;
...
static void g_inet_address_mask_dispose(GObject* object)
{
    GInetAddressMask* mask = G_INET_ADDRESS_MASK(object);
...
static void g_inet_address_mask_class_init(GInetAddressMaskClass* klass)
{
    ...
    g_object_class_install_property(
        ...
        g_param_spec_object("address", P_("Address"), P_("The base address"),
                            G_TYPE_INET_ADDRESS,
                            G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS));
    g_object_class_install_property(
        gobject_class, PROP_LENGTH,
        g_param_spec_uint("length", P_("Length"), P_("The prefix length"), 0,
                          128, 0, G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS));
}

static gboolean g_inet_address_mask_initable_init(GInitable* initable,
                                                  ...
                                                  GError** error)
{
    ...
    if (mask->priv->length > addrlen * 8)
    {
        g_set_error(error, G_IO_ERROR, G_IO_ERROR_INVALID_ARGUMENT,
                    _("Length %u is too long for address"), mask->priv->length);
        return FALSE;
    }
...
static void g_inet_address_mask_init(GInetAddressMask* mask)
{
 ...
 * Since: 2.32
 */
GInetAddressMask* g_inet_address_mask_new(GInetAddress* addr, guint length,
                                          GError** error)
{
    return g_initable_new(G_TYPE_INET_ADDRESS_MASK, NULL, error, "address",
                          addr, "length", length, NULL);
}
...
gchar* g_inet_address_mask_to_string(GInetAddressMask* mask)
{
    ...
    if (mask->priv->length ==
        (g_inet_address_get_native_size(mask->priv->addr) * 8))
        return addr_string;
