...
static void finish_dispatch(struct dispatch_state* state, krb5_error_code code,
                            krb5_data* response)
{
...
static void finish_dispatch_cache(void* arg, krb5_error_code code,
                                  krb5_data* response)
{
...
static void reseed_random(krb5_context kdc_err_context)
{
    ...
    if (retval == 0)
    {
        ...
        krb5_c_random_add_entropy(kdc_err_context, KRB5_C_RANDSOURCE_TIMING,
                                  &data);
        last_usec = now_usec;
    }
}

void dispatch(void* cb, const krb5_fulladdr* local_addr,
