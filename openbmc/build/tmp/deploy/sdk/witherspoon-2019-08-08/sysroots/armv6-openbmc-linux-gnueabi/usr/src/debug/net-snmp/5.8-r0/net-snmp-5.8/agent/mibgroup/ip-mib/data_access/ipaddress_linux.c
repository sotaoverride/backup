...
netsnmp_feature_require(prefix_info) netsnmp_feature_require(find_prefix_info)
...
int _load_v6(netsnmp_container* container, int idx_offset)
{
    ...
    char if_name[IFNAMSIZ +
                 1]; /* +1 for '\0' because of the ugly sscanf below */
    u_char* buf;
...
#define PROCFILE "/proc/net/if_inet6"
    ...
    while (fgets(line, sizeof(line), in))
    {
        ...
        rc = sscanf(
            line,
            "%39s %08x %08x %04x %02x %" SNMP_MACRO_VAL_TO_STR(IFNAMSIZ) "s\n",
            addr, &if_index, &pfx_len, &scope, &flags, if_name);
        if (6 != rc)
        {
            snmp_log(LOG_ERR,
                     PROCFILE " data format error (%d!=6), line ==|%s|\n", rc,
                     line);
            continue;
        }
        DEBUGMSGTL(
            ("access:ipaddress:container",
             ...
             if_index, pfx_len, scope, flags, if_name));
        /*
         */
        ...
        */
        ...
        else
        {
            ...
            DEBUGMSGTL(
                ("access:ipaddress:ipv6", "unknown flags 0x%x\n", flags));
        }
...
#ifdef HAVE_LINUX_RTNETLINK_H
        if (netsnmp_access_ipaddress_extra_prefix_info(
                entry->if_index, &entry->ia_prefered_lifetime,
                &entry->ia_valid_lifetime, addr) < 0)
        {
            DEBUGMSGTL(("access:ipaddress:container",
                        "unable to fetch extra prefix info\n"));
        }
#else
...
#ifdef SUPPORT_PREFIX_FLAGS
        {
            ...
            if (net_snmp_find_prefix_info(&prefix_head_list, addr,
                                          &prefix_val) < 0)
            {
                DEBUGMSGTL(
                    ("access:ipaddress:container", "unable to find info\n"));
            ...
            else
            {
                ...
                entry->ia_autonomous_flag =
                    prefix_val.ipAddressPrefixAutonomousFlag;
            }
        }
#else
        entry->ia_onlink_flag = 1;     /*Set by default as true*/
        ...
        if (index == rtmp->ifa_index)
        {
            for (; RTA_OK(rtatp, rtattrlen); rtatp = RTA_NEXT(rtatp, rtattrlen))
            {
                if (rtatp->rta_type == IFA_ADDRESS)
                {
                    ...
                    sprintf(tmpaddr, "%04x%04x%04x%04x%04x%04x%04x%04x",
                            NIP6(*in6p));
