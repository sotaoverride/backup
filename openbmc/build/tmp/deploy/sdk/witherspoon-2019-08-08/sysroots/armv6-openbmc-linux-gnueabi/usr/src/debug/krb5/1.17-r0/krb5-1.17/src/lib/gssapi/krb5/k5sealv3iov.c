    ...
    int* conf_state, gss_iov_buffer_desc* iov, int iov_count, int toktype)
{
    ...
    key_usage =
        (toktype == KG_TOK_WRAP_MSG ? (ctx->initiate ? KG_USAGE_INITIATOR_SEAL
                                                     : KG_USAGE_ACCEPTOR_SEAL)
                                    : (ctx->initiate ? KG_USAGE_INITIATOR_SIGN
    ...
    if (toktype == KG_TOK_WRAP_MSG && conf_req_flag)
    {
        ...
        /* flags */
        outbuf[2] =
            (acceptor_flag | (conf_req_flag ? FLAG_WRAP_CONFIDENTIAL : 0) |
        ...
        /* EC | copy of header to be encrypted, located in (possibly rotated)
         * trailer */
        if (trailer == NULL)
        ...
        code = kg_encrypt_iov(context, ctx->proto,
                              ((ctx->gss_flags & GSS_C_DCE_STYLE) != 0), ec,
                              rrc, key, key_usage, 0, iov, iov_count);
        if (code != 0)
            goto cleanup;
    ...
    wrap_with_checksum:
        ...
        code = krb5_c_crypto_length(context, key->keyblock.enctype,
                                    KRB5_CRYPTO_TYPE_CHECKSUM, &gss_trailerlen);
        if (code != 0)
            goto cleanup;
        ...
        code = kg_make_checksum_iov_v3(context, cksumtype, rrc, key, key_usage,
                                    ...
                                    int toktype)
{
    ...
    key_usage = (toktype == KG_TOK_WRAP_MSG
                     ? (!ctx->initiate ? KG_USAGE_INITIATOR_SEAL
                                       : KG_USAGE_ACCEPTOR_SEAL)
                     : (!ctx->initiate ? KG_USAGE_INITIATOR_SIGN
                                       : KG_USAGE_ACCEPTOR_SIGN));
    ...
    if (toktype == KG_TOK_WRAP_MSG)
    {
        ...
        if (conf_flag)
        {
            ...
            if (load_16_be(althdr) != KG2_TOK_WRAP_MSG || althdr[2] != ptr[2] ||
                althdr[3] != ptr[3] || memcmp(althdr + 8, ptr + 8, 8) != 0)
            {
                *minor_status = 0;
                return GSS_S_BAD_SIG;
            }
        ...
        else
        {
            ...
            code = kg_verify_checksum_iov_v3(context, cksumtype, rrc, key,
                                             key_usage, iov, iov_count, toktype,
                                             &valid);
    ...
    verify_mic_1:
        ...
        code =
            kg_verify_checksum_iov_v3(context, cksumtype, rrc, key, key_usage,
