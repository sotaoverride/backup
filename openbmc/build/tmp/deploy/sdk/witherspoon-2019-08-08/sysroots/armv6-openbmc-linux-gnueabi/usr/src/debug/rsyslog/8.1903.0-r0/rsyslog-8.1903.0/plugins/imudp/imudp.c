...
DEFobjCurrIf(glbl) DEFobjCurrIf(net) DEFobjCurrIf(datetime) DEFobjCurrIf(prop)
...
static int bDoACLCheck; /* are ACL checks neeed? Cached once immediately before
                           listener startup */
static int iMaxLine;    /* maximum UDP message size supported */
#define BATCH_SIZE_DFLT                                                        \
    32 /* do not overdo, has heavy toll on memory, especially with large msgs  \
        */
#define TIME_REQUERY_DFLT 2
#define SCHED_PRIO_UNSET                                                       \
    -12345678 /* a value that indicates that the scheduling priority has not   \
                 been set */
...
static struct configSettings_s
{
    ...
    int iTimeRequery; /* how often is time to be queried inside tight recv loop?
                         0=always */
} cs;
...
static rsRetVal addInstance(void __attribute__((unused)) * pVal, uchar* pNewVal)
{
    ...
    CHKmalloc(inst->pszBindPort =
                  ustrdup((pNewVal == NULL || *pNewVal == '\0') ? (uchar*)"514"
                                                                : pNewVal));
...
static rsRetVal addListner(instanceConf_t* inst)
{
    ...
    /* check which address to bind to. We could do this more compact, but have
     * not done so in order to make the code more readable. -- rgerhards,
     * 2007-12-27
     */
    ...
    port = (inst->pszBindPort == NULL || *inst->pszBindPort == '\0')
               ? (uchar*)"514"
               : inst->pszBindPort;
    ...
    newSocks = net.create_udp_socket(bindAddr, port, 1, inst->rcvbuf, 0,
                                     inst->ipfreebind, inst->pszBindDevice);
    if (newSocks != NULL)
    {
        ...
        {
            CHKmalloc(newlcnfinfo =
                          (struct lstn_s*)calloc(1, sizeof(struct lstn_s)));
            ...
            snprintf((char*)dispname, sizeof(dispname), "%s(%s:%s)", inputname,
                     bindName, port);
            dispname[sizeof(dispname) - 1] =
                '\0'; /* just to be on the save side... */
            CHKiRet(
                ratelimitNew(&newlcnfinfo->ratelimiter, (char*)dispname, NULL));
            ...
            STATSCOUNTER_INIT(newlcnfinfo->ctrSubmit,
                              newlcnfinfo->mutCtrSubmit);
            CHKiRet(statsobj.AddCounter(
                newlcnfinfo->stats, UCHAR_CONSTANT("submitted"), ctrType_IntCtr,
                CTR_FLAG_RESETTABLE, &(newlcnfinfo->ctrSubmit)));
            STATSCOUNTER_INIT(newlcnfinfo->ctrDisallowed,
                              newlcnfinfo->mutCtrDisallowed);
            CHKiRet(statsobj.AddCounter(newlcnfinfo->stats,
                                        UCHAR_CONSTANT("disallowed"),
                                        ctrType_IntCtr, CTR_FLAG_RESETTABLE,
                                        &(newlcnfinfo->ctrDisallowed)));
...
static inline void std_checkRuleset_genErrMsg(__attribute__((unused))
                                              modConfData_t* modConf,
                                              instanceConf_t* inst)
{
    LogError(0, NO_ERRCODE,
...
static rsRetVal processPacket(struct lstn_s* lstn,
                              ...
                              int* pbIsPermitted, uchar* rcvBuf,
                              ssize_t lenRcvBuf, struct syslogTime* stTime,
                              time_t ttGenTime,
                              struct sockaddr_storage* frominet,
                              socklen_t socklen, multi_submit_t* multiSub)
{
    DEFiRet;
    smsg_t* pMsg = NULL;
    ...
    /* if we reach this point, we had a good receive and can process the packet
     * received */
    /* check if we have a different sender than before, if so, we need to query
     * some new values */
    if (bDoACLCheck)
    {
        ...
        {
            memcpy(frominetPrev, frominet,
                   socklen); /* update cache indicator */
            /* Here we check if a host is permitted to send us syslog messages.
             * If it isn't, we do not further process the message but log a
             * warning (if we are configured to do this). However, if the check
             * would require name resolution, it is postponed to the main queue.
             * See also my blog post at
             * http://blog.gerhards.net/2009/11/acls-imudp-and-accepting-messages.html
             * rgerhards, 2009-11-16
             */
            *pbIsPermitted = net.isAllowedSender2(
                (uchar*)"UDP", (struct sockaddr*)frominet, "", 0);
    ...
    DBGPRINTF("recv(%d,%d),acl:%d,msg:%.*s\n", lstn->sock, (int)lenRcvBuf,
              *pbIsPermitted, (int)lenRcvBuf, rcvBuf);
    ...
    if (*pbIsPermitted != 0)
    {
        ...
        {
            pMsg->msgFlags |=
                NEEDS_ACLCHK_U; /* request ACL check after resolution */
        }
 ...
 */
#ifdef HAVE_RECVMMSG
static rsRetVal processSocket(struct wrkrInfo_s* pWrkr, struct lstn_s* lstn,
                              struct sockaddr_storage* frominetPrev,
                              int* pbIsPermitted)
{
    DEFiRet;
    ...
    iNbrTimeUsed = 0;
    while (1)
    { /* loop is terminated if we have a "bad" receive, done below in the body
       */
        if (pWrkr->pThrd->bShallStop == RSTRUE)
            ABORT_FINALIZE(RS_RET_FORCE_TERM);
        memset(pWrkr->recvmsg_iov, 0,
               runModConf->batchSize * sizeof(struct iovec));
        memset(pWrkr->recvmsg_mmh, 0,
               runModConf->batchSize * sizeof(struct mmsghdr));
        for (i = 0; i < runModConf->batchSize; ++i)
        {
            pWrkr->recvmsg_iov[i].iov_base =
                pWrkr->pRcvBuf + (i * (iMaxLine + 1));
            pWrkr->recvmsg_iov[i].iov_len = iMaxLine;
            pWrkr->recvmsg_mmh[i].msg_hdr.msg_namelen =
        ...
        nelem = recvmmsg(lstn->sock, pWrkr->recvmsg_mmh, runModConf->batchSize,
                         0, NULL);
        ...
        if (nelem < 0 && errno == ENOSYS)
        {
             ...
             */
            DBGPRINTF("imudp: error ENOSYS on call to recvmmsg() - fall back "
                      "to recvmsg\n");
        ...
        if (nelem < 0)
        {
            if (errno != EINTR && errno != EAGAIN)
            {
                ...
                LogError(errno, NO_ERRCODE,
                         "imudp: error receiving on socket: %s", errStr);
            }
            ABORT_FINALIZE(RS_RET_ERR);
            // this most often is NOT an error, state is not checked by caller!
        }

        if ((runModConf->iTimeRequery == 0) ||
            (iNbrTimeUsed++ % runModConf->iTimeRequery) == 0)
        {
        ...
        for (i = 0; i < nelem; ++i)
        {
            processPacket(lstn, frominetPrev, pbIsPermitted,
                          ...
                          pWrkr->recvmsg_mmh[i].msg_len, &stTime, ttGenTime,
                          &(pWrkr->frominet[i]),
                              ...
                              int* pbIsPermitted)
{
    ...
    { /* loop is terminated if we have a bad receive, done below in the body */
        ...
        CHKiRet(processPacket(lstn, frominetPrev, pbIsPermitted, pWrkr->pRcvBuf,
                              lenRcvBuf, &stTime, ttGenTime, &frominet,
                              mh.msg_namelen, &multiSub));
    }
...
static rsRetVal checkSchedulingPolicy(modConfData_t* modConf)
{
    ...
    if (0)
    { /* trick to use conditional compilation */
#ifdef SCHED_FIFO
    }
...
static rsRetVal checkSchedParam(modConfData_t* modConf)
{
    ...
    if (modConf->pszSchedPolicy != NULL &&
        modConf->iSchedPrio == SCHED_PRIO_UNSET)
    {
        LogError(0, RS_RET_ERR_SCHED_PARAMS,
                 "imudp: scheduling policy set, but without priority - "
                 "ignoring settings");
        ABORT_FINALIZE(RS_RET_ERR_SCHED_PARAMS);
    }
    else if (modConf->pszSchedPolicy == NULL &&
             modConf->iSchedPrio != SCHED_PRIO_UNSET)
    {
        LogError(0, RS_RET_ERR_SCHED_PARAMS,
                 "imudp: scheduling priority set, but without policy - "
                 "ignoring settings");
        ABORT_FINALIZE(RS_RET_ERR_SCHED_PARAMS);
    }
    else if (modConf->pszSchedPolicy != NULL &&
    ...
    { /* nothing set */
        modConf->iSchedPrio =
            SCHED_PRIO_UNSET; /* prevents doing the activation call */
    }
...
static rsRetVal setSchedParams(modConfData_t* modConf)
{
    ...
    if (err != 0)
    {
        LogError(err, NO_ERRCODE,
                 "imudp: pthread_setschedparam() failed - ignoring");
    }
finalize_it:
#endif
...
/* This function implements the main reception loop. Depending on the
 * environment, we either use the traditional (but slower) select() or the
 * Linux-specific epoll() interface. ./configure settings control which one is
 * used. rgerhards, 2009-09-09
 */
...
static rsRetVal rcvMainLoop(struct wrkrInfo_s* const __restrict__ pWrkr)
{
    ...
    for (lstn = lcnfRoot; lstn != NULL; lstn = lstn->next)
    {
        if (lstn->sock != -1)
        {
            ...
            if (epoll_ctl(efd, EPOLL_CTL_ADD, lstn->sock, &(udpEPollEvt[i])) <
                0)
            {
                rs_strerror_r(errno, errStr, sizeof(errStr));
                LogError(errno, NO_ERRCODE,
                         "epoll_ctrl failed on fd %d with %s\n", lstn->sock,
                         errStr);
            }
        }
        i++;
    ...
    while (1)
    {
        ...
        for (i = 0; i < nfds; ++i)
        {
            processSocket(pWrkr, currEvt[i].data.ptr, &frominetPrev,
                          &bIsPermitted);
        }
        if (pWrkr->pThrd->bShallStop == RSTRUE)
            break; /* terminate input! */
        ...
        else if (!strcmp(inppblk.descr[i].name, "name.appendport"))
        {
            if (bAppendPortUsed)
            {
                LogError(0, RS_RET_INVALID_PARAMS,
                         ...
                         "inputname.appendport parameter specified - only one "
                         "can be used");
                ABORT_FINALIZE(RS_RET_INVALID_PARAMS);
            }
...
int i;
int portIdx;
CODESTARTnewInpInst DBGPRINTF("newInpInst (imudp)\n");
...
finalize_it : loadModConf = NULL; /* done loading */
...
{
#ifdef HAVE_RECVMMSG
    CHKmalloc(wrkrInfo[i].recvmsg_iov =
                  malloc(runModConf->batchSize * sizeof(struct iovec)));
    CHKmalloc(wrkrInfo[i].recvmsg_mmh =
                  malloc(runModConf->batchSize * sizeof(struct mmsghdr)));
    CHKmalloc(wrkrInfo[i].frominet = malloc(runModConf->batchSize *
                                            sizeof(struct sockaddr_storage)));
#endif
    CHKmalloc(wrkrInfo[i].pRcvBuf = malloc(lenRcvBuf));
    wrkrInfo[i].id = i;
}
finalize_it : ENDactivateCnf
...
#if defined(HAVE_PRCTL) && defined(PR_SET_NAME)
    /* set thread name - we ignore if the call fails, has no harsh
     * consequences... */
...
CODESTARTrunInput pthread_attr_init(&wrkrThrdAttr);
...
ENDrunInput
    ...
    BEGINwillRun CODESTARTwillRun net.PrintAllowedSenders(1); /* UDP */
net.HasRestrictions(UCHAR_CONSTANT("UDP"), &bDoACLCheck);     /* UDP */
ENDwillRun

    BEGINafterRun struct lstn_s *lstn,
    *lstnDel;
int i;
CODESTARTafterRun
    /* do cleanup here */
                         ...
                         void __attribute__((unused)) * pVal)
{
    ...
    cs.iSchedPrio = SCHED_PRIO_UNSET;
    cs.iTimeRequery =
        TIME_REQUERY_DFLT; /* the default is to query only every second time */
    return RS_RET_OK;
}
...
CHKiRet(omsdRegCFSLineHdlr((uchar*)"inputudpserverbindruleset", 0,
                           eCmdHdlrGetWord, NULL, &cs.pszBindRuleset,
                           STD_LOADABLE_MODULE_ID));
CHKiRet(omsdRegCFSLineHdlr((uchar*)"udpserverrun", 0, eCmdHdlrGetWord,
                           addInstance, NULL, STD_LOADABLE_MODULE_ID));
CHKiRet(omsdRegCFSLineHdlr((uchar*)"udpserveraddress", 0, eCmdHdlrGetWord, NULL,
                           &cs.pszBindAddr, STD_LOADABLE_MODULE_ID));
...
CHKiRet(regCfSysLineHdlr2((uchar*)"imudpschedulingpriority", 0, eCmdHdlrInt,
                          ...
                          &bLegacyCnfModGlobalsPermitted));
CHKiRet(regCfSysLineHdlr2((uchar*)"udpservertimerequery", 0, eCmdHdlrInt, NULL,
                          &cs.iTimeRequery, STD_LOADABLE_MODULE_ID,
                          &bLegacyCnfModGlobalsPermitted));
