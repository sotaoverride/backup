...
{ // Promote, if necessary, & pick the wider of the two floating-point types.
    ...
    typedef typename mpl::if_<
        typename mpl::and_<is_floating_point<T1P>, is_floating_point<T2P>>::
            type, // both T1P and T2P are floating-point?
#ifdef BOOST_MATH_USE_FLOAT128
        typename mpl::if_<
            typename mpl::or_<is_same<__float128, T1P>,
                              is_same<__float128, T2P>>::type, // either long
                                                               // double?
            __float128,
#endif
            typename mpl::if_<
                typename mpl::or_<is_same<long double, T1P>,
                                  is_same<long double, T2P>>::type, // either
                                                                    // long
                                                                    // double?
                long double, // then result type is long double.
                typename mpl::if_<
                    typename mpl::or_<is_same<double, T1P>,
                                      is_same<double, T2P>>::type, // either
                                                                   // double?
        ...
        typename mpl::if_<typename mpl::and_<mpl::not_<is_floating_point<T2P>>,
                                             ::boost::is_convertible<T1P, T2P>>,
                          T2P, T1P>::type>::type type;
}; // promote_arg2
// These full specialisations reduce mpl::if_ usage and speed up
// compilation:
template <>
...
struct promote_args
{
        ...
        typename promote_args_2<
            ...
            typename promote_args_2<
                ...
                typename promote_args_2<
                    ...
                    typename promote_args_2<typename remove_cv<T5>::type,
    ...
    BOOST_STATIC_ASSERT_MSG(
        (0 == ::boost::is_same<type, long double>::value),
        "Sorry, but this platform does not have sufficient long double support "
        "for the special functions to be reliably implemented.");
#endif
};

//
// This struct is the same as above, but has no static assert on long double
// usage, it should be used only on functions that can be implemented for long
// double even when std lib support is missing or broken for that type.
//
template <class T1, class T2 = float, class T3 = float, class T4 = float,
          class T5 = float, class T6 = float>
