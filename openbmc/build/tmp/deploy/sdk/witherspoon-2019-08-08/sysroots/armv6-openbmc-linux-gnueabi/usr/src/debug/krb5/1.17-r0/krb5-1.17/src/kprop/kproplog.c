...
/* ctime() for uint32_t* */
static const char* ctime_uint32(uint32_t* time32)
{
    time_t tmp;
    const char* r;
...
static void print_time(uint32_t* timep)
{
    if (*timep == 0L)
        printf("\t\t\tNone\n");
    else
...
static void print_deltat(uint32_t* deltat)
{
    krb5_error_code ret;
    static char buf[30];
...
static void print_hex(const char* tag, utf8str_t* str)
{
    unsigned int len;
    char* hex;
...
static void print_str(const char* tag, utf8str_t* str)
{
    krb5_error_code ret;
    char* s;
...
static void print_data(const char* tag, kdbe_data_t* data)
{
    printf("\t\t\tmagic: 0x%x\n", data->k_magic);
...
static void print_princ(kdbe_princ_t* princ)
{
    int i, len;
    kdbe_data_t* data;
...
static void print_key(kdbe_key_t* k)
{
    unsigned int i;
    utf8str_t* str;
...
static void print_keydata(kdbe_key_t* keys, unsigned int len)
{
    unsigned int i;
...
/* Display TL item. */
static void print_tl(kdbe_tl_t* tl)
{
    int i, len;
...
static void print_tldata(kdbe_tl_t* tldata, int len)
{
    int i;
 ...
 */
static void print_attr(kdbe_val_t* val, int vverbose)
{
    switch (val->av_type)
    {
        case AT_ATTRFLAGS:
 ...
 */
static void print_update(kdb_hlog_t* ulog, uint32_t entry, uint32_t ulogentries,
...
static kdb_hlog_t* map_ulog(const char* filename)
{
    int fd;
    struct stat st;
    kdb_hlog_t* ulog;
