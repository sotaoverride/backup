...
int journal_bmap(journal_t* journal, blk64_t block, unsigned long long* phys)
{
    ...
    *phys = block;
    ...
    retval = ext2fs_bmap2(inode->i_fs, inode->i_ino, &inode->i_ext2, NULL, 0,
                          block, 0, &pblk);
    *phys = pblk;
    return (int)retval;
#endif
...
struct buffer_head* getblk(kdev_t kdev, blk64_t blocknr, int blocksize)
{
    ...
    int bufsize = sizeof(*bh) + kdev->k_fs->blocksize - sizeof(bh->b_data);
    errcode_t retval;
...
static errcode_t ext2fs_get_journal(ext2_filsys fs, journal_t** ret_journal)
{
    ...
    if (uuid_is_null(sb->s_journal_uuid))
    {
        ...
        retval = ext2fs_read_inode(fs, sb->s_journal_inum, &j_inode->i_ext2);
        if (retval)
        {
        try_backup_journal:
        ...
        pb.last_block = -1;
        retval = ext2fs_block_iterate3(fs, j_inode->i_ino, BLOCK_FLAG_HOLE, 0,
        ...
        {
            retval =
                ext2fs_write_inode(fs, sb->s_journal_inum, &j_inode->i_ext2);
            if (retval)
                goto errout;
        }
    ...
    else
    {
        ...
        if (!fs->journal_name)
        {
            ...
            fs->journal_name = blkid_get_devname(blkid, "UUID", uuid);
            if (!fs->journal_name)
...
errcode_t ext2fs_run_ext3_journal(ext2_filsys* fsp)
{
     ...
     */
    if (fs->super->s_kbytes_written && fs->io->manager->get_stats)
