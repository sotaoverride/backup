...
static int resolvconf_help(void)
{
    ...
    printf(
        ...
        "\n"
        "This is a compatibility alias for the resolvectl(1) tool, providing "
        "native\n"
        "command line compatibility with the resolvconf(8) tool of various "
        "Linux\n"
        "distributions and BSD systems. Some options supported by other "
        "implementations\n"
        "are not supported and are ignored: -m, -p. Various options supported "
        "by other\n"
        "implementations are not supported and will cause the invocation to "
        "fail: -u,\n"
...
int resolvconf_parse_argv(int argc, char* argv[])
{
    ...
    while ((c = getopt_long(argc, argv, "hadxpfm:uIi:l:Rr:vV", options,
        ...
        switch (c)
        {
            ...
            /* The exclusive/private/force stuff is an openresolv invention, we
             * support in some skewed way */
            case 'x':
                type = TYPE_EXCLUSIVE;
                break;
            ...
            /* The metrics stuff is an openresolv invention we ignore (and don't
             * really need) */
            case 'm':
                log_debug("Switch -%c ignored.", c);
                break;

            /* Everybody else can agree on the existance of -u but we don't
             * support it. */
            case 'u':
    ...
    if (arg_mode == MODE_SET_LINK)
    {
        ...
        for (;;)
        {
            ...
            log_syntax(NULL, LOG_DEBUG, "stdin", n, 0,
        ...
        if (type == TYPE_EXCLUSIVE)
        {
            ...
            /* If -x mode is selected, let's preferably route non-suffixed
             * lookups to this interface. This
