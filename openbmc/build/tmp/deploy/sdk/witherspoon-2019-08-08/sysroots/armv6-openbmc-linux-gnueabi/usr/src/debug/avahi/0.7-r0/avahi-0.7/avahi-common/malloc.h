...
static inline void* AVAHI_GCC_ALLOC_SIZE2(1, 2)
    avahi_new_internal(unsigned n, size_t k)
{
    assert(n < INT_MAX / k);
...
static inline void* AVAHI_GCC_ALLOC_SIZE2(1, 2)
    avahi_new0_internal(unsigned n, size_t k)
{
    assert(n < INT_MAX / k);
...
typedef struct AvahiAllocator
{
    ...
    void* (*calloc)(size_t nmemb,
                    size_t size)AVAHI_GCC_ALLOC_SIZE2(1, 2); /**< May be NULL */
} AvahiAllocator;
...
/** Like sprintf() but store the result in a freshly allocated buffer. Free this
 * with avahi_free() */
