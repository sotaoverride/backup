        ...
        static void usage(const char* progname)
{
    ...
    fprintf(stderr,
            ...
            "  -m MIB[" ENV_SEPARATOR
            "...]\t\tload given list of MIBs (ALL loads "
            "everything)\n"
            "  -M DIR[" ENV_SEPARATOR
    ...
    fprintf(stderr, "  -P MIBOPTS\t\tToggle various defaults controlling mib "
                    "parsing:\n");
...
static void change_state(tState new_state)
{
...
static int handle_agentx_response(int operation, netsnmp_session* sp, int reqid,
...
static void StateDisconnect(tState self)
{
    snmp_log(LOG_ERR, "Unexpected disconnect in state %s\n", self->name);
    change_state(&Disconnecting);
}

static void StateClose(tState self, netsnmp_pdu* act)
{
...
static void ConnectingEntry(tState self)
{
    ...
    else if (!(sess = snmp_sess_add_ex(&init, t, NULL, agentx_parse, NULL, NULL,
...
const struct tState_s Connecting = {ConnectingEntry, NULL, NULL,          NULL,
                                    StateDisconnect, NULL, "Connnecting", 0};

static netsnmp_pdu* pdu_create_opt_context(int command, const char* context,
                                           size_t len)
{
...
static void OpeningEntry(tState self)
{
    netsnmp_pdu* act =
...
static void OpeningRes(tState self, netsnmp_pdu* act)
{
...
const struct tState_s Opening = {
    OpeningEntry,    NULL,       OpeningRes, &Disconnecting,
    StateDisconnect, StateClose, "Opening",  0};

static void NotifyingEntry(tState self)
{
...
static void NotifyingRes(tState self, netsnmp_pdu* act)
{
    if (act->errstat == AGENTX_ERR_NOERROR)
        result = 0;
    else
...
static void ClosingEntry(tState self)
{
    /* CLOSE pdu->errstat */
    netsnmp_pdu* act =
...
static void ClosingRes(tState self, netsnmp_pdu* act)
{
    if (act->errstat != AGENTX_ERR_NOERROR)
    {
...
static void ClosingDisconnect(tState self)
{
    change_state(&Disconnecting);
}

static void ClosingClose(tState self, netsnmp_pdu* act)
{
    change_state(&Disconnecting);
}

const struct tState_s Closing = {
    ClosingEntry,      NULL,         ClosingRes, &Disconnecting,
    ClosingDisconnect, ClosingClose, "Closing",  1};

static void DisconnectingEntry(tState self)
{
    snmp_sess_close(sessp);
    sessp = NULL;
    change_state(&Exit);
...
const struct tState_s Disconnecting = {
    DisconnectingEntry, NULL, NULL, NULL, NULL, NULL, "Disconnecting", 0};

const struct tState_s Exit = {NULL, NULL, NULL, NULL, NULL, NULL, "Exit", 0};
