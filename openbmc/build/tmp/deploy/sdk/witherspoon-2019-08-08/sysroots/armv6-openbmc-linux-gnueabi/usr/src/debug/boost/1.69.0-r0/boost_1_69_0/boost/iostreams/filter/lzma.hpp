...
class BOOST_IOSTREAMS_DECL lzma_base
{
  ...
  protected:
    ...
    template <typename Alloc>
    void init(const lzma_params& p, bool compress,
              lzma_allocator<Alloc>& zalloc)
    {
        bool custom = lzma_allocator<Alloc>::custom;
        do_init(p, compress, custom ? lzma_allocator<Alloc>::allocate : 0,
                custom ? lzma_allocator<Alloc>::deallocate : 0, &zalloc);
    }
  ...
  private:
    void do_init(const lzma_params& p, bool compress, lzma::alloc_func,
                 lzma::free_func, void* derived);
    ...
    : symmetric_filter<detail::lzma_compressor_impl<Alloc>, Alloc>
{
  ...
  public:
    ...
    basic_lzma_compressor(
        const lzma_params& = lzma::default_compression,
...
void* lzma_allocator<Alloc, Base>::allocate(void* self, size_t items,
                                            size_t size)
{
...
template <typename Alloc>
bool lzma_compressor_impl<Alloc>::filter(const char*& src_begin,
...
template <typename Alloc>
bool lzma_decompressor_impl<Alloc>::filter(const char*& src_begin,
                                           const char* src_end,
