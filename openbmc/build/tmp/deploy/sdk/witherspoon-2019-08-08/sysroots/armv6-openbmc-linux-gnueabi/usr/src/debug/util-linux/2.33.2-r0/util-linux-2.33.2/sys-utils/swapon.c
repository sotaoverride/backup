...
#define SWAP_FLAGS_DISCARD_VALID                                               \
...
struct swapon_ctl
{
    ...
    unsigned int all : 1,  /* turn on all swap devices */
...
static inline struct colinfo* get_column_info(const struct swapon_ctl* ctl,
                                              unsigned num)
{
    return &infos[get_column_id(ctl, num)];
}

static void add_scols_line(const struct swapon_ctl* ctl,
                           struct libscols_table* table, struct libmnt_fs* fs)
{
    int i;
...
static int display_summary(void)
{
    ...
    while (mnt_table_next_fs(st, itr, &fs) == 0)
    {
        printf("%-39s\t%-8s\t%jd\t%jd\t%d\n", mnt_fs_get_source(fs),
               mnt_fs_get_swaptype(fs), mnt_fs_get_size(fs),
               mnt_fs_get_usedsize(fs), mnt_fs_get_priority(fs));
    }
...
static int swap_rewrite_signature(const struct swap_device* dev)
{
    ...
    if (write(fd, (void*)SWAP_SIGNATURE, SWAP_SIGNATURE_SZ) !=
        SWAP_SIGNATURE_SZ)
    {
...
static int swapon_checks(const struct swapon_ctl* ctl, struct swap_device* dev)
{
    ...
    if ((st.st_mode & permMask) != 0)
        warnx(_("%s: insecure permissions %04o, %04o suggested."), dev->path,
              st.st_mode & 07777, ~permMask & 0666);

    if (S_ISREG(st.st_mode) && st.st_uid != 0)
        warnx(_("%s: insecure file owner %d, 0 (root) suggested."), dev->path,
              st.st_uid);
    ...
    if (S_ISREG(st.st_mode))
    {
        ...
        {
            warnx(_("%s: skipping - it appears to have holes."), dev->path);
            goto err;
        }
        devsize = st.st_size;
    }
    ...
    if (ctl->verbose)
        warnx(_("%s: found signature [pagesize=%d, signature=%s]"), dev->path,
              dev->pagesize,
              sig == SIG_SWAPSPACE
                  ? "swap"
    ...
    if (sig == SIG_SWAPSPACE && dev->pagesize)
    {
        ...
        if (ctl->verbose)
            warnx(_("%s: pagesize=%d, swapsize=%llu, devsize=%llu"), dev->path,
                  dev->pagesize, swapsize, devsize);
        ...
        else if (syspg < 0 || (unsigned int)syspg != dev->pagesize)
        {
            if (ctl->fix_page_size)
            {
                ...
                warnx(_("%s: swap format pagesize does not match."), dev->path);
...
static int do_swapon(const struct swapon_ctl* ctl, const struct swap_prop* prop,
                     const char* spec, int canonic)
{
    ...
    if (priority >= 0)
    {
        ...
        flags = SWAP_FLAG_PREFER |
                ((priority & SWAP_FLAG_PRIO_MASK) << SWAP_FLAG_PRIO_SHIFT);
    }
#endif
    /*
...
static int swapon_by_label(struct swapon_ctl* ctl, const char* label)
{
    ...
    return device ? do_swapon(ctl, &ctl->props, device, TRUE)
                  : cannot_find(label);
}
...
static void __attribute__((__noreturn__)) usage(void)
{
    ...
    fputs(_(" -a, --all                enable all swaps from /etc/fstab\n"),
          out);
    fputs(_(" -d, --discard[=<policy>] enable swap discards, if supported by "
            "device\n"),
          out);
    fputs(_(" -e, --ifexists           silently skip devices that do not "
            "exist\n"),
          out);
    fputs(_(" -f, --fixpgsz            reinitialize the swap space if "
            "necessary\n"),
          out);
    fputs(_(" -o, --options <list>     comma-separated list of swap options\n"),
          out);
    fputs(_(" -p, --priority <prio>    specify the priority of the swap "
            "device\n"),
          out);
    fputs(_(" -s, --summary            display summary about used swap devices "
            "(DEPRECATED)\n"),
          out);
    fputs(_("     --show[=<columns>]   display summary in definable table\n"),
          out);
    fputs(_("     --noheadings         don't print table heading (with "
            "--show)\n"),
          out);
    fputs(_("     --raw                use the raw output format (with "
            "--show)\n"),
          out);
    fputs(_("     --bytes              display swap size in bytes in --show "
            "output\n"),
          out);
    ...
    fputs(_("\nAvailable discard policy types (for --discard):\n"
            ...
            "If no policy is selected, both discard types are enabled "
            "(default).\n"),
          out);
...
int main(int argc, char* argv[])
{
    ...
    while ((c = getopt_long(argc, argv, "ahd::efo:p:svVL:U:", long_opts,
                            NULL)) != -1)
    {
        ...
        switch (c)
        {
            ...
            case 'p': /* priority */
                ctl.props.priority =
                    strtos16_or_err(optarg, _("failed to parse priority"));
                break;
            case 'L':
                add_label(optarg);
                break;
            ...
            case 'd':
                ...
                if (optarg)
                {
                    ...
                    if (strcmp(optarg, "once") == 0)
                        ...
                        errx(EXIT_FAILURE, _("unsupported discard policy: %s"),
                             optarg);
                }
                break;
            case 'e': /* ifexists */
            ...
            case SHOW_OPTION:
                if (optarg)
                {
                    ctl.ncolumns = string_to_idarray(optarg, ctl.columns,
                ...
                break;
            case OPT_LIST_TYPES:
                for (ctl.ncolumns = 0; (size_t)ctl.ncolumns < ARRAY_SIZE(infos);
                     ctl.ncolumns++)
    ...
    if (ctl.show ||
        (!ctl.all && !numof_labels() && !numof_uuids() && *argv == NULL))
    {
        if (!ctl.ncolumns)
        {
            /* default columns */
