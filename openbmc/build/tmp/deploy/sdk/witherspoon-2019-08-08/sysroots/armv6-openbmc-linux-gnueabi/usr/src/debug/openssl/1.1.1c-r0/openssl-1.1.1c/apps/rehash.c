    ...
    for (ep = bp->first_entry; ep; ep = ep->next)
    {
        if (digest && memcmp(digest, ep->digest, evpmdsize) == 0)
        {
            BIO_printf(bio_err, "%s: warning: skipping duplicate %s in %s\n",
                       opt_getprog(), type == TYPE_CERT ? "certificate" : "CRL",
                       filename);
            return 0;
        }
...
static int do_file(const char* filename, const char* fullpath, enum Hash h)
{
    ...
    if (name != NULL)
    {
        ...
        if ((h == HASH_OLD) || (h == HASH_BOTH))
            errs += add_entry(type, X509_NAME_hash_old(name), filename, digest,
                              1, ~0);
    }
...
static int do_dir(const char* dirname, enum Hash h)
{
    ...
    for (n = 0; n < numfiles; ++n)
    {
        ...
        if (BIO_snprintf(buf, buflen, "%s%s%s", dirname, pathsep, filename) >=
            buflen)
            continue;
        if (lstat(buf, &st) < 0)
            continue;
        ...
        for (bp = hash_table[i]; bp; bp = nextbp)
        {
            ...
            for (ep = bp->first_entry; ep; ep = nextep)
            {
                ...
                else if (ep->need_symlink)
                {
                    ...
                    BIO_snprintf(buf, buflen, "%s%s%n%08x.%s%d", dirname,
                                 pathsep, &n, bp->hash, suffixes[bp->type],
                                 nextid);
                    if (verbose)
                        BIO_printf(bio_out, "link %s -> %s\n", ep->filename,
                                   &buf[n]);
