...
#define LINE_PARSER(EOLSET, BODY)                                              \
    parser_stclass int parse_line(char* line, struct STRUCTURE* result,        \
                                  struct parser_data* data, size_t datalen,    \
                                  int* errnop EXTRA_ARGS_DECL)                 \
    {                                                                          \
        ENTDATA_DECL(data)                                                     \
...
#define STRING_LIST(variable, terminator_c)                                    \
    {                                                                          \
        char** list =                                                          \
            parse_list(&line, buf_start, buf_end, terminator_c, errnop);       \
        if (list)                                                              \
            variable = list;                                                   \
        else                                                                   \
...
#define INT_FIELD_MAYBE_NULL(variable, terminator_p, swallow, base, convert,   \
    ...
    {                                                                          \
        char* endp;                                                            \
        if (*line == '\0')                                                     \
            /* We expect some more input, so don't allow the string to end     \
             * here. */                                                        \
            return 0;                                                          \
        variable = convert(strtou32(line, &endp, base));                       \
        if (endp == line)                                                      \
    ...
    {                                                                          \
        ...
        {                                                                      \
            if (line >= data->linebuffer && line < buf_end)                    \
                ...
                /* LINE does not point within DATA->linebuffer, so that space  \
                   is not being used for scratch space right now.  We can use                       \
                   all of it for the pointer vector storage.  */                                        \
                buf_start = data->linebuffer;                                  \
        }                                                                      \
                                                                               \
