...
static int pkey_dh_derive(EVP_PKEY_CTX* ctx, unsigned char* key, size_t* keylen)
{
    ...
    else if (dctx->kdf_type == EVP_PKEY_DH_KDF_X9_42)
    {
        ...
        if (DH_compute_key_padded(Z, dhpub, dh) <= 0)
            goto err;
        if (!DH_KDF_X9_42(key, *keylen, Z, Zlen, dctx->kdf_oid, dctx->kdf_ukm,
...
const EVP_PKEY_METHOD dh_pkey_meth = {EVP_PKEY_DH,
                                      0,
                                      pkey_dh_init,
                                      pkey_dh_copy,
                                      pkey_dh_cleanup,
...
const EVP_PKEY_METHOD dhx_pkey_meth = {EVP_PKEY_DHX,
                                       0,
                                       pkey_dh_init,
                                       pkey_dh_copy,
                                       pkey_dh_cleanup,
