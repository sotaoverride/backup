    ...
    : enable_shared_from_this<thread_data_base>
{
    ...
    // These data must be at the end so that the access to the other fields
    // doesn't change when BOOST_THREAD_PROVIDES_INTERRUPTIONS is defined.
    // Another option is to have them always
    pthread_mutex_t* cond_mutex;
    pthread_cond_t* current_cond;
    //#endif
    typedef std::vector<std::pair<condition_variable*, mutex*>
                        //, hidden_allocator<std::pair<condition_variable*,
                        //mutex*> >
                        >
        notify_list_t;
    ...
    //#endif
    thread_data_base() :
        thread_handle(0), done(false), join_started(false), joined(false),
        thread_exit_callbacks(0),
        //#if defined BOOST_THREAD_PROVIDES_INTERRUPTIONS
        cond_mutex(0), current_cond(0),
        //#endif
        notify()
        //#ifndef BOOST_NO_EXCEPTIONS
        ,
        ...
        //#if defined BOOST_THREAD_PROVIDES_INTERRUPTIONS
        ,
        interrupt_enabled(true), interrupt_requested(false)
    //#endif
    {
    }
...
class interruption_checker
{
  ...
  public:
    explicit interruption_checker(pthread_mutex_t* cond_mutex,
        ...
        m(cond_mutex), set(thread_info && thread_info->interrupt_enabled),
        done(false)
    {
        if (set)
        {
