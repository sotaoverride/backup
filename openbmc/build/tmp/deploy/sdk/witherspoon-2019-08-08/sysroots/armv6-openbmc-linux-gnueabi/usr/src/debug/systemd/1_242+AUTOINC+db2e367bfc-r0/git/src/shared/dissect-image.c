...
{
    /* Try to find device content type and return it in *ret_fstype. If nothing
     * is found, 0/NULL will be returned. -EUCLEAN will be returned for ambigous
     * results, and an different error otherwise. */
...
static bool device_is_mmc_special_partition(sd_device* d)
{
    ...
    return startswith(sysname, "mmcblk") &&
           (endswith(sysname, "rpmb") || endswith(sysname, "boot0") ||
            endswith(sysname, "boot1"));
}
...
static int wait_for_partitions_to_appear(int fd, sd_device* d,
                                         ...
                                         sd_device_enumerator** ret_enumerator)
{
    ...
    /* The kernel has probed fewer partitions than blkid? Maybe the kernel
     * prober is still running or it got EBUSY because udev already opened the
     * device. Let's reprobe the device, which is a synchronous
    ...
    for (unsigned j = 0;; j++)
    {
        ...
        if (r == -EINVAL)
        {
            ...
            /* If we are running on a loop device that has partition scanning
             * off, return an explicit recognizable error about this, so that
             * callers can generate a proper message explaining the situation.
             */
        ...
        /* If something else has the device open, such as an udev rule, the
         * ioctl will return EBUSY. Since there's no way to wait until it isn't
         * busy anymore, let's just wait a bit, and try again.
         *
...
static int loop_wait_for_partitions_to_appear(
    int fd, sd_device* d, unsigned num_partitions, DissectImageFlags flags,
    sd_device_enumerator** ret_enumerator)
{
    ...
    log_debug("Waiting for device (parent + %d partitions) to appear...",
              num_partitions);
    ...
    {
        r = wait_for_partitions_to_appear(fd, device, num_partitions, flags,
                                          ret_enumerator);
        if (r != -EAGAIN)
            return r;
    }
...
int dissect_image(int fd, const void* root_hash, size_t root_hash_size,
                  DissectImageFlags flags, DissectedImage** ret)
{
     ...
     *
     * Returns -ENOPKG if no suitable partition table or file system could be
     * found. Returns -EADDRNOTAVAIL if a root hash was specified but no
     * matching root/verity partitions found. */

    if (root_hash)
    {
        /* If a root hash is supplied, then we use the root partition that has a
         * UUID that match the first 128bit of the root hash. And we use the
         * verity partition that has a UUID that match the final 128bit. */
    ...
    {
        /* Look for file system superblocks, unless we only shall look for GPT
         * partition tables */
        blkid_probe_enable_superblocks(b, 1);
        blkid_probe_set_superblocks_flags(b, BLKID_SUBLKS_TYPE |
                                                 BLKID_SUBLKS_USAGE);
    ...
    r = loop_wait_for_partitions_to_appear(
        fd, d, blkid_partlist_numof_partitions(pl), flags, &e);
    if (r < 0)
        return r;
    ...
    FOREACH_DEVICE(e, q)
    {
        ...
        if (is_gpt)
        {
            int designator = _PARTITION_DESIGNATOR_INVALID,
                architecture = _ARCHITECTURE_INVALID;
            ...
            else if (sd_id128_equal(type_id, GPT_ESP))
            {
                ...
                /* Note that we don't check the GPT_FLAG_NO_AUTO flag for the
                 * ESP, as it is not defined there. We instead check the
                 * GPT_FLAG_NO_BLOCK_IO_PROTOCOL, as recommended by the UEFI
                 * spec (See "12.3.3 Number and Location of System Partitions").
                 */
        ...
        else if (is_mbr)
        {
            ...
            switch (blkid_partition_get_type(pp))
            {
                ...
                case 0xEA:
                { /* Boot Loader Spec extended $BOOT partition */
    ...
    {
        /* No root partition found? Then let's see if ther's one for the
         * secondary architecture. And if not either, then check if there's a
         * single generic one, and use that. */
        ...
        if (m->partitions[PARTITION_ROOT_SECONDARY].found)
        {
            ...
            m->partitions[PARTITION_ROOT_VERITY] =
                m->partitions[PARTITION_ROOT_SECONDARY_VERITY];
            zero(m->partitions[PARTITION_ROOT_SECONDARY_VERITY]);
        }
        else if (flags & DISSECT_IMAGE_REQUIRE_ROOT)
        {

            /* If the root has was set, then we won't fallback to a generic
             * node, because the root hash decides */
            if (root_hash)
                return -EADDRNOTAVAIL;

            /* If we didn't find a generic node, then we can't fix this up
             * either */
            if (!generic_node)
                return -ENXIO;

            /* If we didn't find a properly marked root partition, but we did
             * find a single suitable
             * generic Linux partition, then use this as root partition, if the
             * caller asked for it. */
            if (multiple_generic)
                return -ENOTUNIQ;
    ...
    if (root_hash)
    {
        ...
        /* If we found the primary root with the hash, then we definitely want
         * to suppress any secondary root (which would be weird, after all the
         * root hash should only be assigned to one pair of partitions... */
        m->partitions[PARTITION_ROOT_SECONDARY].found = false;
        m->partitions[PARTITION_ROOT_SECONDARY_VERITY].found = false;

        /* If we found a verity setup, then the root partition is necessarily
         * read-only. */
...
static int is_loop_device(const char* path)
{
    ...
    if (access(s, F_OK) < 0)
    {
        ...
        /* The device itself isn't a loop device, but maybe it's a partition and
         * its parent is? */
        xsprintf_sys_block_path(s, "/../loop/", st.st_dev);
        if (access(s, F_OK) < 0)
...
static int mount_partition(DissectedPartition* m, const char* where,
                           const char* directory, uid_t uid_shift,
                           DissectImageFlags flags)
{
    ...
    if (uid_is_valid(uid_shift) && uid_shift != 0 && fstype_can_uid_gid(fstype))
    {
        ...
        if (asprintf(&uid_option, "uid=" UID_FMT ",gid=" GID_FMT, uid_shift,
                     (gid_t)uid_shift) < 0)
            return -ENOMEM;
    ...
    r = mount_verbose(LOG_DEBUG, node, p, fstype,
                      MS_NODEV | (rw ? 0 : MS_RDONLY), options);
    if (r < 0)
        return r;
...
int dissected_image_mount(DissectedImage* m, const char* where, uid_t uid_shift,
                          DissectImageFlags flags)
{
    int r, boot_mounted;
    ...
    {
        r = mount_partition(m->partitions + PARTITION_ROOT, where, NULL,
                            uid_shift, flags);
        if (r < 0)
            return r;
    ...
    r = mount_partition(m->partitions + PARTITION_HOME, where, "/home",
                        uid_shift, flags);
    if (r < 0)
        return r;

    r = mount_partition(m->partitions + PARTITION_SRV, where, "/srv", uid_shift,
                        flags);
    if (r < 0)
        return r;

    boot_mounted = mount_partition(m->partitions + PARTITION_XBOOTLDR, where,
                                   "/boot", uid_shift, flags);
    if (boot_mounted < 0)
        return boot_mounted;

    if (m->partitions[PARTITION_ESP].found)
    {
        /* Mount the ESP to /efi if it exists. If it doesn't exist, use /boot
         * instead, but only if it exists and is empty, and we didn't already
         * mount the XBOOTLDR partition into it. */

        r = chase_symlinks("/efi", where, CHASE_PREFIX_ROOT, NULL);
        if (r >= 0)
        {
            r = mount_partition(m->partitions + PARTITION_ESP, where, "/efi",
                                uid_shift, flags);
            if (r < 0)
                return r;
        }
        else if (boot_mounted <= 0)
        {
            ...
            {
                r = mount_partition(m->partitions + PARTITION_ESP, where,
                                    "/boot", uid_shift, flags);
                if (r < 0)
                    return r;
            }
...
DecryptedImage* decrypted_image_unref(DecryptedImage* d)
{
    ...
    for (i = 0; i < d->n_decrypted; i++)
    {
        ...
        if (p->device && p->name && !p->relinquished)
        {
            ...
            if (r < 0)
                log_debug_errno(
                    r, "Failed to deactivate encrypted partition %s", p->name);
        }
...
static int make_dm_name_and_node(const void* original_node, const char* suffix,
                                 char** ret_name, char** ret_node)
{
...
static int decrypt_partition(DissectedPartition* m, const char* passphrase,
                             DissectImageFlags flags, DecryptedImage* d)
{
    ...
    r = crypt_activate_by_passphrase(
        cd, name, CRYPT_ANY_SLOT, passphrase, strlen(passphrase),
...
static int verity_partition(DissectedPartition* m, DissectedPartition* v,
                            ...
                            DissectImageFlags flags, DecryptedImage* d)
{
    ...
    r = crypt_activate_by_volume_key(cd, name, root_hash, root_hash_size,
...
int dissected_image_decrypt(DissectedImage* m, const char* passphrase,
                            const void* root_hash, size_t root_hash_size,
                            DissectImageFlags flags, DecryptedImage** ret)
{
    ...
    for (i = 0; i < _PARTITION_DESIGNATOR_MAX; i++)
    {
        ...
        if (k >= 0)
        {
            r = verity_partition(p, m->partitions + k, root_hash,
                                 root_hash_size, flags, d);
            if (r < 0)
                return r;
        }
...
int dissected_image_decrypt_interactively(
    DissectedImage* m, const char* passphrase, const void* root_hash,
    size_t root_hash_size, DissectImageFlags flags, DecryptedImage** ret)
{
    ...
    for (;;)
    {
        r = dissected_image_decrypt(m, passphrase, root_hash, root_hash_size,
                                    flags, ret);
        if (r >= 0)
            return r;
        if (r == -EKEYREJECTED)
        ...
        r = ask_password_auto("Please enter image passphrase:", NULL, "dissect",
                              "dissect", USEC_INFINITY, 0, &z);
        if (r < 0)
...
static int deferred_remove(DecryptedPartition* p)
{
    ...
    /* Unfortunately, libcryptsetup doesn't provide a proper API for this, hence
...
int decrypted_image_relinquish(DecryptedImage* d)
{
    ...
    /* Turns on automatic removal after the last use ended for all DM devices of
     * this image, and sets a boolean so that we don't clean it up ourselves
     * either anymore */
    ...
    for (i = 0; i < d->n_decrypted; i++)
    {
        ...
        r = deferred_remove(p);
        if (r < 0)
            return log_debug_errno(r, "Failed to mark %s for auto-removal: %m",
                                   p->name);
...
int dissected_image_acquire_metadata(DissectedImage* m)
{
    ...
    r = safe_fork("(sd-dissect)",
                  FORK_RESET_SIGNALS | FORK_DEATHSIG | FORK_NEW_MOUNTNS |
                      FORK_MOUNTNS_SLAVE,
                  &child);
    if (r < 0)
        goto finish;
    if (r == 0)
    {
        r = dissected_image_mount(m, t, UID_INVALID,
        ...
        for (k = 0; k < _META_MAX; k++)
        {
            ...
            {
                fd = chase_symlinks_and_open(p, t, CHASE_PREFIX_ROOT,
            ...
            {
                log_debug_errno(fd,
                                "Failed to read %s file of image, ignoring: %m",
                                paths[k]);
                continue;
            }
        ...
        switch (k)
        {
            ...
            case META_MACHINE_ID:
            {
                ...
                else if (r == 33)
                {
                    ...
                    if (r < 0)
                        log_debug_errno(
                            r, "Image contains invalid /etc/machine-id: %s",
                            line);
                }
                else if (r == 0)
...
int dissect_image_and_warn(int fd, const char* name, const void* root_hash,
                           size_t root_hash_size, DissectImageFlags flags,
                           DissectedImage** ret)
{
    ...
    switch (r)
    {
        ...
        case -EOPNOTSUPP:
            return log_error_errno(r, "Dissecting images is not supported, "
                                      "compiled without blkid support.");

        case -ENOPKG:
            return log_error_errno(r,
                                   "Couldn't identify a suitable partition "
                                   "table or file system in '%s'.",
                                   name);

        case -EADDRNOTAVAIL:
            return log_error_errno(
                r, "No root partition for specified root hash found in '%s'.",
                name);

        case -ENOTUNIQ:
            return log_error_errno(
                r, "Multiple suitable root partitions found in image '%s'.",
                name);

        case -ENXIO:
            return log_error_errno(
                r, "No suitable root partition found in image '%s'.", name);

        case -EPROTONOSUPPORT:
            return log_error_errno(
                r,
                "Device '%s' is loopback block device with partition scanning "
                "turned off, please turn it on.",
                name);

        default:
            if (r < 0)
                return log_error_errno(r, "Failed to dissect image '%s': %m",
                                       name);
