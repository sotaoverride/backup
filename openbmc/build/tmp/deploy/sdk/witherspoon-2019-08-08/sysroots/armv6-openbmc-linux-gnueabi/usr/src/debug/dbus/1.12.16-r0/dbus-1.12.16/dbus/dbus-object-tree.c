...
/**
 * @defgroup DBusObjectTree A hierarchy of objects with container-contained
 * relationship
...
static DBusObjectSubtree* _dbus_object_subtree_new(
    const char* name, const DBusObjectPathVTable* vtable, void* user_data);
...
struct DBusObjectSubtree
{
    ...
    DBusObjectPathUnregisterFunction
        unregister_function; /**< Function to call on unregister */
    ...
    unsigned int
        invoke_as_fallback : 1; /**< Whether to invoke message_function when
                                   child nodes don't handle the message */
...
static DBusObjectSubtree* find_subtree_recurse(DBusObjectSubtree* subtree,
                                               const char** path,
                                               ...
                                               dbus_bool_t* exact_match)
{
    ...
    while (i < j)
    {
        ...
        if (v == 0)
        {
            ...
            if (return_deepest_match)
            {
                ...
                next = find_subtree_recurse(subtree->subtrees[k], &path[1],
                                            create_if_not_found,
                ...
                return find_subtree_recurse(subtree->subtrees[k], &path[1],
                                            create_if_not_found,
                                            index_in_parent, exact_match);
        }
        else if (v < 0)
    ...
    if (create_if_not_found)
    {
        ...
        if (new_n_subtrees > subtree->max_subtrees)
        {
            ...
            new_max_subtrees =
                subtree->max_subtrees == 0 ? 1 : 2 * subtree->max_subtrees;
            new_subtrees =
                dbus_realloc(subtree->subtrees,
        ...
        return find_subtree_recurse(child, &path[1], create_if_not_found,
                                    index_in_parent, exact_match);
    }
    else
    {
        if (exact_match != NULL)
            *exact_match = FALSE;
        return (return_deepest_match && subtree->invoke_as_fallback) ? subtree
                                                                     : NULL;
    }
}

#ifdef DBUS_ENABLE_EMBEDDED_TESTS
static DBusObjectSubtree* find_subtree(DBusObjectTree* tree, const char** path,
                                       int* index_in_parent)
{
    ...
    subtree =
        find_subtree_recurse(tree->root, path, FALSE, index_in_parent, NULL);
...
static DBusObjectSubtree* find_handler(DBusObjectTree* tree, const char** path,
                                       dbus_bool_t* exact_match)
{
#if VERBOSE_FIND
...
static DBusObjectSubtree* ensure_subtree(DBusObjectTree* tree,
                                         const char** path)
{
#if VERBOSE_FIND
...
dbus_bool_t _dbus_object_tree_register(DBusObjectTree* tree,
                                       dbus_bool_t fallback, const char** path,
 ...
 */
static dbus_bool_t attempt_child_removal(DBusObjectSubtree* parent,
                                         int child_index)
{
...
static dbus_bool_t unregister_and_free_path_recurse(
    DBusObjectSubtree* subtree, const char** path,
    ...
    void** user_data_out)
{
    ...
    if (path[0] == NULL)
        return unregister_subtree(subtree, unregister_function_out,
                                  user_data_out);
    ...
    while (i < j)
    {
        ...
        if (v == 0)
        {
            ...
            freed = unregister_and_free_path_recurse(
                subtree->subtrees[k], &path[1], continue_removal_attempts,
                unregister_function_out, user_data_out);
 ...
 *
 * @param tree the global object tree
 * @param path path to the subtree (same as the one passed to
                                             ...
                                             const char** path)
{
    ...
    found_subtree = unregister_and_free_path_recurse(
        tree->root, path, &continue_removal_attempts, &unregister_function,
        &user_data);

#ifndef DBUS_DISABLE_CHECKS
    if (found_subtree == FALSE)
    {
        _dbus_warn("Attempted to unregister path (path[0] = %s path[1] = %s) "
...
static void free_subtree_recurse(DBusConnection* connection,
                                 DBusObjectSubtree* subtree)
{
    ...
    if (subtree->unregister_function)
        (*subtree->unregister_function)(connection, subtree->user_data);
...
{
    if (tree->root)
        free_subtree_recurse(tree->connection, tree->root);
    tree->root = NULL;
}

static dbus_bool_t _dbus_object_tree_list_registered_unlocked(
    DBusObjectTree* tree, const char** parent_path, char*** child_entries)
{
    DBusObjectSubtree* subtree;
    char** retval;
...
static DBusHandlerResult handle_default_introspect_and_unlock(
    DBusObjectTree* tree, DBusMessage* message, const char** path)
{
    DBusString xml;
...
DBusHandlerResult _dbus_object_tree_dispatch_and_unlock(
    DBusObjectTree* tree, DBusMessage* message, dbus_bool_t* found_object)
{
    char** path;
    dbus_bool_t exact_match;
    DBusList* list;
  ...
  _dbus_verbose ("Dispatch of message by object path\n");
    ...
    {
        if (subtree->message_function != NULL &&
            (exact_match || subtree->invoke_as_fallback))
        {
...
static DBusObjectSubtree* allocate_subtree_object(const char* name)
{
    ...
    subtree =
        dbus_malloc0(MAX(front_padding + (len + 1), sizeof(DBusObjectSubtree)));
 ...
 */
dbus_bool_t _dbus_decompose_path(const char* data, int len, char*** path,
                                 int* path_len)
{
    char** retval;
    int n_components;
...
static StrComparison path_contains(const char** container, const char** child)
{
    ...
    while (child[i] != NULL)
    {
        ...
        if (v != 0)
            return STR_DIFFERENT; /* they overlap until here and then are
...
typedef struct
{
    ...
    dbus_bool_t
        handler_fallback; /**< true if the handler may be called as fallback */
    dbus_bool_t
        message_handled; /**< Gets set to true if message handler called */
    dbus_bool_t handler_unregistered; /**< gets set to true if handler is
                                         unregistered */
} TreeTestData;

static void test_unregister_function(DBusConnection* connection,
                                     void* user_data)
{
...
static DBusHandlerResult test_message_function(DBusConnection* connection,
                                               DBusMessage* message,
                                               void* user_data)
{
    TreeTestData* ttd = user_data;
...
static dbus_bool_t do_register(DBusObjectTree* tree, const char** path,
                               dbus_bool_t fallback, int i,
                               TreeTestData* tree_test_data)
{
    ...
    if (!_dbus_object_tree_register(tree, fallback, path, &vtable,
                                    &tree_test_data[i], NULL))
        return FALSE;
...
static dbus_bool_t do_test_dispatch(DBusObjectTree* tree, const char** path,
                                    int i, TreeTestData* tree_test_data,
                                    int n_test_data)
{
    DBusMessage* message;
    int j;
    ...
    message = dbus_message_new_method_call(
        NULL, flat, "org.freedesktop.TestInterface", "Foo");
    dbus_free(flat);
    if (message == NULL)
        goto oom;
    ...
    while (j < n_test_data)
    {
        if (tree_test_data[j].message_handled)
        {
            if (tree_test_data[j].handler_fallback)
                _dbus_assert(path_contains(tree_test_data[j].path, path) !=
                             STR_DIFFERENT);
            else
                _dbus_assert(path_contains(tree_test_data[j].path, path) ==
                             STR_EQUAL);
...
static dbus_bool_t run_decompose_tests(void)
{
    ...
    while (i < _DBUS_N_ELEMENTS(decompose_tests))
    {
        ...
        if (!_dbus_decompose_path(decompose_tests[i].path,
                                  strlen(decompose_tests[i].path), &result,
                                  &result_len))
            return FALSE;
        ...
        if (result_len !=
                (int)_dbus_string_array_length((const char**)result) ||
            expected_len != result_len ||
            path_contains(decompose_tests[i].result, (const char**)result) !=
                STR_EQUAL)
        {
            int real_len = _dbus_string_array_length((const char**)result);
            _dbus_warn("Expected decompose of %s to have len %d, returned %d, "
                       "appears to have %d",
...
static dbus_bool_t object_tree_test_iteration(void* data)
{
    ...
    const char* path7[] = {"blah",   "boof", "this", "is",
                           "really", "long", NULL};
    ...
    _dbus_assert(find_handler(tree, path1, &exact_match) == tree->root &&
                 !exact_match);
    _dbus_assert(find_handler(tree, path2, &exact_match) == tree->root &&
                 !exact_match);
    _dbus_assert(find_handler(tree, path3, &exact_match) == tree->root &&
                 !exact_match);
    _dbus_assert(find_handler(tree, path4, &exact_match) == tree->root &&
                 !exact_match);
    _dbus_assert(find_handler(tree, path5, &exact_match) == tree->root &&
                 !exact_match);
    _dbus_assert(find_handler(tree, path6, &exact_match) == tree->root &&
                 !exact_match);
    _dbus_assert(find_handler(tree, path7, &exact_match) == tree->root &&
                 !exact_match);
    _dbus_assert(find_handler(tree, path8, &exact_match) == tree->root &&
                 !exact_match);
    ...
    _dbus_assert(find_handler(tree, path0, &exact_match) == tree->root &&
                 exact_match);
    _dbus_assert(find_handler(tree, path1, &exact_match) != tree->root &&
                 exact_match);
    _dbus_assert(find_handler(tree, path2, &exact_match) != tree->root &&
  ...
  _dbus_object_tree_unregister_and_unlock (tree, path1);
    ...
    if (!_dbus_object_tree_register(tree, TRUE, path2, &test_vtable, NULL,
                                    NULL))
        goto out;
    ...
    if (!_dbus_object_tree_register(tree, TRUE, path12, &test_vtable, NULL,
                                    NULL))
        goto out;

    _dbus_assert(find_subtree(tree, path12, NULL));

    if (!_dbus_object_tree_register(tree, TRUE, path13, &test_vtable, NULL,
                                    NULL))
        goto out;

    _dbus_assert(find_subtree(tree, path13, NULL));

    if (!_dbus_object_tree_register(tree, TRUE, path14, &test_vtable, NULL,
                                    NULL))
        goto out;
  ...
  spew_tree (tree);
    ...
    if (!do_test_dispatch(tree, path0, 0, tree_test_data,
                          _DBUS_N_ELEMENTS(tree_test_data)))
        goto out;
    if (!do_test_dispatch(tree, path1, 1, tree_test_data,
                          _DBUS_N_ELEMENTS(tree_test_data)))
        goto out;
    if (!do_test_dispatch(tree, path2, 2, tree_test_data,
                          _DBUS_N_ELEMENTS(tree_test_data)))
        goto out;
    if (!do_test_dispatch(tree, path3, 3, tree_test_data,
                          _DBUS_N_ELEMENTS(tree_test_data)))
        goto out;
    if (!do_test_dispatch(tree, path4, 4, tree_test_data,
                          _DBUS_N_ELEMENTS(tree_test_data)))
        goto out;
    if (!do_test_dispatch(tree, path5, 5, tree_test_data,
                          _DBUS_N_ELEMENTS(tree_test_data)))
        goto out;
    if (!do_test_dispatch(tree, path6, 6, tree_test_data,
                          _DBUS_N_ELEMENTS(tree_test_data)))
        goto out;
    if (!do_test_dispatch(tree, path7, 7, tree_test_data,
                          _DBUS_N_ELEMENTS(tree_test_data)))
        goto out;
    if (!do_test_dispatch(tree, path8, 8, tree_test_data,
                          _DBUS_N_ELEMENTS(tree_test_data)))
        goto out;
...
{
    return _dbus_test_oom_handling("object tree", object_tree_test_iteration,
                                   NULL);
}
