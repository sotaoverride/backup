...
/** An opaque mutex type provided by the #DBusThreadFunctions implementation
 * installed by dbus_threads_init(). */
typedef struct DBusMutex DBusMutex;
/** An opaque condition variable type provided by the #DBusThreadFunctions
...
/** Deprecated, provide DBusRecursiveMutexLockFunction instead. Return value is
 * lock success, but gets ignored in practice. */
typedef dbus_bool_t (*DBusMutexLockFunction)(DBusMutex* mutex);
/** Deprecated, provide DBusRecursiveMutexUnlockFunction instead. Return value
 * is unlock success, but gets ignored in practice. */
...
/** Wakes one waiting thread on a condition variable.  Found in
 * #DBusThreadFunctions.
 *
 * Has no error conditions. Must succeed if it returns.
 */
...
/** Wakes all waiting threads on a condition variable.  Found in
 * #DBusThreadFunctions.
 *
 * Has no error conditions. Must succeed if it returns.
 */
...
/**
 * Flags indicating which functions are present in #DBusThreadFunctions. Used to
 * allow the library to detect older callers of dbus_threads_init() if new
 * possible functions are added to #DBusThreadFunctions.
 */
typedef enum
{
...
typedef struct
{
    ...
    DBusMutexNewFunction
        mutex_new; /**< Function to create a mutex; optional and deprecated. */
    DBusMutexFreeFunction
        mutex_free; /**< Function to free a mutex; optional and deprecated. */
    DBusMutexLockFunction
        mutex_lock; /**< Function to lock a mutex; optional and deprecated. */
    DBusMutexUnlockFunction mutex_unlock; /**< Function to unlock a mutex;
                                             optional and deprecated. */

    DBusCondVarNewFunction
        condvar_new; /**< Function to create a condition variable */
    DBusCondVarFreeFunction
        condvar_free; /**< Function to free a condition variable */
    DBusCondVarWaitFunction
        condvar_wait; /**< Function to wait on a condition */
    DBusCondVarWaitTimeoutFunction
        condvar_wait_timeout; /**< Function to wait on a condition with a
                                 timeout */
    DBusCondVarWakeOneFunction
        condvar_wake_one; /**< Function to wake one thread waiting on the
                             condition */
    DBusCondVarWakeAllFunction
        condvar_wake_all; /**< Function to wake all threads waiting on the
                             condition */

    DBusRecursiveMutexNewFunction
        recursive_mutex_new; /**< Function to create a recursive mutex */
    DBusRecursiveMutexFreeFunction
        recursive_mutex_free; /**< Function to free a recursive mutex */
    DBusRecursiveMutexLockFunction
        recursive_mutex_lock; /**< Function to lock a recursive mutex */
    DBusRecursiveMutexUnlockFunction
        recursive_mutex_unlock; /**< Function to unlock a recursive mutex */
