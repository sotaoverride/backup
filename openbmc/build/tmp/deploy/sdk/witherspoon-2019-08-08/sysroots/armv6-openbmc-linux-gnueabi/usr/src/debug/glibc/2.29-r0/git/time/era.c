...
#define ERA_DATE_CMP(a, b)                                                     \
    ...
    static void _nl_init_era_entries(struct __locale_data* current)
{
    size_t cnt;
    ...
    if (!data->era_initialized)
    {
        ...
        else
        {
            ...
            if (data->num_eras != new_num_eras)
                new_eras = (struct era_entry*)realloc(
                    data->eras, new_num_eras * sizeof(struct era_entry));
            if (new_eras == NULL)
            {
                free(data->eras);
                data->num_eras = 0;
...
{
    if (current->private.time == NULL ||
        !current->private.time->era_initialized)
    ...
    if (current->private.time != NULL)
    {
        ...
        for (cnt = 0; cnt < current->private.time->num_eras; ++cnt)
            if ((ERA_DATE_CMP(current->private.time->eras[cnt].start_date,
                              tdate) &&
                 ERA_DATE_CMP(tdate,
...
struct era_entry* _nl_select_era_entry(int cnt, struct __locale_data* current)
{
    if (current->private.time == NULL ||
        !current->private.time->era_initialized)
        _nl_init_era_entries(current);

    return (current->private.time == NULL ? NULL
                                          : &current->private.time->eras[cnt]);
}
