...
int switch_root(
    const char* new_root,
    const char* old_root_after, /* path below the new root, where to place the
                                   old root after the transition */
    bool unmount_old_root, unsigned long mount_flags)
{ /* MS_MOVE or MS_BIND */
    ...
    if (old_root_remove)
    {
        old_root_fd = open("/", O_RDONLY | O_NONBLOCK | O_CLOEXEC | O_NOCTTY |
                                    O_DIRECTORY);
        if (old_root_fd < 0)
    ...
    r = chase_symlinks(old_root_after, new_root,
                       CHASE_PREFIX_ROOT | CHASE_NONEXISTENT,
                       &resolved_old_root_after);
    if (r < 0)
        return log_error_errno(r, "Failed to resolve %s/%s: %m", new_root,
                               old_root_after);
     ...
     * systems are mounted MS_SHARED. Hence remount them MS_PRIVATE here as a
     * work-around.
     *
     * https://bugzilla.redhat.com/show_bug.cgi?id=847418 */
    if (mount(NULL, "/", NULL, MS_REC | MS_PRIVATE, NULL) < 0)
        return log_error_errno(
            errno, "Failed to set \"/\" mount propagation to private: %m");

    FOREACH_STRING(i, "/sys", "/dev", "/run", "/proc")
    {
        _cleanup_free_ char* chased = NULL;

        r = chase_symlinks(i, new_root, CHASE_PREFIX_ROOT | CHASE_NONEXISTENT,
                           &chased);
        if (r < 0)
            return log_error_errno(r, "Failed to resolve %s/%s: %m", new_root,
                                   i);
        if (r > 0)
        {
            ...
            if (r < 0)
                return log_error_errno(
                    r, "Failed to determine whether %s is a mount point: %m",
                    chased);
        ...
        if (mount(i, chased, NULL, mount_flags, NULL) < 0)
            return log_error_errno(errno, "Failed to mount %s to %s: %m", i,
                                   chased);
    }

    /* Do not fail if base_filesystem_create() fails. Not all switch roots are
     * like base_filesystem_create() wants them to look like. They might even
     * boot, if they are RO and don't have the FS layout. Just ignore the error
     * and switch_root() nevertheless. */
    (void)base_filesystem_create(new_root, UID_INVALID, GID_INVALID);

    if (chdir(new_root) < 0)
        return log_error_errno(errno, "Failed to change directory to %s: %m",
                               new_root);

    /* We first try a pivot_root() so that we can umount the old root dir. In
     * many cases (i.e. where rootfs is /), that's not possible however, and
     * hence we simply overmount root */
