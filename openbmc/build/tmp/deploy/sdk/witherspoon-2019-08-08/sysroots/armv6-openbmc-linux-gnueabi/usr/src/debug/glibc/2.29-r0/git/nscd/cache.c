...
struct datahead* cache_search(request_type type, const void* key, size_t len,
...
int cache_add(int type, const void* key, size_t len, struct datahead* packet,
              bool first, struct database_dyn* table, uid_t owner,
              bool prune_wakeup)
{
    if (__glibc_unlikely(debug_level >= 2))
    {
        ...
        if (type == GETHOSTBYADDR || type == GETHOSTBYADDRv6)
            str = inet_ntop(type == GETHOSTBYADDR ? AF_INET : AF_INET6, key,
                            buf, sizeof(buf));
        else
            str = key;

        dbg_log(_("add new entry \"%s\" of type %s for %s to cache%s"), str,
                serv2str[type], dbnames[table - dbs],
                first ? _(" (first)") : "");
    }
    ...
    while (!atomic_compare_exchange_weak_release(
        &table->head->array[hash], (ref_t*)&newp->next,
        (ref_t)((char*)newp - table->data)))
        ;
...
time_t prune_cache(struct database_dyn* table, time_t now, int fd)
{
...
#endif
            {
                ...
                else
                {
                    /* This must be `!=` to catch cases where users turn the
                       clocks back and we still want to detect any time
                       difference in mtime.  */
    ...
    if (__glibc_unlikely(debug_level > 2))
        dbg_log(_("pruning %s cache; time %ld"), dbnames[table - dbs],
                (long int)now);
    ...
    do
    {
        ...
        while (run != ENDREF)
        {
            ...
            if (__glibc_unlikely(debug_level > 2))
            {
                ...
                if (runp->type == GETHOSTBYADDR ||
                    runp->type == GETHOSTBYADDRv6)
                {
