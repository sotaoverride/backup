...
#define TRACE(ctx, ...)                                                        \
    do                                                                         \
    {                                                                          \
        if (ctx->trace_callback != NULL)                                       \
            krb5int_trace(ctx, __VA_ARGS__);                                   \
    } while (0)
#else
...
#define TRACE_CC_DESTROY(c, cache) TRACE(c, "Destroying ccache {ccache}", cache)
...
#define TRACE_CC_GET_CONFIG(c, cache, princ, key, data)                        \
    TRACE(c, "Read config in {ccache} for {princ}: {str}: {data}", cache,      \
          princ, key, data)
#define TRACE_CC_INIT(c, cache, princ)                                         \
    TRACE(c, "Initializing {ccache} with default princ {princ}", cache, princ)
...
#define TRACE_CC_RETRIEVE(c, cache, creds, ret)                                \
    TRACE(c, "Retrieving {creds} from {ccache} with result: {kerr}", creds,    \
          cache, ret)
...
#define TRACE_CC_SET_CONFIG(c, cache, princ, key, data)                        \
    TRACE(c, "Storing config in {ccache} for {princ}: {str}: {data}", cache,   \
          princ, key, data)
...
#define TRACE_CCSELECT_MODCHOICE(c, name, server, cache, princ)                \
    TRACE(c,                                                                   \
          "ccselect module {str} chose cache {ccache} with client "            \
          "principal {princ} for server principal {princ}",                    \
          name, cache, princ, server)
#define TRACE_CCSELECT_MODNOTFOUND(c, name, server, princ)                     \
    TRACE(c,                                                                   \
          "ccselect module {str} chose client principal {princ} "              \
          "for server principal {princ} but found no cache",                   \
          name, princ, server)
#define TRACE_CCSELECT_MODFAIL(c, name, ret, server)                           \
    TRACE(c,                                                                   \
          "ccselect module {str} yielded error {kerr} for server "             \
          "principal {princ}",                                                 \
          name, ret, server)
#define TRACE_CCSELECT_NOTFOUND(c, server)                                     \
    TRACE(c,                                                                   \
          "ccselect can't find appropriate cache for server "                  \
          "principal {princ}",                                                 \
          server)
#define TRACE_CCSELECT_DEFAULT(c, cache, server)                               \
    TRACE(c,                                                                   \
          "ccselect choosing default cache {ccache} for server "               \
          "principal {princ}",                                                 \
          cache, server)

#define TRACE_DNS_SRV_ANS(c, host, port, prio, weight)                         \
    TRACE(c, "SRV answer: {int} {int} {int} \"{str}\"", prio, weight, port,    \
          host)
#define TRACE_DNS_SRV_NOTFOUND(c) TRACE(c, "No SRV records found")
...
#define TRACE_DNS_URI_NOTFOUND(c) TRACE(c, "No URI records found")
...
#define TRACE_FAST_DECODE(c) TRACE(c, "Decoding FAST response")
...
#define TRACE_GIC_PWD_MASTER(c) TRACE(c, "Retrying AS request with master KDC")
...
#define TRACE_INIT_CREDS_GAK(c, salt, s2kparams)                               \
    TRACE(c, "Getting AS key, salt \"{data}\", params \"{data}\"", salt,       \
          s2kparams)
...
#define TRACE_KT_GET_ENTRY(c, keytab, princ, vno, enctype, err)                \
    TRACE(c,                                                                   \
          "Retrieving {princ} from {keytab} (vno {int}, enctype {etype}) "     \
          "with result: {kerr}",                                               \
          princ, keytab, (int)vno, enctype, err)

#define TRACE_LOCALAUTH_INIT_CONFLICT(c, type, oldname, newname)               \
    TRACE(c,                                                                   \
          "Ignoring localauth module {str} because it conflicts "              \
          "with an2ln type {str} from module {str}",                           \
...
#define TRACE_MK_REP(c, ctime, cusec, subkey, seqnum)                          \
    TRACE(c,                                                                   \
          "Creating AP-REP, time {long}.{int}, subkey {keyblock}, "            \
          "seqnum {int}",                                                      \
          (long)ctime, (int)cusec, subkey, (int)seqnum)

#define TRACE_MK_REQ(c, creds, seqnum, subkey, sesskeyblock)                   \
    TRACE(c,                                                                   \
          "Creating authenticator for {creds}, seqnum {int}, "                 \
          "subkey {key}, session key {keyblock}",                              \
          creds, (int)seqnum, subkey, sesskeyblock)
...
#define TRACE_PREAUTH_CONFLICT(c, name1, name2, patype)                        \
    TRACE(c,                                                                   \
          "Preauth module {str} conflicts with module {str} for pa "           \
          "type {patype}",                                                     \
          name1, name2, patype)
...
#define TRACE_PREAUTH_ENC_TS(c, sec, usec, plain, enc)                         \
    TRACE(c,                                                                   \
          "Encrypted timestamp (for {long}.{int}): plain {hexdata}, "          \
          "encrypted {hexdata}",                                               \
          (long)sec, (int)usec, plain, enc)
...
#define TRACE_PREAUTH_ETYPE_INFO(c, etype, salt, s2kparams)                    \
    TRACE(c,                                                                   \
          "Selected etype info: etype {etype}, salt \"{data}\", "              \
          "params \"{data}\"",                                                 \
          etype, salt, s2kparams)
...
#define TRACE_PREAUTH_PROCESS(c, name, patype, real, code)                     \
    TRACE(c,                                                                   \
          "Preauth module {str} ({int}) ({str}) returned: "                    \
          "{kerr}",                                                            \
          name, (int)patype, real ? "real" : "info", code)
...
#define TRACE_PREAUTH_SALT(c, salt, patype)                                    \
    TRACE(c, "Received salt \"{data}\" via padata type {patype}", salt, patype)
#define TRACE_PREAUTH_SKIP(c, name, patype)                                    \
    TRACE(c, "Skipping previously used preauth module {str} ({int})", name,    \
          (int)patype)
...
#define TRACE_PREAUTH_TRYAGAIN(c, name, patype, code)                          \
    TRACE(c, "Preauth module {str} ({int}) tryagain returned: {kerr}", name,   \
          (int)patype, code)
...
#define TRACE_RD_REP(c, ctime, cusec, subkey, seqnum)                          \
    TRACE(c,                                                                   \
          "Read AP-REP, time {long}.{int}, subkey {keyblock}, "                \
          "seqnum {int}",                                                      \
          (long)ctime, (int)cusec, subkey, (int)seqnum)
...
#define TRACE_RD_REQ_DECRYPT_ANY(c, princ, keyblock)                           \
    TRACE(c,                                                                   \
          "Decrypted AP-REQ with server principal {princ}: "                   \
          "{keyblock}",                                                        \
          princ, keyblock)
#define TRACE_RD_REQ_DECRYPT_SPECIFIC(c, princ, keyblock)                      \
    TRACE(c,                                                                   \
          "Decrypted AP-REQ with specified server principal {princ}: "         \
          "{keyblock}",                                                        \
          princ, keyblock)
...
#define TRACE_RD_REQ_NEGOTIATED_ETYPE(c, etype)                                \
    TRACE(c, "Negotiated enctype based on authenticator: {etype}", etype)
...
#define TRACE_SENDTO_KDC_ERROR_SET_MESSAGE(c, raddr, err)                      \
    TRACE(c, "Error preparing message to send to {raddr}: {errno}", raddr, err)
#define TRACE_SENDTO_KDC(c, len, rlm, master, tcp)                             \
    TRACE(c, "Sending request ({int} bytes) to {data}{str}{str}", len, rlm,    \
          (master) ? " (master)" : "", (tcp) ? " (tcp only)" : "")
...
#define TRACE_TGS_REPLY(c, client, server, keyblock)                           \
    TRACE(c,                                                                   \
          "TGS reply is for {princ} -> {princ} with session key "              \
          "{keyblock}",                                                        \
          client, server, keyblock)
...
#define TRACE_TLS_ERROR(c, errs) TRACE(c, "TLS error: {str}", errs)
...
#define TRACE_TLS_SERVER_NAME_MISMATCH(c, hostname)                            \
    TRACE(c,                                                                   \
          "TLS certificate name mismatch: server certificate is "              \
          "not for \"{str}\"",                                                 \
          hostname)
...
#define TRACE_TKT_CREDS(c, creds, cache)                                       \
    TRACE(c, "Getting credentials {creds} using ccache {ccache}", creds, cache)
...
#define TRACE_TKT_CREDS_FALLBACK(c, realm)                                     \
    TRACE(c, "Local realm referral failed; trying fallback realm {data}", realm)
...
#define TRACE_TKT_CREDS_NON_TGT(c, princ)                                      \
    TRACE(c,                                                                   \
          "Received non-TGT referral response ({princ}); trying "              \
          "again without referrals",                                           \
          princ)
...
#define TRACE_TKT_CREDS_SAME_REALM_TGT(c, realm)                               \
    TRACE(c,                                                                   \
          "Received TGT referral back to same realm ({data}); trying "         \
          "again without referrals",                                           \
          realm)
...
#define TRACE_CHECK_REPLY_SERVER_DIFFERS(c, request, reply)                    \
    TRACE(c, "Reply server {princ} differs from requested {princ}", reply,     \
          request)

#define TRACE_GET_CRED_VIA_TKT_EXT(c, request, reply, kdcoptions)              \
    TRACE(c,                                                                   \
          "Get cred via TGT {princ} after requesting {princ} "                 \
          "(canonicalize {str})",                                              \
