...
static int validate_request_name_parameters(sd_bus* bus, const char* name,
                                            uint64_t flags, uint32_t* ret_param)
{
    ...
    assert_return(
        !(flags & ~(SD_BUS_NAME_ALLOW_REPLACEMENT |
...
_public_ int sd_bus_request_name(sd_bus* bus, const char* name, uint64_t flags)
{
    ...
    r = sd_bus_call_method(bus, "org.freedesktop.DBus", "/org/freedesktop/DBus",
                           "org.freedesktop.DBus", "RequestName", NULL, &reply,
                           "su", name, param);
    if (r < 0)
        return r;
...
static int default_request_name_handler(sd_bus_message* m, void* userdata,
...
_public_ int sd_bus_request_name_async(sd_bus* bus, sd_bus_slot** ret_slot,
                                       ...
                                       void* userdata)
{
    ...
    return sd_bus_call_method_async(
        bus, ret_slot, "org.freedesktop.DBus", "/org/freedesktop/DBus",
        "org.freedesktop.DBus", "RequestName",
        callback ?: default_request_name_handler, userdata, "su", name, param);
}
...
_public_ int sd_bus_release_name(sd_bus* bus, const char* name)
{
    ...
    r = sd_bus_call_method(bus, "org.freedesktop.DBus", "/org/freedesktop/DBus",
                           "org.freedesktop.DBus", "ReleaseName", NULL, &reply,
                           "s", name);
    if (r < 0)
        return r;
...
static int default_release_name_handler(sd_bus_message* m, void* userdata,
                                        sd_bus_error* ret_error)
{
    ...
    switch (ret)
    {
        ...
        case BUS_NAME_NON_EXISTENT:
            log_debug(
                "Name asked to release is not taken currently, ignoring.");
            return 1;

        case BUS_NAME_NOT_OWNER:
            log_debug(
                "Name asked to release is owned by somebody else, ignoring.");
            return 1;
...
_public_ int sd_bus_release_name_async(sd_bus* bus, sd_bus_slot** ret_slot,
                                       const char* name,
                                       sd_bus_message_handler_t callback,
                                       void* userdata)
{
    ...
    return sd_bus_call_method_async(
        ...
        callback ?: default_release_name_handler, userdata, "s", name);
}

_public_ int sd_bus_list_names(sd_bus* bus, char*** acquired,
                               char*** activatable)
{
    _cleanup_(sd_bus_message_unrefp) sd_bus_message* reply = NULL;
...
_public_ int sd_bus_get_name_creds(sd_bus* bus, const char* name, uint64_t mask,
                                   sd_bus_creds** creds)
{

    _cleanup_(sd_bus_message_unrefp) sd_bus_message *reply_unique = NULL,
                                                    *reply = NULL;
    ...
    /* Turn off augmenting if this isn't a local connection. If the connection
     * is not local, then /proc is not going to match. */
    if (!bus->is_local)
    ...
    /* Only query the owner if the caller wants to know it and the name is not
     * unique anyway, or if the caller just
    ...
    if (mask != 0)
    {
        ...
        need_pid =
            ...
            ((mask & SD_BUS_CREDS_AUGMENT) &&
             (mask &
              (SD_BUS_CREDS_UID | SD_BUS_CREDS_SUID | SD_BUS_CREDS_FSUID |
        ...
        if (need_pid + need_uid + need_selinux > 1)
        {
            ...
            /* If we need more than one of the credentials, then use
             * GetConnectionCredentials() */

            r = sd_bus_call_method(
                bus, "org.freedesktop.DBus", "/org/freedesktop/DBus",
                "org.freedesktop.DBus", "GetConnectionCredentials", &error,
                &reply, "s", unique ?: name);
            ...
            if (r < 0)
            {
                ...
                /* If we got an unknown method error, fall back to the invidual
                 * calls... */
        ...
        if (need_separate_calls)
        {
            if (need_pid)
            {
                ...
                r = sd_bus_call_method(
                    bus, "org.freedesktop.DBus", "/org/freedesktop/DBus",
                    "org.freedesktop.DBus", "GetConnectionUnixProcessID", NULL,
                    &reply, "s", unique ?: name);
                if (r < 0)
                    return r;
            ...
            if (need_uid)
            {
                ...
                r = sd_bus_call_method(
                    bus, "org.freedesktop.DBus", "/org/freedesktop/DBus",
                    "org.freedesktop.DBus", "GetConnectionUnixUser", NULL,
                    &reply, "s", unique ?: name);
                if (r < 0)
                    return r;
...
_public_ int sd_bus_get_owner_creds(sd_bus* bus, uint64_t mask,
                                    sd_bus_creds** ret)
{
    _cleanup_(sd_bus_creds_unrefp) sd_bus_creds* c = NULL;
    ...
    do_groups =
        bus->n_groups != (size_t)-1 && (mask & SD_BUS_CREDS_SUPPLEMENTARY_GIDS);
...
#define append_eavesdrop(bus, m)                                               \
    ((bus)->is_monitor ? (isempty(m) ? "eavesdrop='true'"                      \
...
int bus_add_match_internal(sd_bus* bus, const char* match)
{
    ...
    return sd_bus_call_method(bus, "org.freedesktop.DBus",
                              "/org/freedesktop/DBus", "org.freedesktop.DBus",
                              "AddMatch", NULL, NULL, "s", e);
}

int bus_add_match_internal_async(sd_bus* bus, sd_bus_slot** ret_slot,
                                 const char* match,
                                 sd_bus_message_handler_t callback,
                                 void* userdata)
{
    ...
    return sd_bus_call_method_async(
        bus, ret_slot, "org.freedesktop.DBus", "/org/freedesktop/DBus",
        "org.freedesktop.DBus", "AddMatch", callback, userdata, "s", e);
}
...
int bus_remove_match_internal(sd_bus* bus, const char* match)
{
    ...
    return sd_bus_call_method_async(
        ...
        "org.freedesktop.DBus", "RemoveMatch", NULL, NULL, "s", e);
}

_public_ int sd_bus_get_name_machine_id(sd_bus* bus, const char* name,
                                        sd_id128_t* machine)
{
    ...
    r = sd_bus_message_new_method_call(
        bus, &m, name, "/", "org.freedesktop.DBus.Peer", "GetMachineId");
    if (r < 0)
        return r;
