...
static int c_dvar_write_data(CDVar* var, int alignment, const void* data,
                             size_t n_data)
{
    size_t n, align;
    int shift;
    ...
    align = ALIGN_TO(var->current->i_buffer, 1 << alignment) -
    ...
    while ((c = *format++))
    {
        ...
        switch (c)
        {
            case '[':
                ...
                r = c_dvar_write_data(
                    var, (var->current->i_type + 1)->alignment, NULL, 0);
                if (r)
                    return r;
            ...
            case '<':
                ...
                for (n = 0; n < type->length; ++n)
                    var->data[var->current->i_buffer - type->length - 1 + n] =
                        type[n].element;
            ...
            case ']':
                /* compute array size */
                u32 = c_dvar_bswap32(var, var->current->i_buffer -
                                              (var->current - 1)->i_buffer);
 ...
 */
_public_ void c_dvar_begin_write(CDVar* var, bool big_endian,
                                 const CDVarType* types, size_t n_types)
{
    size_t i;
