...
static void g_themed_icon_get_property(GObject* object, guint prop_id,
...
static void g_themed_icon_set_property(GObject* object, guint prop_id,
...
static void g_themed_icon_class_init(GThemedIconClass* klass)
{
    ...
    g_object_class_install_property(
        ...
        g_param_spec_string(
            "name", P_("name"), P_("The name of the icon"), NULL,
     ...
     */
    g_object_class_install_property(
        gobject_class, PROP_NAMES,
        g_param_spec_boxed(
            "names", P_("names"), P_("An array containing the icon names"),
            G_TYPE_STRV,
     ...
     */
    g_object_class_install_property(
        gobject_class, PROP_USE_DEFAULT_FALLBACKS,
        g_param_spec_boolean(
            "use-default-fallbacks", P_("use default fallbacks"),
            P_("Whether to use default fallbacks found by shortening the name "
               "at “-” characters. Ignores names after the first if multiple "
               "names are given."),
            FALSE,
            G_PARAM_CONSTRUCT_ONLY | G_PARAM_READWRITE | G_PARAM_STATIC_NAME |
                G_PARAM_STATIC_BLURB | G_PARAM_STATIC_NICK));
...
static void g_themed_icon_update_names(GThemedIcon* themed)
{
    ...
    g_return_if_fail(themed->init_names != NULL &&
                     themed->init_names[0] != NULL);
    ...
    for (i = 0; themed->init_names[i]; i++)
    {
        ...
        if (is_symbolic)
            name = g_strndup(themed->init_names[i],
                             strlen(themed->init_names[i]) - 9);
        else
        ...
        if (themed->use_default_fallbacks)
        {
            ...
            while ((dashp = strrchr(last, '-')) != NULL)
            {
                ...
                if (is_symbolic)
                {
                    ...
                    fallback = last;
                if (g_list_find_custom(names, fallback,
    ...
    g_strfreev(themed->names);
    themed->names =
        g_new(char*, g_list_length(names) + g_list_length(variants) + 1);
...
GIcon* g_themed_icon_new_from_names(char** iconnames, int len)
{
    ...
    if (len >= 0)
    {
        ...
        icon =
            G_ICON(g_object_new(G_TYPE_THEMED_ICON, "names", iconnames, NULL));
...
GIcon* g_themed_icon_new_with_default_fallbacks(const char* iconname)
{
    ...
    return G_ICON(g_object_new(G_TYPE_THEMED_ICON, "name", iconname,
                               "use-default-fallbacks", TRUE, NULL));
}
...
void g_themed_icon_append_name(GThemedIcon* icon, const char* iconname)
{
    ...
    icon->init_names =
        g_realloc(icon->init_names, sizeof(char*) * (num_names + 2));
...
static gboolean g_themed_icon_to_tokens(GIcon* icon, GPtrArray* tokens,
                                        gint* out_version)
{
    ...
    *out_version = 0;
    ...
    for (n = 0; themed_icon->names[n] != NULL; n++)
        g_ptr_array_add(tokens, g_strdup(themed_icon->names[n]));

    return TRUE;
}

static GIcon* g_themed_icon_from_tokens(gchar** tokens, gint num_tokens,
                                        gint version, GError** error)
{
    GIcon* icon;
    gchar** names;
    int n;
...
static GVariant* g_themed_icon_serialize(GIcon* icon)
{
    GThemedIcon* themed_icon = G_THEMED_ICON(icon);

    return g_variant_new("(sv)", "themed",
                         g_variant_new("^as", themed_icon->names));
}
