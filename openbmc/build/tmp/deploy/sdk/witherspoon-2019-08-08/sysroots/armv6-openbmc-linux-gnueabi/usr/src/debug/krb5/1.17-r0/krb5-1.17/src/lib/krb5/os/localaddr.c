...
#endif
#define ifreq_size(i)                                                          \
    max(sizeof(struct ifreq), sizeof((i).ifr_name) + (i).ifr_addr.sa_len)
#else
...
/*@modifies fileSystem@*/
{
    ...
    printf("%p ", (void*)sa);
    err =
        getnameinfo(sa, sa_socklen(sa), buf, sizeof(buf), 0, 0, NI_NUMERICHOST);
    if (err)
        printf("<getnameinfo error %d: %s> family=%d", err, gai_strerror(err),
               sa->sa_family);
    else
        printf("%s", buf);
}
...
static int is_loopback_address(struct sockaddr* sa)
{
    switch (sa->sa_family)
    {
        case AF_INET:
...
static int addr_eq(struct sockaddr* s1, struct sockaddr* s2)
{
...
static /*@null@*/ void* grow_or_free(/*@only@*/ void* ptr, size_t newsize)
/*@*/
{
    void* newptr;
...
static int get_ifconf(int s, size_t* lenp, /*@out@*/ char* buf)
/*@modifies *buf,*lenp@*/
{
    int ret;
    struct ifconf ifc;
...
static int get_lifconf(int af, int s, size_t* lenp, /*@out@*/ char* buf)
/*@modifies *buf,*lenp@*/
{
    int ret;
    struct lifconf lifc;
...
/* #include <net/if6.h> */
static int get_if_laddrconf(int af, int s, size_t* lenp, /*@out@*/ char* buf)
/*@modifies *buf,*lenp@*/
{
    int ret;
...
static struct linux_ipv6_addr_list* get_linux_ipv6_addrs()
{
    ...
    if (f)
    {
        ...
        while (fscanf(f,
                      ...
                      &addrbyte[15], &idx, &pfxlen, &scope, &dadstat,
                      ifname) != EOF)
        {
            for (i = 0; i < 16; i++)
...
punt:
    ...
    defined(HAVE_STRUCT_LIFCONF) /* Solaris 8 and later; Sol 7? */
        ...
        for (i = 0; i + sizeof(*lifr) <= P.buf_size; i += sizeof(*lifr))
        {
            ...
            for (j = 0; j < i; j += sizeof(*lifr2))
            {
                ...
                if (lifr2->lifr_addr.ss_family == lifr->lifr_addr.ss_family
                    ...
                    &&
                    !memcmp(&lifr2->lifr_addr, &lifr->lifr_addr, sizeof(*lifr)))
                {
...
#elif defined(SIOCGLIFNUM) && defined(HAVE_STRUCT_IF_LADDRCONF) &&             \
    0 /* HP-UX 11 support being debugged */
            ...
            for (j = 0; j < i; j += sizeof(*lifr2))
            {
                ...
                if (lifr2->iflr_addr.sa_family == lifr->iflr_addr.sa_family
                    /* Compare address info.  If this isn't good enough --
                       ...
                       to do it on a per address family basis.  */
                    &&
                    !memcmp(&lifr2->iflr_addr, &lifr->iflr_addr, sizeof(*lifr)))
                {
                    Tprintf(("  duplicate addr\n"));
                    goto skip;
                }
...
static int get_ifreq_array(char** bufp, size_t* np, int s)
{
    int code;
    int est_if_count = 8;
...
static krb5_error_code get_localaddrs(krb5_context context,
                                      krb5_address*** addr, int use_profile);
...
/*@modifies fileSystem@*/
{
    ...
    len = sa_socklen(sa);
    err = getnameinfo(sa, len, hostbuf, (socklen_t)sizeof(hostbuf), (char*)NULL,
                      0, NI_NUMERICHOST);
    if (err)
    {
        int e = errno;
...
static int count_addrs(void* P_data, struct sockaddr* a)
/*@*/
{
...
static int allocate(void* P_data)
/*@*/
{
    struct localaddr_data* data = P_data;
    int i;
...
static /*@null@*/ krb5_address* make_addr(int type, size_t length,
                                          const void* contents)
/*@*/
{
    krb5_address* a;
...
static int add_addr(void* P_data, struct sockaddr* a)
/*@modifies *P_data@*/
{
    struct localaddr_data* data = P_data;
...
static krb5_error_code krb5_os_localaddr_profile(krb5_context context,
                                                 struct localaddr_data* datap)
{
    krb5_error_code err;
    static const char* const profile_name[] = {KRB5_CONF_LIBDEFAULTS,
                                               KRB5_CONF_EXTRA_ADDRESSES, 0};
    char** values;
    char** iter;
    ...
    for (iter = values; *iter; iter++)
    {
        ...
        for (cp = *iter, next = 0; *cp; cp = next)
        {
            ...
            {
#ifdef DEBUG
                fprintf(stderr, "    %d: family %d", i, newaddrs[i]->addrtype);
                fprintf(stderr, "\n");
#endif
            }
            count = i;
                                      ...
                                      krb5_address*** addr, int use_profile)
{
    ...
    else
    {
        ...
        *addr = (krb5_address**)realloc(data.addr_temp,
                                        (sizeof(krb5_address*) * data.cur_idx));
        if (*addr == 0)
...
#ifdef DEBUG
    {
        ...
        for (j = 0; addr[0][j]; j++)
        {
            ...
            err2 = getnameinfo(ss2sa(&ss), sa_socklen(ss2sa(&ss)), namebuf,
                               sizeof(namebuf), 0, 0, NI_NUMERICHOST);
            if (err2 == 0)
                                                ...
                                                krb5_address*** addr)
{
    ...
    *addr = 0;
    ...
    for (i = 0; i < count; i++)
    {
        ...
        memcpy(paddr[i]->contents, hostrec->h_addr_list[i], paddr[i]->length);
    }
