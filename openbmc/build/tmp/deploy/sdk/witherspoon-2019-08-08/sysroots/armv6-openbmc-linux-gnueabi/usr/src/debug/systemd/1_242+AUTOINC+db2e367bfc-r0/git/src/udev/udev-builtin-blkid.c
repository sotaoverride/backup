...
static void print_property(sd_device* dev, bool test, const char* name,
                           const char* value)
{
    char s[256];
...
static int find_gpt_root(sd_device* dev, blkid_probe pr, bool test)
{
    ...
    if (found_esp && root_id)
        udev_builtin_add_property(dev, test, "ID_PART_GPT_AUTO_ROOT_UUID",
                                  root_id);
#endif
...
static int builtin_blkid(sd_device* dev, int argc, char* argv[], bool test)
{
            ...
            case 'o':
                ...
                if (r < 0)
                    return log_device_error_errno(
                        dev, r, "Failed to parse '%s' as an integer: %m",
                        optarg);
                if (offset < 0)
                    return log_device_error_errno(
                        dev, SYNTHETIC_ERRNO(ERANGE),
                        "Invalid offset %" PRIi64 ": %m", offset);
                break;
            case 'R':
                noraid = true;
                break;
    ...
    pr = blkid_new_probe();
    if (!pr)
        return log_device_debug_errno(dev, errno > 0 ? errno : ENOMEM,
    ...
    if (fd < 0)
        return log_device_debug_errno(
            dev, errno, "Failed to open block device %s: %m", devnode);

    errno = 0;
    r = blkid_probe_set_device(pr, fd, offset, 0);
    if (r < 0)
        return log_device_debug_errno(
            dev, errno > 0 ? errno : ENOMEM,
            "Failed to set device to blkid prober: %m");

    log_device_debug(dev, "Probe %s with %sraid and offset=%" PRIi64, devnode,
                     noraid ? "no" : "", offset);
    ...
    /* If the device is a partition then its parent passed the root partition
     * UUID to the device */
    (void)sd_device_get_property_value(dev, "ID_PART_GPT_AUTO_ROOT_UUID",
                                       &root_partition);
    ...
    for (i = 0; i < nvals; i++)
    {
        ...
        if (root_partition && streq(name, "PART_ENTRY_UUID") &&
            streq(data, root_partition))
