...
void set_compression(const char* fname)
{
    ...
    if (!(node = suftree) || !(s = strrchr(fname, '.')) || s == fname ||
        !(ltr = *++s))
        return;
...
static void send_deflated_token(int f, int32 token, struct map_struct* buf,
                                OFF_T offset, int32 nb, int32 toklen)
{
    int32 n, r;
    ...
    if (last_token == -1)
    {
        ...
        if (!init_done)
        {
            ...
            tx_strm.zalloc = NULL;
            tx_strm.zfree = NULL;
            if (deflateInit2(&tx_strm, compression_level, Z_DEFLATED, -15, 8,
