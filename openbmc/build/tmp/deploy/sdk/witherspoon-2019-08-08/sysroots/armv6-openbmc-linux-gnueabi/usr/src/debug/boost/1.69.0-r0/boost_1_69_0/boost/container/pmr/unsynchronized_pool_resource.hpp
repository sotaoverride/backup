...
//!   returned via calls to do_allocate. Each call to do_allocate(size,
//!   alignment) is dispatched to the pool serving the smallest blocks
//!   accommodating at least size bytes.
//!
...
//!   With each successive replenishment, the chunk size obtained increases
//!   geometrically. [ Note: By allocating memory in chunks, the pooling
//!   strategy increases the chance that consecutive allocations will be close
...
//!
//! An unsynchronized_pool_resource class may not be accessed from multiple
//! threads simultaneously and thus avoids the cost of synchronization entirely
//! in single-threaded applications.
class BOOST_CONTAINER_DECL unsynchronized_pool_resource : public memory_resource
{
  ...
  public:
    ...
    //!
    //! <b>Effects</b>: Constructs a pool resource object that will obtain
    //! memory
    ...
    //!   upstream points. [ Note: The intention is that calls to
    //!   upstream->allocate() will be substantially fewer than calls to
    //!   this->allocate() in most cases. - end note The behavior of the pooling
    //!   mechanism is tuned according to the value of the opts argument.
    //!
    //! <b>Throws</b>: Nothing unless upstream->allocate() throws. It is
    //! unspecified if
  ...
  public:
    ...
    //! <b>Effects</b>: Calls Calls `upstream_resource()->deallocate()` as
    //! necessary
    ...
    //! <b>Returns</b>: The options that control the pooling behavior of this
    //! resource.
    //!   The values in the returned struct may differ from those supplied to
    //!   the pool resource constructor in that values of zero will be replaced
    //!   with implementation-defined defaults and sizes may be rounded to
  ...
  protected:
    ...
    //!
    //! <b>Effects</b>: If the pool selected for a block of size bytes is unable
    //! to
    ...
    //! <b>Effects</b>: Return the memory at p to the pool. It is unspecified if
    //! or under
  ...
  public:
    //! <b>Returns</b>: The number of pools that will be used in the pool
    //! resource.
    //!
    //! <b>Note</b>: Non-standard extension.
    std::size_t pool_count() const;

    //! <b>Returns</b>: The index of the pool that will be used to serve the
    //! allocation of `bytes`.
    //!   Returns `pool_count()` if `bytes` is bigger
    //!   than `options().largest_required_pool_block` (no pool will be used to
    //!   serve this).
    //!
    //! <b>Note</b>: Non-standard extension.
    ...
    //!
    //! <b>Returns</b>: The number blocks that will be allocated in the next
    //! chunk
    ...
    //! <b>Requires</b>: `pool_idx < pool_index()`
    //!
    //! <b>Returns</b>: The number of bytes of the block that the specified
    //! `pool_idx` pool manages.
    //!
    //! <b>Note</b>: Non-standard extension.
    ...
    //! <b>Requires</b>: `pool_idx < pool_index()`
    //!
    //! <b>Returns</b>: The number of blocks that the specified `pool_idx` pool
    //! has cached
