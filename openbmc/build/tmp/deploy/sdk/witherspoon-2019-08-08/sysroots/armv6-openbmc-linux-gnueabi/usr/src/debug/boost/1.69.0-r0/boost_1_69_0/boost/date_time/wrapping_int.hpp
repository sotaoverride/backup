...
//! A wrapping integer used to support time durations (WARNING: only instantiate
//! with a signed type)
...
class wrapping_int
{
  public:
    ...
    //! Subtract will return '+d' if wrapping under took place ('d' is the
    //! number of wraps)
    /*! The sign of the returned value will indicate which direction the
...
//! A wrapping integer used to wrap around at the top (WARNING: only instantiate
//! with a signed type)
...
class wrapping_int2
{
  public:
    ...
    {
        int_type remainder =
            static_cast<int_type>(v % (wrap_max - wrap_min + 1));
