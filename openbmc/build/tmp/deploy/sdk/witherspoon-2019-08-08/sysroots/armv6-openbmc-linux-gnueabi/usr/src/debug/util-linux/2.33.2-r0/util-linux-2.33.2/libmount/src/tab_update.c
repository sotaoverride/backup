 ...
 * systems both with and without /etc/mtab. On systems without the regular
 * /etc/mtab file, the userspace mount options (e.g. user=) are stored in the
 * /run/mount/utab file.
 *
                      ...
                      const char* target, struct libmnt_fs* fs)
{
    ...
    DBG(UPDATE, ul_debugobj(upd, "resetting FS [target=%s, flags=0x%08lx]",
                            target, mountflags));
    if (fs)
    {
                          ...
                          unsigned long mountflags)
{
    ...
    if (o)
    {
        ...
        rc = mnt_optstr_get_options(
            o, &u, mnt_get_builtin_optmap(MNT_USERSPACE_MAP), MNT_NOMTAB);
        if (rc)
            goto err;
    }
    ...
    if (mountflags & MS_BIND)
    {
        ...
        if (src)
        {
            ...
            if (rc)
             ...
             (strcmp(fstype, "btrfs") == 0 || strcmp(fstype, "auto") == 0))
    {
        if (!upd->mountinfo)
            upd->mountinfo = mnt_new_table_from_file(_PATH_PROC_MOUNTINFO);
    }

    src_fs = mnt_table_get_fs_root(upd->mountinfo, fs, mountflags, &fsroot);
    if (src_fs)
    {
...
static int fprintf_mtab_fs(FILE* f, struct libmnt_fs* fs)
{
    ...
    if (m1 && m2 && m3 && m4)
    {
        ...
        rc = fprintf(f, "%s %s %s %s %d %d\n", m1, m2, m3, m4,
                     mnt_fs_get_freq(fs), mnt_fs_get_passno(fs));
        if (rc > 0)
            rc = 0;
    }
    else
...
static int update_table(struct libmnt_update* upd, struct libmnt_table* tb)
{
    ...
    if (f)
    {
        ...
        while (mnt_table_next_fs(tb, &itr, &fs) == 0)
        {
            ...
            if (rc)
            {
                DBG(UPDATE, ul_debugobj(upd, "%s: write entry failed: %m", uq));
                goto leave;
            }
        }
...
static int update_add_entry(struct libmnt_update* upd, struct libmnt_lock* lc)
{
    ...
    tb = __mnt_new_table_from_file(
        upd->filename, upd->userspace_only ? MNT_FMT_UTAB : MNT_FMT_MTAB, 1);
    if (tb)
                               ...
                               struct libmnt_lock* lc)
{
    ...
    if (tb)
    {
        struct libmnt_fs* rem =
            mnt_table_find_target(tb, upd->target, MNT_ITER_BACKWARD);
        if (rem)
        {
                                ...
                                struct libmnt_lock* lc)
{
    ...
    if (tb)
    {
        struct libmnt_fs* cur = mnt_table_find_target(
            tb, mnt_fs_get_srcpath(upd->fs), MNT_ITER_BACKWARD);
        if (cur)
        {
                                 ...
                                 struct libmnt_lock* lc)
{
    ...
    if (tb)
    {
        struct libmnt_fs* cur =
            mnt_table_find_target(tb, mnt_fs_get_target(fs), MNT_ITER_BACKWARD);
        if (cur)
        {
            if (upd->userspace_only)
...
int mnt_update_table(struct libmnt_update* upd, struct libmnt_lock* lc)
{
    ...
    upd->ready = FALSE;
    DBG(UPDATE,
        ul_debugobj(upd, "%s: update tab: done [rc=%d]", upd->filename, rc));
    if (lc != lc0)
        mnt_free_lock(lc);
    return rc;
}
...
int mnt_update_already_done(struct libmnt_update* upd, struct libmnt_lock* lc)
{
    ...
    DBG(UPDATE,
        ul_debugobj(upd, "%s: checking for previous update", upd->filename));
    ...
    if (upd->fs)
    {
        ...
        {
            DBG(UPDATE,
                ul_debugobj(upd, "%s: found %s %s", upd->filename, src, tgt));
            rc = 1;
        }
    }
    else if (upd->target)
...
static int update(const char* target, struct libmnt_fs* fs,
                  unsigned long mountflags)
{
    int rc;
    struct libmnt_update* upd;
...
int main(int argc, char* argv[])
{
    struct libmnt_test tss[] = {
        ...
        {"--remount", test_remount,
         "<target>  <options>           MS_REMOUNT mtab change"},
        {"--replace", test_replace,
         "<src> <target>                Add a line to LIBMOUNT_FSTAB and "
