...
td_err_e td_thr_tsd(const td_thrhandle_t* th, const thread_key_t tk,
                    void** data)
{
    td_err_e err;
    ...
    err = DB_GET_FIELD(level1, th->th_ta_p, th->th_unique, pthread, specific,
                       idx1st);
    if (err == TD_NOAPLIC)
        return TD_DBERR;
    if (err != TD_OK)
    ...
    err = DB_GET_FIELD_ADDRESS(level2, th->th_ta_p, level1,
                               pthread_key_data_level2, data, idx2nd);
    if (err == TD_NOAPLIC)
        return TD_DBERR;
    if (err != TD_OK)
        return err;
    ...
    err =
        DB_GET_FIELD_LOCAL(value, th->th_ta_p, copy, pthread_key_data, data, 0);
    if (err == TD_OK)
        *data = value;
