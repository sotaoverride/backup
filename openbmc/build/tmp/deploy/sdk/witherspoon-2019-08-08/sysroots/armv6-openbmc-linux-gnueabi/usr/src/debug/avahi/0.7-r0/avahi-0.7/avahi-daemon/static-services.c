...
static void entry_group_callback(AvahiServer* s,
                                 AVAHI_GCC_UNUSED AvahiSEntryGroup* eg,
                                 AvahiEntryGroupState state, void* userdata)
{
    ...
    switch (state)
    {
        ...
        case AVAHI_ENTRY_GROUP_COLLISION:
        {
            ...
            avahi_log_notice(
                "Service name conflict for \"%s\" (%s), retrying with \"%s\".",
                g->name, g->filename, g->chosen_name);
        ...
        case AVAHI_ENTRY_GROUP_ESTABLISHED:
            avahi_log_info("Service \"%s\" (%s) successfully established.",
                           g->chosen_name, g->filename);
            break;

        case AVAHI_ENTRY_GROUP_FAILURE:
            avahi_log_warn("Failed to publish service \"%s\" (%s): %s",
                           g->chosen_name, g->filename,
...
static void add_static_service_group_to_server(StaticServiceGroup* g)
{
    ...
    if (!g->entry_group)
        g->entry_group =
            avahi_s_entry_group_new(avahi_server, entry_group_callback, g);
    ...
    for (s = g->services; s; s = s->services_next)
    {
        ...
        if (avahi_server_add_service_strlst(
                avahi_server, g->entry_group, AVAHI_IF_UNSPEC, s->protocol, 0,
                g->chosen_name, s->type, s->domain_name, s->host_name, s->port,
                s->txt_records) < 0)
        {
            avahi_log_error("Failed to add service '%s' of type '%s', ignoring "
                            "service group (%s): %s",
        ...
        for (i = s->subtypes; i; i = i->next)
        {
            ...
            if (avahi_server_add_service_subtype(
                    avahi_server, g->entry_group, AVAHI_IF_UNSPEC, s->protocol,
                    0, g->chosen_name, s->type, s->domain_name,
                    (char*)i->text) < 0)
            {

                avahi_log_error(
                    "Failed to add subtype '%s' for service '%s' of type '%s', "
                    "ignoring subtype (%s): %s",
...
static void XMLCALL xml_start(void* data, const char* el, const char* attr[])
{
    ...
    else if (u->current_tag == XML_TAG_SERVICE_GROUP &&
             strcmp(el, "service") == 0)
    {
        ...
        if (attr[0])
        {
            if (strcmp(attr[0], "protocol") == 0)
            {
                ...
                {
                    avahi_log_error("%s: parse failure: invalid protocol "
                                    "specification \"%s\".",
                                    u->group->filename, attr[1]);
                    u->failed = 1;
                    return;
                }
    ...
    else if (u->current_tag == XML_TAG_SERVICE &&
             strcmp(el, "domain-name") == 0)
    {
        if (attr[0])
            goto invalid_attr;
    ...
    else if (u->current_tag == XML_TAG_SERVICE && strcmp(el, "txt-record") == 0)
    {
        if (attr[0])
        {
            if (strcmp(attr[0], "value-format") == 0)
            {
                ...
                {
                    avahi_log_error("%s: parse failure: invalid txt record "
                                    "value format specification \"%s\".",
                                    u->group->filename, attr[1]);
                    u->failed = 1;
                    return;
                }
    ...
    {
        avahi_log_error("%s: parse failure: didn't expect element <%s>.",
                        u->group->filename, el);
        u->failed = 1;
    }
...
invalid_attr:
    avahi_log_error("%s: parse failure: invalid attribute for element <%s>.",
                    u->group->filename, el);
    u->failed = 1;
    return;
}
...
static int decode_hex_buf(struct xml_userdata* u, uint8_t** out_buf,
                          size_t* out_buf_len)
{
    ...
    if (buf_len % 2)
    {
        avahi_log_error("%s: parse failure: hex value of the txt record should "
                        "have an even length",
                        u->group->filename);
        u->failed = 1;
        return -1;
    ...
    for (iter = 0; iter < raw_buf_len; iter++)
    {
        ...
        {
            avahi_log_error(
                "%s: parse failure: failed to parse hex data: invalid hex data",
                u->group->filename);
            u->failed = 1;
            avahi_free(raw_buf);
            return -1;
...
{
    avahi_log_error(
        "%s: parse failure: failed to parse base64 data: invalid base64 data",
        u->group->filename);
    u->failed = 1;
    avahi_free(raw_buf);
    return -1;
...
static int decode_base64_buf(struct xml_userdata* u, uint8_t** out_buf,
                             size_t* out_buf_len)
{
    const char* buf = (u->buf != NULL) ? u->buf : "";
    size_t buf_len = strlen(buf);
    ...
    if (buf_len % 4)
    {
        avahi_log_error("%s: parse failure: length of the base64 value of the "
                        "txt record should be a multiple of 4",
                        u->group->filename);
        u->failed = 1;
        return -1;
    }
    ...
    raw_buf = avahi_malloc(raw_buf_len);
    for (iter = 0, raw_iter = 0; iter < buf_len_no_equals;
         iter += 4, raw_iter += 3)
    {
...
static void XMLCALL xml_end(void* data, AVAHI_GCC_UNUSED const char* el)
{
    ...
    switch (u->current_tag)
    {
        case XML_TAG_SERVICE_GROUP:
            ...
            {
                avahi_log_error("%s: parse failure: service group incomplete.",
                                u->group->filename);
                u->failed = 1;
                return;
            }
        ...
        case XML_TAG_SERVICE:
            ...
            if (!u->service->type)
            {
                avahi_log_error("%s: parse failure: service incomplete.",
                                u->group->filename);
                u->failed = 1;
                return;
            }
        ...
        case XML_TAG_PORT:
        {
            ...
            {
                avahi_log_error(
                    "%s: parse failure: invalid port specification \"%s\".",
                    u->group->filename, u->buf);
                u->failed = 1;
                return;
            }
            ...
            if (u->txt_key != NULL)
            {
                ...
                switch (u->txt_type)
                {
                    ...
                    case TXT_RECORD_VALUE_BINARY_BASE64:
                        if (decode_base64_buf(u, &value_buf, &value_buf_len) <
                            0)
                            return;
                        free_value_buf = value_buf;
                        break;
                ...
                u->service->txt_records = avahi_string_list_add_anonymous(
                    u->service->txt_records, key_len + 1 + value_buf_len);
                memcpy(u->service->txt_records->text, u->txt_key, key_len);
                u->service->txt_records->text[key_len] = '=';
                memcpy(u->service->txt_records->text + key_len + 1, value_buf,
                       value_buf_len);
                ...
                u->service->txt_records = avahi_string_list_add(
                    u->service->txt_records, u->buf ? u->buf : "");
        ...
        case XML_TAG_SUBTYPE:
        {
            ...
            u->service->subtypes = avahi_string_list_add(u->service->subtypes,
                                                         u->buf ? u->buf : "");
            u->current_tag = XML_TAG_SERVICE;
            break;
        }
...
static void XMLCALL xml_cdata(void* data, const XML_Char* s, int len)
{
    ...
    switch (u->current_tag)
    {
        ...
        case XML_TAG_DOMAIN_NAME:
            assert(u->service);
            u->service->domain_name =
                append_cdata(u->service->domain_name, s, len);
            break;
        ...
        case XML_TAG_TXT_RECORD:
            ...
            if (u->txt_key == NULL)
            {
                ...
                if (equals != NULL)
                {
                    ...
                    u->buf = NULL;
                    /* len is now length of the rest of the string past the
                     * equals sign */
                    len -= equals - s + 1;
                    s = equals + 1;
                }
...
static int static_service_group_load(StaticServiceGroup* g)
{
    ...
    {
        avahi_log_error("open(\"%s\", O_RDONLY): %s", g->filename,
                        strerror(errno));
        goto finish;
    }
...
static void load_file(char* n)
{
    ...
    {
        avahi_log_error("Failed to load service group file %s, ignoring.",
                        g->filename);
...
void static_service_load(int in_chroot)
{
    ...
    for (g = groups; g; g = n)
    {
        ...
        if (stat(g->filename, &st) < 0)
        {
            ...
            if (errno == ENOENT)
                avahi_log_info(
                    "Service group file %s vanished, removing services.",
                    g->filename);
            else
                avahi_log_warn("Failed to stat() file %s, ignoring: %s",
                               g->filename, strerror(errno));

            static_service_group_free(g);
        }
        else if (st.st_mtime != g->mtime)
        {
            avahi_log_info("Service group file %s changed, reloading.",
                           g->filename);

            if (static_service_group_load(g) < 0)
            {
                avahi_log_warn(
                    "Failed to load service group file %s, removing service.",
                    g->filename);
                static_service_group_free(g);
            }
        }
    ...
    if ((globret = glob(in_chroot ? "/services/*.service"
                                  : AVAHI_SERVICE_DIR "/*.service",
                        GLOB_ERR, NULL, &globbuf)) != 0)
