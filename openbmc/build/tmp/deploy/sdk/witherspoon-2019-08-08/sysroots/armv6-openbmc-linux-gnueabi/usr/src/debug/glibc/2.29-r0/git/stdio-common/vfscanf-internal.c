...
#define ungetc(c, s) ((void)(c == WEOF || (--read_in, _IO_sputbackwc(s, c))))
#define ungetc_not_eof(c, s) ((void)(--read_in, _IO_sputbackwc(s, c)))
#define inchar()                                                               \
    (c == WEOF                                                                 \
         ? ((errno = inchar_errno), WEOF)                                      \
         : ((c = _IO_getwc_unlocked(s)),                                       \
            (void)(c != WEOF ? ++read_in : (size_t)(inchar_errno = errno)),    \
            c))
...
#else
#define ungetc(c, s)                                                           \
    ((void)((int)c == EOF || (--read_in, _IO_sputbackc(s, (unsigned char)c))))
#define ungetc_not_eof(c, s)                                                   \
    ((void)(--read_in, _IO_sputbackc(s, (unsigned char)c)))
#define inchar()                                                               \
    (c == EOF                                                                  \
         ? ((errno = inchar_errno), EOF)                                       \
         : ((c = _IO_getc_unlocked(s)),                                        \
            (void)(c != EOF ? ++read_in : (size_t)(inchar_errno = errno)), c))
#define ISSPACE(Ch) __isspace_l(Ch, loc)
#define ISDIGIT(Ch) __isdigit_l(Ch, loc)
#define ISXDIGIT(Ch) __isxdigit_l(Ch, loc)
#define TOLOWER(Ch) __tolower_l((unsigned char)(Ch), loc)
#define ORIENT                                                                 \
    if (_IO_vtable_offset(s) == 0 && _IO_fwide(s, -1) != -1)                   \
    return EOF
...
#define add_ptr_to_free(ptr)                                                   \
    ...
    {                                                                          \
        if (ptrs_to_free == NULL ||                                            \
            ptrs_to_free->count ==                                             \
                (sizeof(ptrs_to_free->ptrs) / sizeof(ptrs_to_free->ptrs[0])))  \
        {                                                                      \
...
#ifdef COMPILE_WSCANF
int __vfwscanf_internal(FILE* s, const wchar_t* format, va_list argptr,
                        unsigned int mode_flags)
#else
int __vfscanf_internal(FILE* s, const char* format, va_list argptr,
                       unsigned int mode_flags)
#endif
{
    va_list arg;
...
#define ARG(type)                                                              \
    (argpos == 0 ? va_arg(arg, type) : ({                                      \
        ...
        arg = (va_list)((char*)arg + (argpos - 1) * __va_rounded_size(void*)); \
        va_arg(arg, type);                                                     \
    }))
#else
#define ARG(type)                                                              \
    ...
    got_width:
            ...
            (fc != L_('[') && fc != L_('c') && fc != L_('C') && fc != L_('n')))
        {
            ...
            do
                ...
                if (__builtin_expect(
                        (c == EOF || inchar() == EOF) && errno == EINTR, 0))
                    input_error();
            while (ISSPACE(c));
    ...
    while (0)
            ...
            case L_('C'):
                            ...
                            wstr == (wchar_t*)*strptr + strsize)
                        {
                            ...
                            if (wstr == NULL)
                            {
                                ...
                                wstr = (wchar_t*)realloc(
                                    *strptr, (strsize + 1) * sizeof(wchar_t));
                                if (wstr == NULL)
                                {
                ...
                if (!(flags & SUPPRESS))
                {
                    ...
                    {
                        wchar_t* cp = (wchar_t*)realloc(
                            *strptr,
                            ((wstr - (wchar_t*)*strptr) * sizeof(wchar_t)));
                        if (cp != NULL)
                            *strptr = (char*)cp;
                    }
                    strptr = NULL;
                    ...
                    do
                    {
                           ...
                           store them.  */
                        {
                            ...
                            if (!(flags & SUPPRESS) && (flags & MALLOC) &&
                                *strptr + strsize - str <= MB_LEN_MAX)
                            {
                                /* We have to enlarge the buffer if the `a' or
                                   `m' flag was given.  */
                                size_t strleng = str - *strptr;
                                char* newstr;
                    ...
                    if (!(flags & SUPPRESS))
                    {
                        *wstr++ = c;
                            ...
                            wstr == (wchar_t*)*strptr + strsize)
                        {
                            /* Enlarge the buffer.  */
                            wstr = (wchar_t*)realloc(
                                *strptr, (2 * strsize) * sizeof(wchar_t));
                            if (wstr == NULL)
                            {
                                /* Can't allocate that much.  Last-ditch
                                   effort.  */
            ...
            number:
                ...
                if (base == 10 && __builtin_expect((flags & I18N) != 0, 0))
                {
...
#ifdef COMPILE_WSCANF
                    to_level =
                        _NL_CURRENT_WORD(LC_CTYPE, _NL_CTYPE_INDIGITS_WC_LEN) -
                        1;
#else
                    to_level =
                        (uint32_t)curctype
                            ->values[_NL_ITEM_INDEX(_NL_CTYPE_INDIGITS_MB_LEN)]
                            .word -
                        1;
#endif
                    ...
                    {
                        /*  Adding new level for extra digits set in locale
                         * file.  */
                        ++to_level;
                        ...
                        {
#ifdef COMPILE_WSCANF
                            wcdigits[n] = (const wchar_t*)_NL_CURRENT(
                                LC_CTYPE, _NL_CTYPE_INDIGITS0_WC + n);

                            wchar_t* wc_extended = (wchar_t*)alloca(
                                (to_level + 2) * sizeof(wchar_t));
...
#else
                            mbdigits[n] =
                                curctype->values[_NL_CTYPE_INDIGITS0_MB + n]
                                    .string;
                            ...
                            char* mb_extended;
                            mb_extended =
                                (char*)alloca(mbdigits_len + mblen + 1);

                            /*  And get the mbdigits + extra_digit string.  */
                            *(char*)__mempcpy(__mempcpy(mb_extended,
                                                        mbdigits[n],
                                                        mbdigits_len),
                    ...
                    while (c != EOF && width != 0)
                    {
                        ...
                        if (n == 10)
                        {
                            ...
                            for (level = from_level + 1; level <= to_level;
                                 ++level)
                            {
                        ...
                        else if (flags & GROUP)
                        {
                            ...
                            /* The last thousands character will be added back
                           by the char_buffer_add below.  */
                            --charbuf.current;
#endif
                        }
                        else
                    ...
                    while (c != EOF && width != 0)
                    {
                        ...
                        else if (!ISDIGIT(c) || (int)(c - L_('0')) >= base)
                        {
                            if (base == 10 && (flags & GROUP))
                            {
                                ...
                                /* The last thousands character will be added
                                   back by the char_buffer_add below.  */
                                --charbuf.current;
#endif
                            }
                            else
                ...
                if (char_buffer_size(&charbuf) == 0 ||
                    (char_buffer_size(&charbuf) == 1 &&
                     (char_buffer_start(&charbuf)[0] == L_('+') ||
                      char_buffer_start(&charbuf)[0] == L_('-'))))
                {
                    /* There was no number.  If we are supposed to read a
                   pointer we must recognize "(nil)" as well.  */
                    if (__builtin_expect(char_buffer_size(&charbuf) == 0 &&
                                             ...
                                             inchar() == L_(')'),
                                         1))
                        /* We must produce the value of a NULL pointer.  A
                    ...
                    /* The just read character is not part of the number
                     * anymore.  */
                    ungetc(c, s);
                ...
                if (need_longlong && (flags & LONGDBL))
                {
                    if (flags & NUMBER_SIGNED)
                        num.q = __strtoll_internal(char_buffer_start(&charbuf),
                                                   &tw, base, flags & GROUP);
                    else
                        num.uq =
                            __strtoull_internal(char_buffer_start(&charbuf),
                                                &tw, base, flags & GROUP);
                }
                else
                {
                    if (flags & NUMBER_SIGNED)
                        num.l = __strtol_internal(char_buffer_start(&charbuf),
                                                  &tw, base, flags & GROUP);
                    else
                        num.ul = __strtoul_internal(char_buffer_start(&charbuf),
                                                    &tw, base, flags & GROUP);
                }
            ...
            case L_('A'):
                ...
                if (TOLOWER(c) == L_('n'))
                {
                    /* Maybe "nan".  */
                    char_buffer_add(&charbuf, c);
                    if (__builtin_expect(width == 0 || inchar() == EOF ||
                                             TOLOWER(c) != L_('a'),
                                         0))
                        conv_error();
                    if (width > 0)
                    ...
                    else
                    {
                        ...
                        else
                        {
                           ...
                           we can compare against it.  */
                            ...
                            {
                                /* The last read character is not part of the
                                   number anymore.  */
                                ungetc(c, s);
                                break;
                            }
                    ...
                    (map = __wctrans("to_inpunct")) != NULL)
                {
                   ...
                   digits.  Note, we test GOT_DOT above.  */
                    ...
                    size_t mblen =
                        __wcrtomb(mbdigits[11], wcdigits[11], &state);
                    if (mblen != (size_t)-1)
                    {
                        mbdigits[11][mblen] = '\0';
                        match_so_far |= (char_buffer_size(&charbuf) ==
                                             strlen(decimal) + got_sign &&
...
#endif
                    {
                        ...
                        for (int n = 0; n < 11; ++n)
                        {
                            ...
                            size_t mblen =
                                __wcrtomb(mbdigits[n], wcdigits[n], &state);
                            if (mblen == (size_t)-1)
                            {
                                if (n == 10)
                                {
                        ...
                        while (1)
                        {
                            ...
                            if (got_e && charbuf.current[-1] == exp_char &&
                                (c == L_('-') || c == L_('+')))
                                char_buffer_add(&charbuf, c);
                            else if (char_buffer_size(&charbuf) > got_sign &&
                                     !got_e && (CHAR_T)TOLOWER(c) == exp_char)
                            {
                                char_buffer_add(&charbuf, exp_char);
                                got_e = got_dot = 1;
                            }
                            ...
                            {
                                /* Check against localized digits, decimal
                                ...
                                for (n = 0; n < 12; ++n)
                                {
                                    ...
                                    if (c == wcdigits[n])
                                    {
                                        ...
                                        else if (n == 11 && !got_dot)
                                        {
                                            ...
                                            /* The last read character is not
                                               part of the number anymore.  */
                                            n = 12;
                ...
                if (__glibc_unlikely(
                        char_buffer_size(&charbuf) == got_sign ||
                        ((flags & HEXA_FLOAT) &&
                         (char_buffer_size(&charbuf) == 2 + got_sign))))
                    conv_error();
            ...
            scan_float:
                ...
                {
                    double d = __strtod_internal(char_buffer_start(&charbuf),
                                                 &tw, flags & GROUP);
                    if (!(flags & SUPPRESS) &&
                        tw != char_buffer_start(&charbuf))
                        *ARG(double*) = d;
                ...
                {
                    float d = __strtof_internal(char_buffer_start(&charbuf),
                                                &tw, flags & GROUP);
                    if (!(flags & SUPPRESS) &&
                        tw != char_buffer_start(&charbuf))
                        *ARG(float*) = d;
            ...
            case L_('['): /* Character class.  */
                ...
                if (!scratch_buffer_set_array_size(&charbuf.scratch,
                                                   UCHAR_MAX + 1, 1))
                {
                    done = EOF;
                    goto errout;
                ...
                while ((fc = *f++) != '\0' && fc != ']')
                        ...
                        (unsigned char)f[-2] <= (unsigned char)*f)
                    {
                        ...
                        for (fc = (unsigned char)f[-2]; fc < (unsigned char)*f;
                             ++fc)
                            ((char*)charbuf.scratch.data)[fc] = 1;
                    }
                    else
