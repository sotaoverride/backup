...
static int build_subject(X509_REQ* req, const char* subj, unsigned long chtype,
                         int multirdn);
static int prompt_info(X509_REQ* req, STACK_OF(CONF_VALUE) * dn_sk,
                       const char* dn_sect, STACK_OF(CONF_VALUE) * attr_sk,
                       const char* attr_sect, int attribs,
...
static int build_data(char* text, const char* def, char* value, int n_min,
                      int n_max, char* buf, const int buf_size,
                      const char* desc1, const char* desc2);
static int req_check_len(int len, int n_min, int n_max);
static int check_end(const char* str, const char* end);
static int join(char buf[], size_t buf_size, const char* name, const char* tail,
                const char* desc);
static EVP_PKEY_CTX* set_keygen_ctx(const char* gstr, int* pkey_type,
...
static int duplicated(LHASH_OF(OPENSSL_STRING) * addexts, char* kv)
{
    ...
    *p = '\0';
    ...
    if ((p = (char*)lh_OPENSSL_STRING_insert(addexts, (OPENSSL_STRING*)kv)) !=
            NULL ||
        lh_OPENSSL_STRING_error(addexts))
    {
...
int req_main(int argc, char** argv)
{
  ...
  private
    ...
    if (newreq && (pkey == NULL))
    {
        ...
        if (keyalg != NULL)
        {
            genctx = set_keygen_ctx(keyalg, &pkey_type, &newkey, &keyalgstr,
                                    gen_eng);
            if (genctx == NULL)
                goto end;
        }
        ...
        {
            genctx =
                set_keygen_ctx(NULL, &pkey_type, &newkey, &keyalgstr, gen_eng);
            if (!genctx)
                goto end;
        }
    ...
    loop:
        assert(private);
        if (!PEM_write_bio_PrivateKey(out, pkey, cipher, NULL, 0, NULL,
                                      passout))
        {
    ...
    if (newreq || x509)
    {
        ...
        if (x509)
        {
            ...
            /* Add extensions */
            if (extensions != NULL &&
                !X509V3_EXT_add_nconf(req_conf, &ext_ctx, extensions, x509ss))
            {
                BIO_printf(bio_err, "Error Loading extension section %s\n",
                           extensions);
                goto end;
            ...
            if (addext_conf != NULL &&
                !X509V3_EXT_add_nconf(addext_conf, &ext_ctx, "default", x509ss))
            {
                BIO_printf(bio_err, "Error Loading command line extensions\n");
                goto end;
            }
            ...
            if (precert)
            {
                if (X509_add1_ext_i2d(x509ss, NID_ct_precert_poison, NULL, 1,
                                      0) != 1)
                {
    ...
    if (subj && !x509)
    {
        if (verbose)
        {
            ...
            print_name(bio_err, "old subject=", X509_REQ_get_subject_name(req),
                       get_nameopt());
        }
        ...
        if (verbose)
        {
            print_name(bio_err, "new subject=", X509_REQ_get_subject_name(req),
                       get_nameopt());
        }
    }
    ...
    out = bio_open_default(
        outfile,
        keyout != NULL && outfile != NULL && strcmp(keyout, outfile) == 0 ? 'a'
                                                                          : 'w',
        outformat);
    if (out == NULL)
        goto end;
...
static int prompt_info(X509_REQ* req, STACK_OF(CONF_VALUE) * dn_sk,
                       const char* dn_sect, STACK_OF(CONF_VALUE) * attr_sk,
                       const char* attr_sect, int attribs, unsigned long chtype)
{
    int i;
    char *p, *q;
    char buf[100];
    ...
    start:
        for (;;)
        {
            ...
            if (!add_DN_object(subj, v->value, def, value, nid, n_min, n_max,
                               chtype, mval))
                return 0;
        }
        ...
        start2:
            for (;;)
            {
                ...
                if (!add_attribute_object(req, v->value, def, value, nid, n_min,
                                          n_max, chtype))
                    return 0;
            }
        }
                         ...
                         int mval)
{
    ...
    if (!X509_NAME_add_entry_by_NID(n, nid, chtype, (unsigned char*)buf, -1, -1,
                                    mval))
        ret = 0;
