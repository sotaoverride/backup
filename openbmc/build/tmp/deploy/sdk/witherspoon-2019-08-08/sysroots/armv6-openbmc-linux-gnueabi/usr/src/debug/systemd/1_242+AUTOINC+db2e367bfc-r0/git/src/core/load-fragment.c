...
DEFINE_CONFIG_PARSE_ENUM(config_parse_device_policy, cgroup_device_policy,
                         CGroupDevicePolicy, "Failed to parse device policy");
DEFINE_CONFIG_PARSE_ENUM(config_parse_exec_keyring_mode, exec_keyring_mode,
                         ExecKeyringMode, "Failed to parse keyring mode");
DEFINE_CONFIG_PARSE_ENUM(config_parse_exec_utmp_mode, exec_utmp_mode,
                         ExecUtmpMode, "Failed to parse utmp mode");
DEFINE_CONFIG_PARSE_ENUM(config_parse_job_mode, job_mode, JobMode,
                         "Failed to parse job mode");
DEFINE_CONFIG_PARSE_ENUM(config_parse_kill_mode, kill_mode, KillMode,
...
DEFINE_CONFIG_PARSE_ENUM(config_parse_protect_system, protect_system,
                         ProtectSystem, "Failed to parse protect system value");
DEFINE_CONFIG_PARSE_ENUM(config_parse_runtime_preserve_mode, exec_preserve_mode,
                         ExecPreserveMode,
...
DEFINE_CONFIG_PARSE_ENUM(config_parse_socket_bind,
...
DEFINE_CONFIG_PARSE_ENUM_WITH_DEFAULT(config_parse_ip_tos, ip_tos, int, -1,
                                      "Failed to parse IP TOS value");
DEFINE_CONFIG_PARSE_PTR(config_parse_blockio_weight, cg_blkio_weight_parse,
                        uint64_t, "Invalid block IO weight");
DEFINE_CONFIG_PARSE_PTR(config_parse_cg_weight, cg_weight_parse, uint64_t,
                        "Invalid weight");
DEFINE_CONFIG_PARSE_PTR(config_parse_cpu_shares, cg_cpu_shares_parse, uint64_t,
                        "Invalid CPU shares");
DEFINE_CONFIG_PARSE_PTR(config_parse_exec_mount_flags,
                        mount_propagation_flags_from_string, unsigned long,
                        "Failed to parse mount flag");

int config_parse_unit_deps(const char* unit, const char* filename,
                           unsigned line, const char* section,
                           unsigned section_line, const char* lvalue, int ltype,
...
int config_parse_obsolete_unit_deps(const char* unit, const char* filename,
                                    unsigned line, const char* section,
                                    ...
                                    void* userdata)
{
    ...
    log_syntax(unit, LOG_WARNING, filename, line, 0,
               "Unit dependency type %s= is obsolete, replacing by %s=, please "
               "update your unit file",
               lvalue, unit_dependency_to_string(ltype));

    return config_parse_unit_deps(unit, filename, line, section, section_line,
                                  lvalue, ltype, rvalue, data, userdata);
}

int config_parse_unit_string_printf(const char* unit, const char* filename,
                                    unsigned line, const char* section,
                                    unsigned section_line, const char* lvalue,
                                    int ltype, const char* rvalue, void* data,
                                    void* userdata)
{
    ...
    return config_parse_string(unit, filename, line, section, section_line,
                               lvalue, ltype, k, data, userdata);
}

int config_parse_unit_strv_printf(const char* unit, const char* filename,
                                  unsigned line, const char* section,
                                  unsigned section_line, const char* lvalue,
                                  int ltype, const char* rvalue, void* data,
                                  void* userdata)
...
int config_parse_unit_path_printf(const char* unit, const char* filename,
                                  unsigned line, const char* section,
                                  unsigned section_line, const char* lvalue,
                                  int ltype, const char* rvalue, void* data,
                                  void* userdata)
{
    ...
    {
        log_syntax(unit, LOG_ERR, filename, line, 0, "%s value too long%s.",
    ...
    {
        log_syntax(unit, LOG_ERR, filename, line, r,
                   "Failed to resolve unit specifiers in '%s'%s: %m", rvalue,
                   fatal ? "" : ", ignoring");
...
int config_parse_unit_path_strv_printf(const char* unit, const char* filename,
                                       unsigned line, const char* section,
                                       ...
                                       void* userdata)
{
    ...
    for (p = rvalue;;)
    {
        ...
        r = path_simplify_and_warn(k, PATH_CHECK_ABSOLUTE, unit, filename, line,
                                   lvalue);
        if (r < 0)
            return 0;
...
static int patch_var_run(const char* unit, const char* filename, unsigned line,
                         const char* lvalue, char** path)
{
    ...
    log_syntax(unit, LOG_NOTICE, filename, line, 0,
               "%s= references a path below legacy directory /var/run/, "
               "updating %s → %s; "
...
int config_parse_socket_listen(const char* unit, const char* filename,
                               unsigned line, const char* section,
                               unsigned section_line, const char* lvalue,
                               int ltype, const char* rvalue, void* data,
                               void* userdata)
{
    ...
    else if (streq(lvalue, "ListenNetlink"))
    {
        ...
        {
            log_syntax(unit, LOG_ERR, filename, line, r,
                       "Failed to parse address value in '%s', ignoring: %m",
                       k);
            return 0;
        }
...
int config_parse_exec_nice(const char* unit, const char* filename,
                           unsigned line, const char* section,
                           unsigned section_line, const char* lvalue, int ltype,
                           const char* rvalue, void* data, void* userdata)
{
    ...
    if (r < 0)
    {
        if (r == -ERANGE)
            ...
            log_syntax(unit, LOG_ERR, filename, line, r,
                       "Failed to parse nice priority '%s', ignoring: %m",
                       rvalue);
        return 0;
    }
...
int config_parse_exec_oom_score_adjust(const char* unit, const char* filename,
                                       unsigned line, const char* section,
                                       unsigned section_line,
                                       const char* lvalue, int ltype,
                                       const char* rvalue, void* data,
...
int config_parse_exec(const char* unit, const char* filename, unsigned line,
                      ...
                      void* data, void* userdata)
{
    ...
    do
    {
        ...
        r = extract_first_word_and_warn(&p, &firstword, NULL,
                                        EXTRACT_QUOTES | EXTRACT_CUNESCAPE,
                                        unit, filename, line, rvalue);
        if (r <= 0)
            return 0;
        ...
        for (;;)
        {
            /* We accept an absolute path as first argument.  If it's prefixed
             * with - and the path doesn't exist, we ignore it instead of
             * erroring out; if it's prefixed with @, we allow overriding of
             * argv[0]; if it's prefixed with :, we will not do environment
             * variable substitution; if it's prefixed with +, it will be run
             * with full privileges and no sandboxing; if it's prefixed with '!'
             * we apply sandboxing, but do not change user/group credentials; if
             * it's prefixed with '!!', then we apply user/group credentials if
             * the kernel supports ambient capabilities -- if it doesn't we
             * don't apply the credentials themselves, but do apply most other
             * sandboxing, with some special exceptions for changing UID.
             *
             * The idea is that '!!' may be used to write services that can take
             * benefit of systemd's UID/GID dropping if the kernel supports
             * ambient creds, but provide an automatic fallback to privilege
             * dropping within the daemon if the kernel does not offer that. */
        ...
        {
            log_syntax(unit, LOG_ERR, filename, line, r,
                       "Failed to resolve unit specifiers in '%s'%s: %m", f,
                       ignore ? ", ignoring" : "");
        ...
        if (!path_is_absolute(path))
        {
            ...
            {
                log_syntax(unit, LOG_ERR, filename, line, 0,
                           "Neither a valid executable name nor an absolute "
                           "path%s: %s",
                           ignore ? ", ignoring" : "", path);
                return ignore ? 0 : -ENOEXEC;
            }
            ...
            {
                log_syntax(unit, LOG_ERR, filename, line, 0,
                           "Executable \"%s\" not found in path \"%s\"%s", path,
                           DEFAULT_PATH, ignore ? ", ignoring" : "");
                return ignore ? 0 : -ENOEXEC;
            }
        }
        ...
        while (!isempty(p))
        {
            ...
            /* Check for \; explicitly, to not confuse it with \\; or "\;" or
             * "\\;" etc.
             * extract_first_word() would return the same for all of those.  */
            if (p[0] == '\\' && p[1] == ';' &&
                (!p[2] || strchr(WHITESPACE, p[2])))
            {
                char* w;
            ...
            r = extract_first_word_and_warn(&p, &word, NULL,
                                            EXTRACT_QUOTES | EXTRACT_CUNESCAPE,
                                            unit, filename, line, rvalue);
            if (r == 0)
                break;
...
int config_parse_socket_bindtodevice(const char* unit, const char* filename,
                                     unsigned line, const char* section,
                                     unsigned section_line, const char* lvalue,
                                     int ltype, const char* rvalue, void* data,
                                     void* userdata)
{
    ...
    {
        log_syntax(unit, LOG_ERR, filename, line, 0,
                   "Invalid interface name, ignoring: %s", rvalue);
        return 0;
    }
...
int config_parse_exec_input(const char* unit, const char* filename,
                            unsigned line, const char* section,
                            unsigned section_line, const char* lvalue,
                            int ltype, const char* rvalue, void* data,
                            void* userdata)
{
    ...
    if (n)
    {
        ...
        if (r < 0)
            return log_syntax(unit, LOG_ERR, filename, line, r,
                              "Failed to resolve unit specifiers in '%s': %m",
                              n);
        ...
        {
            log_syntax(unit, LOG_ERR, filename, line, 0,
                       "Invalid file descriptor name: %s", resolved);
            return -ENOEXEC;
        }
    ...
    else if ((n = startswith(rvalue, "file:")))
    {
        ...
        r = path_simplify_and_warn(resolved,
                                   PATH_CHECK_ABSOLUTE | PATH_CHECK_FATAL, unit,
...
int config_parse_exec_input_text(const char* unit, const char* filename,
                                 unsigned line, const char* section,
                                 unsigned section_line, const char* lvalue,
                                 int ltype, const char* rvalue, void* data,
                                 void* userdata)
...
int config_parse_exec_input_data(const char* unit, const char* filename,
                                 unsigned line, const char* section,
                                 unsigned section_line, const char* lvalue,
                                 int ltype, const char* rvalue, void* data,
                                 void* userdata)
...
int config_parse_exec_output(const char* unit, const char* filename,
                             unsigned line, const char* section,
                             unsigned section_line, const char* lvalue,
                             int ltype, const char* rvalue, void* data,
                             void* userdata)
{
    ...
    else
    {
        ...
        {
            log_syntax(unit, LOG_ERR, filename, line, 0,
                       "Failed to parse output specifier, ignoring: %s",
                       rvalue);
            return 0;
        }
    }
...
int config_parse_exec_io_class(const char* unit, const char* filename,
                               unsigned line, const char* section,
                               unsigned section_line, const char* lvalue,
                               int ltype, const char* rvalue, void* data,
                               void* userdata)
...
int config_parse_exec_io_priority(const char* unit, const char* filename,
                                  unsigned line, const char* section,
                                  unsigned section_line, const char* lvalue,
                                  int ltype, const char* rvalue, void* data,
                                  void* userdata)
...
int config_parse_exec_cpu_sched_policy(const char* unit, const char* filename,
                                       unsigned line, const char* section,
                                       unsigned section_line,
                                       const char* lvalue, int ltype,
                                       const char* rvalue, void* data,
                                       void* userdata)
{
    ...
    {
        log_syntax(unit, LOG_ERR, filename, line, 0,
                   "Failed to parse CPU scheduling policy, ignoring: %s",
                   rvalue);
        return 0;
    }
    ...
    c->cpu_sched_priority =
        CLAMP(c->cpu_sched_priority, sched_get_priority_min(x),
                                     ...
                                     void* userdata)
{
    ...
    {
        log_syntax(unit, LOG_ERR, filename, line, r,
                   "Failed to parse CPU scheduling priority, ignoring: %s",
                   rvalue);
        return 0;
    }
    ...
    {
        log_syntax(unit, LOG_ERR, filename, line, 0,
                   "CPU scheduling priority is out of range, ignoring: %s",
                   rvalue);
        return 0;
    }
                                   ...
                                   void* userdata)
{
    ...
    ncpus =
        parse_cpu_set_and_warn(rvalue, &cpuset, unit, filename, line, lvalue);
    if (ncpus < 0)
        return ncpus;
...
int config_parse_capability_set(const char* unit, const char* filename,
                                unsigned line, const char* section,
                                unsigned section_line, const char* lvalue,
                                int ltype, const char* rvalue, void* data,
                                void* userdata)
...
int config_parse_exec_selinux_context(const char* unit, const char* filename,
                                      unsigned line, const char* section,
                                      unsigned section_line, const char* lvalue,
                                      int ltype, const char* rvalue, void* data,
                                      void* userdata)
...
int config_parse_exec_apparmor_profile(const char* unit, const char* filename,
                                       unsigned line, const char* section,
                                       unsigned section_line,
                                       const char* lvalue, int ltype,
                                       const char* rvalue, void* data,
...
int config_parse_timer(const char* unit, const char* filename, unsigned line,
                       const char* section, unsigned section_line,
                       const char* lvalue, int ltype, const char* rvalue,
                       void* data, void* userdata)
{
...
int config_parse_trigger_unit(const char* unit, const char* filename,
                              unsigned line, const char* section,
                              unsigned section_line, const char* lvalue,
                              int ltype, const char* rvalue, void* data,
                              void* userdata)
{
    ...
    r = unit_add_two_dependencies_by_name(u, UNIT_BEFORE, UNIT_TRIGGERS, p,
...
int config_parse_path_spec(const char* unit, const char* filename,
                           unsigned line, const char* section,
                           unsigned section_line, const char* lvalue, int ltype,
                           const char* rvalue, void* data, void* userdata)
{
...
int config_parse_socket_service(const char* unit, const char* filename,
                                unsigned line, const char* section,
                                unsigned section_line, const char* lvalue,
                                int ltype, const char* rvalue, void* data,
                                void* userdata)
{
    ...
    {
        log_syntax(unit, LOG_ERR, filename, line, 0,
                   "Unit must be of type service: %s", rvalue);
        return -ENOEXEC;
    }
...
int config_parse_fdname(const char* unit, const char* filename, unsigned line,
                        const char* section, unsigned section_line,
                        const char* lvalue, int ltype, const char* rvalue,
                        void* data, void* userdata)
{
    ...
    {
        log_syntax(unit, LOG_ERR, filename, line, 0,
                   "Invalid file descriptor name, ignoring: %s", p);
        return 0;
    }
...
int config_parse_service_sockets(const char* unit, const char* filename,
                                 unsigned line, const char* section,
                                 unsigned section_line, const char* lvalue,
                                 int ltype, const char* rvalue, void* data,
                                 void* userdata)
{
    ...
    for (;;)
    {
        ...
        r = unit_add_two_dependencies_by_name(UNIT(s), UNIT_WANTS, UNIT_AFTER,
                                              k, true, UNIT_DEPENDENCY_FILE);
        if (r < 0)
            log_syntax(unit, LOG_ERR, filename, line, r,
                       "Failed to add dependency on %s, ignoring: %m", k);

        r = unit_add_dependency_by_name(UNIT(s), UNIT_TRIGGERED_BY, k, true,
...
int config_parse_bus_name(const char* unit, const char* filename, unsigned line,
                          const char* section, unsigned section_line,
                          const char* lvalue, int ltype, const char* rvalue,
                          void* data, void* userdata)
{
...
int config_parse_service_timeout(const char* unit, const char* filename,
                                 unsigned line, const char* section,
                                 unsigned section_line, const char* lvalue,
                                 int ltype, const char* rvalue, void* data,
                                 void* userdata)
{
    ...
    /* Traditionally, these options accepted 0 to disable the timeouts. However,
     * a timeout of 0 suggests it happens immediately, hence fix this to become
     * USEC_INFINITY instead. This is in-line with how we internally handle
     * all other timeouts. */
...
int config_parse_sec_fix_0(const char* unit, const char* filename,
                           unsigned line, const char* section,
                           unsigned section_line, const char* lvalue, int ltype,
                           const char* rvalue, void* data, void* userdata)
{
    ...
    /* This is pretty much like config_parse_sec(), except that this treats a
     * time of 0 as infinity, for compatibility with older versions of systemd
     * where 0 instead of infinity was used as indicator to turn off a
     * timeout. */
...
int config_parse_user_group(const char* unit, const char* filename,
                            unsigned line, const char* section,
                            unsigned section_line, const char* lvalue,
                            int ltype, const char* rvalue, void* data,
                            void* userdata)
{
    ...
    {
        log_syntax(unit, LOG_ERR, filename, line, 0,
                   "Invalid user/group name or numeric ID: %s", k);
        return -ENOEXEC;
    }
                                 ...
                                 void* userdata)
{
    ...
    for (;;)
    {
        ...
        if (r < 0)
        {
            log_syntax(unit, LOG_ERR, filename, line, r, "Invalid syntax: %s",
                       rvalue);
            return -ENOEXEC;
        }
...
int config_parse_working_directory(const char* unit, const char* filename,
                                   unsigned line, const char* section,
                                   unsigned section_line, const char* lvalue,
                                   int ltype, const char* rvalue, void* data,
                                   void* userdata)
{
    ...
    else
    {
        ...
        {
            log_syntax(unit, LOG_ERR, filename, line, r,
                       "Failed to resolve unit specifiers in working directory "
                       "path '%s'%s: %m",
                       rvalue, missing_ok ? ", ignoring" : "");
            return missing_ok ? 0 : -ENOEXEC;
        }

        r = path_simplify_and_warn(
            k, PATH_CHECK_ABSOLUTE | (missing_ok ? 0 : PATH_CHECK_FATAL), unit,
            filename, line, lvalue);
        if (r < 0)
            return missing_ok ? 0 : -ENOEXEC;
...
int config_parse_unit_env_file(const char* unit, const char* filename,
                               unsigned line, const char* section,
                               unsigned section_line, const char* lvalue,
                               int ltype, const char* rvalue, void* data,
                               void* userdata)
{
    ...
    r = path_simplify_and_warn(n[0] == '-' ? n + 1 : n, PATH_CHECK_ABSOLUTE,
...
int config_parse_environ(const char* unit, const char* filename, unsigned line,
                         const char* section, unsigned section_line,
                         const char* lvalue, int ltype, const char* rvalue,
                         void* data, void* userdata)
{
    ...
    for (p = rvalue;;)
    {
        ...
        r = extract_first_word(&p, &word, NULL,
                               EXTRACT_CUNESCAPE | EXTRACT_QUOTES);
        if (r == 0)
            return 0;
        if (r == -ENOMEM)
            return log_oom();
...
int config_parse_pass_environ(const char* unit, const char* filename,
                              unsigned line, const char* section,
                              unsigned section_line, const char* lvalue,
                              int ltype, const char* rvalue, void* data,
                              void* userdata)
{
    ...
    for (;;)
    {
        ...
        {
            log_syntax(unit, LOG_ERR, filename, line, 0,
                       "Invalid environment name for %s, ignoring: %s", lvalue,
                       k);
            continue;
        }
...
int config_parse_unset_environ(const char* unit, const char* filename,
                               unsigned line, const char* section,
                               unsigned section_line, const char* lvalue,
                               int ltype, const char* rvalue, void* data,
                               void* userdata)
{
    ...
    for (;;)
    {
        ...
        {
            log_syntax(
                unit, LOG_ERR, filename, line, 0,
                "Invalid environment name or assignment %s, ignoring: %s",
                lvalue, k);
            continue;
        }
...
int config_parse_log_extra_fields(const char* unit, const char* filename,
                                  unsigned line, const char* section,
                                  unsigned section_line, const char* lvalue,
                                  int ltype, const char* rvalue, void* data,
                                  void* userdata)
{
    ...
    for (;;)
    {
        ...
        {
            log_syntax(unit, LOG_ERR, filename, line, 0,
                       "Log field lacks '=' character, ignoring: %s", k);
            continue;
        }
        ...
        {
            log_syntax(unit, LOG_ERR, filename, line, 0,
                       "Log field name is invalid, ignoring: %s", k);
            continue;
        }
                                     ...
                                     void* userdata)
{
    ...
    r = path_simplify_and_warn(p, PATH_CHECK_ABSOLUTE, unit, filename, line,
                               lvalue);
    if (r < 0)
        return 0;
...
int config_parse_unit_condition_string(const char* unit, const char* filename,
                                       unsigned line, const char* section,
                                       unsigned section_line,
                                       const char* lvalue, int ltype,
                                       const char* rvalue, void* data,
                                     ...
                                     void* userdata)
{
    ...
    log_syntax(unit, LOG_WARNING, filename, line, 0,
               "%s= is deprecated, please do not use.", lvalue);
    ...
    void* data, void* userdata)
{
    ...
    for (;;)
    {
        ...
        r = path_simplify_and_warn(resolved, PATH_CHECK_ABSOLUTE, unit,
                                   filename, line, lvalue);
        if (r < 0)
            continue;
        ...
        {
            log_syntax(unit, LOG_ERR, filename, line, r,
                       "Failed to add required mount '%s', ignoring: %m",
                       resolved);
            continue;
        }
    }
...
int config_parse_documentation(const char* unit, const char* filename,
                               unsigned line, const char* section,
                               unsigned section_line, const char* lvalue,
                               int ltype, const char* rvalue, void* data,
                               void* userdata)
{
    ...
    r = config_parse_unit_strv_printf(unit, filename, line, section,
                                      section_line, lvalue, ltype, rvalue, data,
                                      userdata);
    if (r < 0)
        return r;
...
#if HAVE_SECCOMP
int config_parse_syscall_filter(const char* unit, const char* filename,
                                unsigned line, const char* section,
                                unsigned section_line, const char* lvalue,
                                int ltype, const char* rvalue, void* data,
                                void* userdata)
{
    ...
    if (!c->syscall_filter)
    {
        ...
        else
        {
            ...
            r = seccomp_parse_syscall_filter("@default", -1, c->syscall_filter,
...
int config_parse_syscall_archs(const char* unit, const char* filename,
                               unsigned line, const char* section,
                               unsigned section_line, const char* lvalue,
                               int ltype, const char* rvalue, void* data,
                               void* userdata)
...
int config_parse_syscall_errno(const char* unit, const char* filename,
                               unsigned line, const char* section,
                               unsigned section_line, const char* lvalue,
                               int ltype, const char* rvalue, void* data,
                               void* userdata)
...
int config_parse_address_families(const char* unit, const char* filename,
                                  unsigned line, const char* section,
                                  unsigned section_line, const char* lvalue,
                                  int ltype, const char* rvalue, void* data,
                                  void* userdata)
...
int config_parse_restrict_namespaces(const char* unit, const char* filename,
                                     unsigned line, const char* section,
                                     unsigned section_line, const char* lvalue,
                                     int ltype, const char* rvalue, void* data,
                                     void* userdata)
{
    ...
    {
        log_syntax(unit, LOG_ERR, filename, line, r,
                   "Failed to parse namespace type string, ignoring: %s",
                   rvalue);
        return 0;
    }
    ...
    if (c->restrict_namespaces == NAMESPACE_FLAGS_INITIAL)
        ...
        c->restrict_namespaces =
            invert ? (~flags) & NAMESPACE_FLAGS_ALL : flags;
    else
...
int config_parse_unit_slice(const char* unit, const char* filename,
                            unsigned line, const char* section,
                            unsigned section_line, const char* lvalue,
                            int ltype, const char* rvalue, void* data,
                            void* userdata)
{
    ...
    {
        log_syntax(unit, LOG_ERR, filename, line, r,
                   "Failed to assign slice %s to unit %s, ignoring: %m",
                   slice->id, u->id);
        return 0;
    }
...
int config_parse_cpu_quota(const char* unit, const char* filename,
                           unsigned line, const char* section,
                           unsigned section_line, const char* lvalue, int ltype,
                           const char* rvalue, void* data, void* userdata)
{
...
int config_parse_memory_limit(const char* unit, const char* filename,
                              unsigned line, const char* section,
                              unsigned section_line, const char* lvalue,
                              int ltype, const char* rvalue, void* data,
                              void* userdata)
{
    ...
    if (!isempty(rvalue) && !streq(rvalue, "infinity"))
    {
        ...
        {
            log_syntax(unit, LOG_ERR, filename, line, 0,
                       "Memory limit '%s' out of range, ignoring.", rvalue);
            return 0;
        }
    }
...
int config_parse_tasks_max(const char* unit, const char* filename,
                           unsigned line, const char* section,
                           unsigned section_line, const char* lvalue, int ltype,
                           const char* rvalue, void* data, void* userdata)
{
    ...
    if (r < 0)
    {
        ...
        {
            log_syntax(unit, LOG_ERR, filename, line, r,
                       "Invalid maximum tasks value '%s', ignoring: %m",
                       rvalue);
            return 0;
        }
    }
...
int config_parse_delegate(const char* unit, const char* filename, unsigned line,
                          const char* section, unsigned section_line,
                          const char* lvalue, int ltype, const char* rvalue,
                          void* data, void* userdata)
{
    ...
    /* We either accept a boolean value, which may be used to turn on delegation
     * for all controllers, or turn it off for all. Or it takes a list of
     * controller names, in which case we add the specified controllers to the
     * mask to delegate. */
                              ...
                              void* userdata)
{
    ...
    if (!STARTSWITH_SET(resolved, "block-", "char-"))
    {
        ...
        {
            log_syntax(unit, LOG_ERR, filename, line, 0,
                       "Invalid device node path '%s', ignoring.", resolved);
            return 0;
        }
    }
...
int config_parse_io_device_weight(const char* unit, const char* filename,
                                  unsigned line, const char* section,
                                  unsigned section_line, const char* lvalue,
                                  int ltype, const char* rvalue, void* data,
                                  void* userdata)
...
int config_parse_io_device_latency(const char* unit, const char* filename,
                                   unsigned line, const char* section,
                                   unsigned section_line, const char* lvalue,
                                   int ltype, const char* rvalue, void* data,
                                   void* userdata)
...
int config_parse_io_limit(const char* unit, const char* filename, unsigned line,
                          const char* section, unsigned section_line,
                          const char* lvalue, int ltype, const char* rvalue,
                          void* data, void* userdata)
{
...
int config_parse_blockio_device_weight(const char* unit, const char* filename,
                                       unsigned line, const char* section,
                                       unsigned section_line,
                                       const char* lvalue, int ltype,
                                       const char* rvalue, void* data,
                                       void* userdata)
{
    ...
    if (isempty(rvalue))
    {
        while (c->blockio_device_weights)
            cgroup_context_free_blockio_device_weight(
                c, c->blockio_device_weights);
...
int config_parse_job_mode_isolate(const char* unit, const char* filename,
                                  unsigned line, const char* section,
                                  unsigned section_line, const char* lvalue,
                                  int ltype, const char* rvalue, void* data,
                                  void* userdata)
{
    ...
    log_notice("%s is deprecated. Please use OnFailureJobMode= instead",
               lvalue);
...
int config_parse_exec_directories(const char* unit, const char* filename,
                                  unsigned line, const char* section,
                                  unsigned section_line, const char* lvalue,
                                  int ltype, const char* rvalue, void* data,
                                  void* userdata)
{
    ...
    for (p = rvalue;;)
    {
        ...
        r = path_simplify_and_warn(k, PATH_CHECK_RELATIVE, unit, filename, line,
                                   lvalue);
        if (r < 0)
            continue;
        ...
        {
            log_syntax(unit, LOG_ERR, filename, line, 0,
                       "%s= path can't be 'private', ignoring assignment: %s",
                       lvalue, word);
            continue;
        }
...
int config_parse_set_status(const char* unit, const char* filename,
                            unsigned line, const char* section,
                            unsigned section_line, const char* lvalue,
                            int ltype, const char* rvalue, void* data,
                            void* userdata)
                                     ...
                                     void* userdata)
{
    ...
    for (;;)
    {
        ...
        joined = strjoin(ignore_enoent ? "-" : "", shall_prefix ? "+" : "",
                         resolved);
...
int config_parse_temporary_filesystems(const char* unit, const char* filename,
                                       unsigned line, const char* section,
                                       unsigned section_line,
                                       const char* lvalue, int ltype,
                                       const char* rvalue, void* data,
                                       void* userdata)
{
    ...
    for (;;)
    {
        ...
        {
            log_syntax(unit, LOG_ERR, filename, line, 0,
                       "Invalid syntax, ignoring: %s", word);
            continue;
        }
        ...
        r = temporary_filesystem_add(&c->temporary_filesystems,
                                     &c->n_temporary_filesystems, resolved, w);
        if (r < 0)
            return log_oom();
    }
}
                            ...
                            void* userdata)
{
    ...
    for (;;)
    {
        ...
        r = extract_first_word(&p, &source, ":" WHITESPACE,
                               EXTRACT_QUOTES |
                                   EXTRACT_DONT_COALESCE_SEPARATORS);
        if (r == 0)
            break;
        ...
        r = path_simplify_and_warn(s, PATH_CHECK_ABSOLUTE, unit, filename, line,
                                   lvalue);
        if (r < 0)
            continue;
        ...
        if (p && p[-1] == ':')
        {
            r = extract_first_word(&p, &destination, ":" WHITESPACE,
                                   EXTRACT_QUOTES |
                                       EXTRACT_DONT_COALESCE_SEPARATORS);
            if (r == -ENOMEM)
                return log_oom();
            ...
            {
                log_syntax(unit, LOG_ERR, filename, line, 0,
                           "Missing argument after ':', ignoring: %s", s);
                continue;
            }
            ...
            if (r < 0)
            {
                log_syntax(
                    unit, LOG_ERR, filename, line, r,
                    "Failed to resolved specifiers in \"%s\", ignoring: %m",
                    destination);
                continue;
            }

            r = path_simplify_and_warn(dresolved, PATH_CHECK_ABSOLUTE, unit,
                                       filename, line, lvalue);
            if (r < 0)
                continue;
            ...
            if (p && p[-1] == ':')
            {
                ...
                {
                    log_syntax(unit, LOG_ERR, filename, line, 0,
                               "Invalid option string, ignoring setting: %s",
                               options);
                    continue;
                }
            }
...
int config_parse_job_timeout_sec(const char* unit, const char* filename,
                                 unsigned line, const char* section,
                                 unsigned section_line, const char* lvalue,
                                 int ltype, const char* rvalue, void* data,
                                 void* userdata)
{
    ...
    {
        log_syntax(unit, LOG_ERR, filename, line, r,
                   "Failed to parse JobTimeoutSec= parameter, ignoring: %s",
                   rvalue);
        return 0;
    }

    /* If the user explicitly changed JobTimeoutSec= also change
     * JobRunningTimeoutSec=, for compatibility with old versions. If
     * JobRunningTimeoutSec= was explicitly set, avoid this however as whatever
     * the user picked should count. */
...
int config_parse_job_running_timeout_sec(const char* unit, const char* filename,
                                         unsigned line, const char* section,
                                         unsigned section_line,
                                         const char* lvalue, int ltype,
                                         const char* rvalue, void* data,
...
int config_parse_emergency_action(const char* unit, const char* filename,
                                  unsigned line, const char* section,
                                  unsigned section_line, const char* lvalue,
                                  int ltype, const char* rvalue, void* data,
                                  void* userdata)
    ...
    if (r < 0)
    {
        if (r == -EOPNOTSUPP && MANAGER_IS_USER(m))
        {
            ...
            log_syntax(unit, LOG_INFO, filename, line, r,
                       "%s= in user mode specified as \"%s\", using "
                       "\"exit-force\" instead.",
                       lvalue, rvalue);
        ...
        if (r == -EOPNOTSUPP)
            log_syntax(unit, LOG_ERR, filename, line, r,
                       "%s= specified as %s mode action, ignoring: %s", lvalue,
                       MANAGER_IS_SYSTEM(m) ? "user" : "system", rvalue);
        else
            log_syntax(unit, LOG_ERR, filename, line, r,
                       "Failed to parse %s=, ignoring: %s", lvalue, rvalue);
        return 0;
...
int config_parse_pid_file(const char* unit, const char* filename, unsigned line,
                          const char* section, unsigned section_line,
                          const char* lvalue, int ltype, const char* rvalue,
                          void* data, void* userdata)
{
    ...
    r = path_simplify_and_warn(n, PATH_CHECK_ABSOLUTE, unit, filename, line,
                               lvalue);
    if (r < 0)
        return r;
...
int config_parse_exit_status(const char* unit, const char* filename,
                             unsigned line, const char* section,
                             unsigned section_line, const char* lvalue,
                             int ltype, const char* rvalue, void* data,
                             void* userdata)
...
int config_parse_disable_controllers(const char* unit, const char* filename,
                                     unsigned line, const char* section,
                                     unsigned section_line, const char* lvalue,
                                     int ltype, const char* rvalue, void* data,
                                     void* userdata)
...
static int open_follow(char** filename, FILE** _f, Set* names, char** _final)
{
    ...
    for (;;)
    {
        ...
        /* Hmm, so this is a symlink. Let's read the name, and follow it
         * manually */
...
static int load_from_path(Unit* u, const char* path)
{
    ...
    {
        log_unit_warning(u,
                         "Unit type of %s does not support alias names, "
                         "refusing loading via symlink.",
                         u->id);
        return -ELOOP;
    }
...
int unit_load_fragment(Unit* u)
{
    ...
    if (u->load_state == UNIT_STUB && u->instance)
    {
        ...
        {
            if (r == -ENOEXEC)
                log_unit_notice(u, "Unit configuration has fatal error, unit "
                                   "will not be started.");
            return r;
        }
...
void unit_dump_config_items(FILE* f)
{
    ...
    NULSTR_FOREACH(i, load_fragment_gperf_nulstr)
    {
        ...
        if (p->parse == config_parse_warn_compat && p->ltype == DISABLED_LEGACY)
            continue;
