...
DBusHandlerResult avahi_dbus_respond_error(DBusConnection* c, DBusMessage* m,
...
DBusHandlerResult avahi_dbus_respond_string(DBusConnection* c, DBusMessage* m,
                                            const char* text)
{
    DBusMessage* reply;
...
DBusHandlerResult avahi_dbus_respond_int32(DBusConnection* c, DBusMessage* m,
                                           int32_t i)
{
    DBusMessage* reply;
...
DBusHandlerResult avahi_dbus_respond_uint32(DBusConnection* c, DBusMessage* m,
                                            uint32_t u)
{
    DBusMessage* reply;
...
DBusHandlerResult avahi_dbus_respond_boolean(DBusConnection* c, DBusMessage* m,
                                             int b)
{
    DBusMessage* reply;
...
DBusHandlerResult avahi_dbus_respond_path(DBusConnection* c, DBusMessage* m,
                                          const char* path)
{
    DBusMessage* reply;
...
DBusHandlerResult avahi_dbus_handle_introspect(DBusConnection* c,
                                               DBusMessage* m,
                                               const char* fname)
{
    char *contents, *path;
...
int avahi_dbus_is_our_own_service(Client* c, AvahiIfIndex interface,
                                  AvahiProtocol protocol, const char* name,
                                  const char* type, const char* domain)
{
    AvahiSEntryGroup* g;

    if (avahi_server_get_group_of_service(avahi_server, interface, protocol,
                                          name, type, domain, &g) == AVAHI_OK)
    {
        EntryGroupInfo* egi;
...
int avahi_dbus_append_rdata(DBusMessage* message, const void* rdata,
                            size_t size)
{
    DBusMessageIter iter, sub;
