...
typedef enum
{ /* IDs of base methods supported by all objects - used for jump table, so
...
#define OBJ_NUM_METHODS                                                        \
    8 /* must be updated to contain the max number of methods supported */
...
struct interface_s
{
    ...
    int ifIsLoaded;
    /* is the interface loaded? (0-no, 1-yes, 2-load failed; if not 1, functions
     * can NOT be called! */
};
...
struct obj_s
{ /* the dummy struct that each derived class can be casted to */
    objInfo_t* pObjInfo;
...
/* macros which must be gloablly-visible (because they are used during
...
#define ISOBJ_assert(pObj)                                                     \
    ...
    {                                                                          \
        assert((pObj) != NULL);                                                \
        assert((unsigned)((obj_t*)(pObj))->iObjCooCKiE ==                      \
               (unsigned)0xBADEFEE);                                           \
    } while (0);
#define ISOBJ_TYPE_assert(pObj, objType)                                       \
    ...
    {                                                                          \
        ...
        {                                                                      \
            dbgprintf("%s:%d ISOBJ assert failure: invalid object type, "      \
                      "expected '%s' "                                         \
                      "actual '%s', cookie: %X\n",                             \
                      __FILE__, __LINE__, #objType,                            \
                      (((obj_t*)pObj)->pObjInfo->pszID),                       \
                      ((obj_t*)(pObj))->iObjCooCKiE);                          \
            fprintf(stderr,                                                    \
                    "%s:%d ISOBJ assert failure: invalid object type, "        \
                    "expected '%s' "                                           \
                    "actual '%s', cookie: %X\n",                               \
                    __FILE__, __LINE__, #objType,                              \
                    (((obj_t*)pObj)->pObjInfo->pszID),                         \
            ...
            fflush(stderr);                                                    \
            assert(                                                            \
                !strcmp((char*)(((obj_t*)pObj)->pObjInfo->pszID), #objType));  \
        }                                                                      \
        assert((unsigned)((obj_t*)(pObj))->iObjCooCKiE ==                      \
               (unsigned)0xBADEFEE);                                           \
    } while (0)
...
/* class initializer */
#define PROTOTYPEObjClassInit(objName)                                         \
    rsRetVal objName##ClassInit(struct modInfo_s*)
/* below: objName must be the object name (e.g. vm, strm, ...) and ISCORE must
 * be 1 if the module is a statically linked core module and 0 if it is a
 * dynamically loaded one. -- rgerhards, 2008-02-29
 */
#define OBJ_IS_CORE_MODULE                                                     \
    1 /* This should better be renamed to something like "OBJ_IS_NOT_LIBHEAD"  \
         or so... ;) */
...
#define BEGINObjClassInit(objName, objVers, objType)                           \
    ...
    {                                                                          \
        ...
        { /* are we a core module? */                                          \
            CHKiRet(                                                           \
                objGetObjInterface(&obj)); /* this provides the root pointer   \
                                              for all other queries */         \
        }                                                                      \
        CHKiRet(obj.InfoConstruct(                                             \
            &pObjInfoOBJ, (uchar*)#objName, objVers,                           \
...
#define BEGINAbstractObjClassInit(objName, objVers, objType)                   \
    ...
    {                                                                          \
        ...
        CHKiRet(obj.InfoConstruct(                                             \
            &pObjInfoOBJ, (uchar*)#objName, objVers, NULL, NULL,               \
            (rsRetVal(*)(interface_t*))objName##QueryInterface, pModInfo));
...
/* note: there was a long-time bug in the macro below that lead to *ppThis =
 * NULL only when the object was actually destructed. I discovered this issue
 * during introduction of the pRcvFrom property in smsg_t, but it potentially
 * had other effects, too. I am not sure if some experienced instability
 * resulted from this bug OR if its fix will cause harm to so-far "correctly"
 * running code. The later may very well be. Thus I will change it only for the
 * current branch and also the beta, but not in all old builds. Let's see how
 * things evolve. rgerhards, 2009-06-30
 */
 ...
 * ------------------------------ * The following code builds a dynamic object
 * loader system. The root idea is that all objects are dynamically loadable,
 * which is necessary to get a clean plug-in interface where every plugin can
 * access rsyslog's rich object model via simple and quite portable methods.
 *
 * To do so, each object defines one or more interfaces. They are essentially
 * structures with function (method) pointers. Anyone interested in calling an
 * object must first obtain the interface and can then call through it.
 *
 * The interface data type must always be called <obj>_if_t, as this is expected
 * by the macros. Having consitent naming is also easier for the programmer. By
 * default, macros create a static variable named like the object in each
 * calling objects static data block.
 *
 * rgerhards, 2008-02-21 (initial implementation), 2008-04-17 (update of this
 * note)
 */
...
#define PROTOTYPEObjQueryInterface(obj)                                        \
    rsRetVal obj##QueryInterface(obj##_if_t* pIf)
 ...
 * its ClassExit() entry points are only called if no object is referenced,
 * which would never happen as the library references itself. rgerhards,
 * 2008-03-11
 */
#define CORE_COMPONENT NULL /* use this to indicate this is a core component   \
                             */
#define DONT_LOAD_LIB                                                          \
    NULL /* do not load a library to obtain object interface (currently same   \
            as CORE_COMPONENT) */
...
#define objRelease(objName, FILENAME)                                          \
    obj.ReleaseObj(__FILE__, (uchar*)#objName, (uchar*)FILENAME,               \
                   (void*)&objName)

/* defines data that must always be present at the very begin of the interface
 * structure */
#define ifBEGIN                                                                \
    int ifVersion;  /* must be set to version requested */                     \
    int ifIsLoaded; /* is the interface loaded? (0-no, 1-yes; if no, functions \
                       can NOT be called! */
