...
FILE* open_pager(void)
{
    ...
    if (!pager || (strcmp(pager, "__none__") == 0) ||
...
int check_fs_not_open(char* name)
{
    if (current_fs)
    {
        com_err(name, 0, "Filesystem %s is still open.  Close it first.\n",
...
extern __s64 string_to_time(const char* arg)
{
    ...
    if (tmp == NULL)
        goto fallback;
#else
    sscanf(arg, "%4d%2d%2d%2d%2d%2d", &ts.tm_year, &ts.tm_mon, &ts.tm_mday,
    ...
    if (ts.tm_yday == 0)
        ts.tm_yday = (ts.tm_mon * 31) - ((ts.tm_mon - (ts.tm_mon > 7)) / 2) -
 ...
 */
unsigned long parse_ulong(const char* str, const char* cmd, const char* descr,
                          int* err)
{
    char* tmp;
    unsigned long ret;
 ...
 */
int strtoblk(const char* cmd, const char* str, const char* errmsg, blk64_t* ret)
{
    blk64_t blk;
    int err;
 ...
 */
int common_inode_args_process(int argc, char* argv[], ext2_ino_t* inode,
                              int flags)
{
 ...
 */
int common_block_args_process(int argc, char* argv[], blk64_t* block,
                              blk64_t* count)
{
    int err;

    if (common_args_process(argc, argv, 2, 3, argv[0], "<block> [count]",
                            CHECK_FS_BITMAPS))
        return 1;
...
int debugfs_write_inode_full(ext2_ino_t ino, struct ext2_inode* inode,
                             const char* cmd, int bufsize)
{
    int retval;

    retval = ext2fs_write_inode_full(current_fs, ino, inode, bufsize);
    if (retval)
    {
