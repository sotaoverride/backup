 ...
 * @retval !0 errors
 */
int netsnmp_arch_defaultrouter_container_load(netsnmp_container* container,
                                              u_int load_flags)
{
    int rc = 0;
...
static int _load(netsnmp_container* container)
{
    ...
    if (setsockopt(nlsk, SOL_SOCKET, SO_RCVBUF, &rcvbuf_size,
     ...
     */
    count = sendto(nlsk, sndbuf, hdr->nlmsg_len, 0, (struct sockaddr*)&addr,
    ...
    do
    {
         ...
         * Get the message
         */
        count = recvfrom(nlsk, rcvbuf, RCVBUF_SIZE, 0, (struct sockaddr*)&addr,
                         &sock_len);
        if (count < 0)
        {
        ...
        while (NLMSG_OK(nlmhp, count))
        {
            ...
            if (address_len != 0 && if_index != -1 && lifetime != 0 &&
                preference != -3)
            {
