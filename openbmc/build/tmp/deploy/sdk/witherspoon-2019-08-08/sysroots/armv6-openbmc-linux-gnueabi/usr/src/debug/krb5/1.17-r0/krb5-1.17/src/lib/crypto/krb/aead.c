                                 ...
                                 krb5_crypto_iov* data, size_t num_data)
{
    ...
    for (j = 0; j < num_data; j++)
    {
        if (data[j].flags == KRB5_CRYPTO_TYPE_DATA)
        {
            ...
            data[j].data.length =
                stream->data.length - header_len - trailer_len;
        }
    ...
    iov[i].data = make_data(
        stream->data.data + stream->data.length - trailer_len, trailer_len);
    i++;
...
static size_t next_iov_to_process(struct iov_cursor* cursor, size_t ind)
{
