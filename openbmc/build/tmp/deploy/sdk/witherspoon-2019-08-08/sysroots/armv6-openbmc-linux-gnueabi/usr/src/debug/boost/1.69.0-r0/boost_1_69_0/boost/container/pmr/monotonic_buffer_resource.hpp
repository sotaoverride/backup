...
//! A monotonic_buffer_resource is a special-purpose memory resource intended
//! for very fast memory allocations in situations where memory is used to build
//! up a few objects and then is released all at once when the memory resource
//! object is destroyed. It has the following qualities:
//!
...
class BOOST_CONTAINER_DECL monotonic_buffer_resource : public memory_resource
{
  ...
  public:
    //! The number of bytes that will be requested by the default in the first
    //! call to the upstream allocator
    //!
    //! <b>Note</b>: Non-standard extension.
    static const std::size_t initial_next_buffer_size = 32u * sizeof(void*);

    //! <b>Requires</b>: `upstream` shall be the address of a valid memory
    //! resource or `nullptr`
    //!
    //! <b>Effects</b>: If `upstream` is not nullptr, sets the internal resource
    //! to `upstream`,
    //!   to get_default_resource() otherwise.
    //!   Sets the internal `current_buffer` to `nullptr` and the internal
    //!   `next_buffer_size` to an implementation-defined size.
    explicit monotonic_buffer_resource(memory_resource* upstream = 0)
        BOOST_NOEXCEPT;
    ...
    //! <b>Effects</b>: If `upstream` is not nullptr, sets the internal resource
    //! to `upstream`,
    //!   to get_default_resource() otherwise. Sets the internal
    //!   `current_buffer` to `nullptr` and `next_buffer_size` to at least
    //!   `initial_size`.
    explicit monotonic_buffer_resource(
        std::size_t initial_size, memory_resource* upstream = 0) BOOST_NOEXCEPT;
    ...
    //! to `upstream`,
    //!   to get_default_resource() otherwise. Sets the internal
    //!   `current_buffer` to `buffer`, and `next_buffer_size` to `buffer_size`
    //!   (but not less than an implementation-defined size), then increases
    //!   `next_buffer_size` by an implementation-defined growth factor (which
    //!   need not be integral).
    monotonic_buffer_resource(void* buffer, std::size_t buffer_size,
                              memory_resource* upstream = 0) BOOST_NOEXCEPT;
  ...
  public:
    ...
    //! <b>Effects</b>: `upstream_resource()->deallocate()` as necessary to
    //! release all allocated memory.
    //!   [Note: memory is released back to `upstream_resource()` even if some
    ...
    //!   and the number of bytes wasted due to the requested alignment.
    //!
    //! <b>Note</b>: Non-standard extension.
    std::size_t remaining_storage(std::size_t alignment,
                                  std::size_t& wasted_due_to_alignment) const
        BOOST_NOEXCEPT;
    ...
    //! <b>Note</b>: Non-standard extension.
    std::size_t
        remaining_storage(std::size_t alignment = 1u) const BOOST_NOEXCEPT;
    ...
    //! <b>Returns</b>:
    //!   The number of bytes that will be requested for the next buffer once
  ...
  protected:
    //! <b>Returns</b>: A pointer to allocated storage with a size of at least
    //! `bytes`. The size
    //!   and alignment of the allocated memory shall meet the requirements for
    //!   a class derived from `memory_resource`.
    //!
    //! <b>Effects</b>: If the unused space in the internal `current_buffer` can
    //! fit a block with the specified
    //!   bytes and alignment, then allocate the return block from the internal
    //!   `current_buffer`; otherwise sets the internal `current_buffer` to
    //!   `upstream_resource()->allocate(n, m)`, where `n` is not less than
    ...
    //!   newly-allocated internal `current_buffer`.
    //!
    ...
    //! <b>Throws</b>: Nothing
    //!
    //! <b>Remarks</b>: Memory used by this resource increases monotonically
