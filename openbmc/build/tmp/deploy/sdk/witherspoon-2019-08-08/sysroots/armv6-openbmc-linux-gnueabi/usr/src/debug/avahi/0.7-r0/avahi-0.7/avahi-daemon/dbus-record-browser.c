...
void avahi_dbus_record_browser_free(RecordBrowserInfo* i)
{
    ...
    AVAHI_LLIST_REMOVE(RecordBrowserInfo, record_browsers,
                       i->client->record_browsers, i);
                                                     ...
                                                     void* userdata)
{
    ...
    if (dbus_message_is_method_call(m, DBUS_INTERFACE_INTROSPECTABLE,
        ...
        return avahi_dbus_handle_introspect(
            c, m, "org.freedesktop.Avahi.RecordBrowser.xml");

    /* Access control */
    if (strcmp(dbus_message_get_sender(m), i->client->name))
        return avahi_dbus_respond_error(c, m, AVAHI_ERR_ACCESS_DENIED, NULL);

    if (dbus_message_is_method_call(m, AVAHI_DBUS_INTERFACE_RECORD_BROWSER,
                                    "Free"))
    {
    ...
    void* userdata)
{
    ...
    m = dbus_message_new_signal(i->path, AVAHI_DBUS_INTERFACE_RECORD_BROWSER,
    ...
    if (event == AVAHI_BROWSER_NEW || event == AVAHI_BROWSER_REMOVE)
    {
        ...
        if (!(dbus_message_append_args(
                ...
                DBUS_TYPE_STRING, &record->key->name, DBUS_TYPE_UINT16,
                &record->key->clazz, DBUS_TYPE_UINT16, &record->key->type,
                DBUS_TYPE_INVALID)))
            goto fail;

        if ((size = avahi_rdata_serialize(record, rdata, sizeof(rdata))) ==
                (size_t)-1 ||
        ...
        dbus_message_append_args(m, DBUS_TYPE_UINT32, &u_flags,
                                 DBUS_TYPE_INVALID);
    }
