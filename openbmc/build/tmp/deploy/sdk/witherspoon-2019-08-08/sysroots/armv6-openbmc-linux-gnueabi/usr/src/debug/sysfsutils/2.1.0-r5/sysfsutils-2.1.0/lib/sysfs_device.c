...
static int get_dev_driver(struct sysfs_device* dev)
{
    ...
    if (!sysfs_path_is_link(path))
    {
        ...
        {
            if (!sysfs_get_name_from_path(devpath, dev->driver_name,
                                          SYSFS_NAME_LEN))
                return 0;
        }
    }
...
int sysfs_get_device_bus(struct sysfs_device* dev)
{
    ...
    if (!sysfs_path_is_link(path))
    {
        if (!sysfs_get_link(path, devpath, SYSFS_PATH_MAX))
        {
            if (!sysfs_get_name_from_path(devpath, dev->bus, SYSFS_NAME_LEN))
                return 0;
        }
    }
    return -1;
...
static int get_dev_subsystem(struct sysfs_device* dev)
{
    ...
    if (!sysfs_path_is_link(path))
    {
        if (!sysfs_get_link(path, devpath, SYSFS_PATH_MAX))
        {
            if (!sysfs_get_name_from_path(devpath, dev->subsystem,
                                          SYSFS_NAME_LEN))
                return 0;
        }
    }
...
void sysfs_close_device_tree(struct sysfs_device* devroot)
{
    if (devroot)
    {
        if (devroot->children)
        {
            ...
            dlist_for_each_data(devroot->children, child, struct sysfs_device)
...
struct sysfs_device* sysfs_open_device_tree(const char* path)
{
    ...
    if (devlist->children)
    {
        dlist_for_each_data(devlist->children, cur, struct sysfs_device)
        {
...
struct sysfs_device* sysfs_open_device(const char* bus, const char* bus_id)
{
    ...
    if (get_device_absolute_path(bus_id, bus, sysfs_path, SYSFS_PATH_MAX))
    {
        dprintf("Error getting to device %s\n", bus_id);
        return NULL;
    }
