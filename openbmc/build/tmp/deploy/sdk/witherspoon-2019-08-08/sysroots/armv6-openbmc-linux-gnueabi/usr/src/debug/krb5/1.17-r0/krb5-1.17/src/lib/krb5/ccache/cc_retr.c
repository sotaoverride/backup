...
/*
 * Copyright 1990,1991,1999,2007,2008 by the Massachusetts Institute of
...
static int times_match_exact(const krb5_ticket_times* t1,
                             const krb5_ticket_times* t2)
{
    return (t1->authtime == t2->authtime && t1->starttime == t2->starttime &&
            t1->endtime == t2->endtime && t1->renew_till == t2->renew_till);
}

static krb5_boolean times_match(const krb5_ticket_times* t1,
                                const krb5_ticket_times* t2)
{
    if (t1->renew_till)
    {
...
static krb5_boolean princs_match(krb5_context context, krb5_flags whichfields,
...
static krb5_boolean authdata_match(krb5_authdata* const* mdata,
                                   krb5_authdata* const* data)
{
...
static krb5_boolean data_match(const krb5_data* data1, const krb5_data* data2)
{
    if (!data1)
    {
        if (!data2)
...
static int pref(krb5_enctype my_ktype, int nktypes, krb5_enctype* ktypes)
{
    int i;
    for (i = 0; i < nktypes; i++)
...
krb5_boolean krb5int_cc_creds_match_request(krb5_context context,
...
static krb5_error_code krb5_cc_retrieve_cred_seq(
    krb5_context context, krb5_ccache id, krb5_flags whichfields,
    krb5_creds* mcreds, krb5_creds* creds, int nktypes, krb5_enctype* ktypes)
{
...
#define fetchcreds (fetched.creds)
    ...
    {
        if (krb5int_cc_creds_match_request(context, whichfields, mcreds,
                                           &fetchcreds))
        {
            if (ktypes)
            {
                fetched.pref =
                    pref(fetchcreds.keyblock.enctype, nktypes, ktypes);
                if (fetched.pref < 0)
...
krb5_error_code k5_cc_retrieve_cred_default(krb5_context context,
                                            ...
                                            krb5_creds* creds)
{
    ...
    {
        return krb5_cc_retrieve_cred_seq(context, id, flags, mcreds, creds, 0,
                                         0);
    }
}
