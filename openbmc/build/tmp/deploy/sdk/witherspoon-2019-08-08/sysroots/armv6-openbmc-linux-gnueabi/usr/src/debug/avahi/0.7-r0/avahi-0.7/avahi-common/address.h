...
#define AVAHI_IF_VALID(ifindex)                                                \
    (((ifindex) >= 0) || ((ifindex) == AVAHI_IF_UNSPEC))

/** Return TRUE if the specified protocol is valid */
#define AVAHI_PROTO_VALID(protocol)                                            \
    (((protocol) == AVAHI_PROTO_INET) || ((protocol) == AVAHI_PROTO_INET6) ||  \
...
/** Compare two addresses. Returns 0 when equal, a negative value when a < b, a
...
/** Convert the specified address *a to a human readable character string, use
 * AVAHI_ADDRESS_STR_MAX to allocate an array of the right size */
 ...
 * family detection. */
AvahiAddress* avahi_address_parse(const char* s, AvahiProtocol af,
...
char* avahi_reverse_lookup_name(const AvahiAddress* a, char* ret_s,
                                size_t length);
/** \endcond */
...
/** Return a textual representation of the specified protocol number. i.e.
 * "IPv4", "IPv6" or "UNSPEC" */
