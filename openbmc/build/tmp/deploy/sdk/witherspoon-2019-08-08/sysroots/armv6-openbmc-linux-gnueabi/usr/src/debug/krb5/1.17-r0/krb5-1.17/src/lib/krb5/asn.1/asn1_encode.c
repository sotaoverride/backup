...
krb5_error_code k5_asn1_encode_generaltime(asn1buf* buf, time_t val)
{
    ...
    else
    {
        ...
        if (gtime->tm_year > 8099 || gtime->tm_mon > 11 ||
            gtime->tm_mday > 31 || gtime->tm_hour > 23 || gtime->tm_min > 59 ||
            gtime->tm_sec > 59)
            return ASN1_BAD_GMTIME;
...
#define c2i(c) ((c) - '0')
    ts.tm_year =
        1000 * c2i(s[0]) + 100 * c2i(s[1]) + 10 * c2i(s[2]) + c2i(s[3]) - 1900;
 ...
 */
static krb5_error_code get_tag(const uint8_t* asn1, size_t len,
                               taginfo* tag_out, const uint8_t** contents_out,
                               size_t* clen_out, const uint8_t** remainder_out,
                               size_t* rlen_out)
{
    krb5_error_code ret;
    uint8_t o;
...
static size_t get_nullterm_sequence_len(const void* valp,
                                        const struct atype_info* seq)
{
    size_t i;
...
static krb5_error_code encode_sequence_of(asn1buf* buf, size_t seqlen,
                                          const void* val,
...
static krb5_error_code load_count(const void* val,
                                  const struct counted_info* counted,
                                  size_t* count_out)
{
...
static krb5_error_code split_der(asn1buf* buf, uint8_t* const* der, size_t len,
                                 taginfo* tag_out)
{
    krb5_error_code ret;
 ...
 * DER encoding.
 */
static krb5_error_code store_der(const taginfo* t, const uint8_t* asn1,
                                 size_t len, void* val, size_t* count_out)
{
    uint8_t* der;
    size_t der_len;
...
static krb5_error_code encode_sequence(asn1buf* buf, const void* val,
                                       const struct seq_info* seq);
static krb5_error_code encode_cntype(asn1buf* buf, const void* val, size_t len,
...
static krb5_error_code encode_atype(asn1buf* buf, const void* val,
                                    const struct atype_info* a,
                                    taginfo* tag_out)
{
    krb5_error_code ret;
    ...
    switch (a->type)
    {
        ...
        case atype_nonempty_nullterm_sequence_of:
            assert(a->tinfo != NULL);
            ret = encode_nullterm_sequence_of(
                buf, val, a->tinfo, a->type == atype_nullterm_sequence_of);
            if (ret)
                return ret;
            tag_out->asn1class = UNIVERSAL;
            tag_out->construction = CONSTRUCTED;
...
static krb5_error_code encode_atype_and_tag(asn1buf* buf, const void* val,
                                            const struct atype_info* a)
{
    taginfo t;
    krb5_error_code ret;
 ...
 */
static krb5_error_code encode_cntype(asn1buf* buf, const void* val,
                                     size_t count, const struct cntype_info* c,
                                     taginfo* tag_out)
{
    krb5_error_code ret;
    ...
    switch (a->type)
    {
        ...
        case atype_counted:
        {
            ...
            switch (counted->basetype->type)
            {
                ...
                case cntype_der:
                    /*
                     * We treat any tag as matching a stored DER encoding.  In
                     * some cases we know what the tag should be; in others, we
                     * truly want to accept any tag.  If it ever becomes an
                     * issue, we could add optional tag info to the type and
                     * check it here.
                     */
                    return 1;
                case cntype_choice:
                    /*
                     * ASN.1 choices may or may not be extensible.  For now, we
                     * treat all choices as extensible and match any tag.  We
                     * should consider modeling whether choices are extensible
                     * before making the encoder visible to plugins.
                     */
                    return 1;
                default:
                    abort();
...
static krb5_error_code decode_cntype(const taginfo* t, const uint8_t* asn1,
                                     size_t len, const struct cntype_info* c,
                                     void* val, size_t* count_out);
static krb5_error_code decode_atype_to_ptr(const taginfo* t,
                                           const uint8_t* asn1, size_t len,
                                           const struct atype_info* basetype,
                                           void** ptr_out);
static krb5_error_code decode_sequence(const uint8_t* asn1, size_t len,
                                       const struct seq_info* seq, void* val);
static krb5_error_code decode_sequence_of(const uint8_t* asn1, size_t len,
                                          const struct atype_info* elemtype,
                                          void** seq_out, size_t* count_out);
...
static krb5_error_code decode_atype(const taginfo* t, const uint8_t* asn1,
                                    ...
                                    void* val)
{
    ...
    switch (a->type)
    {
        ...
        case atype_ptr:
        {
            ...
            {
                ret =
                    decode_atype_to_ptr(t, asn1, len, ptrinfo->basetype, &ptr);
                if (ret)
                    return ret;
        ...
        case atype_counted:
        {
            ...
            assert(counted->basetype != NULL);
            ret =
                decode_cntype(t, asn1, len, counted->basetype, dataptr, &count);
            if (ret)
                return ret;
        ...
        default:
            /* Null-terminated sequence types are handled in
             * decode_atype_to_ptr, since they create variable-sized objects. */
...
static krb5_error_code null_terminate(const struct atype_info* eltinfo,
                                      void* ptr, size_t count, void** ptr_out)
{
...
krb5_error_code k5_asn1_encode_atype(asn1buf* buf, const void* val,
                                     const struct atype_info* a,
                                     taginfo* tag_out)
{
...
krb5_error_code k5_asn1_decode_atype(const taginfo* t, const uint8_t* asn1,
                                     size_t len, const struct atype_info* a,
                                     void* val)
{
...
krb5_error_code k5_asn1_full_encode(const void* rep, const struct atype_info* a,
                                    krb5_data** code_out)
{
    krb5_error_code ret;
    asn1buf buf;
...
krb5_error_code k5_asn1_full_decode(const krb5_data* code,
                                    const struct atype_info* a, void** retrep)
{
    ...
    *retrep = NULL;
    ret = get_tag((uint8_t*)code->data, code->length, &t, &contents, &clen,
                  &remainder, &rlen);
    if (ret)
        return ret;
