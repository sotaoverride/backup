...
static off64_t _IO_cookie_seek(FILE* fp, off64_t offset, int dir)
{
    ...
    return ((seek_cb == NULL ||
             (seek_cb(cfile->__cookie, &offset, dir) == -1) ||
             offset == (off64_t)-1)
                ? _IO_pos_BAD
                : offset);
}
...
static off64_t _IO_cookie_seekoff(FILE* fp, off64_t offset, int dir, int mode)
{
...
static off64_t attribute_compat_text_section _IO_old_cookie_seek(FILE* fp,
                                                                 off64_t offset,
                                                                 int dir)
{
    struct _IO_cookie_file* cfile = (struct _IO_cookie_file*)fp;
...
FILE* attribute_compat_text_section _IO_old_fopencookie(
    void* cookie, const char* mode, cookie_io_functions_t io_functions)
{
    FILE* ret;
