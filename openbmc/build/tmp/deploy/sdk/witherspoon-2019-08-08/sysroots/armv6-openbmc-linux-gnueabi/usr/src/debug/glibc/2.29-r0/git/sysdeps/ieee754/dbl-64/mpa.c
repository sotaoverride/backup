...
void SECTION __dbl_mp(double x, mp_no* y, int p)
{
    long i, n;
    long p2 = p;
...
/* Add magnitudes of *X and *Y assuming that abs (*X) >= abs (*Y) > 0.  The
   ...
   truncated.  */
static void SECTION add_magnitudes(const mp_no* x, const mp_no* y, mp_no* z,
                                   int p)
{
    long i, j, k;
    long p2 = p;
...
/* Subtract the magnitudes of *X and *Y assuming that abs (*x) > abs (*y) > 0.
   ...
   ULP.  */
static void SECTION sub_magnitudes(const mp_no* x, const mp_no* y, mp_no* z,
                                   int p)
{
    long i, j, k;
    long p2 = p;
...
/* Add *X and *Y and store the result in *Z.  X and Y may overlap, but not X
   and Z or Y and Z.  One guard digit is used.  The error is less than one
   ULP.  */
void SECTION __add(const mp_no* x, const mp_no* y, mp_no* z, int p)
{
    int n;
...
/* Subtract *Y from *X and return the result in *Z.  X and Y may overlap but
   ...
   one ULP.  */
void SECTION __sub(const mp_no* x, const mp_no* y, mp_no* z, int p)
{
    int n;
...
void SECTION __mul(const mp_no* x, const mp_no* y, mp_no* z, int p)
{
    long i, j, k, ip, ip2;
    long p2 = p;
    mantissa_store_t zk;
    ...
    while (k > p2)
    {
        ...
        if (k % 2 == 0)
            /* We want to add this only once, but since we subtract it in the
               sum of products above, we add twice.  */
    ...
    /* The real deal.  Mantissa digit Z[k] is the sum of all X[i] * Y[j] where i
       ...
       add the diagonal element X[k/2]Y[k/2].  Through the half range, we
       compute X[i] * Y[j] as (X[i] + X[j]) * (Y[i] + Y[j]) - X[i] * Y[i] - X[j]
       * Y[j].
    ...
    /* Get the exponent sum into an intermediate variable.  This is a subtle
       optimization, where given enough registers, all operations on the
       exponent
...
/* Square *X and store result in *Y.  X and Y may not overlap.  For P in
   ...
   multiplication.  */
void SECTION __sqr(const mp_no* x, mp_no* y, int p)
{
    long i, j, k, ip;
    mantissa_store_t yk;
...
static void SECTION __inv(const mp_no* x, mp_no* y, int p)
{
    long i;
    double t;
    mp_no z, w;
    static const int np1[] = {0, 0, 0, 0, 1, 2, 2, 2, 2, 3, 3, 3, 3, 3, 3, 3, 3,
...
/* Divide *X by *Y and store result in *Z.  X and Y may overlap but not X and Z
   ...
   *X = 0 is not permissible.  */
void SECTION __dvd(const mp_no* x, const mp_no* y, mp_no* z, int p)
{
    mp_no w;
