...
static const char optionsstr[] =
    ...
    "-p, --peb-size=<bytes>       size of the physical eraseblock of the "
    "flash\n"
    ...
    "                             header)\n"
    "-e, --erase-counter=<num>    the erase counter value to put to EC "
    "headers\n"
    "                             (default is 0)\n"
...
static const char usage[] =
    ...
    "Generate UBI images. An UBI image may contain one or more UBI volumes "
    "which\n"
...
static int parse_opt(int argc, char* const argv[])
{
    ...
    while (1)
    {
        ...
        key =
            getopt_long(argc, argv, "o:p:m:s:O:e:x:Q:vhV", long_options, NULL);
        if (key == -1)
            break;
        ...
        switch (key)
        {
            ...
            case 'p':
                ...
                if (args.peb_size <= 0)
                    return errmsg("bad physical eraseblock size: \"%s\"",
                                  optarg);
                break;
    ...
    if (optind == argc)
        return errmsg(
            "input configuration file was not specified (use -h for help)");

    if (optind != argc - 1)
        return errmsg(
            "more then one configuration file was specified (use -h for help)");

    args.f_in = argv[optind];

    if (args.peb_size < 0)
        return errmsg(
            "physical eraseblock size was not specified (use -h for help)");
    ...
    if (args.peb_size % args.min_io_size)
        return errmsg(
            "physical eraseblock should be multiple of min. I/O units");
    ...
    *img = NULL;
    ...
    else
    {
        if (!strcmp(p, "static"))
            ...
            return errmsg("invalid volume type \"%s\" in section  \"%s\"", p,
                          sname);
    }
        ...
        *img = p;
        if (stat(p, st))
            return sys_errmsg("cannot stat \"%s\" referred from section \"%s\"",
                              p, sname);
        if (st->st_size == 0)
            return errmsg("empty file \"%s\" referred from section \"%s\"", p,
                          sname);
    }
    else if (vi->type == UBI_VID_STATIC)
        return errmsg(
            "image is not specified for static volume in section \"%s\"",
            sname);
    ...
    if (vi->id < 0)
        return errmsg("negative volume ID %d in section \"%s\"", vi->id, sname);
    ...
    if (p)
    {
        ...
        if (*img && st->st_size > vi->bytes)
            return errmsg(
                "error in section \"%s\": size of the image file "
    ...
    else
    {
        ...
        if (!*img)
            return errmsg("neither image file (\"image=\") nor volume size "
                          "(\"vol_size=\") specified in section \"%s\"",
                          sname);
    ...
    if (vi->name_len > UBI_VOL_NAME_MAX)
        return errmsg(
            "too long volume name in section \"%s\", max. is %d characters",
    ...
    if (vi->type == UBI_VID_DYNAMIC)
        vi->used_ebs =
            (vi->bytes + vi->usable_leb_size - 1) / vi->usable_leb_size;
    else
        vi->used_ebs =
            (st->st_size + vi->usable_leb_size - 1) / vi->usable_leb_size;
    vi->compat = 0;
    return 0;
}
...
int main(int argc, char* const argv[])
{
    ...
    {
        errmsg("too many sections (%d) in the ini-file \"%s\"", sects,
               args.f_in);
    ...
    err =
        ubigen_write_layout_vol(&ui, 0, 1, args.ec, args.ec, vtbl, args.out_fd);
    if (err)
    {
