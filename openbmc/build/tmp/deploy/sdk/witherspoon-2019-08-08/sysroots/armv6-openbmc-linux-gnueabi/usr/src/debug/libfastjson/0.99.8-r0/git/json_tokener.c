...
const char* fjson_tokener_error_desc(enum fjson_tokener_error jerr)
{
    ...
    if (jerr_int < 0 || jerr_int >= (int)(sizeof(fjson_tokener_errors) /
                                          sizeof(fjson_tokener_errors[0])))
        return "Unknown error, invalid fjson_tokener_error value passed to "
               "fjson_tokener_error_desc()";
...
struct fjson_tokener* fjson_tokener_new_ex(int depth)
{
    ...
    if (!tok)
        return NULL;
    tok->stack = (struct fjson_tokener_srec*)calloc(
        depth, sizeof(struct fjson_tokener_srec));
    if (!tok->stack)
    {
        free(tok);
        return NULL;
...
#define PEEK_CHAR(dest, tok)                                                   \
    (((tok)->char_offset == len)                                               \
         ? (((tok)->depth == 0 && state == fjson_tokener_state_eatws &&        \
 ...
 *   For convenience of existing conditionals, returns the old value of c (0 on
 * eof) Implicit inputs:  c var
 */
#define ADVANCE_CHAR(str, tok) (++(str), ((tok)->char_offset)++, c)

/* End optimization macro defs */

struct fjson_object* fjson_tokener_parse_ex(struct fjson_tokener* tok,
        ...
        switch (state)
        {
            ...
            case fjson_tokener_state_inf: /* aka starts with 'i' */
            {
                    ...
                    (strncmp(fjson_inf_str, infbuf, size_inf) == 0))
                {
                    ...
                    {
                        current = fjson_object_new_double(
                            is_negative ? -INFINITY : INFINITY);
                        saved_state = fjson_tokener_state_finish;
                        state = fjson_tokener_state_eatws;
                        goto redo_char;
                    }
                    ...
                    (strncmp(fjson_null_str, tok->pb->buf, size) == 0))
                {
                    if (tok->st_pos == fjson_null_str_len)
                    {
                        ...
                        goto redo_char;
                          ...
                          strncasecmp(fjson_nan_str, tok->pb->buf, size_nan) ==
                              0) ||
            ...
            case fjson_tokener_state_string:
            {
                ...
                while (1)
                {
                    if (c == tok->quote_char)
                    {
                        printbuf_memappend_fast(tok->pb, case_start,
                                                str - case_start);
                        current = fjson_object_new_string_len(tok->pb->buf,
                                                              tok->pb->bpos);
                        saved_state = fjson_tokener_state_finish;
                        state = fjson_tokener_state_eatws;
                        break;
            ...
            case fjson_tokener_state_escape_unicode:
            {
                ...
                /* Handle a 4-byte sequence, or two sequences if a surrogate
                 * pair */
                while (1)
                {
                    if (strchr(fjson_hex_chars, c))
                    {
                        ...
                        if (tok->st_pos == 4)
                        {
                            ...
                            if (got_hi_surrogate)
                            {
                                ...
                                {
                                    /* Recalculate the ucs_char, then fall thru
                                       to process normally */
                                    tok->ucs_char = DECODE_SURROGATE_PAIR(
                                ...
                                else
                                {
                                     ...
                                     * surrogate */
                                    /* Replace the hi and process the rest
                                     * normally */
                                ...
                                got_hi_surrogate = 0;
                                /* clang static analyzer thins that
                                 * got_hi_surrogate is never read, * however, it
                                 * is read on each iteration. So I assume clang
                                 * has a false positive. We use the otherwise
                                 * nonsense statement below to make it happy.
                                 */
                                if (got_hi_surrogate)
                                {
                                };
                            ...
                            {
                                /* Got a high surrogate.  Remember it and look
                                 * for the the beginning of another sequence,
                                 * which should be the low surrogate.
                                 */
                                got_hi_surrogate = tok->ucs_char;
                                /* Not at end, and the next two chars should be
                                 * "\u" */
                                    ...
                                    (str[1] == '\\') && (str[2] == 'u'))
                                {
                                    ...
                                    /* Advance to the first char of the next
                                     * sequence and continue processing with the
                                     * next sequence.
                                     */
                                    if (!ADVANCE_CHAR(str, tok) ||
                                        !PEEK_CHAR(c, tok))
                                ...
                                {
                                    /* Got a high surrogate without another
                                     * sequence following it.  Put a replacement
                                     * char in for the hi surrogate and pretend
                                     * we finished.
                                     */
                                    printbuf_memappend_fast(
                                        tok->pb, (char*)utf8_replacement_char,
                            ...
                            {
                                /* Don't know what we got--insert the
                                 * replacement char */
                                printbuf_memappend_fast(
                                    tok->pb, (char*)utf8_replacement_char, 3);
                            }
                    ...
                    (strncmp(fjson_true_str, tok->pb->buf, size1) == 0))
                {
                    if (tok->st_pos == fjson_true_str_len)
                    {
                        ...
                        goto redo_char;
                          ...
                          strncasecmp(fjson_false_str, tok->pb->buf, size2) ==
                              0) ||
            ...
            case fjson_tokener_state_number:
            {
                ...
                while (c && strchr(fjson_number_chars, c))
                {
                    ...
                    if (c == '.')
                    {
                        if (tok->is_double != 0)
                        {
                            /* '.' can only be found once, and out of the
                               exponent part. Thus, if the input is already
                               flagged as double, it is invalid. */
            ...
            }
                {
                    ...
                    double numd;
                    if (!tok->is_double &&
                        fjson_parse_int64(tok->pb->buf, &num64) == 0)
                    {
                        ...
                        current = fjson_object_new_int64(num64);
                             ...
                             fjson_parse_double(tok->pb->buf, &numd) == 0)
                    {
            ...
            case fjson_tokener_state_array:
                if (c == ']')
                {
                    if (state == fjson_tokener_state_array_after_sep &&
                        (tok->flags & FJSON_TOKENER_STRICT))
                    {
                        tok->err = fjson_tokener_error_parse_unexpected;
                        goto out;
...
out:
    if (c && (state == fjson_tokener_state_finish) && (tok->depth == 0) &&
        (tok->flags & FJSON_TOKENER_STRICT))
    {
    ...
    { /* We hit an eof char (0) */
        if (state != fjson_tokener_state_finish &&
            saved_state != fjson_tokener_state_finish)
