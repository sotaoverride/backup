 ...
 *   c<bus_id>                             — bus id of a grouped CCW or CCW
 * device, with all leading zeros stripped [s390]
...
static int get_virtfn_info(sd_device* dev, struct netnames* names,
                           struct virtfn_info* ret)
{
...
static int dev_pci_onboard(sd_device* dev, struct netnames* names)
{
    ...
    /* ACPI _DSM  — device specific method for naming a PCI or PCI Express
     * device */
    ...
    /* Some BIOSes report rubbish indexes that are excessively high (2^24-1 is
     * an index VMware likes to report for example). Let's define a cut-off
     * where we don't consider the index reliable anymore. We pick some
     * arbitrary cut-off, which is somewhere beyond the realistic number of
     * physical network interface a system might
     * have. Ideally the kernel would already filter his crap for us, but it
     * doesn't currently. */
...
static int dev_pci_slot(sd_device* dev, struct netnames* names)
{
    ...
    if (naming_scheme_has(NAMING_NPAR_ARI) && is_pci_ari_enabled(names->pcidev))
        /* ARI devices support up to 256 functions on a single device ("slot"),
         * and interpret the traditional 5-bit slot and 3-bit function number as
         * a single 8-bit function number, where the slot makes up the upper 5
         * bits. */
        func += slot * 8;
    ...
    {
        dev_port = strtoul(attr, NULL, 10);
        /* With older kernels IP-over-InfiniBand network interfaces sometimes
         * erroneously provide the port number in the 'dev_id' sysfs attribute
         * instead of 'dev_port', which thus stays initialized as 0. */
        if (dev_port == 0 &&
    ...
    while (hotplug_slot_dev)
    {
        ...
        if (sd_device_get_parent_with_subsystem_devtype(
                hotplug_slot_dev, "pci", NULL, &hotplug_slot_dev) < 0)
            break;
        rewinddir(dir);
    }
...
static int names_vio(sd_device* dev, struct netnames* names)
{
    ...
    if (sscanf(syspath, "/sys/devices/vio/%4x%4x/net/eth%u", &busid, &slotid,
               &ethid) != 3)
        return -EINVAL;
...
static int names_platform(sd_device* dev, struct netnames* names, bool test)
{
    ...
    /* Vendor ID can be either PNP ID (3 chars A-Z) or ACPI ID (4 chars A-Z and
     * numerals) */
     ...
     * The Vendor (3 or 4 char), followed by hexdecimal model number : instance
     * id.
     */
...
static int names_pci(sd_device* dev, struct netnames* names)
{
    ...
    {
        r = sd_device_get_parent_with_subsystem_devtype(dev, "pci", NULL,
                                                        &names->pcidev);
        if (r < 0)
            return r;
    }
    ...
    {
        /* If this is an SR-IOV virtual device, get base name using physical
         * device and add virtfn suffix. */
        vf_names.pcidev = vf_info.physfn_pcidev;
        dev_pci_onboard(dev, &vf_names);
        dev_pci_slot(dev, &vf_names);
        if (vf_names.pci_onboard[0])
            if (strlen(vf_names.pci_onboard) + strlen(vf_info.suffix) <
                sizeof(names->pci_onboard))
                strscpyl(names->pci_onboard, sizeof(names->pci_onboard),
                         vf_names.pci_onboard, vf_info.suffix, NULL);
        if (vf_names.pci_slot[0])
            if (strlen(vf_names.pci_slot) + strlen(vf_info.suffix) <
        ...
        if (vf_names.pci_path[0])
            if (strlen(vf_names.pci_path) + strlen(vf_info.suffix) <
                sizeof(names->pci_path))
...
static int names_usb(sd_device* dev, struct netnames* names)
{
    ...
    r = sd_device_get_parent_with_subsystem_devtype(dev, "usb", "usb_interface",
                                                    &usbdev);
    if (r < 0)
        return r;
...
static int names_bcma(sd_device* dev, struct netnames* names)
{
    ...
    r = sd_device_get_parent_with_subsystem_devtype(dev, "bcma", NULL,
                                                    &bcmadev);
    if (r < 0)
        return r;
...
static int ieee_oui(sd_device* dev, struct netnames* names, bool test)
{
    ...
    if (memcmp(names->mac, "\0\0\0", 3) == 0)
        return -EINVAL;
    xsprintf(str, "OUI:%02X%02X%02X%02X%02X%02X", names->mac[0], names->mac[1],
             names->mac[2], names->mac[3], names->mac[4], names->mac[5]);
...
static int builtin_net_id(sd_device* dev, int argc, char* argv[], bool test)
{
    ...
    udev_builtin_add_property(dev, test, "ID_NET_NAMING_SCHEME",
                              naming_scheme()->name);
    ...
    if (r >= 0 && names.mac_valid)
    {
        ...
        xsprintf(str, "%sx%02x%02x%02x%02x%02x%02x", prefix, names.mac[0],
    ...
    if (names.type == NET_PCI)
    {
        ...
        if (names.pci_onboard_label &&
            snprintf_ok(str, sizeof str, "%s%s", prefix,
    ...
    if (names_usb(dev, &names) >= 0 && names.type == NET_USB)
    {
        ...
        if (names.pci_path[0] && snprintf_ok(str, sizeof str, "%s%s%s", prefix,
                                             names.pci_path, names.usb_ports))
            udev_builtin_add_property(dev, test, "ID_NET_NAME_PATH", str);

        if (names.pci_slot[0] && snprintf_ok(str, sizeof str, "%s%s%s", prefix,
                                             names.pci_slot, names.usb_ports))
            udev_builtin_add_property(dev, test, "ID_NET_NAME_SLOT", str);
        return 0;
    }
    ...
    if (names_bcma(dev, &names) >= 0 && names.type == NET_BCMA)
    {
        ...
        if (names.pci_path[0] && snprintf_ok(str, sizeof str, "%s%s%s", prefix,
                                             names.pci_path, names.bcma_core))
            udev_builtin_add_property(dev, test, "ID_NET_NAME_PATH", str);

        if (names.pci_slot[0] && snprintf(str, sizeof str, "%s%s%s", prefix,
                                          names.pci_slot, names.bcma_core))
            udev_builtin_add_property(dev, test, "ID_NET_NAME_SLOT", str);
        return 0;
    }
