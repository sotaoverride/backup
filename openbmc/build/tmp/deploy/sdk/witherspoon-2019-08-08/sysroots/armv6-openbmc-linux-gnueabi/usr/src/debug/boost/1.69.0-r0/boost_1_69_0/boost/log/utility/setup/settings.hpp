...
// This workaround is needed for MSVC 10 to work around ICE caused by stack
// overflow
...
struct basic_settings_section_iterator_base<SectionT, true>
{
    ...
    typedef typename SectionT::property_tree_type::const_iterator
        base_iterator_type;
    typedef iterator_adaptor<iterator_type, base_iterator_type, SectionT,
 ...
 *
 * The section refers to a sub-tree of the library settings container. It does
 * not own the referred sub-tree but allows for convenient access to parameters
...
class basic_settings_section
{
  ...
  public:
    ...
    //! Property tree type
    typedef property_tree::basic_ptree<std::string, string_type>
        property_tree_type;
  ...
  private:
    ...
    class ref
    {
      private:
        typedef typename mpl::if_c<
            IsConstV, basic_settings_section<char_type> const,
            basic_settings_section<char_type>>::type section_type;
      ...
      public:
        ref(section_type& section, std::string const& section_name) :
            m_section(section), m_path(section_name)
        {
        }
        ...
        ref& operator=(ref<V> const& value)
        {
            ...
            else if (optional<property_tree_type&> node =
                         m_section.m_ptree->get_child_optional(m_path))
            {
        ...
        optional<string_type> get() const
        {
            if (m_section.m_ptree)
                return m_section.m_ptree->template get_optional<string_type>(
                    m_path);
            else
        ...
        section_type get_section() const
        {
            if (m_section.m_ptree)
                return section_type(
                    m_section.m_ptree->get_child_optional(m_path).get_ptr());
            else
                return section_type();
        }
    ...
    !(defined(__GNUC__) && !defined(__INTEL_COMPILER) &&                       \
      !defined(__clang__) && !defined(__PATHSCALE__) &&                        \
      !defined(__GXX_EXPERIMENTAL_CXX0X__) &&                                  \
      (__GNUC__ == 4 && __GNUC_MINOR__ <= 5))
#if !defined(BOOST_NO_CXX11_TRAILING_RESULT_TYPES)
        template <typename T>
        auto or_default(T const& def_value) const
            -> BOOST_LOG_TYPEOF(property_tree_type().get(
                typename property_tree_type::path_type(), def_value))
        {
            if (m_section.m_ptree)
...
#else
        // GCC up to 4.5 (inclusively) segfaults on the following code, if C++11
        // mode is not enabled
        template <typename T>
        BOOST_LOG_TYPEOF(property_tree_type().get(
            typename property_tree_type::path_type(), boost::declval<T>()))
        or_default(T const& def_value) const
        {
            if (m_section.m_ptree)
                return m_section.m_ptree->get(m_path, def_value);
            else
        ...
        typename boost::enable_if_c<
            boost::property_tree::detail::is_character<T>::value,
        ...
        string_type
            or_default(typename string_type::value_type const* def_value) const
        {
...
#endif
    template <bool IsConstV>
    class iter : public aux::basic_settings_section_iterator_base<
                     basic_settings_section<char_type>, IsConstV>::type
    {
        friend class boost::iterator_core_access;

        typedef typename iter::iterator_adaptor_ iterator_adaptor_;
        // NOTE: This typedef must not come from iterator_adaptor_::base_type in
        // order to work around MSVC 10 ICE
        typedef typename aux::basic_settings_section_iterator_base<
            basic_settings_section<char_type>, IsConstV>::base_iterator_type
            base_iterator_type;
      ...
      private:
        ...
        {
            return reference(
                const_cast<property_tree_type*>(&this->base()->second));
        }
    };
  ...
  public:
    ...
    /*!
     * Checks if the container is empty (i.e. contains no sections and
     * parameters).
     */
    bool empty() const
    {
    ...
    /*!
     * Accessor to a single parameter. This operator should be used in
     * conjunction with the subsequent subscript operator that designates the
     * parameter name.
     *
     * \param section_name The name of the section in which the parameter
     * resides \return An unspecified reference type that can be used for
     * parameter name specifying
     */
     ...
     *
     * \param section_name The name of the section in which the parameter
     * resides \param param_name The name of the parameter
     */
                       ...
                       string_type const& param_name) const
    {
        if (m_ptree)
        {
            optional<property_tree_type&> section =
                m_ptree->get_child_optional(section_name);
            if (!!section)
...
template <typename CharT>
inline void swap(basic_settings_section<CharT>& left,
 ...
 *
 * All settings are presented as a number of named parameters divided into named
 * sections. The parameters values are stored as strings. Individual parameters
 * may be queried via subscript operators, like this:
 *
 * <code><pre>
 * optional< string > param = settings["Section1"]["Param1"]; // reads parameter
 * "Param1" in section "Section1"
 *                                                            // returns an
 * empty value if no such parameter exists settings["Section2"]["Param2"] = 10;
 * // sets the parameter "Param2" in section "Section2"
 *                                      // to value "10"
 * </pre></code>
 *
 ...
 */
template <typename CharT>
class basic_settings : public basic_settings_section<CharT>
{
  ...
  public:
    ...
    basic_settings(basic_settings const& that) :
        section(that.m_ptree ? new property_tree_type(*that.m_ptree)
                             : static_cast<property_tree_type*>(NULL))
    {
    }
    ...
    /*!
     * Initializing constructor. Creates a settings container with the copy of
     * the specified property tree.
     */
    explicit basic_settings(property_tree_type const& tree) :
        section(new property_tree_type(tree))
    {
    }
...
typedef basic_settings<char>
    settings; //!< Convenience typedef for narrow-character logging
typedef basic_settings_section<char>
    settings_section; //!< Convenience typedef for narrow-character logging
#endif
#ifdef BOOST_LOG_USE_WCHAR_T
typedef basic_settings<wchar_t>
    wsettings; //!< Convenience typedef for wide-character logging
typedef basic_settings_section<wchar_t>
    wsettings_section; //!< Convenience typedef for wide-character logging
#endif
