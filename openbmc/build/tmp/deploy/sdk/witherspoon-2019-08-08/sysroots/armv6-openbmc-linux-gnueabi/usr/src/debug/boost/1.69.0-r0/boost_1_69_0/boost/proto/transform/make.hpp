...
template <typename R, typename Expr, typename State,
          typename Data BOOST_PROTO_TEMPLATE_ARITY_PARAM(
...
struct make_if_<R, Expr, State, Data, true>
    : uncvref<
          typename when<_, R>::template impl<Expr, State, Data>::result_type>
{
    static bool const applied = true;
};
...
// work around GCC bug
template <typename Tag, typename Args, long N, typename Expr, typename State,
          typename Data>
...
///
/// <tt>boost::result_of\<make\<T\<X0,X1,...\> \>(Expr, State,
/// Data)\>::type</tt> is evaluated as follows. For each \c X in
/// <tt>X0,X1,...</tt>, do:
///
/// \li If \c X is a template like <tt>U\<Y0,Y1,...\></tt>, then let <tt>X'</tt>
///     be <tt>boost::result_of\<make\<U\<Y0,Y1,...\> \>(Expr, State,
///     Data)\>::type</tt> (which evaluates this procedure recursively). Note
///     whether any substitutions took place during this operation.
