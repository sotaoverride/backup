...
void avahi_interface_address_update_rrs(AvahiInterfaceAddress* a,
                                        int remove_rrs)
{
    ...
    if (m->list_complete && avahi_interface_address_is_relevant(a) &&
        avahi_interface_is_relevant(a->interface) && !remove_rrs &&
         ...
         m->server->state == AVAHI_SERVER_REGISTERING))
    {
        ...
        if (!a->entry_group)
            a->entry_group = avahi_s_entry_group_new(
                m->server, avahi_host_rr_entry_group_callback, NULL);
        ...
        if (avahi_s_entry_group_is_empty(a->entry_group))
        {
            ...
            p = (a->interface->protocol == AVAHI_PROTO_INET &&
                 m->server->config.publish_a_on_ipv6) ||
                        (a->interface->protocol == AVAHI_PROTO_INET6 &&
                         m->server->config.publish_aaaa_on_ipv4)
                    ? AVAHI_PROTO_UNSPEC
                    : a->interface->protocol;

            avahi_address_snprint(t, sizeof(t), &a->address);
            avahi_log_info("Registering new address record for %s on %s.%s.", t,
                           a->interface->hardware->name,
                           p == AVAHI_PROTO_UNSPEC ? "*"
                                                   : avahi_proto_to_string(p));

            if (avahi_server_add_address(m->server, a->entry_group,
                                         a->interface->hardware->index, p, 0,
                                         NULL, &a->address) < 0)
            {
                avahi_log_warn(__FILE__
                               ": avahi_server_add_address() failed: %s",
                               avahi_strerror(m->server->error));
    ...
    else
    {
        ...
        if (a->entry_group && !avahi_s_entry_group_is_empty(a->entry_group))
        {
            ...
            avahi_log_info("Withdrawing address record for %s on %s.", t,
                           a->interface->hardware->name);

            if (avahi_s_entry_group_get_state(a->entry_group) ==
...
void avahi_hw_interface_update_rrs(AvahiHwInterface* hw, int remove_rrs)
{
    ...
    if (m->list_complete && !remove_rrs &&
        m->server->config.publish_workstation &&
        (m->server->state == AVAHI_SERVER_RUNNING))
    {

        if (!hw->entry_group)
            hw->entry_group = avahi_s_entry_group_new(
                m->server, avahi_host_rr_entry_group_callback, NULL);
        ...
        if (avahi_s_entry_group_is_empty(hw->entry_group))
        {
            ...
            avahi_format_mac_address(mac, sizeof(mac), hw->mac_address,
                                     hw->mac_address_size);
            snprintf(name, sizeof(name), "%s [%s]", unescaped, mac);

            if (avahi_server_add_service(
                    m->server, hw->entry_group, hw->index, AVAHI_PROTO_UNSPEC,
                    0, name, "_workstation._tcp", NULL, NULL, 9, NULL) < 0)
            {
                avahi_log_warn(__FILE__
                               ": avahi_server_add_service() failed: %s",
                               avahi_strerror(m->server->error));
    ...
    else
    {
        ...
        if (hw->entry_group && !avahi_s_entry_group_is_empty(hw->entry_group))
        {
            ...
            if (avahi_s_entry_group_get_state(hw->entry_group) ==
                    AVAHI_ENTRY_GROUP_REGISTERING &&
                m->server->state == AVAHI_SERVER_REGISTERING)
                avahi_server_decrease_host_rr_pending(m->server);
...
void avahi_interface_monitor_update_rrs(AvahiInterfaceMonitor* m,
                                        int remove_rrs)
{
    AvahiHwInterface* hw;
...
static int interface_mdns_mcast_join(AvahiInterface* i, int join)
{
    ...
    avahi_log_info(
        "%s mDNS multicast group on interface %s.%s with address %s.",
        join ? "Joining" : "Leaving", i->hardware->name,
        avahi_proto_to_string(i->protocol),
        avahi_address_snprint(at, sizeof(at), &i->local_mcast_address));

    if (i->protocol == AVAHI_PROTO_INET6)
        r = avahi_mdns_mcast_join_ipv6(i->monitor->server->fd_ipv6,
                                       &i->local_mcast_address.data.ipv6,
                                       i->hardware->index, join);
    else
    {
        assert(i->protocol == AVAHI_PROTO_INET);

        r = avahi_mdns_mcast_join_ipv4(i->monitor->server->fd_ipv4,
                                       &i->local_mcast_address.data.ipv4,
                                       i->hardware->index, join);
    }
...
static int interface_mdns_mcast_rejoin(AvahiInterface* i)
{
    ...
    if (found && !usable)
        /* No action necessary: the address still exists and no better one has
         * been found */
        return 0;
...
void avahi_interface_address_free(AvahiInterfaceAddress* a)
{
    ...
    AVAHI_LLIST_REMOVE(AvahiInterfaceAddress, address, a->interface->addresses,
                       a);
...
void avahi_hw_interface_free(AvahiHwInterface* hw, int send_goodbye)
{
    ...
    AVAHI_LLIST_REMOVE(AvahiHwInterface, hardware, hw->monitor->hw_interfaces,
                       hw);
...
AvahiInterface* avahi_interface_new(AvahiInterfaceMonitor* m,
                                    AvahiHwInterface* hw,
                                    AvahiProtocol protocol)
{
    ...
    if (!i->cache || !i->response_scheduler || !i->query_scheduler ||
        !i->probe_scheduler)
        goto fail; /* OOM */
...
AvahiHwInterface* avahi_hw_interface_new(AvahiInterfaceMonitor* m,
                                         AvahiIfIndex idx)
{
    AvahiHwInterface* hw;
...
AvahiInterfaceAddress* avahi_interface_address_new(AvahiInterfaceMonitor* m,
                                                   AvahiInterface* i,
...
void avahi_interface_check_relevant(AvahiInterface* i)
{
    ...
    if (m->list_complete && b && !i->announcing)
    {
        ...
        if (i->mcast_joined)
        {
            avahi_log_info("New relevant interface %s.%s for mDNS.",
                           i->hardware->name,
                           avahi_proto_to_string(i->protocol));
            ...
            avahi_multicast_lookup_engine_new_interface(
                m->server->multicast_lookup_engine, i);
        }
    }
    else if (!b && i->announcing)
    {
        avahi_log_info("Interface %s.%s no longer relevant for mDNS.",
                       i->hardware->name, avahi_proto_to_string(i->protocol));
...
AvahiInterface* avahi_interface_monitor_get_interface(AvahiInterfaceMonitor* m,
                                                      AvahiIfIndex idx,
                                                      AvahiProtocol protocol)
{
    AvahiHwInterface* hw;
...
AvahiHwInterface*
    avahi_interface_monitor_get_hw_interface(AvahiInterfaceMonitor* m,
                                             AvahiIfIndex idx)
{
    assert(m);
    assert(idx >= 0);
...
AvahiInterfaceAddress* avahi_interface_monitor_get_address(
    AvahiInterfaceMonitor* m, AvahiInterface* i, const AvahiAddress* raddr)
{
...
void avahi_interface_send_packet_unicast(AvahiInterface* i, AvahiDnsPacket* p,
                                         const AvahiAddress* a, uint16_t port)
{
    ...
    if (i->monitor->server->config.ratelimit_interval > 0)
    {
        ...
        if (i->hardware->ratelimit_counter >
            i->monitor->server->config.ratelimit_burst)
            return;
    ...
    if (i->protocol == AVAHI_PROTO_INET && i->monitor->server->fd_ipv4 >= 0)
        avahi_send_dns_packet_ipv4(
            i->monitor->server->fd_ipv4, i->hardware->index, p,
            i->mcast_joined ? &i->local_mcast_address.data.ipv4 : NULL,
            a ? &a->data.ipv4 : NULL, port);
    else if (i->protocol == AVAHI_PROTO_INET6 &&
             i->monitor->server->fd_ipv6 >= 0)
        avahi_send_dns_packet_ipv6(
            i->monitor->server->fd_ipv6, i->hardware->index, p,
            i->mcast_joined ? &i->local_mcast_address.data.ipv6 : NULL,
            a ? &a->data.ipv6 : NULL, port);
}
...
int avahi_interface_post_query(AvahiInterface* i, AvahiKey* key,
                               int immediately, unsigned* ret_id)
{
    assert(i);
    assert(key);
...
int avahi_interface_post_response(AvahiInterface* i, AvahiRecord* record,
                                  int flush_cache, const AvahiAddress* querier,
                                  int immediately)
{
    assert(i);
    assert(record);
    ...
    return avahi_response_scheduler_post(i->response_scheduler, record,
                                         flush_cache, querier, immediately);
}

int avahi_interface_post_probe(AvahiInterface* i, AvahiRecord* record,
                               int immediately)
{
    assert(i);
    assert(record);
                      ...
                      void* userdata)
{
    ...
    for (i = m->interfaces; i; i = i->interface_next)
    {
        ...
        {
            char ln[256];
            snprintf(ln, sizeof(ln), ";;; INTERFACE %s.%s ;;;",
                     i->hardware->name, avahi_proto_to_string(i->protocol));
...
int avahi_interface_match(AvahiInterface* i, AvahiIfIndex idx,
                          AvahiProtocol protocol)
{
    assert(i);
...
void avahi_interface_monitor_walk(AvahiInterfaceMonitor* m,
                                  AvahiIfIndex interface,
                                  AvahiProtocol protocol,
                                  AvahiInterfaceMonitorWalkCallback callback,
                                  void* userdata)
{
    assert(m);
    assert(callback);
    ...
    if (interface != AVAHI_IF_UNSPEC)
    {
        if (protocol != AVAHI_PROTO_UNSPEC)
        {
            ...
            if ((i = avahi_interface_monitor_get_interface(m, interface,
                                                           protocol)))
...
int avahi_interface_address_on_link(AvahiInterface* i, const AvahiAddress* a)
{
    ...
    for (ia = i->addresses; ia; ia = ia->address_next)
    {
        ...
        if (a->proto == AVAHI_PROTO_INET)
        {
            ...
            if ((ntohl(a->data.ipv4.address) & m) ==
                (ntohl(ia->address.data.ipv4.address) & m))
                return 1;
        }
        else
        {
            ...
            for (j = 0; j < 16; j++)
            {
                ...
                if ((a->data.ipv6.address[j] & m) !=
                    (ia->address.data.ipv6.address[j] & m))
                    break;
            }
        }
    }
...
int avahi_interface_has_address(AvahiInterfaceMonitor* m, AvahiIfIndex iface,
...
AvahiIfIndex avahi_find_interface_for_address(AvahiInterfaceMonitor* m,
                                              const AvahiAddress* a)
{
    AvahiInterface* i;
    assert(m);
