    ...
    if (n <= RFB_BUF_SIZE)
    {
        ...
        while (client->buffered < n)
        {
            ...
            if (client->tlsSession)
                i = ReadFromTLS(client, client->buf + client->buffered,
                                RFB_BUF_SIZE - client->buffered);
            else
#ifdef LIBVNCSERVER_HAVE_SASL
                if (client->saslconn)
                i = ReadFromSASL(client, client->buf + client->buffered,
                                 RFB_BUF_SIZE - client->buffered);
            else
            {
#endif /* LIBVNCSERVER_HAVE_SASL */
                i = read(client->sock, client->buf + client->buffered,
                         RFB_BUF_SIZE - client->buffered);
#ifdef WIN32
                if (i < 0)
...
rfbBool WriteToRFBServer(rfbClient* client, char* buf, int n)
{
    ...
    if (client->tlsSession)
    {
        /* WriteToTLS() will guarantee either everything is written, or
         * error/eof returns */
...
int ListenAtTcpPortAndAddress(int port, const char* address)
{
    ...
    hints.ai_socktype = SOCK_STREAM;
    hints.ai_flags =
        AI_PASSIVE; /* fill in wildcard address if address == NULL */
    ...
    for (p = servinfo; p != NULL; p = p->ai_next)
    {
        ...
        if (p->ai_family == AF_INET6 &&
            setsockopt(sock, IPPROTO_IPV6, IPV6_V6ONLY, (char*)&one,
                       sizeof(one)) < 0)
        {
            rfbClientErr("ListenAtTcpPortAndAddress: error in setsockopt "
                         "IPV6_V6ONLY: %s\n",
                         strerror(errno));
            close(sock);
...
/*
 * SetDSCP sets a socket's IP QoS parameters aka Differentiated Services Code
 * Point field
 */
...
rfbBool SetDSCP(int sock, int dscp)
{
    ...
    {
        rfbClientErr(
            "Setting socket QoS failed while getting socket address: %s\n",
            strerror(errno));
        return FALSE;
    }
