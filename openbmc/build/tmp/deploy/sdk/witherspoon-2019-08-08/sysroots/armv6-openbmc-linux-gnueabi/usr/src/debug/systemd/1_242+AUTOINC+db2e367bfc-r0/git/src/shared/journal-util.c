...
static int access_check_var_log_journal(sd_journal* j, bool want_other_users)
{
    ...
    if (r < 0)
        return log_error_errno(
            r, "Failed to check if we are in the 'systemd-journal' group: %m");
    if (r > 0)
        return 0;
    ...
    if (!strv_isempty(g))
    {
        ...
        log_notice(
            ...
            want_other_users ? "other users and the system" : "the system", s);
        return 1;
    }
#endif
    ...
    log_notice("Hint: You are currently not seeing messages from %s.\n"
               "      Users in the 'systemd-journal' group can see all "
               "messages. Pass -q to\n"
                                  ...
                                  bool want_other_users)
{
    ...
    if (hashmap_contains(j->errors, INT_TO_PTR(-EACCES)))
    {
        ...
        if (ordered_hashmap_isempty(j->files))
            r = log_error_errno(EACCES, "No journal files were opened due to "
    ...
    HASHMAP_FOREACH_KEY(path, code, j->errors, it)
    {
        ...
        switch (err)
        {
            ...
            case ENODATA:
                log_warning_errno(
                    err, "Journal file %s is truncated, ignoring file.", path);
                break;

            case EPROTONOSUPPORT:
                log_warning_errno(err,
                                  "Journal file %1$s uses an unsupported "
                                  "feature, ignoring file.\n"
                                  "Use SYSTEMD_LOG_LEVEL=debug journalctl "
                                  "--file=%1$s to see the details.",
                                  path);
                break;

            case EBADMSG:
                log_warning_errno(
                    err, "Journal file %s corrupted, ignoring file.", path);
                break;

            default:
                log_warning_errno(
                    err,
                    "An error was encountered while opening journal file or "
                    "directory %s, ignoring file: %m",
                    path);
                break;
        }
    }
