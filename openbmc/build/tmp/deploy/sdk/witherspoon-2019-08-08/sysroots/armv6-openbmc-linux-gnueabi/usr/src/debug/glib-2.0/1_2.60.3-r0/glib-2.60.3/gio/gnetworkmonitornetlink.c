...
static void
    g_network_monitor_netlink_iface_init(GNetworkMonitorInterface* iface);
static void
...
static gboolean read_netlink_messages_callback(GSocket* socket,
                                               ...
                                               gpointer user_data);
static gboolean request_dump(GNetworkMonitorNetlink* nl, GError** error);

#define g_network_monitor_netlink_get_type _g_network_monitor_netlink_get_type
G_DEFINE_TYPE_WITH_CODE(
    GNetworkMonitorNetlink, g_network_monitor_netlink,
    ...
    g_io_extension_point_implement(G_NETWORK_MONITOR_EXTENSION_POINT_NAME,
                                   g_define_type_id, "netlink", 20))

static void g_network_monitor_netlink_init(GNetworkMonitorNetlink* nl)
{
    nl->priv = g_network_monitor_netlink_get_instance_private(nl);
}

static gboolean g_network_monitor_netlink_initable_init(
    GInitable* initable, GCancellable* cancellable, GError** error)
{
    ...
    if (!g_socket_set_option(nl->priv->sock, SOL_SOCKET, SO_PASSCRED, TRUE,
                             NULL))
    {
        int errsv = errno;
        g_set_error(error, G_IO_ERROR, g_io_error_from_errno(errsv),
    ...
    g_source_set_callback(nl->priv->source,
                          (GSourceFunc)read_netlink_messages_callback, nl,
                          NULL);
...
static gboolean request_dump(GNetworkMonitorNetlink* nl, GError** error)
{
    ...
    if (g_socket_send(nl->priv->sock, buf, sizeof(buf), NULL, error) < 0)
    {
...
static gboolean timeout_request_dump(gpointer user_data)
{
...
static void queue_request_dump(GNetworkMonitorNetlink* nl)
{
...
static GInetAddressMask* create_inet_address_mask(GSocketFamily family,
                                                  const guint8* dest,
                                                  gsize dest_len)
{
...
static void add_network(GNetworkMonitorNetlink* nl, GSocketFamily family,
                        const guint8* dest, gsize dest_len)
{
    GInetAddressMask* network =
        create_inet_address_mask(family, dest, dest_len);
...
static void remove_network(GNetworkMonitorNetlink* nl, GSocketFamily family,
                           const guint8* dest, gsize dest_len)
{
    GInetAddressMask* network =
        create_inet_address_mask(family, dest, dest_len);
    ...
    if (nl->priv->dump_networks)
    {
        GInetAddressMask** dump_networks =
    ...
    {
        g_network_monitor_base_remove_network(G_NETWORK_MONITOR_BASE(nl),
                                              network);
    }
...
static void finish_dump(GNetworkMonitorNetlink* nl)
{
    g_network_monitor_base_set_networks(
        G_NETWORK_MONITOR_BASE(nl),
        (GInetAddressMask**)nl->priv->dump_networks->pdata,
                                      ...
                                      GError** error)
{
    ...
    len = g_socket_receive_message(nl->priv->sock, NULL, &iv, 1, NULL, NULL,
                                   &flags, NULL, &local_error);
    if (len < 0)
    {
        retval = FALSE;
        goto done;
    ...
    iv.size = len;
    len = g_socket_receive_message(nl->priv->sock, &addr, &iv, 1, NULL, NULL,
    ...
    if (!g_socket_address_to_native(addr, &source_sockaddr,
                                    sizeof(source_sockaddr), &local_error))
    {
        retval = FALSE;
        goto done;
    }

    /* If the sender port id is 0 (not fakeable) then the message is from the
     * kernel */
    ...
    for (; len > 0; msg = NLMSG_NEXT(msg, len))
    {
        ...
        {
            g_set_error_literal(
                &local_error, G_IO_ERROR, G_IO_ERROR_PARTIAL_INPUT,
...
#define UNALIGNED_IN6_IS_ADDR_MC_LINKLOCAL(a)                                  \
    ((a[0] == 0xff) && ((a[1] & 0xf) == 0x2))
                    ...
                    if (msg->nlmsg_type == RTM_NEWROUTE)
                        add_network(nl, rtmsg->rtm_family, dest,
                                    rtmsg->rtm_dst_len);
                    else
                        remove_network(nl, rtmsg->rtm_family, dest,
                                       rtmsg->rtm_dst_len);
            ...
            case NLMSG_ERROR:
            {
                ...
                g_set_error(&local_error, G_IO_ERROR,
                            ...
                            "netlink error: %s", g_strerror(-e->error));
            }
                retval = FALSE;
                goto done;
            ...
            default:
                g_set_error(&local_error, G_IO_ERROR, G_IO_ERROR_INVALID_DATA,
                            "unexpected netlink message %d", msg->nlmsg_type);
                retval = FALSE;
                goto done;
        }
    }
    ...
    if (local_error)
        g_propagate_prefixed_error(error, local_error,
                                   "Error on netlink socket: ");

    return retval;
}

static void g_network_monitor_netlink_finalize(GObject* object)
{
...
static void g_network_monitor_netlink_iface_init(
