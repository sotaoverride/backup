 ...
 */
static krb5_error_code krb5_ccache_size(krb5_context, krb5_pointer, size_t*);
static krb5_error_code krb5_ccache_externalize(krb5_context, krb5_pointer,
                                               krb5_octet**, size_t*);
static krb5_error_code krb5_ccache_internalize(krb5_context, krb5_pointer*,
                                               krb5_octet**, size_t*);
 ...
 */
static krb5_error_code krb5_ccache_size(krb5_context kcontext, krb5_pointer arg,
                                        size_t* sizep)
{
 ...
 */
static krb5_error_code krb5_ccache_externalize(krb5_context kcontext,
                                               krb5_pointer arg,
                                               krb5_octet** buffer,
                                               size_t* lenremain)
{
    ...
    {
        kret = ENOMEM;
        if (!krb5_ccache_size(kcontext, arg, &required) && (required <= remain))
        {
            /* Our identifier */
            ...
            if (ccname)
            {
                ...
                (void)krb5_ser_pack_int32((krb5_int32)strlen(ccname), &bp,
                                          &remain);

                /* Put the name */
                (void)krb5_ser_pack_bytes((krb5_octet*)ccname, strlen(ccname),
                                          &bp, &remain);
 ...
 */
static krb5_error_code krb5_ccache_internalize(krb5_context kcontext,
                                               krb5_pointer* argp,
                                               krb5_octet** buffer,
                                               size_t* lenremain)
{
    ...
    *argp = NULL;
    ...
    if (!ccname)
        return ENOMEM;
    kret =
        krb5_ser_unpack_bytes((krb5_octet*)ccname, (size_t)ibuf, &bp, &remain);
    if (kret)
        goto cleanup;
    ccname[ibuf] = '\0';
 ...
 */
krb5_error_code KRB5_CALLCONV krb5_ser_ccache_init(krb5_context kcontext)
{
