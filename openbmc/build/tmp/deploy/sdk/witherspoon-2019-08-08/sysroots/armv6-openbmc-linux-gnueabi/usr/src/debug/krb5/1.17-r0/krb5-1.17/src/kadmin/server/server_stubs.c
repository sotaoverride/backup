...
static int gss_to_krb5_name(kadm5_server_handle_t handle, gss_name_t gss_name,
...
static int cmp_gss_names_rel_1(gss_name_t n1, gss_name_t n2)
{
    ...
    if (n1)
        (void)gss_release_name(&min_stat, &n1);
    return ret;
}
                                     ...
                                     kadm5_server_handle_t* out_handle)
{
    ...
    *handle = *(kadm5_server_handle_t)global_server_handle;
    ...
    if (!gss_to_krb5_name(handle, rqst2name(rqstp), &handle->current_caller))
    {
        free(handle);
 ...
 */
int setup_gss_names(struct svc_req* rqstp, gss_buffer_desc* client_name,
                    gss_buffer_desc* server_name)
{
    ...
    if (gss_name_to_string(rqst2name(rqstp), client_name) != 0)
        return -1;
    maj_stat =
        gss_inquire_context(&min_stat, rqstp->rq_svccred, NULL,
                            &server_gss_name, NULL, NULL, NULL, NULL, NULL);
...
static gss_name_t acceptor_name(gss_ctx_id_t context)
{
    ...
    maj_stat = gss_inquire_context(&min_stat, context, NULL, &name, NULL, NULL,
                                   NULL, NULL, NULL);
    if (maj_stat != GSS_S_COMPLETE)
        return NULL;
    return name;
 ...
 */
static kadm5_ret_t
    stub_setup(krb5_ui_4 api_version, struct svc_req* rqstp,
               krb5_principal princ, kadm5_server_handle_t* handle_out,
               krb5_ui_4* api_version_out, gss_buffer_t client_name_out,
               gss_buffer_t service_name_out, char** princ_str_out)
{
    kadm5_ret_t ret;
...
static void stub_cleanup(kadm5_server_handle_t handle, char* princ_str,
...
static krb5_boolean stub_auth(kadm5_server_handle_t handle, int opcode,
                              krb5_const_principal p1, krb5_const_principal p2,
                              const char* s1, const char* s2)
{
    return auth(handle->context, opcode, handle->current_caller, p1, p2, s1, s2,
                NULL, 0);
}

static krb5_boolean stub_auth_pol(kadm5_server_handle_t handle, int opcode,
                                  const char* policy,
...
static krb5_boolean stub_auth_restrict(kadm5_server_handle_t handle, int opcode,
                                       kadm5_principal_ent_t ent, long* mask)
{
    return auth_restrict(handle->context, opcode, handle->current_caller, ent,
                         mask);
}

/* Return true if the client authenticated to kadmin/changepw and princ is not
 * the client principal. */
static krb5_boolean changepw_not_self(kadm5_server_handle_t handle,
                                      struct svc_req* rqstp,
...
static krb5_boolean ticket_is_initial(struct svc_req* rqstp)
{
    OM_uint32 status, minor_stat;
    krb5_flags flags;
...
static kadm5_ret_t check_self_keychange(kadm5_server_handle_t handle,
                                        struct svc_req* rqstp,
                                        krb5_principal princ)
{
...
static int log_unauth(char* op, char* target, gss_buffer_t client,
                      gss_buffer_t server, struct svc_req* rqstp)
{
    ...
    return krb5_klog_syslog(
        LOG_NOTICE,
        _("Unauthorized request: %s, %.*s%s, "
          "client=%.*s%s, service=%.*s%s, addr=%s"),
        op, (int)tlen, target, tdots, (int)clen, (char*)client->value, cdots,
        (int)slen, (char*)server->value, sdots, client_addr(rqstp->rq_xprt));
}

static int log_done(char* op, char* target, const char* errmsg,
                    gss_buffer_t client, gss_buffer_t server,
                    struct svc_req* rqstp)
{
    size_t tlen, clen, slen;
    char *tdots, *cdots, *sdots;
    ...
    return krb5_klog_syslog(LOG_NOTICE,
                            _("Request: %s, %.*s%s, %s, "
                              "client=%.*s%s, service=%.*s%s, addr=%s"),
                            op, (int)tlen, target, tdots, errmsg, (int)clen,
                            (char*)client->value, cdots, (int)slen,
                            (char*)server->value, sdots,
                            client_addr(rqstp->rq_xprt));
}

bool_t create_principal_2_svc(cprinc_arg* arg, generic_ret* ret,
                              struct svc_req* rqstp)
{
    char* prime_arg = NULL;
    ...
    ret->code =
        stub_setup(arg->api_version, rqstp, arg->rec.principal, &handle,
                   &ret->api_version, &client_name, &service_name, &prime_arg);
    if (ret->code)
        goto exit_func;
        ...
        !stub_auth_restrict(handle, OP_ADDPRINC, &arg->rec, &arg->mask))
    {
        ...
        log_unauth("kadm5_create_principal", prime_arg, &client_name,
                   &service_name, rqstp);
    }
    else
    {
        ret->code =
            kadm5_create_principal(handle, &arg->rec, arg->mask, arg->passwd);

        if (ret->code != 0)
            errmsg = krb5_get_error_message(handle->context, ret->code);

        log_done("kadm5_create_principal", prime_arg, errmsg, &client_name,
                 &service_name, rqstp);
...
bool_t create_principal3_2_svc(cprinc3_arg* arg, generic_ret* ret,
                               struct svc_req* rqstp)
{
    char* prime_arg = NULL;
    gss_buffer_desc client_name = GSS_C_EMPTY_BUFFER;
...
static kadm5_ret_t check_lockdown_keys(kadm5_server_handle_t handle,
                                       krb5_principal princ)
{
...
bool_t delete_principal_2_svc(dprinc_arg* arg, generic_ret* ret,
                              struct svc_req* rqstp)
{
    char* prime_arg = NULL;
    gss_buffer_desc client_name = GSS_C_EMPTY_BUFFER;
    ...
    ret->code =
        stub_setup(arg->api_version, rqstp, arg->princ, &handle,
                   &ret->api_version, &client_name, &service_name, &prime_arg);
    if (ret->code)
        goto exit_func;
    ...
    if (ret->code != KADM5_AUTH_DELETE)
    {
        ...
        log_done("kadm5_delete_principal", prime_arg, errmsg, &client_name,
                 &service_name, rqstp);
...
bool_t modify_principal_2_svc(mprinc_arg* arg, generic_ret* ret,
                              struct svc_req* rqstp)
{
    char* prime_arg = NULL;
    gss_buffer_desc client_name = GSS_C_EMPTY_BUFFER;
    ...
    if (ret->code == KADM5_OK)
    {
        ...
        log_done("kadm5_modify_principal", prime_arg, errmsg, &client_name,
                 &service_name, rqstp);
...
bool_t rename_principal_2_svc(rprinc_arg* arg, generic_ret* ret,
                              struct svc_req* rqstp)
{
    ...
    ret->code =
        stub_setup(arg->api_version, rqstp, NULL, &handle, &ret->api_version,
    ...
    if (ret->code != KADM5_OK)
    {
        ...
        krb5_klog_syslog(LOG_NOTICE,
                         _("Unauthorized request: kadm5_rename_principal, "
                           "%.*s%s to %.*s%s, "
                           "client=%.*s%s, service=%.*s%s, addr=%s"),
                         (int)tlen1, prime_arg1, tdots1, (int)tlen2, prime_arg2,
                         tdots2, (int)clen, (char*)client_name.value, cdots,
    ...
    else
    {
        ...
        krb5_klog_syslog(LOG_NOTICE,
                         _("Request: kadm5_rename_principal, "
                           ...
                           "client=%.*s%s, service=%.*s%s, addr=%s"),
                         (int)tlen1, prime_arg1, tdots1, (int)tlen2, prime_arg2,
                         tdots2, errmsg ? errmsg : _("success"), (int)clen,
                         (char*)client_name.value, cdots, (int)slen,
                         (char*)service_name.value, sdots,
                         client_addr(rqstp->rq_xprt));
...
bool_t get_principal_2_svc(gprinc_arg* arg, gprinc_ret* ret,
                           struct svc_req* rqstp)
{
    ...
    {
        ret->code =
            kadm5_get_principal(handle, arg->princ, &ret->rec, arg->mask);

        if (ret->code != 0)
            errmsg = krb5_get_error_message(handle->context, ret->code);

        log_done(funcname, prime_arg, errmsg, &client_name, &service_name,
                 rqstp);
...
bool_t get_princs_2_svc(gprincs_arg* arg, gprincs_ret* ret,
                        struct svc_req* rqstp)
{
    char* prime_arg = NULL;
    gss_buffer_desc client_name = GSS_C_EMPTY_BUFFER;
    ...
    {
        ret->code =
            kadm5_get_principals(handle, arg->exp, &ret->princs, &ret->count);
        if (ret->code != 0)
            errmsg = krb5_get_error_message(handle->context, ret->code);

        log_done("kadm5_get_principals", prime_arg, errmsg, &client_name,
                 &service_name, rqstp);
...
bool_t chpass_principal_2_svc(chpass_arg* arg, generic_ret* ret,
                              struct svc_req* rqstp)
{
    char* prime_arg = NULL;
    gss_buffer_desc client_name = GSS_C_EMPTY_BUFFER;
    ...
    if (ret->code != KADM5_AUTH_CHANGEPW)
    {
        ...
        log_done("kadm5_chpass_principal", prime_arg, errmsg, &client_name,
                 &service_name, rqstp);
...
bool_t chpass_principal3_2_svc(chpass3_arg* arg, generic_ret* ret,
                               struct svc_req* rqstp)
{
    char* prime_arg = NULL;
    gss_buffer_desc client_name = GSS_C_EMPTY_BUFFER;
...
bool_t setv4key_principal_2_svc(setv4key_arg* arg, generic_ret* ret,
                                struct svc_req* rqstp)
{
    char* prime_arg = NULL;
    gss_buffer_desc client_name = GSS_C_EMPTY_BUFFER;
    ...
    {
        ret->code = kadm5_setv4key_principal(handle, arg->princ, arg->keyblock);
    }
    else
    {
        log_unauth("kadm5_setv4key_principal", prime_arg, &client_name,
    ...
    if (ret->code != KADM5_AUTH_SETKEY)
    {
        ...
        log_done("kadm5_setv4key_principal", prime_arg, errmsg, &client_name,
                 &service_name, rqstp);
...
bool_t setkey_principal_2_svc(setkey_arg* arg, generic_ret* ret,
                              struct svc_req* rqstp)
{
    char* prime_arg = NULL;
    gss_buffer_desc client_name = GSS_C_EMPTY_BUFFER;
...
bool_t setkey_principal3_2_svc(setkey3_arg* arg, generic_ret* ret,
                               struct svc_req* rqstp)
{
    char* prime_arg = NULL;
    gss_buffer_desc client_name = GSS_C_EMPTY_BUFFER;
...
bool_t setkey_principal4_2_svc(setkey4_arg* arg, generic_ret* ret,
                               struct svc_req* rqstp)
{
    char* prime_arg = NULL;
    gss_buffer_desc client_name = GSS_C_EMPTY_BUFFER;
...
static kadm5_ret_t chrand_check_lockdown(kadm5_server_handle_t handle,
                                         krb5_principal princ,
...
bool_t chrand_principal_2_svc(chrand_arg* arg, chrand_ret* ret,
                              struct svc_req* rqstp)
{
    char *funcname, *prime_arg = NULL;
    gss_buffer_desc client_name = GSS_C_EMPTY_BUFFER;
...
bool_t chrand_principal3_2_svc(chrand3_arg* arg, chrand_ret* ret,
                               struct svc_req* rqstp)
{
    char *funcname, *prime_arg = NULL;
    gss_buffer_desc client_name = GSS_C_EMPTY_BUFFER;
...
bool_t create_policy_2_svc(cpol_arg* arg, generic_ret* ret,
                           struct svc_req* rqstp)
{
    char* prime_arg = NULL;
    gss_buffer_desc client_name = GSS_C_EMPTY_BUFFER;
    ...
    if (CHANGEPW_SERVICE(rqstp) ||
        !stub_auth_pol(handle, OP_ADDPOL, arg->rec.policy, &arg->rec,
                       arg->mask))
    {
        ret->code = KADM5_AUTH_ADD;
        log_unauth("kadm5_create_policy", prime_arg, &client_name,
                   &service_name, rqstp);
    }
    else
    {
...
bool_t delete_policy_2_svc(dpol_arg* arg, generic_ret* ret,
                           struct svc_req* rqstp)
{
    char* prime_arg = NULL;
    gss_buffer_desc client_name = GSS_C_EMPTY_BUFFER;
    ...
    {
        log_unauth("kadm5_delete_policy", prime_arg, &client_name,
                   &service_name, rqstp);
        ret->code = KADM5_AUTH_DELETE;
    }
    else
...
bool_t modify_policy_2_svc(mpol_arg* arg, generic_ret* ret,
                           struct svc_req* rqstp)
{
    char* prime_arg = NULL;
    gss_buffer_desc client_name = GSS_C_EMPTY_BUFFER;
    ...
    if (CHANGEPW_SERVICE(rqstp) ||
        !stub_auth_pol(handle, OP_MODPOL, arg->rec.policy, &arg->rec,
                       arg->mask))
    {
        log_unauth("kadm5_modify_policy", prime_arg, &client_name,
                   &service_name, rqstp);
        ret->code = KADM5_AUTH_MODIFY;
    }
    else
...
bool_t get_policy_2_svc(gpol_arg* arg, gpol_ret* ret, struct svc_req* rqstp)
{
    char *funcname, *prime_arg = NULL;
    gss_buffer_desc client_name = GSS_C_EMPTY_BUFFER;
    gss_buffer_desc service_name = GSS_C_EMPTY_BUFFER;
    ...
    else
    {
        ...
        log_done(funcname, ((prime_arg == NULL) ? "(null)" : prime_arg), errmsg,
                 &client_name, &service_name, rqstp);
        if (errmsg != NULL)
            krb5_free_error_message(handle->context, errmsg);
    }
...
bool_t get_pols_2_svc(gpols_arg* arg, gpols_ret* ret, struct svc_req* rqstp)
{
    char* prime_arg = NULL;
    gss_buffer_desc client_name = GSS_C_EMPTY_BUFFER;
    gss_buffer_desc service_name = GSS_C_EMPTY_BUFFER;
    ...
    {
        ret->code = KADM5_AUTH_LIST;
        log_unauth("kadm5_get_policies", prime_arg, &client_name, &service_name,
                   rqstp);
    }
    else
    {
        ret->code =
            kadm5_get_policies(handle, arg->exp, &ret->pols, &ret->count);
        if (ret->code != 0)
            errmsg = krb5_get_error_message(handle->context, ret->code);

        log_done("kadm5_get_policies", prime_arg, errmsg, &client_name,
                 &service_name, rqstp);
...
bool_t get_privs_2_svc(krb5_ui_4* arg, getprivs_ret* ret, struct svc_req* rqstp)
{
    gss_buffer_desc client_name = GSS_C_EMPTY_BUFFER;
    gss_buffer_desc service_name = GSS_C_EMPTY_BUFFER;
    kadm5_server_handle_t handle;
    ...
    log_done("kadm5_get_privs", client_name.value, errmsg, &client_name,
             &service_name, rqstp);
...
bool_t purgekeys_2_svc(purgekeys_arg* arg, generic_ret* ret,
                       struct svc_req* rqstp)
{
    char *funcname, *prime_arg = NULL;
    gss_buffer_desc client_name = GSS_C_EMPTY_BUFFER;
...
bool_t get_strings_2_svc(gstrings_arg* arg, gstrings_ret* ret,
                         struct svc_req* rqstp)
{
    char* prime_arg = NULL;
    gss_buffer_desc client_name = GSS_C_EMPTY_BUFFER;
    ...
    {
        ret->code = KADM5_AUTH_GET;
        log_unauth("kadm5_get_strings", prime_arg, &client_name, &service_name,
                   rqstp);
    }
    else
    {
        ret->code =
            kadm5_get_strings(handle, arg->princ, &ret->strings, &ret->count);
        if (ret->code != 0)
            errmsg = krb5_get_error_message(handle->context, ret->code);

        log_done("kadm5_get_strings", prime_arg, errmsg, &client_name,
                 &service_name, rqstp);
...
bool_t set_string_2_svc(sstring_arg* arg, generic_ret* ret,
                        struct svc_req* rqstp)
{
    char* prime_arg = NULL;
    gss_buffer_desc client_name = GSS_C_EMPTY_BUFFER;
    ...
    if (CHANGEPW_SERVICE(rqstp) ||
        !stub_auth(handle, OP_SETSTR, arg->princ, NULL, arg->key, arg->value))
    {
        ret->code = KADM5_AUTH_MODIFY;
        log_unauth("kadm5_mod_strings", prime_arg, &client_name, &service_name,
                   rqstp);
    }
    else
    {
        ...
        log_done("kadm5_mod_strings", prime_arg, errmsg, &client_name,
                 &service_name, rqstp);
...
bool_t init_2_svc(krb5_ui_4* arg, generic_ret* ret, struct svc_req* rqstp)
{
    gss_buffer_desc client_name = GSS_C_EMPTY_BUFFER;
    gss_buffer_desc service_name = GSS_C_EMPTY_BUFFER;
    kadm5_server_handle_t handle;
    ...
    krb5_klog_syslog(
        LOG_NOTICE,
        _("Request: kadm5_init, %.*s%s, %s, "
          "client=%.*s%s, service=%.*s%s, addr=%s, "
          "vers=%d, flavor=%d"),
        (int)clen, (char*)client_name.value, cdots,
        errmsg ? errmsg : _("success"), (int)clen, (char*)client_name.value,
        cdots, (int)slen, (char*)service_name.value, sdots,
        client_addr(rqstp->rq_xprt),
        ret->api_version & ~(KADM5_API_VERSION_MASK), rqstp->rq_cred.oa_flavor);
    if (errmsg != NULL)
        krb5_free_error_message(handle->context, errmsg);
...
bool_t get_principal_keys_2_svc(getpkeys_arg* arg, getpkeys_ret* ret,
                                struct svc_req* rqstp)
{
    char* prime_arg = NULL;
    gss_buffer_desc client_name = GSS_C_EMPTY_BUFFER;
    ...
    {
        log_unauth("kadm5_get_principal_keys", prime_arg, &client_name,
                   &service_name, rqstp);
    ...
    if (ret->code != KADM5_AUTH_EXTRACT)
    {
        ...
        log_done("kadm5_get_principal_keys", prime_arg, errmsg, &client_name,
                 &service_name, rqstp);
