...
void do_get_xattr(int argc, char** argv)
{
    ...
    while ((i = getopt(argc, argv, "Cf:rxV")) != -1)
    {
        switch (i)
        {
            ...
            case 'x':
                ...
                break;
            case 'V':
                print_flags |= PRINT_XATTR_RAW | PRINT_XATTR_NOQUOTES;
                break;
            case 'C':
    ...
    else
    {
        ...
        {
            print_xattr(stdout, argv[optind + 1], buf, buflen, print_flags);
        }
        printf("\n");
    }
                                   ...
                                   unsigned value_start)
{
    ...
    while (off < len)
    {
        ...
        fprintf(f,
                ...
                "value_size = %u\n",
                ent.e_value_offs, vstart, ent.e_value_inum, ent.e_value_size);
        ...
        else if (ent.e_value_offs >= len || (vstart + ent.e_value_size) > len)
            fprintf(f, "<runs off end>");
        if (is_mostly_printable((char*)(buf + vstart), ent.e_value_size))
            safe_print(f, (char*)(buf + vstart), ent.e_value_size);
        else
        {
            fprintf(f, "<hexdump>\n");
...
void block_xattr_dump(FILE* f, unsigned char* buf, unsigned int len)
{
    ...
    ext2fs_swap_ext_attr_header(&header, (struct ext2_ext_attr_header*)buf);
#else
    ...
    dump_xattr_raw_entries(f, buf, sizeof(struct ext2_ext_attr_header), len, 0);
}
