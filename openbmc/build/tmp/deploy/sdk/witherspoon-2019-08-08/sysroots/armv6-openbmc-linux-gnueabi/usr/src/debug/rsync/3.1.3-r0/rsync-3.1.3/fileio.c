...
static int write_sparse(int f, int use_seek, OFF_T offset, const char* buf,
                        int len)
{
    int l1 = 0, l2 = 0;
    int ret;
...
int skip_matched(int fd, OFF_T offset, const char* buf, int len)
{
    ...
    {
        rsyserr(FERROR_XFER, errno, "lseek returned %s, not %s", big_num(pos),
                big_num(offset));
        return -1;
    }
...
char* map_ptr(struct map_struct* map, OFF_T offset, int32 len)
{
    ...
    if (map->p_fd_offset != read_start)
    {
        ...
        if (ret != read_start)
        {
            rsyserr(FERROR, errno, "lseek returned %s, not %s", big_num(ret),
                    big_num(read_start));
            exit_cleanup(RERR_FILEIO);
        }
