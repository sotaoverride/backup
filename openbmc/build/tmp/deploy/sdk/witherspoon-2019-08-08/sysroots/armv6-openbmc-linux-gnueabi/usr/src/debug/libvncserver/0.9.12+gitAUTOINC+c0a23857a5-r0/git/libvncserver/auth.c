...
static rfbSecurityHandler VncSecurityHandlerVncAuth = {
    rfbSecTypeVncAuth, rfbVncAuthSendChallenge, NULL};

static rfbSecurityHandler VncSecurityHandlerNone = {rfbSecTypeNone,
                                                    rfbVncAuthNone, NULL};
...
#define MAX_SECURITY_TYPES 255
    ...
    /* Fill in the list of security types in the client structure. (NOTE: Not
     * really in the client structure) */
    switch (primaryType)
    {
        case rfbSecTypeNone:
    ...
    for (handler = securityHandlers; handler && size < MAX_SECURITY_TYPES;
 ...
 */
static void rfbSendSecurityType(rfbClientPtr cl, int32_t securityType)
{
    uint32_t value32;
...
void rfbAuthProcessClientMessage(rfbClientPtr cl)
{
    ...
    if (!cl->screen->passwordCheck(cl, (const char*)response, CHALLENGESIZE))
    {
        ...
        /* support RFB 3.8 clients, they expect a reason *why* it was
         * disconnected */
