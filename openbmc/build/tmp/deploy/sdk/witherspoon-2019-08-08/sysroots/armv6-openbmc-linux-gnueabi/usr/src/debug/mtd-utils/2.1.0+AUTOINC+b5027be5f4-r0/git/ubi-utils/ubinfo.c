...
static const char doc[] =
    PROGRAM_NAME " version " VERSION " - a tool to print UBI information.";

static const char optionsstr[] =
    "-d, --devn=<UBI device number>  UBI device number to get information "
    "about\n"
    "-n, --vol_id=<volume ID>        ID of UBI volume to print information "
    "about\n"
    "-N, --name=<volume name>        name of UBI volume to print information "
    "about\n"
    "-a, --all                       print information about all devices and "
    "volumes,\n"
    "                                or about all volumes if the UBI device "
    "was\n"
    "                                specified\n"
...
static const char usage[] =
    "Usage 1: " PROGRAM_NAME " [-d <UBI device number>] [-n <volume ID> | -N "
                             "<volume name>] [-a] [-h] [-V]\n"
    "\t\t[--vol_id=<volume ID> | --name <volume name>] [--devn <UBI device "
    "number>] [--all] [--help] [--version]\n"
    "Usage 2: " PROGRAM_NAME
    " <UBI device node file name> [-a] [-h] [-V] [--all] [--help] [--version]\n"
    "Usage 3: " PROGRAM_NAME
    " <UBI volume node file name> [-h] [-V] [--help] [--version]\n\n"
    "Example 1: " PROGRAM_NAME
    " - (no arguments) print general UBI information\n"
    "Example 2: " PROGRAM_NAME
    " -d 1 - print information about UBI device number 1\n"
    "Example 3: " PROGRAM_NAME
    " /dev/ubi0 -a - print information about all volumes of UBI\n"
    "           device /dev/ubi0\n"
    "Example 4: " PROGRAM_NAME
    " /dev/ubi1_0 - print information about UBI volume /dev/ubi1_0\n"
...
static int translate_dev(libubi_t libubi, const char* node)
{
    ...
    if (err == -1)
    {
        ...
        return errmsg("\"%s\" does not correspond to any UBI device or volume",
                      node);
    }
    ...
    if (err == 1)
    {
        ...
        if (err)
            return sys_errmsg("cannot get information about UBI device \"%s\"",
                              node);
    ...
    else
    {
        ...
        if (err)
            return sys_errmsg("cannot get information about UBI volume \"%s\"",
                              node);
...
static int get_vol_id_by_name(libubi_t libubi, int dev_num, const char* name)
{
    ...
    if (err)
        return sys_errmsg(
            "cannot get information about volume \"%s\" on ubi%d\n", name,
            dev_num);
...
static int print_dev_info(libubi_t libubi, int dev_num, int all)
{
    ...
    if (err)
        return sys_errmsg("cannot get information about UBI device %d",
                          dev_num);
    ...
    printf("Total amount of logical eraseblocks:     %d (",
           dev_info.total_lebs);
    util_print_bytes(dev_info.total_bytes, 0);
    printf(")\n");

    printf("Amount of available logical eraseblocks: %d (",
           dev_info.avail_lebs);
    ...
    for (i = dev_info.lowest_vol_id; i <= dev_info.highest_vol_id; i++)
    {
        ...
        if (err == -1)
        {
            ...
            return sys_errmsg("libubi failed to probe volume %d on ubi%d", i,
                              dev_info.dev_num);
        }
...
static int print_general_info(libubi_t libubi, int all)
{
    ...
    for (i = ubi_info.lowest_dev_num; i <= ubi_info.highest_dev_num; i++)
    {
