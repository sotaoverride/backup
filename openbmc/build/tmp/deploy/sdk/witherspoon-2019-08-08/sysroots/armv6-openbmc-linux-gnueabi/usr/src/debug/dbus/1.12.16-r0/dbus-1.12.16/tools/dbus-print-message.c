...
static void print_fd(int fd, int depth)
{
    ...
    switch (addr.sa.sa_family)
    {
        ...
        case AF_INET:
            ...
            if (has_peer &&
                inet_ntop(AF_INET, &peer.ipv4.sin_addr, hostip, sizeof(hostip)))
            {
                indent(depth + 1);
        ...
        case AF_INET6:
            printf("inet6\n");
            if (inet_ntop(AF_INET6, &addr.ipv6.sin6_addr, hostip,
                          sizeof(hostip)))
            {
                indent(depth + 1);
                printf("name %s port %u\n", hostip, ntohs(addr.ipv6.sin6_port));
            }
            if (has_peer && inet_ntop(AF_INET6, &peer.ipv6.sin6_addr, hostip,
                                      sizeof(hostip)))
            {
                indent(depth + 1);
    ...
    do
    {
        ...
        switch (type)
        {
            ...
            case DBUS_TYPE_STRUCT:
            {
                ...
                printf("struct {\n");
                while ((current_type = dbus_message_iter_get_arg_type(
...
void print_message(DBusMessage* message, dbus_bool_t literal, long sec,
                   long usec)
{
    ...
    if (!literal)
    {
        ...
        {
            printf("%s sender=%s -> destination=%s", type_to_name(message_type),
                   sender ? sender : "(null sender)",
                   destination ? destination : "(null destination)");
        }
