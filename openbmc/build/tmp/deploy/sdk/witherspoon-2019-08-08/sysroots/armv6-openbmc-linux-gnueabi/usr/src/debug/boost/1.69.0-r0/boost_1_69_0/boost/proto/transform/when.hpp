...
struct when_impl : transform<when<Grammar, Fun>>
{
    ...
    struct impl : transform_impl<Expr, State, Data>
    {
        ...
        typedef
            typename mpl::if_c<is_callable<R>::value,
    ...
    : proto::transform<when<Grammar, external_transform>>
{
    ...
    struct impl
        : remove_reference<typename mpl::eval_if_c<
              ...
              proto::result_of::env_var<Data, data_type>>::type>::type::
              template when<Grammar>::template impl<Expr, State, Data>
    {
    };
};

/// \brief For defining a map of Rule/Transform pairs for use with
/// <tt>when\<T, external_transform\></tt> to make transforms external to the
/// grammar
///
...
/// passes that transforms map at the Data parameter to the grammar. In this
/// way, the behavior of the grammar can be modified post-hoc by passing a
/// different action_map.
///
/// \code
/// struct int_terminal
///   : proto::terminal<int>
...
/// \endcode
template <BOOST_PP_ENUM_PARAMS_WITH_A_DEFAULT(BOOST_MPL_LIMIT_MAP_SIZE,
                                              typename T, mpl::na)>
struct external_transforms
{
    typedef mpl::map<BOOST_PP_ENUM_PARAMS(BOOST_MPL_LIMIT_MAP_SIZE, T)>
        map_type;
