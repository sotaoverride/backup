...
static const FT_UInt32 ft_extra_glyph_unicodes[EXTRA_GLYPH_LIST_SIZE] = {
    /* WGL 4 */
    0x0394, 0x03A9, 0x2215, 0x00AD, 0x02C9, 0x03BC, 0x2219, 0x00A0,
    /* Romanian */
    0x021A, 0x021B};

static const char ft_extra_glyph_names[] = {
    'D', 'e', 'l', 't', 'a', 0,   'O', 'm', 'e', 'g', 'a', 0,   'f', 'r', 'a',
    'c', 't', 'i', 'o', 'n', 0,   'h', 'y', 'p', 'h', 'e', 'n', 0,   'm', 'a',
    'c', 'r', 'o', 'n', 0,   'm', 'u', 0,   'p', 'e', 'r', 'i', 'o', 'd', 'c',
    'e', 'n', 't', 'e', 'r', 'e', 'd', 0,   's', 'p', 'a', 'c', 'e', 0,   'T',
    'c', 'o', 'm', 'm', 'a', 'a', 'c', 'c', 'e', 'n', 't', 0,   't', 'c', 'o',
    'm', 'm', 'a', 'a', 'c', 'c', 'e', 'n', 't', 0};

static const FT_Int ft_extra_glyph_name_offsets[EXTRA_GLYPH_LIST_SIZE] = {
    0, 6, 12, 21, 28, 35, 38, 53, 59, 72};

static void ps_check_extra_glyph_name(const char* gname, FT_UInt glyph,
                                      FT_UInt* extra_glyphs, FT_UInt* states)
{
    FT_UInt n;

    for (n = 0; n < EXTRA_GLYPH_LIST_SIZE; n++)
    {
        if (ft_strcmp(ft_extra_glyph_names + ft_extra_glyph_name_offsets[n],
                      gname) == 0)
        {
            if (states[n] == 0)
            {
...
static void ps_check_extra_glyph_unicode(FT_UInt32 uni_char, FT_UInt* states)
{
    FT_UInt n;
...
static FT_Error ps_unicodes_init(FT_Memory memory, PS_Unicodes table,
                                 FT_UInt num_glyphs,
                                 ...
                                 FT_Pointer glyph_data)
{
    ...
    if (!FT_NEW_ARRAY(table->maps, num_glyphs + EXTRA_GLYPH_LIST_SIZE))
    {
        ...
        for (n = 0; n < num_glyphs; n++)
        {
            ...
            if (gname)
            {
                ps_check_extra_glyph_name(gname, n, extra_glyphs,
        ...
        else
        {
            ...
            if (count < num_glyphs / 2)
            {
                (void)FT_RENEW_ARRAY(table->maps,
                                     num_glyphs + EXTRA_GLYPH_LIST_SIZE, count);
                error = FT_Err_Ok;
            }
...
static FT_UInt ps_unicodes_char_index(PS_Unicodes table, FT_UInt32 unicode)
{
    ...
    while (min <= max)
    {
        ...
        if (base_glyph == unicode)
            result =
                mid; /* remember match but continue search for base glyph */
