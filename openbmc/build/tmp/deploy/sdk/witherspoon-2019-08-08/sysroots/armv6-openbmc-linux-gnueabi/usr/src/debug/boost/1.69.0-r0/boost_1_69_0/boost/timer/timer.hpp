...
{
  public:
    //  Explicit defaults for os are not provided to avoid including <iostream>,
    //  which has high costs even when the standard streams are not actually
    //  used. Explicit defaults for format are not provided to avoid
    //  order-of-dynamic-initialization issues with a std::string.
    ...
    //    postconditions and ease specification of other functionality without
    //    resorting to "for exposition only" private members.
