...
static void ensure_session_directory(void)
{
    ...
    dir = malloc(strlen(home) + strlen(DBUS_DIR) +
                 strlen(DBUS_SESSION_BUS_DIR) + 3);
    if (dir == NULL)
    {
    ...
    if (mkdir(dir, 0700) < 0)
    {
        /* only print a warning here, writing the session file itself will fail
         * later */
        if (errno != EEXIST)
...
static int init_x_atoms(Display* display)
{
    ...
    if (user == NULL)
    {
        verbose("Could not determine user information; aborting X11 "
                "integration.\n");
        return FALSE;
    }
    ...
    atom_name = malloc(strlen(machine) + strlen(user_name) + 2 +
                       MAX(strlen(selection_prefix),
                           MAX(strlen(address_prefix), strlen(pid_prefix))));
    if (atom_name == NULL)
    {
...
int x11_get_address(char** paddress, pid_t* pid, long* wid)
{
    ...
    *paddress = NULL;
    ...
    result = XGetWindowProperty(xdisplay, owner, address_atom, 0, 1024, False,
                                ...
                                (unsigned char**)&data);
    if (result != Success || type == None || after != 0 || data == NULL ||
        format != 8)
        return FALSE; /* error */
    ...
    *paddress = xstrdup(data);
    ...
    {
        *pid = 0;
        result = XGetWindowProperty(xdisplay, owner, pid_atom, 0, sizeof pid,
...
static Window set_address_in_x11(char* address, pid_t pid)
{
    ...
    Window wid = None;
    unsigned long pid32; /* Xlib property functions want _long_ not 32-bit for
                            format "32" */
    ...
    /* Create our window */
    wid = XCreateWindow(xdisplay, RootWindow(xdisplay, 0), -20, -20, 10, 10, 0,
...
static int set_address_in_file(char* address, pid_t pid, Window wid)
{
    ...
    fprintf(
        ...
        get_machine_uuid(), getenv("DISPLAY"), address, (long)pid, (long)wid);
