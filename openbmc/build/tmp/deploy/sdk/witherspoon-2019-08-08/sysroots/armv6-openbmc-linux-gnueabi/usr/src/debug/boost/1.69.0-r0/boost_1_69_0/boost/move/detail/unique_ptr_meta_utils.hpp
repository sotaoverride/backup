...
struct pointer_type
{
    typedef typename pointer_type_imp<typename remove_extent<T>::type,
                                      typename remove_reference<D>::type>::type
        type;
};
...
#define BOOST_MOVEUP_HAS_VIRTUAL_DESTRUCTOR(T) __has_virtual_destructor(T)
    ...
    ((__GNUC__ > 4) ||                                                         \
     ((__GNUC__ == 4) && (__GNUC_MINOR__ >= 3) && !defined(__GCCXML__))) &&    \
    !defined(BOOST_CLANG)
#define BOOST_MOVEUP_HAS_VIRTUAL_DESTRUCTOR(T) __has_virtual_destructor(T)
...
template <class T, class U,
          bool enable = is_convertible<U*, T*>::value && !is_array<T>::value &&
                        !is_same<typename remove_cv<T>::type, void>::value &&
                        !is_same<typename remove_cv<U>::type,
