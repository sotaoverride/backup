 ...
 * {
 *   { "repeats", 'r', 0, G_OPTION_ARG_INT, &repeats, "Average over N
 * repetitions", "N" }, { "max-size", 'm', 0, G_OPTION_ARG_INT, &max_size, "Test
 * up to 2^M items", "M" }, { "verbose", 'v', 0, G_OPTION_ARG_NONE, &verbose,
 * "Be verbose", NULL }, { "beep", 'b', 0, G_OPTION_ARG_NONE, &beep, "Beep when
 * done", NULL }, { "rand", 0, 0, G_OPTION_ARG_NONE, &randomize, "Randomize the
 * data", NULL }, { NULL }
 * };
 *
 * int
...
#define TRANSLATE(group, str)                                                  \
    (((group)->translate_func                                                  \
          ? (*(group)->translate_func)((str), (group)->translate_data)         \
          : (str)))
...
static glong _g_utf8_strwidth(const gchar* p)
{
    glong len = 0;
                                       ...
                                       const gchar* translation_domain)
{
    ...
    g_option_group_set_translation_domain(context->main_group,
                                          translation_domain);
}

static gint calculate_max_length(GOptionGroup* group, GHashTable* aliases)
{
    GOptionEntry* entry;
    ...
    for (i = 0; i < group->n_entries; i++)
    {
        ...
        if (!NO_ARG(entry) && entry->arg_description)
            len +=
                1 + _g_utf8_strwidth(TRANSLATE(group, entry->arg_description));
...
static void print_entry(GOptionGroup* group, gint max_length,
                        const GOptionEntry* entry, GString* string,
                        GHashTable* aliases)
{
    GString* str;
    const gchar* long_name;
    ...
    if (entry->short_name)
        g_string_append_printf(str, "  -%c, --%s", entry->short_name,
                               long_name);
    else
    ...
    if (entry->arg_description)
        g_string_append_printf(str, "=%s",
                               TRANSLATE(group, entry->arg_description));
...
static gboolean group_has_visible_entries(GOptionContext* context,
                                          GOptionGroup* group,
                                          gboolean main_entries)
{
    ...
    for (i = 0, l = (group ? group->n_entries : 0); i < l; i++)
    {
        ...
        if (main_entries && !main_group &&
            !(entry->flags & G_OPTION_FLAG_IN_MAIN))
            continue;
...
static gboolean group_list_has_visible_entries(GOptionContext* context,
                                               GList* group_list,
                                               gboolean main_entries)
{
    while (group_list)
 ...
 * Since: 2.14
 */
gchar* g_option_context_get_help(GOptionContext* context, gboolean main_help,
                                 GOptionGroup* group)
{
    GList* list;
    ...
    while (list != NULL)
    {
        ...
        for (i = 0; i < g->n_entries; i++)
        {
            ...
            {
                g_hash_table_insert(
                    aliases, &entry->long_name,
                    g_strdup_printf("%s-%s", g->name, entry->long_name));
            }
            else
                g_hash_table_insert(shadow_map, (gpointer)entry->long_name,
                                    entry);
...
G_GNUC_NORETURN
static void print_help(GOptionContext* context, gboolean main_help,
                       GOptionGroup* group)
{
    gchar* help;
...
static gboolean parse_int(const gchar* arg_name, const gchar* arg, gint* result,
                          GError** error)
{
    gchar* end;
    glong tmp;
    ...
    if (*arg == '\0' || *end != '\0')
    {
        g_set_error(error, G_OPTION_ERROR, G_OPTION_ERROR_BAD_VALUE,
                    _("Cannot parse integer value “%s” for %s"), arg, arg_name);
        return FALSE;
    }
    ...
    *result = tmp;
    ...
    {
        g_set_error(error, G_OPTION_ERROR, G_OPTION_ERROR_BAD_VALUE,
                    _("Integer value “%s” for %s out of range"), arg, arg_name);
        return FALSE;
    }
...
static gboolean parse_double(const gchar* arg_name, const gchar* arg,
                             gdouble* result, GError** error)
{
    gchar* end;
    gdouble tmp;
    ...
    {
        g_set_error(error, G_OPTION_ERROR, G_OPTION_ERROR_BAD_VALUE,
                    _("Cannot parse double value “%s” for %s"), arg, arg_name);
        return FALSE;
    }
    if (errno == ERANGE)
    {
        g_set_error(error, G_OPTION_ERROR, G_OPTION_ERROR_BAD_VALUE,
                    _("Double value “%s” for %s out of range"), arg, arg_name);
        return FALSE;
    }
...
static gboolean parse_int64(const gchar* arg_name, const gchar* arg,
                            gint64* result, GError** error)
{
    gchar* end;
    gint64 tmp;
...
static Change* get_change(GOptionContext* context, GOptionArg arg_type,
                          gpointer arg_data)
{
    GList* list;
    Change* change = NULL;
...
static gboolean parse_arg(GOptionContext* context, GOptionGroup* group,
                          GOptionEntry* entry, const gchar* value,
                          const gchar* option_name, GError** error)

{
    ...
    switch (entry->arg)
    {
        ...
        {
            (void)get_change(context, G_OPTION_ARG_NONE, entry->arg_data);

            *(gboolean*)entry->arg_data =
                !(entry->flags & G_OPTION_FLAG_REVERSE);
            break;
        }
        ...
        case G_OPTION_ARG_STRING_ARRAY:
        {
            ...
            change =
                get_change(context, G_OPTION_ARG_STRING_ARRAY, entry->arg_data);
            ...
            change->allocated.array.data[change->allocated.array.len + 1] =
                NULL;
...
#endif
            change =
                get_change(context, G_OPTION_ARG_FILENAME, entry->arg_data);
        ...
        case G_OPTION_ARG_CALLBACK:
        {
            ...
            if (!(entry->flags &
                  (G_OPTION_FLAG_NO_ARG | G_OPTION_FLAG_OPTIONAL_ARG)) &&
                !data)
                return FALSE;

            retval = (*(GOptionArgFunc)entry->arg_data)(
                option_name, data, group->user_data, error);
...
static gboolean parse_short_option(GOptionContext* context, GOptionGroup* group,
                                   gint idx, gint* new_idx, gchar arg,
                                   ...
                                   gboolean* parsed)
{
    ...
    for (j = 0; j < group->n_entries; j++)
    {
        if (arg == group->entries[j].short_name)
        {
            ...
            else
            {
                ...
                if (idx < *argc - 1)
                {
                    ...
                    else
                    {
                        ...
                        {
                            value = (*argv)[idx + 1];
                            add_pending_null(context, &((*argv)[idx + 1]),
                                             NULL);
                            *new_idx = idx + 1;
                        }
                    }
            ...
            if (!parse_arg(context, group, &group->entries[j], value,
                           option_name, error))
            {
                g_free(option_name);
                return FALSE;
...
static gboolean parse_long_option(GOptionContext* context, GOptionGroup* group,
                                  gint* idx, gchar* arg, gboolean aliased,
                                  gint* argc, gchar*** argv, GError** error,
                                  gboolean* parsed)
{
    gint j;
    ...
    for (j = 0; j < group->n_entries; j++)
    {
            ...
            strcmp(arg, group->entries[j].long_name) == 0)
        {
            ...
            retval = parse_arg(context, group, &group->entries[j], NULL,
                               option_name, error);
            g_free(option_name);
        ...
        else
        {
                ...
                (arg[len] == '=' || arg[len] == 0))
            {
                ...
                add_pending_null(context, &((*argv)[*idx]), NULL);
                option_name =
                    g_strconcat("--", group->entries[j].long_name, NULL);
...
static gboolean parse_remaining_arg(GOptionContext* context,
                                    GOptionGroup* group, gint* idx, gint* argc,
                                    ...
                                    gboolean* parsed)
{
    ...
    for (j = 0; j < group->n_entries; j++)
    {
        ...
        g_return_val_if_fail(
            group->entries[j].arg == G_OPTION_ARG_CALLBACK ||
        ...
        if (!parse_arg(context, group, &group->entries[j], (*argv)[*idx], "",
                       error))
            return FALSE;
 ...
 * Since: 2.6
 **/
gboolean g_option_context_parse(GOptionContext* context, gint* argc,
                                gchar*** argv, GError** error)
{
    gint i, j, k;
    GList* list;
    ...
    while (list)
    {
        ...
        {
            if (!(*group->pre_parse_func)(context, group, group->user_data,
                                          error))
                goto fail;
        }
    ...
    if (context->main_group && context->main_group->pre_parse_func)
    {
        if (!(*context->main_group->pre_parse_func)(
                context, context->main_group, context->main_group->user_data,
                error))
            goto fail;
    }
        ...
        for (i = 1; i < *argc; i++)
        {
            ...
            if ((*argv)[i][0] == '-' && (*argv)[i][1] != '\0' && !stop_parsing)
            {
                if ((*argv)[i][1] == '-')
                {
                    ...
                    if (context->main_group &&
                        !parse_long_option(context, context->main_group, &i,
                                           arg, FALSE, argc, argv, error,
                                           &parsed))
                        goto fail;
                    ...
                    while (list)
                    {
                        ...
                        if (!parse_long_option(context, group, &i, arg, FALSE,
                    ...
                    if (dash && arg < dash)
                    {
                        ...
                        while (list)
                        {
                            ...
                            {
                                if (!parse_long_option(context, group, &i,
                                                       dash + 1, TRUE, argc,
                                                       argv, error, &parsed))
                                    goto fail;
                ...
                { /* short option */
                    ...
                    for (j = 0; j < arg_length; j++)
                    {
                        ...
                        parsed = FALSE;
                        if (context->main_group &&
                            !parse_short_option(context, context->main_group, i,
                                                &new_i, arg[j], argc, argv,
                                                error, &parsed))
                            goto fail;
                        if (!parsed)
                        {
                            ...
                            {
                                GOptionGroup* group = list->data;
                                if (!parse_short_option(context, group, i,
                                                        &new_i, arg[j], argc,
                                                        argv, error, &parsed))
                                    goto fail;
                                if (parsed)
            ...
            else
            {
                ...
                if (context->main_group &&
                    !parse_remaining_arg(context, context->main_group, &i, argc,
                                         argv, error, &parsed))
                    goto fail;
    ...
    while (list)
    {
        ...
        {
            if (!(*group->post_parse_func)(context, group, group->user_data,
                                           error))
                goto fail;
        }
 ...
 *   This string is translated using the translation domain or translation
 *function of the group
 * @user_data: (nullable): user data that will be passed to the pre- and
 *post-parse hooks, the error hook and to callbacks of %G_OPTION_ARG_CALLBACK
 *options, or %NULL
 ...
 * Since: 2.6
 **/
GOptionGroup* g_option_group_new(const gchar* name, const gchar* description,
                                ...
                                const GOptionEntry* entries)
{
    ...
    group->entries =
        g_renew(GOptionEntry, group->entries, group->n_entries + n_entries);
    ...
    for (i = group->n_entries; i < group->n_entries + n_entries; i++)
    {
        ...
        {
            g_warning(G_STRLOC ": ignoring reverse flag on option of arg-type "
                               "%d in entry %s:%s",
                      group->entries[i].arg, group->name,
        ...
        if (group->entries[i].arg != G_OPTION_ARG_CALLBACK &&
            (group->entries[i].flags &
             (G_OPTION_FLAG_NO_ARG | G_OPTION_FLAG_OPTIONAL_ARG |
              G_OPTION_FLAG_FILENAME)) != 0)
        {
            g_warning(G_STRLOC
                      ": ignoring no-arg, optional-arg or filename flags (%d) "
                      "on option of arg-type %d in entry %s:%s",
                      group->entries[i].flags, group->entries[i].arg,
 ...
 * @destroy_notify: (nullable): a function which gets called to free @data, or
 *%NULL
 *
 ...
 * Since: 2.6
 **/
void g_option_group_set_translate_func(GOptionGroup* group, GTranslateFunc func,
                                       gpointer data,
                                       GDestroyNotify destroy_notify)
{
    g_return_if_fail(group != NULL);
...
static const gchar* dgettext_swapped(const gchar* msgid,
                                           ...
                                           const gchar* domain)
{
    ...
    g_option_group_set_translate_func(group, (GTranslateFunc)dgettext_swapped,
 ...
 * this function only affects the @parameter_string (see
 *g_option_context_new()), the summary (see g_option_context_set_summary()) and
...
void g_option_context_set_translate_func(GOptionContext* context,
                                         GTranslateFunc func, gpointer data,
                                         GDestroyNotify destroy_notify)
{
    g_return_if_fail(context != NULL);
                                             ...
                                             const gchar* domain)
{
    ...
    g_option_context_set_translate_func(
        context, (GTranslateFunc)dgettext_swapped, g_strdup(domain), g_free);
}
