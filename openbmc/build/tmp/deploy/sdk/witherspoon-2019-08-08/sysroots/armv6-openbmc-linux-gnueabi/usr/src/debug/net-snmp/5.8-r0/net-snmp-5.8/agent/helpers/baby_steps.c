...
static u_short
    set_mode_map[SNMP_MSG_INTERNAL_SET_MAX][BABY_STEPS_PER_MODE_MAX] = {
        /*R1*/
        {MODE_BSTEP_PRE_REQUEST, MODE_BSTEP_OBJECT_LOOKUP,
         MODE_BSTEP_ROW_CREATE, MODE_BSTEP_CHECK_VALUE},
        /*R2*/
        {MODE_BSTEP_UNDO_SETUP, BABY_STEP_NONE, BABY_STEP_NONE, BABY_STEP_NONE},
        /*A */
        {MODE_BSTEP_SET_VALUE, MODE_BSTEP_CHECK_CONSISTENCY, MODE_BSTEP_COMMIT,
         BABY_STEP_NONE},
        /*C */
        {MODE_BSTEP_IRREVERSIBLE_COMMIT, MODE_BSTEP_UNDO_CLEANUP,
         MODE_BSTEP_POST_REQUEST, BABY_STEP_NONE},
        /*F */
...
static int _baby_steps_helper(netsnmp_mib_handler* handler,
                              netsnmp_handler_registration* reginfo,
                              netsnmp_agent_request_info* reqinfo,
                              netsnmp_request_info* requests);
static int _baby_steps_access_multiplexer(netsnmp_mib_handler* handler,
                                          netsnmp_handler_registration* reginfo,
                                          netsnmp_agent_request_info* reqinfo,
                                          netsnmp_request_info* requests);
...
static void netsnmp_baby_steps_modes_deref(netsnmp_baby_steps_modes* md)
{
    if (--md->refcnt == 0)
        free(md);
}
                              ...
                              netsnmp_request_info* requests)
{
    ...
    for (i = 0; i < BABY_STEPS_PER_MODE_MAX; ++i)
    {
        ...
        if (BSTEP_USE_ORIGINAL != mode_map_ptr[i])
        {
            ...
            {
                DEBUGMSGTL(
                    ("baby_steps", "   skipping commit undo (no commit)\n"));
                continue;
            }
            ...
            {
                DEBUGMSGTL(
                    ("baby_steps", "   skipping mode (not registered)\n"));
                continue;
            }
        }
        else
...
/** @defgroup access_multiplexer baby_steps_access_multiplexer: calls individual
 * access methods based on baby_step mode.
 *  @ingroup baby_steps
 *  @{
 */
                                          ...
                                          netsnmp_request_info* requests)
{
    ...
    netsnmp_assert((handler != NULL) && (reginfo != NULL) &&
                   (reqinfo != NULL) && (requests != NULL));
