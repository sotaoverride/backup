...
/* Goes through /etc/fstab and remounts all API file systems, applying options
 * that are in /etc/fstab that systemd might not have respected */
...
static int do_remount(const char* path, bool force_rw, Hashmap** pids)
{
    ...
    {
        /* Child */
        execv(MOUNT_PATH, STRV_MAKE(MOUNT_PATH, path, "-o",
...
static int run(int argc, char* argv[])
{
    ...
    if (!has_root)
    {
        /* The $SYSTEMD_REMOUNT_ROOT_RW environment variable is set by
         * systemd-gpt-auto-generator to tell us whether to remount things. We
         * honour it only if there's no explicit line in /etc/fstab configured
         * which takes precedence. */

        r = getenv_bool("SYSTEMD_REMOUNT_ROOT_RW");
        if (r < 0 && r != -ENXIO)
            log_warning_errno(
                r, "Failed to parse $SYSTEMD_REMOUNT_ROOT_RW, ignoring: %m");
    ...
    while (!hashmap_isempty(pids))
    {
            ...
            !is_clean_exit(si.si_code, si.si_status, EXIT_CLEAN_COMMAND, NULL))
        {
            if (si.si_code == CLD_EXITED)
                log_error(MOUNT_PATH " for %s exited with exit status %i.", s,
                          si.si_status);
            else
                log_error(MOUNT_PATH " for %s terminated by signal %s.", s,
