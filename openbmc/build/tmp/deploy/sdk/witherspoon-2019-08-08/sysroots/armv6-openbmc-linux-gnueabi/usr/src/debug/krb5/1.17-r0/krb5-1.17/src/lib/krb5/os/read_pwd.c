...
krb5_error_code krb5_read_password(krb5_context context, const char* prompt,
...
#ifdef _WIN32
static krb5_error_code read_console_password(krb5_context context,
                                             const char* prompt,
                                             const char* prompt2,
                                             char* password, int* pwsize)
{
    HANDLE handle;
...
static int CALLBACK read_pwd_proc(HWND hdlg, UINT msg, WPARAM wParam,
                                  LPARAM lParam)
{
    pwd_params* dp;
    ...
    switch (msg)
    {
        ...
        case WM_COMMAND:
            ...
            {
                case IDOK:
                    *(dp->pwd_size_return) = GetDlgItemText(
                        hdlg, ID_READ_PWD_PWD, dp->pwd_return_pwd,
                        *(dp->pwd_size_return));
                    EndDialog(hdlg, TRUE);
                    break;
...
krb5_error_code
    KRB5_CALLCONV krb5_read_password(context, prompt, prompt2, return_pwd,
                                     size_return) krb5_context context;
const char* prompt;
const char* prompt2;
char* return_pwd;
int* size_return;
{
    ...
    if (_isatty(_fileno(stdin)))
        return (read_console_password(context, prompt, prompt2, return_pwd,
                                      size_return));
#endif
...
#endif
    rc = DialogBoxParam(hinst, MAKEINTRESOURCE(ID_READ_PWD_DIALOG), 0, dlgproc,
                        (LPARAM)&dps);
#ifndef _WIN32
