...
static int sg_err_category_new(int scsi_status, int msg_status, int host_status,
                               int driver_status,
...
static int sg_err_category4(struct sg_io_v4* hp)
{
    return sg_err_category_new(
        hp->device_status, 0, hp->transport_status, hp->driver_status,
        (unsigned char*)(uintptr_t)hp->response, hp->response_len);
}
                           ...
                           unsigned char* sense_buffer, int sb_len)
{
    ...
    if (sense_class == 7)
    {
        ...
        log_debug("%s: sense key 0x%x ASC 0x%x ASCQ 0x%x", dev_scsi->kernel,
                  sense_key, asc, ascq);
    }
    else
    {
        ...
        if (sense_buffer[0] < 15)
            ...
            log_debug("%s: sense = %2x %2x", dev_scsi->kernel, sense_buffer[0],
                      sense_buffer[2]);
...
static int scsi_dump(struct scsi_id_device* dev_scsi, struct sg_io_hdr* io)
{
    ...
    log_debug("%s: sg_io failed status 0x%x 0x%x 0x%x 0x%x", dev_scsi->kernel,
              io->driver_status, io->host_status, io->msg_status, io->status);
...
{
    unsigned char inq_cmd[INQUIRY_CMDLEN] = {INQUIRY_CMD, evpd,   page,
                                             0,           buflen, 0};
                                 ...
                                 unsigned char* buffer, unsigned len)
{
    ...
    if (buffer[3] > MODEL_LENGTH)
    {
         ...
         * invalid.
         */
        if (strneq((char*)&buffer[VENDOR_LENGTH], dev_scsi->vendor,
                   VENDOR_LENGTH))
        {
...
static int prepend_vendor_model(struct scsi_id_device* dev_scsi, char* serial)
{
    ...
    {
        log_debug("%s: expected length %d, got length %d", dev_scsi->kernel,
...
static int check_fill_0x83_id(struct scsi_id_device* dev_scsi,
                              ...
                              char* wwn, char* wwn_vendor_extension,
                              char* tgpt_group)
{
    int i, j, s, len;
                                  ...
                                  char* wwn_vendor_extension, char* tgpt_group)
{
     ...
     */
    for (id_ind = 0;
         id_ind < sizeof(id_search_list) / sizeof(id_search_list[0]); id_ind++)
    {
        /*
        ...
        for (j = 4; j <= (unsigned)page_83[3] + 3; j += page_83[j + 3] + 4)
        {
            retval = check_fill_0x83_id(
                dev_scsi, &page_83[j], &id_search_list[id_ind], serial,
                serial_short, len, wwn, wwn_vendor_extension, tgpt_group);
            if (!retval)
                return retval;
            else if (retval < 0)
                return retval;
 ...
 */
static int do_scsi_page83_prespc3_inquiry(struct scsi_id_device* dev_scsi,
                                          int fd, char* serial,
                    ...
                    int page_code, int len)
{
    ...
    {
        if (do_scsi_page80_inquiry(dev_scsi, fd, dev_scsi->serial,
    ...
    {
        retval = do_scsi_page83_prespc3_inquiry(dev_scsi, fd, dev_scsi->serial,
                                                dev_scsi->serial_short, len);
        if (retval)
        {
            /*
