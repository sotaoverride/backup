...
struct pthread* __find_in_stack_list(struct pthread* pd)
{
    list_t* entry;
...
void __free_tcb(struct pthread* pd)
{
    ...
    if (__builtin_expect(
            atomic_bit_test_set(&pd->cancelhandling, TERMINATED_BIT) == 0, 1))
    {
...
START_THREAD_DEFN
{
    ...
    if (__glibc_unlikely(pd->parent_cancelhandling & CANCELING_BITMASK))
    {
        ...
        (void)INTERNAL_SYSCALL(rt_sigprocmask, err, 4, SIG_UNBLOCK, &mask, NULL,
                               _NSIG / 8);
    }
#endif
    ...
    else if (__glibc_unlikely(pd->cancelhandling & SETXID_BITMASK))
    {
       ...
       us to reply.  In this case wait until we did that.  */
        do
            /* XXX This differs from the typical futex_wait_simple pattern in
               that the futex_wait condition (setxid_futex) is different from
               the condition used in the surrounding loop (cancelhandling).  We
               need to check and document why this is correct.  */
