...
static const char* default_cfg_paths[] = {
    "/run/depmod.d", SYSCONFDIR "/depmod.d", "/lib/depmod.d", NULL};
...
static void help(void)
{
    printf(
        ...
        "\n"
        "depmod will output a dependency list suitable for the modprobe "
        "utility.\n"
        "\n"
        "Options:\n"
...
static void index__checkstring(const char* str)
{
    ...
    for (i = 0; str[i]; i++)
    {
        ...
        if (ch >= INDEX_CHILDMAX)
            CRIT("Module index: bad character '%c'=0x%x - only 7-bit ASCII is "
                 "supported:"
                 "\n%s\n",
                 (char)ch, (int)ch, str);
    }
...
static int cfg_override_add(struct cfg* cfg, const char* modname,
                            const char* subdir)
{
...
static int cfg_files_insert_sorted(struct cfg_file*** p_files,
                                   size_t* p_n_files, const char* dir,
                                   const char* name)
{
...
static int depmod_module_add(struct depmod* depmod, struct kmod_module* kmod)
{
    ...
    if (mod->relpath != NULL)
    {
        ...
        err = hash_add_unique(depmod->modules_by_uncrelpath, mod->uncrelpath,
                              mod);
        if (err < 0)
        {
                                      ...
                                      size_t namelen, const char* path)
{
    ...
    if (depmod_module_is_higher_priority(depmod, mod, baselen, namelen,
                                         modnamelen, path))
    {
...
static int depmod_modules_search_dir(struct depmod* depmod, DIR* d,
                                     size_t baselen, struct scratchbuf* s_path)
{
    struct dirent* de;
...
static int depmod_load_module_dependencies(struct depmod* depmod,
                                           struct mod* mod)
{
    const struct cfg* cfg = depmod->cfg;
    struct kmod_list* l;
...
static int depmod_output(struct depmod* depmod, FILE* out)
{
    ...
    } * itr, depfiles[] = {{"modules.dep", output_deps},
...
static int depfile_up_to_date_dir(DIR* d, time_t mtime, size_t baselen,
                                  char* path)
{
    struct dirent* de;
...
static int do_depmod(int argc, char* argv[])
{
    ...
    cfg.dirnamelen = snprintf(cfg.dirname, PATH_MAX, "%s/lib/modules/%s",
