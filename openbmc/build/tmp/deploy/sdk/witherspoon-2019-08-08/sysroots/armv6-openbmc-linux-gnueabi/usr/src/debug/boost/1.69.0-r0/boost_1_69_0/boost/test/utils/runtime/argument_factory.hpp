    ...
    ValueType interpret(cstring param_name, cstring source) const
    {
        ...
        if (!unit_test::utils::string_as<ValueType>(source, res))
            BOOST_TEST_I_THROW(format_error(param_name)
                               << source
                               << " can't be interpreted as value of parameter "
                               << param_name << ".");
        return res;
    }
};
...
struct value_interpreter<bool, false>
{
    ...
    bool interpret(cstring param_name, cstring source) const
    {
        ...
        if (source.is_empty() || case_ins_eq(source, s_YES) ||
            case_ins_eq(source, s_Y) || case_ins_eq(source, s_one) ||
            case_ins_eq(source, s_TRUE))
            return true;

        if (case_ins_eq(source, s_NO) || case_ins_eq(source, s_N) ||
            case_ins_eq(source, s_zero) || case_ins_eq(source, s_FALSE))
            return false;
...
struct value_interpreter<EnumType, true>
{
    ...
    {
        typename std::map<cstring, EnumType>::const_iterator found =
            m_name_to_value.find(source);

        BOOST_TEST_I_ASSRT(
            found != m_name_to_value.end(),
            format_error(param_name)
                << source
                << " is not a valid enumeration value name for parameter "
                << param_name << ".");
...
class argument_factory<ValueType, is_enum, false>
{
  public:
    ...
    void produce_argument(cstring source, cstring param_name,
                          arguments_store& store) const
    {
        store.set(param_name,
                  source.empty() ? m_optional_value
                                 : m_interpreter.interpret(param_name, source));
    }
...
class argument_factory<ValueType, is_enum, true>
{
  public:
    template <typename Modifiers>
    explicit argument_factory(Modifiers const& m) : m_interpreter(m)
    {
    }
                          ...
                          arguments_store& store) const
    {
        ...
        if (store.has(param_name))
        {
            std::vector<ValueType>& values =
                store.get<std::vector<ValueType>>(param_name);
