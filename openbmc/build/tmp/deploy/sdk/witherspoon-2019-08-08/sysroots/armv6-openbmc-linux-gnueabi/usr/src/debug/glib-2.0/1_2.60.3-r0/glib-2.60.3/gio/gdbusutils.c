...
static gboolean is_valid_bus_name_character(gint c, gboolean allow_hyphen)
{
    return (c >= '0' && c <= '9') || (c >= 'A' && c <= 'Z') ||
           (c >= 'a' && c <= 'z') || (c == '_') || (allow_hyphen && c == '-');
}

static gboolean
    is_valid_initial_bus_name_character(gint c, gboolean allow_initial_digit,
...
static gboolean is_valid_name(const gchar* start, guint len,
                              ...
                              gboolean allow_hyphen)
{
    ...
    while (s != end)
    {
        ...
        {
            s += 1;
            if (G_UNLIKELY(!is_valid_initial_bus_name_character(
                    *s, allow_initial_digit, allow_hyphen)))
                goto out;
            has_dot = TRUE;
        }
...
gboolean g_dbus_is_member_name(const gchar* string)
{
    ...
    if (G_UNLIKELY(
            !is_valid_initial_bus_name_character(string[0], FALSE, FALSE)))
        goto out;
...
/* TODO: maybe move to glib? if so, it should conform to
 * http://en.wikipedia.org/wiki/Guid and/or http://tools.ietf.org/html/rfc4122 -
 * specifically it should have hyphens then.
 */
 ...
 * @value: A #GVariant.
 * @out_gvalue: (out): Return location pointing to a zero-filled (uninitialized)
 * #GValue.
 *
                                    ...
                                    const GVariantType* type)
{
    ...
    /* @type can easily be e.g. "s" with the GValue holding a GVariant - for
     * example this can happen when using the org.gtk.GDBus.C.ForceGVariant
     * annotation with the gdbus-codegen(1) tool.
     */
        ...
        switch (g_variant_type_peek_string(type)[0])
        {
            case G_VARIANT_CLASS_BOOLEAN:
                ret = g_variant_ref_sink(
                    g_variant_new_boolean(g_value_get_boolean(gvalue)));
                break;

            case G_VARIANT_CLASS_BYTE:
                ret = g_variant_ref_sink(
                    g_variant_new_byte(g_value_get_uchar(gvalue)));
                break;

            case G_VARIANT_CLASS_INT16:
                ret = g_variant_ref_sink(
                    g_variant_new_int16(g_value_get_int(gvalue)));
                break;

            case G_VARIANT_CLASS_UINT16:
                ret = g_variant_ref_sink(
                    g_variant_new_uint16(g_value_get_uint(gvalue)));
                break;

            case G_VARIANT_CLASS_INT32:
                ret = g_variant_ref_sink(
                    g_variant_new_int32(g_value_get_int(gvalue)));
                break;

            case G_VARIANT_CLASS_UINT32:
                ret = g_variant_ref_sink(
                    g_variant_new_uint32(g_value_get_uint(gvalue)));
                break;

            case G_VARIANT_CLASS_INT64:
                ret = g_variant_ref_sink(
                    g_variant_new_int64(g_value_get_int64(gvalue)));
                break;

            case G_VARIANT_CLASS_UINT64:
                ret = g_variant_ref_sink(
                    g_variant_new_uint64(g_value_get_uint64(gvalue)));
                break;

            case G_VARIANT_CLASS_DOUBLE:
                ret = g_variant_ref_sink(
                    g_variant_new_double(g_value_get_double(gvalue)));
                break;
                        ...
                        switch (g_variant_type_peek_string(type)[2])
                        {
                            case G_VARIANT_CLASS_BYTE:
                                ...
                                if (as == NULL)
                                    as = empty_strv;
                                ret = g_variant_ref_sink(
                                    g_variant_new_bytestring_array(as, -1));
                                break;
    ...
    if (ret == NULL)
    {
        ...
        untrusted_empty =
            g_variant_new_from_data(type, NULL, 0, FALSE, NULL, NULL);
