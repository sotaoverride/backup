                            ...
                            bool active_low, const char* consumer)
{
    ...
    rv = gpiod_ctxless_get_value_multiple(device, &offset, &value, 1,
                                          active_low, consumer);
    if (rv < 0)
        return rv;
                                     ...
                                     gpiod_ctxless_set_value_cb cb, void* data)
{
    ...
    rv = gpiod_line_request_bulk_output_flags(&bulk, consumer, flags, values);
    if (rv < 0)
    {
        gpiod_chip_close(chip);
        return -1;
...
{
    return gpiod_ctxless_event_monitor(device, GPIOD_CTXLESS_EVENT_BOTH_EDGES,
                                       offset, active_low, consumer, timeout,
                                       poll_cb, event_cb, data);
}

int gpiod_ctxless_event_loop_multiple(
    const char* device, const unsigned int* offsets, unsigned int num_lines,
    bool active_low, const char* consumer, const struct timespec* timeout,
    gpiod_ctxless_event_poll_cb poll_cb, gpiod_ctxless_event_handle_cb event_cb,
    void* data)
{
    return gpiod_ctxless_event_monitor_multiple(
        device, GPIOD_CTXLESS_EVENT_BOTH_EDGES, offsets, num_lines, active_low,
        consumer, timeout, poll_cb, event_cb, data);
}
...
int gpiod_ctxless_event_monitor(const char* device, int event_type,
                                ...
                                void* data)
{
    return gpiod_ctxless_event_monitor_multiple(device, event_type, &offset, 1,
                                                active_low, consumer, timeout,
                                                poll_cb, event_cb, data);
}

int gpiod_ctxless_event_monitor_multiple(
    const char* device, int event_type, const unsigned int* offsets,
    unsigned int num_lines, bool active_low, const char* consumer,
    ...
    gpiod_ctxless_event_handle_cb event_cb, void* data)
{
    ...
    for (;;)
    {
        ...
        {
            rv = event_cb(GPIOD_CTXLESS_EVENT_CB_TIMEOUT, 0, &event.ts, data);
