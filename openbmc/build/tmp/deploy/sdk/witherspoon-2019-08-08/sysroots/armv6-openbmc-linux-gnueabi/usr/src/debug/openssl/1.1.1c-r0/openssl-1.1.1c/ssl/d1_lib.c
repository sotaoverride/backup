...
const SSL3_ENC_METHOD DTLSv1_enc_data = {tls1_enc,
                                         tls1_mac,
...
int dtls1_handle_timeout(SSL* s)
{
    ...
    if (s->d1->timer_cb != NULL)
        s->d1->timeout_duration_us =
            s->d1->timer_cb(s, s->d1->timeout_duration_us);
    else
...
int DTLSv1_listen(SSL* s, BIO_ADDR* client)
{
    ...
    do
    {
        ...
        if (s->msg_callback)
            s->msg_callback(0, 0, SSL3_RT_HEADER, buf, DTLS1_RT_HEADER_LENGTH,
...
{
    return (
        g_probable_mtu[(sizeof(g_probable_mtu) / sizeof(g_probable_mtu[0])) -
                       1]);
}
...
size_t DTLS_get_data_mtu(const SSL* s)
{
    ...
    if (!ssl_cipher_get_overhead(ciph, &mac_overhead, &int_overhead, &blocksize,
                                 &ext_overhead))
        return 0;
