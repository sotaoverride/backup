...
static int zsendfile __P((struct zm_fileinfo * zi, const char* buf,
                          size_t blen));
...
static RETSIGTYPE onintr(int n LRZSZ_ATTRIB_UNUSED)
{
...
int main(int argc, char** argv)
{
    ...
    if (((cp = getenv("SHELL")) != NULL) &&
        (strstr(cp, "rsh") || strstr(cp, "rksh") || strstr(cp, "rbash") ||
         strstr(cp, "rshell")))
    {
        under_rsh = TRUE;
        Restricted = 1;
                         ...
                         long_options, (int*)0)) != EOF)
    {
        ...
        switch (c)
        {
            ...
            case 'L':
                ...
                {
                    char meld[256];
                    sprintf(meld, _("packetlength out of range 24..%ld"),
                            (long)MAX_BLOCK);
                    usage(2, meld);
                }
                break;
            case 'l':
                ...
                {
                    char meld[256];
                    sprintf(meld, _("framelength out of range 32..%ld"),
                            (long)MAX_BLOCK);
                    usage(2, meld);
                }
                break;
            ...
            case 'U':
                if (!under_rsh)
                    ...
                    error(1, 0,
                          _("security violation: can't do that under "
                            "restricted shell\n"));
                break;
            case 'v':
                ++Verbose;
    ...
    if (getuid() != geteuid())
    {
        error(1, 0, _("this program was never intended to be used setuid\n"));
    }
    zsendline_init();
...
static int send_pseudo(const char* name, const char* data)
{
    char* tmp;
    const char* p;
    int ret = 0; /* ok */
    ...
    do
    {
        ...
        {
            free(tmp);
            vstringf(_("send_pseudo %s: cannot open tmpfile %s: %s"), name, tmp,
                     strerror(errno));
            vstring("\r\n");
            return 1;
        }
        ...
        if (fd != -1)
        {
            ...
            if (0 != lstat(tmp, &st))
            {
                vstringf(_("send_pseudo %s: cannot lstat tmpfile %s: %s"), name,
                         tmp, strerror(errno));
                vstring("\r\n");
                unlink(tmp);
                close(fd);
    ...
    if (write(fd, data, strlen(data)) != (signed long)strlen(data) ||
        close(fd) != 0)
    {
        vstringf(_("send_pseudo %s: cannot write to tmpfile %s: %s"), name, tmp,
                 strerror(errno));
        vstring("\r\n");
        free(tmp);
        return 1;
...
#ifdef PUBDIR
            || (oname[0] == '/' &&
                strncmp(oname, MK_STRING(PUBDIR), strlen(MK_STRING(PUBDIR))))
#endif
        )
        {
            canit(STDOUT_FILENO);
            vchar('\r');
            error(1, 0, _("security violation: not allowed to upload from %s"),
                  oname);
        }
    }
...
#ifdef ENABLE_SYSLOG
            if (enable_syslog)
                lsyslog(LOG_INFO, _("%s/%s: error occured"), protname(),
                        shortname);
#endif
            return ERROR;
        case ZSKIP:
...
static int wctxpn(struct zm_fileinfo* zi)
{
    ...
    if (protocol == ZM_XMODEM)
    {
        ...
        {
            vstringf(_("Sending %s, %ld blocks: "), zi->fname,
                     (long)(f.st_size >> 7));
        }
    ...
    if (!zmodem_requested)
        if (getnak())
        {
            vfile("getnak failed");
            DO_SYSLOG(
                (LOG_INFO, "%s/%s: getnak failed", shortname, protname()));
            return ERROR;
        }
    ...
    *q++ = 0;
    ...
    while (q < (txbuf + MAX_BLOCK))
        *q++ = 0;
    /* note that we may lose some information here in case mode_t is wider than
     * an int. But i believe sending %lo instead of %o _could_ break
     * compatability
     */
    if (!Ascii && (input_f != stdin) && *zi->fname &&
        fstat(fileno(input_f), &f) != -1)
        sprintf(p, "%lu %lo %o 0 %d %ld", (long)f.st_size, f.st_mtime,
                (unsigned int)((no_unixmode) ? 0 : f.st_mode), Filesleft,
                Totalleft);
    if (Verbose)
    ...
    if (wcputsec(txbuf, 0, 128) == ERROR)
    {
        ...
        DO_SYSLOG((LOG_INFO, "%s/%s: wcputsec failed", shortname, protname()));
        return ERROR;
    }
    return OK;
}
...
static int wctx(struct zm_fileinfo* zi)
{
    ...
    do
    {
        ...
        flushmo();
        ++attempts;
    } while ((firstch = (READLINE_PF(Rxtimeout)) != ACK) &&
             attempts < RETRYMAX);
...
static int wcputsec(char* buf, int sectnum, size_t cseclen)
{
    ...
    if (Verbose > 1)
    {
        ...
        if (protocol == ZM_XMODEM)
        {
            vstringf(_("Xmodem sectors/kbytes sent: %3d/%2dk"), Totsecs,
                     Totsecs / 8);
        }
        else
        {
            vstringf(_("Ymodem sectors/kbytes sent: %3d/%2dk"), Totsecs,
                     Totsecs / 8);
        }
    }
    ...
    gotnak:
        switch (firstch)
        {
            case CAN:
                ...
                break;
            case TIMEOUT:
                zperr(_("Timeout on sector ACK"));
                continue;
            case WANTCRC:
                if (firstsec)
                    Crcflg = TRUE;
            case NAK:
                zperr(_("NAK on sector"));
                continue;
            case ACK:
                firstsec = FALSE;
                Totsecs += (cseclen >> 7);
                return OK;
            case ERROR:
                zperr(_("Got burst for sector ACK"));
                break;
            default:
                zperr(_("Got %02x for sector ACK"), firstch);
                break;
        }
        for (;;)
        {
...
static void usage(int exitcode, const char* what)
{
    ...
    fputs(
        _("  -+, --append                append to existing destination file "
          "(Z)\n"
          "  -2, --twostop               use 2 stop bits\n"
          "  -4, --try-4k                go up to 4K blocksize\n"
          "      --start-4k              start with 4K blocksize (doesn't try "
          "8)\n"
          ...
          "      --delay-startup N       sleep N seconds before doing "
          "anything\n"
          "  -e, --escape                escape all control characters (Z)\n"
          "  -E, --rename                force receiver to rename files it "
          "already has\n"
          "  -f, --full-path             send full pathname (Y/Z)\n"
          "  -i, --immediate-command CMD send remote CMD, return immediately "
          "(Z)\n"
          "  -h, --help                  print this usage message\n"
          "  -k, --1k                    send 1024 byte packets (X)\n"
          "  -L, --packetlen N           limit subpacket length to N bytes "
          "(Z)\n"
          "  -l, --framelen N            limit frame length to N bytes (l>=L) "
          "(Z)\n"
        ...
        f);
    fputs(
        _("  -n, --newer                 send file if source newer (Z)\n"
          "  -N, --newer-or-longer       send file if source newer or longer "
          "(Z)\n"
          "  -o, --16-bit-crc            use 16 bit CRC instead of 32 bit CRC "
          "(Z)\n"
          "  -O, --disable-timeouts      disable timeout code, wait forever\n"
          "  -p, --protect               protect existing destination file "
          "(Z)\n"
          ...
          "seconds\n"
          "      --tcp                   build a TCP connection to transmit "
          "files\n"
          "      --tcp-server            open socket, wait for connection\n"
          "  -u, --unlink                unlink file after transmission\n"
          "  -U, --unrestrict            turn off restricted mode (if allowed "
          "to)\n"
...
static int zsendfile(struct zm_fileinfo* zi, const char* buf, size_t blen)
{
    ...
    again:
        ...
        switch (c)
        {
            ...
            case TIMEOUT:
                DO_SYSLOG(
                    (LOG_INFO, "%s/%s: got TIMEOUT", shortname, protname()));
                return ERROR;
            case ZABORT:
                DO_SYSLOG(
                    (LOG_INFO, "%s/%s: got ZABORT", shortname, protname()));
                return ERROR;
            case ZFIN:
                DO_SYSLOG((LOG_INFO, "%s/%s: got ZFIN", shortname, protname()));
                return ERROR;
            case ZCRC:
                crc = 0xFFFFFFFFL;
#ifdef HAVE_MMAP
                if (use_mmap && !mm_addr)
                {
                    ...
                    {
                        mm_size = st.st_size;
                        mm_addr = mmap(0, mm_size, PROT_READ, MAP_SHARED,
                                       fileno(input_f), 0);
            ...
            case ZSKIP:
                ...
                DO_SYSLOG((LOG_INFO, "%s/%s: receiver skipped", shortname,
                           protname()));
                return c;
            case ZRPOS:
                /*
...
static int zsendfdata(struct zm_fileinfo* zi)
{
    static int c;
    int newcnt;
    ...
    if (use_mmap && !mm_addr)
    {
        ...
        if (fstat(fileno(input_f), &st) == 0)
        {
            mm_size = st.st_size;
            mm_addr =
                mmap(0, mm_size, PROT_READ, MAP_SHARED, fileno(input_f), 0);
            if ((caddr_t)mm_addr == (caddr_t)-1)
                mm_addr = NULL;
            else
            {
        ...
        switch (c)
        {
            default:
                if (input_f)
                    fclose(input_f);
                DO_SYSLOG(
                    (LOG_INFO, "%s/%s: got %d", shortname, protname(), c));
                return ERROR;
            case ZCAN:
                if (input_f)
                    fclose(input_f);
                DO_SYSLOG(
                    (LOG_INFO, "%s/%s: got ZCAN", shortname, protname(), c));
                return ERROR;
            case ZSKIP:
                if (input_f)
                    fclose(input_f);
                DO_SYSLOG(
                    (LOG_INFO, "%s/%s: got ZSKIP", shortname, protname(), c));
                return c;
            case ZACK:
            case ZRPOS:
                break;
    ...
    do
    {
             ...
             zi->bytes_sent > last_bps / 2 + last_txpos))
        {
                ...
                if (low_bps)
                {
                    if (last_bps < min_bps)
                    {
                        if (now - low_bps >= min_bps_time)
                        {
                            ...
                            {
                                vstringf(
                                    _("zsendfdata: bps rate %ld below min %ld"),
                                    last_bps, min_bps);
                                vstring("\r\n");
                            }
                            DO_SYSLOG(
                                (LOG_INFO, "%s/%s: bps rate low: %ld <%ld",
            ...
            if (stop_time && now >= stop_time)
            {
                ...
                DO_SYSLOG((LOG_INFO, "%s/%s: reached stop time", shortname,
                           protname()));
                return ERROR;
            }
            ...
            if (Verbose > 1)
            {
                ...
                vstringf(_("Bytes Sent:%7ld/%7ld   BPS:%-8ld ETA %02d:%02d  "),
                         (long)zi->bytes_sent, (long)zi->bytes_total, last_bps,
                         minleft, secleft);
            }
        ...
        if (Txwindow)
        {
            ...
            while ((tcount = zi->bytes_sent - Lrxpos) >= Txwindow)
            {
                vfile("%ld (%ld,%ld) window >= %u", tcount,
                      (long)zi->bytes_sent, (long)Lrxpos, Txwindow);
                if (e != ZCRCQ)
        ...
        switch (getinsync(zi, 0))
        {
            ...
            case ZSKIP:
                if (input_f)
                    fclose(input_f);
                DO_SYSLOG(
                    (LOG_INFO, "%s/%s: got ZSKIP", shortname, protname()));
                return c;
            default:
                if (input_f)
                    fclose(input_f);
...
static int calc_blklen(long total_sent)
{
    ...
    if (d < 4)
    {
        ...
        {
            vstringf(
                _("calc_blklen: returned old value %d due to low bpe diff\n"),
                last_blklen);
 ...
 */
static int getinsync(struct zm_fileinfo* zi, int flag)
{
    int c;
    size_t rxpos;
