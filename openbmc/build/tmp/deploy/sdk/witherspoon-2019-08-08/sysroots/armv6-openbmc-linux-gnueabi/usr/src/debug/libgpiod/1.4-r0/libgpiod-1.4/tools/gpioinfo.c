...
static void print_help(void)
{
    ...
    printf("Print information about all lines of the specified GPIO chip(s) "
           "(or all gpiochips if none are specified).\n");
    printf("\n");
    printf("Options:\n");
...
static void list_lines(struct gpiod_chip* chip)
{
    ...
    gpiod_foreach_line(iter, line)
    {
        ...
        prinfo(&of, 8, "%s ",
               direction == GPIOD_LINE_DIRECTION_INPUT ? "input" : "output");
        prinfo(&of, 13, "%s ",
               active_state == GPIOD_LINE_ACTIVE_STATE_LOW ? "active-low"
                                                           : "active-high");
...
int main(int argc, char** argv)
{
    ...
    if (argc == 0)
    {
        ...
        gpiod_foreach_chip(chip_iter, chip) list_lines(chip);
