...
typedef int (*ControllerMethodFn)(Controller* controller, const char* path,
                                  CDVar* var_in, FDList* fds_in,
                                  CDVar* var_out);
...
static const CDVarType controller_type_in_v[] = {
    C_DVAR_T_INIT(C_DVAR_T_TUPLE1(C_DVAR_T_v))};
static const CDVarType controller_type_in_ohv[] = {
    C_DVAR_T_INIT(C_DVAR_T_TUPLE3(C_DVAR_T_o, C_DVAR_T_h, C_DVAR_T_v))};
static const CDVarType controller_type_in_osu[] = {
    C_DVAR_T_INIT(C_DVAR_T_TUPLE3(C_DVAR_T_o, C_DVAR_T_s, C_DVAR_T_u))};
static const CDVarType controller_type_out_unit[] = {
    C_DVAR_T_INIT(CONTROLLER_T_MESSAGE(C_DVAR_T_TUPLE0))};

static void controller_dvar_write_signature_out(CDVar* var,
...
static int controller_dvar_verify_signature_in(const CDVarType* type,
...
static void controller_write_reply_header(CDVar* var, uint32_t serial,
                                          const CDVarType* type)
{
    c_dvar_write(
        var, "(yyyyuu[(y<u>)(y<", c_dvar_is_big_endian(var) ? 'B' : 'l',
...
static int controller_send_error(Connection* connection, uint32_t serial,
                                 const char* error)
{
    ...
    c_dvar_write(&var, "((yyyyuu[(y<u>)(y<s>)])())",
                 ...
                 c_dvar_type_u, serial, DBUS_MESSAGE_FIELD_ERROR_NAME,
                 c_dvar_type_s, error);
...
static int controller_method_add_name(Controller* controller, const char* _path,
                                      CDVar* in_v, FDList* fds, CDVar* out_v)
{
    ...
    if (strncmp(path, "/org/bus1/DBus/Name/", strlen("/org/bus1/DBus/Name/")) !=
        0)
...
static int controller_method_add_listener(Controller* controller,
                                          const char* _path, CDVar* in_v,
                                          FDList* fds, CDVar* out_v)
{
    ...
    if (r)
        return (r == POLICY_E_INVALID) ? CONTROLLER_E_LISTENER_INVALID_POLICY
                                       : error_fold(r);
    ...
    if (strncmp(path, "/org/bus1/DBus/Listener/",
                strlen("/org/bus1/DBus/Listener/")) != 0)
        return CONTROLLER_E_UNEXPECTED_PATH;
    ...
    r = controller_add_listener(controller, &listener, path, listener_fd,
                                policy);
    if (r)
        return error_trace(r);
...
static int controller_method_listener_release(Controller* controller,
                                              const char* path, CDVar* in_v,
                                              FDList* fds, CDVar* out_v)
{
    ControllerListener* listener;
    int r;
...
static int controller_method_name_release(Controller* controller,
                                          const char* path, CDVar* in_v,
                                          FDList* fds, CDVar* out_v)
{
    ControllerName* name;
...
static int controller_method_listener_set_policy(Controller* controller,
                                                 const char* path, CDVar* in_v,
                                                 FDList* fds, CDVar* out_v)
{
    _c_cleanup_(policy_registry_freep) PolicyRegistry* policy = NULL;
...
static int controller_method_name_reset(Controller* controller,
                                        const char* path, CDVar* in_v,
                                        FDList* fds, CDVar* out_v)
{
    ControllerName* name;
...
static int controller_handle_method(const ControllerMethod* method,
                                    Controller* controller, const char* path,
                                    uint32_t serial, const char* signature_in,
                                    Message* message_in)
{
...
static int controller_dispatch_controller(Controller* controller,
                                          ...
                                          Message* message)
{
    static const ControllerMethod methods[] = {
        {"AddName", controller_method_add_name, controller_type_in_osu,
         controller_type_out_unit},
        {"AddListener", controller_method_add_listener, controller_type_in_ohv,
         controller_type_out_unit},
    };
    ...
    for (size_t i = 0; i < C_ARRAY_SIZE(methods); i++)
    {
        ...
        return controller_handle_method(&methods[i], controller, path, serial,
                                        signature, message);
    }
...
static int controller_dispatch_name(Controller* controller, uint32_t serial,
                                    const char* method, const char* path,
                                    const char* signature, Message* message)
{
    static const ControllerMethod methods[] = {
        {"Reset", controller_method_name_reset, c_dvar_type_unit,
         controller_type_out_unit},
        {"Release", controller_method_name_release, c_dvar_type_unit,
         controller_type_out_unit},
    };
...
static int controller_dispatch_listener(Controller* controller, uint32_t serial,
                                        const char* method, const char* path,
                                        const char* signature, Message* message)
{
    static const ControllerMethod methods[] = {
        {"Release", controller_method_listener_release, c_dvar_type_unit,
         controller_type_out_unit},
        {"SetPolicy", controller_method_listener_set_policy,
...
static int controller_dispatch_object(Controller* controller, uint32_t serial,
                                      ...
                                      Message* message)
{
    ...
    {
        if (interface &&
            _c_unlikely_(strcmp(interface, "org.bus1.DBus.Broker") != 0))
            return CONTROLLER_E_UNEXPECTED_INTERFACE;

        return controller_dispatch_controller(controller, serial, member, path,
                                              signature, message);
    }
    else if (strncmp(path, "/org/bus1/DBus/Name/",
                     strlen("/org/bus1/DBus/Name/")) == 0)
    {
        if (interface &&
            _c_unlikely_(strcmp(interface, "org.bus1.DBus.Name") != 0))
            return CONTROLLER_E_UNEXPECTED_INTERFACE;

        return controller_dispatch_name(controller, serial, member, path,
                                        signature, message);
    }
    else if (strncmp(path, "/org/bus1/DBus/Listener/",
                     strlen("/org/bus1/DBus/Listener/")) == 0)
    {
        if (interface &&
            _c_unlikely_(strcmp(interface, "org.bus1.DBus.Listener") != 0))
            return CONTROLLER_E_UNEXPECTED_INTERFACE;

        return controller_dispatch_listener(controller, serial, member, path,
                                            signature, message);
    }
...
static int controller_dispatch_reply(Controller* controller, uint32_t serial,
                                     const char* signature, Message* message)
{
...
static int controller_dispatch_error(Controller* controller, uint32_t serial,
                                     const char* error_name, Message* message)
{
    ControllerReload* reload;
    int r;
...
int controller_dbus_dispatch(Controller* controller, Message* message)
{
    ...
    switch (message->header->type)
    {
        case DBUS_MESSAGE_TYPE_METHOD_CALL:
            r = controller_dispatch_object(
                controller, message_read_serial(message),
        ...
        case DBUS_MESSAGE_TYPE_METHOD_RETURN:
            r = controller_dispatch_reply(
                controller, message->metadata.fields.reply_serial,
                message->metadata.fields.signature, message);
            break;
        case DBUS_MESSAGE_TYPE_ERROR:
            r = controller_dispatch_error(
                controller, message->metadata.fields.reply_serial,
                message->metadata.fields.error_name, message);
            break;
        default:
            return CONTROLLER_E_PROTOCOL_VIOLATION;
    }
    ...
    switch (r)
    {
        ...
        case CONTROLLER_E_UNEXPECTED_PATH:
            r = controller_send_error(
                connection, message_read_serial(message),
        ...
        case CONTROLLER_E_UNEXPECTED_SIGNATURE:
            r = controller_send_error(connection, message_read_serial(message),
                                      "org.freedesktop.DBus.Error.InvalidArgs");
            break;
        case CONTROLLER_E_LISTENER_EXISTS:
            r = controller_send_error(connection, message_read_serial(message),
                                      "org.bus1.DBus.Listener.Exists");
            break;
        case CONTROLLER_E_LISTENER_INVALID_FD:
            r = controller_send_error(connection, message_read_serial(message),
                                      "org.bus1.DBus.Listener.InvalidFD");
            break;
        case CONTROLLER_E_LISTENER_INVALID_POLICY:
            r = controller_send_error(connection, message_read_serial(message),
                                      "org.bus1.DBus.Listener.InvalidPolicy");
            break;
        case CONTROLLER_E_NAME_EXISTS:
            r = controller_send_error(connection, message_read_serial(message),
                                      "org.bus1.DBus.Name.Exists");
            break;
        case CONTROLLER_E_NAME_IS_ACTIVATABLE:
            r = controller_send_error(connection, message_read_serial(message),
                                      "org.bus1.DBus.Name.IsActivatable");
            break;
        case CONTROLLER_E_NAME_INVALID:
            r = controller_send_error(connection, message_read_serial(message),
                                      "org.bus1.DBus.Name.Invalid");
            break;
        case CONTROLLER_E_LISTENER_NOT_FOUND:
            r = controller_send_error(connection, message_read_serial(message),
                                      "org.bus1.DBus.Listener.NotFound");
            break;
        case CONTROLLER_E_NAME_NOT_FOUND:
            r = controller_send_error(connection, message_read_serial(message),
                                      "org.bus1.DBus.Name.NotFound");
            break;
        default:
            break;
    }
...
int controller_dbus_send_activation(Controller* controller, const char* path)
{
    ...
    c_dvar_write(&var, "((yyyyuu[(y<o>)(y<s>)(y<s>)])())",
                 ...
                 1, 0, (uint32_t)-1, DBUS_MESSAGE_FIELD_PATH, c_dvar_type_o,
                 path, DBUS_MESSAGE_FIELD_INTERFACE, c_dvar_type_s,
                 "org.bus1.DBus.Name", DBUS_MESSAGE_FIELD_MEMBER, c_dvar_type_s,
                 "Activate");
 ...
 */
int controller_dbus_send_environment(Controller* controller,
                                     const char* const* env, size_t n_env)
{
    static const CDVarType type[] = {
        C_DVAR_T_INIT(CONTROLLER_T_MESSAGE(C_DVAR_T_TUPLE1(
    ...
    c_dvar_begin_write(&var, (__BYTE_ORDER == __BIG_ENDIAN), type, 1);
    c_dvar_write(
        &var, "((yyyyuu[(y<o>)(y<s>)(y<s>)(y<g>)])([",
        c_dvar_is_big_endian(&var) ? 'B' : 'l', DBUS_MESSAGE_TYPE_SIGNAL,
        DBUS_HEADER_FLAG_NO_REPLY_EXPECTED, 1, 0, (uint32_t)-1,
 ...
 */
int controller_dbus_send_reload(Controller* controller, User* user,
                                uint32_t serial)
{
    static const CDVarType type[] = {
        C_DVAR_T_INIT(CONTROLLER_T_MESSAGE(C_DVAR_T_TUPLE0))};
    ...
    c_dvar_write(&var, "((yyyyuu[(y<o>)(y<s>)(y<s>)])())",
                 c_dvar_is_big_endian(&var) ? 'B' : 'l',
                 DBUS_MESSAGE_TYPE_METHOD_CALL, 0, 1, 0, serial,
                 DBUS_MESSAGE_FIELD_PATH, c_dvar_type_o,
                 "/org/bus1/DBus/Controller", DBUS_MESSAGE_FIELD_INTERFACE,
                 c_dvar_type_s, "org.bus1.DBus.Controller",
