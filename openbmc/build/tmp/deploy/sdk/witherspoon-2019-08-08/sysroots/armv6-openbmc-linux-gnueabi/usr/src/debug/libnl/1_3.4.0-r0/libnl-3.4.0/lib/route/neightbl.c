                               ...
                               struct nl_parser_param* pp)
{
    ...
    if (tb[NDTA_CONFIG])
    {
        nla_memcpy(&ntbl->nt_config, tb[NDTA_CONFIG], sizeof(ntbl->nt_config));
        ntbl->ce_mask |= NEIGHTBL_ATTR_CONFIG;
    }

    if (tb[NDTA_STATS])
    {
        nla_memcpy(&ntbl->nt_stats, tb[NDTA_STATS], sizeof(ntbl->nt_stats));
...
static void neightbl_dump_line(struct nl_object* arg, struct nl_dump_params* p)
{
    ...
    if (ntbl->nt_parms.ntp_mask & NEIGHTBLPARM_ATTR_IFINDEX)
    {
        ...
        if (link_cache)
        {
            char buf[32];
            nl_dump(p, "<%s> ",
                    rtnl_link_i2name(link_cache, ntbl->nt_parms.ntp_ifindex,
                                     buf, sizeof(buf)));
                                  ...
                                  struct nl_dump_params* p)
{
    ...
    {
        nl_dump_line(
            p, "    key-len %u entry-size %u last-flush %s\n",
            ntbl->nt_config.ndtc_key_len, ntbl->nt_config.ndtc_entry_size,
            nl_msec2str(ntbl->nt_config.ndtc_last_flush, x, sizeof(x)));
        ...
        nl_dump_line(p, "    hash-rand 0x%08X/0x%08X last-rand %s\n",
                     ...
                     nl_msec2str(ntbl->nt_config.ndtc_last_rand, x, sizeof(x)));
    }
    ...
    if (ntbl->ce_mask & NEIGHTBL_ATTR_PARMS)
    {
        ...
        nl_dump_line(p,
                     ...
                     pa->ntp_refcnt, pa->ntp_queue_len, pa->ntp_proxy_qlen);
        ...
        nl_dump_line(p,
                     ...
                     nl_msec2str(pa->ntp_base_reachable_time, y, sizeof(y)),
...
static void neightbl_dump_stats(struct nl_object* arg, struct nl_dump_params* p)
{
    ...
    nl_dump_line(p,
                 "   "
                 " lookups %" PRIu64 " hits %" PRIu64 " failed %" PRIu64
                 " allocations %" PRIu64 " destroys %" PRIu64 "\n",
                 ntbl->nt_stats.ndts_lookups, ntbl->nt_stats.ndts_hits,
                 ntbl->nt_stats.ndts_res_failed, ntbl->nt_stats.ndts_allocs,
                 ntbl->nt_stats.ndts_destroys);

    nl_dump_line(p,
                 "   "
                 " hash-grows %" PRIu64 " forced-gc-runs %" PRIu64
...
/**
 * Build a neighbour table cache including all neighbour tables currently
                                       ...
                                       struct nl_msg** result)
{
    ...
    if (tmpl->ce_mask & NEIGHTBL_ATTR_PARMS)
    {
        ...
        if (old->nt_parms.ntp_mask & NEIGHTBLPARM_ATTR_IFINDEX)
            NLA_PUT_U32(parms, NDTPA_IFINDEX, old->nt_parms.ntp_ifindex);
        ...
        if (p->ntp_mask & NEIGHTBLPARM_ATTR_UCAST_PROBES)
            NLA_PUT_U32(parms, NDTPA_UCAST_PROBES, p->ntp_ucast_probes);

        if (p->ntp_mask & NEIGHTBLPARM_ATTR_MCAST_PROBES)
            NLA_PUT_U32(parms, NDTPA_MCAST_PROBES, p->ntp_mcast_probes);
        ...
        if (p->ntp_mask & NEIGHTBLPARM_ATTR_RETRANS_TIME)
            NLA_PUT_U64(parms, NDTPA_RETRANS_TIME, p->ntp_retrans_time);

        if (p->ntp_mask & NEIGHTBLPARM_ATTR_GC_STALETIME)
            NLA_PUT_U64(parms, NDTPA_GC_STALETIME, p->ntp_gc_stale_time);

        if (p->ntp_mask & NEIGHTBLPARM_ATTR_DELAY_PROBE_TIME)
            NLA_PUT_U64(parms, NDTPA_DELAY_PROBE_TIME, p->ntp_proxy_delay);

        if (p->ntp_mask & NEIGHTBLPARM_ATTR_ANYCAST_DELAY)
            NLA_PUT_U64(parms, NDTPA_ANYCAST_DELAY, p->ntp_anycast_delay);

        if (p->ntp_mask & NEIGHTBLPARM_ATTR_PROXY_DELAY)
            NLA_PUT_U64(parms, NDTPA_PROXY_DELAY, p->ntp_proxy_delay);
...
/**
 * Set the queue length for pending requests of a neighbour table to the
 * specified value
...
/**
 * Set the queue length for delay proxy arp requests of a neighbour table to the
 * specified value
 * @arg ntbl		neighbour table to change
 * @arg len		new queue len
 */
...
/**
 * Set the number of application probes of a neighbour table to the specified
 * value
 * @arg ntbl		neighbour table to change
...
/**
 * Set the number of multicast probes of a neighbour table to the specified
 * value
 * @arg ntbl		neighbour table to change
 * @arg probes		new probes value
 */
