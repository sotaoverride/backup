...
netsnmp_feature_require(container_lifo)
     ...
     */
    int ipAddressTable_init_data(
        ipAddressTable_registration* ipAddressTable_reg)
{
    DEBUGMSGTL(("verbose:ipAddressTable:ipAddressTable_init_data", "called\n"));
 ...
 */
static void _clear_times(ipAddressTable_rowreq_ctx* rowreq_ctx, void* magic)
{
 ...
 */
void ipAddressTable_container_init(netsnmp_container** container_ptr_ptr,
                                   netsnmp_cache* cache)
{
    DEBUGMSGTL(
        ("verbose:ipAddressTable:ipAddressTable_container_init", "called\n"));
    ...
    *container_ptr_ptr =
    ...
    if (NULL == cache)
    {
        snmp_log(LOG_ERR, "bad cache param to ipAddressTable_container_init\n");
        return;
    }
 ...
 */
void ipAddressTable_container_shutdown(netsnmp_container* container_ptr)
{
    ...
    {
        snmp_log(LOG_ERR, "bad params to ipAddressTable_container_shutdown\n");
        return;
    }
                                     ...
                                     void** magic)
{
    ...
    netsnmp_ipaddress_entry* ipaddress_entry =
        (netsnmp_ipaddress_entry*)CONTAINER_FIND(ipaddress_container,
                                                 rowreq_ctx->data);
    ...
    else
    {
        ...
        /*
         * Remember not to add this entry from 'ipaddress_container' to
         * 'container' later. Simple CONTAINER_REMOVE(ipaddress_container, ..)
         * would be slow.
         */
        if (NULL == to_ignore)
        {
                           ...
                           void** magic)
{
                                    ...
                                    ipaddress_entry->ia_address_len)))
    {
        ...
        rowreq_ctx->ipAddressLastChanged = rowreq_ctx->ipAddressCreated =
 ...
 *
 */
int ipAddressTable_container_load(netsnmp_container* container)
{
    netsnmp_container* ipaddress_container;
    void* tmp_ptr[3];

    DEBUGMSGTL(
        ("verbose:ipAddressTable:ipAddressTable_cache_load", "called\n"));
     ...
     */
    ipaddress_container = netsnmp_access_ipaddress_container_load(
        NULL, NETSNMP_ACCESS_IPADDRESS_LOAD_ADDL_IDX_BY_ADDR);
    /*
    ...
    tmp_ptr[2] =
        NULL; /* list of interfaces to be ignored in ipaddress_container */
    ...
    CONTAINER_FOR_EACH(ipaddress_container,
                       (netsnmp_container_obj_func*)_add_new_entry, tmp_ptr);
    ...
    netsnmp_access_ipaddress_container_free(
        ipaddress_container, NETSNMP_ACCESS_IPADDRESS_FREE_DONT_CLEAR);
    ...
    if (NULL != tmp_ptr[1])
    {
        ...
        while (CONTAINER_SIZE(tmp_container))
        {
             ...
             */
            tmp_ctx =
                (ipAddressTable_rowreq_ctx*)CONTAINER_FIRST(tmp_container);
    ...
    if (NULL != tmp_ptr[2])
    {
        ...
        {
            ipaddress_entry =
                (netsnmp_ipaddress_entry*)CONTAINER_FIRST(to_ignore);
...
{
    DEBUGMSGTL(
        ("verbose:ipAddressTable:ipAddressTable_container_free", "called\n"));
...
{
    DEBUGMSGTL(
        ("verbose:ipAddressTable:ipAddressAddrType_check_index", "called\n"));
 ...
 * If there a no other checks you need to do, simply return MFD_SUCCESS.
 */
int ipAddressAddr_check_index(ipAddressTable_rowreq_ctx* rowreq_ctx)
{
    DEBUGMSGTL(
        ("verbose:ipAddressTable:ipAddressAddr_check_index", "called\n"));
 ...
 */
int ipAddressTable_validate_index(
    ipAddressTable_registration* ipAddressTable_reg,
    ipAddressTable_rowreq_ctx* rowreq_ctx)
{
    int rc = MFD_SUCCESS;
