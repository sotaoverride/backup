                  ...
                  const unsigned char* tbs, size_t tbslen)
{
    ...
    M_check_autoarg(ctx, sig, siglen,
                    EVP_F_EVP_PKEY_SIGN) return ctx->pmeth->sign(ctx, sig,
                                                                 siglen, tbs,
                                                                 tbslen);
}
                            ...
                            size_t siglen)
{
    ...
    M_check_autoarg(ctx, rout, routlen,
                    EVP_F_EVP_PKEY_VERIFY_RECOVER) return ctx->pmeth
        ->verify_recover(ctx, rout, routlen, sig, siglen);
}
                     ...
                     const unsigned char* in, size_t inlen)
{
    ...
    M_check_autoarg(ctx, out, outlen, EVP_F_EVP_PKEY_ENCRYPT) return ctx->pmeth
                     ...
                     const unsigned char* in, size_t inlen)
{
    ...
    M_check_autoarg(ctx, out, outlen, EVP_F_EVP_PKEY_DECRYPT) return ctx->pmeth
...
int EVP_PKEY_derive_set_peer(EVP_PKEY_CTX* ctx, EVP_PKEY* peer)
{
    ...
    if (ctx->operation != EVP_PKEY_OP_DERIVE &&
        ...
        ctx->operation != EVP_PKEY_OP_DECRYPT)
    {
        EVPerr(EVP_F_EVP_PKEY_DERIVE_SET_PEER, EVP_R_OPERATON_NOT_INITIALIZED);
        return -1;
    }
...
int EVP_PKEY_derive(EVP_PKEY_CTX* ctx, unsigned char* key, size_t* pkeylen)
{
    ...
    M_check_autoarg(ctx, key, pkeylen,
                    EVP_F_EVP_PKEY_DERIVE) return ctx->pmeth->derive(ctx, key,
                                                                     pkeylen);
}
