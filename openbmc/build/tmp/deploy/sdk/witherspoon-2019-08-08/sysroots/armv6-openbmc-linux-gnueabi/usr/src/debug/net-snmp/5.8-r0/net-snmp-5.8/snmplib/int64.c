 ...
 */
void divBy10(struct counter64 u64, struct counter64* pu64Q, unsigned int* puR)
{
    unsigned long ulT;
    unsigned long ulQ;
    unsigned long ulR;
 ...
 */
void multBy10(struct counter64 u64, struct counter64* pu64P)
{
    unsigned long ulT;
    unsigned long ulP;
    unsigned long ulK;
 ...
 *
 */
void incrByU16(struct counter64* pu64, unsigned int u16)
{
    incrByU32(pu64, u16);
}
 ...
 *
 */
void incrByU32(struct counter64* pu64, unsigned int u32)
{
    uint32_t tmp;
...
void u64Subtract(const struct counter64* pu64one,
                 const struct counter64* pu64two, struct counter64* pu64out)
{
    int carry;
 ...
 */
void zeroU64(struct counter64* pu64)
{
    pu64->low = 0;
    pu64->high = 0;
}
 ...
 */
int isZeroU64(const struct counter64* pu64)
{
...
int netsnmp_c64_check_for_32bit_wrap(struct counter64* old_val,
                                     struct counter64* new_val, int adjust)
{
     ...
     * check for wraps
     */
    if ((new_val->low >= old_val->low) && (new_val->high == old_val->high))
    {
                                   ...
                                   int* need_wrap_check)
{
    ...
    if (32 == rc)
    {
        ...
        if (1 != new_val->high)
            DEBUGMSGTL(
                ("c64", "error expanding to 64 bits: new_val->high != 1"));
        new_val->high = 0;
    }
    else if (64 == rc)
    {
...
void printU64(char* buf, /* char [I64CHARSZ+1]; */
...
void printI64(char* buf, /* char [I64CHARSZ+1]; */
              const struct counter64* pu64)
{
    struct counter64 u64a;
    ...
    if (pu64->high & 0x80000000)
    {
        ...
        incrByU32(&u64a,
                  1); /* bit invert and incr by 1 to print 2s complement */
        buf[0] = '-';
        printU64(buf + 1, &u64a);
    }
    else
...
int read64(struct counter64* i64, const char* str)
{
