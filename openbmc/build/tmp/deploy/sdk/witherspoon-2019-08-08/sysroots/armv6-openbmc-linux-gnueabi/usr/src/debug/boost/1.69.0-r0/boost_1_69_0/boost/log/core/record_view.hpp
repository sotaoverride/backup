 ...
 * The view is immutable, it is implemented as a wrapper around a
...
class record_view
{
  ...
  private:
    ...
    struct public_data
    {
        ...
        explicit public_data(BOOST_RV_REF(attribute_value_set)
                                 values) BOOST_NOEXCEPT
            : m_ref_counter(1),
  ...
  public:
    /*!
     * Default constructor. Creates an empty record view that is equivalent to
     * the invalid record handle.
     *
     * \post <tt>!*this == true</tt>
     */
    BOOST_CONSTEXPR record_view() BOOST_NOEXCEPT
#if !defined(BOOST_LOG_NO_CXX11_DEFAULTED_NOEXCEPT_FUNCTIONS) &&               \
    !defined(BOOST_LOG_NO_CXX11_DEFAULTED_CONSTEXPR_CONSTRUCTORS)
        = default;
#else
    {
    }
     ...
     * Copy assignment
     */
    record_view& operator=(BOOST_COPY_ASSIGN_REF(record_view)
                               that) BOOST_NOEXCEPT
    {
        m_impl = that.m_impl;
        return *this;
     ...
     * \return \c true if both <tt>*this</tt> and \a that identify the same log
     * record or both do not identify any record, \c false otherwise.
     */
