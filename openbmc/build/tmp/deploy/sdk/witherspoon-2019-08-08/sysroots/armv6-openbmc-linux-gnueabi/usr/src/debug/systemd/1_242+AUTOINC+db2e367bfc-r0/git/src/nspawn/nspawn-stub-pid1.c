...
int stub_pid1(sd_id128_t uuid)
{
    ...
    char new_environment[] = "container=systemd-nspawn\0"
                             "container_uuid=XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX";

    /* Implements a stub PID 1, that reaps all processes and processes a couple
     * of standard signals. This is useful
     * for allowing arbitrary processes run in a container, and still have all
     * zombies reaped. */
    ...
    /* Flush out /proc/self/environ, so that we don't leak the environment from
     * the host into the container. Also, set $container= and $container_uuid=
     * so that clients in the container that query it from /proc/1/environ
     * find them set. */
    sd_id128_to_string(uuid, new_environment + sizeof(new_environment) -
                                 SD_ID128_STRING_MAX);
    ...
    assert_se(sigset_add_many(&waitmask, SIGCHLD, /* posix: process died */
    ...
    /* Note that we ignore SIGTERM (sysv's reexec), SIGHUP (reload), and all
     * other signals here, since we don't
    ...
    for (;;)
    {
        ...
        if (si.si_pid == pid || current_usec >= quit_usec)
        {
            ...
            { /* dispatch a queued reboot */
                ...
                (void)reboot(RB_POWER_OFF); /* if this fails, fall back to
                                               normal exit. */
        ...
        {
            struct timespec ts;
            r = sigtimedwait(&waitmask, &si,
                             timespec_store(&ts, quit_usec - current_usec));
        }
        if (r < 0)
        {
            if (errno == EINTR) /* strace -p attach can result in EINTR, let's
                                   handle this nicely. */
                continue;
        ...
        /* Would love to use a switch() statement here, but SIGRTMIN is actually
         * a function call, not a constant… */
        ...
        /* Let's send a SIGHUP after the SIGTERM, as shells tend to ignore
         * SIGTERM but do react to SIGHUP. We do it strictly in this order, so
         * that the SIGTERM is dispatched first, and SIGHUP second for those
         * processes which handle both. That's because services tend to bind
