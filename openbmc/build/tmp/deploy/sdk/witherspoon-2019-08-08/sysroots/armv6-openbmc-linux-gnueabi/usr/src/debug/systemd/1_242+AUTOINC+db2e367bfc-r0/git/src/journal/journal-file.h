...
{
    /* For all these: -1 means "pick automatically", and 0 means "no limit
     * enforced" */
    uint64_t max_size;  /* how large journal files grow at max */
    uint64_t min_size;  /* how large journal files grow at least */
    uint64_t max_use;   /* how much disk space to use in total at max, keep_free
                           permitting */
    uint64_t min_use;   /* how much disk space to use in total at least, even if
                           keep_free says not to */
...
int journal_file_append_entry(JournalFile* f, const dual_timestamp* ts,
                              ...
                              uint64_t* seqno, Object** ret, uint64_t* offset);
...
int journal_file_enable_post_change_timer(JournalFile* f, sd_event* e,
                                          usec_t t);
