 ...
 */
static krb5_error_code iakerb_save_token(iakerb_ctx_id_t ctx,
 ...
 */
static krb5_error_code iakerb_parse_token(iakerb_ctx_id_t ctx,
                                          int initialContextToken,
                                          const gss_buffer_t token,
                                          krb5_data* realm, krb5_data** cookie,
                                          krb5_data* request)
{
    krb5_error_code code;
    ...
    code = g_verify_token_header(gss_mech_iakerb, &bodysize, &ptr,
 ...
 */
static krb5_error_code iakerb_make_token(iakerb_ctx_id_t ctx, krb5_data* realm,
                                         krb5_data* cookie, krb5_data* request,
                                         int initialContextToken,
                                         gss_buffer_t token)
{
    krb5_error_code code;
 ...
 */
static krb5_error_code iakerb_acceptor_step(iakerb_ctx_id_t ctx,
                                            int initialContextToken,
                                            ...
                                            gss_buffer_t output_token)
{
    ...
    {
        use_master = 0;
        code = krb5_sendto_kdc(ctx->k5c, &request, &realm, &reply, &use_master,
                               tcp_only);
 ...
 */
static krb5_error_code iakerb_init_creds_ctx(iakerb_ctx_id_t ctx,
                                             ...
                                             OM_uint32 time_req)
{
    ...
    code =
        krb5_init_creds_init(ctx->k5c, cred->name->princ, NULL, /* prompter */
                             NULL,                              /* data */
                             0,                                 /* start_time */
                             ctx->gic_opts, &ctx->icc);
    if (code != 0)
        goto cleanup;
    ...
    {
        code =
            krb5_init_creds_set_keytab(ctx->k5c, ctx->icc, cred->client_keytab);
    }
    else
    {
 ...
 */
static krb5_error_code iakerb_tkt_creds_ctx(iakerb_ctx_id_t ctx,
                                            krb5_gss_cred_id_t cred,
...

{
    ...
    {
        code = krb5_authdata_export_authdata(ctx->k5c, cred->name->ad_context,
                                             AD_USAGE_TGS_REQ, &creds.authdata);
        if (code != 0)
            goto cleanup;
    }
 ...
 */
static krb5_error_code iakerb_initiator_step(iakerb_ctx_id_t ctx,
                                             krb5_gss_cred_id_t cred,
                                             krb5_gss_name_t name,
                                             OM_uint32 time_req,
                                             const gss_buffer_t input_token,
                                             gss_buffer_t output_token)
{
    ...
    if (out.length != 0)
    {
        ...
        code =
            iakerb_make_token(ctx, &realm, cookie, &out,
                              (input_token == GSS_C_NO_BUFFER), output_token);
        if (code != 0)
            goto cleanup;
 ...
 */
static krb5_error_code iakerb_get_initial_state(iakerb_ctx_id_t ctx,
                                                krb5_gss_cred_id_t cred,
                                                ...
                                                enum iakerb_state* state)
{
    ...
    {
        code =
            krb5_authdata_export_authdata(ctx->k5c, cred->name->ad_context,
                                          AD_USAGE_TGS_REQ, &in_creds.authdata);
        if (code != 0)
            goto cleanup;
    }
...
static krb5_boolean iakerb_is_iakerb_token(const gss_buffer_t token)
{
    krb5_error_code code;
    ...
    code = g_verify_token_header(gss_mech_iakerb, &bodysize, &ptr,
                                 IAKERB_TOK_PROXY, token->length, 0);

    return (code == 0);
}

static void iakerb_make_exts(iakerb_ctx_id_t ctx, krb5_gss_ctx_ext_rec* exts)
{
    ...
    OM_uint32* time_rec, gss_cred_id_t* delegated_cred_handle)
{
    ...
    if (iakerb_is_iakerb_token(input_token))
    {
        ...
        code = iakerb_acceptor_step(ctx, initialContextToken, input_token,
                                    output_token);
    ...
    else
    {
        ...
        major_status = krb5_gss_accept_sec_context_ext(
            &code, &ctx->gssc, verifier_cred_handle, input_token,
            input_chan_bindings, src_name, NULL, output_token, ret_flags,
            time_rec, delegated_cred_handle, &exts);
    ...
    OM_uint32* time_rec)
{
    ...
    if (initialContextToken)
    {
        ...
        {
            major_status = iakerb_gss_acquire_cred(
                minor_status, NULL, GSS_C_INDEFINITE, GSS_C_NULL_OID_SET,
                GSS_C_INITIATE, &ctx->defcred, NULL, NULL);
    ...
    major_status = kg_cred_resolve(minor_status, ctx->k5c, claimant_cred_handle,
                                   target_name);
    if (GSS_ERROR(major_status))
        goto cleanup;
    cred_locked = TRUE;
    ...
    {
        code =
            iakerb_get_initial_state(ctx, kcred, kname, time_req, &ctx->state);
        if (code != 0)
        {
            *minor_status = code;
            goto cleanup;
    ...
    if (ctx->state != IAKERB_AP_REQ)
    {
        ...
        code = iakerb_initiator_step(ctx, kcred, kname, time_req, input_token,
                                     output_token);
    ...
    if (ctx->state == IAKERB_AP_REQ)
    {
        ...
        major_status = krb5_gss_init_sec_context_ext(
            minor_status, (gss_cred_id_t)kcred, &ctx->gssc, target_name,
            (gss_OID)gss_mech_iakerb, req_flags, time_req, input_chan_bindings,
            input_token, NULL, output_token, ret_flags, time_rec, &exts);
        if (major_status == GSS_S_COMPLETE)
            ctx->established = 1;
...
OM_uint32 KRB5_CALLCONV iakerb_gss_inquire_context(
    ...
    OM_uint32* ctx_flags, int* initiate, int* opened)
{
    OM_uint32 ret;
    iakerb_ctx_id_t ctx = (iakerb_ctx_id_t)context_handle;
    ...
    ret = krb5_gss_inquire_context(minor_status, ctx->gssc, src_name, targ_name,
                                   lifetime_rec, mech_type, ctx_flags, initiate,
                                   opened);
...
OM_uint32 KRB5_CALLCONV iakerb_gss_wrap_iov(
    OM_uint32* minor_status, gss_ctx_id_t context_handle, int conf_req_flag,
...
OM_uint32 KRB5_CALLCONV iakerb_gss_pseudo_random(
    OM_uint32* minor_status, gss_ctx_id_t context_handle, int prf_key,
