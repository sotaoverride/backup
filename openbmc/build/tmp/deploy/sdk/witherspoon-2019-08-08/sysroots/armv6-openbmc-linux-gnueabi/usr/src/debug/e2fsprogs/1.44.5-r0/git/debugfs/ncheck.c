                       ...
                       char* buf EXT2FS_ATTR((unused)), void* private)
{
    ...
    for (i = 0; i < iw->num_inodes; i++)
    {
        if (iw->iarray[i] == dirent->inode)
        {
            ...
            {
                retval =
                    ext2fs_get_pathname(current_fs, iw->dir, 0, &iw->parent);
                if (retval)
                {
                    com_err("ncheck", retval,
                            "while calling ext2fs_get_pathname for inode #%u",
                            iw->dir);
                    iw->get_pathname_failed = 1;
                }
            }
            if (iw->parent)
                printf("%u\t%s/%.*s", iw->iarray[i], iw->parent,
                       ext2fs_dirent_name_len(dirent), dirent->name);
            else
                printf("%u\t<%u>/%.*s", iw->iarray[i], iw->dir,
                       ext2fs_dirent_name_len(dirent), dirent->name);
            if (iw->check_dirent && filetype)
            {
                if (!debugfs_read_inode(dirent->inode, &inode, "ncheck") &&
...
void do_ncheck(int argc, char** argv)
{
    ...
    if (!iw.iarray)
    {
        com_err("ncheck", ENOMEM, "while allocating inode number array");
        return;
    }
    ...
    while (ino)
    {
        ...
        if (retval)
        {
            com_err("ncheck", retval, "while calling ext2_dir_iterate");
            goto next;
        }
    ...
    next:
        ...
        if (retval)
        {
            com_err("ncheck", retval, "while doing inode scan");
            goto error_out;
        }
    }
