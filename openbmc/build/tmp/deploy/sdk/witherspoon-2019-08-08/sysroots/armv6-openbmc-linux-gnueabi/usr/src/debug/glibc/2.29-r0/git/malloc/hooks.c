...
/* Visualize the chunk as being partitioned into blocks of 255 bytes from the
   ...
   corruption.  */
static size_t malloc_check_get_size(mchunkptr p)
{
    size_t size;
    unsigned char c;
    ...
    for (size = chunksize(p) - 1 + (chunk_is_mmapped(p) ? 0 : SIZE_SZ);
         (c = ((unsigned char*)p)[size]) != magic; size -= c)
    {
...
static mchunkptr mem2chunk_check(void* mem, unsigned char** magic_p)
{
    ...
    if (!chunk_is_mmapped(p))
    {
        ...
        if ((contig &&
            ...
            (!prev_inuse(p) &&
             ((prev_size(p) & MALLOC_ALIGN_MASK) != 0 ||
...
static void top_check(void)
{
    ...
    if (t == initial_top(&main_arena) ||
        (!chunk_is_mmapped(t) && chunksize(t) >= MINSIZE && prev_inuse(t) &&
...
/* Dummy implementation which always fails.  We need to provide this
   ...
   BIND_NOW.  */
void* attribute_compat_text_section malloc_get_state(void)
{
    __set_errno(ENOSYS);
    return NULL;
}
...
int attribute_compat_text_section malloc_set_state(void* msptr)
{
