...
#define my_wipememory2(_ptr, _set, _len)                                       \
    do                                                                         \
    {                                                                          \
 ...
 */
static void transform(hmac256_context_t hd, const void* data_arg)
{
...
static void finalize(hmac256_context_t hd)
{
    u32 t, msb, lsb;
    unsigned char* p;
...
void _gcry_hmac256_release(hmac256_context_t ctx)
{
    if (ctx)
    {
...
/* Update the message digest with the contents of BUFFER containing
   LENGTH bytes.  */
void _gcry_hmac256_update(hmac256_context_t hd, const void* buffer,
                          size_t length)
{
...
int main(int argc, char** argv)
{
    ...
    key =
        use_stdkey ? "What am I, a doctor or a moonshuttle conductor?" : *argv;
    argc--, argv++;
    keylen = strlen(key);
    use_stdin = !argc;
