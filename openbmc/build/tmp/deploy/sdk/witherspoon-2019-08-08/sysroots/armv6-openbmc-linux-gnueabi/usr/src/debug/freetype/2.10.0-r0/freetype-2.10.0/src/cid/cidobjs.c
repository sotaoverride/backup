...
cid_size_request(FT_Size size, FT_Size_Request req)
{
    ...
    if (funcs)
        funcs->set_scale((PSH_Globals)size->internal->module_data,
                         size->metrics.x_scale, size->metrics.y_scale, 0, 0);
 ...
 */
FT_LOCAL_DEF(FT_Error)
cid_face_init(FT_Stream stream, FT_Face cidface, /* CID_Face */
              FT_Int face_index, FT_Int num_params, FT_Parameter* params)
{
    ...
    if (!psaux)
    {
        psaux = (PSAux_Service)FT_Get_Module_Interface(FT_FACE_LIBRARY(face),
                                                       "psaux");
