 ...
 * Prototypes
 */
int usm_check_secLevel_vs_protocols(int level, const oid* authProtocol,
                                    u_int authProtocolLen,
                                    const oid* privProtocol,
                                    u_int privProtocolLen);
int usm_calc_offsets(size_t globalDataLen, int secLevel, size_t secEngineIDLen,
                     size_t secNameLen, size_t scopedPduLen, u_long engineboots,
                     long engine_time, size_t* theTotalLength,
                     size_t* authParamsOffset, size_t* privParamsOffset,
                     size_t* dataOffset, size_t* datalen,
                     size_t* msgAuthParmLen, size_t* msgPrivParmLen,
...
int free_enginetime_on_shutdown(int majorid, int minorid, void* serverarg,
                                void* clientarg)
{
...
struct usmStateReference* usm_malloc_usmStateReference(void)
{
    struct usmStateReference* retval =
        (struct usmStateReference*)calloc(1, sizeof(struct usmStateReference));
...
int usm_set_usmStateReference_name(struct usmStateReference* ref, char* name,
                                   size_t name_len)
{
    MAKE_ENTRY(char, name, name_len, usr_name, usr_name_length);
}

int usm_set_usmStateReference_engine_id(struct usmStateReference* ref,
                                        u_char* engine_id, size_t engine_id_len)
{
    MAKE_ENTRY(u_char, engine_id, engine_id_len, usr_engine_id,
               usr_engine_id_length);
}

int usm_set_usmStateReference_auth_protocol(struct usmStateReference* ref,
                                            oid* auth_protocol,
                                            size_t auth_protocol_len)
{
    MAKE_ENTRY(oid, auth_protocol, auth_protocol_len, usr_auth_protocol,
               usr_auth_protocol_length);
}

int usm_set_usmStateReference_auth_key(struct usmStateReference* ref,
                                       u_char* auth_key, size_t auth_key_len)
{
    MAKE_ENTRY(u_char, auth_key, auth_key_len, usr_auth_key,
               usr_auth_key_length);
}

int usm_set_usmStateReference_priv_protocol(struct usmStateReference* ref,
                                            oid* priv_protocol,
                                            size_t priv_protocol_len)
{
    MAKE_ENTRY(oid, priv_protocol, priv_protocol_len, usr_priv_protocol,
               usr_priv_protocol_length);
}

int usm_set_usmStateReference_priv_key(struct usmStateReference* ref,
                                       u_char* priv_key, size_t priv_key_len)
{
    MAKE_ENTRY(u_char, priv_key, priv_key_len, usr_priv_key,
               usr_priv_key_length);
}

int usm_set_usmStateReference_sec_level(struct usmStateReference* ref,
                                        int sec_level)
{
    if (ref == NULL)
        return -1;
                                ...
                                struct usmStateReference** to)
{
    ...
    *to = usm_malloc_usmStateReference();
    ...
    if (usm_set_usmStateReference_name(cloned_usmStateRef, from->usr_name,
                                       from->usr_name_length) ||
        usm_set_usmStateReference_engine_id(cloned_usmStateRef,
                                            from->usr_engine_id,
                                            from->usr_engine_id_length) ||
        usm_set_usmStateReference_auth_protocol(
            cloned_usmStateRef, from->usr_auth_protocol,
            from->usr_auth_protocol_length) ||
        usm_set_usmStateReference_auth_key(cloned_usmStateRef,
                                           from->usr_auth_key,
                                           from->usr_auth_key_length) ||
        usm_set_usmStateReference_priv_protocol(
            cloned_usmStateRef, from->usr_priv_protocol,
            from->usr_priv_protocol_length) ||
        usm_set_usmStateReference_priv_key(cloned_usmStateRef,
                                           from->usr_priv_key,
                                           from->usr_priv_key_length) ||
        usm_set_usmStateReference_sec_level(cloned_usmStateRef,
                                            from->usr_sec_level))
    {
 ...
 *	[12] = theTotalLength
 */
int usm_calc_offsets(
    size_t globalDataLen, /* SNMPv3Message + HeaderData */
    int secLevel, size_t secEngineIDLen, size_t secNameLen,
...
{                 /* Size of msgSecurityP. SEQ.              */
    ...
    *msgPrivParmLen = (secLevel == SNMP_SEC_LEVEL_AUTHPRIV) ? 8 : 0;
     ...
     * Calculate lengths.
     */
    if ((engIDlen = asn_predict_length(ASN_OCTET_STR, NULL, secEngineIDLen)) ==
        -1)
    {
        return -1;
    }

    if ((engBtlen = asn_predict_length(ASN_INTEGER, (u_char*)&engineboots,
                                       sizeof(long))) == -1)
    {
        return -1;
    }

    if ((engTmlen = asn_predict_length(ASN_INTEGER, (u_char*)&engine_time,
                                       sizeof(long))) == -1)
    {
        return -1;
    }

    if ((namelen = asn_predict_length(ASN_OCTET_STR, NULL, secNameLen)) == -1)
    {
        return -1;
    }

    if ((authlen = asn_predict_length(ASN_OCTET_STR, NULL, *msgAuthParmLen)) ==
        -1)
    {
        return -1;
    }

    if ((privlen = asn_predict_length(ASN_OCTET_STR, NULL, *msgPrivParmLen)) ==
        -1)
    {
        return -1;
    }
    ...
    *seq_len = engIDlen + engBtlen + engTmlen + namelen + authlen + privlen;
    ...
    if ((ret = asn_predict_length(ASN_SEQUENCE, NULL, *seq_len)) == -1)
    {
        return -1;
    }
    *otstlen = (size_t)ret;

    if ((ret = asn_predict_length(ASN_OCTET_STR, NULL, *otstlen)) == -1)
    {
        return -1;
    }
    ...
    *privParamsOffset =
        *authParamsOffset + *msgAuthParmLen + (privlen - *msgPrivParmLen);
 ...
 */
int usm_set_salt(u_char* iv, size_t* iv_length, u_char* priv_salt,
                 size_t priv_salt_length, u_char* msgSalt)
{
 ...
 *	The IV result is returned individually for further use.
 */
int usm_set_aes_iv(u_char* iv, size_t* iv_length, u_int net_boots,
                   u_int net_time, u_char* salt)
{
    /*
...
int usm_secmod_generate_out_msg(struct snmp_secmod_outgoing_params* parms)
{
    if (!parms)
        return SNMPERR_GENERR;

    return usm_generate_out_msg(
        parms->msgProcModel, parms->globalData, parms->globalDataLen,
        parms->maxMsgSize, parms->secModel, parms->secEngineID,
        parms->secEngineIDLen, parms->secName, parms->secNameLen,
        parms->secLevel, parms->scopedPdu, parms->scopedPduLen,
        parms->secStateRef, parms->secParams, parms->secParamsLen,
...
{ /* IN/OUT - Len available, len returned. */
     ...
     */
    if (usm_check_secLevel_vs_protocols(theSecLevel, theAuthProtocol,
                                        theAuthProtocolLength, thePrivProtocol,
     ...
     */
    if (get_enginetime(theEngineID, theEngineIDLength, &boots_uint, &time_uint,
                       FALSE) == -1)
    {
    ...
    if (usm_calc_offsets(globalDataLen, theSecLevel, theEngineIDLength,
                         ...
                         &theTotalLength, &authParamsOffset, &privParamsOffset,
                         &dataOffset, &datalen, &msgAuthParmLen,
                         &msgPrivParmLen, &otstlen, &seq_len,
                         &msgSecParmLen) == -1)
    {
    ...
    if (theSecLevel == SNMP_SEC_LEVEL_AUTHPRIV)
    {
        ...
        int priv_type = sc_get_privtype(thePrivProtocol, thePrivProtocolLength);
#ifdef HAVE_AES
        if (USM_CREATE_USER_PRIV_AES == (priv_type & USM_PRIV_MASK_ALG))
        {
            if (!thePrivKey ||
                usm_set_aes_iv(salt, &salt_length, htonl(boots_uint),
                               htonl(time_uint), &ptr[privParamsOffset]) == -1)
            {
        ...
        {
            if (!thePrivKey || (usm_set_salt(salt, &salt_length, thePrivKey + 8,
                                             thePrivKeyLength - 8,
        ...
        if (sc_encrypt(thePrivProtocol, thePrivProtocolLength, thePrivKey,
                       thePrivKeyLength, salt, salt_length, scopedPdu,
                       scopedPduLen, &ptr[dataOffset],
                       &encrypted_length) != SNMP_ERR_NOERROR)
        {
        ...
        {
            dump_chunk("usm/dump", "This data was encrypted:", scopedPdu,
                       scopedPduLen);
            dump_chunk("usm/dump", "salt + Encrypted form:", salt, salt_length);
            dump_chunk("usm/dump", NULL, &ptr[dataOffset], encrypted_length);
            dump_chunk("usm/dump", "*wholeMsg:", *wholeMsg, theTotalLength);
        }
#endif
...
int usm_secmod_rgenerate_out_msg(struct snmp_secmod_outgoing_params* parms)
{
    if (!parms)
        return SNMPERR_GENERR;

    return usm_rgenerate_out_msg(
        parms->msgProcModel, parms->globalData, parms->globalDataLen,
        parms->maxMsgSize, parms->secModel, parms->secEngineID,
        parms->secEngineIDLen, parms->secName, parms->secNameLen,
        parms->secLevel, parms->scopedPdu, parms->scopedPduLen,
        parms->secStateRef, parms->wholeMsg, parms->wholeMsgLen,
        parms->wholeMsgOffset);
}

int usm_rgenerate_out_msg(
    int msgProcModel,   /* (UNUSED) */
    u_char* globalData, /* IN */
    /*
...
)
{
    ...
    {
        DEBUGMSGTL(
            ("usm", "Unsupported Security Level or type (%d)\n", theSecLevel));
    ...
    if (theSecLevel == SNMP_SEC_LEVEL_AUTHPRIV)
    {
        ...
        if (sc_encrypt(thePrivProtocol, thePrivProtocolLength, thePrivKey,
                       thePrivKeyLength, salt, salt_length, scopedPdu,
                       scopedPduLen, ciphertext,
                       &ciphertextlen) != SNMP_ERR_NOERROR)
        {
            DEBUGMSGTL(("usm", "encryption error.\n"));
            usm_free_usmStateReference(secStateRef);
            SNMP_FREE(ciphertext);
        ...
        *offset = 0;
        rc = asn_realloc_rbuild_string(
            ...
            (u_char)(ASN_UNIVERSAL | ASN_PRIMITIVE | ASN_OCTET_STR), ciphertext,
            ciphertextlen);
        if (rc == 0)
        {
        ...
        if (debug_is_token_registered("usm/dump") == SNMPERR_SUCCESS)
        {
            dump_chunk("usm/dump", "salt + Encrypted form: ", salt,
                       salt_length);
            dump_chunk("usm/dump",
                       "wholeMsg:", (*wholeMsg + *wholeMsgLen - *offset),
                       *offset);
        }
#endif
    ...
    rc = asn_realloc_rbuild_string(
        wholeMsg, wholeMsgLen, offset, 1,
        (u_char)(ASN_UNIVERSAL | ASN_PRIMITIVE | ASN_OCTET_STR), iv,
        save_salt_length);
    DEBUGINDENTLESS();
    if (rc == 0)
    {
        ...
        theSecLevel == SNMP_SEC_LEVEL_AUTHPRIV)
    {
        ...
        msgAuthParmLen = sc_get_auth_maclen(
            sc_get_authtype(theAuthProtocol, theAuthProtocolLength));
    }

    rc = asn_realloc_rbuild_string(
        wholeMsg, wholeMsgLen, offset, 1,
        (u_char)(ASN_UNIVERSAL | ASN_PRIMITIVE | ASN_OCTET_STR), authParams,
        msgAuthParmLen);
    DEBUGINDENTLESS();
    if (rc == 0)
    {
     ...
     */
    DEBUGDUMPHEADER("send", "msgAuthoritativeEngineTime");
    rc = asn_realloc_rbuild_int(
        wholeMsg, wholeMsgLen, offset, 1,
        (u_char)(ASN_UNIVERSAL | ASN_PRIMITIVE | ASN_INTEGER), &time_long,
        sizeof(long));
    DEBUGINDENTLESS();
    if (rc == 0)
    {
    ...
    rc = asn_realloc_rbuild_int(
        wholeMsg, wholeMsgLen, offset, 1,
        (u_char)(ASN_UNIVERSAL | ASN_PRIMITIVE | ASN_INTEGER), &boots_long,
        sizeof(long));
    DEBUGINDENTLESS();
    if (rc == 0)
    {
    ...
    rc = asn_realloc_rbuild_string(
        wholeMsg, wholeMsgLen, offset, 1,
        (u_char)(ASN_UNIVERSAL | ASN_PRIMITIVE | ASN_OCTET_STR), theEngineID,
        theEngineIDLength);
    DEBUGINDENTLESS();
    if (rc == 0)
    {
    ...
    *offset += globalDataLen;
        ...
        theSecLevel == SNMP_SEC_LEVEL_AUTHPRIV)
    {
        ...
        if (sc_generate_keyed_hash(theAuthProtocol, theAuthProtocolLength,
                                   theAuthKey, theAuthKeyLength, proto_msg,
                                   proto_msg_len, temp_sig,
                                   &temp_sig_len) != SNMP_ERR_NOERROR)
        {
            SNMP_FREE(temp_sig);
            DEBUGMSGTL(("usm", "Signing failed.\n"));
            usm_free_usmStateReference(secStateRef);
 ...
 *	incoming buffer.
 */
int usm_parse_security_parameters(u_char* secParams, size_t remaining,
                                  u_char* secEngineID, size_t* secEngineIDLen,
                                  u_int* boots_uint, u_int* time_uint,
                                  char* secName, size_t* secNameLen,
                                  u_char* signature, size_t* signature_length,
                                  u_char* salt, size_t* salt_length,
                                  u_char** data_ptr)
{
     ...
     */
    if ((value_ptr =
             asn_parse_sequence(parse_ptr, &octet_string_length, &type_value,
    ...
    if ((value_ptr = asn_parse_sequence(
             parse_ptr, &sequence_length, &type_value,
             (ASN_SEQUENCE | ASN_CONSTRUCTOR), "usm sequence")) == NULL)
    {
        /*
         * RETURN parse error
         */
    ...
    *time_uint = (u_int)time_long;
    ...
    { /* Blanking for authentication step later */
 ...
 */
int usm_check_and_update_timeliness(u_char* secEngineID, size_t secEngineIDLen,
                                    u_int boots_uint, u_int time_uint,
                                    int* error)
{
        ...
        memcmp(secEngineID, myID, myIDLength) == 0)
    {
        u_int time_difference =
            myTime > time_uint ? myTime - time_uint : time_uint - myTime;

        if (boots_uint == ENGINEBOOT_MAX || boots_uint != myBoots ||
    ...
    else
    {
        ...
        if (get_enginetime_ex(secEngineID, secEngineIDLen, &theirBoots,
                              &theirTime, &theirLastTime,
        ...
        time_difference = theirTime > time_uint ? theirTime - time_uint
                                                : time_uint - theirTime;
...
int usm_secmod_process_in_msg(struct snmp_secmod_incoming_params* parms)
{
    if (!parms)
        return SNMPERR_GENERR;

    return usm_process_in_msg(
        parms->msgProcModel, parms->maxMsgSize, parms->secParams,
        parms->secModel, parms->secLevel, parms->wholeMsg, parms->wholeMsgLen,
        parms->secEngineID, parms->secEngineIDLen, parms->secName,
        parms->secNameLen, parms->scopedPdu, parms->scopedPduLen,
        parms->maxSizeResponse, parms->secStateRef, parms->sess,
        parms->msg_flags);
}
...
{ /* IN     - v3 Message flags.              */
    ...
    if ((rc = usm_parse_security_parameters(
             secParams, remaining, secEngineID, secEngineIDLen, &boots_uint,
             &time_uint, secName, secNameLen, signature, &signature_length,
             salt, &salt_length, &data_ptr)) < 0)
    {
    ...
    if (secStateRef)
    {
         ...
         */
        if (usm_set_usmStateReference_name(*secStateRef, secName,
                                           *secNameLen) == -1)
        {
            DEBUGMSGTL(("usm", "%s\n", "Couldn't cache name."));
            return SNMPERR_USM_GENERICERROR;
     ...
     */
    if ((user = usm_get_user_from_list(
             secEngineID, *secEngineIDLen, secName, userList,
             (((sess && sess->isAuthoritative == SNMP_SESS_AUTHORITATIVE) ||
               (!sess))
                  ? 0
                  : 1))) == NULL)
    {
        ...
        secLevel == SNMP_SEC_LEVEL_AUTHPRIV)
    {
        if (sc_check_keyed_hash(user->authProtocol, user->authProtocolLen,
                                user->authKey, user->authKeyLen, wholeMsg,
                                wholeMsgLen, signature,
                                signature_length) != SNMP_ERR_NOERROR)
        {
    ...
    if (secStateRef)
    {
        if (usm_set_usmStateReference_auth_protocol(
                *secStateRef, user->authProtocol, user->authProtocolLen) == -1)
        {
            DEBUGMSGTL(
        ...
        if (usm_set_usmStateReference_auth_key(*secStateRef, user->authKey,
                                               user->authKeyLen) == -1)
        {
        ...
        if (usm_set_usmStateReference_priv_protocol(
                *secStateRef, user->privProtocol, user->privProtocolLen) == -1)
        {
            DEBUGMSGTL(("usm", "%s\n", "Couldn't cache privacy protocol."));
            return SNMPERR_USM_GENERICERROR;
        }

        if (usm_set_usmStateReference_priv_key(*secStateRef, user->privKey,
                                               user->privKeyLen) == -1)
        {
    ...
    {
        set_enginetime(secEngineID, *secEngineIDLen, boots_uint, time_uint,
                       FALSE);
    }
    ...
    {
        int priv_type =
            sc_get_privtype(user->privProtocol, user->privProtocolLen);
        remaining = wholeMsgLen - (data_ptr - wholeMsg);

        if ((value_ptr = asn_parse_sequence(
                 data_ptr, &remaining, &type_value,
                 (ASN_UNIVERSAL | ASN_PRIMITIVE | ASN_OCTET_STR),
                 "encrypted sPDU")) == NULL)
        {
        ...
        if (USM_CREATE_USER_PRIV_DES == (priv_type & USM_PRIV_MASK_ALG))
        {
            ...
            {
                DEBUGMSGTL(("usm",
                            "Ciphertext is %lu bytes, not an integer multiple "
                            "of 8 (rem %lu)\n",
                            (unsigned long)remaining,
                            (unsigned long)remaining % 8));
                snmp_increment_statistic(STAT_USMSTATSDECRYPTIONERRORS);
                usm_free_usmStateReference(*secStateRef);
                *secStateRef = NULL;
                return SNMPERR_USM_DECRYPTIONERROR;
            ...
            {
                DEBUGMSGTL(
                    ("usm", "No privacy pass phrase for %s\n", user->secName));
                snmp_increment_statistic(STAT_USMSTATSDECRYPTIONERRORS);
                usm_free_usmStateReference(*secStateRef);
                *secStateRef = NULL;
                return SNMPERR_USM_DECRYPTIONERROR;
...
#endif
        if (sc_decrypt(user->privProtocol, user->privProtocolLen, user->privKey,
                       user->privKeyLen, iv, iv_length, value_ptr, remaining,
                       *scopedPdu, scopedPduLen) != SNMP_ERR_NOERROR)
        {
            DEBUGMSGTL(("usm", "%s\n", "Failed decryption."));
            snmp_increment_statistic(STAT_USMSTATSDECRYPTIONERRORS);
            return SNMPERR_USM_DECRYPTIONERROR;
        ...
        if (debug_is_token_registered("usm/dump") == SNMPERR_SUCCESS)
        {
            dump_chunk("usm/dump", "Decrypted chunk:", *scopedPdu,
                       *scopedPduLen);
        }
#endif
    }
...
void usm_handle_report(void* sessp, netsnmp_transport* transport,
                       netsnmp_session* session, int result, netsnmp_pdu* pdu)
{
    /*
    ...
    switch (result)
    {
        case SNMPERR_USM_AUTHENTICATIONFAILURE:
        {
            ...
            if (session->callback)
            {
                session->callback(NETSNMP_CALLBACK_OP_RECEIVED_MESSAGE, session,
                                  pdu->reqid, pdu, session->callback_magic);
            }
...
int usm_extend_user_kul(struct usmUser* user, u_int privKeyBufSize)
{
...
int usm_session_init(netsnmp_session* in_session, netsnmp_session* session)
{
    char* cp;
    size_t i;
    ...
    if (in_session->securityAuthProtoLen > 0)
    {
        session->securityAuthProto = snmp_duplicate_objid(
            in_session->securityAuthProto, in_session->securityAuthProtoLen);
    ...
    if (in_session->securityPrivProtoLen > 0)
    {
        session->securityPrivProto = snmp_duplicate_objid(
            in_session->securityPrivProto, in_session->securityPrivProtoLen);
                                     ...
                                     NETSNMP_DS_LIB_AUTHMASTERKEY))))
    {
        ...
        if (!snmp_hex_to_binary(&tmpp, &buflen, &session->securityAuthKeyLen, 0,
                                cp))
        {
                                          ...
                                          NETSNMP_DS_LIB_PASSPHRASE))))
    {
        ...
        if (generate_Ku(session->securityAuthProto,
                        session->securityAuthProtoLen, (u_char*)cp, strlen(cp),
                        session->securityAuthKey,
                        &session->securityAuthKeyLen) != SNMPERR_SUCCESS)
        {
            snmp_set_detail("Error generating a key (Ku) from the supplied "
                                     ...
                                     NETSNMP_DS_LIB_PRIVMASTERKEY))))
    {
        ...
        /* it will be a hex string */
        if (!snmp_hex_to_binary(&tmpp, &buflen, &session->securityPrivKeyLen, 0,
                                cp))
        {
...
int usm_create_user_from_session(netsnmp_session* session)
{
    ...
    if (SNMP_FLAGS_USER_CREATED == (session->flags & SNMP_FLAGS_USER_CREATED) ||
        ...
        session->version != SNMP_VERSION_3 || session->securityNameLen == 0 ||
    ...
    user = usm_get_user_from_list(session->securityEngineID,
                                  ...
                                  session->securityName, usm_get_userList(), 0);
    if (NULL != user)
    ...
    if (user->authProtocol == NULL && session->securityAuthProto != NULL)
    {
        ...
        user->authProtocol = snmp_duplicate_objid(
    ...
    if (user->privProtocol == NULL && session->securityPrivProto != NULL)
    {
        ...
        user->privProtocol = snmp_duplicate_objid(
            session->securityPrivProto, session->securityPrivProtoLen);
...
int usm_create_user_from_session_hook(void* slp, netsnmp_session* session)
{
    DEBUGMSGTL(
...
static int usm_build_probe_pdu(netsnmp_pdu** pdu)
{
    struct usmUser* user;
...
int usm_discover_engineid(void* slpv, netsnmp_session* session)
{
    ...
    if (snmp_get_do_debugging())
    {
        ...
        for (i = 0; i < slp->session->securityEngineIDLen; i++)
            DEBUGMSG(
                ("snmp_sess_open", "%02x", slp->session->securityEngineID[i]));
...
int usm_lookup_alg_type(const char* str, usm_alg_type_t* types)
{
    int i, l;
    l = strlen(str);
...
const char* usm_lookup_alg_str(int value, usm_alg_type_t* types)
{
    int i;
    for (i = 0; types[i].label; ++i)
...
{
    register_config_handler(app, "usmUser", usm_parse_config_usmUser, NULL,
                            NULL);
    register_config_handler(
        app, "createUser", usm_parse_create_usmUser, NULL,
        "username [-e ENGINEID] "
        "(MD5|SHA|SHA-512|SHA-384|SHA-256|SHA-224|default) authpassphrase "
...
#ifndef NETSNMP_DISABLE_MD5
    noNameUser = usm_create_initial_user(
        "", usmHMACMD5AuthProtocol, OID_LENGTH(usmHMACMD5AuthProtocol),
        SNMP_DEFAULT_PRIV_PROTO, SNMP_DEFAULT_PRIV_PROTOLEN);
#else
    noNameUser = usm_create_initial_user(
...
int usm_check_secLevel(int level, struct usmUser* user)
{
    ...
    if ((level == SNMP_SEC_LEVEL_AUTHPRIV ||
        ...
        (netsnmp_oid_equals(user->authProtocol, user->authProtocolLen,
                            usmNoAuthProtocol,
                                    ...
                                    u_int privProtocolLen)
{
    ...
    if (level == SNMP_SEC_LEVEL_AUTHPRIV &&
        (netsnmp_oid_equals(privProtocol, privProtocolLen, usmNoPrivProtocol,
                            sizeof(usmNoPrivProtocol) / sizeof(oid)) == 0))
    {
        DEBUGMSGTL(("usm", "Level: %d\n", level));
    ...
    if ((level == SNMP_SEC_LEVEL_AUTHPRIV ||
         level == SNMP_SEC_LEVEL_AUTHNOPRIV) &&
        (netsnmp_oid_equals(authProtocol, authProtocolLen, usmNoAuthProtocol,
                            sizeof(usmNoAuthProtocol) / sizeof(oid)) == 0))
    {
        DEBUGMSGTL(("usm", "Level: %d\n", level));
        DEBUGMSGTL(("usm", "Auth Protocol: "));
                                     ...
                                     struct usmUser* puserList)
{
         ...
         pptr = nptr, nptr = nptr->next)
    {
        ...
        if (!(nptr->engineID == NULL && user->engineID != NULL))
        {
            if (nptr->engineIDLen == user->engineIDLen &&
                ((nptr->engineID == NULL && user->engineID == NULL) ||
                 memcmp(nptr->engineID, user->engineID, user->engineIDLen) ==
                     0) &&
...
struct usmUser* usm_free_user(struct usmUser* user)
{
    ...
    if (user->next != NULL)
    {
        ...
        if (user->prev !=
            NULL) /* ack this is really bad, because it means
...
struct usmUser* usm_cloneFrom_user(struct usmUser* from, struct usmUser* to)
{
    ...
    if ((to->authProtocol = snmp_duplicate_objid(
             from->authProtocol, from->authProtocolLen)) != NULL)
    ...
    if (from->authKeyLen > 0 &&
        (to->authKey = (u_char*)malloc(from->authKeyLen)) != NULL)
    {
    ...
    if ((to->privProtocol = snmp_duplicate_objid(
             from->privProtocol, from->privProtocolLen)) != NULL)
    ...
    if (from->privKeyLen > 0 &&
        (to->privKey = (u_char*)malloc(from->privKeyLen)) != NULL)
    {
...
struct usmUser* usm_create_user(void)
{
     ...
     */
    if ((newUser->authProtocol = snmp_duplicate_objid(
             usmNoAuthProtocol, sizeof(usmNoAuthProtocol) / sizeof(oid))) ==
        NULL)
        return usm_free_user(newUser);
    newUser->authProtocolLen = sizeof(usmNoAuthProtocol) / sizeof(oid);

    if ((newUser->privProtocol = snmp_duplicate_objid(
             usmNoPrivProtocol, sizeof(usmNoPrivProtocol) / sizeof(oid))) ==
        NULL)
        return usm_free_user(newUser);
                                        ...
                                        size_t privProtocolLen)
{
    ...
    if ((newUser->privProtocol =
             snmp_duplicate_objid(privProtocol, privProtocolLen)) == NULL)
    {
        return usm_free_user(newUser);
    }
    ...
    if ((newUser->authProtocol =
             snmp_duplicate_objid(authProtocol, authProtocolLen)) == NULL)
    {
        return usm_free_user(newUser);
    }
 ...
 */
int usm_store_users(int majorID, int minorID, void* serverarg, void* clientarg)
{
    /*
...
void usm_save_users_from_list(struct usmUser* puserList, const char* token,
                              const char* type)
{
    struct usmUser* uptr;
...
void usm_save_user(struct usmUser* user, const char* token, const char* type)
{
    ...
    cptr =
        read_config_save_octet_string(cptr, user->engineID, user->engineIDLen);
    *cptr++ = ' ';
    cptr = read_config_save_octet_string(
        cptr, (u_char*)user->name,
        (user->name == NULL) ? 0 : strlen(user->name));
    *cptr++ = ' ';
    cptr = read_config_save_octet_string(
        cptr, (u_char*)user->secName,
        (user->secName == NULL) ? 0 : strlen(user->secName));
    *cptr++ = ' ';
    cptr = read_config_save_objid(cptr, user->cloneFrom, user->cloneFromLen);
    *cptr++ = ' ';
    cptr =
        read_config_save_objid(cptr, user->authProtocol, user->authProtocolLen);
    *cptr++ = ' ';
    cptr = read_config_save_octet_string(cptr, user->authKey, user->authKeyLen);
    *cptr++ = ' ';
    cptr =
        read_config_save_objid(cptr, user->privProtocol, user->privProtocolLen);
    *cptr++ = ' ';
...
struct usmUser* usm_read_user(const char* line)
{
    ...
    line = read_config_read_octet_string(line, (u_char**)&user->name, &len);
    line = read_config_read_octet_string(line, (u_char**)&user->secName, &len);
    SNMP_FREE(user->cloneFrom);
    user->cloneFromLen = 0;
    ...
    line =
        read_config_read_octet_string(line, &user->privKey, &user->privKeyLen);
 ...
 */
void usm_set_user_password(struct usmUser* user, const char* token, char* line)
{
    char* cp = line;
    ...
    if (type < 2)
    {
        ...
        *keyLen = SNMP_MAXBUF_SMALL;
        ret = generate_kul(user->authProtocol, user->authProtocolLen, engineID,
                           engineIDLen, userKey, userKeyLen, *key, keyLen);
        if (ret != SNMPERR_SUCCESS)
        {
    ...
    *errorMsg = NULL; /* no errors yet */
    ...
    if ((strncmp(cp, "default", 7) == 0) && (NULL != def_auth_prot))
    {
        ...
        newuser->authProtocol =
            snmp_duplicate_objid(def_auth_prot, def_auth_prot_len);
    ...
    else if (strcmp(buf, "-l") != 0)
    {
        ...
        {
            *errorMsg = "could not generate the authentication key from the "
    ...
    if (strcmp(buf, "-l") == 0)
    {
        ...
        if (!snmp_hex_to_binary(&newuser->authKey, &ret, &newuser->authKeyLen,
                                0, buf))
        {
    ...
    else
    {
        ret2 = generate_kul(newuser->authProtocol, newuser->authProtocolLen,
                            newuser->engineID, newuser->engineIDLen, userKey,
                            userKeyLen, newuser->authKey, &newuser->authKeyLen);
        if (ret2 != SNMPERR_SUCCESS)
        {
            *errorMsg = "could not generate localized authentication key (Kul) "
    ...
    if (!cp)
    {
         ...
         */
        newuser->privKey =
            netsnmp_memdup(newuser->authKey, newuser->authKeyLen);
        privKeySize = newuser->privKeyLen = newuser->authKeyLen;
        if (newuser->flags & USMUSER_FLAG_KEEP_MASTER_KEY)
        {
            newuser->privKeyKu =
                netsnmp_memdup(newuser->authKeyKu, newuser->authKeyKuLen);
    ...
    else
    {
        ...
        else if (strcmp(buf, "-l") != 0)
        {
            ...
            if (ret2 != SNMPERR_SUCCESS)
            {
                *errorMsg = "could not generate the privacy key from the "
                            "supplied pass phrase.";
                goto fail;
            }
            /* save master key */
        ...
        else
        {
            ret2 = generate_kul(newuser->authProtocol, newuser->authProtocolLen,
                                ...
                                userKey, userKeyLen, newuser->privKey,
                                &newuser->privKeyLen);
            if (ret2 != SNMPERR_SUCCESS)
            {
                *errorMsg = "could not generate localized privacy key (Kul) "
                            "from the master key (Ku).";
                goto fail;
            }
        }
        ...
        if (newuser->privKeyLen < properPrivKeyLen)
        {
            ...
            if (ret != SNMPERR_SUCCESS)
            {
                *errorMsg = "could not extend localized privacy key to "
                            "required length.";
                goto fail;
            }
        }
    ...
    {
        DEBUGMSGTL(("9:usmUser",
                    "truncating privKeyLen from %" NETSNMP_PRIz "d to %d\n",
    ...
    {
        DEBUGMSGTL(("usmUser",
                    "privKey length %" NETSNMP_PRIz
                    "d < %d required by privProtocol\n",
                    newuser->privKeyLen, properPrivKeyLen));
...
struct usmUser* usm_create_usmUser(const char* userName, const char* engineID,
                                   u_int flags, int authType,
                                   const char* authPass, int privType,
                                   const char* privPass, const char** errorMsg)
{
...
const oid* get_default_authtype(size_t* len)
{
...
const oid* get_default_privtype(size_t* len)
{
