...
#define fdisk_parttype_is_invisible(_x)                                        \
    ((_x) && ((_x)->flags & FDISK_PARTTYPE_INVISIBLE))
#define fdisk_parttype_is_allocated(_x)                                        \
    ((_x) && ((_x)->flags & FDISK_PARTTYPE_ALLOCATED))
...
struct fdisk_label_operations
{
    ...
    int (*part_toggle_flag)(struct fdisk_context* cxt, size_t i,
                            unsigned long flag);
...
struct fdisk_ask
{
    ...
    union
    {
        ...
        struct ask_number
        {
            ...
            unsigned int relative : 1, inchars : 1, wrap_negative : 1;
        } num;
...
int fdisk_ask_number_set_wrap_negative(struct fdisk_ask* ask,
                                       int wrap_negative);
...
/* dos.c */
extern struct dos_partition* fdisk_dos_get_partition(struct fdisk_context* cxt,
                                                     size_t i);

/* wipe.c */
void fdisk_free_wipe_areas(struct fdisk_context* cxt);
int fdisk_set_wipe_area(struct fdisk_context* cxt, uint64_t start,
                        uint64_t size, int enable);
int fdisk_do_wipe(struct fdisk_context* cxt);
int fdisk_has_wipe_area(struct fdisk_context* cxt, uint64_t start,
                        uint64_t size);
