...
template <class T>
inline std::string string_stub_dump(T const& x)
{
...
{
    return exception_detail::to_string_dispatch::dispatch(
        x, &exception_detail::string_stub_dump<T>);
}
...
inline std::string to_string_stub(std::pair<T, U> const& x, Stub s)
{
    return std::string("(") + to_string_stub(x.first, s) + ',' +
           to_string_stub(x.second, s) + ')';
}
} // namespace boost
