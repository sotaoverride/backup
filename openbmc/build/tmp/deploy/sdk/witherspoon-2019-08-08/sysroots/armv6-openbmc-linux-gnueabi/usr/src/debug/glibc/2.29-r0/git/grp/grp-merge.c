               ...
               struct group* destgrp, char* destbuf, char** endptr)
{
    ...
    {
        uintptr_t mis_align =
            ((uintptr_t)destbuf + c) & (__alignof__(char**) - 1);
                    ...
                    size_t buflen, struct group* mergegrp, char* mergebuf)
{
    ...
    /* Back up into the savedbuf until we get back to the NULL-terminator of the
       group member list. (This means walking back savedmemcount + 1 (char *)
       pointers and the member count value. The value of c is going to be the
       used length of the buffer backed up by the member count and further
       backed up by the size of the pointers.  */
    c = savedend - savedbuf - sizeof(size_t) -
    ...
    {
        uintptr_t mis_align =
            ((uintptr_t)savedbuf + c) & (__alignof__(char**) - 1);
        c += __alignof__(char**) - mis_align;
    }
