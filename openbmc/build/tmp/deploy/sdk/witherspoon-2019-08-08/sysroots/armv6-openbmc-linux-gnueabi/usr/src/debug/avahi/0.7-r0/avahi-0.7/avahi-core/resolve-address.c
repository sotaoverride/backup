...
static void finish(AvahiSAddressResolver* r, AvahiResolverEvent event)
{
    ...
    switch (event)
    {
        ...
        case AVAHI_RESOLVER_FOUND:
            assert(r->ptr_record);
            r->callback(r, r->interface, r->protocol, event, &r->address,
                        r->ptr_record->data.ptr.name, r->flags, r->userdata);
            break;
    }
}
                            ...
                            void* userdata)
{
    ...
    switch (event)
    {
        ...
        case AVAHI_BROWSER_REMOVE:
            ...
            if (r->ptr_record &&
                avahi_record_equal_no_ttl(record, r->ptr_record))
            {
        ...
        case AVAHI_BROWSER_FAILURE:
            ...
            if (r->retry_with_multicast)
            {
                ...
                r->record_browser = avahi_s_record_browser_new(
                    r->server, r->interface, r->protocol, r->key,
                    AVAHI_LOOKUP_USE_MULTICAST, record_browser_callback, r);
...
AvahiSAddressResolver* avahi_s_address_resolver_new(
    ...
    const AvahiAddress* address, AvahiLookupFlags flags,
    AvahiSAddressResolverCallback callback, void* userdata)
{
    ...
    AVAHI_LLIST_PREPEND(AvahiSAddressResolver, resolver,
                        server->address_resolvers, r);
...
void avahi_s_address_resolver_free(AvahiSAddressResolver* r)
{
    ...
    AVAHI_LLIST_REMOVE(AvahiSAddressResolver, resolver,
                       r->server->address_resolvers, r);
