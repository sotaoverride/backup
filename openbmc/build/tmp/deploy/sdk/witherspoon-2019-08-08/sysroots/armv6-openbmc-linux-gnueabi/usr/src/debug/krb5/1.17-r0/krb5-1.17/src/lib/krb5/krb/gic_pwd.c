...
krb5_error_code
    krb5_get_as_key_password(krb5_context context, krb5_principal client,
                             ...
                             void* gak_data, k5_response_items* ritems)
{
    ...
    if (as_key == NULL)
    {
        ...
        return k5_response_items_ask_question(
            ritems, KRB5_RESPONDER_QUESTION_PASSWORD, "");
    }
    ...
    if (gp->password == NULL)
    {
        ...
        snprintf(promptstr, sizeof(promptstr), _("Password for %s"), clientstr);
        free(clientstr);
...
static void get_expiry_times(krb5_enc_kdc_rep_part* enc_part2,
                             krb5_timestamp* pw_exp, krb5_timestamp* acct_exp,
 ...
 * instead.
 */
static void warn_pw_expiry(krb5_context context,
 ...
 */
static krb5_error_code make_chpw_options(krb5_context context,
                                         krb5_get_init_creds_opt* in,
    ...
    krb5_get_init_creds_opt* options)
{
    ...
    if (!use_master)
    {
        ...
        ret = k5_get_init_creds(
            context, creds, client, prompter, data, start_time, in_tkt_service,
            options, krb5_get_as_key_password, &gakpw, &use_master, &as_reply);
    ...
    if (ret)
        goto cleanup;
    ret =
        k5_get_init_creds(context, &chpw_creds, client, prompter, data,
                          start_time, "kadmin/changepw", chpw_opts,
                          krb5_get_as_key_password, &gakpw, &use_master, NULL);
    if (ret)
        goto cleanup;
...
krb5_error_code KRB5_CALLCONV krb5_get_in_tkt_with_password(
    ...
    const char* password, krb5_ccache ccache, krb5_creds* creds,
    krb5_kdc_rep** ret_as_reply)
{
    ...
    retval = k5_get_init_creds(
        context, creds, creds->client, krb5_prompter_posix, NULL, 0, server,
        opts, krb5_get_as_key_password, &gakpw, &use_master, ret_as_reply);
