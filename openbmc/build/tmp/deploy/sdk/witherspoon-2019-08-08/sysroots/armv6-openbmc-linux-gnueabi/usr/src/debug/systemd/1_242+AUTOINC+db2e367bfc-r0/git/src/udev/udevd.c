...
DEFINE_PRIVATE_HASH_OPS_WITH_VALUE_DESTRUCTOR(worker_hash_op, void,
                                              trivial_hash_func,
                                              trivial_compare_func,
                                              struct worker, worker_free);

static int worker_new(struct worker** ret, Manager* manager,
                      sd_device_monitor* worker_monitor, pid_t pid)
{
...
static int on_event_timeout(sd_event_source* s, uint64_t usec, void* userdata)
{
    ...
    log_device_error(
        event->dev, "Worker [" PID_FMT "] processing SEQNUM=%" PRIu64 " killed",
        event->worker->pid, event->seqnum);

    return 1;
}

static int on_event_timeout_warning(sd_event_source* s, uint64_t usec,
                                    void* userdata)
{
    struct event* event = userdata;
    ...
    log_device_warning(event->dev,
                       "Worker [" PID_FMT "] processing SEQNUM=%" PRIu64
...
static int worker_lock_block_device(sd_device* dev, int* ret_fd)
{
    ...
    if (fd < 0)
    {
        log_device_debug_errno(dev, errno, "Failed to open '%s', ignoring: %m",
                               val);
        return 0;
    }

    if (flock(fd, LOCK_SH | LOCK_NB) < 0)
        return log_device_debug_errno(dev, errno, "Failed to flock(%s): %m",
                                      val);
...
static int worker_process_device(Manager* manager, sd_device* dev)
{
    ...
    udev_event_execute_rules(udev_event, arg_event_timeout_usec,
                             manager->properties, manager->rules);
...
static int worker_device_monitor_handler(sd_device_monitor* monitor,
                                         sd_device* dev, void* userdata)
{
    ...
    if (r < 0)
        log_device_warning_errno(
            dev, r, "Failed to send signal to main daemon, ignoring: %m");

    return 1;
}

static int worker_main(Manager* _manager, sd_device_monitor* monitor,
                       sd_device* first_device)
{
    ...
    if (r < 0)
        return log_error_errno(
            r, "Failed to attach event loop to device monitor: %m");

    r = sd_device_monitor_start(monitor, worker_device_monitor_handler,
                                manager);
    if (r < 0)
        return log_error_errno(r, "Failed to start device monitor: %m");

    (void)sd_event_source_set_description(
        sd_device_monitor_get_event_source(monitor), "worker-device-monitor");
...
{
    _cleanup_(sd_device_monitor_unrefp) sd_device_monitor* worker_monitor =
        NULL;
    struct worker* worker;
    pid_t pid;
    int r;
    ...
    if (r < 0)
        return log_error_errno(
            r, "Worker: Failed to enable receiving of device: %m");
...
static void event_run(Manager* manager, struct event* event)
{
    ...
    if (hashmap_size(manager->workers) >= arg_children_max)
    {
        if (arg_children_max > 1)
            log_debug("Maximum number (%u) of children reached.",
                      hashmap_size(manager->workers));
        return;
    }
...
static int is_device_busy(Manager* manager, struct event* event)
{
    ...
    LIST_FOREACH(event, loop_event, manager->events)
    {
        ...
        /* we already found a later event, earlier cannot block us, no need to
         * check again */
...
static void manager_exit(Manager* manager)
{
    ...
    r = sd_event_add_time(manager->event, NULL, CLOCK_MONOTONIC,
                          usec + 30 * USEC_PER_SEC, USEC_PER_SEC,
...
static int on_kill_workers_event(sd_event_source* s, uint64_t usec,
                                 void* userdata)
{
    Manager* manager = userdata;
...
static void event_queue_start(Manager* manager)
{
    ...
    if (LIST_IS_EMPTY(manager->events) || manager->exit ||
    ...
    if (r < 0)
        log_warning_errno(r, "Failed to disable event source for cleaning up "
                             "idle workers, ignoring: %m");
...
static int on_worker(sd_event_source* s, int fd, uint32_t revents,
                     void* userdata)
{
    Manager* manager = userdata;
    ...
    for (;;)
    {
        ...
        {
            log_warning("Ignoring worker message with invalid size %zi bytes",
                        size);
            continue;
        }
        ...
        if (!worker)
        {
            log_debug("Worker [" PID_FMT "] returned, but is no longer tracked",
                      ucred->pid);
            continue;
        }
...
static int on_ctrl_msg(struct udev_ctrl* uctrl, enum udev_ctrl_msg_type type,
                       const union udev_ctrl_msg_value* value, void* userdata)
{
    ...
    switch (type)
    {
        case UDEV_CTRL_SET_LOG_LEVEL:
            log_debug("Received udev control message (SET_LOG_LEVEL), setting "
                      "log_priority=%i",
                      value->intval);
        ...
        case UDEV_CTRL_SET_ENV:
        {
            ...
            old_val =
                hashmap_remove2(manager->properties, key, (void**)&old_key);

            r = hashmap_ensure_allocated(&manager->properties,
                                         &string_hash_ops);
            if (r < 0)
            {
                log_oom();
            ...
            if (!isempty(eq))
            {
                log_debug("Received udev control message (ENV), unsetting '%s'",
                          key);
            ...
            else
            {
                ...
                log_debug(
                    "Received udev control message (ENV), setting '%s=%s'", key,
                    val);
        ...
        case UDEV_CTRL_SET_CHILDREN_MAX:
            if (value->intval <= 0)
            {
                log_debug("Received invalid udev control message "
                          "(SET_MAX_CHILDREN, %i), ignoring.",
                          value->intval);
                return 0;
            }

            log_debug("Received udev control message (SET_MAX_CHILDREN), "
                      "setting children_max=%i",
                      value->intval);
...
static int synthesize_change(sd_device* dev)
{
    ...
    if (streq_ptr("block", subsystem) && streq_ptr("disk", devtype) &&
        !startswith(sysname, "dm-"))
    {
        ...
        FOREACH_DEVICE(e, d)
        {
            ...
            log_debug(
                "Device '%s' is closed, synthesising partition '%s' 'change'",
                devname, n);
...
static int on_inotify(sd_event_source* s, int fd, uint32_t revents,
                      void* userdata)
{
    Manager* manager = userdata;
...
static int on_sigterm(sd_event_source* s, const struct signalfd_siginfo* si,
                      void* userdata)
{
    Manager* manager = userdata;
...
static int on_sighup(sd_event_source* s, const struct signalfd_siginfo* si,
                     void* userdata)
{
    Manager* manager = userdata;
                      ...
                      void* userdata)
{
    ...
    for (;;)
    {
        ...
        else if (WIFCONTINUED(status))
        {
            ...
            log_warning("Worker [" PID_FMT "] exit with status 0x%04x", pid,
                        status);

        if ((!WIFEXITED(status) || WEXITSTATUS(status) != 0) && worker->event)
        {
            log_device_error(worker->event->dev, "Worker [" PID_FMT "] failed",
                             pid);
            ...
            r = device_monitor_send_device(manager->monitor, NULL,
                                           worker->event->dev_kernel);
            if (r < 0)
                log_device_error_errno(
                    worker->event->dev_kernel, r,
    ...
    if (hashmap_isempty(manager->workers))
    {
        r = event_source_disable(manager->kill_workers_event);
        if (r < 0)
            log_warning_errno(r, "Failed to disable event source for cleaning "
                                 "up idle workers, ignoring: %m");
    }
...
static int on_post(sd_event_source* s, void* userdata)
{
    ...
    if (!hashmap_isempty(manager->workers))
    {
        ...
        (void)event_reset_time(
            manager->event, &manager->kill_workers_event, CLOCK_MONOTONIC,
    ...
    if (manager->cgroup)
        ...
        (void)cg_kill(SYSTEMD_CGROUP_CONTROLLER, manager->cgroup, SIGKILL,
                      CGROUP_IGNORE_SELF, NULL, NULL, NULL);
 ...
 *   udev.children_max=<number of workers>     events are fully serialized if
 * set to 1 udev.exec_delay=<number of seconds>       delay execution of every
 * executed program udev.event_timeout=<number of seconds>    seconds to wait
 * before terminating an event
 */
                                   ...
                                   void* data)
{
    ...
    else if (proc_cmdline_key_streq(key, "udev.exec_delay"))
    {
        ...
        log_warning("Unknown udev kernel command line option \"%s\", ignoring",
                    key);

    if (r < 0)
        log_warning_errno(r, "Failed to parse \"%s=%s\", ignoring: %m", key,
                          value);
...
static int help(void)
{
    ...
    printf(
        ...
        "  -t --event-timeout=SECONDS  Seconds to wait before terminating an "
        "event\n"
    ...
    while ((c = getopt_long(argc, argv, "c:de:Dt:N:hV", options, NULL)) >= 0)
    {
        switch (c)
        {
            ...
            case 'c':
                ...
                if (r < 0)
                    log_warning_errno(r,
                                      "Failed to parse --children-max= value "
                                      "'%s', ignoring: %m",
                                      optarg);
                break;
            case 'e':
                ...
                if (r < 0)
                    log_warning_errno(r,
                                      "Failed to parse --exec-delay= value "
                                      "'%s', ignoring: %m",
                                      optarg);
                break;
            case 't':
                ...
                if (r < 0)
                    log_warning_errno(r,
                                      "Failed to parse --event-timeout= value "
                                      "'%s', ignoring: %m",
                                      optarg);
                break;
            case 'D':
            ...
            case 'N':
            {
                ...
                if (t < 0)
                    log_warning(
                        "Invalid --resolve-names= value '%s', ignoring.",
                        optarg);
                else
                       ...
                       const char* cgroup)
{
    ...
    *manager = (Manager){
    ...
    r = device_monitor_new_full(&manager->monitor, MONITOR_GROUP_KERNEL,
                                fd_uevent);
    if (r < 0)
        return log_error_errno(r, "Failed to initialize device monitor: %m");

    (void)sd_device_monitor_set_receive_buffer_size(manager->monitor,
                                                    128 * 1024 * 1024);
...
static int main_loop(Manager* manager)
{
    ...
    r = socketpair(AF_LOCAL, SOCK_DGRAM | SOCK_CLOEXEC, 0,
                   manager->worker_watch);
    if (r < 0)
        return log_error_errno(
            errno,
    ...
    assert_se(sigprocmask_many(SIG_BLOCK, NULL, SIGTERM, SIGINT, SIGHUP,
                               SIGCHLD, -1) >= 0);
     ...
     */
    r = sd_event_source_set_priority(udev_ctrl_get_event_source(manager->ctrl),
                                     SD_EVENT_PRIORITY_IDLE);
    if (r < 0)
        return log_error_errno(r, "Failed to set IDLE event priority for udev "
                                  "control event source: %m");

    r = sd_event_add_io(manager->event, &manager->inotify_event,
    ...
    (void)sd_event_source_set_description(
        sd_device_monitor_get_event_source(manager->monitor), "device-monitor");

    r = sd_event_add_io(manager->event, NULL, fd_worker, EPOLLIN, on_worker,
                        manager);
    if (r < 0)
    ...
    if (r < 0)
        log_error_errno(
            r, "Failed to apply permissions on static device nodes: %m");
...
static int run(int argc, char* argv[])
{
    ...
    udev_parse_config_full(&arg_children_max, &arg_exec_delay_usec,
                           &arg_event_timeout_usec, &arg_resolve_name_timing);
    ...
    {
        /* Get our own cgroup, we regularly kill everything udev has left
         * behind. We only do this on systemd systems, and only if we are
         * directly spawned by PID1. Otherwise we are not guaranteed to have a
         * dedicated cgroup. */
    ...
    if (arg_daemonize)
    {
        ...
        if (log_get_max_level() < LOG_DEBUG)
        {
            ...
            if (r < 0)
                log_warning_errno(
                    r, "Failed to redirect standard streams to /dev/null: %m");
        }
