...
void manager_mdns_stop(Manager* m)
{
    ...
    m->mdns_ipv4_event_source =
        sd_event_source_unref(m->mdns_ipv4_event_source);
    m->mdns_ipv4_fd = safe_close(m->mdns_ipv4_fd);

    m->mdns_ipv6_event_source =
        sd_event_source_unref(m->mdns_ipv6_event_source);
...
eaddrinuse:
    log_warning("Another mDNS responder prohibits binding the socket to the "
                "same port. Turning off mDNS support.");
...
static int mdns_rr_compare(DnsResourceRecord* const* a,
                           DnsResourceRecord* const* b)
{
    DnsResourceRecord *x = *(DnsResourceRecord**)a,
                      *y = *(DnsResourceRecord**)b;
    size_t m;
    int r;
...
static int proposed_rrs_cmp(DnsResourceRecord** x, unsigned x_size,
                            DnsResourceRecord** y, unsigned y_size)
{
    unsigned m;
    int r;
...
static int mdns_packet_extract_matching_rrs(DnsPacket* p, DnsResourceKey* key,
                                            DnsResourceRecord*** ret_rrs)
{
    ...
    for (size_t i = DNS_PACKET_ANCOUNT(p);
         i < (DNS_PACKET_ANCOUNT(p) + DNS_PACKET_NSCOUNT(p)); i++)
    {
...
static int mdns_do_tiebreak(DnsResourceKey* key, DnsAnswer* answer,
                            DnsPacket* p)
{
...
static int mdns_scope_process_query(DnsScope* s, DnsPacket* p)
{
    ...
    r = dns_scope_make_reply_packet(s, DNS_PACKET_ID(p), DNS_RCODE_SUCCESS,
                                    NULL, full_answer, NULL, false, &reply);
    if (r < 0)
...
static int on_mdns_packet(sd_event_source* s, int fd, uint32_t revents,
                          void* userdata)
{
    ...
    if (dns_packet_validate_reply(p) > 0)
    {
        ...
        /*
         * mDNS is different from regular DNS and LLMNR with regard to handling
         * responses. While on other protocols, we can ignore every answer that
         * doesn't match a question we broadcast earlier, RFC6762, section 18.1
         * recommends looking at and caching all incoming information,
         * regardless of the DNS packet ID.
         *
         * Hence, extract the packet here, and try to find a transaction for
         * answer the we got and complete it. Also store the new information in
         * scope's cache.
         */
        r = dns_packet_extract(p);
        if (r < 0)
        ...
        DNS_ANSWER_FOREACH(rr, p->answer)
        {
            ...
            /* If the received reply packet contains ANY record that is not
             * .local or .in-addr.arpa,
             * we assume someone's playing tricks on us and discard the packet
             * completely. */
            ...
            t = dns_scope_find_transaction(
                scope,
                &DNS_RESOURCE_KEY_CONST(rr->key->class, DNS_TYPE_ANY,
            ...
            t = dns_scope_find_transaction(
                scope,
                &DNS_RESOURCE_KEY_CONST(DNS_CLASS_ANY, rr->key->type,
                                        dns_resource_key_name(rr->key)),
                false);
            if (t)
                dns_transaction_process_reply(t, p);
            ...
            t = dns_scope_find_transaction(
                scope,
                &DNS_RESOURCE_KEY_CONST(DNS_CLASS_ANY, DNS_TYPE_ANY,
                                        dns_resource_key_name(rr->key)),
                false);
            if (t)
                dns_transaction_process_reply(t, p);
        ...
        dns_cache_put(&scope->cache, NULL, DNS_PACKET_RCODE(p), p->answer,
                      false, (uint32_t)-1, 0, p->family, &p->sender);
    }
...
int manager_mdns_ipv4_fd(Manager* m)
{
    ...
    if (r < 0)
        return log_error_errno(
            r, "mDNS-IPv4: Failed to set IP_MULTICAST_LOOP: %m");
    ...
    if (r < 0)
    {
        ...
        log_warning("mDNS-IPv4: There appears to be another mDNS responder "
                    "running, or previously systemd-resolved crashed with some "
    ...
    r = sd_event_add_io(m->event, &m->mdns_ipv4_event_source, s, EPOLLIN,
                        on_mdns_packet, m);
    if (r < 0)
        return log_error_errno(r,
                               "mDNS-IPv4: Failed to create event source: %m");
...
int manager_mdns_ipv6_fd(Manager* m)
{
    ...
    if (r < 0)
        return log_error_errno(
            r, "mDNS-IPv6: Failed to set IPV6_UNICAST_HOPS: %m");
    ...
    if (r < 0)
        return log_error_errno(
            r, "mDNS-IPv6: Failed to set IPV6_MULTICAST_HOPS: %m");

    r = setsockopt_int(s, IPPROTO_IPV6, IPV6_MULTICAST_LOOP, true);
    if (r < 0)
        return log_error_errno(
            r, "mDNS-IPv6: Failed to set IPV6_MULTICAST_LOOP: %m");
    ...
    if (r < 0)
        return log_error_errno(
            r, "mDNS-IPv6: Failed to set IPV6_RECVHOPLIMIT: %m");
    ...
    if (r < 0)
    {
        ...
        log_warning("mDNS-IPv6: There appears to be another mDNS responder "
                    "running, or previously systemd-resolved crashed with some "
                    "outstanding transfers.");
    ...
    r = sd_event_add_io(m->event, &m->mdns_ipv6_event_source, s, EPOLLIN,
                        on_mdns_packet, m);
    if (r < 0)
        return log_error_errno(r,
                               "mDNS-IPv6: Failed to create event source: %m");
