...
DEFobjStaticHelpers DEFobjCurrIf(glbl)
    ...
    BEGINobjConstruct(
        nsdsel_ptcp) /* be sure to specify the object type also in END macro! */
    pThis->currfds = 0;
...
finalize_it
    : ENDobjConstruct(nsdsel_ptcp)
      ...
      BEGINobjDestruct(nsdsel_ptcp) /* be sure to specify the object type also
                                       in END and CODESTART macros! */
      CODESTARTobjDestruct(nsdsel_ptcp) free(pThis->fds);
ENDobjDestruct(nsdsel_ptcp)

    /* Add a socket to the select set */
    static rsRetVal ATTR_NONNULL()
        Add(nsdsel_t* const pNsdsel, nsd_t* const pNsd,
            const nsdsel_waitOp_t waitOp)
{
    DEFiRet;
...
/* perform the select()  piNumReady returns how many descriptors are ready for
 * IO
 * TODO: add timeout!
 */
static rsRetVal ATTR_NONNULL()
    Select(nsdsel_t* const pNsdsel, int* const piNumReady)
{
    ...
    if (Debug)
    {
        dbgprintf("--------<NSDSEL_PTCP> calling poll, active fds (%d): ",
                  pThis->currfds);
    ...
    if (*piNumReady < 0)
    {
        ...
        else
        {
            LogMsg(errno, RS_RET_POLL_ERR, LOG_WARNING,
                   "ndssel_ptcp: poll system call failed, may cause further "
                   "troubles");
        }
        *piNumReady = 0;
    }
...
static rsRetVal ATTR_NONNULL()
    IsReady(nsdsel_t* const pNsdsel, nsd_t* const pNsd,
            const nsdsel_waitOp_t waitOp, int* const pbIsReady)
{
    DEFiRet;
    ...
    if (idx >= pThis->currfds)
    {
        LogMsg(0, RS_RET_INTERNAL_ERROR, LOG_ERR,
               "ndssel_ptcp: could not find socket %d which should be present",
               sock);
...
BEGINobjQueryInterface(nsdsel_ptcp)
    CODESTARTobjQueryInterface(nsdsel_ptcp) if (pIf->ifVersion !=
                                                nsdCURR_IF_VERSION)
...
finalize_it
    : ENDobjQueryInterface(nsdsel_ptcp)
       ...
       */
      BEGINObjClassExit(
          nsdsel_ptcp, OBJ_IS_CORE_MODULE) /* CHANGE class also in END MACRO! */
