...
/*
   ...
   malloc_consolidate, it does not affect correctness.  As a result we can
   safely use relaxed atomic accesses.
 */
...
static void* realloc_hook_ini(void* ptr, size_t sz, const void* caller) __THROW;
static void* memalign_hook_ini(size_t alignment, size_t sz,
                               const void* caller) __THROW;

#if HAVE_MALLOC_INIT_HOOK
void weak_variable (*__malloc_initialize_hook)(void) = NULL;
compat_symbol(libc, __malloc_initialize_hook, __malloc_initialize_hook,
              GLIBC_2_0);
#endif

void weak_variable (*__free_hook)(void* __ptr, const void*) = NULL;
void* weak_variable (*__malloc_hook)(size_t __size,
                                     const void*) = malloc_hook_ini;
void* weak_variable (*__realloc_hook)(void* __ptr, size_t __size,
                                      const void*) = realloc_hook_ini;
void* weak_variable (*__memalign_hook)(size_t __alignment, size_t __size,
                                       const void*) = memalign_hook_ini;
    ...
    for (i = 1; i < NBINS; ++i)
    {
        ...
        for (p = last(b); p != b; p = p->bk)
        {
            ...
            if (i >= 2)
            {
                ...
                assert(idx == i);
                /* lists are sorted */
                assert(p->bk == b || (unsigned long)chunksize(p->bk) >=
            ...
            for (q = next_chunk(p); (q != av->top && inuse(q) &&
    ...
    try_mmap:
        ...
        if ((unsigned long)(size) > (unsigned long)(nb))
        {
            ...
            if (mm != MAP_FAILED)
            {
                ...
                if (MALLOC_ALIGNMENT == 2 * SIZE_SZ)
                {
                    /* For glibc, chunk2mem increases the address by 2*SIZE_SZ
                       and MALLOC_ALIGN_MASK is 2*SIZE_SZ-1.  Each mmap'ed area
                       is page aligned and therefore definitely
                       MALLOC_ALIGN_MASK-aligned.  */
                    assert(((INTERNAL_SIZE_T)chunk2mem(mm) &
                            MALLOC_ALIGN_MASK) == 0);
                    front_misalign = 0;
                }
                else
    ...
    { /* Request enough space for nb + pad + overhead */
        ...
        else
        {
            /*
               ...
               cannot be used. This is worth doing on systems that have "holes"
               in address space, so sbrk cannot extend to give contiguous space,
               but space is available elsewhere.  Note that we ignore mmap max
               count and threshold limits, since the space will not be used as a
            ...
            if ((unsigned long)(size) > (unsigned long)(nb))
            {
                ...
                if (mbrk != MAP_FAILED)
                {
                    ...
                    /*
                       ...
                       ever rely on contiguous space since this could
                       incorrectly bridge regions.
                     */
                    set_noncontiguous(av);
                }
            }
            ...
            else
            {
                ...
                if (contiguous(av))
                {
                    ...
                    else
                    {
                        /* Call the `morecore' hook if necessary.  */
                        void (*hook)(void) =
                            atomic_forced_read(__after_morecore_hook);
                        if (__builtin_expect(hook != NULL, 0))
                            (*hook)();
                    }
                }
                ...
                if (snd_brk != (char*)(MORECORE_FAILURE))
                {
                    ...
                    /*
                       ...
                       are artificial chunks that are marked as inuse and are in
                       any case too small to use.  We need two to make sizes and
                       alignments work out.
                     */
                    ...
                    if (old_size != 0)
                    {
                         ...
                         */
                        old_size =
                            (old_size - 4 * SIZE_SZ) & ~MALLOC_ALIGN_MASK;
                        set_head(old_top, old_size | PREV_INUSE);

                        /*
                           Note that the following assignments completely
                           overwrite old_top when old_size was previously
                           MINSIZE.  This is intentional. We need the fencepost,
                           even if old_top otherwise gets lost.
                         */
                        set_head(chunk_at_offset(old_top, old_size),
                                 (2 * SIZE_SZ) | PREV_INUSE);
                        set_head(
                            chunk_at_offset(old_top, old_size + 2 * SIZE_SZ),
                            (2 * SIZE_SZ) | PREV_INUSE);
...
static mchunkptr mremap_chunk(mchunkptr p, size_t new_size)
{
    ...
    cp = (char*)__mremap((char*)block, total_size, new_size, MREMAP_MAYMOVE);
...
static void* _mid_memalign(size_t alignment, size_t bytes, void* address)
{
    mstate ar_ptr;
    void* p;
...
/* For ISO C11.  */
weak_alias(__libc_memalign, aligned_alloc) libc_hidden_def(__libc_memalign)
            ...
            *fb = p;
            ...
            do
            {
                ...
                if (__builtin_expect(old == p, 0))
                    malloc_printerr("double free or corruption (fasttop)");
                p->fd = old2 = old;
            } while ((old = catomic_compare_and_exchange_val_rel(
                          fb, p, old2)) != old2);
                   ...
                   INTERNAL_SIZE_T nb)
{
    ...
    else
    {
        ...
        if (next == av->top && (unsigned long)(newsize = oldsize + nextsize) >=
        ...
        if (i == 1 || i >= psindex)
        {
            ...
            for (mchunkptr p = last(bin); p != bin; p = p->bk)
            {
                ...
                if (size > psm1 + sizeof(struct malloc_chunk))
                {
                     ...
                     */
                    char* paligned_mem =
                        (char*)(((uintptr_t)p + sizeof(struct malloc_chunk) +
                                 psm1) &
                                ~psm1);
...
static void int_mallinfo(mstate av, struct mallinfo* m)
{
    size_t i;
    mbinptr b;
    mchunkptr p;
...
/*
   ------------------------------ mallopt ------------------------------
 */
static inline int __always_inline do_set_trim_threshold(size_t value)
{
...
static inline int __always_inline do_set_top_pad(size_t value)
{
...
static inline int __always_inline do_set_mmap_threshold(size_t value)
{
...
static inline int __always_inline do_set_mmaps_max(int32_t value)
{
...
static inline int __always_inline do_set_mallopt_check(int32_t value)
{
    return 1;
}

static inline int __always_inline do_set_perturb_byte(int32_t value)
{
...
static inline int __always_inline do_set_arena_test(size_t value)
{
...
static inline int __always_inline do_set_arena_max(size_t value)
{
...
#if USE_TCACHE
static inline int __always_inline do_set_tcache_max(size_t value)
{
...
static inline int __always_inline do_set_tcache_unsorted_limit(size_t value)
{
    ...
    strong_alias(__libc_calloc, __calloc) weak_alias(__libc_calloc, calloc)
        strong_alias(__libc_free, __free) strong_alias(__libc_free, free)
            strong_alias(__libc_malloc, __malloc) strong_alias(
                __libc_malloc, malloc) strong_alias(__libc_memalign, __memalign)
                weak_alias(__libc_memalign,
                           memalign) strong_alias(__libc_realloc, __realloc)
                    strong_alias(__libc_realloc,
                                 realloc) strong_alias(__libc_valloc, __valloc)
                        weak_alias(__libc_valloc, valloc) strong_alias(
                            __libc_pvalloc,
                            __pvalloc) weak_alias(__libc_pvalloc, pvalloc)
                            strong_alias(__libc_mallinfo,
                                         __mallinfo) weak_alias(__libc_mallinfo,
                                                                mallinfo)
