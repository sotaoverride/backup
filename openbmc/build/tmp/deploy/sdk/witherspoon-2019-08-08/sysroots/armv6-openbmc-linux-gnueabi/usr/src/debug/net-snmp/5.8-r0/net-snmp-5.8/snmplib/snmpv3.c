...
void snmpv3_secLevel_conf(const char* word, char* cptr)
{
    ...
    {
        netsnmp_ds_set_int(NETSNMP_DS_LIBRARY_ID, NETSNMP_DS_LIB_SECLEVEL,
                           secLevel);
    }
    else
    {
...
int snmpv3_parse_arg(int arg, char* optarg, netsnmp_session* session,
                     ...
                     int flags)
{
    ...
    switch (arg)
    {
        ...
        case 'l':
                ...
                !strcasecmp(optarg, "noauth") || !strcasecmp(optarg, "nanp"))
            {
                session->securityLevel = SNMP_SEC_LEVEL_NOAUTH;
                     ...
                     !strcasecmp(optarg, "auth") || !strcmp(optarg, "2") ||
                     !strcasecmp(optarg, "anp"))
            {
                session->securityLevel = SNMP_SEC_LEVEL_AUTHNOPRIV;
            }
            else if (!strcasecmp(optarg, "authPriv") || !strcmp(optarg, "3") ||
                     !strcasecmp(optarg, "priv") || !strcasecmp(optarg, "ap"))
            {
        ...
        case 'a':
        {
            ...
            {
                fprintf(stderr,
                        "Invalid authentication protocol specified after -3a "
                        "flag: %s\n",
                        optarg);
                return (-1);
            }
        ...
        case 'k':
        {
            ...
            if (!snmp_hex_to_binary(&kbuf, &kbuf_len, &kout_len, 1, optarg))
            {
...
int snmpv3_parse_args(char* optarg, netsnmp_session* session, char** Apsz,
...
int setup_engineID(u_char** eidp, const char* text)
{
    int enterpriseid = htonl(NETSNMP_ENTERPRISE_OID),
        netsnmpoid = htonl(NETSNMP_OID), localsetup = (eidp) ? 0 : 1;
    ...
    switch (localEngineIDType)
    {
        case ENGINEID_TYPE_TEXT:
            if (NULL == text)
            {
                snmp_log(LOG_ERR, "Can't set up engineID of type text from an "
                                  "empty string.\n");
                return -1;
            }
        ...
        default:
            snmp_log(LOG_ERR,
                     "Unknown EngineID type requested for setup (%d).  Using "
                     "IPv4.\n",
                     localEngineIDType);
    ...
    if (localEngineIDType == ENGINEID_TYPE_NETSNMP_RND)
         ...
         */
        memcpy(bufp, &netsnmpoid,
               sizeof(netsnmpoid)); /* XXX Must be 4 bytes! */
    else
        memcpy(bufp, &enterpriseid,
               sizeof(enterpriseid)); /* XXX Must be 4 bytes! */
    ...
    switch (localEngineIDType)
    {
        case ENGINEID_TYPE_NETSNMP_RND:
            ...
            else
            {
                 ...
                 * suffer from.
                 *
                 * Ours is built from 2 fairly random elements: a random number
                 * and the current time in seconds.  This method suffers from
                 * boxes that may not have a correct clock setting and random
                 * number seed at startup, but few OSes should have that
                 * problem.
                 */
...
int free_engineID(int majorid, int minorid, void* serverarg, void* clientarg)
{
    SNMP_FREE(engineID);
 ...
 */
void engineBoots_conf(const char* word, char* cptr)
{
 ...
 */
void engineIDType_conf(const char* word, char* cptr)
{
 ...
 *		eth0 is default
 */
void engineIDNic_conf(const char* word, char* cptr)
{
    /*
     ...
     */
    {
        ...
        if (NULL != engineIDNic)
        {
            ...
            DEBUGMSGTL(
                ("snmpv3", "Initializing engineIDNic: %s\n", engineIDNic));
        }
        else
        {
            DEBUGMSGTL(
                ("snmpv3", "Error allocating memory for engineIDNic!\n"));
        }
    }
    else
    {
        DEBUGMSGTL(
            ("snmpv3", "NOT setting engineIDNic, engineID already set\n"));
    }
}
...
void version_conf(const char* word, char* cptr)
{
    ...
    if ((strcmp(cptr, "1") == 0) || (strcmp(cptr, "v1") == 0))
    {
    ...
    if ((strcasecmp(cptr, "2c") == 0) || (strcasecmp(cptr, "v2c") == 0))
    {
        netsnmp_ds_set_int(NETSNMP_DS_LIBRARY_ID, NETSNMP_DS_LIB_SNMPVERSION,
    ...
    DEBUGMSGTL((
        "snmpv3", "set default version to %d\n",
        netsnmp_ds_get_int(NETSNMP_DS_LIBRARY_ID, NETSNMP_DS_LIB_SNMPVERSION)));
}
...
void exactEngineID_conf(const char* word, char* cptr)
{
    ...
    if (new_engineIDLength > MAX_ENGINEID_LENGTH)
    {
        ...
        netsnmp_config_error(
            "exactEngineID '%s' too long; truncating to %d bytes", cptr,
            MAX_ENGINEID_LENGTH);
    }
 ...
 */
void init_snmpv3(const char* type)
{
    ...
    register_prenetsnmp_mib_handler(type, "engineID", engineID_conf, NULL,
                                    "string");
    register_prenetsnmp_mib_handler(type, "oldEngineID", oldengineID_conf, NULL,
                                    NULL);
    register_prenetsnmp_mib_handler(type, "exactEngineID", exactEngineID_conf,
                                    NULL, NULL);
    register_prenetsnmp_mib_handler(type, "engineIDType", engineIDType_conf,
                                    NULL, "num");
    register_prenetsnmp_mib_handler(type, "engineIDNic", engineIDNic_conf, NULL,
                                    "string");
                            ...
                            void* clientarg)
{
     ...
     */
    if (oldEngineIDLength != (size_t)0 &&
        (engineIDLen != oldEngineIDLength || oldEngineID == NULL ||
         c_engineID == NULL ||
     ...
     */
    set_enginetime(c_engineID, engineIDLen, snmpv3_local_snmpEngineBoots(),
...
int init_snmpv3_post_premib_config(int majorid, int minorid, void* serverarg,
                                   void* clientarg)
{
    if (!engineIDIsSet)
 ...
 *	*type
 */
int snmpv3_store(int majorID, int minorID, void* serverarg, void* clientarg)
{
 ...
 *
 */
int snmpv3_clone_engineID(u_char** dest, size_t* destlen, u_char* src,
                          size_t srclen)
{
    if (!dest || !destlen)
        return 0;
 ...
 */
u_char* snmpv3_generate_engineID(size_t* length)
{
    u_char* newID;
 ...
 */
#if defined(IFHWADDRLEN) && defined(SIOCGIFHWADDR)
static int getHwAddress(const char* networkDevice, /* e.g. "eth0", "eth1" */
                        char* addressOut)
{ /* return address. Len=IFHWADDRLEN */
     ...
     * *
     * *  Input Parameter(s):
     * *      networkDevice - a null terminated string with the name of a
     * network
     * *                      device.  Examples: eth0, eth1, etc...
     * *
     * *  Output Parameter(s):
     * *      addressOut -    This is the binary value of the hardware address.
     * *                      This value is NOT converted into a hexadecimal
     * string.
