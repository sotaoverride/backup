...
static int probe_vmfs_fs(blkid_probe pr, const struct blkid_idmag* mag)
{
    ...
    blkid_probe_sprintf_uuid(
        pr, (unsigned char*)header->uuid, 16,
        "%02x%02x%02x%02x-%02x%02x%02x%02x-"
        "%02x%02x-%02x%02x%02x%02x%02x%02x",
        header->uuid[3], header->uuid[2], header->uuid[1], header->uuid[0],
        header->uuid[7], header->uuid[6], header->uuid[5], header->uuid[4],
        header->uuid[9], header->uuid[8], header->uuid[10], header->uuid[11],
...
const struct blkid_idinfo vmfs_fs_idinfo = {
    ...
    .magics = {{.magic = "\x5e\xf1\xab\x2f", .len = 4, .kboff = 2048}, {NULL}}};

const struct blkid_idinfo vmfs_volume_idinfo = {
    .name = "VMFS_volume_member",
    .usage = BLKID_USAGE_RAID,
    .probefunc = probe_vmfs_volume,
    .magics = {{.magic = "\x0d\xd0\x01\xc0", .len = 4, .kboff = 1024}, {NULL}}};
