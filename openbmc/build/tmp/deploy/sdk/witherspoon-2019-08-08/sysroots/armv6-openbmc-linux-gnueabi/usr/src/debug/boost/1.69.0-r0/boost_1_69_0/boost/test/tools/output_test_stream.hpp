...
class BOOST_TEST_DECL output_test_stream
    : public wrap_stringstream::wrapped_stream
{
  ...
  public:
    //! Constructor
    //!
    //!@param[in] pattern_file_name indicates the name of the file for matching.
    //!If the
    //!           string is empty, the standard input or output streams are used
    //!           instead (depending on match_or_save)
    //!@param[in] match_or_save if true, the pattern file will be read,
    //!otherwise it will be
    //!           written
    //!@param[in] text_or_binary if false, opens the stream in binary mode.
    //!Otherwise the stream
    //!           is opened with default flags and the carriage returns are
    //!           ignored.
    ...
    //!@param[in] flush_stream if true, flushes the stream after the call. Set
    //!to false to call
    //!           additional checks on the same content.
    virtual assertion_result check_length(std::size_t length,
    ...
    //!@param[in] flush_stream if true, flushes the stream after the call.
    virtual assertion_result is_equal(const_string arg_,
                                      bool flush_stream = true);

    //! Checks the content of the stream against a pattern file
    //!
    //!@param[in] flush_stream if true, flushes/resets the stream after the
    //!call.
  ...
  protected:
    ...
    //!
    //! May be overriden in order to mutate the string before the matching
    //! operations.
