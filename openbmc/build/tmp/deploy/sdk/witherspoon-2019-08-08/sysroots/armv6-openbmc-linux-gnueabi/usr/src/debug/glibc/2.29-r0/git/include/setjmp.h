...
libc_hidden_proto(_setjmp) libc_hidden_proto(__sigsetjmp)
...
#define TEST_SIZE(type, size)                                                  \
    _Static_assert(sizeof(type) == size, "size of " #type " != " STR(size))
#define TEST_ALIGN(type, align)                                                \
    _Static_assert(__alignof__(type) == align,                                 \
                   "align of " #type " != " STR(align))
#define TEST_OFFSET(type, member, offset)                                      \
    _Static_assert(offsetof(type, member) == offset,                           \
                   "offset of " #member " field of " #type " != " STR(offset))
...
TEST_OFFSET(struct __jmp_buf_tag, __mask_was_saved, MASK_WAS_SAVED_OFFSET);
TEST_OFFSET(struct __jmp_buf_tag, __saved_mask, SAVED_MASK_OFFSET);
#endif
