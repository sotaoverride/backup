...
static int probe_bsd_pt(blkid_probe pr, const struct blkid_idmag* mag)
{
    ...
    if (parent)
    {
        switch (blkid_partition_get_type(parent))
        {
            ...
            default:
                DBG(LOWPROBE,
                    ul_debug("WARNING: BSD label detected on unknown (0x%x) "
                             "primary partition",
    ...
    if (le16_to_cpu(l->d_npartitions) < BSD_MAXPARTITIONS)
        ...
        DBG(LOWPROBE,
            ul_debug("WARNING: ignore %d more BSD partitions",
    ...
    for (i = 0, p = l->d_partitions; i < nparts; i++, p++)
    {
        ...
        {
            DBG(LOWPROBE,
                ul_debug("WARNING: BSD partition (%d) same like parent, "
                         "ignore",
                         i));
            continue;
        }
        ...
        {
            DBG(LOWPROBE, ul_debug("WARNING: BSD partition (%d) overflow "
                                   "detected, ignore",
                                   i));
            continue;
        }
...
const struct blkid_idinfo bsd_pt_idinfo = {
    ...
    .magics = {{.magic = "\x57\x45\x56\x82", .len = 4, .sboff = 512},
