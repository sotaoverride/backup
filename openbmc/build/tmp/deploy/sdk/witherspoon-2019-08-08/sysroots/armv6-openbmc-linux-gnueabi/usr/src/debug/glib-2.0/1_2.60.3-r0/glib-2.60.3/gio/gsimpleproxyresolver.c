...
G_DEFINE_TYPE_WITH_CODE(
    GSimpleProxyResolver, g_simple_proxy_resolver, G_TYPE_OBJECT,
...
static void g_simple_proxy_resolver_finalize(GObject* object)
{
...
static void g_simple_proxy_resolver_init(GSimpleProxyResolver* resolver)
{
    resolver->priv = g_simple_proxy_resolver_get_instance_private(resolver);
    resolver->priv->uri_proxies =
        g_hash_table_new_full(g_str_hash, g_str_equal, g_free, g_free);
}

static void g_simple_proxy_resolver_set_property(GObject* object, guint prop_id,
                                                 const GValue* value,
                                                 GParamSpec* pspec)
{
    GSimpleProxyResolver* resolver = G_SIMPLE_PROXY_RESOLVER(object);
...
static void g_simple_proxy_resolver_get_property(GObject* object, guint prop_id,
                                                 GValue* value,
                                                 GParamSpec* pspec)
{
    GSimpleProxyResolver* resolver = G_SIMPLE_PROXY_RESOLVER(object);
...
static void reparse_ignore_hosts(GSimpleProxyResolver* resolver)
{
    ...
    ignore_domains =
        g_array_new(TRUE, FALSE, sizeof(GSimpleProxyResolverDomain));
    ...
    if (ignore_domains->len)
        priv->ignore_domains =
            (GSimpleProxyResolverDomain*)ignore_domains->data;
    g_array_free(ignore_domains, ignore_domains->len == 0);
}

static gboolean ignore_host(GSimpleProxyResolver* resolver, const gchar* host,
                            gushort port)
{
    GSimpleProxyResolverPrivate* priv = resolver->priv;
...
static gchar** g_simple_proxy_resolver_lookup(GProxyResolver* proxy_resolver,
                                              const gchar* uri,
...
static void g_simple_proxy_resolver_lookup_async(GProxyResolver* proxy_resolver,
                                                 const gchar* uri,
                                                 GCancellable* cancellable,
                                                 ...
                                                 gpointer user_data)
{
    ...
    proxies = g_simple_proxy_resolver_lookup(proxy_resolver, uri, cancellable,
                                             &error);
    if (proxies)
...
static gchar** g_simple_proxy_resolver_lookup_finish(GProxyResolver* resolver,
                                                     GAsyncResult* result,
                                                     GError** error)
{
...
static void g_simple_proxy_resolver_class_init(
    GSimpleProxyResolverClass* resolver_class)
{
    ...
    g_object_class_install_property(
        object_class, PROP_DEFAULT_PROXY,
        g_param_spec_string("default-proxy", P_("Default proxy"),
                            P_("The default proxy URI"), NULL,
     ...
     */
    g_object_class_install_property(
        object_class, PROP_IGNORE_HOSTS,
        g_param_spec_boxed("ignore-hosts", P_("Ignore hosts"),
                           P_("Hosts that will not use the proxy"), G_TYPE_STRV,
                           G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS));
}
...
GProxyResolver* g_simple_proxy_resolver_new(const gchar* default_proxy,
                                            gchar** ignore_hosts)
{
    return g_object_new(G_TYPE_SIMPLE_PROXY_RESOLVER, "default-proxy",
                        default_proxy, "ignore-hosts", ignore_hosts, NULL);
}
                                           ...
                                           const gchar* proxy)
{
    ...
    g_hash_table_replace(resolver->priv->uri_proxies,
                         g_ascii_strdown(uri_scheme, -1), g_strdup(proxy));
}
