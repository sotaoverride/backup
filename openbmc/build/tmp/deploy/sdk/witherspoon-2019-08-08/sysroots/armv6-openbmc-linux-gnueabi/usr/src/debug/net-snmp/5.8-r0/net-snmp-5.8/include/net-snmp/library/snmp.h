...
#define SNMP_PORT                                                              \
    161 /* standard UDP port for SNMP agents                                   \
         * to receive requests messages */
#define SNMP_TRAP_PORT                                                         \
    162 /* standard UDP port for SNMP                                          \
...
#define SNMP_CMD_CONFIRMED(c)                                                  \
    (c == SNMP_MSG_INFORM || c == SNMP_MSG_GETBULK || c == SNMP_MSG_GETNEXT || \
     c == SNMP_MSG_GET)
#else /* !NETSNMP_NO_WRITE_SUPPORT */
#define SNMP_CMD_CONFIRMED(c)                                                  \
    (c == SNMP_MSG_INFORM || c == SNMP_MSG_GETBULK || c == SNMP_MSG_GETNEXT || \
     c == SNMP_MSG_GET || c == SNMP_MSG_SET)
#endif /* !NETSNMP_NO_WRITE_SUPPORT */
...
#define SNMP_VALIDATE_ERR(x)                                                   \
    ((x > MAX_SNMP_ERR) ? SNMP_ERR_GENERR                                      \
                        : (x < SNMP_ERR_NOERROR) ? SNMP_ERR_GENERR : x)
...
NETSNMP_IMPORT
u_char* snmp_parse_var_op(u_char*, oid*, size_t*, u_char*, size_t*, u_char**,
                          size_t*);
NETSNMP_IMPORT
...
int snmp_realloc_rbuild_var_op(u_char** pkt, size_t* pkt_len, size_t* offset,
                               int allow_realloc, const oid* name,
                               size_t* name_len, u_char value_type,
