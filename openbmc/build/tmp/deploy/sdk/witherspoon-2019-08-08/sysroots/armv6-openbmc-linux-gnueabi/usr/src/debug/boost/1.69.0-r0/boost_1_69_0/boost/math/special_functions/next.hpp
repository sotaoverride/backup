...
#endif
template <>
struct has_hidden_guard_digits<boost::math::concepts::real_concept>
    : public mpl::false_
{
};
template <>
...
template <class T>
struct has_hidden_guard_digits_10<T, true>
    : public mpl::bool_<(std::numeric_limits<T>::digits10 !=
...
template <class T>
struct has_hidden_guard_digits
    : public has_hidden_guard_digits_10<
...
template <class T>
inline const T& normalize_value(const T& val, const mpl::false_&)
{
    return val;
}
template <class T>
inline T normalize_value(const T& val, const mpl::true_&)
{
    ...
    boost::intmax_t shift = (boost::intmax_t)std::numeric_limits<T>::digits -
                            (boost::intmax_t)ilogb(val) - 1;
...
inline T get_smallest_value(mpl::true_ const&)
{
    ...
    return (_mm_getcsr() & (_MM_FLUSH_ZERO_ON | 0x40)) ? tools::min_value<T>()
                                                       : m;
    ;
#else
...
inline T get_smallest_value()
{
    ...
    return get_smallest_value<T>(
        ...
        (std::numeric_limits<T>::has_denorm == 1) > ());
#else
    return get_smallest_value<T>(
        mpl::bool_ < std::numeric_limits<T>::is_specialized &&
...
template <class T>
const typename min_shift_initializer<T>::init
    min_shift_initializer<T>::initializer;
...
{
    static const T val = calc_min_shifted<T>(
        mpl::bool_ < !std::numeric_limits<T>::is_specialized ||
...
T float_next_imp(const T& val, const mpl::true_&, const Policy& pol)
{
    ...
    if ((fpclass == (int)FP_NAN) || (fpclass == (int)FP_INFINITE))
    {
        ...
        return policies::raise_domain_error<T>(
            function, "Argument must be finite, but got %1%", val, pol);
    }
    ...
    if ((fpclass != (int)FP_SUBNORMAL) && (fpclass != (int)FP_ZERO) &&
        (fabs(val) < detail::get_min_shift_value<T>()) &&
        (val != -tools::min_value<T>()))
    {
        //
        // Special case: if the value of the least significant bit is a denorm,
        // and the result would not be a denorm, then shift the input,
        // increment, and shift back. This avoids issues with the Intel SSE2
        // registers when the FTZ or DAZ flags are set.
        //
        return ldexp(float_next(T(ldexp(val, 2 * tools::digits<T>())), pol),
                     -2 * tools::digits<T>());
    }
...
T float_next_imp(const T& val, const mpl::false_&, const Policy& pol)
{
        ...
        (val != -tools::min_value<T>()))
    {
        ...
        //
        return scalbn(
            float_next(T(scalbn(val, 2 * std::numeric_limits<T>::digits)), pol),
                                                        ...
                                                        const Policy& pol)
{
    ...
    return detail::float_next_imp(
        detail::normalize_value(
            static_cast<result_type>(val),
            typename detail::has_hidden_guard_digits<result_type>::type()),
        mpl::bool_ < !std::numeric_limits<result_type>::is_specialized ||
            (std::numeric_limits<result_type>::radix == 2) > (),
        pol);
}
...
T float_prior_imp(const T& val, const mpl::true_&, const Policy& pol)
{
        ...
        (val != tools::min_value<T>()))
    {
        ...
        // registers when the FTZ or DAZ flags are set.
        //
        return ldexp(float_prior(T(ldexp(val, 2 * tools::digits<T>())), pol),
                     -2 * tools::digits<T>());
    }
...
T float_prior_imp(const T& val, const mpl::false_&, const Policy& pol)
{
        ...
        (val != tools::min_value<T>()))
    {
        ...
        //
        return scalbn(
            float_prior(T(scalbn(val, 2 * std::numeric_limits<T>::digits)),
                        pol),
            -2 * std::numeric_limits<T>::digits);
    }
...
inline typename tools::promote_args<T, U>::type
    nextafter(const T& val, const U& direction, const Policy& pol)
{
    typedef typename tools::promote_args<T, U>::type result_type;
    return val < direction
               ? boost::math::float_next<result_type>(val, pol)
               : val == direction
                     ? val
                     : boost::math::float_prior<result_type>(val, pol);
}

template <class T, class U>
inline typename tools::promote_args<T, U>::type nextafter(const T& val,
                                                          const U& direction)
{
...
template <class T, class Policy>
T float_distance_imp(const T& a, const T& b, const mpl::true_&,
                     const Policy& pol)
{
    BOOST_MATH_STD_USING
    //
    ...
    if (!(boost::math::isfinite)(a))
        return policies::raise_domain_error<T>(
            function, "Argument a must be finite, but got %1%", a, pol);
    if (!(boost::math::isfinite)(b))
        return policies::raise_domain_error<T>(
            function, "Argument b must be finite, but got %1%", b, pol);
    //
    // Special cases:
    //
    if (a > b)
    ...
    if (a == 0)
        return 1 +
               fabs(float_distance(
                   static_cast<T>((b < 0) ? T(-detail::get_smallest_value<T>())
                                          : detail::get_smallest_value<T>()),
                   b, pol));
    if (b == 0)
        return 1 +
               fabs(float_distance(
                   static_cast<T>((a < 0) ? T(-detail::get_smallest_value<T>())
                                          : detail::get_smallest_value<T>()),
                   a, pol));
    ...
    //
    frexp(((boost::math::fpclassify)(a) == (int)FP_SUBNORMAL)
              ? tools::min_value<T>()
              : a,
          &expon);
    ...
    T mb, x, y, z;
    if (((boost::math::fpclassify)(a) == (int)FP_SUBNORMAL) ||
        (b - a < tools::min_value<T>()))
    {
        //
...
//
template <class T, class Policy>
T float_distance_imp(const T& a, const T& b, const mpl::false_&,
                     const Policy& pol)
{
    BOOST_STATIC_ASSERT(std::numeric_limits<T>::is_specialized);
    BOOST_STATIC_ASSERT(std::numeric_limits<T>::radix != 2);
    ...
    // significant bits in the representation:
    //
    expon = 1 + ilogb(((boost::math::fpclassify)(a) == (int)FP_SUBNORMAL)
                          ? tools::min_value<T>()
                          : a);
    ...
    if (b > upper)
    {
        ...
        result += (expon2 - expon - 1) *
                  scalbn(T(1), std::numeric_limits<T>::digits - 1);
    }
    //
    // Use compensated double-double addition to avoid rounding
    // errors in the subtraction:
...
{
    typedef typename tools::promote_args<T, U>::type result_type;
    return detail::float_distance_imp(
        detail::normalize_value(
...
T float_advance_imp(T val, int distance, const mpl::true_&, const Policy& pol)
{
    ...
    if ((fpclass == (int)FP_NAN) || (fpclass == (int)FP_INFINITE))
        return policies::raise_domain_error<T>(
            function, "Argument val must be finite, but got %1%", val, pol);
    ...
    if (fabs(val) < detail::get_min_shift_value<T>())
    {
        ...
        // This avoids issues with the Intel SSE2 registers when the FTZ or DAZ
        // flags are set.
        //
        if (distance > 0)
        {
    ...
    while (fabs(limit_distance) < abs(distance))
    {
        ...
        if (distance && (limit_distance == 0))
        {
            return policies::raise_evaluation_error<T>(
                function,
                "Internal logic failed while trying to increment floating "
                "point value %1%: most likely your FPU is in non-IEEE "
                "conforming mode.",
                val, pol);
        }
    }
...
template <class T, class Policy>
inline typename tools::promote_args<T>::type float_advance(T val, int distance,
                                                           const Policy& pol)
{
    typedef typename tools::promote_args<T>::type result_type;
    return detail::float_advance_imp(
        detail::normalize_value(
            static_cast<result_type>(val),
            typename detail::has_hidden_guard_digits<result_type>::type()),
        distance,
...
template <class T>
inline typename tools::promote_args<T>::type float_advance(const T& val,
                                                           int distance)
{
