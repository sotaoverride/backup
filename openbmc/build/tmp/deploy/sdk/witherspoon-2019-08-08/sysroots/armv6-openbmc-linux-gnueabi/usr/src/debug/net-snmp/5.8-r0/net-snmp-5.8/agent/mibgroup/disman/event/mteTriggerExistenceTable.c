...
void init_mteTriggerExistenceTable(void)
{
    ...
    reg = netsnmp_create_handler_registration(
        "mteTriggerExistenceTable", mteTriggerExistenceTable_handler,
        mteTExistTable_oid, mteTExistTable_oid_len, HANDLER_CAN_RWRITE);
#else  /* !NETSNMP_NO_WRITE_SUPPORT */
    reg = netsnmp_create_handler_registration(
        "mteTriggerExistenceTable", mteTriggerExistenceTable_handler,
        mteTExistTable_oid, mteTExistTable_oid_len, HANDLER_CAN_RONLY);
#endif /* !NETSNMP_NO_WRITE_SUPPORT */
...
int mteTriggerExistenceTable_handler(netsnmp_mib_handler* handler,
                                     ...
                                     netsnmp_request_info* requests)
{
    ...
    DEBUGMSGTL(
        ("disman:event:mib", "Exist Table handler (%d)\n", reqinfo->mode));
            ...
            for (request = requests; request; request = request->next)
            {
                ...
                switch (tinfo->colnum)
                {
                    ...
                    case COLUMN_MTETRIGGEREXISTENCESTARTUP:
                        ...
                        break;
                    case COLUMN_MTETRIGGEREXISTENCEOBJECTSOWNER:
                        memset(entry->mteTExObjOwner, 0,
                               sizeof(entry->mteTExObjOwner));
                        memcpy(entry->mteTExObjOwner,
                               request->requestvb->val.string,
                               request->requestvb->val_len);
                        break;
                    case COLUMN_MTETRIGGEREXISTENCEOBJECTS:
                        memset(entry->mteTExObjects, 0,
                               sizeof(entry->mteTExObjects));
                        memcpy(entry->mteTExObjects,
                               request->requestvb->val.string,
                               request->requestvb->val_len);
                        break;
                    case COLUMN_MTETRIGGEREXISTENCEEVENTOWNER:
                        memset(entry->mteTExEvOwner, 0,
                               sizeof(entry->mteTExEvOwner));
                        memcpy(entry->mteTExEvOwner,
                               request->requestvb->val.string,
                               request->requestvb->val_len);
                        break;
                    case COLUMN_MTETRIGGEREXISTENCEEVENT:
                        memset(entry->mteTExEvent, 0,
                               sizeof(entry->mteTExEvent));
                        memcpy(entry->mteTExEvent,
                               request->requestvb->val.string,
                               request->requestvb->val_len);
                        break;
                }
