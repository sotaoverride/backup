...
#error                                                                         \
    "G_CREDENTIALS_UNIX_CREDENTIALS_MESSAGE_SUPPORTED is set but there is no msg_type defined for this platform"
#else
    return 0;
#endif
}
...
static GSocketControlMessage*
    g_unix_credentials_message_deserialize(gint level, gint type, gsize size,
                                           gpointer data)
{
#if G_CREDENTIALS_UNIX_CREDENTIALS_MESSAGE_SUPPORTED
    ...
    if (level != SOL_SOCKET ||
        type != g_unix_credentials_message_get_msg_type(NULL))
        return NULL;

    if (size != G_CREDENTIALS_NATIVE_SIZE)
    {
        g_warning("Expected a credentials struct of %" G_GSIZE_FORMAT
                  " bytes but "
...
static void g_unix_credentials_message_get_property(GObject* object,
                                                    guint prop_id,
                                                    GValue* value,
                                                    GParamSpec* pspec)
{
...
static void g_unix_credentials_message_set_property(GObject* object,
                                                    guint prop_id,
                                                    const GValue* value,
                                                    GParamSpec* pspec)
{
...
static void g_unix_credentials_message_constructed(GObject* object)
{
    ...
    if (G_OBJECT_CLASS(g_unix_credentials_message_parent_class)->constructed !=
        NULL)
        G_OBJECT_CLASS(g_unix_credentials_message_parent_class)
            ->constructed(object);
}
 ...
 *
 * Creates a new #GUnixCredentialsMessage with credentials matching the current
 * processes.
 *
    ...
    g_unix_credentials_message_new_with_credentials(GCredentials* credentials)
{
    ...
    g_return_val_if_fail(g_unix_credentials_message_is_supported(), NULL);
    return g_object_new(G_TYPE_UNIX_CREDENTIALS_MESSAGE, "credentials",
                        credentials, NULL);
}
 ...
 *
 * Returns: (transfer none): A #GCredentials instance. Do not free, it is owned
 * by @message.
 *
 * Since: 2.26
 */
