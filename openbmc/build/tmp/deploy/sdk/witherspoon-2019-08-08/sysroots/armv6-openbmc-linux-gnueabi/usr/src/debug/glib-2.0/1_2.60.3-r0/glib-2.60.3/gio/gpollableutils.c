...
static gboolean pollable_source_dispatch(GSource* source, GSourceFunc callback,
                                         gpointer user_data)
{
 ...
 * Since: 2.34
 */
gssize g_pollable_stream_read(GInputStream* stream, void* buffer, gsize count,
                              ...
                              GError** error)
{
    ...
    else
    {
        return g_pollable_input_stream_read_nonblocking(
            G_POLLABLE_INPUT_STREAM(stream), buffer, count, cancellable, error);
    }
}
 ...
 * Since: 2.34
 */
gssize g_pollable_stream_write(GOutputStream* stream, const void* buffer,
                               ...
                               GCancellable* cancellable, GError** error)
{
    ...
    else
    {
        return g_pollable_output_stream_write_nonblocking(
            G_POLLABLE_OUTPUT_STREAM(stream), buffer, count, cancellable,
            error);
    }
}
 ...
 * Since: 2.34
 */
gboolean g_pollable_stream_write_all(GOutputStream* stream, const void* buffer,
                                     gsize count, gboolean blocking,
                                     gsize* bytes_written,
                                     GCancellable* cancellable, GError** error)
{
    ...
    {
        res = g_pollable_stream_write(stream, (char*)buffer + _bytes_written,
                                      count - _bytes_written, blocking,
                                      cancellable, error);
        if (res == -1)
        {
            if (bytes_written)
