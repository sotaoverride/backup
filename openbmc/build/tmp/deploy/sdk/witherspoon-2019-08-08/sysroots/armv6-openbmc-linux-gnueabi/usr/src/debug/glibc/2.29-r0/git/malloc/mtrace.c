...
static void* (*tr_old_realloc_hook)(void* ptr, size_t size, const void*);
...
libc_hidden_proto(tr_break) void tr_break(void)
{
}
    ...
    if (caller != NULL)
    {
        if (info != NULL)
        {
            ...
            if (info->dli_sname != NULL)
            {
                ...
                buf[0] = '(';
                __stpcpy(
                    _fitoa(caller >= (const void*)info->dli_saddr
                               ? caller - (const void*)info->dli_saddr
                               : (const void*)info->dli_saddr - caller,
                           __stpcpy(__mempcpy(buf + 1, info->dli_sname, len),
                                    caller >= (void*)info->dli_saddr ? "+0x"
                                                                     : "-0x"),
                           16, 0),
                    ")");
            }
...
static void* tr_memalignhook(size_t alignment, size_t size, const void* caller)
{
    void* hdr;
...
/* This function gets called to make sure all memory the library
   ...
   the mtrace output.  */
static void __libc_freeres_fn_section release_libc_mem(void)
{
