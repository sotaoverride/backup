...
static int swap_dispatch_timer(sd_event_source* source, usec_t usec,
                               void* userdata);
static int swap_dispatch_io(sd_event_source* source, int fd, uint32_t revents,
                            void* userdata);

static bool SWAP_STATE_WITH_PROCESS(SwapState state)
{
    return IN_SET(state, SWAP_ACTIVATING, SWAP_ACTIVATING_DONE,
                  SWAP_DEACTIVATING, SWAP_DEACTIVATING_SIGTERM,
...
static int swap_set_devnode(Swap* s, const char* devnode)
{
    ...
    r = hashmap_ensure_allocated(&UNIT(s)->manager->swaps_by_devnode,
                                 &path_hash_ops);
    if (r < 0)
        return r;
...
static int swap_arm_timer(Swap* s, usec_t usec)
{
    ...
    r = sd_event_add_time(UNIT(s)->manager->event, &s->timer_event_source,
                          CLOCK_MONOTONIC, usec, 0, swap_dispatch_timer, s);
    if (r < 0)
        return r;
...
static int swap_add_device_dependencies(Swap* s)
{
    ...
    if (is_device_path(s->what))
        ...
        return unit_add_dependency_by_name(UNIT(s), UNIT_AFTER,
                                           SPECIAL_REMOUNT_FS_SERVICE, true,
...
static int swap_add_default_dependencies(Swap* s)
{
    ...
    return unit_add_two_dependencies_by_name(
        UNIT(s), UNIT_BEFORE, UNIT_CONFLICTS, SPECIAL_UMOUNT_TARGET, true,
...
static int swap_verify(Swap* s)
{
    ...
    if (r < 0)
        return log_unit_error_errno(
            UNIT(s), r, "Failed to generate unit name from path: %m");

    if (!unit_has_name(UNIT(s), e))
    {
        log_unit_error(
            UNIT(s), "Value of What= and unit name do not match, not loading.");
        return -ENOEXEC;
    }
        ...
        s->kill_context.kill_mode != KILL_CONTROL_GROUP)
    {
        log_unit_error(UNIT(s), "Unit has PAM enabled. Kill mode must be set "
                                "to 'control-group'. Refusing to load.");
        return -ENOEXEC;
    }
...
static int swap_load(Unit* u)
{
    ...
    /* Add in some extras, and do so either when we successfully loaded
     * something or when /proc/swaps is already active. */
...
static int swap_setup_unit(Manager* m, const char* what,
                           const char* what_proc_swaps, int priority,
                           bool set_flags)
{
    ...
    if (r < 0)
        return log_unit_error_errno(
            u, r, "Failed to generate unit name from path: %m");

    u = manager_get_unit(m, e);
    if (u && SWAP(u)->from_proc_swaps &&
        !path_equal(SWAP(u)->parameters_proc_swaps.what, what_proc_swaps))
        return log_error_errno(
            SYNTHETIC_ERRNO(EEXIST),
            "Swap %s appeared twice with different device paths %s and %s", e,
            SWAP(u)->parameters_proc_swaps.what, what_proc_swaps);
    ...
    /* The unit is definitely around now, mark it as loaded if it was previously
     * referenced but could not be loaded. After all we can load it now, from
     * the data in /proc/swaps. */
...
static int swap_process_new(Manager* m, const char* device, int prio,
                            bool set_flags)
{
    _cleanup_(sd_device_unrefp) sd_device* d = NULL;
    ...
    FOREACH_DEVICE_DEVLINK(d, devlink)
    {
        ...
        if (stat(devlink, &st_link) >= 0 &&
            (!S_ISBLK(st_link.st_mode) || st_link.st_rdev != st.st_rdev))
            continue;
...
static void swap_dump(Unit* u, FILE* f, const char* prefix)
{
    ...
    fprintf(f,
            ...
            "%sFrom /proc/swaps: %s\n"
            "%sFrom fragment: %s\n",
            prefix, swap_state_to_string(s->state), prefix,
            swap_result_to_string(s->result), prefix, s->what, prefix,
            yes_no(s->from_proc_swaps), prefix, yes_no(s->from_fragment));
    ...
    if (p)
        fprintf(f,
                "%sPriority: %i\n"
                "%sOptions: %s\n",
                prefix, p->priority, prefix, strempty(p->options));

    fprintf(f, "%sTimeoutSec: %s\n", prefix,
            format_timespan(buf, sizeof(buf), s->timeout_usec, USEC_PER_SEC));
...
static void swap_enter_dead(Swap* s, SwapResult f)
{
    ...
    unit_log_result(UNIT(s), s->result == SWAP_SUCCESS,
                    swap_result_to_string(s->result));
...
static void swap_enter_signal(Swap* s, SwapState state, SwapResult f)
{
    ...
    r = unit_kill_context(UNIT(s), &s->kill_context, kop, -1, s->control_pid,
                          false);
    if (r < 0)
        goto fail;
...
static void swap_enter_activating(Swap* s)
{
    ...
    if (s->from_fragment)
    {
        ...
        if (r < 0)
            log_warning_errno(
                r, "Failed to parse swap priority \"%s\", ignoring: %m",
                s->parameters_fragment.options);
        else if (r == 1 && s->parameters_fragment.priority >= 0)
            log_warning("Duplicate swap priority configuration by Priority and "
                        "Options fields.");

        if (r <= 0 && s->parameters_fragment.priority >= 0)
        {
            if (s->parameters_fragment.options)
                r = asprintf(&opts, "%s,pri=%i", s->parameters_fragment.options,
...
static void swap_enter_deactivating(Swap* s)
{
    ...
    r = exec_command_set(s->control_command, "/sbin/swapoff", s->what, NULL);
    if (r < 0)
        goto fail;
...
static int swap_start(Unit* u)
{
    ...
    if (IN_SET(s->state, SWAP_DEACTIVATING, SWAP_DEACTIVATING_SIGTERM,
               SWAP_DEACTIVATING_SIGKILL))
        return -EAGAIN;
...
static int swap_deserialize_item(Unit* u, const char* key, const char* value,
                                 FDSet* fds)
{
    Swap* s = SWAP(u);
...
static void swap_sigchld_event(Unit* u, pid_t pid, int code, int status)
{
    ...
    unit_log_process_exit(
        u, f == SWAP_SUCCESS ? LOG_DEBUG : LOG_NOTICE, "Swap process",
        swap_exec_command_to_string(s->control_command_id), code, status);
                               ...
                               void* userdata)
{
    ...
    switch (s->state)
    {
        ...
        case SWAP_ACTIVATING_DONE:
            ...
            swap_enter_signal(s, SWAP_DEACTIVATING_SIGTERM,
        ...
        case SWAP_DEACTIVATING_SIGKILL:
            log_unit_warning(
                UNIT(s), "Swap process still around after SIGKILL. Ignoring.");
            swap_enter_dead_or_active(s, SWAP_FAILURE_TIMEOUT);
            break;
...
static void swap_enumerate(Manager* m)
{
    ...
    if (!m->proc_swaps)
    {
        ...
        r = sd_event_add_io(m->event, &m->swap_event_source,
                            fileno(m->proc_swaps), EPOLLPRI, swap_dispatch_io,
                            m);
        if (r < 0)
        {
        ...
        r = sd_event_source_set_priority(m->swap_event_source,
        ...
        (void)sd_event_source_set_description(m->swap_event_source,
                                              "swap-proc");
    }
...
static bool swap_supported(void)
{
    ...
    if (supported < 0)
        supported = access("/proc/swaps", F_OK) >= 0 && detect_container() <= 0;
