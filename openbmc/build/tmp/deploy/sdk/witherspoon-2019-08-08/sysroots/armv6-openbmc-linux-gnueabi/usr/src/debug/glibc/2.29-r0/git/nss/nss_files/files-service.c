...
#define ISSLASH(c) ((c) == '/')
LINE_PARSER("#", STRING_FIELD(result->s_name, isspace, 1);
...
DB_LOOKUP(
    servbyname, ':', strlen(name) + 2 + (proto == NULL ? 0 : strlen(proto)),
    ("%s/%s", name, proto ?: ""),
    {
...
DB_LOOKUP(
    servbyport, '=', 21 + (proto ? strlen(proto) : 0),
    ("%zd/%s", (ssize_t)ntohs(port), proto ?: ""),
    {
        /* Must match both port and protocol.  */
        if (result->s_port == port &&
            (proto == NULL || strcmp(result->s_proto, proto) == 0))
            break;
    },
