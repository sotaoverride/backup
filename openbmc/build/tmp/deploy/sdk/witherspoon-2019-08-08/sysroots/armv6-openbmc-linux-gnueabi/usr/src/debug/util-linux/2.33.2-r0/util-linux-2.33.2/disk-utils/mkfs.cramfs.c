...
static char* do_mmap(char* path, unsigned int size, unsigned int mode)
{
    int fd;
    char* start = NULL;
...
static void do_munmap(char* start, unsigned int size, unsigned int mode)
{
    if (S_ISLNK(mode))
        free(start);
    else
...
static void mdfile(struct entry* e)
{
    char* start;
...
static int identical_file(struct entry* e1, struct entry* e2)
{
    char *start1, *start2;
    int equal;
...
static unsigned int parse_directory(struct entry* root_entry, const char* name,
    ...
    *endpath = '/';
    ...
    for (dirindex = 0; dirindex < dircount; dirindex++)
    {
        ...
        *fslen_ub += size;
        ...
        {
            entry->size =
                parse_directory(root_entry, path, &entry->child, fslen_ub);
        }
...
static void set_data_offset(struct entry* entry, char* base,
                            unsigned long offset)
{
    struct cramfs_inode* inode =
        (struct cramfs_inode*)(base + entry->dir_offset);
    ...
    for (;;)
    {
        ...
        while (entry)
        {
            ...
            if (entry->child)
            {
                ...
                {
                    stack_size *= 2;
                    entry_stack = xrealloc(entry_stack,
...
static unsigned int do_compress(char* base, unsigned int offset,
                                unsigned char const* name, char* path,
    ...
    for (e = entry; e; e = e->next)
    {
        if (e->path)
        {
            ...
            else if (e->size)
            {
                ...
                e->offset = offset;
                offset = do_compress(base, offset, e->name, e->path, e->size,
                                     e->mode);
            }
        }
        else if (e->child)
 ...
 */
static unsigned int maxfslen(void)
{
...
int main(int argc, char** argv)
{
    ...
    int c;
    cramfs_is_big_endian =
        HOST_IS_BIG_ENDIAN; /* default is to use host order */
    ...
    while ((c = getopt(argc, argv, "hb:Ee:i:n:N:psVvz")) != EOF)
    {
        switch (c)
        {
            ...
            case 'b':
                blksize =
                    strtou32_or_err(optarg, _("invalid blocksize argument"));
                break;
            case 'E':
                opt_errors = 1;
                break;
            case 'e':
                opt_edition = strtou32_or_err(
                    optarg, _("invalid edition number argument"));
                break;
            case 'N':
    ...
    root_entry->size =
        parse_directory(root_entry, dirname, &root_entry->child, &fslen_ub);
    ...
    if (fslen_ub > fslen_max)
    {
        warnx(_("warning: guestimate of required size (upper bound) "
              ...
              (long long)fslen_ub >> 20, fslen_max >> 20);
        fslen_ub = fslen_max;
    }
    ...
    rom_image = mmap(NULL, fslen_ub ? fslen_ub : 1, PROT_READ | PROT_WRITE,
    ...
    ((struct cramfs_super*)(rom_image + opt_pad))->fsid.crc =
        u32_toggle_endianness(cramfs_is_big_endian, crc);
    if (verbose)
        printf(_("CRC: %x\n"), crc);
    ...
    if (offset != written)
        errx(MKFS_EX_ERROR, _("ROM image write failed (%zd %zd)"), written,
             offset);
    if (close_fd(fd) != 0)
    ...
    if (warn_namelen)
        ...
        warnx(_("warning: filenames truncated to %u bytes."),
              MAX_INPUT_NAMELEN);
    if (warn_skip)
