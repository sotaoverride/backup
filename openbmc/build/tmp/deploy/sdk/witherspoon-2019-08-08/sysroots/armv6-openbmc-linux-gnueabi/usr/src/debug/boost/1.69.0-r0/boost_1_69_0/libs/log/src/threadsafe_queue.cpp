 ...
 *
 * The lock-free version of the mentioned algorithms contain a race condition
...
class threadsafe_queue_impl_generic : public threadsafe_queue_impl
{
  private:
     ...
     * The alignment below allows to eliminate false sharing, it should not be
     * less than CPU cache line size.
     */
    struct BOOST_ALIGNMENT(BOOST_LOG_CPU_CACHE_LINE_SIZE) pointer
    {
        ...
        mutex_type mutex;
        //  128 bytes padding is chosen to mitigate false sharing for NetBurst
        //  CPUs, which load two cache lines in one go.
        unsigned char
            padding[128U - (sizeof(node_base*) + sizeof(mutex_type)) % 128U];
    };
  ...
  private:
    ...
    BOOST_DELETED_FUNCTION(threadsafe_queue_impl_generic& operator=(
        threadsafe_queue_impl_generic const&))
};

BOOST_LOG_API threadsafe_queue_impl*
    threadsafe_queue_impl::create(node_base* first_node)
{
