 ...
 * \date   01.03.2008
 *
 * The header contains implementation of convenience tools to apply visitors to
 * an attribute value in the view.
 */
 ...
 *
 * The main purpose of this class is to provide a convenient interface for
 * checking whether the attribute value visitation succeeded or not. It also
 * allows to discover the actual cause of failure, should the operation fail.
 */
class visitation_result
{
  public:
    ...
    enum error_code
    {
        ...
        value_has_invalid_type //!< The attribute value is present in the view,
                               //!< but has an unexpected type
    };
  ...
  public:
     ...
     *
     * \return \c true if the value was visited successfully, \c false
     * otherwise.
     */
     ...
     *
     * \return \c false if the value was visited successfully, \c true
     * otherwise.
     */
 ...
 *
 * Attribute value invoker is a functional object that attempts to find and
 * extract the stored attribute value from the attribute value view or a log
 * record. The extracted value is passed to a unary function object (the
 * visitor) provided by user.
 *
 * The invoker can be specialized on one or several attribute value types that
 * should be specified in the second template argument.
 */
...
class value_visitor_invoker : private FallbackPolicyT
{
  ...
  public:
    ...
    /*!
     * Visitation operator. Attempts to acquire the stored value of one of the
     * supported types. If acquisition succeeds, the value is passed to \a
     * visitor.
     *
     * \param attr An attribute value to apply the visitor to.
     * \param visitor A receiving function object to pass the attribute value
     * to. \return The result of visitation.
     */
     ...
     * acquisition succeeds, the value is passed to \a visitor.
     *
     * \param name Attribute value name.
     * \param attrs A set of attribute values in which to look for the specified
     * attribute value. \param visitor A receiving function object to pass the
     * attribute value to. \return The result of visitation.
     */
    template <typename VisitorT>
     ...
     * associated with the record. \param visitor A receiving function object to
     * pass the attribute value to. \return The result of visitation.
     */
    template <typename VisitorT>
     ...
     * those associated with the record. \param visitor A receiving function
     * object to pass the attribute value to. \return The result of visitation.
     */
    template <typename VisitorT>
...
/*!
 * The function applies a visitor to an attribute value from the view. The user
 * has to explicitly specify the type or set of possible types of the attribute
 * value to be visited.
 *
...
inline visitation_result visit(attribute_name const& name,
                               attribute_value_set const& attrs,
                               VisitorT visitor)
{
 ...
 */
template <typename T, typename VisitorT>
inline visitation_result visit(attribute_name const& name, record const& rec,
                               VisitorT visitor)
{
    value_visitor_invoker<T> invoker;
...
/*!
 * The function applies a visitor to an attribute value. The user has to
 ...
 */
template <typename T, typename VisitorT>
inline visitation_result visit(attribute_value const& value, VisitorT visitor)
{
    value_visitor_invoker<T> invoker;
