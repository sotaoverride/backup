...
{
    static size_t size(std::basic_string<CharType> const& s,
                       std::locale const& /*l*/)
    {
        return s.size();
    }
...
class base_num_format : public std::num_put<CharType>
{
  ...
  protected:
    virtual iter_type do_put(iter_type out, std::ios_base& ios, char_type fill,
                             long val) const
    {
  ...
  private:
    ...
    iter_type do_real_put(iter_type out, std::ios_base& ios, char_type fill,
                          ValueType val) const
    {
        ...
        switch (info.display_flags())
        {
            ...
            case flags::date:
                return format_time(out, ios, fill,
                                   static_cast<std::time_t>(val), 'x');
            case flags::time:
                return format_time(out, ios, fill,
                                   static_cast<std::time_t>(val), 'X');
            case flags::datetime:
                return format_time(out, ios, fill,
                                   static_cast<std::time_t>(val), 'c');
            case flags::strftime:
                return format_time(out, ios, fill,
            ...
            case flags::currency:
            {
                ...
                bool intl = !nat;
                return do_format_currency(intl, out, ios, fill,
    ...
    virtual iter_type do_format_currency(bool intl, iter_type out,
                                         std::ios_base& ios, char_type fill,
                                         long double val) const
    {
        if (intl)
    ...
    template <bool intl>
    iter_type format_currency(iter_type out, std::ios_base& ios, char_type fill,
                              long double val) const
    {
        std::locale loc = ios.getloc();
        int digits =
            std::use_facet<std::moneypunct<char_type, intl>>(loc).frac_digits();
        while (digits > 0)
        {
            val *= 10;
            digits--;
        ...
        out = std::use_facet<std::money_put<char_type>>(loc).put(out, intl, ios,
                                                                 fill, val);
        ios.flags(f);
        return out;
    }

    iter_type format_time(iter_type out, std::ios_base& ios, char_type fill,
                          std::time_t time, char c) const
    {
        string_type fmt;
        fmt += char_type('%');
        fmt += char_type(c);
    ...
    iter_type format_time(iter_type out, std::ios_base& ios, char_type fill,
                          std::time_t time, string_type const& format) const
    {
        ...
        std::use_facet<std::time_put<char_type>>(ios.getloc())
            .put(tmp_out, tmp_out, fill, &tm, format.c_str(),
        ...
        if (points < ios.width())
        {
            ...
            std::ios_base::fmtflags flags =
                ios.flags() & std::ios_base::adjustfield;

            //
            // we do not really know internal point, so we assume that it does
            // not exist. so according to the standard field should be right
            // aligned
            //
...
class base_num_parse : public std::num_get<CharType>
{
  ...
  protected:
    ...
    virtual iter_type do_get(iter_type in, iter_type end, std::ios_base& ios,
...
#ifndef BOOST_NO_LONG_LONG
    virtual iter_type do_get(iter_type in, iter_type end, std::ios_base& ios,
                             std::ios_base::iostate& err, long long& val) const
    {
        return do_real_get(in, end, ios, err, val);
    }
  ...
  private:
    template <typename ValueType>
    iter_type do_real_get(iter_type in, iter_type end, std::ios_base& ios,
                          std::ios_base::iostate& err, ValueType& val) const
    {
    ...
    template <bool intl>
    iter_type parse_currency(iter_type in, iter_type end, std::ios_base& ios,
                             std::ios_base::iostate& err,
                             long double& val) const
    {
        std::locale loc = ios.getloc();
        int digits =
            std::use_facet<std::moneypunct<char_type, intl>>(loc).frac_digits();
        long double rval;
        in = std::use_facet<std::money_get<char_type>>(loc).get(in, end, intl,
                                                                ios, err, rval);
