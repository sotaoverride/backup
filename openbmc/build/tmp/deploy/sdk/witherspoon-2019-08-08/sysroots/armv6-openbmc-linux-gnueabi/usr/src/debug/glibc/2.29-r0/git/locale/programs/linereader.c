...
struct linereader* lr_create(FILE* fp, const char* fname, kw_hash_fct_t hf)
{
...
struct token* lr_token(struct linereader* lr, const struct charmap_t* charmap,
                       struct localedef_t* locale,
    ...
    else
    {
        ...
        while ((ch = lr_getc(lr)) != '"' && ch != '\n' && ch != EOF)
        {
                        ...
                        if (seq == NULL)
                        {
                            ...
                            if (locale != NULL)
                            {
                                ...
                                if ((locale->avail & CTYPE_LOCALE) == 0)
                                {
                                    ...
                                    locale->needed = 0;
                                    locale =
                                        load_locale(LC_CTYPE, locale->name,
                                ...
                                if ((locale->avail & CTYPE_LOCALE) != 0 &&
                                    ((translit = find_translit(locale, charmap,
                                                               wch)) != NULL))
            ...
            /* We now have the symbolic name in buf[startidx] to
               ...
               order).  */
            seq =
                charmap_find_value(charmap, &buf[startidx], bufact - startidx);
        ...
        {
            ADDWC(0);
            lr->token.val.str.startwc =
                xrealloc(buf2, buf2act * sizeof(uint32_t));
