...
void nsswitch_check_reload(void)
{
        ...
        ((t = time(NULL)) > (cached_shadow_lastcheck + CACHED_SHADOW_TIMEOUT)))
    {
        ...
        {
            log_log(LOG_ERR, "stat(%s) failed: %s", NSSWITCH_FILE,
                    strerror(errno));
                       ...
                       const char* filename, int lnr)
{
    ...
    while (services[i] != '\0')
    {
        ...
        if ((strncmp(services + i, service, l) == 0) &&
            (!isalnum(services[i + l])))
            return 1;
        ...
        if (services[i] == '[')
        {
            ...
            if (services[i] != ']')
            {
                log_log(LOG_WARNING, "%s: error parsing line %d", filename,
                        lnr);
...
static int shadow_uses_ldap(void)
{
    ...
    while (fgets(linebuf, sizeof(linebuf), fp) != NULL)
    {
        ...
        if (services != NULL)
            passwd_has_ldap =
                has_service(services, MODULE_NAME, NSSWITCH_FILE, lnr);
    }
    fclose(fp);
    if (shadow_found)
        return 0;
