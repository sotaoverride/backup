...
static struct salt* find_by_salt(unsigned char* salt, size_t salt_len)
{
    ...
    for (i = 0, p = salt_list; i < num_salt; i++, p++)
        if ((p->salt_len == salt_len) && !memcmp(p->salt, salt, salt_len))
            return p;
    return NULL;
}
...
static void parse_salt(char* salt_str, int flags)
{
    ...
    else if (strncmp(cp, "0x", 2) == 0)
    {
        ...
        while (*cp)
        {
            ...
            if (!h || !l)
                goto invalid_salt;
            buf[salt_len++] = (((unsigned char)(h - hexchars) << 4) +
                               (unsigned char)(l - hexchars));
        }
    }
...
static void set_policy(struct salt* set_salt, int pad, int argc, char* argv[],
...
static void insert_key_into_keyring(const char* keyring, struct salt* salt)
{
    ...
    sprintf(key_ref_full, "%s%s", EXT2FS_KEY_DESC_PREFIX, salt->key_ref_str);
    rc = keyctl(KEYCTL_SEARCH, keyring_id, EXT2FS_KEY_TYPE_LOGON, key_ref_full,
                0);
    if (rc != -1)
    {
    ...
    rc = add_key(EXT2FS_KEY_TYPE_LOGON, key_ref_full, (void*)&key, sizeof(key),
                 keyring_id);
    if (rc == -1)
    {
        if (errno == EDQUOT)
...
static void do_add_key(int argc, char** argv, const struct cmd_desc* cmd)
{
    ...
    {
        pbkdf2_sha512(in_passphrase, salt, EXT4_PBKDF2_ITERATIONS, salt->key);
...
static void do_set_policy(int argc, char** argv, const struct cmd_desc* cmd)
{
    ...
    if ((strlen(argv[optind]) != (EXT4_KEY_DESCRIPTOR_SIZE * 2)) ||
        hex2byte(argv[optind], (EXT4_KEY_DESCRIPTOR_SIZE * 2), saltbuf.key_desc,
                 EXT4_KEY_DESCRIPTOR_SIZE))
    {
...
#define CMD(name)                                                              \
    {                                                                          \
#name, do_##name, name##_desc, name##_help, 0                          \
    }
#define _CMD(name)                                                             \
    {                                                                          \
#name, do_##name, NULL, NULL, CMD_HIDDEN                               \
    }

const struct cmd_desc cmd_list[] = {
    _CMD(help),       CMD(add_key),    CMD(get_policy),
    CMD(new_session), CMD(set_policy), {NULL, NULL, NULL, NULL, 0}};
