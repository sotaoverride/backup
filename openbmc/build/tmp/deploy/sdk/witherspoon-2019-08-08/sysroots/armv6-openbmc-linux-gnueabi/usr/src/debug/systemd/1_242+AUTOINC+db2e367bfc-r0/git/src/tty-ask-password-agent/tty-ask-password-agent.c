...
static int ask_password_plymouth(const char* message, usec_t until,
                                 AskPasswordFlags flags, const char* flag_file,
                                 char*** ret)
{
    ...
    if (flag_file)
    {
        ...
        r = inotify_add_watch(notify, flag_file,
                              IN_ATTRIB); /* for the link count */
        if (r < 0)
            return -errno;
    }
    ...
    for (;;)
    {
        ...
        if (buffer[0] == 5)
        {
            ...
            if (flags & ASK_PASSWORD_ACCEPT_CACHED)
            {
                ...
                if (asprintf(&packet, "*\002%c%s%n", (int)(strlen(message) + 1),
                             message, &n) < 0)
                {
                    r = -ENOMEM;
                    goto finish;
...
static int parse_password(const char* filename, char** wall)
{
    ...
    r = config_parse(NULL, filename, NULL, NULL, config_item_table_lookup,
                     items, CONFIG_PARSE_RELAXED | CONFIG_PARSE_WARN, NULL);
    if (r < 0)
        return r;
    ...
    else
    {
        ...
        if (access(socket_name, W_OK) < 0)
        {
            if (arg_action == ACTION_QUERY)
                log_info("Not querying '%s' (PID %u), lacking privileges.",
                         message, pid);
        ...
        if (arg_plymouth)
            r = ask_password_plymouth(message, not_after,
                                      accept_cached ? ASK_PASSWORD_ACCEPT_CACHED
                                                    : 0,
                                      filename, &passwords);
        else
        {
            ...
            if (arg_console)
            {
                ...
                tty_fd =
                    acquire_terminal(con, ACQUIRE_TERMINAL_WAIT, USEC_INFINITY);
                if (tty_fd < 0)
                    return log_error_errno(tty_fd, "Failed to acquire %s: %m",
                                           con);
...
static bool wall_tty_match(const char* path, void* userdata)
{
    ...
    if (asprintf(&p, "/run/systemd/ask-password-block/%u:%u", major(st.st_rdev),
                 minor(st.st_rdev)) < 0)
    {
        log_oom();
        return true;
...
static int show_passwords(void)
{
    ...
    FOREACH_DIRENT_ALL(
        de, d, return log_error_errno(errno, "Failed to read directory: %m"))
    {
                          ...
                          IN_CLOSE_WRITE | IN_MOVED_TO) < 0)
    {
        if (errno == ENOSPC)
            return log_error_errno(
                errno, "Failed to add /run/systemd/ask-password to directory "
                       "watch: inotify watch limit reached");
        else
            return log_error_errno(errno,
                                   "Failed to add /run/systemd/ask-password to "
                                   "directory watch: %m");
    }
...
/*
 * To be able to ask on all terminal devices of /dev/console the devices are
 * collected. If more than one device is found, then on each of the terminals a
 * inquiring task is forked.  Every task has its own session and its own
 * controlling terminal.  If one of the tasks does handle a password, the
 * remaining tasks will be terminated.
 */
static int ask_on_this_console(const char* tty, pid_t* ret_pid,
                               char** arguments)
{
...
static int ask_on_consoles(char* argv[])
{
    ...
    if (r < 0)
        return log_error_errno(
            r, "Failed to determine devices of /dev/console: %m");
    ...
    if (!is_clean_exit(status.si_code, status.si_status, EXIT_CLEAN_DAEMON,
                       NULL))
