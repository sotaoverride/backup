...
typedef struct _instanceData
{
    ...
    pthread_mutex_t
        mutWrite;    /* guard against multiple instances writing to same pipe */
...
BEGINinitConfVars /* (re)set config variables to default values */
    CODESTARTinitConfVars ENDinitConfVars
...
ENDisCompatibleWithFeature
    ...
    BEGINdbgPrintInstInfo CODESTARTdbgPrintInstInfo dbgprintf("pipe %s",
                                                              pData->pipe);
if (pData->fd == -1)
    dbgprintf(" (unused)");
ENDdbgPrintInstInfo
    ...
    if (pData->fd < 0)
    {
        ...
        if (!pData->bHadError)
        {
            LogError(errno, RS_RET_NO_FILE_ACCESS,
                     "Could not open output pipe '%s':", pData->pipe);
            pData->bHadError = 1;
        }
...
static rsRetVal writePipe(uchar** ppString, instanceData* pData)
{
    ...
    if (pData->fd == -1)
    {
        ...
        if ((iRetLocal != RS_RET_OK) || (pData->fd == -1))
            ABORT_FINALIZE(RS_RET_SUSPENDED); /* whatever the failure was, we
                                                 need to retry */
    }
...
ENDfreeCnf
    ...
    BEGINcreateInstance CODESTARTcreateInstance pData->pipe = NULL;
pData->fd = -1;
pData->bHadError = 0;
...
ENDfreeInstance
    ...
    BEGINfreeWrkrInstance CODESTARTfreeWrkrInstance ENDfreeWrkrInstance
        ...
        BEGINtryResume instanceData* __restrict__ const pData =
            pWrkrData->pData;
fd_set wrds;
struct timeval tv;
int ready;
...
CHKiRet(OMSRsetEntry(*ppOMSR, 0,
                     (uchar*)strdup((pData->tplName == NULL)
                                        ? "RSYSLOG_FileFormat"
                                        : (char*)pData->tplName),
...
ENDnewActInst
    ...
    BEGINparseSelectorAct CODESTARTparseSelectorAct
...
++p;
CHKiRet(cflineParseFileName(p, (uchar*)pData->pipe, *ppOMSR, 0,
                            OMSR_NO_RQD_TPL_OPTS, getDfltTpl()));

CODE_STD_FINALIZERparseSelectorAct ENDparseSelectorAct

    BEGINdoHUP CODESTARTdoHUP if (pData->fd != -1)
{
    close(pData->fd);
    pData->fd = -1;
}
ENDdoHUP
    ...
    BEGINmodExit CODESTARTmodExit ENDmodExit
        ...
        BEGINqueryEtryPt CODESTARTqueryEtryPt CODEqueryEtryPt_STD_OMOD_QUERIES
            CODEqueryEtryPt_STD_OMOD8_QUERIES CODEqueryEtryPt_doHUP
                        ...
                        CODEqueryEtryPt_STD_CONF2_setModCnf_QUERIES
                            CODEqueryEtryPt_STD_CONF2_OMOD_QUERIES
                                ENDqueryEtryPt
                                    ...
                                    BEGINmodInit(Pipe) CODESTARTmodInit
    INITLegCnfVars* ipIFVersProvided =
