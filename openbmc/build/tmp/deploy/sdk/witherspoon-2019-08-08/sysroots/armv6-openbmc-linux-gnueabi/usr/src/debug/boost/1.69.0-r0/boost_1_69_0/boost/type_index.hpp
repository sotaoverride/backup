...
/// library.
///
/// By inclusion of this file most optimal type index classes will be included
/// and used as a boost::typeindex::type_index and boost::typeindex::type_info.
...
#pragma detect_mismatch(                                                       \
    ...
    "user defined type_index class is used: " BOOST_STRINGIZE(                 \
...
#ifdef BOOST_HAS_PRAGMA_DETECT_MISMATCH
#pragma detect_mismatch("boost__type_index__abi",                              \
                        "RTTI is off - typeid() is used only for templates")
#endif
#else
#ifdef BOOST_HAS_PRAGMA_DETECT_MISMATCH
...
/// boost::typeindex::ctti_type_index class to deduce the name of a type. If
/// your compiler is not recognized by the TypeIndex library and you wish to
/// work with boost::typeindex::ctti_type_index, you may define this macro by
/// yourself.
///
/// BOOST_TYPE_INDEX_FUNCTION_SIGNATURE must be defined to a compiler specific
/// macro that outputs the \b whole function signature \b including \b template
/// \b parameters.
///
/// If your compiler is not recognised and BOOST_TYPE_INDEX_FUNCTION_SIGNATURE
/// is not defined, then a compile-time error will arise at any attempt to use
/// boost::typeindex::ctti_type_index classes.
///
/// See BOOST_TYPE_INDEX_REGISTER_CTTI_PARSING_PARAMS and
/// BOOST_TYPE_INDEX_CTTI_USER_DEFINED_PARSING for an information of how to tune
/// the implementation to make a nice pretty_name() output.
...
/// '(begin_skip, end_skip, runtime_skip, runtime_skip_until)' with parameters
/// for adding a support for compilers, that by default are not recognized by
/// TypeIndex library.
///
/// \b Example:
///
/// Imagine the situation when
/// \code boost::typeindex::ctti_type_index::type_id<int>().pretty_name()
/// \endcode returns the following string: \code "static const char
/// *boost::detail::ctti<int>::n() [T = int]" \endcode and \code
/// boost::typeindex::ctti_type_index::type_id<short>().pretty_name() \endcode
/// returns the following: \code "static const char
/// *boost::detail::ctti<short>::n() [T = short]" \endcode
///
/// As we may see first 39 characters are "static const char
/// *boost::detail::ctti<" and they do not depend on the type T. After first 39
/// characters we have a human readable type name which is duplicated at the end
...
/// \code "short>::n() [T = short" \endcode for
/// `boost::typeindex::ctti_type_index::type_id<short>().pretty_name()`.
///
/// Now we need to take additional care of the characters that go before the
/// last mention of our type. We'll do that by telling the macro that we need to
/// cut off everything that goes before the "T = " including the "T = " itself:
///
...
/// \endcode
/// \param begin_skip How many characters must be skipped at the beginning of
/// the type holding string. Must be a compile time constant. \param end_skip
/// How many characters must be skipped at the end of the type holding string.
/// Must be a compile time constant.
/// \param runtime_skip Do we need additional checks at runtime to cut off the
/// more characters. Must be `true` or `false`. \param runtime_skip_until Skip
/// all the characters before the following string (including the string
/// itself). Must be a compile time array of characters.
///
/// See [RTTI emulation
/// limitations](boost_typeindex/rtti_emulation_limitations.html) for more info.
...
/// Depending on a compiler flags, optimal implementation of type_info will be
/// used as a default boost::typeindex::type_info.
///
...
#define BOOST_TYPE_INDEX_USER_TYPEINDEX                                        \
...
/// type detection.
///
/// Depending on the typeid() availability this macro will expand to nothing or
/// to virtual helper function `virtual const type_info&
...
/// defined if mixing RTTI on/off modules. See [Mixing sources with RTTI on and
/// RTTI off](boost_typeindex/mixing_sources_with_rtti_on_and_.html) section of
...
///
/// \b Requirements: RTTI available or Base and Derived classes must be marked
/// with BOOST_TYPE_INDEX_REGISTER_CLASS.
///
/// \b Example:
/// \code
