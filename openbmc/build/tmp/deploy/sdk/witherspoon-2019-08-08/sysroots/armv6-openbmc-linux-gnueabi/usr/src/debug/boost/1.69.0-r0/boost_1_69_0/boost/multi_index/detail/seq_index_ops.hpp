...
void sequenced_index_collate(BOOST_DEDUCED_TYPENAME Node::impl_type* x,
                             ...
                             Compare comp)
{
    ...
    while (first0 != last0 && first1 != last1)
    {
        if (comp(Node::from_impl(first1)->value(),
                 Node::from_impl(first0)->value()))
        {
...
void sequenced_index_sort(Node* header, Compare comp)
{
     ...
     * http://www.cs.rpi.edu/~musser/gp/List/lists1.html. The implementation is
     * a little convoluted: in the original code counter elements and carry are
     * std::lists: here we do not want to use multi_index instead, so we do
     * things at a lower level, managing directly the internal node
     * representation. Incidentally, the implementations I've seen of this
     * algorithm (SGI, Dinkumware, STLPort) are not exception-safe: this is.
     * Moreover, we do not use any dynamic storage.
     */

    if (header->next() == header->impl() ||
        header->next()->next() == header->impl())
        return;
    ...
    typedef typename aligned_storage<
        sizeof(impl_type), alignment_of<impl_type>::value>::type carry_spc_type;
    carry_spc_type carry_spc;
    impl_type& carry = *reinterpret_cast<impl_type*>(&carry_spc);
    typedef typename aligned_storage<
        sizeof(impl_type[sequenced_index_sort_max_fill]),
        alignment_of<impl_type[sequenced_index_sort_max_fill]>::value>::type
        counter_spc_type;
    counter_spc_type counter_spc;
    impl_type* counter = reinterpret_cast<impl_type*>(&counter_spc);
    std::size_t fill = 0;

    carry.prior() = carry.next() = static_cast<impl_pointer>(&carry);
    counter[0].prior() = counter[0].next() =
        static_cast<impl_pointer>(&counter[0]);
    ...
    BOOST_TRY
    {
        while (header->next() != header->impl())
        {
            ...
            std::size_t i = 0;
            while (i < fill &&
                   counter[i].next() != static_cast<impl_pointer>(&counter[i]))
            {
                sequenced_index_collate<Node>(&carry, &counter[i++], comp);
            }
            impl_type::swap(static_cast<impl_pointer>(&carry),
        ...
        impl_type::swap(header->impl(),
                        static_cast<impl_pointer>(&counter[fill - 1]));
    }
    BOOST_CATCH(...)
    {
        impl_type::relink(header->impl(), carry.next(),
                          static_cast<impl_pointer>(&carry));
        for (std::size_t i = 0; i <= fill; ++i)
        {
            impl_type::relink(header->impl(), counter[i].next(),
                              static_cast<impl_pointer>(&counter[i]));
        }
        BOOST_RETHROW;
    }
