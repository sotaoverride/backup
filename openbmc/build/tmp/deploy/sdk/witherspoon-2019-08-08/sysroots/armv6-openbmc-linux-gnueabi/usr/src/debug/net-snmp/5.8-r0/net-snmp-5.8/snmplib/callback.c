...
static struct snmp_gen_callback* thecallbacks[MAX_CALLBACK_IDS]
                                             [MAX_CALLBACK_SUBIDS];
...
NETSNMP_STATIC_INLINE int _callback_lock(int major, int minor, const char* warn,
                                         int do_assert)
{
    int lock_holded = 0;
    ...
    DEBUGMSGTL(("9:callback:lock", "locked (%s,%s)\n", types[major],
                (SNMP_CALLBACK_LIBRARY == major) ? SNMP_STRORNULL(lib[minor])
                                                 : "null"));
#endif
    ...
    if (lock_holded >= 100)
    {
        if (NULL != warn)
            snmp_log(LOG_WARNING,
                     "lock in _callback_lock sleeps more than 100 milliseconds "
                     "in %s\n",
                     warn);
        if (do_assert)
 ...
 */
int netsnmp_register_callback(int major, int minor, SNMPCallback* new_callback,
...
int snmp_unregister_callback(int major, int minor, SNMPCallback* target,
...
int netsnmp_callback_clear_client_arg(void* ptr, int i, int j)
{
    ...
    for (; i < MAX_CALLBACK_IDS; i++, j = 0)
    {
        for (; j < MAX_CALLBACK_SUBIDS; j++)
        {
            ...
            {
                if ((NULL != scp->sc_callback) &&
                    (scp->sc_client_arg != NULL) && (scp->sc_client_arg == ptr))
                {
