    ...
    netsnmp_feature_child_of(ipaddress_common_copy_utilities, ipaddress_common)
        netsnmp_feature_child_of(ipaddress_entry_copy, ipaddress_common)
            netsnmp_feature_child_of(ipaddress_entry_update, ipaddress_common)
                netsnmp_feature_child_of(ipaddress_prefix_copy,
                                         ipaddress_common_copy_utilities)
...
/**
 */
netsnmp_container* netsnmp_access_ipaddress_container_init(u_int flags)
{
    ...
    if (flags & NETSNMP_ACCESS_IPADDRESS_INIT_ADDL_IDX_BY_ADDR)
    {
        netsnmp_container* container2 = netsnmp_container_find(
            "ipaddress_addr:access_ipaddress:table_container");
        if (NULL == container2)
        {
 ...
 */
static netsnmp_container* _remove_duplicates(netsnmp_container* container,
                                             u_int container_flags)
{
    netsnmp_container* c;
    netsnmp_iterator* it;
    ...
    {
        if (prev_entry &&
            _access_ipaddress_entry_compare_addr(prev_entry, entry) == 0)
        {
                                            ...
                                            u_int load_flags)
{
    ...
    if (0 != rc)
    {
        netsnmp_access_ipaddress_container_free(
            container, NETSNMP_ACCESS_IPADDRESS_FREE_NOFLAGS);
        container = NULL;
    }
                                             ...
                                             u_int free_flags)
{
    ...
    if (!(free_flags & NETSNMP_ACCESS_IPADDRESS_FREE_DONT_CLEAR))
    {
        ...
        CONTAINER_CLEAR(
            container,
            (netsnmp_container_obj_func*)_access_ipaddress_entry_release, NULL);
    }
...
/**
 */
netsnmp_ipaddress_entry* netsnmp_access_ipaddress_entry_create(void)
{
...
int netsnmp_access_ipaddress_entry_set(netsnmp_ipaddress_entry* entry)
{
    ...
    else
    {
        ...
        netsnmp_assert(
            !"ipaddress_entry_set == unknown mode"); /* always false */
        rc = -1;
    }
 ...
 */
int netsnmp_ipaddress_prefix_copy(u_char* dst, u_char* src, int addr_len,
                                  int pfx_len)
{
    int bytes = pfx_len / 8;
    int bits = pfx_len % 8;
...
/**
 */
void _access_ipaddress_entry_release(netsnmp_ipaddress_entry* entry,
                                     void* context)
{
    netsnmp_access_ipaddress_entry_free(entry);
}
        ...
        {               /*Link Layer*/
            if (!flags) /*Global address assigned by router adv*/
                (*ipAddressPrefixOrigin) =
                    5 /*IPADDRESSPREFIXORIGINTC_ROUTERADV*/;
            else
                (*ipAddressPrefixOrigin) =
                    3 /*IPADDRESSPREFIXORIGINTC_WELLKNOWN*/;
        }
