...
static void g_local_file_finalize(GObject* object)
{
    GLocalFile* local;
...
static void g_local_file_class_init(GLocalFileClass* klass)
{
    ...
    g_file_attribute_info_list_add(list, G_FILE_ATTRIBUTE_UNIX_MODE,
                                   G_FILE_ATTRIBUTE_TYPE_UINT32,
                                   G_FILE_ATTRIBUTE_INFO_COPY_WITH_FILE |
                                       G_FILE_ATTRIBUTE_INFO_COPY_WHEN_MOVED);

#ifdef G_OS_UNIX
    g_file_attribute_info_list_add(list, G_FILE_ATTRIBUTE_UNIX_UID,
                                   G_FILE_ATTRIBUTE_TYPE_UINT32,
                                   G_FILE_ATTRIBUTE_INFO_COPY_WHEN_MOVED);
    g_file_attribute_info_list_add(list, G_FILE_ATTRIBUTE_UNIX_GID,
                                   G_FILE_ATTRIBUTE_TYPE_UINT32,
                                   G_FILE_ATTRIBUTE_INFO_COPY_WHEN_MOVED);
#endif
    ...
    g_file_attribute_info_list_add(list,
                                   ...
                                   G_FILE_ATTRIBUTE_TYPE_BYTE_STRING, 0);
#endif

#ifdef HAVE_UTIMES
    g_file_attribute_info_list_add(list, G_FILE_ATTRIBUTE_TIME_MODIFIED,
     ...
     */
    g_file_attribute_info_list_add(list, G_FILE_ATTRIBUTE_TIME_ACCESS,
                                   G_FILE_ATTRIBUTE_TYPE_UINT64,
                                   G_FILE_ATTRIBUTE_INFO_COPY_WHEN_MOVED);
    g_file_attribute_info_list_add(list, G_FILE_ATTRIBUTE_TIME_ACCESS_USEC,
                                   G_FILE_ATTRIBUTE_TYPE_UINT32,
                                   G_FILE_ATTRIBUTE_INFO_COPY_WHEN_MOVED);
#endif
...
static void g_local_file_init(GLocalFile* local)
{
}
 ...
 * Returns: a #GFile
 */
GFile* g_local_file_new_from_dirname_and_basename(const gchar* dirname,
                                                  const gchar* basename)
{
    GLocalFile* local;

    g_return_val_if_fail(dirname != NULL, NULL);
    g_return_val_if_fail(basename && basename[0] && !strchr(basename, '/'),
                         NULL);
...
static gboolean g_local_file_is_native(GFile* file)
{
    return TRUE;
}

static gboolean g_local_file_has_uri_scheme(GFile* file, const char* uri_scheme)
{
    return g_ascii_strcasecmp(uri_scheme, "file") == 0;
}

static char* g_local_file_get_uri_scheme(GFile* file)
{
    return g_strdup("file");
}

static char* g_local_file_get_basename(GFile* file)
{
    return g_path_get_basename(G_LOCAL_FILE(file)->filename);
}

static char* g_local_file_get_path(GFile* file)
{
    return g_strdup(G_LOCAL_FILE(file)->filename);
}

static char* g_local_file_get_uri(GFile* file)
{
...
static gboolean name_is_valid_for_display(const char* string,
                                          gboolean is_valid_utf8)
{
    char c;

    if (!is_valid_utf8 && !g_utf8_validate(string, -1, NULL))
        return FALSE;
...
static char* g_local_file_get_parse_name(GFile* file)
{
    const char* filename;
    char* parse_name;
    const gchar* charset;
    ...
    {
        utf8_filename =
            g_convert(filename, -1, "UTF-8", charset, NULL, NULL, NULL);
        ...
        if (utf8_filename != NULL)
        {
            ...
            roundtripped_filename = g_convert(utf8_filename, -1, charset,
    ...
    else
    {
        ...
        escaped_path = g_uri_escape_string(
            filename, G_URI_RESERVED_CHARS_ALLOWED_IN_PATH_ELEMENT "/", TRUE);
        parse_name = g_strconcat("file://", (*escaped_path != '/') ? "/" : "",
                                 escaped_path, NULL);
...
static GFile* g_local_file_get_parent(GFile* file)
{
...
static GFile* g_local_file_dup(GFile* file)
{
    GLocalFile* local = G_LOCAL_FILE(file);
...
static guint g_local_file_hash(GFile* file)
{
    GLocalFile* local = G_LOCAL_FILE(file);
...
static gboolean g_local_file_equal(GFile* file1, GFile* file2)
{
...
static gboolean g_local_file_prefix_matches(GFile* parent, GFile* descendant)
{
    ...
    remainder =
        match_prefix(descendant_local->filename, parent_local->filename);
    if (remainder != NULL && G_IS_DIR_SEPARATOR(*remainder))
        return TRUE;
    return FALSE;
}

static char* g_local_file_get_relative_path(GFile* parent, GFile* descendant)
{
    GLocalFile* parent_local = G_LOCAL_FILE(parent);
    GLocalFile* descendant_local = G_LOCAL_FILE(descendant);
    const char* remainder;
...
static GFile* g_local_file_resolve_relative_path(GFile* file,
...
static GFileEnumerator*
    g_local_file_enumerate_children(GFile* file, const char* attributes,
                                    GFileQueryInfoFlags flags,
                                    GCancellable* cancellable, GError** error)
{
    GLocalFile* local = G_LOCAL_FILE(file);
    return _g_local_file_enumerator_new(local, attributes, flags, cancellable,
                                        error);
}

static GFile* g_local_file_get_child_for_display_name(GFile* file,
...
static const char* get_fs_type(long f_type)
{
...
static gboolean device_equal(gconstpointer v1, gconstpointer v2)
{
    return *(dev_t*)v1 == *(dev_t*)v2;
}

static guint device_hash(gconstpointer v)
{
    return (guint) * (dev_t*)v;
}

static void get_mount_info(GFileInfo* fs_info, const char* path,
                           GFileAttributeMatcher* matcher)
{
    ...
    if (mount_info_hash == NULL)
        mount_info_hash =
            g_hash_table_new_full(device_hash, device_equal, g_free, NULL);

    if (g_unix_mounts_changed_since(mount_info_hash_cache_time))
        g_hash_table_remove_all(mount_info_hash);

    got_info = g_hash_table_lookup_extended(mount_info_hash, &buf.st_dev, NULL,
                                            &info_as_ptr);
    ...
    if (mount_info & MOUNT_INFO_READONLY)
        g_file_info_set_attribute_boolean(
            fs_info, G_FILE_ATTRIBUTE_FILESYSTEM_READONLY, TRUE);
}
...
static gboolean is_xp_or_later(void)
{
    static int result = -1;
    ...
    if (result == -1)
    {
        ...
        result =
            VerifyVersionInfo(&ver_info, VER_MAJORVERSION | VER_MINORVERSION,
                              cond_mask) != 0;
#else
...
static wchar_t* get_volume_for_path(const char* path)
{
    long len;
    wchar_t* wpath;
    wchar_t* result;
...
static void get_filesystem_readonly(GFileInfo* info, const char* path)
{
    wchar_t* rootdir;
    ...
    if (rootdir)
    {
        ...
        {
            DWORD flags;
            if (GetVolumeInformationW(rootdir, NULL, 0, NULL, NULL, &flags,
                                      NULL, 0))
                g_file_info_set_attribute_boolean(
...
static void g_set_io_error(GError** error, const gchar* msg, GFile* file,
                           gint errsv)
{
    GLocalFile* local = G_LOCAL_FILE(file);
    gchar* display_name;
    ...
    g_set_error(error, G_IO_ERROR, g_io_error_from_errno(errsv), msg,
                display_name, g_strerror(errsv));
    g_free(display_name);
}
#pragma GCC diagnostic pop

static GFileInfo* g_local_file_query_filesystem_info(GFile* file,
                                                     ...
                                                     GError** error)
{
...
#elif STATFS_ARGS == 4
    statfs_result =
        statfs(local->filename, &statfs_buffer, sizeof(statfs_buffer), 0);
#endif /* STATFS_ARGS == 2 */
    ...
    if (statfs_result == 0 && statfs_buffer.f_bavail == 0 &&
        statfs_buffer.f_bfree == 0 &&
    ...
    if (statfs_result == -1)
    {
        ...
        g_set_io_error(error, _("Error getting filesystem info for %s: %s"),
                       file, errsv);
        return NULL;
    }
    ...
    if (!no_size && g_file_attribute_matcher_matches(
                        attribute_matcher, G_FILE_ATTRIBUTE_FILESYSTEM_FREE))
    {
#ifdef G_OS_WIN32
        ...
        if (GetDiskFreeSpaceExW(wdirname, &li, NULL, NULL))
            g_file_info_set_attribute_uint64(
                info, G_FILE_ATTRIBUTE_FILESYSTEM_FREE, (guint64)li.QuadPart);
        g_free(wdirname);
#else
#if defined(USE_STATFS) || defined(USE_STATVFS)
        g_file_info_set_attribute_uint64(info, G_FILE_ATTRIBUTE_FILESYSTEM_FREE,
                                         block_size * statfs_buffer.f_bavail);
#endif
#endif
    }
    if (!no_size && g_file_attribute_matcher_matches(
                        attribute_matcher, G_FILE_ATTRIBUTE_FILESYSTEM_SIZE))
    {
#ifdef G_OS_WIN32
        gchar* localdir = g_path_get_dirname(local->filename);
        wchar_t* wdirname = g_utf8_to_utf16(localdir, -1, NULL, NULL, NULL);
        ...
        if (GetDiskFreeSpaceExW(wdirname, NULL, &li, NULL))
            g_file_info_set_attribute_uint64(
                info, G_FILE_ATTRIBUTE_FILESYSTEM_SIZE, (guint64)li.QuadPart);
        g_free(wdirname);
#else
#if defined(USE_STATFS) || defined(USE_STATVFS)
        g_file_info_set_attribute_uint64(info, G_FILE_ATTRIBUTE_FILESYSTEM_SIZE,
                                         block_size * statfs_buffer.f_blocks);
#endif
#endif /* G_OS_WIN32 */
    }

    if (!no_size && g_file_attribute_matcher_matches(
                        attribute_matcher, G_FILE_ATTRIBUTE_FILESYSTEM_USED))
    {
#ifdef G_OS_WIN32
        gchar* localdir = g_path_get_dirname(local->filename);
        wchar_t* wdirname = g_utf8_to_utf16(localdir, -1, NULL, NULL, NULL);
        ...
        if (GetDiskFreeSpaceExW(wdirname, &li_free, &li_total, NULL))
            g_file_info_set_attribute_uint64(
                ...
                (guint64)li_total.QuadPart - (guint64)li_free.QuadPart);
        g_free(wdirname);
#else
        g_file_info_set_attribute_uint64(
            info, G_FILE_ATTRIBUTE_FILESYSTEM_USED,
            block_size * (statfs_buffer.f_blocks - statfs_buffer.f_bfree));
#endif /* G_OS_WIN32 */
    }
    ...
    if (fstype && g_file_attribute_matcher_matches(
                      attribute_matcher, G_FILE_ATTRIBUTE_FILESYSTEM_TYPE))
...
error:
    g_set_io_error(error,
                    ...
                    * exists.
                    */
                   _("Containing mount for file %s not found"), file, 0);

    return NULL;
}

static GFile* g_local_file_set_display_name(GFile* file,
                                            const char* display_name,
                                            GCancellable* cancellable,
                                            GError** error)
{
    ...
    if (g_lstat(new_local->filename, &statbuf) == -1)
    {
        ...
        if (errsv != ENOENT)
        {
            g_set_io_error(error, _("Error renaming file %s: %s"), new_file,
                           errsv);
            return NULL;
        }
    }
    ...
    if (g_rename(local->filename, new_local->filename) == -1)
    {
        ...
        if (errsv == EINVAL)
            ...
            g_set_io_error(error, _("Error renaming file %s: %s"), file, errsv);
...
static GFileInfo* g_local_file_query_info(GFile* file, const char* attributes,
                                          GFileQueryInfoFlags flags,
                                          GCancellable* cancellable,
                                          GError** error)
{
    ...
    info = _g_local_file_info_get(basename, local->filename, matcher, flags,
                                  &parent_info, error);
...
static GFileAttributeInfoList* g_local_file_query_settable_attributes(
    GFile* file, GCancellable* cancellable, GError** error)
{
    return g_file_attribute_info_list_ref(local_writable_attributes);
}

static GFileAttributeInfoList* g_local_file_query_writable_namespaces(
    GFile* file, GCancellable* cancellable, GError** error)
{
    GFileAttributeInfoList* list;
    GVfsClass* class;
    ...
    if (g_once_init_enter(&local_writable_namespaces))
    {
        ...
        g_file_attribute_info_list_add(
            list, "xattr", G_FILE_ATTRIBUTE_TYPE_STRING,
            G_FILE_ATTRIBUTE_INFO_COPY_WITH_FILE |
                G_FILE_ATTRIBUTE_INFO_COPY_WHEN_MOVED);
        g_file_attribute_info_list_add(list, "xattr-sys",
...
static gboolean g_local_file_set_attribute(GFile* file, const char* attribute,
                                           ...
                                           GError** error)
{
    ...
    return _g_local_file_info_set_attribute(local->filename, attribute, type,
                                            value_p, flags, cancellable, error);
}

static gboolean g_local_file_set_attributes_from_info(GFile* file,
                                                      GFileInfo* info,
                                                      GFileQueryInfoFlags flags,
                                                      GCancellable* cancellable,
                                                      GError** error)
{
    ...
    res = _g_local_file_info_set_attributes(local->filename, info, flags,
                                            cancellable, error);
    ...
    chained_res = (default_iface->set_attributes_from_info)(file, info, flags,
                                                            cancellable, error);
...
static GFileInputStream*
    g_local_file_read(GFile* file, GCancellable* cancellable, GError** error)
{
    GLocalFile* local = G_LOCAL_FILE(file);
    int fd, ret;
    GLocalFileStat buf;
...
#endif
        g_set_io_error(error, _("Error opening file %s: %s"), file, errsv);
        return NULL;
    }
    ...
    if (ret == 0 && S_ISDIR(buf.st_mode))
    {
        ...
        g_set_io_error(error, _("Error opening file %s: %s"), file, EISDIR);
        return NULL;
    }
...
static GFileOutputStream* g_local_file_append_to(GFile* file,
...
static GFileOutputStream* g_local_file_create(GFile* file,
                                              GFileCreateFlags flags,
                                              GCancellable* cancellable,
                                              GError** error)
{
    return _g_local_file_output_stream_create(
        G_LOCAL_FILE(file)->filename, FALSE, flags, NULL, cancellable, error);
}

static GFileOutputStream* g_local_file_replace(GFile* file, const char* etag,
                                               ...
                                               GError** error)
{
    return _g_local_file_output_stream_replace(G_LOCAL_FILE(file)->filename,
                                               FALSE, etag, make_backup, flags,
                                               NULL, cancellable, error);
}

static GFileIOStream* g_local_file_open_readwrite(GFile* file,
                                                  GCancellable* cancellable,
                                                  GError** error)
{
...
static GFileIOStream* g_local_file_create_readwrite(GFile* file,
                                                    GFileCreateFlags flags,
                                                    GCancellable* cancellable,
                                                    GError** error)
{
    ...
    output = _g_local_file_output_stream_create(
        G_LOCAL_FILE(file)->filename, TRUE, flags, NULL, cancellable, error);
    if (output == NULL)
        return NULL;
...
static GFileIOStream*
    g_local_file_replace_readwrite(GFile* file, const char* etag,
...
static gboolean g_local_file_delete(GFile* file, GCancellable* cancellable,
                                    GError** error)
{
    GLocalFile* local = G_LOCAL_FILE(file);
    GVfsClass* class;
    ...
    if (g_remove(local->filename) == -1)
    {
       ...
       expects */
        ...
        g_set_io_error(error, _("Error removing file %s: %s"), file, errsv);
        return FALSE;
    }
...
static char* expand_symlink(const char* link)
{
...
static char* expand_symlinks(const char* path, dev_t* dev)
{
    char *tmp, *target;
...
static char* get_parent(const char* path, dev_t* parent_dev)
{
    char *parent, *res;
    char* path_copy;
...
static char* expand_all_symlinks(const char* path)
{
...
char* _g_local_file_find_topdir_for(const char* file)
{
    char* dir;
...
static char* get_unique_filename(const char* basename, int id)
{
    const char* dot;
    ...
    dot = strchr(basename, '.');
    if (dot)
        return g_strdup_printf("%.*s.%d%s", (int)(dot - basename), basename, id,
                               dot);
    else
...
static gboolean path_has_prefix(const char* path, const char* prefix)
{
    ...
    if (strncmp(path, prefix, prefix_len) == 0 &&
        (prefix_len == 0 || /* empty prefix always matches */
         prefix[prefix_len - 1] ==
             '/' || /* last char in prefix was a /, so it must be in path too */
         path[prefix_len] == 0 ||
         path[prefix_len] == '/'))
        return TRUE;
...
static char* try_make_relative(const char* path, const char* base)
{
    char *path2, *base2;
    char* relative;
...
gboolean _g_local_file_has_trash_dir(const char* dirname, dev_t dir_dev)
{
    ...
    if (g_lstat(globaldir, &global_stat) == 0 && S_ISDIR(global_stat.st_mode) &&
    ...
    /* No global trash dir, or it failed the tests, fall back to
     * $topdir/.Trash-$uid */
    uid = geteuid();
    ...
    if (g_lstat(trashdir, &trash_stat) == 0)
    {
        g_free(topdir);
        g_free(trashdir);
        return S_ISDIR(trash_stat.st_mode) && trash_stat.st_uid == uid;
    }
    g_free(trashdir);
...
#ifdef G_OS_UNIX
gboolean _g_local_file_is_lost_found_dir(const char* path, dev_t path_dev)
{
    gboolean ret = FALSE;
    ...
    if (!(S_ISDIR(statbuf.st_mode) && statbuf.st_uid == 0 &&
          statbuf.st_gid == 0))
        goto out;
...
static gboolean g_local_file_trash(GFile* file, GCancellable* cancellable,
                                   GError** error)
{
    GLocalFile* local = G_LOCAL_FILE(file);
    ...
    if (g_lstat(local->filename, &file_stat) != 0)
    {
        ...
        g_set_io_error(error, _("Error trashing file %s: %s"), file, errsv);
        return FALSE;
    }
     ...
     * instead we should stat the directory that the file we're deleting is in
     * as this will have the same st_dev.
     */
    ...
    if (file_stat.st_dev == home_stat.st_dev)
    {
        ...
        if (g_mkdir_with_parents(trashdir, 0700) < 0)
        {
            ...
            g_set_error(error, G_IO_ERROR, g_io_error_from_errno(errsv),
                        _("Unable to create trash dir %s: %s"), display_name,
                        g_strerror(errsv));
            g_free(display_name);
            g_free(trashdir);
            return FALSE;
    ...
    else
    {
            ...
            (global_stat.st_mode & S_ISVTX) != 0)
        {
            ...
            if (g_lstat(trashdir, &trash_stat) == 0)
            {
                if (!S_ISDIR(trash_stat.st_mode) || trash_stat.st_uid != uid)
                {
        ...
        retry:
            ...
            {
                if (!tried_create && g_mkdir(trashdir, 0700) != -1)
                {
    ...
    if (g_rename(local->filename, trashfile) == -1)
    {
        ...
        if (errsv == EXDEV)
            ...
            g_set_io_error(
                error,
                _("Unable to trash file %s across filesystem boundaries"), file,
                ENOTSUP);
        else
            g_set_io_error(error, _("Unable to trash file %s: %s"), file,
                           errsv);
        return FALSE;
    }
                                   ...
                                   GError** error)
{
    ...
    if (success && op.fAnyOperationsAborted)
    {
        ...
        g_set_io_error(error, _("Unable to trash file %s"), file, 0);
...
static gboolean g_local_file_make_directory(GFile* file,
                                            GCancellable* cancellable,
                                            GError** error)
{
    GLocalFile* local = G_LOCAL_FILE(file);
    ...
    if (g_mkdir(local->filename, 0777) == -1)
    {
        ...
        if (errsv == EINVAL)
            ...
            g_set_io_error(error, _("Error creating directory %s: %s"), file,
                           errsv);
        return FALSE;
    }
...
static gboolean g_local_file_make_symbolic_link(GFile* file,
                                                ...
                                                GError** error)
{
    ...
    if (symlink(symlink_value, local->filename) == -1)
    {
        ...
        if (errsv == EINVAL)
            ...
            g_set_io_error(error, _("Error making symbolic link %s: %s"), file,
                           errsv);
        return FALSE;
    }
    return TRUE;
...
static gboolean g_local_file_copy(GFile* source, GFile* destination,
...
static gboolean g_local_file_move(GFile* source, GFile* destination,
                                  GFileCopyFlags flags,
                                  GCancellable* cancellable,
                                  GFileProgressCallback progress_callback,
                                  gpointer progress_callback_data,
                                  GError** error)
{
    ...
    if (!G_IS_LOCAL_FILE(source) || !G_IS_LOCAL_FILE(destination))
    {
    ...
    if (res == -1)
    {
        ...
        g_set_io_error(error, _("Error moving file %s: %s"), source, errsv);
        return FALSE;
    }
    ...
    if (res == 0)
    {
        ...
        {
            g_set_io_error(error, _("Error moving file %s: %s"), source,
                           EEXIST);
            return FALSE;
        }
    }
    ...
    {
        /* Source is a dir, destination exists (and is not a dir, because that
       would have failed earlier), and we're overwriting. Manually remove the
       target so we can do the rename. */
        ...
        if (res == -1)
        {
            ...
            g_set_error(error, G_IO_ERROR, g_io_error_from_errno(errsv),
                        _("Error removing target file: %s"), g_strerror(errsv));
            return FALSE;
        }
    }
    ...
    class = G_VFS_GET_CLASS(vfs);
    if (class->local_file_moved)
        class->local_file_moved(vfs, local_source->filename,
                                              ...
                                              GError** error)
{
    ...
    return g_local_file_monitor_new_for_path(local_file->filename, TRUE, flags,
                                             error);
}

static GFileMonitor* g_local_file_monitor_file(GFile* file,
                                               GFileMonitorFlags flags,
                                               GCancellable* cancellable,
                                               GError** error)
{
    ...
    return g_local_file_monitor_new_for_path(local_file->filename, FALSE, flags,
                                             error);
}
...
static gboolean g_local_file_measure_size_error(GFileMeasureFlags flags,
                                                gint saved_errno, GSList* name,
                                                GError** error)
{
...
static gboolean g_local_file_measure_size_of_contents(gint fd, GSList* dir_name,
                                                      MeasureState* state,
                                                      GError** error);

static gboolean g_local_file_measure_size_of_file(gint parent_fd, GSList* name,
                                                  MeasureState* state,
                                                  GError** error)
{
    GLocalFileStat buf;
    ...
    {
        int errsv = errno;
        return g_local_file_measure_size_error(state->flags, errsv, name,
                                               error);
    }
    ...
    if (state->progress_callback)
    {
        ...
        if (state->last_progress_report)
        {
            ...
            if (state->last_progress_report + 200 * G_TIME_SPAN_MILLISECOND <
                now)
            {
                (*state->progress_callback)(TRUE, state->disk_usage,
                                            state->num_dirs, state->num_files,
                                            state->progress_data);
                                                      ...
                                                      GError** error)
{
    ...
    if (dir == NULL)
    {
        ...
        return g_local_file_measure_size_error(state->flags, saved_errno,
                                               dir_name, error);
    }
...
static gboolean g_local_file_measure_disk_usage(
