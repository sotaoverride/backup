...
static const char doc[] =
    PROGRAM_NAME " version " VERSION " - a tool to resize UBI volumes.";
...
static const char usage[] =
    "Usage: " PROGRAM_NAME
    " <UBI device node file name> [-n <volume id>] [--vol_id=<volume id>]\n\n"
    "         [-N <volume name>] [--name=<volume name>] [-s <bytes>] [-S "
    "<LEBs>] [-h] [--help]\n\n"
    "Example: " PROGRAM_NAME
    " /dev/ubi0 -n 1 -s 1MiB resize UBI volume 1 to 1 MiB on\n"
    "         UBI device corresponding to /dev/ubi0\n"
    "         " PROGRAM_NAME " /dev/ubi0 -N my_vol -s 1MiB - resize UBI volume "
                             "named \"my_vol\" to 1 MiB\n"
...
int main(int argc, char* const argv[])
{
    ...
    if (args.name)
    {
        err = ubi_get_vol_info1_nm(libubi, dev_info.dev_num, args.name,
                                   &vol_info);
        if (err)
        {
    ...
    {
        err =
            ubi_get_vol_info1(libubi, dev_info.dev_num, args.vol_id, &vol_info);
        if (err)
        {
