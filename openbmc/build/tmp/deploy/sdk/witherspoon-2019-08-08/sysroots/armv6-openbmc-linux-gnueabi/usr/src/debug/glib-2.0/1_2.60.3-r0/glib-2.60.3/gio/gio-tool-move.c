...
static const GOptionEntry entries[] = {
    ...
    {"no-copy-fallback", 'C', 0, G_OPTION_ARG_NONE, &no_copy_fallback,
...
int handle_move(int argc, char* argv[], gboolean do_help)
{
    ...
    g_option_context_set_summary(
        context, _("Move one or more files from SOURCE to DEST."));
    ...
    if (!dest_is_dir && argc > 3)
    {
        char* message;
        message =
            g_strdup_printf(_("Target %s is not a directory"), argv[argc - 1]);
    ...
    for (i = 1; i < argc - 1; i++)
    {
        ...
        if (!g_file_move(source, target, flags, NULL,
