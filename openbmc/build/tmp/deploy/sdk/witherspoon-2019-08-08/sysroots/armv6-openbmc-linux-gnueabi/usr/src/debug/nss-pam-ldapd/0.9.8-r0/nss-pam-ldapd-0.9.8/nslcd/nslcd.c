...
static char* sane_environment[] = {"HOME=/", "TMPDIR=/tmp", "LDAPNOINIT=1",
                                   NULL};
...
static void display_version(FILE* fp)
{
    ...
    fprintf(fp, "Copyright (C) 1997-2017 Luke Howard, Arthur de Jong and West "
                "Consulting\n"
...
static struct option const nslcd_options[] = {
    {"check", no_argument, NULL, 'c'},   {"debug", no_argument, NULL, 'd'},
    {"nofork", no_argument, NULL, 'n'},  {"help", no_argument, NULL, 'h'},
...
static void parse_cmdline(int argc, char* argv[])
{
                               ...
                               NULL)) != -1)
    {
        switch (optc)
        {
            case 'c': /* -c, --check        check if the daemon already is
                         running */
                nslcd_checkonly = 1;
                break;
            case 'd': /* -d, --debug        don't fork and print debugging to
                         stderr */
                nslcd_debugging++;
...
static void mkdirname(const char* filename)
{
    ...
    if (mkdir(path, (mode_t)0755) == 0)
    {
        ...
        if (lchown(path, nslcd_cfg->uid, nslcd_cfg->gid) < 0)
            log_log(LOG_WARNING, "problem setting permissions for %s: %s", path,
                    strerror(errno));
    }
    free(tmpname);
}
...
static int create_socket(const char* filename)
{
    ...
    {
        log_log(LOG_DEBUG, "unlink() of %s failed (ignored): %s", filename,
                strerror(errno));
    }
    ...
    {
        log_log(LOG_ERR, "fctnl(F_SETFL,FD_CLOEXEC) on %s failed: %s", filename,
                strerror(errno));
        if (close(sock))
            log_log(LOG_WARNING, "problem closing socket: %s", strerror(errno));
        exit(EXIT_FAILURE);
    ...
    {
        log_log(LOG_ERR, "chmod(0666) of %s failed: %s", filename,
                strerror(errno));
        if (close(sock))
            log_log(LOG_WARNING, "problem closing socket: %s", strerror(errno));
        exit(EXIT_FAILURE);
...
static int read_header(TFILE* fp, int32_t* action)
{
    ...
    {
        log_log(LOG_DEBUG, "invalid nslcd version id: 0x%08x",
...
static void handleconnection(int sock, MYLDAP_SESSION* session)
{
    ...
    if (getpeercred(sock, &uid, &gid, &pid))
        log_log(LOG_DEBUG, "connection from unknown client: %s",
                strerror(errno));
    else
        log_log(LOG_DEBUG, "connection from pid=%lu uid=%lu gid=%lu",
                (unsigned long int)pid, (unsigned long int)uid,
                (unsigned long int)gid);
    /* create a stream object */
    if ((fp = tio_fdopen(sock, READ_TIMEOUT, WRITE_TIMEOUT, READBUFFER_MINSIZE,
                         READBUFFER_MAXSIZE, WRITEBUFFER_MINSIZE,
                         WRITEBUFFER_MAXSIZE)) == NULL)
    {
    ...
    switch (action)
    {
        case NSLCD_ACTION_CONFIG_GET:
            (void)nslcd_config_get(fp, session);
            break;
        case NSLCD_ACTION_ALIAS_BYNAME:
            (void)nslcd_alias_byname(fp, session);
            break;
        case NSLCD_ACTION_ALIAS_ALL:
            (void)nslcd_alias_all(fp, session);
            break;
        case NSLCD_ACTION_ETHER_BYNAME:
            (void)nslcd_ether_byname(fp, session);
            break;
        case NSLCD_ACTION_ETHER_BYETHER:
            (void)nslcd_ether_byether(fp, session);
            break;
        case NSLCD_ACTION_ETHER_ALL:
            (void)nslcd_ether_all(fp, session);
            break;
        case NSLCD_ACTION_GROUP_BYNAME:
            (void)nslcd_group_byname(fp, session);
            break;
        case NSLCD_ACTION_GROUP_BYGID:
            (void)nslcd_group_bygid(fp, session);
            break;
        case NSLCD_ACTION_GROUP_BYMEMBER:
            (void)nslcd_group_bymember(fp, session);
            break;
        case NSLCD_ACTION_GROUP_ALL:
            if (!nslcd_cfg->nss_disable_enumeration)
                (void)nslcd_group_all(fp, session);
            break;
        case NSLCD_ACTION_HOST_BYNAME:
            (void)nslcd_host_byname(fp, session);
            break;
        case NSLCD_ACTION_HOST_BYADDR:
            (void)nslcd_host_byaddr(fp, session);
            break;
        case NSLCD_ACTION_HOST_ALL:
            (void)nslcd_host_all(fp, session);
            break;
        case NSLCD_ACTION_NETGROUP_BYNAME:
            (void)nslcd_netgroup_byname(fp, session);
            break;
        case NSLCD_ACTION_NETGROUP_ALL:
            (void)nslcd_netgroup_all(fp, session);
            break;
        case NSLCD_ACTION_NETWORK_BYNAME:
            (void)nslcd_network_byname(fp, session);
            break;
        case NSLCD_ACTION_NETWORK_BYADDR:
            (void)nslcd_network_byaddr(fp, session);
            break;
        case NSLCD_ACTION_NETWORK_ALL:
            (void)nslcd_network_all(fp, session);
            break;
        case NSLCD_ACTION_PASSWD_BYNAME:
            (void)nslcd_passwd_byname(fp, session, uid);
            break;
        case NSLCD_ACTION_PASSWD_BYUID:
            (void)nslcd_passwd_byuid(fp, session, uid);
            break;
        ...
        case NSLCD_ACTION_PROTOCOL_BYNAME:
            (void)nslcd_protocol_byname(fp, session);
            break;
        case NSLCD_ACTION_PROTOCOL_BYNUMBER:
            (void)nslcd_protocol_bynumber(fp, session);
            break;
        case NSLCD_ACTION_PROTOCOL_ALL:
            (void)nslcd_protocol_all(fp, session);
            break;
        case NSLCD_ACTION_RPC_BYNAME:
            (void)nslcd_rpc_byname(fp, session);
            break;
        case NSLCD_ACTION_RPC_BYNUMBER:
            (void)nslcd_rpc_bynumber(fp, session);
            break;
        case NSLCD_ACTION_RPC_ALL:
            (void)nslcd_rpc_all(fp, session);
            break;
        case NSLCD_ACTION_SERVICE_BYNAME:
            (void)nslcd_service_byname(fp, session);
            break;
        case NSLCD_ACTION_SERVICE_BYNUMBER:
            (void)nslcd_service_bynumber(fp, session);
            break;
        case NSLCD_ACTION_SERVICE_ALL:
            (void)nslcd_service_all(fp, session);
            break;
        case NSLCD_ACTION_SHADOW_BYNAME:
            (void)nslcd_shadow_byname(fp, session, uid);
            break;
        case NSLCD_ACTION_SHADOW_ALL:
            if (!nslcd_cfg->nss_disable_enumeration)
                (void)nslcd_shadow_all(fp, session, uid);
            break;
        case NSLCD_ACTION_PAM_AUTHC:
            (void)nslcd_pam_authc(fp, session, uid);
            break;
        case NSLCD_ACTION_PAM_AUTHZ:
            (void)nslcd_pam_authz(fp, session);
            break;
        case NSLCD_ACTION_PAM_SESS_O:
            (void)nslcd_pam_sess_o(fp, session);
            break;
        case NSLCD_ACTION_PAM_SESS_C:
            (void)nslcd_pam_sess_c(fp, session);
            break;
        case NSLCD_ACTION_PAM_PWMOD:
            (void)nslcd_pam_pwmod(fp, session, uid);
            break;
        case NSLCD_ACTION_USERMOD:
            (void)nslcd_usermod(fp, session, uid);
            break;
        default:
            log_log(LOG_WARNING, "invalid request id: 0x%08x",
                    (unsigned int)action);
            break;
    }
...
static void create_pidfile(const char* filename)
{
    ...
    if (filename != NULL)
    {
        ...
        {
            log_log(LOG_ERR, "cannot create pid file (%s): %s", filename,
                    strerror(errno));
            exit(EXIT_FAILURE);
        }
        if (lockf(fd, F_TLOCK, 0) < 0)
        {
            log_log(LOG_ERR, "cannot lock pid file (%s): %s", filename,
                    strerror(errno));
            exit(EXIT_FAILURE);
        }
        if (ftruncate(fd, 0) < 0)
        {
            log_log(LOG_ERR, "cannot truncate pid file (%s): %s", filename,
                    strerror(errno));
            exit(EXIT_FAILURE);
        }
        ...
        {
            log_log(LOG_ERR, "error writing pid file (%s): %s", filename,
                    strerror(errno));
            exit(EXIT_FAILURE);
        }
...
static void* worker(void UNUSED(*arg))
{
    ...
    while (1)
    {
        ...
        {
            if (errno == EINTR)
                log_log(LOG_DEBUG, "select() failed (ignored): %s",
                        strerror(errno));
            else
        ...
        if (csock < 0)
        {
            if ((errno == EINTR) || (errno == EAGAIN) || (errno == EWOULDBLOCK))
                log_log(LOG_DEBUG, "accept() failed (ignored): %s",
                        strerror(errno));
            else
        ...
        if ((j = fcntl(csock, F_GETFL, 0)) < 0)
        {
            log_log(LOG_ERR, "fctnl(F_GETFL) failed: %s", strerror(errno));
            if (close(csock))
                log_log(LOG_WARNING, "problem closing socket: %s",
                        strerror(errno));
            continue;
        }
...
static void disable_nss_ldap(void)
{
    ...
    if (handle == NULL)
    {
        log_log(LOG_WARNING, "Warning: NSS_LDAP module not loaded: %s",
                dlerror());
        return;
    }
...
static void adjust_oom_score(void)
{
    ...
    if ((oom_adj_fd = open(OOM_SCORE_ADJ_FILE, O_WRONLY)) >= 0)
    {
        if (write(oom_adj_fd, OOM_SCORE_ADJ, strlen(OOM_SCORE_ADJ)) < 0)
            log_log(LOG_WARNING,
                    "writing oom score adjustment of %s failed: %s",
...
int main(int argc, char* argv[])
{
    ...
    {
        log_log(LOG_ERR,
                "nslcd may already be active, cannot acquire lock (%s): %s",
    ...
    if (nslcd_cfg->gid != NOGID)
    {
        ...
        log_log(LOG_DEBUG, "setgid(%lu) done",
                (unsigned long int)nslcd_cfg->gid);
    }
    /* change to nslcd uid */
    if (nslcd_cfg->uid != NOUID)
    {
        ...
        log_log(LOG_DEBUG, "setuid(%lu) done",
                (unsigned long int)nslcd_cfg->uid);
    }
    ...
    for (i = 0; i < nslcd_cfg->threads; i++)
    {
        ...
        {
            log_log(LOG_ERR, "unable to start worker thread %d: %s", i,
                    strerror(errno));
    ...
    {
        sleep(
            INT_MAX); /* sleep as long as we can or until we receive a signal */
    ...
    for (i = 0; i < nslcd_cfg->threads; i++)
        if (pthread_cancel(nslcd_threads[i]))
            log_log(LOG_WARNING, "failed to stop thread %d (ignored): %s", i,
                    strerror(errno));
    ...
    for (i = 0; i < nslcd_cfg->threads; i++)
    {
        ...
        if (pthread_kill(nslcd_threads[i], 0) == 0)
            log_log(LOG_ERR, "thread %d is still running, shutting down anyway",
                    i);
    }
    /* we're done */
    return EXIT_SUCCESS;
