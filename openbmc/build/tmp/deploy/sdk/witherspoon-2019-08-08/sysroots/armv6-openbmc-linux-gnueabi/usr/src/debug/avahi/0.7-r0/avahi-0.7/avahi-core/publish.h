 ...
 * AvahiSEntryGroup object changes */
typedef void (*AvahiSEntryGroupCallback)(AvahiServer* s, AvahiSEntryGroup* g,
...
/** Free an entry group. All RRs assigned to the group are removed from the
 * server */
void avahi_s_entry_group_free(AvahiSEntryGroup* g);

/** Commit an entry group. This starts the probing and registration process for
 * all RRs in the group */
int avahi_s_entry_group_commit(AvahiSEntryGroup* g);

/** Remove all entries from the entry group and reset the state to
...
/** Add a new resource record to the server. Returns 0 on success, negative
 * otherwise. */
...
int avahi_server_add_service(
    ...
    const char* domain,
    const char* host, /**< Host name where this servcie resides, or NULL if on
                         the local host */
    uint16_t port,    /**< Port number of the service */
    ... /**< Text records, terminated by NULL */) AVAHI_GCC_SENTINEL;

/** Mostly identical to avahi_server_add_service(), but takes an AvahiStringList
 * object for the TXT records.  The AvahiStringList object is copied. */
...
int avahi_server_add_service_subtype(
    ...
    AvahiProtocol protocol, AvahiPublishFlags flags,
    const char*
        name, /**< Specify the name of main service you already added here */
    const char* type,   /**< Specify the main type of the service you already
                           added here */
    const char* domain, /**< Specify the main type of the service you already
                           added here */
    const char* subtype /**< The new subtype for the specified service */);

/** Update the TXT record for a service with the data from the specified string
 * list */
...
/** Check if there is a service locally defined and return the entry group it is
 * attached to. Returns NULL if the service isn't local*/
