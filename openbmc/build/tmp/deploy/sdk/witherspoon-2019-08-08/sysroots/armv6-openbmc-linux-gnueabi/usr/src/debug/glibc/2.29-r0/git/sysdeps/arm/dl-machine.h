...
static inline Elf32_Addr __attribute__((unused)) elf_machine_dynamic(void)
{
...
static inline Elf32_Addr __attribute__((unused)) elf_machine_load_address(void)
{
...
#else
    extern Elf32_Addr __dl_relocate_static_pie(void*) asm(
        "_dl_relocate_static_pie") attribute_hidden;
    ...
    /* Clear the low bit of the function address.
       ...
       it's Thumb function address.  PCREL_ADDR comes from PC-relative
       calculation which will finish during assembling.  GAS assembler before
       the fix for PR gas/21458 was not setting the lsb but does after that.
       Always do the strip for both, so the code works with various combinations
       of glibc and Binutils.  */
    ...
    if (l->l_info[DT_JMPREL] && lazy)
    {
       ...
       don't store the address in the GOT so that all future calls also
        ...
        if (profile)
        {
            ...
            /* This function will get called to fix up the GOT entry indicated
               by the offset on the stack, and then jump to the resolved
               address.  */
...
static inline Elf32_Addr elf_machine_plt_value(struct link_map* map,
                                               const Elf32_Rel* reloc,
                                               Elf32_Addr value)
{
    return value;
...
auto void relocate_pc24(struct link_map* map, Elf32_Addr value,
...
%s: Symbol `%s' has different size in shared object, consider re-linking\n",
            ...
            case R_ARM_TLS_DTPOFF32:
                *reloc_addr =
                    (sym == NULL ? 0 : sym->st_value) + reloc->r_addend;
                break;
...
auto inline void __attribute__((always_inline))
elf_machine_lazy_rel(struct link_map* map, Elf32_Addr l_addr,
                     const Elf32_Rel* reloc, int skip_ifunc)
{
