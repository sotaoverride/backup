...
static void g_unix_input_stream_pollable_iface_init(
    GPollableInputStreamInterface* iface);
static void g_unix_input_stream_file_descriptor_based_iface_init(
    GFileDescriptorBasedIface* iface);

G_DEFINE_TYPE_WITH_CODE(
    GUnixInputStream, g_unix_input_stream, G_TYPE_INPUT_STREAM,
...
static void g_unix_input_stream_set_property(GObject* object, guint prop_id,
                                             const GValue* value,
                                             GParamSpec* pspec);
static void g_unix_input_stream_get_property(GObject* object, guint prop_id,
                                             GValue* value, GParamSpec* pspec);
static gssize g_unix_input_stream_read(GInputStream* stream, void* buffer,
...
static void g_unix_input_stream_class_init(GUnixInputStreamClass* klass)
{
    ...
    g_object_class_install_property(
        ...
        g_param_spec_int("fd", P_("File descriptor"),
                         P_("The file descriptor to read from"), G_MININT,
                         G_MAXINT, -1,
...
GInputStream* g_unix_input_stream_new(gint fd, gboolean close_fd)
{
    ...
    stream = g_object_new(G_TYPE_UNIX_INPUT_STREAM, "fd", fd, "close-fd",
                          close_fd, NULL);
