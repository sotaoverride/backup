...
static /*@observer@*/ /*@null*/ const struct subordinate_range*
    subordinate_next(struct commonio_db* db)
{
 ...
 */
static const struct subordinate_range*
    find_range(struct commonio_db* db, const char* owner, unsigned long val)
{
    const struct subordinate_range* range;
     ...
     */
    if ((0 != strcmp(db->filename, "/etc/subuid")) &&
        (0 != strcmp(db->filename, "/etc/subgid")))
        return NULL;
 ...
 */
static bool have_range(struct commonio_db* db, const char* owner,
 ...
 */
static unsigned long find_free_range(struct commonio_db* db, unsigned long min,
 ...
 */
static int add_range(struct commonio_db* db, const char* owner,
                     unsigned long start, unsigned long count)
{
    struct subordinate_range range;
    range.owner = owner;
 ...
 */
static int remove_range(struct commonio_db* db, const char* owner,
                        unsigned long start, unsigned long count)
{
