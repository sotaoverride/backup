 ...
 *
 * The \c basic_string_literal is a thin wrapper around a constant string
 * literal. It provides interface similar to STL strings, but because of
 * read-only nature of string literals, lacks ability to modify string contents.
 * However, \c basic_string_literal objects can be assigned to and cleared.
 *
 * The main advantage of this class comparing to other string classes is that
 * it doesn't dynamically allocate memory and therefore is fast, thin and
 * exception safe.
 */
...
class basic_string_literal
{
  ...
  public:
     ...
     */
    BOOST_CONSTEXPR basic_string_literal() BOOST_NOEXCEPT
    ...
    BOOST_CONSTEXPR basic_string_literal(
        T (&p)[LenV]
        //! \cond
        ,
        typename boost::enable_if_c<is_same<T, const value_type>::value,
                                    boost::log::aux::sfinae_dummy>::type =
            boost::log::aux::sfinae_dummy()
        //! \endcond
        ) BOOST_NOEXCEPT : m_pStart(p),
                           m_Len(LenV - 1)
    {
    }
    ...
    BOOST_CXX14_CONSTEXPR this_type&
        operator=(this_type const& that) BOOST_NOEXCEPT
    {
        return assign(that);
    }
    /*!
     ...
     * \return \c true if the comparand string equals to this string, \c false
     * otherwise
     */
    bool operator==(this_type const& that) const BOOST_NOEXCEPT
    {
        return (compare_internal(m_pStart, m_Len, that.m_pStart, that.m_Len) ==
                0);
    }
    /*!
     * Lexicographical comparison (equality)
     *
     * \param str Comparand. Must point to a zero-terminated sequence of
     * characters, must not be NULL. \return \c true if the comparand string
     * equals to this string, \c false otherwise
     */
    bool operator==(const_pointer str) const BOOST_NOEXCEPT
    {
        return (compare_internal(m_pStart, m_Len, str,
                                 traits_type::length(str)) == 0);
    }
    /*!
     * Lexicographical comparison (equality)
     *
    ...
    {
        return (compare_internal(m_pStart, m_Len, that.c_str(), that.size()) ==
                0);
    }
     ...
     *
     * \param that Comparand
     * \return \c true if the comparand string is not equal to this string, \c
     * false otherwise
     */
    bool operator!=(this_type const& that) const BOOST_NOEXCEPT
    {
     ...
     *
     * \param str Comparand. Must point to a zero-terminated sequence of
     * characters, must not be NULL. \return \c true if the comparand string is
     * not equal to this string, \c false otherwise
     */
    bool operator!=(const_pointer str) const BOOST_NOEXCEPT
    {
        return (compare_internal(m_pStart, m_Len, str,
     ...
     *
     * \param that Comparand
     * \return \c true if this string is less than the comparand, \c false
     * otherwise
     */
    bool operator<(this_type const& that) const BOOST_NOEXCEPT
    {
     ...
     *
     * \param str Comparand. Must point to a zero-terminated sequence of
     * characters, must not be NULL. \return \c true if this string is less than
     * the comparand, \c false otherwise
     */
    bool operator<(const_pointer str) const BOOST_NOEXCEPT
    {
        return (compare_internal(m_pStart, m_Len, str,
     ...
     *
     * \param that Comparand
     * \return \c true if this string is less or equal to the comparand, \c
     * false otherwise
     */
    bool operator<=(this_type const& that) const BOOST_NOEXCEPT
    {
     ...
     *
     * \param str Comparand. Must point to a zero-terminated sequence of
     * characters, must not be NULL. \return \c true if this string is less or
     * equal to the comparand, \c false otherwise
     */
    bool operator<=(const_pointer str) const BOOST_NOEXCEPT
    {
        return (compare_internal(m_pStart, m_Len, str,
     ...
     *
     * \param that Comparand
     * \return \c true if this string is greater than the comparand, \c false
     * otherwise
     */
    bool operator>(this_type const& that) const BOOST_NOEXCEPT
    {
     ...
     *
     * \param str Comparand. Must point to a zero-terminated sequence of
     * characters, must not be NULL. \return \c true if this string is greater
     * than the comparand, \c false otherwise
     */
    bool operator>(const_pointer str) const BOOST_NOEXCEPT
    {
        return (compare_internal(m_pStart, m_Len, str,
     ...
     *
     * \param that Comparand
     * \return \c true if this string is greater or equal to the comparand, \c
     * false otherwise
     */
    bool operator>=(this_type const& that) const BOOST_NOEXCEPT
    {
     ...
     * \param str Comparand. Must point to a zero-terminated sequence of
     * characters, must not be NULL. \return \c true if this string is greater
     * or equal to the comparand, \c false otherwise
     */
    bool operator>=(const_pointer str) const BOOST_NOEXCEPT
    {
        return (compare_internal(m_pStart, m_Len, str,
     ...
     * \return Constant reference to the requested character
     *
     * \b Throws: An <tt>std::exception</tt>-based exception if index \a i is
    ...
    const_reference at(size_type i) const
    {
        if (BOOST_UNLIKELY(i >= m_Len))
            BOOST_THROW_EXCEPTION(std::out_of_range(
                "basic_string_literal::at: the index value is out of range"));
        return m_pStart[i];
    }
     ...
     */
    BOOST_CONSTEXPR const_pointer c_str() const BOOST_NOEXCEPT
    {
        return m_pStart;
    }
    /*!
    ...
    /*!
     * \return Reverse iterator that points before the first character of the
     * literal
     */
     ...
     */
    BOOST_CXX14_CONSTEXPR this_type&
        assign(this_type const& that) BOOST_NOEXCEPT
    {
        m_pStart = that.m_pStart;
        m_Len = that.m_Len;
        return *this;
     ...
     *            The buffer must have enough capacity to accommodate the
     * requested number of characters. \param n Maximum number of characters to
     * copy \param pos Starting position to start copying from \return Number of
     * characters copied
     *
     * \b Throws: An <tt>std::exception</tt>-based exception if \a pos is out of
     * range.
     */
    size_type copy(value_type* str, size_type n, size_type pos = 0) const
    {
        if (BOOST_UNLIKELY(pos > m_Len))
            BOOST_THROW_EXCEPTION(std::out_of_range(
                "basic_string_literal::copy: the position is out of range"));
     ...
     * \param str Comparand. Must point to a sequence of characters, must not be
     * NULL. \param len Number of characters in the sequence \a str. \return
     * Zero if the comparand equals this string, a negative value if this string
     * is less than the comparand, a positive value if this string is greater
     * than the comparand.
     *
     * \b Throws: An <tt>std::exception</tt>-based exception if \a pos is out of
     * range.
     */
    int compare(size_type pos, size_type n, const_pointer str,
                size_type len) const
    {
        if (BOOST_UNLIKELY(pos > m_Len))
            BOOST_THROW_EXCEPTION(std::out_of_range(
                "basic_string_literal::compare: the position is out of range"));
        ...
        if (compare_size > n)
            compare_size = n;
        return compare_internal(m_pStart + pos, compare_size, str,
                                compare_size);
    }
    /*!
     * Lexicographically compares the argument string to a part of this string
     ...
     * \param n Length of the substring of this string to perform comparison to
     * \param str Comparand. Must point to a zero-terminated sequence of
     * characters, must not be NULL. \return Zero if the comparand equals this
     * string, a negative value if this string is less than the comparand, a
     * positive value if this string is greater than the comparand.
     *
     * \b Throws: An <tt>std::exception</tt>-based exception if \a pos is out of
     * range.
     */
    int compare(size_type pos, size_type n,
                const_pointer str) const BOOST_NOEXCEPT
    {
        return compare(pos, n, str, traits_type::length(str));
    }
    /*!
     * Lexicographically compares the argument string literal to a part of this
     * string
     *
     * \pre <tt>pos <= size()</tt>
     * \param pos Starting position within this string to perform comparison to
     * \param n Length of the substring of this string to perform comparison to
     * \param that Comparand
     * \return Zero if the comparand equals this string, a negative value if
     * this string is less than the comparand, a positive value if this string
     * is greater than the comparand.
     *
     * \b Throws: An <tt>std::exception</tt>-based exception if \a pos is out of
     * range.
     */
    int compare(size_type pos, size_type n,
                this_type const& that) const BOOST_NOEXCEPT
    {
  ...
  private:
    ...
    static int compare_internal(const_pointer pLeft, size_type LeftLen,
                                const_pointer pRight,
                                size_type RightLen) BOOST_NOEXCEPT
    {
        if (pLeft != pRight)
        {
            const int result = traits_type::compare(
...
template <typename CharT, typename TraitsT>
inline void insert_fill_chars(std::basic_ostream<CharT, TraitsT>& strm,
                              std::size_t n)
{
    enum
    {
...
template <typename CharT, typename TraitsT>
void insert_aligned(std::basic_ostream<CharT, TraitsT>& strm, const CharT* p,
                    std::size_t size)
{
    const std::size_t alignment_size =
        static_cast<std::size_t>(strm.width()) - size;
    const bool align_left =
        (strm.flags() & std::basic_ostream<CharT, TraitsT>::adjustfield) ==
        std::basic_ostream<CharT, TraitsT>::left;
    if (align_left)
    {
        strm.write(p, size);
        if (strm.good())
...
inline std::basic_ostream<CharT, StrmTraitsT>&
...
inline BOOST_CXX14_CONSTEXPR void
    swap(basic_string_literal<CharT, TraitsT>& left,
         basic_string_literal<CharT, TraitsT>& right) BOOST_NOEXCEPT
{
    left.swap(right);
}
...
#ifndef BOOST_LOG_DOXYGEN_PASS
    typename boost::enable_if_c<is_same<T, const char>::value,
                                string_literal>::type
#else
...
template <typename T, std::size_t LenV>
inline BOOST_CONSTEXPR
    typename boost::enable_if_c<is_same<T, const wchar_t>::value,
                                wstring_literal>::type
        str_literal(T (&p)[LenV]) BOOST_NOEXCEPT
{
