...
_public_ char* sd_id128_to_string(sd_id128_t id,
...
_public_ int sd_id128_get_boot(sd_id128_t* ret)
{
    ...
    {
        r = id128_read("/proc/sys/kernel/random/boot_id", ID128_UUID,
                       &saved_boot_id);
        if (r < 0)
            return r;
    }
...
#define MAX_PERMS                                                              \
    ((unsigned long)(KEY_POS_VIEW | KEY_POS_READ | KEY_POS_SEARCH |            \
...
_public_ int sd_id128_get_invocation(sd_id128_t* ret)
{
    ...
    {
        /* We first check the environment. The environment variable is primarily
         * relevant for user services, and sufficiently safe as long as no
         * privilege boundary is involved. */
        r = get_invocation_from_environment(&saved_invocation_id);
        if (r < 0 && r != -ENXIO)
            return r;

        /* The kernel keyring is relevant for system services (as for user
         * services we don't store
         * the invocation ID in the keyring, as there'd be no trust benefit in
         * that). */
...
_public_ int sd_id128_randomize(sd_id128_t* ret)
{
    ...
    /* We allow usage if x86-64 RDRAND here. It might not be trusted enough for
     * keeping secrets, but it should be fine for UUIDS. */
...
_public_ int sd_id128_get_machine_app_specific(sd_id128_t app_id,
                                               sd_id128_t* ret)
{
    sd_id128_t id;
    int r;
