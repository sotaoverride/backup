...
static void g_tls_password_init(GTlsPassword* password)
{
    password->priv = g_tls_password_get_instance_private(password);
}

static const guchar* g_tls_password_real_get_value(GTlsPassword* password,
                                                   gsize* length)
{
    if (length)
...
static void g_tls_password_real_set_value(GTlsPassword* password, guchar* value,
    ...
    g_tls_password_real_get_default_warning(GTlsPassword* password)
{
    ...
    if (flags & G_TLS_PASSWORD_FINAL_TRY)
        return _("This is the last chance to enter the password correctly "
                 "before your access is locked out.");
    if (flags & G_TLS_PASSWORD_MANY_TRIES)
        /* Translators: This is not the 'This is the last chance' string. It is
         * displayed when more than one attempt is allowed. */
        return _("Several passwords entered have been incorrect, and your "
                 "access will be locked out after further failures.");
...
static void g_tls_password_get_property(GObject* object, guint prop_id,
...
static void g_tls_password_set_property(GObject* object, guint prop_id,
...
static void g_tls_password_finalize(GObject* object)
{
    GTlsPassword* password = G_TLS_PASSWORD(object);
...
static void g_tls_password_class_init(GTlsPasswordClass* klass)
{
    ...
    g_object_class_install_property(
        ...
        g_param_spec_flags("flags", P_("Flags"), P_("Flags about the password"),
                           G_TYPE_TLS_PASSWORD_FLAGS, G_TLS_PASSWORD_NONE,
                           G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS));

    g_object_class_install_property(
        gobject_class, PROP_DESCRIPTION,
        g_param_spec_string("description", P_("Description"),
                            P_("Description of what the password is for"), NULL,
                            G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS));

    g_object_class_install_property(
        gobject_class, PROP_WARNING,
        g_param_spec_string("warning", P_("Warning"),
                            P_("Warning about the password"), NULL,
                            G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS));
}
...
{
    return g_object_new(G_TYPE_TLS_PASSWORD, "flags", flags, "description",
                        description, NULL);
}
 ...
 * Since: 2.30
 */
void g_tls_password_set_value(GTlsPassword* password, const guchar* value,
                              gssize length)
{
    ...
    g_tls_password_set_value_full(password, g_memdup(value, length), length,
                                  g_free);
}
 ...
 * Since: 2.30
 */
void g_tls_password_set_value_full(GTlsPassword* password, guchar* value,
                                   gssize length, GDestroyNotify destroy)
{
    g_return_if_fail(G_IS_TLS_PASSWORD(password));
    G_TLS_PASSWORD_GET_CLASS(password)->set_value(password, value, length,
                                                  destroy);
}
...
const gchar* g_tls_password_get_warning(GTlsPassword* password)
{
    ...
    if (password->priv->warning == NULL)
        return G_TLS_PASSWORD_GET_CLASS(password)->get_default_warning(
            password);
