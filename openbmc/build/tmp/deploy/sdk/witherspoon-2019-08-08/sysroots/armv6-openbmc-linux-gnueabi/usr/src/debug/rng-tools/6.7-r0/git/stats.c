...
static void scale_mult_unit(char* unit, int unitsize, const char* baseunit,
                            double* value_min, double* value_avg,
                            double* value_max)
{
    int mult = 0;
...
uint64_t elapsed_time(struct timeval* start, struct timeval* stop)
{
    uint64_t diff;
...
char* dump_stat_counter(char* buf, int size, const char* msg, uint64_t value)
{
    buf[size - 1] = 0;
...
char* dump_stat_stat(char* buf, int size, const char* msg, const char* unit,
...
char* dump_stat_bw(char* buf, int size, const char* msg, const char* unit,
                   struct rng_stat* stat, uint64_t blocksize)
{
    char unitscaled[20];
    ...
    scale_mult_unit(unitscaled, sizeof(unitscaled), unit, &bw_min, &bw_avg,
                    &bw_max);
