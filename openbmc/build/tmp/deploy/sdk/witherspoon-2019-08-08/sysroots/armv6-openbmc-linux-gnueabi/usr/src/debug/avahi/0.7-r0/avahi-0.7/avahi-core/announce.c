...
static void remove_announcer(AvahiServer* s, AvahiAnnouncer* a)
{
    ...
    AVAHI_LLIST_REMOVE(AvahiAnnouncer, by_interface, a->interface->announcers,
                       a);
...
static void set_timeout(AvahiAnnouncer* a, const struct timeval* tv)
{
    ...
    else
    {
        ...
        if (a->time_event)
            ...
            a->time_event = avahi_time_event_new(a->server->time_event_queue,
                                                 tv, elapse_announce, a);
    }
}
...
static void next_state(AvahiAnnouncer* a)
{
    ...
    else if (a->state == AVAHI_PROBING)
    {
        ...
        if (a->n_iteration >= 4)
        {
            ...
            if (a->entry->group &&
                a->entry->group->state == AVAHI_ENTRY_GROUP_REGISTERING)
    ...
    else if (a->state == AVAHI_ANNOUNCING)
    {
        ...
        if (a->entry->flags & AVAHI_PUBLISH_UNIQUE)
            ...
            avahi_server_prepare_matching_responses(a->server, a->interface,
                                                    a->entry->record->key, 0);
        else
            avahi_server_prepare_response(a->server, a->interface, a->entry, 0,
                                          0);

        avahi_server_generate_response(a->server, a->interface, NULL, NULL, 0,
                                       0, 0);
        ...
        {
            struct timeval tv;
            avahi_elapse_time(&tv, a->sec_delay * 1000,
                              AVAHI_ANNOUNCEMENT_JITTER_MSEC);
...
static AvahiAnnouncer* get_announcer(AvahiServer* s, AvahiEntry* e,
                                     AvahiInterface* i)
{
    AvahiAnnouncer* a;
...
static void go_to_initial_state(AvahiAnnouncer* a)
{
    ...
    if ((e->flags & AVAHI_PUBLISH_UNIQUE) &&
        !(e->flags & AVAHI_PUBLISH_NO_PROBE))
...
static void new_announcer(AvahiServer* s, AvahiInterface* i, AvahiEntry* e)
{
    ...
    if (!avahi_interface_match(i, e->interface, e->protocol) ||
        !i->announcing || !avahi_entry_is_commited(e))
        return;
...
static void announce_walk_callback(AvahiInterfaceMonitor* m, AvahiInterface* i,
                                   void* userdata)
{
...
void avahi_announce_entry(AvahiServer* s, AvahiEntry* e)
{
    ...
    avahi_interface_monitor_walk(s->monitor, e->interface, e->protocol,
                                 announce_walk_callback, e);
}
...
int avahi_entry_is_probing(AvahiServer* s, AvahiEntry* e, AvahiInterface* i)
{
    ...
    return a->state == AVAHI_PROBING ||
           (a->state == AVAHI_WAITING && (e->flags & AVAHI_PUBLISH_UNIQUE));
}

void avahi_entry_return_to_initial_state(AvahiServer* s, AvahiEntry* e,
                                         AvahiInterface* i)
{
    AvahiAnnouncer* a;
...
static int is_duplicate_entry(AvahiServer* s, AvahiEntry* e)
{
    ...
    for (i = avahi_hashmap_lookup(s->entries_by_key, e->record->key); i;
         i = i->by_key_next)
    {
...
static void send_goodbye_callback(AvahiInterfaceMonitor* m, AvahiInterface* i,
                                  void* userdata)
{
    AvahiEntry* e = userdata;
    AvahiRecord* g;
    ...
    avahi_interface_post_response(i, g, e->flags & AVAHI_PUBLISH_UNIQUE, NULL,
                                  1);
...
static void reannounce(AvahiAnnouncer* a)
{
    ...
    /* If the group this entry belongs to is not even commited, there's nothing
     * to reannounce */
    if (e->group && (e->group->state == AVAHI_ENTRY_GROUP_UNCOMMITED ||
                     e->group->state == AVAHI_ENTRY_GROUP_COLLISION))
        return;
    ...
    if (a->state == AVAHI_PROBING ||
        ...
        /* We were probing or waiting after probe, so we restart probing from
         * the beginning here */
...
static void reannounce_walk_callback(AvahiInterfaceMonitor* m,
                                     AvahiInterface* i, void* userdata)
{
    AvahiEntry* e = userdata;
    AvahiAnnouncer* a;
...
void avahi_reannounce_entry(AvahiServer* s, AvahiEntry* e)
{
    ...
    avahi_interface_monitor_walk(s->monitor, e->interface, e->protocol,
                                 reannounce_walk_callback, e);
}

void avahi_goodbye_interface(AvahiServer* s, AvahiInterface* i,
                             int send_goodbye, int remove)
{
    assert(s);
    assert(i);
                         ...
                         int remove)
{
    ...
    if (send_goodbye)
        if (!e->dead)
            avahi_interface_monitor_walk(s->monitor, AVAHI_IF_UNSPEC,
                                         AVAHI_PROTO_UNSPEC,
                                         send_goodbye_callback, e);
