 ...
 * Since: 2.26
 **/
void g_permission_impl_update(GPermission* permission, gboolean allowed,
...
static void g_permission_get_property(GObject* object, guint prop_id,
...
static gboolean acquire_or_release(GPermission* permission,
                                   GCancellable* cancellable, GError** error)
{
...
static void acquire_or_release_async(GPermission* permission,
                                     GCancellable* cancellable,
                                     GAsyncReadyCallback callback,
                                     gpointer user_data)
{
    g_task_report_new_error(permission, callback, user_data, NULL, G_IO_ERROR,
                            G_IO_ERROR_NOT_SUPPORTED,
                            "Can't acquire or release permission");
}

static gboolean acquire_or_release_finish(GPermission* permission,
                                          GAsyncResult* result, GError** error)
{
...
static void g_permission_class_init(GPermissionClass* class)
{
    ...
    g_object_class_install_property(
        object_class, PROP_ALLOWED,
        g_param_spec_boolean(
            "allowed", P_("Is allowed"),
            P_("If the caller is allowed to perform the action"), FALSE,
     ...
     */
    g_object_class_install_property(
        object_class, PROP_CAN_ACQUIRE,
        g_param_spec_boolean(
            "can-acquire", P_("Can acquire"),
            P_("If calling g_permission_acquire() makes sense"), FALSE,
            G_PARAM_STATIC_STRINGS | G_PARAM_READABLE));
     ...
     */
    g_object_class_install_property(
        object_class, PROP_CAN_RELEASE,
        g_param_spec_boolean(
            "can-release", P_("Can release"),
            P_("If calling g_permission_release() makes sense"), FALSE,
            G_PARAM_STATIC_STRINGS | G_PARAM_READABLE));
}
