...
/* dbus-sysdeps.c Wrappers around system/libc features shared between UNIX and
 * Windows (internal to D-Bus implementation)
 *
...
/* NOTE: If you include any unix/windows-specific headers here, you are probably
 * doing something wrong and should be putting some code in dbus-sysdeps-unix.c
 * or dbus-sysdeps-win.c.
 *
                                        ...
                                        DBusError* error)
{
    ...
    while (i < len)
    {
        _dbus_string_set_byte(
            str, i,
            letters[_dbus_string_get_byte(str, i) % (sizeof(letters) - 1)]);
