...
OM_uint32 KRB5_CALLCONV krb5_gss_wrap_size_limit(
    minor_status, context_handle, conf_req_flag, qop_req, req_output_size,
    max_input_size) OM_uint32* minor_status;
...
OM_uint32* max_input_size;
{
    ...
    if (ctx->proto == 1)
    {
        ...
        if (conf_req_flag)
        {
            ...
            key =
                ctx->have_acceptor_subkey ? ctx->acceptor_subkey : ctx->subkey;
            enctype = key->keyblock.enctype;

            while (sz > 0 &&
                   krb5_encrypt_size(sz, enctype) + 16 > req_output_size)
                sz--;
        ...
        else
        {
            ...
            cksumtype = ctx->have_acceptor_subkey
                            ? ctx->acceptor_subkey_cksumtype
                            : ctx->cksumtype;

            err =
                krb5_c_checksum_length(ctx->k5_context, cksumtype, &cksumsize);
            if (err)
            {
                *minor_status = err;
    ...
    if (ohlen + overhead < req_output_size)
         ...
         */
        *max_input_size =
            (req_output_size - ohlen - overhead) & (~(OM_uint32)7);
    else
        *max_input_size = 0;
