...
static int maybe_use_dns(krb5_context context, const char* name, int defalt)
{
    ...
    code = profile_get_string(context->profile, KRB5_CONF_LIBDEFAULTS, name, 0,
                              0, &value);
    if (value == 0 && code == 0)
    {
...
static krb5_boolean use_dns_uri(krb5_context ctx)
{
    krb5_error_code ret;
    int use;
...
int _krb5_use_dns_kdc(krb5_context context)
{
    return maybe_use_dns(context, KRB5_CONF_DNS_LOOKUP_KDC, DEFAULT_LOOKUP_KDC);
}

int _krb5_use_dns_realm(krb5_context context)
{
...
void k5_free_serverlist(struct serverlist* list)
{
    size_t i;
...
static struct server_entry* new_server_entry(struct serverlist* list)
{
...
static int add_addr_to_list(struct serverlist* list, k5_transport transport,
                            int family, size_t addrlen, struct sockaddr* addr)
{
...
static int add_host_to_list(struct serverlist* list, const char* hostname,
                            int port, k5_transport transport, int family,
                            const char* uri_path, int master)
{
    struct server_entry* entry;
...
static void parse_uri_if_https(const char* host_or_uri, k5_transport* transport,
...
static krb5_boolean server_list_contains(struct serverlist* list,
...
static krb5_error_code locate_srv_conf_1(krb5_context context,
...
#ifdef TEST
static krb5_error_code krb5_locate_srv_conf(krb5_context context,
                                            const krb5_data* realm,
                                            const char* name,
...
#ifdef KRB5_DNS_LOOKUP
static krb5_error_code locate_srv_dns_1(krb5_context context,
                                        const krb5_data* realm,
                                        const char* service,
                                        const char* protocol,
                                        struct serverlist* serverlist)
{
    ...
    code =
        krb5int_make_srv_query_realm(context, realm, service, protocol, &head);
    if (code)
        return 0;
    ...
    for (entry = head; entry != NULL; entry = entry->next)
    {
        ...
        code = add_host_to_list(serverlist, entry->host, entry->port, transport,
                                AF_UNSPEC, NULL, -1);
        if (code)
            goto cleanup;
    }
...
static int module_callback(void* cbdata, int socktype, struct sockaddr* sa)
{
    ...
    if (add_addr_to_list(d->list, transport, sa->sa_family, addrlen, sa) != 0)
    {
...
static krb5_error_code module_locate_server(krb5_context ctx,
                                            const krb5_data* realm,
                                            struct serverlist* serverlist,
                                            ...
                                            k5_transport transport)
{
    ...
    int socktype, i;
    struct module_callback_data cbdata = {
        0,
    };
    const char* msg;
    ...
    code = krb5int_get_plugin_dir_data(&ctx->libkrb5_plugins, "service_locator",
                                       &ptrs, &ctx->err);
    if (code)
    {
...
static krb5_error_code prof_locate_server(krb5_context context,
                                          const krb5_data* realm,
                                          struct serverlist* serverlist,
                                          enum locate_service_type svc,
                                          k5_transport transport)
{
    ...
    switch (svc)
    {
        case locate_service_kdc:
            ...
            /* We used to use /etc/services for these, but enough systems have
             * old, crufty, wrong settings that this is probably better. */
        kdc_ports:
 ...
 */
static void parse_uri_fields(const char* uri, k5_transport* transport_out,
 ...
 */
static krb5_error_code locate_uri(krb5_context context, const krb5_data* realm,
                                  const char* req_service,
                                  struct serverlist* serverlist,
...
static krb5_error_code dns_locate_server_uri(krb5_context context,
                                             const krb5_data* realm,
                                             struct serverlist* serverlist,
                                             enum locate_service_type svc,
                                             k5_transport transport)
...
static krb5_error_code dns_locate_server_srv(krb5_context context,
                                             const krb5_data* realm,
                                             struct serverlist* serverlist,
                                             enum locate_service_type svc,
                                             k5_transport transport)
 ...
 */
static krb5_error_code locate_server(krb5_context context,
                                     const krb5_data* realm,
                                     struct serverlist* serverlist,
                                     enum locate_service_type svc,
                                     k5_transport transport)
...
krb5_error_code k5_locate_server(krb5_context context, const krb5_data* realm,
                                 struct serverlist* serverlist,
                                 enum locate_service_type svc,
                                 krb5_boolean no_udp)
{
    ...
    if (serverlist->nservers == 0)
    {
        ...
        k5_setmsg(context, KRB5_REALM_UNKNOWN,
                  _("Cannot find KDC for realm \"%.*s\""), realm->length,
                  realm->data);
