            ...
            const char* errstring)
{
    ...
    _dl_fatal_printf(
        "%s: %s: %s%s%s%s%s\n", RTLD_PROGNAME,
        occasion ?: N_("error while loading shared libraries"), objname,
        *objname ? ": " : "", errstring, errcode ? ": " : "",
        (errcode ? __strerror_r(errcode, buffer, sizeof buffer) : ""));
}
                          ...
                          const char* occasion)
{
    ...
    if (lcatch != NULL)
    {
        ...
        *lcatch->errcode = errcode;
        ...
        fatal_error(errcode, exception->objname, occasion,
                    exception->errstring);
}
libc_hidden_def(_dl_signal_exception)

    void _dl_signal_error(int errcode, const char* objname,
...
{
    if (__builtin_expect(
            GLRO(dl_debug_mask) & ~(DL_DEBUG_STATISTICS | DL_DEBUG_PRELINK), 0))
        _dl_debug_printf("%s: error: %s: %s (%s)\n", exception->objname,
                         occasion, exception->errstring,
                         receiver ? "continued" : "fatal");
...
void _dl_signal_cerror(int errcode, const char* objname, const char* occation,
                       const char* errstring)
{
    if (__builtin_expect(
            GLRO(dl_debug_mask) & ~(DL_DEBUG_STATISTICS | DL_DEBUG_PRELINK), 0))
