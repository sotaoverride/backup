...
static int allocate_dir_block(e2fsck_t ctx,
                              struct ext2_db_entry2* dir_blocks_info, char* buf,
...
void e2fsck_pass2(e2fsck_t ctx)
{
    ...
    cd.pctx.errcode =
        e2fsck_setup_icount(ctx, "inode_count", EXT2_ICOUNT_OPT_INCREMENT,
    ...
    cd.pctx.errcode = ext2fs_dblist_iterate2(fs->dblist, check_dir_func, &cd);
    ...
    for (i = 0; (dx_dir = e2fsck_dx_dir_info_iter(ctx, &i)) != 0;)
    {
        ...
        for (b = 0, dx_db = dx_dir->dx_block; b < dx_dir->numblocks;
             b++, dx_db++)
        {
            pctx.blkcount = b;
             ...
             * at block #0
             */
            if (b && (dx_db->max_hash > dx_db->node_max_hash))
            {
 ...
 */
static int check_dot(e2fsck_t ctx, struct ext2_dir_entry* dirent,
                     ext2_ino_t ino, struct problem_context* pctx)
{
    ...
    else if ((ext2fs_dirent_name_len(dirent) != 1) || (dirent->name[0] != '.'))
    ...
    if (rec_len > 12)
    {
        ...
        if (new_len > 12)
        {
            if (created || fix_problem(ctx, PR_2_SPLIT_DOT, pctx))
            {
                nextdir = (struct ext2_dir_entry*)((char*)dirent + 12);
                dirent->rec_len = 12;
                (void)ext2fs_set_rec_len(ctx->fs, new_len, nextdir);
                nextdir->inode = 0;
                ext2fs_dirent_set_name_len(nextdir, 0);
                ext2fs_dirent_set_file_type(nextdir, EXT2_FT_UNKNOWN);
                status = 1;
            }
        }
    }
 ...
 */
static int check_dotdot(e2fsck_t ctx, struct ext2_dir_entry* dirent,
                        ext2_ino_t ino, struct problem_context* pctx)
{
    problem_t problem = 0;
 ...
 * characters.
 */
static int check_name(e2fsck_t ctx, struct ext2_dir_entry* dirent,
                      struct problem_context* pctx)
{
    int i;
    int fixup = -1;
...
static int encrypted_check_name(e2fsck_t ctx, struct ext2_dir_entry* dirent,
                                struct problem_context* pctx)
{
 ...
 */
static _INLINE_ int check_filetype(e2fsck_t ctx, struct ext2_dir_entry* dirent,
                                   ...
                                   struct problem_context* pctx)
{
    ...
    {
        if (filetype == 0 || !fix_problem(ctx, PR_2_CLEAR_FILETYPE, pctx))
            return 0;
        ext2fs_dirent_set_file_type(dirent, EXT2_FT_UNKNOWN);
        return 1;
    }
    ...
    else if (ext2fs_test_inode_bitmap2(ctx->inode_reg_map, dirent->inode))
    {
        should_be = EXT2_FT_REG_FILE;
    }
    else if (ctx->inode_bad_map &&
             ext2fs_test_inode_bitmap2(ctx->inode_bad_map, dirent->inode))
        should_be = 0;
    else
    {
        e2fsck_read_inode(ctx, dirent->inode, &inode, "check_filetype");
...
static void parse_int_node(ext2_filsys fs, struct ext2_db_entry2* db,
                           struct check_dir_struct* cd,
                           ...
                           int failed_csum)
{
    ...
    if (db->blockcnt == 0)
    {
        ...
        if (failed_csum &&
            (e2fsck_dir_will_be_rehashed(cd->ctx, cd->pctx.ino) ||
             fix_problem(cd->ctx, PR_2_HTREE_ROOT_CSUM_INVALID, &cd->pctx)))
            goto clear_and_exit;
    }
    else
    {
        ...
        if (failed_csum &&
            (e2fsck_dir_will_be_rehashed(cd->ctx, cd->pctx.ino) ||
             fix_problem(cd->ctx, PR_2_HTREE_NODE_CSUM_INVALID, &cd->pctx)))
            goto clear_and_exit;
    }
    ...
    expect_limit = (fs->blocksize - (csum_size + ((char*)ent - block_buf))) /
    ...
    for (i = 0; i < count; i++)
    {
        ...
        {
            cd->pctx.blk = blk;
            if (fix_problem(cd->ctx, PR_2_HTREE_BADBLK, &cd->pctx))
                goto clear_and_exit;
            continue;
        }
        if (hash < prev_hash &&
        ...
        dx_db->previous =
            i ? (ext2fs_le32_to_cpu(ent[i - 1].block) & EXT4_DX_BLOCK_MASK) : 0;
        ...
        if ((i + 1) < count)
            dx_db->node_max_hash = ext2fs_le32_to_cpu(ent[i + 1].hash) & ~1;
        else
        {
...
static void salvage_directory(ext2_filsys fs, struct ext2_dir_entry* dirent,
                              struct ext2_dir_entry* prev, unsigned int* offset,
                              unsigned int block_len)
{
     ...
     * record length.
     */
    if ((left < 0) && ((int)rec_len + left > EXT2_DIR_ENTRY_HEADER_LEN) &&
...
static errcode_t insert_dirent_tail(ext2_filsys fs, void* dirbuf)
{
    ...
    if (d != top)
    {
        unsigned int min_size =
            EXT2_DIR_REC_LEN(ext2fs_dirent_name_len(dirbuf));
                                     ...
                                     struct problem_context* pctx, char* buf)
{
    ...
    if (old_size > EXT4_MIN_INLINE_DATA_SIZE &&
        old_size < EXT4_MIN_INLINE_DATA_SIZE + EXT2_DIR_REC_LEN(1))
    {
    ...
    if (retval == EXT2_ET_INLINE_DATA_NO_SPACE)
    {
        ...
        if (retval)
        {
            if (fix_problem(ctx, PR_2_FIX_INLINE_DIR_FAILED, pctx))
            {
                new_size = 0;
                goto write_inode;
            }
...
static int check_dir_block2(ext2_filsys fs, struct ext2_db_entry2* db,
                            void* priv_data)
{
    int err;
    ...
    {
        err = e2fsck_readahead_dblist(
            fs, E2FSCK_RA_DBLIST_IGNORE_BLOCKCNT, fs->dblist,
            cd->list_offset + cd->ra_entries / 8, cd->ra_entries);
        if (err)
            cd->ra_entries = 0;
...
static int check_dir_block(ext2_filsys fs, struct ext2_db_entry2* db,
                           void* priv_data)
{
    struct dx_dir_info* dx_dir;
    ...
    if (ctx->dirs_to_hash && ext2fs_u32_list_test(ctx->dirs_to_hash, ino))
        dups_found++;
    ...
    if (inline_data_size)
    {
        ...
        *((__u32*)buf) = ext2fs_le32_to_cpu(*((__u32*)buf));
        cd->pctx.errcode = ext2fs_dirent_swab_in2(
            fs, buf + EXT4_INLINE_DATA_DOTDOT_SIZE,
            EXT4_MIN_INLINE_DATA_SIZE - EXT4_INLINE_DATA_DOTDOT_SIZE, 0);
        if (cd->pctx.errcode)
            goto inline_read_fail;
    skip_first_read_swab:
        if (inline_data_size <= EXT4_MIN_INLINE_DATA_SIZE || !db->blockcnt)
            goto inline_read_fail;
        cd->pctx.errcode = ext2fs_dirent_swab_in2(
            fs, buf + EXT4_MIN_INLINE_DATA_SIZE,
            inline_data_size - EXT4_MIN_INLINE_DATA_SIZE, 0);
#endif
    }
    else
...
inline_read_fail:
    ...
    if (((inline_data_size & 3) ||
         (inline_data_size > EXT4_MIN_INLINE_DATA_SIZE &&
          inline_data_size <
              EXT4_MIN_INLINE_DATA_SIZE + EXT2_DIR_REC_LEN(1))) &&
        fix_problem(ctx, PR_2_BAD_INLINE_DIR_SIZE, &pctx))
    {
        errcode_t err =
            fix_inline_dir_size(ctx, ino, &inline_data_size, &pctx, buf);
        if (err)
            return DIRENT_ABORT;
    }
    ...
    if (cd->pctx.errcode)
    {
        ...
        ext2fs_new_dir_block(fs, db->blockcnt == 0 ? ino : 0, EXT2_ROOT_INO,
                             &buf2);
    ...
    if (dx_dir && dx_dir->numblocks)
    {
        ...
        {
            pctx.dir = ino;
            if (fix_problem(ctx, PR_2_UNEXPECTED_HTREE_BLOCK, &pctx))
            {
                clear_htree(ctx, ino);
                dx_dir->numblocks = 0;
                dx_db = 0;
        ...
        if (db->blockcnt == 0)
        {
            ...
            if ((root->reserved_zero || root->info_length < 8 ||
                 root->indirect_levels >= ext2_dir_htree_level(fs)) &&
        ...
        else if ((dirent->inode == 0) && (rec_len == fs->blocksize) &&
...
out_htree:
        ...
        !ext2fs_dirent_has_tail(fs, (struct ext2_dir_entry*)buf))
    {
        ...
        if (!fix_problem(cd->ctx, PR_2_LEAF_NODE_MISSING_CSUM, &cd->pctx))
            goto skip_checksum;
...
skip_checksum:
    ...
    do
    {
        ...
        if (!inline_data_size || dot_state > 1)
        {
            ...
            cd->pctx.num = offset;
            if ((offset + rec_len > max_block_size) || (rec_len < 12) ||
                ((rec_len % 4) != 0) ||
                (((unsigned)ext2fs_dirent_name_len(dirent) +
                  EXT2_DIR_ENTRY_HEADER_LEN) > rec_len))
            {
                if (fix_problem(ctx, PR_2_DIR_CORRUPTED, &cd->pctx))
                {
                    ...
                    if (rec_len < EXT2_DIR_ENTRY_HEADER_LEN || rec_len % 4)
                    {
                        ...
                        ext2fs_dirent_swab_in2(
                            fs, ((char*)dirent) + EXT2_DIR_ENTRY_HEADER_LEN,
                    ...
                    if (need_reswab)
                    {
                        (void)ext2fs_get_rec_len(fs, dirent, &rec_len);
                        ext2fs_dirent_swab_in2(
                            fs, ((char*)dirent) + offset + rec_len,
                            max_block_size - offset - rec_len, 0);
                    }
#endif
                    dir_modified++;
                    continue;
        ...
        else
        {
            ...
            else if (dot_state == 1)
            {
                ...
                dirent = &dotdot;
                dirent->inode = ((struct ext2_dir_entry*)buf)->inode;
            ...
            (dirent->inode > fs->super->s_inodes_count))
        {
            problem = PR_2_BAD_INO;
                 ...
                 (ext2fs_test_inode_bitmap2(ctx->inode_bb_map, dirent->inode)))
        {
            /*
        ...
        else if ((dot_state > 1) && (name_len == 1) && (dirent->name[0] == '.'))
        {
            /*
        ...
        else if ((dot_state > 1) && (dirent->inode == EXT2_ROOT_INO))
        {
            /*
        ...
        first_unused_inode = group * fs->super->s_inodes_per_group + 1 +
        ...
        if (ext2fs_bg_flags_test(fs, group, EXT2_BG_INODE_UNINIT))
        {
            ...
            if (fix_problem(ctx, PR_2_INOREF_BG_INO_UNINIT, &cd->pctx))
            {
         ...
         */
        if (!(ctx->flags & E2F_FLAG_RESTART_LATER) &&
            !(ext2fs_test_inode_bitmap2(ctx->inode_used_map, dirent->inode)))
         ...
         */
        if ((dot_state > 1) &&
            (ext2fs_test_inode_bitmap2(ctx->inode_dir_map, dirent->inode)))
        {
            if (e2fsck_dir_info_get_parent(ctx, dirent->inode, &subdir_parent))
            {
            ...
            if (subdir_parent)
            {
                ...
                if (fix_problem(ctx, PR_2_LINK_DIR, &cd->pctx))
                {
                    dirent->inode = 0;
                    dir_modified++;
                    goto next;
            ...
            {
                (void)e2fsck_dir_info_set_parent(ctx, dirent->inode, ino);
            }
        }
    ...
    if (dx_db)
    {
        ...
        printf("db_block %d, type %d, min_hash 0x%0x, max_hash 0x%0x\n",
               db->blockcnt, dx_db->type, dx_db->min_hash, dx_db->max_hash);
#endif
    ...
    if (dir_modified)
    {
        ...
        if (ext2fs_has_feature_metadata_csum(fs->super) && is_leaf &&
            !inline_data_size &&
            !ext2fs_dirent_has_tail(fs, (struct ext2_dir_entry*)buf))
        {
    ...
    write_and_fix:
        ...
        if (inline_data_size)
        {
            ...
            *((__u32*)buf) = ext2fs_le32_to_cpu(*((__u32*)buf));
            cd->pctx.errcode = ext2fs_dirent_swab_out2(
                fs, buf + EXT4_INLINE_DATA_DOTDOT_SIZE,
                EXT4_MIN_INLINE_DATA_SIZE - EXT4_INLINE_DATA_DOTDOT_SIZE, 0);
            if (cd->pctx.errcode)
...
#endif
            cd->pctx.errcode =
                ext2fs_inline_data_set(fs, ino, 0, buf, inline_data_size);
        }
        else
            cd->pctx.errcode =
                ext2fs_write_dir_block4(fs, block_nr, buf, 0, ino);
        if (will_rehash)
    ...
    else if (is_leaf && failed_csum && !dir_modified)
    {
         ...
         */
        if (fix_problem(ctx, PR_2_LEAF_NODE_ONLY_CSUM_INVALID, &cd->pctx))
            goto write_and_fix;
    }
 ...
 */
static int deallocate_inode_block(ext2_filsys fs, blk64_t* block_nr,
...
static void deallocate_inode(e2fsck_t ctx, ext2_ino_t ino, char* block_buf)
{
    ...
    {
        pctx.errcode = ext2fs_adjust_ea_refcount3(
            fs, ext2fs_file_acl_block(fs, &inode), block_buf, -1, &count, ino);
        ...
        if (count == 0)
        {
            ext2fs_block_alloc_stats2(fs, ext2fs_file_acl_block(fs, &inode),
                                      -1);
        }
...
int e2fsck_process_bad_inode(e2fsck_t ctx, ext2_ino_t dir, ext2_ino_t ino,
                             char* buf)
{
    ext2_filsys fs = ctx->fs;
    struct ext2_inode inode;
 ...
 */
static int allocate_dir_block(e2fsck_t ctx, struct ext2_db_entry2* db,
                              ...
                              struct problem_context* pctx)
{
    ...
    pctx->errcode =
        ext2fs_map_cluster_block(fs, db->ino, &inode, db->blockcnt, &blk);
    ...
    if (db->blockcnt)
        ...
        pctx->errcode =
            ext2fs_new_dir_block(fs, db->ino, EXT2_ROOT_INO, &block);
    ...
    {
        pctx->errcode = ext2fs_inode_size_set(
            fs, &inode, (db->blockcnt + 1) * fs->blocksize);
        if (pctx->errcode)
        {
    ...
    db->blk = blk;
    pctx->errcode =
        ext2fs_bmap2(fs, db->ino, &inode, 0, BMAP_SET, db->blockcnt, 0, &blk);
    if (pctx->errcode)
    {
