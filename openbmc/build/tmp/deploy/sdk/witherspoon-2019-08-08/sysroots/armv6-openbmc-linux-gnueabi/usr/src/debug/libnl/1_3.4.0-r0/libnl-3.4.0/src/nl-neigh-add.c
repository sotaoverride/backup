...
int main(int argc, char* argv[])
{
    ...
    for (;;)
    {
        ...
        switch (c)
        {
            ...
            case 'a':
                ok++;
                nl_cli_neigh_parse_dst(neigh, optarg);
                break;
            case 'l':
                nl_cli_neigh_parse_lladdr(neigh, optarg);
                break;
            case 'd':
                nl_cli_neigh_parse_dev(neigh, link_cache, optarg);
                break;
            case ARG_STATE:
                nl_cli_neigh_parse_state(neigh, optarg);
                break;
        }
    }
    ...
    if ((err = rtnl_neigh_add(sock, neigh, nlflags)) < 0)
        nl_cli_fatal(err, "Unable to add neighbour: %s", nl_geterror(err));
