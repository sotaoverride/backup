...
/* Open the gconv database if necessary.  A non-negative return value
   means success.  */
struct __gconv_loaded_object* __gconv_find_shlib(const char* name)
{
    ...
    if (keyp == NULL)
    {
        ...
        if (found != NULL)
        {
            ...
            if (__builtin_expect(
                    __tsearch(found, &loaded, known_compare) == NULL, 0))
            {
...
static void __libc_freeres_fn_section do_release_all(void* nodep)
{
    struct __gconv_loaded_object* obj = (struct __gconv_loaded_object*)nodep;
...
static void __attribute__((used)) print_all(void)
{
