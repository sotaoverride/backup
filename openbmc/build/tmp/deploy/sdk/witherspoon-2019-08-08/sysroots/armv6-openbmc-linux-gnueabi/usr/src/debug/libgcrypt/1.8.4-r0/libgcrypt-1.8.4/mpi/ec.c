...
mpi_point_t _gcry_mpi_point_new(unsigned int nbits)
{
    mpi_point_t p;
...
void _gcry_mpi_point_release(mpi_point_t p)
{
    if (p)
    {
...
static void point_set(mpi_point_t d, mpi_point_t s)
{
    mpi_set(d->x, s->x);
    mpi_set(d->y, s->y);
    mpi_set(d->z, s->z);
...
static void point_resize(mpi_point_t p, mpi_ec_t ctx)
{
    /*
...
static void point_swap_cond(mpi_point_t d, mpi_point_t s, unsigned long swap,
                            mpi_ec_t ctx)
{
...
mpi_point_t _gcry_mpi_point_set(mpi_point_t point, gcry_mpi_t x, gcry_mpi_t y,
                                gcry_mpi_t z)
{
    if (!point)
...
mpi_point_t _gcry_mpi_point_snatch_set(mpi_point_t point, gcry_mpi_t x,
...
/* W = W mod P.  */
static void ec_mod(gcry_mpi_t w, mpi_ec_t ec)
{
...
static void ec_subm(gcry_mpi_t w, gcry_mpi_t u, gcry_mpi_t v, mpi_ec_t ec)
{
    mpi_sub(w, u, v);
    while (w->sign)
        mpi_add(w, w, ec->p);
...
/* W = 2 * U mod P.  */
static void ec_mul2(gcry_mpi_t w, gcry_mpi_t u, mpi_ec_t ctx)
{
    mpi_lshift(w, u, 1);
    ec_mod(w, ctx);
}

static void ec_powm(gcry_mpi_t w, const gcry_mpi_t b, const gcry_mpi_t e,
                    mpi_ec_t ctx)
{
...
static void ec_pow2(gcry_mpi_t w, const gcry_mpi_t b, mpi_ec_t ctx)
{
...
/* Shortcut for
   ...
   for easier optimization.  */
static void ec_pow3(gcry_mpi_t w, const gcry_mpi_t b, mpi_ec_t ctx)
{
...
static int ec_get_a_is_pminus3(mpi_ec_t ec)
{
    gcry_mpi_t tmp;
...
/* Accessor for helper variable.  */
static gcry_mpi_t ec_get_two_inv_p(mpi_ec_t ec)
{
...
static void ec_p_init(mpi_ec_t ctx, enum gcry_mpi_ec_models model,
                      enum ecc_dialects dialect, int flags, gcry_mpi_t p,
                      gcry_mpi_t a, gcry_mpi_t b)
{
    int i;
    ...
    ctx->t.p_barrett =
        use_barrett > 0 ? _gcry_mpi_barrett_init(ctx->p, 0) : NULL;
...
static void ec_deinit(void* opaque)
{
    mpi_ec_t ctx = opaque;
    int i;
...
mpi_ec_t _gcry_mpi_ec_p_internal_new(enum gcry_mpi_ec_models model,
                                     enum ecc_dialects dialect, int flags,
...
gpg_err_code_t _gcry_mpi_ec_decode_point(mpi_point_t result, gcry_mpi_t value,
                                         mpi_ec_t ec)
{
    gcry_err_code_t rc;
...
static void dup_point_weierstrass(mpi_point_t result, mpi_point_t point,
                                  mpi_ec_t ctx)
{
#define x3 (result->x)
#define y3 (result->y)
...
static void dup_point_montgomery(mpi_point_t result, mpi_point_t point,
                                 mpi_ec_t ctx)
{
    (void)result;
    (void)point;
...
static void dup_point_edwards(mpi_point_t result, mpi_point_t point,
                              mpi_ec_t ctx)
{
#define X1 (point->x)
#define Y1 (point->y)
...
/*  RESULT = 2 * POINT  */
void _gcry_mpi_ec_dup_point(mpi_point_t result, mpi_point_t point, mpi_ec_t ctx)
{
    switch (ctx->model)
    {
...
static void add_points_weierstrass(mpi_point_t result, mpi_point_t p1,
                                   mpi_point_t p2, mpi_ec_t ctx)
{
#define x1 (p1->x)
#define y1 (p1->y)
#define z1 (p1->z)
...
static void add_points_montgomery(mpi_point_t result, mpi_point_t p1,
                                  mpi_point_t p2, mpi_ec_t ctx)
{
    (void)result;
    (void)p1;
...
static void add_points_edwards(mpi_point_t result, mpi_point_t p1,
                               mpi_point_t p2, mpi_ec_t ctx)
{
#define X1 (p1->x)
#define Y1 (p1->y)
...
static void montgomery_ladder(mpi_point_t prd, mpi_point_t sum, mpi_point_t p1,
                              mpi_point_t p2, gcry_mpi_t dif_x, mpi_ec_t ctx)
{
...
/* RESULT = P1 + P2 */
void _gcry_mpi_ec_add_points(mpi_point_t result, mpi_point_t p1, mpi_point_t p2,
                             mpi_ec_t ctx)
{
    switch (ctx->model)
    {
...
/* RESULT = P1 - P2  (Weierstrass version).*/
static void sub_points_weierstrass(mpi_point_t result, mpi_point_t p1,
                                   mpi_point_t p2, mpi_ec_t ctx)
{
    (void)result;
    (void)p1;
...
/* RESULT = P1 - P2  (Montgomery version).*/
static void sub_points_montgomery(mpi_point_t result, mpi_point_t p1,
                                  mpi_point_t p2, mpi_ec_t ctx)
{
    (void)result;
    (void)p1;
...
/* RESULT = P1 - P2  (Twisted Edwards version).*/
static void sub_points_edwards(mpi_point_t result, mpi_point_t p1,
                               mpi_point_t p2, mpi_ec_t ctx)
{
...
/* RESULT = P1 - P2 */
void _gcry_mpi_ec_sub_points(mpi_point_t result, mpi_point_t p1, mpi_point_t p2,
                             mpi_ec_t ctx)
{
    switch (ctx->model)
    {
...
void _gcry_mpi_ec_mul_point(mpi_point_t result, gcry_mpi_t scalar,
                            mpi_point_t point, mpi_ec_t ctx)
{
    ...
    if (ctx->model == MPI_EC_EDWARDS ||
        (ctx->model == MPI_EC_WEIERSTRASS && mpi_is_secure(scalar)))
    {
        /* Simple left to right binary method.  Algorithm 3.27 from
         * {author={Hankerson, Darrel and Menezes, Alfred J. and Vanstone,
         * Scott}, title = {Guide to Elliptic Curve Cryptography}, year =
         * {2003}, isbn = {038795273X}, url =
         * {http://www.cacr.math.uwaterloo.ca/ecc/}, publisher =
