...
static errcode_t follow_link(ext2_filsys fs, ext2_ino_t root, ext2_ino_t dir,
                             ext2_ino_t inode, int link_count, char* buf,
                             ext2_ino_t* res_inode)
{
    ...
    else if (ei.i_flags & EXT4_INLINE_DATA_FL)
    {
        ...
        retval = ext2fs_inline_data_get(fs, inode, &ei, buffer, NULL);
        if (retval)
        {
    ...
    retval = open_namei(fs, root, dir, pathname, ei.i_size, 1, link_count, buf,
                        res_inode);
    if (buffer)
        ext2fs_free_mem(&buffer);
    return retval;
...
static errcode_t dir_namei(ext2_filsys fs, ext2_ino_t root, ext2_ino_t dir,
                           const char* pathname, int pathlen, int link_count,
                           char* buf, const char** name, int* namelen,
                           ext2_ino_t* res_inode)
{
    char c;
    const char* thisname;
    ...
    while (1)
    {
        ...
        if (retval)
            return retval;
        retval = follow_link(fs, root, dir, inode, link_count, buf, &dir);
        if (retval)
            return retval;
    }
    *name = thisname;
                            ...
                            int link_count, char* buf, ext2_ino_t* res_inode)
{
    ...
    printf("open_namei: root=%lu, dir=%lu, path=%.*s, lc=%d\n", root, base,
           pathlen, pathname, link_count);
#endif
    retval = dir_namei(fs, root, base, pathname, pathlen, link_count, buf,
                       &base_name, &namelen, &dir);
    if (retval)
        return retval;
    if (!namelen)
    ...
    if (follow)
    {
        retval = follow_link(fs, root, dir, inode, link_count, buf, &inode);
        if (retval)
            return retval;
    }
#ifdef NAMEI_DEBUG
                       ...
                       const char* name, ext2_ino_t* inode)
{
    ...
    retval = open_namei(fs, root, cwd, name, strlen(name), 0, 0, buf, inode);
                              ...
                              const char* name, ext2_ino_t* inode)
{
    ...
    retval = open_namei(fs, root, cwd, name, strlen(name), 1, 0, buf, inode);
