...
static void enum_state_free(gpointer data)
{
...
static EnumState* enum_state_new(gboolean is_flags)
{
    EnumState* state;
...
static void enum_state_add_value(EnumState* state, const gchar* nick,
                                 const gchar* valuestr, GError** error)
{
    gint64 value;
    gchar* end;
    ...
    if (*end || (state->is_flags ? (value > G_MAXUINT32 || value < 0)
...
static void enum_state_end(EnumState** state_ptr, GError** error)
{
    EnumState* state;
...
static KeyState* key_state_new(const gchar* type_string,
                               const gchar* gettext_domain, gboolean is_enum,
                               gboolean is_flags, GString* strinfo)
{
    KeyState* state;
...
static KeyState* key_state_override(KeyState* state,
                                    const gchar* gettext_domain)
{
    ...
    copy->strinfo = g_string_new_len(state->strinfo->str, state->strinfo->len);
...
static KeyState* key_state_new_child(const gchar* child_schema)
{
    KeyState* state;
...
static gboolean is_valid_choices(GVariant* variant, GString* strinfo)
{
 ...
 */
static void key_state_check_range(KeyState* state, GError** error)
{
...
static void key_state_set_range(KeyState* state, const gchar* min_str,
                                const gchar* max_str, GError** error)
{
    const struct
    {
        const gchar type;
...
static GString* key_state_start_default(KeyState* state, const gchar* l10n,
...
static void key_state_end_default(KeyState* state, GString** string,
                                  GError** error)
{
    state->unparsed_default_value = *string;
    *string = NULL;

    state->default_value = g_variant_parse(
        state->type, state->unparsed_default_value->str, NULL, NULL, error);
    if (!state->default_value)
    {
        gchar* type = g_variant_type_dup_string(state->type);
        g_prefix_error(
            error, _("Failed to parse <default> value of type “%s”: "), type);
        g_free(type);
    }
...
static void key_state_start_choices(KeyState* state, GError** error)
{
...
static void key_state_add_choice(KeyState* state, const gchar* choice,
                                 GError** error)
{
...
static void key_state_end_choices(KeyState* state, GError** error)
{
    if (!state->has_choices)
    {
        g_set_error(error, G_MARKUP_ERROR, G_MARKUP_ERROR_INVALID_CONTENT,
...
static void key_state_start_aliases(KeyState* state, GError** error)
{
...
static void key_state_add_alias(KeyState* state, const gchar* alias,
                                const gchar* target, GError** error)
{
    if (strinfo_builder_contains(state->strinfo, alias))
    {
        if (strinfo_is_string_valid((guint32*)state->strinfo->str,
                                    state->strinfo->len / 4, alias))
        {
            if (state->is_enum)
                g_set_error(error, G_MARKUP_ERROR,
                            G_MARKUP_ERROR_INVALID_CONTENT,
                            _("<alias value='%s'/> given when “%s” is already "
                              "a member of the enumerated type"),
                            alias, alias);
...
static void key_state_end_aliases(KeyState* state, GError** error)
{
    if (!state->has_aliases)
    {
        g_set_error(error, G_MARKUP_ERROR, G_MARKUP_ERROR_INVALID_CONTENT,
...
static gboolean key_state_check(KeyState* state, GError** error)
{
    if (state->checked)
        return TRUE;
...
static GVariant* key_state_serialise(KeyState* state)
{
    if (state->serialised == NULL)
    {
        ...
        else
        {
            ...
            if (state->strinfo->len)
            {
                ...
                array = g_variant_new_from_data(G_VARIANT_TYPE("au"), data,
                                                size, TRUE, g_free, data);
                ...
                g_variant_builder_add(
                    &builder, "(y@au)",
                    state->is_flags ? 'f' : state->is_enum ? 'e' : 'c', array);
            }

            /* range */
            if (state->minimum || state->maximum)
                g_variant_builder_add(&builder, "(y(**))", 'r', state->minimum,
                                      state->maximum);
...
static void key_state_free(gpointer data)
{
    KeyState* state = data;
...
static gboolean is_valid_keyname(const gchar* key, GError** error)
{
    gint i;
    ...
    for (i = 1; key[i]; i++)
    {
        if (key[i] != '-' && !g_ascii_islower(key[i]) &&
...
static SchemaState* schema_state_new(const gchar* path,
                                     const gchar* gettext_domain,
                                     SchemaState* extends,
                                     ...
                                     const gchar* list_of)
{
    ...
    state->keys =
        g_hash_table_new_full(g_str_hash, g_str_equal, g_free, key_state_free);

    return state;
}

static void schema_state_free(gpointer data)
{
...
static void schema_state_add_child(SchemaState* state, const gchar* name,
                                   const gchar* schema, GError** error)
{
    gchar* childname;
    ...
    g_hash_table_insert(state->keys, childname, key_state_new_child(schema));
}

static KeyState*
    schema_state_add_key(SchemaState* state, GHashTable* enum_table,
                         GHashTable* flags_table, const gchar* name,
                         const gchar* type_string, const gchar* enum_type,
                         const gchar* flags_type, GError** error)
{
    SchemaState* node;
    GString* strinfo;
    KeyState* key;
    ...
    key = key_state_new(type_string, state->gettext_domain, enum_type != NULL,
                        flags_type != NULL, strinfo);
    g_hash_table_insert(state->keys, g_strdup(name), key);

    return key;
}

static void schema_state_add_override(SchemaState* state, KeyState** key_state,
                                      GString** string, const gchar* key,
                                      const gchar* l10n, const gchar* context,
                                      GError** error)
{
    SchemaState* parent;
    KeyState* original;
...
static void override_state_end(KeyState** key_state, GString** string,
                               GError** error)
{
...
static gboolean is_subclass(const gchar* class_name,
...
static void parse_state_start_schema(ParseState* state, const gchar* id,
                                     const gchar* path,
                                     const gchar* gettext_domain,
                                     const gchar* extends_name,
                                     const gchar* list_of, GError** error)
{
    SchemaState* extends;
    gchar* my_id;
    ...
    state->schema_state =
        schema_state_new(path, gettext_domain, extends, extends_name, list_of);
...
static void parse_state_start_enum(ParseState* state, const gchar* id,
                                   gboolean is_flags, GError** error)
{
    GSList** list =
        is_flags ? &state->this_file_flagss : &state->this_file_enums;
...
#define COLLECT(first, ...)                                                    \
    g_markup_collect_attributes(element_name, attribute_names,                 \
                                attribute_values, error, first, __VA_ARGS__,   \
...
#define NO_ATTRS() COLLECT(G_MARKUP_COLLECT_INVALID, NULL)
    ...
    if (container == NULL)
    {
        ...
        {
            COLLECT(OPTIONAL | STRDUP, "gettext-domain",
    ...
    else if (strcmp(container, "schemalist") == 0)
    {
        if (strcmp(element_name, "schema") == 0)
        {
            ...
            if (COLLECT(STRING, "id", &id, OPTIONAL | STRING, "path", &path,
                        OPTIONAL | STRING, "gettext-domain", &gettext_domain,
                        OPTIONAL | STRING, "extends", &extends,
                        OPTIONAL | STRING, "list-of", &list_of))
                parse_state_start_schema(
                    state, id, path,
                    gettext_domain ? gettext_domain : state->schemalist_domain,
    ...
    else if (strcmp(container, "schema") == 0)
    {
        if (strcmp(element_name, "key") == 0)
        {
            ...
            if (COLLECT(STRING, "name", &name, OPTIONAL | STRING, "type",
                        &type_string, OPTIONAL | STRING, "enum", &enum_type,
                        OPTIONAL | STRING, "flags", &flags_type))

                state->key_state = schema_state_add_key(
                    state->schema_state, state->enum_table, state->flags_table,
                    name, type_string, enum_type, flags_type, error);
            return;
        }
        else if (strcmp(element_name, "child") == 0)
        {
            ...
            if (COLLECT(STRING, "name", &name, STRING, "schema", &schema))
                schema_state_add_child(state->schema_state, name, schema,
                                       error);
            return;
        }
        else if (strcmp(element_name, "override") == 0)
        {
            ...
            if (COLLECT(STRING, "name", &name, OPTIONAL | STRING, "l10n", &l10n,
    ...
    else if (strcmp(container, "key") == 0)
    {
        if (strcmp(element_name, "default") == 0)
        {
            ...
            if (COLLECT(STRING | OPTIONAL, "l10n", &l10n, STRING | OPTIONAL,
                        "context", &context))
                state->string = key_state_start_default(state->key_state, l10n,
                                                        context, error);
            return;
        }
        ...
        {
            const gchar *min, *max;
            if (COLLECT(STRING | OPTIONAL, "min", &min, STRING | OPTIONAL,
                        "max", &max))
    ...
    /* children of <enum> {{{3 */
    else if (strcmp(container, "enum") == 0 || strcmp(container, "flags") == 0)
    {
        if (strcmp(element_name, "value") == 0)
        {
            const gchar *nick, *valuestr;
            if (COLLECT(STRING, "nick", &nick, STRING, "value", &valuestr))
    ...
    if (container)
        g_set_error(error, G_MARKUP_ERROR, G_MARKUP_ERROR_UNKNOWN_ELEMENT,
                    _("Element <%s> not allowed inside <%s>"), element_name,
                    container);
    else
        g_set_error(error, G_MARKUP_ERROR, G_MARKUP_ERROR_UNKNOWN_ELEMENT,
                    _("Element <%s> not allowed at the top level"),
                    element_name);
}
/* 2}}} */
/* End element {{{2 */

static void key_state_end(KeyState** state_ptr, GError** error)
{
    KeyState* state;
...
static void schema_state_end(SchemaState** state_ptr, GError** error)
{
    *state_ptr = NULL;
}
...
static void output_key(gpointer key, gpointer value, gpointer user_data)
{
        ...
        !g_hash_table_lookup(data->schema_table, state->child_schema))
    {
        ...
        message = g_strdup_printf(
            _("Warning: undefined reference to <schema id='%s'/>"),
            state->child_schema);
        g_printerr("%s\n", message);
        g_free(message);
    }
...
static void output_schema(gpointer key, gpointer value, gpointer user_data)
{
    ...
    if (data.l10n)
        gvdb_hash_table_insert_string(data.pair.table, ".gettext-domain",
                                      state->gettext_domain);

    gvdb_pair_clear(&data.pair);
}

static gboolean write_to_file(GHashTable* schema_table, const gchar* filename,
                              GError** error)
{
    ...
    success = gvdb_table_write_contents(data.root_pair.table, filename,
                                        G_BYTE_ORDER != G_LITTLE_ENDIAN, error);
...
/* Parser driver {{{1 */
static GHashTable* parse_gschema_files(gchar** files, gboolean strict)
{
    ...
    state.enum_table =
        g_hash_table_new_full(g_str_hash, g_str_equal, g_free, enum_state_free);

    state.flags_table =
        g_hash_table_new_full(g_str_hash, g_str_equal, g_free, enum_state_free);

    state.schema_table = g_hash_table_new_full(g_str_hash, g_str_equal, g_free,
                                               schema_state_free);
    ...
    while ((filename = *files++) != NULL)
    {
            ...
            !g_markup_parse_context_end_parse(context, &error))
        {
            ...
            fprintf(stderr, "%s:%d:%d  %s.  ", filename, line, col,
                    error->message);
            g_clear_error(&error);
...
static gboolean set_overrides(GHashTable* schema_table, gchar** files,
                              gboolean strict)
{
    const gchar* filename;
    GError* error = NULL;
    ...
    while ((filename = *files++))
    {
        ...
        for (i = 0; groups[i]; i++)
        {
            ...
            g_debug("Processing group '%s' (schema '%s', %s)", group,
                    schema_name, desktop_id ? desktop_id : "all desktops");
            ...
            for (j = 0; keys[j]; j++)
            {
                ...
                value =
                    g_variant_parse(state->type, string, NULL, NULL, &error);
                ...
                if (desktop_id != NULL)
                {
                    ...
                    g_variant_dict_insert_value(state->desktop_overrides,
                                                desktop_id, value);
                    g_variant_unref(value);
                }
                else
...
int main(int argc, char** argv)
{
    ...
    GOptionEntry entries[] = {
        {"version", 0, 0, G_OPTION_ARG_NONE, &show_version_and_exit,
         N_("Show program version and exit"), NULL},
        {"targetdir", 0, 0, G_OPTION_ARG_FILENAME, &targetdir,
        ...
        {"allow-any-name", 0, 0, G_OPTION_ARG_NONE, &allow_any_name,
         N_("Do not enforce key name restrictions")},

        /* These options are only for use in the gschema-compile tests */
        {"schema-file", 0, G_OPTION_FLAG_HIDDEN, G_OPTION_ARG_FILENAME_ARRAY,
         &schema_files, NULL, NULL},
        {"override-file", 0, G_OPTION_FLAG_HIDDEN, G_OPTION_ARG_FILENAME_ARRAY,
         &override_files, NULL, NULL},
        {NULL}};
    ...
    target = g_build_filename(targetdir ? targetdir : srcdir,
    ...
    if (override_files != NULL && !set_overrides(table, override_files, strict))
    {
        retval = 1;
        goto done;
    }
