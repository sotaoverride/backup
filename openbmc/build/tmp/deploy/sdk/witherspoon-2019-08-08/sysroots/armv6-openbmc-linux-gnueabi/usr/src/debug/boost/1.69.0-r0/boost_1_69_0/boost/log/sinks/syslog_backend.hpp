 ...
 * \date   08.01.2008
 *
 * The header contains implementation of a Syslog sink backend along with its
 * setup facilities.
 */
...
//! The enumeration defined the possible implementation types for the syslog
//! backend
enum impl_types
{
 ...
 * simply returns the extracted attribute value converted to the Syslog severity
 * level.
 */
 ...
 *
 * The class allows to setup a custom mapping between an attribute and Syslog
 * severity levels. The mapping should be initialized similarly to the standard
 * \c map container, by using indexing operator and assignment.
 */
template <typename AttributeValueT = int>
 ...
 * that in that case you will only have one chance to specify syslog facility
 * and process identification string - on the first native syslog backend
 * construction. Other native syslog backends will ignore these parameters.
...
class syslog_backend : public basic_formatted_sink_backend<char>
{
  ...
  public:
    ...
    typedef boost::log::aux::light_function<syslog::level(record_view const&)>
  ...
  public:
     ...
     * <tt>syslog::user</tt> facility code. IPv4 protocol will be used.
     */
     ...
     *
     * \li \c facility - Specifies the facility code. If not specified,
     * <tt>syslog::user</tt> will be used. \li \c use_impl - Specifies the
     * backend implementation. Can be one of: \li \c native - Use the native
     * syslog API, if available. If no native API is available, it is equivalent
     * to \c udp_socket_based. \li \c udp_socket_based - Use the UDP
     * socket-based implementation, conforming to RFC3164 protocol
     * specification. This is the default. \li \c ip_version - Specifies IP
     * protocol version to use, in case if socket-based implementation is used.
     * Can be either \c v4 (the default one) or \c v6. \li \c ident - Process
     * identification string. This parameter is only supported by native syslog
     * implementation.
     */
     ...
     *
     * \note Does not have effect if the backend was constructed to use native
     * syslog API
     *
     * \param addr The local address
     * \param port The local port number
     */
    BOOST_LOG_API void set_local_address(std::string const& addr,
     ...
     * \param port The local port number
     */
    BOOST_LOG_API void set_local_address(boost::asio::ip::address const& addr,
                                         unsigned short port = 514);
     ...
     */
    BOOST_LOG_API void set_target_address(std::string const& addr,
                                          unsigned short port = 514);
    /*!
     ...
     * \param port The port number on the remote host
     */
    BOOST_LOG_API void set_target_address(boost::asio::ip::address const& addr,
                                          unsigned short port = 514);

#endif // !defined(BOOST_LOG_NO_ASIO)

    /*!
     * The method passes the formatted message to the syslog API or sends to a
     * syslog server
     */
  ...
  private:
    ...
    {
        construct(args[keywords::facility | syslog::user],
#if !defined(BOOST_LOG_NO_ASIO)
    ...
    BOOST_LOG_API void construct(syslog::facility facility,
