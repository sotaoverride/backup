...
static gboolean g_property_action_get_invert_boolean(GAction* action)
{
...
static const gchar* g_property_action_get_name(GAction* action)
{
    GPropertyAction* paction = G_PROPERTY_ACTION(action);
...
static const GVariantType* g_property_action_get_parameter_type(GAction* action)
{
    GPropertyAction* paction = G_PROPERTY_ACTION(action);

    return paction->pspec->value_type == G_TYPE_BOOLEAN ? NULL
                                                        : paction->state_type;
}

static const GVariantType* g_property_action_get_state_type(GAction* action)
{
    GPropertyAction* paction = G_PROPERTY_ACTION(action);
...
static GVariant* g_property_action_get_state_hint(GAction* action)
{
    return NULL;
}

static gboolean g_property_action_get_enabled(GAction* action)
{
    return TRUE;
}

static void g_property_action_set_state(GPropertyAction* paction,
                                        GVariant* variant)
{
...
static void g_property_action_change_state(GAction* action, GVariant* value)
{
    GPropertyAction* paction = G_PROPERTY_ACTION(action);
...
static GVariant* g_property_action_get_state(GAction* action)
{
    GPropertyAction* paction = G_PROPERTY_ACTION(action);
    GValue value = G_VALUE_INIT;
    GVariant* result;
...
static void g_property_action_activate(GAction* action, GVariant* parameter)
{
    GPropertyAction* paction = G_PROPERTY_ACTION(action);
    ...
    if (paction->pspec->value_type == G_TYPE_BOOLEAN)
    {
        ...
        g_return_if_fail(paction->pspec->value_type == G_TYPE_BOOLEAN &&
                         parameter == NULL);
    ...
    else
    {
        g_return_if_fail(parameter != NULL &&
                         g_variant_is_of_type(parameter, paction->state_type));

        g_property_action_set_state(paction, parameter);
    }
}

static const GVariantType* g_property_action_determine_type(GParamSpec* pspec)
{
    ...
    switch (pspec->value_type)
    {
        ...
        default:
            g_critical("Unable to use GPropertyAction with property '%s::%s' "
                       "of type '%s'",
...
static void g_property_action_notify(GObject* object, GParamSpec* pspec,
                                     gpointer user_data)
{
...
static void g_property_action_set_property_name(GPropertyAction* paction,
                                                const gchar* property_name)
{
    GParamSpec* pspec;
    gchar* detailed;

    pspec = g_object_class_find_property(G_OBJECT_GET_CLASS(paction->object),
                                         property_name);
    ...
    {
        g_critical("Property '%s::%s' used with GPropertyAction must be "
                   "readable, writable, and not construct-only",
                   G_OBJECT_TYPE_NAME(paction->object), property_name);
        return;
    }
    ...
    g_signal_connect(paction->object, detailed,
                     G_CALLBACK(g_property_action_notify), paction);
    g_free(detailed);
}

static void g_property_action_set_property(GObject* object, guint prop_id,
                                           const GValue* value,
                                           GParamSpec* pspec)
{
    ...
    switch (prop_id)
    {
        ...
        case PROP_PROPERTY_NAME:
            g_property_action_set_property_name(paction,
...
static void g_property_action_get_property(GObject* object, guint prop_id,
...
static void g_property_action_finalize(GObject* object)
{
    GPropertyAction* paction = G_PROPERTY_ACTION(object);

    g_signal_handlers_disconnect_by_func(paction->object,
                                         g_property_action_notify, paction);
    g_object_unref(paction->object);
    g_free(paction->name);

    G_OBJECT_CLASS(g_property_action_parent_class)->finalize(object);
}

void g_property_action_init(GPropertyAction* property)
{
}

void g_property_action_iface_init(GActionInterface* iface)
{
...
void g_property_action_class_init(GPropertyActionClass* class)
{
    ...
    g_object_class_install_property(
        ...
        g_param_spec_string("name", P_("Action Name"),
                            P_("The name used to invoke the action"), NULL,
    ...
    g_object_class_install_property(
        ...
        g_param_spec_boxed("parameter-type", P_("Parameter Type"),
    ...
    g_object_class_install_property(
        ...
        g_param_spec_boolean("enabled", P_("Enabled"),
                             P_("If the action can be activated"), TRUE,
                             G_PARAM_READABLE | G_PARAM_STATIC_STRINGS));
     ...
     **/
    g_object_class_install_property(
        object_class, PROP_STATE_TYPE,
        g_param_spec_boxed("state-type", P_("State Type"),
    ...
    g_object_class_install_property(
        ...
        g_param_spec_variant("state", P_("State"),
    ...
    g_object_class_install_property(
        ...
        g_param_spec_object("object", P_("Object"),
     ...
     **/
    g_object_class_install_property(
        object_class, PROP_PROPERTY_NAME,
        g_param_spec_string("property-name", P_("Property name"),
                            P_("The name of the property to wrap"), NULL,
                            G_PARAM_WRITABLE | G_PARAM_CONSTRUCT_ONLY |
                                G_PARAM_STATIC_STRINGS));
     ...
     */
    g_object_class_install_property(
        object_class, PROP_INVERT_BOOLEAN,
        g_param_spec_boolean(
            "invert-boolean", P_("Invert boolean"),
            P_("Whether to invert the value of a boolean property"), FALSE,
            G_PARAM_READWRITE | G_PARAM_CONSTRUCT_ONLY |
                G_PARAM_STATIC_STRINGS));
}
 ...
 * Since: 2.38
 **/
GPropertyAction* g_property_action_new(const gchar* name, gpointer object,
                                       const gchar* property_name)
{
    ...
    return g_object_new(G_TYPE_PROPERTY_ACTION, "name", name, "object", object,
                        "property-name", property_name, NULL);
}
