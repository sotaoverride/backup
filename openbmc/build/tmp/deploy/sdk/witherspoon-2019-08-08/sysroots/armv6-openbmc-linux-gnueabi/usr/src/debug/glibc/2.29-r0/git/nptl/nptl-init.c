...
#define __set_futex_clock_realtime() __have_futex_clock_realtime = 1
#else
...
#endif
    void
    __nptl_set_robust(struct pthread*);
...
static void sigcancel_handler(int sig, siginfo_t* si, void* ctx)
{
    ...
    while (1)
    {
       ...
       from another process) is has to be done here.  */
        ...
        int curval =
            THREAD_ATOMIC_CMPXCHG_VAL(self, cancelhandling, newval, oldval);
        if (curval == oldval)
        {
