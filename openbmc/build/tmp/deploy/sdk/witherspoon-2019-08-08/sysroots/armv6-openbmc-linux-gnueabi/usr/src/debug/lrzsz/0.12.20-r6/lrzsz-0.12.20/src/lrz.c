...
int tryzhdrtype =
    ZRINIT; /* Header type to send corresponding to Last rx close */
time_t stop_time;
...
#endif
#define DO_SYSLOG_FNAME(message)                                               \
    do                                                                         \
    {                                                                          \
        if (enable_syslog)                                                     \
        {                                                                      \
...
int main(int argc, char* argv[])
{
    ...
    while ((c = getopt_long(argc, argv, "a+bB:cCDeEhm:M:OprRqs:St:uUvw:XZy",
        ...
        switch (c)
        {
            ...
            case 'S':
                ...
                if (timesync_flag == 2)
                {
                    ...
                    error(0, 0,
                          _("don't have settimeofday, will not set time\n"));
#endif
                    if (getuid() != 0)
                        error(0, 0,
                              _("not running as root (this is good!), can not "
                                "set time\n"));
                }
#endif
                break;
...
static void usage(int exitcode, const char* what)
{
    ...
    fputs(_("    (X) = option applies to XMODEM only\n"
          ...
          f);
    fputs(
        _("  -+, --append                append to existing files\n"
          "  -a, --ascii                 ASCII transfer (change CR/LF to LF)\n"
          "  -b, --binary                binary transfer\n"
          "  -B, --bufsize N             buffer N bytes (N==auto: buffer whole "
          "file)\n"
          "  -c, --with-crc              Use 16 bit CRC (X)\n"
          "  -C, --allow-remote-commands allow execution of remote commands "
          "(Z)\n"
          ...
          "for data\n"
          "      --o-sync                open output file(s) in synchronous "
          "write mode\n"
          "  -p, --protect               protect existing files\n"
          "  -q, --quiet                 quiet, no progress reports\n"
          "  -r, --resume                try to resume interrupted file "
          "transfer (Z)\n"
          ...
          "  -U, --unrestrict            disable restricted mode (if allowed "
          "to)\n"
...
#ifdef ENABLE_SYSLOG
            if (enable_syslog)
                lsyslog(LOG_ERR, "%s/%s: cannot open: %m", shortname,
                        protname());
#endif
            return ERROR;
        }
        ...
        )
        {
            ...
            if (enable_syslog)
                lsyslog(LOG_INFO, "%s/%s: %ld Bytes, %ld BPS", shortname,
                        protname(), zi.bytes_received, bps);
#endif
        }
    }
    return OK;
...
#ifdef ENABLE_SYSLOG
    if (enable_syslog)
        lsyslog(LOG_ERR, "%s/%s: got error", shortname ? shortname : "no.name",
                protname());
#endif
    canit(STDOUT_FILENO);
    if (Topipe && fout)
...
static int procheader(char* name, struct zm_fileinfo* zi)
{
    ...
    if (junk_path)
    {
        ...
        if (p)
        {
            ...
            if (!*p)
            {
                ...
                DO_SYSLOG(
                    (LOG_ERR, "file name ends with a /, skipped: %s", name));
                return ERROR;
            }
            name = p;
        }
        ...
        && (fout = fopen(name, "r")))
    {
        ...
        if (zmanag == ZF1_ZMNEW || zmanag == ZF1_ZMNEWL)
        {
            if (-1 == fstat(fileno(fout), &sta))
            {
                ...
                DO_SYSLOG((LOG_ERR, "cannot fstat open file %s: %s", name,
                           strerror(e)));
                return ERROR;
            }
            if (zmanag == ZF1_ZMNEW)
            {
                ...
                {
                    DO_SYSLOG(
                        (LOG_INFO, "skipping %s: newer file exists", name));
            ...
            else
            {
                ...
                {
                    DO_SYSLOG((LOG_INFO,
                               "skipping %s: longer+newer file exists", name));
                    return ERROR; /* skips file */
                }
            }
            fclose(fout);
    ...
    if (in_tcpsync)
    {
        ...
        {
            error(1, errno,
                  _("cannot tmpfile() for tcp protocol synchronization"));
        }
        zi->bytes_received = 0;
        return OK;
    }
    ...
    else
    {
        ...
        if (Thisbinary && zconv == ZCRESUM)
        {
            ...
            if (fout && 0 == fstat(fileno(fout), &st))
            {
                ...
                if (can_resume)
                {
                    ...
                    zi->bytes_skipped =
                        0; /* resume impossible, file has changed */
                goto buffer_it;
            }
            zi->bytes_skipped = 0;
            if (fout)
        ...
        if (!fout)
        {
            ...
            DO_SYSLOG(
                (LOG_ERR, "%s: cannot open: %s", protname(), strerror(e)));
            return ERROR;
        }
    }
buffer_it:
    if (Topipe == 0)
    {
        ...
        if (o_sync)
        {
            ...
            {
                oldflags |= O_SYNC;
                fcntl(fileno(fout), F_SETFD,
                      oldflags); /* errors don't matter */
            }
        }
#endif
        ...
        switch (c)
        {
            case ZEOF:
                ...
                )
                {
                    ...
                    DO_SYSLOG_FNAME((LOG_INFO, "%s/%s: %ld Bytes, %ld BPS",
                                     shortname, protname(),
                                     (long)zi->bytes_total, bps));
                }
                /* FALL THROUGH */
            case ZSKIP:
                if (c == ZSKIP)
                {
                    if (Verbose)
                        vstringf(_("Skipped"));
                    DO_SYSLOG_FNAME(
                        (LOG_INFO, "%s/%s: skipped", shortname, protname()));
                }
                switch (tryz())
                {
                    case ZCOMPL:
            ...
            case ERROR:
                DO_SYSLOG_FNAME(
                    (LOG_INFO, "%s/%s: error", shortname, protname()));
                return ERROR;
        }
    }
}
...
static int rzfile(struct zm_fileinfo* zi)
{
    ...
    {
        DO_SYSLOG_FNAME(
            (LOG_INFO, "%s/%s: procheader error", shortname, protname()));
    ...
    nxthdr:
        ...
        if (anker)
        {
            ...
            for (akt = anker, last = NULL; akt;
                 last = akt ? akt : last, akt = next)
            {
                ...
                if (akt->pos < zi->bytes_received)
                {
                    vfile("removing unneeded saved out-of-sync-paket %lx, len "
                          "%ld",
                          akt->pos, akt->len);
                    if (last)
                        last->next = akt->next;
    ...
    skip_oosb:
        ...
        switch (c)
        {
            ...
            case TIMEOUT:
                ...
                {
                    DO_SYSLOG_FNAME((
                        LOG_INFO, "%s/%s: error: zgethdr returned %s",
                        shortname, protname(), c == ZNAK ? "ZNAK" : "TIMEOUT"));
                    vfile("rzfile: zgethdr returned %d", c);
                    return ERROR;
                }
            case ZFILE:
            ...
            case ZEOF:
                ...
                {
                    tryzhdrtype = ZFERR;
                    DO_SYSLOG_FNAME((LOG_INFO,
                                     "%s/%s: error: closeit return <>0",
                                     shortname, protname()));
            ...
            moredata:
                     ...
                     zi->bytes_received > last_bps / 2 + last_rxbytes))
                {
                        ...
                        if (low_bps)
                        {
                            if (last_bps < min_bps)
                            {
                                if (now - low_bps >= min_bps_time)
                                {
                                    /* too bad */
                                    vfile(
                                        _("rzfile: bps rate %ld below min %ld"),
                                        last_bps, min_bps);
                                    DO_SYSLOG_FNAME(
                                        (LOG_INFO,
                                         "%s/%s: bps rate low: %ld < %ld",
                                         shortname, protname(), last_bps,
                                         min_bps));
                                    return ERROR;
                                }
                            }
                    ...
                    if (Verbose > 1)
                    {
                        vstringf(_("\rBytes received: %7ld/%7ld   BPS:%-6ld "
                                   "ETA %02d:%02d  "),
                                 (long)zi->bytes_received,
                                 (long)zi->bytes_total, last_bps, minleft,
                                 secleft);
                ...
                switch (
                    c = zrdata(secbuf + chinseg, MAX_BLOCK, &bytes_in_block))
#else
                switch (c = zrdata(secbuf, MAX_BLOCK, &bytes_in_block))
#endif
                {
...
static int closeit(struct zm_fileinfo* zi)
{
    ...
    if (in_tcpsync)
    {
        ...
        {
            error(1, errno,
                  _("fgets for tcp protocol synchronization failed: "));
        }
        fclose(fout);
        return OK;
    }
