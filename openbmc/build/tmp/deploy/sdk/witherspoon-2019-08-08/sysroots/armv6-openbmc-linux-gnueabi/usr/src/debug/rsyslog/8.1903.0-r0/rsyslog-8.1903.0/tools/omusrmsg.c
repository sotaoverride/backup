...
#else
#define UNAMESZ                                                                \
    32 /* length of a login name, 32 seems current (2018) good bet */
#endif
            ...
            BEGINcreateWrkrInstance CODESTARTcreateWrkrInstance
                ENDcreateWrkrInstance
                    ...
                    BEGINisCompatibleWithFeature
                        CODESTARTisCompatibleWithFeature if (
                            eFeat == sFEATURERepeatedMsgReduction) iRet =
                            RS_RET_OK;
...
ENDfreeInstance
    ...
    BEGINfreeWrkrInstance CODESTARTfreeWrkrInstance ENDfreeWrkrInstance
        ...
        BEGINdbgPrintInstInfo register int i;
...
void endutent(void)
{
     ...
     */
    static rsRetVal
    wallmsg(uchar* pMsg, instanceData* pData)
{
    ...
    while ((uptr = getutent()))
    {
         ...
         * wait indefinitely. So we need to use non-blocking I/O. In case we
         * would block, we simply do not send the message, because that's the
         * best we can do. -- rgerhards, 2008-07-04
         */
...
BEGINtryResume CODESTARTtryResume ENDtryResume
    ...
    BEGINdoAction CODESTARTdoAction dbgprintf("\n");
    ...
    populateUsers(instanceData* pData, es_str_t* usrs)
{
    ...
    {
        for (iDst = 0; iDst < UNAMESZ && iUsr < len && c[iUsr] != ',';
             ++iDst, ++iUsr)
        {
...
{
    CHKiRet(OMSRsetEntry(*ppOMSR, 0, (uchar*)strdup((char*)pData->tplName),
                         OMSR_NO_RQD_TPL_OPTS));
}
CODE_STD_FINALIZERnewActInst cnfparamvalsDestruct(pvals, &actpblk);
ENDnewActInst

    BEGINparseSelectorAct es_str_t* usrs;
int bHadWarning;
...
else
{
    ...
    es_deleteStr(usrs);
    if ((iRet = cflineParseTemplateName(&p, *ppOMSR, 0, OMSR_NO_RQD_TPL_OPTS,
                                        (uchar*)" StdUsrMsgFmt")) != RS_RET_OK)
        goto finalize_it;
}
        ...
        BEGINqueryEtryPt CODESTARTqueryEtryPt CODEqueryEtryPt_STD_OMOD_QUERIES
            CODEqueryEtryPt_STD_OMOD8_QUERIES
                CODEqueryEtryPt_STD_CONF2_OMOD_QUERIES ENDqueryEtryPt
                    ...
                    BEGINmodInit(UsrMsg)
                        CODESTARTmodInit INITLegCnfVars* ipIFVersProvided =
