...
static void* _get(netsnmp_container* c, const void* key, int exact)
{
 ...
 *
 **********************************************************************/
static int _ssll_free(netsnmp_container* c)
{
    if (c)
    {
        free(c);
...
static int _ssll_insert(netsnmp_container* c, const void* data)
{
    sl_container* sl = (sl_container*)c;
...
static int _ssll_remove(netsnmp_container* c, const void* data)
{
    sl_container* sl = (sl_container*)c;
    sl_node* curr;
     ...
     */
    if ((NULL == data) || (sl->c.compare(sl->head->data, data) == 0))
    {
        curr = sl->head;
...
static size_t _ssll_size(netsnmp_container* c)
{
    sl_container* sl = (sl_container*)c;
...
static void _ssll_for_each(netsnmp_container* c, netsnmp_container_obj_func* f,
                           void* context)
{
    sl_container* sl = (sl_container*)c;
    sl_node* curr;
...
static void _ssll_clear(netsnmp_container* c, netsnmp_container_obj_func* f,
                        void* context)
{
    sl_container* sl = (sl_container*)c;
    sl_node *curr, *next;
...
netsnmp_container* netsnmp_container_get_ssll(void)
{
    ...
    netsnmp_init_container((netsnmp_container*)sl, NULL, _ssll_free, _ssll_size,
                           NULL, _ssll_insert, _ssll_remove, _ssll_find);
...
netsnmp_factory* netsnmp_container_get_ssll_factory(void)
{
    static netsnmp_factory f = {
        "sorted_singly_linked_list",
        (netsnmp_factory_produce_f*)netsnmp_container_get_ssll};
...
netsnmp_factory* netsnmp_container_get_usll_factory(void)
{
    static netsnmp_factory f = {
        "unsorted_singly_linked_list-lifo",
        (netsnmp_factory_produce_f*)netsnmp_container_get_usll};

    return &f;
}

netsnmp_factory* netsnmp_container_get_fifo_factory(void)
{
    static netsnmp_factory f = {
        "unsorted_singly_linked_list-fifo",
        (netsnmp_factory_produce_f*)netsnmp_container_get_fifo};
...
void netsnmp_container_ssll_init(void)
{
    ...
    netsnmp_container_register("lifo", netsnmp_container_get_usll_factory());
    netsnmp_container_register("fifo", netsnmp_container_get_fifo_factory());
}
 ...
 *
 */
NETSNMP_STATIC_INLINE sl_container* _ssll_it2cont(ssll_iterator* it)
{
    if (NULL == it)
    {
...
static void* _ssll_iterator_curr(ssll_iterator* it)
{
...
static void* _ssll_iterator_first(ssll_iterator* it)
{
    sl_container* t = _ssll_it2cont(it);
...
static void* _ssll_iterator_next(ssll_iterator* it)
{
    sl_container* t = _ssll_it2cont(it);
    if ((NULL == t) || (NULL == it->pos))
        return NULL;
...
static void* _ssll_iterator_last(ssll_iterator* it)
{
    sl_node* n;
    sl_container* t = _ssll_it2cont(it);
    if (NULL == t)
...
static int _ssll_iterator_reset(ssll_iterator* it)
{
    sl_container* t;
...
static int _ssll_iterator_release(netsnmp_iterator* it)
{
    free(it);
