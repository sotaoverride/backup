...
inline boost::intmax_t umax(mpl::false_ const&)
{
    ...
    return std::numeric_limits<boost::intmax_t>::is_specialized
...
{
    return umax(mpl::bool_<std::numeric_limits<boost::intmax_t>::digits >=
...
class basic_regex_parser : public basic_regex_creator<charT, traits>
{
  public:
    ...
    digraph<charT>
        get_next_set_literal(basic_char_set<charT, traits>& char_set);
  ...
  private:
    ...
    std::ptrdiff_t m_paren_start;      // where the last seen ')' began (where
                                       // repeats are inserted).
    std::ptrdiff_t m_alt_insert_point; // where to insert the next alternative
    bool m_has_case_change; // true if somewhere in the current block the case
                            // has changed
    unsigned m_recursion_count; // How many times we've called parse_all.
#if defined(BOOST_MSVC) && defined(_M_IX86)
    // This is an ugly warning suppression workaround (for warnings *inside*
    // std::vector that can not otherwise be suppressed)...
    ...
    std::vector<std::ptrdiff_t>
        m_alt_jumps; // list of alternative in the current scope.
#endif
...
template <class charT, class traits>
basic_regex_parser<charT, traits>::basic_regex_parser(
    regex_data<charT, traits>* data) :
    basic_regex_creator<charT, traits>(data),
    m_mark_count(0), m_mark_reset(-1), m_max_mark(0), m_paren_start(0),
    m_alt_insert_point(0), m_has_case_change(false), m_recursion_count(0)
{
}

template <class charT, class traits>
void basic_regex_parser<charT, traits>::parse(const charT* p1, const charT* p2,
                                              unsigned l_flags)
{
    ...
    if ((p1 == p2) && (((l_flags & regbase::main_option_type) !=
                        regbase::perl_syntax_group) ||
    ...
    switch (l_flags & regbase::main_option_type)
    {
        case regbase::perl_syntax_group:
        {
            ...
            //
            re_brace* br = static_cast<re_brace*>(
                this->append_state(syntax_element_startmark, sizeof(re_brace)));
            br->index = 0;
        ...
        case regbase::literal:
            ...
            break;
        default:
            // Ooops, someone has managed to set more than one of the main
            // option flags, so this must be an error:
            fail(regex_constants::error_unknown, 0,
                 "An invalid combination of regular expression syntax flags "
                 "was used.");
            return;
    }
    ...
    if (!result)
    {
        fail(regex_constants::error_paren,
             ::boost::BOOST_REGEX_DETAIL_NS::distance(m_base, m_position),
...
void basic_regex_parser<charT, traits>::fail(
    regex_constants::error_type error_code, std::ptrdiff_t position)
{
...
{
    if (0 ==
        this->m_pdata->m_status) // update the error code if not already set
    ...
    if (start_pos == position)
        start_pos = (std::max)(static_cast<std::ptrdiff_t>(0),
                               position - static_cast<std::ptrdiff_t>(10));
    std::ptrdiff_t end_pos =
        (std::min)(position + static_cast<std::ptrdiff_t>(10),
                   static_cast<std::ptrdiff_t>(m_end - m_base));
    if (error_code != regex_constants::error_empty)
    {
        if ((start_pos != 0) || (end_pos != (m_end - m_base)))
            message += "  The error occurred while parsing the regular "
                       "expression fragment: '";
        else
            message +=
...
bool basic_regex_parser<charT, traits>::parse_all()
{
    if (++m_recursion_count > 400)
    {
        ...
        fail(boost::regex_constants::error_complexity, m_position - m_base,
...
bool basic_regex_parser<charT, traits>::parse_basic()
{
    switch (this->m_traits.syntax_type(*m_position))
    {
        ...
        case regex_constants::syntax_star:
            if (!(this->m_last_state) ||
                (this->m_last_state->type == syntax_element_start_line))
...
bool basic_regex_parser<charT, traits>::parse_extended()
{
    ...
    switch (this->m_traits.syntax_type(*m_position))
    {
        ...
        case regex_constants::syntax_caret:
            ++m_position;
            this->append_state((this->flags() & regex_constants::no_mod_m
        ...
        case regex_constants::syntax_star:
            if (m_position == this->m_base)
            {
                fail(regex_constants::error_badrepeat, 0,
                     "The repeat operator \"*\" cannot start a regular "
                     "expression.");
                return false;
            }
            ++m_position;
        ...
        case regex_constants::syntax_close_brace:
            if ((this->flags() & regbase::no_perl_ex) == regbase::no_perl_ex)
            {
                fail(regex_constants::error_brace,
                     ...
                     "Found a closing repetition operator } with no "
                     "corresponding {.");
                return false;
            }
        ...
        case regex_constants::syntax_hash:
            ...
            //
            if ((this->flags() & (regbase::no_perl_ex | regbase::mod_x)) ==
                regbase::mod_x)
            {
...
bool basic_regex_parser<charT, traits>::parse_literal()
{
    ...
    if (((this->flags() & (regbase::main_option_type | regbase::mod_x |
...
bool basic_regex_parser<charT, traits>::parse_open_paren()
{
    ...
    {
        if (this->m_traits.syntax_type(*m_position) ==
    ...
    if (0 == (this->flags() & regbase::nosubs))
    {
        ...
        if (this->flags() & regbase::save_subexpression_location)
            this->m_pdata->m_subs.push_back(std::pair<std::size_t, std::size_t>(
                std::distance(m_base, m_position) - 1, 0));
#else
        if (this->flags() & regbase::save_subexpression_location)
            this->m_pdata->m_subs.push_back(std::pair<std::size_t, std::size_t>(
                (m_position - m_base) - 1, 0));
#endif
    }
    ...
    if (m_position == m_end)
    {
        this->fail(regex_constants::error_paren,
                   ::boost::BOOST_REGEX_DETAIL_NS::distance(m_base, m_end));
        return false;
    }
    if (this->m_traits.syntax_type(*m_position) !=
    ...
    if (markid && (this->flags() & regbase::save_subexpression_location))
        this->m_pdata->m_subs.at(markid - 1).second =
    ...
    //
    pb = static_cast<re_brace*>(
        this->append_state(syntax_element_endmark, sizeof(re_brace)));
    pb->index = markid;
    pb->icase = this->flags() & regbase::icase;
                ...
                switch (*m_position)
                {
                    ...
                    case 'C':
                        ...
                        fail(regex_constants::error_escape, m_position - m_base,
                             "The \\c and \\C escape sequences are not "
                             "supported by POSIX basic regular expressions: "
...
bool basic_regex_parser<charT, traits>::parse_extended_escape()
{
    ...
    switch (this->m_traits.escape_syntax_type(*m_position))
    {
        ...
        escape_type_class_jump:
            ...
            m_type m =
                this->m_traits.lookup_classname(m_position, m_position + 1);
            if (m != 0)
            {
                basic_char_set<charT, traits> char_set;
                if (negate)
        ...
        case regex_constants::escape_type_property:
        {
            ...
            fail(regex_constants::error_ctype, m_position - m_base,
                 "Escape sequence was neither a valid property nor a valid "
                 "character class name.");
            return false;
        }
        case regex_constants::escape_type_reset_start_mark:
            if (0 == (this->flags() &
                      (regbase::main_option_type | regbase::no_perl_ex)))
            {
                re_brace* pb = static_cast<re_brace*>(this->append_state(
                    syntax_element_startmark, sizeof(re_brace)));
                pb->index = -5;
                pb->icase = this->flags() & regbase::icase;
                this->m_pdata->m_data.align();
                ++m_position;
        ...
        case regex_constants::escape_type_extended_backref:
                      ...
                      (regbase::main_option_type | regbase::no_perl_ex)))
            {
                ...
                // maybe have \g{ddd}
                regex_constants::syntax_type syn =
                    this->m_traits.syntax_type(*m_position);
                ...
                if ((i < 0) && syn_end)
                {
                    // Check for a named capture, get the leftmost one if there
                    // is more than one:
                    const charT* base = m_position;
                    while ((m_position != m_end) &&
                           (this->m_traits.syntax_type(*m_position) != syn_end))
                    {
                        ++m_position;
                    }
                ...
                if (negative)
                    i = 1 + m_mark_count - i;
                if (((i > 0) && (i < std::numeric_limits<unsigned>::digits) &&
                     (i - 1 < static_cast<boost::intmax_t>(sizeof(unsigned) *
                                                           CHAR_BIT)) &&
                     (this->m_backrefs & (1u << (i - 1)))) ||
                    ((i > 10000) && (this->m_pdata->get_id(i) > 0) &&
                     (this->m_pdata->get_id(i) - 1 <
                      static_cast<boost::intmax_t>(sizeof(unsigned) *
                                                   CHAR_BIT)) &&
                     (this->m_backrefs &
...
bool basic_regex_parser<charT, traits>::parse_match_any()
{
    ...
    static_cast<re_dot*>(
        ...
        this->flags() & regbase::no_mod_s
            ? BOOST_REGEX_DETAIL_NS::force_not_newline
            : this->flags() & regbase::mod_s
                  ? BOOST_REGEX_DETAIL_NS::force_newline
                  : BOOST_REGEX_DETAIL_NS::dont_care);
    return true;
}

template <class charT, class traits>
bool basic_regex_parser<charT, traits>::parse_repeat(std::size_t low,
                                                     std::size_t high)
{
    bool greedy = true;
          ...
          (this->flags() & (regbase::main_option_type | regbase::emacs_ex)))))
    {
        ...
        if ((m_position != m_end) && (this->m_traits.syntax_type(*m_position) ==
                                      regex_constants::syntax_question))
        {
            greedy = false;
            ++m_position;
    ...
    else if ((this->m_last_state->type == syntax_element_literal) &&
             (static_cast<re_literal*>(this->m_last_state)->length > 1))
    {
        // the last state was a literal with more than one character, split it
        // in two:
        re_literal* lit = static_cast<re_literal*>(this->m_last_state);
        charT c =
            (static_cast<charT*>(static_cast<void*>(lit + 1)))[lit->length - 1];
        lit->length -= 1;
        // now append new state:
        lit = static_cast<re_literal*>(this->append_state(
            syntax_element_literal, sizeof(re_literal) + sizeof(charT)));
        lit->length = 1;
    ...
    {
        // repeat the last state whatever it was, need to add some error
        // checking here:
    ...
    //
    re_repeat* rep = static_cast<re_repeat*>(
        this->insert_state(insert_point, syntax_element_rep, re_repeater_size));
    rep->min = low;
    rep->max = high;
    rep->greedy = greedy;
    rep->leading = false;
    ...
    re_jump* jmp = static_cast<re_jump*>(
        this->append_state(syntax_element_jump, sizeof(re_jump)));
    ...
    if (pocessive)
    {
        ...
        {
            //
            // Check for illegal following quantifier, we have to do this here,
            // because the extra states we insert below circumvents our usual
            // error checking :-(
            //
            bool contin = false;
            do
            {
                if ((this->flags() & (regbase::main_option_type |
                                      regbase::mod_x | regbase::no_perl_ex)) ==
                    regbase::mod_x)
                {
                    // whitespace skip:
                    while (
                    ...
                    switch (this->m_traits.syntax_type(*m_position))
                    {
                        ...
                        case regex_constants::syntax_open_mark:
                            ...
                            if ((m_position + 2 < m_end) &&
                                this->m_traits.syntax_type(*(m_position + 1)) ==
                                    regex_constants::syntax_question &&
                                this->m_traits.syntax_type(*(m_position + 2)) ==
                                    regex_constants::syntax_hash)
                            {
                                while ((m_position != m_end) &&
                                       (this->m_traits.syntax_type(
                                            *m_position++) !=
                                        regex_constants::syntax_close_mark))
                                {
                                }
        ...
        re_brace* pb = static_cast<re_brace*>(this->insert_state(
            insert_point, syntax_element_startmark, sizeof(re_brace)));
        pb->index = -3;
        pb->icase = this->flags() & regbase::icase;
        jmp = static_cast<re_jump*>(
            this->insert_state(insert_point + sizeof(re_brace),
                               syntax_element_jump, sizeof(re_jump)));
        this->m_pdata->m_data.align();
...
bool basic_regex_parser<charT, traits>::parse_repeat_range(bool isbasic)
{
    ...
    if (this->m_position == this->m_end)
    {
        ...
        // Treat the opening '{' as a literal character, rewind to start of
        // error:
        --m_position;
        while (this->m_traits.syntax_type(*m_position) !=
               regex_constants::syntax_open_brace)
            --m_position;
        return parse_literal();
    }
        ...
        regex_constants::syntax_comma)
    {
        ...
        if (this->m_position == this->m_end)
        {
            ...
            {
                fail(regex_constants::error_brace,
                     this->m_position - this->m_base, incomplete_message);
                return false;
            }
            // Treat the opening '{' as a literal character, rewind to start of
            // error:
...
bool basic_regex_parser<charT, traits>::parse_alt()
{
    ...
    //
    if (((this->m_last_state == 0) ||
         (this->m_last_state->type == syntax_element_startmark)) &&
        !(((this->flags() & regbase::main_option_type) ==
           regbase::perl_syntax_group) &&
    ...
    //
    re_syntax_base* pj = this->append_state(
        BOOST_REGEX_DETAIL_NS::syntax_element_jump, sizeof(re_jump));
    std::ptrdiff_t jump_offset = this->getoffset(pj);
    //
    // now insert the alternative:
    //
    re_alt* palt = static_cast<re_alt*>(this->insert_state(
        this->m_alt_insert_point, syntax_element_alt, re_alt_size));
...
{
    static const char* incomplete_message =
        "Character set declaration starting with [ terminated prematurely - "
        "either no ] was found or the set had no content.";
    ++m_position;
    if (m_position == m_end)
    {
...
template <class charT, class traits>
bool basic_regex_parser<charT, traits>::parse_inner_set(
    basic_char_set<charT, traits>& char_set)
{
    static const char* incomplete_message =
        "Character class declaration starting with [ terminated prematurely - "
        "either no ] was found or the set had no content.";
    //
    ...
    switch (this->m_traits.syntax_type(*m_position))
    {
        ...
        case regex_constants::syntax_colon:
        {
            ...
            if ((this->flags() &
                 (regbase::main_option_type | regbase::no_char_classes)) ==
        ...
        case regex_constants::syntax_equal:
        {
            ...
            string_type m =
                this->m_traits.lookup_collatename(name_first, name_last);
...
template <class charT, class traits>
void basic_regex_parser<charT, traits>::parse_set_literal(
    basic_char_set<charT, traits>& char_set)
{
...
{
    return valid_value(c, v,
                       mpl::bool_<(sizeof(charT) < sizeof(boost::intmax_t))>());
}
...
#pragma warning(disable : 4127)
    ...
    switch (this->m_traits.escape_syntax_type(*m_position))
    {
        ...
        case regex_constants::escape_type_hex:
                ...
                regex_constants::syntax_open_brace)
            {
                ...
                if ((m_position == m_end) || (i < 0) ||
                    ((std::numeric_limits<charT>::is_specialized) &&
                     (i >
                      (boost::intmax_t)(std::numeric_limits<charT>::max)())) ||
                    (this->m_traits.syntax_type(*m_position) !=
                     regex_constants::syntax_close_brace))
                {
                    // Rewind to start of escape:
            ...
            {
                std::ptrdiff_t len =
                    (std::min)(static_cast<std::ptrdiff_t>(2),
                               static_cast<std::ptrdiff_t>(m_end - m_position));
                boost::intmax_t i =
                    this->m_traits.toi(m_position, m_position + len, 16);
        ...
        case regex_constants::syntax_digit:
        {
            ...
            std::ptrdiff_t len = (std::min)(
                ::boost::BOOST_REGEX_DETAIL_NS::distance(m_position, m_end),
            ...
            if ((val < 0) ||
                (val > (boost::intmax_t)(std::numeric_limits<charT>::max)()))
            {
                // Rewind to start of escape:
                --m_position;
                while (this->m_traits.syntax_type(*m_position) !=
        ...
        case regex_constants::escape_type_named_char:
        {
                ...
                regex_constants::syntax_open_brace)
            {
                ...
                string_type s =
                    this->m_traits.lookup_collatename(++base, m_position++);
                if (s.empty())
                {
                    // Rewind to start of escape:
                    --m_position;
...
bool basic_regex_parser<charT, traits>::parse_backref()
{
    ...
    if ((i == 0) || (((this->flags() & regbase::main_option_type) ==
                      regbase::perl_syntax_group) &&
    ...
    else if ((i > 0) && (this->m_backrefs & (1u << (i - 1))))
    {
        m_position = pc;
        re_brace* pb = static_cast<re_brace*>(
            this->append_state(syntax_element_backref, sizeof(re_brace)));
        pb->index = i;
        pb->icase = this->flags() & regbase::icase;
    }
    else
...
bool basic_regex_parser<charT, traits>::parse_perl_extension()
{
    ...
    switch (this->m_traits.syntax_type(*m_position))
    {
        ...
        case regex_constants::syntax_digit:
        {
            ...
            //
            v = this->m_traits.toi(m_position, m_end, 10);
            if ((v < 0) || (this->m_traits.syntax_type(*m_position) !=
                            regex_constants::syntax_close_mark))
            {
                // Rewind to start of (? sequence:
                --m_position;
                while (this->m_traits.syntax_type(*m_position) !=
                    ...
                    --m_position;
                fail(regex_constants::error_perl_extension, m_position - m_base,
                     "The recursive sub-expression refers to an invalid "
                     "marking group, or is unterminated.");
                return false;
            }
        insert_recursion:
            pb->index = markid = 0;
            re_recurse* pr = static_cast<re_recurse*>(
                this->append_state(syntax_element_recurse, sizeof(re_recurse)));
            pr->alt.i = v;
            pr->state_id = 0;
            static_cast<re_case*>(
                this->append_state(syntax_element_toggle_case, sizeof(re_case)))
        ...
        case regex_constants::syntax_plus:
            ...
            if ((std::numeric_limits<boost::intmax_t>::max)() - m_mark_count <
                v)
            {
                fail(regex_constants::error_perl_extension, m_position - m_base,
                     "An invalid or unterminated recursive sub-expression.");
        ...
        case regex_constants::escape_type_left_word:
        {
            ...
            if (++m_position == m_end)
            {
                ...
                while (this->m_traits.syntax_type(*m_position) !=
                       regex_constants::syntax_open_mark)
                    --m_position;
                fail(regex_constants::error_perl_extension,
                     m_position - m_base);
                return false;
            }
            regex_constants::syntax_type t =
                this->m_traits.syntax_type(*m_position);
        ...
        case regex_constants::syntax_open_mark:
        {
            ...
            if (*m_position == charT('R'))
            {
                ...
                if (*m_position == charT('&'))
                {
                    ...
                    v = -static_cast<int>(
                        hash_value_from_capture_name(base, m_position));
                }
                else
                {
                    v = -this->m_traits.toi(m_position, m_end, 10);
                ...
                re_brace* br = static_cast<re_brace*>(this->append_state(
                    syntax_element_assert_backref, sizeof(re_brace)));
                br->index = v < 0 ? (v - 1) : 0;
                if (this->m_traits.syntax_type(*m_position) !=
                    regex_constants::syntax_close_mark)
                {
            ...
            else if ((*m_position == charT('\'')) ||
                     (*m_position == charT('<')))
            {
                const charT* base = ++m_position;
                while ((m_position != m_end) && (*m_position != charT('>')) &&
                       (*m_position != charT('\'')))
                    ++m_position;
                if (m_position == m_end)
                {
                    ...
                    (++m_position == m_end))
                {
                    ...
                    while (this->m_traits.syntax_type(*m_position) !=
                        ...
                        --m_position;
                    fail(regex_constants::error_perl_extension,
                         m_position - m_base, "Unterminated named capture.");
                    return false;
                }
                if (this->m_traits.syntax_type(*m_position) !=
                    regex_constants::syntax_close_mark)
            ...
            else if (*m_position == charT('D'))
            {
                ...
                while (*def && (m_position != m_end) &&
                       (*m_position == charT(*def)))
                    ++m_position, ++def;
                ...
                if (this->flags() & regbase::save_subexpression_location)
                    this->m_pdata->m_subs.push_back(
                        std::pair<std::size_t, std::size_t>(
                            std::distance(m_base, m_position) - 2, 0));
#else
                if (this->flags() & regbase::save_subexpression_location)
                    this->m_pdata->m_subs.push_back(
                        std::pair<std::size_t, std::size_t>(
                            (m_position - m_base) - 2, 0));
#endif
            }
            pb->index = markid;
            const charT* base = ++m_position;
        ...
        option_group_jump:
            ...
            m_has_case_change =
                ((opts & regbase::icase) != (this->flags() & regbase::icase));
            pb->index = markid = 0;
            if (this->m_traits.syntax_type(*m_position) ==
                regex_constants::syntax_close_mark)
            {
                ...
                restore_flags = false;
                old_case_change |=
                    m_has_case_change; // defer end of scope by one ')'
            }
            else if (this->m_traits.syntax_type(*m_position) ==
                     regex_constants::syntax_colon)
            {
            ...
            {
                static_cast<re_case*>(
                    this->append_state(syntax_element_toggle_case,
                                       sizeof(re_case)))
                    ->icase = opts & regbase::icase;
            }
    }
    ...
    BOOST_ASSERT(this->m_traits.syntax_type(*m_position) ==
                 regex_constants::syntax_close_mark);
    ++m_position;
    //
    // restore the flags:
    ...
    if (markid == -4)
    {
        ...
        {
            re_alt* alt = static_cast<re_alt*>(this->insert_state(
                expected_alt_point, syntax_element_alt, sizeof(re_alt)));
            alt->alt.i = this->m_pdata->m_data.size() - this->getoffset(alt);
        }
        else if (((std::ptrdiff_t)this->m_pdata->m_data.size() >
                  (static_cast<re_alt*>(b)->alt.i + this->getoffset(b))) &&
                 (static_cast<re_alt*>(b)->alt.i > 0) &&
                 this->getaddress(static_cast<re_alt*>(b)->alt.i, b)->type ==
                     syntax_element_alt)
        {
            ...
            fail(regex_constants::error_bad_pattern, m_position - m_base,
                 "More than one alternation operator | was encountered inside "
                 "a conditional expression.");
            return false;
        }
        else
        {
            ...
            if ((b->type == syntax_element_assert_backref) &&
                (static_cast<re_brace*>(b)->index == 9999))
            {
                // Rewind to start of (? sequence:
                --m_position;
                while (this->m_traits.syntax_type(*m_position) !=
                    ...
                    --m_position;
                fail(regex_constants::error_bad_pattern, m_position - m_base,
                     "Alternation operators are not allowed inside a DEFINE "
                     "block.");
                return false;
            }
        }
            ...
            (b->type != syntax_element_startmark))
        {
            ...
            while (this->m_traits.syntax_type(*m_position) !=
                ...
                --m_position;
            fail(regex_constants::error_badrepeat, m_position - m_base,
                 "A repetition operator cannot be applied to a zero-width "
                 "assertion.");
            return false;
        }
    }
    ...
    if (markid > 0)
    {
        ...
        if (this->flags() & regbase::save_subexpression_location)
            this->m_pdata->m_subs.at(markid - 1).second =
                std::distance(m_base, m_position) - 1;
#else
        if (this->flags() & regbase::save_subexpression_location)
            this->m_pdata->m_subs.at(markid - 1).second =
                (m_position - m_base) - 1;
    ...
    switch (*m_position)
    {
        ...
        case 'C':
            ...
            if (match_verb("OMMIT"))
            {
                ...
                ++m_position;
                static_cast<re_commit*>(
                    this->append_state(syntax_element_commit,
                                       sizeof(re_commit)))
                    ->action = commit_commit;
...
bool basic_regex_parser<charT, traits>::add_emacs_code(bool negate)
{
    ...
    switch (*m_position)
    {
        ...
        case '.':
            char_set.add_class(
                this->m_traits.lookup_classname(s_punct, s_punct + 5));
            break;
        case '(':
            char_set.add_single(digraph<charT>(charT('(')));
            char_set.add_single(digraph<charT>(charT('[')));
...
regex_constants::syntax_option_type
    basic_regex_parser<charT, traits>::parse_options()
{
...
template <class charT, class traits>
bool basic_regex_parser<charT, traits>::unwind_alts(
    std::ptrdiff_t last_paren_start)
{
    //
    // If we didn't actually add any states after the last
    // alternative then that's an error:
    //
    if ((this->m_alt_insert_point ==
         static_cast<std::ptrdiff_t>(this->m_pdata->m_data.size())) &&
    ...
    {
        fail(
            regex_constants::error_empty, this->m_position - this->m_base,
