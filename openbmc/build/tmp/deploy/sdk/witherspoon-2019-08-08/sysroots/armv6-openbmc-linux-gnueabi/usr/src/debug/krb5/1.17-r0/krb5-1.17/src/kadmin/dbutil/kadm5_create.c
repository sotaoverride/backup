...
int kadm5_create(kadm5_config_params* params)
{
    ...
    if (retval)
    {
        com_err(progname, retval,
                _("while looking up the Kerberos "
                  "configuration"));
        return 1;
    }
...
int kadm5_create_magic_princs(kadm5_config_params* params, krb5_context context)
{
    int retval;
    void* handle;
    ...
    if ((retval = kadm5_init(context, progname, NULL, NULL, params,
                             ...
                             db5util_db_args, &handle)))
    {
        com_err(progname, retval,
                _("while initializing the Kerberos admin "
                  "interface"));
        return retval;
    }
...
static int add_admin_princs(void* handle, krb5_context context, char* realm)
{
    ...
    if ((ret = add_admin_princ(handle, context, service_name, realm,
                               KRB5_KDB_DISALLOW_TGT_BASED |
                                   KRB5_KDB_LOCKDOWN_KEYS,
                               ADMIN_LIFETIME)))
        goto clean_and_exit;

    if ((ret = add_admin_princ(handle, context, KADM5_ADMIN_SERVICE, realm,
                               KRB5_KDB_DISALLOW_TGT_BASED |
                                   KRB5_KDB_LOCKDOWN_KEYS,
                               ADMIN_LIFETIME)))
        goto clean_and_exit;

    if ((ret = add_admin_princ(handle, context, KADM5_CHANGEPW_SERVICE, realm,
                               KRB5_KDB_DISALLOW_TGT_BASED |
...
int add_admin_princ(void* handle, krb5_context context, char* name, char* realm,
                    int attrs, int lifetime)
{
    char* fullname;
    krb5_error_code ret;
