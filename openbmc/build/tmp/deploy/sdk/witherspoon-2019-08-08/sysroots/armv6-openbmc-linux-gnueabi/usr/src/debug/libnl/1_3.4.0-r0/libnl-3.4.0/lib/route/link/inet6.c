...
static const uint8_t map_stat_id_from_IPSTATS_MIB_v1[__IPSTATS_MIB_MAX] = {
     ...
     * version before the API change in commit
     * 14a196807482e6fc74f15fc03176d5c08880588f. This version was valid since
     * commit edf391ff17232f097d72441c9ad467bcb3b5db18, which predates support
     * for parsing IFLA_PROTINFO in libnl3. Such an even older meaning of the
     * flags is not supported in libnl3. */
...
static const uint8_t map_stat_id_from_IPSTATS_MIB_v2[__IPSTATS_MIB_MAX] = {
     ...
     * version since the API change in commit
                                ...
                                void* data)
{
    ...
    if (tb[IFLA_INET6_CONF])
        nla_memcpy(&i6->i6_conf, tb[IFLA_INET6_CONF], sizeof(i6->i6_conf));
    ...
    if (tb[IFLA_INET6_STATS])
    {
        ...
        if (len < 32 || (tb[IFLA_INET6_ICMP6STATS] &&
                         nla_len(tb[IFLA_INET6_ICMP6STATS]) < 6))
        {
            /* kernel commit 14a196807482e6fc74f15fc03176d5c08880588f reordered
             * the values. The later commit
             * 6a5dc9e598fe90160fee7de098fa319665f5253e added values
             * IPSTATS_MIB_CSUMERRORS/ICMP6_MIB_CSUMERRORS. If the netlink is
             * shorter then this, assume that the kernel uses the previous
    ...
    if (tb[IFLA_INET6_ICMP6STATS])
    {
        ...
        int i;
        int len =
            min_t(int, __ICMP6_MIB_MAX, nla_len(tb[IFLA_INET6_ICMP6STATS]) / 8);

        for (i = 1; i < len; i++)
        {
            memcpy(&stat, &cnt[i * sizeof(stat)], sizeof(stat));
            rtnl_link_set_stat(link, RTNL_LINK_ICMP6_INMSGS + i - 1, stat);
        }
    }
...
{
    return __flags2str(flags, buf, len, inet6_flags, ARRAY_SIZE(inet6_flags));
}
...
{
    return __type2str(type, buf, len, inet6_devconf, ARRAY_SIZE(inet6_devconf));
}
...
static void inet6_dump_details(struct rtnl_link* link, struct nl_dump_params* p,
                               void* data)
{
    struct inet6_data* i6 = data;
    struct nl_addr* addr;
    ...
    nl_dump_line(
        p, "      create-stamp %.2fs reachable-time %s",
    ...
    nl_dump(p, "      link-local address mode %s\n",
            rtnl_link_inet6_addrgenmode2str(i6->i6_addr_gen_mode, buf,
                                            sizeof(buf)));
...
static void inet6_dump_stats(struct rtnl_link* link, struct nl_dump_params* p,
                             void* data)
{
    double octets;
    char* octetsUnit;
...
int rtnl_link_inet6_set_token(struct rtnl_link* link, struct nl_addr* addr)
{
    ...
    memcpy(&id->i6_token, nl_addr_get_binary_addr(addr), sizeof(id->i6_token));
    return 0;
}
