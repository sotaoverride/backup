...
G_DEFINE_TYPE_WITH_CODE(GEmblemedIcon, g_emblemed_icon, G_TYPE_OBJECT,
                        G_ADD_PRIVATE(GEmblemedIcon) G_IMPLEMENT_INTERFACE(
                            G_TYPE_ICON, g_emblemed_icon_icon_iface_init))
...
static void g_emblemed_icon_set_property(GObject* object, guint property_id,
...
static void g_emblemed_icon_get_property(GObject* object, guint property_id,
...
static void g_emblemed_icon_class_init(GEmblemedIconClass* klass)
{
    ...
    properties[PROP_GICON] = g_param_spec_object(
        "gicon", P_("The base GIcon"), P_("The GIcon to attach emblems to"),
        G_TYPE_ICON,
...
GIcon* g_emblemed_icon_new(GIcon* icon, GEmblem* emblem)
{
    ...
    emblemed = G_EMBLEMED_ICON(
        g_object_new(G_TYPE_EMBLEMED_ICON, "gicon", icon, NULL));
...
void g_emblemed_icon_add_emblem(GEmblemedIcon* emblemed, GEmblem* emblem)
{
    ...
    g_object_ref(emblem);
    emblemed->priv->emblems = g_list_insert_sorted(
        emblemed->priv->emblems, emblem, (GCompareFunc)g_emblem_comp);
}
...
static gboolean g_emblemed_icon_to_tokens(GIcon* icon, GPtrArray* tokens,
                                          gint* out_version)
{
...
static GIcon* g_emblemed_icon_from_tokens(gchar** tokens, gint num_tokens,
                                          gint version, GError** error)
{
    ...
    for (n = 1; n < num_tokens; n++)
    {
        ...
        emblemed_icon->priv->emblems =
            g_list_append(emblemed_icon->priv->emblems, emblem);
    }
...
static GVariant* g_emblemed_icon_serialize(GIcon* icon)
{
    GEmblemedIcon* emblemed_icon = G_EMBLEMED_ICON(icon);
    ...
    for (node = emblemed_icon->priv->emblems; node != NULL; node = node->next)
    {
        ...
        if (icon_data)
        {
            ...
            if (g_variant_is_of_type(icon_data, G_VARIANT_TYPE("(sv)")))
            {
                ...
                if (g_str_equal(name, "emblem") &&
                    g_variant_is_of_type(content, G_VARIANT_TYPE("(va{sv})")))
