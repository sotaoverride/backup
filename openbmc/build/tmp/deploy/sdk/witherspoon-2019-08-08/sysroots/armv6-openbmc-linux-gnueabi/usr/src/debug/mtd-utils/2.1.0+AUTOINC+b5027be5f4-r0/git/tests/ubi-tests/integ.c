...
static long long max_ebs_per_vol =
    0; /* max number of ebs per vol (zero => no max) */
...
static int write_to_erase_block(struct erase_block_info* erase_block, int fd)
{
    ...
    if (erase_block->writes)
        next_offset = erase_block->writes->offset_within_block +
                      erase_block->writes->size;
...
static void operate_on_open_volume(struct volume_fd* vol_fd)
{
    ...
    */
    ...
    else
    {
        ...
        operate_on_erase_block(&vol_fd->volume->erase_blocks[eb_no],
                               vol_fd->fd);
    }
}
...
static void operate_on_ubi_device(struct ubi_device_info* ubi_device)
{
    ...
    */
    if (ubi_device->info.vol_count == 0)
    {
        ...
        if (ubi_get_vol_info1(libubi, ubi_device->info.dev_num, req.vol_id,
                              &s->info))
        ...
        if (fd == -1)
        {
            ...
            sprintf(dev_name, "mknod %s c %d %d", s->device_file_name, maj,
                    req.vol_id + 1);
            if (system(dev_name))
...
static void get_ubi_devices_info(void)
{
    ...
    for (i = info.lowest_dev_num; i <= info.highest_dev_num; ++i)
    {
        ...
        if (max_ebs_per_vol &&
            s->info.leb_size * max_ebs_per_vol < s->info.avail_bytes)
...
static int get_short_arg(int* pos, const char* name, long long* result,
                         int argc, char* argv[])
{
    const char* p = NULL;
    int i = *pos;
...
static int get_long_arg(int* pos, const char* name, long long* result, int argc,
                        char* argv[])
{
    const char* p = NULL;
    int i = *pos;
...
static int remove_all_volumes(void)
{
    ...
    for (i = 0; i < info.dev_count; ++i)
    {
        ...
        while (vol)
        {
            int res = ubi_rmvol(libubi, ubi_device->device_file_name,
                                vol->info.vol_id);
            if (res)
                return res;
            vol = vol->next;
...
int main(int argc, char* argv[])
{
    ...
    {
        fprintf(stderr,
                "Usage is: ubi_integ [<options>] <UBI Module load command>\n");
        fprintf(stderr, "    Options: \n");
        fprintf(stderr, "        -h, --help              Help\n");
        fprintf(stderr,
                "        -n arg, --repeat=arg    Repeat test arg times\n");
        fprintf(stderr,
                "        -m arg, --maxebs=arg    Max no. of erase blocks\n");
        return 1;
    }
