...
static nss_status_t read_rpcent(TFILE* fp, struct rpcent* result, char* buffer,
...
{
    NSS_GETONE(NSLCD_ACTION_RPC_BYNAME, WRITE_STRING(fp, name),
...
nss_status_t NSS_NAME(getrpcbynumber_r)(int number, struct rpcent* result,
                                        ...
                                        int* errnop)
{
    NSS_GETONE(NSLCD_ACTION_RPC_BYNUMBER, WRITE_INT32(fp, number),
               read_rpcent(fp, result, buffer, buflen, errnop));
}
...
static nss_backend_op_t rpc_ops[] = {nss_ldap_destructor, rpc_endrpcent,
                                     rpc_setrpcent,       rpc_getrpcent,
                                     rpc_getrpcbyname,    rpc_getrpcbynumber};
