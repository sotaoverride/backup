...
static krb5_boolean is_kdc_issued_authdatum(krb5_context context,
...
static krb5_boolean has_kdc_issued_authdata(krb5_context context,
                                            krb5_authdata** authdata,
                                            krb5_authdatatype desired_type)
{
    int i;
...
static krb5_boolean has_mandatory_for_kdc_authdata(krb5_context context,
                                                   krb5_authdata** authdata)
{
    int i;
 ...
 */
static krb5_error_code merge_authdata(krb5_context context,
...
static krb5_error_code copy_request_authdata(krb5_context context,
                                             ...
                                             krb5_authdata*** tkt_authdata)
{
    ...
    {
        ret =
            krb5_c_decrypt(context, client_key, KRB5_KEYUSAGE_TGS_REQ_AD_SUBKEY,
                           0, &req->authorization_data, &plaintext);
    }
    if (ret)
        goto cleanup;
     ...
     * types. */
    ret =
        merge_authdata(context, req->unenc_authdata, tkt_authdata, TRUE, TRUE);
...
static krb5_error_code copy_tgt_authdata(krb5_context context,
                       ...
                       krb5_enc_tkt_part* enc_tkt_reply)
{
    ...
    ret = krb5_db_sign_authdata(
        context, flags, actual_client, client, server, krbtgt, client_key,
        server_key, krbtgt_key, enc_tkt_reply->session,
        enc_tkt_reply->times.authtime, tgt_authdata, &db_authdata);
    if (ret)
...
static krb5_error_code verify_signedpath_checksum(
    krb5_context context, krb5_db_entry* local_tgt,
    krb5_enc_tkt_part* enc_tkt_part, krb5_principal* deleg_path,
    krb5_pa_data** method_data, krb5_checksum* cksum, krb5_boolean* valid_out)
{
    krb5_error_code ret;
    krb5_data* data;
    krb5_key_data* kd;
    ...
    *valid_out = FALSE;
    ...
    ret = make_signedpath_data(
        context, enc_tkt_part->client, enc_tkt_part->times.authtime, deleg_path,
        method_data, enc_tkt_part->authorization_data, &data);
    if (ret)
        return ret;
    ...
    do
    {
        ...
        ret = krb5_c_verify_checksum(
            context, &tgtkey, KRB5_KEYUSAGE_AD_SIGNEDPATH, data, cksum, &valid);
...
static krb5_error_code verify_signedpath(krb5_context context,
                                         krb5_db_entry* local_tgt,
                                         krb5_enc_tkt_part* enc_tkt_part,
                                         krb5_principal** delegated_out,
                                         krb5_boolean* pathsigned_out)
{
    krb5_error_code ret;
...
static krb5_error_code make_signedpath_checksum(
    krb5_context context, krb5_const_principal for_user_princ,
    krb5_db_entry* local_tgt, krb5_enc_tkt_part* enc_tkt_part,
    krb5_principal* deleg_path, krb5_pa_data** method_data,
    krb5_checksum* cksum_out, krb5_enctype* enctype_out)
{
    krb5_error_code ret;
    krb5_data* data = NULL;
    ...
    *enctype_out = ENCTYPE_NULL;
    ...
    ret = krb5_c_make_checksum(context, 0, &tgtkey, KRB5_KEYUSAGE_AD_SIGNEDPATH,
                               data, cksum_out);
...
static krb5_error_code make_signedpath(krb5_context context,
...
static void free_deleg_path(krb5_context context, krb5_principal* deleg_path)
{
    int i;
 ...
 * authorization data. */
static krb5_boolean only_pac_p(krb5_context context, krb5_authdata** authdata)
{
...
static krb5_error_code handle_signticket(
    krb5_context context, unsigned int flags, krb5_db_entry* client,
    krb5_db_entry* server, krb5_db_entry* local_tgt, krb5_kdc_req* req,
    krb5_const_principal for_user_princ, krb5_enc_tkt_part* enc_tkt_req,
    krb5_enc_tkt_part* enc_tkt_reply)
{
...
static krb5_error_code add_auth_indicators(krb5_context context,
...
krb5_error_code handle_authdata(
    ...
    krb5_kdc_req* req, krb5_const_principal for_user_princ,
    krb5_enc_tkt_part* enc_tkt_req, krb5_data* const* auth_indicators,
    krb5_enc_tkt_part* enc_tkt_reply)
{
    kdcauthdata_handle* h;
    krb5_error_code ret = 0;
    ...
    if (!isflagset(enc_tkt_reply->flags, TKT_FLG_ANONYMOUS))
    {
        ...
        ret = handle_signticket(context, flags, client, server, local_tgt, req,
                                for_user_princ, enc_tkt_req, enc_tkt_reply);
        if (ret)
            return ret;
    }
