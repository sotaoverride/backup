...
ASN1_SEQUENCE(NAMING_AUTHORITY) =
    {
        ...
        ASN1_SEQUENCE(PROFESSION_INFO) =
            {
                ASN1_EXP_OPT(PROFESSION_INFO, namingAuthority, NAMING_AUTHORITY,
                             0),
                ASN1_SEQUENCE_OF(PROFESSION_INFO, professionItems,
                                 DIRECTORYSTRING),
                ASN1_SEQUENCE_OF_OPT(PROFESSION_INFO, professionOIDs,
                                     ASN1_OBJECT),
                ASN1_OPT(PROFESSION_INFO, registrationNumber,
                ...
                ASN1_SEQUENCE(ADMISSIONS) =
                    {
                        ASN1_EXP_OPT(ADMISSIONS, admissionAuthority,
                                     GENERAL_NAME, 0),
                        ASN1_EXP_OPT(ADMISSIONS, namingAuthority,
                                     NAMING_AUTHORITY, 1),
                        ASN1_SEQUENCE_OF(ADMISSIONS, professionInfos,
                                         PROFESSION_INFO),
} ASN1_SEQUENCE_END(ADMISSIONS)

                        ASN1_SEQUENCE(ADMISSION_SYNTAX) =
                            {
                                ASN1_OPT(ADMISSION_SYNTAX, admissionAuthority,
                                         GENERAL_NAME),
                                ASN1_SEQUENCE_OF(ADMISSION_SYNTAX,
                                ...
                                BIO* bp, int ind)
{
    ...
    if (namingAuthority->namingAuthorityId != NULL)
    {
        char objbuf[128];
        const char* ln =
            OBJ_nid2ln(OBJ_obj2nid(namingAuthority->namingAuthorityId));

        if (BIO_printf(bp, "%*s  admissionAuthorityId: ", ind, "") <= 0)
            goto err;

        OBJ_obj2txt(objbuf, sizeof(objbuf), namingAuthority->namingAuthorityId,
                    1);
                                ...
                                BIO* bp, int ind)
{
    ...
    {
        ADMISSIONS* entry =
            sk_ADMISSIONS_value(admission->contentsOfAdmissions, i);
        ...
        {
            if (i2r_NAMING_AUTHORITY(method, entry->namingAuthority, bp, ind) <=
                0)
                goto err;
        }
        ...
        {
            PROFESSION_INFO* pinfo =
                sk_PROFESSION_INFO_value(entry->professionInfos, j);
            ...
            {
                if (BIO_printf(bp, "%*s    registrationNumber: ", ind, "") <=
                        0 ||
            ...
            {
                if (i2r_NAMING_AUTHORITY(method, pinfo->namingAuthority, bp,
                                         ind + 2) <= 0)
                    goto err;
            }
            ...
            if (pinfo->professionItems != NULL)
            {
                ...
                {
                    ASN1_STRING* val =
                        sk_ASN1_STRING_value(pinfo->professionItems, k);
            ...
            if (pinfo->professionOIDs != NULL)
            {
                ...
                {
                    ASN1_OBJECT* obj =
                        sk_ASN1_OBJECT_value(pinfo->professionOIDs, k);
