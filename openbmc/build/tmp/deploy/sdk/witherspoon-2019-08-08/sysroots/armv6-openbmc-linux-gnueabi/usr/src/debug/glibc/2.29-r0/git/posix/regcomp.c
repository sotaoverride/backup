            ...
            case ANCHOR:
                switch (dfa->nodes[node].opr.ctx_type)
                {
                    ...
                    case BUF_LAST:
                        break;
                    default:
                        /* Word anchors etc. cannot be handled.  It's okay to
                           test opr.ctx_type since constraints (for all DFA
                           nodes) are
...
static reg_errcode_t duplicate_node_closure(re_dfa_t* dfa, Idx top_org_node,
                      ...
                      reg_syntax_t syntax)
{
    ...
    switch (c)
    {
        ...
        case '^':
            if (!(syntax &
                  (RE_CONTEXT_INDEP_ANCHORS | RE_CARET_ANCHORS_HERE)) &&
                                    ...
                                    Idx nest, reg_errcode_t* err)
{
    ...
    switch (token->type)
    {
        ...
        case OP_BACK_REF:
            if (!__glibc_likely(dfa->completed_bkref_map &
                                (1 << token->opr.idx)))
            {
                *err = REG_ESUBREG;
                return NULL;
           ...
           token->type == OP_DUP_QUESTION || token->type == OP_OPEN_DUP_NUM)
    {
        ...
        if ((syntax & RE_CONTEXT_INVALID_DUP) &&
            (token->type == OP_DUP_ASTERISK || token->type == OP_OPEN_DUP_NUM))
        {
            if (tree != NULL)
                postorder(tree, free_tree, NULL);
            *err = REG_BADRPT;
...
static bin_tree_t* parse_dup_op(bin_tree_t* elem, re_string_t* regexp,
                                re_dfa_t* dfa, re_token_t* token,
                                reg_syntax_t syntax, reg_errcode_t* err)
{
                                     ...
                                     reg_errcode_t* err)
{
    ...
    auto inline int32_t __attribute__((always_inline))
    ...
    auto inline unsigned int __attribute__((always_inline))
    ...
    auto inline reg_errcode_t __attribute__((always_inline))
...
static bin_tree_t* create_token_tree(re_dfa_t* dfa, bin_tree_t* left,
                                     bin_tree_t* right, const re_token_t* token)
{
    bin_tree_t* tree;
