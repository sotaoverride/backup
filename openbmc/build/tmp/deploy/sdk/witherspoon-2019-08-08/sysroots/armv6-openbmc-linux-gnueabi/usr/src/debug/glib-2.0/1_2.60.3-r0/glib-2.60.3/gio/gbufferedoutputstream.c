...
G_DEFINE_TYPE_WITH_CODE(
    GBufferedOutputStream, g_buffered_output_stream,
    ...
    g_buffered_output_stream_class_init(GBufferedOutputStreamClass* klass)
{
    ...
    g_object_class_install_property(
        object_class, PROP_AUTO_GROW,
        g_param_spec_boolean("auto-grow", P_("Auto-grow"),
                                                  ...
                                                  GParamSpec* pspec)
{
    ...
    {
        case PROP_BUFSIZE:
            g_buffered_output_stream_set_buffer_size(stream,
                                                     g_value_get_uint(value));
            break;

        case PROP_AUTO_GROW:
            g_buffered_output_stream_set_auto_grow(stream,
...
GOutputStream* g_buffered_output_stream_new(GOutputStream* base_stream)
{
    ...
    stream = g_object_new(G_TYPE_BUFFERED_OUTPUT_STREAM, "base-stream",
                          base_stream, NULL);
                                                  ...
                                                  gsize size)
{
    ...
    stream = g_object_new(G_TYPE_BUFFERED_OUTPUT_STREAM, "base-stream",
                          base_stream, "buffer-size", size, NULL);

    return stream;
}

static gboolean flush_buffer(GBufferedOutputStream* stream,
                             GCancellable* cancellable, GError** error)
{
    ...
    res = g_output_stream_write_all(base_stream, priv->buffer, priv->pos,
...
static void flush_buffer_thread(GTask* task, gpointer object,
