 ...
 *
 * The rsyslog runtime library is free software: you can redistribute it and/or
...
/* static data */
DEFobjStaticHelpers DEFobjCurrIf(glbl) DEFobjCurrIf(strm) DEFobjCurrIf(datetime)
    DEFobjCurrIf(statsobj)

#if __GNUC__ >= 8
#pragma GCC diagnostic ignored                                                 \
    "-Wcast-function-type" // TODO: investigate further!
#endif                     /* if __GNUC__ >= 8 */
...
/* forward-definitions */
static rsRetVal doEnqSingleObj(qqueue_t* pThis, flowControl_t flowCtlType,
                               smsg_t* pMsg);
...
static rsRetVal qqueueMultiEnqObjNonDirect(qqueue_t* pThis,
                                           multi_submit_t* pMultiSub);
static rsRetVal qqueueMultiEnqObjDirect(qqueue_t* pThis,
                                        multi_submit_t* pMultiSub);
static rsRetVal qAddDirect(qqueue_t* pThis, smsg_t* pMsg);
static rsRetVal qDestructDirect(qqueue_t __attribute__((unused)) * pThis);
static rsRetVal qConstructDirect(qqueue_t __attribute__((unused)) * pThis);
static rsRetVal qDestructDisk(qqueue_t* pThis);
rsRetVal qqueueSetSpoolDir(qqueue_t* pThis, uchar* pszSpoolDir,
                           int lenSpoolDir);
...
static struct cnfparamblk pblk = {
    CNFPARAMBLK_VERSION, sizeof(cnfpdescr) / sizeof(struct cnfparamdescr),
    cnfpdescr};
...
static rsRetVal tdlAdd(qqueue_t* pQueue, qDeqID deqID, int nElemDeq)
{
    ...
    /* now find right spot */
    for (pPrev = pQueue->toDeleteLst; pPrev != NULL && deqID > pPrev->deqID;
         pPrev = pPrev->pNext)
    {
        /*JUST SEARCH*/;
    }
...
/* get the logical queue size (that is store size minus logically dequeued
 * elements). Must only be called while mutex is locked! rgerhards, 2009-05-19
 */
...
/* This function drains the queue in cases where this needs to be done. The most
 * probable reason is a HUP which needs to discard data (because the queue is
 * configured to be lossy). During a shutdown, this is typically not needed, as
 * the OS frees up ressources and does this much quicker than when we clean up
 * ourselvs. -- rgerhards, 2008-10-21 This function returns void, as it makes no
 * sense to communicate an error back, even if it happens. This functions works
 * "around" the regular deque mechanism, because it is only used to clean up (in
 * cases where message loss is acceptable).
 */
static void queueDrain(qqueue_t* pThis)
{
    ...
    DBGOPRINT((obj_t*)pThis,
              "queue (type %d) will lose %d messages, destroying...\n",
    ...
    if (!pThis->bEnqOnly)
    {
        if (pThis->bIsDA && getLogicalQueueSize(pThis) >= pThis->iHighWtrMrk)
        {
            ...
            wtpAdviseMaxWorkers(pThis->pWtpDA, 1,
                                DENY_WORKER_START_DURING_SHUTDOWN);
        ...
        {
            iMaxWorkers =
                getLogicalQueueSize(pThis) / pThis->iMinMsgsPerWrkr + 1;
        }
...
static rsRetVal qqueueChkIsDA(qqueue_t* pThis)
{
    ...
    if (pThis->pszFilePrefix != NULL)
    {
        pThis->bIsDA = 1;
        DBGOPRINT((obj_t*)pThis,
                  "is disk-assisted, disk will be used on demand\n");
    }
    else
    {
...
static rsRetVal StartDA(qqueue_t* pThis)
{
    ...
    CHKiRet(
        qqueueConstruct(&pThis->pqDA, QUEUETYPE_DISK, 1, 0, pThis->pConsumer));

    /* give it a name */
    snprintf((char*)pszDAQName, sizeof(pszDAQName), "%s[DA]",
    ...
    CHKiRet(qqueueSetFilePrefix(pThis->pqDA, pThis->pszFilePrefix,
                                pThis->lenFilePrefix));
    CHKiRet(
        qqueueSetSpoolDir(pThis->pqDA, pThis->pszSpoolDir, pThis->lenSpoolDir));
    ...
    if (iRet != RS_RET_OK && iRet != RS_RET_FILE_NOT_FOUND)
    {
        ...
        LogError(errno, iRet,
                 "error starting up disk queue, using pure in-memory mode");
...
/* initiate DA mode
 * param bEnqOnly tells if the disk queue is to be run in enqueue-only mode.
 * This may be needed during shutdown of memory queues which need to be
 * persisted to disk. If this function fails (should not happen), DA mode is not
 * turned on. rgerhards, 2008-01-16
 */
    ...
    InitDA(qqueue_t* const pThis, const int bLockMutex)
{
    ...
    /* check if we already have a DA worker pool. If not, initiate one. Please
     * note that the pool is created on first need but never again destructed
     * (until the queue is). This is intentional. We assume that when we need it
     * once, we may also need it on another occasion. Ressources used are quite
     * minimal when no worker is running. rgerhards, 2008-01-24 NOTE: this is
     * the DA worker *pool*, not the DA queue!
     */
    lenBuf = snprintf((char*)pszBuf, sizeof(pszBuf), "%s:DAwpool",
                      obj.GetName((obj_t*)pThis));
    CHKiRet(wtpConstruct(&pThis->pWtpDA));
    CHKiRet(wtpSetDbgHdr(pThis->pWtpDA, pszBuf, lenBuf));
    CHKiRet(wtpSetpfChkStopWrkr(
        pThis->pWtpDA, (rsRetVal(*)(void* pUsr, int))qqueueChkStopWrkrDA));
    CHKiRet(wtpSetpfGetDeqBatchSize(
        pThis->pWtpDA, (rsRetVal(*)(void* pUsr, int*))GetDeqBatchSize));
    CHKiRet(wtpSetpfDoWork(pThis->pWtpDA,
                           (rsRetVal(*)(void* pUsr, void* pWti))ConsumerDA));
    CHKiRet(wtpSetpfObjProcessed(
        pThis->pWtpDA, (rsRetVal(*)(void* pUsr, wti_t* pWti))batchProcessed));
 ...
 * but for this specific type of queue. The mapping to these handlers happens
 * during queue construction. Later on, handlers are called by pointers present
 * in the queue instance object.
 */
...
static rsRetVal qConstructFixedArray(qqueue_t* pThis)
{
    ...
    if ((pThis->tVars.farray.pBuf =
             malloc(sizeof(void*) * pThis->iMaxQueueSize)) == NULL)
    {
...
static rsRetVal qAddLinkedList(qqueue_t* pThis, smsg_t* pMsg)
{
    ...
    {
        pThis->tVars.linklist.pDelRoot = pThis->tVars.linklist.pDeqRoot =
 ...
 */
static rsRetVal queueSwitchToEmergencyMode(qqueue_t* pThis,
                                           rsRetVal initiatingError)
{
    pThis->iQueueSize = 0;
    pThis->nLogDeq = 0;
...
static rsRetVal qqueueLoadPersStrmInfoFixup(strm_t* pStrm,
                                            qqueue_t __attribute__((unused)) *
                                                pThis)
{
    DEFiRet;
...
static rsRetVal qqueueTryLoadPersistedInfo(qqueue_t* pThis)
{
    ...
    if (stat((char*)pThis->pszQIFNam, &stat_buf) == -1)
    {
        ...
        else
        {
            LogError(errno, RS_RET_IO_ERROR,
                     "queue: %s: error %d could not access .qi file",
    ...
    CHKiRet(obj.Deserialize(
        &pThis->tVars.disk.pWrite, (uchar*)"strm", psQIF,
        (rsRetVal(*)(obj_t*, void*))qqueueLoadPersStrmInfoFixup, pThis));
    CHKiRet(obj.Deserialize(
        &pThis->tVars.disk.pReadDel, (uchar*)"strm", psQIF,
        (rsRetVal(*)(obj_t*, void*))qqueueLoadPersStrmInfoFixup, pThis));
    /* create a duplicate for the read "pointer". */
    CHKiRet(strm.Dup(pThis->tVars.disk.pReadDel, &pThis->tVars.disk.pReadDeq));
    CHKiRet(strm.SetbDeleteOnClose(pThis->tVars.disk.pReadDeq,
                                   0)); /* deq must NOT delete the files! */
    ...
    if (pThis->useCryprov)
    {
        ...
        CHKiRet(
            strm.SetcryprovData(pThis->tVars.disk.pWrite, pThis->cryprovData));
        CHKiRet(strm.Setcryprov(pThis->tVars.disk.pReadDeq, &pThis->cryprov));
        CHKiRet(strm.SetcryprovData(pThis->tVars.disk.pReadDeq,
                                    pThis->cryprovData));
        CHKiRet(strm.Setcryprov(pThis->tVars.disk.pReadDel, &pThis->cryprov));
        CHKiRet(strm.SetcryprovData(pThis->tVars.disk.pReadDel,
                                    pThis->cryprovData));
    }
...
finalize_it:
    ...
    {
        DBGOPRINT((obj_t*)pThis,
                  "state %d reading .qi file - can not read persisted info (if "
                  "any)\n",
                  iRet);
    }
 ...
 * Note that we use a file limit of 10,000,000 files. That number should never
 * pose a problem. If so, I guess the user has a design issue... But of course,
 * the code can always be changed (though it would probably be more appropriate
 * to increase the allowed file size at this point - that should be a config
 * setting... rgerhards, 2008-01-10
 */
static rsRetVal qConstructDisk(qqueue_t* pThis)
{
    ...
    /* and now check if there is some persistent information that needs to be
     * read in */
    ...
    else
    {
        ...
        CHKiRet(
            strm.SetbSync(pThis->tVars.disk.pWrite, pThis->bSyncQueueFiles));
        CHKiRet(strm.SetDir(pThis->tVars.disk.pWrite, pThis->pszSpoolDir,
                            pThis->lenSpoolDir));
        CHKiRet(strm.SetiMaxFiles(pThis->tVars.disk.pWrite, 10000000));
        CHKiRet(strm.SettOperationsMode(pThis->tVars.disk.pWrite,
                                        STREAMMODE_WRITE));
        CHKiRet(
            strm.SetsType(pThis->tVars.disk.pWrite, STREAMTYPE_FILE_CIRCULAR));
        if (pThis->useCryprov)
        {
            CHKiRet(strm.Setcryprov(pThis->tVars.disk.pWrite, &pThis->cryprov));
            CHKiRet(strm.SetcryprovData(pThis->tVars.disk.pWrite,
                                        pThis->cryprovData));
        }
        ...
        CHKiRet(strm.SetDir(pThis->tVars.disk.pReadDeq, pThis->pszSpoolDir,
                            pThis->lenSpoolDir));
        CHKiRet(strm.SetiMaxFiles(pThis->tVars.disk.pReadDeq, 10000000));
        CHKiRet(strm.SettOperationsMode(pThis->tVars.disk.pReadDeq,
                                        STREAMMODE_READ));
        CHKiRet(strm.SetsType(pThis->tVars.disk.pReadDeq,
                              STREAMTYPE_FILE_CIRCULAR));
        if (pThis->useCryprov)
        {
            CHKiRet(
                strm.Setcryprov(pThis->tVars.disk.pReadDeq, &pThis->cryprov));
            CHKiRet(strm.SetcryprovData(pThis->tVars.disk.pReadDeq,
                                        pThis->cryprovData));
        }
        CHKiRet(strm.ConstructFinalize(pThis->tVars.disk.pReadDeq));
        ...
        CHKiRet(
            strm.SetbSync(pThis->tVars.disk.pReadDel, pThis->bSyncQueueFiles));
        CHKiRet(strm.SetbDeleteOnClose(pThis->tVars.disk.pReadDel, 1));
        CHKiRet(strm.SetDir(pThis->tVars.disk.pReadDel, pThis->pszSpoolDir,
                            pThis->lenSpoolDir));
        CHKiRet(strm.SetiMaxFiles(pThis->tVars.disk.pReadDel, 10000000));
        CHKiRet(strm.SettOperationsMode(pThis->tVars.disk.pReadDel,
                                        STREAMMODE_READ));
        CHKiRet(strm.SetsType(pThis->tVars.disk.pReadDel,
                              STREAMTYPE_FILE_CIRCULAR));
        if (pThis->useCryprov)
        {
            CHKiRet(
                strm.Setcryprov(pThis->tVars.disk.pReadDel, &pThis->cryprov));
            CHKiRet(strm.SetcryprovData(pThis->tVars.disk.pReadDel,
                                        pThis->cryprovData));
        }
        CHKiRet(strm.ConstructFinalize(pThis->tVars.disk.pReadDel));

        CHKiRet(strm.SetFName(pThis->tVars.disk.pWrite, pThis->pszFilePrefix,
                              pThis->lenFilePrefix));
        CHKiRet(strm.SetFName(pThis->tVars.disk.pReadDeq, pThis->pszFilePrefix,
                              pThis->lenFilePrefix));
        CHKiRet(strm.SetFName(pThis->tVars.disk.pReadDel, pThis->pszFilePrefix,
                              pThis->lenFilePrefix));
    }

    /* now we set (and overwrite in case of a persisted restart) some parameters
     * which should always reflect the current configuration variables. Be
     * careful by doing so, for example file name generation must not be changed
     * as that would break the ability to read existing queue files. --
     * rgerhards, 2008-01-12
     */
    CHKiRet(
        strm.SetiMaxFileSize(pThis->tVars.disk.pWrite, pThis->iMaxFileSize));
    CHKiRet(
        strm.SetiMaxFileSize(pThis->tVars.disk.pReadDeq, pThis->iMaxFileSize));
    CHKiRet(
        strm.SetiMaxFileSize(pThis->tVars.disk.pReadDel, pThis->iMaxFileSize));
...
static rsRetVal ATTR_NONNULL(1, 2) qAddDisk(qqueue_t* const pThis, smsg_t* pMsg)
{
    ...
    CHKiRet(strm.SetWCntr(pThis->tVars.disk.pWrite,
                          NULL)); /* no more counting for now... */
    ...
    DBGOPRINT((obj_t*)pThis,
              "write wrote %lld octets to disk, queue disk size now %lld "
              "octets, EnqOnly:%d\n",
...
{
    DEFiRet;
    iRet = objDeserializeWithMethods(
        ppMsg, (uchar*)"msg", 3, pThis->tVars.disk.pReadDeq, NULL, NULL,
        msgConstructForDeserializer, NULL, MsgDeserialize);
    if (iRet != RS_RET_OK)
    {
...
static rsRetVal qAddDirectWithWti(qqueue_t* pThis, smsg_t* pMsg, wti_t* pWti)
{
    ...
    /* calling the consumer is quite different here than it is from a worker
     * thread */
    /* we need to provide the consumer's return value back to the caller because
     * in direct mode the consumer probably has a lot to convey (which get's
     * lost in the other modes because they are asynchronous. But direct mode is
     * deliberately synchronous. rgerhards, 2008-02-12 We use our knowledge
     * about the batch_t structure below, but without that, we pay a too-large
     * performance toll... -- rgerhards, 2009-04-22
     */
...
#endif
        DBGOPRINT((obj_t*)pThis,
                  "qqueueAdd: entry added, size now log %d, phys %d entries\n",
...
static rsRetVal qqueueDeq(qqueue_t* pThis, smsg_t** ppMsg)
{
     ...
     * will not be decremented, what will most probably result in an endless
     * loop. If we decrement, however, we may lose a message. But that is better
     * than losing the whole process because it loops... -- rgerhards,
     * 2008-01-03
     */
    iRet = pThis->qDeq(pThis, ppMsg);
    ATOMIC_INC(&pThis->nLogDeq, &pThis->mutLogDeq);

    //	DBGOPRINT((obj_t*) pThis, "entry deleted, size now log %d, phys %d
    //entries\n", 		  getLogicalQueueSize(pThis), getPhysicalQueueSize(pThis));
    ...
    tryShutdownWorkersWithinQueueTimeout(qqueue_t* const pThis)
{
    ...
    if (pThis->bIsDA)
    {
        /* We need to lock the mutex, as otherwise we may have a race that
         * prevents us from awaking the DA worker. */
        d_pthread_mutex_lock(pThis->mut);

        /* tell regular queue DA worker to stop shuffling messages to DA
         * queue... */
        ...
        /* also tell the DA queue worker to shut down, so that it already
         * knows... */
        ...
        DBGOPRINT(
            (obj_t*)pThis,
            "awoke DA queue regular worker, told it to shut down when done.\n");

        d_pthread_mutex_unlock(pThis->mut);
    }

    /* first calculate absolute timeout - we need the absolute value here,
     * because we need to coordinate shutdown of both the regular and DA queue
     * on *the same* timeout.
     */
    ...
    /* OK, the worker for the regular queue is processed, on the the DA queue
     * regular worker. */
    if (pThis->pqDA != NULL)
    {
        DBGOPRINT((obj_t*)pThis,
                  "we have a DA queue (0x%lx), requesting its shutdown.\n",
                  qqueueGetID(pThis->pqDA));
        /* we use the same absolute timeout as above, so we do not use more than
         * the configured timeout interval!
         */
        DBGOPRINT((obj_t*)pThis,
                  "trying shutdown of regular worker of DA queue\n");
        iRetLocal =
            wtpShutdownAll(pThis->pqDA->pWtpReg, wtpState_SHUTDOWN, &tTimeout);
        if (iRetLocal == RS_RET_TIMED_OUT)
        {
            LogMsg(
                0, RS_RET_TIMED_OUT, LOG_INFO,
...
static rsRetVal tryShutdownWorkersWithinActionTimeout(qqueue_t* pThis)
{
    ...
    DBGOPRINT((obj_t*)pThis,
              "trying to shutdown workers within Action Timeout");
    ...
    // TODO: make sure we have at minimum a 10ms timeout - workers deserve a
    // chance...
    /* now give the queue workers a last chance to gracefully shut down (based
     * on action timeout setting) */
    timeoutComp(&tTimeout, pThis->toActShutdown);
    DBGOPRINT((obj_t*)pThis,
              "trying immediate shutdown of regular workers (if any)\n");
    iRetLocal =
        wtpShutdownAll(pThis->pWtpReg, wtpState_SHUTDOWN_IMMEDIATE, &tTimeout);
    if (iRetLocal == RS_RET_TIMED_OUT)
    {
        LogMsg(0, RS_RET_TIMED_OUT, LOG_INFO,
               "%s: immediate shutdown timed out on primary queue (this is "
               "acceptable and "
    ...
    else if (iRetLocal != RS_RET_OK)
    {
        LogMsg(0, iRetLocal, LOG_WARNING,
               "%s: potential internal error: unexpected return state after "
               "trying "
    ...
    if (pThis->pqDA != NULL)
    {
        ...
        DBGOPRINT((obj_t*)pThis,
                  "trying immediate shutdown of DA queue workers\n");
        iRetLocal = wtpShutdownAll(pThis->pqDA->pWtpReg,
                                   wtpState_SHUTDOWN_IMMEDIATE, &tTimeout);
        if (iRetLocal == RS_RET_TIMED_OUT)
        {
            LogMsg(0, RS_RET_TIMED_OUT, LOG_INFO,
                   "%s: immediate shutdown timed out on DA queue (this is "
                   "acceptable and "
                   "triggers cancellation)",
                   objGetName((obj_t*)pThis));
        }
        ...
        {
            LogMsg(0, iRetLocal, LOG_WARNING,
                   "%s: potential internal error: unexpected return state "
                   "after trying "
        ...
        /* and now we need to terminate the DA worker itself. We always grant it
         * a 100ms timeout, which should be sufficient and usually not be
         * required (it is expected to have finished long before while we were
         * processing the queue timeout in shutdown phase 1). rgerhards,
         * 2009-10-14
         */
        timeoutComp(&tTimeout, 100);
        DBGOPRINT((obj_t*)pThis,
                  "trying regular shutdown of main queue DA worker pool\n");
        iRetLocal = wtpShutdownAll(pThis->pWtpDA, wtpState_SHUTDOWN_IMMEDIATE,
                                   &tTimeout);
        if (iRetLocal == RS_RET_TIMED_OUT)
        {
            LogMsg(0, iRetLocal, LOG_WARNING,
...
/* This function cancels all remaining regular workers for both the main and the
 * DA queue. rgerhards, 2009-05-29
 */
static rsRetVal cancelWorkers(qqueue_t* pThis)
{
    ...
    /* Now queue workers should have terminated. If not, we need to cancel them
     * as we have applied all timeout setting. If any worker in any queue still
     * executes, its consumer is possibly long-running and cancelling is the
     * only way to get rid of it.
     */
    DBGOPRINT((obj_t*)pThis, "checking to see if we need to cancel any worker "
                             "threads of the primary queue\n");
    ...
    /* ... and now the DA queue, if it exists (should always be after the
     * primary one) */
    if (pThis->pqDA != NULL)
    {
        DBGOPRINT((obj_t*)pThis,
                  ...
                  "the DA queue\n");
        iRetLocal =
            wtpCancelAll(pThis->pqDA->pWtpReg, objGetName((obj_t*)pThis));
        /* returns immediately if all threads already have terminated */
        if (iRetLocal != RS_RET_OK)
        {
            DBGOPRINT(
        ...
        /* finally, we cancel the main queue's DA worker pool, if it still is
         * running. It may be restarted later to persist the queue. But we stop
         * it, because otherwise we get into big trouble when resetting the
         * logical dequeue pointer. This operation can only be done when *no*
         * worker is running. So time for a shutdown... -- rgerhards, 2009-05-28
         */
        DBGOPRINT((obj_t*)pThis, "checking to see if main queue DA worker pool "
 ...
 * in DA case. This is necessary because their timeouts are tightly coupled.
 * Most importantly, the timeouts would be applied twice (or logic be extremely
 * complex) if each would have its own shutdown. The function does not self
 * check this condition - the caller must make sure it is not called with a
 * parent. rgerhards, 2009-05-26: we do NO longer persist the queue here if
 * bSaveOnShutdown is set. This must be handled by the caller. Not doing that
 * cleans up the queue shutdown considerably. Also, older engines had a
 * potential hang condition when the DA queue was already started and the DA
 * worker configured for infinite retries and the action was during retry
 * processing. This was a design issue, which is solved as of now. Note that the
 * shutdown now may take a little bit longer, because we no longer can persist
 * the queue in parallel to waiting on worker timeouts.
 */
rsRetVal ATTR_NONNULL(1) qqueueShutdownWorkers(qqueue_t* const pThis)
{
     ...
     * Well, more precisely, they *are in termination*. Some cancel cleanup
     * handlers may still be running. Note that the main queue's DA worker may
     * still be running.
     */
    DBGOPRINT(
        (obj_t*)pThis,
        "worker threads terminated, remaining queue size log %d, phys %d.\n",
        getLogicalQueueSize(pThis), getPhysicalQueueSize(pThis));
 ...
 * is done by queueStart(). The reason is that we want to give the caller a
 * chance to modify some parameters before the queue is actually started.
 */
rsRetVal qqueueConstruct(qqueue_t** ppThis, queueType_t qType,
                         int iWorkerThreads, int iMaxQueueSize,
                         rsRetVal (*pConsumer)(void*, batch_t*, wti_t*))
{
    ...
    /* set some water marks so that we have useful defaults if none are set
     * specifically */
    pThis->iFullDlyMrk = -1;
    ...
    pThis->iDeqBatchSize =
        8; /* conservative default, should still provide good performance */
    pThis->iMinDeqBatchSize =
        0; /* conservative default, should still provide good performance */
 ...
 * been added in support of the new v6 config system. It expect properly
 * pre-initialized objects, but we need to differentiate between ruleset main
 * and action queues. In order to avoid unnecessary complexity, we provide the
 * necessary defaults via specific function calls.
 */
void qqueueSetDefaultsActionQueue(qqueue_t* pThis)
{
    ...
    pThis->iNumWorkerThreads =
        1; /* number of worker threads for the mm queue above */
    ...
    pThis->toActShutdown =
        actq_dflt_toActShutdown;    /* action shutdown (in phase 2) */
    pThis->toEnq = actq_dflt_toEnq; /* timeout for queue enque */
    pThis->toWrkShutdown =
        actq_dflt_toWrkShutdown; /* timeout for worker thread shutdown */
    pThis->iMinMsgsPerWrkr =
        -1; /* minimum messages per worker needed to start a new one */
...
{
    pThis->qType =
        QUEUETYPE_FIXED_ARRAY;    /* type of the main message queue above */
...
/* This function checks if the provided message shall be discarded and does so,
 * if needed. In DA mode, we do not discard any messages as we assume the disk
 * subsystem is fast enough to provide real-time creation of spool files. Note:
 * cached copies of iQueueSize is provided so that no mutex locks are required.
 * The caller must have obtained them while the mutex was locked. Of course,
 * these values may no longer be current, but that is OK for the discard check.
 * At worst, the message is either processed or discarded when it should not
 * have been. As discarding is in itself somewhat racy and erratic, that is no
 * problems for us. This function MUST NOT lock the queue mutex, it could result
 * in deadlocks! If the message is discarded, it can no longer be processed by
 * the caller. So be sure to check the return state! rgerhards, 2008-01-24
 */
static int qqueueChkDiscardMsg(qqueue_t* pThis, int iQueueSize, smsg_t* pMsg)
{
    ...
    if (pThis->iDiscardMrk > 0 && iQueueSize >= pThis->iDiscardMrk)
    {
        ...
        {
            DBGOPRINT((obj_t*)pThis,
                      "queue nearly full (%d entries), discarded severity %d "
                      "message\n",
 ...
 */
static rsRetVal DoDeleteBatchFromQStore(qqueue_t* pThis, int nElem)
{
    int i;
    ...
    {
        strmMultiFileSeek(pThis->tVars.disk.pReadDel,
                          pThis->tVars.disk.deqFileNumOut,
                          pThis->tVars.disk.deqOffs, &bytesDel);
        /* We need to correct the on-disk file size. This time it is a bit
         * tricky: we free disk space only upon file deletion. So we need to
         * keep track of what we have read until we get an out-offset that is
         * lower than the in-offset (which indicates file change). Then, we can
         * subtract the whole thing from the on-disk size. -- rgerhards,
         * 2008-01-30
         */
        if (bytesDel != 0)
        {
            ...
            pthread_cond_signal(
                &pThis->notFull); /* we hold the mutex while we are in here! */
        }
    }
    else
    { /* memory queue */
    ...
    DBGPRINTF(
        "doDeleteBatch: delete batch from store, new sizes: log %d, phys %d\n",
        getLogicalQueueSize(pThis), getPhysicalQueueSize(pThis));
    ...
    {
        qqueuePersist(pThis,
                      QUEUE_CHECKPOINT); /* robustness persist .qi file */
    }

    RETiRet;
}

/* remove messages from the physical queue store that are fully processed. This
 * is controlled via the to-delete list.
 */
...
static rsRetVal DeleteProcessedBatch(qqueue_t* pThis, batch_t* pBatch)
{
    ...
    for (i = 0; i < pBatch->nElem; ++i)
    {
        ...
        {
            localRet =
                doEnqSingleObj(pThis, eFLOWCTL_NO_DELAY, MsgAddRef(pMsg));
            ++nEnqueued;
    ...
    DBGPRINTF(
        "DeleteProcessedBatch: we deleted %d objects and enqueued %d objects\n",
    ...
    pBatch->nElem = pBatch->nElemDeq = 0;
        /* reset batch */ // TODO: more fine init, new fields! 2010-06-14
                              ...
                              int* const pSkippedMsgs)
{
    ...
    if (pThis->qType == QUEUETYPE_DISK)
    {
        pThis->tVars.disk.deqFileNumIn =
            strmGetCurrFileNum(pThis->tVars.disk.pReadDeq);
    }
    ...
    {
        timeoutComp(&timeout,
                    pThis->toMinDeqBatchSize); /* get absolute timeout */
    }

    while ((iQueueSize = getLogicalQueueSize(pThis)) > 0 &&
           nDequeued < pThis->iDeqBatchSize)
    {
        ...
        if (rd_fd != -1 && rd_fd == wr_fd && rd_offs == wr_offs)
        {
            DBGPRINTF("problem on disk queue '%s': "
                      //"queue size log %d, phys %d, but rd_fd=wr_rd=%d and
                      //offs=%lld\n",
                      "queue size log %d, phys %d, but rd_fd=wr_rd=%d and "
                      "offs=%" PRId64 "\n",
        ...
        {
            while (!pThis->bShutdownImmediate && keep_running &&
                   ...
                   getLogicalQueueSize(pThis) == 0)
            {
                ...
                if (wtiWaitNonEmpty(pWti, timeout) == 0)
                { /* timeout? */
                    DBGPRINTF(
                        "%s minDeqBatchSize timeout, batch is %d messages\n",
                        obj.GetName((obj_t*)pThis), nDequeued);
                    keep_running = 0;
                }
            }
    ...
    if (pThis->qType == QUEUETYPE_DISK)
    {
        strm.GetCurrOffset(pThis->tVars.disk.pReadDeq,
                           &pThis->tVars.disk.deqOffs);
        pThis->tVars.disk.deqFileNumOut =
            strmGetCurrFileNum(pThis->tVars.disk.pReadDeq);
    }
    ...
    DBGOPRINT(
        (obj_t*)pThis, "dequeued %d consumable elements, szlog %d sz phys %d\n",
 ...
 * I made a radical change - we now dequeue multiple elements, and store these
 * objects in an array of user pointers. We expect that this increases
 * performance. rgerhards, 2009-04-22
 */
static rsRetVal DequeueConsumable(qqueue_t* pThis, wti_t* pWti,
                                  int* const pSkippedMsgs)
{
    DEFiRet;
    ...
    *pSkippedMsgs = 0;
    ...
    if (*pSkippedMsgs > 0)
    {
        LogError(0, RS_RET_ERR,
                 "%s: lost %d messages from diskqueue (invalid .qi file)",
                 obj.GetName((obj_t*)pThis), *pSkippedMsgs);
    }

    /* awake some flow-controlled sources if we can do this right now */
    /* TODO: this could be done better from a performance point of view -- do it
     * only if we have someone waiting for the condition (or only when we hit
     * the watermark right on the nail [exact value]) -- rgerhards, 2008-03-14
     * now that we dequeue batches of pointers, this is much less an issue...
     * rgerhards, 2009-04-22
     */
    if (iQueueSize < pThis->iFullDlyMrk / 2 ||
        glbl.GetGlobalInputTermState() == 1)
    {
 ...
 * level. However, that would limit them to be used together with actions. We
 * have taken the broader approach, moving it right into the queue. This is even
 * necessary if we want to prevent spawning of multiple unnecessary worker
 * threads as described above. -- rgerhards, 2008-04-02
 *
 *
 * time window: tCurr is current time; tFrom is start time, tTo is end time (in
 * mil 24h format). We may have tFrom = 4, tTo = 10 --> run from 4 to 10 hrs.
 * nice and happy we may also have tFrom= 22, tTo = 4 -> run from 10pm to 4am,
 * which is actually two windows: 0-4; 22-23:59 so when to run? Let's assume we
 * have 3am
 *
 * if(tTo < tFrom) {
 ...
 * }
 *
 * Bottom line: we need to check which type of window we have and need to adjust
 * our logic accordingly. Of course, sleep calculations need to be done up to
 * the minute, but you get the idea from the code above.
 */
static rsRetVal RateLimiter(qqueue_t* pThis)
{
    DEFiRet;
    ...
    { /* 25 means disabled */
        ...
        {
            if (iHrCurr < pThis->iDeqtWinToHr ||
                iHrCurr > pThis->iDeqtWinFromHr)
            {
                ; /* do not delay */
            }
            else
    ...
    if (iDelay > 0)
    {
        pthread_mutex_unlock(pThis->mut);
        DBGOPRINT((obj_t*)pThis,
                  "outside dequeue time window, delaying %d seconds\n", iDelay);
        srSleep(iDelay, 0);
        pthread_mutex_lock(pThis->mut);
    }
 ...
 */
static rsRetVal DequeueForConsumer(qqueue_t* pThis, wti_t* pWti,
                                   int* const pSkippedMsgs)
{
    DEFiRet;
...
static rsRetVal ConsumerReg(qqueue_t* pThis, wti_t* pWti)
{
    ...
    if (iRet == RS_RET_FILE_NOT_FOUND)
    {
        ...
        d_pthread_mutex_unlock(pThis->mut);
        DBGOPRINT((obj_t*)pThis,
                  "got 'file not found' error %d, queue defunct\n", iRet);
        iRet = queueSwitchToEmergencyMode(pThis, iRet);
        // TODO: think about what to return as iRet -- keep
    ...
    /* we now have a non-idle batch of work, so we can release the queue mutex
     * and process it */
    d_pthread_mutex_unlock(pThis->mut);
    bNeedReLock = 1;
     ...
     */
    // TODO: MULTIQUEUE: the following setting is no longer correct - need to
 ...
 * When this method is entered, the mutex is always locked and needs to be
 * unlocked as part of the processing. rgerhards, 2008-01-14
 */
static rsRetVal ConsumerDA(qqueue_t* pThis, wti_t* pWti)
{
    int i;
    ...
    {
        iRet = qqueueEnqMsg(pThis->pqDA, eFLOWCTL_NO_DELAY,
        ...
        pWti->batch.eltState[i] =
            BATCH_STATE_COMM; /* commited to other queue! */
    }
...
finalize_it:
    /*	Check the last return state of qqueueEnqMsg. If an error was returned,
     *we acknowledge it only. Unless the error code is
     *RS_RET_ERR_QUEUE_EMERGENCY, we reset the return state to RS_RET_OK.
     *	Otherwise the Caller functions would run into an infinite Loop trying to
     *enqueue the same messages over and over again.
     *
     ...
     *	These return states are important for Queue handling of the upper laying
     *functions. RGer: Note that checking for iRet < 0 is a bit bold. In theory,
     *positive iRet values are "OK" states, and things that the caller shall
     *deal with. However, this has not been done so consistently. Andre
     *convinced me that the current code is an elegant solution. However, if
     *problems with queue workers and/or shutdown come up, this code here should
     *be looked at suspiciously. In those cases it may work out to check all
     *status codes explicitely, just to avoid a pitfall due to unexpected states
     *being passed on to the caller.
     */
    if (iRet != RS_RET_OK && iRet != RS_RET_ERR_QUEUE_EMERGENCY && iRet < 0)
    {
        DBGOPRINT((obj_t*)pThis,
                  "ConsumerDA:qqueueEnqMsg Resetting iRet from %d back to "
                  "RS_RET_OK\n",
                  iRet);
        iRet = RS_RET_OK;
    }
    ...
    {
        DBGOPRINT((obj_t*)pThis,
                  "ConsumerDA:qqueueEnqMsg returns with iRet %d\n", iRet);
    }
...
static rsRetVal qqueueChkStopWrkrDA(qqueue_t* pThis)
{
    ...
    /*DBGPRINTF("XXXX: chkStopWrkrDA called, low watermark %d, log Size %d, phys
    Size %d, bEnqOnly %d\n", pThis->iLowWtrMrk, getLogicalQueueSize(pThis),
    getPhysicalQueueSize(pThis), pThis->bEnqOnly);*/
    if (pThis->bEnqOnly)
    {
...
/* must only be called when the queue mutex is locked, else results
 * are not stable!
 * If we are a child, we have done our duty when the queue is empty. In that
 * case, we can terminate. Version for the regular worker thread.
 */
static rsRetVal ChkStopWrkrReg(qqueue_t* pThis)
{
    DEFiRet;
    /*DBGPRINTF("XXXX: chkStopWrkrReg called, low watermark %d, log Size %d,
    phys Size %d, bEnqOnly %d\n",
    pThis->iLowWtrMrk, getLogicalQueueSize(pThis), getPhysicalQueueSize(pThis),
    pThis->bEnqOnly);*/
    if (pThis->bEnqOnly)
    {
        iRet = RS_RET_TERMINATE_NOW;
...
rsRetVal qqueueStart(qqueue_t* pThis) /* this is the ConstructionFinalizer */
{
    ...
    if (pThis->pszSpoolDir == NULL)
    {
         ...
         */
        if ((pThis->pszSpoolDir = (uchar*)strdup((char*)glbl.GetWorkDir())) ==
            NULL)
            ABORT_FINALIZE(RS_RET_OUT_OF_MEMORY);
        pThis->lenSpoolDir = ustrlen(pThis->pszSpoolDir);
    }
    ...
    if (pThis->iMaxQueueSize < 100 && (pThis->qType == QUEUETYPE_LINKEDLIST ||
    ...
    if (pThis->pszFilePrefix != NULL)
    { /* This means we have a potential DA queue */
    ...
    if (pThis->iHighWtrMrk < 2 || pThis->iHighWtrMrk > pThis->iMaxQueueSize)
    {
        ...
        if (pThis->iHighWtrMrk == 0)
        { /* guard against very low max queue sizes! */
            pThis->iHighWtrMrk = pThis->iMaxQueueSize;
        }
    }
    if (pThis->iLowWtrMrk < 2 || pThis->iLowWtrMrk > pThis->iMaxQueueSize ||
    ...
    {
        pThis->iMinMsgsPerWrkr =
            pThis->iMaxQueueSize / pThis->iNumWorkerThreads;
    }
    ...
    if (pThis->bIsDA)
    {
        /* if we are in DA mode, we must make sure full delayable messages do
         * not initiate going to disk!
         */
        wrk = pThis->iHighWtrMrk -
              (pThis->iHighWtrMrk / 100) * 50; /* 50% of high water mark */
    ...
    DBGOPRINT(
        (obj_t*)pThis,
        "params: type %d, enq-only %d, disk assisted %d, spoolDir '%s', "
        "maxFileSz %lld, "
        ...
        pThis->iMaxFileSize, pThis->iMaxQueueSize, getLogicalQueueSize(pThis),
        getPhysicalQueueSize(pThis), pThis->pqParent == NULL ? 0 : 1,
        pThis->iFullDlyMrk, pThis->iLightDlyMrk, pThis->iDeqBatchSize,
        pThis->iMinDeqBatchSize, pThis->iHighWtrMrk, pThis->iLowWtrMrk,
    ...
    CHKiRet(wtpSetpfRateLimiter(pThis->pWtpReg,
                                (rsRetVal(*)(void* pUsr))RateLimiter));
    CHKiRet(wtpSetpfChkStopWrkr(pThis->pWtpReg,
                                (rsRetVal(*)(void* pUsr, int))ChkStopWrkrReg));
    CHKiRet(wtpSetpfGetDeqBatchSize(
        pThis->pWtpReg, (rsRetVal(*)(void* pUsr, int*))GetDeqBatchSize));
    CHKiRet(wtpSetpfDoWork(pThis->pWtpReg,
                           (rsRetVal(*)(void* pUsr, void* pWti))ConsumerReg));
    CHKiRet(wtpSetpfObjProcessed(
        pThis->pWtpReg, (rsRetVal(*)(void* pUsr, wti_t* pWti))batchProcessed));
    ...
    /* if the queue already contains data, we need to start the correct number
     * of worker threads. This can be the case when a disk queue has been
     * loaded. If we did not start it here, it would never start.
     */
    ...
    CHKiRet(statsobj.AddCounter(pThis->statsobj, UCHAR_CONSTANT("enqueued"),
                                ctrType_IntCtr, CTR_FLAG_RESETTABLE,
                                &pThis->ctrEnqueued));
    ...
    CHKiRet(statsobj.AddCounter(
        pThis->statsobj, UCHAR_CONSTANT("discarded.full"), ctrType_IntCtr,
        CTR_FLAG_RESETTABLE, &pThis->ctrFDscrd));
...
/* persist the queue to disk (write the .qi file). If we have something to
 * persist, we first save the information on the queue properties itself and
 * then we call the queue-type specific drivers. Variable bIsCheckpoint is set
 * to 1 if the persist is for a checkpoint, and 0 otherwise. rgerhards,
 * 2008-01-10
 */
static rsRetVal qqueuePersist(qqueue_t* pThis, int bIsCheckpoint)
{
    ...
    if (pThis->qType != QUEUETYPE_DISK)
    {
        ...
        {
            /* This error code is OK, but we will probably not implement this
             * any time The reason is that persistence happens via DA queues.
             * But I would like to leave the code as is, as we so have a hook in
             * case we need one.
        ...
        (getPhysicalQueueSize(pThis) == 0))
    {
        ...
        if (pThis->tVars.disk.pReadDel !=
            NULL) /* may be NULL if we had a startup failure! */
            CHKiRet(strm.SetbDeleteOnClose(pThis->tVars.disk.pReadDel, 1));
     ...
     * And, surprisingly enough, we currently need to persist only the size of
     * the queue. All the rest is re-created with then-current config parameters
     * when the queue is re-created. Well, we'll also save the current queue
     * type, just so that we know when somebody has changed the queue type... --
     * rgerhards, 2008-01-11
     */
    ...
    strm.Destruct(&psQIF);
    if (tmpQIFName != (char*)pThis->pszQIFNam)
    { /* pointer, not string comparison! */
    ...
    /* tell the input file object that it must not delete the file on close if
     * the queue is non-empty - but only if we are not during a simple
     * checkpoint
     */
    ...
    /* we have persisted the queue object. So whenever it comes to an empty
     * queue, we need to delete the QIF. Thus, we indicte that need.
     */
    pThis->bNeedDelQIF = 1;

finalize_it:
    if (tmpQIFName !=
        (char*)pThis->pszQIFNam) /* pointer, not string comparison! */
        free(tmpQIFName);
    if (psQIF != NULL)
        strm.Destruct(&psQIF);
...
static rsRetVal DoSaveOnShutdown(qqueue_t* pThis)
{
    ...
    /* we reduce the low water mark, otherwise the DA worker would terminate
     * when it is reached.
     */
    DBGOPRINT((obj_t*)pThis, "bSaveOnShutdown set, restarting DA worker...\n");
    pThis->bShutdownImmediate = 0; /* would termiante the DA worker! */
    pThis->iLowWtrMrk = 0;
    wtpSetState(pThis->pWtpDA,
                wtpState_SHUTDOWN); /* shutdown worker (only) when done (was
                                       _IMMEDIATE!) */
    wtpAdviseMaxWorkers(
        pThis->pWtpDA, 1,
        PERMIT_WORKER_START_DURING_SHUTDOWN); /* restart DA worker */
...
BEGINobjDestruct(qqueue) /* be sure to specify the object type also in END and
                            CODESTART macros! */
...
if (pThis->bQueueStarted)
{
     ...
     * We do not need to shutdown workers when we are in enqueue-only mode or we
     * are a direct queue - because in both cases we have none... ;) with a
     * child! -- rgerhards, 2008-01-28
     */
    if (pThis->qType != QUEUETYPE_DIRECT && !pThis->bEnqOnly &&
        pThis->pqParent == NULL && pThis->pWtpReg != NULL)
     ...
     * Note: currently pWtpReg is never NULL, but if we optimize our logic, this
     * may happen, e.g. when they are not created in enqueue-only mode. We
     * already check the condition as this may otherwise be very hard to find
     * once we optimize (and have long forgotten about this condition here ;)
     ...
     * Note that the wtp must be destructed first, it may be in cancel cleanup
     * handler *right now* and actually *need* to access the queue object to
     * persist some final data (re-queueing case). So we need to destruct the
     * wtp first, which will make sure all workers have terminated. Please note
     * that this also generates a situation where it is possible that the DA
     * queue has a parent pointer but the parent has no WtpDA associated with it
     * - which is perfectly legal thanks to this code here.
     */
    ...
    /* persist the queue (we always do that - queuePersits() does cleanup if the
     * queue is empty) This handler is most important for disk queues, it will
     * finally persist the necessary on-disk structures. In theory, other
     * queueing modes may implement their other (non-DA) methods of persisting a
     * queue between runs, but in practice all of this is done via disk queues
     * and DA mode. Anyhow, it doesn't hurt to know that we could extend it here
    ...
    if (pThis->pqParent == NULL)
    {
        /* if we are not a child, we allocated our own mutex, which we now need
         * to destroy */
 ...
 */
rsRetVal qqueueSetFilePrefix(qqueue_t* pThis, uchar* pszPrefix,
                             size_t iLenPrefix)
{
    DEFiRet;
 ...
 * Note that the queue mutex MUST already be locked when this function is
 * called. rgerhards, 2009-06-16
 */
static rsRetVal doEnqSingleObj(qqueue_t* pThis, flowControl_t flowCtlType,
                               smsg_t* pMsg)
{
    ...
    /* handle flow control
     * There are two different flow control mechanisms: basic and advanced flow
     * control. Basic flow control has always been implemented and protects the
     * queue structures in that it makes sure no more data is enqueued than the
     * queue is configured to support. Enhanced flow control is being added
     * today. There are some sources which can easily be stopped, e.g. a file
     * reader. This is the case because it is unlikely that blocking those
     * sources will have negative effects (after all, the file is continued to
     * be written). Other sources can somewhat be blocked (e.g. the kernel log
     * reader or the local log stream reader): in general, nothing is lost if
     * messages from these sources are not picked up immediately. HOWEVER, they
     * can not block for an extended period of time, as this either causes
     * message loss or - even worse - some other bad effects (e.g. unresponsive
     * system in respect to the main system log socket). Finally, there are some
     * (few) sources which can not be blocked at all. UDP syslog is a prime
     * example. If a UDP message is not received, it is simply lost. So we can't
     * do anything against UDP sockets that come in too fast. The core idea of
     * advanced flow control is that we take into account the different natures
     * of the sources and select flow control mechanisms that fit these needs.
     * This also means, in the end result, that non-blockable sources like UDP
     * syslog receive priority in the system. It's a side effect, but a good one
    ...
    if (flowCtlType == eFLOWCTL_FULL_DELAY)
    {
               ...
               !glbl.GetGlobalInputTermState())
        {
             ...
             * TODO/v6(at earliest): check if we could signal the condition
             * during shutdown. However, this requires new queue registries and
             * thus is far to much change for a stable version (and I am still
             * not sure it is worth the effort, given how seldom this situation
             * occurs and how few resources the wakeups need). -- rgerhards,
             * 2012-05-03 In any case, this was the old code (if we do the
             * TODO): pthread_cond_wait(&pThis->belowFullDlyWtrMrk, pThis->mut);
             */
            DBGOPRINT((obj_t*)pThis,
                      ...
                      pThis->iQueueSize);
            timeoutComp(&t, 1000);
            err = pthread_cond_timedwait(&pThis->belowLightDlyWtrMrk,
                                         pThis->mut, &t);
            if (err != 0 && err != ETIMEDOUT)
            {
                /* Something is really wrong now. Report to debug log and abort
                 * the wait. That keeps us running, even though we may lose
                 * messages.
                 */
                DBGOPRINT((obj_t*)pThis,
             ...
             !glbl.GetGlobalInputTermState())
    {
        if (pThis->iQueueSize >= pThis->iLightDlyMrk)
        {
            ...
            timeoutComp(
                &t,
                1000); /* 1000 millisconds = 1 second TODO: make configurable */
            err = pthread_cond_timedwait(&pThis->belowLightDlyWtrMrk,
                                         pThis->mut, &t);
            if (err != 0 && err != ETIMEDOUT)
            {
    ...
    /* from our regular flow control settings, we are now ready to enqueue the
     * object. However, we now need to do a check if the queue permits to add
     * more data. If that is not the case, basic flow control enters the field,
     * which means we wait for the queue to become ready or drop the new
     * message. -- rgerhards, 2008-03-14
     */
    while ((pThis->iMaxQueueSize > 0 &&
            pThis->iQueueSize >= pThis->iMaxQueueSize) ||
           ((pThis->qType == QUEUETYPE_DISK || pThis->bIsDA) &&
            pThis->sizeOnDiskMax != 0 &&
            pThis->tVars.disk.sizeOnDisk > pThis->sizeOnDiskMax))
    {
        ...
        else
        {
            DBGOPRINT(
                (obj_t*)pThis,
                "doEnqSingleObject: queue FULL - waiting %dms to drain.\n",
                pThis->toEnq);
            ...
            const int r =
                pthread_cond_timedwait(&pThis->notFull, pThis->mut, &t);
            if (dbgTimeoutToStderr && r != 0)
            {
                fprintf(stderr,
                        "%lld: queue timeout(%dms), error %d%s, "
                        "lost message %s\n",
                        (long long)time(NULL), pThis->toEnq, r,
                        (r == ETIMEDOUT) ? "[ETIMEDOUT]" : "", pMsg->pszRawMsg);
            }
            if (r == ETIMEDOUT)
            {
                DBGOPRINT(
                    (obj_t*)pThis,
                    "doEnqSingleObject: cond timeout, dropping message!\n");
                STATSCOUNTER_INC(pThis->ctrFDscrd, pThis->mutCtrFDscrd);
                msgDestruct(&pMsg);
                ABORT_FINALIZE(RS_RET_QUEUE_FULL);
            }
            else if (r != 0)
            {
                DBGOPRINT(
                    (obj_t*)pThis,
                    "doEnqSingleObject: cond error %d, dropping message!\n", r);
                STATSCOUNTER_INC(pThis->ctrFDscrd, pThis->mutCtrFDscrd);
                msgDestruct(&pMsg);
                ABORT_FINALIZE(RS_RET_QUEUE_FULL);
            }
            dbgoprint((obj_t*)pThis, "doEnqSingleObject: wait solved queue "
                                     "full condition, enqueing\n");
        }
    }
    ...
    {
        DBGOPRINT((obj_t*)pThis,
                  ".qi file write required for robustness reasons (n=%d)\n",
 ...
 * ------------------------------ */
/* enqueue multiple user data elements at once. The aim is to provide a faster
 * interface for object submission. Uses the multi_submit_t helper object.
                                           ...
                                           multi_submit_t* pMultiSub)
{
    ...
    {
        localRet = doEnqSingleObj(pThis, pMultiSub->ppMsgs[i]->flowCtlType,
                                  (void*)pMultiSub->ppMsgs[i]);
...
static rsRetVal initCryprov(qqueue_t* pThis, struct nvlst* lst)
{
    ...
    if (snprintf((char*)szDrvrName, sizeof(szDrvrName), "lmcry_%s",
                 pThis->cryprovName) == sizeof(szDrvrName))
    {
        LogError(0, RS_RET_ERR,
    ...
    CHKiRet(pThis->cryprov.SetCnfParam(pThis->cryprovData, lst,
                                       CRYPROV_PARAMTYPE_DISK));
...
rsRetVal qqueueApplyCnfParam(qqueue_t* pThis, struct nvlst* lst)
{
    ...
    for (i = 0; i < pblk.nParams; ++i)
    {
        ...
        else if (!strcmp(pblk.descr[i].name,
...
DEFpropSetMeth(qqueue, bSyncQueueFiles, int) DEFpropSetMeth(
    qqueue, iPersistUpdCnt, int) DEFpropSetMeth(qqueue, iDeqtWinFromHr, int)
    DEFpropSetMeth(qqueue, iDeqtWinToHr, int) DEFpropSetMeth(
        qqueue, toQShutdown, long) DEFpropSetMeth(qqueue, toActShutdown, long)
        DEFpropSetMeth(qqueue, toWrkShutdown, long) DEFpropSetMeth(
            qqueue, toEnq, long) DEFpropSetMeth(qqueue, iHighWtrMrk, int)
            DEFpropSetMeth(qqueue, iLowWtrMrk,
                           int) DEFpropSetMeth(qqueue, iDiscardMrk, int)
                DEFpropSetMeth(qqueue, iDiscardSeverity,
                               int) DEFpropSetMeth(qqueue, iLightDlyMrk, int)
                    DEFpropSetMeth(qqueue, iNumWorkerThreads, int)
                        DEFpropSetMeth(qqueue, iMinMsgsPerWrkr, int)
                            DEFpropSetMeth(qqueue, bSaveOnShutdown,
                                           int) DEFpropSetMeth(qqueue, pAction,
                                                               action_t*)
                                DEFpropSetMeth(qqueue, iDeqSlowdown, int)
                                    DEFpropSetMeth(qqueue, iDeqBatchSize, int)
                                        DEFpropSetMeth(qqueue, iMinDeqBatchSize,
                                                       int)
                                            DEFpropSetMeth(qqueue,
                                                           sizeOnDiskMax, int64)
                                                DEFpropSetMeth(qqueue,
                                                               iSmpInterval,
                                                               int)
 ...
 */
#define isProp(name)                                                           \
    !rsCStrSzStrCmp(pProp->pcsName, (uchar*)name, sizeof(name) - 1)
                                                    static rsRetVal
    qqueueSetProperty(qqueue_t* pThis, var_t* pProp)
{
    DEFiRet;
