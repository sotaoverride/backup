 ...
 * @next_position: char number of the last token from
...
/* --- defines --- */
#define to_lower(c)                                                            \
    ((guchar)(((((guchar)(c)) >= 'A' && ((guchar)(c)) <= 'Z') * ('a' - 'A')) | \
...
static const GScannerConfig g_scanner_config_template = {
    ...
    (G_CSET_a_2_z "_" G_CSET_A_2_Z G_CSET_DIGITS G_CSET_LATINS
...
/* --- prototypes --- */
static inline GScannerKey* g_scanner_lookup_internal(GScanner* scanner,
                                                     guint scope_id,
                                                     const gchar* symbol);
static gboolean g_scanner_key_equal(gconstpointer v1, gconstpointer v2);
static guint g_scanner_key_hash(gconstpointer v);
static void g_scanner_get_token_ll(GScanner* scanner, GTokenType* token_p,
                                   GTokenValue* value_p, guint* line_p,
                                   guint* position_p);
static void g_scanner_get_token_i(GScanner* scanner, GTokenType* token_p,
                                  GTokenValue* value_p, guint* line_p,
                                  guint* position_p);

static guchar g_scanner_peek_next_char(GScanner* scanner);
static guchar g_scanner_get_char(GScanner* scanner, guint* line_p,
                                 guint* position_p);
static void g_scanner_msg_handler(GScanner* scanner, gchar* message,
                                  gboolean is_error);

/* --- functions --- */
static inline gint g_scanner_char_2_num(guchar c, guchar base)
{
    if (c >= '0' && c <= '9')
        c -= '0';
    else if (c >= 'A' && c <= 'Z')
...
GScanner* g_scanner_new(const GScannerConfig* config_templ)
{
    ...
    scanner->config->cset_identifier_first =
        config_templ->cset_identifier_first;
    ...
    scanner->config->scan_identifier_1char =
        config_templ->scan_identifier_1char;
    ...
    scanner->symbol_table =
        g_hash_table_new(g_scanner_key_hash, g_scanner_key_equal);
    scanner->input_fd = -1;
    scanner->text = NULL;
...
static inline void g_scanner_free_value(GTokenType* token_p,
                                        GTokenValue* value_p)
{
    switch (*token_p)
    {
...
static void g_scanner_destroy_symbol_table_entry(gpointer _key, gpointer _value,
                                                 gpointer _data)
{
 ...
 *
 */
gpointer g_scanner_scope_lookup_symbol(GScanner* scanner, guint scope_id,
                                       const gchar* symbol)
{
    GScannerKey* key;
...
static void g_scanner_foreach_internal(gpointer _key, gpointer _value,
                                       gpointer _user_data)
{
    GScannerKey* key;
    gpointer* d;
...
GTokenType g_scanner_peek_next_token(GScanner* scanner)
{
    ...
    if (scanner->next_token == G_TOKEN_NONE)
    {
        ...
        g_scanner_get_token_i(scanner, &scanner->next_token,
                              &scanner->next_value, &scanner->next_line,
...
GTokenType g_scanner_get_next_token(GScanner* scanner)
{
    ...
    if (scanner->next_token != G_TOKEN_NONE)
    {
        ...
        g_scanner_get_token_i(scanner, &scanner->token, &scanner->value,
 ...
 */
void g_scanner_unexp_token(GScanner* scanner, GTokenType expected_token,
                           const gchar* identifier_spec,
                           const gchar* symbol_spec, const gchar* symbol_name,
                           const gchar* message, gint is_error)
{
    ...
    switch (scanner->token)
    {
        ...
        default:
            if (scanner->token >= 1 && scanner->token <= 255)
            {
                if ((scanner->token >= ' ' && scanner->token <= '~') ||
                    strchr(scanner->config->cset_identifier_first,
                           scanner->token) ||
                    strchr(scanner->config->cset_identifier_nth,
                           scanner->token))
                    _g_snprintf(token_string, token_string_len,
                                "character '%c'", scanner->token);
                else
                    _g_snprintf(token_string, token_string_len,
                                "character '\\%o'", scanner->token);
                break;
            }
            else if (!scanner->config->symbol_2_token)
            {
                _g_snprintf(token_string, token_string_len,
                            "(unknown) token <%d>", scanner->token);
                break;
            }
            /* fall through */
        case G_TOKEN_SYMBOL:
            if (expected_token == G_TOKEN_SYMBOL ||
                ...
                print_unexp = FALSE;
            if (symbol_name)
                _g_snprintf(token_string, token_string_len, "%s%s '%s'",
                            print_unexp ? "" : "invalid ", symbol_spec,
                            symbol_name);
            else
                _g_snprintf(token_string, token_string_len, "%s%s",
                            print_unexp ? "" : "invalid ", symbol_spec);
            break;
        ...
        case G_TOKEN_CHAR:
            _g_snprintf(token_string, token_string_len, "character '%c'",
        ...
        case G_TOKEN_IDENTIFIER_NULL:
            if (expected_token == G_TOKEN_IDENTIFIER ||
                ...
                print_unexp = FALSE;
            _g_snprintf(token_string, token_string_len, "%s%s '%s'",
                        print_unexp ? "" : "invalid ", identifier_spec,
        ...
        case G_TOKEN_HEX:
            if (scanner->config->store_int64)
                ...
                _g_snprintf(token_string, token_string_len, "number '%lu'",
                            scanner->value.v_int);
            break;

        case G_TOKEN_FLOAT:
            _g_snprintf(token_string, token_string_len, "number '%.3f'",
    ...
    switch (expected_token)
    {
        ...
        default:
            if (expected_token >= 1 && expected_token <= 255)
            {
                if ((expected_token >= ' ' && expected_token <= '~') ||
                    ...
                    strchr(scanner->config->cset_identifier_nth,
                           expected_token))
                    _g_snprintf(expected_string, expected_string_len,
        ...
        case G_TOKEN_SYMBOL:
            ...
            _g_snprintf(expected_string, expected_string_len, "%s%s",
                        need_valid ? "valid " : "", symbol_spec);
            /* FIXME: should we attempt to lookup the symbol_name for
             * symbol_2_token? */
            break;
        case G_TOKEN_CHAR:
        ...
        case G_TOKEN_IDENTIFIER_NULL:
            need_valid = (scanner->token == G_TOKEN_IDENTIFIER_NULL ||
                          scanner->token == G_TOKEN_IDENTIFIER);
            _g_snprintf(expected_string, expected_string_len, "%s%s",
                        need_valid ? "valid " : "", identifier_spec);
            break;
        case G_TOKEN_COMMENT_SINGLE:
            tstring = "single-line";
    ...
    {
        msg_handler(scanner, "failure around %s%s%s", token_string,
                    message_prefix, message);
    }
    else if (expected_token == G_TOKEN_NONE)
    {
        if (print_unexp)
            msg_handler(scanner, "unexpected %s%s%s", token_string,
                        message_prefix, message);
        else
            msg_handler(scanner, "%s%s%s", token_string, message_prefix,
                        message);
    }
    else
    {
        if (print_unexp)
            msg_handler(scanner, "unexpected %s, expected %s%s%s", token_string,
                        expected_string, message_prefix, message);
        else
            msg_handler(scanner, "%s, expected %s%s%s", token_string,
                        expected_string, message_prefix, message);
    }
                                  ...
                                  guint* position_p)
{
    ...
    if (*token_p == G_TOKEN_INT && scanner->config->int_2_float)
    {
                                   ...
                                   guint* position_p)
{
    ...
    do /* while (ch != 0) */
    {
        ...
        switch (ch)
        {
            ...
            case '/':
                ...
                while ((ch = g_scanner_get_char(scanner, line_p, position_p)) !=
                       0)
                {
                        ...
                        if (ch == '\\')
                        {
                            ...
                            switch (ch)
                            {
                                ...
                                case '7':
                                    ...
                                    if (fchar >= '0' && fchar <= '7')
                                    {
                                        ch = g_scanner_get_char(scanner, line_p,
                                                                position_p);
                                        i = i * 8 + ch - '0';
                                        fchar =
                                            g_scanner_peek_next_char(scanner);
                        ...
                        switch (ch)
                        {
                            case '.':
                                if (token != G_TOKEN_INT &&
                                    token != G_TOKEN_OCTAL)
                                {
                                    value.v_error = token == G_TOKEN_FLOAT
    ...
    if (token == G_TOKEN_IDENTIFIER)
    {
        if (config->scan_symbols)
        {
            ...
            scope_id = scanner->scope_id;
            key = g_scanner_lookup_internal(scanner, scope_id,
                                            value.v_identifier);
            if (!key && scope_id && scanner->config->scope_0_fallback)
        ...
        if (token == G_TOKEN_IDENTIFIER && config->scan_identifier_NULL &&
