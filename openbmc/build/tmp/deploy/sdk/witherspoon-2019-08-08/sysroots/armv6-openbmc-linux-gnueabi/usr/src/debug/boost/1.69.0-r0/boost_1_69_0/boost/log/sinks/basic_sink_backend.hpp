 ...
 * all sink backends are required to define. All sink backends have to derive
 * from the class.
 */
...
struct basic_sink_backend
{
    ...
    BOOST_DELETED_FUNCTION(
        basic_sink_backend& operator=(basic_sink_backend const&))
};

/*!
 * \brief A base class for a logging sink backend with message formatting
 * support
 *
 * The \c basic_formatted_sink_backend class template indicates to the frontend
 * that the backend requires logging record formatting.
 *
 * The class allows to request encoding conversion in case if the sink backend
 * requires the formatted string in some particular encoding (e.g. if underlying
 * API supports only narrow or wide characters). In order to perform conversion
 * one should specify the desired final character type in the \c TargetCharT
 * template parameter.
 */
template <typename CharT, typename FrontendRequirementsT = synchronized_feeding>
struct basic_formatted_sink_backend
    : public basic_sink_backend<typename combine_requirements<
          FrontendRequirementsT, formatted_records>::type>
{
  private:
    typedef basic_sink_backend<typename combine_requirements<
        FrontendRequirementsT, formatted_records>::type>
        base_type;
