...
static int padto =
    0; /* pad the output with 0xFF to the end of the final eraseblock */
static int add_cleanmarkers = 1; /* add cleanmarker to output */
static int use_input_cleanmarker_size =
    1; /* use input file's cleanmarker size (default) */
...
static struct option long_options[] = {{"output", 1, NULL, 'o'},
                                       {"input", 1, NULL, 'i'},
                                       {"eraseblock", 1, NULL, 'e'},
                                       {"help", 0, NULL, 'h'},
                                       {"verbose", 0, NULL, 'v'},
...
static const char helptext[] =
    ...
    "                            (usually 16 bytes on NAND, and will be set "
    "to\n"
    "                            this value if left at the default 12). Will "
    "be\n"
    "                            stored in OOB after each physical page "
    "composing\n"
...
static void process_options(int argc, char** argv)
{
    ...
    while ((opt = getopt_long(argc, argv, short_options, long_options, &c)) >=
           0)
    {
        switch (opt)
        {
...
static void clean_sumlist(void)
{
    ...
    if (sum_collected)
    {
        ...
        if (sum_collected->sum_num != 0)
            warnmsg("Ooops, something wrong happened! sum_num != 0, but "
                    "sum_list = null ???");
...
static void dump_sum_records(void)
{
    ...
    isum.hdr_crc =
        cpu_to_je32(mtd_crc32(0, &isum, sizeof(struct jffs2_unknown_node) - 4));
    ...
    while (sum_collected->sum_num)
    {
        switch (je16_to_cpu(sum_collected->sum_list_head->u.nodetype))
        {
            ...
            case JFFS2_NODETYPE_DIRENT:
            {
                ...
                wpage += JFFS2_SUMMARY_DIRENT_SIZE(
                    sum_collected->sum_list_head->d.nsize);
                break;
            }
...
static inline void pad_block_if_less_than(int req, int plus)
{
    ...
    int datasize = req + plus + sum_collected->sum_size +
...
static void flush_buffers(void)
{
    ...
    if ((add_cleanmarkers == 1) && (found_cleanmarkers == 1))
    { /* CLEANMARKER */
        if (data_ofs != cleanmarker_size)
        { /* INODE BUFFER */

            int datasize =
                sum_collected->sum_size + sizeof(struct jffs2_raw_summary) + 8;
            datasize += (4 - (datasize % 4)) % 4;

            /* If we have a full inode buffer, then write out inode and summary
             * data  */
            if (data_ofs + sizeof(struct jffs2_raw_inode) +
                    2 * JFFS2_MIN_DATA_LEN >
                erase_block_size - datasize)
            {
                dump_sum_records();
                write_buff_to_file();
            }
...
{
    pad_block_if_less_than(je32_to_cpu(node->d.totlen),
                           JFFS2_SUMMARY_DIRENT_SIZE(node->d.nsize));
...
{
    pad_block_if_less_than(je32_to_cpu(node->i.totlen),
                           JFFS2_SUMMARY_INODE_SIZE);
    add_sum_inode_mem(node); /* Add inode summary mem to summary list */
    full_write(
        data_buffer + data_ofs, &(node->i),
        je32_to_cpu(node->i.totlen)); /* Write out the inode to inode_buffer */
    padword();
}

static void write_xattr_to_buff(union jffs2_node_union* node)
{
    pad_block_if_less_than(je32_to_cpu(node->x.totlen),
...
{
    pad_block_if_less_than(je32_to_cpu(node->r.totlen),
    ...
    while (p < (file_buffer + inp_size))
    {
        ...
        switch (je16_to_cpu(node->u.nodetype))
        {
            case JFFS2_NODETYPE_INODE:
                bareverbose(
                    verbose,
                    "%8s Inode      node at 0x%08zx, totlen 0x%08x, #ino  %5d, "
                    "version %5d, isize %8d, csize %8d, dsize %8d, offset "
                    "%8d\n",
                ...
                {
                    warnmsg("Wrong node_crc at  0x%08zx, 0x%08x instead of "
                            "0x%08x\n",
                            p - file_buffer, je32_to_cpu(node->i.node_crc),
                            crc);
                ...
                {
                    warnmsg("Wrong data_crc at  0x%08zx, 0x%08x instead of "
                            "0x%08x\n",
                            p - file_buffer, je32_to_cpu(node->i.data_crc),
                            crc);
                    p += PAD(je32_to_cpu(node->i.totlen));
                    continue;
                }
            ...
            case JFFS2_NODETYPE_DIRENT:
                ...
                bareverbose(
                    verbose,
                    "%8s Dirent     node at 0x%08zx, totlen 0x%08x, #pino %5d, "
                    "version %5d, #ino  %8d, nsize %8d, name %s\n",
                    obsolete ? "Obsolete" : "", p - file_buffer,
                ...
                if (crc != je32_to_cpu(node->d.node_crc))
                {
                    warnmsg("Wrong node_crc at  0x%08zx, 0x%08x instead of "
                            "0x%08x\n",
                            p - file_buffer, je32_to_cpu(node->d.node_crc),
                            crc);
                ...
                {
                    warnmsg("Wrong name_crc at  0x%08zx, 0x%08x instead of "
                            "0x%08x\n",
                            p - file_buffer, je32_to_cpu(node->d.name_crc),
                            crc);
                    p += PAD(je32_to_cpu(node->d.totlen));
                    continue;
                }
            ...
            case JFFS2_NODETYPE_XATTR:
                if (je32_to_cpu(node->x.node_crc) == 0xffffffff)
                    obsolete = 1;
                bareverbose(verbose,
                            "%8s Xdatum     node at 0x%08zx, totlen 0x%08x, "
                            "#xid  %5u, version %5u\n",
                            obsolete ? "Obsolete" : "", p - file_buffer,
                ...
                {
                    warnmsg(
                        "Wrong node_crc at 0x%08zx, 0x%08x instead of 0x%08x\n",
                ...
                {
                    warnmsg(
                        "Wrong data_crc at 0x%08zx, 0x%08x instead of 0x%08x\n",
            ...
            case JFFS2_NODETYPE_XREF:
                if (je32_to_cpu(node->r.node_crc) == 0xffffffff)
                    obsolete = 1;
                bareverbose(verbose,
                            "%8s Xref       node at 0x%08zx, totlen 0x%08x, "
                            "#ino  %5u, xid     %5u\n",
                            obsolete ? "Obsolete" : "", p - file_buffer,
            ...
            case JFFS2_NODETYPE_CLEANMARKER:
                ...
                if (!found_cleanmarkers)
                {
                    ...
                    if (add_cleanmarkers == 1 &&
                        use_input_cleanmarker_size == 1)
                    {
