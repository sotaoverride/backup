...
static void g_volume_default_init(GVolumeInterface* iface)
{
     ...
     */
    g_signal_new(I_("changed"), G_TYPE_VOLUME, G_SIGNAL_RUN_LAST,
                 G_STRUCT_OFFSET(GVolumeIface, changed), NULL, NULL,
     ...
     */
    g_signal_new(I_("removed"), G_TYPE_VOLUME, G_SIGNAL_RUN_LAST,
                 G_STRUCT_OFFSET(GVolumeIface, removed), NULL, NULL,
                 g_cclosure_marshal_VOID__VOID, G_TYPE_NONE, 0);
}
 ...
 *
 * Returns: (transfer full) (nullable): a #GMount or %NULL if @volume isn't
 * mounted. The returned object should be unreffed with g_object_unref() when no
 * longer needed.
 */
 ...
 * @mount_operation: (nullable): a #GMountOperation or %NULL to avoid user
 * interaction
                    ...
                    GAsyncReadyCallback callback, gpointer user_data)
{
    ...
    {
        g_task_report_new_error(volume, callback, user_data, g_volume_mount,
                                G_IO_ERROR, G_IO_ERROR_NOT_SUPPORTED,
                                _("volume doesn’t implement mount"));
        return;
    }

    (*iface->mount_fn)(volume, flags, mount_operation, cancellable, callback,
                       user_data);
}
                    ...
                    gpointer user_data)
{
    ...
    if (iface->eject == NULL)
    {
        g_task_report_new_error(volume, callback, user_data,
                                g_volume_eject_with_operation, G_IO_ERROR,
                                   ...
                                   gpointer user_data)
{
    ...
    if (iface->eject == NULL && iface->eject_with_operation == NULL)
    {
        g_task_report_new_error(
            volume, callback, user_data, g_volume_eject_with_operation,
            G_IO_ERROR, G_IO_ERROR_NOT_SUPPORTED,
    ...
    if (iface->eject_with_operation != NULL)
        (*iface->eject_with_operation)(volume, flags, mount_operation,
 ...
 *   volume_activation_root = g_volume_get_activation_root (volume); // assume
 * not NULL
 * ]|
 * then the expression
 * |[<!-- language="C" -->
 ...
 *
 * Returns: (nullable): Sorting key for @volume or %NULL if no such key is
 * available
 *
 * Since: 2.32
 */
