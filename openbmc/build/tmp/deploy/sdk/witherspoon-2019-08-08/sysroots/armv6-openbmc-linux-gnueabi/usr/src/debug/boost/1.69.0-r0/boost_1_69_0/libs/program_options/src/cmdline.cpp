...
vector<option> cmdline::run()
{
    ...
    if ((m_style & allow_short) && (m_style & allow_slash_for_short))
        style_parsers.push_back(
            boost::bind(&cmdline::parse_dos_option, this, _1));
                            ...
                            const vector<style_parser>& style_parsers)
{
    ...
    //
    // Be defensive:
    // will have no original token if option created by
    ...
    try
    {
        ...
        const option_description* xd = m_desc->find_nothrow(
            opt.string_key, is_style_active(allow_guessing),
            is_style_active(long_case_insensitive),
            is_style_active(short_case_insensitive));
        ...
        if (present_tokens >= min_tokens)
        {
            ...
            // Grab min_tokens values from other_tokens, but only if those
            // tokens are not recognized as options themselves.
            ...
            for (; !other_tokens.empty() && min_tokens--;)
            {
                ...
                // we use style parsers to check if it is syntactically an
                // option, additionally we check if an option_description exists
                vector<option> followed_option;
                vector<string> next_token(1, other_tokens[0]);
                for (unsigned i = 0;
                     followed_option.empty() && i < style_parsers.size(); ++i)
                {
    ...
    // use only original token for unknown_option / ambiguous_option since by
    // definition
...
vector<option> cmdline::parse_long_option(vector<string>& args)
{
    ...
    if (tok.size() >= 3 && tok[0] == '-' && tok[1] == '-')
    {
        ...
        if (p != tok.npos)
        {
            ...
            if (adjacent.empty())
                boost::throw_exception(invalid_command_line_syntax(
                    invalid_command_line_syntax::empty_adjacent_parameter, name,
                    name, get_canonical_option_prefix()));
        }
        else
        {
            name = tok.substr(2);
