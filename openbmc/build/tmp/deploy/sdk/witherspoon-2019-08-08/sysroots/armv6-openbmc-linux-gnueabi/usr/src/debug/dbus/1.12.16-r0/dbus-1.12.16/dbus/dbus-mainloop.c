...
static void free_watch_table_entry(void* data)
{
    ...
    for (watch = _dbus_list_pop_first(watches); watch != NULL;
...
DBusLoop* _dbus_loop_new(void)
{
    ...
    loop->watches =
        _dbus_hash_table_new(DBUS_HASH_POLLABLE, NULL, free_watch_table_entry);
    ...
    if (loop->refcount == 0)
    {
        while (loop->need_dispatch)
        {
            DBusConnection* connection =
                _dbus_list_pop_first(&loop->need_dispatch);
...
static dbus_bool_t gc_watch_table_entry(DBusLoop* loop, DBusList** watches,
                                        DBusPollable fd)
{
...
static void refresh_watches_for_fd(DBusLoop* loop, DBusList** watches,
                                   DBusPollable fd)
{
    DBusList* link;
 ...
 * to do this.
 */
static dbus_bool_t check_timeout(long tv_sec, long tv_usec,
                                 TimeoutCallback* tcb, int* timeout)
{
    long sec_remaining;
    long msec_remaining;
...
#define N_STACK_DESCRIPTORS 64
    ...
    _dbus_verbose(
        ...
        loop->depth, loop->timeout_count, loop->watch_count);
#endif
