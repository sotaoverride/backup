...
static void g_socket_listener_finalize(GObject* object)
{
    ...
    /* Do not explicitly close the sockets. Instead, let them close themselves
     * if their final reference is dropped, but keep them open if a reference is
...
static void g_socket_listener_get_property(GObject* object, guint prop_id,
...
static void g_socket_listener_set_property(GObject* object, guint prop_id,
                                           const GValue* value,
                                           GParamSpec* pspec)
{
    GSocketListener* listener = G_SOCKET_LISTENER(object);
...
static void g_socket_listener_class_init(GSocketListenerClass* klass)
{
    ...
    g_object_class_install_property(
        ...
        g_param_spec_int(
            "listen-backlog", P_("Listen backlog"),
            P_("outstanding connections in the listen queue"), 0, 2000, 10,
    ...
    signals[EVENT] = g_signal_new(
        ...
        G_STRUCT_OFFSET(GSocketListenerClass, event), NULL, NULL, NULL,
        G_TYPE_NONE, 2, G_TYPE_SOCKET_LISTENER_EVENT, G_TYPE_SOCKET);
...
gboolean g_socket_listener_add_socket(GSocketListener* listener,
                                      GSocket* socket, GObject* source_object,
                                      GError** error)
{
 ...
 * @source_object: (nullable): Optional #GObject identifying this source
 * @effective_address: (out) (optional): location to store the address that was
 * bound to, or %NULL.
 * @error: #GError for error reporting, or %NULL to ignore.
 *
...
gboolean g_socket_listener_add_address(
    GSocketListener* listener, GSocketAddress* address, GSocketType type,
    GSocketProtocol protocol, GObject* source_object,
    GSocketAddress** effective_address, GError** error)
{
    ...
    g_signal_emit(listener, signals[EVENT], 0, G_SOCKET_LISTENER_BINDING,
                  socket);
    ...
    g_signal_emit(listener, signals[EVENT], 0, G_SOCKET_LISTENER_BOUND, socket);
    g_signal_emit(listener, signals[EVENT], 0, G_SOCKET_LISTENER_LISTENING,
                  socket);
    ...
    g_signal_emit(listener, signals[EVENT], 0, G_SOCKET_LISTENER_LISTENED,
                  socket);
...
gboolean g_socket_listener_add_inet_port(GSocketListener* listener,
                                         guint16 port, GObject* source_object,
                                         GError** error)
{
    ...
    socket6 = g_socket_new(G_SOCKET_FAMILY_IPV6, G_SOCKET_TYPE_STREAM,
     ...
     */
    {
        ...
        g_signal_emit(listener, signals[EVENT], 0, G_SOCKET_LISTENER_BOUND,
                      socket6);
        g_signal_emit(listener, signals[EVENT], 0, G_SOCKET_LISTENER_LISTENING,
                      socket6);
     ...
     */
    {
        socket4 = g_socket_new(G_SOCKET_FAMILY_IPV4, G_SOCKET_TYPE_STREAM,
...
static GList* add_sources(GSocketListener* listener, GSocketSourceFunc callback,
                          ...
                          GMainContext* context)
{
    ...
    for (i = 0; i < listener->priv->sockets->len; i++)
    {
        ...
        g_source_set_callback(source, (GSourceFunc)callback, callback_data,
                              NULL);
...
static gboolean accept_callback(GSocket* socket, GIOCondition condition,
                                gpointer user_data)
{
 ...
 * @listener: a #GSocketListener
 * @source_object: (out) (transfer none) (optional) (nullable): location where
 * #GObject pointer will be stored, or %NULL.
                                         ...
                                         GError** error)
{
    ...
    if (listener->priv->sockets->len == 1)
    {
        ...
        if (!g_socket_condition_wait(accept_socket, G_IO_IN, cancellable,
                                     error))
            return NULL;
    }
    else
    {
        ...
        data.loop = loop;
        sources = add_sources(listener, accept_callback, &data, cancellable,
    ...
    if (source_object)
        *source_object =
            g_object_get_qdata(G_OBJECT(accept_socket), source_quark);
                                            ...
                                            GError** error)
{
    ...
    socket = g_socket_listener_accept_socket(listener, source_object,
                                             cancellable, error);
    if (socket == NULL)
        return NULL;
...
static void accept_socket_async_data_free(AcceptSocketAsyncData* data)
{
    free_sources(data->sources);
    g_free(data);
}

static gboolean accept_ready(GSocket* accept_socket, GIOCondition condition,
                             gpointer user_data)
{
    ...
    socket =
        g_socket_accept(accept_socket, g_task_get_cancellable(task), &error);
    if (socket)
    {
        source_object =
            g_object_get_qdata(G_OBJECT(accept_socket), source_quark);
        if (source_object)
            g_object_set_qdata_full(G_OBJECT(task), source_quark,
                                    g_object_ref(source_object),
                                    g_object_unref);
                                           ...
                                           gpointer user_data)
{
    ...
    data->sources = add_sources(listener, accept_ready, task, cancellable,
 ...
 * @source_object: (out) (transfer none) (optional) (nullable): Optional
 * #GObject identifying this source
...
void g_socket_listener_accept_async(GSocketListener* listener,
                                    ...
                                    gpointer user_data)
{
    g_socket_listener_accept_socket_async(listener, cancellable, callback,
                                          user_data);
}
                                                   ...
                                                   GError** error)
{
    ...
    socket = g_socket_listener_accept_socket_finish(listener, result,
                                                    source_object, error);
    if (socket == NULL)
        return NULL;
                                            ...
                                            GError** error)
{
    ...
    while (attempts--)
    {
        ...
        socket4 =
            g_socket_new(G_SOCKET_FAMILY_IPV4, G_SOCKET_TYPE_STREAM,
                         G_SOCKET_PROTOCOL_DEFAULT, socket6 ? NULL : error);
    ...
    while (sockets_to_close)
    {
        ...
        sockets_to_close =
            g_slist_delete_link(sockets_to_close, sockets_to_close);
    }
