...
static int service_dispatch_inotify_io(sd_event_source* source, int fd,
...
static int service_dispatch_watchdog(sd_event_source* source, usec_t usec,
                                     void* userdata);
static int service_dispatch_exec_io(sd_event_source* source, int fd,
                                    uint32_t events, void* userdata);
...
static void service_unwatch_pid_file(Service* s)
{
    ...
    log_unit_debug(UNIT(s), "Stopping watch for PID file %s",
...
static int service_set_main_pid(Service* s, pid_t pid)
{
    ...
    if (s->main_pid_alien)
        log_unit_warning(UNIT(s),
                         ...
                         " which is not our child. We'll most likely not "
...
static void service_start_watchdog(Service* s)
{
    ...
    if (s->watchdog_event_source)
    {
        r = sd_event_source_set_time(
            ...
            usec_add(s->watchdog_timestamp.monotonic, watchdog_usec));
        if (r < 0)
        {
    ...
    {
        r = sd_event_add_time(
            UNIT(s)->manager->event, &s->watchdog_event_source, CLOCK_MONOTONIC,
        ...
        (void)sd_event_source_set_description(s->watchdog_event_source,
                                              "service-watchdog");
...
static void service_extend_event_source_timeout(Service* s,
                                                ...
                                                usec_t extended)
{
    ...
    /* Extends the specified event source timer to at least the specified time,
     * unless it is already later anyway. */
    ...
    if (r < 0)
    {
        ...
        log_unit_warning_errno(UNIT(s), r,
                               "Failed to retrieve timeout time for event "
                               "source '%s', ignoring: %m",
                               strna(desc));
        return;
    }
    ...
    if (current >=
        extended) /* Current timeout is already longer, ignore this. */
        return;
    ...
    if (r < 0)
    {
        ...
        log_unit_warning_errno(
            UNIT(s), r,
            "Failed to set timeout time for even source '%s', ignoring %m",
            strna(desc));
    }
}
...
static void service_override_watchdog_timeout(Service* s,
                                              usec_t watchdog_override_usec)
{
    ...
    log_unit_debug(UNIT(s), "watchdog_override_usec=" USEC_FMT,
                   s->watchdog_override_usec);
}
...
static int on_fd_store_io(sd_event_source* e, int fd, uint32_t revents,
                          void* userdata)
{
    ...
    /* If we get either EPOLLHUP or EPOLLERR, it's time to remove this entry
     * from the fd store */
...
static int service_add_fd_store(Service* s, int fd, const char* name)
{
    ...
    if (s->n_fd_store >= s->n_fd_store_max)
        return -EXFULL; /* Our store is full.
                         * Use this errno rather than E[NM]FILE to distinguish
                         * from the case where systemd itself hits the file
                         * limit. */
    ...
    { /* EPERM indicates fds that aren't pollable, which is OK */
        ...
        (void)sd_event_source_set_description(fs->event_source,
                                              "service-fd-store");
    ...
    while (fdset_size(fds) > 0)
    {
        ...
        if (r == -EXFULL)
            return log_unit_warning_errno(
                UNIT(s), r,
                "Cannot store more fds than FileDescriptorStoreMax=%u, closing "
                "remaining.",
                s->n_fd_store_max);
        if (r < 0)
        ...
        if (r > 0)
            log_unit_debug(UNIT(s), "Added fd %u (%s) to fd store.", fd,
                           strna(name));
        fd = -1;
    }
...
static void service_remove_fd_store(Service* s, const char* name)
{
    ...
    LIST_FOREACH_SAFE(fd_store, fs, n, s->fd_store)
    {
        ...
        log_unit_debug(UNIT(s),
                       "Got explicit request to remove fd %i (%s), closing.",
                       fs->fd, name);
        service_fd_store_unlink(fs);
    }
}
...
static int service_arm_timer(Service* s, usec_t usec)
{
    ...
    r = sd_event_add_time(UNIT(s)->manager->event, &s->timer_event_source,
                          CLOCK_MONOTONIC, usec, 0, service_dispatch_timer, s);
    if (r < 0)
        return r;

    (void)sd_event_source_set_description(s->timer_event_source,
                                          "service-timer");
...
static int service_verify(Service* s)
{
    ...
    {
        /* FailureAction= only makes sense if one of the start or stop commands
         * is specified. SuccessAction= will be executed unconditionally if no
         * commands are specified. Hence,
         * either a command or SuccessAction= are required. */

        log_unit_error(UNIT(s), "Service has no ExecStart=, ExecStop=, or "
    ...
    if (s->type != SERVICE_ONESHOT && !s->exec_command[SERVICE_EXEC_START])
    {
        log_unit_error(UNIT(s),
                       "Service has no ExecStart= setting, which is only "
    ...
    {
        log_unit_error(
            UNIT(s), "Service has no ExecStart= and no SuccessAction= settings "
                     "and does not have RemainAfterExit=yes set. Refusing.");
        return -ENOEXEC;
    }
    ...
    {
        log_unit_error(UNIT(s),
                       "Service has more than one ExecStart= setting, which is "
    ...
    {
        log_unit_error(UNIT(s),
                       "Service has Restart= setting other than no, which "
                       "isn't allowed for Type=oneshot services. Refusing.");
        return -ENOEXEC;
    }
    ...
    {
        log_unit_error(UNIT(s),
                       "Service has RestartForceStatus= set, which isn't "
                       "allowed for Type=oneshot services. Refusing.");
        return -ENOEXEC;
    }
    ...
    {
        log_unit_error(UNIT(s), "Service is of type D-Bus but no D-Bus service "
                                "name has been specified. Refusing.");
        return -ENOEXEC;
    }

    if (s->bus_name && s->type != SERVICE_DBUS)
        log_unit_warning(UNIT(s), "Service has a D-Bus service name specified, "
                                  "but is not of type dbus. Ignoring.");

    if (s->exec_context.pam_name &&
        !IN_SET(s->kill_context.kill_mode, KILL_CONTROL_GROUP, KILL_MIXED))
    {
        log_unit_error(UNIT(s), "Service has PAM enabled. Kill mode must be "
                                "set to 'control-group' or 'mixed'. Refusing.");
        return -ENOEXEC;
    }

    if (s->usb_function_descriptors && !s->usb_function_strings)
        log_unit_warning(UNIT(s),
                         "Service has USBFunctionDescriptors= setting, but no "
                         "USBFunctionStrings=. Ignoring.");

    if (!s->usb_function_descriptors && s->usb_function_strings)
        log_unit_warning(UNIT(s), "Service has USBFunctionStrings= setting, "
                                  "but no USBFunctionDescriptors=. Ignoring.");

    if (s->runtime_max_usec != USEC_INFINITY && s->type == SERVICE_ONESHOT)
        log_unit_warning(UNIT(s), "MaxRuntimeSec= has no effect in combination "
...
static int service_add_default_dependencies(Service* s)
{
    ...
    if (MANAGER_IS_SYSTEM(UNIT(s)->manager))
    {
        ...
        r = unit_add_two_dependencies_by_name(
            UNIT(s), UNIT_AFTER, UNIT_REQUIRES, SPECIAL_SYSINIT_TARGET, true,
    ...
    else
    {
        ...
        r = unit_add_dependency_by_name(UNIT(s), UNIT_REQUIRES,
                                        SPECIAL_BASIC_TARGET, true,
                                        UNIT_DEPENDENCY_DEFAULT);
        if (r < 0)
            return r;
    }
     ...
     * even require it. */
    r = unit_add_dependency_by_name(UNIT(s), UNIT_AFTER, SPECIAL_BASIC_TARGET,
    ...
    return unit_add_two_dependencies_by_name(
        UNIT(s), UNIT_BEFORE, UNIT_CONFLICTS, SPECIAL_SHUTDOWN_TARGET, true,
        UNIT_DEPENDENCY_DEFAULT);
}
...
static void service_fix_output(Service* s)
{
    ...
    /* If nothing has been explicitly configured, patch default output in. If
     * input is socket/tty we avoid this however, since in that case we want
     * output to default to the same place as we read input from. */
...
static int service_setup_bus_name(Service* s)
{
    ...
    r = unit_add_dependency_by_name(UNIT(s), UNIT_REQUIRES, SPECIAL_DBUS_SOCKET,
                                    true, UNIT_DEPENDENCY_FILE);
    if (r < 0)
        return log_unit_error_errno(
            UNIT(s), r,
            "Failed to add dependency on " SPECIAL_DBUS_SOCKET ": %m");

    /* We always want to be ordered against dbus.socket if both are in the
     * transaction. */
    r = unit_add_dependency_by_name(UNIT(s), UNIT_AFTER, SPECIAL_DBUS_SOCKET,
                                    true, UNIT_DEPENDENCY_FILE);
    if (r < 0)
        return log_unit_error_errno(
            UNIT(s), r,
    ...
    if (r == -EEXIST)
        return log_unit_error_errno(UNIT(s), r,
                                    "Two services allocated for the same bus "
                                    "name %s, refusing operation.",
                                    s->bus_name);
    if (r < 0)
        return log_unit_error_errno(UNIT(s), r, "Cannot watch bus name %s: %m",
                                    s->bus_name);
...
static int service_add_extras(Service* s)
{
    ...
    if (s->notify_access == NOTIFY_NONE &&
        (s->type == SERVICE_NOTIFY || s->watchdog_usec > 0 ||
         s->n_fd_store_max > 0))
...
{
    char buf_restart[FORMAT_TIMESPAN_MAX], buf_start[FORMAT_TIMESPAN_MAX],
        buf_stop[FORMAT_TIMESPAN_MAX];
    ...
    if (s->main_pid > 0)
        fprintf(f,
                ...
                "%sMain PID Known: %s\n"
                "%sMain PID Alien: %s\n",
                prefix, s->main_pid, prefix, yes_no(s->main_pid_known), prefix,
                yes_no(s->main_pid_alien));
    ...
    if (UNIT_ISSET(s->accept_socket))
        fprintf(f, "%sAccept Socket: %s\n", prefix,
                UNIT_DEREF(s->accept_socket)->id);
    ...
    fprintf(f,
            ...
            format_timespan(buf_restart, sizeof(buf_restart), s->restart_usec,
                            USEC_PER_SEC),
            prefix,
            format_timespan(buf_start, sizeof(buf_start), s->timeout_start_usec,
                            USEC_PER_SEC),
            prefix,
            format_timespan(buf_stop, sizeof(buf_stop), s->timeout_stop_usec,
                            USEC_PER_SEC),
            prefix,
            format_timespan(buf_runtime, sizeof(buf_runtime),
                            s->runtime_max_usec, USEC_PER_SEC),
            prefix,
            format_timespan(buf_watchdog, sizeof(buf_watchdog),
                            s->watchdog_usec, USEC_PER_SEC));
...
static int service_is_suitable_main_pid(Service* s, pid_t pid, int prio)
{
    ...
    /* Checks whether the specified PID is suitable as main PID for this
     * service. returns negative if not, 0 if the PID is questionnable but
     * should be accepted if the source of configuration is trusted. > 0 if the
     * PID is good */

    if (pid == getpid_cached() || pid == 1)
    {
        log_unit_full(UNIT(s), prio, 0,
    ...
    if (pid == s->control_pid)
    {
        log_unit_full(
            UNIT(s), prio, 0,
            "New main PID " PID_FMT " is the control process, refusing.", pid);
        return -EPERM;
    }
    ...
    {
        log_unit_full(UNIT(s), prio, 0,
                      "New main PID " PID_FMT " does not exist or is a zombie.",
                      pid);
        return -ESRCH;
    }
    ...
    if (owner == UNIT(s))
    {
        log_unit_debug(
            UNIT(s),
            "New main PID " PID_FMT " belongs to service, we are happy.", pid);
        return 1; /* Yay, it's definitely a good PID */
    }

    return 0; /* Hmm it's a suspicious PID, let's accept it if configuration
                 source is trusted */
}
...
static int service_load_pid_file(Service* s, bool may_warn)
{
    ...
    if (fd == -ENOLINK)
    {
        log_unit_full(UNIT(s), LOG_DEBUG, fd,
                      "Potentially unsafe symlink chain, will now retry with "
                      "relaxed checks: %s",
                      s->pid_file);
    ...
    /* Let's read the PID file now that we chased it down. But we need to
     * convert the O_PATH fd chase_symlinks() returned us into a proper fd
     * first. */
    xsprintf(procfs, "/proc/self/fd/%i", fd);
    r = read_one_line_file(procfs, &k);
    if (r < 0)
        return log_unit_error_errno(UNIT(s), r,
                                    "Can't convert PID files %s O_PATH file "
                                    "descriptor to proper file descriptor: %m",
                                    s->pid_file);

    r = parse_pid(k, &pid);
    if (r < 0)
        return log_unit_full(UNIT(s), prio, r,
                             "Failed to parse PID from file %s: %m",
                             s->pid_file);
    ...
    if (r == 0)
    {
        ...
        {
            log_unit_error(UNIT(s),
                           "Refusing to accept PID outside of service control "
                           "group, acquired through unsafe symlink chain: %s",
                           s->pid_file);
            return -EPERM;
        }

        /* Hmm, it's not clear if the new main PID is safe. Let's allow this if
         * the PID file is owned by root */

        if (fstat(fd, &st) < 0)
            return log_unit_error_errno(
                UNIT(s), errno, "Failed to fstat() PID file O_PATH fd: %m");

        if (st.st_uid != 0)
        {
            log_unit_error(UNIT(s),
                           "New main PID " PID_FMT
                           " does not belong to service, and PID file is not "
        ...
        log_unit_debug(UNIT(s),
                       "New main PID " PID_FMT
                       " does not belong to service, but we'll accept it since "
                       "PID file is owned by root.",
                       pid);
    }

    if (s->main_pid_known)
    {
        log_unit_debug(UNIT(s), "Main PID changing: " PID_FMT " -> " PID_FMT,
                       s->main_pid, pid);
    ...
    if (r < 0) /* FIXME: we need to do something here */
        return log_unit_warning_errno(
            UNIT(s), r, "Failed to watch PID " PID_FMT " for service: %m", pid);
...
static void service_search_main_pid(Service* s)
{
    ...
    if (r < 0)
        ...
        log_unit_warning_errno(UNIT(s), r,
                               "Failed to watch PID " PID_FMT " from: %m", pid);
}
...
static void service_set_state(Service* s, ServiceState state)
{
    ...
    if (!IN_SET(state, SERVICE_START_PRE, SERVICE_START, SERVICE_START_POST,
                ...
                SERVICE_STOP_WATCHDOG, SERVICE_STOP_SIGTERM,
    ...
    if (!IN_SET(state, SERVICE_START, SERVICE_START_POST, SERVICE_RUNNING,
                SERVICE_RELOAD, SERVICE_STOP, SERVICE_STOP_WATCHDOG,
    ...
    if (state != SERVICE_START)
        s->exec_fd_event_source =
            sd_event_source_unref(s->exec_fd_event_source);
    ...
    unit_notify(
        UNIT(s), table[old_state], table[state],
...
static usec_t service_coldplug_timeout(Service* s)
{
    ...
    switch (s->deserialized_state)
    {
        ...
        case SERVICE_RELOAD:
            return usec_add(UNIT(s)->state_change_timestamp.monotonic,
                            s->timeout_start_usec);

        case SERVICE_RUNNING:
            return usec_add(UNIT(s)->active_enter_timestamp.monotonic,
                            s->runtime_max_usec);
...
static int service_coldplug(Unit* u)
{
    ...
    if (s->main_pid > 0 && pid_is_unwaited(s->main_pid) &&
    ...
    if (s->control_pid > 0 && pid_is_unwaited(s->control_pid) &&
        IN_SET(s->deserialized_state, SERVICE_START_PRE, SERVICE_START,
               SERVICE_START_POST, SERVICE_RELOAD, SERVICE_STOP,
               SERVICE_STOP_WATCHDOG, SERVICE_STOP_SIGTERM,
               SERVICE_STOP_SIGKILL, SERVICE_STOP_POST, SERVICE_FINAL_SIGTERM,
               SERVICE_FINAL_SIGKILL))
    {
...
static int service_collect_fds(Service* s, int** fds, char*** fd_names,
                               size_t* n_socket_fds, size_t* n_storage_fds)
{
    ...
    if (s->n_fd_store > 0)
    {
        ...
        nl = reallocarray(rfd_names, rn_socket_fds + s->n_fd_store + 1,
                          sizeof(char*));
        if (!nl)
            return -ENOMEM;
...
static int
    service_allocate_exec_fd_event_source(Service* s, int fd,
                                          sd_event_source** ret_event_source)
{
    ...
    r = sd_event_add_io(UNIT(s)->manager->event, &source, fd, 0,
                        service_dispatch_exec_io, s);
    if (r < 0)
        return log_unit_error_errno(
            UNIT(s), r, "Failed to allocate exec_fd event source: %m");

    /* This is a bit lower priority than SIGCHLD, as that carries a lot more
     * interesting failure information */
    ...
    if (r < 0)
        return log_unit_error_errno(
            UNIT(s), r, "Failed to pass ownership of fd to event source: %m");

    *ret_event_source = TAKE_PTR(source);
    return 0;
}

static int service_allocate_exec_fd(Service* s,
                                    sd_event_source** ret_event_source,
                                    int* ret_exec_fd)
{
                         ...
                         ExecFlags flags, pid_t* _pid)
{
    ...
    r = unit_prepare_exec(
        UNIT(s)); /* This realizes the cgroup, among other things */
    if (r < 0)
        return r;

    if (flags & EXEC_IS_CONTROL)
    {
        /* If this is a control process, mask the permissions/chroot application
        ...
        s->exec_context.std_error == EXEC_OUTPUT_SOCKET)
    {
        ...
        log_unit_debug(UNIT(s), "Passing %zu fds to service",
    ...
    if (s->socket_fd >= 0)
    {
        ...
        /* If this is a per-connection service instance, let's set $REMOTE_ADDR
         * and $REMOTE_PORT to something useful. Note that we do this only when
         * we are still connected at this point in time, which we might very
         * well not be. Hence we ignore all errors when retrieving peer
         * information (as that might result
    ...
    {
        if (asprintf(our_env + n_env++, "SERVICE_RESULT=%s",
                     service_result_to_string(s->result)) < 0)
            return -ENOMEM;

        if (s->main_exec_status.pid > 0 &&
            dual_timestamp_is_set(&s->main_exec_status.exit_timestamp))
        {
            if (asprintf(our_env + n_env++, "EXIT_CODE=%s",
                         sigchld_code_to_string(s->main_exec_status.code)) < 0)
                return -ENOMEM;

            if (s->main_exec_status.code == CLD_EXITED)
                r = asprintf(our_env + n_env++, "EXIT_STATUS=%i",
                             s->main_exec_status.status);
            else
                r = asprintf(our_env + n_env++, "EXIT_STATUS=%s",
                             signal_to_string(s->main_exec_status.status));
            if (r < 0)
                return -ENOMEM;
        }
    }
    ...
    SET_FLAG(exec_params.flags, EXEC_NSS_BYPASS_BUS,
             MANAGER_IS_SYSTEM(UNIT(s)->manager) &&
                 unit_has_name(UNIT(s), SPECIAL_DBUS_SERVICE));
    ...
    r = exec_spawn(UNIT(s), c, &s->exec_context, &exec_params, s->exec_runtime,
                   &s->dynamic_creds, &pid);
    if (r < 0)
        return r;
...
static int control_pid_good(Service* s)
{
    ...
    /* Returns 0 if the control PID is dead, > 0 if it is good. We never
     * actually return < 0 here, but in order to make this function as similar
     * as possible to main_pid_good() and cgroup_good(), we pretend that < 0
     * also means: we can't figure it out. */
...
static int cgroup_good(Service* s)
{
    ...
    /* Returns 0 if the cgroup is empty or doesn't exist, > 0 if it is exists
     * and is populated, < 0 if we can't figure it out */
...
static bool service_shall_restart(Service* s)
{
    ...
    if (exit_status_set_test(&s->restart_prevent_status,
                             s->main_exec_status.code,
                             s->main_exec_status.status))
        return false;

    /* Restart if the exit code/status are configured as restart triggers */
    if (exit_status_set_test(&s->restart_force_status, s->main_exec_status.code,
                             s->main_exec_status.status))
        return true;
    ...
    switch (s->restart)
    {
        ...
        case SERVICE_RESTART_ON_ABNORMAL:
            return !IN_SET(s->result, SERVICE_SUCCESS,
        ...
        case SERVICE_RESTART_ON_ABORT:
            return IN_SET(s->result, SERVICE_FAILURE_SIGNAL,
...
static void service_enter_dead(Service* s, ServiceResult f, bool allow_restart)
{
    ...
    /* If there's a stop job queued before we enter the DEAD state, we shouldn't
     * act on Restart=, in order to not undo what has already been enqueued. */
    ...
    unit_log_result(UNIT(s), s->result == SERVICE_SUCCESS,
                    service_result_to_string(s->result));

    if (allow_restart && service_shall_restart(s))
        s->will_auto_restart = true;

    /* Make sure service_release_resources() doesn't destroy our FD store, while
     * we are changing through
    ...
    if (s->will_auto_restart)
    {
        ...
        r = service_arm_timer(s,
                              usec_add(now(CLOCK_MONOTONIC), s->restart_usec));
        if (r < 0)
        {
            s->n_keep_fd_store--;
            goto fail;
        ...
        /* If we shan't restart, then flush out the restart counter. But don't
         * do that immediately, so that the user can still introspect the
         * counter. Do so on the next start. */
        s->flush_n_restarts = true;

    /* The new state is in effect, let's decrease the fd store ref counter
     * again. Let's also readd us to the GC queue, so that the fd store is
     * possibly gc'ed again */
    s->n_keep_fd_store--;
    unit_add_to_gc_queue(UNIT(s));

    /* The next restart might not be a manual stop, hence reset the flag
     * indicating manual stops */
    ...
    if (s->exec_context.runtime_directory_preserve_mode == EXEC_PRESERVE_NO ||
        (s->exec_context.runtime_directory_preserve_mode ==
             EXEC_PRESERVE_RESTART &&
         !service_will_restart(UNIT(s))))
        /* Also, remove the runtime directory */
        exec_context_destroy_runtime_directory(
            &s->exec_context, UNIT(s)->manager->prefix[EXEC_DIRECTORY_RUNTIME]);
...
static void service_enter_stop_post(Service* s, ServiceResult f)
{
    ...
    if (s->control_command)
    {
        ...
        r = service_spawn(s, s->control_command, s->timeout_stop_usec,
                          EXEC_APPLY_SANDBOXING | EXEC_APPLY_CHROOT |
                              EXEC_APPLY_TTY_STDIN | EXEC_IS_CONTROL |
                              EXEC_SETENV_RESULT | EXEC_CONTROL_CGROUP,
                          &s->control_pid);
        if (r < 0)
            goto fail;
                                 ...
                                 ServiceResult f)
{
    ...
    /* Also, enqueue a job that we recheck all our PIDs a bit later, given that
     * it's likely some processes have died now */
    (void)unit_enqueue_rewatch_pids(UNIT(s));

    r = unit_kill_context(UNIT(s), &s->kill_context,
                          state_to_kill_operation(state), s->main_pid,
                          s->control_pid, s->main_pid_alien);
    if (r < 0)
        goto fail;
    ...
    else if (IN_SET(state, SERVICE_STOP_WATCHDOG, SERVICE_STOP_SIGTERM) &&
...
fail:
    ...
    if (IN_SET(state, SERVICE_STOP_WATCHDOG, SERVICE_STOP_SIGTERM,
               SERVICE_STOP_SIGKILL))
...
static void service_enter_stop(Service* s, ServiceResult f)
{
    ...
    if (s->control_command)
    {
        ...
        r = service_spawn(s, s->control_command, s->timeout_stop_usec,
                          EXEC_APPLY_SANDBOXING | EXEC_APPLY_CHROOT |
                              EXEC_IS_CONTROL | EXEC_SETENV_RESULT |
                              EXEC_CONTROL_CGROUP,
                          &s->control_pid);
        if (r < 0)
            goto fail;
...
static void service_enter_running(Service* s, ServiceResult f)
{
    ...
    else if (service_good(s))
    {
        ...
        /* If there are any queued up sd_notify() notifications, process them
         * now */
        ...
        else
        {
            ...
            service_arm_timer(
                s, usec_add(UNIT(s)->active_enter_timestamp.monotonic,
                            s->runtime_max_usec));
        }
    }
...
static int service_adverse_to_leftover_processes(Service* s)
{
    ...
    /* KillMode=mixed and control group are used to indicate that all process
     * should be killed off. SendSIGKILL is used for services that require a
     * clean shutdown. These are typically database service where a SigKilled
     * process would result in a lengthy recovery and who's shutdown or startup
     * time is quite variable (so Timeout settings aren't of use).
     *
     * Here we take these two factors and refuse to start a service if there are
     * existing processes within a control group. Databases, while generally
     * having some protection against multiple instances running, lets not
     * stress the rigor of these. Also ExecStartPre parts of the service
    ...
    {
        return log_unit_error_errno(
            UNIT(s), SYNTHETIC_ERRNO(EBUSY),
            "Will not start SendSIGKILL=no service of type "
            "KillMode=control-group or mixed while processes exist");
    }
    return 0;
}
...
static void service_enter_start(Service* s)
{
    ...
    if (!c)
    {
        if (s->type != SERVICE_ONESHOT)
        {
            /* There's no command line configured for the main command? Hmm,
             * that is strange. This can only happen if the configuration
             * changes at runtime. In this case, let's enter a failure state. */
            log_unit_error(UNIT(s),
        ...
        /* We force a fake state transition here. Otherwise, the unit would go
         * directly from SERVICE_DEAD to SERVICE_DEAD without SERVICE_ACTIVATING
         * or SERVICE_ACTIVE inbetween. This way we can later trigger actions
         * that depend on the state transition, including SuccessAction=. */
    ...
    if (IN_SET(s->type, SERVICE_SIMPLE, SERVICE_IDLE))
        /* For simple + idle this is the main process. We don't apply any
         * timeout here, but service_enter_running() will later apply the
         * .runtime_max_usec timeout. */
        timeout = USEC_INFINITY;
    else
        timeout = s->timeout_start_usec;

    r = service_spawn(s, c, timeout,
                      EXEC_PASS_FDS | EXEC_APPLY_SANDBOXING |
                          EXEC_APPLY_CHROOT | EXEC_APPLY_TTY_STDIN |
                          EXEC_SET_WATCHDOG,
                      &pid);
    if (r < 0)
        goto fail;
                    ...
                    SERVICE_EXEC))
    {
        ...
        /* For oneshot services we wait until the start process exited, too, but
         * it is our main process. */

        /* For D-Bus services we know the main pid right away, but wait for the
         * bus name to appear on the bus. 'notify' and 'exec' services are
         * similar. */
...
static void service_enter_restart(Service* s)
{
     ...
     * as well. */
    r = manager_add_job(UNIT(s)->manager, JOB_RESTART, UNIT(s), JOB_REPLACE,
                        NULL, &error, NULL);
    if (r < 0)
        goto fail;

    /* Count the jobs we enqueue for restarting. This counter is maintained as
     * long as the unit isn't fully stopped, i.e. as long as it remains up or
     * remains in auto-start states. The use can reset the counter explicitly
     * however via the usual "systemctl reset-failure" logic. */
    s->n_restarts++;
    s->flush_n_restarts = false;

    log_struct(LOG_INFO, "MESSAGE_ID=" SD_MESSAGE_UNIT_RESTART_SCHEDULED_STR,
               LOG_UNIT_ID(UNIT(s)), LOG_UNIT_INVOCATION_ID(UNIT(s)),
               LOG_UNIT_MESSAGE(
                   UNIT(s), "Scheduled restart job, restart counter is at %u.",
                   s->n_restarts),
...
fail:
    log_unit_warning(UNIT(s), "Failed to schedule restart job: %s",
...
static void service_enter_reload_by_notify(Service* s)
{
    ...
    /* service_enter_reload_by_notify is never called during a reload, thus no
     * loops are possible. */
    r = manager_propagate_reload(UNIT(s)->manager, UNIT(s), JOB_FAIL, &error);
    if (r < 0)
        log_unit_warning(UNIT(s),
                         "Failed to schedule propagation of reload: %s",
                         bus_error_message(&error, -r));
}
...
static void service_run_next_control(Service* s)
{
    ...
    r = service_spawn(s, s->control_command, timeout,
                      EXEC_APPLY_SANDBOXING | EXEC_APPLY_CHROOT |
                          EXEC_IS_CONTROL |
                          (IN_SET(s->control_command_id, SERVICE_EXEC_START_PRE,
                                  SERVICE_EXEC_STOP_POST)
                               ? EXEC_APPLY_TTY_STDIN
                               : 0) |
                          (IN_SET(s->control_command_id, SERVICE_EXEC_STOP,
                                  SERVICE_EXEC_STOP_POST)
                               ? EXEC_SETENV_RESULT
                               : 0) |
                          (IN_SET(s->control_command_id,
                                  SERVICE_EXEC_START_POST, SERVICE_EXEC_RELOAD,
                                  SERVICE_EXEC_STOP, SERVICE_EXEC_STOP_POST)
                               ? EXEC_CONTROL_CGROUP
                               : 0),
                      &s->control_pid);
    if (r < 0)
...
static void service_run_next_main(Service* s)
{
    ...
    r = service_spawn(s, s->main_command, s->timeout_start_usec,
                      EXEC_PASS_FDS | EXEC_APPLY_SANDBOXING |
                          EXEC_APPLY_CHROOT | EXEC_APPLY_TTY_STDIN |
                          EXEC_SET_WATCHDOG,
                      &pid);
...
static int service_start(Unit* u)
{
     ...
     * please! */
    if (IN_SET(s->state, SERVICE_STOP, SERVICE_STOP_WATCHDOG,
               SERVICE_STOP_SIGTERM, SERVICE_STOP_SIGKILL, SERVICE_STOP_POST,
               SERVICE_FINAL_SIGTERM, SERVICE_FINAL_SIGKILL))
        return -EAGAIN;
    ...
    exec_command_reset_status_list_array(s->exec_command,
...
static unsigned service_exec_command_index(Unit* u, ServiceExecCommand id,
                                           ExecCommand* current)
{
    Service* s = SERVICE(u);
    unsigned idx = 0;
...
static int service_serialize_exec_command(Unit* u, FILE* f,
                                          ExecCommand* command)
{
    ...
    return serialize_item_format(f, key, "%s %u %s %s",
                                 service_exec_command_to_string(id), idx, p,
                                 args);
}
...
static int service_serialize(Unit* u, FILE* f, FDSet* fds)
{
    ...
    (void)serialize_item(f, "reload-result",
    ...
    {
        r = serialize_fd(f, fds, "exec-fd",
                         sd_event_source_get_io_fd(s->exec_fd_event_source));
        if (r < 0)
            return r;
    ...
    if (UNIT_ISSET(s->accept_socket))
    {
        r = serialize_item(f, "accept-socket",
                           UNIT_DEREF(s->accept_socket)->id);
        if (r < 0)
            return r;
    }
    ...
    LIST_FOREACH(fd_store, fs, s->fd_store)
    {
        ...
        if (copy < 0)
            return log_error_errno(
                copy, "Failed to copy file descriptor for serialization: %m");
    ...
    if (s->main_exec_status.pid > 0)
    {
        (void)serialize_item_format(f, "main-exec-status-pid", PID_FMT,
                                    s->main_exec_status.pid);
        (void)serialize_dual_timestamp(f, "main-exec-status-start",
                                       &s->main_exec_status.start_timestamp);
        (void)serialize_dual_timestamp(f, "main-exec-status-exit",
                                       &s->main_exec_status.exit_timestamp);

        if (dual_timestamp_is_set(&s->main_exec_status.exit_timestamp))
        {
            (void)serialize_item_format(f, "main-exec-status-code", "%i",
                                        s->main_exec_status.code);
            (void)serialize_item_format(f, "main-exec-status-status", "%i",
                                        s->main_exec_status.status);
        }
    }

    (void)serialize_dual_timestamp(f, "watchdog-timestamp",
                                   &s->watchdog_timestamp);
    (void)serialize_bool(f, "forbid-restart", s->forbid_restart);

    if (s->watchdog_override_enable)
        (void)serialize_item_format(f, "watchdog-override-usec", USEC_FMT,
                                    s->watchdog_override_usec);

    if (s->watchdog_original_usec != USEC_INFINITY)
        (void)serialize_item_format(f, "watchdog-original-usec", USEC_FMT,
                                    s->watchdog_original_usec);

    return 0;
}

static int service_deserialize_exec_command(Unit* u, const char* key,
                                            const char* value)
{
    Service* s = SERVICE(u);
    int r;
    ...
    /* Let's check whether exec command on given offset matches data that we
     * just deserialized */
    ...
    if (!found)
    {
        /* Command at the index we serialized is different, let's look for
         * command that exactly matches but is on different index. If there is
         * no such command we will not resume execution. */
    ...
    if (command && control)
        ...
        log_unit_warning(u, "Current command vanished from the unit file, "
                                    ...
                                    FDSet* fds)
{
    ...
    else if (streq(key, "main-pid-known"))
    {
        ...
        if (b < 0)
            log_unit_debug(u, "Failed to parse main-pid-known value: %s",
                           value);
        else
            s->main_pid_known = b;
    }
    ...
    else if (streq(key, "status-text"))
    {
        ...
        if (r < 0)
            log_unit_debug_errno(
                u, r, "Failed to unescape status text '%s': %m", value);
        else
    ...
    else if (streq(key, "accept-socket"))
    {
        ...
        if (r < 0)
            log_unit_debug_errno(
                u, r, "Failed to load accept-socket unit '%s': %m", value);
        else
        {
    ...
    else if (streq(key, "main-exec-status-pid"))
    {
        ...
        if (parse_pid(value, &pid) < 0)
            log_unit_debug(u, "Failed to parse main-exec-status-pid value: %s",
                           value);
        else
    ...
    else if (streq(key, "main-exec-status-code"))
    {
        ...
        if (safe_atoi(value, &i) < 0)
            log_unit_debug(u, "Failed to parse main-exec-status-code value: %s",
                           value);
        else
    ...
    else if (streq(key, "main-exec-status-status"))
    {
        ...
        if (safe_atoi(value, &i) < 0)
            log_unit_debug(
                u, "Failed to parse main-exec-status-status value: %s", value);
        else
    ...
    else if (streq(key, "forbid-restart"))
    {
        ...
        b = parse_boolean(value);
        if (b < 0)
            log_unit_debug(u, "Failed to parse forbid-restart value: %s",
                           value);
        else
            s->forbid_restart = b;
    }
    ...
    else if (streq(key, "exec-fd"))
    {
        ...
        else
        {
            ...
            if (service_allocate_exec_fd_event_source(
                    s, fd, &s->exec_fd_event_source) < 0)
                safe_close(fd);
        }
    }
    else if (streq(key, "watchdog-override-usec"))
    {
        if (deserialize_usec(value, &s->watchdog_override_usec) < 0)
            log_unit_debug(
                u, "Failed to parse watchdog_override_usec value: %s", value);
        else
            s->watchdog_override_enable = true;
    }
    else if (streq(key, "watchdog-original-usec"))
    {
        if (deserialize_usec(value, &s->watchdog_original_usec) < 0)
            log_unit_debug(
                u, "Failed to parse watchdog_original_usec value: %s", value);
    }
    else if (STR_IN_SET(key, "main-command", "control-command"))
    {
        ...
        if (r < 0)
            log_unit_debug_errno(
                u, r, "Failed to parse serialized command \"%s\": %m", value);
    }
    else if (streq(key, "n-restarts"))
    {
        ...
        if (r < 0)
            log_unit_debug_errno(
                u, r, "Failed to parse serialized restart counter '%s': %m",
                value);
    }
    else if (streq(key, "flush-n-restarts"))
    {
        ...
        if (r < 0)
            log_unit_debug_errno(u, r,
                                 "Failed to parse serialized flush restart "
                                 "counter setting '%s': %m",
                                 value);
        else
...
_pure_ static UnitActiveState service_active_state(Unit* u)
{
    ...
    table = SERVICE(u)->type == SERVICE_IDLE ? state_translation_table_idle
                                             : state_translation_table;
...
static bool service_may_gc(Unit* u)
{
    ...
    /* Never clean up services that still have a process around, even if the
     * service is formally dead. Note that unit_may_gc() already checked our
     * cgroup for us, we just check our two additional PIDs, too, in case they
...
static int service_watch_pid_file(Service* s)
{
    ...
    log_unit_debug(UNIT(s), "Setting watch for PID file %s",
                   s->pid_file_pathspec->path);
    ...
    log_unit_debug(UNIT(s), "Trying to read PID file %s in case it changed",
                   s->pid_file_pathspec->path);
...
fail:
    log_unit_error_errno(UNIT(s), r,
                         "Failed to set a watch for PID file %s: %m",
                         s->pid_file_pathspec->path);
    service_unwatch_pid_file(s);
    return r;
}
                                    ...
                                    uint32_t events, void* userdata)
{
    ...
    /* If Type=exec is set, we'll consider a service started successfully the
     * instant we invoked execve() successfully for it. We implement this
     * through a pipe() towards the child, which the kernel automatically closes
     * for us due to O_CLOEXEC on execve() in the child, which then triggers EOF
     * on the pipe in the parent. We need to be careful however, as there are
     * other reasons that we might cause the child's side of the pipe to be
     * closed (for example, a simple exit()). To deal with that we'll ignore
     * EOFs on the pipe unless the child signalled us first that it is about to
     * call the execve(). It does so by sending us a simple non-zero byte via
     * the pipe. We also provide the child with a way to inform us in case
     * execve() failed: if it sends a zero byte we'll ignore POLLHUP on the fd
     * again. */
    ...
    for (;;)
    {
        ...
        if (n < 0)
        {
            if (errno == EAGAIN) /* O_NONBLOCK in effect → everything queued has
                                    now been processed. */
                return 0;
        ...
        if (n == 0)
        { /* EOF → the event we are waiting for */
            ...
            { /* Did the child tell us to expect EOF now? */
                ...
                /* Nice! This is what we have been waiting for. Transition to
                 * next state. */
...
static void service_notify_cgroup_empty_event(Unit* u)
{
    ...
    switch (s->state)
    {
        ...
        case SERVICE_START:
            if (s->type == SERVICE_NOTIFY && main_pid_good(s) == 0 &&
            ...
            _fallthrough_;
        case SERVICE_START_POST:
            if (s->pid_file_pathspec && main_pid_good(s) == 0 &&
                control_pid_good(s) == 0)
            {
...
static void service_sigchld_event(Unit* u, pid_t pid, int code, int status)
{
    ...
    if (s->type == SERVICE_ONESHOT ||
        (s->control_pid == pid && s->control_command_id != SERVICE_EXEC_START))
    ...
    if (s->main_pid == pid)
    {
        ...
        s->main_pid = 0;
        exec_status_exit(&s->main_exec_status, &s->exec_context, pid, code,
                         status);
        ...
        else if (s->exec_command[SERVICE_EXEC_START])
        {
            ...
            if (s->exec_command[SERVICE_EXEC_START]->flags &
                EXEC_COMMAND_IGNORE_FAILURE)
                f = SERVICE_SUCCESS;
        }

        /* When this is a successful exit, let's log about the exit code on
         * DEBUG level. If this is a failure and the process exited on its own
         * via exit(), then let's make this a NOTICE, under the assumption that
         * the service already logged the reason at a higher log level on its
         * own. (Internally,
         * unit_log_process_exit() will possibly bump this to WARNING if the
         * service died due to a signal.) */
        unit_log_process_exit(
            u, f == SERVICE_SUCCESS ? LOG_DEBUG : LOG_NOTICE, "Main process",
            service_exec_command_to_string(SERVICE_EXEC_START), code, status);
            ...
            s->type == SERVICE_ONESHOT && f == SERVICE_SUCCESS)
        {
            ...
            log_unit_debug(u, "Running next main command for state %s.",
        ...
        else
        {
            ...
            switch (s->state)
            {
                ...
                case SERVICE_START:
                    ...
                    {
                        /* Only enter running through a notification, so that
                         * the SERVICE_START state signifies that no ready
                         * notification has been received */
                        if (f != SERVICE_SUCCESS)
                            service_enter_signal(s, SERVICE_STOP_SIGTERM, f);
                        else if (!s->remain_after_exit ||
                                 s->notify_access == NOTIFY_MAIN)
                            /* The service has never been and will never be
                             * active */
                            service_enter_signal(s, SERVICE_STOP_SIGTERM,
    ...
    else if (s->control_pid == pid)
    {
        ...
        if (s->control_command)
        {
            exec_status_exit(&s->control_command->exec_status, &s->exec_context,
                             pid, code, status);
        ...
        unit_log_process_exit(
            u, f == SERVICE_SUCCESS ? LOG_DEBUG : LOG_NOTICE, "Control process",
            service_exec_command_to_string(s->control_command_id), code,
            status);
            ...
            f == SERVICE_SUCCESS)
        {
            ...
            log_unit_debug(u, "Running next control command for state %s.",
                           service_state_to_string(s->state));
        ...
        else
        {
            ...
            log_unit_debug(u, "Got final SIGCHLD for state %s.",
                           service_state_to_string(s->state));
            ...
            switch (s->state)
            {
                ...
                case SERVICE_START:
                    if (s->type != SERVICE_FORKING)
                        /* Maybe spurious event due to a reload that changed the
                         * type? */
                        break;
                    ...
                    if (s->pid_file)
                    {
                         ...
                         * The PID file might actually be created by a
                         * START_POST script. In that case don't worry if the
                         * loading fails. */
    ...
    /* We watch the main/control process otherwise we can't retrieve the unit
     * they belong to with cgroupv1. But if they are not our direct child, we
     * won't get a SIGCHLD for them. Therefore we need to look for others to
     * watch so we can detect when the cgroup becomes empty. Note that the
     * control process is always
                                  ...
                                  void* userdata)
{
    ...
    switch (s->state)
    {
        ...
        case SERVICE_START_PRE:
        case SERVICE_START:
            log_unit_warning(UNIT(s), "%s operation timed out. Terminating.",
        ...
        case SERVICE_RELOAD:
            log_unit_warning(
                UNIT(s), "Reload operation timed out. Killing reload process.");
        ...
        case SERVICE_FINAL_SIGTERM:
            ...
            {
                log_unit_warning(
                    UNIT(s), "State 'stop-final-sigterm' timed out. Killing.");
            ...
            {
                log_unit_warning(UNIT(s),
                                 "State 'stop-final-sigterm' timed out. "
        ...
        case SERVICE_FINAL_SIGKILL:
            log_unit_warning(UNIT(s), "Processes still around after final "
        ...
        case SERVICE_AUTO_RESTART:
            if (s->restart_usec > 0)
            {
                ...
                log_unit_info(
                    ...
                    format_timespan(buf_restart, sizeof buf_restart,
                                    s->restart_usec, USEC_PER_SEC));
            }
            else
                log_unit_info(UNIT(s), "Service has no hold-off time "
                                     ...
                                     void* userdata)
{
    ...
    if (UNIT(s)->manager->service_watchdogs)
    {
        ...
        service_enter_signal(s, SERVICE_STOP_WATCHDOG,
                             SERVICE_FAILURE_WATCHDOG);
    }
    else
        log_unit_warning(
            UNIT(s), "Watchdog disabled! Ignoring watchdog timeout (limit %s)!",
            format_timespan(t, sizeof(t), watchdog_usec, 1));
                                              ...
                                              char** tags, FDSet* fds)
{
    ...
    if (s->notify_access == NOTIFY_MAIN && pid != s->main_pid)
    {
        if (s->main_pid != 0)
            ...
            log_unit_warning(UNIT(s),
                             "Got notification message from PID " PID_FMT
                             ", but reception only permitted for main PID "
    ...
    if (s->notify_access == NOTIFY_EXEC && pid != s->main_pid &&
        pid != s->control_pid)
    {
        if (s->main_pid != 0 && s->control_pid != 0)
            ...
            log_unit_warning(UNIT(s),
                             "Got notification message from PID " PID_FMT
                             ", but reception only permitted for main PID and "
                             "control PID which are currently not known",
                             pid);
...
static void service_notify_message(Unit* u, const struct ucred* ucred,
                                   char** tags, FDSet* fds)
{
    ...
    if (DEBUG_LOGGING)
    {
        ...
        cc = strv_join(tags, ", ");
        log_unit_debug(u, "Got notification message from PID " PID_FMT " (%s)",
                       ucred->pid, isempty(cc) ? "n/a" : cc);
    }

    /* Interpret MAINPID= */
    e = strv_find_startswith(tags, "MAINPID=");
    if (e && IN_SET(s->state, SERVICE_START, SERVICE_START_POST,
                    SERVICE_RUNNING, SERVICE_RELOAD))
    {
        pid_t new_main_pid;
        ...
        if (parse_pid(e, &new_main_pid) < 0)
            log_unit_warning(u,
                             "Failed to parse MAINPID= field in notification "
                             "message, ignoring: %s",
                             e);
        else if (!s->main_pid_known || new_main_pid != s->main_pid)
        {
            ...
            if (r == 0)
            {
                /* The new main PID is a bit suspicous, which is OK if the
                 * sender is privileged. */

                if (ucred->uid == 0)
                {
                    log_unit_debug(u,
                                   "New main PID " PID_FMT
                                   " does not belong to service, but we'll "
                                   "accept it as the request to change it came "
    ...
    if (e)
    {
        ...
        if (!isempty(e))
        {
            /* Note that this size limit check is mostly paranoia: since the
             * datagram size we are willing to process is already limited to
             * NOTIFY_BUFFER_MAX, this limit here should never be hit. */
            if (strlen(e) > STATUS_TEXT_MAX)
                log_unit_warning(
                    u, "Status message overly long (%zu > %u), ignoring.",
                    strlen(e), STATUS_TEXT_MAX);
            else if (!utf8_is_valid(e))
                log_unit_warning(u, "Status message in notification message is "
                                    "not UTF-8 clean, ignoring.");
            else
            {
                t = strdup(e);
                if (!t)
    ...
    if (e)
    {
        ...
        if (status_errno < 0)
            log_unit_warning_errno(u, status_errno,
                                   "Failed to parse ERRNO= field value '%s' in "
    ...
    /* Process FD store messages. Either FDSTOREREMOVE=1 for removal, or
     * FDSTORE=1 for addition. In both cases, process FDNAME= for picking the
     * file descriptor name to use. Note that FDNAME= is required when removing
    ...
    if (strv_find(tags, "FDSTOREREMOVE=1"))
    {
        ...
        if (!name || !fdname_is_valid(name))
            log_unit_warning(u, "FDSTOREREMOVE=1 requested, but no valid file "
                                "descriptor name passed, ignoring.");
        else
...
static void service_bus_name_owner_change(Unit* u, const char* name,
                                          ...
                                          const char* new_owner)
{
    ...
    if (old_owner && new_owner)
        log_unit_debug(u, "D-Bus name %s changed owner from %s to %s", name,
                       old_owner, new_owner);
    else if (old_owner)
        log_unit_debug(u, "D-Bus name %s no longer registered by %s", name,
                       old_owner);
    else
        log_unit_debug(u, "D-Bus name %s now registered by %s", name,
                       new_owner);

    s->bus_name_good = !!new_owner;

    /* Track the current owner, so we can reconstruct changes after a daemon
     * reload */
    r = free_and_strdup(&s->bus_name_owner, new_owner);
    if (r < 0)
    {
        log_unit_error_errno(u, r, "Unable to set new bus name owner %s: %m",
                             new_owner);
        return;
    }
                    ...
                    SERVICE_RUNNING, SERVICE_RELOAD))
    {
        ...
        r = sd_bus_get_name_creds(u->manager->api_bus, name, SD_BUS_CREDS_PID,
                                  &creds);
        if (r >= 0)
            r = sd_bus_creds_get_pid(creds, &pid);
        if (r >= 0)
        {
            log_unit_debug(u, "D-Bus name %s is now owned by process " PID_FMT,
                           name, pid);
                          ...
                          bool selinux_context_net)
{
    ...
    /* This is called by the socket code when instantiating a new service for a
     * stream socket and the socket needs
    ...
    r = unit_add_two_dependencies(UNIT(sock), UNIT_BEFORE, UNIT_TRIGGERS,
                                  UNIT(s), false, UNIT_DEPENDENCY_IMPLICIT);
    if (r < 0)
        return r;
...
static bool service_needs_console(Unit* u)
{
    ...
    /* We provide our own implementation of this here, instead of relying of the
     * generic implementation unit_needs_console() provides, since we want to
     * return false if we are in SERVICE_EXITED state. */
    ...
    return IN_SET(s->state, SERVICE_START_PRE, SERVICE_START,
                  ...
                  SERVICE_STOP, SERVICE_STOP_WATCHDOG, SERVICE_STOP_SIGTERM,
...
static const char* const service_type_table[_SERVICE_TYPE_MAX] = {
    [SERVICE_SIMPLE] = "simple",   [SERVICE_FORKING] = "forking",
    [SERVICE_ONESHOT] = "oneshot", [SERVICE_DBUS] = "dbus",
    [SERVICE_NOTIFY] = "notify",   [SERVICE_IDLE] = "idle",
    [SERVICE_EXEC] = "exec",
};
