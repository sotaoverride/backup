...
#define G_KEYFILE_SETTINGS_BACKEND(inst)                                       \
    (G_TYPE_CHECK_INSTANCE_CAST((inst), G_TYPE_KEYFILE_SETTINGS_BACKEND,       \
                                GKeyfileSettingsBackend))
#define G_IS_KEYFILE_SETTINGS_BACKEND(inst)                                    \
    (G_TYPE_CHECK_INSTANCE_TYPE((inst), G_TYPE_KEYFILE_SETTINGS_BACKEND))
...
typedef struct
{
    ...
    GHashTable*
        system_locks; /* Used as a set, owning the strings it contains */
...
G_DEFINE_TYPE_WITH_CODE(
    GKeyfileSettingsBackend, g_keyfile_settings_backend,
    G_TYPE_SETTINGS_BACKEND, _g_io_modules_ensure_extension_points_registered();
    g_io_extension_point_implement(G_SETTINGS_BACKEND_EXTENSION_POINT_NAME,
                                   g_define_type_id, "keyfile",
                                   EXTENSION_PRIORITY))

static void compute_checksum(guint8* digest, gconstpointer contents,
                             gsize length)
{
    GChecksum* checksum;
    gsize len = 32;
...
static gboolean convert_path(GKeyfileSettingsBackend* kfsb, const gchar* key,
                             gchar** group, gchar** basename)
{
...
static gboolean path_is_valid(GKeyfileSettingsBackend* kfsb, const gchar* path)
{
    return convert_path(kfsb, path, NULL, NULL);
}

static GVariant* get_from_keyfile(GKeyfileSettingsBackend* kfsb,
                                  const GVariantType* type, const gchar* key)
{
    ...
    if (convert_path(kfsb, key, &group, &name))
    {
        ...
        if (sysstr &&
            (g_hash_table_contains(kfsb->system_locks, key) || str == NULL))
        {
            g_free(str);
...
static gboolean set_to_keyfile(GKeyfileSettingsBackend* kfsb, const gchar* key,
                               GVariant* value)
{
    gchar *group, *name;
...
static GVariant*
    g_keyfile_settings_backend_read(GSettingsBackend* backend, const gchar* key,
...
static gboolean g_keyfile_settings_backend_write_one(gpointer key,
                                                     gpointer value,
                                                     gpointer user_data)
{
...
static gboolean g_keyfile_settings_backend_check_one(gpointer key,
                                                     gpointer value,
                                                     gpointer user_data)
{
    WriteManyData* data = user_data;
    ...
    return data->failed =
               g_hash_table_contains(data->kfsb->system_locks, key) ||
               !path_is_valid(data->kfsb, key);
}

static gboolean g_keyfile_settings_backend_write_tree(GSettingsBackend* backend,
                                                      GTree* tree,
                                                      gpointer origin_tag)
{
...
static gboolean g_keyfile_settings_backend_write(GSettingsBackend* backend,
                                                 const gchar* key,
                                                 GVariant* value,
                                                 gpointer origin_tag)
{
...
static void g_keyfile_settings_backend_reset(GSettingsBackend* backend,
                                             const gchar* key,
                                             gpointer origin_tag)
{
    GKeyfileSettingsBackend* kfsb = G_KEYFILE_SETTINGS_BACKEND(backend);
                                            ...
                                            const gchar* name)
{
    ...
    return kfsb->writable && !g_hash_table_contains(kfsb->system_locks, name) &&
...
static void keyfile_to_tree(GKeyfileSettingsBackend* kfsb, GTree* tree,
                            GKeyFile* keyfile, gboolean dup_check)
{
    gchar** groups;
    gint i;
    ...
    for (i = 0; groups[i]; i++)
    {
        ...
        for (j = 0; keys[j]; j++)
        {
            ...
            if (is_root_group)
                ...
                path = g_strdup_printf("%s%s/%s", kfsb->prefix, groups[i],
                                       keys[j]);
    ...
    g_keyfile_settings_backend_keyfile_writable(GKeyfileSettingsBackend* kfsb)
{
    ...
    if (fileinfo)
    {
        writable = g_file_info_get_attribute_boolean(
                       fileinfo, G_FILE_ATTRIBUTE_ACCESS_CAN_WRITE) &&
                   g_file_info_get_attribute_boolean(
                       fileinfo, G_FILE_ATTRIBUTE_ACCESS_CAN_EXECUTE);
...
static void dir_changed(GFileMonitor* monitor, GFile* file, GFile* other_file,
                        GFileMonitorEvent event_type, gpointer user_data)
{
    GKeyfileSettingsBackend* kfsb = user_data;
...
static void load_system_settings(GKeyfileSettingsBackend* kfsb)
{
    GError* error = NULL;
     ...
     */
    if (!g_key_file_load_from_file(kfsb->system_keyfile, path, G_KEY_FILE_NONE,
                                   &error))
    {
    ...
    /* The locks file is a text file containing a list paths to lock, one per
     * line. It can be produced from a dconf database using: dconf list-locks
     */
...
static void g_keyfile_settings_backend_constructed(GObject* object)
{
    GKeyfileSettingsBackend* kfsb = G_KEYFILE_SETTINGS_BACKEND(object);

    if (kfsb->file == NULL)
    {
        char* filename = g_build_filename(g_get_user_config_dir(), "glib-2.0",
    ...
    kfsb->file_monitor =
        g_file_monitor(kfsb->file, G_FILE_MONITOR_NONE, NULL, NULL);
    kfsb->dir_monitor =
        g_file_monitor(kfsb->dir, G_FILE_MONITOR_NONE, NULL, NULL);

    compute_checksum(kfsb->digest, NULL, 0);

    g_signal_connect(kfsb->file_monitor, "changed", G_CALLBACK(file_changed),
                     kfsb);
    g_signal_connect(kfsb->dir_monitor, "changed", G_CALLBACK(dir_changed),
                     kfsb);
...
static void g_keyfile_settings_backend_set_property(GObject* object,
                                                    guint prop_id,
                                                    const GValue* value,
                                                    GParamSpec* pspec)
{
...
static void g_keyfile_settings_backend_get_property(GObject* object,
                                                    guint prop_id,
                                                    GValue* value,
                                                    GParamSpec* pspec)
{
    ...
    g_keyfile_settings_backend_class_init(GKeyfileSettingsBackendClass* class)
{
    ...
    g_object_class_install_property(
        ...
        g_param_spec_string("filename", P_("Filename"), P_("The filename"),
                            NULL,
     ...
     */
    g_object_class_install_property(
        object_class, PROP_ROOT_PATH,
        g_param_spec_string("root-path", P_("Root path"), P_("The root path"),
                            NULL,
                            G_PARAM_READWRITE | G_PARAM_CONSTRUCT_ONLY |
                                G_PARAM_STATIC_STRINGS));
     ...
     */
    g_object_class_install_property(
        object_class, PROP_ROOT_GROUP,
        g_param_spec_string("root-group", P_("Root group"),
                            P_("The root group"), NULL,
                            G_PARAM_READWRITE | G_PARAM_CONSTRUCT_ONLY |
                                G_PARAM_STATIC_STRINGS));
     ...
     */
    g_object_class_install_property(
        object_class, PROP_DEFAULTS_DIR,
        g_param_spec_string("defaults-dir", P_("Default dir"),
                            P_("Defaults dir"), NULL,
                            G_PARAM_READWRITE | G_PARAM_CONSTRUCT_ONLY |
                                G_PARAM_STATIC_STRINGS));
}
                                                 ...
                                                 const gchar* root_group)
{
    ...
    return G_SETTINGS_BACKEND(
        g_object_new(G_TYPE_KEYFILE_SETTINGS_BACKEND, "filename", filename,
                     "root-path", root_path, "root-group", root_group, NULL));
}
