...
class partial_date : public year_based_generator<date_type>
{
  public:
    ...
    partial_date(day_type d, month_type m) : day_(d), month_(m)
    {
    }
...
class nth_kday_of_month : public year_based_generator<date_type>
{
  public:
    ...
    {
        std::ostringstream ss;
        ss << 'M' << static_cast<int>(month_) << '.' << static_cast<int>(wn_)
...
class last_kday_of_month : public year_based_generator<date_type>
{
  public:
    ...
    date_type get_date(year_type year) const
    {
        date_type d(year, month_,
                    calendar_type::end_of_month_day(year, month_));
        duration_type one_day(1);
        while (dow_ != d.day_of_week())
        {
            d = d - one_day;
...
inline typename date_type::duration_type
...
//! Generates a date object representing the date of the following weekday from
//! the given date
...
//! Generates a date object representing the date of the previous weekday from
//! the given date
