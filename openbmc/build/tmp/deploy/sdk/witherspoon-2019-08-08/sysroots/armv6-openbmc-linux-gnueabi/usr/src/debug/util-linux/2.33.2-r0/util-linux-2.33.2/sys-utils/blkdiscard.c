...
static void __attribute__((__noreturn__)) usage(void)
{
    ...
    fputs(
        _(" -l, --length <num>  length of bytes to discard from the offset\n"),
        out);
    fputs(_(" -p, --step <num>    size of the discard iterations within the "
            "offset\n"),
          out);
...
int main(int argc, char** argv)
{
    ...
    while ((c = getopt_long(argc, argv, "hVsvo:l:p:z", longopts, NULL)) != -1)
    {
        switch (c)
        {
            ...
            case 'o':
                ...
                break;
            case 'p':
                step = strtosize_or_err(optarg, _("failed to parse step"));
                break;
            case 's':
                act = ACT_SECURE;
                break;
    ...
    if (range[1] % secsize)
        errx(EXIT_FAILURE,
             _("%s: length %" PRIu64 " is not aligned "
               "to sector size %i"),
             path, range[1], secsize);
        ...
        switch (act)
        {
            ...
            case ACT_SECURE:
                if (ioctl(fd, BLKSECDISCARD, &range))
                    err(EXIT_FAILURE, _("%s: BLKSECDISCARD ioctl failed"),
                        path);
                break;
            case ACT_DISCARD:
