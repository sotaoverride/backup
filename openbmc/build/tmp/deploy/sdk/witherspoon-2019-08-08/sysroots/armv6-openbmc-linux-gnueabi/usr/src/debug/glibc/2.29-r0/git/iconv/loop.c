...
#define get16(addr)                                                            \
    (((const unsigned char*)(addr))[1] << 8 | ((const unsigned char*)(addr))[0])
#define get32(addr)                                                            \
...
#else
#define get16(addr)                                                            \
    (((const unsigned char*)(addr))[0] << 8 | ((const unsigned char*)(addr))[1])
#define get32(addr)                                                            \
...
#define STANDARD_FROM_LOOP_ERR_HANDLER(Incr)                                   \
    {                                                                          \
        ...
        inptr += (Incr);                                                       \
        ++*irreversible;                                                       \
        /* But we keep result == __GCONV_ILLEGAL_INPUT, because of the         \
           constraint                                                          \
    ...
    {                                                                          \
        ...
        if (irreversible == NULL)                                              \
           ...
           case we are not doing any error recovery outself.  */               \
            break;                                                             \
                                                                               \
        /* If needed, flush any conversion state, so that                      \
           __gconv_transliterate starts with current shift state.  */                                          \
        UPDATE_PARAMS;                                                         \
                                                                               \
        /* First try the transliteration methods.  */                          \
        if ((step_data->__flags & __GCONV_TRANSLIT) != 0)                      \
            result = __gconv_transliterate(step, step_data, *inptrp, &inptr,   \
...
FCTNAME(LOOPFCT)(struct __gconv_step* step, struct __gconv_step_data* step_data,
    ...
    while (inptr != inend)
    {
       ...
       since MIN_NEEDED_OUTPUT is always a constant.  */
        ...
        if ((MIN_NEEDED_OUTPUT != 1 &&
            ...
            (MIN_NEEDED_OUTPUT == 1 && __builtin_expect(outptr >= outend, 0)))
        {
...
static inline int __attribute((always_inline))
SINGLE(LOOPFCT)(struct __gconv_step* step, struct __gconv_step_data* step_data,
                const unsigned char** inptrp, const unsigned char* inend,
...
static wint_t gconv_btowc(struct __gconv_step* step,
                          unsigned char c) ONEBYTE_BODY
