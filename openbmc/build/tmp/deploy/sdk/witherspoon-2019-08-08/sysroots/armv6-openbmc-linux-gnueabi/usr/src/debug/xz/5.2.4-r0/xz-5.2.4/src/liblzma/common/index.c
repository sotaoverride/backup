...
static void index_tree_node_end(
    index_tree_node* node, const lzma_allocator* allocator,
...
static void index_tree_end(index_tree* tree, const lzma_allocator* allocator,
...
extern LZMA_API(uint64_t) lzma_index_memusage(lzma_vli streams, lzma_vli blocks)
{
    ...
    const size_t stream_base =
        sizeof(index_stream) + sizeof(index_group) + 2 * alloc_overhead;
    ...
    if (streams == 0 || streams > UINT32_MAX || blocks > LZMA_VLI_MAX ||
        streams > limit / stream_base || groups > limit / group_base ||
...
static lzma_vli index_file_size(lzma_vli compressed_base, lzma_vli unpadded_sum,
...
{
    return (LZMA_VLI_C(4) -
            index_size_unpadded(i->record_count, i->index_list_size)) &
           3;
}
                      ...
                      lzma_vli unpadded_size, lzma_vli uncompressed_size)
{
    ...
    if (i == NULL || unpadded_size < UNPADDED_SIZE_MIN ||
        unpadded_size > UNPADDED_SIZE_MAX || uncompressed_size > LZMA_VLI_MAX)
        return LZMA_PROG_ERROR;

    index_stream* s = (index_stream*)(i->streams.rightmost);
    index_group* g = (index_group*)(s->groups.rightmost);

    const lzma_vli compressed_base =
        g == NULL ? 0 : vli_ceil4(g->records[g->last].unpadded_sum);
    const lzma_vli uncompressed_base =
        g == NULL ? 0 : g->records[g->last].uncompressed_sum;
    const uint32_t index_list_size_add =
        lzma_vli_size(unpadded_size) + lzma_vli_size(uncompressed_size);
    ...
    else
    {
        ...
        g = lzma_alloc(sizeof(index_group) + i->prealloc * sizeof(index_record),
                       allocator);
        if (g == NULL)
    ...
    g->records[g->last].unpadded_sum = compressed_base + unpadded_size;
                   ...
                   const lzma_allocator* allocator)
{
    ...
    {
        const lzma_vli dest_size =
            index_size_unpadded(dest->record_count, dest->index_list_size);
        const lzma_vli src_size =
            index_size_unpadded(src->record_count, src->index_list_size);
    ...
    // to be done before modifying dest or src.
    {
        ...
        if (g != NULL && g->last + 1 < g->allocated)
        {
            ...
            index_group* newg = lzma_alloc(
                sizeof(index_group) + (g->last + 1) * sizeof(index_record),
                allocator);
            if (newg == NULL)
                return LZMA_MEM_ERROR;
            ...
            memcpy(newg->records, g->records,
                   newg->allocated * sizeof(index_record));
                                      ...
                                      const lzma_allocator* allocator)
{
    ...
    index_stream* dest = index_stream_init(
        src->node.compressed_base, src->node.uncompressed_base, src->number,
    ...
    index_group* destg = lzma_alloc(
        sizeof(index_group) + src->record_count * sizeof(index_record),
        allocator);
    if (destg == NULL)
    {
...
static void iter_set_info(lzma_index_iter* iter)
{
    ...
    if (stream->groups.rightmost == NULL)
    {
        ...
        iter->stream.compressed_size =
            index_size(0, 0) + 2 * LZMA_STREAM_HEADER_SIZE;
    ...
    if (group != NULL)
    {
        ...
        iter->block.number_in_file =
            iter->block.number_in_stream + stream->block_number_base;
        ...
        iter->block.uncompressed_size =
            ...
            iter->block.uncompressed_stream_offset;
        iter->block.unpadded_size = group->records[record].unpadded_sum -
                                    iter->block.compressed_stream_offset;
