...
int mnt_context_is_loopdev(struct libmnt_context* cxt)
{
    ...
    if (cxt->user_mountflags & (MNT_MS_LOOP | MNT_MS_OFFSET | MNT_MS_SIZELIMIT))
    {
     ...
     *
     * Note that there is no restriction (on kernel side) that would prevent a
     * regular file as a mount(2) source argument. A filesystem that is able to
     * mount regular files could be implemented.
     */
        ...
        (!type || strcmp(type, "auto") == 0 || blkid_known_fstype(type)))
    {
        ...
        if (stat(src, &st) == 0 && S_ISREG(st.st_mode) && st.st_size > 1024)
        {
...
is_mounted_same_loopfile(struct libmnt_context* cxt, const char* target,
                         const char* backing_file, uint64_t offset)
{
    ...
    DBG(LOOP,
        ul_debugobj(cxt, "checking if %s mounted on %s", backing_file, target));
...
int mnt_context_setup_loopdev(struct libmnt_context* cxt)
{
    ...
    if (rc == 0 && is_mounted_same_loopfile(cxt, mnt_context_get_target(cxt),
                                            backing_file, offset))
        rc = -EBUSY;
        ...
        switch (rc)
        {
            ...
            case 2: /* overlap -- full size and offset match (reuse) */
            {
                    ...
                    lc_encrypt_type != LO_CRYPT_NONE)
                {
                    DBG(LOOP,
                        ul_debugobj(
                            cxt, "encryption no longer supported for device %s",
                            loopcxt_get_device(&lc)));
                    rc = -MNT_ERR_LOOPOVERLAP;
                    goto done;
                }
    ...
    do
    {
        ...
        if (!loopdev)
        {
            ...
            if (rc)
                goto done;
            DBG(LOOP,
                ul_debugobj(cxt, "trying to use %s", loopcxt_get_device(&lc)));
        }
...
success:
    ...
    if (!rc)
    {
        ...
        if (reuse ||
            ((cxt->user_mountflags & MNT_MS_LOOP) && loopcxt_is_autoclear(&lc)))
        {
            /*
        ...
        if (!(cxt->mountflags & MS_RDONLY) && loopcxt_is_readonly(&lc))
            /*
