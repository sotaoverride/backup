...
static int log_stream_send(Log* log)
{
    ...
    if (log_warn)
    {
        r = log_loop_send(log->log_fd, LOG_WARNING_DROPPED,
...
static int log_journal_send(Log* log)
{
    ...
    l = send(log->log_fd, log->map, log->offset,
             log->lossy ? MSG_NOSIGNAL | MSG_DONTWAIT : MSG_NOSIGNAL);
...
out_drop:
    if (!log->n_dropped++)
    {
        l = send(log->log_fd, LOG_WARNING_DROPPED, strlen(LOG_WARNING_DROPPED),
                 MSG_NOSIGNAL);
        if (l < 0)
            return error_origin(-errno);
    }
...
void log_vappendf(Log* log, const char* format, va_list args)
{
    ...
    r = vsnprintf(log->map + log->offset, LOG_SIZE_MAX - log->offset, format,
                  args);
 ...
 */
void log_append_common(Log* log, int level, int error, const char* file,
                       int line, const char* func)
{
     ...
     */
    level =
        LOG_MAKEPRI(LOG_FAC(level) ?: LOG_DAEMON, LOG_PRI(level) ?: LOG_INFO);
    ...
    if (log_alloc(log))
        log_appendf(log,
                    ...
                    program_invocation_short_name, error, file, line, func,
                    log->n_dropped);
}
