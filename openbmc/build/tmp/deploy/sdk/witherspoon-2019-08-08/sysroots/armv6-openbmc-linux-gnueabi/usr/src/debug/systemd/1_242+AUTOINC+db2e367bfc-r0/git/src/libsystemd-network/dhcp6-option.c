...
int dhcp6_option_append_fqdn(uint8_t** buf, size_t* buflen, const char* fqdn)
{
    ...
    buffer[0] =
        DHCP6_FQDN_FLAG_S; /* Request server to perform AAAA RR DNS updates */
...
int dhcp6_option_parse_status(DHCP6Option* option, size_t len)
{
    ...
    if (len < sizeof(DHCP6StatusOption) ||
        be16toh(option->len) + offsetof(DHCP6Option, data) <
                                      ...
                                      uint32_t* lifetime_valid)
{
    ...
    if (be16toh(option->len) + offsetof(DHCP6Option, data) <
        sizeof(*addr_option))
        return -ENOBUFS;
    ...
    if (lt_valid == 0 || lt_pref > lt_valid)
    {
        log_dhcp6_client(client,
                         "Valid lifetime of an IA address is zero or preferred "
                         "lifetime %d > valid lifetime %d",
                         lt_pref, lt_valid);
                                       ...
                                       uint32_t* lifetime_valid)
{
    ...
    {
        log_dhcp6_client(client,
                         "Valid lifetieme of a PD prefix is zero or preferred "
                         "lifetime %d > valid lifetime %d",
                         lt_pref, lt_valid);
...
int dhcp6_option_parse_ia(DHCP6Option* iaoption, DHCP6IA* ia)
{
    ...
    switch (iatype)
    {
        case SD_DHCP6_OPTION_IA_NA:
            ...
            {
                log_dhcp6_client(client, "IA NA T1 %ds > T2 %ds", lt_t1, lt_t2);
                return -EINVAL;
            }
        ...
        case SD_DHCP6_OPTION_IA_PD:
            ...
            if (lt_t1 && lt_t2 && lt_t1 > lt_t2)
            {
                log_dhcp6_client(client, "IA PD T1 %ds > T2 %ds", lt_t1, lt_t2);
                return -EINVAL;
            }
    ...
    while (i < len)
    {
        ...
        if (len < i + sizeof(*option) ||
            len < i + sizeof(*option) + be16toh(option->len))
            return -ENOBUFS;
        ...
        switch (opt)
        {
            case SD_DHCP6_OPTION_IAADDR:
                ...
                if (!IN_SET(ia->type, SD_DHCP6_OPTION_IA_NA,
                            SD_DHCP6_OPTION_IA_TA))
                {
                    log_dhcp6_client(
                        client, "IA Address option not in IA NA or TA option");
                    return -EINVAL;
                }
            ...
            case SD_DHCP6_OPTION_STATUS_CODE:
                ...
                status = dhcp6_option_parse_status(
                    option, optlen + offsetof(DHCP6Option, data));
                if (status < 0)
                    return status;
                if (status > 0)
                {
                    log_dhcp6_client(client, "IA status %d", status);
    ...
    switch (iatype)
    {
        case SD_DHCP6_OPTION_IA_NA:
            if (!ia->ia_na.lifetime_t1 && !ia->ia_na.lifetime_t2)
            {
                ...
                log_dhcp6_client(
                    client,
                    "Computed IA NA T1 %ds and T2 %ds as both were zero", lt_t1,
                    lt_t2);
            }
        ...
        case SD_DHCP6_OPTION_IA_PD:
            if (!ia->ia_pd.lifetime_t1 && !ia->ia_pd.lifetime_t2)
            {
                ...
                log_dhcp6_client(
                    client,
                    "Computed IA PD T1 %ds and T2 %ds as both were zero", lt_t1,
                    lt_t2);
            }
