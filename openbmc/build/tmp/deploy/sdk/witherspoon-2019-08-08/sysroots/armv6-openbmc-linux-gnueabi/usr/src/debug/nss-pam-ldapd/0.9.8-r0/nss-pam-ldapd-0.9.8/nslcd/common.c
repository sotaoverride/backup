...
const char* signame(int signum)
{
    switch (signum)
    {
        ...
        case SIGABRT:
            return "SIGABRT"; /* Abort signal from abort(3) */
        case SIGFPE:
        ...
        case SIGALRM:
            return "SIGALRM"; /* Timer signal from alarm(2) */
        case SIGTERM:
        ...
        case SIGCONT:
            return "SIGCONT"; /* Continue if stopped */
        case SIGSTOP:
            return "SIGSTOP"; /* Stop process */
        case SIGTSTP:
            return "SIGTSTP"; /* Stop typed at tty */
        case SIGTTIN:
            return "SIGTTIN"; /* tty input for background process */
        case SIGTTOU:
            return "SIGTTOU"; /* tty output for background process */
#ifdef SIGBUS
        case SIGBUS:
...
#ifdef SIGSYS
        case SIGSYS:
            return "SIGSYS"; /* Bad argument to routine */
#endif
#ifdef SIGTRAP
        case SIGTRAP:
                  ...
                  const char* addr)
{
    ...
    else
    {
        ...
        log_log(LOG_WARNING, "%s: %s: \"%s\" unparsable", myldap_get_dn(entry),
                attr, addr);
...
char* sid2search(const char* sid)
{
    ...
    while (*tmp != '\0')
    {
        ...
        sprintf(res + strlen(res), "\\%02x\\%02x\\%02x\\%02x",
                (unsigned int)(l & 0xff), (unsigned int)((l >> 8) & 0xff),
