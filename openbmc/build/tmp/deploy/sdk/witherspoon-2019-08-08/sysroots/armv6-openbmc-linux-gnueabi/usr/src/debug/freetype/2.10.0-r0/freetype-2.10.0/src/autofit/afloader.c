...
#define af_intToFixed(i) ((FT_Fixed)((FT_UInt32)(i) << 16))
#define af_fixedToInt(x) ((FT_Short)(((FT_UInt32)(x) + 0x8000U) >> 16))
#define af_floatToFixed(f) ((FT_Fixed)((f)*65536.0 + 0.5))

static FT_Error af_loader_embolden_glyph_in_slot(AF_Loader loader, FT_Face face,
                                                 AF_StyleMetrics style_metrics)
{
    ...
    FT_Bool size_changed =
        size_metrics->x_ppem != globals->stem_darkening_for_ppem;
    ...
    if (writing_system_class->style_metrics_getstdw)
        writing_system_class->style_metrics_getstdw(style_metrics, &stdHW,
                                                    &stdVW);
    else
    {
        ...
        (stdVW > 0 && stdVW != globals->standard_vertical_width))
    {
        ...
        darken_by_font_units_x =
            af_intToFixed(af_loader_compute_darkening(loader, face, stdVW));
        darken_x = FT_DivFix(
            FT_MulFix(darken_by_font_units_x, size_metrics->x_scale), em_ratio);
        ...
        (stdHW > 0 && stdHW != globals->standard_horizontal_width))
    {
        ...
        darken_by_font_units_y =
            af_intToFixed(af_loader_compute_darkening(loader, face, stdHW));
        darken_y = FT_DivFix(
            FT_MulFix(darken_by_font_units_y, size_metrics->y_scale), em_ratio);
         ...
         */
        globals->scale_down_factor = FT_DivFix(
            em_size - (darken_by_font_units_y + af_intToFixed(8)), em_size);
    }
...
FT_LOCAL_DEF(FT_Error)
af_loader_load_glyph(AF_Loader loader, AF_Module module, FT_Face face,
                     FT_UInt glyph_index, FT_Int32 load_flags)
{
        ...
        size_internal->autohint_mode != FT_LOAD_TARGET_MODE(load_flags))
    {
...
#ifdef AF_CONFIG_OPTION_TT_SIZE_METRICS
        {
            ...
            size_metrics->ascender =
                FT_PIX_ROUND(FT_MulFix(face->ascender, size_metrics->y_scale));
            size_metrics->descender =
                FT_PIX_ROUND(FT_MulFix(face->descender, size_metrics->y_scale));
            size_metrics->height =
                FT_PIX_ROUND(FT_MulFix(face->height, size_metrics->y_scale));

            size_metrics->x_scale =
                FT_DivFix(size_metrics->x_ppem << 6, face->units_per_EM);
            size_metrics->y_scale =
                FT_DivFix(size_metrics->y_ppem << 6, face->units_per_EM);
            size_metrics->max_advance = FT_PIX_ROUND(
                FT_MulFix(face->max_advance_width, size_metrics->x_scale));
        }
    ...
    {
        error = writing_system_class->style_hints_init(hints, style_metrics);
        if (error)
            goto Exit;
    }
    ...
    switch (slot->format)
    {
        case FT_GLYPH_FORMAT_OUTLINE:
             ...
             */
            if (loader->transformed)
                FT_Outline_Translate(&slot->outline, loader->trans_delta.x,
            ...
            loader->pp2.x =
                FT_MulFix(slot->metrics.horiAdvance, hints->x_scale) +
                hints->x_delta;
            ...
            if (writing_system_class->style_hints_apply)
            {
                error = writing_system_class->style_hints_apply(
                    glyph_index, hints, &gloader->base.outline, style_metrics);
                if (error)
                    goto Exit;
            }
            ...
            if (scaler.render_mode != FT_RENDER_MODE_LIGHT)
            {
                ...
                if (axis->num_edges > 1 && AF_HINTS_DO_ADVANCE(hints))
                {
                    ...
                    AF_Edge edge2 =
                        edge1 + axis->num_edges - 1; /* rightmost edge */
...
Hint_Metrics:
{
          ...
          style_metrics->digits_have_same_width)))
    {
        slot->metrics.horiAdvance =
            FT_MulFix(slot->metrics.horiAdvance, style_metrics->scaler.x_scale);
    ...
    slot->metrics.vertAdvance =
        FT_MulFix(slot->metrics.vertAdvance, style_metrics->scaler.y_scale);
...
af_loader_compute_darkening(AF_Loader loader, FT_Face face,
                            FT_Pos standard_width)
{
    ...
    ppem = FT_MAX(af_intToFixed(4), af_intToFixed(face->size->metrics.x_ppem));
    units_per_EM = face->units_per_EM;

    em_ratio = FT_DivFix(af_intToFixed(1000), af_intToFixed(units_per_EM));
    ...
    log_base_2 =
        FT_MSB((FT_UInt32)stem_width_per_1000) + FT_MSB((FT_UInt32)ppem);
    ...
    else if (scaled_stem < af_intToFixed(x2))
    {
        FT_Int xdelta = x2 - x1;
        FT_Int ydelta = y2 - y1;
        FT_Int x = stem_width_per_1000 - FT_DivFix(af_intToFixed(x1), ppem);

        if (!xdelta)
            goto Try_x3;

        darken_amount =
            FT_MulDiv(x, ydelta, xdelta) + FT_DivFix(af_intToFixed(y1), ppem);
    }
    ...
    Try_x3:
    {
        FT_Int xdelta = x3 - x2;
        FT_Int ydelta = y3 - y2;
        FT_Int x = stem_width_per_1000 - FT_DivFix(af_intToFixed(x2), ppem);

        if (!xdelta)
            goto Try_x4;

        darken_amount =
            FT_MulDiv(x, ydelta, xdelta) + FT_DivFix(af_intToFixed(y2), ppem);
    }
    }
    ...
    Try_x4:
    {
        FT_Int xdelta = x4 - x3;
        FT_Int ydelta = y4 - y3;
        FT_Int x = stem_width_per_1000 - FT_DivFix(af_intToFixed(x3), ppem);

        if (!xdelta)
            goto Use_y4;

        darken_amount =
            FT_MulDiv(x, ydelta, xdelta) + FT_DivFix(af_intToFixed(y3), ppem);
    }
    }
