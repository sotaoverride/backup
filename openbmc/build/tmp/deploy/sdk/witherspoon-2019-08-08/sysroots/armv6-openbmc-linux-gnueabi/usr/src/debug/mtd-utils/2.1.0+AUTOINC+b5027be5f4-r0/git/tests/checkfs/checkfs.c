 ...
 * limitations under the Licence.
  ...
  fixed small issue with write() checking for < 0 instead of < (bytes to be
 written). Now it does the latter (as it should).
  ...
  Changed to use write() rather than fwrite() when creating new file.
 Additionally, and more important, it now does a single write() for the entire
 data. This will enable us to use this program to test for power fail *data*
 reliability when writing over an existing file, specially on powr fail "safe"
 file systems as jffs/jffs2. Also added a new cmdline parameter "-e" that
 specifies the max # of errors that can be tolerated. This should be set to ZERO
 to test for the above, as old data should be reliabily maintained if the newer
 write never "took" before power failed. If the write did succeed, then the
 newer data will have its own CRC in place when it gets checked => hence no
 error. In theory at least!
...
#define MAX_INTS_ALLOW                                                         \
    100000                  /* max # of int's in the file written.             \
...
static int setupSerial(const char* dev)
{
    ...
    {
        fprintf(stderr, "Could not get terminal attributes: %s",
                strerror(errno));
        exit(1);
    }

    tios.c_cflag = CS7 | CREAD | // Enable Receiver
...
static int check_crc_ccitt(char* filename)
{
    ...
    if (!fp)
    {
        ...
        fprintf(logfp,
                "Verify checksum:Error! Cannot open filename passed for verify "
                "checksum: %s\n",
                filename);
        fclose(logfp);
        return FALSE;
    ...
    if (len == 0)
    {
        ...
        logfp = fopen("logfile", "a"); /*open for appending only.*/
        fprintf(logfp,
                "verify checksum: first int claims there are 0 data in file. "
                "Error!: %s\n",
                filename);
        fclose(fp);
        fclose(logfp);
...
static void send_pwrdn_ok(void)
{
    ...
    if (!cyclefp)
    {
        fprintf(stderr, "Error! cannot open file for write:\"cycleCnt\". "
                        "Cannot continue.\n");
        exit(1);
    }
...
static void make_new_file(char* filename)
{
    ...
    /*file should not be less than 5 ints long. (so that we have decent length
      files, that's all)*/
    while (((data_size = (int)(1 + (int)((FileSizeMax / sizeof(int)) * rand() /
                                         (RAND_MAX + 1.0)))) < 5))
        ;
    ...
    /* Don't forget to increase the size of data written by the 2 chars of CRC
     at end. These 2 bytes are NOT included in the sizeInBytes field. */
    if ((err =
             write(dfd, (void*)&dataBuf, dataBuf.sizeInBytes + sizeof(short))) <
        (dataBuf.sizeInBytes + sizeof(short)))
    {
        printf("Error writing data buffer to file. Written %i bytes rather "
               "than %i bytes.",
    ...
    /* Now that the data is (hopefully) safely written. I can truncate the file
       to the new length so that I can reclaim any unused space, if the older
       file was larger.
     */
...
static void printHelp(char** argv)
{
    ...
    printf("%s <n>: Set Max errors allowed when checking all files for CRC on "
           "start.\n",
           CMDLINE_MAXERROR);
...
static void processCmdLine(int argc, char** argv)
{
    ...
    for (cnt = 1; cnt < argc; cnt++)
    {
        ...
        else if (strcmp(argv[cnt], CMDLINE_MAXFILEBYTES) == 0)
        {
        ...
        else if (strcmp(argv[cnt], CMDLINE_HELPSHORT) == 0)
        {
            printHelp(argv);
            exit(0);
        }
        else if (strcmp(argv[cnt], CMDLINE_HELPLONG) == 0)
        {
            printHelp(argv);
            exit(0);
        }
...
int main(int argc, char** argv)
{
    ...
    for (counter = 0; counter < MAX_NUM_FILES; counter++)
    {
        ...
        if (!check_crc_ccitt(filename))
        {
            ...
            if (errorCnt > MaxErrAllowed)
            {
                ...
                {
                    fprintf(stderr,
                            "Error! Cannot sync file data with disk.\n");
                    exit(1);
                }
                fclose(logfp);
                (void)sync();
    ...
    */
    ...
    filenameCounter = (int)(1 + (int)((float)(MAX_NUM_FILES - 1) * rand() /
                                      (RAND_MAX + 1.0)));
