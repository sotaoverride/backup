...
#define FOREACH_INOTIFY_EVENT(e, buffer, sz)                                   \
    for ((e) = &buffer.ev; (uint8_t*)(e) < (uint8_t*)(buffer.raw) + (sz);      \
         (e) =                                                                 \
             (struct inotify_event*)((uint8_t*)(e) +                           \
...
{
    CHASE_PREFIX_ROOT =
        1 << 0, /* If set, the specified path will be prefixed by the specified
                   root before beginning the iteration */
    CHASE_NONEXISTENT =
        1 << 1, /* If set, it's OK if the path doesn't actually exist. */
    CHASE_NO_AUTOFS =
        1 << 2, /* If set, return -EREMOTE if autofs mount point found */
    CHASE_SAFE = 1 << 3, /* If set, return EPERM if we ever traverse from
                            unprivileged to privileged files or directories */
    CHASE_OPEN =
    ...
    CHASE_NOFOLLOW =
        1 << 7, /* Only valid with CHASE_OPEN: when the path's right-most
