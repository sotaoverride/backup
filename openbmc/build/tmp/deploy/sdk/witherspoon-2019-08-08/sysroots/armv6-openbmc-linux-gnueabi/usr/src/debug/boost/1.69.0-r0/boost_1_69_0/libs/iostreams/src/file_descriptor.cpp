...
void file_descriptor_impl::open(const detail::path& p, BOOST_IOS::openmode mode)
{
    ...
    HANDLE handle =
        p.is_wide()
            ? ::CreateFileW(p.c_wstr(), dwDesiredAccess,
                            FILE_SHARE_READ | FILE_SHARE_WRITE,
                            NULL, // lpSecurityAttributes
                            dwCreationDisposition, FILE_ATTRIBUTE_NORMAL,
                            NULL)
            : // hTemplateFile
            ::CreateFileA(p.c_str(), dwDesiredAccess,
                          FILE_SHARE_READ | FILE_SHARE_WRITE,
                          NULL, // lpSecurityAttributes
                          dwCreationDisposition, FILE_ATTRIBUTE_NORMAL,
                          NULL); // hTemplateFile
...
std::streampos file_descriptor_impl::seek(stream_offset off,
                                          BOOST_IOS::seekdir way)
{
    ...
    if (dwResultLow == INVALID_SET_FILE_POINTER && ::GetLastError() != NO_ERROR)
    {
        boost::throw_exception(system_failure("failed seeking"));
    }
    else
    {
        return offset_to_position((stream_offset(lDistanceToMoveHigh) << 32) +
                                  dwResultLow);
    }
#else  // #ifdef BOOST_IOSTREAMS_WINDOWS
    ...
    stream_offset result = BOOST_IOSTREAMS_FD_SEEK(
        handle_, static_cast<BOOST_IOSTREAMS_FD_OFFSET>(off),
...
file_descriptor::file_descriptor(const char* path, BOOST_IOS::openmode mode) :
    pimpl_(new impl_type)
{
    open(path, mode);
}
...
detail::file_handle file_descriptor::handle() const
{
...
void file_descriptor::open(const detail::path& path, BOOST_IOS::openmode mode,
...
file_descriptor_source::file_descriptor_source(handle_type fd,
...
file_descriptor_source::file_descriptor_source(const std::string& path,
                                               BOOST_IOS::openmode mode)
{
    open(path, mode);
}

file_descriptor_source::file_descriptor_source(const char* path,
                                               BOOST_IOS::openmode mode)
{
    open(path, mode);
}
...
void file_descriptor_source::open(const std::string& path,
                                  BOOST_IOS::openmode mode)
{
    open(detail::path(path), mode);
}

void file_descriptor_source::open(const char* path, BOOST_IOS::openmode mode)
{
    open(detail::path(path), mode);
}

void file_descriptor_source::open(const detail::path& path,
                                  BOOST_IOS::openmode mode)
{
...
file_descriptor_sink::file_descriptor_sink(handle_type fd,
                                           file_descriptor_flags f)
{
    open(fd, f);
}
...
#if defined(BOOST_IOSTREAMS_USE_DEPRECATED)
file_descriptor_sink::file_descriptor_sink(handle_type fd, bool close_on_exit)
{
    open(fd, close_on_exit);
}
#endif
...
file_descriptor_sink::file_descriptor_sink(const std::string& path,
                                           BOOST_IOS::openmode mode)
{
    open(path, mode);
}

file_descriptor_sink::file_descriptor_sink(const char* path,
                                           BOOST_IOS::openmode mode)
{
    open(path, mode);
}
...
void file_descriptor_sink::open(const std::string& path,
                                BOOST_IOS::openmode mode)
{
    open(detail::path(path), mode);
}

void file_descriptor_sink::open(const char* path, BOOST_IOS::openmode mode)
{
    open(detail::path(path), mode);
}

void file_descriptor_sink::open(const detail::path& path,
                                BOOST_IOS::openmode mode)
{
