...
template <class T>
inline BOOST_CONSTEXPR_OR_CONST T make_big_value(largest_float v, const char*,
                                                 mpl::true_ const&,
                                                 mpl::false_ const&)
...
#endif
template <class T>
inline BOOST_MATH_CONSTEXPR T make_big_value(largest_float, const char* s,
                                             mpl::false_ const&,
                                             mpl::true_ const&)
    BOOST_MATH_NOEXCEPT(T)
{
...
#define BOOST_MATH_BIG_CONSTANT(T, D, x)                                       \
    boost::math::tools::make_big_value<T>(                                     \
        BOOST_MATH_LARGEST_FLOAT_C(x), BOOST_STRINGIZE(x),                     \
        mpl::bool_ < (is_convertible<boost::math::tools::largest_float,        \
                                     T>::value) &&                             \
            ((D <= boost::math::tools::numeric_traits<                         \
                       boost::math::tools::largest_float>::digits) ||          \
             ...
             (boost::math::tools::numeric_traits<T>::is_specialized &&         \
              (boost::math::tools::numeric_traits<T>::digits10 <=              \
               boost::math::tools::numeric_traits<                             \
                   boost::math::tools::largest_float>::digits10))) > (),       \
        boost::is_convertible<const char*, T>())
//
// For constants too huge for any conceivable long double (and which generate
// compiler errors if we try and declare them as such):
//
#define BOOST_MATH_HUGE_CONSTANT(T, D, x)                                      \
    boost::math::tools::make_big_value<T>(                                     \
        0.0L, BOOST_STRINGIZE(x),                                              \
