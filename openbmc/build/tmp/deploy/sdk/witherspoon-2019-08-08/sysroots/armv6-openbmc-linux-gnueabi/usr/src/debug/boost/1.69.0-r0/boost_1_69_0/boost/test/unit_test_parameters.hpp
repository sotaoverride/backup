...
template <typename T>
inline T const& get(runtime::cstring parameter_name)
{
...
class stream_holder
{
  public:
    ...
    void setup(const const_string& stream_name,
               boost::function<void()> const& cleaner_callback =
                   boost::function<void()>())
    {
        ...
        if (stream_name == "stderr")
        {
            ...
            {
                m_cleaner =
                    boost::make_shared<callback_cleaner>(cleaner_callback);
            }
            else
            {
                m_cleaner.reset();
        ...
        else
        {
            ...
            m_cleaner->m_file.open(
                std::string(stream_name.begin(), stream_name.end()).c_str());
  ...
  private:
    struct callback_cleaner
    {
        callback_cleaner(boost::function<void()> cleaner_callback) :
            m_cleaner_callback(cleaner_callback), m_file()
        {
        }
        ~callback_cleaner()
        {
