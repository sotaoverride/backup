...
static const GVariantType* g_simple_action_get_parameter_type(GAction* action)
{
    GSimpleAction* simple = G_SIMPLE_ACTION(action);
...
static void g_simple_action_change_state(GAction* action, GVariant* value)
{
    GSimpleAction* simple = G_SIMPLE_ACTION(action);
    ...
    if (g_signal_has_handler_pending(
            action, g_simple_action_signals[SIGNAL_CHANGE_STATE], 0, TRUE))
        g_signal_emit(action, g_simple_action_signals[SIGNAL_CHANGE_STATE], 0,
                      value);
...
void g_simple_action_set_state(GSimpleAction* simple, GVariant* value)
{
...

    {
        ...
        state_type = simple->state ? g_variant_get_type(simple->state) : NULL;
...
static void g_simple_action_activate(GAction* action, GVariant* parameter)
{
    ...
    g_return_if_fail(
        simple->parameter_type == NULL
            ? parameter == NULL
            : (parameter != NULL &&
               g_variant_is_of_type(parameter, simple->parameter_type)));
    ...
    if (simple->enabled)
    {
         ...
         */
        if (g_signal_has_handler_pending(
                action, g_simple_action_signals[SIGNAL_ACTIVATE], 0, TRUE))
            g_signal_emit(action, g_simple_action_signals[SIGNAL_ACTIVATE], 0,
                          parameter);
        ...
        else if (simple->state)
        {
            ...
            if (parameter == NULL &&
                g_variant_is_of_type(simple->state, G_VARIANT_TYPE_BOOLEAN))
            {
                gboolean was_enabled = g_variant_get_boolean(simple->state);
                g_simple_action_change_state(
                    action, g_variant_new_boolean(!was_enabled));
            }

            /* else, if the parameter and state type are the same, do a
             * change-state */
            else if (g_variant_is_of_type(simple->state,
...
static void g_simple_action_set_property(GObject* object, guint prop_id,
...
static void g_simple_action_get_property(GObject* object, guint prop_id,
...
void g_simple_action_class_init(GSimpleActionClass* class)
{
     ...
     * @parameter: (nullable): the parameter to the activation, or %NULL if it
     * has no parameter
     *
     ...
     * Since: 2.28
     */
    g_simple_action_signals[SIGNAL_ACTIVATE] = g_signal_new(
        I_("activate"), G_TYPE_SIMPLE_ACTION,
        G_SIGNAL_RUN_LAST | G_SIGNAL_MUST_COLLECT, 0, NULL, NULL,
        g_cclosure_marshal_VOID__VARIANT, G_TYPE_NONE, 1, G_TYPE_VARIANT);
     ...
     * Since: 2.30
     */
    g_simple_action_signals[SIGNAL_CHANGE_STATE] = g_signal_new(
        I_("change-state"), G_TYPE_SIMPLE_ACTION,
        G_SIGNAL_RUN_LAST | G_SIGNAL_MUST_COLLECT, 0, NULL, NULL,
        g_cclosure_marshal_VOID__VARIANT, G_TYPE_NONE, 1, G_TYPE_VARIANT);
    ...
    g_object_class_install_property(
        ...
        g_param_spec_variant(
            "state", P_("State"), P_("The state the action is in"),
                                   ...
                                   const GVariantType* parameter_type)
{
    ...
    return g_object_new(G_TYPE_SIMPLE_ACTION, "name", name, "parameter-type",
                        parameter_type, NULL);
}
...
{
    return g_object_new(G_TYPE_SIMPLE_ACTION, "name", name, "parameter-type",
                        parameter_type, "state", state, NULL);
}
