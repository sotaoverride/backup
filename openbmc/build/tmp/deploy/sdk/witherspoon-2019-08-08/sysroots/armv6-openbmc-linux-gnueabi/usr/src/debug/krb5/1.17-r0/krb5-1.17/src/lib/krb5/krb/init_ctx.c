...
/*
 * Copyright 1994,1999,2000, 2002, 2003, 2007, 2008, 2009  by the Massachusetts
...
static krb5_error_code get_integer(krb5_context ctx, const char* name,
                                   int def_val, int* int_out)
{
    krb5_error_code retval;

    retval = profile_get_integer(ctx->profile, KRB5_CONF_LIBDEFAULTS, name,
                                 NULL, def_val, int_out);
    if (retval)
        TRACE_PROFILE_ERR(ctx, name, KRB5_CONF_LIBDEFAULTS, retval);
    return retval;
}

static krb5_error_code get_boolean(krb5_context ctx, const char* name,
                                   int def_val, int* boolean_out)
{
    krb5_error_code retval;

    retval = profile_get_boolean(ctx->profile, KRB5_CONF_LIBDEFAULTS, name,
                                 NULL, def_val, boolean_out);
    if (retval)
        TRACE_PROFILE_ERR(ctx, name, KRB5_CONF_LIBDEFAULTS, retval);
    return retval;
}
...
krb5_error_code krb5int_init_context_kdc(krb5_context* context)
{
...
krb5_error_code KRB5_CALLCONV krb5_init_context_profile(
    profile_t profile, krb5_flags flags, krb5_context* context_out)
{
    krb5_context ctx = 0;
    krb5_error_code retval;
    struct
    ...
    *context_out = NULL;
    ...
    if ((retval = krb5_c_random_os_entropy(ctx, 0, NULL)) != 0)
        goto cleanup;
    if ((retval =
             krb5_crypto_us_timeofday(&seed_data.now, &seed_data.now_usec)))
        goto cleanup;
    seed_data.pid = getpid();
    seed.length = sizeof(seed_data);
    seed.data = (char*)&seed_data;
    if ((retval =
             krb5_c_random_add_entropy(ctx, KRB5_C_RANDSOURCE_TIMING, &seed)))
        goto cleanup;
    ...
    get_integer(ctx, KRB5_CONF_KDC_REQ_CHECKSUM_TYPE, CKSUMTYPE_RSA_MD5, &tmp);
    ctx->kdc_req_sumtype = tmp;

    get_integer(ctx, KRB5_CONF_AP_REQ_CHECKSUM_TYPE, 0, &tmp);
    ctx->default_ap_req_sumtype = tmp;

    get_integer(ctx, KRB5_CONF_SAFE_CHECKSUM_TYPE, CKSUMTYPE_RSA_MD5_DES, &tmp);
    ctx->default_safe_sumtype = tmp;

    get_integer(ctx, KRB5_CONF_KDC_DEFAULT_OPTIONS, KDC_OPT_RENEWABLE_OK, &tmp);
...
#define DEFAULT_KDC_TIMESYNC 1
    ...
    if (retval)
    {
        TRACE_PROFILE_ERR(ctx, KRB5_CONF_PLUGIN_BASE_DIR, KRB5_CONF_LIBDEFAULTS,
                          retval);
        goto cleanup;
    }
...
void KRB5_CALLCONV krb5_free_context(krb5_context ctx)
{
    if (ctx == NULL)
        return;
 ...
 */
static krb5_error_code set_default_etype_var(krb5_context context,
...
krb5_error_code krb5_set_default_in_tkt_ktypes(krb5_context context,
                                               const krb5_enctype* etypes)
{
...
krb5_error_code krb5_set_default_tgs_ktypes(krb5_context context,
                                            const krb5_enctype* etypes)
{
    return set_default_etype_var(context, etypes, &context->tgs_etypes);
}
 ...
 */
static void mod_list(krb5_enctype etype, krb5_boolean add,
                     krb5_boolean allow_weak, krb5_enctype** list_ptr)
{
    size_t i;
 ...
 * not set.
 */
static krb5_error_code get_profile_etype_list(krb5_context context,
                                              krb5_enctype** etypes_ptr,
                                              char* profkey,
...
krb5_error_code krb5_get_default_in_tkt_ktypes(krb5_context context,
                                               krb5_enctype** ktypes)
{
    return get_profile_etype_list(context, ktypes,
                                  KRB5_CONF_DEFAULT_TKT_ENCTYPES,
                                  context->in_tkt_etypes, default_enctype_list);
}
...
krb5_error_code KRB5_CALLCONV krb5_get_tgs_ktypes(krb5_context context,
                                                  ...
                                                  krb5_enctype** ktypes)
{
    if (context->use_conf_ktypes)
        ...
        return get_profile_etype_list(
            context, ktypes, KRB5_CONF_DEFAULT_TGS_ENCTYPES,
            context->tgs_etypes, default_enctype_list);
}

krb5_error_code KRB5_CALLCONV krb5_get_permitted_enctypes(krb5_context context,
                                                          krb5_enctype** ktypes)
{
...
krb5_boolean krb5_is_permitted_enctype(krb5_context context, krb5_enctype etype)
{
    krb5_enctype* list;
    krb5_boolean ret;
