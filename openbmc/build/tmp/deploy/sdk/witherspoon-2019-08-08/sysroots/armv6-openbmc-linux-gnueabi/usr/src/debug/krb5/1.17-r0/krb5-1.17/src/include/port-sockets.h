...
#define SG_ADVANCE(SG, N)                                                      \
    ((SG)->len < (N) ? (abort(), 0) : ((SG)->buf += (N), (SG)->len -= (N), 0))
...
#define SOCKET_WRITEV(FD, SG, LEN, TMP)                                        \
    (WSASend((FD), (SG), (LEN), &(TMP), 0, 0, 0) ? (ssize_t)-1 : (ssize_t)(TMP))
...
static inline int socket_connect(int fd, const struct sockaddr* addr,
                                 socklen_t addrlen)
{
    int st;
#ifdef SO_NOSIGPIPE
...
static inline ssize_t socket_sendmsg(SOCKET fd, sg_buf* iov, int iovcnt)
{
    struct msghdr msg;
    int flags = 0;
