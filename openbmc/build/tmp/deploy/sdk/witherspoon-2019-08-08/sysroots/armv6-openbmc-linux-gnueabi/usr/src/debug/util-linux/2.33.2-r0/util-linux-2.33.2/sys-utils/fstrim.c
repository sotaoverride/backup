...
struct fstrim_control
{
    ...
    unsigned int verbose : 1, fstab : 1, dryrun : 1;
};
                             ...
                             const char* devname)
{
    ...
    if (ctl->verbose)
    {
        char* str = size_to_human_string(
            SIZE_SUFFIX_3LETTER | SIZE_SUFFIX_SPACE, (uint64_t)range.len);
        if (devname)
            /* TRANSLATORS: The standard value here is a very large number. */
            printf(_("%s: %s (%" PRIu64 " bytes) trimmed on %s\n"), path, str,
                   (uint64_t)range.len, devname);
        else
            /* TRANSLATORS: The standard value here is a very large number. */
...
static int uniq_fs_source_cmp(struct libmnt_table* tb
                              __attribute__((__unused__)),
                              struct libmnt_fs* a, struct libmnt_fs* b)
{
    if (mnt_fs_is_pseudofs(a) || mnt_fs_is_netfs(a) || mnt_fs_is_pseudofs(b) ||
        mnt_fs_is_netfs(b))
        return 1;
...
static int fstrim_all(struct fstrim_control* ctl)
{
    ...
    {
        const char *src = mnt_fs_get_srcpath(fs), *tgt = mnt_fs_get_target(fs);
        char* path;
        int rc = 1;
...
static void __attribute__((__noreturn__)) usage(void)
{
    ...
    fprintf(out, _(" %s [options] <mount point>\n"),
    ...
    fputs(_(" -a, --all           trim all supported mounted filesystems\n"),
          out);
    fputs(_(" -A, --fstab         trim all supported mounted filesystems from "
            "/etc/fstab\n"),
          out);
    fputs(_(" -o, --offset <num>  the offset in bytes to start discarding "
            "from\n"),
          out);
...
int main(int argc, char** argv)
{
    char* path = NULL;
    int c, rc, all = 0;
    struct fstrim_control ctl = {.range = {.len = ULLONG_MAX}};
    ...
    while ((c = getopt_long(argc, argv, "Aahl:m:no:Vv", longopts, NULL)) != -1)
    {
        switch (c)
        {
            ...
            case 'l':
                ctl.range.len =
                    strtosize_or_err(optarg, _("failed to parse length"));
                break;
            case 'o':
                ctl.range.start =
                    strtosize_or_err(optarg, _("failed to parse offset"));
                break;
            case 'm':
                ctl.range.minlen = strtosize_or_err(
                    optarg, _("failed to parse minimum extent length"));
                break;
            case 'v':
                ctl.verbose = 1;
                break;
