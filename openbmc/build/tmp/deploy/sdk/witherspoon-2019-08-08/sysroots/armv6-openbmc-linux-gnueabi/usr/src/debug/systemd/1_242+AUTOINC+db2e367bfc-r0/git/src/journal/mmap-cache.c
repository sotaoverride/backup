...
static void window_invalidate(Window* w)
{
    ...
    assert_se(mmap(w->ptr, w->size, w->prot,
                   MAP_PRIVATE | MAP_ANONYMOUS | MAP_FIXED, -1, 0) == w->ptr);
    w->invalidated = true;
}
...
_pure_ static bool window_matches(Window* w, int prot, uint64_t offset,
                                  size_t size)
{
    assert(w);
    assert(size > 0);

    return prot == w->prot && offset >= w->offset &&
           offset + size <= w->offset + w->size;
}

_pure_ static bool window_matches_fd(Window* w, MMapFileDescriptor* f, int prot,
...
static Window* window_add(MMapCache* m, MMapFileDescriptor* f, int prot,
                          bool keep_always, uint64_t offset, size_t size,
                          void* ptr)
{
    Window* w;
...
static int try_context(MMapCache* m, MMapFileDescriptor* f, int prot,
                       unsigned context, bool keep_always, uint64_t offset,
                       size_t size, void** ret, size_t* ret_size)
{
...
static int find_mmap(MMapCache* m, MMapFileDescriptor* f, int prot,
                     unsigned context, bool keep_always, uint64_t offset,
                     size_t size, void** ret, size_t* ret_size)
{
...
static int mmap_try_harder(MMapCache* m, void* addr, MMapFileDescriptor* f,
                           int prot, int flags, uint64_t offset, size_t size,
                           void** res)
{
    void* ptr;
...
static int add_mmap(MMapCache* m, MMapFileDescriptor* f, int prot,
                    unsigned context, bool keep_always, uint64_t offset,
                    size_t size, struct stat* st, void** ret, size_t* ret_size)
{
...
int mmap_cache_get(MMapCache* m, MMapFileDescriptor* f, int prot,
                   unsigned context, bool keep_always, uint64_t offset,
                   size_t size, struct stat* st, void** ret, size_t* ret_size)
{
