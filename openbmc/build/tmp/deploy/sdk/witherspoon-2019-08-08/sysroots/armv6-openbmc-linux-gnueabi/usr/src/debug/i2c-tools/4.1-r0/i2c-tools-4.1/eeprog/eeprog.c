...
void do_usage_if(int b, int line)
{
    static const char* eeprog_usage =
        ...
        "Usage: eeprog [-fqxdh] [-16|-8] [ -r addr[:count] | -w addr ]  "
        "/dev/i2c-N  i2c-address\n"
        "\n"
        "  Address modes:\n"
        ...
        "	-q		Quiet mode\n"
        "\n"
        "The following environment variables could be set instead of the "
        "command\n"
        "line arguments:\n"
        ...
        "\n"
        "	Examples\n"
        "	1- read 64 bytes from the EEPROM at address 0x54 on bus 0 "
        "starting\n"
...
int main(int argc, char** argv)
{
    ...
    die_if(eeprom_open(device, i2c_addr, eeprom_type, &e) < 0,
           "unable to open eeprom device file (check that the file exists and "
           "that it's readable)");
    switch (op)
    {
        case 'r':
