...
#else
    b = avahi_new0(uint8_t,
                   sizeof(struct ArpPacket) + ETHER_HDR_SIZE + packet_len);
    p = (ArpPacket*)b;
...
static ArpPacket* packet_new_with_info(const ArpPacketInfo* info,
                                       size_t* packet_len)
{
    ArpPacket* p = NULL;
    uint8_t* r;
...
static ArpPacket* packet_new_probe(uint32_t ip_address,
...
static ArpPacket* packet_new_announcement(uint32_t ip_address,
                                          const uint8_t* hw_address,
                                          size_t* packet_len)
{
    ArpPacketInfo info;
...
static int packet_parse(const ArpPacket* packet, size_t packet_len,
                        ArpPacketInfo* info)
{
    const uint8_t* p;
    ...
    if (p[4] != ETHER_ADDRLEN || p[5] != 4 || p[6] != 0 ||
        (p[7] != 1 && p[7] != 2))
        return -1;
...
static void set_state(State st, int reset_counter, uint32_t address)
{
    ...
    {
        n_iteration++;
        DEBUG(daemon_log(LOG_DEBUG, "State iteration %s-%i", state_table[state],
                         n_iteration));
    }
    else
    {
        DEBUG(daemon_log(LOG_DEBUG, "State transition %s-%i -> %s-0",
                         state_table[state], n_iteration, state_table[st]));
        state = st;
        n_iteration = 0;
    }
    ...
    if (state == STATE_SLEEPING)
        ...
        avahi_set_proc_title(argv0, "%s: [%s] announcing %s", argv0,
                             interface_name,
                             inet_ntop(AF_INET, &address, buf, sizeof(buf)));
    else if (state == STATE_RUNNING)
        avahi_set_proc_title(argv0, "%s: [%s] bound %s", argv0, interface_name,
                             inet_ntop(AF_INET, &address, buf, sizeof(buf)));
    else
        avahi_set_proc_title(argv0, "%s: [%s] probing %s", argv0,
                             interface_name,
                             inet_ntop(AF_INET, &address, buf, sizeof(buf)));
}
...
static int send_packet(int fd, int iface, ArpPacket* packet, size_t packet_len)
{
    ...
    if (sendto(fd, packet->ether_payload, packet_len, 0, (struct sockaddr*)&sa,
               sizeof(sa)) < 0)
    {
...
static int recv_packet(int fd, ArpPacket** packet, size_t* packet_len)
{
    ...
    *packet = packet_new(s);
    ...
    sa_len = sizeof(sa);
    if ((r = recvfrom(fd, (*packet)->ether_payload, s, 0, (struct sockaddr*)&sa,
                      &sa_len)) < 0)
    {
...
static int open_socket(int iface, uint8_t* hw_address)
{
    ...
    filter = avahi_strdup_printf("arp and (ether dst ff:ff:ff:ff:ff:ff or "
                                 ...
                                 __lladdr[0], __lladdr[1], __lladdr[2],
                                 __lladdr[3], __lladdr[4], __lladdr[5]);
 ...
 */
static int send_packet(int fd AVAHI_GCC_UNUSED, int iface AVAHI_GCC_UNUSED,
                       ArpPacket* packet, size_t packet_len)
{
...
static int recv_packet(int fd AVAHI_GCC_UNUSED, ArpPacket** packet,
                       size_t* packet_len)
{
...
static struct timeval* elapse_time(struct timeval* tv, unsigned msec,
                                   unsigned jitter)
{
    assert(tv);
    ...
    if (jitter)
        avahi_timeval_add(
            tv, (AvahiUsec)(jitter * 1000.0 * rand() / (RAND_MAX + 1.0)));
...
static FILE* fork_dispatcher(void)
{
    ...
    else if (pid == 0)
    {
        ...
        avahi_set_proc_title(argv0, "%s: [%s] callout dispatcher", argv0,
                             interface_name);
        ...
        for (;;)
        {
            ...
            {
                daemon_log(LOG_ERR, "if_indextoname() failed: %s",
                           strerror(errno));
                continue;
            }

            if (daemon_exec("/", &k, action_script, action_script,
                            callout_event_table[info.event], name,
                            inet_ntop(AF_INET, &info.address, buf, sizeof(buf)),
                            NULL) < 0)
            {

                daemon_log(LOG_ERR, "Failed to run script: %s",
                           strerror(errno));
                continue;
            }
            ...
            if (k != 0)
                daemon_log(LOG_WARNING,
                           "Script execution failed with return value %i", k);
        }
...
static int do_callout(FILE* f, CalloutEvent event, int iface, uint32_t addr)
{
    ...
    {
        daemon_log(LOG_ERR, "Failed to write callout event: %s",
                   strerror(errno));
        return -1;
    }
...
static int drop_privs(void)
{
    ...
    if (!no_drop_root)
    {
        ...
        {
            daemon_log(LOG_ERR,
                       "Failed to find user '" AVAHI_AUTOIPD_USER "'.");
            return -1;
        }
        ...
        {
            daemon_log(LOG_ERR,
                       "Failed to find group '" AVAHI_AUTOIPD_GROUP "'.");
            return -1;
        }

        daemon_log(LOG_INFO,
                   "Found user '" AVAHI_AUTOIPD_USER
                   "' (UID %lu) and group '" AVAHI_AUTOIPD_GROUP "' (GID %lu).",
    ...
    if (r < 0 && errno != EEXIST)
    {
        daemon_log(LOG_ERR, "mkdir(\"" AVAHI_IPDATA_DIR "\"): %s",
                   strerror(errno));
        return -1;
    }
    ...
    if (!no_drop_root)
    {
        ...
        {
            daemon_log(LOG_ERR,
                       "Failed to create runtime directory " AVAHI_IPDATA_DIR
                       ".");
            return -1;
        }
    }
    ...
    if (!no_drop_root)
    {
        ...
        {
            daemon_log(LOG_ERR, "Failed to change group list: %s",
                       strerror(errno));
            return -1;
        }
...
static int loop(int iface, uint32_t addr)
{
    ...
    address_fn = avahi_strdup_printf(
        "%s/%02x:%02x:%02x:%02x:%02x:%02x", p, hw_address[0], hw_address[1],
        hw_address[2], hw_address[3], hw_address[4], hw_address[5]);
    ...
    {
        daemon_log(LOG_WARNING,
                   "Requested address %s is not from IPv4LL range 169.254/16 "
                   "or a reserved address, ignoring.",
                   inet_ntop(AF_INET, &addr, buf, sizeof(buf)));
        addr = 0;
    }
    ...
    daemon_log(LOG_INFO, "Starting with address %s",
               inet_ntop(AF_INET, &addr, buf, sizeof(buf)));
    ...
    for (;;)
    {
        ...
        if (state == STATE_START)
        {
            ...
            next_wakeup_valid = 1;
                 ...
                 (state == STATE_PROBING && event == EVENT_TIMEOUT &&
                  n_iteration < PROBE_NUM - 2))
        {
            ...
            elapse_time(&next_wakeup, PROBE_MIN * 1000,
                        (PROBE_MAX - PROBE_MIN) * 1000);
            next_wakeup_valid = 1;
        }
        else if (state == STATE_PROBING && event == EVENT_TIMEOUT &&
                 n_iteration >= PROBE_NUM - 2)
        {
            ...
            next_wakeup_valid = 1;
                 ...
                 (state == STATE_ANNOUNCING && event == EVENT_TIMEOUT &&
                  n_iteration < ANNOUNCE_NUM - 1))
        {

            /* Send announcement packet */
            out_packet =
                packet_new_announcement(addr, hw_address, &out_packet_len);
        ...
        else if ((state == STATE_ANNOUNCING && event == EVENT_TIMEOUT &&
                  n_iteration >= ANNOUNCE_NUM - 1))
        {

            daemon_log(LOG_INFO, "Successfully claimed IP address %s",
                       inet_ntop(AF_INET, &addr, buf, sizeof(buf)));
        ...
        else if (event == EVENT_PACKET)
        {
            ...
            else
            {
                ...
                if (info.sender_ip_address == addr)
                {
                    ...
                    if (memcmp(hw_address, info.sender_hw_address,
                               ETHER_ADDRLEN))
                    {
                        /* Normal conflict */
                        conflict = 1;
                        ...
                        daemon_log(LOG_DEBUG, "Received ARP packet back on "
                                              "source interface. Ignoring.");
                }
                else if (state == STATE_WAITING_PROBE ||
                         state == STATE_PROBING ||
                         state == STATE_WAITING_ANNOUNCE)
                {
                    /* Probe conflict */
                    conflict = info.target_ip_address == addr &&
                               info.sender_ip_address == 0 &&
                               memcmp(hw_address, info.sender_hw_address,
                                      ETHER_ADDRLEN);
                ...
                if (conflict)
                {
                    ...
                    if (state == STATE_RUNNING || state == STATE_ANNOUNCING)
                        if (do_callout(dispatcher, CALLOUT_CONFLICT, iface,
                                       addr) < 0)
                            goto fail;
                    ...
                    {
                        daemon_log(LOG_WARNING, "Got too many conflicts, rate "
                                                "limiting new probes.");
        ...
        else if (event == EVENT_ROUTABLE_ADDR_UNCONFIGURED &&
                 state == STATE_SLEEPING && !force_bind)
        {

            daemon_log(LOG_INFO, "No longer a routable address configured, "
...
static void help(FILE* f, const char* a0)
{
    fprintf(f,
            ...
            "    -s --syslog         Write log messages to syslog(3) instead "
            "of STDERR\n"
            "    -k --kill           Kill a running daemon\n"
            "    -r --refresh        Request a running daemon refresh its IP "
            "address\n"
            "    -c --check          Return 0 if a daemon is already running\n"
            "    -V --version        Show version\n"
            "    -S --start=ADDRESS  Start with this address from the IPv4LL "
            "range\n"
            "                        169.254.0.0/16\n"
            "    -t --script=script  Action script to run (defaults to\n"
            "                        " AVAHI_IPCONF_SCRIPT ")\n"
            "    -w --wait           Wait until an address has been acquired "
            "before\n"
            "                        daemonizing\n"
            "       --force-bind     Assign an IPv4LL address even if a "
            "routable address\n"
            "                        is already assigned\n"
...
static int parse_command_line(int argc, char* argv[])
{
    ...
    while ((c = getopt_long(argc, argv, "hDskrcVS:t:w", long_options, NULL)) >=
           0)
    {
    ...
    if (command == DAEMON_RUN || command == DAEMON_KILL ||
        command == DAEMON_REFRESH || command == DAEMON_CHECK)
    {
...
int main(int argc, char* argv[])
{
    ...
    daemon_log_ident = log_ident =
        avahi_strdup_printf("%s(%s)", argv0, interface_name);
    daemon_pid_file_proc = pid_file_proc;
    pid_file_name = avahi_strdup_printf(
        AVAHI_RUNTIME_DIR "/avahi-autoipd.%s.pid", interface_name);
    ...
    if (command == DAEMON_RUN)
    {
        ...
        {
            daemon_log(LOG_ERR,
                       "Failed to get index for interface name '%s': %s",
        ...
        {
            daemon_log(LOG_ERR, "Failed to create PID file: %s",
                       strerror(errno));
        ...
        avahi_set_proc_title(argv0, "%s: [%s] starting up", argv0,
                             interface_name);
