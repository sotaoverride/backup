...
#undef putc
#define putc(c, f)                                                             \
    (wide ? (int)_IO_putwc_unlocked(c, f) : _IO_putc_unlocked(c, f))
...
extern mp_size_t __mpn_extract_double(mp_ptr res_ptr, mp_size_t size, int* expt,
...
static wchar_t hack_digit(struct hack_digit_param* p)
{
    ...
    else if (p->scalesize == 0)
    {
        ...
        p->frac[p->fracsize - 1] =
            __mpn_mul_1(p->frac, p->frac, p->fracsize - 1, 10);
    }
    else
    {
        ...
        {
            hi = mpn_divmod(p->tmp, p->frac, p->fracsize, p->scale,
                            p->scalesize);
...
int __printf_fp_l(FILE* fp, locale_t loc, const struct printf_info* info,
...
#define GREATER_MANT_DIG LDBL_MANT_DIG
    ...
    /* We need just a few limbs for the input before shifting to the right
       position.	*/
    mp_limb_t
        fp_input[(GREATER_MANT_DIG + BITS_PER_MP_LIMB - 1) / BITS_PER_MP_LIMB];
    ...
    if (info->extra == 0)
    {
        ...
        decimalwc =
            _nl_lookup_word(loc, LC_NUMERIC, _NL_NUMERIC_DECIMAL_POINT_WC);
    }
    else
    {
        ...
        if (*decimal == '\0')
            decimal = _nl_lookup(loc, LC_NUMERIC, DECIMAL_POINT);
        decimalwc =
            _nl_lookup_word(loc, LC_MONETARY, _NL_MONETARY_DECIMAL_POINT_WC);
        if (decimalwc == L'\0')
            decimalwc =
                _nl_lookup_word(loc, LC_NUMERIC, _NL_NUMERIC_DECIMAL_POINT_WC);
    }
        ...
        else
        {
            ...
            if (wide)
            {
                if (info->extra == 0)
                    ...
                    thousands_sepwc = _nl_lookup_word(
                        loc, LC_MONETARY, _NL_MONETARY_THOUSANDS_SEP_WC);
            }
            else
            {
                if (info->extra == 0)
                    ...
                    thousands_sep =
                        _nl_lookup(loc, LC_MONETARY, MON_THOUSANDS_SEP);
            }
    ...
    {                                                                          \
        ...
        {                                                                      \
            p.fracsize = __mpn_extract_##SUFFIX(                               \
                fp_input, array_length(fp_input), &p.exponent, &is_neg, VAR);  \
...
#undef PRINTF_FP_FETCH
    ...
    {
        mp_size_t bignum_size =
            ((abs(p.exponent) + BITS_PER_MP_LIMB - 1) / BITS_PER_MP_LIMB +
    ...
    if (p.exponent > 2)
    {
        ...
        {
            cy = __mpn_lshift(
                p.frac + (p.exponent + to_shift) / BITS_PER_MP_LIMB, fp_input,
                p.fracsize, (p.exponent + to_shift) % BITS_PER_MP_LIMB);
            p.fracsize += (p.exponent + to_shift) / BITS_PER_MP_LIMB;
            if (cy)
    ...
    (((FLT128_MANT_DIG + BITS_PER_MP_LIMB - 1) / BITS_PER_MP_LIMB) -           \
     _FPIO_CONST_OFFSET)
                       ...
                       IEEE 854 quad long double (_Float128).  */
                        p.tmpsize =
                            powers->arraysize + _FLT128_FPIO_CONST_SHIFT;
                       ...
                       bigger too.  */
                        p.exponent +=
                            _FLT128_FPIO_CONST_SHIFT * BITS_PER_MP_LIMB;
                    }
                    else
...
#define _FPIO_CONST_SHIFT                                                      \
    ...
    else if (p.exponent < 0)
    {
            ...
            if (p.exponent >= powers->m_expo)
            {
               ...
               bigger than the second.  */
                if (p.fracsize < powers->arraysize - _FPIO_CONST_OFFSET)
                    cy = __mpn_mul(
                        p.tmp, &__tens[powers->arrayoff + _FPIO_CONST_OFFSET],
                        powers->arraysize - _FPIO_CONST_OFFSET, p.frac,
                        p.fracsize);
                else
                    cy = __mpn_mul(
                        p.tmp, p.frac, p.fracsize,
                        &__tens[powers->arrayoff + _FPIO_CONST_OFFSET],
                        powers->arraysize - _FPIO_CONST_OFFSET);
               ...
               to the right position.	 */
                if (incr == p.exponent + 3)
                {
                    ...
                    {
                        topval[0] = 0;
                        topval[1] = ((mp_limb_t)10)
                                    << (BITS_PER_MP_LIMB - 4 - cnt_h);
                    }
                    else
                    {
        ...
        if (p.exponent > 0)
        {
            ...
            if (cnt_l < MIN(4, p.exponent))
            {
                ...
                (void)__mpn_rshift(p.frac, p.tmp, p.tmpsize,
                                   MIN(4, p.exponent));
    ...
    else
    {
       ...
       leading digit.	 (Of course this division is not really made.)	*/
        ...
        /* Now shift the input value to its right place.	*/
        cy =
            __mpn_lshift(p.frac, fp_input, p.fracsize, (p.exponent + to_shift));
        p.frac[p.fracsize++] = cy;
        p.exponent = 0;
    }

    {
                       ...
                       more_bits, rounding_mode))
        {
            ...
            if (fracdig_no > 0)
            {
                ...
                if (*wtp != decimalwc)
                    /* Round up.  */
                    (*wtp)++;
                else if (__builtin_expect(spec == 'g' && p.type == 'f' &&
                                              info->alt && wtp == wstartp + 1 &&
                                              wstartp[0] == L'0',
                                          0))
                ...
                /* It is more critical.  All digits were 9's.  */
                {
                    if (p.type != 'f')
                    {
                        *wstartp = '1';
                        ...
                        /* The above p.exponent adjustment could lead
                           to 1.0e-00, e.g. for 0.999999999.  Make sure
                           p.exponent 0 always uses + sign.  */
                        if (p.exponent == 0)
                            p.expsign = 0;
                    }
        ...
        if (grouping)
        {
            ...
            wcp = group_number(wstartp, wcp, intdig_no, grouping,
                               thousands_sepwc, ngroups);
        }

        /* Write the p.exponent if it is needed.  */
        if (p.type != 'f')
        {
            if (__glibc_unlikely(p.expsign != 0 && p.exponent == 4 &&
                                 spec == 'g'))
            {
                /* This is another special case.  The p.exponent of the number
                   is really smaller than -4, which requires the 'e'/'E' format.
...

        {
            ...
            if (!wide)
            {
                ...
                if (info->i18n)
                    factor =
                        _nl_lookup_word(loc, LC_CTYPE, _NL_CTYPE_MB_CUR_MAX);
                else
                    factor = 1;
                ...
                for (cp = buffer, copywc = wstartp; copywc < wcp; ++copywc)
                    if (*copywc == decimalwc)
                        ...
                        cp = (char*)__mempcpy(cp, thousands_sep,
                                              thousands_sep_len);
                    else
                        *cp++ = (char)*copywc;
            }
...
ldbl_hidden_def(___printf_fp, __printf_fp)
    ...
    unsigned int __guess_grouping(unsigned int intdig_max, const char* grouping)
{
    unsigned int groups;
