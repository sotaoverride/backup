...
{
    qDeqID deqID;
    int nElemDeq; /* numbe of elements that were dequeued and as such must now
                     be discarded */
...
typedef enum
{
    QUEUETYPE_FIXED_ARRAY = 0, /* a simple queue made out of a fixed (initially
                                  malloced) array fast but memoryhog */
    QUEUETYPE_LINKEDLIST = 1,  /* linked list used as buffer, lower fixed memory
                                  overhead but slower */
...
struct queue_s
{
    ...
    sbool bSaveOnShutdown; /* persists everthing on shutdown (if DA!)? 1-yes,
                              0-no */
    sbool bQueueStarted;   /* has queueStart() been called on this queue? 1-yes,
                              0-no */
    ...
    int iMinMsgsPerWrkr;
    /* minimum nbr of msgs per worker thread, if more, a new worker is started
     * until max wrkrs */
    wtp_t* pWtpDA;
    wtp_t* pWtpReg;
    action_t* pAction;     /* for action queues, ptr to action object; for main
                              queues unused */
    int iUpdsSincePersist; /* nbr of queue updates since the last persist call
                            */
    int iPersistUpdCnt; /* persits queue info after this nbr of updates - 0 ->
                           persist only on shutdown */
    ...
    int iDiscardMrk;  /* if the queue is above this mark, low-severity messages
                         are discarded */
    int iFullDlyMrk;  /* if the queue is above this mark, FULL_DELAYable message
                         are put on hold */
    int iLightDlyMrk; /* if the queue is above this mark, LIGHT_DELAYable
                         message are put on hold */
    int iDiscardSeverity; /* messages of this severity above are discarded on
                             too-full queue */
    sbool bNeedDelQIF;    /* does the QIF file need to be deleted when queue
                             becomes empty? */
    int toQShutdown;      /* timeout for regular queue shutdown in ms */
    int toActShutdown;    /* timeout for long-running action shutdown in ms */
    int toWrkShutdown; /* timeout for idle workers in ms, -1 means indefinite (0
                          is immediate) */
    ...
    int iDeqBatchSize; /* max number of elements that shall be dequeued at once
                        */
    int iMinDeqBatchSize;  /* min number of elements that shall be dequeued at
                              once */
    ...
    int iDeqtWinToHr;   /* end of dequeue time window (hour only), set to 25 to
                           disable deq window! */
    /* note that begin and end have specific semantics. It is a big difference
     * if we have begin 4, end 22 or begin 22, end 4. In the later case,
     * dequeuing will run from 10p, throughout the night and stop at 4 in the
     * morning. In the first case, it will start at 4am, run throughout the day,
     * and stop at 10 in the evening! So far, not logic is applied to detect
     * user configuration errors (and tell me how should we detect what the user
     * really wanted...). -- rgerhards, 2008-04-02
     */
    /* end dequeue time window */
    rsRetVal (*pConsumer)(
        void*, batch_t*,
        wti_t*); /* user-supplied consumer function for dequeued messages */
    /* calling interface for pConsumer: arg1 is the global user pointer from
     * this structure, arg2 is the user pointer array that was dequeued (actual
     * sample: for actions, arg1 is the pAction and arg2 is pointer to an array
     * of message message pointers)
     */
    ...
    /* public entry points (set during construction, permit to set best
     * algorithm for params selected) */
    ...
    pthread_cond_t
        belowLightDlyWtrMrk; /* below eFLOWCTL_FULL_DELAY watermark */
    ...
    struct queue_s*
        pqParent;   /* pointer to the parent (if this is a child queue) */
    int bDAEnqOnly; /* EnqOnly setting for DA queue */
    /* now follow queueing mode specific data elements */
    // union {			/* different data elements based on queue type (qType)
    // */
    struct
    { /* different data elements based on queue type (qType) */
        struct
        {
...
/* the define below is an "eternal" timeout for the timeout settings which
 * require a value. It is one day, which is not really eternal, but comes close
 * to it if we think about rsyslog (e.g.: do you want to wait on shutdown for
 * more than a day? ;)) rgerhards, 2008-01-17
 */
