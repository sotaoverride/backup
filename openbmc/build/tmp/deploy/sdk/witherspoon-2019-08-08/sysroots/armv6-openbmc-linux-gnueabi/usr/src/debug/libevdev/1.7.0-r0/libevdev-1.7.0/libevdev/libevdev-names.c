...
static const struct name_entry* lookup_name(const struct name_entry* array,
                                            size_t asize,
...
LIBEVDEV_EXPORT int libevdev_event_type_from_name(const char* name)
{
    return libevdev_event_type_from_name_n(name, strlen(name));
}

LIBEVDEV_EXPORT int libevdev_event_type_from_name_n(const char* name,
                                                    size_t len)
{
    struct name_lookup lookup;
    const struct name_entry* entry;
...
LIBEVDEV_EXPORT int libevdev_event_code_from_name(unsigned int type,
                                                  const char* name)
{
    return libevdev_event_code_from_name_n(type, name, strlen(name));
}

LIBEVDEV_EXPORT int libevdev_event_code_from_name_n(unsigned int type,
                                                    const char* name,
                                                    size_t len)
{
    struct name_lookup lookup;
...
LIBEVDEV_EXPORT int libevdev_event_value_from_name(unsigned int type,
                                                   unsigned int code,
                                                   const char* name)
{
...
LIBEVDEV_EXPORT int libevdev_event_value_from_name_n(unsigned int type,
                                                     unsigned int code,
                                                     const char* name,
                                                     size_t len)
{
    ...
    entry =
        lookup_name(tool_type_names, ARRAY_LENGTH(tool_type_names), &lookup);

    return entry ? (int)entry->value : -1;
}

LIBEVDEV_EXPORT int libevdev_property_from_name(const char* name)
{
    return libevdev_property_from_name_n(name, strlen(name));
}

LIBEVDEV_EXPORT int libevdev_property_from_name_n(const char* name, size_t len)
{
    struct name_lookup lookup;
    const struct name_entry* entry;
...
LIBEVDEV_EXPORT int libevdev_event_code_from_code_name(const char* name)
{
    return libevdev_event_code_from_code_name_n(name, strlen(name));
}

LIBEVDEV_EXPORT int libevdev_event_code_from_code_name_n(const char* name,
                                                         size_t len)
{
    const struct name_entry* entry;
    struct name_lookup lookup;
...
LIBEVDEV_EXPORT int libevdev_event_type_from_code_name(const char* name)
{
    return libevdev_event_type_from_code_name_n(name, strlen(name));
}

LIBEVDEV_EXPORT int libevdev_event_type_from_code_name_n(const char* name,
                                                         size_t len)
{
    const struct name_entry* entry;
    struct name_lookup lookup;
