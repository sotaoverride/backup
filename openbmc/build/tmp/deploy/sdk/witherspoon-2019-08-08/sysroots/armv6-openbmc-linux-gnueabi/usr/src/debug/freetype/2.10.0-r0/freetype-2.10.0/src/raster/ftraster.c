 ...
 *   SUCCESS on success, FAILURE on render pool overflow.
 */
static Bool Bezier_Up(RAS_ARGS Int degree, TSplitter splitter, Long miny,
                      Long maxy)
{
    Long y1, y2, e, e2, e0;
    Short f1;
    ...
    do
    {
        ...
        if (y2 > e)
        {
            ...
            {
                *top++ = arc[degree].x +
                         FMulDiv(arc[0].x - arc[degree].x, e - y1, y2 - y1);
                arc -= degree;
                e += ras.precision;
            }
        }
 ...
 *   SUCCESS on success, FAILURE on render pool overflow.
 */
static Bool Bezier_Down(RAS_ARGS Int degree, TSplitter splitter, Long miny,
                        Long maxy)
{
    TPoint* arc = ras.arc;
    Bool result, fresh;
...
static Bool Line_To(RAS_ARGS Long x, Long y)
{
    ...
    {
        case Ascending_State:
            if (Line_Up(RAS_VARS ras.lastX, ras.lastY, x, y, ras.minY,
                        ras.maxY))
                return FAILURE;
            break;

        case Descending_State:
            if (Line_Down(RAS_VARS ras.lastX, ras.lastY, x, y, ras.minY,
                          ras.maxY))
                return FAILURE;
            break;
        ...
        switch (tag)
        {
            ...
            Do_Conic:
                if (point < limit)
                {
                    ...
                    if (Conic_To(RAS_VARS v_control.x, v_control.y, v_middle.x,
                                 v_middle.y))
                        goto Fail;
                ...
                if (Conic_To(RAS_VARS v_control.x, v_control.y, v_start.x,
                             v_start.y))
                    goto Fail;
...
static Bool Convert_Glyph(RAS_ARGS Int flipped)
{
    ...
    for (i = 0; i < ras.outline.n_contours; i++)
    {
        ...
        if (Decompose_Curve(RAS_VARS(UShort) start,
                            (UShort)ras.outline.contours[i], flipped))
            return FAILURE;

        start = (UShort)ras.outline.contours[i] + 1;

        /* we must now check whether the extreme arcs join or not */
        if (FRAC(ras.lastY) == 0 && ras.lastY >= ras.minY &&
            ras.lastY <= ras.maxY)
            if (ras.gProfile && (ras.gProfile->flags & Flow_Up) ==
                                    (ras.cProfile->flags & Flow_Up))
                ras.top--;
                                ...
                                PProfile left, PProfile right)
{
    ...
    FT_TRACE7(("  y=%d x=[%.12f;%.12f], drop-out=%d", y,
               x1 / (double)ras.precision, x2 / (double)ras.precision,
               dropOutControl));
    ...
    if (dropOutControl != 2 &&
        x2 - x1 - ras.precision <= ras.precision_jitter && e1 != x1 && e2 != x2)
        e2 = e1;
                                ...
                                PProfile left, PProfile right)
{
    ...
    FT_TRACE7(("  y=%d x=[%.12f;%.12f]", y, x1 / (double)ras.precision,
...
static void Horizontal_Sweep_Span(RAS_ARGS Short y, FT_F26Dot6 x1,
                                  FT_F26Dot6 x2, PProfile left, PProfile right)
{
    FT_UNUSED(left);
    FT_UNUSED(right);
    ...
    if (x2 - x1 < ras.precision)
    {
        ...
        FT_TRACE7(("  x=%d y=[%.12f;%.12f]", y, x1 / (double)ras.precision,
                   x2 / (double)ras.precision));
...
#endif
            ras.Proc_Sweep_Drop(RAS_VARS y, P_Left->X, P_Right->X, P_Left,
                                P_Right);
        }
...
static FT_Error Render_Glyph(RAS_ARG)
{
    ...
    ras.second_pass = (Bool)(!(ras.outline.flags & FT_OUTLINE_SINGLE_PASS));
