        ...
        if (client->connection != NULL)
        {
            ...
            if (client->name_acquired_subscription_id > 0)
                g_dbus_connection_signal_unsubscribe(
                    client->connection, client->name_acquired_subscription_id);
            if (client->name_lost_subscription_id > 0)
                g_dbus_connection_signal_unsubscribe(
                    client->connection, client->name_lost_subscription_id);
                             ...
                             CallType call_type)
{
    switch (call_type)
    {
        case CALL_TYPE_NAME_ACQUIRED:
            ...
            {
                client->name_acquired_handler(connection, client->name,
                                              client->user_data);
            }
            break;
        ...
        case CALL_TYPE_NAME_LOST:
            ...
            {
                client->name_lost_handler(connection, client->name,
                                          client->user_data);
            }
            break;
...
static void on_name_lost_or_acquired(GDBusConnection* connection,
...
static void request_name_cb(GObject* source_object, GAsyncResult* res,
                            gpointer user_data)
{
    Client* client = user_data;
    GVariant* result;
    ...
    if (subscribe)
    {
        ...
        /* if cancelled, there is no point in subscribing to signals - if not,
         * make sure we use a known good Connection object since it may be set
         * to NULL at any point after being cancelled
         */
        G_LOCK(lock);
        if (!client->cancelled)
        ...
        if (connection != NULL)
        {
            client->name_lost_subscription_id =
                g_dbus_connection_signal_subscribe(
                    ...
                    G_DBUS_SIGNAL_FLAGS_NONE, on_name_lost_or_acquired, client,
                    NULL);
...
static void has_connection(Client* client)
{
    ...
    client->disconnected_signal_handler_id =
        g_signal_connect(client->connection, "closed",
                         G_CALLBACK(on_connection_disconnected), client);
    ...
    g_dbus_connection_call(
        ...
        g_variant_new("(su)", client->name, client->flags),
        G_VARIANT_TYPE("(u)"), G_DBUS_CALL_FLAGS_NONE, -1, NULL,
        (GAsyncReadyCallback)request_name_cb, client_ref(client));
}
                              ...
                              gpointer user_data)
{
    ...
    {
        client->bus_acquired_handler(client->connection, client->name,
                                     client->user_data);
    }
 ...
 * @name_acquired_handler: (nullable): handler to invoke when @name is acquired
 * or %NULL
 ...
 * @name: the well-known name to own
 * @flags: a set of flags from the #GBusNameOwnerFlags enumeration
 * @bus_acquired_handler: (nullable): handler to invoke when connected to the
 * bus of type @bus_type or %NULL
 * @name_acquired_handler: (nullable): handler to invoke when @name is acquired
 * or %NULL
 * @name_lost_handler: (nullable): handler to invoke when @name is lost or %NULL
 * @user_data: user data to pass to handlers
                                      ...
                                      GClosure* name_lost_closure)
{
    ...
    if (bus_acquired_closure != NULL)
    {
        ...
        if (G_CLOSURE_NEEDS_MARSHAL(bus_acquired_closure))
            g_closure_set_marshal(bus_acquired_closure,
    ...
    if (name_acquired_closure != NULL)
    {
        ...
        if (G_CLOSURE_NEEDS_MARSHAL(name_acquired_closure))
            g_closure_set_marshal(name_acquired_closure,
                                  g_cclosure_marshal_generic);
    }
    ...
    if (name_lost_closure != NULL)
    {
        ...
        if (G_CLOSURE_NEEDS_MARSHAL(name_lost_closure))
            g_closure_set_marshal(name_lost_closure,
                                  g_cclosure_marshal_generic);
    }
...
static void own_with_closures_on_bus_acquired(GDBusConnection* connection,
                                              const gchar* name,
                                              gpointer user_data)
{
...
static void own_with_closures_on_name_acquired(GDBusConnection* connection,
                                               const gchar* name,
                                               gpointer user_data)
{
    OwnNameData* data = user_data;
...
static void own_with_closures_on_name_lost(GDBusConnection* connection,
                                           const gchar* name,
                                           gpointer user_data)
{
    OwnNameData* data = user_data;
 ...
 * Since: 2.26
 */
guint g_bus_own_name_with_closures(GBusType bus_type, const gchar* name,
                                   GBusNameOwnerFlags flags,
                                   GClosure* bus_acquired_closure,
                                   GClosure* name_acquired_closure,
                                   GClosure* name_lost_closure)
{
    return g_bus_own_name(
        bus_type, name, flags,
        bus_acquired_closure != NULL ? own_with_closures_on_bus_acquired : NULL,
        name_acquired_closure != NULL ? own_with_closures_on_name_acquired
                                      : NULL,
        name_lost_closure != NULL ? own_with_closures_on_name_lost : NULL,
        own_name_data_new(bus_acquired_closure, name_acquired_closure,
                          name_lost_closure),
        bus_own_name_free_func);
}

/**
 * g_bus_own_name_on_connection_with_closures: (rename-to
 * g_bus_own_name_on_connection)
 * @connection: a #GDBusConnection
 * @name: the well-known name to own
 * @flags: a set of flags from the #GBusNameOwnerFlags enumeration
...
{
    return g_bus_own_name_on_connection(
        connection, name, flags,
        name_acquired_closure != NULL ? own_with_closures_on_name_acquired
                                      : NULL,
        name_lost_closure != NULL ? own_with_closures_on_name_lost : NULL,
        own_name_data_new(NULL, name_acquired_closure, name_lost_closure),
        bus_own_name_free_func);
}
...
void g_bus_unown_name(guint owner_id)
{
    ...
    if (owner_id == 0 || map_id_to_client == NULL ||
        (client = g_hash_table_lookup(map_id_to_client,
                                      GUINT_TO_POINTER(owner_id))) == NULL)
    {
    ...
    g_warn_if_fail(
        g_hash_table_remove(map_id_to_client, GUINT_TO_POINTER(owner_id)));
...
out:
    ...
    if (client != NULL)
    {
            ...
            !g_dbus_connection_is_closed(client->connection))
        {
            ...
            /* TODO: it kinda sucks having to do a sync call to release the name
             * - but if we don't, then a subsequent grab of the name will make
             * the bus daemon return IN_QUEUE which will trigger name_lost().
             *
            ...
            result = g_dbus_connection_call_sync(
                ...
                g_variant_new("(s)", client->name), G_VARIANT_TYPE("(u)"),
                G_DBUS_CALL_FLAGS_NONE, -1, NULL, &error);
            if (result == NULL)
            {
                g_warning("Error releasing name %s: %s", client->name,
                          error->message);
                g_error_free(error);
            }
            else
            {
                ...
                if (release_name_reply !=
                    1 /* DBUS_RELEASE_NAME_REPLY_RELEASED */)
                {
                    g_warning("Unexpected reply %d when releasing name %s",
                              release_name_reply, client->name);
                }
                g_variant_unref(result);
            }
        }
