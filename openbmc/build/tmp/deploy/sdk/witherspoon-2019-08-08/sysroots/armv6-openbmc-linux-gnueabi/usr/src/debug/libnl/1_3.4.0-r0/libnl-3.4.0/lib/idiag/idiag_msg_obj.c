...
struct idiagnl_vegasinfo*
    idiagnl_msg_get_vegasinfo(const struct idiagnl_msg* msg)
{
...
static void idiag_msg_dump_details(struct nl_object* a,
                                   struct nl_dump_params* p)
{
    ...
    if (msg->idiag_shutdown)
    {
        nl_dump(p, "socket shutdown: %s\n",
                idiagnl_shutdown2str(msg->idiag_shutdown, buf, sizeof(buf)));
    }
...
static void idiag_msg_dump_stats(struct nl_object* obj,
                                 struct nl_dump_params* p)
{
    ...
    nl_dump(p, "\tsocket state: %s\n",
            idiagnl_state2str(msg->idiag_tcpinfo.tcpi_state, buf, sizeof(buf)));
    nl_dump(p, "\ttcp state: %s\n",
            idiagnl_tcpstate2str(msg->idiag_tcpinfo.tcpi_ca_state, buf,
                                 sizeof(buf)));
    ...
    nl_dump(
        p, "\toptions: %s\n",
        idiagnl_tcpopts2str(msg->idiag_tcpinfo.tcpi_options, buf, sizeof(buf)));
    ...
    nl_dump(p, "\tsnd_mss: %s\n",
            nl_size2str(msg->idiag_tcpinfo.tcpi_snd_mss, buf, sizeof(buf)));
    nl_dump(p, "\trcv_mss: %s\n",
            nl_size2str(msg->idiag_tcpinfo.tcpi_rcv_mss, buf, sizeof(buf)));
    ...
    nl_dump(
        p, "\tlast data sent: %s\n",
        nl_msec2str(msg->idiag_tcpinfo.tcpi_last_data_sent, buf, sizeof(buf)));
    nl_dump(
        p, "\tlast ack sent: %s\n",
        nl_msec2str(msg->idiag_tcpinfo.tcpi_last_ack_sent, buf, sizeof(buf)));
    nl_dump(
        p, "\tlast data recv: %s\n",
        nl_msec2str(msg->idiag_tcpinfo.tcpi_last_data_recv, buf, sizeof(buf)));
    nl_dump(
        p, "\tlast ack recv: %s\n",
        nl_msec2str(msg->idiag_tcpinfo.tcpi_last_ack_recv, buf, sizeof(buf)));
    nl_dump(p, "\tpath mtu: %s\n",
            nl_size2str(msg->idiag_tcpinfo.tcpi_pmtu, buf, sizeof(buf)));
    ...
    nl_dump(
        p, "\tsnd ss threshold: %s\n",
        nl_size2str(msg->idiag_tcpinfo.tcpi_snd_ssthresh, buf, sizeof(buf)));
    nl_dump(p, "\tsend congestion window: %d\n",
            msg->idiag_tcpinfo.tcpi_snd_cwnd);
    nl_dump(p, "\tadvertised mss: %s\n",
            nl_size2str(msg->idiag_tcpinfo.tcpi_advmss, buf, sizeof(buf)));
    ...
    nl_dump(p, "\treceive queue space: %s\n",
            nl_size2str(msg->idiag_tcpinfo.tcpi_rcv_space, buf, sizeof(buf)));
    ...
    if (msg->idiag_meminfo)
    {
        nl_dump(p, "meminfo:  [\n");
        nl_dump(p, "\trmem: %s\n",
                nl_size2str(msg->idiag_meminfo->idiag_rmem, buf, sizeof(buf)));
        nl_dump(p, "\twmem: %s\n",
                nl_size2str(msg->idiag_meminfo->idiag_wmem, buf, sizeof(buf)));
        nl_dump(p, "\tfmem: %s\n",
                nl_size2str(msg->idiag_meminfo->idiag_fmem, buf, sizeof(buf)));
        nl_dump(p, "\ttmem: %s\n",
                nl_size2str(msg->idiag_meminfo->idiag_tmem, buf, sizeof(buf)));
        nl_dump(p, "]\n");
    }
    ...
    if (msg->idiag_vegasinfo)
    {
        ...
        {
            nl_dump(p, "\trtt cnt: %d", msg->idiag_vegasinfo->tcpv_rttcnt);
            nl_dump(p, "\trtt (propagation delay): %d",
                    msg->idiag_vegasinfo->tcpv_rtt);
            nl_dump(p, "\tmin rtt: %d", msg->idiag_vegasinfo->tcpv_minrtt);
        }
        nl_dump(p, "]\n");
    }
    ...
    if (msg->ce_mask & IDIAGNL_ATTR_MEMINFO)
    {
        ...
        nl_dump(p, "\trcv buf: %s\n",
                nl_size2str(msg->idiag_skmeminfo[SK_MEMINFO_RCVBUF], buf,
                            sizeof(buf)));
        nl_dump(p, "\twmem alloc: %d\n",
                msg->idiag_skmeminfo[SK_MEMINFO_WMEM_ALLOC]);
        nl_dump(p, "\tsnd buf: %s\n",
                nl_size2str(msg->idiag_skmeminfo[SK_MEMINFO_SNDBUF], buf,
                            sizeof(buf)));
        nl_dump(p, "\tfwd alloc: %d\n",
                msg->idiag_skmeminfo[SK_MEMINFO_FWD_ALLOC]);
        nl_dump(p, "\twmem queued: %s\n",
                nl_size2str(msg->idiag_skmeminfo[SK_MEMINFO_WMEM_QUEUED], buf,
                            sizeof(buf)));
...
static int idiagnl_msg_clone(struct nl_object* _dst, struct nl_object* _src)
{
    ...
    dst->ce_mask &= ~(IDIAGNL_ATTR_CONG | IDIAGNL_ATTR_SRC | IDIAGNL_ATTR_DST |
                      IDIAGNL_ATTR_MEMINFO | IDIAGNL_ATTR_VEGASINFO);
    ...
    {
        if (!(dst->idiag_meminfo = (struct idiagnl_meminfo*)nl_object_clone(
    ...
    {
        if (!(dst->idiag_vegasinfo = (struct idiagnl_vegasinfo*)nl_object_clone(
...
int idiagnl_msg_parse(struct nlmsghdr* nlh, struct idiagnl_msg** result)
{
    ...
    msg->ce_mask =
        (IDIAGNL_ATTR_FAMILY | IDIAGNL_ATTR_STATE | IDIAGNL_ATTR_TIMER |
         IDIAGNL_ATTR_RETRANS | IDIAGNL_ATTR_EXPIRES | IDIAGNL_ATTR_RQUEUE |
         IDIAGNL_ATTR_WQUEUE | IDIAGNL_ATTR_UID | IDIAGNL_ATTR_INODE |
         IDIAGNL_ATTR_SPORT | IDIAGNL_ATTR_DPORT | IDIAGNL_ATTR_IFINDEX);
    ...
    if (tb[INET_DIAG_MEMINFO])
    {
        ...
        raw_minfo = (struct inet_diag_meminfo*)nla_data(tb[INET_DIAG_MEMINFO]);
    ...
    if (tb[INET_DIAG_VEGASINFO])
    {
        ...
        raw_vinfo = (struct tcpvegas_info*)nla_data(tb[INET_DIAG_VEGASINFO]);
...
#define _DIFF(ATTR, EXPR) ATTR_DIFF(attrs, IDIAGNL_ATTR_##ATTR, a, b, EXPR)
    ...
    diff |= _DIFF(MEMINFO, nl_object_diff((struct nl_object*)a->idiag_meminfo,
                                          (struct nl_object*)b->idiag_meminfo));
    diff |=
        _DIFF(VEGASINFO, nl_object_diff((struct nl_object*)a->idiag_vegasinfo,
                                        (struct nl_object*)b->idiag_vegasinfo));
    diff |= _DIFF(TCPINFO, memcmp(&a->idiag_tcpinfo, &b->idiag_tcpinfo,
                                  sizeof(a->idiag_tcpinfo)));
    diff |= _DIFF(SKMEMINFO, memcmp(a->idiag_skmeminfo, b->idiag_skmeminfo,
                           ...
                           uint32_t table_sz)
{
    ...
    *hashkey = nl_hash(&key, key_sz, 0) % table_sz;
    ...
    NL_DBG(5,
           "idiagnl %p key (fam %d src_hash %d dst_hash %d sport %d dport %d) "
           "keysz %d, hash 0x%x\n",
           msg, key.family, key.src_hash, key.dst_hash, key.sport, key.dport,
           key_sz, *hashkey);
...
struct nl_object_ops idiagnl_msg_obj_ops = {
    ...
    .oo_id_attrs = (IDIAGNL_ATTR_FAMILY | IDIAGNL_ATTR_SRC | IDIAGNL_ATTR_DST |
                    IDIAGNL_ATTR_SPORT | IDIAGNL_ATTR_DPORT),
};
/** @endcond */
