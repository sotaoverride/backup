...
DEFobjCurrIf(glbl) DEFobjCurrIf(prop) DEFobjCurrIf(net) DEFobjCurrIf(parser)
    DEFobjCurrIf(datetime) DEFobjCurrIf(statsobj) DEFobjCurrIf(ruleset)
...
typedef struct lstn_s
{
    ...
    prop_t* hostName; /* host-name override - if set, use this instead of actual
                         name */
    ...
    int ratelimitBurst;
    ratelimit_t*
        dflt_ratelimiter; /*ratelimiter to apply if none else is to be used */
    intTiny ratelimitSev; /* severity level (and below) for which rate-limiting
                             shall apply */
    ...
    sbool bUseSysTimeStamp; /* use timestamp from system (instead of from
                               message) */
    sbool bUnlink; /* unlink&re-create socket at start and end of processing */
    sbool bUseSpecialParser; /* use "canned" log socket parser instead of parser
                                chain? */
    ruleset_t* pRuleset;
} lstn_t;
static lstn_t* listeners;

static prop_t* pLocalHostIP = NULL;  /* there is only one global IP for all
                                        internally-generated messages */
static prop_t* pInputName = NULL;    /* our inputName currently is always
                                        "imuxsock", and this will hold it */
static int startIndexUxLocalSockets; /* process fd from that index on (used to
                                      * suppress local logging. rgerhards
                                      * 2005-08-01 read-only after startup
                                      */
static int nfd = 1;    /* number of active unix sockets  (socket 0 is always
               reserved for the system    socket, even if it is not enabled. */
...
static struct configSettings_s
{
    ...
    int bUseSysTimeStamp;        /* use timestamp from system (rather than from
                                    message) */
...
struct instanceConf_s
{
    ...
    uchar* pLogHostName; /* host name to use with this socket */
    sbool
        bUseFlowCtl; /* use flow control or not (if yes, only LIGHT is used! */
    sbool bIgnoreTimestamp; /* ignore timestamps present in the incoming
                               message? */
    ...
    ruleset_t* pBindRuleset; /* ruleset to bind listener to (use system default
                                if unspecified) */
...
static struct cnfparamdescr inppdescr[] = {
    {"socket", eCmdHdlrString,
     CNFPARAM_REQUIRED}, /* legacy: addunixlistensocket */
...
static struct cnfparamblk inppblk = {
    CNFPARAMBLK_VERSION, sizeof(inppdescr) / sizeof(struct cnfparamdescr),
    inppdescr};

#include "im-helper.h" /* must be included AFTER the type definitions! */

static int bLegacyCnfModGlobalsPermitted; /* are legacy module-global config
...
static rsRetVal addListner(instanceConf_t* inst)
{
    ...
    else
    {
        ...
        CHKiRet(prop.SetString(listeners[nfd].hostName, inst->pLogHostName,
                               ustrlen(inst->pLogHostName)));
        CHKiRet(prop.ConstructFinalize(listeners[nfd].hostName));
    }
    if (inst->ratelimitInterval > 0)
    {
        if ((listeners[nfd].ht =
                 create_hashtable(100, hash_from_key_fn, key_equals_fn,
    ...
    listeners[nfd].flowCtl =
        inst->bUseFlowCtl ? eFLOWCTL_LIGHT_DELAY : eFLOWCTL_NO_DELAY;
    listeners[nfd].flags = inst->bIgnoreTimestamp ? IGNDATE : NOFLAG;
    listeners[nfd].bCreatePath = inst->bCreatePath;
    listeners[nfd].sockName = ustrdup(inst->sockName);
    listeners[nfd].bUseCreds =
        (inst->bDiscardOwnMsgs || inst->bWritePid || inst->ratelimitInterval ||
...
static rsRetVal discardLogSockets(void)
{
    ...
    if (startIndexUxLocalSockets == 0)
    {
        ...
        {
            hashtable_destroy(listeners[0].ht,
                              1); /* 1 => free all values automatically */
        }
    ...
    for (i = 1; i < nfd; i++)
    {
        ...
        {
            hashtable_destroy(listeners[i].ht,
                              1); /* 1 => free all values automatically */
        }
    ...
    createLogSocket(lstn_t* pLstn)
{
    ...
    {
        makeFileParentDirs((uchar*)pLstn->sockName, ustrlen(pLstn->sockName),
                           0755, -1, -1, 0);
    }
...
static rsRetVal openLogSocket(lstn_t* pLstn)
{
    ...
    if (sd_fds > 0)
    {
        ...
        for (fd = SD_LISTEN_FDS_START; fd < SD_LISTEN_FDS_START + sd_fds; fd++)
        {
            if (sd_is_socket_unix(fd, SOCK_DGRAM, -1,
                                  (const char*)pLstn->sockName, 0) == 1)
            {
                /* ok, it matches -- just use as is */
                pLstn->fd = fd;

                LogMsg(0, NO_ERRCODE, LOG_INFO,
                       "imuxsock: Acquired UNIX socket '%s' (fd %d) from "
                       "systemd.\n",
    ...
    if (pLstn->fd == -1)
    {
        ...
        assert(pLstn->fd !=
               -1); /* else createLogSocket() should have failed! */
    }
    ...
    {
        one = 1;
        if (setsockopt(pLstn->fd, SOL_SOCKET, SO_PASSCRED, &one,
                       (socklen_t)sizeof(one)) != 0)
        {
            LogError(errno, NO_ERRCODE, "set SO_PASSCRED failed on '%s'",
                     pLstn->sockName);
            pLstn->bUseCreds = 0;
        }
        // TODO: move to its own #if
        if (setsockopt(pLstn->fd, SOL_SOCKET, SO_TIMESTAMP, &one,
                       sizeof(one)) != 0)
        {
            LogError(errno, NO_ERRCODE, "set SO_TIMESTAMP failed on '%s'",
                     pLstn->sockName);
        }
    }
 ...
 */
static rsRetVal findRatelimiter(lstn_t* pLstn, struct ucred* cred,
                                ratelimit_t** prl)
{
    ratelimit_t* rl = NULL;
    int r;
    ...
    if (rl == NULL)
    {
        ...
        snprintf(procName, sizeof(procName), "/proc/%lu/cmdline",
                 (unsigned long)cred->pid);
        ...
        ratelimitSetLinuxLike(rl, pLstn->ratelimitInterval,
                              pLstn->ratelimitBurst);
...
static void fixPID(uchar* bufTAG, int* lenTag, struct ucred* cred)
{
    ...
    lenPID =
        snprintf(bufPID, sizeof(bufPID), "[%lu]:", (unsigned long)cred->pid);
    ...
    if (i < 0)
        i = *lenTag -
            1; /* go right at end of TAG, pid was not present (-1 for ':') */
 ...
 */
static rsRetVal getTrustedProp(struct ucred* cred, const char* propName,
                               uchar* buf, size_t lenBuf, int* lenProp)
{
    int fd;
    int i;
    int lenRead;
    ...
    if (snprintf(namebuf, sizeof(namebuf), "/proc/%lu/%s",
                 (long unsigned)cred->pid, propName) >= (int)sizeof(namebuf))
    {
 ...
 */
static rsRetVal getTrustedExe(struct ucred* cred, uchar* buf, size_t lenBuf,
                              int* lenProp)
{
    int lenRead;
    char namebuf[1024];
    ...
    if (snprintf(namebuf, sizeof(namebuf), "/proc/%lu/exe",
                 (long unsigned)cred->pid) >= (int)sizeof(namebuf))
    {
        ABORT_FINALIZE(RS_RET_ERR);
    }
 ...
 */
static rsRetVal SubmitMsg(uchar* pRcv, int lenRcv, lstn_t* pLstn,
                          struct ucred* cred, struct timeval* ts)
{
    smsg_t* pMsg = NULL;
    int lenMsg;
    int offs;
    ...
    findRatelimiter(pLstn, cred,
                    &ratelimiter); /* ignore error, better so than others... */
...
#define CHKjson(operation, toBeFreed)                                          \
    ...
    {                                                                          \
        ...
        ABORT_FINALIZE(RS_RET_OUT_OF_MEMORY);                                  \
            ...
            if (getTrustedProp(cred, "comm", propBuf, sizeof(propBuf),
                               &lenProp) == RS_RET_OK)
            {
                CHKjson(jval = json_object_new_string((char*)propBuf), json);
                json_object_object_add(json, "appname", jval);
            }
            if (getTrustedExe(cred, propBuf, sizeof(propBuf), &lenProp) ==
                RS_RET_OK)
            {
                CHKjson(jval = json_object_new_string((char*)propBuf), json);
                json_object_object_add(json, "exe", jval);
            }
            if (getTrustedProp(cred, "cmdline", propBuf, sizeof(propBuf),
                               &lenProp) == RS_RET_OK)
            {
                CHKjson(jval = json_object_new_string((char*)propBuf), json);
...
#undef CHKjson
        ...
        else
        {
            ...
            lenProp =
                snprintf((char*)propBuf, sizeof(propBuf),
                         "_PID=%lu _UID=%lu _GID=%lu", (long unsigned)cred->pid,
    ...
    {
        /* this is the legacy "log socket" parser which was written on the
         * assumption that the log socket format would be fixed. While many
         * folks said so, it seems to be different in practice, and this is why
         * we now have choices... rgerhards, 2015-03-03
         */
        parser.SanitizeMsg(pMsg);
        lenMsg = pMsg->iLenRawMsg -
                 offs; /* SanitizeMsg() may have changed the size */
        msgSetPRI(pMsg, pri);
        ...
        if (ts == NULL)
        {
            if ((pLstn->flags & IGNDATE))
            {
                 ...
                 * accordingly.
                 */
                if (datetime.ParseTIMESTAMP3339(&dummyTS, &parse, &lenMsg) !=
                    RS_RET_OK)
                {
                    datetime.ParseTIMESTAMP3164(&dummyTS, &parse, &lenMsg,
                                                NO_PARSE3164_TZSTRING,
                                                NO_PERMIT_YEAR_AFTER_TIME);
                }
            }
            else
            {
                if (datetime.ParseTIMESTAMP3339(&(pMsg->tTIMESTAMP), &parse,
                                                &lenMsg) != RS_RET_OK &&
                    datetime.ParseTIMESTAMP3164(&(pMsg->tTIMESTAMP), &parse,
                                                &lenMsg, NO_PARSE3164_TZSTRING,
                                                NO_PERMIT_YEAR_AFTER_TIME) !=
                        RS_RET_OK)
                {
        ...
        { /* if we pulled the time from the system, we need to update the
             message text */
            uchar* tmpParse = parse; /* just to check correctness of TS */
            if (datetime.ParseTIMESTAMP3339(&dummyTS, &tmpParse, &lenMsg) ==
                    RS_RET_OK ||
                datetime.ParseTIMESTAMP3164(
                    &dummyTS, &tmpParse, &lenMsg, NO_PARSE3164_TZSTRING,
                    NO_PERMIT_YEAR_AFTER_TIME) == RS_RET_OK)
            {
                ...
                parse[15] =
                    ' '; /* re-write \0 from fromatTimestamp3164 by SP */
    ...
    { /* we are configured to use regular parser chain */
        pMsg->msgFlags |= NEEDS_PARSING;
    }

    MsgSetRcvFrom(pMsg, pLstn->hostName == NULL ? glbl.GetLocalHostNameProp()
                                                : pLstn->hostName);
...
static rsRetVal readSocket(lstn_t* pLstn)
{
...
#ifdef HAVE_SCM_CREDENTIALS
    /* aux is a union rather than a direct char array to force alignment with
     * cmsghdr */
    union
    {
        char buf[128];
    ...
    /* we optimize performance: if iMaxLine is below 4K (which it is in almost
     * all cases, we use a fixed buffer on the stack. Only if it is higher, heap
     * memory is used. We could use alloca() to achive a similar aspect, but
     * there are so many issues with alloca() that I do not want to take that
     * route. rgerhards, 2008-09-02
     */
...
#define MSG_DONTWAIT MSG_NONBLOCK
    ...
    if (iRcvd > 0)
    {
        ...
        if (pLstn->bUseCreds)
        {
            ...
            {
#ifdef HAVE_SCM_CREDENTIALS
                if (pLstn->bUseCreds && cm->cmsg_level == SOL_SOCKET &&
                ...
                if (pLstn->bUseSysTimeStamp && cm->cmsg_level == SOL_SOCKET &&
        ...
        CHKiRet(SubmitMsg(pRcv, iRcvd, pLstn, (cred_set ? &cred : NULL),
                          (ts_set ? &ts : NULL)));
    }
...
static rsRetVal activateListeners(void)
{
    ...
    if (startIndexUxLocalSockets == 0)
    {
        ...
        {
            if ((listeners[0].ht = create_hashtable(
                     100, hash_from_key_fn, key_equals_fn, NULL)) == NULL)
            {
        ...
        listeners[0].bUseCreds =
            (runModConf->bWritePidSysSock ||
             runModConf->ratelimitIntervalSysSock ||
        ...
        listeners[0].flowCtl =
            runModConf->bUseFlowCtl ? eFLOWCTL_LIGHT_DELAY : eFLOWCTL_NO_DELAY;
        ...
        ratelimitSetSeverity(listeners[0].dflt_ratelimiter,
                             listeners[0].ratelimitSev);
    }
    ...
    if (sd_fds < 0)
    {
        LogError(-sd_fds, NO_ERRCODE,
                 "imuxsock: Failed to acquire systemd socket");
        ABORT_FINALIZE(RS_RET_ERR_CRE_AFUX);
    }
#endif
...
ENDsetModCnf
    ...
    BEGINnewInpInst struct cnfparamvals* pvals;
instanceConf_t* inst;
int i;
CODESTARTnewInpInst DBGPRINTF("newInpInst (imuxsock)\n");
...
ENDnewInpInst
    ...
    BEGINendCnfLoad CODESTARTendCnfLoad if (!loadModConf->configSetViaV2Method)
{
...
ENDendCnfLoad
    ...
    /* function to generate error message if framework does not find requested
       ruleset */
    static void
...
BEGINcheckCnf instanceConf_t* inst;
...
ENDcheckCnf
    ...
    BEGINactivateCnfPrePrivDrop instanceConf_t* inst;
int nLstn;
int i;
CODESTARTactivateCnfPrePrivDrop runModConf = pModConf;
#ifdef OS_SOLARIS
...
finalize_it : ENDactivateCnfPrePrivDrop

                  BEGINactivateCnf CODESTARTactivateCnf ENDactivateCnf

                      BEGINfreeCnf instanceConf_t* inst,
              *del;
CODESTARTfreeCnf free(pModConf->pLogSockName);
...
ENDfreeCnf
    ...
    BEGINrunInput int nfds;
int i;
CODESTARTrunInput struct pollfd* const pollfds =
    calloc(nfd, sizeof(struct pollfd));
CHKmalloc(pollfds);
...
finalize_it : free(pollfds);
ENDrunInput

    BEGINwillRun CODESTARTwillRun ENDwillRun
...
ENDafterRun
    ...
    BEGINmodExit CODESTARTmodExit free(listeners);
if (pInputName != NULL)
...
ENDmodExit
    ...
    BEGINisCompatibleWithFeature
        CODESTARTisCompatibleWithFeature if (eFeat ==
                                             sFEATURENonCancelInputTermination)
            iRet = RS_RET_OK;
ENDisCompatibleWithFeature

    BEGINqueryEtryPt CODESTARTqueryEtryPt CODEqueryEtryPt_STD_IMOD_QUERIES
...
CHKiRet(prop.SetString(pInputName, UCHAR_CONSTANT("imuxsock"),
                       sizeof("imuxsock") - 1));
CHKiRet(prop.ConstructFinalize(pInputName));

/* right now, glbl does not permit per-instance IP address notation. As long as
 * this is the case, it is OK to query the HostIP once here at this location.
 * HOWEVER, the whole concept is not 100% clean and needs to be addressed on a
 * higher layer.
...
CHKiRet(omsdRegCFSLineHdlr((uchar*)"inputunixlistensocketignoremsgtimestamp", 0,
                           eCmdHdlrBinary, NULL, &cs.bIgnoreTimestamp,
                           STD_LOADABLE_MODULE_ID));
CHKiRet(omsdRegCFSLineHdlr((uchar*)"inputunixlistensockethostname", 0,
                           eCmdHdlrGetWord, NULL, &cs.pLogHostName,
                           STD_LOADABLE_MODULE_ID));
CHKiRet(omsdRegCFSLineHdlr((uchar*)"inputunixlistensocketflowcontrol", 0,
                           eCmdHdlrBinary, NULL, &cs.bUseFlowCtl,
                           STD_LOADABLE_MODULE_ID));
CHKiRet(omsdRegCFSLineHdlr((uchar*)"inputunixlistensocketannotate", 0,
                           eCmdHdlrBinary, NULL, &cs.bAnnotate,
                           STD_LOADABLE_MODULE_ID));
CHKiRet(omsdRegCFSLineHdlr((uchar*)"inputunixlistensocketcreatepath", 0,
                           eCmdHdlrBinary, NULL, &cs.bCreatePath,
                           STD_LOADABLE_MODULE_ID));
CHKiRet(omsdRegCFSLineHdlr((uchar*)"inputunixlistensocketusesystimestamp", 0,
                           eCmdHdlrBinary, NULL, &cs.bUseSysTimeStamp,
                           STD_LOADABLE_MODULE_ID));
CHKiRet(omsdRegCFSLineHdlr((uchar*)"addunixlistensocket", 0, eCmdHdlrGetWord,
                           addInstance, NULL, STD_LOADABLE_MODULE_ID));
CHKiRet(omsdRegCFSLineHdlr((uchar*)"inputunixlistensocketusepidfromsystem", 0,
                           eCmdHdlrBinary, NULL, &cs.bWritePid,
                           STD_LOADABLE_MODULE_ID));
...
CHKiRet(omsdRegCFSLineHdlr((uchar*)"imuxsockratelimitseverity", 0, eCmdHdlrInt,
                           NULL, &cs.ratelimitSeverity,
                           STD_LOADABLE_MODULE_ID));
CHKiRet(omsdRegCFSLineHdlr((uchar*)"resetconfigvariables", 1,
                           eCmdHdlrCustomHandler, resetConfigVariables, NULL,
                           STD_LOADABLE_MODULE_ID));
/* the following one is a (dirty) trick: the system log socket is not added via
 * an "addUnixListenSocket" config format. As such, it's properties can not be
 * modified via $InputUnixListenSocket*". So we need to add a special directive
 * for that. We should revisit all of that once we have the new config format...
 * rgerhards, 2008-03-06
 */
CHKiRet(regCfSysLineHdlr2((uchar*)"omitlocallogging", 0, eCmdHdlrBinary, NULL,
                          &cs.bOmitLocalLogging, STD_LOADABLE_MODULE_ID,
...
CHKiRet(regCfSysLineHdlr2((uchar*)"systemlogsocketname", 0, eCmdHdlrGetWord,
                          ...
                          &bLegacyCnfModGlobalsPermitted));
CHKiRet(regCfSysLineHdlr2((uchar*)"systemlogsocketignoremsgtimestamp", 0,
                          eCmdHdlrBinary, NULL, &cs.bIgnoreTimestampSysSock,
                          STD_LOADABLE_MODULE_ID,
                          &bLegacyCnfModGlobalsPermitted));
CHKiRet(regCfSysLineHdlr2((uchar*)"systemlogsocketflowcontrol", 0,
                          eCmdHdlrBinary, NULL, &cs.bUseFlowCtlSysSock,
                          STD_LOADABLE_MODULE_ID,
                          &bLegacyCnfModGlobalsPermitted));
CHKiRet(regCfSysLineHdlr2((uchar*)"systemlogusesystimestamp", 0, eCmdHdlrBinary,
                          NULL, &cs.bUseSysTimeStampSysSock,
                          STD_LOADABLE_MODULE_ID,
                          &bLegacyCnfModGlobalsPermitted));
...
CHKiRet(regCfSysLineHdlr2((uchar*)"systemlogparsetrusted", 0, eCmdHdlrBinary,
                          NULL, &cs.bParseTrusted, STD_LOADABLE_MODULE_ID,
                          &bLegacyCnfModGlobalsPermitted));
CHKiRet(regCfSysLineHdlr2((uchar*)"systemlogusepidfromsystem", 0,
                          eCmdHdlrBinary, NULL, &cs.bWritePidSysSock,
                          STD_LOADABLE_MODULE_ID,
                          &bLegacyCnfModGlobalsPermitted));
CHKiRet(regCfSysLineHdlr2((uchar*)"systemlogratelimitinterval", 0, eCmdHdlrInt,
                          NULL, &cs.ratelimitIntervalSysSock,
                          STD_LOADABLE_MODULE_ID,
                          &bLegacyCnfModGlobalsPermitted));
CHKiRet(regCfSysLineHdlr2((uchar*)"systemlogratelimitburst", 0, eCmdHdlrInt,
                          NULL, &cs.ratelimitBurstSysSock,
                          STD_LOADABLE_MODULE_ID,
                          &bLegacyCnfModGlobalsPermitted));
CHKiRet(regCfSysLineHdlr2((uchar*)"systemlogratelimitseverity", 0, eCmdHdlrInt,
                          NULL, &cs.ratelimitSeveritySysSock,
                          STD_LOADABLE_MODULE_ID,
                          &bLegacyCnfModGlobalsPermitted));
...
CHKiRet(statsobj.AddCounter(modStats,
                            UCHAR_CONSTANT("ratelimit.numratelimiters"),
                            ctrType_IntCtr, CTR_FLAG_RESETTABLE,
                            &ctrNumRatelimiters));
