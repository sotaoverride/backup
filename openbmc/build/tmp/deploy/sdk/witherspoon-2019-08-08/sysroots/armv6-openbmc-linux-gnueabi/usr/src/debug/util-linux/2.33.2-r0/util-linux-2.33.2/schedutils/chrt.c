...
static void __attribute__((__noreturn__)) usage(void)
{
    ...
    fputs(_(" -R, --reset-on-fork       set SCHED_RESET_ON_FORK for FIFO or "
            "RR\n"),
          out);
    ...
    fputs(_(" -a, --all-tasks      operate on all the tasks (threads) for a "
            "given pid\n"),
          out);
...
fallback:
...
#endif
    {
        ...
        if (policy == (SCHED_FIFO | SCHED_RESET_ON_FORK) ||
            policy == (SCHED_BATCH | SCHED_RESET_ON_FORK))
            reset_on_fork = 1;
#endif
    }

    if (ctl->altered)
        printf(_("pid %d's new scheduling policy: %s"), pid,
               get_policy_name(policy));
    else
        printf(_("pid %d's current scheduling policy: %s"), pid,
               get_policy_name(policy));
    ...
    {
        if (ctl->altered)
            printf(_("pid %d's new runtime/deadline/period parameters: "
                     "%ju/%ju/%ju\n"),
                   pid, runtime, deadline, period);
        else
            printf(_("pid %d's current runtime/deadline/period parameters: "
                     "%ju/%ju/%ju\n"),
                   pid, runtime, deadline, period);
    }
#endif
...
int main(int argc, char** argv)
{
    ...
    while ((c = getopt_long(argc, argv, "+abdD:fiphmoP:T:rRvV", longopts,
                            NULL)) != -1)
    {
        switch (c)
        {
            ...
            case 'p':
                errno = 0;
                ctl->pid =
                    strtos32_or_err(argv[argc - 1], _("invalid PID argument"));
                break;
            case 'r':
                ctl->policy = SCHED_RR;
                break;
            ...
            case 'T':
                ctl->runtime =
                    strtou64_or_err(optarg, _("invalid runtime argument"));
                break;
            case 'P':
                ctl->period =
                    strtou64_or_err(optarg, _("invalid period argument"));
                break;
            case 'D':
                ctl->deadline =
                    strtou64_or_err(optarg, _("invalid deadline argument"));
                break;
            case 'V':
    ...
    errno = 0;
    ctl->priority =
        strtos32_or_err(argv[optind], _("invalid priority argument"));

#ifdef SCHED_RESET_ON_FORK
    if (ctl->reset_on_fork && ctl->policy != SCHED_FIFO &&
        ctl->policy != SCHED_RR)
    ...
    if (ctl->priority < sched_get_priority_min(ctl->policy) ||
        ...
        errx(EXIT_FAILURE,
             _("unsupported priority value for the policy: %d: see --max for "
               "valid range"),
             ctl->priority);
    set_sched(ctl);
