...
static int address_pool_new(Manager* m, AddressPool** ret, int family,
...
int address_pool_new_from_string(Manager* m, AddressPool** ret, int family,
                                 const char* p, unsigned prefixlen)
{
...
static bool address_pool_prefix_is_taken(AddressPool* p,
                                         ...
                                         unsigned prefixlen)
{
    ...
    HASHMAP_FOREACH(l, p->manager->links, i)
    {
        ...
        SET_FOREACH(a, l->addresses, j)
        {
            ...
            if (in_addr_prefix_intersect(p->family, u, prefixlen, &a->in_addr,
                                         a->prefixlen))
                return true;
        }

        /* Don't clash with addresses already pulled from the pool, but not
         * assigned yet */
...
int address_pool_acquire(AddressPool* p, unsigned prefixlen,
