...
static void usage(char* program, int error)
{
...
#if HAVE_GETOPT_LONG
    fprintf(file,
            _("usage: %s [-EfrsvVh] [-y SYSROOT] [--error-on-no-fonts] "
              "[--force|--really-force] [--sysroot=SYSROOT] [--system-only] "
    ...
    fprintf(file, "\n");
#if HAVE_GETOPT_LONG
    fprintf(file, _("  -E, --error-on-no-fonts  raise an error if no fonts in "
                    "a directory\n"));
    fprintf(file, _("  -f, --force              scan directories with "
                    "apparently valid caches\n"));
    fprintf(file, _("  -r, --really-force       erase all existing caches, "
                    "then rescan\n"));
    fprintf(
        file,
        _("  -s, --system-only        scan system-wide directories only\n"));
    fprintf(file, _("  -y, --sysroot=SYSROOT    prepend SYSROOT to all paths "
                    "for scanning\n"));
    fprintf(file, _("  -v, --verbose            display status information "
                    "while busy\n"));
    fprintf(
        file,
        _("  -V, --version            display font config version and exit\n"));
    fprintf(file, _("  -h, --help               display this help and exit\n"));
#else
    fprintf(file, _("  -E         (error-on-no-fonts)\n"));
    fprintf(file, _("                       raise an error if no fonts in a "
                    "directory\n"));
    fprintf(file, _("  -f         (force)   scan directories with apparently "
                    "valid caches\n"));
    fprintf(
        file,
        _("  -r,   (really force) erase all existing caches, then rescan\n"));
    fprintf(file,
            _("  -s         (system)  scan system-wide directories only\n"));
    fprintf(file, _("  -y SYSROOT (sysroot) prepend SYSROOT to all paths for "
                    "scanning\n"));
    fprintf(
        file,
        _("  -v         (verbose) display status information while busy\n"));
    fprintf(file,
            _("  -V         (version) display font config version and exit\n"));
    fprintf(file, _("  -h         (help)    display this help and exit\n"));
#endif
...
static int scanDirs(FcStrList* list, FcConfig* config, FcBool force,
                    FcBool really_force, FcBool verbose,
                    FcBool error_on_no_fonts, int* changed)
{
    int ret = 0;
    const FcChar8* dir;
    FcStrSet* subdirs;
    ...
    while ((dir = FcStrListNext(list)))
    {
        ...
        if (was_valid)
        {
            if (verbose)
                printf(
                    _("skipping, existing cache is valid: %d fonts, %d dirs\n"),
