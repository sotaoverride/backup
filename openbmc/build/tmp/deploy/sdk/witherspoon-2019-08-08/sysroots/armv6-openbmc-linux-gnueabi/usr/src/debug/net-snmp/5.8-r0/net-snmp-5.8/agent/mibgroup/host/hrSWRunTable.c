...
/** Initialize the hrSWRunTable table by defining its contents and how it's
 * structured */
...
#define SWRUN_ACCESS_LEVEL HANDLER_CAN_RONLY
    ...
    reg = netsnmp_create_handler_registration(
        MYTABLE, hrSWRunTable_handler, hrSWRunTable_oid, hrSWRunTable_oid_len,
        SWRUN_ACCESS_LEVEL);
    if (NULL == reg)
    {
        snmp_log(LOG_ERR,
...
int hrSWRunTable_handler(netsnmp_mib_handler* handler,
                ...
                switch (table_info->colnum)
                {
                    case COLUMN_HRSWRUNSTATUS:
                        if (*request->requestvb->val.integer !=
                            HRSWRUNSTATUS_INVALID)
                        {
                            netsnmp_set_request_error(reqinfo, request,
                                                      SNMP_ERR_WRONGVALUE);
                            return SNMP_ERR_NOERROR;
                        }
                        pid = request->requestvb
                                  ->name[request->requestvb->name_length - 1];
                        if (1 == pid)
                        {
