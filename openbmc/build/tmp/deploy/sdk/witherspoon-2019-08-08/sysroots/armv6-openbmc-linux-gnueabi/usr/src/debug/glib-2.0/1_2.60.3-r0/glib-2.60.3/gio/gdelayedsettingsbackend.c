...
G_DEFINE_TYPE_WITH_PRIVATE(GDelayedSettingsBackend, g_delayed_settings_backend,
...
static void g_delayed_settings_backend_notify_unapplied(
...
static GVariant*
    g_delayed_settings_backend_read(GSettingsBackend* backend, const gchar* key,
...
static GVariant* g_delayed_settings_backend_read_user_value(
    ...
    const GVariantType* expected_type)
{
    ...
    g_mutex_lock(&delayed->priv->lock);
    value_found =
        g_tree_lookup_extended(delayed->priv->delayed, key, NULL, &result);
    if (result)
        g_variant_ref(result);
    g_mutex_unlock(&delayed->priv->lock);
    ...
    return g_settings_backend_read_user_value(delayed->priv->backend, key,
                                              expected_type);
}

static gboolean g_delayed_settings_backend_write(GSettingsBackend* backend,
                                                 const gchar* key,
                                                 GVariant* value,
                                                 gpointer origin_tag)
{
...
static gboolean g_delayed_settings_backend_write_tree(GSettingsBackend* backend,
                                                      GTree* tree,
                                                      gpointer origin_tag)
{
    GDelayedSettingsBackend* delayed = G_DELAYED_SETTINGS_BACKEND(backend);
...
static void g_delayed_settings_backend_reset(GSettingsBackend* backend,
                                             const gchar* key,
                                             gpointer origin_tag)
{
    GDelayedSettingsBackend* delayed = G_DELAYED_SETTINGS_BACKEND(backend);
...
static void g_delayed_settings_backend_subscribe(GSettingsBackend* backend,
                                                 const char* name)
{
    GDelayedSettingsBackend* delayed = G_DELAYED_SETTINGS_BACKEND(backend);
...
static void g_delayed_settings_backend_unsubscribe(GSettingsBackend* backend,
                                                   const char* name)
{
    GDelayedSettingsBackend* delayed = G_DELAYED_SETTINGS_BACKEND(backend);
...
/* method calls */
gboolean g_delayed_settings_backend_get_has_unapplied(
    GDelayedSettingsBackend* delayed)
{
...
void g_delayed_settings_backend_apply(GDelayedSettingsBackend* delayed)
{
    if (g_tree_nnodes(delayed->priv->delayed) > 0)
    {
        ...
        success = g_settings_backend_write_tree(delayed->priv->backend, tmp,
                                                delayed->priv);
        g_mutex_unlock(&delayed->priv->lock);

        if (!success)
            g_settings_backend_changed_tree(G_SETTINGS_BACKEND(delayed), tmp,
                                            NULL);
...
/* change notification */
static void delayed_backend_changed(GObject* target, GSettingsBackend* backend,
                                    const gchar* key, gpointer origin_tag)
{
    ...
    if (origin_tag != delayed->priv)
        g_settings_backend_changed(G_SETTINGS_BACKEND(delayed), key,
                                   origin_tag);
}

static void delayed_backend_keys_changed(GObject* target,
                                         GSettingsBackend* backend,
                                         ...
                                         const gchar* const* items)
{
    ...
    if (origin_tag != delayed->priv)
        g_settings_backend_keys_changed(G_SETTINGS_BACKEND(delayed), path,
                                        items, origin_tag);
}

static void delayed_backend_path_changed(GObject* target,
                                         GSettingsBackend* backend,
                                         const gchar* path, gpointer origin_tag)
{
    GDelayedSettingsBackend* delayed = G_DELAYED_SETTINGS_BACKEND(target);
    ...
    if (origin_tag != delayed->priv)
        g_settings_backend_path_changed(G_SETTINGS_BACKEND(delayed), path,
                                        origin_tag);
}

static void delayed_backend_writable_changed(GObject* target,
                                             GSettingsBackend* backend,
                                             const gchar* key)
{
    GDelayedSettingsBackend* delayed = G_DELAYED_SETTINGS_BACKEND(target);
...
static void delayed_backend_path_writable_changed(GObject* target,
                                                  GSettingsBackend* backend,
                                                  const gchar* path)
{
    GDelayedSettingsBackend* delayed = G_DELAYED_SETTINGS_BACKEND(target);
...
static void g_delayed_settings_backend_disown(gpointer data,
...
GDelayedSettingsBackend*
    g_delayed_settings_backend_new(GSettingsBackend* backend, gpointer owner,
                                   GMainContext* owner_context)
{
    static GSettingsListenerVTable vtable = {
        delayed_backend_changed, delayed_backend_path_changed,
        delayed_backend_keys_changed, delayed_backend_writable_changed,
    ...
    g_settings_backend_watch(delayed->priv->backend, &vtable, G_OBJECT(delayed),
                             NULL);
