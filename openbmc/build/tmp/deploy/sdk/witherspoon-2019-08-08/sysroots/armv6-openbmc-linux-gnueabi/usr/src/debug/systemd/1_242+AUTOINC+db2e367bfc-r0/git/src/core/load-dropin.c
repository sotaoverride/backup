...
static int process_deps(Unit* u, UnitDependency dependency,
                        const char* dir_suffix)
{
    ...
    r = unit_file_find_dropin_paths(NULL, u->manager->lookup_paths.search_path,
                                    u->manager->unit_path_cache, dir_suffix,
                                    NULL, u->names, &paths);
    if (r < 0)
        return r;
    ...
    STRV_FOREACH(p, paths)
    {
        ...
        {
            log_unit_warning(
                u, "%s dependency dropin %s is not a symlink, ignoring.",
                unit_dependency_to_string(dependency), *p);
            continue;
        }
        ...
        /* We don't treat this as an error, especially because we didn't check
         * this for a long time. Nevertheless, we warn, because such mismatch
         * can be mighty confusing. */
        ...
        if (r == 0)
            log_unit_warning(
                u, "%s dependency dropin %s target %s has different name",
                unit_dependency_to_string(dependency), *p, target);

        r = unit_add_dependency_by_name(u, dependency, entry, true,
                                        UNIT_DEPENDENCY_FILE);
        if (r < 0)
            log_unit_warning_errno(
                u, r, "Cannot add %s dependency on %s, ignoring: %m",
...
int unit_load_dropin(Unit* u)
{
    ...
    (void)config_parse(u->id, *f, NULL, UNIT_VTABLE(u)->sections,
                       config_item_perf_lookup, load_fragment_gperf_lookup, 0,
                       u);
