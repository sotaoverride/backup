...
int bus_dnssd_method_unregister(sd_bus_message* message, void* userdata,
                                sd_bus_error* error)
{
    ...
    r = bus_verify_polkit_async(
        message, CAP_SYS_ADMIN, "org.freedesktop.resolve1.unregister-service",
        NULL, false, s->originator, &m->polkit_registry, error);
    if (r < 0)
        return r;
    if (r == 0)
    ...
    HASHMAP_FOREACH(l, m->links, i)
    {
        if (l->mdns_ipv4_scope)
        {
            ...
            if (r < 0)
                log_warning_errno(
                    r, "Failed to send goodbye messages in IPv4 scope: %m");
        ...
        if (l->mdns_ipv6_scope)
        {
            ...
            if (r < 0)
                log_warning_errno(
                    r, "Failed to send goodbye messages in IPv6 scope: %m");
...
const sd_bus_vtable dnssd_vtable[] = {SD_BUS_VTABLE_START(0),
                                      ...
                                      SD_BUS_METHOD("Unregister", NULL, NULL,
...
int dnssd_object_find(sd_bus* bus, const char* path, const char* interface,
...
int dnssd_node_enumerator(sd_bus* bus, const char* path, void* userdata,
