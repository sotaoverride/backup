...
{
    return __type2str(state, buf, len, idiag_states, ARRAY_SIZE(idiag_states));
}
...
{
    return __type2str(timer, buf, len, idiag_timers, ARRAY_SIZE(idiag_timers));
}
...
static const struct trans_tbl idiag_attrs[] = {
    __ADD(INET_DIAG_NONE, none),         __ADD(INET_DIAG_MEMINFO, meminfo),
    __ADD(INET_DIAG_INFO, info),         __ADD(INET_DIAG_VEGASINFO, vegasinfo),
    __ADD(INET_DIAG_CONG, congestion),   __ADD(INET_DIAG_TOS, tos),
    __ADD(INET_DIAG_TCLASS, tclass),     __ADD(INET_DIAG_SKMEMINFO, skmeminfo),
 ...
 *   ( (1<<(INET_DIAG_MEMINFO-1)) | (1<<(INET_DIAG_CONG-1)) |
 * (1<<(INET_DIAG_TOS-1)) ) )
...
static const struct trans_tbl idiagnl_tcpstates[] = {
    __ADD(TCP_CA_Open, open), __ADD(TCP_CA_Disorder, disorder),
    __ADD(TCP_CA_CWR, cwr),   __ADD(TCP_CA_Recovery, recovery),
