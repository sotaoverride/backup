...
#else
int blkdev_get_physector_size(int fd __attribute__((__unused__)),
                              int* sector_size)
{
    *sector_size = DEFAULT_SECTOR_SIZE;
    return 0;
...
int open_blkdev_or_file(const struct stat* st, const char* name,
                        const int oflag)
{
    int fd;
...
#else
int blkdev_get_geometry(int fd __attribute__((__unused__)), unsigned int* h,
                        unsigned int* s)
{
    *h = 0;
    *s = 0;
