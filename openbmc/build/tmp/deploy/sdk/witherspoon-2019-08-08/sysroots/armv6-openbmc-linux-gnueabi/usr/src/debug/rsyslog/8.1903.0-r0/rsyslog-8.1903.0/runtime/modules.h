...
typedef enum eModType_
{
    ...
    eMOD_ANY = 6 /* meta-name for "any type of module" -- to be used in function
                    calls */
} eModType_t;
...
{
    struct modInfo_s*
        pPrev; /* support for creating a double linked module list */
    ...
    uchar* cnfName;           /* name to be used in config statements (e.g.
                                 'name="omusrmsg"') */
    unsigned
        uRefCnt; /* reference count for this module; 0 -> may be unloaded */
    sbool bSetModCnfCalled; /* is setModCnf already called? Needed for built-in
                               modules */
    /* functions supported by all types of modules */
    rsRetVal (*modInit)(int, int*,
                        rsRetVal (**)(void*)); /* initialize the module */
    /* be sure to support version handshake! */
    rsRetVal (*modQueryEtryPt)(
        uchar* name,
        rsRetVal (**EtryPoint)()); /* query entry point addresses */
    rsRetVal (*isCompatibleWithFeature)(syslogFeature);
    rsRetVal (*freeInstance)(
        void*); /* called before termination or module unload */
    rsRetVal (*dbgPrintInstInfo)(
        void*); /* called before termination or module unload */
    rsRetVal (*tryResume)(
        void*); /* called to see if module actin can be resumed now */
    ...
    rsRetVal (*activateCnf)(
        void* Cnf); /* make provided config the running conf */
    rsRetVal (*freeCnf)(void* Cnf);
    /* end v2 config system specific */
    union
    {
        struct
        { /* data for input modules */
            /* TODO: remove? */ rsRetVal (*willRun)(
                void); /* check if the current config will be able to run*/
            rsRetVal (*runInput)(
        ...
        struct
        { /* data for output modules */
            /* below: perform the configured action
             */
            rsRetVal (*beginTransaction)(void*);
            rsRetVal (*commitTransaction)(void* const, actWrkrIParams_t* const,
                                          const unsigned);
            rsRetVal (*doAction)(void** params, void* pWrkrData);
            rsRetVal (*endTransaction)(void*);
            rsRetVal (*parseSelectorAct)(uchar**, void**,
                                         omodStringRequest_t**);
            rsRetVal (*newActInst)(uchar* modName, struct nvlst* lst, void**,
                                   omodStringRequest_t**);
        ...
        struct
        { /* data for library modules */
            char dummy;
        } lm;
        struct
        { /* data for parser modules */
        ...
        struct
        { /* data for strgen modules */
            rsRetVal (*strgen)(const smsg_t* const,
                               actWrkrIParams_t* const iparam);
        } sm;
        struct
        { /* data for rscript modules */
            rsRetVal (*getFunctArray)(int* const, struct scriptFunct**);
        } fm;
    } mod;
    void* pModHdlr; /* handler to the dynamic library holding the module */
#ifdef DEBUG
    /* we add some home-grown support to track our users (and detect who does
     * not free us). */
...
cfgmodules_etry_t* (*GetNxtCnfType)(rsconf_t* cnf, cfgmodules_etry_t* pThis,
                                    eModType_t rqtdType);
...
rsRetVal (*Release)(const char* srcFile,
                    modInfo_t** ppThis); /**< release a module (ref counting) */
void (*PrintList)(void);
...
modInfo_t* (*FindWithCnfName)(rsconf_t* cnf, uchar* name,
                              eModType_t rqtdType); /* added v3, 2011-07-19 */
ENDinterface(module)
#define moduleCURR_IF_VERSION                                                  \
    5 /* increment whenever you change the interface structure! */
    /* Changes:
     * v2
 ...
 * ... and we need to know that none of the module functions are called from
 * plugins! rgerhards, 2012-09-24
 */
