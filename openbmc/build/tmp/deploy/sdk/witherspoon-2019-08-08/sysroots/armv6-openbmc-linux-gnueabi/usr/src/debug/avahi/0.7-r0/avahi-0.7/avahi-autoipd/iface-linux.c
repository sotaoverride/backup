...
static int process_nlmsg(struct nlmsghdr* n)
{
    ...
    if (n->nlmsg_type == RTM_NEWLINK || n->nlmsg_type == RTM_DELLINK)
    {
        ...
        if ((ifi->ifi_flags & IFF_LOOPBACK) || (ifi->ifi_flags & IFF_NOARP) ||
...
static int process_response(int wait_for_done, unsigned seq)
{
    ...
    do
    {
        ...
        if (!(cmsghdr = CMSG_FIRSTHDR(&msghdr)) ||
        ...
        for (; bytes > 0; p = NLMSG_NEXT(p, bytes))
        {
            ...
            if (!NLMSG_OK(p, bytes) || bytes < sizeof(struct nlmsghdr) ||
                bytes < p->nlmsg_len)
            {
            ...
            if (p->nlmsg_type == NLMSG_DONE && wait_for_done &&
                p->nlmsg_seq == seq && (pid_t)p->nlmsg_pid == getpid())
                return 0;
            ...
            if (p->nlmsg_type == NLMSG_ERROR)
            {
                ...
                if (e->error)
                {
                    daemon_log(LOG_ERR, "Netlink error: %s",
                               strerror(-e->error));
                    return -1;
                }
            }
