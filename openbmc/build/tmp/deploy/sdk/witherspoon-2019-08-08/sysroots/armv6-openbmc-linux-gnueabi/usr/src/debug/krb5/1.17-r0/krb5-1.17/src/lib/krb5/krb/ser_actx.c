 ...
 */
static krb5_error_code krb5_auth_context_size(krb5_context, krb5_pointer,
                                              size_t*);
static krb5_error_code krb5_auth_context_externalize(krb5_context, krb5_pointer,
                                                     krb5_octet**, size_t*);
static krb5_error_code krb5_auth_context_internalize(krb5_context,
                                                     krb5_pointer*,
                                                     krb5_octet**, size_t*);
 ...
 */
static krb5_error_code krb5_auth_context_size(krb5_context kcontext,
                                              krb5_pointer arg, size_t* sizep)
{
    ...
    if ((auth_context = (krb5_auth_context)arg))
    {
        ...
        {
            kret = krb5_size_opaque(kcontext, KV5M_ADDRESS,
        ...
        if (!kret && auth_context->key)
        {
            kret = krb5_size_opaque(kcontext, KV5M_KEYBLOCK,
                                    (krb5_pointer)&auth_context->key->keyblock,
                                    &required);
            if (!kret)
                required += sizeof(krb5_int32);
        }
        ...
        if (!kret && auth_context->send_subkey)
        {
            kret = krb5_size_opaque(
                ...
                (krb5_pointer)&auth_context->send_subkey->keyblock, &required);
            if (!kret)
                required += sizeof(krb5_int32);
        }
        ...
        if (!kret && auth_context->recv_subkey)
        {
            kret = krb5_size_opaque(
                kcontext, KV5M_KEYBLOCK,
                (krb5_pointer)&auth_context->recv_subkey->keyblock, &required);
            if (!kret)
                required += sizeof(krb5_int32);
        }
        ...
        if (!kret && auth_context->authentp)
            kret = krb5_size_opaque(kcontext, KV5M_AUTHENTICATOR,
 ...
 */
static krb5_error_code krb5_auth_context_externalize(krb5_context kcontext,
                                                     krb5_pointer arg,
                                                     krb5_octet** buffer,
                                                     size_t* lenremain)
{
    ...
    if ((auth_context = (krb5_auth_context)arg))
    {
            ...
            (required <= remain))
        {
            ...
            (void)krb5_ser_pack_int32(auth_context->auth_context_flags, &bp,
                                      &remain);
            (void)krb5_ser_pack_int32(auth_context->remote_seq_number, &bp,
                                      &remain);
            (void)krb5_ser_pack_int32(auth_context->local_seq_number, &bp,
                                      &remain);
            ...
            if (!kret && auth_context->remote_addr)
            {
                ...
                kret = krb5_externalize_opaque(
                    kcontext, KV5M_ADDRESS,
                    (krb5_pointer)auth_context->remote_addr, &bp, &remain);
            }
            ...
            if (!kret && auth_context->local_addr)
            {
                ...
                kret = krb5_externalize_opaque(
                    kcontext, KV5M_ADDRESS,
                    (krb5_pointer)auth_context->local_addr, &bp, &remain);
            }
            ...
            if (!kret && auth_context->key)
            {
                ...
                kret = krb5_externalize_opaque(
                    kcontext, KV5M_KEYBLOCK,
                    (krb5_pointer)&auth_context->key->keyblock, &bp, &remain);
            }
            ...
            if (!kret && auth_context->send_subkey)
            {
                ...
                kret = krb5_externalize_opaque(
                    kcontext, KV5M_KEYBLOCK,
                    (krb5_pointer)&auth_context->send_subkey->keyblock, &bp,
                    &remain);
            }
            ...
            if (!kret && auth_context->recv_subkey)
            {
                ...
                kret = krb5_externalize_opaque(
                    kcontext, KV5M_KEYBLOCK,
                    (krb5_pointer)&auth_context->recv_subkey->keyblock, &bp,
                    &remain);
            }
            ...
            if (!kret && auth_context->authentp)
                kret = krb5_externalize_opaque(
                    kcontext, KV5M_AUTHENTICATOR,
                    (krb5_pointer)auth_context->authentp, &bp, &remain);
 ...
 */
static krb5_error_code krb5_auth_context_internalize(krb5_context kcontext,
                                                     krb5_pointer* argp,
                                                     krb5_octet** buffer,
                                                     size_t* lenremain)
{
    ...
    if (ibuf == KV5M_AUTH_CONTEXT)
    {
        ...
        if ((remain >= (5 * sizeof(krb5_int32))) &&
            (auth_context = (krb5_auth_context)calloc(
                 1, sizeof(struct _krb5_auth_context))))
        {
            ...
            if (cstate_len)
            {
                ...
                if (!kret)
                {
                    kret = krb5_ser_unpack_bytes(
                        (krb5_octet*)auth_context->cstate.data, cstate_len, &bp,
                        &remain);
                }
            }
            else
            ...
            {
                if (!(kret = krb5_internalize_opaque(
                          kcontext, KV5M_ADDRESS,
                          (krb5_pointer*)&auth_context->remote_addr, &bp,
                          &remain)))
                    kret = krb5_ser_unpack_int32(&tag, &bp, &remain);
            }
            ...
            if (!kret && (tag == TOKEN_RPORT))
            {
                if (!(kret = krb5_internalize_opaque(
                          kcontext, KV5M_ADDRESS,
                          (krb5_pointer*)&auth_context->remote_port, &bp,
                          &remain)))
                    kret = krb5_ser_unpack_int32(&tag, &bp, &remain);
            }
            ...
            if (!kret && (tag == TOKEN_LADDR))
            {
                if (!(kret = krb5_internalize_opaque(
                          kcontext, KV5M_ADDRESS,
                          (krb5_pointer*)&auth_context->local_addr, &bp,
                          &remain)))
                    kret = krb5_ser_unpack_int32(&tag, &bp, &remain);
            }
            ...
            if (!kret && (tag == TOKEN_LPORT))
            {
                if (!(kret = krb5_internalize_opaque(
                          kcontext, KV5M_ADDRESS,
                          (krb5_pointer*)&auth_context->local_port, &bp,
                          &remain)))
                    kret = krb5_ser_unpack_int32(&tag, &bp, &remain);
            }
            ...
            {
                if (!(kret = intern_key(kcontext, &auth_context->key, &bp,
                                        &remain)))
                    kret = krb5_ser_unpack_int32(&tag, &bp, &remain);
            }
            ...
            {
                if (!(kret = intern_key(kcontext, &auth_context->send_subkey,
                                        &bp, &remain)))
                    kret = krb5_ser_unpack_int32(&tag, &bp, &remain);
            }
            ...
            if (!kret)
            {
                ...
                {
                    kret = intern_key(kcontext, &auth_context->recv_subkey, &bp,
                                      &remain);
                }
                else
                {
 ...
 */
krb5_error_code KRB5_CALLCONV krb5_ser_auth_context_init(krb5_context kcontext)
{
    krb5_error_code kret;
