 ...
 * #GDBusProxy::g-properties-changed and #GObject::notify) are emitted in the
...
static void signal_subscription_unref(SignalSubscriptionData* data)
{
...
G_DEFINE_TYPE_WITH_CODE(
    GDBusProxy, g_dbus_proxy, G_TYPE_OBJECT,
...
static void g_dbus_proxy_dispose(GObject* object)
{
...
static void g_dbus_proxy_finalize(GObject* object)
{
    ...
    if (proxy->priv->signals_subscription_id > 0)
        g_dbus_connection_signal_unsubscribe(
            proxy->priv->connection, proxy->priv->signals_subscription_id);
    ...
    if (proxy->priv->object != NULL)
        g_object_remove_weak_pointer(G_OBJECT(proxy->priv->object),
                                     (gpointer*)&proxy->priv->object);

    G_OBJECT_CLASS(g_dbus_proxy_parent_class)->finalize(object);
}

static void g_dbus_proxy_get_property(GObject* object, guint prop_id,
...
static void g_dbus_proxy_set_property(GObject* object, guint prop_id,
...
static void g_dbus_proxy_class_init(GDBusProxyClass* klass)
{
    ...
    /* Note that all property names are prefixed to avoid collisions with D-Bus
     * property names in derived classes */
    ...
    g_object_class_install_property(
        ...
        g_param_spec_boxed(
            "g-interface-info", P_("Interface Information"),
            P_("Interface Information"), G_TYPE_DBUS_INTERFACE_INFO,
     ...
     */
    g_object_class_install_property(
        gobject_class, PROP_G_CONNECTION,
        g_param_spec_object("g-connection", P_("g-connection"),
     ...
     */
    g_object_class_install_property(
        gobject_class, PROP_G_BUS_TYPE,
        g_param_spec_enum(
            "g-bus-type", P_("Bus Type"), P_("The bus to connect to, if any"),
     ...
     */
    g_object_class_install_property(
        gobject_class, PROP_G_FLAGS,
        g_param_spec_flags("g-flags", P_("g-flags"), P_("Flags for the proxy"),
                           G_TYPE_DBUS_PROXY_FLAGS, G_DBUS_PROXY_FLAGS_NONE,
                           G_PARAM_READABLE | G_PARAM_WRITABLE |
                               G_PARAM_CONSTRUCT_ONLY | G_PARAM_STATIC_NAME |
                               G_PARAM_STATIC_BLURB | G_PARAM_STATIC_NICK));
     ...
     */
    g_object_class_install_property(
        gobject_class, PROP_G_NAME,
        g_param_spec_string(
            "g-name", P_("g-name"),
            P_("The well-known or unique name that the proxy is for"), NULL,
     ...
     */
    g_object_class_install_property(
        gobject_class, PROP_G_NAME_OWNER,
        g_param_spec_string("g-name-owner", P_("g-name-owner"),
                            P_("The unique name for the owner"), NULL,
     ...
     */
    g_object_class_install_property(
        gobject_class, PROP_G_OBJECT_PATH,
        g_param_spec_string("g-object-path", P_("g-object-path"),
                            P_("The object path the proxy is for"), NULL,
                            G_PARAM_READABLE | G_PARAM_WRITABLE |
                                G_PARAM_CONSTRUCT_ONLY | G_PARAM_STATIC_NAME |
                                G_PARAM_STATIC_BLURB | G_PARAM_STATIC_NICK));
     ...
     */
    g_object_class_install_property(
        gobject_class, PROP_G_INTERFACE_NAME,
        g_param_spec_string("g-interface-name", P_("g-interface-name"),
     ...
     */
    g_object_class_install_property(
        gobject_class, PROP_G_DEFAULT_TIMEOUT,
        g_param_spec_int("g-default-timeout", P_("Default Timeout"),
                         P_("Timeout for remote method invocation"), -1,
                         G_MAXINT, -1,
                         G_PARAM_READABLE | G_PARAM_WRITABLE |
                             G_PARAM_CONSTRUCT | G_PARAM_STATIC_NAME |
                             G_PARAM_STATIC_BLURB | G_PARAM_STATIC_NICK));
     ...
     * (type: `a{sv}`)
     * @invalidated_properties: A %NULL terminated array of properties that was
     * invalidated
     *
     ...
     * Since: 2.26
     */
    signals[PROPERTIES_CHANGED_SIGNAL] =
        g_signal_new(I_("g-properties-changed"), G_TYPE_DBUS_PROXY,
                     G_SIGNAL_RUN_LAST | G_SIGNAL_MUST_COLLECT,
                     G_STRUCT_OFFSET(GDBusProxyClass, g_properties_changed),
                     NULL, NULL, NULL, G_TYPE_NONE, 2, G_TYPE_VARIANT,
                     G_TYPE_STRV | G_SIGNAL_TYPE_STATIC_SCOPE);

    /**
     * GDBusProxy::g-signal:
     * @proxy: The #GDBusProxy emitting the signal.
     * @sender_name: (nullable): The sender of the signal or %NULL if the
     * connection is not a bus connection.
     ...
     * Since: 2.26
     */
    signals[SIGNAL_SIGNAL] = g_signal_new(
        I_("g-signal"), G_TYPE_DBUS_PROXY,
        G_SIGNAL_RUN_LAST | G_SIGNAL_MUST_COLLECT,
        G_STRUCT_OFFSET(GDBusProxyClass, g_signal), NULL, NULL, NULL,
        G_TYPE_NONE, 3, G_TYPE_STRING, G_TYPE_STRING, G_TYPE_VARIANT);
}

static void g_dbus_proxy_init(GDBusProxy* proxy)
{
    proxy->priv = g_dbus_proxy_get_instance_private(proxy);
    proxy->priv->signal_subscription_data =
        g_slice_new0(SignalSubscriptionData);
    proxy->priv->signal_subscription_data->ref_count = 1;
    proxy->priv->signal_subscription_data->proxy = proxy;
    proxy->priv->properties = g_hash_table_new_full(
 ...
 * returned value
 */
static const GDBusPropertyInfo* lookup_property_info(GDBusProxy* proxy,
                                                     const gchar* property_name)
{
    ...
    info = g_dbus_interface_info_lookup_property(
        proxy->priv->expected_interface, property_name);
                                           ...
                                           const gchar* property_name)
{
    ...
    if (info != NULL)
    {
        ...
        {
            g_warning("Trying to get property %s with type %s but according to "
                      "the expected "
                      "interface the type is %s",
                      property_name, type_string, info->signature);
            value = NULL;
            goto out;
        }
    }
 ...
 * @proxy: A #GDBusProxy
 * @property_name: Property name.
 * @value: (nullable): Value for the property or %NULL to remove it from the
 * cache.
 *
                                      ...
                                      GVariant* value)
{
    ...
    if (value != NULL)
    {
        ...
        if (info != NULL)
        {
            if (g_strcmp0(info->signature, g_variant_get_type_string(value)) !=
                0)
            {
                g_warning("Trying to set property %s of type %s but according "
                          "to the expected "
                          "interface the type is %s",
                          property_name, g_variant_get_type_string(value),
                          info->signature);
                goto out;
            }
        }
        g_hash_table_insert(proxy->priv->properties, g_strdup(property_name),
                               ...
                               gpointer user_data)
{
    ...
    if (proxy->priv->name_owner != NULL &&
        g_strcmp0(sender_name, proxy->priv->name_owner) != 0)
    {
        G_UNLOCK(properties_lock);
        goto out;
    }
    ...
    if (proxy->priv->expected_interface != NULL)
    {
        ...
        info = g_dbus_interface_info_lookup_signal(
            proxy->priv->expected_interface, signal_name);
        if (info != NULL)
        {
            GVariantType* expected_type;
            expected_type = _g_dbus_compute_complete_signature(info->args);
            if (!g_variant_type_equal(expected_type,
                                      g_variant_get_type(parameters)))
            {
                gchar* expected_type_string =
                    g_variant_type_dup_string(expected_type);
                g_warning("Dropping signal %s of type %s since the type from "
                          "the expected interface is %s",
                          info->name, g_variant_get_type_string(parameters),
    ...
    g_signal_emit(proxy, signals[SIGNAL_SIGNAL], 0, sender_name, signal_name,
                  parameters);
...
static void insert_property_checked(GDBusProxy* proxy, gchar* property_name,
                                    GVariant* value)
{
    if (proxy->priv->expected_interface != NULL)
    {
        ...
        if (info != NULL)
        {
            ...
            if (g_strcmp0(info->signature, g_variant_get_type_string(value)) !=
                0)
            {
                g_warning("Received property %s with type %s does not match "
                          "expected type "
...
static void invalidated_property_get_cb(GDBusConnection* connection,
                                        GAsyncResult* res, gpointer user_data)
{
    ...
    {
        g_warning("Expected type '(v)' for Get() reply, got '%s'",
    ...
    G_LOCK(properties_lock);
    insert_property_checked(data->proxy, data->prop_name, /* adopts string */
                            unpacked_value);              /* adopts value */
    data->prop_name = NULL;
    G_UNLOCK(properties_lock);

    g_signal_emit(data->proxy, signals[PROPERTIES_CHANGED_SIGNAL], 0,
                                  ...
                                  GVariant* parameters, gpointer user_data)
{
    ...
    {
        g_warning("Value for PropertiesChanged signal with type '%s' does not "
                  "match '(sa{sv}as)'",
    ...
    {
        insert_property_checked(proxy, key, /* adopts string */
                                value);     /* adopts value */
        emit_g_signal = TRUE;
    }
        ...
        if (proxy->priv->name_owner != NULL)
        {
            for (n = 0; invalidated_properties[n] != NULL; n++)
            {
                ...
                g_dbus_connection_call(
                    ...
                    proxy->priv->object_path, "org.freedesktop.DBus.Properties",
                    "Get",
                    g_variant_new("(ss)", proxy->priv->interface_name,
                                  data->prop_name),
                    G_VARIANT_TYPE("(v)"), G_DBUS_CALL_FLAGS_NONE,
                    -1,   /* timeout */
                    NULL, /* GCancellable */
                    (GAsyncReadyCallback)invalidated_property_get_cb, data);
            }
        }
    }
    else
    {
        ...
        for (n = 0; invalidated_properties[n] != NULL; n++)
        {
            g_hash_table_remove(proxy->priv->properties,
    ...
    if (emit_g_signal)
    {
        g_signal_emit(proxy, signals[PROPERTIES_CHANGED_SIGNAL], 0,
                      changed_properties, invalidated_properties);
    }
...
static void process_get_all_reply(GDBusProxy* proxy, GVariant* result)
{
    ...
    {
        g_warning(
            "Value for GetAll reply with type '%s' does not match '(a{sv})'",
...
static void on_name_owner_changed_get_all_cb(GDBusConnection* connection,
                                             GAsyncResult* res,
                                             gpointer user_data)
{
    ...
    if (result == NULL)
    {
        ...
        {
            g_debug("error: %d %d %s", error->domain, error->code,
                    error->message);
        }
        g_error_free(error);
    }
    ...
    if (!cancelled)
    {
        ...
        data->name_owner =
            NULL; /* to avoid an extra copy, we steal the string */
                                  ...
                                  GVariant* parameters, gpointer user_data)
{
    ...
    if (strlen(new_owner) == 0)
    {
            ...
            g_hash_table_size(proxy->priv->properties) > 0)
        {
            ...
            /* Build changed_properties (always empty) and
             * invalidated_properties ... */
            g_variant_builder_init(&builder, G_VARIANT_TYPE("a{sv}"));
    ...
    else
    {
        ...
        else
        {
            ...
            /* start loading properties.. only then emit notify::g-name-owner ..
             * we need to be able to cancel this in the event another
             * NameOwnerChanged signal suddenly happens
             */
            ...
            g_dbus_connection_call(
                proxy->priv->connection, data->name_owner,
                proxy->priv->object_path, "org.freedesktop.DBus.Properties",
                "GetAll", g_variant_new("(s)", proxy->priv->interface_name),
                G_VARIANT_TYPE("(a{sv})"), G_DBUS_CALL_FLAGS_NONE,
                -1, /* timeout */
                proxy->priv->get_all_cancellable,
                (GAsyncReadyCallback)on_name_owner_changed_get_all_cb, data);
        }
    }
...
static void async_init_get_all_cb(GDBusConnection* connection,
                                  GAsyncResult* res, gpointer user_data)
{
...
static void async_init_data_set_name_owner(GTask* task, const gchar* name_owner)
{
    ...
    if (get_all)
    {
        ...
        g_dbus_connection_call(
            proxy->priv->connection, name_owner, proxy->priv->object_path,
            "org.freedesktop.DBus.Properties", "GetAll",
            g_variant_new("(s)", proxy->priv->interface_name),
            G_VARIANT_TYPE("(a{sv})"), G_DBUS_CALL_FLAGS_NONE, -1, /* timeout */
            g_task_get_cancellable(task),
            (GAsyncReadyCallback)async_init_get_all_cb, task);
    }
    else
    {
...
static void async_init_get_name_owner_cb(GDBusConnection* connection,
                                         GAsyncResult* res, gpointer user_data)
{
    GTask* task = user_data;
    GError* error;
...
static void async_init_call_get_name_owner(GTask* task)
{
    GDBusProxy* proxy = g_task_get_source_object(task);

    g_dbus_connection_call(
        proxy->priv->connection, "org.freedesktop.DBus", /* name */
        "/org/freedesktop/DBus",                         /* object path */
        "org.freedesktop.DBus",                          /* interface */
        "GetNameOwner", g_variant_new("(s)", proxy->priv->name),
        G_VARIANT_TYPE("(s)"), G_DBUS_CALL_FLAGS_NONE, -1, /* timeout */
        g_task_get_cancellable(task),
        (GAsyncReadyCallback)async_init_get_name_owner_cb, task);
}

static void async_init_start_service_by_name_cb(GDBusConnection* connection,
                                                GAsyncResult* res,
                                                gpointer user_data)
{
    GTask* task = user_data;
    ...
    if (result == NULL)
    {
         ...
         * org.gnome.Epiphany2 was not provided by any .service files
         *
         * or (see #677718)
         *
    ...
    else
    {
        ...
        else
        {
            error = g_error_new(
                ...
                start_service_result, proxy->priv->name);
            goto failed;
        }
    }
...
static void async_init_call_start_service_by_name(GTask* task)
{
    GDBusProxy* proxy = g_task_get_source_object(task);
    ...
    g_dbus_connection_call(
        ...
        "/org/freedesktop/DBus",                         /* object path */
        "org.freedesktop.DBus",                          /* interface */
        "StartServiceByName", g_variant_new("(su)", proxy->priv->name, 0),
        G_VARIANT_TYPE("(u)"), G_DBUS_CALL_FLAGS_NONE, -1, /* timeout */
        g_task_get_cancellable(task),
        (GAsyncReadyCallback)async_init_start_service_by_name_cb, task);
}

static void async_initable_init_second_async(GAsyncInitable* initable,
                                             gint io_priority,
                                             GCancellable* cancellable,
...
static gboolean async_initable_init_second_finish(GAsyncInitable* initable,
                                                  GAsyncResult* res,
                                                  GError** error)
{
    GDBusProxy* proxy = G_DBUS_PROXY(initable);
...
static void async_initable_init_first(GAsyncInitable* initable)
{
    ...
    if (!(proxy->priv->flags & G_DBUS_PROXY_FLAGS_DO_NOT_LOAD_PROPERTIES))
    {
        ...
        proxy->priv->properties_changed_subscription_id =
            g_dbus_connection_signal_subscribe(
                proxy->priv->connection, proxy->priv->name,
                "org.freedesktop.DBus.Properties", "PropertiesChanged",
                proxy->priv->object_path, proxy->priv->interface_name,
    ...
    if (!(proxy->priv->flags & G_DBUS_PROXY_FLAGS_DO_NOT_CONNECT_SIGNALS))
    {
        ...
        proxy->priv->signals_subscription_id =
            g_dbus_connection_signal_subscribe(
                proxy->priv->connection, proxy->priv->name,
                proxy->priv->interface_name, NULL, /* member */
                proxy->priv->object_path, NULL,    /* arg0 */
         ...
         G_DBUS_CONNECTION_FLAGS_MESSAGE_BUS_CONNECTION))
    {
        proxy->priv->name_owner_changed_subscription_id =
            g_dbus_connection_signal_subscribe(
                ...
                G_DBUS_SIGNAL_FLAGS_NONE, on_name_owner_changed,
                signal_subscription_ref(proxy->priv->signal_subscription_data),
                (GDestroyNotify)signal_subscription_unref);
    }
}
...
static void init_second_async_cb(GObject* source_object, GAsyncResult* res,
                                 gpointer user_data)
{
    GTask* task = user_data;
    GError* error = NULL;

    if (async_initable_init_second_finish(G_ASYNC_INITABLE(source_object), res,
                                          &error))
...
static void get_connection_cb(GObject* source_object, GAsyncResult* res,
                              gpointer user_data)
{
    GTask* task = user_data;
    GDBusProxy* proxy = g_task_get_source_object(task);
    ...
    else
    {
        ...
        async_initable_init_second_async(
            G_ASYNC_INITABLE(proxy), g_task_get_priority(task),
            g_task_get_cancellable(task), init_second_async_cb, task);
    }
}

static void async_initable_init_async(GAsyncInitable* initable,
                                      gint io_priority,
                                      GCancellable* cancellable,
                                      GAsyncReadyCallback callback,
                                      gpointer user_data)
{
    ...
    if (proxy->priv->bus_type != G_BUS_TYPE_NONE)
    {
        ...
        g_bus_get(proxy->priv->bus_type, cancellable, get_connection_cb, task);
    }
    else
    {
...
static gboolean async_initable_init_finish(GAsyncInitable* initable,
                              ...
                              GError** error)
{
    ...
    if (proxy->priv->bus_type != G_BUS_TYPE_NONE)
    {
        g_assert(proxy->priv->connection == NULL);
        proxy->priv->connection =
            g_bus_get_sync(proxy->priv->bus_type, cancellable, error);
        if (proxy->priv->connection == NULL)
            goto out;
    }
    ...
    async_initable_init_second_async(G_ASYNC_INITABLE(initable),
                                     ...
                                     async_initable_init_async_cb, data);
 ...
 * @name: (nullable): A bus name (well-known or unique) or %NULL if @connection
 ...
 * up match rules for signals. Connect to the #GDBusProxy::g-signal signal to
 ...
 * Since: 2.26
 */
void g_dbus_proxy_new(GDBusConnection* connection, GDBusProxyFlags flags,
                      ...
                      gpointer user_data)
{
    ...
    g_async_initable_new_async(
        G_TYPE_DBUS_PROXY, G_PRIORITY_DEFAULT, cancellable, callback, user_data,
        "g-flags", flags, "g-interface-info", info, "g-name", name,
        "g-connection", connection, "g-object-path", object_path,
        "g-interface-name", interface_name, NULL);
}
                                  ...
                                  GCancellable* cancellable, GError** error)
{
    ...
    g_return_val_if_fail((name == NULL && g_dbus_connection_get_unique_name(
                                              connection) == NULL) ||
                             g_dbus_is_name(name),
                         NULL);
    ...
    initable = g_initable_new(
        G_TYPE_DBUS_PROXY, cancellable, error, "g-flags", flags,
        "g-interface-info", info, "g-name", name, "g-connection", connection,
        "g-object-path", object_path, "g-interface-name", interface_name, NULL);
    if (initable != NULL)
 ...
 * Since: 2.26
 */
void g_dbus_proxy_new_for_bus(GBusType bus_type, GDBusProxyFlags flags,
                              GDBusInterfaceInfo* info, const gchar* name,
                              const gchar* object_path,
                              const gchar* interface_name,
                              GCancellable* cancellable,
...
GDBusProxy* g_dbus_proxy_new_for_bus_sync(
    GBusType bus_type, GDBusProxyFlags flags, GDBusInterfaceInfo* info,
    const gchar* name, const gchar* object_path, const gchar* interface_name,
    GCancellable* cancellable, GError** error)
{
    GInitable* initable;
    ...
    initable = g_initable_new(
        G_TYPE_DBUS_PROXY, cancellable, error, "g-flags", flags,
        "g-interface-info", info, "g-name", name, "g-bus-type", bus_type,
        "g-object-path", object_path, "g-interface-name", interface_name, NULL);
    if (initable != NULL)
        return G_DBUS_PROXY(initable);
    else
                                     ...
                                     GDBusInterfaceInfo* info)
{
    ...
    proxy->priv->expected_interface =
        info != NULL ? g_dbus_interface_info_ref(info) : NULL;
    if (proxy->priv->expected_interface != NULL)
...
static gboolean maybe_split_method_name(const gchar* method_name,
...
static void reply_cb(GDBusConnection* connection, GAsyncResult* res,
                     gpointer user_data)
{
    GTask* task = user_data;
    GVariant* value;
 ...
 * returned value
 */
static const GDBusMethodInfo* lookup_method_info(GDBusProxy* proxy,
                                                 const gchar* method_name)
{
    ...
    info = g_dbus_interface_info_lookup_method(proxy->priv->expected_interface,
                                               method_name);
...
static void g_dbus_proxy_call_internal(
    GDBusProxy* proxy, const gchar* method_name, GVariant* parameters,
    GDBusCallFlags flags, gint timeout_msec, GUnixFDList* fd_list,
    GCancellable* cancellable, GAsyncReadyCallback callback, gpointer user_data)
{
    ...
    g_return_if_fail(G_IS_DBUS_PROXY(proxy));
    g_return_if_fail(g_dbus_is_member_name(method_name) ||
                     g_dbus_is_interface_name(method_name));
    ...
    was_split = maybe_split_method_name(method_name, &split_interface_name,
                                        &split_method_name);
    ...
    if (!was_split)
    {
        ...
        if (expected_method_info != NULL)
            reply_type = _g_dbus_compute_complete_signature(
        ...
        if (destination == NULL)
        {
            if (task != NULL)
            {
                g_task_return_new_error(
                    ...
                    _("Cannot invoke method; proxy is for the well-known name "
                      "%s without an owner, and proxy was constructed with the "
    ...
    g_dbus_connection_call_with_unix_fd_list(
        ...
        target_interface_name, target_method_name, parameters, reply_type,
        flags, timeout_msec == -1 ? proxy->priv->timeout_msec : timeout_msec,
        fd_list, cancellable, my_callback, task);
#else
    g_dbus_connection_call(
        proxy->priv->connection, destination, proxy->priv->object_path,
        target_interface_name, target_method_name, parameters, reply_type,
        flags, timeout_msec == -1 ? proxy->priv->timeout_msec : timeout_msec,
        cancellable, my_callback, task);
#endif
...
static GVariant* g_dbus_proxy_call_finish_internal(GDBusProxy* proxy,
                                                   ...
                                                   GError** error)
{
    ...
    if (out_fd_list != NULL)
        *out_fd_list =
            data->fd_list != NULL ? g_object_ref(data->fd_list) : NULL;
#endif
...
static GVariant* g_dbus_proxy_call_sync_internal(
    GDBusProxy* proxy, const gchar* method_name, GVariant* parameters,
    GDBusCallFlags flags, gint timeout_msec, GUnixFDList* fd_list,
    GUnixFDList** out_fd_list, GCancellable* cancellable, GError** error)
{
    ...
    g_return_val_if_fail(G_IS_DBUS_PROXY(proxy), NULL);
    g_return_val_if_fail(g_dbus_is_member_name(method_name) ||
                             g_dbus_is_interface_name(method_name),
                         NULL);
    ...
    if (proxy->priv->name != NULL)
    {
        ...
        if (destination == NULL)
        {
            g_set_error(error, G_IO_ERROR, G_IO_ERROR_FAILED,
                        _("Cannot invoke method; proxy is for the well-known "
                          "name %s without an owner, and proxy was constructed "
    ...
    ret = g_dbus_connection_call_with_unix_fd_list_sync(
        ...
        target_interface_name, target_method_name, parameters, reply_type,
        flags, timeout_msec == -1 ? proxy->priv->timeout_msec : timeout_msec,
        fd_list, out_fd_list, cancellable, error);
#else
 ...
 * @parameters: (nullable): A #GVariant tuple with parameters for the signal or
 * %NULL if not passing parameters.
 * @flags: Flags from the #GDBusCallFlags enumeration.
 * @timeout_msec: The timeout in milliseconds (with %G_MAXINT meaning
 *                "infinite") or -1 to use the proxy default timeout.
 * @cancellable: (nullable): A #GCancellable or %NULL.
 * @callback: (nullable): A #GAsyncReadyCallback to call when the request is
 * satisfied or %NULL if you don't care about the result of the method
 * invocation.
 ...
 * Since: 2.26
 */
void g_dbus_proxy_call(GDBusProxy* proxy, const gchar* method_name,
                       GVariant* parameters, GDBusCallFlags flags,
                       gint timeout_msec, GCancellable* cancellable,
                       GAsyncReadyCallback callback, gpointer user_data)
{
    g_dbus_proxy_call_internal(proxy, method_name, parameters, flags,
                               timeout_msec, NULL, cancellable, callback,
                               user_data);
}
 ...
 * Since: 2.26
 */
GVariant* g_dbus_proxy_call_finish(GDBusProxy* proxy, GAsyncResult* res,
                                   GError** error)
{
 ...
 * Since: 2.26
 */
GVariant* g_dbus_proxy_call_sync(GDBusProxy* proxy, const gchar* method_name,
                                 GVariant* parameters, GDBusCallFlags flags,
                                 gint timeout_msec, GCancellable* cancellable,
                                 GError** error)
{
    return g_dbus_proxy_call_sync_internal(proxy, method_name, parameters,
                                           flags, timeout_msec, NULL, NULL,
                                           cancellable, error);
}
...
{
    g_dbus_proxy_call_internal(proxy, method_name, parameters, flags,
                               timeout_msec, fd_list, cancellable, callback,
                               user_data);
}
...
static GDBusInterfaceInfo* _g_dbus_proxy_get_info(GDBusInterface* interface)
{
    GDBusProxy* proxy = G_DBUS_PROXY(interface);
    return g_dbus_proxy_get_interface_info(proxy);
}

static GDBusObject* _g_dbus_proxy_get_object(GDBusInterface* interface)
{
    GDBusProxy* proxy = G_DBUS_PROXY(interface);
    return proxy->priv->object;
}

static GDBusObject* _g_dbus_proxy_dup_object(GDBusInterface* interface)
{
    GDBusProxy* proxy = G_DBUS_PROXY(interface);
...
static void _g_dbus_proxy_set_object(GDBusInterface* interface,
                                     GDBusObject* object)
{
    GDBusProxy* proxy = G_DBUS_PROXY(interface);
    G_LOCK(properties_lock);
    ...
    if (proxy->priv->object != NULL)
        g_object_add_weak_pointer(G_OBJECT(proxy->priv->object),
                                  (gpointer*)&proxy->priv->object);
    G_UNLOCK(properties_lock);
}
