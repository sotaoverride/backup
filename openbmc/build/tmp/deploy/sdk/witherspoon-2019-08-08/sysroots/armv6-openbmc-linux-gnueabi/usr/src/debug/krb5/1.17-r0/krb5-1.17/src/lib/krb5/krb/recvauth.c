...
static krb5_error_code recvauth_common(krb5_context context,
                                       ...
                                       krb5_ticket** ticket, krb5_data* version)
{
    ...
    if ((!problem) && rcache == NULL)
    {
        ...
        {
            problem =
                krb5_get_server_rcache(context, &server->data[0], &rcache);
        }
        else
        {
    ...
    if (!problem)
    {
        problem = krb5_rd_req(context, auth_context, &inbuf, server, keytab,
                              &ap_option, ticket);
        free(inbuf.data);
    }
...
{
    return recvauth_common(context, auth_context, fd, appl_version, server,
                           flags, keytab, ticket, 0);
}

krb5_error_code KRB5_CALLCONV
    krb5_recvauth_version(krb5_context context, krb5_auth_context* auth_context,
                          /* IN */
                          ...
                          krb5_ticket** ticket, krb5_data* version)
{
    return recvauth_common(context, auth_context, fd, 0, server, flags, keytab,
                           ticket, version);
}
