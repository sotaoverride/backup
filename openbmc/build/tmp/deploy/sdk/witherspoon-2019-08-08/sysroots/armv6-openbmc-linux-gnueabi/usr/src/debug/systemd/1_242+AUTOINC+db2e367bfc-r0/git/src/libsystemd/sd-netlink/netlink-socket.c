...
static int broadcast_groups_get(sd_netlink* nl)
{
    ...
    for (i = 0; i < len; i++)
    {
        for (j = 0; j < sizeof(uint32_t) * 8; j++)
        {
            ...
            r = hashmap_put(nl->broadcast_group_refs, UINT_TO_PTR(group),
                            UINT_TO_PTR(1));
            if (r < 0)
                return r;
        }
...
static unsigned broadcast_group_get_ref(sd_netlink* nl, unsigned group)
{
    ...
    return PTR_TO_UINT(
        hashmap_get(nl->broadcast_group_refs, UINT_TO_PTR(group)));
}

static int broadcast_group_set_ref(sd_netlink* nl, unsigned group,
                                   unsigned n_ref)
{
    int r;
    ...
    r = hashmap_replace(nl->broadcast_group_refs, UINT_TO_PTR(group),
                        UINT_TO_PTR(n_ref));
    if (r < 0)
        return r;
...
static int broadcast_group_join(sd_netlink* nl, unsigned group)
{
    ...
    r = setsockopt(nl->fd, SOL_NETLINK, NETLINK_ADD_MEMBERSHIP, &group,
                   sizeof(group));
    if (r < 0)
        return -errno;
...
static int broadcast_group_leave(sd_netlink* nl, unsigned group)
{
    ...
    r = setsockopt(nl->fd, SOL_NETLINK, NETLINK_DROP_MEMBERSHIP, &group,
                   sizeof(group));
    if (r < 0)
        return -errno;
...
static int socket_recv_message(int fd, struct iovec* iov, uint32_t* _group,
                               bool peek)
{
    ...
    if (sender.nl.nl_pid != 0)
    {
        ...
        log_debug("rtnl: ignoring message from portid %" PRIu32,
                  sender.nl.nl_pid);
...
/* On success, the number of bytes received is returned and *ret points to the
 * received message which has a valid header and the correct size. If nothing
 * useful was received 0 is returned. On failure, a negative error code is
 * returned.
 */
int socket_read_message(sd_netlink* rtnl)
{
    ...
    if (!greedy_realloc((void**)&rtnl->rbuffer, &rtnl->rbuffer_allocated, len,
                        sizeof(uint8_t)))
        return -ENOMEM;
    ...
    if (NLMSG_OK(rtnl->rbuffer, len) &&
        rtnl->rbuffer->nlmsg_flags & NLM_F_MULTI)
    {
        multi_part = true;
    ...
    for (new_msg = rtnl->rbuffer; NLMSG_OK(new_msg, len) && !done;
         new_msg = NLMSG_NEXT(new_msg, len))
    {
        ...
        r = type_system_get_type(type_system_root, &nl_type,
                                 new_msg->nlmsg_type);
        if (r < 0)
        {
            if (r == -EOPNOTSUPP)
