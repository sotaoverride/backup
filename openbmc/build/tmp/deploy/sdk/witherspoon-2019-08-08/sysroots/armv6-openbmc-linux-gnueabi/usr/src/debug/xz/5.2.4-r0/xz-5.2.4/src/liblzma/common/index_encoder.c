    ...
    while (*out_pos < out_size)
        switch (coder->sequence)
        {
            ...
            case SEQ_COUNT:
            {
                ...
                ret =
                    lzma_vli_encode(count, &coder->pos, out, out_pos, out_size);
            ...
            case SEQ_UNCOMPRESSED:
            {
                ...
                ret =
                    lzma_vli_encode(size, &coder->pos, out, out_pos, out_size);
                if (ret != LZMA_STREAM_END)
                    goto out;
            ...
            case SEQ_PADDING:
                ...
                coder->crc32 = lzma_crc32(out + out_start, *out_pos - out_start,
                                          coder->crc32);
            ...
            case SEQ_CRC32:
                ...
                do
                {
                    ...
                    out[*out_pos] = (coder->crc32 >> (coder->pos * 8)) & 0xFF;
                    ++*out_pos;
...
out:
    // Update the CRC32.
    coder->crc32 =
        lzma_crc32(out + out_start, *out_pos - out_start, coder->crc32);
                             ...
                             size_t out_size)
{
    ...
    const size_t out_start = *out_pos;
    lzma_ret ret = index_encode(&coder, NULL, NULL, NULL, 0, out, out_pos,
                                out_size, LZMA_RUN);
