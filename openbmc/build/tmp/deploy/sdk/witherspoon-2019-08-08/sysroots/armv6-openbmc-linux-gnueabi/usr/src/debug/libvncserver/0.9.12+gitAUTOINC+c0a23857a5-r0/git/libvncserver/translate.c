...
static const rfbPixelFormat BGR233Format = {8, 8, 0, 1, 7, 7, 3, 0, 3, 6, 0, 0};
    ...
    ((x.bitsPerPixel == y.bitsPerPixel) && (x.depth == y.depth) &&             \
     ...
     (!x.trueColour ||                                                         \
      ((x.redMax == y.redMax) && (x.greenMax == y.greenMax) &&                 \
       (x.blueMax == y.blueMax) && (x.redShift == y.redShift) &&               \
       (x.greenShift == y.greenShift) && (x.blueShift == y.blueShift))))
...
static rfbInitCMTableFnType rfbInitColourMapSingleTableFns[COUNT_OFFSETS] = {
    rfbInitColourMapSingleTable8, rfbInitColourMapSingleTable16,
#ifdef LIBVNCSERVER_ALLOW24BPP
...
static rfbInitTableFnType rfbInitTrueColourSingleTableFns[COUNT_OFFSETS] = {
    rfbInitTrueColourSingleTable8, rfbInitTrueColourSingleTable16,
#ifdef LIBVNCSERVER_ALLOW24BPP
...
static rfbInitTableFnType rfbInitTrueColourRGBTablesFns[COUNT_OFFSETS] = {
    rfbInitTrueColourRGBTables8, rfbInitTrueColourRGBTables16,
#ifdef LIBVNCSERVER_ALLOW24BPP
    rfbInitTrueColourRGBTables24,
#endif
    rfbInitTrueColourRGBTables32};

static rfbTranslateFnType
    rfbTranslateWithSingleTableFns[COUNT_OFFSETS][COUNT_OFFSETS] = {
        {rfbTranslateWithSingleTable8to8, rfbTranslateWithSingleTable8to16,
#ifdef LIBVNCSERVER_ALLOW24BPP
...
#ifdef LIBVNCSERVER_ALLOW24BPP
        {rfbTranslateWithSingleTable24to8, rfbTranslateWithSingleTable24to16,
         rfbTranslateWithSingleTable24to24, rfbTranslateWithSingleTable24to32},
#endif
        {rfbTranslateWithSingleTable32to8, rfbTranslateWithSingleTable32to16,
#ifdef LIBVNCSERVER_ALLOW24BPP
...
static rfbTranslateFnType
    rfbTranslateWithRGBTablesFns[COUNT_OFFSETS][COUNT_OFFSETS] = {
        {rfbTranslateWithRGBTables8to8, rfbTranslateWithRGBTables8to16,
#ifdef LIBVNCSERVER_ALLOW24BPP
         rfbTranslateWithRGBTables8to24,
#endif
         rfbTranslateWithRGBTables8to32},
        {rfbTranslateWithRGBTables16to8, rfbTranslateWithRGBTables16to16,
#ifdef LIBVNCSERVER_ALLOW24BPP
         rfbTranslateWithRGBTables16to24,
#endif
         rfbTranslateWithRGBTables16to32},
#ifdef LIBVNCSERVER_ALLOW24BPP
        {rfbTranslateWithRGBTables24to8, rfbTranslateWithRGBTables24to16,
         rfbTranslateWithRGBTables24to24, rfbTranslateWithRGBTables24to32},
#endif
        {rfbTranslateWithRGBTables32to8, rfbTranslateWithRGBTables32to16,
#ifdef LIBVNCSERVER_ALLOW24BPP
...
rfbBool rfbSetTranslateFunction(rfbClientPtr cl)
{
    ...
    if ((cl->format.bitsPerPixel != 8) && (cl->format.bitsPerPixel != 16) &&
#ifdef LIBVNCSERVER_ALLOW24BPP
         ...
         (cl->screen->serverFormat.bitsPerPixel == 16)))
    {
        ...
        cl->translateFn =
            rfbTranslateWithSingleTableFns[BPP2OFFSET(
                cl->screen->serverFormat.bitsPerPixel)]
                                          [BPP2OFFSET(cl->format.bitsPerPixel)];

        if (cl->screen->serverFormat.trueColour)
            (*rfbInitTrueColourSingleTableFns[BPP2OFFSET(
                cl->format.bitsPerPixel)])(&cl->translateLookupTable,
                                           &(cl->screen->serverFormat),
                                           &cl->format);
        else
            (*rfbInitColourMapSingleTableFns[BPP2OFFSET(
                cl->format.bitsPerPixel)])(&cl->translateLookupTable,
                                           &(cl->screen->serverFormat),
                                           &cl->format, &cl->screen->colourMap);
    }
    else
    {
        ...
        cl->translateFn =
            rfbTranslateWithRGBTablesFns[BPP2OFFSET(
                cl->screen->serverFormat.bitsPerPixel)]
                                        [BPP2OFFSET(cl->format.bitsPerPixel)];

        (*rfbInitTrueColourRGBTablesFns[BPP2OFFSET(cl->format.bitsPerPixel)])(
...
rfbBool rfbSetClientColourMap(rfbClientPtr cl, int firstColour, int nColours)
{
    ...
    {
        LOCK(cl->updateMutex);
        (*rfbInitColourMapSingleTableFns[BPP2OFFSET(cl->format.bitsPerPixel)])(
            &cl->translateLookupTable, &cl->screen->serverFormat, &cl->format,
            &cl->screen->colourMap);
...
void rfbSetClientColourMaps(rfbScreenInfoPtr rfbScreen, int firstColour,
                            int nColours)
{
    ...
    else
    {
        ...
        if (pf->trueColour)
        {
            rfbLog("  true colour: max r %d g %d b %d, shift r %d g %d b %d\n",
                   pf->redMax, pf->greenMax, pf->blueMax, pf->redShift,
                   pf->greenShift, pf->blueShift);
        }
        else
        {
