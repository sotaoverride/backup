...
static krb5_error_code mk_priv_basic(krb5_context context,
                                     const krb5_data* userdata, krb5_key key,
                                     ...
                                     krb5_data* cstate, krb5_data* outbuf)
{
    ...
    if ((retval = krb5_c_encrypt_length(context, enctype, scratch1->length,
                                        &enclen)))
        goto clean_scratch;
    ...
    if ((retval = krb5_k_encrypt(context, key, KRB5_KEYUSAGE_KRB_PRIV_ENCPART,
                                 (cstate->length > 0) ? cstate : NULL, scratch1,
                                 &privmsg.enc_part)))
        goto clean_encpart;
    ...
    *outbuf = empty_data();
...

    {
        ...
        if (auth_context->remote_addr)
        {
            ...
            {
                if (!(retval = krb5_make_fulladdr(
                          context, auth_context->remote_addr,
