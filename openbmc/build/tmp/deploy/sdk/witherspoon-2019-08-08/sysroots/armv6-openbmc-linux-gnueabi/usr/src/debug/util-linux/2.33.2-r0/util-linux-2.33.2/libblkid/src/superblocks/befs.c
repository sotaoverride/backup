...
#define FS16_TO_CPU(value, fs_is_le)                                           \
    (fs_is_le ? le16_to_cpu(value) : be16_to_cpu(value))
#define FS32_TO_CPU(value, fs_is_le)                                           \
    (fs_is_le ? le32_to_cpu(value) : be32_to_cpu(value))
#define FS64_TO_CPU(value, fs_is_le)                                           \
    (fs_is_le ? le64_to_cpu(value) : be64_to_cpu(value))
...
static unsigned char* get_block_run(blkid_probe pr,
                                    const struct befs_super_block* bs,
                                    const struct block_run* br, int fs_le)
{
    return blkid_probe_get_buffer(
        pr,
...
static unsigned char* get_tree_node(blkid_probe pr,
                                    const struct befs_super_block* bs,
                                    const struct data_stream* ds, int64_t start,
                                    uint32_t length, int fs_le)
{
    if (start < (int64_t)FS64_TO_CPU(ds->max_direct_range, fs_le))
    {
        ...
        for (i = 0; i < NUM_DIRECT_BLOCKS; i++)
        {
            br_len = (int64_t)FS16_TO_CPU(ds->direct[i].len, fs_le)
                     << FS32_TO_CPU(bs->block_shift, fs_le);
            if (start < br_len)
                return get_custom_block_run(pr, bs, &ds->direct[i], start,
                                            length, fs_le);
            else
                start -= br_len;
        }
    ...
    else if (start < (int64_t)FS64_TO_CPU(ds->max_indirect_range, fs_le))
    {
        ...
        br = (struct block_run*)get_block_run(pr, bs, &ds->indirect, fs_le);
        if (!br)
            return NULL;
        ...
        for (i = 0; i < max_br; i++)
        {
            br_len = (int64_t)FS16_TO_CPU(br[i].len, fs_le)
                     << FS32_TO_CPU(bs->block_shift, fs_le);
            if (start < br_len)
                return get_custom_block_run(pr, bs, &br[i], start, length,
                                            fs_le);
            else
                start -= br_len;
        }
    ...
    else if (start < (int64_t)FS64_TO_CPU(ds->max_double_indirect_range, fs_le))
    {
        ...
        di_br_size = (int64_t)FS16_TO_CPU(ds->double_indirect.len, fs_le)
                     << FS32_TO_CPU(bs->block_shift, fs_le);
        if (di_br_size == 0)
            return NULL;
        ...
        br = (struct block_run*)get_block_run(pr, bs, &ds->double_indirect,
                                              fs_le);
        if (!br)
            return NULL;
        ...
        br = (struct block_run*)get_block_run(pr, bs, &br[di_index], fs_le);
        if (!br)
            return NULL;

        return get_custom_block_run(pr, bs, &br[i_index], start, length, fs_le);
    }
    return NULL;
}
...
static int64_t get_key_value(blkid_probe pr, const struct befs_super_block* bs,
                             const struct befs_inode* bi, const char* key,
                             int fs_le)
{
    ...
    errno = 0;
    bh = (struct bplustree_header*)get_tree_node(
        pr, bs, &bi->data, 0, sizeof(struct bplustree_header), fs_le);
    if (!bh)
    ...
    do
    {
        ...
        bn = (struct bplustree_node*)get_tree_node(
            pr, bs, &bi->data, node_pointer, bn_size, fs_le);
        if (!bn)
            return errno ? -errno : -ENOENT;

        all_key_count = FS16_TO_CPU(bn->all_key_count, fs_le);
        all_key_length = FS16_TO_CPU(bn->all_key_length, fs_le);
        keylengths_offset = (sizeof(struct bplustree_node) + all_key_length +
                             sizeof(int64_t) - 1) &
                            ~(sizeof(int64_t) - 1);
        values_offset = keylengths_offset + all_key_count * sizeof(uint16_t);
        ...
        else
        {
            while (first <= last)
            {
                ...
                cmp = compare_keys(bn->name, keylengths, mid, key, strlen(key),
                                   all_key_length, fs_le);
                if (cmp == BAD_KEYS)
                    return -ENOENT;
                ...
                if (cmp == 0)
                {
                    if ((int64_t)FS64_TO_CPU(bn->overflow_link, fs_le) ==
                        BPLUSTREE_NULL)
                        return FS64_TO_CPU(values[mid], fs_le);
                    else
                        break;
                }
                else if (cmp < 0)
            ...
            if (cmp < 0)
                node_pointer = FS64_TO_CPU(values[mid + 1], fs_le);
            else
                node_pointer = FS64_TO_CPU(values[mid], fs_le);
        }
    } while (++loop_detect < 100 &&
             (int64_t)FS64_TO_CPU(bn->overflow_link, fs_le) != BPLUSTREE_NULL);
    return 0;
}
                    ...
                    uint64_t* const uuid, int fs_le)
{
    ...
    while (offset + sizeof(struct small_data) <= sd_total_size)
    {
            ...
            strcmp(sd->name, KEY_NAME) == 0)
        {
            ...
            memcpy(uuid, sd->name + FS16_TO_CPU(sd->name_size, fs_le) + 3,
                   sizeof(uint64_t));
         ...
         FS16_TO_CPU(bi->attributes.len, fs_le) != 0))
    {
        ...
        bi = (struct befs_inode*)get_block_run(pr, bs, &bi->attributes, fs_le);
        if (!bi)
            return errno ? -errno : BLKID_PROBE_NONE;
        ...
        {
            bi = (struct befs_inode*)blkid_probe_get_buffer(
                pr, value << FS32_TO_CPU(bs->block_shift, fs_le),
            ...
            if (FS32_TO_CPU(bi->type, fs_le) == B_UINT64_TYPE &&
                ...
                FS16_TO_CPU(bi->data.direct[0].len, fs_le) == 1)
            {
                uint64_t* attr_data;

                attr_data = (uint64_t*)get_block_run(
                    pr, bs, &bi->data.direct[0], fs_le);
                if (!attr_data)
                    return errno ? -errno : BLKID_PROBE_NONE;
...
static int probe_befs(blkid_probe pr, const struct blkid_idmag* mag)
{
    ...
    bs = (struct befs_super_block*)blkid_probe_get_buffer(
        pr, mag->sboff - B_OS_NAME_LENGTH, sizeof(struct befs_super_block));
    if (!bs)
        return errno ? -errno : BLKID_PROBE_NONE;
    ...
    if (block_shift < 10 || block_shift > 13 || block_size != 1U << block_shift)
        return BLKID_PROBE_NONE;
     ...
     */
    if (*bs->name != '\0')
        blkid_probe_set_label(pr, (unsigned char*)bs->name, sizeof(bs->name));
    if (version)
...
const struct blkid_idinfo befs_idinfo = {
    ...
    .minsz = 1024 * 1440,
    .magics = {{.magic = "BFS1", .len = 4, .sboff = B_OS_NAME_LENGTH},
               {.magic = "1SFB", .len = 4, .sboff = B_OS_NAME_LENGTH},
               {.magic = "BFS1", .len = 4, .sboff = 0x200 + B_OS_NAME_LENGTH},
               {.magic = "1SFB", .len = 4, .sboff = 0x200 + B_OS_NAME_LENGTH},
               {NULL}}};
