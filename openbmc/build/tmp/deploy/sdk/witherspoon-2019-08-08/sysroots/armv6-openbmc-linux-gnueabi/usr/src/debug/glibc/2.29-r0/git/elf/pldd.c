...
static const struct argp_option options[] = {{NULL, 0, NULL, 0, NULL}};
...
int main(int argc, char* argv[])
{
    ...
    ssize_t nexe;
    while ((nexe = readlinkat(dfd, "exe", exebuf.data, exebuf.length)) ==
           exebuf.length)
    {
...
static void wait_for_ptrace_stop(long int pid)
{
    ...
    while (waitpid(pid, &status, __WALL) == pid && WIFSTOPPED(status))
    {
        ...
        if (WSTOPSIG(status) == SIGSTOP)
            return;
        if (ptrace(PTRACE_CONT, pid, NULL, (void*)(uintptr_t)WSTOPSIG(status)))
