                                         ...
                                         krb5_principal* canon_user)
{
    ...
    *canon_user = NULL;
        ...
        in_creds->client->type != KRB5_NT_ENTERPRISE_PRINCIPAL)
    {
        ...
        return krb5_copy_principal(
            context, anonymous ? in_creds->server : in_creds->client,
            canon_user);
    }
    ...
    code =
        k5_get_init_creds(context, &creds, &client, NULL, NULL, 0, NULL, opts,
                          krb5_get_as_key_noop, &userid, &use_master, NULL);
...
static krb5_error_code make_pa_for_user_checksum(krb5_context context,
                                                 krb5_keyblock* key,
                                                 ...
                                                 krb5_checksum* cksum)
{
    ...
    code = krb5_c_make_checksum(context, CKSUMTYPE_HMAC_MD5_ARCFOUR, key,
                                KRB5_KEYUSAGE_APP_DATA_CKSUM, &data, cksum);
...
static krb5_error_code build_pa_for_user(krb5_context context, krb5_creds* tgt,
                                         krb5_s4u_userid* userid,
                                         krb5_pa_data** out_padata)
{
    ...
    code = make_pa_for_user_checksum(context, &tgt->keyblock, &for_user,
                                     &for_user.cksum);
    if (code != 0)
        goto cleanup;
 ...
 */
static krb5_error_code build_pa_s4u_x509_user(krb5_context context,
                                              ...
                                              void* gcvt_data)
{
    ...
    {
        code =
            krb5int_c_mandatory_cksumtype(context, subkey->enctype, &cksumtype);
    }
    if (code != 0)
        goto cleanup;
    ...
    padata = realloc(tgsreq->padata, (i + 2) * sizeof(krb5_pa_data*));
    if (padata == NULL)
    {
        code = ENOMEM;
        goto cleanup;
                          ...
                          krb5_pa_data** rep_padata, krb5_pa_data** enc_padata)
{
    ...
    enc_s4u_padata =
        krb5int_find_pa_data(context, enc_padata, KRB5_PADATA_S4U_X509_USER);
    ...
    if (not_newer)
    {
        ...
        else
        {
            ...
            if (memcmp(enc_s4u_padata->contents, req_s4u_user->cksum.contents,
...
static krb5_error_code convert_to_enterprise(krb5_context context,
...
static krb5_error_code
    krb5_get_self_cred_from_kdc(krb5_context context, krb5_flags options,
                                krb5_ccache ccache, krb5_creds* in_creds,
                                krb5_data* subject_cert, krb5_data* user_realm,
                                krb5_creds** out_creds)
{
    ...
    *out_creds = NULL;
    ...
    if (in_creds->client != NULL && in_creds->client->length > 0)
    {
        if (in_creds->client->type == KRB5_NT_ENTERPRISE_PRINCIPAL)
        {
            code = krb5_build_principal_ext(
                context, &s4u_user.user_id.user, user_realm->length,
                user_realm->data, in_creds->client->data[0].length,
                in_creds->client->data[0].data, 0);
            if (code != 0)
                goto cleanup;
        ...
        {
            code = krb5_copy_principal(context, in_creds->client,
    ...
    code = krb5int_tgtname(context, user_realm, &in_creds->server->realm, &tgs);
    if (code != 0)
        goto cleanup;
         ...
         referral_count++)
    {
        ...
        code = krb5_get_cred_via_tkt_ext(
            context, tgtptr,
            KDC_OPT_CANONICALIZE | FLAGS2OPTS(tgtptr->ticket_flags) | kdcopt,
            tgtptr->addresses, in_padata, &s4u_creds, build_pa_s4u_x509_user,
            &s4u_user, &out_padata, &enc_padata, out_creds, &subkey);
        if (code != 0)
        {
            krb5_free_checksum_contents(context, &s4u_user.cksum);
            krb5_free_pa_data(context, in_padata);
        ...
        else if (IS_TGS_PRINC((*out_creds)->server))
        {
            ...
            {
                if (krb5_principal_compare(context, (*out_creds)->server,
...
krb5_error_code KRB5_CALLCONV krb5_get_credentials_for_user(
    ...
    krb5_creds* in_creds, krb5_data* subject_cert, krb5_creds** out_creds)
{
    krb5_error_code code;
    ...
    krb5_creds* in_creds, krb5_ticket* evidence_tkt, krb5_creds** out_creds)
{
    ...
    *out_creds = NULL;
    ...
    code = krb5int_construct_matching_creds(context, options, in_creds, &mcreds,
                                            &fields);
    if (code != 0)
        goto cleanup;
    ...
    code =
        krb5_get_credentials(context, options | KRB5_GC_CONSTRAINED_DELEGATION,
