...
// The trait attempts to detect if the T type is an iterator. Class-type
// iterators are assumed to have the nested type iterator_category. Strictly
// speaking, this is not required to be the case (e.g. a user can specialize
// iterator_traits for T without defining T::iterator_category). Still, this is
// a good heuristic in practice, and we can't do anything better anyway. Since
// C++17 we can test for iterator_traits<T>::iterator_category presence instead
// as it is required to be only present for iterators.
...
template <typename T, typename Distance,
          bool HasPlus = has_plus<T, Distance>::value>
...
template <typename T, typename Distance,
          bool HasPlusAssign = has_plus_assign<T, Distance>::value>
...
template <typename T, typename Distance,
          bool IsIterator = is_iterator<T>::value>
...
template <typename T, typename Distance,
          bool HasMinus = has_minus<T, Distance>::value>
...
template <typename T, typename Distance,
          bool HasMinusAssign = has_minus_assign<T, Distance>::value>
