...
static struct idlist* add_to_list(struct idlist** root, id_t id,
                                  union name_or_id noiu, id_t id2, uint16 flags)
{
...
static int is_in_group(gid_t gid)
{
    ...
    if (ngroups < -1)
    {
        ...
        if (DEBUG_GTE(OWN, 2))
        {
            ...
            if (!gidbuf)
                out_of_memory("is_in_group");
            pos = snprintf(gidbuf, 32, "process has %d gid%s: ", ngroups,
                           ngroups == 1 ? "" : "s");
            for (n = 0; n < ngroups; n++)
            {
...
static struct idlist* recv_add_id(struct idlist** idlist_ptr,
                                  struct idlist* idmap, id_t id,
                                  const char* name)
{
    struct idlist* node;
    ...
    flag = idlist_ptr == &gidlist && !am_root && !is_in_group(id2)
               ? FLAG_SKIP_GROUP
               : 0;
    ...
    if (DEBUG_GTE(OWN, 2))
    {
        rprintf(FINFO, "%sid %u(%s) maps to %u\n",
                idlist_ptr == &uidlist ? "u" : "g", (unsigned)id, name,
                (unsigned)id2);
    }
...
uid_t recv_user_name(int f, uid_t uid)
{
    ...
    node =
        recv_add_id(&uidlist, uidmap, uid, name); /* node keeps name's memory */
    return node->id2;
}
...
gid_t recv_group_name(int f, gid_t gid, uint16* flags_ptr)
{
    ...
    node =
        recv_add_id(&gidlist, gidmap, gid, name); /* node keeps name's memory */
...
void recv_id_list(int f, struct file_list* flist)
{
    ...
    if (preserve_gid && (!am_root || !numeric_ids || groupmap))
    {
        for (i = 0; i < flist->used; i++)
        {
            F_GROUP(flist->files[i]) =
                match_gid(F_GROUP(flist->files[i]), &flist->files[i]->flags);
        }
    }
}
...
void parse_name_map(char* map, BOOL usernames)
{
    ...
    recv_add_id(idlist_ptr, *idmap_ptr, 0,
                numeric_ids ? NULL
                            : usernames ? uid_to_user(0) : gid_to_group(0));
}
...
const char* getallgroups(uid_t uid, item_list* gid_list)
{
    ...
    if (getgrouplist(pw->pw_name, pw->pw_gid, gid_list->items, &size) < 0)
    {
        if (size > (int)gid_list->malloced)
        {
            ...
            if (getgrouplist(pw->pw_name, pw->pw_gid, gid_list->items, &size) <
                0)
                size = -1;
        }
        else
