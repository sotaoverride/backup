...
G_DEFINE_TYPE_WITH_CODE(
    GUnixVolumeMonitor, g_unix_volume_monitor, G_TYPE_NATIVE_VOLUME_MONITOR,
    g_io_extension_point_implement(G_NATIVE_VOLUME_MONITOR_EXTENSION_POINT_NAME,
                                   g_define_type_id, "unix", 0));

static void g_unix_volume_monitor_finalize(GObject* object)
{
    ...
    g_signal_handlers_disconnect_by_func(monitor->mount_monitor,
                                         mountpoints_changed, monitor);
    g_signal_handlers_disconnect_by_func(monitor->mount_monitor, mounts_changed,
                                         monitor);

    g_object_unref(monitor->mount_monitor);

    g_list_free_full(monitor->last_mountpoints,
...
static void g_unix_volume_monitor_dispose(GObject* object)
{
    GUnixVolumeMonitor* monitor;
...
static GMount* get_mount_for_mount_path(const char* mount_path,
...
static void g_unix_volume_monitor_class_init(GUnixVolumeMonitorClass* klass)
{
...
void _g_unix_volume_monitor_update(GUnixVolumeMonitor* unix_monitor)
{
...
static void g_unix_volume_monitor_init(GUnixVolumeMonitor* unix_monitor)
{

    unix_monitor->mount_monitor = g_unix_mount_monitor_get();

    g_signal_connect(unix_monitor->mount_monitor, "mounts-changed",
                     G_CALLBACK(mounts_changed), unix_monitor);

    g_signal_connect(unix_monitor->mount_monitor, "mountpoints-changed",
                     G_CALLBACK(mountpoints_changed), unix_monitor);
...
static void diff_sorted_lists(GList* list1, GList* list2, GCompareFunc compare,
...
GUnixVolume* _g_unix_volume_monitor_lookup_volume_for_mount_path(
    GUnixVolumeMonitor* monitor, const char* mount_path)
{
    GList* l;
...
static GUnixMount* find_mount_by_mountpath(GUnixVolumeMonitor* monitor,
...
static void update_volumes(GUnixVolumeMonitor* monitor)
{
    ...
    new_mountpoints =
        g_list_sort(new_mountpoints, (GCompareFunc)g_unix_mount_point_compare);

    diff_sorted_lists(monitor->last_mountpoints, new_mountpoints,
                      (GCompareFunc)g_unix_mount_point_compare, &added,
                      &removed);
    ...
    for (l = removed; l != NULL; l = l->next)
    {
        ...
        volume = _g_unix_volume_monitor_lookup_volume_for_mount_path(
            monitor, g_unix_mount_point_get_mount_path(mountpoint));
        if (volume)
        {
...
static void update_mounts(GUnixVolumeMonitor* monitor)
{
    ...
    diff_sorted_lists(monitor->last_mounts, new_mounts,
                      (GCompareFunc)g_unix_mount_compare, &added, &removed);

    for (l = removed; l != NULL; l = l->next)
    {
        GUnixMountEntry* mount_entry = l->data;

        mount = find_mount_by_mountpath(
            monitor, g_unix_mount_get_mount_path(mount_entry));
        if (mount)
        {
    ...
    for (l = added; l != NULL; l = l->next)
    {
        ...
        volume = _g_unix_volume_monitor_lookup_volume_for_mount_path(
            monitor, mount_path);
        mount =
            _g_unix_mount_new(G_VOLUME_MONITOR(monitor), mount_entry, volume);
        if (mount)
        {
