 ...
 * D-Bus Messages
 *
 * This encapsulates incoming and outgoing D-Bus messages. This is used to hold
 * the message data, the attached FDs and optional the cached metadata.
 */
...
static_assert(_DBUS_MESSAGE_FIELD_N <= 8 * sizeof(unsigned int),
              "Header fields exceed bitmap");
 ...
 * with header @header. Only the header is initialized, the backing memory for
 * the message payload is allocated, but not yet initialized.
 *
...
int message_new_outgoing(Message** messagep, void* data, size_t n_data)
{
    ...
    header->n_body =
        n_data - sizeof(MessageHeader) - c_align8(header->n_fields);
...
static int message_parse_header(Message* message, MessageMetadata* metadata)
{
    static const CDVarType type[] = {
        C_DVAR_T_INIT(
            C_DVAR_T_TUPLE7(C_DVAR_T_y, C_DVAR_T_y, C_DVAR_T_y, C_DVAR_T_y,
                            C_DVAR_T_u, C_DVAR_T_u,
                            C_DVAR_T_ARRAY(C_DVAR_T_TUPLE2(
                                C_DVAR_T_y, C_DVAR_T_v)))), /* (yyyyuua(yv)) */
    };
    ...
    c_dvar_begin_read(&v, message->big_endian, type, 1, message->header,
                      message->n_header);
    ...
    c_dvar_read(&v, "(yyyyuu[", NULL, &metadata->header.type,
                &metadata->header.flags, &metadata->header.version, NULL,
    ...
    while (c_dvar_more(&v))
    {
        ...
        switch (field)
        {
            ...
            case DBUS_MESSAGE_FIELD_PATH:
                ...
                if (!strcmp(metadata->fields.path,
            ...
            case DBUS_MESSAGE_FIELD_INTERFACE:
                c_dvar_read(&v, "<s>)", c_dvar_type_s,
                            &metadata->fields.interface);

                if (!strcmp(metadata->fields.interface,
                            "org.freedesktop.DBus.Local"))
                    return MESSAGE_E_INVALID_HEADER;

                if (!dbus_validate_interface(
                        metadata->fields.interface,
                        strlen(metadata->fields.interface)))
                    return MESSAGE_E_INVALID_HEADER;
            ...
            case DBUS_MESSAGE_FIELD_MEMBER:
                ...
                if (!dbus_validate_member(metadata->fields.member,
                                          strlen(metadata->fields.member)))
                    return MESSAGE_E_INVALID_HEADER;
            ...
            case DBUS_MESSAGE_FIELD_ERROR_NAME:
                c_dvar_read(&v, "<s>)", c_dvar_type_s,
                            &metadata->fields.error_name);

                if (!dbus_validate_error_name(
                        metadata->fields.error_name,
                        strlen(metadata->fields.error_name)))
                    return MESSAGE_E_INVALID_HEADER;

                break;

            case DBUS_MESSAGE_FIELD_REPLY_SERIAL:
                c_dvar_read(&v, "<u>)", c_dvar_type_u,
            ...
            case DBUS_MESSAGE_FIELD_DESTINATION:
                ...
                if (!dbus_validate_name(metadata->fields.destination,
                                        strlen(metadata->fields.destination)))
                    return MESSAGE_E_INVALID_HEADER;
            ...
            case DBUS_MESSAGE_FIELD_SENDER:
                c_dvar_read(&v, "<s>)", c_dvar_type_s,
                            &metadata->fields.sender);

                if (!dbus_validate_name(metadata->fields.sender,
                                        strlen(metadata->fields.sender)))
                    return MESSAGE_E_INVALID_HEADER;
...
static int message_parse_body(Message* message, MessageMetadata* metadata)
{
    ...
    c_dvar_begin_read(&v, message->big_endian, types, n_types, message->body,
                      message->n_body);
        ...
        switch (t->element)
        {
            ...
            case 'o':
                if (i < C_ARRAY_SIZE(metadata->args))
                {
                    ...
                    c_dvar_read(&v, (char[2]){t->element, 0},
                                &metadata->args[i].value);
                    metadata->n_args = i + 1;
                    break;
                }
 ...
 *
 * This parses the message, verifies its complience to the spec, and caches its
 * metadata. If the message contains more FDs than expected, the excess ones are
 * dropped, otherwise the message object is not altered.
 *
 * This method is idempotent.
 *
 * Return: 0 on success,
 *         MESSAGE_E_MISSING_FDS if the message contains fewer FDs than declared
 * in the metadata, MESSAGE_E_INVALID_HEADER if the header violates the spec in
 * other ways, MESSAGE_E_INVALID_BODY if the body could not be parsed.
 */
...
void message_stitch_sender(Message* message, uint64_t sender_id)
{
    ...
    static_assert(1 + 3 + 4 + ADDRESS_ID_STRING_MAX + 1 <=
                      sizeof(message->patch),
