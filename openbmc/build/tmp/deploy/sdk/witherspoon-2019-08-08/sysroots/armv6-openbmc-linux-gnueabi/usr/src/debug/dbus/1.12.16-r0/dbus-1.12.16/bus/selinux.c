...
static void log_audit_callback(void* data, security_class_t class, char* buf,
                               size_t bufleft);
...
static const struct avc_memory_callback mem_cb = {.func_malloc = dbus_malloc,
                                                  .func_free = dbus_free};
static const struct avc_log_callback log_cb = {
    .func_log = log_callback, .func_audit = log_audit_callback};
 ...
 */
static int policy_reload_callback(u_int32_t event, security_id_t ssid,
...
static struct security_class_mapping dbus_map[] = {
    {"dbus", {"acquire_svc", "send_msg", NULL}}, {NULL}};
#endif /* HAVE_SELINUX */
...
dbus_bool_t bus_selinux_full_init(void)
{
    ...
    {
        _dbus_warn("Failed to set up security class mapping "
                   "(selinux_set_mapping():%s).",
                   strerror(errno));
        return FALSE;
    }
    ...
    if (avc_add_callback(policy_reload_callback, AVC_CALLBACK_RESET, NULL, NULL,
                         0, 0) < 0)
    {
 ...
 */
#ifdef HAVE_SELINUX
static dbus_bool_t bus_selinux_check(BusSELinuxID* sender_sid,
                                     ...
                                     DBusString* auxdata)
{
    ...
    if (avc_has_perm(SELINUX_SID_FROM_BUS(sender_sid),
                     override_sid ? SELINUX_SID_FROM_BUS(override_sid)
                                  : bus_sid,
                                               ...
                                               DBusError* error)
{
    ...
    ret = bus_selinux_check(connection_sid, service_sid, SECCLASS_DBUS,
                            DBUS__ACQUIRE_SVC, &auxdata);
...
dbus_bool_t bus_selinux_allows_send(DBusConnection* sender,
                                    ...
                                    const char* msgtype, const char* interface,
                                    ...
                                    DBusError* error)
{
    ...
    if (!proposed_recipient ||
        !dbus_connection_get_unix_process_id(proposed_recipient, &tpid))
        tpid = 0;
    ...
    ret = bus_selinux_check(sender_sid, recipient_sid, SECCLASS_DBUS,
                            DBUS__SEND_MSG, &auxdata);
...
dbus_bool_t bus_selinux_append_context(DBusMessage* message, BusSELinuxID* sid,
                                       DBusError* error)
{
#ifdef HAVE_SELINUX
    char* context;
    ...
    if (!dbus_message_append_args(message, DBUS_TYPE_ARRAY, DBUS_TYPE_BYTE,
                                  &context, strlen(context), DBUS_TYPE_INVALID))
    {
        _DBUS_SET_OOM(error);
        return FALSE;
    }
...
#ifdef HAVE_SELINUX
static dbus_bool_t
    bus_connection_read_selinux_context(DBusConnection* connection, char** con)
{
    int fd;
                                             ...
                                             DBusError* error)
{
    ...
    if (avc_context_to_sid(con, &sid) < 0)
    {
        ...
        _dbus_warn("Error getting SID from context \"%s\": %s", con,
                   _dbus_strerror(errno));
...
{
    return _dbus_hash_table_new(DBUS_HASH_STRING, (DBusFreeFunction)dbus_free,
                                NULL);
}
                                        ...
                                        const char* service_context)
{
    ...
    if (!_dbus_hash_table_insert_string(service_table, key,
                                          ...
                                          const DBusString* service_name)
{
    ...
    sid = _dbus_hash_table_lookup_string(
        service_table, _dbus_string_get_const_data(service_name));
