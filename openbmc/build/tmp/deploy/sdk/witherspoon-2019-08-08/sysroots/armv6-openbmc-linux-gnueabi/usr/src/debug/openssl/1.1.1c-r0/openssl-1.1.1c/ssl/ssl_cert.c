...
CERT* ssl_cert_dup(CERT* cert)
{
    ...
    {
        ret->conf_sigalgs =
            OPENSSL_malloc(cert->conf_sigalgslen * sizeof(*cert->conf_sigalgs));
 ...
 * be included. \return 1 for success, 0 for failure. Note that in the case of
 * failure some certs may have been added to \c stack.
 */
...
int ssl_build_cert_chain(SSL* s, SSL_CTX* ctx, int flags)
{
    ...
    if (i <= 0)
    {
        ...
        ERR_add_error_data(2,
                           "Verify error:", X509_verify_cert_error_string(i));
