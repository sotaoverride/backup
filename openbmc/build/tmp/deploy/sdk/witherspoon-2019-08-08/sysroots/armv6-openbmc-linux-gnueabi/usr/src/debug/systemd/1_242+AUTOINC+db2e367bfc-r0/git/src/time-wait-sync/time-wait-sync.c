...
typedef struct ClockState
{
    ...
    sd_event_source*
        timerfd_event_source; /* non-null is the active io event source */
    int inotify_fd;
...
static int timerfd_handler(sd_event_source* s, int fd, uint32_t revents,
                           void* userdata)
{
...
static void process_inotify_event(sd_event* event, ClockState* sp,
                                  struct inotify_event* e)
{
    if (e->wd == sp->run_systemd_wd)
    {
        /* Only thing we care about is seeing if we can start watching
...
static int inotify_handler(sd_event_source* s, int fd, uint32_t revents,
                           void* userdata)
{
...
static int clock_state_update(ClockState* sp, sd_event* event)
{
    ...
    /* The kernel supports cancelling timers whenever its realtime clock is
     * "set" (which can happen in a variety of ways, generally adjustments of at
     * least 500 ms). The way this module works is we set up a timerfd that will
     * wake when the clock is set, and when that happens we read the clock
     * synchronization state from the return value of adjtimex(2), which
     * supports the NTP time adjustment protocol.
     *
     * The kernel determines whether the clock is synchronized using
     * driver-specific tests, based on time information passed by an
     * application, generally through adjtimex(2). If the application asserts
     * the clock is synchronized, but does not also do something that "sets the
     * clock", the timer will not be cancelled and synchronization will not be
     * detected.
     *
     * Similarly, this service will never complete if the application sets the
     * time without also providing information that adjtimex(2) can use to
     * determine that the clock is synchronized. This generally doesn't happen,
     * but can if the system has a hardware clock that is accurate enough that
     * the adjustment is too small to be a "set".
     *
     * Both these failure-to-detect situations are covered by having the
     * presence/creation of /run/systemd/timesync/synchronized, which is
     * considered sufficient to indicate a synchronized clock even if the kernel
     * has not been updated.
     *
     * For timesyncd the initial setting of the time uses settimeofday(2), which
     * sets the clock but does not mark it synchronized. When an NTP source is
     * selected it sets the clock again with clock_adjtime(2) which marks it
     * synchronized and also touches /run/systemd/timesync/synchronized which
     * covers the case when the clock wasn't "set". */
    ...
    {
        /* Not synchronized.  Do a one-shot wait on the descriptor and inform
         * the caller we need to keep running. */
