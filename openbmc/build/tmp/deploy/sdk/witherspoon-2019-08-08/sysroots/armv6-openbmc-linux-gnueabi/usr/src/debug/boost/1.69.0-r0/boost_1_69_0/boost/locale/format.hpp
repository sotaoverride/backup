...
///
/// This module provides printf like functionality integrated into iostreams and
...
struct formattible
{
    ...
    formattible(formattible const& other) :
        pointer_(other.pointer_), writer_(other.writer_)
    {
    }
...
{
  public:
    format_parser(std::ios_base& ios, void*,
    ...
    template <typename CharType>
    void set_flag_with_str(std::string const& key,
                           std::basic_string<CharType> const& value)
    {
...
///
/// \brief a printf like class that allows type-safe and locale aware message
/// formatting
///
/// This class creates a formatted message similar to printf or boost::format
/// and receives formatted entries via operator %.
///
/// For example
/// \code
...
/// \endcode
///
/// Formatting is enclosed between curly brackets \c { \c } and defined by a
/// comma separated list of flags in the format key[=value] value may also be
/// text included between single quotes \c ' that is used for special purposes
/// where inclusion of non-ASCII text is allowed
///
/// Including of literal \c { and \c } is possible by specifying double brackets
/// \c {{ and \c }} accordingly.
///
///
/// For example:
///
/// \code
///   cout << format("The height of water at {1,time} is
///   {2,num=fixed,precision=3}") % time % height;
/// \endcode
///
/// The special key -- a number without a value defines the position of an input
/// parameter. List of keys:
/// -   \c [0-9]+ -- digits, the index of a formatted parameter -- mandatory
/// key.
...
/// -  \c date, \c time , \c datetime or \c dt -- format date, time or date and
/// time. Optional values are:
...
///     .
/// -  \c ftime with string (quoted) parameter -- display as with \c strftime
/// see, \c as::ftime manipulator
...
/// -  \c locale -- with parameter -- switch locale for current operation. This
/// command generates locale
///     with formatting facets giving more fine grained control of formatting.
///     For example:
///    \code
///    cout << format("Today {1,date}
///    ({1,date,locale=he_IL.UTF-8@calendar=hebrew,date} Hebrew Date)") % date;
///    \endcode
/// -  \c timezone or \c tz -- the name of the timezone to display the time in.
/// For example:\n
///    \code
///    cout << format("Time is: Local {1,time}, ({1,time,tz=EET} Eastern
///    European Time)") % date; \endcode
...
///    {1,time,gmt}") % time; \endcode
///
///
/// Invalid formatting strings are slightly ignored. This would prevent from
/// translator to crash the program in unexpected location.
///
template <typename CharType>
class basic_format
{
  public:
    ...
    typedef basic_message<char_type>
        message_type; ///< The translation message type
    /// \cond INTERNAL
    typedef details::formattible<CharType> formattible_type;
    /// \endcond

    typedef std::basic_string<CharType>
        string_type; ///< string type for this type of character
    typedef std::basic_ostream<CharType>
        stream_type; ///< output stream type for this type of character

    ///
    /// Create a format class for \a format_string
    ///
    basic_format(string_type format_string) :
        format_(format_string), translate_(false), parameters_count_(0)
    {
    }
    ///
    /// Create a format class using message \a trans. The message if translated
    /// first according to the rules of target locale and then interpreted as
    /// format string
    ///
    basic_format(message_type const& trans) :
        message_(trans), translate_(true), parameters_count_(0)
    {
    }

    ///
    /// Add new parameter to format list. The object should be a type
    /// with defined expression out << object where \c out is \c
    /// std::basic_ostream.
    ///
  ...
  private:
    ...
    void format_output(stream_type& out, string_type const& sformat) const
    {
        ...
        while (format[pos] != 0)
        {
            ...
            details::format_parser fmt(out, static_cast<void*>(&out),
                                       &basic_format::imbue_locale);
...
///
/// This operator actually causes actual text formatting. It uses the locale of
/// \a out stream
///
template <typename CharType>
...
/// \example whello.cpp
///
/// Basic example of using various functions with wide strings provided by this
/// library
///
///
