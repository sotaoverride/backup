...
int scols_table_remove_columns(struct libscols_table* tb)
{
    ...
    {
        struct libscols_column* cl =
            list_entry(tb->tb_columns.next, struct libscols_column, cl_columns);
                            ...
                            struct libscols_column* cl)
{
    ...
    DBG(TAB, ul_debugobj(tb, "move column %zu behind %zu", cl->seqnum,
                         pre ? pre->seqnum : 0));
                                               ...
                                               int flags)
{
    ...
    DBG(TAB, ul_debugobj(tb, "new column name=%s, whint=%g, flags=%d", name,
                         whint, flags));
    cl = scols_new_column();
    if (!cl)
        return NULL;
 ...
 * will be reduced, this behavior affects subsequenced
 * scols_table_get_termwidth() calls.
 *
 * Returns: 0, a negative value in case of an error.
 */
 ...
 * @ln: line
 *
 * Note that this function does not destroy the parent<->child relationship
 * between lines. You have to call scols_line_remove_child()
 *
 * Returns: 0, a negative value in case of an error.
 */
int scols_table_remove_line(struct libscols_table* tb, struct libscols_line* ln)
{
    if (!tb || !ln)
        return -EINVAL;
 ...
 * @tb: table
 *
 * This empties the table and also destroys all the parent<->child
 * relationships.
 */
void scols_table_remove_lines(struct libscols_table* tb)
{
    ...
    {
        struct libscols_line* ln =
            list_entry(tb->tb_lines.next, struct libscols_line, ln_lines);
        if (ln->parent)
 ...
 * Returns: 0, a negative value in case of an error.
 */
int scols_table_next_line(struct libscols_table* tb, struct libscols_iter* itr,
                          struct libscols_line** ln)
{
    int rc = 1;
                                           ...
                                           struct libscols_line* parent)
{
    ...
    scols_unref_line(
        ln); /* ref-counter incremented by scols_table_add_line() */
    return ln;
err:
    scols_unref_line(ln);
    return NULL;
 ...
 */
struct libscols_line* scols_table_get_line(struct libscols_table* tb, size_t n)
{
    struct libscols_iter itr;
    struct libscols_line* ln;
...
int scols_table_set_default_symbols(struct libscols_table* tb)
{
    ...
    if (!scols_table_is_ascii(tb) && !strcmp(nl_langinfo(CODESET), "UTF-8"))
    {
 ...
 * default temporary symbols to draw output by
 * scols_table_set_default_symbols().
 *
...
int scols_table_enable_header_repeat(struct libscols_table* tb, int enable)
{
    if (!tb)
        return -EINVAL;
    DBG(TAB,
        ul_debugobj(tb, "header-repeat: %s", enable ? "ENABLE" : "DISABLE"));
 ...
 * @enable: 1 or 0
 *
 * Never continue on next line, remove last column(s) when too large, truncate
 * last column.
 *
 * Returns: 0 on success, negative number in case of an error.
 *
...
static int cells_cmp_wrapper_lines(struct list_head* a, struct list_head* b,
                                   void* data)
{
...
static int cells_cmp_wrapper_children(struct list_head* a, struct list_head* b,
                                      void* data)
{
    struct libscols_column* cl = (struct libscols_column*)data;
    struct libscols_line *ra, *rb;
...
static int sort_line_children(struct libscols_line* ln,
                              struct libscols_column* cl)
{
    struct list_head* p;
...
static struct libscols_line* move_line_and_children(struct libscols_line* ln,
                                                    struct libscols_line* pre)
{
    if (pre)
    {
        list_del_init(&ln->ln_lines); /* remove from old position */
        list_add(&ln->ln_lines,
                 &pre->ln_lines); /* add to the new place (behind @pre) */
    }
    pre = ln;
 ...
 * @tb: table
 *
 * Returns: SCOLS_TERMFORCE_{NEVER,ALWAYS,AUTO} or a negative value in case of
 * an error.
 *
 * Since: 2.29
 */
