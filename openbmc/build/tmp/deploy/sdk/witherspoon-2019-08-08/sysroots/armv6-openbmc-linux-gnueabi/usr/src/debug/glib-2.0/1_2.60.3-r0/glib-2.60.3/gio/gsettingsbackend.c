...
G_DEFINE_ABSTRACT_TYPE_WITH_PRIVATE(GSettingsBackend, g_settings_backend,
                                    G_TYPE_OBJECT)
...
{
    void (*function)(GObject* target, GSettingsBackend* backend,
                     const gchar* name, gpointer origin_tag, gchar** names);
...
static void g_settings_backend_watch_weak_notify(gpointer data,
...
void g_settings_backend_watch(GSettingsBackend* backend,
                              const GSettingsListenerVTable* vtable,
                              GObject* target, GMainContext* context)
{
...
static void g_settings_backend_dispatch_signal(GSettingsBackend* backend,
                                               ...
                                               const gchar* const* names)
{
    ...
    for (watch = backend->priv->watches; watch; watch = watch->next)
    {
        ...
        closure->function =
            G_STRUCT_MEMBER(void*, watch->vtable, function_offset);
 ...
 * Since: 2.26
 **/
void g_settings_backend_changed(GSettingsBackend* backend, const gchar* key,
                                gpointer origin_tag)
{
    ...
    g_settings_backend_dispatch_signal(
        backend, G_STRUCT_OFFSET(GSettingsListenerVTable, changed), key,
        origin_tag, NULL);
}
                                     ...
                                     gpointer origin_tag)
{
    ...
    g_settings_backend_dispatch_signal(
        backend, G_STRUCT_OFFSET(GSettingsListenerVTable, keys_changed), path,
        origin_tag, items);
}
                                     ...
                                     const gchar* path, gpointer origin_tag)
{
    ...
    g_settings_backend_dispatch_signal(
        backend, G_STRUCT_OFFSET(GSettingsListenerVTable, path_changed), path,
        origin_tag, NULL);
}
                                         ...
                                         const gchar* key)
{
    ...
    g_settings_backend_dispatch_signal(
        backend, G_STRUCT_OFFSET(GSettingsListenerVTable, writable_changed),
        key, NULL, NULL);
}
                                              ...
                                              const gchar* path)
{
    ...
    g_settings_backend_dispatch_signal(
        backend,
        G_STRUCT_OFFSET(GSettingsListenerVTable, path_writable_changed), path,
        NULL, NULL);
}
...
static gboolean g_settings_backend_flatten_one(gpointer key, gpointer value,
                                               gpointer user_data)
{
 ...
 * Since: 2.26
 **/
void g_settings_backend_flatten_tree(GTree* tree, gchar** path,
                                     const gchar*** keys, GVariant*** values)
{
    FlattenState state = {
        0,
    };
 ...
 * Since: 2.26
 **/
void g_settings_backend_changed_tree(GSettingsBackend* backend, GTree* tree,
                                     gpointer origin_tag)
{
    const gchar** keys;
    gchar* path;
 ...
 */
GVariant* g_settings_backend_read(GSettingsBackend* backend, const gchar* key,
                                  const GVariantType* expected_type,
                                  gboolean default_value)
{
    GVariant* value;

    value = G_SETTINGS_BACKEND_GET_CLASS(backend)->read(
    ...
    if
        G_UNLIKELY(value && !g_variant_is_of_type(value, expected_type))
        {
                                             ...
                                             const GVariantType* expected_type)
{
    ...
    value = G_SETTINGS_BACKEND_GET_CLASS(backend)->read_user_value(
        backend, key, expected_type);
 ...
 */
gboolean g_settings_backend_write(GSettingsBackend* backend, const gchar* key,
                                  GVariant* value, gpointer origin_tag)
{
    gboolean success;

    g_variant_ref_sink(value);
    success = G_SETTINGS_BACKEND_GET_CLASS(backend)->write(backend, key, value,
                                                           origin_tag);
    g_variant_unref(value);
 ...
 * old values.
 */
gboolean g_settings_backend_write_tree(GSettingsBackend* backend, GTree* tree,
                                       gpointer origin_tag)
{
    return G_SETTINGS_BACKEND_GET_CLASS(backend)->write_tree(backend, tree,
                                                             origin_tag);
}
 ...
 * unsets it.
 */
void g_settings_backend_reset(GSettingsBackend* backend, const gchar* key,
                              gpointer origin_tag)
{
...
static void g_settings_backend_variant_unref0(gpointer data)
{
    if (data != NULL)
...
{
    return g_tree_new_full((GCompareDataFunc)strcmp, NULL, g_free,
                           g_settings_backend_variant_unref0);
}

static gboolean g_settings_backend_verify(gpointer impl)
{
