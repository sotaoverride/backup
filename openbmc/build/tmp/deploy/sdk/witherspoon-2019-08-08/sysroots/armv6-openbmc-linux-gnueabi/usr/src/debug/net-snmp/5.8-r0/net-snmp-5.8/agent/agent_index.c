...
netsnmp_variable_list* register_oid_index(oid* name, size_t name_len,
...
netsnmp_variable_list* register_index(netsnmp_variable_list* varbind, int flags,
                                      netsnmp_session* ss)
{
    ...
    if (res == 0 && idxptr)
    {
        ...
        else
        {
                 ...
                 prev_idx_ptr = idxptr2, idxptr2 = idxptr2->next_idx)
            {
                ...
                {
                    case ASN_INTEGER:
                        res2 = (*varbind->val.integer -
                                *idxptr2->varbind->val.integer);
                        break;
                    case ASN_OCTET_STR:
                        i = SNMP_MIN(varbind->val_len,
                                     idxptr2->varbind->val_len);
                        res2 = memcmp(varbind->val.string,
                                      idxptr2->varbind->val.string, i);
                        break;
                    case ASN_OBJECT_ID:
                        res2 = snmp_oid_compare(
                            varbind->val.objid, varbind->val_len / sizeof(oid),
    ...
    if (NULL == snmp_varlist_add_variable(
                    &new_index->varbind, varbind->name, varbind->name_length,
                    varbind->type, varbind->val.string, varbind->val_len))
    {
        /*
...
netsnmp_feature_child_of(release_index, netsnmp_unused)
    ...
    int release_index(netsnmp_variable_list* varbind)
{
 ...
 */
int remove_index(netsnmp_variable_list* varbind, netsnmp_session* ss)
{
    return (unregister_index(varbind, FALSE, ss));
}
#endif /* NETSNMP_FEATURE_REMOVE_REMOVE_INDEX */

void unregister_index_by_session(netsnmp_session* ss)
{
...
int unregister_index(netsnmp_variable_list* varbind, int remember,
                     netsnmp_session* ss)
{
    struct snmp_index *idxptr, *idxptr2;
    struct snmp_index *prev_oid_ptr, *prev_idx_ptr;
         ...
         prev_idx_ptr = idxptr2, idxptr2 = idxptr2->next_idx)
    {
        switch (varbind->type)
        {
            ...
            case ASN_OCTET_STR:
                ...
                res2 = memcmp(varbind->val.string, idxptr2->varbind->val.string,
                              i);
                break;
            case ASN_OBJECT_ID:
                res2 = snmp_oid_compare(
...
netsnmp_feature_child_of(unregister_indexes, netsnmp_unused)
    ...
    int unregister_string_index(oid* name, size_t name_len, char* cp)
{
    netsnmp_variable_list varbind;
...
int unregister_int_index(oid* name, size_t name_len, int val)
{
    netsnmp_variable_list varbind;
...
int unregister_oid_index(oid* name, size_t name_len, oid* value,
                         size_t value_len)
{
    netsnmp_variable_list varbind;
                                ...
                                int include_unallocated)
{
    ...
    {
        if (netsnmp_oid_equals(name, namelen, i->varbind->name,
...
void test_string_register(int n, char* cp)
{
    varbind->name[4] = n;
...
void test_int_register(int n, int val)
{
    varbind->name[4] = n;
...
void test_oid_register(int n, int subid)
{
