...
static int component_length_quoted(const krb5_data* src, int flags)
{
    ...
    if ((flags & KRB5_PRINCIPAL_UNPARSE_DISPLAY) == 0)
    {
        ...
        for (j = 0; j < length; j++, cp++)
            if ((!no_realm && *cp == REALM_SEP) || *cp == COMPONENT_SEP ||
                *cp == '\0' || *cp == '\\' || *cp == '\t' || *cp == '\n' ||
                *cp == '\b')
                size++;
    }
...
static int copy_component_quoting(char* dest, const krb5_data* src, int flags)
{
    int j;
    const char* cp = src->data;
    char* q = dest;
...
static krb5_error_code k5_unparse_name(krb5_context context,
