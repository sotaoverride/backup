...
static void log_msg_dump(struct nl_object* a, struct nl_dump_params* p)
{
    ...
    if (msg->ce_mask & LOG_MSG_ATTR_INDEV)
    {
        if (link_cache)
            nl_dump(p, "IN=%s ",
                    rtnl_link_i2name(link_cache, msg->log_msg_indev, buf,
                                     sizeof(buf)));
        else
    ...
    if (msg->ce_mask & LOG_MSG_ATTR_PHYSINDEV)
    {
        if (link_cache)
            nl_dump(p, "PHYSIN=%s ",
                    rtnl_link_i2name(link_cache, msg->log_msg_physindev, buf,
                                     sizeof(buf)));
        else
    ...
    if (msg->ce_mask & LOG_MSG_ATTR_OUTDEV)
    {
        if (link_cache)
            nl_dump(p, "OUT=%s ",
                    rtnl_link_i2name(link_cache, msg->log_msg_outdev, buf,
                                     sizeof(buf)));
        else
    ...
    if (msg->ce_mask & LOG_MSG_ATTR_PHYSOUTDEV)
    {
        if (link_cache)
            nl_dump(p, "PHYSOUT=%s ",
                    rtnl_link_i2name(link_cache, msg->log_msg_physoutdev, buf,
                                     sizeof(buf)));
        else
    ...
    if (msg->ce_mask & LOG_MSG_ATTR_HWPROTO)
        nl_dump(
            p, "HWPROTO=%s ",
            nl_ether_proto2str(ntohs(msg->log_msg_hwproto), buf, sizeof(buf)));

    if (msg->ce_mask & LOG_MSG_ATTR_HOOK)
        nl_dump(p, "HOOK=%s ",
                nfnl_inet_hook2str(msg->log_msg_hook, buf, sizeof(buf)));
...
int nfnl_log_msg_set_payload(struct nfnl_log_msg* msg, uint8_t* payload,
                             int len)
{
    free(msg->log_msg_payload);
