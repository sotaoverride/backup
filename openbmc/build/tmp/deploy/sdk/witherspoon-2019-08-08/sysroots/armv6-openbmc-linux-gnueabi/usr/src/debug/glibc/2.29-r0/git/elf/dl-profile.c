...
#define SCALE_1_TO_1 0x10000L
    ...
    for (ph = GL(dl_profile_map)->l_phdr;
        ...
        if (ph->p_type == PT_LOAD && (ph->p_flags & PF_X))
        {
            ...
            ElfW(Addr) end =
                ((ph->p_vaddr + ph->p_memsz + GLRO(dl_pagesize) - 1) &
                 ~(GLRO(dl_pagesize) - 1));
    ...
    expected_size =
        (sizeof(struct gmon_hdr) + 4 + sizeof(struct gmon_hist_hdr) +
         kcountsize + 4 + 4 + fromssize * sizeof(struct here_cg_arc_record));
    ...
    *cp++ = '/';
    ...
    fd =
        __open64_nocancel(filename, O_RDWR | O_CREAT | O_NOFOLLOW, DEFFILEMODE);
    if (fd == -1)
    {
        char buf[400];
        int errnum;
    ...
    if (st.st_size == 0)
    {
        ...
        if (__lseek(fd, expected_size & ~(GLRO(dl_pagesize) - 1), SEEK_SET) ==
            -1)
        {
        cannot_create:
        ...
        if (TEMP_FAILURE_RETRY(__write_nocancel(
                fd, buf, (expected_size & (GLRO(dl_pagesize) - 1)))) < 0)
            goto cannot_create;
    }
    ...
    while (fromp->here->from_pc != frompc)
    {
        ...
        check_new_or_add:
            ...
            while (narcs != *narcsp && narcs < fromlimit)
            {
                ...
                size_t newfromidx;
                to_index =
                    (data[narcs].self_pc / (HASHFRACTION * sizeof(*tos)));
