...
/** Offset from start of _dbus_header_signature_str to the signature of the
 * fields array */
#define FIELDS_ARRAY_SIGNATURE_OFFSET 6
/** Offset from start of _dbus_header_signature_str to the signature of an
...
static void correct_header_padding(DBusHeader* header)
{
    ...
    if (!_dbus_string_align_length(&header->data, 8))
        _dbus_assert_not_reached(
            "couldn't pad header though enough padding was preallocated");
 ...
 */
static void _dbus_header_cache_one(DBusHeader* header, int field_code,
...
static void _dbus_header_cache_revalidate(DBusHeader* header)
{
    ...
    _dbus_type_reader_init(&reader, _dbus_header_get_byte_order(header),
                           &_dbus_header_signature_str,
                           FIELDS_ARRAY_SIGNATURE_OFFSET, &header->data,
    ...
    while (_dbus_type_reader_get_current_type(&array) != DBUS_TYPE_INVALID)
    {
        ...
        _dbus_assert(_dbus_type_reader_get_current_type(&sub) ==
                     DBUS_TYPE_BYTE);
 ...
 */
static dbus_bool_t _dbus_header_cache_known_nonexistent(DBusHeader* header,
                                                        int field)
{
    _dbus_assert(field <= DBUS_HEADER_FIELD_LAST);

    return (header->fields[field].value_pos ==
 ...
 */
static dbus_bool_t write_basic_field(DBusTypeWriter* writer, int field,
                                     int type, const void* value)
{
    ...
    if (!_dbus_type_writer_recurse(writer, DBUS_TYPE_STRUCT, NULL, 0, &sub))
        goto append_failed;

    field_byte = field;
    if (!_dbus_type_writer_write_basic(&sub, DBUS_TYPE_BYTE, &field_byte))
        goto append_failed;
    ...
    if (!_dbus_type_writer_recurse(&sub, DBUS_TYPE_VARIANT, &contained_type, 0,
                                   &variant))
        goto append_failed;
...
append_failed:
    _dbus_string_delete(writer->value_str, start,
                        _dbus_string_get_length(writer->value_str) - start -
                            padding);
    return FALSE;
}
 ...
 *
 * @param reader the reader (should be iterating over the array pointing at the
 * field to set)
 * @param field the header field
 * @param type the type of the value
 * @param value the value as for _dbus_marshal_set_basic()
 ...
 * @returns #FALSE if no memory
 */
static dbus_bool_t set_basic_field(DBusTypeReader* reader, int field, int type,
                                   const void* value,
...
void _dbus_header_set_serial(DBusHeader* header, dbus_uint32_t serial)
{
    ...
    _dbus_marshal_set_uint32(&header->data, SERIAL_OFFSET, serial,
...
{
    return _dbus_marshal_read_uint32(&header->data, SERIAL_OFFSET,
                                     _dbus_header_get_byte_order(header), NULL);
}
 ...
 * @returns #FALSE if not enough memory
 */
dbus_bool_t _dbus_header_create(DBusHeader* header, int byte_order,
                                int message_type, const char* destination,
                                ...
                                const char* member, const char* error_name)
{
    ...
    _dbus_assert(
        ((interface || message_type != DBUS_MESSAGE_TYPE_SIGNAL) && member) ||
        (error_name) || !(interface || member || error_name));
    ...
    _dbus_type_writer_init_values_only(
        &writer, byte_order, &_dbus_header_signature_str, 0, &header->data,
    ...
    if (!_dbus_type_writer_recurse(&writer, DBUS_TYPE_ARRAY,
                                   &_dbus_header_signature_str,
                                   FIELDS_ARRAY_SIGNATURE_OFFSET, &array))
        goto oom;
    ...
    if (path != NULL)
    {
        if (!write_basic_field(&array, DBUS_HEADER_FIELD_PATH,
    ...
    if (destination != NULL)
    {
        if (!write_basic_field(&array, DBUS_HEADER_FIELD_DESTINATION,
    ...
    if (interface != NULL)
    {
        if (!write_basic_field(&array, DBUS_HEADER_FIELD_INTERFACE,
    ...
    if (member != NULL)
    {
        if (!write_basic_field(&array, DBUS_HEADER_FIELD_MEMBER,
    ...
    if (error_name != NULL)
    {
        if (!write_basic_field(&array, DBUS_HEADER_FIELD_ERROR_NAME,
 ...
 * @returns #TRUE if the data is long enough for the claimed length, and the
 * lengths were valid
 */
dbus_bool_t _dbus_header_have_message_untrusted(
    int max_message_length, DBusValidity* validity, int* byte_order,
...

{
    ...
    *byte_order = _dbus_string_get_byte(str, start + BYTE_ORDER_OFFSET);
    ...
    fields_array_len_unsigned = _dbus_marshal_read_uint32(
        str, start + FIELDS_ARRAY_LENGTH_OFFSET, *byte_order, NULL);
    ...
    body_len_unsigned = _dbus_marshal_read_uint32(
        str, start + BODY_LENGTH_OFFSET, *byte_order, NULL);
...
static DBusValidity load_and_validate_field(DBusHeader* header, int field,
                                            DBusTypeReader* variant_reader)
{
    int type;
    int expected_type;
    ...
    dbus_uint32_t v_UINT32;
    int bad_string_code;
    dbus_bool_t (*string_validation_func)(const DBusString* str, int start,
                                          int len);
    ...
    {
        _dbus_assert_not_reached(
            "none of the known fields should have this type");
    }
    ...
    switch (field)
    {
        ...
        case DBUS_HEADER_FIELD_PATH:
            ...
            if (_dbus_string_equal_substring(
                    &_dbus_local_path_str, 0,
                    _dbus_string_get_length(&_dbus_local_path_str), value_str,
                    str_data_pos))
            {
    ...
    if (string_validation_func)
    {
        ...
        len = _dbus_marshal_read_uint32(
            value_str, value_pos, _dbus_header_get_byte_order(header), NULL);
 ...
 */
dbus_bool_t _dbus_header_load(DBusHeader* header, DBusValidationMode mode,
                              DBusValidity* validity, int byte_order,
                              int fields_array_len, int header_len,
                              int body_len, const DBusString* str, int start,
                              int len)
{
    int leftover;
    DBusValidity v;
    ...
    _dbus_type_reader_init(&reader, byte_order, &_dbus_header_signature_str, 0,
                           str, start);
    ...
    /* unknown message types are supposed to be ignored, so only validation here
     * is that it isn't invalid
     */
    ...
    /* BODY LENGTH */
    _dbus_assert(_dbus_type_reader_get_current_type(&reader) ==
                 DBUS_TYPE_UINT32);
    _dbus_assert(_dbus_type_reader_get_value_pos(&reader) ==
                 BODY_LENGTH_OFFSET);
    ...
    while (_dbus_type_reader_get_current_type(&array_reader) !=
           DBUS_TYPE_INVALID)
    {
        ...
        _dbus_assert(_dbus_type_reader_get_current_type(&array_reader) ==
                     DBUS_TYPE_STRUCT);

        _dbus_type_reader_recurse(&array_reader, &struct_reader);

        _dbus_assert(_dbus_type_reader_get_current_type(&struct_reader) ==
                     DBUS_TYPE_BYTE);
...
{
    _dbus_marshal_set_uint32(&header->data, BODY_LENGTH_OFFSET, body_len,
                             _dbus_header_get_byte_order(header));
}
 ...
 */
static dbus_bool_t find_field_for_modification(DBusHeader* header, int field,
                                               DBusTypeReader* reader,
                                               DBusTypeReader* realign_root)
{
    ...
    _dbus_type_reader_init(realign_root, _dbus_header_get_byte_order(header),
                           &_dbus_header_signature_str,
                           FIELDS_ARRAY_SIGNATURE_OFFSET, &header->data,
                           FIELDS_ARRAY_LENGTH_OFFSET);
 ...
 * @returns #FALSE if no memory
 */
dbus_bool_t _dbus_header_set_field_basic(DBusHeader* header, int field,
                                         int type, const void* value)
{
    _dbus_assert(field <= DBUS_HEADER_FIELD_LAST);
    ...
    if (_dbus_header_cache_check(header, field))
    {
        ...
        if (!find_field_for_modification(header, field, &reader, &realign_root))
            _dbus_assert_not_reached(
                "field was marked present in cache but wasn't found");
    ...
    else
    {
        ...
        _dbus_type_writer_init_values_only(
            &writer, _dbus_header_get_byte_order(header),
            &_dbus_header_signature_str, FIELDS_ARRAY_SIGNATURE_OFFSET,
            &header->data, FIELDS_ARRAY_LENGTH_OFFSET);

        /* recurse into array without creating a new length, and jump to
         * end of array.
         */
        if (!_dbus_type_writer_append_array(
                &writer, &_dbus_header_signature_str,
                FIELDS_ARRAY_ELEMENT_SIGNATURE_OFFSET, &array))
            _dbus_assert_not_reached(
                "recurse into ARRAY should not have used memory");
        ...
        if (!write_basic_field(&array, field, type, value))
            return FALSE;

        if (!_dbus_type_writer_unrecurse(&writer, &array))
            _dbus_assert_not_reached(
                "unrecurse from ARRAY should not have used memory");
    }
 ...
 * @returns #FALSE if the field doesn't exist
 */
dbus_bool_t _dbus_header_get_field_basic(DBusHeader* header, int field,
                                         int type, void* value)
{
    _dbus_assert(field != DBUS_HEADER_FIELD_INVALID);
    _dbus_assert(field <= DBUS_HEADER_FIELD_LAST);
    ...
    _dbus_marshal_read_basic(&header->data, header->fields[field].value_pos,
 ...
 * @returns #FALSE if the field doesn't exist
 */
dbus_bool_t _dbus_header_get_field_raw(DBusHeader* header, int field,
                                       const DBusString** str, int* pos)
{
    if (!_dbus_header_cache_check(header, field))
        return FALSE;
...
dbus_bool_t _dbus_header_delete_field(DBusHeader* header, int field)
{
    ...
    if (!_dbus_type_reader_delete(&reader, &realign_root))
        return FALSE;
    ...
    _dbus_assert(
        !_dbus_header_cache_check(header, field)); /* Expensive assertion ... */
 ...
 */
void _dbus_header_toggle_flag(DBusHeader* header, dbus_uint32_t flag,
                              dbus_bool_t value)
{
...
void _dbus_header_byteswap(DBusHeader* header, int new_order)
{
    ...
    _dbus_marshal_byteswap(&_dbus_header_signature_str, 0, byte_order,
                           new_order, &header->data, 0);
