...
#define IPV4LL_DONT_DESTROY(ll)                                                \
    _cleanup_(sd_ipv4ll_unrefp) _unused_ sd_ipv4ll* _dont_destroy_##ll =       \
        sd_ipv4ll_ref(ll)
...
int sd_ipv4ll_set_callback(sd_ipv4ll* ll, sd_ipv4ll_callback_t cb,
                           void* userdata)
{
    assert_return(ll, -EINVAL);
...
#define PICK_HASH_KEY                                                          \
    SD_ID128_MAKE(15, ac, 82, a6, d6, 3f, 49, 78, 98, 77, 5d, 0c, 69, 02, 94,  \
                  0b)
...
static int ipv4ll_pick_address(sd_ipv4ll* ll)
{
    ...
    (void)in_addr_to_string(AF_INET, &(union in_addr_union){.in.s_addr = addr},
                            &address);
...
#define MAC_HASH_KEY                                                           \
    SD_ID128_MAKE(df, 04, 22, 98, 3f, ad, 14, 52, f9, 87, 2e, d1, 9c, 70, e2,  \
                  f2)
...
static int ipv4ll_start_internal(sd_ipv4ll* ll, bool reset_generation)
{
    ...
    if (!ll->seed_set)
        ll->seed.value = htole64(
            siphash24(ll->mac.ether_addr_octet, ETH_ALEN, MAC_HASH_KEY.bytes));
    ...
    if (r < 0)
    {
        ...
        /* We couldn't start? If so, let's forget the picked address again, the
         * user might make a change and
         * retry, and we want the new data to take effect when picking an
         * address. */
        if (picked_address)
            ll->address = 0;
