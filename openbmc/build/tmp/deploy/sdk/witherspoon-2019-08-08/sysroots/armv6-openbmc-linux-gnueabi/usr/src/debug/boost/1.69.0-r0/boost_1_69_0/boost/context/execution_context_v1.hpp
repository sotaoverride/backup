...
struct BOOST_CONTEXT_DECL ecv1_activation_record
{
    ...
    ecv1_activation_record(fcontext_t fctx_, stack_context sctx_) noexcept :
        fctx{fctx_},
        sctx(sctx_), // sctx{ sctx_ } - clang-3.6: no viable conversion from
                     // 'boost::context::stack_context' to 'std::size_t'
        main_ctx{false}
    {
    }
...
class ecv1_capture_record : public ecv1_activation_record
{
  ...
  public:
    ecv1_capture_record(stack_context sctx, StackAlloc&& salloc,
                        fcontext_t fctx, ecv1_activation_record* caller,
                        Fn&& fn, Args&&... args) noexcept :
        ecv1_activation_record{fctx, sctx},
        salloc_{std::forward<StackAlloc>(salloc)}, fn_(std::forward<Fn>(fn)),
        args_(std::forward<Args>(args)...), caller_{caller}
    {
    }
...
#if defined(BOOST_NO_CXX17_STD_APPLY)
        boost::context::detail::apply(fn_,
                                      std::tuple_cat(args_, std::tie(data)));
#else
        std::apply(fn_, std::tuple_cat(args_, std::tie(data)));
#endif
        BOOST_ASSERT_MSG(
            !main_ctx,
            "main-context does not execute activation-record::run()");
    }
};
...
class BOOST_CONTEXT_DECL execution_context
{
  private:
    ...
    {
        detail::ecv1_data_t* dp =
            reinterpret_cast<detail::ecv1_data_t*>(t.data);
    ...
    template <typename StackAlloc, typename Fn, typename... Args>
    static detail::ecv1_activation_record*
        create_context(StackAlloc&& salloc, Fn&& fn, Args&&... args)
    {
        typedef detail::ecv1_capture_record<StackAlloc, Fn, Args...> capture_t;
...
#endif
        // create fast-context
        const detail::fcontext_t fctx = detail::make_fcontext(
            sp, size, &execution_context::entry_func<capture_t>);
        BOOST_ASSERT(nullptr != fctx);
        // get current activation record
        auto curr = execution_context::current().ptr_;
        // placment new for control structure on fast-context stack
        return ::new (sp) capture_t{sctx,
    ...
    template <typename StackAlloc, typename Fn, typename... Args>
    static detail::ecv1_activation_record*
        create_context(preallocated palloc, StackAlloc&& salloc, Fn&& fn,
                       Args&&... args)
    {
        typedef detail::ecv1_capture_record<StackAlloc, Fn, Args...> capture_t;
        ...
        BOOST_ASSERT(nullptr != sp);
        // calculate remaining size
        const std::size_t size = palloc.size - (static_cast<char*>(palloc.sp) -
                                                static_cast<char*>(sp));
#endif
        // create fast-context
        const detail::fcontext_t fctx = detail::make_fcontext(
        ...
        auto curr = execution_context::current().ptr_;
        // placment new for control structure on fast-context stack
        return ::new (sp) capture_t{palloc.sctx,
                                    std::forward<StackAlloc>(salloc),
                                    fctx,
                                    curr.get(),
                                    std::forward<Fn>(fn),
  ...
  public:
    ...
    execution_context(Fn&& fn, Args&&... args) :
        ...
        ptr_{create_context(segmented_stack(), std::forward<Fn>(fn),
                            std::forward<Args>(args)...)}
    {
    ...
    template <typename Fn, typename... Args>
    execution_context(std::allocator_arg_t, segmented_stack salloc, Fn&& fn,
                      Args&&... args) :
        // deferred execution of fn and its arguments
        // arguments are stored in std::tuple<>
        // non-type template parameter pack via std::index_sequence_for<>
        // preserves the number of arguments
        // used to extract the function arguments from std::tuple<>
        ptr_{create_context(salloc, std::forward<Fn>(fn),
                            std::forward<Args>(args)...)}
    {
        ptr_->resume(ptr_.get());
    }
    ...
    execution_context(std::allocator_arg_t, preallocated palloc,
                      segmented_stack salloc, Fn&& fn, Args&&... args) :
        // deferred execution of fn and its arguments
        // arguments are stored in std::tuple<>
        // non-type template parameter pack via std::index_sequence_for<>
        // preserves the number of arguments
        // used to extract the function arguments from std::tuple<>
        ptr_{create_context(palloc, salloc, std::forward<Fn>(fn),
                            std::forward<Args>(args)...)}
    {
        ptr_->resume(ptr_.get());
    }
    ...
    execution_context(Fn&& fn, Args&&... args) :
        ...
        // preserves the number of arguments
        // used to extract the function arguments from std::tuple<>
        ptr_{create_context(fixedsize_stack(), std::forward<Fn>(fn),
                            std::forward<Args>(args)...)}
    {
        ptr_->resume(ptr_.get());
    }
    ...
    execution_context(execution_context&& other) noexcept : ptr_{other.ptr_}
    {
        other.ptr_.reset();
    }
    ...
    {
        return ptr_->resume_ontop(vp, std::forward<Fn>(fn));
    }
...
inline void swap(execution_context& l, execution_context& r) noexcept
{
    l.swap(r);
}
