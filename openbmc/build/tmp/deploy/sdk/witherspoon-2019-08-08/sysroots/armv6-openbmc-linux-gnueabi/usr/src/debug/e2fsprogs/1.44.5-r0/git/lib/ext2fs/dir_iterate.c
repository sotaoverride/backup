...
errcode_t ext2fs_get_rec_len(ext2_filsys fs, struct ext2_dir_entry* dirent,
...
errcode_t ext2fs_set_rec_len(ext2_filsys fs, unsigned int len,
 ...
 * if not valid.
 */
static int ext2fs_validate_entry(ext2_filsys fs, char* buf, unsigned int offset,
...
struct xlate
{
    int (*func)(struct ext2_dir_entry* dirent, int offset, int blocksize,
...
static int xlate_func(ext2_ino_t dir EXT2FS_ATTR((unused)),
                      ...
                      struct ext2_dir_entry* dirent, int offset, int blocksize,
                      char* buf, void* priv_data)
{
...
errcode_t ext2fs_dir_iterate(ext2_filsys fs, ext2_ino_t dir, int flags,
                             char* block_buf,
                             ...
                             void* priv_data)
{
    ...
    return ext2fs_dir_iterate2(fs, dir, flags, block_buf, xlate_func, &xl);
}
 ...
 */
int ext2fs_process_dir_block(ext2_filsys fs, blk64_t* blocknr,
                             ...
                             void* priv_data)
{
    ...
    while (offset < buflen - 8)
    {
        ...
        if (ext2fs_get_rec_len(fs, dirent, &rec_len))
            return BLOCK_ABORT;
        if (((offset + rec_len) > buflen) || (rec_len < 8) ||
            ((rec_len % 4) != 0) ||
            ((ext2fs_dirent_name_len(dirent) + 8) > (int)rec_len))
        {
        ...
        if (!dirent->inode)
        {
             ...
             * should be blocksize.
             */
            if (!inline_data && (offset == buflen - csum_size) &&
        ...
        ret = (ctx->func)(
            ctx->dir, (next_real_entry > offset) ? DIRENT_DELETED_FILE : entry,
            dirent, offset, buflen, ctx->buf, ctx->priv_data);
    ...
    next:
        ...
        if (ctx->flags & DIRENT_FLAG_INCLUDE_REMOVED)
        {
            ...
            if (rec_len != size)
            {
                ...
                while (
                    offset < final_offset &&
                    !ext2fs_validate_entry(fs, ctx->buf, offset, final_offset))
                    offset += 4;
                continue;
            }
        }
