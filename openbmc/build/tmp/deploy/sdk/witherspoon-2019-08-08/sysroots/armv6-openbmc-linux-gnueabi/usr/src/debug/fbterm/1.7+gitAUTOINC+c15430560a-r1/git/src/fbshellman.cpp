...
{
    if (mShellList[mCurShell])
        delete mShellList[mCurShell];
}
...
void FbShellManager::historyScroll(bool down)
{
    if (mActiveShell)
    {
        mActiveShell->historyDisplay(false, down ? mActiveShell->h()
                                                 : -mActiveShell->h());
    }
}
...
#define STEP()                                                                 \
    ...
    for (u32 i = NR_SHELLS; i--;)
    {
        ...
        if ((shell == SHELL_ANY && mShellList[index]) ||
            shell == mShellList[index])
            break;
        STEP();
    }
...
void FbShellManager::childProcessExited(s32 pid)
{
    ...
    {
        if (mShellList[i] && mShellList[i]->childProcessExited(pid))
            break;
    }
}
