...
static void output_itemized_counts(const char* prefix, int* counts)
{
    ...
    if (total)
    {
        ...
        for (j = 0; j < 5; j++)
        {
            if (counts[j])
            {
                len += snprintf(buf + len, sizeof buf - len - 2, "%s%s: %s",
...
static const char* bytes_per_sec_human_dnum(void)
{
    if (starttime == (time_t)-1 || endtime == (time_t)-1)
        return "UNKNOWN";
    return human_dnum(
        (total_written + total_read) / (0.5 + (endtime - starttime)), 2);
}
...
static void output_summary(void)
{
    if (INFO_GTE(STATS, 2))
    {
        ...
        if (protocol_version >= 29)
            output_itemized_counts("Number of created files",
                                   &stats.created_files);
        if (protocol_version >= 31)
            output_itemized_counts("Number of deleted files",
                                   &stats.deleted_files);
        ...
        {
            rprintf(FINFO, "File list generation time: %s seconds\n",
                    comma_dnum((double)stats.flist_buildtime / 1000, 3));
            rprintf(FINFO, "File list transfer time: %s seconds\n",
    ...
    if (INFO_GTE(STATS, 1))
    {
        ...
        rprintf(
            FINFO, "total size is %s  speedup is %s%s\n",
            human_num(stats.total_size),
            comma_dnum((double)stats.total_size / (total_written + total_read),
                       2),
...
static void show_malloc_stats(void)
{
    ...
    rprintf(FINFO, RSYNC_NAME "[%d] (%s%s%s) heap statistics:\n", (int)getpid(),
            am_server ? "server " : "", am_daemon ? "daemon " : "", who_am_i());
...
static pid_t do_cmd(char* cmd, char* machine, char* user, char** remote_argv,
                    int remote_argc, int* f_in_p, int* f_out_p)
{
    int i, argc = 0;
...
static char* get_local_name(struct file_list* flist, char* dest_path)
{
    ...
    if ((statret = do_stat(dest_path, &st)) == 0)
    {
        ...
        if (file_total > 1)
        {
            rprintf(FERROR, "ERROR: destination must be a directory when"
                            " copying more than 1 file\n");
            exit_cleanup(RERR_FILESELECT);
        }
        if (file_total == 1 && S_ISDIR(flist->files[0]->mode))
        {
            rprintf(FERROR, "ERROR: cannot overwrite non-directory"
                            " with a directory\n");
            exit_cleanup(RERR_FILESELECT);
        }
    }
    ...
    *cp = '\0';
    if (!change_dir(dest_path, CD_NORMAL))
    {
        rsyserr(FERROR, errno, "change_dir#3 %s failed", full_fname(dest_path));
        exit_cleanup(RERR_FILESELECT);
    }
    *cp = '/';
...
static void read_final_goodbye(int f_in, int f_out)
{
    ...
    {
        i = read_ndx_and_attrs(f_in, f_out, &iflags, &fnamecmp_type, xname,
                               &xlen);
...
static void do_server_sender(int f_in, int f_out, int argc, char* argv[])
{
    ...
    if (!relative_paths)
    {
        ...
        {
            rsyserr(FERROR, errno, "change_dir#3 %s failed", full_fname(dir));
            exit_cleanup(RERR_FILESELECT);
        }
    }
    argc--;
    ...
    {
        /* Make sure input buffering is off so we can't hang in
         * noop_io_until_death(). */
...
static int do_recv(int f_in, int f_out, char* local_name)
{
    ...
    if (pid == 0)
    {
        ...
        write_int(f_out, NDX_DONE);
        send_msg(MSG_STATS, (char*)&stats.total_read, sizeof stats.total_read,
                 0);
        io_flush(FULL_FLUSH);
...
static void do_server_recv(int f_in, int f_out, int argc, char* argv[])
{
    ...
    if (argc > 0)
    {
        ...
        {
            rsyserr(FERROR, errno, "change_dir#4 %s failed", full_fname(dir));
            exit_cleanup(RERR_FILESELECT);
        }
    }
    ...
    if (sanitize_paths)
    {
        ...
        for (dir_p = basis_dir; *dir_p; dir_p++)
            *dir_p =
                sanitize_path(NULL, *dir_p, NULL, curr_dir_depth, SP_DEFAULT);
        if (partial_dir)
...
int client_run(int f_in, int f_out, pid_t pid, int argc, char* argv[])
{
    ...
    if (am_sender)
    {
        ...
        if (always_checksum && (log_format_has(stdout_format, 'C') ||
        ...
        if (protocol_version >= 30)
            ...
            io_start_buffering_out(f_out);
        if (protocol_version >= 31 ||
            (!filesfrom_host && protocol_version >= 23))
            io_start_multiplex_in(f_in);
        else
            io_start_buffering_in(f_in);
...
static int start_client(int argc, char* argv[])
{
    ...
    { /* for read_batch, NO source is specified */
        ...
        { /* source is remote */
            ...
            *argv = path;
            ...
            {
                rprintf(FERROR, "--files-from hostname is not the same as the "
                                "transfer hostname\n");
                exit_cleanup(RERR_SYNTAX);
            }
            am_sender = 0;
        ...
        { /* source is local, check dest arg */
            ...
            { /* no hostspec found, so src & dest are local */
                ...
                if (filesfrom_host)
                {
                    rprintf(FERROR, "--files-from cannot be remote when the "
                                    "transfer is local\n");
                    exit_cleanup(RERR_SYNTAX);
                }
                shell_machine = NULL;
            ...
            { /* hostspec was found, so dest is remote */
                argv[argc] = path;
                if (rsync_port)
                    daemon_over_rsh = shell_cmd ? 1 : -1;
            }
    ...
    if (!rsync_port && remote_argc &&
        !**remote_argv) /* Turn an empty arg into a dot dir. */
        *remote_argv = ".";
    ...
    else
    {
        ...
        {
            char* arg =
                check_for_hostspec(remote_argv[i], &dummy_host, &dummy_port);
            if (!arg)
            {
                rprintf(FERROR, "Unexpected local arg: %s\n", remote_argv[i]);
                rprintf(FERROR, "If arg is a remote file/dir, prefix it with a "
                                "colon (:).\n");
                exit_cleanup(RERR_SYNTAX);
            }
    ...
    if (daemon_over_rsh < 0)
        return start_socket_client(shell_machine, remote_argc, remote_argv,
                                   argc, argv);
    ...
    if (DEBUG_GTE(CMD, 2))
    {
        rprintf(FINFO, "cmd=%s machine=%s user=%s path=%s\n", NS(shell_cmd),
                NS(shell_machine), NS(shell_user), NS(remote_argv[0]));
    }
    ...
    {
        int tmpret;
        tmpret = start_inband_exchange(f_in, f_out, shell_user, remote_argc,
                                       remote_argv);
        if (tmpret < 0)
            return tmpret;
    }
...
/**
 * Handle a fatal signal by launching a debugger, controlled by
 *$RSYNC_PANIC_ACTION.
 *
...
int main(int argc, char* argv[])
{
    ...
    if ((write_batch || read_batch) && !am_server)
    {
        ...
        else
        {
            batch_fd =
                do_open(batch_name,
                        write_batch ? O_WRONLY | O_CREAT | O_TRUNC : O_RDONLY,
                        S_IRUSR | S_IWUSR);
        }
        if (batch_fd < 0)
        {
