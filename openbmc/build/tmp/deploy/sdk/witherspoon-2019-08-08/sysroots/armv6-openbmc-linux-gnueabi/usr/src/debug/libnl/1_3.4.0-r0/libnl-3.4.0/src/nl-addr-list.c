...
static void print_usage(void)
{
    printf("Usage: nl-addr-list [OPTION]... [ADDRESS]\n"
           ...
           "     --env                 Print address details in sh env "
           "variable syntax.\n"
           ...
           "     --valid-lifetime=TS   Valid lifetime before route expires "
           "(IPv6).\n"
...
static void env_dump(struct nl_object* obj, void* arg)
{
    ...
    if (link_cache)
        nl_dump_line(p, "%s_IFNAME=%s\n", pfx,
                     rtnl_link_i2name(link_cache, rtnl_addr_get_ifindex(addr),
                                      buf, sizeof(buf)));

    if ((a = rtnl_addr_get_peer(addr)))
        nl_dump_line(p, "%s_PEER=%s\n", pfx, nl_addr2str(a, buf, sizeof(buf)));
...
int main(int argc, char* argv[])
{
    ...
    for (;;)
    {
        ...
        static struct option long_opts[] = {{"details", 0, 0, ARG_DETAILS},
                                            {"env", 0, 0, ARG_ENV},
        ...
        switch (c)
        {
            ...
            case 'v':
                ...
                break;
            case 'a':
                nl_cli_addr_parse_local(addr, optarg);
                break;
            case 'd':
                nl_cli_addr_parse_dev(addr, link_cache, optarg);
                break;
            case ARG_FAMILY:
                nl_cli_addr_parse_family(addr, optarg);
                break;
            case ARG_LABEL:
                nl_cli_addr_parse_label(addr, optarg);
                break;
            case ARG_PEER:
                nl_cli_addr_parse_peer(addr, optarg);
                break;
            case ARG_SCOPE:
                nl_cli_addr_parse_scope(addr, optarg);
                break;
            case ARG_BROADCAST:
                nl_cli_addr_parse_broadcast(addr, optarg);
                break;
            case ARG_PREFERRED:
                nl_cli_addr_parse_preferred(addr, optarg);
                break;
            case ARG_VALID:
                nl_cli_addr_parse_valid(addr, optarg);
                break;
        }
    }

    if (dump_env)
        nl_cache_foreach_filter(addr_cache, OBJ_CAST(addr), env_dump, &params);
    else
