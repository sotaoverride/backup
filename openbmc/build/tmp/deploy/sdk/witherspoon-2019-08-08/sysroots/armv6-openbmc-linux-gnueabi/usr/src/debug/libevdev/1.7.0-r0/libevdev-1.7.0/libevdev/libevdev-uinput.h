 ...
 *
 * Creation of uinput devices based on existing libevdev devices. These
 * functions help to create uinput devices that emulate libevdev devices. In the
 * simplest form it serves to duplicate an existing device:
 *
 * @code
 * int err;
 * int fd, new_fd, uifd;
 ...
 * will be an exact copy of the libevdev device, minus the bits that uinput
 * doesn't allow to be set.
 *
 * If uinput_fd is @ref LIBEVDEV_UINPUT_OPEN_MANAGED,
 * libevdev_uinput_create_from_device() will open @c /dev/uinput in read/write
 * mode and manage the file descriptor. Otherwise, uinput_fd must be opened by
 * the caller and opened with the appropriate permissions.
 *
 ...
 * A file descriptor can only create one uinput device at a time; the second
 * device will fail with -EINVAL.
 *
 ...
 * @param uinput_fd @ref LIBEVDEV_UINPUT_OPEN_MANAGED or a file descriptor to @c
 * /dev/uinput,
 ...
 * fd pointing to <strong>/dev/uinput</strong>. This file descriptor may be used
 * to write events that are emitted by the uinput device. Closing this file
 * descriptor will destroy the uinput device, you should call
