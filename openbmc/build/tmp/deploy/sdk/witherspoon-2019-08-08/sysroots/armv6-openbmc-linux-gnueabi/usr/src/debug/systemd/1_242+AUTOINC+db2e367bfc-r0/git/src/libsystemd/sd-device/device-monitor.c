...
int device_monitor_allow_unicast_sender(sd_device_monitor* m,
                                        sd_device_monitor* sender)
{
...
_public_ int sd_device_monitor_set_receive_buffer_size(sd_device_monitor* m,
                                                       size_t size)
{
    int r, n = (int)size;
...
int device_monitor_new_full(sd_device_monitor** ret, MonitorNetlinkGroup group,
                            int fd)
{
    ...
    if (group == MONITOR_GROUP_UDEV && access("/run/udev/control", F_OK) < 0 &&
        dev_is_devtmpfs() <= 0)
    {
        ...
        log_debug("sd-device-monitor: The udev service seems not to be active, "
    ...
    if (fd < 0)
    {
        sock = socket(PF_NETLINK, SOCK_RAW | SOCK_CLOEXEC | SOCK_NONBLOCK,
                      NETLINK_KOBJECT_UEVENT);
        if (sock < 0)
            return log_debug_errno(
                errno, "sd-device-monitor: Failed to create socket: %m");
    }
    ...
    *m = (sd_device_monitor){
    ...
    if (fd >= 0)
    {
        ...
        if (r < 0)
            return log_debug_errno(
                r, "sd-device-monitor: Failed to set netlink address: %m");
    }
...
static int device_monitor_event_handler(sd_event_source* s, int fd,
                                     ...
                                     void* userdata)
{
    ...
    r = sd_event_add_io(m->event, &m->event_source, m->sock, EPOLLIN,
                        device_monitor_event_handler, m);
    if (r < 0)
        return r;
...
_public_ int sd_device_monitor_attach_event(sd_device_monitor* m,
                                            sd_event* event)
{
    int r;
...
_public_ sd_event_source*
    sd_device_monitor_get_event_source(sd_device_monitor* m)
{
    assert_return(m, NULL);
...
int device_monitor_enable_receiving(sd_device_monitor* m)
{
    ...
    if (r < 0)
        return log_debug_errno(
            r, "sd-device-monitor: Failed to update filter: %m");
    ...
    if (!m->bound)
    {
        ...
        if (r < 0)
            return log_debug_errno(r, "sd-device-monitor: Failed to set socket "
                                      "option SO_PASSCRED: %m");
        ...
        if (r < 0)
            return log_debug_errno(
                r, "sd-device-monitor: Failed to set address: %m");
    }
...
DEFINE_PUBLIC_TRIVIAL_REF_UNREF_FUNC(sd_device_monitor, sd_device_monitor,
                                     device_monitor_free);
...
int device_monitor_receive_device(sd_device_monitor* m, sd_device** ret)
{
    ...
    else if (snl.nl.nl_groups == MONITOR_GROUP_KERNEL)
    {
        if (snl.nl.nl_pid > 0)
            return log_debug_errno(SYNTHETIC_ERRNO(EAGAIN),
                                   "sd-device-monitor: Multicast kernel "
                                   "netlink message from PID %" PRIu32
                                   " ignored.",
                                   snl.nl.nl_pid);
    }
    ...
    if (!cmsg || cmsg->cmsg_type != SCM_CREDENTIALS)
        return log_debug_errno(SYNTHETIC_ERRNO(EAGAIN),
                               "sd-device-monitor: No sender credentials "
    ...
    r = device_new_from_nulstr(&device, (uint8_t*)&buf.raw[bufpos],
                               buflen - bufpos);
    if (r < 0)
        return log_debug_errno(r, "sd-device-monitor: Failed to create device "
                                  "from received message: %m");
    ...
    if (r < 0)
        return log_device_debug_errno(device, r,
                                      "sd-device-monitor: Failed to check "
                                      "received device passing filter: %m");
    if (r == 0)
        log_device_debug(device, "sd-device-monitor: Received device does not "
                                 "pass filter, ignoring");
    else
        *ret = TAKE_PTR(device);
...
int device_monitor_send_device(sd_device_monitor* m,
                               ...
                               sd_device* device)
{
    ...
    if (blen < 32)
    {
        log_device_debug(device,
                         "sd-device-monitor: Length of device property nulstr "
                         "is too small to contain valid device information");
        return -EINVAL;
    }
    ...
    if (count < 0)
    {
        if (!destination && errno == ECONNREFUSED)
        {
            ...
            return log_device_debug_errno(device, errno,
                                          "sd-device-monitor: Failed to send "
                                          "device to netlink monitor: %m");
    }

    log_device_debug(
        device, "sd-device-monitor: Passed %zi byte to netlink monitor", count);
    return count;
}

static void bpf_stmt(struct sock_filter* ins, unsigned* i, unsigned short code,
                     unsigned data)
{
...
static void bpf_jmp(struct sock_filter* ins, unsigned* i, unsigned short code,
                    unsigned data, unsigned short jt, unsigned short jf)
{
    ins[(*i)++] = (struct sock_filter){
        .code = code,
        .jt = jt,
...
_public_ int sd_device_monitor_filter_update(sd_device_monitor* m)
{
    ...
    bpf_stmt(ins, &i, BPF_LD | BPF_W | BPF_ABS,
             offsetof(monitor_netlink_header, magic));
    ...
    if (!set_isempty(m->tag_filter))
    {
        ...
        SET_FOREACH(tag, m->tag_filter, it)
        {
            ...
            tag_matches--;
            bpf_jmp(ins, &i, BPF_JMP | BPF_JEQ | BPF_K, tag_bloom_lo,
                    1 + (tag_matches * 6), 0);
        }
    ...
    if (setsockopt(m->sock, SOL_SOCKET, SO_ATTACH_FILTER, &filter,
                   sizeof(filter)) < 0)
        return -errno;
...
_public_ int sd_device_monitor_filter_add_match_subsystem_devtype(
    sd_device_monitor* m, const char* subsystem, const char* devtype)
{
...
_public_ int sd_device_monitor_filter_add_match_tag(sd_device_monitor* m,
                                                    const char* tag)
{
...
_public_ int sd_device_monitor_filter_remove(sd_device_monitor* m)
{
    ...
    if (setsockopt(m->sock, SOL_SOCKET, SO_DETACH_FILTER, &filter,
                   sizeof(filter)) < 0)
        return -errno;
