...
static uint32_t _hash(uint64_t key)
{
    key = (~key) + (key << 18);
    key = key ^ (key >> 31);
    key = key * 21;
...
static void _set(ino64_t inode, pid_t pid)
{
    ...
    for (i = 0; i < INODE_PID_TABLE_MAX_COLLISIONS; i++)
    {
        ...
        /* Check if this entry is empty, or the actual inode we were looking
         * for.*/
        /* The second part should never happen, but it is here for
         * completeness.*/
    ...
    /* We will silently fail to insert the inode if we get too many
     * collisions.*/
...
void netsnmp_get_pid_from_inode_init(void)
{
    ...
    while ((procinfo = readdir(procdirs)) != NULL)
    {
        ...
        filelen = snprintf(path_name, PATH_MAX, PROC_PATH "/%s/fd/",
                           procinfo->d_name);
...
pid_t netsnmp_get_pid_from_inode(ino64_t inode)
{
    return _get(inode);
}
