                                   ...
                                   uint8_t command, struct nl_msg** result)
{
    ...
    msg = nfnlmsg_alloc_simple(NFNL_SUBSYS_QUEUE, NFQNL_MSG_CONFIG, 0, family,
                               queuenum);
    if (msg == NULL)
        return -NLE_NOMEM;
                                    ...
                                    struct nl_msg** result)
{
    ...
    msg = nfnlmsg_alloc_simple(NFNL_SUBSYS_QUEUE, NFQNL_MSG_CONFIG, 0, 0,
                               nfnl_queue_get_group(queue));
    if (msg == NULL)
        return -NLE_NOMEM;
