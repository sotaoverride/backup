...
static gboolean variant_type_string_scan_internal(const gchar* string,
                                                  const gchar* limit,
                                                  const gchar** endptr,
                                                  gsize* depth,
                                                  gsize depth_limit)
{
    ...
    switch (*string++)
    {
        case '(':
            ...
            {
                if (depth_limit == 0 ||
                    !variant_type_string_scan_internal(
                        string, limit, &string, &child_depth, depth_limit - 1))
                    return FALSE;
 ...
 * Since: 2.24
 **/
gboolean g_variant_type_string_scan(const gchar* string, const gchar* limit,
                                    const gchar** endptr)
{
...
gsize g_variant_type_n_items(const GVariantType* type)
{
    ...
    for (type = g_variant_type_first(type); type;
 ...
 **/
static GVariantType*
    g_variant_type_new_tuple_slow(const GVariantType* const* items, gint length)
{
                                       ...
                                       gint length)
{
    ...
    if (length < 0)
        for (length_unsigned = 0; items[length_unsigned] != NULL;
             length_unsigned++)
            ;
    else
