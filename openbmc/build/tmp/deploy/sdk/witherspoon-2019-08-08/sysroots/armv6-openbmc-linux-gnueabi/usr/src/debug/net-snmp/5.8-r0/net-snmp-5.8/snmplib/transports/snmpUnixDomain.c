...
static char* netsnmp_unix_fmtaddr(netsnmp_transport* t, const void* data,
                                  int len)
{
...
static void netsnmp_unix_get_taddr(netsnmp_transport* t, void** addr,
                                   size_t* addr_len)
{
...
static int netsnmp_unix_recv(netsnmp_transport* t, void* buf, int size,
...
static int netsnmp_unix_send(netsnmp_transport* t, const void* buf, int size,
                             void** opaque, int* olength)
{
    int rc = -1;
...
static int netsnmp_unix_close(netsnmp_transport* t)
{
    int rc = 0;
...
static int netsnmp_unix_accept(netsnmp_transport* t)
{
                                          ...
                                          int local)
{
    ...
    */
    ...
    {
        char* str =
            netsnmp_unix_fmtaddr(NULL, addr, sizeof(struct sockaddr_un));
        DEBUGMSGTL(
            ("netsnmp_unix", "open %s %s\n", local ? "local" : "remote", str));
        free(str);
    }
...
netsnmp_transport* netsnmp_unix_create_tstring(const char* string, int local,
...
netsnmp_transport* netsnmp_unix_create_ostring(const void* ostring,
...
int netsnmp_unix_getSecName(void* opaque, int olength, const char* community,
                            ...
                            const char** contextName)
{
    ...
    {
        DEBUGMSGTL(
            ("netsnmp_unix_getSecName", "no unix destine address in PDU?\n"));
        return 1;
    }
...
void netsnmp_unix_parse_security(const char* token, char* param)
{
...

    {
        ...
        DEBUGMSGTL(("netsnmp_unix_parse_security",
                    "<\"%s\", \"%.*s\"> => \"%s\"\n", community,
                    (int)sockpathLen, sockpath, secName));
...
void netsnmp_unix_com2SecList_free(void)
{
