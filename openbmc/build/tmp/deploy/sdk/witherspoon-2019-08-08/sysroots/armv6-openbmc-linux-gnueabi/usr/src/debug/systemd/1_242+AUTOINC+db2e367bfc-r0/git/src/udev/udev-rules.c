...
struct UdevRules
{
    ...
    /* all key strings are copied and de-duplicated in a single continuous
     * string buffer */
...
static const char* operation_str(enum operation_type type)
{
    static const char* const operation_strs[] = {
        [OP_UNSET] = "UNSET",     [OP_MATCH] = "match",
        [OP_NOMATCH] = "nomatch", [OP_MATCH_MAX] = "MATCH_MAX",

        [OP_ADD] = "add",         [OP_REMOVE] = "remove",
        [OP_ASSIGN] = "assign",   [OP_ASSIGN_FINAL] = "assign-final",
    };
    ...
    switch (type)
    {
        case TK_RULE:
        {
            ...
            log_debug("* RULE %s:%u, token: %u, count: %u, label: '%s'",
                      &rules->strbuf->buf[token->rule.filename_off],
                      token->rule.filename_line, idx, token->rule.token_count,
        ...
        case TK_A_RUN_PROGRAM:
            log_debug("%s %s '%s'(%s)", token_str(type), operation_str(op),
                      value, string_glob_str(glob));
            break;
        case TK_M_IMPORT_BUILTIN:
            log_debug("%s %i '%s'", token_str(type), token->key.builtin_cmd,
                      value);
            break;
        case TK_M_ATTR:
        case TK_M_SYSCTL:
        ...
        case TK_A_SYSCTL:
        case TK_A_ENV:
            log_debug("%s %s '%s' '%s'(%s)", token_str(type), operation_str(op),
                      attr, value, string_glob_str(glob));
            break;
        case TK_M_TAG:
        case TK_A_TAG:
        ...
        case TK_A_DB_PERSIST:
            ...
            break;
        case TK_M_TEST:
            log_debug("%s %s '%s'(%s) %#o", token_str(type), operation_str(op),
                      value, string_glob_str(glob), token->key.mode);
            break;
        ...
        case TK_A_DEVLINK_PRIO:
            ...
            break;
        case TK_A_OWNER_ID:
            log_debug("%s %s %u", token_str(type), operation_str(op),
                      token->key.uid);
            break;
        case TK_A_GROUP_ID:
            log_debug("%s %s %u", token_str(type), operation_str(op),
            ...
            break;
        case TK_A_MODE_ID:
            log_debug("%s %s %#o", token_str(type), operation_str(op),
                      token->key.mode);
            break;
        ...
        case TK_A_SECLABEL:
            ...
            break;
        case TK_A_GOTO:
            log_debug("%s '%s' %u", token_str(type), value,
                      token->key.rule_goto);
            break;
        case TK_END:
            log_debug("* %s", token_str(type));
...
static void dump_rules(UdevRules* rules)
{
    ...
    log_debug("Dumping %zu (%zu bytes) tokens, %zu (%zu bytes) strings",
              rules->token_cur, rules->token_cur * sizeof(struct token),
              rules->strbuf->nodes_count, rules->strbuf->len);
...
static void log_unknown_owner(sd_device* dev, int error, const char* entity,
                              const char* owner)
{
    if (IN_SET(abs(error), ENOENT, ESRCH))
        log_device_error(dev, "Specified %s '%s' unknown", entity, owner);
    else
        log_device_error_errno(dev, error, "Failed to resolve %s '%s': %m",
                               entity, owner);
}
    ...
    if (IN_SET(val[0], '"', '\''))
    {
        if (len == 1 || val[len - 1] != val[0])
            return log_debug_errno(SYNTHETIC_ERRNO(EINVAL),
                                   "Inconsistent quoting: '%s', skip", line);
        val[len - 1] = '\0';
        val++;
    }
...
static int import_program_into_properties(UdevEvent* event, usec_t timeout_usec,
                                          const char* program)
{
    ...
    r = udev_event_spawn(event, timeout_usec, true, program, result,
                         sizeof result);
    if (r < 0)
        return r;
    if (r > 0)
...
static int get_key(char** line, char** key, enum operation_type* op,
                   char** value)
{
    char* linepos;
    char* temp;
...
static int rule_add_key(struct rule_tmp* rule_tmp, enum token_type type,
                        enum operation_type op, const char* value,
                        const void* data)
{
...
#define LOG_RULE_FULL(level, fmt, ...)                                         \
    log_full(level, "%s:%u: " fmt, filename, lineno, ##__VA_ARGS__)
#define LOG_RULE_ERROR(fmt, ...) LOG_RULE_FULL(LOG_ERR, fmt, ##__VA_ARGS__)
#define LOG_RULE_WARNING(fmt, ...)                                             \
    LOG_RULE_FULL(LOG_WARNING, fmt, ##__VA_ARGS__)
#define LOG_RULE_DEBUG(fmt, ...) LOG_RULE_FULL(LOG_DEBUG, fmt, ##__VA_ARGS__)
#define LOG_AND_RETURN(fmt, ...)                                               \
    {                                                                          \
        LOG_RULE_ERROR(fmt, __VA_ARGS__);                                      \
        return;                                                                \
    }
#define LOG_AND_RETURN_ADD_KEY                                                 \
    LOG_AND_RETURN("Temporary rule array too small, aborting event "           \
                   "processing with %zu items",                                \
                   rule_tmp.token_cur);

static void add_rule(UdevRules* rules, char* line, const char* filename,
                     unsigned filename_off, unsigned lineno)
{
    char* linepos;
    const char* attr;
    ...
    for (;;)
    {
        ...
        if (get_key(&linepos, &key, &op, &value) < 0)
        {
            ...
            if (!strchr(NEWLINE, *linepos))
            {
                ...
                tmp = cescape(buf);
                LOG_RULE_ERROR(
                    "Invalid key/value pair, starting at character %tu ('%s')",
                    linepos - line + 1, tmp);
                if (*linepos == '#')
                    LOG_RULE_ERROR(
                        "Hint: comments can only start at beginning of line");
            }
            break;
        }
        ...
        else if (streq(key, "SUBSYSTEM"))
        {
            ...
            if (STR_IN_SET(value, "subsystem", "bus", "class"))
            {
                if (!streq(value, "subsystem"))
                    LOG_RULE_WARNING(
                        "'%s' must be specified as 'subsystem'; please fix",
                        value);

                r = rule_add_key(&rule_tmp, TK_M_SUBSYSTEM, op,
        ...
        else if (startswith(key, "ATTRS{"))
        {
            ...
            if (startswith(attr, "device/"))
                LOG_RULE_WARNING("'device' link may not be available in future "
                                 "kernels; please fix");
            if (strstr(attr, "../"))
                LOG_RULE_WARNING("Direct reference to parent sysfs directory, "
                                 "may break in future kernels; please fix");
        ...
        else if (startswith(key, "ENV{"))
        {
            ...
            {
                if (STR_IN_SET(attr, "ACTION", "SEQNUM", "SUBSYSTEM", "DEVTYPE",
                               "MAJOR", "MINOR", "DRIVER", "IFINDEX", "DEVNAME",
                               "DEVLINKS", "DEVPATH", "TAGS"))
                    LOG_AND_RETURN("Invalid ENV attribute, '%s' cannot be set",
                                   attr);
        ...
        else if (startswith(key, "IMPORT"))
        {
            ...
            if (streq(attr, "program"))
            {
                ...
                if (value[0] != '/')
                {
                    const enum udev_builtin_cmd cmd =
                        udev_builtin_lookup(value);

                    if (cmd >= 0)
                    {
                        LOG_RULE_DEBUG("IMPORT found builtin '%s', replacing",
                                       value);
                        if (rule_add_key(&rule_tmp, TK_M_IMPORT_BUILTIN, op,
                                         value, &cmd) < 0)
                            LOG_AND_RETURN_ADD_KEY;
                        continue;
                    }
            ...
            else if (streq(attr, "builtin"))
            {
                ...
                if (cmd < 0)
                {
                    LOG_RULE_WARNING("IMPORT{builtin} '%s' unknown, ignoring",
                                     value);
                    continue;
                }
                else
                    r = rule_add_key(&rule_tmp, TK_M_IMPORT_BUILTIN, op, value,
                                     &cmd);
            }
                ...
                r = rule_add_key(&rule_tmp, TK_M_IMPORT_CMDLINE, op, value,
                                 NULL);
            else if (streq(attr, "parent"))
                r = rule_add_key(&rule_tmp, TK_M_IMPORT_PARENT, op, value,
                                 NULL);
            else
            {
                LOG_RULE_ERROR("Ignoring unknown %s{} type '%s'", "IMPORT",
                               attr);
                continue;
            }
            if (r < 0)
        ...
        else if (startswith(key, "RUN"))
        {
            ...
            if (streq(attr, "builtin"))
            {
                ...
                if (cmd < 0)
                {
                    LOG_RULE_ERROR("RUN{builtin}: '%s' unknown, ignoring",
                                   value);
                    continue;
                }
                else
                    r = rule_add_key(&rule_tmp, TK_A_RUN_BUILTIN, op, value,
                                     &cmd);
            }
        ...
        else if (startswith(key, "NAME"))
        {
            ...
            else
            {
                ...
                {
                    LOG_RULE_WARNING(
                        "NAME=\"%%k\" is ignored, because it breaks kernel "
                ...
                {
                    LOG_RULE_DEBUG(
                        "NAME=\"\" is ignored, because udev will not delete "
                        "any device nodes; please remove");
                    continue;
                }
        ...
        else if (streq(key, "OPTIONS"))
        {
            ...
            if (pos)
            {
                ...
                if (rule_add_key(&rule_tmp, TK_A_DEVLINK_PRIO, op, NULL,
                                 &prio) < 0)
                    LOG_AND_RETURN_ADD_KEY;
            }
            ...
            if (pos)
            {
                ...
                if (startswith(pos, "none"))
                    r = rule_add_key(&rule_tmp, TK_A_STRING_ESCAPE_NONE, op,
                                     NULL, NULL);
                else if (startswith(pos, "replace"))
                    r = rule_add_key(&rule_tmp, TK_A_STRING_ESCAPE_REPLACE, op,
                                     NULL, NULL);
                else
                {
                    LOG_RULE_ERROR(
                        "OPTIONS: unknown string_escape mode '%s', ignoring",
                        pos);
                    r = 0;
                }
                if (r < 0)
            ...
            if (pos)
                if (rule_add_key(&rule_tmp, TK_A_DB_PERSIST, op, NULL, NULL) <
                    0)
                    LOG_AND_RETURN_ADD_KEY;
            ...
            if (pos)
            {
                ...
                if (rule_add_key(&rule_tmp, TK_A_INOTIFY_WATCH, op, NULL,
                                 &zero) < 0)
                    LOG_AND_RETURN_ADD_KEY;
            }
            else
            ...
            if (pos)
            {
                ...
                if (rule_add_key(&rule_tmp, TK_A_STATIC_NODE, op, pos, NULL) <
                    0)
                    LOG_AND_RETURN_ADD_KEY;
        ...
        if (rules->tokens[i].type == TK_A_GOTO)
        {
            ...
            for (j = i + 1; j < rules->token_cur; j++)
            {
                ...
                if (rules->tokens[j].rule.label_off == 0)
                    continue;
                if (!streq(label,
                           rules_str(rules, rules->tokens[j].rule.label_off)))
                    continue;
                rules->tokens[i].key.rule_goto = j;
                break;
            }
            if (rules->tokens[i].key.rule_goto == 0)
                log_error("GOTO '%s' has no matching label in: '%s'", label,
                          filename);
        }
    }
    return 0;
...
int udev_rules_new(UdevRules** ret_rules, ResolveNameTiming resolve_name_timing)
{
    ...
    assert(resolve_name_timing >= 0 &&
           resolve_name_timing < _RESOLVE_NAME_TIMING_MAX);
    ...
    *rules = (UdevRules){
    ...
    log_debug(
        "Rules contain %zu bytes tokens (%zu * %zu bytes), %zu bytes strings",
        rules->token_max * sizeof(struct token), rules->token_max,
        sizeof(struct token), rules->strbuf->len);

    /* cleanup temporary strbuf data */
    log_debug("%zu strings (%zu bytes), %zu de-duplicated (%zu bytes), %zu "
              "trie nodes used",
              rules->strbuf->in_count, rules->strbuf->in_len,
              rules->strbuf->dedup_count, rules->strbuf->dedup_len,
...
static bool match_key(UdevRules* rules, struct token* token, const char* val)
{
    ...
    switch (token->key.glob)
    {
        ...
        case GL_SPLIT_GLOB:
        {
            ...
            strscpy(value, sizeof(value),
                    rules_str(rules, token->key.value_off));
            key_value = value;
            while (key_value)
            {
...
static bool match_attr(UdevRules* rules, sd_device* dev, UdevEvent* event,
                       struct token* cur)
{
...
int udev_rules_apply_to_event(UdevRules* rules, UdevEvent* event,
                              usec_t timeout_usec, Hashmap* properties_list)
{
    ...
    for (;;)
    {
        ...
        switch (cur->type)
        {
            case TK_RULE:
                /* current rule */
                rule = cur;
                /* possibly skip rules which want to set NAME, SYMLINK, OWNER,
                 * GROUP, MODE */
            ...
            case TK_M_SYSCTL:
            {
                ...
                udev_event_apply_format(event,
                                        rules_str(rules, cur->key.attr_off),
            ...
            case TK_M_TAGS:
            {
                ...
                next = cur;
                while (next->type > TK_M_PARENTS_MIN &&
                       next->type < TK_M_PARENTS_MAX)
                    next++;
                ...
                for (;;)
                {
                    ...
                    for (key = cur; key < next; key++)
                    {
                        ...
                        {
                            case TK_M_KERNELS:
                                if (sd_device_get_sysname(event->dev_parent,
                                                          &val) < 0)
                                    goto try_parent;
                                ...
                                break;
                            case TK_M_SUBSYSTEMS:
                                if (sd_device_get_subsystem(event->dev_parent,
                                                            &val) < 0)
                                    goto try_parent;
                                if (!match_key(rules, key, val))
                                    goto try_parent;
                                break;
                            case TK_M_DRIVERS:
                                if (sd_device_get_driver(event->dev_parent,
                                                         &val) < 0)
                                    goto try_parent;
                                if (!match_key(rules, key, val))
                                    goto try_parent;
                                break;
                            case TK_M_ATTRS:
                                if (!match_attr(rules, event->dev_parent, event,
                                                key))
                                    goto try_parent;
                                break;
                            case TK_M_TAGS:
                            {
                                bool match = sd_device_has_tag(
                                    event->dev_parent,
                ...
                try_parent:
                    if (sd_device_get_parent(event->dev_parent,
                                             &event->dev_parent) < 0)
                    {
            ...
            case TK_M_TEST:
            {
                ...
                udev_event_apply_format(event,
                                        rules_str(rules, cur->key.value_off),
                                        filename, sizeof(filename), false);
                if (util_resolve_subsys_kernel(filename, filename,
                                               sizeof(filename), false) < 0)
                {
                    if (filename[0] != '/')
                    {
                        ...
                        strscpyl(filename, sizeof(filename), val, "/", tmp,
                                 NULL);
                    }
                }
            ...
            case TK_M_PROGRAM:
            {
                ...
                udev_event_apply_format(event,
                                        rules_str(rules, cur->key.value_off),
                                        program, sizeof(program), false);
                log_device_debug(dev, "PROGRAM '%s' %s:%u", program,
                                 rules_str(rules, rule->rule.filename_off),
                                 rule->rule.filename_line);

                if (udev_event_spawn(event, timeout_usec, true, program, result,
                                     sizeof(result)) != 0)
                {
                ...
                else
                {
                    ...
                    {
                        count = util_replace_chars(result,
                                                   UDEV_ALLOWED_CHARS_INPUT);
                        if (count > 0)
                            log_device_debug(
                                dev,
                                "Replaced %i character(s) from result of '%s'",
                                count, program);
                    }
            ...
            case TK_M_IMPORT_PROG:
            {
                ...
                udev_event_apply_format(event,
                                        rules_str(rules, cur->key.value_off),
                                        import, sizeof(import), false);
                log_device_debug(dev, "IMPORT '%s' %s:%u", import,
                                 rules_str(rules, rule->rule.filename_off),
                                 rule->rule.filename_line);

                if (import_program_into_properties(event, timeout_usec,
                                                   import) < 0)
                    if (cur->key.op != OP_NOMATCH)
                        goto nomatch;
                break;
            ...
            case TK_M_IMPORT_BUILTIN:
            {
                ...
                if (r < 0)
                {
                    ...
                    log_device_debug_errno(
                        dev, r, "IMPORT builtin '%s' fails: %m",
                        udev_builtin_name(cur->key.builtin_cmd));
            ...
            case TK_M_IMPORT_DB:
            {
                ...
                if (event->dev_db_clone &&
                    sd_device_get_property_value(event->dev_db_clone, key,
                                                 &val) >= 0)
                    device_add_property(dev, key, val);
            ...
            case TK_M_IMPORT_CMDLINE:
            {
                ...
                key = rules_str(rules, cur->key.value_off);
                r = proc_cmdline_get_key(key, PROC_CMDLINE_VALUE_OPTIONAL,
                                         &value);
                if (r < 0)
                    log_device_debug_errno(
                        dev, r,
                        "Failed to read %s from /proc/cmdline, ignoring: %m",
                        key);
                else if (r > 0)
                {
                    imported = true;
            ...
            case TK_A_OWNER:
            {
                ...
                log_device_debug(dev, "OWNER %u %s:%u", event->uid,
                                 rules_str(rules, rule->rule.filename_off),
                                 rule->rule.filename_line);
                break;
            }
            case TK_A_GROUP:
            {
                ...
                log_device_debug(dev, "GROUP %u %s:%u", event->gid,
                                 rules_str(rules, rule->rule.filename_off),
                                 rule->rule.filename_line);
                break;
            }
            case TK_A_MODE:
            {
                ...
                udev_event_apply_format(event,
                                        rules_str(rules, cur->key.value_off),
                                        mode_str, sizeof(mode_str), false);
                r = parse_mode(mode_str, &mode);
                if (r < 0)
                {
                    log_device_error_errno(
                        dev, r, "Failed to parse mode '%s': %m", mode_str);
                    break;
                }
                if (cur->key.op == OP_ASSIGN_FINAL)
                ...
                event->mode_set = true;
                event->mode = mode;
                log_device_debug(dev, "MODE %#o %s:%u", event->mode,
                                 rules_str(rules, rule->rule.filename_off),
                                 rule->rule.filename_line);
                break;
            }
            ...
            case TK_A_SECLABEL:
            {
                ...
                udev_event_apply_format(event,
                                        rules_str(rules, cur->key.value_off),
                                        label_str, sizeof(label_str), false);
                ...
                r = ordered_hashmap_ensure_allocated(&event->seclabel_list,
                                                     NULL);
                if (r < 0)
                    return log_oom();
                ...
                if (r < 0)
                    return log_oom();
                log_device_debug(dev, "SECLABEL{%s}='%s' %s:%u", name, label,
                                 rules_str(rules, rule->rule.filename_off),
                                 rule->rule.filename_line);
                name = label = NULL;
            ...
            case TK_A_ENV:
            {
                    ...
                    sd_device_get_property_value(dev, name, &value_old) >= 0)
                {
                    ...
                    udev_event_apply_format(event, val, temp, sizeof(temp),
                                            false);
                    strscpyl(value_new, sizeof(value_new), value_old, " ", temp,
                             NULL);
                }
                else
                    udev_event_apply_format(event, val, value_new,
                                            sizeof(value_new), false);
            ...
            case TK_A_TAG:
            {
                ...
                for (p = tag; *p != '\0'; p++)
                {
                    if ((*p >= 'a' && *p <= 'z') || (*p >= 'A' && *p <= 'Z') ||
                        (*p >= '0' && *p <= '9') || IN_SET(*p, '-', '_'))
                        continue;
                    log_device_error(dev, "Ignoring invalid tag name '%s'",
                                     tag);
                    break;
                }
            ...
            case TK_A_NAME:
            {
                ...
                if (IN_SET(esc, ESCAPE_UNSET, ESCAPE_REPLACE))
                {
                    ...
                    if (count > 0)
                        log_device_debug(dev,
                                         "Replaced %i character(s) from result "
                                         "of NAME=\"%s\"",
                                         count, name);
                }
                     ...
                     !streq(name_str, val + STRLEN("/dev/"))))
                {
                    log_device_error(
                        dev,
                        "Kernel device nodes cannot be renamed, ignoring "
                        "NAME=\"%s\"; please fix it in %s:%u\n",
                        name, rules_str(rules, rule->rule.filename_off),
                        rule->rule.filename_line);
                    break;
                }
                ...
                log_device_debug(dev, "NAME '%s' %s:%u", event->name,
                                 rules_str(rules, rule->rule.filename_off),
                                 rule->rule.filename_line);
                break;
            }
            case TK_A_DEVLINK:
            {
                char temp[UTIL_PATH_SIZE], filename[UTIL_PATH_SIZE], *pos,
                    *next;
                int count = 0;
                ...
                udev_event_apply_format(event,
                                        rules_str(rules, cur->key.value_off),
                                        temp, sizeof(temp), esc != ESCAPE_NONE);
                if (esc == ESCAPE_UNSET)
                ...
                if (count > 0)
                    log_device_debug(
                        dev, "Replaced %i character(s) from result of LINK",
                        count);
                pos = temp;
                while (isspace(pos[0]))
                    pos++;
            ...
            case TK_A_ATTR:
            {
                ...
                if (util_resolve_subsys_kernel(key_name, attr, sizeof(attr),
                                               false) < 0 &&
                ...
                log_device_debug(dev, "ATTR '%s' writing '%s' %s:%u", attr,
                                 value,
                                 rules_str(rules, rule->rule.filename_off),
                                 rule->rule.filename_line);
                f = fopen(attr, "we");
                if (!f)
                    log_device_error_errno(
                        dev, errno, "Failed to open ATTR{%s} for writing: %m",
                        attr);
                else if (fprintf(f, "%s", value) <= 0)
                    log_device_error_errno(
                        dev, errno, "Failed to write ATTR{%s}: %m", attr);
                break;
            }
            case TK_A_SYSCTL:
            {
                ...
                udev_event_apply_format(event,
                                        rules_str(rules, cur->key.value_off),
                                        value, sizeof(value), false);
                log_device_debug(dev, "SYSCTL '%s' writing '%s' %s:%u",
                                 filename, value,
                                 rules_str(rules, rule->rule.filename_off),
                                 rule->rule.filename_line);
                ...
                if (r < 0)
                    log_device_error_errno(
                        dev, r, "Failed to write SYSCTL{%s}='%s': %m", filename,
                        value);
                break;
            }
            case TK_A_RUN_BUILTIN:
            case TK_A_RUN_PROGRAM:
            {
                ...
                r = ordered_hashmap_put(event->run_list, cmd,
                                        INT_TO_PTR(cur->key.builtin_cmd));
                if (r < 0)
                    return log_oom();
    ...
    for (;;)
    {
        switch (cur->type)
        {
            ...
            case TK_A_STATIC_NODE:
            {
                ...
                /* we assure, that the permissions tokens are sorted before the
                 * static token */

                if (mode == 0 && uid == 0 && gid == 0 && !tags)
                    goto next;

                strscpyl(device_node, sizeof(device_node), "/dev/",
                         rules_str(rules, cur->key.value_off), NULL);
                if (stat(device_node, &stats) < 0)
                    break;
                if (!S_ISBLK(stats.st_mode) && !S_ISCHR(stats.st_mode))
                    break;

                /* export the tags to a directory as symlinks, allowing
                 * otherwise dead nodes to be tagged */
                if (tags)
                {
                    STRV_FOREACH(t, tags)
                    {
                        ...
                        strscpyl(tags_dir, sizeof(tags_dir),
                                 "/run/udev/static_node-tags/", *t, "/", NULL);
                        r = mkdir_p(tags_dir, 0755);
                        if (r < 0)
                            return log_error_errno(r, "Failed to create %s: %m",
                                                   tags_dir);

                        unescaped_filename =
                            xescape(rules_str(rules, cur->key.value_off), "/.");

                        strscpyl(tag_symlink, sizeof(tag_symlink), tags_dir,
                                 unescaped_filename, NULL);
                        r = symlink(device_node, tag_symlink);
                        if (r < 0 && errno != EEXIST)
                            return log_error_errno(
                                errno, "Failed to create symlink %s -> %s: %m",
                ...
                if ((uid != 0 && uid != stats.st_uid) ||
                    (gid != 0 && gid != stats.st_gid))
                {
