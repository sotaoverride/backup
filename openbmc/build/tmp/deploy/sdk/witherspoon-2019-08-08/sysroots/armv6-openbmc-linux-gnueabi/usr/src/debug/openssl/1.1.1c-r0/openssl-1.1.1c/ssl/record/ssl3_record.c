...
int ssl3_get_record(SSL* s)
{
    ...
    do
    {
             ...
             SSL3_RT_HEADER_LENGTH))
        {
            ...
            rret =
                ssl3_read_n(s, SSL3_RT_HEADER_LENGTH, SSL3_BUFFER_get_len(rbuf),
                            0, num_recs == 0 ? 1 : 0, &n);
            if (rret <= 0)
            ...
            if (s->server && RECORD_LAYER_is_first_record(&s->rlayer) &&
                (sslv2len & 0x8000) != 0 && (type == SSL2_MT_CLIENT_HELLO))
            {
                /*
                 *  SSLv2 style record
                 *
                ...
                if (thisrr->length >
                    SSL3_BUFFER_get_len(rbuf) - SSL2_RT_HEADER_LENGTH)
                {
        ...
        {
            more =
                thisrr->length + SSL2_RT_HEADER_LENGTH - SSL3_RT_HEADER_LENGTH;
        }
        else
        {
            more = thisrr->length;
    ...
    } while (num_recs < max_recs && thisrr->type == SSL3_RT_APPLICATION_DATA &&
             SSL_USE_EXPLICIT_IV(s) && s->enc_read_ctx != NULL &&
             (EVP_CIPHER_flags(EVP_CIPHER_CTX_cipher(s->enc_read_ctx)) &
              EVP_CIPH_FLAG_PIPELINE) &&
             ssl3_record_app_data_waiting(s));

    if (num_recs == 1 && thisrr->type == SSL3_RT_CHANGE_CIPHER_SPEC &&
...
int ssl3_do_uncompress(SSL* ssl, SSL3_RECORD* rr)
{
    ...
    i = COMP_expand_block(ssl->expand, rr->comp, SSL3_RT_MAX_PLAIN_LENGTH,
                          rr->data, (int)rr->length);
    if (i < 0)
        return 0;
    else
        rr->length = i;
...
int tls1_mac(SSL* ssl, SSL3_RECORD* rec, unsigned char* md, int sending)
{
    ...
    int stream_mac =
        (sending ? (ssl->mac_flags & SSL_MAC_FLAG_WRITE_MAC_STREAM)
        ...
        ssl3_cbc_record_digest_supported(mac_ctx))
    {
        ...
        if (ssl3_cbc_digest_record(mac_ctx, md, &md_size, header, rec->input,
                                   rec->length + md_size, rec->orig_len,
 ...
 *  -1: otherwise.
 */
int ssl3_cbc_remove_padding(SSL3_RECORD* rec, size_t block_size,
                            size_t mac_size)
{
 ...
 *  -1: otherwise.
 */
int tls1_cbc_remove_padding(const SSL* s, SSL3_RECORD* rec, size_t block_size,
                            size_t mac_size)
{
    size_t good;
...
int ssl3_cbc_copy_mac(unsigned char* out, const SSL3_RECORD* rec,
                      size_t md_size)
{
    ...
    if (!ossl_assert(rec->orig_len >= md_size && md_size <= EVP_MAX_MD_SIZE))
        return 0;
...
int dtls1_process_record(SSL* s, DTLS1_BITMAP* bitmap)
{
    ...
    /* r->length is now the compressed data plus mac */
    if ((sess != NULL) && !SSL_READ_ETM(s) && (s->enc_read_ctx != NULL) &&
...
#define dtls1_get_processed_record(s)                                          \
    dtls1_retrieve_buffered_record(                                            \
        (s), &(DTLS_RECORD_LAYER_get_processed_rcds(&s->rlayer)))
