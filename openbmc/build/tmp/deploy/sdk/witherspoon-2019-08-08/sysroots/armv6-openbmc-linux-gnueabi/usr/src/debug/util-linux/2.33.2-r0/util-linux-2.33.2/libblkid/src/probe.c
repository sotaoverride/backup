 ...
 *
 *   1. The NAME=value (tag) interface. This interface is older and returns all
 * data as strings. This interface is generic for all chains.
 *
 *   2. The binary interfaces. These interfaces return data in the native
 * formats. The interface is always specific to the probing chain.
 *
 ...
 * </informalexample>
 *
 * overwrites the previous probing result for the partitions chain, the
 * superblocks result is not modified.
 */
...
void blkid_probe_chain_reset_values(blkid_probe pr, struct blkid_chain* chn)
{
    ...
    {
        struct blkid_prval* v = list_entry(p, struct blkid_prval, prvals);
...
void blkid_probe_free_values_list(struct list_head* vals)
{
    ...
    {
        struct blkid_prval* v =
            list_entry(vals->next, struct blkid_prval, prvals);
        blkid_probe_free_value(v);
    }
}
...
int __blkid_probe_filter_types(blkid_probe pr, int chain, int flag,
                               char* names[])
{
    unsigned long* fltr;
    struct blkid_chain* chn;
...
static struct blkid_bufinfo* read_buffer(blkid_probe pr, uint64_t real_off,
                                         uint64_t len)
{
    ssize_t ret;
    ...
    DBG(LOWPROBE,
        ul_debug("\tread: off=%" PRIu64 " len=%" PRIu64 "", real_off, len));
 ...
 */
static struct blkid_bufinfo* get_cached_buffer(blkid_probe pr, uint64_t off,
                                               uint64_t len)
{
    ...
    {
        struct blkid_bufinfo* x = list_entry(p, struct blkid_bufinfo, bufs);

        if (real_off >= x->off && real_off + len <= x->off + x->len)
        {
            DBG(BUFFER, ul_debug("\treuse: off=%" PRIu64 " len=%" PRIu64
                                 " (for off=%" PRIu64 " len=%" PRIu64 ")",
...
/*
 * Zeroize in-memory data in already read buffer. The next
 * blkid_probe_get_buffer() will return modified buffer. This is usable when you
 * want to call the same probing function more than once and hide previously
 * detected magic strings.
 *
...
static int hide_buffer(blkid_probe pr, uint64_t off, uint64_t len)
{
    ...
    list_for_each(p, &pr->buffers)
    {
        ...
        if (real_off >= x->off && real_off + len <= x->off + x->len)
        {
            ...
            DBG(BUFFER,
                ul_debug("\thidding: off=%" PRIu64 " len=%" PRIu64, off, len));
            memset(data, 0, len);
            ct++;
        }
    }
 ...
 * pr->off and pr->size.
 */
unsigned char* blkid_probe_get_buffer(blkid_probe pr, uint64_t off,
                                      uint64_t len)
{
    struct blkid_bufinfo* bf = NULL;
    uint64_t real_off = pr->off + off;
    ...
    /*
    DBG(BUFFER, ul_debug("\t>>>> off=%ju, real-off=%ju (probe <%ju..%ju>,
    len=%ju", off, real_off, pr->off, pr->off + pr->size, len));
    */
    ...
    if (pr->parent && pr->parent->devno == pr->devno &&
...
int blkid_probe_reset_buffers(blkid_probe pr)
{
    ...
    while (!list_empty(&pr->buffers))
    {
        struct blkid_bufinfo* bf =
            list_entry(pr->buffers.next, struct blkid_bufinfo, bufs);
        ct++;
        len += bf->len;
        list_del(&bf->bufs);

        DBG(BUFFER,
            ul_debug(" remove buffer: [off=%" PRIu64 ", len=%" PRIu64 "]",
                     bf->off, bf->len));
        free(bf);
    }

    DBG(LOWPROBE, ul_debug(" buffers summary: %" PRIu64 " bytes by %" PRIu64
                           " read() calls",
                           len, ct));
...
static void blkid_probe_reset_values(blkid_probe pr)
{
    ...
    {
        struct blkid_prval* v =
            list_entry(pr->values.next, struct blkid_prval, prvals);
        blkid_probe_free_value(v);
    }
...
failed:
    ...
    DBG(LOWPROBE, ul_debug("CDROM: reduce size from %ju to %ju.",
                           (uintmax_t)pr->size, (uintmax_t)n << 9));
    pr->size = n << 9;
}
 ...
 */
int blkid_probe_set_device(blkid_probe pr, int fd, blkid_loff_t off,
                           blkid_loff_t size)
{
    struct stat sb;
    uint64_t devsiz = 0;
    ...
    if (S_ISBLK(sb.st_mode) && sysfs_devno_is_dm_private(sb.st_rdev, &dm_uuid))
    {
...
#ifdef CDROM_GET_CAPABILITY
    else if (S_ISBLK(sb.st_mode) && !blkid_probe_is_tiny(pr) && !dm_uuid &&
    ...
    DBG(LOWPROBE,
        ul_debug("ready for low-probing, offset=%" PRIu64 ", size=%" PRIu64 "",
                 pr->off, pr->size));
...
{
    DBG(LOWPROBE,
        ul_debug("changing probing area: size=%" PRIu64 ", off=%" PRIu64 " "
                          ...
                          uint64_t* offset, const struct blkid_idmag** res)
{
    ...
    while (mag && mag->magic)
    {
        ...
        if (buf && !memcmp(mag->magic, buf + (mag->sboff & 0x3ff), mag->len))
        {

            DBG(LOWPROBE, ul_debug("\tmagic sboff=%u, kboff=%ld", mag->sboff,
                                   mag->kboff));
            if (offset)
...
int blkid_do_probe(blkid_probe pr)
{
    ...
    do
    {
         ...
         */
                 ...
                 (chn->enabled == FALSE ||
                  chn->idx + 1 == (int)chn->driver->nidinfos || chn->idx == -1))
        {
        ...
        DBG(LOWPROBE,
            ul_debug("chain probe %s %s (idx=%d)", chn->driver->name,
                     chn->enabled ? "ENABLED" : "DISABLED", chn->idx));
...
int blkid_do_wipe(blkid_probe pr, int dryrun)
{
    ...
    DBG(LOWPROBE, ul_debug("do_wipe [offset=0x%" PRIx64 " (%" PRIu64
                           "), len=%zu, chain=%s, idx=%d, dryrun=%s]\n",
                           offset, offset, len, chn->driver->name, chn->idx,
                           dryrun ? "yes" : "not"));
 ...
 *
 *		// partition tables
 *		if (len == 0 && blkid_probe_lookup_value(pr, "PTMAGIC_OFFSET", &ostr,
 *NULL) == 0) blkid_probe_lookup_value(pr, "PTMAGIC", NULL, &len);
 *
 *		if (!len || !str)
 *			continue;
 *
...
int blkid_do_safeprobe(blkid_probe pr)
{
    ...
    for (i = 0; i < BLKID_NCHAINS; i++)
    {
        ...
        DBG(LOWPROBE, ul_debug("chain safeprobe %s %s", chn->driver->name,
...
int blkid_do_fullprobe(blkid_probe pr)
{
    ...
    for (i = 0; i < BLKID_NCHAINS; i++)
    {
        ...
        DBG(LOWPROBE, ul_debug("chain fullprobe %s: %s", chn->driver->name,
                               chn->enabled ? "ENABLED" : "DISABLED"));
 ...
 */
int blkid_probe_value_set_data(struct blkid_prval* v, const unsigned char* data,
                               size_t len)
{
...
int blkid_probe_sprintf_value(blkid_probe pr, const char* name, const char* fmt,
                              ...)
{
    int rc;
    va_list ap;
...
int blkid_probe_set_magic(blkid_probe pr, uint64_t offset, size_t len,
                          const unsigned char* magic)
{
    ...
    switch (chn->driver->id)
    {
        case BLKID_CHAIN_SUBLKS:
            ...
            if (!rc)
                rc = blkid_probe_sprintf_value(pr, "SBMAGIC_OFFSET", "%llu",
        ...
        case BLKID_CHAIN_PARTS:
            ...
            if (!rc)
                rc = blkid_probe_sprintf_value(pr, "PTMAGIC_OFFSET", "%llu",
                                               (unsigned long long)offset);
            break;
        default:
            break;
...
int blkid_probe_verify_csum(blkid_probe pr, uint64_t csum, uint64_t expected)
{
    if (csum != expected)
    {
        ...
        DBG(LOWPROBE, ul_debug("incorrect checksum for type %s,"
                               " got %" PRIX64 ", expected %" PRIX64 "",
                               blkid_probe_get_probername(pr), csum, expected));
        /*
 ...
 * @pr: probe
 *
 * This function returns size of probing area as defined by
 * blkid_probe_set_device(). If the size of the probing area is unrestricted
 ...
 * @pr: probe
 *
 * This function returns offset of probing area as defined by
 * blkid_probe_set_device().
 *
...
void blkid_probe_set_wiper(blkid_probe pr, uint64_t off, uint64_t size)
{
    ...
    if (!chn || !chn->driver || chn->idx < 0 ||
        (size_t)chn->idx >= chn->driver->nidinfos)
        return;
    ...
    DBG(LOWPROBE,
        ul_debug("wiper set to %s::%s off=%" PRIu64 " size=%" PRIu64 "",
                 chn->driver->name, chn->driver->idinfos[chn->idx]->name,
 ...
 */
int blkid_probe_is_wiped(blkid_probe pr, struct blkid_chain** chn, uint64_t off,
                         uint64_t size)
{
    if (!size)
        return 0;
