...
class weak_ptr
{
  ...
  public:
...
#endif
        BOOST_SP_NOEXCEPT : px(r.lock().get()),
                            pn(r.pn)
    {
    ...
    weak_ptr(weak_ptr&& r) BOOST_SP_NOEXCEPT
        : px(r.px),
          pn(static_cast<boost::detail::weak_count&&>(r.pn))
    {
        r.px = 0;
    ...
    template <typename Y>
    void _internal_aliasing_assign(weak_ptr<Y> const& r,
                                   element_type* px2) BOOST_SP_NOEXCEPT
    {
        px = px2;
        pn = r.pn;
    }
...
inline bool operator<(weak_ptr<T> const& a,
                      weak_ptr<U> const& b) BOOST_SP_NOEXCEPT
{
    return a.owner_before(b);
}

template <class T>
void swap(weak_ptr<T>& a, weak_ptr<T>& b) BOOST_SP_NOEXCEPT
{
    a.swap(b);
}
