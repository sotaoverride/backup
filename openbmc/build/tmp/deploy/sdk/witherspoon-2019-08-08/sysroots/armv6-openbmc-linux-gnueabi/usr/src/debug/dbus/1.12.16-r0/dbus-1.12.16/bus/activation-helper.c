...
static BusDesktopFile* desktop_file_for_name(BusConfigParser* parser,
...
static dbus_bool_t check_service_name(BusDesktopFile* desktop_file,
                                      ...
                                      DBusError* error)
{
    ...
    if (!bus_desktop_file_get_string(desktop_file, DBUS_SERVICE_SECTION,
                                     DBUS_SERVICE_NAME, &name_tmp, error))
        goto failed;
...
static dbus_bool_t get_parameters_for_service(BusDesktopFile* desktop_file,
                                              const char* service_name,
                                              char** exec, char** user,
                                              DBusError* error)
{
    ...
    /* get the user that should run this service - user is compulsary for system
     * activation */
    if (!bus_desktop_file_get_string(desktop_file, DBUS_SERVICE_SECTION,
                                     DBUS_SERVICE_USER, &user_tmp, error))
    {
        _DBUS_ASSERT_ERROR_IS_SET(error);
        goto failed;
    }
...
static dbus_bool_t exec_for_correct_user(char* exec, char* user,
                                         DBusError* error)
{
    char** argv;
    int argc;
...
static dbus_bool_t get_correct_parser(BusConfigParser** parser,
                                      DBusError* error)
{
    ...
    if (getuid() != geteuid())
        _dbus_assert_not_reached(
            "dbus-daemon-launch-helper-test binary is setuid!");

    /* this is not a security hole. The environment variable is only passed in
     * the dbus-daemon-lauch-helper-test NON-SETUID launcher */
...
static dbus_bool_t launch_bus_name(const char* bus_name,
                                   BusConfigParser* parser, DBusError* error)
{
    BusDesktopFile* desktop_file;
    char *exec, *user;
    dbus_bool_t retval;
    ...
    if (!get_parameters_for_service(desktop_file, bus_name, &exec, &user,
                                    error))
        goto finish;
