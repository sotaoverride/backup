...
// R_J(x, y, z, p) = 1.5 * \int_{0}^{\infty} (t+p)^{-1} [(t+x)(t+y)(t+z)]^{-1/2}
// dt Carlson, Numerische Mathematik, vol 33, 1 (1979)
...
T ellint_rc1p_imp(T y, const Policy& pol)
{
    ...
    if (y == -1)
    {
        return policies::raise_domain_error<T>(
            function, "Argument y must not be zero but got %1%", y, pol);
    }
    ...
    else
    {
        ...
        {
            T arg = sqrt(-y);
            result = (boost::math::log1p(arg) - boost::math::log1p(-arg)) /
                     (2 * sqrt(-y));
        }
        else
        {
...
T ellint_rj_imp(T x, T y, T z, T p, const Policy& pol)
{
    ...
    {
        return policies::raise_domain_error<T>(
            function, "Argument x must be non-negative, but got x = %1%", x,
            pol);
    }
    if (y < 0)
    {
        return policies::raise_domain_error<T>(
            function, "Argument y must be non-negative, but got y = %1%", y,
            pol);
    }
    if (z < 0)
    {
        return policies::raise_domain_error<T>(
            function, "Argument z must be non-negative, but got z = %1%", z,
            pol);
    }
    if (p == 0)
    {
        return policies::raise_domain_error<T>(
            function, "Argument p must not be zero, but got p = %1%", p, pol);
    }
    ...
    if (p < 0)
    {
        ...
        value += 3 * sqrt((x * y * z) / (x * y + p * q)) *
                 ellint_rc_imp(T(x * y + p * q), T(p * q), pol);
        value /= (z + q);
        return value;
    }
    ...
    if (x == y)
    {
        ...
        else
        {
            ...
            {
                return 3 *
                       (ellint_rc_imp(x, y, pol) - ellint_rc_imp(x, p, pol)) /
                       (p - y);
            }
            // Otherwise fall through to normal method, special case above will
            // suffer too much cancellation...
        }
    }
    if (y == z)
    {
        ...
        {
            // y = z:
            return 3 * (ellint_rc_imp(x, y, pol) - ellint_rc_imp(x, p, pol)) /
                   (p - y);
        }
        // Otherwise fall through to normal method, special case above will
        // suffer too much cancellation...
    ...
    T A0 = An;
    T delta = (p - x) * (p - y) * (p - z);
    T Q = pow(tools::epsilon<T>() / 5, -T(1) / 8) *
          (std::max)((std::max)(fabs(An - x), fabs(An - y)),
                     (std::max)(fabs(An - z), fabs(An - p)));
    ...
    for (n = 0; n < policies::get_max_series_iterations<Policy>(); ++n)
    {
        ...
        if ((En < -0.5) && (En > -1.5))
        {
            ...
            //
            // 1+E_0 = 1 + (p-x)*(p-y)*(p-z)/((sqrt(p) +
            // sqrt(x))*(sqrt(p)+sqrt(y))*(sqrt(p)+sqrt(z)))^2
            //       = 2*sqrt(p)*(p+sqrt(x) * (sqrt(y)+sqrt(z)) +
            //       sqrt(y)*sqrt(z)) / ((sqrt(p) + sqrt(x))*(sqrt(p) +
            //       sqrt(y)*(sqrt(p)+sqrt(z))))
            //
            // And since this is just an application of the duplication formula
            // for RJ, the same expression works for 1+En if we use x,y,z,p_n
            // etc. This branch is taken only once or twice at the start of
            // iteration, after than En reverts to it's usual very small values.
            //
    ...
    T result = fmn * pow(An, T(-3) / 2) *
               (1 - 3 * E2 / 14 + E3 / 6 + 9 * E2 * E2 / 88 - 3 * E4 / 22 -
    ...
    ellint_rj(T1 x, T2 y, T3 z, T4 p, const Policy& pol)
{
    ...
    return policies::checked_narrowing_cast<result_type, Policy>(
        detail::ellint_rj_imp(
            ...
            static_cast<value_type>(z), static_cast<value_type>(p), pol),
        "boost::math::ellint_rj<%1%>(%1%,%1%,%1%,%1%)");
}

template <class T1, class T2, class T3, class T4>
inline typename tools::promote_args<T1, T2, T3, T4>::type ellint_rj(T1 x, T2 y,
                                                                    T3 z, T4 p)
{
