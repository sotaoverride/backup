...
netsnmp_feature_provide(helper_statistics)
    ...
    netsnmp_feature_require(statistics)
        ...
        static int netsnmp_get_statistic_helper_handler(
...
static netsnmp_mib_handler* netsnmp_get_statistic_handler(int offset)
{
    netsnmp_mib_handler* ret = netsnmp_create_handler(
        "get_statistic", netsnmp_get_statistic_helper_handler);
    if (ret)
    {
...
int netsnmp_register_statistic_handler(netsnmp_handler_registration* reginfo,
                                       oid start, int begin, int end)
{
    netsnmp_mib_handler* handler = netsnmp_get_statistic_handler(begin - start);
    if (!handler ||
