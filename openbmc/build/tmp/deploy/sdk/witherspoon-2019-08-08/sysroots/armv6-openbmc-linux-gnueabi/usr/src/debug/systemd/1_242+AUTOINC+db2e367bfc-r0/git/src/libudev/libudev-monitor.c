 ...
 **/
_public_ struct udev_monitor* udev_monitor_new_from_netlink(struct udev* udev,
                                                            const char* name)
{
                                         ...
                                         int size)
{
    ...
    return sd_device_monitor_set_receive_buffer_size(udev_monitor->monitor,
                                                     (size_t)size);
}
 ...
 * Returns: #NULL
 **/
DEFINE_PUBLIC_TRIVIAL_REF_UNREF_FUNC(struct udev_monitor, udev_monitor,
                                     udev_monitor_free);
...
static int udev_monitor_receive_sd_device(struct udev_monitor* udev_monitor,
                                          sd_device** ret)
{
    struct pollfd pfd;
    int r;
    ...
    for (;;)
    {
        /* r == 0 means a device is received but it does not pass the current
         * filter. */
 ...
 *
 * The filter must be installed before the monitor is switched to listening
 * mode.
 *
 * Returns: 0 on success, otherwise a negative error value.
 */
_public_ int udev_monitor_filter_add_match_subsystem_devtype(
    struct udev_monitor* udev_monitor, const char* subsystem,
    const char* devtype)
{
    assert_return(udev_monitor, -EINVAL);

    return sd_device_monitor_filter_add_match_subsystem_devtype(
        udev_monitor->monitor, subsystem, devtype);
}
