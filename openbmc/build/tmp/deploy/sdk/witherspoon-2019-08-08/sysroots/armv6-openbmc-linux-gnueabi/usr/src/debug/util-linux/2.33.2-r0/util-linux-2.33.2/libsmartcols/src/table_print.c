 ...
 */
#define titlepadding_symbol(tb)                                                \
    ((tb)->symbols->title_padding ? (tb)->symbols->title_padding : " ")
...
#define cellpadding_symbol(tb)                                                 \
    ((tb)->padding_debug                                                       \
         ? "."                                                                 \
         : ((tb)->symbols->cell_padding ? (tb)->symbols->cell_padding : " "))

#define want_repeat_header(tb)                                                 \
    (!(tb)->header_repeat || (tb)->header_next <= (tb)->termlines_used)
...
static char* buffer_get_safe_data(struct libscols_table* tb,
                                  struct libscols_buffer* buf, size_t* cells,
...
/* returns size in bytes of the ascii art (according to art_idx) in safe
 * encoding */
                             ...
                             size_t bufsz)
{
    ...
    if (ln && scols_column_is_tree(cl))
    {
        ...
        {
            /* use the same draw function as though we were intending to draw an
             * L-shape */
...
static int set_pending_data(struct libscols_column* cl, const char* data,
                            size_t sz)
{
    char* p = NULL;
...
{
    DBG(COL, ul_debugobj(cl, "step pending data %zu -= %zu",
...
static int print_pending_data(struct libscols_table* tb,
                              struct libscols_column* cl,
                              struct libscols_line* ln, /* optional */
...
static int print_data(struct libscols_table* tb, struct libscols_column* cl,
                      struct libscols_line* ln, /* optional */
                      struct libscols_cell* ce, /* optional */
                      struct libscols_buffer* buf)
{
    ...
    if (is_last && len < width && !scols_table_is_maxout(tb) &&
    ...
    if (len > width && !scols_column_is_trunc(cl))
        print_newline_padding(tb, cl, ln,
                              buf->bufsz); /* next column starts on next line */
    else
        fputs(colsep(tb), tb->out); /* columns separator */
...
static int cell_to_buffer(struct libscols_table* tb, struct libscols_line* ln,
                          struct libscols_column* cl,
                          struct libscols_buffer* buf)
{
    const char* data;
 ...
 */
static int print_line(struct libscols_table* tb, struct libscols_line* ln,
                      struct libscols_buffer* buf)
{
    int rc = 0, pending = 0;
    struct libscols_column* cl;
    ...
    while (rc == 0 && scols_table_next_column(tb, &itr, &cl) == 0)
    {
        ...
        if (rc == 0)
            rc = print_data(tb, cl, ln, scols_line_get_cell(ln, cl->seqnum),
                            buf);
    ...
    while (rc == 0 && pending)
    {
        ...
        while (rc == 0 && scols_table_next_column(tb, &itr, &cl) == 0)
        {
            ...
            if (cl->pending_data)
            {
                rc = print_pending_data(tb, cl, ln,
...
static int print_title(struct libscols_table* tb)
{
    ...
    switch (scols_cell_get_alignment(&tb->title))
    {
        ...
        default:
             ...
             */
            if (len < width && !scols_table_is_maxout(tb) &&
    ...
    rc = mbsalign_with_padding(buf, title, titlesz, &width, align, 0,
                               (int)*titlepadding_symbol(tb));
...
static int print_header(struct libscols_table* tb, struct libscols_buffer* buf)
{
    ...
    if ((tb->header_printed == 1 && tb->header_repeat == 0) ||
        scols_table_is_noheadings(tb) || scols_table_is_export(tb) ||
        scols_table_is_json(tb) || list_empty(&tb->tb_lines))
        return 0;
...
static int print_range(struct libscols_table* tb, struct libscols_buffer* buf,
                       struct libscols_iter* itr, struct libscols_line* end)
{
    int rc = 0;
    struct libscols_line* ln;
...
static int print_tree_line(struct libscols_table* tb, struct libscols_line* ln,
                           struct libscols_buffer* buf, int last,
                           int last_in_table)
{
    int rc;
...
static void dbg_column(struct libscols_table* tb, struct libscols_column* cl)
{
    ...
    DBG(COL,
        ul_debugobj(cl,
                    ...
                    cl->width_avg, cl->width_max, cl->width_min,
 ...
 */
static int recount_widths(struct libscols_table* tb,
                          struct libscols_buffer* buf)
{
    struct libscols_column* cl;
    struct libscols_iter itr;
    ...
    DBG(TAB,
        ul_debugobj(tb, "recounting widths (termwidth=%zu)", tb->termwidth));
    ...
    while (scols_table_next_column(tb, &itr, &cl) == 0)
    {
        ...
        width += cl->width +
                 (is_last ? 0 : colsepsz); /* separator for non-last column */
    ...
    if (width_min > tb->termwidth && scols_table_is_maxout(tb))
    {
        DBG(TAB,
            ul_debugobj(
                tb, " min width larger than terminal! [width=%zu, term=%zu]",
                width_min, tb->termwidth));
    ...
    for (stage = 1; width > tb->termwidth && stage <= 3;)
    {
        ...
        DBG(TAB, ul_debugobj(
                     tb, " reduce width - #%d stage (current=%zu, wanted=%zu)",
        ...
        while (scols_table_next_column(tb, &itr, &cl) == 0)
        {
            ...
            DBG(TAB,
                ul_debugobj(cl, "   checking %s (width=%zu, treeart=%zu)",
            ...
            switch (stage)
            {
                ...
                case 1:
                    if (!trunc_flag) /* ignore: missing flag */
                        break;
                    if (cl->width_hint <= 0 ||
                        cl->width_hint >= 1) /* ignore: no relative */
                        break;
                    if (cl->width <
                        (size_t)(cl->width_hint *
                                 tb->termwidth)) /* ignore: smaller than
                                                    expected width */
                        break;

                    DBG(TAB,
                        ul_debugobj(tb, "     reducing (relative with flag)"));
                    cl->width--;
                    width--;
                    break;
                ...
                case 3:
                    ...
                    DBG(TAB, ul_debugobj(
                                 tb, "     reducing (relative without flag)"));
                    cl->width--;
                    width--;
                    break;
            }
...
static void cleanup_printing(struct libscols_table* tb,
                             struct libscols_buffer* buf)
{
    if (!tb)
        return;
...
static int initialize_printing(struct libscols_table* tb,
                               struct libscols_buffer** buf)
{
    ...
    *buf = NULL;
    ...
    if (tb->format == SCOLS_FMT_HUMAN)
        tb->is_term = tb->termforce == SCOLS_TERMFORCE_NEVER
                          ? 0
                          : tb->termforce == SCOLS_TERMFORCE_ALWAYS
                                ? 1
    ...
    if (!tb->is_term || tb->format != SCOLS_FMT_HUMAN ||
        ...
        case SCOLS_FMT_EXPORT:
        {
            ...
            while (scols_table_next_column(tb, &itr, &cl) == 0)
            {
                if (scols_column_is_hidden(cl))
                    continue;
                extra_bufsz +=
                    strlen(scols_cell_get_data(&cl->header)); /* data */
...
int scols_table_print_range_to_string(struct libscols_table* tb,
                                      struct libscols_line* start,
                                      struct libscols_line* end, char** data)
{
