...
template <typename CharT, typename TraitsT>
BOOST_LOG_API std::basic_istream<CharT, TraitsT>&
 ...
 *
 * This tag can be used to acquire the logger that is used with lrivial logging
 * macros. This may be useful when the logger is used with other macros which
 * require a logger.
 */
struct logger
{
...
/*!
 * The macro is used to initiate logging. The \c lvl argument of the macro
 * specifies one of the following severity levels: \c trace, \c debug, \c info,
 * \c warning, \c error or \c fatal (see \c severity_level enum). Following the
 * macro, there may be a streaming expression that composes the record message
 * string. For example:
 *
 * \code
