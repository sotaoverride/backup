...
static uint32_t generate_local_port(void)
{
    ...
    for (j = 0; j < 32; j++)
    {
         ...
         * (this is relevant only if the applications ends up using both
         * versions of the library and doesn't hurt otherwise). */
        if (j == 31)
            i = 0;
        else
            i = (((i - 1) + 7) % 31) + 1;
...
{
    nl_cb_set(sk->s_cb, NL_CB_SEQ_CHECK, NL_CB_CUSTOM, noop_seq_check, NULL);
}
...
uint32_t _nl_socket_set_local_port_no_release(struct nl_sock* sk,
                                              int generate_other)
{
    uint32_t port;
...
int nl_socket_add_memberships(struct nl_sock* sk, int group, ...)
{
    ...
    while (group != 0)
    {
        ...
        err = setsockopt(sk->s_fd, SOL_NETLINK, NETLINK_ADD_MEMBERSHIP, &group,
                         sizeof(group));
        if (err < 0)
        {
            va_end(ap);
            NL_DBG(4,
                   "nl_socket_add_memberships(%p): setsockopt() failed with %d "
                   "(%s)\n",
...
int nl_socket_drop_memberships(struct nl_sock* sk, int group, ...)
{
    ...
    while (group != 0)
    {
        ...
        err = setsockopt(sk->s_fd, SOL_NETLINK, NETLINK_DROP_MEMBERSHIP, &group,
                         sizeof(group));
        if (err < 0)
        {
            va_end(ap);
            NL_DBG(4,
                   "nl_socket_drop_memberships(%p): setsockopt() failed with "
                   "%d (%s)\n",
                   sk, errno, nl_strerror_l(errno));
            return -nl_syserr2nlerr(errno);
        }
 ...
 * @see nl_connect()
 *
 * @return 0 on success or a negative error code. On error, @fd is not closed
 * but possibly unusable.
 *
...
int nl_socket_set_fd(struct nl_sock* sk, int protocol, int fd)
{
    ...
    err = getsockname(fd, (struct sockaddr*)&local, &addrlen);
    if (err < 0)
    {
        NL_DBG(4,
    ...
    {
        NL_DBG(4,
               "nl_socket_set_fd(%p,%d): getsockname() returned family %d "
               "instead of %d (AF_NETLINK)\n",
    ...
    {
        NL_DBG(4,
               "nl_socket_set_fd(%p,%d): getsockopt() for SO_TYPE failed with "
               "%d (%s)\n",
               sk, fd, errno, nl_strerror_l(errno));
        return -nl_syserr2nlerr(errno);
    }
    ...
    {
        NL_DBG(4,
               "nl_socket_set_fd(%p,%d): getsockopt() returned SO_TYPE %d "
               "instead of %d (SOCK_RAW)\n",
    ...
    if (err < 0)
    {
        if (errno == ENOPROTOOPT)
            goto no_so_protocol;
        NL_DBG(4,
               "nl_socket_set_fd(%p,%d): getsockopt() for SO_PROTOCOL failed "
               "with %d (%s)\n",
               sk, fd, errno, nl_strerror_l(errno));
        return -nl_syserr2nlerr(errno);
    }
    ...
    {
        NL_DBG(4,
               "nl_socket_set_fd(%p,%d): getsockopt() for SO_PROTOCOL returned "
               "%d instead of %d\n",
    ...
    no_so_protocol:
        ...
        {
            NL_DBG(4,
                   "nl_socket_set_fd(%p,%d): unknown protocol and unable to "
                   "detect it via SO_PROTOCOL socket option\n",
                   sk, fd);
            return -NLE_INVAL;
        }
 ...
 *
 * By default, MSG_PEEK is enabled unless the user calls either
 * nl_socket_disable_msg_peek()/ nl_socket_enable_msg_peek() or sets the message
 * buffer size to a positive value. See capability
 * NL_CAPABILITY_NL_RECVMSGS_PEEK_BY_DEFAULT for that.
 *
 * @return 0 on success or a negative error code.
 */
...
int nl_socket_recv_pktinfo(struct nl_sock* sk, int state)
{
    ...
    err = setsockopt(sk->s_fd, SOL_NETLINK, NETLINK_PKTINFO, &state,
                     sizeof(state));
    if (err < 0)
    {
        NL_DBG(4,
