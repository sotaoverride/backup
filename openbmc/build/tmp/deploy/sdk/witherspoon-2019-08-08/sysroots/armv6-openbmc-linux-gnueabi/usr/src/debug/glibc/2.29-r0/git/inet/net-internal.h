...
libc_hidden_proto(__idna_to_dns_encoding) int __idna_from_dns_encoding(
...
/* Check the specified name for non-ASCII characters and backslashes
   or encoding errors.  */
enum idna_name_classification
    __idna_name_classify(const char* name) attribute_hidden;
...
static inline struct deadline __deadline_first(struct deadline left,
...
int __deadline_to_ms(struct deadline_current_time,
                     struct deadline) attribute_hidden;
