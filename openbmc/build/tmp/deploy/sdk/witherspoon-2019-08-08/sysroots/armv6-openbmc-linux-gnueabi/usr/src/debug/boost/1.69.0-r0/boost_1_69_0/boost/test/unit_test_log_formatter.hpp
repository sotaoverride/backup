...
///
/// You can define a class with implements this interface and use an instance of
/// it as a Unit Test Framework log formatter
// ***************************************************************************
...
///
/// During the test module execution Unit Test Framework can report messages
/// about success or failure of assertions, which test suites are being run and
/// more (specifically which messages are reported depends on log level
/// threshold selected by the user).
///
/// All these messages constitute Unit Test Framework log. There are many ways
/// (formats) to present these messages to the user.
///
/// Boost.Test comes with three formats:
/// - Compiler-like log format: intended for human consumption/diagnostic
/// - XML based log format:  intended for processing by automated regression
/// test systems.
/// - JUNIT based log format:  intended for processing by automated regression
/// test systems.
///
/// If you want to produce some other format you need to implement class with
/// specific interface and use method @c unit_test_log_t::set_formatter during a
/// test module initialization to set an active formatter. The class
/// unit_test_log_formatter defines this interface.
///
/// This interface requires you to format all possible messages being produced
/// in the log. These includes error messages about failed assertions, messages
/// about caught exceptions and information messages about test units being
/// started/ended. All the methods in this interface takes a reference to
/// standard stream as a first argument. This is where final messages needs to
/// be directed to. Also you are given all the information necessary to produce
/// a message.
///
/// @par Since Boost 1.62:
/// - Each formatter may indicate the default output stream. This is convenient
/// for instance for streams intended
///   for automated processing that indicate a file. See @c
///   get_default_stream_description for more details.
/// - Each formatter may manage its own log level through the getter/setter @c
/// get_log_level and @c set_log_level .
///
/// @see
/// - boost::unit_test::test_observer for an indication of the calls of the test
/// observer interface
class BOOST_TEST_DECL unit_test_log_formatter
{
  public:
    ...
    {
        BOOST_UTL_ET_INFO,       ///< Information message from the framework
        BOOST_UTL_ET_MESSAGE,    ///< Information message from the user
        BOOST_UTL_ET_WARNING,    ///< Warning (non error) condition notification
                                 ///< message
    ...
    /// @see test_unit_start
    virtual void test_unit_finish(std::ostream& os, test_unit const& tu,
    ...
    virtual void test_unit_skipped(std::ostream& /* os */,
                                   test_unit const& /* tu */)
    {
    }

    /// Invoked when a test unit is aborted
    virtual void test_unit_aborted(std::ostream& /* os */,
                                   test_unit const& /* tu */)
    {
    }
    ...
    ///
    /// The logging of the exception information is finilized by a call to @c
    /// log_exception_finish.
    ///
    /// @param[in] os   output stream to write a messages into
    /// @param[in] lcd  information about the last checkpoint before the
    /// exception was triggered
    /// @param[in] ex   information about the caught exception
    /// @see log_exception_finish
    virtual void log_exception_start(std::ostream& os,
                                     log_checkpoint_data const& lcd,
    ...
    /// Call to this function starts new log entry. It is followed by series of
    /// log_entry_value calls and finally call to log_entry_finish. A log entry
    /// may consist of one or more values being reported. Some of these values
    /// will be plain strings, while others can be complicated expressions in a
    /// form of "lazy" expression template lazy_ostream.
    /// @param[in] os   output stream to write a messages into
    /// @param[in] led  log entry attributes
    /// @param[in] let  log entry type log_entry_finish
    /// @see log_entry_value, log_entry_finish
    ///
    /// @note call to this function may happen before any call to
    /// test_unit_start or all calls to test_unit_finish as the framework might
    /// log errors raised during global initialization/shutdown.
    virtual void log_entry_start(std::ostream& os, log_entry_data const& led,
                                 log_entry_types let) = 0;

    /// Invoked by Unit Test Framework to report a log entry content
    ///
    /// This is one of two overloaded methods to report log entry content. This
    /// one is used to report plain string value.
    /// @param[in] os   output stream to write a messages into.
    ...
    /// This is one of two overloaded methods to report log entry content. This
    /// one is used to report some complicated expression passed as an
    /// expression template lazy_ostream. In most cases default implementation
    /// provided by the framework should work as is (it just converts the lazy
    /// expression into a string.
    /// @param[in] os   output stream to write a messages into
    /// @param[in] value log entry "lazy" value
    /// @see log_entry_start, log_entry_finish
    virtual void
        log_entry_value(std::ostream& os,
                        lazy_ostream const& value); // there is a default impl
    ...
    //
    /// Unit Test Framework logs for failed assertions and uncaught exceptions
    /// context if one was defined by a test module. Context consists of
    /// multiple "scopes" identified by description messages assigned by the
    /// test module using BOOST_TEST_INFO/BOOST_TEST_CONTEXT statements.
    /// @param[in] os   output stream to write a messages into
    ...
    /// Invoked by Unit Test Framework to report log entry context "scope"
    /// description
    //
    /// Each "scope" description is reported by separate call to
    /// log_entry_context.
    /// @param[in] os   output stream to write a messages into
    /// @param[in] l    entry log_level, to be used to fine tune the message
    /// @param[in] value  context "scope" description
    /// @see log_entry_start, entry_context_finish
    virtual void log_entry_context(std::ostream& os, log_level l,
                                   const_string value) = 0;
    ...
    /// the command line @c "--log_sink" parameter. With that regards, @b stdout
    /// and @b stderr have special meaning indicating the standard output or
