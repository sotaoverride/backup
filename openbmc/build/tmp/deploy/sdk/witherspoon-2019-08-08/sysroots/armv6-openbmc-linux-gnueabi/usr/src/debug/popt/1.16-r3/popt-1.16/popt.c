...
static void prtcon(const char* msg, poptContext con)
{
    ...
    fprintf(
        ...
        con, con->os, (con->os->nextCharArg ? con->os->nextCharArg : ""),
        (con->os->nextArg ? con->os->nextArg : ""), con->os->next,
    ...
    if (opt != NULL)
        for (; opt->longName || opt->shortName || opt->arg; opt++)
        {
            ...
            if (arg.ptr)
                ...
                {
                    case POPT_ARG_INCLUDE_TABLE:         /* Recurse on included
                                                            sub-tables. */
                    ...
                    case POPT_ARG_CALLBACK: /* Perform callback. */
                        if (!CBF_ISSET(opt, PRE))
                            /*@switchbreak@*/ break;
                        /*@-noeffectuncon @*/ /* XXX no known way to annotate
                                                 (*vector) calls. */
                        arg.cb(con, POPT_CALLBACK_REASON_PRE, NULL, NULL,
                               opt->descrip);
                        /*@=noeffectuncon @*/
                        /*@switchbreak@*/ break;
                }
            ...
            if (arg.ptr)
                switch (poptArgType(opt))
                {
                    ...
                    case POPT_ARG_CALLBACK: /* Perform callback. */
                        ...
                        /*@-noeffectuncon @*/ /* XXX no known way to annotate
                                                 (*vector) calls. */
                        arg.cb(con, POPT_CALLBACK_REASON_POST, NULL, NULL,
                               opt->descrip);
                        /*@=noeffectuncon @*/
                        /*@switchbreak@*/ break;
                }
...
static void invokeCallbacksOPTION(poptContext con, const struct poptOption* opt,
                           ...
                           const struct poptOption* options, unsigned int flags)
{
    ...
    con->finalArgv =
        calloc((size_t)con->finalArgvAlloced, sizeof(*con->finalArgv));
    con->execAbsolute = 1;
    con->arg_strip = NULL;
...
/*@modifies con @*/
{
    ...
    if ((con->finalArgvCount + 1) >= (con->finalArgvAlloced))
    {
        ...
        con->finalArgv = realloc(con->finalArgv, sizeof(*con->finalArgv) *
...
/*@modifies con @*/
{
    ...
    if (item)
    {
        if (longName && item->option.longName != NULL &&
            ...
            return 0;
        else if (shortName && shortName == item->option.shortName)
            return 0;
    }
...
static /*@null@*/
    const char*
    findProgramPath(/*@null@*/ const char* argv0)
/*@*/
{
...
/*@modifies internalState @*/
{
    ...
    {
        char* s =
            malloc(strlen(con->execPath) + strlen(item->argv[0]) + sizeof("/"));
        if (s)
...
/*@observer@*/ /*@null@*/
static const struct poptOption* findOption(
    const struct poptOption* opt,
    /*@null@*/ const char* longName, size_t longNameLen, char shortName,
    /*@null@*/ /*@out@*/ poptCallbackType* callback,
    /*@null@*/ /*@out@*/ const void** callbackData, unsigned int argInfo)
    ...
    for (; opt->longName || opt->shortName || opt->arg; opt++)
    {
        ...
        switch (poptArgType(opt))
        {
            case POPT_ARG_INCLUDE_TABLE: /* Recurse on included sub-tables. */
            {
                ...
                opt2 = findOption(arg.opt, longName, longNameLen, shortName,
                                  callback, callbackData, argInfo);
                if (opt2 == NULL)
                    continue;
...
static const char* findNextArg(/*@special@*/ poptContext con, unsigned argx,
                               int delete_arg)
...
/*@modifies *bitsp, _poptBitsN, _poptBitsM, _poptBitsK @*/
{
    ...
    if (*bitsp == NULL)
    {
        ...
        if (_poptBitsM == 0U)
            _poptBitsM = (3 * _poptBitsN) / 2;
        if (_poptBitsK == 0U || _poptBitsK > 32U)
            _poptBitsK = _POPT_BITS_K;
    ...
    switch (LF_ISSET(LOGICALOPS))
    {
        case 0:
            *arg = (short)aLong;
            break;
        case POPT_ARGFLAG_OR:
            *(unsigned short*)arg |= (unsigned short)aLong;
            break;
        case POPT_ARGFLAG_AND:
            *(unsigned short*)arg &= (unsigned short)aLong;
            break;
...
/*@*/
{
    ...
    if (con->os->argv != NULL && con->os->next > 0 && opt->longName != NULL)
        if (LF_ISSET(TOGGLE))
        {
            ...
            if (longName[0] != opt->longName[0] ||
                longName[1] != opt->longName[1])
            {
                if (!LF_ISSET(XOR))
...
#define LLONG_MIN (-LLONG_MAX - 1LL)
                    ...
                    /*@innerbreak@*/ break;
                case POPT_ARG_LONG:
                    rc = !(aNUM < (long long)LONG_MIN ||
                           aNUM > (long long)LONG_MAX)
                             ? poptSaveLong(arg.longp, argInfo, (long)aNUM)
                             : POPT_ERROR_OVERFLOW;
                    /*@innerbreak@*/ break;
                case POPT_ARG_INT:
                    rc = !(aNUM < (long long)INT_MIN ||
                           aNUM > (long long)INT_MAX)
                             ? poptSaveInt(arg.intp, argInfo, (long)aNUM)
                             : POPT_ERROR_OVERFLOW;
                    /*@innerbreak@*/ break;
                case POPT_ARG_SHORT:
                    rc = !(aNUM < (long long)SHRT_MIN ||
                           aNUM > (long long)SHRT_MAX)
...
int poptGetNextOpt(poptContext con)
{
    ...
    while (!done)
    {
        ...
        if (con->os->nextCharArg)
        {
            ...
            opt = findOption(con->options, NULL, 0, *nextCharArg, &cb, &cbData,
                             0);
            if (!opt)
                return POPT_ERROR_BADOPT;
            shorty = 1;
        ...
        if (opt == NULL)
            return POPT_ERROR_BADOPT; /* XXX can't happen */
        ...
        if (con->finalArgv != NULL)
        {
            char* s = malloc((opt->longName ? strlen(opt->longName) : 0) +
                             sizeof("--"));
            if (s != NULL)
            { /* XXX can't happen */
...
static /*@null@*/
    poptItem
    poptFreeItems(/*@only@*/ /*@null@*/ poptItem items, int nitems)
/*@modifies items @*/
{
    if (items != NULL)
    {
...
int poptAddItem(poptContext con, poptItem newItem, int flags)
{
    ...
    *items = realloc((*items), ((*nitems) + 1) * sizeof(**items));
    ...
    item->option.argDescrip =
        (newItem->option.argDescrip ? xstrdup(newItem->option.argDescrip)
                                    : NULL);
