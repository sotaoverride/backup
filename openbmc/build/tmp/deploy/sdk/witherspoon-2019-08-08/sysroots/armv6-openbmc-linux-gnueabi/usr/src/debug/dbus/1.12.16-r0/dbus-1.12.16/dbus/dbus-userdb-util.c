                                                ...
                                                DBusError* error)
{
    ...
    if (gid != DBUS_GID_UNSET)
        ...
        info = _dbus_hash_table_lookup_string(
            db->groups_by_name, _dbus_string_get_const_data(groupname));
    if (info)
    {
    ...
    else
    {
        ...
        if (!_dbus_hash_table_insert_string(db->groups_by_name, info->groupname,
                                            info))
        {
...
{
    *info =
        _dbus_user_database_lookup_group(db, DBUS_GID_UNSET, groupname, error);
    return *info != NULL;
}
 ...
 * @returns #FALSE if error is set
 */
dbus_bool_t _dbus_user_database_get_gid(DBusUserDatabase* db, dbus_gid_t gid,
                                        const DBusGroupInfo** info,
                                        DBusError* error)
{
 ...
 */
dbus_bool_t _dbus_groups_from_uid(dbus_uid_t uid, dbus_gid_t** group_ids,
                                  int* n_group_ids)
{
    DBusUserDatabase* db;
    const DBusUserInfo* info;
    ...
    *n_group_ids = 0;
    ...
    if (info->n_group_ids > 0)
    {
        ...
        *n_group_ids = info->n_group_ids;
        ...
        memcpy(*group_ids, info->group_ids,
               info->n_group_ids * sizeof(dbus_gid_t));
    }
