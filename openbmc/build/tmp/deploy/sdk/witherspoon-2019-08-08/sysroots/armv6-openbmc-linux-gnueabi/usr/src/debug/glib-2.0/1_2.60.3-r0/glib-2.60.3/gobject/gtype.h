 ...
 * is then a plain memory copy of the contents of TypeAClass into TypeBClass and
...
/**
 * GTypeInfo:
 * @class_size: Size of the class structure (required for interface, classed and
 ...
 * current contents. To extend on our above string example:
 *  |[<!-- language="C" -->
 ...
 * the values collected from a variable argument list into contents suitable for
 * storage in a GValue. This function should setup
 ...
 * into @collect_values according to @lcopy_format.
 ...
 *declaration of the instance structure, it is not possible to subclass the type
 *   and therefore the fact that the size of the class structure is exposed is
 *not a concern and it can be freely changed at any point in the future.
 *
 ...
 *
 * The declaration of the _get_type() function is the first thing emitted by the
 *macro.  This allows this macro to be used in the usual way with export control
 *and API versioning macros.
 *
 ...
 *convert a type from using G_DECLARE_FINAL_TYPE() to G_DECLARE_DERIVABLE_TYPE()
 *without breaking API or ABI.  As a precaution, you should therefore use
 *G_DECLARE_FINAL_TYPE() until you are sure that it makes sense for your class
 *to be subclassed.  Once a class structure has been exposed it is not possible
 *to change its size or remove or reorder items without breaking the API and/or
 *ABI.
 *
 * Since: 2.44
 **/
#define G_DECLARE_FINAL_TYPE(ModuleObjName, module_obj_name, MODULE, OBJ_NAME, \
                             ParentName)                                       \
 ...
 *
 * A convenience macro for emitting the usual declarations in the header file
 *for a type which will is intended to be subclassed.
 *
 * You might use it in a header as follows:
 *
 * |[
 ...
 *instance variables.
 *
 * - the GtkFrobberClass type is defined as a typedef to struct
 *_GtkFrobberClass, which is left undefined. You should do this from the header
 *file directly after you use the macro.
 *
 * - the GTK_FROBBER() and GTK_FROBBER_CLASS() casts are emitted as static
 *inline functions along with the GTK_IS_FROBBER() and GTK_IS_FROBBER_CLASS()
 *type checking functions and GTK_FROBBER_GET_CLASS() function.
 *
 * - g_autoptr() support being added for your type, based on the type of your
 *parent class
 *
 ...
 *to be subclassed.  Once a class structure has been exposed it is not possible
 *to change its size or remove or reorder items without breaking the API and/or
 *ABI.  If you want to declare your own class structure, use
 * G_DECLARE_DERIVABLE_TYPE().  If you want to declare a class without exposing
 *the class or instance structures, use G_DECLARE_FINAL_TYPE().
 *
 * If you must use G_DECLARE_DERIVABLE_TYPE() you should be sure to include some
 *padding at the bottom of your class structure to leave space for the addition
 *of future virtual functions.
 *
 * Since: 2.44
 **/
#define G_DECLARE_DERIVABLE_TYPE(ModuleObjName, module_obj_name, MODULE,       \
                                 OBJ_NAME, ParentName)                         \
    GType module_obj_name##_get_type(void);                                    \
    G_GNUC_BEGIN_IGNORE_DEPRECATIONS                                           \
    typedef struct _##ModuleObjName ModuleObjName;                             \
    ...
    static inline ModuleObjName##Class* MODULE##_##OBJ_NAME##_CLASS(           \
        gpointer ptr)                                                          \
    {                                                                          \
        return G_TYPE_CHECK_CLASS_CAST(ptr, module_obj_name##_get_type(),      \
                                       ModuleObjName##Class);                  \
    }                                                                          \
    static inline gboolean MODULE##_IS_##OBJ_NAME(gpointer ptr)                \
    {                                                                          \
    ...
    static inline ModuleObjName##Class* MODULE##_##OBJ_NAME##_GET_CLASS(       \
        gpointer ptr)                                                          \
    {                                                                          \
        return G_TYPE_INSTANCE_GET_CLASS(ptr, module_obj_name##_get_type(),    \
                                         ModuleObjName##Class);                \
    }                                                                          \
    G_GNUC_END_IGNORE_DEPRECATIONS
 ...
 * @MODULE: The name of the module, in all caps (like 'GTK')
 * @OBJ_NAME: The bare name of the type, in all caps (like 'WIDGET')
 * @PrerequisiteName: the name of the prerequisite type, in camel case (like
 *GtkWidget)
 *
 * A convenience macro for emitting the usual declarations in the header file
 ...
 *_MyModelInterface, which is left undefined. You should do this from the header
 *file directly after you use the macro.
 *
 ...
 * Since: 2.44
 **/
#define G_DECLARE_INTERFACE(ModuleObjName, module_obj_name, MODULE, OBJ_NAME,  \
                            PrerequisiteName)                                  \
    GType module_obj_name##_get_type(void);                                    \
    G_GNUC_BEGIN_IGNORE_DEPRECATIONS                                           \
    typedef struct _##ModuleObjName ModuleObjName;                             \
    ...
    static inline ModuleObjName##Interface* MODULE##_##OBJ_NAME##_GET_IFACE(   \
        gpointer ptr)                                                          \
    {                                                                          \
        return G_TYPE_INSTANCE_GET_INTERFACE(                                  \
            ptr, module_obj_name##_get_type(), ModuleObjName##Interface);      \
    }                                                                          \
    G_GNUC_END_IGNORE_DEPRECATIONS
 ...
 * *_get_type() function, e.g. interface implementations via
 * G_IMPLEMENT_INTERFACE(). See G_DEFINE_TYPE_EXTENDED() for an example.
 *
 * Since: 2.4
 */
 ...
 *                                        (GClassInitFunc)
 * gtk_gadget_class_intern_init, sizeof (GtkGadget), (GInstanceInitFunc)
 * gtk_gadget_init, 0);
 *       {
 *         const GInterfaceInfo g_implement_interface_info = {
 *           (GInterfaceInitFunc) gtk_gadget_gizmo_init
 *         };
 *         g_type_add_interface_static (g_define_type_id, TYPE_GIZMO,
 ...
 * @iface_init: (type GInterfaceInitFunc): The interface init function, of type
 * #GInterfaceInitFunc
 *
...
/* Added for _G_DEFINE_TYPE_EXTENDED_WITH_PRELUDE */
#define _G_DEFINE_TYPE_EXTENDED_BEGIN_REGISTER(TypeName, type_name,            \
                                               TYPE_PARENT, flags)             \
    if (g_once_init_enter(&g_define_type_id__volatile))                        \
    {                                                                          \
    ...
    {                                                                          \
        GType g_define_type_id = g_type_register_static_simple(                \
            TYPE_PARENT, g_intern_static_string(#TypeName),                    \
...
/* This was defined before we had G_DEFINE_TYPE_WITH_CODE_AND_PRELUDE, it's
 * simplest to keep it.
 */
#define _G_DEFINE_TYPE_EXTENDED_BEGIN(TypeName, type_name, TYPE_PARENT, flags) \
    _G_DEFINE_TYPE_EXTENDED_BEGIN_PRE(TypeName, type_name, TYPE_PARENT)        \
    _G_DEFINE_TYPE_EXTENDED_BEGIN_REGISTER(TypeName, type_name, TYPE_PARENT,   \
                                           flags)
...
#define _G_DEFINE_INTERFACE_EXTENDED_BEGIN(TypeName, type_name, TYPE_PREREQ)   \
    ...
    {                                                                          \
        ...
        {                                                                      \
            GType g_define_type_id = g_type_register_static_simple(            \
                G_TYPE_INTERFACE, g_intern_static_string(#TypeName),           \
                sizeof(TypeName##Interface),                                   \
                (GClassInitFunc)(void (*)(void))type_name##_default_init, 0,   \
 ...
 *                                gdk_rectangle_copy,
 *                                gdk_rectangle_free,
 *                                register_rectangle_transform_funcs
 * (g_define_type_id))
 * ]|
 *
...
#define _G_DEFINE_BOXED_TYPE_BEGIN(TypeName, type_name, copy_func, free_func)  \
    ...
    {                                                                          \
        ...
        GType g_define_type_id = _g_register_boxed(                            \
            g_intern_static_string(#TypeName), copy_func, free_func);          \
        { /* custom code follows */
#else
#define _G_DEFINE_BOXED_TYPE_BEGIN(TypeName, type_name, copy_func, free_func)  \
    static GType type_name##_get_type_once(void);                              \
    ...
    {                                                                          \
        GType g_define_type_id = g_boxed_type_register_static(                 \
            g_intern_static_string(#TypeName), (GBoxedCopyFunc)copy_func,      \
...
gboolean g_type_check_instance_is_fundamentally_a(
    GTypeInstance* instance, GType fundamental_type) G_GNUC_PURE;
GLIB_AVAILABLE_IN_ALL
...
#define _G_TYPE_CIFT(ip, ft)                                                   \
    (g_type_check_instance_is_fundamentally_a((GTypeInstance*)ip, ft))
#ifdef __GNUC__
...
#define _G_TYPE_CVH(vl, gt)                                                    \
    (G_GNUC_EXTENSION({                                                        \
        const GValue* __val = (const GValue*)vl;                               \
        GType __t = gt;                                                        \
        gboolean __r;                                                          \
        if (!__val)                                                            \
            __r = FALSE;                                                       \
