...
int custom_mount_prepare_all(const char* dest, CustomMount* l, size_t n)
{
    ...
    /* Prepare all custom mounts. This will make source we know all temporary
     * directories. This is called in the parent process, so that we know the
     * temporary directories to remove on exit before we fork off the
     * children. */
    ...
    for (i = 0; i < n; i++)
    {
        ...
        /* /proc we mount in the inner child, i.e. when we acquired
         * CLONE_NEWPID. All other mounts we mount already in the outer child,
         * so that the mounts are already established before CLONE_NEWPID and in
         * particular CLONE_NEWUSER. This also means any custom mounts below
         * /proc also need to be mounted in
        ...
        if (m->type == CUSTOM_MOUNT_BIND)
        {
            ...
            {
                /* No source specified? In that case, use a throw-away temporary
                ...
                if (!mkdtemp(m->rm_rf_tmpdir))
                {
                    ...
                    return log_error_errno(
                        errno, "Failed to acquire temporary directory: %m");
                }
        ...
        if (m->type == CUSTOM_MOUNT_OVERLAY)
        {
            ...
            else
            {
                ...
                if (r < 0)
                    return log_error_errno(
                        r, "Failed to acquire working directory: %m");
            }
...
int bind_mount_parse(CustomMount** l, size_t* n, const char* s, bool read_only)
{
    ...
    r = extract_many_words(&p, ":", EXTRACT_DONT_COALESCE_SEPARATORS, &source,
                           &destination, NULL);
    if (r < 0)
        return r;
    if (r == 0)
...
int overlay_mount_parse(CustomMount** l, size_t* n, const char* s,
                        bool read_only)
{
    ...
    if (k == 2)
    {
        /* If two parameters are specified, the first one is the lower, the
    ...
    else
    {
        ...
        /* If more than two parameters are specified, the last one is the
         * destination, the second to last one
        ...
        /* If the upper directory is unspecified, then let's create it
         * automatically as a throw-away directory in /var/tmp */
        if (isempty(upper))
            upper = mfree(upper);
...
int tmpfs_patch_options(const char* options, uid_t uid_shift,
                        const char* selinux_apifs_context, char** ret)
{
    ...
    if (uid_shift != UID_INVALID)
    {
        if (asprintf(&buf, "%s%suid=" UID_FMT ",gid=" UID_FMT,
                     strempty(options), options ? "," : "", uid_shift,
                     uid_shift) < 0)
            return -ENOMEM;
    ...
    if (selinux_apifs_context)
    {
        ...
        t = strjoin(strempty(options), options ? "," : "", "context=\"",
...
int mount_sysfs(const char* dest, MountSettingsMask mount_settings)
{
    ...
    if (r < 0)
        return log_error_errno(
            r, "Failed to determine filesystem type of %s: %m", top);
     ...
     * don't have the right to do so, see
    ...
    FOREACH_STRING(x, "block", "bus", "class", "dev", "devices", "kernel")
    {
        ...
        r = mount_verbose(LOG_ERR, NULL, to, NULL,
                          MS_BIND | MS_NOSUID | MS_NOEXEC | MS_NODEV |
                              MS_REMOUNT | extra_flags,
                          NULL);
        if (r < 0)
            return r;
    }
...
static int mkdir_userns_p(const char* prefix, const char* path, mode_t mode,
                          uid_t uid_shift)
{
    const char *p, *e;
    int r;
...
int mount_all(const char* dest, MountSettingsMask mount_settings,
              uid_t uid_shift, const char* selinux_apifs_context)
{
...
#define PROC_READ_ONLY(path)                                                   \
    ...
    static const MountPoint mount_table[] = {
        /* First we list inner child mounts (i.e. mounts applied *after*
           entering user namespacing) */
        {"proc", "/proc", "proc", NULL, MS_NOSUID | MS_NOEXEC | MS_NODEV,
         MOUNT_FATAL | MOUNT_IN_USERNS},

        {"/proc/sys", "/proc/sys", NULL, NULL, MS_BIND,
         MOUNT_FATAL | MOUNT_IN_USERNS |
             MOUNT_APPLY_APIVFS_RO}, /* Bind mount first ... */
        ...
        {NULL, "/proc/sys", NULL, NULL,
         ...
         MOUNT_FATAL | MOUNT_IN_USERNS |
             MOUNT_APPLY_APIVFS_RO}, /* ... then, make it r/o */

        /* Make these files inaccessible to container payloads: they potentially
         * leak information about kernel internals or the host's execution
        ...
        /* Make these directories read-only to container payloads: they show
         * hardware information, and in some cases contain tunables the
         * container really shouldn't have access to. */
        ...
        {"mqueue", "/dev/mqueue", "mqueue", NULL,
         MS_NOSUID | MS_NOEXEC | MS_NODEV, MOUNT_IN_USERNS},
        ...
        {"sysfs", "/sys", "sysfs", NULL,
         ...
         MOUNT_FATAL |
             MOUNT_APPLY_APIVFS_RO}, /* skipped if above was mounted */
        ...
        {"tmpfs", "/dev/shm", "tmpfs", "mode=1777",
         MS_NOSUID | MS_NODEV | MS_STRICTATIME, MOUNT_FATAL},
    ...
    for (k = 0; k < ELEMENTSOF(mount_table); k++)
    {
        ...
        if (in_userns !=
            (bool)(mount_table[k].mount_settings & MOUNT_IN_USERNS))
            continue;

        if (!netns &&
            (bool)(mount_table[k].mount_settings & MOUNT_APPLY_APIVFS_NETNS))
            continue;

        if (!ro &&
            (bool)(mount_table[k].mount_settings & MOUNT_APPLY_APIVFS_RO))
            continue;

        if (!tmpfs_tmp &&
            (bool)(mount_table[k].mount_settings & MOUNT_APPLY_TMPFS_TMP))
            continue;

        r = chase_symlinks(mount_table[k].where, dest,
                           CHASE_NONEXISTENT | CHASE_PREFIX_ROOT, &where);
        if (r < 0)
            return log_error_errno(r, "Failed to resolve %s/%s: %m", dest,
                                   mount_table[k].where);
        ...
        if (mount_table[k].what)
        {
            ...
            if (r < 0 && r != -ENOENT)
                return log_error_errno(
                    r, "Failed to detect whether %s is a mount point: %m",
                    where);
            if (r > 0)
                continue;
        }

        r = mkdir_userns_p(dest, where, 0755,
                           (use_userns && !in_userns) ? uid_shift
                                                      : UID_INVALID);
        if (r < 0 && r != -EEXIST)
        {
            if (fatal && r != -EROFS)
                return log_error_errno(r, "Failed to create directory %s: %m",
                                       where);
        ...
        {
            r = tmpfs_patch_options(o, in_userns ? 0 : uid_shift,
                                    selinux_apifs_context, &options);
            if (r < 0)
                return log_oom();
            if (r > 0)
                o = options;
        ...
        r = mount_verbose(fatal ? LOG_ERR : LOG_DEBUG, mount_table[k].what,
                          where, mount_table[k].type, mount_table[k].flags, o);
        if (r < 0 && fatal)
            return r;
    }
...
static int mount_bind(const char* dest, CustomMount* m)
{
    ...
    r = chase_symlinks(m->destination, dest,
                       CHASE_PREFIX_ROOT | CHASE_NONEXISTENT, &where);
    if (r < 0)
        return log_error_errno(r, "Failed to resolve %s/%s: %m", dest,
                               m->destination);
    if (r > 0)
    ...
    { /* Path doesn't exist yet? */
        ...
        if (r < 0)
            return log_error_errno(r, "Failed to make parents of %s: %m",
                                   where);
        ...
        if (S_ISDIR(source_st.st_mode))
            ...
            r = touch(where);
        if (r < 0)
            return log_error_errno(r, "Failed to create mount point %s: %m",
                                   where);
    }

    r = mount_verbose(LOG_ERR, m->source, where, NULL, MS_BIND | MS_REC,
                      m->options);
    if (r < 0)
        return r;
...
static int mount_tmpfs(const char* dest, CustomMount* m, bool userns,
                       ...
                       const char* selinux_apifs_context)
{
    ...
    { /* Doesn't exist yet? */
        ...
        if (r < 0)
            return log_error_errno(
                r, "Creating mount point for tmpfs %s failed: %m", where);
    }

    r = tmpfs_patch_options(m->options,
                            uid_shift == 0 ? UID_INVALID : uid_shift,
                            selinux_apifs_context, &buf);
    if (r < 0)
        return log_oom();
    options = r > 0 ? buf : m->options;

    return mount_verbose(LOG_ERR, "tmpfs", where, "tmpfs",
                         MS_NODEV | MS_STRICTATIME, options);
}
...
static int mount_overlay(const char* dest, CustomMount* m)
{
    ...
    { /* Doesn't exist yet? */
        ...
        if (r < 0)
            return log_error_errno(
                r, "Creating mount point for overlay %s failed: %m", where);
    }
    ...
    else
    {
        ...
        options = strjoina("lowerdir=", lower, ",upperdir=", escaped_source,
                           ",workdir=", escaped_work_dir);
    }

    return mount_verbose(LOG_ERR, "overlay", where, "overlay",
...
static int mount_inaccessible(const char* dest, CustomMount* m)
{
    ...
    r = chase_symlinks_and_stat(m->destination, dest, CHASE_PREFIX_ROOT, &where,
                                &st);
    if (r < 0)
    {
        log_full_errno(m->graceful ? LOG_DEBUG : LOG_ERR, r,
                       "Failed to resolve %s/%s: %m", dest, m->destination);
        return m->graceful ? 0 : r;
    }

    assert_se(source = mode_to_inaccessible_node(st.st_mode));

    r = mount_verbose(m->graceful ? LOG_DEBUG : LOG_ERR, source, where, NULL,
                      MS_BIND, NULL);
    if (r < 0)
        return m->graceful ? 0 : r;

    r = mount_verbose(m->graceful ? LOG_DEBUG : LOG_ERR, NULL, where, NULL,
...
static int mount_arbitrary(const char* dest, CustomMount* m)
{
    ...
    { /* Doesn't exist yet? */
        ...
        if (r < 0)
            return log_error_errno(
                r, "Creating mount point for mount %s failed: %m", where);
    }

    return mount_verbose(LOG_ERR, m->source, where, m->type_argument, 0,
                         m->options);
}

int mount_custom(const char* dest, CustomMount* mounts, size_t n, bool userns,
                 uid_t uid_shift, uid_t uid_range,
                 const char* selinux_apifs_context, bool in_userns)
{
    ...
    for (i = 0; i < n; i++)
    {
        ...
        switch (m->type)
        {
            ...
            case CUSTOM_MOUNT_TMPFS:
                r = mount_tmpfs(dest, m, userns, uid_shift, uid_range,
                                selinux_apifs_context);
                break;
...
static int setup_volatile_state(const char* directory, bool userns,
                                uid_t uid_shift, uid_t uid_range,
                                const char* selinux_apifs_context)
{
    ...
    /* --volatile=state means we simply overmount /var with a tmpfs, and the
     * rest read-only. */

    r = bind_remount_recursive(directory, MS_RDONLY, MS_RDONLY, NULL);
    if (r < 0)
        return log_error_errno(r, "Failed to remount %s read-only: %m",
                               directory);
    ...
    options = "mode=755";
    r = tmpfs_patch_options(options, uid_shift == 0 ? UID_INVALID : uid_shift,
                            selinux_apifs_context, &buf);
    if (r < 0)
        return log_oom();
    if (r > 0)
...
static int setup_volatile_yes(const char* directory, bool userns,
                              uid_t uid_shift, uid_t uid_range,
                              const char* selinux_apifs_context)
{
    ...
    /* --volatile=yes means we mount a tmpfs to the root dir, and the original
       /usr to use inside it, and that read-only. */
    ...
    r = mount_verbose(LOG_ERR, "tmpfs", template, "tmpfs", MS_STRICTATIME,
                      options);
    if (r < 0)
        goto fail;
...
static int setup_volatile_overlay(const char* directory, bool userns,
                                  uid_t uid_shift, uid_t uid_range,
                                  const char* selinux_apifs_context)
{

    _cleanup_free_ char *buf = NULL, *escaped_directory = NULL,
                        *escaped_upper = NULL, *escaped_work = NULL;
    char template[] = "/tmp/nspawn-volatile-XXXXXX";
    ...
    /* And now, let's overmount the root dir with an overlayfs that uses the
     * root dir as lower dir. It's kinda nice that the kernel allows us to do
     * that without going through some mount point rearrangements. */
    ...
    options = strjoina("lowerdir=", escaped_directory,
                       ",upperdir=", escaped_upper, ",workdir=", escaped_work);
...
int setup_volatile_mode(const char* directory, VolatileMode mode, bool userns,
                        uid_t uid_shift, uid_t uid_range,
                        const char* selinux_apifs_context)
{
    ...
    switch (mode)
    {
        ...
        case VOLATILE_YES:
            return setup_volatile_yes(directory, userns, uid_shift, uid_range,
                                      selinux_apifs_context);

        case VOLATILE_STATE:
            return setup_volatile_state(directory, userns, uid_shift, uid_range,
                                        selinux_apifs_context);

        case VOLATILE_OVERLAY:
            return setup_volatile_overlay(directory, userns, uid_shift,
                                          uid_range, selinux_apifs_context);
...
/* Expects *pivot_root_new and *pivot_root_old to be initialised to allocated
 * memory or NULL. */
int pivot_root_parse(char** pivot_root_new, char** pivot_root_old,
                     const char* s)
{
    ...
    r = extract_first_word(&p, &root_new, ":",
...
int setup_pivot_root(const char* directory, const char* pivot_root_new,
                     const char* pivot_root_old)
{
    ...
    r = mount_verbose(LOG_ERR, directory_pivot_root_new,
                      directory_pivot_root_new, NULL, MS_BIND, NULL);
    if (r < 0)
        goto done;
    ...
    if (pivot_root_old)
    {
        ...
        r = mount_verbose(LOG_ERR, directory_pivot_root_new, pivot_tmp, NULL,
                          MS_MOVE, NULL);
        if (r < 0)
            goto done;

        r = mount_verbose(LOG_ERR, directory, pivot_tmp_pivot_root_old, NULL,
                          MS_MOVE, NULL);
        if (r < 0)
            goto done;
    ...
    {
        r = mount_verbose(LOG_ERR, directory_pivot_root_new, directory, NULL,
                          MS_MOVE, NULL);
        if (r < 0)
            goto done;
    }
