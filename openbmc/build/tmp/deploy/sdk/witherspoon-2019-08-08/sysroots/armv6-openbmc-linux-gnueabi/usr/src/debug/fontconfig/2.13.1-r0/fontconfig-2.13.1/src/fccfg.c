...
static FcChar32 FcHashAsStrIgnoreCase(const void* data)
{
    return FcStrHashIgnoreCase(data);
}

static int FcCompareAsStr(const void* v1, const void* v2)
{
...
static void FcDestroyAsStr(void* data)
{
    FcStrFree(data);
}
...
FcConfig* FcConfigCreate(void)
{
    ...
    config->uuid_table =
        FcHashTableCreate(FcHashAsStrIgnoreCase, FcCompareAsStr, FcHashStrCopy,
                          FcHashUuidCopy, FcDestroyAsStr, FcHashUuidFree);
...
FcBool FcConfigUptoDate(FcConfig* config)
{
    ...
    if ((config_time.set && config_time.time - config->rescanTime > 0) ||
        (config_dir_time.set &&
         (config_dir_time.time - config->rescanTime) > 0) ||
        (font_time.set && (font_time.time - config->rescanTime) > 0))
    {
        /* We need to check for potential clock problems here (OLPC ticket
         * #6046) */
        ...
        {
            fprintf(stderr, "Fontconfig warning: Directory/file mtime in the "
                            "future. New fonts may not be detected.\n");
...
FcBool FcConfigAddCache(FcConfig* config, FcCache* cache, FcSetName set,
                        FcStrSet* dirSet, FcChar8* forDir)
{
    FcFontSet* fs;
    intptr_t* dirs;
    int i;
    ...
    if (fs)
    {
        ...
        for (i = 0; i < fs->nfont; i++)
        {
            ...
            if (FcPatternObjectGetString(font, FC_FILE_OBJECT, 0, &font_file) ==
                FcResultMatch)
            {
                if (relocated)
                {
                    ...
                    relocated_font_file =
                        FcStrBuildFilename(forDir, slash + 1, NULL);
            ...
            {
                font =
                    FcPatternCacheRewriteFile(font, cache, relocated_font_file);
                free(relocated_font_file);
            }
...
static FcBool FcConfigAddDirList(FcConfig* config, FcSetName set,
                                 FcStrSet* dirSet)
{
    FcStrList* dirlist;
    FcChar8* dir;
...
FcBool FcConfigAddFontDir(FcConfig* config, const FcChar8* d)
{
...
FcChar8* FcConfigGetCache(FcConfig* config FC_UNUSED)
{
    return NULL;
}
...
void FcBlanksDestroy(FcBlanks* b FC_UNUSED)
{
    /* Deprecated. */
}

FcBool FcBlanksAdd(FcBlanks* b FC_UNUSED, FcChar32 ucs4 FC_UNUSED)
{
    /* Deprecated. */
    return FcFalse;
}

FcBool FcBlanksIsMember(FcBlanks* b FC_UNUSED, FcChar32 ucs4 FC_UNUSED)
{
    /* Deprecated. */
    return FcFalse;
}

FcBlanks* FcConfigGetBlanks(FcConfig* config FC_UNUSED)
{
    /* Deprecated. */
    return NULL;
}

FcBool FcConfigAddBlank(FcConfig* config FC_UNUSED, FcChar32 blank FC_UNUSED)
{
    /* Deprecated. */
    return FcFalse;
}
...
static FcValue FcConfigPromote(FcValue v, FcValue u,
...
FcBool FcConfigCompareValue(const FcValue* left_o, unsigned int op_,
        ...
        switch (left.type)
        {
            ...
            case FcTypeBool:
                switch ((int)op)
                {
                    ...
                    case FcOpNotContains:
                        ret =
                            !(left.u.b == right.u.b || left.u.b == FcDontCare);
                        break;
                    case FcOpLess:
                ...
                switch ((int)op)
                {
                    ...
                    case FcOpListing:
                        if (flags & FcOpFlagIgnoreBlanks)
                            ret = FcStrCmpIgnoreBlanksAndCase(left.u.s,
                                                              right.u.s) == 0;
                        else
                                ...
                                FcMatchKind kind, FcExpr* e)
{
    ...
    switch ((int)op)
    {
        ...
        case FcOpMatrix:
        {
            ...
            v.type = FcTypeMatrix;
            xx = FcConfigPromote(
                FcConfigEvaluate(p, p_pat, kind, e->u.mexpr->xx), v, NULL);
            xy = FcConfigPromote(
                FcConfigEvaluate(p, p_pat, kind, e->u.mexpr->xy), v, NULL);
            yx = FcConfigPromote(
                FcConfigEvaluate(p, p_pat, kind, e->u.mexpr->yx), v, NULL);
            yy = FcConfigPromote(
                FcConfigEvaluate(p, p_pat, kind, e->u.mexpr->yy), v, NULL);
        ...
        case FcOpField:
            ...
            {
                if (FcResultMatch !=
                    FcPatternObjectGet(p_pat, e->u.name.object, 0, &v))
                    v.type = FcTypeVoid;
            }
            else if (kind == FcMatchPattern && e->u.name.kind == FcMatchFont)
            {
                fprintf(stderr,
                        "Fontconfig warning: <name> tag has target=\"font\" in "
                        "a <match target=\"pattern\">.\n");
                v.type = FcTypeVoid;
            }
            else
            {
                if (FcResultMatch !=
                    FcPatternObjectGet(p, e->u.name.object, 0, &v))
                    v.type = FcTypeVoid;
            }
            v = FcValueSave(v);
            break;
        ...
        case FcOpQuest:
            ...
            {
                if (vl.u.b)
                    v = FcConfigEvaluate(p, p_pat, kind,
                                         e->u.tree.right->u.tree.left);
                else
                    v = FcConfigEvaluate(p, p_pat, kind,
                                         e->u.tree.right->u.tree.right);
            }
            else
                v.type = FcTypeVoid;
            FcValueDestroy(vl);
...
static FcValueList* FcConfigMatchValueList(FcPattern* p, FcPattern* p_pat,
                                           FcMatchKind kind, FcTest* t,
                                           FcValueList* values)
{
    FcValueList* ret = 0;
    FcExpr* e = t->expr;
...
static FcValueList* FcConfigValues(FcPattern* p, FcPattern* p_pat,
                                   FcMatchKind kind, FcExpr* e,
...
static FcBool FcConfigAdd(FcValueListPtr* head, FcValueList* position,
                          FcBool append, FcValueList* new, FcObject object)
{
...
static void FcConfigDel(FcValueListPtr* head, FcValueList* position)
{
    FcValueListPtr* prev;
...
static void FcConfigPatternAdd(FcPattern* p, FcObject object, FcValueList* list,
                               FcBool append)
{
    if (list)
    {
 ...
 */
static void FcConfigPatternDel(FcPattern* p, FcObject object)
{
                                 ...
                                 FcPattern* p_pat, FcMatchKind kind)
{
        ...
        if (strs)
        {
            ...
            while (l && (lang = FcStrListNext(l)))
            {
                ...
                if (e)
                {
                    ...
                    for (ll = FcPatternEltValues(e); ll;
                         ll = FcValueListNext(ll))
                    {
    ...
    for (; FcPtrListIterIsValid(s, &iter); FcPtrListIterNext(s, &iter))
    {
        ...
        for (; FcPtrListIterIsValid(rs->subst[kind], &iter2);
             FcPtrListIterNext(rs->subst[kind], &iter2))
        {
            ...
            for (; r; r = r->next)
            {
                switch (r->type)
                {
                    ...
                    case FcRuleTest:
                         ...
                         */
                        vl = FcConfigMatchValueList(m, p_pat, kind, r->u.test,
                                                    e->values);
                        /* different 'kind' won't be the target of edit */
                        if (!value[object] && kind == r->u.test->kind)
                            value[object] = vl;
                        if (!vl ||
                            (r->u.test->qual == FcQualFirst &&
                             vl != e->values) ||
                            (r->u.test->qual == FcQualNotFirst &&
                             vl == e->values))
                        {
                            if (FcDebug() & FC_DBG_EDIT)
                                printf("No match\n");
                    ...
                    case FcRuleEdit:
                         ...
                         */
                        l = FcConfigValues(p, p_pat, kind, r->u.edit->expr,
                                           r->u.edit->binding);
                        if (tst[object] && (tst[object]->kind == FcMatchFont ||
                                            kind == FcMatchPattern))
                            elt[object] =
                                FcPatternObjectFindElt(p, tst[object]->object);
                        ...
                        switch (FC_OP_GET_OP(r->u.edit->op))
                        {
                            case FcOpAssign:
                                ...
                                if (value[object])
                                {
                                    ...
                                    /*
                                     * Append the new list of values after the
                                     * current value
                                     */
                                    FcConfigAdd(&elt[object]->values, thisValue,
                                                FcTrue, l, r->u.edit->object);
                                    /*
                                     * Delete the marked value
                                     */
                                    if (thisValue)
                                        FcConfigDel(&elt[object]->values,
                                                    thisValue);
                                    /*
                                     * Adjust a pointer into the value list to
                                     * ensure future edits occur at the same
                                     * place
                                     */
                            ...
                            case FcOpAssignReplace:
                                ...
                                FcConfigPatternAdd(p, r->u.edit->object, l,
                                                   FcTrue);
                                /*
                                 * Adjust a pointer into the value list as they
                                 * no longer point to anything valid
                                 */
                                value[object] = NULL;
                                break;
                            case FcOpPrepend:
                                if (value[object])
                                {
                                    FcConfigAdd(&elt[object]->values,
                                                value[object], FcFalse, l,
                                                r->u.edit->object);
                                    break;
                                }
                            /* fall through ... */
        ...
        case DLL_PROCESS_ATTACH:
            ...
            if (p)
            {
                *p = '\0';
                p = (FcChar8*)strrchr((const char*)fontconfig_path, '\\');
                if (p &&
                    (FcStrCmpIgnoreCase(p + 1, (const FcChar8*)"bin") == 0 ||
...
static FcChar8* FcConfigFileExists(const FcChar8* dir, const FcChar8* file)
{
    ...
    /*
     * workaround valgrind warning because glibc takes advantage of how it knows
     * memory is allocated to implement strlen by reading in groups of 4
     */
    size = (osize + 3) & ~3;
    ...
    else
    {
        ...
        if (ret)
        {
            if (home)
                memcpy(ret, home, len);
            memcpy(&ret[len],
                   FC_DIR_SEPARATOR_S ".local" FC_DIR_SEPARATOR_S "share", 13);
            ret[len + 13] = 0;
        }
    }
...
static FcBool FcConfigPatternsMatch(const FcFontSet* patterns,
...
void FcRuleSetAddDescription(FcRuleSet* rs, const FcChar8* domain,
...
int FcRuleSetAdd(FcRuleSet* rs, FcRule* rule, FcMatchKind kind)
{
    ...
    if (!rs || kind < FcMatchKindBegin || kind >= FcMatchKindEnd)
        return -1;
                               ...
                               FcBool* enabled)
{
    ...
    if (name)
        *name = FcStrdup(r->name && r->name[0] ? r->name
                                               : (const FcChar8*)"fonts.conf");
    if (description)
        *description = FcStrdup(
            !r->description ? _("No description")
                            : dgettext(r->domain ? (const char*)r->domain
                                                 : GETTEXT_PACKAGE "-conf",
