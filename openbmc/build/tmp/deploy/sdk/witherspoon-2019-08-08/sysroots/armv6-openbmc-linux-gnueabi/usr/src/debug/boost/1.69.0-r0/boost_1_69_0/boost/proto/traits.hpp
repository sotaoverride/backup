...
#endif
#pragma warning(                                                               \
    disable : 4714) // function 'xxx' marked as __forceinline not inlined
#endif
...
struct is_callable<proto::transform<PrimitiveTransform, X>> : mpl::false_
{
    ...
    (BOOST_WORKAROUND(__GNUC__, == 4) && __GNUC_MINOR__ == 0)
// work around GCC bug
...
/// the case for any type that derives from an instantiation of \c
/// proto::transform.) \li Otherwise, <tt>is_transform\<T\>::value</tt> is \c
/// false.
template <typename T>
...
struct value
{
    ...
    /// \li <tt>T</tt> becomes <tt>T</tt>
    typedef
        typename detail::term_traits<typename Expr::proto_child0>::value_type
            type;
};
...
struct value<Expr&>
{
    ...
    /// \li <tt>T &</tt> becomes <tt>T &</tt>
    /// \li <tt>T</tt> becomes <tt>T &</tt>
    typedef typename detail::term_traits<typename Expr::proto_child0>::reference
        type;
};
...
struct value<Expr const&>
{
    ...
    typedef typename detail::term_traits<
...
struct if_else_ : proto::transform<if_else_<T, U, V>, int>
{
    ...
    typedef proto::basic_expr<proto::tag::if_else_, list3<T, U, V>, 3>
        proto_grammar;
...
struct nullary_expr : proto::transform<nullary_expr<Tag, T>, int>
{
    ...
    struct impl : transform_impl<Expr, State, Data>
    {
        ...
        /// \param e The current expression
        /// \pre <tt>matches\<Expr, nullary_expr\<Tag, T\> \>::value</tt> is \c
        /// true. \return \c e \throw nothrow
        BOOST_FORCEINLINE
        BOOST_PROTO_RETURN_TYPE_STRICT_LOOSE(result_type,
                                             typename impl::expr_param)
        operator()(typename impl::expr_param e, typename impl::state_param,
...
struct unary_expr : proto::transform<unary_expr<Tag, T>, int>
{
    ...
    template <typename Expr, typename State, typename Data>
    struct impl : detail::pass_through_impl<unary_expr, deduce_domain, Expr,
                                            State, Data>
    {
    };
...
struct binary_expr : proto::transform<binary_expr<Tag, T, U>, int>
{
    ...
    template <typename Expr, typename State, typename Data>
    struct impl : detail::pass_through_impl<binary_expr, deduce_domain, Expr,
                                            State, Data>
    {
    };
...
#define BOOST_PROTO_DEFINE_BINARY_METAFUNCTION(Op)                             \
    ...
    {                                                                          \
        ...
        typedef proto::basic_expr<proto::tag::Op, list2<T, U>, 2>              \
            proto_grammar;                                                     \
                                                                               \
        template <typename Expr, typename State, typename Data>                \
        struct impl                                                            \
...
struct as_expr
{
    ...
    template <typename T, std::size_t N_>
    BOOST_FORCEINLINE typename add_const<
        typename result<as_expr(T const (&)[N_])>::type>::type
...
struct child_c
{
    ...
    ///
    template <typename Expr>
    BOOST_FORCEINLINE typename result_of::child_c<Expr const&, N>::type
...
struct child
{
    ...
    /// \throw nothrow
    template <typename Expr>
    BOOST_FORCEINLINE typename result_of::child<Expr&, N>::type
        operator()(Expr& e) const
    {
    ...
    ///
    template <typename Expr>
    BOOST_FORCEINLINE typename result_of::child<Expr const&, N>::type
        operator()(Expr const& e) const
    {
...
struct value
{
    ...
    /// \throw nothrow
    template <typename Expr>
    BOOST_FORCEINLINE typename result_of::value<Expr&>::type
        operator()(Expr& e) const
    {
    ...
    ///
    template <typename Expr>
    BOOST_FORCEINLINE typename result_of::value<Expr const&>::type
        operator()(Expr const& e) const
    {
        return e.proto_base().child0;
    }
...
struct left
{
    ...
    /// \throw nothrow
    template <typename Expr>
    BOOST_FORCEINLINE typename result_of::left<Expr&>::type
        operator()(Expr& e) const
    {
        return e.proto_base().child0;
    }
    ...
    ///
    template <typename Expr>
    BOOST_FORCEINLINE typename result_of::left<Expr const&>::type
        operator()(Expr const& e) const
    {
        return e.proto_base().child0;
    }
...
struct right
{
    ...
    /// \throw nothrow
    template <typename Expr>
    BOOST_FORCEINLINE typename result_of::right<Expr&>::type
        operator()(Expr& e) const
    {
    ...
    template <typename Expr>
    BOOST_FORCEINLINE typename result_of::right<Expr const&>::type
        operator()(Expr const& e) const
    {
        return e.proto_base().child1;
    }
...
template <typename T>
BOOST_FORCEINLINE typename add_const<
    typename result_of::as_expr<T, default_domain>::type>::type
    as_expr(T& t BOOST_PROTO_DISABLE_IF_IS_CONST(T)
                BOOST_PROTO_DISABLE_IF_IS_FUNCTION(T))
{
...
template <typename T>
BOOST_FORCEINLINE typename add_const<
    typename result_of::as_expr<T const, default_domain>::type>::type
    as_expr(T const& t)
{
...
template <typename T>
BOOST_FORCEINLINE typename add_const<
    typename result_of::as_child<T, default_domain>::type>::type
...
template <typename T>
BOOST_FORCEINLINE typename add_const<
    typename result_of::as_child<T const, default_domain>::type>::type
    as_child(T const& t)
{
...
template <typename Domain, typename T>
BOOST_FORCEINLINE typename add_const<
    typename result_of::as_child<T const, Domain>::type>::type
    as_child(T const& t)
{
    return typename Domain::template as_child<T const>()(t);
}
...
BOOST_FORCEINLINE typename detail::expr_traits<
    typename Expr2::proto_base_expr::proto_child0>::reference
...
template <typename Expr2>
BOOST_FORCEINLINE typename detail::expr_traits<
    typename Expr2::proto_base_expr::proto_child0>::const_reference
