...
/* The LLDP spec calls this msgTxInterval, but we subtract half the jitter off
 * it. */
...
static int lldp_make_packet(LLDPEmit mode, const struct ether_addr* hwaddr,
                            const char* machine_id, const char* ifname,
                            uint16_t ttl, const char* port_description,
                            const char* hostname, const char* pretty_hostname,
                            uint16_t system_capabilities,
                            uint16_t enabled_capabilities, void** ret,
                            size_t* sz)
{

    size_t machine_id_length, ifname_length, port_description_length = 0,
                                             hostname_length = 0,
    ...
    r = lldp_write_tlv_header(&p, SD_LLDP_TYPE_CHASSIS_ID,
                              1 + machine_id_length);
    if (r < 0)
        return r;
    ...
    *(p++) = SD_LLDP_PORT_SUBTYPE_INTERFACE_NAME;
    ...
    if (port_description)
    {
        r = lldp_write_tlv_header(&p, SD_LLDP_TYPE_PORT_DESCRIPTION,
    ...
    if (hostname)
    {
        r = lldp_write_tlv_header(&p, SD_LLDP_TYPE_SYSTEM_NAME,
                                  hostname_length);
        if (r < 0)
            return r;
    ...
    if (pretty_hostname)
    {
        r = lldp_write_tlv_header(&p, SD_LLDP_TYPE_SYSTEM_DESCRIPTION,
...
static int lldp_send_packet(int ifindex, const struct ether_addr* address,
...
static int link_send_lldp(Link* link)
{
    ...
    (void)parse_env_file(NULL, "/etc/machine-info", "PRETTY_HOSTNAME",
                         &pretty_hostname);

    assert_cc(LLDP_TX_INTERVAL_USEC * LLDP_TX_HOLD + 1 <=
              (UINT16_MAX - 1) * USEC_PER_SEC);
    ...
    r = lldp_make_packet(link->network->lldp_emit, &link->mac,
                         sd_id128_to_string(machine_id, machine_id_string),
                         link->ifname, (uint16_t)ttl,
                         link->network ? link->network->description : NULL,
                         hostname, pretty_hostname,
                         SD_LLDP_SYSTEM_CAPABILITIES_STATION |
                             SD_LLDP_SYSTEM_CAPABILITIES_BRIDGE |
                             SD_LLDP_SYSTEM_CAPABILITIES_ROUTER,
                         caps, &packet, &packet_size);
    if (r < 0)
        return r;

    return lldp_send_packet(link->ifindex,
                            lldp_multicast_addr + link->network->lldp_emit,
                            packet, packet_size);
}
...
static int on_lldp_timer(sd_event_source* s, usec_t t, void* userdata)
{
    ...
    assert_se(sd_event_now(sd_event_source_get_event(s),
                           clock_boottime_or_monotonic(), &current) >= 0);

    delay = link->lldp_tx_fast > 0 ? LLDP_FAST_TX_USEC : LLDP_TX_INTERVAL_USEC;
    next = usec_add(usec_add(current, delay),
...
int link_lldp_emit_start(Link* link)
{
    ...
    /* Starts the LLDP transmission in "fast" mode. If it is already started,
     * turns "fast" mode back on again. */

    link->lldp_tx_fast = LLDP_TX_FAST_INIT;

    next = usec_add(
        usec_add(now(clock_boottime_or_monotonic()), LLDP_FAST_TX_USEC),
        (usec_t)random_u64() % LLDP_JITTER_USEC);
    ...
    {
        r = sd_event_add_time(
            link->manager->event, &link->lldp_emit_event_source,
            clock_boottime_or_monotonic(), next, 0, on_lldp_timer, link);
        if (r < 0)
            return r;

        (void)sd_event_source_set_description(link->lldp_emit_event_source,
                                              "lldp-tx");
    }
...
void link_lldp_emit_stop(Link* link)
{
    ...
    link->lldp_emit_event_source =
        sd_event_source_unref(link->lldp_emit_event_source);
}

int config_parse_lldp_emit(const char* unit, const char* filename,
