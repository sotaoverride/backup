...
static RETSIGTYPE listen_int_handler(signo) int signo;
{
    putc('\n', stdout);
    longjmp(listen_jmpb, 1);
}

int ss_listen(sci_idx) int sci_idx;
{
    char* cp;
    ss_data* info;
    char* input;
...
void ss_abort_subsystem(sci_idx, code) int sci_idx;
int code;
{
...
void ss_quit(argc, argv, sci_idx, infop) int argc;
