...
static krb5_error_code get_db_opt(char* input, char** opt, char** val)
{
...
static void ctx_clear(krb5_db2_context* dbc)
{
    /*
...
static krb5_error_code ctx_get(krb5_context context, krb5_db2_context** dbc_out)
{
...
static krb5_error_code configure_context(krb5_context context,
                                         char* conf_section, char** db_args)
{
    ...
    for (t_ptr = db_args; t_ptr && *t_ptr; t_ptr++)
    {
        ...
        {
            status = EINVAL;
            k5_setmsg(context, status, _("Unsupported argument \"%s\" for db2"),
                      opt ? opt : val);
            goto cleanup;
        }
    }
    ...
    if (dbc->db_name == NULL)
    {
        ...
        if (status == 0 && pval == NULL)
        {
            ...
            status = profile_get_string(
                profile, KDB_REALM_SECTION, KRB5_DB_GET_REALM(context),
                KDB_DB2_DATABASE_NAME, DEFAULT_KDB_FILE, &pval);
        }
        if (status != 0)
            goto cleanup;
 ...
 * SUFFIX_POLICY_LOCK.
 */
static krb5_error_code ctx_dbsuffix(krb5_db2_context* dbc, const char* sfx,
                                    char** out)
{
    char* result;
    const char* tilde;
...
static krb5_error_code ctx_allfiles(krb5_db2_context* dbc, char** dbname_out,
                                    char** lockname_out, char** polname_out,
                                    char** plockname_out)
{
 ...
 */
static krb5_error_code open_db(krb5_context context, krb5_db2_context* dbc,
                               int flags, int mode, DB** db_out)
{
    char* fname = NULL;
    DB* db;
    BTREEINFO bti;
    ...
    *db_out = NULL;
    ...
    db = dbopen(fname, flags, mode, dbc->hashfirst ? DB_HASH : DB_BTREE,
                dbc->hashfirst ? (void*)&hashi : (void*)&bti);

    if (db == NULL && IS_EFTYPE(errno))
    {
        db = dbopen(fname, flags, mode, dbc->hashfirst ? DB_BTREE : DB_HASH,
...
static krb5_error_code ctx_unlock(krb5_context context, krb5_db2_context* dbc)
{
    ...
    if (--(dbc->db_locks_held) == 0)
    {
        ...
        retval2 =
            krb5_lock_file(context, dbc->db_lf_file, KRB5_LOCKMODE_UNLOCK);
        if (retval2)
            return retval2;
    }
...
static krb5_error_code ctx_lock(krb5_context context, krb5_db2_context* dbc,
                                int lockmode)
{
...
static krb5_error_code ctx_init(krb5_db2_context* dbc)
{
    krb5_error_code retval;
...
static void ctx_fini(krb5_db2_context* dbc)
{
    if (dbc->db_lf_file != -1)
...
krb5_error_code krb5_db2_fini(krb5_context context)
{
...
static krb5_error_code check_openable(krb5_context context)
{
    krb5_error_code retval;
    DB* db;
    krb5_db2_context* dbc;
...
krb5_error_code krb5_db2_get_age(krb5_context context, char* db_name,
                                 time_t* age)
{
    krb5_db2_context* dbc;
    struct stat st;
...
static void ctx_update_age(krb5_db2_context* dbc)
{
    struct stat st;
    time_t now;
...
krb5_error_code krb5_db2_lock(krb5_context context, int lockmode)
{
    if (!inited(context))
        return KRB5_KDB_DBNOTINITED;
...
krb5_error_code krb5_db2_unlock(krb5_context context)
{
    if (!inited(context))
        return KRB5_KDB_DBNOTINITED;
...
static krb5_error_code destroy_file(char* filename)
{
    struct stat statb;
...
static krb5_error_code ctx_create_db(krb5_context context,
                                     krb5_db2_context* dbc)
{
    krb5_error_code retval = 0;
    char *dbname = NULL, *polname = NULL, *plockname = NULL;

    retval = ctx_allfiles(dbc, &dbname, &dbc->db_lf_name, &polname, &plockname);
    if (retval)
        return retval;
...
krb5_error_code krb5_db2_get_principal(krb5_context context,
...
krb5_error_code krb5_db2_put_principal(krb5_context context,
...
krb5_error_code krb5_db2_delete_principal(krb5_context context,
                                          krb5_const_principal searchfor)
{
    krb5_error_code retval;
    krb5_db_entry* entry;
...
static krb5_error_code curs_lock(iter_curs* curs)
{
    krb5_error_code retval;
...
static void curs_unlock(iter_curs* curs)
{
...
static krb5_error_code curs_init(iter_curs* curs, krb5_context ctx,
                                 krb5_db2_context* dbc, krb5_flags iterflags)
{
    ...
    if (isrecurse)
    {
        ...
        if (dbc->hashfirst)
        {
            k5_setmsg(ctx, EINVAL,
                      _("Recursive iteration is not supported "
                        "for hash databases"));
            return EINVAL;
        }
        prevflag = R_RPREV;
...
#else
        k5_setmsg(ctx, EINVAL,
                  _("Recursive iteration not supported "
...
/* Get initial entry. */
static int curs_start(iter_curs* curs)
{
    DB* db = curs->dbc->db;
...
static krb5_error_code curs_save(iter_curs* curs)
{
...
static void curs_free(iter_curs* curs)
{
...
static int curs_step(iter_curs* curs)
{
    int dbret;
...
static krb5_error_code curs_run_cb(iter_curs* curs, ctx_iterate_cb func,
...
static void curs_fini(iter_curs* curs)
{
    curs_free(curs);
    if (curs->islocked)
        curs_unlock(curs);
...
static krb5_error_code ctx_iterate(krb5_context context, krb5_db2_context* dbc,
...
krb5_error_code krb5_db2_iterate(krb5_context context, char* match_expr,
                                 ctx_iterate_cb func, krb5_pointer func_arg,
                                 krb5_flags iterflags)
{
    if (!inited(context))
        return KRB5_KDB_DBNOTINITED;
    return ctx_iterate(context, context->dal_handle->db_context, func, func_arg,
                       iterflags);
}

krb5_boolean krb5_db2_set_lockmode(krb5_context context, krb5_boolean mode)
{
    krb5_boolean old;
    krb5_db2_context* dbc;
...
krb5_error_code krb5_db2_open(krb5_context context, char* conf_section,
...
krb5_error_code krb5_db2_create(krb5_context context, char* conf_section,
                                char** db_args)
{
    krb5_error_code status = 0;
    krb5_db2_context* dbc;
...
krb5_error_code krb5_db2_destroy(krb5_context context, char* conf_section,
                                 char** db_args)
{
    krb5_error_code status;
    krb5_db2_context* dbc;
...
/* policy functions */
krb5_error_code krb5_db2_create_policy(krb5_context context,
...
krb5_error_code krb5_db2_get_policy(krb5_context context, char* name,
                                    osa_policy_ent_t* policy)
{
    krb5_db2_context* dbc = context->dal_handle->db_context;
...
krb5_error_code krb5_db2_put_policy(krb5_context context,
                                    osa_policy_ent_t policy)
{
    krb5_db2_context* dbc = context->dal_handle->db_context;
...
krb5_error_code krb5_db2_iter_policy(krb5_context context, char* match_entry,
...
krb5_error_code krb5_db2_delete_policy(krb5_context context, char* policy)
{
    krb5_db2_context* dbc = context->dal_handle->db_context;
 ...
 */
static krb5_error_code krb5_db2_merge_principal(krb5_context context,
                                                krb5_db_entry* src,
                                                krb5_db_entry* dst,
                                                int* changed)
{
 ...
 * old database.
 */
static krb5_error_code krb5_db2_merge_nra_iterator(krb5_pointer ptr,
                                                   krb5_db_entry* entry)
{
 ...
 */
static krb5_error_code ctx_merge_nra(krb5_context context,
                                     krb5_db2_context* dbc_temp,
 ...
 */
static krb5_error_code ctx_promote(krb5_context context,
                                   krb5_db2_context* dbc_temp,
                                   krb5_db2_context* dbc_real)
{
    krb5_error_code retval;
...
krb5_error_code krb5_db2_promote_db(krb5_context context, char* conf_section,
                                    char** db_args)
{
    krb5_error_code retval;
...
void krb5_db2_audit_as_req(krb5_context kcontext, krb5_kdc_req* request,
