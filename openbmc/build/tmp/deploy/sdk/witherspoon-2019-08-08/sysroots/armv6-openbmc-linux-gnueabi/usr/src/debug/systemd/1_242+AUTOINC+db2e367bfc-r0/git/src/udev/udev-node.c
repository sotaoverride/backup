...
static int node_symlink(sd_device* dev, const char* node, const char* slink)
{
    ...
    if (r < 0)
        return log_device_error_errno(
            dev, r, "Failed to get relative path from '%s' to '%s': %m", slink,
            node);
    ...
    if (lstat(slink, &stats) == 0)
    {
        ...
        {
            log_device_error(dev,
                             "Conflicting device node '%s' found, link to '%s' "
                             "will not be created.",
                             slink, node);
            return -EOPNOTSUPP;
        }
        else if (S_ISLNK(stats.st_mode))
        {
            ...
            {
                log_device_debug(
                    dev, "Preserve already existing symlink '%s' to '%s'",
                    slink, target);
    ...
    if (r < 0)
        return log_device_error_errno(
            dev, r, "Failed to create symlink '%s' to '%s': %m", slink_tmp,
            target);

    if (rename(slink_tmp, slink) < 0)
    {
        r = log_device_error_errno(dev, errno,
                                   "Failed to rename '%s' to '%s' failed: %m",
                                   slink_tmp, slink);
        (void)unlink(slink_tmp);
    }
                                 ...
                                 char** ret)
{
    ...
    FOREACH_DIRENT_ALL(dent, dir, break)
    {
        ...
        log_device_debug(dev, "Found '%s' claiming '%s'", dent->d_name,
                         stackdir);
        ...
        log_device_debug(dev_db, "Device claims priority %i for '%s'", db_prio,
                         stackdir);
    ...
    if (add)
        do
        {
            ...
            if (!IN_SET(r, 0, -ENOENT))
                break;
            fd = open(filename,
                      O_WRONLY | O_CREAT | O_CLOEXEC | O_TRUNC | O_NOFOLLOW,
                      0444);
            if (fd < 0)
                r = -errno;
        } while (r == -ENOENT);
...
static int node_permissions_apply(sd_device* dev, bool apply, mode_t mode,
                                  uid_t uid, gid_t gid,
                                  OrderedHashmap* seclabel_list)
{
    ...
    if (((stats.st_mode & S_IFMT) != (mode & S_IFMT)) ||
        (stats.st_rdev != devnum))
        return log_device_debug_errno(
            dev, SYNTHETIC_ERRNO(EEXIST),
    ...
    if (apply)
    {
        ...
        if ((stats.st_mode & 0777) != (mode & 0777) || stats.st_uid != uid ||
            stats.st_gid != gid)
        {
            log_device_debug(dev, "Setting permissions %s, %#o, uid=%u, gid=%u",
                             devnode, mode, uid, gid);
            if (chmod(devnode, mode) < 0)
                r = log_device_warning_errno(
                    dev, errno, "Failed to set mode of %s to %#o: %m", devnode,
                    mode);
            ...
            log_device_debug(dev,
                             "Preserve permissions of %s, %#o, uid=%u, gid=%u",
                             devnode, mode, uid, gid);
        ...
        ORDERED_HASHMAP_FOREACH_KEY(label, name, seclabel_list, i)
        {
            ...
            if (streq(name, "selinux"))
            {
                ...
                if (q < 0)
                    log_device_error_errno(
                        dev, q,
                        "SECLABEL: failed to set SELinux label '%s': %m",
                        label);
                else
                    log_device_debug(dev, "SECLABEL: set SELinux label '%s'",
                                     label);
            }
            else if (streq(name, "smack"))
            {
                ...
                if (q < 0)
                    log_device_error_errno(
                        dev, q, "SECLABEL: failed to set SMACK label '%s': %m",
                        label);
                else
                    log_device_debug(dev, "SECLABEL: set SMACK label '%s'",
                                     label);
            }
            else
                log_device_error(
                    dev, "SECLABEL: unknown subsystem, ignoring '%s'='%s'",
                    name, label);
        }
    ...
    /* always update timestamp when we re-use the node, like on media change
     * events */
...
static int xsprintf_dev_num_path_from_sd_device(sd_device* dev, char** ret)
{
    ...
    xsprintf_dev_num_path(filename,
                          streq(subsystem, "block") ? "block" : "char", devnum);
