...
class lexer
{
  public:
    ...
    lexer(IteratorT const& first, IteratorT const& last, PositionT const& pos,
...
inline TokenT& lexer<IteratorT, PositionT, TokenT>::get(TokenT& result)
{
    ...
    {
        case T_IDENTIFIER:
            // test identifier characters for validity (throws if invalid chars
            // found)
            value = string_type((char const*)scanner.tok,
                                scanner.cur - scanner.tok);
            if (!boost::wave::need_no_character_validation(language))
                impl::validate_identifier_name(value, actline, scanner.column,
                                               filename);
            break;
        ...
        case T_CHARLIT:
        case T_RAWSTRINGLIT:
            // test literal characters for validity (throws if invalid chars
            // found)
            value = string_type((char const*)scanner.tok,
                                scanner.cur - scanner.tok);
            ...
            if (!boost::wave::need_no_character_validation(language))
                impl::validate_literal(value, actline, scanner.column,
                                       filename);
            break;
        ...
        case T_LONGINTLIT: // supported in C++11, C99 and long_long mode
            ...
            if (!boost::wave::need_long_long(language))
            {
                ...
                BOOST_WAVE_LEXER_THROW(
                    lexing_exception, invalid_long_long_literal, value.c_str(),
                    actline, scanner.column, filename.c_str());
            }
            break;
        ...
        case T_EOF:
            // T_EOF is returned as a valid token, the next call will return
            // T_EOI, i.e. the actual end of input
            at_eof = true;
            value.clear();
            break;
    ...
    //     std::cerr << boost::wave::get_token_name(id) << ": " << value <<
    //     std::endl;

    // the re2c lexer reports the new line number for newline tokens
    result =
        token_type(id, value, PositionT(filename, actline, scanner.column));
        ...
        boost::wave::language_support language)
{
    ...
    return new lex_functor<IteratorT, PositionT, TokenT>(first, last, pos,
                                                         language);
}
