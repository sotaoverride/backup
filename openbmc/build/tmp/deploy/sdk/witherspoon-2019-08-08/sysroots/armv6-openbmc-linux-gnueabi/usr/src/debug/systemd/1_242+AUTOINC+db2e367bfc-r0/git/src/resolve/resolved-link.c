...
void link_allocate_scopes(Link* l)
{
    ...
    /* If a link that used to be relevant is no longer, or a link that did not
     * use to be relevant now becomes relevant, let's reinit the learnt global
     * DNS server information, since we might talk to different servers now,
     * even if they have the same addresses as before. */
    ...
    if (unicast_relevant != l->unicast_relevant)
    {
        ...
        /* Also, flush the global unicast scope, to deal with split horizon
         * setups, where talking through one interface reveals different DNS
         * zones than through others. */
        if (l->manager->unicast_scope)
            dns_cache_flush(&l->manager->unicast_scope->cache);
    }

    /* And now, allocate all scopes that makes sense now if we didn't have them
     * yet, and drop those which we don't need anymore */
    ...
    if (unicast_relevant && l->dns_servers)
    {
        if (!l->unicast_scope)
        {
            ...
            r = dns_scope_new(l->manager, &l->unicast_scope, l,
                              DNS_PROTOCOL_DNS, AF_UNSPEC);
            if (r < 0)
        ...
        l->manager->llmnr_support != RESOLVE_SUPPORT_NO)
    {
        ...
        {
            r = dns_scope_new(l->manager, &l->llmnr_ipv4_scope, l,
                              DNS_PROTOCOL_LLMNR, AF_INET);
            if (r < 0)
        ...
        socket_ipv6_is_supported())
    {
        ...
        {
            r = dns_scope_new(l->manager, &l->llmnr_ipv6_scope, l,
                              DNS_PROTOCOL_LLMNR, AF_INET6);
            if (r < 0)
        ...
        l->manager->mdns_support != RESOLVE_SUPPORT_NO)
    {
        ...
        {
            r = dns_scope_new(l->manager, &l->mdns_ipv4_scope, l,
                              DNS_PROTOCOL_MDNS, AF_INET);
            if (r < 0)
        ...
        l->manager->mdns_support != RESOLVE_SUPPORT_NO)
    {
        ...
        {
            r = dns_scope_new(l->manager, &l->mdns_ipv6_scope, l,
                              DNS_PROTOCOL_MDNS, AF_INET6);
            if (r < 0)
...
void link_add_rrs(Link* l, bool force_remove)
{
    ...
    if (!force_remove && l->mdns_support == RESOLVE_SUPPORT_YES &&
...
void link_set_dns_over_tls_mode(Link* l, DnsOverTlsMode mode)
{
    ...
    if (mode != DNS_OVER_TLS_NO)
        log_warning("DNS-over-TLS option for the link cannot be set to "
                    "opportunistic when systemd-resolved is built without "
                    "DNS-over-TLS support. Turning off DNS-over-TLS support.");
    return;
#endif
...
void link_set_dnssec_mode(Link* l, DnssecMode mode)
{
    ...
    if (IN_SET(mode, DNSSEC_YES, DNSSEC_ALLOW_DOWNGRADE))
        log_warning("DNSSEC option for the link cannot be enabled or set to "
                    "allow-downgrade when systemd-resolved is built without "
                    "gcrypt support. Turning off DNSSEC support.");
    return;
#endif
        ...
        (l->dnssec_mode == DNSSEC_ALLOW_DOWNGRADE && mode == DNSSEC_YES))
    {
        ...
        /* When switching from non-DNSSEC mode to DNSSEC mode, flush the cache.
         * Also when switching from the allow-downgrade mode to full DNSSEC
         * mode, flush it too. */
        if (l->unicast_scope)
...
static int link_update_search_domain_one(Link* l, const char* name,
                                         bool route_only)
{
    DnsSearchDomain* d;
    int r;
    ...
    {
        r = dns_search_domain_new(l->manager, &d, DNS_SEARCH_DOMAIN_LINK, l,
                                  name);
        if (r < 0)
            return r;
    }
...
static void link_read_settings(Link* l)
{
    ...
    /* Read settings from networkd, except when networkd is not managing this
     * interface. */
    ...
    {
        log_warning_errno(
            r, "Failed to determine whether interface %s is managed: %m",
            l->name);
        return;
    }
    if (r == 0)
    {
        ...
        /* If this link used to be managed, but is now unmanaged, flush all our
         * settings — but only once. */
        if (l->is_managed)
            link_flush_settings(l);
    ...
    if (r < 0)
        log_warning_errno(
            r, "Failed to read DNS servers for interface %s, ignoring: %m",
            l->name);

    r = link_update_llmnr_support(l);
    if (r < 0)
        log_warning_errno(
            r, "Failed to read LLMNR support for interface %s, ignoring: %m",
            l->name);

    r = link_update_mdns_support(l);
    if (r < 0)
        log_warning_errno(
            r, "Failed to read mDNS support for interface %s, ignoring: %m",
            l->name);
    ...
    if (r < 0)
        log_warning_errno(
            r,
            "Failed to read DNS-over-TLS mode for interface %s, ignoring: %m",
            l->name);

    r = link_update_dnssec_mode(l);
    if (r < 0)
        log_warning_errno(
            r, "Failed to read DNSSEC mode for interface %s, ignoring: %m",
            l->name);

    r = link_update_dnssec_negative_trust_anchors(l);
    if (r < 0)
        log_warning_errno(r,
                          "Failed to read DNSSEC negative trust anchors for "
                          "interface %s, ignoring: %m",
                          l->name);

    r = link_update_search_domains(l);
    if (r < 0)
        log_warning_errno(
            r, "Failed to read search domains for interface %s, ignoring: %m",
            l->name);

    r = link_update_default_route(l);
    if (r < 0)
        log_warning_errno(r,
                          "Failed to read default route setting for interface "
                          "%s, proceeding anyway: %m",
                          l->name);
}
...
bool link_relevant(Link* l, int family, bool local_multicast)
{
    ...
    /* A link is relevant for local multicast traffic if it isn't a loopback
     * device, has a link beat, can do multicast and has at least one link-local
     * (or better) IP address.
     *
     * A link is relevant for non-multicast traffic if it isn't a loopback
     * device, has a link beat, and has at least one routable address. */
    ...
    if (state && !STR_IN_SET(state, "unknown", "degraded", "degraded-carrier",
                             "routable"))
        return false;

    LIST_FOREACH(addresses, a, l->addresses)
    if ((family == AF_UNSPEC || a->family == family) &&
        link_address_relevant(a, local_multicast))
        return true;

    return false;
}

LinkAddress* link_find_address(Link* l, int family,
...
DnsServer* link_set_dns_server(Link* l, DnsServer* s)
{
    ...
    if (s)
        log_debug("Switching to DNS server %s for interface %s.",
                  dns_server_string(s), l->name);
...
int link_address_new(Link* l, LinkAddress** ret, int family,
                     const union in_addr_union* in_addr)
{
    LinkAddress* a;
    ...
    if (a->link)
    {
        ...
        if (a->llmnr_address_rr)
        {
            if (a->family == AF_INET && a->link->llmnr_ipv4_scope)
                dns_zone_remove_rr(&a->link->llmnr_ipv4_scope->zone,
                                   a->llmnr_address_rr);
            else if (a->family == AF_INET6 && a->link->llmnr_ipv6_scope)
                dns_zone_remove_rr(&a->link->llmnr_ipv6_scope->zone,
                                   a->llmnr_address_rr);
        }
        ...
        if (a->mdns_address_rr)
        {
            if (a->family == AF_INET && a->link->mdns_ipv4_scope)
                dns_zone_remove_rr(&a->link->mdns_ipv4_scope->zone,
                                   a->mdns_address_rr);
            else if (a->family == AF_INET6 && a->link->mdns_ipv6_scope)
                dns_zone_remove_rr(&a->link->mdns_ipv6_scope->zone,
                                   a->mdns_address_rr);
        }
...
void link_address_add_rrs(LinkAddress* a, bool force_remove)
{
    ...
    if (a->family == AF_INET)
    {
        ...
        if (!force_remove && link_address_relevant(a, true) &&
            ...
            a->link->manager->llmnr_support == RESOLVE_SUPPORT_YES)
        {
            ...
            if (!a->llmnr_address_rr)
            {
                a->llmnr_address_rr = dns_resource_record_new(
                    a->link->manager->llmnr_host_ipv4_key);
                if (!a->llmnr_address_rr)
                {
                    r = -ENOMEM;
                    goto fail;
            ...
            if (!a->llmnr_ptr_rr)
            {
                r = dns_resource_record_new_reverse(
                    &a->llmnr_ptr_rr, a->family, &a->in_addr,
                    a->link->manager->llmnr_hostname);
                if (r < 0)
                    goto fail;
            ...
            r = dns_zone_put(&a->link->llmnr_ipv4_scope->zone,
                             a->link->llmnr_ipv4_scope, a->llmnr_address_rr,
                             true);
            if (r < 0)
                log_warning_errno(r,
            ...
            r = dns_zone_put(&a->link->llmnr_ipv4_scope->zone,
                             a->link->llmnr_ipv4_scope, a->llmnr_ptr_rr, false);
            if (r < 0)
                log_warning_errno(
                    r, "Failed to add IPv4 PTR record to LLMNR zone: %m");
        }
        else
        {
            if (a->llmnr_address_rr)
            ...
            a->link->manager->mdns_support == RESOLVE_SUPPORT_YES)
        {
            ...
            if (!a->mdns_address_rr)
            {
                a->mdns_address_rr = dns_resource_record_new(
                    a->link->manager->mdns_host_ipv4_key);
                if (!a->mdns_address_rr)
                {
                    r = -ENOMEM;
                    goto fail;
            ...
            {
                r = dns_resource_record_new_reverse(
                    &a->mdns_ptr_rr, a->family, &a->in_addr,
            ...
            r = dns_zone_put(&a->link->mdns_ipv4_scope->zone,
                             a->link->mdns_ipv4_scope, a->mdns_address_rr,
                             true);
            if (r < 0)
                log_warning_errno(r, "Failed to add A record to MDNS zone: %m");

            r = dns_zone_put(&a->link->mdns_ipv4_scope->zone,
                             a->link->mdns_ipv4_scope, a->mdns_ptr_rr, false);
            if (r < 0)
                log_warning_errno(
                    r, "Failed to add IPv4 PTR record to MDNS zone: %m");
        }
        else
        {
            if (a->mdns_address_rr)
    ...
    if (a->family == AF_INET6)
    {
            ...
            a->link->manager->llmnr_support == RESOLVE_SUPPORT_YES)
        {
            ...
            {
                a->link->manager->llmnr_host_ipv6_key =
                    dns_resource_key_new(DNS_CLASS_IN, DNS_TYPE_AAAA,
                                         a->link->manager->llmnr_hostname);
                if (!a->link->manager->llmnr_host_ipv6_key)
                {
                    r = -ENOMEM;
            ...
            r = dns_zone_put(&a->link->llmnr_ipv6_scope->zone,
                             a->link->llmnr_ipv6_scope, a->llmnr_address_rr,
                             true);
            if (r < 0)
                log_warning_errno(
                    r, "Failed to add AAAA record to LLMNR zone: %m");

            r = dns_zone_put(&a->link->llmnr_ipv6_scope->zone,
                             a->link->llmnr_ipv6_scope, a->llmnr_ptr_rr, false);
            if (r < 0)
                log_warning_errno(
                    r, "Failed to add IPv6 PTR record to LLMNR zone: %m");
        }
        else
        {
            if (a->llmnr_address_rr)
            ...
            a->link->manager->mdns_support == RESOLVE_SUPPORT_YES)
        {
            ...
            {
                a->link->manager->mdns_host_ipv6_key =
                    dns_resource_key_new(DNS_CLASS_IN, DNS_TYPE_AAAA,
                                         a->link->manager->mdns_hostname);
                if (!a->link->manager->mdns_host_ipv6_key)
                {
            ...
            r = dns_zone_put(&a->link->mdns_ipv6_scope->zone,
                             a->link->mdns_ipv6_scope, a->mdns_address_rr,
                             true);
            if (r < 0)
                log_warning_errno(r,
            ...
            r = dns_zone_put(&a->link->mdns_ipv6_scope->zone,
                             a->link->mdns_ipv6_scope, a->mdns_ptr_rr, false);
            if (r < 0)
                log_warning_errno(
                    r, "Failed to add IPv6 PTR record to MDNS zone: %m");
        }
        else
        {
            if (a->mdns_address_rr)
...
{
    _cleanup_free_ char *llmnr = NULL, *mdns = NULL, *dnssec = NULL,
                        *servers = NULL, *domains = NULL, *ntas = NULL,
                        *default_route = NULL;
    ...
    if (l->is_managed)
        return 0; /* if the device is managed, then networkd is our
                     configuration source, not the bus API */

    r = parse_env_file(NULL, l->state_file, "LLMNR", &llmnr, "MDNS", &mdns,
                       "DNSSEC", &dnssec, "SERVERS", &servers, "DOMAINS",
                       &domains, "NTAS", &ntas, "DEFAULT_ROUTE",
                       &default_route);
    if (r == -ENOENT)
        return 0;
    if (r < 0)
    ...
    /* If we can't recognize the LLMNR or MDNS setting we don't override the
     * default */
    ...
    /* If we can't recognize the DNSSEC setting, then set it to invalid, so that
     * the daemon default is used. */
    ...
    for (p = servers;;)
    {
        ...
        if (r < 0)
        {
            log_debug_errno(r, "Failed to load DNS server '%s', ignoring: %m",
                            word);
            continue;
        }
    }
    ...
    for (p = domains;;)
    {
        ...
        {
            log_debug_errno(
                r, "Failed to load search domain '%s', ignoring: %m", word);
            continue;
        }
    }
