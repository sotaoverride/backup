...
class basic_regex_creator
{
  public:
    ...
    {
        return static_cast<re_syntax_base*>(
            static_cast<void*>(static_cast<char*>(base) + off));
    }
    ...
    re_syntax_base* append_state(syntax_element_type t,
  ...
  protected:
    regex_data<charT, traits>*
        m_pdata; // pointer to the basic_regex_data struct we are filling in
    ...
    boost::uintmax_t
        m_bad_repeats;     // bitmask of repeats we can't deduce a startmap for;
    bool m_has_recursions; // set when we have recursive expresisons to fixup
    std::vector<unsigned char>
        m_recursion_checks; // notes which recursions we've followed while
                            // analysing this expression
    typename traits::char_class_type
        m_word_mask; // mask used to determine if a character is a word
                     // character
    typename traits::char_class_type
        m_mask_space; // mask used to determine if a character is a word
                      // character
    typename traits::char_class_type
        m_lower_mask; // mask used to determine if a character is a lowercase
                      // character
    typename traits::char_class_type
        m_upper_mask; // mask used to determine if a character is an uppercase
                      // character
    typename traits::char_class_type
        m_alpha_mask; // mask used to determine if a character is an alphabetic
                      // character
  private:
    ...
    void create_startmap(re_syntax_base* state, unsigned char* l_map,
...
template <class charT, class traits>
basic_regex_creator<charT, traits>::basic_regex_creator(
    regex_data<charT, traits>* data) :
    m_pdata(data),
    m_traits(*(data->m_ptraits)), m_last_state(0), m_repeater_id(0),
    m_has_backrefs(false), m_backrefs(0), m_has_recursions(false)
{
...
template <class charT, class traits>
re_syntax_base*
    basic_regex_creator<charT, traits>::append_state(syntax_element_type t,
                                                     std::size_t s)
{
...
template <class charT, class traits>
re_syntax_base* basic_regex_creator<charT, traits>::insert_state(
    std::ptrdiff_t pos, syntax_element_type t, std::size_t s)
{
    // append a new state, start by aligning our last one:
    m_pdata->m_data.align();
    // set the offset to the next state in our last one:
    ...
    re_syntax_base* new_state =
        static_cast<re_syntax_base*>(m_pdata->m_data.insert(pos, s));
    // fill in boilerplate options in the new state:
    new_state->next.i = s;
    new_state->type = t;
...
re_literal* basic_regex_creator<charT, traits>::append_literal(charT c)
{
    ...
    if ((0 == m_last_state) || (m_last_state->type != syntax_element_literal))
    {
        ...
        result = static_cast<re_literal*>(append_state(
            syntax_element_literal, sizeof(re_literal) + sizeof(charT)));
        result->length = 1;
        *static_cast<charT*>(static_cast<void*>(result + 1)) =
            m_traits.translate(c, m_icase);
    }
    else
    {
    ...
    const basic_char_set<charT, traits>& char_set, mpl::false_*)
{
    ...
    re_set_long<m_type>* result = static_cast<re_set_long<m_type>*>(
        append_state(syntax_element_long_set, sizeof(re_set_long<m_type>)));
    //
    // fill in the basics:
    //
    result->csingles =
        static_cast<unsigned int>(::boost::BOOST_REGEX_DETAIL_NS::distance(
            char_set.singles_begin(), char_set.singles_end()));
    result->cranges =
        static_cast<unsigned int>(::boost::BOOST_REGEX_DETAIL_NS::distance(
            char_set.ranges_begin(), char_set.ranges_end())) /
        2;
    result->cequivalents =
        static_cast<unsigned int>(::boost::BOOST_REGEX_DETAIL_NS::distance(
            char_set.equivalents_begin(), char_set.equivalents_end()));
    ...
    while (sfirst != slast)
    {
        charT* p = static_cast<charT*>(this->m_pdata->m_data.extend(
            sizeof(charT) * (sfirst->first == static_cast<charT>(0)
                                 ? 1
                                 : sfirst->second ? 3 : 2)));
    ...
    while (first != last)
    {
        ...
        charT* p = static_cast<charT*>(this->m_pdata->m_data.extend(
            sizeof(charT) * (s1.size() + s2.size() + 2)));
    ...
    while (sfirst != slast)
    {
        ...
        charT* p = static_cast<charT*>(
            this->m_pdata->m_data.extend(sizeof(charT) * (s.size() + 1)));
    ...
    //
    m_last_state = result =
        static_cast<re_set_long<m_type>*>(getaddress(offset));
    return result;
}
    ...
    const basic_char_set<charT, traits>& char_set, mpl::true_*)
{
    ...
    re_set* result =
        static_cast<re_set*>(append_state(syntax_element_set, sizeof(re_set)));
    ...
    while (sfirst != slast)
    {
        ...
        {
            if (this->m_traits.translate(static_cast<charT>(i),
                                         this->m_icase) ==
    ...
    while (first != last)
    {
        ...
        else
        {
            ...
            std::memset(result->_map + static_cast<unsigned char>(c1), true,
                        1 + static_cast<unsigned char>(c2) -
    ...
    {
        // adjust m as needed:
        if (((m & m_lower_mask) == m_lower_mask) ||
            ((m & m_upper_mask) == m_upper_mask))
            m |= m_alpha_mask;
    }
    if (m != 0)
    {
...
template <class charT, class traits>
void basic_regex_creator<charT, traits>::finalize(const charT* p1,
                                                  const charT* p2)
{
    ...
    charT* ps = static_cast<charT*>(
        m_pdata->m_data.extend(sizeof(charT) * (1 + (p2 - p1))));
    ...
    m_pdata->m_first_state =
        static_cast<re_syntax_base*>(m_pdata->m_data.data());
    ...
    create_startmap(m_pdata->m_first_state, m_pdata->m_startmap,
                    &(m_pdata->m_can_be_null), mask_all);
    ...
    while (state)
    {
        switch (state->type)
        {
            ...
            case syntax_element_alt:
                std::memset(static_cast<re_alt*>(state)->_map, 0,
                            sizeof(static_cast<re_alt*>(state)->_map));
                static_cast<re_alt*>(state)->can_be_null = 0;
                BOOST_FALLTHROUGH;
            case syntax_element_jump:
                static_cast<re_jump*>(state)->alt.p =
                    getaddress(static_cast<re_jump*>(state)->alt.i, state);
                BOOST_FALLTHROUGH;
            default:
                if (state->next.i)
                    state->next.p = getaddress(state->next.i, state);
    ...
    while (state)
    {
        switch (state->type)
        {
                ...
                if (idx < 0)
                {
                    ...
                    if (idx >= 10000)
                    {
                        ...
                        if (idx <= 0)
                        {
                            ...
                            if (0 == this->m_pdata
                                         ->m_status) // update the error code if
                                                     // not already set
                                this->m_pdata->m_status =
                            ...
                            //
                            if (0 ==
                                (this->flags() & regex_constants::no_except))
                            {
                                std::string message =
                                    "Encountered a forward reference to a "
                                    "marked sub-expression that does not "
                                    "exist.";
                                boost::regex_error e(
                                    message,
                                    boost::regex_constants::error_bad_pattern,
            ...
            case syntax_element_recurse:
            {
                ...
                {
                    //
                    // There may be more than one capture group with this hash,
                    // just do what Perl does and recurse to the leftmost:
                    //
                ...
                else
                {
                    while (p)
                    {
                        if ((p->type == syntax_element_startmark) &&
                            (static_cast<re_brace*>(p)->index == idx))
                        {
                            //
                            // We've found the target of the recursion, set the
                            // jump target:
                            //
                            ...
                            while (p)
                            {
                                switch (p->type)
                                {
                                    ...
                                    case syntax_element_short_set_rep:
                                    case syntax_element_long_set_rep:
                                        next_rep_id = static_cast<re_repeat*>(p)
                                                          ->state_id;
                                        break;
                                    case syntax_element_endmark:
                                        if (static_cast<const re_brace*>(p)
                                                ->index == idx)
                                            next_rep_id = -1;
                                        break;
                                    default:
                            ...
                            if (next_rep_id > 0)
                            {
                                static_cast<re_recurse*>(state)->state_id =
                                    next_rep_id - 1;
                            }
                ...
                if (!ok)
                {
                    ...
                    if (0 == this->m_pdata->m_status) // update the error code
                                                      // if not already set
                        this->m_pdata->m_status =
                            boost::regex_constants::error_bad_pattern;
                    //
                    ...
                    {
                        std::string message =
                            "Encountered a forward reference to a recursive "
                            "sub-expression that does not exist.";
                        boost::regex_error e(
                            message, boost::regex_constants::error_bad_pattern,
                            0);
                        e.raise();
                    }
                }
...
void basic_regex_creator<charT, traits>::create_startmaps(re_syntax_base* state)
{
        ...
        switch (state->type)
        {
            ...
            case syntax_element_backstep:
                ...
                if (static_cast<re_brace*>(state)->index < 0)
                {
                    ...
                    {
                        std::string message =
                            "Invalid lookbehind assertion encountered in the "
                            "regular expression.";
                        boost::regex_error e(
                            message, boost::regex_constants::error_bad_pattern,
                            0);
    ...
    while (v.size())
    {
        ...
        // Build maps:
        m_bad_repeats = 0;
        create_startmap(state->next.p, static_cast<re_alt*>(state)->_map,
                        &static_cast<re_alt*>(state)->can_be_null, mask_take);
        m_bad_repeats = 0;

        if (m_has_recursions)
            m_recursion_checks.assign(1 + m_pdata->m_mark_count, 0u);
        create_startmap(static_cast<re_alt*>(state)->alt.p,
...
template <class charT, class traits>
int basic_regex_creator<charT, traits>::calculate_backstep(
        ...
        switch (state->type)
        {
            ...
            case syntax_element_backstep:
            {
                ...
                else if (state->type == syntax_element_long_set_rep)
                {
                    ...
                    if (static_cast<re_set_long<m_type>*>(rep->next.p)
                            ->singleton == 0)
                        return -1;
                    if (rep->max != rep->min)
                        return -1;
    ...
    while (state)
    {
        switch (state->type)
        {
            ...
            case syntax_element_literal:
            {
                ...
                {
                    l_map[0] |= mask_init;
                    charT first_char = *static_cast<charT*>(static_cast<void*>(
                        static_cast<re_literal*>(state) + 1));
                    for (unsigned int i = 0; i < (1u << CHAR_BIT); ++i)
                    {
                        if (m_traits.translate(static_cast<charT>(i),
                                               l_icase) == first_char)
                            l_map[i] |= mask;
                    }
                }
            ...
            case syntax_element_recurse:
            {
                BOOST_ASSERT(static_cast<const re_jump*>(state)->alt.p->type ==
                             syntax_element_startmark);
                recursion_sub = static_cast<re_brace*>(
                                    static_cast<const re_jump*>(state)->alt.p)
                                    ->index;
            ...
            case syntax_element_word_start:
            {
                ...
                if (l_map)
                {
                    ...
                    for (unsigned int i = 0; i < (1u << CHAR_BIT); ++i)
                    {
                        if (!m_traits.isctype(static_cast<charT>(i),
                                              m_word_mask))
            ...
            case syntax_element_set:
                if (l_map)
                {
                    ...
                    for (unsigned int i = 0; i < (1u << CHAR_BIT); ++i)
                    {
                        if (static_cast<re_set*>(state)
                                ->_map[static_cast<unsigned char>(
                                    m_traits.translate(static_cast<charT>(i),
                                                       l_icase))])
                            l_map[i] |= mask;
                    }
                }
            ...
            case syntax_element_endmark:
                ...
                else if (recursion_start && (recursion_sub != 0) &&
                         (recursion_sub ==
                          static_cast<re_brace*>(state)->index))
                {
                ...
                //
                // Normally we just go to the next state... but if this
                // sub-expression is the target of a recursion, then we might be
                // ending a recursion, in which case we should check whatever
                // follows that recursion, as well as whatever follows this
                // state:
                //
                if (m_pdata->m_has_recursions &&
                    static_cast<re_brace*>(state)->index)
                {
                    bool ok = false;
                    ...
                    while (p)
                    {
                        ...
                        {
                            re_brace* p2 = static_cast<re_brace*>(
                                static_cast<re_jump*>(p)->alt.p);
                    ...
                    if (ok && ((m_recursion_checks[static_cast<re_brace*>(state)
                                                       ->index] &
                                2u) == 0))
                    {
                        m_recursion_checks[static_cast<re_brace*>(state)
                                               ->index] |= 2u;
            ...
            case syntax_element_commit:
                set_all_masks(l_map, mask);
                // Continue scanning so we can figure out whether we can be
                // null:
                state = state->next.p;
                break;
            case syntax_element_startmark:
...
template <class charT, class traits>
unsigned
    basic_regex_creator<charT, traits>::get_restart_type(re_syntax_base* state)
{
    //
...
template <class charT, class traits>
void basic_regex_creator<charT, traits>::set_all_masks(unsigned char* bits,
                                                       unsigned char mask)
{
    //
    ...
    switch (pt->type)
    {
        ...
        case syntax_element_long_set_rep:
        {
            ...
            if (state_id >= sizeof(m_bad_repeats) * CHAR_BIT)
                return true; // run out of bits, assume we can't traverse this
                             // one.
...
template <class charT, class traits>
syntax_element_type
    basic_regex_creator<charT, traits>::get_repeat_type(re_syntax_base* state)
{
    typedef typename traits::char_class_type m_type;
        ...
        if (state->next.p->next.p->next.p == static_cast<re_alt*>(state)->alt.p)
        {
            switch (state->next.p->type)
            {
                ...
                case BOOST_REGEX_DETAIL_NS::syntax_element_long_set:
                    if (static_cast<
                            BOOST_REGEX_DETAIL_NS::re_set_long<m_type>*>(
                            state->next.p)
                            ->singleton)
                        return BOOST_REGEX_DETAIL_NS::
...
template <class charT, class traits>
void basic_regex_creator<charT, traits>::probe_leading_repeat(
    re_syntax_base* state)
{
        ...
        switch (state->type)
        {
            case syntax_element_startmark:
                    ...
                    (static_cast<re_brace*>(state)->index == -2))
                {
                    ...
                    state = static_cast<const re_jump*>(state->next.p)
                                ->alt.p->next.p;
                    continue;
                }
                if (static_cast<re_brace*>(state)->index == -3)
