...
\details Template class simple_segregated_storage controls access to a free list
of memory chunks. Please note that this is a very simple class, with
preconditions on almost all its functions. It is intended to be the fastest and
smallest possible quick memory allocator - e.g., something to use in embedded
systems. This class delegates many difficult preconditions to the user (i.e.,
alignment issues).

An object of type simple_segregated_storage<SizeType>  is empty  if its free
list is empty. If it is not empty, then it is ordered  if its free list is
ordered. A free list is ordered if repeated calls to <tt>malloc()</tt> will
result in a constantly-increasing sequence of values, as determined by
<tt>std::less<void *></tt>. A member function is <i>order-preserving</i> if the
free list maintains its order orientation (that is, an ordered free list is
...
class simple_segregated_storage
{
   ...
   */
    ...
    {   //! The return value is just *ptr cast to the appropriate type. ptr must
        //! not be 0. (For the sake of code readability :)
        //! As an example, let us assume that we want to truncate the free list
        //! after the first chunk. That is, we want to set *first to 0; this
        //! will result in a free list with only one entry. The normal way to do
        //! this is to first cast first to a pointer to a pointer to void, and
        //! then dereference and assign (*static_cast<void **>(first) = 0;).
        //! This can be done more easily through the use of this convenience
        //! function (nextof(first) = 0;). \returns dereferenced pointer.
  ...
  public:
    ...
    void validate()
    {
        ...
        {
            void* pt = nextof(
                ptr); // trigger possible segfault *before* we update variables
            ++index;
            old = ptr;
            ptr = nextof(ptr);
        }
...
//! \note Note that this function finds the location previous to where ptr would
//! go if it was in the free list. It does not find the entry in the free list
//! before ptr (unless ptr is already in the free list). Specifically,
//! find_prev(0) will return 0, not the last entry in the free list. \returns
...
void* simple_segregated_storage<SizeType>::segregate(
    void* const block, const size_type sz, const size_type partition_sz,
    void* const end)
{
    ...
    // Handle border case where sz == partition_sz (i.e., we're handling an
    // array
    //  of 1 element)
    if (old == block)
        return block;
...
//! first. If not found, returns 0. It is strongly recommended (but not
//! required) that the free list be ordered, as this algorithm will fail to find
//! a contiguous sequence unless it is contiguous in the free list as well.
//! Order-preserving. O(N) with respect to the size of the free list.
template <typename SizeType>
void* simple_segregated_storage<SizeType>::malloc_n(
    const size_type n, const size_type partition_size)
{
    BOOST_POOL_VALIDATE_INTERNALS
    if (n == 0)
        return 0;
