...
int main(int argc, char** argv)
{
    ...
    {
        fprintf(stderr,
                "usage: %s <host> <port> <image> <erasesize> [<tx_rate>]\n",
                PROGRAM_NAME);
        exit(1);
    }
    ...
    if (st.st_size % erasesize)
    {
        fprintf(
            stderr,
            "Image size %lld bytes is not a multiple of erasesize %d bytes\n",
    ...
    {
        printf("\rChecking block CRCS.... %d/%d", block_nr + 1, nr_blocks);
        fflush(stdout);
        block_crcs[block_nr] =
            mtd_crc32(-1, image + (block_nr * erasesize), erasesize);
    }

    printf("\nImage size %ld KiB (0x%08lx). %d blocks at %d pkts/block\n"
           "Estimated transmit time per cycle: %ds\n",
           (long)st.st_size / 1024, (long)st.st_size, nr_blocks, pkts_per_block,
    ...
    while (1)
        for (pkt_nr = 0; pkt_nr < total_pkts_per_block; pkt_nr++)
        {
            ...
            for (block_nr = 0; block_nr < nr_blocks; block_nr++)
            {
                ...
                fec_encode_linear(fec, blockptr, pktbuf.data, actualpkt,
                                  PKT_SIZE);

                pktbuf.hdr.thiscrc =
                    htonl(mtd_crc32(-1, pktbuf.data, PKT_SIZE));
                ...
                printf("\rSending data block %08x packet %3d/%d",
                       block_nr * erasesize, pkt_nr, total_pkts_per_block);

                if (pkt_nr && !block_nr)
                {
                    unsigned long amt_sent =
                        pkt_nr * nr_blocks * sizeof(pktbuf);
