...
static void g_tls_certificate_get_property(GObject* object, guint prop_id,
                                           GValue* value, GParamSpec* pspec)
{
    G_OBJECT_WARN_INVALID_PROPERTY_ID(object, prop_id, pspec);
}

static void g_tls_certificate_set_property(GObject* object, guint prop_id,
                                           const GValue* value,
                                           GParamSpec* pspec)
{
    G_OBJECT_WARN_INVALID_PROPERTY_ID(object, prop_id, pspec);
...
static void g_tls_certificate_class_init(GTlsCertificateClass* class)
{
    ...
    g_object_class_install_property(
        ...
        g_param_spec_boxed("certificate", P_("Certificate"),
     ...
     */
    g_object_class_install_property(
        gobject_class, PROP_CERTIFICATE_PEM,
        g_param_spec_string("certificate-pem", P_("Certificate (PEM)"),
     ...
     */
    g_object_class_install_property(
        gobject_class, PROP_PRIVATE_KEY_PEM,
        g_param_spec_string(
            "private-key-pem", P_("Private key (PEM)"),
            P_("The PEM representation of the certificate’s private key"), NULL,
            G_PARAM_WRITABLE | G_PARAM_CONSTRUCT_ONLY |
                G_PARAM_STATIC_STRINGS));
    /**
     ...
     */
    g_object_class_install_property(
        gobject_class, PROP_ISSUER,
        g_param_spec_object("issuer", P_("Issuer"),
...
static GTlsCertificate*
    g_tls_certificate_new_internal(const gchar* certificate_pem,
                                   ...
                                   GTlsCertificate* issuer, GError** error)
{
    GObject* cert;
    GTlsBackend* backend;
    ...
    cert =
        g_initable_new(g_tls_backend_get_certificate_type(backend), NULL, error,
                       "certificate-pem", certificate_pem, "private-key-pem",
                       private_key_pem, "issuer", issuer, NULL);
...
static gchar* parse_private_key(const gchar* data, gsize data_len,
                                gboolean required, GError** error)
{
    const gchar *header_start = NULL, *header_end, *footer_start = NULL,
                *footer_end;

    header_end = g_strstr_len(data, data_len, PEM_PRIVKEY_HEADER_END);
    if (header_end)
        header_start =
            g_strrstr_len(data, header_end - data, PEM_PRIVKEY_HEADER_BEGIN);
    ...
    if (strncmp(header_start, PEM_PKCS8_ENCRYPTED_HEADER,
                header_end - header_start) == 0)
    {
        g_set_error_literal(error, G_TLS_ERROR, G_TLS_ERROR_BAD_CERTIFICATE,
    ...
    footer_end = g_strstr_len(header_end, data_len - (header_end - data),
                              PEM_PRIVKEY_FOOTER_END);
    if (footer_end)
        footer_start = g_strrstr_len(header_end, footer_end - header_end,
                                     PEM_PRIVKEY_FOOTER_BEGIN);
...
static gchar* parse_next_pem_certificate(const gchar** data,
...
static GSList* parse_and_create_certificate_list(const gchar* data,
...
static GTlsCertificate* create_certificate_chain_from_list(GSList* pem_list,
                                                           const gchar* key_pem)
{
...
static GTlsCertificate* parse_and_create_certificate(const gchar* data,
                                                     gsize data_len,
                                                     const gchar* key_pem,
                                                     GError** error)
...
GList* g_tls_certificate_list_new_from_file(const gchar* file, GError** error)
{
    ...
    while (p && *p)
    {
        ...
        if (cert_pem)
        {
            cert = g_tls_certificate_new_internal(cert_pem, NULL, NULL,
                                                  &parse_error);
            g_free(cert_pem);
        }
        if (!cert)
...
{
    return G_TLS_CERTIFICATE_GET_CLASS(cert)->verify(cert, identity,
                                                     trusted_ca);
}
                                   ...
                                   GTlsCertificate* cert_two)
{
    ...
    equal = (b1->len == b2->len && memcmp(b1->data, b2->data, b1->len) == 0);
