                            ...
                            const char* message, const struct ucred* ucred)
{
    ...
    struct timespec ts;
    char tbuf[STRLEN("[] ") + DECIMAL_STR_MAX(ts.tv_sec) +
              DECIMAL_STR_MAX(ts.tv_nsec) - 3 + 1];
    ...
    if (prefix_timestamp())
    {
        ...
        xsprintf(tbuf, "[%5" PRI_TIME ".%06" PRI_NSEC "] ", ts.tv_sec,
    ...
    /* Before you ask: yes, on purpose we open/close the console for each log
     * line we write individually. This is a good strategy to avoid journald
     * getting killed by the kernel's SAK concept (it doesn't fix this entirely,
     * but minimizes the time window the kernel might end up killing journald
     * due to SAK). It also makes things
     * easier for us so that we don't have to recover from hangups and suchlike
