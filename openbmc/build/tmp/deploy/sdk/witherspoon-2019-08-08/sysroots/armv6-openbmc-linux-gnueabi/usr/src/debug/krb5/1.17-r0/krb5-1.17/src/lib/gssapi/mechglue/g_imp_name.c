...
static OM_uint32 val_imp_name_args(OM_uint32* minor_status,
...
OM_uint32 KRB5_CALLCONV gss_import_name(minor_status, input_name_buffer,
...

{
    ...
    major_status = val_imp_name_args(minor_status, input_name_buffer,
                                     input_name_type, output_name);
    ...
    {
        major_status =
            importExportName(minor_status, union_name, input_name_type);
        if (major_status != GSS_S_COMPLETE)
            goto allocation_failure;
    }
...
static OM_uint32 importExportName(minor, unionName,
                                  inputNameType) OM_uint32* minor;
...
gss_OID inputNameType;
{
    ...
    mechOid.length =
        gssint_get_der_length(&buf, (expName.length - curLength), &bytes);
    ...
    if (mech->gss_export_name)
    {
        ...
        {
            major =
                generic_gss_copy_oid(minor, &mechOid, &unionName->mech_type);
            if (major != GSS_S_COMPLETE)
                map_errcode(minor);
        }
        return (major);
    ...
    if (mech->gssspi_import_name_by_mech)
    {
        major = mech->gssspi_import_name_by_mech(
            minor, &mechOid, &expName, GSS_C_NULL_OID, &unionName->mech_name);
    }
    else
    {
        major = mech->gss_import_name(minor, &expName, GSS_C_NULL_OID,
                                      &unionName->mech_name);
    }
    if (major != GSS_S_COMPLETE)
    {
