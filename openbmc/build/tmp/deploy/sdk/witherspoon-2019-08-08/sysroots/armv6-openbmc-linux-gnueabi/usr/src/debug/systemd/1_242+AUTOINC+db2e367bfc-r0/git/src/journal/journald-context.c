...
/* This implements a metadata cache for clients, which are identified by their
 * PID. Requesting metadata through /proc is expensive, hence let's cache the
 * data if we can. Note that this means the metadata might be out-of-date when
 * we store it, but it might already be anyway, as we request the data
 * asynchronously from /proc at a different time the log entry was originally
 * created. We hence just increase the "window of inaccuracy" a bit.
 *
 * The cache is indexed by the PID. Entries may be "pinned" in the cache, in
 * which case the entries are not removed until they are unpinned. Unpinned
 * entries are kept around until cache pressure is seen. Cache entries older
 * than 5s are never used (a sad attempt to deal with the UNIX weakness of PIDs
 * reuse), cache entries older than 1s are refreshed in an incremental way
 * (meaning: data is reread from /proc, but any old data we can't refresh is not
 * flushed out). Data newer than 1s is used immediately without refresh.
 *
 * Log stream clients (i.e. all clients using the AF_UNIX/SOCK_STREAM
 * stdout/stderr transport) will pin a cache entry as long as their socket is
 * connected. Note that cache entries are shared between different transports.
 * That means a cache entry pinned for the stream connection logic may be reused
 * for the syslog or native protocols.
 *
 * Caching metadata like this has two major benefits:
 *
 * 1. Reading metadata is expensive, and we can thus substantially speed up log
 * processing under flood.
 *
 * 2. Because metadata caching is shared between stream and datagram transports
 * and stream connections pin a cache entry there's a good chance we can
 * properly map a substantial set of datagram log messages to their originating
 *    service, as all services (unless explicitly configured otherwise) will
 * have their stdout/stderr connected to a stream connection. This should
 * improve cases where a service process logs immediately before exiting and we
 *    previously had trouble associating the log message with the service.
 *
 * NB: With and without the metadata cache: the implicitly added entry metadata
 * in the journal (with the exception of UID/PID/GID and SELinux label) must be
 * understood as possibly slightly out of sync (i.e. sometimes slighly older and
 * sometimes slightly newer than what was current at the log event).
 */
...
/* Keep at most 16K entries in the cache. (Note though that this limit may be
 * violated if enough streams pin entries in the cache, in which case we *do*
 * permit this limit to be breached. That's safe however, as the number of
 * stream clients itself is limited.) */
...
static size_t cache_max(void)
{
    ...
    if (cached == (size_t)-1)
    {
        ...
        {
            /* Cache entries are usually a few kB, but the process cmdline is
             * controlled by the user and can be up to _SC_ARG_MAX, usually 2MB.
             * Let's say that approximately up to 1/8th of memory may be used by
             * the cache.
             *
             * In the common case, this formula gives 64 cache entries for each
             * GB of RAM.
             */
            long l = sysconf(_SC_ARG_MAX);
            assert(l > 0);

            cached = CLAMP(mem_total / 8 / (uint64_t)l, CACHE_MAX_MIN,
                           CACHE_MAX_MAX);
        }
    }
...
static void client_context_read_uid_gid(ClientContext* c,
                                        const struct ucred* ucred)
{
    assert(c);
    assert(pid_is_valid(c->pid));

    /* The ucred data passed in is always the most current and accurate, if we
     * have any. Use it. */
...
static int client_context_read_label(ClientContext* c, const char* label,
                                     size_t label_size)
{
...
static int client_context_read_cgroup(Server* s, ClientContext* c,
                                      const char* unit_id)
{
    ...
    {
        /* We use the unit ID passed in as fallback if we have nothing cached
         * yet and cg_pid_get_path_shifted() failed or process is running in a
         * root cgroup. Zombie processes are automatically migrated to root
         * cgroup on cgroup v1 and we want to be able to map log messages from
         * them too. */
        if (unit_id && !c->unit)
        {
...
static int client_context_read_invocation_id(Server* s, ClientContext* c)
{
    ...
    /* Read the invocation ID of a unit off a unit. PID 1 stores it in a
     * per-unit symlink in /run/systemd/units/ */
...
static int client_context_read_extra_fields(Server* s, ClientContext* c)
{
    ...
    if (fstat(fileno(f), &st) <
        0) /* The file might have been replaced since the stat() above, let's
            * get a new one, that matches the stuff we are reading */
        return -errno;
...
static void client_context_really_refresh(Server* s, ClientContext* c,
                                          const struct ucred* ucred,
                                          const char* label, size_t label_size,
                                          const char* unit_id, usec_t timestamp)
{
    ...
    {
        assert(c->n_ref == 0);
        assert_se(prioq_reshuffle(s->client_contexts_lru, c, &c->lru_index) >=
                  0);
    }
}

void client_context_maybe_refresh(Server* s, ClientContext* c,
                                  const struct ucred* ucred, const char* label,
                                  size_t label_size, const char* unit_id,
                                  usec_t timestamp)
{
    ...
    /* If the data isn't pinned and if the cashed data is older than the upper
     * limit, we flush it out entirely. This follows the logic that as long as
     * an entry is pinned the PID reuse is unlikely. */
    ...
    /* If the data is older than the lower limit, we refresh, but keep the old
     * data for all we can't update */
    if (c->timestamp + REFRESH_USEC < timestamp)
        goto refresh;

    /* If the data passed along doesn't match the cached data we also do a
     * refresh */
    ...
    if (label_size > 0 && (label_size != c->label_size ||
                           memcmp(label, c->label, label_size) != 0))
        goto refresh;

    return;

refresh:
    client_context_really_refresh(s, c, ucred, label, label_size, unit_id,
                                  timestamp);
}
...
static void client_context_try_shrink_to(Server* s, size_t limit)
{
    ...
    /* Flush any cache entries for PIDs that have already moved on. Don't do
     * this too often, since it's a slow process. */
    t = now(CLOCK_MONOTONIC);
    if (s->last_cache_pid_flush + MAX_USEC < t)
    {
        unsigned n = prioq_size(s->client_contexts_lru), idx = 0;

        /* We do a number of iterations based on the initial size of the prioq.
         * When we remove an item, a new item is moved into its places, and
         * items to the right might be reshuffled.
         */
    ...
    /* Bring the number of cache entries below the indicated limit, so that we
     * can create a new entry without breaching the limit. Note that we only
     * flush out entries that aren't pinned here. This means the number of
     * cache entries may very well grow beyond the limit, if all entries stored
     * remain pinned. */
...
void client_context_flush_all(Server* s)
{
    ...
    /* Flush out all remaining entries. This assumes all references are already
     * dropped. */
...
static int client_context_get_internal(Server* s, pid_t pid,
                                       const struct ucred* ucred,
                                       const char* label, size_t label_len,
                                       const char* unit_id, bool add_ref,
                                       ClientContext** ret)
{
    ...
    if (c)
    {
        ...
        if (add_ref)
        {
            if (c->in_lru)
            {
                /* The entry wasn't pinned so far, let's remove it from the LRU
                 * list then */
                assert(c->n_ref == 0);
                assert_se(prioq_remove(s->client_contexts_lru, c,
                                       &c->lru_index) >= 0);
                c->in_lru = false;
            }
        ...
        client_context_maybe_refresh(s, c, ucred, label, label_len, unit_id,
                                     USEC_INFINITY);
    ...
    client_context_really_refresh(s, c, ucred, label, label_len, unit_id,
                                  USEC_INFINITY);
...
int client_context_get(Server* s, pid_t pid, const struct ucred* ucred,
                       const char* label, size_t label_len, const char* unit_id,
                       ClientContext** ret)
{

    return client_context_get_internal(s, pid, ucred, label, label_len, unit_id,
                                       false, ret);
}

int client_context_acquire(Server* s, pid_t pid, const struct ucred* ucred,
                           const char* label, size_t label_len,
                           const char* unit_id, ClientContext** ret)
{
...
ClientContext* client_context_release(Server* s, ClientContext* c)
{
    ...
    /* The entry is not pinned anymore, let's add it to the LRU prioq if we can.
     * If we can't we'll drop it right-away */
...
void client_context_acquire_default(Server* s)
{
    ...
    /* Ensure that our own and PID1's contexts are always pinned. Our own
     * context is particularly useful to generate driver messages. */
    ...
    if (!s->my_context)
    {
        ...
        r = client_context_acquire(s, ucred.pid, &ucred, NULL, 0, NULL,
                                   &s->my_context);
        if (r < 0)
            log_warning_errno(
                r, "Failed to acquire our own context, ignoring: %m");
    }
