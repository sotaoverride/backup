                                                     ...
                                                     const CharT* end)
{
    ...
    while (begin != end)
    {
        ...
        if ((end - p) >= 2)
        {
            ...
            // From here on, no more literals are possible. Append the literal
            // element.
            {
                const unsigned int literal_chars_size =
                    static_cast<unsigned int>(descr.literal_chars.size());
                if (literal_start_pos < literal_chars_size)
                {
                    descr.format_elements.push_back(format_element::literal(
                        literal_start_pos,
                        literal_chars_size - literal_start_pos));
                ...
                if (c != static_cast<char_type>('0'))
                {
                        ...
                        *pp != static_cast<char_type>('%'))
                    {
                        boost::throw_exception(
                            ...
                            << boost::log::position_info(
                                   static_cast<unsigned int>(p -
                                                             original_begin)));
                    }

                    // Safety check against ridiculously large argument numbers
                    // which would lead to excessive memory consumption. This
                    // could be useful if the format string is gathered from an
                    // external source (e.g. a config file).
                    if (n > 1000)
                    {
                        boost::throw_exception(
                            boost::enable_error_info(limitation_error(
                                "Positional format placeholder too big"))
                            << boost::throw_file(__FILE__)
                            << boost::throw_line(__LINE__)
                            << boost::log::position_info(
                                   static_cast<unsigned int>(p -
                    ...
                    // We count positional arguments from 0, not from 1 as in
                    // format strings
                    descr.format_elements.push_back(
                        format_element::positional_argument(n - 1));
            ...
            boost::throw_exception(
                boost::enable_error_info(
                    parse_error("Unsupported format placeholder"))
                << boost::throw_file(__FILE__) << boost::throw_line(__LINE__)
                << boost::log::position_info(
                       static_cast<unsigned int>(p - original_begin)));
        }
        else
        {
            if (p != end)
    ...
    if (literal_start_pos < literal_chars_size)
        descr.format_elements.push_back(format_element::literal(
            literal_start_pos, literal_chars_size - literal_start_pos));
...
template BOOST_LOG_API format_description<char> parse_format(const char* begin,
                                                             const char* end);
...
template BOOST_LOG_API format_description<wchar_t>
    parse_format(const wchar_t* begin, const wchar_t* end);
