...
tt_face_load_cpal(TT_Face face, FT_Stream stream)
{
    FT_Error error;
    ...
    if (CPAL_V0_HEADER_BASE_SIZE + face->palette_data.num_palettes * 2U >
        table_size)
        goto InvalidTable;
    ...
    if (cpal->version == 1)
    {
        ...
        if (type_offset)
        {
            if (type_offset >= table_size)
                goto InvalidTable;
            if (face->palette_data.num_palettes * 2 > table_size - type_offset)
                goto InvalidTable;
        ...
        if (label_offset)
        {
            if (label_offset >= table_size)
                goto InvalidTable;
            if (face->palette_data.num_palettes * 2 > table_size - label_offset)
                goto InvalidTable;
    ...
    if (FT_NEW_ARRAY(face->palette, face->palette_data.num_palette_entries))
        goto NoCpal;
...
tt_face_palette_set(TT_Face face, FT_UInt palette_index)
{
    ...
    if (color_index + face->palette_data.num_palette_entries > cpal->num_colors)
