...
static void help(void)
{
    fprintf(
        stderr,
        "Usage: i2cset [-f] [-y] [-m MASK] [-r] [-a] I2CBUS CHIP-ADDRESS "
                   ...
                   int pec)
{
    ...
    fprintf(stderr,
            "I will write to device file %s, chip address "
            "0x%02x, data address\n0x%02x, ",
...
int main(int argc, char* argv[])
{
    ...
    {
        /* "c", "cp",  or implicit "b" */
        if (!strcmp(argv[flags + 4], "c") || !strcmp(argv[flags + 4], "cp"))
        {
            size = I2C_SMBUS_BYTE;
    ...
    if (!yes && !confirm(filename, address, size, daddress, value, vmask, block,
                         len, pec))
        exit(0);
