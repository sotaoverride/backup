...
{
    dt_char,    // character is delimeter if it among explicit list of some
                // characters
...
class delim_policy
{
  ...
  public:
    // Constructor
    explicit delim_policy(ti_delimeter_type type_ = dt_char,
                          cstring delimeters_ = cstring()) :
        m_type(type_)
    {
        set_delimeters(delimeters_);
    }

    void set_delimeters(ti_delimeter_type type_)
    {
        m_type = type_;
    }
...
struct token_assigner
{
    ...
    static void assign(Iterator b, Iterator e, basic_cstring<C>& t)
    {
        t.assign(b, e);
    }
#else
...
#endif
    template <typename Iterator, typename Token>
    static void append_move(Iterator& b, Token&)
    {
        ++b;
    }
};
...
struct token_assigner<single_pass_traversal_tag>
{
    ...
    template <typename Iterator, typename Token>
    static void append_move(Iterator& b, Token& t)
    {
        t += *b;
        ++b;
    }
    ...
    : public input_iterator_facade<Derived, ValueType, Reference, Traversal>
{
    ...
    typedef input_iterator_facade<Derived, ValueType, Reference, Traversal>
        base;

  protected:
    // Constructor
    explicit token_iterator_base() :
        m_is_dropped(dt_isspace), m_is_kept(dt_ispunct),
        m_keep_empty_tokens(false), m_tokens_left(static_cast<std::size_t>(-1)),
    ...
    {
        typedef ut_detail::token_assigner<
            BOOST_DEDUCED_TYPENAME iterator_traversal<Iter>::type>
            Assigner;
        Iter check_point;
...
class basic_string_token_iterator
    : public token_iterator_base<
          basic_string_token_iterator<CharT, CharCompare>, CharT, CharCompare>
{
    typedef basic_cstring<CharT const> cstring;
    typedef token_iterator_base<basic_string_token_iterator<CharT, CharCompare>,
                                CharT, CharCompare>
        base;
  ...
  public:
    ...
    explicit basic_string_token_iterator(cstring src) : m_src(src)
    {
        this->init();
    }

    // warning: making the constructor accept anything else than a cstring
    // should ensure that no temporary object is created during string creation
    // (previous definition was "template<typename Src, typename Modifier>
    // basic_string_token_iterator( Src src ..." which may create a temporary
    // string copy when called with an std::string.
    template <typename Modifier>
    basic_string_token_iterator(cstring src, Modifier const& m) : m_src(src)
    {
...
template <typename Iter,
          typename CharCompare = ut_detail::default_char_compare<
              BOOST_DEDUCED_TYPENAME iterator_value<Iter>::type>,
          typename ValueType = std::basic_string<
              BOOST_DEDUCED_TYPENAME iterator_value<Iter>::type>,
          typename Reference = ValueType const&>
class range_token_iterator
    : public token_iterator_base<
          range_token_iterator<Iter, CharCompare, ValueType, Reference>,
          typename iterator_value<Iter>::type, CharCompare, ValueType,
          Reference>
{
    typedef basic_cstring<typename ValueType::value_type> cstring;
    typedef token_iterator_base<
        range_token_iterator<Iter, CharCompare, ValueType, Reference>,
  ...
  public:
    ...
    range_token_iterator(range_token_iterator const& rhs) : base(rhs)
    {
        if (this->m_valid)
        {
            m_begin = rhs.m_begin;
...
template <typename Iter>
inline range_token_iterator<Iter> make_range_token_iterator(Iter begin,
                                                            Iter end = Iter())
{
