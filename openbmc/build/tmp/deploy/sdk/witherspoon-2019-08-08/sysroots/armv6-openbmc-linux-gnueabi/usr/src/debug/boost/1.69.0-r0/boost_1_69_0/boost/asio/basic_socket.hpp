...
class basic_socket : BOOST_ASIO_SVC_ACCESS basic_io_object<BOOST_ASIO_SVC_T>,
                     public socket_base
{
  public:
    ...
    {
        boost::system::error_code ec;
        this->get_service().assign(this->get_implementation(), protocol,
                                   native_socket, ec);
    ...
    basic_socket(
        ...
        typename enable_if<is_convertible<Protocol1, Protocol>::value>::type* =
            0) :
        basic_io_object<BOOST_ASIO_SVC_T>(other.get_service(),
    ...
    typename enable_if<is_convertible<Protocol1, Protocol>::value,
                       basic_socket>::type&
        operator=(basic_socket<Protocol1 BOOST_ASIO_SVC_TARG1>&& other)
    {
    ...
    void connect(const endpoint_type& peer_endpoint)
    {
        ...
        this->get_service().connect(this->get_implementation(), peer_endpoint,
                                    ec);
        boost::asio::detail::throw_error(ec, "connect");
    }
     ...
     * using boost::asio::io_context::post().
     *
     * @par Example
     * @code
                  ...
                  BOOST_ASIO_MOVE_ARG(ConnectHandler) handler)
    {
        ...
        return this->get_service().async_connect(
            this->get_implementation(), peer_endpoint,
            BOOST_ASIO_MOVE_CAST(ConnectHandler)(handler));
#else  // defined(BOOST_ASIO_ENABLE_OLD_SERVICES)
        async_completion<ConnectHandler, void(boost::system::error_code)> init(
            handler);
     ...
     * boost::asio::error::would_block if they are unable to perform the
     ...
     * error boost::asio::error::would_block.
     */
    BOOST_ASIO_SYNC_OP_VOID non_blocking(bool mode,
                                         boost::system::error_code& ec)
    {
        this->get_service().non_blocking(this->get_implementation(), mode, ec);
        BOOST_ASIO_SYNC_OP_VOID_RETURN(ec);
     ...
     * mode and direct system calls may fail with
     * boost::asio::error::would_block (or the equivalent system error).
     *
     ...
     * } @endcode
     */
    BOOST_ASIO_SYNC_OP_VOID native_non_blocking(bool mode,
                                                boost::system::error_code& ec)
    {
        this->get_service().native_non_blocking(this->get_implementation(),
                                                mode, ec);
    ...
    {
        return this->get_service().local_endpoint(this->get_implementation(),
                                                  ec);
    }
    ...
    async_wait(wait_type w, BOOST_ASIO_MOVE_ARG(WaitHandler) handler)
    {
        ...
        return this->get_service().async_wait(
            this->get_implementation(), w,
            BOOST_ASIO_MOVE_CAST(WaitHandler)(handler));
#else  // defined(BOOST_ASIO_ENABLE_OLD_SERVICES)
        async_completion<WaitHandler, void(boost::system::error_code)> init(
            handler);

        this->get_service().async_wait(this->get_implementation(), w,
                                       init.completion_handler);
