...
#define FILE_ATTRIBUTE_DEVICE 64
    ...
    while (test & G_FILE_TEST_IS_EXECUTABLE)
    {
        ...
        if (_stricmp(lastdot, ".exe") == 0 || _stricmp(lastdot, ".cmd") == 0 ||
            _stricmp(lastdot, ".bat") == 0 || _stricmp(lastdot, ".com") == 0)
            return TRUE;
...
static char* format_error_message(const gchar* filename,
                                  const gchar* format_string, int saved_errno)
    G_GNUC_FORMAT(2);
 ...
 */
static void set_file_error(GError** error, const gchar* filename,
                           const gchar* format_string, int saved_errno)
{
    char* msg = format_error_message(filename, format_string, saved_errno);

    g_set_error_literal(error, G_FILE_ERROR,
                        g_file_error_from_errno(saved_errno), msg);
    g_free(msg);
}

static gboolean get_contents_stdio(const gchar* filename, FILE* f,
    ...
    while (!feof(f))
    {
        ...
        if (ferror(f))
        {
            ...
            g_set_error(error, G_FILE_ERROR,
                        ...
                        _("Error reading file “%s”: %s"), display_filename,
...
file_too_large:
    ...
    g_set_error(error, G_FILE_ERROR, G_FILE_ERROR_FAILED,
                _("File “%s” is too large"), display_filename);
    g_free(display_filename);
...
static gboolean get_contents_regfile(const gchar* filename,
                                     struct stat* stat_buf, gint fd,
                                     gchar** contents, gsize* length,
                                     GError** error)
{
    gchar* buf;
    ...
    if (buf == NULL)
    {
        ...
        g_set_error(
            ...
            g_dngettext(GETTEXT_PACKAGE,
                        ...
                        "Could not allocate %lu bytes to read file “%s”",
                        (gulong)alloc_size),
            (gulong)alloc_size, display_filename);
        g_free(display_filename);
        goto error;
    }
...
static gboolean get_contents_posix(const gchar* filename, gchar** contents,
                                   gsize* length, GError** error)
{
    ...
    if (fd < 0)
    {
        ...
        set_file_error(error, filename, _("Failed to open file “%s”: %s"),
                       saved_errno);
    ...
    {
        gboolean retval = get_contents_regfile(filename, &stat_buf, fd,
...
static gboolean get_contents_win32(const gchar* filename, gchar** contents,
                                   gsize* length, GError** error)
{
    FILE* f;
    gboolean retval;
...
/**
 * g_file_get_contents:
 * @filename: (type filename): name of a file to read contents from, in the GLib
 *file name encoding
 * @contents: (out) (array length=length) (element-type guint8): location to
 *store an allocated string, use g_free() to free the returned string
 * @length: (nullable): location to store length in bytes of the contents, or
 *%NULL
...
static gboolean rename_file(const char* old_name, const char* new_name,
                            GError** err)
{
    errno = 0;
    if (g_rename(old_name, new_name) == -1)
    {
        ...
        g_set_error(
            err, G_FILE_ERROR, g_file_error_from_errno(save_errno),
...
static gchar* write_to_temp_file(const gchar* contents, gssize length,
                                 const gchar* dest_file, GError** err)
{
    gchar* tmp_name;
    gchar* retval;
    gint fd;
...
#ifdef HAVE_FSYNC
    {
         ...
         */
        if (g_lstat(dest_file, &statbuf) == 0 && statbuf.st_size > 0 &&
            fsync(fd) != 0)
        {
            int saved_errno = errno;
            set_file_error(err, tmp_name,
...
/**
 * g_file_set_contents:
 * @filename: (type filename): name of a file to write @contents to, in the GLib
 * file name encoding
 * @contents: (array length=length) (element-type guint8): string to write to
 * the file
 ...
 */
static gint wrap_g_mkdir(const gchar* filename, int flags G_GNUC_UNUSED,
                         int mode)
{
...
static gint wrap_g_open(const gchar* filename, int flags, int mode)
{
...
static gint g_get_tmp_name(const gchar* tmpl, gchar** name_used,
                           GTmpFileCallback f, gint flags, gint mode,
                           GError** error)
{
    int retval;
    const char* tmpdir;
    ...
    if (strstr(tmpl, "XXXXXX") == NULL)
    {
        gchar* display_tmpl = g_filename_display_name(tmpl);
        g_set_error(error, G_FILE_ERROR, G_FILE_ERROR_FAILED,
                    _("Template “%s” doesn’t contain XXXXXX"), display_tmpl);
        g_free(display_tmpl);
        return -1;
    }
    ...
    {
        int saved_errno = errno;
        set_file_error(error, fulltemplate, _("Failed to create file “%s”: %s"),
                       saved_errno);
        g_free(fulltemplate);
        return -1;
    }
...
gint g_file_open_tmp(const gchar* tmpl, gchar** name_used, GError** error)
{
    ...
    result = g_get_tmp_name(tmpl, &fulltemplate, wrap_g_open,
                            O_CREAT | O_EXCL | O_RDWR | O_BINARY, 0600, error);
    if (result != -1)
    {
        if (name_used)
            *name_used = fulltemplate;
...
/**
 * g_build_path:
 * @separator: (type filename): a string used to separator the elements of the
 *path.
...
static gchar* g_build_filename_va(const gchar* first_argument, va_list* args,
                                  gchar** str_array)
{
    gchar* str;
...
const gchar* g_path_skip_root(const gchar* file_name)
{
    ...
    if (G_IS_DIR_SEPARATOR(file_name[0]) && G_IS_DIR_SEPARATOR(file_name[1]) &&
        file_name[2] && !G_IS_DIR_SEPARATOR(file_name[2]))
    {
        gchar* p;
...
#ifdef G_OS_WIN32
    if (last_nonslash == 1 && g_ascii_isalpha(file_name[0]) &&
        file_name[1] == ':')
...
gchar* g_path_get_dirname(const gchar* file_name)
{
     ...
     */
    if (base == file_name + 1 && g_ascii_isalpha(file_name[0]) &&
        file_name[1] == ':')
        base++;
    else if (G_IS_DIR_SEPARATOR(file_name[0]) &&
             G_IS_DIR_SEPARATOR(file_name[1]) && file_name[2] &&
             !G_IS_DIR_SEPARATOR(file_name[2]) && base >= file_name + 2)
    {
...
gchar* g_canonicalize_filename(const gchar* filename, const gchar* relative_to)
{
    ...
    g_return_val_if_fail(relative_to == NULL || g_path_is_absolute(relative_to),
                         NULL);
...
gchar* g_get_current_dir(void)
{
    ...
    pwd = g_getenv("PWD");
    if (pwd != NULL && g_stat(".", &dotbuf) == 0 && g_stat(pwd, &pwdbuf) == 0 &&
...
_GLIB_EXTERN gboolean g_file_test_utf8(const gchar* filename, GFileTest test);
_GLIB_EXTERN gboolean g_file_get_contents_utf8(const gchar* filename,
                                               gchar** contents, gsize* length,
                                               GError** error);
_GLIB_EXTERN gint g_mkstemp_utf8(gchar* tmpl);
_GLIB_EXTERN gint g_file_open_tmp_utf8(const gchar* tmpl, gchar** name_used,
                                       GError** error);
...
gboolean g_file_get_contents_utf8(const gchar* filename, gchar** contents,
                                  gsize* length, GError** error)
{
