...
static OM_uint32 kg_unseal_v1_iov(krb5_context context, OM_uint32* minor_status,
                                  krb5_gss_ctx_id_rec* ctx,
                                  gss_iov_buffer_desc* iov, int iov_count,
                                  size_t token_wrapper_len, int* conf_state,
                                  gss_qop_t* qop_state, int toktype)
{
    ...
    if (toktype == KG_TOK_WRAP_MSG)
    {
        if (sealalg != 0xFFFF)
        {
            if (ctx->sealalg == SEAL_ALG_MICROSOFT_RC4)
            {
                ...
                code = kg_arcfour_docrypt_iov(
                    context, enc_key, 0, &bigend_seqnum[0], 4, iov, iov_count);
            ...
            else
            {
                code = kg_decrypt_iov(context, 0,
                                      ...
                                      0 /*EC*/, 0 /*RRC*/, ctx->enc,
                                      KG_USAGE_SEAL, NULL, iov, iov_count);
            }
            if (code != 0)
            {
                retval = GSS_S_FAILURE;
    ...
    /* compute the checksum of the message */
    code = kg_make_checksum_iov_v1(context, md5cksum.checksum_type, cksum_len,
                                   ctx->seq, ctx->enc, sign_usage, iov,
                                   iov_count, toktype, &md5cksum);
    if (code != 0)
    {
        retval = GSS_S_FAILURE;
        goto cleanup;
    ...
    switch (signalg)
    {
        case SGN_ALG_DES_MAC_MD5:
        case SGN_ALG_3:
            code = kg_encrypt_inplace(
                context, ctx->seq, KG_USAGE_SEAL,
                (g_OID_equal(ctx->mech_used, gss_mech_krb5_old)
                     ? ctx->seq->keyblock.contents
                     : NULL),
     ...
     */
    if (toktype == KG_TOK_WRAP_MSG && (ctx->gss_flags & GSS_C_DCE_STYLE) == 0)
    {
 ...
 */
static OM_uint32 kg_unseal_iov_token(OM_uint32* minor_status,
                                     krb5_gss_ctx_id_rec* ctx, int* conf_state,
                                     gss_qop_t* qop_state,
                                     gss_iov_buffer_desc* iov, int iov_count,
                                     int toktype)
{
    ...
    if ((ctx->gss_flags & GSS_C_DCE_STYLE) == 0 && toktype == KG_TOK_WRAP_MSG)
    {
    ...
    code = g_verify_token_header(ctx->mech_used, &bodysize, &ptr, -1,
                                 input_length, 0);
    if (code != 0)
    {
        *minor_status = code;
    ...
    switch (toktype2)
    {
        ...
        case KG2_TOK_DEL_CTX:
            code = gss_krb5int_unseal_v3_iov(context, minor_status, ctx, iov,
                                             iov_count, conf_state, qop_state,
                                             toktype);
            break;
        case KG_TOK_MIC_MSG:
        ...
        case KG_TOK_DEL_CTX:
            code = kg_unseal_v1_iov(
                context, minor_status, ctx, iov, iov_count,
 ...
 */
static OM_uint32 kg_unseal_stream_iov(OM_uint32* minor_status,
                                      krb5_gss_ctx_id_rec* ctx, int* conf_state,
                                      gss_qop_t* qop_state,
                                      gss_iov_buffer_desc* iov, int iov_count,
                                      int toktype)
{
    ...
    tiov = (gss_iov_buffer_desc*)calloc((size_t)iov_count + 2,
        ...
        case KG2_TOK_DEL_CTX:
        {
            ...
            if (rrc != 0)
            {
                if (!gss_krb5int_rotate_left(
                        (unsigned char*)stream->buffer.value + 16,
            ...
            if (conf_req_flag)
            {
                code = krb5_c_crypto_length(
                    context, enctype, KRB5_CRYPTO_TYPE_HEADER, &k5_headerlen);
                if (code != 0)
                    goto cleanup;
                theader->buffer.length +=
                    k5_headerlen; /* length validated later */
            }

            /* no PADDING for CFX, EC is used instead */
            code =
                krb5_c_crypto_length(context, enctype,
                                     conf_req_flag ? KRB5_CRYPTO_TYPE_TRAILER
            ...
            ttrailer->buffer.length =
                ec + (conf_req_flag ? 16 : 0 /* E(Header) */) + k5_trailerlen;
    ...
    /* CFX: GSS-Header | Kerb-Header | Data  |     | EC | E(Header) |
     * Kerb-Trailer */
    /* GSS:
     * -------GSS-HEADER--------+-DATA--+-PAD-+----------GSS-TRAILER----------*/
    ...
    major_status = kg_unseal_iov_token(&code, ctx, conf_state, qop_state, tiov,
                                       i, toktype);
    ...
    gss_qop_t* qop_state, gss_iov_buffer_desc* iov, int iov_count)
{
    ...
    major_status = kg_unseal_iov(minor_status, context_handle, conf_state,
                                 qop_state, iov, iov_count, KG_TOK_WRAP_MSG);

    return major_status;
}

OM_uint32 KRB5_CALLCONV krb5_gss_verify_mic_iov(OM_uint32* minor_status,
                                                ...
                                                int iov_count)
{
    ...
    major_status = kg_unseal_iov(minor_status, context_handle, NULL, qop_state,
