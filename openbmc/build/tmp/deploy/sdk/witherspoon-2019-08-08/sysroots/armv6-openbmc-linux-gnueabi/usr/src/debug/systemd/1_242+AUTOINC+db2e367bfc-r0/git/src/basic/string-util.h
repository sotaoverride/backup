...
{
    return !str || str[0] == 0 || (str[0] == '-' && str[1] == 0);
}
...
#define _FOREACH_WORD(word, length, s, separator, flags, state)                \
    for ((state) = (s),                                                        \
        (word) = split(&(state), &(length), (separator), (flags));             \
         (word); (word) = split(&(state), &(length), (separator), (flags)))
...
#define strjoina(a, ...)                                                       \
    ({                                                                         \
        ...
        size_t _len_ = 0;                                                      \
        size_t _i_;                                                            \
        for (_i_ = 0; _i_ < ELEMENTSOF(_appendees_) && _appendees_[_i_];       \
             _i_++)                                                            \
...
char* ellipsize_mem(const char* s, size_t old_length_bytes,
                    size_t new_length_columns, unsigned percent);
...
static inline void* memory_startswith(const void* p, size_t sz,
                                      const char* token)
{
    size_t n;
 ...
 */
static inline void* memory_startswith_no_case(const void* p, size_t sz,
                                              const char* token)
{
    size_t n, i;
