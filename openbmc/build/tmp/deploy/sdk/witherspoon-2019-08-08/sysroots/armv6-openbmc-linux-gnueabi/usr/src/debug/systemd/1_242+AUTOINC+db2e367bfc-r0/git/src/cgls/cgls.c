...
static void show_cg_info(const char* controller, const char* path)
{
    ...
    if (cg_all_unified() == 0 && controller &&
...
static int run(int argc, char* argv[])
{
    ...
    output_flags = arg_all * OUTPUT_SHOW_ALL |
        ...
        STRV_FOREACH(name, arg_names)
        {
            ...
            if (arg_show_unit != SHOW_UNIT_NONE)
            {
                ...
                if (!bus)
                {
                    ...
                    r = bus_connect_transport_systemd(
                        ...
                        arg_show_unit == SHOW_UNIT_USER, &bus);
                    if (r < 0)
                        return log_error_errno(
                            r, "Failed to create bus connection: %m");
                }
            ...
            else
            {
                ...
                if (!root)
                {
                    ...
                    if (r < 0)
                        return log_error_errno(
                            r, "Failed to list cgroup tree: %m");
                }

                q = cg_split_spec(*name, &c, &p);
                if (q < 0)
                {
                    log_error_errno(q, "Failed to split argument %s: %m",
                                    *name);
                    goto failed;
                }
    ...
    else
    {
        ...
        if (!arg_machine)
        {
            ...
            if (r < 0)
                return log_error_errno(
                    r, "Cannot determine current working directory: %m");
