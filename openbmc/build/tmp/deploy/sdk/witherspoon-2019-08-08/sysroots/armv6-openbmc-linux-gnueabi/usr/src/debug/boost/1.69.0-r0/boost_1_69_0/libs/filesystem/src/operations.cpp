...
// Versions of MinGW or Cygwin that support Filesystem V3 support at least
// WINVER 0x501. See MinGW's windef.h
#define WINVER 0x501
#endif
#include <io.h>
#include <windows.h>
...
#if !defined(                                                                  \
    REPARSE_DATA_BUFFER_HEADER_SIZE) // mingw winnt.h does provide the defs
...
inline std::wstring wgetenv(const wchar_t* name)
{
    ...
    return (buf.empty() ||
            ::GetEnvironmentVariableW(name, &buf[0],
...
#define BOOST_COPY_DIRECTORY(F, T)                                             \
    (!(::stat(from.c_str(), &from_stat) != 0 ||                                \
       ::mkdir(to.c_str(), from_stat.st_mode) != 0))
#define BOOST_COPY_FILE(F, T, FailIfExistsBool)                                \
    copy_file_api(F, T, FailIfExistsBool)
...
#define BOOST_CREATE_SYMBOLIC_LINK(F, T, Flag)                                 \
...
//  error_num is value of errno on POSIX, error code (from ::GetLastError()) on
//  Windows. Interface changed 30 Jan 15 to have caller supply error_num as
//  ::SetLastError() values were apparently getting cleared before they could be
//  retrieved by error().
    ...
    { //  error
        if (ec == 0)
            BOOST_FILESYSTEM_THROW(filesystem_error(
                message, error_code(error_num, system_category())));
        else
...
bool error(err_t error_num, const path& p, error_code* ec, const char* message)
{
    ...
    { //  error
        if (ec == 0)
            BOOST_FILESYSTEM_THROW(filesystem_error(
                message, p, error_code(error_num, system_category())));
        else
            ec->assign(error_num, system_category());
    }
    return error_num != 0;
           ...
           const char* message)
{
    ...
    { //  error
        if (ec == 0)
            BOOST_FILESYSTEM_THROW(filesystem_error(
                message, p1, p2, error_code(error_num, system_category())));
        else
            ec->assign(error_num, system_category());
    }
    return error_num != 0;
...
{
    return (ec != 0 ? fs::directory_iterator(p, *ec)
                    : fs::directory_iterator(p)) == end_dir_itr;
}
...
bool remove_directory(const path& p) // true if succeeds or not found
{
    return BOOST_REMOVE_DIRECTORY(p.c_str()) ||
           not_found_error(
               BOOST_ERRNO); // mitigate possible file system race. See #11166
}
...
bool // true if ok
    copy_file_api(const std::string& from_p, const std::string& to_p,
                  bool fail_if_exists)
{
    ...
    // introduced a gratuitous race condition; the stat()is now done after the
    // open()
    ...
    while (sz_read > 0 && (sz_read = ::read(infile, buf.get(), buf_sz)) > 0)
    {
        ...
        do
        {
            BOOST_ASSERT(
                sz_read - sz_write >
                0); // #1
                    // ticket 4438 claimed possible infinite loop if write
                    // returns 0. My analysis is that POSIX specifies 0 return
                    // only if 3rd arg is 0, and that will never happen due to
                    // loop entry and coninuation conditions. BOOST_ASSERT #1
                    // above and #2 below added to verify that analysis.
...
bool not_found_error(int errval)
{
    return errval == ERROR_FILE_NOT_FOUND || errval == ERROR_PATH_NOT_FOUND ||
           ...
           || errval ==
                  ERROR_INVALID_DRIVE   // USB card reader with no card inserted
...
static bool equal_extension(wchar_t const* p, wchar_t const (&x1)[5],
                            wchar_t const (&x2)[5])
{
...
bool is_reparse_point_a_symlink(const path& p)
{
    handle_wrapper h(create_file_handle(
        p, FILE_READ_EA, FILE_SHARE_READ | FILE_SHARE_WRITE | FILE_SHARE_DELETE,
        NULL, OPEN_EXISTING,
    ...
    DWORD dwRetLen;
    BOOL result =
        ::DeviceIoControl(h.handle, FSCTL_GET_REPARSE_POINT, NULL, 0, buf.get(),
    ...
    return reinterpret_cast<const REPARSE_DATA_BUFFER*>(buf.get())
           ...
           // directories. That is equivalent to recognizing them as symlinks,
           // and then the normal symlink mechanism will take care of
           // recognizing them as directories.
           //
           ...
           // target-path".
           || reinterpret_cast<const REPARSE_DATA_BUFFER*>(buf.get())
                      ->ReparseTag ==
                  IO_REPARSE_TAG_MOUNT_POINT; // aka "directory junction" or
                                              // "junction"
}

inline std::size_t get_full_path_name(const path& src, std::size_t len,
                                      wchar_t* buf, wchar_t** p)
{
...
fs::file_status process_status_failure(const path& p, error_code* ec)
{
    ...
    if (ec == 0)
        BOOST_FILESYSTEM_THROW(
            filesystem_error("boost::filesystem::status", p,
                             error_code(errval, system_category())));
    return fs::file_status(fs::status_error);
}

//  differs from symlink_status() in that directory symlinks are reported as
//  _detail_directory_symlink, as required on Windows by remove() and its
//  helpers.
fs::file_type query_file_type(const path& p, error_code* ec)
{
    ...
    return (attr & FILE_ATTRIBUTE_DIRECTORY) ? fs::directory_file
                                             : fs::regular_file;
}
...
path absolute(const path& p, const path& base)
{
    ...
    //    if (p.has_root_name())
    //      return p.root_name()
    //        / abs_base.root_directory() / abs_base.relative_path() /
    //        p.relative_path();
    //    //  !p.has_root_name()
    //# endif
    //    return abs_base / p;
...
void copy_file(const path& from, const path& to, copy_option option,
               error_code* ec)
{
    error(!BOOST_COPY_FILE(from.c_str(), to.c_str(), option == fail_if_exists)
              ? BOOST_ERRNO
              : 0,
...
bool create_directories(const path& p, system::error_code* ec)
{
    if (p.empty())
    {
        if (ec == 0)
            ...
            ec->assign(system::errc::invalid_argument,
                       system::generic_category());
        return false;
    }
...
bool create_directory(const path& p, error_code* ec)
{
    ...
    if (ec == 0)
        BOOST_FILESYSTEM_THROW(
            filesystem_error("boost::filesystem::create_directory", p,
                             error_code(errval, system_category())));
    else
...
{
#if defined(BOOST_WINDOWS_API) &&                                              \
    _WIN32_WINNT < 0x0600 // SDK earlier than Vista and Server 2008
    ...
    if (error(!create_symbolic_link_api ? BOOST_ERROR_NOT_SUPPORTED : 0, to,
              from, ec, "boost::filesystem::create_directory_symlink"))
        return;
#endif
...
void create_hard_link(const path& to, const path& from, error_code* ec)
{
...
#if defined(BOOST_WINDOWS_API) &&                                              \
    _WIN32_WINNT < 0x0500 // SDK earlier than Win 2K
    ...
    // see if actually supported by Windows runtime dll
    if (error(!create_hard_link_api ? BOOST_ERROR_NOT_SUPPORTED : 0, to, from,
              ec, "boost::filesystem::create_hard_link"))
        return;
#endif

    error(!BOOST_CREATE_HARD_LINK(from.c_str(), to.c_str()) ? BOOST_ERRNO : 0,
          to, from, ec, "boost::filesystem::create_hard_link");
#endif
}
...
void create_symlink(const path& to, const path& from, error_code* ec)
{
    ...
    // see if actually supported by Windows runtime dll
    if (error(!create_symbolic_link_api ? BOOST_ERROR_NOT_SUPPORTED : 0, to,
              from, ec, "boost::filesystem::create_symlink"))
        return;
#endif

    error(!BOOST_CREATE_SYMBOLIC_LINK(from.c_str(), to.c_str(), 0) ? BOOST_ERRNO
                                                                   : 0,
...
path current_path(error_code* ec)
{
    ...
    for (long path_max = 128;; path_max *= 2) // loop 'til buffer large enough
    {
        ...
        {
            if (error(errno != ERANGE ? errno
                                      : 0
            // bug in some versions of the Metrowerks C lib on the Mac: wrong
            // errno set
#if defined(__MSL__) &&                                                        \
...
{
    error(!BOOST_SET_CURRENT_DIRECTORY(p.c_str()) ? BOOST_ERRNO : 0, p, ec,
          "boost::filesystem::current_path");
}
...
bool equivalent(const path& p1, const path& p2, system::error_code* ec)
{
    ...
    handle_wrapper h2(create_file_handle(
        p2.c_str(), 0, FILE_SHARE_DELETE | FILE_SHARE_READ | FILE_SHARE_WRITE,
        0, OPEN_EXISTING, FILE_FLAG_BACKUP_SEMANTICS, 0));

    handle_wrapper h1(create_file_handle(
        p1.c_str(), 0, FILE_SHARE_DELETE | FILE_SHARE_READ | FILE_SHARE_WRITE,
        0, OPEN_EXISTING, FILE_FLAG_BACKUP_SEMANTICS, 0));
    ...
    if (error(!::GetFileInformationByHandle(h1.handle, &info1) ? BOOST_ERRNO
                                                               : 0,
              p1, p2, ec, "boost::filesystem::equivalent"))
        return false;

    if (error(!::GetFileInformationByHandle(h2.handle, &info2) ? BOOST_ERRNO
                                                               : 0,
              p1, p2, ec, "boost::filesystem::equivalent"))
        return false;
...
boost::uintmax_t file_size(const path& p, error_code* ec)
{
    ...
    if (error(!S_ISREG(path_stat.st_mode) ? EPERM : 0, p, ec,
              "boost::filesystem::file_size"))
        return static_cast<boost::uintmax_t>(-1);
    ...
    if (error(::GetFileAttributesExW(p.c_str(), ::GetFileExInfoStandard,
                                     &fad) == 0
                  ? BOOST_ERRNO
                  : 0,
...
boost::uintmax_t hard_link_count(const path& p, system::error_code* ec)
{
    ...
    struct stat path_stat;
    return error(::stat(p.c_str(), &path_stat) != 0 ? BOOST_ERRNO : 0, p, ec,
    ...
    handle_wrapper h(create_file_handle(
        p.c_str(), 0, FILE_SHARE_DELETE | FILE_SHARE_READ | FILE_SHARE_WRITE, 0,
        OPEN_EXISTING, FILE_FLAG_BACKUP_SEMANTICS, 0));
    return !error(h.handle == INVALID_HANDLE_VALUE ? BOOST_ERRNO : 0, p, ec,
                  "boost::filesystem::hard_link_count") &&
...
bool is_empty(const path& p, system::error_code* ec)
{
    ...
    struct stat path_stat;
    if (error(::stat(p.c_str(), &path_stat) != 0, p, ec,
              "boost::filesystem::is_empty"))
        return false;
    return S_ISDIR(path_stat.st_mode) ? is_empty_directory(p, ec)
                                      : path_stat.st_size == 0;
#else
...
std::time_t last_write_time(const path& p, system::error_code* ec)
{
    ...
    if (error(hw.handle == INVALID_HANDLE_VALUE ? BOOST_ERRNO : 0, p, ec,
              "boost::filesystem::last_write_time"))
        return std::time_t(-1);

    FILETIME lwt;

    if (error(::GetFileTime(hw.handle, 0, 0, &lwt) == 0 ? BOOST_ERRNO : 0, p,
              ec, "boost::filesystem::last_write_time"))
        return std::time_t(-1);
                     ...
                     system::error_code* ec)
{
    ...
    error(::utime(p.c_str(), &buf) != 0 ? BOOST_ERRNO : 0, p, ec,
          "boost::filesystem::last_write_time");

#else

    handle_wrapper hw(create_file_handle(
        p.c_str(), FILE_WRITE_ATTRIBUTES,
    ...
    error(::SetFileTime(hw.handle, 0, 0, &lwt) == 0 ? BOOST_ERRNO : 0, p, ec,
          "boost::filesystem::last_write_time");
#endif
}
...
void permissions(const path& p, perms prms, system::error_code* ec)
{
    ...
    if (error(attr == 0 ? BOOST_ERRNO : 0, p, ec,
    ...
    error(::SetFileAttributesW(p.c_str(), attr) == 0 ? BOOST_ERRNO : 0, p, ec,
          "boost::filesystem::permissions");
#endif
}
...
path read_symlink(const path& p, system::error_code* ec)
{
    ...
    for (std::size_t path_max = 64;;
         path_max *= 2) // loop 'til buffer large enough
    {
        ...
        if ((result = ::readlink(p.c_str(), buf.get(), path_max)) == -1)
        {
            if (ec == 0)
                BOOST_FILESYSTEM_THROW(
                    filesystem_error("boost::filesystem::read_symlink", p,
    ...
    handle_wrapper h(create_file_handle(
        p.c_str(), GENERIC_READ, 0, 0, OPEN_EXISTING,
        FILE_FLAG_BACKUP_SEMANTICS | FILE_FLAG_OPEN_REPARSE_POINT, 0));

    if (error(h.handle == INVALID_HANDLE_VALUE ? BOOST_ERRNO : 0, p, ec,
              "boost::filesystem::read_symlink"))
        return symlink_path;

    DWORD sz;

    if (!error(::DeviceIoControl(h.handle, FSCTL_GET_REPARSE_POINT, 0, 0,
                                 info.buf, sizeof(info), &sz, 0) == 0
                   ? BOOST_ERRNO
                   : 0,
               p, ec, "boost::filesystem::read_symlink"))
        symlink_path.assign(
            static_cast<wchar_t*>(
                info.rdb.SymbolicLinkReparseBuffer.PathBuffer) +
...
bool remove(const path& p, error_code* ec)
{
    ...
    // Since POSIX remove() is specified to work with either files or
    // directories, in a perfect world it could just be called. But some
    // important real-world operating systems (Windows, Mac OS X, for example)
    // don't implement the POSIX spec. So remove_file_or_directory() is always
    // called to keep it simple.
...
{
    error(!BOOST_MOVE_FILE(old_p.c_str(), new_p.c_str()) ? BOOST_ERRNO : 0,
          old_p, new_p, ec, "boost::filesystem::rename");
}
...
#if defined(BOOST_POSIX_API)
    if (BOOST_UNLIKELY(
            size > static_cast<uintmax_t>((std::numeric_limits<off_t>::max)())))
    {
        error(system::errc::file_too_large, p, ec,
...
space_info space(const path& p, error_code* ec)
{
    ...
    space_info info;
    if (!error(::BOOST_STATVFS(p.c_str(), &vfs) ? BOOST_ERRNO : 0, p, ec,
               "boost::filesystem::space"))
    {
        info.capacity = static_cast<boost::uintmax_t>(vfs.f_blocks) *
                        BOOST_STATVFS_F_FRSIZE;
        info.free =
            static_cast<boost::uintmax_t>(vfs.f_bfree) * BOOST_STATVFS_F_FRSIZE;
        info.available = static_cast<boost::uintmax_t>(vfs.f_bavail) *
                         BOOST_STATVFS_F_FRSIZE;
    }
    ...
    if (!error(::GetDiskFreeSpaceExW(p.c_str(), &avail, &total, &free) == 0, p,
               ec, "boost::filesystem::space"))
    {
        info.capacity = (static_cast<boost::uintmax_t>(total.HighPart) << 32) +
                        total.LowPart;
        info.free =
            (static_cast<boost::uintmax_t>(free.HighPart) << 32) + free.LowPart;
        info.available = (static_cast<boost::uintmax_t>(avail.HighPart) << 32) +
                         avail.LowPart;
    }
...
file_status status(const path& p, error_code* ec)
{
    ...
    if (S_ISDIR(path_stat.st_mode))
        return fs::file_status(fs::directory_file,
                               static_cast<perms>(path_stat.st_mode) &
                                   fs::perms_mask);
...
path temp_directory_path(system::error_code* ec)
{
    ...
    (val = std::getenv("TMPDIR")) || (val = std::getenv("TMP")) ||
        (val = std::getenv("TEMP")) || (val = std::getenv("TEMPDIR"));
    ...
    const wchar_t* env_list[] = {tmp_env, temp_env, localappdata_env,
                                 userprofile_env, 0};
    ...
    for (int i = 0; env_list[i]; ++i)
    {
        ...
        if (!env.empty())
        {
            ...
            if (i >= 2)
                p /= L"Temp";
            error_code lcl_ec;
            if (exists(p, lcl_ec) && !lcl_ec && is_directory(p, lcl_ec) &&
                !lcl_ec)
                break;
            p.clear();
        }
...
path system_complete(const path& p, system::error_code* ec)
{
    ...
    if (error(len == 0 ? BOOST_ERRNO : 0, p, ec,
...
path weakly_canonical(const path& p, system::error_code* ec)
{
    ...
    for (; !head.empty(); --itr)
    {
        ...
        if (error(head_status.type() == fs::status_error, head, ec,
...
error_code dir_itr_first(void*& handle, void*& buffer, const char* dir,
                         string& target, fs::file_status&, fs::file_status&)
{
    ...
    std::size_t path_size(
        0); // initialization quiets gcc warning (ticket #3509)
    error_code ec = path_max(path_size);
    if (ec)
        return ec;
    dirent de;
    buffer = std::malloc((sizeof(dirent) - sizeof(de.d_name)) + path_size +
                         1); // + 1 for "/0"
    return ok;
}

// warning: the only dirent member updated is d_name
inline int readdir_r_simulator(
...
error_code dir_itr_increment(void*& handle, void*& buffer, string& target,
                             fs::file_status& sf, fs::file_status& symlink_sf)
{
    ...
    dirent* result;
    int return_code;
    if ((return_code = readdir_r_simulator(static_cast<DIR*>(handle), entry,
                                           &result)) != 0)
        return error_code(errno, system_category());
    if (result == 0)
...
error_code dir_itr_first(void*& handle, const fs::path& dir, wstring& target,
                         fs::file_status& sf, fs::file_status& symlink_sf)
...
// error. It is treated as eof instead.
{
    ...
    dirpath += (dirpath.empty() || (dirpath[dirpath.size() - 1] != L'\\' &&
    ...
    // reparse points are complex, so don't try to handle them here; instead
    // just mark them as status_error which causes directory_entry caching to
    // call status() and symlink_status() which do handle reparse points fully
    {
                             ...
                             fs::file_status& sf, fs::file_status& symlink_sf)
{
    ...
    if (::FindNextFileW(handle, &data) == 0) // fails
    {
        ...
        return error_code(error == ERROR_NO_MORE_FILES ? 0 : error,
                          system_category());
    }
    target = data.cFileName;
    if (data.dwFileAttributes & FILE_ATTRIBUTE_REPARSE_POINT)
                                  ...
                                  system::error_code* ec)
{
    ...
    else                  // not eof
    {
        ...
        if (filename[0] == dot // dot or dot-dot
            && (filename.size() == 1 ||
                (filename[1] == dot && filename.size() == 2)))
        {
                                  ...
                                  system::error_code* ec)
{
    ...
    for (;;)
    {
        ...
        if (temp_ec) // happens if filesystem is corrupt, such as on a damaged
                     // optical disc
        {
            path error_path(
                it.m_imp->dir_entry.path().parent_path()); // fix ticket #5900
            it.m_imp.reset();
            if (ec == 0)
                BOOST_FILESYSTEM_THROW(filesystem_error(
                    "boost::filesystem::directory_iterator::operator++",
                    error_path, error_code(BOOST_ERRNO, system_category())));
        ...
        if (!(filename[0] == dot // !(dot or dot-dot)
              && (filename.size() == 1 ||
                  (filename[1] == dot && filename.size() == 2))))
        {
            it.m_imp->dir_entry.replace_filename(filename, file_stat,
                                                 symlink_file_stat);
            return;
        }
    }
