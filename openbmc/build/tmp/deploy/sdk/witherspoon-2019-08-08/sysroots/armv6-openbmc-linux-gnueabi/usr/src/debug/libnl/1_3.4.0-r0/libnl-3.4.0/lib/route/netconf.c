...
static void netconf_dump_line(struct nl_object* obj, struct nl_dump_params* p)
{
    ...
    switch (nc->ifindex)
    {
        ...
        default:
            ...
            {
                nl_dump(p, "dev %s ",
                        rtnl_link_i2name(link_cache, nc->ifindex, buf,
                                         sizeof(buf)));
    ...
    {
        nl_dump(p, "forwarding %s ", nc->forwarding ? "on" : "off");
    }
    ...
    {
        nl_dump(p, "mc_forwarding %s ", nc->mc_forwarding ? "on" : "off");
    }
                           ...
                           uint32_t table_sz)
{
    ...
    *hashkey = nl_hash(&nckey, nckey_sz, 0) % table_sz;
    ...
    NL_DBG(5, "netconf %p key (dev %d fam %d) keysz %d, hash 0x%x\n", nc,
           nckey.nc_index, nckey.nc_family, nckey_sz, *hashkey);
}
...
#define NETCONF_DIFF(ATTR, EXPR)                                               \
    ATTR_DIFF(attrs, NETCONF_ATTR_##ATTR, a, b, EXPR)
    ...
    diff |= NETCONF_DIFF(IGNORE_RT_LINKDWN, a->ignore_routes_linkdown !=
                                                b->ignore_routes_linkdown);
                                             ...
                                             int ifindex)
{
    ...
    {
        if (nc->ifindex == ifindex && nc->family == family)
        {
...
static struct nl_object_ops netconf_obj_ops = {
    ...
    .oo_id_attrs = (NETCONF_ATTR_FAMILY | NETCONF_ATTR_IFINDEX)};
