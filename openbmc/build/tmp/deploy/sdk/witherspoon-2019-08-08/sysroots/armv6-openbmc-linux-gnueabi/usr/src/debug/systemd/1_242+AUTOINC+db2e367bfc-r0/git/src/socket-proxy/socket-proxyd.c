...
static int connection_create_pipes(Connection* c, int buffer[static 2],
                                   size_t* sz)
{
    int r;
...
static int connection_shovel(Connection* c, int* from, int buffer[2], int* to,
                             ...
                             sd_event_source** to_source)
{
    ...
    do
    {
        ...
        {
            z = splice(*from, NULL, buffer[1], NULL, *sz - *full,
        ...
        if (*full > 0 && *to >= 0)
        {
            z = splice(buffer[0], NULL, *to, NULL, *full,
                       SPLICE_F_MOVE | SPLICE_F_NONBLOCK);
            if (z > 0)
            {
                *full -= z;
...
static int traffic_cb(sd_event_source* s, int fd, uint32_t revents,
                      void* userdata)
{
    ...
    r = connection_shovel(c, &c->server_fd, c->server_to_client_buffer,
                          &c->client_fd, &c->server_to_client_buffer_full,
                          &c->server_to_client_buffer_size,
                          &c->server_event_source, &c->client_event_source);
    if (r < 0)
        goto quit;

    r = connection_shovel(c, &c->client_fd, c->client_to_server_buffer,
                          &c->server_fd, &c->client_to_server_buffer_full,
...
static int connection_enable_event_sources(Connection* c)
{
    ...
    if (c->server_event_source)
        ...
        r = sd_event_add_io(c->context->event, &c->server_event_source,
                            c->server_fd, a, traffic_cb, c);
    else
        r = 0;
...
static int connection_complete(Connection* c)
{
    ...
    r = connection_create_pipes(c, c->server_to_client_buffer,
                                &c->server_to_client_buffer_size);
    if (r < 0)
        goto fail;

    r = connection_create_pipes(c, c->client_to_server_buffer,
                                &c->client_to_server_buffer_size);
    if (r < 0)
        goto fail;
...
static int connect_cb(sd_event_source* s, int fd, uint32_t revents,
                      void* userdata)
{
    Connection* c = userdata;
    socklen_t solen;
...
static int connection_start(Connection* c, struct sockaddr* sa, socklen_t salen)
{
    ...
    c->client_fd =
        socket(sa->sa_family, SOCK_STREAM | SOCK_NONBLOCK | SOCK_CLOEXEC, 0);
    if (c->client_fd < 0)
    {
    ...
    if (r < 0)
    {
        ...
        {
            r = sd_event_add_io(c->context->event, &c->client_event_source,
                                c->client_fd, EPOLLOUT, connect_cb, c);
            if (r < 0)
            {
...
static int resolve_handler(sd_resolve_query* q, int ret,
                           const struct addrinfo* ai, Connection* c)
{
    assert(q);
    assert(c);
...
static int resolve_remote(Connection* c)
{
    ...
    if (IN_SET(arg_remote_host[0], '/', '@'))
    {
        ...
        if (salen < 0)
        {
            log_error_errno(salen, "Specified address doesn't fit in an "
                                   "AF_UNIX address, refusing: %m");
            goto fail;
        }
    ...
    r = resolve_getaddrinfo(c->context->resolve, &c->resolve_query, node,
                            service, &hints, resolve_handler, NULL, c);
    if (r < 0)
    {
...
static int accept_cb(sd_event_source* s, int fd, uint32_t revents,
                     void* userdata)
{
...
static int add_listen_socket(Context* context, int fd)
{
    ...
    if (r < 0)
        return log_error_errno(
            r, "Failed to mark file descriptor non-blocking: %m");

    r = sd_event_add_io(context->event, &source, fd, EPOLLIN, accept_cb,
                        context);
    if (r < 0)
...
static int help(void)
{
    ...
    printf("%1$s [HOST:PORT]\n"
           "%1$s [SOCKET]\n\n"
           "Bidirectionally proxy local sockets to another (possibly remote) "
           "socket.\n\n"
           "  -c --connections-max=  Set the maximum number of connections to "
           "be accepted\n"
    ...
    while ((c = getopt_long(argc, argv, "c:h", options, NULL)) >= 0)
        ...
        switch (c)
        {
            ...
            case 'c':
                ...
                if (r < 0)
                {
                    log_error("Failed to parse --connections-max= argument: %s",
                              optarg);
                    return r;
                }
