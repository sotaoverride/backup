...
static void job_set_elapse_time(AvahiProbeScheduler* s, AvahiProbeJob* pj,
                                unsigned msec, unsigned jitter)
{
    ...
    if (pj->time_event)
        ...
        pj->time_event =
            avahi_time_event_new(s->time_event_queue, &tv, elapse_callback, pj);
}
...
static int packet_add_probe_query(AvahiProbeScheduler* s, AvahiDnsPacket* p,
                                  AvahiProbeJob* pj)
{
    size_t size;
    AvahiKey* k;
    ...
    size = avahi_key_get_estimate_size(pj->record->key) +
    ...
    if (!(k = avahi_key_new(pj->record->key->name, pj->record->key->clazz,
                            AVAHI_DNS_TYPE_ANY)))
        return 0; /* OOM */
    ...
    for (pj = s->jobs; pj; pj = pj->jobs_next)
    {
        ...
        if (k->clazz != pj->record->key->clazz ||
            !avahi_domain_equal(k->name, pj->record->key->name))
            continue;
        ...
        /* reserve size for record data */
        avahi_dns_packet_reserve_size(
            p, avahi_record_get_estimate_size(pj->record));
...
static void elapse_callback(AVAHI_GCC_UNUSED AvahiTimeEvent* e, void* data)
{
    ...
    if (!packet_add_probe_query(s, p, pj))
    {
        ...
        b = avahi_dns_packet_append_key(p, k, 0) &&
            avahi_dns_packet_append_record(p, pj->record, 0, 0);
        avahi_key_unref(k);
...
static AvahiProbeJob* find_scheduled_job(AvahiProbeScheduler* s,
                                         AvahiRecord* record)
{
    AvahiProbeJob* pj;
...
static AvahiProbeJob* find_history_job(AvahiProbeScheduler* s,
                                       AvahiRecord* record)
{
    AvahiProbeJob* pj;
...
int avahi_probe_scheduler_post(AvahiProbeScheduler* s, AvahiRecord* record,
                               int immediately)
{
    AvahiProbeJob* pj;
    struct timeval tv;
    ...
    if ((pj = find_scheduled_job(s, record)))
    {
        ...
        {
            /* If the new entry should be scheduled earlier, update the old
             * entry */
            pj->delivery = tv;
    ...
    else
    {
        ...
        pj->delivery = tv;
        pj->time_event = avahi_time_event_new(
            s->time_event_queue, &pj->delivery, elapse_callback, pj);
