...
static nss_status_t read_group(TFILE* fp, struct group* result, char* buffer,
                              ...
                              int* errnop)
{
    ...
    while (res == (int32_t)NSLCD_RESULT_BEGIN)
    {
        ...
        if (gid != skipgroup)
        {
            ...
            if ((*start) >= (*size))
            {
                /* for some reason Glibc expects us to grow the array
                   (completely different from all other NSS functions) */
                /* calculate new size */
                newsize = 2 * (*size);
                if ((limit > 0) && (*start >= limit))
                    newsize = limit;
...
{
    NSS_GETONE(NSLCD_ACTION_GROUP_BYNAME, WRITE_STRING(fp, name),
...
nss_status_t NSS_NAME(getgrgid_r)(gid_t gid, struct group* result, char* buffer,
                                  size_t buflen, int* errnop)
{
    NSS_GETONE(NSLCD_ACTION_GROUP_BYGID, WRITE_INT32(fp, gid),
               read_group(fp, result, buffer, buflen, errnop));
}
...
#define buflen *size
    NSS_GETONE(NSLCD_ACTION_GROUP_BYMEMBER, WRITE_STRING(fp, user),
...
static nss_status_t group_getgrgid(nss_backend_t UNUSED(*be), void* args)
{
    NSS_GETONE(NSLCD_ACTION_GROUP_BYGID,
               WRITE_INT32(fp, NSS_ARGS(args)->key.gid), read_result(fp, args));
}
...
static nss_status_t group_getgroupsbymember(nss_backend_t UNUSED(*be),
                                            void* args)
{
    ...
    NSS_GETONE(NSLCD_ACTION_GROUP_BYMEMBER, WRITE_STRING(fp, argp->username),
...
static nss_backend_op_t group_ops[] = {
    nss_ldap_destructor, group_endgrent, group_setgrent,         group_getgrent,
    group_getgrnam,      group_getgrgid, group_getgroupsbymember};
