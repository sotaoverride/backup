...
/// @brief this algorithm search through two collections for first mismatch
/// position that get returned as a pair of iterators, first pointing to the
/// mismatch position in first collection, second iterator in second one
///
...
/// @brief this algorithm search through two collections for first mismatch
/// position that get returned as a pair of iterators, first pointing to the
/// mismatch position in first collection, second iterator in second one. This
/// algorithms uses supplied predicate for collection elements comparison
///
/// @param first1 - first collection begin iterator
/// @param last1 - first collection end iterator
/// @param first2 - second collection begin iterator
...
/// @brief this algorithm search through first collection for first element that
/// does not belong a second one
///
/// @param first1 - first collection begin iterator
/// @param last1 - first collection end iterator
/// @param first2 - second collection begin iterator
...
/// @brief this algorithm search through first collection for first element that
/// does not satisfy binary predicate in conjunction will any element in second
/// collection
///
/// @param first1 - first collection begin iterator
/// @param last1 - first collection end iterator
...
/// @brief this algorithm search through first collection for last element that
/// belongs to a second one
///
/// @param first1 - first collection begin iterator
/// @param last1 - first collection end iterator
/// @param first2 - second collection begin iterator
                 ...
                 ForwardIterator2 first2, ForwardIterator2 last2)
{
    ...
    return it1 == first1 && std::find(first2, last2, *it1) == last2 ? last1
                                                                    : it1;
}
...
/// @brief this algorithm search through first collection for last element that
/// satisfy binary predicate in conjunction will at least one element in second
/// collection
///
/// @param first1 - first collection begin iterator
/// @param last1 - first collection end iterator
                 ...
                 Predicate pred)
{
    ...
    BidirectionalIterator1 it1 = last1;
    while (--it1 != first1 &&
           std::find_if(first2, last2, BOOST_TEST_BIND1ST(pred, *it1)) == last2)
    {
    }

    return it1 == first1 &&
                   std::find_if(first2, last2,
...
/// @brief this algorithm search through first collection for last element that
/// does not belong to a second one
///
/// @param first1 - first collection begin iterator
/// @param last1 - first collection end iterator
/// @param first2 - second collection begin iterator
/// @param last2 - second collection end iterator
template <class BidirectionalIterator1, class ForwardIterator2>
inline BidirectionalIterator1 find_last_not_of(BidirectionalIterator1 first1,
...
inline BidirectionalIterator1
    find_last_not_of(BidirectionalIterator1 first1,
                     ...
                     ForwardIterator2 last2, Predicate pred)
{
    if (first1 == last1 || first2 == last2)
        return last1;
...
/// @brief This algorithm replaces all occurrences of a set of substrings by
/// another substrings
///
...
inline StringClass
    replace_all_occurrences_of(StringClass str, ForwardIterator first1,
...
/// @brief This algorithm replaces all occurrences of a string with basic
/// wildcards with another (optionally containing wildcards as well).
///
/// @param str - string to transform
/// @param it_string_to_find - iterator to the beginning of the substrings to
/// replace
/// @param it_string_to_find_end - iterator to the end of the substrings to
/// replace
/// @param it_string_to_replace - iterator to the beginning of the substrings to
/// replace with
/// @param it_string_to_replace_end - iterator to the end of the substrings to
/// replace with
///
/// The wildcard is the symbol '*'. Only a unique wildcard per string is
/// supported. The replacement string may also contain a wildcard, in which case
/// it is considered as a placeholder to the content of the wildcard in the
/// source string. Example:
/// - In order to replace the occurrences of @c 'time=\"some-variable-value\"'
/// to a constant string,
///   one may use @c 'time=\"*\"' as the string to search for, and
///   'time=\"0.0\"' as the replacement string.
/// - In order to replace the occurrences of 'file.cpp(XX)' per 'file.cpp:XX',
/// where XX is a variable to keep,
///   on may use @c 'file.cpp(*)' as the string to search for, and 'file.cpp:*'
///   as the replacement string.
template <class StringClass, class ForwardIterator>
inline StringClass replace_all_occurrences_with_wildcards(
    StringClass str, ForwardIterator it_string_to_find,
    ForwardIterator it_string_to_find_end, ForwardIterator it_string_to_replace,
    ForwardIterator it_string_to_replace_end)
{
    for (; it_string_to_find != it_string_to_find_end &&
           it_string_to_replace != it_string_to_replace_end;
         ++it_string_to_find, ++it_string_to_replace)
    {
        ...
        if (wildcard_pos == StringClass::npos)
        {
            ...
            str = replace_all_occurrences_of(
                str, it_string_to_find, ++it_to_find_current_end,
        ...
        std::size_t found_begin =
            str.find(it_string_to_find->substr(0, wildcard_pos));
        while (found_begin != StringClass::npos)
        {
            std::size_t found_end =
                str.find(it_string_to_find->substr(wildcard_pos + 1),
                         found_begin + wildcard_pos + 1); // to simplify
            if (found_end != StringClass::npos)
            {
                ...
                if (wildcard_pos_replace == StringClass::npos)
                {
                    ...
                    str.replace(
                        found_begin,
                        found_end +
                            (it_string_to_find->size() - wildcard_pos - 1) -
                            found_begin,
                        replace_content);
                }
                else
            ...
            found_begin = str.find(it_string_to_find->substr(0, wildcard_pos),
                                   found_begin + 1);
        }
    }
