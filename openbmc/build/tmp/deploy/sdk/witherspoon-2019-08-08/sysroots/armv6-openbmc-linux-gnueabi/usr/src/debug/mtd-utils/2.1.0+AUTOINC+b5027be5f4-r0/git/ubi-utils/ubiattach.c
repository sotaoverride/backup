...
static const char doc[] =
    PROGRAM_NAME " version " VERSION " - a tool to attach MTD device to UBI.";

static const char optionsstr[] =
    "-d, --devn=<number>   the number to assign to the newly created UBI "
    "device\n"
    "                      (assigned automatically if this is not specified)\n"
    "-p, --dev-path=<path> path to MTD device node to attach\n"
    "-m, --mtdn=<number>   MTD device number to attach (alternative method, "
    "e.g\n"
    "                      if the character device node does not exist)\n"
    "-O, --vid-hdr-offset  VID header offset (do not specify this unless you "
    "really\n"
    "                      know what you are doing, the default should be "
    "optimal)\n"
    "-b, --max-beb-per1024 maximum expected bad block number per 1024 "
    "eraseblock.\n"
    "                      The default value is correct for most NAND "
    "devices.\n"
    "                      Allowed range is 0-768, 0 means the default kernel "
    "value.\n"
...
static const char usage[] =
    ...
    "\t[-m <MTD device number>] [-d <UBI device number>] [-p <path to "
    "device>]\n"
    ...
    " -m 0 -d 3 - attach MTD device 0 (mtd0) to UBI\n"
    "           and create UBI device number 3 (ubi3)\n"
    "Example 4: " PROGRAM_NAME
    " -m 1 -b 25 - attach /dev/mtd1 to UBI and reserve\n"
    "           25*C/1024 eraseblocks for bad block handling, where C is the "
    "flash\n"
    "           is total flash chip eraseblocks count, that is flash chip size "
    "in\n"
    "           eraseblocks (including bad eraseblocks). E.g., if the flash "
    "chip\n"
...
static int parse_opt(int argc, char* const argv[])
{
        ...
        switch (key)
        {
            ...
            case 'b':
                ...
                if (error || args.max_beb_per1024 < 0 ||
                    ...
                    return errmsg(
                        "bad maximum of expected bad blocks (0-768): \"%s\"",
                        optarg);

                if (args.max_beb_per1024 == 0)
                    warnmsg("the default kernel value will be used for maximum "
                            "expected bad blocks");
    ...
    if (optind == argc)
        ...
        return errmsg(
            "more then one UBI control device specified (use -h for help)");
    else
        args.node = argv[optind];

    if (args.mtdn == -1 && args.dev == NULL)
        return errmsg(
            "MTD device to attach was not specified (use -h for help)");
...
int main(int argc, char* const argv[])
{
    ...
    else if (err == 1)
    {
        ...
        warnmsg("the --max-beb-per1024=%d parameter was ignored",
                args.max_beb_per1024);
        normsg("the UBI kernel driver does not support does not allow changing "
               "the reserved PEBs count");
        normsg("the support was added in kernel version 3.7, probably you are "
    ...
    printf("UBI device number %d, total %d LEBs (", dev_info.dev_num,
           dev_info.total_lebs);
