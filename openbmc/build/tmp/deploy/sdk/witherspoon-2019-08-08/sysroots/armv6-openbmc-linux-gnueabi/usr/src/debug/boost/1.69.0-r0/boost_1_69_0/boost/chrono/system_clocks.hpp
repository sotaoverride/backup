...
TODO:
  ...
  * Consider issues raised by Michael Marcin:
    ...
    > especially with SpeedStep processors on laptops. This was many years ago
and > might have been fixed by service packs and drivers.
    >
    > Typically you check the results of QPC against GetTickCount to see if the
    > results are reasonable.
    > http://support.microsoft.com/kb/274323
    >
    > I've also heard of problems with QueryPerformanceCounter in
multi-processor > systems.
    ...
    > physical processor if it isn't already on cpu0 but they claim it worked
well > in practice because they called the timing functions infrequently.
    >
...
typedef steady_clock
    high_resolution_clock; // as permitted by [time.clock.hires]
#else
...
// You'll need to know how to get the current time and implement that under
// now(). You'll need to know what units (tick period) and representation makes
// the most sense for your clock and set those accordingly. If you know how to
// map this clock to time_t (perhaps your clock is std::time, which makes that
// trivial), then you can fill out system_clock's to_time_t() and from_time_t().
...
class BOOST_CHRONO_DECL system_clock
{
  public:
    ...
    static BOOST_CHRONO_INLINE std::time_t
...
//  As permitted, steady_clock or system_clock is a typedef for
//  high_resolution_clock. See synopsis.
...
struct clock_string<system_clock, CharT>
{
    ...
    {
        static const CharT u[] = {'s', 'y', 's', 't', 'e', 'm',
                                  '_', 'c', 'l', 'o', 'c', 'k'};
        static const std::basic_string<CharT> str(u,
                                                  u + sizeof(u) / sizeof(u[0]));
        return str;
    }
...
struct clock_string<steady_clock, CharT>
{
    static std::basic_string<CharT> name()
    {
        static const CharT u[] = {'s', 't', 'e', 'a', 'd', 'y',
                                  '_', 'c', 'l', 'o', 'c', 'k'};
        static const std::basic_string<CharT> str(u,
                                                  u + sizeof(u) / sizeof(u[0]));
        return str;
