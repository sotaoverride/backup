...
static void help(void)
{
    fprintf(stderr,
            ...
            "    1) read/write-flag 2) LENGTH (range 0-65535) 3) I2C address "
            "(use last one if omitted)\n"
...
static void print_msgs(struct i2c_msg* msgs, __u32 nmsgs, unsigned flags)
{
    ...
    for (i = 0; i < nmsgs; i++)
    {
        ...
        if (flags & PRINT_HEADER)
            fprintf(output, "msg %u: addr 0x%02x, %s, len %u", i, msgs[i].addr,
                    read ? "read" : "write", msgs[i].len);
...
int main(int argc, char* argv[])
{
        ...
        switch (state)
        {
            case PARSE_GET_DESC:
                ...
                if (*arg_ptr)
                {
                    ...
                    {
                        fprintf(stderr,
                                "Error: Unknown separator after length\n");
                        goto err_out_with_arg;
                    }
            ...
            case PARSE_GET_DATA:
                ...
                while (buf_idx < len)
                {
                    ...
                    switch (*end)
                    {
                        ...
                        default:
                            fprintf(stderr,
                                    "Error: Invalid data byte suffix\n");
                            goto err_out_with_arg;
                    }
                }
            ...
            default:
                /* Should never happen */
                fprintf(stderr,
                        "Internal Error: Unknown state in state machine!\n");
                goto err_out;
        }
    ...
    if (yes || confirm(filename, msgs, nmsgs))
    {
        ...
        print_msgs(msgs, nmsgs_sent,
                   PRINT_READ_BUF |
                       (verbose ? PRINT_HEADER | PRINT_WRITE_BUF : 0));
    }
