...
static OM_uint32 val_imp_sec_ctx_args(OM_uint32* minor_status,
...
OM_uint32 KRB5_CALLCONV gss_import_sec_context(minor_status, interprocess_token,
                                               context_handle)
...

{
    ...
    status =
        val_imp_sec_ctx_args(minor_status, interprocess_token, context_handle);
    ...
    status = gssint_select_mech_type(minor_status, &token_mech, &selected_mech);
    if (status != GSS_S_COMPLETE)
        goto error_out;
    ...
    if (generic_gss_copy_oid(minor_status, selected_mech, &ctx->mech_type) !=
        GSS_S_COMPLETE)
    {
        status = GSS_S_FAILURE;
        goto error_out;
    ...
    if (mech->gssspi_import_sec_context_by_mech)
    {
        ...
        status = mech->gssspi_import_sec_context_by_mech(
            minor_status, public_mech, &token, &mctx);
    }
    else
    {
