...
#define static_assert(expr, message) extern int(*xcrypt_static_assert_fn(      \
    void))[!!sizeof(struct {int xcrypt_error_if_negative : (expr) ? 2 : -1;})]
#endif
...
#define XCRYPT_SECURE_MEMSET(s, len) memset_s(s, len, 0x00, len)
#elif defined HAVE_EXPLICIT_BZERO
/* explicit_bzero() should give us enough guarantees.  */
#define XCRYPT_SECURE_MEMSET(s, len) explicit_bzero(s, len)
#elif defined HAVE_EXPLICIT_MEMSET
/* Same guarantee goes for explicit_memset().  */
#define XCRYPT_SECURE_MEMSET(s, len) explicit_memset(s, 0x00, len)
#else
/* The best hope we have in this case.  */
#define INCLUDE_XCRYPT_SECURE_MEMSET 1
extern void _crypt_secure_memset(void*, size_t);
#define XCRYPT_SECURE_MEMSET(s, len) _crypt_secure_memset(s, len)
#endif
...
extern size_t _crypt_strcpy_or_abort(void*, const size_t, const void*);
...
extern void gensalt_yescrypt_rn(unsigned long, const uint8_t*, size_t, uint8_t*,
                                size_t);
#endif
#endif
...
extern void gensalt_descrypt_rn(unsigned long, const uint8_t*, size_t, uint8_t*,
                                size_t);
#endif
...
extern void crypt_yescrypt_rn(const char*, size_t, const char*, size_t,
...
extern void gensalt_sha_rn(char tag, size_t maxsalt, unsigned long defcount,
                           ...
                           unsigned long count, const uint8_t* rbytes,
