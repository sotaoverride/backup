        ...
        dlist_for_each_data(linklist, curlink, char)
        {
            if (bus->devices)
            {
                dev = (struct sysfs_device*)dlist_find_custom(
                    bus->devices, (void*)curlink, name_equal);
                if (dev)
                    continue;
            }
        ...
        dlist_for_each_data(dirlist, curdir, char)
        {
            if (bus->drivers)
            {
                drv = (struct sysfs_driver*)dlist_find_custom(
                    bus->drivers, (void*)curdir, name_equal);
                if (drv)
                    continue;
            }
 ...
 */
struct sysfs_device* sysfs_get_bus_device(struct sysfs_bus* bus, const char* id)
{
    ...
    if (bus->devices)
    {
        dev = (struct sysfs_device*)dlist_find_custom(bus->devices, (void*)id,
                                                      name_equal);
        if (dev)
            return dev;
    }
    ...
    if (!sysfs_get_link(devpath, target, SYSFS_PATH_MAX))
    {
        ...
        if (!bus->devices)
            bus->devices = dlist_new_with_delete(sizeof(struct sysfs_device),
                                                 sysfs_close_dev);
        dlist_unshift_sorted(bus->devices, dev, sort_list);
    }
    return dev;
                                          ...
                                          const char* drvname)
{
    ...
    if (!bus->drivers)
        bus->drivers =
            dlist_new_with_delete(sizeof(struct sysfs_driver), sysfs_close_drv);
    dlist_unshift_sorted(bus->drivers, drv, sort_list);
    return drv;
}
