...
#define G_MEMORY_SETTINGS_BACKEND(inst)                                        \
    (G_TYPE_CHECK_INSTANCE_CAST((inst), G_TYPE_MEMORY_SETTINGS_BACKEND,        \
...
G_DEFINE_TYPE_WITH_CODE(
    GMemorySettingsBackend, g_memory_settings_backend, G_TYPE_SETTINGS_BACKEND,
    g_io_extension_point_implement(G_SETTINGS_BACKEND_EXTENSION_POINT_NAME,
                                   g_define_type_id, "memory", 10))

static GVariant*
    g_memory_settings_backend_read(GSettingsBackend* backend, const gchar* key,
...
static gboolean g_memory_settings_backend_write(GSettingsBackend* backend,
                                                const gchar* key,
                                                GVariant* value,
                                                gpointer origin_tag)
{
...
static gboolean g_memory_settings_backend_write_one(gpointer key,
                                                    gpointer value,
                                                    gpointer data)
{
...
static gboolean g_memory_settings_backend_write_tree(GSettingsBackend* backend,
                                                     GTree* tree,
                                                     gpointer origin_tag)
{
...
static void g_memory_settings_backend_reset(GSettingsBackend* backend,
                                            const gchar* key,
                                            gpointer origin_tag)
{
    GMemorySettingsBackend* memory = G_MEMORY_SETTINGS_BACKEND(backend);
...
static void g_memory_settings_backend_finalize(GObject* object)
{
...
static void g_memory_settings_backend_init(GMemorySettingsBackend* memory)
{
