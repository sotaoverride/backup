...
static const pw_response_header disabled = {.version = NSCD_VERSION,
                                            .found = -1,
                                            .pw_name_len = 0,
                                            .pw_passwd_len = 0,
                                            .pw_uid = -1,
...
const struct iovec pwd_iov_disabled = {.iov_base = (void*)&disabled,
                                       .iov_len = sizeof(disabled)};

/* This is the standard reply in case we haven't found the dataset.  */
static const pw_response_header notfound = {.version = NSCD_VERSION,
                                            .found = 0,
                                            .pw_name_len = 0,
                                            .pw_passwd_len = 0,
                                            .pw_uid = -1,
                          ...
                          int errval)
{
    ...
    if (pwd == NULL)
    {
        ...
        else
        {
            ...
            if (fd != -1 && TEMP_FAILURE_RETRY(send(fd, &notfound, total,
                                                    MSG_NOSIGNAL)) != total)
                all_written = false;
    ...
    else
    {
        ...
        int key_offset;
        n = snprintf(buf, buf_len, "%d%c%n%s", pwd->pw_uid, '\0', &key_offset,
                     (char*)key) +
            1;

        total = (offsetof(struct dataset, strdata) + pw_name_len +
                 pw_passwd_len + pw_gecos_len + pw_dir_len + pw_shell_len);
       ...
       new record we can still allocate real memory.  */
        ...
        timeout = datahead_init_pos(
            &dataset->head, total + n, total - offsetof(struct dataset, resp),
       ...
       stored on the stack.  */
        if (!alloca_used)
        {
            ...
            if (db->persistent)
            {
                ...
                uintptr_t pval = (uintptr_t)dataset & ~pagesize_m1;
                msync((void*)pval,
                      ((uintptr_t)dataset & pagesize_m1) + total + n, MS_ASYNC);
            }
...
static int lookup(int type, union keytype key, struct passwd* resultbufp,
                  char* buffer, size_t buflen, struct passwd** pwd)
{
    if (type == GETPWBYNAME)
                       ...
                       struct hashentry* he, struct datahead* dh)
{
    ...
    while (lookup(req->type, key, &resultbuf, tmpbuf.data, tmpbuf.length,
                  &pwd) != 0 &&
           (errval = errno) == ERANGE)
...
time_t readdpwbyname(struct database_dyn* db, struct hashentry* he,
                     struct datahead* dh)
{
    request_header req = {.type = GETPWBYNAME, .key_len = he->len};
    union keytype u = {.v = db->data + he->key};

    return addpwbyX(db, -1, &req, u, db->data + he->key, he->owner, he, dh);
}

void addpwbyuid(struct database_dyn* db, int fd, request_header* req, void* key,
                uid_t c_uid)
{
    char* ep;
                    ...
                    struct datahead* dh)
{
    ...
    request_header req = {.type = GETPWBYUID, .key_len = he->len};
    union keytype u = {.u = uid};
