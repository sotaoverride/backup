...
static GSList* get_list_node(GMarkupParseContext* context, gpointer data)
{
    ...
    if (context->spare_list_nodes != NULL)
    {
        ...
        context->spare_list_nodes =
            g_slist_remove_link(context->spare_list_nodes, node);
    }
    else
        node = g_slist_alloc();
    node->data = data;
...
GMarkupParseContext*
    g_markup_parse_context_new(const GMarkupParser* parser,
                               GMarkupParseFlags flags, gpointer user_data,
...
static void set_error(GMarkupParseContext* context, GError** error,
                      GMarkupError code, const gchar* format, ...)
    G_GNUC_PRINTF(4, 5);

static void set_error_literal(GMarkupParseContext* context, GError** error,
                              GMarkupError code, const gchar* message)
{
    GError* tmp_error;

    tmp_error = g_error_new_literal(G_MARKUP_ERROR, code, message);

    g_prefix_error(&tmp_error, _("Error on line %d char %d: "),
                   context->line_number, context->char_number);
...
static void propagate_error(GMarkupParseContext* context, GError** dest,
                            GError* src)
{
    if (context->flags & G_MARKUP_PREFIX_ERROR_POSITION)
        g_prefix_error(&src, _("Error on line %d char %d: "),
                       context->line_number, context->char_number);
...
static gboolean slow_name_validate(GMarkupParseContext* context,
                                   const gchar* name, GError** error)
{
    ...
    if (!g_utf8_validate(name, -1, NULL))
    {
        set_error(context, error, G_MARKUP_ERROR_BAD_UTF8,
                  _("Invalid UTF-8 encoded text in name — not valid “%s”"),
                  name);
        return FALSE;
    }
    ...
    if (!(g_ascii_isalpha(*p) ||
          (!IS_COMMON_NAME_END_CHAR(*p) &&
           (*p == '_' || *p == ':' || g_unichar_isalpha(g_utf8_get_char(p))))))
    {
 ...
 */
static gboolean name_validate(GMarkupParseContext* context, const gchar* name,
                              GError** error)
{
    char mask;
    const char* p;
...
static gboolean text_validate(GMarkupParseContext* context, const gchar* p,
...
static gchar* utf8_str(const gchar* utf8, gsize max_len, gchar* buf)
{
...
G_GNUC_PRINTF(5, 6)
static void set_unescape_error(GMarkupParseContext* context, GError** error,
                               const gchar* remaining_text, GMarkupError code,
                               const gchar* format, ...)
{
    GError* tmp_error;
    gchar* s;
    ...
    tmp_error = g_error_new(G_MARKUP_ERROR, code, _("Error on line %d: %s"),
                            context->line_number - remaining_newlines, s);
 ...
 */
static gboolean unescape_gstring_inplace(GMarkupParseContext* context,
                                         GString* string, gboolean* is_ascii,
                                         GError** error)
{
    char mask, *to;
    const char* from;
        ...
        *to = *from;
        ...
        if (*from == '&')
        {
            ...
            if (*from == '#')
            {
                ...
                if (end == from || errno != 0)
                {
                    set_unescape_error(context, error, from,
                ...
                {
                    set_unescape_error(
                        context, error, from, G_MARKUP_ERROR_PARSE,
...
static void add_to_partial(GMarkupParseContext* context,
                           const gchar* text_start, const gchar* text_end)
{
    ...
    { /* allocate a new chunk to parse into */
        ...
        if (context->spare_chunks != NULL)
        {
            ...
            context->spare_chunks =
                g_slist_remove_link(context->spare_chunks, node);
            context->partial_chunk = node->data;
            free_list_node(context, node);
        }
        else
            context->partial_chunk =
                g_string_sized_new(MAX(28, text_end - text_start));
    }

    if (text_start != text_end)
        g_string_insert_len(context->partial_chunk, -1, text_start,
                            text_end - text_start);
}
...
static void push_partial_as_tag(GMarkupParseContext* context)
{
    ...
    context->tag_stack =
        g_slist_concat(get_list_node(context, str->str), context->tag_stack);
    context->tag_stack_gstr =
        g_slist_concat(get_list_node(context, str), context->tag_stack_gstr);
    context->partial_chunk = NULL;
}
...
static void pop_tag(GMarkupParseContext* context)
{
    ...
    context->tag_stack_gstr =
        g_slist_remove_link(context->tag_stack_gstr, nodeb);
...
static void add_attribute(GMarkupParseContext* context, GString* str)
{
    if (context->cur_attr + 2 >= context->alloc_attrs)
    {
        ...
        context->attr_names = g_realloc(
            context->attr_names, sizeof(GString*) * context->alloc_attrs);
        context->attr_values = g_realloc(
            context->attr_values, sizeof(GString*) * context->alloc_attrs);
    }
    context->cur_attr++;
 ...
 */
static inline void emit_start_element(GMarkupParseContext* context,
                                      GError** error)
{
    int i, j = 0;
     ...
     */
    if ((context->flags & G_MARKUP_IGNORE_QUALIFIED) &&
    ...
    if (context->parser->start_element)
        (*context->parser->start_element)(
            context, start_name, (const gchar**)attr_names,
            (const gchar**)attr_values, context->user_data, &tmp_error);
    clear_attributes(context);
...
static void emit_end_element(GMarkupParseContext* context, GError** error)
{
    ...
    if (context->parser->end_element)
        (*context->parser->end_element)(context, current_element(context),
                                        context->user_data, &tmp_error);
        ...
        switch (context->state)
        {
            ...
            case STATE_AFTER_ATTRIBUTE_NAME:
                ...
                if (context->iter != context->current_text_end)
                {
                     ...
                     * if any; push it on the stack; enter next state.
                     */
                    if (!name_validate(context, context->partial_chunk->str,
                                       error))
                        break;
            ...
            case STATE_INSIDE_ATTRIBUTE_VALUE_DQ:
                ...
                else
                {
                    ...
                    if (unescape_gstring_inplace(context,
                                                 context->partial_chunk,
                                                 &is_ascii, error) &&
                        (is_ascii ||
                         text_validate(context, context->partial_chunk->str,
                ...
                if (context->iter != context->current_text_end)
                {
                                       ...
                                       context->partial_chunk->len, error)))
                    {
                        ...
                        if (context->parser->text)
                            (*context->parser->text)(
                                context, context->partial_chunk->str,
                                context->partial_chunk->len, context->user_data,
                                &tmp_error);
            ...
            case STATE_AFTER_CLOSE_TAG_NAME:
                ...
                if (context->iter != context->current_text_end)
                {
                    ...
                    else if (strcmp(close_name->str,
                                    current_element(context)) != 0)
                    {
                        set_error(
                            context, error, G_MARKUP_ERROR_PARSE,
                            _("Element “%s” was closed, but the currently "
                              "open element is “%s”"),
                            close_name->str, current_element(context));
                    }
                    else
                    {
                        advance_char(context);
            ...
            case STATE_INSIDE_PASSTHROUGH:
                ...
                else
                {
                    ...
                    if (context->flags & G_MARKUP_TREAT_CDATA_AS_TEXT &&
                        strncmp(context->partial_chunk->str, "<![CDATA[", 9) ==
                            0)
                    {
                        if (context->parser->text &&
                            text_validate(
                                context, context->partial_chunk->str + 9,
                                context->partial_chunk->len - 12, error))
                            (*context->parser->text)(
                                context, context->partial_chunk->str + 9,
                                          ...
                                          GError** error)
{
    ...
    switch (context->state)
    {
        ...
        case STATE_AFTER_OPEN_ANGLE:
            set_error_literal(context, error, G_MARKUP_ERROR_PARSE,
                              _("Document ended unexpectedly just after an "
                                "open angle bracket “<”"));
            break;
                                 ...
                                 gpointer user_data)
{
    ...
    context->subparser_stack =
        g_slist_prepend(context->subparser_stack, tracker);
}
            ...
            default:
                ...
                else if (c == 0xc2)
                {
                    ...
                    if ((0x7f < u && u <= 0x84) || (0x86 <= u && u <= 0x9f))
                    {
                         ...
                         * Increment to skip 0xc2 and point to the right
                         * location.
                         */
                        p++;
                    }
...
static const char* find_conversion(const char* format, const char** after)
{
    ...
    while (*cp == 'h' || *cp == 'L' || *cp == 'l' || *cp == 'j' || *cp == 'z' ||
           *cp == 'Z' || *cp == 't')
        cp++;
    ...
    while (type != G_MARKUP_COLLECT_INVALID)
    {
        ...
        if (value == NULL && mandatory)
        {
            g_set_error(error, G_MARKUP_ERROR, G_MARKUP_ERROR_MISSING_ATTRIBUTE,
                        "element '%s' requires attribute '%s'", element_name,
                        attr);
