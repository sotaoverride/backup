...
static int setreuseaddr(int sock, int value)
{
    return setsockopt(sock, SOL_SOCKET, SO_REUSEADDR, &value, sizeof(value));
}

#if defined(IPV6_V6ONLY)
static int setv6only(int sock, int value)
{
...
static const char* paddr(struct sockaddr* sa)
{
    static char buf[100];
    char portbuf[10];
    if (getnameinfo(sa, sa_socklen(sa), buf, sizeof(buf), portbuf,
    ...
    ((set.max + incr < set.max ||                                              \
      ((set.max + incr) * sizeof(set.data[0]) / sizeof(set.data[0]) !=         \
       set.max + incr))                                                        \
         ? 0 /* overflow */                                                    \
         : ((tmpptr =                                                          \
                 realloc(set.data, (set.max + incr) * sizeof(set.data[0])))    \
...
#define DEL(set, idx) (set.data[idx] = set.data[--set.n], 0)
...
static void do_break(verto_ctx* ctx, verto_ev* ev)
{
...
static void do_reset(verto_ctx* ctx, verto_ev* ev)
{
...
static void free_sighup_context(verto_ctx* ctx, verto_ev* ev)
{
 ...
 */
static krb5_error_code loop_add_address(const char* address, int port,
                                        enum bind_type type,
                                        struct rpc_svc_data* rpc_data)
{
    ...
    FOREACH_ELT(bind_addresses, i, val)
    {
        ...
        {
            krb5_klog_syslog(LOG_DEBUG, _("Address already added to server"));
            return 0;
        }
    }
 ...
 *      connections are created.  Ignored for other types.
 */
static krb5_error_code loop_add_addresses(const char* addresses,
                                          int default_port, enum bind_type type,
                                          struct rpc_svc_data* rpc_data)
{
...
static void remove_event_from_set(verto_ev* ev)
{
    verto_ev* tmp;
    int i;
...
static void free_socket(verto_ctx* ctx, verto_ev* ev)
{
...
static verto_ev* make_event(verto_ctx* ctx, verto_ev_flag flags,
                            verto_callback callback, int sock,
...
static krb5_error_code add_fd(int sock, enum conn_type conntype,
                              verto_ev_flag flags, void* handle,
                              const char* prog, verto_ctx* ctx,
                              verto_callback callback, verto_ev** ev_out)
{
    ...
    *ev_out = NULL;
    ...
    if (newconn == NULL)
    {
        com_err(prog, ENOMEM, _("cannot allocate storage for connection info"));
        return ENOMEM;
    }
 ...
 */
static krb5_error_code create_server_socket(struct sockaddr* addr, int type,
                                            const char* prog, int* fd_out)
{
    int sock, e;
    ...
    *fd_out = -1;
    ...
    {
        close(sock);
        com_err(prog, 0, _("TCP socket fd number %d (for %s) too high"), sock,
                paddr(addr));
        return EMFILE;
    }
#endif
 ...
 */
static krb5_error_code setup_socket(struct bind_address* ba,
                                    struct sockaddr* sock_address, void* handle,
                                    const char* prog, verto_ctx* ctx,
                                    int tcp_listen_backlog, verto_callback vcb,
 ...
 */
static krb5_error_code setup_addresses(verto_ctx* ctx, void* handle,
                                       const char* prog, int tcp_listen_backlog)
{
    ...
    for (i = 0; i < bind_addresses.n; i++)
    {
        ...
        {
            krb5_klog_syslog(
                LOG_ERR, _("Failed getting address info (for %s): %s"),
                (addr.address == NULL) ? "<wildcard>" : addr.address,
                gai_strerror(err));
            ret = EIO;
            goto cleanup;
        }
        ...
        for (ai = ai_list; ai != NULL; ai = ai->ai_next)
        {
            ...
            {
                krb5_klog_syslog(
                    LOG_ERR, _("Failed setting up a %s socket (for %s)"),
                    bind_type_names[addr.type], paddr(ai->ai_addr));
                if (ret != EAFNOSUPPORT)
                    goto cleanup;
            }
            else
        ...
        case AF_INET6:
            if (IN6_IS_ADDR_V4MAPPED(&sa2sin6(sa)->sin6_addr))
            {
                ...
                faddr->address->contents =
                    12 + (krb5_octet*)&sa2sin6(sa)->sin6_addr;
            }
            else
            {
...
static void process_packet_response(void* arg, krb5_error_code code,
                                    krb5_data* response)
{
    ...
    cc = send_to_from(
        state->port_fd, response->data, (socklen_t)response->length, 0,
        (struct sockaddr*)&state->saddr, state->saddr_len,
        (struct sockaddr*)&state->daddr, state->daddr_len, &state->auxaddr);
    if (cc == -1)
    {
        ...
        if (getnameinfo((struct sockaddr*)&state->daddr, state->daddr_len,
                        daddrbuf, sizeof(daddrbuf), 0, 0, NI_NUMERICHOST) != 0)
        {
...
static int kill_lru_tcp_or_rpc_connection(void* handle, verto_ev* newev)
{
    ...
    FOREACH_ELT(events, i, ev)
    {
        ...
        if (c->type != CONN_TCP && c->type != CONN_RPC)
            continue;
        if (oldest_c == NULL || oldest_c->start_time > c->start_time)
        {
            oldest_ev = ev;
            oldest_c = c;
        }
...
static void accept_tcp_connection(verto_ctx* ctx, verto_ev* ev)
{
    ...
    if (getnameinfo((struct sockaddr*)&addr_s, addrlen, newconn->addrbuf,
                    sizeof(newconn->addrbuf), tmpbuf, sizeof(tmpbuf),
...
static void process_tcp_response(void* arg, krb5_error_code code,
                                 krb5_data* response)
{
...
static struct tcp_dispatch_state* prepare_for_dispatch(verto_ctx* ctx,
                                                       verto_ev* ev)
{
...
static void process_tcp_connection_read(verto_ctx* ctx, verto_ev* ev)
{
    ...
    if (conn->offset < 4)
    {
        ...
        len = 4 - conn->offset;
        nread = SOCKET_READ(verto_get_fd(ev), conn->buffer + conn->offset, len);
        if (nread < 0) /* error */
            goto kill_tcp_connection;
        if (nread == 0) /* eof */
            goto kill_tcp_connection;
            ...
            if (conn->msglen > conn->bufsiz - 4)
            {
                ...
                krb5_klog_syslog(LOG_ERR,
                                 _("TCP client %s wants %lu bytes, "
                                   "cap is %lu"),
                                 conn->addrbuf, (unsigned long)conn->msglen,
...
static void process_tcp_connection_write(verto_ctx* ctx, verto_ev* ev)
{
    ...
    nwrote = SOCKET_WRITEV(sock, conn->sgp, conn->sgnum, tmp);
    if (nwrote > 0)
...
static int have_event_for_fd(int fd)
{
    verto_ev* ev;
    int i;
...
static void accept_rpc_connection(verto_ctx* ctx, verto_ev* ev)
{
    ...
    for (s = 0; s < FD_SETSIZE; s++)
    {
        ...
        if (getpeername(s, addr, &addrlen) ||
            getnameinfo(addr, addrlen, newconn->addrbuf,
                        sizeof(newconn->addrbuf), tmpbuf, sizeof(tmpbuf),
                        NI_NUMERICHOST | NI_NUMERICSERV))
        {
            strlcpy(newconn->addrbuf, "???", sizeof(newconn->addrbuf));
