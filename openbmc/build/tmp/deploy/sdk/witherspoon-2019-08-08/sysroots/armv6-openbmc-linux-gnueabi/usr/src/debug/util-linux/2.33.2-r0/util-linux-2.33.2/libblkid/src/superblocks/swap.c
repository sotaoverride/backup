...
static int swap_set_info(blkid_probe pr, const char* version)
{
    ...
    hdr = (struct swap_header_v1_2*)blkid_probe_get_buffer(
        pr, 1024, sizeof(struct swap_header_v1_2));
    if (!hdr)
    ...
    {
        if (hdr->volume[0] &&
            blkid_probe_set_label(pr, hdr->volume, sizeof(hdr->volume)) < 0)
            return 1;
...
const struct blkid_idinfo swsuspend_idinfo = {
    ...
    .minsz = 10 * 4096, /* 10 pages */
    .magics = {{TOI_MAGIC_STRING, TOI_MAGIC_STRLEN, 0, 0},
               {"S1SUSPEND", 9, 0, 0xff6},
               {"S2SUSPEND", 9, 0, 0xff6},
               {"ULSUSPEND", 9, 0, 0xff6},
