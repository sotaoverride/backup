 ...
 *  and another to a regular file (if you want the data) or /dev/null if you
 *don't.
 *
 ...
 *  Added README file.
 *  In JitterTest.c: Changed operation of periodic timer to one shot. The timer
 *is now reset when the task wakes. This way every "jitter" is from the last
 *time and jitters from previous times are not accumulated and screw aroud with
 *our display.
 *
 *  All jitter is now +ve. (as it should be). Also added a "read file"
 *functionality to test for jitter in task if we have to read from JFFS fs. The
 *program now also prints data to console- where it can be logged, interspersed
 *with other "interesting" printk's from the kernel and drivers (flash sector
 *erases etc.)
 *
...
static int WriteBytes =
    DEFAULT_WRITE_BYTES;             /* pad out file to these many bytes. */
...
static int GrabKProfile = FALSE; /* To help determine system bottle necks
                                    ...
                                    /dev/console also.
                                 */
static long ProfileTriggerMSecs =
    15000l; /* Jitter time in seconds that triggers
         a snapshot of the profile to be taken

      */
static int SignalGCTask =
    FALSE; /* should be signal SIGSTOP/SIGCONT to gc task?*/
...
int main(int argc, char* argv[])
{
    ...
    if (RunAsRTTask)
    {
        ...
        if (sched_setscheduler(0, PRIORITY_POLICY, &schedParam) != (int)0)
        {
        ...
        {
            printf("Not running with request priority %i. running with "
                   "priority %i instead!\n",
    ...
    */
    if (DoRead)
    {
        ...
        if ((Fd2 = open(ReadFile, O_RDWR | O_CREAT | O_SYNC | O_TRUNC,
                        S_IRWXU)) <= 0)
        {
    ...
    */
    ...
    if (gettimeofday(&PrevTimeVal, NULL) != (int)0)
    {
        ...
        sprintf(tmpBuf, "Unable to initialize previous time of day.\n");
...
static void doGrabKProfile(int jitterusec, char* fileName)
{
    ...
    {
        fprintf(stderr, "Could not open file /proc/profile. Make sure you "
 ...
 *  output: N/A
 ***********************************************************************/
void AlarmHandler(int sigNum) /* signal number (not used)         */
{
    ...
    if (gettimeofday(&CurrTimeVal, NULL) == (int)0)
    {
        ...
        timeDiffusec = (long)(((((long long)CurrTimeVal.tv_sec) * 1000000L) +
                               CurrTimeVal.tv_usec) -
                              (((long long)PrevTimeVal.tv_sec * 1000000L) +
                               PrevTimeVal.tv_usec));
        ...
        sprintf(&tmpBuf[strlen(tmpBuf)], " %f ms",
                (float)timeDiffusec / 1000.0);
        ...
        if (labs(timeDiffusec) > LastMaxDiff)
        {
            ...
            if ((GrabKProfile == TRUE) &&
                (ProfileTriggerMSecs < (labs(timeDiffusec) / 1000)))
            {
                ...
                /* Say so on the console so that a marker gets put in the
                 * console log */
        ...
        /* The "-1" below takes out the '\n' at the end that we appended for the
         msg printed on the console.*/
        ...
        if (WriteBytes > MIN_WRITE_BYTES)
        {
            ...
            for (cntr = strlen(tmpBuf); cntr < WriteBytes - 1;
                 cntr++) /* "-1" adj for '\n' at end */
            {
                /* don't accept any # < 'SPACE' */
                padChar =
                    (char)(MIN_ASCII + (int)((MAX_ASCII - (float)MIN_ASCII) *
                                             rand() / (RAND_MAX + 1.0)));
 ...
 *  output: stack size
 ***********************************************************************/
void HandleCmdLineArgs(int argc,     /* number of command-line arguments */
                       char* argv[]) /* ptrs to command-line arguments   */
{
    ...
    if (argc > (int)1)
    {
        ...
        for (argNum = (int)1; argNum < argc; argNum++)
        {
            ...
            else if ((strcmp(argv[argNum], "--priority") == STRINGS_EQUAL) ||
            ...
            else if ((strcmp(argv[argNum], "--readfile") == STRINGS_EQUAL) ||
            ...
            else if ((strcmp(argv[argNum], "--write_bytes") == STRINGS_EQUAL) ||
            ...
            else if ((strcmp(argv[argNum], "--consolefile") == STRINGS_EQUAL) ||
            ...
            else if ((strcmp(argv[argNum], "--grab_kprofile") == STRINGS_EQUAL))
            {
            ...
            else if ((strcmp(argv[argNum], "--siggc") == STRINGS_EQUAL))
            {
 ...
 *  output: N/A
 ***********************************************************************/
void SetFileName(char* pFileName) /* ptr to desired output file name  */
{
 ...
 *  output: N/A
 ***********************************************************************/
void SetInterruptPeriod(char* /* ptr to desired interrupt period  */
...
void PrintHelpInfo(void)
{
    ...
    printf("  [-f FILE, --file FILE]     Set output file name to FILE. "
           "Typically you would put this on the fs under test\n");
    printf("  [-c FILE, --consolefile]   Set device or file to write the "
           "console log to.\n\tTypically you would set this to /dev/console "
           "and save it on another computer.\n");
    printf(
        "  [-w BYTES, --write_bytes BYTES  Write BYTES to FILE each period.\n");
    printf("  [-r FILE, --readfile FILE]     Also read 1 byte every cycle from "
           "FILE. FILE will be created and filled with data.\n");
    printf("  [-t <n>, --time <n>]       ");
    printf("Set interrupt period to <n> milliseconds.\n");
    printf("                           ");
    printf("Range: [%ld, %ld] milliseconds.\n", MIN_INT_PERIOD_MILLISEC,
           MAX_INT_PERIOD_MILLISEC);
    printf("  [-p <n>, --priority <n>]  ");
    printf("Set scheduler priority to <n>.\n");
    printf("                           ");
    printf("Range: [%d, %d] (higher number = higher priority)\n", MinPriority,
           MaxPriority);
    printf("  [--grab_kprofile <THRESHOLD in ms>]    Read the /proc/profile if "
           "jitter is > THRESHOLD and store in file.\n");
    printf("  [--siggc <PID>]   Before writing to fs send SIGSTOP to PID. "
           "After write send SIGCONT\n");
    return;
}

/* A common write that checks for write errors and exits. Pass it __LINE__ for
 * lineNo */
int Write(int fd, void* buf, size_t bytes, int lineNo)
{
    ...
    if (err < bytes)
    {
        ...
        printf("Write Error at line %i! Wanted to write %zu bytes, but wrote "
               "only %i bytes.\n",
               lineNo, bytes, err);
        perror(
            "Write did not complete. Error. Bye:"); /* show error from errno. */
        exit(1);
    }
