 ...
 */
static gchar* desktop_file_dir_get_alternative_dir(DesktopFileDir* dir)
{
    gchar* parent;
...
static void desktop_file_dir_changed(GFileMonitor* monitor, GFile* file,
                                     GFile* other_file,
                                     ...
                                     gpointer user_data)
{
    ...
    if (dir->alternatively_watching)
    {
        ...
        do_nothing = alternative_dir &&
                     g_str_equal(dir->alternatively_watching, alternative_dir);
 ...
 */
static gboolean desktop_file_dir_app_name_is_masked(DesktopFileDir* dir,
...
static const gchar* const* get_lowercase_current_desktops(void)
{
    static gchar** result;
...
static const gchar* const* get_current_desktops(const gchar* value)
{
    static gchar** result;
 ...
 * reference.
 */
static void add_to_table_if_appropriate(GHashTable* apps, const gchar* app_name,
...
const char* const exec_key_match_blacklist[] = {
    "bash",    "env",     "flatpak", "gjs",  "pkexec", "python",
    "python2", "python3", "sh",      "wine", "wine64", NULL};

static gchar* desktop_key_get_name(guint key_id)
{
    switch (key_id)
    {
...
static gint compare_results(gconstpointer a, gconstpointer b)
{
...
static gint compare_categories(gconstpointer a, gconstpointer b)
{
    const struct search_result* ra = a;
    const struct search_result* rb = b;
...
static void add_token_result(const gchar* app_name, guint16 category)
{
    if
        G_UNLIKELY(static_token_results_size == static_token_results_allocated)
        {
            static_token_results_allocated =
                MAX(16, static_token_results_allocated * 2);
            static_token_results =
                g_renew(struct search_result, static_token_results,
                        static_token_results_allocated);
        }
...
static void merge_token_results(gboolean first)
{
    if (static_token_results_size != 0)
        qsort(static_token_results, static_token_results_size,
              sizeof(struct search_result), compare_results);
    ...
    if (first)
    {
        ...
        if
            G_UNLIKELY(static_search_results_allocated <
                       static_token_results_size)
            {
                static_search_results_allocated =
                    static_token_results_allocated;
                static_search_results =
                    g_renew(struct search_result, static_search_results,
                            static_search_results_allocated);
            }
        ...
        for (i = 0; i < static_token_results_size; i++)
        {
            ...
            static_search_results[static_search_results_size++] =
                static_token_results[i];
        }
    }
    else
    {
        ...
        for (i = 0; i < static_token_results_size; i++)
        {
            ...
            while (k < static_search_results_size &&
                   static_search_results[k].app_name <
                       static_token_results[i].app_name)
                k++;
                    ...
                    static_token_results[i].app_name)
            {
                 ...
                 * numerically larger).
                 */
                static_search_results[j].app_name =
                    static_search_results[k].app_name;
                static_search_results[j].category =
                    MAX(static_search_results[k].category,
...
static void reset_total_search_results(void)
{
    static_total_results_size = 0;
}

static void sort_total_search_results(void)
{
    if (static_total_results_size != 0)
        qsort(static_total_results, static_total_results_size,
              sizeof(struct search_result), compare_categories);
}

static void merge_directory_results(void)
{
    if
        G_UNLIKELY(static_total_results_size + static_search_results_size >
                   static_total_results_allocated)
        {
            static_total_results_allocated =
                MAX(16, static_total_results_allocated);
            while (static_total_results_allocated <
                   static_total_results_size + static_search_results_size)
                static_total_results_allocated *= 2;
            static_total_results =
                g_renew(struct search_result, static_total_results,
                        static_total_results_allocated);
        }
...
static void get_apps_from_dir(GHashTable** apps, const char* dirname,
                              const char* prefix)
{
    const char* basename;
    GDir* dir;
    ...
    while ((basename = g_dir_read_name(dir)) != NULL)
    {
        ...
        if (g_str_has_suffix(basename, ".desktop"))
        {
            ...
            if (*apps == NULL)
                *apps = g_hash_table_new_full(g_str_hash, g_str_equal, g_free,
                                              g_free);
...
static void free_mime_tweaks(gpointer data)
{
...
/* consumes 'to_add' */
static void expand_strv(gchar*** strv_ptr, gchar** to_add,
...
static void desktop_file_dir_unindexed_read_mimeapps_list(
    DesktopFileDir* dir, const gchar* filename, const gchar* added_group,
    gboolean tweaks_permitted)
{
    ...
    if
        G_UNLIKELY(mime_types != NULL && !tweaks_permitted)
        {
            g_warning("%s contains a [%s] group, but it is not permitted here. "
                      " Only the non-desktop-specific "
                      "mimeapps.list file may add or remove associations.",
                      filename, added_group);
    ...
    if (mime_types != NULL)
    {
        for (i = 0; mime_types[i] != NULL; i++)
        {
            desktop_file_ids = g_key_file_get_string_list(
                key_file, added_group, mime_types[i], NULL, NULL);

            if (desktop_file_ids)
            {
                tweaks =
                    desktop_file_dir_unindexed_get_tweaks(dir, mime_types[i]);
                expand_strv(&tweaks->additions, desktop_file_ids,
                            tweaks->removals);
            }
        }
    ...
    mime_types =
        g_key_file_get_keys(key_file, REMOVED_ASSOCIATIONS_GROUP, NULL, NULL);
    ...
    if
        G_UNLIKELY(mime_types != NULL && !tweaks_permitted)
        {
            g_warning("%s contains a [%s] group, but it is not permitted here. "
                      " Only the non-desktop-specific "
                      "mimeapps.list file may add or remove associations.",
                      filename, REMOVED_ASSOCIATIONS_GROUP);
            g_strfreev(mime_types);
            mime_types = NULL;
        }
    ...
    if (mime_types != NULL)
    {
        ...
        {
            desktop_file_ids =
                g_key_file_get_string_list(key_file, REMOVED_ASSOCIATIONS_GROUP,
                                           mime_types[i], NULL, NULL);

            if (desktop_file_ids)
            {
                tweaks =
                    desktop_file_dir_unindexed_get_tweaks(dir, mime_types[i]);
                expand_strv(&tweaks->removals, desktop_file_ids,
                            tweaks->additions);
            }
        }
    ...
    mime_types =
        g_key_file_get_keys(key_file, DEFAULT_APPLICATIONS_GROUP, NULL, NULL);
    ...
    if (mime_types != NULL)
    {
        ...
        {
            desktop_file_ids =
                g_key_file_get_string_list(key_file, DEFAULT_APPLICATIONS_GROUP,
                                           mime_types[i], NULL, NULL);
...
static void desktop_file_dir_unindexed_read_mimeapps_lists(DesktopFileDir* dir)
{
    const gchar* const* desktops;
    gchar* filename;
    gint i;

    dir->mime_tweaks = g_hash_table_new_full(g_str_hash, g_str_equal, g_free,
                                             free_mime_tweaks);
    ...
    {
        filename =
            g_strdup_printf("%s/%s-mimeapps.list", dir->path, desktops[i]);
        desktop_file_dir_unindexed_read_mimeapps_list(
            dir, filename, ADDED_ASSOCIATIONS_GROUP, FALSE);
        g_free(filename);
    }
    ...
    desktop_file_dir_unindexed_read_mimeapps_list(
        dir, filename, ADDED_ASSOCIATIONS_GROUP, TRUE);
    g_free(filename);
    ...
    desktop_file_dir_unindexed_read_mimeapps_list(dir, filename,
                                                  MIME_CACHE_GROUP, TRUE);
    g_free(filename);
}

static void desktop_file_dir_unindexed_init(DesktopFileDir* dir)
{
    if (!dir->is_config)
...
static void desktop_file_dir_unindexed_get_all(DesktopFileDir* dir,
                                               GHashTable* apps)
{
    GHashTableIter iter;
    gpointer app_name;
    ...
    while (g_hash_table_iter_next(&iter, &app_name, &filename))
    {
        ...
        add_to_table_if_appropriate(
            apps, app_name, g_desktop_app_info_new_from_filename(filename));
    }
}
...
static void memory_index_entry_free(gpointer data)
{
...
static void memory_index_add_token(MemoryIndex* mi, const gchar* token,
...
static void memory_index_add_string(MemoryIndex* mi, const gchar* string,
                                    gint match_category, const gchar* app_name)
{
...
static MemoryIndex* memory_index_new(void)
{
    return g_hash_table_new_full(g_str_hash, g_str_equal, g_free,
                                 memory_index_entry_free);
}

static void desktop_file_dir_unindexed_setup_search(DesktopFileDir* dir)
{
    GHashTableIter iter;
    gpointer app, path;
    ...
    while (g_hash_table_iter_next(&iter, &app, &path))
    {
            ...
            !g_key_file_get_boolean(key_file, "Desktop Entry", "Hidden", NULL))
        {
            ...
            for (i = 0; i < G_N_ELEMENTS(desktop_key_match_category); i++)
            {
                ...
                raw = g_key_file_get_locale_string(key_file, "Desktop Entry",
                                                   desktop_key_get_name(i),
                                                   NULL, NULL);
                value = raw;
                ...
                if (value)
                    memory_index_add_string(dir->memory_index, value,
                                            desktop_key_match_category[i], app);

                g_free(raw);
            }

            /* Make note of the Implements= line */
            implements = g_key_file_get_string_list(key_file, "Desktop Entry",
                                                    "Implements", NULL, NULL);
            for (i = 0; implements && implements[i]; i++)
                memory_index_add_token(dir->memory_implementations,
                                       implements[i], 0, app);
...
static void desktop_file_dir_unindexed_search(DesktopFileDir* dir,
...
static gboolean array_contains(GPtrArray* array, const gchar* str)
{
    gint i;
...
static void desktop_file_dir_unindexed_mime_lookup(DesktopFileDir* dir,
                                                   const gchar* mime_type,
                                                   GPtrArray* hits,
                                                   GPtrArray* blacklist)
{
    ...
    if (tweaks->additions)
    {
        for (i = 0; tweaks->additions[i]; i++)
        {
            ...
            if (!desktop_file_dir_app_name_is_masked(dir, app_name) &&
                !array_contains(blacklist, app_name) &&
                !array_contains(hits, app_name))
...
static void desktop_file_dir_unindexed_default_lookup(DesktopFileDir* dir,
                                                      const gchar* mime_type,
                                                      GPtrArray* results)
{
    UnindexedMimeTweaks* tweaks;
...
static void desktop_file_dir_unindexed_get_implementations(
    DesktopFileDir* dir, GList** results, const gchar* interface)
{
    ...
    for (mie = g_hash_table_lookup(dir->memory_implementations, interface); mie;
         mie = mie->next)
 ...
 * to @array.
 */
static void desktop_file_dir_create(GArray* array, const gchar* data_dir)
{
    DesktopFileDir dir = {
        0,
    };
 ...
 * here.
 */
static void desktop_file_dir_create_for_config(GArray* array,
...
static void desktop_file_dir_reset(DesktopFileDir* dir)
{
    ...
    if (dir->monitor)
    {
        g_signal_handlers_disconnect_by_func(dir->monitor,
                                             desktop_file_dir_changed, dir);
 ...
 */
static void desktop_file_dir_init(DesktopFileDir* dir)
{
    ...
    watch_dir =
        dir->alternatively_watching ? dir->alternatively_watching : dir->path;
     ...
     */
    dir->monitor =
        g_local_file_monitor_new_in_worker(watch_dir, TRUE, G_FILE_MONITOR_NONE,
 ...
 */
static GDesktopAppInfo* desktop_file_dir_get_app(DesktopFileDir* dir,
                                                 const gchar* desktop_id)
{
    if (!dir->app_names)
        return NULL;
 ...
 */
static void desktop_file_dir_get_all(DesktopFileDir* dir, GHashTable* apps)
{
 ...
 */
static void desktop_file_dir_mime_lookup(DesktopFileDir* dir,
                                         const gchar* mime_type,
                                         GPtrArray* hits, GPtrArray* blacklist)
{
 ...
 */
static void desktop_file_dir_default_lookup(DesktopFileDir* dir,
                                            const gchar* mime_type,
                                            GPtrArray* results)
{
 ...
 */
static void desktop_file_dir_search(DesktopFileDir* dir,
                                    const gchar* search_token)
{
    desktop_file_dir_unindexed_search(dir, search_token);
}

static void desktop_file_dir_get_implementations(DesktopFileDir* dir,
                                                 GList** results,
...
static void desktop_file_dirs_lock(void)
{
    gint i;
    ...
    {
        g_debug("%s: Resetting desktop app info dirs from %s to %s", G_STRFUNC,
    ...
    if (desktop_file_dirs == NULL)
    {
        ...
        /* Now the data.  Highest priority: the user's
...
static void desktop_file_dirs_unlock(void)
{
    g_mutex_unlock(&desktop_file_dir_lock);
}

static void desktop_file_dirs_invalidate_user_config(void)
{
    g_mutex_lock(&desktop_file_dir_lock);

    if (n_desktop_file_dirs)
        desktop_file_dir_reset(
            &desktop_file_dirs[desktop_file_dir_user_config_index]);

    g_mutex_unlock(&desktop_file_dir_lock);
}

static void desktop_file_dirs_invalidate_user_data(void)
{
    g_mutex_lock(&desktop_file_dir_lock);

    if (n_desktop_file_dirs)
        desktop_file_dir_reset(
            &desktop_file_dirs[desktop_file_dir_user_data_index]);
...
static void g_desktop_app_info_set_property(GObject* object, guint prop_id,
                                            const GValue* value,
                                            GParamSpec* pspec)
{
...
static void g_desktop_app_info_get_property(GObject* object, guint prop_id,
...
static gboolean g_desktop_app_info_load_from_keyfile(GDesktopAppInfo* info,
                                                     GKeyFile* key_file)
{
    char* start_group;
    char* type;
    ...
    if (start_group == NULL ||
        strcmp(start_group, G_KEY_FILE_DESKTOP_GROUP) != 0)
    {
        g_free(start_group);
        return FALSE;
    }
    ...
    try_exec = g_key_file_get_string(key_file, G_KEY_FILE_DESKTOP_GROUP,
    ...
    info->name =
        g_key_file_get_locale_string(key_file, G_KEY_FILE_DESKTOP_GROUP,
                                     G_KEY_FILE_DESKTOP_KEY_NAME, NULL, NULL);
    info->generic_name = g_key_file_get_locale_string(
        key_file, G_KEY_FILE_DESKTOP_GROUP, GENERIC_NAME_KEY, NULL, NULL);
    info->fullname = g_key_file_get_locale_string(
        key_file, G_KEY_FILE_DESKTOP_GROUP, FULL_NAME_KEY, NULL, NULL);
    info->keywords = g_key_file_get_locale_string_list(
        key_file, G_KEY_FILE_DESKTOP_GROUP, KEYWORDS_KEY, NULL, NULL, NULL);
    info->comment = g_key_file_get_locale_string(
        key_file, G_KEY_FILE_DESKTOP_GROUP, G_KEY_FILE_DESKTOP_KEY_COMMENT,
        NULL, NULL);
    info->nodisplay = g_key_file_get_boolean(key_file, G_KEY_FILE_DESKTOP_GROUP,
    ...
    info->not_show_in = g_key_file_get_string_list(
        key_file, G_KEY_FILE_DESKTOP_GROUP, G_KEY_FILE_DESKTOP_KEY_NOT_SHOW_IN,
        NULL, NULL);
    info->try_exec = try_exec;
    info->exec = exec;
    info->path = g_key_file_get_string(key_file, G_KEY_FILE_DESKTOP_GROUP,
                                       G_KEY_FILE_DESKTOP_KEY_PATH, NULL);
    info->terminal =
        g_key_file_get_boolean(key_file, G_KEY_FILE_DESKTOP_GROUP,
                               G_KEY_FILE_DESKTOP_KEY_TERMINAL, NULL) != FALSE;
    info->startup_notify =
        g_key_file_get_boolean(key_file, G_KEY_FILE_DESKTOP_GROUP,
                               G_KEY_FILE_DESKTOP_KEY_STARTUP_NOTIFY,
                               NULL) != FALSE;
    info->no_fuse = g_key_file_get_boolean(key_file, G_KEY_FILE_DESKTOP_GROUP,
                                           "X-GIO-NoFuse", NULL) != FALSE;
    info->hidden =
        g_key_file_get_boolean(key_file, G_KEY_FILE_DESKTOP_GROUP,
                               G_KEY_FILE_DESKTOP_KEY_HIDDEN, NULL) != FALSE;
    info->categories =
        g_key_file_get_string(key_file, G_KEY_FILE_DESKTOP_GROUP,
                              G_KEY_FILE_DESKTOP_KEY_CATEGORIES, NULL);
    info->startup_wm_class = g_key_file_get_string(
        key_file, G_KEY_FILE_DESKTOP_GROUP, STARTUP_WM_CLASS_KEY, NULL);
    info->mime_types = g_key_file_get_string_list(
        key_file, G_KEY_FILE_DESKTOP_GROUP, G_KEY_FILE_DESKTOP_KEY_MIME_TYPE,
        NULL, NULL);
    bus_activatable =
        g_key_file_get_boolean(key_file, G_KEY_FILE_DESKTOP_GROUP,
                               G_KEY_FILE_DESKTOP_KEY_DBUS_ACTIVATABLE, NULL);
    info->actions =
        g_key_file_get_string_list(key_file, G_KEY_FILE_DESKTOP_GROUP,
                                   G_KEY_FILE_DESKTOP_KEY_ACTIONS, NULL, NULL);
...
static gboolean g_desktop_app_info_load_file(GDesktopAppInfo* self)
{
    ...
    if (g_key_file_load_from_file(key_file, self->filename, G_KEY_FILE_NONE,
                                  NULL))
...
static gboolean g_desktop_app_info_equal(GAppInfo* appinfo1, GAppInfo* appinfo2)
{
    ...
    if (info1->desktop_id == NULL || info2->desktop_id == NULL)
        return info1 == info2;
...
static char* expand_macro_single(char macro, const char* uri)
{
    GFile* file;
    char* result = NULL;
    char* path = NULL;
...
static char* expand_macro_uri(char macro, const char* uri,
...
static void expand_macro(char macro, GString* exec, GDesktopAppInfo* info,
                         GList** uri_list)
{
    GList* uris = *uri_list;
    char* expanded = NULL;
    ...
    switch (macro)
    {
        ...
        case 'n':
            ...
            {
                uri = uris->data;
                expanded = expand_macro_uri(macro, uri, force_file_uri,
...
static gboolean expand_application_parameters(GDesktopAppInfo* info,
...
static void notify_desktop_launch(GDBusConnection* session_bus,
                                  GDesktopAppInfo* info, long pid,
                                  const char* display, const char* sn_id,
                                  GList* uris)
{
    GDBusMessage* msg;
    ...
    if (sn_id != NULL && g_utf8_validate(sn_id, -1, NULL))
        g_variant_builder_add(&extras_variant, "{sv}", "startup-id",
                              g_variant_new("s", sn_id));
    gio_desktop_file = g_getenv("GIO_LAUNCHED_DESKTOP_FILE");
    if (gio_desktop_file != NULL)
        g_variant_builder_add(&extras_variant, "{sv}", "origin-desktop-file",
                              g_variant_new_bytestring(gio_desktop_file));
    if (g_get_prgname() != NULL)
        g_variant_builder_add(&extras_variant, "{sv}", "origin-prgname",
                              g_variant_new_bytestring(g_get_prgname()));
    g_variant_builder_add(&extras_variant, "{sv}", "origin-pid",
    ...
    msg = g_dbus_message_new_signal("/org/gtk/gio/DesktopAppInfo",
                                    "org.gtk.gio.DesktopAppInfo", "Launched");
    g_dbus_message_set_body(
        msg, g_variant_new("(@aysxasa{sv})",
                           g_variant_new_bytestring(desktop_file_id),
                           display ? display : "", (gint64)pid, &uri_variant,
                           &extras_variant));
...
static gboolean g_desktop_app_info_launch_uris_with_spawn(
    GDesktopAppInfo* info, GDBusConnection* session_bus, const gchar* exec_line,
    GList* uris, GAppLaunchContext* launch_context, GSpawnFlags spawn_flags,
    GSpawnChildSetupFunc user_setup, gpointer user_setup_data,
    GDesktopAppLaunchCallback pid_callback, gpointer pid_callback_data,
    gint stdin_fd, gint stdout_fd, gint stderr_fd, GError** error)
{
    ...
    do
    {
        ...
        old_uris = dup_uris;
        if (!expand_application_parameters(info, exec_line, &dup_uris, &argc,
                                           &argv, error))
            goto out;

        /* Get the subset of URIs we're launching with this process */
        launched_uris = NULL;
        for (iter = old_uris; iter != NULL && iter != dup_uris;
             iter = iter->next)
        ...
        if (info->filename)
            envp = g_environ_setenv(envp, "GIO_LAUNCHED_DESKTOP_FILE",
                                    info->filename, TRUE);
        ...
        if (launch_context)
        {
            ...
            if (info->startup_notify)
            {
                sn_id = g_app_launch_context_get_startup_notify_id(
                    launch_context, G_APP_INFO(info), launched_files);
                if (sn_id)
                    envp = g_environ_setenv(envp, "DESKTOP_STARTUP_ID", sn_id,
                                            TRUE);
            }
        ...
        if (!g_spawn_async_with_fds(info->path, wrapped_argv, envp, spawn_flags,
                                    user_setup, user_setup_data, &pid, stdin_fd,
        ...
        if (launch_context != NULL)
        {
            ...
            g_signal_emit_by_name(launch_context, "launched", info,
                                  platform_data);
            g_variant_unref(platform_data);
        }

        notify_desktop_launch(session_bus, info, pid, NULL, sn_id,
                              launched_uris);
...
static gchar* object_path_from_appid(const gchar* appid)
{
...
static GVariant*
    g_desktop_app_info_make_platform_data(GDesktopAppInfo* info, GList* uris,
        ...
        if (info->startup_notify)
        {
            ...
            if (sn_id)
                g_variant_builder_add(&builder, "{sv}", "desktop-startup-id",
                                      g_variant_new_take_string(sn_id));
        }
...
static void launch_uris_with_dbus(GDesktopAppInfo* info,
                                  GDBusConnection* session_bus, GList* uris,
                                  GAppLaunchContext* launch_context,
                                  ...
                                  gpointer user_data)
{
    ...
    g_variant_builder_add_value(&builder, g_desktop_app_info_make_platform_data(
                                              info, uris, launch_context));

    object_path = object_path_from_appid(info->app_id);
    g_dbus_connection_call(
        session_bus, info->app_id, object_path, "org.freedesktop.Application",
        uris ? "Open" : "Activate", g_variant_builder_end(&builder), NULL,
        G_DBUS_CALL_FLAGS_NONE, -1, cancellable, callback, user_data);
    g_free(object_path);
}

static gboolean g_desktop_app_info_launch_uris_with_dbus(
    GDesktopAppInfo* info, GDBusConnection* session_bus, GList* uris,
    ...
    GAsyncReadyCallback callback, gpointer user_data)
{
    ...
    launch_uris_with_dbus(info, session_bus, ruris, launch_context, cancellable,
                          callback, user_data);
...
static gboolean g_desktop_app_info_launch_uris_internal(
    GAppInfo* appinfo, GList* uris, GAppLaunchContext* launch_context,
    GSpawnFlags spawn_flags, GSpawnChildSetupFunc user_setup,
    gpointer user_setup_data, GDesktopAppLaunchCallback pid_callback,
    gpointer pid_callback_data, gint stdin_fd, gint stdout_fd, gint stderr_fd,
    GError** error)
{
    GDesktopAppInfo* info = G_DESKTOP_APP_INFO(appinfo);
    ...
    if (session_bus && info->app_id)
        ...
        g_desktop_app_info_launch_uris_with_dbus(
            info, session_bus, uris, launch_context, NULL, NULL, NULL);
    else
        success = g_desktop_app_info_launch_uris_with_spawn(
            info, session_bus, info->exec, uris, launch_context, spawn_flags,
...
static gboolean
    g_desktop_app_info_launch_uris(GAppInfo* appinfo, GList* uris,
                                   GAppLaunchContext* launch_context,
                                   GError** error)
{
    return g_desktop_app_info_launch_uris_internal(
        appinfo, uris, launch_context, _SPAWN_FLAGS_DEFAULT, NULL, NULL, NULL,
        NULL, -1, -1, -1, error);
}
...
static void launch_uris_data_free(LaunchUrisData* data)
{
...
static void launch_uris_with_dbus_cb(GObject* object, GAsyncResult* result,
                                     gpointer user_data)
{
...
static void launch_uris_flush_cb(GObject* object, GAsyncResult* result,
                                 gpointer user_data)
{
    GTask* task = G_TASK(user_data);
...
static void launch_uris_bus_get_cb(GObject* object, GAsyncResult* result,
                                   gpointer user_data)
{
    GTask* task = G_TASK(user_data);
    ...
    if (session_bus && info->app_id)
    {
        /* FIXME: The g_document_portal_add_documents() function, which is
         * called from the g_desktop_app_info_launch_uris_with_dbus() function,
         * still uses blocking calls.
         */
        g_desktop_app_info_launch_uris_with_dbus(
            info, session_bus, data->uris, data->context, cancellable,
            launch_uris_with_dbus_cb, g_steal_pointer(&task));
    }
    else
    {
         ...
         */
        g_desktop_app_info_launch_uris_with_spawn(
            info, session_bus, info->exec, data->uris, data->context,
            _SPAWN_FLAGS_DEFAULT, NULL, NULL, NULL, NULL, -1, -1, -1, &error);
        if (error != NULL)
        {
            g_task_return_error(task, g_steal_pointer(&error));
            g_object_unref(task);
            ...
            g_dbus_connection_flush(session_bus, cancellable,
...
static void g_desktop_app_info_launch_uris_async(GAppInfo* appinfo, GList* uris,
                                                 GAppLaunchContext* context,
                                                 GCancellable* cancellable,
                                                 GAsyncReadyCallback callback,
                                                 gpointer user_data)
{
    ...
    g_task_set_task_data(task, g_steal_pointer(&data),
                         (GDestroyNotify)launch_uris_data_free);

    g_bus_get(G_BUS_TYPE_SESSION, cancellable, launch_uris_bus_get_cb, task);
}

static gboolean g_desktop_app_info_launch_uris_finish(GAppInfo* appinfo,
                                                      GAsyncResult* result,
                                                      GError** error)
{
...
static gboolean g_desktop_app_info_supports_uris(GAppInfo* appinfo)
{
    ...
    return info->exec && ((strstr(info->exec, "%u") != NULL) ||
...
static gboolean g_desktop_app_info_supports_files(GAppInfo* appinfo)
{
    ...
    return info->exec && ((strstr(info->exec, "%f") != NULL) ||
                          (strstr(info->exec, "%F") != NULL));
}

static gboolean g_desktop_app_info_launch(GAppInfo* appinfo, GList* files,
                                          GAppLaunchContext* launch_context,
                                          GError** error)
{
    GList* uris;
 ...
 * @pid_callback_data: (closure pid_callback) (nullable): User data for
 * @callback
 ...
 * Since: 2.58
 */
gboolean g_desktop_app_info_launch_uris_as_manager_with_fds(
    GDesktopAppInfo* appinfo, GList* uris, GAppLaunchContext* launch_context,
    GSpawnFlags spawn_flags, GSpawnChildSetupFunc user_setup,
    gpointer user_setup_data, GDesktopAppLaunchCallback pid_callback,
    gpointer pid_callback_data, gint stdin_fd, gint stdout_fd, gint stderr_fd,
    GError** error)
{
    return g_desktop_app_info_launch_uris_internal(
        (GAppInfo*)appinfo, uris, launch_context, spawn_flags, user_setup,
        user_setup_data, pid_callback, pid_callback_data, stdin_fd, stdout_fd,
        stderr_fd, error);
}
    ...
    gpointer pid_callback_data, GError** error)
{
    return g_desktop_app_info_launch_uris_as_manager_with_fds(
        appinfo, uris, launch_context, spawn_flags, user_setup, user_setup_data,
        pid_callback, pid_callback_data, -1, -1, -1, error);
}
...
static char* ensure_dir(DirType type, GError** error)
{
    ...
    switch (type)
    {
        ...
        case MIMETYPE_DIR:
            path = g_build_filename(g_get_user_data_dir(), "mime", "packages",
                                    NULL);
            break;
    ...
    if (type == APP_DIR)
        g_set_error(
            error, G_IO_ERROR, g_io_error_from_errno(errsv),
...
static gboolean update_mimeapps_list(const char* desktop_id,
                                     const char* content_type,
                                     UpdateMimeFlags flags, GError** error)
{
    ...
    else
    {
        content_types = g_key_file_get_keys(
            key_file, DEFAULT_APPLICATIONS_GROUP, NULL, NULL);
    }

    for (k = 0; content_types && content_types[k]; k++)
    {
        /* set as default, if requested so */
        string = g_key_file_get_string(key_file, DEFAULT_APPLICATIONS_GROUP,
                                       content_types[k], NULL);
        ...
        if (string == NULL || desktop_id == NULL)
            g_key_file_remove_key(key_file, DEFAULT_APPLICATIONS_GROUP,
                                  content_types[k], NULL);
        else
            g_key_file_set_string(key_file, DEFAULT_APPLICATIONS_GROUP,
                                  content_types[k], string);
    ...
    else
    {
        ...
        content_types =
            g_key_file_get_keys(key_file, ADDED_ASSOCIATIONS_GROUP, NULL, NULL);
    }
    ...
    for (k = 0; content_types && content_types[k]; k++)
    {
        ...
        length = 0;
        old_list =
            g_key_file_get_string_list(key_file, ADDED_ASSOCIATIONS_GROUP,
        ...
        /* if we're adding a last-used hint, just put the application in front
         * of the list */
        ...
        if (old_list)
        {
            for (j = 0; old_list[j] != NULL; j++)
            {
                ...
                {
                    /* rewrite other entries if they're different from the new
                     * one */
                    list[i++] = g_strdup(old_list[j]);
                }
                else if (flags & UPDATE_MIME_SET_NON_DEFAULT)
                {
                    /* we encountered an old entry which is equal to the one
                     * we're adding as non-default, don't change its position in
                     * the list.
                     */
                    flags ^= UPDATE_MIME_SET_NON_DEFAULT;
                    list[i++] = g_strdup(old_list[j]);
        ...
        if (list[0] == NULL || desktop_id == NULL)
            g_key_file_remove_key(key_file, ADDED_ASSOCIATIONS_GROUP,
                                  content_types[k], NULL);
        else
            g_key_file_set_string_list(key_file, ADDED_ASSOCIATIONS_GROUP,
                                       content_types[k],
    ...
    else
    {
        g_strfreev(content_types);
        content_types = g_key_file_get_keys(
            key_file, REMOVED_ASSOCIATIONS_GROUP, NULL, NULL);
    }
    ...
    for (k = 0; content_types && content_types[k]; k++)
    {
        ...
        if (list[0] == NULL || desktop_id == NULL)
            g_key_file_remove_key(key_file, REMOVED_ASSOCIATIONS_GROUP,
                                  content_types[k], NULL);
        else
            g_key_file_set_string_list(key_file, REMOVED_ASSOCIATIONS_GROUP,
                                       content_types[k],
                                       (const char* const*)list, i);
...
static gboolean g_desktop_app_info_set_as_last_used_for_type(
    GAppInfo* appinfo, const char* content_type, GError** error)
{
    ...
    return update_mimeapps_list(
        info->desktop_id, content_type,
        UPDATE_MIME_SET_NON_DEFAULT | UPDATE_MIME_SET_LAST_USED, error);
}

static gboolean g_desktop_app_info_set_as_default_for_type(
    GAppInfo* appinfo, const char* content_type, GError** error)
{
    GDesktopAppInfo* info = G_DESKTOP_APP_INFO(appinfo);
...
static void run_update_command(char* command, char* subdir)
{
    ...
    if (g_spawn_async("/", argv, NULL, /* envp */
...
static gboolean g_desktop_app_info_set_as_default_for_extension(
...
static gboolean g_desktop_app_info_add_supports_type(GAppInfo* appinfo,
                                                     const char* content_type,
                                                     GError** error)
{
    GDesktopAppInfo* info = G_DESKTOP_APP_INFO(appinfo);
    ...
    return update_mimeapps_list(info->desktop_id, content_type,
                                UPDATE_MIME_SET_NON_DEFAULT, error);
}
...
static gboolean g_desktop_app_info_remove_supports_type(
    GAppInfo* appinfo, const char* content_type, GError** error)
{
    GDesktopAppInfo* info = G_DESKTOP_APP_INFO(appinfo);
    ...
    return update_mimeapps_list(info->desktop_id, content_type,
                                UPDATE_MIME_REMOVE, error);
}

static const char** g_desktop_app_info_get_supported_types(GAppInfo* appinfo)
{
    GDesktopAppInfo* info = G_DESKTOP_APP_INFO(appinfo);
                                                ...
                                                GError** error)
{
    ...
    g_key_file_set_string(key_file, G_KEY_FILE_DESKTOP_GROUP, "Encoding",
                          "UTF-8");
    ...
    if (info->fullname != NULL)
        g_key_file_set_string(key_file, G_KEY_FILE_DESKTOP_GROUP, FULL_NAME_KEY,
                              info->fullname);
...
static gboolean g_desktop_app_info_delete(GAppInfo* appinfo)
{
    ...
    if (info->filename)
    {
        ...
        {
            update_mimeapps_list(info->desktop_id, NULL, UPDATE_MIME_NONE,
                                 NULL);
 ...
 * @application_name: (nullable): the application name, or %NULL to use
 *@commandline
                                             ...
                                             GError** error)
{
    ...
    info->startup_notify =
        (flags & G_APP_INFO_CREATE_SUPPORTS_STARTUP_NOTIFICATION) != 0;
    info->hidden = FALSE;
    ...
    {
        /* FIXME: this should be more robust. Maybe g_shell_parse_argv and use
         * argv[0] */
...
static void g_desktop_app_info_iface_init(GAppInfoIface* iface)
{
    ...
    iface->set_as_last_used_for_type =
 ...
 */
static gchar** get_list_of_mimetypes(const gchar* content_type,
                                     gboolean include_fallback)
{
    ...
    if (include_fallback)
    {
        ...
        {
            gchar** parents =
                _g_unix_content_type_get_parents(g_ptr_array_index(array, i));
            gint j;
...
static gchar** g_desktop_app_info_get_desktop_ids_for_content_type(
    const gchar* content_type, gboolean include_fallback)
{
    ...
    for (i = 0; types[i]; i++)
        for (j = 0; j < n_desktop_file_dirs; j++)
            desktop_file_dir_mime_lookup(&desktop_file_dirs[j], types[i], hits,
                                         blacklist);
...
GList* g_app_info_get_recommended_for_type(const gchar* content_type)
{
    ...
    desktop_ids = g_desktop_app_info_get_desktop_ids_for_content_type(
        content_type, FALSE);
...
GList* g_app_info_get_fallback_for_type(const gchar* content_type)
{
    ...
    recommended_ids = g_desktop_app_info_get_desktop_ids_for_content_type(
        content_type, FALSE);
    all_ids =
        g_desktop_app_info_get_desktop_ids_for_content_type(content_type, TRUE);
                                          ...
                                          gboolean must_support_uris)
{
    ...
    for (i = 0; types[i]; i++)
    {
        ...
        for (j = 0; j < n_desktop_file_dirs; j++)
            desktop_file_dir_default_lookup(&desktop_file_dirs[j], types[i],
                                            results);

        /* Consider the associations as well... */
        for (j = 0; j < n_desktop_file_dirs; j++)
            desktop_file_dir_mime_lookup(&desktop_file_dirs[j], types[i],
                                         results, blacklist);
        ...
        for (j = 0; j < results->len; j++)
        {
            ...
            {
                info = (GAppInfo*)desktop_file_dir_get_app(
                    &desktop_file_dirs[k], desktop_id);
...
GList* g_desktop_app_info_get_implementations(const gchar* interface)
{
    ...
    for (i = 0; i < n_desktop_file_dirs; i++)
        desktop_file_dir_get_implementations(&desktop_file_dirs[i], &result,
                                             interface);
...
gchar*** g_desktop_app_info_search(const gchar* search_string)
{
    ...
    for (i = 0; i < n_categories; i++)
    {
        ...
        while (start_of_category + n_items_in_category <
                   static_total_results_size &&
               static_total_results[start_of_category + n_items_in_category]
        ...
        for (j = 0; j < n_items_in_category; j++)
            results[i][j] =
                g_strdup(static_total_results[start_of_category + j].app_name);
        results[i][j] = NULL;
 ...
 * the `Hidden` key set.
 *
 * Returns: (element-type GAppInfo) (transfer full): a newly allocated #GList of
 *references to #GAppInfos.
 **/
...
G_DEFINE_INTERFACE(GDesktopAppInfoLookup, g_desktop_app_info_lookup,
                   G_TYPE_OBJECT)

static void g_desktop_app_info_lookup_default_init(
    GDesktopAppInfoLookupInterface* iface)
{
}
 ...
 * Returns: (transfer full): #GAppInfo for given @uri_scheme or %NULL on error.
 *
 * Deprecated: The #GDesktopAppInfoLookup interface is deprecated and unused by
 * gio.
 */
...
char* g_desktop_app_info_get_string(GDesktopAppInfo* info, const char* key)
{
    ...
    return g_key_file_get_string(info->keyfile, G_KEY_FILE_DESKTOP_GROUP, key,
                                 NULL);
}
                                           ...
                                           const char* key)
{
    ...
    return g_key_file_get_locale_string(info->keyfile, G_KEY_FILE_DESKTOP_GROUP,
                                        key, NULL, NULL);
}
...
gboolean g_desktop_app_info_get_boolean(GDesktopAppInfo* info, const char* key)
{
    ...
    return g_key_file_get_boolean(info->keyfile, G_KEY_FILE_DESKTOP_GROUP, key,
                                  NULL);
}
 ...
 * (transfer full): a %NULL-terminated string array or %NULL if the specified
                                           ...
                                           const char* key, gsize* length)
{
    ...
    return g_key_file_get_string_list(info->keyfile, G_KEY_FILE_DESKTOP_GROUP,
                                      key, length, NULL);
}
...
gboolean g_desktop_app_info_has_key(GDesktopAppInfo* info, const char* key)
{
    ...
    return g_key_file_has_key(info->keyfile, G_KEY_FILE_DESKTOP_GROUP, key,
                              NULL);
}
...
static gboolean app_info_has_action(GDesktopAppInfo* info,
                                          ...
                                          const gchar* action_name)
{
    ...
    result = g_key_file_get_locale_string(info->keyfile, group_name, "Name",
                                          NULL, NULL);
    g_free(group_name);
                                      ...
                                      GAppLaunchContext* launch_context)
{
    ...
    if (session_bus && info->app_id)
    {
        ...
        object_path = object_path_from_appid(info->app_id);
        g_dbus_connection_call(
            session_bus, info->app_id, object_path,
            "org.freedesktop.Application", "ActivateAction",
            g_variant_new("(sav@a{sv})", action_name, NULL,
                          g_desktop_app_info_make_platform_data(
                              info, NULL, launch_context)),
    ...
    else
    {
        ...
        group_name = g_strdup_printf("Desktop Action %s", action_name);
        exec_line =
            g_key_file_get_string(info->keyfile, group_name, "Exec", NULL);
        g_free(group_name);

        if (exec_line)
            g_desktop_app_info_launch_uris_with_spawn(
                info, session_bus, exec_line, NULL, launch_context,
                _SPAWN_FLAGS_DEFAULT, NULL, NULL, NULL, NULL, -1, -1, -1, NULL);
