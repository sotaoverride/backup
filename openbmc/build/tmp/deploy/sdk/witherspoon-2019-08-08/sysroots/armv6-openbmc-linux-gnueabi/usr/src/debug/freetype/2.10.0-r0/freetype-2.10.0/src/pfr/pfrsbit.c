...
static void pfr_bitwriter_init(PFR_BitWriter writer, FT_Bitmap* target,
                               FT_Bool decreasing)
{
...
static void pfr_bitwriter_decode_bytes(PFR_BitWriter writer, FT_Byte* p,
                                       FT_Byte* limit)
{
    FT_UInt n, reload;
...
static void pfr_bitwriter_decode_rle1(PFR_BitWriter writer, FT_Byte* p,
                                      FT_Byte* limit)
{
...
static void pfr_bitwriter_decode_rle2(PFR_BitWriter writer, FT_Byte* p,
                                      FT_Byte* limit)
{
    FT_Int phase, count;
    FT_UInt n, reload;
...
static void pfr_lookup_bitmap_data(FT_Byte* base, FT_Byte* limit, FT_UInt count,
                                   FT_UInt* flags, FT_UInt char_code,
                                   FT_ULong* found_offset, FT_ULong* found_size)
{
...
/*                                                                    */
static FT_Error pfr_load_bitmap_metrics(FT_Byte** pdata, FT_Byte* limit,
                                        FT_Long scaled_advance, FT_Long* axpos,
                                        FT_Long* aypos, FT_UInt* axsize,
                                        FT_UInt* aysize, FT_Long* aadvance,
                                        FT_UInt* aformat)
{
...
static FT_Error pfr_load_bitmap_bits(FT_Byte* p, FT_Byte* limit, FT_UInt format,
                                     FT_Bool decreasing, FT_Bitmap* target)
{
    FT_Error error = FT_Err_Ok;
...
Found_Strike:
    ...
    /* now look up the glyph's position within the file */
    {
        ...
        pfr_lookup_bitmap_data(stream->cursor, stream->limit,
                               strike->num_bitmaps, &strike->flags,
                               character->char_code, &gps_offset, &gps_size);
    ...
    /* get the bitmap metrics */
    {
        ...
        if (phys->metrics_resolution != phys->outline_resolution)
            advance = FT_MulDiv(advance, (FT_Long)phys->outline_resolution,
        ...
        advance =
            FT_MulDiv((FT_Fixed)size->root.metrics.x_ppem << 8,
                      character->advance, (FT_Long)phys->metrics_resolution);
        ...
        p = stream->cursor;
        error =
            pfr_load_bitmap_metrics(&p, stream->limit, advance, &xpos, &ypos,
                                    &xsize, &ysize, &advance, &format);
        if (error)
            goto Exit1;
         ...
         */
        if (xpos > FT_INT_MAX || xpos < FT_INT_MIN || ysize > FT_INT_MAX ||
        ...
        if (!error)
        {
            ...
            /* Allocate and read bitmap data */
            {
                ...
                if (!error)
                    error =
                        pfr_load_bitmap_bits(p, stream->limit, format,
