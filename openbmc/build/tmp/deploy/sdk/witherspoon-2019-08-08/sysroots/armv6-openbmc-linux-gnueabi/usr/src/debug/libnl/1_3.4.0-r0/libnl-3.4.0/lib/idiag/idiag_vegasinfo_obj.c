...
{
    return (struct idiagnl_vegasinfo*)nl_object_alloc(
...
void idiagnl_vegasinfo_set_enabled(struct idiagnl_vegasinfo* vinfo,
                                   uint32_t enabled)
{
...
void idiagnl_vegasinfo_set_rttcnt(struct idiagnl_vegasinfo* vinfo,
                                  uint32_t rttcnt)
{
...
void idiagnl_vegasinfo_set_minrtt(struct idiagnl_vegasinfo* vinfo,
                                  uint32_t minrtt)
{
...
/** @cond SKIP */
static uint64_t idiagnl_vegasinfo_compare(struct nl_object* _a,
                                          struct nl_object* _b, uint64_t attrs,
                                          int flags)
{
    ...
    return a->tcpv_enabled != b->tcpv_enabled ||
           a->tcpv_rttcnt != b->tcpv_rttcnt || a->tcpv_rtt != b->tcpv_rtt ||
