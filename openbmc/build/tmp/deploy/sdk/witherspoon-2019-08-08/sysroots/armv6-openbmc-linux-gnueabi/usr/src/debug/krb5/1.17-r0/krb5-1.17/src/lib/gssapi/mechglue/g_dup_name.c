...
static OM_uint32 val_dup_name_args(OM_uint32* minor_status,
...
OM_uint32 KRB5_CALLCONV gss_duplicate_name(minor_status, src_name,
                                           dest_name) OM_uint32* minor_status;
const gss_name_t src_name;
gss_name_t* dest_name;
{
    ...
    {
        major_status = generic_gss_copy_oid(minor_status, src_union->mech_type,
        ...
        major_status =
            gssint_import_internal_name(minor_status, src_union->mech_type,
                                        src_union, &dest_union->mech_name);
        if (major_status != GSS_S_COMPLETE)
            goto allocation_failure;
    }
...
allocation_failure:
    if (dest_union)
    {
        ...
        if (dest_union->name_type)
            (void)generic_gss_release_oid(minor_status, &dest_union->name_type);
        if (dest_union->mech_name)
            (void)gssint_release_internal_name(
                minor_status, dest_union->mech_type, &dest_union->mech_name);
        if (dest_union->mech_type)
            (void)generic_gss_release_oid(minor_status, &dest_union->mech_type);
        free(dest_union);
    }
    return (major_status);
} /*	gss_duplicate_name	*/
