...
static void print_header(struct filefrag_struct* fs)
{
    ...
    {
        fprintf(fs->f, "%4s %*s %*s %*s %*s\n", "ext", fs->logical_width,
                "logical", fs->physical_width, "physical", fs->physical_width,
                "expected", fs->logical_width, "length");
    }
}
    ...
    if (fs->options & VERBOSE_OPT)
    {
        if (fs->expected)
            fprintf(fs->f, "%4d %*lu %*llu %*llu %*lu\n", fs->ext,
                    fs->logical_width, (unsigned long)fs->logical_start,
                    fs->physical_width, fs->physical_start, fs->physical_width,
                    fs->expected, fs->logical_width, (unsigned long)fs->num);
        else
                     ...
                     struct filefrag_struct* fs)
{
    ...
    fs->logical_width =
        int_log10((EXT2_I_SIZE(inode) + blocksize - 1) / blocksize) + 1;
    ...
    if (fs->options & VERBOSE_OPT)
    {
        ...
        fprintf(fs->f, "\n%s has %llu block(s), i_size is %llu\n", fs->name,
                num_blocks, EXT2_I_SIZE(inode));
    }
    print_header(fs);
    if (ext2fs_inode_has_valid_blocks2(current_fs, inode))
    {
        retval = ext2fs_block_iterate3(current_fs, ino, BLOCK_FLAG_READ_ONLY,
                                       NULL, filefrag_blocks_proc, fs);
        if (retval)
...
static int filefrag_dir_proc(ext2_ino_t dir EXT2FS_ATTR((unused)), int entry,
...
static void dir_iterate(ext2_ino_t ino, struct filefrag_struct* fs)
{
    ...
    {
        retval =
            ext2fs_dir_iterate2(current_fs, ino, 0, 0, filefrag_dir_proc, fs);
        if (retval)
