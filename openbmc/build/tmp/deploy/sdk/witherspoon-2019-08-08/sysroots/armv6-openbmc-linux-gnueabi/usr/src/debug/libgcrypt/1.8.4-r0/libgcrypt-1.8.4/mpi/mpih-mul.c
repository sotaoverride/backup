...
static mpi_limb_t mul_n_basecase(mpi_ptr_t prodp, mpi_ptr_t up, mpi_ptr_t vp,
                                 mpi_size_t size)
{
    mpi_size_t i;
    mpi_limb_t cy;
...
static void mul_n(mpi_ptr_t prodp, mpi_ptr_t up, mpi_ptr_t vp, mpi_size_t size,
                  mpi_ptr_t tspace)
{
    if (size & 1)
    {
    ...
    else
    {
        ...
        cy = _gcry_mpih_add_n(prodp + size, prodp + size, prodp + size + hsize,
                              hsize);
        ...
        cy += _gcry_mpih_add_n(prodp + hsize, prodp + hsize, tspace, size);
        if (cy)
            _gcry_mpih_add_1(prodp + hsize + size, prodp + hsize + size, hsize,
                             cy);

        MPN_COPY(prodp, tspace, hsize);
        cy = _gcry_mpih_add_n(prodp + hsize, prodp + hsize, tspace + hsize,
                              hsize);
        if (cy)
...
void _gcry_mpih_mul_karatsuba_case(mpi_ptr_t prodp, mpi_ptr_t up,
                                   mpi_size_t usize, mpi_ptr_t vp,
                                   mpi_size_t vsize, struct karatsuba_ctx* ctx)
{
    ...
    if (!ctx->tspace || ctx->tspace_size < vsize)
    {
        ...
        ctx->tspace = mpi_alloc_limb_space(
            2 * vsize, (_gcry_is_secure(up) || _gcry_is_secure(vp)));
    ...
    if (usize)
    {
        ...
        else
        {
            ...
            _gcry_mpih_mul_karatsuba_case(ctx->tspace, vp, vsize, up, usize,
                                          ctx->next);
        }
