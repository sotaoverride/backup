...
static int menuitem_on_page(struct cfdisk* cf, size_t idx)
{
    ...
    if (m->page_sz == 0 || m->idx / m->page_sz == idx / m->page_sz)
        return 1;
    return 0;
}
...
static int ui_create_label(struct cfdisk* cf)
{
    ...
    while (!sig_die)
    {
        ...
        {
            ui_draw_menu(cf);
            ui_hint(_("Select a type to create a new label or press 'L' to "
                      "load script file."));
            refresh();
            refresh_menu = 0;
        }
...
static int main_menu_action(struct cfdisk* cf, int key)
{
    ...
    switch (key)
    {
        ...
        {
            int fl = fdisk_is_label(cf->cxt, DOS)
                         ? DOS_FLAG_ACTIVE
        ...
        case 'n': /* New */
        {
            ...
            if (ui_get_size(cf, _("Partition size: "), &size,
                            fdisk_get_sector_size(cf->cxt), max_size,
                            &expsize) == -CFDISK_ERR_ESC)
                break;
        ...
        case 'W': /* Write */
        {
            ...
            ref = 1;
            if (rc <= 0 ||
                (strcasecmp(buf, "yes") != 0 && strcasecmp(buf, _("yes")) != 0))
            {
...
static int ui_run(struct cfdisk* cf)
{
    ...
    if (!fdisk_has_label(cf->cxt) || cf->zero_start)
    {
        ...
        if (rc < 0)
            ui_errx(EXIT_FAILURE, _("failed to create a new disklabel"));
        if (rc)
            return rc;
    }
