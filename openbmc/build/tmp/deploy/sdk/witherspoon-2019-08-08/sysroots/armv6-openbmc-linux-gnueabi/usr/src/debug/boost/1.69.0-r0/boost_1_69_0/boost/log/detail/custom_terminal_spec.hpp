 ...
 * \date   29.01.2012
 *
 * The header contains Boost.Phoenix custom terminal specialization for
 * Boost.Log terminals.
 */
...
struct custom_terminal<T, typename T::_is_boost_log_terminal>
{
    ...
    {
        typedef typename remove_cv<typename remove_reference<TermT>::type>::type
            term;
        typedef typename boost::result_of<const term(ContextT)>::type type;
    };

    template <typename ContextT>
    typename result<const this_type(T const&, ContextT&)>::type
