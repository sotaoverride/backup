                           ...
                           struct nlmsghdr* nlh, struct nl_parser_param* pp)
{
    ...
    addr->a_flags = tb[IFA_FLAGS] ? nla_get_u32(tb[IFA_FLAGS]) : ifa->ifa_flags;
    addr->a_ifindex = ifa->ifa_index;

    addr->ce_mask = (ADDR_ATTR_FAMILY | ADDR_ATTR_PREFIXLEN | ADDR_ATTR_FLAGS |
                     ADDR_ATTR_SCOPE | ADDR_ATTR_IFINDEX);
    ...
    if (family == AF_INET)
    {
        ...
        /* for IPv4/AF_INET, kernel always sets IFA_LOCAL and IFA_ADDRESS,
         * unless it is effectively 0.0.0.0. */
        if (tb[IFA_LOCAL])
        ...
        if (!nl_addr_cmp(addr->a_local, addr->a_peer))
        {
             ...
             * there is no peer. It means the peer is equal to the local
             * address, which is the case for "normal" addresses.
             *
    ...
    if (tb[IFA_MULTICAST])
    {
        addr->a_multicast = nl_addr_alloc_attr(tb[IFA_MULTICAST], family);
        if (!addr->a_multicast)
            goto errout_nomem;
...
static void addr_dump_line(struct nl_object* obj, struct nl_dump_params* p)
{
    ...
    if (addr->ce_mask & ADDR_ATTR_LOCAL)
        nl_dump_line(p, "%s", nl_addr2str(addr->a_local, buf, sizeof(buf)));
    else
        nl_dump_line(p, "none");
    ...
    if (link_cache)
        nl_dump(
            p, "dev %s ",
            rtnl_link_i2name(link_cache, addr->a_ifindex, buf, sizeof(buf)));
    else
...
static void addr_dump_details(struct nl_object* obj, struct nl_dump_params* p)
{
    ...
    if (addr->ce_mask &
        (ADDR_ATTR_LABEL | ADDR_ATTR_BROADCAST | ADDR_ATTR_MULTICAST))
    {
        nl_dump_line(p, "  ");
    ...
    if (addr->ce_mask & ADDR_ATTR_CACHEINFO)
    {
        ...
        nl_dump_line(p, "   valid-lifetime %s",
                     ci->aci_valid == 0xFFFFFFFFU
                         ? "forever"
                         : nl_msec2str(ci->aci_valid * 1000, buf, sizeof(buf)));

        nl_dump(p, " preferred-lifetime %s\n",
                ci->aci_prefered == 0xFFFFFFFFU
                    ? "forever"
                    : nl_msec2str(ci->aci_prefered * 1000, buf, sizeof(buf)));

        nl_dump_line(
            p, "   created boot-time+%s ",
            nl_msec2str(addr->a_cacheinfo.aci_cstamp * 10, buf, sizeof(buf)));

        nl_dump(
            p, "last-updated boot-time+%s\n",
            nl_msec2str(addr->a_cacheinfo.aci_tstamp * 10, buf, sizeof(buf)));
    }
}
...
static uint32_t addr_id_attrs_get(struct nl_object* obj)
{
    ...
    {
        case AF_INET:
            rv = (ADDR_ATTR_FAMILY | ADDR_ATTR_IFINDEX | ADDR_ATTR_LOCAL |
                  ADDR_ATTR_PREFIXLEN);
            if (addr->a_peer)
                rv |= ADDR_ATTR_PEER;
            return rv;
        case AF_INET6:
            return (ADDR_ATTR_FAMILY | ADDR_ATTR_IFINDEX | ADDR_ATTR_LOCAL);
        default:
            return (ADDR_ATTR_FAMILY | ADDR_ATTR_IFINDEX | ADDR_ATTR_LOCAL |
                    ADDR_ATTR_PREFIXLEN);
    }
...
#define ADDR_DIFF(ATTR, EXPR) ATTR_DIFF(attrs, ADDR_ATTR_##ATTR, a, b, EXPR)
    ...
    if (attrs & ADDR_ATTR_PEER)
    {
        ...
        {
            /* when comparing two IPv4 addresses for id-equality, the network
             * part of the PEER address shall be compared.
             */
    ...
    diff |= ADDR_DIFF(MULTICAST, nl_addr_cmp(a->a_multicast, b->a_multicast));
    ...
    if (flags & LOOSE_COMPARISON)
        diff |= ADDR_DIFF(FLAGS, (a->a_flags ^ b->a_flags) & b->a_flag_mask);
    else
...
{
    return __flags2str(attrs, buf, len, addr_attrs, ARRAY_SIZE(addr_attrs));
}
                                ...
                                struct nl_addr* addr)
{
    ...
    nl_list_for_each_entry(a, &cache->c_items, ce_list)
    {
        ...
        if (a->ce_mask & ADDR_ATTR_LOCAL && !nl_addr_cmp(a->a_local, addr))
        {
                          ...
                          struct nl_msg** result)
{
    ...
    {
        /* compatibility hack */
        if (tmpl->a_family == AF_INET && tmpl->ce_mask & ADDR_ATTR_LOCAL &&
    ...
    if (tmpl->a_flags & ~0xFF)
    {
         ...
         *
         * Older kernels refuse RTM_NEWADDR and RTM_NEWROUTE messages with
         * EINVAL if they contain unknown netlink attributes. See
         * net/core/rtnetlink.c, which was fixed by kernel commit
         * 661d2967b3f1b34eeaa7e212e7b9bbe8ee072b59.
         *
         * With this workaround, libnl will function correctly with older
         * kernels, unless there is a new libnl user that wants to set these
         * flags. In this case it's up to the user to workaround this issue. */
...
int rtnl_addr_set_anycast(struct rtnl_addr* addr, struct nl_addr* anycast)
{
    ...
    return __assign_addr(addr, &addr->a_anycast, anycast, ADDR_ATTR_ANYCAST);
}
...
{
    return __flags2str(flags, buf, size, addr_flags, ARRAY_SIZE(addr_flags));
}
