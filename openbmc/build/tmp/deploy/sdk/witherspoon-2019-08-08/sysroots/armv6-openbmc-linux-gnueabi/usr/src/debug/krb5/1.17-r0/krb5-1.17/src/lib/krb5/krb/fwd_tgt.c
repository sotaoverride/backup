...
/* Should forwarded TGT also be forwardable? */
{
    ...
    { /* must server be non-NULL when rhost is given? */
    ...
    retval =
        krb5int_tgtname(context, &client->realm, &client->realm, &creds.server);
    if (retval)
        goto errout;
    ...
    if ((retval = krb5_get_cred_via_tkt(context, &tgt, kdcoptions, addrs,
                                        &creds, &pcreds)))
    {
        if (enctype)
        {
    ...
    retval =
        krb5_mk_1cred(context, auth_context, pcreds, &scratch, &replaydata);
