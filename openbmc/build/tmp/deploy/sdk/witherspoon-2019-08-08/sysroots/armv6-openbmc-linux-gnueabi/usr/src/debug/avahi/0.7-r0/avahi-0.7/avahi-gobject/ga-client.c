...
#define GA_CLIENT_GET_PRIVATE(o)                                               \
    (G_TYPE_INSTANCE_GET_PRIVATE((o), GA_TYPE_CLIENT, GaClientPrivate))
...
static void ga_client_set_property(GObject* object, guint property_id,
...
static void ga_client_get_property(GObject* object, guint property_id,
...
static void ga_client_class_init(GaClientClass* ga_client_class)
{
    ...
    param_spec = g_param_spec_enum(
        "state", "Client state", "The state of the Avahi client",
        GA_TYPE_CLIENT_STATE, GA_CLIENT_STATE_NOT_STARTED,
        G_PARAM_READABLE | G_PARAM_STATIC_NAME | G_PARAM_STATIC_BLURB);
    g_object_class_install_property(object_class, PROP_STATE, param_spec);

    param_spec = g_param_spec_enum(
        "flags", "Client flags", "The flags the Avahi client is started with",
        GA_TYPE_CLIENT_FLAGS, GA_CLIENT_FLAG_NO_FLAGS,
        G_PARAM_READWRITE | G_PARAM_CONSTRUCT_ONLY | G_PARAM_STATIC_NAME |
            G_PARAM_STATIC_BLURB);
    g_object_class_install_property(object_class, PROP_FLAGS, param_spec);

    signals[STATE_CHANGED] = g_signal_new(
        "state-changed", G_OBJECT_CLASS_TYPE(ga_client_class),
        G_SIGNAL_RUN_LAST | G_SIGNAL_DETAILED, 0, NULL, NULL,
        g_cclosure_marshal_VOID__ENUM, G_TYPE_NONE, 1, GA_TYPE_CLIENT_STATE);
}
...
gboolean ga_client_start_in_context(GaClient* client, GMainContext* context,
                                    GError** error)
{
    GaClientPrivate* priv = GA_CLIENT_GET_PRIVATE(client);
    AvahiClient* aclient;
    ...
    aclient = avahi_client_new(avahi_glib_poll_get(priv->poll), priv->flags,
