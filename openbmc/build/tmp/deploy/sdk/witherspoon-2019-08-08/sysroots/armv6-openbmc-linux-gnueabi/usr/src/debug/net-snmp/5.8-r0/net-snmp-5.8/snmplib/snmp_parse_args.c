...
void snmp_parse_args_usage(FILE* outf)
{
    fprintf(outf, "[OPTIONS] AGENT");
}

void snmp_parse_args_descriptions(FILE* outf)
{
    ...
    fprintf(outf, "  -a PROTOCOL\t\tset authentication protocol "
    ...
    fprintf(outf, "  -l LEVEL\t\tset security level "
    ...
    fprintf(outf, "  -m MIB[" ENV_SEPARATOR
                  "...]\t\tload given list of MIBs (ALL loads everything)\n");
    fprintf(outf, "  -M DIR[" ENV_SEPARATOR
                  "...]\t\tlook in given list of directories for MIBs\n");
...
#endif
    fprintf(outf, "  -O OUTOPTS\t\tToggle various defaults controlling output "
                  "display:\n");
...
int netsnmp_parse_args(int argc, char** argv, netsnmp_session* session,
                       const char* localOpts,
                       void (*proc)(int, char* const*, int), int flags)
{
    int arg, sp = 0;
    char* cp;
    char* Apsz = NULL;
    ...
    while ((arg = getopt(argc, argv, Opts)) != EOF)
    {
        ...
        switch (arg)
        {
            ...
            case 'T':
            {
                char leftside[SNMP_MAXBUF_MEDIUM],
                    rightside[SNMP_MAXBUF_MEDIUM];
                char *tmpcp, *tmpopt;
                ...
                *tmpcp++ = '\0';
                ...
                CONTAINER_INSERT(
                    session->transport_configuration,
                    netsnmp_transport_create_config(leftside, rightside));
                free(tmpopt);
            }
            break;
...
#define SNMPV3_CMD_OPTIONS
    ...
    /*
     * save command line parameters which should have precedence above config
     * file settings (There ought to be a more scalable approach than this....)
     */
    if (netsnmp_ds_get_string(NETSNMP_DS_LIBRARY_ID,
                              NETSNMP_DS_LIB_OUTPUT_PRECISION))
    {
        backup_NETSNMP_DS_LIB_OUTPUT_PRECISION = strdup(netsnmp_ds_get_string(
            NETSNMP_DS_LIBRARY_ID, NETSNMP_DS_LIB_OUTPUT_PRECISION));
    }

    /*
     * read in MIB database and initialize the snmp library, read the config
     * file
     */
    init_snmp(NETSNMP_APPLICATION_CONFIG_TYPE);

    /*
     * restore command line parameters which should have precedence above config
     * file settings
     */
    ...
    if (session->version == SNMP_DEFAULT_VERSION)
    {
        ...
        if (!session->version)
        {
            switch (NETSNMP_DEFAULT_SNMP_VERSION)
            {
                ...
                default:
                    snmp_log(LOG_ERR, "Can't determine a valid SNMP version "
                                      "for the session\n");
                    return (NETSNMP_PARSE_ARGS_ERROR);
            }
        }
...
#ifndef NETSNMP_DISABLE_SNMPV1
            if (session->version ==
                NETSNMP_DS_SNMP_VERSION_1) /* bogus value.  version 1 actually =
                                              0 */
                session->version = SNMP_VERSION_1;
#endif
        }
    ...
    if (Apsz)
    {
        ...
        if (session->securityAuthProto == NULL)
        {
            session->securityAuthProto = snmp_duplicate_objid(
                SNMP_DEFAULT_AUTH_PROTO, SNMP_DEFAULT_AUTH_PROTOLEN);
            session->securityAuthProtoLen = SNMP_DEFAULT_AUTH_PROTOLEN;
        }
        if (generate_Ku(session->securityAuthProto,
                        session->securityAuthProtoLen, (u_char*)Apsz,
                        strlen(Apsz), session->securityAuthKey,
                        &session->securityAuthKeyLen) != SNMPERR_SUCCESS)
        {
            snmp_perror(argv[0]);
            fprintf(stderr, "Error generating a key (Ku) from the supplied "
    ...
    if (Xpsz)
    {
        ...
        if (generate_Ku(session->securityAuthProto,
                        session->securityAuthProtoLen, (u_char*)Xpsz,
                        strlen(Xpsz), session->securityPrivKey,
...
int snmp_parse_args(int argc, char** argv, netsnmp_session* session,
