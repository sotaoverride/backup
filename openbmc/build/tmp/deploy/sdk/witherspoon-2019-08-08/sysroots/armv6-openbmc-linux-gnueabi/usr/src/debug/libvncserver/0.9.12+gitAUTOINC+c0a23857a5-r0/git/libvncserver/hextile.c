...
#define PUT_PIXEL16(pix)                                                       \
    (cl->updateBuf[cl->ublen++] = ((char*)&(pix))[0],                          \
     cl->updateBuf[cl->ublen++] = ((char*)&(pix))[1])

#define PUT_PIXEL32(pix)                                                       \
    (cl->updateBuf[cl->ublen++] = ((char*)&(pix))[0],                          \
     cl->updateBuf[cl->ublen++] = ((char*)&(pix))[1],                          \
...
#define DEFINE_SEND_HEXTILES(bpp)                                              \
    ...
    static rfbBool subrectEncode##bpp(rfbClientPtr cli, uint##bpp##_t* data,   \
                                      int w, int h, uint##bpp##_t bg,          \
                                      uint##bpp##_t fg, rfbBool mono);         \
    static void testColours##bpp(uint##bpp##_t* data, int size, rfbBool* mono, \
                                 rfbBool* solid, uint##bpp##_t* bg,            \
                                 uint##bpp##_t* fg);                           \
                                                                               \
    /*                                                                         \
     * rfbSendHextiles                                                         \
     */                                                                        \
                                                                               \
    static rfbBool sendHextiles##bpp(rfbClientPtr cl, int rx, int ry, int rw,  \
                                     int rh)                                   \
    {                                                                          \
        int x, y, w, h;                                                        \
        int startUblen;                                                        \
        ...
        {                                                                      \
            ...
            {                                                                  \
                ...
                rfbStatRecordEncodingSentAdd(cl, rfbEncodingHextile, 1);       \
                                                                               \
                testColours##bpp(clientPixelData, w* h, &mono, &solid, &newBg, \
                                 &newFg);                                      \
                                                                               \
                if (!validBg || (newBg != bg))                                 \
                {                                                              \
                ...
                {                                                              \
                    ...
                    cl->updateBuf[startUblen] |= rfbHextileSubrectsColoured;   \
                                                                               ...
                                                                               \
                if (!subrectEncode##bpp(cl, clientPixelData, w, h, bg, fg,     \
                                        mono))                                 \
                {                                                              \
        ...
        return TRUE;                                                           \
                                                                               ...
                                                                               \
    static rfbBool subrectEncode##bpp(rfbClientPtr cl, uint##bpp##_t* data,    \
                                      int w, int h, uint##bpp##_t bg,          \
                                      uint##bpp##_t fg, rfbBool mono)          \
    {                                                                          \
        uint##bpp##_t cl2;                                                     \
