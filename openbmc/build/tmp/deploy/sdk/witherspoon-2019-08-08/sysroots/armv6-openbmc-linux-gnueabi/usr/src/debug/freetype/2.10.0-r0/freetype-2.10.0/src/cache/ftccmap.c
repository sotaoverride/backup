...
ftc_cmap_node_new(FTC_Node* ftcanode, FT_Pointer ftcquery, FTC_Cache cache)
{
    ...
    if (!FT_NEW(node))
    {
        ...
        node->first =
            (query->char_code / FTC_CMAP_INDICES_MAX) * FTC_CMAP_INDICES_MAX;
...
FT_CALLBACK_DEF(FT_Bool)
ftc_cmap_node_remove_faceid(FTC_Node ftcnode, FT_Pointer ftcface_id,
...
{
    return FTC_Manager_RegisterCache(manager, &ftc_cmap_cache_class,
                                     FTC_CACHE_P(acache));
}
...
FTC_CMapCache_Lookup(FTC_CMapCache cmap_cache, FTC_FaceID face_id,
                     FT_Int cmap_index, FT_UInt32 char_code)
{
...
#if 1
    FTC_CACHE_LOOKUP_CMP(cache, ftc_cmap_node_compare, hash, &query, node,
                         error);
#else
    ...
    if (gindex == FTC_CMAP_UNKNOWN)
    {
        ...
        error = FTC_Manager_LookupFace(cache->manager,
                                       FTC_CMAP_NODE(node)->face_id, &face);
        if (error)
            goto Exit;
