...
static const char usage[] =
    "Usage: " PROGRAM_NAME
    " <UBI device node file name> [<old name> <new name>|...]\n\n"
    "Example: " PROGRAM_NAME
    " /dev/ubi0 A B C D - rename volume A to B, and C to D\n\n"
...
int main(int argc, char* const argv[])
{
    ...
    {
        errmsg("too many volumes to re-name, max. is %d", UBI_MAX_RNVOL);
        return -1;
    }
    ...
    if (err == 2)
    {
        errmsg("\"%s\" is an UBI volume node, not an UBI device node", node);
        goto out_libubi;
    }
    else if (err < 0)
    {
