...
/* Thread-safe, exported version of that.  */
enum nss_status _nss_files_endaliasent(void)
{
    __libc_lock_lock(lock);
...
static enum nss_status get_next_alias(FILE* stream, const char* match,
        ...
        else
        {
            ...
            *first_unused++ = '\0';
            ...
            /* When we search for a specific alias we can avoid all the
               ...
               alias is found.  */
            ignore =
                (match != NULL && __strcasecmp(result->alias_name, match) != 0);
...
enum nss_status _nss_files_getaliasent_r(struct aliasent* result, char* buffer,
                                         size_t buflen, int* errnop)
{
    ...
    if (status == NSS_STATUS_SUCCESS)
    {
        ...
        do
            status =
                get_next_alias(stream, NULL, result, buffer, buflen, errnop);
...
enum nss_status _nss_files_getaliasbyname_r(const char* name,
                                            ...
                                            int* errnop)
{
    ...
    if (status == NSS_STATUS_SUCCESS)
    {
        ...
        do
            status =
                get_next_alias(stream, name, result, buffer, buflen, errnop);
        while (status == NSS_STATUS_RETURN);
    }
