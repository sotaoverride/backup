...
static char* replay_protect_str(char* buf, uint8_t replay_protect,
                                uint8_t window)
{
    if (replay_protect == 1)
    {
...
/** @cond SKIP */
#define PRINT_FLAG(buf, i, field, c)                                           \
    ({                                                                         \
        if (i->field == 1)                                                     \
            *buf++ = c;                                                        \
    })
...
static void macsec_dump_line(struct rtnl_link* link, struct nl_dump_params* p)
{
    ...
    nl_dump(p, "sci %016llx <%s>", ntohll(info->sci),
            flags_str(tmp, sizeof(tmp), info));
}

static void macsec_dump_details(struct rtnl_link* link,
                                struct nl_dump_params* p)
{
    ...
    nl_dump(p,
            "    sci %016llx protect %s encoding_sa %d encrypt %s send_sci %s "
            "validate %s %s\n",
            ntohll(info->sci), values_on_off[info->protect], info->encoding_sa,
...
static int macsec_put_attrs(struct nl_msg* msg, struct rtnl_link* link)
{
    ...
    if (info->cipher_suite != MACSEC_DEFAULT_CIPHER_ID ||
        info->icv_len != DEFAULT_ICV_LEN)
    {
...
#define MACSEC_DIFF(ATTR, EXPR) ATTR_DIFF(attrs, MACSEC_ATTR_##ATTR, a, b, EXPR)
    ...
    {
        int d =
            MACSEC_DIFF(REPLAY_PROTECT, a->replay_protect != b->replay_protect);
