...
static ServiceInfo* find_service(AvahiIfIndex interface, AvahiProtocol protocol,
...
static void print_service_line(Config* config, char c, AvahiIfIndex interface,
                               AvahiProtocol protocol, const char* name,
                               const char* type, const char* domain, int nl)
{
    ...
    if (config->parsable)
    {
        ...
        printf("%c;%s;%s;%s;%s;%s%s", c,
               ...
               protocol != AVAHI_PROTO_UNSPEC ? avahi_proto_to_string(protocol)
                                              : _("n/a"),
               avahi_escape_label(name, strlen(name), &e, &l), type, domain,
               nl ? "\n" : "");
    }
    else
    {
    ...
    AVAHI_GCC_UNUSED AvahiLookupResultFlags flags, void* userdata)
{
    ...
    switch (event)
    {
        case AVAHI_RESOLVER_FOUND:
        {
            ...
            print_service_line(i->config, '=', interface, protocol, name, type,
                               domain, 0);

            if (i->config->parsable)
                printf(";%s;%s;%u;%s\n", host_name, address, port, t);
            else
                printf("   hostname = [%s]\n"
                       "   address = [%s]\n"
                       "   port = [%u]\n"
        ...
        case AVAHI_RESOLVER_FAILURE:
            ...
            fprintf(stderr,
                    _("Failed to resolve service '%s' of type '%s' in domain "
                      "'%s': %s\n"),
                    name, type, domain,
...
static ServiceInfo* add_service(Config* c, AvahiIfIndex interface,
                                AvahiProtocol protocol, const char* name,
                                const char* type, const char* domain)
{
    ...
    if (c->resolve)
    {
        if (!(i->resolver = avahi_service_resolver_new(
                  ...
                  AVAHI_PROTO_UNSPEC, 0, service_resolver_callback, i)))
        {
            avahi_free(i);
            fprintf(stderr,
                    _("Failed to resolve service '%s' of type '%s' in domain "
...
static void browse_service_type(Config* c, const char* stype,
                                const char* domain)
{
    ...
    if (!(b = avahi_service_browser_new(client, AVAHI_IF_UNSPEC,
                                        AVAHI_PROTO_UNSPEC, stype, domain, 0,
                                        service_browser_callback, c)))
    {
...
static void service_type_browser_callback(
    AvahiServiceTypeBrowser* b, AVAHI_GCC_UNUSED AvahiIfIndex interface,
    AVAHI_GCC_UNUSED AvahiProtocol protocol, AvahiBrowserEvent event,
    const char* type, const char* domain,
    AVAHI_GCC_UNUSED AvahiLookupResultFlags flags, void* userdata)
{
...
static void browse_all(Config* c)
{
    ...
    if (!(b = avahi_service_type_browser_new(client, AVAHI_IF_UNSPEC,
                                             AVAHI_PROTO_UNSPEC, c->domain, 0,
                                             service_type_browser_callback, c)))
    {
...
static void domain_browser_callback(
    AvahiDomainBrowser* b, AVAHI_GCC_UNUSED AvahiIfIndex interface,
    AVAHI_GCC_UNUSED AvahiProtocol protocol, AvahiBrowserEvent event,
...
static void browse_domains(Config* c)
{
    ...
    if (!(b = avahi_domain_browser_new(
              client, AVAHI_IF_UNSPEC, AVAHI_PROTO_UNSPEC, c->domain,
              AVAHI_DOMAIN_BROWSER_BROWSE, 0, domain_browser_callback, c)))
    {
...
static int parse_command_line(Config* c, const char* argv0, int argc,
                              char* argv[])
{
    int o;
    ...
    c->command = strstr(argv0, "domain") ? COMMAND_BROWSE_DOMAINS
                                         : COMMAND_BROWSE_SERVICES;
    c->verbose = c->terminate_on_cache_exhausted = c->terminate_on_all_for_now =
        c->ignore_local = c->resolve = c->no_fail = c->parsable = 0;
