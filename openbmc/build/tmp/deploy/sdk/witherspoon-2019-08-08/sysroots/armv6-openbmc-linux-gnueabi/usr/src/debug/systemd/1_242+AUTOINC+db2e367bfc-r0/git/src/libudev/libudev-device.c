 ...
 *
 * This is only valid if the device was received through a monitor. Devices read
 *from sys do not have a sequence number.
 *
 * Returns: the kernel event sequence number, or 0 if there is no sequence
 *number available.
 **/
_public_ unsigned long long
 ...
 *
 * Returns: the devtype name of the udev device, or #NULL if it cannot be
 *determined
 **/
 ...
 * contain any "/".
 *
 * Returns: the subsystem name of the udev device, or #NULL if it cannot be
 *determined
 **/
 ...
 **/
_public_ struct udev_device* udev_device_new_from_syspath(struct udev* udev,
                                                          const char* syspath)
{
 ...
 * Returns: a new udev device, or #NULL, if it does not exist
 **/
_public_ struct udev_device* udev_device_new_from_device_id(struct udev* udev,
                                                            const char* id)
{
    _cleanup_(sd_device_unrefp) sd_device* device = NULL;
    int r;
    ...
    struct udev_device* udev_device, const char* subsystem, const char* devtype)
{
    ...
    r = sd_device_get_parent_with_subsystem_devtype(
        udev_device->device, subsystem, devtype, &parent);
    if (r < 0)
        return_with_errno(NULL, r);
 ...
 * Returns: #NULL
 **/
DEFINE_PUBLIC_TRIVIAL_REF_UNREF_FUNC(struct udev_device, udev_device,
                                     udev_device_free);
 ...
 *
 * Returns: the device node file name of the udev device, or #NULL if no device
 *node exists
 **/
    ...
    udev_device_get_devlinks_list_entry(struct udev_device* udev_device)
{
    ...
    if (device_get_devlinks_generation(udev_device->device) !=
            udev_device->devlinks_generation ||
        !udev_device->devlinks_read)
    {
        ...
        udev_device->devlinks_generation =
            device_get_devlinks_generation(udev_device->device);
    }
    ...
    udev_device_get_properties_list_entry(struct udev_device* udev_device)
{
    ...
    if (device_get_properties_generation(udev_device->device) !=
            udev_device->properties_generation ||
        !udev_device->properties_read)
    {
        ...
        udev_device->properties_generation =
            device_get_properties_generation(udev_device->device);
    }
 ...
 *
 * This is only valid if the device was received through a monitor. Devices read
 *from sys do not have an action string. Usual actions are: add, remove, change,
 *online, offline.
 *
 * Returns: the kernel action value, or #NULL if there is no action value
 *available.
 **/
    ...
    udev_device_get_tags_list_entry(struct udev_device* udev_device)
{
    ...
    if (device_get_tags_generation(udev_device->device) !=
            udev_device->tags_generation ||
 ...
 **/
_public_ int udev_device_has_tag(struct udev_device* udev_device,
                                 const char* tag)
{
