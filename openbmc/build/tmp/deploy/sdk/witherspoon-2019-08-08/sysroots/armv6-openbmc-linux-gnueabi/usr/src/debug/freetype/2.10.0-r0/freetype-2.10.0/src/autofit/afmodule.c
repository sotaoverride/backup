...
static FT_Error af_property_set(FT_Module ft_module, const char* property_name,
                                const void* value, FT_Bool value_is_string)
{
    FT_Error error = FT_Err_Ok;
    ...
    else if (!ft_strcmp(property_name, "darkening-parameters"))
    {
        ...
        if (x1 < 0 || x2 < 0 || x3 < 0 || x4 < 0 || y1 < 0 || y2 < 0 ||
            y3 < 0 || y4 < 0 || x1 > x2 || x2 > x3 || x3 > x4 || y1 > 500 ||
...
static FT_Error af_property_get(FT_Module ft_module, const char* property_name,
                                void* value)
{
    FT_Error error = FT_Err_Ok;
    AF_Module module = (AF_Module)ft_module;
...
FT_DEFINE_SERVICEDESCREC1(af_services,

                          FT_SERVICE_ID_PROPERTIES, &af_service_properties)

FT_CALLBACK_DEF(FT_Module_Interface)
af_get_interface(FT_Module module, const char* module_interface)
{
    FT_UNUSED(module);
...
FT_CALLBACK_DEF(FT_Error)
af_autofitter_load_glyph(AF_Module module, FT_GlyphSlot slot, FT_Size size,
                         FT_UInt glyph_index, FT_Int32 load_flags)
{
    ...
    error = af_loader_load_glyph(loader, module, slot->face, glyph_index,
                                 load_flags);
...
FT_DEFINE_MODULE(autofit_module_class,
                 ...
                 FT_MODULE_HINTER, sizeof(AF_ModuleRec),

                 "autofitter", 0x10000L, /* version 1.0 of the autofitter  */
