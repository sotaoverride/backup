...
static void g_unix_output_stream_pollable_iface_init(
    GPollableOutputStreamInterface* iface);
static void g_unix_output_stream_file_descriptor_based_iface_init(
    GFileDescriptorBasedIface* iface);

G_DEFINE_TYPE_WITH_CODE(
    GUnixOutputStream, g_unix_output_stream, G_TYPE_OUTPUT_STREAM,
...
static void g_unix_output_stream_set_property(GObject* object, guint prop_id,
                                              const GValue* value,
                                              GParamSpec* pspec);
static void g_unix_output_stream_get_property(GObject* object, guint prop_id,
                                              GValue* value, GParamSpec* pspec);
static gssize g_unix_output_stream_write(GOutputStream* stream,
                                         const void* buffer, gsize count,
                                         GCancellable* cancellable,
                                         GError** error);
static gboolean g_unix_output_stream_writev(
...
static GSource*
    g_unix_output_stream_pollable_create_source(GPollableOutputStream* stream,
                                                GCancellable* cancellable);
static GPollableReturn g_unix_output_stream_pollable_writev_nonblocking(
    GPollableOutputStream* stream, const GOutputVector* vectors,
...
static void g_unix_output_stream_class_init(GUnixOutputStreamClass* klass)
{
    ...
    g_object_class_install_property(
        ...
        g_param_spec_int("fd", P_("File descriptor"),
                         P_("The file descriptor to write to"), G_MININT,
                         G_MAXINT, -1,
     ...
     */
    g_object_class_install_property(
        gobject_class, PROP_CLOSE_FD,
        g_param_spec_boolean("close-fd", P_("Close file descriptor"),
                             P_("Whether to close the file descriptor when the "
                                "stream is closed"),
                             TRUE,
                             G_PARAM_READABLE | G_PARAM_WRITABLE |
    ...
    GPollableOutputStreamInterface* iface)
{
    ...
    iface->writev_nonblocking =
        g_unix_output_stream_pollable_writev_nonblocking;
}
                                              ...
                                              GParamSpec* pspec)
{
    ...
    switch (prop_id)
    {
        case PROP_FD:
            ...
            if (lseek(unix_stream->priv->fd, 0, SEEK_CUR) == -1 &&
                errno == ESPIPE)
...
GOutputStream* g_unix_output_stream_new(gint fd, gboolean close_fd)
{
    ...
    stream = g_object_new(G_TYPE_UNIX_OUTPUT_STREAM, "fd", fd, "close-fd",
                          close_fd, NULL);
                                           ...
                                           GError** error)
{
    ...
    /* This might block during the close. Doesn't seem to be a way to avoid it
     * though. */
    ...
    if (res == -1)
    {
        ...
        g_set_error(error, G_IO_ERROR, g_io_error_from_errno(errsv),
                    _("Error closing file descriptor: %s"), g_strerror(errsv));
    }
