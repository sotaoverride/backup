...
#define G_DATALIST_GET_POINTER(datalist)                                       \
    ((GData*)((gsize)g_atomic_pointer_get(datalist) &                          \
              ~(gsize)G_DATALIST_FLAGS_MASK_INTERNAL))
...
#define G_DATALIST_SET_POINTER(datalist, pointer)                              \
    ...
    {                                                                          \
        ...
        {                                                                      \
            ...
            _newv =                                                            \
                (gpointer)(((gsize)_oldv & G_DATALIST_FLAGS_MASK_INTERNAL) |   \
                           (gsize)pointer);                                    \
        } while (!g_atomic_pointer_compare_and_exchange((void**)datalist,      \
                                                        _oldv, _newv));        \
    }                                                                          \
    G_STMT_END
...
static inline gpointer g_data_set_internal(GData** datalist, GQuark key_id,
                                           gpointer data,
 ...
 * which protects that modification of the non-flags part of the datalist
 * pointer and the contents of the datalist.
 *
...
static void g_datalist_lock(GData** datalist)
{
    g_pointer_bit_lock((void**)datalist, DATALIST_LOCK_BIT);
}

static void g_datalist_unlock(GData** datalist)
{
 ...
 */
static void g_datalist_clear_i(GData** datalist)
{
    GData* data;
    guint i;
 ...
 * function is called if it has been set.
 **/
void g_dataset_id_set_data_full(gconstpointer dataset_location, GQuark key_id,
                                gpointer data, GDestroyNotify destroy_func)
{
    ...
    if (!dataset)
    {
        ...
        g_hash_table_insert(g_dataset_location_ht, (gpointer)dataset->location,
                            dataset);
    }

    g_data_set_internal(&dataset->datalist, key_id, data, destroy_func,
                        dataset);
    G_UNLOCK(g_dataset_global);
}
 ...
 **/
void g_datalist_id_set_data_full(GData** datalist, GQuark key_id, gpointer data,
                                 GDestroyNotify destroy_func)
{
    g_return_if_fail(datalist != NULL);
    if (!data)
                                       ...
                                       GQuark key_id)
{
    ...
    if (key_id && g_dataset_location_ht)
    {
        ...
        dataset = g_dataset_lookup(dataset_location);
        if (dataset)
            ret_data = g_data_set_internal(&dataset->datalist, key_id, NULL,
                                           (GDestroyNotify)42, dataset);
    }
    G_UNLOCK(g_dataset_global);
...
gpointer g_datalist_id_remove_no_notify(GData** datalist, GQuark key_id)
{
    ...
    if (key_id)
        ret_data = g_data_set_internal(datalist, key_id, NULL,
                                       (GDestroyNotify)42, NULL);
 ...
 * Since: 2.34
 */
gpointer g_datalist_id_dup_data(GData** datalist, GQuark key_id,
 ...
 * Since: 2.34
 */
gboolean g_datalist_id_replace_data(GData** datalist, GQuark key_id,
            ...
            if (data->key == key_id)
            {
                ...
                if (val == oldval)
                {
                    ...
                    else
                    {
                        ...
                        /* We don't bother to shrink, but if all data are now
                         * gone we at least free the memory
                         */
                        if (d->len == 0)
                        {
                            G_DATALIST_SET_POINTER(datalist, NULL);
 ...
 **/
void g_dataset_foreach(gconstpointer dataset_location, GDataForeachFunc func,
                       gpointer user_data)
{
    GDataset* dataset;
                        ...
                        gpointer user_data)
{
    ...
    for (i = 0; i < len; i++)
    {
        /* A previous callback might have removed a later item, so always check
       that it still exists before calling */
        d = G_DATALIST_GET_POINTER(datalist);
