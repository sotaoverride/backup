...
static void growhashtable(DICT* dict)
{
    ...
    newtable =
        (struct dict_entry**)malloc(newsize * sizeof(struct dict_entry*));
    if (newtable == NULL)
...
DICT* dict_new(void)
{
    ...
    dict->table =
        (struct dict_entry**)malloc(DICT_INITSIZE * sizeof(struct dict_entry*));
    if (dict->table == NULL)
    {
        free(dict);
        return NULL;
...
void* dict_get(DICT* dict, const char* key)
{
    ...
    for (entry = dict->table[hash % dict->size]; entry != NULL;
         entry = entry->next)
    {
...
int dict_put(DICT* dict, const char* key, void* value)
{
    ...
    for (entry = dict->table[idx], prev = NULL; entry != NULL;
