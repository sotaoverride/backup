...
oid old_extensible_variables_oid[] = {NETSNMP_UCDAVIS_MIB, NETSNMP_SHELLMIBNUM,
                                      1};
#ifndef NETSNMP_NO_WRITE_SUPPORT
...
extend_registration_block* _find_extension_block(oid* name, size_t name_len)
{
...
extend_registration_block* _register_extend(oid* base, size_t len)
{
    extend_registration_block* eptr;
    ...
    reg = netsnmp_create_handler_registration(
        "nsExtendConfigTable", handle_nsExtendConfigTable, oid_buf, len + 1,
        HANDLER_CAN_RWRITE);
#else  /* !NETSNMP_NO_WRITE_SUPPORT */
    reg = netsnmp_create_handler_registration(
        "nsExtendConfigTable", handle_nsExtendConfigTable, oid_buf, len + 1,
    ...
    oid_buf[len] = 3;
    reg = netsnmp_create_handler_registration(
        "nsExtendOut1Table", handle_nsExtendOutput1Table, oid_buf, len + 1,
        HANDLER_CAN_RONLY);
    rc = netsnmp_register_table_data(reg, dinfo, tinfo);
    if (rc != SNMPERR_SUCCESS)
        goto bail;
    ...
    oid_buf[len] = 4;
    reg = netsnmp_create_handler_registration(
        "nsExtendOut2Table", handle_nsExtendOutput2Table, oid_buf, len + 1,
        HANDLER_CAN_RONLY);
    ...
    winfo = netsnmp_create_watcher_info(&(eptr->num_entries),
                                        sizeof(eptr->num_entries), ASN_INTEGER,
                                        WATCHER_FIXED_SIZE);
...
static void _unregister_extend(extend_registration_block* eptr)
{
...
int extend_clear_callback(int majorID, int minorID, void* serverarg,
                          void* clientarg)
{
...
void init_extend(void)
{
    ...
    compatability_entries = (netsnmp_old_extend*)calloc(
        max_compatability_entries, sizeof(netsnmp_old_extend));
    REGISTER_MIB("ucd-extensible", old_extensible_variables, variable2,
...
int extend_load_cache(netsnmp_cache* cache, void* magic)
{
    ...
    if (extension->args)
        snprintf(cmd_buf, cmd_len, "%s %s", extension->command,
                 extension->args);
    else
    ...
    if (ret >= 0)
    {
        ...
        {
            extension->lines =
                (char**)calloc(sizeof(char*), extension->numlines);
...
void extend_free_cache(netsnmp_cache* cache, void* magic)
{
    netsnmp_extend* extension = (netsnmp_extend*)magic;
    if (!magic)
        return;
...
netsnmp_extend* _new_extension(char* exec_name, int exec_flags,
                               extend_registration_block* ereg)
{
    ...
    extension->cache =
        netsnmp_cache_create(0, extend_load_cache, extend_free_cache, NULL, 0);
    if (extension->cache)
    ...
    extension->row = row;
    netsnmp_table_row_add_index(row, ASN_OCTET_STR, exec_name,
                                strlen(exec_name));
     ...
     */
    for (eptr1 = ereg->ehead, eptr2 = NULL; eptr1;
...
void extend_parse_config(const char* token, char* cptr)
{
    ...
    if (*exec_name == '.')
    {
        ...
        {
            config_perror("ERROR: This output format has been deprecated - "
    ...
    if (!eptr)
    {
        snmp_log(
            LOG_ERR,
            "Failed to register extend entry '%s' - possibly duplicate name.\n",
            exec_name);
        return;
    }
    ...
    if (!strcmp(token, "execFix"))
    {
        ...
        {
            if (!strcmp(exec_name2, compatability_entries[i].exec_entry->token))
                break;
        }
    ...
    else if (!strcmp(token, "sh") || !strcmp(token, "exec"))
    {
        if (num_compatability_entries == max_compatability_entries)
        {
            ...
            new_compatability_entries =
                realloc(compatability_entries, max_compatability_entries * 2 *
        ...
        if (num_compatability_entries != max_compatability_entries)
            compatability_entries[num_compatability_entries++].exec_entry =
                extension;
    }
#endif
}
...
int handle_nsExtendConfigTable(netsnmp_mib_handler* handler,
                               ...
                               netsnmp_request_info* requests)
{
    ...
    for (request = requests; request; request = request->next)
    {
        ...
        switch (reqinfo->mode)
        {
            case MODE_GET:
                switch (table_info->colnum)
                {
                    case COLUMN_EXTCFG_COMMAND:
                        snmp_set_var_typed_value(
                            ...
                            extension->command,
                            (extension->command) ? strlen(extension->command)
                                                 : 0);
                        break;
                    case COLUMN_EXTCFG_ARGS:
                        snmp_set_var_typed_value(
                            request->requestvb, ASN_OCTET_STR, extension->args,
                            (extension->args) ? strlen(extension->args) : 0);
                        break;
                    case COLUMN_EXTCFG_INPUT:
                        snmp_set_var_typed_value(
                            request->requestvb, ASN_OCTET_STR, extension->input,
                 ...
                 *     (if read from a config file) or 'volatile' (if set via
                 *SNMP) The string-based settings of a 'permanent' entry cannot
            ...
            case MODE_SET_RESERVE1:
                ...
                switch (table_info->colnum)
                {
                    case COLUMN_EXTCFG_COMMAND:
                        ...
                        if (extension &&
                            extension->flags & NS_EXTEND_FLAGS_CONFIG)
                        {
                            /*
                             * config entries are "permanent" so can't be
                             * changed
                             */
                            netsnmp_set_request_error(reqinfo, request,
                    ...
                    case COLUMN_EXTCFG_RUNTYPE:
                              ...
                              (extension->flags & NS_EXTEND_FLAGS_WRITEABLE)))
                        {
                            netsnmp_set_request_error(
                                reqinfo, request, SNMP_ERR_INCONSISTENTVALUE);
                ...
                switch (table_info->colnum)
                {
                    case COLUMN_EXTCFG_STATUS:
                        ...
                        switch (i)
                        {
                            case RS_CREATEANDGO:
                            case RS_CREATEANDWAIT:
                                eptr = _find_extension_block(
                                    request->requestvb->name,
                                    request->requestvb->name_length);
                                extension = _new_extension(
                                    (char*)table_info->indexes->val.string, 0,
                                    eptr);
                                if (!extension)
                                { /* failed */
                                    netsnmp_set_request_error(
                                ...
                                netsnmp_insert_table_row(request,
                                                         extension->row);
                        }
                }
                break;
                    ...
                    case COLUMN_EXTCFG_RUNTYPE:
                        ...
                        switch (i)
                        {
                            ...
                            case 2:
                                ...
                                break;
                            case 3:
                                (void)netsnmp_cache_check_and_reload(
                                    extension->cache);
                                break;
                        }
                        break;
    ...
    if (need_to_validate)
    {
        for (request = requests; request; request = request->next)
        {
            ...
            table_info = netsnmp_extract_table_info(request);
            extension =
                (netsnmp_extend*)netsnmp_extract_table_row_data(request);
            switch (table_info->colnum)
            {
                case COLUMN_EXTCFG_STATUS:
                    i = *request->requestvb->val.integer;
                    if ((i == RS_ACTIVE || i == RS_CREATEANDGO) && !(extension &&
                                                                     extension
                                                                         ->command &&
                                                                     extension->command
                                                                             [0] ==
...
int handle_nsExtendOutput1Table(netsnmp_mib_handler* handler,
                                netsnmp_handler_registration* reginfo,
                                netsnmp_agent_request_info* reqinfo,
                                netsnmp_request_info* requests)
{
            ...
            case MODE_GET:
                ...
                switch (table_info->colnum)
                {
                    ...
                    case COLUMN_EXTOUT1_OUTPUT1:
                        ...
                        {
                            len =
                                (extension->lines[1]) - (extension->output) - 1;
                        }
 ...
 */
netsnmp_extend* _extend_find_entry(netsnmp_request_info* request,
                                   netsnmp_table_request_info* table_info,
                                   int mode)
{
    netsnmp_extend* eptr;
    ...
    else
    {
        ...
        if (eptr)
        {
             ...
             */
            snmp_set_var_value(table_info->indexes, eptr->token,
                               strlen(eptr->token));
 ...
 */
int handle_nsExtendOutput2Table(netsnmp_mib_handler* handler,
                                netsnmp_handler_registration* reginfo,
                                netsnmp_agent_request_info* reqinfo,
                                netsnmp_request_info* requests)
{
...
u_char* var_extensible_old(struct variable* vp, oid* name, size_t* length,
        ...
        switch (vp->magic)
        {
            ...
            case ERRORFIXCMD:
                ...
                else
                {
                    *var_len = 0;
                    return (
                        (u_char*)&long_return); /* Just needs to be non-null! */
                }
        }
        return NULL;
    }
...
int fixExec2Error(int action, u_char* var_val, u_char var_val_type,
