...
static struct nl_sock* get_nbd_socket(int* driver_id)
{
    ...
    *driver_id = genl_ctrl_resolve(socket, "nbd");
    if (*driver_id < 0)
        err("Couldn't resolve the nbd netlink family, make sure the nbd module "
...
int opennet(char* name, char* portstr, int sdp)
{
    ...
    if (sdp)
    {
        ...
        if (ai->ai_family == AF_INET)
            ...
            (ai->ai_family == AF_INET6) ai->ai_family = AF_INET6_SDP;
#else
...
void send_info_request(int sock, uint32_t opt, int n_reqs, uint16_t* reqs,
                       char* name)
{
    uint16_t rlen = htons(n_reqs);
    uint32_t nlen = htonl(strlen(name));

    send_request(sock, opt,
                 sizeof(uint32_t) + strlen(name) + sizeof(uint16_t) +
                     n_reqs * sizeof(uint16_t),
                 NULL);
...
void send_opt_exportname(int sock, u64* rsize64, uint16_t* flags,
                         bool can_opt_go, char* name, uint16_t global_flags)
{
    ...
    {
        err("E: server does not support NBD_OPT_GO and dropped connection "
            "after sending NBD_OPT_EXPORT_NAME. Try -g.");
    }
               ...
               char* tlshostname, bool tls, bool can_opt_go)
{
    ...
    if (tls)
    {
        ...
        if (read(sock, &tmp32, sizeof(tmp32)) < 0)
            err("Could not read option data length: %m");
        tmp32 = ntohl(tmp32);
        if (tmp32 != 0)
        {
            err("Option reply data length != 0");
        }
        s = tlssession_new(0, keyfile, certfile, cacertfile, tlshostname,
        ...
        if (set_nonblocking(plainfd[0], 0) < 0 ||
            set_nonblocking(plainfd[1], 0) < 0 || set_nonblocking(sock, 0) < 0)
        {
            close(plainfd[0]);
            close(plainfd[1]);
    ...
    if (!can_opt_go)
    {
        send_opt_exportname(sock, rsize64, flags, can_opt_go, name,
                            global_flags);
        return;
    }
                ...
                case NBD_REP_ERR_POLICY:
                    if (rep->datasize > 0)
                    {
                        ...
                        snprintf(errstr, sizeof errstr,
                                 "Connection not allowed by server policy. "
                                 "Server said: %s",
                                 rep->data);
                        err(errstr);
                    }
                ...
                default:
                    if (rep->datasize > 0)
                    {
                        ...
                        snprintf(
                            ...
                            "Unknown error returned by server. Server said: %s",
                            rep->data);
                        err(errstr);
                    }
                    else
...
bool get_from_config(char* cfgname, char** name_ptr, char** dev_ptr,
                     ...
                     int* swap, int* sdp, int* b_unix, char** port,
                     int* num_conns, char** certfile, char** keyfile,
                     char** cacertfile, char** tlshostname, bool* can_opt_go)
{
...
#define CHECK_LEN                                                              \
    field_len = strcspn(loc, fsep);                                            \
    ws_len = strspn(loc + field_len, fsep);                                    \
    if (field_len > line_len || line_len <= 0)                                 \
    {                                                                          \
        goto out;                                                              \
    }
#define MOVE_NEXT                                                              \
    line_len -= field_len + ws_len;                                            \
    loc += field_len + ws_len
    // find length of line
...
#undef MOVE_NEXT
    ...
    do
    {
        ...
        if (*loc != '_')
        {
            ...
            fprintf(stderr, "Warning: unknown option '%s' found in nbdtab file",
                    s);
            free(s);
        }
        loc += l;
...
void usage(char* errmsg, ...)
{
    ...
    fprintf(stderr, "Usage: nbd-client -name|-N name host [port] "
                    "nbd_device\n\t[-block-size|-b block size] [-timeout|-t "
                    "timeout] [-swap|-s] [-sdp|-S]\n\t[-persist|-p] "
                    "[-nofork|-n] [-systemd-mark|-m] [-nonetlink|-L]\n");
#else
    fprintf(stderr, "Usage: nbd-client -name|-N name host [port] "
                    "nbd_device\n\t[-block-size|-b block size] [-timeout|-t "
                    "timeout] [-swap|-s] [-sdp|-S]\n\t[-persist|-p] "
    ...
    fprintf(stderr,
            ...
            "certfile] [-K|-keyfile keyfile]\n\t[-A|-cacertfile cacertfile] "
            "[-H|-tlshostname hostname] [-x|-enable-tls]\n");
#endif
...
int main(int argc, char* argv[])
{
    ...
    while ((c = getopt_long_only(argc, argv, short_opts, long_options, NULL)) >=
           0)
    {
        switch (c)
        {
            case 1:
                ...
                if (strchr(optarg, '='))
                {
                    ...
                    // argument
                    fprintf(stderr, "WARNING: old-style command-line argument "
                                    "encountered. This is deprecated.\n");
...
#ifdef __ANDROID__
    if (swap)
        err("swap option unsupported on Android because mlockall is "
            "unsupported.");
#endif
    if (hostname)
    {
        if ((!name || !nbddev) && !(opts & NBDC_DO_LIST))
        {
            if (!strncmp(hostname, "nbd", 3) ||
                !strncmp(hostname, "/dev/nbd", 8))
            {
                if (!get_from_config(hostname, &name, &nbddev, &hostname,
                                     &blocksize, &timeout, &cont, &swap, &sdp,
                                     &b_unix, &port, &num_connections,
                                     &certfile, &keyfile, &cacertfile,
                                     &hostname, &can_opt_go))
                {
                    usage("no valid configuration for specified device found",
                          hostname);
                    exit(EXIT_FAILURE);
                }
            }
            ...
            {
                usage("not enough information specified, and argument didn't "
                      "look like an nbd device");
                exit(EXIT_FAILURE);
            }
        }
    }
    ...
    if (certfile != NULL || keyfile != NULL || cacertfile != NULL ||
        tlshostname != NULL)
    {
        tls = true;
    }
    ...
    if (strlen(name) == 0 && !(opts & NBDC_DO_LIST))
    {
        printf("Warning: the oldstyle protocol is no longer supported.\nThis "
               "method now uses the newstyle protocol with a default export\n");
    }
    ...
    for (i = 0; i < num_connections; i++)
    {
        ...
        negotiate(&sock, &size64, &flags, name, needed_flags, cflags, opts,
                  certfile, keyfile, cacertfile, tlshostname, tls, can_opt_go);
        if (netlink)
        {
            sockfds[i] = sock;
            continue;
    ...
    if (netlink)
    {
        ...
        netlink_configure(index, sockfds, num_connections, size64, blocksize,
                          flags, timeout);
        return 0;
    }
    /* Go daemon */
        ...
        if (ioctl(nbd, NBD_DO_IT) < 0)
        {
            ...
            else
            {
                if (cont)
                {
                    ...
                    if (nbd < 0)
                        err("Cannot open NBD: %m");
                    negotiate(&sock, &new_size, &new_flags, name, needed_flags,
                              cflags, opts, certfile, keyfile, cacertfile,
                              tlshostname, tls, can_opt_go);
                    if (size64 != new_size)
                    {
