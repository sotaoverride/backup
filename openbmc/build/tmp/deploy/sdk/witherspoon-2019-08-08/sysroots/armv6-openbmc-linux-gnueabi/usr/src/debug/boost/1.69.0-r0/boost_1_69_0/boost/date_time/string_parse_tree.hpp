...
{
    parse_match_result() : match_depth(0), current_match(PARSE_ERROR)
    {
    }
...
{
    os << "cm: " << mr.current_match << " C: '" << mr.cache
       << "' md: " << mr.match_depth << " R: " << mr.remaining();
    return os;
}
...
struct string_parse_tree
{
    ...
    void insert(const string_type& s, unsigned short value)
    {
        ...
        while (i < s.size())
        {
            if (i == 0)
            {
                if (i == (s.size() - 1))
                {
                    ti = m_next_chars.insert(
                        value_type(s[i], string_parse_tree<charT>(value)));
                }
                else
                {
                    ti = m_next_chars.insert(
                        value_type(s[i], string_parse_tree<charT>()));
                }
            }
            else
            {
                if (i == (s.size() - 1))
                {
                    ti = ti->second.m_next_chars.insert(
                        value_type(s[i], string_parse_tree<charT>(value)));
                }
                ...
                parse_match_result_type& result, unsigned int& level) const
    {
        ...
        { // equal if not found
            ...
            { // -1 is default value
                ...
                litr->second.match(sitr, stream_end, result, level);
                level--;
            }
            else
            {
    ...
    void printme(std::ostream& os, int& level)
    {
        ...
        while (itr != end)
        {
            os << "level:  " << level << " node:  " << itr->first
               << " value: " << itr->second.m_value << std::endl;
    ...
    void printmatch(std::ostream& os, charT c)
    {
        ...
        {
            os << " node:  " << litr->first
               << " value: " << litr->second.m_value << std::endl;
            litr++;
        }
    }
};
