...
extern uint64_t tuklib_physmem(void)
{
    ...
    if ((GetVersion() & 0xFF) >= 5)
    {
        ...
        if (kernel32 != NULL)
        {
            ...
            gmse_type gmse =
                (gmse_type)GetProcAddress(kernel32, "GlobalMemoryStatusEx");
            if (gmse != NULL)
            {
...
#elif defined(__OS2__)
    unsigned long mem;
    if (DosQuerySysInfo(QSV_TOTPHYSMEM, QSV_TOTPHYSMEM, &mem, sizeof(mem)) == 0)
        ret = mem;

#elif defined(__DJGPP__)
    __dpmi_free_mem_info meminfo;
    if (__dpmi_get_free_memory_information(&meminfo) == 0 &&
        meminfo.total_number_of_physical_pages != (unsigned long)-1)
    ...
    int memkb;
    int start = 0;
    if (getsysinfo(GSI_PHYSMEM, (caddr_t)&memkb, sizeof(memkb), &start) != -1)
    ...
    if (setinvent_r(&st) != -1)
    {
        ...
        {
            if (i->inv_class == INV_MEMORY && i->inv_type == INV_MAIN_MB)
            {
