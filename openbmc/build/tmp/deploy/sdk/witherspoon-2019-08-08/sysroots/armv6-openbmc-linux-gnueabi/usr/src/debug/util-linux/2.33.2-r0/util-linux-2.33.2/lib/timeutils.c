...
int get_gmtoff(const struct tm* tp)
{
    ...
    if (lt == (time_t)-1)
    {
        if (!localtime_r(&lt, &tm) ||
            ((ltm.tm_sec ^ tm.tm_sec) | (ltm.tm_min ^ tm.tm_min) |
             (ltm.tm_hour ^ tm.tm_hour) | (ltm.tm_mday ^ tm.tm_mday) |
             (ltm.tm_mon ^ tm.tm_mon) | (ltm.tm_year ^ tm.tm_year)))
            return 0;
    }
    ...
    int days =
        (365 * years + intervening_leap_days + (ltm.tm_yday - gtm.tm_yday));
...
static int format_iso_time(struct tm* tm, suseconds_t usec, int flags,
                           char* buf, size_t bufsz)
{
    ...
    if (flags & ISO_DATE)
    {
        len = snprintf(p, bufsz, "%4ld-%.2d-%.2d", tm->tm_year + (long)1900,
    ...
    if (flags & ISO_TIME)
    {
        len = snprintf(p, bufsz, "%02d:%02d:%02d", tm->tm_hour, tm->tm_min,
                       tm->tm_sec);
        if (len < 0 || (size_t)len > bufsz)
            goto err;
        bufsz -= len;
...
int strtime_short(const time_t* t, struct timeval* now, int flags, char* buf,
                  size_t bufsz)
{
    struct tm tm;
    int rc = 0;
...
int main(int argc, char* argv[])
{
    ...
    strtimeval_iso(&tv, ISO_DATE | ISO_TIME | ISO_COMMAUSEC | ISO_T, buf,
                   sizeof(buf));
    printf("Full: '%s'\n", buf);
