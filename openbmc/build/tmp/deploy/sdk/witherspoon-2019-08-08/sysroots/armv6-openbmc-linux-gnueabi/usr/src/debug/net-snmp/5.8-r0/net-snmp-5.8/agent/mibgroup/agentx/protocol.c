...
int agentx_realloc_build_int(u_char** buf, size_t* buf_len, size_t* out_len,
                             int allow_realloc, unsigned int value,
                             int network_order)
{
...
void agentx_build_int(u_char* bufp, u_int value, int network_byte_order)
{
...
int agentx_realloc_build_short(u_char** buf, size_t* buf_len, size_t* out_len,
                               int allow_realloc, unsigned short value,
                               int network_order)
{
...
int agentx_realloc_build_oid(u_char** buf, size_t* buf_len, size_t* out_len,
                             int allow_realloc, int inclusive, oid* name,
                             size_t name_len, int network_order)
{
     ...
     */
    if (name_len >= 5 && (name[0] == 1 && name[1] == 3 && name[2] == 6 &&
                          name[3] == 1 && name[4] > 0 && name[4] < 256))
    {
        prefix = name[4];
        name += 5;
        name_len -= 5;
...
int agentx_realloc_build_string(u_char** buf, size_t* buf_len, size_t* out_len,
                                int allow_realloc, u_char* string,
                                size_t string_len, int network_order)
{
    size_t ilen = *out_len, i = 0;
...
int agentx_realloc_build_double(u_char** buf, size_t* buf_len, size_t* out_len,
                                int allow_realloc, double double_val,
                                int network_order)
{
    union
    {
    ...
    return agentx_realloc_build_string(buf, buf_len, out_len, allow_realloc,
                                       opaque_buffer, 3 + sizeof(double),
                                       network_order);
}

int agentx_realloc_build_float(u_char** buf, size_t* buf_len, size_t* out_len,
                               int allow_realloc, float float_val,
                               int network_order)
{
    union
    {
    ...
    return agentx_realloc_build_string(buf, buf_len, out_len, allow_realloc,
                                       opaque_buffer, 3 + sizeof(float),
                                       network_order);
}
#endif

int agentx_realloc_build_varbind(u_char** buf, size_t* buf_len, size_t* out_len,
                                 int allow_realloc, netsnmp_variable_list* vp,
                                 int network_order)
{
    ...
    *(*buf + *out_len) = 0;
    ...
    switch (vp->type)
    {
        ...
        case ASN_OPAQUE_FLOAT:
            ...
            if (!agentx_realloc_build_float(buf, buf_len, out_len,
                                            allow_realloc, *(vp->val.floatVal),
                                            network_order))
            {
                DEBUGINDENTLESS();
                DEBUGINDENTLESS();
        ...
        case ASN_IPADDRESS:
        case ASN_OPAQUE:
            if (!agentx_realloc_build_string(buf, buf_len, out_len,
                                             allow_realloc, vp->val.string,
...
int agentx_realloc_build_header(u_char** buf, size_t* buf_len, size_t* out_len,
                                int allow_realloc, netsnmp_pdu* pdu)
{
    ...
    *(*buf + *out_len) = 0;
    ...
    if (!agentx_realloc_build_int(buf, buf_len, out_len, allow_realloc, 0,
                                  network_order))
    {
        DEBUGINDENTLESS();
        DEBUGINDENTLESS();
...
static int _agentx_realloc_build(u_char** buf, size_t* buf_len, size_t* out_len,
                                 int allow_realloc, netsnmp_session* session,
                                 netsnmp_pdu* pdu)
{
    size_t ilen = *out_len;
    ...
    switch (pdu->command)
    {
        ...
        case AGENTX_MSG_OPEN:
            ...
            *(*buf + *out_len) = (u_char)pdu->time;
            ...
            if (!agentx_realloc_build_oid(buf, buf_len, out_len, allow_realloc,
                                          0, pdu->variables->name,
        ...
        case AGENTX_MSG_GETNEXT:
            ...
            for (vp = pdu->variables; vp != NULL; vp = vp->next_variable)
            {
                ...
                if (!agentx_realloc_build_oid(buf, buf_len, out_len,
                                              allow_realloc, inc, vp->name,
        ...
        case AGENTX_MSG_RESPONSE:
            ...
            if (!agentx_realloc_build_short(
                    buf, buf_len, out_len, allow_realloc, (u_short)pdu->errstat,
                    network_order) ||
        ...
        case AGENTX_MSG_TESTSET:
            ...
            for (vp = pdu->variables; vp != NULL; vp = vp->next_variable)
            {
                if (!agentx_realloc_build_varbind(buf, buf_len, out_len,
                                                  allow_realloc, vp,
                                                  network_order))
                {
                    DEBUGINDENTLESS();
...
{
    if (session == NULL || buf_len == NULL || out_len == NULL || pdu == NULL ||
        buf == NULL)
    {
        return -1;
    }
...
int agentx_parse_int(u_char* data, u_int network_byte_order)
{
    u_int value = 0;
...
u_char* agentx_parse_oid(u_char* data, size_t* length, int* inc, oid* oid_buf,
                         size_t* oid_len, u_int network_byte_order)
{
    u_int n_subid;
    u_int prefix;
    u_int tmp_oid_len;
    ...
    *length -= 4;
    ...
    {
        DEBUGMSGTL(("agentx",
                    "Oversized Object ID (buf=%" NETSNMP_PRIz "d"
                    " pdu=%d)\n",
                    *oid_len, tmp_oid_len));
        DEBUGINDENTLESS();
        return NULL;
...
#define endianoff 0
    ...
    {
        if (int_offset == 2)
        { /* align OID values in 64 bit agent */
...
u_char* agentx_parse_string(u_char* data, size_t* length, u_char* string,
                            size_t* str_len, u_int network_byte_order)
{
    u_int len;
    ...
    {
        DEBUGMSGTL(
            ("agentx", "Incomplete string (too short: %d)\n", (int)*length));
        return NULL;
    }
    ...
    *str_len = len;
    ...
    if (*length < len + 4)
    {
        DEBUGMSGTL(
            ("agentx",
             "Packet too short for string padding (still too short: %d)\n",
             (int)*length));
        return NULL;
    }
    ...
    *length -= (len + 4);
    ...
    DEBUGIF("dumpv_recv")
    {
        ...
        if (sprint_realloc_asciistring(&buf, &buf_len, &out_len, 1, string,
                                       len))
        {
...
u_char* agentx_parse_opaque(u_char* data, size_t* length, int* type,
                            ...
                            u_int network_byte_order)
{
    ...
    u_char* buf;
#endif
    u_char* const cp = agentx_parse_string(data, length, opaque_buf, opaque_len,
                                           network_byte_order);
...
u_char* agentx_parse_varbind(u_char* data, size_t* length, int* type,
                             oid* oid_buf, size_t* oid_len, u_char* data_buf,
                             size_t* data_len, u_int network_byte_order)
{
    u_char* bufp = data;
    u_int int_val;
    ...
    *length -= 4;
    ...
    switch (*type)
    {
        ...
        case ASN_PRIV_EXCL_RANGE:
        case ASN_OBJECT_ID:
            bufp = agentx_parse_oid(bufp, length, NULL, (oid*)data_buf,
 ...
 */
u_char* agentx_parse_header(netsnmp_pdu* pdu, u_char* data, size_t* length)
{
    ...
    pdu->sessid =
        agentx_parse_int(bufp, pdu->flags & AGENTX_FLAGS_NETWORK_BYTE_ORDER);
    DEBUGINDENTLESS();
    bufp += 4;
...
int agentx_parse(netsnmp_session* session, netsnmp_pdu* pdu, u_char* data,
                 size_t len)
{
    register u_char* bufp = data;
    u_char buffer[65536];
...
#define SNMPERR_INCOMPLETE_PACKET SNMPERR_ASN_PARSE_ERR
    ...
    if (pdu->flags & AGENTX_MSG_FLAG_NON_DEFAULT_CONTEXT)
    {
        ...
        bufp =
            agentx_parse_string(bufp, length, buffer, &buf_len,
        ...
        snmp_clone_mem((void**)&pdu->community, (void*)buffer,
                       (unsigned)buf_len);
    ...
    switch (pdu->command)
    {
        case AGENTX_MSG_OPEN:
            ...
            *length -= 4;
            ...
            bufp =
                agentx_parse_oid(bufp, length, NULL, oid_buffer, &oid_buf_len,
                                 pdu->flags & AGENTX_FLAGS_NETWORK_BYTE_ORDER);
            DEBUGINDENTLESS();
            if (bufp == NULL)
            {
            ...
            snmp_pdu_add_variable(pdu, oid_buffer, oid_buf_len, ASN_OCTET_STR,
                                  buffer, buf_len);
            ...
            if (pdu->range_subid)
            {
                ...
                *length -= 4;
                ...
                snmp_pdu_add_variable(pdu, oid_buffer, oid_buf_len,
                                      ASN_PRIV_INCL_RANGE, (u_char*)end_oid_buf,
                                      end_oid_buf_len);
            }
            else
            {
        ...
        case AGENTX_MSG_GETBULK:
            ...
            pdu->non_repeaters = agentx_parse_short(
                bufp, pdu->flags & AGENTX_FLAGS_NETWORK_BYTE_ORDER);
            DEBUGINDENTLESS();
            DEBUGDUMPHEADER("recv", "Max-repeaters");
            pdu->max_repetitions = agentx_parse_short(
                bufp + 2, pdu->flags & AGENTX_FLAGS_NETWORK_BYTE_ORDER);
            DEBUGINDENTLESS();
            bufp += 4;
            *length -= 4;
            /* FALLTHROUGH */
        ...
        case AGENTX_MSG_GET:
            ...
            while (*length > 0)
            {
                bufp = agentx_parse_oid(
                    bufp, length, &inc, oid_buffer, &oid_buf_len,
                    pdu->flags & AGENTX_FLAGS_NETWORK_BYTE_ORDER);
                if (bufp == NULL)
                {
                    DEBUGINDENTLESS();
                ...
                bufp = agentx_parse_oid(
                    bufp, length, NULL, end_oid_buf, &end_oid_buf_len,
                    pdu->flags & AGENTX_FLAGS_NETWORK_BYTE_ORDER);
                if (bufp == NULL)
                {
                    DEBUGINDENTLESS();
                ...
                if (inc)
                {
                    snmp_pdu_add_variable(
                        pdu, oid_buffer, oid_buf_len, ASN_PRIV_INCL_RANGE,
                        (u_char*)end_oid_buf, end_oid_buf_len);
                }
                else
                {
                    snmp_pdu_add_variable(
                        pdu, oid_buffer, oid_buf_len, ASN_PRIV_EXCL_RANGE,
                        (u_char*)end_oid_buf, end_oid_buf_len);
                }
                oid_buf_len = MAX_OID_LEN;
        ...
        case AGENTX_MSG_RESPONSE:
             ...
             * sysUpTime
             */
            pdu->time = agentx_parse_int(
                bufp, pdu->flags & AGENTX_FLAGS_NETWORK_BYTE_ORDER);
            bufp += 4;
            *length -= 4;

            pdu->errstat = agentx_parse_short(
                bufp, pdu->flags & AGENTX_FLAGS_NETWORK_BYTE_ORDER);
            pdu->errindex = agentx_parse_short(
                bufp + 2, pdu->flags & AGENTX_FLAGS_NETWORK_BYTE_ORDER);
            bufp += 4;
            *length -= 4;
            /* FALL THROUGH */
        ...
        case AGENTX_MSG_TESTSET:
            ...
            while (*length > 0)
            {
                bufp = agentx_parse_varbind(
                    bufp, length, &type, oid_buffer, &oid_buf_len, buffer,
                    &buf_len, pdu->flags & AGENTX_FLAGS_NETWORK_BYTE_ORDER);
                if (bufp == NULL)
                {
                    DEBUGINDENTLESS();
                    DEBUGINDENTLESS();
...
testit(netsnmp_pdu* pdu1)
{
    ...
    if (len2 != 0)
        DEBUGMSGTL(
            ("agentx", "Warning - parsed packet has %d bytes left\n", len2));
    ...
    {
        DEBUGMSGTL(
            ("agentx", "Error: first build data is different to second\n"));
        exit(1);
    }
