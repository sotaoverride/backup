...
static int manager_process_link(sd_netlink* rtnl, sd_netlink_message* mm,
                                void* userdata)
{
    Manager* m = userdata;
    uint16_t type;
...
static int manager_process_address(sd_netlink* rtnl, sd_netlink_message* mm,
                                   void* userdata)
{
    Manager* m = userdata;
    ...
    switch (family)
    {
        ...
        case AF_INET:
            ...
            if (r < 0)
            {
                r = sd_netlink_message_read_in_addr(mm, IFA_ADDRESS,
                                                    &address.in);
                if (r < 0)
                    goto fail;
            }
        ...
        case AF_INET6:
            ...
            if (r < 0)
            {
                r = sd_netlink_message_read_in6_addr(mm, IFA_ADDRESS,
                                                     &address.in6);
                if (r < 0)
                    goto fail;
            }
...
static int manager_rtnl_listen(Manager* m)
{
    ...
    r = sd_netlink_add_match(m->rtnl, NULL, RTM_NEWLINK, manager_process_link,
                             NULL, m, "resolve-NEWLINK");
    if (r < 0)
        return r;

    r = sd_netlink_add_match(m->rtnl, NULL, RTM_DELLINK, manager_process_link,
                             NULL, m, "resolve-DELLINK");
    if (r < 0)
        return r;

    r = sd_netlink_add_match(m->rtnl, NULL, RTM_NEWADDR,
                             manager_process_address, NULL, m,
                             "resolve-NEWADDR");
    if (r < 0)
        return r;
...
static int on_network_event(sd_event_source* s, int fd, uint32_t revents,
                            void* userdata)
{
    Manager* m = userdata;
    Iterator i;
    ...
    HASHMAP_FOREACH(l, m->links, i)
    {
        ...
        if (r < 0)
            log_warning_errno(r,
                              "Failed to update monitor information for %i: %m",
                              l->ifindex);
    }
...
static int manager_network_monitor_listen(Manager* m)
{
    ...
    r = sd_event_add_io(m->event, &m->network_event_source, fd, events,
                        &on_network_event, m);
    if (r < 0)
        return r;

    r = sd_event_source_set_priority(m->network_event_source,
                                     SD_EVENT_PRIORITY_IMPORTANT + 5);
    if (r < 0)
        return r;

    (void)sd_event_source_set_description(m->network_event_source,
                                          "network-monitor");

    return 0;
}

static int determine_hostname(char** full_hostname, char** llmnr_hostname,
                              char** mdns_hostname)
{
    ...
    /* Extract and normalize the first label of the locally configured hostname,
     * and check it's not "localhost". */
...
static const char* fallback_hostname(void)
{
    ...
    /* Determine the fall back hostname. For exposing this system to the outside
     * world, we cannot have it to be
     * "localhost" even if that's the compiled in hostname. In this case, let's
     * revert to "linux" instead. */
...
static int make_fallback_hostnames(char** full_hostname, char** llmnr_hostname,
                                   char** mdns_hostname)
{
...
static int on_hostname_change(sd_event_source* es, int fd, uint32_t revents,
                              void* userdata)
{
    _cleanup_free_ char *full_hostname = NULL, *llmnr_hostname = NULL,
                        *mdns_hostname = NULL;
    Manager* m = userdata;
    int r;
...
static int manager_watch_hostname(Manager* m)
{
    ...
    r = sd_event_add_io(m->event, &m->hostname_event_source, m->hostname_fd, 0,
                        on_hostname_change, m);
    if (r < 0)
    {
        if (r == -EPERM)
            /* kernels prior to 3.2 don't support polling this file. Ignore the
             * failure. */
    ...
    r = determine_hostname(&m->full_hostname, &m->llmnr_hostname,
                           &m->mdns_hostname);
    if (r < 0)
    {
        log_info("Defaulting to hostname '%s'.", fallback_hostname());

        r = make_fallback_hostnames(&m->full_hostname, &m->llmnr_hostname,
                                    &m->mdns_hostname);
        if (r < 0)
            return r;
    }
...
static int manager_sigusr1(sd_event_source* s,
...
static int manager_sigusr2(sd_event_source* s,
                           const struct signalfd_siginfo* si, void* userdata)
{
    Manager* m = userdata;
...
static int manager_sigrtmin1(sd_event_source* s,
                             const struct signalfd_siginfo* si, void* userdata)
{
    Manager* m = userdata;
...
int manager_new(Manager** ret)
{
    ...
    *m = (Manager){
    ...
    (void)sd_event_add_signal(m->event, &m->sigusr1_event_source, SIGUSR1,
                              manager_sigusr1, m);
    (void)sd_event_add_signal(m->event, &m->sigusr2_event_source, SIGUSR2,
                              manager_sigusr2, m);
    (void)sd_event_add_signal(m->event, &m->sigrtmin1_event_source,
                              SIGRTMIN + 1, manager_sigrtmin1, m);
...
Manager* manager_free(Manager* m)
{
    ...
    /* At this point only orphaned streams should remain. All others should have
     * been freed already by their owners */
    while (m->dns_streams)
...
int manager_recv(Manager* m, int fd, DnsProtocol protocol, DnsPacket** ret)
{
    ...
    union
    {
        ...
        uint8_t
            buffer[CMSG_SPACE(MAXSIZE(struct in_pktinfo, struct in6_pktinfo)) +
                   CMSG_SPACE(int)    /* ttl/hoplimit */
                   + EXTRA_CMSG_SPACE /* kernel appears to require extra buffer
                                         space */
        ];
    } control;
...
int manager_write(Manager* m, int fd, DnsPacket* p)
{
    ...
    log_debug("Sending %s packet with id %" PRIu16 ".",
              DNS_PACKET_QR(p) ? "response" : "query", DNS_PACKET_ID(p));
...
static int manager_ipv4_send(Manager* m, int fd, int ifindex,
                             const struct in_addr* destination, uint16_t port,
                             const struct in_addr* source, DnsPacket* p)
{
    union
    {
        struct cmsghdr header; /* For alignment */
...
static int manager_ipv6_send(Manager* m, int fd, int ifindex,
                             const struct in6_addr* destination, uint16_t port,
                             const struct in6_addr* source, DnsPacket* p)
{
...
int manager_send(Manager* m, int fd, int ifindex, int family,
                 const union in_addr_union* destination, uint16_t port,
                 const union in_addr_union* source, DnsPacket* p)
{
    ...
    if (family == AF_INET)
        return manager_ipv4_send(m, fd, ifindex, &destination->in, port,
                                 source ? &source->in : NULL, p);
    if (family == AF_INET6)
        return manager_ipv6_send(m, fd, ifindex, &destination->in6, port,
                                 source ? &source->in6 : NULL, p);
...
int manager_find_ifindex(Manager* m, int family,
...
int manager_next_hostname(Manager* m)
{
    ...
    log_info(
        "Hostname conflict, changing published hostname from '%s' to '%s'.",
        m->llmnr_hostname, h);
...
LinkAddress* manager_find_link_address(Manager* m, int family,
                                       const union in_addr_union* in_addr)
{
    Iterator i;
    Link* l;
...
void manager_dnssec_verdict(Manager* m, DnssecVerdict verdict,
...
bool manager_routable(Manager* m, int family)
{
    ...
    /* Returns true if the host has at least one interface with a routable
...
void manager_cleanup_saved_user(Manager* m)
{
    ...
    /* Clean up all saved per-link files in /run/systemd/resolve/netif/ that
     * don't have a matching interface anymore. These files are created to
     * persist settings pushed in by the user via the bus, so that resolved can
    ...
    FOREACH_DIRENT_ALL(
        de, d, log_error_errno(errno, "Failed to read interface directory: %m"))
    {
        ...
        if (l->is_managed) /* now managed by networkd, hence the bus settings
                              are useless */
            goto rm;
...
bool manager_next_dnssd_names(Manager* m)
{
    ...
    HASHMAP_FOREACH(s, m->dnssd_services, i)
    {
        ...
        if (r < 0)
        {
            log_warning_errno(r, "Failed to get new name for service '%s': %m",
                              s->name);
            continue;
        }
