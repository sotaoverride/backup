...
#define log_ipv4acd_errno(acd, error, fmt, ...)                                \
    log_internal(LOG_DEBUG, error, __FILE__, __LINE__, __func__,               \
                 "IPV4ACD: " fmt, ##__VA_ARGS__)
#define log_ipv4acd(acd, fmt, ...) log_ipv4acd_errno(acd, 0, fmt, ##__VA_ARGS__)

static void ipv4acd_set_state(sd_ipv4acd* acd, IPv4ACDState st,
                              bool reset_counter)
{
    assert(acd);
...
static int ipv4acd_on_timeout(sd_event_source* s, uint64_t usec,
                              void* userdata);

static int ipv4acd_set_next_wakeup(sd_ipv4acd* acd, usec_t usec,
                                   usec_t random_usec)
{
    ...
    assert_se(sd_event_now(acd->event, clock_boottime_or_monotonic(),
                           &time_now) >= 0);

    return event_reset_time(acd->event, &acd->timer_event_source,
                            clock_boottime_or_monotonic(),
                            time_now + next_timeout, 0, ipv4acd_on_timeout, acd,
...
static int ipv4acd_on_timeout(sd_event_source* s, uint64_t usec, void* userdata)
{
    ...
    switch (acd->state)
    {
            ...
            if (acd->n_conflict >= MAX_CONFLICTS)
            {
                ...
                log_ipv4acd(acd, "Max conflicts reached, delaying by %s",
                            format_timespan(ts, sizeof(ts),
                                            RATE_LIMIT_INTERVAL_USEC, 0));

                r = ipv4acd_set_next_wakeup(acd, RATE_LIMIT_INTERVAL_USEC,
                                            PROBE_WAIT_USEC);
                if (r < 0)
                    goto fail;
            }
        ...
        case IPV4ACD_STATE_PROBING:
            /* Send a probe */
            r = arp_send_probe(acd->fd, acd->ifindex, acd->address,
                               &acd->mac_addr);
            if (r < 0)
            {
            ...
            if (acd->n_iteration < PROBE_NUM - 2)
            {
                ...
                r = ipv4acd_set_next_wakeup(acd, PROBE_MIN_USEC,
                                            (PROBE_MAX_USEC - PROBE_MIN_USEC));
                if (r < 0)
                    goto fail;
            }
            else
        ...
        case IPV4ACD_STATE_WAITING_ANNOUNCE:
            ...
            r = arp_send_announcement(acd->fd, acd->ifindex, acd->address,
                                      &acd->mac_addr);
            if (r < 0)
            {
                log_ipv4acd_errno(acd, r,
...
static int ipv4acd_on_packet(sd_event_source* s, int fd, uint32_t revents,
                             void* userdata)
{
    ...
    switch (acd->state)
    {
        ...
        case IPV4ACD_STATE_RUNNING:
            ...
            if (ipv4acd_arp_conflict(acd, &packet))
            {
                ...
                assert_se(sd_event_now(acd->event,
                                       clock_boottime_or_monotonic(),
                                       &ts) >= 0);
                ...
                if (ts > acd->defend_window)
                {
                    ...
                    r = arp_send_announcement(acd->fd, acd->ifindex,
                                              acd->address, &acd->mac_addr);
                    if (r < 0)
                    {
                        log_ipv4acd_errno(
                            acd, r, "Failed to send ARP announcement: %m");
                        goto fail;
                    }
                    else
...
int sd_ipv4acd_set_callback(sd_ipv4acd* acd, sd_ipv4acd_callback_t cb,
                            void* userdata)
{
    assert_return(acd, -EINVAL);
...
int sd_ipv4acd_start(sd_ipv4acd* acd)
{
    ...
    r = sd_event_add_io(acd->event, &acd->receive_message_event_source, acd->fd,
                        EPOLLIN, ipv4acd_on_packet, acd);
    if (r < 0)
        goto fail;

    r = sd_event_source_set_priority(acd->receive_message_event_source,
                                     acd->event_priority);
    if (r < 0)
        goto fail;

    (void)sd_event_source_set_description(acd->receive_message_event_source,
