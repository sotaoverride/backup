 ...
 */
static inline const gchar* const* _g_win32_get_system_data_dirs(void)
{
    return g_win32_get_system_data_dirs_for_module(
        (void (*)(void)) & _g_win32_get_system_data_dirs);
}
 ...
 */
    ...
    (defined(_MSC_VER) && !defined(_INC_STDLIB))
...
static inline guint g_bit_storage_impl(gulong number)
{
    ...
    return G_LIKELY(number)
               ? ((GLIB_SIZEOF_LONG * 8U - 1) ^ (guint)__builtin_clzl(number)) +
                     1
               : 1;
#else
    guint n_bits = 0;
    ...
    {                                                                          \
        ...
        {                                                                      \
            case DLL_PROCESS_ATTACH:                                           \
                GetModuleFileNameW((HMODULE)hinstDLL, wcbfr,                   \
                                   G_N_ELEMENTS(wcbfr));                       \
