...
#define PTHREAD_MUTEX_TYPE(m) (atomic_load_relaxed(&((m)->__data.__kind)) & 127)
...
extern int __pthread_tpp_change_priority(int prev_prio,
                                         int new_prio) attribute_hidden;
...
extern void
    __pthread_unwind(__pthread_unwind_buf_t* __buf) __cleanup_fct_attribute
...
extern void __pthread_unregister_cancel(__pthread_unwind_buf_t* __buf)
    __cleanup_fct_attribute;
#if IS_IN(libpthread)
hidden_proto(__pthread_unwind) hidden_proto(__pthread_unwind_next)
...
static inline void __attribute((noreturn, always_inline)) __do_cancel(void)
{
...
#elif IS_IN(librt)
#define LIBC_CANCEL_ASYNC() __librt_enable_asynccancel()
#define LIBC_CANCEL_RESET(val) __librt_disable_asynccancel(val)
#else
...
extern struct pthread*
    __find_in_stack_list(struct pthread* pd) attribute_hidden;
...
hidden_proto(__nptl_create_event) hidden_proto(__nptl_death_event)
...
extern int __pthread_attr_getdetachstate(const pthread_attr_t* attr,
                                         int* detachstate);
extern int __pthread_attr_setdetachstate(pthread_attr_t* attr, int detachstate);
...
extern int __pthread_attr_getstackaddr(const pthread_attr_t* __restrict __attr,
...
hidden_proto(__pthread_mutex_init) hidden_proto(__pthread_mutex_destroy)
    hidden_proto(__pthread_mutex_lock) hidden_proto(__pthread_mutex_trylock)
        hidden_proto(__pthread_mutex_unlock) hidden_proto(
            __pthread_rwlock_rdlock) hidden_proto(__pthread_rwlock_wrlock)
            hidden_proto(__pthread_rwlock_unlock) hidden_proto(
                __pthread_key_create) hidden_proto(__pthread_getspecific)
...
extern void __nptl_setxid_error(struct xid_command* cmdp,
                                int error) attribute_hidden;
