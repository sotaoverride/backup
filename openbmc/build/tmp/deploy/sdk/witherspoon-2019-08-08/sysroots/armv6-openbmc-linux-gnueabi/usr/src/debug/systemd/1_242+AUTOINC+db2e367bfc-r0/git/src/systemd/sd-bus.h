...
enum
{
    ...
    SD_BUS_CREDS_AUGMENT =
        1ULL << 63, /* special flag, if on sd-bus will augment creds struct, in
                       a potentially race-full way. */
...
typedef int (*sd_bus_property_set_t)(sd_bus* bus, const char* path,
                                     ...
                                     sd_bus_error* ret_error);
typedef int (*sd_bus_object_find_t)(sd_bus* bus, const char* path,
                                    const char* interface, void* userdata,
                                    void** ret_found, sd_bus_error* ret_error);
typedef int (*sd_bus_node_enumerator_t)(sd_bus* bus, const char* prefix,
...
int sd_bus_get_owner_creds(sd_bus* bus, uint64_t creds_mask,
                           sd_bus_creds** ret);
...
int sd_bus_slot_get_destroy_callback(sd_bus_slot* s,
...
sd_bus_creds*
    sd_bus_message_get_creds(sd_bus_message* m); /* do not unref the result */
...
int sd_bus_message_set_allow_interactive_authorization(sd_bus_message* m,
                                                       int b);
...
int sd_bus_message_append_array(sd_bus_message* m, char type, const void* ptr,
                                size_t size);
...
int sd_bus_get_name_creds(sd_bus* bus, const char* name, uint64_t mask,
                          sd_bus_creds** creds); /* unref the result! */
...
int sd_bus_reply_method_errorf(sd_bus_message* call, const char* name,
                               const char* format, ...) _sd_printf_(3, 4);
...
int sd_bus_emit_interfaces_added(sd_bus* bus, const char* path,
                                 const char* interface, ...) _sd_sentinel_;
...
#define SD_BUS_ERROR_MAKE_CONST(name, message)                                 \
    ((const sd_bus_error){(name), (message), 0})
...
#define SD_BUS_MESSAGE_APPEND_ID128(x)                                         \
    16, (x).bytes[0], (x).bytes[1], (x).bytes[2], (x).bytes[3], (x).bytes[4],  \
        (x).bytes[5], (x).bytes[6], (x).bytes[7], (x).bytes[8], (x).bytes[9],  \
        (x).bytes[10], (x).bytes[11], (x).bytes[12], (x).bytes[13],            \
        (x).bytes[14], (x).bytes[15]

#define SD_BUS_MESSAGE_READ_ID128(x)                                           \
    16, &(x).bytes[0], &(x).bytes[1], &(x).bytes[2], &(x).bytes[3],            \
...
int sd_bus_track_set_destroy_callback(sd_bus_track* s,
                                      sd_bus_destroy_t callback);
int sd_bus_track_get_destroy_callback(sd_bus_track* s, sd_bus_destroy_t* ret);

/* Define helpers so that __attribute__((cleanup(sd_bus_unrefp))) and similar
 * may be used. */
