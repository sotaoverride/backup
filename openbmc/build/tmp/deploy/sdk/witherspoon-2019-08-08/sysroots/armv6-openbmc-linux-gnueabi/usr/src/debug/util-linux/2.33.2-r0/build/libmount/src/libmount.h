...
#define MNT_NOHLPS                                                             \
    (1 << 4) /* don't add the option to mount.<type> helpers command line */
...
/* utils.c */
extern char* mnt_mangle(const char* str) __ul_attribute__((warn_unused_result));
...
extern int mnt_split_optstr(const char* optstr, char** user, char** vfs,
...
extern int mnt_table_set_intro_comment(struct libmnt_table* tb,
                                       const char* comm);
extern int mnt_table_append_intro_comment(struct libmnt_table* tb,
                                          const char* comm);
extern int mnt_table_set_trailing_comment(struct libmnt_table* tb,
                                          const char* comm);
extern const char* mnt_table_get_trailing_comment(struct libmnt_table* tb);
extern int mnt_table_append_trailing_comment(struct libmnt_table* tb,
                                             const char* comm);

extern int mnt_table_set_cache(struct libmnt_table* tb,
...
extern int mnt_table_next_fs(struct libmnt_table* tb, struct libmnt_iter* itr,
                             struct libmnt_fs** fs);
extern int mnt_table_next_child_fs(struct libmnt_table* tb,
                                   struct libmnt_iter* itr,
                                   struct libmnt_fs* parent,
                                   struct libmnt_fs** chld);
extern int mnt_table_get_root_fs(struct libmnt_table* tb,
...
extern struct libmnt_fs* mnt_table_find_srcpath(struct libmnt_table* tb,
                                                const char* path,
                                                int direction);
extern struct libmnt_fs* mnt_table_find_tag(struct libmnt_table* tb,
                                            const char* tag, const char* val,
                                            int direction);
extern struct libmnt_fs*
    mnt_table_find_target_with_option(struct libmnt_table* tb, const char* path,
...
extern int mnt_table_is_fs_mounted(struct libmnt_table* tb,
...
extern int mnt_table_replace_file(struct libmnt_table* tb,
                                  const char* filename);
extern int mnt_table_write_file(struct libmnt_table* tb, FILE* file);

extern int mnt_update_is_ready(struct libmnt_update* upd);
extern int mnt_update_set_fs(struct libmnt_update* upd,
                             unsigned long mountflags, const char* target,
                             struct libmnt_fs* fs);
...
extern int mnt_tabdiff_next_change(struct libmnt_tabdiff* df,
                                   ...
                                   struct libmnt_fs** new_fs, int* oper);
...
extern int mnt_monitor_enable_userspace(struct libmnt_monitor* mn, int enable,
                                        const char* filename);
...
enum
{
    ...
    MNT_OMODE_APPEND =
        (1 << 2), /* append mtab/fstab options to existing options */
    MNT_OMODE_PREPEND =
        (1 << 3), /* prepend mtab/fstab options to existing options */
    MNT_OMODE_REPLACE =
        (1 << 4), /* replace existing options with options from mtab/fstab */

    MNT_OMODE_FORCE = (1 << 5), /* always read mtab/fstab options */

    MNT_OMODE_FSTAB = (1 << 10), /* read from fstab */
    MNT_OMODE_MTAB =
        (1 << 11), /* read from mtab if fstab not enabled or failed */
...
extern int mnt_context_init_helper(struct libmnt_context* cxt, int action,
                                   int flags);
extern int mnt_context_helper_setopt(struct libmnt_context* cxt, int c,
                                     char* arg);

extern int mnt_context_set_optsmode(struct libmnt_context* cxt, int mode);
extern int mnt_context_disable_canonicalize(struct libmnt_context* cxt,
                                            int disable);
extern int mnt_context_enable_lazy(struct libmnt_context* cxt, int enable);
extern int mnt_context_enable_rdonly_umount(struct libmnt_context* cxt,
                                            int enable);
extern int mnt_context_enable_rwonly_mount(struct libmnt_context* cxt,
                                           int enable);
...
extern int mnt_context_disable_swapmatch(struct libmnt_context* cxt,
                                         int disable);
...
extern int mnt_context_set_source(struct libmnt_context* cxt,
                                  const char* source);
extern int mnt_context_set_target(struct libmnt_context* cxt,
                                  const char* target);
extern int mnt_context_set_fstype(struct libmnt_context* cxt,
                                  const char* fstype);
...
extern int mnt_context_set_options(struct libmnt_context* cxt,
                                   const char* optstr);
...
extern int mnt_context_set_passwd_cb(struct libmnt_context* cxt,
                                     ...
                                     void (*release)(struct libmnt_context*,
                                                     char*))
...
extern int mnt_context_strerror(struct libmnt_context* cxt, char* buf,
                                size_t bufsiz) __ul_attribute__((deprecated));

extern int mnt_context_get_excode(struct libmnt_context* cxt, int rc, char* buf,
                                  size_t bufsz);

extern int mnt_context_set_target_ns(struct libmnt_context* cxt,
                                     const char* path);
...
extern struct libmnt_ns* mnt_context_switch_ns(struct libmnt_context* cxt,
                                               struct libmnt_ns* ns);
extern struct libmnt_ns*
...
extern int mnt_context_find_umount_fs(struct libmnt_context* cxt,
                                      const char* tgt, struct libmnt_fs** pfs);
...
extern int mnt_context_set_syscall_status(struct libmnt_context* cxt,
                                          int status);
...
#ifndef MS_RELATIME
#define MS_RELATIME                                                            \
    (1 << 21) /* 2097152: Update atime relative to mtime/ctime */
#endif
#ifndef MS_I_VERSION
