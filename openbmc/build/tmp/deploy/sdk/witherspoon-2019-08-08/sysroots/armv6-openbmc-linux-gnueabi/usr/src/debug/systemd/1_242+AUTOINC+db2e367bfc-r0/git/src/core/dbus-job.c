...
static int property_get_unit(sd_bus* bus, const char* path,
...
int bus_job_method_cancel(sd_bus_message* message, void* userdata,
                          sd_bus_error* error)
{
    Job* j = userdata;
    int r;
                               ...
                               sd_bus_message_get_sender(message)))
    {
        ...
        if (r < 0)
            return r;
        if (r == 0)
            return 1; /* No authorization for now, but the async polkit stuff
                         will call us again when it has it */
    }
...
int bus_job_method_get_waiting_jobs(sd_bus_message* message, void* userdata,
                                    sd_bus_error* error)
{
    ...
    for (i = 0; i < n; i++)
    {
        ...
        r = sd_bus_message_append(
            reply, "(usssoo)", list[i]->id, list[i]->unit->id,
            job_type_to_string(list[i]->type),
            job_state_to_string(list[i]->state), job_path, unit_path);
        if (r < 0)
            return r;
    }
...
const sd_bus_vtable bus_job_vtable[] = {
    ...
    SD_BUS_METHOD("GetAfter", NULL, "a(usssoo)",
                  bus_job_method_get_waiting_jobs, SD_BUS_VTABLE_UNPRIVILEGED),
    SD_BUS_METHOD("GetBefore", NULL, "a(usssoo)",
                  bus_job_method_get_waiting_jobs, SD_BUS_VTABLE_UNPRIVILEGED),
...
static int send_changed_signal(sd_bus* bus, void* userdata)
{
    ...
    return sd_bus_emit_properties_changed(
        bus, p, "org.freedesktop.systemd1.Job", "State", NULL);
}
...
void bus_job_send_change_signal(Job* j)
{
    ...
    /* Make sure that any change signal on the unit is reflected before we send
     * out the change signal on the job */
    ...
    r = bus_foreach_bus(
        j->manager, j->bus_track,
        j->sent_dbus_new_signal ? send_changed_signal : send_new_signal, j);
    if (r < 0)
        log_debug_errno(r, "Failed to send job change signal for %u: %m",
                        j->id);
...
void bus_job_send_removed_signal(Job* j)
{
    ...
    if (r < 0)
        log_debug_errno(r, "Failed to send job remove signal for %u: %m",
                        j->id);
}
...
static int bus_job_track_handler(sd_bus_track* t, void* userdata)
{
    ...
    j->bus_track =
        sd_bus_track_unref(j->bus_track); /* make sure we aren't called again */
...
static int bus_job_allocate_bus_track(Job* j)
{
    ...
    return sd_bus_track_new(j->unit->manager->api_bus, &j->bus_track,
                            bus_job_track_handler, j);
}
