...
struct mdp1_super_block
{
    ...
    uint32_t raid_disks;
    uint32_t
        bitmap_offset; /* sectors after start of superblock that bitmap starts
    ...
    uint64_t recovery_offset; /* sectors before this offset (from data_offset)
                                 have been recovered */
    uint32_t dev_number; /* permanent identifier of this  device - not role in
                            raid */
    uint32_t cnt_corrected_read; /* number of read errors that were corrected by
                                    re-writing */
    ...
    uint64_t resync_offset; /* data before this offset (from data_offset) known
                               to be in sync */
     ...
     */
    uint16_t dev_roles[0]; /* role in array, or 0xffff for a spare, or 0xfffe
                              for faulty */
};
...
static int probe_raid0(blkid_probe pr, uint64_t off)
{
    ...
    if (pr->size < MD_RESERVED_BYTES)
        return 1;
    mdp0 = (struct mdp0_super_block*)blkid_probe_get_buffer(
        pr, off, sizeof(struct mdp0_super_block));
    if (!mdp0)
    ...
    if ((S_ISREG(pr->mode) || blkid_probe_is_wholedisk(pr)) &&
        blkid_probe_is_covered_by_pt(pr, off - size, /* min. start  */
                                     size + MD_RESERVED_BYTES))
    { /* min. length */
...
static int probe_raid1(blkid_probe pr, off_t off)
{
    ...
    mdp1 = (struct mdp1_super_block*)blkid_probe_get_buffer(
        pr, off, sizeof(struct mdp1_super_block));
    if (!mdp1)
        return errno ? -errno : 1;
    ...
    if (blkid_probe_set_uuid(pr, (unsigned char*)mdp1->set_uuid) != 0)
        return 1;
    if (blkid_probe_set_uuid_as(pr, (unsigned char*)mdp1->device_uuid,
                                "UUID_SUB") != 0)
        return 1;
                      ...
                      const struct blkid_idmag* mag __attribute__((__unused__)))
{
    ...
    if (pr->size > MD_RESERVED_BYTES)
    {
        ...
        uint64_t sboff =
            (pr->size & ~(MD_RESERVED_BYTES - 1)) - MD_RESERVED_BYTES;
...
const struct blkid_idinfo linuxraid_idinfo = {.name = "linux_raid_member",
