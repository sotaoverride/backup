...
#define SUBTREE_MAX_CACHE_SIZE 32
        int lookup_cache_size =
            0; /*enabled later after registrations are loaded */
 ...
 *  @param previous Previous subtree item.
 */
NETSNMP_STATIC_INLINE void lookup_cache_replace(lookup_cache* ptr,
                                                netsnmp_subtree* next,
                                                netsnmp_subtree* previous)
{
...
/** @defgroup agent_context_cache Context cache, storing the OIDs under their
 * contexts. Maintain the cache used for locating sub-trees registered under
 * different contexts.
 *   @ingroup agent_registry
 *
 * @{
                             ...
                             const char* context_name)
{
    ...
    DEBUGMSGTL(
        ("subtree", "adding subtree for context: \"%s\"\n", context_name));
...
void netsnmp_subtree_free(netsnmp_subtree* a)
{
    ...
    {
        if (a->variables != NULL &&
            netsnmp_oid_equals(a->name_a, a->namelen, a->start_a,
                               a->start_len) == 0)
        {
...
netsnmp_subtree* netsnmp_subtree_deepcopy(netsnmp_subtree* a)
{
    ...
    if (b != NULL)
    {
        ...
        if (b->name_a == NULL || b->start_a == NULL || b->end_a == NULL ||
            b->label_a == NULL)
        {
        ...
        {
            b->variables =
                (struct variable*)malloc(a->variables_len * a->variables_width);
            if (b->variables != NULL)
            {
                memcpy(b->variables, a->variables,
                       a->variables_len * a->variables_width);
            }
            else
            {
                netsnmp_subtree_free(b);
 ...
 */
NETSNMP_STATIC_INLINE void netsnmp_subtree_change_next(netsnmp_subtree* ptr,
                                                       netsnmp_subtree* thenext)
{
    ptr->next = thenext;
    if (thenext)
        netsnmp_oid_compare_ll(ptr->start_a, ptr->start_len, thenext->start_a,
                               thenext->start_len, &thenext->oid_off);
}

/** @private
 *  Replaces previous subtree pointer in given subtree.
 */
NETSNMP_STATIC_INLINE void netsnmp_subtree_change_prev(netsnmp_subtree* ptr,
                                                       netsnmp_subtree* theprev)
{
    ptr->prev = theprev;
    if (theprev)
        netsnmp_oid_compare_ll(theprev->start_a, theprev->start_len,
                               ptr->start_a, ptr->start_len, &ptr->oid_off);
}
...
netsnmp_feature_child_of(netsnmp_subtree_compare, netsnmp_unused)
     ...
     *  @see snmp_oid_compare()
     */
    int netsnmp_subtree_compare(const netsnmp_subtree* ap,
                                const netsnmp_subtree* bp)
{
 ...
 */
netsnmp_subtree* netsnmp_subtree_split(netsnmp_subtree* current, oid name[],
                                       int name_len)
{
    ...
    for (vp = current->variables; i > 0; i--)
    {
        ...
        rc = snmp_oid_compare(vp->name, vp->namelen, name + current->namelen,
        ...
        rc2 = snmp_oid_compare(vp->name, common_len, name + current->namelen,
                               common_len);
        ...
        if (rc2 < 0)
        {
            ...
            new_sub->variables =
                (struct variable*)(cp + new_sub->variables_width);
        }
    ...
    {
        new_sub->children =
            netsnmp_subtree_split(current->children, name, name_len);
    }
...
int netsnmp_subtree_load(netsnmp_subtree* new_sub, const char* context_name)
{
    ...
    if (!netsnmp_subtree_find_first(context_name))
    {
        ...
        if (!inloop)
        {
            ...
            netsnmp_register_null_context(
                snmp_duplicate_objid(joint_ccitt_iso, 1), 1, context_name);
            inloop = 0;
        }
    }
    ...
    tree1 = netsnmp_subtree_find(new_sub->start_a, new_sub->start_len, NULL,
                                 context_name);
    ...
    if (tree1 == NULL)
    {
        ...
        else
        {
            netsnmp_subtree_change_prev(
                new_sub,
                netsnmp_subtree_find_prev(new_sub->start_a, new_sub->start_len,
                                          NULL, context_name));
    ...
    else
    {
        ...
        switch (snmp_oid_compare(new_sub->end_a, new_sub->end_len, tree1->end_a,
                                 tree1->end_len))
        {
            ...
            case 0:
                ...
                if (next && (next->namelen == new_sub->namelen) &&
                    (next->priority == new_sub->priority))
                {
                    if (new_sub->namelen != 1)
                    { /* ignore root OID dups */
                        size_t out_len = 0;
                        size_t buf_len = 0;
                        char* buf = NULL;
                        int buf_overflow = 0;

                        netsnmp_sprint_realloc_objid((u_char**)&buf, &buf_len,
                                                     &out_len, 1, &buf_overflow,
                                                     new_sub->start_a,
                                                     new_sub->start_len);
                        snmp_log(LOG_ERR,
                                 "duplicate registration: MIB modules %s and "
                                 "%s (oid %s%s).\n",
                ...
                else
                {
                    ...
                    for (next = new_sub->next; next != NULL;
                         next = next->children)
                    {
                        netsnmp_subtree_change_prev(next, new_sub);
                    }

                    for (prev = new_sub->prev; prev != NULL;
                         prev = prev->children)
                    {
            ...
            case 1:
                ...
                {
                    netsnmp_subtree* new2 = netsnmp_subtree_split(
                        new_sub, tree1->end_a, tree1->end_len);
...
netsnmp_subtree* netsnmp_subtree_find_prev(const oid* name, size_t len,
...
#define WTEST_OPTIMIZATION 1
    ...
    for (; myptr != NULL; previous = myptr, myptr = myptr->next)
    {
        ...
        if (!(ll_off && myptr->oid_off && myptr->oid_off > ll_off) &&
            netsnmp_oid_compare_ll(name, len, myptr->start_a, myptr->start_len,
                                   &ll_off) < 0)
        {
#else
...
netsnmp_subtree* netsnmp_subtree_find_next(const oid* name, size_t len,
                                           netsnmp_subtree* subtree,
                                           const char* context_name)
{
    ...
    if (myptr != NULL)
    {
        myptr = myptr->next;
        while (myptr != NULL &&
               (myptr->variables == NULL || myptr->variables_len == 0))
        {
            myptr = myptr->next;
        }
        return myptr;
    }
    else if (subtree != NULL && snmp_oid_compare(name, len, subtree->start_a,
                                                 subtree->start_len) < 0)
    {
        return subtree;
    }
...
netsnmp_subtree* netsnmp_subtree_find(const oid* name, size_t len,
                                      netsnmp_subtree* subtree,
                                      const char* context_name)
{
 ...
 */
int netsnmp_register_mib(const char* moduleName, struct variable* var,
                         size_t varsize, size_t numvars, oid* mibloc,
                         size_t mibloclen, int priority, int range_subid,
                         oid range_ubound, netsnmp_session* ss,
                         ...
                         int perform_callback)
{
    ...
    DEBUGMSGOIDRANGE(
        ("register_mib", mibloc, mibloclen, range_subid, range_ubound));
    DEBUGMSG(
        ("register_mib", " with context \"%s\"\n", SNMP_STRORNULL(context)));
         ...
         (0 != strcmp(context, reginfo->contextName))))
    {
        snmp_log(LOG_WARNING,
                 "context passed during registration does not "
                 "equal the reginfo contextName! ('%s' != '%s')\n",
                 context, reginfo->contextName);
        netsnmp_assert(
            !"register context == reginfo->contextName"); /* always false */
    }
    ...
    if (res == MIB_REGISTERED_OK && range_subid != 0)
    {
        ...
        for (i = mibloc[range_subid - 1] + 1; i <= (int)range_ubound; i++)
        {
            ...
            if (sub2 == NULL)
            {
                unregister_mib_context(mibloc, mibloclen, priority, range_subid,
                                       range_ubound, context);
 ...
 */
static void register_mib_reattach_node(netsnmp_subtree* s)
{
...
/** Register a new module into the MIB database, with all possible custom
 * options
 *
 ...
 *                     This is how many variables the function will try to
 * register.
 *
 ...
 *                     If zero, then register the module against the full OID
 * subtree.
 *
 ...
 *  @see unregister_mib()
 */
int register_mib_context(const char* moduleName, const struct variable* var,
                         size_t varsize, size_t numvars, const oid* mibloc,
                         size_t mibloclen, int priority, int range_subid,
                         oid range_ubound, netsnmp_session* ss,
                         const char* context, int timeout, int flags)
{
    return netsnmp_register_old_api(moduleName, var, varsize, numvars, mibloc,
                                    mibloclen, priority, range_subid,
                                    range_ubound, ss, context, timeout, flags);
}
 ...
 *  @see unregister_mib()
 */
int register_mib_range(const char* moduleName, const struct variable* var,
                       size_t varsize, size_t numvars, const oid* mibloc,
                       size_t mibloclen, int priority, int range_subid,
                       oid range_ubound, netsnmp_session* ss)
{
    return register_mib_context(moduleName, var, varsize, numvars, mibloc,
                                mibloclen, priority, range_subid, range_ubound,
                                ss, "", -1, 0);
}
 ...
 *  @see unregister_mib()
 */
int register_mib_priority(const char* moduleName, const struct variable* var,
                          size_t varsize, size_t numvars, const oid* mibloc,
                          size_t mibloclen, int priority)
{
    return register_mib_range(moduleName, var, varsize, numvars, mibloc,
                              mibloclen, priority, 0, 0, NULL);
}

/** Register a new module into the MIB database, using default priority and
 * context
 *
 *  @param  moduleName Text name of the module.
 *                     The given name will be used to identify the module
 ...
 *  @see unregister_mib()
 */
int register_mib(const char* moduleName, const struct variable* var,
                 size_t varsize, size_t numvars, const oid* mibloc,
                 size_t mibloclen)
{
    return register_mib_priority(moduleName, var, varsize, numvars, mibloc,
                                 mibloclen, DEFAULT_MIB_PRIORITY);
}
 ...
 */
void netsnmp_subtree_unload(netsnmp_subtree* sub, netsnmp_subtree* prev,
                            const char* context)
{
    netsnmp_subtree* ptr;
    ...
    { /* just remove this node completely */
...
/**
 * Unregisters a module registered against a given OID (or range) in a specified
 * context. Typically used when a module has multiple contexts defined. The
 * parameters priority, range_subid, range_ubound and context should match those
 * used to register the module originally.
 *
 ...
 */
int unregister_mib_context(oid* name, size_t len, int priority, int range_subid,
                           oid range_ubound, const char* context)
{
    ...
    while (unregistering)
    {
        ...
        DEBUGMSGOIDRANGE(
            ("register_mib", name, len, range_subid, range_ubound));
        DEBUGMSG(("register_mib", "\n"));

        list = netsnmp_subtree_find(
            name, len, netsnmp_subtree_find_first(context), context);
        if (list == NULL)
        {
        ...
        {
            if (netsnmp_oid_equals(child->name_a, child->namelen, name, len) ==
                    0 &&
        ...
        for (list = myptr->next; list != NULL; list = next)
        {
            ...
            for (child = list, prev = NULL; child != NULL;
                 prev = child, child = child->children)
            {
                if ((netsnmp_oid_equals(child->name_a, child->namelen, name,
                                        len) == 0) &&
                    (child->priority == priority))
                {
                    netsnmp_subtree_unload(child, prev, context);
...
int netsnmp_unregister_mib_table_row(oid* name, size_t len, int priority,
...
/**
 * Unregisters a module registered against a given OID (or range) in the default
 * context. Typically used when a module has multiple contexts defined. The
 * parameters priority, range_subid, and range_ubound should match those used to
 * register the module originally.
 *
 * @param name  the specific OID to unregister if it conatins the associated
 *              context.
 *
 ...
 * @see unregister_mib_context()
 */
int unregister_mib_range(oid* name, size_t len, int priority, int range_subid,
                         oid range_ubound)
{
...
/**
 * Unregisters a module registered against a given OID at the specified
 * priority. The priority parameter should match that used to register the
 * module originally.
 *
 * @param name  the specific OID to unregister if it conatins the associated
 *              context.
        ...
        for (list = contextptr->first_subtree; list != NULL; list = list2)
        {
            ...
            for (child = list, prev = NULL; child != NULL; child = next_child)
            {
                ...
                if (((!ss || ss->flags & SNMP_FLAGS_SUBSESSION) &&
                     child->session == ss) ||
                    (!(!ss || ss->flags & SNMP_FLAGS_SUBSESSION) &&
                     child->session && child->session->subsession == ss))
                {
...
netsnmp_feature_child_of(get_session_for_oid, netsnmp_unused)
    ...
    netsnmp_session* get_session_for_oid(const oid* name, size_t len,
                                         const char* context_name)
{
    netsnmp_subtree* myptr;

    myptr = netsnmp_subtree_find_prev(
        name, len, netsnmp_subtree_find_first(context_name), context_name);
...
void setup_tree(void)
{
    ...
    /*
     * we need to have the oid's in the heap, that we can *free* it for every
     * case, thats the purpose of the duplicate_objid's
     */
...
int remove_tree_entry(oid* name, size_t len)
{
    ...
    return unregister_mib_context(name, len, sub->priority, sub->range_subid,
                                  sub->range_ubound, "");
}
        ...
        (v = (u_char*)calloc(sl, 1)) != NULL)
    {
        ...
        for (ptr = context_subtrees; ptr; ptr = ptr->next)
        {
            ...
            for (myptr = ptr->first_subtree; myptr != NULL; myptr = myptr->next)
            {
                ...
                if (!sprint_realloc_objid(&s, &sl, &sl_o, 1, myptr->start_a,
                                          myptr->start_len))
                {
                    break;
                }
                if (!sprint_realloc_objid(&e, &el, &el_o, 1, myptr->end_a,
                                          myptr->end_len))
                {
                    break;
                }
                ...
                if (myptr->variables)
                {
                    ...
                    for (i = 0, vp = myptr->variables; i < myptr->variables_len;
                         i++)
                    {
                        vl_o = 0;
                        if (!sprint_realloc_objid(&v, &vl, &vl_o, 1, vp->name,
                                                  vp->namelen))
                        {
                            break;
                        }
...
#endif
        default:
            snmp_log(LOG_CRIT, "register_signal: signal %d cannot be handled\n",
                     sig);
