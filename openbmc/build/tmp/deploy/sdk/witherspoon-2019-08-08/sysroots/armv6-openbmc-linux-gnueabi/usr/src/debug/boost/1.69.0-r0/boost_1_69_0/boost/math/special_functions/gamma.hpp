...
#pragma warning(push)
#pragma warning(                                                               \
    disable : 4702) // unreachable code (return after domain_error throw).
...
T gamma_imp(T z, const Policy& pol, const Lanczos& l)
{
    ...
    if (!b)
    {
        std::cout << "tgamma_imp called with " << typeid(z).name() << " "
    ...
    if (z <= 0)
    {
        if (floor(z) == z)
            return policies::raise_pole_error<T>(
                function, "Evaluation of tgamma at a negative integer %1%.", z,
                pol);
        if (z <= -20)
        {
            result = gamma_imp(T(-z), pol, l) * sinpx(z);
            BOOST_MATH_INSTRUMENT_VARIABLE(result);
            if ((fabs(result) < 1) && (tools::max_value<T>() * fabs(result) <
                                       boost::math::constants::pi<T>()))
                return -boost::math::sign(result) *
                       policies::raise_overflow_error<T>(
                           function,
                           "Result of tgamma is too large to represent.", pol);
            result = -boost::math::constants::pi<T>() / result;
            if (result == 0)
                return policies::raise_underflow_error<T>(
                    function, "Result of tgamma is too small to represent.",
                    pol);
            if ((boost::math::fpclassify)(result) == (int)FP_SUBNORMAL)
                return policies::raise_denorm_error<T>(
                    function, "Result of tgamma is denormalized.", result, pol);
            BOOST_MATH_INSTRUMENT_VARIABLE(result);
            return result;
        }
    ...
    else
    {
        ...
        T zgh = (z + static_cast<T>(Lanczos::g()) -
        ...
        else
        {
            ...
            BOOST_MATH_INSTRUMENT_VARIABLE(
                pow(zgh, z - boost::math::constants::half<T>()));
            BOOST_MATH_INSTRUMENT_VARIABLE(exp(zgh));
            result *=
                pow(zgh, z - boost::math::constants::half<T>()) / exp(zgh);
            BOOST_MATH_INSTRUMENT_VARIABLE(result);
        }
    }
    return result;
...
T lgamma_imp(T z, const Policy& pol, const Lanczos& l, int* sign = 0)
{
    ...
    if (!b)
    {
        std::cout << "lgamma_imp called with " << typeid(z).name() << " "
                  << typeid(l).name() << std::endl;
        b = true;
    }
#endif
    ...
    if (z <= -tools::root_epsilon<T>())
    {
        ...
        if (floor(z) == z)
            return policies::raise_pole_error<T>(
                function, "Evaluation of lgamma at a negative integer %1%.", z,
                pol);
        ...
        result = log(boost::math::constants::pi<T>()) - lgamma_imp(z, pol, l) -
                 log(t);
    }
    else if (z < tools::root_epsilon<T>())
    {
        if (0 == z)
            return policies::raise_pole_error<T>(
                function, "Evaluation of lgamma at %1%.", z, pol);
    ...
    else if (z < 15)
    {
        ...
        typedef typename mpl::if_<
            mpl::and_<mpl::less_equal<precision_type, mpl::int_<64>>,
                      mpl::greater<precision_type, mpl::int_<0>>>,
            mpl::int_<64>,
            typename mpl::if_<
                mpl::and_<mpl::less_equal<precision_type, mpl::int_<113>>,
                          mpl::greater<precision_type, mpl::int_<0>>>,
                mpl::int_<113>, mpl::int_<0>>::type>::type tag_type;
        result = lgamma_small_imp<T>(z, T(z - 1), T(z - 2), tag_type(), pol, l);
    }
    else if ((z >= 3) && (z < 100) &&
             (std::numeric_limits<T>::max_exponent >= 1024))
    {
        // taking the log of tgamma reduces the error, no danger of overflow
        // here:
    ...
    {
        // regular evaluation:
        T zgh = static_cast<T>(z + Lanczos::g() -
                               boost::math::constants::half<T>());
        result = log(zgh) - 1;
        result *= z - 0.5f;
...
inline T lower_gamma_series(T a, T z, const Policy& pol, T init_value = 0)
{
    ...
    policies::check_series_iterations<T>(
        "boost::math::detail::lower_gamma_series<%1%>(%1%)", max_iter, pol);
    return result;
}
...
T gamma_imp(T z, const Policy& pol, const lanczos::undefined_lanczos&)
{
    ...
    if ((is_at_zero) || ((boost::math::isinf)(z) && (z < 0)))
        return policies::raise_domain_error<T>(
            function, "Evaluation of tgamma at %1%.", z, pol);
    ...
    if ((!b_neg) && floor_of_z_is_equal_to_z &&
        (z < boost::math::max_factorial<T>::value))
    {
    ...
    if (zz < tools::cbrt_epsilon<T>())
    {
        ...
        const T six_euler_squared((boost::math::constants::euler<T>() *
                                   boost::math::constants::euler<T>()) *
                                  6);
        const T a2((six_euler_squared - boost::math::constants::pi_sqr<T>()) /
                   12);
    ...
    const T min_arg_for_recursion =
        minimum_argument_for_bernoulli_recursion<T>();
    ...
    if (n_recur)
    {
        ...
        // overflow with the first division, we must save dividing by |z| till
        // last, so the optimal order of divides is z+1, z+2,
        // z+3...z+n_recur-1,z.
        zz = fabs(z) + 1;
    ...
    if (b_neg)
    {
        ...
        // Check if the argument of tgamma is exactly equal to a negative
        // integer.
        ...
        const bool result_is_too_large_to_represent =
            ((abs(gamma_value) < 1) &&
             ((tools::max_value<T>() * abs(gamma_value)) <
              boost::math::constants::pi<T>()));

        if (result_is_too_large_to_represent)
            return policies::raise_overflow_error<T>(
                function, "Result of tgamma is too large to represent.", pol);
        ...
        if (gamma_value == 0)
            return policies::raise_underflow_error<T>(
                function, "Result of tgamma is too small to represent.", pol);

        if ((boost::math::fpclassify)(gamma_value) ==
            static_cast<int>(FP_SUBNORMAL))
            return policies::raise_denorm_error<T>(
                function, "Result of tgamma is denormalized.", gamma_value,
                pol);
    }
             ...
             int* sign)
{
    ...
    if (is_at_zero)
        return policies::raise_domain_error<T>(
            function, "Evaluation of lgamma at zero %1%.", z, pol);
    ...
    if (zz < min_arg_for_recursion)
    {
        ...
        {
            // Reflection formula may fail if z is very close to zero, let the
            // series expansion for tgamma close to zero do the work:
            log_gamma_value =
                log(abs(gamma_imp(z, pol, lanczos::undefined_lanczos())));
            if (sign)
            {
                *sign = z < 0 ? -1 : 1;
            }
        ...
        else
        {
            ...
            log_gamma_value =
                log(abs(gamma_imp(zz, pol, lanczos::undefined_lanczos())));
        }
    }
    else
    {
        ...
        const std::size_t number_of_bernoullis_b2n =
            highest_bernoulli_index<T>();

        T one_over_x_pow_two_n_minus_one = 1 / zz;
        const T one_over_x2 =
            one_over_x_pow_two_n_minus_one * one_over_x_pow_two_n_minus_one;
        T sum = (boost::math::bernoulli_b2n<T>(1) / 2) *
                one_over_x_pow_two_n_minus_one;
        const T target_epsilon_to_break_loop =
            (sum * boost::math::tools::epsilon<T>()) * T(1.0E-10F);
        ...
        for (std::size_t n = 2U; n < number_of_bernoullis_b2n; ++n)
        {
            ...
            const T term = (boost::math::bernoulli_b2n<T>(static_cast<int>(n)) *
                            one_over_x_pow_two_n_minus_one) /
                           (n2 * (n2 - 1U));

            if ((n >= 8U) && (abs(term) < target_epsilon_to_break_loop))
            {
                // We have reached the desired precision in Stirling's
                // expansion. Adding additional terms to the sum of this
                // divergent asymptotic expansion will not improve the result.
        ...
        log_gamma_value =
            ((((zz - boost::math::constants::half<T>()) * log(zz)) - zz) +
             half_ln_two_pi) +
            sum;
    }
    ...
    if (b_neg)
    {
        ...
        // Check if the argument of lgamma is exactly equal to a negative
        // integer.
        if (floor_of_z_is_equal_to_z)
            return policies::raise_pole_error<T>(
                function, "Evaluation of lgamma at a negative integer %1%.", z,
        ...
        log_gamma_value =
            -log_gamma_value + log(boost::math::constants::pi<T>()) - log(t);
    }

    if (sign != static_cast<int*>(0U))
    {
        *sign = sign_of_result;
    }
...
T tgammap1m1_imp(T dz, Policy const& pol, const Lanczos& l)
{
    ...
    typedef typename mpl::if_<
        ...
        typename mpl::if_<mpl::and_<is_same<Lanczos, lanczos::lanczos24m113>,
                                    mpl::greater<precision_type, mpl::int_<0>>>,
                          mpl::int_<113>, mpl::int_<0>>::type,
        typename mpl::if_<mpl::less_equal<precision_type, mpl::int_<64>>,
                          mpl::int_<64>, mpl::int_<113>>::type>::type tag_type;
    ...
    if (dz < 0)
    {
        ...
        {
            // Use expm1 on lgamma:
            result = boost::math::expm1(
                -boost::math::log1p(dz, pol) +
    ...
    else
    {
        if (dz < 2)
        {
            // Use expm1 on lgamma:
            result = boost::math::expm1(
                lgamma_small_imp<T>(dz + 1, dz, dz - 1, tag_type(), pol, l),
                pol);
            BOOST_MATH_INSTRUMENT_CODE(result);
        }
        else
...
T full_igamma_prefix(T a, T z, const Policy& pol)
{
    ...
    if (z >= 1)
    {
        if ((alz < tools::log_max_value<T>()) &&
            (-z > tools::log_min_value<T>()))
        {
...
T regularised_gamma_prefix(T a, T z, const Policy& pol, const Lanczos& l)
{
    ...
    if (a < 1)
    {
        ...
        {
            // Oh dear, have to use logs, should be free of cancellation errors
            // though:
    ...
    else if ((fabs(d * d * a) <= 100) && (a > 150))
    {
        ...
        prefix = a * boost::math::log1pmx(d, pol) +
                 z * static_cast<T>(0.5 - Lanczos::g()) / agh;
        prefix = exp(prefix);
    }
    else
    {
        ...
        T alz = a * log(z / agh);
        T amz = a - z;
        if (((std::min)(alz, amz) <= tools::log_min_value<T>()) ||
            ((std::max)(alz, amz) >= tools::log_max_value<T>()))
        {
            T amza = amz / a;
            if (((std::min)(alz, amz) / 2 > tools::log_min_value<T>()) &&
                ((std::max)(alz, amz) / 2 < tools::log_max_value<T>()))
            {
                // compute square root of the result and then square it:
                T sq = pow(z / agh, a / 2) * exp(amz / 2);
                prefix = sq * sq;
            }
            else if (((std::min)(alz, amz) / 4 > tools::log_min_value<T>()) &&
                     ((std::max)(alz, amz) / 4 < tools::log_max_value<T>()) &&
                     (z > a))
            {
            ...
            else if ((amza > tools::log_min_value<T>()) &&
                     (amza < tools::log_max_value<T>()))
            {
    ...
    prefix *= sqrt(agh / boost::math::constants::e<T>()) /
...
//
template <class T, class Policy>
T regularised_gamma_prefix(T a, T z, const Policy& pol,
                           const lanczos::undefined_lanczos&)
{
    ...
    sum += detail::upper_gamma_fraction(
        a, limit, ::boost::math::policies::get_epsilon<T, Policy>());
    ...
    T alzoa = a * log(zoa);
    T prefix;
    if (((std::min)(alzoa, amz) <= tools::log_min_value<T>()) ||
        ((std::max)(alzoa, amz) >= tools::log_max_value<T>()))
    {
        T amza = amz / a;
        if ((amza <= tools::log_min_value<T>()) ||
            (amza >= tools::log_max_value<T>()))
        {
...
inline T tgamma_small_upper_part(T a, T x, const Policy& pol, T* pgam = 0,
                                 bool invert = false, T* pderivative = 0)
{
    ...
    boost::uintmax_t max_iter =
        policies::get_max_series_iterations<Policy>() - 10;
    p += 1;
    if (pderivative)
        *pderivative = p / (*pgam * exp(x));
    T init_value = invert ? *pgam : 0;
    result = -p * tools::sum_series(
                      s, boost::math::policies::get_epsilon<T, Policy>(),
                      max_iter, (init_value - result) / p);
    policies::check_series_iterations<T>(
        "boost::math::tgamma_small_upper_part<%1%>(%1%, %1%)", max_iter, pol);
    if (invert)
        result = -result;
    return result;
}
...
inline T finite_gamma_q(T a, T x, Policy const& pol, T* pderivative = 0)
{
    ...
    if (pderivative)
    {
        *pderivative =
            e * pow(x, a) /
            boost::math::unchecked_factorial<T>(itrunc(T(a - 1), pol));
    }
    return sum;
}
//
                       ...
                       const Policy& pol, T* p_derivative)
{
    ...
    if (x < 0)
        return policies::raise_domain_error<T>(
            function,
            "Argument x to the incomplete gamma function must be >= 0 (got "
            "x=%1%).",
            x, pol);
    ...
    T result = 0; // Just to avoid warning C4701: potentially uninitialized
    ...
    if (a >= max_factorial<T>::value && !normalised)
    {
        ...
        else if (!invert && (a > 4 * x))
        {
            ...
            T init_value = 0;
            result +=
                log(detail::lower_gamma_series(a, x, pol, init_value) / a);
        }
        else
        {
            result =
                gamma_incomplete_imp(a, x, true, invert, pol, p_derivative);
            if (result == 0)
            {
                if (invert)
                {
                    ...
                    result = log(result) - a + (a - 0.5f) * log(a) +
                             log(boost::math::constants::root_two_pi<T>());
                    if (p_derivative)
                        *p_derivative = exp(a * log(x) - x);
                }
                else
                {
                    // This is method 2 below, done in logs, we're really
                    // outside the range of this method, but since the result is
                    // almost certainly infinite, we should probably be OK:
                    result = a * log(x) - x;
                    if (p_derivative)
                        *p_derivative = exp(result);
                    T init_value = 0;
                    result += log(
                        detail::lower_gamma_series(a, x, pol, init_value) / a);
                }
            }
            else
            {
    ...
    bool is_small_a =
        (a < 30) && (a <= x + 1) && (x < tools::log_max_value<T>());
    if (is_small_a)
    {
        T fa = floor(a);
        is_int = (fa == a);
    ...
    else
    {
        ...
        else
        {
            ...
            // Note that series computation of P is about x2 faster than
            // continued fraction calculation of Q, so try and use the CF only
            // when really necessary, especially for small x.
            //
            if (x - (1 / (3 * x)) < a)
            {
                eval_method = 2;
    ...
    switch (eval_method)
    {
        ...
        case 1:
        {
            ...
            if (p_derivative && (*p_derivative == 0))
                *p_derivative =
                    regularised_gamma_prefix(a, x, pol, lanczos_type());
            break;
        }
        case 2:
        {
            ...
            if (result != 0)
            {
                ...
                if (invert)
                {
                    ...
                    if (normalised || (result >= 1) ||
                        (tools::max_value<T>() * result > init_value))
                    {
                        init_value /= result;
                        if (normalised || (a < 1) ||
                            (tools::max_value<T>() / a > init_value))
                        {
                            init_value *= -a;
        ...
        case 3:
        {
            ...
            T g;
            result =
                tgamma_small_upper_part(a, x, pol, &g, invert, p_derivative);
            invert = false;
            if (normalised)
                result /= g;
            break;
        ...
        case 4:
        {
            ...
            if (result != 0)
                result *= upper_gamma_fraction(
                    a, x, policies::get_epsilon<T, Policy>());
            break;
        }
        case 5:
        {
            ...
            //
            typedef
                typename policies::precision<T, Policy>::type precision_type;
            ...
            typedef typename mpl::if_<
                ...
                typename mpl::if_<
                    ...
                    typename mpl::if_<
                        ...
                        mpl::int_<64>, mpl::int_<113>>::type>::type>::type
                tag_type;

            result =
                igamma_temme_large(a, x, pol, static_cast<tag_type const*>(0));
            if (x >= a)
                invert = !invert;
            if (p_derivative)
                *p_derivative =
        ...
        case 6:
        {
            ...
            result = !normalised ? pow(x, a) / (a)
                                 : pow(x, a) / boost::math::tgamma(a + 1, pol);
            result *= 1 - a * x / (a + 1);
        }
    }
...
//
template <class T, class Policy, class Lanczos>
T tgamma_delta_ratio_imp_lanczos(T z, T delta, const Policy& pol,
                                 const Lanczos& l)
{
    BOOST_MATH_STD_USING
    if (z < tools::epsilon<T>())
    {
        ...
        {
            T ratio = tgamma_delta_ratio_imp_lanczos(
                delta, T(boost::math::max_factorial<T>::value - delta), pol, l);
            ratio *= z;
            ratio *= boost::math::unchecked_factorial<T>(
                boost::math::max_factorial<T>::value - 1);
            return 1 / ratio;
        }
        else
        {
    ...
    {
        if (fabs(delta) < 10)
            result = exp((constants::half<T>() - z) *
                         boost::math::log1p(delta / zgh, pol));
        else
            result = 1;
    }
    else
                                 ...
                                 const lanczos::undefined_lanczos&)
{
    ...
    sum += detail::upper_gamma_fraction(
        z, z, ::boost::math::policies::get_epsilon<T, Policy>());
    T sumd = detail::lower_gamma_series(zd, zd, pol) / zd;
    sumd += detail::upper_gamma_fraction(
        zd, zd, ::boost::math::policies::get_epsilon<T, Policy>());
    sum /= sumd;
...
T tgamma_delta_ratio_imp(T z, T delta, const Policy& pol)
{
    ...
    if ((z <= 0) || (z + delta <= 0))
    {
        ...
        return boost::math::tgamma(z, pol) /
               boost::math::tgamma(z + delta, pol);
    }
    ...
    if (floor(delta) == delta)
    {
        ...
        {
            //
            // Both z and delta are integers, see if we can just use table
            // lookup of the factorials to get the result:
            //
            if ((z <= max_factorial<T>::value) &&
                (z + delta <= max_factorial<T>::value))
            {
                return unchecked_factorial<T>((unsigned)itrunc(z, pol) - 1) /
                       unchecked_factorial<T>(
                           (unsigned)itrunc(T(z + delta), pol) - 1);
            }
        }
        if (fabs(delta) < 20)
        {
...
T tgamma_ratio_imp(T x, T y, const Policy& pol)
{
    ...
    if ((x <= 0) || (boost::math::isinf)(x))
        return policies::raise_domain_error<T>(
            "boost::math::tgamma_ratio<%1%>(%1%, %1%)",
            "Gamma function ratios only implemented for positive arguments "
            "(got a=%1%).",
            x, pol);
    ...
    {
        // Rather than subtracting values, lets just call the gamma functions
        // directly:
    ...
    if (x < 1)
    {
        if (y < 2 * max_factorial<T>::value)
        {
            ...
            return prefix * boost::math::tgamma(x, pol) /
                   boost::math::tgamma(y, pol);
        }
        //
        // result is almost certainly going to underflow to zero, try logs just
        // in case:
        //
...
T gamma_p_derivative_imp(T a, T x, const Policy& pol)
{
    ...
    if (a <= 0)
        return policies::raise_domain_error<T>(
            "boost::math::gamma_p_derivative<%1%>(%1%, %1%)",
            "Argument a to the incomplete gamma function must be greater than "
            "zero (got a=%1%).",
            a, pol);
    if (x < 0)
    ...
    if ((x < 1) && (tools::max_value<T>() * x < f1))
    {
        // overflow:
        return policies::raise_overflow_error<T>(
            "boost::math::gamma_p_derivative<%1%>(%1%, %1%)", 0, pol);
    }
    if (f1 == 0)
    {
    ...
    tgamma(T z, const Policy& /* pol */, const mpl::true_)
{
    ...
    return policies::checked_narrowing_cast<result_type, forwarding_policy>(
        detail::gamma_imp(static_cast<value_type>(z), forwarding_policy(),
                          evaluation_type()),
...
struct igamma_initializer
{
    struct init
    {
        ...
        static void do_init(const mpl::int_<N>&)
        {
            ...
            {
                boost::math::gamma_p(static_cast<T>(400), static_cast<T>(400),
                                     Policy());
            }
        }
...
template <class T, class Policy>
const typename igamma_initializer<T, Policy>::init
    igamma_initializer<T, Policy>::initializer;
...
template <class T, class Policy>
const typename lgamma_initializer<T, Policy>::init
    lgamma_initializer<T, Policy>::initializer;
    ...
    tgamma(T1 a, T2 z, const Policy&, const mpl::false_)
{
    ...
    typedef typename policies::evaluation<result_type, Policy>::type value_type;
    // typedef typename lanczos::lanczos<value_type, Policy>::type
    // evaluation_type;
    typedef typename policies::normalise<
        Policy, policies::promote_float<false>, policies::promote_double<false>,
        policies::discrete_quantile<>, policies::assert_undefined<>>::type
    ...
    return policies::checked_narrowing_cast<result_type, forwarding_policy>(
        detail::gamma_incomplete_imp(
            static_cast<value_type>(a), static_cast<value_type>(z), false, true,
            forwarding_policy(), static_cast<value_type*>(0)),
        "boost::math::tgamma<%1%>(%1%, %1%)");
}

template <class T1, class T2>
inline typename tools::promote_args<T1, T2>::type tgamma(T1 a, T2 z,
...
template <class T>
inline typename tools::promote_args<T>::type tgamma(T z)
{
    return tgamma(z, policies::policy<>());
}

template <class T, class Policy>
inline typename tools::promote_args<T>::type lgamma(T z, int* sign,
                                                    const Policy&)
{
    BOOST_FPU_EXCEPTION_GUARD
    typedef typename tools::promote_args<T>::type result_type;
    ...
    detail::lgamma_initializer<value_type,
                               forwarding_policy>::force_instantiate();

    return policies::checked_narrowing_cast<result_type, forwarding_policy>(
        detail::lgamma_imp(static_cast<value_type>(z), forwarding_policy(),
                           evaluation_type(), sign),
...
template <class T, class Policy>
inline typename tools::promote_args<T>::type lgamma(T x, const Policy& pol)
{
...
template <class T, class Policy>
inline typename tools::promote_args<T>::type tgamma1pm1(T z,
                                                        const Policy& /* pol */)
{
    BOOST_FPU_EXCEPTION_GUARD
    typedef typename tools::promote_args<T>::type result_type;
    ...
    return policies::checked_narrowing_cast<
        typename remove_cv<result_type>::type, forwarding_policy>(
        detail::tgammap1m1_imp(static_cast<value_type>(z), forwarding_policy(),
                               evaluation_type()),
...
//
template <class T1, class T2, class Policy>
inline typename tools::promote_args<T1, T2>::type tgamma_lower(T1 a, T2 z,
                                                               const Policy&)
{
    BOOST_FPU_EXCEPTION_GUARD
    typedef typename tools::promote_args<T1, T2>::type result_type;
    ...
    detail::igamma_initializer<value_type,
                               forwarding_policy>::force_instantiate();

    return policies::checked_narrowing_cast<result_type, forwarding_policy>(
        detail::gamma_incomplete_imp(
            static_cast<value_type>(a), static_cast<value_type>(z), false,
            false, forwarding_policy(), static_cast<value_type*>(0)),
        "tgamma_lower<%1%>(%1%, %1%)");
}
template <class T1, class T2>
inline typename tools::promote_args<T1, T2>::type tgamma_lower(T1 a, T2 z)
    ...
    gamma_q(T1 a, T2 z, const Policy& /* pol */)
{
    ...
    return policies::checked_narrowing_cast<result_type, forwarding_policy>(
        detail::gamma_incomplete_imp(
            static_cast<value_type>(a), static_cast<value_type>(z), true, true,
            forwarding_policy(), static_cast<value_type*>(0)),
        "gamma_q<%1%>(%1%, %1%)");
}
template <class T1, class T2>
inline typename tools::promote_args<T1, T2>::type gamma_q(T1 a, T2 z)
{
...
//
template <class T1, class T2, class Policy>
inline typename tools::promote_args<T1, T2>::type gamma_p(T1 a, T2 z,
                                                          const Policy&)
{
    BOOST_FPU_EXCEPTION_GUARD
    typedef typename tools::promote_args<T1, T2>::type result_type;
    ...
    return policies::checked_narrowing_cast<result_type, forwarding_policy>(
        detail::gamma_incomplete_imp(
            static_cast<value_type>(a), static_cast<value_type>(z), true, false,
            forwarding_policy(), static_cast<value_type*>(0)),
        "gamma_p<%1%>(%1%, %1%)");
}
template <class T1, class T2>
    ...
    tgamma_delta_ratio(T1 z, T2 delta, const Policy& /* pol */)
{
    ...
    return policies::checked_narrowing_cast<result_type, forwarding_policy>(
        detail::tgamma_delta_ratio_imp(static_cast<value_type>(z),
                                       static_cast<value_type>(delta),
                                       forwarding_policy()),
        "boost::math::tgamma_delta_ratio<%1%>(%1%, %1%)");
}
template <class T1, class T2>
inline typename tools::promote_args<T1, T2>::type tgamma_delta_ratio(T1 z,
                                                                     T2 delta)
{
    return tgamma_delta_ratio(z, delta, policies::policy<>());
}
template <class T1, class T2, class Policy>
inline typename tools::promote_args<T1, T2>::type tgamma_ratio(T1 a, T2 b,
                                                               const Policy&)
{
    typedef typename tools::promote_args<T1, T2>::type result_type;
    typedef typename policies::evaluation<result_type, Policy>::type value_type;
    ...
    return policies::checked_narrowing_cast<result_type, forwarding_policy>(
        detail::tgamma_ratio_imp(static_cast<value_type>(a),
    ...
    gamma_p_derivative(T1 a, T2 x, const Policy&)
{
    ...
    return policies::checked_narrowing_cast<result_type, forwarding_policy>(
        detail::gamma_p_derivative_imp(static_cast<value_type>(a),
                                       static_cast<value_type>(x),
                                       forwarding_policy()),
        "boost::math::gamma_p_derivative<%1%>(%1%, %1%)");
}
template <class T1, class T2>
inline typename tools::promote_args<T1, T2>::type gamma_p_derivative(T1 a, T2 x)
{
