...
} dos_nested[] = {{MBR_FREEBSD_PARTITION, &bsd_pt_idinfo},
                              ...
                              uint32_t ex_start, uint32_t ex_size, int ssf)
{
    ...
    DBG(LOWPROBE, ul_debug("parse EBR [start=%d, size=%d]", ex_start / ssf,
                           ex_size / ssf));
    if (ex_start == 0)
    {
        DBG(LOWPROBE,
            ul_debug("Bad offset in primary extended partition -- ignore"));
        return 0;
    }
    ...
    while (1)
    {
        ...
        for (p = p0, i = 0; i < 4; i++, p++)
        {
            ...
            {
                DBG(LOWPROBE, ul_debug("#%d: EBR duplicate data partition [abs "
                                       "start=%u] -- ignore",
                                       i + 1, abs_start));
                continue;
            }
        ...
        for (p = p0, i = 0; i < 4; i++, p++)
        {
            ...
            if (size && is_extended(p))
            {
                if (start == 0)
                    DBG(LOWPROBE,
                        ul_debug("#%d: EBR link offset is zero -- ignore",
                                 i + 1));
                else
                    break;
            }
                        ...
                        __attribute__((__unused__)))
{
    ...
    if (!blkid_probe_is_tiny(pr))
    {
        ...
        for (i = 0; i < nparts; i++)
        {
            ...
            if (pa == NULL || blkid_partition_get_size(pa) == 0 ||
            ...
            for (n = 0; n < ARRAY_SIZE(dos_nested); n++)
            {
                ...
                rc = blkid_partitions_do_subprobe(pr, pa, dos_nested[n].id);
                if (rc < 0)
                    return rc;
                break;
            }
