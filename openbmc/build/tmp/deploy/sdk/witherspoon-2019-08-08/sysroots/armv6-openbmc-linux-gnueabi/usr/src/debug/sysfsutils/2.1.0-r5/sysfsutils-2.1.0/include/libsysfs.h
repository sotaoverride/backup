...
extern struct sysfs_driver* sysfs_open_driver(const char* bus_name,
                                              const char* drv_name);
extern struct sysfs_driver* sysfs_open_driver_path(const char* path);
extern struct sysfs_attribute* sysfs_get_driver_attr(struct sysfs_driver* drv,
                                                     const char* name);
...
extern struct sysfs_device* sysfs_open_device(const char* bus,
                                              const char* bus_id);
...
extern struct sysfs_attribute* sysfs_get_device_attr(struct sysfs_device* dev,
                                                     const char* name);
extern struct dlist* sysfs_get_device_attributes(struct sysfs_device* dev);

/* generic sysfs class access */
extern void sysfs_close_class_device(struct sysfs_class_device* dev);
extern struct sysfs_class_device*
    sysfs_open_class_device_path(const char* path);
extern struct sysfs_class_device* sysfs_open_class_device(const char* classname,
                                                          const char* name);
extern struct sysfs_class_device*
    sysfs_get_classdev_parent(struct sysfs_class_device* clsdev);
extern struct sysfs_attribute*
    sysfs_get_classdev_attr(struct sysfs_class_device* clsdev,
                            const char* name);
extern struct dlist*
    sysfs_get_classdev_attributes(struct sysfs_class_device* clsdev);
extern struct sysfs_device*
    sysfs_get_classdev_device(struct sysfs_class_device* clsdev);
extern void sysfs_close_class(struct sysfs_class* cls);
extern struct sysfs_class* sysfs_open_class(const char* name);
extern struct sysfs_class_device*
    sysfs_get_class_device(struct sysfs_class* cls, const char* name);
...
extern struct sysfs_attribute*
    sysfs_get_module_attr(struct sysfs_module* module, const char* name);
extern struct sysfs_attribute*
    sysfs_get_module_parm(struct sysfs_module* module, const char* parm);
extern struct sysfs_attribute*
    sysfs_get_module_section(struct sysfs_module* module, const char* section);
...
static inline int sort_list(void* new_elem, void* old_elem)
{
    return ((strncmp(((struct sysfs_attribute*)new_elem)->name,
                     ...
                     strlen(((struct sysfs_attribute*)new_elem)->name))) < 0
                ? 1
                : 0);
}
