    ...
    if (!f)
    {
        if (source && errno == EEXIST)
            return log_error_errno(errno,
                                   "Failed to create unit file %s, as it "
                                   "already exists. Duplicate entry in %s?",
                                   unit, source);
        else
...
static int write_fsck_sysroot_service(const char* dir, const char* what)
{
    ...
    if (r < 0)
        return log_error_errno(
            r, "Failed to convert device \"%s\" to unit name: %m", what);
                              ...
                              const char* where, const char* fstype)
{
    ...
    {
        log_warning(
            "Checking was requested for \"%s\", but it is not a device.", what);
        return 0;
    }
    ...
    if (!isempty(fstype) && !streq(fstype, "auto"))
    {
        ...
        if (r < 0)
            log_warning_errno(r,
                              "Checking was requested for %s, but couldn't "
                              "detect if fsck.%s may be used, proceeding: %m",
                              what, fstype);
        else if (r == 0)
        {
            /* treat missing check as essentially OK */
            log_debug(
                "Checking was requested for %s, but fsck.%s does not exist.",
                what, fstype);
            return 0;
        }
    }
    ...
    if (path_equal(where, "/"))
    {
        ...
        if (symlink(SYSTEM_DATA_UNIT_PATH "/" SPECIAL_FSCK_ROOT_SERVICE, lnk) <
            0)
            return log_error_errno(errno, "Failed to create symlink %s: %m",
                                   lnk);
    }
    else
    {
        ...
        {
            r = unit_name_from_path_instance("systemd-fsck", what, ".service",
                                             &_fsck);
            if (r < 0)
                return log_error_errno(
                    r, "Failed to create fsck service name: %m");
                             ...
                             char** filtered)
{
     ...
     * swap partition to show up. This is useful to support endless device
     * timeouts for devices that show up only after user input, like crypto
     * devices. */
                                ...
                                const char* where, const char* opts)
{
    ...
    if (r < 0)
        return log_error_errno(
            r, "Failed to make unit name from path \"%s\": %m", node);
    ...
    return write_drop_in_format(dir, unit, 50, "netdev-dependencies",
                                "# Automatically generated by %s\n\n"
                                "[Unit]\n"
                                "After=" SPECIAL_NETWORK_ONLINE_TARGET
...
int generator_write_initrd_root_device_deps(const char* dir, const char* what)
{
    ...
    if (r < 0)
        return log_error_errno(
            r, "Failed to make unit name from path \"%s\": %m", what);

    return write_drop_in_format(dir, SPECIAL_INITRD_ROOT_DEVICE_TARGET, 50,
                                "root-device",
                                "# Automatically generated by %s\n\n"
                                "[Unit]\n"
                                "Requires=%s\n"
...
int generator_hook_up_mkswap(const char* dir, const char* what)
{
    ...
    _cleanup_free_ char *node = NULL, *unit = NULL, *escaped = NULL,
                        *where_unit = NULL;
    _cleanup_fclose_ FILE* f = NULL;
    ...
    if (!is_device_path(node))
        return log_error_errno(
            ...
            "Cannot format something that is not a device node: %s", node);

    r = unit_name_from_path_instance("systemd-mkswap", node, ".service", &unit);
    if (r < 0)
        return log_error_errno(
            r, "Failed to make unit instance name from path \"%s\": %m", node);
    ...
    fprintf(f,
            ...
            "TimeoutSec=0\n",
            program_invocation_short_name, where_unit, escaped);
                           ...
                           const char* type)
{
    ...
    if (r < 0)
        return log_error_errno(
            r, "Failed to make unit name from path \"%s\": %m", where);
    ...
    fprintf(
        ...
        "TimeoutSec=0\n",
        program_invocation_short_name, where_unit, type, escaped);
                             ...
                             const char* target)
{
    ...
    r = unit_name_from_path_instance("systemd-growfs", where, ".service",
                                     &unit);
    if (r < 0)
        return log_error_errno(
            r, "Failed to make unit instance name from path \"%s\": %m", where);
    ...
    fprintf(f,
            ...
            "TimeoutSec=0\n",
            program_invocation_short_name, target, escaped);
