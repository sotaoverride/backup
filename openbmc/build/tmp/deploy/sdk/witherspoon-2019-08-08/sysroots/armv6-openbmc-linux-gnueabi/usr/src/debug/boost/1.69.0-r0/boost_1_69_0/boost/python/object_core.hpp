...
// Put this in an inner namespace so that the generalized operators won't take
// over
namespace api
{
...
#define BOOST_PP_ITERATION_PARAMS_1                                            \
    (3, (1, BOOST_PYTHON_MAX_ARITY, <boost / python / object_call.hpp>))
#include BOOST_PP_ITERATE()
  ...
  private: // def visitation for adding callable objects as class methods
               ...
               python::detail::def_helper<DocStringT> const& helper) const
    {
        ...
        objects::add_to_namespace(cl, name, this->derived_visitor(),
                                  helper.doc());
    }
...
struct is_derived : boost::python::detail::is_convertible<
                        typename detail::remove_reference<T>::type*, U const*>
{
};
...
PyObject* object_base_initializer(T const& x)
{
    typedef typename is_derived<
        BOOST_DEDUCED_TYPENAME objects::unforward_cref<T>::type, object>::type
        is_obj;

    return object_initializer<
        BOOST_DEDUCED_TYPENAME unwrap_reference<T>::type>::get(x, is_obj());
}
...
class object : public object_base
{
  public:
    ...
    template <class T>
    explicit object(T const& x) : object_base(object_base_initializer(x))
    {
    }
...
#define BOOST_PYTHON_FORWARD_OBJECT_CONSTRUCTORS(derived, base)                \
    ...
    inline explicit derived(                                                   \
        ::boost::python::detail::new_non_null_reference p) :                   \
        base(p)                                                                \
    {                                                                          \
    }
...
{
    static PyObject* get(object const& x, detail::true_)
    {
...
template <class T>
struct object_initializer
    : object_initializer_impl<is_proxy<T>::value,
...
class args_proxy : public call_proxy
{
  public:
    ...
    kwds_proxy operator*() const
    {
        return kwds_proxy(*this);
    }
};
} // namespace detail
...
template <typename U>
object api::object_operators<U>::
    operator()(detail::args_proxy const& args) const
{
    U const& self = *static_cast<U const*>(this);
    PyObject* result =
        PyObject_Call(get_managed_object(self, boost::python::tag),
                      args.operator object().ptr(), 0);
    return object(boost::python::detail::new_reference(result));
}

template <typename U>
object api::object_operators<U>::
    operator()(detail::args_proxy const& args,
               detail::kwds_proxy const& kwds) const
{
    U const& self = *static_cast<U const*>(this);
    PyObject* result = PyObject_Call(
        get_managed_object(self, boost::python::tag),
        args.operator object().ptr(), kwds.operator object().ptr());
    return object(boost::python::detail::new_reference(result));
}
...
inline object::object() : object_base(python::incref(Py_None))
{
}
...
inline api::object_base::object_base(PyObject* p) : m_ptr(p)
{
}

inline api::object_base& api::object_base::
    operator=(api::object_base const& rhs)
{
    Py_INCREF(rhs.m_ptr);
