    ...
    : public detail::common_iarchive<Archive>
{
  ...
  protected:
    ...
    BOOST_STATIC_ASSERT(sizeof(object_reference_type) ==
    ...
    void load_override(class_id_type& t)
    {
         ...
         *
         *
         * 1) in boost 1.43 and inferior, class_id_type is always a 16bit value,
         * with no check on the library version
         *   --> this means all archives with version v <= 7 are written with a
         * 16bit class_id_type 2) in boost 1.44 this load_override has
         * disappeared (and thus boost 1.44 is not backward compatible at all
         * !!) 3) recent boosts reintroduced load_override with a test on the
         * version :
         ...
         *
         * the fix here decodes class_id_type on 16bit for all v <= 7, which
         * seems to be the correct behaviour ...
         */
    ...
    void load_override(version_type& t)
    {
        ...
        else if (boost::archive::library_version_type(6) < lvt)
        {
            uint_least8_t x = 0;
            *this->This() >> x;
            t = boost::archive::version_type(x);
        }
        else if (boost::archive::library_version_type(5) < lvt)
        {
            uint_least16_t x = 0;
            *this->This() >> x;
            t = boost::archive::version_type(x);
        }
        else if (boost::archive::library_version_type(2) < lvt)
        {
            // upto 255 versions
            unsigned char x = 0;
            *this->This() >> x;
    ...
    {
        if (boost::archive::library_version_type(5) <
    ...
    BOOST_ARCHIVE_OR_WARCHIVE_DECL void load_override(class_name_type& t);
