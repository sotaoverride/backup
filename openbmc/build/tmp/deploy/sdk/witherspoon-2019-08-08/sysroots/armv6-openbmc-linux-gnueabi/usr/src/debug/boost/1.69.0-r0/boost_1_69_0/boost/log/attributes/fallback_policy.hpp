 ...
 * \date   18.08.2012
 *
 * The header contains definition of fallback policies when attribute value
 * visitation or extraction fails.
 */
...
/*!
 * The \c fallback_to_none policy results in returning an empty value reference
 * if the attribute value cannot be extracted.
 */
struct fallback_to_none
{
    ...
    /*!
     * The method is called in order to apply a function object to the default
     * value.
     */
    ...
    /*!
     * The method is called when value extraction failed because the attribute
     * value has different type than requested.
     */
...
/*!
 * The \c fallback_to_throw policy results in throwing an exception if the
 * attribute value cannot be extracted.
 */
struct fallback_to_throw
{
    ...
    static void on_invalid_type(typeindex::type_index const& t)
    {
        BOOST_LOG_THROW_DESCR_PARAMS(
            invalid_type, "Attribute value has incompatible type", (t));
    }
...
/*!
 * The \c fallback_to_default policy results in a default value if the attribute
...
struct fallback_to_default
{
    ...
    //! Default value type
    typedef typename remove_cv<typename remove_reference<DefaultT>::type>::type
        default_type;
