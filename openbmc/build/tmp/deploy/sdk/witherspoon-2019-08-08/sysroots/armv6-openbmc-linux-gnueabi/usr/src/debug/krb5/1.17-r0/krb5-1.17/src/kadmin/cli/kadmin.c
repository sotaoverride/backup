...
static char* strdur(time_t duration)
{
    ...
    duration %= 60;
    seconds = duration;
    snprintf(out, sizeof(out), "%s%d %s %02d:%02d:%02d", neg ? "-" : "", days,
             days == 1 ? "day" : "days", hours, minutes, seconds);
    return out;
}

static const char* strdate(krb5_timestamp when)
{
    struct tm* tm;
    static char out[40];
 ...
 * and return (time_t)-1. */
static time_t parse_date(char* str, time_t now)
{
    time_t date;
 ...
 */
static time_t parse_interval(char* str, time_t now)
{
    time_t date;
    krb5_deltat delta;
...
static krb5_error_code create_princ(kadm5_principal_ent_rec* princ, long mask,
                                    int n_ks, krb5_key_salt_tuple* ks,
                                    char* pass)
{
    if (ks)
...
krb5_error_code randkey_princ(void* lhandle, krb5_principal princ,
                              krb5_boolean keepold, int n_ks,
                              krb5_key_salt_tuple* ks, krb5_keyblock** key,
                              int* n_keys)
{
    krb5_error_code ret;
...
void kadmin_startup(int argc, char* argv[], char** request_out,
                    char*** args_out)
{
    extern char* optarg;
    ...
    if ((ccache_name && use_keytab) || (keytab_name && !use_keytab) ||
        (ccache_name && use_anonymous) || (use_anonymous && use_keytab))
        usage();
    ...
    if (princstr == NULL)
    {
        ...
        else if (!krb5_cc_get_principal(context, cc, &princ))
        {
            ...
            if (cp != NULL)
                *cp = '\0';
            if (asprintf(&princstr, "%s/admin%s%s", canon, (realm) ? "@" : "",
                         (realm) ? realm : "") < 0)
            {
                error(_("%s: out of memory\n"), whoami);
                exit(1);
    ...
    else
    {
        ...
        retval = kadm5_init_with_password(
            context, princstr, password, svcname, &params, KADM5_STRUCT_VERSION,
            KADM5_API_VERSION_4, db_args, &handle);
    }
    if (retval)
    {
...
void kadmin_renameprinc(int argc, char* argv[])
{
    ...
    {
        com_err("rename_principal", retval,
                _("while renaming principal \"%s\" to \"%s\""), ocanon, ncanon);
        goto cleanup;
    }
...
void kadmin_cpw(int argc, char* argv[])
{
    ...
    for (argv++, argc--; argc > 1; argc--, argv++)
    {
        ...
        else if (!strcmp("-e", *argv))
        {
            ...
            retval = krb5_string_to_keysalts(*++argv, NULL, NULL, 0, &ks_tuple,
                                             &n_ks_tuple);
            if (retval)
            {
...
static void add_tl_data(krb5_int16* n_tl_datap, krb5_tl_data** tl_datap,
...
static void unlock_princ(kadm5_principal_ent_t princ, long* mask,
                         const char* caller)
{
    krb5_error_code retval;
    krb5_timestamp now;
    ...
    *mask |= KADM5_FAIL_AUTH_COUNT;
    ...
    add_tl_data(&princ->n_tl_data, &princ->tl_data, KRB5_TL_LAST_ADMIN_UNLOCK,
                4, timebuf);
    *mask |= KADM5_TL_DATA;
}
 ...
 * filled in on error.
 */
static int kadmin_parse_princ_args(int argc, char* argv[],
                                   kadm5_principal_ent_t oprinc, long* mask,
                                   char** pass, krb5_boolean* randkey,
                                   krb5_boolean* nokey,
    ...
    *nokey = FALSE;
    for (i = 1; i < argc - 1; i++)
    {
        if (!strcmp("-x", argv[i]))
        {
            ...
            add_tl_data(&oprinc->n_tl_data, &oprinc->tl_data, KRB5_TL_DB_ARGS,
                        strlen(argv[i]) + 1, (krb5_octet*)argv[i]);
            *mask |= KADM5_TL_DATA;
            continue;
        }
        ...
        if (!strcmp("-e", argv[i]))
        {
            if (++i > argc - 2)
                return -1;
            retval = krb5_string_to_keysalts(argv[i], NULL, NULL, 0, ks_tuple,
                                             n_ks_tuple);
            if (retval)
            {
...
void kadmin_addprinc(int argc, char* argv[])
{
    ...
    else if (!(mask & KADM5_POLICY_CLR))
    {
        ...
        if (policy_exists("default"))
        {
            ...
            {
                fprintf(stderr,
                        _("NOTICE: no policy specified for %s; "
                          "assigning \"default\"\n"),
                        canon);
            }
        ...
        {
            fprintf(stderr,
                    _("WARNING: no policy specified for %s; "
...
void kadmin_getprinc(int argc, char* argv[])
{
    ...
    if (argc == 2)
    {
        ...
        printf(_("Expiration date: %s\n"),
               dprinc.princ_expire_time ? strdate(dprinc.princ_expire_time)
                                        : _("[never]"));
        printf(_("Last password change: %s\n"),
               dprinc.last_pwd_change ? strdate(dprinc.last_pwd_change)
                                      : _("[never]"));
        printf(_("Password expiration date: %s\n"),
               dprinc.pw_expiration ? strdate(dprinc.pw_expiration)
                                    : _("[never]"));
        ...
        for (i = 0; i < dprinc.n_key_data; i++)
        {
            ...
            if (krb5_enctype_to_name(key_data->key_data_type[0], FALSE, enctype,
                                     sizeof(enctype)))
    ...
    else
    {
        printf("\"%s\"\t%d\t%d\t%d\t%d\t\"%s\"\t%d\t%d\t%d\t%d\t\"%s\""
               ...
               dprinc.mod_date, dprinc.attributes, dprinc.kvno, dprinc.mkvno,
               dprinc.policy ? dprinc.policy : "[none]",
               dprinc.max_renewable_life, dprinc.last_success,
               dprinc.last_failed, dprinc.fail_auth_count, dprinc.n_key_data);
        for (i = 0; i < dprinc.n_key_data; i++)
            printf("\t%d\t%d\t%d\t%d", dprinc.key_data[i].key_data_ver,
...
static int kadmin_parse_policy_args(int argc, char* argv[],
...
void kadmin_getpol(int argc, char* argv[])
{
    ...
    else
    {
        ...
        printf(
            "\"%s\"\t%ld\t%ld\t%ld\t%ld\t%ld\t0\t%lu\t%ld\t%ld\t%s\n",
            policy.policy, policy.pw_max_life, policy.pw_min_life,
            policy.pw_min_length, policy.pw_min_classes, policy.pw_history_num,
            (unsigned long)policy.pw_max_fail, (long)policy.pw_failcnt_interval,
            (long)policy.pw_lockout_duration,
            (policy.allowed_keysalts == NULL) ? "-" : policy.allowed_keysalts);
    }
