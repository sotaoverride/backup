...
static const char doc[] = PROGRAM_NAME
    " version " VERSION " - a tool to format MTD devices and flash UBI images";
...
static const char usage[] =
    "Usage: " PROGRAM_NAME
    " <MTD device node file name> [-s <bytes>] [-O <offs>] [-n]\n"
    "\t\t\t[-Q <num>] [-f <file>] [-S <bytes>] [-e <value>] [-x <num>] [-y] "
    "[-q] [-v] [-h]\n"
    "\t\t\t[--sub-page-size=<bytes>] [--vid-hdr-offset=<offs>] "
    "[--no-volume-table]\n"
    "\t\t\t[--flash-image=<file>] [--image-size=<bytes>] "
    "[--erase-counter=<value>]\n"
    "\t\t\t[--image-seq=<num>] [--ubi-ver=<num>] [--yes] [--quiet] "
    "[--verbose]\n"
    "\t\t\t[--help] [--version]\n\n"
    "Example 1: " PROGRAM_NAME
    " /dev/mtd0 -y - format MTD device number 0 and do\n"
    "           not ask questions.\n"
    "Example 2: " PROGRAM_NAME
    " /dev/mtd0 -q -e 0 - format MTD device number 0,\n"
...
static int parse_opt(int argc, char* const argv[])
{
    ...
    while (1)
    {
        ...
        key =
            getopt_long(argc, argv, "nh?Vyqve:x:s:O:f:S:", long_options, NULL);
        if (key == -1)
            break;
        ...
        switch (key)
        {
            ...
            case 'e':
                ...
                if (args.ec >= EC_MAX)
                    return errmsg("too high erase %llu, counter, max is %u",
                                  args.ec, EC_MAX);
                args.override_ec = 1;
                break;
            ...
            case 'Q':
                ...
                if (error || image_seq > 0xFFFFFFFF)
                    return errmsg("bad UBI image sequence number: \"%s\"",
                                  optarg);
    ...
    if (args.quiet && args.verbose)
        return errmsg(
            "using \"-q\" and \"-v\" at the same time does not make sense");
...
static int open_file(off_t* sz)
{
    ...
    if (!strcmp(args.image, "-"))
    {
        if (args.image_sz == 0)
            return errmsg(
                "must use '-S' with non-zero value when reading from stdin");
                       ...
                       const struct ubigen_info* ui, struct ubi_scan_info* si)
{
    ...
    {
        sys_errmsg("file \"%s\" is too large (%lld bytes)", args.image,
                   (long long)st_size);
        goto out_close;
    }
    ...
    {
        return sys_errmsg("file \"%s\" (size %lld bytes) is not multiple of "
    ...
    for (eb = 0; eb < mtd->eb_cnt; eb++)
    {
        ...
        if (!args.quiet && !args.verbose)
        {
            printf("\r" PROGRAM_NAME
                   ": flashing eraseblock %d -- %2lld %% complete  ",
        ...
        if (err)
        {
            errmsg("bad EC header at eraseblock %d of \"%s\"", written_ebs,
                   args.image);
            goto out_close;
        }
        ...
        err = mtd_write(libmtd, mtd, args.node_fd, eb, 0, buf, new_len, NULL, 0,
                        0);
        if (err)
        {
                  ...
                  int start_eb, int novtbl)
{
    ...
    for (eb = start_eb; eb < mtd->eb_cnt; eb++)
    {
        ...
        {
            printf("\r" PROGRAM_NAME
                   ": formatting eraseblock %d -- %2lld %% complete  ",
                   eb,
                   (long long)(eb + 1 - start_eb) * 100 /
                       (mtd->eb_cnt - start_eb));
            fflush(stdout);
        }
        ...
        err = mtd_write(libmtd, mtd, args.node_fd, eb, 0, hdr, write_size, NULL,
                        0, 0);
        if (err)
        {
            if (!args.quiet && !args.verbose)
                printf("\n");
            sys_errmsg(
                "cannot write EC header (%d bytes buffer) to eraseblock %d",
                write_size, eb);
    ...
    verbose(args.verbose, "write volume table to eraseblocks %d and %d", eb1,
            eb2);
    vtbl = ubigen_create_empty_vtbl(ui);
    if (!vtbl)
        goto out_free;

    err = ubigen_write_layout_vol(ui, eb1, eb2, ec1, ec2, vtbl, args.node_fd);
    free(vtbl);
    if (err)
    {
...
int main(int argc, char* const argv[])
{
    ...
    if (args.vid_hdr_offs != 0)
    {
        ...
        {
            errmsg(
                "VID header offset has to be multiple of min. I/O unit size");
            goto out_close;
        }
    ...
    if (libubi)
    {
        ...
        if (!err)
        {
            errmsg("please, first detach mtd%d (%s) from ubi%d", mtd.mtd_num,
                   args.node, ubi_dev_num);
            goto out_close;
        }
    }
    ...
    if (si->good_cnt < 2)
    {
        errmsg("too few non-bad eraseblocks (%d) on mtd%d", si->good_cnt,
               mtd.mtd_num);
        goto out_free;
    }
    ...
    if (!args.quiet)
    {
        if (si->ok_cnt)
            normsg(
                "%d eraseblocks have valid erase counter, mean value is %lld",
    ...
    if (si->alien_cnt)
    {
        if (!args.yes || !args.quiet)
            warnmsg("%d of %d eraseblocks contain non-UBI data", si->alien_cnt,
                    si->good_cnt);
    ...
    if (!args.override_ec && si->empty_cnt < si->good_cnt)
    {
        ...
        if (percent < 50)
        {
            if (!args.yes || !args.quiet)
            {
                ...
                normsg("note, arbitrary erase counter value may be specified "
                       "using -e option");
            }
            if (!args.yes && want_exit())
            {
        ...
        else if (percent < 95)
        {
            if (!args.yes || !args.quiet)
            {
                warnmsg("only %d of %d eraseblocks have valid erase counter",
                        si->ok_cnt, si->good_cnt);
                normsg("mean erase counter %lld will be used for the rest of "
                       "eraseblock",
                       si->mean_ec);
            }
            if (!args.yes && want_exit())
    ...
    if (si->vid_hdr_offs != -1 && ui.vid_hdr_offs != si->vid_hdr_offs)
    {
        ...
        if (!args.yes || !args.quiet)
        {
            warnmsg("VID header and data offsets on flash are %d and %d, "
                    ...
                    ui.data_offs);
            normsg_cont("use new offsets %d and %d? ", ui.vid_hdr_offs,
                        ui.data_offs);
        }
        if (args.yes || answer_is_yes(NULL))
        {
            ...
            ubigen_info_init(&ui, mtd.eb_size, mtd.min_io_size, 0,
                             si->vid_hdr_offs, args.ubi_ver, args.image_seq);
