...
#else
                BOOST_REGEX_NOEH_ASSERT((0 == std::ferror(hfile)) &&
                                        (read_size != 0));
#endif
            }
        }
...
{
#ifdef BOOST_NO_ANSI_APIS
    if (::WideCharToMultiByte(CP_ACP, 0, data.cFileName, -1, path, max_size,
                              NULL, NULL) == 0)
...
file_iterator::file_iterator(const char* wild)
{
    ...
    try
    {
        ...
        {
            copy_find_file_result_with_overflow_check(
                ref->_data, ptr, (MAX_PATH - (ptr - _path)));
...
directory_iterator& directory_iterator::
    operator=(const directory_iterator& other)
{
    BOOST_REGEX_DETAIL_NS::overflow_error_if_not_zero(
        BOOST_REGEX_DETAIL_NS::strcpy_s(_root, MAX_PATH, other._root));
    BOOST_REGEX_DETAIL_NS::overflow_error_if_not_zero(
    ...
    while (*mask && *name)
    {
        switch (*mask)
        {
            ...
            case '.':
                ...
                // fall through
            default:
                if (BOOST_REGEX_FI_TRANSLATE(*mask) !=
...
unsigned _fi_attributes(const char* root, const char* name)
{
    ...
    if (std::strlen(root) + std::strlen(_fi_sep) + std::strlen(name) >=
        MAX_PATH)
        return 0;
    int r;
    if (((root[0] == *_fi_sep) || (root[0] == *_fi_sep_alt)) &&
        (root[1] == '\0'))
