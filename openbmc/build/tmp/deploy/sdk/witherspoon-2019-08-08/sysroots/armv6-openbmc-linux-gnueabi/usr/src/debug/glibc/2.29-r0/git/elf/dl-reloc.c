...
int _dl_try_allocate_static_tls(struct link_map* map)
{
    ...
    size_t offset =
        (ALIGN_UP(GL(dl_tls_static_used) - map->l_tls_firstbyte_offset,
                  map->l_tls_align) +
         map->l_tls_firstbyte_offset);
...
void __attribute_noinline__ _dl_allocate_static_tls(struct link_map* map)
{
    if (map->l_tls_offset == FORCED_DYNAMIC_TLS_OFFSET ||
       ...
       long enough to relocate them.  */
        ...
        for (ph = l->l_phdr; ph < &l->l_phdr[l->l_phnum]; ++ph)
            if (ph->p_type == PT_LOAD && (ph->p_flags & PF_W) == 0)
            {
                ...
                newp->len =
                    ALIGN_UP(ph->p_vaddr + ph->p_memsz, GLRO(dl_pagesize)) -
                ...
                if (__mprotect(newp->start, newp->len,
                               newp->prot | PROT_WRITE) < 0)
                {
                    errstring =
         ...
         ? ((__builtin_expect((*ref) == l->l_lookup_cache.sym, 0) &&           \
             elf_machine_type_class(r_type) == l->l_lookup_cache.type_class)   \
                ? (bump_num_cache_relocations(),                               \
                   (*ref) = l->l_lookup_cache.ret, l->l_lookup_cache.value)    \
                : ({                                                           \
                      lookup_t _lr;                                            \
                      ...
                      if ((version) != NULL && (version)->hash != 0)           \
                          v = (version);                                       \
                      _lr = _dl_lookup_symbol_x(                               \
                          strtab + (*ref)->st_name, l, (ref), scope, v, _tc,   \
...
{
    ElfW(Addr) start =
        ALIGN_DOWN((l->l_addr + l->l_relro_addr), GLRO(dl_pagesize));
    ElfW(Addr) end = ALIGN_DOWN((l->l_addr + l->l_relro_addr + l->l_relro_size),
                                GLRO(dl_pagesize));
    if (start != end && __mprotect((void*)start, end - start, PROT_READ) < 0)
    {
...
void __attribute_noinline__ _dl_reloc_bad_type(struct link_map* map,
...
#define DIGIT(b) _itoa_lower_digits[(b)&0xf];
    ...
    static const char msg[2][32
                             ...
                             + 6
#endif
    ] = {"unexpected reloc type 0x", "unexpected PLT reloc type 0x"};
