...
static const char* ignored_types[] = {"ignore", "iso9660", "sw", NULL};

static const char* really_wanted[] = {"minix",   "ext2", "ext3",    "ext4",
                                      "ext4dev", "jfs",  "reiserfs"};
...
struct fsck_fs_data
{
    const char* device;
    dev_t disk;
    unsigned int stacked : 1, done : 1, eval_device : 1;
};
...
static int is_irrotational_disk(dev_t disk)
{
    ...
    rc = snprintf(path, sizeof(path), "/sys/dev/block/%d:%d/queue/rotational",
...
static void lock_disk(struct fsck_instance* inst)
{
    ...
    {
        int rc =
            mkdir(FSCK_RUNTIME_DIRNAME, S_IWUSR | S_IRUSR | S_IRGRP | S_IROTH |
                                            S_IXUSR | S_IXGRP | S_IXOTH);
        if (rc && errno != EEXIST)
        {
            warn(_("cannot create directory %s"), FSCK_RUNTIME_DIRNAME);
            goto done;
        }
    }
    ...
    if (inst->lock >= 0)
    {
        ...
        if (verbose && (rc = flock(inst->lock, LOCK_EX | LOCK_NB)) != 0 &&
            errno == EWOULDBLOCK)
            printf(_("(waiting) "));
...
static void print_stats(struct fsck_instance* inst)
{
    ...
    if (report_stats_file)
        fprintf(report_stats_file,
                "%s %d %ld "
                "%ld.%06ld %ld.%06ld %ld.%06ld\n",
                fs_get_device(inst->fs), inst->exit_status,
                inst->rusage.ru_maxrss, (long)delta.tv_sec, (long)delta.tv_usec,
 ...
 */
static int execute(const char* progname, const char* progpath, const char* type,
                   struct libmnt_fs* fs, int interactive)
{
    char* argv[80];
    int argc, i;
    ...
    if (progress &&
        ((strcmp(type, "ext2") == 0) || (strcmp(type, "ext3") == 0) ||
         (strcmp(type, "ext4") == 0) || (strcmp(type, "ext4dev") == 0)))
    {
...
static struct fsck_instance* wait_one(int flags)
{
    ...
    do
    {
        ...
        for (prev = NULL, inst = instance_list; inst;
    ...
    {
        warnx(_("%s %s: status is %x, should never happen."), inst->prog,
              fs_get_device(inst->fs), status);
        status = FSCK_EX_ERROR;
    }
    ...
    if (progress && (inst->flags & FLAG_PROGRESS) && !progress_active())
    {
        for (inst2 = instance_list; inst2; inst2 = inst2->next)
        {
            ...
            if (strcmp(inst2->type, "ext2") && strcmp(inst2->type, "ext3") &&
                strcmp(inst2->type, "ext4") && strcmp(inst2->type, "ext4dev"))
                continue;
            /*
...
static int fsck_device(struct libmnt_fs* fs, int interactive)
{
    ...
    return 0;
err:
    warnx(_("error %d (%m) while executing fsck.%s for %s"), retval, type,
          fs_get_device(fs));
...
static void compile_fs_type(char* fs_type, struct fs_type_compile* cmp)
{
    ...
    while (s)
    {
        ...
        else
        {
            ...
            if ((negate && !cmp->negate) || (!negate && cmp->negate))
            {
                errx(FSCK_EX_USAGE, _("Either all or none of the filesystem "
                                      "types passed to -t must be prefixed\n"
                                      "with 'no' or '!'."));
            }
        }
...
static int ignore(struct libmnt_fs* fs)
{
    ...
    {
        if (fs_check_required(type))
            warnx(_("cannot check %s: fsck.%s not found"), fs_get_device(fs),
                  type);
        return 1;
    }
...
static int count_slaves(dev_t disk)
{
    ...
    snprintf(dirname, sizeof(dirname), "/sys/dev/block/%u:%u/slaves/",
             major(disk), minor(disk));
...
static int check_all(void)
{
    ...
    while (not_done_yet)
    {
        ...
        status |= wait_many(pass_done ? FLAG_WAIT_ALL : FLAG_WAIT_ATLEAST_ONE);
        if (pass_done)
        {
            if (verbose > 1)
                printf("----------------------------------\n");
...
static void parse_argv(int argc, char* argv[])
{
        ...
        if ((arg[0] == '/' && !opts_for_fsck) || strchr(arg, '='))
        {
            ...
            if (!dev && strchr(arg, '='))
            {
                ...
                if (geteuid())
                    ...
                    errx(FSCK_EX_ERROR,
                         _("couldn't find matching filesystem: %s"), arg);
            }
        ...
        for (j = 1; arg[j]; j++)
        {
            ...
            switch (arg[j])
            {
                ...
                case 'r':
                    ...
                    if (arg[j + 1])
                    { /* -r<fd> */
                        report_stats_fd = strtou32_or_err(
                            arg + j + 1, _("invalid argument of -r"));
                        goto next_arg;
                    }
                    ...
                    { /* -r <fd> */
                        report_stats_fd = strtou32_or_err(
                            argv[i + 1], _("invalid argument of -r"));
                        ++i;
                        goto next_arg;
                    }
                    break;
    ...
    if (report_stats_fd >= 0)
    {
        ...
        if (!report_stats_file)
            err(FSCK_EX_ERROR, _("invalid argument of -r: %d"),
                report_stats_fd);
    }
