...
rfbBool rfbSendCursorShape(rfbClientPtr cl)
{
    ...
    if (pCursor && pCursor->width == 1 && pCursor->height == 1 &&
        pCursor->mask[0] == 0)
    {
        pCursor = NULL;
    }
    ...
    dataBytes =
        (cl->useRichCursorEncoding)
            ? (pCursor->width * pCursor->height * (cl->format.bitsPerPixel / 8))
            : maskBytes;

    /* Send buffer contents if needed. */

    if (cl->ublen + sz_rfbFramebufferUpdateRectHeader + sz_rfbXCursorColors +
            maskBytes + dataBytes >
        UPDATE_BUF_SIZE)
    {
        if (!rfbSendUpdateBuf(cl))
    ...
    else
    {
        ...
        (*cl->translateFn)(
            ...
            (char*)pCursor->richSource, &cl->updateBuf[cl->ublen],
    ...
    rfbStatRecordEncodingSent(
        cl,
        (cl->useRichCursorEncoding ? rfbEncodingRichCursor
                                   : rfbEncodingXCursor),
        sz_rfbFramebufferUpdateRectHeader + (cl->ublen - saved_ublen),
        sz_rfbFramebufferUpdateRectHeader + (cl->ublen - saved_ublen));
...
rfbBool rfbSendCursorPos(rfbClientPtr cl)
{
    ...
    rfbStatRecordEncodingSent(cl, rfbEncodingPointerPos,
                              sz_rfbFramebufferUpdateRectHeader,
                              sz_rfbFramebufferUpdateRectHeader);
...
void rfbConvertLSBCursorBitmapOrMask(int width, int height,
...
rfbCursorPtr rfbMakeXCursor(int width, int height, char* cursorString,
                            char* maskString)
{
    int i, j, w = (width + 7) / 8;
    ...
    for (j = 0, cp = cursorString; j < height; j++)
        for (i = 0, bit = 0x80; i < width;
             i++, bit = (bit & 1) ? 0x80 : bit >> 1, cp++)
            if (*cp != ' ')
    ...
    if (maskString)
    {
        ...
        for (j = 0, cp = maskString; j < height; j++)
            for (i = 0, bit = 0x80; i < width;
                 i++, bit = (bit & 1) ? 0x80 : bit >> 1, cp++)
                if (*cp != ' ')
                    cursor->mask[j * w + i / 8] |= bit;
    }
    else
        cursor->mask = (unsigned char*)rfbMakeMaskForXCursor(
            width, height, (char*)cursor->source);
...
char* rfbMakeMaskFromAlphaSource(int width, int height,
                                  ...
                                  rfbCursorPtr cursor)
{
    ...
    background =
        ((format->redMax * cursor->backRed) / 0xffff) << format->redShift |
        ((format->greenMax * cursor->backGreen) / 0xffff)
            << format->greenShift |
        ((format->blueMax * cursor->backBlue) / 0xffff) << format->blueShift;

#define SETRGB(u)                                                              \
    r = (255 * (((format->redMax << format->redShift) & (*u)) >>               \
                format->redShift)) /                                           \
        format->redMax;                                                        \
    g = (255 * (((format->greenMax << format->greenShift) & (*u)) >>           \
                format->greenShift)) /                                         \
        format->greenMax;                                                      \
    b = (255 * (((format->blueMax << format->blueShift) & (*u)) >>             \
                format->blueShift)) /                                          \
        format->blueMax;
    ...
    {
        for (i = 0, bit = 0x80; i < cursor->width;
             i++, bit = (bit & 1) ? 0x80 : bit >> 1)
        {
            if (interp)
            {
            ...
            else if (memcmp(cursor->richSource + j * width + i * bpp, back,
                            bpp))
            {
                cursor->source[j * w + i / 8] |= bit;
            }
                                  ...
                                  rfbCursorPtr cursor)
{
    ...
    cp = cursor->richSource =
        (unsigned char*)calloc(cursor->width * bpp, cursor->height);
    ...
    background = (uint32_t)cursor->backRed << format->redShift |
                 (uint32_t)cursor->backGreen << format->greenShift |
                 (uint32_t)cursor->backBlue << format->blueShift;
    foreground = (uint32_t)cursor->foreRed << format->redShift |
                 (uint32_t)cursor->foreGreen << format->greenShift |
                 (uint32_t)cursor->foreBlue << format->blueShift;

    for (j = 0; j < cursor->height; j++)
        for (i = 0, bit = 0x80; i < cursor->width;
             i++, bit = (bit & 1) ? 0x80 : bit >> 1, cp += bpp)
            if (cursor->source[j * w + i / 8] & bit)
                memcpy(cp, fore, bpp);
            else
                memcpy(cp, back, bpp);
}
...
void rfbHideCursor(rfbClientPtr cl)
{
    ...
    for (j = 0; j < y2; j++)
        memcpy(s->frameBuffer + (y1 + j) * rowstride + x1 * bpp,
               s->underCursorBuffer + j * x2 * bpp, x2 * bpp);
...
void rfbShowCursor(rfbClientPtr cl)
{
    ...
    else
    {
        ...
        for (j = 0; j < y2; j++)
            for (i = 0; i < x2; i++)
                if ((c->mask[(j + j1) * w + (i + i1) / 8] << ((i + i1) & 7)) &
                    0x80)
                    memcpy(s->frameBuffer + (j + y1) * rowstride +
                               (i + x1) * bpp,
                           c->richSource + (j + j1) * c->width * bpp +
                               (i + i1) * bpp,
                           bpp);
    }
...
static void rfbPrintXCursor(rfbCursorPtr cursor)
{
    ...
    for (j = 0; j < cursor->height; j++)
    {
        for (i = 0, i1 = 0, bit = 0x80; i1 < cursor->width;
