...
BOOST_CXX14_CONSTEXPR bool all_of(InputIterator first, InputIterator last,
                                  Predicate p)
{
...
BOOST_CXX14_CONSTEXPR bool all_of_equal(InputIterator first, InputIterator last,
                                        const T& val)
{
    for (; first != last; ++first)
        if (val != *first)
