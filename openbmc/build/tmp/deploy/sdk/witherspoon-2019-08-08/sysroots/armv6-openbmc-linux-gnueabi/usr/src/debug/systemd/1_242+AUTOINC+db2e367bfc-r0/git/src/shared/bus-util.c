...
int bus_async_unregister_and_exit(sd_event* e, sd_bus* bus, const char* name)
{
    ...
    match = strjoina("sender='org.freedesktop.DBus',"
                     "type='signal',"
                     "interface='org.freedesktop.DBus',"
                     "member='NameOwnerChanged',"
                     "path='/org/freedesktop/DBus',"
                     "arg0='",
                     name, "',", "arg1='", unique, "',", "arg2=''");

    r = sd_bus_add_match_async(bus, NULL, match, name_owner_change_callback,
                               NULL, e);
    if (r < 0)
        return r;
...
int bus_event_loop_with_idle(sd_event* e, sd_bus* bus, const char* name,
                             usec_t timeout, check_idle_t check_idle,
                             void* userdata)
{
    bool exiting = false;
    int r, code;
...
int bus_name_has_owner(sd_bus* c, const char* name, sd_bus_error* error)
{
    ...
    r = sd_bus_call_method(c, "org.freedesktop.DBus", "/org/freedesktop/dbus",
                           "org.freedesktop.DBus", "NameHasOwner", error, &rep,
                           "s", name);
    if (r < 0)
        return r;
...
static int check_good_user(sd_bus_message* m, uid_t good_user)
{
    ...
    assert_return(
        (sd_bus_creds_get_augmented_mask(creds) & SD_BUS_CREDS_EUID) == 0,
        -EPERM);
...
int bus_test_polkit(sd_bus_message* call, int capability, const char* action,
                    const char** details, uid_t good_user, bool* _challenge,
                    sd_bus_error* e)
{
    ...
    else
    {
        ...
        r = sd_bus_message_new_method_call(
            call->bus, &request, "org.freedesktop.PolicyKit1",
            "/org/freedesktop/PolicyKit1/Authority",
            "org.freedesktop.PolicyKit1.Authority", "CheckAuthorization");
        if (r < 0)
            return r;

        r = sd_bus_message_append(request, "(sa{sv})s", "system-bus-name", 1,
                                  "name", "s", sender, action);
        if (r < 0)
            return r;
...
int bus_verify_polkit_async(sd_bus_message* call, int capability,
                            ...
                            bool interactive, uid_t good_user,
                            Hashmap** registry, sd_bus_error* error)
{
    ...
    if (q)
    {
        ...
        if (challenge)
            return sd_bus_error_set(
                error, SD_BUS_ERROR_INTERACTIVE_AUTHORIZATION_REQUIRED,
    ...
    r = sd_bus_message_new_method_call(
        call->bus, &pk, "org.freedesktop.PolicyKit1",
        "/org/freedesktop/PolicyKit1/Authority",
        "org.freedesktop.PolicyKit1.Authority", "CheckAuthorization");
    if (r < 0)
        return r;

    r = sd_bus_message_append(pk, "(sa{sv})s", "system-bus-name", 1, "name",
                              "s", sender, action);
    if (r < 0)
        return r;
...
int bus_print_property_value(const char* name, const char* expected_value,
                             bool only_value, const char* value)
{
    assert(name);
...
int bus_print_property_valuef(const char* name, const char* expected_value,
                              bool only_value, const char* fmt, ...)
{
    va_list ap;
    int r;
...
static int bus_print_property(const char* name, const char* expected_value,
                              sd_bus_message* m, bool value, bool all)
{
    ...
    switch (type)
    {
        ...
        case SD_BUS_TYPE_UINT64:
        {
                                ...
                                "AmbientCapabilities"))
            {
                ...
                bus_print_property_value(name, expected_value, value, s);
                      ...
                      u == CGROUP_WEIGHT_INVALID) ||
                     (STR_IN_SET(name, "CPUShares", "StartupCPUShares") &&
                      u == CGROUP_CPU_SHARES_INVALID) ||
                     (STR_IN_SET(name, "BlockIOWeight",
                                 "StartupBlockIOWeight") &&
                      u == CGROUP_BLKIO_WEIGHT_INVALID) ||
                     (STR_IN_SET(name, "MemoryCurrent", "TasksCurrent") &&
                      u == (uint64_t)-1) ||
                ...
                bus_print_property_value(name, expected_value, value,
                                 ...
                                 "MemorySwapMax", "MemoryLimit") &&
                      u == CGROUP_LIMIT_MAX) ||
                     (STR_IN_SET(name, "TasksMax", "DefaultTasksMax") &&
                      u == (uint64_t)-1) ||
                ...
                bus_print_property_value(name, expected_value, value,
                                         "infinity");
            else if (STR_IN_SET(name, "IPIngressBytes", "IPIngressPackets",
                                "IPEgressBytes", "IPEgressPackets") &&
                     u == (uint64_t)-1)
                bus_print_property_value(name, expected_value, value,
                                         "[no data]");
            else
        ...
        case SD_BUS_TYPE_INT64:
        {
            ...
            bus_print_property_valuef(name, expected_value, value, "%" PRIi64,
                                      i);
            return 1;
        }
        ...
        case SD_BUS_TYPE_UINT32:
        {
            ...
            if (strstr(name, "UMask") || strstr(name, "Mode"))
                bus_print_property_valuef(name, expected_value, value, "%04o",
                                          u);
        ...
        case SD_BUS_TYPE_INT32:
        {
            ...
            bus_print_property_valuef(name, expected_value, value, "%" PRIi32,
                                      i);
            return 1;
        }
        ...
        case SD_BUS_TYPE_ARRAY:
            if (streq(contents, "s"))
            {
                ...
                while ((r = sd_bus_message_read_basic(m, SD_BUS_TYPE_STRING,
                                                      &str)) > 0)
                {
                    bool good;
            ...
            else if (streq(contents, "y"))
            {
                ...
                r = sd_bus_message_read_array(m, SD_BUS_TYPE_BYTE,
                                              (const void**)&u, &n);
                if (r < 0)
                    return r;
            ...
            else if (streq(contents, "u"))
            {
                ...
                r = sd_bus_message_read_array(m, SD_BUS_TYPE_UINT32,
                                              (const void**)&u, &n);
                if (r < 0)
                    return r;
...
int bus_message_print_all_properties(sd_bus_message* m,
                                     bus_message_print_t func, char** filter,
                                     bool value, bool all,
...
int bus_print_all_properties(sd_bus* bus, const char* dest, const char* path,
                             bus_message_print_t func, char** filter,
                             bool value, bool all, Set** found_properties)
{
    ...
    r = sd_bus_call_method(bus, dest, path, "org.freedesktop.DBus.Properties",
                           "GetAll", &error, &reply, "s", "");
    if (r < 0)
        return r;

    return bus_message_print_all_properties(reply, func, filter, value, all,
                                            found_properties);
}
...
static int map_basic(sd_bus* bus, const char* member, sd_bus_message* m,
                     unsigned flags, sd_bus_error* error, void* userdata)
{
    char type;
    int r;
...
int bus_message_map_all_properties(sd_bus_message* m,
                                   ...
                                   unsigned flags, sd_bus_error* error,
                                   void* userdata)
{
                                               ...
                                               "sv")) > 0)
    {
        ...
        if (prop)
        {
            ...
            if (map[i].set)
                ...
                r = map_basic(sd_bus_message_get_bus(m), member, m, flags,
                              error, v);
            if (r < 0)
                return r;
...
int bus_map_all_properties(sd_bus* bus, const char* destination,
                           const char* path,
                           const struct bus_properties_map* map, unsigned flags,
                           ...
                           void* userdata)
{
    ...
    r = sd_bus_call_method(bus, destination, path,
                           "org.freedesktop.DBus.Properties", "GetAll", error,
                           &m, "s", "");
    if (r < 0)
        return r;
...
int bus_connect_transport(BusTransport transport, const char* host, bool user,
                          sd_bus** ret)
{
    _cleanup_(sd_bus_close_unrefp) sd_bus* bus = NULL;
    int r;
    ...
    switch (transport)
    {
        ...
        case BUS_TRANSPORT_LOCAL:
            ...
            else
            {
                if (sd_booted() <= 0)
                {
                    /* Print a friendly message when the local system is
                     * actually not running systemd as PID 1. */
                    log_error("System has not been booted with systemd as init "
...
int bus_connect_transport_systemd(BusTransport transport, const char* host,
...
int bus_property_set_bool(sd_bus* bus, const char* path, const char* interface,
...
int bus_property_get_id128(sd_bus* bus, const char* path, const char* interface,
                           const char* property, sd_bus_message* reply,
                           void* userdata, sd_bus_error* error)
{
 ...
 */
int bus_path_encode_unique(sd_bus* b, const char* prefix, const char* sender_id,
 ...
 */
int bus_path_decode_unique(const char* path, const char* prefix,
                           char** ret_sender, char** ret_external)
{
    const char *p, *q;
                            ...
                            sd_bus_error* error)
{
    ...
    /* rlim_t might have different sizes, let's map RLIMIT_INFINITY to
     * (uint64_t) -1, so that it is the same on all archs */
...
int bus_open_system_watch_bind_with_description(sd_bus** ret,
                                                const char* description)
{
    ...
    /* Match like sd_bus_open_system(), but with the "watch_bind" feature and
     * the Connected() signal turned on. */
...
int bus_reply_pair_array(sd_bus_message* m, char** l)
{
    ...
    /* Reply to the specified message with a message containing a dictionary put
     * together from the specified strv */
