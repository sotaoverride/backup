...
static const char* const pixel_modes[] = {"none",
                                          "monochrome bitmap",
                                          "gray 8-bit bitmap",
                                          "gray 2-bit bitmap",
                                          "gray 4-bit bitmap",
...
FT_Stream_New(FT_Library library, const FT_Open_Args* args, FT_Stream* astream)
{
    ...
    *astream = NULL;
    ...
    if (args->flags & FT_OPEN_MEMORY)
    {
        ...
        FT_Stream_OpenMemory(stream, (const FT_Byte*)args->memory_base,
                           ...
                           const FT_Vector* origin)
{
    ...
    switch (mode)
    {
        case FT_RENDER_MODE_MONO:
            ...
            {
                if (((cbox.xMin + 31) & 63) - 31 + ((cbox.xMax + 32) & 63) -
                        32 <
                    0)
                    pbox.xMin -= 1;
                else
            ...
            {
                if (((cbox.yMin + 31) & 63) - 31 + ((cbox.yMax + 32) & 63) -
                        32 <
                    0)
                    pbox.yMin -= 1;
                else
    ...
    if (pbox.xMin < -0x8000 || pbox.xMax > 0x7FFF || pbox.yMin < -0x8000 ||
        pbox.yMax > 0x7FFF)
    {
        FT_TRACE3(("ft_glyphslot_preset_bitmap: [%ld %ld %ld %ld]\n", pbox.xMin,
                   pbox.yMin, pbox.xMax, pbox.yMax));
        return 1;
    }
...
FT_Set_Transform(FT_Face face, FT_Matrix* matrix, FT_Vector* delta)
{
    ...
    if ((matrix->xy | matrix->yx) || matrix->xx != 0x10000L ||
        matrix->yy != 0x10000L)
...
static void ft_glyphslot_grid_fit_metrics(FT_GlyphSlot slot, FT_Bool vertical)
{
    ...
    if (vertical)
    {
        ...
        right =
            FT_PIX_CEIL_LONG(ADD_LONG(metrics->vertBearingX, metrics->width));
        bottom =
            FT_PIX_CEIL_LONG(ADD_LONG(metrics->vertBearingY, metrics->height));
    ...
    else
    {
        ...
        right =
            FT_PIX_CEIL_LONG(ADD_LONG(metrics->horiBearingX, metrics->width));
...
FT_Load_Glyph(FT_Face face, FT_UInt glyph_index, FT_Int32 load_flags)
{
    ...
    if (hinter && !(load_flags & FT_LOAD_NO_HINTING) &&
        !(load_flags & FT_LOAD_NO_AUTOHINT) && FT_IS_SCALABLE(face) &&
        !FT_IS_TRICKY(face) &&
          ...
          face->internal->transform_matrix.yx != 0)))
    {
        ...
        else
        {
            ...
            if ((mode == FT_RENDER_MODE_LIGHT &&
                 (!FT_DRIVER_HINTS_LIGHTLY(driver) && !is_light_type1)) ||
    ...
    if (autohint)
    {
        ...
        {
            error = driver->clazz->load_glyph(slot, face->size, glyph_index,
        ...
        if (slot->format == FT_GLYPH_FORMAT_OUTLINE)
        {
            ...
            if (!(load_flags & FT_LOAD_NO_HINTING))
                ft_glyphslot_grid_fit_metrics(
                    slot, FT_BOOL(load_flags & FT_LOAD_VERTICAL_LAYOUT));
#endif
        }
    }
...
Load_Ok:
        ...
        if (internal->transform_flags)
        {
            ...
            if (renderer)
                error = renderer->clazz->transform_glyph(
                    renderer, slot, &internal->transform_matrix,
    ...
    if (!error && (load_flags & FT_LOAD_NO_SCALE) == 0 &&
    ...
    FT_TRACE5(("  bitmap %dx%d, %s (mode %d)\n", slot->bitmap.width,
               slot->bitmap.rows, pixel_modes[slot->bitmap.pixel_mode],
...
static void destroy_face(FT_Memory memory, FT_Face face, FT_Driver driver)
{
    ...
    FT_List_Finalize(&face->sizes_list, (FT_List_Destructor)destroy_size,
                     memory, driver);
    face->size = NULL;
...
{
    FT_List_Finalize(&driver->faces_list, (FT_List_Destructor)destroy_face,
                     driver->root.memory, driver);
}
 ...
 */
static FT_Error open_face(FT_Driver driver, FT_Stream* astream,
                          FT_Bool external_stream, FT_Long face_index,
                          ...
                          FT_Face* aface)
{
    ...
    if (clazz->init_face)
        error = clazz->init_face(*astream, face, (FT_Int)face_index, num_params,
                                 params);
...
FT_EXPORT_DEF(FT_Error)
FT_New_Face(FT_Library library, const char* pathname, FT_Long face_index,
            FT_Face* aface)
{
    FT_Open_Args args;
...
/* From `ftmac.c'.                                      */
static FT_Error new_memory_stream(FT_Library library, FT_Byte* base,
                                  FT_ULong size, FT_Stream_CloseFunc close,
                                  FT_Stream* astream)
{
    FT_Error error;
    FT_Memory memory;
...
open_face_from_buffer(FT_Library library, FT_Byte* base, FT_ULong size,
                      FT_Long face_index, const char* driver_name,
                      FT_Face* aface)
{
    FT_Open_Args args;
    FT_Error error;
    ...
    error =
        new_memory_stream(library, base, size, memory_stream_close, &stream);
    if (error)
    {
        FT_FREE(base);
        return error;
...
static FT_Error ft_lookup_PS_in_sfnt_stream(FT_Stream stream,
                                            FT_Long face_index,
                                            FT_ULong* offset, FT_ULong* length,
                                            FT_Bool* is_sfnt_cid)
{
    FT_Error error;
    FT_UShort numTables;
    ...
    *is_sfnt_cid = FALSE;
    ...
    {
        if (FT_READ_ULONG(tag) || FT_STREAM_SKIP(4) || FT_READ_ULONG(*offset) ||
...
FT_LOCAL_DEF(FT_Error)
open_face_PS_from_sfnt_stream(FT_Library library, FT_Stream stream,
                              FT_Long face_index, FT_Int num_params,
                              FT_Parameter* params, FT_Face* aface)
{
    ...
    error = ft_lookup_PS_in_sfnt_stream(stream, face_index, &offset, &length,
                                        &is_sfnt_cid);
    if (error)
        goto Exit;
    ...
    error =
        open_face_from_buffer(library, sfnt_ps, length, FT_MIN(face_index, 0),
                              is_sfnt_cid ? "cid" : "type1", aface);
Exit:
{
    FT_Error error1;
...
/* must be 0 (or -1).                                             */
/*                                                                */
static FT_Error Mac_Read_POST_Resource(FT_Library library, FT_Stream stream,
                                       FT_Long* offsets, FT_Long resource_cnt,
                                       FT_Long face_index, FT_Face* aface)
{
    ...
    return open_face_from_buffer(library, pfb_data, pfb_pos, face_index,
                                 "type1", aface);
...
/*                                                             */
static FT_Error Mac_Read_sfnt_Resource(FT_Library library, FT_Stream stream,
                                       FT_Long* offsets, FT_Long resource_cnt,
                                       FT_Long face_index, FT_Face* aface)
{
    FT_Memory memory = library->memory;
    ...
    error = open_face_PS_from_sfnt_stream(library, stream, face_index, 0, NULL,
                                          aface);
    if (!error)
        goto Exit;
    ...
    error = open_face_from_buffer(library, sfnt_data, (FT_ULong)rlen,
                                  face_index_in_resource,
                                  is_cff ? "cff" : "truetype", aface);
...
/*                                                             */
static FT_Error IsMacResource(FT_Library library, FT_Stream stream,
                              FT_Long resource_offset, FT_Long face_index,
                              FT_Face* aface)
{
    FT_Memory memory = library->memory;
    FT_Error error;
    ...
    error = FT_Raccess_Get_DataOffsets(library, stream, map_offset, rdata_pos,
                                       TTAG_POST, TRUE, &data_offsets, &count);
    if (!error)
    {
    ...
    /* sfnt resources should not be sorted to preserve the face order by
       QuickDraw API */
    error = FT_Raccess_Get_DataOffsets(library, stream, map_offset, rdata_pos,
                                       TTAG_sfnt, FALSE, &data_offsets, &count);
    if (!error)
    {
...
/*                                                          */
static FT_Error IsMacBinary(FT_Library library, FT_Stream stream,
                            FT_Long face_index, FT_Face* aface)
{
    ...
    if (header[0] != 0 || header[74] != 0 || header[82] != 0 ||
        header[1] == 0 || header[1] > 33 || header[63] != 0 ||
        header[2 + header[1]] != 0 || header[0x53] > 0x7F)
        return FT_THROW(Unknown_File_Format);
...
#define FT_COMPONENT raccess
    ...
    for (i = 0; i < FT_RACCESS_N_RULES; i++)
    {
        ...
        FT_TRACE3(
            ("Try rule %d: %s (offset=%d) ...", i, args2.pathname, offsets[i]));
...
/*                                                               */
static FT_Error load_mac_face(FT_Library library, FT_Stream stream,
                              FT_Long face_index, FT_Face* aface,
                              const FT_Open_Args* args)
{
    FT_Error error;
        ...
        for (; cur < limit; cur++)
        {
            ...
            if (FT_MODULE_IS_DRIVER(cur[0]))
            {
                    ...
                    FT_ERR_EQ(error, Table_Missing))
                {
                    ...
                    error = open_face_PS_from_sfnt_stream(
                        library, stream, face_index, num_params, params, aface);
                    if (!error)
                    {
                        FT_Stream_Free(stream, external_stream);
                        return error;
...
Exit:
    ...
    if (!error && face_index < 0)
    {
        FT_TRACE3(("FT_Open_Face: The font has %ld face%s\n"
                   ...
                   face->style_flags >> 16,
                   (face->style_flags >> 16) == 1 ? "" : "s", -face_index - 1));
    }
#endif
...
FT_Attach_Stream(FT_Face face, FT_Open_Args* parameters)
{
    ...
    FT_Stream_Free(stream, FT_BOOL(parameters->stream &&
...
#ifdef GRID_FIT_METRICS
    metrics->ascender =
        FT_PIX_CEIL(FT_MulFix(face->ascender, metrics->y_scale));

    metrics->descender =
        FT_PIX_FLOOR(FT_MulFix(face->descender, metrics->y_scale));
...
FT_Select_Size(FT_Face face, FT_Int strike_index)
{
...
#ifdef FT_DEBUG_LEVEL_TRACE
    {
        ...
        FT_TRACE5(("  x scale: %d (%f)\n", metrics->x_scale,
                   metrics->x_scale / 65536.0));
        FT_TRACE5(("  y scale: %d (%f)\n", metrics->y_scale,
                   metrics->y_scale / 65536.0));
               ...
               FT_UInt kern_mode, FT_Vector* akerning)
{
    ...
    {
        error =
            driver->clazz->get_kerning(face, left_glyph, right_glyph, akerning);
        if (!error)
        {
            if (kern_mode != FT_KERNING_UNSCALED)
            {
                ...
                if (kern_mode != FT_KERNING_UNFITTED)
                {
                    ...
                    if (face->size->metrics.x_ppem < 25)
                        akerning->x =
                            FT_MulDiv(orig_x, face->size->metrics.x_ppem, 25);
                    if (face->size->metrics.y_ppem < 25)
                        akerning->y =
                            FT_MulDiv(orig_y, face->size->metrics.y_ppem, 25);
                     ...
                     FT_Fixed* akerning)
{
    ...
    error = service->get_track(face, point_size, degree, akerning);
        ...
        for (i = 0; i < face->num_charmaps; i++)
        {
            if ((FT_CMap)face->charmaps[i] == cmap)
            {
                FT_CharMap last_charmap =
                    face->charmaps[face->num_charmaps - 1];

                if (FT_RENEW_ARRAY(face->charmaps, face->num_charmaps,
                                   face->num_charmaps - 1))
                    return;
                   ...
                   FT_Parameter* properties)
{
    ...
    for (; num_properties > 0; num_properties--)
    {
        ...
        else if (properties->tag == FT_PARAM_TAG_LCD_FILTER_WEIGHTS)
        {
            ...
            if (properties->data)
            {
                ft_memcpy(face->internal->lcd_weights, properties->data,
                            ...
                            FT_ULong variantSelector)
{
    ...
    if (face && face->charmap && face->charmap->encoding == FT_ENCODING_UNICODE)
    {
        ...
        if (charmap)
        {
            ...
            result = vcmap->clazz->char_var_index(
                vcmap, ucmap, (FT_UInt32)charcode, (FT_UInt32)variantSelector);
        }
    }
                                ...
                                FT_ULong variantSelector)
{
    ...
    if (face)
    {
        ...
        if (charmap)
        {
            ...
            result = vcmap->clazz->char_var_default(vcmap, (FT_UInt32)charcode,
    ...
    switch (slot->format)
    {
        ...
        default:
            if (slot->internal->load_flags & FT_LOAD_COLOR)
            {
                ...
                iterator.p = NULL;
                have_layers = FT_Get_Color_Glyph_Layer(
                    face, base_glyph, &glyph_index, &color_index, &iterator);
                if (have_layers)
                {
                    error = FT_New_GlyphSlot(face, NULL);
                    if (!error)
                    {
                        ...
                        do
                        {
                            ...
                            /* disable the `FT_LOAD_COLOR' flag to avoid
                             * recursion */
                            ...
                            error = sfnt->colr_blend(ttface, color_index, slot,
                                                     face->glyph);
                            if (error)
                                break;

                        } while (FT_Get_Color_Glyph_Layer(
                            face, base_glyph, &glyph_index, &color_index,
                            &iterator));
...

            {
                ...
                while (renderer)
                {
                     ...
                     */
                    /* is unsupported by the current renderer for this glyph
                     * image */
                    /* format. */
...
#define FT_COMPONENT checksum
    ...
    if (!error && ft_trace_levels[trace_checksum] >= 3 && slot->bitmap.buffer)
    {
        FT_Bitmap bitmap;
        FT_Error err;
        ...
        if (!err)
        {
            ...
            FT_TRACE3(("FT_Render_Glyph: bitmap %dx%d, %s (mode %d)\n", pitch,
                       rows, pixel_modes[slot->bitmap.pixel_mode],
                       slot->bitmap.pixel_mode));
    ...
    if (!error && ft_trace_levels[trace_checksum] >= 7)
    {
        if (slot->bitmap.rows < 128U && slot->bitmap.width < 128U &&
            slot->bitmap.buffer)
        {
                    ...
                    for (i = 0; i < rows; i++)
                    {
                        for (j = 0; j < width;)
                            for (m = 128; m > 0 && j < width; m >>= 1, j++)
                                FT_TRACE7((" %d", (topleft[i * pitch + j / 8] &
                                                   m) != 0));
                        FT_TRACE7(("\n"));
                    }
                    break;
...
static FT_Error ft_property_do(FT_Library library, const FT_String* module_name,
                               const FT_String* property_name, void* value,
                               FT_Bool set, FT_Bool value_is_string)
{
    FT_Module* cur;
    FT_Module* limit;
    ...
    interface = cur[0]->clazz->get_interface(cur[0], FT_SERVICE_ID_PROPERTIES);
    if (!interface)
    {
        FT_ERROR(("%s: module `%s' doesn't support properties\n", func_name,
                  module_name));
...
{
    return ft_property_do(library, module_name, property_name, (void*)value,
                          TRUE, FALSE);
}
...
FT_Property_Get(FT_Library library, const FT_String* module_name,
                const FT_String* property_name, void* value)
{
    return ft_property_do(library, module_name, property_name, value, FALSE,
                          FALSE);
}
...
FT_Done_Library(FT_Library library)
{
     ...
     */
    {
        ...
        for (m = 0; m < sizeof(driver_name) / sizeof(driver_name[0]); m++)
        {
    ...
    if (library)
    {
        ...
        if (module)
        {
            ...
            service = (FT_Service_TrueTypeEngine)ft_module_get_service(
                module, FT_SERVICE_ID_TRUETYPE_ENGINE, 0);
            if (service)
                         ...
                         FT_LayerIterator* iterator)
{
    ...
    if (!face || !aglyph_index || !acolor_index || !iterator ||
    ...
    if (sfnt->get_colr_layer)
        return sfnt->get_colr_layer(ttface, base_glyph, aglyph_index,
                                    acolor_index, iterator);
    else
        return 0;
}
