                               ...
                               struct sigevent* sig)
{
    ...
    for (cnt = 0; cnt < nent; ++cnt)
        if (list[cnt] != NULL && list[cnt]->aio_lio_opcode != LIO_NOP)
        {
            ...
            requests[cnt] = __aio_enqueue_request(
                (aiocb_union*)list[cnt],
                (list[cnt]->aio_lio_opcode | LIO_OPCODE_BASE));
    ...
    else
    {
        ...
        waitlist = (struct async_waitlist*)malloc(
            sizeof(struct async_waitlist) + (nent * sizeof(struct waitlist)));
...
#if SHLIB_COMPAT(librt, GLIBC_2_1, GLIBC_2_4)
int attribute_compat_text_section __lio_listio_21(int mode,
...
int __lio_listio_item_notify(int mode, struct aiocb* const list[], int nent,
                             struct sigevent* sig)
{
    /* Check arguments.  */
    if (mode != LIO_WAIT && mode != LIO_NOWAIT)
