...
static int register_stuff(Config* config)
{
    ...
    {
        if (!(entry_group =
                  avahi_entry_group_new(client, entry_group_callback, config)))
        {
    ...
    if (config->command == COMMAND_PUBLISH_ADDRESS)
    {
        ...
        if (avahi_entry_group_add_address(
                entry_group, AVAHI_IF_UNSPEC, AVAHI_PROTO_UNSPEC,
                config->no_reverse ? AVAHI_PUBLISH_NO_REVERSE : 0, config->name,
                &config->address) < 0)
        {
    ...
    else
    {
        ...
        if (avahi_entry_group_add_service_strlst(
                entry_group, AVAHI_IF_UNSPEC, AVAHI_PROTO_UNSPEC, 0,
                config->name, config->stype, config->domain, config->host,
                config->port, config->txt) < 0)
        {
        ...
        for (i = config->subtypes; i; i = i->next)
            if (avahi_entry_group_add_service_subtype(
                    entry_group, AVAHI_IF_UNSPEC, AVAHI_PROTO_UNSPEC, 0,
                    config->name, config->stype, config->domain,
                    (char*)i->text) < 0)
            {
...
static void help(FILE* f, const char* argv0)
{
    fprintf(
        ...
        _("%s [options] %s <name> <type> <port> [<txt ...>]\n"
          ...
          "       --subtype=SUBTYPE An additional subtype to register this "
          "service with\n"
          "    -R --no-reverse      Do not publish reverse entry with address\n"
          "    -f --no-fail         Don't fail if the daemon is not "
          "available\n"),
        argv0, strstr(argv0, "service") ? "[-s]" : "-s", argv0,
        strstr(argv0, "address") ? "[-a]" : "-a");
}
                              ...
                              char* argv[])
{
    ...
    c->command = strstr(argv0, "address")
                     ? COMMAND_PUBLISH_ADDRESS
                     : (strstr(argv0, "service") ? COMMAND_PUBLISH_SERVICE
                                                 : COMMAND_UNSPEC);
    ...
    while ((o = getopt_long(argc, argv, "hVsavRd:H:f", long_options, NULL)) >=
           0)
    {
...
int main(int argc, char* argv[])
{
    ...
    switch (config.command)
    {
        ...
        case COMMAND_PUBLISH_ADDRESS:
            ...
            if (avahi_client_get_state(client) != AVAHI_CLIENT_CONNECTING &&
                config.verbose)
            {
