...
class memory_resource
{
  public:
    ...
    static BOOST_CONSTEXPR_OR_CONST std::size_t max_align =
        boost::move_detail::alignment_of<
            boost::move_detail::max_align_t>::value;
  ...
  protected:
    ...
    //!
    //! <b>Returns</b>: A derived class shall implement this function to return
    //! a pointer
    //!   to allocated storage with a size of at least bytes. The returned
    //!   storage is aligned to the specified alignment, if such alignment is
    //!   supported; otherwise it is aligned to max_align.
    //!
    //! <b>Throws</b>: A derived class implementation shall throw an appropriate
    //! exception if
    ...
    //!
    //! <b>Effects</b>: A derived class shall implement this function to dispose
    ...
    //!   otherwise it shall return false. <i>[Note: The most-derived type of
    //!   other might not match the type of this. For a derived class, D, a
    //!   typical implementation of this function will compute
    //!   `dynamic_cast<const D*>(&other)` and go no further (i.e., return
    //!   false) if it returns nullptr. - end note]</i>.
    virtual bool
        do_is_equal(const memory_resource& other) const BOOST_NOEXCEPT = 0;
};
