...
#define FT_Stream_FTell(stream) (FT_ULong)((stream)->cursor - (stream)->base)
#define FT_Stream_SeekSet(stream, off)                                         \
    (stream)->cursor = ((off) < (FT_ULong)((stream)->limit - (stream)->base))  \
                           ? (stream)->base + (off)                            \
                           : (stream)->limit

/* some macros we need */
#define FT_fdot14ToFixed(x) ((FT_Fixed)((FT_ULong)(x) << 2))
#define FT_intToFixed(i) ((FT_Fixed)((FT_ULong)(i) << 16))
#define FT_fixedToInt(x) ((FT_Short)(((FT_UInt32)(x) + 0x8000U) >> 16))
 ...
 */
static FT_UShort* ft_var_readpackedpoints(FT_Stream stream, FT_ULong size,
                                          FT_UInt* point_cnt)
{
 ...
 *   step).
 */
static FT_Fixed* ft_var_readpackeddeltas(FT_Stream stream, FT_ULong size,
                                         FT_UInt delta_cnt)
{
...
static FT_Error ft_var_load_item_variation_store(TT_Face face, FT_ULong offset,
                                                 GX_ItemVarStore itemStore)
{
    ...
    if (FT_STREAM_SEEK(offset) || FT_READ_USHORT(format))
        goto Exit;
    ...
    if (FT_READ_ULONG(region_offset) || FT_READ_USHORT(itemStore->dataCount))
        goto Exit;
    ...
    for (i = 0; i < itemStore->regionCount; i++)
    {
        ...
        for (j = 0; j < itemStore->axisCount; j++)
        {
            ...
            if (FT_READ_SHORT(start) || FT_READ_SHORT(peak) ||
                FT_READ_SHORT(end))
                goto Exit;
    ...
    for (i = 0; i < itemStore->dataCount; i++)
    {
        ...
        if (shortDeltaCount > varData->regionIdxCount)
        {
            FT_TRACE2(("bad short count %d or region count %d\n",
                       shortDeltaCount, varData->regionIdxCount));
            error = FT_THROW(Invalid_Table);
            goto Exit;
        }
        ...
        if (varData->regionIdxCount > itemStore->regionCount)
        {
            FT_TRACE2(("inconsistent regionCount %d in varData[%d]\n",
                       varData->regionIdxCount, i));
            error = FT_THROW(Invalid_Table);
            goto Exit;
        }

        /* parse region indices */
        if (FT_NEW_ARRAY(varData->regionIndices, varData->regionIdxCount))
            goto Exit;
...
static FT_Error ft_var_load_delta_set_index_mapping(TT_Face face,
                                                    FT_ULong offset,
 ...
 */
static FT_Error ft_var_load_hvvar(TT_Face face, FT_Bool vertical)
{
    FT_Stream stream = FT_FACE_STREAM(face);
    FT_Memory memory = stream->memory;
    ...
    /* skip minor version */
    if (FT_READ_USHORT(majorVersion) || FT_STREAM_SKIP(2))
        goto Exit;
    ...
    if (FT_READ_ULONG(store_offset) || FT_READ_ULONG(widthMap_offset))
        goto Exit;
    ...
    error = ft_var_load_item_variation_store(face, table_offset + store_offset,
                                             &table->itemStore);
    if (error)
        goto Exit;
    ...
    if (widthMap_offset)
    {
        error = ft_var_load_delta_set_index_mapping(
            face, table_offset + widthMap_offset, &table->widthMap,
            &table->itemStore);
        if (error)
            goto Exit;
    }
...
static FT_Int ft_var_get_item_delta(TT_Face face, GX_ItemVarStore itemStore,
                                    FT_UInt outerIndex, FT_UInt innerIndex)
{
    GX_ItemVarData varData;
    FT_Short* deltaSet;
    ...
    for (master = 0; master < varData->regionIdxCount; master++)
    {
        ...
        for (j = 0; j < itemStore->axisCount; j++, axis++)
        {
            ...
            /* cumulative product of all the axis scalars */
                ...
                scalar = FT_MulDiv(
                    scalar, face->blend->normalizedcoords[j] - axis->startCoord,
                    axis->peakCoord - axis->startCoord);
            else
                scalar = FT_MulDiv(
                    scalar, axis->endCoord - face->blend->normalizedcoords[j],
 ...
 */
static FT_Error tt_hvadvance_adjust(TT_Face face, FT_UInt gindex,
                                    FT_Int* avalue, FT_Bool vertical)
{
    FT_Error error = FT_Err_Ok;
    ...
    delta =
        ft_var_get_item_delta(face, &table->itemStore, outerIndex, innerIndex);

    FT_TRACE5(("%s value %d adjusted by %d unit%s (%s)\n",
               vertical ? "vertical height" : "horizontal width", *avalue,
               delta, delta == 1 ? "" : "s", vertical ? "VVAR" : "HVAR"));
...
tt_hadvance_adjust(TT_Face face, FT_UInt gindex, FT_Int* avalue)
{
    return tt_hvadvance_adjust(face, gindex, avalue, 0);
}

FT_LOCAL_DEF(FT_Error)
tt_vadvance_adjust(TT_Face face, FT_UInt gindex, FT_Int* avalue)
{
...
static FT_Short* ft_var_get_value_pointer(TT_Face face, FT_ULong mvar_tag)
{
    FT_Short* p;
 ...
 *     The font face.
 */
static void ft_var_load_mvar(TT_Face face)
{
    FT_Stream stream = FT_FACE_STREAM(face);
    FT_Memory memory = stream->memory;
    ...
    if (FT_STREAM_SKIP(4) || FT_READ_USHORT(blend->mvar_table->valueCount) ||
    ...
    if (FT_NEW_ARRAY(blend->mvar_table->values, blend->mvar_table->valueCount))
        return;
    ...
    for (; value < limit; value++)
    {
        ...
        if (value->outerIndex >= itemStore->dataCount ||
            value->innerIndex >=
                itemStore->varData[value->outerIndex].itemCount)
        {
            error = FT_THROW(Invalid_Table);
            break;
        }
    ...
    for (; value < limit; value++)
    {
        ...
        if (p)
            ...
            FT_TRACE1(("ft_var_load_mvar: Ignoring unknown tag `%c%c%c%c'\n",
                       (FT_Char)(value->tag >> 24), (FT_Char)(value->tag >> 16),
                       (FT_Char)(value->tag >> 8), (FT_Char)(value->tag)));
#endif
    }
...
static FT_Error tt_size_reset_iterator(FT_ListNode node, void* user)
{
...
tt_apply_mvar(TT_Face face)
{
    ...
    for (; value < limit; value++)
    {
        ...
        delta = ft_var_get_item_delta(face, &blend->mvar_table->itemStore,
        ...
        if (p)
        {
            FT_TRACE5(
                ("value %c%c%c%c (%d unit%s) adjusted by %d unit%s (MVAR)\n",
                 (FT_Char)(value->tag >> 24), (FT_Char)(value->tag >> 16),
                 (FT_Char)(value->tag >> 8), (FT_Char)(value->tag),
                 value->unmodified, value->unmodified == 1 ? "" : "s", delta,
                 delta == 1 ? "" : "s"));
    ...
    /* adjust all derived values */
    {
         ...
         *    whether they were actually changed or the font had the OS/2
         * table's fsSelection's bit 7 (USE_TYPO_METRICS) set.
         */
        ...
        root->height = root->ascender - root->descender + current_line_gap +
                       mvar_hlgp_delta;
        ...
        FT_List_Iterate(&root->sizes_list, tt_size_reset_iterator, NULL);
    }
}
...
#define FT_STRUCTURE GX_GVar_Head
    ...
    if (FT_SET_ERROR(face->goto_table(face, TTAG_gvar, stream, &table_len)))
    {
        FT_TRACE2(("is missing\n"));
        goto Exit;
    }
    ...
    if ((FT_ULong)gvar_head.glyphCount * ((gvar_head.flags & 1) ? 4 : 2) >
        table_len)
    {
    ...
    FT_TRACE5(("gvar: there %s %d shared coordinate%s:\n",
               blend->tuplecount == 1 ? "is" : "are", blend->tuplecount,
    ...
    if (blend->tuplecount != 0)
    {
        ...
        for (i = 0; i < blend->tuplecount; i++)
        {
            ...
            for (j = 0; j < (FT_UInt)gvar_head.axisCount; j++)
            {
                ...
                FT_TRACE5(
                    ("%.5f ", blend->tuplecoords[i * gvar_head.axisCount + j] /
                                  65536.0));
            }
            FT_TRACE5(("]\n"));
        }
 ...
 */
static FT_Fixed ft_var_apply_tuple(GX_Blend blend, FT_UShort tupleIndex,
                                   ...
                                   FT_Fixed* im_end_coords)
{
    ...
    for (i = 0; i < blend->num_axis; i++)
    {
        FT_TRACE6(("    axis %d coordinate %.5f:\n", i,
                   blend->normalizedcoords[i] / 65536.0));
        ...
        if (!(tupleIndex & GX_TI_INTERMEDIATE_TUPLE))
        {
            ...
            FT_TRACE6(("      tuple coordinate %.5f fits\n",
                       tuple_coords[i] / 65536.0));
            apply =
                FT_MulDiv(apply, blend->normalizedcoords[i], tuple_coords[i]);
        }
        else
        {
            /* intermediate tuple */
                ...
                blend->normalizedcoords[i] >= im_end_coords[i])
            {
                FT_TRACE6(
                    ("      intermediate tuple range ]%.5f;%.5f[ is exceeded,"
                     " stop\n",
                     im_start_coords[i] / 65536.0, im_end_coords[i] / 65536.0));
                apply = 0;
                break;
            }
            ...
            if (blend->normalizedcoords[i] < tuple_coords[i])
                apply = FT_MulDiv(
                    apply, blend->normalizedcoords[i] - im_start_coords[i],
...
static void ft_var_to_normalized(TT_Face face, FT_UInt num_coords,
                                 FT_Fixed* coords, FT_Fixed* normalized)
{
    GX_Blend blend;
    FT_MM_Var* mmvar;
    FT_UInt i, j;
    ...
    for (i = 0; i < num_coords; i++, a++)
    {
        ...
        if (coord > a->maximum || coord < a->minimum)
        {
            FT_TRACE1((
                ...
                coord / 65536.0, a->minimum / 65536.0, a->maximum / 65536.0));
...
static void ft_var_to_design(TT_Face face, FT_UInt num_coords, FT_Fixed* coords,
                             FT_Fixed* design)
{
    GX_Blend blend;
    FT_MM_Var* mmvar;
    ...
    {
        if (design[i] < 0)
            design[i] = a->def + FT_MulFix(design[i], a->def - a->minimum);
        else if (design[i] > 0)
            design[i] = a->def + FT_MulFix(design[i], a->maximum - a->def);
        else
            design[i] = a->def;
    }
}
...
#define FT_STRUCTURE GX_FVar_Axis
        ...
        FT_FRAME_START(20),      FT_FRAME_ULONG(axisTag),
        FT_FRAME_LONG(minValue), FT_FRAME_LONG(defaultValue),
        FT_FRAME_LONG(maxValue), FT_FRAME_USHORT(flags),
        FT_FRAME_USHORT(nameID), FT_FRAME_END};
    ...
    if (need_init)
    {
        ...
        if (FT_SET_ERROR(face->goto_table(face, TTAG_gvar, stream, &table_len)))
        {
            ...
            if (FT_SET_ERROR(
                    face->goto_table(face, TTAG_CFF2, stream, &table_len)))
            {
                FT_TRACE1(
                    ("\n"
        ...
        if (FT_SET_ERROR(face->goto_table(face, TTAG_fvar, stream, &table_len)))
        {
        ...
        usePsName =
            FT_BOOL(fvar_head.instanceSize == 6 + 4 * fvar_head.axisCount);
...
#define ALIGN_SIZE(n) (((n) + sizeof(void*) - 1) & ~(sizeof(void*) - 1))
    ...
    axis_flags_size = ALIGN_SIZE(num_axes * sizeof(FT_UShort));
    axis_size = ALIGN_SIZE(num_axes * sizeof(FT_Var_Axis));
    namedstyle_size = ALIGN_SIZE(num_instances * sizeof(FT_Var_Named_Style));
    next_coords_size = ALIGN_SIZE(num_instances * num_axes * sizeof(FT_Fixed));
    next_name_size = num_axes * 5;

    if (need_init)
    {
        face->blend->mmvar_len = mmvar_size + axis_flags_size + axis_size +
        ...
        mmvar->num_designs = ~0U; /* meaningless in this context; each glyph */
        ...
        next_coords = (FT_Fixed*)((char*)mmvar->namedstyle + namedstyle_size);
        ...
        next_name = (FT_String*)((char*)mmvar->namedstyle + namedstyle_size +
                                 next_coords_size);
        ...
        for (i = 0; i < num_axes; i++)
        {
            ...
            *axis_flags = axis_rec.flags;
            ...
            FT_TRACE5(("  %3d  `%s'"
                       ...
                       i, a->name, a->minimum / 65536.0, a->def / 65536.0,
                       a->maximum / 65536.0, *axis_flags,
        ...
        for (i = 0; i < fvar_head.instanceCount; i++, ns++)
        {
            ...
            if (FT_FRAME_ENTER((usePsName ? 6L : 4L) + 4L * num_axes))
                goto Exit;
...
#ifdef FT_DEBUG_LEVEL_TRACE
            {
                ...
                if (ns->strid != 0xFFFF)
                {
                    (void)sfnt->get_name(face, (FT_UShort)ns->strid, &strname);
                ...
                if (ns->psid != 0xFFFF)
                {
                    (void)sfnt->get_name(face, (FT_UShort)ns->psid, &psname);
                ...
                FT_TRACE5(("  instance %d (%s%s%s, %s%s%s)\n", i,
                           strname ? "name: `" : "",
                           strname ? strname : "unnamed", strname ? "'" : "",
                           psname ? "PS name: `" : "",
                           psname ? psname : "no PS name", psname ? "'" : ""));
        ...
        if (num_instances != fvar_head.instanceCount)
        {
            ...
            found = sfnt->get_name_id(face, TT_NAME_ID_TYPOGRAPHIC_SUBFAMILY,
                                      &dummy1, &dummy2);
            if (found)
                strid = TT_NAME_ID_TYPOGRAPHIC_SUBFAMILY;
            else
            {
                found = sfnt->get_name_id(face, TT_NAME_ID_FONT_SUBFAMILY,
                                          &dummy1, &dummy2);
                if (found)
            ...
            if (found)
            {
                found = sfnt->get_name_id(face, TT_NAME_ID_PS_NAME, &dummy1,
                                          &dummy2);
                if (found)
                {
                    FT_TRACE5(
...
static FT_Error tt_set_mm_blend(TT_Face face, FT_UInt num_coords,
                                FT_Fixed* coords, FT_Bool set_design_coords)
{
    FT_Error error = FT_Err_Ok;
    GX_Blend blend;
    FT_MM_Var* mmvar;
    ...
    else
    {
        ...
        if (FT_IS_NAMED_INSTANCE(FT_FACE(face)))
        {
            ...
            n = blend->normalized_stylecoords +
                (instance_index - 1) * mmvar->num_axis + i;
    ...
    if (set_design_coords)
        ft_var_to_design(face, all_design_coords ? blend->num_axis : num_coords,
                         blend->normalizedcoords, blend->coords);
    ...
    if (face->cvt)
    {
        ...
        {
            case mcvt_load:
                /* The cvt table has been loaded already; every time we change
                 * the */
 ...
 */
FT_LOCAL_DEF(FT_Error)
TT_Get_MM_Blend(TT_Face face, FT_UInt num_coords, FT_Fixed* coords)
{
    FT_Error error = FT_Err_Ok;
    GX_Blend blend;
    FT_UInt i, nc;
 ...
 */
FT_LOCAL_DEF(FT_Error)
TT_Get_Var_Design(TT_Face face, FT_UInt num_coords, FT_Fixed* coords)
{
    FT_Error error = FT_Err_Ok;
    GX_Blend blend;
    FT_UInt i, nc;
 ...
 */
FT_LOCAL_DEF(FT_Error)
TT_Set_Named_Instance(TT_Face face, FT_UInt instance_index)
{
    ...
    if (instance_index > 0 && mmvar->namedstyle)
    {
        ...
        error =
            sfnt->get_name(face, (FT_UShort)named_style->strid, &style_name);
        if (error)
            goto Exit;
        ...
        error = TT_Set_Var_Design(face, mmvar->num_axis, named_style->coords);
        if (error)
            goto Exit;
    }
    else
    ...
    face->root.face_index =
        (instance_index << 16) | (face->root.face_index & 0xFFFFL);
    face->root.face_flags &= ~FT_FACE_FLAG_VARIATION;
...
tt_face_vary_cvt(TT_Face face, FT_Stream stream)
{
    ...
    /* rough sanity test */
    if (offsetToData + (tupleCount & GX_TC_TUPLE_COUNT_MASK) * 4 > table_len)
    {
    ...
    if (tupleCount & GX_TC_TUPLES_SHARE_POINT_NUMBERS)
    {
        ...
        sharedpoints =
            ft_var_readpackedpoints(stream, table_len, &spoint_count);
    ...
    for (i = 0; i < (tupleCount & GX_TC_TUPLE_COUNT_MASK); i++)
    {
        ...
        apply = ft_var_apply_tuple(blend, (FT_UShort)tupleIndex, tuple_coords,
                                   im_start_coords, im_end_coords);
        ...
        {
            localpoints =
                ft_var_readpackedpoints(stream, table_len, &point_count);
            points = localpoints;
        }
        else
        {
        ...
        deltas = ft_var_readpackeddeltas(
            stream, table_len, point_count == 0 ? face->cvt_size : point_count);
        ...
        else if (localpoints == ALL_POINTS)
        {
            ...
            for (j = 0; j < face->cvt_size; j++)
            {
                ...
                {
                    FT_TRACE7(("      %d: %f -> %f\n", j,
                               (FT_intToFixed(face->cvt[j]) + old_cvt_delta) /
                                   65536.0,
                               (FT_intToFixed(face->cvt[j]) + cvt_deltas[j]) /
                                   65536.0));
                    count++;
                }
#endif
        ...
        else
        {
            ...
            for (j = 0; j < point_count; j++)
            {
                ...
                cvt_deltas[pindex] =
                    old_cvt_delta + FT_MulFix(deltas[j], apply);
                ...
                if (old_cvt_delta != cvt_deltas[pindex])
                {
                    FT_TRACE7(
                        ("      %d: %f -> %f\n", pindex,
                         (FT_intToFixed(face->cvt[pindex]) + old_cvt_delta) /
                             65536.0,
...
static void tt_delta_shift(int p1, int p2, int ref, FT_Vector* in_points,
...
static void tt_delta_interpolate(int p1, int p2, int ref1, int ref2,
                                 FT_Vector* in_points, FT_Vector* out_points)
{
    ...
    for (i = 0; i <= 1; i++)
    {
        ...
        {
            FT_Fixed scale = in1 != in2 ? FT_DivFix(out2 - out1, in2 - in1) : 0;
...
static void tt_interpolate_deltas(FT_Outline* outline, FT_Vector* out_points,
                                  FT_Vector* in_points, FT_Bool* has_delta)
{
    FT_Int first_point;
    FT_Int end_point;
    ...
    do
    {
        ...
        if (point <= end_point)
        {
            ...
            while (point <= end_point)
            {
                ...
                if (has_delta[point])
                {
                    tt_delta_interpolate(cur_delta + 1, point - 1, cur_delta,
                                         point, in_points, out_points);
                    cur_delta = point;
                }
            ...
            if (cur_delta == first_delta)
                tt_delta_shift(first_point, end_point, cur_delta, in_points,
                               out_points);
            else
            {
                /* otherwise handle remaining points       */
                /* at the end and beginning of the contour */
                tt_delta_interpolate(cur_delta + 1, end_point, cur_delta,
                                     first_delta, in_points, out_points);

                if (first_delta > 0)
                    tt_delta_interpolate(first_point, first_delta - 1,
                                         cur_delta, first_delta, in_points,
                                         out_points);
            }
        }
        contour++;
 ...
 */
FT_LOCAL_DEF(FT_Error)
TT_Vary_Apply_Glyph_Deltas(TT_Face face, FT_UInt glyph_index,
                           FT_Outline* outline, FT_UInt n_points)
{
    FT_Error error;
    FT_Stream stream = face->root.stream;
    FT_Memory memory = stream->memory;
    ...
    if (FT_NEW_ARRAY(points_org, n_points) ||
        FT_NEW_ARRAY(points_out, n_points) || FT_NEW_ARRAY(has_delta, n_points))
        goto Fail1;

    dataSize =
        blend->glyphoffsets[glyph_index + 1] - blend->glyphoffsets[glyph_index];
    ...
    if (tupleCount & GX_TC_TUPLES_SHARE_POINT_NUMBERS)
    {
        ...
        sharedpoints =
            ft_var_readpackedpoints(stream, blend->gvar_size, &spoint_count);
        offsetToData = FT_Stream_FTell(stream);
    ...
    for (i = 0; i < (tupleCount & GX_TC_TUPLE_COUNT_MASK); i++)
    {
        ...
        {
            localpoints =
                ft_var_readpackedpoints(stream, blend->gvar_size, &point_count);
            points = localpoints;
        }
        else
        {
        ...
        deltas_x =
            ft_var_readpackeddeltas(stream, blend->gvar_size,
 ...
 */
FT_LOCAL_DEF(FT_Error)
tt_get_var_blend(TT_Face face, FT_UInt* num_coords, FT_Fixed** coords,
...
static void ft_var_done_item_variation_store(TT_Face face,
                                             GX_ItemVarStore itemStore)
{
