 ...
 * \date   23.07.2012
 *
 * The header contains implementation of a generic attribute presence checker in
...
class has_attribute
{
  ...
  public:
     ...
     * \return \c true if the log record contains the sought attribute value, \c
     * false otherwise
     */
    template <typename ArgT>
    result_type operator()(ArgT const& arg) const
    {
        return m_visitor_invoker(m_name, arg, nop()).code() ==
               visitation_result::ok;
    }
};

/*!
 * An attribute value presence checker. This specialization does not check the
...
/*!
 * The function generates a terminal node in a template expression. The node
 * will check for the attribute value presence in a log record. The node will
 * also check that the attribute value has the specified type, if present.
 */
...
{
    typedef aux::unary_function_terminal<has_attribute<AttributeValueT>>
        terminal_type;
...
/*!
 * The function generates a terminal node in a template expression. The node
 * will check for the attribute value presence in a log record.
 */
BOOST_FORCEINLINE
    phoenix::actor<aux::unary_function_terminal<has_attribute<void>>>
    has_attr(attribute_name const& name)
{
...
BOOST_FORCEINLINE ActorT<aux::unary_function_terminal<
    has_attribute<typename DescriptorT::value_type>>>
    has_attr(attribute_keyword<DescriptorT, ActorT> const&)
{
    typedef aux::unary_function_terminal<
        has_attribute<typename DescriptorT::value_type>>
        terminal_type;
