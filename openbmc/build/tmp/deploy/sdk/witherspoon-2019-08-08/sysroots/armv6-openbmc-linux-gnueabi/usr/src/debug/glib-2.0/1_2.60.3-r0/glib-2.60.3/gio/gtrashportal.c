...
static GXdpTrash* ensure_trash_portal(void)
{
    ...
    if (g_once_init_enter(&trash))
    {
        ...
        if (connection != NULL)
        {
            proxy = gxdp_trash_proxy_new_sync(
                ...
                "/org/freedesktop/portal/desktop", NULL, NULL);
...
gboolean g_trash_portal_trash_file(GFile* file, GError** error)
{
    char* path = NULL;
    ...
    ret = gxdp_trash_call_trash_file_sync(proxy, g_variant_new_handle(fd_in),
