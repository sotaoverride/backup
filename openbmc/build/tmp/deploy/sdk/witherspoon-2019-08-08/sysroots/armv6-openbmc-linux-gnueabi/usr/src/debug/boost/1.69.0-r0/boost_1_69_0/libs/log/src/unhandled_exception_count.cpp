...
// MinGW GCC 4.4 seem to not work the same way the newer GCC versions do. As a
// result, __cxa_get_globals based implementation will always return 0. Just
// disable it for now and fall back to std::uncaught_exception().
#if !defined(__MINGW32__) ||                                                   \
    (defined(__GNUC__) &&                                                      \
     (__GNUC__ > 4 || (__GNUC__ == 4 && __GNUC_MINOR__ >= 5)))
// Only GCC 4.7 declares __cxa_get_globals() in cxxabi.h, older compilers do not
// expose this function but it's there
...
BOOST_LOG_API unsigned int unhandled_exception_count() BOOST_NOEXCEPT
{
    ...
    return *(reinterpret_cast<const unsigned int*>(
        static_cast<const char*>(__cxa_get_globals()) +
        sizeof(void*))); // __cxa_eh_globals::uncaughtExceptions, x32 offset -
                         // 0x4, x64 - 0x8
#elif defined(BOOST_LOG_HAS_GETPTD)
    // MSVC specific. Tested on
    // {MSVC2005SP1,MSVC2008SP1,MSVC2010SP1,MSVC2012}x{x32,x64}.
    return *(reinterpret_cast<const unsigned int*>(
        static_cast<const char*>(_getptd()) +
        (sizeof(void*) == 8 ? 0x100 : 0x90))); // _tiddata::_ProcessingThrow,
                                               // x32 offset - 0x90, x64 - 0x100
#else
    // Portable implementation. Does not allow to detect multiple nested
    // exceptions.
