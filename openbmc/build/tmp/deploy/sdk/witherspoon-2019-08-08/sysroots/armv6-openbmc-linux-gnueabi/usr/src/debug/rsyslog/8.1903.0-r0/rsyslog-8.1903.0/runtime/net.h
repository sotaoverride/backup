...
#define IPFREEBIND_DISABLED 0x00 /* don't enable IP_FREEBIND in  sock option   \
                                  */
#define IPFREEBIND_ENABLED_NO_LOG                                              \
    0x01 /* enable IP_FREEBIND but no warn on success */
#define IPFREEBIND_ENABLED_WITH_LOG                                            \
    0x02 /* enable IP_FREEBIND and warn on success */
...
struct AllowedSenders
{
    ...
    uint8_t SignificantBits; /* defines how many bits should be discarded (eqiv
                                to mask) */
    struct AllowedSenders* pNext;
};

/* this structure is a helper to implement wildcards in permittedPeers_t. It
 * specifies the domain component and the matching mode. rgerhards, 2008-05-27
 */
struct permittedPeerWildcard_s
{
    ...
    enum
    {
        ...
        PEER_WILDCARD_EMPTY_COMPONENT =
            4 /**< special case: domain component empty (e.g. "..") */
    } wildcardType;
 ...
 * solution so that our users can begin to enjoy authenticated TLS. The next
 * step (hopefully) is to enhance RainerScript. -- rgerhards, 2008-05-19
 */
struct permittedPeers_s
{
    ...
    enum
    {
        PERM_PEER_TYPE_UNDECIDED = 0, /**< we have not yet decided the type
                                         (fine in some auth modes) */
        PERM_PEER_TYPE_PLAIN = 1,     /**< just plain text contained */
        PERM_PEER_TYPE_WILDCARD =
            2, /**< wildcards are contained, wildcard struture is filled */
    } etryType;
    permittedPeers_t* pNext;
    permittedPeerWildcard_t*
        pWildcardRoot; /**< root of the wildcard, NULL if not initialized */
    permittedPeerWildcard_t*
        pWildcardLast; /**< end of the wildcard list, NULL if not initialized */
};

/* interfaces */
BEGINinterface(net) /* name must also be changed in ENDinterface macro! */
    rsRetVal (*cvthname)(struct sockaddr_storage* f, prop_t** localName,
...
int* (*create_udp_socket)(uchar* hostname, uchar* LogPort, int bIsServer,
                          int rcvbuf, int sndbuf, int ipfreebind, char* device);
void (*closeUDPListenSockets)(int* finet);
int (*isAllowedSender)(uchar* pszType, struct sockaddr* pFrom,
                       const char* pszFromHost); /* deprecated! */
rsRetVal (*getLocalHostname)(uchar**);
int (*should_use_so_bsdcompat)(void);
/* permitted peer handling should be replaced by something better (see comments
 * above) */
...
rsRetVal (*PermittedPeerWildcardMatch)(permittedPeers_t* pPeer,
...
int (*isAllowedSender2)(uchar* pszType, struct sockaddr* pFrom,
...
ENDinterface(net)
#define netCURR_IF_VERSION                                                     \
    9 /* increment whenever you change the interface structure! */
