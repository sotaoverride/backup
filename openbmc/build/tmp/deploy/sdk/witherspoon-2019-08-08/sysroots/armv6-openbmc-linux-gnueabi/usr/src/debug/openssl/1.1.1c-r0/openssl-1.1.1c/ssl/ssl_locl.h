...
#define n2l3(c, l)                                                             \
    ((l = (((unsigned long)((c)[0])) << 16) |                                  \
          (((unsigned long)((c)[1])) << 8) | (((unsigned long)((c)[2])))),     \
     (c) += 3)
...
#define SSL_aCERT (SSL_aRSA | SSL_aDSS | SSL_aECDSA | SSL_aGOST01 | SSL_aGOST12)
...
#define SSL_IS_TLS13(s)                                                        \
    (!SSL_IS_DTLS(s) && (s)->method->version >= TLS1_3_VERSION &&              \
...
#define SSL_IS_FIRST_HANDSHAKE(S)                                              \
    ((s)->s3->tmp.finish_md_len == 0 || (s)->s3->tmp.peer_finish_md_len == 0)
 ...
 */
#define SSL_CLIENT_USE_SIGALGS(s) SSL_CLIENT_USE_TLS1_2_CIPHERS(s)
...
struct ssl_method_st
{
    ...
    int (*ssl_read_bytes)(SSL* s, int type, int* recvd_type, unsigned char* buf,
 ...
 * servername TLS extension PSK_identity_hint [ 7 ] EXPLICIT OCTET STRING, --
 * optional PSK identity hint PSK_identity [ 8 ] EXPLICIT OCTET STRING,  --
 * optional PSK identity Ticket_lifetime_hint [9] EXPLICIT INTEGER, -- server's
 * lifetime hint for session ticket Ticket [10]             EXPLICIT OCTET
 * STRING, -- session ticket (clients only) Compression_meth [11]   EXPLICIT
 * OCTET STRING, -- optional compression method SRP_username [ 12 ] EXPLICIT
 * OCTET STRING -- optional SRP username flags [ 13 ] EXPLICIT INTEGER --
 * optional flags
 *      }
...
struct ssl_ctx_st
{
    ...
    struct
    {
        ...
        TSAN_QUALIFIER int
            sess_connect_good;          /* SSL new conne/reneg - finished */
        ...
        TSAN_QUALIFIER int
            sess_timeout; /* reuse attempt on timeouted session */
        TSAN_QUALIFIER int
            sess_cache_full;            /* session removed due to full cache */
    ...
    struct
    {
        ...
        int (*ticket_key_cb)(SSL* ssl, unsigned char* name, unsigned char* iv,
...
typedef struct ssl3_enc_method
{
    ...
    int (*generate_master_secret)(SSL*, unsigned char*, unsigned char*, size_t,
                                  size_t*);
    ...
    int (*alert_value)(int);
    int (*export_keying_material)(SSL*, unsigned char*, size_t, const char*,
...
#define SSL_USE_PSS(s)                                                         \
    (s->s3->tmp.peer_sigalg != NULL &&                                         \
...
__owur const SSL_CIPHER*
    ssl_get_cipher_by_char(SSL* ssl, const unsigned char* ptr, int all);
...
void dtls1_set_message_header(SSL* s, unsigned char mt, size_t len,
...
int tls13_generate_secret(SSL* s, const EVP_MD* md,
                          ...
                          const unsigned char* insecret, size_t insecretlen,
...
__owur int tls1_set_groups(uint16_t** pext, size_t* pextlen, int* curves,
                           size_t ncurves);
...
__owur SSL_TICKET_STATUS tls_get_ticket_from_client(SSL* s,
...
__owur int tls12_copy_sigalgs(SSL* s, WPACKET* pkt, const uint16_t* psig,
                              size_t psiglen);
 ...
 */
__owur int ssl_log_secret(SSL* ssl, const char* label, const uint8_t* secret,
                          size_t secret_len);
...
__owur int ssl3_cbc_digest_record(
    const EVP_MD_CTX* ctx, unsigned char* md_out, size_t* md_out_size,
    const unsigned char header[13], const unsigned char* data,
    size_t data_plus_mac_size, size_t data_plus_mac_plus_padding_size,
    const unsigned char* mac_secret, size_t mac_secret_length, char is_sslv3);
