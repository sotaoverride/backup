...
#if !defined(                                                                  \
    CPP_EXPRESSION_VALUE_HPP_452FE66D_8754_4107_AF1E_E42255A0C18A_INCLUDED)
...
class closure_value
{
  public:
    ...
    explicit closure_value(unsigned int ui,
    ...
    explicit closure_value(int_literal_type i,
                           value_error valid_ = error_noerror) :
        type(is_int),
        valid(valid_)
    {
    ...
    explicit closure_value(uint_literal_type ui,
                           value_error valid_ = error_noerror) :
        type(is_uint),
        valid(valid_)
    {
    ...
    friend closure_value operator-(closure_value const& rhs)
    {
        switch (rhs.type)
        {
            ...
            case is_bool:
                return closure_value(-as_long(rhs), rhs.valid);
            case is_uint:
                break;
        }
    ...
    friend closure_value operator!(closure_value const& rhs)
    {
        switch (rhs.type)
        {
            case is_int:
                return closure_value(!as_long(rhs), rhs.valid);
            case is_bool:
                return closure_value(!as_bool(rhs), rhs.valid);
            case is_uint:
                break;
        }
        return closure_value(!as_ulong(rhs), rhs.valid);
    ...
    // comparison
    friend closure_value operator==(closure_value const& lhs,
                                    closure_value const& rhs)
    {
        ...
        switch (lhs.type)
        {
            case is_int:
                ...
                {
                    case is_bool:
                        cmp = as_bool(lhs) == rhs.value.b;
                        break;
                    case is_int:
                        cmp = lhs.value.i == rhs.value.i;
                        break;
                    case is_uint:
                        cmp = lhs.value.ui == rhs.value.ui;
                        break;
                }
                break;

            case is_uint:
                cmp = lhs.value.ui == as_ulong(rhs);
                break;
            case is_bool:
                cmp = lhs.value.b == as_bool(rhs);
                break;
        }
    ...
    friend closure_value operator!=(closure_value const& lhs,
                                    closure_value const& rhs)
    {
        return closure_value(!as_bool(lhs == rhs),
                             (value_error)(lhs.valid | rhs.valid));
    }
    friend closure_value operator>(closure_value const& lhs,
                                   closure_value const& rhs)
    {
            ...
            case is_int:
                switch (rhs.type)
                {
                    ...
                    case is_uint:
                    {
                        ...
                        // Note: The usual arithmetic conversions are not
                        // performed on
    ...
    // handle the ?: operator
    closure_value& handle_questionmark(closure_value const& cond,
                                       closure_value const& val2)
    {
        switch (type)
        {
            case is_int:
                ...
                {
                    case is_bool:
                        value.b = as_bool(cond) ? value.b : as_bool(val2);
                        break;
                    case is_int:
                        value.i = as_bool(cond) ? value.i : as_long(val2);
                        break;
                    case is_uint:
