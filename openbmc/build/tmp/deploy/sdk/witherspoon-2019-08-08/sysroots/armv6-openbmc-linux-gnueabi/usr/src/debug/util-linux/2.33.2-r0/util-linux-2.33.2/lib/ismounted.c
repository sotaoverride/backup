    ...
    *mount_flags = 0;
    ...
    while ((mnt = getmntent(f)) != NULL)
    {
        ...
        if (S_ISBLK(st_buf.st_mode))
        {
            ...
            if (file_dev && major(st_buf.st_rdev) == LOOPDEV_MAJOR &&
        ...
        {
            if (file_dev &&
                ((file_dev == st_buf.st_dev) && (file_ino == st_buf.st_ino)))
                break;
        }
    }
    ...
    if (mnt == NULL)
    {
         ...
         */
        if (file_rdev && stat("/", &st_buf) == 0 && st_buf.st_dev == file_rdev)
        {
...
static int check_mntent(const char* file, int* mount_flags, char* mtpt,
                        int mtlen)
{
    int retval;

#ifdef DEBUG
    retval = check_mntent_file("/tmp/mtab", file, mount_flags, mtpt, mtlen);
    if (retval == 0)
        return 0;
#endif /* DEBUG */
#ifdef __linux__
...
static int check_getmntinfo(const char* file, int* mount_flags, char* mtpt,
                            int mtlen)
{
    struct statfs* mp;
    int len, n;
...
static int is_swap_device(const char* file)
{
    ...
    file_dev = 0;
#ifndef __GNU__ /* The GNU hurd is broken with respect to stat devices */
    if ((stat(file, &st_buf) == 0) && S_ISBLK(st_buf.st_mode))
...
#ifndef __GNU__
        if (file_dev && (stat(buf, &st_buf) == 0) && S_ISBLK(st_buf.st_mode) &&
            file_dev == st_buf.st_rdev)
        {
            ret++;
            break;
...
#pragma argsused
#endif
int check_mount_point(const char* device, int* mount_flags, char* mtpt,
                      int mtlen)
{
    int retval = 0;
