...
static int do_passwd(int passed_salt, char** salt_p, char** salt_malloc_p,
                     ...
                     size_t pw_maxlen, passwd_modes mode);
        ...
        if (1)
        {
            ...
            if (in == NULL)
            {
                if (EVP_read_pw_string(
                        passwd_malloc, passwd_malloc_size,
    ...
    else
    {
        ...
        do
        {
            ...
            if (r > 0)
            {
                ...
                if (!do_passwd(passed_salt, &salt, &salt_malloc, passwd,
                               bio_out, quiet, table, reverse, pw_maxlen, mode))
                    goto end;
            }
            done = (r <= 0);
        } while (!done);
...
static char* md5crypt(const char* passwd, const char* magic, const char* salt)
{
    ...
    for (i = 0; i < 1000; i++)
    {
        if (!EVP_DigestInit_ex(md2, EVP_md5(), NULL))
            goto err;
        if (!EVP_DigestUpdate(md2, (i & 1) ? (unsigned const char*)passwd : buf,
        ...
        if (!EVP_DigestUpdate(md2, (i & 1) ? buf : (unsigned const char*)passwd,
...
#define ROUNDS_MAX 999999999
    ...
    while (n)
    {
        if (!EVP_DigestUpdate(md, (n & 1) ? buf : (unsigned const char*)passwd,
