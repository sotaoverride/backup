...
static gpg_err_code_t int2octets(unsigned char** r_frame, gcry_mpi_t value,
                                 size_t nbytes)
{
    gpg_err_code_t rc;
...
/* Connert the bit string BITS of length NBITS into an octet string
   ...
   R_FRAME.  */
static gpg_err_code_t bits2octets(unsigned char** r_frame, const void* bits,
                                  unsigned int nbits, gcry_mpi_t q,
                                  unsigned int qbits)
{
    gpg_err_code_t rc;
    gcry_mpi_t z1;
                                        ...
                                        unsigned int qbits)
{
    ...
    if (mpi_is_opaque(input))
    {
        ...
        rc =
            _gcry_mpi_scan(&hash, GCRYMPI_FMT_USG, abuf, (abits + 7) / 8, NULL);
        if (rc)
            return rc;
        if (abits > qbits)
