...
int header_hrdevice(struct variable*, oid*, size_t*, int, size_t*,
                    WriteMethod**);
...
struct variable4 hrdevice_variables[] = {
    {HRDEV_INDEX, ASN_INTEGER, NETSNMP_OLDAPI_RONLY, var_hrdevice, 2, {1, 1}},
    {HRDEV_TYPE, ASN_OBJECT_ID, NETSNMP_OLDAPI_RONLY, var_hrdevice, 2, {1, 2}},
    {HRDEV_DESCR, ASN_OCTET_STR, NETSNMP_OLDAPI_RONLY, var_hrdevice, 2, {1, 3}},
    {HRDEV_ID, ASN_OBJECT_ID, NETSNMP_OLDAPI_RONLY, var_hrdevice, 2, {1, 4}},
    {HRDEV_STATUS, ASN_INTEGER, NETSNMP_OLDAPI_RONLY, var_hrdevice, 2, {1, 5}},
    {HRDEV_ERRORS, ASN_COUNTER, NETSNMP_OLDAPI_RONLY, var_hrdevice, 2, {1, 6}}};
...
int header_hrdevice(struct variable* vp, oid* name, size_t* length, int exact,
...
u_char* var_hrdevice(struct variable* vp, oid* name, size_t* length, int exact,
                     size_t* var_len, WriteMethod** write_method)
{
    int dev_idx, type;
    oid* oid_p;
...
really_try_next:
    ...
    switch (vp->magic)
    {
        ...
        default:
            DEBUGMSGTL(
                ("snmpd", "unknown sub-id %d in var_hrdevice\n", vp->magic));
    }
