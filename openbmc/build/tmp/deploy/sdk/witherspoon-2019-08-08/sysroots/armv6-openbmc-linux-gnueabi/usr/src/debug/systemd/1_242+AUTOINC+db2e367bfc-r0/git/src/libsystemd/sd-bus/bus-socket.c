...
static int bus_socket_auth_verify_client(sd_bus* b)
{
    ...
    {
        /*
         * Old versions of the server-side implementation of `sd-bus` replied
         * with "OK <id>" to "AUTH" requests from a client, even if the "AUTH"
         * line did not contain inlined arguments. Therefore, we also accept "OK
         * <id>" here, even though it is technically the wrong reply. We ignore
         * the "<id>" parameter, though, since it has no real value.
         */
...
static int bus_socket_auth_verify_server(sd_bus* b)
{
    ...
    for (;;)
    {
        ...
        if (line_begins(line, l, "AUTH ANONYMOUS"))
        {
            ...
            r = verify_anonymous_token(b, line + strlen("AUTH ANONYMOUS"),
        ...
        else if (line_begins(line, l, "AUTH EXTERNAL"))
        {
            ...
            r = verify_external_token(b, line + strlen("AUTH EXTERNAL"),
...
static int bus_socket_read_auth(sd_bus* b)
{
    ...
    iov =
        IOVEC_MAKE((uint8_t*)b->rbuffer + b->rbuffer_size, n - b->rbuffer_size);
...
static int bus_socket_start_auth_client(sd_bus* b)
{
    static const char sasl_auth_anonymous[] = {
        /*
         * We use an arbitrary trace-string for the ANONYMOUS authentication. It
         * can be used by the message broker to aid debugging of clients. We
         * fully anonymize the connection and use a static default.
         */
        "\0AUTH ANONYMOUS\r\n"
    ...
    if (b->anonymous_auth)
        b->auth_iovec[i++] = IOVEC_MAKE((char*)sasl_auth_anonymous,
                                        sizeof(sasl_auth_anonymous) - 1);
    else
        b->auth_iovec[i++] = IOVEC_MAKE((char*)sasl_auth_external,
...
static int bus_socket_inotify_setup(sd_bus* b)
{
    ...
    /* Sets up an inotify fd in case watch_bind is enabled: wait until the
     * configured AF_UNIX file system socket appears before connecting to it.
     * The implemented is pretty simplistic: we just subscribe to relevant
     * changes to all prefix components of the path, and every time we get an
     * event for that we try to reconnect again, without actually caring what
     * precisely the event we got told us. If we still can't connect we
     * re-subscribe to all relevant changes of anything in the path, so that our
    ...
    /* Watch all parent directories, and don't mind any prefix that doesn't
     * exist yet. For the innermost directory that exists we want to know when
     * files are created or moved into it. For all parents of it we just care if
    ...
    /* Start with the top-level directory, which is a bit simpler than the rest,
     * since it can't be a symlink, and always exists */
    ...
    for (;;)
    {
        _cleanup_free_ char *component = NULL, *prefix = NULL,
                            *destination = NULL;
        ...
        wd = inotify_add_watch(b->inotify_fd, prefix,
                               IN_DELETE_SELF | IN_MOVE_SELF | IN_ATTRIB |
                                   IN_CREATE | IN_MOVED_TO | IN_DONT_FOLLOW);
        log_debug("Added inotify watch for %s on bus %s: %i", prefix,
                  strna(b->description), wd);

        if (wd < 0)
        {
            if (IN_SET(errno, ENOENT, ELOOP))
                break; /* This component doesn't exist yet, or the path contains
                          a cyclic symlink right now */

            r = log_debug_errno(errno, "Failed to add inotify watch on %s: %m",
        ...
        /* Check if this is possibly a symlink. If so, let's follow it and watch
         * it too. */
        ...
        if (max_follow <= 0)
        { /* Let's make sure we don't follow symlinks forever */
            r = -ELOOP;
            goto fail;
        }
        ...
        else
        {
            ...
            /* For relative symlinks we replace the last component, and try
             * again */
    ...
    /* And now, let's remove all watches from the previous iteration we don't
     * need anymore */
    ...
    for (;;)
    {
        ...
        if (connect(b->input_fd, &b->sockaddr.sa, b->sockaddr_size) < 0)
        {
            if (errno == EINPROGRESS)
            {
                ...
                /* If we have any inotify watches open, close them now, we don't
                 * need them anymore, as we have successfully initiated a
                 * connection */
                bus_close_inotify_fd(b);

                /* Note that very likely we are already in BUS_OPENING state
                 * here, as we enter it when we start parsing the address
                 * string. The only reason we set the state explicitly
                 * here, is to undo BUS_WATCH_BIND, in case we did the inotify
                 * magic. */
            ...
            if (IN_SET(
                    errno, ENOENT,
                    ECONNREFUSED) && /* ENOENT → unix socket doesn't exist at
                ...
                b->sockaddr.un.sun_path[0] != 0)
            {
                ...
                /* This connection attempt failed, let's release the socket for
                 * now, and start with a fresh one when reconnecting. */
                bus_close_io_fds(b);

                if (inotify_done)
                {
                    /* inotify set up already, don't do it again, just return
                     * now, and remember
                ...
                /* This is a file system socket, and the inotify logic is
                 * enabled. Let's create the necessary inotify fd. */
                r = bus_socket_inotify_setup(b);
                if (r < 0)
                    return r;

                /* Let's now try to connect a second time, because in theory
                 * there's otherwise a race here: the socket might have been
                 * created in the time between our first connect() and the time
                 * we set up the inotify logic. But let's remember that we set
                 * up inotify now, so that we don't do the connect() more than
                 * twice. */
                inotify_done = true;
            }
            else
...
int bus_socket_exec(sd_bus* b)
{
    ...
    r = safe_fork_full("(sd-busexec)", s + 1, 1,
...
int bus_socket_write_message(sd_bus* bus, sd_bus_message* m, size_t* idx)
{
    ...
    else
    {
        ...
        if (m->n_fds > 0 && *idx == 0)
        {
            ...
            mh.msg_control = control =
                alloca(CMSG_SPACE(sizeof(int) * m->n_fds));
...
static int bus_socket_make_message(sd_bus* bus, size_t size)
{
    ...
    r = bus_message_from_malloc(bus, bus->rbuffer, size, bus->fds, bus->n_fds,
                                NULL, &t);
    if (r == -EBADMSG)
    {
        log_debug_errno(
            r, "Received invalid message from connection %s, dropping.",
            strna(bus->description));
        free(bus->rbuffer); /* We want to drop current rbuffer and proceed with
                               whatever remains in b */
    }
    else if (r < 0)
    {
        free(b);
    ...
    /* rbuffer ownership was either transferred to t, or we got EBADMSG and
     * dropped it. */
    bus->rbuffer = b;
    bus->rbuffer_size -= size;
...
int bus_socket_read_message(sd_bus* bus)
{
    ...
    iov = IOVEC_MAKE((uint8_t*)bus->rbuffer + bus->rbuffer_size,
                     need - bus->rbuffer_size);
    ...
    if (handle_cmsg)
    {
        ...
        if (cmsg->cmsg_level == SOL_SOCKET && cmsg->cmsg_type == SCM_RIGHTS)
        {
            ...
            for (i = 0; i < n; i++)
                f[bus->n_fds++] =
                    fd_move_above_stdio(((int*)CMSG_DATA(cmsg))[i]);
            bus->fds = f;
        }
        else
            log_debug("Got unexpected auxiliary data with level=%d and type=%d",
...
int bus_socket_process_watch_bind(sd_bus* b)
{
    ...
    /* We flushed events out of the inotify fd. In that case, maybe the socket
     * is valid now? Let's try to connect to it again */
