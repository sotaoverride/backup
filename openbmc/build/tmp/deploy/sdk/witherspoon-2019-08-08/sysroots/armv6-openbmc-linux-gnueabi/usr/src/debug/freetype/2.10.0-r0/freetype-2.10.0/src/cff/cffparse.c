...
cff_parser_init(CFF_Parser parser, FT_UInt code, void* object,
                FT_Library library, FT_UInt stackSize, FT_UShort num_designs,
                FT_UShort num_axes)
{
...
static FT_Long cff_parse_integer(FT_Byte* start, FT_Byte* limit)
{
    ...
    else if (v == 29)
    {
        ...
        val = (FT_Long)(((FT_ULong)p[0] << 24) | ((FT_ULong)p[1] << 16) |
...
static const FT_Long power_ten_limits[] = {
    FT_LONG_MAX / 1L,         FT_LONG_MAX / 10L,
    FT_LONG_MAX / 100L,       FT_LONG_MAX / 1000L,
    FT_LONG_MAX / 10000L,     FT_LONG_MAX / 100000L,
    FT_LONG_MAX / 1000000L,   FT_LONG_MAX / 10000000L,
    FT_LONG_MAX / 100000000L, FT_LONG_MAX / 1000000000L,
};

/* read a real */
static FT_Fixed cff_parse_real(FT_Byte* start, FT_Byte* limit,
                               FT_Long power_ten, FT_Long* scaling)
{
    FT_Byte* p = start;
    FT_Int nib;
    FT_UInt phase;
...
cff_parse_num(CFF_Parser parser, FT_Byte** d)
{
    ...
    else if (**d == 255)
    {
      ...
      return (FT_Short)(
        ...
        return (FT_Short)(
            ((((FT_UInt32) * (d[0] + 1) << 16) |
              ((FT_UInt32) * (d[0] + 2) << 8) | (FT_UInt32) * (d[0] + 3)) +
             0x80U) >>
            8);
#endif
    }
...
static FT_Fixed cff_parse_fixed_scaled(CFF_Parser parser, FT_Byte** d,
                                       FT_Long scaling)
{
...
static FT_Fixed cff_parse_fixed_dynamic(CFF_Parser parser, FT_Byte** d,
                                        FT_Long* scaling)
{
    FT_ASSERT(scaling);
...
static FT_Error cff_parse_font_matrix(CFF_Parser parser)
{
    ...
    if (parser->top >= parser->stack + 6)
    {
        ...
        if (max_scaling < -9 || max_scaling > 0 ||
            (max_scaling - min_scaling) < 0 || (max_scaling - min_scaling) > 9)
        {
...
static FT_Error cff_parse_font_bbox(CFF_Parser parser)
{
    ...
    if (parser->top >= parser->stack + 4)
    {
        ...
        FT_TRACE4((" [%d %d %d %d]\n", bbox->xMin / 65536, bbox->yMin / 65536,
                   bbox->xMax / 65536, bbox->yMax / 65536));
    }
...
static FT_Error cff_parse_cid_ros(CFF_Parser parser)
{
    ...
    if (parser->top >= parser->stack + 3)
    {
        ...
        FT_TRACE4((" %d %d %d\n", dict->cid_registry, dict->cid_ordering,
                   dict->cid_supplement));
    }
...
#define CFF_FIELD_BLEND(code, id)                                              \
    {cff_kind_blend, code | CFFCODE, 0, 0, cff_parse_blend, 0, 0},
...
#define CFF_FIELD_CALLBACK(code, name, id)                                     \
    {cff_kind_callback, code | CFFCODE, 0, 0, cff_parse_##name, 0, 0, id},

#define CFF_FIELD_BLEND(code, id)                                              \
    {cff_kind_blend, code | CFFCODE, 0, 0, cff_parse_blend, 0, 0, id},
...
static void destruct_t2s_item(FT_Memory memory, void* data, void* user)
{
    FT_UNUSED(user);
...
cff_parser_run(CFF_Parser parser, FT_Byte* start, FT_Byte* limit)
{
    ...
    while (p < limit)
    {
        ...
        else if (v == 31)
        {
            ...
            node = (FT_ListNode)memory->alloc(memory, sizeof(FT_ListNodeRec));
            if (!node)
                ...
                if (field->code == (FT_Int)code)
                {
                    ...
                    switch (field->kind)
                    {
                        ...
                        case cff_kind_fixed_thousand:
                            val = cff_parse_fixed_scaled(parser, parser->stack,
                                                         3);
                        ...
                        {
                            FT_Byte* qcount =
                                (FT_Byte*)parser->object + field->count_offset;
