...
static int make_backup(const char* target, const char* x)
{
    ...
    if (r < 0)
        log_warning_errno(
            r, "Failed to change access mode or ownership of %s: %m", backup);

    ts[0] = st.st_atim;
    ts[1] = st.st_mtim;
    if (futimens(fileno(dst), ts) < 0)
        log_warning_errno(
            errno, "Failed to fix access and modification time of %s: %m",
            backup);
...
static int sync_rights(FILE* from, FILE* to)
{
    ...
    return fchmod_and_chown(fileno(to), st.st_mode & 07777, st.st_uid,
                            st.st_gid);
}
...
static int write_temporary_passwd(const char* passwd_path, FILE** tmpfile,
                                  char** tmpfile_path)
{
...
static int write_temporary_shadow(const char* shadow_path, FILE** tmpfile,
                                  char** tmpfile_path)
{
    ...
    ORDERED_HASHMAP_FOREACH(i, todo_uids, iterator)
    {
        struct spwd n = {
            ...
            .sp_inact = -1,
            .sp_expire = -1,
            .sp_flag = (unsigned long)-1, /* this appears to be what everybody
                                             does ... */
        };
...
static int write_temporary_group(const char* group_path, FILE** tmpfile,
                                 char** tmpfile_path)
{
    ...
    if (ordered_hashmap_size(todo_gids) == 0 &&
        ordered_hashmap_size(members) == 0)
        return 0;
...
static int write_temporary_gshadow(const char* gshadow_path, FILE** tmpfile,
                                   char** tmpfile_path)
{
#if ENABLE_GSHADOW
    ...
    r = fopen_temporary_label("/etc/gshadow", gshadow_path, &gshadow,
                              &gshadow_tmp);
    if (r < 0)
        return r;
...
static int write_files(void)
{
    _cleanup_fclose_ FILE *passwd = NULL, *group = NULL, *shadow = NULL,
                          *gshadow = NULL;
    _cleanup_(unlink_and_freep) char *passwd_tmp = NULL, *group_tmp = NULL,
                                     *shadow_tmp = NULL, *gshadow_tmp = NULL;
    const char *passwd_path = NULL, *group_path = NULL, *shadow_path = NULL,
               *gshadow_path = NULL;
    int r;
...
static int uid_is_ok(uid_t uid, const char* name, bool check_with_gid)
{
    ...
    /* Let's also check via NSS, to avoid UID clashes over LDAP and such, just
     * in case */
    if (!arg_root)
    {
        errno = 0;
...
static int read_id_from_file(Item* i, uid_t* _uid, gid_t* _gid)
{
    ...
    if ((_uid || (_gid && !found_gid)) && i->uid_path &&
        root_stat(i->uid_path, &st) >= 0)
    {
        ...
        /* If we need the gid, but had no success yet, also derive it from the
         * uid path */
        if (_gid && !found_gid)
        {
            gid = st.st_gid;
...
static int add_user(Item* i)
{
    ...
    if (!arg_root)
    {
        ...
        if (!IN_SET(errno, 0, ENOENT))
            return log_error_errno(
                errno, "Failed to check if user %s already exists: %m",
                i->name);
    }
    ...
    if (i->uid_set)
    {
        ...
        if (r < 0)
            return log_error_errno(r, "Failed to verify uid " UID_FMT ": %m",
                                   i->uid);
        if (r == 0)
        {
            log_debug("Suggested user ID " UID_FMT " for %s already used.",
                      i->uid, i->name);
            i->uid_set = false;
        }
    }
    ...
    if (!i->uid_set)
    {
        ...
        if (read_id_from_file(i, &c, NULL) > 0)
        {
            ...
            if (c <= 0 || !uid_range_contains(uid_range, n_uid_range, c))
                log_debug("User ID " UID_FMT " of file not suitable for %s.", c,
                          i->name);
            else
            {
                ...
                else if (r > 0)
                {
                    ...
                    log_debug("User ID " UID_FMT
                              " of file for %s is already used.",
                              c, i->name);
            }
        }
    }
    ...
    if (!i->uid_set)
    {
        for (;;)
        {
            ...
            if (r < 0)
                return log_error_errno(r, "No free user ID available for %s.",
                                       i->name);
...
static int add_group(Item* i)
{
    ...
    if (!arg_root)
    {
        ...
        if (!IN_SET(errno, 0, ENOENT))
            return log_error_errno(
                errno, "Failed to check if group %s already exists: %m",
                i->name);
    }
    ...
    if (i->gid_set)
    {
        r = gid_is_ok(i->gid);
        if (r < 0)
            return log_error_errno(r, "Failed to verify gid " GID_FMT ": %m",
                                   i->gid);
        if (i->id_set_strict)
        {
            ...
            if (r > 0)
                return log_error_errno(
                    ...
                    "Failed to create %s: please create GID %d", i->name,
                    i->gid);
            if (r == 0)
                return 0;
        }
        if (r == 0)
        {
            log_debug("Suggested group ID " GID_FMT " for %s already used.",
                      i->gid, i->name);
            i->gid_set = false;
        }
    }
    ...
    if (!i->gid_set)
    {
        ...
        if (read_id_from_file(i, NULL, &c) > 0)
        {
            ...
            if (c <= 0 || !uid_range_contains(uid_range, n_uid_range, c))
                log_debug("Group ID " GID_FMT " of file not suitable for %s.",
                          c, i->name);
            else
            {
                r = gid_is_ok(c);
                ...
                else if (r > 0)
                {
                    ...
                    log_debug("Group ID " GID_FMT
                              " of file for %s already used.",
                              c, i->name);
            }
        }
    }
    ...
    if (!i->gid_set)
    {
        for (;;)
        {
            ...
            r = uid_range_next_lower(uid_range, n_uid_range, &search_uid);
            if (r < 0)
                return log_error_errno(r, "No free group ID available for %s.",
                                       i->name);
...
DEFINE_PRIVATE_HASH_OPS_WITH_VALUE_DESTRUCTOR(item_hash_ops, char,
                                              string_hash_func,
                                              string_compare_func, Item,
                                              item_free);
...
static int add_implicit(void)
{
    ...
    /* Implicitly create additional users and groups, if they were listed in "m"
     * lines */
...
DEFINE_PRIVATE_HASH_OPS_FULL(members_hash_ops, char, string_hash_func,
                             string_compare_func, free, char*, strv_free);
...
static int parse_line(const char* fname, unsigned line, const char* buffer)
{
    ...
    _cleanup_free_ char *action = NULL, *name = NULL, *resolved_name = NULL,
                        *id = NULL, *resolved_id = NULL, *description = NULL,
                        *resolved_description = NULL, *home = NULL,
                        *resolved_home = NULL, *shell = NULL,
                        *resolved_shell = NULL;
    ...
    /* Parse columns */
    p = buffer;
    r = extract_many_words(&p, NULL, EXTRACT_QUOTES, &action, &name, &id,
                           &description, &home, &shell, NULL);
    if (r < 0)
    ...
    if (strlen(action) != 1)
        return log_error_errno(SYNTHETIC_ERRNO(EINVAL),
                               "[%s:%u] Unknown modifier '%s'", fname, line,
                               action);

    if (!IN_SET(action[0], ADD_USER, ADD_GROUP, ADD_MEMBER, ADD_RANGE))
        return log_error_errno(SYNTHETIC_ERRNO(EBADMSG),
                               "[%s:%u] Unknown command type '%c'.", fname,
                               line, action[0]);
    ...
    if (name)
    {
        ...
        if (r < 0)
            log_error_errno(r, "[%s:%u] Failed to replace specifiers: %s",
                            fname, line, name);

        if (!valid_user_group_name(resolved_name))
            return log_error_errno(
                SYNTHETIC_ERRNO(EINVAL),
                "[%s:%u] '%s' is not a valid user or group name.", fname, line,
                resolved_name);
    }
    ...
    if (description)
    {
        r = specifier_printf(description, specifier_table, NULL,
    ...
    if (home)
    {
        ...
        if (!valid_home(resolved_home))
            return log_error_errno(
                SYNTHETIC_ERRNO(EINVAL),
                "[%s:%u] '%s' is not a valid home directory field.", fname,
                line, resolved_home);
    }
    ...
    if (shell)
    {
        ...
        if (!valid_shell(resolved_shell))
            return log_error_errno(
                SYNTHETIC_ERRNO(EINVAL),
                "[%s:%u] '%s' is not a valid login shell field.", fname, line,
                resolved_shell);
    }
    ...
    switch (action[0])
    {
        ...
        case ADD_RANGE:
            if (resolved_name)
                return log_error_errno(
                    SYNTHETIC_ERRNO(EINVAL),
                    "[%s:%u] Lines of type 'r' don't take a name field.", fname,
                    line);

            if (!resolved_id)
                return log_error_errno(SYNTHETIC_ERRNO(EINVAL),
                                       "[%s:%u] Lines of type 'r' require a ID "
                                       "range in the third field.",
                                       fname, line);

            if (description || home || shell)
                return log_error_errno(
                    SYNTHETIC_ERRNO(EINVAL),
                    "[%s:%u] Lines of type '%c' don't take a %s field.", fname,
                    line, action[0],
                    description ? "GECOS"
                                : home ? "home directory" : "login shell");

            r = uid_range_add_str(&uid_range, &n_uid_range, resolved_id);
            if (r < 0)
                return log_error_errno(SYNTHETIC_ERRNO(EINVAL),
                                       "[%s:%u] Invalid UID range %s.", fname,
                                       line, resolved_id);
        ...
        case ADD_MEMBER:
        {
            ...
            if (!name)
                return log_error_errno(SYNTHETIC_ERRNO(EINVAL),
                                       "[%s:%u] Lines of type 'm' require a "
                                       "user name in the second field.",
                                       fname, line);

            if (!resolved_id)
                return log_error_errno(SYNTHETIC_ERRNO(EINVAL),
                                       "[%s:%u] Lines of type 'm' require a "
                                       "group name in the third field.",
                                       fname, line);

            if (!valid_user_group_name(resolved_id))
                return log_error_errno(
                    SYNTHETIC_ERRNO(EINVAL),
                    "[%s:%u] '%s' is not a valid user or group name.", fname,
                    line, resolved_id);
...
static int read_config_file(const char* fn, bool ignore_enoent)
{
    ...
    {
        r = search_and_fopen(fn, "re", arg_root,
...
static int read_config_files(char** args)
{
    ...
    r = conf_files_list_with_replacement(
        arg_root, CONF_PATHS_STRV("sysusers.d"), arg_replace, &files, &p);
    if (r < 0)
        return r;
...
static int run(int argc, char* argv[])
{
    ...
    /* Let's tell nss-systemd not to synthesize the "root" and "nobody" entries
     * for it, so that our detection whether the names or UID/GID area already
     * used otherwise doesn't get confused. After all, even though nss-systemd
     * synthesizes these users/groups, they should still appear in /etc/passwd
     * and /etc/group, as the synthesizing logic is merely supposed to be
     * fallback for cases where we run with a completely unpopulated /etc. */
    if (setenv("SYSTEMD_NSS_BYPASS_SYNTHETIC", "1", 1) < 0)
        return log_error_errno(errno,
                               "Failed to set SYSTEMD_NSS_BYPASS_SYNTHETIC "
