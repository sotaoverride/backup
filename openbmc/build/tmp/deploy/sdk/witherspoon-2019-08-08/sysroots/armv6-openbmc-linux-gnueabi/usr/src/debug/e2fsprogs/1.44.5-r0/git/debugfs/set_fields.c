...
static errcode_t parse_string(struct field_set_info* info, char* field,
                              char* arg);
static errcode_t parse_uuid(struct field_set_info* info, char* field,
                            char* arg);
static errcode_t parse_hashalg(struct field_set_info* info, char* field,
                               char* arg);
static errcode_t parse_time(struct field_set_info* info, char* field,
                            char* arg);
...
static struct field_set_info super_fields[] = {
    ...
    {"blocks_count", &set_sb.s_blocks_count, &set_sb.s_blocks_count_hi, 4,
     parse_uint},
    {"r_blocks_count", &set_sb.s_r_blocks_count, &set_sb.s_r_blocks_count_hi, 4,
     parse_uint},
    {"free_blocks_count", &set_sb.s_free_blocks_count, &set_sb.s_free_blocks_hi,
     4, parse_uint},
    ...
    {"algorithm_usage_bitmap", &set_sb.s_algorithm_usage_bitmap, NULL, 4,
     parse_uint},
    {"prealloc_blocks", &set_sb.s_prealloc_blocks, NULL, 1, parse_uint},
    {"prealloc_dir_blocks", &set_sb.s_prealloc_dir_blocks, NULL, 1, parse_uint},
    {"reserved_gdt_blocks", &set_sb.s_reserved_gdt_blocks, NULL, 2, parse_uint},
    ...
    {"snapshot_r_blocks_count", &set_sb.s_snapshot_r_blocks_count, NULL, 8,
     parse_uint},
    ...
    {"backup_bgs", &set_sb.s_backup_bgs[0], NULL, 4, parse_uint, FLAG_ARRAY, 2},
    ...
    {"encrypt_algos", &set_sb.s_encrypt_algos, NULL, 1, parse_uint, FLAG_ARRAY,
     4},
...
static struct field_set_info inode_fields[] = {
    ...
    {"uid", &set_inode.i_uid, &set_inode.osd2.linux2.l_i_uid_high, 2,
     parse_uint},
    {"size", &set_inode.i_size, &set_inode.i_size_high, 4, parse_uint},
    {"atime", &set_inode.i_atime, &set_inode.i_atime_extra, 4, parse_time},
    {"ctime", &set_inode.i_ctime, &set_inode.i_ctime_extra, 4, parse_time},
    {"mtime", &set_inode.i_mtime, &set_inode.i_mtime_extra, 4, parse_time},
    {"dtime", &set_inode.i_dtime, NULL, 4, parse_time},
    {"gid", &set_inode.i_gid, &set_inode.osd2.linux2.l_i_gid_high, 2,
     parse_uint},
    {"links_count", &set_inode.i_links_count, NULL, 2, parse_uint},
    /* Special case: i_blocks is 4 bytes, i_blocks_high is 2 bytes */
    {"blocks", &set_inode.i_blocks, &set_inode.osd2.linux2.l_i_blocks_hi, 6,
     parse_uint},
    {"flags", &set_inode.i_flags, NULL, 4, parse_uint},
    {"version", &set_inode.osd1.linux1.l_i_version, &set_inode.i_version_hi, 4,
     parse_uint},
    ...
    {"author", &set_inode.osd2.hurd2.h_i_author, NULL, 4, parse_uint,
     FLAG_ALIAS},
    {"extra_isize", &set_inode.i_extra_isize, NULL, 2, parse_uint},
    {"ctime_extra", &set_inode.i_ctime_extra, NULL, 4, parse_uint, FLAG_ALIAS},
    {"mtime_extra", &set_inode.i_mtime_extra, NULL, 4, parse_uint, FLAG_ALIAS},
    {"atime_extra", &set_inode.i_atime_extra, NULL, 4, parse_uint, FLAG_ALIAS},
    {"crtime", &set_inode.i_crtime, &set_inode.i_crtime_extra, 4, parse_time},
    {"crtime_extra", &set_inode.i_crtime_extra, NULL, 4, parse_uint,
     FLAG_ALIAS},
...
static struct field_set_info ext4_bg_fields[] = {
    {"block_bitmap", &set_gd4.bg_block_bitmap, &set_gd4.bg_block_bitmap_hi, 4,
     parse_uint},
    {"inode_bitmap", &set_gd4.bg_inode_bitmap, &set_gd4.bg_inode_bitmap_hi, 4,
     parse_uint},
    {"inode_table", &set_gd4.bg_inode_table, &set_gd4.bg_inode_table_hi, 4,
     parse_uint},
    ...
    {"itable_unused", &set_gd4.bg_itable_unused, &set_gd4.bg_itable_unused_hi,
     2, parse_uint},
                                     ...
                                     const void* data, size_t size)
{
    ...
    for (ss = fields; ss->name; ss++)
    {
        ...
        for (ss2 = fields; ss2 != ss; ss2++)
        {
            ...
            assert(!ss->ptr || (ss->ptr != ss2->ptr && ss->ptr != ss2->ptr2));
            assert(!ss->ptr2 ||
                   (ss->ptr2 != ss2->ptr && ss->ptr2 != ss2->ptr2));
        }
    }
}
                            ...
                            char* field EXT2FS_ATTR((unused)), char* arg)
{
    ...
    if ((strcasecmp(arg, "null") == 0) || (strcasecmp(arg, "clear") == 0))
    {
        uuid_clear(p);
    }
                            ...
                            char* field EXT2FS_ATTR((unused)), char* arg)
{
    ...
    retval = ext2fs_bmap2(current_fs, set_ino, (struct ext2_inode*)&set_inode,
                          NULL, BMAP_ALLOC | BMAP_SET, array_idx, NULL, &blk);
    if (retval)
    {
...
void do_set_super(int argc, char* argv[])
{
    ...
    if (common_args_process(argc, argv, 3, 3, "set_super_value", usage,
                            CHECK_FS_RW))
        return;
...
void do_set_inode(int argc, char* argv[])
{
    ...
    if (debugfs_read_inode_full(set_ino, (struct ext2_inode*)&set_inode,
                                argv[1], sizeof(set_inode)))
        return;

    if (ss->func(ss, argv[2], argv[3]) == 0)
    {
        if (debugfs_write_inode_full(set_ino, (struct ext2_inode*)&set_inode,
                                     argv[1], sizeof(set_inode)))
            return;
    }
}
...
void do_set_block_group_descriptor(int argc, char* argv[])
{
    ...
    if (common_args_process(argc, argv, 4, 4, "set_block_group", usage,
                            CHECK_FS_RW))
        return;
...
void do_set_mmp_value(int argc, char* argv[])
{
    ...
    if (common_args_process(argc, argv, 2, 3, "set_mmp_value", usage,
                            CHECK_FS_RW))
        return;
    ...
    if (mmp_s == NULL)
    {
        ...
        retval =
            ext2fs_mmp_read(current_fs, current_fs->super->s_mmp_block, mmp_s);
        if (retval)
        {
    ...
    {
        ext2fs_mmp_write(current_fs, current_fs->super->s_mmp_block, &set_mmp);
        *mmp_s = set_mmp;
    }
}
#else
