...
static int HandleZRLETile(rfbClient* client, uint8_t* buffer,
                          size_t buffer_length, int x, int y, int w, int h);
...
static rfbBool HandleZRLE(rfbClient* client, int rx, int ry, int rw, int rh)
{
    ...
    if (client->decompStreamInited == FALSE)
    {
        ...
        {
            rfbClientLog("inflateInit returned error: %d, msg: %s\n",
                         inflateResult, client->decompStream.msg);
            return FALSE;
        }
    ...
    if (inflateResult == Z_OK)
    {
        ...
        for (j = 0; j < rh; j += rfbZRLETileHeight)
            ...
            {
                int subWidth =
                    (i + rfbZRLETileWidth > rw) ? rw - i : rfbZRLETileWidth;
                int subHeight =
                    (j + rfbZRLETileHeight > rh) ? rh - j : rfbZRLETileHeight;
                int result =
                    HandleZRLETile(client, (uint8_t*)buf, remaining, rx + i,
                          ...
                          size_t buffer_length, int x, int y, int w, int h)
{
    ...
    uint8_t zywrle_level = (client->appData.qualityLevel & 0x80)
                               ? 0
                               : (3 - client->appData.qualityLevel / 3);
#endif
    ...
    buffer++;
    {
        if (type == 0) /* raw */
            ...
            {
                CARDBPP* pFrame =
                    (CARDBPP*)client->frameBuffer + y * client->width + x;
                int ret;
                client->appData.qualityLevel |= 0x80;
                ret = HandleZRLETile(client, buffer, buffer_end - buffer, x, y,
                                     w, h);
                ...
                ZYWRLE_SYNTHESIZE(pFrame, pFrame, w, h, client->width,
                                  zywrle_level, (int*)client->zlib_buffer);
                buffer += ret;
            }
            else
#endif
        ...
        else if ((type >= 2) && (type <= 127)) /* packed Palette */
        {
            ...
            if (1 + type * REALBPP / 8 + ((w + divider - 1) / divider) * h >
                buffer_length)
                return -5;
        ...
        else if (type == 128) /* plain RLE */
        {
            ...
            while (j < h)
            {
                ...
                {
                    ((CARDBPP*)
                         client->frameBuffer)[(y + j) * client->width + x + i] =
                        color;
                    length--;
                    i++;
                    if (i >= w)
