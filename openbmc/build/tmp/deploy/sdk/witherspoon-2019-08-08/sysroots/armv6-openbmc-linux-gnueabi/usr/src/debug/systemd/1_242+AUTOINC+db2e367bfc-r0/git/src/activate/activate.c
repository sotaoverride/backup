...
static int add_epoll(int epoll_fd, int fd)
{
    ...
    if (epoll_ctl(epoll_fd, EPOLL_CTL_ADD, fd, &ev) < 0)
        return log_error_errno(
            errno, "Failed to add event on epoll fd:%d for fd:%d: %m", epoll_fd,
            fd);
...
static int open_sockets(int* epoll_fd, bool accept)
{
    ...
    {
        fd = make_socket_fd(LOG_DEBUG, *address, arg_socket_type,
                            (arg_accept * SOCK_CLOEXEC));
        if (fd < 0)
        {
            log_open();
...
static int exec_process(const char* name, char** argv, char** env, int start_fd,
                        size_t n_fds)
{
    ...
    if (arg_inetd)
    {
        ...
        r = rearrange_stdio(
            start_fd, start_fd,
            STDERR_FILENO); /* invalidates start_fd on success + error */
        if (r < 0)
    ...
    else
    {
        ...
        if (asprintf((char**)(envp + n_env++), "LISTEN_PID=" PID_FMT,
                     getpid_cached()) < 0)
            return log_oom();
        ...
        if (arg_fdnames)
        {
            ...
            if (len == 1)
            {
                ...
                log_warning("The number of fd names is different than number "
                            "of fds: %zu vs %zu",
                            len, n_fds);
...
static int fork_and_exec_process(const char* child, char** argv, char** env,
                                 int fd)
{
    _cleanup_free_ char* joined = NULL;
    pid_t child_pid;
...
static int help(void)
{
    ...
    printf("%s [OPTIONS...]\n\n"
           ...
           "  -d --datagram              Listen on datagram instead of stream "
           "socket\n"
           "     --seqpacket             Listen on SOCK_SEQPACKET instead of "
           "stream socket\n"
           "  -a --accept                Spawn separate child for each "
           "connection\n"
           "  -E --setenv=NAME[=VALUE]   Pass an environment variable to "
           "children\n"
           "     --fdname=NAME[:NAME...] Specify names for file descriptors\n"
           "     --inetd                 Enable inetd file descriptor passing "
           "protocol\n"
           "\nNote: file descriptors from sd_listen_fds() will be passed "
           "through.\n"
        ...
        switch (c)
        {
            ...
            case ARG_FDNAME:
            {
                ...
                if (!fdname_is_valid(*s))
                {
                    ...
                    esc = cescape(*s);
                    log_warning("File descriptor name \"%s\" is not valid.",
                                esc);
                }

                /* Empty optargs means one empty name */
                r = strv_extend_strv(
                    &arg_fdnames, strv_isempty(names) ? STRV_MAKE("") : names,
                    false);
                if (r < 0)
...
int main(int argc, char** argv, char** envp)
{
    ...
    exec_process(argv[optind], argv + optind, envp, SD_LISTEN_FDS_START,
                 (size_t)n);
