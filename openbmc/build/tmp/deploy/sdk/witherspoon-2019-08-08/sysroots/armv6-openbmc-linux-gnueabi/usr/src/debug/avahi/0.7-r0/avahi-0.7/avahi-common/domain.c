                            ...
                            const char* type, const char* domain)
{
    ...
    if (!(avahi_normalize_name(domain, normalized_domain,
                               sizeof(normalized_domain))))
        return AVAHI_ERR_INVALID_DOMAIN_NAME;

    /* Concatenation */

    snprintf(p, size, "%s%s%s.%s", name ? escaped_name : "", name ? "." : "",
        ...
        switch (state)
        {
            ...
            case TYPE:
                ...
                if (buf[0] == '_')
                {
                    ...
                    if (!(avahi_escape_label(buf, strlen(buf), &type,
                                             &type_size)))
                        return AVAHI_ERR_NO_MEMORY;
            ...
            case DOMAIN:
                ...
                if (!(avahi_escape_label(buf, strlen(buf), &domain,
                                         &domain_size)))
                    return AVAHI_ERR_NO_MEMORY;
