...
static void __attribute__((__noreturn__)) usage(void)
{
    ...
    fprintf(out, _(" -m, --mapfile <mapfile>   (defaults: \"%s\" and\n"),
            defaultmap);
    fprintf(out, _("                                      \"%s\")\n"),
            boot_uname_r_str());
    fprintf(out, _(" -p, --profile <pro-file>  (default:  \"%s\")\n"),
            defaultpro);
    fputs(_(" -M, --multiplier <mult>   set the profiling multiplier to "
            "<mult>\n"),
          out);
    fputs(_(" -i, --info                print only info about the sampling "
            "step\n"),
          out);
    fputs(_(" -v, --verbose             print verbose data\n"), out);
    fputs(
        _(" -a, --all                 print all symbols, even if count is 0\n"),
        out);
    fputs(
        _(" -b, --histbin             print individual histogram-bin counts\n"),
        out);
    fputs(_(" -s, --counters            print individual counters within "
            "functions\n"),
          out);
...
#define next (current ^ 1)
    ...
    while ((c = getopt_long(argc, argv, "m:p:M:ivabsrnVh", longopts, NULL)) !=
           -1)
    {
        switch (c)
        {
    ...
    while (fgets(mapline, S_LEN, map))
    {
        if (sscanf(mapline, "%llx %7[^\n ] %127[^\n ]", &fn_add, mode,
                   fn_name) != 3)
            errx(EXIT_FAILURE, _("%s(%i): wrong map line"), mapFile, maplineno);
    ...
    while (fgets(mapline, S_LEN, map))
    {
        ...
        else if ((this || optAll) && (fn_len = next_add - fn_add) != 0)
        {
            if (optVerbose)
