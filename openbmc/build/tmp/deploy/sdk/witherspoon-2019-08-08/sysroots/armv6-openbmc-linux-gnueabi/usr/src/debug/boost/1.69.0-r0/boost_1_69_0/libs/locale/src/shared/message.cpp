...
class c_file
{
  ...
  public:
    ...
    bool open(std::string const& file_name, std::string const& encoding)
    {
        ...
        // std::istream::open(wchar_t const *) we would use old and good stdio
        // and _wfopen CRTL functions
        //
...
class mo_file
{
  public:
    ...
    static bool key_equals(char const* real_key, char const* cntx,
                           char const* key)
    {
        if (cntx == 0)
...
struct mo_file_use_traits
{
    ...
    static pair_type use(mo_file const& /*mo*/, char_type const* /*context*/,
                         char_type const* /*key*/)
    {
...
struct mo_file_use_traits<char>
{
    ...
    typedef std::pair<char_type const*, char_type const*> pair_type;
    static pair_type use(mo_file const& mo, char const* context,
                         char const* key)
    {
...
struct hash_function
{
    size_t operator()(message_key<CharType> const& msg) const
    {
        ...
        if (*p != 0)
        {
            ...
            while (*e)
                e++;
            state = pj_winberger_hash::update_state(
                state, reinterpret_cast<char const*>(p),
...
template <>
char const* runtime_conversion(char const* msg, std::string& buffer,
                               bool do_conversion,
                               std::string const& locale_encoding,
                               std::string const& key_encoding)
{
    ...
    if (details::is_us_ascii_string(msg))
        return msg;
    std::string tmp =
        conv::between(msg, locale_encoding, key_encoding, conv::skip);
    buffer.swap(tmp);
    return buffer.c_str();
}
...
class mo_message : public message_format<CharType>
{
...
#ifdef BOOST_LOCALE_UNORDERED_CATALOG
    typedef boost::unordered_map<key_type, string_type, hash_function<CharType>>
        catalog_type;
#else
  ...
  public:
    ...
    virtual char_type const* get(int domain_id, char_type const* context,
                                 char_type const* single_id, int n) const
    {
    ...
    mo_message(messages_info const& inf)
    {
        ...
        for (unsigned id = 0; id < domains.size(); id++)
        {
            ...
            for (unsigned j = 0; !found && j < paths.size(); j++)
            {
                ...
                {
                    std::string full_path = search_paths[i] + "/" + paths[j] +
                                            "/" + lc_cat + "/" + domain + ".mo";
                    found = load_file(full_path, encoding, key_encoding, id,
                                      inf.callback);
                }
            }
        }
    ...
    {
        return runtime_conversion<char_type>(msg, buffer,
  ...
  private:
    ...
    {
        return convert_encoding_name(left).compare(
            convert_encoding_name(right));
    }
    ...
    bool load_file(std::string const& file_name,
                   std::string const& locale_encoding,
                   std::string const& key_encoding, int id,
                   messages_info::callback_type const& callback)
    {
        ...
        std::string mo_encoding =
            extract(mo->value(0).first, "charset=", " \r\n;");
    ...
    static std::string extract(std::string const& meta, std::string const& key,
    ...
    pair_type get_string(int domain_id, char_type const* context,
                         char_type const* in_id) const
    {
        ...
        {
            return mo_file_use_traits<char_type>::use(*mo_catalogs_[domain_id],
                                                      context, in_id);
        }
        else
        {
            ...
            return pair_type(p->second.data(),
