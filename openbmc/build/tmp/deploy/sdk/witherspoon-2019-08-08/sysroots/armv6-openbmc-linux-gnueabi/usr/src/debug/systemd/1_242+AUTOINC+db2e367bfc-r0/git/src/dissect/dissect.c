...
static DissectImageFlags arg_flags =
    DISSECT_IMAGE_REQUIRE_ROOT | DISSECT_IMAGE_DISCARD_ON_LOOP;
...
static void help(void)
{
    printf("%s [OPTIONS...] IMAGE\n"
           ...
           "     --discard=MODE    Choose 'discard' mode (disabled, loop, all, "
           "crypto)\n"
...
static int parse_argv(int argc, char* argv[])
{
        ...
        switch (c)
        {
            ...
            case ARG_DISCARD:
            {
                ...
                if (streq(optarg, "disabled"))
                    ...
                    flags =
                        DISSECT_IMAGE_DISCARD_ON_LOOP | DISSECT_IMAGE_DISCARD;
            ...
            case ARG_ROOT_HASH:
            {
                ...
                if (r < 0)
                    return log_error_errno(
                        r, "Failed to parse root hash '%s': %m", optarg);
                if (l < sizeof(sd_id128_t))
                {
                    log_error("Root hash must be at least 128bit long: %s",
                              optarg);
                    free(p);
                    return -EINVAL;
                }
    ...
    switch (arg_action)
    {
        ...
        case ACTION_MOUNT:
            if (optind + 2 != argc)
                return log_error_errno(SYNTHETIC_ERRNO(EINVAL),
                                       "Expected a file path and mount point "
                                       "path as only arguments.");
...
static int run(int argc, char* argv[])
{
    ...
    r = loop_device_make_by_path(
        arg_image, (arg_flags & DISSECT_IMAGE_READ_ONLY) ? O_RDONLY : O_RDWR,
        &d);
    if (r < 0)
    ...
    if (!arg_root_hash)
    {
        ...
        if (r < 0)
            return log_error_errno(
                r, "Failed to read root hash file for %s: %m", arg_image);
    }

    r = dissect_image_and_warn(d->fd, arg_image, arg_root_hash,
                               arg_root_hash_size, arg_flags, &m);
    if (r < 0)
        return r;
    ...
    switch (arg_action)
    {
        ...
        case ACTION_DISSECT:
        {
            ...
            for (i = 0; i < _PARTITION_DESIGNATOR_MAX; i++)
            {
                ...
                if (!sd_id128_is_null(p->uuid))
                    printf(" (UUID " SD_ID128_FORMAT_STR ")",
                           SD_ID128_FORMAT_VAL(p->uuid));
                ...
                if (k >= 0)
                    printf(" %s verity",
                           m->partitions[k].found ? "with" : "without");
        ...
        case ACTION_MOUNT:
            ...
            if (di)
            {
                ...
                if (r < 0)
                    return log_error_errno(
                        r, "Failed to relinquish DM devices: %m");
            }
