...
int avahi_client_set_dbus_error(AvahiClient* client, DBusError* error)
{
    ...
    return avahi_client_set_errno(client,
                                  avahi_error_dbus_to_number(error->name));
}
...
static DBusHandlerResult filter_func(DBusConnection* bus, DBusMessage* message,
                                     void* userdata)
{
                                    ...
                                    "NameOwnerChanged"))
    {
            ...
            dbus_error_is_set(&error))
        {
            ...
            fprintf(stderr,
                    "WARNING: Failed to parse NameOwnerChanged signal: %s\n",
                    error.message);
        ...
        if (strcmp(name, AVAHI_DBUS_NAME) == 0)
        {
            ...
            else if (client->state == AVAHI_CLIENT_CONNECTING &&
                     (!old || *old == 0))
            {
                int ret;
    ...
    else if (dbus_message_is_signal(message, AVAHI_DBUS_INTERFACE_SERVER,
                                    "StateChanged"))
    {
        int32_t state;
        char* e = NULL;
            ...
            dbus_error_is_set(&error))
        {
            ...
            fprintf(stderr,
                    "WARNING: Failed to parse Server.StateChanged signal: %s\n",
                    error.message);
            avahi_client_set_errno(client, AVAHI_ERR_DBUS_ERROR);
            goto fail;
        }
    ...
    else if (dbus_message_is_signal(message, AVAHI_DBUS_INTERFACE_ENTRY_GROUP,
                                    "StateChanged"))
    {
        const char* path;
        AvahiEntryGroup* g;
        ...
        if (g)
        {
                ...
                dbus_error_is_set(&error))
            {
                ...
                fprintf(stderr,
                        "WARNING: Failed to parse EntryGroup.StateChanged "
                        "signal: %s\n",
                        error.message);
                avahi_client_set_errno(client, AVAHI_ERR_DBUS_ERROR);
                goto fail;
            ...
            avahi_entry_group_set_state(g, state);
                 ...
                 message, AVAHI_DBUS_INTERFACE_DOMAIN_BROWSER, "ItemNew"))
        return avahi_domain_browser_event(client, AVAHI_BROWSER_NEW, message);
    else if (dbus_message_is_signal(
                 message, AVAHI_DBUS_INTERFACE_DOMAIN_BROWSER, "ItemRemove"))
        return avahi_domain_browser_event(client, AVAHI_BROWSER_REMOVE,
                                          message);
    else if (dbus_message_is_signal(message,
                                    AVAHI_DBUS_INTERFACE_DOMAIN_BROWSER,
                                    "CacheExhausted"))
        return avahi_domain_browser_event(client, AVAHI_BROWSER_CACHE_EXHAUSTED,
                                          message);
    else if (dbus_message_is_signal(
                 message, AVAHI_DBUS_INTERFACE_DOMAIN_BROWSER, "AllForNow"))
        return avahi_domain_browser_event(client, AVAHI_BROWSER_ALL_FOR_NOW,
                                          message);
    else if (dbus_message_is_signal(
                 message, AVAHI_DBUS_INTERFACE_DOMAIN_BROWSER, "Failure"))
        return avahi_domain_browser_event(client, AVAHI_BROWSER_FAILURE,
                                          message);

    else if (dbus_message_is_signal(
                 message, AVAHI_DBUS_INTERFACE_SERVICE_TYPE_BROWSER, "ItemNew"))
        return avahi_service_type_browser_event(client, AVAHI_BROWSER_NEW,
                                                message);
    else if (dbus_message_is_signal(message,
                                    AVAHI_DBUS_INTERFACE_SERVICE_TYPE_BROWSER,
                                    "ItemRemove"))
        return avahi_service_type_browser_event(client, AVAHI_BROWSER_REMOVE,
                                                message);
    else if (dbus_message_is_signal(message,
                                    AVAHI_DBUS_INTERFACE_SERVICE_TYPE_BROWSER,
                                    "CacheExhausted"))
        return avahi_service_type_browser_event(
            client, AVAHI_BROWSER_CACHE_EXHAUSTED, message);
    else if (dbus_message_is_signal(message,
                                    AVAHI_DBUS_INTERFACE_SERVICE_TYPE_BROWSER,
                                    "AllForNow"))
        return avahi_service_type_browser_event(
            client, AVAHI_BROWSER_ALL_FOR_NOW, message);
    else if (dbus_message_is_signal(
                 message, AVAHI_DBUS_INTERFACE_SERVICE_TYPE_BROWSER, "Failure"))
        return avahi_service_type_browser_event(client, AVAHI_BROWSER_FAILURE,
                                                message);

    else if (dbus_message_is_signal(
                 message, AVAHI_DBUS_INTERFACE_SERVICE_BROWSER, "ItemNew"))
        return avahi_service_browser_event(client, AVAHI_BROWSER_NEW, message);
    else if (dbus_message_is_signal(
                 message, AVAHI_DBUS_INTERFACE_SERVICE_BROWSER, "ItemRemove"))
        return avahi_service_browser_event(client, AVAHI_BROWSER_REMOVE,
                                           message);
    else if (dbus_message_is_signal(message,
                                    AVAHI_DBUS_INTERFACE_SERVICE_BROWSER,
                                    "CacheExhausted"))
        return avahi_service_browser_event(
            client, AVAHI_BROWSER_CACHE_EXHAUSTED, message);
    else if (dbus_message_is_signal(
                 message, AVAHI_DBUS_INTERFACE_SERVICE_BROWSER, "AllForNow"))
        return avahi_service_browser_event(client, AVAHI_BROWSER_ALL_FOR_NOW,
                                           message);
    else if (dbus_message_is_signal(
                 message, AVAHI_DBUS_INTERFACE_SERVICE_BROWSER, "Failure"))
        return avahi_service_browser_event(client, AVAHI_BROWSER_FAILURE,
                                           message);

    else if (dbus_message_is_signal(
                 message, AVAHI_DBUS_INTERFACE_SERVICE_RESOLVER, "Found"))
        return avahi_service_resolver_event(client, AVAHI_RESOLVER_FOUND,
                                            message);
    else if (dbus_message_is_signal(
                 message, AVAHI_DBUS_INTERFACE_SERVICE_RESOLVER, "Failure"))
        return avahi_service_resolver_event(client, AVAHI_RESOLVER_FAILURE,
                                            message);

    else if (dbus_message_is_signal(
                 message, AVAHI_DBUS_INTERFACE_HOST_NAME_RESOLVER, "Found"))
        return avahi_host_name_resolver_event(client, AVAHI_RESOLVER_FOUND,
                                              message);
    else if (dbus_message_is_signal(
                 message, AVAHI_DBUS_INTERFACE_HOST_NAME_RESOLVER, "Failure"))
        return avahi_host_name_resolver_event(client, AVAHI_RESOLVER_FAILURE,
                                              message);

    else if (dbus_message_is_signal(
                 message, AVAHI_DBUS_INTERFACE_ADDRESS_RESOLVER, "Found"))
        return avahi_address_resolver_event(client, AVAHI_RESOLVER_FOUND,
                                            message);
    else if (dbus_message_is_signal(
                 message, AVAHI_DBUS_INTERFACE_ADDRESS_RESOLVER, "Failure"))
        return avahi_address_resolver_event(client, AVAHI_RESOLVER_FAILURE,
                                            message);

    else if (dbus_message_is_signal(
                 message, AVAHI_DBUS_INTERFACE_RECORD_BROWSER, "ItemNew"))
        return avahi_record_browser_event(client, AVAHI_BROWSER_NEW, message);
    else if (dbus_message_is_signal(
                 message, AVAHI_DBUS_INTERFACE_RECORD_BROWSER, "ItemRemove"))
        return avahi_record_browser_event(client, AVAHI_BROWSER_REMOVE,
                                          message);
    else if (dbus_message_is_signal(message,
                                    AVAHI_DBUS_INTERFACE_RECORD_BROWSER,
                                    "CacheExhausted"))
        return avahi_record_browser_event(client, AVAHI_BROWSER_CACHE_EXHAUSTED,
                                          message);
    else if (dbus_message_is_signal(
                 message, AVAHI_DBUS_INTERFACE_RECORD_BROWSER, "AllForNow"))
        return avahi_record_browser_event(client, AVAHI_BROWSER_ALL_FOR_NOW,
                                          message);
    else if (dbus_message_is_signal(
                 message, AVAHI_DBUS_INTERFACE_RECORD_BROWSER, "Failure"))
        return avahi_record_browser_event(client, AVAHI_BROWSER_FAILURE,
                                          message);
...
static int get_server_state(AvahiClient* client, int* ret_error)
{
    ...
    reply = dbus_connection_send_with_reply_and_block(client->bus, message, -1,
                                                      &error);

    if (!reply || dbus_error_is_set(&error))
        goto fail;

    if (!dbus_message_get_args(reply, &error, DBUS_TYPE_INT32, &state,
                               DBUS_TYPE_INVALID) ||
        dbus_error_is_set(&error))
        goto fail;
...
static int check_version(AvahiClient* client, int* ret_error)
{
    ...
    if (!reply || dbus_error_is_set(&error))
    {
        ...
        if (!dbus_error_is_set(&error) ||
            strcmp(error.name, DBUS_ERROR_UNKNOWN_METHOD))
            goto fail;
        ...
        if (!(message = dbus_message_new_method_call(
                  AVAHI_DBUS_NAME, AVAHI_DBUS_PATH_SERVER,
                  AVAHI_DBUS_INTERFACE_SERVER, "GetVersionString")))
            goto fail;

        reply = dbus_connection_send_with_reply_and_block(client->bus, message,
                                                          -1, &error);
    ...
    else
    {
        ...
        if (!dbus_message_get_args(reply, &error, DBUS_TYPE_UINT32, &version,
                                   DBUS_TYPE_INVALID) ||
            dbus_error_is_set(&error))
            goto fail;
    }
...
AvahiClient* avahi_client_new(const AvahiPoll* poll_api, AvahiClientFlags flags,
                              AvahiClientCallback callback, void* userdata,
                              int* ret_error)
{
    ...
    AVAHI_LLIST_HEAD_INIT(AvahiServiceTypeBrowser,
...
static char* avahi_client_get_string_reply_and_block(AvahiClient* client,
                                                     const char* method,
                                                     const char* param)
{
    DBusMessage *message = NULL, *reply = NULL;
    ...
    if (!(message = dbus_message_new_method_call(
              AVAHI_DBUS_NAME, AVAHI_DBUS_PATH_SERVER,
              AVAHI_DBUS_INTERFACE_SERVER, method)))
    {
    ...
    if (param)
    {
        if (!dbus_message_append_args(message, DBUS_TYPE_STRING, &param,
                                      DBUS_TYPE_INVALID))
        {
            avahi_client_set_errno(client, AVAHI_ERR_NO_MEMORY);
            goto fail;
    ...
    if (!dbus_message_get_args(reply, &error, DBUS_TYPE_STRING, &ret,
                               DBUS_TYPE_INVALID) ||
        dbus_error_is_set(&error))
        goto fail;
...
const char* avahi_client_get_version_string(AvahiClient* client)
{
    ...
    if (!client->version_string)
        client->version_string = avahi_client_get_string_reply_and_block(
            client, "GetVersionString", NULL);
...
const char* avahi_client_get_domain_name(AvahiClient* client)
{
    ...
    if (!client->domain_name)
        client->domain_name = avahi_client_get_string_reply_and_block(
            client, "GetDomainName", NULL);
...
const char* avahi_client_get_host_name(AvahiClient* client)
{
    ...
    if (!client->host_name)
        client->host_name = avahi_client_get_string_reply_and_block(
            client, "GetHostName", NULL);
...
int avahi_client_simple_method_call(AvahiClient* client, const char* path,
                                    const char* interface, const char* method)
{
    ...
    if (!(message = dbus_message_new_method_call(AVAHI_DBUS_NAME, path,
                                                 interface, method)))
    {
...
int avahi_client_is_connected(AvahiClient* client)
{
    ...
    return client->bus && dbus_connection_get_is_connected(client->bus) &&
           (client->state == AVAHI_CLIENT_S_RUNNING ||
            client->state == AVAHI_CLIENT_S_REGISTERING ||
