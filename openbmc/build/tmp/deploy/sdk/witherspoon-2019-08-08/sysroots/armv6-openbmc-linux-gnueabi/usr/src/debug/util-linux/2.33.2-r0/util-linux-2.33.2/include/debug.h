...
#define UL_DEBUG_EMPTY_MASKNAMES                                               \
    {                                                                          \
        {                                                                      \
            NULL, 0, NULL                                                      \
        }                                                                      \
    }
#define UL_DEBUG_DEFINE_MASKNAMES(m)                                           \
    static const struct ul_debug_maskname m##_masknames[]
...
#define __UL_DBG_FLUSH(l, p)                                                   \
    do                                                                         \
    {                                                                          \
        if (l##_debug_mask && l##_debug_mask != p##INIT)                       \
        {                                                                      \
            fflush(stderr);                                                    \
        }                                                                      \
    } while (0)
    ...
    {                                                                          \
        ...
        {                                                                      \
            ...
            {                                                                  \
                ...
                fprintf(stderr,                                                \
                        "%d: %s: don't print memory addresses (SUID "          \
                        "executable).\n",                                      \
                        getpid(), #lib);                                       \
            }                                                                  \
        }                                                                      \
...
static inline int
    ul_debug_parse_mask(const struct ul_debug_maskname flagnames[],
                        const char* mask)
{
    int res;
    char* ptr;
...
static inline void
    ul_debug_print_masks(const char* env,
