...
static void dbg_show_header(struct ext3_extent_header* eh)
{
    printf("header: magic=%x entries=%u max=%u depth=%u generation=%u\n",
           ext2fs_le16_to_cpu(eh->eh_magic), ext2fs_le16_to_cpu(eh->eh_entries),
           ext2fs_le16_to_cpu(eh->eh_max), ext2fs_le16_to_cpu(eh->eh_depth),
...
static void dbg_show_index(struct ext3_extent_idx* ix)
{
    printf("index: block=%u leaf=%u leaf_hi=%u unused=%u\n",
           ext2fs_le32_to_cpu(ix->ei_block), ext2fs_le32_to_cpu(ix->ei_leaf),
...
static void dbg_show_extent(struct ext3_extent* ex)
{
    printf("extent: block=%u-%u len=%u start=%u start_hi=%u\n",
           ...
           ext2fs_le32_to_cpu(ex->ee_block) + ext2fs_le16_to_cpu(ex->ee_len) -
               1,
           ext2fs_le16_to_cpu(ex->ee_len), ext2fs_le32_to_cpu(ex->ee_start),
...
static void dbg_print_extent(char* desc, struct ext2fs_extent* extent)
{
    if (desc)
        printf("%s: ", desc);
    printf(
        "extent: lblk %llu--%llu, len %u, pblk %llu, flags: ", extent->e_lblk,
        extent->e_lblk + extent->e_len - 1, extent->e_len, extent->e_pblk);
...
#else
#define dbg_show_header(eh)                                                    \
    do                                                                         \
    {                                                                          \
    } while (0)
#define dbg_show_index(ix)                                                     \
    do                                                                         \
    {                                                                          \
    } while (0)
#define dbg_show_extent(ex)                                                    \
    do                                                                         \
    {                                                                          \
    } while (0)
#define dbg_print_extent(desc, ex)                                             \
    do                                                                         \
    {                                                                          \
    } while (0)
#define dump_path(tag, handle, path)                                           \
    do                                                                         \
    {                                                                          \
    } while (0)
#endif
                              ...
                              ext2_extent_handle_t* ret_handle)
{
    ...
    retval = ext2fs_get_memzero(handle->max_paths * sizeof(struct extent_path),
                                &handle->path);
 ...
 */
errcode_t ext2fs_extent_get(ext2_extent_handle_t handle, int flags,
...
retry:
    ...
    {
        if ((handle->level < handle->max_depth) && (path->left == 0))
            op = EXT2_EXTENT_DOWN;
        else
    ...
    switch (op)
    {
        ...
        case EXT2_EXTENT_DOWN_AND_LAST:
            ...
            if (!newpath->buf)
            {
                retval = ext2fs_get_mem(handle->fs->blocksize, &newpath->buf);
                if (retval)
                    return retval;
            }
            ...
            {
                retval =
                    io_channel_read_blk64(handle->fs->io, blk, 1, newpath->buf);
                if (retval)
                    return retval;
            }
            handle->level++;
            ...
            if (!(handle->fs->flags & EXT2_FLAG_IGNORE_CSUM_ERRORS) &&
                !ext2fs_extent_block_csum_verify(handle->fs, handle->ino, eh))
                failed_csum = 1;
    ...
    if ((orig_op == EXT2_EXTENT_LAST_LEAF) &&
        ((handle->level != handle->max_depth) || (path->left != 0)))
        goto retry;
...
static errcode_t update_path(ext2_extent_handle_t handle)
{
    ...
    if (handle->level == 0)
    {
        retval = ext2fs_write_inode(handle->fs, handle->ino, handle->inode);
    }
    else
    {
        ...
        eh = (struct ext3_extent_header*)handle->path[handle->level].buf;
        retval = ext2fs_extent_block_csum_set(handle->fs, handle->ino, eh);
        if (retval)
            return retval;

        retval = io_channel_write_blk64(handle->fs->io, blk, 1,
 ...
 * extent before blk.
 */
errcode_t ext2fs_extent_goto2(ext2_extent_handle_t handle, int leaf_level,
                              blk64_t blk)
{
    struct ext2fs_extent extent;
    errcode_t retval;
    ...
    while (1)
    {
        if (handle->max_depth - handle->level == leaf_level)
        {
            ...
            if ((blk >= extent.e_lblk) && (blk < extent.e_lblk + extent.e_len))
                return 0;
            if (blk < extent.e_lblk)
            {
                retval =
                    ext2fs_extent_get(handle, EXT2_EXTENT_PREV_SIB, &extent);
                return EXT2_ET_EXTENT_NOT_FOUND;
            }
            retval = ext2fs_extent_get(handle, EXT2_EXTENT_NEXT_SIB, &extent);
            if (retval == EXT2_ET_EXTENT_NO_NEXT)
                return EXT2_ET_EXTENT_NOT_FOUND;
            if (retval)
                return retval;
        ...
        retval = ext2fs_extent_get(handle, EXT2_EXTENT_PREV_SIB, &extent);
        if (retval)
            return retval;
...
errcode_t ext2fs_extent_goto(ext2_extent_handle_t handle, blk64_t blk)
{
                                ...
                                struct ext2fs_extent* extent)
{
    ...
    if (handle->level == handle->max_depth)
    {
        ...
        if (extent->e_flags & EXT2_EXTENT_FLAGS_UNINIT)
        {
            ...
            ex->ee_len = ext2fs_cpu_to_le16(extent->e_len + EXT_INIT_MAX_LEN);
        }
        else
        {
            if (extent->e_len > EXT_INIT_MAX_LEN)
                                   ...
                                   int expand_allowed)
{
    ...
    if (handle->level == 0)
    {
        ...
        retval = ext2fs_get_memzero(
            (handle->max_paths + 1) * sizeof(struct extent_path), &newpath);
        if (retval)
            goto done;
    }
    else
    ...
    printf("will copy out %d of %d entries at level %d\n", tocopy,
           ext2fs_le16_to_cpu(eh->eh_entries), handle->level);
#endif
    ...
    if (!goal_blk)
        goal_blk =
            ext2fs_find_inode_goal(handle->fs, handle->ino, handle->inode, 0);
    retval =
        ext2fs_alloc_block2(handle->fs, goal_blk, block_buf, &new_node_pblk);
    if (retval)
        goto done;
    ...
    neweh->eh_max = ext2fs_cpu_to_le16(
        (handle->fs->blocksize - sizeof(struct ext3_extent_header)) /
        sizeof(struct ext3_extent));

    /* then the entries for the new block... */
    memcpy(EXT_FIRST_INDEX(neweh),
           EXT_FIRST_INDEX(eh) + (ext2fs_le16_to_cpu(eh->eh_entries) - tocopy),
    ...
    retval =
        io_channel_write_blk64(handle->fs->io, new_node_pblk, 1, block_buf);
    ...
    if (handle->level == 0)
    {
        memcpy(newpath, path, sizeof(struct extent_path) * handle->max_paths);
        handle->path = newpath;
        newpath = path;
        path = handle->path;
        path->entries = 1;
    ...
    else
    {
        ...
        extent.e_len = new_node_length;
        retval =
            ext2fs_extent_insert(handle, EXT2_EXTENT_INSERT_AFTER, &extent);
        if (retval)
            goto done;
    }
                               ...
                               struct ext2fs_extent* extent)
{
    ...
    if (path->left >= 0)
        memmove(ix + 1, ix, (path->left + 1) * sizeof(struct ext3_extent_idx));
    path->left++;
    path->entries++;
...
errcode_t ext2fs_extent_set_bmap(ext2_extent_handle_t handle, blk64_t logical,
                                 blk64_t physical, int flags)
{
    ...
    {
        dbg_print_extent("set_bmap: next_extent", &next_extent);
        has_next = 1;
    ...
    {
        has_prev = 1;
        dbg_print_extent("set_bmap: prev_extent", &prev_extent);
    ...
    if (!mapped)
    {
        ...
        {
            retval =
                ext2fs_extent_get(handle, EXT2_EXTENT_NEXT_LEAF, &next_extent);
            if (retval)
                goto done;
            next_extent.e_len++;
            next_extent.e_lblk--;
    ...
    else if (logical == extent.e_lblk)
    {
        ...
        {
            if (has_prev &&
                (logical == (prev_extent.e_lblk + prev_extent.e_len)) &&
                ...
                ((int)prev_extent.e_len < max_len - 1))
            {
                ...
                if (retval)
                    goto done;
                prev_extent.e_len++;
                retval = ext2fs_extent_replace(handle, 0, &prev_extent);
            }
            else
                retval = ext2fs_extent_insert(handle, 0, &newextent);
            if (retval)
            ...
            if (retval)
                goto done;
            retval = ext2fs_extent_get(handle, EXT2_EXTENT_NEXT_LEAF, &extent);
            if (retval)
                goto done;
        }
        extent.e_pblk++;
    ...
    else
    {
        ...
        if (physical)
        {
            ...
            if (retval)
            {
                ...
                if (r2 == 0)
                    (void)ext2fs_extent_replace(handle, 0, &save_extent);
                goto done;
            }
        }
        ...
        if (retval)
        {
            if (physical)
            {
                r2 = ext2fs_extent_goto(handle, newextent.e_lblk);
                if (r2 == 0)
    ...
    if (path->entries == 0 && handle->level)
    {
        if (!(flags & EXT2_EXTENT_DELETE_KEEP_EMPTY))
        {
            ...
            handle->inode->i_blocks -=
                (handle->fs->blocksize * EXT2FS_CLUSTER_RATIO(handle->fs)) /
                512;
            retval = ext2fs_write_inode(handle->fs, handle->ino, handle->inode);
...
size_t ext2fs_max_extent_depth(ext2_extent_handle_t handle)
{
    ...
    last_result =
        1 + ((ul_log2(EXT_MAX_EXTENT_LBLK) - ul_log2(iblock_extents)) /
