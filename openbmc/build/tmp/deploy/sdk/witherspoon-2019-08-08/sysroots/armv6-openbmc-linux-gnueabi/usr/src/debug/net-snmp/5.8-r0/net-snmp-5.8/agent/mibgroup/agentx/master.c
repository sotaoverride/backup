            ...
            void real_init_master(void)
{
    ...
    {
        agentx_sockets = strdup(netsnmp_ds_get_string(
        ...
        if (agentx_sockets[0] != '/')
        {
            ...
            {
                snmp_log(LOG_ERR,
                         "Error: %s transport is not supported, disabling "
                         "agentx/master.\n",
                         agentx_sockets);
    ...
    {
        int agentx_dir_perm = netsnmp_ds_get_int(NETSNMP_DS_APPLICATION_ID,
    ...
    while (cp1)
    {
        ...
        else
        {
            ...
            if (t->domain == netsnmp_UnixDomain && t->local != NULL)
            {
                 ...
                 * AgentX socket
                 */
                int agentx_sock_perm = netsnmp_ds_get_int(
                    NETSNMP_DS_APPLICATION_ID, NETSNMP_DS_AGENT_X_SOCK_PERM);
                int agentx_sock_user = netsnmp_ds_get_int(
                    NETSNMP_DS_APPLICATION_ID, NETSNMP_DS_AGENT_X_SOCK_USER);
                int agentx_sock_group = netsnmp_ds_get_int(
                    NETSNMP_DS_APPLICATION_ID, NETSNMP_DS_AGENT_X_SOCK_GROUP);
 ...
 */
int agentx_got_response(int operation, netsnmp_session* session, int reqid,
                        netsnmp_pdu* pdu, void* magic)
{
    ...
    {
        DEBUGMSGTL(
            ("agentx/master", "response too late on session %8p\n", session));
    ...
    switch (operation)
    {
        ...
        case NETSNMP_CALLBACK_OP_SEND_FAILED:
            ...
            {
                DEBUGMSGTL(
                    ("agentx/master", "disconnect on session %8p\n", session));
            }
            else
            {
                DEBUGMSGTL(
                    ("agentx/master", "send failed on session %8p\n", session));
            }
            close_agentx_session(session, -1);
            netsnmp_handler_mark_requests_as_delegated(
                requests, REQUEST_IS_NOT_DELEGATED);
            netsnmp_set_request_error(cache->reqinfo,
                                      requests, /* XXXWWW: should be index=0 */
                                      SNMP_ERR_GENERR);
            netsnmp_free_delegated_cache(cache);
            return 0;
    ...
    DEBUGMSGTL(("agentx/master",
                "got response errstat=%ld, (req=0x%x,trans="
                "0x%x,sess=0x%x)\n",
    ...
    if (pdu->errstat != AGENTX_ERR_NOERROR)
    {
        ...
        for (request = requests, i = 1; request; request = request->next, i++)
        {
            if (i == pdu->errindex)
            {
                 ...
                 */
                netsnmp_set_request_error(cache->reqinfo, request, err);
                ret = 1;
            }
 ...
 *                 [FREE]
 */
int agentx_master_handler(netsnmp_mib_handler* handler,
                          ...
                          netsnmp_request_info* requests)
{
    ...
    while (request)
    {
        ...
        if (reqinfo->mode == MODE_GETNEXT || reqinfo->mode == MODE_GETBULK)
        {
            ...
            {
                DEBUGMSGTL(
                    ("agentx/master", "inexact request preceding region ("));
        ...
        else
        {
            snmp_pdu_add_variable(
                pdu, request->requestvb->name, request->requestvb->name_length,
                request->requestvb->type, request->requestvb->val.string,
     ...
     */
    if (pdu->command != AGENTX_MSG_CLEANUPSET)
        cb_data = netsnmp_create_delegated_cache(handler, reginfo, reqinfo,
                                                 requests, (void*)ax_session);
    else
        cb_data = NULL;
     ...
     */
    DEBUGMSGTL(
        ("agentx/master", "sending pdu (req=0x%x,trans=0x%x,sess=0x%x)\n",
