...
struct bde_header_win7
{
    ...
    /*  71 */ unsigned char
        volume_label[11]; /* "NO NAME\x20\x20\x20\x20" only */
...
{
    size_t i;
    static const char* map[] = {[BDE_VERSION_VISTA] = BDE_MAGIC_VISTA,
 ...
 */
static int get_bitlocker_headers(blkid_probe pr, int* type,
                                 ...
                                 const unsigned char** buf_fve)
{
    ...
    switch (kind)
    {
        case BDE_VERSION_WIN7:
            off = le64_to_cpu(
                ((const struct bde_header_win7*)buf)->fve_metadata_offset);
            break;
        case BDE_VERSION_TOGO:
            off = le64_to_cpu(
                ((const struct bde_header_togo*)buf)->fve_metadata_offset);
            break;
...
static int probe_bitlocker(blkid_probe pr, const struct blkid_idmag* mag
                           __attribute__((__unused__)))
{
    ...
    {
        const struct bde_header_win7* hdr =
            (const struct bde_header_win7*)buf_hdr;

        /* Unfortunately, it seems volume_serial is always zero */
        blkid_probe_sprintf_uuid(pr, (const unsigned char*)&hdr->volume_serial,
                                 sizeof(hdr->volume_serial), "%016d",
                                 le32_to_cpu(hdr->volume_serial));
    }

    if (buf_fve)
    {
        const struct bde_fve_metadata* fve =
            (const struct bde_fve_metadata*)buf_fve;
...
const struct blkid_idinfo bitlocker_idinfo = {
    ...
    .magics = {{.magic = BDE_MAGIC_VISTA, .len = 11},
