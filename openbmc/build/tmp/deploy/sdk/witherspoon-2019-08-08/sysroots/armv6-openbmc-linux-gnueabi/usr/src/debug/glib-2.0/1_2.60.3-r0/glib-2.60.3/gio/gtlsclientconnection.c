...
G_DEFINE_INTERFACE(GTlsClientConnection, g_tls_client_connection,
    ...
    g_tls_client_connection_default_init(GTlsClientConnectionInterface* iface)
{
    ...
    g_object_interface_install_property(
        iface, g_param_spec_flags("validation-flags", P_("Validation flags"),
    ...
    g_object_interface_install_property(
        iface,
        g_param_spec_object("server-identity", P_("Server identity"),
    ...
    g_object_interface_install_property(
        iface,
        g_param_spec_boolean("use-ssl3", P_("Use fallback"),
                             P_("Use fallback version of SSL/TLS rather than "
                                "most recent version"),
                             FALSE,
                             G_PARAM_READWRITE | G_PARAM_CONSTRUCT |
    ...
    /**
     * GTlsClientConnection:accepted-cas: (type GLib.List) (element-type
     * GLib.ByteArray)
     *
     ...
     */
    g_object_interface_install_property(
        iface, g_param_spec_pointer("accepted-cas", P_("Accepted CAs"),
                                    P_("Distinguished names of the CAs the "
                                       "server accepts certificates from"),
                                       ...
                                       GError** error)
{
    ...
    conn = g_initable_new(g_tls_backend_get_client_connection_type(backend),
                          NULL, error, "base-io-stream", base_io_stream,
                          "server-identity", server_identity, NULL);
                                                ...
                                                GTlsClientConnection* source)
{
    ...
    g_return_if_fail(
        G_TLS_CLIENT_CONNECTION_GET_INTERFACE(conn)->copy_session_state !=
        NULL);
