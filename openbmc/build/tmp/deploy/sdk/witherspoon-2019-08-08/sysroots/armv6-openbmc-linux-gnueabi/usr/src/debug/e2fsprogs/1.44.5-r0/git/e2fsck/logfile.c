...
static void expand_percent_expression(e2fsck_t ctx, char ch, struct string* s,
                                      int* flags)
{
...
void set_up_logging(e2fsck_t ctx)
{
    ...
    if (ctx->log_fn)
        ...
        profile_get_string(ctx->profile, "options", "log_filename", 0, 0,
                           &log_fn);
