...
static OPT_PAIR formats[] = {
    {"PEM/DER", OPT_FMT_PEMDER}, {"pkcs12", OPT_FMT_PKCS12},
...
static void opt_number_error(const char* v)
{
    ...
    struct strstr_pair_st
    {
        ...
        {"0x", "a hexadecimal"}, {"0X", "a hexadecimal"}, {"0", "an octal"}};

    for (i = 0; i < OSSL_NELEM(b); i++)
    {
        if (strncmp(v, b[i].prefix, strlen(b[i].prefix)) == 0)
        {
            BIO_printf(bio_err, "%s: Can't parse \"%s\" as %s number\n", prog,
                       v, b[i].name);
            return;
        }
    }
...
int opt_verify(int opt, X509_VERIFY_PARAM* vpm)
{
    ...
    switch ((enum range)opt)
    {
        ...
        case OPT_V_PURPOSE:
            ...
            {
                BIO_printf(bio_err, "%s: Internal error setting purpose %s\n",
                           prog, opt_arg());
                return 0;
            }
            break;
...
int opt_next(void)
{
    ...
    for (o = opts; o->name; ++o)
    {
        ...
        if (o->valtype == 0 || o->valtype == '-')
        {
            if (arg)
            {
                BIO_printf(bio_err, "%s: Option -%s does not take a value\n",
                           prog, p);
                return -1;
            }
            return o->retval;
        ...
        if (arg == NULL)
        {
            ...
            {
                BIO_printf(bio_err, "%s: Option -%s needs a value\n", prog,
                           o->name);
                return -1;
            }
            arg = argv[opt_index++];
        ...
        switch (o->valtype)
        {
            ...
            case 'p':
            case 'n':
                if (!opt_int(arg, &ival) || (o->valtype == 'p' && ival <= 0))
                {
                    BIO_printf(bio_err,
            ...
            case 'M':
                ...
                {
                    BIO_printf(bio_err, "%s: Invalid number \"%s\" for -%s\n",
                               prog, arg, o->name);
                    return -1;
                }
                break;
            ...
            case 'f':
                if (opt_format(arg,
                               ...
                               &ival))
                    break;
                BIO_printf(bio_err, "%s: Invalid format \"%s\" for -%s\n", prog,
                           arg, o->name);
                return -1;
        }
