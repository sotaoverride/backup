...
libc_hidden_proto(ether_aton_r) libc_hidden_proto(ether_ntoa_r)
...
#define DECLARE_NSS_PROTOTYPES(service)                                        \
    ...
    extern enum nss_status _nss_##service##_getetherent_r(                     \
        struct etherent* result, char* buffer, size_t buflen, int* errnop);    \
    extern enum nss_status _nss_##service##_gethostton_r(                      \
        const char* name, struct etherent* eth, char* buffer, size_t buflen,   \
        int* errnop);                                                          \
    extern enum nss_status _nss_##service##_getntohost_r(                      \
        const struct ether_addr* addr, struct etherent* eth, char* buffer,     \
        size_t buflen, int* errnop);

    DECLARE_NSS_PROTOTYPES(files) DECLARE_NSS_PROTOTYPES(nis)
