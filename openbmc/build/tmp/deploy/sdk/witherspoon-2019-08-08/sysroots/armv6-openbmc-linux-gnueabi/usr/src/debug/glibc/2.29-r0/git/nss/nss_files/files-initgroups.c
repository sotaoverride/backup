...
enum nss_status _nss_files_initgroups_dyn(const char* user, gid_t group,
    ...
    while (1)
    {
        ...
        if (n < 0)
        {
            if (!feof_unlocked(stream))
                status = ((*errnop = errno) == ENOMEM ? NSS_STATUS_TRYAGAIN
                                                      : NSS_STATUS_UNAVAIL);
            break;
        }

        struct group grp;
        int res = _nss_files_parse_grent(line, &grp, tmpbuf.data, tmpbuf.length,
                                         errnop);
        if (res == -1)
        {
