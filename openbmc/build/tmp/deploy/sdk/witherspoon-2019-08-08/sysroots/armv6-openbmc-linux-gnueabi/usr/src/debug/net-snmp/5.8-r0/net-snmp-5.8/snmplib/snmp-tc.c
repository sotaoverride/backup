...
netsnmp_feature_child_of(snmp_tc_all, libnetsnmp)
    ...
    netsnmp_feature_child_of(netsnmp_dateandtime_set_buf_from_vars,
                             netsnmp_unused)
    ...
    */
    ...
    int netsnmp_dateandtime_set_buf_from_vars(
        u_char* buf, size_t* bufsize, u_short year, u_char month, u_char day,
        u_char hour, u_char minutes, u_char seconds, u_char deci_seconds,
        int utc_offset_direction, u_char utc_offset_hours,
...
u_char* date_n_time(const time_t* when, size_t* length)
{
    struct tm* tm_p;
...
time_t ctime_to_timet(const char* str)
{
    struct tm tm;
 ...
 */
char check_rowstatus_transition(int oldValue, int newValue)
{
    /*
...
char check_rowstatus_with_storagetype_transition(int oldValue, int newValue,
                                                 int oldStorage)
{
    /*
     * can not destroy permanent or readonly rows
     */
    if ((RS_DESTROY == newValue) && ((SNMP_STORAGE_PERMANENT == oldStorage) ||
...
netsnmp_feature_child_of(check_storage_transition, snmp_tc_all)
    ...
    char check_storage_transition(int oldValue, int newValue)
{
    /*
     * From the SNMPv2-TC MIB:
