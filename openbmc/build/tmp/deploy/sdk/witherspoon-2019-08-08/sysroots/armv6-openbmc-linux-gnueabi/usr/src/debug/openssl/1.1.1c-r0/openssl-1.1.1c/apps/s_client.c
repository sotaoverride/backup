...
static int srp_Verify_N_and_g(const BIGNUM* N, const BIGNUM* g)
{
    ...
    int ret =
        ...
        BN_mod_exp(r, g, p, N, bn_ctx) && BN_add_word(r, 1) &&
        BN_cmp(r, N) == 0;
...
static int ssl_srp_verify_param_cb(SSL* s, void* arg)
{
    ...
    {
        if (srp_arg->debug)
            BIO_printf(bio_err, "SRP param N and g are not known params, going "
                                "to check deeper.\n");
...
static ossl_ssize_t checked_uint8(const char** inptr, void* out)
{
    ...
    if (((v == LONG_MIN || v == LONG_MAX) && e == ERANGE) || endp == in ||
        !isspace(_UC(*endp)) || v != (*result = (uint8_t)v))
    {
        return -1;
    }
...
const OPTIONS s_client_options[] = {
    ...
    {"maxfraglen", OPT_MAXFRAGLEN, 'p',
     "Enable Maximum Fragment Length Negotiation (len values: 512, 1024, 2048 "
     "and 4096)"},
    {"fallback_scsv", OPT_FALLBACKSCSV, '-', "Send the fallback SCSV"},
    {"name", OPT_PROTOHOST, 's',
     "Hostname to use for \"-starttls lmtp\", \"-starttls smtp\" or "
    ...
    {"alpn", OPT_ALPN, 's',
     "Enable ALPN extension, considering named protocols supported "
    ...
    {"nextprotoneg", OPT_NEXTPROTONEG, 's',
     "Enable NPN extension, considering named protocols supported "
     "(comma-separated list)"},
#endif
...
static const OPT_PAIR services[] = {{"smtp", PROTO_SMTP},
                                    {"pop3", PROTO_POP3},
                                    {"imap", PROTO_IMAP},
                                    {"ftp", PROTO_FTP},
                                    {"xmpp", PROTO_XMPP},
...
#define IS_INET_FLAG(o)                                                        \
    (o == OPT_4 || o == OPT_6 || o == OPT_HOST || o == OPT_PORT ||             \
     o == OPT_CONNECT)
...
/* Known false-positive of MemorySanitizer. */
        ...
        switch (o)
        {
            ...
            case OPT_MAXFRAGLEN:
                ...
                switch (len)
                {
                    ...
                    default:
                        BIO_printf(bio_err,
                                   "%s: Max Fragment Len %u is out of "
                                   "permitted values",
                                   prog, len);
                        goto opthelp;
                }
    ...
    if (argc == 1)
    {
        ...
        {
            BIO_printf(bio_err,
                       "%s: must not provide both -connect option and target "
                       "parameter\n",
                       prog);
            goto opthelp;
        }
    ...
    else
    {
        ...
        {
            BIO_printf(bio_err,
                       "%s: -connect argument or target parameter malformed or "
                       "ambiguous\n",
                       prog);
            goto end;
        }
    ...
    {
        if (!SSL_CTX_add_client_custom_ext(ctx, serverinfo_types[i], NULL, NULL,
...
re_start:
    ...
    switch ((PROTOCOL_CHOICE)starttls_proto)
    {
        ...
        case PROTO_SMTP:
        {
            ...
            BIO_free(fbio);
            if (!foundit)
                BIO_printf(bio_err, "Didn't find STARTTLS in server response,"
                                    " trying anyway...\n");
        ...
        case PROTO_TELNET:
        {
            static const unsigned char tls_do[] = {/* IAC    DO   START_TLS */
                                                   255, 253, 46};
            static const unsigned char tls_will[] = {/* IAC  WILL START_TLS */
                                                     255, 251, 46};
        ...
        case PROTO_IRC:
        {
            ...
            do
            {
                 ...
                 */
                if (!BIO_get_buffer_num_lines(fbio) && !BIO_pending(fbio) &&
                    !BIO_pending(sbio) &&
                    select(width, (void*)&readfds, NULL, NULL, &timeout) < 1)
                {
                    BIO_printf(bio_err,
        ...
        case PROTO_MYSQL:
        {
            ...
            else if (bytes < 21)
            {
                ...
                goto shut;
                     ...
                     (4 + packet[0] + (packet[1] << 8) + (packet[2] << 16)))
            {
            ...
            if (packet[pos++] != '\0')
            { /* verify filler */
                BIO_printf(bio_err, "MySQL packet is broken.\n");
                goto shut;
            }
    ...
    if (early_data_file != NULL &&
        ((SSL_get0_session(con) != NULL &&
         ...
         (psksess != NULL && SSL_SESSION_get_max_early_data(psksess) > 0)))
    {
        ...
        if (!ssl_pending)
        {
            ...
            if (!write_tty)
            {
                if (read_tty)
                {
                    tv.tv_sec = 1;
                    tv.tv_usec = 0;
                    i = select(width, (void*)&readfds, (void*)&writefds, NULL,
                               &tv);
        ...
        if (!ssl_pending && FD_ISSET(SSL_get_fd(con), &writefds))
        {
            ...
            switch (SSL_get_error(con, k))
            {
                ...
                case SSL_ERROR_SYSCALL:
                    ...
                    break;
                case SSL_ERROR_WANT_ASYNC_JOB:
                    /* This shouldn't ever happen in s_client - treat as an
                     * error */
                case SSL_ERROR_SSL:
                    ERR_print_errors(bio_err);
                    goto shut;
...
static int is_dNS_name(const char* host)
{
    ...
    /* dNS name must not be all numeric and labels must be shorter than 64
     * characters. */
