...
// Getting error at this point means, that your STL library is
// old/lame/misconfigured. If nothing can be done with STL library, define
// BOOST_LEXICAL_CAST_ASSUME_C_LOCALE, but beware: lexical_cast will understand
// only 'C' locale delimeters and thousands separators.
#error                                                                         \
    "Unable to use <locale> header. Define BOOST_LEXICAL_CAST_ASSUME_C_LOCALE to force "
...
class basic_unlockedbuf : public basic_pointerbuf<CharT, BufferType>
{
  public:
    ...
    charT* pbase() const
    {
        return base_type::pbase();
    }
    BufferType* setbuf(char_type* s, streamsize n)
    {
...
struct out_stream_helper_trait
{
    ...
    typedef basic_unlockedbuf<std::basic_stringbuf<CharT, Traits>, CharT>
        stringbuffer_t;
...
template <class CharT // a result of widest_char transformation
          ,
          class Traits, bool RequiresStringbuffer,
...
class lexical_istream_limited_src : boost::noncopyable
{
  ...
  private:
    ...
    bool shl_char_array_limited(CharT const* str,
                                std::size_t max_size) BOOST_NOEXCEPT
    {
        start = str;
    ...
    !defined(_STLPORT_VERSION)
                 ...
                 sprintf(begin,
                           ...
                           "%.*g",
                           static_cast<int>(
                               boost::detail::lcast_get_precision<float>()),
                           val_as_double);
        return finish > start;
    }
    ...
    !defined(_STLPORT_VERSION)
            ...
            sprintf(begin,
                      ...
                      "%.*Lg",
                      static_cast<int>(
                          boost::detail::lcast_get_precision<long double>()),
                      val);
        return finish > start;
    }
#endif

#if !defined(BOOST_LCAST_NO_WCHAR_T) && !defined(BOOST_NO_SWPRINTF) &&         \
    !defined(__MINGW32__)
    bool shl_real_type(float val, wchar_t* begin)
    {
        ...
        const double val_as_double = val;
        finish =
            start + swprintf(begin, CharacterBufferSize, L"%.*g",
                             static_cast<int>(
                                 boost::detail::lcast_get_precision<float>()),
                             val_as_double);
        return finish > start;
    ...
    bool shl_real_type(long double val, wchar_t* begin)
    {
        ...
        finish =
            start +
            swprintf(begin, CharacterBufferSize, L"%.*Lg",
                     static_cast<int>(
                         boost::detail::lcast_get_precision<long double>()),
                     val);
        return finish > start;
  ...
  public:
    ...
    template <class Alloc>
    bool operator<<(boost::container::basic_string<CharT, Traits, Alloc> const&
                        str) BOOST_NOEXCEPT
    {
        start = str.data();
        finish = start + str.length();
    ...
    bool
        operator<<(const iterator_range<const signed char*>& rng) BOOST_NOEXCEPT
    {
    ...
    bool operator<<(wchar_t const* str)
    {
        return shl_char_array(str);
    }
    ...
    bool operator<<(char16_t ch)
    {
        return shl_char(ch);
    }
    bool operator<<(char16_t* str)
    {
        return shl_char_array(str);
    }
    bool operator<<(char16_t const* str)
    {
        return shl_char_array(str);
    }
#endif
    ...
    bool operator<<(char32_t* str)
    {
        return shl_char_array(str);
    }
    bool operator<<(char32_t const* str)
    {
        return shl_char_array(str);
    }
#endif
    ...
    bool operator<<(boost::ulong_long_type n)
    {
        return shl_unsigned(n);
    }
    bool operator<<(boost::long_long_type n)
    {
        return shl_signed(n);
    }
#elif defined(BOOST_HAS_MS_INT64)
    bool operator<<(unsigned __int64 n)
    {
        return shl_unsigned(n);
    }
    bool operator<<(__int64 n)
    {
        return shl_signed(n);
    }
#endif
    ...
    bool operator<<(const boost::int128_type& n)
    {
        return shl_signed(n);
    }
#endif
        ...
        operator<<(boost::array<C, N> const& input) BOOST_NOEXCEPT
    {
        BOOST_STATIC_ASSERT_MSG(
            ...
            "boost::array<C, N> and boost::array<const C, N> must have exactly "
            "the same layout.");
        return ((*this) << reinterpret_cast<boost::array<const C, N> const&>(
                    input));
    }
    ...
    bool operator<<(boost::array<const unsigned char, N> const& input)
        BOOST_NOEXCEPT
    {
        return ((*this) << reinterpret_cast<boost::array<const char, N> const&>(
                    input));
    }
...
class lexical_ostream_limited_src : boost::noncopyable
{
  ...
  public:
    lexical_ostream_limited_src(const CharT* begin,
                                const CharT* end) BOOST_NOEXCEPT : start(begin),
                                                                   finish(end)
    {
    }
  ...
  private:
    ...
    bool shr_unsigned(Type& output)
    {
        ...
        bool const succeed =
            lcast_ret_unsigned<Traits, Type, CharT>(output, start, finish)
                .convert();
    ...
    bool shr_signed(Type& output)
    {
        ...
        bool succeed =
            lcast_ret_unsigned<Traits, utype, CharT>(out_tmp, start, finish)
                .convert();
        if (has_minus)
        {
            utype const comp_val =
                (static_cast<utype>(1) << std::numeric_limits<Type>::digits);
    ...
    bool shr_using_base_class(InputStreamable& output)
    {
        ...
        BOOST_STATIC_ASSERT_MSG((boost::is_same<char, CharT>::value),
                                "boost::lexical_cast can not convert, because "
                                "your STL library does not "
                                "support such conversions. Try updating it.");
#endif

#if defined(BOOST_NO_STRINGSTREAM)
        std::istrstream stream(
            start, static_cast<std::istrstream::streamsize>(finish - start));
#else
  ...
  public:
...
#ifdef BOOST_HAS_INT128
    bool operator>>(boost::uint128_type& output)
    {
        return shr_unsigned(output);
    }
    ...
    bool operator>>(std::array<C, N>& output) BOOST_NOEXCEPT
    {
        BOOST_STATIC_ASSERT_MSG(
            (sizeof(std::array<C, N>) == sizeof(boost::array<C, N>)),
            "std::array<C, N> and boost::array<C, N> must have exactly the "
            "same layout.");
