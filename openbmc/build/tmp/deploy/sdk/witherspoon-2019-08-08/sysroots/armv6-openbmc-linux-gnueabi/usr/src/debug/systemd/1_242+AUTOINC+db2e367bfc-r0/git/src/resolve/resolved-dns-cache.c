...
static int dns_cache_init(DnsCache* c)
{
    ...
    r = prioq_ensure_allocated(&c->by_expiry,
...
static usec_t calculate_until(DnsResourceRecord* rr, uint32_t nsec_ttl,
                              usec_t timestamp, bool use_soa_minimum)
{
    uint32_t ttl;
    usec_t u;
...
static void dns_cache_item_update_positive(
    DnsCache* c, DnsCacheItem* i, DnsResourceRecord* rr, bool authenticated,
    bool shared_owner, usec_t timestamp, int ifindex, int owner_family,
...
static int dns_cache_put_positive(DnsCache* c, DnsResourceRecord* rr,
                                  bool authenticated, bool shared_owner,
                                  usec_t timestamp, int ifindex,
                                  int owner_family,
                                  const union in_addr_union* owner_address)
{
    ...
    if (rr->ttl <= 0)
    {
        ...
        log_debug("%s: %s",
                  k > 0 ? "Removed zero TTL entry from cache"
    ...
    if (existing)
    {
        dns_cache_item_update_positive(c, existing, rr, authenticated,
                                       shared_owner, timestamp, ifindex,
...
static int dns_cache_put_negative(DnsCache* c, DnsResourceKey* key, int rcode,
                                  bool authenticated, uint32_t nsec_ttl,
                                  usec_t timestamp, DnsResourceRecord* soa,
                                  int owner_family,
                                  const union in_addr_union* owner_address)
{
    ...
    if (IN_SET(rcode, DNS_RCODE_SUCCESS, DNS_RCODE_NXDOMAIN))
    {
        ...
        {
            log_debug(
                "Not caching negative entry with zero SOA/NSEC/NSEC3 TTL: %s",
    ...
    i->type = rcode == DNS_RCODE_SUCCESS
                  ...
                  : rcode == DNS_RCODE_NXDOMAIN ? DNS_CACHE_NXDOMAIN
                                                : DNS_CACHE_RCODE;
    i->until = i->type == DNS_CACHE_RCODE
                   ? timestamp + CACHE_TTL_STRANGE_RCODE_USEC
    ...
    if (i->type == DNS_CACHE_NXDOMAIN)
    {
        ...
        i->key = dns_resource_key_new(key->class, DNS_TYPE_ANY,
...
static void dns_cache_remove_previous(DnsCache* c, DnsResourceKey* key,
                                      DnsAnswer* answer)
{
...
static bool rr_eligible(DnsResourceRecord* rr)
{
    ...
    /* When we see an NSEC/NSEC3 RR, we'll only cache it if it is from the lower
     * zone, not the upper zone, since that's where the interesting bits are
     * (with exception of DS RRs). Of course, this way we cannot derive DS
...
int dns_cache_put(DnsCache* c, DnsResourceKey* key, int rcode,
                  DnsAnswer* answer, bool authenticated, uint32_t nsec_ttl,
                  usec_t timestamp, int owner_family,
                  const union in_addr_union* owner_address)
{
    ...
    /* We only care for positive replies and NXDOMAINs, on all other replies we
     * will simply flush the respective entries, and that's it. (Well, with one
     * further exception: since some DNS zones (akamai!) return SERVFAIL
     * consistently for some lookups, and forwarders tend to propagate that
    ...
    if (IN_SET(rcode, DNS_RCODE_SUCCESS, DNS_RCODE_NXDOMAIN))
    {
        if (dns_answer_size(answer) <= 0)
        {
            if (key)
            {
                ...
                log_debug(
                    "Not caching negative entry without a SOA record: %s",
                    dns_resource_key_to_string(key, key_str, sizeof key_str));
            }
            return 0;
        }
    ...
    {
        /* Only cache SERVFAIL as "weird" rcode for now. We can add more later,
         * should that turn out to be beneficial. */
    ...
    DNS_ANSWER_FOREACH_FULL(rr, ifindex, flags, answer)
    {
        ...
        r = dns_cache_put_positive(c, rr, flags & DNS_ANSWER_AUTHENTICATED,
                                   flags & DNS_ANSWER_SHARED_OWNER, timestamp,
                                   ifindex, owner_family, owner_address);
        if (r < 0)
            goto fail;
    }
    ...
    /* See https://tools.ietf.org/html/rfc2308, which say that a matching SOA
     * record in the packet is used to enable negative caching. We apply one
     * exception though: if we are about to cache a weird rcode we do so
     * regardless of a SOA. */
    ...
    r = dns_cache_put_negative(c, key, rcode, authenticated, nsec_ttl,
                               timestamp, soa, owner_family, owner_address);
    if (r < 0)
        goto fail;
...
static DnsCacheItem*
    dns_cache_get_by_key_follow_cname_dname_nsec(DnsCache* c, DnsResourceKey* k)
{
    DnsCacheItem* i;
    const char* n;
    int r;
    ...
    i = hashmap_get(c->by_key,
                    &DNS_RESOURCE_KEY_CONST(k->class, DNS_TYPE_ANY, n));
    ...
    if (dns_type_may_redirect(k->type))
    {
        ...
        i = hashmap_get(c->by_key,
                        &DNS_RESOURCE_KEY_CONST(k->class, DNS_TYPE_CNAME, n));
        ...
        for (;;)
        {
            ...
            i = hashmap_get(c->by_key, &DNS_RESOURCE_KEY_CONST(
                                           k->class, DNS_TYPE_DNAME, n));
            if (i && i->type != DNS_CACHE_NODATA)
                return i;
    ...
    if (k->type != DNS_TYPE_NSEC)
    {
        ...
        i = hashmap_get(c->by_key,
                        &DNS_RESOURCE_KEY_CONST(k->class, DNS_TYPE_NSEC, n));
        if (i)
            return i;
    }
...
int dns_cache_lookup(DnsCache* c, DnsResourceKey* key, bool clamp_ttl,
                     int* rcode, DnsAnswer** ret, bool* authenticated)
{
    ...
    if (found_rcode >= 0)
    {
        log_debug("RCODE %s cache hit for %s", dns_rcode_to_string(found_rcode),
                  dns_resource_key_to_string(key, key_str, sizeof(key_str)));
    ...
    {
        /* Note that we won't derive information for DS RRs from an NSEC,
         * because we only cache NSEC RRs from
    ...
    log_debug("%s cache hit for %s",
              n > 0 ? "Positive" : nxdomain ? "NXDOMAIN" : "NODATA",
              dns_resource_key_to_string(key, key_str, sizeof key_str));
    ...
    LIST_FOREACH(by_key, j, first)
    {
        ...
        if (clamp_ttl)
        {
            ...
            r = dns_resource_record_clamp_ttl(&rr, LESS_BY(j->until, current) /
                                                       USEC_PER_SEC);
            if (r < 0)
                return r;
        }

        r = dns_answer_add(answer, rr ?: j->rr, j->ifindex,
                           j->authenticated ? DNS_ANSWER_AUTHENTICATED : 0);
        if (r < 0)
            return r;
    }
...
int dns_cache_check_conflicts(DnsCache* cache, DnsResourceRecord* rr,
                              int owner_family,
                              const union in_addr_union* owner_address)
{
...
int dns_cache_export_shared_to_packet(DnsCache* cache, DnsPacket* p)
{
    ...
    HASHMAP_FOREACH(i, cache->by_key, iterator)
    {
        ...
        LIST_FOREACH(by_key, j, i)
        {
            ...
            {
                /* For mDNS, if we're unable to stuff all known answers into the
                 * given packet, allocate a new one, push the RR into that one
                 * and link it to the current one.
                 */
    ...
    HASHMAP_FOREACH(i, cache->by_key, iterator)
    {
        ...
        LIST_FOREACH(by_key, j, i)
        {
            ...
            else
            {
                ...
                fputs(
                    dns_resource_key_to_string(j->key, key_str, sizeof key_str),
                    f);
                fputs(" -- ", f);
