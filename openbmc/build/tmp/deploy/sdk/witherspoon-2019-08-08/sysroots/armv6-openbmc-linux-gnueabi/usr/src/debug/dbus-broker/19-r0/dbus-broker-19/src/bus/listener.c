 ...
 */
int listener_init_with_fd(Listener* l, Bus* bus, DispatchContext* dispatcher,
                          int socket_fd, PolicyRegistry* policy)
{
    ...
    *listener = (Listener)LISTENER_NULL(*listener);
    ...
    r = dispatch_file_init(&listener->socket_file, dispatcher,
                           listener_dispatch, socket_fd, EPOLLIN, EPOLLIN);
    if (r)
        return error_fold(r);
...
int listener_set_policy(Listener* listener, PolicyRegistry* registry)
{
    ...
    c_list_for_each_entry(peer, &listener->peer_list, listener_link)
    {
        ...
        r = policy_snapshot_new(&policy, registry, peer->seclabel,
                                peer->user->uid, peer->gids, peer->n_gids);
        if (r)
            return error_fold(r);
