...
static int release_inode_block(ext2_filsys fs, blk64_t* block_nr,
                               ...
                               void* priv_data)
{
    ...
    if (pb->truncating)
    {
        ...
        if (blockcnt < 0)
        {
            ...
            pb->errcode = io_channel_read_blk64(fs->io, blk, 1, pb->buf);
            if (pb->errcode)
                goto return_abort;

            limit = fs->blocksize >> 2;
            for (i = 0, bp = (blk_t*)pb->buf; i < limit; i++, bp++)
                if (*bp)
                    return 0;
        }
        /*
        ...
        if ((blockcnt == pb->truncate_block) && pb->truncate_offset)
        {
            ...
            pb->errcode = io_channel_write_blk64(fs->io, blk, 1, pb->buf);
            if (pb->errcode)
                goto return_abort;
        }
        pb->truncated_blocks++;
                                ...
                                struct problem_context* pctx)
{
    ...
    if (inode->i_links_count)
    {
        ...
        pb.truncate_block = (e2_blkcnt_t)(
            (EXT2_I_SIZE(inode) + fs->blocksize - 1) / fs->blocksize);
    ...
    if (retval)
    {
        com_err("release_inode_blocks", retval,
                _("while calling ext2fs_block_iterate for inode %u"), ino);
        return 1;
    }
    if (pb.abort)
        return 1;
    ...
    {
        retval =
            ext2fs_adjust_ea_refcount3(fs, blk, block_buf, -1, &count, ino);
        ...
        {
            if (ctx->qctx)
                quota_data_sub(ctx->qctx, inode, 0, ctx->fs->blocksize);
            ext2fs_block_alloc_stats2(fs, blk, -1);
            ctx->free_blocks++;
        }
...
static int release_orphan_inodes(e2fsck_t ctx)
{
    ...
    while (ino)
    {
        ...
        if (next_ino && ((next_ino < EXT2_FIRST_INODE(fs->super)) ||
        ...
        e2fsck_write_inode_full(ctx, ino, EXT2_INODE(&inode), sizeof(inode),
                                "delete_file");
        ino = next_ino;
    }
...
void check_resize_inode(e2fsck_t ctx)
{
    ...
    if (!ext2fs_has_feature_resize_inode(fs->super))
    {
        if (fs->super->s_reserved_gdt_blocks)
        {
            ...
            if (fix_problem(ctx, PR_0_NONZERO_RESERVED_GDT_BLOCKS, &pctx))
            {
                fs->super->s_reserved_gdt_blocks = 0;
                ext2fs_mark_super_dirty(fs);
            }
    ...
    if (!ext2fs_has_feature_resize_inode(fs->super))
    {
            ...
            fix_problem(ctx, PR_0_CLEAR_RESIZE_INODE, &pctx))
        {
            ...
            e2fsck_write_inode(ctx, EXT2_RESIZE_INO, &inode, "clear_resize");
        }
        return;
    }
...
void check_super_block(e2fsck_t ctx)
{
    ...
    ctx->invalid_inode_bitmap_flag = (int*)e2fsck_allocate_memory(
        ctx, sizeof(int) * fs->group_desc_count, "invalid_inode_bitmap");
    ctx->invalid_block_bitmap_flag = (int*)e2fsck_allocate_memory(
        ctx, sizeof(int) * fs->group_desc_count, "invalid_block_bitmap");
    ctx->invalid_inode_table_flag = (int*)e2fsck_allocate_memory(
        ctx, sizeof(int) * fs->group_desc_count, "invalid_inode_table");
     ...
     */
    if (!check_super_value(ctx, "inodes_count", sb->s_inodes_count, MIN_CHECK,
                           1, 0))
        return;
    ...
    if (!check_super_value(
            ...
            (EXT2_MAX_CLUSTER_LOG_SIZE - EXT2_MIN_CLUSTER_LOG_SIZE)))
        return;
    if (!check_super_value(ctx, "clusters_per_group", sb->s_clusters_per_group,
    ...
    if (!check_super_value(ctx, "reserved_gdt_blocks",
        ...
        return;
    if (!check_super_value(ctx, "desc_size", sb->s_desc_size,
                           MAX_CHECK | LOG2_CHECK, 0, EXT2_MAX_DESC_SIZE))
        return;
    ...
    if (sb->s_rev_level > EXT2_GOOD_OLD_REV &&
        !check_super_value(ctx, "first_ino", sb->s_first_ino,
                           MIN_CHECK | MAX_CHECK, EXT2_GOOD_OLD_FIRST_INO,
                           sb->s_inodes_count))
        return;
    inode_size = EXT2_INODE_SIZE(sb);
    if (!check_super_value(ctx, "inode_size", inode_size,
                           MIN_CHECK | MAX_CHECK | LOG2_CHECK,
                           EXT2_GOOD_OLD_INODE_SIZE, fs->blocksize))
        return;
    if (sb->s_blocks_per_group !=
        (sb->s_clusters_per_group * EXT2FS_CLUSTER_RATIO(fs)))
    {
    ...
    should_be =
        (sb->s_log_block_size == 0 && EXT2FS_CLUSTER_RATIO(fs) == 1) ? 1 : 0;
    ...
    {
        unsigned min = sizeof(((struct ext2_inode_large*)0)->i_extra_isize) +
        ...
        if (sb->s_min_extra_isize &&
            (sb->s_min_extra_isize < min || sb->s_min_extra_isize > max ||
             sb->s_min_extra_isize & 3) &&
            fix_problem(ctx, PR_0_BAD_MIN_EXTRA_ISIZE, &pctx))
        {
            sb->s_min_extra_isize =
                (sizeof(struct ext2_inode_large) - EXT2_GOOD_OLD_INODE_SIZE);
            ext2fs_mark_super_dirty(fs);
        }
        pctx.num = sb->s_want_extra_isize;
        if (sb->s_want_extra_isize &&
            (sb->s_want_extra_isize < min || sb->s_want_extra_isize > max ||
     ...
     */
    profile_get_boolean(ctx->profile, "options", "clear_test_fs_flag", 0, 1,
                        &clear_test_fs_flag);
    if (!(ctx->options & E2F_OPT_READONLY) && clear_test_fs_flag &&
    ...
    if (!(ctx->options & E2F_OPT_READONLY) &&
        ...
        (fs->super->s_feature_compat || fs->super->s_feature_ro_compat ||
     ...
     */
    profile_get_boolean(ctx->profile, "options", "accept_time_fudge", 0, 1,
                        &accept_time_fudge);
    profile_get_boolean(ctx->profile, "options", "buggy_init_scripts", 0,
                        accept_time_fudge, &accept_time_fudge);
     ...
     */
    if (!broken_system_clock && !(ctx->flags & E2F_FLAG_TIME_INSANE) &&
...
int check_backup_super_block(e2fsck_t ctx)
{
    ...
    if (((fs->flags & EXT2_FLAG_MASTER_SB_ONLY) == 0) ||
        !ext2fs_test_valid(fs) || (fs->super->s_state & EXT2_ERROR_FS) ||
    ...
    for (g = 1; g < fs->group_desc_count; g++)
    {
        ...
        retval = io_channel_read_blk(fs->io, sb, -SUPERBLOCK_SIZE, buf);
        if (retval)
            continue;
...
#define SUPER_DIFFERENT(x) (fs->super->x != backup_sb->x)
