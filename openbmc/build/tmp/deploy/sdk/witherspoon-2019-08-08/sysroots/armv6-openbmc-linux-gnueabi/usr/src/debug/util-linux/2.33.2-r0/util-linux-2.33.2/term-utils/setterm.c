...
static const char* colornames[] = {[BLACK] = "black", [RED] = "red",
                                   [GREEN] = "green", [YELLOW] = "yellow",
                                   [BLUE] = "blue",   [MAGENTA] = "magenta",
                                   [CYAN] = "cyan",   [WHITE] = "white",
                                   [GREY] = "grey",   [DEFAULT] = "default"};
...
struct setterm_control
{
    ...
    /* colors */
    unsigned int opt_fo_color : 4, opt_ba_color : 4, opt_ul_color : 4,
        opt_hb_color : 4;
    /* boolean options */
    ...
    uint64_t opt_term : 1, opt_reset : 1, opt_resize : 1, opt_initialize : 1,
        opt_cursor : 1, opt_linewrap : 1, opt_default : 1, opt_foreground : 1,
...
static int parse_ulhb_color(char** av, int* oi)
{
    ...
    if (bright && (color == BLACK || color == GREY))
        errx(EXIT_FAILURE, _("argument error: bright %s is not supported"),
             color_name);
...
static void __attribute__((__noreturn__)) usage(void)
{
    ...
    fputs(_(" --term          <terminal_name>   override TERM environment "
            "variable\n"),
          out);
    fputs(_(" --reset                           reset terminal to power-on "
            "state\n"),
          out);
    fputs(_(" --resize                          reset terminal rows and "
            "columns\n"),
          out);
    fputs(_(" --initialize                      display init string, and use "
            "default settings\n"),
          out);
    fputs(
        _(" --default                         use default terminal settings\n"),
        out);
    fputs(_(" --store                           save current terminal settings "
            "as default\n"),
          out);
    ...
    fputs(_(" --appcursorkeys [on|off]          cursor key application mode\n"),
          out);
    fputs(_(" --linewrap      [on|off]          continue on a new line when a "
            "line is full\n"),
          out);
    fputs(_(" --inversescreen [on|off]          swap colors for the whole "
            "screen\n"),
          out);
    ...
    fputs(_("                 <color>: black blue cyan green grey magenta red "
            "white yellow\n"),
          out);
    ...
    fputs(_(" --reverse       [on|off]          swap foreground and background "
            "colors\n"),
          out);
    fputs(_(" --clear         [all|rest]        clear screen and set cursor "
            "position\n"),
          out);
    fputs(_(" --tabs          [<number>...]     set these tab stop positions, "
            "or show them\n"),
          out);
    fputs(_(" --clrtabs       [<number>...]     clear these tab stop "
            "positions, or all\n"),
          out);
    fputs(_(" --regtabs       [1-160]           set a regular tab stop "
            "interval\n"),
          out);
    fputs(_(" --blank         [0-60|force|poke] set time of inactivity before "
            "screen blanks\n"),
          out);
    fputs(_(" --dump          [<number>]        write vcsa<number> console "
            "dump to file\n"),
          out);
    fputs(_(" --append        [<number>]        append vcsa<number> console "
            "dump to file\n"),
          out);
    fputs(_(" --file          <filename>        name of the dump file\n"), out);
    fputs(_(" --msg           [on|off]          send kernel messages to "
            "console\n"),
          out);
    ...
    fputs(
        _("                                   set vesa powersaving features\n"),
        out);
    fputs(_(" --powerdown     [0-60]            set vesa powerdown interval in "
            "minutes\n"),
          out);
    fputs(_(" --blength       [0-2000]          duration of the bell in "
            "milliseconds\n"),
          out);
...
static void parse_option(struct setterm_control* ctl, int ac, char** av)
{
    ...
    static const struct option longopts[] = {
        ...
        {NULL, 0, NULL, 0}};
    static const ul_excl_t excl[] = {{OPT_DEFAULT, OPT_STORE},
    ...
    while ((c = getopt_long_only(ac, av, "", longopts, NULL)) != -1)
    {
        ...
        switch (c)
        {
            ...
            case OPT_CURSOR:
                ...
                ctl->opt_cu_on = parse_switch(optarg, _("argument error"), "on",
                                              "off", NULL);
                break;
            case OPT_REPEAT:
            ...
            case OPT_LINEWRAP:
                ...
                ctl->opt_li_on = parse_switch(optarg, _("argument error"), "on",
                                              "off", NULL);
                break;
            case OPT_DEFAULT:
            ...
            case OPT_BOLD:
                ...
                ctl->opt_bo_on = parse_switch(optarg, _("argument error"), "on",
                                              "off", NULL);
                break;
            case OPT_HALF_BRIGHT:
                ctl->opt_halfbright = set_opt_flag(ctl->opt_halfbright);
                ctl->opt_hb_on = parse_switch(optarg, _("argument error"), "on",
                                              "off", NULL);
                break;
            case OPT_BLINK:
                ctl->opt_blink = set_opt_flag(ctl->opt_blink);
                ctl->opt_bl_on = parse_switch(optarg, _("argument error"), "on",
                                              "off", NULL);
                break;
            case OPT_REVERSE:
                ctl->opt_reverse = set_opt_flag(ctl->opt_reverse);
                ctl->opt_re_on = parse_switch(optarg, _("argument error"), "on",
                                              "off", NULL);
                break;
            case OPT_UNDERLINE:
                ctl->opt_underline = set_opt_flag(ctl->opt_underline);
                ctl->opt_un_on = parse_switch(optarg, _("argument error"), "on",
                                              "off", NULL);
                break;
            case OPT_STORE:
...
static void screendump(struct setterm_control* ctl)
{
    ...
    if (!out)
        err(EXIT_DUMPFILE, _("cannot open dump file %s for output"),
            ctl->opt_sn_name);
...
{
    if (!ctl->vcterm && err)
        warnx(_("terminal %s does not support %s"), ctl->opt_te_terminal_name,
              err);
    return ctl->vcterm;
}
...
static int resizetty(void)
{
    ...
    for (pos = 0; pos < sizeof(retstr) - 1;)
    {
        if (0 == select_wait())
            break;
        if ((rc = read(STDIN_FILENO, retstr + pos, sizeof(retstr) - 1 - pos)) <
            0)
        {
            if (errno == EINTR)
                continue;
...
static void perform_sequence(struct setterm_control* ctl)
{
    ...
    {
        result =
            klogctl(SYSLOG_ACTION_CONSOLE_LEVEL, NULL, ctl->opt_msglevel_num);
        if (result != 0)
            warn(_("klogctl error"));
    }
...
static void init_terminal(struct setterm_control* ctl)
{
    ...
    if (setupterm(ctl->opt_te_terminal_name, STDOUT_FILENO, &term_errno))
        switch (term_errno)
        {
            ...
            case 0:
                errx(EXIT_FAILURE, _("%s: unknown terminal type"),
                     ctl->opt_te_terminal_name);
            case 1:
