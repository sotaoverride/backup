...
#error                                                                         \
    "Boost.Log: Cannot use std::regex because it is not supported by the standard library."
#endif

#if !defined(BOOST_LOG_USE_BOOST_REGEX) &&                                     \
    !defined(BOOST_LOG_USE_STD_REGEX) &&                                       \
    !defined(BOOST_LOG_USE_BOOST_XPRESSIVE)
// Use Boost.Regex backend by default. It produces smaller executables and also
// has the best performance for small string matching. Note: This default has to
// be in sync with Boost.Log Jamfile.v2.
...
BOOST_LOG_ANONYMOUS_NAMESPACE
{
    ...
    struct matches_predicate : public matches_fun
    {
        ...
        result_type operator()(T const& val) const
        {
            ...
            return matches_fun::operator()(
                val, fusion::at_key<regex_type>(m_operands));
        }
        ...
        struct initializer
        {
            ...
            result_type operator()(T& val) const
            {
                try
                {
                    ...
                    std::basic_string<target_char_type> str;
                    log::aux::code_convert(m_initializer, str);
                    val = T::compile(str.c_str(), str.size(),
                                     T::ECMAScript | T::optimize);
                }
                catch (...)
                {
                }
    ...
    struct matches_predicate : public matches_fun
    {
        ...
        explicit matches_predicate(string_type const& operand) :
            m_operand(regex_type::compile(operand.c_str(), operand.size(),
                                          regex_type::ECMAScript |
                                              regex_type::optimize))
        {
        }
...
template <typename CharT>
filter parse_matches_relation(attribute_name const& name,
                              std::basic_string<CharT> const& operand)
{
#if defined(BOOST_LOG_USE_CHAR) && defined(BOOST_LOG_USE_WCHAR_T)
    return predicate_wrapper<log::string_types::type, matches_predicate>(
        name, matches_predicate(operand));
#else
    return predicate_wrapper<std::basic_string<CharT>,
                             matches_predicate<CharT>>(
        name, matches_predicate<CharT>(operand));
#endif
}
...
template filter
    parse_matches_relation<char>(attribute_name const& name,
                                 std::basic_string<char> const& operand);
#endif
#ifdef BOOST_LOG_USE_WCHAR_T
template filter
    parse_matches_relation<wchar_t>(attribute_name const& name,
                                    std::basic_string<wchar_t> const& operand);
#endif
