...
#define BOOST_SPIRIT_ASSERT_FSQ_SIZE                                           \
    BOOST_SPIRIT_ASSERT(((m_tail + N + 1) - m_head) % (N + 1) ==               \
                        m_size % (N + 1))                                      \
    /**/
...
#error                                                                         \
    "Please use at least Boost V1.31.0 while compiling the fixed_size_queue class!"
#else // BOOST_ITERATOR_ADAPTORS_VERSION < 0x0200

template <typename QueueT, typename T, typename PointerT>
class fsq_iterator
    : public boost::iterator_adaptor<fsq_iterator<QueueT, T, PointerT>,
                                     PointerT, T,
                                     std::random_access_iterator_tag>
{
  public:
    typedef typename QueueT::position_t position;
    typedef boost::iterator_adaptor<fsq_iterator<QueueT, T, PointerT>, PointerT,
  ...
  private:
    ...
    bool equal(iterator_adaptor<OtherDerivedT, OtherIteratorT, V, C, R,
                                D> const& x) const
    {
    ...
    typename base_t::difference_type distance_to(
        iterator_adaptor<OtherDerivedT, OtherIteratorT, V, C, R, D> const& x)
        const
    {
...
template <typename T, std::size_t N>
inline fixed_size_queue<T, N>& fixed_size_queue<T, N>::
    operator=(const fixed_size_queue& x)
{
    if (this != &x)
    {
        m_head = x.m_head;
