...
struct counted_time_rep
{
    ...
    counted_time_rep(const date_type& d,
        ...
        time_count_(1)
    {
        ...
        {
            time_count_ =
                (d.day_number() * frac_sec_per_day()) + time_of_day.ticks();
        }
    }
    explicit counted_time_rep(int_type count) : time_count_(count)
    {
    }
    explicit counted_time_rep(impl_type count) : time_count_(count)
    {
    }
    date_type date() const
    {
        ...
        {
            typename calendar_type::date_int_type dc =
                static_cast<typename calendar_type::date_int_type>(day_count());
    ...
    unsigned long day_count() const
    {
        ...
        return static_cast<unsigned long>(
            resolution_traits::as_number(time_count_) / frac_sec_per_day());
    }
  ...
  public:
    ...
    static time_rep_type get_time_rep(special_values sv)
    {
        switch (sv)
        {
            ...
            case max_date_time:
            {
                time_duration_type td = time_duration_type(24, 0, 0, 0) -
                                        time_duration_type(0, 0, 0, 1);
                return time_rep_type(date_type(max_date_time), td);
            }
            case min_date_time:
                return time_rep_type(date_type(min_date_time),
                                  ...
                                  const date_duration_type& dd)
    {
        ...
        {
            return time_rep_type(
                base.time_count() +
                (dd.days() * time_rep_type::frac_sec_per_day()));
        }
    }
                                             ...
                                             const time_rep_type& rhs)
    {
        ...
        {
            return (time_duration_type(impl_type::to_special(
                (lhs.get_rep() - rhs.get_rep()).as_number())));
        }
        else
        {
