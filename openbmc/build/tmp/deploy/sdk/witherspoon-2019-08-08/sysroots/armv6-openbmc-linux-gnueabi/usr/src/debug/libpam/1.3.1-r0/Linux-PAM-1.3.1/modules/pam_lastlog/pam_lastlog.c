...
static int _pam_auth_parse(pam_handle_t* pamh, int flags, int argc,
                           const char** argv, time_t* inactive)
{
    int ctrl = 0;
...
static int _pam_session_parse(pam_handle_t* pamh, int flags, int argc,
                              const char** argv)
{
...
static int last_login_open(pam_handle_t* pamh, int announce, uid_t uid)
{
    int last_fd;

    /* obtain the last login date and all the relevant info */
    last_fd =
        open(_PATH_LASTLOG, announce & LASTLOG_UPDATE ? O_RDWR : O_RDONLY);
    if (last_fd < 0)
    {
        if (errno == ENOENT && (announce & LASTLOG_UPDATE))
        {
            ...
            if (last_fd < 0)
            {
                pam_syslog(pamh, LOG_ERR, "unable to create %s: %m",
                           _PATH_LASTLOG);
                D(("unable to create %s file", _PATH_LASTLOG));
                return -1;
            }
            pam_syslog(pamh, LOG_NOTICE, "file %s created", _PATH_LASTLOG);
...
static int last_login_read(pam_handle_t* pamh, int announce, int last_fd,
                           uid_t uid, time_t* lltime)
{
    ...
    if (fcntl(last_fd, F_SETLK, &last_lock) < 0)
    {
        ...
        pam_syslog(pamh, LOG_WARNING, "file %s is locked/read", _PATH_LASTLOG);
        sleep(LASTLOG_IGNORE_LOCK_TIME);
    }

    if (pam_modutil_read(last_fd, (char*)&last_login, sizeof(last_login)) !=
        sizeof(last_login))
    {
    ...
    *lltime = last_login.ll_time;
    if (!last_login.ll_time)
    {
        ...
        {
            pam_syslog(pamh, LOG_DEBUG, "first login for user with uid %lu",
    ...
    if (!(announce & LASTLOG_QUIET))
    {
        ...
        if (last_login.ll_time)
        {
            ...
            if ((announce & LASTLOG_HOST) && (last_login.ll_host[0] != '\0'))
            {
            ...
            if ((announce & LASTLOG_LINE) && (last_login.ll_line[0] != '\0'))
            {
            ...
            if (date != NULL || host != NULL || line != NULL)
                 ...
                 */
                retval =
                    pam_info(pamh, _("Last login:%s%s%s"), date ? date : "",
                             host ? host : "", line ? line : "");
        }
...
static int last_login_write(pam_handle_t* pamh, int announce, int last_fd,
                            uid_t uid, const char* user)
{
    ...
    if (pam_modutil_write(last_fd, (char*)&last_login, sizeof(last_login)) !=
        sizeof(last_login))
    {
...
static int last_login_failed(pam_handle_t* pamh, int announce, const char* user,
                             time_t lltime)
{
    int retval;
    int fd;
    ...
    while ((retval = pam_modutil_read(fd, (void*)&ut, sizeof(ut))) ==
           sizeof(ut))
    {
        if (ut.ut_tv.tv_sec >= lltime &&
    ...
    if (failed)
    {
        ...
        /* we want & have the host? */
        if ((announce & LASTLOG_HOST) && (utuser.ut_host[0] != '\0'))
        {
            /* TRANSLATORS: " from <host>" */
            if (asprintf(&host, _(" from %.*s"), UT_HOSTSIZE, utuser.ut_host) <
                0)
            {
                pam_syslog(pamh, LOG_CRIT, "out of memory");
                retval = PAM_BUF_ERR;
        ...
        /* we want and have the terminal? */
        if ((announce & LASTLOG_LINE) && (utuser.ut_line[0] != '\0'))
        {
            /* TRANSLATORS: " on <terminal>" */
            if (asprintf(&line, _(" on %.*s"), UT_LINESIZE, utuser.ut_line) < 0)
            {
                pam_syslog(pamh, LOG_CRIT, "out of memory");
                retval = PAM_BUF_ERR;
                goto cleanup;
        ...
        {
            /* TRANSLATORS: "Last failed login: <date> from <host> on
             * <terminal>" */
            pam_info(pamh, _("Last failed login:%s%s%s"), date ? date : "",
                     host ? host : "", line ? line : "");
        }
        ...
        retval = asprintf(&line,
                          dngettext(PACKAGE,
                                    "There was %d failed login attempt since "
                                    "the last successful login.",
                                    "There were %d failed login attempts since "
                                    "the last successful login.",
                                    failed),
                          failed);
#else
        if (failed == 1)
            retval = asprintf(&line,
                              _("There was %d failed login attempt since the "
            ...
            retval = asprintf(
                ...
                _("There were %d failed login attempts since the last "
                  "successful login."),
                failed);
#endif
        if (retval >= 0)
