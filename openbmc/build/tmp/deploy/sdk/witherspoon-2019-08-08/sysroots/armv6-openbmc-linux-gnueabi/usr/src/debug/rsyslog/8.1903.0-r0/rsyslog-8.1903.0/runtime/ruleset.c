 ...
 * (conf->rulesets.llRulesets) hold alls rule sets that we know. Included in
 * each list is a list of rules (which contain a list of actions, but that's a
 * different story).
 *
...
/* static data */
DEFobjStaticHelpers DEFobjCurrIf(parser)
...
static struct cnfparamblk rspblk = {
    CNFPARAMBLK_VERSION, sizeof(rspdescr) / sizeof(struct cnfparamdescr),
    rspdescr};
...
static void scriptIterateAllActions(struct cnfstmt* root,
                                    ...
                                    void* pParam)
{
    ...
    for (stmt = root; stmt != NULL; stmt = stmt->next)
    {
        switch (stmt->nodetype)
        {
            ...
            case S_CALL: /* call does not need to do anything - done in called
                            ruleset! */
                break;
            case S_ACT:
            ...
            case S_IF:
                if (stmt->d.s_if.t_then != NULL)
                    scriptIterateAllActions(stmt->d.s_if.t_then, pFunc, pParam);
                if (stmt->d.s_if.t_else != NULL)
                    scriptIterateAllActions(stmt->d.s_if.t_else, pFunc, pParam);
                break;
            case S_FOREACH:
                if (stmt->d.s_foreach.body != NULL)
                    scriptIterateAllActions(stmt->d.s_foreach.body, pFunc,
                                            pParam);
                break;
            case S_PRIFILT:
                if (stmt->d.s_prifilt.t_then != NULL)
                    scriptIterateAllActions(stmt->d.s_prifilt.t_then, pFunc,
                                            pParam);
                if (stmt->d.s_prifilt.t_else != NULL)
                    scriptIterateAllActions(stmt->d.s_prifilt.t_else, pFunc,
                                            pParam);
                break;
            case S_PROPFILT:
                scriptIterateAllActions(stmt->d.s_propfilt.t_then, pFunc,
                                        pParam);
                break;
 ...
 */
static rsRetVal iterateAllActions(rsconf_t* conf,
                                  rsRetVal (*pFunc)(void*, void*), void* pParam)
{
    ...
    CHKiRet(
        llExecFunc(&(conf->rulesets.llRulesets), doIterateAllActions, &params));
...
static rsRetVal ATTR_NONNULL()
    execSet(const struct cnfstmt* const stmt, smsg_t* const pMsg,
...
static rsRetVal execCallIndirect(struct cnfstmt* const __restrict__ stmt,
                                 smsg_t* pMsg, wti_t* const __restrict__ pWti)
{
    ruleset_t* pRuleset;
    struct svar result;
    ...
    DBGPRINTF("CALL_INDIRECT obtained ruleset ptr %p for ruleset '%s' "
              "[hasQueue:%d]\n",
...
static rsRetVal execIf(struct cnfstmt* const stmt, smsg_t* const pMsg,
                       wti_t* const pWti)
{
    sbool bRet;
    DEFiRet;
...
static rsRetVal invokeForeachBodyWith(struct cnfstmt* stmt, json_object* o,
                                      smsg_t* pMsg, wti_t* pWti)
{
    struct svar v;
    v.datatype = 'J';
    v.d.json = o;
    DEFiRet;
    CHKiRet(
        msgSetJSONFromVar(pMsg, (uchar*)stmt->d.s_foreach.iter->var, &v, 1));
    CHKiRet(scriptExec(stmt->d.s_foreach.body, pMsg, pWti));
finalize_it:
    RETiRet;
}

static rsRetVal callForeachArray(struct cnfstmt* stmt, json_object* arr,
                                 smsg_t* pMsg, wti_t* pWti)
{
    DEFiRet;
...
static rsRetVal callForeachObject(struct cnfstmt* stmt, json_object* arr,
                                  smsg_t* pMsg, wti_t* pWti)
{
...
static rsRetVal ATTR_NONNULL()
    execForeach(struct cnfstmt* const stmt, smsg_t* const pMsg,
                wti_t* const pWti)
{
    ...
    {
        DBGPRINTF("foreach loop skipped, as object to iterate upon is not an "
                  "array\n");
        FINALIZE;
    }
...
static rsRetVal execPRIFILT(struct cnfstmt* stmt, smsg_t* pMsg, wti_t* pWti)
{
    ...
    if ((stmt->d.s_prifilt.pmask[pMsg->iFacility] == TABLE_NOPRI) ||
        ((stmt->d.s_prifilt.pmask[pMsg->iFacility] & (1 << pMsg->iSeverity)) ==
         0))
        bRet = 0;
    else
        bRet = 1;
...
static int evalPROPFILT(struct cnfstmt* stmt, smsg_t* pMsg)
{
    ...
    pszPropVal = MsgGetProp(pMsg, NULL, &stmt->d.s_propfilt.prop, &propLen,
                            &pbMustBeFreed, NULL);
    ...
    {
        case FIOP_CONTAINS:
            if (rsCStrLocateInSzStr(stmt->d.s_propfilt.pCSCompValue,
                                    (uchar*)pszPropVal) != -1)
                bRet = 1;
            break;
        case FIOP_ISEMPTY:
            ...
            break;
        case FIOP_ISEQUAL:
            if (rsCStrSzStrCmp(stmt->d.s_propfilt.pCSCompValue, pszPropVal,
                               propLen) == 0)
                bRet = 1; /* process message! */
            break;
        case FIOP_STARTSWITH:
            ...
            break;
        case FIOP_REGEX:
            if (rsCStrSzStrMatchRegex(
                    stmt->d.s_propfilt.pCSCompValue, (unsigned char*)pszPropVal,
                    0, &stmt->d.s_propfilt.regex_cache) == RS_RET_OK)
                bRet = 1;
            break;
        case FIOP_EREREGEX:
            if (rsCStrSzStrMatchRegex(
                    stmt->d.s_propfilt.pCSCompValue, (unsigned char*)pszPropVal,
                    1, &stmt->d.s_propfilt.regex_cache) == RS_RET_OK)
                bRet = 1;
            break;
        case FIOP_NOP:
        default:
...
static rsRetVal ATTR_NONNULL(2, 3)
    scriptExec(struct cnfstmt* const root, smsg_t* const pMsg,
               wti_t* const pWti)
{
    struct cnfstmt* stmt;
    DEFiRet;
...
static rsRetVal processBatch(batch_t* pBatch, wti_t* pWti)
{
    ...
    for (i = 0; i < batchNumMsgs(pBatch) && !*(pWti->pbShutdownImmediate); ++i)
    {
        ...
        pRuleset =
            (pMsg->pRuleset == NULL) ? ourConf->rulesets.pDflt : pMsg->pRuleset;
...
{
    return (pMsg->pRuleset == NULL) ? conf->rulesets.pDflt->pParserLst
                                    : pMsg->pRuleset->pParserLst;
}
 ...
 */
rsRetVal rulesetGetRuleset(rsconf_t* conf, ruleset_t** ppRuleset,
                           uchar* pszName)
{
    DEFiRet;
...
/* Set a new default rule set. If the default can not be found, no change
 * happens.
 */
...
/* Set a new current rule set. If the ruleset can not be found, no change
 * happens */
 ...
 */
BEGINobjConstruct(
    ruleset) /* be sure to specify the object type also in END macro! */
    pThis->root = NULL;
pThis->last = NULL;
...
BEGINobjDestruct(ruleset) /* be sure to specify the object type also in END and
                             CODESTART macros! */
...
BEGINobjDebugPrint(ruleset) /* be sure to specify the object type also in END
...
static rsRetVal doRulesetCreateQueue(rsconf_t* conf, int* pNewVal)
{
    ...
    rsname = (conf->rulesets.pCurr->pszName == NULL)
                 ? (uchar*)"[ruleset]"
...
static rsRetVal rulesetCreateQueue(void __attribute__((unused)) * pVal,
                                   int* pNewVal)
{
 ...
 * parser automatically disables the default parsers. If they are needed as
 * well, the must be added via explicit config directives. Note: this is the
 * only spot in the code that requires the parser object. In order to solve some
 * class init bootstrap sequence problems, we get the object handle here instead
 * of during module initialization. Note that objUse() is capable of being
...
static rsRetVal rulesetAddParser(void __attribute__((unused)) * pVal,
                                 uchar* pName)
{
...
rsRetVal rulesetProcessCnf(struct cnfobj* o)
{
    ...
    if (localRet == RS_RET_OK)
    {
        LogError(0, RS_RET_RULESET_EXISTS,
                 "error: ruleset '%s' specified more than once", rsName);
    ...
    {
        rsname = (pRuleset->pszName == NULL) ? (uchar*)"[ruleset]"
                                             : pRuleset->pszName;
        DBGPRINTF("adding a ruleset-specific \"main\" queue for ruleset '%s'\n",
                  rsname);
...
BEGINobjQueryInterface(ruleset)
    CODESTARTobjQueryInterface(ruleset) if (pIf->ifVersion !=
                                            rulesetCURR_IF_VERSION)
...
CHKiRet(regCfSysLineHdlr((uchar*)"rulesetparser", 0, eCmdHdlrGetWord,
                         rulesetAddParser, NULL, NULL));
CHKiRet(regCfSysLineHdlr((uchar*)"rulesetcreatemainqueue", 0, eCmdHdlrBinary,
                         rulesetCreateQueue, NULL, NULL));
