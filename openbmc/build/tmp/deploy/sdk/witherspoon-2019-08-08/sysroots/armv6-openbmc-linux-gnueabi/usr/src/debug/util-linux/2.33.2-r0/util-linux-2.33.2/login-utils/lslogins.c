...
static const struct lslogins_coldesc coldescs[] = {
    [COL_USER] = {"USER", N_("user name"), N_("Username"), 0.1,
                  SCOLS_FL_NOEXTREMES},
    ...
    [COL_GECOS] = {"GECOS", N_("full user name"), N_("Gecos field"), 0.1,
                   SCOLS_FL_TRUNC},
    [COL_LAST_LOGIN] = {"LAST-LOGIN", N_("date of last login"),
                        N_("Last login"), 0.1, SCOLS_FL_RIGHT},
    [COL_LAST_TTY] = {"LAST-TTY", N_("last tty used"), N_("Last terminal"),
                      0.05},
    [COL_LAST_HOSTNAME] = {"LAST-HOSTNAME",
    ...
    [COL_HUSH_STATUS] = {"HUSHED", N_("user's hush settings"), N_("Hushed"), 1,
                         SCOLS_FL_RIGHT},
    [COL_PWD_WARN] = {"PWD-WARN",
                      N_("days user is warned of password expiration"),
                      N_("Password expiration warn interval"), 0.1,
                      SCOLS_FL_RIGHT},
...
int main(int argc, char* argv[])
{
    ...
    while ((c = getopt_long(argc, argv, "acefGg:hLl:no:prsuVzZ", longopts,
                            NULL)) != -1)
    {
