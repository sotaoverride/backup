...
static void Add_HR_Disk_entry(const char*, int, int, int, int, const char*, int,
                              int);
#endif
...
struct variable4 hrdisk_variables[] = {
    {HRDISK_ACCESS, ASN_INTEGER, NETSNMP_OLDAPI_RONLY, var_hrdisk, 2, {1, 1}},
    {HRDISK_MEDIA, ASN_INTEGER, NETSNMP_OLDAPI_RONLY, var_hrdisk, 2, {1, 2}},
    {HRDISK_REMOVEABLE,
     ASN_INTEGER,
...
void init_hr_disk(void)
{
    ...
    Add_HR_Disk_entry("/dev/rdsk/c%dt%xd%d", 0, 1, 0, 15, "/dev/rdsk/c%dt%xd0",
                      0, 4);
#else /* hpux9 */
    ...
    Add_HR_Disk_entry("/dev/rdsk/c%dd%ds%d", 0, 7, 0, 15, "/dev/rdsk/c%dd%ds0",
                      0, 7);
#elif defined(darwin)
...
static void parse_disk_config(const char* token, char* cptr)
{
...
static int match_disk_config_item(const char* name, conf_disk_item* di_ptr)
{
    int result = 0;
    size_t len;
    details_set* d_set;
        ...
        switch (di_ptr->item_type)
        {
            case ITEM_STRING:
                ...
                if (!strncmp(name, (const char*)di_ptr->item_details, len))
                    result =
                        match_disk_config_item(name + len, di_ptr->item_next);
                break;
            case ITEM_SET:
                if (*name)
                {
                    ...
                    if ((*d_set)[c / 8] & (unsigned char)(1 << (c % 8)))
                        result =
                            match_disk_config_item(name + 1, di_ptr->item_next);
                }
                break;
            case ITEM_STAR:
                if (di_ptr->item_next)
                {
                    for (; !result && *name; name++)
                        result =
                            match_disk_config_item(name, di_ptr->item_next);
                }
                else
                {
                    result = 1;
...
static int match_disk_config(const char* name)
{
...
int header_hrdisk(struct variable* vp, oid* name, size_t* length, int exact,
...
u_char* var_hrdisk(struct variable* vp, oid* name, size_t* length, int exact,
                   size_t* var_len, WriteMethod** write_method)
{
    int disk_idx;
    ...
    switch (vp->magic)
    {
        ...
        default:
            DEBUGMSGTL(
                ("snmpd", "unknown sub-id %d in var_hrdisk\n", vp->magic));
    }
    return NULL;
}
...
#if !(defined(aix4) || defined(aix5) || defined(aix6) || defined(aix7))
static void Add_HR_Disk_entry(const char* devpart_string, int first_ctl,
                              int last_ctl, int first_dev, int last_dev,
                              const char* devfull_string, int first_partn,
                              int last_partn)
{
...
#if DEBUG_TEST
            DEBUGMSGTL(("host/hr_disk", "Add_HR %02d '%s' first=%d last=%d\n",
...
int Get_Next_HR_Disk(void)
{
    ...
    while (HRD_type_index < HR_number_disk_types)
    {
        ...
        while (HRD_index < max_disks)
        {
            ...
            {
                DEBUGMSGTL(("host/hr_disk", "Get_Next_HR_Disk: can't open %s\n",
                            string));
            }
...
int Get_HR_Disk_Label(char* string, size_t str_len, const char* devfull)
{
    ...
    /** model */
    str_ref =
        (CFStringRef)CFDictionaryGetValue(desc, kDADiskDescriptionMediaNameKey);
    if (str_ref)
    {
        strlcpy(string, CFStringGetCStringPtr(str_ref, sys_encoding), str_len);
...
static void Save_HR_Disk_Specific(void)
{
    ...
    HRD_savedCapacity =
        HRD_cap.dkg_ncyl * HRD_cap.dkg_nhead * HRD_cap.dkg_nsect / 2; /* ??? */
#endif
#ifdef HAVE_LINUX_HDREG_H
...
static const char* describe_disk(int idx)
{
...
static int Query_Disk(int fd, const char* devfull)
{
    int result = -1;
    ...
    else if (HRD_type_index != 3)
    { /* SCSI hard disk, md and LVM devices */
        long h;
    ...
    number_ref =
        (CFNumberRef)CFDictionaryGetValue(desc, kDADiskDescriptionMediaSizeKey);
    if (number_ref)
    ...
    /** writable?  */
    bool_ref = (CFBooleanRef)CFDictionaryGetValue(
        desc, kDADiskDescriptionMediaWritableKey);
    if (bool_ref)
    {
    ...
    /** get type */
    str_ref =
        (CFStringRef)CFDictionaryGetValue(desc, kDADiskDescriptionMediaTypeKey);
    if (str_ref)
    {
        HRD_type =
            _get_type_value(CFStringGetCStringPtr(str_ref, sys_encoding));
        DEBUGMSGTL(("verbose:diskmgr:darwin", " type %s / %d\n",
                    CFStringGetCStringPtr(str_ref, sys_encoding), HRD_type));
    }
    else
    {
        str_ref = (CFStringRef)CFDictionaryGetValue(
            desc, kDADiskDescriptionDeviceProtocolKey);
        if (str_ref)
        {
            HRD_type = _get_type_from_protocol(
...
static void Add_LVM_Disks(void)
{
    ...
    if (res != 0)
    {
        ...
        DEBUGMSGTL(("host/hr_disk", "Add_LVM_Disks: cannot compile regexp: %s",
                    error));
        return;
    }
    ...
    if (dir == NULL)
    {
        DEBUGMSGTL(("host/hr_disk", "Add_LVM_Disks: cannot open /dev/mapper"));
        regfree(&lvol);
        return;
    }
