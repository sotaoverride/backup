...
static int exp_parse_proto(struct nfnl_exp* exp, int tuple, struct nlattr* attr)
{
    ...
    if (tb[CTA_PROTO_ICMP_ID] || tb[CTA_PROTO_ICMP_TYPE] ||
...
static int exp_parse_nat(struct nfnl_exp* exp, struct nlattr* attr)
{
    ...
    {
        err =
            exp_parse_tuple(exp, NFNL_EXP_TUPLE_NAT, tb[CTA_EXPECT_NAT_TUPLE]);
        if (err < 0)
            return err;
    }
...
int nfnlmsg_exp_parse(struct nlmsghdr* nlh, struct nfnl_exp** result)
{
    ...
    {
        err =
            exp_parse_tuple(exp, NFNL_EXP_TUPLE_MASTER, tb[CTA_EXPECT_MASTER]);
        if (err < 0)
            goto errout;
    }
                                ...
                                int cta)
{
    ...
    if (addr)
        NLA_PUT_ADDR(msg, family == AF_INET ? CTA_IP_V4_SRC : CTA_IP_V6_SRC,
                     addr);

    addr = nfnl_exp_get_dst(exp, type);
    if (addr)
        NLA_PUT_ADDR(msg, family == AF_INET ? CTA_IP_V4_DST : CTA_IP_V6_DST,
                     addr);
    ...
    if (nfnl_exp_test_icmp(exp, type))
    {
        ...
        NLA_PUT_U8(msg, CTA_PROTO_ICMP_TYPE, nfnl_exp_get_icmp_type(exp, type));

        NLA_PUT_U8(msg, CTA_PROTO_ICMP_CODE, nfnl_exp_get_icmp_code(exp, type));
    }
...
static int nfnl_exp_build_nat(struct nl_msg* msg, const struct nfnl_exp* exp)
{
    ...
    {
        NLA_PUT_U32(msg, CTA_EXPECT_NAT_DIR, nfnl_exp_get_nat_dir(exp));
    }
...
static int nfnl_exp_build_message(const struct nfnl_exp* exp, int cmd,
                                  int flags, struct nl_msg** result)
{
    ...
    if (nfnl_exp_test_helper_name(exp))
        NLA_PUT_STRING(msg, CTA_EXPECT_HELP_NAME,
                       nfnl_exp_get_helper_name(exp));
