...
static void g_type_module_complete_type_info(GTypePlugin* plugin, GType g_type,
                                             GTypeInfo* info,
...
static void g_type_module_dispose(GObject* object)
{
    GTypeModule* module = G_TYPE_MODULE(object);

    if (module->type_infos || module->interface_infos)
    {
        g_warning(G_STRLOC ": unsolicitated invocation of "
                           "g_object_run_dispose() on GTypeModule");
...
static void g_type_module_finalize(GObject* object)
{
    GTypeModule* module = G_TYPE_MODULE(object);
...
static void g_type_module_class_init(GTypeModuleClass* class)
{
...
static void g_type_module_iface_init(GTypePluginClass* iface)
{
...
GType g_type_module_get_type(void)
{
    ...
    if (!type_module_type)
    {
        ...
        type_module_type = g_type_register_static(
            G_TYPE_OBJECT, g_intern_static_string("GTypeModule"),
            &type_module_info, G_TYPE_FLAG_ABSTRACT);

        g_type_add_interface_static(type_module_type, G_TYPE_TYPE_PLUGIN,
                                    &iface_info);
    }
...
static ModuleTypeInfo* g_type_module_find_type_info(GTypeModule* module,
                                                    GType type)
{
...
static ModuleInterfaceInfo*
    g_type_module_find_interface_info(GTypeModule* module, GType instance_type,
                                      GType interface_type)
{
                                             ...
                                             GTypeValueTable* value_table)
{
    ...
    ModuleTypeInfo* module_type_info =
        g_type_module_find_type_info(module, g_type);
...
{
    GTypeModule* module = G_TYPE_MODULE(plugin);
    ModuleInterfaceInfo* module_interface_info =
        g_type_module_find_interface_info(module, instance_type,
                                          interface_type);
                                  ...
                                  const GTypeInfo* type_info, GTypeFlags flags)
{
    ...
    if (module == NULL)
    {
        /* Cannot pass type_info directly to g_type_register_static() here
         * because
         * it has class_finalize != NULL and that's forbidden for static types
         */
        return g_type_register_static_simple(
            parent_type, type_name, type_info->class_size,
            type_info->class_init, type_info->instance_size,
            type_info->instance_init, flags);
    }
    ...
    if (type)
    {
        ...
        if (module_type_info->parent_type != parent_type)
        {
            ...
            g_warning("Type '%s' recreated with different parent type."
                      "(was '%s', now '%s')",
                      type_name, g_type_name(module_type_info->parent_type),
    ...
    else
    {
        ...
        module_type_info->type = g_type_register_dynamic(
            parent_type, type_name, G_TYPE_PLUGIN(module), flags);

        module->type_infos =
            g_slist_prepend(module->type_infos, module_type_info);
    }

    module_type_info->loaded = TRUE;
    module_type_info->info = *type_info;
    if (type_info->value_table)
        module_type_info->info.value_table =
            g_memdup(type_info->value_table, sizeof(GTypeValueTable));
 ...
 * instead. This can be used when making a static build of the module.
 */
void g_type_module_add_interface(GTypeModule* module, GType instance_type,
                                 GType interface_type,
                                 const GInterfaceInfo* interface_info)
{
    ...
    if (g_type_is_a(instance_type, interface_type))
    {
        ...
        {
            g_warning("Two different plugins tried to register interface '%s' "
                      "for '%s'.",
                      g_type_name(interface_type), g_type_name(instance_type));
            return;
        }

        module_interface_info = g_type_module_find_interface_info(
            module, instance_type, interface_type);
    ...
    else
    {
        ...
        module->interface_infos =
            g_slist_prepend(module->interface_infos, module_interface_info);
    }
 ...
 * Returns: the new or existing type ID
 */
GType g_type_module_register_enum(GTypeModule* module, const gchar* name,
                                  const GEnumValue* const_static_values)
{
    ...
    g_enum_complete_type_info(G_TYPE_ENUM, &enum_type_info,
                              const_static_values);

    return g_type_module_register_type(G_TYPE_MODULE(module), G_TYPE_ENUM, name,
                                       &enum_type_info, 0);
}
 ...
 * Returns: the new or existing type ID
 */
GType g_type_module_register_flags(GTypeModule* module, const gchar* name,
                                   const GFlagsValue* const_static_values)
{
    ...
    g_flags_complete_type_info(G_TYPE_FLAGS, &flags_type_info,
                               const_static_values);

    return g_type_module_register_type(G_TYPE_MODULE(module), G_TYPE_FLAGS,
                                       name, &flags_type_info, 0);
}
