...
class raii_casemap
{
  ...
  public:
    ...
    std::string convert(Conv func, char const* begin, char const* end) const
    {
        ...
        UErrorCode err = U_ZERO_ERROR;
        int size =
            func(map_, &buf.front(), buf.size(), begin, end - begin, &err);
...
class utf8_converter_impl : public converter<char>
{
  public:
    utf8_converter_impl(cdata const& d) :
        locale_id_(d.locale.getName()), map_(locale_id_)
    {
    }
...
std::locale create_convert(std::locale const& in, cdata const& cd,
