 ...
 *                       gconstpointer user_data)
 * {
 *   g_assert_cmpstr (my_object_get_property (fixture->obj), ==,
 * "initial-value");
 * }
 *
 * static void
...
static const char* const g_test_result_names[] = {"OK", "SKIP", "FAIL", "TODO"};
...
static void g_test_log(GTestLogType lbit, const gchar* string1,
                       const gchar* string2, guint n_args, long double* largs)
{
    GTestResult result;
    gboolean fail;
    GTestLogMsg msg;
...
static void parse_args(gint* argc_p, gchar*** argv_p)
{
    ...
    for (i = 1; i < argc; i++)
    {
        ...
        else if (strcmp("--GTestLogFD", argv[i]) == 0 ||
                 strncmp("--GTestLogFD=", argv[i], 13) == 0)
        {
        ...
        else if (strcmp("--GTestSkipCount", argv[i]) == 0 ||
                 strncmp("--GTestSkipCount=", argv[i], 17) == 0)
        {
        ...
        else if (strcmp("-s", argv[i]) == 0 || strncmp("-s=", argv[i], 3) == 0)
        {
            ...
            if (*equal == '=')
                test_paths_skipped =
                    g_slist_prepend(test_paths_skipped, equal + 1);
            else if (i + 1 < argc)
            {
                argv[i++] = NULL;
                test_paths_skipped =
                    g_slist_prepend(test_paths_skipped, argv[i]);
            }
            argv[i] = NULL;
        }
                 ...
                 strcmp("--help", argv[i]) == 0)
        {
            printf(
                ...
                "  -l                             List test cases available in "
                "a test executable\n"
                "  -m {perf|slow|thorough|quick}  Execute tests according to "
                "mode\n"
                "  -m {undefined|no-undefined}    Execute tests according to "
                "mode\n"
                "  -p TESTPATH                    Only start test cases "
                "matching TESTPATH\n"
                "  -s TESTPATH                    Skip all tests matching "
                "TESTPATH\n"
                "  --seed=SEEDSTRING              Start tests with random seed "
                "SEEDSTRING\n"
 ...
 * directory. */
static gboolean test_do_isolate_dirs(GError** error)
{
    gchar* subdir = NULL;
    ...
    /* The @test_run_name includes the test suites, so may be several
     * directories deep. Add a `.dirs` directory to contain all the paths we
     * create, and guarantee none of them clash with test paths below the
     * current one — test paths may not contain components starting with `.`. */
    ...
    if (g_mkdir_with_parents(runtime_dir, 0700) != 0)
    {
        ...
        g_set_error(error, G_FILE_ERROR, g_file_error_from_errno(saved_errno),
                    "Failed to create XDG_RUNTIME_DIR ‘%s’: %s", runtime_dir,
     ...
     * this list changes. */
    g_set_user_dirs("HOME", home_dir, "XDG_CACHE_HOME", cache_dir,
                    "XDG_CONFIG_DIRS", config_dirs, "XDG_CONFIG_HOME",
                    config_dir, "XDG_DATA_DIRS", data_dirs, "XDG_DATA_HOME",
                    data_dir, "XDG_RUNTIME_DIR", runtime_dir, NULL);
...
void(g_test_init)(int* argc, char*** argv, ...)
{
    ...
    g_snprintf(seedstr, sizeof(seedstr), "R02S%08x%08x%08x%08x", g_random_int(),
     ...
     * G_TEST_TMPDIR variable which gives the top-level temporary directory.
     * This allows test subprocesses to reuse the same temporary directory when
    ...
    if (test_isolate_dirs)
    {
        if (g_getenv("G_TEST_TMPDIR") == NULL)
        {
             ...
             * accidentally work, and should be fairly greppable. */
            {
                ...
                for (i = 0; i < G_N_ELEMENTS(overridden_environment_variables);
                     i++)
                    g_setenv(overridden_environment_variables[i], "/dev/null",
                             TRUE);
            }
        }
    ...
    if (test_tap_log)
    {
        if (test_paths || test_startup_skip_count)
        {
             ...
             */
            g_printerr("%s: -p and --GTestSkipCount options are incompatible "
                       "with --tap\n",
                       (*argv)[0]);
            exit(1);
        }
    ...
    if (1)
    {
        ...
        guint32 t1 = g_rand_int(rg), t2 = g_rand_int(rg), t3 = g_rand_int(rg),
                t4 = g_rand_int(rg);
        /* g_print ("GRand-current: 0x%x 0x%x 0x%x 0x%x\n", t1, t2, t3, t4); */
        if (t1 != 0xfab39f9b || t2 != 0xb948fb0e || t3 != 0x3d31be26 ||
            t4 != 0x43a19d66)
            g_warning("random numbers are not GRand-2.2 compatible, seeds may "
                      "be broken (check $G_RANDOM_VERSION)");
        g_rand_free(rg);
    }
    ...
    g_test_log(G_TEST_LOG_START_BINARY, g_get_prgname(), test_run_seedstr, 0,
               NULL);
...
static void test_run_seed(const gchar* rseed)
{
    ...
    while (strchr(" \t\v\r\n\f", *rseed))
        rseed++;
    if (strncmp(rseed, "R02S", 4) ==
        0) /* seed for random generator 02 (GRand-2.2) */
    {
...
{
    test_user_stamp =
        test_user_timer ? g_timer_elapsed(test_user_timer, NULL) : 0;
 ...
 * Since: 2.16
 */
GTestCase* g_test_create_case(const char* test_name, gsize data_size,
...
static gint find_suite(gconstpointer l, gconstpointer s)
{
...
static gint find_case(gconstpointer l, gconstpointer s)
{
 ...
 * Since: 2.28
 */
void g_test_add_vtable(const char* testpath, gsize data_size,
                       gconstpointer test_data, GTestFixtureFunc data_setup,
                       ...
                       GTestFixtureFunc data_teardown)
{
    ...
    for (ui = 0; segments[ui] != NULL; ui++)
    {
        ...
        else /* islast */
        {
            ...
            tc = g_test_create_case(seg, data_size, test_data, data_setup,
                                    fixture_test_func, data_teardown);
...
void g_test_add_func(const char* testpath, GTestFunc test_func)
{
    ...
    g_test_add_vtable(testpath, 0, NULL, NULL, (GTestFixtureFunc)test_func,
                      NULL);
}
 ...
 * Since: 2.16
 */
void g_test_add_data_func(const char* testpath, gconstpointer test_data,
                          GTestDataFunc test_func)
{
    ...
    g_test_add_vtable(testpath, 0, test_data, NULL, (GTestFixtureFunc)test_func,
                      NULL);
}
 ...
 * Since: 2.34
 */
void g_test_add_data_func_full(const char* testpath, gpointer test_data,
                               GTestDataFunc test_func,
...
static gboolean g_test_suite_case_exists(GTestSuite* suite,
...
static gboolean test_case_run(GTestCase* tc)
{
    ...
    else
    {
        ...
        if (test_paths_skipped &&
            g_slist_find_custom(test_paths_skipped, test_run_name,
        ...
        else
        {
            ...
            {
                g_test_log(G_TEST_LOG_ERROR, local_error->message, NULL, 0,
                           NULL);
                g_test_fail();
                g_error_free(local_error);
            }
            else
            {
                ...
                fixture = tc->fixture_size ? g_malloc0(tc->fixture_size)
                                           : tc->test_data;
                test_run_seed(test_run_seedstr);
                if (tc->fixture_setup)
        ...
        g_test_log(G_TEST_LOG_STOP_CASE, test_run_name, test_run_msg,
                   G_N_ELEMENTS(largs), largs);
        g_clear_pointer(&test_run_msg, g_free);
    ...
    return (success == G_TEST_RUN_SUCCESS || success == G_TEST_RUN_SKIPPED ||
...
static gboolean test_should_run(const char* test_path, const char* cmp_path)
{
...
static int g_test_suite_count(GTestSuite* suite)
{
    int n = 0;
    GSList* iter;
                                      ...
                                      gpointer unused_data)
{
    ...
    g_test_log(fatal ? G_TEST_LOG_ERROR : G_TEST_LOG_MESSAGE, msg, NULL, 0,
               NULL);
...
void g_assertion_message_cmpnum(const char* domain, const char* file, int line,
                                const char* func, const char* expr,
                                ...
                                long double arg2, char numtype)
{
    ...
    {
        case 'i':
            s = g_strdup_printf("assertion failed (%s): (%" G_GINT64_MODIFIER
                                "i %s %" G_GINT64_MODIFIER "i)",
                                expr, (gint64)arg1, cmp, (gint64)arg2);
            break;
        case 'x':
            s = g_strdup_printf(
                "assertion failed (%s): (0x%08" G_GINT64_MODIFIER
                "x %s 0x%08" G_GINT64_MODIFIER "x)",
                expr, (guint64)arg1, cmp, (guint64)arg2);
            break;
        case 'f':
            s = g_strdup_printf("assertion failed (%s): (%.9g %s %.9g)", expr,
                                (double)arg1, cmp, (double)arg2);
            break;
...
void g_assertion_message_cmpstr(const char* domain, const char* file, int line,
                                const char* func, const char* expr,
                                const char* arg1, const char* cmp,
                                const char* arg2)
{
    char *a1, *a2, *s, *t1 = NULL, *t2 = NULL;
    a1 = arg1 ? g_strconcat("\"", t1 = g_strescape(arg1, NULL), "\"", NULL)
              : g_strdup("NULL");
    a2 = arg2 ? g_strconcat("\"", t2 = g_strescape(arg2, NULL), "\"", NULL)
              : g_strdup("NULL");
    g_free(t1);
    g_free(t2);
...
void g_assertion_message_error(const char* domain, const char* file, int line,
                               const char* func, const char* expr,
                               const GError* error, GQuark error_domain,
                               int error_code)
{
    GString* gstring;
...
{
    if (data->child_status != -1 && data->stdout_io == NULL &&
        data->stderr_io == NULL)
...
static gboolean child_timeout(gpointer user_data)
{
    WaitForChildData* data = user_data;
...
static gboolean child_read(GIOChannel* io, GIOCondition cond,
                           gpointer user_data)
{
    WaitForChildData* data = user_data;
    GIOStatus status;
...
static void wait_for_child(GPid pid, int stdout_fd, gboolean echo_stdout,
                           int stderr_fd, gboolean echo_stderr, guint64 timeout)
{
...
gboolean g_test_trap_fork(guint64 usec_timeout, GTestTrapFlags test_trap_flags)
{
    ...
    {
        errsv = errno;
        g_error("failed to create pipes to fork test program: %s",
                g_strerror(errsv));
    }
    ...
    if (test_trap_last_pid == 0) /* child */
    {
        ...
        {
            errsv = errno;
            g_error("failed to dup2() in forked test program: %s",
                    g_strerror(errsv));
        }
        if (fd0 >= 3)
            close(fd0);
    ...
    else /* parent */
    {
        ...
        wait_for_child(
            test_trap_last_pid, stdout_pipe[0],
            !(test_trap_flags & G_TEST_TRAP_SILENCE_STDOUT), stderr_pipe[0],
            !(test_trap_flags & G_TEST_TRAP_SILENCE_STDERR), usec_timeout);
        return FALSE;
    }
#else
 ...
 * Since: 2.38
 */
void g_test_trap_subprocess(const char* test_path, guint64 usec_timeout,
                            GTestSubprocessFlags test_flags)
{
    ...
    g_assert(
        (test_flags & (G_TEST_TRAP_INHERIT_STDIN | G_TEST_TRAP_SILENCE_STDOUT |
                       G_TEST_TRAP_SILENCE_STDERR)) == 0);

    if (test_path)
    {
        if (!g_test_suite_case_exists(g_test_get_root(), test_path))
            g_error("g_test_trap_subprocess: test does not exist: %s",
                    test_path);
    }
    else
    {
    ...
    if (!g_spawn_async_with_pipes(test_initial_cwd, (char**)argv->pdata, NULL,
                                  flags, NULL, NULL, &pid, NULL, &stdout_fd,
                                  &stderr_fd, &error))
    {
        g_error("g_test_trap_subprocess() failed: %s", error->message);
    }
    g_ptr_array_free(argv, TRUE);

    wait_for_child(pid, stdout_fd,
                   !!(test_flags & G_TEST_SUBPROCESS_INHERIT_STDOUT), stderr_fd,
                   !!(test_flags & G_TEST_SUBPROCESS_INHERIT_STDERR),
                   usec_timeout);
}
...
static gboolean log_child_output(const gchar* process_id)
{
    gchar* escaped;
    ...
    {
        g_test_message("child process (%s) unknown wait status %d", process_id,
    ...
    if (test_trap_last_status == 0)
        ...
        g_test_message("child process (%s) exit status: %d (error)", process_id,
                       test_trap_last_status);
#endif
    ...
    const char* pattern)
{
    ...
    const char* match_error =
        match_result ? "failed to match" : "contains invalid match";
    char* process_id;
    ...
    if (must_pass && !g_test_trap_has_passed())
    {
        ...
        logged_child_output =
            logged_child_output || log_child_output(process_id);

        msg = g_strdup_printf("child process (%s) failed unexpectedly",
                              process_id);
    ...
    if (must_fail && g_test_trap_has_passed())
    {
        ...
        msg = g_strdup_printf("child process (%s) did not fail as expected",
                              process_id);
        g_assertion_message(domain, file, line, func, msg);
        g_free(msg);
    }
    if (stdout_pattern &&
        match_result ==
            !g_pattern_match_simple(stdout_pattern, test_trap_last_stdout))
    {
        char* msg;
        ...
        msg = g_strdup_printf(
            "stdout of child process (%s) %s: %s\nstderr was:\n%s", process_id,
            match_error, stdout_pattern, test_trap_last_stdout);
        g_assertion_message(domain, file, line, func, msg);
        g_free(msg);
    }
    if (stderr_pattern &&
        match_result ==
            !g_pattern_match_simple(stderr_pattern, test_trap_last_stderr))
    {
        char* msg;
        ...
        msg = g_strdup_printf(
            "stderr of child process (%s) %s: %s\nstderr was:\n%s", process_id,
            match_error, stderr_pattern, test_trap_last_stderr);
        g_assertion_message(domain, file, line, func, msg);
        g_free(msg);
    }
    g_free(process_id);
...
static gboolean g_test_log_extract(GTestLogBuffer* tbuffer)
{
    ...
    if (net_int(&p) == 0)
    {
        ...
        if (p <= tbuffer->data->str + mlength)
        {
            ...
            tbuffer->msgs =
                g_slist_prepend(tbuffer->msgs, g_memdup(&msg, sizeof(msg)));
            return TRUE;
        }
 ...
 * provided.
 */
void g_test_log_buffer_push(GTestLogBuffer* tbuffer, guint n_bytes,
                            const guint8* bytes)
{
    g_return_if_fail(tbuffer != NULL);
    if (n_bytes)
...
static gchar* g_test_build_filename_va(GTestFileType file_type,
                                       const gchar* first_path, va_list ap)
{
    ...
    for (num_path_segments = 2; num_path_segments < G_N_ELEMENTS(pathv);
         num_path_segments++)
    {
                                 ...
                                 const gchar* first_path, ...)
{
    ...
    if (test_filename_free_list == NULL)
        g_error(
            "g_test_get_filename() can only be used within testcase functions");
    ...
    while (!g_atomic_pointer_compare_and_exchange(test_filename_free_list,
                                                  node->next, node));
