...
#define FTC_NODE_TOP_FOR_HASH(cache, hash)                                     \
    ((cache)->buckets + ((((hash) & (cache)->mask) < (cache)->p)               \
                             ? ((hash) & ((cache)->mask * 2 + 1))              \
                             : ((hash) & (cache)->mask)))
#else
FT_LOCAL(FTC_Node*)
...
typedef FT_Error (*FTC_Node_NewFunc)(FTC_Node* pnode, FT_Pointer query,
                                     FTC_Cache cache);

typedef FT_Offset (*FTC_Node_WeightFunc)(FTC_Node node, FTC_Cache cache);

/* compare a node to a given key pair */
typedef FT_Bool (*FTC_Node_CompareFunc)(FTC_Node node, FT_Pointer key,
...
#ifndef FTC_INLINE
FT_LOCAL(FT_Error)
FTC_Cache_Lookup(FTC_Cache cache, FT_Offset hash, FT_Pointer query,
                 FTC_Node* anode);
#endif

FT_LOCAL(FT_Error)
FTC_Cache_NewNode(FTC_Cache cache, FT_Offset hash, FT_Pointer query,
                  FTC_Node* anode);
...
#define FTC_CACHE_LOOKUP_CMP(cache, nodecmp, hash, query, node, error)         \
    ...
    {                                                                          \
        ...
        if (_node != _manager->nodes_list)                                     \
            FTC_MruNode_Up((FTC_MruNode*)_nl, (FTC_MruNode)_node);             \
    }                                                                          \
    goto Ok_;                                                                  \
                                                                               \
    NewNode_:                                                                  \
...
#define FTC_CACHE_LOOKUP_CMP(cache, nodecmp, hash, query, node, error)         \
    FT_BEGIN_STMNT                                                             \
    error =                                                                    \
        FTC_Cache_Lookup(FTC_CACHE(cache), hash, query, (FTC_Node*)&(node));   \
    FT_END_STMNT
