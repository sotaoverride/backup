 ...
 * Since: 2.10
 */
void g_async_queue_push_sorted(GAsyncQueue* queue, gpointer data,
 ...
 * Since: 2.10
 */
void g_async_queue_push_sorted_unlocked(GAsyncQueue* queue, gpointer data,
                                        GCompareDataFunc func,
                                        gpointer user_data)
{
    SortData sd;
    ...
    g_queue_insert_sorted(&queue->queue, data,
                          (GCompareDataFunc)g_async_queue_invert_compare, &sd);
    if (queue->waiting_threads > 0)
        g_cond_signal(&queue->cond);
}

static gpointer g_async_queue_pop_intern_unlocked(GAsyncQueue* queue,
                                                  gboolean wait,
                                                  gint64 end_time)
{
    gpointer retval;
...
gpointer g_async_queue_timed_pop(GAsyncQueue* queue, GTimeVal* end_time)
{
    ...
    if (end_time != NULL)
    {
        m_end_time = g_get_monotonic_time() +
                     ((gint64)end_time->tv_sec * G_USEC_PER_SEC +
                      end_time->tv_usec - g_get_real_time());
    }
    else
        m_end_time = -1;
 ...
 * Since: 2.10
 */
void g_async_queue_sort(GAsyncQueue* queue, GCompareDataFunc func,
                        gpointer user_data)
{
    g_return_if_fail(queue != NULL);
 ...
 * Since: 2.10
 */
void g_async_queue_sort_unlocked(GAsyncQueue* queue, GCompareDataFunc func,
                                 gpointer user_data)
{
    SortData sd;
    ...
    g_queue_sort(&queue->queue, (GCompareDataFunc)g_async_queue_invert_compare,
                 &sd);
}
