...
G_DEFINE_TYPE_WITH_PRIVATE(GDBusAuthMechanismAnon, _g_dbus_auth_mechanism_anon,
...
static void _g_dbus_auth_mechanism_anon_finalize(GObject* object)
{
    ...
    if (G_OBJECT_CLASS(_g_dbus_auth_mechanism_anon_parent_class)->finalize !=
        NULL)
        G_OBJECT_CLASS(_g_dbus_auth_mechanism_anon_parent_class)
            ->finalize(object);
}
...
{
    mechanism->priv =
        _g_dbus_auth_mechanism_anon_get_instance_private(mechanism);
}
...
static gint mechanism_get_priority(void)
{
    /* We prefer ANONYMOUS to most other mechanism (such as DBUS_COOKIE_SHA1)
     * but not to EXTERNAL */
    return 50;
}
    ...
    mechanism_server_get_state(GDBusAuthMechanism* mechanism)
{
    ...
    g_return_val_if_fail(G_IS_DBUS_AUTH_MECHANISM_ANON(mechanism),
