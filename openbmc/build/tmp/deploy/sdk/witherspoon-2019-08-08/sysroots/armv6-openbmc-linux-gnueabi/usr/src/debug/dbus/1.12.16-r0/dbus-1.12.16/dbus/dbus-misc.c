 ...
 * @def DBUS_VERSION
 *
 * The COMPILE TIME version of libdbus, as a single integer that has 0 in the
 * most significant byte, the major version in the next most significant byte,
 ...
 * and #DBUS_VERSION which allow you to test the VERSION YOU ARE COMPILED
 * AGAINST. In other words, you can get either the runtime or the compile-time
 * version. Think carefully about which of these you want in a given case.
 *
 ...
 *
 */
void dbus_get_version(int* major_version_p, int* minor_version_p,
                      int* micro_version_p)
{
    if (major_version_p)
