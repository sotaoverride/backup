...
static void g_file_enumerator_real_next_files_async(
    GFileEnumerator* enumerator, int num_files, int io_priority,
...
static void g_file_enumerator_set_property(GObject* object, guint property_id,
                                           const GValue* value,
                                           GParamSpec* pspec)
{
...
static void next_async_callback_wrapper(GObject* source_object,
                                        GAsyncResult* res, gpointer user_data)
{
    ...
    if (enumerator->priv->outstanding_callback)
        (*enumerator->priv->outstanding_callback)(source_object, res,
                                                  user_data);
                                        ...
                                        gpointer user_data)
{
    ...
    if (enumerator->priv->closed)
    {
        g_task_report_new_error(enumerator, callback, user_data,
                                g_file_enumerator_next_files_async, G_IO_ERROR,
                                G_IO_ERROR_CLOSED,
 ...
 *g_file_enumerator_next_files_async().
 *
 * Returns: (transfer full) (element-type Gio.FileInfo): a #GList of
 *#GFileInfos. You must free the list with g_list_free() and unref the infos
 *with g_object_unref() when you're done with them.
 **/
...
static void close_async_callback_wrapper(GObject* source_object,
                                         GAsyncResult* res, gpointer user_data)
{
    GFileEnumerator* enumerator = G_FILE_ENUMERATOR(source_object);
 ...
 **/
void g_file_enumerator_close_async(GFileEnumerator* enumerator, int io_priority,
                                   GCancellable* cancellable,
                                   GAsyncReadyCallback callback,
                                   gpointer user_data)
{
    ...
    {
        g_task_report_new_error(enumerator, callback, user_data,
                                g_file_enumerator_close_async, G_IO_ERROR,
                                G_IO_ERROR_CLOSED,
                                _("File enumerator is already closed"));
        return;
    }
 ...
 * @out_info: (out) (transfer none) (optional): Output location for the next
 * #GFileInfo, or %NULL
 * @out_child: (out) (transfer none) (optional): Output location for the next
 * #GFile, or %NULL
 ...
 * "error".  End of iteration is signaled by @out_info or @out_child being
 * %NULL.
 *
 ...
 *   {
 *     GFileInfo *info;
 *     if (!g_file_enumerator_iterate (direnum, &info, NULL, cancellable,
 * error)) goto out; if (!info) break;
...
gboolean g_file_enumerator_iterate(GFileEnumerator* direnum,
                                   GFileInfo** out_info, GFile** out_child,
                                   GCancellable* cancellable, GError** error)
{
    gboolean ret = FALSE;
    ...
    if (ret_info)
    {
        if (out_child != NULL)
        {
            ...
            if (G_UNLIKELY(name == NULL))
                g_warning("g_file_enumerator_iterate() created without "
                          "standard::name");
            else
            {
                *out_child = g_file_get_child(
                    g_file_enumerator_get_container(direnum), name);
                g_object_set_qdata_full((GObject*)direnum, cached_child_quark,
                                        *out_child,
        ...
        if (out_info != NULL)
        {
            g_object_set_qdata_full((GObject*)direnum, cached_info_quark,
                                    ret_info, (GDestroyNotify)g_object_unref);
            *out_info = ret_info;
        }
        else
...
static void next_files_thread(GTask* task, gpointer source_object,
                              gpointer task_data, GCancellable* cancellable)
{
    ...
    for (i = 0; i < num_files; i++)
    {
        ...
        if (info == NULL)
        {
            /* If we get an error after first file, return that on next
             * operation */
            if (error != NULL && i > 0)
            {
                if (g_error_matches(error, G_IO_ERROR, G_IO_ERROR_CANCELLED))
                    g_error_free(error); /* Never propagate cancel errors to
                                            other call */
                else
...
static GList* g_file_enumerator_real_next_files_finish(
