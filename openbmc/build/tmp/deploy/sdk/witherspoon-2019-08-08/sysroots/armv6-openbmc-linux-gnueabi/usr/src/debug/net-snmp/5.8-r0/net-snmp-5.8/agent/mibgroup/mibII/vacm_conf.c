...
void init_vacm_config_tokens(void)
{
    ...
    snmpd_register_config_handler(
        "access", vacm_parse_access, vacm_free_access,
        "name context model level prefix read write notify");
    snmpd_register_config_handler(
        "setaccess", vacm_parse_setaccess, vacm_free_access,
    ...
    snmpd_register_const_config_handler("vacmView", vacm_parse_config_view,
                                        NULL, NULL);
    snmpd_register_const_config_handler("vacmGroup", vacm_parse_config_group,
                                        NULL, NULL);
    snmpd_register_const_config_handler("vacmAccess", vacm_parse_config_access,
                                        NULL, NULL);
    snmpd_register_const_config_handler(
        "vacmAuthAccess", vacm_parse_config_auth_access, NULL, NULL);

    /* easy community auth handler */
    snmpd_register_config_handler(
        "authcommunity", vacm_parse_authcommunity, NULL,
        "authtype1,authtype2 community [default|hostname|network/bits [oid|-V "
        "view [context]]]");

    /* easy user auth handler */
    snmpd_register_config_handler("authuser", vacm_parse_authuser, NULL,
                                  "authtype1,authtype2 [-s secmodel] user "
                                  "[noauth|auth|priv [oid|-V view [context]]]");
    /* easy group auth handler */
    snmpd_register_config_handler("authgroup", vacm_parse_authuser, NULL,
                                  "authtype1,authtype2 [-s secmodel] group "
                                  "[noauth|auth|priv [oid|-V view [context]]]");

    snmpd_register_config_handler(
        "authaccess", vacm_parse_authaccess, vacm_free_access,
        "name authtype1,authtype2 [-s secmodel] group view [noauth|auth|priv "
        "[context|context*]]");

    /*
     * Define standard views "_all_" and "_none_"
     */
    snmp_register_callback(SNMP_CALLBACK_LIBRARY, SNMP_CALLBACK_PRE_READ_CONFIG,
...
void init_vacm_snmpd_easy_tokens(void)
{
    ...
    snmpd_register_config_handler(
        "rwcommunity", vacm_parse_rwcommunity, NULL,
        "community [default|hostname|network/bits [oid|-V view [context]]]");
    snmpd_register_config_handler(
        "rocommunity", vacm_parse_rocommunity, NULL,
        "community [default|hostname|network/bits [oid|-V view [context]]]");
#ifdef NETSNMP_TRANSPORT_UDPIPV6_DOMAIN
    snmpd_register_config_handler(
        "rwcommunity6", vacm_parse_rwcommunity6, NULL,
        "community [default|hostname|network/bits [oid|-V view [context]]]");
    snmpd_register_config_handler(
        "rocommunity6", vacm_parse_rocommunity6, NULL,
        "community [default|hostname|network/bits [oid|-V view [context]]]");
#endif
#endif /* support for community based SNMP */
    snmpd_register_config_handler(
        "rwuser", vacm_parse_rwuser, NULL,
        "user [noauth|auth|priv [oid|-V view [context]]]");
    snmpd_register_config_handler(
        "rouser", vacm_parse_rouser, NULL,
        "user [noauth|auth|priv [oid|-V view [context]]]");
}
...
void vacm_parse_group(const char* token, char* param)
{
    ...
    {
        config_perror("bad security model \"any\" should be: v1, v2c, usm or a "
                      "registered security plugin name - installing anyway");
    ...
    else
    {
        ...
        {
            config_perror("bad security model, should be: v1, v2c or usm or a "
                          "registered security plugin name");
            return;
        }
    }
...
int _vacm_parse_access_common(const char* token, char* param, char** st,
                              ...
                              int* ilevel, int* iprefix)
{
    ...
    *context = strtok_r(NULL, " \t\n", st);
    ...
    {
        config_perror("bad prefix match parameter \"0\", should be: exact or "
                      "prefix - installing anyway");
        *iprefix = 1;
    }
    else
    {
...
void vacm_parse_authuser(const char* token, char* confline)
{
...
void vacm_parse_authcommunity(const char* token, char* confline)
{
    int viewtypes = vacm_parse_authtokens(token, &confline);
    if (viewtypes != -1)
...
void vacm_parse_authaccess(const char* token, char* confline)
{
    char *group, *view, *tmp;
    const char* context;
    ...
    if (strcasecmp(view, "-s") == 0)
    {
        ...
        if (tmp)
        {
            ...
            else
            {
                ...
                if (model == SE_DNE)
                {
                    config_perror("bad security model, should be: v1, v2c or "
                                  "usm or a registered security plugin name");
                    return;
                }
            }
        }
    ...
    else
    {
        ...
        if (model == SNMP_SEC_MODEL_SNMPv1 || model == SNMP_SEC_MODEL_SNMPv2c)
            level = SNMP_SEC_LEVEL_NOAUTH;
        else
            level = SNMP_SEC_LEVEL_AUTHNOPRIV;
    }
    ...
    {
        DEBUGMSGTL(
            ("vacm:conf:authaccess", "existing access found, using it\n"));
    }
    if (!ap)
    {
...
void vacm_parse_setaccess(const char* token, char* param)
{
    ...
    if (_vacm_parse_access_common(token, param, &st, &name, &context, &imodel,
                                  &ilevel, &iprefix) == PARSE_FAIL)
    {
        return;
    }
...
void vacm_parse_view(const char* token, char* param)
{
    ...
    if (mask)
    {
        unsigned int val;
        i = 0;
        for (mask = strtok_r(mask, " .:", &st); mask;
             mask = strtok_r(NULL, " .:", &st))
        {
...
void vacm_gen_com2sec(int commcount, const char* community,
                      const char* addressname, const char* publishtoken,
                      void (*parser)(const char*, char*), char* secname,
                      size_t secname_len, char* viewname, size_t viewname_len,
                      int version, const char* context)
{
    ...
    if (context && *context)
        snprintf(line, sizeof(line), "-Cn %s %s %s '%s'", context, secname,
                 addressname, community);
    else
        snprintf(line, sizeof(line), "%s %s '%s'", secname, addressname,
                 community);
    ...
    {
        snprintf(line, sizeof(line), "grp%.28s v1 %s", secname, secname);
        line[sizeof(line) - 1] = 0;
        DEBUGMSGTL((publishtoken, "passing: %s %s\n", "group", line));
    ...
    {
        snprintf(line, sizeof(line), "grp%.28s v2c %s", secname, secname);
        line[sizeof(line) - 1] = 0;
        DEBUGMSGTL((publishtoken, "passing: %s %s\n", "group", line));
        vacm_parse_group("group", line);
    }
...
void vacm_parse_rwuser(const char* token, char* confline)
{
...
void vacm_parse_rocommunity(const char* token, char* confline)
{
    vacm_create_simple(token, confline, VACM_CREATE_SIMPLE_COMIPV4,
                       VACM_VIEW_READ_BIT);
}

void vacm_parse_rwcommunity(const char* token, char* confline)
{
    vacm_create_simple(token, confline, VACM_CREATE_SIMPLE_COMIPV4,
                       VACM_VIEW_READ_BIT | VACM_VIEW_WRITE_BIT);
}

void vacm_parse_rocommunity6(const char* token, char* confline)
{
    vacm_create_simple(token, confline, VACM_CREATE_SIMPLE_COMIPV6,
                       VACM_VIEW_READ_BIT);
}

void vacm_parse_rwcommunity6(const char* token, char* confline)
{
    vacm_create_simple(token, confline, VACM_CREATE_SIMPLE_COMIPV6,
                       VACM_VIEW_READ_BIT | VACM_VIEW_WRITE_BIT);
}

void vacm_create_simple(const char* token, char* confline, int parsetype,
                        int viewtypes)
{
    char line[SPRINT_MAX_LEN];
    ...
    int commversion = SNMP_SEC_MODEL_SNMPv1 | SNMP_SEC_MODEL_SNMPv2c;
#endif
    const char* rw = "none";
    ...
    {
        vacm_gen_com2sec(commcount, community, addressname, "com2sec",
                         &netsnmp_udp_parse_security, secname, sizeof(secname),
    ...
    if (parsetype == VACM_CREATE_SIMPLE_COMIPV6 ||
        parsetype == VACM_CREATE_SIMPLE_COM)
    {
        vacm_gen_com2sec(commcount, community, addressname, "com2sec6",
                         &netsnmp_udp6_parse_security, secname, sizeof(secname),
                         view_ptr, sizeof(viewname), commversion, context);
    }
#endif
#endif /* support for community based SNMP */
        ...
        else
        {
            ...
            for (tmp = grpname; *tmp; tmp++)
                if (!isalnum((unsigned char)(*tmp)))
                    *tmp = '_';
            snprintf(line, sizeof(line), "%s %s \"%s\"", grpname, model,
                     secname);
            line[sizeof(line) - 1] = 0;
            DEBUGMSGTL((token, "passing: %s %s\n", "group", line));
            vacm_parse_group("group", line);
        ...
        (viewtypes == (VACM_VIEW_READ_BIT | VACM_VIEW_WRITE_BIT)))
    {
        ...
        /*
         * access  anonymousGroupNameNUM  "" MODEL AUTHTYPE prefix
         * anonymousViewNUM [none/anonymousViewNUM] [none/anonymousViewNUM]
         */
        snprintf(line, sizeof(line), "%s %s %s %s %s %s %s %s", grpname,
                 context[0] ? context : "\"\"", model, authlevel,
                 (ctxprefix ? "prefix" : "exact"), viewname, rw, rw);
        line[sizeof(line) - 1] = 0;
        DEBUGMSGTL((token, "passing: %s %s\n", "access", line));
    ...
    else
    {
        ...
        /*
         * setaccess  anonymousGroupNameNUM  "" MODEL AUTHTYPE prefix viewname
         * viewval
         */
        int i;
        ...
        for (i = 0; i <= VACM_MAX_VIEWS; i++)
        {
            if (viewtypes & (1 << i))
            {
                snprintf(line, sizeof(line), "%s %s %s %s %s %s %s", grpname,
                         context[0] ? context : "\"\"", model, authlevel,
...
int vacm_standard_views(int majorID, int minorID, void* serverarg,
                        void* clientarg)
{
    char line[SPRINT_MAX_LEN];
...
int vacm_warn_if_not_configured(int majorID, int minorID, void* serverarg,
                                void* clientarg)
{
    const char* name =
...
int vacm_in_view_callback(int majorID, int minorID, void* serverarg,
                          void* clientarg)
{
 ...
 * VACM_SUBTREE_UNKNOWN(7) When testing an entire subtree, UNKNOWN (ie, the
 *entire subtree has both allowed and disallowed portions)
 *
 * Debug output listed as follows:
 *	\<securityName\> \<groupName\> \<viewName\> \<viewType\>
 */
int vacm_in_view(netsnmp_pdu* pdu, oid* name, size_t namelen, int check_subtree)
{
    int viewtype;
 ...
 *	\<securityName\> \<groupName\> \<viewName\> \<viewType\>
 */
int vacm_check_view(netsnmp_pdu* pdu, oid* name, size_t namelen,
...
int vacm_check_view_contents(netsnmp_pdu* pdu, oid* name, size_t namelen,
...
#define CONTEXTNAMEINDEXLEN 32
        ...
        if (find_sec_mod(pdu->securityModel))
    {
        ...
        DEBUGMSG(("mibII/vacm_vars",
                  ...
                  pdu->securityModel, pdu->securityName));
        sn = pdu->securityName;
    }
    else
    {
    ...
    if (name == NULL)
    { /* only check the setup of the vacm for the request */
