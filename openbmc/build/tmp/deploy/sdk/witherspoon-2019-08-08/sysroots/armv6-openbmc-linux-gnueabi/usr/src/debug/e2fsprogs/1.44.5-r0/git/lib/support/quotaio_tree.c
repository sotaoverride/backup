...
static void read_blk(struct quota_handle* h, unsigned int blk, dqbuf_t buf)
{
    ...
    err = h->e2fs_read(&h->qh_qf, blk << QT_BLKSIZE_BITS, buf, QT_BLKSIZE);
    if (err < 0)
...
static int write_blk(struct quota_handle* h, unsigned int blk, dqbuf_t buf)
{
    ...
    err = h->e2fs_write(&h->qh_qf, blk << QT_BLKSIZE_BITS, buf, QT_BLKSIZE);
                                ...
                                unsigned int blk)
{
    ...
    struct qt_disk_dqdbheader* dh = (struct qt_disk_dqdbheader*)buf;
    unsigned int nextblk = ext2fs_le32_to_cpu(dh->dqdh_next_free),
                 prevblk =
                                      ...
                                      struct dquot* dquot, int* err)
{
    ...
    *err = 0;
    ...
    /* Block will be full? */
    if (ext2fs_le16_to_cpu(dh->dqdh_entries) + 1 >= qtree_dqstr_in_blk(info))
    ...
    for (i = 0;
         i < qtree_dqstr_in_blk(info) && !qtree_entry_unused(info, ddquot); i++)
                          ...
                          unsigned int* treeblk, int depth)
{
    ...
    if (newson && ret >= 0)
    {
        ref[get_index(dquot->dq_id, depth)] = ext2fs_cpu_to_le32(newblk);
...
static void dq_insert_tree(struct quota_handle* h, struct dquot* dquot)
{
    ...
    if (do_insert_tree(h, dquot, &tmp, 0) < 0)
        log_err("Cannot write quota (id %u): %s", (unsigned int)dquot->dq_id,
                strerror(errno));
}
...
void qtree_write_dquot(struct dquot* dquot)
{
    ...
    log_debug("writing ddquot 1: off=%llu, info->dqi_entry_size=%u",
              dquot->dq_dqb.u.v2_mdqb.dqb_off, info->dqi_entry_size);
    retval = ext2fs_get_mem(info->dqi_entry_size, &ddquot);
    if (retval)
    {
        errno = ENOMEM;
        log_err("Quota write failed (id %u): %s", (unsigned int)dquot->dq_id,
                strerror(errno));
        return;
    }
                         ...
                         unsigned int blk)
{
    ...
    if (dquot->dq_dqb.u.v2_mdqb.dqb_off >> QT_BLKSIZE_BITS != blk)
        log_err(
            "Quota structure has offset to other block (%u) "
            "than it should (%u).",
            blk,
            (unsigned int)(dquot->dq_dqb.u.v2_mdqb.dqb_off >> QT_BLKSIZE_BITS));
                                      ...
                                      struct dquot* dquot, unsigned int blk)
{
    ...
    if (i == qtree_dqstr_in_blk(info))
        log_err("Quota for id %u referenced but not present.", dquot->dq_id);
    freedqbuf(buf);
...
struct dquot* qtree_read_dquot(struct quota_handle* h, qid_t id)
{
    ...
    if (offset > 0)
    {
        ...
        ret = h->e2fs_read(&h->qh_qf, offset, ddquot, info->dqi_entry_size);
        if (ret != info->dqi_entry_size)
        {
            if (ret > 0)
                errno = EIO;
            log_err("Cannot read quota structure for id %u: %s", dquot->dq_id,
                    strerror(errno));
        }
...
static int report_tree(struct dquot* dquot, unsigned int blk, int depth,
                       char* bitmap, int (*process_dquot)(struct dquot*, void*),
                       void* data)
{
    int entries = 0, i;
    dqbuf_t buf = getdqbuf();
    ...
    if (depth == QT_TREEDEPTH - 1)
    {
        for (i = 0; i<QT_BLKSIZE>> 2; i++)
        {
            ...
            if (blk && !get_bit(bitmap, blk))
                entries +=
                    report_block(dquot, blk, bitmap, process_dquot, data);
        }
    }
    else
    {
        for (i = 0; i<QT_BLKSIZE>> 2; i++)
        {
            ...
            {
                check_reference(dquot->dq_h, blk);
                entries += report_tree(dquot, blk, depth + 1, bitmap,
                                       process_dquot, data);
            }
        }
    }
                      ...
                      int (*process_dquot)(struct dquot*, void*), void* data)
{
    ...
    v2info->dqi_used_entries =
        report_tree(dquot, QT_TREEOFF, 0, bitmap, process_dquot, data);
