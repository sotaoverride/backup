...
/* Note: on GCC "no_sanitize_address" is a function attribute only, on llvm it
 * may also be applied to global variables. We define a specific macro which
 * knows this. Note that on GCC we don't need this decorator so much, since our
 * primary usecase for this attribute is registration structures placed in named
 * ELF sections which shall not be padded, but GCC doesn't pad those anyway if
...
#define __CMP(aq, a, bq, b)                                                    \
    ({                                                                         \
        const typeof(a) UNIQ_T(A, aq) = (a);                                   \
        const typeof(b) UNIQ_T(B, bq) = (b);                                   \
        UNIQ_T(A, aq)<UNIQ_T(B, bq) ? -1 : UNIQ_T(A, aq)> UNIQ_T(B, bq) ? 1    \
                                                                        : 0;   \
    })
...
#define __CLAMP(xq, x, lowq, low, highq, high)                                 \
    ({                                                                         \
        ...
        UNIQ_T(X, xq) > UNIQ_T(HIGH, highq)                                    \
            ? UNIQ_T(HIGH, highq)                                              \
            : UNIQ_T(X, xq) < UNIQ_T(LOW, lowq) ? UNIQ_T(LOW, lowq)            \
                                                : UNIQ_T(X, xq);               \
    })
...
#define assert_message_se(expr, message)                                       \
    ...
    {                                                                          \
        if (_unlikely_(!(expr)))                                               \
            log_assert_failed(message, __FILE__, __LINE__,                     \
                              __PRETTY_FUNCTION__);                            \
    } while (false)

#define assert_log(expr, message)                                              \
    ((_likely_(expr)) ? (true)                                                 \
                      : (log_assert_failed_return(message, __FILE__, __LINE__, \
                                                  __PRETTY_FUNCTION__),        \
                         false))
...
#define FOR_EACH_MAKE_CASE(...)                                                \
    GET_CASE_F(__VA_ARGS__, CASE_F_20, CASE_F_19, CASE_F_18, CASE_F_17,        \
               CASE_F_16, CASE_F_15, CASE_F_14, CASE_F_13, CASE_F_12,          \
               CASE_F_11, CASE_F_10, CASE_F_9, CASE_F_8, CASE_F_7, CASE_F_6,   \
               CASE_F_5, CASE_F_4, CASE_F_3, CASE_F_2, CASE_F_1)               \
    (CASE_F, __VA_ARGS__)

#define IN_SET(x, ...)                                                         \
    ({                                                                         \
        bool _found = false;                                                   \
        /* If the build breaks in the line below, you need to extend the case  \
         * macros. (We use "long double" as type for the array, in the hope                                               \
         * that checkers such as ubsan don't complain that the initializers    \
         * for the array are not representable by the base type. Ideally we'd                \
         * use typeof(x) as base type, but that doesn't work, as we want to                                                   \
         * use this on bitfields and gcc refuses typeof() on bitfields.) */    \
        assert_cc((sizeof((long double[]){__VA_ARGS__}) /                      \
                   sizeof(long double)) <= 20);                                \
        switch (x)                                                             \
        {                                                                      \
