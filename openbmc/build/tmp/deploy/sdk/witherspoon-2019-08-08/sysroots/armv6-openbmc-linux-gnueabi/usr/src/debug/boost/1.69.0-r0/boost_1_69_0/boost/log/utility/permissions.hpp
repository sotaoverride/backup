 ...
 *
 * On Windows platforms, it represents a pointer to \c SECURITY_ATTRIBUTES. The
 * user is responsible for allocating and reclaiming resources associated with
 * the pointer, \c permissions instance does not own them.
 *
...
class permissions
{
  ...
  public:
     ...
     * Initializing constructor.
     */
    BOOST_LOG_API permissions(boost::interprocess::permissions const& perms)
        BOOST_NOEXCEPT;
    ...
    /*!
     * Sets the default permissions, which are equivalent to \c NULL \c
     * SECURITY_ATTRIBUTES on Windows and \c 0644 on POSIX platforms.
     */
    ...
    /*!
     * Sets unrestricted permissions, which are equivalent to \c
     * SECURITY_ATTRIBUTES with \c NULL DACL on Windows and \c 0666 on POSIX
     * platforms.
     */
