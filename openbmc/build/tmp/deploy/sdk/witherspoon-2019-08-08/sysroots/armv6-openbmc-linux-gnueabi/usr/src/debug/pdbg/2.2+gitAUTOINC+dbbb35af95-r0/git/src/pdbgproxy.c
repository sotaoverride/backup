...
command_cb callbacks[LAST_CMD + 1] = {
    cmd_default, get_gprs, get_spr, get_mem,   stop_reason, set_thread,
    v_contc,     v_conts,  put_mem, interrupt, disconnect,  NULL};
...
static int gdbserver(uint16_t port)
{
    ...
    for_each_path_target_class("thread", target)
    {
        ...
        {
            fprintf(stderr,
                    "GDB server cannot be run on multiple threads at once.\n");
            return 0;
        }
    }
