...
static int add_cgroup(Hashmap* cgroups, const char* path, bool is_const,
                      struct CGroupInfo** ret)
{
...
static int add_process(Hashmap* cgroups, const char* path, pid_t pid,
                       const char* name)
{
...
static int cgroup_info_compare_func(struct CGroupInfo* const* a,
                                    struct CGroupInfo* const* b)
{
    return strcmp((*a)->cgroup_path, (*b)->cgroup_path);
}

static int dump_processes(Hashmap* cgroups, const char* cgroup_path,
                          ...
                          OutputFlags flags)
{
    ...
    if (!hashmap_isempty(cg->pids))
    {
        ...
        for (i = 0; i < n; i++)
        {
            ...
            special = special_glyph(more ? SPECIAL_GLYPH_TREE_BRANCH
                                         : SPECIAL_GLYPH_TREE_RIGHT);

            fprintf(stdout, "%s%s%*" PID_PRI " %s\n", prefix, special, width,
                    pids[i], name);
        }
    }
    ...
    if (cg->children)
    {
        ...
        for (i = 0; i < n; i++)
        {
            ...
            special = special_glyph(more ? SPECIAL_GLYPH_TREE_VERTICAL
            ...
            r = dump_processes(cgroups, child->cgroup_path, pp, n_columns,
                               flags);
            if (r < 0)
                return r;
        }
...
static int dump_extra_processes(Hashmap* cgroups, const char* prefix,
                                unsigned n_columns, OutputFlags flags)
{
    ...
    /* Prints the extra processes, i.e. those that are in cgroups we haven't
     * displayed yet. We show them as combined, sorted, linear list. */
    ...
    for (k = 0; k < n; k++)
    {
        ...
        fprintf(stdout, "%s%s %*" PID_PRI " %s\n", prefix,
                special_glyph(SPECIAL_GLYPH_TRIANGULAR_BULLET), width, pids[k],
                name);
    }
                        ...
                        OutputFlags flags, sd_bus_error* error)
{
    ...
    r = sd_bus_call_method(bus, "org.freedesktop.systemd1",
                           ...
                           "GetUnitProcesses", error, &reply, "s", unit);
    if (r < 0)
        return r;
