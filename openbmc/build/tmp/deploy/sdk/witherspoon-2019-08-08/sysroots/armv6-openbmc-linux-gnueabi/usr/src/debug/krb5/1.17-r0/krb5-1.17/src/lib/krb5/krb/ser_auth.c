 ...
 */
static krb5_error_code krb5_authenticator_size(krb5_context, krb5_pointer,
                                               size_t*);
static krb5_error_code krb5_authenticator_externalize(krb5_context,
                                                      krb5_pointer,
                                                      krb5_octet**, size_t*);
static krb5_error_code krb5_authenticator_internalize(krb5_context,
                                                      krb5_pointer*,
                                                      krb5_octet**, size_t*);
 ...
 */
static krb5_error_code krb5_authenticator_size(krb5_context kcontext,
                                               krb5_pointer arg, size_t* sizep)
{
    ...
    if ((authenticator = (krb5_authenticator*)arg))
    {
        ...
        if (!kret && authenticator->checksum)
            kret = krb5_size_opaque(kcontext, KV5M_CHECKSUM,
        ...
        if (!kret && authenticator->authorization_data)
        {
            ...
            for (i = 0; !kret && authenticator->authorization_data[i]; i++)
            {
                kret = krb5_size_opaque(
                    ...
                    (krb5_pointer)authenticator->authorization_data[i],
                    &required);
            }
        }
    }
 ...
 */
static krb5_error_code krb5_authenticator_externalize(krb5_context kcontext,
                                                      krb5_pointer arg,
                                                      krb5_octet** buffer,
                                                      size_t* lenremain)
{
    ...
    if ((authenticator = (krb5_authenticator*)arg))
    {
            ...
            (required <= remain))
        {
            ...
            /* Now ctime */
            (void)krb5_ser_pack_int32((krb5_int32)authenticator->ctime, &bp,
                                      &remain);

            /* Now cusec */
            (void)krb5_ser_pack_int32((krb5_int32)authenticator->cusec, &bp,
                                      &remain);

            /* Now seq_number */
            (void)krb5_ser_pack_int32(authenticator->seq_number, &bp, &remain);

            /* Now handle client, if appropriate */
            if (authenticator->client)
                kret = krb5_externalize_opaque(
                    kcontext, KV5M_PRINCIPAL,
                    (krb5_pointer)authenticator->client, &bp, &remain);
            else
                kret = 0;
            ...
            if (!kret && authenticator->checksum)
                kret = krb5_externalize_opaque(
                    kcontext, KV5M_CHECKSUM,
                    (krb5_pointer)authenticator->checksum, &bp, &remain);

            /* Now handle subkey, if appropriate */
            if (!kret && authenticator->subkey)
                kret = krb5_externalize_opaque(
                    kcontext, KV5M_KEYBLOCK,
                    (krb5_pointer)authenticator->subkey, &bp, &remain);
 ...
 */
static krb5_error_code krb5_authenticator_internalize(krb5_context kcontext,
                                                      krb5_pointer* argp,
                                                      krb5_octet** buffer,
                                                      size_t* lenremain)
{
    ...
    if (ibuf == KV5M_AUTHENTICATOR)
    {
        ...
        if ((remain >= (3 * sizeof(krb5_int32))) &&
            (authenticator =
                 (krb5_authenticator*)calloc(1, sizeof(krb5_authenticator))))
        {
            ...
            kret = krb5_internalize_opaque(
                kcontext, KV5M_PRINCIPAL, (krb5_pointer*)&authenticator->client,
                &bp, &remain);
            if (kret == EINVAL)
                kret = 0;
            ...
            if (!(kret = krb5_ser_unpack_int32(&ibuf, &bp, &remain)))
            {
                ...
                if ((authenticator->authorization_data =
                         (krb5_authdata**)calloc(len, sizeof(krb5_authdata*))))
                {
 ...
 */
krb5_error_code krb5_ser_authenticator_init(krb5_context kcontext)
{
