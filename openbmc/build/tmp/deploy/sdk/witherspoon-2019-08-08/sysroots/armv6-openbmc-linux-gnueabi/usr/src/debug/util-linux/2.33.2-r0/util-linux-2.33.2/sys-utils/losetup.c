...
static struct colinfo infos[] = {
    ...
    [COL_BACK_INO] = {"BACK-INO", 4, SCOLS_FL_RIGHT,
                      N_("backing file inode number"), SCOLS_JSON_NUMBER},
    [COL_BACK_MAJMIN] = {"BACK-MAJ:MIN", 6, 0,
                         N_("backing file major:minor device number")},
    [COL_NAME] = {"NAME", 0.25, 0, N_("loop device name")},
    [COL_OFFSET] = {"OFFSET", 5, SCOLS_FL_RIGHT,
...
static int printf_loopdev(struct loopdev_cxt* lc)
{
    ...
    if (!dev && !ino)
    {
         ...
         */
        printf("%s: []: (%s)", loopcxt_get_device(lc), fname);
    ...
    printf("%s: [%04d]:%" PRIu64 " (%s)", loopcxt_get_device(lc), (int)dev, ino,
           fname);
        ...
        switch (get_column_id(i))
        {
            ...
            case COL_MAJMIN:
            {
                ...
                if (loopcxt_get_device(lc) &&
                    ...
                    S_ISBLK(st.st_mode) && major(st.st_rdev) == LOOPDEV_MAJOR)
...
static int show_table(struct loopdev_cxt* lc, const char* file, uint64_t offset,
                      int flags)
{
    struct stat sbuf, *st = &sbuf;
...
static void __attribute__((__noreturn__)) usage(void)
{
    ...
    fputs(_(" -j, --associated <file>       list all devices associated with "
            "<file>\n"),
          out);
    fputs(_(" -L, --nooverlap               avoid possible conflict between "
            "devices\n"),
          out);
    ...
    fputs(_(" -o, --offset <num>            start at offset <num> into file\n"),
          out);
    fputs(_("     --sizelimit <num>         device is limited to <num> bytes "
            "of the file\n"),
          out);
    fputs(_(" -b  --sector-size <num>       set the logical sector size to "
            "<num>\n"),
          out);
    fputs(
        _(" -P, --partscan                create a partitioned loop device\n"),
        out);
    fputs(_(" -r, --read-only               set up a read-only loop device\n"),
          out);
    fputs(_("     --direct-io[=<on|off>]    open backing file with O_DIRECT\n"),
          out);
    fputs(_("     --show                    print device name after setup "
            "(with -f)\n"),
          out);
    ...
    fputs(_(" -l, --list                    list info about all or specified "
            "(default)\n"),
          out);
    fputs(_(" -n, --noheadings              don't print headings for --list "
            "output\n"),
          out);
    fputs(_(" -O, --output <cols>           specify columns to output for "
            "--list\n"),
          out);
...
static void warn_size(const char* filename, uint64_t size, uint64_t offset,
                      int flags)
{
    struct stat st;
...
static int create_loop(struct loopdev_cxt* lc, int nooverlap, int lo_flags,
                       int flags, const char* file, uint64_t offset,
                       uint64_t sizelimit)
{
    ...
    if (!hasdev && nooverlap)
    {
        ...
        switch (rc)
        {
            ...
            case 1: /* overlap */
                loopcxt_deinit(lc);
                errx(EXIT_FAILURE, _("%s: overlapping loop device exists"),
                     file);
            ...
            case 2: /* overlap -- full size and offset match (reuse) */
            {
                ...
                if (loopcxt_is_readonly(lc) && !(lo_flags & LO_FLAGS_READ_ONLY))
                {
                    loopcxt_deinit(lc);
                    errx(EXIT_FAILURE,
                         _("%s: overlapping read-only loop device exists"),
                         file);
                }
                    ...
                    lc_encrypt_type != LO_CRYPT_NONE)
                {
                    loopcxt_deinit(lc);
                    errx(EXIT_FAILURE,
                         _("%s: overlapping encrypted loop device exists"),
                         file);
                }
    ...
    do
    {
        ...
        /* errors */
        errpre =
            hasdev && loopcxt_get_fd(lc) < 0 ? loopcxt_get_device(lc) : file;
...
int main(int argc, char** argv)
{
    ...
    while ((c = getopt_long(argc, argv, "ab:c:d:Dfhj:JlLno:O:PrvV", longopts,
                            NULL)) != -1)
    {
        ...
        switch (c)
        {
            ...
            case 'b':
                set_blocksize = 1;
                blocksize = strtosize_or_err(
                    optarg, _("failed to parse logical block size"));
                break;
            case 'c':
                act = A_SET_CAPACITY;
                if (!is_loopdev(optarg) || loopcxt_set_device(&lc, optarg))
                    err(EXIT_FAILURE, _("%s: failed to use device"), optarg);
                break;
            case 'r':
            ...
            case OPT_DIO:
                use_dio = set_dio = 1;
                if (optarg)
                    use_dio = parse_switch(optarg, _("argument error"), "on",
                                           "off", NULL);
                break;
            case 'v':
                break;
    ...
    if (!act && optind + 1 == argc)
    {
        ...
        if (set_blocksize)
            act = A_SET_BLOCKSIZE;
        if (!is_loopdev(argv[optind]) || loopcxt_set_device(&lc, argv[optind]))
            err(EXIT_FAILURE, _("%s: failed to use device"), argv[optind]);
        optind++;
    }
    if (!act)
    {
    ...
    if (act != A_CREATE && (sizelimit || lo_flags || showdev))
        errx(EXIT_FAILURE,
             _("the options %s are allowed during loop device setup only"),
             "--{sizelimit,read-only,show}");

    if ((flags & LOOPDEV_FL_OFFSET) && act != A_CREATE &&
        (act != A_SHOW || !file))
        errx(EXIT_FAILURE,
    ...
    switch (act)
    {
        ...
        case A_DELETE:
            ...
            while (optind < argc)
            {
                if (!is_loopdev(argv[optind]) ||
                    ...
                    warn(_("%s: failed to use device"), argv[optind]);
                optind++;
                res += delete_loop(&lc);
            }
            break;
        ...
        case A_SET_CAPACITY:
            ...
            if (res)
                warn(_("%s: set capacity failed"), loopcxt_get_device(&lc));
            break;
