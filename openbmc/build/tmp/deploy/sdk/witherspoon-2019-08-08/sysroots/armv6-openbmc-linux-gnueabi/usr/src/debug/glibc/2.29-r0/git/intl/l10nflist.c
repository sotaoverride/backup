    ...
    const char* filename, int do_allocate)
{
    ...
    abs_filename = (char*)malloc(
        dirlist_len + strlen(language) +
        ((mask & XPG_TERRITORY) != 0 ? strlen(territory) + 1 : 0) +
        ((mask & XPG_CODESET) != 0 ? strlen(codeset) + 1 : 0) +
        ((mask & XPG_NORM_CODESET) != 0 ? strlen(normalized_codeset) + 1 : 0) +
        ((mask & XPG_MODIFIER) != 0 ? strlen(modifier) + 1 : 0) + 1 +
        strlen(filename) + 1);
    ...
    *cp++ = '/';
    ...
    retval = (struct loaded_l10nfile*)malloc(
        sizeof(*retval) + (__argz_count(dirlist, dirlist_len) *
                           (1 << pop(mask)) * sizeof(struct loaded_l10nfile*)));
    if (retval == NULL)
    {
        free(abs_filename);
        return NULL;
    ...
    /* If more than one directory is in the list this is a pseudo-entry
       ...
       ever.  */
    retval->decided =
        (__argz_count(dirlist, dirlist_len) != 1 ||
        ...
        if ((cnt & ~mask) == 0)
        {
            ...
            while ((dir = __argz_next((char*)dirlist, dirlist_len, dir)) !=
                ...
                retval->successor[entries++] = _nl_make_l10nflist(
                    l10nfile_list, dir, strlen(dir) + 1, cnt, language,
                    territory, codeset, normalized_codeset, modifier, filename,
                    1);
        }
