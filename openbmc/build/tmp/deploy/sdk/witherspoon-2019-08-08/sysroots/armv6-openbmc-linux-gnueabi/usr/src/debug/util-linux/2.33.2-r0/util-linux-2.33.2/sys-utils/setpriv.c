...
/* The shell to set SHELL env.variable if none is given in the user's passwd
 * entry.  */
...
struct privctx
{
    unsigned int nnp : 1,    /* no_new_privs */
...
static void __attribute__((__noreturn__)) usage(void)
{
    ...
    fputs(_(" -d, --dump                  show current state (and do not "
            "exec)\n"),
          out);
    ...
    fputs(_(" --init-groups               initialize supplementary groups\n"),
          out);
    fputs(_(" --groups <group,...>        set supplementary groups by UID or "
            "name\n"),
          out);
    ...
    fputs(
        _(" --reset-env                 clear all environment and initialize\n"
          "                               HOME, SHELL, USER, LOGNAME and "
          "PATH\n"),
        out);
    ...
    fputs(USAGE_SEPARATOR, out);
    fputs(
        _(" This tool can be dangerous.  Read the manpage, and be careful.\n"),
        out);
...
static void parse_pdeathsig(struct privctx* opts, const char* str)
{
    if (!strcmp(str, "keep"))
    {
        if (prctl(PR_GET_PDEATHSIG, &opts->pdeathsig) != 0)
            errx(SETPRIV_EXIT_PRIVERR, _("failed to get parent death signal"));
    }
...
static int cap_update(capng_act_t action, enum cap_type type, unsigned int cap)
{
    switch (type)
    {
        case CAP_TYPE_EFFECTIVE:
...
static void do_caps(enum cap_type type, const char* caps)
{
    ...
    while ((c = strsep(&my_caps, ",")))
    {
        ...
        else
        {
            ...
            else if (sscanf(c + 1, "cap_%d", &cap) == 1 && 0 <= cap &&
                     cap <= real_cap_last_cap())
                cap_update(action, type, cap);
            else
...
static void parse_securebits(struct privctx* opts, const char* arg)
{
    ...
    if (opts->securebits &
        ~(int)(SECBIT_NOROOT | SECBIT_NOROOT_LOCKED | SECBIT_NO_SETUID_FIXUP |
    ...
    opts->securebits |=
        SECBIT_KEEP_CAPS; /* We need it, and it's reset on exec */
...
static void do_selinux_label(const char* label)
{
    ...
    if (fd == -1)
        err(SETPRIV_EXIT_PRIVERR, _("cannot open %s"), _PATH_PROC_ATTR_EXEC);

    len = strlen(label);
    errno = 0;
    if (write(fd, label, len) != (ssize_t)len)
        err(SETPRIV_EXIT_PRIVERR, _("write failed: %s"), _PATH_PROC_ATTR_EXEC);

    if (close(fd) != 0)
        err(SETPRIV_EXIT_PRIVERR, _("close failed: %s"), _PATH_PROC_ATTR_EXEC);
}
...
int main(int argc, char** argv)
{
    ...
    while ((c = getopt_long(argc, argv, "+dhV", longopts, NULL)) != -1)
    {
        ...
        switch (c)
        {
            ...
            case REGID:
                ...
                opts.rgid = opts.egid =
                    get_group(optarg, _("failed to parse regid"));
                break;
            case CLEAR_GROUPS:
            ...
            case INIT_GROUPS:
                if (opts.init_groups)
                    errx(EXIT_FAILURE, _("duplicate --init-groups option"));
                opts.init_groups = 1;
                break;
            case GROUPS:
                if (opts.have_groups)
            ...
            case PDEATHSIG:
                if (opts.pdeathsig)
                    errx(EXIT_FAILURE, _("duplicate --keep-pdeathsig option"));
            ...
            case INHCAPS:
                if (opts.caps_to_inherit)
                    errx(EXIT_FAILURE, _("duplicate --inh-caps option"));
                opts.caps_to_inherit = optarg;
                break;
            case AMBCAPS:
                if (opts.ambient_caps)
                    errx(EXIT_FAILURE, _("duplicate --ambient-caps option"));
    ...
    if ((opts.have_rgid || opts.have_egid) && !opts.keep_groups &&
        !opts.clear_groups && !opts.init_groups && !opts.have_groups)
        errx(EXIT_FAILURE, _("--[re]gid requires --keep-groups, "
                             "--clear-groups, --init-groups, or --groups"));

    if (opts.init_groups && !opts.have_ruid)
        errx(EXIT_FAILURE, _("--init-groups requires --ruid or --reuid"));
    ...
    if (opts.have_securebits &&
        prctl(PR_SET_SECUREBITS, opts.securebits, 0, 0, 0) != 0)
    ...
    if (opts.bounding_set)
    {
        ...
        errno =
            EPERM; /* capng doesn't set errno if we're missing CAP_SETPCAP */
    ...
    if (opts.pdeathsig &&
        prctl(PR_SET_PDEATHSIG, opts.pdeathsig < 0 ? 0 : opts.pdeathsig) != 0)
