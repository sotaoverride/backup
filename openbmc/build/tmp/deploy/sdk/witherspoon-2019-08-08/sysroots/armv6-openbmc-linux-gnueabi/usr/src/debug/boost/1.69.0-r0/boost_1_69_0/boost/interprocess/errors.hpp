...
inline void fill_system_message(int sys_err_code, std::string& str)
{
    ...
    unsigned long ret = winapi::format_message(
        ...
        0, sys_err_code,
        winapi::make_lang_id(winapi::lang_neutral,
                             winapi::sublang_default), // Default language
        reinterpret_cast<char*>(&lpMsgBuf), 0, 0);
    if (ret != 0)
    {
...
struct error_info
{
    error_info(error_code_t ec = other_error) : m_nat(0), m_ec(ec)
    {
    }
