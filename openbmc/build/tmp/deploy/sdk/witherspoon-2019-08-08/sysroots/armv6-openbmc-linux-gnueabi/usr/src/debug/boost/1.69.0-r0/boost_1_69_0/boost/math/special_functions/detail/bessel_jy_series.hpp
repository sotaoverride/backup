...
inline T bessel_j_small_z_series(T v, T x, const Policy& pol)
{
    ...
    policies::check_series_iterations<T>(
        "boost::math::bessel_j_small_z_series<%1%>(%1%,%1%)", max_iter, pol);
...
struct bessel_y_small_z_series_term_a
{
    ...
    bessel_y_small_z_series_term_a(T v_, T x) : N(0), v(v_)
    {
        BOOST_MATH_STD_USING
        mult = x / 2;
        mult *= -mult;
...
struct bessel_y_small_z_series_term_b
{
    ...
    bessel_y_small_z_series_term_b(T v_, T x) : N(0), v(v_)
    {
        BOOST_MATH_STD_USING
        mult = x / 2;
        mult *= -mult;
...
// This series is only useful when the second term is small compared to the
// first otherwise we get catestrophic cancellation errors.
//
// Approximating tgamma(v) by v^v, and assuming |tgamma(-z)| < eps we end up
// requiring: eps/2 * v^v(x/2)^-v > (x/2)^v or log(eps/2) > v log((x/2)^2/v)
//
template <class T, class Policy>
inline T bessel_y_small_z_series(T v, T x, T* pscale, const Policy& pol)
{
    ...
    T p = log(x / 2);
    T scale = 1;
    bool need_logs = (v >= max_factorial<T>::value) ||
                     (tools::log_max_value<T>() / v < fabs(p));
    if (!need_logs)
    {
...
#endif
    policies::check_series_iterations<T>(
        "boost::math::bessel_y_small_z_series<%1%>(%1%,%1%)", max_iter, pol);
    result *= prefix;

    if (!need_logs)
    {
        prefix = boost::math::tgamma(-v, pol) * boost::math::cos_pi(v) * p /
                 constants::pi<T>();
    }
    else
    {
...
#if BOOST_WORKAROUND(__BORLANDC__, BOOST_TESTED_AT(0x582))
    T b = boost::math::tools::sum_series(
        s2, boost::math::policies::get_epsilon<T, Policy>(), max_iter, zero);
#else
    T b = boost::math::tools::sum_series(
        s2, boost::math::policies::get_epsilon<T, Policy>(), max_iter);
#endif
...
T bessel_yn_small_z(int n, T z, T* scale, const Policy& pol)
{
    ...
    //
    // Note that when called we assume that x < epsilon and n is a positive
    // integer.
    //
    BOOST_MATH_STD_USING
    BOOST_ASSERT(n >= 0);
    ...
    else if (n == 2)
    {
        return (z * z) / (4 * constants::pi<T>()) * log(z / 2) -
               ...
               ((z * z) / (8 * constants::pi<T>())) *
                   (T(3) / 2 - 2 * constants::euler<T>());
    }
    else
    {
        T p = pow(z / 2, n);
