...
static int asn1_primitive_print(BIO* out, ASN1_VALUE** fld, const ASN1_ITEM* it,
                               ...
                               const ASN1_PCTX* pctx)
{
    ...
    switch (it->itype)
    {
        case ASN1_ITYPE_PRIMITIVE:
            if (it->templates)
            {
                if (!asn1_template_print_ctx(out, fld, indent, it->templates,
                                             pctx))
                    return 0;
                break;
            }
        ...
        case ASN1_ITYPE_NDEF_SEQUENCE:
            ...
            for (i = 0, tt = it->templates; i < it->tcount; i++, tt++)
            {
                ...
                if (!asn1_template_print_ctx(out, tmpfld, indent + 2, seqtt,
                                             pctx))
                    return 0;
            }
            if (pctx->flags & ASN1_PCTX_FLAGS_SHOW_SEQUENCE)
                                   ...
                                   const ASN1_PCTX* pctx)
{
    ...
    return asn1_item_print_ctx(out, fld, indent, ASN1_ITEM_ptr(tt->item), fname,
                               sname, 0, pctx);
}
