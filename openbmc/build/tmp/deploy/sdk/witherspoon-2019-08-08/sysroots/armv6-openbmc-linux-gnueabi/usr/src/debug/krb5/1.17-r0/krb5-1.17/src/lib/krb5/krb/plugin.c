...
const char* interface_names[] = {
    "pwqual",    "kadm5_hook", "clpreauth", "kdcpreauth", "ccselect",
    "localauth", "hostrealm",  "audit",     "tls",        "kdcauthdata",
    "certauth",  "kadm5_auth", "kdcpolicy",
};

/* Return the context's interface structure for id, or NULL if invalid. */
static inline struct plugin_interface* get_interface(krb5_context context,
                                                     int id)
{
...
static void free_plugin_mapping(struct plugin_mapping* map)
{
    if (map == NULL)
        return;
    free(map->modname);
...
static void free_mapping_list(struct plugin_mapping** list)
{
...
static krb5_error_code make_plugin_mapping(krb5_context context,
 ...
 */
static krb5_error_code register_module(krb5_context context,
 ...
 * entry. */
static krb5_error_code parse_modstr(krb5_context context, const char* modstr,
                                    struct plugin_mapping** map_out)
{
    const char* sep;
...
static krb5_boolean find_in_list(char** list, const char* value)
{
    for (; *list != NULL; list++)
    {
...
static krb5_error_code get_profile_var(krb5_context context, int id,
                                       const char* varname, char*** out)
{
    krb5_error_code ret;
    const char* path[4];
...
static krb5_error_code make_full_list(krb5_context context, char** modstrs,
...
static void remove_disabled_modules(struct plugin_mapping** list,
                                    char** disable)
{
...
static void filter_enabled_modules(struct plugin_mapping** list, char** enable)
{
...
static krb5_error_code configure_interface(krb5_context context, int id)
{
    krb5_error_code ret;
...
static void load_if_needed(krb5_context context, struct plugin_mapping* map,
                           const char* iname)
{
    char* symname = NULL;
