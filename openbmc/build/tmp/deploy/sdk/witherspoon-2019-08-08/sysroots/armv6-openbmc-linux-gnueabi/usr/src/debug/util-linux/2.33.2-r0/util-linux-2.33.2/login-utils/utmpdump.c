...
static void print_utline(struct utmpx* ut, FILE* out)
{
    ...
    if (ut->ut_addr_v6[1] || ut->ut_addr_v6[2] || ut->ut_addr_v6[3])
        addr_string =
            inet_ntop(AF_INET6, &(ut->ut_addr_v6), buffer, sizeof(buffer));
    else
        addr_string =
            inet_ntop(AF_INET, &(ut->ut_addr_v6), buffer, sizeof(buffer));
    ...
    fprintf(out,
            ...
            ut->ut_type, ut->ut_pid, ut->ut_id, 8, (int)sizeof(ut->ut_user),
            ut->ut_user, 12, (int)sizeof(ut->ut_line), ut->ut_line, 20,
            (int)sizeof(ut->ut_host), ut->ut_host, addr_string, time_string);
}
...
static int follow_by_inotify(FILE* in, const char* filename, FILE* out)
{
    ...
    while (wd >= 0)
    {
        ...
        if (length < 0 && (errno == EINTR || errno == EAGAIN))
            continue;
        if (length < 0)
            err(EXIT_FAILURE, _("%s: cannot read inotify events"), filename);

        for (event = 0; event < length;)
        {
            struct inotify_event* ev = (struct inotify_event*)&buf[event];
...
static void __attribute__((__noreturn__)) usage(void)
{
    ...
    fprintf(out, _(" %s [options] [filename]\n"),
