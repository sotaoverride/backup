...
G_DEFINE_TYPE_WITH_CODE(
    GSocks4aProxy, g_socks4a_proxy, G_TYPE_OBJECT,
    G_IMPLEMENT_INTERFACE(G_TYPE_PROXY, g_socks4a_proxy_iface_init)
        _g_io_modules_ensure_extension_points_registered();
    g_io_extension_point_implement(G_PROXY_EXTENSION_POINT_NAME,
                                   g_define_type_id, "socks4a", 0))

static void g_socks4a_proxy_finalize(GObject* object)
{
    /* must chain up */
    G_OBJECT_CLASS(g_socks4a_proxy_parent_class)->finalize(object);
}

static void g_socks4a_proxy_init(GSocks4aProxy* proxy)
{
                            ...
                            const char* username, GError** error)
{
    ...
    if (addr_len != 4)
    {
        g_set_error(error, G_IO_ERROR, G_IO_ERROR_PROXY_FAILED,
                    _("SOCKSv4 does not support IPv6 address “%s”"), ip);
        g_object_unref(addr);
        return -1;
    }
...
static gboolean parse_connect_reply(const guint8* data, GError** error)
{
...
static GIOStream* g_socks4a_proxy_connect(GProxy* proxy, GIOStream* io_stream,
...
static void g_socks4a_proxy_connect_async(GProxy* proxy, GIOStream* io_stream,
                                          GProxyAddress* proxy_address,
                                          GCancellable* cancellable,
                                          ...
                                          gpointer user_data)
{
    ...
    data->length =
        set_connect_msg(data->buffer, hostname, port, username, &error);
    data->offset = 0;
...
static gboolean g_socks4a_proxy_supports_hostname(GProxy* proxy)
{
    return G_SOCKS4A_PROXY(proxy)->supports_hostname;
}

static void g_socks4a_proxy_class_init(GSocks4aProxyClass* class)
{
