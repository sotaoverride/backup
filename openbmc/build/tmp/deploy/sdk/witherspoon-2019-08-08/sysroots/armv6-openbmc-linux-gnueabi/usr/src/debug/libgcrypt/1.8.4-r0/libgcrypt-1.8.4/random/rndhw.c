...
#ifdef USE_PADLOCK
static size_t poll_padlock(void (*add)(const void*, size_t,
                                       enum random_origins),
...
#endif
static inline int rdrand_long(volatile unsigned long* v)
{
    int ok;
...
static inline int rdrand_nlong(volatile unsigned long* v, int count)
{
    while (count--)
        if (!rdrand_long(v++))
            return 0;
