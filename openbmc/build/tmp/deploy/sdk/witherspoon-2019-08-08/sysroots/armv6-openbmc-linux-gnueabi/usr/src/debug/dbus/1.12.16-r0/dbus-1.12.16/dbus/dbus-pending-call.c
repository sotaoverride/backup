...
static void _dbus_pending_call_trace_ref(DBusPendingCall* pending_call,
                                         ...
                                         const char* why)
{
    ...
    _dbus_trace_ref("DBusPendingCall", pending_call, old_refcount, new_refcount,
                    why, "DBUS_PENDING_CALL_TRACE", &enabled);
#endif
}
                                    ...
                                    DBusTimeoutHandler timeout_handler)
{
    ...
    {
        timeout = _dbus_timeout_new(timeout_milliseconds, timeout_handler,
                                    pending, NULL);
                                           ...
                                           DBusMessage* message)
{
    ...
    _dbus_assert(pending->reply_serial ==
...
dbus_bool_t _dbus_pending_call_set_timeout_error_unlocked(
    DBusPendingCall* pending, DBusMessage* message, dbus_uint32_t serial)
{
    DBusList* reply_link;
    DBusMessage* reply;
                                         ...
                                         DBusFreeFunction free_data_func)
{
    ...
    retval = _dbus_data_slot_list_set(&slot_allocator, &pending->slot_list,
