...
static const hst_response_header disabled = {.version = NSCD_VERSION,
                                             .found = -1,
                                             .h_name_len = 0,
                                             .h_aliases_cnt = 0,
                                             .h_addrtype = -1,
...
const struct iovec hst_iov_disabled = {.iov_base = (void*)&disabled,
                                       .iov_len = sizeof(disabled)};

/* This is the standard reply in case we haven't found the dataset.  */
static const hst_response_header notfound = {.version = NSCD_VERSION,
                                             .found = 0,
                                             .h_name_len = 0,
                                             .h_aliases_cnt = 0,
                                             .h_addrtype = -1,
...
static const hst_response_header tryagain = {.version = NSCD_VERSION,
                                             .found = 0,
                                             .h_name_len = 0,
                                             .h_aliases_cnt = 0,
                                             .h_addrtype = -1,
...
static time_t cache_addhst(struct database_dyn* db, int fd, request_header* req,
                           ...
                           struct hashentry* const he, struct datahead* dh,
                           int errval, int32_t ttl)
{
    ...
    if (hst == NULL)
    {
        ...
        else
        {
            ...
            const hst_response_header* resp =
                (errval == EAGAIN ? &tryagain : &notfound);

            if (fd != -1 && TEMP_FAILURE_RETRY(
                                send(fd, resp, total, MSG_NOSIGNAL)) != total)
                all_written = false;
            ...
            else if ((dataset = mempool_alloc(
                          db, (sizeof(struct dataset) + req->key_len), 1)) !=
                     NULL)
            {
                timeout = datahead_init_neg(
                    &dataset->head, (sizeof(struct dataset) + req->key_len),
                    total, (ttl == INT32_MAX ? db->negtimeout : ttl));
    ...
    else
    {
        ...
        total += (sizeof(struct dataset) + h_name_len +
       ...
       handling just for handling such a special case. */
        ...
        assert(cp ==
               dataset->strdata + total - offsetof(struct dataset, strdata));
        ...
        if (he != NULL)
        {
            ...
            else
            {
                if (h_addr_list_cnt == 1)
                {
                    ...
                    struct dataset* newp = (struct dataset*)mempool_alloc(
                        db, total + req->key_len, 1);
                    if (newp != NULL)
                    {
       ...
       handling just for handling such a special case. */
        if (!alloca_used)
        {
            ...
            /* Avoid adding names if more than one address is available.  See
               above for more info.  */
            assert(req->type == GETHOSTBYNAME || req->type == GETHOSTBYNAMEv6 ||
                   req->type == GETHOSTBYADDR || req->type == GETHOSTBYADDRv6);

            (void)cache_add(req->type, key_copy, req->key_len, &dataset->head,
                            true, db, owner, he == NULL);
                  ...
                  size_t buflen, struct hostent** hst, int32_t* ttlp)
{
    ...
    if (type == GETHOSTBYNAMEv6)
        return __gethostbyname3_r(key, AF_INET6, resultbufp, buffer, buflen,
                                  hst, &h_errno, ttlp, NULL);
                        ...
                        struct datahead* dh)
{
    ...
    while (lookup(req->type, key, &resultbuf, tmpbuf.data, tmpbuf.length, &hst,
                  &ttl) != 0 &&
           h_errno == NETDB_INTERNAL && (errval = errno) == ERANGE)
...
time_t readdhstbyname(struct database_dyn* db, struct hashentry* he,
                      struct datahead* dh)
{
    request_header req = {.type = GETHOSTBYNAME, .key_len = he->len};
...
time_t readdhstbyaddr(struct database_dyn* db, struct hashentry* he,
                      struct datahead* dh)
{
    request_header req = {.type = GETHOSTBYADDR, .key_len = he->len};
...
time_t readdhstbynamev6(struct database_dyn* db, struct hashentry* he,
                        struct datahead* dh)
{
    request_header req = {.type = GETHOSTBYNAMEv6, .key_len = he->len};
...
time_t readdhstbyaddrv6(struct database_dyn* db, struct hashentry* he,
                        struct datahead* dh)
{
    request_header req = {.type = GETHOSTBYADDRv6, .key_len = he->len};
