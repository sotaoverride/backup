...
static const struct help_topic topics[] = {
    {"help", N_("Print help"), N_("Print help"), N_("[COMMAND]")},
    {"version", N_("Print version"), N_("Print version information and exit")},
    {"list-apps", N_("List applications"),
     N_("List the installed D-Bus activatable applications (by .desktop "
        "files)")},
    {"launch", N_("Launch an application"),
     N_("Launch the application (with optional files to open)"),
     N_("APPID [FILE…]")},
    {"action", N_("Activate an action"),
     N_("Invoke an action on the application"), N_("APPID ACTION [PARAMETER]")},
...
static const struct help_substvar substvars[] = {
    ...
    {N_("APPID"),
     N_("Application identifier in D-Bus format (eg: org.example.viewer)")},
    {N_("FILE"),
     N_("Optional relative or absolute filenames, or URIs to open")},
    {N_("ACTION"), N_("The action name to invoke")},
    {N_("PARAMETER"),
     N_("Optional parameter to the action invocation, in GVariant format")}};

static int app_help(gboolean requested, const gchar* command)
{
    ...
    if (topic)
    {
        ...
        g_string_append_printf(string, "\n  %s %s %s\n\n", "gapplication",
                               topic->command,
                               topic->synopsis ? _(topic->synopsis) : "");
            ...
            for (i = 0; i < G_N_ELEMENTS(substvars); i++)
                if (strstr(topic->synopsis, substvars[i].var))
                    g_string_append_printf(string, "  %-*.*s   %s\n", maxwidth,
                                           maxwidth, _(substvars[i].var),
                                           _(substvars[i].description));
    ...
    else
    {
        ...
        g_string_append_printf(
            string, _("Use “%s help COMMAND” to get detailed help.\n\n"),
            "gapplication");
    }
...
static gboolean app_check_name(gchar** args, const gchar* command)
{
    if (args[0] == NULL)
    {
        g_printerr(
            _("%s command requires an application id to directly follow\n\n"),
            command);
        return FALSE;
    }
...
static int app_no_args(const gchar* command)
{
...
static int app_version(gchar** args)
{
...
static int app_list(gchar** args)
{
    GList* apps;
...
static int app_call(const gchar* app_id, const gchar* method_name,
                    GVariant* parameters)
{
    ...
    result = g_dbus_connection_call_sync(
        session, app_id, object_path, "org.freedesktop.Application",
        method_name, parameters, G_VARIANT_TYPE_UNIT, G_DBUS_CALL_FLAGS_NONE,
        -1, NULL, &error);
...
static int app_action(gchar** args)
{
    ...
    return app_call(
        args[0], "ActivateAction",
        g_variant_new("(sav@a{sv})", name, &params, app_get_platform_data()));
}

static int app_activate(const gchar* app_id)
{
    return app_call(app_id, "Activate",
                    g_variant_new("(@a{sv})", app_get_platform_data()));
}

static int app_launch(gchar** args)
{
    ...
    return app_call(
        args[0], "Open",
        g_variant_new("(as@a{sv})", &files, app_get_platform_data()));
}

static int app_list_actions(gchar** args)
{
