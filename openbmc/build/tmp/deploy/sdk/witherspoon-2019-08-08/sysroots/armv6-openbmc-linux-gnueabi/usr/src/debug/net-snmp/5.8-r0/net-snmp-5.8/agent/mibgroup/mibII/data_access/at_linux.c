...
void ARP_Scan_Init(void)
{
    ...
    if (time(NULL) < tm + 1)
    { /*Our cool one second cache implementation :-) */
        return;
    }
    ...
    while (fgets(line, sizeof(line), in))
    {
        ...
        {
            struct arptab* newtab = (struct arptab*)realloc(
                at, (sizeof(struct arptab) *
...
int ARP_Scan_Next(in_addr_t* IPAddr, char* PhysAddr, int* PhysAddrLen,
                  u_long* ifType, u_short* ifIndex)
{
    ...
    *ifType =
        at[arptab_current].at_flags & ATF_PERM ? 4 /*static */ : 3 /*dynamic */;
