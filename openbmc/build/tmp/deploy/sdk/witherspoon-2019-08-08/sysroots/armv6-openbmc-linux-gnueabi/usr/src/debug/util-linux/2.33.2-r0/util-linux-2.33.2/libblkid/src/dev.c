...
/*
 * NOTE: reference manual is not structured as code. The following section is a
 * generic section for all high-level cache search+iterate routines.
 */
...
void blkid_free_dev(blkid_dev dev)
{
    ...
    {
        blkid_tag tag =
            list_entry(dev->bid_tags.next, struct blkid_struct_tag, bit_tags);
        blkid_free_tag(tag);
    }
    free(dev->bid_xname);
    free(dev->bid_name);
...
int blkid_dev_set_search(blkid_dev_iterate iter, const char* search_type,
 ...
 */
int blkid_dev_next(blkid_dev_iterate iter, blkid_dev* ret_dev)
{
    blkid_dev dev;
    ...
    *ret_dev = NULL;
    while (iter->p != &iter->cache->bic_devs)
    {
        ...
        iter->p = iter->p->next;
        if (iter->search_type &&
            !blkid_dev_has_tag(dev, iter->search_type, iter->search_value))
            continue;
        *ret_dev = dev;
        return 0;
    }
