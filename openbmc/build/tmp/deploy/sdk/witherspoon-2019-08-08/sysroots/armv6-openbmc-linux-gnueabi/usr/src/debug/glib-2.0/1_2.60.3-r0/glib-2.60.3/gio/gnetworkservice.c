...
static void g_network_service_set_property(GObject* object, guint prop_id,
                                           const GValue* value,
                                           GParamSpec* pspec);
static void g_network_service_get_property(GObject* object, guint prop_id,
                                           GValue* value, GParamSpec* pspec);

static void
    g_network_service_connectable_iface_init(GSocketConnectableIface* iface);
...
G_DEFINE_TYPE_WITH_CODE(GNetworkService, g_network_service, G_TYPE_OBJECT,
                        G_ADD_PRIVATE(GNetworkService) G_IMPLEMENT_INTERFACE(
...
static void g_network_service_class_init(GNetworkServiceClass* klass)
{
    ...
    g_object_class_install_property(
        gobject_class, PROP_SERVICE,
        g_param_spec_string("service", P_("Service"),
                            P_("Service name, eg \"ldap\""), NULL,
                            G_PARAM_READWRITE | G_PARAM_CONSTRUCT_ONLY |
                                G_PARAM_STATIC_STRINGS));
    g_object_class_install_property(
        gobject_class, PROP_PROTOCOL,
        g_param_spec_string("protocol", P_("Protocol"),
                            P_("Network protocol, eg \"tcp\""), NULL,
                            G_PARAM_READWRITE | G_PARAM_CONSTRUCT_ONLY |
                                G_PARAM_STATIC_STRINGS));
    g_object_class_install_property(
        gobject_class, PROP_DOMAIN,
        g_param_spec_string("domain", P_("Domain"),
                            P_("Network domain, eg, \"example.com\""), NULL,
                            G_PARAM_READWRITE | G_PARAM_CONSTRUCT_ONLY |
                                G_PARAM_STATIC_STRINGS));
    g_object_class_install_property(
        gobject_class, PROP_DOMAIN,
        g_param_spec_string("scheme", P_("Scheme"),
                            P_("Network scheme (default is to use service)"),
                            NULL, G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS));
}

static void g_network_service_connectable_iface_init(
    GSocketConnectableIface* connectable_iface)
{
    ...
    connectable_iface->proxy_enumerate =
        g_network_service_connectable_proxy_enumerate;
...
GSocketConnectable* g_network_service_new(const gchar* service,
                                          ...
                                          const gchar* domain)
{
    return g_object_new(G_TYPE_NETWORK_SERVICE, "service", service, "protocol",
                        protocol, "domain", domain, NULL);
}
...
#define G_TYPE_NETWORK_SERVICE_ADDRESS_ENUMERATOR                              \
    (_g_network_service_address_enumerator_get_type())
#define G_NETWORK_SERVICE_ADDRESS_ENUMERATOR(obj)                              \
    (G_TYPE_CHECK_INSTANCE_CAST((obj),                                         \
                                G_TYPE_NETWORK_SERVICE_ADDRESS_ENUMERATOR,     \
...
G_DEFINE_TYPE(GNetworkServiceAddressEnumerator,
              _g_network_service_address_enumerator,
              G_TYPE_SOCKET_ADDRESS_ENUMERATOR)

static GSocketAddress* g_network_service_address_enumerator_next(
    ...
    GError** error)
{
    ...
    if (!srv_enum->srv->priv->targets)
    {
        ...
        targets = g_resolver_lookup_service(
            srv_enum->resolver, srv_enum->srv->priv->service,
            srv_enum->srv->priv->protocol, srv_enum->srv->priv->domain,
    ...
    do
    {
        if (srv_enum->addr_enum == NULL && srv_enum->t)
        {
            ...
            uri = _g_uri_from_authority(
                g_network_service_get_scheme(srv_enum->srv), hostname,
                g_srv_target_get_port(target), NULL);
            g_free(hostname);

            addr = g_network_address_parse_uri(
                uri, g_srv_target_get_port(target), &error);
            g_free(uri);
            ...
            if (srv_enum->use_proxy)
                srv_enum->addr_enum =
                    g_socket_connectable_proxy_enumerate(addr);
            else
...
static void g_network_service_address_enumerator_next_async(
    GSocketAddressEnumerator* enumerator, GCancellable* cancellable,
    GAsyncReadyCallback callback, gpointer user_data)
{
    ...
    if (!srv_enum->srv->priv->targets)
    {
        g_resolver_lookup_service_async(
            srv_enum->resolver, srv_enum->srv->priv->service,
            srv_enum->srv->priv->protocol, srv_enum->srv->priv->domain,
            cancellable, next_async_resolved_targets, task);
    }
    else
...
static void next_async_have_targets(GTask* task)
{
    GNetworkServiceAddressEnumerator* srv_enum = g_task_get_source_object(task);
...
static GSocketAddress* g_network_service_address_enumerator_next_finish(
    GSocketAddressEnumerator* enumerator, GAsyncResult* result, GError** error)
{
    return g_task_propagate_pointer(G_TASK(result), error);
}

static void _g_network_service_address_enumerator_init(
    GNetworkServiceAddressEnumerator* enumerator)
{
}
...
static void g_network_service_address_enumerator_finalize(GObject* object)
{
    ...
    G_OBJECT_CLASS(_g_network_service_address_enumerator_parent_class)
        ->finalize(object);
}

static void _g_network_service_address_enumerator_class_init(
    GNetworkServiceAddressEnumeratorClass* srvenum_class)
{
    ...
    enumerator_class->next_finish =
        g_network_service_address_enumerator_next_finish;
