...
static worker_state worker_encode(worker_thread* thr, worker_state state)
{
    ...
    ret = lzma_block_encoder_init(&thr->block_encoder, thr->allocator,
                                  &thr->block_options);
    if (ret != LZMA_OK)
    {
        worker_error(thr, ret);
    ...
    do
    {
        ...
        ret = thr->block_encoder.code(
            thr->block_encoder.coder, thr->allocator, thr->in, &in_pos,
            in_limit, thr->outbuf->buf, &thr->outbuf->size, out_size, action);
    ...
    switch (ret)
    {
        case LZMA_STREAM_END:
            ...
            ret =
                lzma_block_header_encode(&thr->block_options, thr->outbuf->buf);
            if (ret != LZMA_OK)
            {
                worker_error(thr, ret);
                return THR_STOP;
        ...
        case LZMA_OK:
            ...
            thr->outbuf->size = 0;
            ret = lzma_block_uncomp_encode(&thr->block_options, thr->in,
                                           in_size, thr->outbuf->buf,
...
static MYTHREAD_RET_TYPE worker_start(void* thr_ptr)
{
    ...
    while (true)
    {
        ...
        mythread_sync(thr->coder->mutex)
        {
            ...
            thr->coder->progress_in += thr->outbuf->uncompressed_size;
                                 ...
                                 lzma_action action)
{
    while (*in_pos < in_size || (coder->thr != NULL && action != LZMA_RUN))
    {
        ...
        lzma_bufcpy(in, in_pos, in_size, coder->thr->in, &thr_in_size,
                    coder->block_size);
...
static bool wait_for_work(lzma_stream_coder* coder, mythread_condtime* wait_abs,
                          bool* has_blocked, bool has_input)
{
    ...
    mythread_sync(coder->mutex)
    {
        ...
        while ((!has_input || coder->threads_free == NULL ||
               ...
               coder->thread_error == LZMA_OK && !timed_out)
        {
            if (coder->timeout != 0)
                timed_out = mythread_cond_timedwait(&coder->cond, &coder->mutex,
                                                    wait_abs) != 0;
            else
    ...
    switch (coder->sequence)
    {
        case SEQ_STREAM_HEADER:
            lzma_bufcpy(coder->header, &coder->header_pos,
                        sizeof(coder->header), out, out_pos, out_size);
        ...
        case SEQ_BLOCK:
        {
            ...
            while (true)
            {
                mythread_sync(coder->mutex)
                {
                    ...
                    ret = lzma_outq_read(&coder->outq, out, out_pos, out_size,
                ...
                if (ret == LZMA_STREAM_END)
                {
                    ...
                    ret = lzma_index_append(coder->index, allocator,
                                            unpadded_size, uncompressed_size);
                ...
                ret = stream_encode_in(coder, allocator, in, in_pos, in_size,
                                       action);
                if (ret != LZMA_OK)
                {
                    threads_stop(coder, false);
            ...
            return_if_error(lzma_index_encoder_init(&coder->index_encoder,
                                                    allocator, coder->index));
            ...
            coder->progress_out +=
                lzma_index_size(coder->index) + LZMA_STREAM_HEADER_SIZE;
        }
        ...
        case SEQ_INDEX:
        {
            ...
            const lzma_ret ret = coder->index_encoder.code(
                coder->index_encoder.coder, allocator, NULL, NULL, 0, out,
                out_pos, out_size, LZMA_RUN);
            if (ret != LZMA_STREAM_END)
                return ret;
        ...
        case SEQ_STREAM_FOOTER:
            lzma_bufcpy(coder->header, &coder->header_pos,
                        sizeof(coder->header), out, out_pos, out_size);
            return coder->header_pos < sizeof(coder->header) ? LZMA_OK
                                                             : LZMA_STREAM_END;
    }
...
static void get_progress(void* coder_ptr, uint64_t* progress_in,
                         uint64_t* progress_out)
{
    lzma_stream_coder* coder = coder_ptr;
        ...
        *progress_out = coder->progress_out;
        ...
        for (size_t i = 0; i < coder->threads_initialized; ++i)
        {
            mythread_sync(coder->threads[i].mutex)
            {
                ...
                *progress_out += coder->threads[i].progress_out;
            }
        }
    }
                                       ...
                                       const lzma_mt* options)
{
    ...
    uint64_t outbuf_size_max;
    return_if_error(
        get_options(options, &easy, &filters, &block_size, &outbuf_size_max));
    ...
    if (coder->threads_max != options->threads)
    {
        ...
        coder->threads =
            lzma_alloc(options->threads * sizeof(worker_thread), allocator);
    ...
    // Output queue
    return_if_error(lzma_outq_init(&coder->outq, allocator, outbuf_size_max,
                                   options->threads));
    ...
    return_if_error(
        lzma_stream_header_encode(&coder->stream_flags, coder->header));
    ...
    lzma_stream_encoder_mt_memusage(const lzma_mt* options)
{
    ...
    if (get_options(options, &easy, &filters, &block_size, &outbuf_size_max) !=
        LZMA_OK)
        return UINT64_MAX;
    ...
    const uint64_t outq_memusage =
        lzma_outq_memusage(outbuf_size_max, options->threads);
    if (outq_memusage == UINT64_MAX)
        return UINT64_MAX;

    // Sum them with overflow checking.
    uint64_t total_memusage = LZMA_MEMUSAGE_BASE + sizeof(lzma_stream_coder) +
