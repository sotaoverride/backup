...
static int test_keys(RSA_secret_key* sk, unsigned int nbits)
{
...
static int check_exponent(void* arg, gcry_mpi_t a)
{
    gcry_mpi_t e = arg;
    gcry_mpi_t tmp;
    int result;
 ...
 */
static gpg_err_code_t generate_std(RSA_secret_key* sk, unsigned int nbits,
                                   unsigned long use_e, int transient_key)
{
    ...
    */
    ...
    do
    {
        ...
        { /* We check the exponent later. */
            p = _gcry_generate_secret_prime(nbits / 2, random_level, NULL,
                                            NULL);
            q = _gcry_generate_secret_prime(nbits / 2, random_level, NULL,
                                            NULL);
        }
 ...
 * Returns: 2 structures filled with all needed values
 */
static gpg_err_code_t generate_fips(RSA_secret_key* sk, unsigned int nbits,
                                    unsigned long use_e, gcry_sexp_t testparms,
                                    int transient_key)
{
    gcry_mpi_t p, q; /* the two primes */
    gcry_mpi_t d;    /* the private key */
...
static gcry_mpi_t gen_x931_parm_xp(unsigned int nbits)
{
    gcry_mpi_t xp;
...
/* Helper for generate_x931.  */
static gcry_mpi_t gen_x931_parm_xi(void)
{
    gcry_mpi_t xi;
...
/* Variant of the standard key generation code using the algorithm
   ...
   testing.  */
static gpg_err_code_t generate_x931(RSA_secret_key* sk, unsigned int nbits,
                                    ...
                                    gcry_sexp_t deriveparms, int* swapped)
{
    ...
    *swapped = 0;
    ...
    /* Compute or extract the derive parameters.  */
    {
        ...
        xp = NULL;
        _gcry_mpi_release(xp1);
        xp1 = NULL;
        _gcry_mpi_release(xp2);
        xp2 = NULL;
        _gcry_mpi_release(xq);
        xq = NULL;
        _gcry_mpi_release(xq1);
        xq1 = NULL;
        _gcry_mpi_release(xq2);
        xq2 = NULL;
        if (!p || !q)
        {
            _gcry_mpi_release(p);
 ...
 */
static void public(gcry_mpi_t output, gcry_mpi_t input, RSA_public_key* pkey)
{
 ...
 *	m = c^d mod n
 */
static void secret_core_std(gcry_mpi_t M, gcry_mpi_t C, gcry_mpi_t D,
                            gcry_mpi_t N)
{
    mpi_powm(M, C, D, N);
}
 ...
 *      m = m1 + h * p
 */
static void secret_core_crt(gcry_mpi_t M, gcry_mpi_t C, gcry_mpi_t D,
                            unsigned int Nlimbs, gcry_mpi_t P, gcry_mpi_t Q,
                            gcry_mpi_t U)
{
...
static void secret_blinded(gcry_mpi_t output, gcry_mpi_t input,
...
static gcry_err_code_t rsa_generate(const gcry_sexp_t genparms,
                                    gcry_sexp_t* r_skey)
{
    gpg_err_code_t ec;
    unsigned int nbits;
    ...
    deriveparms =
        (genparms ? sexp_find_token(genparms, "derive-parms", 0) : NULL);
    if (!deriveparms)
    {
    ...
    if (!ec)
    {
        ec = sexp_build(r_skey, NULL,
                        ...
                        " %S)",
                        sk.n, sk.e, sk.n, sk.e, sk.d, sk.p, sk.q, sk.u,
                        swap_info);
    }
...
static gcry_err_code_t rsa_check_secret_key(gcry_sexp_t keyparms)
{
    gcry_err_code_t rc;
    RSA_secret_key sk = {NULL, NULL, NULL, NULL, NULL, NULL};

    /* To check the key we need the optional parameters. */
    rc = sexp_extract_param(keyparms, NULL, "nedpqu", &sk.n, &sk.e, &sk.d,
                            &sk.p, &sk.q, &sk.u, NULL);
    if (rc)
        goto leave;
...
static gcry_err_code_t rsa_encrypt(gcry_sexp_t* r_ciph, gcry_sexp_t s_data,
                                   gcry_sexp_t keyparms)
{
  ...
  public
    ...
    if ((ctx.flags & PUBKEY_FLAG_FIXEDLEN))
    {
        ...
        {
            rc =
                sexp_build(r_ciph, NULL, "(enc-val(rsa(a%b)))", (int)emlen, em);
            xfree(em);
        }
    }
    else
...
static gcry_err_code_t rsa_decrypt(gcry_sexp_t* r_plain, gcry_sexp_t s_data,
                                   gcry_sexp_t keyparms)

{
    ...
    /* Extract the key.  */
    rc = sexp_extract_param(keyparms, NULL, "nedp?q?u?", &sk.n, &sk.e, &sk.d,
                            &sk.p, &sk.q, &sk.u, NULL);
    if (rc)
        goto leave;
    if (DBG_CIPHER)
    ...
    switch (ctx.encoding)
    {
        case PUBKEY_ENC_PKCS1:
            ...
            plain = NULL;
            if (!rc)
                rc = sexp_build(r_plain, NULL, "(value %b)", (int)unpadlen,
                                unpad);
            break;

        case PUBKEY_ENC_OAEP:
            rc = _gcry_rsa_oaep_decode(&unpad, &unpadlen, ctx.nbits,
                                       ctx.hash_algo, plain, ctx.label,
                                       ctx.labellen);
            mpi_free(plain);
            plain = NULL;
            if (!rc)
        ...
        default:
            ...
            rc = sexp_build(
                r_plain, NULL,
                (ctx.flags & PUBKEY_FLAG_LEGACYRESULT) ? "%m" : "(value %m)",
                plain);
            break;
    }
...
static gcry_err_code_t rsa_sign(gcry_sexp_t* r_sig, gcry_sexp_t s_data,
                                gcry_sexp_t keyparms)
{
    gpg_err_code_t rc;
    struct pk_encoding_ctx ctx;
...
static gcry_err_code_t rsa_verify(gcry_sexp_t s_sig, gcry_sexp_t s_data,
                                  gcry_sexp_t keyparms)
{
    gcry_err_code_t rc;
    struct pk_encoding_ctx ctx;
...
static const char* selftest_sign_2048(gcry_sexp_t pkey, gcry_sexp_t skey)
{
...
static const char* selftest_encr_2048(gcry_sexp_t pkey, gcry_sexp_t skey)
{
    const char* errtxt = NULL;
    gcry_error_t err;
...
static gpg_err_code_t selftests_rsa(selftest_report_func_t report)
{
    const char* what;
    const char* errtxt;
    gcry_error_t err;
    ...
    if (!err)
        err = sexp_sscan(&pkey, NULL, sample_public_key,
                         strlen(sample_public_key));
    if (err)
    {
