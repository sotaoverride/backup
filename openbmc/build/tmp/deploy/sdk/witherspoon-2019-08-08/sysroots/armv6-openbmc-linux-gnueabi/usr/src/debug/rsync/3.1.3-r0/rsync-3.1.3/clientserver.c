...
static int exchange_protocols(int f_in, int f_out, char* buf, size_t bufsiz,
                              int am_client)
{
    int remote_sub = -1;
    ...
    if (remote_sub < 0)
    {
        ...
        {
            if (am_client)
                rprintf(FERROR, "rsync: server is speaking an incompatible "
                                "beta of protocol 30\n");
            else
                io_printf(f_out, "@ERROR: your client is speaking an "
...
int start_inband_exchange(int f_in, int f_out, const char* user, int argc,
                          char* argv[])
{
    int i, modlen;
...
static char* finish_pre_exec(pid_t pid, int write_fd, int read_fd,
                             char* request, char** early_argv, char** argv)
{
    ...
    *bp = '\0';
    ...
    if (wait_process(pid, &status, 0) < 0 || !WIFEXITED(status) ||
        WEXITSTATUS(status) != 0)
    {
        char* e;
        if (asprintf(&e, "pre-xfer exec returned failure (%d)%s%s%s\n%s",
                     status, status < 0 ? ": " : "",
                     status < 0 ? strerror(errno) : "", *buf ? ":" : "",
                     buf) < 0)
...
static int rsync_module(int f_in, int f_out, int i, const char* addr,
                        const char* host)
{
    int argc;
    ...
    if (*lp_prexfer_exec(i) || *lp_postxfer_exec(i))
    {
        ...
        if (*lp_prexfer_exec(i))
        {
            ...
            if (pipe(arg_fds) < 0 || pipe(error_fds) < 0 ||
                (pre_exec_pid = fork()) < 0)
            {
            ...
            if (pre_exec_pid == 0)
            {
                ...
                {
                    len =
                        read_arg_from_pipe(pre_exec_arg_fd, buf, BIGPATHBUFLEN);
                    if (len <= 0)
                    {
                        if (!len)
                            break;
    ...
    {
        err_msg =
            finish_pre_exec(pre_exec_pid, pre_exec_arg_fd, pre_exec_error_fd,
...
int start_daemon(int f_in, int f_out)
{
    ...
    if (*p)
    {
        ...
        if (setgid(gid) < 0)
        {
            rsyserr(FLOG, errno, "Unable to set group to daemon gid %ld",
                    (long)gid);
            return -1;
        }
        our_gid = MY_GID();
    ...
    if (*p)
    {
        ...
        if (setuid(uid) < 0)
        {
            rsyserr(FLOG, errno, "Unable to set user to daemon uid %ld",
                    (long)uid);
            return -1;
        }
        our_uid = MY_UID();
