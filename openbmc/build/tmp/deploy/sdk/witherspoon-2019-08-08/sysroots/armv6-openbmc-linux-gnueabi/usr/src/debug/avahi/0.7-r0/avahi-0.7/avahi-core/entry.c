...
static void transport_flags_from_domain(AvahiServer* s,
                                        ...
                                        const char* domain)
{
    ...
    assert(!((*flags & AVAHI_PUBLISH_USE_MULTICAST) &&
...
static int check_record_conflict(AvahiServer* s, AvahiIfIndex interface,
                                 AvahiProtocol protocol, AvahiRecord* r,
                                 AvahiPublishFlags flags)
{
    AvahiEntry* e;
    ...
    for (e = avahi_hashmap_lookup(s->entries_by_key, r->key); e;
         e = e->by_key_next)
    {
        if (e->dead)
            continue;

        if (!(flags & AVAHI_PUBLISH_UNIQUE) &&
        ...
        if ((interface <= 0 || e->interface <= 0 ||
             e->interface == interface) &&
            (protocol == AVAHI_PROTO_UNSPEC ||
             e->protocol == AVAHI_PROTO_UNSPEC || e->protocol == protocol))
...
static AvahiEntry* server_add_internal(AvahiServer* s, AvahiSEntryGroup* g,
                                       AvahiIfIndex interface,
                                       AvahiProtocol protocol,
                                       AvahiPublishFlags flags, AvahiRecord* r)
{
    ...
    AVAHI_CHECK_VALIDITY_RETURN_NULL(
        s, s->state != AVAHI_SERVER_FAILURE && s->state != AVAHI_SERVER_INVALID,
        AVAHI_ERR_BAD_STATE);
    AVAHI_CHECK_VALIDITY_RETURN_NULL(s, AVAHI_IF_VALID(interface),
                                     AVAHI_ERR_INVALID_INTERFACE);
    AVAHI_CHECK_VALIDITY_RETURN_NULL(s, AVAHI_PROTO_VALID(protocol),
                                     AVAHI_ERR_INVALID_PROTOCOL);
    AVAHI_CHECK_VALIDITY_RETURN_NULL(
        s,
        AVAHI_FLAGS_VALID(
            flags, AVAHI_PUBLISH_NO_ANNOUNCE | AVAHI_PUBLISH_NO_PROBE |
                       AVAHI_PUBLISH_UNIQUE | AVAHI_PUBLISH_ALLOW_MULTIPLE |
                       AVAHI_PUBLISH_UPDATE | AVAHI_PUBLISH_USE_WIDE_AREA |
                       AVAHI_PUBLISH_USE_MULTICAST),
        AVAHI_ERR_INVALID_FLAGS);
    AVAHI_CHECK_VALIDITY_RETURN_NULL(s,
                                     avahi_is_valid_domain_name(r->key->name),
                                     AVAHI_ERR_INVALID_HOST_NAME);
    AVAHI_CHECK_VALIDITY_RETURN_NULL(s, r->ttl != 0, AVAHI_ERR_INVALID_TTL);
    AVAHI_CHECK_VALIDITY_RETURN_NULL(s, !avahi_key_is_pattern(r->key),
                                     AVAHI_ERR_IS_PATTERN);
    AVAHI_CHECK_VALIDITY_RETURN_NULL(s, avahi_record_is_valid(r),
                                     AVAHI_ERR_INVALID_RECORD);
    AVAHI_CHECK_VALIDITY_RETURN_NULL(s, r->key->clazz == AVAHI_DNS_CLASS_IN,
                                     AVAHI_ERR_INVALID_DNS_CLASS);
    AVAHI_CHECK_VALIDITY_RETURN_NULL(
        s,
        (r->key->type != 0) && (r->key->type != AVAHI_DNS_TYPE_ANY) &&
            (r->key->type != AVAHI_DNS_TYPE_OPT) &&
            (r->key->type != AVAHI_DNS_TYPE_TKEY) &&
            (r->key->type != AVAHI_DNS_TYPE_TSIG) &&
            (r->key->type != AVAHI_DNS_TYPE_IXFR) &&
            (r->key->type != AVAHI_DNS_TYPE_AXFR),
        AVAHI_ERR_INVALID_DNS_TYPE);

    transport_flags_from_domain(s, &flags, r->key->name);
    AVAHI_CHECK_VALIDITY_RETURN_NULL(s, flags & AVAHI_PUBLISH_USE_MULTICAST,
                                     AVAHI_ERR_NOT_SUPPORTED);
    AVAHI_CHECK_VALIDITY_RETURN_NULL(s, !s->config.disable_publishing,
                                     AVAHI_ERR_NOT_PERMITTED);
    AVAHI_CHECK_VALIDITY_RETURN_NULL(
        s,
        !g ||
            (g->state != AVAHI_ENTRY_GROUP_ESTABLISHED &&
             g->state != AVAHI_ENTRY_GROUP_REGISTERING) ||
    ...
    if (flags & AVAHI_PUBLISH_UPDATE)
    {
        ...
        for (e = avahi_hashmap_lookup(s->entries_by_key, r->key); e;
             e = e->by_key_next)
        {
            if (!e->dead && e->group == g && e->interface == interface &&
                e->protocol == protocol)
                break;
        ...
        if (!avahi_record_equal_no_ttl(old_record, r) &&
            (!g || g->state != AVAHI_ENTRY_GROUP_UNCOMMITED))
        {
...
int avahi_server_add(AvahiServer* s, AvahiSEntryGroup* g,
...
const AvahiRecord* avahi_server_iterate(AvahiServer* s, AvahiSEntryGroup* g,
                                        void** state)
{
...
int avahi_server_dump(AvahiServer* s, AvahiDumpCallback callback,
                      void* userdata)
{
    AvahiEntry* e;
    ...
    for (e = s->entries; e; e = e->entries_next)
    {
        ...
        snprintf(ln, sizeof(ln), "%s ; iface=%i proto=%i", t, e->interface,
                 e->protocol);
        avahi_free(t);
    ...
    if (s->wide_area_lookup_engine)
        avahi_wide_area_cache_dump(s->wide_area_lookup_engine, callback,
                                   userdata);
    return AVAHI_OK;
}

static AvahiEntry* server_add_ptr_internal(AvahiServer* s, AvahiSEntryGroup* g,
                                           AvahiIfIndex interface,
                                           AvahiProtocol protocol,
                                           AvahiPublishFlags flags,
                                           uint32_t ttl, const char* name,
                                           const char* dest)
{
    ...
    AVAHI_CHECK_VALIDITY_RETURN_NULL(s,
                                     ...
                                     AVAHI_ERR_INVALID_HOST_NAME);
    AVAHI_CHECK_VALIDITY_RETURN_NULL(s, avahi_is_valid_domain_name(dest),
                                     AVAHI_ERR_INVALID_HOST_NAME);

    if (!name)
        name = s->host_name_fqdn;

    if (!(r = avahi_record_new_full(name, AVAHI_DNS_CLASS_IN,
                                    AVAHI_DNS_TYPE_PTR, ttl)))
    {
        avahi_server_set_errno(s, AVAHI_ERR_NO_MEMORY);
        return NULL;
    }
...
int avahi_server_add_ptr(AvahiServer* s, AvahiSEntryGroup* g,
                         AvahiIfIndex interface, AvahiProtocol protocol,
                         AvahiPublishFlags flags, uint32_t ttl,
                         const char* name, const char* dest)
{
    ...
    if (!(e = server_add_ptr_internal(s, g, interface, protocol, flags, ttl,
                                      name, dest)))
        return avahi_server_errno(s);
...
int avahi_server_add_address(AvahiServer* s, AvahiSEntryGroup* g,
                             AvahiIfIndex interface, AvahiProtocol protocol,
                             ...
                             AvahiAddress* a)
{
    ...
    AVAHI_CHECK_VALIDITY(
        s, AVAHI_PROTO_VALID(protocol) && AVAHI_PROTO_VALID(a->proto),
        AVAHI_ERR_INVALID_PROTOCOL);
    AVAHI_CHECK_VALIDITY(
        s,
        AVAHI_FLAGS_VALID(flags,
                          AVAHI_PUBLISH_NO_REVERSE | AVAHI_PUBLISH_NO_ANNOUNCE |
                              AVAHI_PUBLISH_NO_PROBE | AVAHI_PUBLISH_UPDATE |
                              AVAHI_PUBLISH_USE_WIDE_AREA |
                              AVAHI_PUBLISH_USE_MULTICAST),
        AVAHI_ERR_INVALID_FLAGS);
    AVAHI_CHECK_VALIDITY(s, !name || avahi_is_valid_fqdn(name),
                         AVAHI_ERR_INVALID_HOST_NAME);
    ...
    AVAHI_CHECK_VALIDITY(s, flags & AVAHI_PUBLISH_USE_MULTICAST,
                         AVAHI_ERR_NOT_SUPPORTED);
    ...
    if (!(flags & AVAHI_PUBLISH_NO_REVERSE))
    {
        ...
        if (!(reverse = server_add_ptr_internal(
                  s, g, interface, protocol, flags | AVAHI_PUBLISH_UNIQUE,
                  AVAHI_DEFAULT_TTL_HOST_NAME, reverse_n, name)))
        {
            ret = avahi_server_errno(s);
            goto finish;
        }
...
static AvahiEntry*
    server_add_txt_strlst_nocopy(AvahiServer* s, AvahiSEntryGroup* g,
                                 AvahiIfIndex interface, AvahiProtocol protocol,
                                 AvahiPublishFlags flags, uint32_t ttl,
                                 const char* name, AvahiStringList* strlst)
{
    ...
    if (!(r = avahi_record_new_full(name ? name : s->host_name_fqdn,
                                    AVAHI_DNS_CLASS_IN, AVAHI_DNS_TYPE_TXT,
                                    ttl)))
    {
...
static AvahiStringList* add_magic_cookie(AvahiServer* s,
...
static int server_add_service_strlst_nocopy(
    AvahiServer* s, AvahiSEntryGroup* g, AvahiIfIndex interface,
    AvahiProtocol protocol, AvahiPublishFlags flags, const char* name,
    const char* type, const char* domain, const char* host, uint16_t port,
    AvahiStringList* strlst)
{
    ...
    AVAHI_CHECK_VALIDITY_SET_RET_GOTO_FAIL(
        s,
        AVAHI_FLAGS_VALID(flags, AVAHI_PUBLISH_NO_COOKIE |
                                     AVAHI_PUBLISH_UPDATE |
                                     AVAHI_PUBLISH_USE_WIDE_AREA |
                                     AVAHI_PUBLISH_USE_MULTICAST),
        AVAHI_ERR_INVALID_FLAGS);
    AVAHI_CHECK_VALIDITY_SET_RET_GOTO_FAIL(s, avahi_is_valid_service_name(name),
                                           AVAHI_ERR_INVALID_SERVICE_NAME);
    AVAHI_CHECK_VALIDITY_SET_RET_GOTO_FAIL(
        s, avahi_is_valid_service_type_strict(type),
        AVAHI_ERR_INVALID_SERVICE_TYPE);
    AVAHI_CHECK_VALIDITY_SET_RET_GOTO_FAIL(
        s, !domain || avahi_is_valid_domain_name(domain),
        AVAHI_ERR_INVALID_DOMAIN_NAME);
    AVAHI_CHECK_VALIDITY_SET_RET_GOTO_FAIL(
        s, !host || avahi_is_valid_fqdn(host), AVAHI_ERR_INVALID_HOST_NAME);
    ...
    AVAHI_CHECK_VALIDITY_SET_RET_GOTO_FAIL(
        s, flags & AVAHI_PUBLISH_USE_MULTICAST, AVAHI_ERR_NOT_SUPPORTED);
    ...
    if ((ret = avahi_service_name_join(svc_name, sizeof(svc_name), name, type,
                                       domain)) < 0 ||
        (ret = avahi_service_name_join(ptr_name, sizeof(ptr_name), NULL, type,
                                       domain)) < 0 ||
    ...
    if (!(ptr_entry =
              server_add_ptr_internal(s, g, interface, protocol, 0,
    ...
    h = NULL;
    srv_entry =
        server_add_internal(s, g, interface, protocol, AVAHI_PUBLISH_UNIQUE, r);
    avahi_record_unref(r);
    ...
    txt_entry = server_add_txt_strlst_nocopy(
        s, g, interface, protocol, AVAHI_PUBLISH_UNIQUE, AVAHI_DEFAULT_TTL,
        svc_name, strlst);
    strlst = NULL;
    ...
    if (!(enum_entry =
              server_add_ptr_internal(s, g, interface, protocol, 0,
                                      AVAHI_DEFAULT_TTL, enum_ptr, ptr_name)))
    {
        ret = avahi_server_errno(s);
        goto fail;
    }
...
int avahi_server_add_service_strlst(AvahiServer* s, AvahiSEntryGroup* g,
                                    AvahiIfIndex interface,
                                    AvahiProtocol protocol,
                                    AvahiPublishFlags flags, const char* name,
                                    ...
                                    AvahiStringList* strlst)
{
    ...
    return server_add_service_strlst_nocopy(s, g, interface, protocol, flags,
                                            ...
                                            avahi_string_list_copy(strlst));
}

int avahi_server_add_service(AvahiServer* s, AvahiSEntryGroup* g,
                             AvahiIfIndex interface, AvahiProtocol protocol,
                             AvahiPublishFlags flags, const char* name,
                             const char* type, const char* domain,
                             const char* host, uint16_t port, ...)
{
    ...
    va_start(va, port);
    ret = server_add_service_strlst_nocopy(s, g, interface, protocol, flags,
                                           name, type, domain, host, port,
                                           avahi_string_list_new_va(va));
    va_end(va);
...
static int server_update_service_txt_strlst_nocopy(
    AvahiServer* s, AvahiSEntryGroup* g, AvahiIfIndex interface,
    AvahiProtocol protocol, AvahiPublishFlags flags, const char* name,
    const char* type, const char* domain, AvahiStringList* strlst)
{
    ...
    e = server_add_txt_strlst_nocopy(
        s, g, interface, protocol, AVAHI_PUBLISH_UNIQUE | AVAHI_PUBLISH_UPDATE,
        AVAHI_DEFAULT_TTL, svc_name, strlst);
    strlst = NULL;
    ...
    const char* type, const char* domain, AvahiStringList* strlst)
{
    ...
    return server_update_service_txt_strlst_nocopy(
        s, g, interface, protocol, flags, name, type, domain,
        avahi_string_list_copy(strlst));
}

/** Update the TXT record for a service with the NULL termonate list of strings
 */
int avahi_server_update_service_txt(AvahiServer* s, AvahiSEntryGroup* g,
                                    AvahiIfIndex interface,
                                    AvahiProtocol protocol,
                                    AvahiPublishFlags flags, const char* name,
                                    const char* type, const char* domain, ...)
{
    ...
    va_start(va, domain);
    ret = server_update_service_txt_strlst_nocopy(s, g, interface, protocol,
                                                  flags, name, type, domain,
                                                  avahi_string_list_new_va(va));
    va_end(va);
...
int avahi_server_add_service_subtype(AvahiServer* s, AvahiSEntryGroup* g,
                                     AvahiIfIndex interface,
                                     AvahiProtocol protocol,
                                     AvahiPublishFlags flags, const char* name,
                                     const char* type, const char* domain,
                                     const char* subtype)
{
    ...
    AVAHI_CHECK_VALIDITY_SET_RET_GOTO_FAIL(
        s,
        AVAHI_FLAGS_VALID(flags, AVAHI_PUBLISH_USE_MULTICAST |
                                     AVAHI_PUBLISH_USE_WIDE_AREA),
        AVAHI_ERR_INVALID_FLAGS);
    AVAHI_CHECK_VALIDITY_SET_RET_GOTO_FAIL(s, avahi_is_valid_service_name(name),
                                           AVAHI_ERR_INVALID_SERVICE_NAME);
    ...
    AVAHI_CHECK_VALIDITY_SET_RET_GOTO_FAIL(
        ...
        AVAHI_ERR_INVALID_DOMAIN_NAME);
    AVAHI_CHECK_VALIDITY_SET_RET_GOTO_FAIL(
        s, avahi_is_valid_service_subtype(subtype),
    ...
    if ((ret = avahi_server_add_ptr(s, g, interface, protocol, 0,
                                    AVAHI_DEFAULT_TTL, ptr_name, svc_name)) < 0)
        goto fail;
...
static AvahiEntry*
    server_add_dns_server_name(AvahiServer* s, AvahiSEntryGroup* g,
                               AvahiIfIndex interface, AvahiProtocol protocol,
                               AvahiPublishFlags flags, const char* domain,
                               AvahiDNSServerType type, const char* name,
                               uint16_t port /** should be 53 */)
{
    ...
    AVAHI_CHECK_VALIDITY_RETURN_NULL(
        s,
        AVAHI_FLAGS_VALID(flags, AVAHI_PUBLISH_USE_WIDE_AREA |
                                     AVAHI_PUBLISH_USE_MULTICAST),
        AVAHI_ERR_INVALID_FLAGS);
    AVAHI_CHECK_VALIDITY_RETURN_NULL(
        s, type == AVAHI_DNS_SERVER_UPDATE || type == AVAHI_DNS_SERVER_RESOLVE,
        AVAHI_ERR_INVALID_FLAGS);
    AVAHI_CHECK_VALIDITY_RETURN_NULL(s, port != 0, AVAHI_ERR_INVALID_PORT);
    AVAHI_CHECK_VALIDITY_RETURN_NULL(s, avahi_is_valid_fqdn(name),
                                     AVAHI_ERR_INVALID_HOST_NAME);
    AVAHI_CHECK_VALIDITY_RETURN_NULL(
        s, !domain || avahi_is_valid_domain_name(domain),
        AVAHI_ERR_INVALID_DOMAIN_NAME);
    ...
    AVAHI_ASSERT_TRUE(
        avahi_normalize_name(domain, normalized_d, sizeof(normalized_d)));

    snprintf(t, sizeof(t), "%s.%s",
             type == AVAHI_DNS_SERVER_RESOLVE ? "_domain._udp"
                                              : "_dns-update._udp",
             normalized_d);

    if (!(r = avahi_record_new_full(t, AVAHI_DNS_CLASS_IN, AVAHI_DNS_TYPE_SRV,
                                    AVAHI_DEFAULT_TTL_HOST_NAME)))
    {
        avahi_server_set_errno(s, AVAHI_ERR_NO_MEMORY);
        avahi_free(n);
...
int avahi_server_add_dns_server_address(
    AvahiServer* s, AvahiSEntryGroup* g, AvahiIfIndex interface,
    AvahiProtocol protocol, AvahiPublishFlags flags, const char* domain,
    AvahiDNSServerType type, const AvahiAddress* address,
    uint16_t port /** should be 53 */)
{
    ...
    AVAHI_CHECK_VALIDITY(s, AVAHI_IF_VALID(interface),
                         AVAHI_ERR_INVALID_INTERFACE);
    AVAHI_CHECK_VALIDITY(
        s, AVAHI_PROTO_VALID(protocol) && AVAHI_PROTO_VALID(address->proto),
        AVAHI_ERR_INVALID_PROTOCOL);
    AVAHI_CHECK_VALIDITY(
        s,
        AVAHI_FLAGS_VALID(flags, AVAHI_PUBLISH_USE_MULTICAST |
    ...
    a_entry = server_add_internal(
        s, g, interface, protocol,
    ...
    if (!(s_entry = server_add_dns_server_name(s, g, interface, protocol, flags,
                                               domain, type, n, port)))
    {
        if (!(flags & AVAHI_PUBLISH_UPDATE))
...
void avahi_s_entry_group_change_state(AvahiSEntryGroup* g,
...
AvahiSEntryGroup* avahi_s_entry_group_new(AvahiServer* s,
...
static void cleanup_time_event_callback(AVAHI_GCC_UNUSED AvahiTimeEvent* e,
                                        void* userdata)
{
...
static void schedule_cleanup(AvahiServer* s)
{
    ...
    if (!s->cleanup_time_event)
        s->cleanup_time_event = avahi_time_event_new(
            s->time_event_queue, avahi_elapse_time(&tv, 1000, 0),
            &cleanup_time_event_callback, s);
}
...
static void
    entry_group_register_time_event_callback(AVAHI_GCC_UNUSED AvahiTimeEvent* e,
                                             void* userdata)
{
...
int avahi_s_entry_group_commit(AvahiSEntryGroup* g)
{
    ...
    if (g->state != AVAHI_ENTRY_GROUP_UNCOMMITED &&
        g->state != AVAHI_ENTRY_GROUP_COLLISION)
    ...
    else
    {
        ...
        g->register_time_event =
            avahi_time_event_new(g->server->time_event_queue, &g->register_time,
                                 entry_group_register_time_event_callback, g);
...
int avahi_entry_is_commited(AvahiEntry* e)
{
    ...
    return !e->group || e->group->state == AVAHI_ENTRY_GROUP_REGISTERING ||
