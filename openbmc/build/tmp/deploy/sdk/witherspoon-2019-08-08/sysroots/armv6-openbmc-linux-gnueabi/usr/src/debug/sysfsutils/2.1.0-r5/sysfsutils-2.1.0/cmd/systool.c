 ...
 * printed in hex.
 */
static char* binary_files[] = {"config", "data"};
...
{
    unsigned short val =
        (unsigned char)buf[offset] | ((unsigned char)buf[offset + 1] << 8);
    return val;
}
        ...
        if (isbinaryvalue(attr))
        {
            ...
            for (i = 0; i < attr->len; i++)
            {
                if (!(i % 16) && (i != 0))
                {
                    ...
                    fprintf(stdout, " ");
                fprintf(stdout, " %02x", (unsigned char)attr->value[i]);
            }
            fprintf(stdout, "\n");
        }
...
static void show_attribute(struct sysfs_attribute* attr, int level)
{
    ...
    else if ((show_options & SHOW_ATTRIBUTES) ||
             ((show_options & SHOW_ATTRIBUTE_VALUE) &&
              (strcmp(attr->name, attribute_to_show) == 0)))
    {
        indent(level);
        fprintf(stdout, "%-20s", attr->name);
...
static void show_attributes(struct dlist* attributes, int level)
{
    if (attributes)
    {
        ...
        dlist_for_each_data(attributes, cur, struct sysfs_attribute)
        {
    ...
    if (device)
    {
        ...
        if (show_bus && (!(strcmp(show_bus, "pci"))))
        {
            ...
            if (attr)
            {
                ...
                {
                    vendor_id = get_pciconfig_word(PCI_VENDOR_ID, attr->value);
                    device_id = get_pciconfig_word(PCI_DEVICE_ID, attr->value);
                    fprintf(
                        stdout, "%s\n",
        ...
        if (show_options &
            (SHOW_ATTRIBUTES | SHOW_ATTRIBUTE_VALUE | SHOW_ALL_ATTRIB_VALUES))
        {
...
static void show_driver(struct sysfs_driver* driver, int level)
{
    ...
    if (driver)
    {
        ...
        if (devlist)
        {
            ...
            dlist_for_each_data(devlist, cur, struct sysfs_device)
            {
                if (show_options & SHOW_DRIVERS)
                {
...
static int show_sysfs_bus(char* busname)
{
    ...
    if (show_options & SHOW_DEVICES)
    {
        ...
        if (devlist)
        {
            ...
            {
                if (!device_to_show ||
                    (strcmp(device_to_show, curdev->bus_id) == 0))
    ...
    if (show_options & SHOW_DRIVERS)
    {
        ...
        if (drvlist)
        {
            dlist_for_each_data(drvlist, curdrv, struct sysfs_driver)
            {
...
static int show_sysfs_class(char* classname)
{
    ...
    if (clsdevlist)
    {
        dlist_for_each_data(clsdevlist, cur, struct sysfs_class_device)
        {
            if (device_to_show == NULL ||
                (strcmp(device_to_show, cur->name) == 0))
...
int main(int argc, char* argv[])
{
    ...
    while ((opt = getopt(argc, argv, cmd_options)) != EOF)
    {
        switch (opt)
        {
            ...
            case 'A':
                ...
                {
                    fprintf(stderr, "Attribute name %s is too long\n", optarg);
                    exit(1);
                }
    ...
    switch (argc)
    {
        ...
        case 1:
            ...
            {
                fprintf(stderr, "Invalid argument - device name too long\n");
                exit(1);
            }
            break;
        default:
    ...
    if ((!show_bus && !show_class && !show_module && !show_root) &&
        (show_options & (SHOW_ATTRIBUTES | SHOW_ATTRIBUTE_VALUE | SHOW_DEVICES |
