...
G_DEFINE_TYPE_WITH_CODE(
    GNetworkMonitorBase, g_network_monitor_base, G_TYPE_OBJECT,
    ...
    g_io_extension_point_implement(G_NETWORK_MONITOR_EXTENSION_POINT_NAME,
                                   g_define_type_id, "base", 0))

static void g_network_monitor_base_init(GNetworkMonitorBase* monitor)
{
...
static void g_network_monitor_base_constructed(GObject* object)
{
...
static void g_network_monitor_base_get_property(GObject* object, guint prop_id,
                                                GValue* value,
                                                GParamSpec* pspec)
{
    GNetworkMonitorBase* monitor = G_NETWORK_MONITOR_BASE(object);
    ...
    switch (prop_id)
    {
        ...
        case PROP_CONNECTIVITY:
            g_value_set_enum(value, monitor->priv->is_available
...
static void g_network_monitor_base_finalize(GObject* object)
{
    GNetworkMonitorBase* monitor = G_NETWORK_MONITOR_BASE(object);
...
static void can_reach_async_got_address(GObject* object, GAsyncResult* result,
                                        gpointer user_data)
{
...
static void g_network_monitor_base_can_reach_async(
...
static gboolean g_network_monitor_base_can_reach_finish(
...
static gboolean g_network_monitor_base_initable_init(GInitable* initable,
...
static gboolean emit_network_changed(gpointer user_data)
{
...
static void queue_network_changed(GNetworkMonitorBase* monitor)
{
    if (!monitor->priv->network_changed_source && !monitor->priv->initializing)
    {
        GSource* source;
                                        ...
                                        GInetAddressMask* network)
{
    ...
    {
        if (g_inet_address_mask_equal(monitor->priv->networks->pdata[i],
                                      network))
            return;
    }
