...
struct dropbear_progress_connection
{
    ...
    struct Queue* writequeue; /* A queue of encrypted packets to send with TCP
                                fastopen, or NULL. */
...
/* Deallocate a progress connection. Removes from the pending list if
iter!=NULL. Does not close sockets */
static void remove_connect(struct dropbear_progress_connection* c,
                           m_list_elem* iter)
{
    if (c->res)
    {
...
static void cancel_callback(int result, int sock, void* UNUSED(data),
    ...
    for (r = c->res_iter; r; r = r->ai_next)
    {
        ...
        if (c->bind_address || c->bind_port)
        {
            ...
            if (err)
            {
                ...
                snprintf(c->errstring, len,
                         "Error resolving bind address '%s' (port %s). %s",
            ...
            if (res < 0)
            {
                ...
                snprintf(c->errstring, len,
                         "Error binding local address '%s' (port %s). %s",
        ...
        if (fastopen)
        {
            ...
            if (res < 0)
            {
                if (errno != EINPROGRESS)
                {
                    ...
                    /* Not entirely sure which kind of errors are normal
                    - 2.6.32 seems to return EPIPE for any (nonblocking?)
                    sendmsg(). just fall back */
                    TRACE(("sendmsg tcp_fastopen failed, falling back. %s",
                           strerror(errno)));
...
struct dropbear_progress_connection*
    connect_remote(const char* remotehost, const char* remoteport,
                   connect_callback cb, void* cb_data, const char* bind_address,
...
void handle_connect_fds(const fd_set* writefd)
{
    ...
    for (iter = ses.conn_pending.first; iter; iter = iter->next)
    {
        ...
        TRACE(("handling %s port %s socket %d", c->remotehost, c->remoteport,
               c->sock));

        if (getsockopt(c->sock, SOL_SOCKET, SO_ERROR, &val, &vallen) != 0)
        {
            TRACE(("handle_connect_fds getsockopt(%d) SO_ERROR failed: %s",
                   c->sock, strerror(errno)))
            /* This isn't expected to happen - Unix has surprises though,
             * continue gracefully. */
            m_close(c->sock);
            c->sock = -1;
        }
        else if (val != 0)
        {
            /* Connect failed */
            TRACE(
                ("connect to %s port %s failed.", c->remotehost, c->remoteport))
            m_close(c->sock);
            c->sock = -1;
...
void connect_set_writequeue(struct dropbear_progress_connection* c,
                            struct Queue* writequeue)
{
    c->writequeue = writequeue;
}

void packet_queue_to_iovec(const struct Queue* queue, struct iovec* iov,
...
#define IOV_MAX UIO_MAXIOV
    ...
    *iov_count = MIN(MIN(queue->count, IOV_MAX), *iov_count);
    ...
    for (l = queue->head, i = 0; i < *iov_count; l = l->link, i++)
    {
        ...
        TRACE2(("write_packet writev #%d len %d/%d", i, len, writebuf->len))
...
void set_listen_fast_open(int sock)
{
    ...
    {
        TRACE(("set_listen_fast_open failed for socket %d: %s", sock,
               strerror(errno)))
    }
}
...
void set_sock_priority(int sock, enum dropbear_prio prio)
{
    ...
    rc = setsockopt(sock, IPPROTO_IPV6, IPV6_TCLASS, (void*)&iptos_val,
                    sizeof(iptos_val));
...
#endif
    rc = setsockopt(sock, IPPROTO_IP, IP_TOS, (void*)&iptos_val,
                    sizeof(iptos_val));
    if (rc < 0 && errno != ENOTSOCK)
    {
    ...
    rc = setsockopt(sock, SOL_SOCKET, SO_PRIORITY, (void*)&so_prio_val,
                    sizeof(so_prio_val));
    if (rc < 0 && errno != ENOTSOCK)
...
int get_sock_port(int sock)
{
    ...
    /* Return port number. */
    if ((r = getnameinfo((struct sockaddr*)&from, fromlen, NULL, 0, strport,
                         sizeof(strport), NI_NUMERICSERV)) != 0)
    {
        TRACE(
            ("netio.c/get_sock_port/getnameinfo NI_NUMERICSERV failed: %d", r))
    }
    return atoi(strport);
}
 ...
 * string.*/
int dropbear_listen(const char* address, const char* port, int* socks,
                    unsigned int sockcount, char** errstring, int* maxfd)
{
    ...
    nsock = 0;
    for (res = res0; res != NULL && nsock < sockcount; res = res->ai_next)
    {
                        ...
                        char** remote_host, char** remote_port, int host_lookup)
{
    ...
    if (fuzz.fuzzing)
    {
        fuzz_get_socket_address(fd, local_host, local_port, remote_host,
                                remote_port, host_lookup);
        return;
    }
#endif
...
void getaddrstring(struct sockaddr_storage* addr, char** ret_host,
                   char** ret_port, int host_lookup)
{
    ...
    ret = getnameinfo((struct sockaddr*)addr, len, host, sizeof(host) - 1, serv,
                      sizeof(serv) - 1, flags);
    ...
    if (ret != 0)
    {
        if (host_lookup)
        {
             ...
             * somehow. Eew. So we'll just return the IP, since that doesn't
             * seem to exhibit that behaviour. */
