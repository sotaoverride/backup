...
int attribute_compat_text_section __compat_regexec(
...
static reg_errcode_t __attribute_warn_unused_result__ re_search_internal(
    const regex_t* preg, const char* string, Idx length, Idx start,
    Idx last_start, Idx stop, size_t nmatch, regmatch_t pmatch[], int eflags)
{
    reg_errcode_t err;
...
static Idx __attribute_warn_unused_result__ check_matching(
    re_match_context_t* mctx, bool fl_longest_match, Idx* p_match_first)
{
    const re_dfa_t* const dfa = mctx->dfa;
    reg_errcode_t err;
    Idx match = 0;
...
static bool check_halt_node_context(const re_dfa_t* dfa, Idx node,
                                    unsigned int context)
{
...
static Idx proceed_next_node(const re_match_context_t* mctx, Idx nregs,
                                             ...
                                             re_node_set* dest_nodes)
{
    ...
    const re_node_set* candidates;
    candidates =
        ((mctx->state_log[str_idx] == NULL) ? NULL
...
static reg_errcode_t __attribute_warn_unused_result__ add_epsilon_src_nodes(
    const re_dfa_t* dfa, re_node_set* dest_nodes, const re_node_set* candidates)
{
    reg_errcode_t err = REG_NOERROR;
    Idx i;
...
static reg_errcode_t sub_epsilon_src_nodes(const re_dfa_t* dfa, Idx node,
                                           re_node_set* dest_nodes,
                                           const re_node_set* candidates)
{
    Idx ecl_idx;
                             ...
                             Idx dst_idx, Idx src_node, Idx src_idx)
{
    ...
    for (lim_idx = 0; lim_idx < limits->nelem; ++lim_idx)
    {
        ...
        dst_pos =
            check_dst_limits_calc_pos(mctx, limits->elems[lim_idx], subexp_idx,
                      ...
                      Idx str_idx, const re_node_set* candidates)
{
    ...
    local_sctx.sifted_states =
        NULL; /* Mark that it hasn't been initialized.  */
    ...
    for (node_idx = 0; node_idx < candidates->nelem; ++node_idx)
    {
        ...
        do
        {
            ...
            if (to_idx > sctx->last_str_idx ||
                ...
                check_dst_limits(mctx, &sctx->limits, node, str_idx, dst_node,
                                 to_idx))
                continue;
...
static re_dfastate_t* __attribute_warn_unused_result__ transit_state(
    reg_errcode_t* err, re_match_context_t* mctx, re_dfastate_t* state)
{
                                    ...
                                    Idx bkref_node, Idx bkref_str)
{
    ...
    err =
        check_arrival(mctx, &sub_last->path, sub_last->node, sub_last->str_idx,
                      bkref_node, bkref_str, OP_OPEN_SUBEXP);
    if (err != REG_NOERROR)
        return err;
                  ...
                  Idx top_str, Idx last_node, Idx last_str, int type)
{
    ...
    re_node_set_free(&next_nodes);
    cur_nodes =
        (mctx->state_log[last_str] == NULL ? NULL
...
static bool build_trtable(const re_dfa_t* dfa, re_dfastate_t* state)
{
    ...
    if (__libc_use_alloca((sizeof(re_node_set) + sizeof(bitset_t)) * SBC_MAX +
        ...
        dest_states =
            (re_dfastate_t**)alloca(ndests * 3 * sizeof(re_dfastate_t*));
    else
    {
    ...
    for (i = 0; i < ndests; ++i)
    {
       ...
       build appropriate states for these contexts.  */
        if (dest_states[i]->has_constraint)
        {
            ...
            dest_states_nl[i] =
                re_acquire_state_context(&err, dfa, &follows, CONTEXT_NEWLINE);
...
static reg_errcode_t __attribute_warn_unused_result__ match_ctx_add_entry(
    re_match_context_t* mctx, Idx node, Idx str_idx, Idx from, Idx to)
{
