...
tt_glyphzone_new(FT_Memory memory, FT_UShort maxPoints, FT_Short maxContours,
                 TT_GlyphZone zone)
{
    FT_Error error;
...
#define TRICK_NAMES_COUNT 26
    ...
    static const char
        trick_names[TRICK_NAMES_COUNT][TRICK_NAMES_MAX_CHARACTERS + 1] = {
            ...
            "DFKaiShu", "DFKaiShu-Md-HK-BF", /* maybe DynaLab Inc. */
...
static FT_ULong tt_get_sfnt_checksum(TT_Face face, FT_UShort i)
{
    ...
    if (face->goto_table(face, face->dir_tables[i].Tag, face->root.stream,
                         NULL))
        return 0;
...
#define TRICK_SFNT_IDS_NUM_FACES 29
    ...
    static const tt_sfnt_id_rec
        sfnt_id[TRICK_SFNT_IDS_NUM_FACES][TRICK_SFNT_IDS_PER_FACE] = {
...
static FT_Bool tt_check_single_notdef(FT_Face ttface)
{
    ...
    if (count == 1)
    {
        ...
        else
        {
            ...
            if (!error && buf[0] == '.' && !ft_strncmp(buf, ".notdef", 8))
                result = TRUE;
        }
    }
             ...
             FT_Int face_index, FT_Int num_params, FT_Parameter* params)
{
    ...
    if (face->format_tag != 0x00010000L && /* MS fonts */
        ...
        face->format_tag !=
            TTAG_0xA5lst) /* `LastResort.dfont' (legacy Mac OS X) */
    {
...
#endif
        {
            if (ttface->num_fixed_sizes && face->glyph_locations &&
...

    {
        ...
        if (FT_HAS_MULTIPLE_MASTERS(ttface) && instance_index > 0)
        {
...
tt_size_run_fpgm(TT_Size size, FT_Bool pedantic)
{
    ...
    TT_Set_CodeRange(exec, tt_coderange_font, face->font_program,
...
tt_size_run_prep(TT_Size size, FT_Bool pedantic)
{
    ...
    TT_Set_CodeRange(exec, tt_coderange_cvt, face->cvt_program,
...
static FT_Error tt_size_init_bytecode(FT_Size ftsize, FT_Bool pedantic)
{
    FT_Error error;
    TT_Size size = (TT_Size)ftsize;
    TT_Face face = (TT_Face)ftsize->face;
    ...
    /* set `face->interpreter' according to the debug hook present */
    {
        ...
        face->interpreter =
            (TT_Interpreter)library->debug_hooks[FT_DEBUG_HOOK_TRUETYPE];
        if (!face->interpreter)
    ...
    *size_metrics = size->root.metrics;
    ...
    if (face->header.Flags & 8)
    {
        ...
        size_metrics->ascender =
            FT_PIX_ROUND(FT_MulFix(face->root.ascender, size_metrics->y_scale));
        size_metrics->descender = FT_PIX_ROUND(
            FT_MulFix(face->root.descender, size_metrics->y_scale));
        size_metrics->height =
            FT_PIX_ROUND(FT_MulFix(face->root.height, size_metrics->y_scale));
    }
    ...
    if (face->header.Flags & 8)
    {
        ...
        size_metrics->x_scale =
            FT_DivFix(size_metrics->x_ppem << 6, face->root.units_per_EM);
        size_metrics->y_scale =
            FT_DivFix(size_metrics->y_ppem << 6, face->root.units_per_EM);

        size_metrics->max_advance = FT_PIX_ROUND(
            FT_MulFix(face->root.max_advance_width, size_metrics->x_scale));
    }
    ...
    if (size_metrics->x_ppem >= size_metrics->y_ppem)
    {
        ...
        size->ttmetrics.y_ratio =
            FT_DivFix(size_metrics->y_ppem, size_metrics->x_ppem);
    }
    else
    {
        ...
        size->ttmetrics.x_ratio =
            FT_DivFix(size_metrics->x_ppem, size_metrics->y_ppem);
