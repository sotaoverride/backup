...
static const char LinkTarget[] = {0x13, 0x03, 'C', 'I', 'S'};
static const char DataOrg[] = {0x46, 0x39, 0x00, 'F', 'T',
                               'L',  '1',  '0',  '0', 0x00};
                         ...
                         u_int BootSize)
{
    ...
    hdr->NumEraseUnits =
        cpu_to_le16((RegionSize - BootSize) >> hdr->EraseUnitSize);
    ...
    nbam = ((1 << (hdr->EraseUnitSize - hdr->BlockSize)) * sizeof(u_int) +
            le32_to_cpu(hdr->BAMOffset) + (1 << hdr->BlockSize) - 1) >>
           hdr->BlockSize;
...
static int format_partition(int fd, int quiet, int interrogate, u_int spare,
                            int reserve, u_int bootsize)
{
    ...
    if (interrogate)
        if (!prompt("This will destroy all data on the target device. Confirm?",
                    false))
            return -1;
    ...
    step = spare ? (le16_to_cpu(hdr.NumEraseUnits) / spare)
                 : (le16_to_cpu(hdr.NumEraseUnits) + 1);
    for (i = 0; i < le16_to_cpu(hdr.NumEraseUnits); i++)
    {
        off_t ofs = (off_t)(i + le16_to_cpu(hdr.FirstPhysicalEUN))
                    << hdr.EraseUnitSize;
...
int main(int argc, char* argv[])
{
    ...
    while ((optch = getopt_long(argc, argv, short_opts, long_opts, &c)) != -1)
    {
        switch (optch)
        {
            ...
            case 'i':
                ...
                break;
            case 's':
                spare = strtoul(optarg, NULL, 0);
                break;
            case 'r':
                reserve = strtoul(optarg, NULL, 0);
                break;
            case 'b':
    ...
    ret = format_partition(fd, quiet, interrogate, spare, reserve, bootsize);
    if (!quiet)
    {
        if (ret)
