...
static OM_uint32 val_acq_cred_args(OM_uint32* minor_status,
                                   gss_name_t desired_name, OM_uint32 time_req,
...
OM_uint32 KRB5_CALLCONV gss_acquire_cred(minor_status, desired_name, time_req,
...
OM_uint32 KRB5_CALLCONV gss_acquire_cred_from(minor_status, desired_name,
...

{
    ...
    major = val_acq_cred_args(minor_status, desired_name, time_req,
    ...
    for (i = 0, major = GSS_S_UNAVAILABLE; i < mechs->count; i++)
    {
        major = gss_add_cred_from(
            &tmpMinor, (gss_cred_id_t)creds, desired_name, &mechs->elements[i],
            cred_usage, time_req, time_req, cred_store, NULL, NULL,
            time_rec ? &initTimeOut : NULL, time_rec ? &acceptTimeOut : NULL);
        if (major == GSS_S_COMPLETE)
        {
            ...
            else
            {
                ...
                if (initTimeOut > acceptTimeOut)
                    outTime =
                        (outTime > acceptTimeOut) ? acceptTimeOut : outTime;
                else
...
static OM_uint32 val_add_cred_args(
    ...
    gss_const_key_value_set_t cred_store, OM_uint32 initiator_time_req,
    ...
    OM_uint32* acceptor_time_rec)
{
    ...
    if (input_cred_handle == GSS_C_NO_CREDENTIAL && output_cred_handle == NULL)
...
static OM_uint32 copy_mech_cred(OM_uint32* minor_status, gss_cred_id_t cred_in,
                                gss_OID mech_oid, gss_cred_id_t* cred_out)
{
    ...
    else if (mech->gss_inquire_cred != NULL && mech->gss_acquire_cred != NULL)
    {
...
static OM_uint32 copy_union_cred(OM_uint32* minor_status, gss_cred_id_t cred_in,
...
/* V2 KRB5_CALLCONV */
OM_uint32 KRB5_CALLCONV gss_add_cred(minor_status, input_cred_handle,
                                     desired_name, desired_mech, cred_usage,
                                     initiator_time_req, acceptor_time_req,
                                     output_cred_handle, actual_mechs,
                                     initiator_time_rec,
                                     acceptor_time_rec) OM_uint32* minor_status;
...
OM_uint32* acceptor_time_rec;
{
    return gss_add_cred_from(
        minor_status, input_cred_handle, desired_name, desired_mech, cred_usage,
        initiator_time_req, acceptor_time_req, NULL, output_cred_handle,
        actual_mechs, initiator_time_rec, acceptor_time_rec);
}

OM_uint32 KRB5_CALLCONV gss_add_cred_from(
    minor_status, input_cred_handle, desired_name, desired_mech, cred_usage,
    initiator_time_req, acceptor_time_req, cred_store, output_cred_handle,
...
OM_uint32* acceptor_time_rec;
{
    ...
    status = val_add_cred_args(
        minor_status, input_cred_handle, desired_name, desired_mech, cred_usage,
        cred_store, initiator_time_req, acceptor_time_req, output_cred_handle,
        actual_mechs, initiator_time_rec, acceptor_time_rec);
    if (status != GSS_S_COMPLETE)
        return (status);

    status =
        gssint_select_mech_type(minor_status, desired_mech, &selected_mech);
    if (status != GSS_S_COMPLETE)
        return (status);
    ...
    {
        status = mech->gss_acquire_cred_from(
            minor_status, internal_name, time_req, &target_mechs, cred_usage,
            cred_store, &cred, NULL, time_recp);
    }
    ...
    {
        status =
            gssint_make_public_oid_set(minor_status, union_cred->mechs_array,
                                       union_cred->count + 1, actual_mechs);
        if (GSS_ERROR(status))
            goto errout;
    }
