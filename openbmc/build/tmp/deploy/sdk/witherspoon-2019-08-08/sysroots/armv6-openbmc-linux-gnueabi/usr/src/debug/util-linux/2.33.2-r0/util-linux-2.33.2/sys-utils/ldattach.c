...
static const struct ld_table ld_discs[] = {{"TTY", N_TTY},
                                           {"SLIP", N_SLIP},
                                           {"MOUSE", N_MOUSE},
                                           {"PPP", N_PPP},
                                           {"STRIP", N_STRIP},
...
int main(int argc, char** argv)
{
    ...
    while ((optc = getopt_long(argc, argv, "dhV78neo12s:i:c:p:", opttbl,
                               NULL)) >= 0)
    {
        switch (optc)
        {
            ...
            case 'p':
                intropause =
                    strtou32_or_err(optarg, _("invalid pause argument"));
                if (intropause > 10)
    ...
    if (ldisc < 0)
        ldisc = strtos32_or_err(argv[optind],
    ...
    dbg("set to raw %d %c%c%c: cflag=0x%x", speed, bits, parity, stop,
        ts.c_cflag);
    ...
    if (introparm && *introparm)
    {
        ...
        if (write_all(tty_fd, introparm, strlen(introparm)) != 0)
            err(EXIT_FAILURE, _("cannot write intro command to %s"), dev);
