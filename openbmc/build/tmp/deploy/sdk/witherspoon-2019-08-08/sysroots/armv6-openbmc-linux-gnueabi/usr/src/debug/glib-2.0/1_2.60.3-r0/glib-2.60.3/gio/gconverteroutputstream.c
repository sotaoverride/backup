...
static void g_converter_output_stream_pollable_iface_init(
                                             ...
                                             GConverter* converter)
{
    ...
    stream = g_object_new(G_TYPE_CONVERTER_OUTPUT_STREAM, "base-stream",
...
static gboolean flush_buffer(GConverterOutputStream* stream, gboolean blocking,
                             GCancellable* cancellable, GError** error)
{
    ...
    if (available > 0)
    {
        res = g_pollable_stream_write_all(
            base_stream, buffer_data(&priv->converted_buffer), available,
            blocking, &nwritten, cancellable, error);
...
static gssize write_internal(GOutputStream* stream, const void* buffer,
                             gsize count, gboolean blocking,
                             GCancellable* cancellable, GError** error)
{
    GConverterOutputStream* cstream;
    GConverterOutputStreamPrivate* priv;
    ...
    while (!priv->finished && converted_bytes < to_convert_size)
    {
        ...
        my_error = NULL;
        res = g_converter_convert(priv->converter, to_convert + converted_bytes,
                                  to_convert_size - converted_bytes,
                                  buffer_data(&priv->converted_buffer) +
                                      buffer_data_size(&priv->converted_buffer),
                                  buffer_tailspace(&priv->converted_buffer), 0,
                                  &bytes_read, &bytes_written, &my_error);
        ...
        else
        {
            ...
            if (g_error_matches(my_error, G_IO_ERROR, G_IO_ERROR_NO_SPACE))
            {
                 ...
                 * Note: if we actually grow the buffer (as opposed to
                 * compacting it), this will double the size, not just add one
                 * byte. */
            ...
            {
                /* We got an conversion error, but we did convert some bytes
               before that, so handle those before reporting the error */
                g_error_free(my_error);
                break;
            }
                                                ...
                                                GError** error)
{
    ...
    while (!priv->finished && !flushed)
    {
        ...
        res = g_converter_convert(
            priv->converter, buffer_data(&priv->output_buffer),
    ...
    g_converter_output_stream_can_poll(GPollableOutputStream* stream)
{
    ...
    return (G_IS_POLLABLE_OUTPUT_STREAM(base_stream) &&
            g_pollable_output_stream_can_poll(
                G_POLLABLE_OUTPUT_STREAM(base_stream)));
}
...
static gssize
    g_converter_output_stream_write_nonblocking(GPollableOutputStream* stream,
                                                ...
                                                GError** error)
{
    return write_internal(G_OUTPUT_STREAM(stream), buffer, count, FALSE, NULL,
                          error);
}
                                            ...
                                            GCancellable* cancellable)
{
    ...
    base_source = g_pollable_output_stream_create_source(
        G_POLLABLE_OUTPUT_STREAM(base_stream), NULL);
    pollable_source =
