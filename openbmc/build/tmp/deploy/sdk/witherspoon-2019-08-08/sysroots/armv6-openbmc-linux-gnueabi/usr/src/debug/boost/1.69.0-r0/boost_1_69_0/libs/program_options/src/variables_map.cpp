...
void store(const parsed_options& options, variables_map& xm, bool utf8)
{
    ...
    for (i = 0; i < all.size(); ++i)
    {
        ...
        if (d.semantic()->is_required())
        {
            ...
            // For option names specified in multiple ways, e.g. on the command
            // line, config file etc, the following precedence rules apply:
            //  "--"  >  ("-" or "/")  >  ""
...
abstract_variables_map::abstract_variables_map() : m_next(0)
{
}
...
const variable_value& abstract_variables_map::
