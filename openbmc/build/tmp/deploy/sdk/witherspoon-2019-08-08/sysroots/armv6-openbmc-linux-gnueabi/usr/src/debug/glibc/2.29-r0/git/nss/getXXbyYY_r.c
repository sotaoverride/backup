...
#ifndef DEEPCOPY_FN
static inline int __copy_einval(LOOKUP_TYPE a, const size_t b, LOOKUP_TYPE* c,
                                char* d, char** e)
{
    return EINVAL;
}
...
#ifndef MERGE_FN
static inline int __merge_einval(LOOKUP_TYPE* a, char* b, char* c, size_t d,
                                 LOOKUP_TYPE* e, char* f)
{
    return EINVAL;
}
...
libc_hidden_proto(DB_LOOKUP_FCT)
    ...
    int INTERNAL(REENTRANT_NAME)(ADD_PARAMS, LOOKUP_TYPE* resbuf, char* buffer,
                                 size_t buflen,
                                 LOOKUP_TYPE** result H_ERRNO_PARM EXTRA_PARAMS)
{
    ...
    switch (__nss_hostname_digits_dots(name, resbuf, &buffer, NULL, buflen,
                                       result, &status, AF_VAL, H_ERRNO_VAR_P))
    {
        case -1:
#ifdef NEED__RES
        ...
        if (do_merge)
        {
            ...
            else
            {
                /* If the result wasn't SUCCESS, copy the saved buffer back
                   ...
                   loop.
                    * If the next action is CONTINUE, it will overwrite the
                   value currently in the buffer and return the new value.
    ...
    *result = status == NSS_STATUS_SUCCESS ? resbuf : NULL;
    ...
    if (status == NSS_STATUS_UNAVAIL && !any_service && errno != ENOENT)
        /* This happens when we weren't able to use a service for reasons other
           than the module not being found.  In such a case, we'd want to tell
           the caller that errno has the real reason for failure.  */
        *h_errnop = NETDB_INTERNAL;
...
{
    int ret = INTERNAL(REENTRANT_NAME)(ADD_VARIABLES, resbuf, buffer, buflen,
                                       result H_ERRNO_VAR);
