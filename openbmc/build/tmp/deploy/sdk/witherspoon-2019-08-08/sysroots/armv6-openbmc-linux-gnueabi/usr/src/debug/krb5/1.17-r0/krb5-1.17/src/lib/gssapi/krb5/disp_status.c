...
static inline int compare_OM_uint32(OM_uint32 a, OM_uint32 b)
{
    if (a < b)
        return -1;
    else if (a == b)
...
#ifdef DEBUG
    fprintf(stderr, "%s(%lu, %s)", __FUNCTION__, (unsigned long)minor_code,
            msg);
#endif
...
OM_uint32 KRB5_CALLCONV krb5_gss_display_status(
    minor_status, status_value, status_type, mech_type, message_context,
    status_string) OM_uint32* minor_status;
