...
class VTerm
{
  public:
    struct CharAttr
    {
        ...
        {
            return fcolor != a.fcolor || bcolor != a.bcolor ||
                   intensity != a.intensity || italic != a.italic ||
                   underline != a.underline || blink != a.blink ||
                   reverse != a.reverse;
        }
    ...
    u16 charCode(u16 x, u16 y)
    {
        return text[get_line(y) * max_width + x];
    }
    CharAttr charAttr(u16 x, u16 y)
    {
        return attrs[get_line(y) * max_width + x];
    }

    static s32 charWidth(u32 ucs);

  protected:
    virtual void drawChars(CharAttr attr, u16 x, u16 y, u16 w, u16 num,
                           u16* chars, bool* dws) = 0;
  ...
  private:
    ...
    void draw_cursor();
    u16 get_line(u16 y);
    u16 total_history_lines()
    {
        return history_full ? history_lines : history_save_line;
    }
...
#define MAX_ESCAPE_CODE 128
    ...
    static u8 control_map[MAX_CONTROL_CODE],
        escape_map[NR_STATES][MAX_ESCAPE_CODE];
