                        ...
                        int (*comp)(void*, void*))
{
    ...
    dlist_for_each_nomark(
        list,
        nodepointer) if (comp(target,
                              nodepointer->data)) return (nodepointer->data);
    return (NULL);
}
...
{
    struct dl_node* nodepointer;
    dlist_for_each_nomark(list, nodepointer) node_operation(nodepointer->data);
}
                          ...
                          int (*sorter)(void*, void*))
{
    for (dlist_start(list), dlist_next(list);
         list->marker != list->head && !sorter(new, list->marker->data);
         dlist_next(list))
        ;
