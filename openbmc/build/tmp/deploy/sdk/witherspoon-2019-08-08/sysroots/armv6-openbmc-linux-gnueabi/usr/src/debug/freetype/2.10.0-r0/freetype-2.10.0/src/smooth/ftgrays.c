...
#define ADD_LONG(a, b) (long)((unsigned long)(a) + (unsigned long)(b))
#define SUB_LONG(a, b) (long)((unsigned long)(a) - (unsigned long)(b))
#define MUL_LONG(a, b) (long)((unsigned long)(a) * (unsigned long)(b))
#define NEG_LONG(a) (long)(-(unsigned long)(a))
...
int FT_Throw(int error, int line, const char* file)
{
    FT_UNUSED(error);
    FT_UNUSED(line);
    FT_UNUSED(file);
...
#define FT_THROW(e)                                                            \
    (FT_Throw(FT_ERR_CAT(ErrRaster_, e), __LINE__, __FILE__) |                 \
...
#define FT_DIV_MOD(type, dividend, divisor, quotient, remainder)               \
    FT_BEGIN_STMNT(quotient) = (type)((dividend) / (divisor));                 \
...
#define FT_UDIVPREP(c, b)                                                      \
    long b##_r = c ? (long)(FT_ULONG_MAX >> PIXEL_BITS) / (b) : 0
#define FT_UDIV(a, b)                                                          \
 ...
 */
static void gray_render_scanline(RAS_ARG_ TCoord ey, TPos x1, TCoord y1,
                                 TPos x2, TCoord y2)
{
...
static int gray_conic_to(const FT_Vector* control, const FT_Vector* to,
                         gray_PWorker worker)
{
...
static int gray_cubic_to(const FT_Vector* control1, const FT_Vector* control2,
                         const FT_Vector* to, gray_PWorker worker)
{
    gray_render_cubic(RAS_VAR_ control1, control2, to);
    return 0;
}

static void gray_hline(RAS_ARG_ TCoord x, TCoord y, TArea coverage,
                       TCoord acount)
{
            ...
            switch (tag)
            {
                ...
                Do_Conic:
                    ...
                    FT_TRACE5(("  conic to (%.2f, %.2f)"
                               ...
                               v_control.x / 64.0, v_control.y / 64.0));
                    error =
                        func_interface->conic_to(&v_control, &v_start, user);
                    goto Close;
                ...
                default: /* FT_CURVE_TAG_CUBIC */
                {
                    ...
                    if (point <= limit)
                    {
                        ...
                        FT_TRACE5(
                            ("  cubic to (%.2f, %.2f)"
                             ...
                             vec.x / 64.0, vec.y / 64.0, vec1.x / 64.0,
                             vec1.y / 64.0, vec2.x / 64.0, vec2.y / 64.0));
                        error =
                            func_interface->cubic_to(&vec1, &vec2, &vec, user);
                        if (error)
                            goto Exit;
                        continue;
                    }
                    ...
                    FT_TRACE5(("  cubic to (%.2f, %.2f)"
                               " with controls (%.2f, %.2f) and (%.2f, %.2f)\n",
                               v_start.x / 64.0, v_start.y / 64.0,
                               vec1.x / 64.0, vec1.y / 64.0, vec2.x / 64.0,
                               vec2.y / 64.0));
                    error =
                        func_interface->cubic_to(&vec1, &vec2, &v_start, user);
                    goto Close;
                }
            }
        }
...
static int gray_convert_glyph_inner(RAS_ARG, int continued)
{
    ...
    if (ft_setjmp(ras.jump_buffer) == 0)
    {
        ...
        FT_TRACE7(("band [%d..%d]: %d cell%s\n", ras.min_ey, ras.max_ey,
                   ras.num_cells, ras.num_cells == 1 ? "" : "s"));
    }
    else
    {
...
static void gray_raster_reset(FT_Raster raster, unsigned char* pool_base,
...
static int gray_raster_set_mode(FT_Raster raster, unsigned long mode,
                                void* args)
{
    FT_UNUSED(raster);
    FT_UNUSED(mode);
