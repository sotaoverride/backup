...
static int get_hwcap(unsigned int* hwcap, unsigned int* hwcap2)
{
    struct
    {
...
static unsigned int detect_arm_proc_cpuinfo(unsigned int* broken_hwfs)
{
    ...
    struct
    {
        ...
        {"CPU implementer", &cpu_implementer}, {"CPU architecture", &cpu_arch},
        {"CPU variant", &cpu_variant},         {"CPU part", &cpu_part},
     ...
     */
    if (cpu_implementer == 0x51 && cpu_arch == 7 && cpu_variant == 1 &&
        cpu_part == 0x4d && cpu_revision == 0)
    {
