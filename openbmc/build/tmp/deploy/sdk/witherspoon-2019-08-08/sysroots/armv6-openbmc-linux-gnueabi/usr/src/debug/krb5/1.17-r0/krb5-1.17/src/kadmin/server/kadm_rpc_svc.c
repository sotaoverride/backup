...
void kadm_1(rqstp, transp) struct svc_req* rqstp;
SVCXPRT* transp;
{
    union
    {
...
static int check_rpcsec_auth(struct svc_req* rqstp)
{
    gss_ctx_id_t ctx;
    krb5_context kctx;
...
int gss_to_krb5_name_1(struct svc_req* rqstp, krb5_context ctx,
                       ...
                       gss_buffer_t gss_str)
{
    ...
    {
        krb5_klog_syslog(LOG_ERR,
                         _("gss_to_krb5_name: failed display_name "
                           "status %d"),
                         status);
