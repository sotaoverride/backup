...
extern uint64_t elapsed_time(struct timeval* start, struct timeval* stop);
...
/* Dump simple counter */
extern char* dump_stat_counter(char* buf, int size, const char* msg,
                               uint64_t value);

/* Dump min-max time stat */
extern char* dump_stat_stat(char* buf, int size, const char* msg,
                            const char* unit, struct rng_stat* stat);

/*
 * Dump min-max speed stat, base time unit is a microsecond
 */
extern char* dump_stat_bw(char* buf, int size, const char* msg,
                          const char* unit, struct rng_stat* stat,
                          uint64_t blocksize);
