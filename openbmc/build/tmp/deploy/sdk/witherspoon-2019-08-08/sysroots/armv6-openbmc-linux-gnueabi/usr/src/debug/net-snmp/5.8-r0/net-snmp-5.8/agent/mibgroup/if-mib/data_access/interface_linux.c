...
netsnmp_feature_require(fd_event_manager)
    netsnmp_feature_require(delete_prefix_info)
        netsnmp_feature_require(create_prefix_info)
            netsnmp_feature_child_of(interface_arch_set_admin_status,
                                     interface_all)
...
static void netsnmp_pci_error(char* msg, ...)
{
    va_list args;
    char* buf;
    int buflen;
...
#define PROC_SYS_NET_IPVx_NEIGH_RETRANS_TIME_MS                                \
    "/proc/sys/net/ipv%d/neigh/%s/retrans_time_ms"
#define PROC_SYS_NET_IPVx_NEIGH_RETRANS_TIME                                   \
    "/proc/sys/net/ipv%d/neigh/%s/retrans_time"
static const char* proc_sys_retrans_time;
static unsigned short retrans_time_factor = 1;

#define PROC_SYS_NET_IPVx_BASE_REACHABLE_TIME_MS                               \
    "/proc/sys/net/ipv%d/neigh/%s/base_reachable_time_ms"
#define PROC_SYS_NET_IPVx_BASE_REACHABLE_TIME                                  \
    "/proc/sys/net/ipv%d/neigh/%s/base_reachable_time"
...
void netsnmp_arch_interface_init(void)
{
    ...
    snprintf(proc_path, sizeof(proc_path),
             PROC_SYS_NET_IPVx_BASE_REACHABLE_TIME_MS, 6, "default");
    snprintf(proc_path2, sizeof(proc_path),
             PROC_SYS_NET_IPVx_BASE_REACHABLE_TIME, 4, "default");
                              ...
                              netsnmp_container* addr_container)
{
    ...
    *flags &= ~NETSNMP_INTERFACE_FLAGS_HAS_IPV6;
    ...
    if (NULL == addr_container)
    {
        ...
        addr_container =
            netsnmp_access_ipaddress_container_load(NULL, addr_container_flags);
        if (NULL == addr_container)
        {
    ...
    {
        DEBUGMSGTL(
            ("access:ifcontainer", "couldn't get ip addresses iterator\n"));
 ...
 * @internal
 */
static void _arch_interface_flags_v4_get(netsnmp_interface_entry* entry)
{
    FILE* fin;
    char line[256];
     ...
     */
    snprintf(line, sizeof(line), proc_sys_retrans_time, 4, entry->name);
    if (!(fin = fopen(line, "r")))
    {
        DEBUGMSGTL(("access:interface", "Failed to open %s\n", line));
    }
    else
    {
 ...
 */
static void _arch_interface_description_get(netsnmp_interface_entry* entry)
{
    const char* descr;
    char buf[256];
    ...
    snprintf(buf, sizeof(buf), "/sys/class/net/%s/device/vendor", entry->name);

    if (!sysfs_get_id(buf, &vendor_id))
        return;

    snprintf(buf, sizeof(buf), "/sys/class/net/%s/device/device", entry->name);

    if (!sysfs_get_id(buf, &device_id))
        return;

    descr = pci_lookup_name(pci_access, buf, sizeof(buf),
                            PCI_LOOKUP_VENDOR | PCI_LOOKUP_DEVICE, vendor_id,
                            device_id, 0, 0);
    if (descr)
    {
        free(entry->descr);
 ...
 * @internal
 */
static void _arch_interface_flags_v6_get(netsnmp_interface_entry* entry)
{
    FILE* fin;
    char line[256];
     ...
     * get the retransmit time
     */
    snprintf(line, sizeof(line), proc_sys_retrans_time, 6, entry->name);
    if (!(fin = fopen(line, "r")))
    {
        DEBUGMSGTL(("access:interface", "Failed to open %s\n", line));
    }
        ...
        if (fgets(line, sizeof(line), fin))
        {
            ...
            else
            {
                entry->reachable_time =
                    atoi(line) * 1000; /* sec to  millisec */
            }
 ...
 * @internal
 */
static int _parse_stats(netsnmp_interface_entry* entry, char* stats,
                        int expected)
{
    /*
     * scanline_2_2:
    ...
    const char* scan_line_2_2 =
        "%" SCNuMAX " %" SCNuMAX " %" SCNuMAX " %" SCNuMAX " %*" SCNuMAX
        " %*" SCNuMAX " %*" SCNuMAX " %" SCNuMAX " %" SCNuMAX " %" SCNuMAX
    ...
    {
        scan_count = sscanf(stats, scan_line_to_use, &rec_oct, &rec_pkt,
                            &rec_err, &rec_drop, &rec_mcast, &snd_oct, &snd_pkt,
                            &snd_err, &snd_drop, &coll);
    ...
    {
        scan_count = sscanf(stats, scan_line_to_use, &rec_pkt, &rec_err,
 ...
 */
int netsnmp_arch_interface_container_load(netsnmp_container* container,
                                          u_int load_flags)
{
    FILE* devin;
    char line[256];
    ...
    DEBUGMSGTL(
        ("access:interface:container:arch", "load (flags %x)\n", load_flags));
    ...
    {
        DEBUGMSGTL(
            ("access:interface", "Failed to load Interface Table (linux1)\n"));
    ...
    if (0 == scan_expected)
    {
        if (strstr(line, "compressed"))
        {
            scan_expected = 10;
            DEBUGMSGTL(
                ("access:interface", "using linux 2.2 kernel /proc/net/dev\n"));
        }
        else
        {
            scan_expected = 5;
            DEBUGMSGTL(
                ("access:interface", "using linux 2.0 kernel /proc/net/dev\n"));
        }
    }
        ...
        *stats++ = 0; /* null terminate name */
        ...
        if (NULL == entry)
        {
            ...
            netsnmp_access_interface_container_free(
                container, NETSNMP_ACCESS_INTERFACE_FREE_NOFLAGS);
            fclose(devin);
            close(fd);
            return -3;
        }
        ...
        if (0 == entry->type)
        {
            ...
            static match_if lmatch_if[] = {
                ...
                {IANAIFTYPE_PROPVIRTUAL,
                 "bond"}, /* Bonding driver find fastest slave */
                                  ...
                                  int ifAdminStatus_val)
{
    ...
    return netsnmp_access_interface_ioctl_flags_set(-1, entry, IFF_UP,
                                                    and_complement);
}
...
#endif
{
    ...
    const unsigned long long media_speeds[] = {10000000,  10000000, 100000000,
    ...
    {
        DEBUGMSGTL(
            ("mibII/interfaces", "SIOCGMIIPHY on %s failed\n", ifr.ifr_name));
        return retspeed;
    }
...
void netsnmp_prefix_process(int fd, void* data)
{
    ...
    for (nlmp = (struct nlmsghdr*)buf; status > sizeof(*nlmp);)
    {
        ...
        if (nlmp->nlmsg_type == RTM_NEWADDR || nlmp->nlmsg_type == RTM_DELADDR)
        {
            ...
            if (length < 0)
            {
                DEBUGMSGTL(("access:interface:prefix",
                            "wrong nlmsg length %d\n", length));
                continue;
            }
            ...
            if (!ifa->ifa_flags)
            {
                ...
                while (RTA_OK(rtatp, length))
                {
                    if (rtatp->rta_type == IFA_ADDRESS)
                    {
                        ...
                        {
                            snprintf(in6pAddr, sizeof(in6pAddr),
    ...
    if ((have_addr == 1) && (have_prefix == 1))
    {
        if (!(new = net_snmp_create_prefix_info(onlink, autonomous, in6pAddr)))
            DEBUGMSGTL(
                ("access:interface:prefix", "Unable to create prefix info\n"));
        else
        {

            iret =
                net_snmp_search_update_prefix_info(list_info.list_head, new, 0);
            if (iret < 0)
            {
                DEBUGMSGTL(("access:interface:prefix",
                            "Unable to add/update prefix info\n"));
                free(new);
            }
            if (iret ==
                2) /*Only when enrty already exists and we are only updating*/
                free(new);
        }
    ...
    else if (have_addr == -1)
    {
        ...
        if (iret < 0)
            DEBUGMSGTL(("access:interface:prefix",
                        "Unable to delete the prefix info\n"));
        if (!iret)
            DEBUGMSGTL(("access:interface:prefix",
                        "Unable to find the node to delete\n"));
        have_addr = 0;
    }
}
#endif
