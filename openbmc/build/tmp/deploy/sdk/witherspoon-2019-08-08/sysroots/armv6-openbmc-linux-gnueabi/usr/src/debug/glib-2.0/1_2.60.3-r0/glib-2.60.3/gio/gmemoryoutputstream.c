...
struct _GMemoryOutputStreamPrivate
{
    ...
    gsize
        len; /* Current length of the data buffer. Can change with resizing. */
...
static void g_memory_output_stream_set_property(GObject* object, guint prop_id,
                                                const GValue* value,
                                                GParamSpec* pspec);
static void g_memory_output_stream_get_property(GObject* object, guint prop_id,
                                                GValue* value,
                                                GParamSpec* pspec);
...
static gboolean g_memory_output_stream_seek(GSeekable* seekable, goffset offset,
                                            GSeekType type,
                                            GCancellable* cancellable,
                                            GError** error);
...
static void g_memory_output_stream_pollable_iface_init(
...
static void g_memory_output_stream_class_init(GMemoryOutputStreamClass* klass)
{
    ...
    g_object_class_install_property(
        ...
        g_param_spec_ulong("size", P_("Data Buffer Size"),
                           P_("Current size of the data buffer."), 0,
                           G_MAXULONG, 0,
                           G_PARAM_READWRITE | G_PARAM_CONSTRUCT_ONLY |
                               G_PARAM_STATIC_STRINGS));
     ...
     **/
    g_object_class_install_property(
        gobject_class, PROP_DATA_SIZE,
        g_param_spec_ulong("data-size", P_("Data Size"),
                           P_("Size of data written to the buffer."), 0,
                           G_MAXULONG, 0,
     ...
     **/
    g_object_class_install_property(
        gobject_class, PROP_REALLOC_FUNCTION,
        g_param_spec_pointer(
            "realloc-function", P_("Memory Reallocation Function"),
     ...
     **/
    g_object_class_install_property(
        gobject_class, PROP_DESTROY_FUNCTION,
        g_param_spec_pointer("destroy-function",
                             P_("Destroy Notification Function"),
                             P_("Function called with the buffer as argument "
 ...
 * @realloc_function: (nullable): a function with realloc() semantics (like
 *g_realloc()) to be called when @data needs to be grown, or %NULL
 * @destroy_function: (nullable): a function to be called on @data when the
 *stream is finalized, or %NULL
 *
 ...
 **/
GOutputStream* g_memory_output_stream_new(gpointer data, gsize size,
                                          GReallocFunc realloc_function,
                                          GDestroyNotify destroy_function)
{
    GOutputStream* stream;

    stream = g_object_new(G_TYPE_MEMORY_OUTPUT_STREAM, "data", data, "size",
                          size, "realloc-function", realloc_function,
                          "destroy-function", destroy_function, NULL);
...
gpointer g_memory_output_stream_steal_data(GMemoryOutputStream* ostream)
{
    ...
    g_return_val_if_fail(G_IS_MEMORY_OUTPUT_STREAM(ostream), NULL);
    g_return_val_if_fail(g_output_stream_is_closed(G_OUTPUT_STREAM(ostream)),
                         NULL);
...
GBytes* g_memory_output_stream_steal_as_bytes(GMemoryOutputStream* ostream)
{
    ...
    result = g_bytes_new_with_free_func(
        ostream->priv->data, ostream->priv->valid_len, ostream->priv->destroy,
        ostream->priv->data);
    ostream->priv->data = NULL;
...
static gboolean array_resize(GMemoryOutputStream* ostream, gsize size,
                             gboolean allow_partial, GError** error)
{
    GMemoryOutputStreamPrivate* priv;
    gpointer data;
    gsize len;
    ...
    if (!priv->realloc_fn)
    {
        if (allow_partial && priv->pos < priv->len)
