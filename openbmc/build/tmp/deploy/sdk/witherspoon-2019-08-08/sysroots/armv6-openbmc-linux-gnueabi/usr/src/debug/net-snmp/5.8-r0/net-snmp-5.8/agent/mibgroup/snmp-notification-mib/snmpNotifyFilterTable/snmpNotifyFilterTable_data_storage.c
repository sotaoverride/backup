...
static void _snmpNotifyFilter_parse(const char* token, char* buf);

static void _nf_free_item(void* data, void* dontcare)
{
...
netsnmp_container* snmpNotifyFilter_storage_container_create(void)
{
    if (NULL == _container)
    {
        _container = netsnmp_container_find("snmpNotifyFilter:table_container");
        if (NULL == _container)
        {
...
void init_snmpNotifyFilterTable_data_storage(void)
{
    ...
    register_config_handler(NULL, "notificationFilter", _snmpNotifyFilter_parse,
                            NULL, NULL);
}
...
void shutdown_snmpNotifyFilterTable_data_storage(void)
{
    ...
    _container = NULL;
    DEBUGMSGTL(
        ("trap:notifyFilter:storage:shutdown", "active count %d\n", _active));
    if (_active != 0)
    {
        DEBUGMSGTL(("trap:notifyFilter:storage:shutdown",
                    "unexpected count %d after cleanup!\n", _active));
        snmp_log(LOG_WARNING, "notifyFilter count %d, not 0, after shutdown.\n",
                 _active);
    }
}
                                    ...
                                    const oid* subtree, size_t subtree_len)
{
    ...
    {
        DEBUGMSGTL(("snmpNotifyFilter:storage:create", "index(es) too long\n"));
        return NULL;
    }
...
void snmpNotifyFilter_storage_dispose(snmpNotifyFilter_data_storage* data)
{
    free(data);
    --_active;
}

int snmpNotifyFilter_storage_insert(snmpNotifyFilter_data_storage* data)
{
    int rc, i;
    oid* pos;
...
int snmpNotifyFilter_storage_remove(snmpNotifyFilter_data_storage* data)
{
    int rc;
...
snmpNotifyFilter_data_storage* snmpNotifyFilter_storage_add(
    const u_char* profileName, size_t profileName_len, const oid* filterSubtree,
    size_t filterSubtree_len, u_char* filterMask, size_t filterMask_len,
    u_long filterType)
{
    snmpNotifyFilter_data_storage* data;
 ...
 */
struct vacm_viewEntry* snmpNotifyFilter_vacm_view_subtree(const char* profile)
{
...
static void _snmpNotifyFilter_parse(const char* token, char* buf)
{
    ...
    {
        snmp_log(LOG_ERR, "unknown token in _snmpNotifyFilter_parse\n");
        return;
    }
    ...
    buf = read_config_read_memory(
        ASN_OCTET_STR, buf, (char*)&data.snmpNotifyFilterProfileName,
    ...
    len = sizeof(type_buf);
    buf = read_config_read_memory(ASN_OCTET_STR, buf, type_buf, (size_t*)&len);
    ...
    row = snmpNotifyFilter_storage_add(
        data.snmpNotifyFilterProfileName, data.snmpNotifyFilterProfileName_len,
        data.snmpNotifyFilterSubtree, data.snmpNotifyFilterSubtree_len,
        data.snmpNotifyFilterMask, data.snmpNotifyFilterMask_len,
