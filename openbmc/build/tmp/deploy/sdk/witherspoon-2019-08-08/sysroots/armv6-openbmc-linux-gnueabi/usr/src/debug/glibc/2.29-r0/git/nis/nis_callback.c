    ...
    static void cb_prog_1(struct svc_req* rqstp, SVCXPRT* transp)
{
    ...
    switch (rqstp->rq_proc)
    {
        ...
        case CBPROC_RECEIVE:
        {
            ...
            bool_result = FALSE;
            for (i = 0; i < argument.cbproc_receive_1_arg.entries.entries_len;
                 ++i)
            {
...
static nis_error internal_nis_do_callback(struct dir_binding* bptr,
                                          netobj* cookie, struct nis_cb* cb)
{
    ...
    for (;;)
    {
        ...
        switch (i = TEMP_FAILURE_RETRY(
                    __poll(my_pollfd, svc_max_pollfd, 25 * 1000)))
        {
            case -1:
                return NIS_CBERROR;
            case 0:
...
struct nis_cb* __nis_create_callback(int (*callback)(const_nis_name,
                                                     const nis_object*,
                                                     const void*),
                                     const void* userdata, unsigned int flags)
{
    ...
    cb = (struct nis_cb*)calloc(1, sizeof(struct nis_cb) + sizeof(nis_server));
    ...
    if (asprintf(&cb->serv->ep.ep_val[0].uaddr, "%s.%d.%d",
                 inet_ntoa(sin.sin_addr), (port & 0xFF00) >> 8,
                 port & 0x00FF) < 0)
        goto failed;
