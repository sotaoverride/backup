 ...
 */
static void _access_route_entry_release(netsnmp_route_entry* entry,
                                        void* unused);
                                        ...
                                        u_int load_flags)
{
    ...
    if (NULL == container)
    {
        ...
        {
            snmp_log(LOG_ERR,
                     "no container specified/found for access_route\n");
            return NULL;
        }
    }
    ...
    if (0 != rc)
    {
        netsnmp_access_route_container_free(container,
                                            NETSNMP_ACCESS_ROUTE_FREE_NOFLAGS);
        container = NULL;
    }
                                         ...
                                         u_int free_flags)
{
    ...
    if (!(free_flags & NETSNMP_ACCESS_ROUTE_FREE_DONT_CLEAR))
    {
         ...
         */
        CONTAINER_CLEAR(
            container, (netsnmp_container_obj_func*)_access_route_entry_release,
            NULL);
    }
    else
    {
 ...
 */
netsnmp_route_entry* netsnmp_access_route_entry_create(void)
{
...
int netsnmp_access_route_entry_set(netsnmp_route_entry* entry)
{
    ...
    else if (entry->flags & NETSNMP_ACCESS_ROUTE_CHANGE)
    {
        ...
        snmp_log(LOG_ERR,
                 "netsnmp_access_route_entry_set change not supported yet\n");
        rc = -1;
    }
