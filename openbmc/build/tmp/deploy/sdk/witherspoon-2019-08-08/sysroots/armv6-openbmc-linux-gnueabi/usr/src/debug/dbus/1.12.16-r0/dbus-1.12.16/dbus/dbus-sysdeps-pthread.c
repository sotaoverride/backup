...
/* dbus-sysdeps-pthread.c Implements threads using pthreads (internal to
 * libdbus)
 *
    ...
    {                                                                          \
        ...
        {                                                                      \
            _dbus_warn_check_failed(                                           \
                "pthread function %s failed with %d %s in %s", func_name, tmp, \
                strerror(tmp), _DBUS_FUNCTION_NAME);                           \
        }                                                                      \
    } while (0)
...
{
    PTHREAD_CHECK("pthread_cond_wait",
                                                ...
                                                int timeout_milliseconds)
{
    ...
    end_time.tv_nsec =
        (time_now.tv_usec + (timeout_milliseconds % 1000) * 1000) * 1000;
...
/*
 * If we can identify the current process and/or thread, print them to stderr
 * followed by a colon.
 */
