...
static uint32_t get_literal_price(const lzma_lzma1_encoder* const coder,
                                  const uint32_t pos, const uint32_t prev_byte,
                                  const bool match_mode, uint32_t match_byte,
                                  uint32_t symbol)
{
    const probability* const subcoder =
        literal_subcoder(coder->literal, coder->literal_context_bits,
                         coder->literal_pos_mask, pos, prev_byte);
...
static inline uint32_t get_len_price(const lzma_length_encoder* const lencoder,
                                     const uint32_t len,
...
static inline uint32_t get_pure_rep_price(const lzma_lzma1_encoder* const coder,
...
static inline uint32_t get_rep_price(const lzma_lzma1_encoder* const coder,
                                     const uint32_t rep_index,
                                     const uint32_t len,
                                     const lzma_lzma_state state,
                                     const uint32_t pos_state)
...
static inline uint32_t get_dist_len_price(const lzma_lzma1_encoder* const coder,
                                          const uint32_t dist,
                                          const uint32_t len,
                                          const uint32_t pos_state)
{
...
static void fill_dist_prices(lzma_lzma1_encoder* coder)
{
    for (uint32_t dist_state = 0; dist_state < DIST_STATES; ++dist_state)
    {
        ...
        for (uint32_t dist_slot = 0; dist_slot < coder->dist_table_size;
             ++dist_slot)
            dist_slot_prices[dist_slot] = rc_bittree_price(
                coder->dist_slot[dist_state], DIST_SLOT_BITS, dist_slot);
        ...
        for (uint32_t dist_slot = DIST_MODEL_END;
            ...
            dist_slot_prices[dist_slot] +=
                rc_direct_price(((dist_slot >> 1) - 1) - ALIGN_BITS);

        // Distances in the range [0, 3] are fully encoded with
        // dist_slot, so they are used for coder->dist_prices
        // as is.
        for (uint32_t i = 0; i < DIST_MODEL_START; ++i)
            coder->dist_prices[dist_state][i] = dist_slot_prices[i];
    }
    ...
    for (uint32_t i = DIST_MODEL_START; i < FULL_DISTANCES; ++i)
    {
        ...
        const uint32_t price = rc_bittree_reverse_price(
            coder->dist_special + base - dist_slot - 1, footer_bits, i - base);
...
static void fill_align_prices(lzma_lzma1_encoder* coder)
{
    for (uint32_t i = 0; i < ALIGN_SIZE; ++i)
        coder->align_prices[i] =
            rc_bittree_reverse_price(coder->dist_align, ALIGN_BITS, i);
...
static inline void make_literal(lzma_optimal* optimal)
{
    optimal->back_prev = UINT32_MAX;
    optimal->prev_1_is_literal = false;
}

static inline void make_short_rep(lzma_optimal* optimal)
{
    optimal->back_prev = 0;
    optimal->prev_1_is_literal = false;
}
...
static void backward(lzma_lzma1_encoder* restrict coder,
                     uint32_t* restrict len_res, uint32_t* restrict back_res,
                     uint32_t cur)
{
...
static inline uint32_t helper1(lzma_lzma1_encoder* restrict coder,
                               lzma_mf* restrict mf,
                               ...
                               uint32_t* restrict len_res, uint32_t position)
{
    ...
    coder->opts[1].price =
        rc_bit_0_price(coder->is_match[coder->state][pos_state]) +
        get_literal_price(coder, position, buf[-1],
                          !is_literal_state(coder->state), match_byte,
                          current_byte);

    make_literal(&coder->opts[1]);

    const uint32_t match_price =
        rc_bit_1_price(coder->is_match[coder->state][pos_state]);
    const uint32_t rep_match_price =
        match_price + rc_bit_1_price(coder->is_rep[coder->state]);

    if (match_byte == current_byte)
    {
        const uint32_t short_rep_price =
            rep_match_price +
            get_short_rep_price(coder, coder->state, pos_state);
    ...
    for (uint32_t i = 0; i < REPS; ++i)
    {
        ...
        const uint32_t price =
            rep_match_price +
            get_pure_rep_price(coder, i, coder->state, pos_state);

        do
        {
            const uint32_t cur_and_len_price =
                price +
                get_len_price(&coder->rep_len_encoder, rep_len, pos_state);
    ...
    const uint32_t normal_match_price =
        match_price + rc_bit_0_price(coder->is_rep[coder->state]);
    ...
    if (len <= len_main)
    {
        ...
        for (;; ++len)
        {
            ...
            const uint32_t cur_and_len_price =
                normal_match_price +
                get_dist_len_price(coder, dist, len, pos_state);
...
static inline uint32_t helper2(lzma_lzma1_encoder* coder, uint32_t* reps,
                               const uint8_t* buf, uint32_t len_end,
                               ...
                               const uint32_t buf_avail_full)
{
    ...
    const uint32_t cur_and_1_price =
        cur_price + rc_bit_0_price(coder->is_match[state][pos_state]) +
        get_literal_price(coder, position, buf[-1], !is_literal_state(state),
    ...
    const uint32_t match_price =
        cur_price + rc_bit_1_price(coder->is_match[state][pos_state]);
    const uint32_t rep_match_price =
        match_price + rc_bit_1_price(coder->is_rep[state]);

    if (match_byte == current_byte && !(coder->opts[cur + 1].pos_prev < cur &&
                                        coder->opts[cur + 1].back_prev == 0))
    {

        const uint32_t short_rep_price =
            rep_match_price + get_short_rep_price(coder, state, pos_state);
    ...
    { // speed optimization
        ...
        if (len_test >= 2)
        {
            ...
            const uint32_t cur_and_len_price =
                ...
                get_rep_price(coder, 0, len_test, state_2, pos_state_next);
    ...
    for (uint32_t rep_index = 0; rep_index < REPS; ++rep_index)
    {
        ...
        for (; len_test_2 < limit && buf[len_test_2] == buf_back[len_test_2];
             ++len_test_2)
            ;
        ...
        if (len_test_2 >= 2)
        {
            ...
            const uint32_t cur_and_len_literal_price =
                ...
                get_literal_price(coder, position + len_test, buf[len_test - 1],
                                  true, buf_back[len_test], buf[len_test]);
    ...
    {
        const uint32_t normal_match_price =
            match_price + rc_bit_0_price(coder->is_rep[state]);
        ...
        for (uint32_t len_test = start_len;; ++len_test)
        {
            ...
            uint32_t cur_and_len_price =
                normal_match_price +
                get_dist_len_price(coder, cur_back, len_test, pos_state);
            ...
            if (len_test == coder->matches[i].len)
            {
                ...
                const uint32_t limit =
                    my_min(buf_avail_full, len_test_2 + nice_len);
                                     ...
                                     uint32_t position)
{
    ...
    if (coder->opts_end_index != coder->opts_current_index)
    {
        ...
        *back_res = coder->opts[coder->opts_current_index].back_prev;
        coder->opts_current_index =
            coder->opts[coder->opts_current_index].pos_prev;
        return;
    }
    ...
    for (cur = 1; cur < len_end; ++cur)
    {
        ...
        coder->longest_match_length =
            mf_find(mf, &coder->matches_count, coder->matches);

        if (coder->longest_match_length >= mf->nice_len)
            break;

        len_end =
            helper2(coder, reps, mf_ptr(mf) - 1, len_end, position + cur, cur,
                    mf->nice_len, my_min(mf_avail(mf) + 1, OPTS - 1 - cur));
    }
