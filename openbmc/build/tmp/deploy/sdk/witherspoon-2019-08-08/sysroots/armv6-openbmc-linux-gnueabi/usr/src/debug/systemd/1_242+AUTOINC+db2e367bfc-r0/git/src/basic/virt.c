...
static int detect_vm_device_tree(void)
{
    ...
    if (r == -ENOENT)
    {
        ...
        {
            log_debug("Virtualization QEMU: \"fw-cfg\" present in "
                      "/proc/device-tree/%s",
                      dent->d_name);
    ...
    log_debug(
        "Virtualization %s found in /proc/device-tree/hypervisor/compatible",
        hvtype);
...
static int detect_vm_dmi(void)
{
    ...
    static const char* const dmi_vendors[] = {
        "/sys/class/dmi/id/product_name", /* Test this before sys_vendor to
                                             detect KVM over QEMU */
    ...
    for (i = 0; i < ELEMENTSOF(dmi_vendors); i++)
    {
        ...
        for (j = 0; j < ELEMENTSOF(dmi_vendor_table); j++)
            ...
            {
                log_debug("Virtualization %s found in DMI (%s)", s,
                          dmi_vendors[i]);
...
static int detect_vm_xen_dom0(void)
{
    ...
    {
        log_debug(
            "Virtualization XEN because /proc/xen/capabilities does not exist");
        return 0;
    }
    if (r < 0)
        return r;
...
static int detect_vm_uml(void)
{
    ...
    if (!f)
    {
        ...
        {
            log_debug(
                "/proc/cpuinfo not found, assuming no UML virtualization.");
            return VIRTUALIZATION_NONE;
        }
        return -errno;
    }
...
int detect_vm(void)
{
     ...
     *
     * → First, try to detect Oracle Virtualbox, even if it uses KVM, as well as
     * Xen even if it cloaks as Microsoft Hyper-V.
     *
     * → Second, try to detect from CPUID, this will report KVM for whatever
     * software is used even if info in DMI is overwritten.
     *
...
int detect_container(void)
{
    ...
    /* /proc/vz exists in container and outside of the container, /proc/bc only
    ...
    if (getpid_cached() == 1)
    {
        /* If we are PID 1 we can just check our own environment variable, and
    ...
    /* Otherwise, PID 1 might have dropped this information into a file in /run.
     * This is better than accessing /proc/1/environ, since we don't need
     * CAP_SYS_PTRACE for that. */
    ...
    /* Fallback for cases where PID 1 was not systemd (for example, cases where
     * init=/bin/sh is used. */
    ...
    if (r < 0) /* This only works if we have CAP_SYS_PTRACE, hence let's better
                  ignore failures here */
        log_debug_errno(r, "Failed to read $container of PID 1, ignoring: %m");

    /* Interestingly /proc/1/sched actually shows the host's PID for what we see
     * as PID 1. Hence, if the PID shown
...
finish:
    log_debug("Found container virtualization %s.",
              virtualization_to_string(r));
    cached_found = r;
    return r;
}
...
static int userns_has_mapping(const char* name)
{
    ...
    if (n < 0)
    {
        if (feof(f))
        {
            log_debug("%s is empty, we're in an uninitialized user namespace",
                      name);
            return true;
        }
...
int running_in_userns(void)
{
     ...
     * possible to compile a kernel without CONFIG_USER_NS, in which case
     * "setgroups" also does not exist. We cannot distinguish those two cases,
     * so assume that we're running on a stripped-down recent kernel, rather
    ...
    log_debug("/proc/self/setgroups contains \"%s\", %s user namespace", line,
              r ? "in" : "not in");
    return r;
}
