                              ...
                              dev_t file_dev, ino_t file_ino)
{
    ...
    if (major(mnt_rdev) == LOOP_MAJOR)
    {
        ...
        if (file_dev == loopinfo.lo_device && file_ino == loopinfo.lo_inode)
            return 1;
    }
        ...
        if (stat(mnt->mnt_fsname, &st_buf) == 0)
        {
            if (ext2fsP_is_disk_device(st_buf.st_mode))
            {
                ...
                if (file_rdev && (file_rdev == st_buf.st_rdev))
                    break;
                if (check_loop_mounted(mnt->mnt_fsname, st_buf.st_rdev,
                                       file_dev, file_ino) == 1)
                    break;
#endif /* __GNU__ */
            }
...
static errcode_t check_mntent(const char* file, int* mount_flags, char* mtpt,
                              int mtlen)
{
    errcode_t retval;
...
static int is_swap_device(const char* file)
{
    ...
    file_dev = 0;
#ifndef __GNU__ /* The GNU hurd is broken with respect to stat devices */
    if ((stat(file, &st_buf) == 0) && ext2fsP_is_disk_device(st_buf.st_mode))
...
int main(int argc, char** argv)
{
    ...
    mntpt[0] = 0;
    retval =
        ext2fs_check_mount_point(argv[1], &mount_flags, mntpt, sizeof(mntpt));
    if (retval)
    {
        com_err(argv[0], retval, "while calling ext2fs_check_if_mounted");
        exit(1);
    }
