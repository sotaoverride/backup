 ...
 */
errcode_t ext2fs_super_and_bgd_loc2(ext2_filsys fs, dgrp_t group,
 ...
 * group descriptors.
 */
int ext2fs_super_and_bgd_loc(ext2_filsys fs, dgrp_t group, blk_t* ret_super_blk,
                             blk_t* ret_old_desc_blk, blk_t* ret_new_desc_blk,
                             int* ret_meta_bg)
{
    ...
    ext2fs_super_and_bgd_loc2(fs, group, &ret_super_blk2, &ret_old_desc_blk2,
                              &ret_new_desc_blk2, &ret_used_blks);
                                          ...
                                          struct ext2_super_block* super)
{
    ...
    fallback:
        ...
        retval = io_channel_write_blk64(fs->io, 1, -SUPERBLOCK_SIZE, super);
...
#endif
        retval =
            io_channel_write_byte(fs->io, SUPERBLOCK_OFFSET + (2 * write_idx),
                                  size, new_super + write_idx);
...
errcode_t ext2fs_flush2(ext2_filsys fs, int flags)
{
    ...
    if (retval)
        goto errout;
    retval = ext2fs_get_array(fs->desc_blocks, fs->blocksize, &group_shadow);
    if (retval)
        goto errout;
    memcpy(super_shadow, fs->super, sizeof(struct ext2_super_block));
    memcpy(group_shadow, fs->group_desc,
           (size_t)fs->blocksize * fs->desc_blocks);
    ...
    if (fs->progress_ops && fs->progress_ops->init)
        (fs->progress_ops->init)(fs, &progress, NULL, fs->group_desc_count);
    ...
    for (i = 0; i < fs->group_desc_count; i++)
    {
        ...
        {
            retval = write_backup_super(fs, i, super_blk, super_shadow);
            if (retval)
                goto errout;
        }
        ...
        {
            retval = io_channel_write_blk64(fs->io, old_desc_blk,
                                            old_desc_blocks, group_ptr);
            if (retval)
                goto errout;
        }
        if (new_desc_blk)
        {
            ...
            retval = io_channel_write_blk64(
                fs->io, new_desc_blk, 1, group_ptr + (meta_bg * fs->blocksize));
            if (retval)
                goto errout;
        }
    }
...
errcode_t ext2fs_close2(ext2_filsys fs, int flags)
{
    ...
    if (stats && stats->bytes_written && (fs->flags & EXT2_FLAG_RW))
    {
        ...
        if (!(fs->flags & EXT2_FLAG_SUPER_ONLY))
            fs->super->s_kbytes_written += meta_blks / (fs->blocksize / 1024);
