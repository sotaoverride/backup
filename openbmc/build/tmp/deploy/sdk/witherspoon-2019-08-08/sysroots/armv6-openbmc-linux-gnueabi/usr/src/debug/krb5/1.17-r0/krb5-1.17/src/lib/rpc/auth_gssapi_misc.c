                            ...
                            gss_buffer_t out_buf)
{
    ...
    gssstat = gss_seal(&minor_stat, context, 0, GSS_C_QOP_DEFAULT, &in_buf,
                       NULL, out_buf);
    if (gssstat != GSS_S_COMPLETE)
    {
        PRINTF(("gssapi_seal_seq: failed\n"));
        AUTH_GSSAPI_DISPLAY_STATUS(
            ("sealing sequence number", gssstat, minor_stat));
        return FALSE;
    }
    return TRUE;
}
                              ...
                              uint32_t* seq_num)
{
    ...
    gssstat = gss_unseal(&minor_stat, context, in_buf, &out_buf, NULL, NULL);
    if (gssstat != GSS_S_COMPLETE)
    {
        PRINTF(("gssapi_unseal_seq: failed\n"));
        AUTH_GSSAPI_DISPLAY_STATUS(
            ("unsealing sequence number", gssstat, minor_stat));
        return FALSE;
    }
    ...
    while (1)
    {
        ...
        if (gssstat != GSS_S_COMPLETE)
        {
            if (!rec)
            {
                ...
                auth_gssapi_display_status_1(m, minor_stat, GSS_C_MECH_CODE, 1);
            }
            else
            {
                             ...
                             caddr_t xdr_ptr)
{
    ...
    *major = gss_seal(minor, context, 1, GSS_C_QOP_DEFAULT, &in_buf,
                      &conf_state, &out_buf);
    ...
    if (!xdr_bytes(out_xdrs, (char**)&out_buf.value, (unsigned int*)&length,
                   out_buf.length))
    {
                               ...
                               caddr_t xdr_ptr)
{
    ...
    *major = gss_unseal(minor, context, &in_buf, &out_buf, &conf, &qop);
    free(in_buf.value);
    if (*major != GSS_S_COMPLETE)
        return FALSE;
