...
static int test_mkvol(void)
{
    ...
    req.alignment = 0;
    ret = ubi_mkvol(libubi, node, &req);
    if (check_failed(ret, EINVAL, "ubi_mkvol", "alignment = %d", req.alignment))
        return -1;
    ...
    req.bytes =
        (long long)(dev_info.leb_size - dev_info.leb_size % req.alignment) *
            dev_info.avail_lebs +
        1;
    ret = ubi_mkvol(libubi, node, &req);
    if (check_failed(ret, ENOSPC, "ubi_mkvol", "bytes = %lld", req.bytes))
    ...
    ret = ubi_mkvol(libubi, node, &req);
    if (check_failed(ret, EINVAL, "ubi_mkvol", "vol_type = %d", req.vol_type))
        return -1;
...
static int test_rmvol(void)
{
    ...
    if (check_failed(ret, ENODEV, "ubi_rmvol", "removed non-existing volume 0"))
        return -1;
