...
static gboolean g_socket_connection_close(GIOStream* stream,
                                          ...
                                          GError** error);
static void g_socket_connection_close_async(GIOStream* stream, int io_priority,
                                            GCancellable* cancellable,
...
G_DEFINE_TYPE_WITH_PRIVATE(GSocketConnection, g_socket_connection,
                           G_TYPE_IO_STREAM)
...
static GInputStream* g_socket_connection_get_input_stream(GIOStream* io_stream)
{
    ...
    if (connection->priv->input_stream == NULL)
        connection->priv->input_stream =
            (GInputStream*)_g_socket_input_stream_new(connection->priv->socket);
    ...
    g_socket_connection_get_output_stream(GIOStream* io_stream)
{
    ...
    if (connection->priv->output_stream == NULL)
        connection->priv->output_stream =
            (GOutputStream*)_g_socket_output_stream_new(
                connection->priv->socket);
                                     ...
                                     GCancellable* cancellable, GError** error)
{
    ...
    return g_socket_connect(connection->priv->socket, address, cancellable,
                            error);
}
                                       ...
                                       gpointer user_data)
{
    ...
    if (g_socket_connect(connection->priv->socket, address, cancellable,
                         &tmp_error))
    {
    ...
    else if (g_error_matches(tmp_error, G_IO_ERROR, G_IO_ERROR_PENDING))
    {
        ...
        source = g_socket_create_source(connection->priv->socket, G_IO_OUT,
                                        cancellable);
 ...
 */
void g_socket_connection_set_cached_remote_address(
    GSocketConnection* connection, GSocketAddress* address)
{
    g_clear_object(&connection->priv->cached_remote_address);
    connection->priv->cached_remote_address =
        address ? g_object_ref(address) : NULL;
}

static void g_socket_connection_get_property(GObject* object, guint prop_id,
...
static void g_socket_connection_set_property(GObject* object, guint prop_id,
                                             const GValue* value,
                                             GParamSpec* pspec)
{
    GSocketConnection* connection = G_SOCKET_CONNECTION(object);
...
static void g_socket_connection_class_init(GSocketConnectionClass* klass)
{
    ...
    g_object_class_install_property(
        ...
        g_param_spec_object("socket", P_("Socket"),
                            P_("The underlying GSocket"), G_TYPE_SOCKET,
                                          ...
                                          GError** error)
{
    ...
    if (connection->priv->output_stream)
        g_output_stream_close(connection->priv->output_stream, cancellable,
                              NULL);
    if (connection->priv->input_stream)
        g_input_stream_close(connection->priv->input_stream, cancellable, NULL);
                                               ...
                                               GSocketType type, gint protocol)
{
    ...
    if (connection_factories == NULL)
        connection_factories = g_hash_table_new_full(
            connection_factory_hash, connection_factory_equal,
    ...
    g_socket_connection_factory_create_connection(GSocket* socket)
{
    ...
    type = g_socket_connection_factory_lookup_type(
        g_socket_get_family(socket), g_socket_get_socket_type(socket),
