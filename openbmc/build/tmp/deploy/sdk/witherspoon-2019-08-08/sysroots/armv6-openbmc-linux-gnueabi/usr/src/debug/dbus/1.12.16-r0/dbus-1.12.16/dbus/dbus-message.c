    ...
    if (enabled < 0)
    {
        ...
        if (s && *s)
        {
            if (*s == '0')
                ...
                _dbus_warn(
                    "DBUS_MESSAGE_CACHE should be 0 or 1 if set, not '%s'", s);
        }
    }
...
void _dbus_message_trace_ref(DBusMessage* message, int old_refcount,
                             int new_refcount, const char* why)
{
    static int enabled = -1;
...
struct DBusMessageRealIter
{
    ...
    dbus_uint32_t changed_stamp
        : CHANGED_STAMP_BITS;    /**< stamp to detect invalid iters */
...
static void get_const_signature(DBusHeader* header,
                                const DBusString** type_str_p, int* type_pos_p)
{
    if (_dbus_header_get_field_raw(header, DBUS_HEADER_FIELD_SIGNATURE,
                                   type_str_p, type_pos_p))
    {
...
static void _dbus_message_byteswap(DBusMessage* message)
{
    ...
    _dbus_marshal_byteswap(type_str, type_pos, byte_order,
                           DBUS_COMPILER_BYTE_ORDER, &message->body, 0);
 ...
 */
void _dbus_message_get_unix_fds(DBusMessage* message, const int** fds,
                                unsigned* n_fds)
{
    _dbus_assert(message->locked);
...
static dbus_bool_t set_or_delete_string_field(DBusMessage* message, int field,
                                              int typecode, const char* value)
{
    if (value == NULL)
        return _dbus_header_delete_field(&message->header, field);
    else
        return _dbus_header_set_field_basic(&message->header, field, typecode,
                                            &value);
}
...
static void dbus_message_cache_or_finalize(DBusMessage* message)
{
    ...
    if ((_dbus_string_get_length(&message->header.data) +
         _dbus_string_get_length(&message->body)) > MAX_MESSAGE_SIZE_TO_CACHE)
        goto out;
...
static dbus_bool_t _dbus_message_iter_check(DBusMessageRealIter* iter)
{
    ...
    else if (iter->iter_type == DBUS_MESSAGE_ITER_TYPE_WRITER)
    {
        ...
        {
            _dbus_warn_check_failed("dbus message changed byte order since "
                                    "append iterator was created");
            return FALSE;
        }
        /* because we swap the message into compiler order when you init an iter
         */
    ...
    {
        _dbus_warn_check_failed(
            "dbus message iterator invalid because the message has been "
            "modified (or perhaps the iterator is just uninitialized)");
        return FALSE;
    }
 ...
 * @param var_args return location for first argument, followed by list of
 * type/location pairs
    ...
    while (spec_type != DBUS_TYPE_INVALID)
    {
        ...
        if (msg_type != spec_type)
        {
            dbus_set_error(error, DBUS_ERROR_INVALID_ARGS,
                           ...
                           i, _dbus_type_to_string(spec_type),
        ...
        else if (spec_type == DBUS_TYPE_ARRAY)
        {
                ...
                element_type != DBUS_TYPE_UNIX_FD)
            {
                ...
                _dbus_type_reader_read_fixed_multi(&array, (void*)ptr,
                                                   n_elements_p);
            }
            else if (_DBUS_TYPE_IS_STRINGLIKE(spec_element_type))
            {
                ...
                _dbus_assert(_dbus_type_reader_get_current_type(&array) ==
                             DBUS_TYPE_INVALID);
                _dbus_assert(j == n_elements);
            ...
            {
                _dbus_warn("you can't read arrays of container types (struct, "
        ...
        if (!_dbus_type_reader_next(&real->u.reader) &&
 ...
 *
 * Convenience functions dbus_message_is_method_call(),
 * dbus_message_is_signal(), and dbus_message_is_error() check several header
 * fields at once and are slightly more efficient than checking the header
 * fields with individual accessor functions.
 *
...
DBusMessage* dbus_message_new(int message_type)
{
    ...
    if (!_dbus_header_create(&message->header, DBUS_COMPILER_BYTE_ORDER,
                             message_type, NULL, NULL, NULL, NULL, NULL))
    {
                                          ...
                                          const char* method)
{
    ...
    _dbus_return_val_if_fail(
        iface == NULL || _dbus_check_is_valid_interface(iface), NULL);
    ...
    if (!_dbus_header_create(&message->header, DBUS_COMPILER_BYTE_ORDER,
                             DBUS_MESSAGE_TYPE_METHOD_CALL, destination, path,
                             iface, method, NULL))
    {
        dbus_message_unref(message);
        return NULL;
...
DBusMessage* dbus_message_new_method_return(DBusMessage* method_call)
{
    ...
    if (!_dbus_header_create(&message->header, DBUS_COMPILER_BYTE_ORDER,
                             DBUS_MESSAGE_TYPE_METHOD_RETURN, sender, NULL,
                             NULL, NULL, NULL))
    {
        dbus_message_unref(message);
        return NULL;
                                     ...
                                     const char* name)
{
    ...
    if (!_dbus_header_create(&message->header, DBUS_COMPILER_BYTE_ORDER,
                             DBUS_MESSAGE_TYPE_SIGNAL, NULL, path, iface, name,
                             NULL))
    {
        dbus_message_unref(message);
        return NULL;
 ...
 * @param error_message the error message string (or #NULL for none, but please
 * give a message)
                                    ...
                                    const char* error_message)
{
    ...
    if (!_dbus_header_create(&message->header, DBUS_COMPILER_BYTE_ORDER,
                             DBUS_MESSAGE_TYPE_ERROR, sender, NULL, NULL, NULL,
                             error_name))
    {
        dbus_message_unref(message);
        return NULL;
 ...
 * will not be locked.
 *
 * @todo This function can't be used in programs that try to recover from OOM
 * errors.
 *
 * @param message the message
 * @returns the new message.or #NULL if not enough memory or Unix file
 * descriptors (in case the message to copy includes Unix file descriptors) can
 * be allocated.
 */
DBusMessage* dbus_message_copy(const DBusMessage* message)
{
    ...
    if (!_dbus_string_init_preallocated(
            &retval->body, _dbus_string_get_length(&message->body)))
    {
    ...
    if (!_dbus_string_copy(&message->body, 0, &retval->body, 0))
        goto failed_copy;
    ...
    for (retval->n_unix_fds = 0; retval->n_unix_fds < message->n_unix_fds;
         retval->n_unix_fds++)
    {
        retval->unix_fds[retval->n_unix_fds] =
            _dbus_dup(message->unix_fds[retval->n_unix_fds], NULL);
...
/**
 * Like dbus_message_append_args() but takes a va_list for use by language
 * bindings.
 *
                                            ...
                                            va_list var_args)
{
    ...
    while (type != DBUS_TYPE_INVALID)
    {
        ...
        else if (type == DBUS_TYPE_ARRAY)
        {
            ...
            buf[0] = element_type;
            buf[1] = '\0';
            if (!dbus_message_iter_open_container(&iter, DBUS_TYPE_ARRAY, buf,
                                                  &array))
                goto failed;
                ...
                element_type != DBUS_TYPE_UNIX_FD)
            {
                ...
                if (!dbus_message_iter_append_fixed_array(&array, element_type,
                                                          value, n_elements))
                {
            ...
            else if (_DBUS_TYPE_IS_STRINGLIKE(element_type))
            {
                ...
                while (i < n_elements)
                {
                    if (!dbus_message_iter_append_basic(&array, element_type,
                                                        &value[i]))
                    {
                        dbus_message_iter_abandon_container(&iter, &array);
                        goto failed;
 ...
 * @param ... location for first argument value, then list of type-location
 * pairs
 ...
 * @returns #FALSE if error was set
 */
dbus_bool_t dbus_message_get_args_valist(DBusMessage* message, DBusError* error,
                                         int first_arg_type, va_list var_args)
{
    ...
    return _dbus_message_iter_get_args_valist(&iter, error, first_arg_type,
                                              var_args);
}

static void _dbus_message_iter_init_common(DBusMessage* message,
                                           ...
                                           int iter_type)
{
     ...
     * every byte. */
    _DBUS_STATIC_ASSERT(sizeof(DBusMessageIter) == 4 * sizeof(void*) +
 ...
 * DBUS_TYPE_INVALID) dbus_message_iter_next (&iter);
 * @endcode
 *
...
dbus_bool_t dbus_message_iter_init(DBusMessage* message, DBusMessageIter* iter)
{
    ...
    _dbus_type_reader_init(&real->u.reader,
                           ...
                           type_str, type_pos, &message->body, 0);
...
dbus_bool_t dbus_message_iter_has_next(DBusMessageIter* iter)
{
    ...
    _dbus_return_val_if_fail(real->iter_type == DBUS_MESSAGE_ITER_TYPE_READER,
                             FALSE);
...
int dbus_message_iter_get_element_type(DBusMessageIter* iter)
{
    ...
    _dbus_return_val_if_fail(real->iter_type == DBUS_MESSAGE_ITER_TYPE_READER,
                             DBUS_TYPE_INVALID);
    _dbus_return_val_if_fail(dbus_message_iter_get_arg_type(iter) ==
                                 DBUS_TYPE_ARRAY,
                             DBUS_TYPE_INVALID);
...
char* dbus_message_iter_get_signature(DBusMessageIter* iter)
{
    ...
    if (!_dbus_string_append_len(&retstr,
                                 _dbus_string_get_const_data(sig) + start, len))
        return NULL;
...
int dbus_message_iter_get_element_count(DBusMessageIter* iter)
{
    ...
    _dbus_return_val_if_fail(
        _dbus_type_reader_get_current_type(&real->u.reader) == DBUS_TYPE_ARRAY,
        0);
 ...
 */
void dbus_message_iter_get_fixed_array(DBusMessageIter* iter, void* value,
                                       int* n_elements)
{
    DBusMessageRealIter* real = (DBusMessageRealIter*)iter;
#ifndef DBUS_DISABLE_CHECKS
...
void dbus_message_iter_init_append(DBusMessage* message, DBusMessageIter* iter)
{
    ...
    _dbus_type_writer_init_types_delayed(
        &real->u.writer, _dbus_header_get_byte_order(&message->header),
        &message->body, _dbus_string_get_length(&message->body));
}
...
static dbus_bool_t _dbus_message_iter_open_signature(DBusMessageRealIter* real)
{
    ...
    if (!_dbus_header_get_field_raw(&real->message->header,
                                    DBUS_HEADER_FIELD_SIGNATURE, &current_sig,
                                    &current_sig_pos))
        current_sig = NULL;
    ...
    _dbus_type_writer_add_types(&real->u.writer, str,
...
static dbus_bool_t _dbus_message_iter_append_check(DBusMessageRealIter* iter)
{
    ...
    {
        _dbus_warn_check_failed("dbus append iterator can't be used: message "
                                "is locked (has already been sent)");
        return FALSE;
    }
                                           ...
                                           const void* value)
{
    ...
    _dbus_return_val_if_fail(real->iter_type == DBUS_MESSAGE_ITER_TYPE_WRITER,
                             FALSE);
    ...
    switch (type)
    {
        ...
        case DBUS_TYPE_STRING:
            string_p = value;
            _dbus_return_val_if_fail(_dbus_check_is_valid_utf8(*string_p),
                                     FALSE);
            break;

        case DBUS_TYPE_OBJECT_PATH:
            string_p = value;
            _dbus_return_val_if_fail(_dbus_check_is_valid_path(*string_p),
                                     FALSE);
            break;
        ...
        case DBUS_TYPE_SIGNATURE:
            ...
            signature_validity = _dbus_validate_signature_with_reason(
    ...
    if (type == DBUS_TYPE_UNIX_FD)
    {
        ...
        *fds = _dbus_dup(*(int*)value, NULL);
        ...
        if (!(ret = _dbus_type_writer_write_basic(&real->u.writer,
                                                  DBUS_TYPE_UNIX_FD, &u)))
        {
 ...
 * if (!dbus_message_iter_append_fixed_array (&iter, DBUS_TYPE_INT32, &v_ARRAY,
 * 3)) fprintf (stderr, "No memory!\n");
 * @endcode
                                                 ...
                                                 int n_elements)
{
    ...
    _dbus_return_val_if_fail(real->iter_type == DBUS_MESSAGE_ITER_TYPE_WRITER,
                             FALSE);
    _dbus_return_val_if_fail(dbus_type_is_fixed(element_type) &&
                                 element_type != DBUS_TYPE_UNIX_FD,
                             FALSE);
    ...
    _dbus_return_val_if_fail(n_elements <=
                                 DBUS_MAXIMUM_ARRAY_LENGTH /
                                     _dbus_type_get_alignment(element_type),
                             FALSE);
    ...
    if (element_type == DBUS_TYPE_BOOLEAN)
    {
        ...
        {
            _dbus_return_val_if_fail((*bools)[i] == 0 || (*bools)[i] == 1,
                                     FALSE);
        }
    }
#endif

    ret = _dbus_type_writer_write_fixed_multi(&real->u.writer, element_type,
                                              value, n_elements);
                                             ...
                                             DBusMessageIter* sub)
{
    ...
    if (contained_signature != NULL)
    {
        ...
        contained_signature_validity = _dbus_validate_signature_with_reason(
            &contained_str, 0, _dbus_string_get_length(&contained_str));
    ...
    _dbus_return_val_if_fail(
        (type == DBUS_TYPE_ARRAY && contained_signature &&
         *contained_signature == DBUS_DICT_ENTRY_BEGIN_CHAR) ||
    ...
    *real_sub = *real;
    ...
    if (contained_signature != NULL)
    {
        ...
        ret = _dbus_type_writer_recurse(&real->u.writer, type, &contained_str,
                                        0, &real_sub->u.writer);
    }
    else
    {
        ret = _dbus_type_writer_recurse(&real->u.writer, type, NULL, 0,
                                        &real_sub->u.writer);
    }
                                              ...
                                              DBusMessageIter* sub)
{
    ...
    _dbus_return_val_if_fail(
        real_sub->iter_type == DBUS_MESSAGE_ITER_TYPE_WRITER, FALSE);
...
void dbus_message_set_no_reply(DBusMessage* message, dbus_bool_t no_reply)
{
    ...
    _dbus_header_toggle_flag(&message->header,
                             DBUS_HEADER_FLAG_NO_REPLY_EXPECTED, no_reply);
}
...
void dbus_message_set_auto_start(DBusMessage* message, dbus_bool_t auto_start)
{
    ...
    _dbus_header_toggle_flag(&message->header, DBUS_HEADER_FLAG_NO_AUTO_START,
                             !auto_start);
}
...
dbus_bool_t dbus_message_set_path(DBusMessage* message, const char* object_path)
{
    ...
    _dbus_return_val_if_fail(!message->locked, FALSE);
    _dbus_return_val_if_fail(
        object_path == NULL || _dbus_check_is_valid_path(object_path), FALSE);

    return set_or_delete_string_field(message, DBUS_HEADER_FIELD_PATH,
...
const char* dbus_message_get_path(DBusMessage* message)
{
    ...
    _dbus_header_get_field_basic(&message->header, DBUS_HEADER_FIELD_PATH,
                                 DBUS_TYPE_OBJECT_PATH, (void*)&v);
    return v;
}
 ...
 *
 * @param message the message
 * @param path place to store allocated array of path components; #NULL set here
 * if no path field exists
...
dbus_bool_t dbus_message_get_path_decomposed(DBusMessage* message, char*** path)
{
    ...
    *path = NULL;
    ...
    if (v != NULL)
    {
        if (!_dbus_decompose_path(v, strlen(v), path, NULL))
            return FALSE;
    }
    return TRUE;
}
...
dbus_bool_t dbus_message_set_interface(DBusMessage* message, const char* iface)
{
    ...
    _dbus_return_val_if_fail(!message->locked, FALSE);
    _dbus_return_val_if_fail(
        iface == NULL || _dbus_check_is_valid_interface(iface), FALSE);

    return set_or_delete_string_field(message, DBUS_HEADER_FIELD_INTERFACE,
...
const char* dbus_message_get_interface(DBusMessage* message)
{
    ...
    v = NULL; /* in case field doesn't exist */
    _dbus_header_get_field_basic(&message->header, DBUS_HEADER_FIELD_INTERFACE,
                                 DBUS_TYPE_STRING, (void*)&v);
    return v;
}
...
dbus_bool_t dbus_message_set_member(DBusMessage* message, const char* member)
{
    ...
    _dbus_return_val_if_fail(!message->locked, FALSE);
    _dbus_return_val_if_fail(
        member == NULL || _dbus_check_is_valid_member(member), FALSE);

    return set_or_delete_string_field(message, DBUS_HEADER_FIELD_MEMBER,
...
const char* dbus_message_get_member(DBusMessage* message)
{
    ...
    v = NULL; /* in case field doesn't exist */
    _dbus_header_get_field_basic(&message->header, DBUS_HEADER_FIELD_MEMBER,
                                 DBUS_TYPE_STRING, (void*)&v);
    return v;
}
                                        ...
                                        const char* error_name)
{
    ...
    return set_or_delete_string_field(message, DBUS_HEADER_FIELD_ERROR_NAME,
...
const char* dbus_message_get_error_name(DBusMessage* message)
{
    ...
    v = NULL; /* in case field doesn't exist */
    _dbus_header_get_field_basic(&message->header, DBUS_HEADER_FIELD_ERROR_NAME,
                                 DBUS_TYPE_STRING, (void*)&v);
    return v;
}
                                         ...
                                         const char* destination)
{
    ...
    return set_or_delete_string_field(message, DBUS_HEADER_FIELD_DESTINATION,
...
dbus_bool_t dbus_message_set_sender(DBusMessage* message, const char* sender)
{
    ...
    _dbus_return_val_if_fail(!message->locked, FALSE);
    _dbus_return_val_if_fail(
        sender == NULL || _dbus_check_is_valid_bus_name(sender), FALSE);

    return set_or_delete_string_field(message, DBUS_HEADER_FIELD_SENDER,
...
const char* dbus_message_get_sender(DBusMessage* message)
{
    ...
    v = NULL; /* in case field doesn't exist */
    _dbus_header_get_field_basic(&message->header, DBUS_HEADER_FIELD_SENDER,
                                 DBUS_TYPE_STRING, (void*)&v);
    return v;
}
...
static dbus_bool_t _dbus_message_has_type_interface_member(DBusMessage* message,
                                                           int type,
                                                           const char* iface,
                                                           const char* member)
{
 ...
 */
dbus_bool_t dbus_message_is_method_call(DBusMessage* message, const char* iface,
                                        const char* method)
{
    _dbus_return_val_if_fail(message != NULL, FALSE);
    ...
    return _dbus_message_has_type_interface_member(
        message, DBUS_MESSAGE_TYPE_METHOD_CALL, iface, method);
}
                                   ...
                                   const char* signal_name)
{
    ...
    return _dbus_message_has_type_interface_member(
        message, DBUS_MESSAGE_TYPE_SIGNAL, iface, signal_name);
}
...
void _dbus_message_loader_get_buffer(DBusMessageLoader* loader,
                                     DBusString** buffer, int* max_to_read,
        ...
        *may_read_fds = TRUE;
        ...
        while (remain > 0)
        {
            ...
            if (!_dbus_header_have_message_untrusted(
                    loader->max_message_size, &validity, &byte_order,
                    &fields_array_len, &header_len, &body_len, &loader->data,
                    offset, remain))
            {
                                              ...
                                              int** fds, unsigned* max_n_fds)
{
    ...
    {
        int* a =
            dbus_realloc(loader->unix_fds, loader->max_message_unix_fds *
                                               sizeof(loader->unix_fds[0]));
...
void _dbus_message_loader_return_unix_fds(DBusMessageLoader* loader, int* fds,
                                          unsigned n_fds)
{
    _dbus_assert(loader->unix_fds_outstanding);
 ...
 */
static dbus_bool_t load_message(DBusMessageLoader* loader, DBusMessage* message,
                                int byte_order, int fields_array_len,
                                int header_len, int body_len)
{
    dbus_bool_t oom;
  ...
  _dbus_verbose_bytes_of_string (&loader->data, 0, header_len /* + body_len */);
    ...
    _dbus_assert((header_len + body_len) <=
                 _dbus_string_get_length(&loader->data));

    if (!_dbus_header_load(&message->header, mode, &validity, byte_order,
                           fields_array_len, header_len, body_len,
                           &loader->data, 0,
                           _dbus_string_get_length(&loader->data)))
    {
        ...
        /* assert here so we can catch any code that still uses DBUS_VALID to
           indicate
    ...
    if (mode != DBUS_VALIDATION_MODE_WE_TRUST_THIS_DATA_ABSOLUTELY)
    {
        ...
        validity = _dbus_validate_body_with_reason(
            type_str, type_pos, byte_order, NULL, &loader->data, header_len,
            body_len);
        if (validity != DBUS_VALID)
        {
    ...
    _dbus_header_get_field_basic(&message->header, DBUS_HEADER_FIELD_UNIX_FDS,
                                 DBUS_TYPE_UINT32, &n_unix_fds);

#ifdef HAVE_UNIX_FD_PASSING

    if (n_unix_fds > loader->n_unix_fds)
    {
        _dbus_verbose("Message contains references to more unix fds than were "
                      "sent %u != %u\n",
    ...
    if (n_unix_fds > 0)
    {
        message->unix_fds = _dbus_memdup(
            loader->unix_fds, n_unix_fds * sizeof(message->unix_fds[0]));
        ...
        memmove(loader->unix_fds, loader->unix_fds + n_unix_fds,
                loader->n_unix_fds * sizeof(loader->unix_fds[0]));
    ...
    if (!_dbus_string_copy_len(&loader->data, header_len, body_len,
                               &message->body, 0))
    {
...
failed:
    ...
    _dbus_verbose_bytes_of_string(&loader->data, 0,
                                  _dbus_string_get_length(&loader->data));
...
dbus_bool_t _dbus_message_loader_queue_messages(DBusMessageLoader* loader)
{
           ...
           _dbus_string_get_length(&loader->data) >= DBUS_MINIMUM_HEADER_SIZE)
    {
        ...
        if (_dbus_header_have_message_untrusted(
                loader->max_message_size, &validity, &byte_order,
                &fields_array_len, &header_len, &body_len, &loader->data, 0,
                _dbus_string_get_length(&loader->data)))
        {
            DBusMessage* message;
            ...
            if (!load_message(loader, message, byte_order, fields_array_len,
                              header_len, body_len))
            {
                dbus_message_unref(message);
            ...
            _dbus_assert(loader->messages != NULL);
            _dbus_assert(_dbus_list_find_last(&loader->messages, message) !=
                         NULL);
        }
        else
        {
            _dbus_verbose("Initial peek at header says we don't have a whole "
                          "message yet, or data broken with invalid code %d\n",
                          validity);
            if (validity != DBUS_VALID)
            {
                loader->corrupted = TRUE;
 ...
 */
dbus_bool_t dbus_message_set_data(DBusMessage* message, dbus_int32_t slot,
                                  void* data, DBusFreeFunction free_data_func)
{
    ...
    retval = _dbus_data_slot_list_set(&slot_allocator, &message->slot_list,
 ...
 */
dbus_bool_t dbus_message_marshal(DBusMessage* msg, char** marshalled_data_p,
                                 int* len_p)
{
    DBusString tmp;
 ...
 * @returns -1 if there was no valid data to be demarshalled, 0 if there wasn't
 * enough data to determine how much should be demarshalled. Otherwise returns
 * the number of bytes to be demarshalled
 *
 */
int dbus_message_demarshal_bytes_needed(const char* buf, int len)
{
    ...
    have_message = _dbus_header_have_message_untrusted(
        DBUS_MAXIMUM_MESSAGE_LENGTH, &validity, &byte_order, &fields_array_len,
        &header_len, &body_len, &str, 0, len);
    _dbus_string_free(&str);
    ...
    dbus_message_get_allow_interactive_authorization(DBusMessage* message)
{
    ...
    return _dbus_header_get_flag(
        &message->header, DBUS_HEADER_FLAG_ALLOW_INTERACTIVE_AUTHORIZATION);
}
...
DBusVariant* _dbus_variant_read(DBusMessageIter* reader)
{
    ...
    int type;
    /* The string, start position within that string, and length of the
     * signature of the single complete type of the item reader points to */
    const DBusString* sig;
    int start, len;
    ...
    if (type == DBUS_TYPE_ARRAY)
    {
        ...
        DBusMessageRealIter* real_array_reader =
            (DBusMessageRealIter*)&array_reader;
             ...
             type == DBUS_TYPE_STRUCT)
    {
        ...
        DBusMessageRealIter* real_inner_reader =
            (DBusMessageRealIter*)&inner_reader;
...
dbus_bool_t _dbus_variant_write(DBusVariant* self, DBusMessageIter* writer)
{
    ...
    ret =
        _dbus_type_writer_write_reader(&real_writer->u.writer, &variant_reader);
