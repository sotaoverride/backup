...
#define GA_SERVICE_RESOLVER_GET_PRIVATE(o)                                     \
    (G_TYPE_INSTANCE_GET_PRIVATE((o), GA_TYPE_SERVICE_RESOLVER,                \
                                 GaServiceResolverPrivate))
...
static void ga_service_resolver_set_property(GObject* object, guint property_id,
                                             const GValue* value,
                                             GParamSpec* pspec)
{
...
static void ga_service_resolver_get_property(GObject* object, guint property_id,
...
static void ga_service_resolver_class_init(
    GaServiceResolverClass* ga_service_resolver_class)
{
    ...
    signals[FOUND] = g_signal_new(
        "found", G_OBJECT_CLASS_TYPE(ga_service_resolver_class),
    ...
    param_spec = g_param_spec_enum(
        "flags", "Lookup flags for the resolver", "Resolver lookup flags",
...
static void _avahi_service_resolver_cb(
    AVAHI_GCC_UNUSED AvahiServiceResolver* resolver, AvahiIfIndex interface,
    ...
    AvahiLookupResultFlags flags, void* userdata)
{
    ...
    switch (event)
    {
        case AVAHI_RESOLVER_FOUND:
        {
            ...
            priv->address = *a;
            priv->port = port;
            g_signal_emit(self, signals[FOUND], 0, interface, protocol, name,
                          type, domain, host_name, a, port, txt, flags);
            break;
        }
        case AVAHI_RESOLVER_FAILURE:
        {
            ...
            error = g_error_new(GA_ERROR, aerrno, "Resolving failed: %s",
                                    ...
                                    GaClient* client, GError** error)
{
    ...
    priv->resolver = avahi_service_resolver_new(
        client->avahi_client, priv->interface, priv->protocol, priv->name,
        priv->type, priv->domain, priv->aprotocol, priv->flags,
        _avahi_service_resolver_cb, resolver);
