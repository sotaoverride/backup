...
static struct argp_option options[] = {
    ...
    {"list", 'l', 0, 0,
     "List the operational entropy sources on this system and exit"},

    {"option", 'O', "options", 0,
     "rng specific options in the form source:key:value"},
    ...
    {"fill-watermark", 'W', "n", 0,
     "Do not stop feeding entropy to random-device until at least n bits of "
     "entropy are available in the pool (default: 2048), 0 <= n <= 4096"},
    ...
    {"entropy-count", 'e', "n", 0,
     "Number of entropy bits to support (default: 8), 1 <= n <= 8"},
...
static struct rng_option darn_options[] = {[DARN_OPT_AES] =
                                               {
                                                   .key = "use_aes",
                                                   .type = VAL_INT,
                                                   .int_val = 1,
...
static struct rng_option jitter_options[] = {[JITTER_OPT_THREADS] =
                                                 {
                                                     .key = "thread_count",
                                                     .type = VAL_INT,
                                                     .int_val = 4,
...
static int find_ent_src_idx(const char* name_idx)
{
    ...
    else
    {
        ...
        message(LOG_CONS | LOG_INFO,
                "Note, reference of entropy sources by index "
...
static error_t parse_opt(int key, char* arg, struct argp_state* state)
{
    ...
    switch (key)
    {
        ...
        case 'O':
            ...
            if (!search)
            {
                message(LOG_CONS | LOG_ERR,
                        "Options tuple not specified correctly\n");
                return -EINVAL;
            }
            ...
            *search = ':';
            ...
            message(LOG_CONS | LOG_INFO,
                    "Option %s not found for source idx %lu\n", optkey, idx);
            return -ERANGE;
            break;
...
static void do_loop(int random_step)
{
    ...
    int (*random_add_fn)(struct rng * rng, int random_step, unsigned char* buf,
                         fips_ctx_t* fipsctx_in);
...
continue_trying:
    for (no_work = 0; no_work < 100; no_work = (work_done ? 0 : no_work + 1))
    {
         ...
         * but can actually hinder performance when large amounts of entropy are
         * needed owing to the fact that they may block while generating said
         * entropy So, lets prioritize the faster sources.  Start by only trying
         * to collect entropy from the fast sources, then iff that fails, start
         * including the slower sources as well.  Once we get some entropy,
         * return to only using fast sources
         */
        if (no_work)
    ...
    message(LOG_DAEMON | LOG_ERR, "No entropy sources working, exiting rngd\n");
}
...
static int discard_initial_data(struct rng* ent_src)
{
    ...
    return tempbuf[0] | (tempbuf[1] << 8) | (tempbuf[2] << 16) |
           (tempbuf[3] << 24);
}
...
int main(int argc, char** argv)
{
    ...
    for (i = 0; i < ENT_MAX; i++)
    {
        if (entropy_sources[i].init && entropy_sources[i].disabled == false)
        {
            if (!entropy_sources[i].init(&entropy_sources[i]))
            {
                ...
                fips_init(entropy_sources[i].fipsctx,
                          discard_initial_data(&entropy_sources[i]));
                message(LOG_INFO | LOG_DAEMON,
                        "Initializing entropy source %s\n",
                        entropy_sources[i].rng_sname);
            }
            else
            {
                message(LOG_ERR | LOG_DAEMON,
                        "Failed to init entropy source %s\n",
                        entropy_sources[i].rng_sname);
    ...
    if (!ent_sources)
    {
        message(LOG_DAEMON | LOG_ERR, "can't open any entropy source");
        message(LOG_DAEMON | LOG_ERR,
    ...
    if (arguments->test && test_iterations)
    {
        ...
        test_time = ((test_time * NSECS_IN_SECOND) +
        ...
        {
            message(LOG_CONS | LOG_INFO,
                    "\nEntropy rate: %6.4g Mbits/sec averaged over %lu "
                    "iterations for %6.4g seconds\n",
                    (sum_entropy / test_time / MEGABITS), test_iterations,
                    test_time);
        }
        else
        {
            message(LOG_CONS | LOG_INFO,
                    "\nEntropy rate: %6.4g Kbits/sec averaged over %lu "
                    "iterations for %6.4g seconds\n",
                    (sum_entropy / test_time / KILOBITS), test_iterations,
                    test_time);
        }
    }
