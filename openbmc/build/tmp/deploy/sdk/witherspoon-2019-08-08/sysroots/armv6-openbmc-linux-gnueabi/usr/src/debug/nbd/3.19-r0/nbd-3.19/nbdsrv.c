...
#define LINELEN                                                                \
    256 /**< Size of static buffer used to read the                            \
                     ...
                     GError** err)
{
    ...
    {
        g_set_error(err, NBDS_ERR, NBDS_ERR_GAI,
    ...
    while (res)
    {
        ...
        {
            msg(LOG_DEBUG,
                "client address does not match %d/%d: address family mismatch "
                "(IPv4 vs IPv6?)",
        ...
        switch (addr->sa_family)
        {
            case AF_INET:
                byte_s =
                    (const uint8_t*)(&(((struct sockaddr_in*)addr)->sin_addr));
                byte_t = (uint8_t*)(&(
                    ((struct sockaddr_in*)(res->ai_addr))->sin_addr));
                break;
            case AF_INET6:
                byte_s = (const uint8_t*)(&(
...
uint64_t size_autodetect(int fhandle)
{
    ...
    {
        DEBUG("lseek failed: %d",
              errno == EBADF
                  ? 1
                  : (errno == ESPIPE ? 2 : (errno == EINVAL ? 3 : 4)));
    }
...
int exptrim(struct nbd_request* req, CLIENT* client)
{
    ...
    if (client->server->flags & F_TREEFILES)
    {
        ...
        off_t min =
            ((req->from + TREEPAGESIZE - 1) / TREEPAGESIZE) * TREEPAGESIZE;
        ...
        DEBUG("Performed TRIM request on TREE structure from %llu to %llu",
