...
#define EXT2_INODE_SIZE(s)                                                     \
    (((s)->s_rev_level == EXT2_GOOD_OLD_REV) ? EXT2_GOOD_OLD_INODE_SIZE        \
                                             : (s)->s_inode_size)
#define EXT2_FIRST_INO(s)                                                      \
    (((s)->s_rev_level == EXT2_GOOD_OLD_REV) ? EXT2_GOOD_OLD_FIRST_INO         \
                                             : (s)->s_first_ino)
#endif
...
#define EXT4_BG_INODE_BITMAP_CSUM_HI_END                                       \
    (offsetof(struct ext4_group_desc, bg_inode_bitmap_csum_hi) + sizeof(__u16))
#define EXT4_BG_BLOCK_BITMAP_CSUM_HI_LOCATION                                  \
    (offsetof(struct ext4_group_desc, bg_block_bitmap_csum_hi) + sizeof(__u16))
...
#define EXT2_MAX_BLOCKS_PER_GROUP(s)                                           \
    ((((unsigned)1 << 16) - 8) * (EXT2_CLUSTER_SIZE(s) / EXT2_BLOCK_SIZE(s)))
#define EXT2_MAX_CLUSTERS_PER_GROUP(s) (((unsigned)1 << 16) - 8)
#define EXT2_MAX_INODES_PER_GROUP(s)                                           \
    (((unsigned)1 << 16) - EXT2_INODES_PER_BLOCK(s))
#ifdef __KERNEL__
...
#define EXT2_GROUPS_TO_CLUSTERS(s, g)                                          \
    ((blk64_t)EXT2_CLUSTERS_PER_GROUP(s) * (g))
...
#define inode_uid(inode)                                                       \
    ((inode).i_uid | (inode).osd2.linux2.l_i_uid_high << 16)
#define inode_gid(inode)                                                       \
    ((inode).i_gid | (inode).osd2.linux2.l_i_gid_high << 16)
...
#define EXT4_FEATURE_COMPAT_FUNCS(name, ver, flagname)                         \
    ...
    {                                                                          \
        EXT2_SB(sb)->s_feature_compat |= EXT##ver##_FEATURE_COMPAT_##flagname; \
    }                                                                          \
    static inline void ext2fs_clear_feature_##name(                            \
...
#define EXT2_DIR_REC_LEN(name_len)                                             \
    (((name_len) + EXT2_DIR_ENTRY_HEADER_LEN + EXT2_DIR_ROUND) &               \
     ~EXT2_DIR_ROUND)
