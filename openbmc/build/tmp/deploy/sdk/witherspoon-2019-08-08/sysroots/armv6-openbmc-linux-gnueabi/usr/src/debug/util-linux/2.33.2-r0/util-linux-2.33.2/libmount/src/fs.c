 ...
 *
 * This function does not copy userdata (se mnt_fs_set_userdata()). A new copy
 * is not linked with any existing mnt_tab.
 *
 ...
 * in the mountinfo file. The kernel default is MS_PRIVATE, this flag is not
 * stored in the mountinfo file.
 *
 * Returns: 0 on success or negative number in case of error.
 */
 ...
 * @fs: filesystem
 *
 * Returns: 1 if the filesystem description is read from kernel e.g.
 * /proc/mounts.
 */
 ...
 *
 * Returns: pointer to string (can be freed by free(3)) or NULL in case of
 * error.
 */
 ...
 * @optstr: mount options
 *
 * Parses (splits) @optstr and prepends the results to VFS, FS and userspace
 * lists of options.
 *
 * If @optstr is NULL, then @fs is not modified and 0 is returned.
 *
 ...
 * @fs: /proc/self/mountinfo entry
 *
 * Returns: mount ID (unique identifier of the mount) or negative number in case
 * of error.
 */
 ...
 * @fs: fstab/mtab/mountinfo entry pointer
 * @name: option name
 * @value: returns pointer to the beginning of the value (e.g. name=VALUE) or
 * NULL
 * @valsz: returns size of options value or 0
 *
 * Returns: 0 on success, 1 when @name not found or negative number in case of
 * error.
 */
int mnt_fs_get_option(struct libmnt_fs* fs, const char* name, char** value,
                      size_t* valsz)
{
    char rc = 1;
 ...
 * error.
 */
int mnt_fs_get_attribute(struct libmnt_fs* fs, const char* name, char** value,
                         size_t* valsz)
{
    char rc = 1;
 ...
 *	   path @target or @fs->target is found in the @mtab, the canonicalization
 *is is not performed (see mnt_resolve_target()).
 *
 ...
 *
 * Copies the information from @fs to struct mntent @mnt. If @mnt is already
 * set, then the struct mntent items are reallocated and updated. See also
 * mnt_free_mntent().
 *
