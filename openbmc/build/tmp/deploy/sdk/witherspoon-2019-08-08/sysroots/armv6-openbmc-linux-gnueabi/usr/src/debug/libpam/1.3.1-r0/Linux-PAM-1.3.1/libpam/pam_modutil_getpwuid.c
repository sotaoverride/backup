...
struct passwd* pam_modutil_getpwuid(pam_handle_t* pamh, uid_t uid)
{
    ...
    do
    {
        ...
        errno = 0;
        status = getpwuid_r(uid, buffer, sizeof(struct passwd) + (char*)buffer,
                            length, &result);
        if (!status && (result == buffer))
        {
            ...
            if (pamh != NULL)
            {
                for (i = 0; i < INT_MAX; i++)
                {
                    sprintf(data_name, "_pammodutil_getpwuid_%ld_%d", (long)uid,
                            i);
                    status = PAM_NO_MODULE_DATA;
                    if (pam_get_data(pamh, data_name, &ignore) != PAM_SUCCESS)
                    {
                        status = pam_set_data(pamh, data_name, result,
                                              pam_modutil_cleanup);
                    }
