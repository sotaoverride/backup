                       ...
                       netsnmp_trapd_handler* handler)
{
    ...
    if (pdu->version == SNMP_VERSION_1)
    {
        ...
        if (!newpdu)
        {
            snmp_log(
                LOG_ERR,
                "Failed to duplicate incoming PDU.  Refusing to authorize.\n");
            return NETSNMPTRAPD_HANDLER_FINISH;
        }
    }
#endif
    ...
    {
        if (netsnmp_oid_equals(var->name, var->name_length, snmptrapoid,
                               snmptrapoid_len) == 0)
            break;
    }
    ...
    for (i = 0; i < VACM_MAX_VIEWS; i++)
    {
        ...
        if (vacm_check_view_contents(
                newpdu, var->val.objid, var->val_len / sizeof(oid), 0, i,
                VACM_CHECK_VIEW_CONTENTS_DNE_CONTEXT_OK) == VACM_SUCCESS)
        {
