...
class pool_allocator : public std::allocator<T>
{
  ...
  public:
    ...
    pool_allocator(pool_allocator const& that) :
        base_type(static_cast<base_type const&>(that)), m_PooledCount(0)
    {
    }
...
struct attribute_set::implementation
{
  ...
  public:
    ...
    {
        node_list::const_iterator it = that.m_Nodes.begin(),
                                  end = that.m_Nodes.end();
        for (; it != end; ++it)
        {
    ...
    iterator begin()
    {
        return iterator(m_Nodes.begin().pointed_node());
    }
    iterator end()
    {
        return iterator(m_Nodes.end().pointed_node());
    }
    ...
    void erase(iterator it)
    {
        ...
        if (p == b.first)
        {
            ...
            else
            {
                ...
                b.first =
                    value_traits::to_value_ptr(node_traits::get_next(b.first));
            }
        }
        else if (p == b.last)
        {
            ...
            b.last =
                value_traits::to_value_ptr(node_traits::get_previous(b.last));
        }

        m_Nodes.erase_and_dispose(m_Nodes.iterator_to(*p),
                                  disposer(m_Allocator));
    }
