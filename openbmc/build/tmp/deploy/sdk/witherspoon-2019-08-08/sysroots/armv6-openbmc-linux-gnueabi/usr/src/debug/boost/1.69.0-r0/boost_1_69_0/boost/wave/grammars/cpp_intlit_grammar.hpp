...
#if !defined(                                                                  \
    CPP_INTLIT_GRAMMAR_HPP_2E1E70B1_F15C_4132_8554_10A231B0D91C_INCLUDED)
...
#define TRACE_INTLIT_GRAMMAR                                                   \
    bool(BOOST_SPIRIT_DEBUG_FLAGS_CPP &                                        \
         BOOST_SPIRIT_DEBUG_FLAGS_INTLIT_GRAMMAR) /**/

struct intlit_grammar
    : boost::spirit::classic::grammar<intlit_grammar,
                                      closures::intlit_closure::context_t>
{
            ...
            int_lit =
                (sub_int_lit =
                     (ch_p('0')[self.val = 0] >> (hex_lit | oct_lit) |
                      dec_lit) >>
                     !as_lower_d
                         [(ch_p('u')[phx::var(self.is_unsigned) = true] ||
                           ch_p('l')) |
                          (ch_p('l') ||
                           ch_p('u')[phx::var(self.is_unsigned) = true])],

                 hex_lit = (ch_p('X') | ch_p('x')) >>
                           uint_parser<uint_literal_type,
                                       16>()[self.val = phx::arg1,
                                             phx::var(self.is_unsigned) = true],

                 oct_lit = !uint_parser<uint_literal_type,
                                        8>()[self.val = phx::arg1,
                                             phx::var(self.is_unsigned) = true],

                 dec_lit = uint_parser<uint_literal_type, 10>()[self.val =
                                                                    phx::arg1]);
...
BOOST_WAVE_INTLITGRAMMAR_GEN_INLINE uint_literal_type
    intlit_grammar_gen<TokenT>::evaluate(TokenT const& token, bool& is_unsigned)
{
    using namespace boost::spirit::classic;
