...
typedef FT_String* (*TT_Name_ConvertFunc)(TT_Name entry, FT_Memory memory);
...
#define FT_STRUCTURE WOFF_HeaderRec
    ...
    if (FT_ALLOC(sfnt, 12 + woff.num_tables * 16UL) || FT_NEW(sfnt_stream))
        goto Exit;
    ...
    for (nn = 0; nn < woff.num_tables; nn++)
    {
        ...
        FT_TRACE2(("  %c%c%c%c  %08lx  %08lx  %08lx  %08lx\n",
                   (FT_Char)(table->Tag >> 24), (FT_Char)(table->Tag >> 16),
                   (FT_Char)(table->Tag >> 8), (FT_Char)(table->Tag),
                   table->Offset, table->CompLength, table->OrigLength,
                   table->CheckSum));
    ...
    ft_qsort(indices, woff.num_tables, sizeof(WOFF_Table), compare_offsets);
    ...
    for (nn = 0; nn < woff.num_tables; nn++)
    {
        ...
        if (table->Offset != woff_offset || table->CompLength > woff.length ||
    ...
    if (sfnt_offset != woff.totalSfntSize || woff_offset != woff.length)
    {
    ...
    if (FT_REALLOC(sfnt, 12 + woff.num_tables * 16UL, woff.totalSfntSize))
        goto Exit;
    ...
    for (nn = 0; nn < woff.num_tables; nn++)
    {
        ...
        /* Write table data. */
        if (FT_STREAM_SEEK(table->Offset) || FT_FRAME_ENTER(table->CompLength))
            goto Exit;

        if (table->CompLength == table->OrigLength)
        {
            /* Uncompressed data; just copy. */
            ft_memcpy(sfnt + table->OrigOffset, stream->cursor,
                      table->OrigLength);
        }
        else
        {
            ...
            error = FT_Gzip_Uncompress(memory, sfnt + table->OrigOffset,
                                       &output_len, stream->cursor,
                                       table->CompLength);
            if (error)
                goto Exit;
    ...
    FT_Stream_Free(face->root.stream,
...
#define FT_STRUCTURE TTC_HeaderRec
        ...
        FT_FRAME_START(8), FT_FRAME_LONG(version),
...
retry:
    ...
    if (tag != 0x00010000UL && tag != TTAG_ttcf && tag != TTAG_OTTO &&
        tag != TTAG_true && tag != TTAG_typ1 && tag != TTAG_0xA5kbd &&
        tag != TTAG_0xA5lst && tag != 0x00020000UL)
    {
               ...
               FT_Int num_params, FT_Parameter* params)
{
    ...
    if (!face->mm)
    {
        ...
        face->mm =
            ft_module_get_service(tt_module, FT_SERVICE_ID_MULTI_MASTERS, 0);
    }
    ...
    if (!face->var)
    {
        ...
        face->var = ft_module_get_service(tt_module,
                                          FT_SERVICE_ID_METRICS_VARIATIONS, 0);
    }
#endif
...
#ifdef TT_CONFIG_OPTION_GX_VAR_SUPPORT
    {
        ...
        if (face->goto_table(face, TTAG_fvar, stream, &fvar_len) ||
            fvar_len < 20 || FT_READ_ULONG(version) || FT_READ_USHORT(offset) ||
            FT_STREAM_SKIP(2) /* reserved */ || FT_READ_USHORT(num_axes) ||
            FT_READ_USHORT(axis_size) || FT_READ_USHORT(num_instances) ||
        ...
        if (version != 0x00010000UL || axis_size != 20 || num_axes == 0 ||
            ...
            offset + axis_size * num_axes + instance_size * num_instances >
                fvar_len)
            num_instances = 0;
        else
              ...
              FT_ALLOC(instance_values, num_axes * 4)))
        {
            ...
            {
                (void)FT_STREAM_READ_AT(instance_offset, instance_values,
                                        num_axes * 4);
...
#define LOADM_(x, vertical)                                                    \
    do                                                                         \
    {                                                                          \
        FT_TRACE2(("`%s" #x "' ", vertical ? "vertical " : ""));               \
        FT_TRACE3(("-->\n"));                                                  \
                                                                               \
               ...
               FT_Int num_params, FT_Parameter* params)
{
    ...
    /* now set up root fields */
    {
        ...
        if (face->sbit_table_type == TT_SBIT_TABLE_TYPE_CBLC ||
            face->sbit_table_type == TT_SBIT_TABLE_TYPE_SBIX || face->colr)
...
#ifdef TT_CONFIG_OPTION_POSTSCRIPT_NAMES
        if (!psnames_error && face->postscript.FormatType != 0x00030000L)
        ...
        /* set the encoding fields */
        {
            ...
            if (!has_unicode)
            {
                ...
                error = FT_CMap_New((FT_CMap_Class)&tt_cmap_unicode_class_rec,
                                    NULL, &cmaprec, NULL);
                if (error && FT_ERR_NEQ(error, No_Unicode_Glyph_Name) &&
         ...
         */
        {
            ...
            if (count > 0)
            {
                ...
                for (strike_idx = 0; strike_idx < count; strike_idx++)
                {
                    ...
                    error =
                        sfnt->load_strike_metrics(face, strike_idx, &metrics);
                    if (error)
                        continue;
        ...
        if (FT_IS_SCALABLE(root))
        {
             ...
             * values in the horizontal header, although there is nothing
             * stopping the values from being unreliable. Even with a OS/2
             * table, certain fonts set the sTypoAscender, sTypoDescender and
             * sTypoLineGap fields to 0 and instead correctly set usWinAscent
             * and usWinDescent.
             *
             ...
             * have the same values in their sTypo* fields, except ARIALNB.ttf
             * which sets them to 0. All of them have different
             * usWinAscent/Descent values. The OS/2 table therefore cannot be
             * trusted for computing the text height reliably.
             *
            ...
            if (face->os2.version != 0xFFFFU && face->os2.fsSelection & 128)
            {
                ...
                root->height =
                    root->ascender - root->descender + face->os2.sTypoLineGap;
            }
            else
            {
            ...
            root->max_advance_height = (FT_Short)(
                face->vertical_info ? face->vertical.advance_Height_Max
                                    : root->height);
