...
static int testMechAttr(gss_const_OID attr, gss_const_OID_set against)
{
    int present = 0;
    OM_uint32 minor;

    if (GSS_ERROR(generic_gss_test_oid_set_member(
            &minor, attr, (gss_OID_set)against, &present)))
        return 0;
 ...
 */
static int testMechAttrsOffered(gss_const_OID_set desired,
 ...
 */
static int testMechAttrsKnown(gss_const_OID_set critical,
...
OM_uint32 KRB5_CALLCONV gss_indicate_mechs_by_attrs(
    OM_uint32* minor, gss_const_OID_set desired_mech_attrs,
    gss_const_OID_set except_mech_attrs, gss_const_OID_set critical_mech_attrs,
    gss_OID_set* mechs)
{
    ...
    *mechs = GSS_C_NO_OID_SET;
    ...
    for (i = 0; i < allMechs->count; i++)
    {
        ...
        if (testMechAttrsOffered(desired_mech_attrs, except_mech_attrs,
                                 supportedAttrs) &&
            testMechAttrsKnown(critical_mech_attrs, knownAttrs))
        {
            status =
                gss_add_oid_set_member(minor, &allMechs->elements[i], mechs);
            if (GSS_ERROR(status))
            {
...
OM_uint32 KRB5_CALLCONV gss_inquire_attrs_for_mech(
    OM_uint32* minor, gss_const_OID mech_oid, gss_OID_set* mech_attrs,
    gss_OID_set* known_mech_attrs)
{
    ...
    *minor = 0;
    ...
    {
        status = generic_gss_copy_oid_set(minor, gss_ma_known_attrs,
                                          known_mech_attrs);
        if (GSS_ERROR(status))
        {
...
OM_uint32 KRB5_CALLCONV gss_display_mech_attr(OM_uint32* minor,
                                              gss_const_OID mech_attr,
                                              gss_buffer_t name,
                                              gss_buffer_t short_desc,
                                              gss_buffer_t long_desc)
{
    return generic_gss_display_mech_attr(minor, mech_attr, name, short_desc,
                                         long_desc);
}
