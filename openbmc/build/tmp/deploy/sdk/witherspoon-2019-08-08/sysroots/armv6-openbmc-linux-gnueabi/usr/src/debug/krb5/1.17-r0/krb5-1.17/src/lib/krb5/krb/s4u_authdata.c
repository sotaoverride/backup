...
static krb5_error_code s4u2proxy_init(krb5_context kcontext,
                                      void** plugin_context)
{
    *plugin_context = NULL;
    return 0;
...
static void s4u2proxy_flags(krb5_context kcontext, void* plugin_context,
                            krb5_authdatatype ad_type, krb5_flags* flags)
{
    *flags = AD_USAGE_KDC_ISSUED;
}

static void s4u2proxy_fini(krb5_context kcontext, void* plugin_context)
{
    return;
}

static krb5_error_code s4u2proxy_request_init(krb5_context kcontext,
...
static void s4u2proxy_free_internal(krb5_context kcontext,
                                    krb5_authdata_context context,
...
static krb5_error_code s4u2proxy_import_authdata(
    krb5_context kcontext, krb5_authdata_context context, void* plugin_context,
    void* request_context, krb5_authdata** authdata, krb5_boolean kdc_issued,
    krb5_const_principal kdc_issuer)
{
    struct s4u2proxy_context* s4uctx =
        (struct s4u2proxy_context*)request_context;
    krb5_error_code code;
    ...
    s4u2proxy_free_internal(kcontext, context, plugin_context, request_context,
                            s4uctx->delegated);
...
static krb5_error_code s4u2proxy_export_authdata(
    krb5_context kcontext, krb5_authdata_context context, void* plugin_context,
    void* request_context, krb5_flags usage, krb5_authdata*** out_authdata)
{
    struct s4u2proxy_context* s4uctx =
        (struct s4u2proxy_context*)request_context;
    krb5_error_code code;
...
static krb5_error_code
    s4u2proxy_verify(krb5_context kcontext, krb5_authdata_context context,
                     void* plugin_context, void* request_context,
                     const krb5_auth_context* auth_context,
                     const krb5_keyblock* key, const krb5_ap_req* req)
{
    /*
...
static void s4u2proxy_request_fini(krb5_context kcontext,
                                   krb5_authdata_context context,
                                   void* plugin_context, void* request_context)
{
    struct s4u2proxy_context* s4uctx =
...
static krb5_data s4u2proxy_transited_services_attr = {
    KV5M_DATA, sizeof("urn:constrained-delegation:transited-services") - 1,
    "urn:constrained-delegation:transited-services"};

static krb5_error_code s4u2proxy_get_attribute_types(
    krb5_context kcontext, krb5_authdata_context context, void* plugin_context,
    void* request_context, krb5_data** out_attrs)
{
    ...
    code = krb5int_copy_data_contents(
        kcontext, &s4u2proxy_transited_services_attr, &attrs[i++]);
    if (code != 0)
        goto cleanup;
...
static krb5_error_code s4u2proxy_get_attribute(
    krb5_context kcontext, krb5_authdata_context context, void* plugin_context,
    void* request_context, const krb5_data* attribute,
    krb5_boolean* authenticated, krb5_boolean* complete, krb5_data* value,
...
static krb5_error_code
    s4u2proxy_set_attribute(krb5_context kcontext,
                            ...
                            void* request_context, krb5_boolean complete,
...
static krb5_error_code s4u2proxy_export_internal(
    krb5_context kcontext, krb5_authdata_context context, void* plugin_context,
    void* request_context, krb5_boolean restrict_authenticated, void** ptr)
{
    struct s4u2proxy_context* s4uctx =
        (struct s4u2proxy_context*)request_context;
    krb5_error_code code;
    ...
    *ptr = NULL;
    ...
    {
        code =
            krb5_copy_principal(kcontext, s4uctx->delegated[i], &delegated[i]);
        if (code != 0)
            goto cleanup;
    }
...
static krb5_error_code s4u2proxy_size(krb5_context kcontext,
                                      krb5_authdata_context context,
                                      void* plugin_context,
                                      void* request_context, size_t* sizep)
{
    struct s4u2proxy_context* s4uctx =
        (struct s4u2proxy_context*)request_context;
...
static krb5_error_code
    s4u2proxy_externalize(krb5_context kcontext, krb5_authdata_context context,
                          void* plugin_context, void* request_context,
                          krb5_octet** buffer, size_t* lenremain)
{
    ...
    for (i = 0; i < s4uctx->count; i++)
    {
        code = krb5_externalize_opaque(kcontext, KV5M_PRINCIPAL,
                                       (krb5_pointer)s4uctx->delegated[i], &bp,
                                       &remain);
        if (code != 0)
            return code;
    }

    krb5_ser_pack_int32(s4uctx->authenticated, &bp,
...
static krb5_error_code
    s4u2proxy_internalize(krb5_context kcontext, krb5_authdata_context context,
                          void* plugin_context, void* request_context,
                          krb5_octet** buffer, size_t* lenremain)
{
    struct s4u2proxy_context* s4uctx =
...
static krb5_error_code
    s4u2proxy_copy(krb5_context kcontext, krb5_authdata_context context,
                   void* plugin_context, void* request_context,
                   void* dst_plugin_context, void* dst_request_context)
{
    struct s4u2proxy_context* srcctx =
        (struct s4u2proxy_context*)request_context;
    struct s4u2proxy_context* dstctx =
        (struct s4u2proxy_context*)dst_request_context;
    krb5_error_code code;

    code = s4u2proxy_export_internal(kcontext, context, plugin_context,
                                     request_context, FALSE,
                                     (void**)&dstctx->delegated);
