...
static gboolean _g_socket_read_with_control_messages_ready(
    GSocket* socket, GIOCondition condition, gpointer user_data)
{
    ...
    result =
        g_socket_receive_message(socket, NULL, /* address */
                                 &vector, 1, data->messages, data->num_messages,
                                 NULL, g_task_get_cancellable(task), &error);
...
static void _g_socket_read_with_control_messages(
    GSocket* socket, void* buffer, gsize count,
    GSocketControlMessage*** messages, gint* num_messages, gint io_priority,
    GCancellable* cancellable, GAsyncReadyCallback callback, gpointer user_data)
{
    ...
    source = g_socket_create_source(socket, G_IO_IN | G_IO_HUP | G_IO_ERR,
                                    cancellable);
    g_task_attach_source(
        task, source, (GSourceFunc)_g_socket_read_with_control_messages_ready);
    g_source_unref(source);
}

static gssize _g_socket_read_with_control_messages_finish(GSocket* socket,
                                                          GAsyncResult* result,
                                                          GError** error)
{
...
static void release_required_types(void)
{
...
static void ensure_required_types(void)
{
     ...
     * is work on a more general approach in
...
static gpointer gdbus_shared_thread_func(gpointer user_data)
{
...
static SharedThreadData* _g_dbus_shared_thread_ref(void)
{
    ...
    if (g_once_init_enter(&shared_thread_data))
    {
        ...
        data->thread = g_thread_new("gdbus", gdbus_shared_thread_func, data);
...
static void _g_dbus_shared_thread_unref(SharedThreadData* data)
{
...
struct GDBusWorker
{
    ...
    /* TODO: frozen (e.g. G_DBUS_CONNECTION_FLAGS_DELAY_MESSAGE_PROCESSING)
     * currently only affects messages received from the other peer (since
     * GDBusServer is the only user) - we might want it to affect messages sent
     * to the other peer too?
     */
    gboolean frozen;
     ...
     * Only the worker thread may change its value, and only with the
     * write_lock. Other threads may read its value when holding the write_lock.
...
static void _g_dbus_worker_unref(GDBusWorker* worker)
{
    if (g_atomic_int_dec_and_test(&worker->ref_count))
    {
        ...
        g_queue_free_full(worker->received_messages_while_frozen,
                          (GDestroyNotify)g_object_unref);
        g_mutex_clear(&worker->write_lock);
        g_queue_free_full(worker->write_queue,
                          (GDestroyNotify)message_to_write_data_free);
...
static void _g_dbus_worker_emit_disconnected(GDBusWorker* worker,
                                             gboolean remote_peer_vanished,
                                             GError* error)
{
    if (!g_atomic_int_get(&worker->stopped))
        worker->disconnected_callback(worker, remote_peer_vanished, error,
                                      worker->user_data);
}

static void _g_dbus_worker_emit_message_received(GDBusWorker* worker,
                                                 ...
                                                 GDBusMessage* message)
{
    GDBusMessage* ret;
    if (!g_atomic_int_get(&worker->stopped))
        ret = worker->message_about_to_be_sent_callback(worker, message,
                                                        worker->user_data);
    else
        ret = message;
    return ret;
...
/* can only be called from private thread with read-lock held - takes ownership
 * of @message */
static void
...
{
    if (worker->frozen ||
        g_queue_get_length(worker->received_messages_while_frozen) > 0)
    {
        /* queue up */
...
/* called in private thread shared by all GDBusConnection instances (without
 * read-lock held) */
static gboolean unfreeze_in_idle_cb(gpointer user_data)
{
    ...
    if (worker->frozen)
    {
        while ((message = g_queue_pop_head(
                    worker->received_messages_while_frozen)) != NULL)
        {
            _g_dbus_worker_emit_message_received(worker, message);
            g_object_unref(message);
        }
    ...
    {
        g_assert(g_queue_get_length(worker->received_messages_while_frozen) ==
                 0);
    }
...
void _g_dbus_worker_unfreeze(GDBusWorker* worker)
{
    ...
    g_source_set_callback(idle_source, unfreeze_in_idle_cb,
...
/* called in private thread shared by all GDBusConnection instances (without
 * read-lock held) */
static void _g_dbus_worker_do_read_cb(GInputStream* input_stream,
                                      GAsyncResult* res, gpointer user_data)
{
    ...
    if (worker->socket == NULL)
        bytes_read = g_input_stream_read_finish(
            g_io_stream_get_input_stream(worker->stream), res, &error);
    else
    ...
    if (worker->read_num_ancillary_messages > 0)
    {
        ...
        for (n = 0; n < worker->read_num_ancillary_messages; n++)
        {
            GSocketControlMessage* control_message =
                G_SOCKET_CONTROL_MESSAGE(worker->read_ancillary_messages[n]);
            ...
            else if (G_IS_UNIX_FD_MESSAGE(control_message))
            {
                ...
                {
                    worker->read_fd_list =
                        g_unix_fd_list_new_from_array(fds, num_fds);
                }
                else
                {
                    gint n;
                    for (n = 0; n < num_fds; n++)
                    {
                        ...
                        g_unix_fd_list_append(worker->read_fd_list, fds[n],
                                              NULL);
            ...
            else
            {
                if (error == NULL)
                {
                    g_set_error(
                        ...
                        "Unexpected ancillary message of type %s received from "
                        "peer",
    ...
    if (worker->read_buffer_bytes_wanted == worker->read_buffer_cur_size)
    {
        ...
        if (worker->read_buffer_bytes_wanted == 16)
        {
            ...
            message_len = g_dbus_message_bytes_needed(
                (guchar*)worker->read_buffer, 16, &error);
            if (message_len == -1)
            {
                g_warning("_g_dbus_worker_do_read_cb: error determining bytes "
                          "needed: %s",
                          error->message);
        ...
        else
        {
            ...
            message = g_dbus_message_new_from_blob(
                (guchar*)worker->read_buffer, worker->read_buffer_cur_size,
                worker->capabilities, &error);
            if (message == NULL)
            {
                gchar* s;
                s = _g_dbus_hexdump(worker->read_buffer,
                                    worker->read_buffer_cur_size, 2);
                g_warning("Error decoding D-Bus message of %" G_GSIZE_FORMAT
                          " bytes\n"
                          "The error is: %s\n"
            ...
            if (G_UNLIKELY(_g_dbus_debug_message()))
            {
                ...
                g_print("======================================================"
                        "==================\n"
                        "GDBus-debug:Message:\n"
                        "  <<<< RECEIVED D-Bus message (%" G_GSIZE_FORMAT
                        " bytes)\n",
...
/* called in private thread shared by all GDBusConnection instances (with
 * read-lock held) */
static void _g_dbus_worker_do_read_unlocked(GDBusWorker* worker)
{
    ...
    {
        /* TODO: 4096 is randomly chosen; might want a better chosen default
         * minimum */
        worker->read_buffer_allocated_size =
            MAX(worker->read_buffer_bytes_wanted, 4096);
        worker->read_buffer =
            g_realloc(worker->read_buffer, worker->read_buffer_allocated_size);
    }
    ...
    if (worker->socket == NULL)
        g_input_stream_read_async(
            ...
            G_PRIORITY_DEFAULT, worker->cancellable,
    ...
    else
    {
        ...
        _g_socket_read_with_control_messages(
            worker->socket, worker->read_buffer + worker->read_buffer_cur_size,
            worker->read_buffer_bytes_wanted - worker->read_buffer_cur_size,
            &worker->read_ancillary_messages,
            &worker->read_num_ancillary_messages, G_PRIORITY_DEFAULT,
            worker->cancellable, (GAsyncReadyCallback)_g_dbus_worker_do_read_cb,
            _g_dbus_worker_ref(worker));
    }
}
 ...
 */
static void write_message_async_cb(GObject* source_object, GAsyncResult* res,
                                   gpointer user_data)
{
    ...
    /* Note: we can't access data->task after calling g_task_return_* () because
     * the callback can free @data and we're not completing in idle. So use a
     * copy of the pointer.
     */
    task = data->task;
 ...
 */
#ifdef G_OS_UNIX
static gboolean on_socket_ready(GSocket* socket, GIOCondition condition,
                                gpointer user_data)
{
    MessageToWriteData* data = user_data;
    write_message_continue_writing(data);
...
static void write_message_continue_writing(MessageToWriteData* data)
{
    ...
    else if (G_IS_SOCKET_OUTPUT_STREAM(ostream) && data->total_written == 0)
    {
        ...
        bytes_written = g_socket_send_message(
            data->worker->socket, NULL, /* address */
            &vector, 1, control_message != NULL ? &control_message : NULL,
            control_message != NULL ? 1 : 0, G_SOCKET_MSG_NONE,
            data->worker->cancellable, &error);
        ...
        if (bytes_written == -1)
        {
            ...
            if (g_error_matches(error, G_IO_ERROR, G_IO_ERROR_WOULD_BLOCK))
            {
                ...
                g_source_set_callback(source, (GSourceFunc)on_socket_ready,
                                      data, NULL); /* GDestroyNotify */
    ...
    else
    {
        ...
        g_output_stream_write_async(
            ostream, (const gchar*)data->blob + data->total_written,
            data->blob_size - data->total_written, G_PRIORITY_DEFAULT,
            data->worker->cancellable, write_message_async_cb, data);
    }
#ifdef G_OS_UNIX
out:
#endif
 ...
 * output_pending is PENDING_WRITE on entry
 */
static void write_message_async(GDBusWorker* worker, MessageToWriteData* data,
...
static void flush_data_list_complete(const GList* flushers, const GError* error)
{
    const GList* l;
 ...
 */
static void ostream_flush_cb(GObject* source_object, GAsyncResult* res,
                             gpointer user_data)
{
    ...
    data->worker->write_num_messages_flushed =
        data->worker->write_num_messages_written;
 ...
 * output_pending is PENDING_FLUSH on entry
 */
static void start_flush(FlushAsyncData* data)
{
    g_output_stream_flush_async(
        g_io_stream_get_output_stream(data->worker->stream), G_PRIORITY_DEFAULT,
        data->worker->cancellable, ostream_flush_cb, data);
}
 ...
 */
static void message_written_unlocked(GDBusWorker* worker,
 ...
 */
static FlushAsyncData* prepare_flush_unlocked(GDBusWorker* worker)
{
    GList* l;
    GList* ll;
    GList* flushers;
 ...
 * output_pending is PENDING_WRITE on entry
 */
static void write_message_cb(GObject* source_object, GAsyncResult* res,
                             gpointer user_data)
{
    MessageToWriteData* data = user_data;
    GError* error;
 ...
 */
static void iostream_close_cb(GObject* source_object, GAsyncResult* res,
                              gpointer user_data)
{
    GDBusWorker* worker = user_data;
    GError* error = NULL;
    ...
    while (pending_close_attempts != NULL)
    {
        ...
        pending_close_attempts =
            g_list_delete_link(pending_close_attempts, pending_close_attempts);
...
write_next:
    ...
    if (worker->pending_close_attempts != NULL)
    {
        ...
        if (!g_input_stream_has_pending(input))
        {
            ...
            g_io_stream_close_async(worker->stream, G_PRIORITY_DEFAULT, NULL,
                                    iostream_close_cb,
                                    _g_dbus_worker_ref(worker));
        }
    }
    ...
    else if (data != NULL)
    {
        ...
        data->message =
            _g_dbus_worker_emit_message_about_to_be_sent(worker, data->message);
        ...
        else
        {
            ...
            error = NULL;
            new_blob = g_dbus_message_to_blob(data->message, &new_blob_size,
                                              worker->capabilities, &error);
            if (new_blob == NULL)
            {
                /* if filter make the GDBusMessage unencodeable, just complain
                 * on stderr and send the old message instead
                 */
                g_warning("Error encoding GDBusMessage with serial %d altered "
                          "by filter function: %s",
 ...
 */
static gboolean continue_writing_in_idle_cb(gpointer user_data)
{
    GDBusWorker* worker = user_data;
 ...
 * output_pending may be anything
 */
static void schedule_writing_unlocked(GDBusWorker* worker,
                                      ...
                                      CloseData* close_data)
{
    ...
    if (flush_data != NULL)
        worker->write_pending_flushes =
            g_list_prepend(worker->write_pending_flushes, flush_data);

    if (close_data != NULL)
        worker->pending_close_attempts =
            g_list_prepend(worker->pending_close_attempts, close_data);
    ...
    if (worker->output_pending == PENDING_NONE)
    {
        ...
        idle_source = g_idle_source_new();
        g_source_set_priority(idle_source, G_PRIORITY_DEFAULT);
        g_source_set_callback(idle_source, continue_writing_in_idle_cb,
                              _g_dbus_worker_ref(worker),
                              (GDestroyNotify)_g_dbus_worker_unref);
 ...
 * output_pending may be anything
 */
void _g_dbus_worker_send_message(GDBusWorker* worker, GDBusMessage* message,
                                 gchar* blob, gsize blob_len)
{
    MessageToWriteData* data;
...
GDBusWorker* _g_dbus_worker_new(
    GIOStream* stream, GDBusCapabilityFlags capabilities,
    ...
    GDBusWorkerDisconnectedCallback disconnected_callback, gpointer user_data)
{
    ...
    worker->message_about_to_be_sent_callback =
    ...
    if (G_IS_SOCKET_CONNECTION(worker->stream))
        worker->socket =
            g_socket_connection_get_socket(G_SOCKET_CONNECTION(worker->stream));
    ...
    idle_source = g_idle_source_new();
    g_source_set_priority(idle_source, G_PRIORITY_DEFAULT);
    g_source_set_callback(idle_source, _g_dbus_worker_do_initial_read,
                          _g_dbus_worker_ref(worker),
                          (GDestroyNotify)_g_dbus_worker_unref);
 ...
 * output_pending may be anything
 */
void _g_dbus_worker_close(GDBusWorker* worker, GTask* task)
{
    CloseData* close_data;
                                   ...
                                   GCancellable* cancellable, GError** error)
{
    ...
    if (pending_writes > 0 || worker->write_num_messages_written !=
                                  worker->write_num_messages_flushed)
    {
        data = g_new0(FlushData, 1);
        g_mutex_init(&data->mutex);
        g_cond_init(&data->cond);
        data->number_to_wait_for =
            worker->write_num_messages_written + pending_writes;
...
void _g_dbus_initialize(void)
{
    ...
    if (g_once_init_enter(&initialized))
    {
        ...
        if (debug != NULL)
        {
            ...
            _gdbus_debug_flags =
                g_parse_debug_string(debug, keys, G_N_ELEMENTS(keys));
...
gchar* _g_dbus_win32_get_user_sid(void)
{
    ...
    {
        g_warning("OpenProcessToken failed with error code %d",
                  (gint)GetLastError());
        goto out;
    }
    ...
    if (!GetTokenInformation(h, TokenUser, NULL, 0, &token_information_len))
    {
        ...
        {
            g_warning("GetTokenInformation() failed with error code %d",
                      (gint)GetLastError());
            goto out;
        }
    }
    user = g_malloc(token_information_len);
    if (!GetTokenInformation(h, TokenUser, user, token_information_len,
...
gchar* _g_dbus_get_machine_id(GError** error)
{
    ...
    *dest = 0;
    ...
    ret = NULL;
    first_error = NULL;
    if (!g_file_get_contents("/var/lib/dbus/machine-id", &ret, NULL,
                             &first_error) &&
        !g_file_get_contents("/etc/machine-id", &ret, NULL, NULL))
    {
                                               ...
                                               GDBusWorker* worker)
{
    ...
    if (size >= 16)
        message_length = g_dbus_message_bytes_needed(
            (guchar*)worker->read_buffer, size, NULL);
    if (size >= 1)
    {
        switch (worker->read_buffer[0])
        {
            case 'l':
                if (size >= 12)
                    serial =
                        GUINT32_FROM_LE(((guint32*)worker->read_buffer)[2]);
                break;
            case 'B':
                if (size >= 12)
                    serial =
                        GUINT32_FROM_BE(((guint32*)worker->read_buffer)[2]);
                break;
            default:
    ...
    g_print("=================================================================="
            ...
            bytes_read, serial, message_length, worker->read_buffer_cur_size,
            g_type_name(G_TYPE_FROM_INSTANCE(
                g_io_stream_get_input_stream(worker->stream))));
    _g_dbus_debug_print_unlock();
out:;
...
gchar* _g_dbus_hexencode(const gchar* str, gsize str_len)
{
    gsize n;
    GString* s;
