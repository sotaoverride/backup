...
static int parse_env_file_internal(FILE* f, const char* fname,
                                   int (*push)(const char* filename,
                                               unsigned line, const char* key,
                                   ...
                                   void* userdata, int* n_pushed)
{
    ...
    if (IN_SET(state, PRE_VALUE, VALUE, VALUE_ESCAPE, SINGLE_QUOTE_VALUE,
...
static int check_utf8ness_and_warn(const char* filename, unsigned line,
...
int parse_env_file_sentinel(FILE* f, const char* fname, ...)
{
...
static int load_env_file_push_pairs(const char* filename, unsigned line,
                                    const char* key, char* value,
                                    void* userdata, int* n_pushed)
{
    char*** m = userdata;
...
static int merge_env_file_push(const char* filename, unsigned line,
                               const char* key, char* value, void* userdata,
                               int* n_pushed)
{
    ...
    if (!value)
    {
        log_error("%s:%u: invalid syntax (around \"%s\"), ignoring.",
                  strna(filename), line, key);
        return 0;
    }

    if (!env_name_is_valid(key))
    {
        log_error("%s:%u: invalid variable name \"%s\", ignoring.",
                  strna(filename), line, key);
        free(value);
        return 0;
    }
