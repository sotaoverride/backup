...
/* Prototype for the setgrent functions we use here.  */
typedef enum nss_status (*get_function)(struct group*, char*, size_t, int*);

static enum nss_status compat_call(service_user* nip, const char* user,
                                   ...
                                   gid_t** groupsp, long int limit, int* errnop)
{
    ...
    do
    {
        while ((status = DL_CALL_FCT(getgrent_fct, (&grpbuf, tmpbuf.data,
                ...
                if (strcmp(*m, user) == 0)
                {
                    ...
                    if (cnt == *start)
                    {
                        ...
                        if (__glibc_unlikely(*start == *size))
                        {
                            ...
                            newgroups =
                                realloc(groups, newsize * sizeof(*groups));
                            if (newgroups == NULL)
                                goto done;
