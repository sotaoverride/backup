...
/*
** set password functions added by Paul W. Nelson, Thursby Software Systems,
*Inc.
*/
                                    ...
                                    krb5_data* packet)
{
    ...
    if ((ret =
             krb5_mk_priv(context, auth_context, &clearpw, &cipherpw, &replay)))
        goto cleanup;
 ...
 * *edata_out. */
static krb5_error_code get_error_edata(krb5_context context,
                                       const krb5_data* error_packet,
                                       krb5_data** edata_out)
{
    krb5_error_code ret;
...
static krb5_error_code get_clear_result(krb5_context context,
                                        krb5_auth_context auth_context,
...
krb5_error_code KRB5_CALLCONV krb5_chpw_result_code_string(krb5_context context,
                                                           int result_code,
                                                           char** code_string)
{
    switch (result_code)
                                     ...
                                     const char* passwd, krb5_data* packet)
{
    ...
    if ((ret = krb5_mk_priv(context, auth_context, encoded_setpw, &cipherpw,
                            NULL)) != 0)
    {
...
static void add_spaces(struct k5buf* buf)
{
    if (buf->len > 0)
        k5_buf_add(buf, "  ");
}

static krb5_error_code decode_ad_policy_info(const krb5_data* data,
                                             char** msg_out)
{
    ...
    *msg_out = NULL;
    ...
    if (policy.password_history)
    {
        add_spaces(&buf);
        k5_buf_add_fmt(&buf,
                       ngettext("The password must be different from "
    ...
    if (policy.min_passwordage)
    {
        ...
        add_spaces(&buf);
        k5_buf_add_fmt(&buf,
                       ngettext("The password can only be changed once "
                                "a day.",
                                                ...
                                                char** message_out)
{
    ...
    *message_out = NULL;
    ...
    if (server_string->length > 0 &&
        ...
        krb5int_utf8_normalize(server_string, &string, KRB5_UTF8_APPROX) == 0)
    {
