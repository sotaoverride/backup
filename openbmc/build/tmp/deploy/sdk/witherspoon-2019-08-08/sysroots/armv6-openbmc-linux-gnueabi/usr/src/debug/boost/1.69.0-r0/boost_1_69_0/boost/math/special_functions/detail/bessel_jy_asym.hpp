...
T asymptotic_bessel_phase_mx(T v, T x)
{
    ...
    // which we'll factor in later when we calculate the sines/cosines of the
    // result:
    //
    T mu = 4 * v * v;
    T denom = 4 * x;
...
inline T asymptotic_bessel_y_large_x_2(T v, T x)
{
    ...
    T sin_phase =
        sin(phase) * (cx * ci + sx * si) + cos(phase) * (sx * ci - cx * si);
...
inline T asymptotic_bessel_j_large_x_2(T v, T x)
{
    ...
    T si = sin_pi(v / 2 + 0.25f);
    T sin_phase =
        cos(phase) * (cx * ci + sx * si) - sin(phase) * (sx * ci - cx * si);
...
inline bool asymptotic_bessel_large_x_limit(const T& v, const T& x)
{
    ...
    // At double precision eps^1/8 ~= 0.01.
    //
    return (std::max)(T(fabs(v)), T(1)) <
           x * sqrt(tools::forth_root_epsilon<T>());
}
...
{
    T c = 1;
    T p = (v / boost::math::sin_pi(v, pol)) * pow(x / 2, -v) /
          boost::math::tgamma(1 - v, pol);
    T q = (v / boost::math::sin_pi(v, pol)) * pow(x / 2, v) /
          boost::math::tgamma(1 + v, pol);
    T f = (p - q) / v;
...
T asymptotic_bessel_i_large_x(T v, T x, const Policy& pol)
{
    ...
    return (boost::math::isfinite)(s)
               ...
               : policies::raise_overflow_error<T>(
                     "boost::math::asymptotic_bessel_i_large_x<%1%>(%1%,%1%)",
                     0, pol);
}
