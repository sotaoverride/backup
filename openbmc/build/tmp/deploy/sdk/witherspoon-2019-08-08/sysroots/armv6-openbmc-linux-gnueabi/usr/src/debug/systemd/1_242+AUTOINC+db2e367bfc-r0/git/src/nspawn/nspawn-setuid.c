...
static int spawn_getent(const char* database, const char* key, pid_t* rpid)
{
    ...
    r = safe_fork("(getent)", FORK_RESET_SIGNALS | FORK_DEATHSIG | FORK_LOG,
                  &pid);
    if (r < 0)
    {
...
int change_uid_gid_raw(uid_t uid, gid_t gid, const gid_t* supplementary_gids,
...
int change_uid_gid(const char* user, char** _home)
{
    ...
    *x = 0;
    ...
    FOREACH_WORD(word, l, x, state)
    {
        ...
        if (r < 0)
            return log_error_errno(
                r, "Failed to parse group data from getent: %m");
    }
