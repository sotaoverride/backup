...
struct has_setup
{
  private:
    template <typename U>
    static char Test(fixture_detect<U, &U::setup>*);
    template <typename U>
    static int Test(...);
...
struct has_teardown
{
  private:
    template <typename U>
    static char Test(fixture_detect<U, &U::teardown>*);
    template <typename U>
    static int Test(...);
...
struct has_setup
{
  private:
    template <typename U>
    static auto Test(U*) ->
        typename fixture_detect<decltype(boost::declval<U>().setup())>::type;
    template <typename U>
    static int Test(...);
...
struct has_teardown
{
  private:
    template <typename U>
    static auto Test(U*) ->
        typename fixture_detect<decltype(boost::declval<U>().teardown())>::type;
    template <typename U>
    static int Test(...);
...
//! Calls the fixture "setup" if detected by the compiler, otherwise does
//! nothing.
template <class U>
...
//! Calls the fixture "teardown" if detected by the compiler, otherwise does
//! nothing.
template <class U>
void teardown_conditional(U& u)
{
    return impl_fixture::call_teardown<impl_fixture::has_teardown<U>::value>()(
        u);
}
...
class function_based_fixture : public test_unit_fixture
{
  public:
    // Constructor
    function_based_fixture(boost::function<void()> const& setup_,
                           boost::function<void()> const& teardown_) :
        m_setup(setup_),
