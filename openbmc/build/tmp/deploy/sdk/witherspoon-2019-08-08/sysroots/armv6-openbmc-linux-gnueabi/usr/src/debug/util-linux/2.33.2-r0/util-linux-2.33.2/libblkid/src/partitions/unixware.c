                             ...
                             __attribute__((__unused__)))
{
    ...
    l = (struct unixware_disklabel*)blkid_probe_get_sector(pr, UNIXWARE_SECTOR);
    if (!l)
    {
        if (errno)
            return -errno;
     ...
     */
    for (i = 1, p = &l->vtoc.v_slice[1]; i < UNIXWARE_MAXPARTITIONS; i++, p++)
    {
        ...
        if (tag == UNIXWARE_TAG_UNUSED || tag == UNIXWARE_TAG_ENTIRE_DISK ||
        ...
        {
            DBG(LOWPROBE, ul_debug("WARNING: unixware partition (%d) overflow "
                                   "detected, ignore",
                                   i));
            continue;
        }
...
const struct blkid_idinfo unixware_pt_idinfo = {
    ...
    .magics = {{.magic = "\x0D\x60\xE5\xCA", /* little-endian magic string */
