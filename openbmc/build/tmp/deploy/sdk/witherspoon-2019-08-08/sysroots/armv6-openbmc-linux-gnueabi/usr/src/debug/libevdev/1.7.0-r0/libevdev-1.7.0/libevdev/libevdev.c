                       ...
                       int line, const char* func, const char* format, ...)
{
    ...
    if (dev && dev->log.device_handler)
        dev->log.device_handler(dev, priority, dev->log.userdata, file, line,
                                func, format, args);
    else
...
LIBEVDEV_EXPORT int libevdev_new_from_fd(int fd, struct libevdev** dev)
{
    struct libevdev* d;
    int rc;
...
LIBEVDEV_EXPORT void libevdev_set_log_function(libevdev_log_func_t logfunc,
                                               void* data)
{
...
LIBEVDEV_EXPORT enum libevdev_log_priority libevdev_get_log_priority(void)
{
    return log_data.priority;
}

LIBEVDEV_EXPORT void libevdev_set_device_log_function(
    struct libevdev* dev, libevdev_device_log_func_t logfunc,
...
enum libevdev_log_priority _libevdev_log_priority(const struct libevdev* dev)
{
    if (dev && dev->log.device_handler)
...
static int update_led_state(struct libevdev* dev, const struct input_event* e)
{
    if (!libevdev_has_event_type(dev, EV_LED))
        return 1;
...
static int update_sw_state(struct libevdev* dev, const struct input_event* e)
{
    if (!libevdev_has_event_type(dev, EV_SW))
        return 1;
                 ...
                 (ev->value < 0 || ev->value >= dev->num_slots)))
    {
        ...
        /* Drop any invalid tracking IDs, they are only supposed to go from
           ...
           */
                 ...
                 ((ev->value == -1 && *slot_value(dev, dev->current_slot,
...
LIBEVDEV_EXPORT const char* libevdev_get_phys(const struct libevdev* dev)
{
    return dev->phys;
}

LIBEVDEV_EXPORT const char* libevdev_get_uniq(const struct libevdev* dev)
{
    return dev->uniq;
}

#define STRING_SETTER(field)                                                   \
    LIBEVDEV_EXPORT void libevdev_set_##field(struct libevdev* dev,            \
                                              const char* field)               \
    {                                                                          \
        if (field == NULL)                                                     \
            return;                                                            \
...
#define PRODUCT_SETTER(field)                                                  \
    LIBEVDEV_EXPORT void libevdev_set_id_##field(struct libevdev* dev,         \
                                                 int field)                    \
    {                                                                          \
        dev->ids.field = field;                                                \
    }
...
LIBEVDEV_EXPORT int libevdev_get_driver_version(const struct libevdev* dev)
{
    return dev->driver_version;
}

LIBEVDEV_EXPORT int libevdev_has_property(const struct libevdev* dev,
                                          unsigned int prop)
{
    return (prop <= INPUT_PROP_MAX) && bit_is_set(dev->props, prop);
}

LIBEVDEV_EXPORT int libevdev_enable_property(struct libevdev* dev,
                                             unsigned int prop)
{
...
LIBEVDEV_EXPORT int libevdev_get_event_value(const struct libevdev* dev,
                                             unsigned int type,
                                             unsigned int code)
{
    int value = 0;
...
LIBEVDEV_EXPORT int libevdev_set_event_value(struct libevdev* dev,
                                             unsigned int type,
...
LIBEVDEV_EXPORT int libevdev_fetch_event_value(const struct libevdev* dev,
                                               unsigned int type,
                                               unsigned int code, int* value)
{
    if (libevdev_has_event_type(dev, type) &&
...
LIBEVDEV_EXPORT int libevdev_get_slot_value(const struct libevdev* dev,
                                            unsigned int slot,
                                            unsigned int code)
{
    if (!libevdev_has_event_type(dev, EV_ABS) ||
...
LIBEVDEV_EXPORT int libevdev_set_slot_value(struct libevdev* dev,
                                            unsigned int slot,
                                            unsigned int code, int value)
{
    if (!libevdev_has_event_type(dev, EV_ABS) ||
...
LIBEVDEV_EXPORT int libevdev_fetch_slot_value(const struct libevdev* dev,
                                              unsigned int slot,
                                              unsigned int code, int* value)
{
    if (libevdev_has_event_type(dev, EV_ABS) &&
        libevdev_has_event_code(dev, EV_ABS, code) && dev->num_slots >= 0 &&
...
LIBEVDEV_EXPORT int libevdev_get_current_slot(const struct libevdev* dev)
{
...
#define ABS_GETTER(name)                                                       \
    LIBEVDEV_EXPORT int libevdev_get_abs_##name(const struct libevdev* dev,    \
                                                unsigned int code)             \
    {                                                                          \
...
#define ABS_SETTER(field)                                                      \
    LIBEVDEV_EXPORT void libevdev_set_abs_##field(struct libevdev* dev,        \
...
LIBEVDEV_EXPORT void libevdev_set_abs_info(struct libevdev* dev,
                                           unsigned int code,
...
LIBEVDEV_EXPORT int libevdev_disable_event_type(struct libevdev* dev,
                                                unsigned int type)
{
    int max;
...
LIBEVDEV_EXPORT int libevdev_disable_event_code(struct libevdev* dev,
                                                unsigned int type,
                                                unsigned int code)
{
    unsigned int max;
...
LIBEVDEV_EXPORT int libevdev_grab(struct libevdev* dev,
...
LIBEVDEV_EXPORT const char* libevdev_event_value_get_name(unsigned int type,
                                                          unsigned int code,
                                                          int value)
{
...
LIBEVDEV_EXPORT int libevdev_get_repeat(const struct libevdev* dev, int* delay,
                                        int* period)
{
...
LIBEVDEV_EXPORT int libevdev_kernel_set_led_value(struct libevdev* dev,
                                                  unsigned int code,
...
LIBEVDEV_EXPORT int libevdev_kernel_set_led_values(struct libevdev* dev, ...)
{
...
LIBEVDEV_EXPORT int libevdev_set_clock_id(struct libevdev* dev, int clockid)
{
    if (!dev->initialized)
    {
        log_bug(dev, "device not initialized. call libevdev_set_fd() first\n");
