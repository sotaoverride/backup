...
static krb5_error_code pkinit_encode_dh_params(const BIGNUM*, const BIGNUM*,
                                               const BIGNUM*, uint8_t**,
                                               unsigned int*);
...
static krb5_error_code
    pkinit_sign_data(krb5_context context,
                     pkinit_identity_crypto_context cryptoctx,
                     unsigned char* data, unsigned int data_len,
                     unsigned char** sig, unsigned int* sig_len);

static krb5_error_code create_signature(unsigned char**, unsigned int*,
...
static krb5_error_code
    pkinit_decode_data(krb5_context context,
                       ...
                       uint8_t** decoded, unsigned int* decoded_len);
...
static int prepare_enc_data(const uint8_t* indata, int indata_len,
                            uint8_t** outdata, int* outdata_len);

static int openssl_callback(int, X509_STORE_CTX*);
static int openssl_callback_ignore_crls(int, X509_STORE_CTX*);

static int pkcs7_decrypt(krb5_context context,
                         pkinit_identity_crypto_context id_cryptoctx, PKCS7* p7,
                         BIO* bio);

static BIO* pkcs7_dataDecode(krb5_context context,
                             pkinit_identity_crypto_context id_cryptoctx,
                             PKCS7* p7);

static ASN1_OBJECT*
    pkinit_pkcs7type2oid(pkinit_plg_crypto_context plg_cryptoctx,
                         int pkcs7_type);
...
static krb5_error_code pkinit_create_sequence_of_principal_identifiers(
    ...
    pkinit_identity_crypto_context id_cryptoctx, int type,
    krb5_pa_data*** e_data_out);

#ifndef WITHOUT_PKCS11
static krb5_error_code pkinit_find_private_key(pkinit_identity_crypto_context,
                                               CK_ATTRIBUTE_TYPE usage,
                                               CK_OBJECT_HANDLE* objp);
static krb5_error_code pkinit_login(krb5_context context,
                                    pkinit_identity_crypto_context id_cryptoctx,
                                    CK_TOKEN_INFO* tip, const char* password);
static krb5_error_code
    pkinit_open_session(krb5_context context,
                        pkinit_identity_crypto_context id_cryptoctx);
static void* pkinit_C_LoadModule(const char* modname,
                                 CK_FUNCTION_LIST_PTR_PTR p11p);
static CK_RV pkinit_C_UnloadModule(void* handle);
#ifdef SILLYDECRYPT
CK_RV pkinit_C_Decrypt(pkinit_identity_crypto_context id_cryptoctx,
...
static krb5_error_code
    create_identifiers_from_stack(STACK_OF(X509) * sk,
                                  krb5_external_principal_identifier*** ids);
static int wrap_signeddata(unsigned char* data, unsigned int data_len,
                           unsigned char** out, unsigned int* out_len);

static char* pkinit_pkcs11_code_to_text(int err);
...
#define pkinit_CMS_free1_certs(_sk_x509) sk_X509_pop_free((_sk_x509), X509_free)
...
#define pkinit_CMS_get0_content_signed(_p7)                                    \
    (&((_p7)->d.sign->contents->d.other->value.octet_string))
#define pkinit_CMS_get0_content_data(_p7)                                      \
    (&((_p7)->d.other->value.octet_string))
...
static void compat_dh_get0_pqg(const DH* dh, const BIGNUM** p, const BIGNUM** q,
                               const BIGNUM** g)
{
    if (p != NULL)
        *p = dh->p;
 ...
 */
static krb5_error_code oerr_cert(krb5_context context, krb5_error_code code,
                                 X509_STORE_CTX* certctx, const char* msg)
{
...
krb5_error_code pkinit_init_plg_crypto(pkinit_plg_crypto_context* cryptoctx)
{
...
void pkinit_fini_plg_crypto(pkinit_plg_crypto_context cryptoctx)
{
...
void pkinit_fini_identity_crypto(pkinit_identity_crypto_context idctx)
{
    if (idctx == NULL)
        return;
...
krb5_error_code pkinit_init_req_crypto(pkinit_req_crypto_context* cryptoctx)
{
    krb5_error_code retval = ENOMEM;
...
void pkinit_fini_req_crypto(pkinit_req_crypto_context req_cryptoctx)
{
...
static krb5_error_code pkinit_init_pkinit_oids(pkinit_plg_crypto_context ctx)
{
...
static krb5_error_code get_cert(char* filename, X509** retcert)
{
    X509* cert = NULL;
    BIO* tmp = NULL;
    int code;
...
static int get_key_cb(char* buf, int size, int rwflag, void* userdata)
{
    ...
    if (data->id_cryptoctx->defer_id_prompt)
    {
        ...
        pkinit_set_deferred_id(&data->id_cryptoctx->deferred_ids, data->fsname,
                               0, NULL);
        return -1;
    }
    if (data->password == NULL)
    {
        ...
        if (data->id_cryptoctx->prompter == NULL)
            return -1;
        if (asprintf(&prompt, "%s %s", _("Pass phrase for"), data->filename) <
            0)
            return -1;
        rdat.data = buf;
        rdat.length = size;
...
static krb5_error_code get_key(krb5_context context,
                               pkinit_identity_crypto_context id_cryptoctx,
                               char* filename, const char* fsname,
...
static void pkinit_fini_pkinit_oids(pkinit_plg_crypto_context ctx)
{
    if (ctx == NULL)
        return;
...
static DH* make_oakley_dh(uint8_t* prime, size_t len)
{
    DH* dh = NULL;
...
static krb5_error_code pkinit_init_dh_params(pkinit_plg_crypto_context plgctx)
{
    krb5_error_code retval = ENOMEM;
...
static void pkinit_fini_dh_params(pkinit_plg_crypto_context plgctx)
{
    if (plgctx->dh_1024 != NULL)
...
static krb5_error_code create_contentinfo(krb5_context context,
                                          ASN1_OBJECT* oid, unsigned char* data,
                                          size_t data_len, PKCS7** p7_out)
{
    PKCS7* p7 = NULL;
...
krb5_error_code
    cms_contentinfo_create(krb5_context context,                        /* IN */
                           ...
                           int cms_msg_type, unsigned char* data,
                           unsigned int data_len, unsigned char** out_data,
...
krb5_error_code cms_signeddata_create(
    krb5_context context, pkinit_plg_crypto_context plg_cryptoctx,
    pkinit_req_crypto_context req_cryptoctx,
    pkinit_identity_crypto_context id_cryptoctx, int cms_msg_type,
    int include_certchain, unsigned char* data, unsigned int data_len,
    unsigned char** signed_data, unsigned int* signed_data_len)
{
    krb5_error_code retval = ENOMEM;
    ...
    if (id_cryptoctx->my_certs != NULL)
    {
        ...
        else
        {
            ...
            /* create the signature over signed attributes. get DER encoded
             * value */
            /* This is the place where smartcard signature needs to be
             * calculated */
            sk = p7si->auth_attr;
         ...
         * DigestInfo DigestInfo::=SEQUENCE { digestAlgorithm
            ...
            id_cryptoctx->mech == CKM_RSA_PKCS)
        {
            ...
            print_buffer_bin(digestInfo_buf, digestInfo_len,
                             "/tmp/pkcs7_tosign");
#endif
...
#endif
        {
            pkiDebug("mech = %s\n", id_cryptoctx->pkcs11_method == 1
                                        ? "CKM_SHA1_RSA_PKCS"
                                        : "FS");
            retval = pkinit_sign_data(context, id_cryptoctx, abuf, alen, &sig,
                                      &sig_len);
        }
#ifdef DEBUG_SIG
...
krb5_error_code cms_signeddata_verify(
    krb5_context context, pkinit_plg_crypto_context plgctx,
    pkinit_req_crypto_context reqctx, pkinit_identity_crypto_context idctx,
    int cms_msg_type, int require_crl_checking, unsigned char* signed_data,
    unsigned int signed_data_len, unsigned char** data, unsigned int* data_len,
    unsigned char** authz_data, unsigned int* authz_data_len, int* is_signed)
{
    /*
    ...
    else
    {
        ...
        /* create available intermediate CAs chains (get local intermediateCAs
         * and include the CA chain received in the CMS message
         */
        ...
        if (intermediateCAs != NULL)
        {
            ...
            {
                X509_NAME_oneline(
                    X509_get_subject_name(sk_X509_value(intermediateCAs, i)),
                    buf, sizeof(buf));
                pkiDebug("cert #%d: %s\n", i, buf);
            }
        }
        if (idctx->trustedCAs != NULL)
        {
            ...
            {
                X509_NAME_oneline(
                    X509_get_subject_name(sk_X509_value(idctx->trustedCAs, i)),
                    buf, sizeof(buf));
                pkiDebug("cert #%d: %s\n", i, buf);
            }
        }
        ...
        if (i <= 0)
        {
            ...
            for (j = 0; j < size; j++)
            {
                ...
                X509_NAME_oneline(X509_get_subject_name(tmp_cert), buf,
                                  sizeof(buf));
                pkiDebug("cert #%d: %s\n", j, buf);
            }
#endif
    ...
    else if (cms_msg_type == CMS_SIGN_DRAFT9)
    {
        ...
        if (!OBJ_cmp(etype, client_oid) || !OBJ_cmp(etype, server_oid) ||
    ...
    if (x)
    {
        ...
        if (cms_msg_type == CMS_SIGN_CLIENT || cms_msg_type == CMS_SIGN_DRAFT9)
        {
            ...
            *authz_data = NULL;
            ...
            retval = k5int_encode_krb5_td_trusted_certifiers(
                (krb5_external_principal_identifier* const*)krb5_verified_chain,
                &authz);
            if (retval)
            {
...
krb5_error_code cms_envelopeddata_create(
    krb5_context context, pkinit_plg_crypto_context plgctx,
    pkinit_req_crypto_context reqctx, pkinit_identity_crypto_context idctx,
    krb5_preauthtype pa_type, int include_certchain, unsigned char* key_pack,
    unsigned int key_pack_len, unsigned char** out, unsigned int* out_len)
{
    ...
    retval = cms_signeddata_create(
        context, plgctx, reqctx, idctx, cms_msg_type, include_certchain,
        key_pack, key_pack_len, &signed_data, (unsigned int*)&signed_data_len);
    if (retval)
    {
...
krb5_error_code cms_envelopeddata_verify(
    krb5_context context, pkinit_plg_crypto_context plg_cryptoctx,
    pkinit_req_crypto_context req_cryptoctx,
    pkinit_identity_crypto_context id_cryptoctx, krb5_preauthtype pa_type,
    int require_crl_checking, unsigned char* enveloped_data,
    unsigned int enveloped_data_len, unsigned char** data,
    unsigned int* data_len)
{
    ...
    {
        retval =
            wrap_signeddata(tmp_buf, tmp_buf_len, &tmp_buf2, &tmp_buf2_len);
        if (retval)
        {
    ...
    retval = cms_signeddata_verify(context, plg_cryptoctx, req_cryptoctx,
                                   id_cryptoctx, msg_type, require_crl_checking,
                                   vfy_buf, vfy_buf_len, data, data_len, NULL,
                                   NULL, NULL);
...
static krb5_error_code crypto_retrieve_X509_sans(
    krb5_context context, pkinit_plg_crypto_context plgctx,
    pkinit_req_crypto_context reqctx, X509* cert, krb5_principal** princs_ret,
    char*** upn_ret, unsigned char*** dns_ret)
{
    ...
    for (i = 0; i < num_sans; i++)
    {
        ...
        switch (gen->type)
        {
            case GEN_OTHERNAME:
                ...
                name.data =
                    (char*)gen->d.otherName->value->value.sequence->data;
                if (princs != NULL && OBJ_cmp(plgctx->id_pkinit_san,
            ...
            case GEN_DNS:
                if (dnss != NULL)
                {
                    ...
                    pkiDebug("%s: found dns name = %s\n", __FUNCTION__,
                             gen->d.dNSName->data);
                    dnss[d] =
                        (unsigned char*)strdup((char*)gen->d.dNSName->data);
                    if (dnss[d] == NULL)
                    {
...
krb5_error_code crypto_retrieve_cert_sans(
    krb5_context context, pkinit_plg_crypto_context plgctx,
    pkinit_req_crypto_context reqctx, pkinit_identity_crypto_context idctx,
    krb5_principal** princs_ret, char*** upn_ret, unsigned char*** dns_ret)
{
    krb5_error_code retval = EINVAL;
    ...
    return crypto_retrieve_X509_sans(context, plgctx, reqctx,
                                     reqctx->received_cert, princs_ret, upn_ret,
                                     dns_ret);
}

krb5_error_code crypto_check_cert_eku(krb5_context context,
                                      pkinit_plg_crypto_context plgctx,
                                      pkinit_req_crypto_context reqctx,
                                      pkinit_identity_crypto_context idctx,
                                      int checking_kdc_cert,
                                      int allow_secondary_usage, int* valid_eku)
{
    char buf[DN_BUF_LEN];
    int found_eku = 0;
    krb5_error_code retval = EINVAL;
    ...
    *valid_eku = 0;
    ...
    X509_NAME_oneline(X509_get_subject_name(reqctx->received_cert), buf,
                      sizeof(buf));

    if ((i = X509_get_ext_by_NID(reqctx->received_cert, NID_ext_key_usage,
                                 -1)) >= 0)
    {
        ...
        if (extusage)
        {
            ...
            for (i = 0; found_eku == 0 && i < sk_ASN1_OBJECT_num(extusage); i++)
            {
                ...
                {
                    if ((OBJ_cmp(tmp_oid, plgctx->id_pkinit_KPClientAuth) ==
                         0) ||
                        (allow_secondary_usage &&
        ...
        if (found_eku)
        {
            ...
            if ((usage = X509_get_ext_d2i(reqctx->received_cert, NID_key_usage,
                                          NULL, NULL)))
            {
...
krb5_error_code pkinit_octetstring2key(krb5_context context, krb5_enctype etype,
                                       unsigned char* key,
 ...
 */
static krb5_error_code pkinit_alg_values(krb5_context context,
                                         ...
                                         const EVP_MD* (**func)(void))
{
    ...
    *func = NULL;
    if ((alg_id->length == krb5_pkinit_sha1_oid_len) &&
        (0 ==
         memcmp(alg_id->data, &krb5_pkinit_sha1_oid, krb5_pkinit_sha1_oid_len)))
    {
        *hash_bytes = 20;
        *func = &EVP_sha1;
        return 0;
 ...
 */
krb5_error_code pkinit_alg_agility_kdf(krb5_context context, krb5_data* secret,
                                       krb5_data* alg_oid,
                                       ...
                                       krb5_keyblock* key_block)
{
     ...
     */
    if (party_u_info && krb5_principal_compare_any_realm(
                            context, party_u_info, krb5_anonymous_principal()))
    ...
    if (0 != (retval = encode_krb5_sp80056a_other_info(&other_info_fields,
                                                       &other_info)))
        goto cleanup;
...
static void compute_dh(unsigned char* buf, int size, BIGNUM* server_pub_key,
                       DH* dh)
{
    int len, pad;
...
krb5_error_code client_create_dh(krb5_context context,
                                 pkinit_plg_crypto_context plg_cryptoctx,
                                 pkinit_req_crypto_context cryptoctx,
                                 pkinit_identity_crypto_context id_cryptoctx,
                                 int dh_size, unsigned char** dh_params_out,
...
krb5_error_code client_process_dh(krb5_context context,
                                  pkinit_plg_crypto_context plg_cryptoctx,
                                  pkinit_req_crypto_context cryptoctx,
                                  pkinit_identity_crypto_context id_cryptoctx,
...
static int check_dh_wellknown(pkinit_plg_crypto_context cryptoctx, DH* dh,
                              int nbits)
{
...
krb5_error_code server_check_dh(krb5_context context,
...
static DH* dup_dh_params(const DH* src)
{
...
/* kdc's dh function */
krb5_error_code server_process_dh(krb5_context context,
                                  pkinit_plg_crypto_context plg_cryptoctx,
                                  pkinit_req_crypto_context cryptoctx,
                                  pkinit_identity_crypto_context id_cryptoctx,
                                  unsigned char* data, unsigned int data_len,
...
static krb5_error_code pkinit_encode_dh_params(const BIGNUM* p, const BIGNUM* g,
                                               const BIGNUM* q, uint8_t** buf,
...
ASN1_SEQUENCE(DHvparams) =
    {ASN1_SIMPLE(int_dhvparams, seed, ASN1_BIT_STRING),
     ASN1_SIMPLE(int_dhvparams, counter,
                 BIGNUM)} static_ASN1_SEQUENCE_END_name(int_dhvparams,
                                                        DHvparams)
            ...
            * decode_dh_params(const uint8_t* p, unsigned int len)
{
    ...
    params =
        (int_dhx942_dh*)ASN1_item_d2i(NULL, &p, len, ASN1_ITEM_rptr(DHxparams));
    if (params == NULL)
    {
        DH_free(dh);
        return NULL;
...
static DH* decode_dh_params_int(DH** a, uint8_t** pp, unsigned int len)
{
...
static DH* decode_dh_params(const uint8_t* p, unsigned int len)
{
    ...
    krb5_pa_data*** e_data_out)
{
    ...
    switch (type)
    {
        ...
        case TD_INVALID_CERTIFICATES:
            retval = create_krb5_invalidCertificates(
                context, plg_cryptoctx, req_cryptoctx, id_cryptoctx,
                &krb5_trusted_certifiers);
            if (retval)
            {
    ...
    retval = k5int_encode_krb5_td_trusted_certifiers(
        (krb5_external_principal_identifier* const*)krb5_trusted_certifiers,
        &td_certifiers);
    if (retval)
    {
        pkiDebug("encode_krb5_td_trusted_certifiers failed\n");
...
#ifdef DEBUG_ASN1
    print_buffer_bin((unsigned char*)td_certifiers->data, td_certifiers->length,
...
krb5_error_code pkinit_create_td_trusted_certifiers(
    krb5_context context, pkinit_plg_crypto_context plg_cryptoctx,
    pkinit_req_crypto_context req_cryptoctx,
    pkinit_identity_crypto_context id_cryptoctx, krb5_pa_data*** e_data_out)
{
    krb5_error_code retval = KRB5KRB_ERR_GENERIC;

    retval = pkinit_create_sequence_of_principal_identifiers(
        context, plg_cryptoctx, req_cryptoctx, id_cryptoctx,
...
krb5_error_code pkinit_create_td_dh_parameters(
    krb5_context context, pkinit_plg_crypto_context plg_cryptoctx,
    pkinit_req_crypto_context req_cryptoctx,
    pkinit_identity_crypto_context id_cryptoctx, pkinit_plg_opts* opts,
    krb5_pa_data*** e_data_out)
{
    krb5_error_code retval = ENOMEM;
    ...
    retval = k5int_encode_krb5_td_dh_parameters(
        (krb5_algorithm_identifier* const*)algId, &encoded_algId);
    if (retval)
        goto cleanup;
#ifdef DEBUG_ASN1
    print_buffer_bin((unsigned char*)encoded_algId->data, encoded_algId->length,
...
krb5_error_code pkinit_check_kdc_pkid(
    krb5_context context, pkinit_plg_crypto_context plg_cryptoctx,
    pkinit_req_crypto_context req_cryptoctx,
    pkinit_identity_crypto_context id_cryptoctx, unsigned char* pdid_buf,
...
krb5_error_code pkinit_process_td_dh_params(
    krb5_context context, pkinit_plg_crypto_context cryptoctx,
    pkinit_req_crypto_context req_cryptoctx,
    pkinit_identity_crypto_context id_cryptoctx,
    krb5_algorithm_identifier** algId, int* new_dh_size)
{
    krb5_error_code retval = KRB5KDC_ERR_DH_KEY_PARAMETERS_NOT_ACCEPTED;
...
static int openssl_callback(int ok, X509_STORE_CTX* ctx)
{
#ifdef DEBUG
    if (!ok)
    {
...
static int openssl_callback_ignore_crls(int ok, X509_STORE_CTX* ctx)
{
    if (ok)
        return ok;
...
static ASN1_OBJECT* pkinit_pkcs7type2oid(pkinit_plg_crypto_context cryptoctx,
                                         int pkcs7_type)
{
    switch (pkcs7_type)
    {
                           ...
                           unsigned char** out, unsigned int* out_len)
{
    ...
    ASN1_put_object(&p, 1, (int)(orig_len + oid_len), V_ASN1_SEQUENCE,
                    V_ASN1_UNIVERSAL);
                                 ...
                                 CK_FUNCTION_LIST_PTR_PTR p11p)
{
    ...
    getflist =
        (CK_RV(*)(CK_FUNCTION_LIST_PTR_PTR))dlsym(handle, "C_GetFunctionList");
                                    ...
                                    CK_TOKEN_INFO* tip, const char* password)
{
    ...
    else
    {
        if (tip->flags & CKF_USER_PIN_LOCKED)
            ...
            warning = "";
        if (asprintf(&prompt, "%.*s PIN%s", (int)sizeof(tip->label), tip->label,
                     warning) < 0)
            return ENOMEM;
...
static krb5_error_code pkinit_open_session(krb5_context context,
                                           pkinit_identity_crypto_context cctx)
{
    ...
    /* Load module */
    cctx->p11_module = pkinit_C_LoadModule(cctx->p11_module_name, &cctx->p11);
    if (cctx->p11_module == NULL)
        return KRB5KDC_ERR_PREAUTH_FAILED;
    ...
    for (i = 0; i < count; i++)
    {
        ...
        /* Open session */
        if ((r = cctx->p11->C_OpenSession(slotlist[i], CKF_SERIAL_SESSION, NULL,
                                          NULL, &cctx->session)) != CKR_OK)
        {
    ...
    if (tinfo.flags & CKF_LOGIN_REQUIRED)
    {
        if (cctx->p11_module_name != NULL)
        {
            ...
            {
                if (asprintf(&p11name, "PKCS11:module_name=%s,token=%.*s",
                             cctx->p11_module_name, (int)label_len,
                             tinfo.label) < 0)
                    p11name = NULL;
            }
        }
        ...
        if (cctx->defer_id_prompt)
        {
            ...
            pkinit_set_deferred_id(&cctx->deferred_ids, p11name, tinfo.flags,
                                   NULL);
            free(p11name);
...
krb5_error_code
    pkinit_find_private_key(pkinit_identity_crypto_context id_cryptoctx,
                            CK_ATTRIBUTE_TYPE usage, CK_OBJECT_HANDLE* objp)
{
    CK_OBJECT_CLASS cls;
    ...
    r = id_cryptoctx->p11->C_FindObjectsInit(id_cryptoctx->session, attrs,
                                             nattrs);
    if (r != CKR_OK)
    {
    ...
    r = id_cryptoctx->p11->C_FindObjects(id_cryptoctx->session, objp, 1,
                                         &count);
 ...
 * caller.
 */
static CK_RV pkinit_C_Decrypt(pkinit_identity_crypto_context id_cryptoctx,
...
krb5_error_code pkinit_decode_data(krb5_context context,
                                   pkinit_identity_crypto_context id_cryptoctx,
                                   const uint8_t* data, unsigned int data_len,
...
krb5_error_code pkinit_sign_data(krb5_context context,
                                 pkinit_identity_crypto_context id_cryptoctx,
                                 unsigned char* data, unsigned int data_len,
                                 unsigned char** sig, unsigned int* sig_len)
{
    ...
    if (id_cryptoctx == NULL || id_cryptoctx->pkcs11_method != 1)
        retval = pkinit_sign_data_fs(context, id_cryptoctx, data, data_len, sig,
                                     sig_len);
#ifndef WITHOUT_PKCS11
    else
...
static krb5_error_code create_signature(unsigned char** sig,
                                        unsigned int* sig_len,
                                        unsigned char* data,
                                        unsigned int data_len, EVP_PKEY* pkey)
{
    krb5_error_code retval = ENOMEM;
    EVP_MD_CTX* ctx;
 ...
 */
krb5_error_code pkinit_get_kdc_cert(krb5_context context,
                                    pkinit_plg_crypto_context plg_cryptoctx,
                                    pkinit_req_crypto_context req_cryptoctx,
                                    pkinit_identity_crypto_context id_cryptoctx,
...
static char* reassemble_pkcs12_name(const char* filename)
{
    char* ret;
...
static krb5_error_code pkinit_get_certs_pkcs12(
    krb5_context context, pkinit_plg_crypto_context plg_cryptoctx,
    pkinit_req_crypto_context req_cryptoctx, pkinit_identity_opts* idopts,
    pkinit_identity_crypto_context id_cryptoctx, krb5_principal princ)
{
    krb5_error_code retval = KRB5KDC_ERR_PREAUTH_FAILED;
...
static char* reassemble_files_name(const char* certfile, const char* keyfile)
{
    char* ret;
...
static krb5_error_code
    pkinit_load_fs_cert_and_key(krb5_context context,
                                pkinit_identity_crypto_context id_cryptoctx,
                                char* certname, char* keyname, int cindex)
{
    krb5_error_code retval;
    X509* x = NULL;
    EVP_PKEY* y = NULL;
    ...
    {
        retval =
            oerr(context, 0, _("Cannot read certificate file '%s'"), certname);
        goto cleanup;
    }
    /* Load the key. */
    ...
    id_cryptoctx->creds[cindex]->name =
...
static krb5_error_code pkinit_get_certs_fs(
    krb5_context context, pkinit_plg_crypto_context plg_cryptoctx,
    pkinit_req_crypto_context req_cryptoctx, pkinit_identity_opts* idopts,
    pkinit_identity_crypto_context id_cryptoctx, krb5_principal princ)
{
    ...
    retval = pkinit_load_fs_cert_and_key(
        context, id_cryptoctx, idopts->cert_filename, idopts->key_filename, 0);
cleanup:
    return retval;
}

static krb5_error_code pkinit_get_certs_dir(
    krb5_context context, pkinit_plg_crypto_context plg_cryptoctx,
    pkinit_req_crypto_context req_cryptoctx, pkinit_identity_opts* idopts,
    pkinit_identity_crypto_context id_cryptoctx, krb5_principal princ)
{
    ...
    while ((i < MAX_CREDS_ALLOWED) && (dentry = readdir(d)) != NULL)
    {
        ...
        retval = pkinit_load_fs_cert_and_key(context, id_cryptoctx, certname,
                                             keyname, i);
        if (retval == 0)
        {
...
#ifndef WITHOUT_PKCS11
static char* reassemble_pkcs11_name(pkinit_identity_opts* idopts)
{
    struct k5buf buf;
    int n = 0;
    char* ret;
    ...
    {
        k5_buf_add_fmt(&buf, "%stoken=%s", n++ ? ":" : "", idopts->token_label);
    }
...
static krb5_error_code pkinit_get_certs_pkcs11(
    krb5_context context, pkinit_plg_crypto_context plg_cryptoctx,
    pkinit_req_crypto_context req_cryptoctx, pkinit_identity_opts* idopts,
    pkinit_identity_crypto_context id_cryptoctx, krb5_principal princ)
{
    ...
    if (idopts->cert_id_string != NULL)
    {
        r = k5_hex_decode(idopts->cert_id_string, &id_cryptoctx->cert_id,
    ...
    if (mechp == NULL)
        return ENOMEM;
    if ((r = id_cryptoctx->p11->C_GetMechanismList(id_cryptoctx->slotid, mechp,
                                                   &count)) != CKR_OK)
        return KRB5KDC_ERR_PREAUTH_FAILED;
    for (i = 0; i < count; i++)
    {
        if ((r = id_cryptoctx->p11->C_GetMechanismInfo(
                 id_cryptoctx->slotid, mechp[i], &info)) != CKR_OK)
            return KRB5KDC_ERR_PREAUTH_FAILED;
    ...
    for (i = 0;; i++)
    {
        ...
        /* Look for x.509 cert */
        if ((r = id_cryptoctx->p11->C_FindObjects(id_cryptoctx->session, &obj,
                                                  1, &count)) != CKR_OK ||
            count <= 0)
        {
        ...
        if ((r = id_cryptoctx->p11->C_GetAttributeValue(
                 id_cryptoctx->session, obj, attrs, 2)) != CKR_OK &&
...
static void free_cred_info(krb5_context context,
                           pkinit_identity_crypto_context id_cryptoctx,
...
krb5_error_code crypto_load_certs(krb5_context context,
                                  pkinit_plg_crypto_context plg_cryptoctx,
                                  pkinit_req_crypto_context req_cryptoctx,
                                  ...
                                  krb5_boolean defer_id_prompts)
{
    ...
    {
        case IDTYPE_FILE:
            retval = pkinit_get_certs_fs(context, plg_cryptoctx, req_cryptoctx,
                                         idopts, id_cryptoctx, princ);
            break;
        case IDTYPE_DIR:
            retval = pkinit_get_certs_dir(context, plg_cryptoctx, req_cryptoctx,
                                          idopts, id_cryptoctx, princ);
            break;
#ifndef WITHOUT_PKCS11
        case IDTYPE_PKCS11:
 ...
 */
static krb5_error_code crypto_retrieve_X509_key_usage(
    krb5_context context, pkinit_plg_crypto_context plgcctx,
    pkinit_req_crypto_context reqcctx, X509* x, unsigned int* ret_ku_bits,
...
static krb5_error_code rfc2253_name(X509_NAME* name, char** str_out)
{
    BIO* b = NULL;
    char* str;
...
void crypto_cert_free_matching_data(krb5_context context,
 ...
 */
void crypto_cert_free_matching_data_list(krb5_context context,
                      ...
                      pkinit_cert_matching_data** md_out)
{
    ...
    *md_out = NULL;
    ...
    /* Get the SAN data. */
    ret = crypto_retrieve_X509_sans(context, plg_cryptoctx, req_cryptoctx, cert,
                                    &md->sans, &md->upns, NULL);
    if (ret)
        goto cleanup;

    /* Get the KU and EKU data. */
    ret = crypto_retrieve_X509_key_usage(context, plg_cryptoctx, req_cryptoctx,
 ...
 */
krb5_error_code crypto_cert_select(krb5_context context,
                                   pkinit_identity_crypto_context idctx,
                                   size_t cred_index)
{
...
static krb5_error_code load_cas_and_crls(
    krb5_context context, pkinit_plg_crypto_context plg_cryptoctx,
    pkinit_req_crypto_context req_cryptoctx,
    pkinit_identity_crypto_context id_cryptoctx, int catype, char* filename)
{
...
static krb5_error_code load_cas_and_crls_dir(
    krb5_context context, pkinit_plg_crypto_context plg_cryptoctx,
    pkinit_req_crypto_context req_cryptoctx,
    pkinit_identity_crypto_context id_cryptoctx, int catype, char* dirname)
{
    krb5_error_code retval = EINVAL;
    DIR* d = NULL;
    struct dirent* dentry = NULL;
...
krb5_error_code crypto_load_cas_and_crls(
    krb5_context context, pkinit_plg_crypto_context plg_cryptoctx,
    pkinit_req_crypto_context req_cryptoctx, pkinit_identity_opts* idopts,
    pkinit_identity_crypto_context id_cryptoctx, int idtype, int catype,
    char* id)
{
    switch (idtype)
    {
...
krb5_error_code pkinit_process_td_trusted_certifiers(
    ...
    pkinit_req_crypto_context req_cryptoctx,
    pkinit_identity_crypto_context id_cryptoctx,
    krb5_external_principal_identifier** krb5_trusted_certifiers, int td_type)
{
    krb5_error_code retval = ENOMEM;
    ...
    while (krb5_trusted_certifiers[i] != NULL)
    {
        if (krb5_trusted_certifiers[i]->subjectName.data != NULL)
        {
            ...
            xn = d2i_X509_NAME(
                NULL, &p, (int)krb5_trusted_certifiers[i]->subjectName.length);
            if (xn == NULL)
                goto cleanup;
        ...
        {
            p = (unsigned char*)krb5_trusted_certifiers[i]
                    ->issuerAndSerialNumber.data;
            is = d2i_PKCS7_ISSUER_AND_SERIAL(
                NULL, &p,
...
#ifdef DEBUG_DH
static void print_dh(DH* dh, char* msg)
{
    BIO* bio_err = NULL;
...
static void print_pubkey(BIGNUM* key, char* msg)
{
    BIO* bio_err = NULL;
                           ...
                           const char* identity, const char* password)
{
    ...
    return pkinit_set_deferred_id(&id_cryptoctx->deferred_ids, identity,
                                  ck_flags, password);
}
...
krb5_error_code crypto_encode_der_cert(krb5_context context,
                                       pkinit_req_crypto_context reqctx,
 ...
 */
krb5_error_code crypto_req_cert_matching_data(
    krb5_context context, pkinit_plg_crypto_context plgctx,
    pkinit_req_crypto_context reqctx, pkinit_cert_matching_data** md_out)
{
    *md_out = NULL;
