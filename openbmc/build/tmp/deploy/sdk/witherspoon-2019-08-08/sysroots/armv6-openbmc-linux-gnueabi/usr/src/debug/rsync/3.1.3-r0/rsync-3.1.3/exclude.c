...
static void teardown_mergelist(filter_rule* ex)
{
    ...
    if (DEBUG_GTE(FILTER, 2))
    {
        rprintf(FINFO, "[%s] deactivating mergelist #%d%s\n", who_am_i(),
                mergelist_cnt - 1, ex->u.mergelist->debug_type);
    }
...
static void add_rule(filter_rule_list* listp, const char* pat,
                     unsigned int pat_len, filter_rule* rule, int xflags)
{
    const char* cp;
    ...
    if (DEBUG_GTE(FILTER, 2))
    {
        rprintf(FINFO, "[%s] add_rule(%s%.*s%s)%s\n", who_am_i(),
                get_rule_prefix(rule, pat, 0, NULL), (int)pat_len, pat,
    ...
    if (strpbrk(rule->pattern, "*[?"))
    {
        ...
        if ((cp = strstr(rule->pattern, "**")) != NULL)
        {
            ...
            if (pat_len >= 3 && rule->pattern[pat_len - 3] == '*' &&
    ...
    if (rule->rflags & FILTRULE_PERDIR_MERGE)
    {
        ...
        for (i = 0; i < mergelist_cnt; i++)
        {
            ...
            if (s)
                ...
                s = ex->pattern;
            len = strlen(s);
            if (len == pat_len - (cp - rule->pattern) &&
                memcmp(s, cp, len) == 0)
            {
                free_filter(rule);
                return;
        ...
        if (mergelist_cnt == mergelist_size)
        {
            mergelist_size += 5;
            mergelist_parents =
                realloc_array(mergelist_parents, filter_rule*, mergelist_size);
...
void* push_local_filters(const char* dir, unsigned int dirlen)
{
    ...
    push = (struct local_filter_state*)new_array(
        char, sizeof(struct local_filter_state) +
    ...
    for (i = 0; i < mergelist_cnt; i++)
    {
        ...
        if (strlcpy(dirbuf + dirbuf_len, ex->pattern, MAXPATHLEN - dirbuf_len) <
            MAXPATHLEN - dirbuf_len)
        {
            parse_filter_file(lp, dirbuf, ex, XFLG_ANCHORED2ABS);
        }
        else
        {
...
void pop_local_filters(void* mem)
{
    ...
    for (i = mergelist_cnt; i-- > 0;)
    {
        ...
        if (i >= old_mergelist_cnt && lp->head)
        {
             ...
             * still has inherited rules.  This can sometimes happen if a
             * per-dir merge file calls setup_merge_file() in
             * push_local_filters() and that leaves some inherited rules that
             * aren't in the pushed list state. */
            if (DEBUG_GTE(FILTER, 2))
            {
                rprintf(
                    FINFO,
...
static int rule_matches(const char* fname, filter_rule* ex, int name_flags)
{
    ...
    if (!anchored_match && ex->u.slash_cnt && !(ex->rflags & FILTRULE_WILD2))
    {
    ...
    else
    {
        ...
        if (l2 <= l1 && strcmp(name + (l1 - l2), pattern) == 0 &&
            (l1 == l2 || name[l1 - (l2 + 1)] == '/'))
        {
            return ret_match;
        }
...
static void report_filter_result(enum logcode code, char const* name,
                                 filter_rule const* ent, int name_flags,
                                 const char* type)
{
    ...
    {
        static char* actions[2][2] = {{"show", "hid"}, {"risk", "protect"}};
        const char* w = who_am_i();
        const char* t = name_flags & NAME_IS_XATTR
                            ? "xattr"
                            : name_flags & NAME_IS_DIR ? "directory" : "file";
        rprintf(code, "[%s] %sing %s %s because of pattern %s%s%s\n", w,
                actions[*w != 's'][!(ent->rflags & FILTRULE_INCLUDE)], t, name,
                ent->pattern, ent->rflags & FILTRULE_DIRECTORY ? "/" : "",
                type);
    }
}
...
int name_is_excluded(const char* fname, int name_flags, int filter_level)
{
    if (daemon_filter_list.head &&
        check_filter(&daemon_filter_list, FLOG, fname, name_flags) < 0)
    {
    ...
    if (filter_list.head &&
        check_filter(&filter_list, FINFO, fname, name_flags) < 0)
        return 1;
...
int check_filter(filter_rule_list* listp, enum logcode code, const char* name,
                 int name_flags)
{
    filter_rule* ent;
    ...
    for (ent = listp->head; ent; ent = ent->next)
    {
        ...
        {
            report_filter_result(code, name, ent, name_flags,
                                 listp->debug_type);
...
static const uchar* rule_strcmp(const uchar* str, const char* rule,
                                int rule_len)
{
...
#define FILTRULES_FROM_CONTAINER                                               \
    (FILTRULE_ABS_PATH | FILTRULE_INCLUDE | FILTRULE_DIRECTORY |               \
                                   ...
                                   unsigned int* pat_len_ptr)
{
    ...
    else
    {
        ...
        {
            case ':':
                rule->rflags |= FILTRULE_PERDIR_MERGE | FILTRULE_FINISH_SETUP;
                /* FALL THROUGH */
            case '.':
        ...
        while (ch != '!' && *++s && *s != ' ' && *s != '_')
        {
            ...
            switch (*s)
            {
                ...
                invalid:
                    rprintf(FERROR,
                            "invalid modifier '%c' at position %d in filter "
                            "rule: %s\n",
                            *s, (int)(s - (const uchar*)*rulestr_ptr),
                            *rulestr_ptr);
                    exit_cleanup(RERR_SYNTAX);
                case '-':
                    if (!BITS_SETnUNSET(rule->rflags, FILTRULE_MERGE_FILE,
                ...
                case '+':
                    if (!BITS_SETnUNSET(rule->rflags, FILTRULE_MERGE_FILE,
                                        FILTRULE_NO_PREFIXES))
                        goto invalid;
                    rule->rflags |= FILTRULE_NO_PREFIXES | FILTRULE_INCLUDE;
                    break;
                case '/':
                    rule->rflags |= FILTRULE_ABS_PATH;
                    break;
                ...
                case 'C':
                    if (rule->rflags & FILTRULE_NO_PREFIXES ||
                        ...
                        goto invalid;
                    rule->rflags |= FILTRULE_NO_PREFIXES | FILTRULE_WORD_SPLIT |
                                    FILTRULE_NO_INHERIT | FILTRULE_CVS_IGNORE;
                    break;
                case 'e':
                    if (!(rule->rflags & FILTRULE_MERGE_FILE))
                        goto invalid;
    ...
    if (rule->rflags & FILTRULE_CLEAR_LIST)
    {
        ...
        {
            rprintf(FERROR, "'!' rule has trailing characters: %s\n",
                    *rulestr_ptr);
            exit_cleanup(RERR_SYNTAX);
        }
        if (len > 1)
    ...
    /* --delete-excluded turns an un-modified include/exclude into a sender-side
     * rule.  */
    if (delete_excluded &&
        !(rule->rflags &
...
static void get_cvs_excludes(uint32 rflags)
{
    ...
    parse_filter_str(
        &cvs_filter_list, default_cvsignore,
        rule_template(rflags |
                      (protocol_version >= 30 ? FILTRULE_PERISHABLE : 0)),
        0);
    ...
    parse_filter_str(&cvs_filter_list, getenv("CVSIGNORE"),
                     rule_template(rflags), 0);
}
                      ...
                      const filter_rule* template, int xflags)
{
    ...
    while (1)
    {
        ...
        if (!(rule =
                  parse_rule_tok(&rulestr, template, xflags, &pat, &pat_len)))
            break;
        ...
        if (new_rflags & FILTRULE_CLEAR_LIST)
        {
            if (DEBUG_GTE(FILTER, 2))
            {
                rprintf(FINFO, "[%s] clearing filter list%s\n", who_am_i(),
                        listp->debug_type);
            }
        ...
        if (new_rflags & FILTRULE_MERGE_FILE)
        {
            ...
            if (new_rflags & FILTRULE_EXCLUDE_SELF)
            {
                 ...
                 */
                for (name = pat + pat_len; name > pat && name[-1] != '/';
                     name--)
                {
                }
                       ...
                       const filter_rule* template, int xflags)
{
    ...
    if (DEBUG_GTE(FILTER, 2))
    {
        rprintf(FINFO, "[%s] parse_filter_file(%s,%x,%x)%s\n", who_am_i(),
                fname, template->rflags, xflags, fp ? "" : " [not found]");
    }
                      ...
                      unsigned int* plen_ptr)
{
    ...
    if (rule->rflags & FILTRULE_PERDIR_MERGE)
    {
        ...
        *op++ = '+';
    else if (legal_len != 1 || ((*pat == '-' || *pat == '+') && pat[1] == ' '))
        *op++ = '-';
    else
        legal_len = 0;
    ...
    if (rule->rflags & FILTRULE_RECEIVER_SIDE &&
        (!for_xfer || protocol_version >= 29 || (delete_excluded && am_sender)))
        *op++ = 'r';
