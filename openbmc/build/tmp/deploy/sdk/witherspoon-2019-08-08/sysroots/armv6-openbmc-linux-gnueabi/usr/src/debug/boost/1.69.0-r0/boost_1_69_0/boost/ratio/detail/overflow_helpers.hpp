...
// The way overflow is managed for ratio_less is taken from
...
//
// We simply cannot include this header on gcc without getting copious warnings
// of the kind:
//
// boost/integer.hpp:77:30: warning: use of C99 long long integer constant
//
// And yet there is no other reasonable implementation, so we declare this a
// system header to suppress these warnings.
//
...
template <boost::intmax_t X, boost::intmax_t Y,
          boost::intmax_t = mpl::sign_c<boost::intmax_t, Y>::value>
class br_add;
...
class br_add<X, Y, 1>
{
    ...
    static const boost::intmax_t max =
...
class br_mul
{
    static const boost::intmax_t nan = boost::intmax_t(
        BOOST_RATIO_UINTMAX_C(1) << (sizeof(boost::intmax_t) * CHAR_BIT - 1));
    static const boost::intmax_t min =
        boost::integer_traits<boost::intmax_t>::const_min;
    static const boost::intmax_t max =
        boost::integer_traits<boost::intmax_t>::const_max;
...
struct ratio_add
{
    // The nested typedef type shall be a synonym for ratio<T1, T2>::type where
    // T1 has the value R1::num * R2::den + R2::num * R1::den and T2 has the
    // value R1::den * R2::den.
    // As the preceding doesn't works because of overflow on boost::intmax_t we
    // need something more elaborated.
  private:
    static const boost::intmax_t gcd_n1_n2 =
        mpl::gcd_c<boost::intmax_t, R1::num, R2::num>::value;
    static const boost::intmax_t gcd_d1_d2 =
        mpl::gcd_c<boost::intmax_t, R1::den, R2::den>::value;

  public:
    // No need to normalize as ratio_multiply is already normalized
    typedef typename ratio_multiply<
        ratio<gcd_n1_n2, R1::den / gcd_d1_d2>,
        ratio<boost::ratio_detail::br_add<
                  boost::ratio_detail::br_mul<R1::num / gcd_n1_n2,
                                              R2::den / gcd_d1_d2>::value,
                  boost::ratio_detail::br_mul<
                      R2::num / gcd_n1_n2, R1::den / gcd_d1_d2>::value>::value,
              R2::den>>::type type;
};
...
struct ratio_subtract
{
    // The nested typedef type shall be a synonym for ratio<T1, T2>::type where
    // T1 has the value
    // R1::num *R2::den - R2::num * R1::den and T2 has the value R1::den *
    // R2::den. As the preceding doesn't works because of overflow on
    // boost::intmax_t we need something more elaborated.
  private:
    static const boost::intmax_t gcd_n1_n2 =
        mpl::gcd_c<boost::intmax_t, R1::num, R2::num>::value;
    static const boost::intmax_t gcd_d1_d2 =
  ...
  public:
    ...
    typedef typename ratio_multiply<
        ratio<gcd_n1_n2, R1::den / gcd_d1_d2>,
        ratio<boost::ratio_detail::br_sub<
                  boost::ratio_detail::br_mul<R1::num / gcd_n1_n2,
                                              R2::den / gcd_d1_d2>::value,
                  boost::ratio_detail::br_mul<
                      R2::num / gcd_n1_n2, R1::den / gcd_d1_d2>::value>::value,
...
struct ratio_multiply
{
    // The nested typedef type  shall be a synonym for ratio<R1::num * R2::den -
    // R2::num * R1::den, R1::den * R2::den>::type. As the preceding doesn't
    // works because of overflow on boost::intmax_t we need something more
    // elaborated.
  private:
    static const boost::intmax_t gcd_n1_d2 =
        mpl::gcd_c<boost::intmax_t, R1::num, R2::den>::value;
    static const boost::intmax_t gcd_d1_n2 =
        mpl::gcd_c<boost::intmax_t, R1::den, R2::num>::value;

  public:
    typedef typename ratio<
        boost::ratio_detail::br_mul<R1::num / gcd_n1_d2,
                                    R2::num / gcd_d1_n2>::value,
        boost::ratio_detail::br_mul<R2::den / gcd_n1_d2,
                                    R1::den / gcd_d1_n2>::value>::type type;
};
...
{
    // The nested typedef type  shall be a synonym for ratio<R1::num * R2::den,
    // R2::num * R1::den>::type. As the preceding doesn't works because of
    // overflow on boost::intmax_t we need something more elaborated.
  private:
    static const boost::intmax_t gcd_n1_n2 =
        mpl::gcd_c<boost::intmax_t, R1::num, R2::num>::value;
    static const boost::intmax_t gcd_d1_d2 =
  ...
  public:
    typedef typename ratio<
        boost::ratio_detail::br_mul<R1::num / gcd_n1_n2,
                                    R2::den / gcd_d1_d2>::value,
        boost::ratio_detail::br_mul<R2::num / gcd_n1_n2,
                                    R1::den / gcd_d1_d2>::value>::type type;
};
template <class R1, class R2>
struct is_evenly_divisible_by
{
  ...
  public:
    typedef integral_constant<bool, ((R2::num / gcd_n1_n2 == 1) &&
                                     (R1::den / gcd_d1_d2) == 1)>
        type;
};
...
template <class R1, class R2, boost::intmax_t Q1 = R1::num / R1::den,
          boost::intmax_t M1 = R1::num % R1::den,
          boost::intmax_t Q2 = R2::num / R2::den,
          boost::intmax_t M2 = R2::num % R2::den>
struct ratio_less1
{
...
struct ratio_less1<R1, R2, Q, M1, Q, M2>
{
    static const bool value =
        ratio_less1<ratio<R2::den, M2>, ratio<R1::den, M1>>::value;
};
