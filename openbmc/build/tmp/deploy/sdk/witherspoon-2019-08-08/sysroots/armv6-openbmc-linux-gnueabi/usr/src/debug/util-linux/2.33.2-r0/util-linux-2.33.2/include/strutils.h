...
static inline char* mem2strcpy(char* dest, const void* src, size_t n,
                               size_t nmax)
{
    if (n + 1 > nmax)
        n = nmax - 1;
...
extern int string_to_bitmask(const char* list, unsigned long* mask,
...
extern const char* split(const char** state, size_t* l, const char* separator,
                         int quoted);
