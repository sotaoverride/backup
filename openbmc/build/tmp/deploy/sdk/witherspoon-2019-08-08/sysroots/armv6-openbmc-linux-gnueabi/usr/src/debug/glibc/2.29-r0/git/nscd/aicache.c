...
static const ai_response_header notfound = {.version = NSCD_VERSION,
                                            .found = 0,
                                            .naddrs = 0,
                                            .addrslen = 0,
                                            .canonlen = 0,
                        ...
                        struct datahead* dh)
{
    ...
    if (hosts_database == NULL)
        no_more = __nss_database_lookup(
            "hosts", NULL, "dns [!UNAVAIL=return] files", &hosts_database);
    else
        no_more = 0;
    nip = hosts_database;
    ...
    while (!no_more)
    {
        ...
        if (fct4 != NULL)
        {
            ...
            while (1)
            {
                ...
                herrno = 0;
                status[1] =
                    DL_CALL_FCT(fct4, (key, &at, tmpbuf6.data, tmpbuf6.length,
                                       &rc6, &herrno, &ttl));
                if (rc6 != ERANGE ||
                    (herrno != NETDB_INTERNAL && herrno != TRY_AGAIN))
                    break;
        ...
        else
        {
            ...
            {
                rc4 = 0;
                status[1] = DL_CALL_FCT(
                    fct, (key, AF_INET, &th[1], tmpbuf4.data, tmpbuf4.length,
                          &rc4, &herrno, ttl == INT32_MAX ? &ttl : NULL,
                ...
                else
                {
                    ...
                    while (1)
                    {
                        rc = __gethostbyaddr2_r(addr, addrlen, addrfamily,
                                                &hstent_mem, canonbuf.data,
                                                canonbuf.length, &hstent,
                                                &herrno, NULL);
       ...
       result, so be it.  */
        if (rc4 == EAGAIN || __builtin_expect(db->negtimeout == 0, 0))
        {
            ...
            dataset = NULL;
                      ...
                      db, (sizeof(struct dataset) + req->key_len), 1)) != NULL)
        {
...
time_t readdhstai(struct database_dyn* db, struct hashentry* he,
                  struct datahead* dh)
{
    request_header req = {.type = GETAI, .key_len = he->len};
