 ...
 */
static krb5_error_code krb5_context_size(krb5_context, krb5_pointer, size_t*);

static krb5_error_code krb5_context_externalize(krb5_context, krb5_pointer,
                                                krb5_octet**, size_t*);

static krb5_error_code krb5_context_internalize(krb5_context, krb5_pointer*,
                                                krb5_octet**, size_t*);

static krb5_error_code krb5_oscontext_size(krb5_context, krb5_pointer, size_t*);

static krb5_error_code krb5_oscontext_externalize(krb5_context, krb5_pointer,
                                                  krb5_octet**, size_t*);

static krb5_error_code krb5_oscontext_internalize(krb5_context, krb5_pointer*,
                                                  krb5_octet**, size_t*);
...
static inline unsigned int etypes_len(krb5_enctype* list)
{
 ...
 *                        krb5_context.
 */
static krb5_error_code krb5_context_size(krb5_context kcontext,
                                         krb5_pointer arg, size_t* sizep)
{
    ...
    if ((context = (krb5_context)arg))
    {
        ...
        kret = krb5_size_opaque(kcontext, KV5M_OS_CONTEXT,
                                (krb5_pointer)&context->os_context, &required);

        /* Calculate size required by db_context, if appropriate */
        if (!kret && context->dal_handle)
            kret =
                krb5_size_opaque(kcontext, KV5M_DB_CONTEXT,
                                 (krb5_pointer)context->dal_handle, &required);

        /* Finally, calculate size required by profile, if appropriate */
        if (!kret && context->profile)
            kret = krb5_size_opaque(kcontext, PROF_MAGIC_PROFILE,
                                    (krb5_pointer)context->profile, &required);
    }
    if (!kret)
        *sizep += required;
    return (kret);
 ...
 */
static krb5_error_code krb5_context_externalize(krb5_context kcontext,
                                                krb5_pointer arg,
                                                krb5_octet** buffer,
                                                size_t* lenremain)
{
    ...
    kret = krb5_ser_pack_int32((context->default_realm)
                                   ? (krb5_int32)strlen(context->default_realm)
                                   : 0,
                               &bp, &remain);
    if (kret)
        return (kret);
    ...
    {
        kret =
            krb5_ser_pack_bytes((krb5_octet*)context->default_realm,
                                strlen(context->default_realm), &bp, &remain);
        if (kret)
            return (kret);
    }
    ...
    kret =
        krb5_ser_pack_int32(etypes_len(context->in_tkt_etypes), &bp, &remain);
    if (kret)
        return (kret);
    ...
    if (context->in_tkt_etypes)
    {
        ...
        {
            kret = krb5_ser_pack_int32(context->in_tkt_etypes[i], &bp, &remain);
            if (kret)
                return (kret);
        }
    }
    ...
    kret = krb5_ser_pack_int32((krb5_int32)context->clockskew, &bp, &remain);
    if (kret)
        return (kret);

    /* Now kdc_req_sumtype */
    kret =
        krb5_ser_pack_int32((krb5_int32)context->kdc_req_sumtype, &bp, &remain);
    if (kret)
        return (kret);

    /* Now default ap_req_sumtype */
    kret = krb5_ser_pack_int32((krb5_int32)context->default_ap_req_sumtype, &bp,
                               &remain);
    if (kret)
        return (kret);

    /* Now default safe_sumtype */
    kret = krb5_ser_pack_int32((krb5_int32)context->default_safe_sumtype, &bp,
                               &remain);
    if (kret)
        return (kret);

    /* Now kdc_default_options */
    kret = krb5_ser_pack_int32((krb5_int32)context->kdc_default_options, &bp,
                               &remain);
    if (kret)
        return (kret);
    ...
    /* Now library_options */
    kret =
        krb5_ser_pack_int32((krb5_int32)context->library_options, &bp, &remain);
    if (kret)
        return (kret);

    /* Now profile_secure */
    kret =
        krb5_ser_pack_int32((krb5_int32)context->profile_secure, &bp, &remain);
    if (kret)
        return (kret);

    /* Now fcc_default_format */
    kret = krb5_ser_pack_int32((krb5_int32)context->fcc_default_format, &bp,
                               &remain);
    if (kret)
        return (kret);
    ...
    kret = krb5_externalize_opaque(kcontext, KV5M_OS_CONTEXT,
                                   (krb5_pointer)&context->os_context, &bp,
                                   &remain);
    if (kret)
        return (kret);
    ...
    if (context->dal_handle)
    {
        kret = krb5_externalize_opaque(kcontext, KV5M_DB_CONTEXT,
                                       (krb5_pointer)context->dal_handle, &bp,
                                       &remain);
        if (kret)
            return (kret);
    }
    ...
    if (context->profile)
    {
        kret = krb5_externalize_opaque(kcontext, PROF_MAGIC_PROFILE,
                                       (krb5_pointer)context->profile, &bp,
                                       &remain);
        if (kret)
            return (kret);
    }
 ...
 */
static krb5_error_code krb5_context_internalize(krb5_context kcontext,
                                                krb5_pointer* argp,
                                                krb5_octet** buffer,
                                                size_t* lenremain)
{
    ...
    kret =
        krb5_internalize_opaque(kcontext, PROF_MAGIC_PROFILE,
                                (krb5_pointer*)&context->profile, &bp, &remain);
    if (kret && (kret != EINVAL) && (kret != ENOENT))
        goto cleanup;
 ...
 *                                the krb5_os_context.
 */
static krb5_error_code krb5_oscontext_size(krb5_context kcontext,
                                           krb5_pointer arg, size_t* sizep)
{
    /*
 ...
 */
static krb5_error_code krb5_oscontext_externalize(krb5_context kcontext,
                                                  krb5_pointer arg,
                                                  krb5_octet** buffer,
                                                  size_t* lenremain)
{
 ...
 */
static krb5_error_code krb5_oscontext_internalize(krb5_context kcontext,
                                                  krb5_pointer* argp,
                                                  krb5_octet** buffer,
                                                  size_t* lenremain)
{
    ...
    if (ibuf == KV5M_OS_CONTEXT)
    {
        ...
        /* Get memory for the context */
        if ((os_ctx =
                 (krb5_os_context)calloc(1, sizeof(struct _krb5_os_context))) &&
 ...
 */
krb5_error_code KRB5_CALLCONV krb5_ser_context_init(krb5_context kcontext)
{
    krb5_error_code kret;
