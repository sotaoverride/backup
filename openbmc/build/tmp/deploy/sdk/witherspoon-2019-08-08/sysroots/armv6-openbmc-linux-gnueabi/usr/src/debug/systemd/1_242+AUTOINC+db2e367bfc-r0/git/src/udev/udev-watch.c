...
int udev_watch_restore(void)
{
    ...
    if (rename("/run/udev/watch", "/run/udev/watch.old") < 0)
    {
        if (errno != ENOENT)
            return log_warning_errno(
                errno, "Failed to move watches directory /run/udev/watch. Old "
                       "watches will not be restored: %m");
    ...
    if (!dir)
        return log_warning_errno(
            errno, "Failed to open old watches directory /run/udev/watch.old. "
    ...
    FOREACH_DIRENT_ALL(ent, dir, break)
    {
        ...
        {
            log_debug_errno(
                r, "Failed to read link '/run/udev/watch.old/%s', ignoring: %m",
                ent->d_name);
            goto unlink;
        }
        ...
        {
            log_debug_errno(
                r, "Failed to create sd_device object for '%s', ignoring: %m",
                device);
            goto unlink;
        }
...
int udev_watch_begin(sd_device* dev)
{
    ...
    if (wd < 0)
        return log_device_full(dev, errno == ENOENT ? LOG_DEBUG : LOG_ERR,
                               errno, "Failed to add device '%s' to watch: %m",
                               devnode);
    ...
    if (r < 0)
        return log_device_error_errno(dev, r,
                                      "Failed to get device id-filename: %m");

    if (symlink(id_filename, filename) < 0)
        return log_device_error_errno(
            dev, errno, "Failed to create symlink %s: %m", filename);
...
int udev_watch_end(sd_device* dev)
{
    ...
    if (r < 0)
        return log_device_debug_errno(
            dev, r, "Failed to get watch handle, ignoring: %m");
...
int udev_watch_lookup(int wd, sd_device** ret)
{
    ...
    if (r < 0)
        return log_debug_errno(
            r, "Failed to create sd_device object for '%s': %m", device);
