...
static void show_header(int mhoffs)
{
    ...
    printf("  bootRecordID          = %s\n"
           ...
           le32_to_cpu(mh->NoOfBDTLPartitions), bmbits,
    ...
    if (numunits > 32768)
    {
        printf("WARNING: More than 32768 units! Unexpectedly small "
               "BlockMultiplierBits.\n");
    }
    ...
    for (i = 0; i < 4; i++)
    {
        ...
        if (start > nextunit)
        {
            printf("  Unpartitioned space: %d bytes\n"
                   ...
                   "    firstUnit     = %d\n"
                   "    lastUnit      = %d\n",
                   (start - nextunit) * unitsize, start - nextunit, nextunit,
                   start - 1);
        }
        ...
        printf(" %d bytes\n"
               ...
               "    flags         = 0x%x\n"
               "    spareUnits    = %d\n",
               num * unitsize, num, start, end, le32_to_cpu(ip->flags),
               le32_to_cpu(ip->spareUnits));
        if (num > (1 + end - start))
        {
    ...
    if ((i + 1) != numpart)
    {
        printf("ERROR: Number of partitions != (NoOfBinaryPartitions + "
               "NoOfBDTLPartitions)\n");
    }
    ...
    if (nextunit < numunits)
    {
        printf("  Unpartitioned space: %d bytes\n"
               ...
               "    firstUnit     = %d\n"
               "    lastUnit      = %d\n",
               (numunits - nextunit) * unitsize, numunits - nextunit, nextunit,
               numunits - 1);
    }
}
...
int main(int argc, char** argv)
{
    ...
    if (argc < 2)
    {
        printf("Usage: %s <mtddevice> [<size1> [<size2> [<size3> [<size4]]]]\n"
               "  Sizes are in device units (run with no sizes to show "
               "unitsize and current\n"
    ...
    {
        if ((ret = pread(fd, buf, meminfo.erasesize,
                         mhblock * meminfo.erasesize)) < 0)
        {
            if (errno == EBADMSG)
            {
        ...
        if (!strcmp("BNAND", mh->bootRecordID))
            break;
    }
    if (mhblock >= MAXSCAN)
    {
    ...
    printf("\nReady to update device.  Type 'yes' to proceed, anything else to "
           "abort: ");
    ...
    printf("Success.  REBOOT or unload the diskonchip module to update "
           "partitions!\n");
    return 0;
}
