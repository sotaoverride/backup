        ...
        *exp2_adj = x_adj_log2 * (int)x_adj_int;
        float ret =
            (__ieee754_powf(x_adj_mant, x_adj) *
             __ieee754_exp2f(x_adj_log2 * x_adj_frac) * __ieee754_expf(-x_adj) *
             sqrtf(2 * (float)M_PI / x_adj) / prod);
            ...
            *signgamp = (tx == 2.0f * truncf(tx / 2.0f)) ? -1 : 1;
            ...
            else
            {
                ...
                if (frac > 0.5f)
                    frac = 1.0f - frac;
                float sinpix =
                    (frac <= 0.25f ? __sinf((float)M_PI * frac)
                                   : __cosf((float)M_PI * (0.5f - frac)));
                int exp2_adj;
                float tret = (float)M_PI /
                             (-x * sinpix * gammaf_positive(-x, &exp2_adj));
                ret = __scalbnf(tret, -exp2_adj);
