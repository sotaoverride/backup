...
int loop_device_make(int fd, int open_flags, LoopDevice** ret)
{
    ...
    if (S_ISBLK(st.st_mode))
    {
        ...
        *d = (LoopDevice){
            .fd = copy,
            .nr = -1,
            .relinquished = true, /* It's not allocated by us, don't destroy it
    ...
    control =
        open("/dev/loop-control", O_RDWR | O_CLOEXEC | O_NOCTTY | O_NONBLOCK);
    if (control < 0)
        return -errno;
...
void loop_device_relinquish(LoopDevice* d)
{
    ...
    /* Don't attempt to clean up the loop device anymore from this point on.
     * Leave the clean-ing up to the kernel itself, using the loop device
     * "auto-clear" logic we already turned on when creating the device. */
