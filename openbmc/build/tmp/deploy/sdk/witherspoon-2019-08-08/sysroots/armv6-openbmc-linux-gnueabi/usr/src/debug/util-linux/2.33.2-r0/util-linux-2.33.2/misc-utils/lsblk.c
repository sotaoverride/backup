...
static struct colinfo infos[] = {
    [COL_NAME] = {"NAME", 0.25, SCOLS_FL_TREE | SCOLS_FL_NOEXTREMES,
                  N_("device name")},
    ...
    [COL_FSAVAIL] = {"FSAVAIL", 5, SCOLS_FL_RIGHT,
    ...
    [COL_FSUSEPERC] = {"FSUSE%", 3, SCOLS_FL_RIGHT,
                       N_("filesystem use percentage")},

    [COL_TARGET] = {"MOUNTPOINT", 0.10, SCOLS_FL_TRUNC,
                    N_("where the device is mounted")},
    [COL_LABEL] = {"LABEL", 0.1, 0, N_("filesystem LABEL")},
    [COL_UUID] = {"UUID", 36, 0, N_("filesystem UUID")},

    [COL_PTUUID] = {"PTUUID", 36, 0,
                    N_("partition table identifier (usually UUID)")},
    ...
    [COL_HOTPLUG] = {"HOTPLUG", 1, SCOLS_FL_RIGHT,
    ...
    [COL_DALIGN] = {"DISC-ALN", 6, SCOLS_FL_RIGHT,
    ...
    [COL_SUBSYS] = {"SUBSYSTEMS", 0.1, SCOLS_FL_NOEXTREMES,
...
static char* get_type(struct blkdev_cxt* cxt)
{
    ...
    if (is_dm(cxt->name))
    {
        ...
        if (ul_path_read_string(cxt->sysfs, &dm_uuid, "dm/uuid") > 0 && dm_uuid)
        {
            char* tmp = dm_uuid;
...
static char* get_transport(struct blkdev_cxt* cxt)
{
    ...
    {
        attr = sysfs_blkdev_scsi_host_strdup_attribute(sysfs, "fc",
                                                       "symbolic_name");
        if (!attr)
            return NULL;
    ...
    {
        attr =
            sysfs_blkdev_scsi_host_strdup_attribute(sysfs, "scsi", "proc_name");
        if (!attr)
            return NULL;
...
#define is_parsable(_l)                                                        \
    (scols_table_is_raw((_l)->table) || scols_table_is_export((_l)->table) ||  \
...
static void set_sortdata_u64_from_string(struct libscols_line* ln, int col,
                                         const char* str)
{
    uint64_t x;
...
static void unref_sortdata(struct libscols_table* tb)
{
    ...
    while (scols_table_next_line(tb, itr, &ln) == 0)
    {
        struct libscols_cell* ce =
            scols_line_get_column_cell(ln, lsblk->sort_col);
...
static char* get_vfs_attribute(struct blkdev_cxt* cxt, int id)
{
    ...
    switch (id)
    {
        ...
        case COL_FSAVAIL:
            ...
            break;
        case COL_FSUSED:
            vfs_attr = cxt->fsstat.f_frsize *
                       (cxt->fsstat.f_blocks - cxt->fsstat.f_bfree);
            break;
        case COL_FSUSEPERC:
...
static void set_scols_data(struct blkdev_cxt* cxt, int col, int id,
                           struct libscols_line* ln)
{
    ...
    switch (id)
    {
        ...
        case COL_RM:
            ...
            if (!str && sysfs_blkdev_get_parent(cxt->sysfs))
                ul_path_read_string(sysfs_blkdev_get_parent(cxt->sysfs), &str,
                                    "removable");
            break;
        case COL_HOTPLUG:
            str = sysfs_blkdev_is_hotpluggable(cxt->sysfs) ? xstrdup("1")
                                                           : xstrdup("0");
            break;
        case COL_ROTA:
        ...
        case COL_DGRAN:
            if (lsblk->bytes)
            {
                ul_path_read_string(cxt->sysfs, &str,
            ...
            {
                uint64_t x;
                if (ul_path_read_u64(cxt->sysfs, &x,
...
static void fill_table_line(struct blkdev_cxt* cxt,
                            struct libscols_line* scols_parent)
{
    size_t i;
...
static int set_cxt(struct blkdev_cxt* cxt, struct blkdev_cxt* parent,
                   struct blkdev_cxt* wholedisk, const char* name)
{
    dev_t devno;

    DBG(CXT,
        ul_debugobj(cxt, "setting context for %s [parent=%p, wholedisk=%p]",
    ...
    devno = __sysfs_devname_to_devno(lsblk->sysroot, cxt->name,
                                     wholedisk ? wholedisk->name : NULL);
    ...
    if (lsblk->inverse)
    {
        cxt->sysfs = ul_new_sysfs_path(
            devno, wholedisk ? wholedisk->sysfs : NULL, lsblk->sysroot);
        if (!cxt->sysfs)
        {
            DBG(CXT, ul_debugobj(cxt, "%s: failed to initialize sysfs handler",
                                 cxt->name));
            return -1;
        }
        if (parent)
    ...
    {
        cxt->sysfs = ul_new_sysfs_path(devno, parent ? parent->sysfs : NULL,
                                       lsblk->sysroot);
        if (!cxt->sysfs)
        {
            DBG(CXT, ul_debugobj(cxt, "%s: failed to initialize sysfs handler",
    ...
    DBG(CXT,
        ul_debugobj(cxt, "%s: npartitions=%d, nholders=%d, nslaves=%d",
                    cxt->name, cxt->npartitions, cxt->nholders, cxt->nslaves));

    /* ignore non-SCSI devices */
    if (lsblk->scsi &&
        sysfs_blkdev_scsi_get_hctl(cxt->sysfs, NULL, NULL, NULL, NULL))
    {
    ...
    DBG(CXT,
        ul_debugobj(cxt, "%s: context successfully initialized", cxt->name));
    return 0;
}
 ...
 */
static int list_partitions(struct blkdev_cxt* wholedisk_cxt,
                           struct blkdev_cxt* parent_cxt, const char* part_name)
{
    DIR* dir;
    struct dirent* d;
    ...
    while ((d = xreaddir(dir)))
    {
        ...
        if (lsblk->inverse)
        {
            ...
            fill_table_line(&part_cxt,
                            parent_cxt ? parent_cxt->scols_line : NULL);
            if (!lsblk->nodeps)
        ...
        else
        {
             ...
             */
            int ps =
                set_cxt(&part_cxt, wholedisk_cxt, wholedisk_cxt, d->d_name);

            /* Print whole disk only once */
            if (r)
                fill_table_line(wholedisk_cxt,
                                parent_cxt ? parent_cxt->scols_line : NULL);
    ...
    DBG(CXT,
        ul_debugobj(wholedisk_cxt, "probe whole-disk for partitions -- done"));
    closedir(dir);
    return r;
}
...
/*
 * List device dependencies: partitions, holders (inverse = 0) or slaves
 * (inverse = 1).
 */
static int list_deps(struct blkdev_cxt* cxt)
{
    ...
    DBG(CXT, ul_debugobj(cxt, "%s: checking for '%s' dependence", cxt->name,
                         depname));
    ...
    DBG(CXT,
        ul_debugobj(cxt, "%s: checking for '%s' -- done", cxt->name, depname));
    return 0;
}
                          ...
                          int do_partitions, const char* part_name)
{
    if (do_partitions && cxt->npartitions)
        ...
        fill_table_line(cxt, parent ? parent->scols_line
                                    : NULL); /* whole-disk only */
 ...
 */
static int cmp_u64_cells(struct libscols_cell* a, struct libscols_cell* b,
...
static void __attribute__((__noreturn__)) usage(void)
{
    ...
    fputs(_(" -b, --bytes          print SIZE in bytes rather than in human "
            "readable format\n"),
          out);
    ...
    fputs(_(" -e, --exclude <list> exclude devices by major number (default: "
            "RAM disks)\n"),
          out);
    fputs(_(" -f, --fs             output info about filesystems\n"), out);
    fputs(_(" -i, --ascii          use ascii characters only\n"), out);
    fputs(_(" -I, --include <list> show only devices with specified major "
            "numbers\n"),
          out);
...
int main(int argc, char* argv[])
{
    ...
    while ((c = getopt_long(argc, argv, "abdDze:fhJlnmo:OpPiI:rstVSTx:",
                            longopts, NULL)) != -1)
    {
    ...
    scols_table_enable_noheadings(lsblk->table,
                                  !!(lsblk->flags & LSBLK_NOHEADINGS));
    ...
    for (i = 0; i < ncolumns; i++)
    {
        ...
        if (!lsblk->sort_col && lsblk->sort_id == id)
        {
            ...
            scols_column_set_cmpfunc(cl,
                                     ci->type == COLTYPE_NUM
                                         ? cmp_u64_cells
                                         : ci->type == COLTYPE_SIZE
                                               ? cmp_u64_cells
                                               : ci->type == COLTYPE_SORTNUM
                                                     ? cmp_u64_cells
                                                     : scols_cmpstr_cells,
                                     NULL);
        }
