...
rfbBool HandleCursorShape(rfbClient* client, int xhot, int yhot, int width,
                          int height, uint32_t enc)
{
    int bytesPerPixel;
    ...
    if (enc == rfbEncodingXCursor)
    {
        ...
        switch (bytesPerPixel)
        {
            ...
            case 2:
                for (x = 0; x < width * height; x++)
                    ((uint16_t*)client->rcSource)[x] =
                        (uint16_t)colors[client->rcSource[x * 2]];
                break;
            case 4:
                for (x = 0; x < width * height; x++)
                    ((uint32_t*)client->rcSource)[x] =
                        colors[client->rcSource[x * 4]];
                break;
        }
    }
    else
    { /* enc == rfbEncodingRichCursor */

        if (!ReadFromRFBServer(client, (char*)client->rcSource,
    ...
    {
        client->GotCursorShape(client, xhot, yhot, width, height,
                               bytesPerPixel);
    }
