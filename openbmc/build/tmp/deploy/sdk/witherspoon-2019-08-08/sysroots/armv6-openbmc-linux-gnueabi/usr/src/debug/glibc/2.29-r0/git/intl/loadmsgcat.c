                     ...
                     struct binding* domainbinding)
{
    ...
    data =
        (struct mo_file_header*)mmap(NULL, size, PROT_READ, MAP_PRIVATE, fd, 0);
    ...
    switch (revision >> 16)
    {
        ...
        case 1:
            ...
            domain->orig_tab =
                (const struct string_desc*)((char*)data +
                                            W(domain->must_swap,
            ...
            switch (revision & 0xffff)
            {
                ...
                default:
                {
                    ...
                    if (n_sysdep_strings > 0)
                    {
                        ...
                        /* Compute the amount of additional memory needed for
                           the system dependent strings and the augmented hash
                           table. At the same time, also drop string pairs which
                           refer to an undefined system dependent segment.  */
                                ...
                                if (valid)
                                {
                                    for (j = 0; j < 2; j++)
                                    {
                                        ...
                                        if (W(domain->must_swap,
                                              p->sysdepref) == SEGMENTS_END)
                                        {
                                            /* Only one static segment.  */
                                            inmem_tab_entry->length = W(
                                                domain->must_swap, p->segsize);
...
#ifdef _LIBC
void __libc_freeres_fn_section _nl_unload_domain(struct loaded_domain* domain)
{
    size_t i;
