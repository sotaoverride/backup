...
static gboolean
    g_file_output_stream_seekable_truncate(GSeekable* seekable, goffset offset,
                                           GCancellable* cancellable,
                                           GError** error);
static void g_file_output_stream_real_query_info_async(
    GFileOutputStream* stream, const char* attributes, int io_priority,
    GCancellable* cancellable, GAsyncReadyCallback callback,
    gpointer user_data);
static GFileInfo* g_file_output_stream_real_query_info_finish(
...
G_DEFINE_TYPE_WITH_CODE(
    GFileOutputStream, g_file_output_stream, G_TYPE_OUTPUT_STREAM,
    ...
    GCancellable* cancellable, GAsyncReadyCallback callback, gpointer user_data)
{
    ...
    if (!g_output_stream_set_pending(output_stream, &error))
    {
        g_task_report_error(stream, callback, user_data,
                            g_file_output_stream_query_info_async, error);
        return;
    }
...
static gboolean g_file_output_stream_seek(GFileOutputStream* stream,
...
static gboolean g_file_output_stream_seekable_seek(GSeekable* seekable,
                                                   ...
                                                   GError** error)
{
    return g_file_output_stream_seek(G_FILE_OUTPUT_STREAM(seekable), offset,
                                     type, cancellable, error);
}
...
static gboolean g_file_output_stream_truncate(GFileOutputStream* stream,
                                              goffset size,
                                              GCancellable* cancellable,
                                              GError** error)
{
...
static gboolean
    g_file_output_stream_seekable_truncate(GSeekable* seekable, goffset size,
                                           GCancellable* cancellable,
                                           GError** error)
{
    return g_file_output_stream_truncate(G_FILE_OUTPUT_STREAM(seekable), size,
                                         cancellable, error);
}
/********************************************
...
static void query_info_async_thread(GTask* task, gpointer source_object,
                                    gpointer task_data,
                                    GCancellable* cancellable)
{
...
static GFileInfo* g_file_output_stream_real_query_info_finish(
    GFileOutputStream* stream, GAsyncResult* res, GError** error)
{
