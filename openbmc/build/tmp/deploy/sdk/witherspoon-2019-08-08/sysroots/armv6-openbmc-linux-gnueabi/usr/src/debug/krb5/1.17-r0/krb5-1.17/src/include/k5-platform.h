...
/*
 * Copyright 2003, 2004, 2005, 2007, 2008, 2009 Massachusetts Institute of
...
#define MAKE_INIT_FUNCTION(NAME)                                               \
    ...
    static k5_init_t JOIN__2(NAME, once) = {K5_ONCE_INIT, 0, 0,                \
                                            JOIN__2(NAME, aux)};               \
    ...
    static int NAME(void)
#define CALL_INIT_FUNCTION(NAME) k5_call_init_function(&JOIN__2(NAME, once))
...
{
    int error;
    unsigned char did_run;
} k5_init_t;
#define MAKE_INIT_FUNCTION(NAME)                                               \
    MAYBE_DUMMY_INIT(NAME)                                                     \
    static k5_init_t JOIN__2(NAME, ran) = {0, 2};                              \
    static void JOIN__2(NAME, aux)(void) __attribute__((constructor));         \
    static int NAME(void);                                                     \
    static void JOIN__2(NAME, aux)(void)                                       \
    {                                                                          \
...
#define CALL_INIT_FUNCTION(NAME)                                               \
    (JOIN__2(NAME, ran).did_run == 3 ? JOIN__2(NAME, ran).error : (abort(), 0))
#define INITIALIZER_RAN(NAME)                                                  \
    (JOIN__2(NAME, ran).did_run == 3 && JOIN__2(NAME, ran).error == 0)
...
#define CALL_INIT_FUNCTION(NAME)                                               \
    (JOIN__2(NAME, ran).did_run == 3 ? JOIN__2(NAME, ran).error : (abort(), 0))
#define INITIALIZER_RAN(NAME) (JOIN__2(NAME, ran).error == 0)
...
#define MAKE_FINI_FUNCTION(NAME)                                               \
    ...
    void JOIN__2(NAME, auxfini)(shl_t h, int l)                                \
    {                                                                          \
        if (!l)                                                                \
            NAME();                                                            \
    }                                                                          \
...
#define MAKE_FINI_FUNCTION(NAME) void NAME(void)
 ...
 */
#define MAKE_FINI_FUNCTION(NAME) static void NAME(void) UNUSED
...
#define K5_LE
    ...
    (defined(__hpux) && defined(__ia64__))
#define K5_BE
#endif
#endif
#if defined(K5_BE) && defined(K5_LE)
...
/* Optimize for GCC on platforms with known byte orders.
   ...
   GCC.  */
#ifdef __GNUC__
#define PUT(SIZE, PTR, VAL)                                                    \
    (((struct { uint##SIZE##_t i; } __attribute__((packed))*)(PTR))->i = (VAL))
#define GET(SIZE, PTR)                                                         \
    (((const struct { uint##SIZE##_t i; } __attribute__((packed))*)(PTR))->i)
...
static inline void store_16_be(unsigned int val, void* vp)
{
...
static inline void store_32_be(unsigned int val, void* vp)
{
    unsigned char* p = (unsigned char*)vp;
#if defined(__GNUC__) && defined(K5_BE) && !defined(__cplusplus)
    PUT(32, p, val);
...
static inline void store_64_be(uint64_t val, void* vp)
{
    unsigned char* p = (unsigned char*)vp;
#if defined(__GNUC__) && defined(K5_BE) && !defined(__cplusplus)
    PUT(64, p, val);
...
static inline unsigned int load_32_be(const void* cvp)
{
    const unsigned char* p = (const unsigned char*)cvp;
#if defined(__GNUC__) && defined(K5_BE) && !defined(__cplusplus)
    return GET(32, p);
...
#else
    return (p[3] | (p[2] << 8) | ((uint32_t)p[1] << 16) |
            ((uint32_t)p[0] << 24));
#endif
}
static inline uint64_t load_64_be(const void* cvp)
{
    const unsigned char* p = (const unsigned char*)cvp;
#if defined(__GNUC__) && defined(K5_BE) && !defined(__cplusplus)
    return GET(64, p);
...
static inline void store_16_le(unsigned int val, void* vp)
{
    unsigned char* p = (unsigned char*)vp;
...
static inline void store_32_le(unsigned int val, void* vp)
{
    unsigned char* p = (unsigned char*)vp;
#if defined(__GNUC__) && defined(K5_LE) && !defined(__cplusplus)
    PUT(32, p, val);
...
static inline void store_64_le(uint64_t val, void* vp)
{
    unsigned char* p = (unsigned char*)vp;
#if defined(__GNUC__) && defined(K5_LE) && !defined(__cplusplus)
    PUT(64, p, val);
...
static inline unsigned short load_16_le(const void* cvp)
{
    const unsigned char* p = (const unsigned char*)cvp;
#if defined(__GNUC__) && defined(K5_LE) && !defined(__cplusplus)
    return GET(16, p);
...
static inline unsigned int load_32_le(const void* cvp)
{
    const unsigned char* p = (const unsigned char*)cvp;
#if defined(__GNUC__) && defined(K5_LE) && !defined(__cplusplus)
    return GET(32, p);
...
static inline uint64_t load_64_le(const void* cvp)
{
    const unsigned char* p = (const unsigned char*)cvp;
#if defined(__GNUC__) && defined(K5_LE) && !defined(__cplusplus)
    return GET(64, p);
...
static inline void store_16_n(unsigned int val, void* vp)
{
    UINT16_TYPE n = val;
    memcpy(vp, &n, 2);
}
static inline void store_32_n(unsigned int val, void* vp)
{
    UINT32_TYPE n = val;
    memcpy(vp, &n, 4);
}
static inline void store_64_n(uint64_t val, void* vp)
{
    uint64_t n = val;
    memcpy(vp, &n, 8);
}
static inline unsigned short load_16_n(const void* p)
{
    UINT16_TYPE n;
    memcpy(&n, p, 2);
    return n;
}
static inline unsigned int load_32_n(const void* p)
{
    UINT32_TYPE n;
    memcpy(&n, p, 4);
    return n;
}
static inline uint64_t load_64_n(const void* p)
{
    uint64_t n;
    memcpy(&n, p, 8);
    return n;
...
static inline uint64_t k5_htonll(uint64_t val)
{
#ifdef K5_BE
    return val;
...
static inline uint64_t k5_ntohll(uint64_t val)
{
...
#define k5_getpwnam_r(NAME, REC, BUF, BUFSIZE, OUT)                            \
    (getpwnam_r(NAME, REC, BUF, BUFSIZE, OUT) == 0 ? (*(OUT) == NULL ? -1 : 0) \
                                                   : -1)
#else
/* POSIX drafts? */
#ifdef GETPWNAM_R_RETURNS_INT
#define k5_getpwnam_r(NAME, REC, BUF, BUFSIZE, OUT)                            \
    (getpwnam_r(NAME, REC, BUF, BUFSIZE) == 0 ? (*(OUT) = REC, 0)              \
                                              : (*(OUT) = NULL, -1))
#else
#define k5_getpwnam_r(NAME, REC, BUF, BUFSIZE, OUT)                            \
...
#define k5_getpwuid_r(UID, REC, BUF, BUFSIZE, OUT)                             \
    (getpwuid_r(UID, REC, BUF, BUFSIZE, OUT) == 0 ? (*(OUT) == NULL ? -1 : 0)  \
                                                  : -1)
#else
...
#ifdef GETPWNAM_R_RETURNS_INT
#define k5_getpwuid_r(UID, REC, BUF, BUFSIZE, OUT)                             \
    (getpwuid_r(UID, REC, BUF, BUFSIZE) == 0 ? (*(OUT) = REC, 0)               \
                                             : (*(OUT) = NULL, -1))
#else
#define k5_getpwuid_r(UID, REC, BUF, BUFSIZE, OUT)                             \
