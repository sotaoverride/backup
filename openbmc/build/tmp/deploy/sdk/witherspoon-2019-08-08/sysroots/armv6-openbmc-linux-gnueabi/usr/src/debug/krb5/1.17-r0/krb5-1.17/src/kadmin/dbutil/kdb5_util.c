...
/*
 * (C) Copyright 1990,1991, 1996, 2008, 2009 by the Massachusetts Institute of
...
static struct _cmd_table* cmd_lookup(name) char* name;
{
...
int add_db_arg(char* arg)
{
    ...
    temp = realloc(db5util_db_args, sizeof(char*) * (db5util_db_args_size + 1));
    if (temp == NULL)
        return 0;
...
char* argv[];
{
    ...
    retval = kadm5_get_config_params(util_context, 1, &global_params,
                                     &global_params);
    if (retval)
    {
        com_err(progname, retval,
...
static void add_random_key(argc, argv) int argc;
char** argv;
{
    krb5_error_code ret;
    ...
    ret = krb5_string_to_keysalts(ks_str, NULL, NULL, 0, &keysalts,
                                  &num_keysalts);
    if (ret)
    {
