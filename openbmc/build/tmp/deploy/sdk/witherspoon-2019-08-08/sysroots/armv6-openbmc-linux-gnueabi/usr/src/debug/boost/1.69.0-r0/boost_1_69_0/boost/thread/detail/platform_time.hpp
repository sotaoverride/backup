...
//   MachInfo.denom are run time constants supplied by the OS.  This clock has
//   no relationship to the Gregorian calendar.  It's main use is as a high
//   resolution timer.
...
struct platform_duration
{
    ...
    boost::time_max_t getNs() const
    {
        return timespec_to_ns(ts_val);
    }
#else
    ...
    platform_duration(boost::posix_time::time_duration const& rel_time)
    {
        ...
        ts_val.tv_nsec =
            static_cast<long>(rel_time.fractional_seconds() *
                              (1000000000l / rel_time.ticks_per_second()));
#else
        ns_val = static_cast<boost::time_max_t>(rel_time.total_seconds()) *
                 1000000000l;
        ns_val += rel_time.fractional_seconds() *
                  (1000000000l / rel_time.ticks_per_second());
#endif
    }
#endif
...
inline bool operator==(platform_duration const& lhs,
                       platform_duration const& rhs)
{
...
struct real_platform_timepoint
{
    ...
    real_platform_timepoint(
        chrono::time_point<chrono::system_clock, Duration> const& abs_time) :
...
inline bool operator==(real_platform_timepoint const& lhs,
                       real_platform_timepoint const& rhs)
{
    return lhs.getNs() == rhs.getNs();
}
inline bool operator!=(real_platform_timepoint const& lhs,
...
inline real_platform_timepoint operator+(real_platform_timepoint const& lhs,
                                         platform_duration const& rhs)
{
    return real_platform_timepoint(lhs.getNs() + rhs.getNs());
}
inline real_platform_timepoint operator+(platform_duration const& lhs,
                                         real_platform_timepoint const& rhs)
{
    return real_platform_timepoint(lhs.getNs() + rhs.getNs());
}
inline platform_duration operator-(real_platform_timepoint const& lhs,
                                   real_platform_timepoint const& rhs)
{
...
struct real_platform_clock
{
    static real_platform_timepoint now()
    {
        ...
        boost::time_max_t ns =
            ((((static_cast<boost::time_max_t>(ft.dwHighDateTime) << 32) |
               ft.dwLowDateTime) -
        ...
        if (::clock_gettime(CLOCK_REALTIME, &ts))
        {
            BOOST_ASSERT(
                0 &&
                "Boost::Thread - clock_gettime(CLOCK_REALTIME) Internal Error");
...
#if defined BOOST_THREAD_USES_CHRONO
    // This conversion assumes that chrono::steady_clock::time_point and
    // mono_platform_timepoint share the same epoch.
    template <class Duration>
    mono_platform_timepoint(
        chrono::time_point<chrono::steady_clock, Duration> const& abs_time) :
        dur(abs_time.time_since_epoch())
    {
    }
#endif
...
inline bool operator==(mono_platform_timepoint const& lhs,
                       mono_platform_timepoint const& rhs)
{
    return lhs.getNs() == rhs.getNs();
}
inline bool operator!=(mono_platform_timepoint const& lhs,
...
inline mono_platform_timepoint operator+(mono_platform_timepoint const& lhs,
                                         platform_duration const& rhs)
{
    return mono_platform_timepoint(lhs.getNs() + rhs.getNs());
}
inline mono_platform_timepoint operator+(platform_duration const& lhs,
                                         mono_platform_timepoint const& rhs)
{
    return mono_platform_timepoint(lhs.getNs() + rhs.getNs());
}
inline platform_duration operator-(mono_platform_timepoint const& lhs,
                                   mono_platform_timepoint const& rhs)
{
    return platform_duration(lhs.getNs() - rhs.getNs());
}
...
struct mono_platform_clock
{
    static mono_platform_timepoint now()
    {
        ...
        // Chrono so that chrono::steady_clock::now() and this function share
        // the same epoch and so can be converted between each other.
        boost::detail::winapi::LARGE_INTEGER_ freq;
        if (!boost::detail::winapi::QueryPerformanceFrequency(&freq))
        {
            BOOST_ASSERT(
                0 &&
                "Boost::Thread - QueryPerformanceFrequency Internal Error");
        ...
        while (!boost::detail::winapi::QueryPerformanceCounter(&pcount))
        {
            if (++times > 3)
            {
                BOOST_ASSERT(
                    0 &&
                    "Boost::Thread - QueryPerformanceCounter Internal Error");
                return mono_platform_timepoint(0);
            }
        }
...
#elif defined(BOOST_THREAD_CHRONO_MAC_API)
        kern_return_t err;
        threads::chrono_details::FP fp =
            threads::chrono_details::init_steady_clock(err);
        if (err != 0)
        {
            BOOST_ASSERT(0 && "Boost::Chrono - Internal Error");
        }
        ...
        {
            BOOST_ASSERT(0 && "Boost::Thread - clock_gettime(CLOCK_MONOTONIC) "
                              "Internal Error");
            return mono_platform_timepoint(0);
        }
        return mono_platform_timepoint(ts);
