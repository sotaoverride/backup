...
static void request_dispatch(ConnectionData* d, int enable)
{
    ...
    if (enable)
    {
        assert(dbus_connection_get_dispatch_status(d->connection) ==
...
static void watch_callback(AvahiWatch* avahi_watch, AVAHI_GCC_UNUSED int fd,
...
static dbus_bool_t update_watch(const AvahiPoll* poll_api,
                                DBusWatch* dbus_watch)
{
    ...
    if (b && !avahi_watch)
    {
     ...
     DBUS_VERSION_MICRO >= 1) ||                                               \
    (DBUS_VERSION_MAJOR == 1 && DBUS_VERSION_MINOR > 1) ||                     \
...
static void update_timeout(TimeoutData* timeout)
{
    ...
    {
        struct timeval tv;
        avahi_elapse_time(&tv, dbus_timeout_get_interval(timeout->dbus_timeout),
                          0);
...
static dbus_bool_t add_timeout(DBusTimeout* dbus_timeout, void* userdata)
{
    ...
    if (!(timeout->avahi_timeout = d->poll_api->timeout_new(
              d->poll_api, b ? &tv : NULL, timeout_callback, timeout)))
    {
        avahi_free(timeout);
        return FALSE;
    }

    dbus_timeout_set_data(dbus_timeout, timeout,
                          (DBusFreeFunction)timeout_data_unref);
    return TRUE;
}
...
static void timeout_toggled(DBusTimeout* dbus_timeout,
...
static void dispatch_status(AVAHI_GCC_UNUSED DBusConnection* connection,
                            DBusDispatchStatus new_status, void* userdata)
{
    ConnectionData* d = userdata;
...
int avahi_dbus_connection_glue(DBusConnection* c, const AvahiPoll* poll_api)
{
    ...
    if (!(d->dispatch_timeout = poll_api->timeout_new(
              poll_api, NULL, dispatch_timeout_callback, d)))
        goto fail;

    if (!(dbus_connection_set_watch_functions(
            c, add_watch, remove_watch, watch_toggled, connection_data_ref(d),
            (DBusFreeFunction)connection_data_unref)))
        goto fail;

    if (!(dbus_connection_set_timeout_functions(
            c, add_timeout, remove_timeout, timeout_toggled,
            connection_data_ref(d), (DBusFreeFunction)connection_data_unref)))
        goto fail;

    dbus_connection_set_dispatch_status_function(
        c, dispatch_status, connection_data_ref(d),
        (DBusFreeFunction)connection_data_unref);
