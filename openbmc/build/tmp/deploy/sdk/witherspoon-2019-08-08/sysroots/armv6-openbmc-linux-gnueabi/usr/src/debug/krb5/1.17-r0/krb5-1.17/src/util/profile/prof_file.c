...
static struct global_shared_profile_data krb5int_profile_shared_data = {
    0, K5_MUTEX_PARTIAL_INITIALIZER};
...
prf_data_t profile_make_prf_data(const char* filename)
{
    prf_data_t d;
                            ...
                            prf_file_t* ret_prof, char** ret_modspec)
{
    ...
    if (home_env)
    {
        if (asprintf(&expanded_filename, "%s%s", home_env, filespec + 1) < 0)
            expanded_filename = 0;
    }
    else
...
errcode_t profile_update_file_data_locked(prf_data_t data, char** ret_modspec)
{
    ...
    frac = 0;
#endif
    if (st.st_mtime == data->timestamp && frac == data->frac_ts &&
        data->root != NULL)
    {
        return 0;
    }
...
static int make_hard_link(const char* oldpath, const char* newpath)
{
#ifdef _WIN32
    return -1;
#else
