...
static int verify_chain(X509_STORE_CTX* ctx)
{
    ...
    if ((ok = build_chain(ctx)) == 0 ||
        ...
        (ok = check_auth_level(ctx)) == 0 || (ok = check_id(ctx)) == 0 || 1)
...
static int check_name_constraints(X509_STORE_CTX* ctx)
{
    ...
    for (i = sk_X509_num(ctx->chain) - 1; i >= 0; i--)
    {
        ...
        if (x->ex_flags & EXFLAG_PROXY)
        {
             ...
             * multivalued RDN
             */
            if (X509_NAME_ENTRY_set(
                    X509_NAME_get_entry(tmpsubject, last_object_loc)) ==
                X509_NAME_ENTRY_set(
                    X509_NAME_get_entry(tmpsubject, last_object_loc - 1)))
            {
                err = X509_V_ERR_PROXY_SUBJECT_NAME_VIOLATION;
                goto proxy_name_done;
            }
            ...
            tmpentry = X509_NAME_delete_entry(tmpsubject, last_object_loc);
            last_object_nid = OBJ_obj2nid(X509_NAME_ENTRY_get_object(tmpentry));
        ...
        proxy_name_done:
            if (err != X509_V_OK && !verify_cb_cert(ctx, x, i, err))
                return 0;
        }
    ...
    if (ret == X509_PCY_TREE_INVALID)
    {
        ...
        for (i = 1; i < sk_X509_num(ctx->chain); i++)
        {
            ...
            if (!(x->ex_flags & EXFLAG_INVALID_POLICY))
                continue;
            if (!verify_cb_cert(ctx, x, i, X509_V_ERR_INVALID_POLICY_EXTENSION))
                return 0;
        }
        return 1;
    }
