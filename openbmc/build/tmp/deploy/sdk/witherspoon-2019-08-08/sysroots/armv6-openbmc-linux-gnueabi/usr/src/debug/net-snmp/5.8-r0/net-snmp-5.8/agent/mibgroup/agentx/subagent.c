...
int handle_subagent_set_response(int op, netsnmp_session* session, int reqid,
                                 netsnmp_pdu* pdu, void* magic);
#endif /* !NETSNMP_NO_WRITE_SUPPORT */
void subagent_startup_callback(unsigned int clientreg, void* clientarg);
...
int subagent_startup(int majorID, int minorID, void* serverarg, void* clientarg)
{
...
static void subagent_init_callback_session(void)
{
    if (agentx_callback_sess == NULL)
    {
        agentx_callback_sess = netsnmp_callback_open(
            callback_master_num, handle_subagent_response, NULL, NULL);
...
int subagent_init(void)
{
    ...
    snmp_register_callback(SNMP_CALLBACK_LIBRARY,
                           SNMP_CALLBACK_POST_READ_CONFIG, subagent_startup,
                           NULL);
...
#ifndef NETSNMP_NO_WRITE_SUPPORT
struct agent_netsnmp_set_info* save_set_vars(netsnmp_session* ss,
                                             netsnmp_pdu* pdu)
{
    struct agent_netsnmp_set_info* ptr;

    ptr = (struct agent_netsnmp_set_info*)malloc(
        sizeof(struct agent_netsnmp_set_info));
    if (ptr == NULL)
        return NULL;
...
struct agent_netsnmp_set_info* restore_set_vars(netsnmp_session* sess,
                                                netsnmp_pdu* pdu)
{
    struct agent_netsnmp_set_info* ptr;
...
void free_set_vars(netsnmp_session* ss, netsnmp_pdu* pdu)
{
...
static void send_agentx_error(netsnmp_session* session, netsnmp_pdu* pdu,
                              int errstat, int errindex)
{
    ...
    DEBUGMSGTL(("agentx/subagent",
                "Sending AgentX response error stat %d idx %d\n", errstat,
                errindex));
...
int handle_agentx_packet(int operation, netsnmp_session* session, int reqid,
                         netsnmp_pdu* pdu, void* magic)
{
    ...
    if (operation == NETSNMP_CALLBACK_OP_DISCONNECT)
    {
        ...
        int period = netsnmp_ds_get_int(NETSNMP_DS_APPLICATION_ID,
        ...
        {
            /*
             * Pings are enabled, so periodically attempt to re-establish
             * contact with the master agent.  Don't worry about the handle,
             * agentx_reopen_session unregisters itself if it succeeds in
             * talking to the master agent.
             */
    ...
    {
        DEBUGMSGTL(
            ("agentx/subagent", "unexpected callback op %d\n", operation));
        return 1;
    }
    ...
    DEBUGMSGTL(("agentx/subagent",
                "handling AgentX request (req=0x%x,trans="
                "0x%x,sess=0x%x)\n",
    ...
    switch (pdu->command)
    {
        ...
        case AGENTX_MSG_GETNEXT:
             ...
             * if the master agent has requested scoping for some of the
             * varbinds that information is stored there.
             */
        ...
        case AGENTX_MSG_GETBULK:
            ...
            smagic->ovars = snmp_clone_varbind(pdu->variables);
            DEBUGMSGTL(
                ("agentx/subagent", "saved variables at %p\n", smagic->ovars));
            mycallback = handle_subagent_response;
            break;
                             ...
                             netsnmp_pdu* pdu, void* magic)
{
    ...
    DEBUGMSGTL(("agentx/subagent",
                ...
                pdu->command, smagic->original_command, (unsigned)pdu->reqid,
    ...
    {
        DEBUGMSGTL(("agentx/subagent", "do getNext scope processing %p %p\n",
             ...
             u = u->next_variable, v = v->next_variable)
        {
            if (snmp_oid_compare(u->val.objid, u->val_len / sizeof(oid),
                                 nullOid, nullOidLen / sizeof(oid)) != 0)
            {
                /*
                 * The master agent requested scoping for this variable.
                 */
                rc = snmp_oid_compare(v->name, v->name_length, u->val.objid,
                ...
                DEBUGMSGOID(("agentx/subagent", u->val.objid,
                             u->val_len / sizeof(oid)));
...
int agentx_registration_callback(int majorID, int minorID, void* serverarg,
                                 void* clientarg)
{
    ...
    if (minorID == SNMPD_CALLBACK_REGISTER_OID)
        return agentx_register(agentx_ss, reg_parms->name, reg_parms->namelen,
                               reg_parms->priority, reg_parms->range_subid,
                               reg_parms->range_ubound, reg_parms->timeout,
                               reg_parms->flags, reg_parms->contextName);
    else
        return agentx_unregister(agentx_ss, reg_parms->name, reg_parms->namelen,
                                 reg_parms->priority, reg_parms->range_subid,
                                 reg_parms->range_ubound,
                                 reg_parms->contextName);
}

static int agentx_sysOR_callback(int majorID, int minorID, void* serverarg,
                                 void* clientarg)
{
    ...
    if (minorID == SNMPD_CALLBACK_REG_SYSOR)
        return agentx_add_agentcaps(agentx_ss, reg_parms->name,
                                    reg_parms->namelen, reg_parms->descr);
    else
        return agentx_remove_agentcaps(agentx_ss, reg_parms->name,
                                       reg_parms->namelen);
}

static int subagent_shutdown(int majorID, int minorID, void* serverarg,
                             void* clientarg)
{
...
void agentx_register_callbacks(netsnmp_session* s)
{
    ...
    DEBUGMSGTL(
        ("agentx/subagent", "registering callbacks for session %p\n", s));
    ...
    snmp_register_callback(SNMP_CALLBACK_APPLICATION,
                           ...
                           agentx_registration_callback, sess_p);
    snmp_register_callback(SNMP_CALLBACK_APPLICATION, SNMPD_CALLBACK_REG_SYSOR,
                           agentx_sysOR_callback, sess_p);
    snmp_register_callback(SNMP_CALLBACK_APPLICATION,
                           SNMPD_CALLBACK_UNREG_SYSOR, agentx_sysOR_callback,
                           sess_p);
}
...
{
    DEBUGMSGTL(
        ("agentx/subagent", "unregistering callbacks for session %p\n", ss));
    ...
    snmp_unregister_callback(SNMP_CALLBACK_APPLICATION,
                             ...
                             agentx_registration_callback, ss->myvoid, 1);
    snmp_unregister_callback(SNMP_CALLBACK_APPLICATION,
                             SNMPD_CALLBACK_REG_SYSOR, agentx_sysOR_callback,
                             ss->myvoid, 1);
    snmp_unregister_callback(SNMP_CALLBACK_APPLICATION,
                             SNMPD_CALLBACK_UNREG_SYSOR, agentx_sysOR_callback,
                             ss->myvoid, 1);
...
int subagent_open_master_session(void)
{
    ...
    if (add_trap_session(main_session, AGENTX_MSG_NOTIFY, 1, AGENTX_VERSION_1))
    {
    ...
    snmp_call_callbacks(SNMP_CALLBACK_APPLICATION, SNMPD_CALLBACK_INDEX_START,
                        (void*)main_session);

    snmp_log(LOG_INFO, "NET-SNMP version %s AgentX subagent connected\n",
             netsnmp_get_version());
    DEBUGMSGTL(
        ("agentx/subagent", "opening session...  DONE (%p)\n", main_session));

    return 0;
}

static void agentx_reopen_sysORTable(const struct sysORTable* data, void* v)
{
...
void agentx_reopen_session(unsigned int clientreg, void* clientarg)
{
    DEBUGMSGTL(
        ("agentx/subagent", "agentx_reopen_session(%d) called\n", clientreg));
...
static int subagent_register_ping_alarm(int majorID, int minorID,
                                        void* serverarg, void* clientarg)
{

    netsnmp_session* ss = (netsnmp_session*)clientarg;
    int ping_interval = netsnmp_ds_get_int(
    ...
    else
    {
         ...
         */
        DEBUGMSGTL(
            ("agentx/subagent", "subagent not properly attached, postponing "
                                "registration till later....\n"));
        snmp_alarm_register(ping_interval, SA_REPEAT, agentx_reopen_session,
                            NULL);
    }
    return 0;
}
...
void agentx_check_session(unsigned int clientreg, void* clientarg)
{
    ...
    {
        snmp_log(LOG_WARNING, "AgentX master agent failed to respond to ping.  "
