    ...
    if ((cp = strchr(name, '@')) != NULL)
    {
        if (strchr(cp + 1, '.'))
            RHS = cp + 1;
                      ...
                      hesiod_resolve(context, cp + 1, "rhs-extension")) != NULL)
            RHS = *rhs_list;
        else
        {
            __set_errno(ENOENT);
...
char** hesiod_resolve(void* context, const char* name, const char* type)
{
    ...
    if (retvec == NULL && (errno == ENOENT || errno == ECONNREFUSED) &&
        ctx->classes[1])
    ...
    while (fgets(buf, sizeof(buf), fp) != NULL)
    {
        ...
        *cp++ = '\0';
        ...
        else if (strcasecmp(key, "classes") == 0)
        {
            ...
            else if (n == 1 || ctx->classes[0] == ctx->classes[1])
                ctx->classes[1] = 0;
        }
    }
    fclose(fp);
...
static char** get_txt_records(struct hesiod_p* ctx, int class, const char* name)
{
     ...
     */
    n = res_mkquery(QUERY, name, class, T_TXT, NULL, 0, NULL, qbuf,
                    MAX_HESRESP);
    if (n < 0)
    {
