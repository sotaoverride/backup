 ...
 * Since: 2.22
 */
void g_async_initable_init_async(GAsyncInitable* initable, int io_priority,
                                 GCancellable* cancellable,
                                 GAsyncReadyCallback callback,
                                 gpointer user_data)
{
    ...
    (*iface->init_async)(initable, io_priority, cancellable, callback,
                         user_data);
}
...
static void async_init_thread(GTask* task, gpointer source_object,
 ...
 *     finished
 * @user_data: the data to pass to callback function
 * @first_property_name: (nullable): the name of the first property, or %NULL if
 * no properties
 ...
 * Since: 2.22
 */
void g_async_initable_new_async(GType object_type, int io_priority,
                                GCancellable* cancellable,
                                GAsyncReadyCallback callback,
                                gpointer user_data,
                                const gchar* first_property_name, ...)
{
    ...
    g_async_initable_new_valist_async(object_type, first_property_name,
                                      var_args, io_priority, cancellable,
                                      callback, user_data);
    va_end(var_args);
}
 ...
 */
void g_async_initable_newv_async(GType object_type, guint n_parameters,
                                 GParameter* parameters, int io_priority,
                                 GCancellable* cancellable,
                                 GAsyncReadyCallback callback,
                                 gpointer user_data)
{
    ...
    g_async_initable_init_async(G_ASYNC_INITABLE(obj), io_priority, cancellable,
                                callback, user_data);
