...
static void print_qdisc_usage(void)
{
    printf(
        ...
        "\n"
        "EXAMPLE"
        "    # Create hfsc root qdisc 1: and direct unclassified traffic to "
        "class 1:10\n"
        "    nl-qdisc-add --dev=eth1 --parent=root --handle=1: hfsc "
        "--default=10\n");
}
...
static void hfsc_parse_qdisc_argv(struct rtnl_tc* tc, int argc, char** argv)
{
    ...
    for (;;)
    {
        ...
        static struct option long_opts[] = {
            {"help", 0, 0, 'h'}, {"default", 1, 0, ARG_DEFAULT}, {0, 0, 0, 0}};
...
{
    printf("Usage: nl-class-add [...] hfsc [OPTIONS]...\n"
           "\n"
           "OPTIONS\n"
           "     --help                Show this help text.\n"
           ...
           "    nl-class-add --dev=eth1 --parent=1: --classid=1:1 hfsc "
           "--sc=m1:250,d:8,m2:100\n");
}

static int hfsc_get_sc(char* optarg, struct tc_service_curve* sc)
{
...
static void hfsc_parse_class_argv(struct rtnl_tc* tc, int argc, char** argv)
{
    ...
    for (;;)
    {
        ...
        static struct option long_opts[] = {
            {"help", 0, 0, 'h'},  {"rt", 1, 0, ARG_RT}, {"ls", 1, 0, ARG_LS},
            {"sc", 1, 0, ARG_SC}, {"ul", 1, 0, ARG_UL}, {0, 0, 0, 0}};
