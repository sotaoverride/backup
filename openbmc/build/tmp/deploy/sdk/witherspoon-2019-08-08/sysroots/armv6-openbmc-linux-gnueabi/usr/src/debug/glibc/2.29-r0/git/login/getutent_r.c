...
/* Initial Jump table.  */
const struct utfuncs __libc_utmp_unknown_functions = {setutent_unknown,
                                                      getutent_r_unknown,
                                                      getutid_r_unknown,
                                                      getutline_r_unknown,
                                                      pututline_unknown,
...
static int getutid_r_unknown(const struct utmp* id, struct utmp* buffer,
                             struct utmp** result)
{
    /* The backend was not yet initialized.  */
    if (setutent_unknown())
...
static int getutline_r_unknown(const struct utmp* line, struct utmp* buffer,
                               struct utmp** result)
{
    /* The backend was not yet initialized.  */
    if (setutent_unknown())
...
weak_alias(__setutent, setutent)
    ...
    int __getutent_r(struct utmp* buffer, struct utmp** result)
{
    int retval;
...
libc_hidden_def(__getutent_r) weak_alias(__getutent_r, getutent_r)

    struct utmp* __pututline(const struct utmp* data)
{
    struct utmp* buffer;
...
libc_hidden_def(__pututline) weak_alias(__pututline, pututline)
