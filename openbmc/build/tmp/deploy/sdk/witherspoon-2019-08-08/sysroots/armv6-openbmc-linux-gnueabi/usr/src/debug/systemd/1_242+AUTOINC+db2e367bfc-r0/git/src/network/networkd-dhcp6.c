...
static bool dhcp6_get_prefix_delegation(Link* link)
{
    ...
    return IN_SET(link->network->router_prefix_delegation,
                  RADV_PREFIX_DELEGATION_DHCP6, RADV_PREFIX_DELEGATION_BOTH);
}
                                      ...
                                      Link* link)
{
    ...
    if (r < 0)
        log_link_debug_errno(link, r,
                             "Received error on unreachable route removal for "
                             "DHCPv6 delegated subnetl: %m");
...
int dhcp6_lease_pd_prefix_lost(sd_dhcp6_client* client, Link* link)
{
    ...
    while (sd_dhcp6_lease_get_pd(lease, &pd_prefix.in6, &pd_prefix_len,
                                 &lifetime_preferred, &lifetime_valid) >= 0)
    {
        ...
        if (pd_prefix_len < 64)
        {
            ...
            if (r < 0)
            {
                log_link_warning_errno(
                    link, r,
                    "Cannot create unreachable route to delete for DHCPv6 "
                    "delegated subnet %s/%u: %m",
                    strnull(buf), pd_prefix_len);
                continue;
            }

            r = route_add(link, AF_INET6, &pd_prefix, pd_prefix_len, 0, 0, 0,
                          &route);
            if (r < 0)
            {
                log_link_warning_errno(
                    link, r,
                    "Failed to add unreachable route to delete for DHCPv6 "
                    "delegated subnet %s/%u: %m",
                    strnull(buf), pd_prefix_len);
                continue;
            }
            ...
            if (r < 0)
            {
                ...
                log_link_warning_errno(link, r,
                                       "Cannot delete unreachable route for "
                                       "DHCPv6 delegated subnet %s/%u: %m",
                                       strnull(buf), pd_prefix_len);
                                      ...
                                      uint32_t lifetime_valid)
{
    ...
    log_link_debug(dhcp6_link,
                   "Assigning up to %" PRIu64 " prefixes from %s/%u",
    ...
    while (hashmap_iterate(manager->links, i, (void**)&link, NULL))
    {
        ...
        r = dhcp6_pd_prefix_assign(link, &prefix.in6, 64, lifetime_preferred,
                                   lifetime_valid);
        if (r < 0)
        {
            log_link_error_errno(
                link, r, "Unable to %s prefix %s/64 from %s/%u for link: %m",
                assigned_link ? "update" : "assign", strnull(assigned_buf),
                strnull(buf), pd_prefix_len);
            ...
            log_link_debug(link,
                           "Assigned prefix %" PRIu64 "/%" PRIu64
                           " %s/64 from %s/%u to link",
                           n_used + 1, n_prefixes, strnull(assigned_buf),
                           strnull(buf), pd_prefix_len);
...
static int dhcp6_route_handler(sd_netlink* nl, sd_netlink_message* m,
                               Link* link)
{
    int r;
    ...
    if (r < 0 && r != -EEXIST)
        log_link_debug_errno(link, r,
                             "Received error when adding unreachable route for "
                             "DHCPv6 delegated subnet: %m");
...
static int dhcp6_lease_pd_prefix_acquired(sd_dhcp6_client* client, Link* link)
{
                                 ...
                                 &lifetime_preferred, &lifetime_valid) >= 0)
    {
        ...
        if (pd_prefix_len < 64)
        {
            ...
            {
                log_link_warning_errno(link, r,
                                       "Cannot create unreachable route for "
                                       "DHCPv6 delegated subnet %s/%u: %m",
                                       strnull(buf), pd_prefix_len);
                continue;
            }
            ...
            r = route_add(link, AF_INET6, &pd_prefix, pd_prefix_len, 0, 0,
                          table, &route);
            if (r < 0)
            {
                log_link_warning_errno(link, r,
                                       "Failed to add unreachable route for "
                                       "DHCPv6 delegated subnet %s/%u: %m",
                                       strnull(buf), pd_prefix_len);
                continue;
            }
            ...
            {
                log_link_warning_errno(link, r,
                                       "Cannot configure unreachable route for "
                                       "delegated subnet %s/%u: %m",
                                       strnull(buf), pd_prefix_len);
                continue;
            }
        ...
        r = dhcp6_pd_prefix_distribute(link, &i, &pd_prefix.in6, pd_prefix_len,
...
int dhcp6_request_prefix_delegation(Link* link)
{
    ...
    HASHMAP_FOREACH(l, link->manager->links, i)
    {
        ...
        {
            log_link_warning_errno(
                l, r, "Cannot get prefix delegation when adding new link");
            continue;
        }
        ...
        if (enabled != 0)
        {
            log_link_debug(l, "Requesting re-assignment of delegated prefixes "
                              "after adding new link");
        ...
        if (r < 0)
        {
            log_link_warning_errno(l, r,
                                   "Cannot stop DHCPv6 prefix delegation "
                                   "client after adding new link");
            continue;
        }
        ...
        {
            log_link_warning_errno(l, r,
                                   "Cannot restart DHCPv6 prefix delegation "
                                   "client after adding new link");
            continue;
        }

        log_link_debug(l, "Restarted DHCPv6 client to acquire prefix "
                          "delegations after adding new link");
    }
                                 ...
                                 Link* link)
{
    ...
    if (r < 0 && r != -EEXIST)
    {
        ...
        {
            log_link_warning(link, "Could not set extended netlink attributes, "
                                   "reverting to fallback mechanism");
                                ...
                                uint32_t lifetime_valid)
{
    ...
    log_link_info(
        link, "DHCPv6 address %s/%d timeout preferred %d valid %d",
...
static int dhcp6_lease_address_acquired(sd_dhcp6_client* client, Link* link)
{
    ...
    while (sd_dhcp6_lease_get_address(lease, &ip6_addr, &lifetime_preferred,
                                      &lifetime_valid) >= 0)
    {

        r = dhcp6_address_change(link, &ip6_addr, lifetime_preferred,
                                 lifetime_valid);
        if (r < 0)
            return r;
    }
...
int dhcp6_request_address(Link* link, int ir)
{
    ...
    assert(in_addr_is_link_local(
               AF_INET6, (const union in_addr_union*)&link->ipv6ll_address) >
           0);
    ...
    {
        log_link_debug(link, "Enabling managed mode to request DHCPv6 PD with "
    ...
    {
        r = sd_dhcp6_client_set_local_address(link->dhcp6_client,
                                              &link->ipv6ll_address);
        if (r < 0)
            return r;
    }
...
static int dhcp6_set_hostname(sd_dhcp6_client* client, Link* link)
{
    ...
    if (r == -EINVAL && hostname)
         ...
         * DHCP packet. */
        log_link_warning_errno(link, r,
                               "DHCP6 CLIENT: Failed to set hostname from "
...
int dhcp6_configure(Link* link)
{
    ...
    if (r < 0)
        return log_link_error_errno(
            link, r, "DHCP6 CLIENT: Failed to create DHCP6 client: %m");
    ...
    r = sd_dhcp6_client_set_mac(client, (const uint8_t*)&link->mac,
                                sizeof(link->mac), ARPHRD_ETHER);
    if (r < 0)
        return log_link_error_errno(
            link, r, "DHCP6 CLIENT: Failed to set MAC address: %m");
    ...
    if (duid->type == DUID_TYPE_LLT && duid->raw_data_len == 0)
        ...
        r = sd_dhcp6_client_set_duid(
            client, duid->type, duid->raw_data_len > 0 ? duid->raw_data : NULL,
            duid->raw_data_len);
    if (r < 0)
        return log_link_error_errno(link, r,
    ...
    {
        r = sd_dhcp6_client_set_request_option(client,
                                               SD_DHCP6_OPTION_RAPID_COMMIT);
        if (r < 0)
            return log_link_error_errno(link, r,
                                        "DHCP6 CLIENT: Failed to set request "
                                        "flag for rapid commit: %m");
    }
    ...
    if (dhcp6_enable_prefix_delegation(link))
    {
        ...
        if (r < 0)
            return log_link_error_errno(
                link, r, "DHCP6 CLIENT: Failed to set prefix delegation: %m");
    }
