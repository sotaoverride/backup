...
void init_mteTriggerBooleanTable(void)
{
    ...
    reg = netsnmp_create_handler_registration(
        "mteTriggerBooleanTable", mteTriggerBooleanTable_handler,
        mteTBoolTable_oid, mteTBoolTable_oid_len, HANDLER_CAN_RWRITE);
...
int mteTriggerBooleanTable_handler(netsnmp_mib_handler* handler,
                                   ...
                                   netsnmp_request_info* requests)
{
    ...
    DEBUGMSGTL(
        ("disman:event:mib", "Boolean Table handler (%d)\n", reqinfo->mode));
            ...
            for (request = requests; request; request = request->next)
            {
                ...
                switch (tinfo->colnum)
                {
                    ...
                    case COLUMN_MTETRIGGERBOOLEANSTARTUP:
                        ...
                        break;
                    case COLUMN_MTETRIGGERBOOLEANOBJECTSOWNER:
                        memset(entry->mteTBoolObjOwner, 0,
                               sizeof(entry->mteTBoolObjOwner));
                        memcpy(entry->mteTBoolObjOwner,
                               request->requestvb->val.string,
                               request->requestvb->val_len);
                        break;
                    case COLUMN_MTETRIGGERBOOLEANOBJECTS:
                        memset(entry->mteTBoolObjects, 0,
                               sizeof(entry->mteTBoolObjects));
                        memcpy(entry->mteTBoolObjects,
                               request->requestvb->val.string,
                               request->requestvb->val_len);
                        break;
                    case COLUMN_MTETRIGGERBOOLEANEVENTOWNER:
                        memset(entry->mteTBoolEvOwner, 0,
                               sizeof(entry->mteTBoolEvOwner));
                        memcpy(entry->mteTBoolEvOwner,
                               request->requestvb->val.string,
                               request->requestvb->val_len);
                        break;
                    case COLUMN_MTETRIGGERBOOLEANEVENT:
                        memset(entry->mteTBoolEvent, 0,
                               sizeof(entry->mteTBoolEvent));
                        memcpy(entry->mteTBoolEvent,
                               request->requestvb->val.string,
                               request->requestvb->val_len);
                        break;
                }
