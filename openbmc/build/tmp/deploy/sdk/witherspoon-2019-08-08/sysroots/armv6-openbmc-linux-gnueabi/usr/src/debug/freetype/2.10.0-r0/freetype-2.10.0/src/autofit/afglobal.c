...
#define SCRIPT(s, S, d, h, H, ss)                                              \
    AF_DEFINE_SCRIPT_CLASS(af_##s##_script_class, AF_SCRIPT_##S,               \
                           af_##s##_uniranges, af_##s##_nonbase_uniranges,     \
                           AF_##H, ss)
...
#define STYLE(s, S, d, ws, sc, ss, c)                                          \
    AF_DEFINE_STYLE_CLASS(af_##s##_style_class, AF_STYLE_##S, ws, sc, ss, c)
...
static FT_Error af_face_globals_compute_style_coverage(AF_FaceGlobals globals)
{
    ...
    for (ss = 0; af_style_classes[ss]; ss++)
    {
        ...
        {
            if ((FT_UInt)style_class->script == globals->module->default_script)
                dflt = ss;

            for (range = script_class->script_uni_ranges; range->first != 0;
                 range++)
            {
        ...
        for (nn = 0; nn < AF_STYLE_MAX; nn++)
        {
            if (globals->metrics[nn])
            {
                ...
                if (writing_system_class->style_metrics_done)
                    writing_system_class->style_metrics_done(
                        globals->metrics[nn]);
...
FT_LOCAL_DEF(FT_Error)
af_face_globals_get_metrics(AF_FaceGlobals globals, FT_UInt gindex,
                            FT_UInt options, AF_StyleMetrics* ametrics)
{
