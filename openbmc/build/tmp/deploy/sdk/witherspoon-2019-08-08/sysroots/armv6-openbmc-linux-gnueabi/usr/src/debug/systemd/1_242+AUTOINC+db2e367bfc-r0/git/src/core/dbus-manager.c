 ...
 * to serialize our state there, and if we can't we'll fail badly. */
...
static BUS_DEFINE_PROPERTY_GET_GLOBAL(property_get_features, "s",
                                      SYSTEMD_FEATURES);
static BUS_DEFINE_PROPERTY_GET_GLOBAL(
    property_get_architecture, "s",
    architecture_to_string(uname_architecture()));
static BUS_DEFINE_PROPERTY_GET_GLOBAL(property_get_log_target, "s",
                                      log_target_to_string(log_get_target()));
static BUS_DEFINE_PROPERTY_GET2(property_get_system_state, "s", Manager,
                                manager_state, manager_state_to_string);
static BUS_DEFINE_PROPERTY_GET_GLOBAL(property_get_timer_slack_nsec, "t",
                                      (uint64_t)prctl(PR_GET_TIMERSLACK));
static BUS_DEFINE_PROPERTY_GET_REF(property_get_hashmap_size, "u", Hashmap*,
                                   hashmap_size);
static BUS_DEFINE_PROPERTY_GET_REF(property_get_set_size, "u", Set*, set_size);

static int property_get_virtualization(sd_bus* bus, const char* path,
                                       ...
                                       sd_bus_error* error)
{
    ...
    /* Make sure to return the empty string when we detect no virtualization, as
     * that is the API.
     *
...
static int property_get_tainted(sd_bus* bus, const char* path,
...
static int property_set_log_target(sd_bus* bus, const char* path,
                                   const char* interface, const char* property,
...
static int property_get_progress(sd_bus* bus, const char* path,
                                 const char* interface, const char* property,
                                 sd_bus_message* reply, void* userdata,
                                 sd_bus_error* error)
{
...
static int property_get_environment(sd_bus* bus, const char* path,
                                    const char* interface, const char* property,
                                    sd_bus_message* reply, void* userdata,
                                    sd_bus_error* error)
{
...
static int property_get_show_status(sd_bus* bus, const char* path,
                                    const char* interface, const char* property,
                                    sd_bus_message* reply, void* userdata,
                                    sd_bus_error* error)
{
...
static int property_set_runtime_watchdog(sd_bus* bus, const char* path,
                                         const char* interface,
                                         const char* property,
                                         sd_bus_message* value, void* userdata,
                                         sd_bus_error* error)
                                ...
                                sd_bus_error* error)
{
    ...
    /* More or less a wrapper around manager_get_unit() that generates nice
     * errors and has one trick up its sleeve:
...
static int bus_load_unit_by_name(Manager* m, sd_bus_message* message,
                                 const char* name, Unit** ret_unit,
                                 sd_bus_error* error)
{
    assert(m);
    ...
    /* Pretty much the same as bus_get_unit_by_name(), but we also load the unit
     * if necessary. */
...
static int reply_unit_path(Unit* u, sd_bus_message* message,
                           sd_bus_error* error)
{
...
static int method_get_unit(sd_bus_message* message, void* userdata,
                           sd_bus_error* error)
{
    Manager* m = userdata;
    const char* name;
...
static int method_get_unit_by_pid(sd_bus_message* message, void* userdata,
                                  sd_bus_error* error)
{
    Manager* m = userdata;
    pid_t pid;
    ...
    if (!u)
        return sd_bus_error_setf(
            error, BUS_ERROR_NO_UNIT_FOR_PID,
            "PID " PID_FMT " does not belong to any loaded unit.", pid);

    return reply_unit_path(u, message, error);
}

static int method_get_unit_by_invocation_id(sd_bus_message* message,
                                            void* userdata, sd_bus_error* error)
{
    ...
    else
    {
        ...
        if (!u)
            return sd_bus_error_setf(
                error, BUS_ERROR_NO_UNIT_FOR_INVOCATION_ID,
    ...
    /* So here's a special trick: the bus path we return actually references the
     * unit by its invocation ID instead of the unit name. This means it stays
     * valid only as long as the invocation ID stays the same. */
...
static int method_get_unit_by_control_group(sd_bus_message* message,
                                            void* userdata, sd_bus_error* error)
{
    Manager* m = userdata;
    const char* cgroup;
...
static int method_load_unit(sd_bus_message* message, void* userdata,
                            sd_bus_error* error)
{
    Manager* m = userdata;
    const char* name;
...
static int method_start_unit_generic(sd_bus_message* message, Manager* m,
                                     JobType job_type, bool reload_if_possible,
                                     sd_bus_error* error)
{
    const char* name;
    Unit* u;
    ...
    return bus_unit_method_start_generic(message, u, job_type,
                                         reload_if_possible, error);
}

static int method_start_unit(sd_bus_message* message, void* userdata,
                             sd_bus_error* error)
{
    return method_start_unit_generic(message, userdata, JOB_START, false,
                                     error);
}

static int method_stop_unit(sd_bus_message* message, void* userdata,
                            sd_bus_error* error)
{
    return method_start_unit_generic(message, userdata, JOB_STOP, false, error);
}

static int method_reload_unit(sd_bus_message* message, void* userdata,
                              sd_bus_error* error)
{
    return method_start_unit_generic(message, userdata, JOB_RELOAD, false,
                                     error);
}

static int method_restart_unit(sd_bus_message* message, void* userdata,
                               sd_bus_error* error)
{
    return method_start_unit_generic(message, userdata, JOB_RESTART, false,
                                     error);
}

static int method_try_restart_unit(sd_bus_message* message, void* userdata,
                                   sd_bus_error* error)
{
    return method_start_unit_generic(message, userdata, JOB_TRY_RESTART, false,
                                     error);
}

static int method_reload_or_restart_unit(sd_bus_message* message,
                                         void* userdata, sd_bus_error* error)
{
    return method_start_unit_generic(message, userdata, JOB_RESTART, true,
                                     error);
}

static int method_reload_or_try_restart_unit(sd_bus_message* message,
                                             void* userdata,
                                             sd_bus_error* error)
{
    return method_start_unit_generic(message, userdata, JOB_TRY_RESTART, true,
                                     error);
}
...
static int method_generic_unit_operation(sd_bus_message* message, Manager* m,
                                         sd_bus_error* error,
                                         ...
                                         GenericUnitOperationFlags flags)
{
    ...
    /* Read the first argument from the command and pass the operation to the
     * specified per-unit method. */
...
static int method_enqueue_unit_job(sd_bus_message* message, void* userdata,
                                   sd_bus_error* error)
{
    /* We don't bother with GENERIC_UNIT_VALIDATE_LOADED here, as the job logic
     * validates that anyway */
    return method_generic_unit_operation(message, userdata, error,
                                         bus_unit_method_enqueue_job,
                                         GENERIC_UNIT_LOAD);
}

static int method_start_unit_replace(sd_bus_message* message, void* userdata,
                                     sd_bus_error* error)
{
    Manager* m = userdata;
    const char* old_name;
    ...
    if (!u->job || u->job->type != JOB_START)
        return sd_bus_error_setf(error, BUS_ERROR_NO_SUCH_JOB,
                                 "No job queued for unit %s", old_name);

    return method_start_unit_generic(message, m, JOB_START, false, error);
}

static int method_kill_unit(sd_bus_message* message, void* userdata,
                            sd_bus_error* error)
{
    /* We don't bother with GENERIC_UNIT_LOAD nor GENERIC_UNIT_VALIDATE_LOADED
     * here, as it shouldn't matter whether a unit is loaded for killing any
     * processes possibly in the unit's cgroup. */
    return method_generic_unit_operation(message, userdata, error,
                                         bus_unit_method_kill, 0);
}

static int method_reset_failed_unit(sd_bus_message* message, void* userdata,
                                    sd_bus_error* error)
{
    /* Don't load the unit (because unloaded units can't be in failed state),
     * and don't insist on the unit to be loaded properly (since a failed unit
     * might have its unit file disappeared) */
    return method_generic_unit_operation(message, userdata, error,
                                         bus_unit_method_reset_failed, 0);
}

static int method_set_unit_properties(sd_bus_message* message, void* userdata,
                                      sd_bus_error* error)
{
    /* Only change properties on fully loaded units, and load them in order to
     * set properties */
    return method_generic_unit_operation(
        message, userdata, error, bus_unit_method_set_properties,
        GENERIC_UNIT_LOAD | GENERIC_UNIT_VALIDATE_LOADED);
}

static int method_ref_unit(sd_bus_message* message, void* userdata,
                           sd_bus_error* error)
{
    /* Only allow reffing of fully loaded units, and make sure reffing a unit
     * loads it. */
    return method_generic_unit_operation(
...
static int method_unref_unit(sd_bus_message* message, void* userdata,
                             sd_bus_error* error)
{
    /* Dropping a ref OTOH should not require the unit to still be loaded. And
     * since a reffed unit is a
     * loaded unit there's no need to load the unit for unreffing it. */
    return method_generic_unit_operation(message, userdata, error,
                                         bus_unit_method_unref, 0);
}
...
static int reply_unit_info(sd_bus_message* reply, Unit* u)
{
    ...
    return sd_bus_message_append(
        reply, "(ssssssouso)", u->id, unit_description(u),
        unit_load_state_to_string(u->load_state),
        unit_active_state_to_string(unit_active_state(u)),
        unit_sub_state_to_string(u), following ? following->id : "", unit_path,
        u->job ? u->job->id : 0, u->job ? job_type_to_string(u->job->type) : "",
        job_path ? job_path : "/");
}

static int method_list_units_by_names(sd_bus_message* message, void* userdata,
                                      sd_bus_error* error)
{
...
static int method_get_unit_processes(sd_bus_message* message, void* userdata,
                                     sd_bus_error* error)
{
    /* Don't load a unit (since it won't have any processes if it's not loaded),
     * but don't insist on the
     * unit being loaded (because even improperly loaded units might still have
     * processes around */
    return method_generic_unit_operation(message, userdata, error,
                                         bus_unit_method_get_processes, 0);
}

static int method_attach_processes_to_unit(sd_bus_message* message,
                                           void* userdata, sd_bus_error* error)
{
    /* Don't allow attaching new processes to units that aren't loaded. Don't
     * bother with loading a unit for this purpose though, as an unloaded unit
     * is a stopped unit, and we don't allow attaching processes to stopped
     * units anyway. */
    return method_generic_unit_operation(message, userdata, error,
...
static int transient_unit_from_message(Manager* m, sd_bus_message* message,
                                       const char* name, Unit** unit,
                                       sd_bus_error* error)
{
...
static int transient_aux_units_from_message(Manager* m, sd_bus_message* message,
                                            sd_bus_error* error)
{
...
static int method_start_transient_unit(sd_bus_message* message, void* userdata,
                                       sd_bus_error* error)
{
    ...
    if (r < 0)
        return r;
    if (r == 0)
        return 1; /* No authorization for now, but the async polkit stuff will
                     call us again when it has it */
...
static int method_get_job(sd_bus_message* message, void* userdata,
                          sd_bus_error* error)
{
    _cleanup_free_ char* path = NULL;
    Manager* m = userdata;
...
static int method_cancel_job(sd_bus_message* message, void* userdata,
                             sd_bus_error* error)
{
    Manager* m = userdata;
    uint32_t id;
...
static int method_clear_jobs(sd_bus_message* message, void* userdata,
                             sd_bus_error* error)
{
    Manager* m = userdata;
    int r;
...
static int method_reset_failed(sd_bus_message* message, void* userdata,
                               sd_bus_error* error)
{
    Manager* m = userdata;
    int r;
...
static int list_units_filtered(sd_bus_message* message, void* userdata,
                               sd_bus_error* error, char** states,
                               char** patterns)
{
    _cleanup_(sd_bus_message_unrefp) sd_bus_message* reply = NULL;
    Manager* m = userdata;
...
static int method_list_units(sd_bus_message* message, void* userdata,
                             sd_bus_error* error)
{
    return list_units_filtered(message, userdata, error, NULL, NULL);
}

static int method_list_units_filtered(sd_bus_message* message, void* userdata,
                                      sd_bus_error* error)
{
...
static int method_list_units_by_patterns(sd_bus_message* message,
                                         void* userdata, sd_bus_error* error)
{
    _cleanup_strv_free_ char** states = NULL;
...
static int method_list_jobs(sd_bus_message* message, void* userdata,
                            sd_bus_error* error)
{
    _cleanup_(sd_bus_message_unrefp) sd_bus_message* reply = NULL;
    Manager* m = userdata;
    ...
    HASHMAP_FOREACH(j, m->jobs, i)
    {
        ...
        r = sd_bus_message_append(
            reply, "(usssoo)", j->id, j->unit->id, job_type_to_string(j->type),
            job_state_to_string(j->state), job_path, unit_path);
        if (r < 0)
            return r;
    }
...
static int method_subscribe(sd_bus_message* message, void* userdata,
                            sd_bus_error* error)
{
    Manager* m = userdata;
    int r;
    ...
    if (sd_bus_message_get_bus(message) == m->api_bus)
    {
        ...
        if (!m->subscribed)
        {
            r = sd_bus_track_new(sd_bus_message_get_bus(message),
                                 &m->subscribed, NULL, NULL);
            if (r < 0)
                return r;
        }
        ...
        if (r < 0)
            return r;
        if (r == 0)
            return sd_bus_error_setf(error, BUS_ERROR_ALREADY_SUBSCRIBED,
...
static int method_unsubscribe(sd_bus_message* message, void* userdata,
                              sd_bus_error* error)
{
    Manager* m = userdata;
    int r;
    ...
    if (sd_bus_message_get_bus(message) == m->api_bus)
    {
        ...
        if (r < 0)
            return r;
        if (r == 0)
            return sd_bus_error_setf(error, BUS_ERROR_NOT_SUBSCRIBED,
...
static int dump_impl(sd_bus_message* message, void* userdata,
                     sd_bus_error* error, int (*reply)(sd_bus_message*, char*))
{
...
static int method_dump(sd_bus_message* message, void* userdata,
                       sd_bus_error* error)
{
...
static int method_dump_by_fd(sd_bus_message* message, void* userdata,
                             sd_bus_error* error)
{
    return dump_impl(message, userdata, error, reply_dump_by_fd);
}

static int method_refuse_snapshot(sd_bus_message* message, void* userdata,
                                  sd_bus_error* error)
{
...
static int method_reload(sd_bus_message* message, void* userdata,
                         sd_bus_error* error)
{
    Manager* m = userdata;
    int r;
...
static int method_reexecute(sd_bus_message* message, void* userdata,
                            sd_bus_error* error)
{
    Manager* m = userdata;
    int r;
...
static int method_exit(sd_bus_message* message, void* userdata,
                       sd_bus_error* error)
{
    Manager* m = userdata;
    int r;
...
static int method_reboot(sd_bus_message* message, void* userdata,
                         sd_bus_error* error)
{
    Manager* m = userdata;
    int r;
...
static int method_poweroff(sd_bus_message* message, void* userdata,
                           sd_bus_error* error)
{
    Manager* m = userdata;
    int r;
...
static int method_halt(sd_bus_message* message, void* userdata,
                       sd_bus_error* error)
{
    Manager* m = userdata;
    int r;
...
static int method_kexec(sd_bus_message* message, void* userdata,
                        sd_bus_error* error)
{
    Manager* m = userdata;
    int r;
...
static int method_switch_root(sd_bus_message* message, void* userdata,
                              sd_bus_error* error)
{
    ...
    if (available < RELOAD_DISK_SPACE_MIN)
    {
        char fb_available[FORMAT_BYTES_MAX], fb_need[FORMAT_BYTES_MAX];
        log_warning(
            "Dangerously low amount of free space on /run/systemd, root "
            "switching operation might not complete successfully. "
    ...
    if (isempty(init))
    {
        ...
        if (r < 0)
            return sd_bus_error_set_errnof(error, r,
                                           "Failed to determine whether root "
                                           "path '%s' contains an OS tree: %m",
                                           root);
        if (r == 0)
            return sd_bus_error_setf(
                error, SD_BUS_ERROR_INVALID_ARGS,
                "Specified switch root path '%s' does not seem to be an OS "
                "tree. os-release file is missing.",
                root);
    }
    else
    {
        ...
        r = chase_symlinks(init, root, CHASE_PREFIX_ROOT | CHASE_TRAIL_SLASH,
                           &chased);
        if (r < 0)
        ...
        if (laccess(chased, X_OK) < 0)
        {
            ...
            return sd_bus_error_set_errnof(
                error, r,
                "Could not check whether init binary %s is executable: %m",
                init);
        }
    }
...
static int method_set_environment(sd_bus_message* message, void* userdata,
                                  sd_bus_error* error)
{
...
static int method_unset_environment(sd_bus_message* message, void* userdata,
                                    sd_bus_error* error)
{
...
static int method_unset_and_set_environment(sd_bus_message* message,
                                            void* userdata, sd_bus_error* error)
{
...
static int method_set_exit_code(sd_bus_message* message, void* userdata,
                                sd_bus_error* error)
{
    Manager* m = userdata;
    uint8_t code;
    ...
    if (MANAGER_IS_SYSTEM(m) && detect_container() <= 0)
        return sd_bus_error_setf(error, SD_BUS_ERROR_NOT_SUPPORTED,
                                 "ExitCode can only be set for user service "
                                 "managers or in containers.");
...
static int method_lookup_dynamic_user_by_name(sd_bus_message* message,
                                              void* userdata,
                                              sd_bus_error* error)
{
    Manager* m = userdata;
...
static int method_lookup_dynamic_user_by_uid(sd_bus_message* message,
                                             void* userdata,
                                             sd_bus_error* error)
{
...
static int method_get_dynamic_users(sd_bus_message* message, void* userdata,
                                    sd_bus_error* error)
{
    _cleanup_(sd_bus_message_unrefp) sd_bus_message* reply = NULL;
    Manager* m = userdata;
...
static int list_unit_files_by_patterns(sd_bus_message* message, void* userdata,
                                       sd_bus_error* error, char** states,
                                       char** patterns)
{
    _cleanup_(sd_bus_message_unrefp) sd_bus_message* reply = NULL;
...
static int method_list_unit_files(sd_bus_message* message, void* userdata,
                                  sd_bus_error* error)
{
    return list_unit_files_by_patterns(message, userdata, error, NULL, NULL);
}

static int method_list_unit_files_by_patterns(sd_bus_message* message,
                                              void* userdata,
                                              sd_bus_error* error)
{
    _cleanup_strv_free_ char** states = NULL;
    ...
    return list_unit_files_by_patterns(message, userdata, error, states,
                                       patterns);
}

static int method_get_unit_file_state(sd_bus_message* message, void* userdata,
                                      sd_bus_error* error)
{
    Manager* m = userdata;
    const char* name;
...
static int method_get_default_target(sd_bus_message* message, void* userdata,
                                     sd_bus_error* error)
{
...
static int send_unit_files_changed(sd_bus* bus, void* userdata)
{
    ...
    r = sd_bus_message_new_signal(bus, &message, "/org/freedesktop/systemd1",
 ...
 */
static int install_error(sd_bus_error* error, int c, UnitFileChange* changes,
                         size_t n_changes)
{
    ...
    for (i = 0; i < n_changes; i++)
        ...
        switch (changes[i].type)
        {
            ...
            default:
                r = sd_bus_error_set_errnof(error, changes[i].type,
                                            "File %s: %m", changes[i].path);
                goto found;
        }
...
static int reply_unit_file_changes_and_free(Manager* m, sd_bus_message* message,
                                            ...
                                            sd_bus_error* error)
{
    ...
    for (i = 0; i < n_changes; i++)
    {
        ...
        r = sd_bus_message_append(
            reply, "(sss)", unit_file_change_type_to_string(changes[i].type),
            changes[i].path, changes[i].source);
        if (r < 0)
            goto fail;
    ...
    /* If there was a failed change, and no successful change, then return the
     * first failure as proper method call error. */
    if (bad && !good)
...
static int method_enable_unit_files_generic(
    ...
    int (*call)(UnitFileScope scope, UnitFileFlags flags, const char* root_dir,
                char* files[], UnitFileChange** changes, size_t* n_changes),
    bool carries_install_info, sd_bus_error* error)
{
...
static int method_enable_unit_files(sd_bus_message* message, void* userdata,
                                    sd_bus_error* error)
{
    return method_enable_unit_files_generic(message, userdata, unit_file_enable,
                                            true, error);
}

static int method_reenable_unit_files(sd_bus_message* message, void* userdata,
                                      sd_bus_error* error)
{
    return method_enable_unit_files_generic(message, userdata,
                                            unit_file_reenable, true, error);
}

static int method_link_unit_files(sd_bus_message* message, void* userdata,
                                  sd_bus_error* error)
{
    return method_enable_unit_files_generic(message, userdata, unit_file_link,
                                            false, error);
}

static int unit_file_preset_without_mode(UnitFileScope scope,
                                         UnitFileFlags flags,
                                         const char* root_dir, char** files,
                                         UnitFileChange** changes,
                                         size_t* n_changes)
{
    return unit_file_preset(scope, flags, root_dir, files,
                            UNIT_FILE_PRESET_FULL, changes, n_changes);
}

static int method_preset_unit_files(sd_bus_message* message, void* userdata,
                                    sd_bus_error* error)
{
    return method_enable_unit_files_generic(
        message, userdata, unit_file_preset_without_mode, true, error);
}

static int method_mask_unit_files(sd_bus_message* message, void* userdata,
                                  sd_bus_error* error)
{
    return method_enable_unit_files_generic(message, userdata, unit_file_mask,
                                            false, error);
}

static int method_preset_unit_files_with_mode(sd_bus_message* message,
                                              void* userdata,
                                              sd_bus_error* error)
{
    ...
    r = unit_file_preset(m->unit_file_scope, flags, NULL, l, mm, &changes,
                         &n_changes);
    if (r < 0)
        return install_error(error, r, changes, n_changes);
    ...
    sd_bus_error* error)
{
    ...
    r = call(m->unit_file_scope, runtime ? UNIT_FILE_RUNTIME : 0, NULL, l,
             &changes, &n_changes);
    if (r < 0)
        return install_error(error, r, changes, n_changes);
...
static int method_disable_unit_files(sd_bus_message* message, void* userdata,
                                     sd_bus_error* error)
{
    return method_disable_unit_files_generic(message, userdata,
                                             unit_file_disable, error);
}

static int method_unmask_unit_files(sd_bus_message* message, void* userdata,
                                    sd_bus_error* error)
{
    return method_disable_unit_files_generic(message, userdata,
                                             unit_file_unmask, error);
}

static int method_revert_unit_files(sd_bus_message* message, void* userdata,
                                    sd_bus_error* error)
{
    _cleanup_strv_free_ char** l = NULL;
    UnitFileChange* changes = NULL;
...
static int method_set_default_target(sd_bus_message* message, void* userdata,
                                     sd_bus_error* error)
{
    UnitFileChange* changes = NULL;
    size_t n_changes = 0;
    ...
    r = unit_file_set_default(m->unit_file_scope, force ? UNIT_FILE_FORCE : 0,
                              NULL, name, &changes, &n_changes);
    if (r < 0)
        return install_error(error, r, changes, n_changes);
...
static int method_preset_all_unit_files(sd_bus_message* message, void* userdata,
                                        sd_bus_error* error)
{
    UnitFileChange* changes = NULL;
    size_t n_changes = 0;
    ...
    r = unit_file_preset_all(m->unit_file_scope, flags, NULL, mm, &changes,
                             &n_changes);
    if (r < 0)
        return install_error(error, r, changes, n_changes);
...
static int method_add_dependency_unit_files(sd_bus_message* message,
                                            void* userdata, sd_bus_error* error)
{
    _cleanup_strv_free_ char** l = NULL;
    Manager* m = userdata;
    ...
    r = unit_file_add_dependency(m->unit_file_scope, flags, NULL, l, target,
                                 dep, &changes, &n_changes);
    if (r < 0)
        return install_error(error, r, changes, n_changes);
...
static int method_get_unit_file_links(sd_bus_message* message, void* userdata,
                                      sd_bus_error* error)
{
    _cleanup_(sd_bus_message_unrefp) sd_bus_message* reply = NULL;
    UnitFileChange* changes = NULL;
...
static int method_get_job_waiting(sd_bus_message* message, void* userdata,
                                  sd_bus_error* error)
{
    Manager* m = userdata;
    uint32_t id;
...
static int method_abandon_scope(sd_bus_message* message, void* userdata,
                                sd_bus_error* error)
{
    Manager* m = userdata;
    const char* name;
    ...
    if (u->type != UNIT_SCOPE)
        return sd_bus_error_setf(error, SD_BUS_ERROR_INVALID_ARGS,
                                 "Unit '%s' is not a scope unit, refusing.",
                                 name);
...
const sd_bus_vtable bus_manager_vtable[] = {
    ...
    BUS_PROPERTY_DUAL_TIMESTAMP(
        ...
        offsetof(Manager,
                 timestamps[MANAGER_TIMESTAMP_INITRD_GENERATORS_START]),
        SD_BUS_VTABLE_PROPERTY_CONST),
    BUS_PROPERTY_DUAL_TIMESTAMP(
        "InitRDGeneratorsFinishTimestamp",
        offsetof(Manager,
                 timestamps[MANAGER_TIMESTAMP_INITRD_GENERATORS_FINISH]),
        SD_BUS_VTABLE_PROPERTY_CONST),
    BUS_PROPERTY_DUAL_TIMESTAMP(
        "InitRDUnitsLoadStartTimestamp",
        offsetof(Manager,
                 timestamps[MANAGER_TIMESTAMP_INITRD_UNITS_LOAD_START]),
        SD_BUS_VTABLE_PROPERTY_CONST),
    BUS_PROPERTY_DUAL_TIMESTAMP(
        "InitRDUnitsLoadFinishTimestamp",
        offsetof(Manager,
                 timestamps[MANAGER_TIMESTAMP_INITRD_UNITS_LOAD_FINISH]),
        SD_BUS_VTABLE_PROPERTY_CONST),
    ...
    SD_BUS_PROPERTY("NNames", "u", property_get_hashmap_size,
                    offsetof(Manager, units), 0),
    SD_BUS_PROPERTY("NFailedUnits", "u", property_get_set_size,
                    offsetof(Manager, failed_units),
                    SD_BUS_VTABLE_PROPERTY_EMITS_CHANGE),
    SD_BUS_PROPERTY("NJobs", "u", property_get_hashmap_size,
                    offsetof(Manager, jobs), 0),
    ...
    SD_BUS_PROPERTY("UnitPath", "as", NULL,
                    offsetof(Manager, lookup_paths.search_path),
                    SD_BUS_VTABLE_PROPERTY_CONST),
    SD_BUS_PROPERTY("DefaultStandardOutput", "s", bus_property_get_exec_output,
                    offsetof(Manager, default_std_output),
                    SD_BUS_VTABLE_PROPERTY_CONST),
    SD_BUS_PROPERTY("DefaultStandardError", "s", bus_property_get_exec_output,
                    offsetof(Manager, default_std_output),
                    SD_BUS_VTABLE_PROPERTY_CONST),
    SD_BUS_WRITABLE_PROPERTY("RuntimeWatchdogUSec", "t", bus_property_get_usec,
                             property_set_runtime_watchdog,
                             offsetof(Manager, runtime_watchdog), 0),
    SD_BUS_WRITABLE_PROPERTY("ShutdownWatchdogUSec", "t", bus_property_get_usec,
                             bus_property_set_usec,
                             offsetof(Manager, shutdown_watchdog), 0),
    SD_BUS_WRITABLE_PROPERTY("ServiceWatchdogs", "b", bus_property_get_bool,
                             bus_property_set_bool,
                             offsetof(Manager, service_watchdogs), 0),
    SD_BUS_PROPERTY("ControlGroup", "s", NULL, offsetof(Manager, cgroup_root),
                    0),
    SD_BUS_PROPERTY("SystemState", "s", property_get_system_state, 0, 0),
    SD_BUS_PROPERTY("ExitCode", "y", bus_property_get_unsigned,
                    offsetof(Manager, return_value), 0),
    ...
    SD_BUS_PROPERTY("DefaultRestartUSec", "t", bus_property_get_usec,
                    ...
                    SD_BUS_VTABLE_PROPERTY_CONST),
    SD_BUS_PROPERTY("DefaultStartLimitIntervalUSec", "t", bus_property_get_usec,
                    offsetof(Manager, default_start_limit_interval),
                    SD_BUS_VTABLE_PROPERTY_CONST),
    /* The following two items are obsolete alias */
    SD_BUS_PROPERTY("DefaultStartLimitIntervalSec", "t", bus_property_get_usec,
                    offsetof(Manager, default_start_limit_interval),
                    SD_BUS_VTABLE_PROPERTY_CONST | SD_BUS_VTABLE_HIDDEN),
    SD_BUS_PROPERTY("DefaultStartLimitInterval", "t", bus_property_get_usec,
                    offsetof(Manager, default_start_limit_interval),
                    SD_BUS_VTABLE_PROPERTY_CONST | SD_BUS_VTABLE_HIDDEN),
    SD_BUS_PROPERTY("DefaultStartLimitBurst", "u", bus_property_get_unsigned,
    ...
    SD_BUS_PROPERTY("DefaultLimitSIGPENDING", "t", bus_property_get_rlimit,
                    ...
                    SD_BUS_VTABLE_PROPERTY_CONST),
    SD_BUS_PROPERTY("DefaultLimitSIGPENDINGSoft", "t", bus_property_get_rlimit,
                    offsetof(Manager, rlimit[RLIMIT_SIGPENDING]),
                    SD_BUS_VTABLE_PROPERTY_CONST),
    ...
    SD_BUS_METHOD("GetUnitByPID", "u", "o", method_get_unit_by_pid,
                  SD_BUS_VTABLE_UNPRIVILEGED),
    SD_BUS_METHOD("GetUnitByInvocationID", "ay", "o",
                  method_get_unit_by_invocation_id, SD_BUS_VTABLE_UNPRIVILEGED),
    SD_BUS_METHOD("GetUnitByControlGroup", "s", "o",
                  method_get_unit_by_control_group, SD_BUS_VTABLE_UNPRIVILEGED),
    ...
    SD_BUS_METHOD("ReloadOrTryRestartUnit", "ss", "o",
                  ...
                  SD_BUS_VTABLE_UNPRIVILEGED),
    SD_BUS_METHOD("EnqueueUnitJob", "sss", "uososa(uosos)",
                  method_enqueue_unit_job, SD_BUS_VTABLE_UNPRIVILEGED),
    ...
    SD_BUS_METHOD("RefUnit", "s", NULL, method_ref_unit,
                  SD_BUS_VTABLE_UNPRIVILEGED),
    SD_BUS_METHOD("UnrefUnit", "s", NULL, method_unref_unit,
                  SD_BUS_VTABLE_UNPRIVILEGED),
    SD_BUS_METHOD("StartTransientUnit", "ssa(sv)a(sa(sv))", "o",
                  method_start_transient_unit, SD_BUS_VTABLE_UNPRIVILEGED),
    SD_BUS_METHOD("GetUnitProcesses", "s", "a(sus)", method_get_unit_processes,
                  SD_BUS_VTABLE_UNPRIVILEGED),
    SD_BUS_METHOD("AttachProcessesToUnit", "ssau", NULL,
                  method_attach_processes_to_unit, SD_BUS_VTABLE_UNPRIVILEGED),
    SD_BUS_METHOD("AbandonScope", "s", NULL, method_abandon_scope,
                  SD_BUS_VTABLE_UNPRIVILEGED),
    SD_BUS_METHOD("GetJob", "u", "o", method_get_job,
                  SD_BUS_VTABLE_UNPRIVILEGED),
    SD_BUS_METHOD("GetJobAfter", "u", "a(usssoo)", method_get_job_waiting,
                  SD_BUS_VTABLE_UNPRIVILEGED),
    SD_BUS_METHOD("GetJobBefore", "u", "a(usssoo)", method_get_job_waiting,
                  SD_BUS_VTABLE_UNPRIVILEGED),
    SD_BUS_METHOD("CancelJob", "u", NULL, method_cancel_job,
                  SD_BUS_VTABLE_UNPRIVILEGED),
    SD_BUS_METHOD("ClearJobs", NULL, NULL, method_clear_jobs,
                  SD_BUS_VTABLE_UNPRIVILEGED),
    SD_BUS_METHOD("ResetFailed", NULL, NULL, method_reset_failed,
                  SD_BUS_VTABLE_UNPRIVILEGED),
    ...
    SD_BUS_METHOD("ListUnitsByPatterns", "asas", "a(ssssssouso)",
                  method_list_units_by_patterns, SD_BUS_VTABLE_UNPRIVILEGED),
    SD_BUS_METHOD("ListUnitsByNames", "as", "a(ssssssouso)",
                  method_list_units_by_names, SD_BUS_VTABLE_UNPRIVILEGED),
    ...
    SD_BUS_METHOD("DumpByFileDescriptor", NULL, "h", method_dump_by_fd,
                  SD_BUS_VTABLE_UNPRIVILEGED),
    SD_BUS_METHOD("CreateSnapshot", "sb", "o", method_refuse_snapshot,
                  SD_BUS_VTABLE_UNPRIVILEGED | SD_BUS_VTABLE_HIDDEN),
    SD_BUS_METHOD("RemoveSnapshot", "s", NULL, method_refuse_snapshot,
                  SD_BUS_VTABLE_UNPRIVILEGED | SD_BUS_VTABLE_HIDDEN),
    ...
    SD_BUS_METHOD("MaskUnitFiles", "asbb", "a(sss)", method_mask_unit_files,
                  SD_BUS_VTABLE_UNPRIVILEGED),
    SD_BUS_METHOD("UnmaskUnitFiles", "asb", "a(sss)", method_unmask_unit_files,
                  SD_BUS_VTABLE_UNPRIVILEGED),
    SD_BUS_METHOD("RevertUnitFiles", "as", "a(sss)", method_revert_unit_files,
                  SD_BUS_VTABLE_UNPRIVILEGED),
    SD_BUS_METHOD("SetDefaultTarget", "sb", "a(sss)", method_set_default_target,
                  SD_BUS_VTABLE_UNPRIVILEGED),
    ...
    SD_BUS_METHOD("GetUnitFileLinks", "sb", "as", method_get_unit_file_links,
                  SD_BUS_VTABLE_UNPRIVILEGED),
    SD_BUS_METHOD("SetExitCode", "y", NULL, method_set_exit_code,
                  SD_BUS_VTABLE_UNPRIVILEGED),
    SD_BUS_METHOD("LookupDynamicUserByName", "s", "u",
                  method_lookup_dynamic_user_by_name,
                  SD_BUS_VTABLE_UNPRIVILEGED),
    SD_BUS_METHOD("LookupDynamicUserByUID", "u", "s",
                  method_lookup_dynamic_user_by_uid,
                  SD_BUS_VTABLE_UNPRIVILEGED),
    SD_BUS_METHOD("GetDynamicUsers", NULL, "a(us)", method_get_dynamic_users,
                  SD_BUS_VTABLE_UNPRIVILEGED),

    SD_BUS_SIGNAL("UnitNew", "so", 0), SD_BUS_SIGNAL("UnitRemoved", "so", 0),
    SD_BUS_SIGNAL("JobNew", "uos", 0), SD_BUS_SIGNAL("JobRemoved", "uoss", 0),
...
static int send_finished(sd_bus* bus, void* userdata)
{
    ...
    r = sd_bus_message_append(message, "tttttt", times[0], times[1], times[2],
...
void bus_manager_send_finished(Manager* m, usec_t firmware_usec,
                               usec_t loader_usec, usec_t kernel_usec,
                               usec_t initrd_usec, usec_t userspace_usec,
                               usec_t total_usec)
{
...
static int send_changed_signal(sd_bus* bus, void* userdata)
{
    ...
    return sd_bus_emit_properties_changed_strv(
        bus, "/org/freedesktop/systemd1", "org.freedesktop.systemd1.Manager",
        NULL);
}
