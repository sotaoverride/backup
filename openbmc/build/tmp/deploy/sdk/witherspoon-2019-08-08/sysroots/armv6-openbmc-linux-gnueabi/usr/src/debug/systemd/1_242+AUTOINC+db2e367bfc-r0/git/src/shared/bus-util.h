...
enum
{
    BUS_MAP_STRDUP = 1 << 0, /* If set, each "s" message is duplicated. Thus,
                                each pointer needs to be freed. */
    BUS_MAP_BOOLEAN_AS_BOOL =
        1 << 1, /* If set, each "b" message is written to a bool pointer. If not
                   set, "b" is written to a int pointer. */
};
...
typedef int (*bus_message_print_t)(const char* name, const char* expected_value,
