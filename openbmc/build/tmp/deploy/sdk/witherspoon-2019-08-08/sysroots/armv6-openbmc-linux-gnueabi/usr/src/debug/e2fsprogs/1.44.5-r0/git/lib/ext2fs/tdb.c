...
#define TDB_HASH_TOP(hash)                                                     \
    (FREELIST_TOP + (BUCKET(hash) + 1) * sizeof(tdb_off_t))
#define TDB_HASHTABLE_SIZE(tdb)                                                \
    ((tdb->header.hash_size + 1) * sizeof(tdb_off_t))
...
static int tdb_brlock(struct tdb_context* tdb, tdb_off_t offset, int rw_type,
                      int lck_type, int probe, size_t len);
static int tdb_transaction_lock(struct tdb_context* tdb, int ltype);
static int tdb_transaction_unlock(struct tdb_context* tdb);
static int tdb_brlock_upgrade(struct tdb_context* tdb, tdb_off_t offset,
                              size_t len);
...
static int tdb_ofs_read(struct tdb_context* tdb, tdb_off_t offset,
                        tdb_off_t* d);
static int tdb_ofs_write(struct tdb_context* tdb, tdb_off_t offset,
                         tdb_off_t* d);
static void* tdb_convert(void* buf, u32 size);
static int tdb_free(struct tdb_context* tdb, tdb_off_t offset,
                    struct list_struct* rec);
static tdb_off_t tdb_allocate(struct tdb_context* tdb, tdb_len_t length,
                              struct list_struct* rec);
static int tdb_ofs_read(struct tdb_context* tdb, tdb_off_t offset,
                        tdb_off_t* d);
static int tdb_ofs_write(struct tdb_context* tdb, tdb_off_t offset,
...
static int tdb_rec_read(struct tdb_context* tdb, tdb_off_t offset,
                        struct list_struct* rec);
static int tdb_rec_write(struct tdb_context* tdb, tdb_off_t offset,
                         struct list_struct* rec);
static int tdb_do_delete(struct tdb_context* tdb, tdb_off_t rec_ptr,
                         struct list_struct* rec);
static unsigned char* tdb_alloc_read(struct tdb_context* tdb, tdb_off_t offset,
                                     tdb_len_t len);
static int tdb_parse_data(struct tdb_context* tdb, TDB_DATA key,
                          ...
                          int (*parser)(TDB_DATA key, TDB_DATA data,
                                        void* private_data),
                          void* private_data);
static tdb_off_t tdb_find_lock_hash(struct tdb_context* tdb, TDB_DATA key,
                                    u32 hash, int locktype,
                                    struct list_struct* rec);
...
*/
int tdb_brlock(struct tdb_context* tdb, tdb_off_t offset, int rw_type,
               int lck_type, int probe, size_t len)
{
    struct flock fl;
    int ret;
    ...
    if (ret == -1)
    {
        ...
        if (!probe && lck_type != F_SETLK)
        {
            ...
            TDB_LOG((tdb, TDB_DEBUG_TRACE,
                     "tdb_brlock failed (fd=%d) at offset %d rw_type=%d "
                     "lck_type=%d len=%d\n",
...
static int _tdb_lock(struct tdb_context* tdb, int list, int ltype, int op)
{
    ...
    new_lck = (struct tdb_lock_type*)realloc(
        tdb->lockrecs, sizeof(*tdb->lockrecs) * (tdb->num_lockrecs + 1));
    if (new_lck == NULL)
    {
        errno = ENOMEM;
        return -1;
    ...
    if (!mark_lock &&
        tdb->methods->tdb_brlock(tdb, FREELIST_TOP + 4 * list, ltype, op, 0, 1))
    {
        return -1;
    }
...
int tdb_transaction_lock(struct tdb_context* tdb, int ltype)
{
    ...
    if (tdb->methods->tdb_brlock(tdb, TRANSACTION_LOCK, ltype, F_SETLKW, 0,
                                 1) == -1)
    {
        TDB_LOG((tdb, TDB_DEBUG_ERROR,
...
int tdb_transaction_unlock(struct tdb_context* tdb)
{
    ...
    ret = tdb->methods->tdb_brlock(tdb, TRANSACTION_LOCK, F_UNLCK, F_SETLKW, 0,
                                   1);
    if (ret == 0)
    {
...
static int _tdb_lockall(struct tdb_context* tdb, int ltype, int op)
{
    ...
    if (!mark_lock && tdb->methods->tdb_brlock(tdb, FREELIST_TOP, ltype, op, 0,
...
static int _tdb_unlockall(struct tdb_context* tdb, int ltype)
{
    ...
    if (!mark_lock &&
        tdb->methods->tdb_brlock(tdb, FREELIST_TOP, F_UNLCK, F_SETLKW, 0,
                                 4 * tdb->header.hash_size))
    {
...
/* unmark a chain as locked without actually locking it. Warning! use with great
 * caution! */
...
{
    return off ? tdb->methods->tdb_brlock(tdb, off, F_RDLCK, F_SETLKW, 0, 1)
               : 0;
}
...
int tdb_unlock_record(struct tdb_context* tdb, tdb_off_t off)
{
    ...
    for (i = &tdb->travlocks; i; i = i->next)
        if (i->off == off)
            count++;
    return (count == 1
                ? tdb->methods->tdb_brlock(tdb, off, F_UNLCK, F_SETLKW, 0, 1)
                : 0);
}
...
/*
  ...
  The value will then be confirmed with the lock held
*/
...
static int tdb_expand_file(struct tdb_context* tdb, tdb_off_t size,
                           tdb_off_t addition)
{
    char buf[1024];
...
unsigned char* tdb_alloc_read(struct tdb_context* tdb, tdb_off_t offset,
                              tdb_len_t len)
{
    unsigned char* buf;
...
int tdb_parse_data(struct tdb_context* tdb, TDB_DATA key, tdb_off_t offset,
                   tdb_len_t len,
                   int (*parser)(TDB_DATA key, TDB_DATA data,
                                 void* private_data),
                   void* private_data)
...
/* read/write a record */
int tdb_rec_read(struct tdb_context* tdb, tdb_off_t offset,
                 struct list_struct* rec)
{
...
int tdb_rec_write(struct tdb_context* tdb, tdb_off_t offset,
                  struct list_struct* rec)
{
...
static const struct tdb_methods io_methods = {
    tdb_read, tdb_write,       tdb_next_hash_chain,
    tdb_oob,  tdb_expand_file, tdb_brlock};
                             ...
                             const void* buf, tdb_len_t len)
{
         ...
         off > tdb->transaction->old_map_size))
    {
        ...
        el = best_el;
        el->data = (unsigned char*)realloc(el->data, el->length + len);
        if (el->data == NULL)
        {
            tdb->ecode = TDB_ERR_OOM;
            tdb->transaction->transaction_error = 1;
...
static const struct tdb_methods transaction_methods = {
    transaction_read, transaction_write,       transaction_next_hash_chain,
    transaction_oob,  transaction_expand_file, transaction_brlock};
...
int tdb_transaction_start(struct tdb_context* tdb)
{
    ...
    {
        TDB_LOG((tdb, TDB_DEBUG_ERROR,
                 "tdb_transaction_start: cannot start a transaction on a "
    ...
    if (tdb->num_locks != 0 || tdb->global_lock.count)
    {
        ...
        TDB_LOG((tdb, TDB_DEBUG_ERROR,
                 "tdb_transaction_start: cannot start a transaction with locks "
                 "held\n"));
        tdb->ecode = TDB_ERR_LOCK;
        return -1;
    }
    ...
    if (tdb->travlocks.next != NULL)
    {
        /* you cannot use transactions inside a traverse (although you can use
           ...
           deadlock */
        TDB_LOG((tdb, TDB_DEBUG_ERROR,
                 "tdb_transaction_start: cannot start a transaction within a "
                 "traverse\n"));
        tdb->ecode = TDB_ERR_LOCK;
        return -1;
    }
    ...
    tdb->transaction =
        (struct tdb_transaction*)calloc(sizeof(struct tdb_transaction), 1);
    if (tdb->transaction == NULL)
    {
        tdb->ecode = TDB_ERR_OOM;
        return -1;
    ...
    tdb->transaction->hash_heads =
        (u32*)calloc(tdb->header.hash_size + 1, sizeof(u32));
...
int tdb_transaction_cancel(struct tdb_context* tdb)
{
    ...
    {
        TDB_LOG(
            (tdb, TDB_DEBUG_ERROR, "tdb_transaction_cancel: no transaction\n"));
        return -1;
    }
    ...
    if (tdb->num_locks != 0)
    {
        ...
        for (i = 0; i < tdb->num_lockrecs; i++)
        {
            tdb_brlock(tdb, FREELIST_TOP + 4 * tdb->lockrecs[i].list, F_UNLCK,
                       F_SETLKW, 0, 1);
        }
        tdb->num_locks = 0;
        tdb->num_lockrecs = 0;
...
/*
  sync to disk
*/
static int transaction_sync(struct tdb_context* tdb, tdb_off_t offset,
                            tdb_len_t length)
{
    if (fsync(tdb->fd) != 0)
    {
    ...
    if (tdb->map_ptr)
    {
        ...
        if (msync(moffset + (char*)tdb->map_ptr, length + (offset - moffset),
                  MS_SYNC) != 0)
        {
            tdb->ecode = TDB_ERR_IO;
            TDB_LOG((tdb, TDB_DEBUG_FATAL,
                                 ...
                                 tdb_len_t* recovery_max_size)
{
    ...
    if (recovery_head != 0 && methods->tdb_read(tdb, recovery_head, &rec,
                                                sizeof(rec), DOCONV()) == -1)
    {
        TDB_LOG((tdb, TDB_DEBUG_FATAL,
    ...
    *recovery_size = tdb_recovery_size(tdb);
    ...
    if (recovery_head != 0)
    {
        ...
        {
            TDB_LOG((tdb, TDB_DEBUG_FATAL,
                     "tdb_recovery_allocate: failed to free previous recovery "
                     "area\n"));
            return -1;
        }
    }
    ...
    *recovery_max_size =
        TDB_ALIGN(sizeof(rec) + *recovery_size, tdb->page_size) - sizeof(rec);
    *recovery_offset = tdb->map_size;
    recovery_head = *recovery_offset;

    if (methods->tdb_expand_file(
            tdb, tdb->transaction->old_map_size,
            (tdb->map_size - tdb->transaction->old_map_size) + sizeof(rec) +
                *recovery_max_size) == -1)
    {
        TDB_LOG((tdb, TDB_DEBUG_FATAL,
    ...
    /* write the recovery header offset and sync - we can sync without a race
       here as the magic ptr in the recovery record has not been set */
    CONVERT(recovery_head);
    if (methods->tdb_write(tdb, TDB_RECOVERY_HEAD, &recovery_head,
                           sizeof(tdb_off_t)) == -1)
    {
        TDB_LOG((tdb, TDB_DEBUG_FATAL,
                                      ...
                                      tdb_off_t* magic_offset)
{
    ...
    */
    if (tdb_recovery_allocate(tdb, &recovery_size, &recovery_offset,
                              &recovery_max_size) == -1)
    {
        return -1;
    }
    ...
    for (el = tdb->transaction->elements; el; el = el->next)
    {
        ...
        {
            TDB_LOG((tdb, TDB_DEBUG_FATAL,
                     "tdb_transaction_setup_recovery: transaction data over "
                     "new region boundary\n"));
            free(data);
            tdb->ecode = TDB_ERR_CORRUPT;
            return -1;
    ...
    if (methods->tdb_write(tdb, recovery_offset, data,
    ...
    /* as we don't have ordered writes, we have to sync the recovery
       ...
       data is present */
    if (transaction_sync(tdb, recovery_offset, sizeof(*rec) + recovery_size) ==
        -1)
    {
        free(data);
        return -1;
    ...
    *magic_offset = recovery_offset + offsetof(struct list_struct, magic);
    ...
    {
        TDB_LOG((tdb, TDB_DEBUG_FATAL,
                 "tdb_transaction_setup_recovery: failed to write recovery "
                 "magic\n"));
        tdb->ecode = TDB_ERR_IO;
        return -1;
    }
...
int tdb_transaction_commit(struct tdb_context* tdb)
{
    ...
    {
        TDB_LOG(
            (tdb, TDB_DEBUG_ERROR, "tdb_transaction_commit: no transaction\n"));
        return -1;
    }
...
int tdb_transaction_recover(struct tdb_context* tdb)
{
    ...
    if (tdb->methods->tdb_read(tdb, recovery_head, &rec, sizeof(rec),
                               DOCONV()) == -1)
    {
        TDB_LOG((tdb, TDB_DEBUG_FATAL,
    ...
    {
        TDB_LOG((tdb, TDB_DEBUG_FATAL,
                 "tdb_transaction_recover: attempt to recover read only "
                 "database\n"));
        tdb->ecode = TDB_ERR_CORRUPT;
        return -1;
    }
    ...
    while (p + 8 < data + rec.data_len)
    {
        ...
        if (tdb->methods->tdb_write(tdb, ofs, p + 8, len) == -1)
        {
            free(data);
            TDB_LOG((tdb, TDB_DEBUG_FATAL,
                     "tdb_transaction_recover: failed to recover %d bytes at "
                     "offset %d\n",
                     len, ofs));
            tdb->ecode = TDB_ERR_IO;
            return -1;
...
static tdb_off_t tdb_allocate_ofs(struct tdb_context* tdb, tdb_len_t length,
                                  tdb_off_t rec_ptr, struct list_struct* rec,
                                  tdb_off_t last_ptr)
{
...
again:
    ...
    while (rec_ptr)
    {
        ...
        {
            if (bestfit.rec_ptr == 0 || rec->rec_len < bestfit.rec_len)
            {
    ...
    if (bestfit.rec_ptr != 0)
    {
        ...
        newrec_ptr = tdb_allocate_ofs(tdb, length, bestfit.rec_ptr, rec,
                                      bestfit.last_ptr);
        tdb_unlock(tdb, -1, F_WRLCK);
        return newrec_ptr;
    }
...
int tdb_validate_freelist(struct tdb_context* tdb, int* pnum_entries)
{
    ...
    *pnum_entries = 0;
    ...
    mem_tdb =
        tdb_open("flval", tdb->header.hash_size, TDB_INTERNAL, O_RDWR, 0600);
    if (!mem_tdb)
    {
        return -1;
    }
                         ...
                         struct list_struct* rec)
{
    ...
    for (; tlock->hash < tdb->header.hash_size; tlock->hash++)
    {
        ...
        if (!tlock->off)
        {
            if (tdb_ofs_read(tdb, TDB_HASH_TOP(tlock->hash), &tlock->off) == -1)
                goto fail;
        }
        else
        {
        ...
        while (tlock->off)
        {
            ...
            {
                TDB_LOG(
                    (tdb, TDB_DEBUG_FATAL, "tdb_next_lock: loop detected.\n"));
                goto fail;
            }
...
fail:
    ...
    if (tdb_unlock(tdb, tlock->hash, tlock->lock_rw) != 0)
        TDB_LOG(
            (tdb, TDB_DEBUG_FATAL, "tdb_next_lock: On error unlock failed!\n"));
    return -1;
}
...
/* traverse the entire database - calling fn(tdb, key, data) on each element.
  ...
  */
static int tdb_traverse_internal(struct tdb_context* tdb, tdb_traverse_func fn,
                                 void* private_data,
    ...
    while ((ret = tdb_next_lock(tdb, tl, &rec)) > 0)
    {
        ...
        if (!key.dptr)
        {
            ...
            if (tdb_unlock_record(tdb, tl->off) != 0)
                TDB_LOG((tdb, TDB_DEBUG_FATAL,
                         "tdb_traverse: key.dptr == NULL and unlock_record "
                         "failed!\n"));
            goto out;
        }
...
*/
int tdb_traverse_read(struct tdb_context* tdb, tdb_traverse_func fn,
                      void* private_data)
{
...
/*
  ...
  prevent deadlocks
*/
int tdb_traverse(struct tdb_context* tdb, tdb_traverse_func fn,
                 void* private_data)
{
...
TDB_DATA tdb_nextkey(struct tdb_context* tdb, TDB_DATA oldkey)
{
    ...
    if (tdb->travlocks.off)
    {
            ...
            memcmp(k, oldkey.dptr, oldkey.dsize) != 0)
        {
            ...
            if (tdb_unlock(tdb, tdb->travlocks.hash, tdb->travlocks.lock_rw) !=
                0)
            {
                SAFE_FREE(k);
                return tdb_null;
    ...
    if (!tdb->travlocks.off)
    {
        ...
        tdb->travlocks.off = tdb_find_lock_hash(
            tdb, oldkey, tdb->hash_fn(&oldkey), tdb->travlocks.lock_rw, &rec);
        if (!tdb->travlocks.off)
            return tdb_null;
                                 ...
                                 tdb_off_t offset)
{
    ...
    if (tdb->methods->tdb_read(tdb, offset, (char*)&rec, sizeof(rec),
                               DOCONV()) == -1)
    {
...
int tdb_printfreelist(struct tdb_context* tdb)
{
    ...
    while (rec_ptr)
    {
        if (tdb->methods->tdb_read(tdb, rec_ptr, (char*)&rec, sizeof(rec),
                                   DOCONV()) == -1)
        {
            tdb_unlock(tdb, -1, F_WRLCK);
            return -1;
...
/*
  non-blocking increment of the tdb sequence number if the tdb has been opened
  using the TDB_SEQNUM flag
*/
                          ...
                          struct list_struct* r)
{
    ...
    while (rec_ptr)
    {
        ...
        if (!TDB_DEAD(r) && hash == r->full_hash && key.dsize == r->key_len &&
            tdb_parse_data(tdb, key, rec_ptr + sizeof(*r), r->key_len,
                           tdb_key_compare, NULL) == 0)
        {
            return rec_ptr;
        }
        rec_ptr = r->next;
...
/* update an entry in place - this only works if the new data size
   ...
   on failure return -1.
*/
static int tdb_update_hash(struct tdb_context* tdb, TDB_DATA key, u32 hash,
                           TDB_DATA dbuf)
{
    struct list_struct rec;
    tdb_off_t rec_ptr;
...
TDB_DATA tdb_fetch(struct tdb_context* tdb, TDB_DATA key)
{
    ...
    ret.dptr =
        tdb_alloc_read(tdb, rec_ptr + sizeof(rec) + rec.key_len, rec.data_len);
...
int tdb_do_delete(struct tdb_context* tdb, tdb_off_t rec_ptr,
                  struct list_struct* rec)
{
    tdb_off_t last_ptr, i;
...
int tdb_store(struct tdb_context* tdb, TDB_DATA key, TDB_DATA dbuf, int flag)
{
    ...
    if (tdb->max_dead_records != 0)
    {
        ...
        if (rec_ptr != 0)
        {
            ...
            rec.magic = TDB_MAGIC;
            if (tdb_rec_write(tdb, rec_ptr, &rec) == -1 ||
                tdb->methods->tdb_write(tdb, rec_ptr + sizeof(rec), p,
    ...
    if ((tdb->max_dead_records != 0) && (tdb_purge_dead(tdb, hash) == -1))
    {
        tdb_unlock(tdb, -1, F_WRLCK);
        goto fail;
    }
...
int tdb_append(struct tdb_context* tdb, TDB_DATA key, TDB_DATA new_dbuf)
{
    ...
    else
    {
        unsigned char* new_dptr =
            (unsigned char*)realloc(dbuf.dptr, dbuf.dsize + new_dbuf.dsize);
        if (new_dptr == NULL)
        {
            free(dbuf.dptr);
        }
...
static void null_log_fn(struct tdb_context* tdb, enum tdb_debug_level level,
                                ...
                                tdb_hash_func hash_fn)
{
    ...
    if ((tdb_flags & TDB_CLEAR_IF_FIRST) &&
        (locked = (tdb->methods->tdb_brlock(tdb, ACTIVE_LOCK, F_WRLCK, F_SETLK,
                                            0, 1) == 0)))
    {
        open_flags |= O_CREAT;
        if (ftruncate(tdb->fd, 0) == -1)
    ...
    if (locked)
    {
        if (tdb->methods->tdb_brlock(tdb, ACTIVE_LOCK, F_UNLCK, F_SETLK, 0,
                                     1) == -1)
        {
            TDB_LOG((tdb, TDB_DEBUG_ERROR,
                     "tdb_open_ex: "
    ...
    if (tdb_flags & TDB_CLEAR_IF_FIRST)
    {
        ...
        if (tdb->methods->tdb_brlock(tdb, ACTIVE_LOCK, F_RDLCK, F_SETLKW, 0,
                                     1) == -1)
            goto fail;
    }
...
/* reopen a tdb - this can be used after a fork to ensure that we have an
   independent seek pointer from our parent and to re-establish locks */
