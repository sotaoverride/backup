...
typedef int (*wsEncodeFunc)(rfbClientPtr cl, const char* src, int len,
                            char** dst);
...
struct ws_ctx_s
{
    char codeBufDecode[2048 +
                       WSHLENMAX]; /* base64 + maximum frame header length */
    char codeBufEncode[B64LEN(UPDATE_BUF_SIZE) +
                       WSHLENMAX]; /* base64 + maximum frame header length */
    char* writePos;
