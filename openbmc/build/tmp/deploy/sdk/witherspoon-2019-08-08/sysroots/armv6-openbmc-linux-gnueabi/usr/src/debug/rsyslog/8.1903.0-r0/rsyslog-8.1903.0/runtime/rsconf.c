...
extern char* yytext;
/* static data */
DEFobjStaticHelpers DEFobjCurrIf(ruleset) DEFobjCurrIf(module)
    DEFobjCurrIf(conf) DEFobjCurrIf(glbl) DEFobjCurrIf(parser)
        DEFobjCurrIf(datetime)

    /* exported static data */
    rsconf_t* runConf = NULL; /* the currently running config */
rsconf_t* loadConf = NULL; /* the config currently being loaded (no concurrent
...
static uchar template_DebugFormat[] =
    ...
    "'%syslogtag%', programname: '%programname%', "
...
static uchar template_StdPgSQLFmt[] =
    ...
    "'%timegenerated:::date-pgsql%', %iut%, "
    "'%syslogtag%')\",STDSQL";
static uchar template_spoofadr[] = "\"%fromhost-ip%\"";
static uchar template_SysklogdFileFormat[] =
    "\"%TIMESTAMP% %HOSTNAME% %syslogtag%%msg:::sp-if-no-1st-sp%%msg%\n\"";
static uchar template_StdJSONFmt[] =
    "\"{\\\"message\\\":\\\"%msg:::json%\\\",\\\"fromhost\\\":\\\""
    "%HOSTNAME:::json%\\\",\\\"facility\\\":\\\"%syslogfacility-text%\\\","
    "\\\"priority\\\":\\\""
    "%syslogpriority-text%\\\",\\\"timereported\\\":\\\"%timereported:::date-"
    "rfc3339%\\\",\\\"timegenerated\\\":\\\""
    "%timegenerated:::date-rfc3339%\\\"}\"";
static uchar template_StdClickHouseFmt[] =
    "\"INSERT INTO rsyslog.SystemEvents (severity, facility, "
    "timestamp, hostname, tag, message) VALUES (%syslogseverity%, "
    "%syslogfacility%, "
    "'%timereported:::date-unixtimestamp%', '%hostname%', '%syslogtag%', "
    "'%msg%')\",STDSQL";
/* end templates */
...
static struct cnfparamblk parserpblk = {
    CNFPARAMBLK_VERSION, sizeof(parserpdescr) / sizeof(struct cnfparamdescr),
    parserpdescr};
 ...
 */
BEGINobjConstruct(
    rsconf) /* be sure to specify the object type also in END macro! */
...
finalize_it : ENDobjConstruct(rsconf)
...
BEGINobjDestruct(rsconf) /* be sure to specify the object type also in END and
                            CODESTART macros! */
    CODESTARTobjDestruct(rsconf) freeCnf(pThis);
tplDeleteAll(pThis);
...
BEGINobjDebugPrint(rsconf) /* be sure to specify the object type also in END and
                              CODESTART macros! */
...
dbgprintf("Main queue worker threads: %d, wThread shutdown: %d, Perists every "
          "%d updates.\n",
...
dbgprintf("Main queue timeouts: shutdown: %d, action completion shutdown: %d, "
          "enq: %d\n",
          pThis->globals.mainQ.iMainMsgQtoQShutdown,
          pThis->globals.mainQ.iMainMsgQtoActShutdown,
          pThis->globals.mainQ.iMainMsgQtoEnq);
dbgprintf("Main queue watermarks: high: %d, low: %d, discard: %d, "
          "discard-severity: %d\n",
          pThis->globals.mainQ.iMainMsgQHighWtrMark,
          pThis->globals.mainQ.iMainMsgQLowWtrMark,
          pThis->globals.mainQ.iMainMsgQDiscardMark,
          pThis->globals.mainQ.iMainMsgQDiscardSeverity);
dbgprintf("Main queue save on shutdown %d, max disk space allowed %lld\n",
          pThis->globals.mainQ.bMainMsgQSaveOnShutdown,
          pThis->globals.mainQ.iMainMsgQueMaxDiskSpace);
/* TODO: add
iActionRetryCount = 0;
...
100); setQPROP(qqueueSetbSaveOnShutdown, "$MainMsgQueueSaveOnShutdown", 1);
 */
...
CODESTARTobjDebugPrint(rsconf) ENDobjDebugPrint(rsconf)
    ...
    static rsRetVal parserProcessCnf(struct cnfobj* o)
{
    ...
    if ((pMod = module.FindWithCnfName(loadConf, cnfModName, eMOD_PARSER)) ==
        NULL)
    {
        LogError(0, RS_RET_MOD_UNKNOWN, "parser module name '%s' is unknown",
                 cnfModName);
    ...
    if (pMod->mod.pm.newParserInst == NULL)
    {
        LogError(0, RS_RET_MOD_NO_PARSER_STMT,
                 "parser module '%s' does not support parser() statement",
                 cnfModName);
...
static rsRetVal inputProcessCnf(struct cnfobj* o)
{
    ...
    {
        LogError(0, RS_RET_MOD_UNKNOWN, "input module name '%s' is unknown",
                 cnfModName);
        ABORT_FINALIZE(RS_RET_MOD_UNKNOWN);
    }
    if (pMod->mod.im.newInpInst == NULL)
    {
        LogError(0, RS_RET_MOD_NO_INPUT_STMT,
                 "input module '%s' does not support input() statement",
                 cnfModName);
        ABORT_FINALIZE(RS_RET_MOD_NO_INPUT_STMT);
    }
...
static void doDropPrivUid(int iUid)
{
    ...
    {
        rs_strerror_r(errno, (char*)szBuf, sizeof(szBuf));
        LogError(0, NO_ERRCODE, "could not get username for userid %d: %s",
                 iUid, szBuf);
    }
    ...
    if (res)
    {
        /* if we can not set the userid, this is fatal, so let's unconditionally
         * abort */
        perror("could not set requested userid");
        exit(1);
    }
    DBGPRINTF("setuid(%d): %d\n", iUid, res);
    snprintf((char*)szBuf, sizeof(szBuf), "rsyslogd's userid changed to %d",
             iUid);
    logmsgInternal(NO_ERRCODE, LOG_SYSLOG | LOG_INFO, szBuf, 0);
}
...
static rsRetVal dropPrivileges(rsconf_t* cnf)
{
    ...
    if (cnf->globals.gidDropPriv != 0)
    {
        ...
        DBGPRINTF("group privileges have been dropped to gid %u\n",
                  (unsigned)ourConf->globals.gidDropPriv);
    }
    ...
    if (cnf->globals.uidDropPriv != 0)
    {
        ...
        DBGPRINTF("user privileges have been dropped to uid %u\n",
                  (unsigned)ourConf->globals.uidDropPriv);
    }
...
static rsRetVal tellModulesActivateConfigPrePrivDrop(void)
{
    ...
    {
        if (node->pMod->beginCnfLoad != NULL &&
            node->pMod->activateCnfPrePrivDrop != NULL && node->canActivate)
        {
...
static rsRetVal runInputModules(void)
{
    ...
    while (node != NULL)
    {
        if (node->canRun)
        {
            bNeedsCancel = (node->pMod->isCompatibleWithFeature(
                                sFEATURENonCancelInputTermination) == RS_RET_OK)
                               ? 0
                               : 1;
            DBGPRINTF("running module %s with config %p, term mode: %s\n",
                      node->pMod->pszName, node,
                      bNeedsCancel ? "cancel" : "cooperative/SIGTTIN");
            thrdCreate(node->pMod->mod.im.runInput, node->pMod->mod.im.afterRun,
                       bNeedsCancel,
                       (node->pMod->cnfName == NULL) ? node->pMod->pszName
                                                     : node->pMod->cnfName);
        }
...
static rsRetVal activateMainQueue(void)
{
    ...
    if (iRet != RS_RET_OK)
    {
        ...
        fprintf(stderr,
                "fatal error %d: could not create message queue - rsyslogd can "
                "not run!\n",
                iRet);
        FINALIZE;
    }
    ...
    bHaveMainQueue =
        (ourConf->globals.mainQ.MainMsgQueType == QUEUETYPE_DIRECT) ? 0 : 1;
...
static rsRetVal activate(rsconf_t* cnf)
{
     ...
     * NOTE: as of 2009-06-29, the input modules are initialized, but not yet
     * run. Keep in mind. though, that the outputs already run if the queue was
...
static rsRetVal setActionResumeInterval(void __attribute__((unused)) * pVal,
                                        int iNewVal)
{
 ...
 */
static rsRetVal setDefaultRuleset(void __attribute__((unused)) * pVal,
                                  uchar* pszName)
{
    DEFiRet;
 ...
 * rgerhards, 2009-06-12
 */
static rsRetVal setCurrRuleset(void __attribute__((unused)) * pVal,
                               uchar* pszName)
{
    ruleset_t* pRuleset;
    rsRetVal localRet;
 ...
 */
static rsRetVal setMainMsgQueType(void __attribute__((unused)) * pVal,
                                  uchar* pszType)
{
    DEFiRet;
    ...
    else if (!strcasecmp((char*)pszType, "direct"))
    {
        ...
        DBGPRINTF(
            "main message queue type set to DIRECT (no queueing at all)\n");
    }
    else
    {
        LogError(0, RS_RET_INVALID_PARAMS,
                 "unknown mainmessagequeuetype parameter: %s", (char*)pszType);
...
static rsRetVal regBuildInModule(rsRetVal (*modInit)(), uchar* name,
                                 void* pModHdlr)
{
...
static rsRetVal loadBuildInModules(void)
{
    ...
    CHKiRet(
        regBuildInModule(modInitFile, UCHAR_CONSTANT("builtin:omfile"), NULL));
    CHKiRet(
        regBuildInModule(modInitPipe, UCHAR_CONSTANT("builtin:ompipe"), NULL));
    CHKiRet(
        regBuildInModule(modInitShell, UCHAR_CONSTANT("builtin-shell"), NULL));
    CHKiRet(regBuildInModule(modInitDiscard,
                             UCHAR_CONSTANT("builtin:omdiscard"), NULL));
#ifdef SYSLOG_INET
    CHKiRet(
        regBuildInModule(modInitFwd, UCHAR_CONSTANT("builtin:omfwd"), NULL));
#endif

    /* dirty, but this must be for the time being: the usrmsg module must always
     * be loaded as last module. This is because it processes any type of action
     * selector. If we load it before other modules, these others will never
     * have a chance of working with the config file. We may change that
     * implementation so that a user name must start with an alnum, that would
     * definitely help (but would it break backwards compatibility?). *
     * rgerhards, 2007-07-23 User names now must begin with: [a-zA-Z0-9_.]
     */
    CHKiRet(regBuildInModule(modInitUsrMsg, (uchar*)"builtin:omusrmsg", NULL));

    /* load build-in parser modules */
    CHKiRet(regBuildInModule(modInitpmrfc5424,
                             UCHAR_CONSTANT("builtin:pmrfc5424"), NULL));
    CHKiRet(regBuildInModule(modInitpmrfc3164,
                             UCHAR_CONSTANT("builtin:pmrfc3164"), NULL));
    ...
    CHKiRet(regBuildInModule(modInitsmfile, UCHAR_CONSTANT("builtin:smfile"),
                             NULL));
    CHKiRet(regBuildInModule(modInitsmtradfile,
                             UCHAR_CONSTANT("builtin:smtradfile"), NULL));
    CHKiRet(
        regBuildInModule(modInitsmfwd, UCHAR_CONSTANT("builtin:smfwd"), NULL));
    CHKiRet(regBuildInModule(modInitsmtradfwd,
                             UCHAR_CONSTANT("builtin:smtradfwd"), NULL));

finalize_it:
    if (iRet != RS_RET_OK)
    {
        /* we need to do fprintf, as we do not yet have an error reporting
         * system in place.
         */
        fprintf(stderr,
                "fatal error: could not activate built-in modules. Error code "
                "%d.\n",
                iRet);
    }
    RETiRet;
...
static rsRetVal initLegacyConf(void)
{
    ...
    CHKiRet(regCfSysLineHdlr((uchar*)"sleep", 0, eCmdHdlrGoneAway, NULL, NULL,
                             NULL));
    CHKiRet(regCfSysLineHdlr((uchar*)"logrsyslogstatusmessages", 0,
                             eCmdHdlrBinary, NULL,
    ...
    CHKiRet(regCfSysLineHdlr((uchar*)"abortonuncleanconfig", 0, eCmdHdlrBinary,
                             NULL, &loadConf->globals.bAbortOnUncleanConfig,
                             NULL));
    CHKiRet(regCfSysLineHdlr((uchar*)"repeatedmsgreduction", 0, eCmdHdlrBinary,
                             NULL, &loadConf->globals.bReduceRepeatMsgs, NULL));
    CHKiRet(regCfSysLineHdlr(
        (uchar*)"debugprinttemplatelist", 0, eCmdHdlrBinary, NULL,
        &(loadConf->globals.bDebugPrintTemplateList), NULL));
    CHKiRet(regCfSysLineHdlr((uchar*)"debugprintmodulelist", 0, eCmdHdlrBinary,
                             NULL, &(loadConf->globals.bDebugPrintModuleList),
                             NULL));
    CHKiRet(regCfSysLineHdlr(
        (uchar*)"debugprintcfsyslinehandlerlist", 0, eCmdHdlrBinary, NULL,
        &(loadConf->globals.bDebugPrintCfSysLineHandlerList), NULL));
    CHKiRet(regCfSysLineHdlr((uchar*)"privdroptouser", 0, eCmdHdlrUID, NULL,
                             &loadConf->globals.uidDropPriv, NULL));
    CHKiRet(regCfSysLineHdlr((uchar*)"privdroptouserid", 0, eCmdHdlrInt, NULL,
                             &loadConf->globals.uidDropPriv, NULL));
    CHKiRet(regCfSysLineHdlr((uchar*)"privdroptogroup", 0, eCmdHdlrGID, NULL,
                             &loadConf->globals.gidDropPriv, NULL));
    CHKiRet(regCfSysLineHdlr((uchar*)"privdroptogroupid", 0, eCmdHdlrInt, NULL,
                             &loadConf->globals.gidDropPriv, NULL));
    CHKiRet(regCfSysLineHdlr((uchar*)"generateconfiggraph", 0, eCmdHdlrGetWord,
                             NULL, &loadConf->globals.pszConfDAGFile, NULL));
    CHKiRet(regCfSysLineHdlr((uchar*)"umask", 0, eCmdHdlrFileCreateMode, NULL,
                             &loadConf->globals.umask, NULL));
    ...
    CHKiRet(regCfSysLineHdlr(
        (uchar*)"mainmsgqueueworkerthreads", 0, eCmdHdlrInt, NULL,
        &loadConf->globals.mainQ.iMainMsgQueueNumWorkers, NULL));
    CHKiRet(regCfSysLineHdlr(
        (uchar*)"mainmsgqueuetimeoutshutdown", 0, eCmdHdlrInt, NULL,
        &loadConf->globals.mainQ.iMainMsgQtoQShutdown, NULL));
    CHKiRet(regCfSysLineHdlr(
        (uchar*)"mainmsgqueuetimeoutactioncompletion", 0, eCmdHdlrInt, NULL,
        &loadConf->globals.mainQ.iMainMsgQtoActShutdown, NULL));
    CHKiRet(regCfSysLineHdlr((uchar*)"mainmsgqueuetimeoutenqueue", 0,
                             eCmdHdlrInt, NULL,
                             &loadConf->globals.mainQ.iMainMsgQtoEnq, NULL));
    CHKiRet(regCfSysLineHdlr(
        (uchar*)"mainmsgqueueworkertimeoutthreadshutdown", 0, eCmdHdlrInt, NULL,
        &loadConf->globals.mainQ.iMainMsgQtoWrkShutdown, NULL));
    CHKiRet(regCfSysLineHdlr(
        (uchar*)"mainmsgqueuedequeueslowdown", 0, eCmdHdlrInt, NULL,
        &loadConf->globals.mainQ.iMainMsgQDeqSlowdown, NULL));
    CHKiRet(regCfSysLineHdlr(
        (uchar*)"mainmsgqueueworkerthreadminimummessages", 0, eCmdHdlrInt, NULL,
        &loadConf->globals.mainQ.iMainMsgQWrkMinMsgs, NULL));
    CHKiRet(regCfSysLineHdlr(
        (uchar*)"mainmsgqueuemaxfilesize", 0, eCmdHdlrSize, NULL,
        &loadConf->globals.mainQ.iMainMsgQueMaxFileSize, NULL));
    CHKiRet(regCfSysLineHdlr(
        (uchar*)"mainmsgqueuedequeuebatchsize", 0, eCmdHdlrSize, NULL,
        &loadConf->globals.mainQ.iMainMsgQueDeqBatchSize, NULL));
    CHKiRet(regCfSysLineHdlr(
        (uchar*)"mainmsgqueuemaxdiskspace", 0, eCmdHdlrSize, NULL,
        &loadConf->globals.mainQ.iMainMsgQueMaxDiskSpace, NULL));
    CHKiRet(regCfSysLineHdlr(
        (uchar*)"mainmsgqueuesaveonshutdown", 0, eCmdHdlrBinary, NULL,
        &loadConf->globals.mainQ.bMainMsgQSaveOnShutdown, NULL));
    CHKiRet(regCfSysLineHdlr(
        (uchar*)"mainmsgqueuedequeuetimebegin", 0, eCmdHdlrInt, NULL,
        &loadConf->globals.mainQ.iMainMsgQueueDeqtWinFromHr, NULL));
    CHKiRet(regCfSysLineHdlr(
        (uchar*)"mainmsgqueuedequeuetimeend", 0, eCmdHdlrInt, NULL,
     ...
     */
    CHKiRet(regCfSysLineHdlr((uchar*)"moddir", 0, eCmdHdlrGetWord, setModDir,
                             NULL, NULL));
    ...
    pTmp = template_FileFormat; /* new format for files with high-precision
                                   stamp */
    ...
    tplLastStaticInit(
        ourConf, tplAddLine(ourConf, "RSYSLOG_omudpspoofDfltSourceTpl", &pTmp));
 ...
 */
static rsRetVal validateConf(void)
{
    DEFiRet;
...
static rsRetVal load(rsconf_t** cnf, uchar* confFile)
{
    ...
    /* we are done checking the config - now validate if we should actually run
     * or not. If not, terminate. -- rgerhards, 2008-07-25
     * TODO: iConfigVerify -- should it be pulled from the config, or leave as
     * is (option)?
     */
    if (iConfigVerify)
    {
...
BEGINobjQueryInterface(rsconf)
    CODESTARTobjQueryInterface(rsconf) if (pIf->ifVersion !=
                                           rsconfCURR_IF_VERSION)
