...
const int message_progress_sigs[] = {SIGALRM,
#ifdef SIGINFO
                                     SIGINFO,
#endif
#ifdef SIGUSR1
...
static void progress_signal_handler(int sig lzma_attribute((__unused__)))
{
...
static void print_filename(void)
{
    if (!opt_robot && (files_total != 1 || filename != stdin_filename))
    {
        ...
        if (files_total == 0)
            ...
            fprintf(file, "%s (%u/%u)\n", filename, files_pos, files_total);
...
extern void message_filename(const char* src_name)
{
    ...
    if (verbosity >= V_VERBOSE && (progress_automatic || opt_mode == MODE_LIST))
        print_filename();
    else
...
static const char* progress_percentage(uint64_t in_pos)
{
    ...
    double percentage = (double)(in_pos) / (double)(expected_in_size)*99.9;
...
static const char* progress_sizes(uint64_t compressed_pos,
                                  uint64_t uncompressed_pos, bool final)
{
    ...
    my_snprintf(
        &pos, &left, "%s / %s",
        uint64_to_nicestr(compressed_pos, unit_min, NICESTR_TIB, false, 0),
        uint64_to_nicestr(uncompressed_pos, unit_min, NICESTR_TIB, false, 1));

    // Avoid division by zero. If we cannot calculate the ratio, set
    // it to some nice number greater than 10.0 so that it gets caught
    // in the next if-clause.
    const double ratio = uncompressed_pos > 0 ? (double)(compressed_pos) /
                                                    (double)(uncompressed_pos)
                                              : 16.0;
...
static const char* progress_speed(uint64_t uncompressed_pos, uint64_t elapsed)
{
    ...
    double speed =
        (double)(uncompressed_pos) / ((double)(elapsed) * (1024.0 / 1000.0));
    ...
    // Use big enough buffer to hold e.g. a multibyte decimal point.
    static char buf[16];
    snprintf(buf, sizeof(buf), "%.*f %s", speed > 9.9 ? 0 : 1, speed,
             unit[unit_index]);
    return buf;
}
...
static const char* progress_time(uint64_t mseconds)
{
    ...
    if (minutes >= 60)
    {
        ...
        minutes %= 60;
        snprintf(buf, sizeof(buf), "%" PRIu32 ":%02" PRIu32 ":%02" PRIu32,
                 hours, minutes, seconds);
    }
    else
    {
        snprintf(buf, sizeof(buf), "%" PRIu32 ":%02" PRIu32, minutes, seconds);
    }
...
static void progress_pos(uint64_t* in_pos, uint64_t* compressed_pos,
...
extern void message_progress_update(void)
{
    ...
    fprintf(stderr, "\r %*s %*s   %*s %10s   %10s\r",
            tuklib_mbstr_fw(cols[0], 6), cols[0], tuklib_mbstr_fw(cols[1], 35),
            cols[1], tuklib_mbstr_fw(cols[2], 9), cols[2], cols[3], cols[4]);
...
static void progress_flush(bool finished)
{
    ...
    if (progress_automatic)
    {
        ...
        fprintf(stderr, "\r %*s %*s   %*s %10s   %10s\n",
                ...
                tuklib_mbstr_fw(cols[2], 9), cols[2], cols[3], cols[4]);
    }
    else
    {
...
extern void message_progress_end(bool success)
{
...
extern void message(enum message_verbosity v, const char* fmt, ...)
{
    va_list ap;
    va_start(ap, fmt);
    vmessage(v, fmt, ap);
        ...
        switch (filters[i].id)
        {
            ...
            case LZMA_FILTER_LZMA2:
            {
                ...
                my_snprintf(&pos, &left, "lzma%c=dict=%s",
                            filters[i].id == LZMA_FILTER_LZMA2 ? '2' : '1',
                ...
                assert(filters[i].id == LZMA_FILTER_LZMA2 || all_known);

                // Print the rest of the options, which are known
                // only when compressing.
                if (all_known)
                    my_snprintf(&pos, &left,
                                ",lc=%" PRIu32 ",lp=%" PRIu32 ",pb=%" PRIu32
                                ",mode=%s,nice=%" PRIu32 ",mf=%s"
                                ",depth=%" PRIu32,
                                opt->lc, opt->lp, opt->pb, mode, opt->nice_len,
                                mf, opt->depth);
                break;
            }
            ...
            case LZMA_FILTER_SPARC:
            {
                ...
                my_snprintf(&pos, &left, "%s",
                            bcj_names[filters[i].id - LZMA_FILTER_X86]);
            ...
            case LZMA_FILTER_DELTA:
            {
                ...
                my_snprintf(&pos, &left, "delta=dist=%" PRIu32, opt->dist);
                break;
            }
...
extern void message_filters_show(enum message_verbosity v,
...
extern void message_try_help(void)
{
    ...
    message(V_WARNING, _("Try `%s --help' for more information."), progname);
    return;
}
...
extern void message_help(bool long_help)
{
    ...
    puts(_("  -k, --keep          keep (don't delete) input files\n"
           "  -f, --force         force overwrite of output file and "
           "(de)compress links\n"
           "  -c, --stdout        write to standard output and don't delete "
           "input files"));

    if (long_help)
    {
        puts(_("      --single-stream decompress only the first stream, and "
               "silently\n"
               "                      ignore possible remaining input data"));
        puts(_(
            "      --no-sparse     do not create sparse files when "
            "decompressing\n"
            "  -S, --suffix=.SUF   use the suffix `.SUF' on compressed files\n"
            "      --files[=FILE]  read filenames to process from FILE; if "
            "FILE is\n"
            "                      omitted, filenames are read from the "
            "standard input;\n"
            "                      filenames must be terminated with the "
            "newline character\n"
    ...
    if (long_help)
    {
        ...
        puts(_(
            "  -F, --format=FMT    file format to encode or decode; possible "
            "values are\n"
            "                      `auto' (default), `xz', `lzma', and `raw'\n"
            "  -C, --check=CHECK   integrity check type: `none' (use with "
            "caution),\n"
            "                      `crc32', `crc64' (default), or `sha256'"));
        puts(_("      --ignore-check  don't verify the integrity check when "
               "decompressing"));
    }

    puts(_("  -0 ... -9           compression preset; default is 6; take "
           "compressor *and*\n"
           "                      decompressor memory usage into account "
           "before using 7-9!"));

    puts(_("  -e, --extreme       try to improve compression ratio by using "
           "more CPU time;\n"
           "                      does not affect decompressor memory "
           "requirements"));

    puts(_("  -T, --threads=NUM   use at most NUM threads; the default is 1; "
           "set to 0\n"
           "                      to use as many threads as there are "
           "processor cores"));

    if (long_help)
    {
        puts(_("      --block-size=SIZE\n"
               "                      start a new .xz block after every SIZE "
               "bytes of input;\n"
               "                      use this to set the block size for "
               "threaded compression"));
        puts(_("      --block-list=SIZES\n"
               "                      start a new .xz block after the given "
               "comma-separated\n"
               "                      intervals of uncompressed data"));
        puts(_("      --flush-timeout=TIMEOUT\n"
               "                      when compressing, if more than TIMEOUT "
               "milliseconds has\n"
               "                      passed since the previous flush and "
               "reading more input\n"
               "                      would block, all pending data is flushed "
               "out"));
        puts(_( // xgettext:no-c-format
            ...
            "decompression,\n"
            "                      or both; LIMIT is in bytes, % of RAM, or 0 "
            "for defaults"));

        puts(_("      --no-adjust     if compression settings exceed the "
               "memory usage limit,\n"
               "                      give an error instead of adjusting the "
               "settings downwards"));
    }
    ...
    defined(HAVE_ENCODER_LZMA2) || defined(HAVE_DECODER_LZMA2)
        ...
        // mean "literally" here.
        puts(_("\n"
               "  --lzma1[=OPTS]      LZMA1 or LZMA2; OPTS is a "
               "comma-separated list of zero or\n"
               "  --lzma2[=OPTS]      more of the following options (valid "
               "values; default):\n"
               "                        preset=PRE reset options to a preset "
               "(0-9[e])\n"
               "                        dict=NUM   dictionary size (4KiB - "
               "1536MiB; 8MiB)\n"
               "                        lc=NUM     number of literal context "
               "bits (0-4; 3)\n"
               "                        lp=NUM     number of literal position "
               "bits (0-4; 0)\n"
               "                        pb=NUM     number of position bits "
               "(0-4; 2)\n"
               "                        mode=MODE  compression mode (fast, "
               "normal; normal)\n"
               "                        nice=NUM   nice length of a match "
               "(2-273; 64)\n"
               "                        mf=NAME    match finder (hc3, hc4, "
               "bt2, bt3, bt4; bt4)\n"
               "                        depth=NUM  maximum search depth; "
               "0=automatic (default)"));
#endif
        ...
        puts(
            _("\n"
              "  --delta[=OPTS]      Delta filter; valid OPTS (valid values; "
              "default):\n"
    ...
    puts(_("  -q, --quiet         suppress warnings; specify twice to suppress "
           "errors too\n"
           "  -v, --verbose       be verbose; specify twice for even more "
           "verbose"));
    ...
    if (long_help)
    {
        ...
        puts(_("      --robot         use machine-parsable messages (useful "
               "for scripts)"));
        puts("");
        puts(_("      --info-memory   display the total amount of RAM and the "
               "currently active\n"
               "                      memory usage limits, and exit"));
        puts(_("  -h, --help          display the short help (lists only the "
               "basic options)\n"
               "  -H, --long-help     display this long help and exit"));
    }
    else
    {
        puts(_("  -h, --help          display this short help and exit\n"
               "  -H, --long-help     display the long help (lists also the "
               "advanced options)"));
    }

    puts(_("  -V, --version       display the version number and exit"));
    ...
    puts(_("THIS IS A DEVELOPMENT VERSION NOT INTENDED FOR PRODUCTION USE."));
#endif
