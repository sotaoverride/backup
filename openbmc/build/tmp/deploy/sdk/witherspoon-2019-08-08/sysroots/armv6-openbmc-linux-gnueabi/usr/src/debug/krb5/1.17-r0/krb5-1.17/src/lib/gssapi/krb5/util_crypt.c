...
static krb5_error_code kg_copy_keys(krb5_context context,
                                    krb5_gss_ctx_id_rec* ctx, krb5_key subkey)
{
...
static krb5_error_code kg_derive_des_enc_key(krb5_context context,
                                             krb5_key subkey, krb5_key* out)
{
    krb5_error_code code;
 ...
 * NULL. */
static krb5_error_code iv_to_state(krb5_context context, krb5_key key,
                                   krb5_pointer iv, krb5_data** data_out)
{
    krb5_error_code code;
    krb5_data* data;
    size_t blocksize;
                        ...
                        krb5_crypto_iov** pkiov, size_t* pkiov_count)
{
    ...
    *pkiov_count = 0;
    ...
    kiov[i].data.data =
        (char*)header->buffer.value + header->buffer.length - conf_len;
    i++;
 ...
 */
static krb5_error_code
    kg_translate_iov_v3(krb5_context context, int dce_style, size_t ec,
                        size_t rrc, krb5_enctype enctype,
                        gss_iov_buffer_desc* iov, int iov_count,
                        krb5_crypto_iov** pkiov, size_t* pkiov_count)
{
    ...
    *pkiov_count = 0;
    ...
    gss_trailerlen =
        ec + 16 /* E(GSS-Header) */ + k5_trailerlen; /* Kerb-Trailer */
    ...
    kiov[i].data.data =
        (char*)header->buffer.value + header->buffer.length - k5_headerlen;
    i++;
...
static krb5_error_code kg_translate_iov(krb5_context context, int proto,
                                        int dce_style, size_t ec, size_t rrc,
                                        krb5_enctype enctype,
                                        gss_iov_buffer_desc* iov, int iov_count,
                                        krb5_crypto_iov** pkiov,
                                        size_t* pkiov_count)
{
    return proto ? kg_translate_iov_v3(context, dce_style, ec, rrc, enctype,
                               ...
                               int iov_count)
{
    ...
    code = kg_translate_iov(context, proto, dce_style, ec, rrc,
                            key->keyblock.enctype, iov, iov_count, &kiov,
                            &kiov_len);
    if (code == 0)
    {
                                       ...
                                       gss_iov_buffer_desc* iov, int iov_count)
{
    ...
    code = kg_translate_iov(context, 0 /* proto */, 0 /* dce_style */,
                            0 /* ec */, 0 /* rrc */, keyblock->enctype, iov,
                            iov_count, &kiov, &kiov_len);
    if (code)
        return code;
...
void kg_iov_msglen(gss_iov_buffer_desc* iov, int iov_count,
                   size_t* data_length_p, size_t* assoc_data_length_p)
{
    int i;
