...
static FT_Error t1_get_glyph_name(T1_Face face, FT_UInt glyph_index,
...
static FT_Long t1_ps_get_font_value(FT_Face face, PS_Dict_Keys key, FT_UInt idx,
                                    void* value, FT_Long value_len_)
{
    ...
    switch (key)
    {
        ...
        case PS_DICT_FONT_MATRIX:
            if (idx <
                sizeof(type1->font_matrix) / sizeof(type1->font_matrix.xx))
            {
                FT_Fixed val = 0;
        ...
        case PS_DICT_FONT_BBOX:
            if (idx < sizeof(type1->font_bbox) / sizeof(type1->font_bbox.xMin))
            {
                FT_Fixed val = 0;
        ...
        case PS_DICT_SUBR:
        {
            ...
            if (type1->subrs_hash)
            {
                ...
                size_t* val =
                    ft_hash_num_lookup((FT_Int)idx, type1->subrs_hash);
        ...
        case PS_DICT_FAMILY_OTHER_BLUE:
            if (idx < type1->private_dict.num_family_other_blues)
            {
                ...
                if (value && value_len >= retval)
                    *((FT_Short*)value) =
                        type1->private_dict.family_other_blues[idx];
            }
            break;
        ...
        case PS_DICT_FULL_NAME:
            if (type1->font_info.full_name)
            {
                ...
                if (value && value_len >= retval)
                    ft_memcpy(value, (void*)(type1->font_info.full_name),
                              retval);
            }
            break;
 ...
 */
static FT_Error Get_Kerning(FT_Face t1face, /* T1_Face */
                            ...
                            FT_Vector* kerning)
{
    ...
    if (face->afm_data)
        T1_Get_Kerning((AFM_FontInfo)face->afm_data, left_glyph, right_glyph,
                       kerning);
