...
static int process_args(pam_handle_t* pamh, int argc, const char** argv,
                        const char* type, char*** evp, const char** filtername)
{
    int ctrl = 0;
...
static int set_filter(pam_handle_t* pamh, int flags UNUSED, int ctrl,
                      const char** evp, const char* filtername)
{
    ...
    { /* child process *is* application */
        ...
        if (aterminal)
        {
            ...
            {
                pam_syslog(pamh, LOG_ERR,
                           "Cannot grant acccess to slave terminal");
                return PAM_ABORT;
            }
            ...
            if (fd[1] < 0)
            {
                pam_syslog(pamh, LOG_ERR, "cannot open slave terminal: %s: %m",
                           terminal);
                return PAM_ABORT;
            }
        ...
        {
            pam_syslog(pamh, LOG_ERR, "unable to re-assign STDIN/OUT/ERR: %m");
            close(fd[1]);
            return PAM_ABORT;
        }
    ...
    { /* exec the child filter */
        ...
        {
            pam_syslog(pamh, LOG_ERR, "unable to retain APPIN/OUT/ERR: %m");
            close(APPIN_FILENO);
            close(APPOUT_FILENO);
            close(APPERR_FILENO);
            _exit(1);
    ...
    { /* wait for either of the two children to exit */

        while (child && child2)
        { /* loop if there are two children */
            int lstatus = 0;
            int chid;
...
static int need_a_filter(pam_handle_t* pamh, int flags, int argc,
                         const char** argv, const char* name, int which_run)
{
    int ctrl;
    char** evp;
    ...
    {
        retval = set_filter(pamh, flags, ctrl, (const char**)evp, filterfile);
    }

    if (retval == PAM_SUCCESS && !(ctrl & NON_TERM) && (ctrl & NEW_TERM))
    {
        retval = set_the_terminal(pamh);
        if (retval != PAM_SUCCESS)
        {
...
int pam_sm_authenticate(pam_handle_t* pamh, int flags, int argc,
                        const char** argv)
{
    return need_a_filter(pamh, flags, argc, argv, "authenticate", FILTER_RUN1);
}
...
{
    return need_a_filter(pamh, flags, argc, argv, "setcred",
...
int pam_sm_open_session(pam_handle_t* pamh, int flags, int argc,
                        const char** argv)
{
    return need_a_filter(pamh, flags, argc, argv, "open_session", FILTER_RUN1);
}

int pam_sm_close_session(pam_handle_t* pamh, int flags, int argc,
                         const char** argv)
{
    return need_a_filter(pamh, flags, argc, argv, "close_session", FILTER_RUN2);
}
