...
{
    if (!mConnected || !mActive || !keys || !len)
        return;
...
{
    for (s8* end = buf + len;
         buf < end && MSG(buf)->len && MSG(buf)->len <= (end - buf);
         buf += MSG(buf)->len)
    {
        if (mMsgWaitState == WaitingMessage && mMsgWaitType == MSG(buf)->type)
            mMsgWaitState = GotMessage;
        if (!mConnected && MSG(buf)->type != Connect)
            continue;
        ...
        switch (MSG(buf)->type)
        {
            ...
            case PutText:
                ...
                {
                    mShell->imInput(MSG(buf)->texts,
                                    MSG(buf)->len - OFFSET(Message, texts));
                }
                break;

            case SetWin:
                if (MSG(buf)->len == sizeof(Message))
                { // keep compatibility with v1.5
            ...
            case FillRect:
                if (addWinMsg(MSG(buf)) &&
                    FbShellManager::instance()->activeShell() == mShell)
                {
                    doFillRect(MSG(buf));
                }
                break;
...
static IntersectState intersectRectangles(const Rectangle& r1,
                                          const Rectangle& r2)
{
    if (!r1.w || !r1.h)
        return Inside;
    ...
    if (r1.x >= (r2.x + r2.w) || r1.y >= (r2.y + r2.h) ||
        ...
        return Outside;
    if (r1.x >= r2.x && r1.y >= r2.y && (r1.x + r1.w) <= (r2.x + r2.w) &&
        (r1.y + r1.h) <= (r2.y + r2.h))
        return Inside;
    return Intersect;
}
...
void ImProxy::redrawImWin(const Rectangle& rect)
{
    for (u32 num = mValidWinNum, i = 0; num && i < NR_IM_WINS; i++)
    {
        ...
        for (MsgList::iterator it = mWinMsgs[i].begin();
             it != mWinMsgs[i].end(); it++)
        {
            Message* m = *it;

            if (m->type == FillRect)
                doFillRect(m);
            else if (m->type == DrawText)
                doDrawText(m);
        }
    }
}
...
void ImProxy::setImWin(u32 id, Rectangle& rect)
{
    ...
    for (MsgList::iterator it = mWinMsgs[id].begin(); it != mWinMsgs[id].end();
         it++)
    {
        delete[](s8*)(*it);
    }
...
bool ImProxy::addWinMsg(Message* m)
{
    ...
    if (m->type == FillRect)
        rect = m->fillRect.rect;
    else
    {
        if (m->len > MSG_DRAWTEXT_MAX_LEN)
            return false;
    ...
    for (u32 num = mValidWinNum, i = 0; num && i < NR_IM_WINS; i++)
    {
        ...
        if (mWinMsgs[i].size() >= NR_WIN_MSGS)
            return false;
...
void ImProxy::doFillRect(Message* m)
{
    ...
    Screen::instance()->fillRect(rect.x, rect.y, rect.w, rect.h,
                                 m->fillRect.color);
}
...
static void utf8_to_utf16(u8* utf8, u16* utf16, u16& len)
{
    ...
    for (; utf8 < end;)
    {
        ...
        {
            utf16[len++] = ((*utf8 & 0xf) << 12) | ((utf8[1] & 0x3f) << 6) |
                           (utf8[2] & 0x3f);
            utf8 += 3;
        }
        else
...
{
    if (m->len <= OFFSET(Message, drawText.texts))
        return;
    ...
    Screen::instance()->drawText(m->drawText.x, m->drawText.y, m->drawText.fc,
                                 m->drawText.bc, len, utf16, dws);
}
...
{
    if (read && mMsgWaitState == WaitingMessage)
        mMsgWaitState = GotMessage;
}
