...
static struct argp argp = {options, parse_opt, args_doc, doc, NULL, more_help};
...
#define BUILTIN_ALIAS(alias, real) {.from = alias, .to = real},
#define BUILTIN_TRANSFORMATION(From, To, Cost, Name, Fct, BtowcFct, MinF,      \
                               MaxF, MinT, MaxT)
...
extern struct Strent* strtabadd(struct Strtab* st, const char* str, size_t len);
...
static void append_alias(const void* nodep, VISIT value, int level)
{
    ...
    {
        nalias_list_max += 50;
        alias_list = (struct alias**)xrealloc(
            alias_list, (nalias_list_max * sizeof(struct alias*)));
    }
...
static void new_module(const char* fromname, size_t fromlen, const char* toname,
                       ...
                       const char* filename, size_t filelen, int cost,
                       size_t need_ext)
{
    ...
    new_module = (struct module*)xmalloc(sizeof(struct module) + fromlen +
                                         tolen + filelen + need_ext);
    ...
    {
        new_module->fromname_strent =
            strtabadd(strtab, new_module->fromname, fromlen);
        new_module->toname_strent =
            strtabadd(strtab, new_module->toname, tolen);
        new_module->filename_strent =
            strtabadd(strtab, new_module->filename, filelen);
...
static void append_module(const void* nodep, VISIT value, int level)
{
    ...
    {
        nmodule_list_max += 50;
        module_list = (struct module**)xrealloc(
            module_list, (nmodule_list_max * sizeof(struct module*)));
    }
...
static void add_builtins(void)
{
    ...
    for (cnt = 0; cnt < nbuiltin_alias; ++cnt)
        new_alias(builtin_alias[cnt].from, strlen(builtin_alias[cnt].from) + 1,
                  builtin_alias[cnt].to, strlen(builtin_alias[cnt].to) + 1);

    /* add the builtin transformations.  */
    for (cnt = 0; cnt < nbuiltin_trans; ++cnt)
        new_module(builtin_trans[cnt].from, strlen(builtin_trans[cnt].from) + 1,
                   builtin_trans[cnt].to, strlen(builtin_trans[cnt].to) + 1, "",
...
static void generate_name_list(void)
{
    ...
    tsearch(
        new_name("INTERNAL", strtabadd(strtab, "INTERNAL", sizeof("INTERNAL"))),
        &names, name_compare);
    ...
    for (i = 0; i < nmodule_list; ++i)
    {
        ...
        for (runp = module_list[i]; runp != NULL; runp = runp->next)
            if (strcmp(runp->toname, "INTERNAL") != 0)
                tsearch(new_name(runp->toname, runp->toname_strent), &names,
                        name_compare);
    }
}
...
static void generate_name_info(void)
{
    ...
    name_info =
        (struct name_info*)xcalloc(nmodule_list + 1, sizeof(struct name_info));
    ...
    name_info[0].canonical_strent =
        strtabadd(strtab, "INTERNAL", sizeof("INTERNAL"));
    assert(nname_info == 1);
        ...
        for (runp = module_list[i]; runp != NULL; runp = runp->next)
            if (strcmp(runp->fromname, "INTERNAL") == 0)
            {
                ...
                assert(name_info[idx].canonical_name == NULL ||
                       strcmp(name_info[idx].canonical_name, runp->toname) ==
                           0);
            ...
            else if (strcmp(runp->toname, "INTERNAL") == 0)
            {
                ...
                assert(name_info[idx].canonical_name == NULL ||
                       strcmp(name_info[idx].canonical_name, runp->fromname) ==
                           0);
            ...
            else
            {
                ...
                newp = (struct other_conv_list*)xmalloc(
                    sizeof(struct other_conv_list));
                newp->other_conv.module_idx = to_idx;
                newp->other_conv.module = runp;
                newp->other_conv.next =
                    NULL; /* XXX Allow multiple module sequence */
    ...
    for (i = 0; i < nalias_list; ++i)
    {
        ...
        {
            struct name* newp =
                new_name(alias_list[i]->fromname, alias_list[i]->froment);
...
static int write_output(void)
{
    ...
    char tmpfname[(output_file == NULL ? sizeof finalname
                                       : output_file_len + 1) +
                  strlen(".XXXXXX")];
    ...
    if (output_file == NULL)
    {
        ...
        strcpy(
            stpcpy(mempcpy(tmpfname, prefix, prefix_len), GCONV_MODULES_CACHE),
            ".XXXXXX");
    ...
    hash_table =
        (struct hash_entry*)xcalloc(hash_size, sizeof(struct hash_entry));
    /* Fill the hash table.  */
    twalk(names, name_insert);

    /* Create the section for the module list.  */
    module_table =
        (struct module_entry*)xcalloc(sizeof(struct module_entry), nname_info);

    /* Allocate memory for the non-INTERNAL conversions.  The allocated
       memory can be more than is actually needed.  */
    extra_table = (char*)xcalloc(sizeof(struct extra_entry) + sizeof(gidx_t) +
            ...
            do
            {
                ...
                do
                {
                    ...
                    extra->module[extra->module_cnt].outname_offset =
                        runp->next == NULL ? other->dest_idx
                                           : runp->next->module_idx;
    ...
    if ((string_table_size & (sizeof(gidx_t) - 1)) != 0)
    {
        ...
        iov[2].iov_len =
            (sizeof(gidx_t) - (string_table_size & (sizeof(gidx_t) - 1)));
        total += iov[2].iov_len;
        ++idx;
    }
        ...
        || rename(tmpfname, output_file ?: finalname) != 0)
    {
        ...
        close(fd);
        unlink(tmpfname);
        error(EXIT_FAILURE, save_errno, gettext("cannot generate output file"));
    }
