...
static void unexport_all(GDBusObjectManagerServer* manager,
                         gboolean only_manager);

static void g_dbus_object_manager_server_emit_interfaces_added(
    GDBusObjectManagerServer* manager, RegistrationData* data,
    const gchar* const* interfaces, const gchar* object_path);

static void g_dbus_object_manager_server_emit_interfaces_removed(
    GDBusObjectManagerServer* manager, RegistrationData* data,
    const gchar* const* interfaces);

static gboolean g_dbus_object_manager_server_unexport_unlocked(
...
G_DEFINE_TYPE_WITH_CODE(
    GDBusObjectManagerServer, g_dbus_object_manager_server, G_TYPE_OBJECT,
...
static void g_dbus_object_manager_server_finalize(GObject* object)
{
    ...
    if (G_OBJECT_CLASS(g_dbus_object_manager_server_parent_class)->finalize !=
        NULL)
        G_OBJECT_CLASS(g_dbus_object_manager_server_parent_class)
            ->finalize(object);
}

static void g_dbus_object_manager_server_get_property(GObject* object,
                                                      guint prop_id,
                                                      GValue* value,
                                                      GParamSpec* pspec)
{
...
static void g_dbus_object_manager_server_set_property(GObject* object,
                                                      guint prop_id,
                                                      const GValue* value,
                                                      GParamSpec* pspec)
{
    ...
    switch (prop_id)
    {
        ...
        case PROP_OBJECT_PATH:
            ...
            if (g_str_equal(manager->priv->object_path, "/"))
                manager->priv->object_path_ending_in_slash =
                    g_strdup(manager->priv->object_path);
            else
                manager->priv->object_path_ending_in_slash =
                    g_strdup_printf("%s/", manager->priv->object_path);
            break;
...
static void g_dbus_object_manager_server_class_init(
    GDBusObjectManagerServerClass* klass)
{
    ...
    g_object_class_install_property(
        ...
        g_param_spec_object(
            "connection", "Connection", "The connection to export objects on",
    ...
    g_object_class_install_property(
        ...
        g_param_spec_string(
            ...
            "The object path to register the manager object at", NULL,
            G_PARAM_READABLE | G_PARAM_WRITABLE | G_PARAM_CONSTRUCT_ONLY |
                G_PARAM_STATIC_STRINGS));
}

static void g_dbus_object_manager_server_init(GDBusObjectManagerServer* manager)
{
    manager->priv = g_dbus_object_manager_server_get_instance_private(manager);
    g_mutex_init(&manager->priv->lock);
    manager->priv->map_object_path_to_data =
    ...
    g_dbus_object_manager_server_new(const gchar* object_path)
{
    ...
    return G_DBUS_OBJECT_MANAGER_SERVER(g_object_new(
        G_TYPE_DBUS_OBJECT_MANAGER_SERVER, "object-path", object_path, NULL));
}
    ...
    GDBusObjectManagerServer* manager, GDBusConnection* connection)
{
    ...
    manager->priv->connection =
        connection != NULL ? g_object_ref(connection) : NULL;
    if (manager->priv->connection != NULL)
        export_all(manager);
    ...
    GDBusObjectManagerServer* manager)
{
    ...
    g_mutex_lock(&manager->priv->lock);
    ret = manager->priv->connection != NULL
              ? g_object_ref(manager->priv->connection)
              : NULL;
    g_mutex_unlock(&manager->priv->lock);
    return ret;
}
...
static void registration_data_export_interface(
    RegistrationData* data, GDBusInterfaceSkeleton* interface_skeleton,
    const gchar* object_path)
{
    ...
    if (data->manager->priv->connection != NULL)
    {
        ...
        {
            g_warning(
                "%s: Error registering object at %s with interface %s: %s",
                G_STRLOC, object_path, info->name, error->message);
            g_error_free(error);
        }
    }

    g_assert(g_hash_table_lookup(data->map_iface_name_to_iface, info->name) ==
             NULL);
    g_hash_table_insert(data->map_iface_name_to_iface, info->name,
    ...
    if (data->exported)
    {
        ...
        interfaces[1] = NULL;
        g_dbus_object_manager_server_emit_interfaces_added(
            data->manager, data, interfaces, object_path);
    }
}

static void registration_data_unexport_interface(
    RegistrationData* data, GDBusInterfaceSkeleton* interface_skeleton)
{
    GDBusInterfaceInfo* info;
    ...
    g_warn_if_fail(
        g_hash_table_remove(data->map_iface_name_to_iface, info->name));
    ...
    if (data->exported)
    {
        ...
        interfaces[0] = info->name;
        interfaces[1] = NULL;
        g_dbus_object_manager_server_emit_interfaces_removed(data->manager,
                                                             data, interfaces);
    }
}
...
static void on_interface_added(GDBusObject* object, GDBusInterface* interface,
                               gpointer user_data)
{
    ...
    registration_data_export_interface(
        data, G_DBUS_INTERFACE_SKELETON(interface), object_path);
    g_mutex_unlock(&data->manager->priv->lock);
}

static void on_interface_removed(GDBusObject* object, GDBusInterface* interface,
                                 gpointer user_data)
{
    RegistrationData* data = user_data;
    g_mutex_lock(&data->manager->priv->lock);
    registration_data_unexport_interface(data,
...
static void registration_data_free(RegistrationData* data)
{
    ...
    g_signal_handlers_disconnect_by_func(data->object,
                                         G_CALLBACK(on_interface_added), data);
    g_signal_handlers_disconnect_by_func(
        data->object, G_CALLBACK(on_interface_removed), data);
...
static void g_dbus_object_manager_server_export_unlocked(
    ...
    const gchar* object_path)
{
    ...
    g_return_if_fail(g_str_has_prefix(
        object_path, manager->priv->object_path_ending_in_slash));

    interface_names = g_ptr_array_new();

    data = g_hash_table_lookup(manager->priv->map_object_path_to_data,
                               object_path);
    if (data != NULL)
    ...
    data->map_iface_name_to_iface = g_hash_table_new_full(
        g_str_hash, g_str_equal, NULL, (GDestroyNotify)g_object_unref);

    g_signal_connect(object, "interface-added", G_CALLBACK(on_interface_added),
                     data);
    g_signal_connect(object, "interface-removed",
                     G_CALLBACK(on_interface_removed), data);
    ...
    for (l = existing_interfaces; l != NULL; l = l->next)
    {
        ...
        g_ptr_array_add(
            interface_names,
            g_dbus_interface_skeleton_get_info(interface_skeleton)->name);
    }
                                         ...
                                         GDBusObjectSkeleton* object)
{
    ...
    g_dbus_object_manager_server_export_unlocked(
        manager, object, g_dbus_object_get_object_path(G_DBUS_OBJECT(object)));
    g_mutex_unlock(&manager->priv->lock);
}
    ...
    GDBusObjectManagerServer* manager, GDBusObjectSkeleton* object)
{
    ...
    g_return_if_fail(G_IS_DBUS_OBJECT(object));
    g_return_if_fail(g_str_has_prefix(
        orig_object_path, manager->priv->object_path_ending_in_slash));
    ...
    GDBusObjectManagerServer* manager, const gchar* object_path)
{
    ...
    if (data != NULL)
    {
        ...
        g_dbus_object_manager_server_emit_interfaces_removed(
            manager, data, (const gchar* const*)interface_names->pdata);
        g_ptr_array_unref(interface_names);

        g_hash_table_remove(manager->priv->map_object_path_to_data,
                            object_path);
        ret = TRUE;
    }
...
static const GDBusArgInfo* const
    manager_interfaces_added_signal_info_arg_pointers[] = {
        ...
        &manager_interfaces_added_signal_info_arg1, NULL};
...
static const GDBusArgInfo* const
    manager_interfaces_removed_signal_info_arg_pointers[] = {
        ...
        &manager_interfaces_removed_signal_info_arg1, NULL};
...
static const GDBusSignalInfo* const manager_signal_info_pointers[] = {
    ...
    &manager_interfaces_removed_signal_info, NULL};
...
static void manager_method_call(GDBusConnection* connection,
                                ...
                                gpointer user_data)
{
    ...
    if (g_strcmp0(method_name, "GetManagedObjects") == 0)
    {
        ...
        g_hash_table_iter_init(&object_iter,
                               manager->priv->map_object_path_to_data);
        while (g_hash_table_iter_next(&object_iter, NULL, (gpointer)&data))
        {
            ...
            g_variant_builder_init(&interfaces_builder,
                                   G_VARIANT_TYPE("a{sa{sv}}"));
            g_hash_table_iter_init(&interface_iter,
                                   data->map_iface_name_to_iface);
            while (
                g_hash_table_iter_next(&interface_iter, NULL, (gpointer)&iface))
            {
                GVariant* properties =
                    g_dbus_interface_skeleton_get_properties(iface);
                g_variant_builder_add(
                    &interfaces_builder, "{s@a{sv}}",
            ...
            g_variant_builder_add(&array_builder, "{oa{sa{sv}}}",
                                  iter_object_path, &interfaces_builder);
        }

        g_dbus_method_invocation_return_value(
            invocation, g_variant_new("(a{oa{sa{sv}}})", &array_builder));
    }
    else
    {
    ...
    const gchar* const* interfaces, const gchar* object_path)
{
    ...
    for (n = 0; interfaces[n] != NULL; n++)
    {
        ...
        iface =
            g_hash_table_lookup(data->map_iface_name_to_iface, interfaces[n]);
        g_assert(iface != NULL);
        properties = g_dbus_interface_skeleton_get_properties(iface);
        g_variant_builder_add(&array_builder, "{s@a{sv}}", interfaces[n],
                              properties);
        g_variant_unref(properties);
    }
    ...
    g_dbus_connection_emit_signal(
        data->manager->priv->connection, NULL, /* destination_bus_name */
        manager->priv->object_path, manager_interface_info.name,
        "InterfacesAdded",
        g_variant_new("(oa{sa{sv}})", object_path, &array_builder), &error);
    if (error)
    {
        if (!g_error_matches(error, G_IO_ERROR, G_IO_ERROR_CLOSED))
            g_warning("Couldn't emit InterfacesAdded signal: %s",
                      error->message);
        g_error_free(error);
    }
out:;
    ...
    const gchar* const* interfaces)
{
    ...
    g_dbus_connection_emit_signal(
        ...
        manager->priv->object_path, manager_interface_info.name,
        "InterfacesRemoved",
        g_variant_new("(oas)", object_path, &array_builder), &error);
    if (error)
    {
        if (!g_error_matches(error, G_IO_ERROR, G_IO_ERROR_CLOSED))
            g_warning("Couldn't emit InterfacesRemoved signal: %s",
                      error->message);
        g_error_free(error);
    }
out:;
...
static void export_all(GDBusObjectManagerServer* manager)
{
    ...
    manager->priv->manager_reg_id = g_dbus_connection_register_object(
        manager->priv->connection, manager->priv->object_path,
        (GDBusInterfaceInfo*)&manager_interface_info, &manager_interface_vtable,
        manager, NULL, /* user_data_free_func */
        &error);
    if (manager->priv->manager_reg_id == 0)
    {
        g_warning("%s: Error registering manager at %s: %s", G_STRLOC,
                  manager->priv->object_path, error->message);
        g_error_free(error);
    }

    g_hash_table_iter_init(&iter, manager->priv->map_object_path_to_data);
    while (
        g_hash_table_iter_next(&iter, (gpointer)&object_path, (gpointer)&data))
    {
        ...
        {
            g_warn_if_fail(g_dbus_interface_skeleton_get_connection(iface) ==
                           NULL);
            error = NULL;
            if (!g_dbus_interface_skeleton_export(
                    iface, manager->priv->connection, object_path, &error))
            {
                g_warning(
                    "%s: Error registering object at %s with interface %s: %s",
                         ...
                         gboolean only_manager)
{
    ...
    if (manager->priv->manager_reg_id > 0)
    {
        g_warn_if_fail(g_dbus_connection_unregister_object(
            manager->priv->connection, manager->priv->manager_reg_id));
