...
static void g_dbus_object_proxy_finalize(GObject* object)
{
...
static void g_dbus_object_proxy_get_property(GObject* object, guint prop_id,
...
static void g_dbus_object_proxy_set_property(GObject* object, guint prop_id,
                                             const GValue* value,
                                             GParamSpec* pspec)
{
    GDBusObjectProxy* proxy = G_DBUS_OBJECT_PROXY(object);
...
static void g_dbus_object_proxy_class_init(GDBusObjectProxyClass* klass)
{
    ...
    g_object_class_install_property(
        ...
        g_param_spec_string("g-object-path", "Object Path",
                            "The object path of the proxy", NULL,
    ...
    g_object_class_install_property(
        ...
        g_param_spec_object("g-connection", "Connection",
                            "The connection of the proxy",
...
static void g_dbus_object_proxy_init(GDBusObjectProxy* proxy)
{
...
static const gchar* g_dbus_object_proxy_get_object_path(GDBusObject* object)
{
    GDBusObjectProxy* proxy = G_DBUS_OBJECT_PROXY(object);
    const gchar* ret;
    g_mutex_lock(&proxy->priv->lock);
...
static GList* g_dbus_object_proxy_get_interfaces(GDBusObject* object)
{
    GDBusObjectProxy* proxy = G_DBUS_OBJECT_PROXY(object);
    GList* ret;
                                        ...
                                        GDBusProxy* interface_proxy)
{
    ...
    interface_proxy_to_remove =
        g_hash_table_lookup(proxy->priv->map_name_to_iface, interface_name);
    if (interface_proxy_to_remove != NULL)
    {
        g_object_ref(interface_proxy_to_remove);
        g_warn_if_fail(g_hash_table_remove(proxy->priv->map_name_to_iface,
                                           interface_name));
    }
    ...
    if (interface_proxy_to_remove != NULL)
    {
        g_signal_emit_by_name(proxy, "interface-removed",
                              interface_proxy_to_remove);
