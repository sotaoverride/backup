...
static void g_filter_output_stream_set_property(GObject* object, guint prop_id,
                                                const GValue* value,
                                                GParamSpec* pspec);

static void g_filter_output_stream_get_property(GObject* object, guint prop_id,
                                                GValue* value,
                                                GParamSpec* pspec);
...
G_DEFINE_ABSTRACT_TYPE_WITH_PRIVATE(GFilterOutputStream, g_filter_output_stream,
                                           ...
                                           GError** error)
{
    ...
    nwritten = g_output_stream_write(filter_stream->base_stream, buffer, count,
                                     cancellable, error);
                                             ...
                                             GError** error)
{
    ...
    if (priv->close_base)
    {
        res = g_output_stream_close(filter_stream->base_stream, cancellable,
                                    error);
    }
