...
G_DEFINE_BOXED_TYPE(GSrvTarget, g_srv_target, g_srv_target_copy,
                    g_srv_target_free)
 ...
 * Since: 2.22
 */
GSrvTarget* g_srv_target_new(const gchar* hostname, guint16 port,
...
{
    return g_srv_target_new(target->hostname, target->port, target->priority,
                            target->weight);
}
...
static gint compare_target(gconstpointer a, gconstpointer b)
{
