...
int tls_close_construct_packet(SSL* s, WPACKET* pkt, int htype)
{
    ...
    if ((htype != SSL3_MT_CHANGE_CIPHER_SPEC && !WPACKET_close(pkt)) ||
        !WPACKET_get_length(pkt, &msglen) || msglen > INT_MAX)
        return 0;
...
int tls_construct_finished(SSL* s, WPACKET* pkt)
{
     ...
     * client certificate
     */
    if (SSL_IS_TLS13(s) && !s->server && s->s3->tmp.cert_req == 0 &&
...
MSG_PROCESS_RETURN tls_process_key_update(SSL* s, PACKET* pkt)
{
    ...
    if (!PACKET_get_1(pkt, &updatetype) || PACKET_remaining(pkt) != 0)
    {
...
MSG_PROCESS_RETURN tls_process_finished(SSL* s, PACKET* pkt)
{
    ...
    if (CRYPTO_memcmp(PACKET_data(pkt), s->s3->tmp.peer_finish_md, md_len) != 0)
    {
    ...
    if (SSL_IS_TLS13(s))
    {
        if (s->server)
        {
            if (s->post_handshake_auth != SSL_PHA_REQUESTED &&
                !s->method->ssl3_enc->change_cipher_state(
                    s, SSL3_CC_APPLICATION | SSL3_CHANGE_CIPHER_SERVER_READ))
            {
                /* SSLfatal() already called */
                return MSG_PROCESS_ERROR;
            }
        ...
        else
        {
            ...
            if (!s->method->ssl3_enc->change_cipher_state(
                    s, SSL3_CC_APPLICATION | SSL3_CHANGE_CIPHER_CLIENT_READ))
            {
                /* SSLfatal() already called */
                return MSG_PROCESS_ERROR;
            }
...
static int ssl_add_cert_to_wpacket(SSL* s, WPACKET* pkt, X509* x, int chain)
{
    ...
    if (SSL_IS_TLS13(s) &&
        !tls_construct_extensions(s, pkt, SSL_EXT_TLS1_3_CERTIFICATE, x, chain))
    {
        /* SSLfatal() already called */
        return 0;
    }
...
WORK_STATE tls_finish_handshake(SSL* s, WORK_STATE wst, int clearbufs, int stop)
{
    ...
    if (cb != NULL)
    {
        if (cleanuphand || !SSL_IS_TLS13(s) || SSL_IS_FIRST_HANDSHAKE(s))
    ...
    do
    {
        while (s->init_num < SSL3_HM_HEADER_LENGTH)
        {
            i = s->method->ssl_read_bytes(
                s, SSL3_RT_HANDSHAKE, &recvd_type, &p[s->init_num],
                SSL3_HM_HEADER_LENGTH - s->init_num, 0, &readbytes);
            if (i <= 0)
            {
        ...
        if (!s->server)
            if (s->statem.hand_state != TLS_ST_OK &&
                ...
                if (p[1] == 0 && p[2] == 0 && p[3] == 0)
                {
                    ...
                    if (s->msg_callback)
                        s->msg_callback(0, s->version, SSL3_RT_HANDSHAKE, p,
                                        SSL3_HM_HEADER_LENGTH, s,
                                        s->msg_callback_arg);
                }
    } while (skip_message);
    /* s->init_num == SSL3_HM_HEADER_LENGTH */
...
int ssl_version_supported(const SSL* s, int version, const SSL_METHOD** meth)
{
         ...
         ++vent)
    {
        if (vent->cmeth != NULL &&
            ...
            (!s->server || version != TLS1_3_VERSION || is_tls13_capable(s)))
        {
            if (meth != NULL)
                *meth = vent->cmeth();
            return 1;
...
int ssl_set_version_bound(int method_version, int version, int* bound)
{
    ...
    switch (method_version)
    {
        default:
            /*
             * XXX For fixed version methods, should we always fail and not set
             * any bounds, always succeed and not set any bounds, or set the
             * bounds and arrange to fail later if they are not met?  At present
             * fixed-version methods are not subject to controls that disable
    ...
    switch (server_version)
    {
        default:
            if (!SSL_IS_TLS13(s))
            {
                ...
                *dgrd = DOWNGRADE_NONE;
                /*
                 * If this SSL handle is not from a version flexible method we
                 * don't (and never did) check min/max FIPS or Suite B
                 * constraints.  Hope that's OK.  It is up to the caller to not
                 * choose fixed protocol versions they don't want.  If not, then
                 * easy to fix, just return ssl_method_error(s, s->method)
                 */
                return 0;
            }
            /*
             * Fall through if we are TLSv1.3 already (this means we must be
 ...
 * so we don't advertise the wrong protocol version to only reject the outcome
 * later.
 *
                  ...
                  size_t num_groups, int checkallow)
{
    ...
    for (i = 0; i < num_groups; i++)
    {
        ...
        if (group_id == group &&
            (!checkallow || tls_curve_allowed(s, group, SSL_SECOP_CURVE_CHECK)))
        {
            return 1;
        }
    }
                                  ...
                                  size_t hrrlen)
{
    ...
    if (hashval == NULL)
    {
        ...
        if (!ssl3_digest_cached_records(s, 0) ||
            !ssl_handshake_hash(s, hashvaltmp, sizeof(hashvaltmp), &hashlen))
        {
            /* SSLfatal() already called */
            return 0;
        }
    ...
    if (hrr != NULL &&
        (!ssl3_finish_mac(s, hrr, hrrlen) ||
         !ssl3_finish_mac(s, (unsigned char*)s->init_buf->data,
                          s->s3->tmp.message_size + SSL3_HM_HEADER_LENGTH)))
    {
        /* SSLfatal() already called */
        return 0;
    }
    ...
    if (ca_sk != NULL)
    {
        ...
        for (i = 0; i < sk_X509_NAME_num(ca_sk); i++)
        {
            ...
            if (name == NULL || (namelen = i2d_X509_NAME(name, NULL)) < 0 ||
                !WPACKET_sub_allocate_bytes_u16(pkt, namelen, &namebytes) ||
...
int tls13_save_handshake_digest_for_pha(SSL* s)
{
    if (s->pha_dgst == NULL)
    {
        ...
        if (!EVP_MD_CTX_copy_ex(s->pha_dgst, s->s3->handshake_dgst))
        {
            SSLfatal(s, SSL_AD_INTERNAL_ERROR,
                     SSL_F_TLS13_SAVE_HANDSHAKE_DIGEST_FOR_PHA,
                     ERR_R_INTERNAL_ERROR);
...
int tls13_restore_handshake_digest_for_pha(SSL* s)
{
    ...
    if (!EVP_MD_CTX_copy_ex(s->s3->handshake_dgst, s->pha_dgst))
    {
        SSLfatal(s, SSL_AD_INTERNAL_ERROR,
                 SSL_F_TLS13_RESTORE_HANDSHAKE_DIGEST_FOR_PHA,
                 ERR_R_INTERNAL_ERROR);
