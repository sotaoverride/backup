...
//! The function object applies the function object to the bound visitable
//! object and argument
...
struct vistation_invoker
{
    ...
    vistation_invoker(VisitableT& visitable, result_type const& def_val) :
        m_visitable(visitable), m_def_val(def_val)
    {
    }
    ...
    {
        return m_visitable.apply_visitor_or_default(
            binder1st<FunT, ArgT const&>(FunT(), arg), m_def_val);
    }
...
class singular_ref
{
  ...
  public:
    ...
    typename boost::enable_if_c<
        compatibility_traits::is_compatible<value_type, U>::value,
        const U*>::type
    ...
    //! Applies a visitor function object to the referred value
    template <typename VisitorT>
    typename boost::enable_if_c<is_void<typename VisitorT::result_type>::value,
                                bool>::type
    ...
    //! Applies a visitor function object to the referred value
    template <typename VisitorT>
    typename boost::disable_if_c<is_void<typename VisitorT::result_type>::value,
                                 optional<typename VisitorT::result_type>>::type
        apply_visitor_optional(VisitorT visitor) const
    {
    ...
    //! Applies a visitor function object to the referred value or returns a
    //! default value
    template <typename VisitorT, typename DefaultT>
    typename VisitorT::result_type
        apply_visitor_or_default(VisitorT visitor, DefaultT& def_val) const
    {
        if (m_ptr)
            return visitor(*m_ptr);
        else
    ...
    template <typename VisitorT, typename DefaultT>
    typename VisitorT::result_type
        apply_visitor_or_default(VisitorT visitor,
                                 DefaultT const& def_val) const
    {
        if (m_ptr)
            return visitor(*m_ptr);
        else
...
class variant_ref
{
  ...
  protected:
    ...
    //! Initializing constructor
    template <typename U>
    explicit variant_ref(const U* p) BOOST_NOEXCEPT
        : m_ptr(p),
          m_type_idx(mpl::index_of<value_type, U>::type::value)
    {
    }
  ...
  public:
    ...
    typename boost::enable_if_c<
        ...
        get_ptr() const BOOST_NOEXCEPT
    {
        if (m_type_idx == static_cast<unsigned int>(
                              mpl::index_of<value_type, U>::type::value))
  ...
  private:
    ...
    {
        BOOST_ASSERT(m_type_idx <
                     static_cast<unsigned int>(mpl::size<value_type>::value));
        return apply_visitor_dispatch<value_type, VisitorT>::call(
            m_ptr, m_type_idx, visitor);
    }
};
...
struct value_ref_base
{
    typedef typename mpl::eval_if<
        mpl::and_<mpl::is_sequence<T>,
                  mpl::equal_to<mpl::size<T>, mpl::int_<1>>>,
        mpl::front<T>, mpl::identity<T>>::type value_type;

    typedef typename mpl::if_<mpl::is_sequence<value_type>,
                              variant_ref<value_type, TagT>,
                              singular_ref<value_type, TagT>>::type type;
};
 ...
 *
 * The \c value_ref class template provides access to the stored attribute
 * value. It is not a traditional reference wrapper since it may be empty (i.e.
 * refer to no value at all) and it can also refer to values of different types.
 * Therefore its interface and behavior combines features of Boost.Ref,
 * Boost.Optional and Boost.Variant, depending on the use case.
 *
 * The template parameter \c T can be a single type or an MPL sequence of
 * possible types being referred. The reference wrapper will act as either an
 * optional reference or an optional variant of references to the specified
 * types. In any case, the referred values will not be modifiable (i.e. \c
 * value_ref always models a const reference).
 *
 * Template parameter \c TagT is optional. It can be used for customizing the
 * operations on this reference wrapper, such as putting the referred value to
 * log.
 */
template <typename T, typename TagT>
class value_ref : public aux::value_ref_base<T, TagT>::type
{
  ...
  public:
    /*!
     * Default constructor. Creates a reference wrapper that does not refer to a
     * value.
     */
     ...
     * Copy constructor.
     */
    BOOST_DEFAULTED_FUNCTION(value_ref(value_ref const& that), BOOST_NOEXCEPT
                             : base_type(static_cast<base_type const&>(that)){})

    /*!
     * Initializing constructor. Creates a reference wrapper that refers to the
     * specified value.
     */
    template <typename U>
    explicit value_ref(
        U const& val
#ifndef BOOST_LOG_DOXYGEN_PASS
// MSVC-8 can't handle SFINAE in this case properly and often wrongly disables
// this constructor
#if !defined(_MSC_VER) || (_MSC_VER + 0) >= 1500
        ,
        typename boost::enable_if_c<compatibility_traits::BOOST_NESTED_TEMPLATE
                                        is_compatible<value_type, U>::value,
                                    boost::log::aux::sfinae_dummy>::type =
            boost::log::aux::sfinae_dummy()
#endif
#endif
            ) BOOST_NOEXCEPT : base_type(boost::addressof(val))
    {
    }
...
{
    return left.apply_visitor_or_default(
        binder2nd<equal_to, U const&>(equal_to(), right), false);
}

template <typename U, typename T, typename TagT>
inline bool operator==(U const& left, value_ref<T, TagT> const& right)
{
    return right.apply_visitor_or_default(
        binder1st<equal_to, U const&>(equal_to(), left), false);
}

template <typename T1, typename TagT1, typename T2, typename TagT2>
inline bool operator==(value_ref<T1, TagT1> const& left,
                       value_ref<T2, TagT2> const& right)
{
    if (!left && !right)
        return true;
    return left.apply_visitor_or_default(
        aux::vistation_invoker<value_ref<T2, TagT2>, equal_to>(right, false),
        false);
}
...
{
    return left.apply_visitor_or_default(
        binder2nd<not_equal_to, U const&>(not_equal_to(), right), false);
}
...
{
    return right.apply_visitor_or_default(
        binder1st<not_equal_to, U const&>(not_equal_to(), left), false);
}
                       ...
                       value_ref<T2, TagT2> const& right)
{
    if (!left && !right)
        return false;
    return left.apply_visitor_or_default(
        aux::vistation_invoker<value_ref<T2, TagT2>, not_equal_to>(right,
                                                                   false),
        false);
}
...
{
    return left.apply_visitor_or_default(
        binder2nd<less, U const&>(less(), right), false);
}
...
{
    return right.apply_visitor_or_default(
        binder1st<less, U const&>(less(), left), false);
}
...
{
    return left.apply_visitor_or_default(
        aux::vistation_invoker<value_ref<T2, TagT2>, less>(right, false),
        false);
}
...
{
    return left.apply_visitor_or_default(
        binder2nd<greater, U const&>(greater(), right), false);
}
...
{
    return right.apply_visitor_or_default(
        binder1st<greater, U const&>(greater(), left), false);
}
...
{
    return left.apply_visitor_or_default(
        aux::vistation_invoker<value_ref<T2, TagT2>, greater>(right, false),
        false);
}
...
{
    return left.apply_visitor_or_default(
        binder2nd<less_equal, U const&>(less_equal(), right), false);
}
...
{
    return right.apply_visitor_or_default(
        binder1st<less_equal, U const&>(less_equal(), left), false);
}
                       ...
                       value_ref<T2, TagT2> const& right)
{
    if (!left && !right)
        return true;
    return left.apply_visitor_or_default(
        aux::vistation_invoker<value_ref<T2, TagT2>, less_equal>(right, false),
        false);
}
...
{
    return left.apply_visitor_or_default(
        binder2nd<greater_equal, U const&>(greater_equal(), right), false);
}
...
{
    return right.apply_visitor_or_default(
        binder1st<greater_equal, U const&>(greater_equal(), left), false);
}
                       ...
                       value_ref<T2, TagT2> const& right)
{
    if (!left && !right)
        return true;
    return left.apply_visitor_or_default(
        aux::vistation_invoker<value_ref<T2, TagT2>, greater_equal>(right,
                                                                    false),
        false);
}
