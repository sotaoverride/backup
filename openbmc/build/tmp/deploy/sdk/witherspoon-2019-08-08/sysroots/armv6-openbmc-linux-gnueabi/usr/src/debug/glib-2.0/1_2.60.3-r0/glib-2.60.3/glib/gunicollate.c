...
static gsize msc_strxfrm_wrapper(char* string1, const char* string2,
                                 gsize count)
{
    if (!string1 || count <= 0)
    {
...
gint g_utf8_collate(const gchar* str1, const gchar* str2)
{
    ...
    UCCompareTextDefault(kUCCollateStandardOptions, str1_utf16, len1,
                         str2_utf16, len2, NULL, &retval);
    result = retval;
    ...
    {
        gchar* str1_locale =
            g_convert(str1_norm, -1, charset, "UTF-8", NULL, NULL, NULL);
        gchar* str2_locale =
            g_convert(str2_norm, -1, charset, "UTF-8", NULL, NULL, NULL);
...
static gchar* carbon_collate_key_with_collator(const gchar* str, gssize len,
                                               CollatorRef collator)
{
    ...
    ret = UCGetCollationKey(collator, str_utf16, len_utf16, buf_len, &key_len,
                            buf);
    ...
    if (ret == kCollateBufferTooSmall)
    {
        ...
        buf = freeme;
        buf_len = try_len * 2;
        ret = UCGetCollationKey(collator, str_utf16, len_utf16, buf_len,
                                &key_len, buf);
    }
...
gchar* g_utf8_collate_key(const gchar* str, gssize len)
{
    ...
    {
        gchar* str_locale =
            g_convert(str_norm, -1, charset, "UTF-8", NULL, NULL, NULL);
...
gchar* g_utf8_collate_key_for_filename(const gchar* str, gssize len)
{
     ...
     *   file:foo     (file:foo)
     *
     * This has the side-effect of sorting numbers before everything else
     * (except dots), but this is probably OK.
     *
     ...
     * key.
     *
     * To try avoid conflict with any collation key sequence generated by libc
     * we start each switch to a special cased part with a sentinel that
     * hopefully will sort before anything libc will generate.
     */
