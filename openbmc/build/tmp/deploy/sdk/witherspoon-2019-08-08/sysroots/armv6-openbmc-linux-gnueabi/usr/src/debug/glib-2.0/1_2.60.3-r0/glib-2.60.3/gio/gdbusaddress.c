...
gboolean g_dbus_is_address(const gchar* string)
{
    ...
    for (n = 0; a[n] != NULL; n++)
    {
        if (!_g_dbus_address_parse_entry(a[n], NULL, NULL, NULL))
            goto out;
    }
...
static gboolean is_valid_unix(const gchar* address_entry,
                              GHashTable* key_value_pairs, GError** error)
{
    gboolean ret;
    GList* keys;
    GList* l;
    ...
    for (l = keys; l != NULL; l = l->next)
    {
        ...
        else
        {
            g_set_error(error, G_IO_ERROR, G_IO_ERROR_INVALID_ARGUMENT,
                        _("Unsupported key “%s” in address entry “%s”"), key,
                        address_entry);
            goto out;
        }
    }
    ...
    {
        g_set_error(error, G_IO_ERROR, G_IO_ERROR_INVALID_ARGUMENT,
                    _("Address “%s” is invalid (need exactly one of path, "
                      "tmpdir or abstract keys)"),
                    address_entry);
        goto out;
    }
...
static gboolean is_valid_nonce_tcp(const gchar* address_entry,
                                   GHashTable* key_value_pairs, GError** error)
{
    gboolean ret;
    GList* keys;
    ...
    if (family != NULL &&
        !(g_strcmp0(family, "ipv4") == 0 || g_strcmp0(family, "ipv6") == 0))
    {
        g_set_error(
            error, G_IO_ERROR, G_IO_ERROR_INVALID_ARGUMENT,
...
static gboolean is_valid_tcp(const gchar* address_entry,
                             GHashTable* key_value_pairs, GError** error)
{
    gboolean ret;
    GList* keys;
...
gboolean g_dbus_is_supported_address(const gchar* string, GError** error)
{
    ...
    for (n = 0; a[n] != NULL; n++)
    {
        ...
        if (!_g_dbus_address_parse_entry(a[n], &transport_name,
                                         &key_value_pairs, error))
            goto out;
                                     ...
                                     GError** error)
{
    ...
    for (n = 0; kv_pairs != NULL && kv_pairs[n] != NULL; n++)
    {
        ...
        {
            g_set_error(error, G_IO_ERROR, G_IO_ERROR_INVALID_ARGUMENT,
                        _("Error unescaping key or value in Key/Value pair %d, "
                          "“%s”, in address element “%s”"),
                        n, kv_pair, address_entry);
            g_free(key);
            g_free(value);
            goto out;
...
static GIOStream* g_dbus_address_try_connect_one(const gchar* address_entry,
                                                 gchar** out_guid,
 ...
 */
static GIOStream* g_dbus_address_connect(const gchar* address_entry,
                                         ...
                                         GError** error)
{
    ...
    else if (g_strcmp0(transport_name, "unix") == 0)
    {
        ...
        {
            g_set_error(error, G_IO_ERROR, G_IO_ERROR_INVALID_ARGUMENT,
                        _("Error in address “%s” — the unix transport requires "
                          "exactly one of the "
        ...
        {
            connectable =
                G_SOCKET_CONNECTABLE(g_unix_socket_address_new_with_type(
                    abstract, -1, G_UNIX_SOCKET_ADDRESS_ABSTRACT));
        }
        else
        {
            g_assert_not_reached();
             ...
             g_strcmp0(transport_name, "nonce-tcp") == 0)
    {
        const gchar* s;
        const gchar* host;
        glong port;
        ...
        {
            g_set_error(error, G_IO_ERROR, G_IO_ERROR_INVALID_ARGUMENT,
                        _("Error in address “%s” — the host attribute is "
        ...
        {
            g_set_error(error, G_IO_ERROR, G_IO_ERROR_INVALID_ARGUMENT,
                        _("Error in address “%s” — the port attribute is "
                          "missing or malformed"),
                        address_entry);
            goto out;
        }
        ...
        if (is_nonce)
        {
            ...
            {
                g_set_error(error, G_IO_ERROR, G_IO_ERROR_INVALID_ARGUMENT,
                            _("Error in address “%s” — the noncefile attribute "
    ...
    else if (g_strcmp0(address_entry, "autolaunch:") == 0)
    {
        ...
        {
            ret = g_dbus_address_try_connect_one(autolaunch_address, NULL,
                                                 cancellable, error);
    ...
    if (connectable != NULL)
    {
        ...
        /* Disable proxy support to prevent a deadlock on startup, since loading
         * a proxy resolver causes the GIO modules to be loaded, and there will
        ...
        if (nonce_file != NULL)
        {
            ...
            /* be careful to read only 16 bytes - we also check that the file is
             * only 16 bytes long */
            ...
            {
                g_set_error(error, G_IO_ERROR, G_IO_ERROR_INVALID_ARGUMENT,
                            _("Error opening nonce file “%s”: %s"), nonce_file,
                            g_strerror(errsv));
                g_object_unref(ret);
                ret = NULL;
                goto out;
            ...
            if (num_bytes_read != 16)
            {
                if (num_bytes_read == 0)
                {
                    g_set_error(error, G_IO_ERROR, G_IO_ERROR_INVALID_ARGUMENT,
                                ...
                                nonce_file, g_strerror(errsv));
                }
                else
                {
                    g_set_error(error, G_IO_ERROR, G_IO_ERROR_INVALID_ARGUMENT,
                                _("Error reading from nonce file “%s”, "
            ...
            if (!g_output_stream_write_all(g_io_stream_get_output_stream(ret),
                                           nonce_contents, 16, NULL,
                                           cancellable, error))
            {
                g_prefix_error(
                    error,
                    _("Error writing contents of nonce file “%s” to stream:"),
                    nonce_file);
                g_object_unref(ret);
                ret = NULL;
                goto out;
                                                 ...
                                                 GError** error)
{
    ...
    if (!_g_dbus_address_parse_entry(address_entry, &transport_name,
                                     &key_value_pairs, error))
        goto out;

    ret = g_dbus_address_connect(address_entry, transport_name, key_value_pairs,
                                 cancellable, error);
    if (ret == NULL)
        goto out;
...
static void get_stream_thread_func(GTask* task, gpointer source_object,
                                   gpointer task_data,
                                   GCancellable* cancellable)
{
    ...
    stream = g_dbus_address_get_stream_sync(data->address, &data->guid,
                                            cancellable, &error);
    if (stream)
 ...
 * Since: 2.26
 */
void g_dbus_address_get_stream(const gchar* address, GCancellable* cancellable,
 ...
 * g_dbus_address_get_stream().
 * @out_guid: (optional) (out): %NULL or return location to store the GUID
 * extracted from @address, if any.
 * @error: Return location for error or %NULL.
 *
 ...
 * Since: 2.26
 */
GIOStream* g_dbus_address_get_stream_finish(GAsyncResult* res, gchar** out_guid,
                                            GError** error)
{
    GTask* task;
    GetStreamData* data;
                                          ...
                                          GError** error)
{
    ...
    {
        last_error =
            g_error_new_literal(G_IO_ERROR, G_IO_ERROR_INVALID_ARGUMENT,
    ...
    for (n = 0; addr_array != NULL && addr_array[n] != NULL; n++)
    {
        ...
        this_error = NULL;
        ret = g_dbus_address_try_connect_one(addr, out_guid, cancellable,
                                             &this_error);
        if (ret != NULL)
        {
            goto out;
 ...
 */
static gchar* get_session_address_xdg(void)
{
#ifdef G_OS_UNIX
    gchar* ret = NULL;
    gchar* bus;
...
static gchar* get_session_address_dbus_launch(GError** error)
{
    ...
    /* TODO: maybe provide a variable for where to look for the dbus-launch
     * binary? */
    command_line = g_strdup_printf(
        "dbus-launch --autolaunch=%s --binary-syntax --close-stderr",
        machine_id);

    if (G_UNLIKELY(_g_dbus_debug_address()))
    {
        _g_dbus_debug_print_lock();
        g_print("GDBus-debug:Address: Running '%s' to get bus address "
                "(possibly autolaunching)\n",
                command_line);
    ...
    if (!g_spawn_command_line_sync(command_line, &launch_stdout, &launch_stderr,
                                   &exit_status, error))
    {
        goto out;
    }
    ...
    {
        g_prefix_error(error, _("Error spawning command line “%s”: "),
                       command_line);
        goto out;
    }
...
out:
    if (G_UNLIKELY(_g_dbus_debug_address()))
    {
        ...
        if (launch_stdout != NULL)
        {
            s = _g_dbus_hexdump(
                launch_stdout,
                strlen(launch_stdout) + 1 + sizeof(pid_t) + sizeof(long), 2);
            g_print("\n%s", s);
            g_free(s);
        }
        else
...
static void release_mutex(HANDLE mutex)
{
    ReleaseMutex(mutex);
    CloseHandle(mutex);
}

static HANDLE acquire_mutex(const char* mutexname)
{
    HANDLE mutex;
    DWORD res;
...
static gboolean is_mutex_owned(const char* mutexname)
{
    HANDLE mutex;
    gboolean res = FALSE;
...
static HANDLE set_shm(const char* shm_name, const char* value)
{
    HANDLE shared_mem;
    char* shared_data;
...
static gboolean publish_session_bus(const char* address)
{
    HANDLE init_mutex;
...
static void unpublish_session_bus(void)
{
    HANDLE init_mutex;
...
static void wait_console_window(void)
{
    FILE* console = fopen("CONOUT$", "w");

    SetConsoleTitleW(
        L"gdbus-daemon output. Type any character to close this window.");
...
static void idle_timeout_cb(GDBusDaemon* daemon, gpointer user_data)
{
...
static void turn_off_the_starting_cursor(void)
{
    MSG msg;
    BOOL bRet;
...
__declspec(dllexport) void CALLBACK
    g_win32_run_session_bus(HWND hwnd, HINSTANCE hinst, char* cmdline,
                            int nCmdShow);
...
static gchar* get_session_address_dbus_launch(GError** error)
{
    ...
    if (address == NULL)
    {
         ...
         * .\libgio-2.0-0.dll,g_win32_run_session_bus
         *
         ...
         * LoadLibraryExW(".\libgio-2.0-0.dll", NULL,
         * LOAD_WITH_ALTERED_SEARCH_PATH) Actual testing shows that if relative
         * name starts from ".\" the current directory is searched before
         * System32 (win7, win10 1607). So wrong dll would be loaded only if the
        ...
        int gio_path_len = GetModuleFileNameW(_g_io_win32_get_module(),
                                              gio_path, MAX_PATH + 1);
        ...
        if (gio_path_len > 0 && gio_path_len <= MAX_PATH)
        {
            ...
            res = CreateProcessW(rundll_path, args, 0, 0, FALSE,
...
{
    g_set_error(error, G_IO_ERROR, G_IO_ERROR_FAILED,
                _("Cannot determine session bus address (not implemented for "
                  "this OS)"));
    return NULL;
}
                                       ...
                                       GError** error)
{
    ...
    if (G_UNLIKELY(_g_dbus_debug_address()))
    {
        ...
        g_print("GDBus-debug:Address: In g_dbus_address_get_for_bus_sync() for "
                "bus type '%s'\n",
                s);
        g_free(s);
        for (n = 0; n < 3; n++)
    ...
    switch (bus_type)
    {
        ...
        case G_BUS_TYPE_STARTER:
            ...
            {
                ret = g_dbus_address_get_for_bus_sync(
            ...
            {
                ret = g_dbus_address_get_for_bus_sync(
                    G_BUS_TYPE_SYSTEM, cancellable, &local_error);
                goto out;
            }
            else
            {
...
out:
    if (G_UNLIKELY(_g_dbus_debug_address()))
    {
        ...
        if (ret != NULL)
        {
            g_print("GDBus-debug:Address: Returning address '%s' for bus type "
                    "'%s'\n",
                    ret, s);
        }
        else
        {
            g_print("GDBus-debug:Address: Cannot look-up address bus type "
                    "'%s': %s\n",
