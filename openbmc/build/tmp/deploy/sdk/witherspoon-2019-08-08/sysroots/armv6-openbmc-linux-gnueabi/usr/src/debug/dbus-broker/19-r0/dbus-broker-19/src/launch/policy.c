...
static void policy_entries_deinit(PolicyEntries* entries)
{
    ...
    while (
        (record = c_list_first_entry(&entries->recv_list, PolicyRecord, link)))
        policy_record_free(record);
    while (
        (record = c_list_first_entry(&entries->send_list, PolicyRecord, link)))
        policy_record_free(record);
    while (
        (record = c_list_first_entry(&entries->own_list, PolicyRecord, link)))
        policy_record_free(record);
    while ((record =
                c_list_first_entry(&entries->connect_list, PolicyRecord, link)))
        policy_record_free(record);
}
...
void policy_deinit(Policy* policy)
{
    ...
    while ((record =
                c_list_first_entry(&policy->selinux_list, PolicyRecord, link)))
        policy_record_free(record);

    c_rbtree_for_each_entry_safe_postorder_unlink(
        node, t_node, &policy->gid_tree, policy_node) policy_node_free(node);
    c_rbtree_for_each_entry_safe_postorder_unlink(
        node, t_node, &policy->uid_tree, policy_node) policy_node_free(node);
...
static int policy_at_uidgid(CRBTree* tree, PolicyNode** nodep, uint32_t uidgid)
{
    ...
    slot = c_rbtree_find_slot(tree, policy_node_compare, &uidgid, &parent);
    if (slot)
    {
...
static void policy_import_verdict(Policy* policy, PolicyRecord* record,
                                  ConfigNode* cnode)
{
    assert(cnode->parent);
    ...
    record->priority =
        UINT64_MAX / _CONFIG_POLICY_N * cnode->parent->policy.context +
        ++policy->i_priority;
}
...
static int policy_import_connect(Policy* policy, ConfigNode* cnode)
{
    ...
    if ((cnode->allow_deny.user == cnode->allow_deny.group) ||
        cnode->allow_deny.own || cnode->allow_deny.own_prefix ||
        cnode->allow_deny.send_interface || cnode->allow_deny.send_interface ||
        cnode->allow_deny.send_member || cnode->allow_deny.send_error ||
        cnode->allow_deny.send_destination || cnode->allow_deny.send_path ||
        cnode->allow_deny.send_type || cnode->allow_deny.send_broadcast ||
        cnode->allow_deny.send_requested_reply ||
        cnode->allow_deny.recv_interface || cnode->allow_deny.recv_member ||
        cnode->allow_deny.recv_error || cnode->allow_deny.recv_sender ||
        cnode->allow_deny.recv_path || cnode->allow_deny.recv_type ||
        ...
        cnode->parent->policy.context == CONFIG_POLICY_GROUP)
    {
        fprintf(
            stderr,
            "Connection policy not allowed in user/group context in %s +%lu\n",
            cnode->file, cnode->lineno);
        return 0;
    }
...
static int policy_import_own(Policy* policy, ConfigNode* cnode)
{
    ...
    if ((!cnode->allow_deny.own == !cnode->allow_deny.own_prefix) ||
        cnode->allow_deny.user || cnode->allow_deny.group ||
        cnode->allow_deny.send_interface || cnode->allow_deny.send_interface ||
        cnode->allow_deny.send_member || cnode->allow_deny.send_error ||
        cnode->allow_deny.send_destination || cnode->allow_deny.send_path ||
        cnode->allow_deny.send_type || cnode->allow_deny.send_broadcast ||
...
static int policy_import_send(Policy* policy, ConfigNode* cnode)
{
    ...
    {
        if (cnode->type == CONFIG_NODE_DENY &&
            cnode->allow_deny.send_requested_reply == UTIL_TRISTATE_YES)
            fprintf(
                stderr,
                "Policy to deny expected replies in %s +%lu: Explicit policies "
                "on replies and errors are deprecated and ignored\n",
                cnode->file, cnode->lineno);
        else if (cnode->type == CONFIG_NODE_ALLOW &&
                 cnode->allow_deny.send_requested_reply == UTIL_TRISTATE_NO)
            fprintf(
                stderr,
                "Policy to allow unexpected replies in %s +%lu: Explicit "
                "policies on replies and errors are deprecated and ignored\n",
                cnode->file, cnode->lineno);
    ...
    if (cnode->allow_deny.eavesdrop == UTIL_TRISTATE_YES)
    {
        if (cnode->type == CONFIG_NODE_ALLOW)
            /* Ignore the attribute, but keep the rule, it also applies when not
             * eavesdropping. */
            fprintf(stderr,
                    "Policy to allow eavesdropping in %s +%lu: Eavesdropping "
...
static int policy_import_recv(Policy* policy, ConfigNode* cnode)
{
    ...
    if (cnode->allow_deny.user || cnode->allow_deny.group ||
        cnode->allow_deny.own || cnode->allow_deny.own_prefix ||
        cnode->allow_deny.send_interface || cnode->allow_deny.send_member ||
        cnode->allow_deny.send_error || cnode->allow_deny.send_destination ||
        cnode->allow_deny.send_path || cnode->allow_deny.send_type ||
    ...
    {
        if (cnode->type == CONFIG_NODE_DENY &&
            cnode->allow_deny.recv_requested_reply == UTIL_TRISTATE_YES)
            fprintf(
                stderr,
                "Policy to deny expected replies in %s +%lu: Explicit policies "
                "on replies and errors are deprecated and ignored\n",
                cnode->file, cnode->lineno);
                 ...
                 cnode->allow_deny.recv_requested_reply == UTIL_TRISTATE_NO)
            fprintf(
                stderr,
                "Policy to allow unexpected replies in %s +%lu: Explicit "
                "policies on replies and errors are deprecated and ignored\n",
...
static int policy_import_selinux(Policy* policy, ConfigNode* cnode)
{
    ...
    if (!cnode->associate.own || !cnode->associate.context)
    {
        fprintf(stderr, "Invalid policy attribute combination in %s +%lu\n",
                cnode->file, cnode->lineno);
        return 0;
...
int policy_import(Policy* policy, ConfigRoot* root)
{
    ...
    c_list_for_each_entry(i_cnode, &root->node_list, root_link)
    {
        ...
        if (!i_cnode->parent || i_cnode->parent->type != CONFIG_NODE_POLICY ||
        ...
        if (i_cnode->allow_deny.user || i_cnode->allow_deny.group)
        {
            r = policy_import_connect(policy, i_cnode);
            if (r)
                return error_trace(r);
        }
        else if (i_cnode->allow_deny.own || i_cnode->allow_deny.own_prefix)
        {
        ...
        {
            fprintf(stderr,
                    "Invalid policy without defining attributes in %s +%lu\n",
                    i_cnode->file, i_cnode->lineno);
        }
    }
...
static PolicyRecord* policy_list_find_top(CList* list)
{
    ...
    top_record = NULL;
    c_list_for_each_entry(i_record, list,
                          link) if (!top_record ||
                                    i_record->priority > top_record->priority)
        top_record = i_record;
...
static void policy_list_shrink(CList* list, PolicyRecord* keep)
{
    ...
    c_list_for_each_entry_safe(i_record, t_record, list,
                               link) if (i_record != keep)
...
static void policy_optimize_trim(Policy* policy)
{
    ...
    c_rbtree_for_each_entry_safe(
        node, t_node, &policy->uid_tree,
        policy_node) if (policy_entries_is_empty(&node->entries))
        policy_node_free(node);

    c_rbtree_for_each_entry_safe(
        node, t_node, &policy->gid_tree,
        policy_node) if (policy_entries_is_empty(&node->entries))
        policy_node_free(node);
}
...
static int policy_export_connect(Policy* policy, CList* default_list,
                                 CList* specific_list, sd_bus_message* m)
{
...
static int policy_export_own(Policy* policy, CList* list1, CList* list2,
                             sd_bus_message* m)
{
    ...
    if (list1)
    {
        ...
        {
            r = sd_bus_message_append(m, "(btbs)", i_record->verdict,
                                      i_record->priority, i_record->own.prefix,
                                      i_record->own.name);
            if (r < 0)
                return error_origin(r);
        }
...
static int policy_export_xmit(Policy* policy, CList* list1, CList* list2,
                              sd_bus_message* m)
{
    PolicyRecord* i_record;
    int r;
    ...
    if (list1)
    {
        c_list_for_each_entry(i_record, list1, link)
        {
            r = sd_bus_message_append(
                m, "(btssssuutt)", i_record->verdict, i_record->priority,
                i_record->xmit.name, i_record->xmit.path,
                i_record->xmit.interface, i_record->xmit.member,
                i_record->xmit.type, i_record->xmit.broadcast,
                i_record->xmit.min_fds, i_record->xmit.max_fds);
            if (r < 0)
                return error_origin(r);
        }
    }
...
static int policy_export_console(Policy* policy, sd_bus_message* m,
                                 PolicyEntries* entries, uint32_t uid_start,
                                 uint32_t n_uid)
{
    int r;
    ...
    r = sd_bus_message_append(m, "buu", false, uid_start,
                              uid_start + n_uid - 1);
    if (r < 0)
        return error_origin(r);
 ...
 * policy_export() - XXX
 */
int policy_export(Policy* policy, sd_bus_message* m, uint32_t* at_console_uids,
                  size_t n_at_console_uids)
{
    ...
    r = policy_export_connect(policy, &policy->default_entries.connect_list,
                              NULL, m);
    r = r
            ?: policy_export_own(policy, &policy->default_entries.own_list,
                                 NULL, m);
    r = r
            ?: policy_export_xmit(policy, &policy->default_entries.send_list,
                                  NULL, m);
    r = r
            ?: policy_export_xmit(policy, &policy->default_entries.recv_list,
                                  NULL, m);
    if (r)
        return error_trace(r);
    ...
    c_rbtree_for_each_entry(node, &policy->uid_tree, policy_node)
    {
        ...
        r = policy_export_connect(policy, &policy->default_entries.connect_list,
                                  &node->entries.connect_list, m);
        r = r
                ?: policy_export_own(policy, &policy->default_entries.own_list,
                                     &node->entries.own_list, m);
        r = r
                ?: policy_export_xmit(policy,
                                      &policy->default_entries.send_list,
                                      &node->entries.send_list, m);
        r = r
                ?: policy_export_xmit(policy,
                                      &policy->default_entries.recv_list,
                                      &node->entries.recv_list, m);
        if (r)
            return error_trace(r);
...

    {
        ...
        if (n_at_console_uids > 1)
            qsort(at_console_uids, n_at_console_uids, sizeof(*at_console_uids),
                  policy_uid_compare);

        while (n_at_console_uids > 0 &&
               at_console_uids[n_at_console_uids - 1] > (uint32_t)SYSTEMUIDMAX)
            --n_at_console_uids;

        for (size_t i = 0; i < n_at_console_uids; ++i)
        {
            r = policy_export_console(policy, m, &policy->no_console_entries,
                                      next_uid, at_console_uids[i] - next_uid);
            if (r < 0)
                return error_trace(r);

            r = policy_export_console(policy, m, &policy->at_console_entries,
                                      at_console_uids[i], 1);
            if (r < 0)
                return error_trace(r);
        ...
        r = policy_export_console(policy, m, &policy->no_console_entries,
                                  next_uid,
                                  (uint32_t)SYSTEMUIDMAX + 1 - next_uid);
        if (r < 0)
            return error_trace(r);
    }

    r = policy_export_console(policy, m, &policy->at_console_entries,
                              (uint32_t)(SYSTEMUIDMAX + 1),
                              ((uint32_t)-1) - (uint32_t)SYSTEMUIDMAX);
    if (r < 0)
        return error_trace(r);
    ...
    {
        r = sd_bus_message_append(m, "(ss)", i_record->selinux.name,
    ...
    r = sd_bus_message_append(
        m, "b", (policy->apparmor_mode != CONFIG_APPARMOR_DISABLED));
    if (r < 0)
        return error_origin(r);
