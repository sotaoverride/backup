...
struct sequential_composite
{
    ...
    sequential_composite(A0 const& _0, A1 const& _1) : a0(_0), a1(_1)
    {
    }
...
struct if_then_else_composite
{
    ...
    if_then_else_composite(CondT const& cond_, ThenT const& then_,
                           ElseT const& else__) :
        cond(cond_),
        then(then_), else_(else__)
    {
    }
...
struct else_gen
{
    ...
    else_gen(CondT const& cond_, ThenT const& then_) : cond(cond_), then(then_)
    {
    }

    template <typename ElseT>
    actor<if_then_else_composite<CondT, ThenT, typename as_actor<ElseT>::type>>
...
struct if_gen
{
    ...
    if_gen(CondT const& cond_) : cond(cond_)
    {
    }

    template <typename ThenT>
    actor<if_then_composite<typename as_actor<CondT>::type,
                            typename as_actor<ThenT>::type>>
        operator[](ThenT const& then) const
    {
        typedef if_then_composite<typename as_actor<CondT>::type,
                                  typename as_actor<ThenT>::type>
            result;

        return result(as_actor<CondT>::convert(cond),
...
struct while_gen
{
    ...
    while_gen(CondT const& cond_) : cond(cond_)
    {
    }

    template <typename DoT>
    actor<while_composite<typename as_actor<CondT>::type,
                          typename as_actor<DoT>::type>>
        operator[](DoT const& do_) const
    {
        typedef while_composite<typename as_actor<CondT>::type,
                                typename as_actor<DoT>::type>
            result;
...
struct do_composite
{
    ...
    do_composite(DoT const& do__, CondT const& cond_) : do_(do__), cond(cond_)
    {
    }
...
struct do_gen2
{
    ...
    do_gen2(DoT const& do__) : do_(do__)
    {
    }

    template <typename CondT>
    actor<do_composite<typename as_actor<DoT>::type,
                       typename as_actor<CondT>::type>>
        while_(CondT const& cond) const
    {
        typedef do_composite<typename as_actor<DoT>::type,
                             typename as_actor<CondT>::type>
            result;

        return result(as_actor<DoT>::convert(do_),
...
struct for_composite
{
    ...
    for_composite(InitT const& init_, CondT const& cond_, StepT const& step_,
                  DoT const& do__) :
        init(init_),
        cond(cond_), step(step_), do_(do__)
    {
    }
...
struct for_gen
{
    ...
    template <typename DoT>
    actor<for_composite<
        typename as_actor<InitT>::type, typename as_actor<CondT>::type,
        typename as_actor<StepT>::type, typename as_actor<DoT>::type>>
        operator[](DoT const& do_) const
    {
        ...
        return result(
            as_actor<InitT>::convert(init), as_actor<CondT>::convert(cond),
            as_actor<StepT>::convert(step), as_actor<DoT>::convert(do_));
    }
...
//////////////////////////////////
template <typename InitT, typename CondT, typename StepT>
inline for_gen<InitT, CondT, StepT> for_(InitT const& init, CondT const& cond,
                                         StepT const& step)
{
