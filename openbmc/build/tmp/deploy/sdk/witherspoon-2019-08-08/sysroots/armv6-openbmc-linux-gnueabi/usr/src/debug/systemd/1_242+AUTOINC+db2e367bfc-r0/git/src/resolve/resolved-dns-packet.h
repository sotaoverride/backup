...
static inline uint16_t DNS_PACKET_PAYLOAD_SIZE_MAX(DnsPacket* p)
{
    ...
    /* Returns the advertised maximum size for replies, or the DNS default if
     * there's nothing defined. */

    if (p->ipproto == IPPROTO_TCP) /* we ignore EDNS(0) size data on TCP, like
                                      everybody else */
...
{
    /* Returns true if this packet is in a version we support. Which means
     * either non-EDNS or EDNS(0), but not EDNS of any newer versions */
...
#define DNS_PACKET_MAKE_FLAGS(qr, opcode, aa, tc, rd, ra, ad, cd, rcode)       \
    (((uint16_t) !!(qr) << 15) | ((uint16_t)((opcode)&15) << 11) |             \
     ...
     ((uint16_t) !!(ra) << 7) | ((uint16_t) !!(ad) << 5) |                     \
     ((uint16_t) !!(cd) << 4) | ((uint16_t)((rcode)&15)))
...
int dns_packet_new(DnsPacket** p, DnsProtocol protocol, size_t min_alloc_dsize,
                   size_t max_size);
int dns_packet_new_query(DnsPacket** p, DnsProtocol protocol,
...
int dns_packet_append_blob(DnsPacket* p, const void* d, size_t sz,
                           size_t* start);
...
int dns_packet_append_raw_string(DnsPacket* p, const void* s, size_t size,
                                 size_t* start);
int dns_packet_append_label(DnsPacket* p, const char* s, size_t l,
...
int dns_packet_append_name(DnsPacket* p, const char* name,
                           ...
                           size_t* start);
int dns_packet_append_key(DnsPacket* p, const DnsResourceKey* key,
...
enum
{
    ...
    _DNS_RCODE_MAX = 4095 /* 4 bit rcode in the header plus 8 bit rcode in OPT,
                             makes 12 bit */
};
...
#define LLMNR_MULTICAST_IPV4_ADDRESS                                           \
    ((struct in_addr){.s_addr = htobe32(224U << 24 | 252U)})
#define LLMNR_MULTICAST_IPV6_ADDRESS                                           \
    ((struct in6_addr){.s6_addr = {0xFF, 0x02, 0x00, 0x00, 0x00, 0x00, 0x00,   \
                                   0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x01,   \
                                   0x00, 0x03}})

#define MDNS_MULTICAST_IPV4_ADDRESS                                            \
    ((struct in_addr){.s_addr = htobe32(224U << 24 | 251U)})
                                              ...
                                              bool authenticated)
{
    ...
    /* Converts a protocol + family into a flags field as used in queries and
     * responses */
    ...
    switch (protocol)
    {
        ...
        case DNS_PROTOCOL_LLMNR:
            return f | (family == AF_INET6 ? SD_RESOLVED_LLMNR_IPV6
                                           : SD_RESOLVED_LLMNR_IPV4);

        case DNS_PROTOCOL_MDNS:
            return f | (family == AF_INET6 ? SD_RESOLVED_MDNS_IPV6
...
static inline size_t dns_packet_size_max(DnsPacket* p)
{
    ...
    /* Why not insist on a fully initialized max_size during DnsPacket
     * construction? Well, this way it's easy to allocate a transient,
     * throw-away DnsPacket on the stack by simple zero initialization, without
     * having to deal with explicit field initialization. */
