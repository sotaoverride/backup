...
static FT_Error cid_load_keyword(CID_Face face, CID_Loader* loader,
        ...
        T1_FIELD_CALLBACK("FontMatrix", cid_parse_font_matrix, 0)
            T1_FIELD_CALLBACK("ExpansionFactor", parse_expansion_factor, 0)
                T1_FIELD_CALLBACK("FontName", parse_font_name, 0)
                    ...
                    {0, T1_FIELD_LOCATION_CID_INFO, T1_FIELD_TYPE_NONE, 0, 0, 0,
                     0, 0, 0}};

static FT_Error cid_parse_dict(CID_Face face, CID_Loader* loader, FT_Byte* base,
                               FT_ULong size)
{
    CID_Parser* parser = &loader->parser;
                    ...
                    for (;;)
                    {
                            ...
                            len == ft_strlen((const char*)name))
                        {
                            ...
                            if (n >= len)
                            {
                                ...
                                parser->root.error =
                                    cid_load_keyword(face, loader, keyword);
...
static FT_Error cid_hex_to_binary(FT_Byte* data, FT_ULong data_len,
                                  FT_ULong offset, CID_Face face)
{
...
cid_face_open(CID_Face face, FT_Int face_index)
{
    ...
    error = cid_parse_dict(face, &loader, parser->postscript,
    ...
    if (parser->binary_length)
    {
        ...
        FT_Stream_OpenMemory(face->cid_stream, face->binary_data,
                             parser->binary_length);
        cid->data_offset = 0;
    }
    else
    ...
    for (n = 0; n < cid->num_dicts; n++)
    {
        ...
        if (dict->sd_bytes < 0 || (dict->num_subrs && dict->sd_bytes < 1))
        {
    ...
    if (entry_len &&
        cid->cid_count > (binary_length - cid->cidmap_offset) / entry_len)
    {
