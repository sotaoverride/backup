...
void init_file(void)
{
    struct variable2 file_table[] = {
        {FILE_INDEX, ASN_INTEGER, NETSNMP_OLDAPI_RONLY, var_file_table, 1, {1}},
        {FILE_NAME,
         ASN_OCTET_STR,
         ...
         1,
         {2}},
        {FILE_SIZE, ASN_INTEGER, NETSNMP_OLDAPI_RONLY, var_file_table, 1, {3}},
        {FILE_MAX, ASN_INTEGER, NETSNMP_OLDAPI_RONLY, var_file_table, 1, {4}},
        {FILE_ERROR,
         ASN_INTEGER,
         NETSNMP_OLDAPI_RONLY,
         var_file_table,
...
u_char* var_file_table(struct variable* vp, oid* name, size_t* length,
                       int exact, size_t* var_len, WriteMethod** write_method)
{
    ...
    switch (vp->magic)
    {
        ...
        default:
            DEBUGMSGTL(
                ("snmpd", "unknown sub-id %d in var_file_table\n", vp->magic));
    }
