 ...
 */
local void slide_hash(s) deflate_state* s;
{
    unsigned n, m;
    Posf* p;
    uInt wsize = s->w_size;
 ...
 */
local int deflateStateCheck(strm) z_streamp strm;
{
    deflate_state* s;
...
/* ========================================================================= */
int ZEXPORT deflateGetDictionary(strm, dictionary, dictLength) z_streamp strm;
Bytef* dictionary;
uInt* dictLength;
{
    deflate_state* s;
 ...
 */
local unsigned read_buf(strm, buf, size) z_streamp strm;
Bytef* buf;
unsigned size;
{
...
int flush;
{
         ...
         s->strm->avail_in == 0 && left <= have))
    {
        len = MIN(left, have);
        last =
            flush == Z_FINISH && s->strm->avail_in == 0 && len == left ? 1 : 0;
