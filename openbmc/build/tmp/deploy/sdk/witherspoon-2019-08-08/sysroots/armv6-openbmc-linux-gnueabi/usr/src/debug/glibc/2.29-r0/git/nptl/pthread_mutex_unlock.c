...
int attribute_hidden __pthread_mutex_unlock_usercnt(pthread_mutex_t* mutex,
                                                    int decr)
{
...
static int __pthread_mutex_unlock_full(pthread_mutex_t* mutex, int decr)
{
    ...
    switch (PTHREAD_MUTEX_TYPE(mutex))
    {
        ...
        continue_pi_non_robust:
          ...
          private
            = (robust ? PTHREAD_ROBUST_MUTEX_PSHARED(mutex)
                      : PTHREAD_MUTEX_PSHARED(mutex));
            /* Unlock the mutex using a CAS unless there are futex waiters or
           our TID is not the value of __lock anymore, in which case we let the
           ...
           synchronize with acquire MO in lock acquisitions.  */
            ...
            do
            {
                    ...
                    (l != THREAD_GETMEM(THREAD_SELF, tid)))
                {
                    ...
                    INTERNAL_SYSCALL(
                        futex, __err, 2, &mutex->__data.__lock,
                        __lll_private_flag(FUTEX_UNLOCK_PI, private));
                    break;
                }
            } while (!atomic_compare_exchange_weak_release(
                &mutex->__data.__lock, &l, 0));
        ...
        pp:
            ...
            /* Unlock.  Use release MO in the CAS to synchronize with acquire MO
           in lock acquisitions.  */
            int newval;
            ...
            do
            {
                ...
                &mutex->__data.__lock, &oldval, newval));
