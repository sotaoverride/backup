...
static void g_dbus_interface_skeleton_get_property(GObject* object,
...
static void g_dbus_interface_skeleton_set_property(GObject* object,
                                                   guint prop_id,
                                                   const GValue* value,
                                                   GParamSpec* pspec)
{
    ...
    {
        case PROP_G_FLAGS:
            g_dbus_interface_skeleton_set_flags(interface,
...
static gboolean g_dbus_interface_skeleton_g_authorize_method_default(
    GDBusInterfaceSkeleton* interface, GDBusMethodInvocation* invocation)
{
    return TRUE;
}
    ...
    g_dbus_interface_skeleton_class_init(GDBusInterfaceSkeletonClass* klass)
{
    ...
    g_object_class_install_property(
        ...
        g_param_spec_flags(
            "g-flags", "g-flags", "Flags for the interface skeleton",
     ...
     * Since: 2.30
     */
    signals[G_AUTHORIZE_METHOD_SIGNAL] = g_signal_new(
        I_("g-authorize-method"), G_TYPE_DBUS_INTERFACE_SKELETON,
        G_SIGNAL_RUN_LAST,
        G_STRUCT_OFFSET(GDBusInterfaceSkeletonClass, g_authorize_method),
        _g_signal_accumulator_false_handled, NULL, NULL, G_TYPE_BOOLEAN, 1,
        G_TYPE_DBUS_METHOD_INVOCATION);
}

static void g_dbus_interface_skeleton_init(GDBusInterfaceSkeleton* interface)
{
    ...
    g_dbus_interface_skeleton_get_vtable(GDBusInterfaceSkeleton* interface_)
{
    ...
    g_return_val_if_fail(G_IS_DBUS_INTERFACE_SKELETON(interface_), NULL);
    ret =
        G_DBUS_INTERFACE_SKELETON_GET_CLASS(interface_)->get_vtable(interface_);
    g_warn_if_fail(ret != NULL);
    return ret;
}
    ...
    g_dbus_interface_skeleton_get_properties(GDBusInterfaceSkeleton* interface_)
{
    GVariant* ret;
    g_return_val_if_fail(G_IS_DBUS_INTERFACE_SKELETON(interface_), NULL);
    ret = G_DBUS_INTERFACE_SKELETON_GET_CLASS(interface_)
...
static void g_dbus_interface_skeleton_set_object(GDBusInterface* interface_,
                                                 GDBusObject* object)
{
    GDBusInterfaceSkeleton* interface = G_DBUS_INTERFACE_SKELETON(interface_);
    g_mutex_lock(&interface->priv->lock);
    ...
    if (object != NULL)
        g_object_add_weak_pointer(G_OBJECT(interface->priv->object),
                                  (gpointer*)&interface->priv->object);
    g_mutex_unlock(&interface->priv->lock);
}
...
static void dispatch_data_unref(DispatchData* data)
{
    if (g_atomic_int_dec_and_test(&data->ref_count))
        g_slice_free(DispatchData, data);
}

static DispatchData* dispatch_data_ref(DispatchData* data)
{
    g_atomic_int_inc(&data->ref_count);
    return data;
}

static gboolean dispatch_invoke_in_context_func(gpointer user_data)
{
    DispatchData* data = user_data;
    data->method_call_func(
...
static void dispatch_in_thread_func(GTask* task, gpointer source_object,
                                    gpointer task_data,
                                    GCancellable* cancellable)
{
    ...
    if (object != NULL)
    {
        g_signal_emit_by_name(object, "authorize-method", data->interface,
                              data->invocation, &authorized);
    }
    if (authorized)
    {
        g_signal_emit(data->interface, signals[G_AUTHORIZE_METHOD_SIGNAL], 0,
                      data->invocation, &authorized);
    }
    ...
    if (authorized)
    {
        ...
        else
        {
            ...
            g_main_context_invoke_full(
                g_task_get_context(task), g_task_get_priority(task),
                dispatch_invoke_in_context_func, dispatch_data_ref(data),
...
static void g_dbus_interface_method_dispatch_helper(
    GDBusInterfaceSkeleton* interface,
    GDBusInterfaceMethodCallFunc method_call_func,
    GDBusMethodInvocation* invocation)
{
     ...
     *
     *  a) no handler is connected and class handler is not overridden (both
     * interface and object); and b) method calls are not dispatched in a thread
     */
    has_handlers = g_signal_has_handler_pending(
        interface, signals[G_AUTHORIZE_METHOD_SIGNAL], 0, TRUE);
    has_default_class_handler =
        (G_DBUS_INTERFACE_SKELETON_GET_CLASS(interface)->g_authorize_method ==
         g_dbus_interface_skeleton_g_authorize_method_default);
    ...
    if (!emit_authorized_signal)
    {
        if (object != NULL)
            emit_authorized_signal =
                _g_dbus_object_skeleton_has_authorize_method_handlers(
    ...
    if (!emit_authorized_signal && !run_in_thread)
    {
        method_call_func(
            ...
            g_dbus_method_invocation_get_parameters(invocation), invocation,
    ...
    GDBusMethodInvocation* invocation, gpointer user_data)
{
    ...
    g_dbus_interface_method_dispatch_helper(
        interface, g_dbus_interface_skeleton_get_vtable(interface)->method_call,
        invocation);
}
...
static ConnectionData* new_connection(GDBusConnection* connection,
                                      guint registration_id)
{
    ConnectionData* data;
...
static void free_connection(ConnectionData* data)
{
    if (data != NULL)
    {
...
static gboolean add_connection_locked(GDBusInterfaceSkeleton* interface_,
                                      GDBusConnection* connection,
                                      GError** error)
{
    ConnectionData* data;
    ...
    registration_id = g_dbus_connection_register_object(
        connection, interface_->priv->object_path,
        g_dbus_interface_skeleton_get_info(interface_),
        interface_->priv->hooked_vtable, interface_,
    ...
    if (registration_id > 0)
    {
        ...
        interface_->priv->connections =
            g_slist_append(interface_->priv->connections, data);
        ret = TRUE;
    }
    ...
    for (l = interface_->priv->connections; l != NULL; l = l->next)
    {
        ...
        if (data->connection == connection)
        {
            g_warn_if_fail(g_dbus_connection_unregister_object(
                data->connection, data->registration_id));
            free_connection(data);
            interface_->priv->connections =
                g_slist_delete_link(interface_->priv->connections, l);
            /* we are guaranteed that the connection is only added once, so bail
             * out early */
            goto out;
        }
    }
 ...
 *
 * Returns: A string owned by @interface_ or %NULL if @interface_ is not
 ...
 *
 * Returns: %TRUE if the interface was exported on @connection, otherwise %FALSE
 * with
 * @error set.
 *
 * Since: 2.30
                                          ...
                                          GError** error)
{
    ...
    g_return_val_if_fail(
        interface_->priv->object_path == NULL ||
            g_strcmp0(interface_->priv->object_path, object_path) == 0,
        FALSE);
