                                              ...
                                              blk_t* ret_blk)
{
    ...
    if (!b && (flags & BMAP_ALLOC))
    {
        ...
        retval = ext2fs_alloc_block(fs, b, block_buf + fs->blocksize, &b);
        if (retval)
            return retval;
...
static errcode_t extent_bmap(ext2_filsys fs, ext2_ino_t ino,
                             ...
                             ext2_extent_handle_t handle, char* block_buf,
                             int bmap_flags, blk64_t block, int* ret_flags,
                             int* blocks_alloc, blk64_t* phys_blk);
                                       ...
                                       blk64_t lblk, blk64_t* phys_blk)
{
    ...
    for (i = 0; i < EXT2FS_CLUSTER_RATIO(fs); i++)
    {
        if (base_block + i == lblk)
            continue;
        extent_bmap(fs, ino, inode, handle, 0, 0, base_block + i, 0, 0,
                    &pblock);
        if (pblock)
            break;
    }
                             ...
                             int* blocks_alloc, blk64_t* phys_blk)
{
    ...
    {
        retval = ext2fs_extent_set_bmap(handle, block, *phys_blk, set_flags);
        return retval;
    }
...
got_block:
    if ((*phys_blk == 0) && (bmap_flags & BMAP_ALLOC))
    {
        ...
        if (blk64)
            goto set_extent;
        retval = extent_bmap(fs, ino, inode, handle, block_buf, 0, block - 1, 0,
                             blocks_alloc, &blk64);
        if (retval)
        ...
        retval = ext2fs_alloc_block3(fs, blk64, block_buf, &blk64, &alloc_ctx);
        if (retval)
            return retval;
        ...
        alloc++;
    set_extent:
        retval = ext2fs_extent_set_bmap(handle, block, blk64, set_flags);
        if (retval)
        {
...
int ext2fs_file_block_offset_too_big(ext2_filsys fs, struct ext2_inode* inode,
                                     blk64_t offset)
{
                       ...
                       int* ret_flags, blk64_t* phys_blk)
{
    ...
    if (inode->i_flags & EXT4_EXTENTS_FL)
    {
        ...
        if (retval)
            goto done;
        retval = extent_bmap(fs, ino, inode, handle, block_buf, bmap_flags,
                             block, ret_flags, &blocks_alloc, phys_blk);
        goto done;
    }
    ...
    if (block < EXT2_NDIR_BLOCKS)
    {
        ...
        *phys_blk = inode_bmap(inode, block);
        ...
        {
            b64 = b;
            retval = ext2fs_alloc_block3(fs, b64, block_buf, &b64, &alloc_ctx);
            b = b64;
            if (retval)
                goto done;
            inode_bmap(inode, block) = b;
    ...
    if (block < addr_per_block)
    {
        ...
        retval = block_ind_bmap(fs, bmap_flags, b, block_buf, &blocks_alloc,
                                block, &blk32);
        if (retval == 0)
            *phys_blk = blk32;
        goto done;
    ...
    if (block < addr_per_block * addr_per_block)
    {
        ...
        retval = block_dind_bmap(fs, bmap_flags, b, block_buf, &blocks_alloc,
                                 block, &blk32);
        if (retval == 0)
            *phys_blk = blk32;
        goto done;
    ...
    retval = block_tind_bmap(fs, bmap_flags, b, block_buf, &blocks_alloc, block,
                             &blk32);
    if (retval == 0)
        *phys_blk = blk32;
done:
                      ...
                      blk_t* phys_blk)
{
    ...
    ret =
        ext2fs_bmap2(fs, ino, inode, block_buf, bmap_flags, block, 0, &ret_blk);
    if (ret)
        return ret;
