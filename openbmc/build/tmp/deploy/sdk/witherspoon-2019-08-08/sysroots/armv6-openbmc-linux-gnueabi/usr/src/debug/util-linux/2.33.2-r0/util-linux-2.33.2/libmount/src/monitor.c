...
struct monitor_entry
{
    ...
    unsigned int enable : 1, changed : 1;
    ...
    if (mn->refcount <= 0)
    {
        ...
        while (!list_empty(&mn->ents))
        {
            struct monitor_entry* me =
                list_entry(mn->ents.next, struct monitor_entry, ents);
...
static struct monitor_entry* monitor_get_entry(struct libmnt_monitor* mn,
                                               int type)
{
    struct libmnt_iter itr;
    struct monitor_entry* me;
...
static int userspace_add_watch(struct monitor_entry* me, int* final, int* fd)
{
    ...
    {
        DBG(MONITOR,
            ul_debug(" added inotify watch for %s [fd=%d]", filename, wd));
        rc = 0;
        if (final)
            *final = 1;
        if (fd)
...
err:
    ...
    me->fd = -1;
    DBG(MONITOR,
        ul_debugobj(mn, "failed to create userspace monitor [rc=%d]", rc));
    return rc;
}
                                  ...
                                  struct monitor_entry* me)
{
    ...
    do
    {
        ...
        for (p = buf; p < buf + len; p += sizeof(struct inotify_event) + e->len)
        {
            ...
            else
            {
                ...
                {
                    DBG(MONITOR,
                        ul_debugobj(mn, " removing watch [fd=%d]", e->wd));
 ...
 */
int mnt_monitor_enable_userspace(struct libmnt_monitor* mn, int enable,
                                 const char* filename)
{
    struct monitor_entry* me;
    int rc = 0;
    ...
    {
        DBG(MONITOR,
            ul_debugobj(mn, "failed to get userspace mount table path"));
        return -EINVAL;
    }
...
err:
    ...
    free_monitor_entry(me);
    DBG(MONITOR,
        ul_debugobj(mn, "failed to allocate userspace monitor [rc=%d]", rc));
    return rc;
}
...
err:
    rc = -errno;
    DBG(MONITOR,
        ul_debugobj(mn, "failed to create kernel  monitor [rc=%d]", rc));
    return rc;
}
...
err:
    ...
    free_monitor_entry(me);
    DBG(MONITOR,
        ul_debugobj(mn, "failed to allocate kernel monitor [rc=%d]", rc));
    return rc;
}
                                ...
                                struct monitor_entry* me, int enable)
{
    ...
    {
        DBG(MONITOR,
            ul_debugobj(mn, " remove fd=%d (for %s)", me->fd, me->path));
...
int mnt_monitor_get_fd(struct libmnt_monitor* mn)
{
    ...
    DBG(MONITOR,
        ul_debugobj(mn, "adding monitor entries to epoll (fd=%d)", mn->fd));
    while (monitor_next_entry(mn, &itr, &me) == 0)
    {
        if (!me->enable)
            continue;
...
int mnt_monitor_wait(struct libmnt_monitor* mn, int timeout)
{
    ...
    {
        DBG(MONITOR,
            ul_debugobj(mn, "calling epoll_wait(), timeout=%d", timeout));
 ...
 *
 * The function does not wait and it's designed to provide details about
 * changes. It's always recommended to use this function to avoid false
 * positives.
 *
 ...
 */
static int __test_epoll(struct libmnt_test* ts, int argc, char* argv[],
                        int cleanup)
{
    int fd, efd = -1, rc = -1;
...
int main(int argc, char* argv[])
{
    struct libmnt_test tss[] = {
        ...
        {"--epoll-clean", test_epoll_cleanup,
         "<userspace kernel ...>  monitor in epoll and clean events"},
