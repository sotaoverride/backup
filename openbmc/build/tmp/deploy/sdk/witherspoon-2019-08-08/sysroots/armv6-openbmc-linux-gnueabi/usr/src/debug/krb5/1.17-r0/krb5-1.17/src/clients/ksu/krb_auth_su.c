...
krb5_boolean krb5_auth_check(context, client_pname, hostname, options,
                             ...
                             target_uid) krb5_context context;
...
int* path_passwd;
{
    ...
    *path_passwd = 0;
    ...
    retval = krb5_verify_init_creds(context, &tgt, NULL, NULL, NULL, &vfy_opts);
    if (retval)
    {
...
krb5_boolean ksu_get_tgt_via_passwd(context, client, options, zero_password,
                                    creds_out) krb5_context context;
krb5_principal client;
krb5_get_init_creds_opt* options;
krb5_boolean* zero_password;
...
void dump_principal(context, str, p) krb5_context context;
char* str;
krb5_principal p;
{
    char* stname;
...
void plain_dump_principal(context, p) krb5_context context;
krb5_principal p;
{
    char* stname;
    krb5_error_code retval;
...
krb5_error_code get_best_principal(context, plist, client) krb5_context context;
char** plist;
krb5_principal* client;
{
    krb5_error_code retval = 0;
    ...
    while (plist[i])
    {
                    ...
                    *krb5_princ_realm(context, temp_client)))
        {
            ...
            if (nelem && krb5_princ_size(context, *client) > 0 &&
                krb5_princ_size(context, temp_client) > 0)
            {
                krb5_data* p1 = krb5_princ_component(context, *client, 0);
                krb5_data* p2 = krb5_princ_component(context, temp_client, 0);
                ...
                if (data_eq(*p1, *p2))
                {
                    ...
                    if (auth_debug)
                    {
                        fprintf(stderr, "get_best_principal: compare with %s\n",
                                plist[i]);
                    }
