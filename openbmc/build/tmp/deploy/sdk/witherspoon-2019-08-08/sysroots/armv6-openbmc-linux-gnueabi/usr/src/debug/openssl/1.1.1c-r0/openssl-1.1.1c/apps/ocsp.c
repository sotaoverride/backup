...
static void make_ocsp_response(BIO* err, OCSP_RESPONSE** resp,
                               OCSP_REQUEST* req, CA_DB* db,
                               STACK_OF(X509) * ca, X509* rcert, EVP_PKEY* rkey,
                               const EVP_MD* md,
...
int ocsp_main(int argc, char** argv)
{
    ...
    if (req == NULL && reqin == NULL && respin == NULL &&
        !(port != NULL && ridx_filename != NULL))
        goto opthelp;
    ...
    if (rsignfile != NULL)
    {
        ...
        if (!load_certs(rca_filename, &rca_cert, FORMAT_PEM, NULL,
                        "CA certificate"))
            goto end;
        if (rcertfile != NULL)
        {
...
redo_accept:
    ...
    if (req == NULL && (signfile != NULL || reqout != NULL || host != NULL ||
                        add_nonce || ridx_filename != NULL))
    {
    ...
    if (signfile != NULL)
    {
        ...
        key =
            load_key(keyfile, FORMAT_PEM, 0, NULL, NULL, "signer private key");
        if (key == NULL)
            goto end;
    ...
    {
#ifndef OPENSSL_NO_SOCK
        resp = process_responder(req, host, path, port, use_ssl, headers,
                                 req_timeout);
        if (resp == NULL)
            goto end;
#else
...
static int index_changed(CA_DB* rdb)
{
    ...
    if (rdb != NULL && stat(rdb->dbfname, &sb) != -1)
    {
        if (rdb->dbst.st_mtime != sb.st_mtime ||
            ...
            rdb->dbst.st_ino != sb.st_ino || rdb->dbst.st_dev != sb.st_dev)
        {
                               ...
                               int nmin, int ndays, int badsig)
{
    ...
    for (i = 0; i < id_count; i++)
    {
        ...
        {
            *resp =
                OCSP_response_create(OCSP_RESPONSE_STATUS_INTERNALERROR, NULL);
            goto end;
        }
        ...
        if (!found)
        {
            OCSP_basic_add1_status(bs, cid, V_OCSP_CERTSTATUS_UNKNOWN, 0, NULL,
                                   thisupd, nextupd);
            continue;
        }
        ...
        else if (inf[DB_type][0] == DB_TYPE_REV)
        {
            ...
            single = OCSP_basic_add1_status(bs, cid, V_OCSP_CERTSTATUS_REVOKED,
                                            reason, revtm, thisupd, nextupd);
            if (invtm != NULL)
                OCSP_SINGLERESP_add1_ext_i2d(single, NID_invalidity_date, invtm,
                                             0, 0);
            else if (inst != NULL)
                OCSP_SINGLERESP_add1_ext_i2d(single, NID_hold_instruction_code,
                                             inst, 0, 0);
                        ...
                        int timeout)
{
    ...
    *pcbio = cbio;
    ...
    if (strncmp(reqbuf, "GET ", 4) == 0)
    {
        ...
        {
            log_message(LOG_INFO, "Invalid request -- bad HTTP version: %s",
                        client);
            goto out;
        }
        *q = '\0';
        ...
        if (len <= 0)
        {
            log_message(LOG_INFO, "Invalid request -- bad URL encoding: %s",
                        client);
            goto out;
        }
