...
void KRB5_CALLCONV krb5_prepend_error_message(krb5_context ctx,
                                              krb5_error_code code,
                                              const char* fmt, ...)
{
    va_list ap;
...
void KRB5_CALLCONV krb5_wrap_error_message(krb5_context ctx,
                                           ...
                                           krb5_context src_ctx)
{
    ...
    {
        k5_set_error(&dest_ctx->err, src_ctx->err.code, "%s", src_ctx->err.msg);
    }
    else
    {
...
static char* err_fmt_fmt(const char* err_fmt, long code, const char* msg)
{
    struct k5buf buf;
    const char *p, *s;
