...
#define GET_AMPM(d)                                                            \
    ((g_date_time_get_hour(d) < 12) ? nl_langinfo(AM_STR) : nl_langinfo(PM_STR))
...
#define WEEKDAY_ABBR(d)                                                        \
    nl_langinfo(weekday_item[0][g_date_time_get_day_of_week(d) - 1])
#define WEEKDAY_ABBR_IS_LOCALE TRUE
#define WEEKDAY_FULL(d)                                                        \
    nl_langinfo(weekday_item[1][g_date_time_get_day_of_week(d) - 1])
...
static const gchar* get_month_name_standalone(gint month)
{
    switch (month)
    {
        case 1:
            /* Translators: Some languages (Baltic, Slavic, Greek, and some
             * more) need different grammatical forms of month names depending
             * on whether they are standalone or in a complete date context,
             * with the day number.  Some other languages may prefer starting
             * with uppercase when they are standalone and with lowercase when
             * they are in a complete date context.  Here are full month names
             * in a form appropriate when they are used standalone.  If your
             * system is Linux with the glibc version 2.27 (released Feb 1,
             * 2018) or newer or if it is from the BSD family (which includes OS
             * X) then you can refer to the date command line utility and see
             * what the command `date +%OB' produces.  Also in the latest Linux
             * the command `locale alt_mon' in your native locale produces a
             * complete list of month names almost ready to copy and paste here.
             * Note that in most of the languages (western European,
...
static const gchar* get_month_name_abbr_standalone(gint month)
{
    switch (month)
    {
        case 1:
            /* Translators: Some languages need different grammatical forms of
             * month names depending on whether they are standalone or in a
             * complete date context, with the day number.  Some may prefer
             * starting with uppercase when they are standalone and with
             * lowercase when they are in a full date context.  However, as
             * these names are abbreviated the grammatical difference is visible
             * probably only in Belarusian and Russian.  In other languages
             * there is no difference between the standalone and complete date
             * form when they are abbreviated. If your system is Linux with the
             * glibc version 2.27 (released Feb 1, 2018) or newer then you can
             * refer to the date command line utility and see what the command
             * `date +%Ob' produces.  Also in the latest Linux the command
             * `locale ab_alt_mon' in your native locale produces a complete
             * list of month names almost ready to copy and paste here.  Note
             * that this feature is not yet supported by any other platform.
             * Here are abbreviated month names in a form appropriate when they
             * are used standalone.
             */
...
#define MONTH_FULL_STANDALONE(d)                                               \
    nl_langinfo(alt_month_item[g_date_time_get_month(d) - 1])
...
#define MONTH_FULL_WITH_DAY(d)                                                 \
    (get_month_name_with_day(g_date_time_get_month(d)))
#define MONTH_FULL_WITH_DAY_IS_LOCALE FALSE

static const gchar* get_month_name_with_day(gint month)
{
    switch (month)
    {
        case 1:
             ...
             * uppercase when they are standalone and with lowercase when they
             * are in a full date context.  Here are full month names in a form
             ...
             * (western European, non-European) there is no difference between
             * the standalone and complete date form.
             */
...
#define MONTH_ABBR_STANDALONE(d)                                               \
    nl_langinfo(ab_alt_month_item[g_date_time_get_month(d) - 1])
...
#define MONTH_ABBR_WITH_DAY(d)                                                 \
    (get_month_name_abbr_with_day(g_date_time_get_month(d)))
#define MONTH_ABBR_WITH_DAY_IS_LOCALE FALSE

static const gchar* get_month_name_abbr_with_day(gint month)
{
    switch (month)
    {
        case 1:
             ...
             * date context, with the day number.  Some may prefer starting with
             * uppercase when they are standalone and with lowercase when they
             * are in a full date context.  Here are abbreviated month names in
             * a form appropriate when they are used in a full date context,
             * with the day number.  However, as these names are abbreviated the
             * grammatical difference is visible probably only in Belarusian and
             * Russian. In other languages there is no difference between the
             * standalone and complete date form when they are abbreviated.  If
             * your system is Linux with the glibc version 2.27 (released Feb 1,
             * 2018) or newer then you can refer to the date command line
             * utility and see what the command `date +%b' produces.  Also in
             * the latest Linux the command `locale abmon' in your native locale
             * produces a complete list of month names almost ready to copy and
             * paste here.  In other systems due to a bug the result is
...
static const gchar* get_fallback_ampm(gint hour)
{
    if (hour < 12)
...
static void g_date_time_get_week_number(GDateTime* datetime, gint* week_number,
                                        gint* day_of_week, gint* day_of_year)
{
...
static GDateTime* g_date_time_from_instant(GTimeZone* tz, gint64 instant)
{
    ...
    datetime->interval = g_time_zone_find_interval(tz, G_TIME_TYPE_UNIVERSAL,
...
static gboolean g_date_time_deal_with_date_change(GDateTime* datetime)
{
    ...
    datetime->interval =
        g_time_zone_adjust_time(datetime->tz, was_dst, &full_time);
    full_time += UNIX_EPOCH_START * SEC_PER_DAY;
    full_time *= USEC_PER_SECOND;
    full_time += usec;
 ...
 * Since: 2.26
 **/
static GDateTime* g_date_time_new_from_timeval(GTimeZone* tz,
                                               const GTimeVal* tv)
{
    ...
    return g_date_time_from_instant(tz,
                                    tv->tv_usec + UNIX_TO_INSTANT(tv->tv_sec));
}
...
/* Parse integers in the form d (week days), dd (hours etc), ddd (ordinal days)
 * or dddd (years) */
...
static gboolean get_iso8601_seconds(const gchar* text, gsize length,
                                    gdouble* value)
{
    gsize i;
...
static GDateTime* g_date_time_new_ordinal(GTimeZone* tz, gint year,
                                          gint ordinal_day, gint hour,
...
static GDateTime* g_date_time_new_week(GTimeZone* tz, gint year, gint week,
                                       gint week_day, gint hour, gint minute,
                                       gdouble seconds)
{
    gint64 p;
    ...
    return g_date_time_new_ordinal(tz, year, ordinal_day, hour, minute,
                                   seconds);
}

static GDateTime* parse_iso8601_date(const gchar* text, gsize length, gint hour,
                                     gint minute, gdouble seconds,
                                     GTimeZone* tz)
{
    /* YYYY-MM-DD */
    ...
    else if (length == 10 && text[4] == '-' && text[5] == 'W' && text[8] == '-')
    {
        ...
        if (!get_iso8601_int(text, 4, &year) ||
            ...
            return NULL;
        return g_date_time_new_week(tz, year, week, week_day, hour, minute,
                                    seconds);
    }
    /* YYYYWwwD */
...
static GTimeZone* parse_iso8601_timezone(const gchar* text, gsize length,
                                         gssize* tz_offset)
{
    ...
    *tz_offset = i;
    ...
    /* Double-check that the GTimeZone matches our interpretation of the
     * timezone. Failure would indicate a bug either here of in the GTimeZone
     * code. */
    g_assert(g_time_zone_get_offset(tz, 0) ==
             offset_sign * (offset_hours * 3600 + offset_minutes * 60));

    return tz;
}

static gboolean parse_iso8601_time(const gchar* text, gsize length, gint* hour,
                                   gint* minute, gdouble* seconds,
                                   GTimeZone** tz)
{
    gssize tz_offset = -1;
                                        ...
                                        GTimeZone* default_tz)
{
    ...
    {
        if (date_length < 0 &&
            (text[length] == 'T' || text[length] == 't' || text[length] == ' '))
            date_length = length;
    }
    ...
    datetime = parse_iso8601_date(text, date_length, hour, minute, seconds,
                                  tz ? tz : default_tz);
                           ...
                           gint hour, gint minute, gdouble seconds)
{
    ...
    if (year < 1 || year > 9999 || month < 1 || month > 12 || day < 1 ||
        day > days_in_months[GREGORIAN_LEAP(year)][month] || hour < 0 ||
        hour > 23 || minute < 0 || minute > 59 || seconds < 0.0 ||
        seconds >= 60.0)
        return NULL;

    datetime = g_date_time_alloc(tz);
    datetime->days = ymd_to_days(year, month, day);
    datetime->usec = (hour * USEC_PER_HOUR) + (minute * USEC_PER_MINUTE) +
                     (gint64)(seconds * USEC_PER_SECOND);

    full_time =
        SEC_PER_DAY * (ymd_to_days(year, month, day) - UNIX_EPOCH_START) +
        SECS_PER_HOUR * hour + SECS_PER_MINUTE * minute + (int)seconds;

    datetime->interval =
        g_time_zone_adjust_time(datetime->tz, G_TIME_TYPE_STANDARD, &full_time);
 ...
 * Since: 2.26
 **/
GDateTime* g_date_time_new_local(gint year, gint month, gint day, gint hour,
                                 gint minute, gdouble seconds)
{
    GDateTime* datetime;
    GTimeZone* local;
 ...
 * Since: 2.26
 **/
GDateTime* g_date_time_new_utc(gint year, gint month, gint day, gint hour,
                               gint minute, gdouble seconds)
{
    GDateTime* datetime;
    GTimeZone* utc;
...
{
    return g_date_time_from_instant(
        datetime->tz, timespan + g_date_time_to_instant(datetime));
}
 ...
 * Since: 2.26
 */
GDateTime* g_date_time_add_full(GDateTime* datetime, gint years, gint months,
                                ...
                                gdouble seconds)
{
    ...
    full_time =
        ...
        SEC_PER_DAY * (ymd_to_days(year, month, day) + days - UNIX_EPOCH_START);

    interval = g_time_zone_adjust_time(
        datetime->tz, g_time_zone_is_dst(datetime->tz, datetime->interval),
        &full_time);
    ...
    full_time += (hours * USEC_PER_HOUR) + (minutes * USEC_PER_MINUTE) +
                 (gint64)(seconds * USEC_PER_SECOND);

    /* find the new interval */
    interval = g_time_zone_find_interval(datetime->tz, G_TIME_TYPE_UNIVERSAL,
                                         INSTANT_TO_UNIX(full_time));

    /* convert back into local time */
    full_time +=
        USEC_PER_SECOND * g_time_zone_get_offset(datetime->tz, interval);
 ...
 * g_date_time_get_ymd:
 * @datetime: a #GDateTime.
 * @year: (out) (optional): the return location for the gregorian year, or
 *%NULL.
 * @month: (out) (optional): the return location for the month of the year, or
 *%NULL.
 * @day: (out) (optional): the return location for the day of the month, or
 *%NULL.
 *
 ...
 * Since: 2.26
 **/
void g_date_time_get_ymd(GDateTime* datetime, gint* year, gint* month,
                         gint* day)
{
    gint the_year;
    gint the_month;
    ...
    /*
     * We need to convert an offset in days to its year/month/day
     * representation. Leap years makes this a little trickier than it should
     * be, so we use 400, 100 and 4 years cycles here to get to the correct
     * year.
     */
    ...
    if (y1_cycles == 4 || y100_cycles == 4)
    {
        ...
        /* special case that indicates that the date is actually one year
         * before, in the 31th of December */
        the_year--;
        the_month = 12;
        the_day = 31;
        goto end;
    ...
    if (preceding > remaining_days)
    {
        ...
        the_month -= 1;
        preceding -=
            leap ? days_in_months[1][the_month] : days_in_months[0][the_month];
    }
...
static gboolean format_z(GString* outstr, gint offset, guint colons)
{
    gint hours;
    gint minutes;
    gint seconds;
    ...
    {
        case 0:
            g_string_append_printf(outstr, "%c%02d%02d", sign, hours, minutes);
            break;
        ...
        case 2:
            g_string_append_printf(outstr, "%c%02d:%02d:%02d", sign, hours,
                                   minutes, seconds);
            break;
 ...
 */
static const gchar* const* initialize_alt_digits(void)
{
    guint i;
    gsize digit_len;
    gchar* digit;
...
#define MAX_UTF8_ENCODING_LEN 4
    static gchar
        buffer[N_DIGITS * (MAX_UTF8_ENCODING_LEN + 1 /* null separator */)];
#undef N_DIGITS
...
static void format_number(GString* str, gboolean use_alt_digits,
                          const gchar* pad, gint width, guint32 number)
{
    ...
    if (use_alt_digits)
    {
        ...
        if
            G_UNLIKELY(g_once_init_enter(&initialised))
            {
...
static gboolean format_ampm(GDateTime* datetime, GString* outstr,
                            gboolean locale_is_utf8, gboolean uppercase)
{
    const gchar* ampm;
...
static gboolean g_date_time_format_utf8(GDateTime* datetime,
                                        const gchar* format, GString* outstr,
                                          ...
                                          gboolean locale_is_utf8)
{
    ...
    if (locale_is_utf8)
        return g_date_time_format_utf8(datetime, locale_format, outstr,
                                       locale_is_utf8);
    ...
    success =
        g_date_time_format_utf8(datetime, utf8_format, outstr, locale_is_utf8);
    g_free(utf8_format);
    return success;
}

static inline gboolean string_append(GString* string, const gchar* s,
                                     gboolean s_is_utf8)
{
    gchar* utf8;
    gsize utf8_len;
        ...
        switch (c)
        {
            ...
            case 'c':
            {
                if (g_strcmp0(PREFERRED_DATE_TIME_FMT, "") == 0)
                    return FALSE;
                if (!g_date_time_format_locale(datetime,
                                               PREFERRED_DATE_TIME_FMT, outstr,
                                               locale_is_utf8))
                    return FALSE;
            }
            break;
            ...
            case 'r':
            {
                if (g_strcmp0(PREFERRED_12HR_TIME_FMT, "") == 0)
                    return FALSE;
                if (!g_date_time_format_locale(datetime,
                                               PREFERRED_12HR_TIME_FMT, outstr,
                                               locale_is_utf8))
                    return FALSE;
            }
            break;
...
gchar* g_date_time_format(GDateTime* datetime, const gchar* format)
{
     ...
     * ASCII. */
    gboolean locale_is_utf8_compatible =
        g_get_charset(&charset) || g_strcmp0("ASCII", charset) == 0 ||
