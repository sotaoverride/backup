...
static int save_dev(blkid_dev dev, FILE* file)
{
    ...
    DBG(SAVE, ul_debug("device %s, type %s", dev->bid_name,
...
int blkid_flush_cache(blkid_cache cache)
{
    ...
    if (strncmp(filename, BLKID_RUNTIME_DIR "/", sizeof(BLKID_RUNTIME_DIR)) ==
        0)
    {

        /* default destination, create the directory if necessary */
        if (stat(BLKID_RUNTIME_DIR, &st) && errno == ENOENT &&
            mkdir(BLKID_RUNTIME_DIR, S_IWUSR | S_IRUSR | S_IRGRP | S_IROTH |
    ...
    DBG(SAVE, ul_debug("writing cache file %s (really %s)", filename, opened));
    ...
    if (opened != filename)
    {
        ...
        else
        {
            ...
            if (backup)
            {
                ...
                {
                    DBG(SAVE,
                        ul_debug("can't link %s to %s", filename, backup));
                }
                free(backup);
            }
            ...
            {
                ret = errno;
                DBG(SAVE, ul_debug("can't rename %s to %s", opened, filename));
            }
            else
            {
