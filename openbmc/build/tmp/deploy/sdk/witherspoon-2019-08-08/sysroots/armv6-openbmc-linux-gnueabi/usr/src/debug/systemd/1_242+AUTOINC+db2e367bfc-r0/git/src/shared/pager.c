...
static int no_quit_on_interrupt(int exe_name_fd, const char* less_opts)
{
    ...
    log_debug(
        "Pager executable is \"%s\", options \"%s\", quit_on_interrupt: %s",
...
int pager_open(PagerFlags flags)
{
    ...
    if (pager)
    {
        ...
        if (strv_isempty(pager_args) ||
            strv_equal(pager_args, STRV_MAKE("cat")))
            return 0;
    }

    /* Determine and cache number of columns/lines before we spawn the pager so
     * that we get the value from the actual tty */
    (void)columns();
    (void)lines();
    ...
    /* This is a pipe to feed the name of the executed pager binary into the
     * parent */
    ...
    if (r == 0)
    {
        ...
        {
            log_error_errno(
                errno, "Failed to set environment variable LESSCHARSET: %m");
            _exit(EXIT_FAILURE);
        }
        ...
        if (pager_args)
        {
            ...
            log_full_errno(errno == ENOENT ? LOG_DEBUG : LOG_WARNING, errno,
                           "Failed to execute '%s', using fallback pagers: %m",
                           pager_args[0]);
        }
        ...
        FOREACH_STRING(exe, "pager", "less", "more")
        {
            ...
            execlp(exe, exe, NULL);
            log_full_errno(
                errno == ENOENT ? LOG_DEBUG : LOG_WARNING, errno,
                "Failed to execute '%s', using next fallback pager: %m", exe);
        }

        r = loop_write(exe_name_pipe[1], "(built-in)", strlen("(built-in") + 1,
                       false);
        if (r < 0)
        {
            log_error_errno(r, "Failed to write pager name to socket: %m");
