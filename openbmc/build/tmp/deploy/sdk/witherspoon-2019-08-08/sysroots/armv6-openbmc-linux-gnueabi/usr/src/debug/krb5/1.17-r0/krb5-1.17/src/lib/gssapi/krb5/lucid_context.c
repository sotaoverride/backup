 ...
 */
static void free_external_lucid_ctx_v1(gss_krb5_lucid_context_v1_t* ctx);

static void free_lucid_key_data(gss_krb5_lucid_key_t* key);

static krb5_error_code copy_keyblock_to_lucid_key(krb5_keyblock* k5key,
                                                  gss_krb5_lucid_key_t* lkey);

static krb5_error_code make_external_lucid_ctx_v1(krb5_gss_ctx_id_rec* gctx,
                                                  int version, void** out_ptr);
    ...
    const gss_OID desired_object, gss_buffer_set_t* data_set)
{
    ...
    *data_set = GSS_C_NO_BUFFER_SET;
    ...
    retval = generic_gss_oid_decompose(
    ...
    switch (version)
    {
        case 1:
            kret =
                make_external_lucid_ctx_v1((krb5_pointer)ctx, version, &lctx);
            break;
        default:
 ...
 */
OM_uint32 gss_krb5int_free_lucid_sec_context(OM_uint32* minor_status,
                                                  ...
                                                  int version, void** out_ptr)
{
    ...
    else if (gctx->proto == 1)
    {
        ...
        {
            if ((retval = copy_keyblock_to_lucid_key(
                     &gctx->acceptor_subkey->keyblock,
