...
DnsResourceKey* dns_resource_key_new(uint16_t class, uint16_t type,
                                     const char* name)
{
    DnsResourceKey* k;
    size_t l;
...
DnsResourceKey* dns_resource_key_new_redirect(const DnsResourceKey* key,
                                              const DnsResourceRecord* cname)
{
    int r;
...
int dns_resource_key_new_append_suffix(DnsResourceKey** ret,
                                       DnsResourceKey* key, char* name)
{
...
DnsResourceKey* dns_resource_key_new_consume(uint16_t class, uint16_t type,
                                             char* name)
{
    DnsResourceKey* k;
...
bool dns_resource_key_is_address(const DnsResourceKey* key)
{
    ...
    return key->class == DNS_CLASS_IN &&
           IN_SET(key->type, DNS_TYPE_A, DNS_TYPE_AAAA);
}
...
int dns_resource_key_match_rr(const DnsResourceKey* key, DnsResourceRecord* rr,
                              const char* search_domain)
{
    ...
    r = dns_name_equal(dns_resource_key_name(rr->key),
    ...
    if (search_domain)
    {
        ...
        r = dns_name_concat(dns_resource_key_name(key), search_domain, 0,
                            &joined);
        if (r < 0)
            return r;
                                          ...
                                          const char* search_domain)
{
    ...
    if (cname->type == DNS_TYPE_CNAME)
        ...
        r = dns_name_endswith(dns_resource_key_name(key),
                              dns_resource_key_name(cname));
    else
        return 0;
...
int dns_resource_key_match_soa(const DnsResourceKey* key,
                               const DnsResourceKey* soa)
{
    assert(soa);
    assert(key);
    ...
    return dns_name_endswith(dns_resource_key_name(key),
                             dns_resource_key_name(soa));
}

static void dns_resource_key_hash_func(const DnsResourceKey* k,
...
static int dns_resource_key_compare_func(const DnsResourceKey* x,
                                         const DnsResourceKey* y)
{
    int ret;

    ret = dns_name_compare_func(dns_resource_key_name(x),
...
DEFINE_HASH_OPS(dns_resource_key_hash_ops, DnsResourceKey,
                dns_resource_key_hash_func, dns_resource_key_compare_func);

char* dns_resource_key_to_string(const DnsResourceKey* key, char* buf,
                                 size_t buf_size)
{
    const char *c, *t;
    char* ans = buf;
    ...
    snprintf(buf, buf_size, "%s %s%s%.0u %s%s%.0u", dns_resource_key_name(key),
             strempty(c), c ? "" : "CLASS", c ? 0 : key->class, strempty(t),
             t ? "" : "TYPE", t ? 0 : key->type);
...
bool dns_resource_key_reduce(DnsResourceKey** a, DnsResourceKey** b)
{
    ...
    /* Try to replace one RR key by another if they are identical, thus saving a
     * bit of memory. Note that we do this only for RR keys, not for RRs
     * themselves, as they carry a lot of additional metadata (where they come
     * from, validity data, and suchlike), and cannot be replaced so easily by
     * other RRs that have the same superficial data. */
...
DnsResourceRecord* dns_resource_record_new_full(uint16_t class, uint16_t type,
                                                const char* name)
{
...
DEFINE_TRIVIAL_REF_UNREF_FUNC(DnsResourceRecord, dns_resource_record,
                              dns_resource_record_free);

int dns_resource_record_new_reverse(DnsResourceRecord** ret, int family,
...
int dns_resource_record_new_address(DnsResourceRecord** ret, int family,
                                    const union in_addr_union* address,
                                    const char* name)
{
    DnsResourceRecord* rr;
...
int dns_resource_record_payload_equal(const DnsResourceRecord* a,
    ...
    switch (a->unparseable ? _DNS_TYPE_INVALID : a->key->type)
    {
        ...
        case DNS_TYPE_SRV:
            ...
            return a->srv.priority == b->srv.priority &&
                   a->srv.weight == b->srv.weight && a->srv.port == b->srv.port;
        ...
        case DNS_TYPE_A:
            return memcmp(&a->a.in_addr, &b->a.in_addr,
                          sizeof(struct in_addr)) == 0;

        case DNS_TYPE_AAAA:
            return memcmp(&a->aaaa.in6_addr, &b->aaaa.in6_addr,
        ...
        case DNS_TYPE_NSEC:
            return dns_name_equal(a->nsec.next_domain_name,
...
int dns_resource_record_equal(const DnsResourceRecord* a,
                              const DnsResourceRecord* b)
{
    int r;
...
static char* format_location(uint32_t latitude, uint32_t longitude,
                             uint32_t altitude, uint8_t size, uint8_t horiz_pre,
                             uint8_t vert_pre)
{
    char* s;
    ...
    int lat = latitude >= 1U << 31 ? (int)(latitude - (1U << 31))
                                   : (int)((1U << 31) - latitude);
    int lon = longitude >= 1U << 31 ? (int)(longitude - (1U << 31))
                                    : (int)((1U << 31) - longitude);
    double alt = altitude >= 10000000u ? altitude - 10000000u
                                       : -(double)(10000000u - altitude);
    ...
    if (asprintf(&s, "%d %d %.3f %c %d %d %.3f %c %.2fm %.2fm %.2fm %.2fm",
                 (lat / 60000 / 60), (lat / 60000) % 60, (lat % 60000) / 1000.,
                 NS, (lon / 60000 / 60), (lon / 60000) % 60,
                 (lon % 60000) / 1000., EW, alt / 100., siz / 100., hor / 100.,
                 ver / 100.) < 0)
        return NULL;
...
const char* dns_resource_record_to_string(DnsResourceRecord* rr)
{
    ...
    switch (rr->unparseable ? _DNS_TYPE_INVALID : rr->key->type)
    {
        ...
        case DNS_TYPE_SRV:
            r = asprintf(&s, "%s %u %u %u %s", k, rr->srv.priority,
                         rr->srv.weight, rr->srv.port, strna(rr->srv.name));
            if (r < 0)
                return NULL;
            break;
        ...
        case DNS_TYPE_A:
        {
            ...
            r = in_addr_to_string(
                AF_INET, (const union in_addr_union*)&rr->a.in_addr, &x);
            if (r < 0)
                return NULL;
        ...
        case DNS_TYPE_AAAA:
            r = in_addr_to_string(
                AF_INET6, (const union in_addr_union*)&rr->aaaa.in6_addr, &t);
            if (r < 0)
                return NULL;
        ...
        case DNS_TYPE_SOA:
            r = asprintf(&s, "%s %s %s %u %u %u %u %u", k, strna(rr->soa.mname),
                         strna(rr->soa.rname), rr->soa.serial, rr->soa.refresh,
        ...
        case DNS_TYPE_MX:
            r = asprintf(&s, "%s %u %s", k, rr->mx.priority, rr->mx.exchange);
            if (r < 0)
                return NULL;
            break;
        ...
        case DNS_TYPE_LOC:
            ...
            t = format_location(rr->loc.latitude, rr->loc.longitude,
                                rr->loc.altitude, rr->loc.size,
                                rr->loc.horiz_pre, rr->loc.vert_pre);
            if (!t)
                return NULL;
        ...
        case DNS_TYPE_DS:
            ...
            r = asprintf(&s, "%s %u %u %u %s", k, rr->ds.key_tag,
                         rr->ds.algorithm, rr->ds.digest_type, t);
            if (r < 0)
                return NULL;
            break;
        ...
        case DNS_TYPE_SSHFP:
            ...
            r = asprintf(&s, "%s %u %u %s", k, rr->sshfp.algorithm,
                         rr->sshfp.fptype, t);
            if (r < 0)
                return NULL;
            break;
        ...
        case DNS_TYPE_DNSKEY:
        {
            ...
            r = asprintf(&s, "%s %u %u %s", k, rr->dnskey.flags,
                         rr->dnskey.protocol, alg);
            if (r < 0)
                return NULL;

            r = base64_append(&s, r, rr->dnskey.key, rr->dnskey.key_size, 8,
                              columns());
            if (r < 0)
                return NULL;
            ...
            r = asprintf(&ss,
                         ...
                         "        -- Flags:%s%s%s\n"
                         "        -- Key tag: %u",
                         s, rr->dnskey.flags & DNSKEY_FLAG_SEP ? " SEP" : "",
        ...
        {
            _cleanup_free_ char* alg = NULL;
            char expiration[STRLEN("YYYYMMDDHHmmSS") + 1],
                inception[STRLEN("YYYYMMDDHHmmSS") + 1];
            const char* type;
            ...
            r = format_timestamp_dns(expiration, sizeof(expiration),
                                     rr->rrsig.expiration);
            if (r < 0)
                return NULL;

            r = format_timestamp_dns(inception, sizeof(inception),
                                     rr->rrsig.inception);
            if (r < 0)
                return NULL;
            ...
            r = asprintf(&s, "%s %s%.*u %s %u %u %s %s %u %s", k,
                         type ?: "TYPE", type ? 0 : 1,
                         type ? 0u : (unsigned)rr->rrsig.type_covered, alg,
                         rr->rrsig.labels, rr->rrsig.original_ttl, expiration,
                         inception, rr->rrsig.key_tag, rr->rrsig.signer);
            if (r < 0)
                return NULL;

            r = base64_append(&s, r, rr->rrsig.signature,
                              rr->rrsig.signature_size, 8, columns());
            if (r < 0)
                return NULL;
        ...
        case DNS_TYPE_NSEC:
            ...
            r = asprintf(&s, "%s %s %s", k, rr->nsec.next_domain_name, t);
            if (r < 0)
                return NULL;
            break;
        ...
        case DNS_TYPE_NSEC3:
        {
            ...
            hash = base32hexmem(rr->nsec3.next_hashed_name,
                                rr->nsec3.next_hashed_name_size, false);
            if (!hash)
                return NULL;
            ...
            r = asprintf(&s, "%s %" PRIu8 " %" PRIu8 " %" PRIu16 " %s %s %s", k,
                         rr->nsec3.algorithm, rr->nsec3.flags,
                         rr->nsec3.iterations,
                         rr->nsec3.salt_size > 0 ? salt : "-", hash, t);
            if (r < 0)
                return NULL;
        ...
        case DNS_TYPE_TLSA:
        {
            ...
            r = asprintf(&s,
                         ...
                         "        -- Selector: %s\n"
                         "        -- Matching type: %s",
                         k, rr->tlsa.cert_usage, rr->tlsa.selector,
                         rr->tlsa.matching_type, t, cert_usage, selector,
                         matching_type);
            if (r < 0)
                return NULL;
        ...
        case DNS_TYPE_CAA:
        {
            ...
            r = asprintf(&s, "%s %u %s \"%s\"%s%s%s%.0u", k, rr->caa.flags,
                         rr->caa.tag, value,
                         rr->caa.flags ? "\n        -- Flags:" : "",
        ...
        case DNS_TYPE_OPENPGPKEY:
        {
            ...
            r = base64_append(&s, r, rr->generic.data, rr->generic.data_size, 8,
                              columns());
            if (r < 0)
                return NULL;
            break;
...
int dns_resource_record_is_synthetic(DnsResourceRecord* rr)
{
    ...
    /* Returns > 0 if the RR is generated from a wildcard, and is not the
     * asterisk name itself */
...
void dns_resource_record_hash_func(const DnsResourceRecord* rr,
                                   struct siphash* state)
{
    assert(rr);
    ...
    switch (rr->unparseable ? _DNS_TYPE_INVALID : rr->key->type)
    {
        ...
        case DNS_TYPE_SRV:
            siphash24_compress(&rr->srv.priority, sizeof(rr->srv.priority),
                               state);
        ...
        case DNS_TYPE_SPF:
        {
            ...
            LIST_FOREACH(items, j, rr->txt.items)
            {
                ...
                /* Add an extra NUL byte, so that "a" followed by "b" doesn't
                 * result in the same hash as "ab" followed by "". */
        ...
        case DNS_TYPE_AAAA:
            siphash24_compress(&rr->aaaa.in6_addr, sizeof(rr->aaaa.in6_addr),
                               state);
            break;
        ...
        case DNS_TYPE_SOA:
            ...
            siphash24_compress(&rr->soa.refresh, sizeof(rr->soa.refresh),
                               state);
            siphash24_compress(&rr->soa.retry, sizeof(rr->soa.retry), state);
            siphash24_compress(&rr->soa.expire, sizeof(rr->soa.expire), state);
            siphash24_compress(&rr->soa.minimum, sizeof(rr->soa.minimum),
                               state);
            break;

        case DNS_TYPE_MX:
            siphash24_compress(&rr->mx.priority, sizeof(rr->mx.priority),
                               state);
            dns_name_hash_func(rr->mx.exchange, state);
            break;

        case DNS_TYPE_LOC:
            siphash24_compress(&rr->loc.version, sizeof(rr->loc.version),
                               state);
            siphash24_compress(&rr->loc.size, sizeof(rr->loc.size), state);
            siphash24_compress(&rr->loc.horiz_pre, sizeof(rr->loc.horiz_pre),
                               state);
            siphash24_compress(&rr->loc.vert_pre, sizeof(rr->loc.vert_pre),
                               state);
            siphash24_compress(&rr->loc.latitude, sizeof(rr->loc.latitude),
                               state);
            siphash24_compress(&rr->loc.longitude, sizeof(rr->loc.longitude),
                               state);
            siphash24_compress(&rr->loc.altitude, sizeof(rr->loc.altitude),
                               state);
            break;

        case DNS_TYPE_SSHFP:
            siphash24_compress(&rr->sshfp.algorithm,
                               sizeof(rr->sshfp.algorithm), state);
            siphash24_compress(&rr->sshfp.fptype, sizeof(rr->sshfp.fptype),
                               state);
            siphash24_compress(rr->sshfp.fingerprint,
                               rr->sshfp.fingerprint_size, state);
            break;

        case DNS_TYPE_DNSKEY:
            siphash24_compress(&rr->dnskey.flags, sizeof(rr->dnskey.flags),
                               state);
            siphash24_compress(&rr->dnskey.protocol,
                               sizeof(rr->dnskey.protocol), state);
            siphash24_compress(&rr->dnskey.algorithm,
                               sizeof(rr->dnskey.algorithm), state);
            siphash24_compress(rr->dnskey.key, rr->dnskey.key_size, state);
            break;

        case DNS_TYPE_RRSIG:
            siphash24_compress(&rr->rrsig.type_covered,
                               sizeof(rr->rrsig.type_covered), state);
            siphash24_compress(&rr->rrsig.algorithm,
                               sizeof(rr->rrsig.algorithm), state);
            siphash24_compress(&rr->rrsig.labels, sizeof(rr->rrsig.labels),
                               state);
            siphash24_compress(&rr->rrsig.original_ttl,
                               sizeof(rr->rrsig.original_ttl), state);
            siphash24_compress(&rr->rrsig.expiration,
                               sizeof(rr->rrsig.expiration), state);
            siphash24_compress(&rr->rrsig.inception,
                               sizeof(rr->rrsig.inception), state);
            siphash24_compress(&rr->rrsig.key_tag, sizeof(rr->rrsig.key_tag),
                               state);
            dns_name_hash_func(rr->rrsig.signer, state);
            siphash24_compress(rr->rrsig.signature, rr->rrsig.signature_size,
                               state);
            break;
        ...
        case DNS_TYPE_DS:
            ...
            siphash24_compress(&rr->ds.algorithm, sizeof(rr->ds.algorithm),
                               state);
            siphash24_compress(&rr->ds.digest_type, sizeof(rr->ds.digest_type),
                               state);
            siphash24_compress(rr->ds.digest, rr->ds.digest_size, state);
            break;

        case DNS_TYPE_NSEC3:
            siphash24_compress(&rr->nsec3.algorithm,
                               sizeof(rr->nsec3.algorithm), state);
            siphash24_compress(&rr->nsec3.flags, sizeof(rr->nsec3.flags),
                               state);
            siphash24_compress(&rr->nsec3.iterations,
                               sizeof(rr->nsec3.iterations), state);
            siphash24_compress(rr->nsec3.salt, rr->nsec3.salt_size, state);
            siphash24_compress(rr->nsec3.next_hashed_name,
                               rr->nsec3.next_hashed_name_size, state);
            /* FIXME: We leave the bitmaps out */
            break;

        case DNS_TYPE_TLSA:
            siphash24_compress(&rr->tlsa.cert_usage,
                               sizeof(rr->tlsa.cert_usage), state);
            siphash24_compress(&rr->tlsa.selector, sizeof(rr->tlsa.selector),
                               state);
            siphash24_compress(&rr->tlsa.matching_type,
                               sizeof(rr->tlsa.matching_type), state);
...
static int dns_resource_record_compare_func(const DnsResourceRecord* x,
                                            const DnsResourceRecord* y)
{
    int r;
...
DEFINE_HASH_OPS(dns_resource_record_hash_ops, DnsResourceRecord,
    ...
    switch (rr->unparseable ? _DNS_TYPE_INVALID : rr->key->type)
    {
        ...
        case DNS_TYPE_SSHFP:
            ...
            copy->sshfp.fingerprint =
                memdup(rr->sshfp.fingerprint, rr->sshfp.fingerprint_size);
        ...
        case DNS_TYPE_RRSIG:
            ...
            if (!copy->rrsig.signer)
                return NULL;
            copy->rrsig.signature =
                memdup(rr->rrsig.signature, rr->rrsig.signature_size);
        ...
        case DNS_TYPE_NSEC3:
            ...
            copy->nsec3.next_hashed_name = memdup(
        ...
        default:
            copy->generic.data =
                memdup(rr->generic.data, rr->generic.data_size);
...
int dns_txt_item_new_empty(DnsTxtItem** ret)
{
    ...
    i = malloc0(offsetof(DnsTxtItem, data) +
