...
/// Different depending on whether BOOST_TEST_ALTERNATIVE_INIT_API is defined or
/// not
...
BOOST_TEST_DECL void init(init_unit_test_func init_func, int argc,
                          char* argv[]);

/// This function applies all the decorators and figures out default run status.
/// This argument facilitates an ability of the test cases to prepare some other
/// test units (primarily used internally for self testing).
/// @param[in] tu Optional id of the test unit representing root of test tree.
/// If absent, master test suite is used
...
/// Provides both read and write access to current "leaf" auto test suite during
/// the test unit registration phase.
///
/// During auto-registration phase the framework maintain a FIFO queue of test
/// units being registered. New test units become children of the current "leaf"
/// test suite and if this is test suite it is pushed back into queue and
/// becomes a new leaf. When test suite registration is completed, a test suite
/// is popped from the back of the queue. Only automatically registered test
/// suites should be added to this queue. Master test suite is always a zero
/// element in this queue, so if no other test suites are registered all test
/// cases are added to master test suite.

/// This function facilitates all three possible actions:
///    - if no argument are provided it returns the current queue leaf test
///    suite
///    - if test suite is provided and no second argument are set, test suite is
///    added to the queue
///    - if no test suite are provided and last argument is false, the semantic
///    of this function is similar to queue pop: last element is popped from the
///    queue
/// @param[in] ts test suite to push back to the queue
/// @param[in] push_or_pop should we push ts to the queue or pop leaf test suite
/// instead
/// @returns a reference to the currently active/"leaf" test suite
BOOST_TEST_DECL test_suite& current_auto_test_suite(test_suite* ts = 0,
                                                    bool push_or_pop = true);

/// This function add new test case into the global collection of test units the
/// framework aware of.

/// This function also assignes unique test unit id for every test case. Later
/// on one can use this id to locate the test case if necessary. This is the way
/// for the framework to maintain weak references between test units.
/// @param[in]  tc  test case to register
BOOST_TEST_DECL void register_test_unit(test_case* tc);

/// This function add new test suite into the global collection of test units
/// the framework aware of.

/// This function also assignes unique test unit id for every test suite. Later
/// on one can use this id to locate the test case if necessary. This is the way
/// for the framework to maintain weak references between test units.
/// @param[in]  ts  test suite to register
BOOST_TEST_DECL void register_test_unit(test_suite* ts);

/// This function removes the test unit from the collection of known test units
/// and destroys the test unit object.

/// This function also assigns unique test unit id for every test case. Later on
/// one can use this id to located the test case if necessary. This is the way
/// for the framework to maintain weak references between test units.
...
/// After this call the framework can be reinitialized to perform a second test
/// run during the same program lifetime.
BOOST_TEST_DECL void clear();
/// @}

/// @name Test observer registration
/// @{
/// Adds new test execution observer object into the framework's list of test
/// observers.
...
/// Adds a new global fixture to be setup before any other tests starts and tore
/// down after any other tests finished. Test unit fixture lifetime should
/// exceed the testing execution timeframe
...
/// Some context frames are sticky - they can only explicitly cleared by
/// specifying context id. Other (non sticky) context frames cleared after every
/// assertion.
...
/// If context_id is passed clears that specific context frame identified by
/// this id, otherwise clears all non sticky contexts.
BOOST_TEST_DECL void clear_context(int context_id = -1);
/// Produces an instance of small "delegate" object, which facilitates access to
/// collected context.
...
/// is being set up or torn down (fixtures) and when the test-cases of that
/// suite start.
...
/// This function provides an access to an id of the test case currently being
/// executed.

/// This function safer than current_test_case, cause if wont throw if no test
/// case is being executed.
/// @see current_test_case
BOOST_TEST_DECL test_unit_id current_test_case_id(); /* safe version of above */

/// This function provides access to a test unit by id and type combination. It
/// will throw if no test unit located.
...
/// @tparam UnitType compile time type of test unit to get (test_suite or
/// test_case)
...
inline UnitType& get(test_unit_id id)
{
    return static_cast<UnitType&>(
        get(id, static_cast<test_unit_type>(UnitType::type)));
}
///@}
...
/// This function is used to start the test execution from a specific "root"
/// test unit. If no root provided, test is started from master test suite. This
/// second argument facilitates an ability of the test cases to start some other
/// test units (primarily used internally for self testing).
/// @param[in] tu Optional id of the test unit or test unit itself from which
/// the test is started. If absent, master test suite is used
/// @param[in] continue_test true == continue test if it was already started,
/// false == restart the test from scratch regardless
BOOST_TEST_DECL void run(test_unit_id tu = INV_TEST_UNIT_ID,
...
{
    internal_error(const_string m) :
        std::runtime_error(std::string(m.begin(), m.size()))
    {
    }
};
...
#define BOOST_TEST_SETUP_ASSERT(cond, msg)                                     \
    BOOST_TEST_I_ASSRT(cond, unit_test::framework::setup_error(msg))
