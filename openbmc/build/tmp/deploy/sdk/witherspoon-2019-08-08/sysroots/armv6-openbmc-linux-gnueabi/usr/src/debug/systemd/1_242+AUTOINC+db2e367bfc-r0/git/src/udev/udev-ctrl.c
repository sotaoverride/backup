...
DEFINE_TRIVIAL_CLEANUP_FUNC(struct udev_ctrl*,
                            udev_ctrl_disconnect_and_listen_again);

static int udev_ctrl_connection_event_handler(sd_event_source* s, int fd,
                                              uint32_t revents, void* userdata)
{
    _cleanup_(udev_ctrl_disconnect_and_listen_againp) struct udev_ctrl* uctrl =
        NULL;
    ...
    if (cred->uid != 0)
    {
        log_error("Invalid sender uid " UID_FMT ", ignoring message",
                  cred->uid);
        return 0;
    }
    ...
    {
        log_error("Message magic 0x%08x doesn't match, ignoring message",
                  msg_wire.magic);
        return 0;
    }
    ...
    if (uctrl->callback)
        (void)uctrl->callback(uctrl, msg_wire.type, &msg_wire.value,
                              uctrl->userdata);
...
static int udev_ctrl_event_handler(sd_event_source* s, int fd, uint32_t revents,
                                   void* userdata)
{
    ...
    if (ucred.uid > 0)
    {
        log_error("Invalid sender uid " UID_FMT ", closing connection",
                  ucred.uid);
        return 0;
    }
    ...
    r = sd_event_add_io(uctrl->event, &uctrl->event_source_connect, sock,
                        EPOLLIN, udev_ctrl_connection_event_handler, uctrl);
    if (r < 0)
    {
        log_error_errno(
...
int udev_ctrl_start(struct udev_ctrl* uctrl, udev_ctrl_handler_t callback,
                    void* userdata)
{
    int r;
    ...
    r = sd_event_add_io(uctrl->event, &uctrl->event_source, uctrl->sock,
                        EPOLLIN, udev_ctrl_event_handler, uctrl);
    if (r < 0)
        return r;
...
int udev_ctrl_send(struct udev_ctrl* uctrl, enum udev_ctrl_msg_type type,
                   int intval, const char* buf)
{
...
static int udev_ctrl_wait_io_handler(sd_event_source* s, int fd,
                                     uint32_t revents, void* userdata)
{
...
{
    _cleanup_(sd_event_source_unrefp) sd_event_source *source_io = NULL,
                                                      *source_timeout = NULL;
    int r;
    ...
    r = sd_event_add_io(uctrl->event, &source_io, uctrl->sock, EPOLLIN,
                        udev_ctrl_wait_io_handler, NULL);
    if (r < 0)
        return r;

    (void)sd_event_source_set_description(uctrl->event_source,
                                          "udev-ctrl-wait-io");
    ...
    if (timeout != USEC_INFINITY)
    {
        ...
        r = sd_event_add_time(uctrl->event, &source_timeout,
                              clock_boottime_or_monotonic(), usec, 0, NULL,
                              INT_TO_PTR(-ETIMEDOUT));
        if (r < 0)
            return r;

        (void)sd_event_source_set_description(source_timeout,
                                              "udev-ctrl-wait-io");
    }
