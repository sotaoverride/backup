...
class filter
{
  ...
  private:
    ...
    typedef boost::log::aux::light_function<bool(attribute_value_set const&)>
        filter_type;
  ...
  public:
    ...
    filter(BOOST_RV_REF(filter) that) BOOST_NOEXCEPT
        : m_Filter(boost::move(that.m_Filter))
    {
    }

    /*!
     * Initializing constructor. Creates a filter which will invoke the
    ...
    typename boost::disable_if_c<
        is_same<typename remove_cv<FunT>::type, filter>::value, filter&>::type
    ...
    /*!
     * Resets the filter to the default. The default filter always returns \c
     * true.
     */
    void reset()
    {
