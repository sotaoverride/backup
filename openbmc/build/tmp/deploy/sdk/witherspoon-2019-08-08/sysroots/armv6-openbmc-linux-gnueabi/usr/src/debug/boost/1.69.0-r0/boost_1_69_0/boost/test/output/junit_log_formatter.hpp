...
struct junit_log_helper
{
    struct assertion_entry
    {
        ...
        std::string
            logentry_message; // the message associated to the JUnit error/entry
        std::string logentry_type; // the one that will get expanded in the
                                   // final junit (failure, error)
        std::string
            output; // additional information/message generated by the assertion

        log_entry_t
            log_entry; // the type associated to the assertion (or error)
    ...
    // list of failure, errors and messages (assertions message and the full
    // log)
...
class junit_log_formatter : public unit_test_log_formatter
{
  public:
    junit_log_formatter() : m_display_build_info(false)
    {
        ...
        // because we need to know about all the messages/commands going to the
        // logger we decide what we put inside the logs internally
