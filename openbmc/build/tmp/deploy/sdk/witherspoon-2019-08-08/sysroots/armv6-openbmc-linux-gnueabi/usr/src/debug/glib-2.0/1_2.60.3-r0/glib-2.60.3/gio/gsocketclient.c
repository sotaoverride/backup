...
static GSocket* create_socket(GSocketClient* client,
                              GSocketAddress* dest_address, GError** error)
{
    GSocketFamily family;
    GSocket* socket;
    ...
    socket =
        g_socket_new(family, client->priv->type, client->priv->protocol, error);
    if (socket == NULL)
        return NULL;
...
static gboolean can_use_proxy(GSocketClient* client)
{
...
static void clarify_connect_error(GError* error,
                                  ...
                                  GSocketAddress* address)
{
    ...
    {
        name = tmp_name = g_inet_address_to_string(
            g_inet_socket_address_get_address(G_INET_SOCKET_ADDRESS(address)));

        g_prefix_error(&error, _("Could not connect to proxy server %s: "),
                       name);
    }
    else
    {
        if (G_IS_NETWORK_ADDRESS(connectable))
            name =
                g_network_address_get_hostname(G_NETWORK_ADDRESS(connectable));
...
static void g_socket_client_get_property(GObject* object, guint prop_id,
...
static void g_socket_client_set_property(GObject* object, guint prop_id,
                                         const GValue* value, GParamSpec* pspec)
{
    ...
    switch (prop_id)
    {
        ...
        case PROP_LOCAL_ADDRESS:
            g_socket_client_set_local_address(client,
        ...
        case PROP_ENABLE_PROXY:
            g_socket_client_set_enable_proxy(client,
...
static void g_socket_client_class_init(GSocketClientClass* class)
{
    ...
    signals[EVENT] = g_signal_new(
        I_("event"), G_TYPE_FROM_CLASS(gobject_class), G_SIGNAL_RUN_LAST,
        G_STRUCT_OFFSET(GSocketClientClass, event), NULL, NULL, NULL,
        G_TYPE_NONE, 3, G_TYPE_SOCKET_CLIENT_EVENT, G_TYPE_SOCKET_CONNECTABLE,
        G_TYPE_IO_STREAM);

    g_object_class_install_property(
        gobject_class, PROP_FAMILY,
        g_param_spec_enum(
            "family", P_("Socket family"),
    ...
    g_object_class_install_property(
        ...
        g_param_spec_enum("type", P_("Socket type"),
    ...
    g_object_class_install_property(
        ...
        g_param_spec_enum(
            ...
            G_TYPE_SOCKET_PROTOCOL, G_SOCKET_PROTOCOL_DEFAULT,
            G_PARAM_CONSTRUCT | G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS));

    g_object_class_install_property(
        gobject_class, PROP_LOCAL_ADDRESS,
        g_param_spec_object(
            "local-address", P_("Local address"),
    ...
    g_object_class_install_property(
        ...
        g_param_spec_uint(
            "timeout", P_("Socket timeout"),
            P_("The I/O timeout for sockets, or 0 for none"), 0, G_MAXUINT, 0,
            G_PARAM_CONSTRUCT | G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS));

    g_object_class_install_property(
        gobject_class, PROP_ENABLE_PROXY,
        g_param_spec_boolean("enable-proxy", P_("Enable proxy"),
                             P_("Enable proxy support"), TRUE,
                             G_PARAM_CONSTRUCT | G_PARAM_READWRITE |
                                 G_PARAM_STATIC_STRINGS));

    g_object_class_install_property(
        gobject_class, PROP_TLS,
        g_param_spec_boolean(
            "tls", P_("TLS"), P_("Whether to create TLS connections"), FALSE,
            G_PARAM_CONSTRUCT | G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS));
    g_object_class_install_property(
        gobject_class, PROP_TLS_VALIDATION_FLAGS,
        g_param_spec_flags(
            "tls-validation-flags", P_("TLS validation flags"),
            P_("TLS validation flags to use"), G_TYPE_TLS_CERTIFICATE_FLAGS,
     ...
     */
    g_object_class_install_property(
        gobject_class, PROP_PROXY_RESOLVER,
        g_param_spec_object(
            "proxy-resolver", P_("Proxy resolver"),
            P_("The proxy resolver to use"), G_TYPE_PROXY_RESOLVER,
            G_PARAM_CONSTRUCT | G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS));
}

static void g_socket_client_emit_event(GSocketClient* client,
                                       GSocketClientEvent event,
                                       GSocketConnectable* connectable,
                                       GIOStream* connection)
{
    g_signal_emit(client, signals[EVENT], 0, event, connectable, connection);
}
                                           ...
                                           GError** error)
{
    ...
    if (can_use_proxy(client))
    {
            ...
            G_IS_PROXY_ADDRESS_ENUMERATOR(enumerator))
        {
            g_object_set(G_OBJECT(enumerator), "proxy-resolver",
                         client->priv->proxy_resolver, NULL);
        }
    }
    else
    ...
    while (connection == NULL)
    {
        ...
        using_proxy =
            (G_IS_PROXY_ADDRESS(address) && client->priv->enable_proxy);
        ...
        connection =
            (GIOStream*)g_socket_connection_factory_create_connection(socket);
        g_socket_connection_set_cached_remote_address(
            (GSocketConnection*)connection, address);
        g_socket_client_emit_event(client, G_SOCKET_CLIENT_CONNECTING,
                                        ...
                                        address, cancellable, &last_error))
        {
            ...
            g_socket_client_emit_event(client, G_SOCKET_CLIENT_CONNECTED,
                                       connectable, connection);
        }
        else
        {
        ...
        if (connection && using_proxy)
        {
            ...
            if (!G_IS_TCP_CONNECTION(connection))
            {
                ...
                g_set_error_literal(
                    &last_error, G_IO_ERROR, G_IO_ERROR_NOT_SUPPORTED,
            ...
            else if ((proxy = g_proxy_get_default_for_protocol(protocol)))
            {
                ...
                g_socket_client_emit_event(client,
                                           G_SOCKET_CLIENT_PROXY_NEGOTIATING,
                                           connectable, connection);
                proxy_connection = g_proxy_connect(
                    proxy, connection, proxy_addr, cancellable, &last_error);
                g_object_unref(connection);
        ...
        if (!application_proxy && connection && client->priv->tls)
        {
            ...
            tlsconn = g_tls_client_connection_new(connection, connectable,
                                                  &last_error);
            g_object_unref(connection);
            connection = tlsconn;
        ...
        if (connection && !G_IS_SOCKET_CONNECTION(connection))
        {
            ...
            wrapper_connection =
                g_tcp_wrapper_connection_new(connection, socket);
            g_object_unref(connection);
    ...
    g_socket_client_emit_event(client, G_SOCKET_CLIENT_COMPLETE, connectable,
                               connection);
 ...
 * accordingly.
 *
 * Returns: (transfer full): a #GSocketConnection if successful, or %NULL on
 * error
 */
...
static ConnectionAttempt* connection_attempt_new(void)
{
...
static ConnectionAttempt* connection_attempt_ref(ConnectionAttempt* attempt)
{
    g_ref_count_inc(&attempt->ref);
    return attempt;
}

static void connection_attempt_unref(gpointer pointer)
{
...
static void connection_attempt_remove(ConnectionAttempt* attempt)
{
    attempt->data->connection_attempts =
        g_slist_remove(attempt->data->connection_attempts, attempt);
    ...
    g_socket_client_async_connect_complete(GSocketClientAsyncConnectData* data)
{
    ...
    if (!G_IS_SOCKET_CONNECTION(data->connection))
    {
        ...
        wrapper_connection =
            g_tcp_wrapper_connection_new(data->connection, data->socket);
    ...
    if (!data->completed)
    {
        ...
        if (g_cancellable_set_error_if_cancelled(
                g_task_get_cancellable(data->task), &error))
        {
        ...
        else
        {
            ...
            g_task_return_pointer(
                data->task, g_steal_pointer(&data->connection), g_object_unref);
        }
...
static void g_socket_client_enumerator_callback(GObject* object,
                                                GAsyncResult* result,
                                                gpointer user_data);
                                  ...
                                  gboolean add_task_ref)
{
    ...
    g_socket_address_enumerator_next_async(
        data->enumerator, g_task_get_cancellable(data->task),
        g_socket_client_enumerator_callback, data);
}

static void g_socket_client_tls_handshake_callback(GObject* object,
                                                   GAsyncResult* result,
                                                   gpointer user_data)
{
    GSocketClientAsyncConnectData* data = user_data;

    if (g_tls_connection_handshake_finish(G_TLS_CONNECTION(object), result,
                                          &data->last_error))
    {
        g_object_unref(data->connection);
        data->connection = G_IO_STREAM(object);

        g_socket_client_emit_event(data->client, G_SOCKET_CLIENT_TLS_HANDSHAKED,
                                   data->connectable, data->connection);
...
static void g_socket_client_tls_handshake(GSocketClientAsyncConnectData* data)
{
    ...
    tlsconn = g_tls_client_connection_new(data->connection, data->connectable,
                                          &data->last_error);
    if (tlsconn)
    {
        g_tls_client_connection_set_validation_flags(
        ...
        g_socket_client_emit_event(data->client,
                                   G_SOCKET_CLIENT_TLS_HANDSHAKING,
                                   data->connectable, G_IO_STREAM(tlsconn));
        g_tls_connection_handshake_async(
            G_TLS_CONNECTION(tlsconn), G_PRIORITY_DEFAULT,
            g_task_get_cancellable(data->task),
            g_socket_client_tls_handshake_callback, data);
    }
    else
    {
        enumerator_next_async(data, FALSE);
...
static void g_socket_client_proxy_connect_callback(GObject* object,
                                                   GAsyncResult* result,
                                                   gpointer user_data)
{
    GSocketClientAsyncConnectData* data = user_data;
    ...
    g_object_unref(data->connection);
    data->connection =
        g_proxy_connect_finish(G_PROXY(object), result, &data->last_error);
    if (data->connection)
    {
        g_socket_client_emit_event(data->client,
                                   G_SOCKET_CLIENT_PROXY_NEGOTIATED,
...
static gboolean task_completed_or_cancelled(GSocketClientAsyncConnectData* data)
{
...
static void g_socket_client_connected_callback(GObject* source,
                                               GAsyncResult* result,
                                               gpointer user_data)
{
    ...
    if (task_completed_or_cancelled(data) ||
        g_cancellable_is_cancelled(attempt->cancellable))
    {
        g_object_unref(data->task);
        connection_attempt_unref(attempt);
        return;
    ...
    if (!g_socket_connection_connect_finish(G_SOCKET_CONNECTION(source), result,
                                            &error))
    {
    ...
    if (!G_IS_TCP_CONNECTION(data->connection))
    {
        ...
        g_set_error_literal(
            &data->last_error, G_IO_ERROR, G_IO_ERROR_NOT_SUPPORTED,
            _("Proxying over a non-TCP connection is not supported."));
    ...
    else if ((proxy = g_proxy_get_default_for_protocol(protocol)))
    {
        g_socket_client_emit_event(data->client,
                                   G_SOCKET_CLIENT_PROXY_NEGOTIATING,
                                   data->connectable, data->connection);
        g_proxy_connect_async(proxy, data->connection, data->proxy_addr,
                              g_task_get_cancellable(data->task),
                              g_socket_client_proxy_connect_callback, data);
        g_object_unref(proxy);
    }
    else
    {
        ...
        g_set_error(&data->last_error, G_IO_ERROR, G_IO_ERROR_NOT_SUPPORTED,
                    _("Proxy protocol “%s” is not supported."), protocol);

        enumerator_next_async(data, FALSE);
    }
}

static gboolean on_connection_attempt_timeout(gpointer data)
{
...
static void on_connection_cancelled(GCancellable* cancellable, gpointer data)
{
                                                ...
                                                gpointer user_data)
{
    ...
    address = g_socket_address_enumerator_next_finish(data->enumerator, result,
                                                      &error);
    if (address == NULL)
    {
    ...
    g_source_attach(attempt->timeout_source,
                    g_main_context_get_thread_default());
    data->connection_attempts =
        g_slist_append(data->connection_attempts, attempt);

    if (g_task_get_cancellable(data->task))
        g_cancellable_connect(g_task_get_cancellable(data->task),
                              G_CALLBACK(on_connection_cancelled),
                              g_object_ref(attempt->cancellable),
                              g_object_unref);

    g_socket_connection_set_cached_remote_address(
        (GSocketConnection*)attempt->connection, address);
    g_socket_client_emit_event(data->client, G_SOCKET_CLIENT_CONNECTING,
                               data->connectable, attempt->connection);
    g_socket_connection_connect_async(
        G_SOCKET_CONNECTION(attempt->connection), address, attempt->cancellable,
                                   ...
                                   gpointer user_data)
{
    ...
    g_task_set_check_cancellable(data->task,
...
void g_socket_client_connect_to_host_async(
    GSocketClient* client, const gchar* host_and_port, guint16 default_port,
    GCancellable* cancellable, GAsyncReadyCallback callback, gpointer user_data)
{
    ...
    if (connectable == NULL)
    {
        g_task_report_error(client, callback, user_data,
                            g_socket_client_connect_to_host_async, error);
    }
    else
    {
        g_socket_client_connect_async(client, connectable, cancellable,
                                      callback, user_data);
        g_object_unref(connectable);
    }
}
    ...
    GCancellable* cancellable, GAsyncReadyCallback callback, gpointer user_data)
{
    ...
    connectable = g_network_service_new(service, "tcp", domain);
    g_socket_client_connect_async(client, connectable, cancellable, callback,
                                  user_data);
    g_object_unref(connectable);
}
...
void g_socket_client_connect_to_uri_async(
    GSocketClient* client, const gchar* uri, guint16 default_port,
    GCancellable* cancellable, GAsyncReadyCallback callback, gpointer user_data)
{
    GSocketConnectable* connectable;
    GError* error;
    ...
    {
        g_task_report_error(client, callback, user_data,
                            g_socket_client_connect_to_uri_async, error);
    }
    else
    {
        g_socket_client_connect_async(client, connectable, cancellable,
