...
static gcry_mpi_t mpi_read_from_buffer(const unsigned char* buffer,
                                       unsigned* ret_nread, int secure)
{
    int i, j;
...
static unsigned char* do_get_buffer(gcry_mpi_t a, unsigned int fill_le,
                                    int extraalloc, unsigned int* nbytes,
                                    int* sign, int force_secure)
{
 ...
 * SIGN.
 */
void _gcry_mpi_set_buffer(gcry_mpi_t a, const void* buffer_arg,
...
static void onecompl(gcry_mpi_t a)
{
    mpi_ptr_t ap;
    mpi_size_t n;
    unsigned int i;
...
gcry_err_code_t _gcry_mpi_scan(struct gcry_mpi** ret_mpi,
                               enum gcry_mpi_format format,
                               const void* buffer_arg, size_t buflen,
                               size_t* nscanned)
{
    const unsigned char* buffer = (const unsigned char*)buffer_arg;
    ...
    if (format == GCRYMPI_FMT_STD)
    {
        ...
        a = secure ? mpi_alloc_secure((len + BYTES_PER_MPI_LIMB - 1) /
                                      BYTES_PER_MPI_LIMB)
                   : mpi_alloc((len + BYTES_PER_MPI_LIMB - 1) /
                               BYTES_PER_MPI_LIMB);
        if (len)
        {
    ...
    else if (format == GCRYMPI_FMT_SSH)
    {
        ...
        a = secure
                ? mpi_alloc_secure((n + BYTES_PER_MPI_LIMB - 1) /
                                   BYTES_PER_MPI_LIMB)
...
/* Convert the big integer A into the external representation
   ...
   length.  */
gcry_err_code_t _gcry_mpi_print(enum gcry_mpi_format format,
 ...
 * NULL.
 */
gcry_err_code_t _gcry_mpi_aprint(enum gcry_mpi_format format,
                                 ...
                                 struct gcry_mpi* a)
{
    ...
    *buffer =
        mpi_is_secure(a) ? xtrymalloc_secure(n ? n : 1) : xtrymalloc(n ? n : 1);
    if (!*buffer)
        return gpg_err_code_from_syserror();
                                         ...
                                         gcry_mpi_t value, size_t nbytes)
{
    ...
    if (noff)
        memset(frame, 0, noff);
    nframe += noff;
    rc = _gcry_mpi_print(GCRYMPI_FMT_USG, frame + noff, nframe - noff, NULL,
                         value);
    if (rc)
    {
        xfree(frame);
