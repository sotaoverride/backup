 ...
 * call g_atomic_rc_box_acquire(); similarly, when you wish to release a
 * reference you should call g_atomic_rc_box_release():
 *
 * |[<!-- language="C" -->
 ...
 * g_atomic_rc_box_alloc(), g_atomic_rc_box_new(), and g_atomic_rc_box_dup() are
 * guaranteed to be atomic, and thus can be safely be performed by different
 * threads. It is important to note that only the reference acquisition and
 * release are atomic; changes to the content of the data are your
 * responsibility.
 *
...
/**
 * g_atomic_rc_box_release:
 * @mem_block: (transfer full) (not nullable): a pointer to reference counted
 * data
 *
