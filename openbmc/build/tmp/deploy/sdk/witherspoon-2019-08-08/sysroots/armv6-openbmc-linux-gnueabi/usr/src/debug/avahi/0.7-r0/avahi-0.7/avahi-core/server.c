...
static void enum_aux_records(AvahiServer* s, AvahiInterface* i,
                             const char* name, uint16_t type,
                             void (*callback)(AvahiServer* s, AvahiRecord* r,
                                              int flush_cache, void* userdata),
                             void* userdata)
{
    assert(s);
    assert(i);
    ...
    if (type == AVAHI_DNS_TYPE_ANY)
    {
        ...
        for (e = s->entries; e; e = e->entries_next)
            if (!e->dead && avahi_entry_is_registered(s, e, i) &&
                ...
                callback(s, e->record, e->flags & AVAHI_PUBLISH_UNIQUE,
                         userdata);
    }
    else
    {
        ...
        for (e = avahi_hashmap_lookup(s->entries_by_key, k); e;
             e = e->by_key_next)
            if (!e->dead && avahi_entry_is_registered(s, e, i))
                callback(s, e->record, e->flags & AVAHI_PUBLISH_UNIQUE,
                         userdata);
...
void avahi_server_enumerate_aux_records(
    AvahiServer* s, AvahiInterface* i, AvahiRecord* r,
    void (*callback)(AvahiServer* s, AvahiRecord* r, int flush_cache,
                     void* userdata),
    void* userdata)
{
    assert(s);
    ...
    /* Call the specified callback far all records referenced by the one
     * specified in *r */
    ...
    if (r->key->clazz == AVAHI_DNS_CLASS_IN)
    {
        ...
        {
            enum_aux_records(s, i, r->data.ptr.name, AVAHI_DNS_TYPE_SRV,
                             callback, userdata);
            enum_aux_records(s, i, r->data.ptr.name, AVAHI_DNS_TYPE_TXT,
                             callback, userdata);
        }
        else if (r->key->type == AVAHI_DNS_TYPE_SRV)
        {
            enum_aux_records(s, i, r->data.srv.name, AVAHI_DNS_TYPE_A, callback,
                             userdata);
            enum_aux_records(s, i, r->data.srv.name, AVAHI_DNS_TYPE_AAAA,
                             callback, userdata);
        }
        else if (r->key->type == AVAHI_DNS_TYPE_CNAME)
            enum_aux_records(s, i, r->data.cname.name, AVAHI_DNS_TYPE_ANY,
                             callback, userdata);
    }
}

void avahi_server_prepare_response(AvahiServer* s, AvahiInterface* i,
                                   AvahiEntry* e, int unicast_response,
                                   int auxiliary)
{
    assert(s);
    assert(i);
    ...
    avahi_record_list_push(s->record_list, e->record,
                           e->flags & AVAHI_PUBLISH_UNIQUE, unicast_response,
                           auxiliary);
}

void avahi_server_prepare_matching_responses(AvahiServer* s, AvahiInterface* i,
                                             AvahiKey* k, int unicast_response)
{
    assert(s);
    assert(i);
    assert(k);
    ...
    if (avahi_key_is_pattern(k))
    {
        ...
        for (e = s->entries; e; e = e->entries_next)
            if (!e->dead && avahi_key_pattern_match(k, e->record->key) &&
        ...
        k->type != AVAHI_DNS_TYPE_CNAME && k->type != AVAHI_DNS_TYPE_ANY)
    {
        ...
        if (!(cname_key = avahi_key_new(k->name, AVAHI_DNS_CLASS_IN,
                                        AVAHI_DNS_TYPE_CNAME)))
            return;

        avahi_server_prepare_matching_responses(s, i, cname_key,
                                                unicast_response);
...
static void withdraw_rrset(AvahiServer* s, AvahiKey* key)
{
    ...
    for (e = avahi_hashmap_lookup(s->entries_by_key, key); e;
         e = e->by_key_next)
        withdraw_entry(s, e);
}

static void incoming_probe(AvahiServer* s, AvahiRecord* record,
                           AvahiInterface* i)
{
    AvahiEntry *e, *n;
    ...
    for (e = avahi_hashmap_lookup(s->entries_by_key, record->key); e; e = n)
    {
        ...
        if ((cmp = avahi_record_lexicographical_compare(e->record, record)) ==
            0)
        {
            ours = 1;
            break;
    ...
    if (!ours)
    {
        ...
        if (won)
            avahi_log_debug("Received conflicting probe [%s]. Local host won.",
                            t);
        else if (lost)
        {
            avahi_log_debug("Received conflicting probe [%s]. Local host lost. "
                            "Withdrawing.",
                            t);
...
static int handle_conflict(AvahiServer* s, AvahiInterface* i,
                           AvahiRecord* record, int unique)
{
    ...
    for (e = avahi_hashmap_lookup(s->entries_by_key, record->key); e; e = n)
    {
        ...
        if (avahi_record_is_goodbye(record))
        {
            ...
            if (avahi_record_equal_no_ttl(e->record, record))
            {
                ...
                avahi_log_debug("Received goodbye record for one of our "
                                "records [%s]. Refreshing.",
                                t);
                avahi_server_prepare_matching_responses(s, i, e->record->key,
                                                        0);
            ...
            /* If the goodybe packet doesn't match one of our own RRs, we simply
             * ignore it. */
            continue;
        }
        ...
        /* Either our entry or the other is intended to be unique, so let's
         * check */
        ...
        if (avahi_record_equal_no_ttl(e->record, record))
        {
                ...
                avahi_entry_is_registered(s, e, i))
            {
                ...
                avahi_log_debug(
                    "Received record with bad TTL [%s]. Refreshing.", t);
                avahi_server_prepare_matching_responses(s, i, e->record->key,
                                                        0);
                valid = 0;
    ...
    if (!ours && conflict)
    {
        ...
        {
            avahi_log_debug("Received conflicting record [%s] with local "
                            "record to be. Withdrawing.",
                            t);
            withdraw_rrset(s, record->key);
        }
        else
        {
            ...
            avahi_log_debug(
                "Received conflicting record [%s]. Resetting our record.", t);
...
static void append_aux_callback(AvahiServer* s, AvahiRecord* r, int flush_cache,
                                void* userdata)
{
    ...
    avahi_record_list_push(s->record_list, r, flush_cache, *unicast_response,
                           1);
}

static void append_aux_records_to_list(AvahiServer* s, AvahiInterface* i,
                                       AvahiRecord* r, int unicast_response)
{
    assert(s);
    assert(r);

    avahi_server_enumerate_aux_records(s, i, r, append_aux_callback,
                                       &unicast_response);
}

void avahi_server_generate_response(AvahiServer* s, AvahiInterface* i,
                                    AvahiDnsPacket* p, const AvahiAddress* a,
                                    uint16_t port, int legacy_unicast,
                                    int immediately)
{
    ...
    if (legacy_unicast)
    {
        ...
        if (!(reply = avahi_dns_packet_new_reply(
                  p, 512 + AVAHI_DNS_PACKET_EXTRA_SIZE /* unicast DNS maximum
                                                          packet size is 512 */
                  ,
                  1, 1)))
            return; /* OOM */
        ...
        while ((r = avahi_record_list_next(s->record_list, NULL, NULL, NULL)))
        {
            ...
            else
            {
                ...
                avahi_log_warn("Record [%s] not fitting in legacy unicast "
                               "packet, dropping.",
                               t);
                avahi_free(t);
            }
    ...
    else
    {
        ...
        int tc = p && !!(avahi_dns_packet_get_field(p, AVAHI_DNS_FIELD_FLAGS) &
                         AVAHI_DNS_FLAG_TC);

        while ((r = avahi_record_list_next(s->record_list, &flush_cache,
                                           &unicast_response, &auxiliary)))
        {
            ...
            if (flush_cache && !tc && !auxiliary &&
                avahi_record_list_all_flush_cache(s->record_list))
                im = 1;

            if (!avahi_interface_post_response(i, r, flush_cache, a, im) &&
                unicast_response)
            {
                ...
                for (;;)
                {
                                                       ...
                                                       0))
                    {
                        ...
                        avahi_dns_packet_inc_field(reply,
                                                   AVAHI_DNS_FIELD_ANCOUNT);
                        break;
                    }

                    if (avahi_dns_packet_get_field(
                            reply, AVAHI_DNS_FIELD_ANCOUNT) == 0)
                    {
                        size_t size;

                        /* The record is too large for one packet, so create a
                         * larger packet */
                        ...
                        if (!(reply = avahi_dns_packet_new_reply(
                                  p, size + AVAHI_DNS_PACKET_EXTRA_SIZE, 0, 1)))
                            break; /* OOM */

                        if (avahi_dns_packet_append_record(reply, r,
                                                           flush_cache, 0))
                        {

                            /* Appending this record succeeded, so incremeant
                             * the specific header field, and return to the
                             * caller */
                        ...
                        else
                        {
                            ...
                            avahi_log_warn("Record [%s] too large, doesn't fit "
                                           "in any packet!",
                                           t);
                            avahi_free(t);
                            break;
                    ...
                    /* Appending the record didn't succeeed, so let's send this
                     * packet, and create a new one */
                    avahi_interface_send_packet_unicast(i, reply, a, port);
                    avahi_dns_packet_free(reply);
                    reply = NULL;
                }
...
static void reflect_response(AvahiServer* s, AvahiInterface* i, AvahiRecord* r,
                             int flush_cache)
{
    AvahiInterface* j;
...
static void* reflect_cache_walk_callback(AvahiCache* c, AvahiKey* pattern,
                                         AvahiCacheEntry* e, void* userdata)
{
...
static void handle_query_packet(AvahiServer* s, AvahiDnsPacket* p,
                                AvahiInterface* i, const AvahiAddress* a,
                                uint16_t port, int legacy_unicast,
                                int from_local_iface)
{
    size_t n;
    int is_probe;
    ...
    for (n = avahi_dns_packet_get_field(p, AVAHI_DNS_FIELD_QDCOUNT); n > 0; n--)
    {
        ...
        {
            avahi_log_debug(__FILE__
                            ": Packet too short or invalid while reading "
                            "question key. (Maybe a UTF-8 problem?)");
            goto fail;
        }
        ...
        if (avahi_dns_packet_get_field(p, AVAHI_DNS_FIELD_ANCOUNT) == 0 &&
            !(avahi_dns_packet_get_field(p, AVAHI_DNS_FIELD_FLAGS) &
              AVAHI_DNS_FLAG_TC))
    ...
    if (!legacy_unicast)
    {
        ...
        for (n = avahi_dns_packet_get_field(p, AVAHI_DNS_FIELD_ANCOUNT); n > 0;
             n--)
        {
            AvahiRecord* record;
            int unique = 0;
            ...
            {
                avahi_log_debug(
                    __FILE__ ": Packet too short or invalid while reading "
                             "known answer record. (Maybe a UTF-8 problem?)");
                goto fail;
            }
        ...
        /* Probe record */
        for (n = avahi_dns_packet_get_field(p, AVAHI_DNS_FIELD_NSCOUNT); n > 0;
             n--)
        {
            AvahiRecord* record;
            int unique = 0;
            ...
            if (!(record = avahi_dns_packet_consume_record(p, &unique)))
            {
                avahi_log_debug(__FILE__
                                ": Packet too short or invalid while reading "
                                "probe record. (Maybe a UTF-8 problem?)");
                goto fail;
            }
...
static void handle_response_packet(AvahiServer* s, AvahiDnsPacket* p,
                                   AvahiInterface* i, const AvahiAddress* a,
                                   int from_local_iface)
{
    unsigned n;
    ...
    for (n = avahi_dns_packet_get_field(p, AVAHI_DNS_FIELD_ANCOUNT) +
             avahi_dns_packet_get_field(p, AVAHI_DNS_FIELD_ARCOUNT);
         n > 0; n--)
    {
        AvahiRecord* record;
        int cache_flush = 0;
        ...
        if (!(record = avahi_dns_packet_consume_record(p, &cache_flush)))
        {
            avahi_log_debug(__FILE__
                            ": Packet too short or invalid while reading "
                            "response record. (Maybe a UTF-8 problem?)");
            break;
        }
        ...
        if (!avahi_key_is_pattern(record->key))
        {
            ...
            {
                if (!from_local_iface &&
                    !avahi_record_is_link_local_address(record))
                    reflect_response(s, i, record, cache_flush);
                avahi_cache_update(i->cache, record, cache_flush, a);
                avahi_response_scheduler_incoming(i->response_scheduler, record,
                                                  cache_flush);
            }
        }
    ...
    if (!s->legacy_unicast_reflect_slots)
        s->legacy_unicast_reflect_slots =
            avahi_new0(AvahiLegacyUnicastReflectSlot*,
                       AVAHI_LEGACY_UNICAST_REFLECT_SLOTS_MAX);
...
static AvahiLegacyUnicastReflectSlot* find_slot(AvahiServer* s, uint16_t id)
{
    ...
    if (!s->legacy_unicast_reflect_slots[idx] ||
        s->legacy_unicast_reflect_slots[idx]->id != id)
        return NULL;

    return s->legacy_unicast_reflect_slots[idx];
}

static void legacy_unicast_reflect_slot_timeout(AvahiTimeEvent* e,
                                                void* userdata)
{
...
static void reflect_legacy_unicast_query_packet(AvahiServer* s,
                                                AvahiDnsPacket* p,
                                                AvahiInterface* i,
                                                ...
                                                uint16_t port)
{
    ...
    if (!(slot = allocate_slot(s)))
    {
        ...
        avahi_log_warn("No slot available for legacy unicast reflection, "
    ...
    slot->time_event =
        avahi_time_event_new(s->time_event_queue, &slot->elapse_time,
                             legacy_unicast_reflect_slot_timeout, slot);
    ...
    for (j = s->monitor->interfaces; j; j = j->interface_next)
            ...
            (s->config.reflect_ipv || j->protocol == i->protocol))
        {
            ...
            if (j->protocol == AVAHI_PROTO_INET &&
                s->fd_legacy_unicast_ipv4 >= 0)
            {
                avahi_send_dns_packet_ipv4(s->fd_legacy_unicast_ipv4,
                                           j->hardware->index, p, NULL, NULL,
                                           0);
            }
            else if (j->protocol == AVAHI_PROTO_INET6 &&
                     s->fd_legacy_unicast_ipv6 >= 0)
                avahi_send_dns_packet_ipv6(s->fd_legacy_unicast_ipv6,
                                           j->hardware->index, p, NULL, NULL,
                                           0);
        }
...
static int originates_from_local_legacy_unicast_socket(
    AvahiServer* s, const AvahiAddress* address, uint16_t port)
{
    assert(s);
    assert(address);
    assert(port > 0);
    ...
    if (address->proto == AVAHI_PROTO_INET && s->fd_legacy_unicast_ipv4 >= 0)
    {
        ...
        if (getsockname(s->fd_legacy_unicast_ipv4, (struct sockaddr*)&lsa,
                        &l) != 0)
    ...
    if (address->proto == AVAHI_PROTO_INET6 && s->fd_legacy_unicast_ipv6 >= 0)
    {
        ...
        if (getsockname(s->fd_legacy_unicast_ipv6, (struct sockaddr*)&lsa,
                        &l) != 0)
            avahi_log_warn("getsockname(): %s", strerror(errno));
        else
            return avahi_port_from_sockaddr((struct sockaddr*)&lsa) == port;
...
static int is_mdns_mcast_address(const AvahiAddress* a)
{
    ...
    avahi_address_parse(a->proto == AVAHI_PROTO_INET ? AVAHI_IPV4_MCAST_GROUP
                                                     : AVAHI_IPV6_MCAST_GROUP,
                        a->proto, &b);
    return avahi_address_cmp(a, &b) == 0;
}

static int originates_from_local_iface(AvahiServer* s, AvahiIfIndex iface,
...
static void dispatch_packet(AvahiServer* s, AvahiDnsPacket* p,
                            const AvahiAddress* src_address, uint16_t port,
                            const AvahiAddress* dst_address, AvahiIfIndex iface,
                            int ttl)
{
    AvahiInterface* i;
    ...
    if (!(i = avahi_interface_monitor_get_interface(s->monitor, iface,
                                                    src_address->proto)) ||
        !i->announcing)
    {
    ...
    if (port <= 0)
    {
        ...
        avahi_log_debug("Received packet from invalid source port %u.",
                        (unsigned)port);
        return;
    }
    ...
    /* We don't want to reflect local traffic, so we check if this packet is
     * generated locally. */
    if (s->config.enable_reflector)
        from_local_iface =
            originates_from_local_iface(s, iface, src_address, port);
    ...
    if (avahi_dns_packet_is_query(p))
    {
        ...
        handle_query_packet(s, p, i, src_address, port, legacy_unicast,
                            from_local_iface);
    }
    else
    {
        ...
        if (port != AVAHI_MDNS_PORT)
        {
            avahi_log_debug("Received response from host %s with invalid "
                            "source port %u on interface '%s.%i'",
                            avahi_address_snprint(t, sizeof(t), src_address),
        ...
        {
            avahi_log_debug("Received response from host %s with invalid TTL "
                            "%u on interface '%s.%i'.",
                            avahi_address_snprint(t, sizeof(t), src_address),
            ...
            !avahi_interface_address_on_link(i, src_address))
        {
            ...
            avahi_log_debug("Received non-local response from host %s on "
                            "interface '%s.%i'.",
                            avahi_address_snprint(t, sizeof(t), src_address),
                            i->hardware->name, i->protocol);
            return;
...
static void dispatch_legacy_unicast_packet(AvahiServer* s, AvahiDnsPacket* p)
{
    ...
    if (!(slot =
              find_slot(s, avahi_dns_packet_get_field(p, AVAHI_DNS_FIELD_ID))))
    {
        avahi_log_debug("Received legacy unicast response with unknown id");
        return;
    }

    if (!(j = avahi_interface_monitor_get_interface(s->monitor, slot->interface,
                                                    slot->address.proto)) ||
        !j->announcing)
        return;
                               ...
                               void* userdata)
{
    ...
    if (fd == s->fd_ipv4)
    {
        ...
        p = avahi_recv_dns_packet_ipv4(s->fd_ipv4, &src.data.ipv4, &port,
                                       &dest.data.ipv4, &iface, &ttl);
    }
    else
    {
        assert(fd == s->fd_ipv6);
        dest.proto = src.proto = AVAHI_PROTO_INET6;
        p = avahi_recv_dns_packet_ipv6(s->fd_ipv6, &src.data.ipv6, &port,
                                       &dest.data.ipv6, &iface, &ttl);
    }
    ...
    if (p)
    {
        ...
        if (iface != AVAHI_IF_UNSPEC)
            ...
            avahi_log_error(
                "Incoming packet received on address that isn't local.");
...
static void legacy_unicast_socket_event(AvahiWatch* w, int fd,
                                        AvahiWatchEvent events, void* userdata)
{
    ...
    if (fd == s->fd_legacy_unicast_ipv4)
        p = avahi_recv_dns_packet_ipv4(s->fd_legacy_unicast_ipv4, NULL, NULL,
                                       NULL, NULL, NULL);
    else
    {
        assert(fd == s->fd_legacy_unicast_ipv6);
        p = avahi_recv_dns_packet_ipv6(s->fd_legacy_unicast_ipv6, NULL, NULL,
                                       NULL, NULL, NULL);
    }
...
void avahi_host_rr_entry_group_callback(AvahiServer* s, AvahiSEntryGroup* g,
...
static void register_hinfo(AvahiServer* s)
{
    ...
    if (s->hinfo_entry_group)
        ...
        s->hinfo_entry_group = avahi_s_entry_group_new(
            s, avahi_host_rr_entry_group_callback, NULL);

    if (!s->hinfo_entry_group)
    {
        avahi_log_warn("Failed to create HINFO entry group: %s",
                       avahi_strerror(s->error));
        return;
    }

    /* Fill in HINFO rr */
    if ((r = avahi_record_new_full(s->host_name_fqdn, AVAHI_DNS_CLASS_IN,
                                   ...
                                   AVAHI_DEFAULT_TTL_HOST_NAME)))
    {
        ...
        else
        {
            ...
            avahi_log_info("Registering HINFO record with values '%s'/'%s'.",
                           r->data.hinfo.cpu, r->data.hinfo.os);

            if (avahi_server_add(s, s->hinfo_entry_group, AVAHI_IF_UNSPEC,
                                 AVAHI_PROTO_UNSPEC, AVAHI_PUBLISH_UNIQUE,
                                 r) < 0)
            {
                avahi_log_warn("Failed to add HINFO RR: %s",
                               avahi_strerror(s->error));
                return;
            }
        }
    ...
    if (avahi_s_entry_group_commit(s->hinfo_entry_group) < 0)
        avahi_log_warn("Failed to commit HINFO entry group: %s",
                       avahi_strerror(s->error));
}
...
static void register_localhost(AvahiServer* s)
{
    ...
    avahi_server_add_address(s, NULL, AVAHI_IF_UNSPEC, AVAHI_PROTO_UNSPEC,
                             AVAHI_PUBLISH_NO_PROBE | AVAHI_PUBLISH_NO_ANNOUNCE,
                             "localhost", &a);
...
static void register_browse_domain(AvahiServer* s)
{
    ...
    if (!s->browse_domain_entry_group)
    {
        avahi_log_warn("Failed to create browse domain entry group: %s",
                       avahi_strerror(s->error));
        return;
    }

    if (avahi_server_add_ptr(s, s->browse_domain_entry_group, AVAHI_IF_UNSPEC,
                             AVAHI_PROTO_UNSPEC, 0, AVAHI_DEFAULT_TTL,
                             "b._dns-sd._udp.local", s->domain_name) < 0)
    {
        avahi_log_warn("Failed to add browse domain RR: %s",
                       avahi_strerror(s->error));
        return;
    }

    if (avahi_s_entry_group_commit(s->browse_domain_entry_group) < 0)
        avahi_log_warn("Failed to commit browse domain entry group: %s",
                       avahi_strerror(s->error));
}
...
static void register_stuff(AvahiServer* s)
{
    ...
    s->n_host_rr_pending++; /** Make sure that the state isn't changed tp
                               AVAHI_SERVER_RUNNING too early */
...
int avahi_server_set_host_name(AvahiServer* s, const char* host_name)
{
    ...
    AVAHI_CHECK_VALIDITY(s, !host_name || avahi_is_valid_host_name(host_name),
    ...
    if (avahi_domain_equal(s->host_name, hn) &&
...
int avahi_server_set_domain_name(AvahiServer* s, const char* domain_name)
{
    ...
    AVAHI_CHECK_VALIDITY(
        s, !domain_name || avahi_is_valid_domain_name(domain_name),
...
static int setup_sockets(AvahiServer* s)
{
    ...
    s->fd_ipv4 = s->config.use_ipv4
                     ? avahi_open_socket_ipv4(s->config.disallow_other_stacks)
                     : -1;
    s->fd_ipv6 = s->config.use_ipv6
                     ? avahi_open_socket_ipv6(s->config.disallow_other_stacks)
                     : -1;
    ...
    s->fd_legacy_unicast_ipv4 = s->fd_ipv4 >= 0 && s->config.enable_reflector
                                    ? avahi_open_unicast_socket_ipv4()
                                    : -1;
    s->fd_legacy_unicast_ipv6 = s->fd_ipv6 >= 0 && s->config.enable_reflector
                                    ? avahi_open_unicast_socket_ipv6()
                                    : -1;

    s->watch_ipv4 = s->watch_ipv6 = s->watch_legacy_unicast_ipv4 =
        s->watch_legacy_unicast_ipv6 = NULL;

    if (s->fd_ipv4 >= 0)
        s->watch_ipv4 = s->poll_api->watch_new(
            s->poll_api, s->fd_ipv4, AVAHI_WATCH_IN, mcast_socket_event, s);
    if (s->fd_ipv6 >= 0)
        s->watch_ipv6 = s->poll_api->watch_new(
            s->poll_api, s->fd_ipv6, AVAHI_WATCH_IN, mcast_socket_event, s);

    if (s->fd_legacy_unicast_ipv4 >= 0)
        s->watch_legacy_unicast_ipv4 = s->poll_api->watch_new(
            s->poll_api, s->fd_legacy_unicast_ipv4, AVAHI_WATCH_IN,
            legacy_unicast_socket_event, s);
    if (s->fd_legacy_unicast_ipv6 >= 0)
        s->watch_legacy_unicast_ipv6 = s->poll_api->watch_new(
            s->poll_api, s->fd_legacy_unicast_ipv6, AVAHI_WATCH_IN,
            legacy_unicast_socket_event, s);

    return 0;
}

AvahiServer* avahi_server_new(const AvahiPoll* poll_api,
                              const AvahiServerConfig* sc,
                              AvahiServerCallback callback, void* userdata,
                              int* error)
{
    AvahiServer* s;
    int e;
    ...
    s->entries_by_key =
        avahi_hashmap_new((AvahiHashFunc)avahi_key_hash,
                          (AvahiEqualFunc)avahi_key_equal, NULL, NULL);
    AVAHI_LLIST_HEAD_INIT(AvahiEntry, s->entries);
    AVAHI_LLIST_HEAD_INIT(AvahiGroup, s->groups);

    s->record_browser_hashmap =
        avahi_hashmap_new((AvahiHashFunc)avahi_key_hash,
                          (AvahiEqualFunc)avahi_key_equal, NULL, NULL);
    ...
    /* Get host name */
    s->host_name = s->config.host_name
                       ? avahi_normalize_name_strdup(s->config.host_name)
                       : avahi_get_host_name_strdup();
    s->host_name[strcspn(s->host_name, ".")] = 0;
    s->domain_name = s->config.domain_name
                         ? avahi_normalize_name_strdup(s->config.domain_name)
                         : avahi_strdup("local");
    ...
    if (s->config.enable_wide_area)
    {
        ...
        avahi_wide_area_set_servers(s->wide_area_lookup_engine,
                                    s->config.wide_area_servers,
                                    s->config.n_wide_area_servers);
    }
    else
...
static AvahiEntry* find_entry(AvahiServer* s, AvahiIfIndex interface,
                              AvahiProtocol protocol, AvahiKey* key)
{
    AvahiEntry* e;
    ...
    for (e = avahi_hashmap_lookup(s->entries_by_key, key); e;
        ...
        if ((e->interface == interface || e->interface <= 0 ||
             interface <= 0) &&
            (e->protocol == protocol || e->protocol == AVAHI_PROTO_UNSPEC ||
             protocol == AVAHI_PROTO_UNSPEC) &&
            (!e->group || e->group->state == AVAHI_ENTRY_GROUP_ESTABLISHED ||
...
int avahi_server_get_group_of_service(AvahiServer* s, AvahiIfIndex interface,
                                      AvahiProtocol protocol, const char* name,
                                      const char* type, const char* domain,
                                      AvahiSEntryGroup** ret_group)
{
    AvahiKey* key = NULL;
    AvahiEntry* e;
    int ret;
    ...
    AVAHI_CHECK_VALIDITY(s, AVAHI_IF_VALID(interface),
                         AVAHI_ERR_INVALID_INTERFACE);
    AVAHI_CHECK_VALIDITY(s, AVAHI_PROTO_VALID(protocol),
                         AVAHI_ERR_INVALID_PROTOCOL);
    AVAHI_CHECK_VALIDITY(s, avahi_is_valid_service_name(name),
                         AVAHI_ERR_INVALID_SERVICE_NAME);
    AVAHI_CHECK_VALIDITY(s, avahi_is_valid_service_type_strict(type),
                         AVAHI_ERR_INVALID_SERVICE_TYPE);
    AVAHI_CHECK_VALIDITY(s, !domain || avahi_is_valid_domain_name(domain),
                         AVAHI_ERR_INVALID_DOMAIN_NAME);
...
int avahi_server_is_service_local(AvahiServer* s, AvahiIfIndex interface,
                                  AvahiProtocol protocol, const char* name)
{
    AvahiKey* key = NULL;
    AvahiEntry* e;
...
int avahi_server_is_record_local(AvahiServer* s, AvahiIfIndex interface,
                                 AvahiProtocol protocol, AvahiRecord* record)
{
    AvahiEntry* e;
    ...
    for (e = avahi_hashmap_lookup(s->entries_by_key, record->key); e;
         e = e->by_key_next)
...
int avahi_server_set_wide_area_servers(AvahiServer* s, const AvahiAddress* a,
                                       unsigned n)
{
    assert(s);
