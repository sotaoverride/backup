...
static ReadStatus read_ints(int fd, int* buf, int n_ints_in_buf,
                            int* n_ints_read, DBusError* error)
{
    size_t bytes = 0;
    ReadStatus retval;
    ...
    again:
        ...
        if (chunk < 0)
        {
            dbus_set_error(error, DBUS_ERROR_SPAWN_FAILED,
...
struct DBusBabysitter
{
    ...
    DBusSocket
        socket_to_babysitter;  /**< Connection to the babysitter process */
    int error_pipe_from_child; /**< Connection to the process that does the
                                  exec() */
    ...
    unsigned int
        have_child_status : 1; /**< True if child status has been reaped */
    unsigned int
        have_fork_errnum : 1; /**< True if we have an error code from fork() */
    unsigned int
        have_exec_errnum : 1; /**< True if we have an error code from exec() */
};
    ...
    if (got == 1)
    {
        switch (what)
        {
            ...
            case CHILD_EXEC_FAILED:
            {
                ...
                {
                    case READ_STATUS_ERROR:
                        _dbus_warn("Failed to read arg from fd %d: %s", fd,
                                   error.message);
                        dbus_error_free(&error);
                        return r;
                    case READ_STATUS_EOF:
                ...
                if (got == 1)
                {
                    if (what == CHILD_EXITED)
                    {
                         ...
                         * the babysitter reports that the grandchild process
                         * has exited, and there are two ways that can happen:
                         *
                         * 1. grandchild successfully exec()s the desired
                         * process, but then the desired process exits or is
                         * terminated by a signal. The babysitter observes this
                         * and reports CHILD_EXITED.
                         *
                        ...
                        sitter->status = arg;
                        _dbus_verbose(
                            "recorded child status exited = %d signaled = %d "
                            "exitstatus = %d termsig = %d\n",
            ...
            case CHILD_PID:
            {
                ...
                {
                    case READ_STATUS_ERROR:
                        _dbus_warn("Failed to read PID from fd %d: %s", fd,
                                   error.message);
                        dbus_error_free(&error);
                        return r;
                    case READ_STATUS_EOF:
...
static void handle_babysitter_socket(DBusBabysitter* sitter, int revents)
{
    ...
    if (revents & _DBUS_POLLIN)
    {
        ...
        if (read_data(sitter, sitter->socket_to_babysitter.fd) !=
            READ_STATUS_OK)
            close_socket_to_babysitter(sitter);
    }
...
static dbus_bool_t babysitter_iteration(DBusBabysitter* sitter,
                                        dbus_bool_t block)
{
    DBusPollFD fds[2];
    int i;
...
#define LIVE_CHILDREN(sitter)                                                  \
    ((sitter)->socket_to_babysitter.fd >= 0 ||                                 \
     (sitter)->error_pipe_from_child >= 0)

/**
 * Blocks until the babysitter process gives us the PID of the spawned
 * grandchild, then kills the spawned grandchild.
 *
...
void _dbus_babysitter_kill_child(DBusBabysitter* sitter)
{
    ...
    while (LIVE_CHILDREN(sitter) && sitter->grandchild_pid == -1)
                                                   ...
                                                   int* status)
{
    ...
    if (!sitter->have_child_status || !(WIFEXITED(sitter->status)))
        return FALSE;
                                           ...
                                           DBusError* error)
{
    ...
    if (sitter->have_exec_errnum)
    {
        dbus_set_error(error, DBUS_ERROR_SPAWN_EXEC_FAILED,
                       "Failed to execute program %s: %s", sitter->log_name,
                       _dbus_strerror(sitter->errnum));
    }
    else if (sitter->have_fork_errnum)
    {
        dbus_set_error(error, DBUS_ERROR_NO_MEMORY,
                       "Failed to fork a new process %s: %s", sitter->log_name,
                       _dbus_strerror(sitter->errnum));
    }
    else if (sitter->have_child_status)
    {
        if (WIFEXITED(sitter->status))
            dbus_set_error(error, DBUS_ERROR_SPAWN_CHILD_EXITED,
                           "Process %s exited with status %d", sitter->log_name,
                           WEXITSTATUS(sitter->status));
        else if (WIFSIGNALED(sitter->status))
            dbus_set_error(error, DBUS_ERROR_SPAWN_CHILD_SIGNALED,
                           "Process %s received signal %d", sitter->log_name,
                           WTERMSIG(sitter->status));
        else
            dbus_set_error(error, DBUS_ERROR_FAILED,
                           "Process %s exited abnormally", sitter->log_name);
    }
    else
    {
        dbus_set_error(error, DBUS_ERROR_FAILED,
                       "Process %s exited, reason unknown", sitter->log_name);
    }
}
...
dbus_bool_t _dbus_babysitter_set_watch_functions(
    DBusBabysitter* sitter, DBusAddWatchFunction add_function,
    DBusRemoveWatchFunction remove_function,
    DBusWatchToggledFunction toggled_function, void* data,
    DBusFreeFunction free_data_function)
{
    return _dbus_watch_list_set_functions(sitter->watches, add_function,
                                          remove_function, toggled_function,
                                          data, free_data_function);
}

static dbus_bool_t handle_watch(DBusWatch* watch, unsigned int condition,
                                void* data)
{
    ...
    /* fd.o #32992: if the handle_* methods closed their sockets, they
     * previously
     * didn't always remove the watches. Check that we don't regress. */
    _dbus_assert(sitter->socket_to_babysitter.fd != -1 ||
                 sitter->sitter_watch == NULL);
    _dbus_assert(sitter->error_pipe_from_child != -1 ||
                 sitter->error_watch == NULL);
...
static void do_exec(int child_err_report_fd, char* const* argv,
                    char* const* envp, DBusSpawnChildSetupFunc child_setup,
 ...
 * @param log_name the name under which to log messages about this process being
 * spawned
...
dbus_bool_t _dbus_spawn_async_with_babysitter(
    DBusBabysitter** sitter_p, const char* log_name, char* const* argv,
    char** env, DBusSpawnFlags flags, DBusSpawnChildSetupFunc child_setup,
    void* user_data, DBusError* error)
{
    ...
    if (!_dbus_socketpair(&babysitter_pipe[0], &babysitter_pipe[1], TRUE,
                          error))
        goto cleanup_and_fail;
     ...
     */
    sitter->error_watch =
        _dbus_watch_new(child_err_report_pipe[READ_END], DBUS_WATCH_READABLE,
    ...
    sitter->sitter_watch =
        _dbus_watch_new(babysitter_pipe[0].fd, DBUS_WATCH_READABLE, TRUE,
                        handle_watch, sitter, NULL);
    if (sitter->sitter_watch == NULL)
    {
        dbus_set_error(error, DBUS_ERROR_NO_MEMORY, NULL);
        goto cleanup_and_fail;
    ...
    if (pid < 0)
    {
        dbus_set_error(error, DBUS_ERROR_SPAWN_FORK_FAILED,
                       "Failed to fork (%s)", _dbus_strerror(errno));
        goto cleanup_and_fail;
    }
    else if (pid == 0)
    {
...
#endif
            do_exec(child_err_report_pipe[WRITE_END], argv, env, child_setup,
                    user_data);
