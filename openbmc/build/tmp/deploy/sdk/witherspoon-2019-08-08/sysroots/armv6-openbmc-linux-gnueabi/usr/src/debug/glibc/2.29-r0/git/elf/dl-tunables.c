...
static char* tunables_strdup(const char* in)
{
    ...
    /* FIXME: In reality if the allocation fails, __sbrk will crash attempting
       to set the thread-local errno since the TCB has not yet been set up. This
...
static void do_tunable_update_val(tunable_t* cur, const void* valp)
{
    uint64_t val;
...
void __tunable_set_val(tunable_id_t id, void* valp)
{
       ...
       colon.  */
        ...
        for (size_t i = 0; i < sizeof(tunable_list) / sizeof(tunable_t); i++)
        {
            ...
            if (tunable_is_name(cur->name, name))
            {
               ...
               tunable unless it is explicitly marked as secure.  Tunable values
               take precendence over their envvar aliases.  */
                if (__libc_enable_secure)
                {
                    if (cur->security_level == TUNABLE_SECLEVEL_SXID_ERASE)
                    {
                        if (p[len] == '\0')
                        {
                            /* Last tunable in the valstring.  Null-terminate
                               and return.  */
                            *name = '\0';
                            return;
                        }
                        ...
                        {
                            /* Remove the current tunable from the string.  We
                               do this by overwriting the string starting from
                               NAME (which is where the current tunable begins)
                               with the remainder of the string.  We then have P
                               point to NAME so that we continue in the correct
...
/* Enable the glibc.malloc.check tunable in SETUID/SETGID programs only when
   ...
   up.  */
static inline void __always_inline maybe_enable_malloc_check(void)
{
           ...
           NULL)
    {
        ...
        for (int i = 0; i < sizeof(tunable_list) / sizeof(tunable_t); i++)
        {
            ...
            if (tunable_is_name(name, envname))
            {
               ...
               settings of the tunable and decide whether we read the value and
               also whether we erase the value so that child processes don't
               inherit them in the environment.  */
                if (__libc_enable_secure)
                {
                    if (cur->security_level == TUNABLE_SECLEVEL_SXID_ERASE)
                    {
                       ...
                       again from the point we erased.  */
                        envp = prev_envp;
                    }
