...
static FT_Error ps_hint_table_ensure(PS_Hint_Table table, FT_UInt count,
                                     FT_Memory memory)
{
...
static FT_Error ps_hint_table_alloc(PS_Hint_Table table, FT_Memory memory,
                                    PS_Hint* ahint)
{
    FT_Error error = FT_Err_Ok;
    FT_UInt count;
...
static FT_Error ps_mask_table_ensure(PS_Mask_Table table, FT_UInt count,
                                     FT_Memory memory)
{
...
static FT_Error ps_mask_table_alloc(PS_Mask_Table table, FT_Memory memory,
                                    PS_Mask* amask)
{
    FT_UInt count;
    FT_Error error = FT_Err_Ok;
...
static FT_Error ps_mask_table_last(PS_Mask_Table table, FT_Memory memory,
                                   PS_Mask* amask)
{
    FT_Error error = FT_Err_Ok;
    FT_UInt count;
...
static FT_Error ps_mask_table_set_bits(PS_Mask_Table table,
                                       const FT_Byte* source, FT_UInt bit_pos,
                                       FT_UInt bit_count, FT_Memory memory)
{
    FT_Error error;
    PS_Mask mask;
...
static FT_Int ps_mask_table_test_intersect(PS_Mask_Table table, FT_UInt index1,
                                           FT_UInt index2)
{
...
static FT_Error ps_mask_table_merge(PS_Mask_Table table, FT_UInt index1,
                                    FT_UInt index2, FT_Memory memory)
{
    FT_Error error = FT_Err_Ok;
    ...
    if (index1 < index2 && index2 < table->num_masks)
    {
        ...
        if (delta > 0)
        {
            ...
            ft_memmove(mask2, mask2 + 1, (FT_UInt)delta * sizeof(PS_MaskRec));
...
static FT_Error ps_mask_table_merge_all(PS_Mask_Table table, FT_Memory memory)
{
    ...
    for (index1 = (FT_Int)table->num_masks - 1; index1 > 0; index1--)
    {
        ...
        {
            if (ps_mask_table_test_intersect(table, (FT_UInt)index1,
                                             (FT_UInt)index2))
            {
                error = ps_mask_table_merge(table, (FT_UInt)index2,
                                            (FT_UInt)index1, memory);
                if (error)
                    goto Exit;
...
static FT_Error ps_dimension_reset_mask(PS_Dimension dim, FT_UInt end_point,
                                        FT_Memory memory)
{
    PS_Mask mask;
                                           ...
                                           FT_UInt end_point, FT_Memory memory)
{
    ...
    error = ps_mask_table_set_bits(&dim->masks, source, source_pos, source_bits,
                                   memory);
...
static FT_Error ps_dimension_add_t1stem(PS_Dimension dim, FT_Int pos,
                                        FT_Int len, FT_Memory memory,
                                        FT_Int* aindex)
{
    FT_Error error = FT_Err_Ok;
    FT_UInt flags = 0;
...
static FT_Error ps_dimension_add_counter(PS_Dimension dim, FT_Int hint1,
                                         FT_Int hint2, FT_Int hint3,
                                         FT_Memory memory)
{
    FT_Error error = FT_Err_Ok;
...
static FT_Error ps_dimension_end(PS_Dimension dim, FT_UInt end_point,
                                 FT_Memory memory)
{
    /* end hint mask table */
    ps_dimension_end_mask(dim, end_point);
...
static void ps_hints_stem(PS_Hints hints, FT_UInt dimension, FT_Int count,
                          FT_Long* stems)
{
    PS_Dimension dim;
    ...
    for (; count > 0; count--, stems += 2)
    {
        ...
        error = ps_dimension_add_t1stem(dim, (FT_Int)stems[0], (FT_Int)stems[1],
                                        memory, NULL);
        if (error)
        {
        ...
        if (hints->hint_type == PS_HINT_TYPE_1)
        {
            ...
            for (count = 0; count < 3; count++, stems += 2)
            {
                error = ps_dimension_add_t1stem(
                    dim, (FT_Int)FIXED_TO_INT(stems[0]),
                    (FT_Int)FIXED_TO_INT(stems[1]), memory, &idx[count]);
                if (error)
                    goto Fail;
            }
            ...
            error =
                ps_dimension_add_counter(dim, idx[0], idx[1], idx[2], memory);
            if (error)
                goto Fail;
        }
        else
...
static void ps_hints_t1reset(PS_Hints hints, FT_UInt end_point)
{
    ...
    if (!hints->error)
    {
        ...
        if (hints->hint_type == PS_HINT_TYPE_1)
        {
            error = ps_dimension_reset_mask(&hints->dimension[0], end_point,
                                            memory);
            if (error)
                goto Fail;

            error = ps_dimension_reset_mask(&hints->dimension[1], end_point,
                                            memory);
            if (error)
                goto Fail;
        }
...
static void ps_hints_t2mask(PS_Hints hints, FT_UInt end_point,
                            FT_UInt bit_count, const FT_Byte* bytes)
{
    ...
    if (!hints->error)
    {
        ...
        error = ps_dimension_set_mask_bits(&dim[1], bytes, 0, count2, end_point,
                                           memory);
        if (error)
            goto Fail;
    }
...
static void ps_hints_t2counter(PS_Hints hints, FT_UInt bit_count,
                               const FT_Byte* bytes)
{
    FT_Error error;
    ...
    if (!hints->error)
    {
        ...
        /* set-up new horizontal and vertical hint mask now */
        error =
            ps_dimension_set_mask_bits(&dim[0], bytes, 0, count1, 0, memory);
        if (error)
            goto Fail;

        error = ps_dimension_set_mask_bits(&dim[1], bytes, count1, count2, 0,
                                           memory);
        if (error)
            goto Fail;
    }
...
static void t2_hints_stems(T2_Hints hints, FT_UInt dimension, FT_Int count,
                           FT_Fixed* coords)
{
    FT_Pos stems[32], y;
