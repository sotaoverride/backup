                                   ...
                                   kadm5_hook_handle** handles_out)
{
    ...
    *handles_out = NULL;
    ...
    for (mod = modules; *mod != NULL; mod++)
    {
        ...
        if (ret != 0)
        { /* Failed vtable init is non-fatal. */
            free(handle);
            handle = NULL;
            continue;
        }
...
static void log_failure(krb5_context context, const char* name,
                        const char* function, krb5_error_code ret)
{
    const char* e = krb5_get_error_message(context, ret);

    krb5_klog_syslog(LOG_ERR, _("kadm5_hook %s failed postcommit %s: %s"), name,
                     function, e);
...
kadm5_ret_t k5_kadm5_hook_chpass(krb5_context context,
                                 ...
                                 const char* newpass)
{
    ITERATE(chpass, (context, h->data, stage, princ, keepold, n_ks_tuple,
                     ks_tuple, newpass));
    return 0;
}
...
kadm5_ret_t k5_kadm5_hook_create(krb5_context context,
                                 ...
                                 const char* newpass)
{
    ITERATE(create, (context, h->data, stage, princ, mask, n_ks_tuple, ks_tuple,
                     newpass));
    return 0;
}
