...
/* Length of a single label, with all escaping removed, excluding any trailing
 * dot or NUL byte */
#define DNS_LABEL_MAX 63

/* Worst case length of a single label, with all escaping applied and room for a
 * trailing NUL byte. */
#define DNS_LABEL_ESCAPED_MAX (DNS_LABEL_MAX * 4 + 1)

/* Maximum length of a full hostname, consisting of a series of unescaped
 * labels, and no trailing dot or NUL byte */
...
{
    DNS_LABEL_LDH = 1 << 0, /* Follow the "LDH" rule — only letters, digits, and
                               internal hyphens. */
...
int dns_label_unescape(const char** name, char* dest, size_t sz,
                       DNSLabelFlags flags);
int dns_label_unescape_suffix(const char* name, const char** label_end,
                              char* dest, size_t sz);
...
static inline int dns_name_normalize(const char* s, DNSLabelFlags flags,
                                     char** ret)
{
...
int dns_service_join(const char* name, const char* type, const char* domain,
                     char** ret);
int dns_service_split(const char* joined, char** name, char** type,
                      char** domain);
