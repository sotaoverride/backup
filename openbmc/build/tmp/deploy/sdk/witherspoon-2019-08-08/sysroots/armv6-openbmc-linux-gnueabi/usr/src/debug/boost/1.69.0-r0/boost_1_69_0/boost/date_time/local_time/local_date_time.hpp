 ...
 * http://www.boost.org/LICENSE_1_0.txt) Author: Jeff Garland, Bart Garst $Date$
 */
...
struct BOOST_SYMBOL_VISIBLE ambiguous_result : public std::logic_error
{
    ambiguous_result(std::string const& msg = std::string()) :
        std::logic_error(
            std::string("Daylight Savings Results are ambiguous: " + msg))
    {
    }
};
...
struct BOOST_SYMBOL_VISIBLE dst_not_valid : public std::logic_error
{
    dst_not_valid(std::string const& msg = std::string()) :
        std::logic_error(std::string(
            "is_dst flag does not match resulting dst for time label given: " +
            msg))
    {
    }
};
...
class BOOST_SYMBOL_VISIBLE local_date_time_base
    : public date_time::base_time<utc_time_,
                                  boost::posix_time::posix_time_system>
{
  public:
    ...
    local_date_time_base(utc_time_type t, boost::shared_ptr<tz_type> tz) :
        date_time::base_time<utc_time_type, time_system_type>(t), zone_(tz)
    {
    ...
    local_date_time_base(date_type d, time_duration_type td,
        ...
        date_time::base_time<utc_time_type, time_system_type>(
            construction_adjustment(utc_time_type(d, td), tz, dst_flag)),
        zone_(tz)
    {
        if (tz != boost::shared_ptr<tz_type>() && tz->has_dst())
        {
            ...
            bool in_dst =
                (result == is_in_dst); // less processing than is_dst()
        ...
        zone_(tz)
    {
        ...
        if (result == ambiguous)
        {
            ...
            { // NADT on error
                this->time_ = posix_time::posix_time_system::get_time_rep(
                    date_type(date_time::not_a_date_time),
     ...
     */
    static time_is_dst_result check_dst(date_type d, time_duration_type td,
                                        boost::shared_ptr<tz_type> tz)
    {
        if (tz != boost::shared_ptr<tz_type>() && tz->has_dst())
        {
            typedef typename date_time::dst_calculator<date_type,
                                                       time_duration_type>
                dst_calculator;
            return dst_calculator::local_is_dst(
                d, td, tz->dst_local_start_time(d.year()).date(),
    ...
    bool is_dst() const
    {
        if (zone_ != boost::shared_ptr<tz_type>() && zone_->has_dst() &&
            !this->is_special())
        {
            ...
            switch (check_dst(lt.date(), lt.time_of_day(), zone_))
            {
                ...
                case is_in_dst:
                    return true;
                case ambiguous:
                    if (lt + zone_->dst_offset() <
                        zone_->dst_local_end_time(lt.date().year()))
                    {
                        return true;
                    }
                    break;
    ...
    //! returns a posix_time_zone string for the associated time_zone. If no
    //! time_zone, "UTC+00" is returned.
    ...
    /*bool operator==(const
    date_time::base_time<boost::posix_time::ptime,boost::posix_time::posix_time_system>&
    rhs) const { // fails due to rhs.time_ being protected return
    date_time::base_time<boost::posix_time::ptime,boost::posix_time::posix_time_system>::operator==(rhs);
    ...
    {
        return local_date_time_base(time_system_type::add_days(this->time_, dd),
                                    zone_);
    }
    //! Local_date_time += date_duration
    ...
    local_date_time_base operator-(const date_duration_type& dd) const
    {
        return local_date_time_base(
            time_system_type::subtract_days(this->time_, dd), zone_);
    }
    //! Local_date_time -= date_duration
    local_date_time_base operator-=(const date_duration_type& dd)
    {
    ...
    {
        return local_date_time_base(
            time_system_type::add_time_duration(this->time_, td), zone_);
    }
    //! Local_date_time += time_duration
    ...
    {
        return local_date_time_base(
            time_system_type::subtract_time_duration(this->time_, td), zone_);
    }
    //! Local_date_time -= time_duration
    local_date_time_base operator-=(const time_duration_type& td)
    {
  ...
  private:
                               ...
                               const std::string& separator) const
    {
        ...
        ss << std::setw(2) << std::setfill('0')
           << date_time::absolute_value(td.hours()) << separator << std::setw(2)
           << std::setfill('0') << date_time::absolute_value(td.minutes());
        return ss.str();
    }
};
