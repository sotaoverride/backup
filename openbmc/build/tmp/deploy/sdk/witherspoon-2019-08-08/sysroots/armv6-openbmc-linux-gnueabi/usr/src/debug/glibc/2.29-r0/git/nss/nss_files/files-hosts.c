...
LINE_PARSER("#", {
    ...
    /* Parse address.  */
    if (inet_pton(af == AF_UNSPEC ? AF_INET : af, addr, entdata->host_addr) > 0)
...
static enum nss_status gethostbyname3_multi(FILE* stream, const char* name,
                                            ...
                                            int flags)
{
    ...
    while (true)
    {
        status = internal_getent(stream, &tmp_result_buf, tmp_buffer.data,
                                 tmp_buffer.length, errnop, herrnop, af, flags);
    ...
    if (status == NSS_STATUS_SUCCESS)
    {
       ...
       copied again.  */
        ...
        char** out_addresses =
            alloc_buffer_alloc_array(&outbuf, char*, addresses_count + 1);
        char** out_aliases =
            alloc_buffer_alloc_array(&outbuf, char*, aliases_count + 1);
...
enum nss_status _nss_files_gethostbyname3_r(const char* name, int af,
                                            ...
                                            int32_t* ttlp, char** canonp)
{
    ...
    if (status == NSS_STATUS_SUCCESS)
    {
        ...
        if (status == NSS_STATUS_SUCCESS && _res_hconf.flags & HCONF_FLAG_MULTI)
            status = gethostbyname3_multi(stream, name, af, result, buffer,
                                          buflen, errnop, herrnop, flags);
                                           ...
                                           int* herrnop)
{
    ...
    return _nss_files_gethostbyname3_r(name, af, result, buffer, buflen, errnop,
                                       herrnop, NULL, NULL);
}

enum nss_status _nss_files_gethostbyname2_r(const char* name, int af,
                                            struct hostent* result,
                                            char* buffer, size_t buflen,
                                            int* errnop, int* herrnop)
{
...
enum nss_status _nss_files_gethostbyname4_r(const char* name,
                                            ...
                                            int32_t* ttlp)
{
    ...
    if (status == NSS_STATUS_SUCCESS)
    {
        ...
        while (1)
        {
            ...
            uintptr_t pad =
                (-(uintptr_t)buffer % __alignof__(struct hostent_data));
            buffer += pad;
            buflen = buflen > pad ? buflen - pad : 0;
