...
static OM_uint32 val_unwrap_aead_args(OM_uint32* minor_status,
...
static OM_uint32 gssint_unwrap_aead_iov_shim(
...
OM_uint32 gssint_unwrap_aead(gss_mechanism mech, OM_uint32* minor_status,
                             ...
                             int* conf_state, gss_qop_t* qop_state)
{
    ...
    if (mech->gss_unwrap_aead)
    {
        status = mech->gss_unwrap_aead(
            minor_status, ctx->internal_ctx_id, input_message_buffer,
            input_assoc_buffer, output_payload_buffer, conf_state, qop_state);
        if (status != GSS_S_COMPLETE)
            map_error(minor_status, mech);
    }
    else if (mech->gss_unwrap_iov)
    {
        status = gssint_unwrap_aead_iov_shim(
            mech, minor_status, ctx->internal_ctx_id, input_message_buffer,
            input_assoc_buffer, output_payload_buffer, conf_state, qop_state);
    }
    else
...
OM_uint32 KRB5_CALLCONV gss_unwrap_aead(minor_status, context_handle,
                                        input_message_buffer,
                                        input_assoc_buffer,
                                        output_payload_buffer, conf_state,
...
gss_qop_t* qop_state;
{
    ...
    return gssint_unwrap_aead(mech, minor_status, ctx, input_message_buffer,
