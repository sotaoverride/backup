...
static dbus_bool_t _dbus_type_writer_write_reader_partial(
    DBusTypeWriter* writer, DBusTypeReader* reader,
    const DBusTypeReader* start_after, int start_after_new_pos,
    int start_after_new_len, DBusList** fixups);
...
#if RECURSIVE_MARSHAL_WRITE_TRACE
            _dbus_verbose(" applying FIXUP to reader %p at pos %d new_len = %d "
                          "old len %d\n",
...
struct DBusTypeReaderClass
{
    ...
    dbus_bool_t (*check_finished)(
        const DBusTypeReader*
            reader); /**< check whether reader is at the end */
...
static void reader_init(DBusTypeReader* reader, int byte_order,
                        const DBusString* type_str, int type_pos,
                        const DBusString* value_str, int value_pos)
{
    _DBUS_ZERO(*reader);
...
static void base_reader_recurse(DBusTypeReader* sub, DBusTypeReader* parent)
{
    ...
    reader_init(sub, parent->byte_order, parent->type_str, parent->type_pos,
                parent->value_str, parent->value_pos);
}
                                                   ...
                                                   DBusTypeReader* parent)
{
    ...
    _dbus_assert(_dbus_string_get_byte(sub->type_str, sub->type_pos) ==
                     DBUS_STRUCT_BEGIN_CHAR ||
                 _dbus_string_get_byte(sub->type_str, sub->type_pos) ==
                     DBUS_DICT_ENTRY_BEGIN_CHAR);

    sub->type_pos += 1;
}

static void struct_or_dict_entry_reader_recurse(DBusTypeReader* sub,
                                                DBusTypeReader* parent)
{
...
static void array_types_only_reader_recurse(DBusTypeReader* sub,
                                            DBusTypeReader* parent)
{
    base_reader_recurse(sub, parent);
...
static void array_reader_recurse(DBusTypeReader* sub, DBusTypeReader* parent)
{
    ...
    _dbus_assert((sub->u.array.start_pos - (len_pos + 4)) <
                 8); /* only 3 bits in array_len_offset */
    sub->array_len_offset = sub->u.array.start_pos - (len_pos + 4);

#if RECURSIVE_MARSHAL_READ_TRACE
    _dbus_verbose("    type reader %p array start = %d len_offset = %d array "
                  "len = %d array element type = %s\n",
                  sub, sub->u.array.start_pos, sub->array_len_offset,
                  array_reader_get_array_len(sub),
                  _dbus_type_to_string(_dbus_first_type_in_signature(
...
static void variant_reader_recurse(DBusTypeReader* sub, DBusTypeReader* parent)
{
    ...
    contained_alignment = _dbus_type_get_alignment(
        _dbus_first_type_in_signature(sub->type_str, sub->type_pos));
...
static void base_reader_next(DBusTypeReader* reader, int current_type)
{
    switch (current_type)
    {
            ...
            if (!reader->klass->types_only)
                _dbus_marshal_skip_array(
                    ...
                    _dbus_first_type_in_signature(reader->type_str,
                                                  reader->type_pos + 1),
                    reader->byte_order, &reader->value_pos);
        ...
        default:
            if (!reader->klass->types_only)
                _dbus_marshal_skip_basic(reader->value_str, current_type,
                                         reader->byte_order,
                                         &reader->value_pos);
...
static void array_types_only_reader_next(DBusTypeReader* reader,
                                         int current_type)
{
...
#if RECURSIVE_MARSHAL_READ_TRACE
    _dbus_verbose("  reader %p array next START start_pos = %d end_pos = %d "
                  "value_pos = %d current_type = %s\n",
                  reader, reader->u.array.start_pos, end_pos, reader->value_pos,
...
#if RECURSIVE_MARSHAL_READ_TRACE
    _dbus_verbose("  reader %p array next END start_pos = %d end_pos = %d "
                  "value_pos = %d current_type = %s\n",
                  reader, reader->u.array.start_pos, end_pos, reader->value_pos,
                  _dbus_type_to_string(current_type));
#endif
...
static const DBusTypeReaderClass body_reader_class = {
    "body", 0,
    FALSE,  NULL, /* body is always toplevel, so doesn't get recursed into */
    NULL,   base_reader_next};
...
static const DBusTypeReaderClass struct_reader_class = {
    "struct",          2, FALSE, struct_or_dict_entry_reader_recurse, NULL,
    struct_reader_next};

static const DBusTypeReaderClass struct_types_only_reader_class = {
    "struct types", 3,
    TRUE,           struct_or_dict_entry_types_only_reader_recurse,
    NULL,           struct_reader_next};

static const DBusTypeReaderClass dict_entry_reader_class = {
    "dict_entry",          4, FALSE, struct_or_dict_entry_reader_recurse, NULL,
...
static const DBusTypeReaderClass array_types_only_reader_class = {
    "array types", 7,
    TRUE,          array_types_only_reader_recurse,
    NULL,          array_types_only_reader_next};
...
static const DBusTypeReaderClass* const all_reader_classes[] = {
    &body_reader_class,       &body_types_only_reader_class,
    &struct_reader_class,     &struct_types_only_reader_class,
    &dict_entry_reader_class, &dict_entry_types_only_reader_class,
    &array_reader_class,      &array_types_only_reader_class,
 ...
 */
void _dbus_type_reader_init(DBusTypeReader* reader, int byte_order,
                            const DBusString* type_str, int type_pos,
                            const DBusString* value_str, int value_pos)
{
...
#if RECURSIVE_MARSHAL_READ_TRACE
    _dbus_verbose(
        "  type reader %p init type_pos = %d value_pos = %d remaining sig "
        "'%s'\n",
...
void _dbus_type_reader_init_types_only(DBusTypeReader* reader,
                                       const DBusString* type_str, int type_pos)
{
    reader_init(reader, DBUS_COMPILER_BYTE_ORDER /* irrelevant */, type_str,
                type_pos, NULL, _DBUS_INT_MAX /* crashes if we screw up */);
...
int _dbus_type_reader_get_current_type(const DBusTypeReader* reader)
{
    ...
    if (reader->finished || (reader->klass->check_finished &&
...
void _dbus_type_reader_read_basic(const DBusTypeReader* reader, void* value)
{
    ...
    _dbus_marshal_read_basic(reader->value_str, reader->value_pos, t, value,
                             reader->byte_order, NULL);

#if RECURSIVE_MARSHAL_READ_TRACE
    _dbus_verbose(
        "  type reader %p read basic type_pos = %d value_pos = %d remaining "
        "sig '%s'\n",
        reader, reader->type_pos, reader->value_pos,
        _dbus_string_get_const_data_len(reader->type_str, reader->type_pos, 0));
#endif
                                        ...
                                        void* value, int* n_elements)
{
    ...
    element_type =
        _dbus_first_type_in_signature(reader->type_str, reader->type_pos);

    _dbus_assert(element_type !=
                 DBUS_TYPE_INVALID); /* why we don't use get_current_type() */
    ...
    if (remaining_len == 0)
        ...
        *(const DBusBasicValue**)value = (void*)_dbus_string_get_const_data_len(
            reader->value_str, reader->value_pos, remaining_len);
...
#if RECURSIVE_MARSHAL_READ_TRACE
    _dbus_verbose(
        "  type reader %p read fixed array type_pos = %d value_pos = %d "
        "remaining sig '%s'\n",
        reader, reader->type_pos, reader->value_pos,
        _dbus_string_get_const_data_len(reader->type_str, reader->type_pos, 0));
#endif
    ...
    switch (t)
    {
        ...
        default:
            ...
            if (t == DBUS_TYPE_INVALID)
                _dbus_warn_check_failed("You can't recurse into an empty array "
                                        "or off the end of a message body");
...
#if RECURSIVE_MARSHAL_READ_TRACE
    _dbus_verbose(
        "  type reader %p RECURSED type_pos = %d value_pos = %d remaining sig "
        "'%s'\n",
...
#if RECURSIVE_MARSHAL_READ_TRACE
    _dbus_verbose(
        "  type reader %p START next() { type_pos = %d value_pos = %d "
        "remaining sig '%s' current_type = %s\n",
        reader, reader->type_pos, reader->value_pos,
        _dbus_string_get_const_data_len(reader->type_str, reader->type_pos, 0),
        _dbus_type_to_string(t));
#endif
...
#if RECURSIVE_MARSHAL_READ_TRACE
    _dbus_verbose(
        "  type reader %p END next() type_pos = %d value_pos = %d remaining "
        "sig '%s' current_type = %s\n",
        reader, reader->type_pos, reader->value_pos,
        _dbus_string_get_const_data_len(reader->type_str, reader->type_pos, 0),
...
void _dbus_type_reader_get_signature(const DBusTypeReader* reader,
                                     const DBusString** str_p, int* start_p,
                                     int* len_p)
{
...
static dbus_bool_t replacement_block_init(ReplacementBlock* block,
...
static dbus_bool_t replacement_block_replace(ReplacementBlock* block,
                                             DBusTypeReader* reader,
                                             const DBusTypeReader* realign_root)
{
    ...
    _dbus_type_writer_init_values_only(
        &writer, realign_reader.byte_order, realign_reader.type_str,
        realign_reader.type_pos, &block->replacement,
        _dbus_string_get_length(&block->replacement));

    _dbus_assert(realign_reader.value_pos <= reader->value_pos);

#if RECURSIVE_MARSHAL_WRITE_TRACE
    _dbus_verbose("COPYING from reader at value_pos %d to writer %p starting "
                  "after value_pos %d\n",
                  realign_reader.value_pos, &writer, reader->value_pos);
#endif
    fixups = NULL;
    if (!_dbus_type_writer_write_reader_partial(
            &writer, &realign_reader, reader, block->padding,
    ...
    if (!_dbus_string_replace_len(
            &block->replacement, block->padding,
            _dbus_string_get_length(&block->replacement) - block->padding,
            (DBusString*)reader->value_str, reader->value_pos,
            realign_reader.value_pos - reader->value_pos))
        goto oom;
 ...
 */
static dbus_bool_t
    reader_set_basic_variable_length(DBusTypeReader* reader, int current_type,
                                     const void* value,
                                     const DBusTypeReader* realign_root)
{
    dbus_bool_t retval;
    ...
    if (!replacement_block_init(&block, reader))
        ...
        /* Write the new basic value */
#if RECURSIVE_MARSHAL_WRITE_TRACE
    _dbus_verbose("INITIALIZING writer %p to write basic value at value_pos %d "
                  "of replacement string\n",
                  &writer, _dbus_string_get_length(&block.replacement));
#endif
    _dbus_type_writer_init_values_only(
        &writer, reader->byte_order, reader->type_str, reader->type_pos,
        &block.replacement, _dbus_string_get_length(&block.replacement));
#if RECURSIVE_MARSHAL_WRITE_TRACE
    ...
    if (!replacement_block_replace(&block, reader, realign_root))
        goto out;
...
static void reader_set_basic_fixed_length(DBusTypeReader* reader,
                                          int current_type, const void* value)
{
    _dbus_marshal_set_basic((DBusString*)reader->value_str, reader->value_pos,
                            current_type, value, reader->byte_order, NULL,
                            NULL);
}
                                        ...
                                        const DBusTypeReader* realign_root)
{
...
#if RECURSIVE_MARSHAL_WRITE_TRACE
    _dbus_verbose(
        "  SET BASIC type reader %p type_pos = %d value_pos = %d remaining sig "
        "'%s' realign_root = %p with value_pos %d current_type = %s\n",
        reader, reader->type_pos, reader->value_pos,
        _dbus_string_get_const_data_len(reader->type_str, reader->type_pos, 0),
        realign_root, realign_root ? realign_root->value_pos : -1,
        _dbus_type_to_string(current_type));
    _dbus_verbose_bytes_of_string(
        realign_root->value_str, realign_root->value_pos,
    ...
    {
        _dbus_assert(realign_root != NULL);
        return reader_set_basic_variable_length(reader, current_type, value,
                                                realign_root);
    }
}
 ...
 * Returns #TRUE if the first parameter is further along than the second
 * parameter.
 *
 ...
 *
 */
void _dbus_type_writer_init(DBusTypeWriter* writer, int byte_order,
                            DBusString* type_str, int type_pos,
                            DBusString* value_str, int value_pos)
{
    ...
    _dbus_verbose("writer %p init remaining sig '%s'\n", writer,
                  writer->type_str ? _dbus_string_get_const_data_len(
                                         writer->type_str, writer->type_pos, 0)
                                   : "unknown");
#endif
}
...
void _dbus_type_writer_init_types_delayed(DBusTypeWriter* writer,
                                          int byte_order, DBusString* value_str,
                                          int value_pos)
{
 ...
 *
 */
void _dbus_type_writer_add_types(DBusTypeWriter* writer, DBusString* type_str,
                                 int type_pos)
{
 ...
 *
 */
void _dbus_type_writer_init_values_only(DBusTypeWriter* writer, int byte_order,
                                        const DBusString* type_str,
                                        int type_pos, DBusString* value_str,
                                        int value_pos)
{
    _dbus_type_writer_init(writer, byte_order, (DBusString*)type_str, type_pos,
                           value_str, value_pos);
...
static dbus_bool_t
    _dbus_type_writer_write_basic_no_typecode(DBusTypeWriter* writer, int type,
                                              const void* value)
{
    if (writer->enabled)
        return _dbus_marshal_write_basic(writer->value_str, writer->value_pos,
                                         type, value, writer->byte_order,
                                         &writer->value_pos);
    else
        return TRUE;
}
 ...
 */
static void writer_recurse_init_and_check(DBusTypeWriter* writer,
                                          int container_type,
                                          DBusTypeWriter* sub)
{
    _dbus_type_writer_init(sub, writer->byte_order, writer->type_str,
                           writer->type_pos, writer->value_str,
                           writer->value_pos);
    ...
    if (writer->type_pos_is_expectation ||
        (sub->container_type == DBUS_TYPE_ARRAY ||
         sub->container_type == DBUS_TYPE_VARIANT))
    ...
    if (writer->type_pos_is_expectation && writer->type_str)
    {
        ...
        expected =
            _dbus_first_type_in_signature(writer->type_str, writer->type_pos);

        if (expected != sub->container_type)
        {
            if (expected != DBUS_TYPE_INVALID)
                _dbus_warn_check_failed(
                    "Writing an element of type %s, but the expected type here "
                    "is %s\n"
                    "The overall signature expected here was '%s' and we are "
                    "on byte %d of that signature.",
                ...
                _dbus_warn_check_failed(
                    "Writing an element of type %s, but no value is expected "
                    "here\n"
                    "The overall signature expected here was '%s' and we are "
                    "on byte %d of that signature.",
                    _dbus_type_to_string(sub->container_type),
...
#if RECURSIVE_MARSHAL_WRITE_TRACE
    _dbus_verbose("  type writer %p recurse parent %s type_pos = %d value_pos "
                  "= %d is_expectation = %d remaining sig '%s' enabled = %d\n",
                  writer, _dbus_type_to_string(writer->container_type),
                  writer->type_pos, writer->value_pos,
                  writer->type_pos_is_expectation,
                  writer->type_str ? _dbus_string_get_const_data_len(
                                         writer->type_str, writer->type_pos, 0)
                                   : "unknown",
                  writer->enabled);
    _dbus_verbose("  type writer %p recurse sub %s   type_pos = %d value_pos = "
                  "%d is_expectation = %d enabled = %d\n",
                  sub, _dbus_type_to_string(sub->container_type), sub->type_pos,
                  sub->value_pos, sub->type_pos_is_expectation, sub->enabled);
#endif
}

static dbus_bool_t write_or_verify_typecode(DBusTypeWriter* writer,
                                            int typecode)
{
     ...
     */
#if RECURSIVE_MARSHAL_WRITE_TRACE
    _dbus_verbose("  type writer %p write_or_verify start type_pos = %d "
                  "remaining sig '%s' enabled = %d\n",
    ...
    if (writer->type_pos_is_expectation)
    {
#ifndef DBUS_DISABLE_CHECKS
        {
            ...
            expected =
                _dbus_string_get_byte(writer->type_str, writer->type_pos);

            if (expected != typecode)
            {
                if (expected != DBUS_TYPE_INVALID)
                    _dbus_warn_check_failed(
                        "Array or variant type requires that type %s be "
                        "written, but %s was written.\n"
                        "The overall signature expected here was '%s' and we "
                        "are on byte %d of that signature.",
                        _dbus_type_to_string(expected),
                    ...
                    _dbus_warn_check_failed(
                        "Array or variant type wasn't expecting any more "
                        "values to be written into it, but a value %s was "
                        "written.\n"
                        "The overall signature expected here was '%s' and we "
                        "are on byte %d of that signature.",
                        _dbus_type_to_string(typecode),
    ...
    {
        if (!_dbus_string_insert_byte(writer->type_str, writer->type_pos,
                                      typecode))
            return FALSE;
...
#if RECURSIVE_MARSHAL_WRITE_TRACE
    _dbus_verbose(
        "  type writer %p write_or_verify end type_pos = %d remaining sig "
        "'%s'\n",
        writer, writer->type_pos,
...
static dbus_bool_t writer_recurse_struct_or_dict_entry(
    DBusTypeWriter* writer, int begin_char, const DBusString* contained_type,
    int contained_type_start, int contained_type_len, DBusTypeWriter* sub)
{
    ...
    if (writer->enabled)
    {
        if (!_dbus_string_insert_bytes(
                ...
                _DBUS_ALIGN_VALUE(sub->value_pos, 8) - sub->value_pos, '\0'))
            _dbus_assert_not_reached("should not have failed to insert "
                                     "alignment padding for struct");
        sub->value_pos = _DBUS_ALIGN_VALUE(sub->value_pos, 8);
    }
...
static dbus_bool_t writer_recurse_array(DBusTypeWriter* writer,
                                        ...
                                        dbus_bool_t is_array_append)
{
    ...
    {
        if (!_dbus_string_equal_substring(contained_type, contained_type_start,
                                          contained_type_len, writer->type_str,
                                          writer->u.array.element_type_pos + 1))
        {
            _dbus_warn_check_failed(
                "Writing an array of '%s' but this is incompatible with the "
                "expected type of elements in the parent array",
    ...
    if (!writer->type_pos_is_expectation)
    {
        ...
        if (!_dbus_string_copy_len(contained_type, contained_type_start,
                                   contained_type_len, sub->type_str,
    ...
    if (writer->type_str != NULL)
    {
        /* If the parent is an array, we hold type_pos pointing at the array
         * element type; otherwise advance it to reflect the array value we just
         * recursed into
         */
        if (writer->container_type != DBUS_TYPE_ARRAY)
            ...
            _dbus_assert(
                writer->type_pos_is_expectation); /* because it's an array */
    }
    ...
    if (writer->enabled)
    {
        ...
        {
            if (!_dbus_type_writer_write_basic_no_typecode(
                    sub, DBUS_TYPE_UINT32, &value))
                _dbus_assert_not_reached(
         ...
         */
        alignment =
            element_type_get_alignment(contained_type, contained_type_start);
        ...
        if (aligned != sub->value_pos)
        {
            if (!is_array_append)
            {
                if (!_dbus_string_insert_bytes(sub->value_str, sub->value_pos,
                                               aligned - sub->value_pos, '\0'))
                    _dbus_assert_not_reached(
        ...
        if (is_array_append)
        {
            ...
            len = _dbus_unpack_uint32(
                sub->byte_order, _dbus_string_get_const_udata_len(
                                     sub->value_str, sub->u.array.len_pos, 4));
    ...
    {
        /* not enabled, so we won't write the len_pos; set it to -1 to so
         * indicate */
    ...
    _dbus_verbose("  type writer %p recurse array done remaining sig '%s' "
                  "array start_pos = %d len_pos = %d value_pos = %d\n",
                  sub,
                  sub->type_str ? _dbus_string_get_const_data_len(
                                      sub->type_str, sub->type_pos, 0)
                                : "unknown",
 ...
 */
static dbus_bool_t writer_recurse_variant(DBusTypeWriter* writer,
                                          const DBusString* contained_type,
                                          int contained_type_start,
                                          int contained_type_len,
                                          DBusTypeWriter* sub)
{
    ...
    if (!_dbus_string_insert_byte(sub->value_str, sub->value_pos,
                                  contained_type_len))
        _dbus_assert_not_reached(
    ...
    if (!_dbus_string_copy_len(contained_type, contained_type_start,
                               contained_type_len, sub->value_str,
                               sub->value_pos))
        _dbus_assert_not_reached(
    ...
    contained_alignment = _dbus_type_get_alignment(
        _dbus_first_type_in_signature(contained_type, contained_type_start));

    if (!_dbus_string_insert_bytes(
            sub->value_str, sub->value_pos,
            _DBUS_ALIGN_VALUE(sub->value_pos, contained_alignment) -
                sub->value_pos,
            '\0'))
        _dbus_assert_not_reached("should not have failed to insert alignment "
                                 "padding for variant body");
    sub->value_pos = _DBUS_ALIGN_VALUE(sub->value_pos, contained_alignment);
...
static dbus_bool_t _dbus_type_writer_recurse_contained_len(
    DBusTypeWriter* writer, int container_type,
    const DBusString* contained_type, int contained_type_start,
    int contained_type_len, DBusTypeWriter* sub, dbus_bool_t is_array_append)
{
    ...
    switch (container_type)
    {
        case DBUS_TYPE_STRUCT:
            return writer_recurse_struct_or_dict_entry(
                writer, DBUS_STRUCT_BEGIN_CHAR, contained_type,
                contained_type_start, contained_type_len, sub);
            break;
        case DBUS_TYPE_DICT_ENTRY:
            return writer_recurse_struct_or_dict_entry(
                writer, DBUS_DICT_ENTRY_BEGIN_CHAR, contained_type,
                contained_type_start, contained_type_len, sub);
            break;
        case DBUS_TYPE_ARRAY:
            return writer_recurse_array(
                writer, contained_type, contained_type_start,
                contained_type_len, sub, is_array_append);
            break;
        case DBUS_TYPE_VARIANT:
            return writer_recurse_variant(writer, contained_type,
                                          contained_type_start,
                                          contained_type_len, sub);
            break;
        default:
            _dbus_assert_not_reached(
...
/**
 * Opens a new container and writes out the initial information for that
 * container.
 *
                                      ...
                                      DBusTypeWriter* sub)
{
    ...
    if (contained_type)
        contained_type_len =
            find_len_of_complete_type(contained_type, contained_type_start);
    else
        contained_type_len = 0;

    return _dbus_type_writer_recurse_contained_len(
        writer, container_type, contained_type, contained_type_start,
        contained_type_len, sub, FALSE);
}
                                           ...
                                           DBusTypeWriter* sub)
{
    ...
    return _dbus_type_writer_recurse_contained_len(
        writer, DBUS_TYPE_ARRAY, contained_type, contained_type_start,
        contained_type_len, sub, TRUE);
}
                                        ...
                                        DBusTypeWriter* sub)
{
...
#if RECURSIVE_MARSHAL_WRITE_TRACE
    _dbus_verbose("  type writer %p unrecurse type_pos = %d value_pos = %d "
                  "is_expectation = %d container_type = %s\n",
                  writer, writer->type_pos, writer->value_pos,
                  writer->type_pos_is_expectation,
                  _dbus_type_to_string(writer->container_type));
    _dbus_verbose("  type writer %p unrecurse sub type_pos = %d value_pos = %d "
                  "is_expectation = %d container_type = %s\n",
                  sub, sub->type_pos, sub->value_pos,
    ...
    {
        if (sub->u.array.len_pos >=
            0) /* len_pos == -1 if we weren't enabled when we passed it */
        {
            dbus_uint32_t len;

            /* Set the array length */
            len = writer_get_array_len(sub);
            _dbus_marshal_set_uint32(sub->value_str, sub->u.array.len_pos, len,
                                     sub->byte_order);
#if RECURSIVE_MARSHAL_WRITE_TRACE
        ...
        {
            _dbus_verbose("    not filling in sub array len because we were "
                          "disabled when we passed the len\n");
        }
#endif
    }
...
#if RECURSIVE_MARSHAL_WRITE_TRACE
    _dbus_verbose("  type writer %p unrecursed type_pos = %d value_pos = %d "
                  "remaining sig '%s'\n",
                  writer, writer->type_pos, writer->value_pos,
                  writer->type_str ? _dbus_string_get_const_data_len(
                                         writer->type_str, writer->type_pos, 0)
 ...
 * @returns #FALSE if no memory
 */
dbus_bool_t _dbus_type_writer_write_basic(DBusTypeWriter* writer, int type,
                                          const void* value)
{
    dbus_bool_t retval;
...
out:
#if RECURSIVE_MARSHAL_WRITE_TRACE
    _dbus_verbose("  type writer %p basic type_pos = %d value_pos = %d "
                  "is_expectation = %d enabled = %d\n",
                  writer, writer->type_pos, writer->value_pos,
                  writer->type_pos_is_expectation, writer->enabled);
#endif
                                                ...
                                                int n_elements)
{
...
#if RECURSIVE_MARSHAL_WRITE_TRACE
    _dbus_verbose("  type writer %p entering fixed multi type_pos = %d "
                  "value_pos = %d n_elements %d\n",
    ...
    if (writer->enabled)
    {
        if (!_dbus_marshal_write_fixed_multi(
                writer->value_str, writer->value_pos, element_type, value,
                n_elements, writer->byte_order, &writer->value_pos))
            return FALSE;
    }

#if RECURSIVE_MARSHAL_WRITE_TRACE
    _dbus_verbose("  type writer %p fixed multi written new type_pos = %d new "
                  "value_pos = %d n_elements %d\n",
                  writer, writer->type_pos, writer->value_pos, n_elements);
#endif
...
static void enable_if_after(DBusTypeWriter* writer, DBusTypeReader* reader,
...
#if RECURSIVE_MARSHAL_WRITE_TRACE
            _dbus_verbose("ENABLING writer %p at %d because reader at "
                          "value_pos %d is after reader at value_pos %d\n",
                          writer, writer->value_pos, reader->value_pos,
                          start_after->value_pos);
#endif
        }

        _dbus_assert((!writer->enabled &&
                      !_dbus_type_reader_greater_than(reader, start_after)) ||
                     (writer->enabled &&
                      _dbus_type_reader_greater_than(reader, start_after)));
    }
...
static dbus_bool_t append_fixup(DBusList** fixups,
...
static dbus_bool_t writer_write_reader_helper(
    DBusTypeWriter* writer, DBusTypeReader* reader,
    const DBusTypeReader* start_after, int start_after_new_pos,
    int start_after_new_len, DBusList** fixups, dbus_bool_t inside_start_after)
{
    int current_type;
           ...
           DBUS_TYPE_INVALID)
    {
        if (dbus_type_is_container(current_type))
        {
            ...
            _dbus_type_reader_get_signature(&subreader, &sig_str, &sig_start,
                                            &sig_len);

#if RECURSIVE_MARSHAL_WRITE_TRACE
            _dbus_verbose(
                "about to recurse into %s reader at %d subreader at %d writer "
                "at %d start_after reader at %d write target len %d "
                "inside_start_after = %d this_is_start_after = %d\n",
                _dbus_type_to_string(current_type), reader->value_pos,
                subreader.value_pos, writer->value_pos,
                start_after ? start_after->value_pos : -1,
                _dbus_string_get_length(writer->value_str), inside_start_after,
                this_is_start_after);
#endif
            ...
            if (!_dbus_type_writer_recurse_contained_len(
                    writer, current_type, sig_str, sig_start, sig_len,
                    &subwriter, FALSE))
                goto oom;
            ...
            if (!writer_write_reader_helper(
                    &subwriter, &subreader, start_after, start_after_new_pos,
                    start_after_new_len, fixups,
                    inside_start_after || this_is_start_after))
                goto oom;

#if RECURSIVE_MARSHAL_WRITE_TRACE
            _dbus_verbose("about to unrecurse from %s subreader at %d writer "
                          "at %d subwriter at %d  write target len %d\n",
                          _dbus_type_to_string(current_type),
                          subreader.value_pos, writer->value_pos,
                          subwriter.value_pos,
             ...
             * a fixup.
             */
            if (start_after != NULL && !enabled_at_recurse &&
                past_start_after && current_type == DBUS_TYPE_ARRAY &&
                fixups != NULL)
            {
                ...
                old_len = _dbus_unpack_uint32(
                    reader->byte_order,
                    _dbus_string_get_const_udata_len(
                        reader->value_str, fixup.len_pos_in_reader, 4));
...
#if RECURSIVE_MARSHAL_WRITE_TRACE
                _dbus_verbose(
                    "Generated fixup len_pos_in_reader = %d new_len = %d "
                    "reader_array_start_pos = %d start_after->value_pos = %d "
                    "bytes_before_start_after = %d start_after_new_len = %d "
                    "bytes_written_after_start_after = %d\n",
                    fixup.len_pos_in_reader, fixup.new_len,
                    reader_array_start_pos, start_after->value_pos,
                    bytes_before_start_after, start_after_new_len,
                    bytes_written_after_start_after);
#endif
            }
        }
...
#if RECURSIVE_MARSHAL_WRITE_TRACE
            _dbus_verbose("Writing basic value %s at %d write target len %d "
                          "inside_start_after = %d\n",
                          _dbus_type_to_string(current_type), writer->value_pos,
                          _dbus_string_get_length(writer->value_str),
                          inside_start_after);
#endif
 ...
 * @param start_after_new_pos the position of start_after equivalent in the
 * target data
 * @param start_after_new_len the length of start_after equivalent in the target
 * data
...
dbus_bool_t _dbus_type_writer_write_reader(DBusTypeWriter* writer,
                                           DBusTypeReader* reader)
{
    return _dbus_type_writer_write_reader_partial(writer, reader, NULL, 0, 0,
                                                  NULL);
}
