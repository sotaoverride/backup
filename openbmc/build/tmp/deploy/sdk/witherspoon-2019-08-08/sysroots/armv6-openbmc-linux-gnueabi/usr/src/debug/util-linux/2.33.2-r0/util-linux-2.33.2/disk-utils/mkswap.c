 ...
 *               20.12.91 - time began. Got VM working yesterday by doing this
 * by hand.
 *
...
static void init_signature_page(struct mkswap_control* ctl)
{
    ...
    if (ctl->user_pagesize)
    {
        if (ctl->user_pagesize < 0 || !is_power_of_2(ctl->user_pagesize) ||
            ...
            errx(EXIT_FAILURE, _("Bad user-specified page size %u"),
                 ctl->user_pagesize);
...
static void set_signature(const struct mkswap_control* ctl)
{
    ...
    assert(sp);
    memcpy(sp + ctl->pagesize - SWAP_SIGNATURE_SZ, SWAP_SIGNATURE,
           SWAP_SIGNATURE_SZ);
}
...
static void set_uuid_and_label(const struct mkswap_control* ctl)
{
    ...
    if (ctl->opt_label)
    {
        xstrncpy(ctl->hdr->volume_name, ctl->opt_label,
                 sizeof(ctl->hdr->volume_name));
...
static void __attribute__((__noreturn__)) usage(void)
{
    ...
    fprintf(out,
            _("\nOptions:\n"
              " -c, --check               check bad blocks before creating the "
              "swap area\n"
              " -f, --force               allow swap size area be larger than "
              "device\n"
...
static void open_device(struct mkswap_control* ctl)
{
    ...
    {
        ctl->check = 0;
        warnx(_("warning: checking bad blocks from swap file is not supported: "
                "%s"),
              ctl->devname);
    }
}
...
static void wipe_device(struct mkswap_control* ctl)
{
    ...
    if (zap)
    {
        ...
        while (blkid_do_probe(pr) == 0)
        {
            ...
            if (blkid_probe_lookup_value(pr, "TYPE", &data, NULL) == 0 && data)
                warnx(_("%s: warning: wiping old %s signature."), ctl->devname,
                      data);
            blkid_do_wipe(pr, 0);
        }
#endif
    ...
    {
        warnx(_("%s: warning: don't erase bootbits sectors"), ctl->devname);
        if (type)
...
int main(int argc, char** argv)
{
    ...
    while ((c = getopt_long(argc, argv, "cfp:L:v:U:Vh", longopts, NULL)) != -1)
    {
        switch (c)
        {
            ...
            case 'p':
                ctl.user_pagesize =
                    strtou32_or_err(optarg, _("parsing page size failed"));
                break;
            case 'L':
            ...
            case 'v':
                version =
                    strtos32_or_err(optarg, _("parsing version number failed"));
    ...
    if (block_count)
    {
        ...
        uint64_t blks =
            strtou64_or_err(block_count, _("invalid block count argument"));
    ...
    if (ctl.npages < MIN_GOODPAGES)
        errx(EXIT_FAILURE, _("error: swap area needs to be at least %ld KiB"),
    ...
    if ((ctl.devstat.st_mode & permMask) != 0)
        warnx(_("%s: insecure permissions %04o, %04o suggested."), ctl.devname,
              ctl.devstat.st_mode & 07777, ~permMask & 0666);
    if (getuid() == 0 && S_ISREG(ctl.devstat.st_mode) &&
        ctl.devstat.st_uid != 0)
        warnx(_("%s: insecure file owner %d, 0 (root) suggested."), ctl.devname,
              ctl.devstat.st_uid);
    ...
    printf(
        _("Setting up swapspace version %d, size = %s (%" PRIu64 " bytes)\n"),
        version, strsz, sz);
    free(strsz);
    ...
    if (S_ISREG(ctl.devstat.st_mode) && is_selinux_enabled() > 0)
    {
        ...
        {
            if (errno != ENODATA)
                err(EXIT_FAILURE, _("%s: unable to obtain selinux file label"),
                    ctl.devname);
        ...
        if (strcmp(context_string, oldcontext) != 0)
        {
            if (fsetfilecon(ctl.fd, context_string) && errno != ENOTSUP)
                err(EXIT_FAILURE, _("unable to relabel %s to %s"), ctl.devname,
                    context_string);
        }
