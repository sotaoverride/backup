...
const struct utfuncs __libc_utmp_file_functions = {
    setutent_file,  getutent_r_file, getutid_r_file, getutline_r_file,
    pututline_file, endutent_file,   updwtmp_file};
...
static int setutent_file(void)
{
    if (file_fd < 0)
    {
        ...
        file_writable = false;
        file_fd =
            __open_nocancel(file_name, O_RDONLY | O_LARGEFILE | O_CLOEXEC);
        if (file_fd == -1)
            return 0;
    }
...
static int internal_getut_r(const struct utmp* id, struct utmp* buffer,
                            bool* lock_failed)
{
    int result = -1;
...
static int getutid_r_file(const struct utmp* id, struct utmp* buffer,
                          struct utmp** result)
{
    assert(file_fd >= 0);
...
/* For implementing this function we don't use the getutent_r function
   because we can avoid the reposition on every new entry this way.  */
static int getutline_r_file(const struct utmp* line, struct utmp* buffer,
                            struct utmp** result)
{
    assert(file_fd >= 0);
...
static struct utmp* pututline_file(const struct utmp* data)
{
    ...
    if (!file_writable)
    {
        ...
        int new_fd =
            __open_nocancel(file_name, O_RDWR | O_LARGEFILE | O_CLOEXEC);
        if (new_fd == -1)
            return NULL;

        if (__lseek64(new_fd, __lseek64(file_fd, 0, SEEK_CUR), SEEK_SET) ==
                -1 ||
...
static int updwtmp_file(const char* file, const struct utmp* utmp)
{
    ...
    /* Write the entry.  If we can't write all the bytes, reset the file
       ...
       will remain.  */
    if (__write_nocancel(fd, utmp, sizeof(struct utmp)) != sizeof(struct utmp))
    {
        __ftruncate64(fd, offset);
        goto unlock_return;
    }
