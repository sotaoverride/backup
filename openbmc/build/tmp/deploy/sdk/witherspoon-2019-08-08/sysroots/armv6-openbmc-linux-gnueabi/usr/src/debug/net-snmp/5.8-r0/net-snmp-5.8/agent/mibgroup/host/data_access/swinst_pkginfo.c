...
void netsnmp_swinst_arch_init(void)
{
    ...
    DEBUGMSGTL(
        ("swinst:init:arch", " package directory = %s\n", pkg_directory));
    return;
}
...
/* ---------------------------------------------------------------------
 */
int netsnmp_swinst_arch_load(netsnmp_container* container, u_int flags)
{
    DIR* d;
    struct dirent* dp;
    ...
    if (pkgng)
    {
        ...
        while (pkgdb_it_next(it, &pkg, PKG_LOAD_BASIC) == EPKG_OK)
        {
            ...
            entry->swName_len =
                snprintf(entry->swName, sizeof(entry->swName), "%s", pkgname);
...
#else
        entry->swName_len =
            snprintf(entry->swName, sizeof(entry->swName), "%s", dp->d_name);
        if (entry->swName_len >= sizeof(entry->swName))
            entry->swName_len = sizeof(entry->swName) - 1;
