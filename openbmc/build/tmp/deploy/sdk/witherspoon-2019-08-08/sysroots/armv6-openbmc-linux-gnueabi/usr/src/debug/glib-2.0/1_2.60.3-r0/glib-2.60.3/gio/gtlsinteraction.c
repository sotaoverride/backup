 ...
 * @ask_password_finish: complete operation to ask for a password
 * asynchronously. If the implementation returns %G_TLS_INTERACTION_HANDLED,
 * then the password argument of the async method should have been filled in by
 * using g_tls_password_set_value() or a similar function.
...
static void invoke_closure_free(gpointer data)
{
...
static InvokeClosure* invoke_closure_new(GTlsInteraction* interaction,
                                         GObject* argument,
                                         GCancellable* cancellable)
{
...
static GTlsInteractionResult
    invoke_closure_wait_and_free(InvokeClosure* closure, GError** error)
{
    GTlsInteractionResult result;
...
static GTlsInteractionResult
    invoke_closure_complete_and_free(GTlsInteraction* interaction,
                                     InvokeClosure* closure, GError** error)
{
    GTlsInteractionResult result;
    gboolean complete;
...
static void g_tls_interaction_init(GTlsInteraction* interaction)
{
...
static void g_tls_interaction_finalize(GObject* object)
{
...
static void g_tls_interaction_class_init(GTlsInteractionClass* klass)
{
...
static gboolean on_invoke_ask_password_sync(gpointer user_data)
{
    ...
    closure->result = klass->ask_password(
        closure->interaction, G_TLS_PASSWORD(closure->argument),
...
static void on_ask_password_complete(GObject* source, GAsyncResult* result,
                                     gpointer user_data)
{
    InvokeClosure* closure = user_data;
    GTlsInteractionClass* klass;
    ...
    closure->result = klass->ask_password_finish(closure->interaction, result,
                                                 &closure->error);
...
static gboolean on_invoke_ask_password_async_as_sync(gpointer user_data)
{
    InvokeClosure* closure = user_data;
    GTlsInteractionClass* klass;
    ...
    klass->ask_password_async(
        closure->interaction, G_TLS_PASSWORD(closure->argument),
        closure->cancellable, on_ask_password_complete, closure);
    ...
    GCancellable* cancellable, GError** error)
{
    ...
    g_return_val_if_fail(G_IS_TLS_INTERACTION(interaction),
                         G_TLS_INTERACTION_UNHANDLED);
    g_return_val_if_fail(G_IS_TLS_PASSWORD(password),
                         G_TLS_INTERACTION_UNHANDLED);
    ...
    {
        closure =
            invoke_closure_new(interaction, G_OBJECT(password), cancellable);
    ...
    {
        g_return_val_if_fail(klass->ask_password_finish,
                             G_TLS_INTERACTION_UNHANDLED);
                                          ...
                                          GAsyncResult* result, GError** error)
{
    ...
    {
        g_return_val_if_fail(klass->ask_password_async != NULL,
                             G_TLS_INTERACTION_UNHANDLED);
    ...
    else
    {
        g_return_val_if_fail(g_async_result_is_tagged(
                                 result, g_tls_interaction_ask_password_async),
                             G_TLS_INTERACTION_UNHANDLED);
...
static gboolean on_invoke_request_certificate_sync(gpointer user_data)
{
    InvokeClosure* closure = user_data;
    GTlsInteractionClass* klass;
    ...
    closure->result = klass->request_certificate(
        closure->interaction, G_TLS_CONNECTION(closure->argument), 0,
        closure->cancellable, &closure->error);
...
static void on_request_certificate_complete(GObject* source,
                                            GAsyncResult* result,
                                            gpointer user_data)
{
    InvokeClosure* closure = user_data;
    ...
    closure->result = klass->request_certificate_finish(
        closure->interaction, result, &closure->error);
...
static gboolean on_invoke_request_certificate_async_as_sync(gpointer user_data)
{
    InvokeClosure* closure = user_data;
    GTlsInteractionClass* klass;
    ...
    klass->request_certificate_async(
        closure->interaction, G_TLS_CONNECTION(closure->argument), 0,
        closure->cancellable, on_request_certificate_complete, closure);
...
GTlsInteractionResult g_tls_interaction_invoke_request_certificate(
    ...
    GTlsCertificateRequestFlags flags, GCancellable* cancellable,
    GError** error)
{
    GTlsInteractionResult result;
    InvokeClosure* closure;
    ...
    g_return_val_if_fail(G_IS_TLS_INTERACTION(interaction),
                         G_TLS_INTERACTION_UNHANDLED);
    g_return_val_if_fail(G_IS_TLS_CONNECTION(connection),
                         G_TLS_INTERACTION_UNHANDLED);
    g_return_val_if_fail(cancellable == NULL || G_IS_CANCELLABLE(cancellable),
                         G_TLS_INTERACTION_UNHANDLED);
    ...
    {
        closure =
            invoke_closure_new(interaction, G_OBJECT(connection), cancellable);
        g_main_context_invoke(interaction->priv->context,
    ...
    {
        g_return_val_if_fail(klass->request_certificate_finish,
                             G_TLS_INTERACTION_UNHANDLED);

        closure =
            invoke_closure_new(interaction, G_OBJECT(connection), cancellable);
        g_main_context_invoke(interaction->priv->context,
                              on_invoke_request_certificate_async_as_sync,
                              closure);
    ...
    GError** error)
{
    ...
    klass = G_TLS_INTERACTION_GET_CLASS(interaction);
    if (klass->request_certificate)
        return (klass->request_certificate)(interaction, connection, flags,
                                            cancellable, error);
    else
        return G_TLS_INTERACTION_UNHANDLED;
}
 ...
 * the g_tls_interaction_request_certificate_async() completion callback is
 * called.
 *
 * If %G_TLS_INTERACTION_HANDLED is returned, then the #GTlsConnection
    ...
    GTlsInteraction* interaction, GAsyncResult* result, GError** error)
{
    ...
    {
        g_return_val_if_fail(klass->request_certificate_async != NULL,
                             G_TLS_INTERACTION_UNHANDLED);
    ...
    else
    {
        g_return_val_if_fail(
            g_async_result_is_tagged(
                result, g_tls_interaction_request_certificate_async),
            G_TLS_INTERACTION_UNHANDLED);
