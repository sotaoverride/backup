...
static int process_parm(const char* key, const char* value,
                        struct param** params)
{
...
static int modinfo_do(struct kmod_module* mod)
{
    ...
    {
        printf("%-16s%s%c", "filename:", kmod_module_get_path(mod), separator);
    }
    ...
    while (params != NULL)
    {
        ...
        if (p->param == NULL)
            printf("%-16s%.*s:%.*s%c", "parm:", p->namelen, p->name, p->typelen,
                   p->type, separator);
        else if (p->type != NULL)
