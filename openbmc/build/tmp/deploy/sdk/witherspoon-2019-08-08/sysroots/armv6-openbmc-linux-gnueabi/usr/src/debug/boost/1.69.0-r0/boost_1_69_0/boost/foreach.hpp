...
//
// Credits:
//  Anson Tsao        - for the initial inspiration and several good
//  suggestions. Thorsten Ottosen  - for Boost.Range, and for suggesting a way
//  to detect
...
// Some compilers let us detect even const-qualified rvalues at compile-time
    ...
    (BOOST_WORKAROUND(__GNUC__, == 4) && (__GNUC_MINOR__ <= 5) &&              \
     !defined(BOOST_INTEL) && !defined(BOOST_CLANG)) ||                        \
...
// rules of the ternary conditional operator.
    ...
    (BOOST_WORKAROUND(__GNUC__, == 3) && (__GNUC_MINOR__ <= 3) &&              \
...
//   to copy. This tells BOOST_FOREACH it can avoid the rvalue/lvalue detection
//   stuff.
template <typename T>
...
struct is_noncopyable
    ...
    : boost::mpl::or_<boost::is_abstract<T>,
...
//   boost_foreach_is_lightweight_proxy at the global namespace for your type.
template <typename T>
...
inline boost::mpl::true_*
    boost_foreach_is_lightweight_proxy(std::pair<T, T>*&, boost::foreach::tag)
{
    return 0;
}
...
template <typename T>
inline boost::mpl::true_*
    boost_foreach_is_lightweight_proxy(boost::iterator_range<T>*&,
                                       boost::foreach::tag)
{
    return 0;
}
...
template <typename T>
inline boost::mpl::true_*
    boost_foreach_is_lightweight_proxy(boost::sub_range<T>*&,
                                       boost::foreach::tag)
{
    return 0;
}
...
inline boost::foreach::is_noncopyable<T>*
    boost_foreach_is_noncopyable(T*&, BOOST_FOREACH_TAG_DEFAULT)
{
    return 0;
}
...
inline BOOST_DEDUCED_TYPENAME boost::mpl::if_<C, T const, T>::type&
...
struct is_char_array
    : mpl::and_<is_array<T>, mpl::or_<is_convertible<T, char const*>,
...
struct foreach_iterator
{
    ...
    //
    // There is an ambiguity about how to iterate over arrays of char and
    // wchar_t. Should the last array element be treated as a null terminator to
    // be skipped, or is it just like any other element in the array? To fix the
    // problem, you must say which behavior you want.
    //
    ...
    // <boost/range/as_array.hpp>, as in BOOST_FOREACH( char ch,
    // boost::as_array("hello") ) ...
    ...
    typedef BOOST_DEDUCED_TYPENAME
        boost::mpl::eval_if<C, range_const_iterator<container>,
                            range_mutable_iterator<container>>::type type;
};
...
struct foreach_reverse_iterator
{
    ...
    typedef BOOST_DEDUCED_TYPENAME
        boost::mpl::eval_if<C, range_reverse_iterator<container const>,
                            range_reverse_iterator<container>>::type type;
};
...
{
    // This is a work-around for a compiler bug in Borland. If T* is a pointer
    // to array type U(*)[N], then dereferencing it results in a U* instead of
    // U(&)[N]. The cast forces the issue.
...
struct rvalue_probe
{
    ...
    typedef BOOST_DEDUCED_TYPENAME boost::mpl::if_<
        ...
        private_type_, T>::type value_type;
    operator value_type()
    {
...
// can't ever return an array or an abstract type by value
    ...
    typedef BOOST_DEDUCED_TYPENAME
        boost::mpl::if_<boost::is_array<T>, private_type_, T>::type value_type;
#else
    typedef BOOST_DEDUCED_TYPENAME boost::mpl::if_<
        boost::mpl::or_<boost::is_abstract<T>, boost::is_array<T>>,
        private_type_, T>::type value_type;
...
{
    simple_variant(T const* t) : is_rvalue(false)
    {
        *static_cast<T const**>(this->data.address()) = t;
    }

    simple_variant(T const& t) : is_rvalue(true)
    {
...
inline BOOST_DEDUCED_TYPENAME
    boost::enable_if<boost::mpl::or_<LValue, IsProxy>, IsProxy>::type*
...
inline bool* should_copy_impl(boost::mpl::false_*, boost::mpl::false_*,
                              bool* is_rvalue)
{
    return is_rvalue;
}
...
{
    return auto_any<simple_variant<T>>(*rvalue ? simple_variant<T>(t)
                                               : simple_variant<T>(&t));
}
#endif
    ...
    begin(auto_any_t col, type2type<T, C>*, boost::mpl::false_*) // lvalue
{
    ...
    return auto_any<BOOST_DEDUCED_TYPENAME foreach_iterator<T, C>::type>(
        iterator(boost::begin(BOOST_FOREACH_DEREFOF(
            (auto_any_cast<type*, boost::mpl::false_>(col))))));
}
    ...
    begin(auto_any_t col, type2type<T, const_>*, bool*)
{
    return auto_any<BOOST_DEDUCED_TYPENAME foreach_iterator<T, const_>::type>(
        boost::begin(
            *auto_any_cast<simple_variant<T>, boost::mpl::false_>(col).get()));
}
#endif

#ifndef BOOST_NO_FUNCTION_TEMPLATE_ORDERING
template <typename T, typename C>
inline auto_any<T*> begin(auto_any_t col, type2type<T*, C>*,
                          boost::mpl::true_*) // null-terminated C-style strings
{
    ...
    end(auto_any_t col, type2type<T, C>*, boost::mpl::false_*) // lvalue
{
    ...
    typedef BOOST_DEDUCED_TYPENAME foreach_iterator<T, C>::type iterator;
    return auto_any<BOOST_DEDUCED_TYPENAME foreach_iterator<T, C>::type>(
        iterator(boost::end(BOOST_FOREACH_DEREFOF(
            (auto_any_cast<type*, boost::mpl::false_>(col))))));
}
...
#ifndef BOOST_NO_FUNCTION_TEMPLATE_ORDERING
template <typename T, typename C>
inline auto_any<int> end(auto_any_t, type2type<T*, C>*,
                         boost::mpl::true_*) // null-terminated C-style strings
{
...
inline bool done(auto_any_t cur, auto_any_t end, type2type<T, C>*)
{
    ...
    return auto_any_cast<iter_t, boost::mpl::false_>(cur) ==
           auto_any_cast<iter_t, boost::mpl::false_>(end);
}

#ifndef BOOST_NO_FUNCTION_TEMPLATE_ORDERING
template <typename T, typename C>
inline bool done(auto_any_t cur, auto_any_t,
                 type2type<T*, C>*) // null-terminated C-style strings
{
...
{
    return auto_any<
        BOOST_DEDUCED_TYPENAME foreach_reverse_iterator<T, C>::type>(
...
{
    typedef BOOST_DEDUCED_TYPENAME type2type<T, C>::type type;
    typedef BOOST_DEDUCED_TYPENAME foreach_reverse_iterator<T, C>::type
        iterator;
    return auto_any<
        BOOST_DEDUCED_TYPENAME foreach_reverse_iterator<T, C>::type>(
        iterator(boost::rbegin(BOOST_FOREACH_DEREFOF(
            (auto_any_cast<type*, boost::mpl::false_>(col))))));
}
...
template <typename T>
inline auto_any<
    BOOST_DEDUCED_TYPENAME foreach_reverse_iterator<T, const_>::type>
...
inline auto_any<reverse_iterator<T*>>
    rbegin(auto_any_t col, type2type<T*, C>*,
           boost::mpl::true_*) // null-terminated C-style strings
{
    ...
    rend(auto_any_t col, type2type<T, C>*, boost::mpl::false_*) // lvalue
{
    ...
    return auto_any<
        BOOST_DEDUCED_TYPENAME foreach_reverse_iterator<T, C>::type>(
        iterator(boost::rend(BOOST_FOREACH_DEREFOF(
            (auto_any_cast<type*, boost::mpl::false_>(col))))));
}
...
template <typename T, typename C>
inline auto_any<reverse_iterator<T*>>
    rend(auto_any_t col, type2type<T*, C>*,
         boost::mpl::true_*) // null-terminated C-style strings
{
...
// A sneaky way to get the type of the collection without evaluating the
// expression
#define BOOST_FOREACH_TYPEOF(COL)                                              \
    (true ? 0                                                                  \
          : boost::foreach_detail_::encode_type(                               \
                COL, boost::foreach_detail_::is_const_(COL)))
...
// Evaluate the collection expression, and detect if it is an lvalue or and
// rvalue
#define BOOST_FOREACH_EVALUATE(COL)                                            \
    (true ? boost::foreach_detail_::make_probe(                                \
                (COL), BOOST_FOREACH_ID(_foreach_is_rvalue))                   \
          : (COL))

// The rvalue/lvalue-ness of the collection expression is determined
// dynamically, unless the type is an array or is noncopyable or is non-const,
// in which case we know it's an lvalue. If the type happens to be a lightweight
// proxy, always make a copy.
#define BOOST_FOREACH_SHOULD_COPY(COL)                                         \
...
#define BOOST_FOREACH_CONTAIN(COL)                                             \
    boost::foreach_detail_::contain(BOOST_FOREACH_EVALUATE(COL),               \
...
#define BOOST_FOREACH_DONE(COL)                                                \
    boost::foreach_detail_::done(BOOST_FOREACH_ID(_foreach_cur),               \
                                 BOOST_FOREACH_ID(_foreach_end),               \
                                 BOOST_FOREACH_TYPEOF(COL))

#define BOOST_FOREACH_NEXT(COL)                                                \
    boost::foreach_detail_::next(BOOST_FOREACH_ID(_foreach_cur),               \
                                 BOOST_FOREACH_TYPEOF(COL))

#define BOOST_FOREACH_DEREF(COL)                                               \
    boost::foreach_detail_::deref(BOOST_FOREACH_ID(_foreach_cur),              \
                                  BOOST_FOREACH_TYPEOF(COL))
...
#define BOOST_FOREACH_RDONE(COL)                                               \
    boost::foreach_detail_::rdone(BOOST_FOREACH_ID(_foreach_cur),              \
                                  BOOST_FOREACH_ID(_foreach_end),              \
                                  BOOST_FOREACH_TYPEOF(COL))

#define BOOST_FOREACH_RNEXT(COL)                                               \
    boost::foreach_detail_::rnext(BOOST_FOREACH_ID(_foreach_cur),              \
                                  BOOST_FOREACH_TYPEOF(COL))

#define BOOST_FOREACH_RDEREF(COL)                                              \
    boost::foreach_detail_::rderef(BOOST_FOREACH_ID(_foreach_cur),             \
                                   BOOST_FOREACH_TYPEOF(COL))
...
#define BOOST_FOREACH(VAR, COL)                                                \
    ...
    else if (boost::foreach_detail_::auto_any_t BOOST_FOREACH_ID(              \
                 _foreach_cur) = BOOST_FOREACH_BEGIN(COL))                     \
    {                                                                          \
    }                                                                          \
    else if (boost::foreach_detail_::auto_any_t BOOST_FOREACH_ID(              \
                 _foreach_end) = BOOST_FOREACH_END(COL))                       \
    {                                                                          \
    }                                                                          \
    else                                                                       \
        for (bool BOOST_FOREACH_ID(_foreach_continue) = true;                  \
             ...
             BOOST_FOREACH_ID(_foreach_continue) ? BOOST_FOREACH_NEXT(COL)     \
                                                 : (void)0)                    \
            if (boost::foreach_detail_::set_false(                             \
...
#define BOOST_REVERSE_FOREACH(VAR, COL)                                        \
    ...
    {                                                                          \
                 ...
                 _foreach_cur) = BOOST_FOREACH_RBEGIN(COL))                    \
    {                                                                          \
    }                                                                          \
    else if (boost::foreach_detail_::auto_any_t BOOST_FOREACH_ID(              \
                 _foreach_end) = BOOST_FOREACH_REND(COL))                      \
    {                                                                          \
    }                                                                          \
    else                                                                       \
        for (bool BOOST_FOREACH_ID(_foreach_continue) = true;                  \
             BOOST_FOREACH_ID(_foreach_continue) && !BOOST_FOREACH_RDONE(COL); \
             BOOST_FOREACH_ID(_foreach_continue) ? BOOST_FOREACH_RNEXT(COL)    \
                                                 : (void)0)                    \
            if (boost::foreach_detail_::set_false(                             \
                    BOOST_FOREACH_ID(_foreach_continue)))                      \
            {                                                                  \
