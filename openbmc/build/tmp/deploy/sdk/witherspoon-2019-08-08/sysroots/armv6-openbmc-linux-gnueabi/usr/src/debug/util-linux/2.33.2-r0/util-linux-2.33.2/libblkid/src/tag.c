...
void blkid_free_tag(blkid_tag tag)
{
    ...
    DBG(TAG,
        ul_debugobj(tag, "freeing tag %s (%s)", tag->bit_name, tag->bit_val));
 ...
 */
int blkid_set_tag(blkid_dev dev, const char* name, const char* value,
                  const int vlength)
{
    ...
    else if (t)
    {
        ...
        DBG(TAG, ul_debugobj(t, "update (%s) '%s' -> '%s'", t->bit_name,
                             t->bit_val, val));
        free(t->bit_val);
        t->bit_val = val;
    }
    else
    {
        ...
        if (dev->bid_cache)
        {
            ...
            if (!head)
            {
                ...
                DBG(TAG,
                    ul_debugobj(head, "creating new cache tag head %s", name));
                head->bit_name = strdup(name);
                if (!head->bit_name)
                    goto errout;
                list_add_tail(&head->bit_tags, &dev->bid_cache->bic_tags);
            }
 ...
 */
int blkid_tag_next(blkid_tag_iterate iter, const char** type,
                   const char** value)
{
    blkid_tag tag;

    if (!type || !value || !iter || iter->magic != TAG_ITERATE_MAGIC ||
 ...
 */
blkid_dev blkid_find_dev_with_tag(blkid_cache cache, const char* type,
                                  const char* value)
{
    blkid_tag head;
    blkid_dev dev;
...
try_again:
    ...
    if (head)
    {
        ...
        {
            blkid_tag tmp = list_entry(p, struct blkid_struct_tag, bit_names);

            if (!strcmp(tmp->bit_val, value) && (tmp->bit_dev->bid_pri > pri) &&
...
int main(int argc, char** argv)
{
    ...
    if (search_type)
    {
        ...
        printf("Device %s: (%s, %s) %s\n", blkid_dev_devname(dev), search_type,
