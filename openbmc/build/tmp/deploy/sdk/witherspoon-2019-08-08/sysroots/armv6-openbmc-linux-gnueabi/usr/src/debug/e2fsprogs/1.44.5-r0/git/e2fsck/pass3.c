...
void e2fsck_pass3(e2fsck_t ctx)
{
    ...
    pctx.errcode = e2fsck_allocate_inode_bitmap(
        fs, _("inode done bitmap"), EXT2FS_BMAP64_AUTODIR, "inode_done_map",
        &inode_done_map);
    if (pctx.errcode)
    {
        pctx.num = 2;
...
static void check_root(e2fsck_t ctx)
{
    ...
    if (ext2fs_test_inode_bitmap2(ctx->inode_used_map, EXT2_ROOT_INO))
    {
         ...
         * offered to clear it.
         */
        if (!(ext2fs_test_inode_bitmap2(ctx->inode_dir_map, EXT2_ROOT_INO)))
        {
...
skip_new_block:
     ...
     */
    pctx.errcode =
        ext2fs_new_dir_block(fs, EXT2_ROOT_INO, EXT2_ROOT_INO, &block);
    if (pctx.errcode)
    {
    ...
    pctx.errcode = ext2fs_write_dir_block4(fs, blk, block, 0, EXT2_ROOT_INO);
                           ...
                           struct problem_context* pctx)
{
    ...
    while (1)
    {
        ...
        else if (parent_count++ > 2048)
        {
            ...
            {
                pctx->errcode = e2fsck_allocate_inode_bitmap(
                    fs, _("inode loop detection bitmap"), EXT2FS_BMAP64_AUTODIR,
...
ext2_ino_t e2fsck_get_lost_and_found(e2fsck_t ctx, int fix)
{
    ...
    retval = ext2fs_lookup(fs, EXT2_ROOT_INO, name, sizeof(name) - 1, 0, &ino);
    if (will_rehash)
    ...
    if (!retval)
    {
        ...
        retval =
            ext2fs_read_inode_full(fs, ino, EXT2_INODE(&inode), sizeof(inode));
        if (fix && retval)
            return 0;
...
skip_new_block:
     ...
     */
    retval =
        ext2fs_new_inode(fs, EXT2_ROOT_INO, 040700, ctx->inode_used_map, &ino);
...
static void fix_dotdot(e2fsck_t ctx, ext2_ino_t ino, ext2_ino_t parent)
{
    ...
    retval = ext2fs_dir_iterate(fs, ino, DIRENT_FLAG_INCLUDE_EMPTY, 0,
                                fix_dotdot_proc, &fp);
    if (will_rehash)
        ctx->fs->flags = (flags & EXT2_FLAG_IGNORE_CSUM_ERRORS) |
                         (ctx->fs->flags & ~EXT2_FLAG_IGNORE_CSUM_ERRORS);
    ...
    {
        pctx.errcode = retval;
        fix_problem(ctx, retval ? PR_3_FIX_PARENT_ERR : PR_3_FIX_PARENT_NOFIND,
                    &pctx);
        ext2fs_unmark_valid(fs);
    }
...
static int expand_dir_proc(ext2_filsys fs, blk64_t* blocknr,
                           ...
                           void* priv_data)
{
    ...
    if (blockcnt && (EXT2FS_B2C(fs, last_blk) == EXT2FS_B2C(fs, last_blk + 1)))
        new_blk = last_blk + 1;
    else
    {
        last_blk &= ~EXT2FS_CLUSTER_MASK(fs);
        retval =
            ext2fs_new_block2(fs, last_blk, ctx->block_found_map, &new_blk);
        if (retval)
        {
            es->err = retval;
            return BLOCK_ABORT;
    ...
    if (blockcnt > 0)
    {
        ...
        es->num--;
        retval = ext2fs_write_dir_block4(fs, new_blk, block, 0, es->dir);
        ext2fs_free_mem(&block);
    }
    else
...
errcode_t e2fsck_expand_directory(e2fsck_t ctx, ext2_ino_t dir, int num,
                                  int guaranteed_size)
{
    ext2_filsys fs = ctx->fs;
    errcode_t retval;
    ...
    retval = ext2fs_block_iterate3(fs, dir, BLOCK_FLAG_APPEND, 0,
                                   expand_dir_proc, &es);
     ...
     */
    retval = ext2fs_read_inode_full(fs, dir, EXT2_INODE(&inode), sizeof(inode));
    if (retval)
        return retval;
    ...
    e2fsck_write_inode_full(ctx, dir, EXT2_INODE(&inode), sizeof(inode),
                            "expand_directory");
