...
int sd_netlink_open_fd(sd_netlink** ret, int fd)
{
    ...
    if (r < 0)
    {
        rtnl->fd = -1; /* on failure, the caller remains owner of the fd, hence
                          don't close it here */
        rtnl->protocol = -1;
        return r;
    }
...
static int dispatch_rqueue(sd_netlink* rtnl, sd_netlink_message** message)
{
    ...
    *message = rtnl->rqueue[0];
    rtnl->rqueue_size--;
    memmove(rtnl->rqueue, rtnl->rqueue + 1,
            sizeof(sd_netlink_message*) * rtnl->rqueue_size);
    ...
    LIST_FOREACH(match_callbacks, c, rtnl->match_callbacks)
    {
        if (type == c->type)
        {
            ...
            if (r != 0)
            {
                if (r < 0)
                    log_debug_errno(
                        r, "sd-netlink: match callback %s%s%sfailed: %m",
                        slot->description ? "'" : "",
                        strempty(slot->description),
                        slot->description ? "' " : "");
                          ...
                          uint64_t usec, const char* description)
{
    ...
    r = netlink_slot_allocate(nl, !ret_slot, NETLINK_REPLY_CALLBACK,
    ...
    r = hashmap_put(nl->reply_callbacks, &slot->reply_callback.serial,
                    &slot->reply_callback);
    if (r < 0)
        return r;
    ...
    {
        r = prioq_put(nl->reply_callbacks_prioq, &slot->reply_callback,
                      &slot->reply_callback.prioq_idx);
        if (r < 0)
        {
            (void)hashmap_remove(nl->reply_callbacks,
                                 &slot->reply_callback.serial);
            return r;
        }
    }

    /* Set this at last. Otherwise, some failures in above call the destroy
     * callback but some do not. */
...
int sd_netlink_call(sd_netlink* rtnl, sd_netlink_message* message,
                    uint64_t usec, sd_netlink_message** ret)
{
    usec_t timeout;
    uint32_t serial;
    int r;
    ...
    for (;;)
    {
        ...
        for (i = 0; i < rtnl->rqueue_size; i++)
        {
            ...
            {
                _cleanup_(sd_netlink_message_unrefp)
                    sd_netlink_message* incoming = NULL;
                uint16_t type;
...
int sd_netlink_attach_event(sd_netlink* rtnl, sd_event* event, int64_t priority)
{
    ...
    r = sd_event_add_io(rtnl->event, &rtnl->io_event_source, rtnl->fd, 0,
                        io_callback, rtnl);
    if (r < 0)
        goto fail;
    ...
    r = sd_event_source_set_description(rtnl->io_event_source,
    ...
    r = sd_event_add_time(rtnl->event, &rtnl->time_event_source,
                          CLOCK_MONOTONIC, 0, 0, time_callback, rtnl);
    if (r < 0)
        goto fail;
...
int sd_netlink_add_match(sd_netlink* rtnl, sd_netlink_slot** ret_slot,
                         uint16_t type, sd_netlink_message_handler_t callback,
                         sd_netlink_destroy_t destroy_callback, void* userdata,
                         const char* description)
{
    ...
    r = netlink_slot_allocate(rtnl, !ret_slot, NETLINK_MATCH_CALLBACK,
