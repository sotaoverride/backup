...
int cgroup_bpf_whitelist_device(BPFProgram* prog, int type, int major,
                                int minor, const char* acc)
{
...
int cgroup_bpf_whitelist_major(BPFProgram* prog, int type, int major,
                               const char* acc)
{
    struct bpf_insn insn[] = {
...
int cgroup_init_device_bpf(BPFProgram** ret, CGroupDevicePolicy policy,
                           bool whitelist)
{
    ...
    if (policy == CGROUP_CLOSED || whitelist)
    {
        ...
        if (r < 0)
            return log_error_errno(
                r, "Extending device control BPF program failed: %m");
    }
...
int cgroup_apply_device_bpf(Unit* u, BPFProgram* prog,
                            CGroupDevicePolicy policy, bool whitelist)
{
    ...
    if (policy != CGROUP_STRICT || whitelist)
    {
        ...
        r = bpf_program_add_instructions(prog, post_insn,
    ...
    r = bpf_program_cgroup_attach(prog, BPF_CGROUP_DEVICE, path,
                                  BPF_F_ALLOW_MULTI);
    if (r < 0)
        return log_error_errno(
            r, "Attaching device control BPF program to cgroup %s failed: %m",
            path);
...
int bpf_devices_supported(void)
{
    ...
    /* Checks whether BPF device controller is supported. For this, we check
     * five things:
     *
     * a) whether we are privileged
     * b) whether the unified hierarchy is being used
     * c) the BPF implementation in the kernel supports
     * BPF_PROG_TYPE_CGROUP_DEVICE programs, which we require
     */
    ...
    if (r == 0)
    {
        log_debug("Not running with unified cgroups, BPF device control is not "
                  "supported.");
        return supported = 0;
    }
    ...
    if (r < 0)
    {
        log_debug_errno(r, "Can't allocate CGROUP DEVICE BPF program, BPF "
                           "device control is not supported: %m");
        return supported = 0;
    }
    ...
    {
        log_debug_errno(r,
                        "Can't add trivial instructions to CGROUP DEVICE BPF "
                        "program, BPF device control is not supported: %m");
        return supported = 0;
    }
    ...
    if (r < 0)
    {
        log_debug_errno(r, "Can't load kernel CGROUP DEVICE BPF program, BPF "
                           "device control is not supported: %m");
        return supported = 0;
    }
