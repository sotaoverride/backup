...
/** \file avahi-core/lookup.h Functions for browsing/resolving services and
 * other RRs */
...
/** A local domain browsing object. May be used to enumerate domains used on the
 * local LAN */
typedef struct AvahiSDomainBrowser AvahiSDomainBrowser;

/** A DNS-SD service type browsing object. May be used to enumerate the service
 * types of all available services on the local LAN */
typedef struct AvahiSServiceTypeBrowser AvahiSServiceTypeBrowser;

/** A DNS-SD service browser. Use this to enumerate available services of a
 * certain kind on the local LAN. Use AvahiSServiceResolver to get specific
 * service data like address and port for a service. */
typedef struct AvahiSServiceBrowser AvahiSServiceBrowser;

/** A DNS-SD service resolver.  Use this to retrieve addres, port and TXT data
 * for a DNS-SD service */
...
typedef void (*AvahiSHostNameResolverCallback)(
    AvahiSHostNameResolver* r, AvahiIfIndex interface, AvahiProtocol protocol,
    AvahiResolverEvent event, /**< Resolving event */
    const char* host_name, /**< Host name which should be resolved. May differ
                              in case from the query */
    const AvahiAddress*
        a, /**< The address, or NULL if the host name couldn't be resolved. */
    AvahiLookupResultFlags flags, /**< Lookup flags */
    void* userdata);

/** Create an AvahiSHostNameResolver object for resolving a host name to an
 * adddress. See AvahiSRecordBrowser for more info on the paramters. */
AvahiSHostNameResolver* avahi_s_host_name_resolver_new(
    ...
    AvahiProtocol aprotocol, /**< The address family of the desired address or
                                AVAHI_PROTO_UNSPEC if doesn't matter. */
...
typedef void (*AvahiSAddressResolverCallback)(
    AvahiSAddressResolver* r, AvahiIfIndex interface, AvahiProtocol protocol,
    AvahiResolverEvent event, const AvahiAddress* a,
    const char* host_name, /**< A host name for the specified address, if one
                              was found, i.e. event == AVAHI_RESOLVER_FOUND */
    AvahiLookupResultFlags flags, /**< Lookup flags */
    void* userdata);

/** Create an AvahiSAddressResolver object. See AvahiSRecordBrowser for more
...
typedef void (*AvahiSDomainBrowserCallback)(
    AvahiSDomainBrowser* b, AvahiIfIndex interface, AvahiProtocol protocol,
...
typedef void (*AvahiSServiceTypeBrowserCallback)(
    AvahiSServiceTypeBrowser* b, AvahiIfIndex interface, AvahiProtocol protocol,
...
typedef void (*AvahiSServiceBrowserCallback)(
    AvahiSServiceBrowser* b, AvahiIfIndex interface, AvahiProtocol protocol,
...
typedef void (*AvahiSServiceResolverCallback)(
    ...
    AvahiResolverEvent event, /**< Is AVAHI_RESOLVER_FOUND when the service was
                                 resolved successfully, and everytime it
                                 changes. Is AVAHI_RESOLVER_TIMOUT when the
                                 service failed to resolve or disappeared. */
...
/** Create a new AvahiSServiceResolver object. The specified callback function
 * will be called with the resolved service data. */
AvahiSServiceResolver* avahi_s_service_resolver_new(
    AvahiServer* server, AvahiIfIndex interface, AvahiProtocol protocol,
    const char* name, const char* type, const char* domain,
    AvahiProtocol aprotocol, /**< Address family of the desired service address.
                                Use AVAHI_PROTO_UNSPEC if you don't care */
    AvahiLookupFlags flags,  /**< Lookup flags. */
