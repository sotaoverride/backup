...
int do_mknod(const char* pathname, mode_t mode, dev_t dev)
{
    ...
    {
        int fd =
            open(pathname, O_WRONLY | O_CREAT | O_TRUNC, S_IWUSR | S_IRUSR);
        if (fd < 0 || close(fd) < 0)
            return -1;
        return 0;
    }
...
int do_mkstemp(char* template, mode_t perms)
{
    ...
    (!defined HAVE_OPEN64 || defined HAVE_MKSTEMP64)
    {
 ...
 * and will be changed to be at pos + len. */
int do_punch_hole(int fd, UNUSED(OFF_T pos), int len)
{
#ifdef HAVE_FALLOCATE
#ifdef HAVE_FALLOC_FL_PUNCH_HOLE
    if (fallocate(fd, FALLOC_FL_PUNCH_HOLE | FALLOC_FL_KEEP_SIZE, pos, len) ==
        0)
    {
