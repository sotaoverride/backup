...
static const initgr_response_header notfound = {
    .version = NSCD_VERSION, .found = 0, .ngrps = 0};

#include "../grp/compat-initgroups.c"

static time_t addinitgroupsX(struct database_dyn* db, int fd,
                             ...
                             struct hashentry* const he, struct datahead* dh)
{
    ...
    while (!no_more)
    {
        ...
        if (fct == NULL)
        {
            status = compat_call(nip, key, -1, &start, &size, &groups, limit,
                                 &errno);
            ...
            status = DL_CALL_FCT(
                fct, (key, -1, &start, &size, &groups, limit, &errno));
...
void addinitgroups(struct database_dyn* db, int fd, request_header* req,
                   void* key, uid_t uid)
{
...
time_t readdinitgroups(struct database_dyn* db, struct hashentry* he,
                       struct datahead* dh)
{
    request_header req = {.type = INITGROUPS, .key_len = he->len};
