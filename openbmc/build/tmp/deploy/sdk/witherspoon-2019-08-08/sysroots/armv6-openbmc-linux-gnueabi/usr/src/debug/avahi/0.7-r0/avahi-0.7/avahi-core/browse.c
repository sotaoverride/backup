...
static void lookup_handle_cname(AvahiSRBLookup* l, AvahiIfIndex interface,
                                AvahiProtocol protocol, AvahiLookupFlags flags,
                                AvahiRecord* r);
static void lookup_drop_cname(AvahiSRBLookup* l, AvahiIfIndex interface,
                              AvahiProtocol protocol, AvahiLookupFlags flags,
                              AvahiRecord* r);

static void transport_flags_from_domain(AvahiServer* s, AvahiLookupFlags* flags,
                                        const char* domain)
{
    assert(flags);
    assert(domain);

    assert(!((*flags & AVAHI_LOOKUP_USE_MULTICAST) &&
...
static AvahiSRBLookup* lookup_new(AvahiSRecordBrowser* b,
                                  AvahiIfIndex interface,
                                  AvahiProtocol protocol,
                                  AvahiLookupFlags flags, AvahiKey* key)
{
...
static void lookup_unref(AvahiSRBLookup* l)
{
    ...
    while (l->cname_lookups)
    {
        ...
        l->cname_lookups =
            avahi_rlist_remove_by_link(l->cname_lookups, l->cname_lookups);
    }
...
static AvahiSRBLookup* lookup_find(AvahiSRecordBrowser* b,
                                   AvahiIfIndex interface,
                                   AvahiProtocol protocol,
                                   AvahiLookupFlags flags, AvahiKey* key)
{
    ...
    for (l = b->lookups; l; l = l->lookups_next)
    {
        ...
        if ((l->interface == AVAHI_IF_UNSPEC || l->interface == interface) &&
            ...
            l->flags == flags && avahi_key_equal(l->key, key))
...
static void lookup_wide_area_callback(AvahiWideAreaLookupEngine* e,
    ...
    switch (event)
    {
        case AVAHI_BROWSER_NEW:
            ...
            if (r->key->clazz == AVAHI_DNS_CLASS_IN &&
                ...
                /* It's a CNAME record, so let's follow it. We only follow it on
                 * wide area DNS! */
                lookup_handle_cname(l, AVAHI_IF_UNSPEC, AVAHI_PROTO_UNSPEC,
                                    AVAHI_LOOKUP_USE_WIDE_AREA, r);
            else
            {
                /* It's a normal record, so let's call the user callback */
                assert(avahi_key_equal(r->key, l->key));

                b->callback(b, AVAHI_IF_UNSPEC, AVAHI_PROTO_UNSPEC, event, r,
                            flags, b->userdata);
            }
            break;
        ...
        case AVAHI_BROWSER_FAILURE:
            ...
            b->callback(b, AVAHI_IF_UNSPEC, AVAHI_PROTO_UNSPEC, event, NULL,
                        flags, b->userdata);
            break;
    }
...
static void lookup_multicast_callback(AvahiMulticastLookupEngine* e,
                                      AvahiIfIndex interface,
                                      AvahiProtocol protocol,
                                      AvahiBrowserEvent event,
                                      AvahiLookupResultFlags flags,
                                      AvahiRecord* r, void* userdata)
{
    ...
    switch (event)
    {
        case AVAHI_BROWSER_NEW:
            ...
            if (r->key->clazz == AVAHI_DNS_CLASS_IN &&
                r->key->type == AVAHI_DNS_TYPE_CNAME)
                /* It's a CNAME record, so let's follow it. We allow browsing on
                 * both multicast and wide area. */
                lookup_handle_cname(l, interface, protocol, b->flags, r);
            else
            {
                /* It's a normal record, so let's call the user callback */

                if (avahi_server_is_record_local(b->server, interface, protocol,
                                                 r))
...
static int lookup_start(AvahiSRBLookup* l)
{
    ...
    assert(!(l->flags & AVAHI_LOOKUP_USE_WIDE_AREA) !=
    ...
    if (l->flags & AVAHI_LOOKUP_USE_WIDE_AREA)
    {
        ...
        if (!(l->wide_area = avahi_wide_area_lookup_new(
                  l->record_browser->server->wide_area_lookup_engine, l->key,
                  lookup_wide_area_callback, l)))
            return -1;
    }
    else
    {
        ...
        if (!(l->multicast = avahi_multicast_lookup_new(
                  l->record_browser->server->multicast_lookup_engine,
                  l->interface, l->protocol, l->key, lookup_multicast_callback,
                  l)))
            return -1;
    }
...
static int lookup_scan_cache(AvahiSRBLookup* l)
{
    ...
    if (l->flags & AVAHI_LOOKUP_USE_WIDE_AREA)
    {
        n = (int)avahi_wide_area_scan_cache(
            l->record_browser->server->wide_area_lookup_engine, l->key,
            lookup_wide_area_callback, l);
    }
    else
    {
        assert(l->flags & AVAHI_LOOKUP_USE_MULTICAST);
        n = (int)avahi_multicast_lookup_engine_scan_cache(
            l->record_browser->server->multicast_lookup_engine, l->interface,
            l->protocol, l->key, lookup_multicast_callback, l);
    }

    return n;
}

static AvahiSRBLookup* lookup_add(AvahiSRecordBrowser* b,
                                  AvahiIfIndex interface,
                                  AvahiProtocol protocol,
                                  AvahiLookupFlags flags, AvahiKey* key)
{
...
static int lookup_go(AvahiSRBLookup* l)
{
    ...
    if (!l->record_browser->dead && l->ref > 1)
    {
        ...
        if ((l->flags & AVAHI_LOOKUP_USE_MULTICAST) || n == 0)
            /* We do no start a query if the cache contained entries and we're
             * on wide area */
                                ...
                                AvahiRecord* r)
{
    ...
    k = avahi_key_new(r->data.ptr.name, l->record_browser->key->clazz,
                      l->record_browser->key->type);
                              ...
                              AvahiRecord* r)
{
    ...
    for (rl = l->cname_lookups; rl; rl = rl->rlist_next)
    {
        ...
        if ((n->interface == AVAHI_IF_UNSPEC || n->interface == interface) &&
            ...
            n->flags == flags && avahi_key_equal(n->key, k))
            break;
    }
...
static void defer_callback(AVAHI_GCC_UNUSED AvahiTimeEvent* e, void* userdata)
{
    ...
    if (n < 0)
    {
        ...
        b->callback(b, b->interface, b->protocol, AVAHI_BROWSER_FAILURE, NULL,
                    b->flags & AVAHI_LOOKUP_USE_WIDE_AREA
    ...
    if (!b->dead && b->root_lookup &&
        b->root_lookup->flags & AVAHI_LOOKUP_USE_WIDE_AREA && n > 0)
    {
        ...
        b->callback(b, b->interface, b->protocol, AVAHI_BROWSER_ALL_FOR_NOW,
                    NULL, AVAHI_LOOKUP_RESULT_WIDE_AREA, b->userdata);
    }
}
...
AvahiSRecordBrowser* avahi_s_record_browser_new(
    ...
    AvahiKey* key, AvahiLookupFlags flags, AvahiSRecordBrowserCallback callback,
    void* userdata)
{
    ...
    AVAHI_CHECK_VALIDITY_RETURN_NULL(server, AVAHI_IF_VALID(interface),
    ...
    AVAHI_CHECK_VALIDITY_RETURN_NULL(server, avahi_key_is_valid(key),
                                     AVAHI_ERR_INVALID_KEY);
    AVAHI_CHECK_VALIDITY_RETURN_NULL(
        server,
        AVAHI_FLAGS_VALID(flags, AVAHI_LOOKUP_USE_WIDE_AREA |
    ...
    AVAHI_LLIST_PREPEND(AvahiSRecordBrowser, browser, server->record_browsers,
                        b);

    /* The currently cached entries are scanned a bit later, and than we will
     * start querying, too */
    b->defer_time_event =
        avahi_time_event_new(server->time_event_queue, NULL, defer_callback, b);
    assert(b->defer_time_event);
...
void avahi_s_record_browser_destroy(AvahiSRecordBrowser* b)
{
    ...
    AVAHI_LLIST_REMOVE(AvahiSRecordBrowser, browser, b->server->record_browsers,
                       b);
