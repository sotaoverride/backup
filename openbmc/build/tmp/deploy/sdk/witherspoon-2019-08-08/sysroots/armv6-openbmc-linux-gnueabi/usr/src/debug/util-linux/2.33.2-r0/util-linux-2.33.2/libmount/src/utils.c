...
{
    if (tagname && *tagname &&
        (strcmp("UUID", tagname) == 0 || strcmp("LABEL", tagname) == 0 ||
         strcmp("PARTUUID", tagname) == 0 || strcmp("PARTLABEL", tagname) == 0))
        return 1;
 ...
 *
 * Returns: 1 if the @tag is parsable and tag NAME= is supported by libmount, or
 * 0.
 */
 ...
 * @filename returns an allocated buffer with the last path component, for
 * example:
 *
...
int mnt_chdir_to_parent(const char* target, char** filename)
{
    ...
    {
        DBG(UTILS,
            ul_debug("unexpected chdir (expected=%s, cwd=%s)", parent, cwd));
        goto err;
    }

    DBG(CXT, ul_debug("current directory moved to %s [last_component='%s']",
                      parent, last));
...
/*
 * Check if @path is on a read-only filesystem independently of file
 * permissions.
 */
int mnt_is_readonly(const char* path)
{
     ...
     *
     * - for a read-write filesystem with a read-only VFS node (aka -o
     * remount,ro,bind)
     */
    {
...
int mnt_fstype_is_pseudofs(const char* type)
{
    ...
    static const char* pseudofs[] = {
        ...
        "fuse.archivemount", /* Not a true pseudofs (has source), but source is
                                not reported. */
        "fuse.dumpfs", /* In fact, it is a netfs, but source is not reported. */
        "fuse.encfs",  /* Not a true pseudofs (has source), but source is not
                          reported. */
    ...
    return !(bsearch(&type, pseudofs, ARRAY_SIZE(pseudofs), sizeof(char*),
                     fstype_cmp) == NULL);
}
...
{
    if (strcmp(type, "cifs") == 0 || strcmp(type, "smbfs") == 0 ||
        strncmp(type, "nfs", 3) == 0 || strcmp(type, "afs") == 0 ||
        strcmp(type, "ncpfs") == 0 || strcmp(type, "fuse.curlftpfs") == 0 ||
        strcmp(type, "fuse.sshfs") == 0 || strncmp(type, "9p", 2) == 0)
        return 1;
    return 0;
}
...
const char* mnt_statfs_get_fstype(struct statfs* vfs)
{
    ...
    switch (vfs->f_type)
    {
        case STATFS_ADFS_MAGIC:
            return "adfs";
        case STATFS_AFFS_MAGIC:
            return "affs";
        case STATFS_AFS_MAGIC:
            return "afs";
        case STATFS_AUTOFS_MAGIC:
            return "autofs";
        case STATFS_BDEVFS_MAGIC:
            return "bdev";
        case STATFS_BEFS_MAGIC:
            return "befs";
        case STATFS_BFS_MAGIC:
            return "befs";
        case STATFS_BINFMTFS_MAGIC:
            return "binfmt_misc";
        case STATFS_BTRFS_MAGIC:
            return "btrfs";
        case STATFS_CEPH_MAGIC:
            return "ceph";
        case STATFS_CGROUP_MAGIC:
            return "cgroup";
        case STATFS_CIFS_MAGIC:
            return "cifs";
        case STATFS_CODA_MAGIC:
            return "coda";
        case STATFS_CONFIGFS_MAGIC:
            return "configfs";
        case STATFS_CRAMFS_MAGIC:
            return "cramfs";
        case STATFS_DEBUGFS_MAGIC:
            return "debugfs";
        case STATFS_DEVPTS_MAGIC:
            return "devpts";
        case STATFS_ECRYPTFS_MAGIC:
            return "ecryptfs";
        case STATFS_EFIVARFS_MAGIC:
            return "efivarfs";
        case STATFS_EFS_MAGIC:
            return "efs";
        case STATFS_EXOFS_MAGIC:
            return "exofs";
        case STATFS_EXT4_MAGIC:
            return "ext4"; /* all extN use the same magic */
        case STATFS_F2FS_MAGIC:
            return "f2fs";
        case STATFS_FUSE_MAGIC:
            return "fuse";
        case STATFS_FUTEXFS_MAGIC:
            return "futexfs";
        case STATFS_GFS2_MAGIC:
            return "gfs2";
        case STATFS_HFSPLUS_MAGIC:
            return "hfsplus";
        case STATFS_HOSTFS_MAGIC:
            return "hostfs";
        case STATFS_HPFS_MAGIC:
            return "hpfs";
        case STATFS_HPPFS_MAGIC:
            return "hppfs";
        case STATFS_HUGETLBFS_MAGIC:
            return "hugetlbfs";
        case STATFS_ISOFS_MAGIC:
            return "iso9660";
        case STATFS_JFFS2_MAGIC:
            return "jffs2";
        case STATFS_JFS_MAGIC:
            return "jfs";
        case STATFS_LOGFS_MAGIC:
            return "logfs";
        case STATFS_MINIX2_MAGIC:
        case STATFS_MINIX2_MAGIC2:
        case STATFS_MINIX3_MAGIC:
        case STATFS_MINIX_MAGIC:
        case STATFS_MINIX_MAGIC2:
            return "minix";
        case STATFS_MQUEUE_MAGIC:
            return "mqueue";
        case STATFS_MSDOS_MAGIC:
            return "vfat";
        case STATFS_NCP_MAGIC:
            return "ncp";
        case STATFS_NFS_MAGIC:
            return "nfs";
        case STATFS_NILFS_MAGIC:
            return "nilfs2";
        case STATFS_NTFS_MAGIC:
            return "ntfs";
        case STATFS_OCFS2_MAGIC:
            return "ocfs2";
        case STATFS_OMFS_MAGIC:
            return "omfs";
        case STATFS_OPENPROMFS_MAGIC:
            return "openpromfs";
        case STATFS_PIPEFS_MAGIC:
            return "pipefs";
        case STATFS_PROC_MAGIC:
            return "proc";
        case STATFS_PSTOREFS_MAGIC:
            return "pstore";
        case STATFS_QNX4_MAGIC:
            return "qnx4";
        case STATFS_QNX6_MAGIC:
            return "qnx6";
        case STATFS_RAMFS_MAGIC:
            return "ramfs";
        case STATFS_REISERFS_MAGIC:
            return "reiser4";
        case STATFS_ROMFS_MAGIC:
            return "romfs";
        case STATFS_SECURITYFS_MAGIC:
            return "securityfs";
        case STATFS_SELINUXFS_MAGIC:
            return "selinuxfs";
        case STATFS_SMACKFS_MAGIC:
            return "smackfs";
        case STATFS_SMB_MAGIC:
            return "smb";
        case STATFS_SOCKFS_MAGIC:
            return "sockfs";
        case STATFS_SQUASHFS_MAGIC:
            return "squashfs";
        case STATFS_SYSFS_MAGIC:
            return "sysfs";
        case STATFS_TMPFS_MAGIC:
            return "tmpfs";
        case STATFS_UBIFS_MAGIC:
            return "ubifs";
        case STATFS_UDF_MAGIC:
            return "udf";
        case STATFS_UFS2_MAGIC:
        case STATFS_UFS_MAGIC:
            return "ufs";
        case STATFS_V9FS_MAGIC:
            return "9p";
        case STATFS_VXFS_MAGIC:
            return "vxfs";
        case STATFS_XENFS_MAGIC:
            return "xenfs";
...
int mnt_get_uid(const char* username, uid_t* uid)
{
    ...
    {
        DBG(UTILS, ul_debug("cannot convert '%s' username to UID", username));
...
int mnt_get_gid(const char* groupname, gid_t* gid)
{
    ...
    {
        DBG(UTILS, ul_debug("cannot convert '%s' groupname to GID", groupname));
        rc = errno ? -errno : -EINVAL;
    }
...
static int try_write(const char* filename, const char* directory)
{
    ...
    else if (directory)
    {
        ...
        DBG(UTILS,
            ul_debug(" access %s [%s]", rc ? "FAILED" : "OK", directory));
        return rc;
    }
    else
#endif
...
int mnt_has_regular_utab(const char** utab, int* writable)
{
    ...
    if (writable)
    {
        ...
        rc = mkdir(dirname, S_IWUSR | S_IRUSR | S_IRGRP | S_IROTH | S_IXUSR |
                                S_IXGRP | S_IXOTH);
        if (rc && errno != EEXIST)
        {
            free(dirname);
 ...
 * mnt_get_mtab_path:
 *
 * This function returns the *default* location of the mtab file. The result
 * does not have to be writable. See also mnt_has_regular_mtab().
 *
 ...
 *
 * Returns: allocated string with the target of the mounted device or NULL on
 * error
 */
...
char* mnt_get_kernel_cmdline_option(const char* name)
{
    ...
    char buf[BUFSIZ]; /* see kernel include/asm-generic/setup.h:
                         COMMAND_LINE_SIZE */
...
int mnt_guess_system_root(dev_t devno, struct libmnt_cache* cache, char** path)
{
    ...
    DBG(UTILS, ul_debug("guessing system root [devno %u:%u]", major(devno),
                        minor(devno)));
...
{
    int rc;
    char *path = canonicalize_path(argv[1]), *last = NULL;
...
static int test_mkdir(struct libmnt_test* ts, int argc, char* argv[])
{
    ...
    rc = mkdir_p(argv[1], S_IRWXU | S_IRGRP | S_IXGRP | S_IROTH | S_IXOTH);
    if (rc)
...
static int test_statfs_type(struct libmnt_test* ts, int argc, char* argv[])
{
    ...
    if (rc)
        ...
        printf("%-30s: statfs type: %-12s [0x%lx]\n", argv[1],
               mnt_statfs_get_fstype(&vfs), (long)vfs.f_type);
    return rc;
}
...
int main(int argc, char* argv[])
{
    struct libmnt_test tss[] = {
        {"--match-fstype", test_match_fstype,
         "<type> <pattern>     FS types matching"},
        {"--match-options", test_match_options,
         "<options> <pattern>  options matching"},
        {"--filesystems", test_filesystems,
