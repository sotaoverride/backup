 ...
 * SYNC_IN_PROGRESS → libevdev_next_event(LIBEVDEV_READ_FLAG_SYNC_NONE) →
 * SYNC_NONE SYNC_IN_PROGRESS → no sync events left → SYNC_NONE
 *
 */
enum SyncState
{
...
struct logdata
{
    ...
    libevdev_device_log_func_t
        device_handler; /** per-device handler function */
 ...
 */
static inline int queue_pop(struct libevdev* dev, struct input_event* ev)
{
    if (dev->queue_next == 0)
        return 1;
