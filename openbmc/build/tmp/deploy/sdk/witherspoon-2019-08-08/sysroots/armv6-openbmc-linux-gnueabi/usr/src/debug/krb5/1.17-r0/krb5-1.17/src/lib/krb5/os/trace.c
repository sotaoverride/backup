...
static void subfmt(krb5_context context, struct k5buf* buf, const char* fmt,
                   ...);

static krb5_boolean buf_is_printable(const char* p, size_t len)
{
    size_t i;
...
static void buf_add_printable_len(struct k5buf* buf, const char* p, size_t len)
{
    char text[5];
    size_t i;
...
static void buf_add_printable(struct k5buf* buf, const char* p)
{
...
static char* hash_bytes(krb5_context context, const void* ptr, size_t len)
{
    krb5_checksum cksum;
...
static char* principal_type_string(krb5_int32 type)
{
    switch (type)
    {
        ...
        case KRB5_NT_SRV_HST:
            return "service with host as instance";
        case KRB5_NT_SRV_XHST:
            return "service with host as components";
        case KRB5_NT_UID:
            return "unique ID";
        case KRB5_NT_X500_PRINCIPAL:
            return "X.509";
        case KRB5_NT_SMTP_NAME:
            return "SMTP email";
        case KRB5_NT_ENTERPRISE_PRINCIPAL:
            return "Windows 2000 UPN";
        case KRB5_NT_WELLKNOWN:
            return "well-known";
        case KRB5_NT_MS_PRINCIPAL:
            return "Windows 2000 UPN and SID";
        case KRB5_NT_MS_PRINCIPAL_AND_ID:
            return "NT 4 style name";
        case KRB5_NT_ENT_PRINCIPAL_AND_ID:
            return "NT 4 style name and SID";
        default:
            return "?";
    }
}

static char* padata_type_string(krb5_preauthtype type)
{
    switch (type)
    {
        case KRB5_PADATA_TGS_REQ:
            return "PA-TGS-REQ";
        case KRB5_PADATA_ENC_TIMESTAMP:
            return "PA-ENC-TIMESTAMP";
        case KRB5_PADATA_PW_SALT:
            return "PA-PW-SALT";
        case KRB5_PADATA_ENC_UNIX_TIME:
            return "PA-ENC-UNIX-TIME";
        case KRB5_PADATA_ENC_SANDIA_SECURID:
            return "PA-SANDIA-SECUREID";
        case KRB5_PADATA_SESAME:
            return "PA-SESAME";
        case KRB5_PADATA_OSF_DCE:
            return "PA-OSF-DCE";
        case KRB5_CYBERSAFE_SECUREID:
            return "PA-CYBERSAFE-SECUREID";
        case KRB5_PADATA_AFS3_SALT:
            return "PA-AFS3-SALT";
        case KRB5_PADATA_ETYPE_INFO:
            return "PA-ETYPE-INFO";
        case KRB5_PADATA_SAM_CHALLENGE:
            return "PA-SAM-CHALLENGE";
        case KRB5_PADATA_SAM_RESPONSE:
            return "PA-SAM-RESPONSE";
        case KRB5_PADATA_PK_AS_REQ_OLD:
            return "PA-PK-AS-REQ_OLD";
        case KRB5_PADATA_PK_AS_REP_OLD:
            return "PA-PK-AS-REP_OLD";
        ...
        case KRB5_PADATA_SVR_REFERRAL_INFO:
            return "PA-SVR-REFERRAL-INFO";
        case KRB5_PADATA_SAM_REDIRECT:
            return "PA-SAM-REDIRECT";
        case KRB5_PADATA_GET_FROM_TYPED_DATA:
            return "PA-GET-FROM-TYPED-DATA";
        case KRB5_PADATA_SAM_CHALLENGE_2:
            return "PA-SAM-CHALLENGE2";
        case KRB5_PADATA_SAM_RESPONSE_2:
            return "PA-SAM-RESPONSE2";
        case KRB5_PADATA_PAC_REQUEST:
            return "PA-PAC-REQUEST";
        case KRB5_PADATA_FOR_USER:
            return "PA-FOR_USER";
        case KRB5_PADATA_S4U_X509_USER:
            return "PA-FOR-X509-USER";
        case KRB5_PADATA_AS_CHECKSUM:
            return "PA-AS-CHECKSUM";
        case KRB5_PADATA_FX_COOKIE:
            return "PA-FX-COOKIE";
        case KRB5_PADATA_FX_FAST:
            return "PA-FX-FAST";
        case KRB5_PADATA_FX_ERROR:
            return "PA-FX-ERROR";
        case KRB5_PADATA_ENCRYPTED_CHALLENGE:
            return "PA-ENCRYPTED-CHALLENGE";
        case KRB5_PADATA_OTP_CHALLENGE:
            return "PA-OTP-CHALLENGE";
        case KRB5_PADATA_OTP_REQUEST:
            return "PA-OTP-REQUEST";
        case KRB5_PADATA_OTP_PIN_CHANGE:
            return "PA-OTP-PIN-CHANGE";
        case KRB5_PADATA_PKINIT_KX:
            return "PA-PKINIT-KX";
        case KRB5_ENCPADATA_REQ_ENC_PA_REP:
            return "PA-REQ-ENC-PA-REP";
        case KRB5_PADATA_AS_FRESHNESS:
            return "PA_AS_FRESHNESS";
        case KRB5_PADATA_SPAKE:
            return "PA-SPAKE";
        default:
            return NULL;
    }
...
static char* trace_format(krb5_context context, const char* fmt, va_list ap)
{
    struct k5buf buf;
    ...
    while (TRUE)
    {
        ...
        else if (strcmp(tmpbuf, "raddr") == 0)
        {
            ...
            if (getnameinfo((struct sockaddr*)&ra->saddr, ra->len, addrbuf,
                            sizeof(addrbuf), portbuf, sizeof(portbuf),
        ...
        else if (strcmp(tmpbuf, "keyblock") == 0)
        {
            ...
            {
                subfmt(context, &buf, "{etype}/{hashlenstr}", keyblock->enctype,
        ...
        else if (strcmp(tmpbuf, "cksum") == 0)
        {
            ...
            subfmt(context, &buf, "{int}/{hexdata}", (int)cksum->checksum_type,
                   &data);
        }
        ...
        else if (strcmp(tmpbuf, "creds") == 0)
        {
            ...
            subfmt(context, &buf, "{princ} -> {princ}", creds->client,
                   creds->server);
        }
    }
    return buf.data;
...
static void KRB5_CALLCONV file_trace_cb(krb5_context context,
