 ...
 *don't see any include file that is guaranteed to be here, so I'm defining it
...
static void fmtint(char* buffer, size_t* currlen, size_t maxlen, LLONG value,
...
static int add_cnk_list_entry(struct pr_chunk_x** list, int max_num,
                ...
                va_list args_in)
{
        ...
        switch (state)
        {
            ...
            case DP_S_MIN:
                ...
                else if (ch == '*')
                {
                    ...
                    if (pflag)
                    {
                        ...
                        for (num = 0; isdigit((unsigned char)ch);
                             ch = *format++)
                        {
                    ...
                    max_pos =
                        add_cnk_list_entry(&clist, max_pos, cnk->min_star);
            ...
            case DP_S_MAX:
                ...
                else if (ch == '*')
                {
                    ...
                    max_pos =
                        add_cnk_list_entry(&clist, max_pos, cnk->max_star);
                    if (max_pos == 0) /* out of memory :-( */
                        goto done;
    ...
    while (cnk)
    {
        ...
        if (cnk->min_star)
            min = cnk->min_star->value;
        else
            min = cnk->min;
        if (cnk->max_star)
            max = cnk->max_star->value;
        else
            max = cnk->max;
        ...
        switch (cnk->type)
        {
            ...
            case CNK_INT:
            case CNK_UINT:
                fmtint(buffer, &currlen, maxlen, cnk->value, 10, min, max,
                       cnk->flags);
                break;

            case CNK_OCTAL:
                fmtint(buffer, &currlen, maxlen, cnk->value, 8, min, max,
                       cnk->flags);
                break;

            case CNK_HEX:
                fmtint(buffer, &currlen, maxlen, cnk->value, 16, min, max,
                       cnk->flags);
                break;
            ...
            case CNK_STRING:
                ...
                fmtstr(buffer, &currlen, maxlen, cnk->strvalue, cnk->flags, min,
                       max);
                break;

            case CNK_PTR:
                fmtint(buffer, &currlen, maxlen, (long)(cnk->strvalue), 16, min,
                       max, cnk->flags);
                break;
    ...
    if (clist)
    {
        for (pnum = 0; pnum < max_pos; pnum++)
        {
            if (clist[pnum].chunks)
                free(clist[pnum].chunks);
        }
        free(clist);
    }
    return ret;
                              ...
                              struct pr_chunk* chunk)
{
    ...
    if (chunk->num > max_num)
    {
        ...
        {
            l = (struct pr_chunk_x*)realloc(*list,
                                            sizeof(struct pr_chunk_x) * max);
            pos = max_num;
        }
        if (l == NULL)
        {
    ...
    {
        c = (struct pr_chunk**)realloc(l[i].chunks,
                                       sizeof(struct pr_chunk*) * cnum);
    }
    if (c == NULL)
    {
        for (i = 0; i < max; i++)
...
int main(void)
{
    ...
    char buf2[1024];
    char* buf3;
    char* fp_fmt[] = {"%1.1f",   "%-1.5f",  "%1.5f",  "%123.9f", "%10.5f",
                      "% 10.5f", "%+22.9f", "%+4.9f", "%01.3f",  "%4f",
                      "%3.1f",   "%3.2f",   "%.0f",   "%f",      "%-8.8f",
                      "%-9.9f",  NULL};
    double fp_nums[] = {6442452944.1234, -1.5,     134.21,  91340.2, 341.1234,
                        203.9,           0.96,     0.996,   0.9996,  1.996,
                        4.136,           5.030201, 0.00205,
                        /* END LIST */ 0};
    char* int_fmt[] = {"%-1.5d", "%1.5d",   "%123.9d",  "%5.5d",
                       "%10.5d", "% 10.5d", "%+22.33d", "%01.3d",
                       "%4d",    "%d",      NULL};
    long int_nums[] = {-1, 134, 91340, 341, 0203, 1234567890, 0};
    char* str_fmt[] = {"%10.5s", "%-10.5s", "%5.10s", "%-5.10s", "%10.1s",
                       "%0.10s", "%10.0s",  "%1.10s", "%s",      "%.1s",
                       "%.10s",  "%10s",    NULL};
    char* str_vals[] = {"hello", "a", "", "a longer string", NULL};
#ifdef HAVE_LONG_LONG
    char* ll_fmt[] = {"%llu", NULL};
    LLONG ll_nums[] = {134, 91340, 341, 0203, 1234567890, 128006186140000000LL,
                       0};
#endif
    int x, y;
    int fail = 0;
...
#define BUFSZ 2048
    ...
    buf1[0] = buf2[0] = '\0';
    l1 = snprintf(buf1, sizeof(buf1), "%4$*1$d %2$s %3$*1$.*1$f", 3, "pos test",
                  12.3456, 9);
    ...
    buf1[0] = buf2[0] = '\0';
    l1 = snprintf(buf1, sizeof(buf1), "%4$*4$d %2$s %3$*4$.*4$f", 3, "pos test",
                  12.3456, 9);
