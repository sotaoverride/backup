...
static const struct argp_option argp_version_options[] = {
    {"version", 'V', 0, 0, N_("Print program version"), -1}, {0, 0}};
...
static error_t group_parse(struct group* group, struct argp_state* state,
                           int key, char* arg)
{
    if (group->parser)
    {
                ...
                /* A real option (not just documentation).  */
                {
                    ...
                    /* OPT can be used as a long option.  */
                    {
                        ...
                        cvt->long_end->has_arg =
                            (real->arg ? (real->flags & OPTION_ARG_OPTIONAL
                                              ? optional_argument
                                              : required_argument)
                                       : no_argument);
                        cvt->long_end->flag = 0;
...
#define SLEN (szs.short_len + 1)
    ...
    for (group = parser->groups;
         group < parser->egroup && (!err || err == EBADKEY); group++)
    {
        if (group->parent)
        ...
        if (!group->parser && group->argp->children &&
...
static error_t parser_finalize(struct parser* parser, error_t err,
                               int arg_ebadkey, int* end_index)
{
    struct group* group;
           ...
           just a few more times... */
        {
            for (group = parser->groups;
                ...
                if (group->args_processed == 0)
                    err =
                        group_parse(group, &parser->state, ARGP_KEY_NO_ARGS, 0);
            for (group = parser->egroup - 1;
                 group >= parser->groups && (!err || err == EBADKEY); group--)
...
static error_t parser_parse_arg(struct parser* parser, char* val)
{
    ...
    for (group = parser->groups; group < parser->egroup && err == EBADKEY;
         group++)
    {
...
static error_t parser_parse_opt(struct parser* parser, int opt, char* val)
{
       ...
       determine which group OPT came from.  */
    {
        ...
        err = group_parse(&parser->groups[group_key - 1], &parser->state,
...
error_t __argp_parse(const struct argp* argp, int argc, char** argv,
