...
uint32_t nl_cli_parse_u32(const char* arg)
{
    ...
    if (endptr == arg || lval == ULONG_MAX)
        nl_cli_fatal(EINVAL, "Unable to parse \"%s\", not a number.", arg);
...
void nl_cli_print_version(void)
{
    ...
    printf("Copyright (C) 2003-2010 Thomas Graf <tgraf@redhat.com>\n"
           "\n"
...
struct nl_addr* nl_cli_addr_parse(const char* str, int family)
{
    ...
    if ((err = nl_addr_parse(str, family, &addr)) < 0)
        nl_cli_fatal(err, "Unable to parse address \"%s\": %s", str,
                     nl_geterror(err));
                   ...
                   int default_yes)
{
    ...
    for (;;)
    {
        ...
        printf("Delete? (%c/%c) ", default_yes ? 'Y' : 'y',
               default_yes ? 'n' : 'N');
                                              ...
                                              struct nl_cache**))
{
    ...
    if ((err = ac(sock, &cache)) < 0)
        nl_cli_fatal(err, "Unable to allocate %s cache: %s", name,
                     nl_geterror(err));
...
struct nl_cache* nl_cli_alloc_cache_flags(
    struct nl_sock* sock, const char* name, unsigned int flags,
...
void nl_cli_load_module(const char* prefix, const char* name)
{
    ...
    snprintf(path, sizeof(path), "%s/%s/%s.so", PKGLIBDIR, prefix, name);
...
#else
    nl_cli_fatal(ENOTSUP,
                 "Unable to load module \"%s\": built without dynamic "
                 "libraries support\n",
                 path);
#endif
}
