...
void setup_iconv(void)
{
    ...
    if (!am_server && !allow_8bit_chars)
    {
        ...
        if (DEBUG_GTE(ICONV, 2))
        {
            ...
            {
                rprintf(FINFO, "msg checking charset: %s\n", defset);
            }
        }
    }
    else
    ...
    do
    {
        ...
        else if (convert)
        {
            ...
            iconvbufs(ic_send, &inbuf, &outbuf,
                      ICB_EXPAND_OUT | ICB_INCLUDE_BAD |
                          ICB_INCLUDE_INCOMPLETE | ICB_INIT);
...
read_loop:
    while (1)
    {
        ...
        if (ndx < 0 || ndx >= dir_flist->used)
        {
            ...
            rprintf(FERROR, "Invalid dir index: %d (%d - %d) [%s]\n", ndx,
                    NDX_FLIST_OFFSET, NDX_FLIST_OFFSET - dir_flist->used + 1,
                    who_am_i());
            exit_cleanup(RERR_PROTOCOL);
        }
    ...
    if (protocol_version < 30 && ndx == cur_flist->used &&
        iflags == ITEM_IS_NEW)
    {
        if (am_sender)
                   ...
                   const char* fnamecmp, int flags)
{
        ...
        if (DEBUG_GTE(OWN, 1))
        {
            if (change_uid)
            {
                rprintf(FINFO, "set uid of %s from %u to %u\n", fname,
                        (unsigned)sxp->st.st_uid, F_OWNER(file));
            }
            if (change_gid)
            {
                rprintf(FINFO, "set gid of %s from %u to %u\n", fname,
                        (unsigned)sxp->st.st_gid, F_GROUP(file));
            }
        }
        if (am_root >= 0)
        {
            ...
            if (do_lchown(fname, uid, gid) != 0)
            {
                ...
                rsyserr(FERROR_XFER, errno, "%s %s failed",
                        change_uid ? "chown" : "chgrp", full_fname(fname));
                goto cleanup;
            }
...
#ifdef ST_MTIME_NSEC
         || (flags & ATTRS_SET_NANO && NSEC_BUMP(file) &&
             (uint32)sxp->st.ST_MTIME_NSEC != F_MOD_NSEC(file))
#endif
             ))
    {
    ...
    if (!BITS_EQUAL(sxp->st.st_mode, new_mode, CHMOD_BITS))
    {
        ...
        if (ret < 0)
        {
            rsyserr(FERROR_XFER, errno, "failed to set permissions on %s",
                    full_fname(fname));
            goto cleanup;
        }
        if (ret == 0) /* ret == 1 if symlink could not be set */
...
{
    int ret;
    const char* temp_copy_name =
        partialptr && *partialptr != '/' ? partialptr : NULL;
    ...
    if (ret < 0)
    {
        rsyserr(FERROR_XFER, errno, "%s %s -> \"%s\"",
                ret == -2 ? "copy" : "rename", full_fname(fnametmp), fname);
...
not_found:
    if (fatal_error_loc)
    {
        ...
        rprintf(FERROR, "File-list index %d not in %d - %d (%s) [%s]\n", ndx,
                first, last, fatal_error_loc, who_am_i());
        exit_cleanup(RERR_PROTOCOL);
    }
    return NULL;
}
