...
int __initial_check(const char* test, int argc, char* const argv[])
{
    ...
    {
        __errorm(test, __func__, __LINE__, "device %s is not empty", argv[1]);
        goto close;
    }
 ...
 * @fmt   format string
 */
void __errorm(const char* test, const char* func, int line, const char* fmt,
              ...)
{
    va_list args;
 ...
 */
void __failed(const char* test, const char* func, int line, const char* failed)
{
                   ...
                   const struct ubi_mkvol_request* req)
{
    ...
    {
        __errorm(test, func, line, "bad alignment: requested %d, got %d",
    ...
    {
        __errorm(test, func, line, "bad name: requested \"%s\", got \"%s\"",
    ...
    {
        __errorm(test, func, line, "bad usable LEB size %d, should be %d",
    ...
    {
        __errorm(test, func, line, "bad reserved bytes %lld, should be %lld",
                     ...
                     int line, const char* node, uint8_t byte)
{
    ...
    while (bytes < vol_info.data_bytes)
    {
        ...
        if (ret == -1)
        {
            ...
            __errorm(test, func, line, "bytes = %lld, ret = %d", bytes, ret);
            goto close;
        }

        if (ret == 0 && bytes + ret < vol_info.data_bytes)
        {
            __errorm(test, func, line, "EOF, but read only %lld bytes of %lld",
        ...
        for (i = 0; i < ret; i++)
            if (buf[i] != byte)
            {
                __errorm(test, func, line, "byte at %lld is not %#x but %#x",
                      ...
                      int line, const char* node, long long bytes, uint8_t byte)
{
    ...
    while (written != bytes)
    {
        ...
        if (ret == -1)
        {
            ...
            __errorm(test, func, line, "written = %lld, ret = %d", written,
                     ret);
            goto close;
        }
        written += ret;
