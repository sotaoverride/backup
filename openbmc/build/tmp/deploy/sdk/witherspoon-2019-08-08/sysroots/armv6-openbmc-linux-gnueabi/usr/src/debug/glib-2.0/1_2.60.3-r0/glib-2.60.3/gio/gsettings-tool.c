...
static gboolean is_relocatable_schema(GSettingsSchema* schema)
{
    return g_settings_schema_get_path(schema) == NULL;
}

static gboolean check_relocatable_schema(GSettingsSchema* schema,
...
static gboolean check_schema(GSettingsSchema* schema, const gchar* schema_id)
{
    if (schema == NULL)
    {
        g_printerr(_("No such schema “%s”\n"), schema_id);
...
static void output_list(gchar** list)
{
    gint i;
...
static void gsettings_print_version(void)
{
    g_print("%d.%d.%d\n", glib_major_version, glib_minor_version,
            glib_micro_version);
}

static void gsettings_list_schemas(void)
{
    gchar** schemas;

    g_settings_schema_source_list_schemas(global_schema_source, TRUE, &schemas,
                                          NULL);
    output_list(schemas);
    g_strfreev(schemas);
}

static void gsettings_list_schemas_with_paths(void)
{
    gchar** schemas;
    gsize i;
    ...
    for (i = 0; schemas[i] != NULL; i++)
    {
        ...
        schema = g_settings_schema_source_lookup(global_schema_source,
                                                 schema_name, TRUE);
...
static void gsettings_list_relocatable_schemas(void)
{
    gchar** schemas;

    g_settings_schema_source_list_schemas(global_schema_source, TRUE, NULL,
                                          &schemas);
    output_list(schemas);
    g_strfreev(schemas);
}

static void gsettings_list_keys(void)
{
    gchar** keys;
...
static void gsettings_list_children(void)
{
    gchar** children;
    gint max = 0;
    gint i;
    ...
    for (i = 0; children[i]; i++)
    {
        ...
        g_object_get(child, "settings-schema", &schema, "path", &path, NULL);
...
static void enumerate(GSettings* settings)
{
    ...
    for (i = 0; keys[i]; i++)
    {
        ...
        g_print("%s %s %s\n", g_settings_schema_get_id(schema), keys[i],
                printed);
...
static void list_recursively(GSettings* settings)
{
    gchar** children;
    gint i;
...
static void gsettings_list_recursively(void)
{
    if (global_settings)
    {
    ...
    else
    {
        ...
        g_settings_schema_source_list_schemas(global_schema_source, TRUE,
                                              &schemas, NULL);
...
static void gsettings_description(void)
{
...
static void gsettings_range(void)
{
...
static void gsettings_get(void)
{
    GVariant* value;
    gchar* printed;
...
static void gsettings_reset(void)
{
    g_settings_reset(global_settings, global_key);
    g_settings_sync();
}

static void reset_all_keys(GSettings* settings)
{
    GSettingsSchema* schema;
    gchar** keys;
    gint i;
...
static void gsettings_reset_recursively(void)
{
    gchar** children;
    gint i;
...
static void gsettings_writable(void)
{
...
static void value_changed(GSettings* settings, const gchar* key,
                          gpointer user_data)
{
    GVariant* value;
    gchar* printed;
...
static void gsettings_monitor(void)
{
    if (global_key)
    {
        gchar* name;
        ...
        g_signal_connect(global_settings, name, G_CALLBACK(value_changed),
                         NULL);
    }
    else
        g_signal_connect(global_settings, "changed", G_CALLBACK(value_changed),
                         NULL);
...
static void gsettings_set(void)
{
     ...
     */
    if (new == NULL && g_variant_type_equal(type, G_VARIANT_TYPE_STRING) &&
...
static int gsettings_help(gboolean requested, const gchar* command)
{
    ...
    else
    {
        ...
        if (strstr(synopsis, "[COMMAND]"))
            g_string_append(
                string, _("  COMMAND   The (optional) command to explain\n"));

        else if (strstr(synopsis, "SCHEMA"))
            g_string_append(
                string, _("  SCHEMA    The name of the schema\n"
        ...
        if (strstr(synopsis, "VALUE"))
            g_string_append(string, _("  VALUE     The value to set\n"));
...
int main(int argc, char** argv)
{
    ...
    if (argc > 3 && g_str_equal(argv[1], "--schemadir"))
    {
        ...
        /* shift remaining arguments (not correct wrt argv[0], but doesn't
         * matter) */
        argv = argv + 2;
        argc -= 2;
    }
    ...
    else if (argc == 3 && strcmp(argv[1], "list-keys") == 0)
    {
        ...
        function = gsettings_list_children;
             ...
             strcmp(argv[1], "list-recursively") == 0)
    ...
    if (argc > 2 && !skip_third_arg_test)
    {
        ...
        global_schema = g_settings_schema_source_lookup(global_schema_source,
                                                        parts[0], TRUE);
        ...
        if (need_settings)
        {
            if (parts[1])
            {
                if (!check_relocatable_schema(global_schema, parts[0]) ||
                    !check_path(parts[1]))
                    return 1;

                global_settings =
                    g_settings_new_full(global_schema, NULL, parts[1]);
            }
            else
            {
                ...
                global_settings =
                    g_settings_new_full(global_schema, NULL, NULL);
            }
        }
        else
        {
    ...
    if (argc > 3)
    {
        ...
        global_key = argv[3];
        global_schema_key =
            g_settings_schema_get_key(global_schema, global_key);
    }
