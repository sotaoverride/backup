...
BOOST_LOG_ANONYMOUS_NAMESPACE
{
        ...
        : public log::aux::lazy_singleton<filters_repository<CharT>>
    {
        ...
        typedef log::aux::lazy_singleton<filters_repository<char_type>>
            base_type;
        ...
        struct attribute_name_order
        {
            ...
            result_type operator()(attribute_name const& left,
                                   attribute_name const& right) const
            {
                return left.id() < right.id();
            }
        };

        typedef std::map<attribute_name, shared_ptr<filter_factory_type>,
        ...
        //! The method returns the filter factory for the specified attribute
        //! name
        filter_factory_type& get_factory(attribute_name const& name) const
        {
            ...
            else
            {
                ...
                BOOST_LOG_THROW_DESCR(
                    setup_error, "No filter factory registered for attribute " +
                                     name.string());
#endif
            }
        }
    ...
    class filter_parser
    {
      private:
        ...
        typedef filter (filter_factory_type::*comparison_relation_handler_t)(
            attribute_name const&, string_type const&);
      ...
      public:
        //! Constructor
        filter_parser() : m_ComparisonRelation(NULL)
        {
        }
                   ...
                   unsigned int depth = 0)
        {
            ...
            while (p != end)
            {
                ...
                if (logical_op)
                {
                    // This was the right-hand subexpression. Compose the two
                    // top subexpressions into a single filter.
                    (this->*logical_op)();
                    logical_op = NULL;
                }
                ...
                if (p != end)
                {
                    ...
                    if (c == constants::char_paren_bracket_right)
                    {
                        ...
                        if (depth == 0)
                            BOOST_LOG_THROW_DESCR(
                                parse_error, "Invalid filter definition: "
                    ...
                    else if (c == constants::char_and ||
                             scan_keyword(p, end, next,
                                          constants::and_keyword()))
                    {
                        logical_op = &filter_parser::on_and;
                    }
                    else if (c == constants::char_or ||
                             scan_keyword(p, end, next,
                                          constants::or_keyword()))
                    {
            ...
            {
                BOOST_LOG_THROW_DESCR(
                    parse_error, "Invalid filter definition: logical operation "
                                 "without the right-hand subexpression");
            }
      ...
      private:
        ...
        void parse_subexpression(iterator_type& begin, iterator_type end,
                                 unsigned int depth)
        {
            ...
            while (p != end)
            {
                ...
                if (c == constants::char_percent)
                {
                    ...
                    iterator_type start =
                        constants::trim_spaces_left(next, end);
                    p = constants::scan_attr_placeholder(start, end);
                    if (p == end)
                        BOOST_LOG_THROW_DESCR(parse_error,
                    ...
                    // If the filter has negation operator, do not expect a
                    // relation (i.e. "!%attr% > 1" is not valid because
                    // "!%attr%" is interpreted as an attribute presence test)
                    if (!negation_present)
                        p = parse_relation(p, end);
                    else
                        on_relation_complete();
                }
                else if (c == constants::char_exclamation ||
                         scan_keyword(p, end, next, constants::not_keyword()))
                {
                    // We found negation operation. Parse the subexpression to
                    // be negated.
                    negated ^= true;
                ...
                else
                {
                    BOOST_LOG_THROW_DESCR(
                        parse_error,
                        "Invalid filter definition: unexpected character");
                }
            ...
            if (negation_present && !done)
            {
                ...
                BOOST_LOG_THROW_DESCR(parse_error,
                                      "Filter parsing error: negation operator "
                                      "applied to nothingness");
            }
            ...
            if (p != end)
            {
                ...
                if (scan_keyword(p, end, next, constants::equal_keyword()))
                    m_ComparisonRelation =
                        &filter_factory_type::on_equality_relation;
                else if (scan_keyword(p, end, next,
                                      constants::not_equal_keyword()))
                    m_ComparisonRelation =
                        &filter_factory_type::on_inequality_relation;
                else if (scan_keyword(p, end, next,
                                      constants::greater_keyword()))
                    m_ComparisonRelation =
                        &filter_factory_type::on_greater_relation;
                else if (scan_keyword(p, end, next, constants::less_keyword()))
                    m_ComparisonRelation =
                        &filter_factory_type::on_less_relation;
                else if (scan_keyword(p, end, next,
                                      constants::greater_or_equal_keyword()))
                    m_ComparisonRelation =
                        &filter_factory_type::on_greater_or_equal_relation;
                else if (scan_keyword(p, end, next,
                                      constants::less_or_equal_keyword()))
                    m_ComparisonRelation =
                        &filter_factory_type::on_less_or_equal_relation;
                else
                {
                    // Check for custom relation
                    while (next != end && (encoding::isalnum(*next) ||
                                           *next == constants::char_underline))
                        ++next;
                    if (p == next)
                        goto DoneL;
        ...
        DoneL:
            // The relation can be as simple as a sole attribute placeholder
            // (which means that only attribute presence has to be checked). So
            // regardless how we get here, the relation is parsed completely.
            on_relation_complete();
        ...
        static bool scan_keyword(iterator_type begin, iterator_type end,
                                 iterator_type& next, iterator_type keyword)
        {
        ...
        void on_relation_complete()
        {
            if (!!m_AttributeName)
            {
                filters_repository<char_type> const& repo =
                    filters_repository<char_type>::get();
                filter_factory_type& factory =
                    repo.get_factory(m_AttributeName);

                if (!!m_Operand)
                {
                    if (!!m_ComparisonRelation)
                    {
                        m_Subexpressions.push((factory.*m_ComparisonRelation)(
                            m_AttributeName, m_Operand.get()));
                        m_ComparisonRelation = NULL;
                    }
                    else if (!m_CustomRelation.empty())
                    {
                        m_Subexpressions.push(factory.on_custom_relation(
                            m_AttributeName, m_CustomRelation,
                            m_Operand.get()));
                    ...
                    else
                    {
                        ...
                        BOOST_ASSERT_MSG(
                            false,
                            "Filter parser internal error: the attribute name "
                            "or subexpression operation is not set while "
                            "trying to construct a relation");
                        BOOST_LOG_THROW_DESCR(
                            parse_error,
                            "Filter parser internal error: the attribute name "
                            "or subexpression operation is not set while "
                            "trying to construct a subexpression");
                    }
                ...
                {
                    // This branch is taken if the relation is a single
                    // attribute name, which is recognized as the attribute
                    // presence check
                    BOOST_ASSERT_MSG(
                        !m_ComparisonRelation && m_CustomRelation.empty(),
                        "Filter parser internal error: the relation operation "
                        "is set while operand is not");
                    m_Subexpressions.push(
                        factory.on_exists_test(m_AttributeName));
                }
            ...
            else
            {
                // This should never happen
                BOOST_ASSERT_MSG(
                    false, "Filter parser internal error: the attribute name "
                           "is not set while trying to construct a relation");
                BOOST_LOG_THROW_DESCR(
                    parse_error,
                    "Filter parser internal error: the attribute name is not "
                    "set while trying to construct a relation");
            }
        }
        ...
        void on_negation()
        {
            if (!m_Subexpressions.empty())
            {
                m_Subexpressions.top() = !phoenix::bind(
                    m_Subexpressions.top(), phoenix::placeholders::_1);
            }
            else
            {
                ...
                BOOST_LOG_THROW_DESCR(parse_error,
                                      "Filter parsing error: negation operator "
                                      "applied to an empty subexpression");
            }
        }
        ...
        void on_and()
        {
            if (!m_Subexpressions.empty())
            {
                ...
                if (!m_Subexpressions.empty())
                {
                    ...
                    m_Subexpressions.top() =
                        phoenix::bind(left, phoenix::placeholders::_1) &&
                        phoenix::bind(right, phoenix::placeholders::_1);
                    return;
                }
            }
            ...
            // This should never happen
            BOOST_LOG_THROW_DESCR(
                parse_error, "Filter parser internal error: the subexpression "
                             "is not set while trying to construct a filter");
        }
...
//! The function registers a filter factory object for the specified attribute
//! name
template <typename CharT>
                            ...
                            shared_ptr<filter_factory<CharT>> const& factory)
{
    ...
    BOOST_LOG_EXPR_IF_MT(
        log::aux::exclusive_lock_guard<log::aux::light_rw_mutex> lock(
            repo.m_Mutex);)
...
BOOST_LOG_SETUP_API filter parse_filter(const CharT* begin, const CharT* end)
{
    ...
    BOOST_LOG_EXPR_IF_MT(filters_repository<CharT>& repo =
                             filters_repository<CharT>::get();)
    BOOST_LOG_EXPR_IF_MT(
...
template BOOST_LOG_SETUP_API void register_filter_factory<char>(
    attribute_name const& name,
    shared_ptr<filter_factory<char>> const& factory);
template BOOST_LOG_SETUP_API filter parse_filter<char>(const char* begin,
                                                       const char* end);
...
template BOOST_LOG_SETUP_API void register_filter_factory<wchar_t>(
    attribute_name const& name,
    shared_ptr<filter_factory<wchar_t>> const& factory);
template BOOST_LOG_SETUP_API filter parse_filter<wchar_t>(const wchar_t* begin,
                                                          const wchar_t* end);
