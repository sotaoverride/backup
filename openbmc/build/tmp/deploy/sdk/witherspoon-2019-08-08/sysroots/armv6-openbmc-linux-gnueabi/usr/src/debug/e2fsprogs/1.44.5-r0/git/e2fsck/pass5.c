...
static void check_inode_bitmap_checksum(e2fsck_t ctx)
{
    ...
    for (i = 0; i < ctx->fs->group_desc_count; i++)
    {
        ...
        retval = ext2fs_get_inode_bitmap_range2(ctx->fs->inode_map, ino_itr,
                                                nbytes << 3, buf);
        if (retval)
            break;
...
static void check_block_bitmap_checksum(e2fsck_t ctx)
{
    ...
    for (i = 0; i < ctx->fs->group_desc_count; i++)
    {
        ...
        blk_itr = EXT2FS_B2C(ctx->fs, ctx->fs->super->s_first_data_block) +
                  ((blk64_t)i * (nbytes << 3));
        retval = ext2fs_get_block_bitmap_range2(ctx->fs->block_map, blk_itr,
                                                nbytes << 3, buf);
        if (retval)
            break;
...
static void e2fsck_discard_blocks(e2fsck_t ctx, blk64_t start, blk64_t count)
{
 ...
 */
static void e2fsck_discard_inodes(e2fsck_t ctx, dgrp_t group, ext2_ino_t start,
                                  int count)
{
    ext2_filsys fs = ctx->fs;
    blk64_t blk, num;
     ...
     * inodes in the table.
     */
    blk = DIV_ROUND_UP(start, EXT2_INODES_PER_BLOCK(fs->super));
...
static void check_block_bitmaps(e2fsck_t ctx)
{
    ...
    clear_problem_context(&pctx);
    free_array = (unsigned int*)e2fsck_allocate_memory(
        ctx, fs->group_desc_count * sizeof(unsigned int),
...
redo_counts:
    ...
    for (i = B2C(fs->super->s_first_data_block);
         i < ext2fs_blocks_count(fs->super); i += EXT2FS_CLUSTER_RATIO(fs))
    {
        int first_block_in_bg = (B2C(i) - B2C(fs->super->s_first_data_block)) %
         ...
         */
        if (!first_block_in_bg || (group == fs->group_desc_count - 1) ||
            (ctx->options & E2F_OPT_DISCARD))
            goto no_optimize;

        retval = ext2fs_get_block_bitmap_range2(ctx->block_found_map, B2C(i),
                                                fs->super->s_clusters_per_group,
                                                actual_buf);
        if (retval)
            goto no_optimize;
        retval = ext2fs_get_block_bitmap_range2(
            fs->block_map, B2C(i), fs->super->s_clusters_per_group, bitmap_buf);
        if (retval)
            goto no_optimize;
    ...
    do_counts:
        ...
        {
            e2fsck_discard_blocks(ctx, first_free, (i - first_free));
             ...
             EXT2FS_B2C(fs, ext2fs_blocks_count(fs->super) - 1)))
        {
            ...
            if (!bitmap && i >= first_free)
                e2fsck_discard_blocks(ctx, first_free, (i - first_free) + 1);
        next_group:
            first_free = ext2fs_blocks_count(fs->super);
            ...
            group_free = 0;
            if (ctx->progress)
                if ((ctx->progress)(ctx, 5, group, fs->group_desc_count * 2))
                    goto errout;
        }
    }
    if (pctx.blk != NO_BLK)
    ...
    if (fixit == 1)
    {
        ...
        retval = ext2fs_copy_bitmap(ctx->block_found_map, &fs->block_map);
        if (retval)
        {
            clear_problem_context(&pctx);
    ...
    for (g = 0; g < fs->group_desc_count; g++)
    {
        if (free_array[g] != ext2fs_bg_free_blocks_count(fs, g))
        {
            ...
            if (fix_problem(ctx, PR_5_FREE_BLOCK_COUNT_GROUP, &pctx))
            {
...
static void check_inode_bitmaps(e2fsck_t ctx)
{
    ...
    clear_problem_context(&pctx);
    free_array = (ext2_ino_t*)e2fsck_allocate_memory(
        ctx, fs->group_desc_count * sizeof(ext2_ino_t),
        "free inode count array");

    dir_array = (ext2_ino_t*)e2fsck_allocate_memory(
        ctx, fs->group_desc_count * sizeof(ext2_ino_t),
...
redo_counts:
    ...
    save_problem = 0;
    pctx.ino = pctx.ino2 = 0;
    if (csum_flag && (ext2fs_bg_flags_test(fs, group, EXT2_BG_INODE_UNINIT)))
        skip_group++;
    ...
    do_counts:
            ...
            (i == fs->super->s_inodes_count))
        {
            ...
            if ((ctx->options & E2F_OPT_DISCARD) &&
                io_channel_discard_zeroes_data(fs->io) &&
                !(ext2fs_bg_flags_test(fs, group, EXT2_BG_INODE_ZEROED)))
            {
                ext2fs_bg_flags_set(fs, group, EXT2_BG_INODE_ZEROED);
    ...
    if (fixit == 1)
    {
        ...
        retval = ext2fs_copy_bitmap(ctx->inode_used_map, &fs->inode_map);
        if (retval)
        {
            clear_problem_context(&pctx);
    ...
    for (i = 0; i < fs->group_desc_count; i++)
    {
        if (free_array[i] != ext2fs_bg_free_inodes_count(fs, i))
        {
            ...
            if (fix_problem(ctx, PR_5_FREE_INODE_COUNT_GROUP, &pctx))
            {
        ...
        if (dir_array[i] != ext2fs_bg_used_dirs_count(fs, i))
        {
            ...
            if (fix_problem(ctx, PR_5_FREE_DIR_COUNT_GROUP, &pctx))
            {
...
static void check_inode_end(e2fsck_t ctx)
{
    ...
    pctx.errcode =
        ext2fs_fudge_inode_bitmap_end(fs->inode_map, end, &save_inodes_count);
    if (pctx.errcode)
    {
        pctx.num = 1;
    ...
    pctx.errcode =
        ext2fs_fudge_inode_bitmap_end(fs->inode_map, save_inodes_count, 0);
    if (pctx.errcode)
    {
        pctx.num = 2;
        fix_problem(ctx, PR_5_FUDGE_BITMAP_ERROR, &pctx);
...
static void check_block_end(e2fsck_t ctx)
{
    ...
    pctx.errcode =
        ext2fs_fudge_block_bitmap_end2(fs->block_map, end, &save_blocks_count);
    if (pctx.errcode)
    {
        pctx.num = 3;
        fix_problem(ctx, PR_5_FUDGE_BITMAP_ERROR, &pctx);
    ...
    {
        if (!ext2fs_test_block_bitmap2(fs->block_map, EXT2FS_C2B(fs, i)))
        {
            if (fix_problem(ctx, PR_5_BLOCK_BMAP_PADDING, &pctx))
            {
                for (; i <= end; i++)
                    ext2fs_mark_block_bitmap2(fs->block_map, EXT2FS_C2B(fs, i));
                ext2fs_mark_bb_dirty(fs);
            }
            else
                ext2fs_unmark_valid(fs);
    ...
    pctx.errcode =
        ext2fs_fudge_block_bitmap_end2(fs->block_map, save_blocks_count, 0);
    if (pctx.errcode)
    {
        pctx.num = 4;
        fix_problem(ctx, PR_5_FUDGE_BITMAP_ERROR, &pctx);
