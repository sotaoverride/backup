...
td_err_e __td_ta_lookup_th_unique(const td_thragent_t* ta_arg, lwpid_t lwpid,
                                  td_thrhandle_t* th)
{
    ...
    if (ta->ta_howto == ta_howto_unknown)
    {
        ...
        if (err == PS_OK)
        {
            err = ps_pdread(ta->ph, howto, &ta->ta_howto_data.const_thread_area,
        ...
        else
        {
            ...
            err =
                ps_pdread(ta->ph, howto, ta->ta_howto_data.reg, DB_SIZEOF_DESC);
            if (err != PS_OK)
                return TD_ERR;
    ...
    switch (ta->ta_howto)
    {
        ...
        case ta_howto_reg:
            ...
            if (ps_lgetregs(ta->ph, lwpid, regs) != PS_OK)
                return TD_ERR;
            terr = _td_fetch_value_local(ta, ta->ta_howto_data.reg, -1, 0, regs,
                                         &addr);
            if (terr != TD_OK)
                return terr;
...
td_err_e td_ta_map_lwp2thr(const td_thragent_t* ta_arg, lwpid_t lwpid,
                           td_thrhandle_t* th)
{
    td_thragent_t* const ta = (td_thragent_t*)ta_arg;
