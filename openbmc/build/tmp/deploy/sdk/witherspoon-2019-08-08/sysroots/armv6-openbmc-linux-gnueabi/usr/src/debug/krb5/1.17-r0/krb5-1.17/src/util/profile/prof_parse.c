...
static errcode_t parse_std_line(char* line, struct parse_state* state)
{
    ...
    if (ch == '[')
    {
        ...
        *p = '\0';
        retval = profile_find_node_subsection(state->root_section, cp, &iter, 0,
                            ...
                            char** ret_modspec)
{
    ...
    switch (state->state)
    {
        case STATE_INIT_COMMENT:
            ...
            {
                /*
                 * If we are expecting a module declaration, fill in
                 * *ret_modspec and return PROF_MODULE, which will cause parsing
                 * to abort and the module to be loaded instead.  If we aren't
                 * expecting a module declaration, return PROF_MODULE without
                 * filling in *ret_modspec, which will be treated as an ordinary
                 * error.
                 */
                if (ret_modspec)
                {
                         ...
                         void (*cb)(const char*, void*), void* data)
{
    ...
    do
    {
        retval = profile_find_node_relation(root, 0, &iter, &name, &value);
        if (retval)
            break;
        for (i = 0; i < level; i++)
            cb("\t", data);
    ...
    do
    {
        retval = profile_find_node_subsection(root, 0, &iter, &name, &p);
        if (retval)
            break;
        if (level == 0)
        { /* [xxx] */
