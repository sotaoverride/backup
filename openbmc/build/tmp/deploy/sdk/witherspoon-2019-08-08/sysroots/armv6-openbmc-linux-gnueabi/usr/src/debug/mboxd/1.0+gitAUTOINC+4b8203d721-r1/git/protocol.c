                                     ...
                                     struct protocol_create_window* io)
{
    ...
    context->current =
        windows_search(context, offset, context->version == API_VERSION_1);
                                  ...
                                  struct protocol_mark_dirty* io)
{
    ...
    off = offset - ((context->current->flash_offset) >>
                    context->backend.block_size_shift);
    if (off > offset)
    { /* Underflow - before current window */
...
static int generic_flush(struct mbox_context* context)
{
     ...
     */
    for (i = 0;
         i < (context->current->size >> context->backend.block_size_shift); i++)
    {
        uint8_t cur = context->current->dirty_bmap[i];
        if (cur != WINDOW_CLEAN)
        {
            if (cur == prev)
            { /* Same as previous block, incrmnt */
                count++;
            }
            else if (prev == WINDOW_CLEAN)
            { /* Start of run */
                offset = i;
                count++;
            }
            else
            { /* Change in streak type */
...
static uint16_t get_suggested_timeout(struct mbox_context* context)
{
    ...
    MSG_DBG("Suggested Timeout: %us, max window size: %uMB, for %dms/MB\n", ret,
            max_size_mb, FLASH_ACCESS_MS_PER_MB);
    return ret;
}
                                      ...
                                      struct protocol_get_flash_info* io)
{
    ...
    io->resp.v2.flash_size = backend->flash_size >> backend->block_size_shift;
                                     ...
                                     struct protocol_create_window* io)
{
    ...
    io->resp.offset =
        context->current->flash_offset >> context->backend.block_size_shift;
