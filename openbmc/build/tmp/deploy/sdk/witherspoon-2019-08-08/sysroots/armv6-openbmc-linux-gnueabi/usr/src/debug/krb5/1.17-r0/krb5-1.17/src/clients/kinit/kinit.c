...
static void usage()
{
    fprintf(stderr,
            "Usage: %s [-V] "
            "[-l lifetime] [-s start_time] " USAGE_BREAK "[-r renewable_life] "
            "[-f | -F | --forwardable | --noforwardable] " USAGE_BREAK
            "[-p | -P | --proxiable | --noproxiable] " USAGE_BREAK "-n "
            "[-a | -A | --addresses | --noaddresses] " USAGE_BREAK
            "[--request-pac | --no-request-pac] " USAGE_BREAK
            "[-C | --canonicalize] " USAGE_BREAK
            "[-E | --enterprise] " USAGE_BREAK "[-v] [-R] "
            "[-k [-i|-t keytab_file]] "
            "[-c cachename] " USAGE_BREAK
            "[-S service_name] [-T ticket_armor_cache]" USAGE_BREAK
...
static int add_preauth_opt(struct k_opts* opts, char* av)
{
    char *sep, *v;
...
static char* parse_options(int argc, char** argv, struct k_opts* opts)
{
...
static int k5_begin(struct k_opts* opts, struct k5_data* k5)
{
    ...
    else if (opts->anonymous)
    {
        ...
        ret = krb5_build_principal_ext(
            k5->ctx, &k5->me, strlen(defrealm), defrealm,
...
static int k5_kinit(struct k_opts* opts, struct k5_data* k5)
{
    ...
    if (opts->action == INIT_KT && opts->keytab_name != NULL)
    {
        ...
        if (ret)
        {
            com_err(progname, ret, _("resolving keytab %s"), opts->keytab_name);
            goto cleanup;
        }
        if (opts->verbose)
    ...
    {
        ret = krb5_get_init_creds_opt_set_pa(
            k5->ctx, options, opts->pa_opts[i].attr, opts->pa_opts[i].value);
        if (ret)
        {
    ...
    {
        ret =
            krb5_get_init_creds_opt_set_in_ccache(k5->ctx, options, k5->in_cc);
        if (ret)
            goto cleanup;
    }
    ...
    switch (opts->action)
    {
        case INIT_PW:
            ret = krb5_get_init_creds_password(
                k5->ctx, &my_creds, k5->me, 0, kinit_prompter, &pwprompt,
        ...
        case INIT_KT:
            ...
            break;
        case VALIDATE:
            ret = krb5_get_validated_creds(k5->ctx, &my_creds, k5->me,
                                           k5->out_cc, opts->service_name);
            break;
        case RENEW:
    ...
    {
        ret = krb5_cc_initialize(k5->ctx, k5->out_cc,
