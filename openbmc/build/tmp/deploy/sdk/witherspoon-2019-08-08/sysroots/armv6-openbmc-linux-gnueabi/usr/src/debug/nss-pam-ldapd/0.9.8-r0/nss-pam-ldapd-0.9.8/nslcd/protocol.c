...
static int mkfilter_protocol_byname(const char* name, char* buffer,
                                    size_t buflen)
{
    ...
    {
        log_log(LOG_ERR,
                "mkfilter_protocol_byname(): safename buffer too small");
        return -1;
    }
    /* build filter */
    return mysnprintf(buffer, buflen, "(&%s(%s=%s))", protocol_filter,
                      attmap_protocol_cn, safename);
}

/* create a search filter for searching a protocol entry
   by uid, return -1 on errors */
static int mkfilter_protocol_bynumber(int protocol, char* buffer, size_t buflen)
{
    return mysnprintf(buffer, buflen, "(&%s(%s=%d))", protocol_filter,
...
NSLCD_HANDLE(protocol, bynumber, NSLCD_ACTION_PROTOCOL_BYNUMBER, int protocol;
             char filter[BUFLEN_FILTER]; READ_INT32(fp, protocol);
...
NSLCD_HANDLE(protocol, all, NSLCD_ACTION_PROTOCOL_ALL, const char* filter;
             log_setrequest("protocol(all)");
             , (filter = protocol_filter, 0), write_protocol(fp, entry, NULL))
