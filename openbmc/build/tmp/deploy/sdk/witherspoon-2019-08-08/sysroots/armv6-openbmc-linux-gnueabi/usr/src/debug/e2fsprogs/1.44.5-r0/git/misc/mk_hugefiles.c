...
static char* search_sysfs_block(dev_t devno, sysfs_path_t ret_path)
{
    ...
    while ((de = readdir(dir)) != NULL)
    {
        if (!strcmp(de->d_name, ".") || !strcmp(de->d_name, "..") ||
            ...
            continue;
        snprintf(path, SYSFS_PATH_LEN, "/sys/block/%s/dev", de->d_name);
        f = fopen(path, "r");
        if (f && (fscanf(f, "%u:%u", &major, &minor) == 2))
        {
            ...
            {
                snprintf(ret_path, SYSFS_PATH_LEN, "/sys/block/%s", de->d_name);
                goto success;
            }
#ifdef major
        ...
        while ((p_de = readdir(p_dir)) != NULL)
        {
            if (!strcmp(p_de->d_name, ".") || !strcmp(p_de->d_name, "..") ||
                (strlen(p_de->d_name) > SYSFS_PATH_LEN - strlen(path) - 32))
                continue;
            snprintf(p_path, SYSFS_PATH_LEN, "%s/%s/dev", path, p_de->d_name);
            ...
            {
                fclose(f);
                snprintf(ret_path, SYSFS_PATH_LEN, "%s/%s", path, p_de->d_name);
                goto success;
            }
            if (f)
                fclose(f);
...

{
    ...
    while (1)
    {
        ...
        if (*cp)
        {
            retval = ext2fs_new_inode(fs, ino, LINUX_S_IFDIR, NULL, &newdir);
            if (retval)
                goto errout;
...
static errcode_t mk_hugefile(ext2_filsys fs, blk64_t num, ext2_ino_t dir,
                             unsigned long idx, ext2_ino_t* ino)

{
    ...
    while (left)
    {
        ...
        retval = ext2fs_find_first_zero_block_bitmap2(
            fs->block_map, goal, ext2fs_blocks_count(fs->super) - 1, &end);
        if (retval)
            goto errout;
        goal = end;

        retval = ext2fs_find_first_set_block_bitmap2(
            fs->block_map, goal, ext2fs_blocks_count(fs->super) - 1, &bend);
        if (retval == ENOENT)
        {
        ...
        {
            blk64_t ret_blk;
            retval = ext2fs_zero_blocks2(fs, pblk, n, &ret_blk, NULL);
    ...
    retval =
        ext2fs_iblk_add_blocks(fs, &inode, count / EXT2FS_CLUSTER_RATIO(fs));
    if (retval)
        goto errout;
...
static blk64_t calc_overhead(ext2_filsys fs, blk64_t num)
{
     ...
     */
    extents_per_block = ((fs->blocksize - sizeof(struct ext3_extent_header)) /
...
static blk64_t get_start_block(ext2_filsys fs, blk64_t slack)
{
    ...
    while (slack)
    {
        retval = ext2fs_find_first_zero_block_bitmap2(fs->block_map, blk,
                                                      last_blk, &blk);
        if (retval)
            break;

        retval = ext2fs_find_first_set_block_bitmap2(fs->block_map, blk,
                                                     last_blk, &next);
        if (retval)
            next = last_blk;
...
errcode_t mk_hugefiles(ext2_filsys fs, const char* device_name)
{
    ...
    {
        part_offset = get_partition_start(device_name) / (fs->blocksize / 512);
        if (part_offset % EXT2FS_CLUSTER_RATIO(fs))
        {
            fprintf(stderr,
                    _("Partition offset of %llu (%uk) blocks "
                      "not compatible with cluster size %u.\n"),
                    part_offset, fs->blocksize, EXT2_CLUSTER_SIZE(fs->super));
            exit(1);
        }
    }
    num_blocks = round_up_align(num_blocks, align, 0);
    zero_hugefile =
        get_bool_from_profile(fs_types, "zero_hugefiles", zero_hugefile);
    ...
    fn_prefix = get_string_from_profile(fs_types, "hugefiles_name", "hugefile");
    ...
    if ((num_blocks ? num_blocks : fs_blocks) > (0x80000000UL / fs->blocksize))
    ...
    for (i = 0; i < num_files; i++)
    {
        ...
        if (retval)
        {
            com_err(program_name, retval, _("while creating huge file %lu"), i);
            goto errout;
        }
    }
    if (!quiet)
