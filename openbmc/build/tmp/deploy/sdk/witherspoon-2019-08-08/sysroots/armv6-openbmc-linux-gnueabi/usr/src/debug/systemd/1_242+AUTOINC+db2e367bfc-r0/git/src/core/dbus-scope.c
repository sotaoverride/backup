...
int bus_scope_method_abandon(sd_bus_message* message, void* userdata,
                             sd_bus_error* error)
{
    Scope* s = userdata;
    int r;
...
const sd_bus_vtable bus_scope_vtable[] = {
    ...
    SD_BUS_PROPERTY("Controller", "s", NULL, offsetof(Scope, controller),
                    SD_BUS_VTABLE_PROPERTY_EMITS_CHANGE),
    SD_BUS_PROPERTY("TimeoutStopUSec", "t", bus_property_get_usec,
                    offsetof(Scope, timeout_stop_usec),
                    SD_BUS_VTABLE_PROPERTY_CONST),
    SD_BUS_PROPERTY("Result", "s", property_get_result, offsetof(Scope, result),
                    SD_BUS_VTABLE_PROPERTY_EMITS_CHANGE),
    SD_BUS_SIGNAL("RequestStop", NULL, 0),
    SD_BUS_METHOD("Abandon", NULL, NULL, bus_scope_method_abandon,
                  SD_BUS_VTABLE_UNPRIVILEGED),
    SD_BUS_VTABLE_END};

static int bus_scope_set_transient_property(Scope* s, const char* name,
                                            ...
                                            sd_bus_error* error)
{
    ...
    if (streq(name, "TimeoutStopUSec"))
        return bus_set_transient_usec(UNIT(s), name, &s->timeout_stop_usec,
                                      message, flags, error);
    ...
    else if (streq(name, "Controller"))
    {
        ...
        /* We can't support direct connections with this, as direct connections
         * know no service or unique name concept, but the Controller field
         * stores exactly that. */
...
int bus_scope_set_property(Unit* u, const char* name, sd_bus_message* message,
...
static int on_controller_gone(sd_bus_track* track, void* userdata)
{
    ...
    if (s->controller)
    {
        log_unit_debug(UNIT(s), "Controller %s disappeared from bus.",
                       s->controller);
...
int bus_scope_track_controller(Scope* s)
{
    ...
    r = sd_bus_track_new(UNIT(s)->manager->api_bus, &s->controller_track,
                         on_controller_gone, s);
    if (r < 0)
        return r;
