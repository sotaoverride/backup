...
static int lease_parse_u32(const uint8_t* option, size_t len, uint32_t* ret,
                           uint32_t min)
{
    assert(option);
    assert(ret);
...
static int lease_parse_u16(const uint8_t* option, size_t len, uint16_t* ret,
                           uint16_t min)
{
    assert(option);
    assert(ret);
...
static int lease_parse_in_addrs(const uint8_t* option, size_t len,
                                struct in_addr** ret, size_t* n_ret)
{
    assert(option);
    assert(ret);
    assert(n_ret);
...
static int lease_parse_routes(const uint8_t* option, size_t len,
                              ...
                              size_t* routes_size, size_t* routes_allocated)
{
    ...
    while (len >= 8)
    {
        ...
        r = in4_addr_default_prefixlen((struct in_addr*)option,
                                       &route->dst_prefixlen);
        if (r < 0)
        {
            log_debug("Failed to determine destination prefix length from "
                      "class based IP, ignoring");
            continue;
        }
...
static int lease_parse_classless_routes(const uint8_t* option, size_t len,
                                        struct sd_dhcp_route** routes,
                                        size_t* routes_size,
                             ...
                             void* userdata)
{
    ...
    switch (code)
    {
        ...
        case SD_DHCP_OPTION_SERVER_IDENTIFIER:
            ...
            if (r < 0)
                log_debug_errno(
                    r, "Failed to parse server identifier, ignoring: %m");
        ...
        case SD_DHCP_OPTION_BROADCAST:
            ...
            if (r < 0)
                log_debug_errno(
                    r, "Failed to parse broadcast address, ignoring: %m");
            else
                lease->have_broadcast = true;
            break;

        case SD_DHCP_OPTION_ROUTER:
            r = lease_parse_in_addrs(option, len, &lease->router,
                                     &lease->router_size);
            if (r < 0)
                log_debug_errno(
                    r, "Failed to parse router addresses, ignoring: %m");
            break;

        case SD_DHCP_OPTION_DOMAIN_NAME_SERVER:
            r = lease_parse_in_addrs(option, len, &lease->dns,
                                     &lease->dns_size);
            if (r < 0)
        ...
        case SD_DHCP_OPTION_NTP_SERVER:
            r = lease_parse_in_addrs(option, len, &lease->ntp,
                                     &lease->ntp_size);
            if (r < 0)
        ...
        case SD_DHCP_OPTION_STATIC_ROUTE:
            r = lease_parse_routes(option, len, &lease->static_route,
        ...
        case SD_DHCP_OPTION_DOMAIN_SEARCH_LIST:
            r = dhcp_lease_parse_search_domains(option, len,
                                                &lease->search_domains);
            if (r < 0)
                log_debug_errno(
                    r, "Failed to parse Domain Search List, ignoring: %m");
            break;
        ...
        case SD_DHCP_OPTION_CLASSLESS_STATIC_ROUTE:
            r = lease_parse_classless_routes(option, len, &lease->static_route,
                                             &lease->static_route_size,
                                             &lease->static_route_allocated);
            if (r < 0)
                log_debug_errno(
                    r, "Failed to parse classless routes, ignoring: %m");
            break;
        ...
        case SD_DHCP_OPTION_NEW_TZDB_TIMEZONE:
        {
            ...
            {
                log_debug_errno(
                    r, "Failed to parse timezone option, ignoring: %m");
                return 0;
            }
        ...
        default:
            log_debug("Ignoring option DHCP option %" PRIu8 " while parsing.",
                      code);
            break;
    }
...
int dhcp_lease_parse_search_domains(const uint8_t* option, size_t len,
                                    char*** domains)
{
    ...
    while (pos < len)
    {
        ...
        for (;;)
        {
            ...
            else if (c <= 63)
            {
                ...
                if (!GREEDY_REALLOC(name, allocated,
            ...
            else if ((c & 0xc0) == 0xc0)
            {
                ...
                /* Save current location so we don't end up re-parsing what's
                 * parsed so far. */
                if (next_chunk == 0)
                    next_chunk = pos;
...
int dhcp_lease_load(sd_dhcp_lease** ret, const char* lease_file)
{
    ...
    _cleanup_free_ char *address = NULL, *router = NULL, *netmask = NULL,
                        ...
                        *broadcast = NULL, *dns = NULL, *ntp = NULL,
                        *mtu = NULL, *routes = NULL, *domains = NULL,
                        *client_id_hex = NULL, *vendor_specific_hex = NULL,
                        *lifetime = NULL, *t1 = NULL, *t2 = NULL,
                        *options[SD_DHCP_OPTION_PRIVATE_LAST -
                                 SD_DHCP_OPTION_PRIVATE_BASE + 1] = {};
    ...
    r = parse_env_file(
        NULL, lease_file, "ADDRESS", &address, "ROUTER", &router, "NETMASK",
        &netmask, "SERVER_IDENTIFIER", &server_address, "NEXT_SERVER",
        &next_server, "BROADCAST", &broadcast, "DNS", &dns, "NTP", &ntp, "MTU",
        &mtu, "DOMAINNAME", &lease->domainname, "HOSTNAME", &lease->hostname,
        "DOMAIN_SEARCH_LIST", &domains, "ROOT_PATH", &lease->root_path,
        "ROUTES", &routes, "CLIENTID", &client_id_hex, "TIMEZONE",
        &lease->timezone, "VENDOR_SPECIFIC", &vendor_specific_hex, "LIFETIME",
        &lifetime, "T1", &t1, "T2", &t2, "OPTION_224", &options[0],
        "OPTION_225", &options[1], "OPTION_226", &options[2], "OPTION_227",
        &options[3], "OPTION_228", &options[4], "OPTION_229", &options[5],
        "OPTION_230", &options[6], "OPTION_231", &options[7], "OPTION_232",
        &options[8], "OPTION_233", &options[9], "OPTION_234", &options[10],
        "OPTION_235", &options[11], "OPTION_236", &options[12], "OPTION_237",
        &options[13], "OPTION_238", &options[14], "OPTION_239", &options[15],
        "OPTION_240", &options[16], "OPTION_241", &options[17], "OPTION_242",
        &options[18], "OPTION_243", &options[19], "OPTION_244", &options[20],
        "OPTION_245", &options[21], "OPTION_246", &options[22], "OPTION_247",
        &options[23], "OPTION_248", &options[24], "OPTION_249", &options[25],
        "OPTION_250", &options[26], "OPTION_251", &options[27], "OPTION_252",
        &options[28], "OPTION_253", &options[29], "OPTION_254", &options[30]);
    if (r < 0)
        return r;
    ...
    if (router)
    {
        ...
        if (r < 0)
            log_debug_errno(
                r, "Failed to deserialize router addresses %s, ignoring: %m",
                router);
        else
            lease->router_size = r;
    }
    ...
    if (server_address)
    {
        ...
        if (r <= 0)
            log_debug("Failed to parse server address %s, ignoring.",
                      server_address);
    }
    ...
    if (broadcast)
    {
        ...
        if (r <= 0)
            log_debug("Failed to parse broadcast address %s, ignoring.",
                      broadcast);
        else
            lease->have_broadcast = true;
    }
    ...
    if (dns)
    {
        ...
        if (r < 0)
            log_debug_errno(
                r, "Failed to deserialize DNS servers %s, ignoring: %m", dns);
        else
            lease->dns_size = r;
    }
    ...
    if (ntp)
    {
        ...
        if (r < 0)
            log_debug_errno(
                r, "Failed to deserialize NTP servers %s, ignoring: %m", ntp);
        else
            lease->ntp_size = r;
    }
    ...
    if (routes)
    {
        ...
        if (r < 0)
            log_debug_errno(r, "Failed to parse DHCP routes %s, ignoring: %m",
                            routes);
    }
    ...
    if (lifetime)
    {
        ...
        if (r < 0)
            log_debug_errno(r, "Failed to parse lifetime %s, ignoring: %m",
                            lifetime);
    }
    ...
    if (client_id_hex)
    {
        r = unhexmem(client_id_hex, (size_t)-1, &lease->client_id,
                     &lease->client_id_len);
        if (r < 0)
            log_debug_errno(r, "Failed to parse client ID %s, ignoring: %m",
                            client_id_hex);
    }

    if (vendor_specific_hex)
    {
        r = unhexmem(vendor_specific_hex, (size_t)-1, &lease->vendor_specific,
                     &lease->vendor_specific_len);
        if (r < 0)
            log_debug_errno(
                r, "Failed to parse vendor specific data %s, ignoring: %m",
                vendor_specific_hex);
    }

    for (i = 0; i <= SD_DHCP_OPTION_PRIVATE_LAST - SD_DHCP_OPTION_PRIVATE_BASE;
         i++)
    {
        ...
        {
            log_debug_errno(
                r, "Failed to parse private DHCP option %s, ignoring: %m",
                options[i]);
            continue;
        }

        r = dhcp_lease_insert_private_option(
            lease, SD_DHCP_OPTION_PRIVATE_BASE + i, data, len);
        if (r < 0)
            return r;
    }
...
int sd_dhcp_route_get_destination(sd_dhcp_route* route,
...
int sd_dhcp_route_get_destination_prefix_length(sd_dhcp_route* route,
                                                uint8_t* length)
{
    assert_return(route, -EINVAL);
