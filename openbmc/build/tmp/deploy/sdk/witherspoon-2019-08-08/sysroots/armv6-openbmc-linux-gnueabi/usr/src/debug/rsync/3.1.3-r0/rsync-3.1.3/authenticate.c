                                ...
                                const char* challenge, const char* pass)
{
    ...
    else if (lp_strict_modes(module))
    {
        if ((st.st_mode & 06) != 0)
        {
            rprintf(FLOG, "secrets file must not be other-accessible (see "
                          "strict modes option)\n");
            ok = 0;
        }
        else if (MY_UID() == 0 && st.st_uid != 0)
        {
            rprintf(FLOG, "secrets file must be owned by root when running as "
                          "root (see strict modes)\n");
            ok = 0;
        }
    }
    if (!ok)
...
static const char* getpassf(const char* filename)
{
    ...
    else
    {
        ...
        {
            rprintf(FERROR, "ERROR: password file must be owned by root when "
                            "running as root\n");
            exit_cleanup(RERR_SYNTAX);
        }
        ...
        else
        {
            ...
            if (auth_uid_groups_cnt < 0)
            {
                ...
                else
                {
                    ...
                    if ((auth_uid_groups =
                             new_array(char*, auth_uid_groups_cnt)) == NULL)
                        out_of_memory("auth_server");
            ...
            for (j = 0; j < auth_uid_groups_cnt; j++)
            {
                if (auth_uid_groups[j] &&
                    wildmatch(tok + 1, auth_uid_groups[j]))
                {
                    group_match = j;
                    break;
                }
            ...
            if (group_match >= 0)
                break;
#else
            rprintf(FLOG, "your computer doesn't support getgrouplist(), so no "
                          "@group authorization is possible.\n");
#endif
        }
    }
