...
static int fdb_entry_new_static(Network* network, const char* filename,
                                unsigned section_line, FdbEntry** ret)
{
    ...
    *fdb_entry = (FdbEntry){
    ...
    if (filename)
    {
        ...
        r = hashmap_ensure_allocated(&network->fdb_entries_by_section,
                                     &network_config_hash_ops);
        if (r < 0)
            return r;

        r = hashmap_put(network->fdb_entries_by_section, fdb_entry->section,
                        fdb_entry);
        if (r < 0)
            return r;
    }
...
int fdb_entry_configure(Link* link, FdbEntry* fdb_entry)
{
    ...
    r = sd_netlink_message_append_ether_addr(req, NDA_LLADDR,
                                             fdb_entry->mac_addr);
    if (r < 0)
        return rtnl_log_create_error(r);
...
int config_parse_fdb_hwaddr(const char* unit, const char* filename,
...
int config_parse_fdb_vlan_id(const char* unit, const char* filename,
                             unsigned line, const char* section,
                             unsigned section_line, const char* lvalue,
                             int ltype, const char* rvalue, void* data,
                             void* userdata)
{
    ...
    r = config_parse_vlanid(unit, filename, line, section, section_line, lvalue,
                            ltype, rvalue, &fdb_entry->vlan_id, userdata);
    if (r < 0)
        return r;
