...
static kadm5_ret_t init_any(krb5_context context, char* client_name,
                            enum init_type init_type, char* pass,
                            krb5_ccache ccache_in, char* service_name,
                            kadm5_config_params* params,
                            krb5_ui_4 struct_version, krb5_ui_4 api_version,
                            char** db_args, void** server_handle);

static kadm5_ret_t get_init_creds(kadm5_server_handle_t handle,
                                  krb5_principal client,
                                  enum init_type init_type, char* pass,
                                  krb5_ccache ccache_in, char* svcname_in,
                                  char* realm, krb5_principal* server_out);

static kadm5_ret_t gic_iter(kadm5_server_handle_t handle,
                            enum init_type init_type, krb5_ccache ccache,
                            krb5_principal client, char* pass, char* svcname,
                            char* realm, krb5_principal* server_out);

static kadm5_ret_t connect_to_server(const char* hostname, int port, int* fd);

static kadm5_ret_t setup_gss(kadm5_server_handle_t handle,
                             kadm5_config_params* params_in,
                             krb5_principal client, krb5_principal server);

static void rpc_auth(kadm5_server_handle_t handle,
                     kadm5_config_params* params_in,
                     gss_cred_id_t gss_client_creds, gss_name_t gss_target);

kadm5_ret_t kadm5_init_with_creds(krb5_context context, char* client_name,
...
kadm5_ret_t kadm5_init_with_skey(krb5_context context, char* client_name,
                                 ...
                                 void** server_handle)
{
    return init_any(context, client_name, INIT_SKEY, keytab, NULL, service_name,
                    params, struct_version, api_version, db_args,
                    server_handle);
}
                            ...
                            char** db_args, void** server_handle)
{
    ...
    *handle->lhandle = *handle;
    ...
    if ((code = kadm5_get_config_params(handle->context, 0, params_in,
                                        &handle->params)))
    {
        free(handle);
        return (code);
...
#define REQUIRED_PARAMS                                                        \
    (KADM5_CONFIG_REALM | KADM5_CONFIG_ADMIN_SERVER | KADM5_CONFIG_KADMIND_PORT)
...
error:
    ...
    {
        if (krb5_cc_resolve(handle->context, handle->cache_name, &ccache) == 0)
    ...
    *server_out = NULL;
    ...
    if (svcname_in == NULL)
    {
        code = kadm5_get_admin_service_name(
            handle->context, handle->params.realm, svcname, sizeof(svcname));
        if (code)
            goto error;
    }
    else
    ...
    else
    {
        ...
        code = krb5_cc_resolve(handle->context, handle->cache_name, &ccache);
        if (code)
            goto error;
    ...
    code = gic_iter(handle, init_type, ccache, client, pass, svcname, realm,
                    server_out);
    if ((code == KRB5KDC_ERR_S_PRINCIPAL_UNKNOWN || code == KRB5_CC_NOTFOUND) &&
        svcname_in == NULL)
    {
                            ...
                            char* realm, krb5_principal* server_out)
{
    ...
    *server_out = NULL;
    ...
    else if (init_type == INIT_SKEY)
    {
        ...
        code = krb5_get_init_creds_keytab(ctx, &outcreds, client, kt, 0,
                                          svcname, opt);
        if (pass)
    ...
    else if (init_type == INIT_CREDS)
    {
        ...
        code = krb5_parse_name_flags(
            ctx, svcname, KRB5_PRINCIPAL_PARSE_IGNORE_REALM, &mcreds.server);
        if (code)
            goto error;
        code = krb5_set_principal_realm(ctx, mcreds.server, realm);
        if (code)
            goto error;
        code = krb5_cc_retrieve_cred(ctx, ccache, 0, &mcreds, &outcreds);
                             ...
                             krb5_principal client, krb5_principal server)
{
    ...
    gssstat =
        gss_krb5_ccache_name(&minor_stat, handle->cache_name, &c_ccname_orig);
    ...
    gssstat = gss_import_name(&minor_stat, &buf, (gss_OID)gss_nt_krb5_principal,
                              &gss_target);
    if (gssstat != GSS_S_COMPLETE)
        goto error;
    ...
    gssstat = gss_acquire_cred(&minor_stat, gss_client, 0, GSS_C_NULL_OID_SET,
                               GSS_C_INITIATE, &handle->cred, NULL, NULL);
    if (gssstat != GSS_S_COMPLETE)
        goto error;
                     ...
                     gss_cred_id_t gss_client_creds, gss_name_t gss_target)
{
    ...
    if (params_in == NULL || !(params_in->mask & KADM5_CONFIG_OLD_AUTH_GSSAPI))
    {
        ...
        handle->clnt->cl_auth = authgss_create(handle->clnt, gss_target, &sec);
        if (handle->clnt->cl_auth != NULL)
            return;
    }
    ...
    handle->clnt->cl_auth = auth_gssapi_create(
        handle->clnt, &gssstat, &minor_stat, gss_client_creds, gss_target,
        (gss_OID)gss_mech_krb5, GSS_C_MUTUAL_FLAG | GSS_C_REPLAY_FLAG, 0, NULL,
        NULL, NULL);
}
 ...
 */
krb5_error_code kadm5_init_iprop(void* handle, char** db_args)
{
    return (0);
}
