...
finalize_it:
    if (iRet != RS_RET_OK)
    {
        LogError(
            errno, iRet,
            "a lookup table could not be initialized: "
...
{
    return ustrcmp(((lookup_string_tab_entry_t*)s1)->key,
                   ((lookup_string_tab_entry_t*)s2)->key);
}
...
static es_str_t* lookupKey_stub(lookup_t* pThis,
                                lookup_key_t __attribute__((unused)) key)
{
...
static void* bsearch_lte(const void* key, const void* base, size_t nmemb,
                         size_t size, comp_fn_t* comp_fn)
{
    size_t l, u, idx;
    const void* p;
    int comparison;
...
static es_str_t* lookupKey_sprsArr(lookup_t* pThis, lookup_key_t key)
{
    ...
    {
        entry = bsearch_lte(
            &key.k_uint, pThis->table.sprsArr->entries, pThis->nmemb,
...
static rsRetVal build_StringTable(lookup_t* pThis, struct json_object* jtab,
                                  const uchar* name)
{
    uint32_t i;
    ...
    if (pThis->nmemb > 0)
    {
        CHKmalloc(pThis->table.str->entries =
                      calloc(pThis->nmemb, sizeof(lookup_string_tab_entry_t)));
        ...
        for (i = 0; i < pThis->nmemb; i++)
        {
            ...
            CHKmalloc(pThis->table.str->entries[i].key =
                          ustrdup((uchar*)json_object_get_string(jindex)));
            value = (uchar*)json_object_get_string(jvalue);
            uchar** found = (uchar**)bsearch(value, pThis->interned_vals,
                                             pThis->interned_val_count,
                                             sizeof(uchar*), bs_arrcmp_str);
            if (found == NULL)
            {
...
// I give up, I see no way to remove false positive -- rgerhards, 2017-10-24
        ...
        qsort(pThis->table.str->entries, pThis->nmemb,
              sizeof(lookup_string_tab_entry_t), qs_arrcmp_strtab);
    }
...
static rsRetVal build_ArrayTable(lookup_t* pThis, struct json_object* jtab,
                                 const uchar* name)
{
    uint32_t i;
    struct json_object *jrow, *jindex, *jvalue;
    ...
    if (pThis->nmemb > 0)
    {
        ...
        CHKmalloc(pThis->table.arr->interned_val_refs =
                      calloc(pThis->nmemb, sizeof(uchar*)));
        ...
        qsort(indexes, pThis->nmemb, sizeof(uint32_index_val_t),
              qs_arrcmp_uint32_index_val);
        for (i = 0; i < pThis->nmemb; i++)
        {
            ...
            else
            {
                if (_index != ++prev_index)
                {
                    LogError(0, RS_RET_INVALID_VALUE,
                             ...
                             "has non-contiguous members between index '%d' "
                             "and '%d'",
            ...
            uchar* const* const canonicalValueRef_ptr = bsearch(
                indexes[i].val, pThis->interned_vals, pThis->interned_val_count,
                sizeof(uchar*), bs_arrcmp_str);
...
static rsRetVal build_SparseArrayTable(lookup_t* pThis,
                                       ...
                                       const uchar* name)
{
    ...
    pThis->table.str = NULL;
    CHKmalloc(pThis->table.sprsArr =
                  calloc(1, sizeof(lookup_sparseArray_tab_t)));
    if (pThis->nmemb > 0)
    {
        CHKmalloc(pThis->table.sprsArr->entries = calloc(
                      pThis->nmemb, sizeof(lookup_sparseArray_tab_entry_t)));
        ...
        for (i = 0; i < pThis->nmemb; i++)
        {
            ...
            pThis->table.sprsArr->entries[i].key =
                (uint32_t)json_object_get_int(jindex);
            value = (uchar*)json_object_get_string(jvalue);
            uchar* const* const canonicalValueRef_ptr =
                bsearch(value, pThis->interned_vals, pThis->interned_val_count,
                        sizeof(uchar*), bs_arrcmp_str);
            if (canonicalValueRef_ptr == NULL)
            {
                LogError(0, RS_RET_ERR,
            ...
            canonicalValueRef = *canonicalValueRef_ptr;
            assert(canonicalValueRef != NULL);
            pThis->table.sprsArr->entries[i].interned_val_ref =
                canonicalValueRef;
        }
        qsort(pThis->table.sprsArr->entries, pThis->nmemb,
              sizeof(lookup_sparseArray_tab_entry_t), qs_arrcmp_sprsArrtab);
    }
...
static rsRetVal lookupBuildTable_v1(lookup_t* pThis, struct json_object* jroot,
                                    const uchar* name)
{
    ...
    {
        LogError(0, RS_RET_INVALID_VALUE,
                 "lookup table named: '%s' has invalid table definition", name);
        ABORT_FINALIZE(RS_RET_INVALID_VALUE);
    }
    ...
    /* before actual table can be loaded, prepare all-value list and remove
     * duplicates*/
    for (i = 0; i < pThis->nmemb; i++)
    {
        jrow = json_object_array_get_idx(jtab, i);
...
static rsRetVal lookupBuildTable(lookup_t* pThis, struct json_object* jroot,
                                 const uchar* name)
{
...
{
    lookup_t *newlu, *oldlu; /* dummy to be able to use support functions
                                without affecting current settings. */
    DEFiRet;
    ...
    else
    {
        if (stub_val == NULL)
        {
            LogMsg(0, RS_RET_OK, LOG_INFO,
                   "lookup table '%s' reloaded from file '%s'", pThis->name,
                   pThis->filename);
        }
        else
        {
...
static rsRetVal lookupDoStub(lookup_ref_t* pThis, const uchar* stub_val)
{
    ...
    else
    {
        LogError(0, RS_RET_OK,
                 "lookup table '%s' is already stubbed with value '%s'",
                 pThis->name, stub_val);
    }
finalize_it:
    RETiRet;
...
rsRetVal ATTR_NONNULL(1)
    lookupReload(lookup_ref_t* const pThis,
                 const uchar* const stub_val_if_reload_fails)
{
    ...
    {
        locked = 1;
        /*so it doesn't leak memory in situation where 2 reload requests are
         * issued back to back*/
        freeStubValueForReloadFailure(pThis);
        if (stub_val_if_reload_fails != NULL)
        {
            CHKmalloc(pThis->stub_value_for_reload_failure =
                          ustrdup(stub_val_if_reload_fails));
        }
        pThis->do_reload = 1;
        pthread_cond_signal(&pThis->run_reloader);
    }
...
static rsRetVal ATTR_NONNULL()
    lookupReadFile(lookup_t* const pThis, const uchar* const name,
                   const uchar* const filename)
{
    ...
    {
        LogError(errno, RS_RET_READ_ERR, "lookup table file '%s' read error",
                 filename);
    ...
    if (json == NULL)
    {
        LogError(0, RS_RET_JSON_PARSE_ERR,
                 "lookup table file '%s' json parsing error", filename);
