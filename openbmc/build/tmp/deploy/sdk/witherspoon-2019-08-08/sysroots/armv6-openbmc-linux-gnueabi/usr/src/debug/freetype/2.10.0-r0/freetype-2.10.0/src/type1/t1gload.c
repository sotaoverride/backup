                                                   ...
                                                   FT_Bool* force_scaling)
{
    ...
    if (inc)
        error =
            inc->funcs->get_glyph_data(inc->object, glyph_index, char_string);
    else
    ...
    if (!error)
    {
        ...
        if (decoder->builder.metrics_only)
            error = decoder_funcs->parse_metrics(decoder,
        ...
        else
        {
            ...
            error = decoder_funcs->parse_charstrings(
                &psdecoder, (FT_Byte*)char_string->pointer,
...
T1_Parse_Glyph(T1_Decoder decoder, FT_UInt glyph_index)
{
    ...
    FT_Error error = T1_Parse_Glyph_And_Get_Char_String(
        decoder, glyph_index, &glyph_data, &force_scaling);
...
T1_Compute_Max_Advance(T1_Face face, FT_Pos* max_advance)
{
    ...
    *max_advance = 0;
    ...
    error = psaux->t1_decoder_funcs->init(
        &decoder, (FT_Face)face, 0, /* size       */
        0,                          /* glyph slot */
        (FT_Byte**)type1->glyph_names, face->blend, 0, FT_RENDER_MODE_NORMAL,
        T1_Parse_Glyph);
    if (error)
        return error;
...
T1_Get_Advances(FT_Face t1face, /* T1_Face */
                FT_UInt first, FT_UInt count, FT_Int32 load_flags,
                FT_Fixed* advances)
{
              ...
              FT_UInt glyph_index, FT_Int32 load_flags)
{
    ...
    error = decoder_funcs->init(
        &decoder, t1glyph->face, t1size, t1glyph, (FT_Byte**)type1->glyph_names,
        face->blend, hinting, FT_LOAD_TARGET_MODE(load_flags), T1_Parse_Glyph);
    if (error)
        goto Exit;
    ...
    if (!error)
    {
        ...
        else
        {
            ...
            if (load_flags & FT_LOAD_VERTICAL_LAYOUT)
            {
                ...
                metrics->vertAdvance =
                    (face->type1.font_bbox.yMax - face->type1.font_bbox.yMin) >>
                    16;
                t1glyph->linearVertAdvance = metrics->vertAdvance;
            }
            else
            {
                metrics->vertAdvance = FIXED_TO_INT(decoder.builder.advance.y);
            ...
            {
                FT_Outline_Translate(&t1glyph->outline, font_offset.x,
                                     font_offset.y);
