                      ...
                      const struct blkid_idmag* mag __attribute__((__unused__)))
{
    ...
    for (i = 0; i < ARRAY_SIZE(blocks); i++)
    {
        ...
        sb = (struct sysv_super_block*)blkid_probe_get_buffer(
            pr, off, sizeof(struct sysv_super_block));
        if (!sb)
            return errno ? -errno : 1;
            ...
            sb->s_magic == cpu_to_be32(0xfd187e20))
        {
            ...
            if (blkid_probe_set_label(pr, sb->s_fname, sizeof(sb->s_fname)))
                return 1;

            if (blkid_probe_set_magic(
                    pr, off + offsetof(struct sysv_super_block, s_magic),
                    sizeof(sb->s_magic), (unsigned char*)&sb->s_magic))
                return 1;
...
const struct blkid_idinfo xenix_idinfo = {
    ...
    .magics = {{.magic = "\x2b\x55\x44", .len = 3, .kboff = 1, .sboff = 0x400},
