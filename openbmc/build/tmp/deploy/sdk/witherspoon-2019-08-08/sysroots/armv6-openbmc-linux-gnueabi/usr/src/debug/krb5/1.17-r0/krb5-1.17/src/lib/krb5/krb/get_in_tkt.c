...
static krb5_error_code make_preauth_list(krb5_context, krb5_preauthtype*, int,
                                         krb5_pa_data***);
 ...
 */
static krb5_error_code decrypt_as_reply(krb5_context context,
                                        ...
                                        krb5_keyblock* key_out)
{
    ...
    if (ctx->as_key.length)
    {
        ...
        ret =
            krb5int_fast_reply_key(context, strengthen_key, &ctx->as_key, &key);
        if (ret)
            return ret;
 ...
 */
static krb5_error_code verify_anonymous(krb5_context context,
                                        ...
                                        krb5_keyblock* as_key)
{
    ...
    if (ret)
        goto cleanup;
    ret = krb5_c_fx_cf2_simple(context, kdc_key, "PKINIT", as_key,
                               "KEYEXCHANGE", &expected);
    if (ret)
        goto cleanup;
...
static krb5_error_code verify_as_reply(krb5_context context,
                                       ...
                                       krb5_kdc_rep* as_reply)
{
    ...
    if (canon_req)
    {
        ...
        {
            canon_ok = krb5_principal_compare_any_realm(
                context, as_reply->client, krb5_anonymous_principal());
        }
    }
    else
        canon_ok = 0;
...
static krb5_error_code stash_as_reply(krb5_context context,
                                      krb5_kdc_rep* as_reply, krb5_creds* creds,
                                      krb5_ccache ccache)
{
    krb5_error_code retval;
    krb5_data* packet;
    ...
    if ((retval = krb5_copy_keyblock_contents(
             context, as_reply->enc_part2->session, &creds->keyblock)))
        goto cleanup;
...
static krb5_error_code make_preauth_list(krb5_context context,
                                         krb5_preauthtype* ptypes, int nptypes,
                                         krb5_pa_data*** ret_list)
{
    ...
    if ((preauthp = (krb5_pa_data**)malloc((nptypes + 1) *
                                           sizeof(krb5_pa_data*))) == NULL)
        return (ENOMEM);

    for (i = 0; i < nptypes; i++)
    {
        if ((preauthp[i] = (krb5_pa_data*)malloc(sizeof(krb5_pa_data))) == NULL)
        {
            for (; i >= 0; i--)
                free(preauthp[i]);
            free(preauthp);
...
static krb5_error_code add_padata(krb5_pa_data*** padptr,
                                                 ...
                                                 krb5_pa_data** padata)
{
    ...
    ret = krb5int_libdefault_string(
        context, realm, KRB5_CONF_PREFERRED_PREAUTH_TYPES, &preauth_types);
...
static krb5_error_code build_in_tkt_name(krb5_context context,
    ...
    *server_out = NULL;
    ...
    if (in_tkt_service)
    {
        ret = krb5_parse_name_flags(context, in_tkt_service,
                                    KRB5_PRINCIPAL_PARSE_IGNORE_REALM, &server);
        if (ret)
            return ret;
        krb5_free_data_contents(context, &server->realm);
        ret =
            krb5int_copy_data_contents(context, &client->realm, &server->realm);
        if (ret)
        {
            krb5_free_principal(context, server);
            return ret;
    ...
    {
        ret = krb5_build_principal_ext(context, &server, client->realm.length,
...
krb5_error_code k5_init_creds_get(krb5_context context,
                                  krb5_init_creds_context ctx, int* use_master)
{
    ...
    for (;;)
    {
        code = krb5_init_creds_step(context, ctx, &reply, &request, &realm,
                                    &flags);
        ...
        master = *use_master;
        code = krb5_sendto_kdc(context, &request, &realm, &reply, &master,
                               tcp_only);
        if (code != 0)
            break;
...
krb5_error_code KRB5_CALLCONV krb5_init_creds_get_times(
    krb5_context context, krb5_init_creds_context ctx, krb5_ticket_times* times)
{
    if (!ctx->complete)
        return KRB5_NO_TKT_SUPPLIED;
    ...
    krb5_context context, krb5_init_creds_context ctx, krb5_error** error)
{
    ...
    *error = NULL;
    ...
    {
        code =
            krb5_copy_principal(context, ctx->err_reply->client, &ret->client);
        if (code != 0)
            goto cleanup;
    }
    ...
    code =
        krb5int_copy_data_contents(context, &ctx->err_reply->text, &ret->text);
    if (code != 0)
        goto cleanup;
...
krb5_error_code k5_init_creds_current_time(krb5_context context,
                                           krb5_init_creds_context ctx,
                                           krb5_boolean allow_unauth,
...
static krb5_error_code set_request_times(krb5_context context,
                                         krb5_init_creds_context ctx)
{
    krb5_error_code code;
...
static void read_allowed_preauth_type(krb5_context context,
                                      krb5_init_creds_context ctx)
{
    krb5_error_code ret;
    krb5_data config;
...
static krb5_boolean encts_disabled(profile_t profile, const krb5_data* realm)
{
    krb5_error_code ret;
    char* realmstr;
    int bval;
 ...
 */
static krb5_error_code restart_init_creds_loop(krb5_context context,
                                               krb5_init_creds_context ctx,
                                               krb5_boolean fast_upgrade)
{
    ...
    code =
        krb5int_fast_as_armor(context, ctx->fast_state, ctx->opt, ctx->request);
    if (code != 0)
        goto cleanup;
    /* give the preauth plugins a chance to prep the request body */
    k5_preauth_prepare_request(context, ctx->opt, ctx->request);

    code = krb5int_fast_prep_req_body(context, ctx->fast_state, ctx->request,
...
krb5_error_code KRB5_CALLCONV krb5_init_creds_init(
    ...
    void* data, krb5_deltat start_time, krb5_get_init_creds_opt* opt,
    ...
    if (ctx->opt->flags & KRB5_GET_INIT_CREDS_OPT_ETYPE_LIST)
    {
        ctx->request->ktype =
            k5memdup(ctx->opt->etype_list,
                     ctx->opt->etype_list_length * sizeof(krb5_enctype), &code);
        if (code != 0)
            goto cleanup;
        ctx->request->nktypes = ctx->opt->etype_list_length;
    }
    else if (krb5_get_default_in_tkt_ktypes(context, &ctx->request->ktype) == 0)
    {
    ...
    if (ctx->opt->flags & KRB5_GET_INIT_CREDS_OPT_ANONYMOUS)
    {
        ...
        if (client->length == 1 && client->data[0].length == 0)
        {
            ...
            code = krb5_build_principal_ext(
                context, &new_client, client->realm.length, client->realm.data,
...
static krb5_error_code init_creds_validate_reply(krb5_context context,
                                                 krb5_init_creds_context ctx,
                                                 krb5_data* reply)
{
    krb5_error_code code;
...
static krb5_error_code save_selected_preauth_type(krb5_context context,
                                                  krb5_ccache ccache,
                                                  krb5_init_creds_context ctx)
{
...
static krb5_error_code clear_cc_config_out_data(krb5_context context,
                                                krb5_init_creds_context ctx)
{
    k5_json_release(ctx->cc_config_out);
...
static krb5_error_code read_cc_config_in_data(krb5_context context,
                                              krb5_init_creds_context ctx)
{
    k5_json_value val;
    krb5_data config;
...
static krb5_error_code save_cc_config_out_data(krb5_context context,
                                               krb5_ccache ccache,
                                               krb5_init_creds_context ctx)
{
    krb5_data config;
...
static krb5_error_code maybe_add_pac_request(krb5_context context,
                                             krb5_init_creds_context ctx)
{
    krb5_error_code code;
...
static krb5_error_code init_creds_step_request(krb5_context context,
                                               krb5_init_creds_context ctx,
                                               krb5_data* out)
{
    krb5_error_code code;
    ...
    {
        code = add_padata(&ctx->request->padata, KRB5_PADATA_AS_FRESHNESS, NULL,
                          0);
        if (code)
            goto cleanup;
    ...
    code = krb5int_fast_prep_req(context, ctx->fast_state, ctx->request,
                                 ctx->outer_request_body, encode_krb5_as_req,
                                 &ctx->encoded_previous_request);
    if (code != 0)
        goto cleanup;

    code =
        krb5int_copy_data_contents(context, ctx->encoded_previous_request, out);
    if (code != 0)
        goto cleanup;
...
static krb5_error_code check_reply_enctype(krb5_init_creds_context ctx)
{
    int i;
...
static void note_req_timestamp(krb5_context context,
                               krb5_init_creds_context ctx,
                               krb5_timestamp kdc_time, krb5_int32 kdc_usec)
{
    ...
    ctx->pa_offset_state =
        (ctx->fast_state->armor_key != NULL) ? AUTH_OFFSET : UNAUTH_OFFSET;
}
 ...
 */
static krb5_boolean is_referral(krb5_context context, krb5_error* err,
                                krb5_principal client)
{
 ...
 * configuration. */
static krb5_error_code accept_method_data(krb5_context context,
                                          krb5_init_creds_context ctx)
{
    krb5_free_pa_data(context, ctx->method_padata);
...
static krb5_error_code init_creds_step_reply(krb5_context context,
                                             krb5_init_creds_context ctx,
                                             krb5_data* in)
{
    krb5_error_code code;
    ...
    if (ctx->err_reply != NULL)
    {
        ...
        else if (reply_code == KDC_ERR_MORE_PREAUTH_DATA_REQUIRED && retry)
        {
            ...
            ctx->err_padata = NULL;
                 ...
                 is_referral(context, ctx->err_reply, ctx->request->client))
        {
    ...
    code = krb5int_fast_process_response(context, ctx->fast_state, ctx->reply,
                                         &strengthen_key);
    if (code != 0)
        goto cleanup;
    ...
    code = krb5int_fast_verify_nego(context, ctx->fast_state, ctx->reply,
                                    ctx->encoded_previous_request,
                                    &encrypting_key, &fast_avail);
    if (code)
        goto cleanup;
    code =
        verify_as_reply(context, ctx->request_time, ctx->request, ctx->reply);
    if (code != 0)
        goto cleanup;
    code = verify_anonymous(context, ctx->request, ctx->reply, &ctx->as_key);
    if (code)
        goto cleanup;
    ...
    krb5_data* out, krb5_data* realm, unsigned int* flags)
{
    ...
    *flags = 0;
    ...
    if (in->length != 0)
    {
        ...
        {
            code2 = krb5int_copy_data_contents(
                context, ctx->encoded_previous_request, out);
            if (code2 != 0)
            {
                code = code2;
                goto cleanup;
...
copy_realm:
    ...
    code2 = krb5int_copy_data_contents(context, &ctx->request->server->realm,
                                       realm);
    if (code2 != 0)
    {
        code = code2;
...
krb5_error_code KRB5_CALLCONV k5_get_init_creds(
    krb5_context context, krb5_creds* creds, krb5_principal client,
    krb5_prompter_fct prompter, void* prompter_data, krb5_deltat start_time,
    ...
    krb5_kdc_rep** as_reply)
{
    ...
    code = krb5_init_creds_init(context, client, prompter, prompter_data,
