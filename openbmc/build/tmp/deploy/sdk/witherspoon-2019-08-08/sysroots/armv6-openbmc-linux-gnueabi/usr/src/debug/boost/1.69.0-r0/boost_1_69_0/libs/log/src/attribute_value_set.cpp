...
BOOST_FORCEINLINE attribute_value_set::node::node(key_type const& key,
                                                  mapped_type& data,
                                                  bool dynamic) :
    node_base(),
    m_Value(key, mapped_type()), m_DynamicallyAllocated(dynamic)
{
...
struct attribute_value_set::implementation
{
  ...
  private:
    ...
    typedef intrusive::derivation_value_traits<node, node_traits,
                                               intrusive::normal_link>
        value_traits;

    //! A container that provides iteration through elements of the container
    typedef intrusive::list<node, intrusive::value_traits<value_traits>,
    ...
    struct bucket
    {
        ...
        node* first;
        //! Points to the last element in the bucket (not the one after the
        //! last!)
        node* last;
  ...
  private:
    //! Constructor
    implementation(node* storage, node* eos,
        ...
        m_pThreadAttributes(thread_attrs), m_pGlobalAttributes(global_attrs),
        m_pEnd(storage), m_pEOS(eos)
    {
    }
    ...
    static implementation* create(size_type element_count,
                                  attribute_set_impl_type* source_attrs,
                                  attribute_set_impl_type* thread_attrs,
                                  attribute_set_impl_type* global_attrs)
    {
        ...
        const size_type buffer_size =
            header_size + element_count * sizeof(node);

        implementation* p = reinterpret_cast<implementation*>(
            stateless_allocator().allocate(buffer_size));
        node* const storage =
            reinterpret_cast<node*>(reinterpret_cast<char*>(p) + header_size);
        new (p) implementation(storage, storage + element_count, source_attrs,
                               thread_attrs, global_attrs);
  ...
  public:
    //! The function allocates memory and creates the object
    static implementation* create(attribute_set const& source_attrs,
                                  attribute_set const& thread_attrs,
                                  attribute_set const& global_attrs,
                                  size_type reserve_count)
    {
        return create(
            source_attrs.m_pImpl->size() + thread_attrs.m_pImpl->size() +
                global_attrs.m_pImpl->size() + reserve_count,
            source_attrs.m_pImpl, thread_attrs.m_pImpl, global_attrs.m_pImpl);
    }

    //! The function allocates memory and creates the object
    static implementation* create(attribute_value_set const& source_attrs,
                                  attribute_set const& thread_attrs,
                                  attribute_set const& global_attrs,
                                  size_type reserve_count)
    {
        implementation* p =
            create(source_attrs.m_pImpl->size() + thread_attrs.m_pImpl->size() +
                       global_attrs.m_pImpl->size() + reserve_count,
                   NULL, thread_attrs.m_pImpl, global_attrs.m_pImpl);
        p->copy_nodes_from(source_attrs.m_pImpl);
        return p;
    }

    //! The function allocates memory and creates the object
    static implementation* create(BOOST_RV_REF(attribute_value_set)
                                      source_attrs,
                                  attribute_set const& thread_attrs,
                                  attribute_set const& global_attrs,
                                  size_type reserve_count)
    ...
    {
        const size_type buffer_size =
            reinterpret_cast<char*>(p->m_pEOS) - reinterpret_cast<char*>(p);
        p->~implementation();
        stateless_allocator().deallocate(
            reinterpret_cast<stateless_allocator::pointer>(p), buffer_size);
    }
  ...
  private:
    ...
    {
        // Copy all elements
        node_list::iterator it = from->m_Nodes.begin(),
                            end = from->m_Nodes.end();
        for (; it != end; ++it)
        {
            node* n = m_pEnd++;
            ...
            // Since nodes within buckets are ordered, we can simply append the
            // node to the end of the bucket
...
BOOST_LOG_API
    attribute_value_set::attribute_value_set(size_type reserve_count) :
...
BOOST_LOG_API attribute_value_set::attribute_value_set(
    attribute_set const& source_attrs, attribute_set const& thread_attrs,
    attribute_set const& global_attrs, size_type reserve_count) :
    m_pImpl(implementation::create(source_attrs, thread_attrs, global_attrs,
                                   reserve_count))
{
}
...
//! The constructor adopts three attribute sets to the set
BOOST_LOG_API attribute_value_set::attribute_value_set(
    attribute_value_set const& source_attrs, attribute_set const& thread_attrs,
    attribute_set const& global_attrs, size_type reserve_count) :
    m_pImpl(implementation::create(source_attrs, thread_attrs, global_attrs,
                                   reserve_count))
{
...
BOOST_LOG_API void attribute_value_set::construct(
    attribute_value_set& source_attrs, attribute_set const& thread_attrs,
    attribute_set const& global_attrs, size_type reserve_count)
{
    m_pImpl = implementation::create(boost::move(source_attrs), thread_attrs,
                                     global_attrs, reserve_count);
}

//! Copy constructor
BOOST_LOG_API
    attribute_value_set::attribute_value_set(attribute_value_set const& that)
{
    if (that.m_pImpl)
    ...
    attribute_value_set::begin() const
{
    return const_iterator(m_pImpl->begin(),
                          const_cast<attribute_value_set*>(this));
}
...
BOOST_LOG_API attribute_value_set::size_type attribute_value_set::size() const
{
    return m_pImpl->size();
}
...
{
    return const_iterator(m_pImpl->find(key),
                          const_cast<attribute_value_set*>(this));
}

//! The method acquires values of all adopted attributes. Users don't need to
//! call it, since will always get an already frozen set.
    ...
    attribute_value_set::insert(key_type key, mapped_type const& mapped)
{
    ...
    return std::pair<const_iterator, bool>(const_iterator(res.first, this),
                                           res.second);
}
