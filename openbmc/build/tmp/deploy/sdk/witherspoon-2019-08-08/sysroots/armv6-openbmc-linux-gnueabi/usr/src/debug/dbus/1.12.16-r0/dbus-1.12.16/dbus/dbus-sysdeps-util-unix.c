...
/* dbus-sysdeps-util-unix.c Would be in dbus-sysdeps-unix.c, but not used in
 * libdbus
 *
...
dbus_bool_t _dbus_become_daemon(const DBusString* pidfile,
                                DBusPipe* print_pid_pipe, DBusError* error,
 ...
 * @returns #FALSE on failure
 */
static dbus_bool_t _dbus_write_pid_file(const DBusString* filename,
                                        unsigned long pid, DBusError* error)
{
...
dbus_bool_t _dbus_change_to_daemon_user(const char* user, DBusError* error)
{
    ...
    {
        dbus_set_error(error, DBUS_ERROR_FAILED,
                       "User '%s' does not appear to exist?", user);
        return FALSE;
    }
...
dbus_bool_t _dbus_rlimit_raise_fd_limit(DBusError* error)
{
    ...
    if (getuid() == 0)
    {
        ...
        if (lim.rlim_cur != RLIM_INFINITY && lim.rlim_cur < ENOUGH_FDS)
    ...
    if (lim.rlim_max == old.rlim_max && lim.rlim_cur == old.rlim_cur)
        return TRUE;
    ...
    if (setrlimit(RLIMIT_NOFILE, &lim) < 0)
    {
        dbus_set_error(error, _dbus_error_from_errno(errno),
                       ...
                       (unsigned long)lim.rlim_cur, _dbus_strerror(errno));
        return FALSE;
    }
 ...
 */
dbus_bool_t _dbus_stat(const DBusString* filename, DBusStat* statbuf,
                       DBusError* error)
{
...
DBusDirIter* _dbus_directory_open(const DBusString* filename, DBusError* error)
{
    ...
    {
        dbus_set_error(error, _dbus_error_from_errno(errno),
                       "Failed to read directory \"%s\": %s", filename_c,
                       _dbus_strerror(errno));
        return NULL;
    }
...
again:
    ...
    if (!ent)
    {
        ...
        if (err != 0)
            dbus_set_error(error, _dbus_error_from_errno(err), "%s",
                           _dbus_strerror(err));
...
static dbus_bool_t fill_user_info_from_group(struct group* g,
                                             DBusGroupInfo* info,
                                             DBusError* error)
{
...
static dbus_bool_t fill_group_info(DBusGroupInfo* info, dbus_gid_t gid,
...
dbus_bool_t _dbus_group_info_fill(DBusGroupInfo* info,
                                  const DBusString* groupname, DBusError* error)
{
 ...
 * @returns #FALSE if error is set
 */
dbus_bool_t _dbus_group_info_fill_gid(DBusGroupInfo* info, dbus_gid_t gid,
                                      DBusError* error)
{
 ...
 */
dbus_bool_t _dbus_unix_groups_from_uid(dbus_uid_t uid, dbus_gid_t** group_ids,
                                       int* n_group_ids)
{
 ...
 * @param error return location for errors
 * @returns #TRUE if the UID is the same as the console user and there are no
 * errors
 */
                                     ...
                                     DBusString* dirname)
{
    ...
    if (sep == 0 && _dbus_string_get_byte(filename, 0) == '/')
        ...
        return _dbus_string_copy_len(filename, 0, sep - 0, dirname,
 ...
 */
dbus_bool_t _dbus_command_for_pid(unsigned long pid, DBusString* str,
                                  int max_len, DBusError* error)
{
    ...
    {
        dbus_set_error(
            error, _dbus_error_from_errno(errno), "Failed to open \"%s\": %s",
            _dbus_string_get_const_data(&path), _dbus_strerror(errno));
        goto fail;
    }
...
static dbus_bool_t ensure_owned_directory(const char* label,
                                          const DBusString* string,
                                          dbus_bool_t create, DBusError* error)
{
    ...
    {
        dbus_set_error(error, DBUS_ERROR_FAILED,
                       "%s \"%s\" is owned by uid %ld, not our uid %ld", label,
    ...
    {
        dbus_set_error(error, DBUS_ERROR_FAILED,
                       "%s \"%s\" can be written by others (mode 0%o)", label,
                       dir, buf.st_mode);
        return FALSE;
    }
                                                       ...
                                                       DBusError* error)
{
    ...
    if (!ensure_owned_directory("XDG_RUNTIME_DIR", &xrd, FALSE, error) ||
        !ensure_owned_directory("XDG_RUNTIME_DIR subdirectory", &dbus1, TRUE,
                                error) ||
        !ensure_owned_directory("XDG_RUNTIME_DIR subdirectory", &services, TRUE,
                                error))
        goto out;
...
dbus_bool_t _dbus_get_standard_session_servicedirs(DBusList** dirs)
{
    ...
    if (!_dbus_split_paths_and_append(
            &servicedir_path, DBUS_UNIX_STANDARD_SESSION_SERVICEDIR, dirs))
        goto oom;
...
dbus_bool_t _dbus_get_standard_system_servicedirs(DBusList** dirs)
{
     ...
     * be available.
     */
    static const char standard_search_path[] = "/usr/local/share:"
    ...
    return _dbus_split_paths_and_append(
        &servicedir_path, DBUS_UNIX_STANDARD_SYSTEM_SERVICEDIR, dirs);
}
