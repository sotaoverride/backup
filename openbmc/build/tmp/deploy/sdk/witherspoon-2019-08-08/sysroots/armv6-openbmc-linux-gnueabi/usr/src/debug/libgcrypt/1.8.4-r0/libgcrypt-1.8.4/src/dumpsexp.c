...
static void print_version(int with_help)
{
...
#define alpha_p(a) (((a) >= 'A' && (a) <= 'Z') || ((a) >= 'a' && (a) <= 'z'))
#define hexdigit_p(a)                                                          \
    (digit_p(a) || ((a) >= 'A' && (a) <= 'F') || ((a) >= 'a' && (a) <= 'f'))
#define xtoi_1(a)                                                              \
    ((a) <= '9' ? ((a) - '0')                                                  \
                : (a) <= 'F' ? ((a) - 'A' + 10) : ((a) - 'a' + 10))
...
static inline int whitespace_p(int c)
{
    switch (c)
    {
        case ' ':
...
static void flushdatabuffer(void)
{
    int i;
...
static void printcursor(int both)
{
    int i;
...
static void printerr(const char* text)
{
    printcursor(1);
    printf("\n          Error: %s\n", text);
}

static void printctl(const char* text)
{
    if (verbose && !advanced)
    {
        printcursor(0);
...
static int parse_and_print(FILE* fp)
{
