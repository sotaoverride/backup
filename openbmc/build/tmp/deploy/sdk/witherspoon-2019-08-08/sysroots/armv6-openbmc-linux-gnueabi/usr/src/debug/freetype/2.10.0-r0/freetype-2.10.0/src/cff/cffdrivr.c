...
cff_get_kerning(FT_Face ttface, /* TT_Face */
                FT_UInt left_glyph, FT_UInt right_glyph, FT_Vector* kerning)
{
...
FT_CALLBACK_DEF(FT_Error)
cff_get_advances(FT_Face face, FT_UInt start, FT_UInt count, FT_Int32 flags,
                 FT_Fixed* advances)
{
    FT_UInt nn;
    ...
    if (FT_IS_SFNT(face))
    {
        ...
        if (flags & FT_LOAD_VERTICAL_LAYOUT)
        {
            ...
            for (nn = 0; nn < count; nn++)
            {
                ...
                ((SFNT_Service)ttface->sfnt)
                    ->get_metrics(ttface, 1, start + nn, &dummy, &ah);
        ...
        else
        {
            ...
            for (nn = 0; nn < count; nn++)
            {
                ...
                ((SFNT_Service)ttface->sfnt)
                    ->get_metrics(ttface, 0, start + nn, &dummy, &aw);
...
static FT_Error cff_get_glyph_name(CFF_Face face, FT_UInt glyph_index,
    ...
    if (font->version_major == 2)
    {
        ...
        FT_Service_GlyphDict service =
            (FT_Service_GlyphDict)ft_module_get_service(
                sfnt_module, FT_SERVICE_ID_GLYPH_DICT, 0);

        if (service && service->get_name)
            return service->get_name(FT_FACE(face), glyph_index, buffer,
                                     buffer_max);
        else
        {
...
static FT_Error cff_ps_get_font_info(CFF_Face face, PS_FontInfoRec* afont_info)
{
    ...
    if (cff && !cff->font_info)
    {
        ...
        font_info->family_name =
            cff_index_get_sid_string(cff, dict->family_name);
                                      ...
                                      PS_FontExtraRec* afont_extra)
{
    ...
    if (cff && cff->font_extra == NULL)
    {
        ...
        embedded_postscript =
            cff_index_get_sid_string(cff, dict->embedded_postscript);
        if (embedded_postscript)
        {
            ...
            if ((start_fstype = ft_strstr(embedded_postscript, "/FSType")) !=
                    NULL &&
                (start_def = ft_strstr(start_fstype + sizeof("/FSType") - 1,
                                       "def")) != NULL)
            {
                FT_String* s;

                for (s = start_fstype + sizeof("/FSType") - 1; s != start_def;
                     s++)
                {
                    if (*s >= '0' && *s <= '9')
                    {
                    ...
                    {
                        /* Non-whitespace character between `/FSType' and next
                         * `def' */
    ...
    if (FT_IS_SFNT(FT_FACE(face)) && sfnt)
    {
        ...
        FT_Service_PsFontName service =
            (FT_Service_PsFontName)ft_module_get_service(
                sfnt_module, FT_SERVICE_ID_POSTSCRIPT_FONT_NAME, 0);
 ...
 *
 */
static FT_Error cff_get_cmap_info(FT_CharMap charmap, TT_CMapInfo* cmap_info)
{
        ...
        cmap->clazz != &cff_cmap_unicode_class_rec)
    {
        ...
        FT_Service_TTCMaps service = (FT_Service_TTCMaps)ft_module_get_service(
            sfnt, FT_SERVICE_ID_TT_CMAP, 0);
 ...
 *
 */
static FT_Error cff_get_ros(CFF_Face face, const char** registry,
                            const char** ordering, FT_Int* supplement)
{
    FT_Error error = FT_Err_Ok;
    CFF_Font cff = (CFF_Font)face->extra.data;
    ...
    if (cff)
    {
        ...
        if (registry)
        {
            if (!cff->registry)
                cff->registry =
                    cff_index_get_sid_string(cff, dict->cid_registry);
        ...
        if (ordering)
        {
            if (!cff->ordering)
                cff->ordering =
                    cff_index_get_sid_string(cff, dict->cid_ordering);
...
static FT_Error cff_get_cid_from_glyph_index(CFF_Face face, FT_UInt glyph_index,
                                             FT_UInt* cid)
{
    FT_Error error = FT_Err_Ok;
    CFF_Font cff;
...
FT_DEFINE_SERVICE_CIDREC(
    ...
    (FT_CID_GetRegistryOrderingSupplementFunc)cff_get_ros, /* get_ros */
    (FT_CID_GetIsInternallyCIDKeyedFunc)cff_get_is_cid,    /* get_is_cid    */
...
static FT_Error cff_set_mm_blend(CFF_Face face, FT_UInt num_coords,
                                 FT_Fixed* coords)
{
...
static FT_Error cff_get_mm_blend(CFF_Face face, FT_UInt num_coords,
                                 FT_Fixed* coords)
{
    FT_Service_MultiMasters mm = (FT_Service_MultiMasters)face->mm;
...
static FT_Error cff_set_mm_weightvector(CFF_Face face, FT_UInt len,
...
static FT_Error cff_get_mm_weightvector(CFF_Face face, FT_UInt* len,
                                        FT_Fixed* weightvector)
{
    FT_Service_MultiMasters mm = (FT_Service_MultiMasters)face->mm;
...
static FT_Error cff_get_mm_var(CFF_Face face, FT_MM_Var** master)
{
    FT_Service_MultiMasters mm = (FT_Service_MultiMasters)face->mm;
...
static FT_Error cff_set_var_design(CFF_Face face, FT_UInt num_coords,
                                   FT_Fixed* coords)
{
    FT_Service_MultiMasters mm = (FT_Service_MultiMasters)face->mm;
...
static FT_Error cff_get_var_design(CFF_Face face, FT_UInt num_coords,
                                   FT_Fixed* coords)
{
    FT_Service_MultiMasters mm = (FT_Service_MultiMasters)face->mm;
...
static FT_Error cff_set_instance(CFF_Face face, FT_UInt instance_index)
{
    FT_Service_MultiMasters mm = (FT_Service_MultiMasters)face->mm;
...
FT_DEFINE_SERVICE_MULTIMASTERSREC(
    ...
    (FT_Set_MM_WeightVector_Func)
        cff_set_mm_weightvector, /* set_mm_weightvector */
    (FT_Get_MM_WeightVector_Func)
        cff_get_mm_weightvector, /* get_mm_weightvector */
...
static FT_Error cff_hadvance_adjust(CFF_Face face, FT_UInt gindex,
                                    FT_Int* avalue)
{
...
static void cff_metrics_adjust(CFF_Face face)
{
    FT_Service_MetricsVariations var = (FT_Service_MetricsVariations)face->var;
...
FT_DEFINE_SERVICE_CFFLOADREC(cff_service_cff_load,
...
FT_DEFINE_SERVICEDESCREC10(
    ...
    FT_SERVICE_ID_FONT_FORMAT, FT_FONT_FORMAT_CFF, FT_SERVICE_ID_MULTI_MASTERS,
    &cff_service_multi_masters, FT_SERVICE_ID_METRICS_VARIATIONS,
    &cff_service_metrics_variations, FT_SERVICE_ID_POSTSCRIPT_INFO,
    &cff_service_ps_info, FT_SERVICE_ID_POSTSCRIPT_FONT_NAME,
    &cff_service_ps_name, FT_SERVICE_ID_GLYPH_DICT, &cff_service_glyph_dict,
    FT_SERVICE_ID_TT_CMAP, &cff_service_get_cmap_info, FT_SERVICE_ID_CID,
    &cff_service_cid_info, FT_SERVICE_ID_PROPERTIES, &cff_service_properties,
...
FT_DEFINE_SERVICEDESCREC8(cff_services,
                          ...
                          &cff_service_ps_name, FT_SERVICE_ID_GLYPH_DICT,
                          &cff_service_glyph_dict, FT_SERVICE_ID_TT_CMAP,
                          &cff_service_get_cmap_info, FT_SERVICE_ID_CID,
                          &cff_service_cid_info, FT_SERVICE_ID_PROPERTIES,
                          &cff_service_properties, FT_SERVICE_ID_CFF_LOAD,
...
FT_DEFINE_SERVICEDESCREC9(
    ...
    FT_SERVICE_ID_FONT_FORMAT, FT_FONT_FORMAT_CFF, FT_SERVICE_ID_MULTI_MASTERS,
    &cff_service_multi_masters, FT_SERVICE_ID_METRICS_VARIATIONS,
    &cff_service_metrics_var, FT_SERVICE_ID_POSTSCRIPT_INFO,
    &cff_service_ps_info, FT_SERVICE_ID_POSTSCRIPT_FONT_NAME,
    &cff_service_ps_name, FT_SERVICE_ID_TT_CMAP, &cff_service_get_cmap_info,
    FT_SERVICE_ID_CID, &cff_service_cid_info, FT_SERVICE_ID_PROPERTIES,
    &cff_service_properties, FT_SERVICE_ID_CFF_LOAD, &cff_service_cff_load)
#else
FT_DEFINE_SERVICEDESCREC7(cff_services,
                          ...
                          FT_SERVICE_ID_POSTSCRIPT_INFO, &cff_service_ps_info,
                          FT_SERVICE_ID_POSTSCRIPT_FONT_NAME,
                          &cff_service_ps_name, FT_SERVICE_ID_TT_CMAP,
                          &cff_service_get_cmap_info, FT_SERVICE_ID_CID,
                          &cff_service_cid_info, FT_SERVICE_ID_PROPERTIES,
                          &cff_service_properties, FT_SERVICE_ID_CFF_LOAD,
                          &cff_service_cff_load)
...
FT_DEFINE_DRIVER(cff_driver_class,
                 ...
                 sizeof(PS_DriverRec), "cff", 0x10000L, 0x20000L,
                 ...
                 sizeof(TT_FaceRec), sizeof(CFF_SizeRec),
