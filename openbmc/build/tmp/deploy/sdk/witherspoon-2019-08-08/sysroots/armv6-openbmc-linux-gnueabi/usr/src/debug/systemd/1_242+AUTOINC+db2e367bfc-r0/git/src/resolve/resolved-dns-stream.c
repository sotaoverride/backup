...
static int dns_stream_update_io(DnsStream* s)
{
    ...
    /* Let's read a packet if we haven't queued any yet. Except if we already
     * hit a limit of parallel queries for this connection. */
    if ((!s->read_packet ||
    ...
    /* For handshake and clean closing purposes, TLS can override requested
     * events */
...
{
    _cleanup_(dns_stream_unrefp) _unused_ DnsStream* ref =
        dns_stream_ref(s); /* Protect stream while we process it */

    assert(s);
    assert(error >= 0);

    /* Error is > 0 when the connection failed for some reason in the network
     * stack. It's == 0 if we sent
    ...
    if (s->complete)
        s->complete(s, error);
    else /* the default action if no completion function is set is to close the
            stream */
        dns_stream_unref(s);
...
static int dns_stream_identify(DnsStream* s)
{
    union
    {
        ...
        uint8_t buffer
            [CMSG_SPACE(MAXSIZE(struct in_pktinfo, struct in6_pktinfo)) +
    ...
    if (s->ifindex <= 0)
        s->ifindex = manager_find_ifindex(
            s->manager, s->local.sa.sa_family,
    ...
    if (s->protocol == DNS_PROTOCOL_LLMNR && s->ifindex > 0)
    {
        ...
        /* Make sure all packets for this connection are sent on the same
         * interface */
        if (s->local.sa.sa_family == AF_INET)
        {
            r = setsockopt(s->fd, IPPROTO_IP, IP_UNICAST_IF, &ifindex,
                           sizeof(ifindex));
            if (r < 0)
        ...
        {
            r = setsockopt(s->fd, IPPROTO_IPV6, IPV6_UNICAST_IF, &ifindex,
                           sizeof(ifindex));
            if (r < 0)
...
ssize_t dns_stream_writev(DnsStream* s, const struct iovec* iov, size_t iovcnt,
                          int flags)
{
    ssize_t m;
...
static int on_stream_io(sd_event_source* es, int fd, uint32_t revents,
                        void* userdata)
{
    _cleanup_(dns_stream_unrefp) DnsStream* s =
        dns_stream_ref(userdata); /* Protect stream while we process it */
         ...
         s->n_read < sizeof(s->read_size) + s->read_packet->size))
    {
        ...
        if (s->n_read < sizeof(s->read_size))
        {
            ...
            ss = dns_stream_read(s, (uint8_t*)&s->read_size + s->n_read,
        ...
        if (s->n_read >= sizeof(s->read_size))
        {
            ...
            if (s->n_read < sizeof(s->read_size) + be16toh(s->read_size))
            {
                ...
                if (!s->read_packet)
                {
                    r = dns_packet_new(&s->read_packet, s->protocol,
                                       be16toh(s->read_size),
                                       DNS_PACKET_SIZE_MAX);
                    if (r < 0)
                        return dns_stream_complete(s, -r);
                    ...
                    if (s->read_packet->family == AF_INET)
                    {
                        ...
                        s->read_packet->sender_port =
                            be16toh(s->peer.in.sin_port);
                        s->read_packet->destination.in = s->local.in.sin_addr;
                        s->read_packet->destination_port =
                            be16toh(s->local.in.sin_port);
                    }
                    else
                    {
                        ...
                        s->read_packet->sender_port =
                            be16toh(s->peer.in6.sin6_port);
                        s->read_packet->destination.in6 =
                            s->local.in6.sin6_addr;
                        s->read_packet->destination_port =
                            be16toh(s->local.in6.sin6_port);

                        if (s->read_packet->ifindex == 0)
                            s->read_packet->ifindex = s->peer.in6.sin6_scope_id;
                        if (s->read_packet->ifindex == 0)
                            s->read_packet->ifindex =
                                s->local.in6.sin6_scope_id;
                    }
                }
    ...
    /* Call "complete" callback if finished reading and writing one packet, and
     * there's nothing else left to write. */
    ...
    {
        r = sd_event_source_set_time(s->timeout_event_source,
                                     now(clock_boottime_or_monotonic()) +
                                         DNS_STREAM_TIMEOUT_USEC);
        if (r < 0)
            log_warning_errno(
                errno, "Couldn't restart TCP connection timeout, ignoring: %m");
    }
...
int dns_stream_new(Manager* m, DnsStream** ret, DnsStreamType type,
                   DnsProtocol protocol, int fd,
                   const union sockaddr_union* tfo_address)
{
    ...
    *s = (DnsStream){
    ...
    r = sd_event_add_io(m->event, &s->io_event_source, fd, EPOLLIN,
                        on_stream_io, s);
    if (r < 0)
        return r;
    ...
    r = sd_event_add_time(
        m->event, &s->timeout_event_source, clock_boottime_or_monotonic(),
        now(clock_boottime_or_monotonic()) + DNS_STREAM_TIMEOUT_USEC, 0,
        on_stream_timeout, s);
    if (r < 0)
        return r;

    (void)sd_event_source_set_description(s->timeout_event_source,
                                          "dns-stream-timeout");
    ...
    if (tfo_address)
    {
        ...
        s->tfo_salen = tfo_address->sa.sa_family == AF_INET6
                           ? sizeof(tfo_address->in6)
