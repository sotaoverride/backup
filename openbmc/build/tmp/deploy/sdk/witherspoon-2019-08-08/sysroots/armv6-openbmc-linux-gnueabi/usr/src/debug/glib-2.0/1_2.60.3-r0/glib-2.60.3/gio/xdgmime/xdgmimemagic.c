...
static XdgMimeMagicMatch* _xdg_mime_magic_match_new(void)
{
    return calloc(1, sizeof(XdgMimeMagicMatch));
}

static XdgMimeMagicMatchlet* _xdg_mime_magic_matchlet_new(void)
{
    XdgMimeMagicMatchlet* matchlet;
    ...
    _xdg_mime_magic_parse_magic_line(FILE* magic_file, XdgMimeMagicMatch* match)
{
    ...
    if (c == '&')
    {
        ...
        bytes_read =
            fread(matchlet->mask, 1, matchlet->value_length, magic_file);
        if (bytes_read != matchlet->value_length)
        {
            _xdg_mime_magic_matchlet_free(matchlet);
            if (feof(magic_file))
    ...
    if (c == '~')
    {
        ...
        if (matchlet->word_size != 0 && matchlet->word_size != 1 &&
...
static int _xdg_mime_magic_matchlet_compare_level(
    XdgMimeMagicMatchlet* matchlet, const void* data, size_t len, int indent)
{
    while ((matchlet != NULL) && (matchlet->indent == indent))
    {
        if (_xdg_mime_magic_matchlet_compare_to_data(matchlet, data, len))
        {
            ...
            if (_xdg_mime_magic_matchlet_compare_level(matchlet->next, data,
                                                       len, indent + 1))
                return TRUE;
        }
...
static int _xdg_mime_magic_match_compare_to_data(XdgMimeMagicMatch* match,
                                                 const void* data, size_t len)
{
    return _xdg_mime_magic_matchlet_compare_level(match->matchlet, data, len,
                                                  0);
}
    ...
    while (state != XDG_MIME_MAGIC_EOF)
    {
        switch (state)
        {
            case XDG_MIME_MAGIC_SECTION:
                ...
                if (state == XDG_MIME_MAGIC_EOF ||
                    state == XDG_MIME_MAGIC_ERROR)
            ...
            case XDG_MIME_MAGIC_MAGIC:
                ...
                {
                    match->matchlet =
                        _xdg_mime_magic_matchlet_mirror(match->matchlet);
                    _xdg_mime_magic_insert_match(mime_magic, match);
                }
                else if (state == XDG_MIME_MAGIC_EOF ||
                         state == XDG_MIME_MAGIC_ERROR)
                    _xdg_mime_magic_match_free(match);
                break;
