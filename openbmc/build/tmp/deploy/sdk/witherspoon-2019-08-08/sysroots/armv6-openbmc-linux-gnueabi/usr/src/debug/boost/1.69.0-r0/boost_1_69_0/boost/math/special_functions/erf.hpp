...
inline T erf_asymptotic_limit()
{
    ...
    typedef typename mpl::if_<
        ...
        typename mpl::if_<mpl::less_equal<precision_type, mpl::int_<0>>,
                          mpl::int_<0>, mpl::int_<24>>::type,
        typename mpl::if_<
            mpl::less_equal<precision_type, mpl::int_<53>>, mpl::int_<53>,
            typename mpl::if_<
                mpl::less_equal<precision_type, mpl::int_<64>>, mpl::int_<64>,
                typename mpl::if_<
                    mpl::less_equal<precision_type, mpl::int_<106>>,
                    mpl::int_<106>,
                    typename mpl::if_<
                        mpl::less_equal<precision_type, mpl::int_<113>>,
                        mpl::int_<113>, mpl::int_<0>>::type>::type>::type>::
            type>::type tag_type;
...
T erf_imp(T z, bool invert, const Policy& pol, const Tag& t)
{
    ...
    if (!invert && (z > detail::erf_asymptotic_limit<T, Policy>()))
    {
        ...
        result = boost::math::tools::sum_series(
            s, policies::get_epsilon<T, Policy>(), max_iter, 1);
        policies::check_series_iterations<T>("boost::math::erf<%1%>(%1%, %1%)",
                                             max_iter, pol);
    }
    else
    {
        ...
        else
        {
            ...
            result *= upper_gamma_fraction(T(0.5f), x,
...
T erf_imp(T z, bool invert, const Policy& pol, const mpl::int_<53>& t)
{
    ...
    if ((boost::math::isnan)(z))
        return policies::raise_denorm_error(
            "boost::math::erf<%1%>(%1%)",
            "Expected a finite argument but got %1%", z, pol);
    ...
    if (z < 0.5)
    {
        ...
        if (z < 1e-10)
        {
            ...
            {
                static const T c = BOOST_MATH_BIG_CONSTANT(
                    T, 53, 0.003379167095512573896158903121545171688);
        ...
        else
        {
            ...
            T zz = z * z;
            result = z * (Y + tools::evaluate_polynomial(P, zz) /
                                  tools::evaluate_polynomial(Q, zz));
        }
    }
    else if (invert ? (z < 28) : (z < 5.8f))
    {
        ...
        if (z < 1.5f)
        {
            ...
            result = Y + tools::evaluate_polynomial(P, T(z - 0.5)) /
                             tools::evaluate_polynomial(Q, T(z - 0.5));
        ...
        else if (z < 2.5f)
        {
            ...
            result = Y + tools::evaluate_polynomial(P, T(z - 1.5)) /
                             tools::evaluate_polynomial(Q, T(z - 1.5));
            T hi, lo;
            int expon;
        ...
        else if (z < 4.5f)
        {
            ...
            result = Y + tools::evaluate_polynomial(P, T(z - 3.5)) /
                             tools::evaluate_polynomial(Q, T(z - 3.5));
            T hi, lo;
            int expon;
            hi = floor(ldexp(frexp(z, &expon), 26));
            hi = ldexp(hi, expon - 26);
        ...
        else
        {
            ...
            result = Y + tools::evaluate_polynomial(P, T(1 / z)) /
                             tools::evaluate_polynomial(Q, T(1 / z));
            T hi, lo;
            int expon;
            hi = floor(ldexp(frexp(z, &expon), 26));
            hi = ldexp(hi, expon - 26);
    ...
    return result;
} // template <class T, class Lanczos>T erf_imp(T z, bool invert, const Lanczos&
  // l, const mpl::int_<53>& t)
...
T erf_imp(T z, bool invert, const Policy& pol, const mpl::int_<64>& t)
{
    ...
    if (z < 0.5)
    {
        ...
        {
            static const T c = BOOST_MATH_BIG_CONSTANT(
                T, 64, 0.003379167095512573896158903121545171688);
            result = z * 1.125 + z * c;
        }
        else
        {
            ...
            result = z * (Y + tools::evaluate_polynomial(P, T(z * z)) /
                                  tools::evaluate_polynomial(Q, T(z * z)));
        }
    }
    else if (invert ? (z < 110) : (z < 6.4f))
    {
        ...
        if (z < 1.5)
        {
            ...
            result = Y + tools::evaluate_polynomial(P, T(z - 0.5f)) /
                             tools::evaluate_polynomial(Q, T(z - 0.5f));
            T hi, lo;
            int expon;
        ...
        else if (z < 2.5)
        {
            ...
            result = Y + tools::evaluate_polynomial(P, T(z - 1.5f)) /
                             tools::evaluate_polynomial(Q, T(z - 1.5f));
            T hi, lo;
            int expon;
            hi = floor(ldexp(frexp(z, &expon), 32));
            hi = ldexp(hi, expon - 32);
        ...
        else if (z < 4.5)
        {
            ...
            result = Y + tools::evaluate_polynomial(P, T(z - 3.5f)) /
...
T erf_imp(T z, bool invert, const Policy& pol, const mpl::int_<113>& t)
{
    ...
    if (z < 0.5)
    {
        ...
        {
            static const T c = BOOST_MATH_BIG_CONSTANT(
                T, 113, 0.003379167095512573896158903121545171688);
            result = z * 1.125 + z * c;
        }
        else
        {
            ...
            static const T P[] = {
                BOOST_MATH_BIG_CONSTANT(
                    T, 113, 0.0442269454158250738961589031215451778),
                BOOST_MATH_BIG_CONSTANT(T, 113,
                                        -0.35549265736002144875335323556961233),
                BOOST_MATH_BIG_CONSTANT(
                    T, 113, -0.0582179564566667896225454670863270393),
                BOOST_MATH_BIG_CONSTANT(
                    T, 113, -0.0112694696904802304229950538453123925),
                BOOST_MATH_BIG_CONSTANT(
                    T, 113, -0.000805730648981801146251825329609079099),
                BOOST_MATH_BIG_CONSTANT(
                    T, 113, -0.566304966591936566229702842075966273e-4),
                BOOST_MATH_BIG_CONSTANT(
                    T, 113, -0.169655010425186987820201021510002265e-5),
                BOOST_MATH_BIG_CONSTANT(
                    T, 113, -0.344448249920445916714548295433198544e-7),
            };
            static const T Q[] = {
                BOOST_MATH_BIG_CONSTANT(T, 113, 1.0),
                BOOST_MATH_BIG_CONSTANT(T, 113,
                                        0.466542092785657604666906909196052522),
                BOOST_MATH_BIG_CONSTANT(T, 113,
                                        0.100005087012526447295176964142107611),
                BOOST_MATH_BIG_CONSTANT(
                    T, 113, 0.0128341535890117646540050072234142603),
                BOOST_MATH_BIG_CONSTANT(
                    T, 113, 0.00107150448466867929159660677016658186),
                BOOST_MATH_BIG_CONSTANT(
                    T, 113, 0.586168368028999183607733369248338474e-4),
                BOOST_MATH_BIG_CONSTANT(
                    T, 113, 0.196230608502104324965623171516808796e-5),
                BOOST_MATH_BIG_CONSTANT(
                    T, 113, 0.313388521582925207734229967907890146e-7),
            };
            result = z * (Y + tools::evaluate_polynomial(P, T(z * z)) /
                                  tools::evaluate_polynomial(Q, T(z * z)));
        }
    ...
    else if (invert ? (z < 110) : (z < 8.65f))
    {
        ...
        if (z < 1)
        {
            ...
            static const T P[] = {
                BOOST_MATH_BIG_CONSTANT(
                    T, 113, -0.0640320213544647969396032886581290455),
                BOOST_MATH_BIG_CONSTANT(T, 113,
                                        0.200769874440155895637857443946706731),
                BOOST_MATH_BIG_CONSTANT(T, 113,
                                        0.378447199873537170666487408805779826),
                BOOST_MATH_BIG_CONSTANT(T, 113,
                                        0.30521399466465939450398642044975127),
                BOOST_MATH_BIG_CONSTANT(T, 113,
                                        0.146890026406815277906781824723458196),
                BOOST_MATH_BIG_CONSTANT(
                    T, 113, 0.0464837937749539978247589252732769567),
                BOOST_MATH_BIG_CONSTANT(
                    T, 113, 0.00987895759019540115099100165904822903),
                BOOST_MATH_BIG_CONSTANT(
                    T, 113, 0.00137507575429025512038051025154301132),
                BOOST_MATH_BIG_CONSTANT(
                    T, 113, 0.0001144764551085935580772512359680516),
                BOOST_MATH_BIG_CONSTANT(
                    T, 113, 0.436544865032836914773944382339900079e-5),
            };
            static const T Q[] = {
                BOOST_MATH_BIG_CONSTANT(T, 113, 1.0),
                BOOST_MATH_BIG_CONSTANT(T, 113,
                                        2.47651182872457465043733800302427977),
                BOOST_MATH_BIG_CONSTANT(T, 113,
                                        2.78706486002517996428836400245547955),
                BOOST_MATH_BIG_CONSTANT(T, 113,
                                        1.87295924621659627926365005293130693),
                BOOST_MATH_BIG_CONSTANT(T, 113,
                                        0.829375825174365625428280908787261065),
                BOOST_MATH_BIG_CONSTANT(T, 113,
                                        0.251334771307848291593780143950311514),
                BOOST_MATH_BIG_CONSTANT(
                    T, 113, 0.0522110268876176186719436765734722473),
                BOOST_MATH_BIG_CONSTANT(
                    T, 113, 0.00718332151250963182233267040106902368),
                BOOST_MATH_BIG_CONSTANT(
                    T, 113, 0.000595279058621482041084986219276392459),
                BOOST_MATH_BIG_CONSTANT(
                    T, 113, 0.226988669466501655990637599399326874e-4),
                BOOST_MATH_BIG_CONSTANT(
                    T, 113, 0.270666232259029102353426738909226413e-10),
            };
            result = Y + tools::evaluate_polynomial(P, T(z - 0.5f)) /
                             tools::evaluate_polynomial(Q, T(z - 0.5f));
            T hi, lo;
        ...
        else if (z < 1.5)
        {
            ...
            static const T P[] = {
                BOOST_MATH_BIG_CONSTANT(
                    T, 113, -0.0289965858925328393392496555094848345),
                BOOST_MATH_BIG_CONSTANT(
                    T, 113, 0.0868181194868601184627743162571779226),
                BOOST_MATH_BIG_CONSTANT(T, 113,
                                        0.169373435121178901746317404936356745),
                BOOST_MATH_BIG_CONSTANT(T, 113,
                                        0.13350446515949251201104889028133486),
                BOOST_MATH_BIG_CONSTANT(
                    T, 113, 0.0617447837290183627136837688446313313),
                BOOST_MATH_BIG_CONSTANT(
                    T, 113, 0.0185618495228251406703152962489700468),
                BOOST_MATH_BIG_CONSTANT(
                    T, 113, 0.00371949406491883508764162050169531013),
                BOOST_MATH_BIG_CONSTANT(
                    T, 113, 0.000485121708792921297742105775823900772),
                BOOST_MATH_BIG_CONSTANT(
                    T, 113, 0.376494706741453489892108068231400061e-4),
                BOOST_MATH_BIG_CONSTANT(
                    T, 113, 0.133166058052466262415271732172490045e-5),
            };
            static const T Q[] = {
                BOOST_MATH_BIG_CONSTANT(T, 113, 1.0),
                BOOST_MATH_BIG_CONSTANT(T, 113,
                                        2.32970330146503867261275580968135126),
                BOOST_MATH_BIG_CONSTANT(T, 113,
                                        2.46325715420422771961250513514928746),
                BOOST_MATH_BIG_CONSTANT(T, 113,
                                        1.55307882560757679068505047390857842),
                BOOST_MATH_BIG_CONSTANT(T, 113,
                                        0.644274289865972449441174485441409076),
                BOOST_MATH_BIG_CONSTANT(T, 113,
                                        0.182609091063258208068606847453955649),
                BOOST_MATH_BIG_CONSTANT(
                    T, 113, 0.0354171651271241474946129665801606795),
                BOOST_MATH_BIG_CONSTANT(
                    T, 113, 0.00454060370165285246451879969534083997),
                BOOST_MATH_BIG_CONSTANT(
                    T, 113, 0.000349871943711566546821198612518656486),
                BOOST_MATH_BIG_CONSTANT(
                    T, 113, 0.123749319840299552925421880481085392e-4),
            };
            result = Y + tools::evaluate_polynomial(P, T(z - 1.0f)) /
                             tools::evaluate_polynomial(Q, T(z - 1.0f));
            T hi, lo;
            int expon;
            hi = floor(ldexp(frexp(z, &expon), 56));
            hi = ldexp(hi, expon - 56);
        ...
        else if (z < 2.25)
        {
            ...
            static const T P[] = {
                BOOST_MATH_BIG_CONSTANT(
                    T, 113, -0.0201233630504573402185161184151016606),
                BOOST_MATH_BIG_CONSTANT(
                    T, 113, 0.0331864357574860196516686996302305002),
                BOOST_MATH_BIG_CONSTANT(
                    T, 113, 0.0716562720864787193337475444413405461),
                BOOST_MATH_BIG_CONSTANT(
                    T, 113, 0.0545835322082103985114927569724880658),
                BOOST_MATH_BIG_CONSTANT(
                    T, 113, 0.0236692635189696678976549720784989593),
                BOOST_MATH_BIG_CONSTANT(
                    T, 113, 0.00656970902163248872837262539337601845),
                BOOST_MATH_BIG_CONSTANT(
                    T, 113, 0.00120282643299089441390490459256235021),
                BOOST_MATH_BIG_CONSTANT(
                    T, 113, 0.000142123229065182650020762792081622986),
                BOOST_MATH_BIG_CONSTANT(
                    T, 113, 0.991531438367015135346716277792989347e-5),
                BOOST_MATH_BIG_CONSTANT(
                    T, 113, 0.312857043762117596999398067153076051e-6),
            };
            static const T Q[] = {
                BOOST_MATH_BIG_CONSTANT(T, 113, 1.0),
                BOOST_MATH_BIG_CONSTANT(T, 113,
                ...
                BOOST_MATH_BIG_CONSTANT(T, 113,
                                        2.06399257267556230937723190496806215),
                BOOST_MATH_BIG_CONSTANT(T, 113,
                                        1.18678481279932541314830499880691109),
                BOOST_MATH_BIG_CONSTANT(T, 113,
                                        0.447733186643051752513538142316799562),
                BOOST_MATH_BIG_CONSTANT(T, 113,
                                        0

                BOOST_MATH_BIG_CONSTANT(
                    T, 113, 0.204123895931375107397698245752850347e-4),
                BOOST_MATH_BIG_CONSTANT(
                    T, 113, 0.674128352521481412232785122943508729e-6),
                BOOST_MATH_BIG_CONSTANT(
                    T, 113, 0.997637501418963696542159244436245077e-8),
            };
            result = Y + tools::evaluate_polynomial(P, T(z - 6.5f)) /
                             tools::evaluate_polynomial(Q, T(z - 6.5f));
            T hi, lo;
            int expon;
            hi = floor(ldexp(frexp(z, &expon), 56));
            hi = ldexp(hi, expon - 56);
        ...
        else if (z < 11.5)
        {
            ...
            static const T P[] = {
                BOOST_MATH_BIG_CONSTANT(
                    T, 113, 0.000282420728751494363613829834891390121),
                BOOST_MATH_BIG_CONSTANT(
                    T, 113, 0.00175387065018002823433704079355125161),
                BOOST_MATH_BIG_CONSTANT(
                    T, 113, 0.0021344978564889819420775336322920375),
                BOOST_MATH_BIG_CONSTANT(
                    T, 113, 0.00124151356560137532655039683963075661),
                BOOST_MATH_BIG_CONSTANT(
                    T, 113, 0.000423600733566948018555157026862139644),
                BOOST_MATH_BIG_CONSTANT(
                    T, 113, 0.914030340865175237133613697319509698e-4),
                BOOST_MATH_BIG_CONSTANT(
                    T, 113, 0.126999927156823363353809747017945494e-4),
                BOOST_MATH_BIG_CONSTANT(
                    T, 113, 0.110610959842869849776179749369376402e-5),
                BOOST_MATH_BIG_CONSTANT(
                    T, 113, 0.55075079477173482096725348704634529e-7),
                BOOST_MATH_BIG_CONSTANT(
                    T, 113, 0.119735694018906705225870691331543806e-8),
            };
            static const T Q[] = {
                BOOST_MATH_BIG_CONSTANT(T, 113, 1.0),
                BOOST_MATH_BIG_CONSTANT(T, 113,
                                        1.69889613396167354566098060039549882),
                BOOST_MATH_BIG_CONSTANT(T, 113,
                                        1.28824647372749624464956031163282674),
                BOOST_MATH_BIG_CONSTANT(T, 113,
                                        0.572297795434934493541628008224078717),
                BOOST_MATH_BIG_CONSTANT(T, 113,
                                        0.164157697425571712377043857240773164),
                BOOST_MATH_BIG_CONSTANT(
                    T, 113, 0.0315311145224594430281219516531649562),
                BOOST_MATH_BIG_CONSTANT(
                    T, 113, 0.00405588922155632380812945849777127458),
                BOOST_MATH_BIG_CONSTANT(
                    T, 113, 0.000336929033691445666232029762868642417),
                BOOST_MATH_BIG_CONSTANT(
                    T, 113, 0.164033049810404773469413526427932109e-4),
                BOOST_MATH_BIG_CONSTANT(
                    T, 113, 0.356615210500531410114914617294694857e-6),
            };
            result = Y + tools::evaluate_polynomial(P, T(z / 2 - 4.75f)) /
                             tools::evaluate_polynomial(Q, T(z / 2 - 4.75f));
            T hi, lo;
            int expon;
            hi = floor(ldexp(frexp(z, &expon), 56));
            hi = ldexp(hi, expon - 56);
        ...
        else
        {
            ...
            static const T P[] = {
                BOOST_MATH_BIG_CONSTANT(
                    T, 113, 0.000920922048732849448079451574171836943),
                BOOST_MATH_BIG_CONSTANT(
                    T, 113, 0.00321439044532288750501700028748922439),
                BOOST_MATH_BIG_CONSTANT(
                    T, 113, -0.250455263029390118657884864261823431),
                BOOST_MATH_BIG_CONSTANT(
                    T, 113, -0.906807635364090342031792404764598142),
                BOOST_MATH_BIG_CONSTANT(T, 113,
                                        -8.92233572835991735876688745989985565),
                BOOST_MATH_BIG_CONSTANT(T, 113,
                                        -21.7797433494422564811782116907878495),
                BOOST_MATH_BIG_CONSTANT(T, 113,
                                        -91.1451915251976354349734589601171659),
                BOOST_MATH_BIG_CONSTANT(T, 113,
                                        -144.1279109655993927069052125017673),
                BOOST_MATH_BIG_CONSTANT(T, 113,
                                        -313.845076581796338665519022313775589),
                BOOST_MATH_BIG_CONSTANT(T, 113,
                                        -273.11378811923343424081101235736475),
                BOOST_MATH_BIG_CONSTANT(T, 113,
                                        -271.651566205951067025696102600443452),
                BOOST_MATH_BIG_CONSTANT(T, 113,
                                        -60.0530577077238079968843307523245547),
            };
            static const T Q[] = {
                BOOST_MATH_BIG_CONSTANT(T, 113, 1.0),
                BOOST_MATH_BIG_CONSTANT(T, 113,
                ...
                BOOST_MATH_BIG_CONSTANT(T, 113,
                                        34.3563592467165971295915749548313227),
                BOOST_MATH_BIG_CONSTANT(T, 113,
                                        84.4993232033879023178285731843850461),
                BOOST_MATH_BIG_CONSTANT(T, 113,
                                        376.005865281206894120659401340373818),
                BOOST_MATH_BIG_CONSTANT(T, 113,
                                        629.95369438888946233003926191755125),
                BOOST_MATH_BIG_CONSTANT(T, 113,
                                        1568.35771983533158591604513304269098),
                BOOST_MATH_BIG_CONSTANT(T, 113,
                                        1646.02452040831961063640827116581021),
                BOOST_MATH_BIG_CONSTANT(T, 113,
                                        2299.96860633240298708910425594484895),
                BOOST_MATH_BIG_CONSTANT(T, 113,
                                        1222.73204392037452750381340219906374),
                BOOST_MATH_BIG_CONSTANT(T, 113,
                                        799.359797306084372350264298361110448),
                BOOST_MATH_BIG_CONSTANT(T, 113,
                                        72.7415265778588087243442792401576737),
            };
            result = Y + tools::evaluate_polynomial(P, T(1 / z)) /
                             tools::evaluate_polynomial(Q, T(1 / z));
            T hi, lo;
...
template <class T, class Policy, class tag>
const typename erf_initializer<T, Policy, tag>::init
    erf_initializer<T, Policy, tag>::initializer;
...
inline typename tools::promote_args<T>::type erf(T z, const Policy& /* pol */)
{
    ...
    BOOST_MATH_INSTRUMENT_CODE(
        "precision_type = " << typeid(precision_type).name());
    ...
    detail::erf_initializer<value_type, forwarding_policy, tag_type>::
        force_instantiate(); // Force constants to be initialized before main

    return policies::checked_narrowing_cast<result_type, forwarding_policy>(
        detail::erf_imp(static_cast<value_type>(z), false, forwarding_policy(),
                        tag_type()),
        "boost::math::erf<%1%>(%1%, %1%)");
}
...
inline typename tools::promote_args<T>::type erfc(T z, const Policy& /* pol */)
{
    ...
    return policies::checked_narrowing_cast<result_type, forwarding_policy>(
        detail::erf_imp(static_cast<value_type>(z), true, forwarding_policy(),
                        tag_type()),
