...
tt_face_load_loca(TT_Face face, FT_Stream stream)
{
    ...
    if (face->num_locations != (FT_ULong)face->root.num_glyphs + 1)
    {
        ...
        {
            FT_ULong new_loca_len = ((FT_ULong)face->root.num_glyphs + 1)
                                    << shift;
            ...
            else
            {
                face->root.num_glyphs =
                    face->num_locations ? (FT_Long)face->num_locations - 1 : 0;
...
tt_face_load_hdmx(TT_Face face, FT_Stream stream)
{
    ...
    if (num_records > 255 ||
        (num_records > 0 && (record_size > 0x10001L || record_size < 4)))
    {
