 ...
 */
static_assert(alignof(CRBNode) <= alignof(max_align_t),
              "Invalid RBNode alignment");
static_assert(alignof(CRBNode) >= 8, "Invalid CRBNode alignment");
static_assert(alignof(CRBTree) <= alignof(max_align_t),
...
static inline CRBTree* c_rbnode_push_root(CRBNode* n, CRBTree* t)
{
    ...
    {
        if (n)
            n->__parent_and_flags =
                (unsigned long)t | c_rbnode_flags(n) | C_RBNODE_ROOT;
...
static inline void c_rbtree_paint_terminal(CRBNode* n)
{
    ...
    if (p == g->left)
    {
        ...
        if (x)
            c_rbnode_set_parent_and_flags(x, g,
...
static inline void c_rbnode_rebalance_terminal(CRBNode* p, CRBNode* previous)
{
    ...
    if (previous == p->left)
    {
        ...
        if (c_rbnode_is_red(s))
        {
            ...
            c_rbnode_set_parent_and_flags(x, p,
                                          c_rbnode_flags(x) & ~C_RBNODE_RED);
            c_rbnode_set_parent_and_flags(s, g,
                                          c_rbnode_flags(s) & ~C_RBNODE_RED);
            c_rbnode_set_parent_and_flags(p, s,
        ...
        if (!x || c_rbnode_is_black(x))
        {
            ...
            if (!y || c_rbnode_is_black(y))
            {
                ...
                c_rbnode_set_parent_and_flags(s, p,
                                              c_rbnode_flags(s) | C_RBNODE_RED);
                c_rbnode_set_parent_and_flags(
                    p, c_rbnode_parent(p), c_rbnode_flags(p) & ~C_RBNODE_RED);
                return;
            }
...
_public_ void c_rbnode_unlink_stale(CRBNode* n)
{
    ...
    else if (!n->left && n->right)
    {
        ...
        c_rbnode_set_parent_and_flags(n->right, c_rbnode_parent(n),
                                      c_rbnode_flags(n->right) & ~C_RBNODE_RED);
    ...
    else if (n->left && !n->right)
    {
        ...
        c_rbnode_set_parent_and_flags(n->left, c_rbnode_parent(n),
                                      c_rbnode_flags(n->left) & ~C_RBNODE_RED);
    ...
    else /* if (n->left && n->right) */
    {
         ...
         */
        if (c)
            c_rbnode_set_parent_and_flags(c, p,
                                          c_rbnode_flags(c) & ~C_RBNODE_RED);
        else
