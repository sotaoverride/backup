...
{
    return ((p[3] & 0xff) | ((p[2] & 0xff) << 8) | ((p[1] & 0xff) << 16) |
            ((p[0] & 0xff) << 24));
}
...
static int isosize(int argc, char* filenamep, int xflag, long divisor)
{
    ...
    if (pread(fd, volume_space_size, sizeof(volume_space_size), 0x8050) !=
            sizeof(volume_space_size) ||
        pread(fd, logical_block_size, sizeof(logical_block_size), 0x8080) !=
...
static void __attribute__((__noreturn__)) usage(void)
{
    ...
    fprintf(stdout, _(" %s [options] <iso9660_image_file> ...\n"),
    ...
    fputs(
        _(" -d, --divisor=<number>  divide the amount of bytes by <number>\n"),
        stdout);
...
int main(int argc, char** argv)
{
    ...
    while ((opt = getopt_long(argc, argv, "d:xVh", longopts, NULL)) != -1)
    {
        ...
        {
            case 'd':
                divisor = strtol_or_err(optarg, _("invalid divisor argument"));
                break;
            case 'x':
                xflag = 1;
                break;
