...
_public_ int sd_bus_creds_new_from_pid(sd_bus_creds** ret, pid_t pid,
                                       uint64_t mask)
{
    sd_bus_creds* c;
    int r;
...
_public_ int sd_bus_creds_get_supplementary_gids(sd_bus_creds* c,
                                                 const gid_t** gids)
{
    assert_return(c, -EINVAL);
...
_public_ int sd_bus_creds_get_audit_session_id(sd_bus_creds* c,
                                               uint32_t* sessionid)
{
    assert_return(c, -EINVAL);
...
_public_ int sd_bus_creds_get_unique_name(sd_bus_creds* c,
...
_public_ int sd_bus_creds_get_well_known_names(sd_bus_creds* c,
                                               char*** well_known_names)
{
    ...
    {
        static const char* const wkn[] = {"org.freedesktop.DBus", NULL};
    ...
    {
        static const char* const wkn[] = {"org.freedesktop.DBus.Local", NULL};
...
static int has_cap(sd_bus_creds* c, size_t offset, int capability)
{
    ...
    return !!(c->capability[offset * sz + CAP_TO_INDEX((uint32_t)capability)] &
...
int bus_creds_add_more(sd_bus_creds* c, uint64_t mask, pid_t pid, pid_t tid)
{
    ...
    missing =
        mask & ~(c->mask | SD_BUS_CREDS_PID | SD_BUS_CREDS_TID |
                 SD_BUS_CREDS_UNIQUE_NAME | SD_BUS_CREDS_WELL_KNOWN_NAMES |
                 SD_BUS_CREDS_DESCRIPTION | SD_BUS_CREDS_AUGMENT);
    if (missing == 0)
        return 0;
    ...
    if (missing & (SD_BUS_CREDS_PPID | SD_BUS_CREDS_UID | SD_BUS_CREDS_EUID |
            ...
            for (;;)
            {
                ...
                if (missing & SD_BUS_CREDS_PPID)
                {
                    ...
                    if (p)
                    {
                        ...
                        /* Explicitly check for PPID 0 (which is the case for
                         * PID 1) */
                        if (!streq(p, "0"))
                        {
                ...
                if (missing & (SD_BUS_CREDS_UID | SD_BUS_CREDS_EUID |
                               SD_BUS_CREDS_SUID | SD_BUS_CREDS_FSUID))
                {
                    ...
                    if (p)
                    {
                        ...
                        p += strspn(p, WHITESPACE);
                        if (sscanf(p, "%lu %lu %lu %lu", &uid, &euid, &suid,
                                   &fsuid) != 4)
                            return -EIO;
                        ...
                        c->mask |=
                            missing & (SD_BUS_CREDS_UID | SD_BUS_CREDS_EUID |
                                       SD_BUS_CREDS_SUID | SD_BUS_CREDS_FSUID);
                        continue;
                    }
                }

                if (missing & (SD_BUS_CREDS_GID | SD_BUS_CREDS_EGID |
                               SD_BUS_CREDS_SGID | SD_BUS_CREDS_FSGID))
                {
                    ...
                    if (p)
                    {
                        ...
                        p += strspn(p, WHITESPACE);
                        if (sscanf(p, "%lu %lu %lu %lu", &gid, &egid, &sgid,
                                   &fsgid) != 4)
                            return -EIO;
                        ...
                        c->mask |=
                            missing & (SD_BUS_CREDS_GID | SD_BUS_CREDS_EGID |
                                       SD_BUS_CREDS_SGID | SD_BUS_CREDS_FSGID);
                        continue;
                    }
                }
                ...
                if (missing & SD_BUS_CREDS_SUPPLEMENTARY_GIDS)
                {
                    ...
                    if (p)
                    {
                        ...
                        for (;;)
                        {
                            ...
                            if (!GREEDY_REALLOC(c->supplementary_gids,
                                                allocated,
                                                c->n_supplementary_gids + 1))
                                return -ENOMEM;

                            c->supplementary_gids[c->n_supplementary_gids++] =
                                (gid_t)g;
                            p += n;
                        }
    ...
    if (tid > 0 && (missing & SD_BUS_CREDS_TID_COMM))
    {
        ...
        if (asprintf(&p, "/proc/" PID_FMT "/task/" PID_FMT "/comm", pid, tid) <
            0)
            return -ENOMEM;
    ...
    if (missing &
        (SD_BUS_CREDS_CGROUP | SD_BUS_CREDS_UNIT | SD_BUS_CREDS_USER_UNIT |
         SD_BUS_CREDS_SLICE | SD_BUS_CREDS_USER_SLICE | SD_BUS_CREDS_SESSION |
         SD_BUS_CREDS_OWNER_UID))
    {
        ...
        if (c->cgroup)
            c->mask |=
                missing & (SD_BUS_CREDS_CGROUP | SD_BUS_CREDS_UNIT |
...
int bus_creds_extend_by_pid(sd_bus_creds* c, uint64_t mask, sd_bus_creds** ret)
{
    ...
    if (c->mask & mask & SD_BUS_CREDS_SUPPLEMENTARY_GIDS)
    {
        ...
        {
            n->supplementary_gids =
                newdup(gid_t, c->supplementary_gids, c->n_supplementary_gids);
    ...
    if (c->mask & mask &
        (SD_BUS_CREDS_CGROUP | SD_BUS_CREDS_SESSION | SD_BUS_CREDS_UNIT |
         SD_BUS_CREDS_USER_UNIT | SD_BUS_CREDS_SLICE | SD_BUS_CREDS_USER_SLICE |
         SD_BUS_CREDS_OWNER_UID))
    {
        assert(c->cgroup);
        ...
        n->mask |= mask & (SD_BUS_CREDS_CGROUP | SD_BUS_CREDS_SESSION |
                           SD_BUS_CREDS_UNIT | SD_BUS_CREDS_USER_UNIT |
                           SD_BUS_CREDS_SLICE | SD_BUS_CREDS_USER_SLICE |
                           SD_BUS_CREDS_OWNER_UID);
    }
         ...
         SD_BUS_CREDS_INHERITABLE_CAPS | SD_BUS_CREDS_BOUNDING_CAPS))
    {
        ...
        n->capability =
            memdup(c->capability, DIV_ROUND_UP(cap_last_cap(), 32U) * 4 * 4);
        if (!n->capability)
            return -ENOMEM;
