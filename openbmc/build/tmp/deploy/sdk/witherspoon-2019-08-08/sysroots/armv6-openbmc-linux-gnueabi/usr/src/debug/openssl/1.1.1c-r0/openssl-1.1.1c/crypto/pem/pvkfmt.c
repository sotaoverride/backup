...
static int do_i2b(unsigned char** out, EVP_PKEY* pk, int ispub)
{
    ...
    if (bitlen == 0)
        return -1;
    outlen =
        16 + blob_length(bitlen, keyalg == MS_KEYALG_DSS_SIGN ? 1 : 0, ispub);
    if (out == NULL)
        return outlen;
    if (*out)
        p = *out;
...
static int check_bitlen_dsa(DSA* dsa, int ispub, unsigned int* pmagic)
{
    ...
    if ((bitlen & 7) || (BN_num_bits(q) != 160) || (BN_num_bits(g) > bitlen))
        goto badkey;
    if (ispub)
    {
...
static int check_bitlen_rsa(RSA* rsa, int ispub, unsigned int* pmagic)
{
    ...
    else
    {
        ...
        *pmagic = MS_RSA2MAGIC;
        ...
        if ((BN_num_bytes(iqmp) > hnbyte) || (BN_num_bytes(p) > hnbyte) ||
            (BN_num_bytes(q) > hnbyte) || (BN_num_bytes(dmp1) > hnbyte) ||
...
static int derive_pvk_key(unsigned char* key, const unsigned char* salt,
                          unsigned int saltlen, const unsigned char* pass,
                          int passlen)
{
    EVP_MD_CTX* mctx = EVP_MD_CTX_new();
    int rv = 1;
    if (mctx == NULL || !EVP_DigestInit_ex(mctx, EVP_sha1(), NULL) ||
...
static EVP_PKEY* do_PVK_body(const unsigned char** in, unsigned int saltlen,
                             unsigned int keylen, pem_password_cb* cb, void* u)
{
    ...
    if (saltlen)
    {
        ...
        if (!derive_pvk_key(keybuf, p, saltlen, (unsigned char*)psbuf, inlen))
            goto err;
        p += saltlen;
                   ...
                   pem_password_cb* cb, void* u)
{
    ...
    if (enclevel != 0)
    {
        ...
        if (!derive_pvk_key(keybuf, salt, PVK_SALTLEN, (unsigned char*)psbuf,
                            inlen))
            goto error;
        if (enclevel == 1)
            memset(keybuf + 5, 0, 11);
