...
netsnmp_feature_child_of(snmpengine_all, libnetsnmpmibs)
    ...
    netsnmp_feature_child_of(register_snmpEngine_scalars_context,
                             snmpengine_all)
...
void register_snmpEngine_scalars_context(const char* contextName)
{
    register_mib_context(
        "snmpv3/snmpEngine", (struct variable*)snmpEngine_variables,
        sizeof(struct variable2),
        sizeof(snmpEngine_variables) / sizeof(struct variable2),
        snmpEngine_variables_oid,
        sizeof(snmpEngine_variables_oid) / sizeof(oid), DEFAULT_MIB_PRIORITY, 0,
        0, NULL, contextName, -1, 0);
}
...
u_char* var_snmpEngine(struct variable* vp, oid* name, size_t* length,
                       int exact, size_t* var_len, WriteMethod** write_method)
{
    ...
    *write_method =
        (WriteMethod*)0; /* assume it isnt writable for the time being */
    *var_len = sizeof(long_ret); /* assume an integer and change later if not */
    ...
    switch (vp->magic)
    {
        ...
        default:
            DEBUGMSGTL(
                ("snmpd", "unknown sub-id %d in var_snmpEngine\n", vp->magic));
    }
    return NULL;
}
