...
static void __attribute__((__noreturn__)) usage(void)
{
    ...
    fputs(_(" -m, --shmem-id <id>        remove shared memory segment by id\n"),
          out);
    fputs(
        _(" -M, --shmem-key <key>      remove shared memory segment by key\n"),
        out);
...
static int remove_id(int type, int iskey, int id)
{
    ...
    if (ret < 0)
    {
        switch (errno)
        {
            ...
            case EIDRM:
                errmsg =
                    iskey ? _("already removed key") : _("already removed id");
                break;
            default:
                err(EXIT_FAILURE, "%s",
                    iskey ? _("key failed") : _("id failed"));
        }
        warnx("%s (%d)", errmsg, id);
        return 1;
    }
...
static int remove_all(type_id type)
{
    ...
    if (type == SHM || type == ALL)
    {
        ...
        if (maxid < 0)
            errx(EXIT_FAILURE, _("kernel not configured for shared memory"));
    ...
    if (type == SEM || type == ALL)
    {
        ...
        if (maxid < 0)
            errx(EXIT_FAILURE, _("kernel not configured for semaphores"));
        for (id = 0; id <= maxid; id++)
        {
...
#define MSG_INFO 12
    ...
    if (type == MSG || type == ALL)
    {
        ...
        if (maxid < 0)
            errx(EXIT_FAILURE, _("kernel not configured for message queues"));
        for (id = 0; id <= maxid; id++)
        {
...
int main(int argc, char** argv)
{
    ...
    while ((c = getopt_long(argc, argv, "q:m:s:Q:M:S:a::vhV", longopts,
                            NULL)) != -1)
    {
        iskey = 0;
        switch (c)
            ...
            case 'a':
                ...
                if (optarg)
                {
                    if (!strcmp(optarg, "shm"))
                        ...
                        errx(EXIT_FAILURE, _("unknown argument: %s"), optarg);
                }
                else
                {
                    what_all = ALL;
