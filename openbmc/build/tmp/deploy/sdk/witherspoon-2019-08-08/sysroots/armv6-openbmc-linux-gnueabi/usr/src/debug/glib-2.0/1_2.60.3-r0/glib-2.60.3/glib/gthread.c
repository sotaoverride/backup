 ...
 */
/*< private >*/
gpointer g_private_set_alloc0(GPrivate* key, gsize size)
{
...
gboolean(g_once_init_enter)(volatile void* location)
{
    ...
    if (g_atomic_pointer_get(value_location) == NULL)
    {
        if (!g_slist_find(g_once_init_list, (void*)value_location))
        {
            need_init = TRUE;
            g_once_init_list =
                g_slist_prepend(g_once_init_list, (void*)value_location);
        }
        else
            do
                g_cond_wait(&g_once_cond, &g_once_mutex);
 ...
 * Since: 2.32
 */
GThread* g_thread_try_new(const gchar* name, GThreadFunc func, gpointer data,
                          GError** error)
{
    return g_thread_new_internal(name, g_thread_proxy, func, data, 0, error);
}

GThread* g_thread_new_internal(const gchar* name, GThreadFunc proxy,
                               GThreadFunc func, gpointer data,
                               gsize stack_size, GError** error)
{
    g_return_val_if_fail(func != NULL, NULL);

    return (GThread*)g_system_thread_new(proxy, stack_size, name, func, data,
                                         error);
}
...
guint g_get_num_processors(void)
{
    ...
    if (GetProcessAffinityMask(GetCurrentProcess(), &process_cpus,
                               &system_cpus))
    {
