...
static FcBool _FcConfigParse(FcConfig* config, const FcChar8* name,
                             FcBool complain, FcBool load);
...
static FcExpr* FcExprCreateDouble(FcConfig* config, double d)
{
    FcExpr* e = FcConfigAllocExpr(config);
    if (e)
    {
...
static FcExpr* FcExprCreateString(FcConfig* config, const FcChar8* s)
{
    FcExpr* e = FcConfigAllocExpr(config);
    if (e)
    {
...
static FcExprMatrix* FcExprMatrixCopyShallow(const FcExprMatrix* matrix)
{
...
static void FcExprMatrixFreeShallow(FcExprMatrix* m)
{
    if (!m)
        return;
...
static FcExpr* FcExprCreateMatrix(FcConfig* config, const FcExprMatrix* matrix)
{
    FcExpr* e = FcConfigAllocExpr(config);
    if (e)
    {
...
static FcExpr* FcExprCreateRange(FcConfig* config, FcRange* range)
{
    FcExpr* e = FcConfigAllocExpr(config);
    if (e)
    {
...
static FcExpr* FcExprCreateBool(FcConfig* config, FcBool b)
{
    FcExpr* e = FcConfigAllocExpr(config);
    if (e)
    {
...
static FcExpr* FcExprCreateCharSet(FcConfig* config, FcCharSet* charset)
{
    FcExpr* e = FcConfigAllocExpr(config);
    if (e)
    {
...
static FcExpr* FcExprCreateLangSet(FcConfig* config, FcLangSet* langset)
{
    FcExpr* e = FcConfigAllocExpr(config);
    if (e)
    {
...
static FcExpr* FcExprCreateName(FcConfig* config, FcExprName name)
{
    FcExpr* e = FcConfigAllocExpr(config);
    if (e)
    {
...
static FcExpr* FcExprCreateConst(FcConfig* config, const FcChar8* constant)
{
    FcExpr* e = FcConfigAllocExpr(config);
    if (e)
    {
...
static FcExpr* FcExprCreateOp(FcConfig* config, FcExpr* left, FcOp op,
                              FcExpr* right)
{
    FcExpr* e = FcConfigAllocExpr(config);
    if (e)
...
static const char* fcElementIgnoreName[16] = {"its:", NULL};
...
static FcElement FcElementMap(const XML_Char* name)
{
    ...
    for (i = 0; fcElementIgnoreName[i] != NULL; i++)
        if (!strncmp((char*)name, fcElementIgnoreName[i],
                     strlen(fcElementIgnoreName[i])))
...
static void FcConfigMessage(FcConfigParse* parse, FcConfigSeverity severe,
                            const char* fmt, ...)
{
    ...
    {
        if (parse->name)
            fprintf(stderr, "Fontconfig %s: \"%s\", line %d: ", s, parse->name,
...
static FcExpr* FcPopExpr(FcConfigParse* parse);

static const char* FcTypeName(FcType type)
{
    switch (type)
    {
        case FcTypeVoid:
...
static void FcTypecheckValue(FcConfigParse* parse, FcType value, FcType type)
{
...
static void FcTypecheckExpr(FcConfigParse* parse, FcExpr* expr, FcType type)
{
        ...
        case FcOpConst:
            ...
            if (c)
            {
                ...
                FcConfigMessage(parse, FcSevereWarning,
                                "invalid constant used : %s", expr->u.constant);
            break;
        case FcOpQuest:
...
static FcTest* FcTestCreate(FcConfigParse* parse, FcMatchKind kind, FcQual qual,
                            const FcChar8* field, unsigned int compare,
                            FcExpr* expr)
{
...
static FcEdit* FcEditCreate(FcConfigParse* parse, FcObject object, FcOp op,
                            FcExpr* expr, FcValueBinding binding)
{
...
static FcRule* FcRuleCreate(FcRuleType type, void* p)
{
...
static FcVStack* FcVStackCreateAndPush(FcConfigParse* parse)
{
    FcVStack* new;

    if (parse->vstack_static_used <
        sizeof(parse->vstack_static) / sizeof(parse->vstack_static[0]))
...
static FcBool FcVStackPushString(FcConfigParse* parse, FcVStackTag tag,
                                 FcChar8* string)
{
...
static FcBool FcVStackPushInteger(FcConfigParse* parse, int integer)
{
    FcVStack* vstack = FcVStackCreateAndPush(parse);
    if (!vstack)
        return FcFalse;
...
static FcBool FcVStackPushDouble(FcConfigParse* parse, double _double)
{
    FcVStack* vstack = FcVStackCreateAndPush(parse);
    if (!vstack)
        return FcFalse;
...
static FcBool FcVStackPushMatrix(FcConfigParse* parse, FcExprMatrix* matrix)
{
    FcVStack* vstack;
...
static FcBool FcVStackPushRange(FcConfigParse* parse, FcRange* range)
{
    FcVStack* vstack = FcVStackCreateAndPush(parse);
    if (!vstack)
        return FcFalse;
...
static FcBool FcVStackPushBool(FcConfigParse* parse, FcBool bool_)
{
    FcVStack* vstack = FcVStackCreateAndPush(parse);
    if (!vstack)
        return FcFalse;
...
static FcBool FcVStackPushCharSet(FcConfigParse* parse, FcCharSet* charset)
{
    FcVStack* vstack;
    if (!charset)
        return FcFalse;
...
static FcBool FcVStackPushLangSet(FcConfigParse* parse, FcLangSet* langset)
{
    FcVStack* vstack;
    if (!langset)
        return FcFalse;
...
static FcBool FcVStackPushName(FcConfigParse* parse, FcMatchKind kind,
                               FcObject object)
{
    FcVStack* vstack = FcVStackCreateAndPush(parse);
    if (!vstack)
...
static FcBool FcVStackPushTest(FcConfigParse* parse, FcTest* test)
{
    FcVStack* vstack = FcVStackCreateAndPush(parse);
    if (!vstack)
        return FcFalse;
...
static FcBool FcVStackPushExpr(FcConfigParse* parse, FcVStackTag tag,
                               FcExpr* expr)
{
    FcVStack* vstack = FcVStackCreateAndPush(parse);
    if (!vstack)
...
static FcBool FcVStackPushEdit(FcConfigParse* parse, FcEdit* edit)
{
    FcVStack* vstack = FcVStackCreateAndPush(parse);
    if (!vstack)
        return FcFalse;
...
static FcVStack* FcVStackFetch(FcConfigParse* parse, int off)
{
    FcVStack* vstack;
...
static FcVStack* FcVStackPeek(FcConfigParse* parse)
{
...
static void FcVStackPopAndDestroy(FcConfigParse* parse)
{
    FcVStack* vstack = parse->vstack;
...
static void FcVStackClear(FcConfigParse* parse)
{
    while (FcVStackPeek(parse))
        FcVStackPopAndDestroy(parse);
}

static int FcVStackElements(FcConfigParse* parse)
{
    int h = 0;
    FcVStack* vstack = parse->vstack;
...
static FcChar8** FcConfigSaveAttr(const XML_Char** attr, FcChar8** buf,
                                  int size_bytes)
{
    int slen;
    int i;
...
static FcBool FcPStackPush(FcConfigParse* parse, FcElement element,
                           const XML_Char** attr)
{
    FcPStack* new;

    if (parse->pstack_static_used <
        sizeof(parse->pstack_static) / sizeof(parse->pstack_static[0]))
    ...
    new->element = element;
    new->attr = FcConfigSaveAttr(attr, new->attr_buf_static,
                                 sizeof(new->attr_buf_static));
    FcStrBufInit(&new->str, 0, 0);
    parse->pstack = new;
    return FcTrue;
}

static FcBool FcPStackPop(FcConfigParse* parse)
{
    FcPStack* old;
    ...
    if (parse->pstack->element != FcElementNone && parse->pstack->attr)
    {
        ...
        while (*attrs)
        {
            if (attrs[0][0])
            {
                FcConfigMessage(parse, FcSevereError, "invalid attribute '%s'",
                                attrs[0]);
            }
            attrs += 2;
        }
...
static FcBool FcConfigParseInit(FcConfigParse* parse, const FcChar8* name,
                                FcConfig* config, XML_Parser parser,
                                FcBool enabled)
{
    parse->pstack = 0;
...
static void FcConfigCleanup(FcConfigParse* parse)
{
    while (parse->pstack)
        FcPStackPop(parse);
...
static const FcChar8* FcConfigGetAttribute(FcConfigParse* parse,
                                           const char* attr)
{
    FcChar8** attrs;
    if (!parse->pstack)
...
static void FcStartElement(void* userData, const XML_Char* name,
                           const XML_Char** attr)
{
...
static void FcParseInt(FcConfigParse* parse)
{
    FcChar8 *s, *end;
    int l;
...
static double FcStrtod(char* s, char** end)
{
#ifndef __BIONIC__
    ...
    if ((dot = strchr(s, 0x2e)) &&
        (decimal_point[0] != 0x2e || decimal_point[1] != 0))
    {
        char buf[128];
        int slen = strlen(s);
...
static void FcParseDouble(FcConfigParse* parse)
{
    FcChar8 *s, *end;
    double d;
...
static void FcParseString(FcConfigParse* parse, FcVStackTag tag)
{
    FcChar8* s;
...
static void FcParseName(FcConfigParse* parse)
{
    ...
    else
    {
        ...
        {
            FcConfigMessage(parse, FcSevereWarning,
                            "invalid name target \"%s\"", kind_string);
            return;
        }
    }
...
static void FcParseMatrix(FcConfigParse* parse)
{
    FcExprMatrix m;
...
static void FcParseRange(FcConfigParse* parse)
{
    FcVStack* vstack;
    FcRange* r;
    FcChar32 n[2] = {0, 0};
...
static void FcParseBool(FcConfigParse* parse)
{
    FcChar8* s;
...
static void FcParseCharSet(FcConfigParse* parse)
{
    FcVStack* vstack;
...
static void FcParseLangSet(FcConfigParse* parse)
{
    FcVStack* vstack;
    ...
    while ((vstack = FcVStackPeek(parse)))
    {
        switch ((int)vstack->tag)
        {
            case FcVStackString:
                ...
                {
                    FcConfigMessage(parse, FcSevereWarning,
                                    "invalid langset: %s", vstack->u.string);
                }
                else
                    n++;
                break;
...
static FcBool FcConfigLexBinding(FcConfigParse* parse,
                                 ...
                                 FcValueBinding* binding_ret)
{
    ...
    else
    {
        ...
        {
            FcConfigMessage(parse, FcSevereWarning, "invalid binding \"%s\"",
                            binding_string);
            return FcFalse;
        }
    }
...
static void FcParseFamilies(FcConfigParse* parse, FcVStackTag tag)
{
    FcVStack* vstack;
...
static void FcParseFamily(FcConfigParse* parse)
{
    FcChar8* s;
    FcExpr* expr;
...
static void FcParseAlias(FcConfigParse* parse)
{
    ...
    if (!FcConfigLexBinding(parse, FcConfigGetAttribute(parse, "binding"),
                            &binding))
        return;
    while ((vstack = FcVStackPeek(parse)))
    {
        switch ((int)vstack->tag)
        {
            case FcVStackFamily:
                ...
                {
                    FcConfigMessage(parse, FcSevereWarning,
                                    "Having multiple <family> in <alias> isn't "
                                    "supported and may not work as expected");
                    new = FcExprCreateOp(parse->config, vstack->u.expr,
                                         FcOpComma, family);
                    if (!new)
                        FcConfigMessage(parse, FcSevereError, "out of memory");
                    else
    ...
    {
        FcTest* t =
            FcTestCreate(parse, FcMatchPattern, FcQualAny, (FcChar8*)FC_FAMILY,
                         FC_OP(FcOpEqual, FcOpFlagIgnoreBlanks), family);
        if (rule)
        {
    ...
    {
        edit =
            FcEditCreate(parse, FC_FAMILY_OBJECT, FcOpPrepend, prefer, binding);
        if (!edit)
            FcExprDestroy(prefer);
        else
        {
    ...
    {
        edit =
            FcEditCreate(parse, FC_FAMILY_OBJECT, FcOpAppend, accept, binding);
        if (!edit)
            FcExprDestroy(accept);
        else
        {
    ...
    {
        edit =
            FcEditCreate(parse, FC_FAMILY_OBJECT, FcOpAppendLast, def, binding);
        if (!edit)
            FcExprDestroy(def);
        else
        {
...
static void FcParseDescription(FcConfigParse* parse)
{
 ...
 * operand.
 */
static FcExpr* FcPopBinary(FcConfigParse* parse, FcOp op)
{
    FcExpr *left, *expr = 0, *new;
...
static void FcParseBinary(FcConfigParse* parse, FcOp op)
{
...
static FcExpr* FcPopUnary(FcConfigParse* parse, FcOp op)
{
...
static void FcParseUnary(FcConfigParse* parse, FcOp op)
{
...
static void FcParseCacheDir(FcConfigParse* parse)
{
    const FcChar8* attr;
    ...
    else if (strcmp((const char*)data, "LOCAL_APPDATA_FONTCONFIG_CACHE") == 0)
    {
        ...
        if (!(pSHGetFolderPathA &&
              SUCCEEDED(pSHGetFolderPathA(NULL, /* CSIDL_LOCAL_APPDATA */ 28,
                                          NULL, 0, szFPath))))
        {
    ...
    if (strlen((char*)data) == 0)
        FcConfigMessage(parse, FcSevereWarning,
                        "empty cache directory name ignored");
    else if (!parse->scanOnly && (!FcStrUsesHome(data) || FcConfigHome()))
    {
        if (!FcConfigAddCacheDir(parse->config, data))
            FcConfigMessage(parse, FcSevereError,
                            "out of memory; cannot add cache directory %s",
                            data);
    }
    FcStrBufDestroy(&parse->pstack->str);
...
static void FcParseInclude(FcConfigParse* parse)
{
    ...
    FcRuleSetAddDescription(parse->ruleset, ruleset->domain,
                            ruleset->description);
    ...
    else
    {
        ...
        if (deprecated == FcTrue && filename != NULL && userdir != NULL &&
            !FcFileIsLink(filename))
        {
            if (FcFileIsDir(filename))
            {
                    ...
                    symlink((const char*)userdir, (const char*)filename) != 0)
                {
                    ...
                    {
                        FcConfigMessage(
                            parse, FcSevereWarning,
                            "reading configurations from %s is deprecated. "
                            "please move it to %s manually",
                            s, userdir);
                        warn_confd = FcTrue;
                    }
                }
...
static FcOp FcConfigLexOp(const FcChar8* op, const FcOpMap* map, int nmap)
{
    int i;
...
static const FcOpMap fcCompareOps[] = {
    {"eq", FcOpEqual},          {"not_eq", FcOpNotEqual},
    {"less", FcOpLess},         {"less_eq", FcOpLessEqual},
    {"more", FcOpMore},         {"more_eq", FcOpMoreEqual},
    {"contains", FcOpContains}, {"not_contains", FcOpNotContains}};

#define NUM_COMPARE_OPS (int)(sizeof fcCompareOps / sizeof fcCompareOps[0])

static FcOp FcConfigLexCompare(const FcChar8* compare)
{
    return FcConfigLexOp(compare, fcCompareOps, NUM_COMPARE_OPS);
}

static void FcParseTest(FcConfigParse* parse)
{
    const FcChar8* kind_string;
    FcMatchKind kind;
    ...
    else
    {
        ...
        {
            FcConfigMessage(parse, FcSevereWarning, "invalid test qual \"%s\"",
                            qual_string);
            return;
        }
    }
    ...
    {
        FcConfigMessage(parse, FcSevereWarning,
                        "Having multiple values in <test> isn't supported and "
...
static const FcOpMap fcModeOps[] = {
    {"assign", FcOpAssign},   {"assign_replace", FcOpAssignReplace},
    {"prepend", FcOpPrepend}, {"prepend_first", FcOpPrependFirst},
    {"append", FcOpAppend},   {"append_last", FcOpAppendLast},
    {"delete", FcOpDelete},   {"delete_all", FcOpDeleteAll},
};

#define NUM_MODE_OPS (int)(sizeof fcModeOps / sizeof fcModeOps[0])

static FcOp FcConfigLexMode(const FcChar8* mode)
{
    return FcConfigLexOp(mode, fcModeOps, NUM_MODE_OPS);
}

static void FcParseEdit(FcConfigParse* parse)
{
    const FcChar8* name;
    ...
    else
    {
        ...
        if (mode == FcOpInvalid)
        {
            FcConfigMessage(parse, FcSevereWarning, "invalid edit mode \"%s\"",
                            mode_string);
            return;
        }
    }
    ...
    expr = FcPopBinary(parse, FcOpComma);
    if ((mode == FcOpDelete || mode == FcOpDeleteAll) && expr != NULL)
    {
        FcConfigMessage(
            parse, FcSevereWarning,
            "Expression doesn't take any effects for delete and delete_all");
        FcExprDestroy(expr);
        expr = NULL;
    }
    edit =
        FcEditCreate(parse, FcObjectFromName((char*)name), mode, expr, binding);
    if (!edit)
    {
        FcConfigMessage(parse, FcSevereError, "out of memory");
        FcExprDestroy(expr);
...
static void FcParseMatch(FcConfigParse* parse)
{
    ...
    while ((vstack = FcVStackPeek(parse)))
    {
        switch ((int)vstack->tag)
        {
            ...
            case FcVStackEdit:
                if (kind == FcMatchScan &&
                    vstack->u.edit->object > FC_MAX_BASE_OBJECT)
                {
                    FcConfigMessage(parse, FcSevereError,
                                    "<match target=\"scan\"> cannot edit "
    ...
    {
        FcConfigMessage(parse, FcSevereWarning,
                        "No <test> nor <edit> elements in <match>");
        return;
    }
...
static void FcParseAcceptRejectFont(FcConfigParse* parse, FcElement element)
{
    ...
    while ((vstack = FcVStackPeek(parse)))
    {
        switch ((int)vstack->tag)
        {
            case FcVStackGlob:
                if (!parse->scanOnly &&
                    !FcConfigGlobAdd(parse->config, vstack->u.string,
            ...
            case FcVStackPattern:
                if (!parse->scanOnly &&
                    !FcConfigPatternsAdd(parse->config, vstack->u.pattern,
                                         element == FcElementAcceptfont))
                {
                    FcConfigMessage(parse, FcSevereError, "out of memory");
                }
...
static FcValue FcPopValue(FcConfigParse* parse)
{
    ...
    switch ((int)vstack->tag)
    {
        ...
        default:
            FcConfigMessage(parse, FcSevereWarning,
                            "unknown pattern element %d", vstack->tag);
            break;
    }
    FcVStackPopAndDestroy(parse);
...
static void FcEndElement(void* userData, const XML_Char* name FC_UNUSED)
{
    FcConfigParse* parse = userData;
    FcChar8* data;
...
static void FcStartDoctypeDecl(void* userData, const XML_Char* doctypeName,
                               ...
                               int has_internal_subset FC_UNUSED)
{
    ...
    if (strcmp((char*)doctypeName, "fontconfig") != 0)
        FcConfigMessage(parse, FcSevereError, "invalid doctype \"%s\"",
                        doctypeName);
}

#ifdef ENABLE_LIBXML2

static void FcInternalSubsetDecl(void* userData, const XML_Char* doctypeName,
                                 const XML_Char* sysid, const XML_Char* pubid)
{
    FcStartDoctypeDecl(userData, doctypeName, sysid, pubid, 1);
}

static void FcExternalSubsetDecl(void* userData, const XML_Char* doctypeName,
                                 const XML_Char* sysid, const XML_Char* pubid)
{
...
static void FcEndDoctypeDecl(void* userData FC_UNUSED)
{
}

#endif /* ENABLE_LIBXML2 */

static int FcSortCmpStr(const void* a, const void* b)
{
...
static FcBool FcConfigParseAndLoadDir(FcConfig* config, const FcChar8* name,
                                      const FcChar8* dir, FcBool complain,
                                      FcBool load)
{
    DIR* d;
    struct dirent* e;
...
#define TAIL_LEN 5
        ...
        if ('0' <= e->d_name[0] && e->d_name[0] <= '9' &&
            (d_len = strlen(e->d_name)) < FC_MAX_FILE_LEN && d_len > TAIL_LEN &&
...
static FcBool FcConfigParseAndLoadFromMemoryInternal(FcConfig* config,
...
bail1:
    if (error && complain)
    {
        FcConfigMessage(0, FcSevereError, "Cannot %s config file from %s",
                        load ? "load" : "scan", filename);
        return FcFalse;
    }
    if (FcDebug() & FC_DBG_CONFIG)
                             ...
                             FcBool complain, FcBool load)
{
    ...
    if (!pGetSystemWindowsDirectory)
    {
        ...
        if (!(pGetSystemWindowsDirectory = (pfnGetSystemWindowsDirectory)
                  GetProcAddress(hk32, "GetSystemWindowsDirectoryA")))
    ...
    if (!pSHGetFolderPathA)
    {
        ...
        if (hSh)
            pSHGetFolderPathA =
                (pfnSHGetFolderPathA)GetProcAddress(hSh, "SHGetFolderPathA");
    }
#endif
    ...
    {
        ret =
            FcConfigParseAndLoadDir(config, name, realfilename, complain, load);
        FcStrFree(filename);
        FcStrFree(realfilename);
        return ret;
    }
    ...
    do
    {
        ...
        if (len < 0)
        {
            ...
            ebuf[0] = 0;
#endif
            FcConfigMessage(0, FcSevereError,
                            "failed reading config file: %s: %s (errno %d)",
                            realfilename, ebuf, errno_);
            close(fd);
            goto bail1;
        }
    ...
    ret = FcConfigParseAndLoadFromMemoryInternal(
        config, filename, FcStrBufDoneStatic(&sbuf), complain, load);
...
bail0:
    ...
    {
        if (name)
            FcConfigMessage(0, FcSevereError, "Cannot %s config file \"%s\"",
                            load ? "load" : "scan", name);
        else
            FcConfigMessage(0, FcSevereError, "Cannot %s default config file",
                            load ? "load" : "scan");
        return FcFalse;
    }
    return FcTrue;
...
FcBool FcConfigParseAndLoadFromMemory(FcConfig* config, const FcChar8* buffer,
                                      FcBool complain)
{
    return FcConfigParseAndLoadFromMemoryInternal(
        config, (const FcChar8*)"memory", buffer, complain, FcTrue);
}
