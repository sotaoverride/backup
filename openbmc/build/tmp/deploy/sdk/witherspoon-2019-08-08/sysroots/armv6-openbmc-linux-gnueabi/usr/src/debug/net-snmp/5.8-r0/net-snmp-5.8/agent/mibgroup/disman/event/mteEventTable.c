...
void init_mteEventTable(void)
{
    ...
    reg = netsnmp_create_handler_registration(
        ...
        mteEventTable_oid_len, HANDLER_CAN_RWRITE);
#else  /* !NETSNMP_NO_WRITE_SUPPORT */
    reg = netsnmp_create_handler_registration(
        "mteEventTable", mteEventTable_handler, mteEventTable_oid,
        mteEventTable_oid_len, HANDLER_CAN_RONLY);
#endif /* !NETSNMP_NO_WRITE_SUPPORT */
...
int mteEventTable_handler(netsnmp_mib_handler* handler,
                          ...
                          netsnmp_request_info* requests)
{
    ...
    DEBUGMSGTL(
        ("disman:event:mib", "Event Table handler (%d)\n", reqinfo->mode));
            ...
            for (request = requests; request; request = request->next)
            {
                ...
                switch (tinfo->colnum)
                {
                    case COLUMN_MTEEVENTCOMMENT:
                         ...
                         *   (No good reason for this, but that's what the MIB
                         * says!)
                         */
                    ...
                    case COLUMN_MTEEVENTENABLED:
                         ...
                         * Fortunately this ludicrous decision has since been
                         * corrected.
                         */
                        break;
                    ...
                    case COLUMN_MTEEVENTENTRYSTATUS:
                        switch (*request->requestvb->val.integer)
                        {
                            ...
                            case RS_CREATEANDWAIT:
                                ...
                                row =
                                    mteEvent_createEntry(mteOwner, mteEName, 0);
                                if (!row)
                                {
                                    netsnmp_set_request_error(
                                        reqinfo, request,
