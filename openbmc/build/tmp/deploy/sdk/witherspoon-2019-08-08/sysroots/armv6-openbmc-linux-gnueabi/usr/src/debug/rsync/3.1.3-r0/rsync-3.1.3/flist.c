...
void init_flist(void)
{
    ...
    show_filelist_progress =
        INFO_GTE(FLIST, 1) && xfer_dirs && !am_server && !inc_recurse;
}
...
static void flist_expand(struct file_list* flist, int extra)
{
    ...
    if (DEBUG_GTE(FLIST, 1) && flist->malloced != FLIST_START)
    {
        rprintf(FCLIENT,
                ...
                who_am_i(), big_num(sizeof flist->files[0] * flist->malloced),
                            ...
                            int ndx, int first_ndx)
{
    ...
    else if (S_ISDIR(file->mode))
    {
        ...
        if (protocol_version >= 30)
        {
            ...
            xflags =
                file->flags & FLAG_TOP_DIR; /* FLAG_TOP_DIR == XMIT_TOP_DIR */
    }
    else
    {
            ...
            if (DEBUG_GTE(HLINK, 1))
            {
                ...
                else if (DEBUG_GTE(HLINK, 3))
                {
                    rprintf(FINFO, "[%s] dev:inode for #%d is %s:%s\n",
                            who_am_i(), first_ndx + ndx, big_num(tmp_dev),
                            big_num(tmp_ino));
                }
            }
        }
    ...
    for (l1 = 0; lastname[l1] && (fname[l1] == lastname[l1]) && (l1 < 255);
         l1++)
    {
    }
    l2 = strlen(fname + l1);
...
static struct file_struct* recv_file_entry(int f, struct file_list* flist,
                                           int xflags)
{
    ...
    if (l2 >= MAXPATHLEN - l1)
    {
        rprintf(FERROR, "overflow: xflags=0x%x l1=%d l2=%d lastname=%s [%s]\n",
    ...
    if (ic_recv != (iconv_t)-1)
    {
        ...
        {
            io_error |= IOERR_GENERAL;
            rprintf(FERROR_UTF8, "[%s] cannot convert filename: %s (%s)\n",
    ...
    if (*thisname && (clean_fname(thisname, CFN_REFUSE_DOT_DOT_DIRS) < 0 ||
                      (!relative_paths && *thisname == '/')))
    {
        ...
        BITS_SETnUNSET(xflags, XMIT_HLINKED, XMIT_HLINK_FIRST))
    {
        ...
        {
            rprintf(FERROR, "hard-link reference out of range: %d (%d)\n",
                    first_hlink_ndx, flist->ndx_start + flist->used);
            exit_cleanup(RERR_PROTOCOL);
        }
        if (DEBUG_GTE(HLINK, 1))
        {
            rprintf(FINFO, "[%s] #%d hard-links #%d (%sabbrev)\n", who_am_i(),
                    flist->used + flist->ndx_start, first_hlink_ndx,
                    first_hlink_ndx >= flist->ndx_start ? "" : "un");
        }
        if (first_hlink_ndx >= flist->ndx_start)
        {
            struct file_struct* first =
                flist->files[first_hlink_ndx - flist->ndx_start];
    ...
    if (S_ISDIR(mode))
    {
        ...
        else if (in_del_hier)
        {
            if (!relative_paths || !del_hier_name_len ||
                (l1 >= del_hier_name_len && lastname[del_hier_name_len] == '/'))
                file->flags |= FLAG_CONTENT_DIR;
            else
                in_del_hier = 0;
        }
    ...
    else if (readlink_stat(thisname, &st, linkname) != 0)
    {
        ...
        if (save_errno == ENOENT)
        {
            ...
            if ((copy_links || copy_unsafe_links || copy_dirlinks) &&
                x_lstat(thisname, &st, NULL) == 0 && S_ISLNK(st.st_mode))
            {
                io_error |= IOERR_GENERAL;
            ...
            {
                enum logcode c =
                    am_daemon && protocol_version < 28 ? FERROR : FWARNING;
...
skip_filters:
    ...
    if (flist)
    {
        if (flist->prev && S_ISDIR(st.st_mode) && flags & FLAG_DIVERT_DIRS)
        {
    ...
    {
        if (protocol_version >= 28 ? (!S_ISDIR(st.st_mode) && st.st_nlink > 1)
                                   : S_ISREG(st.st_mode))
        {
                                          ...
                                          int flags, int filter_level)
{
    ...
    if (f >= 0)
    {
        ...
        if (ic_send != (iconv_t)-1)
        {
            ...
            convert_error:
                io_error |= IOERR_GENERAL;
                rprintf(FERROR_XFER, "[%s] cannot convert filename: %s (%s)\n",
...
static void send_if_directory(int f, struct file_list* flist,
                              struct file_struct* file, char* fbuf,
                              unsigned int ol, int flags)
{
    char is_dot_dir = fbuf[ol - 1] == '.' && (ol == 1 || fbuf[ol - 2] == '/');

    if (S_ISDIR(file->mode) && !(file->flags & FLAG_MOUNT_DIR) &&
        f_name(file, fbuf))
    {
        void* save_filters;
...
static void fsort(struct file_struct** fp, size_t num)
{
    ...
    {
        struct file_struct** tmp =
            new_array(struct file_struct*, (num + 1) / 2);
                             ...
                             int dir_cnt)
{
    ...
    int32* dp = NULL;
    int32* parent_dp =
        parent_ndx < 0 ? NULL : F_DIR_NODE_P(dir_flist->sorted[parent_ndx]);
...
static void interpret_stat_error(const char* fname, int is_dir)
{
    ...
    {
        io_error |= IOERR_GENERAL;
        rsyserr(FERROR_XFER, errno, "link_stat %s failed", full_fname(fname));
    }
}
                           ...
                           int flags)
{
    ...
    for (errno = 0, di = readdir(d); di; errno = 0, di = readdir(d))
    {
        ...
        {
            io_error |= IOERR_GENERAL;
            rprintf(FERROR_XFER, "cannot send file with empty name in %s\n",
                    full_fname(fbuf));
            continue;
        }
...
static void send_implied_dirs(int f, struct file_list* flist, char* fname,
                              char* start, char* limit, int flags,
                              char name_type)
{
    ...
    len = strlen(limit + 1);
    memcpy(&relname_list, F_DIR_RELNAMES_P(lastpath_struct),
           sizeof relname_list);
    if (!relname_list)
    {
        if (!(relname_list = new0(item_list)))
            out_of_memory("send_implied_dirs");
        memcpy(F_DIR_RELNAMES_P(lastpath_struct), &relname_list,
               sizeof relname_list);
    }
...
static NORETURN void fatal_unsafe_io_error(void)
{
    ...
    rprintf(FERROR_XFER,
            "FATAL I/O ERROR: dying to avoid a --delete-%s issue with a "
            "pre-3.0.7 receiver.\n",
...
static void send1extra(int f, struct file_struct* file, struct file_list* flist)
{
    ...
    for (j = 0; j < relname_list->count; j++)
    {
        ...
        if (slash)
        {
            send_implied_dirs(f, flist, fbuf, fbuf + dlen + 1, slash, flags,
                              name_type);
            continue;
        }
        ...
        if (name_type != NORMAL_NAME)
        {
            ...
            send_file_name(f, flist, fbuf, &st, FLAG_TOP_DIR | flags,
                           ALL_FILTERS);
        }
        else
            send_file_name(f, flist, fbuf, NULL, FLAG_TOP_DIR | flags,
                           ALL_FILTERS);
    }
...
struct file_list* send_file_list(int f, int argc, char* argv[])
{
    ...
    while (1)
    {
        ...
        else
        {
            ...
            {
                if ((p[2] == '/' || p[2] == '\0') && (p == fn || p[-1] == '/'))
                {
                    rprintf(FERROR, "found \"..\" dir in relative path: %s\n",
                            fn);
        ...
        if (link_stat(fbuf, &st, copy_dirlinks || name_type != NORMAL_NAME) !=
                0 ||
            (name_type != DOTDIR_NAME &&
             is_excluded(fbuf, S_ISDIR(st.st_mode) != 0, SERVER_FILTERS)) ||
        ...
        if (inc_recurse && relative_paths && *fbuf)
        {
            if ((p = strchr(fbuf + 1, '/')) != NULL)
            {
                ...
                send_implied_dirs(f, flist, fbuf, fbuf, p, flags,
                                  IS_MISSING_FILE(st) ? MISSING_NAME
                                                      : name_type);
                if (fn == p)
                    continue;
            }
...
struct file_list* recv_file_list(int f, int dir_ndx)
{
    ...
    while ((flags = read_byte(f)) != 0)
    {
        ...
        if (inc_recurse)
        {
            ...
            {
                const char* d = dir_ndx >= 0
                                    ? f_name(dir_flist->files[dir_ndx], NULL)
                                    : empty_dir;
    ...
    if (need_unsorted_flist)
    {
        ...
        if (inc_recurse && dir_flist->used > dstart)
        {
            ...
            {
                dir_flist->sorted =
                    realloc_array(dir_flist->sorted, struct file_struct*,
                                  dir_flist->malloced);
...
void recv_additional_file_list(int f)
{
    ...
    else
    {
        ...
        {
            ndx = NDX_FLIST_OFFSET - ndx;
            rprintf(FERROR, "[%s] Invalid dir index: %d (%d - %d)\n",
    ...
    while (low <= high)
    {
        ...
        if (diff == 0)
        {
            if (protocol_version < 29 &&
                S_ISDIR(flist->sorted[mid_up]->mode) != S_ISDIR(f->mode))
                return -1;
            return mid_up;
        }
        if (diff < 0)
...
int flist_find_name(struct file_list* flist, const char* fname,
                    int want_dir_match)
{
    struct
...
struct file_list* flist_new(int flags, char* msg)
{
    ...
    {
        if (!(flist->file_pool =
                  pool_create(SMALL_EXTENT, 0, out_of_memory, POOL_INTERN)))
            out_of_memory(msg);
    }
    else
    {
        ...
        {
            flist->file_pool =
                pool_create(NORMAL_EXTENT, 0, out_of_memory, POOL_INTERN);
            if (!flist->file_pool)
                out_of_memory(msg);
        ...
        if (j >= 0)
        {
            ...
            if (S_ISDIR(file->mode))
            {
                ...
                else
                {
                    ...
                    { /* Make sure we merge our vital flags. */
                        fp->flags |=
                            file->flags & (FLAG_TOP_DIR | FLAG_CONTENT_DIR);
            ...
            if (keep == i)
            {
                if (flist->low == drop)
                {
                    for (j = drop + 1; j < i && !F_IS_ACTIVE(flist->sorted[j]);
                         j++)
                    {
                    }
                    flist->low = j;
...
static void output_flist(struct file_list* flist)
{
    ...
    rprintf(FINFO, "[%s] flist start=%d, used=%d, low=%d, high=%d\n", who,
            flist->ndx_start, flist->used, flist->low, flist->high);
    for (i = 0; i < flist->used; i++)
    {
        file = flist->files[i];
        if ((am_root || am_sender) && uid_ndx)
        {
            snprintf(uidbuf, sizeof uidbuf, " uid=%u", F_OWNER(file));
        }
        else
            *uidbuf = '\0';
        if (gid_ndx)
        {
            ...
            snprintf(gidbuf, sizeof gidbuf, " gid=%s%u%s", pp, F_GROUP(file),
                     pp + 2);
        }
        else
            *gidbuf = '\0';
        ...
        rprintf(FINFO, "[%s] i=%d %s %s%s%s%s mode=0%o len=%s%s%s flags=%x\n",
                who, i + flist->ndx_start, root, dir, slash, name, trail,
                (int)file->mode, comma_num(F_LENGTH(file)), uidbuf, gidbuf,
                file->flags);
    }
}
...
int f_name_has_prefix(const struct file_struct* f1,
