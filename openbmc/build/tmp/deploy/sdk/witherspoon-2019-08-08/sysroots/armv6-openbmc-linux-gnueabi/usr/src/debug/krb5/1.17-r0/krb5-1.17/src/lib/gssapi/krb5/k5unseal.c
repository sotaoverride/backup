...
static OM_uint32 kg_unseal_v1(context, minor_status, ctx, ptr, bodysize,
                              message_buffer, conf_state, qop_state,
                              toktype) krb5_context context;
...
int toktype;
{
    ...
    if (toktype == KG_TOK_SEAL_MSG)
    {
        ...
        if (sealalg != 0xffff)
        {
            ...
            if (ctx->sealalg == SEAL_ALG_MICROSOFT_RC4)
            {
                ...
                code = kg_arcfour_docrypt(enc_key, 0, &bigend_seqnum[0], 4,
                                          ptr + 14 + cksum_len, tmsglen, plain);
    ...
    switch (signalg)
    {
        ...
        case SGN_ALG_3:
            ...
            code =
                krb5_k_make_checksum(context, md5cksum.checksum_type, ctx->seq,
...
OM_uint32 kg_unseal(minor_status, context_handle, input_token_buffer,
                    message_buffer, conf_state, qop_state,
...
int toktype;
{
    ...
    err = g_verify_token_header(ctx->mech_used, &bodysize, &ptr, -1,
                                input_token_buffer->length, vfyflags);
    if (err)
    {
        *minor_status = err;
        return GSS_S_DEFECTIVE_TOKEN;
    ...
    switch (toktype2)
    {
        ...
        case KG2_TOK_DEL_CTX:
            ret = gss_krb5int_unseal_token_v3(
                &ctx->k5_context, minor_status, ctx, ptr, bodysize,
        ...
        case KG_TOK_DEL_CTX:
            ret =
                kg_unseal_v1(ctx->k5_context, minor_status, ctx, ptr, bodysize,
                             message_buffer, conf_state, qop_state, toktype);
            break;
        default:
            *minor_status = (OM_uint32)G_BAD_TOK_HEADER;
...
OM_uint32 KRB5_CALLCONV krb5_gss_unwrap(minor_status, context_handle,
                                        input_message_buffer,
                                        output_message_buffer, conf_state,
                                        qop_state) OM_uint32* minor_status;
gss_ctx_id_t context_handle;
...
gss_qop_t* qop_state;
{
    ...
    rstat = kg_unseal(minor_status, context_handle, input_message_buffer,
                      output_message_buffer, conf_state, qop_state,
                      KG_TOK_WRAP_MSG);
    return (rstat);
}

OM_uint32 KRB5_CALLCONV krb5_gss_verify_mic(minor_status, context_handle,
...
gss_qop_t* qop_state;
{
    ...
    rstat = kg_unseal(minor_status, context_handle, token_buffer,
                      message_buffer, NULL, qop_state, KG_TOK_MIC_MSG);
    return (rstat);
}
