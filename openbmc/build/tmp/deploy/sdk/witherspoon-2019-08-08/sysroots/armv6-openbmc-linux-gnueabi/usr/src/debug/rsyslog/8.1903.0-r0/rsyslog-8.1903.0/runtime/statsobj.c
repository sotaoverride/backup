...
DEFobjStaticHelpers
     ...
     * upon construction. Enqueue always happens at the front (simplifies
     * logic).
     */
 ...
 */
BEGINobjConstruct(
    statsobj) /* be sure to specify the object type also in END macro! */
 ...
 */
static rsRetVal setReadNotifier(statsobj_t* pThis,
                                statsobj_read_notifier_t notifier, void* ctx)
{
    DEFiRet;
 ...
 * is called.
 */
static rsRetVal addManagedCounter(statsobj_t* pThis, const uchar* ctrName,
                                  statsCtrType_t ctrType, int8_t flags,
                                  void* pCtr, ctr_t** entryRef, int8_t linked)
{
    ctr_t* ctr;
    DEFiRet;
...
static rsRetVal addCounter(statsobj_t* pThis, const uchar* ctrName,
                           statsCtrType_t ctrType, int8_t flags, void* pCtr)
{
    ctr_t* ctr;
    DEFiRet;
...
static rsRetVal addCtrForReporting(json_object* to, const uchar* field_name,
                                   intctr_t value)
{
    json_object* v;
    DEFiRet;
    ...
    /*We should migrate libfastjson to support uint64_t in addition to int64_t.
      Although no counter is likely to grow to int64 max-value, this is
      theoritically incorrect (as intctr_t is uint64)*/
    CHKmalloc(v = json_object_new_int64((int64_t)value));

    json_object_object_add(to, (const char*)field_name, v);
finalize_it:
    /* v cannot be NULL in error case, as this would only happen during malloc
     * fail, which itself sets it to NULL -- so not doing cleanup here.
     */
    RETiRet;
}

static rsRetVal addContextForReporting(json_object* to, const uchar* field_name,
                                       const uchar* value)
{
    json_object* v;
    DEFiRet;
...
static rsRetVal getStatsLineCEE(statsobj_t* pThis, cstr_t** ppcstr,
                                const statsFmtType_t fmt,
                                const int8_t bResetCtrs)
{
    ...
    if (fmt == statsFmt_CEE)
        CHKiRet(rsCStrAppendStrWithLen(pcstr,
                                       UCHAR_CONSTANT(CONST_CEE_COOKIE " "),
                                       CONST_LEN_CEE_COOKIE + 1));
    ...
    {
        CHKiRet(addContextForReporting(root, UCHAR_CONSTANT("origin"),
                                       pThis->origin));
    }
    ...
    else
    {
        ...
        json_object_object_add(root, (const char*)pThis->reporting_ns,
    ...
    for (pCtr = pThis->ctrRoot; pCtr != NULL; pCtr = pCtr->next)
    {
        ...
        {
            CHKiRet(
                addCtrForReporting(values, pCtr->name, accumulatedValue(pCtr)));
        }
    ...
    locked = 0;
    CHKiRet(
        rsCStrAppendStr(pcstr, (const uchar*)json_object_to_json_string(root)));
 ...
 */
static rsRetVal getStatsLine(statsobj_t* pThis, cstr_t** ppcstr,
                             int8_t bResetCtrs)
{
    cstr_t* pcstr;
    ctr_t* pCtr;
 ...
 */
static void getSenderStats(rsRetVal (*cb)(void*, const char*), void* usrptr,
                           statsFmtType_t fmt, const int8_t bResetCtrs)
{
...
static rsRetVal getAllStatsLines(rsRetVal (*cb)(void*, const char*),
                                 void* const usrptr, statsFmtType_t fmt,
                                 const int8_t bResetCtrs)
{
    statsobj_t* o;
    cstr_t* cstr = NULL;
...
rsRetVal statsRecordSender(const uchar* sender, unsigned nMsgs, time_t lastSeen)
{
    ...
    if (stat == NULL)
    {
        ...
        {
            LogMsg(0, RS_RET_SENDER_APPEARED, LOG_INFO, "new sender '%s'",
                   stat->sender);
        }
        if (hashtable_insert(stats_senders, (void*)stat->sender, (void*)stat) ==
            0)
        {
    ...
    DBGPRINTF(
        "DDDDD: statsRecordSender: '%s', nmsgs %u [%llu], lastSeen %llu\n",
        sender, nMsgs, (long long unsigned)stat->nMsgs,
        (long long unsigned)lastSeen);
            ...
            if (stat->lastSeen < rqdLast)
            {
                if (glblReportGoneAwaySenders)
                {
                    ...
                    LogMsg(0, RS_RET_SENDER_GONE_AWAY, LOG_WARNING,
                           ...
                           stat->sender, tm.tm_year + 1900, tm.tm_mon + 1,
...
BEGINobjDestruct(statsobj) /* be sure to specify the object type also in END and
                              CODESTART macros! */
    CODESTARTobjDestruct(statsobj) removeFromObjList(pThis);
...
ENDobjDestruct(statsobj)
    ...
    BEGINobjDebugPrint(statsobj) /* be sure to specify the object type also in
                                    END and CODESTART macros! */
    CODESTARTobjDebugPrint(statsobj)
        dbgoprint((obj_t*)pThis,
                  "statsobj object, currently no state info available\n");
ENDobjDebugPrint(statsobj)

    /* queryInterface function
     */
    BEGINobjQueryInterface(statsobj)
        CODESTARTobjQueryInterface(statsobj) if (pIf->ifVersion !=
                                                 statsobjCURR_IF_VERSION)
...
finalize_it : ENDobjQueryInterface(statsobj)

              /* Initialize the statsobj class. Must be called as the very first
               * method before anything else is called inside this class.
               */
              BEGINAbstractObjClassInit(statsobj, 1,
...
OBJSetMethodHandler(objMethod_CONSTRUCTION_FINALIZER,
                    statsobjConstructFinalize);

/* init other data items */
pthread_mutex_init(&mutStats, NULL);
pthread_mutex_init(&mutSenders, NULL);

if ((stats_senders = create_hashtable(100, hash_from_string, key_equals_string,
                                      NULL)) == NULL)
{
