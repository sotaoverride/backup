                    ...
                    switch (str[1])
                    {
                        ...
                        default:
                            BOOST_ASSERT(!"Definition of punctuation_token "
                                          "does not match switch statement");
                    }
                }
                default:
...
struct parser
{
    ...
    subgraph_info& current()
    {
        return subgraphs[current_subgraph_name];
    }
    properties& current_graph_props()
    {
        return r.graph_props[current_subgraph_name];
    }
    subgraph_member_list& current_members()
    {
    ...
    subgraph_name parse_subgraph(const token& first_token)
    {
        ...
        {
            name =
                "___subgraph_" + boost::lexical_cast<std::string>(++sgcounter);
        }
                ...
                switch (peek().type)
                {
                    ...
                    case token::left_paren:
                    {
                        ...
                        if (peek().type != token::comma)
                            error(
                                "Wanted comma between parts of port location");
                        get();
                        if (peek().type != token::identifier)
                        ...
                        if (peek().type != token::right_paren)
                            error("Wanted right parenthesis to close port "
                                  "location");
                        get();
                        goto parse_more;
                    }
                    default:
                        error("Wanted identifier or left parenthesis as start "
                              "of port location");
                }
            }
            default:
    ...
    void parse_edge_stmt(const edge_endpoint& lhs)
    {
        ...
        {
            do_orig_edge(nodes_in_chain[i], nodes_in_chain[i + 1],
                         this_edge_props);
        }
    }
                                ...
                                ::boost::detail::graph::mutate_graph* mg)
{
    ...
    BOOST_ASSERT(root_graph_props_i !=
                 r.graph_props.end()); // Should not happen
...
BOOST_GRAPH_DECL bool read_graphviz_new(const std::string& str,
                                        boost::detail::graph::mutate_graph* mg)
{
