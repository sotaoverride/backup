...
static int init_keyrings(pam_handle_t* pamh, int force)
{
    ...
    if (!force)
    {
         ...
         * keyring */
        session = syscall(__NR_keyctl, KEYCTL_GET_KEYRING_ID,
        ...
        usession = syscall(__NR_keyctl, KEYCTL_GET_KEYRING_ID,
                           KEY_SPEC_USER_SESSION_KEYRING, 0);
    ...
    ret = syscall(__NR_keyctl, KEYCTL_JOIN_SESSION_KEYRING, NULL);
    ...
    ret = syscall(__NR_keyctl, KEYCTL_LINK, KEY_SPEC_USER_KEYRING,
...
static void kill_keyrings(pam_handle_t* pamh)
{
    ...
    if (my_session_keyring > 0)
    {
        ...
        old_uid = geteuid();
        old_gid = getegid();
        debug(pamh, "UID:%d [%d]  GID:%d [%d]", revoke_as_uid, old_uid,
              revoke_as_gid, old_gid);
        ...
        if (revoke_as_uid != old_uid &&
            setresuid(-1, revoke_as_uid, old_uid) < 0)
            error(pamh, "Unable to change UID to %d temporarily\n",
                  revoke_as_uid);

        syscall(__NR_keyctl, KEYCTL_REVOKE, my_session_keyring);
 ...
 */
int pam_sm_close_session(pam_handle_t* pamh, int flags UNUSED, int argc UNUSED,
                         const char** argv UNUSED)
{
    debug(pamh, "CLOSE %d,%d,%d", session_counter, my_session_keyring,
          do_revoke);
