...
static int property_init(PyObject* self, PyObject* args, PyObject* kwds)
{
    ...
    if (!PyArg_ParseTupleAndKeywords(args, kwds, "|OOOO:property",
                                     const_cast<char**>(kwlist), &get, &set,
                                     &del, &doc))
        return -1;
...
static PyGetSetDef instance_getsets[] = {{const_cast<char*>("__dict__"),
                                          instance_get_dict, instance_set_dict,
                                          NULL, 0},
                                         {0, 0, 0, 0, 0}};

static PyMemberDef instance_members[] = {{const_cast<char*>("__weakref__"),
                                          T_OBJECT,
                              ...
                              char const* docstr)
{
    object property((python::detail::new_reference)PyObject_CallFunction(
        (PyObject*)&PyProperty_Type, const_cast<char*>("Osss"), fget.ptr(),
        (char*)NULL, (char*)NULL, docstr));
...
{
    if (PyObject_SetAttrString(this->ptr(), const_cast<char*>(name), x.ptr()) <
        0)
        throw_error_already_set();
}
