...
T ellint_rg_imp(T x, T y, T z, const Policy& pol)
{
    ...
    if (x == z)
    {
        ...
        else
        {
            // x = z, y != 0
            swap(x, y);
            return (x == 0) ? T(sqrt(z) / 2)
                            : T((z * ellint_rc_imp(x, z, pol) + sqrt(x)) / 2);
        }
    }
    else if (y == z)
    {
        if (x == 0)
            ...
            return (y == 0) ? T(sqrt(x) / 2)
                            : T((y * ellint_rc_imp(x, y, pol) + sqrt(x)) / 2);
    }
    else if (y == 0)
    {
        swap(y, z);
    ...
    ellint_rg(T1 x, T2 y, T3 z, const Policy& pol)
{
    ...
    return policies::checked_narrowing_cast<result_type, Policy>(
        detail::ellint_rg_imp(static_cast<value_type>(x),
...
inline typename tools::promote_args<T1, T2, T3>::type ellint_rg(T1 x, T2 y,
                                                                T3 z)
{
