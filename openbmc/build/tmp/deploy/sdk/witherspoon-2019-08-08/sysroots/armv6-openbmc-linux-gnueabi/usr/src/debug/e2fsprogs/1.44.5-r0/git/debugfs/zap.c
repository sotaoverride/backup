...
void do_zap_block(int argc, char* argv[])
{
    ...
    while ((c = getopt(argc, argv, "b:f:l:o:p:")) != EOF)
    {
        switch (c)
        {
            case 'f':
                ...
                break;
            case 'b':
                bit = parse_ulong(optarg, argv[0], "bit", &err);
                if (err)
                    return;
                ...
                break;
            case 'p':
                pattern = parse_ulong(optarg, argv[0], "pattern", &err);
                if (err)
                    return;
                if (pattern >= 256)
                {
                    com_err(argv[0], 0,
                            "The fill pattern must "
                            "be an 8-bit value\n");
                    return;
                }
                break;
            case 'o':
                offset = parse_ulong(optarg, argv[0], "offset", &err);
                if (err)
                    return;
                ...
                break;
            case 'l':
                length = parse_ulong(optarg, argv[0], "length", &err);
                if (err)
                    return;
                break;
            default:
    ...
    print_usage:
        com_err(0, 0,
                "Usage:\tzap_block [-f file] [-o offset] "
                "[-l length] [-p pattern] block_num");
        com_err(0, 0,
                "\tzap_block [-f file] [-b bit] "
                "block_num");
        return;
    }
    ...
    if (file)
    {
        ...
        if (!inode)
            return;
        errcode = ext2fs_bmap2(current_fs, inode, 0, 0, 0, block, 0, &block);
        if (errcode)
        {
    ...
    if (errcode)
    {
        com_err(argv[0], errcode, "while write block %llu\n", block);
        goto errout;
    }
