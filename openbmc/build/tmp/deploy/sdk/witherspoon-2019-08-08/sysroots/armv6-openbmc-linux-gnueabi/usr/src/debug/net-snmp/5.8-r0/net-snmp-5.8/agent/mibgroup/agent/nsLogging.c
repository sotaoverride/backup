        ...
        void init_nsLogging(void)
{
    ...
    netsnmp_register_table_iterator2(
        netsnmp_create_handler_registration(
            "tzLoggingTable", handle_nsLoggingTable, nsLoggingTable_oid,
            OID_LENGTH(nsLoggingTable_oid), HANDLER_CAN_RWRITE),
        iinfo);
}
...
netsnmp_variable_list* get_first_logging_entry(void** loop_context,
                                               void** data_context,
                                               ...
                                               netsnmp_iterator_info* data)
{
    ...
    snmp_set_var_value(index, (u_char*)&temp, sizeof(temp));
    if (logh_head->token)
        snmp_set_var_value(index->next_variable,
                           (const u_char*)logh_head->token,
...
netsnmp_variable_list* get_next_logging_entry(void** loop_context,
                                              void** data_context,
                                              netsnmp_variable_list* index,
                                              netsnmp_iterator_info* data)
{
...
int handle_nsLoggingTable(netsnmp_mib_handler* handler,
        ...
        case MODE_GET:
            for (request = requests; request; request = request->next)
            {
                if (request->processed != 0)
                    continue;
                logh = (netsnmp_log_handler*)netsnmp_extract_iterator_context(
                    request);
                ...
                switch (table_info->colnum)
                {
                    ...
                    case NSLOGGING_STATUS:
                        ...
                        temp = (logh->type ? (logh->enabled ? RS_ACTIVE
                                                            : RS_NOTINSERVICE)
                                           : RS_NOTREADY);
                        snmp_set_var_typed_value(request->requestvb,
                                                 ASN_INTEGER, (u_char*)&temp,
                    ...
                    case NSLOGGING_STATUS:
                        ...
                        switch (*request->requestvb->val.integer)
                        {
                            ...
                            case RS_CREATEANDGO:
                                ...
                                /*
                                 *  Normally, we'd create the row at a later
                                 * stage (probably during the RESERVE2 or ACTION
                                 * passes)
                                 *
                                 *  But we need to check that the values are
                                 *   consistent during the ACTION pass (which is
                                 * the latest that an error can be safely
                                 * handled), so the values all need to be set up
                                 * before this (i.e. during the RESERVE2 pass)
                                 *  So the new row needs to be created before
                                 * that in order to have somewhere to put them.
                                 *
            ...
            for (request = requests; request; request = request->next)
            {
                ...
                switch (table_info->colnum)
                {
                    case NSLOGGING_TYPE:
                        ...
                        break;
                        /*
                         * Don't need to handle nsLogToken or nsLogStatus in
                         * this pass
                         */
                }
            }
            ...
            for (request = requests; request; request = request->next)
            {
                ...
                switch (table_info->colnum)
                {
                    case NSLOGGING_STATUS:
                        ...
                        break;
                        /*
                         * Don't need to handle nsLogToken or nsLogType in this
                         * pass
                         */
                }
            }
            ...
            for (request = requests; request; request = request->next)
            {
                ...
                switch (table_info->colnum)
                {
                    case NSLOGGING_TYPE:
                         ...
                         *
                         * We need to be careful here - if the reason it failed
                         * is that the type was already set, then we shouldn't
                         * "undo" the assignment (since it won't actually have
                         * been made).
                         *
                         * Check the current value against the 'new' one.  If
                         * they're the same, then this is probably a successful
                         * assignment, and the failure was elsewhere, so we need
                         * to undo it. (Or else there was an attempt to write
                         * the same value!)
                         */
                        if (logh &&
                            logh->type == *request->requestvb->val.integer)
