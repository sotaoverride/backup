...
static const char* to_prio[] = {[IOPRIO_CLASS_NONE] = "none",
...
static void __attribute__((__noreturn__)) usage(void)
{
    ...
    fputs(_("Show or change the I/O-scheduling class and priority of a "
            "process.\n"),
          out);

    fputs(USAGE_OPTIONS, out);
    fputs(_(" -c, --class <class>    name or number of scheduling class,\n"
            "                          0: none, 1: realtime, 2: best-effort, "
            "3: idle\n"),
          out);
    fputs(_(" -n, --classdata <num>  priority (0..7) in the specified "
            "scheduling class,\n"
            "                          only for the realtime and best-effort "
            "classes\n"),
          out);
    fputs(_(" -p, --pid <pid>...     act on these already running processes\n"),
          out);
    fputs(_(" -P, --pgid <pgrp>...   act on already running processes in these "
            "groups\n"),
          out);
    fputs(_(" -t, --ignore           ignore failures\n"), out);
    fputs(_(" -u, --uid <uid>...     act on already running processes owned by "
            "these users\n"),
          out);
...
int main(int argc, char** argv)
{
    ...
    while ((c = getopt_long(argc, argv, "+n:c:p:P:u:tVh", longopts, NULL)) !=
           EOF)
        switch (c)
        {
            case 'n':
                data =
                    strtos32_or_err(optarg, _("invalid class data argument"));
                set |= 1;
                break;
            case 'c':
                if (isdigit(*optarg))
                    ioclass =
                        strtos32_or_err(optarg, _("invalid class argument"));
                else
                {
                    ioclass = parse_ioclass(optarg);
                    if (ioclass < 0)
                        errx(EXIT_FAILURE, _("unknown scheduling class: '%s'"),
                             optarg);
                }
                set |= 2;
                break;
