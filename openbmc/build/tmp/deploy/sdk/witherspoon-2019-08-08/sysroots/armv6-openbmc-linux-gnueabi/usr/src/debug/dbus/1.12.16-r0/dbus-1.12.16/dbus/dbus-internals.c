...
static void init_warnings(void)
{
    if (!warn_initted)
    {
        ...
        if (s && *s)
        {
            ...
            {
                fprintf(stderr,
                        "DBUS_FATAL_WARNINGS should be set to 0 or 1 if set, "
                        "not '%s'",
                        s);
            }
        }
...
*/
static char* _dbus_file_path_extract_elements_from_tail(const char* file,
                                                        int level)
{
    int prefix = 0;
    ...
    const char* format, ...)
{
...
#ifdef DBUS_USE_OUTPUT_DEBUG_STRING
    {
        ...
        sprintf(buf + strlen(buf), "[%s(%d):%s] ",
                _dbus_file_path_extract_elements_from_tail(file, 2), line,
                function);
#endif
...
void _dbus_trace_ref(const char* obj_name, void* obj, int old_refcount,
                     int new_refcount, const char* why, const char* env_var,
                     int* enabled)
{
    ...
    if (*enabled)
    {
        if (old_refcount == -1)
        {
            VALGRIND_PRINTF_BACKTRACE("%s %p ref stolen (%s)", obj_name, obj,
                                      why);
            _dbus_verbose("%s %p ref stolen (%s)\n", obj_name, obj, why);
        }
        else
        {
            VALGRIND_PRINTF_BACKTRACE("%s %p %d -> %d refs (%s)", obj_name, obj,
...
dbus_bool_t _dbus_generate_uuid(DBusGUID* uuid, DBusError* error)
{
    ...
    if (!_dbus_generate_random_bytes_buffer(
            uuid->as_bytes, DBUS_UUID_LENGTH_BYTES - 4, &rand_error))
    {
        dbus_set_error(error, rand_error.name, "Failed to generate UUID: %s",
                       rand_error.message);
...
{
    DBusString binary;
    _dbus_string_init_const_len(&binary, uuid->as_bytes,
                                DBUS_UUID_LENGTH_BYTES);
    return _dbus_string_hex_encode(&binary, 0, encoded,
                                   _dbus_string_get_length(encoded));
}

static dbus_bool_t
    _dbus_read_uuid_file_without_creating(const DBusString* filename,
                                          DBusGUID* uuid, DBusError* error)
{
    DBusString contents;
    DBusString decoded;
    int end;
    ...
    if (_dbus_string_get_length(&contents) != DBUS_UUID_LENGTH_HEX)
    {
        dbus_set_error(error, DBUS_ERROR_INVALID_FILE_CONTENT,
                       "UUID file '%s' should contain a hex string of length "
                       "%d, not length %d, with no other text",
    ...
    {
        dbus_set_error(error, DBUS_ERROR_INVALID_FILE_CONTENT,
                       "UUID file '%s' contains %d bytes of hex-encoded data "
                       "instead of %d",
                       _dbus_string_get_const_data(filename),
    ...
    _dbus_string_copy_to_buffer(&decoded, uuid->as_bytes,
                                DBUS_UUID_LENGTH_BYTES);
...
dbus_bool_t _dbus_write_uuid_file(const DBusString* filename,
                                  const DBusGUID* uuid, DBusError* error)
{
    DBusString encoded;
 ...
 * @param create_if_not_found #TRUE to create a new uuid and save it if the file
 * doesn't exist
 * @param error the error return
 * @returns #FALSE if the error is set
 */
dbus_bool_t _dbus_read_uuid_file(const DBusString* filename, DBusGUID* uuid,
...
void _dbus_warn_return_if_fail(const char* function, const char* assertion,
                               const char* file, int line)
{
    _dbus_warn_check_failed(
        "arguments to %s() were incorrect, assertion \"%s\" failed in file %s "
        "line %d.\n"
 ...
 */
void _dbus_real_assert(dbus_bool_t condition, const char* condition_text,
 ...
 * @param line line the assertion is in
 */
void _dbus_real_assert_not_reached(const char* explanation, const char* file,
                                   int line)
{
    _dbus_warn("File \"%s\" line %d should not have been reached: %s", file,
               line, explanation);
    _dbus_abort();
}
...
static dbus_bool_t run_failing_each_malloc(int n_mallocs,
...
dbus_bool_t _dbus_test_oom_handling(const char* description,
                                    DBusTestMemoryFunction func, void* data)
{
    int approx_mallocs;
    const char* setting;
