...
static void
    journal_dump_trans(journal_transaction_t* trans EXT2FS_ATTR((unused)),
                       const char* tag EXT2FS_ATTR((unused)))
{
    dbg_printf("TRANS %p(%s): tid=%d start=%llu block=%llu end=%llu "
               "flags=0x%x\n",
               trans, tag, trans->tid, trans->start, trans->block, trans->end,
               trans->flags);
}
...
static errcode_t journal_commit_trans(journal_transaction_t* trans)
{
    ...
    if ((trans->flags & J_TRANS_COMMITTED) || !(trans->flags & J_TRANS_OPEN))
        return EXT2_ET_INVALID_ARGUMENT;
    ...
    if (jfs_has_feature_checksum(trans->journal))
    {
        ...
        cbh = getblk(trans->journal->j_dev, 0, trans->journal->j_blocksize);
        if (cbh == NULL)
        {
            err = ENOMEM;
            goto error;
        ...
        {
            err = journal_bmap(trans->journal, cblk, &cbh->b_blocknr);
            if (err)
                goto error;
            ...
            if (err)
                goto error;
            csum_v1 = ext2fs_crc32_be(
                csum_v1, (unsigned char const*)cbh->b_data, cbh->b_size);
        }
                                             ...
                                             size_t revoke_len)
{
    ...
    bh = getblk(trans->journal->j_dev, curr_blk, trans->journal->j_blocksize);
    if (bh == NULL)
        return ENOMEM;
    jrb = buf = bh->b_data;
                                             ...
                                             size_t block_len, FILE* fp)
{
    ...
    data_bh =
        getblk(trans->journal->j_dev, curr_blk, trans->journal->j_blocksize);
    if (data_bh == NULL)
        return ENOMEM;
    buf = data_bh->b_data;
    ...
    for (i = 0; i < block_len; i++)
    {
        ...
        {
            memcpy(jdbt + tag_bytes, trans->journal->j_superblock->s_uuid,
        ...
        jbd2_block_tag_csum_set(trans->journal, jdbt, data_bh, trans->tid);

        /* Write the data block */
        err = journal_bmap(trans->journal, curr_blk, &data_bh->b_blocknr);
        if (err)
            goto error;
        dbg_printf("Writing data block %llu at %llu:%llu tag %d\n",
                   block_list[i], curr_blk, data_bh->b_blocknr, tag_bytes);
...
static errcode_t journal_write(journal_t* journal, int flags,
                               blk64_t* block_list, size_t block_len,
                               blk64_t* revoke_list, size_t revoke_len,
                               FILE* fp)
{
    blk64_t blocks;
    journal_transaction_t trans;
...
void do_journal_write(int argc, char* argv[])
{
    ...
    while ((opt = getopt(argc, argv, "b:r:c")) != -1)
    {
        switch (opt)
        {
            case 'b':
                ...
                if (err)
                    com_err(argv[0], err, "while reading block list");
                break;
            case 'r':
                err = read_list(optarg, &rlist, &rn);
                if (err)
                    com_err(argv[0], err, "while reading revoke list");
                break;
            case 'c':
    ...
    if (bn > 0)
    {
        ...
        if (fp == NULL)
        {
            com_err(argv[0], errno, "while opening journal data file");
            goto out;
        }
    }

    err = journal_write(current_journal, flags, blist, bn, rlist, rn, fp);
    if (err)
...
static errcode_t journal_find_head(journal_t* journal)
{
    ...
    while (1)
    {
        dbg_printf("Scanning for sequence ID %u at %lu/%lu\n", next_commit_ID,
                   (unsigned long)next_log_block, journal->j_last);
...
static void update_journal_csum(journal_t* journal, int ver)
{
    ...
    if (journal->j_tail != 0 || ext2fs_has_feature_journal_needs_recovery(
...
static void update_uuid(journal_t* journal)
{
    ...
    if (jfs_has_feature_64bit(journal) && ext2fs_has_feature_64bit(fs->super))
        return;
