...
int main(int argc, char* argv[])
{
    ...
    if (from_charmap != NULL || to_charmap != NULL)
        ...
        status =
            charmap_conversion(from_code, from_charmap, to_code, to_charmap,
               ...
               connected.  */
                bool from_wrong =
                    (iconv_open("UTF-8", from_code) == (iconv_t)-1 &&
                     errno == EINVAL);
                bool to_wrong = (iconv_open(to_code, "UTF-8") == (iconv_t)-1 &&
                                 errno == EINVAL);
...
conversions from `%s' and to `%s' are not supported"),
                        ...
                        error(0, 0, _("conversion from `%s' is not supported"),
                              from_pretty);
                }
                else
                {
                    if (to_wrong)
                        error(0, 0, _("conversion to `%s' is not supported"),
                              to_pretty);
                    else
                        error(
                            0, 0,
       ...
       the parameter process all from stdin.  */
        if (remaining == argc)
        {
            ...
            do
            {
                ...
                if (fstat64(fd, &st) == 0 &&
                    ((addr = mmap(NULL, st.st_size, PROT_READ, MAP_PRIVATE, fd,
                                  0)) != MAP_FAILED))
                {
                    ...
                    if (ret != 0)
                    {
                        ...
                        if (ret < 0)
                            /* We cannot go on with producing output since it
                           might lead to problem because the last output might
                           leave the output stream in an undefined state.  */
                            break;
                    }
                }
                else
...
static int write_output(const char* outbuf, const char* outptr, FILE** output,
                        const char* output_file)
{
