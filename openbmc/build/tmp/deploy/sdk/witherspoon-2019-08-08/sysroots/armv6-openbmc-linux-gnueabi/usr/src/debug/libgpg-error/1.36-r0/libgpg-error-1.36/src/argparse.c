...
static int is_native_utf8(void)
{
    static char result;
...
static const char* map_fixed_string(const char* string)
{
...
static void deinitialize(gpgrt_argparse_t* arg)
{
    if (arg->internal)
    {
...
static void my_exit(gpgrt_argparse_t* arg, int code)
{
    deinitialize(arg);
    exit(code);
}

static gpg_err_code_t initialize(gpgrt_argparse_t* arg, gpgrt_opt_t* opts,
                                 estream_t fp)
{
    ...
    if (!arg->internal->opts)
    {
        ...
        static gpgrt_opt_t end_marker = ARGPARSE_end();
        int seen_help = 0;
        int seen_version = 0;
        int seen_warranty = 0;
    ...
    if (arg->err)
    {
        ...
        if (fp)
        {
            if (arg->r_opt == ARGPARSE_UNEXPECTED_ARG)
                ...
                s = _("invalid option");
            _gpgrt_log_error("%s:%u: %s\n", _gpgrt_fname_get(fp), arg->lineno,
                             s);
        }
        else
        {
...
static void store_alias(gpgrt_argparse_t* arg, char* name, char* value)
{
...
static int ignore_invalid_option_p(gpgrt_argparse_t* arg, const char* keyword)
{
...
static int ignore_invalid_option_add(gpgrt_argparse_t* arg, estream_t fp)
{
    IIO_ITEM_DEF item;
    int c;
    char name[100];
...
static void ignore_invalid_option_clear(gpgrt_argparse_t* arg)
{
 ...
 */
int _gpgrt_argparse(estream_t fp, gpgrt_argparse_t* arg, gpgrt_opt_t* opts_orig)
{
    gpgrt_opt_t** opts;
    int state, i, c;
    int idx = 0;
    ...
    if (!arg->lineno)
    {
        ...
        if (unread_buf[0] != 0xef || unread_buf[1] != 0xbb ||
            unread_buf[2] != 0xbf)
            unread_buf_count = 3;
    }
    ...
    for (;;)
    {
        ...
        if (c == '\n' || c == EOF)
        {
            ...
            else if (state == 2)
            {
                ...
                {
                    if (opts[i]->long_opt &&
                        !strcmp(opts[i]->long_opt, keyword))
                        break;
                }
                idx = i;
...
static int find_long_option(gpgrt_argparse_t* arg, gpgrt_opt_t** opts,
                            const char* keyword)
{
    int i;
    size_t n;
...
static int arg_parse(gpgrt_argparse_t* arg, gpgrt_opt_t* opts_orig)
{
    int idx;
    gpgrt_opt_t** opts;
    int argc;
...
next_one:
    ...
    else if (*s == '-' && s[1] == '-')
    {
        ...
        else if (i > 0 && opts[i]->short_opt == ARGPARSE_SHORTOPT_DUMP_OPTIONS)
        {
            ...
            {
                if (opts[i]->long_opt &&
                    !(opts[i]->flags & ARGPARSE_OPT_IGNORE))
...
static size_t long_opt_strlen(gpgrt_opt_t* o)
{
...
const char* _gpgrt_strusage(int level)
{
    ...
    switch (level)
    {
        ...
        case 13:
            ...
            break;
        case 14:
            p = "Copyright (C) YEAR NAME";
            break;
        case 15:
            p = "This is free software: you are free to change and "
                "redistribute it.\n"
...
void _gpgrt_set_usage_outfnc(int (*f)(int, const char*))
{
    custom_outfnc = f;
}
