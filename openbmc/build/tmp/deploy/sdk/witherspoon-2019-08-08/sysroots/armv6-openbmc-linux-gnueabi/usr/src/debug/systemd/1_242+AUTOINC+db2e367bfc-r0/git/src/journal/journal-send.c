...
_public_ int sd_journal_printv(int priority, const char* format, va_list ap)
{
    ...
    char buffer[8 + LINE_MAX],
        p[STRLEN("PRIORITY=") + DECIMAL_STR_MAX(int) + 1];
    struct iovec iov[2];
...
_printf_(1, 0) static int fill_iovec_sprintf(const char* format, va_list ap,
                                             int extra, struct iovec** _iov)
{
    PROTECT_ERRNO;
    int r, n = 0, i = 0, j;
    ...
    while (format)
    {
        ...
        (void)strstrip(
            buffer); /* strip trailing whitespace, keep prefixing whitespace */
...
_public_ int sd_journal_sendv(const struct iovec* iov, int n)
{
    ...
    for (i = 0; i < n; i++)
    {
        ...
        if (nl)
        {
            ...
            w[j++] = IOVEC_MAKE(c + 1, iov[i].iov_len -
...
static int fill_iovec_perror_and_send(const char* message, int skip,
                                      struct iovec iov[])
{
    PROTECT_ERRNO;
    size_t n, k;
...
_public_ int sd_journal_stream_fd(const char* identifier, int priority,
                                  int level_prefix)
{
    static const union sockaddr_union sa = {
        .un.sun_family = AF_UNIX,
...
_public_ int sd_journal_print_with_location(int priority, const char* file,
...
_public_ int sd_journal_printv_with_location(int priority, const char* file,
                                             const char* line, const char* func,
...
_public_ int sd_journal_send_with_location(const char* file, const char* line,
...
_public_ int sd_journal_sendv_with_location(const char* file, const char* line,
                                            const char* func,
...
_public_ int sd_journal_perror_with_location(const char* file, const char* line,
                                             const char* func,
                                             const char* message)
{
