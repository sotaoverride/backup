...
static inline struct alloc_buffer alloc_buffer_create(void* start, size_t size)
{
...
static __always_inline __attribute__((nonnull(2))) struct alloc_buffer
    alloc_buffer_allocate(size_t size, void** pptr)
{
...
void* __libc_alloc_buffer_alloc_array(struct alloc_buffer* buf, size_t size,
...
#define alloc_buffer_alloc_array(buf, type, count)                             \
    ((type*)__libc_alloc_buffer_alloc_array(                                   \
        buf, __alloc_buffer_assert_size(sizeof(type)),                         \
        __alloc_buffer_assert_align(__alignof__(type)), count))
...
static inline __attribute__((nonnull(1, 2))) void
    alloc_buffer_copy_bytes(struct alloc_buffer* buf, const void* src,
                            size_t size)
{
