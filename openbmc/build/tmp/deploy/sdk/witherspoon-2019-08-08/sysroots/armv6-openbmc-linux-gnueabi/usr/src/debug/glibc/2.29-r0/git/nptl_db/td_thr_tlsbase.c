...
/* Get the DTV slotinfo list head entry from the dynamic loader state
   into *LISTHEAD.  */
static td_err_e dtv_slotinfo_list(td_thragent_t* ta, psaddr_t* listhead)
{
    td_err_e err;
    psaddr_t head;
    ...
    {
        err = DB_GET_FIELD(head, ta, ta->ta_addr__rtld_global, rtld_global,
    ...
    else
    {
        ...
        err = _td_fetch_value(ta, ta->ta_var__dl_tls_dtv_slotinfo_list,
                              SYM_DESC__dl_tls_dtv_slotinfo_list, 0,
 ...
 *DTVSLOTINFO.  */
static td_err_e dtv_slotinfo(td_thragent_t* ta, unsigned long int modid,
                             psaddr_t* dtvslotinfo)
{
    ...
    while (slot)
    {
        ...
        slbase += len;
        err = DB_GET_FIELD(temp, ta, slot, dtv_slotinfo_list, next, 0);
        if (err != TD_OK)
            return err;
        slot = temp;
    }
    ...
    err = DB_GET_FIELD_ADDRESS(temp, ta, slot, dtv_slotinfo_list, slotinfo,
                               modid - slbase);
    if (err != TD_OK)
        return err;
    slot = temp;
...
td_err_e td_thr_tlsbase(const td_thrhandle_t* th, unsigned long int modid,
                        psaddr_t* base)
{
    td_err_e err;
    ...
    try_static_tls:
        ...
        if (tlsoff != FORCED_DYNAMIC_TLS_OFFSET && tlsoff != NO_TLS_OFFSET)
        {
            psaddr_t tp = pd;
