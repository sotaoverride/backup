...
#define POLICY_VERDICT_INIT_WITH(_v, _p)                                       \
    {                                                                          \
        .verdict = (_v), .priority = (_p)                                      \
    }
...
#define POLICY_XMIT_NULL(_x)                                                   \
    {                                                                          \
        ...
        .verdict = POLICY_VERDICT_INIT, .type = DBUS_MESSAGE_TYPE_INVALID,     \
        .max_fds = UINT64_MAX,                                                 \
    }
...
#define POLICY_BATCH_NULL(_x)                                                  \
    {                                                                          \
        .n_refs = C_REF_INIT, .connect_verdict = POLICY_VERDICT_INIT,          \
...
#define POLICY_REGISTRY_NODE_INDEX_NULL                                        \
    {                                                                          \
        .uidgid_start = (uint32_t)-1, .uidgid_end = (uint32_t)-1,              \
    }
...
#define POLICY_REGISTRY_NULL                                                   \
    {                                                                          \
        .uid_range_tree = C_RBTREE_INIT, .uid_tree = C_RBTREE_INIT,            \
...
int policy_snapshot_new(PolicySnapshot** snapshotp, PolicyRegistry* registry,
                        const char* context, uint32_t uid, const uint32_t* gids,
                        size_t n_gids);
...
int policy_snapshot_check_send(PolicySnapshot* snapshot,
                               const char* subject_context, NameSet* subject,
