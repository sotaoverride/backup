...
static int get_from_registry_indirect(char* name_buf, int name_size)
{
    ...
    newkey[0] = 0;
    GetPrivateProfileString(INI_FILES, "RegKRB5CCNAME", "", newkey,
                            sizeof(newkey), KERBEROS_INI);
    if (!newkey[0])
        return 0;
    ...
    if (RegOpenKeyEx(HKEY_CURRENT_USER, newkey, 0, KEY_QUERY_VALUE, &hkey) !=
        ERROR_SUCCESS)
        return 0;

    name_buf_size = name_size;
    if (RegQueryValueEx(hkey, cp, 0, 0, name_buf, &name_buf_size) !=
        ERROR_SUCCESS)
    {
        RegCloseKey(hkey);
        return 0;
...
static int set_to_registry(HKEY hBaseKey, const char* name_buf)
{
    HRESULT result;
    HKEY hKey;
...
static int get_from_registry(HKEY hBaseKey, char* name_buf, int name_size)
{
    HKEY hKey;
    DWORD name_buf_size = (DWORD)name_size;

    if (RegOpenKeyEx(hBaseKey, key_path, 0, KEY_QUERY_VALUE, &hKey) !=
        ERROR_SUCCESS)
        return 0;
    if (RegQueryValueEx(hKey, value_name, 0, 0, name_buf, &name_buf_size) !=
        ERROR_SUCCESS)
    {
        RegCloseKey(hKey);
        return 0;
...
static int try_dir(char* dir, char* buffer, int buf_len)
{
    struct _stat s;
    if (!dir)
        return 0;
...
static krb5_error_code get_from_os_buffer(char* name_buf,
                                          unsigned int name_size)
{
    ...
    if (get_from_registry(HKEY_CURRENT_USER, name_buf, name_size) != 0)
        return 0;

    if (get_from_registry(HKEY_LOCAL_MACHINE, name_buf, name_size) != 0)
        return 0;
...
static void get_from_os(krb5_context context)
{
    krb5_error_code err;
    char buf[1024];
