...
static OM_uint32
    val_acq_cred_pw_args(OM_uint32* minor_status, const gss_name_t desired_name,
                         ...
                         gss_OID_set* actual_mechs, OM_uint32* time_rec)
{
    ...
    if (password == GSS_C_NO_BUFFER || password->length == 0 ||
        password->value == NULL)
    {
        if (minor_status)
        {
...
OM_uint32 KRB5_CALLCONV gss_acquire_cred_with_password(
    minor_status, desired_name, password, time_req, desired_mechs, cred_usage,
...
OM_uint32* time_rec;
{
    ...
    major = val_acq_cred_pw_args(minor_status, desired_name, password, time_req,
    ...
    for (i = 0; i < mechs->count; i++)
    {
        major = gss_add_cred_with_password(
            minor_status, (gss_cred_id_t)creds, desired_name,
            &mechs->elements[i], password, cred_usage, time_req, time_req, NULL,
...
static OM_uint32 val_add_cred_pw_args(
...
/* V2 KRB5_CALLCONV */
OM_uint32 KRB5_CALLCONV gss_add_cred_with_password(
    minor_status, input_cred_handle, desired_name, desired_mech, password,
...
OM_uint32* acceptor_time_rec;
{
    ...
    status = mech->gssspi_acquire_cred_with_password(
        minor_status, internal_name, password, time_req, target_mechs,
        cred_usage, &cred, NULL, &time_rec);
    if (status != GSS_S_COMPLETE)
    {
