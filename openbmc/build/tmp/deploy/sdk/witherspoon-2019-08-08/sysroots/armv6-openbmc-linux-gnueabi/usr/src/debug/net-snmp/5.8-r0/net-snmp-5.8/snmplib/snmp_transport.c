...
{
    netsnmp_ds_register_config(ASN_BOOLEAN, "snmp", "dontLoadHostConfig",
...
void netsnmp_transport_parse_filterType(const char* word, char* cptr)
{
    ...
    if (type != 42)
    {
        ...
        netsnmp_ds_set_int(NETSNMP_DS_LIBRARY_ID, NETSNMP_DS_LIB_FILTER_TYPE,
                           type);
    }
}
...
int netsnmp_transport_send(netsnmp_transport* t, const void* packet, int length,
                           void** opaque, int* olength)
{
    ...
    debugLength =
        (SNMPERR_SUCCESS == debug_is_token_registered("transport:send"));

    if (dumpPacket | debugLength)
    {
        char* str = netsnmp_transport_peer_string(t, opaque ? *opaque : NULL,
                                                  olength ? *olength : 0);
        if (debugLength)
...
int netsnmp_transport_recv(netsnmp_transport* t, void* packet, int length,
                           void** opaque, int* olength)
{
    int debugLength;
    ...
    debugLength =
        (SNMPERR_SUCCESS == debug_is_token_registered("transport:recv"));
...
int netsnmp_tdomain_register(netsnmp_tdomain* n)
{
    ...
    if (n != NULL)
    {
        for (d = domain_list; d != NULL; d = d->next)
        {
            if (netsnmp_oid_equals(n->name, n->name_length, d->name,
                                   d->name_length) == 0)
            {
                /*
                 * Already registered.
...
static int netsnmp_is_fqdn(const char* thename)
{
    if (!thename)
        return 0;
    while (*thename)
...
netsnmp_transport* netsnmp_tdomain_transport_tspec(netsnmp_tdomain_spec* tspec)
{
    ...
    DEBUGMSGTL(
        ("tdomain",
         ...
         default_target ? default_target : "[NIL]", source ? source : "[NIL]"));
        ...
        netsnmp_is_fqdn(str))
    {
        ...
        if (NULL != (newhost = netsnmp_ds_get_string(NETSNMP_DS_LIBRARY_ID,
                                                     NETSNMP_DS_LIB_HOSTNAME)))
        {
        ...
        netsnmp_ds_set_string(NETSNMP_DS_LIBRARY_ID, NETSNMP_DS_LIB_HOSTNAME,
                              prev_hostname);
    ...
    if (match == NULL)
    {
        ...
        {
            DEBUGMSGTL(("tdomain", "Use user specified default domain \"%s\"\n",
                        default_domain));
        ...
        else
        {
            ...
            {
                DEBUGMSGTL(
                    ("tdomain", "No default domain found, assume \"udp\"\n"));
                match = find_tdomain("udp");
            }
            else
            {
                const char* const* r = spec;
                DEBUGMSGTL(("tdomain", "Use application default domains"));
                while (*r)
                {
...
netsnmp_transport* netsnmp_tdomain_transport(const char* str, int local,
                                             const char* default_domain)
{
    netsnmp_tdomain_spec tspec;
    memset(&tspec, 0x0, sizeof(tspec));
...
netsnmp_transport* netsnmp_tdomain_transport_oid(const oid* dom, size_t dom_len,
                                                 ...
                                                 int local)
{
    ...
    for (d = domain_list; d != NULL; d = d->next)
    {
        for (i = 0; d->prefix[i] != NULL; i++)
        {
            if (netsnmp_oid_equals(dom, dom_len, d->name, d->name_length) == 0)
            {
...
netsnmp_transport* netsnmp_transport_open(const char* application,
                                          const char* str, int local)
{
    return netsnmp_tdomain_transport_full(application, str, local, NULL, NULL);
}

netsnmp_transport* netsnmp_transport_open_server(const char* application,
                                                 const char* str)
{
    return netsnmp_tdomain_transport_full(application, str, 1, NULL, NULL);
}

netsnmp_transport* netsnmp_transport_open_client(const char* application,
                                                 const char* str)
{
...
static void _tc_free(trans_cache* tc)
{
    if (NULL == tc)
        return;
...
static void _tc_remove(trans_cache* tc)
{
...
static trans_cache* _tc_create(int af, int type, int local,
                               ...
                               netsnmp_transport* t)
{
    ...
    if (AF_INET != tc->af && AF_INET6 != tc->af)
        NETSNMP_LOGONCE(
            (LOG_WARNING, "transport cache not tested for af %d\n", tc->af));
    return tc;
}

static trans_cache* _tc_add(int af, int type, int local,
                            const netsnmp_sockaddr_storage* addr,
                            netsnmp_transport* t)
{
    trans_cache* tc;
    ...
    {
        DEBUGMSGTL(
            ("transport:cache:add", "could not create transport cache\n"));
        return NULL;
    }
...
trans_cache* _tc_find(int af, int type, int local,
                      const netsnmp_sockaddr_storage* addr)
{
    trans_cache tc, *rtn;
...
trans_cache* _tc_find_transport(netsnmp_transport* t)
{
    /*
...
#define USE_CACHE 1
    ...
    {
        DEBUGMSGTL(
            ("transport:cache:get", "using existing transport %p\n", tc->t));
        ++tc->count;
        return tc->t;
    }
#endif
    ...
    {
        DEBUGMSGTL(
            ("transport:cache:get", "could not create transport cache\n"));
        /*
