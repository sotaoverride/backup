...
static void g_network_address_set_property(GObject* object, guint prop_id,
                                           const GValue* value,
                                           GParamSpec* pspec);
static void g_network_address_get_property(GObject* object, guint prop_id,
                                           GValue* value, GParamSpec* pspec);

static void
    g_network_address_connectable_iface_init(GSocketConnectableIface* iface);
static GSocketAddressEnumerator*
    g_network_address_connectable_enumerate(GSocketConnectable* connectable);
static GSocketAddressEnumerator* g_network_address_connectable_proxy_enumerate(
...
G_DEFINE_TYPE_WITH_CODE(GNetworkAddress, g_network_address, G_TYPE_OBJECT,
                        G_ADD_PRIVATE(GNetworkAddress) G_IMPLEMENT_INTERFACE(
...
static void g_network_address_class_init(GNetworkAddressClass* klass)
{
    ...
    g_object_class_install_property(
        ...
        g_param_spec_string("hostname", P_("Hostname"),
                            P_("Hostname to resolve"), NULL,
                            G_PARAM_READWRITE | G_PARAM_CONSTRUCT_ONLY |
                                G_PARAM_STATIC_STRINGS));
    g_object_class_install_property(
        gobject_class, PROP_PORT,
        g_param_spec_uint("port", P_("Port"), P_("Network port"), 0, 65535, 0,
                          G_PARAM_READWRITE | G_PARAM_CONSTRUCT_ONLY |
                              G_PARAM_STATIC_STRINGS));

    g_object_class_install_property(
        gobject_class, PROP_SCHEME,
        g_param_spec_string("scheme", P_("Scheme"), P_("URI Scheme"), NULL,
                            G_PARAM_READWRITE | G_PARAM_CONSTRUCT_ONLY |
                                G_PARAM_STATIC_STRINGS));
}

static void g_network_address_connectable_iface_init(
    GSocketConnectableIface* connectable_iface)
{
    connectable_iface->enumerate = g_network_address_connectable_enumerate;
    connectable_iface->proxy_enumerate =
 ...
 */
static GList* inet_addresses_to_inet_socket_addresses(GNetworkAddress* addr,
                                                      GList* addresses)
{
    ...
    for (a = addresses; a; a = a->next)
    {
        GSocketAddress* sockaddr =
            g_inet_socket_address_new(a->data, addr->priv->port);
        socket_addresses =
            g_list_append(socket_addresses, g_steal_pointer(&sockaddr));
 ...
 */
static void g_network_address_set_cached_addresses(GNetworkAddress* addr,
                                                   GList* addresses,
                                                   guint64 resolver_serial)
{
    ...
    if (G_IS_INET_SOCKET_ADDRESS(addresses->data))
        ...
        addr->priv->cached_sockaddrs = inet_addresses_to_inet_socket_addresses(
            addr, g_steal_pointer(&addresses));
    addr->priv->resolver_serial = resolver_serial;
}

static gboolean g_network_address_parse_sockaddr(GNetworkAddress* addr)
{
    GSocketAddress* sockaddr;
    ...
    if (sockaddr)
    {
        addr->priv->cached_sockaddrs =
            g_list_append(addr->priv->cached_sockaddrs, sockaddr);
        return TRUE;
    }
    else
        return FALSE;
...
{
    return g_object_new(G_TYPE_NETWORK_ADDRESS, "hostname", hostname, "port",
                        port, NULL);
}
...
GSocketConnectable* g_network_address_new_loopback(guint16 port)
{
    ...
    addr = g_object_new(G_TYPE_NETWORK_ADDRESS, "hostname", "localhost", "port",
                        port, NULL);
...
gboolean _g_uri_parse_authority(const char* uri, char** host, guint16* port,
                                char** userinfo, GError** error)
{
    ...
    if (at != NULL)
    {
        ...
        while (1)
        {
            ...
            if (!(g_ascii_isalnum(c) || strchr(G_URI_OTHER_UNRESERVED, c) ||
...
gchar* _g_uri_from_authority(const gchar* protocol, const gchar* host,
                             guint port, const gchar* userinfo)
{
    GString* uri;
    ...
    if (userinfo)
    {
        g_string_append_uri_escaped(
            uri, userinfo, G_URI_RESERVED_CHARS_ALLOWED_IN_USERINFO, FALSE);
                                                ...
                                                GError** error)
{
    ...
    conn = g_object_new(G_TYPE_NETWORK_ADDRESS, "hostname", hostname, "port",
                        port, "scheme", scheme, NULL);
...
#define G_TYPE_NETWORK_ADDRESS_ADDRESS_ENUMERATOR                              \
    (_g_network_address_address_enumerator_get_type())
#define G_NETWORK_ADDRESS_ADDRESS_ENUMERATOR(obj)                              \
    (G_TYPE_CHECK_INSTANCE_CAST((obj),                                         \
                                G_TYPE_NETWORK_ADDRESS_ADDRESS_ENUMERATOR,     \
...
G_DEFINE_TYPE(GNetworkAddressAddressEnumerator,
...
static void g_network_address_address_enumerator_finalize(GObject* object)
{
    ...
    G_OBJECT_CLASS(_g_network_address_address_enumerator_parent_class)
        ->finalize(object);
}

static inline GSocketFamily get_address_family(GInetSocketAddress* address)
{
    return g_inet_address_get_family(
        g_inet_socket_address_get_address(address));
}

static void list_split_families(GList* list, GList** out_ipv4, GList** out_ipv6)
{
    g_assert(out_ipv4);
    g_assert(out_ipv6);
...
static GList* list_interleave_families(GList* list1, GList* list2)
{
 ...
 */
static GList* list_copy_interleaved(GList* list)
{
 ...
 * items in the previous list starting from @current_item and concats the
 * results to @parent_list.
 *
 * Returns: (transfer container): New start of list
 */
static GList* list_concat_interleaved(GList* parent_list, GList* current_item,
                                      GList* new_list)
{
    GList *ipv4 = NULL, *ipv6 = NULL, *interleaved, *trailing = NULL;
    GSocketFamily last_family =
        G_SOCKET_FAMILY_IPV4; /* Default to starting with ipv6 */
                               ...
                               GResolver* resolver)
{
    ...
    if (addr_enum->state & RESOLVE_STATE_WAITING_ON_IPV4 ||
    ...
    g_network_address_set_cached_addresses(addr_enum->addr,
                                           g_steal_pointer(&addresses),
                                           g_resolver_get_serial(resolver));
}

static void g_network_address_address_enumerator_add_addresses(
    GNetworkAddressAddressEnumerator* addr_enum, GList* addresses,
    GResolver* resolver)
{
    GList* new_addresses =
        inet_addresses_to_inet_socket_addresses(addr_enum->addr, addresses);

    if (addr_enum->addresses == NULL)
        addr_enum->addresses = g_steal_pointer(&new_addresses);
    else
        addr_enum->addresses = list_concat_interleaved(
            addr_enum->addresses, addr_enum->current_item,
            g_steal_pointer(&new_addresses));

    maybe_update_address_cache(addr_enum, resolver);
}

static gpointer copy_object(gconstpointer src, gpointer user_data)
{
    ...
    init_and_query_next_address(GNetworkAddressAddressEnumerator* addr_enum)
{
    ...
    if (addr_enum->addresses == NULL)
        addr_enum->addresses = g_list_copy_deep(
            addr_enum->addr->priv->cached_sockaddrs, copy_object, NULL);

    /* We always want to look at the next item at call time to get the latest
       results. That means that sometimes ->next is NULL this call but is valid
       next call.
     */
...
static GSocketAddress* g_network_address_address_enumerator_next(
    GSocketAddressEnumerator* enumerator, GCancellable* cancellable,
    GError** error)
{
    GNetworkAddressAddressEnumerator* addr_enum =
    ...
    if (addr_enum->addresses == NULL)
    {
        ...
        if (!addr->priv->cached_sockaddrs)
        {
            ...
            addresses = g_resolver_lookup_by_name(
                resolver, addr->priv->hostname, cancellable, error);
            if (!addresses)
            {
            ...
            g_network_address_set_cached_addresses(
                addr, g_steal_pointer(&addresses), serial);
        }
...
static void complete_queued_task(GNetworkAddressAddressEnumerator* addr_enum,
...
static int on_address_timeout(gpointer user_data)
{
    ...
    if (addr_enum->queued_task != NULL)
        complete_queued_task(addr_enum,
                             g_steal_pointer(&addr_enum->queued_task),
                             g_steal_pointer(&addr_enum->last_error));
    else if (addr_enum->waiting_task != NULL)
        complete_queued_task(addr_enum,
                             g_steal_pointer(&addr_enum->waiting_task), NULL);
...
static void got_ipv6_addresses(GObject* source_object, GAsyncResult* result,
                               gpointer user_data)
{
    GNetworkAddressAddressEnumerator* addr_enum = user_data;
    ...
    if (!error)
        g_network_address_address_enumerator_add_addresses(
            addr_enum, g_steal_pointer(&addresses), resolver);
    else
        ...
        (addr_enum->state & RESOLVE_STATE_WAITING_ON_IPV4))
    {
        ...
        addr_enum->wait_source =
            g_timeout_source_new(HAPPY_EYEBALLS_RESOLUTION_DELAY_MS);
        g_source_set_callback(addr_enum->wait_source, on_address_timeout,
                              addr_enum, NULL);
...
static void got_ipv4_addresses(GObject* source_object, GAsyncResult* result,
                               gpointer user_data)
{
    GNetworkAddressAddressEnumerator* addr_enum = user_data;
    GResolver* resolver = G_RESOLVER(source_object);
     ...
     * If ipv6 hasn't come we should wait a short while for it as RFC 8305
     * suggests.
     */
...
static void g_network_address_address_enumerator_next_async(
    GSocketAddressEnumerator* enumerator, GCancellable* cancellable,
    ...
    if (addr_enum->addresses == NULL && addr_enum->state == RESOLVE_STATE_NONE)
    {
        ...
        if (addr->priv->cached_sockaddrs == NULL)
        {
            ...
            else
            {
                ...
                addr_enum->state = RESOLVE_STATE_WAITING_ON_IPV4 |
                                   RESOLVE_STATE_WAITING_ON_IPV6;
                addr_enum->queued_task = g_steal_pointer(&task);
                /* Lookup in parallel as per RFC 8305 */
                g_resolver_lookup_by_name_with_flags_async(
                    resolver, addr->priv->hostname,
                    G_RESOLVER_NAME_LOOKUP_FLAGS_IPV6_ONLY, cancellable,
                    got_ipv6_addresses, g_object_ref(addr_enum));
                g_resolver_lookup_by_name_with_flags_async(
                    resolver, addr->priv->hostname,
                    G_RESOLVER_NAME_LOOKUP_FLAGS_IPV4_ONLY, cancellable,
...
static GSocketAddress* g_network_address_address_enumerator_next_finish(
...
static void _g_network_address_address_enumerator_init(
    GNetworkAddressAddressEnumerator* enumerator)
{
    enumerator->context = g_main_context_ref_thread_default();
}

static void _g_network_address_address_enumerator_class_init(
    GNetworkAddressAddressEnumeratorClass* addrenum_class)
{
    ...
    enumerator_class->next_async =
        g_network_address_address_enumerator_next_async;
    enumerator_class->next_finish =
    ...
    GSocketConnectable* connectable)
{
    ...
    uri =
        _g_uri_from_authority(self->priv->scheme ? self->priv->scheme : "none",
                              self->priv->hostname, self->priv->port, NULL);

    proxy_enum = g_object_new(G_TYPE_PROXY_ADDRESS_ENUMERATOR, "connectable",
                              connectable, "uri", uri, NULL);
