...
check_suffix:
    ...
    if (frac && pwr)
    {
        ...
        uintmax_t y =
            frac * (zeros_in_pwr == 0 ? 100 : zeros_in_pwr == 1 ? 10 : 1);
...
void strtotimeval_or_err(const char* str, struct timeval* tv,
                         const char* errmesg)
{
    double user_input;
...
char* xstrmode(mode_t mode, char* str)
{
    ...
    str[i++] = (mode & S_ISUID ? (mode & S_IXUSR ? 's' : 'S')
                               : (mode & S_IXUSR ? 'x' : '-'));
    str[i++] = mode & S_IRGRP ? 'r' : '-';
    str[i++] = mode & S_IWGRP ? 'w' : '-';
    str[i++] = (mode & S_ISGID ? (mode & S_IXGRP ? 's' : 'S')
                               : (mode & S_IXGRP ? 'x' : '-'));
    str[i++] = mode & S_IROTH ? 'r' : '-';
    str[i++] = mode & S_IWOTH ? 'w' : '-';
    str[i++] = (mode & S_ISVTX ? (mode & S_IXOTH ? 't' : 'T')
                               : (mode & S_IXOTH ? 'x' : '-'));
    str[i] = '\0';
