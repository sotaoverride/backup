...
_public_ void c_ini_reader_set_mode(CIniReader* reader, unsigned int mode)
{
    ...
    assert(
        !(mode &
          ~(C_INI_MODE_EXTENDED_WHITESPACE | C_INI_MODE_KEEP_DUPLICATE_GROUPS |
...
static int c_ini_reader_parse_entry(CIniReader* reader, CIniRaw* raw,
                                    size_t i_key, size_t i_assignment, size_t n)
{
...
static int c_ini_reader_parse_group(CIniReader* reader, CIniRaw* raw,
                                    size_t i_label, size_t n_label)
{
...
static int c_ini_reader_parse_line(CIniReader* reader, CIniRaw* raw)
{
    ...
    if (data[0] == '[')
    {
        ...
        if (end)
        {
            ...
            if (end - tmp + 1 == (ssize_t)n_tmp)
                return c_ini_reader_parse_group(reader, raw, tmp - raw->data,
                                                n_tmp - 1);
        }
    }
    ...
    if (end)
        return c_ini_reader_parse_entry(reader, raw, data - raw->data,
                                        end - raw->data, n);
...
static int c_ini_reader_append(CIniReader* reader, const uint8_t* data,
                               size_t n_data)
{
    size_t n;
    void* p;
...
_public_ int c_ini_reader_feed(CIniReader* reader, const uint8_t* data,
                               size_t n_data)
{
    const uint8_t* end;
    size_t n;
...
_public_ int c_ini_reader_parse(CIniDomain** domainp, unsigned int mode,
                                const uint8_t* data, size_t n_data)
{
    _cleanup_(c_ini_reader_deinit) CIniReader reader =
        C_INI_READER_NULL(reader);
    int r;
