...
static int mboxctl_directive(struct mboxctl_context* context, const char* cmd)
{
    ...
    rc = sd_bus_message_new_method_call(context->bus, &m, MBOX_DBUS_NAME,
                                        MBOX_DBUS_OBJECT,
                                        MBOX_DBUS_CONTROL_IFACE, cmd);
    if (rc < 0)
    {
...
static int mboxctl_getter(struct mboxctl_context* context, const char* property,
                          uint8_t* resp)
{
    sd_bus_error error = SD_BUS_ERROR_NULL;

    return sd_bus_get_property_trivial(
        context->bus, MBOX_DBUS_NAME, MBOX_DBUS_OBJECT, MBOX_DBUS_CONTROL_IFACE,
...
static int handle_cmd_daemon_state(struct mboxctl_context* context)
{
    ...
    MSG_OUT("Daemon State: %s\n",
            resp == DAEMON_STATE_ACTIVE ? "Active" : "Suspended");
    return 0;
}
...
static int handle_cmd_lpc_state(struct mboxctl_context* context)
{
    ...
    MSG_OUT("LPC Bus Maps: %s\n",
            resp == LPC_STATE_MEM
                ? "BMC Memory"
                : (resp == LPC_STATE_FLASH ? "Flash Device"
...
static int handle_cmd_resume(struct mboxctl_context* context, char* sarg)
{
    ...
    rc = sd_bus_message_new_method_call(context->bus, &m, MBOX_DBUS_NAME,
                                        MBOX_DBUS_OBJECT,
                                        MBOX_DBUS_CONTROL_IFACE, "Resume");
    if (rc < 0)
    {
        MSG_ERR("Failed to init method call: %s\n", strerror(-rc));
        goto out;
...
static int handle_cmd_backend(struct mboxctl_context* context, char* sarg)
{
    ...
    rc = sd_bus_message_new_method_call(context->bus, &m, MBOX_DBUS_NAME,
                                        MBOX_DBUS_OBJECT,
                                        MBOX_DBUS_CONTROL_IFACE, "SetBackend");
    if (rc < 0)
    {
        MSG_ERR("Failed to init method call: %s\n", strerror(-rc));
        goto out;
