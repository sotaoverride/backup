...
static void string_replace_all_occurrences_with_char(char* s, const char* occur,
                                                     char repl_char)
{
    int slen = strlen(s);
    int skip = strlen(occur) -
               1; /* length of the occurence, minus the char we're replacing */
    char* p = s;
...
static int is_valid_index(struct json_object* jo, const char* path,
                          int32_t* idx)
{
                                        ...
                                        struct json_object* value)
{
    ...
    {
        int32_t idx;
        /* RFC (Chapter 4) states that '-' may be used to add new elements to an
         * array */
        if (path[0] == '-' && path[1] == '\0')
    ...
    /* path replacements should have been done in
       json_pointer_get_single_path(), and we should still be good here */
                                      ...
                                      struct json_object** value)
{
    ...
    if (endp)
    {
        *endp = '/'; /* Put the slash back, so that the sanity check passes on
...
int json_pointer_get(struct json_object* obj, const char* path,
...
int json_pointer_getf(struct json_object* obj, struct json_object** res,
                      const char* path_fmt, ...)
{
    char* path_copy = NULL;
    int rc = 0;
...
int json_pointer_set(struct json_object** obj, const char* path,
                     struct json_object* value)
{
    const char* endp;
    char* path_copy = NULL;
...
int json_pointer_setf(struct json_object** obj, struct json_object* value,
                      const char* path_fmt, ...)
{
    char* endp;
    char* path_copy = NULL;
