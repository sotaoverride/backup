...
tt_face_lookup_table(TT_Face face, FT_ULong tag)
{
    ...
    FT_TRACE4(("tt_face_lookup_table: %08p, `%c%c%c%c' -- ", face,
               (FT_Char)(tag >> 24), (FT_Char)(tag >> 16), (FT_Char)(tag >> 8),
               (FT_Char)(tag)));
...
tt_face_goto_table(TT_Face face, FT_ULong tag, FT_Stream stream,
                   FT_ULong* length)
{
    TT_Table table;
    FT_Error error;
...
#define FT_STRUCTURE TT_TableRec
        ...
        FT_FRAME_START(16),       FT_FRAME_ULONG(Tag),
        FT_FRAME_ULONG(CheckSum), FT_FRAME_ULONG(Offset),
        FT_FRAME_ULONG(Length),   FT_FRAME_END};
    ...
    for (nn = 0; nn < sfnt->num_tables; nn++)
    {
        ...
        else if (table.Length > stream->size - table.Offset)
        {
            ...
            if (table.Tag == TTAG_hmtx || table.Tag == TTAG_vmtx)
                valid_entries++;
            else
            {
                FT_TRACE2(("check_table_dir: table entry %d invalid\n", nn));
        ...
        if (table.Tag == TTAG_head || table.Tag == TTAG_bhed)
        {
            ...
            if (FT_STREAM_SEEK(table.Offset + 12) || FT_READ_ULONG(magic))
                goto Exit;
            ...
            if (entry.Tag == TTAG_hmtx || entry.Tag == TTAG_vmtx)
            {
                ...
                FT_TRACE2(
                    ("  %c%c%c%c  %08lx  %08lx  %08lx"
                     ...
                     (FT_Char)(entry.Tag >> 8), (FT_Char)(entry.Tag),
                     entry.Offset, entry.Length, entry.CheckSum, old_length));
            }
            else
                continue;
        }
 ...
 *   FreeType error code.  0 means success.
 */
static FT_Error tt_face_load_generic_header(TT_Face face, FT_Stream stream,
                                            FT_ULong tag)
{
    FT_Error error;
    TT_Header* header;
...
#define FT_STRUCTURE TT_MaxProfile
        ...
        FT_FRAME_START(6), FT_FRAME_LONG(version), FT_FRAME_USHORT(numGlyphs),
        FT_FRAME_END};
...
#define FT_STRUCTURE TT_NameTableRec
        ...
        FT_FRAME_START(6), FT_FRAME_USHORT(format),
        FT_FRAME_USHORT(numNameRecords), FT_FRAME_USHORT(storageOffset),
        FT_FRAME_END};
...
#define FT_STRUCTURE TT_LangTagRec
        ...
        /* no FT_FRAME_START */
        FT_FRAME_USHORT(stringLength), FT_FRAME_USHORT(stringOffset),
        FT_FRAME_END};
    ...
    /* load name records */
    {
        ...
        (void)FT_RENEW_ARRAY(table->names, table->numNameRecords, count);
...
#define FT_STRUCTURE TT_OS2
    ...
    static const FT_Frame_Field os2_fields_extra1[] = {
        FT_FRAME_START(8), FT_FRAME_ULONG(ulCodePageRange1),
        FT_FRAME_ULONG(ulCodePageRange2), FT_FRAME_END};
    ...
    static const FT_Frame_Field os2_fields_extra5[] = {
        FT_FRAME_START(4), FT_FRAME_USHORT(usLowerOpticalPointSize),
        FT_FRAME_USHORT(usUpperOpticalPointSize), FT_FRAME_END};
...
tt_face_load_gasp(TT_Face face, FT_Stream stream)
{
    ...
    for (j = 0; j < num_ranges; j++)
    {
        ...
        FT_TRACE3(("gaspRange %d: rangeMaxPPEM %5d, rangeGaspBehavior 0x%x\n",
                   j, gaspranges[j].maxPPEM, gaspranges[j].gaspFlag));
    }
