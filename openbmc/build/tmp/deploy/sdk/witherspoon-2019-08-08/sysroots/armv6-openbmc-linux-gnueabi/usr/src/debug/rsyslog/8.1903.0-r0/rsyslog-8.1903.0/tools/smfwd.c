...
lenTotal =
    1 + lenPRI + 1 + lenTimeStamp + 1 + lenHOSTNAME + 1 + lenTAG + lenMSG + 1;
if (pMSG[0] != ' ')
          ...
          BEGINmodExit CODESTARTmodExit ENDmodExit
              ...
              BEGINqueryEtryPt CODESTARTqueryEtryPt
                  CODEqueryEtryPt_STD_SMOD_QUERIES ENDqueryEtryPt
                      ...
                      BEGINmodInit(smfwd) CODESTARTmodInit* ipIFVersProvided =
                          CURR_MOD_IF_VERSION; /* we only support the current
                                                  interface specification */
CODEmodInit_QueryRegCFSLineHdlr

    dbgprintf("rsyslog standard (network) forward format strgen init called, "
