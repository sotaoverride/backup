...
static int logic_field(pam_handle_t* pamh, const void* me, const char* x,
                       int rule,
                       int (*agrees)(pam_handle_t* pamh, const void*,
                                     const char*, int, int))
{
...
static int is_same(pam_handle_t* pamh UNUSED, const void* A, const char* b,
...
static int check_time(pam_handle_t* pamh, const void* AT, const char* times,
                      int len, int rule)
{
    int not, pass;
                         ...
                         const char* tty, const char* user)
{
    ...
    do
    {
        ...
        {
            pam_syslog(pamh, LOG_ERR, "%s: malformed rule #%d", PAM_TIME_CONF,
                       count);
            continue;
        }
    ...
    if (rv != PAM_SUCCESS)
    {
        ...
        if (!(ctrl & PAM_NO_AUDIT))
        {
            pam_modutil_audit_write(
                pamh, AUDIT_ANOM_LOGIN_TIME, "pam_time",
                rv); /* ignore return value as we fail anyway */
        }
#endif
