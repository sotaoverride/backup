...
#define BOOST_BGL_ONE_PARAM_CREF(name, key)                                    \
    template <typename PType>                                                  \
    bgl_named_params<PType, BOOST_PP_CAT(key, _t), self> name(const PType& p)  \
        const                                                                  \
    {                                                                          \
...
inline const typename lookup_named_param_def<Tag, Args, param_not_found>::type&
...
template <typename T>
inline bool is_default_param(const T&)
{
    return false;
}
...
template <bool Found>
struct choose_impl_helper;

template <>
struct choose_impl_helper<false>
{
    template <typename Param, typename Graph, typename PropertyTag>
    static typename property_map<typename boost::remove_const<Graph>::type,
...
template <>
struct choose_impl_helper<true>
{
...
{
    return detail::choose_impl_helper<
        !boost::is_same<Param, param_not_found>::value>::f(boost::mpl::true_(),
                                                           g, p, tag);
}
...
{
    return detail::choose_impl_helper<
        !boost::is_same<Param, param_not_found>::value>::f(boost::mpl::false_(),
                                                           g, p, tag);
}
...
struct edge_capacity_value
{
    ...
    typedef typename detail::choose_impl_result<
...
struct convert_bgl_params_to_boost_parameter<boost::no_property>
{
    ...
    static type conv(const boost::no_property&)
    {
        return type();
    }
};
...
{
    typedef boost::parameter::aux::empty_arg_list type;
    static type conv(const boost::no_named_parameters&)
    {
        return type();
    }
};
...
struct parameter_exists
    : boost::mpl::not_<boost::is_same<
          typename boost::parameter::binding<
              ArgPack, KeywordType, bgl_parameter_not_found_type>::type,
...
#define BOOST_GRAPH_DECLARE_CONVERTED_PARAMETERS(old_type, old_var)            \
    ...
    arg_pack_type arg_pack =                                                   \
        boost::detail::convert_bgl_params_to_boost_parameter<old_type>::conv(  \
            old_var);
...
struct override_const_property_result
{
    typedef typename override_const_property_t<
        typename boost::parameter::value_type<ArgPack, Tag, int>::type, Prop,
        Graph,
        boost::detail::parameter_exists<ArgPack, Tag>::value>::result_type type;
};
                            ...
                            const Graph& g, Prop)
{
    return override_const_property_t<
        typename boost::parameter::value_type<ArgPack, Tag, int>::type, Prop,
        Graph, boost::detail::parameter_exists<ArgPack, Tag>::value>()(
        g, ap[t | 0]);
}
...
struct override_property_t
{
    ...
    result_type
        operator()(const Graph&,
                   const typename boost::add_reference<ArgType>::type a) const
    {
        return a;
    }
};
...
typename override_property_result<ArgPack, Tag, Prop, Graph>::type
    override_property(const ArgPack& ap,
                      const boost::parameter::keyword<Tag>& t, const Graph& g,
                      Prop)
{
...
#define BOOST_GRAPH_MAKE_PAIR_PARAM(z, i, _)                                   \
    const boost::parameter::aux::tagged_argument<                              \
        BOOST_PP_CAT(Keyword, i), BOOST_PP_CAT(Arg, i)>& BOOST_PP_CAT(kw, i)
...
#define BOOST_GRAPH_MAKE_AP_TYPE_SPECIALIZATION(z, i, _)                       \
    ...
    struct make_arg_pack_type<void(BOOST_PP_ENUM_PARAMS(i, Keyword),           \
                                   BOOST_PP_ENUM_PARAMS(i, Arg))>              \
    {                                                                          \
        typedef BOOST_PP_REPEAT(                                               \
            i, BOOST_GRAPH_OPENING_PART_OF_PAIR,                               \
...
#define BOOST_GRAPH_MAKE_FORWARDING_FUNCTION_ONEX(z, nnamed, name, nfixed)     \
    template <BOOST_PP_ENUM_PARAMS(nfixed, typename Param)                     \
                  BOOST_PP_ENUM_TRAILING_PARAMS(nnamed, typename Keyword)      \
                      BOOST_PP_ENUM_TRAILING_PARAMS(nnamed, typename Arg)>     \
        typename boost::result_of<detail::BOOST_PP_CAT(name, _impl) <          \
                                  BOOST_PP_ENUM_PARAMS(nfixed, Param)>(        \
            BOOST_PP_ENUM_PARAMS(nfixed, Param) BOOST_PP_COMMA_IF(nfixed)      \
                const typename boost::detail::make_arg_pack_type<void(         \
                    BOOST_PP_ENUM_PARAMS(nnamed, Keyword) BOOST_PP_COMMA_IF(   \
                        nnamed) BOOST_PP_ENUM_PARAMS(nnamed, Arg))>::type&) >  \
        ::type name(BOOST_PP_ENUM_BINARY_PARAMS(nfixed, const Param, &param)   \
                        BOOST_PP_ENUM_TRAILING(                                \
                            nnamed, BOOST_GRAPH_MAKE_PAIR_PARAM, ~))           \
    {                                                                          \
        return detail::BOOST_PP_CAT(                                           \
            name, _impl)<BOOST_PP_ENUM_PARAMS(nfixed, Param)>()(               \
            BOOST_PP_ENUM_PARAMS(nfixed, param),                               \
            (boost::parameter::aux::empty_arg_list()                           \
                 BOOST_PP_ENUM_TRAILING_PARAMS(nnamed, kw)));                  \
    }
...
#define BOOST_GRAPH_MAKE_OLD_STYLE_PARAMETER_FUNCTION(name, nfixed)            \
    ...
    typename boost::result_of<                                                 \
        ::boost::graph::detail::BOOST_PP_CAT(name, _impl) BOOST_PP_EXPR_IF(    \
            nfixed, <) BOOST_PP_ENUM_PARAMS(nfixed, Param)                     \
            BOOST_PP_EXPR_IF(nfixed, >)(                                       \
                BOOST_PP_ENUM_PARAMS(nfixed, Param) BOOST_PP_COMMA_IF(nfixed)  \
                    const typename boost::detail::                             \
    ...
    {                                                                          \
        ...
        BOOST_GRAPH_DECLARE_CONVERTED_PARAMETERS(old_style_params_type,        \
                                                 old_style_params)             \
    ...
    BOOST_PP_EXPR_IF(                                                          \
        nfixed,                                                                \
        template <)                                                            \
        BOOST_PP_ENUM_PARAMS(nfixed, typename Param) BOOST_PP_EXPR_IF(nfixed,  \
                                                                      >)       \
            BOOST_PP_EXPR_IF(nfixed, typename) boost::result_of<               \
                ::boost::graph::detail::BOOST_PP_CAT(                          \
                    name, _impl) BOOST_PP_EXPR_IF(nfixed, <)                   \
                    BOOST_PP_ENUM_PARAMS(nfixed, Param) BOOST_PP_EXPR_IF(      \
                        nfixed, >)(BOOST_PP_ENUM_PARAMS(                       \
                        nfixed, Param) BOOST_PP_COMMA_IF(nfixed) const boost:: \
                                       parameter::aux::empty_arg_list&)>::type \
                name(BOOST_PP_ENUM_BINARY_PARAMS(nfixed, const Param, &param)) \
    {                                                                          \
        BOOST_GRAPH_DECLARE_CONVERTED_PARAMETERS(boost::no_named_parameters,   \
...
{
    typedef typename boost::remove_const<typename override_const_property_t<
    ...
    static map_type make_map(const Graph& g, Value v, const PM&,
                             const ArgPack& ap)
    {
...
struct map_maker
{
    ...
    typedef map_maker_helper<
        has_map, Graph, ArgPack, ValueType,
...
class make_property_map_from_arg_pack_gen<MapTag, void>
{
  public:
    ...
    typename map_maker<Graph, ArgPack, MapTag, ValueType>::map_type
        operator()(const Graph& g, const ArgPack& ap,
...
template <bool Exists, class Graph, class ArgPack, class KeyT, class ValueT,
...
struct priority_queue_maker_helper
{
    ...
    static priority_queue_type make_queue(const Graph&, const ArgPack&, KeyT,
                                          const Q& q)
    {
        return q;
    }
...
template <class Graph, class ArgPack, class KeyT, class ValueT, class KeyMapTag,
          class IndexInHeapMapTag, class Compare, class Q>
                                   ...
                                   KeyMapTag, IndexInHeapMapTag, Compare, Q>
{
    ...
    typedef typename map_maker<Graph, ArgPack, IndexInHeapMapTag,
                               default_index_in_heap_type>::helper::map_type
        index_in_heap_map;
    typedef boost::d_ary_heap_indirect<
        ValueT, 4, index_in_heap_map,
        typename map_maker<Graph, ArgPack, KeyMapTag, KeyT>::helper::map_type,
        Compare>
        priority_queue_type;

    static priority_queue_type make_queue(const Graph& g, const ArgPack& ap,
                                          KeyT defaultKey, const Q&)
    {
        return priority_queue_type(
            map_maker<Graph, ArgPack, KeyMapTag, KeyT>::make_map(g, ap,
                                                                 defaultKey),
...
struct priority_queue_maker
{
    BOOST_STATIC_CONSTANT(
        bool, g_hasQ = (parameter_exists<ArgPack, PriorityQueueTag>::value));
    typedef boost::reference_wrapper<int> int_refw;
    typedef typename boost::parameter::value_type<
        ArgPack, PriorityQueueTag, int_refw>::type param_value_type_wrapper;
    ...
    typedef priority_queue_maker_helper<g_hasQ, Graph, ArgPack, KeyT, ValueT,
                                        KeyMapTag, IndexInHeapMapTag, Compare,
                                        param_value_type_no_const>
        helper;
    ...
    static priority_queue_type make_queue(const Graph& g, const ArgPack& ap,
                                          KeyT defaultKey)
    {
        return helper::make_queue(
            g, ap, defaultKey,
            ap[::boost::parameter::keyword<PriorityQueueTag>::instance | 0]);
    }
};
...
struct make_priority_queue_from_arg_pack_gen
{
    ...
    struct result
    {
        typedef typename remove_const<typename remove_reference<
            typename function_traits<F>::arg1_type>::type>::type graph_type;
        typedef typename remove_const<typename remove_reference<
            typename function_traits<F>::arg2_type>::type>::type arg_pack_type;
        typedef typename priority_queue_maker<
            graph_type, arg_pack_type, KeyT, ValueT, PriorityQueueTag,
            KeyMapTag, IndexInHeapMapTag, Compare>::priority_queue_type type;
    };

    template <class Graph, class ArgPack>
    typename priority_queue_maker<
        Graph, ArgPack, KeyT, ValueT, PriorityQueueTag, KeyMapTag,
        IndexInHeapMapTag, Compare>::priority_queue_type
        operator()(const Graph& g, const ArgPack& ap) const
    {
        return priority_queue_maker<
            Graph, ArgPack, KeyT, ValueT, PriorityQueueTag, KeyMapTag,
            IndexInHeapMapTag, Compare>::make_queue(g, ap, defaultKey);
    }
};

template <typename G>
typename boost::graph_traits<G>::vertex_descriptor get_null_vertex(const G&)
{
    ...
    get_default_starting_vertex(const G& g)
{
    std::pair<typename boost::graph_traits<G>::vertex_iterator,
        ...
        iters = vertices(g);
    return (iters.first == iters.second) ? boost::graph_traits<G>::null_vertex()
                                         : *iters.first;
}
...
// Wrapper to avoid instantiating numeric_limits when users provide distance_inf
// value manually
template <typename T>
struct get_max
{
