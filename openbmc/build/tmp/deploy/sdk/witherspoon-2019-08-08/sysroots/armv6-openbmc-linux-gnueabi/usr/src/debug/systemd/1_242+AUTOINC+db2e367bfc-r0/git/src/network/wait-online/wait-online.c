...
static int help(void)
{
    ...
    printf(
        ...
        "                            Block until at least these interfaces "
        "have appeared\n"
        "     --ignore=INTERFACE     Don't take these interfaces into account\n"
        "  -o --operational-state=OPERSTATE\n"
        "                            Required operational state\n"
        "     --any                  Wait until at least one of the interfaces "
        "is online\n"
        "     --timeout=SECS         Maximum time to wait for network "
        "connectivity\n"
...
static int parse_interface_with_operstate(const char* str)
{
    ...
    if (r == 0)
        return log_error_errno(SYNTHETIC_ERRNO(EINVAL),
                               "Interface name %s is already specified",
                               ifname);
...
static int run(int argc, char* argv[])
{
    ...
    r = manager_new(&m, arg_interfaces, arg_ignore, arg_required_operstate,
                    arg_any, arg_timeout);
    if (r < 0)
