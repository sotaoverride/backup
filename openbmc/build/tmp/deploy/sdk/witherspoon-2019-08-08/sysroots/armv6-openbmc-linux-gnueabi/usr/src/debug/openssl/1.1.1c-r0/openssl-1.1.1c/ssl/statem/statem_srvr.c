...
static int ossl_statem_server13_read_transition(SSL* s, int mt)
{
    ...
    switch (st->hand_state)
    {
        ...
        case TLS_ST_OK:
            /*
             * Its never ok to start processing handshake messages in the middle
             * of early data (i.e. before we've received the end of early data
             * alert)
             */
...
int ossl_statem_server_read_transition(SSL* s, int mt)
{
    ...
    switch (st->hand_state)
    {
        ...
        case TLS_ST_SW_SRVR_DONE:
             ...
             *      AND
             *      b) We are running SSL3 (in TLS1.0+ the client must return a
             * 0 list if we requested a certificate)
             */
            if (mt == SSL3_MT_CLIENT_KEY_EXCHANGE)
            {
                if (s->s3->tmp.cert_request)
                {
                    if (s->version == SSL3_VERSION)
                    {
                        ...
                        {
                            /*
                             * This isn't an unexpected message as such - we're
                             * just not going to accept it because we require a
                             * client cert.
                             */
        ...
        case TLS_ST_SR_KEY_EXCH:
             ...
             * received a Certificate from the client. If so then
             * |s->session->peer| will be non NULL. In some instances a
             * CertificateVerify message is not required even if the peer has
             * sent a Certificate (e.g. such as in the case of static DH). In
             * that case |st->no_cert_verify| should be set.
             */
            if (s->session->peer == NULL || st->no_cert_verify)
            {
                if (mt == SSL3_MT_CHANGE_CIPHER_SPEC)
                {
                     ...
                     * pub key in a certificate, the CertificateVerify message
                     * is not sent. Also for GOST ciphersuites when the client
                     * uses its key from the certificate for key exchange.
                     */
...
static WRITE_TRAN ossl_statem_server13_write_transition(SSL* s)
{
    ...
    switch (st->hand_state)
    {
        ...
        case TLS_ST_SR_FINISHED:
            /*
             * Technically we have finished the handshake at this point, but
             * we're going to remain "in_init" for now and write out any session
             * tickets immediately.
             */
            ...
            {
                /*
                 * If we're not going to renew the ticket then we just finish
        ...
        case TLS_ST_SW_SESSION_TICKET:
             ...
             * Following an initial handshake we send the number of tickets we
    ...
    switch (st->hand_state)
    {
        ...
        case TLS_ST_SW_SESSION_TICKET:
            if (SSL_IS_TLS13(s) && s->sent_tickets == 0)
            {
                 ...
                 * off the handshake, but keep the various buffers active.
                 *
                 * Calls SSLfatal as required.
                 */
            ...
            if (SSL_IS_DTLS(s))
            {
                 ...
                 * flight unless we need to, so we don't use the timer
                 */
                st->use_timer = 0;
            }
            break;
        ...
        case TLS_ST_SW_CHANGE:
            ...
            if (SSL_IS_DTLS(s))
            {
                /*
                 * We're into the last flight. We don't retransmit the last
                 * flight unless we need to, so we don't use the timer. This
                 * might have already been set to 0 if we sent a
                 * NewSessionTicket message, but we'll set it again here in case
                 * we didn't.
                 */
                st->use_timer = 0;
            }
    ...
    switch (st->hand_state)
    {
        ...
        case TLS_ST_SW_CHANGE:
            ...
            if (SSL_IS_TLS13(s))
            {
                if (!s->method->ssl3_enc->setup_key_block(s) ||
                    !s->method->ssl3_enc->change_cipher_state(
                        s, SSL3_CC_HANDSHAKE | SSL3_CHANGE_CIPHER_SERVER_WRITE))
                {
                    /* SSLfatal() already called */
                    return WORK_ERROR;
                }
                ...
                /*
                 * We don't yet know whether the next record we are going to
                 * receive is an unencrypted alert, an encrypted alert, or an
                 * encrypted handshake message. We temporarily tolerate
                 * unencrypted alerts.
                 */
            ...
            if (SSL_IS_DTLS(s) && !s->hit)
            {
                 ...
                 * no SCTP used.
                 */
                BIO_ctrl(SSL_get_wbio(s), BIO_CTRL_DGRAM_SCTP_NEXT_AUTH_KEY, 0,
                         NULL);
            }
#endif
        ...
        case TLS_ST_SW_FINISHED:
            ...
            if (SSL_IS_TLS13(s))
            {
                if (!s->method->ssl3_enc->generate_master_secret(
                        s, s->master_secret, s->handshake_secret, 0,
        ...
        case TLS_ST_SW_SESSION_TICKET:
            ...
            if (SSL_IS_TLS13(s) && statem_flush(s) != 1)
            {
                ...
                {
                    /*
                     * We ignore connection closed errors in TLSv1.3 when
                     * sending a NewSessionTicket and behave as if we were
                     * successful. This is so that we are still able to read
                     * data sent to us by a client that closes soon after the
                     * end of the handshake without waiting to read our
                     * post-handshake NewSessionTickets.
                     */
...
int dtls_construct_hello_verify_request(SSL* s, WPACKET* pkt)
{
    ...
    if (s->ctx->app_gen_cookie_cb == NULL ||
        s->ctx->app_gen_cookie_cb(s, s->d1->cookie, &cookie_leni) == 0 ||
        cookie_leni > 255)
    {
    ...
    if (!dtls_raw_hello_verify_request(pkt, s->d1->cookie, s->d1->cookie_len))
    {
        SSLfatal(s, SSL_AD_NO_ALERT, SSL_F_DTLS_CONSTRUCT_HELLO_VERIFY_REQUEST,
                 ERR_R_INTERNAL_ERROR);
        return 0;
...
static void ssl_check_for_safari(SSL* s, const CLIENTHELLO_MSG* hello)
{
    ...
    if (!PACKET_forward(&tmppkt, 2) || !PACKET_get_net_2(&tmppkt, &type) ||
    ...
    s->s3->is_probably_safari =
        PACKET_equal(&tmppkt, kSafariExtensionsBlock, ext_len);
}
#endif /* !OPENSSL_NO_EC */
...
MSG_PROCESS_RETURN tls_process_client_hello(SSL* s, PACKET* pkt)
{
    ...
    if (clienthello->isv2)
    {
         ...
         * here rather than sizeof(clienthello->random) because that is the
         * limit for SSLv3 and it is fixed. It won't change even if
         * sizeof(clienthello->random) does.
         */
        challenge_len =
            challenge_len > SSL3_RANDOM_SIZE ? SSL3_RANDOM_SIZE : challenge_len;
...
static int tls_early_post_process_client_hello(SSL* s)
{
    ...
    if (s->ctx->client_hello_cb != NULL)
    {
        ...
        switch (s->ctx->client_hello_cb(s, &al, s->ctx->client_hello_cb_arg))
        {
            ...
            default:
                SSLfatal(s, al, SSL_F_TLS_EARLY_POST_PROCESS_CLIENT_HELLO,
    ...
    if (SSL_IS_DTLS(s))
    {
        ...
        if (SSL_get_options(s) & SSL_OP_COOKIE_EXCHANGE)
        {
            ...
            {
                if (s->ctx->app_verify_cookie_cb(
                        s, clienthello->dtls_cookie,
    ...
    if (!tls_parse_extension(s, TLSEXT_IDX_extended_master_secret,
                             SSL_EXT_CLIENT_HELLO, clienthello->pre_proc_exts,
                             NULL, 0))
    {
        /* SSLfatal() already called */
        goto err;
    ...
    if (!s->hit && s->version >= TLS1_VERSION && !SSL_IS_TLS13(s) &&
        !SSL_IS_DTLS(s) && s->ext.session_secret_cb)
    {
        ...
        if (s->ext.session_secret_cb(s, s->session->master_key,
                                     &master_key_length, ciphers, &pref_cipher,
...
int tls_handle_alpn(SSL* s)
{
    ...
    {
        int r = s->ctx->ext.alpn_select_cb(
            s, &selected, &selected_len, s->s3->alpn_proposed,
        ...
        if (r == SSL_TLSEXT_ERR_OK)
        {
            ...
            if (s->session->ext.alpn_selected == NULL ||
                ...
                memcmp(selected, s->session->ext.alpn_selected, selected_len) !=
                    0)
            {
                ...
                if (!s->hit)
                {
                    ...
                    {
                        SSLfatal(s, SSL_AD_INTERNAL_ERROR,
                                 SSL_F_TLS_HANDLE_ALPN, ERR_R_INTERNAL_ERROR);
                        return 0;
                    }
                    s->session->ext.alpn_selected =
                        OPENSSL_memdup(selected, selected_len);
                    if (s->session->ext.alpn_selected == NULL)
                    {
                        SSLfatal(s, SSL_AD_INTERNAL_ERROR,
                                 SSL_F_TLS_HANDLE_ALPN, ERR_R_INTERNAL_ERROR);
        ...
        if (!s->hit || SSL_IS_TLS13(s))
        {
            ...
            if (!s->hit)
            {
                ...
                if (s->not_resumable_session_cb != NULL)
                    s->session->not_resumable = s->not_resumable_session_cb(
                        s, ((s->s3->tmp.new_cipher->algorithm_mkey &
...
int tls_construct_server_hello(SSL* s, WPACKET* pkt)
{
     ...
     */
    if (s->session->not_resumable ||
        (!(s->ctx->session_cache_mode & SSL_SESS_CACHE_SERVER) && !s->hit))
...
int tls_construct_server_key_exchange(SSL* s, WPACKET* pkt)
{
        ...
        if (type & (SSL_kDHE | SSL_kDHEPSK))
    {
        ...
        if (!ssl_security(s, SSL_SECOP_TMP_DH, EVP_PKEY_security_bits(pkdhp), 0,
                          pkdhp))
        {
            SSLfatal(s, SSL_AD_HANDSHAKE_FAILURE,
                     SSL_F_TLS_CONSTRUCT_SERVER_KEY_EXCHANGE,
        ...
        if (type & (SSL_kECDHE | SSL_kECDHEPSK))
    {
        ...
        encodedlen =
            EVP_PKEY_get1_tls_encodedpoint(s->s3->tmp.pkey, &encodedPoint);
        if (encodedlen == 0)
        {
            SSLfatal(s, SSL_AD_INTERNAL_ERROR,
    ...
    if (lu != NULL)
    {
        ...
        tbslen = construct_key_exchange_tbs(
            s, &tbs, s->init_buf->data + paramoffset, paramlen);
        if (tbslen == 0)
        {
            /* SSLfatal() already called */
            goto err;
...
int tls_construct_certificate_request(SSL* s, WPACKET* pkt)
{
    if (SSL_IS_TLS13(s))
    {
        ...
        if (!tls_construct_extensions(
                s, pkt, SSL_EXT_TLS1_3_CERTIFICATE_REQUEST, NULL, 0))
        {
            /* SSLfatal() already called */
            return 0;
        }
    ...
    if (!WPACKET_start_sub_packet_u8(pkt) || !ssl3_get_req_cert_type(s, pkt) ||
        !WPACKET_close(pkt))
    {
        SSLfatal(s, SSL_AD_INTERNAL_ERROR,
    ...
    if (SSL_USE_SIGALGS(s))
    {
        ...
        if (!WPACKET_start_sub_packet_u16(pkt) ||
            ...
            !tls12_copy_sigalgs(s, pkt, psigs, nl) || !WPACKET_close(pkt))
        {
            SSLfatal(s, SSL_AD_INTERNAL_ERROR,
                     SSL_F_TLS_CONSTRUCT_CERTIFICATE_REQUEST,
                     ERR_R_INTERNAL_ERROR);
...
static int tls_process_cke_rsa(SSL* s, PACKET* pkt)
{
    ...
    if (RAND_priv_bytes(rand_premaster_secret, sizeof(rand_premaster_secret)) <=
        0)
    {
        SSLfatal(s, SSL_AD_INTERNAL_ERROR, SSL_F_TLS_PROCESS_CKE_RSA,
                 ERR_R_INTERNAL_ERROR);
    ...
    decrypt_len = (int)RSA_private_decrypt(
        (int)PACKET_remaining(&enc_premaster), PACKET_data(&enc_premaster),
    ...
    if (s->options & SSL_OP_TLS_ROLLBACK_BUG)
    {
        ...
        workaround_good &= constant_time_eq_8(rsa_decrypt[padding_len + 1],
    ...
    for (j = 0; j < sizeof(rand_premaster_secret); j++)
    {
        rsa_decrypt[padding_len + j] =
            constant_time_select_8(decrypt_good, rsa_decrypt[padding_len + j],
...
WORK_STATE tls_post_process_client_key_exchange(SSL* s, WORK_STATE wst)
{
    ...
    if (wst == WORK_MORE_A)
    {
        if (SSL_IS_DTLS(s))
        {
            ...
            if (SSL_export_keying_material(s, sctpauthkey, sizeof(sctpauthkey),
                                           labelbuffer, labellen, NULL, 0,
                                           0) <= 0)
            {
                SSLfatal(s, SSL_AD_INTERNAL_ERROR,
                                 ...
                                 unsigned char* tick_nonce)
{
     ...
     * timeout.
     */
    if (!WPACKET_put_bytes_u32(
            pkt, (s->hit && !SSL_IS_TLS13(s)) ? 0 : s->session->timeout))
    {
                                      ...
                                      unsigned char* tick_nonce)
{
    ...
    if (tctx->ext.ticket_key_cb)
    {
        ...
        int ret = tctx->ext.ticket_key_cb(s, key_name, iv, ctx, hctx, 1);
    ...
    else
    {
        ...
        if (RAND_bytes(iv, iv_len) <= 0 ||
            ...
            !HMAC_Init_ex(hctx, tctx->ext.secure->tick_hmac_key,
                          sizeof(tctx->ext.secure->tick_hmac_key), EVP_sha256(),
                          NULL))
        {
            SSLfatal(s, SSL_AD_INTERNAL_ERROR, SSL_F_CONSTRUCT_STATELESS_TICKET,
                     ERR_R_INTERNAL_ERROR);
    ...
    if (!WPACKET_get_total_written(pkt, &macoffset)
        ...
        !WPACKET_reserve_bytes(pkt, slen + EVP_MAX_BLOCK_LENGTH, &encdata1)
        /* Encrypt session data */
        || !EVP_EncryptUpdate(ctx, encdata1, &len, senc, slen) ||
        !WPACKET_allocate_bytes(pkt, len, &encdata2) || encdata1 != encdata2 ||
        !EVP_EncryptFinal(ctx, encdata1 + len, &lenfinal) ||
        !WPACKET_allocate_bytes(pkt, lenfinal, &encdata2) ||
        encdata1 + len != encdata2 ||
        len + lenfinal > slen + EVP_MAX_BLOCK_LENGTH ||
        !WPACKET_get_total_written(pkt, &macendoffset) ||
        !HMAC_Update(hctx, (unsigned char*)s->init_buf->data + macoffset,
                     macendoffset - macoffset) ||
        !WPACKET_reserve_bytes(pkt, EVP_MAX_MD_SIZE, &macdata1) ||
        !HMAC_Final(hctx, macdata1, &hlen) || hlen > EVP_MAX_MD_SIZE ||
        !WPACKET_allocate_bytes(pkt, hlen, &macdata2) || macdata1 != macdata2)
    {
        SSLfatal(s, SSL_AD_INTERNAL_ERROR, SSL_F_CONSTRUCT_STATELESS_TICKET,
                 ERR_R_INTERNAL_ERROR);
        goto err;
...
int tls_construct_new_session_ticket(SSL* s, WPACKET* pkt)
{
    ...
    if (SSL_IS_TLS13(s))
    {
        ...
        if (!tls13_hkdf_expand(s, md, s->resumption_master_secret, nonce_label,
                               sizeof(nonce_label) - 1, tick_nonce,
    ...
    if (SSL_IS_TLS13(s) &&
        ((s->options & SSL_OP_NO_TICKET) != 0 ||
         (s->max_early_data > 0 && (s->options & SSL_OP_NO_ANTI_REPLAY) == 0)))
    {
    ...
    else if (!construct_stateless_ticket(s, pkt, age_add_u.age_add, tick_nonce))
    {
        /* SSLfatal() already called */
        goto err;
    }
    ...
    if (SSL_IS_TLS13(s))
    {
        if (!tls_construct_extensions(s, pkt, SSL_EXT_TLS1_3_NEW_SESSION_TICKET,
                                      NULL, 0))
        {
            /* SSLfatal() already called */
            goto err;
