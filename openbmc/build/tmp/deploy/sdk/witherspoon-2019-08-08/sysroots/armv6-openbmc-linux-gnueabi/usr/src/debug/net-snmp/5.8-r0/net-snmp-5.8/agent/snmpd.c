...
netsnmp_feature_want(logging_file) netsnmp_feature_want(logging_stdio)
...
static void usage(char* prog)
{
    ...
    printf("  -m MIBLIST\t\tuse MIBLIST instead of the default MIB list\n"
           ...
           "  \t\t\t  Note that some parameters are not relevant when running "
           "as a service\n"
#endif
#if HAVE_UNISTD_H
 ...
 */
#ifdef WIN32SERVICE
static int SnmpDaemonMain(int argc, TCHAR* argv[])
#else
...
#endif
{
    ...
    while ((arg = getopt(argc, argv, options)) != EOF)
    {
        switch (arg)
        {
            ...
            case 'l':
                printf("Warning: -l option is deprecated, use -Lf <file> "
                       "instead\n");
                if (optarg != NULL)
                {
            ...
            case 'S':
                printf("Warning: -S option is deprecated, use -Ls <facility> "
                       "instead\n");
                if (optarg != NULL)
                {
                    switch (*optarg)
                    ...
                    snmp_enable_syslog_ident(snmp_log_syslogname(NULL),
                                             Facility);
                    log_set = 1;
                }
                else
    ...
    if (optind < argc)
    {
        ...
        fprintf(stderr, "You specified ports to open; this agent was built to "
    ...
    (defined(HAVE_CHOWN) || defined(HAVE_SETGID) || defined(HAVE_SETUID))
    {
                                      ...
                                      NETSNMP_DS_AGENT_USERID)) > 0)
        {
            ...
            if (info)
            {
                DEBUGMSGTL(("snmpd/main", "Supplementary groups for %s.\n",
                            info->pw_name));
                if (initgroups(info->pw_name,
                               (gid != 0 ? (gid_t)gid : info->pw_gid)) == -1)
                {
...
static void create_stdin_waiter_thread(void)
{
    ...
    s_thread_handle =
        (HANDLE)_beginthreadex(0, 0, wait_for_stdin, 0, 0, &s_threadid);
    netsnmp_assert(s_thread_handle != 0);
}
...
static int receive(void)
{
    ...
    reselect:
        ...
        DEBUGMSGTL(
            ("snmpd/select", "select( numfds=%d, ..., tvp=%p)\n", numfds, tvp));
        if (tvp)
            DEBUGMSGTL(("timer", "tvp %ld.%ld\n", (long)tvp->tv_sec,
                        (long)tvp->tv_usec));
        count = netsnmp_large_fd_set_select(numfds, &readfds, &writefds,
                                            &exceptfds, tvp);
...
int __cdecl _tmain(int argc, TCHAR* argv[])
{
...
#ifdef IFDESCR
        _T("SNMPv2c / SNMPv3 command responder from Net-SNMP. Supports MIB ")
    ...
    switch (nRunType)
    {
        case REGISTER_SERVICE:
            ...
            return RegisterService(lpszServiceName, lpszServiceDisplayName,
