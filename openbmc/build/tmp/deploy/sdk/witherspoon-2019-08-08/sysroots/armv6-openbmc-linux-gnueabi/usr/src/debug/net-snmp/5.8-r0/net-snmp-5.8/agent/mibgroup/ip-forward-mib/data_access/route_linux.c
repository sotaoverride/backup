...
static int _type_from_flags(unsigned int flags)
{
    /*
...
static int _load_ipv4(netsnmp_container* container, u_long* index)
{
    FILE* in;
    char line[256];
    ...
    DEBUGMSGTL(("access:route:container", "route_container_arch_load ipv4\n"));
    ...
    while (fgets(line, sizeof(line), in))
    {
        ...
        /*
         * as with 1.99.14:
         *    Iface Dest     GW       Flags RefCnt Use Met Mask     MTU  Win
         * IRTT BE eth0  00000000 C0A80101 0003  0      0   0   FFFFFFFF 1500 0
         * 0 LE eth0  00000000 0101A8C0 0003  0      0   0   00FFFFFF    0 0   0
         */
        rc = sscanf(line, "%s %x %x %x %d %u %d %x %*d %*d %*d\n", rtent_name,
                     ...
                     */
                    &flags, &refcnt, &use, &entry->rt_metric1, &mask);
    ...
    inetCidrRoutePolicy OBJECT-TYPE
        ...
        */
        ...
        entry->rt_proto = (flags & RTF_DYNAMIC) ? IANAIPROUTEPROTOCOL_ICMP
...
static int _load_ipv6(netsnmp_container* container, u_long* index)
{
    FILE* in;
    char line[256];
    netsnmp_route_entry* entry = NULL;

    DEBUGMSGTL(("access:route:container", "route_container_arch_load ipv6\n"));
    ...
    {
        DEBUGMSGTL(
            ("9:access:route:container", "cannot open /proc/net/ipv6_route\n"));
        return -2;
    }
    ...
    while (fgets(line, sizeof(line), in))
    {
        ...
        rc = sscanf(
            line,
            "%32s %2x %32s %*x %32s %x %*x %*x %x %" SNMP_MACRO_VAL_TO_STR(
                IFNAMSIZ) "s\n",
            c_dest, &dest_pfx, c_src, /*src_pfx,*/ c_next, &entry->rt_metric1,
            /** ref,*/ /* use, */ &flags, c_name);
        DEBUGMSGTL(("9:access:route:container", "line |%s|\n", line));
        if (7 != rc)
        {
            snmp_log(LOG_ERR,
        ...
        {
            snmp_log(LOG_ERR,
                     "unknown interface in /proc/net/ipv6_route "
                     "('%s')\n",
                     c_name);
            netsnmp_access_route_entry_free(entry);
            continue;
        ...
        netsnmp_hex_to_binary(&temp_uchar_ptr, &buf_len, &buf_offset, 0, c_dest,
                              NULL);
        ...
        netsnmp_hex_to_binary(&temp_uchar_ptr, &buf_len, &buf_offset, 0, c_next,
                              NULL);
