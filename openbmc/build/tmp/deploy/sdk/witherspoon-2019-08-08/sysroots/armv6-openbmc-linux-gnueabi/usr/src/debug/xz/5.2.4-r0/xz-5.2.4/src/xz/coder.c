...
extern void coder_set_compression_settings(void)
{
    ...
    if (filters_count == 0)
    {
        ...
        filters[0].id =
            opt_format == FORMAT_LZMA ? LZMA_FILTER_LZMA1 : LZMA_FILTER_LZMA2;
    ...
    if (opt_format == FORMAT_LZMA &&
        (filters_count != 1 || filters[0].id != LZMA_FILTER_LZMA1))
    ...
    if (opt_format == FORMAT_XZ && mt_options.threads > 1)
    {
        ...
        message(V_WARNING,
                _("Adjusted the number of threads "
        ...
        switch (init_format)
        {
            ...
            case FORMAT_XZ:
                ret = lzma_stream_decoder(
                    &strm, hardware_memlimit_get(MODE_DECOMPRESS), flags);
                break;

            case FORMAT_LZMA:
                ret = lzma_alone_decoder(
                    &strm, hardware_memlimit_get(MODE_DECOMPRESS));
                break;
...
static void split_block(uint64_t* block_remaining,
                        uint64_t* next_block_remaining, size_t* list_pos)
{
...
static bool coder_normal(file_pair* pair)
{
        ...
        if (opt_block_list != NULL)
        {
            if (block_remaining < opt_block_list[list_pos])
            {
                ...
                next_block_remaining =
                    opt_block_list[list_pos] - block_remaining;
            }
            else
            {
                block_remaining = opt_block_list[list_pos];
    ...
    while (!user_abort)
    {
        ...
        if (strm.avail_in == 0 && action == LZMA_RUN)
        {
            ...
            strm.avail_in =
                io_read(pair, &in_buf, my_min(block_remaining, IO_BUFFER_SIZE));
        ...
        if (strm.avail_out == 0)
        {
            if (opt_mode != MODE_TEST &&
                io_write(pair, &out_buf, IO_BUFFER_SIZE - strm.avail_out))
                break;
        ...
        if (ret == LZMA_STREAM_END &&
            (action == LZMA_SYNC_FLUSH || action == LZMA_FULL_BARRIER))
        {
            ...
            else
            {
                ...
                {
                    split_block(&block_remaining, &next_block_remaining,
                                &list_pos);
                }
            }
        ...
        else if (ret != LZMA_OK)
        {
            ...
            const bool stop =
                ret != LZMA_NO_CHECK && ret != LZMA_UNSUPPORTED_CHECK;
            ...
            {
                message_warning("%s: %s", pair->src_name, message_strm(ret));
        ...
        if (init_ret != CODER_INIT_ERROR && !user_abort)
        {
            ...
            if (opt_mode == MODE_TEST || !io_open_dest(pair))
            {
                ...
                const uint64_t in_size =
                    pair->src_st.st_size <= 0 ? 0 : pair->src_st.st_size;
