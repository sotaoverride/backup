...
{
    fprintf(stderr,
            _("Usage: %s [-d debug_flags] [-f] [-F] [-M] [-P] "
...
static void determine_fs_stride(ext2_filsys fs)
{
    ...
    for (group = 0; group < fs->group_desc_count; group++)
    {
        ...
        i_stride = ext2fs_inode_bitmap_loc(fs, group) -
                   ...
                   fs->super->s_blocks_per_group;
        if (b_stride != i_stride || b_stride < 0 ||
...
int main(int argc, char** argv)
{
    ...
    if (ret < 0)
    {
        com_err("open", errno, _("while getting stat information for %s"),
                device_name);
        exit(1);
    }
    ...
    if (flush)
    {
        ...
        if (retval)
        {
            com_err(argv[0], retval, _("while trying to flush %s"),
                    device_name);
            exit(1);
        }
    }
    ...
    if (!force && !(mount_flags & EXT2_MF_MOUNTED))
    {
        ...
        if ((fs->super->s_lastcheck < fs->super->s_mtime) && !print_min_size)
            checkit = 1;
    ...
    {
        com_err(program_name, EXT2_ET_UNSUPP_FEATURE, "(%s)", device_name);
        exit(1);
    }
...
#define _SC_PAGESIZE _SC_PAGE_SIZE
    ...
    {
        new_size = parse_num_blocks2(new_size_str, fs->super->s_log_block_size);
        if (new_size == 0)
        {
            com_err(program_name, 0, _("Invalid new size: %s\n"), new_size_str);
            exit(1);
        }
    }
    else
    ...
    {
        com_err(program_name, 0, _("New size smaller than minimum (%llu)\n"),
                min_size);
        exit(1);
    }
    if (use_stride >= 0)
    {
        ...
        {
            com_err(program_name, 0, "%s", _("Invalid stride length"));
            exit(1);
        }
    ...
    {
        fprintf(stderr,
                _("The containing partition (or device)"
                  " is only %llu (%dk) blocks.\nYou requested a new size"
                  " of %llu blocks.\n\n"),
                max_size, blocksize / 1024, new_size);
        exit(1);
    }
    ...
    {
        fprintf(stderr,
                _("The filesystem is already %llu (%dk) "
                  "blocks long.  Nothing to do!\n\n"),
                new_size, blocksize / 1024);
        exit(0);
    }
    if ((flags & RESIZE_ENABLE_64BIT) && ext2fs_has_feature_64bit(fs->super))
    {
        fprintf(stderr, _("The filesystem is already 64-bit.\n"));
        exit(0);
    }
    if ((flags & RESIZE_DISABLE_64BIT) && !ext2fs_has_feature_64bit(fs->super))
    {
    ...
    else
    {
        ...
        retval = resize_fs(
            fs, &new_size, flags,
            ((flags & RESIZE_PERCENT_COMPLETE) ? resize_progress_func : 0));
    }
    free(mtpt);
    if (retval)
    {
