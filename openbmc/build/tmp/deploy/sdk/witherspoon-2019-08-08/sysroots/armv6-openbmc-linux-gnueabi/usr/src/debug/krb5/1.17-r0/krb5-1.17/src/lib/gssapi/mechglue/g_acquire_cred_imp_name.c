...
static OM_uint32 val_acq_cred_impersonate_name_args(
    ...
    OM_uint32* time_rec)
{
    ...
    major = val_acq_cred_impersonate_name_args(
        minor_status, impersonator_cred_handle, desired_name, time_req,
    ...
    {
        major = gss_add_cred_impersonate_name(
            minor_status, (gss_cred_id_t)creds, impersonator_cred_handle,
            desired_name, &mechs->elements[i], cred_usage, time_req, time_req,
            NULL, NULL, &initTimeOut, &acceptTimeOut);
        if (major == GSS_S_COMPLETE)
        {
    ...
    if (actual_mechs != NULL)
    {
        ...
        if (GSS_ERROR(major))
        {
            (void)gss_release_cred(minor_status, (gss_cred_id_t*)&creds);
            return (major);
        }
    }
...
static OM_uint32 val_add_cred_impersonate_name_args(
    OM_uint32* minor_status, gss_cred_id_t input_cred_handle,
    const gss_cred_id_t impersonator_cred_handle, gss_name_t desired_name,
    gss_OID desired_mech, gss_cred_usage_t cred_usage,
...
OM_uint32 KRB5_CALLCONV gss_add_cred_impersonate_name(
    OM_uint32* minor_status, gss_cred_id_t input_cred_handle,
    const gss_cred_id_t impersonator_cred_handle, const gss_name_t desired_name,
    ...
    OM_uint32* initiator_time_rec, OM_uint32* acceptor_time_rec)
{
    ...
    status = val_add_cred_impersonate_name_args(
        minor_status, input_cred_handle, impersonator_cred_handle, desired_name,
    ...
    mech_impersonator_cred = gssint_get_mechanism_cred(
        (gss_union_cred_t)impersonator_cred_handle, selected_mech);
    ...
    {
        if (gssint_import_internal_name(minor_status, selected_mech, union_name,
    ...
    status = gss_add_oid_set_member(
        minor_status, gssint_get_public_oid(selected_mech), &target_mechs);
    if (status != GSS_S_COMPLETE)
        goto errout;

    status = mech->gss_acquire_cred_impersonate_name(
        minor_status, mech_impersonator_cred, internal_name, time_req,
        target_mechs, cred_usage, &cred, NULL, &time_rec);
    if (status != GSS_S_COMPLETE)
    {
    ...
    new_mechs_array =
        (gss_OID)malloc(sizeof(gss_OID_desc) * (union_cred->count + 1));

    new_cred_array =
        (gss_cred_id_t*)malloc(sizeof(gss_cred_id_t) * (union_cred->count + 1));
    ...
    if (actual_mechs != NULL)
    {
        status = gssint_make_public_oid_set(
            minor_status, new_mechs_array, union_cred->count + 1, actual_mechs);
        if (GSS_ERROR(status))
        {
