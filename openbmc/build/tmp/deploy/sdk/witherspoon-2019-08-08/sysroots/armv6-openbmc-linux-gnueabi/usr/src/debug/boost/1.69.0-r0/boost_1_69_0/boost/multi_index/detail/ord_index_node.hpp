...
struct ordered_index_node_traits
{
    typedef typename boost::detail::allocator::rebind_to<
        Allocator, ordered_index_node_impl<AugmentPolicy, Allocator>>::type
        allocator;
...
{
    typedef ordered_index_node_traits<AugmentPolicy, Allocator> node_traits;
...
{
    typedef ordered_index_node_impl<AugmentPolicy, Allocator>* pointer;
    typedef const ordered_index_node_impl<AugmentPolicy, Allocator>*
        const_pointer;
...
#if !defined(BOOST_MULTI_INDEX_DISABLE_COMPRESSED_ORDERED_INDEX_NODES)
    AugmentPolicy::template augmented_node<typename mpl::if_c<
        !(has_uintptr_type::value) ||
            (alignment_of<ordered_index_node_compressed_base<
                 AugmentPolicy, Allocator>>::value %
             2) ||
            !(is_same<
                typename ordered_index_node_traits<AugmentPolicy,
                                                   Allocator>::pointer,
                ordered_index_node_impl<AugmentPolicy, Allocator>*>::value),
        ordered_index_node_std_base<AugmentPolicy, Allocator>,
        ordered_index_node_compressed_base<AugmentPolicy, Allocator>>::type>::
        type
#else
    AugmentPolicy::template augmented_node<
        ordered_index_node_std_base<AugmentPolicy, Allocator>>::type
#endif
    ...
    : ordered_index_node_impl_base<AugmentPolicy, Allocator>
{
  ...
  public:
    ...
    static void link(pointer x, ordered_index_side side, pointer position,
                     pointer header)
    {
        if (side == to_left)
        {
            position->left() =
                x; /* also makes leftmost=x when parent==header */
            if (position == header)
            {
                header->parent() = x;
                header->right() = x;
        ...
        else
        {
            ...
            {
                header->right() =
                    x; /* maintain rightmost pointing to max node */
            }
        }
        x->parent() = position;
        x->left() = pointer(0);
    ...
    static pointer rebalance_for_erase(pointer z, parent_ref root,
                                       pointer& leftmost, pointer& rightmost)
    {
        pointer y = z;
        pointer x = pointer(0);
        ...
        if (y != z)
        {
            ...
            z->left()->parent() =
                y; /* relink y in place of z. y is z's successor */
            y->left() = z->left();
            if (y != z->right())
            {
                x_parent = y->parent();
        ...
        { /* y==z */
            ...
            if (leftmost == z)
            {
                ...
                {
                    leftmost =
                        minimum(x); /* makes leftmost==header if z==root */
                }
            }
            if (rightmost == z)
            {
                ...
                { /* x==z->left() */
                    rightmost =
                        maximum(x); /* makes rightmost==header if z==root */
                }
            }
        }
        if (y->color() != red)
...
struct ordered_index_node_trampoline
    : ordered_index_node_impl<AugmentPolicy,
                              typename boost::detail::allocator::rebind_to<
                                  typename Super::allocator_type, char>::type>
{
    typedef ordered_index_node_impl<
        AugmentPolicy, typename boost::detail::allocator::rebind_to<
                           typename Super::allocator_type, char>::type>
        impl_type;
};
...
struct ordered_index_node : Super,
                            ordered_index_node_trampoline<AugmentPolicy, Super>
{
  private:
  ...
  public:
    ...
    {
        return static_cast<const_impl_pointer>(static_cast<const impl_type*>(
            static_cast<const trampoline*>(this)));
    }

    static ordered_index_node* from_impl(impl_pointer x)
    {
        return static_cast<ordered_index_node*>(
            static_cast<trampoline*>(raw_ptr<impl_type*>(x)));
    }

    static const ordered_index_node* from_impl(const_impl_pointer x)
    {
        return static_cast<const ordered_index_node*>(
            static_cast<const trampoline*>(raw_ptr<const impl_type*>(x)));
    }
