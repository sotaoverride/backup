...
#undef DB_VARIABLE
    ...
    {
        uint32_t
            const_thread_area; /* Constant argument to ps_get_thread_area.  */
...
static inline bool ta_ok(const td_thragent_t* ta)
{
    ...
    list_for_each(runp,
                  &__td_agent_list) if (list_entry(runp, td_thragent_t, list) ==
                                        ta) return true;
...
#define DB_GET_FIELD(var, ta, ptr, type, field, idx)                           \
    _td_fetch_value((ta), (ta)->ta_field_##type##_##field,                     \
                    SYM_##type##_FIELD_##field, (psaddr_t)0 + (idx), (ptr),    \
                    &(var))
...
extern td_err_e _td_locate_field(td_thragent_t* ta, db_desc_t desc,
                                 int descriptor_name, psaddr_t idx,
...
#define DB_GET_FIELD_LOCAL(var, ta, ptr, type, field, idx)                     \
    _td_fetch_value_local((ta), (ta)->ta_field_##type##_##field,               \
                          SYM_##type##_FIELD_##field, (psaddr_t)0 + (idx),     \
                          (ptr), &(var))
...
extern td_err_e _td_fetch_value(td_thragent_t* ta, db_desc_t field,
                                int descriptor_name, psaddr_t idx,
                                psaddr_t address,
                                psaddr_t* result) attribute_hidden;
extern td_err_e _td_fetch_value_local(td_thragent_t* ta, db_desc_t field,
                                      int descriptor_name, psaddr_t idx,
                                      void* address,
                                      psaddr_t* result) attribute_hidden;
    ...
    (((ta)->ta_addr_##name == 0 &&                                             \
      td_lookup((ta)->ph, SYM_##name, &(ta)->ta_addr_##name) != PS_OK)         \
         ...
         : _td_store_value((ta), (ta)->ta_var_##name, SYM_DESC_##name,         \
                           (psaddr_t)0 + (idx), (ta)->ta_addr_##name,          \
                           (value)))

/* Helper functions for those.  */
extern td_err_e _td_store_value(td_thragent_t* ta, db_desc_t field,
                                int descriptor_name, psaddr_t idx,
                                psaddr_t address,
                                psaddr_t value) attribute_hidden;
extern td_err_e _td_store_value_local(td_thragent_t* ta, db_desc_t field,
                                      int descriptor_name, psaddr_t idx,
                                      void* address,
                                      psaddr_t value) attribute_hidden;
...
#define DB_PUT_STRUCT(ta, ptr, type, copy)                                     \
    ({                                                                         \
        ...
        ps_pdwrite((ta)->ph, (ptr), (copy), (ta)->ta_sizeof_##type) == PS_OK   \
            ? TD_OK                                                            \
            : TD_ERR;                                                          \
    })
...
extern td_err_e __td_ta_lookup_th_unique(const td_thragent_t* ta, lwpid_t lwpid,
                                         td_thrhandle_t* th);
