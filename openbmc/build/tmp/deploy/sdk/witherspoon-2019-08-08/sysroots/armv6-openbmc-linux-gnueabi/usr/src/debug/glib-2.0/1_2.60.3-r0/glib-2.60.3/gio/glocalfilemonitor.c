...
static gint64 pending_change_get_ready_time(const PendingChange* change,
...
static int pending_change_compare_ready_time(gconstpointer a_p,
                                             gconstpointer b_p,
                                             gpointer user_data)
{
...
static gint64 g_file_monitor_source_get_ready_time(GFileMonitorSource* fms)
{
    GSequenceIter* iter;
...
static void g_file_monitor_source_update_ready_time(GFileMonitorSource* fms)
{
    g_source_set_ready_time((GSource*)fms,
...
static void g_file_monitor_source_add_pending_change(GFileMonitorSource* fms,
                                                     const gchar* child,
                                                     gint64 now)
{
    ...
    iter = g_sequence_insert_sorted(fms->pending_changes, change,
                                    pending_change_compare_ready_time, fms);
...
static void g_file_monitor_source_remove_pending_change(GFileMonitorSource* fms,
                                                        GSequenceIter* iter,
                                                        const gchar* child)
{
...
static void g_file_monitor_source_queue_event(GFileMonitorSource* fms,
                                              GFileMonitorEvent event_type,
                                              const gchar* child, GFile* other)
{
    QueuedEvent* event;
    ...
    if (child != NULL && fms->dirname != NULL)
        event->child =
            g_local_file_new_from_dirname_and_basename(fms->dirname, child);
    else if (child != NULL)
    {
        gchar* dirname = g_path_get_dirname(fms->filename);
        event->child =
            g_local_file_new_from_dirname_and_basename(dirname, child);
        g_free(dirname);
    }
    else if (fms->dirname)
...
static gboolean g_file_monitor_source_file_changed(GFileMonitorSource* fms,
                                                   const gchar* child,
                                                   gint64 now)
{
    ...
    if (!pending)
    {
        g_file_monitor_source_queue_event(fms, G_FILE_MONITOR_EVENT_CHANGED,
                                          child, NULL);
...
static void g_file_monitor_source_file_changes_done(GFileMonitorSource* fms,
                                                    const gchar* child)
{
    GSequenceIter* pending;
    ...
    if (pending)
    {
        ...
        g_file_monitor_source_queue_event(
            fms, G_FILE_MONITOR_EVENT_CHANGES_DONE_HINT, child, NULL);
        g_file_monitor_source_remove_pending_change(fms, pending, child);
    }
}

static void g_file_monitor_source_file_created(GFileMonitorSource* fms,
                                               const gchar* child,
                                               gint64 event_time)
{
    ...
    g_file_monitor_source_queue_event(fms, G_FILE_MONITOR_EVENT_CREATED, child,
                                      NULL);
    g_file_monitor_source_add_pending_change(fms, child, event_time);
}

static void g_file_monitor_source_send_event(GFileMonitorSource* fms,
                                             GFileMonitorEvent event_type,
                                             const gchar* child, GFile* other)
{
...
static gboolean is_basename(const gchar* name)
{
    if (name[0] == '.' &&
...
gboolean g_file_monitor_source_handle_event(GFileMonitorSource* fms,
                                            GFileMonitorEvent event_type,
                                            const gchar* child,
                                            const gchar* rename_to,
                                            GFile* other, gint64 event_time)
{
    ...
    switch (event_type)
    {
        ...
        case G_FILE_MONITOR_EVENT_MOVED_IN:
            ...
            if (fms->flags & G_FILE_MONITOR_WATCH_MOVES)
                g_file_monitor_source_send_event(
                    fms, G_FILE_MONITOR_EVENT_MOVED_IN, child, other);
            else
        ...
        case G_FILE_MONITOR_EVENT_MOVED_OUT:
            ...
            if (fms->flags & G_FILE_MONITOR_WATCH_MOVES)
                g_file_monitor_source_send_event(
                    fms, G_FILE_MONITOR_EVENT_MOVED_OUT, child, other);
            else if (other && (fms->flags & G_FILE_MONITOR_SEND_MOVED))
                g_file_monitor_source_send_event(
                    fms, G_FILE_MONITOR_EVENT_MOVED, child, other);
            else
                g_file_monitor_source_send_event(
                    fms, G_FILE_MONITOR_EVENT_DELETED, child, NULL);
            break;

        case G_FILE_MONITOR_EVENT_RENAMED:
            g_assert(!other && rename_to);
            if (fms->flags &
                (G_FILE_MONITOR_WATCH_MOVES | G_FILE_MONITOR_SEND_MOVED))
            {
                GFile* other;
                const gchar* dirname;
                ...
                event = (fms->flags & G_FILE_MONITOR_WATCH_MOVES)
                ...
                other = g_local_file_new_from_dirname_and_basename(dirname,
                                                                   rename_to);
...
static gint64 g_file_monitor_source_get_rate_limit(GFileMonitorSource* fms)
{
    gint64 rate_limit;
...
static gboolean g_file_monitor_source_set_rate_limit(GFileMonitorSource* fms,
                                                     gint64 rate_limit)
{
    gboolean changed;
    ...
    if (rate_limit != fms->rate_limit)
    {
        ...
        g_sequence_sort(fms->pending_changes, pending_change_compare_ready_time,
                        fms);
        g_file_monitor_source_update_ready_time(fms);
...
static gboolean g_file_monitor_source_dispatch(GSource* source,
                                               ...
                                               gpointer user_data)
{
    ...
    while (!g_sequence_is_empty(fms->pending_changes))
    {
        ...
        if (pending->dirty)
        {
            ...
            g_sequence_sort_changed(iter, pending_change_compare_ready_time,
                                    fms);
        }
        else
        {
            ...
            g_file_monitor_source_queue_event(
                fms, G_FILE_MONITOR_EVENT_CHANGES_DONE_HINT, pending->child,
                NULL);
            g_file_monitor_source_remove_pending_change(fms, iter,
                                                        pending->child);
        }
    }
    ...
    while ((event = g_queue_pop_head(&event_queue)))
    {
        ...
        if (fms->instance)
            g_file_monitor_emit_event(fms->instance, event->child, event->other,
                                      event->event_type);
...
static void g_file_monitor_source_dispose(GFileMonitorSource* fms)
{
    g_mutex_lock(&fms->lock);
...
static void g_file_monitor_source_finalize(GSource* source)
{
    GFileMonitorSource* fms = (GFileMonitorSource*)source;
...
static GFileMonitorSource* g_file_monitor_source_new(gpointer instance,
                                                     const gchar* filename,
                                                     gboolean is_directory,
                                                     GFileMonitorFlags flags)
{
    static GSourceFuncs source_funcs = {NULL, NULL,
...
G_DEFINE_ABSTRACT_TYPE(GLocalFileMonitor, g_local_file_monitor,
                       G_TYPE_FILE_MONITOR)
...
static void g_local_file_monitor_get_property(GObject* object, guint prop_id,
...
static void g_local_file_monitor_set_property(GObject* object, guint prop_id,
                                              const GValue* value,
                                              GParamSpec* pspec)
{
    GLocalFileMonitor* monitor = G_LOCAL_FILE_MONITOR(object);
                                        ...
                                        gpointer user_data)
{
    ...
    if (local_monitor->was_mounted != is_mounted)
    {
        if (local_monitor->was_mounted && !is_mounted)
        {
            ...
            g_file_monitor_emit_event(G_FILE_MONITOR(local_monitor), file, NULL,
...
static void g_local_file_monitor_start(GLocalFileMonitor* local_monitor,
                                       const gchar* filename,
                                       gboolean is_directory,
                                       GFileMonitorFlags flags,
                                       GMainContext* context)
{
    GLocalFileMonitorClass* class =
        G_LOCAL_FILE_MONITOR_GET_CLASS(local_monitor);
    ...
    source =
        g_file_monitor_source_new(local_monitor, filename, is_directory, flags);
    local_monitor->source = source; /* owns the ref */

    if (is_directory && !class->mount_notify &&
        (flags & G_FILE_MONITOR_WATCH_MOUNTS))
    {
        ...
        g_signal_connect_object(local_monitor->mount_monitor, "mounts-changed",
                                G_CALLBACK(g_local_file_monitor_mounts_changed),
                                local_monitor, 0);
#endif
    }

    G_LOCAL_FILE_MONITOR_GET_CLASS(local_monitor)
        ->start(local_monitor, source->dirname, source->basename,
                source->filename, source);

    g_source_attach((GSource*)source, context);
}

static void g_local_file_monitor_dispose(GObject* object)
{
...
static void g_local_file_monitor_class_init(GLocalFileMonitorClass* class)
{
    ...
    g_object_class_override_property(gobject_class, PROP_RATE_LIMIT,
                                     "rate-limit");
}

static GLocalFileMonitor* g_local_file_monitor_new(gboolean is_remote_fs,
                                                   gboolean is_directory,
                                                   GError** error)
{
    ...
    if (is_remote_fs)
        type = _g_io_module_get_default_type(
            G_NFS_FILE_MONITOR_EXTENSION_POINT_NAME, "GIO_USE_FILE_MONITOR",
    ...
    if (type == G_TYPE_INVALID && (!is_remote_fs || is_directory))
        type = _g_io_module_get_default_type(
            G_LOCAL_FILE_MONITOR_EXTENSION_POINT_NAME, "GIO_USE_FILE_MONITOR",
            G_STRUCT_OFFSET(GLocalFileMonitorClass, is_supported));
...
GFileMonitor* g_local_file_monitor_new_for_path(const gchar* pathname,
                                                gboolean is_directory,
                                                GFileMonitorFlags flags,
                                                GError** error)
{
...
GFileMonitor* g_local_file_monitor_new_in_worker(
    const gchar* pathname, gboolean is_directory, GFileMonitorFlags flags,
    GFileMonitorCallback callback, gpointer user_data, GError** error)
{
    GLocalFileMonitor* monitor;
    gboolean is_remote_fs;
    ...
    {
        if (callback)
            g_signal_connect(monitor, "changed", G_CALLBACK(callback),
                             user_data);
