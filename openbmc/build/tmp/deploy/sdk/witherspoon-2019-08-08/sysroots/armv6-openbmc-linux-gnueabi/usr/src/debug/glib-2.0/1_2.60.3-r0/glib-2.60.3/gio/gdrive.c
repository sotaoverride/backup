...
static void g_drive_default_init(GDriveInterface* iface)
{
     ...
     **/
    g_signal_new(I_("changed"), G_TYPE_DRIVE, G_SIGNAL_RUN_LAST,
     ...
     * finalized.
     **/
    g_signal_new(I_("disconnected"), G_TYPE_DRIVE, G_SIGNAL_RUN_LAST,
                 G_STRUCT_OFFSET(GDriveIface, disconnected), NULL, NULL,
                 g_cclosure_marshal_VOID__VOID, G_TYPE_NONE, 0);
     ...
     * been pressed.
     **/
    g_signal_new(I_("eject-button"), G_TYPE_DRIVE, G_SIGNAL_RUN_LAST,
                 G_STRUCT_OFFSET(GDriveIface, eject_button), NULL, NULL,
                 g_cclosure_marshal_VOID__VOID, G_TYPE_NONE, 0);
     ...
     * Since: 2.22
     **/
    g_signal_new(I_("stop-button"), G_TYPE_DRIVE, G_SIGNAL_RUN_LAST,
                 G_STRUCT_OFFSET(GDriveIface, stop_button), NULL, NULL,
                 g_cclosure_marshal_VOID__VOID, G_TYPE_NONE, 0);
}
 ...
 *
 * Returns: (element-type GVolume) (transfer full): #GList containing any
 *#GVolume objects on the given @drive.
 **/
 ...
 *
 * Returns: %TRUE if @drive and/or its media is considered removable, %FALSE
 *otherwise.
 *
 * Since: 2.50
 **/
                   ...
                   gpointer user_data)
{
    ...
    if (iface->eject == NULL)
    {
        g_task_report_new_error(drive, callback, user_data,
                                g_drive_eject_with_operation, G_IO_ERROR,
                                  ...
                                  gpointer user_data)
{
    ...
    if (iface->eject == NULL && iface->eject_with_operation == NULL)
    {
        g_task_report_new_error(
            drive, callback, user_data, g_drive_eject_with_operation,
    ...
    if (iface->eject_with_operation != NULL)
        (*iface->eject_with_operation)(drive, flags, mount_operation,
                            ...
                            GAsyncReadyCallback callback, gpointer user_data)
{
    ...
    {
        g_task_report_new_error(drive, callback, user_data,
                                g_drive_poll_for_media, G_IO_ERROR,
                                G_IO_ERROR_NOT_SUPPORTED,
                   ...
                   GAsyncReadyCallback callback, gpointer user_data)
{
    ...
    if (iface->start == NULL)
    {
        g_task_report_new_error(drive, callback, user_data, g_drive_start,
                                G_IO_ERROR, G_IO_ERROR_NOT_SUPPORTED,
                                _("drive doesn’t implement start"));
        return;
    }

    (*iface->start)(drive, flags, mount_operation, cancellable, callback,
                    user_data);
}
                  ...
                  GAsyncReadyCallback callback, gpointer user_data)
{
    ...
    (*iface->stop)(drive, flags, mount_operation, cancellable, callback,
                   user_data);
}
 ...
 *
 * Returns: (nullable): Sorting key for @drive or %NULL if no such key is
 * available.
 *
 * Since: 2.32
 */
