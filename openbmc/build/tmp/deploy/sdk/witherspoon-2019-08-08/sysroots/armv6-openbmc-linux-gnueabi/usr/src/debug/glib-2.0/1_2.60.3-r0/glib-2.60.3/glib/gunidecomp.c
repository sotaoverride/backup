...
static gboolean combine(gunichar a, gunichar b, gunichar* result)
{
    ...
    {
        gunichar res = compose_array[index_a - COMPOSE_FIRST_START]
                                    [index_b - COMPOSE_SECOND_START];
...
gunichar* _g_utf8_normalize_wc(const gchar* str, gssize max_len,
                               GNormalizeMode mode)
{
    gsize n_wc;
    gunichar* wc_buffer;
...
static gboolean decompose_hangul_step(gunichar ch, gunichar* a, gunichar* b)
{
    gint SIndex, TIndex;
                                ...
                                gsize result_len)
{
    ...
    else if ((decomp = find_decomposition(ch, compat)) != NULL)
    {
        ...
        for (p = decomp, i = 0; i < len && i < result_len;
