...
int get_exec_pipes(char* cmd, int* fdIn, int* fdOut, pid_t* pid);
    ...
    prefix_cbx* net_snmp_create_prefix_info(
        unsigned long OnLinkFlag, unsigned long AutonomousFlag, char* in6ptr);
#ifndef WIN32
    ...
    int net_snmp_search_update_prefix_info(
        prefix_cbx * *head, prefix_cbx * node_to_use, int functionality);
