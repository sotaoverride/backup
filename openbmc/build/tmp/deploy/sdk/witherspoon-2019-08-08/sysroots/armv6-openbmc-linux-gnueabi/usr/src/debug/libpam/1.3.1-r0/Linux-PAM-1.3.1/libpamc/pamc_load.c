...
static int __pamc_exec_agent(pamc_handle_t pch, pamc_agent_t* agent)
{
    ...
    D(("reset_length = %d (3+%d+%d)", reset_length, pch->max_path,
       agent->id_length));
                                       ...
                                       int* counter)
{
    if (root)
    {
        ...
        if ((cmp = strcmp(id, root->agent_id)))
        {
            ...
            {
                root->right =
                    __pamc_add_node(root->right, id, &(root->child_count));
            }
            else
            {
                root->left =
                    __pamc_add_node(root->left, id, &(root->child_count));
            }
        }
    ...
    for (i = 0; pch->agent_paths[i]; ++i)
    {
        ...
        if (dir)
        {
            ...
            while ((item = readdir(dir)))
            {
                ...
                if (!__pamc_valid_agent_id(strlen(item->d_name), item->d_name))
                {
                    continue;
                }
