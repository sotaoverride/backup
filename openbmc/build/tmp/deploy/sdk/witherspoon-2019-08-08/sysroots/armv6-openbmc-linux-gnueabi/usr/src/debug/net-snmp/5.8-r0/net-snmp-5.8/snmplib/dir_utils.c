...
netsnmp_feature_child_of(container_directory, container_types)
    ...
    netsnmp_feature_require(file_utils)
        netsnmp_feature_require(container_free_all)
            ...
            static int _insert_nsfile(netsnmp_container* c, const char* name,
                                      struct stat* stats, u_int flags);

/*
 * read file names in a directory, with an optional filter
 */
netsnmp_container* netsnmp_directory_container_read_some(
    netsnmp_container* user_container, const char* dirname,
    netsnmp_directory_filter* filter, void* filter_ctx, u_int flags)
{
    DIR* dir;
    ...
    if (NULL == container)
    {
        if (flags & NETSNMP_DIR_NSFILE)
        {
            ...
            {
                container->compare =
                    (netsnmp_container_compare*)netsnmp_file_compare_name;
                container->free_item =
                    (netsnmp_container_obj_func*)netsnmp_file_container_free;
            }
        }
        else
    ...
    while ((file = readdir(dir)))
    {
        ...
        if (NULL != filter)
        {
            ...
            {
                DEBUGMSGTL(
                    ("directory:container:filtered", "%s\n", file->d_name));
                continue;
            }
        }
...
void netsnmp_directory_container_free(netsnmp_container* container)
{
