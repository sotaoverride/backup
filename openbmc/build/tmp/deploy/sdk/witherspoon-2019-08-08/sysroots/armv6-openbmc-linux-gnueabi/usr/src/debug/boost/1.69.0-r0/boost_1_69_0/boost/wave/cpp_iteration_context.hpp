...
#if !defined(                                                                  \
    CPP_ITERATION_CONTEXT_HPP_00312288_9DDB_4668_AFE5_25D3994FD095_INCLUDED)
...
// #include <boost/spirit/include/iterator/classic_multi_pass.hpp> //
// make_multi_pass
...
struct base_iteration_context
{
    ...
    base_iteration_context(ContextT& ctx_, BOOST_WAVE_STRINGTYPE const& fname,
                           std::size_t if_block_depth = 0) :
        real_filename(fname),
        real_relative_filename(fname), filename(fname), line(1),
        emitted_lines(0), if_block_depth(if_block_depth), ctx(ctx_),
        type(main_file)
    {
    }
    base_iteration_context(ContextT& ctx_, IteratorT const& first_,
                           IteratorT const& last_,
                           BOOST_WAVE_STRINGTYPE const& fname,
                           std::size_t if_block_depth = 0,
                           file_type type_ = main_file) :
        first(first_),
        last(last_), real_filename(fname), real_relative_filename(fname),
        filename(fname), line(1), emitted_lines(0),
        if_block_depth(if_block_depth), ctx(ctx_), type(type_)
    {
    }
    ...
    BOOST_WAVE_STRINGTYPE
        real_relative_filename;     // real relative name of the current file
...
struct iteration_context
    : public base_iteration_context<ContextT, IteratorT>,
      public InputPolicyT::template inner<
          iteration_context<ContextT, IteratorT, InputPolicyT>>
{
    ...
    iteration_context(
        ContextT& ctx, BOOST_WAVE_STRINGTYPE const& fname,
        position_type const& act_pos, boost::wave::language_support language_,
