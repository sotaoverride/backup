...
static struct libevdev_uinput* alloc_uinput_device(const char* name)
{
...
static inline int set_abs(const struct libevdev* dev, int fd, unsigned int code)
{
...
static int set_evbits(const struct libevdev* dev, int fd,
...
static int set_props(const struct libevdev* dev, int fd)
{
    unsigned int prop;
    int rc = 0;
...
static char* fetch_device_node(const char* path)
{
    ...
    {
        if (!devnode &&
            asprintf(&devnode, "/dev/input/%s", namelist[i]->d_name) == -1)
            devnode = NULL;
        free(namelist[i]);
    }
...
static int fetch_syspath_and_devnode(struct libevdev_uinput* uinput_dev)
{
    ...
    for (i = 0; i < ndev; i++)
    {
        ...
        if (fstat(fd, &st) == -1 || st.st_ctime < uinput_dev->ctime[0] ||
        ...
        if (strcmp(buf, uinput_dev->name) == 0)
        {
            if (uinput_dev->syspath)
            {
                ...
                log_info(NULL, "multiple identical devices found. syspath is "
                               "unreliable\n");
                break;
            }
            else
...
static int uinput_create_write(const struct libevdev* dev, int fd,
...
static int uinput_create_DEV_SETUP(const struct libevdev* dev, int fd,
                                   struct libevdev_uinput* new_device)
{
    int rc;
                                       ...
                                       struct libevdev_uinput** uinput_dev)
{
    ...
    if (ioctl(fd, UI_GET_VERSION, &uinput_version) == 0 && uinput_version >= 5)
...
LIBEVDEV_EXPORT void libevdev_uinput_destroy(struct libevdev_uinput* uinput_dev)
{
    if (!uinput_dev)
        return;
