...
#define RESET_INPUT_BUFFER                                                     \
    ...
    {                                                                          \
        if (FROM_LOOP_MIN_NEEDED_FROM % FROM_LOOP_MIN_NEEDED_TO == 0)          \
            ...
            *inptrp -=                                                         \
                (outbuf - outerr) /                                            \
                (FROM_LOOP_MIN_NEEDED_TO / FROM_LOOP_MIN_NEEDED_FROM ?: 1);    \
        else                                                                   \
            ...
            if (status == __GCONV_OK)
            {
                ...
                else
                {
                    ...
                    if (outbuf > outstart)
                    {
                        ...
                        result = DL_CALL_FCT(
                            fct, (next_step, next_data, &outerr, outbuf, NULL,
    ...
    else
    {
       ...
       but only some conversion modules need it.  */
        ...
        unsigned char* outbuf =
            (__builtin_expect(outbufstart == NULL, 1) ? data->__outbuf
                                                      : *outbufstart);
        unsigned char* outend = data->__outbufend;
       ...
       stored in the `state' object.  */
            ...
            consume_incomplete && (data->__statep->__count & 7) != 0)
        {
            ...
            if (TO_LOOP_MAX_NEEDED_FROM == 1 || FROM_DIRECTION)
                status =
                    SINGLE(FROM_LOOP)(step, data, inptrp, inend, &outbuf,
                ...
                status =
                    SINGLE(TO_LOOP)(step, data, inptrp, inend, &outbuf, outend,
        ...
        while (1)
        {
            ...
            if (__glibc_likely(!unaligned))
            {
                if (FROM_DIRECTION)
                    ...
                    status = FROM_LOOP(step, data, inptrp, inend, &outbuf,
                                       outend, lirreversiblep EXTRA_LOOP_ARGS);
                else
                    /* Run the conversion loop.  */
            ...
            else
            {
                if (FROM_DIRECTION)
                    /* Run the conversion loop.  */
                    status = GEN_unaligned(FROM_LOOP)(
                        step, data, inptrp, inend, &outbuf, outend,
                        lirreversiblep EXTRA_LOOP_ARGS);
                else
                    /* Run the conversion loop.  */
                    status = GEN_unaligned(TO_LOOP)(
                        step, data, inptrp, inend, &outbuf, outend,
                        lirreversiblep EXTRA_LOOP_ARGS);
            }
#endif
                        ...
                        *inptrp = inptr;
                        ...
                        if (__glibc_likely(!unaligned))
                        {
                            if (FROM_DIRECTION)
                                /* Run the conversion loop.  */
                                nstatus = FROM_LOOP(
                                    step, data, inptrp, inend, &outbuf, outerr,
                                    lirreversiblep EXTRA_LOOP_ARGS);
                            else
                                /* Run the conversion loop.  */
                                nstatus = TO_LOOP(
                        ...
                        else
                        {
                            if (FROM_DIRECTION)
                                /* Run the conversion loop.  */
                                nstatus = GEN_unaligned(FROM_LOOP)(
                                    step, data, inptrp, inend, &outbuf, outerr,
                                    lirreversiblep EXTRA_LOOP_ARGS);
                            else
                                /* Run the conversion loop.  */
                                nstatus = GEN_unaligned(TO_LOOP)(
                                    step, data, inptrp, inend, &outbuf, outerr,
                                    lirreversiblep EXTRA_LOOP_ARGS);
                        }
#endif
