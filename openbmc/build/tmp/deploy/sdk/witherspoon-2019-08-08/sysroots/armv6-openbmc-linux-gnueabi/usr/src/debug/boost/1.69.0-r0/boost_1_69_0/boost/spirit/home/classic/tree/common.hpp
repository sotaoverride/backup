...
inline std::ostream& operator<<(std::ostream& o, tree_node<T> const& n)
{
    ...
    int c = 0;
    for (typename tree_node<T>::children_t::const_iterator it =
             n.children.begin();
...
inline std::ostream& operator<<(std::ostream& o,
                                node_iter_data<IteratorT, ValueT> const& n)
{
    o << "(id = " << n.id() << " text = \"";
    typedef
        typename node_iter_data<IteratorT, ValueT>::const_iterator_t iterator_t;
...
template <typename IteratorT, typename ValueT>
inline std::ostream& operator<<(std::ostream& o,
                                node_val_data<IteratorT, ValueT> const& n)
{
    o << "(id = " << n.id() << " text = \"";
    typedef
        typename node_val_data<IteratorT, ValueT>::const_iterator_t iterator_t;
    for (iterator_t it = n.begin(); it != n.end(); ++it)
        impl::token_printer(o, *it);
    o << "\" is_root = " << n.is_root() << " value = " << n.value() << ")";
    return o;
  ...
  public:
    ...
    class factory
    {
      public:
        ...
        static node_t create_node(iterator_t const& first,
                                  iterator_t const& last, bool /*is_leaf_node*/)
        {
...
class tree_match : public match<T>
{
  public:
    ...
    explicit tree_match(std::size_t length_) : match<T>(length_), trees()
    {
    }
    ...
    tree_match(std::size_t length_, container_t& c) : match<T>(length_), trees()
    {
...
template <typename IteratorT, typename NodeFactoryT, typename T>
inline std::ostream& operator<<(std::ostream& o,
                                tree_match<IteratorT, NodeFactoryT, T> const& m)
{
    typedef
...
//////////////////////////////////
template <typename MatchPolicyT, typename IteratorT, typename NodeFactoryT,
          typename TreePolicyT, typename T>
struct common_tree_match_policy : public match_policy
{
    ...
    static tree_match<IteratorT, NodeFactoryT, AttrT>
        create_match(std::size_t length, AttrT const& val,
    ...
    (BOOST_SPIRIT_DEBUG_FLAGS & BOOST_SPIRIT_DEBUG_FLAGS_NODES)
        ...
        return tree_match<IteratorT, NodeFactoryT, AttrT>(
            length, val, tree_policy_t::create_node(length, first, last, true));
    }
    ...
    void group_match(MatchT& m, parser_id const& id, IteratorT2 const& first,
...
struct common_tree_tree_policy
{
    ...
    static node_t create_node(std::size_t /*length*/, Iterator1T const& first,
    ...
    : public unary<T, parser<no_tree_gen_node_parser<T>>>
{
        ...
        parse(ScannerT const& scanner) const
    {
        ...
        typedef scanner_policies<iteration_policy_t, match_policy_t,
                                 action_policy_t>
            policies_t;

        return this->subject().parse(
            scanner.change_policies(policies_t(scanner)));
    }
};
...
struct node_parser : public unary<T, parser<node_parser<T, NodeParserT>>>
{
    ...
    typename parser_result<self_t, ScannerT>::type
        parse(ScannerT const& scanner) const
    {
        typename parser_result<self_t, ScannerT>::type hit =
            this->subject().parse(scanner);
        if (hit)
        {
            impl::tree_policy_selector<typename ScannerT::match_policy_t>::
                type::apply_op_to_match(NodeParserT(), hit);
        }
        return hit;
    }
};
...
struct discard_first_node_op
{
    ...
    void operator()(MatchT& m) const
    {
        ...
        else
        {
            ...
            // the end of the discarded node, but I currently don't see any way
            // to get at the begin of the node following this subnode. This
            // should be safe anyway because the it shouldn't get dereferenced
    ...
    : public unary<T, parser<action_directive_parser<T, ActionParserT>>>
{
    ...
    typename ActionParserT::template action<
        action_directive_parser<T, ActionParserT>, ActionT>
        operator[](ActionT const& actor) const
    {
        typedef typename ActionParserT::template action<action_directive_parser,
                                                        ActionT>
            action_t;
...
struct access_match_action::action
    : public unary<ParserT,
                   parser<access_match_action::action<ParserT, ActionT>>>
{
...
//////////////////////////////////
template <typename ParserT, typename ActionT>
access_match_action::action<ParserT, ActionT>::action(ParserT const& subject,
                                                      ActionT const& actor_) :
    unary<ParserT, parser<access_match_action::action<ParserT, ActionT>>>(
        subject),
    actor(actor_)
{
}
...
template <typename ParserT, typename ActionT>
template <typename ScannerT>
typename parser_result<access_match_action::action<ParserT, ActionT>,
                       ScannerT>::type
    access_match_action::action<ParserT, ActionT>::parse(
...
struct access_node_action::action
    : public unary<ParserT,
                   parser<access_node_action::action<ParserT, ActionT>>>
{
    typedef action_parser_category parser_category;
    typedef action<ParserT, ActionT> self_t;
...
//////////////////////////////////
template <typename ParserT, typename ActionT>
access_node_action::action<ParserT, ActionT>::action(ParserT const& subject,
                                                     ActionT const& actor_) :
    unary<ParserT, parser<access_node_action::action<ParserT, ActionT>>>(
        subject),
    actor(actor_)
{
}
...
template <typename ParserT, typename ActionT>
template <typename ScannerT>
typename parser_result<access_node_action::action<ParserT, ActionT>,
                       ScannerT>::type
    access_node_action::action<ParserT, ActionT>::parse(
        ScannerT const& scan) const
{
    typedef typename ScannerT::iterator_t iterator_t;
    typedef typename parser_result<self_t, ScannerT>::type result_t;
...
struct tree_parse_info
{
    ...
    tree_parse_info(tree_parse_info<IteratorT2> const& pi) :
        stop(pi.stop), match(pi.match), full(pi.full), length(pi.length),
        trees()
    {
        using boost::swap;
        using BOOST_SPIRIT_CLASSIC_NS::swap;
    ...
    tree_parse_info(
        IteratorT stop_, bool match_, bool full_, std::size_t length_,
        typename tree_match<IteratorT, NodeFactoryT, T>::container_t trees_) :
        stop(stop_),
        match(match_), full(full_), length(length_), trees()
    {
        using boost::swap;
        using BOOST_SPIRIT_CLASSIC_NS::swap;
        using std::swap;
