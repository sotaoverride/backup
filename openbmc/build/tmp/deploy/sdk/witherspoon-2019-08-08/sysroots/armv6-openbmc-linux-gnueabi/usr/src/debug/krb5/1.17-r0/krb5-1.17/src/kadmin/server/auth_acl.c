...
#define ACL_ALL_MASK                                                           \
    (ACL_ADD | ACL_DELETE | ACL_MODIFY | ACL_CHANGEPW | ACL_INQUIRE |          \
     ACL_LIST | ACL_IPROP | ACL_SETKEY)
...
static const struct acl_op_table acl_op_table[] = {
    {'a', ACL_ADD},      {'d', ACL_DELETE},  {'m', ACL_MODIFY},
    {'c', ACL_CHANGEPW}, {'i', ACL_INQUIRE}, {'l', ACL_LIST},
    {'p', ACL_IPROP},    {'s', ACL_SETKEY},  {'x', ACL_ALL_MASK},
    {'*', ACL_ALL_MASK}, {'e', ACL_EXTRACT}, {'\0', 0}};
 ...
 * read failure.
 */
static char* get_line(FILE* fp, const char* fname, int* lineno, int* incr)
{
 ...
 */
static struct kadm5_auth_restrictions* parse_restrictions(const char* str,
                                                          const char* fname)
{
...
static struct acl_entry* parse_entry(krb5_context context, const char* client,
                                     const char* ops, const char* target,
                                     const char* rs, const char* line,
                                     const char* fname)
{
    ...
    for (op = ops; *op; op++)
    {
        ...
        {
            krb5_klog_syslog(
                LOG_ERR, _("Unrecognized ACL operation '%c' in %s"), *op, line);
            goto error;
        }
    }
...
static struct acl_entry* parse_line(krb5_context context, const char* line,
                                    const char* fname)
{
...
static void free_acl_entries(struct acl_state* state)
{
...
static krb5_error_code load_acl_file(krb5_context context, const char* fname,
        ...
        *entry_slot = parse_line(context, line, fname);
        if (*entry_slot == NULL)
        {
            krb5_klog_syslog(LOG_ERR,
                             _("%s: syntax error at line %d <%.10s...>"), fname,
                             lineno, line);
 ...
 */
static krb5_boolean match_data(const krb5_data* e1, const krb5_data* e2,
...
static krb5_boolean match_princ(krb5_const_principal p1,
                                krb5_const_principal p2,
                                krb5_boolean targetflag, struct wildstate* ws)
{
    int i;
 ...
 * none is found. */
static struct acl_entry* find_entry(struct acl_state* state,
...
static krb5_error_code acl_check(kadm5_auth_moddata data, uint32_t op,
                                 krb5_const_principal client,
                                 krb5_const_principal target,
...
static krb5_error_code acl_init(krb5_context context, const char* acl_file,
...
static void acl_fini(krb5_context context, kadm5_auth_moddata data)
{
    if (data == NULL)
        return;
...
static krb5_error_code acl_setstr(krb5_context context, kadm5_auth_moddata data,
                                  krb5_const_principal client,
                                  krb5_const_principal target, const char* key,
                                  const char* value)
{
    return acl_check(data, ACL_MODIFY, client, target, NULL);
}

static krb5_error_code acl_cpw(krb5_context context, kadm5_auth_moddata data,
                               krb5_const_principal client,
                               krb5_const_principal target)
{
...
static krb5_error_code acl_chrand(krb5_context context, kadm5_auth_moddata data,
                                  krb5_const_principal client,
                                  krb5_const_principal target)
{
    return acl_check(data, ACL_CHANGEPW, client, target, NULL);
...
static krb5_error_code acl_setkey(krb5_context context, kadm5_auth_moddata data,
                                  krb5_const_principal client,
                                  krb5_const_principal target)
{
...
static krb5_error_code acl_purgekeys(krb5_context context,
...
static krb5_error_code acl_delprinc(krb5_context context,
                                    kadm5_auth_moddata data,
                                    krb5_const_principal client,
                                    krb5_const_principal target)
{
...
static krb5_error_code acl_renprinc(krb5_context context,
                                    kadm5_auth_moddata data,
                                    krb5_const_principal client,
...
static krb5_error_code acl_getprinc(krb5_context context,
                                    kadm5_auth_moddata data,
                                    krb5_const_principal client,
                                    krb5_const_principal target)
{
...
static krb5_error_code acl_getstrs(krb5_context context,
                                   kadm5_auth_moddata data,
                                   krb5_const_principal client,
                                   krb5_const_principal target)
{
...
static krb5_error_code acl_extract(krb5_context context,
                                   kadm5_auth_moddata data,
                                   krb5_const_principal client,
                                   krb5_const_principal target)
{
...
static krb5_error_code acl_listprincs(krb5_context context,
                                      kadm5_auth_moddata data,
                                      krb5_const_principal client)
{
...
static krb5_error_code acl_addpol(krb5_context context, kadm5_auth_moddata data,
                                  krb5_const_principal client,
                                  const char* policy,
                                  const struct _kadm5_policy_ent_t* ent,
                                  long mask)
{
    return acl_check(data, ACL_ADD, client, NULL, NULL);
}

static krb5_error_code acl_modpol(krb5_context context, kadm5_auth_moddata data,
                                  krb5_const_principal client,
                                  const char* policy,
                                  const struct _kadm5_policy_ent_t* ent,
                                  long mask)
...
static krb5_error_code acl_delpol(krb5_context context, kadm5_auth_moddata data,
                                  krb5_const_principal client,
                                  const char* policy)
{
...
static krb5_error_code acl_getpol(krb5_context context, kadm5_auth_moddata data,
                                  krb5_const_principal client,
                                  const char* policy, const char* client_policy)
{
    return acl_check(data, ACL_INQUIRE, client, NULL, NULL);
}

static krb5_error_code acl_listpols(krb5_context context,
                                    kadm5_auth_moddata data,
                                    krb5_const_principal client)
{
    return acl_check(data, ACL_LIST, client, NULL, NULL);
...
static krb5_error_code acl_iprop(krb5_context context, kadm5_auth_moddata data,
                                 krb5_const_principal client)
{
