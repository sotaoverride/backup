        ...
        netsnmp_feature_require(calculate_sectime_diff)
            netsnmp_feature_require(allocate_globalcacheid)
                netsnmp_feature_require(remove_index)
                    ...
                    netsnmp_session* find_agentx_session(
                        netsnmp_session* session, int sessid)
{
    netsnmp_session* sp;
...
int open_agentx_session(netsnmp_session* session, netsnmp_pdu* pdu)
{
    netsnmp_session* sp;
    ...
    sp->engineTime =
        (uint32_t)((netsnmp_get_agent_runtime() + 50) / 100) & 0x7fffffffL;
    ...
    DEBUGMSGTL(("agentx/master", "opened %8p = %ld with flags = %02lx\n", sp,
                sp->sessid, sp->flags & AGENTX_MSG_FLAGS_MASK));
...
int close_agentx_session(netsnmp_session* session, int sessid)
{
    ...
    if (sessid == -1)
    {
        ...
        {
            DEBUGMSGTL(
                ("agentx/master", "Continue removing delegated reqests\n"));
        }
        ...
        if (session->subsession != NULL)
        {
            ...
            for (; subsession; subsession = subsession->next)
            {
                    ...
                    netsnmp_remove_delegated_requests_for_session(subsession))
                {
                    DEBUGMSGTL(
                        ("agentx/master",
                         "Continue removing delegated subsession reqests\n"));
                }
            }
        }
    ...
    for (sp = session->subsession; sp != NULL; sp = sp->next)
    {
        ...
        if (sp->sessid == sessid)
        {
            ...
            *prevNext = sp->next;
            ...
            DEBUGMSGTL(
                ("agentx/master", "closed %8p, %d okay\n", session, sessid));
            return AGENTX_ERR_NOERROR;
        }
...
int register_agentx_list(netsnmp_session* session, netsnmp_pdu* pdu)
{
    netsnmp_session* sp;
    char buf[128];
    oid ubound = 0;
    ...
    sprintf(buf, "AgentX subagent %ld, session %8p, subsession %8p", sp->sessid,
            session, sp);
    /*
    ...
    reg = netsnmp_create_handler_registration(
        buf, agentx_master_handler, pdu->variables->name,
        pdu->variables->name_length,
        HANDLER_CAN_RWRITE | HANDLER_CAN_GETBULK); /* fake it */
    if (!session->myvoid)
    {
     ...
     */
    switch (netsnmp_register_mib(
        buf, NULL, 0, 0, pdu->variables->name, pdu->variables->name_length,
        pdu->priority, pdu->range_subid, ubound, sp, (char*)pdu->community,
        pdu->time, flags, reg, 1))
    {
...
int unregister_agentx_list(netsnmp_session* session, netsnmp_pdu* pdu)
{
    netsnmp_session* sp;
    int rc = 0;
    ...
    if (pdu->range_subid != 0)
    {
        ...
        rc = netsnmp_unregister_mib_table_row(
            pdu->variables->name, pdu->variables->name_length, pdu->priority,
            pdu->range_subid, ubound, (char*)pdu->community);
    }
    else
    {
        rc = unregister_mib_context(pdu->variables->name,
                                    pdu->variables->name_length, pdu->priority,
                                    0, 0, (char*)pdu->community);
    }
...
int allocate_idx_list(netsnmp_session* session, netsnmp_pdu* pdu)
{
    netsnmp_session* sp;
...
int release_idx_list(netsnmp_session* session, netsnmp_pdu* pdu)
{
    netsnmp_session* sp;
...
int add_agent_caps_list(netsnmp_session* session, netsnmp_pdu* pdu)
{
    netsnmp_session* sp;
    char* description;
...
int remove_agent_caps_list(netsnmp_session* session, netsnmp_pdu* pdu)
{
    netsnmp_session* sp;
    int rc;
...
int agentx_notify(netsnmp_session* session, netsnmp_pdu* pdu)
{
    netsnmp_session* sp;
    ...
    if (snmp_oid_compare(var->name, var->name_length, sysuptime_oid,
                         sysuptime_oid_len) == 0)
    {
        var = var->next_variable;
    }

    if (!var || snmp_oid_compare(var->name, var->name_length, snmptrap_oid,
                                 snmptrap_oid_len) != 0)
        return AGENTX_ERR_PROCESSING_ERROR;
...
int agentx_ping_response(netsnmp_session* session, netsnmp_pdu* pdu)
{
    netsnmp_session* sp;
...
int handle_master_agentx_packet(int operation, netsnmp_session* session,
                                int reqid, netsnmp_pdu* pdu, void* magic)
{
    ...
    {
        DEBUGMSGTL(("agentx/master", "transport disconnect on session %8p\n",
                    session));
        /*
    ...
    {
        DEBUGMSGTL(
            ("agentx/master", "transport connect on session %8p\n", session));
        return 1;
    }
    ...
    DEBUGMSGTL(("agentx/master",
                "handle pdu (req=0x%lx,trans=0x%lx,sess=0x%lx)\n",
    ...
    DEBUGMSGTL(("agentx/master",
                "send response, stat %d (req=0x%lx,trans="
                "0x%lx,sess=0x%lx)\n",
