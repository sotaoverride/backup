...
extern int mnt_is_readonly(const char* path) __attribute__((nonnull));
...
extern int mnt_guess_system_root(dev_t devno, struct libmnt_cache* cache,
                                 char** path);
...
extern int __mnt_table_parse_mtab(struct libmnt_table* tb, const char* filename,
...
#define mnt_fs_is_regular(_f)                                                  \
    (!(mnt_fs_is_pseudofs(_f) || mnt_fs_is_netfs(_f) || mnt_fs_is_swaparea(_f)))
...
struct libmnt_table
{
    ...
    int (*errcb)(struct libmnt_table* tb, const char* filename, int line);
...
extern struct libmnt_table* __mnt_new_table_from_file(const char* filename,
                                                      int fmt,
...
struct libmnt_context
{
    ...
    struct libmnt_fs*
        fs; /* filesystem description (type, mountpoint, device, ...) */
    ...
    int (*table_errcb)(
        struct libmnt_table* tb, /* callback for libmnt_table structs */
        const char* filename, int line);

    int (*table_fltrcb)(struct libmnt_fs* fs,
                        void* data); /* callback for libmnt_table structs */
    void* table_fltrcb_data;

    char* (*pwd_get_cb)(struct libmnt_context*); /* get encryption password */
    void (*pwd_release_cb)(struct libmnt_context*,
...
#define MNT_FL_RWONLY_MOUNT                                                    \
    (1 << 14) /* explicit mount -w; never try read-only  */
...
#define MNT_FL_LOOPDEV_READY                                                   \
    (1 << 26) /* /dev/loop<N> initialized by the library */
#define MNT_FL_MOUNTOPTS_FIXED (1 << 27)
#define MNT_FL_TABPATHS_CHECKED (1 << 28)
#define MNT_FL_FORCED_RDONLY                                                   \
    (1 << 29) /* mounted read-only on write-protected device */

/* default flags */
#define MNT_FL_DEFAULT 0

/* Flags usable with MS_BIND|MS_REMOUNT */
#define MNT_BIND_SETTABLE                                                      \
...
/* optmap.c */
extern const struct libmnt_optmap*
    mnt_optmap_get_entry(struct libmnt_optmap const** maps, int nmaps,
...
extern int mnt_context_guess_srcpath_fstype(struct libmnt_context* cxt,
                                            char** type);
...
extern int mnt_context_umount_setopt(struct libmnt_context* cxt, int c,
                                     char* arg);
extern int mnt_context_mount_setopt(struct libmnt_context* cxt, int c,
                                    char* arg);
...
extern int mnt_context_get_generic_excode(int rc, char* buf, size_t bufsz,
                                          char* fmt, ...);
extern int mnt_context_get_mount_excode(struct libmnt_context* cxt, int mntrc,
                                        char* buf, size_t bufsz);
extern int mnt_context_get_umount_excode(struct libmnt_context* cxt, int mntrc,
                                         char* buf, size_t bufsz);
