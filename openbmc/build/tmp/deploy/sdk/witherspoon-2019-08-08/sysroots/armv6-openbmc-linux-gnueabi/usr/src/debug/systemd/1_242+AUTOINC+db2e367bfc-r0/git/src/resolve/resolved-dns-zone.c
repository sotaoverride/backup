...
static void dns_zone_item_remove_and_free(DnsZone* z, DnsZoneItem* i)
{
    ...
    if (first)
        assert_se(hashmap_replace(z->by_name,
                                  dns_resource_key_name(first->rr->key),
                                  first) >= 0);
    else
...
static int dns_zone_item_probe_start(DnsZoneItem* i)
{
    ...
    t = dns_scope_find_transaction(
        i->scope,
        &DNS_RESOURCE_KEY_CONST(i->rr->key->class, DNS_TYPE_ANY,
                                dns_resource_key_name(i->rr->key)),
        false);
    if (!t)
    {
        _cleanup_(dns_resource_key_unrefp) DnsResourceKey* key = NULL;

        key = dns_resource_key_new(i->rr->key->class, DNS_TYPE_ANY,
                                   dns_resource_key_name(i->rr->key));
        if (!key)
            return -ENOMEM;
...
static int dns_zone_add_authenticated_answer(DnsAnswer* a, DnsZoneItem* i,
                                             int ifindex)
{
...
int dns_zone_lookup(DnsZone* z, DnsResourceKey* key, int ifindex,
                    DnsAnswer** ret_answer, DnsAnswer** ret_soa,
                    bool* ret_tentative)
{
    _cleanup_(dns_answer_unrefp) DnsAnswer *answer = NULL, *soa = NULL;
    unsigned n_answer = 0;
    ...
    /* Note that we don't actually need the ifindex for anything. However when
     * it is passed we'll initialize the ifindex field in the answer with it */
    ...
    if (key->type == DNS_TYPE_ANY || key->class == DNS_CLASS_ANY)
    {
        ...
        LIST_FOREACH(by_name, j, first)
        {
            if (!IN_SET(j->state, DNS_ZONE_ITEM_PROBING,
                        DNS_ZONE_ITEM_ESTABLISHED, DNS_ZONE_ITEM_VERIFYING))
                continue;
    ...
    if (key->type == DNS_TYPE_ANY || key->class == DNS_CLASS_ANY)
    {
        ...
        if (found && !added)
        {
            r = dns_answer_add_soa(soa, dns_resource_key_name(key),
                                   LLMNR_DEFAULT_TTL, ifindex);
            if (r < 0)
                return r;
        }
    }
...
void dns_zone_item_conflict(DnsZoneItem* i)
{
    ...
    if (!IN_SET(i->state, DNS_ZONE_ITEM_PROBING, DNS_ZONE_ITEM_VERIFYING,
                DNS_ZONE_ITEM_ESTABLISHED))
        return;

    log_info("Detected conflict on %s",
             strna(dns_resource_record_to_string(i->rr)));
    ...
    if (manager_is_own_hostname(i->scope->manager,
                                dns_resource_key_name(i->rr->key)) > 0)
...
void dns_zone_item_notify(DnsZoneItem* i)
{
    ...
    if (IN_SET(i->probe_transaction->state, DNS_TRANSACTION_NULL,
               DNS_TRANSACTION_PENDING, DNS_TRANSACTION_VALIDATING))
        return;
    ...
    if (i->probe_transaction->state == DNS_TRANSACTION_SUCCESS)
    {
        ...
        if (!IN_SET(i->state, DNS_ZONE_ITEM_ESTABLISHED,
                    DNS_ZONE_ITEM_VERIFYING))
        {
            log_debug(
                "Got a successful probe for not yet established RR, we lost.");
            we_lost = true;
        }
        else if (i->probe_transaction->scope->protocol == DNS_PROTOCOL_LLMNR)
        {
            ...
            we_lost = memcmp(&i->probe_transaction->received->sender,
                             ...
                             FAMILY_ADDRESS_SIZE(
                                 i->probe_transaction->received->family)) < 0;
            if (we_lost)
                log_debug("Got a successful probe reply for an established RR, "
                          "and we have a lexicographically larger IP address "
                          "and thus lost.");
        }
        ...
        log_debug("Got a successful probe reply, but peer has "
                  "lexicographically lower IP address and thus lost.");
    }

    log_debug("Record %s successfully probed.",
              strna(dns_resource_record_to_string(i->rr)));
...
bool dns_zone_contains_name(DnsZone* z, const char* name)
{
    ...
    LIST_FOREACH(by_name, i, first)
    {
        if (!IN_SET(i->state, DNS_ZONE_ITEM_PROBING, DNS_ZONE_ITEM_ESTABLISHED,
                    DNS_ZONE_ITEM_VERIFYING))
            continue;
