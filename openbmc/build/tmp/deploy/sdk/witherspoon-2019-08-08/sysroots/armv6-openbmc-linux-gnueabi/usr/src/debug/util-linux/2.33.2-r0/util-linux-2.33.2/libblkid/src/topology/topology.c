...
#ifdef __linux__
    &ioctl_tp_idinfo, &sysfs_tp_idinfo, &md_tp_idinfo, &dm_tp_idinfo,
    &lvm_tp_idinfo,   &evms_tp_idinfo
#endif
};
 ...
 * Driver definition
 */
const struct blkid_chaindrv topology_drv = {.id = BLKID_CHAIN_TOPLGY,
                                            .name = "topology",
                                            .dflt_enabled = FALSE,
                                            .idinfos = idinfos,
...
{
    return (blkid_topology)blkid_probe_get_binary_data(
        pr, &pr->chains[BLKID_CHAIN_TOPLGY]);
}
...
static int topology_probe(blkid_probe pr, struct blkid_chain* chn)
{
    ...
    if (chn->binary)
    {
        ...
        if (chn->data)
            /* reset binary data */
            memset(chn->data, 0, sizeof(struct blkid_struct_topology));
        else
        {
            chn->data = calloc(1, sizeof(struct blkid_struct_topology));
            if (!chn->data)
                return -ENOMEM;
        }
    }
    ...
    DBG(LOWPROBE,
        ul_debug("--> starting probing loop [TOPOLOGY idx=%d]", chn->idx));
    ...
    for (; i < ARRAY_SIZE(idinfos); i++)
    {
        ...
        DBG(LOWPROBE,
            ul_debug("<-- leaving probing loop (type=%s) [TOPOLOGY idx=%d]",
                     id->name, chn->idx));
        return BLKID_PROBE_OK;
    }

    DBG(LOWPROBE,
        ul_debug("<-- leaving probing loop (failed) [TOPOLOGY idx=%d]",
                 chn->idx));
...
int blkid_topology_set_alignment_offset(blkid_probe pr, int val)
{
    ...
    return topology_set_value(
        pr, "ALIGNMENT_OFFSET",
        offsetof(struct blkid_struct_topology, alignment_offset), xval);
}
...
int blkid_topology_set_minimum_io_size(blkid_probe pr, unsigned long val)
{
    return topology_set_value(
        pr, "MINIMUM_IO_SIZE",
        offsetof(struct blkid_struct_topology, minimum_io_size), val);
}
...
int blkid_topology_set_optimal_io_size(blkid_probe pr, unsigned long val)
{
    return topology_set_value(
        pr, "OPTIMAL_IO_SIZE",
        offsetof(struct blkid_struct_topology, optimal_io_size), val);
}
...
static int topology_set_logical_sector_size(blkid_probe pr)
{
    ...
    return topology_set_value(
        pr, "LOGICAL_SECTOR_SIZE",
        offsetof(struct blkid_struct_topology, logical_sector_size), val);
}

int blkid_topology_set_physical_sector_size(blkid_probe pr, unsigned long val)
{
    return topology_set_value(
        pr, "PHYSICAL_SECTOR_SIZE",
        offsetof(struct blkid_struct_topology, physical_sector_size), val);
}
