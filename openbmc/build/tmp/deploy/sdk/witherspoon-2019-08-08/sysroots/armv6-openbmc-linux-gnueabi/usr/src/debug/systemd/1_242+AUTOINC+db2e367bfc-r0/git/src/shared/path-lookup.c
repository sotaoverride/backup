...
static const char* const user_data_unit_paths[] = {
    ...
    USER_DATA_UNIT_PATH,           "/usr/lib/systemd/user",
    "/usr/share/systemd/user",     NULL};

static const char* const user_config_unit_paths[] = {USER_CONFIG_UNIT_PATH,
...
static char** user_dirs(const char* persistent_config,
                        ...
                        const char* generator, const char* generator_early,
                        const char* generator_late, const char* transient,
...
static int acquire_generator_dirs(UnitFileScope scope, const char* tempdir,
                                  char** generator, char** generator_early,
                                  char** generator_late)
{
...
static int acquire_transient_dir(UnitFileScope scope, const char* tempdir,
                                 char** ret)
{
...
static int acquire_config_dirs(UnitFileScope scope, char** persistent,
                               char** runtime)
{
    ...
    switch (scope)
    {
        ...
        case UNIT_FILE_USER:
            ...
            if (r < 0)
            {
                ...
                /* If XDG_RUNTIME_DIR is not set, don't consider that fatal,
                 * simply initialize the runtime directory to NULL */
                *runtime = NULL;
            }
...
static int acquire_control_dirs(UnitFileScope scope, char** persistent,
                                char** runtime)
{
    ...
    switch (scope)
    {
        ...
        case UNIT_FILE_USER:
            ...
            if (r < 0)
            {
                ...
                /* If XDG_RUNTIME_DIR is not set, don't consider this fatal,
                 * simply initialize the directory to NULL */
                *runtime = NULL;
            }
...
static int acquire_attached_dirs(UnitFileScope scope, char** ret_persistent,
                                 char** ret_runtime)
{
...
int lookup_paths_init(LookupPaths* p, UnitFileScope scope,
                      LookupPathsFlags flags, const char* root_dir)
{

    _cleanup_(rmdir_and_freep) char* tempdir = NULL;
    _cleanup_free_ char *root = NULL, *persistent_config = NULL,
                        *runtime_config = NULL,
                        *global_persistent_config = NULL,
                        *global_runtime_config = NULL, *generator = NULL,
                        *generator_early = NULL, *generator_late = NULL,
                        *transient = NULL, *persistent_control = NULL,
                        *runtime_control = NULL, *persistent_attached = NULL,
    ...
    /* Note: when XDG_RUNTIME_DIR is not set this will not return -ENXIO, but
     * simply set runtime_config to NULL */
    ...
    if (scope == UNIT_FILE_USER)
    {
        r = acquire_config_dirs(UNIT_FILE_GLOBAL, &global_persistent_config,
    ...
    {
        /* Note: if XDG_RUNTIME_DIR is not set, this will fail completely with
         * ENXIO */
        r = acquire_generator_dirs(scope, tempdir, &generator, &generator_early,
                                   &generator_late);
    ...
    /* Note: when XDG_RUNTIME_DIR is not set this will not return -ENXIO, but
     * simply set runtime_control to NULL */
    ...
    if (!paths || append)
    {
        ...
        switch (scope)
        {
            ...
            case UNIT_FILE_SYSTEM:
                add = strv_new(
                    ...
                    STRV_IFNOTNULL(runtime_control), STRV_IFNOTNULL(transient),
                    STRV_IFNOTNULL(generator_early), persistent_config,
                    SYSTEM_CONFIG_UNIT_PATH, "/etc/systemd/system",
                    STRV_IFNOTNULL(persistent_attached), runtime_config,
                    "/run/systemd/system", STRV_IFNOTNULL(runtime_attached),
                    STRV_IFNOTNULL(generator), "/usr/local/lib/systemd/system",
                    SYSTEM_DATA_UNIT_PATH, "/usr/lib/systemd/system",
                    STRV_IFNOTNULL(flags & LOOKUP_PATHS_SPLIT_USR
                                       ? "/lib/systemd/system"
                                       : NULL),
            ...
            case UNIT_FILE_GLOBAL:
                add = strv_new(
                    ...
                    STRV_IFNOTNULL(runtime_control), STRV_IFNOTNULL(transient),
                    STRV_IFNOTNULL(generator_early), persistent_config,
                    USER_CONFIG_UNIT_PATH, "/etc/systemd/user", runtime_config,
                    "/run/systemd/user", STRV_IFNOTNULL(generator),
                    "/usr/local/share/systemd/user", "/usr/share/systemd/user",
                    "/usr/local/lib/systemd/user", USER_DATA_UNIT_PATH,
                    "/usr/lib/systemd/user", STRV_IFNOTNULL(generator_late));
                break;
            ...
            case UNIT_FILE_USER:
                add = user_dirs(persistent_config, runtime_config,
                                ...
                                transient, persistent_control, runtime_control);
                break;
        ...
        if (paths)
        {
            ...
            /* Small optimization: if paths is NULL (and it usually is), we can
             * simply assign 'add' to it, and don't have to copy anything */
            paths = TAKE_PTR(add);
    }
...
int lookup_paths_reduce(LookupPaths* p)
{
    ...
    /* Drop duplicates and non-existing directories from the search path. We
     * figure out whether two directories are
    ...
    while (p->search_path[c])
    {
        ...
        r = chase_symlinks_and_stat(p->search_path[c], p->root_dir, 0, NULL,
                                    &st);
        if (r == -ENOENT)
            goto remove_item;
        if (r < 0)
        {
            ...
            log_debug_errno(r, "Failed to chase and stat %s: %m",
                            p->search_path[c]);
            c++;
            continue;
        }
        ...
        if (k <
            n_stats) /* Is there already an entry with the same device/inode? */
            goto remove_item;
    ...
    remove_item:
        ...
        memmove(p->search_path + c, p->search_path + c + 1,
    ...
    switch (scope)
    {
        ...
        case UNIT_FILE_USER:
            return strv_new(
                ...
                "/usr/local/lib/systemd/user-generators", USER_GENERATOR_PATH);
