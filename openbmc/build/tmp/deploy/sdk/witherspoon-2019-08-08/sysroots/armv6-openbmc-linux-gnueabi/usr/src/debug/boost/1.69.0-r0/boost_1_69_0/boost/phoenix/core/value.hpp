...
{
template <typename T>
struct value : expression::terminal<T>
{
...
template <typename T>
inline typename expression::value<T>::type const val(T t)
{
...
struct is_custom_terminal<actor<Expr>> : mpl::true_
{
};
...
struct custom_terminal<actor<Expr>>
{
    ...
    struct result<This(Actor, Context)>
        : boost::remove_const<
              typename boost::remove_reference<typename evaluator::impl<
                  Actor, Context, proto::empty_env>::result_type>::type>
    {
    };
...
{
template <typename T>
struct const_ref : add_reference<typename add_const<T>::type>
{
};

template <typename T>
struct argument_type
    : mpl::eval_if_c<is_function<typename remove_pointer<T>::type>::value,
                     mpl::identity<T>, const_ref<T>>
{
    typedef T type;
};
...
{
    typedef
        typename expression::value<typename meta::decay<T>::type>::type type;

    static type convert(
        typename meta::argument_type<typename meta::decay<T>::type>::type t)
    {
