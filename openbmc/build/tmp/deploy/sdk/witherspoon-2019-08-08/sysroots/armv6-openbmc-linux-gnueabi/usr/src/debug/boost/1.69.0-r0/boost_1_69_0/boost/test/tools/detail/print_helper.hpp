...
//
//  Description : defines level of indiration facilitating workarounds for non
//  printable types
// ***************************************************************************
...
struct print_log_value
{
    ...
    {
        typedef
            typename mpl::or_<is_array<T>, is_function<T>, is_abstract<T>>::type
                cant_use_nl;
    ...
    std::streamsize set_precision(std::ostream& ostr, mpl::false_)
    {
                 ...
                 std::numeric_limits<T>::radix == 10)
        {
            ...
            // so guess that a couple of guard digits more than digits10 will
    ...
    std::streamsize set_precision(std::ostream&, mpl::true_)
    {
        return (std::streamsize)-1;
    }
};
...
{
    // declaration and definition is here because of #12969
    // https://svn.boost.org/trac10/ticket/12969
...
template <typename T>
inline std::ostream& operator<<(std::ostream& ostr, print_helper_t<T> const& ph)
{
