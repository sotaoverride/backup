...
G_DEFINE_TYPE_WITH_CODE(
    GLocalVfs, g_local_vfs, G_TYPE_VFS,
    _g_io_modules_ensure_extension_points_registered();
    g_io_extension_point_implement(G_VFS_EXTENSION_POINT_NAME, g_define_type_id,
                                   "local", 0))
...
static void g_local_vfs_init(GLocalVfs* vfs)
{
}
...
static GFile* g_local_vfs_get_file_for_path(GVfs* vfs, const char* path)
{
    return _g_local_file_new(path);
}

static GFile* g_local_vfs_get_file_for_uri(GVfs* vfs, const char* uri)
{
    char* path;
    GFile* file;
...
static const gchar* const* g_local_vfs_get_supported_uri_schemes(GVfs* vfs)
{
...
static GFile* g_local_vfs_parse_name(GVfs* vfs, const char* parse_name)
{
    GFile* file;
    char* filename;
    char* user_prefix;
...
static gboolean g_local_vfs_is_active(GVfs* vfs)
{
    return TRUE;
}
