...
void init_mteObjectsConf(void)
{
     ...
     */
    snmpd_register_config_handler("_mteOTable", parse_mteOTable, NULL, NULL);
    snmpd_register_config_handler("mteObjectsTable", parse_mteOTable, NULL,
                                  NULL);
    /*
    ...
    snmp_register_callback(SNMP_CALLBACK_APPLICATION,
                           SNMPD_CALLBACK_PRE_UPDATE_CONFIG, clear_mteOTable,
                           NULL);
}
...
int store_mteOTable(int majorID, int minorID, void* serverarg, void* clientarg)
{
    ...
    for (row = netsnmp_tdata_row_first(objects_table_data); row;
         row = netsnmp_tdata_row_next(objects_table_data, row))
    {
        ...
        cptr = read_config_store_data(ASN_OCTET_STR, cptr, &cp, &tint);
        cptr =
            read_config_store_data(ASN_UNSIGNED, cptr, &entry->mteOIndex, NULL);
        vp = entry->mteObjectID;
...
int clear_mteOTable(int majorID, int minorID, void* serverarg, void* clientarg)
{
    ...
    snmp_set_var_typed_value(&owner_var, ASN_OCTET_STR, "snmpd.conf",
                             strlen("snmpd.conf"));
    while ((row = netsnmp_tdata_row_next_byidx(objects_table_data, &owner_var)))
    {
        /*
