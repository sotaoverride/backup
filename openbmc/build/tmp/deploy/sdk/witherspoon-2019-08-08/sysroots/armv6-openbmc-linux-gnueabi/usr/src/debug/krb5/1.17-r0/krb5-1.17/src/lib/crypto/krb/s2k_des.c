...
static krb5_error_code des_cbc_mac(const unsigned char* keybits,
...
static void afs_crypt_setkey(char* key, char* E, char (*KS)[48])
{
    int i, j, k, t;
...
static void afs_encrypt_block(char* block, char* E, char (*KS)[48])
{
    const long edflag = 0;
    int i, ii;
    int t, j, k;
...
static char* afs_crypt(const char* pw, const char* salt, char* iobuf)
{
    int i, j, c;
    int temp;
    char block[66];
...
static krb5_error_code afs_s2k_oneblock(const krb5_data* data,
...
static krb5_error_code afs_s2k_multiblock(const krb5_data* data,
                                          const krb5_data* salt,
                                          unsigned char* key_out)
{
    krb5_error_code ret;
...
static krb5_error_code afs_s2k(const krb5_data* data, const krb5_data* salt,
                               unsigned char* key_out)
{
    if (data->length <= 8)
...
static krb5_error_code des_s2k(const krb5_data* pw, const krb5_data* salt,
                               unsigned char* key_out)
{
    union
    {
    ...
    if (sizeof(temp.uc) != sizeof(temp.ui) || (unsigned char)~0 != 0xFF ||
        (krb5_ui_4) ~(krb5_ui_4)0 != 0xFFFFFFFF ||
        (temp.uc[0] = 1, temp.uc[1] = 2, temp.uc[2] = 3, temp.uc[3] = 4,
         !(temp.ui[0] == 0x01020304 || temp.ui[0] == 0x04030201)))
        abort();
...
#define FIXUP(k)                                                               \
    (k5_des_fixup_key_parity(k), k5_des_is_weak_key(k) ? (k[7] ^= 0xF0) : 0)
