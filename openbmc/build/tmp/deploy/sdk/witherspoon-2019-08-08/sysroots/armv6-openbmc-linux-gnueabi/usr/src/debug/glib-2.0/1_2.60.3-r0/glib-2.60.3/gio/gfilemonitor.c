...
static void g_file_monitor_set_property(GObject* object, guint prop_id,
...
static void g_file_monitor_get_property(GObject* object, guint prop_id,
...
static void g_file_monitor_dispose(GObject* object)
{
...
static void g_file_monitor_init(GFileMonitor* monitor)
{
    monitor->priv = g_file_monitor_get_instance_private(monitor);
}

static void g_file_monitor_class_init(GFileMonitorClass* klass)
{
     ...
     *
     * If using the deprecated flag %G_FILE_MONITOR_SEND_MOVED flag and
     *@event_type is #G_FILE_MONITOR_EVENT_MOVED, @file will be set to a #GFile
     *containing the old path, and @other_file will be set to a #GFile
     *containing the new path.
     *
     * In all the other cases, @other_file will be set to #NULL.
     **/
    g_file_monitor_changed_signal = g_signal_new(
        I_("changed"), G_TYPE_FILE_MONITOR, G_SIGNAL_RUN_LAST,
        G_STRUCT_OFFSET(GFileMonitorClass, changed), NULL, NULL, NULL,
        G_TYPE_NONE, 3, G_TYPE_FILE, G_TYPE_FILE, G_TYPE_FILE_MONITOR_EVENT);

    g_object_class_install_property(
        object_class, PROP_RATE_LIMIT,
        g_param_spec_int("rate-limit", P_("Rate limit"),
                         P_("The limit of the monitor to watch for changes, in "
                            "milliseconds"),
                         0, G_MAXINT, DEFAULT_RATE_LIMIT_MSECS,
                         G_PARAM_READWRITE | G_PARAM_EXPLICIT_NOTIFY |
                             G_PARAM_STATIC_STRINGS));

    g_object_class_install_property(
        object_class, PROP_CANCELLED,
        g_param_spec_boolean("cancelled", P_("Cancelled"),
 ...
 **/
void g_file_monitor_emit_event(GFileMonitor* monitor, GFile* child,
                               GFile* other_file, GFileMonitorEvent event_type)
{
    ...
    g_signal_emit(monitor, g_file_monitor_changed_signal, 0, child, other_file,
                  event_type);
}
