 ...
 */
int krb5int_dns_init(struct krb5int_dns_state** dsp, char* host, int nclass,
                     int ntype)
{
    ...
    *dsp = ds = malloc(sizeof(*ds));
    ...
    do
    {
        p = (ds->ansp == NULL) ? malloc(nextincr) : realloc(ds->ansp, nextincr);
 ...
 * success.
 */
int krb5int_dns_nextans(struct krb5int_dns_state* ds, const unsigned char** pp,
                        int* lenp)
{
    int len;
    ns_rr rr;
 ...
 */
int krb5int_dns_expand(struct krb5int_dns_state* ds, const unsigned char* p,
                       char* buf, int len)
{

#if HAVE_NS_NAME_UNCOMPRESS
    return ns_name_uncompress(ds->ansp, (unsigned char*)ds->ansp + ds->anslen,
                              p, buf, (size_t)len);
#else
    return dn_expand(ds->ansp, (unsigned char*)ds->ansp + ds->anslen, p, buf,
                     len);
#endif
}
 ...
 * Free stuff.
 */
void krb5int_dns_fini(struct krb5int_dns_state* ds)
{
    if (ds == NULL)
        return;
    if (ds->ansp != NULL)
 ...
 * answer records.
 */
static int initparse(struct krb5int_dns_state* ds)
{
    HEADER* hdr;
    unsigned char* p;
...
#else
        len = dn_expand(ds->ansp, (unsigned char*)ds->ansp + ds->anslen, p,
                        host, sizeof(host));
#endif
...
static char* txt_lookup_name(const char* prefix, const char* name)
{
    struct k5buf buf;
                                    ...
                                    const char* name, char** realm)
{
    ...
    *realm = NULL;
    ...
    st = DnsQuery_UTF8(txtname, DNS_TYPE_TEXT, DNS_QUERY_STANDARD, NULL, &rr,
                       NULL);
