...
static void erase(data_node entry)
{
...
void netsnmp_sysORTable_foreach(void (*f)(const struct sysORTable*, void*),
                                void* c)
{
...
int register_sysORTable_sess(oid* oidin, size_t oidlen, const char* descr,
                             netsnmp_session* ss)
{
    data_node entry;
...
int unregister_sysORTable_sess(oid* oidin, size_t oidlen, netsnmp_session* ss)
{
    ...
    if (table)
    {
        ...
        do
        {
            ...
            if (tmp->data.OR_sess == ss &&
                snmp_oid_compare(oidin, oidlen, tmp->data.OR_oid,
                                 tmp->data.OR_oidlen) == 0)
            {
                erase(tmp);
                any_unregistered = 1;
            }
...
{
    DEBUGMSGTL(
        ("agent/sysORTable", "sysORTable unregistering session %p\n", ss));
    ...
    DEBUGMSGTL(
        ("agent/sysORTable", "sysORTable unregistering session %p done\n", ss));
}

static int register_sysOR_callback(int majorID, int minorID, void* serverarg,
                                   void* clientarg)
{
...
static int unregister_sysOR_by_session_callback(int majorID, int minorID,
                                                void* serverarg,
                                                void* clientarg)
{
...
static int unregister_sysOR_callback(int majorID, int minorID, void* serverarg,
                                     void* clientarg)
{
    struct sysORTable* parms = (struct sysORTable*)serverarg;

    return unregister_sysORTable_sess(parms->OR_oid, parms->OR_oidlen,
                                      parms->OR_sess);
}
