...
static void route_dump_line(struct nl_object* a, struct nl_dump_params* p)
{
    ...
    if (!(r->ce_mask & ROUTE_ATTR_DST) || nl_addr_get_len(r->rt_dst) == 0)
        nl_dump(p, "default ");
    else
    ...
    if (r->ce_mask & ROUTE_ATTR_TYPE)
        nl_dump(p, "type %s ", nl_rtntype2str(r->rt_type, buf, sizeof(buf)));
...
static void route_dump_details(struct nl_object* a, struct nl_dump_params* p)
{
    ...
    if (r->ce_mask & ROUTE_ATTR_SCOPE && r->rt_scope != RT_SCOPE_NOWHERE)
        nl_dump(p, "scope %s ", rtnl_scope2str(r->rt_scope, buf, sizeof(buf)));
    ...
    if (r->ce_mask & ROUTE_ATTR_IIF)
    {
        ...
        {
            nl_dump(p, "iif %s ",
                    rtnl_link_i2name(link_cache, r->rt_iif, buf, sizeof(buf)));
        }
        else
    ...
    if (r->ce_mask & ROUTE_ATTR_METRICS)
    {
        ...
        for (i = 0; i < RTAX_MAX; i++)
            if (r->rt_metrics_mask & (1 << i))
                nl_dump(p, "%s %u ",
                        rtnl_route_metric2str(i + 1, buf, sizeof(buf)),
                        r->rt_metrics[i]);
        nl_dump(p, "]\n");
    }
                         ...
                         uint32_t table_sz)
{
    ...
    *hashkey = nl_hash(rkey, rkey_sz, 0) % table_sz;
    ...
    NL_DBG(5,
           ...
           nl_addr2str(addr, buf, sizeof(buf)), rkey_sz, *hashkey);
...
#define ROUTE_DIFF(ATTR, EXPR) ATTR_DIFF(attrs, ROUTE_ATTR_##ATTR, a, b, EXPR)
    ...
    diff |= ROUTE_DIFF(PREF_SRC, nl_addr_cmp(a->rt_pref_src, b->rt_pref_src));
    diff |=
        ROUTE_DIFF(TTL_PROPAGATE, a->rt_ttl_propagate != b->rt_ttl_propagate);
    ...
    if (flags & LOOSE_COMPARISON)
    {
        nl_list_for_each_entry(nh_b, &b->rt_nexthops, rtnh_list)
        {
            ...
            {
                if (!rtnl_route_nh_compare(nh_a, nh_b, nh_b->ce_mask, 1))
                {
                    found = 1;
                    break;
                }
        ...
        diff |=
            ROUTE_DIFF(FLAGS, (a->rt_flags ^ b->rt_flags) & b->rt_flag_mask);
    }
    else
    {
        ...
        for (i = 0; i < RTAX_MAX - 1; i++)
        {
            if ((a->rt_metrics_mask & (1 << i)) ^
                ...
                diff |=
                    ROUTE_DIFF(METRICS, a->rt_metrics[i] != b->rt_metrics[i]);
        }
            ...
            nl_list_for_each_entry(old_nh, &old_route->rt_nexthops, rtnh_list)
            {
                if (!rtnl_route_nh_compare(old_nh, new_nh, ~0, 0))
                {
                    ...
                    NL_DBG(2,
                           ...
                           "nexthop %p via %s\n",
                           old_route, old_nh,
                           nl_addr2str(old_nh->rtnh_gateway, buf, sizeof(buf)));
...
{
    return __flags2str(attrs, buf, len, route_attrs, ARRAY_SIZE(route_attrs));
}
...
static int parse_multipath(struct rtnl_route* route, struct nlattr* attr)
{
    ...
    while (tlen >= sizeof(*rtnh) && tlen >= rtnh->rtnh_len)
    {
        ...
        if (rtnh->rtnh_len > sizeof(*rtnh))
        {
            ...
            err = nla_parse(ntb, RTA_MAX, (struct nlattr*)RTNH_DATA(rtnh),
                            rtnh->rtnh_len - sizeof(*rtnh), route_policy);
            if (err < 0)
                goto errout;
            ...
            if (ntb[RTA_GATEWAY])
            {
                ...
                addr = nl_addr_alloc_attr(ntb[RTA_GATEWAY], route->rt_family);
                if (!addr)
                {
                    err = -NLE_NOMEM;
                    goto errout;
            ...
            if (ntb[RTA_NEWDST])
            {
                ...
                addr = nl_addr_alloc_attr(ntb[RTA_NEWDST], route->rt_family);
                if (!addr)
                    goto errout;
            ...
            {
                err =
                    nh_encap_parse_msg(ntb[RTA_ENCAP], ntb[RTA_ENCAP_TYPE], nh);
                if (err)
                    goto errout;
            }
        }
...
int rtnl_route_parse(struct nlmsghdr* nlh, struct rtnl_route** result)
{
    ...
    route->ce_mask |= ROUTE_ATTR_FAMILY | ROUTE_ATTR_TOS | ROUTE_ATTR_TABLE |
                      ROUTE_ATTR_TYPE | ROUTE_ATTR_SCOPE | ROUTE_ATTR_PROTOCOL |
                      ROUTE_ATTR_FLAGS;
    ...
    {
        rtnl_route_set_ttl_propagate(route, nla_get_u8(tb[RTA_TTL_PROPAGATE]));
    }

    if (tb[RTA_ENCAP] && tb[RTA_ENCAP_TYPE])
    {
        err = nh_encap_parse_msg(tb[RTA_ENCAP], tb[RTA_ENCAP_TYPE], old_nh);
        if (err)
            goto errout;
    }
    ...
    if (old_nh)
    {
        ...
        else
        {
            ...
            if (rtnl_route_nh_compare(old_nh, first, old_nh->ce_mask, 0))
            {
                err = -NLE_INVAL;
                goto errout;
            }
...
int rtnl_route_build_msg(struct nl_msg* msg, struct rtnl_route* route)
{
    ...
    if (rtnl_route_get_nnexthops(route) == 1)
    {
        ...
        if (nh->rtnh_via && rtnl_route_put_via(msg, nh->rtnh_via) < 0)
            goto nla_put_failure;
        if (nh->rtnh_encap && nh_encap_build_msg(msg, nh->rtnh_encap) < 0)
            goto nla_put_failure;
    }
    else if (rtnl_route_get_nnexthops(route) > 1)
    {
        ...
        nl_list_for_each_entry(nh, &route->rt_nexthops, rtnh_list)
        {
            ...
            rtnh->rtnh_len = nlmsg_tail(msg->nm_nlh) - (void*)rtnh;
        }
...
struct nl_object_ops route_obj_ops = {
    ...
    .oo_id_attrs = (ROUTE_ATTR_FAMILY | ROUTE_ATTR_TOS | ROUTE_ATTR_TABLE |
