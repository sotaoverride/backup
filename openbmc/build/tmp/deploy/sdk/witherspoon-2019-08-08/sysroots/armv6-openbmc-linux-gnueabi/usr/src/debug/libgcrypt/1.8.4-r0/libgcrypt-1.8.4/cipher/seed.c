 ...
 */
static gcry_err_code_t do_setkey(SEED_context* ctx, const byte* key,
...
static gcry_err_code_t seed_setkey(void* context, const byte* key,
                                   const unsigned keylen)
{
...
static void do_encrypt(const SEED_context* ctx, byte* outbuf, const byte* inbuf)
{
    u32 x1, x2, x3, x4;
    union wordbuf t0, t1;
...
static unsigned int seed_encrypt(void* context, byte* outbuf, const byte* inbuf)
{
    SEED_context* ctx = context;
...
static unsigned int seed_decrypt(void* context, byte* outbuf, const byte* inbuf)
{
    SEED_context* ctx = context;
...
static const char* selftest(void)
{
    ...
    static const byte plaintext[16] = {0x83, 0xA2, 0xF8, 0xA2, 0x88, 0x64,
                                       ...
                                       0x2F, 0x13, 0x1C, 0x7D};
    static const byte key[16] = {0x47, 0x06, 0x48, 0x08, 0x51, 0xE6,
                                 0x1B, 0xE8, 0x5D, 0x74, 0xBF, 0xB3,
                                 0xFD, 0x95, 0x61, 0x85};
