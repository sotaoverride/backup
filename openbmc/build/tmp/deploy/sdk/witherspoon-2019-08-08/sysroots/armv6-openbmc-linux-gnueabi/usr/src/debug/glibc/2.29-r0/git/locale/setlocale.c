    ...
    if (category == LC_ALL)
    {
       ...
       of entries of the form `CATEGORY=VALUE'.  */
        ...
        while (category-- > 0)
            ...
            {
                newdata[category] =
                    _nl_find_locale(locale_path, locale_path_len, category,
                                    &newnames[category]);
        ...
        composite =
            (category >= 0 ? NULL : new_composite_name(LC_ALL, newnames));
        if (composite != NULL)
        {
...
libc_hidden_def(setlocale)
    ...
    free_category(int category, struct __locale_data* here,
                  struct __locale_data* c_data)
{
...
#define DEFINE_CATEGORY(category, category_name, items, a)                     \
    ...
    {                                                                          \
        ...
        weak_extern(_nl_C_##category) free_category(                           \
            category, *_nl_current_##category, &_nl_C_##category);             \
    }
#include "categories.def"
#undef DEFINE_CATEGORY
#else
