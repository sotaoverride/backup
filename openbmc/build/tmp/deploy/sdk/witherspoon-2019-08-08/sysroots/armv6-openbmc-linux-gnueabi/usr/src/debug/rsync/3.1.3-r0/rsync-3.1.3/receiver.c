...
int get_tmpname(char* fnametmp, const char* fname, BOOL make_unique)
{
    ...
    if (maxname)
    {
         ...
         * is also a high-bit char, just in case we cut into a multi-byte
         * sequence. We are guaranteed to stop because of the leading '.' we
         * added. */
        if ((int)f[added] & 0x80)
        {
    ...
    if (make_unique)
    {
        ...
        if (!counter_limit)
        {
            ...
            if (counter_limit > MAX_UNIQUE_NUMBER ||
                        ...
                        const char* fname, int fd, OFF_T total_size)
{
        ...
        if (inplace)
    {
        ...
        /* The most compatible way to create a sparse file is to start with no
         * length. */
        if (sparse_files > 0 && whole_file && fd >= 0 &&
            do_ftruncate(fd, 0) == 0)
            preallocated_len = 0;
        else
#endif
    ...
    if (append_mode > 0)
    {
        ...
        if (append_mode == 2 && mapbuf)
        {
            for (j = CHUNK_SIZE; j < sum.flength; j += CHUNK_SIZE)
            {
                ...
                sum_update(map_ptr(mapbuf, offset, CHUNK_SIZE), CHUNK_SIZE);
                offset = j;
            }
    ...
    while ((i = recv_token(f_in, &data)) != 0)
    {
        ...
        if (allowed_lull)
            maybe_send_keepalive(time(NULL),
                                 MSK_ALLOW_FLUSH | MSK_ACTIVE_RECEIVER);
        ...
        if (DEBUG_GTE(DELTASUM, 3))
        {
            rprintf(FINFO, "chunk[%d] of size %ld at %s offset=%s%s\n", i,
                    (long)len, big_num(offset2), big_num(offset),
    ...
    if ((inplace || preallocated_len > offset) && fd != -1 &&
        do_ftruncate(fd, offset) < 0)
    {
...
static void handle_delayed_updates(char* local_name)
{
    ...
    for (ndx = -1; (ndx = bitbag_next_bit(delayed_bits, ndx)) >= 0;)
    {
        ...
        if ((partialptr = partial_dir_fname(fname)) != NULL)
        {
            ...
            {
                rsyserr(FERROR_XFER, errno, "rename failed for %s (from %s)",
...
int recv_files(int f_in, int f_out, char* local_name)
{
    ...
    while (1)
    {
        ...
        ndx = read_ndx_and_attrs(f_in, f_out, &iflags, &fnamecmp_type, xname,
                                 &xlen);
        if (ndx == NDX_DONE)
        {
        ...
        if (phase == 2)
        {
            rprintf(FERROR, "got transfer request in phase 2 [%s]\n",
                    who_am_i());
            exit_cleanup(RERR_PROTOCOL);
        }
        ...
        if (protocol_version >= 29)
        {
            switch (fnamecmp_type)
            {
                ...
                case FNAMECMP_FUZZY:
                    if (file->dirname)
                    {
                        pathjoin(fnamecmpbuf, sizeof fnamecmpbuf, file->dirname,
                                 xname);
                        ...
                        fnamecmp_type - FNAMECMP_FUZZY <= basis_dir_cnt)
                    {
                        ...
                        if (file->dirname)
                        {
                            stringjoin(fnamecmpbuf, sizeof fnamecmpbuf,
                                       basis_dir[fnamecmp_type], "/",
                                       file->dirname, "/", xname, NULL);
                        }
                        else
                            pathjoin(fnamecmpbuf, sizeof fnamecmpbuf,
                                     basis_dir[fnamecmp_type], xname);
                    }
                    else if (fnamecmp_type >= basis_dir_cnt)
                    {
                        rprintf(FERROR, "invalid basis_dir index: %d.\n",
                                fnamecmp_type);
                        exit_cleanup(RERR_PROTOCOL);
                    }
                    else
        ...
        if (fd1 == -1 && protocol_version < 29)
        {
            ...
            if (fd1 == -1 && basis_dir[0])
            {
                ...
                pathjoin(fnamecmpbuf, sizeof fnamecmpbuf, basis_dir[0], fname);
                fnamecmp = fnamecmpbuf;
                fd1 = do_open(fnamecmp, O_RDONLY, 0);
            }
        }
        ...
        /* recv file data */
        recv_ok = receive_data(f_in, fnamecmp, fd1, st.st_size, fname, fd2,
                               F_LENGTH(file));
        ...
        if (close(fd2) < 0)
        {
            rsyserr(FERROR, errno, "close failed on %s", full_fname(fnametmp));
            exit_cleanup(RERR_FILEIO);
        }
        ...
        if ((recv_ok && (!delay_updates || !partialptr)) || inplace)
        {
            if (partialptr == fname)
                partialptr = NULL;
            if (!finish_transfer(fname, fnametmp, fnamecmp, partialptr, file,
                                 recv_ok, 1))
                recv_ok = -1;
