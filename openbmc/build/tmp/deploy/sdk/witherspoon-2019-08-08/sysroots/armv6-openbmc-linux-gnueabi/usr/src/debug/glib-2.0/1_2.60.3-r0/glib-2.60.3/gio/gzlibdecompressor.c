...
static void g_zlib_decompressor_set_gzheader(GZlibDecompressor* decompressor)
{
    ...
    decompressor->header_data->gzheader.name =
        (Bytef*)&decompressor->header_data->filename;
    /* We keep one byte to guarantee the string is 0-terminated */
    decompressor->header_data->gzheader.name_max = 256;

    if (inflateGetHeader(&decompressor->zstream,
                         &decompressor->header_data->gzheader) != Z_OK)
...
static void g_zlib_decompressor_set_property(GObject* object, guint prop_id,
                                             const GValue* value,
                                             GParamSpec* pspec)
{
    GZlibDecompressor* decompressor;
...
static void g_zlib_decompressor_get_property(GObject* object, guint prop_id,
...
GZlibDecompressor* g_zlib_decompressor_new(GZlibCompressorFormat format)
{
    ...
    decompressor =
        g_object_new(G_TYPE_ZLIB_DECOMPRESSOR, "format", format, NULL);
...
static GConverterResult g_zlib_decompressor_convert(
    ...
    *bytes_written = outbuf_size - decompressor->zstream.avail_out;
        ...
        decompressor->header_data->gzheader.done == 1)
    {
        ...
        g_file_info_set_attribute_uint32(
            data->file_info, G_FILE_ATTRIBUTE_TIME_MODIFIED_USEC, 0);

        if (data->filename[0] != '\0')
            g_file_info_set_attribute_byte_string(
                data->file_info, G_FILE_ATTRIBUTE_STANDARD_NAME,
                data->filename);
