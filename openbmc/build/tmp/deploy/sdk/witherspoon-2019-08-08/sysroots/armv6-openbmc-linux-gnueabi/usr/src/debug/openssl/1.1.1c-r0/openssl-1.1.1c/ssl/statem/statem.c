...
#define check_fatal(s, f)                                                      \
    ...
    {                                                                          \
        if (!ossl_assert((s)->statem.in_init &&                                \
                         (s)->statem.state == MSG_FLOW_ERROR))                 \
            SSLfatal(s, SSL_AD_INTERNAL_ERROR, (f), SSL_R_MISSING_FATAL);      \
    } while (0)
 ...
 * MSG_FLOW_WRITING and MSG_FLOW_READING have sub-state machines associated with
 * them.
 *
    ...
    while (1)
    {
        switch (st->read_state)
        {
            case READ_STATE_HEADER:
                ...
                {
                    /*
                     * In DTLS we get the whole message in one go - header and
                     * body
                     */
                ...
                /*
                 * Validate that we are allowed to move to the new state and
                 * move to that state if so
                 */
                if (!transition(s, mt))
                    return SUB_STATE_ERROR;
            ...
            case WRITE_STATE_PRE_WORK:
                    ...
                    !ssl_set_handshake_header(s, &pkt, mt))
                {
                    ...
                    SSLfatal(s, SSL_AD_INTERNAL_ERROR,
                             SSL_F_WRITE_STATE_MACHINE, ERR_R_INTERNAL_ERROR);
                    return SUB_STATE_ERROR;
                }
