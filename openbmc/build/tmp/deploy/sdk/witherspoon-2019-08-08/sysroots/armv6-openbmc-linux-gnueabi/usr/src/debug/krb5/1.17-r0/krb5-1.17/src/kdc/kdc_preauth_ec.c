...
static void ec_edata(krb5_context context, krb5_kdc_req* request,
...
static void ec_verify(krb5_context context, krb5_data* req_pkt,
                      ...
                      krb5_kdcpreauth_verify_respond_fn respond, void* arg)
{
    ...
    if (realmstr != NULL)
        retval = profile_get_string(
            context->profile, KRB5_CONF_REALMS, realmstr,
            KRB5_CONF_ENCRYPTED_CHALLENGE_INDICATOR, NULL, &ai);
    ...
    if (retval == 0)
    {
        for (i = 0; client_keys[i].enctype && (retval == 0); i++)
        {
            retval = krb5_c_fx_cf2_simple(
                context, armor_key, "clientchallengearmor", &client_keys[i],
                "challengelongterm", &challenge_key);
            if (retval == 0)
              ...
              krb5_kdcpreauth_moddata moddata, krb5_kdcpreauth_modreq modreq)
{
    ...
    if (retval == 0)
        retval =
            krb5_encrypt_helper(context, challenge_key,
                                KRB5_KEYUSAGE_ENC_CHALLENGE_KDC, plain, &enc);
    if (retval == 0)
...
static krb5_preauthtype ec_types[] = {KRB5_PADATA_ENCRYPTED_CHALLENGE, 0};

krb5_error_code kdcpreauth_encrypted_challenge_initvt(krb5_context context,
