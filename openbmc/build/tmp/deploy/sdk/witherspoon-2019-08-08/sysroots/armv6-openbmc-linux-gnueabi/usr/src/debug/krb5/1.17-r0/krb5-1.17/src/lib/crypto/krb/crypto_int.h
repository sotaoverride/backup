...
typedef krb5_error_code (*verify_func)(const struct krb5_cksumtypes* ctp,
                                       krb5_key key, krb5_keyusage usage,
                                       const krb5_crypto_iov* data,
                                       size_t num_data, const krb5_data* input,
                                       krb5_boolean* valid);
...
krb5_error_code krb5int_des_prf(const struct krb5_keytypes* ktp, krb5_key key,
...
krb5_error_code krb5int_derive_random(const struct krb5_enc_provider* enc,
                                      ...
                                      const krb5_data* in_constant,
                                      enum deriv_alg alg);
krb5_error_code k5_sp800_108_counter_hmac(const struct krb5_hash_provider* hash,
                                          krb5_key inkey, krb5_data* outrnd,
...
#define SIGN_IOV(_iov)                                                         \
    (ENCRYPT_IOV(_iov) || (_iov)->flags == KRB5_CRYPTO_TYPE_SIGN_ONLY)
...
krb5_boolean k5_iov_cursor_get(struct iov_cursor* cursor, unsigned char* block);
...
static inline const struct krb5_keytypes* find_enctype(krb5_enctype enctype)
{
    int i;
...
static inline const struct krb5_cksumtypes* find_cksumtype(krb5_cksumtype ctype)
{
    size_t i;
...
static inline krb5_error_code verify_key(const struct krb5_cksumtypes* ctp,
                                         krb5_key key)
{
...
static inline krb5_error_code encrypt_block(const struct krb5_enc_provider* enc,
                                            krb5_key key, krb5_data* block)
{
    krb5_crypto_iov iov;
 ...
 * iov chain. */
static inline size_t iov_total_length(const krb5_crypto_iov* data,
                                      size_t num_data, krb5_boolean signing)
{
    size_t i, total = 0;
 ...
 */
static inline size_t iov_cursor_contig_blocks(struct iov_cursor* c)
{
 ...
 * might be exhausted. */
static inline unsigned char* iov_cursor_ptr(struct iov_cursor* c)
{
 ...
 */
static inline void iov_cursor_advance(struct iov_cursor* c, size_t nblocks)
{
