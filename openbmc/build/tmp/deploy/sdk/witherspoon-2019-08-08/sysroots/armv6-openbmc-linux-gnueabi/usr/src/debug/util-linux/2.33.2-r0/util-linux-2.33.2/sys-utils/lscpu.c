...
static const char* hv_vendors[] = {[HYPER_NONE] = NULL,
                                   [HYPER_XEN] = "Xen",
                                   [HYPER_KVM] = "KVM",
...
static const int hv_vendor_pci[] = {
    [HYPER_NONE] = 0x0000, [HYPER_XEN] = 0x5853,    [HYPER_KVM] = 0x0000,
    [HYPER_MSHV] = 0x1414, [HYPER_VMWARE] = 0x15ad, [HYPER_VBOX] = 0x80ee,
};

static const int hv_graphics_pci[] = {
    [HYPER_NONE] = 0x0000, [HYPER_XEN] = 0x0001,    [HYPER_KVM] = 0x0000,
    [HYPER_MSHV] = 0x5353, [HYPER_VMWARE] = 0x0710, [HYPER_VBOX] = 0xbeef,
};

/* dispatching modes */
static const char* disp_modes[] = {[DISP_HORIZONTAL] = N_("horizontal"),
...
#define is_cpu_online(_d, _cpu)                                                \
    ((_d) && (_d)->online                                                      \
         ? CPU_ISSET_S((_cpu), CPU_ALLOC_SIZE(maxcpus), (_d)->online)          \
         : 0)
#define is_cpu_present(_d, _cpu)                                               \
    ((_d) && (_d)->present                                                     \
         ? CPU_ISSET_S((_cpu), CPU_ALLOC_SIZE(maxcpus), (_d)->present)         \
         : 0)
...
static struct lscpu_coldesc coldescs[] = {
    ...
    [COL_POLARIZATION] = {"POLARIZATION",
                          N_("CPU dispatching mode on virtual hardware")},
    [COL_ADDRESS] = {"ADDRESS", N_("physical address of a CPU")},
    [COL_CONFIGURED] = {"CONFIGURED",
                        N_("shows if the hypervisor has allocated the CPU")},
    [COL_ONLINE] = {"ONLINE",
                    N_("shows if Linux currently makes use of the CPU")},
 ...
 * line_size=<lsz> associativity=<as>"
 */
static int lookup_cache(char* line, struct lscpu_desc* desc)
{
    ...
    desc->necaches++;
    desc->ecaches =
        xrealloc(desc->ecaches, desc->necaches * sizeof(struct cpu_cache));
 ...
 */
static int init_mode(struct lscpu_modifier* mod)
{
    int m = 0;
    ...
    defined(__s390__) || defined(__sparc_v9__)
    m |= MODE_32BIT;
#endif
    return m;
}
...
static void read_basicinfo(struct lscpu_desc* desc, struct lscpu_modifier* mod)
{
    FILE* fp;
    char buf[BUFSIZ];
    ...
    while (fgets(buf, sizeof(buf), fp) != NULL)
    {
        if (lookup(buf, "vendor", &desc->vendor))
            ...
            ;
        else if (lookup(buf, "CPU implementer", &desc->vendor))
            ; /* ARM and aarch64 */
            ...
            ;
        else if (lookup(buf, "CPU part", &desc->model))
            ; /* ARM and aarch64 */
        else if (lookup(buf, "model name", &desc->modelname))
            ;
        else if (lookup(buf, "stepping", &desc->stepping))
            ;
        else if (lookup(buf, "CPU variant", &desc->stepping))
            ; /* aarch64 */
        else if (lookup(buf, "cpu MHz", &desc->mhz))
            ;
        else if (lookup(buf, "cpu MHz dynamic", &desc->dynamic_mhz))
            ; /* s390 */
        else if (lookup(buf, "cpu MHz static", &desc->static_mhz))
            ; /* s390 */
        else if (lookup(buf, "flags", &desc->flags))
            ; /* x86 */
        else if (lookup(buf, "features", &desc->flags))
            ; /* s390 */
        else if (lookup(buf, "Features", &desc->flags))
            ; /* aarch64 */
        else if (lookup(buf, "type", &desc->flags))
            ; /* sparc64 */
            ...
            ;
        else if (lookup(buf, "CPU revision", &desc->revision))
            ; /* aarch64 */
        else if (lookup(buf, "max thread id", &desc->mtid))
            ; /* s390 */
        else if (lookup(buf, "address sizes", &desc->addrsz))
            ; /* x86 */
    ...
    if (ul_path_readf_cpulist(desc->syscpu, &desc->present, maxcpus,
                              "present") == 0)
        desc->ncpus = CPU_COUNT_S(setsize, desc->present);

    /* get mask for online CPUs */
    if (ul_path_readf_cpulist(desc->syscpu, &desc->online, maxcpus, "online") ==
        0)
...
static int has_pci_device(struct lscpu_desc* desc, unsigned int vendor,
                          unsigned int device)
{
    FILE* f;
...
static void read_hypervisor_cpuid(struct lscpu_desc* desc)
{
...
static int read_hypervisor_powerpc(struct lscpu_desc* desc)
{
    assert(!desc->hyper);
    ...
    else if (ul_path_access(desc->procfs, F_OK,
                            "device-tree/ibm,partition-name") == 0 &&
             ul_path_access(desc->procfs, F_OK, "device-tree/hmc-managed?") ==
                 0 &&
             ul_path_access(desc->procfs, F_OK,
...
static UL_ASAN_BLACKLIST void vmware_bdoor(uint32_t* eax, uint32_t* ebx,
...
static void read_hypervisor(struct lscpu_desc* desc, struct lscpu_modifier* mod)
{
    FILE* fd;
    ...
    else if (ul_path_access(desc->procfs, F_OK, "xen") == 0)
    {
        ...
        if (fd)
        {
            ...
            if (fscanf(fd, "%255s", buf) == 1 && !strcmp(buf, "control_d"))
                dom0 = 1;
            fclose(fd);
        }
    ...
    else if (has_pci_device(desc, hv_vendor_pci[HYPER_XEN],
                            hv_graphics_pci[HYPER_XEN]))
    {
        desc->hyper = HYPER_XEN;
        desc->virtype = VIRT_FULL;
    }
    else if (has_pci_device(desc, hv_vendor_pci[HYPER_VMWARE],
                            hv_graphics_pci[HYPER_VMWARE]))
    {
        desc->hyper = HYPER_VMWARE;
        desc->virtype = VIRT_FULL;
    }
    else if (has_pci_device(desc, hv_vendor_pci[HYPER_VBOX],
                            hv_graphics_pci[HYPER_VBOX]))
    {
    ...
    else if (desc->vendor && (strcmp(desc->vendor, "PowerVM Lx86") == 0 ||
...
static void read_topology(struct lscpu_desc* desc, int idx)
{
    ...
    if (ul_path_accessf(desc->syscpu, F_OK, "cpu%d/topology/thread_siblings",
                        num) != 0)
        return;
    ...
    if (ul_path_readf_s32(desc->syscpu, &coreid, "cpu%d/topology/core_id",
                          num) != 0)
        coreid = -1;

    if (ul_path_readf_s32(desc->syscpu, &socketid,
                          "cpu%d/topology/physical_package_id", num) != 0)
        socketid = -1;

    if (ul_path_readf_s32(desc->syscpu, &bookid, "cpu%d/topology/book_id",
                          num) != 0)
        bookid = -1;

    if (ul_path_readf_s32(desc->syscpu, &drawerid, "cpu%d/topology/drawer_id",
                          num) != 0)
        drawerid = -1;
...
static void read_polarization(struct lscpu_desc* desc, int idx)
{
    char mode[64];
    ...
    ul_path_readf_buffer(desc->syscpu, mode, sizeof(mode), "cpu%d/polarization",
                         num);
...
static void read_address(struct lscpu_desc* desc, int idx)
{
    int num = real_cpu_num(desc, idx);
    ...
    ul_path_readf_s32(desc->syscpu, &desc->addresses[idx], "cpu%d/address",
                      num);
}

static void read_configured(struct lscpu_desc* desc, int idx)
{
    int num = real_cpu_num(desc, idx);
    ...
    ul_path_readf_s32(desc->syscpu, &desc->configured[idx], "cpu%d/configure",
                      num);
}

/* Read overall maximum frequency of cpu */
static char* cpu_max_mhz(struct lscpu_desc* desc, char* buf, size_t bufsz)
{
    int i;
    float cpu_freq = 0.0;
    size_t setsize = CPU_ALLOC_SIZE(maxcpus);
...
static char* cpu_min_mhz(struct lscpu_desc* desc, char* buf, size_t bufsz)
{
    int i;
    float cpu_freq = -1.0;
    size_t setsize = CPU_ALLOC_SIZE(maxcpus);
...
static void read_max_mhz(struct lscpu_desc* desc, int idx)
{
    int num = real_cpu_num(desc, idx);
    int mhz;

    if (ul_path_readf_s32(desc->syscpu, &mhz, "cpu%d/cpufreq/cpuinfo_max_freq",
                          num) != 0)
        return;
    if (!desc->maxmhz)
...
static void read_min_mhz(struct lscpu_desc* desc, int idx)
{
    int num = real_cpu_num(desc, idx);
    int mhz;

    if (ul_path_readf_s32(desc->syscpu, &mhz, "cpu%d/cpufreq/cpuinfo_min_freq",
                          num) != 0)
        return;
    if (!desc->minmhz)
...
static int cachecmp(const void* a, const void* b)
{
...
static void read_cache(struct lscpu_desc* desc, int idx)
{
    char buf[256];
    int i;
    int num = real_cpu_num(desc, idx);
    ...
    if (!desc->ncaches)
    {
        while (ul_path_accessf(desc->syscpu, F_OK, "cpu%d/cache/index%d", num,
                               desc->ncaches) == 0)
            desc->ncaches++;
    ...
    for (i = 0; i < desc->ncaches; i++)
    {
        ...
        if (ul_path_accessf(desc->syscpu, F_OK, "cpu%d/cache/index%d", num,
                            i) != 0)
            continue;
        if (!ca->name)
        {
            ...
            /* cache level */
            ul_path_readf_s32(desc->syscpu, &level, "cpu%d/cache/index%d/level",
                              num, i);
            if (type)
...
#endif
           strncmp(d->d_name, "node", 4) == 0 && isdigit_string(d->d_name + 4);
}
...
static void read_nodes(struct lscpu_desc* desc)
{
    int i = 0;
    DIR* dir;
    struct dirent* d;
    ...
    {
        if (is_node_dirent(d))
            desc->idx2nodenum[i++] = strtol_or_err(
                ((d->d_name) + 4), _("Failed to extract the node number"));
    }
    closedir(dir);
...
static char* get_cell_data(struct lscpu_desc* desc, int idx, int col,
                           struct lscpu_modifier* mod, char* buf, size_t bufsz)
{
    size_t setsize = CPU_ALLOC_SIZE(maxcpus);
    size_t i;
    ...
    *buf = '\0';
    ...
    switch (col)
    {
        ...
        case COL_CORE:
            ...
            {
                if (cpuset_ary_isset(cpu, desc->coremaps, desc->ncores, setsize,
                                     &i) == 0)
        ...
        case COL_SOCKET:
            ...
            {
                if (cpuset_ary_isset(cpu, desc->socketmaps, desc->nsockets,
                                     setsize, &i) == 0)
                    snprintf(buf, bufsz, "%zu", i);
            }
            break;
        case COL_NODE:
            if (cpuset_ary_isset(cpu, desc->nodemaps, desc->nnodes, setsize,
                                 &i) == 0)
        ...
        case COL_DRAWER:
            ...
            {
                if (cpuset_ary_isset(cpu, desc->drawermaps, desc->ndrawers,
                                     setsize, &i) == 0)
                    snprintf(buf, bufsz, "%zu", i);
            }
            break;
        case COL_BOOK:
            ...
            {
                if (cpuset_ary_isset(cpu, desc->bookmaps, desc->nbooks, setsize,
                                     &i) == 0)
                    snprintf(buf, bufsz, "%zu", i);
            }
            break;
        case COL_CACHE:
        {
            ...
            for (j = desc->ncaches - 1; j >= 0; j--)
            {
                ...
                if (cpuset_ary_isset(cpu, ca->sharedmaps, ca->nsharedmaps,
                                     setsize, &i) == 0)
                {
...
static char* get_cell_header(struct lscpu_desc* desc, int col,
                             struct lscpu_modifier* mod, char* buf,
                             size_t bufsz)
{
    *buf = '\0';
 ...
 *	1,1,0,0,1:1:0
 */
static void print_parsable(struct lscpu_desc* desc, int cols[], int ncols,
                           struct lscpu_modifier* mod)
{
     ...
     */
    printf(
        _("# The following is the parsable format, which can be fed to other\n"
    ...
    for (i = 0; i < ncols; i++)
    {
        ...
        if (data && *data && col != COL_CACHE && !coldescs[col].is_abbr)
        {
            /*
 ...
 * [-e] backend
 */
static void print_readable(struct lscpu_desc* desc, int cols[], int ncols,
                           struct lscpu_modifier* mod)
{
    int i;
    char buf[BUFSIZ];
...
add_summary_sprint(struct libscols_table* tb, const char* txt, const char* fmt,
                   ...)
{
...
static void print_cpuset(struct libscols_table* tb, const char* key,
                         cpu_set_t* set, int hex)
{
    size_t setsize = CPU_ALLOC_SIZE(maxcpus);
 ...
 * default output
 */
static void print_summary(struct lscpu_desc* desc, struct lscpu_modifier* mod)
{
    char buf[BUFSIZ];
    int i = 0;
    size_t setsize = CPU_ALLOC_SIZE(maxcpus);
    ...
    if (desc->nsockets)
    {
        ...
        if ((fd = ul_path_fopen(desc->procfs, "r", "sysinfo")))
        {
            ...
            {
                if (sscanf(buf, "CPU Topology SW:%d%d%d%d%d%d", &t0, &t1,
                           &drawers, &books_per_drawer, &sockets_per_book,
                           &cores_per_socket) == 6)
                    break;
            }
            if (fd)
        ...
        if (desc->nbooks)
        {
            ...
            if (desc->ndrawers)
            {
                add_summary_n(tb, _("Book(s) per drawer:"),
                              books_per_drawer
                                  ?: desc->nbooks / desc->ndrawers);
                add_summary_n(tb, _("Drawer(s):"), drawers ?: desc->ndrawers);
            }
            else
            {
                add_summary_n(tb, _("Book(s):"),
                              books_per_drawer ?: desc->nbooks);
            }
        }
        else
        {
    ...
    if (desc->maxmhz)
        add_summary_s(tb, _("CPU max MHz:"),
                      cpu_max_mhz(desc, buf, sizeof(buf)));
    if (desc->minmhz)
        add_summary_s(tb, _("CPU min MHz:"),
    ...
    if (desc->hyper)
    {
        ...
        add_summary_s(tb, _("Virtualization type:"),
                      _(virt_types[desc->virtype]));
    }
    if (desc->dispatching >= 0)
        add_summary_s(tb, _("Dispatching mode:"),
                      _(disp_modes[desc->dispatching]));
    if (desc->ncaches)
    {
        for (i = desc->ncaches - 1; i >= 0; i--)
        {
            snprintf(buf, sizeof(buf), _("%s cache:"), desc->caches[i].name);
    ...
    if (desc->necaches)
    {
        ...
        {
            snprintf(buf, sizeof(buf), _("%s cache:"), desc->ecaches[i].name);
    ...
    for (i = 0; i < desc->nnodes; i++)
    {
        snprintf(buf, sizeof(buf), _("NUMA node%d CPU(s):"),
                 desc->idx2nodenum[i]);
...
static void __attribute__((__noreturn__)) usage(void)
{
    ...
    fputs(_(" -a, --all               print both online and offline CPUs "
            "(default for -e)\n"),
          out);
    fputs(
        _(" -b, --online            print online CPUs only (default for -p)\n"),
        out);
    fputs(_(" -c, --offline           print offline CPUs only\n"), out);
    fputs(
        _(" -J, --json              use JSON for default or extended format\n"),
        out);
    fputs(_(" -e, --extended[=<list>] print out an extended readable format\n"),
          out);
    fputs(_(" -p, --parse[=<list>]    print out a parsable format\n"), out);
    fputs(
        _(" -s, --sysroot <dir>     use specified directory as system root\n"),
        out);
    fputs(_(" -x, --hex               print hexadecimal masks rather than "
            "lists of CPUs\n"),
          out);
...
int main(int argc, char* argv[])
{
    ...
    while ((c = getopt_long(argc, argv, "abce::hJp::s:xyV", longopts, NULL)) !=
           -1)
    {
            ...
            case 'e':
                if (optarg)
                {
                    if (*optarg == '=')
                        optarg++;
                    ncolumns =
                        string_to_idarray(optarg, columns, ARRAY_SIZE(columns),
                                          column_name_to_id);
                    if (ncolumns < 0)
                        return EXIT_FAILURE;
                }
    ...
    if (desc->ecaches)
        qsort(desc->ecaches, desc->necaches, sizeof(struct cpu_cache),
              cachecmp);
