...
/*
  In case you wonder why we have our own JSON implementation, here are a couple
  of reasons why this implementation has benefits over various other
  implementatins:

  - We need support for 64bit signed and unsigned integers, i.e. the full
  64,5bit range of -9223372036854775808…18446744073709551615
  - All our variants are immutable after creation
  - Special values such as true, false, zero, null, empty strings, empty array,
  empty objects require zero dynamic memory
  - Progressive parsing
  - Our integer/real type implicitly converts, but only if that's safe and
  loss-lessly possible
  - There's a "builder" for putting together objects easily in varargs function
  calls
  - There's a "dispatcher" for mapping objects to C data structures
  - Every variant optionally carries parsing location information, which
  simplifies debugging and parse log error generation
  - Formatter has color, line, column support

  Limitations:
  - Doesn't allow embedded NUL in strings
  - Can't store integers outside of the
  -9223372036854775808…18446744073709551615 range (it will use 'long double' for
    values outside this range, which is lossy)
  - Can't store negative zero (will be treated identical to positive zero, and
  not retained across serialization)
  - Can't store non-integer numbers that can't be stored in "long double"
  losslessly
  - Allows creation and parsing of objects with duplicate keys. The "dispatcher"
  will refuse them however. This means we can parse and pass around such
  objects, but will carefully refuse them when we convert them into our own
  data.

  (These limitations should be pretty much in line with those of other JSON
  implementations, in fact might be less limiting in most cases even.)
*/
...
typedef enum JsonVariantType
{
    ...
    JSON_VARIANT_REAL,
    JSON_VARIANT_NUMBER, /* This a pseudo-type: we can never create variants of
                            this type, but we use it as wildcard check for the
                            above three types */
...
typedef enum JsonFormatFlags
{
    ...
    JSON_FORMAT_COLOR_AUTO =
        1 << 3, /* insert ANSI color sequences if colors_enabled() says so */
    ...
    JSON_FORMAT_SEQ =
        1 << 6, /* prefix/suffix with RFC 7464 application/json-seq */
} JsonFormatFlags;
...
#define JSON_BUILD_ARRAY(...)                                                  \
    _JSON_BUILD_ARRAY_BEGIN, __VA_ARGS__, _JSON_BUILD_ARRAY_END
#define JSON_BUILD_OBJECT(...)                                                 \
    _JSON_BUILD_OBJECT_BEGIN, __VA_ARGS__, _JSON_BUILD_OBJECT_END
...
/* A bitmask of flags used by the dispatch logic. Note that this is a combined
 * bit mask, that is generated from the bit mask originally passed into
 * json_dispatch(), the individual bitmask associated with the static
 * JsonDispatch callout entry, as well the bitmask specified for json_log()
 * calls */
typedef enum JsonDispatchFlags
{
    /* The following three may be set in JsonDispatch's .flags field or the
       json_dispatch() flags parameter  */
    JSON_PERMISSIVE =
        1
        << 0, /* Shall parsing errors be considered fatal for this property? */
    JSON_MANDATORY =
        1 << 1,        /* Should existance of this property be mandatory? */
    JSON_LOG = 1 << 2, /* Should the parser log about errors? */

    /* The following two may be passed into log_json() in addition to the three
       above */
    JSON_DEBUG =
        1 << 3, /* Indicates that this log message is a debug message */
    JSON_WARNING =
        1 << 4, /* Indicates that this log message is a warning message */
} JsonDispatchFlags;

typedef int (*JsonDispatchCallback)(const char* name, JsonVariant* variant,
        ...
        static inline int json_dispatch_level(JsonDispatchFlags flags)
{
    ...
    /* Did the user request no logging? If so, then never log higher than
     * LOG_DEBUG. Also, if this is marked as debug message, then also log at
     * debug level. */

    if (!(flags & JSON_LOG) || (flags & JSON_DEBUG))
        return LOG_DEBUG;

    /* Are we invoked in permissive mode, or is this explicitly marked as
     * warning message? Then this should be printed at LOG_WARNING */
...
#define json_log(variant, flags, error, ...)                                   \
    ({                                                                         \
        ...
        (log_get_max_level() >= LOG_PRI(_level))                               \
            ? json_log_internal(variant, _level, _e, __FILE__, __LINE__,       \
