...
static rfbBool HandleHextile8(rfbClient* client, int rx, int ry, int rw,
                              int rh);
static rfbBool HandleHextile16(rfbClient* client, int rx, int ry, int rw,
                               int rh);
static rfbBool HandleHextile32(rfbClient* client, int rx, int ry, int rw,
                               int rh);
...
static rfbBool HandleUltraZip8(rfbClient* client, int rx, int ry, int rw,
                               int rh);
...
static rfbBool HandleTRLE24Up(rfbClient* client, int rx, int ry, int rw,
                              int rh);
...
static rfbBool HandleZRLE24Up(rfbClient* client, int rx, int ry, int rw,
                              int rh);
                                         ...
                                         rfbBool subAuth)
{
    ...
    for (loop = 0; loop < count; loop++)
    {
        ...
        if (tAuth[loop] == rfbVncAuth || tAuth[loop] == rfbNoAuth ||
            ...
            (!subAuth &&
             (tAuth[loop] == rfbTLS ||
              (tAuth[loop] == rfbVeNCrypt && client->GetCredential))))
        {
            ...
            if (flag)
            {
                 ...
                 */
                if (!WriteToRFBServer(client, (char*)&tAuth[loop], 1))
                    return FALSE;
            }
        }
    }
...
static rfbBool HandlePlainAuth(rfbClient* client)
{
    ...
    ulen =
        (cred->userCredential.username ? strlen(cred->userCredential.username)
                                       : 0);
    ulensw = rfbClientSwap32IfLE(ulen);
    plen =
        (cred->userCredential.password ? strlen(cred->userCredential.password)
                                       : 0);
...
#ifdef LIBVNCSERVER_WITH_CLIENT_GCRYPT
static rfbBool rfbMpiToBytes(const gcry_mpi_t value, uint8_t* result,
                             size_t size)
{
    gcry_error_t error;
    size_t len;
...
static rfbBool HandleARDAuth(rfbClient* client)
{
    ...
    while (1)
    {
        ...
        error = gcry_cipher_encrypt(aes, ciphertext, sizeof(ciphertext),
...
void SetClientAuthSchemes(rfbClient* client, const uint32_t* authSchemes,
                          int size)
{
    int i;
...
rfbBool InitialiseRFBConnection(rfbClient* client)
{
    ...
    if (major == 3 && (minor == 14 || minor == 16))
    {
        minor = minor - 10;
        client->minor = minor;
        rfbClientLog("UltraVNC Single Click server detected, enabling UltraVNC "
                     "specific messages\n",
                     pv);
        DefaultSupportedMessagesUltraVNC(client);
    }
    ...
    rfbClientLog("VNC server supports protocol version %d.%d (viewer %d.%d)\n",
                 major, minor, rfbProtocolMajorVersion,
    ...
    switch (authScheme)
    {
        ...
        case rfbTLS:
             ...
             * expected. Note that all following reading/writing are through the
             * TLS session from here.
             */
            ...
            switch (subAuthScheme)
            {
                ...
                default:
                    rfbClientLog("Unknown sub authentication scheme from VNC "
                                 "server: %d\n",
                                 (int)subAuthScheme);
                    return FALSE;
            }
    ...
    {
        rfbClientErr(
            "Too big desktop name length sent by server: %u B > 1 MB\n",
...
rfbBool SetFormatAndEncodings(rfbClient* client)
{
    ...
    else
    {
        ...
        else /* if (!tunnelSpecified) */
        {
           ...
           requesting level 1 compression only if tunneling is not used. */
            encs[se->nEncodings++] =
                rfbClientSwap32IfLE(rfbEncodingCompressLevel1);
        }
    ...
    if (!WriteToRFBServer(client, buf, len))
        return FALSE;
...
rfbBool TextChatOpen(rfbClient* client)
{
    ...
    return (WriteToRFBServer(client, (char*)&chat, sz_rfbTextChatMsg) ? TRUE
                                                                      : FALSE);
}
...
rfbBool HandleRFBServerMessage(rfbClient* client)
{
    ...
    switch (msg.type)
    {
        ...
        case rfbFramebufferUpdate:
        {
            ...
            for (i = 0; i < msg.fu.nRects; i++)
            {
                    ...
                    rect.encoding == rfbEncodingRichCursor)
                {
                    ...
                    if (!HandleCursorShape(client, rect.r.x, rect.r.y, rect.r.w,
                ...
                if (rect.encoding == rfbEncodingSupportedMessages)
                {
                    ...
                    /* msgs is two sets of bit flags of supported messages
                    ...
                    for (loop = 0; loop < 32; loop += 8)
                        rfbClientLog(
                            ...
                            loop, client->supportedMessages.client2server[loop],
                    ...
                    for (loop = 0; loop < 32; loop += 8)
                        rfbClientLog(
                            "%02X: %04x %04x %04x %04x - %04x %04x %04x %04x\n",
                            loop, client->supportedMessages.server2client[loop],
                ...
                if (rect.encoding == rfbEncodingSupportedEncodings)
                {
                    ...
                    /* buffer now contains rect.r.h # of uint32_t encodings that
                     * the server supports */
                    /* currently ignored by this library */
                    free(buffer);
                    continue;
                ...
                /* rfbEncodingUltraZip is a collection of subrects.   x = # of
                 * subrects, and h is always 0 */
                if (rect.encoding != rfbEncodingUltraZip)
                {
                    ...
                    /* UltraVNC with scaling, will send rectangles with a zero W
                  or H
                     *
                    if ((rect.encoding != rfbEncodingTight) &&
                        (rect.r.h * rect.r.w == 0))
                    {
                  rfbClientLog("Zero size rect - ignoring (encoding=%d (0x%08x)
                  %dx, %dy, %dw, %dh)\n", rect.encoding, rect.encoding,
                  rect.r.x, rect.r.y, rect.r.w, rect.r.h); continue;
                    }
                    */

                    /* If RichCursor encoding is used, we should prevent
                   collisions between framebuffer updates and cursor drawing
                   operations. */
                ...
                switch (rect.encoding)
                {
                    ...
                    case rfbEncodingRaw:
                    {
                        ...
                        bytesPerLine =
                            rect.r.w * client->format.bitsPerPixel / 8;
                        /* RealVNC 4.x-5.x on OSX can induce bytesPerLine==0,
                           usually during GPU accel. */
                        /* Regardless of cause, do not divide by zero. */
                        linesToRead =
                            bytesPerLine ? (RFB_BUFFER_SIZE / bytesPerLine) : 0;
        ...
        case rfbServerCutText:
        {
            ...
            {
                rfbClientErr("Ignoring too big cut text length sent by server: "
                             "%u B > 1 MB\n",
            ...
            switch (msg.tc.length)
            {
                case rfbTextChatOpen:
                    ...
                    if (client->HandleTextChat != NULL)
                        client->HandleTextChat(client, (int)rfbTextChatOpen,
                                               NULL);
                    break;
                case rfbTextChatClose:
                    rfbClientLog("Received TextChat Close\n");
                    if (client->HandleTextChat != NULL)
                        client->HandleTextChat(client, (int)rfbTextChatClose,
                                               NULL);
                    break;
                case rfbTextChatFinished:
                    rfbClientLog("Received TextChat Finished\n");
                    if (client->HandleTextChat != NULL)
                        client->HandleTextChat(client, (int)rfbTextChatFinished,
                                               NULL);
                    break;
                default:
                    ...
                    if (client->HandleTextChat != NULL)
                        client->HandleTextChat(client, (int)msg.tc.length,
                                               buffer);
                    free(buffer);
                    break;
            }
    ...
    else
    {
        ...
        if (format->trueColour)
        {
            rfbClientLog("  TRUE colour: max red %d green %d blue %d"
                         ...
                         format->redShift, format->greenShift,
                         format->blueShift);
        }
        else
        {
