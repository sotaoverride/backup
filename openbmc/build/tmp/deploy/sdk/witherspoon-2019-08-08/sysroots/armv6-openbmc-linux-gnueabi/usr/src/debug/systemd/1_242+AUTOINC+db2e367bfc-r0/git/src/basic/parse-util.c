...
int parse_mtu(int family, const char* s, uint32_t* ret)
{
    ...
    if (family == AF_INET6)
        ...
        m = IPV4_MIN_MTU; /* For all other protocols, including 'unspecified' we
                             assume the IPv4 minimal MTU */
...
int parse_size(const char* t, uint64_t base, uint64_t* size)
{
    ...
    do
    {
        ...
        tmp =
            l * table[i].factor + (unsigned long long)(frac * table[i].factor);
        if (tmp > ULLONG_MAX - r)
            return -ERANGE;
...
char* format_bytes(char* buf, size_t l, uint64_t t)
{
    ...
    static const struct
    {
        ...
        {"E", UINT64_C(1024) * UINT64_C(1024) * UINT64_C(1024) *
                  UINT64_C(1024) * UINT64_C(1024) * UINT64_C(1024)},
        {"P", UINT64_C(1024) * UINT64_C(1024) * UINT64_C(1024) *
    ...
    for (i = 0; i < ELEMENTSOF(table); i++)
    {
        ...
        if (t >= table[i].factor)
        {
            snprintf(buf, l, "%" PRIu64 ".%" PRIu64 "%s", t / table[i].factor,
