...
static unsigned __attribute__((nonnull(1))) int hash_from_tregex(void* k)
{
    ...
    // Cast to (void*) is ok here because already used in other parts of the
    // code.
...
static perthread_regex_t* create_perthread_regex(const regex_t* preg,
                                                 uncomp_regex_t* uncomp)
{
    ...
    if (Debug)
    {
        DBGPRINTF("Creating new regex_t for thread %p original regexp_t %p "
                  "(pattern: %s, cflags: %x)\n",
...
static perthread_regex_t* get_perthread_regex(const regex_t* preg)
{
    ...
    {
        uncomp_regex_t* uncomp =
            hashtable_search(regex_to_uncomp, (void*)&preg);
        ...
        if (uncomp)
        {
            ...
            if (!hashtable_insert(perthread_regexs, (void*)entry, entry))
            {
                LogError(0, RS_RET_INTERNAL_ERROR,
                         "error trying to insert thread-regexp into hash-table "
                         "- things "
                         "will not work 100%% correctly (mostly probably out "
                         "of memory issue)");
            }
        }
    }
...
static void remove_uncomp_regexp(regex_t* preg)
{
    ...
    if (uncomp)
    {
        if (Debug)
        {
            DBGPRINTF("Removing everything linked to regexp_t %p (pattern: %s, "
                      "cflags: %x)\n",
...
static void _regfree(regex_t* preg)
{
    ...
    // This can be long to iterate other all regexps, but regfree doesn't get
    // called a lot during processing.
    itr = hashtable_iterator(perthread_regexs);
    do
    {
        perthread_regex_t* entry =
            (perthread_regex_t*)hashtable_iterator_value(itr);
        ...
        {
            // This allows us to avoid freeing this while somebody is still
            // using it.
            pthread_mutex_lock(&entry->lock);
...
static int _regexec(const regex_t* preg, const char* string, size_t nmatch,
...
static size_t _regerror(int errcode, const regex_t* preg, char* errbuf,
                        size_t errbuf_size)
{
    perthread_regex_t* entry = get_perthread_regex(preg);
...
BEGINobjQueryInterface(regexp)
    CODESTARTobjQueryInterface(regexp) if (pIf->ifVersion !=
                                           regexpCURR_IF_VERSION)
...
finalize_it
    : ENDobjQueryInterface(regexp)
       ...
       */
      BEGINAbstractObjClassInit(regexp, 1,
                                OBJ_IS_LOADABLE_MODULE) /* class, version */
      ...
      if (USE_PERTHREAD_REGEX)
{
    ...
    perthread_regexs =
        create_hashtable(100, hash_from_tregex, key_equals_tregex, NULL);
...
CHKiRet(
    regexpClassInit(pModInfo)); /* must be done after tcps_sess, as we use it */
