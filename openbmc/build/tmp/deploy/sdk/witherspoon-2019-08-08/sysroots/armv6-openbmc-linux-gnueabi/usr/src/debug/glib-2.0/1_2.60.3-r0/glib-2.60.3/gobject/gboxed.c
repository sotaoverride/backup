...
static GPollFD* pollfd_copy(GPollFD* src)
{
...
void _g_boxed_type_init(void)
{
    ...
    type = g_type_register_fundamental(
        G_TYPE_BOXED, g_intern_static_string("GBoxed"), &info, &finfo,
...
G_DEFINE_BOXED_TYPE(GValueArray, g_value_array, g_value_array_copy,
                    g_value_array_free)
...
G_DEFINE_BOXED_TYPE(GHashTable, g_hash_table, g_hash_table_ref,
                    g_hash_table_unref)
...
G_DEFINE_BOXED_TYPE(GByteArray, g_byte_array, g_byte_array_ref,
                    g_byte_array_unref)
G_DEFINE_BOXED_TYPE(GBytes, g_bytes, g_bytes_ref, g_bytes_unref)

G_DEFINE_BOXED_TYPE(GRegex, g_regex, g_regex_ref, g_regex_unref)
G_DEFINE_BOXED_TYPE(GMatchInfo, g_match_info, g_match_info_ref,
                    g_match_info_unref)
...
G_DEFINE_BOXED_TYPE(GVariantBuilder, g_variant_builder, g_variant_builder_ref,
                    g_variant_builder_unref)
G_DEFINE_BOXED_TYPE(GVariantDict, g_variant_dict, g_variant_dict_ref,
...
G_DEFINE_BOXED_TYPE(GMainContext, g_main_context, g_main_context_ref,
...
G_DEFINE_BOXED_TYPE(GMarkupParseContext, g_markup_parse_context,
                    g_markup_parse_context_ref, g_markup_parse_context_unref)
...
G_DEFINE_BOXED_TYPE(GOptionGroup, g_option_group, g_option_group_ref,
...
GType g_strv_get_type(void)
{
    ...
    if (g_once_init_enter(&g_define_type_id__volatile))
    {
        GType g_define_type_id = g_boxed_type_register_static(
            g_intern_static_string("GStrv"), (GBoxedCopyFunc)g_strdupv,
...
static void boxed_proxy_value_copy(const GValue* src_value, GValue* dest_value)
{
    if (src_value->data[0].v_pointer)
        dest_value->data[0].v_pointer = _g_type_boxed_copy(
            G_VALUE_TYPE(src_value), src_value->data[0].v_pointer);
    else
...
static gchar* boxed_proxy_collect_value(GValue* value, guint n_collect_values,
                                        ...
                                        guint collect_flags)
{
    ...
    else
    {
        if (collect_flags & G_VALUE_NOCOPY_CONTENTS)
        {
            ...
            value->data[0].v_pointer = _g_type_boxed_copy(
                G_VALUE_TYPE(value), collect_values[0].v_pointer);
    }

    return NULL;
}

static gchar* boxed_proxy_lcopy_value(const GValue* value,
                                      ...
                                      guint collect_flags)
{
    ...
    if (!value->data[0].v_pointer)
        ...
        *boxed_p =
            _g_type_boxed_copy(G_VALUE_TYPE(value), value->data[0].v_pointer);
 ...
 */
GType g_boxed_type_register_static(const gchar* name, GBoxedCopyFunc boxed_copy,
...
gpointer g_boxed_copy(GType boxed_type, gconstpointer src_boxed)
{
    ...
    else
    {
        ...
        if (dest_value.data[1].v_ulong)
            g_warning("the copy_value() implementation of type '%s' seems to "
                      "make use of reserved GValue fields",
...
static inline void value_set_boxed_internal(GValue* value, gconstpointer boxed,
                                            gboolean need_copy,
                                            gboolean need_free)
{
    if (!boxed)
    ...
    value->data[0].v_pointer =
        need_copy ? g_boxed_copy(G_VALUE_TYPE(value), boxed) : (gpointer)boxed;
}
