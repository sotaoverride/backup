...
static int device_amend(sd_device* device, const char* key, const char* value)
{
    ...
    if (streq(key, "DEVPATH"))
    {
        ...
        /* the caller must verify or trust this data (e.g., if it comes from the
         * kernel) */
        r = device_set_syspath(device, path, false);
        if (r < 0)
            return log_device_debug_errno(
                device, r, "sd-device: Failed to set syspath to '%s': %m",
                path);
    }
    else if (streq(key, "SUBSYSTEM"))
    {
        ...
        if (r < 0)
            return log_device_debug_errno(
                device, r, "sd-device: Failed to set subsystem to '%s': %m",
                value);
    }
    else if (streq(key, "DEVTYPE"))
    {
        ...
        if (r < 0)
            return log_device_debug_errno(
                device, r, "sd-device: Failed to set devtype to '%s': %m",
                value);
    }
    else if (streq(key, "DEVNAME"))
    {
        ...
        if (r < 0)
            return log_device_debug_errno(
                device, r, "sd-device: Failed to set devname to '%s': %m",
                value);
    }
    else if (streq(key, "USEC_INITIALIZED"))
    {
        ...
        if (r < 0)
            return log_device_debug_errno(
                device, r, "sd-device: Failed to parse timestamp '%s': %m",
                value);
    ...
    else if (streq(key, "DRIVER"))
    {
        ...
        if (r < 0)
            return log_device_debug_errno(
                device, r, "sd-device: Failed to set driver to '%s': %m",
                value);
    }
    else if (streq(key, "IFINDEX"))
    {
        ...
        if (r < 0)
            return log_device_debug_errno(
                device, r, "sd-device: Failed to set ifindex to '%s': %m",
                value);
    }
    else if (streq(key, "DEVMODE"))
    {
        ...
        if (r < 0)
            return log_device_debug_errno(
                device, r, "sd-device: Failed to set devmode to '%s': %m",
                value);
    }
    else if (streq(key, "DEVUID"))
    {
        ...
        if (r < 0)
            return log_device_debug_errno(
                device, r, "sd-device: Failed to set devuid to '%s': %m",
                value);
    }
    else if (streq(key, "DEVGID"))
    {
        ...
        if (r < 0)
            return log_device_debug_errno(
                device, r, "sd-device: Failed to set devgid to '%s': %m",
                value);
    }
    else if (streq(key, "ACTION"))
    {
        ...
        if (r < 0)
            return log_device_debug_errno(
                device, r, "sd-device: Failed to set action to '%s': %m",
                value);
    }
    else if (streq(key, "SEQNUM"))
    {
        ...
        if (r < 0)
            return log_device_debug_errno(
                device, r, "sd-device: Failed to set SEQNUM to '%s': %m",
                value);
    }
    else if (streq(key, "DEVLINKS"))
    {
        ...
        FOREACH_WORD(word, l, value, state)
        {
            ...
            if (r < 0)
                return log_device_debug_errno(
                    device, r, "sd-device: Failed to add devlink '%s': %m",
                    devlink);
        }
    }
    ...
    else
    {
        ...
        if (r < 0)
            return log_device_debug_errno(
                device, r, "sd-device: Failed to add property '%s=%s': %m", key,
                value);
    }
...
static int device_append(sd_device* device, char* key, const char** _major,
                         const char** _minor)
{
...
static int device_verify(sd_device* device)
{
    ...
    if (!device->devpath || !device->subsystem || device->action < 0 ||
        device->seqnum == 0)
    {
        log_device_debug(device,
                         "sd-device: Device created from strv or nulstr lacks "
                         "devpath, subsystem, action or seqnum.");
        return -EINVAL;
    }
...
int device_new_from_strv(sd_device** ret, char** strv)
{
    ...
    if (major)
    {
        ...
        if (r < 0)
            return log_device_debug_errno(
                device, r, "sd-device: Failed to set devnum %s:%s: %m", major,
                minor);
    }
...
static int device_update_properties_bufs(sd_device* device)
{
    ...
    FOREACH_DEVICE_PROPERTY(device, prop, val)
    {
        ...
        buf_nulstr =
            GREEDY_REALLOC0(buf_nulstr, allocated_nulstr, nulstr_len + len + 2);
        if (!buf_nulstr)
            return -ENOMEM;
...
int device_get_properties_nulstr(sd_device* device, const uint8_t** nulstr,
                                 size_t* len)
{
    int r;
...
int device_rename(sd_device* device, const char* name)
{
    ...
    if (r >= 0)
    {
        /* like DEVPATH_OLD, INTERFACE_OLD is not saved to the db, but only
         * stays around for the current event */
...
int device_new_from_synthetic_event(sd_device** new_device, const char* syspath,
                                    const char* action)
{
    _cleanup_(sd_device_unrefp) sd_device* ret = NULL;
    int r;
...
static int device_tag(sd_device* device, const char* tag, bool add)
{
    ...
    if (add)
    {
        r = touch_file(path, true, USEC_INFINITY, UID_INVALID, GID_INVALID,
                       0444);
        if (r < 0)
            return r;
    }
...
int device_update_db(sd_device* device)
{
    ...
    log_device_debug(device, "sd-device: Created %s file '%s' for '%s'",
                     has_info ? "db" : "empty", path, device->devpath);
...
fail:
    ...
    return log_device_debug_errno(
        device, r, "sd-device: Failed to create %s file '%s' for '%s'",
        has_info ? "db" : "empty", path, device->devpath);
}
