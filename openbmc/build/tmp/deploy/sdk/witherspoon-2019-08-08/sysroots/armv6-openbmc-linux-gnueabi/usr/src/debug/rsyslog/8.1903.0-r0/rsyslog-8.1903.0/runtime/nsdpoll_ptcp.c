...
DEFobjStaticHelpers DEFobjCurrIf(glbl)
    ...
    /* add new entry to list. We assume that the fd is not already present and
     * DO NOT check this! Returns newly created entry in pEvtLst. Note that we
     * currently need to use level-triggered mode, because the upper layers do
     * not work in parallel. As such, in edge-triggered mode we may not get
     * notified, because new data comes in after we have read everything that
     * was present. To use ET mode, we need to change the upper peers so that
     * they immediately start a new wait before processing the data read. That
     * obviously requires more elaborate redesign and we postpone this until the
     * current more simplictic mode has been proven OK in practice. rgerhards,
     * 2009-11-18
     */
    static rsRetVal
    addEvent(nsdpoll_ptcp_t* pThis, int id, void* pUsr, int mode,
             nsd_ptcp_t* pSock, nsdpoll_epollevt_lst_t** pEvtLst)
{
    nsdpoll_epollevt_lst_t* pNew;
    DEFiRet;

    CHKmalloc(pNew = (nsdpoll_epollevt_lst_t*)calloc(
                  1, sizeof(nsdpoll_epollevt_lst_t)));
    pNew->id = id;
    pNew->pUsr = pUsr;
    pNew->pSock = pSock;
    pNew->event.events =
        0; /* TODO: at some time we should be able to use EPOLLET */
    if (mode & NSDPOLL_IN)
 ...
 */
static rsRetVal unlinkEvent(nsdpoll_ptcp_t* pThis, int id, void* pUsr,
                            nsdpoll_epollevt_lst_t** ppEvtLst)
{
 ...
 * rgerhards, 2009-11-23
 */
static rsRetVal delEvent(nsdpoll_epollevt_lst_t** ppEvtLst)
{
    DEFiRet;
    free(*ppEvtLst);
    *ppEvtLst = NULL;
 ...
 */
BEGINobjConstruct(
    nsdpoll_ptcp) /* be sure to specify the object type also in END macro! */
...
#endif
{
    ...
    pThis->efd = epoll_create(
        100); /* size is ignored in newer kernels, but 100 is not bad... */
}
...
finalize_it
    : ENDobjConstruct(nsdpoll_ptcp)
      ...
      BEGINobjDestruct(nsdpoll_ptcp) /* be sure to specify the object type also
                                        in END and CODESTART macros! */
    ...
    Ctl(nsdpoll_t* pNsdpoll, nsd_t* pNsd, int id, void* pUsr, int mode, int op)
{
    ...
    if (op == NSDPOLL_ADD)
    {
        ...
        if (epoll_ctl(pThis->efd, EPOLL_CTL_ADD, pSock->sock,
                      &pEventLst->event) < 0)
        {
            errSave = errno;
    ...
    else if (op == NSDPOLL_DEL)
    {
        ...
        if (epoll_ctl(pThis->efd, EPOLL_CTL_DEL, pSock->sock,
                      &pEventLst->event) < 0)
        {
            errSave = errno;
            rs_strerror_r(errSave, errStr, sizeof(errStr));
 ...
 */
static rsRetVal Wait(nsdpoll_t* pNsdpoll, int timeout, int* numEntries,
...
BEGINobjQueryInterface(nsdpoll_ptcp)
    CODESTARTobjQueryInterface(nsdpoll_ptcp) if (pIf->ifVersion !=
                                                 nsdCURR_IF_VERSION)
...
pIf->Ctl = Ctl;
pIf->Wait = Wait;
finalize_it : ENDobjQueryInterface(nsdpoll_ptcp)

              /* exit our class
               */
              BEGINObjClassExit(
                  nsdpoll_ptcp,
                  OBJ_IS_CORE_MODULE) /* CHANGE class also in END MACRO! */
              CODESTARTObjClassExit(nsdpoll_ptcp)
              /* release objects we no longer need */
              objRelease(glbl, CORE_COMPONENT);
ENDObjClassExit(nsdpoll_ptcp)

    /* Initialize the nsdpoll_ptcp class. Must be called as the very first
     * method before anything else is called inside this class. rgerhards,
     * 2008-02-19
     */
