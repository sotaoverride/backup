                 ...
                 lzma_action action lzma_attribute((__unused__)))
{
    ...
    while (*in_pos < in_size)
        switch (coder->sequence)
        {
            ...
            case SEQ_COUNT:
                ret = lzma_vli_decode(&coder->count, &coder->pos, in, in_pos,
                                      in_size);
            ...
            case SEQ_MEMUSAGE:
                ...
                ret = LZMA_OK;
                coder->sequence =
                    coder->count == 0 ? SEQ_PADDING_INIT : SEQ_UNPADDED;
                break;
            ...
            case SEQ_UNCOMPRESSED:
            {
                ...
                else
                {
                    ...
                    return_if_error(lzma_index_append(
                        coder->index, allocator, coder->unpadded_size,
                        coder->uncompressed_size));
            ...
            case SEQ_PADDING:
                ...
                coder->crc32 =
                    lzma_crc32(in + in_start, *in_pos - in_start, coder->crc32);
...
static lzma_ret index_decoder_memconfig(void* coder_ptr, uint64_t* memusage,
...
{
    // Sanity checks
    if (i == NULL || memlimit == NULL || in == NULL || in_pos == NULL ||
        *in_pos > in_size)
        return LZMA_PROG_ERROR;
    ...
    lzma_ret ret = index_decode(&coder, allocator, in, in_pos, in_size, NULL,
                                NULL, 0, LZMA_RUN);
