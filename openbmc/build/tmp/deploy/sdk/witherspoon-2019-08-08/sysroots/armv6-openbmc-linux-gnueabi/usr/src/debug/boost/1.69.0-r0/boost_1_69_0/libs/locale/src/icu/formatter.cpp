...
struct init
{
    init()
    {
        std::has_facet<icu_formatters_cache>(std::locale::classic());
    }
} instance;
} // namespace
  ...
  private:
    ...
    size_t do_parse(string_type const& str, ValueType& v) const
    {
        ...
        if (pp.getIndex() == 0 || !get_value(tmp_v, val))
            return 0;
        size_t cut =
            cvt_.cut(tmp, str.data(), str.data() + str.size(), pp.getIndex());
        if (cut == 0)
            return 0;
        v = tmp_v;
        return cut;
                                          ...
                                          icu_formatters_cache const* cache = 0)
{
    switch (c)
    {
        ...
        case 'c': // DateTile Full
        {
            ...
            return strftime_to_icu_full(
                icu::DateFormat::createDateTimeInstance(
                    icu::DateFormat::kFull, icu::DateFormat::kFull, locale),
                "yyyy-MM-dd HH:mm:ss");
        }
        ...
        case 'x': // Date
        {
            ...
            return strftime_to_icu_full(icu::DateFormat::createDateInstance(
                                            icu::DateFormat::kMedium, locale),
                                        "yyyy-MM-dd");
        }
        case 'X': // Time
        {
            ...
            return strftime_to_icu_full(icu::DateFormat::createTimeInstance(
                                            icu::DateFormat::kMedium, locale),
                                        "HH:mm:ss");
        }
        case 'y': // Year [00-99]
...
icu::UnicodeString strftime_to_icu(icu::UnicodeString const& ftime,
...
template <typename CharType>
formatter<CharType>* generate_formatter(std::ios_base& ios,
                                        icu::Locale const& locale,
                                        std::string const& encoding)
{
    ...
    icu_formatters_cache const& cache =
        std::use_facet<icu_formatters_cache>(ios.getloc());
    ...
    switch (disp)
    {
        case number:
        {
            std::ios_base::fmtflags how =
                (ios.flags() & std::ios_base::floatfield);
        ...
        {
            icu::NumberFormat* nf =
                cache.number_format(icu_formatters_cache::fmt_per);
            nf->setMaximumFractionDigits(ios.precision());
            std::ios_base::fmtflags how =
                (ios.flags() & std::ios_base::floatfield);
            if (how == std::ios_base::scientific || how == std::ios_base::fixed)
        ...
        break;
        case spellout:
            fmt.reset(new number_format<CharType>(
                cache.number_format(icu_formatters_cache::fmt_spell),
                encoding));
            break;
        case ordinal:
            fmt.reset(new number_format<CharType>(
                cache.number_format(icu_formatters_cache::fmt_ord), encoding));
            break;
        case date:
        case time:
        case datetime:
        case strftime:
        {
                ...
                switch (disp)
                {
                    ...
                    case strftime:
                    {
                            ...
                            !cache.date_time_format_[1][1].isEmpty())
                        {
                            ...
                            std::basic_string<CharType> const& f =
                                info.date_time_pattern<CharType>();
                            pattern = strftime_to_icu(
                                cvt_.icu(f.c_str(), f.c_str() + f.size()),
                                locale);
                        }
                    }
                    break;
            ...
            if (!df)
            {
                ...
                {
                    case time_short:
                        tstyle = icu::DateFormat::kShort;
                        break;
                    case time_medium:
                        tstyle = icu::DateFormat::kMedium;
                        break;
                    case time_long:
                        tstyle = icu::DateFormat::kLong;
                        break;
                    case time_full:
                        tstyle = icu::DateFormat::kFull;
                        break;
                }
                switch (info.date_flags())
                {
                    case date_short:
                        dstyle = icu::DateFormat::kShort;
                        break;
                    case date_medium:
                        dstyle = icu::DateFormat::kMedium;
                        break;
                    case date_long:
                        dstyle = icu::DateFormat::kLong;
                        break;
                    case date_full:
                        dstyle = icu::DateFormat::kFull;
                        break;
                }

                if (disp == date)
                    adf.reset(
                        icu::DateFormat::createDateInstance(dstyle, locale));
                else if (disp == time)
                    adf.reset(
                        icu::DateFormat::createTimeInstance(tstyle, locale));
                else if (disp == datetime)
                    adf.reset(icu::DateFormat::createDateTimeInstance(
                        dstyle, tstyle, locale));
                else
                { // strftime
                    icu_std_converter<CharType> cvt_(encoding);
                    std::basic_string<CharType> const& f =
                        info.date_time_pattern<CharType>();
                    icu::UnicodeString fmt = strftime_to_icu(
                        cvt_.icu(f.data(), f.data() + f.size()), locale);
            ...
            if (adf.get())
                fmt.reset(
                    new date_format<CharType>(adf.release(), true, encoding));
            else
...
template <>
formatter<char>* formatter<char>::create(std::ios_base& ios,
                                         icu::Locale const& l,
                                         std::string const& e)
{
...
template <>
formatter<wchar_t>* formatter<wchar_t>::create(std::ios_base& ios,
                                               icu::Locale const& l,
                                               std::string const& e)
{
...
template <>
formatter<char16_t>* formatter<char16_t>::create(std::ios_base& ios,
                                                 icu::Locale const& l,
                                                 std::string const& e)
{
...
template <>
formatter<char32_t>* formatter<char32_t>::create(std::ios_base& ios,
                                                 icu::Locale const& l,
                                                 std::string const& e)
{
