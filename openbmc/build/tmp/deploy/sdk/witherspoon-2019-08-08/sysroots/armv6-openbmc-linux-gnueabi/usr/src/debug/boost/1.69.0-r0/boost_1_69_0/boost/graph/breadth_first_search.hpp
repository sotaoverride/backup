...
void breadth_first_visit(const IncidenceGraph& g, SourceIterator sources_begin,
                         SourceIterator sources_end, Buffer& Q, BFSVisitor vis,
                         ColorMap color)
{
...
#ifdef BOOST_GRAPH_USE_MPI
template <class DistributedGraph, class ColorMap, class BFSVisitor, class P,
          class T, class R>
...
struct bfs_dispatch
{
              ...
              const bgl_named_params<P, T, R>& params, ColorMap color)
    {
        bfs_helper(
            ...
            params,
            boost::mpl::bool_<boost::is_base_and_derived<
                distributed_graph_tag, typename graph_traits<VertexListGraph>::
    ...
    const bgl_named_params<P, T, R>& params)
{
    ...
    typedef
        typename get_param_type<vertex_color_t, bgl_named_params<P, T, R>>::type
            C;
...
struct breadth_first_search_impl
{
    ...
    void operator()(const Graph& g, const Source& source,
                    const ArgPack& arg_pack)
    {
        using namespace boost::graph::keywords;
        typename boost::graph_traits<Graph>::vertex_descriptor sources[1] = {
            source};
        boost::queue<typename boost::graph_traits<Graph>::vertex_descriptor> Q;
        boost::breadth_first_search(
            g, &sources[0], &sources[1],
