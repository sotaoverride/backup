...
static LookupData* lookup_data_new(const char* hostname, int address_family)
{
...
static void lookup_data_free(LookupData* data)
{
    g_free(data->hostname);
    g_free(data);
}

static void do_lookup_by_name(GTask* task, gpointer source_object,
                              gpointer task_data, GCancellable* cancellable)
{
...
#endif
    /* socktype and protocol don't actually matter, they just get copied into
     * the returned addrinfo structures (and then we ignore them). But if we
     * leave them unset, we'll get back duplicate answers.
     */
    ...
    if (retval == 0)
    {
        ...
        {
            sockaddr =
                g_socket_address_new_from_native(ai->ai_addr, ai->ai_addrlen);
            if (!sockaddr)
                continue;
        ...
        else
        {
            ...
            g_task_return_new_error(task, G_RESOLVER_ERROR,
                                    G_RESOLVER_ERROR_NOT_FOUND,
                                    _("Error resolving “%s”: %s"), hostname,
...
static GList* lookup_by_name(GResolver* resolver, const gchar* hostname,
...
static int flags_to_family(GResolverNameLookupFlags flags)
{
    ...
    if (flags & G_RESOLVER_NAME_LOOKUP_FLAGS_IPV6_ONLY)
    {
        ...
        g_return_val_if_fail(!(flags & G_RESOLVER_NAME_LOOKUP_FLAGS_IPV4_ONLY),
                             address_family);
    }
...
static GList* lookup_by_name_with_flags(GResolver* resolver,
...
static void lookup_by_name_with_flags_async(
    GResolver* resolver, const gchar* hostname, GResolverNameLookupFlags flags,
...
static void lookup_by_name_async(GResolver* resolver, const gchar* hostname,
                                 GCancellable* cancellable,
...
static GList* lookup_by_name_finish(GResolver* resolver, GAsyncResult* result,
                                    GError** error)
{
...
static GList* lookup_by_name_with_flags_finish(GResolver* resolver,
                                               GAsyncResult* result,
                                               GError** error)
{
    g_return_val_if_fail(g_task_is_valid(result, resolver), NULL);
...
static void do_lookup_by_address(GTask* task, gpointer source_object,
                                 gpointer task_data, GCancellable* cancellable)
{
    ...
    retval = getnameinfo((struct sockaddr*)&sockaddr, sockaddr_size, name,
...
static gchar* lookup_by_address(GResolver* resolver, GInetAddress* address,
                                GCancellable* cancellable, GError** error)
{
    GTask* task;
    gchar* name;
...
static void lookup_by_address_async(GResolver* resolver, GInetAddress* address,
                                    GCancellable* cancellable,
                                    GAsyncReadyCallback callback,
                                    gpointer user_data)
{
...
static gchar* lookup_by_address_finish(GResolver* resolver,
...
static GVariant* parse_res_srv(guchar* answer, guchar* end, guchar** p)
{
    gchar namebuf[1024];
    ...
    *p += dn_expand(answer, end, *p, namebuf, sizeof(namebuf));
    ...
    return g_variant_new("(qqqs)", priority, weight, port, namebuf);
}

static GVariant* parse_res_soa(guchar* answer, guchar* end, guchar** p)
{
    gchar mnamebuf[1024];
    gchar rnamebuf[1024];
    ...
    *p += dn_expand(answer, end, *p, rnamebuf, sizeof(rnamebuf));
    ...
    return g_variant_new("(ssuuuuu)", mnamebuf, rnamebuf, serial, refresh,
                         retry, expire, ttl);
}

static GVariant* parse_res_ns(guchar* answer, guchar* end, guchar** p)
{
    gchar namebuf[1024];
...
static GVariant* parse_res_mx(guchar* answer, guchar* end, guchar** p)
{
    gchar namebuf[1024];
    guint16 preference;
    ...
    *p += dn_expand(answer, end, *p, namebuf, sizeof(namebuf));
    ...
    return g_variant_new("(qs)", preference, namebuf);
}

static GVariant* parse_res_txt(guchar* answer, guchar* end, guchar** p)
{
    GVariant* record;
    GPtrArray* array;
    guchar* at = *p;
    ...
    *p = at;
    record = g_variant_new(
        "(@as)", g_variant_new_strv((const gchar**)array->pdata, array->len));
    g_ptr_array_free(array, TRUE);
    return record;
}

static gint g_resolver_record_type_to_rrtype(GResolverRecordType type)
{
    switch (type)
    {
...
static GList* g_resolver_records_from_res_query(const gchar* rrname,
                                                gint rrtype, guchar* answer,
                                                gint len, gint herr,
                                                GError** error)
{
    gint count;
...
static GVariant* parse_dns_srv(DNS_RECORD* rec)
{
    return g_variant_new("(qqqs)", (guint16)rec->Data.SRV.wPriority,
                         (guint16)rec->Data.SRV.wWeight,
                         (guint16)rec->Data.SRV.wPort,
                         rec->Data.SRV.pNameTarget);
}

static GVariant* parse_dns_soa(DNS_RECORD* rec)
{
    return g_variant_new(
        "(ssuuuuu)", rec->Data.SOA.pNamePrimaryServer,
        rec->Data.SOA.pNameAdministrator, (guint32)rec->Data.SOA.dwSerialNo,
        (guint32)rec->Data.SOA.dwRefresh, (guint32)rec->Data.SOA.dwRetry,
        (guint32)rec->Data.SOA.dwExpire, (guint32)rec->Data.SOA.dwDefaultTtl);
}

static GVariant* parse_dns_ns(DNS_RECORD* rec)
{
    return g_variant_new("(s)", rec->Data.NS.pNameHost);
}

static GVariant* parse_dns_mx(DNS_RECORD* rec)
{
    return g_variant_new("(qs)", (guint16)rec->Data.MX.wPreference,
                         rec->Data.MX.pNameExchange);
}

static GVariant* parse_dns_txt(DNS_RECORD* rec)
{
    GVariant* record;
    GPtrArray* array;
    DWORD i;
...
static WORD g_resolver_record_type_to_dnstype(GResolverRecordType type)
{
    switch (type)
    {
        case G_RESOLVER_RECORD_SRV:
...
static GList* g_resolver_records_from_DnsQuery(const gchar* rrname,
                                               WORD dnstype, DNS_STATUS status,
                                               DNS_RECORD* results,
                                               GError** error)
{
    DNS_RECORD* rec;
...
static void free_lookup_records_data(LookupRecordsData* lrd)
{
    g_free(lrd->rrname);
    g_slice_free(LookupRecordsData, lrd);
}

static void free_records(GList* records)
{
...
static void do_lookup_records(GTask* task, gpointer source_object,
                              gpointer task_data, GCancellable* cancellable)
{
     ...
     *
     * What we have currently is not particularly worse than using res_query()
     * in worker threads, since it would transparently call res_init() for each
     * new worker thread. (Although the workers would get reused by the
     * #GThreadPool.)
     *
    ...
    for (;;)
    {
        ...
        len = res_nquery(&res, lrd->rrname, C_IN, rrtype, answer->data,
                         answer->len);
#else
    ...
    herr = h_errno;
    records = g_resolver_records_from_res_query(
        lrd->rrname, rrtype, answer->data, len, herr, &error);
...
#error                                                                         \
    "Your platform has res_ninit() but not res_nclose() or res_ndestroy(). Please file a bug at https://gitlab.gnome.org/GNOME/glib/issues/new"
#endif
    ...
    status = DnsQuery_A(lrd->rrname, dnstype, DNS_QUERY_STANDARD, NULL,
                        &results, NULL);
    records = g_resolver_records_from_DnsQuery(lrd->rrname, dnstype, status,
                                               results, &error);
    if (results != NULL)
...
static GList* lookup_records(GResolver* resolver, const gchar* rrname,
                             GResolverRecordType record_type,
                             GCancellable* cancellable, GError** error)
{
    GTask* task;
...
static void lookup_records_async(GResolver* resolver, const char* rrname,
                                 GResolverRecordType record_type,
                                 GCancellable* cancellable,
                                 GAsyncReadyCallback callback,
                                 gpointer user_data)
...
static GList* lookup_records_finish(GResolver* resolver, GAsyncResult* result,
                                    GError** error)
{
    g_return_val_if_fail(g_task_is_valid(result, resolver), NULL);
    ...
    g_threaded_resolver_class_init(GThreadedResolverClass* threaded_class)
{
    ...
    resolver_class->lookup_by_name_with_flags_async =
        lookup_by_name_with_flags_async;
    resolver_class->lookup_by_name_with_flags_finish =
