...
static int mkfilter_group_byname(const char* name, char* buffer, size_t buflen)
{
    ...
    /* build filter */
    return mysnprintf(buffer, buflen, "(&%s(%s=%s))", group_filter,
...
static int mkfilter_group_bygid(gid_t gid, char* buffer, size_t buflen)
{
    ...
    if (gidSid != NULL)
    {
        ...
        if ((gid >= min_builtin_rid) && (gid <= max_builtin_rid))
            return mysnprintf(
                buffer, buflen, "(&%s(%s=%s\\%02x\\%02x\\%02x\\%02x))",
                group_filter, attmap_group_gidNumber, builtinSid,
                (int)(gid & 0xff), (int)((gid >> 8) & 0xff),
                (int)((gid >> 16) & 0xff), (int)((gid >> 24) & 0xff));
        return mysnprintf(buffer, buflen,
                          "(&%s(%s=%s\\%02x\\%02x\\%02x\\%02x))", group_filter,
                          attmap_group_gidNumber, gidSid, (int)(gid & 0xff),
                          (int)((gid >> 8) & 0xff), (int)((gid >> 16) & 0xff),
                          (int)((gid >> 24) & 0xff));
    }
    else
    {
        return mysnprintf(buffer, buflen, "(&%s(%s=%lu))", group_filter,
                          attmap_group_gidNumber, (unsigned long int)gid);
    }
}

/* create a search filter for searching a group entry
   by member uid, return -1 on errors */
static int mkfilter_group_bymember(MYLDAP_SESSION* session, const char* uid,
                                   char* buffer, size_t buflen)
{
    ...
    return mysnprintf(buffer, buflen, "(&%s(|(%s=%s)(%s=%s)))", group_filter,
                      attmap_group_memberUid, safeuid, attmap_group_member,
                      safedn);
}

static int mkfilter_group_bymemberdn(const char* dn, char* buffer,
                                     size_t buflen)
{
    char safedn[BUFLEN_SAFEDN];
    /* escape DN */
    ...
    {
        log_log(LOG_ERR,
                "mkfilter_group_bymemberdn(): safedn buffer too small");
        return -1;
    }
    return mysnprintf(buffer, buflen, "(&%s(%s=%s))", group_filter,
...
static int do_write_group(TFILE* fp, MYLDAP_ENTRY* entry, const char** names,
                          gid_t gids[], int numgids, const char* passwd,
                          const char** members, const char* reqname)
{
                       ...
                       SET* members, SET* seen, SET* subgroups)
{
    ...
    derefs =
        myldap_get_deref_values(entry, attmap_group_member, attmap_passwd_uid);
    if (derefs != NULL)
    {
    ...
    if (values != NULL)
        for (i = 0; values[i] != NULL; i++)
        {
            if ((seen == NULL) || (!set_contains(seen, values[i])))
            {
                ...
                /* transform the DN into a uid (dn2uid() already checks
                 * validity) */
                       ...
                       MYLDAP_SESSION* session)
{
    ...
    else
    {
        ...
        for (numgids = 0;
             (numgids < MAXGIDS_PER_ENTRY) && (gidvalues[numgids] != NULL);
             numgids++)
        {
            if (gidSid != NULL)
    ...
    passwd = get_userpassword(entry, attmap_group_userPassword, passbuffer,
                              sizeof(passbuffer));
    if (passwd == NULL)
    ...
    if (wantmembers)
    {
        ...
        if (set != NULL)
        {
            ...
            if (subgroups != NULL)
            {
                ...
                {
                    search = myldap_search(session, tmp, LDAP_SCOPE_BASE,
                                           group_filter, group_attrs, NULL);
                    if (search != NULL)
                        while ((entry2 = myldap_get_entry(search, NULL)) !=
                               NULL)
...
NSLCD_HANDLE(
    group, byname, NSLCD_ACTION_GROUP_BYNAME, char name[BUFLEN_NAME];
    char filter[BUFLEN_FILTER]; READ_STRING(fp, name);
...
NSLCD_HANDLE(group, bygid, NSLCD_ACTION_GROUP_BYGID, gid_t gid;
             char filter[BUFLEN_FILTER]; READ_INT32(fp, gid);
...
int nslcd_group_bymember(TFILE* fp, MYLDAP_SESSION* session)
{
    ...
    if ((nslcd_cfg->nss_nested_groups) &&
        (strcasecmp(attmap_group_member, "\"\"") != 0))
    {
        seen = set_new();
        tocheck = set_new();
    ...
    for (i = 0; (base = group_bases[i]) != NULL; i++)
    {
        ...
        {
            if ((seen == NULL) ||
                (!set_contains(seen, dn = myldap_get_dn(entry))))
            {
                if (seen != NULL)
                {
                    set_add(seen, dn);
    ...
    if (tocheck != NULL)
    {
        while ((dn = set_pop(tocheck)) != NULL)
        {
            ...
            {
                log_log(LOG_WARNING,
                        "nslcd_group_bymember(): filter buffer too small");
                free((void*)dn);
                set_free(seen);
                set_free(tocheck);
                return -1;
...
NSLCD_HANDLE(group, all, NSLCD_ACTION_GROUP_ALL, const char* filter;
