...
struct DnsResourceRecord
{
    ...
    /* How many labels to strip to determine "signer" of the RRSIG (aka, the
     * zone). -1 if not signed. */
    unsigned n_skip_labels_signer;
    /* How many labels to strip to determine "synthesizing source" of this RR,
     * i.e. the wildcard's immediate parent. -1 if not signed. */
...
static inline uint8_t
    DNS_RESOURCE_RECORD_OPT_VERSION_SUPPORTED(const DnsResourceRecord* rr)
{
    assert(rr);
...
/* _DNS_{CLASS,TYPE}_STRING_MAX include one byte for NUL, which we use for space
 * instead below.
 * DNS_HOSTNAME_MAX does not include the NUL byte, so we need to add 1. */
#define DNS_RESOURCE_KEY_STRING_MAX                                            \
    (_DNS_CLASS_STRING_MAX + _DNS_TYPE_STRING_MAX + DNS_HOSTNAME_MAX + 1)
...
void dns_resource_record_hash_func(const DnsResourceRecord* i,
