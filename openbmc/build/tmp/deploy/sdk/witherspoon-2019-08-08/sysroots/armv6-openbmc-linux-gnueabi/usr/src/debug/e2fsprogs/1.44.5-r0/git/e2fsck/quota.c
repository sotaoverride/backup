                                  ...
                                  ext2_ino_t to_ino, enum quota_type qtype)
{
    ...
    if (retval)
    {
        com_err("ext2fs_read_bitmaps", retval, "%s", _("in move_quota_inode"));
        return retval;
    }
    ...
    if (retval)
    {
        com_err("ext2fs_read_inode", retval, "%s", _("in move_quota_inode"));
        return retval;
    }
    ...
    if (retval)
    {
        com_err("ext2fs_unlink", retval, "%s", _("in move_quota_inode"));
        return retval;
    }
...
void e2fsck_hide_quota(e2fsck_t ctx)
{
    ...
    if ((ctx->options & E2F_OPT_READONLY) || !ext2fs_has_feature_quota(sb))
        return;
