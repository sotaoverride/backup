...
#define rsa_encrypt_key(_in, _inlen, _out, _outlen, _lparam, _lparamlen,       \
    ...
    rsa_encrypt_key_ex(_in, _inlen, _out, _outlen, _lparam, _lparamlen, _prng, \
                       _prng_idx, _hash_idx, LTC_PKCS_1_OAEP, _key)
...
int rsa_set_key(const unsigned char* N, unsigned long Nlen,
                ...
                const unsigned char* d, unsigned long dlen, rsa_key* key);
int rsa_set_factors(const unsigned char* p, unsigned long plen,
                    const unsigned char* q, unsigned long qlen, rsa_key* key);
...
int dh_set_pg(const unsigned char* p, unsigned long plen,
              const unsigned char* g, unsigned long glen, dh_key* key);
int dh_set_pg_dhparam(const unsigned char* dhparam, unsigned long dhparamlen,
                      dh_key* key);
int dh_set_pg_groupsize(int groupsize, dh_key* key);

int dh_set_key(const unsigned char* in, unsigned long inlen, int type,
               dh_key* key);
...
typedef struct
{
    ...
    /** pointer to domain parameters; either points to NIST curves (identified
     * by idx >= 0) or user supplied curve */
...
int dsa_set_pqg(const unsigned char* p, unsigned long plen,
                ...
                const unsigned char* g, unsigned long glen, dsa_key* key);
int dsa_set_pqg_dsaparam(const unsigned char* dsaparam,
                         unsigned long dsaparamlen, dsa_key* key);
int dsa_generate_pqg(prng_state* prng, int wprng, int group_size,
                     int modulus_size, dsa_key* key);

int dsa_set_key(const unsigned char* in, unsigned long inlen, int type,
                dsa_key* key);
...
/* UTF-8 */
     ...
     defined(__WCHAR_MAX__) || defined(_WCHAR_T) ||                            \
     defined(_WCHAR_T_DEFINED) || defined(__WCHAR_TYPE__)) &&                  \
...
int der_encode_generalizedtime(ltc_generalizedtime* gtime, unsigned char* out,
                               unsigned long* outlen);
