...
redo:
    ...
    do
    {
        if (!BN_priv_rand(blind, BN_num_bits(dsa->q) - 1, BN_RAND_TOP_ANY,
                          BN_RAND_BOTTOM_ANY))
            goto err;
                          ...
                          const unsigned char* dgst, int dlen)
{
    ...
    do
    {
        if (dgst != NULL)
        {
             ...
             */
            if (!BN_generate_dsa_nonce(k, dsa->q, dsa->priv_key, dgst, dlen,
                                       ctx))
                goto err;
        }
    ...
    {
        if (!BN_MONT_CTX_set_locked(&dsa->method_mont_p, dsa->lock, dsa->p,
                                    ctx))
            goto err;
    }
     ...
     */
    if (!BN_add(l, k, dsa->q) || !BN_add(k, l, dsa->q))
        goto err;
                         ...
                         DSA* dsa)
{
    ...
    {
        mont =
            BN_MONT_CTX_set_locked(&dsa->method_mont_p, dsa->lock, dsa->p, ctx);
        if (!mont)
            goto err;
    }
