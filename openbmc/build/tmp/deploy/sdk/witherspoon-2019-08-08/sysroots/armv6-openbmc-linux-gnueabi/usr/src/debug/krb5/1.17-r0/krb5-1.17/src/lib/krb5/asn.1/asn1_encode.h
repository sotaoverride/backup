...
#define DEFFNTYPE(DESCNAME, CTYPENAME, ENCFN, DECFN, CHECKFN, FREEFN)          \
    ...
    static const struct fn_info aux_info_##DESCNAME = {ENCFN, DECFN, CHECKFN,  \
                                                       FREEFN};                \
...
#define DEFBOOLTYPE(DESCNAME, CTYPENAME)                                       \
    typedef CTYPENAME aux_type_##DESCNAME;                                     \
    const struct atype_info k5_atype_##DESCNAME = {atype_bool,                 \
                                                   sizeof(CTYPENAME), NULL}
/* Integer types.  */
#define DEFINTTYPE(DESCNAME, CTYPENAME)                                        \
    typedef CTYPENAME aux_type_##DESCNAME;                                     \
    const struct atype_info k5_atype_##DESCNAME = {atype_int,                  \
                                                   sizeof(CTYPENAME), NULL}
#define DEFUINTTYPE(DESCNAME, CTYPENAME)                                       \
    typedef CTYPENAME aux_type_##DESCNAME;                                     \
    const struct atype_info k5_atype_##DESCNAME = {atype_uint,                 \
                                                   sizeof(CTYPENAME), NULL}
#define DEFINT_IMMEDIATE(DESCNAME, VAL, ERR)                                   \
    typedef int aux_type_##DESCNAME;                                           \
    static const struct immediate_info aux_info_##DESCNAME = {VAL, ERR};       \
    const struct atype_info k5_atype_##DESCNAME = {atype_int_immediate, 0,     \
                                                   &aux_info_##DESCNAME}
...
#define DEFPTRTYPE(DESCNAME, BASEDESCNAME)                                     \
    ...
    const struct atype_info k5_atype_##DESCNAME = {                            \
        atype_ptr, sizeof(aux_type_##DESCNAME), &aux_info_##DESCNAME}
#else
#define DEFPTRTYPE(DESCNAME, BASEDESCNAME)                                     \
    typedef aux_type_##BASEDESCNAME* aux_type_##DESCNAME;                      \
    static void* aux_loadptr_##DESCNAME(const void* p)                         \
    {                                                                          \
        return *(aux_type_##DESCNAME*)p;                                       \
    }                                                                          \
    static void aux_storeptr_##DESCNAME(void* ptr, void* val)                  \
    {                                                                          \
...
#define DEFOFFSETTYPE(DESCNAME, STYPE, FIELDNAME, BASEDESC)                    \
    ...
    static const struct offset_info aux_info_##DESCNAME = {                    \
        OFFOF(STYPE, FIELDNAME, aux_type_##BASEDESC), &k5_atype_##BASEDESC};   \
    const struct atype_info k5_atype_##DESCNAME = {                            \
        atype_offset, sizeof(aux_type_##DESCNAME), &aux_info_##DESCNAME}
#define DEFCOUNTEDTYPE_base(DESCNAME, STYPE, DATAFIELD, COUNTFIELD, SIGNED,    \
    ...
    const struct counted_info aux_info_##DESCNAME = {                          \
        ...
        OFFOF(STYPE, COUNTFIELD, aux_counttype_##CDESC), SIGNED,               \
        sizeof(((STYPE*)0)->COUNTFIELD), &k5_cntype_##CDESC};                  \
    const struct atype_info k5_atype_##DESCNAME = {                            \
        atype_counted, sizeof(STYPE), &aux_info_##DESCNAME}
...
#define DEFOPTIONALTYPE(DESCNAME, PRESENT, INIT, BASEDESC)                     \
    ...
    const struct atype_info k5_atype_##DESCNAME = {                            \
        atype_optional, sizeof(aux_type_##DESCNAME), &aux_info_##DESCNAME}
...
#define DEFOPTIONALZEROTYPE(DESCNAME, BASEDESC)                                \
    static int aux_present_##DESCNAME(const void* p)                           \
    {                                                                          \
...
#define DEFNONEMPTYNULLTERMSEQOFTYPE(DESCNAME, BASEDESCNAME)                   \
    typedef aux_type_##BASEDESCNAME aux_type_##DESCNAME;                       \
    const struct atype_info k5_atype_##DESCNAME = {                            \
        atype_nonempty_nullterm_sequence_of, sizeof(aux_type_##DESCNAME),      \
        &k5_atype_##BASEDESCNAME}
...
#define DEFTAGGEDTYPE(DESCNAME, CLASS, CONSTRUCTION, TAG, IMPLICIT, BASEDESC)  \
    ...
    const struct atype_info k5_atype_##DESCNAME = {                            \
        atype_tagged_thing, sizeof(aux_type_##DESCNAME), &aux_info_##DESCNAME}
...
#define DEFCOUNTEDSTRINGTYPE(DESCNAME, DTYPE, LTYPE, ENCFN, DECFN, TAGVAL)     \
    ...
    static const struct string_info aux_info_##DESCNAME = {ENCFN, DECFN,       \
                                                           TAGVAL};            \
    const struct cntype_info k5_cntype_##DESCNAME = {cntype_string,            \
                                                     &aux_info_##DESCNAME}

#define DEFCOUNTEDDERTYPE(DESCNAME, DTYPE, LTYPE)                              \
    typedef DTYPE aux_ptrtype_##DESCNAME;                                      \
    typedef LTYPE aux_counttype_##DESCNAME;                                    \
    const struct cntype_info k5_cntype_##DESCNAME = {cntype_der, NULL}

#define DEFCOUNTEDSEQOFTYPE(DESCNAME, LTYPE, BASEDESC)                         \
    typedef aux_type_##BASEDESC aux_ptrtype_##DESCNAME;                        \
    typedef LTYPE aux_counttype_##DESCNAME;                                    \
    const struct cntype_info k5_cntype_##DESCNAME = {cntype_seqof,             \
                                                     &k5_atype_##BASEDESC}
...
#define DEFCHOICETYPE(DESCNAME, UTYPE, DTYPE, FIELDS)                          \
    ...
    static const struct choice_info aux_info_##DESCNAME = {                    \
        FIELDS, sizeof(FIELDS) / sizeof(FIELDS[0])};                           \
    const struct cntype_info k5_cntype_##DESCNAME = {cntype_choice,            \
                                                     &aux_info_##DESCNAME}
 ...
 * allocated krb5_data. */
extern krb5_error_code k5_asn1_full_encode(const void* rep,
                                           const struct atype_info* a,
                                           krb5_data** code_out);
krb5_error_code k5_asn1_full_decode(const krb5_data* code,
                                    const struct atype_info* a, void** rep_out);

#define MAKE_ENCODER(FNAME, DESC)                                              \
    krb5_error_code FNAME(const aux_type_##DESC* rep, krb5_data** code_out)    \
    {                                                                          \
...
#define MAKE_DECODER(FNAME, DESC)                                              \
    krb5_error_code FNAME(const krb5_data* code, aux_type_##DESC** rep_out)    \
    {                                                                          \
        krb5_error_code ret;                                                   \
        void* rep;                                                             \
        *rep_out = NULL;                                                       \
 ...
 */
#define WARN_IF_TYPE_MISMATCH(LVALUE, TYPE) (sizeof(0 ? (TYPE*)0 : &(LVALUE)))
