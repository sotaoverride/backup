...
int addRoute(u_long dstip, u_long gwip, u_long iff, u_short flags)
{
    ...
    !(defined(irix6) || defined(__OpenBSD__) || defined(darwin))
...
struct rtent* findCacheRTE(u_long dst)
{
    ...
    for (i = 0; i < MAX_CACHE; i++)
    {
        ...
        if (rtcache[i].in_use && (rtcache[i].rt_dst == dst))
        { /* valid & match? */
            return (&rtcache[i]);
        }
    }
 ...
 * If statP is NULL and ap is non-NULL, the instance exists, but not this
 * variable. If statP is NULL and ap is NULL, then neither this instance nor the
 * variable exists.
 */

int write_rte(int action, u_char* var_val, u_char var_val_type,
    ...
    switch (var)
    {
        ...
        case IPROUTETYPE:
            ...
            if (action == RESERVE1)
            {
                ...
                if ((val < 2) || (val > 4))
                { /* only accept invalid, direct, indirect */
            ...
            else if (action == COMMIT)
            {
                ...
                else
                {
                    ...
                    if (addRoute(rp->rt_dst, rp->rt_nextIR, rp->rt_ifix,
                                 flags) < 0)
                    {
              ...
              size_t var_val_len, u_char* statP, oid* name, size_t length)
{
    ...
    /*
     * #define for ipRouteTable entries are 1 less than corresponding sub-id in
     * MIB
    ...
    switch (action)
    {
        case RESERVE1:
            switch (var)
            {
                ...
                default:
                    DEBUGMSGTL(
                        ("snmpd", "unknown sub-id %d in write_rte\n", var + 1));
        ...
        case ACTION: /* Perform the SET action (if reversible) */
            switch (var)
            {
                ...
                case IPROUTEAGE:
                     ...
                     * * As row will be updated and this field gives the number
                     * of
        ...
        case COMMIT:
             ...
             * for asked entries.
             * * Thats why it is enough if we call
             * SetIpForwardEntry/CreateIpForwardEntry only once
             * * SetIpForwardENtry is not done in ACTION phase, as that will
             * reset ipRouteAge on success
             * * and if any varbind fails, then we can't UNDO the change for
             * ipROuteAge.
             */
            if (route_row)
            {
                if (!create_flag)
                {
                    ...
                    {
                        snmp_log(LOG_ERR, "Can't set route table's row with "
                                          "specified value\n");
                    ...
                    else
                    {
                         ...
                         * creates new row. *If Set succeeds, then delete the
                         * old row.
                         * * Don't know yet whether SET on ipRouteIfIndex
                         * creates new row.
                         * * If it creates then index_flag should be added to
                        ...
                        if (dest_flag || nexthop_flag || mask_flag)
                        {
                            ...
                            {
                                snmp_log(LOG_ERR,
                                         "Set on ipRouteTable created new row, "
                                         "but failed to delete the old row\n");
                ...
                if (create_flag)
                {
                    ...
                    {
                        if ((status = CreateIpForwardEntry(route_row)) !=
                            NO_ERROR)
                        {
                            snmp_log(LOG_ERR,
                                     "Inside COMMIT: CreateIpNetEntry failed, "
                                     "status %lu\n",
                                     status);
                            retval = SNMP_ERR_COMMITFAILED;
                        }
                    ...
                    else
                    {
                         ...
                         * be supplied
                         */
                        snmp_log(LOG_ERR, "case COMMIT, can't create without "
                                          "index, mask, nextHop and metric\n");
