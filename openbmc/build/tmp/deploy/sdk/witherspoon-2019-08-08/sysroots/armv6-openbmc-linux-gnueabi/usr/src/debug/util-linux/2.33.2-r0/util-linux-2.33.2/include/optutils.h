 ...
 * 16, last is zero).
 *
 * The current status of options is stored in excl_st array. The size of the
 *array must be the same as number of the groups in the ul_excl_t array.
 *
 * If you're unsure then see sys-utils/mount.c or misc-utils/findmnt.c.
 */
#define UL_EXCL_STATUS_INIT                                                    \
    {                                                                          \
        0                                                                      \
    }
typedef int ul_excl_t[16];

static inline void err_exclusive_options(int c, const struct option* opts,
                                         const ul_excl_t* excl, int* status)
{
    int e;
    ...
    for (e = 0; excl[e][0] && excl[e][0] <= c; e++)
    {
        ...
        for (; *op && *op <= c; op++)
        {
            ...
            else if (status[e] != c)
            {
                ...
                for (op = excl[e]; ct + 1 < ARRAY_SIZE(excl[0]) && *op;
                     op++, ct++)
                {
