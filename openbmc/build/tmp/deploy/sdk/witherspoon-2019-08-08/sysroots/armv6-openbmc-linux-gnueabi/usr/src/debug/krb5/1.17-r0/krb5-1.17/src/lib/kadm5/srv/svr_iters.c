...
static void get_either_iter(struct iter_data* data, char* name)
{
    ...
    if (match)
    {
        if (data->n_names == data->sz_names)
        {
            ...
            char** new_names = realloc(data->names, new_sz * sizeof(char*));
            if (new_names)
            {
...
static kadm5_ret_t kadm5_get_either(int princ, void* server_handle, char* exp,
                                    char*** princs, int* count)
{
    ...
    *count = 0;
    ...
    {
        ret = krb5_db_iter_policy(handle->context, exp, get_pols_iter,
                                  (void*)&data);
    }
