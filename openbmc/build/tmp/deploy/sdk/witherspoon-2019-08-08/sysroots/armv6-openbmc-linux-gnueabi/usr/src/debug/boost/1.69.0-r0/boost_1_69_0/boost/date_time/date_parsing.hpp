 ...
 */
inline std::string convert_to_lower(std::string inp)
{
#if !defined(BOOST_DATE_TIME_NO_LOCALE)
...
inline unsigned short month_str_to_ushort(std::string const& s)
{
    ...
    else
    {
        ...
        typename month_type::month_map_ptr_type ptr =
            month_type::get_month_map_ptr();
        typename month_type::month_map_type::iterator iter = ptr->find(str);
        if (iter != ptr->end())
        { // required for STLport
            return iter->second;
        }
    }
...
date_type parse_date(const std::string& s, int order_spec = ymd_order_iso)
{
    ...
    typedef boost::tokenizer<
        char_separator_type, std::basic_string<char>::const_iterator,
...
date_type parse_undelimited_date(const std::string& s)
{
    ...
    for (typename tokenizer_type::iterator ti = tok.begin(); ti != tok.end();
         ++ti)
    {
...
inline date_type from_stream_type(iterator_type& beg, iterator_type const& end,
                                  char)
{
...
// This function cannot be compiled with MSVC 6.0 due to internal compiler
// shorcomings
#else
    ...
    from_simple_string_type(const std::basic_string<charT>& s)
{
    ...
    typedef typename boost::tokenizer<
        char_separator, typename std::basic_string<charT>::const_iterator,
    ...
    typename std::basic_string<charT>::iterator date_string_start =
                                                    date_string.begin(),
                                                date_string_end =
                                                    date_string.end();
    typedef typename std::iterator_traits<
        typename std::basic_string<charT>::iterator>::value_type value_type;
    date_type d1 = from_stream_type<date_type>(date_string_start,
                                               date_string_end, value_type());
    date_string = *(++tok_it); // next token
    date_string_start = date_string.begin(),
    date_string_end = date_string.end();
    date_type d2 = from_stream_type<date_type>(date_string_start,
                                               date_string_end, value_type());
