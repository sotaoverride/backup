...
tt_face_load_colr(TT_Face face, FT_Stream stream)
{
    FT_Error error;
...
static FT_Bool find_base_glyph_record(FT_Byte* base_glyph_begin,
                                      FT_Int num_base_glyph, FT_UInt glyph_id,
...
tt_face_get_colr_layer(TT_Face face, FT_UInt base_glyph, FT_UInt* aglyph_index,
...
FT_LOCAL_DEF(FT_Error)
tt_face_colr_blend_layer(TT_Face face, FT_UInt color_index,
                         FT_GlyphSlot dstSlot, FT_GlyphSlot srcSlot)
{
    FT_Error error;
    ...
    else
    {
            ...
            y_max != dstSlot->bitmap_top)
        {
            ...
            q = buf + (int)pitch * (y_max - dstSlot->bitmap_top) +
