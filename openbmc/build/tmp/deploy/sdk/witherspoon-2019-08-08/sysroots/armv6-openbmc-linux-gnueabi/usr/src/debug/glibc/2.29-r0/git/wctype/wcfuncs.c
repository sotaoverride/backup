...
#undef iswalnum
func(alnum, __ISwalnum) libc_hidden_def(__iswalnum) libc_hidden_weak(iswalnum)
#undef iswalpha
    func(alpha, __ISwalpha) libc_hidden_weak(iswalpha)
#undef iswblank
        func(blank, __ISwblank)
#undef iswcntrl
            func(cntrl, __ISwcntrl)
#undef iswdigit
                func(digit, __ISwdigit) libc_hidden_weak(iswdigit)
#undef iswlower
                    func(lower, __ISwlower) libc_hidden_def(
                        __iswlower) libc_hidden_weak(iswlower)
#undef iswgraph
...
libc_hidden_def(__towlower) weak_alias(__towlower, towlower)
    libc_hidden_weak(towlower)

#undef towupper
        wint_t __towupper(wint_t wc)
{
    size_t i = _NL_CURRENT_WORD(LC_CTYPE, _NL_CTYPE_MAP_OFFSET) + __TOW_toupper;
    const char* desc = _NL_CURRENT(LC_CTYPE, i);
    return wctrans_table_lookup(desc, wc);
}
libc_hidden_def(__towupper) weak_alias(__towupper, towupper)
