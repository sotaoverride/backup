...
static int do_foreach_af(struct rtnl_link* link,
                         int (*cb)(struct rtnl_link*, struct rtnl_link_af_ops*,
                                   void*, void*),
                         void* arg)
{
    int i, err;
...
struct nla_policy rtln_link_policy[IFLA_MAX + 1] = {
    ...
    [IFLA_STATS] = {.minlen =
                        _nl_offsetofend(struct rtnl_link_stats, tx_compressed)},
    [IFLA_STATS64] = {.minlen = _nl_offsetofend(struct rtnl_link_stats64,
                                                tx_compressed)},
...
int rtnl_link_info_parse(struct rtnl_link* link, struct nlattr** tb)
{
    ...
    if (tb[IFLA_STATS])
    {
        ...
        if (nla_len(tb[IFLA_STATS]) >=
            _nl_offsetofend(struct rtnl_link_stats, rx_nohandler))
    ...
    if (tb[IFLA_ADDRESS])
    {
        ...
        if (link->l_addr == NULL)
            return -NLE_NOMEM;
        nl_addr_set_family(link->l_addr, nl_addr_guess_family(link->l_addr));
        link->ce_mask |= LINK_ATTR_ADDR;
    }

    if (tb[IFLA_BROADCAST])
    {
        link->l_bcast = nl_addr_alloc_attr(tb[IFLA_BROADCAST], AF_UNSPEC);
        if (link->l_bcast == NULL)
            return -NLE_NOMEM;
        nl_addr_set_family(link->l_bcast, nl_addr_guess_family(link->l_bcast));
    ...
    if (tb[IFLA_MAP])
    {
        nla_memcpy(&link->l_map, tb[IFLA_MAP], sizeof(struct rtnl_link_ifmap));
                           ...
                           struct nlmsghdr* n, struct nl_parser_param* pp)
{
    ...
    link->ce_mask = (LINK_ATTR_IFNAME | LINK_ATTR_FAMILY | LINK_ATTR_ARPTYPE |
    ...
    if (tb[IFLA_LINKINFO])
    {
        ...
        if (li[IFLA_INFO_KIND])
        {
            ...
            if ((af = nl_str2af(kind)) >= 0 && !af_ops &&
                (af_ops = af_lookup_and_alloc(link, af)))
            {
    ...
    if (tb[IFLA_AF_SPEC])
    {
        ...
        {
            err = af_ops_family->ao_parse_af_full(
                link, tb[IFLA_AF_SPEC],
...
static int link_request_update(struct nl_cache* cache, struct nl_sock* sk)
{
    ...
    if (ext_filter_mask)
    {
        err = nla_put(msg, IFLA_EXT_MASK, sizeof(ext_filter_mask),
                      &ext_filter_mask);
        if (err)
            goto nla_put_failure;
    }
...
static void link_dump_details(struct nl_object* obj, struct nl_dump_params* p)
{
    ...
    if (link->ce_mask & LINK_ATTR_BRD)
        nl_dump(p, "brd %s ", nl_addr2str(link->l_bcast, buf, sizeof(buf)));
...
static void link_dump_stats(struct nl_object* obj, struct nl_dump_params* p)
{
    ...
    strcpy(fmt, "     RX %X.2f %s %10" PRIu64 " %10" PRIu64 " %10" PRIu64
                " %10" PRIu64 " %10" PRIu64 "\n");
    fmt[9] = *unit == 'B' ? '9' : '7';

    nl_dump_line(p, fmt, res, unit, link->l_stats[RTNL_LINK_RX_PACKETS],
    ...
    strcpy(fmt, "     TX %X.2f %s %10" PRIu64 " %10" PRIu64 " %10" PRIu64
                " %10" PRIu64 " %10" PRIu64 "\n");
    fmt[9] = *unit == 'B' ? '9' : '7';

    nl_dump_line(p, fmt, res, unit, link->l_stats[RTNL_LINK_TX_PACKETS],
    ...
    nl_dump_line(p,
                 "     RX  %10" PRIu64 " %10" PRIu64 " %10" PRIu64 " %10" PRIu64
                 " %10" PRIu64 " %10" PRIu64 "\n",
    ...
    nl_dump_line(p,
                 "     TX  %10" PRIu64 " %10" PRIu64 " %10" PRIu64 " %10" PRIu64
                 " %10" PRIu64 "\n",
                        ...
                        uint32_t table_sz)
{
    ...
    *hashkey = nl_hash(&lkey, lkey_sz, 0) % table_sz;
    ...
    NL_DBG(5, "link %p key (dev %d fam %d) keysz %d, hash 0x%x\n", link,
           lkey.l_index, lkey.l_family, lkey_sz, *hashkey);
...
#define LINK_DIFF(ATTR, EXPR) ATTR_DIFF(attrs, LINK_ATTR_##ATTR, a, b, EXPR)
    ...
    if (flags & LOOSE_COMPARISON)
        diff |= LINK_DIFF(FLAGS, (a->l_flags ^ b->l_flags) & b->l_flag_mask);
    else
...
{
    return __flags2str(attrs, buf, len, link_attrs, ARRAY_SIZE(link_attrs));
}
 ...
 * @return 0 on success or a negative error code.
 */
int rtnl_link_alloc_cache(struct nl_sock* sk, int family,
...
static int build_link_msg(int cmd, struct ifinfomsg* hdr,
                          struct rtnl_link* link, int flags,
                          struct nl_msg** result)
{
    struct nl_msg* msg;
                                   ...
                                   struct nl_msg** result)
{
    ...
    if (orig->ce_mask & LINK_ATTR_IFINDEX && orig->ce_mask & LINK_ATTR_IFNAME &&
...
{
    return __flags2str(flags, buf, len, link_flags, ARRAY_SIZE(link_flags));
}
...
{
    return __str2type(name, link_operstates, ARRAY_SIZE(link_operstates));
}
...
{
    return __type2str(st, buf, len, carrier_states, ARRAY_SIZE(carrier_states));
}
...
/**
 * @deprecated The weight attribute is unused and obsoleted in all recent
 * kernels
 */
