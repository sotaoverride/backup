...
static void host_name_resolver_callback(
    AvahiHostNameResolver* r, AVAHI_GCC_UNUSED AvahiIfIndex interface,
...
static void address_resolver_callback(
    AvahiAddressResolver* r, AVAHI_GCC_UNUSED AvahiIfIndex interface,
    AVAHI_GCC_UNUSED AvahiProtocol protocol, AvahiResolverEvent event,
    const AvahiAddress* a, const char* name,
    AVAHI_GCC_UNUSED AvahiLookupResultFlags flags,
    AVAHI_GCC_UNUSED void* userdata)
{
...
static void help(FILE* f, const char* argv0)
{
    fprintf(f,
            ...
            argv0, strstr(argv0, "host-name") ? "[-n]" : "-n", argv0,
                              ...
                              char* argv[])
{
    ...
    c->command = strstr(argv0, "address")
                     ? COMMAND_RESOLVE_ADDRESS
                     : (strstr(argv0, "host-name") ? COMMAND_RESOLVE_HOST_NAME
                                                   : COMMAND_UNSPEC);
    ...
    if (c->command == COMMAND_RESOLVE_ADDRESS ||
        c->command == COMMAND_RESOLVE_HOST_NAME)
    {
        if (optind >= argc)
        {
            ...
            for (i = optind; i < argc; i++)
            {
                ...
                if (config.command == COMMAND_RESOLVE_HOST_NAME)
                {
                    ...
                    if (!(avahi_host_name_resolver_new(
                            ...
                            argv[i], config.proto, 0,
                            host_name_resolver_callback, NULL)))
                    {
                        fprintf(stderr,
                                _("Failed to create host name resolver: %s\n"),
                                avahi_strerror(avahi_client_errno(client)));
                        goto fail;
                    }
                }
                else
                {
                    ...
                    if (!(avahi_address_resolver_new(
                            client, AVAHI_IF_UNSPEC, AVAHI_PROTO_UNSPEC, &a, 0,
                            address_resolver_callback, NULL)))
                    {
                        fprintf(stderr,
                                _("Failed to create address resolver: %s\n"),
                                avahi_strerror(avahi_client_errno(client)));
                        goto fail;
                    }
                }
