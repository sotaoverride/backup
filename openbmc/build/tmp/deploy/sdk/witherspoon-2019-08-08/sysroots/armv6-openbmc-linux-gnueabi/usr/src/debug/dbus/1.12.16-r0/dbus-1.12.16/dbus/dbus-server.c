 ...
 * @todo Need notification to apps of disconnection, may matter for some
 * transports
 */
...
void _dbus_server_trace_ref(DBusServer* server, int old_refcount,
 ...
 */
static char* copy_address_with_guid_appended(const DBusString* address,
                                             const DBusString* guid_hex)
{
    ...
    if (!_dbus_string_copy(address, 0, &with_guid,
        ...
        !_dbus_string_copy(guid_hex, 0, &with_guid,
                           _dbus_string_get_length(&with_guid)))
    {
...
dbus_bool_t _dbus_server_init_base(DBusServer* server,
                                   ...
                                   const DBusString* address, DBusError* error)
{
    ...
    server->address =
        copy_address_with_guid_appended(address, &server->guid_hex);
...
static dbus_bool_t protected_change_watch(
    DBusServer* server, DBusWatch* watch, DBusWatchAddFunction add_function,
...
{
    HAVE_LOCK_CHECK(server);
    return protected_change_watch(server, watch, _dbus_watch_list_add_watch,
                                  NULL, NULL, FALSE);
}
...
{
    HAVE_LOCK_CHECK(server);
    protected_change_watch(server, watch, NULL, _dbus_watch_list_remove_watch,
                           NULL, FALSE);
}
...
static dbus_bool_t
    protected_change_timeout(DBusServer* server, DBusTimeout* timeout,
                             ...
                             dbus_bool_t enabled)
{
    ...
    /* This isn't really safe or reasonable; a better pattern is the "do
     * everything, then drop lock and call out" one; but it has to be propagated
     * up through all callers
     */
...
dbus_bool_t _dbus_server_add_timeout(DBusServer* server, DBusTimeout* timeout)
{
    return protected_change_timeout(
        server, timeout, _dbus_timeout_list_add_timeout, NULL, NULL, FALSE);
}
...
{
    protected_change_timeout(server, timeout, NULL,
 ...
 * @param enabled whether to enable or disable
 */
void _dbus_server_toggle_timeout(DBusServer* server, DBusTimeout* timeout,
                                 dbus_bool_t enabled)
{
    protected_change_timeout(server, timeout, NULL, NULL,
...
static const struct
{
    DBusServerListenResult (*func)(DBusAddressEntry* entry,
                                   DBusServer** server_p, DBusError* error);
} listen_funcs[] = {{_dbus_server_listen_socket},
...
out:
    ...
    if (!handled_once)
    {
        ...
        if (len > 0)
            ...
            dbus_set_error(error, DBUS_ERROR_BAD_ADDRESS, "Empty address '%s'",
                           address);
    }
    ...
    if (server == NULL)
    {
        ...
        else
        {
            ...
            _dbus_assert(error == NULL ||
                         dbus_error_is_set(&first_connect_error));
            dbus_move_error(&first_connect_error, error);
        }

        _DBUS_ASSERT_ERROR_IS_CLEAR(
            &first_connect_error); /* be sure we freed it */
...
void dbus_server_set_new_connection_function(
    DBusServer* server, DBusNewConnectionFunction function, void* data,
    ...
    DBusFreeFunction free_data_function)
{
    ...
    if (watches)
    {
        ...
        result = _dbus_watch_list_set_functions(
            watches, add_function, remove_function, toggled_function, data,
            free_data_function);
        SERVER_LOCK(server);
    }
    else
 ...
 * @param toggled_function function to notify when the timeout is
 * enabled/disabled
 * @param data data to pass to add_function and remove_function.
 * @param free_data_function function to be called to free the data.
 * @returns #FALSE on failure (no memory)
...
dbus_bool_t dbus_server_set_timeout_functions(
    DBusServer* server, DBusAddTimeoutFunction add_function,
    ...
    DBusFreeFunction free_data_function)
{
    ...
    if (timeouts)
    {
        ...
        result = _dbus_timeout_list_set_functions(
            timeouts, add_function, remove_function, toggled_function, data,
            free_data_function);
        SERVER_LOCK(server);
    }
    else
 ...
 */
dbus_bool_t dbus_server_set_data(DBusServer* server, int slot, void* data,
                                 DBusFreeFunction free_data_func)
{
    ...
    retval = _dbus_data_slot_list_set(&slot_allocator, &server->slot_list, slot,
                                      data, free_data_func, &old_free_func,
                                      &old_data);
...
dbus_bool_t _dbus_server_test(void)
{
    ...
    for (i = 0; i < _DBUS_N_ELEMENTS(valid_addresses); i++)
    {
        ...
        if (server == NULL)
        {
            _dbus_warn("server listen error: %s: %s", error.name,
                       error.message);
