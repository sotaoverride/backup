...
static void print_status_info(const StatusInfo* i)
{
    ...
    if (setenv("TZ", isempty(i->timezone) ? "UTC" : i->timezone, true) < 0)
        log_warning_errno(
            errno, "Failed to set TZ environment variable, ignoring: %m");
    else
        tzset();
    ...
    else if (IN_SET(arg_transport, BUS_TRANSPORT_LOCAL, BUS_TRANSPORT_MACHINE))
    {
        ...
        log_warning("Could not get time from timedated and not operating "
                    "locally, ignoring.");

    if (have_time)
    {
        n = strftime(a, sizeof a, "%a %Y-%m-%d %H:%M:%S %Z",
                     localtime_r(&sec, &tm));
        printf("               Local time: %s\n", n > 0 ? a : "n/a");

        n = strftime(a, sizeof a, "%a %Y-%m-%d %H:%M:%S UTC",
                     gmtime_r(&sec, &tm));
    ...
    if (i->rtc_time > 0)
    {
        ...
        n = strftime(a, sizeof a, "%a %Y-%m-%d %H:%M:%S",
                     gmtime_r(&rtc_sec, &tm));
        printf("                 RTC time: %s\n", n > 0 ? a : "n/a");
    }
    else
...
static int show_status(int argc, char** argv, void* userdata)
{
    ...
    r = bus_map_all_properties(bus, "org.freedesktop.timedate1",
                               "/org/freedesktop/timedate1", map,
                               BUS_MAP_BOOLEAN_AS_BOOL, &error, &m, &info);
    if (r < 0)
        return log_error_errno(r, "Failed to query server: %s",
...
static int show_properties(int argc, char** argv, void* userdata)
{
    ...
    r = bus_print_all_properties(bus, "org.freedesktop.timedate1",
                                 "/org/freedesktop/timedate1", NULL,
                                 arg_property, arg_value, arg_all, NULL);
    if (r < 0)
...
static int set_time(int argc, char** argv, void* userdata)
{
    ...
    if (r < 0)
        return log_error_errno(r, "Failed to parse time specification '%s': %m",
                               argv[1]);

    r = sd_bus_call_method(bus, "org.freedesktop.timedate1",
                           "/org/freedesktop/timedate1",
                           "org.freedesktop.timedate1", "SetTime", &error, NULL,
                           "xbb", (int64_t)t, relative, interactive);
    if (r < 0)
        return log_error_errno(r, "Failed to set time: %s",
                               bus_error_message(&error, r));
...
static int set_timezone(int argc, char** argv, void* userdata)
{
    ...
    r = sd_bus_call_method(bus, "org.freedesktop.timedate1",
                           "/org/freedesktop/timedate1",
                           "org.freedesktop.timedate1", "SetTimezone", &error,
                           NULL, "sb", argv[1], arg_ask_password);
    if (r < 0)
        return log_error_errno(r, "Failed to set time zone: %s",
                               bus_error_message(&error, r));
...
static int set_local_rtc(int argc, char** argv, void* userdata)
{
    ...
    if (b < 0)
        return log_error_errno(b, "Failed to parse local RTC setting '%s': %m",
                               argv[1]);

    r = sd_bus_call_method(
        bus, "org.freedesktop.timedate1", "/org/freedesktop/timedate1",
        "org.freedesktop.timedate1", "SetLocalRTC", &error, NULL, "bbb", b,
        arg_adjust_system_clock, arg_ask_password);
    if (r < 0)
        return log_error_errno(r, "Failed to set local RTC: %s",
                               bus_error_message(&error, r));
...
static int set_ntp(int argc, char** argv, void* userdata)
{
    ...
    b = parse_boolean(argv[1]);
    if (b < 0)
        return log_error_errno(b, "Failed to parse NTP setting '%s': %m",
                               argv[1]);

    r = sd_bus_call_method(bus, "org.freedesktop.timedate1",
                           "/org/freedesktop/timedate1",
                           "org.freedesktop.timedate1", "SetNTP", &error, NULL,
                           "bb", b, arg_ask_password);
    if (r < 0)
        return log_error_errno(r, "Failed to set ntp: %s",
                               bus_error_message(&error, r));
...
static int list_timezones(int argc, char** argv, void* userdata)
{
    ...
    r = sd_bus_call_method(
        bus, "org.freedesktop.timedate1", "/org/freedesktop/timedate1",
        "org.freedesktop.timedate1", "ListTimezones", &error, &reply, NULL);
    if (r < 0)
...
{
    char ts[FORMAT_TIMESPAN_MAX], tmin[FORMAT_TIMESPAN_MAX],
        tmax[FORMAT_TIMESPAN_MAX];
    ...
    if (i->dest < i->origin || i->trans < i->recv ||
        i->dest - i->origin < i->trans - i->recv)
    {
    ...
    printf("         Leap: %s\n"
           "      Version: %" PRIu32 "\n"
           "      Stratum: %" PRIu32 "\n",
           ntp_leap_to_string(i->leap), i->version, i->stratum);
    if (i->stratum <= 1)
        printf("    Reference: %s\n", i->reference.str);
    else
    ...
    printf("    Precision: %s (%" PRIi32 ")\n",
           format_timespan(
               ts, sizeof(ts),
               DIV_ROUND_UP((nsec_t)(exp2(i->precision) * NSEC_PER_SEC),
                            NSEC_PER_USEC),
               0),
           i->precision);
...
static int map_server_address(sd_bus* bus, const char* member,
...
static int map_ntp_message(sd_bus* bus, const char* member, sd_bus_message* m,
                           sd_bus_error* error, void* userdata)
{
    ...
    r = sd_bus_message_read(m, "uuuuitt", &p->leap, &p->version, &p->mode,
                            &p->stratum, &p->precision, &p->root_delay,
                            &p->root_dispersion);
    if (r < 0)
        return r;
    ...
    r = sd_bus_message_read(m, "ttttbtt", &p->origin, &p->recv, &p->trans,
                            &p->dest, &b, &p->packet_count, &p->jitter);
    if (r < 0)
        return r;
...
static int show_timesync_status_once(sd_bus* bus)
{
    static const struct bus_properties_map map_timesync[] = {
        ...
        {"ServerAddress", "(iay)", map_server_address,
         offsetof(NTPStatusInfo, server_address)},
        ...
        {"RootDistanceMaxUSec", "t", NULL,
         offsetof(NTPStatusInfo, root_distance_max)},
    ...
    r = bus_map_all_properties(bus, "org.freedesktop.timesync1",
                               "/org/freedesktop/timesync1", map_timesync,
                               BUS_MAP_BOOLEAN_AS_BOOL, &error, &m, &info);
    if (r < 0)
        return log_error_errno(r, "Failed to query server: %s",
                               bus_error_message(&error, r));
...
static int show_timesync_status(int argc, char** argv, void* userdata)
{
    ...
    r = sd_bus_match_signal(bus, NULL, "org.freedesktop.timesync1",
...
static int print_timesync_property(const char* name, const char* expected_value,
    ...
    switch (type)
    {
        ...
        case SD_BUS_TYPE_STRUCT:
            if (streq(name, "NTPMessage"))
            {
                ...
                printf(
                    "{ Leap=%u, Version=%u, Mode=%u, Stratum=%u, Precision=%i,",
                ...
                printf(" Ignored=%s PacketCount=%" PRIu64 ",", yes_no(i.spike),
                       i.packet_count);
                printf(" Jitter=%s }\n",
                       format_timespan(ts, sizeof(ts), i.jitter, 0));
...
static int show_timesync(int argc, char** argv, void* userdata)
{
    ...
    r = bus_print_all_properties(
        bus, "org.freedesktop.timesync1", "/org/freedesktop/timesync1",
        print_timesync_property, arg_property, arg_value, arg_all, NULL);
    if (r < 0)
        return bus_log_parse_error(r);
