...
#define CLOSURE_N_NOTIFIERS(cl)                                                \
    (CLOSURE_N_MFUNCS(cl) + (cl)->n_fnotifiers + (cl)->n_inotifiers)
...
#define CHANGE_FIELD(_closure, _field, _OP, _value, _must_set, _SET_OLD,       \
                     _SET_NEW)                                                 \
    G_STMT_START                                                               \
    {                                                                          \
        ...
        {                                                                      \
            ...
            new_int = tmp.vint;                                                \
            success = g_atomic_int_compare_and_exchange(&cunion->vint,         \
                                                        old_int, new_int);     \
...
GClosure* g_closure_new_simple(guint sizeof_closure, gpointer data)
{
    ...
    if (RUNNING_ON_VALGRIND)
    {
        ...
        *(gpointer*)(allocated + private_size + sizeof_closure) =
            allocated + sizeof(gpointer);

        VALGRIND_MALLOCLIKE_BLOCK(allocated + private_size,
                                  sizeof_closure + sizeof(gpointer), 0, TRUE);
        VALGRIND_MALLOCLIKE_BLOCK(allocated + sizeof(gpointer),
                                  private_size - sizeof(gpointer), 0, TRUE);
    }
    else
#endif
...
static inline void closure_invoke_notifiers(GClosure* closure,
                                            guint notify_type)
{
    /* notifier layout:
     ...
     * CLOSURE_N_NOTIFIERS(cl) = CLOSURE_N_MFUNCS(cl) + n_fnotifiers +
     * n_inotifiers
     *
     * constrains/catches:
     * - closure->notifiers may be reloacted during callback
     * - closure->n_fnotifiers and closure->n_inotifiers may change during
     * callback
     ...
     * ->marshal= & ->data=)
     * - must distinguish (->marshal= & ->data=) for INOTIFY vs. FNOTIFY (via
     * ->in_inotify)
    ...
    switch (notify_type)
    {
        ...
        case INOTIFY:
            ...
            while (closure->n_inotifiers)
            {
                ...
                ndata = closure->notifiers + CLOSURE_N_MFUNCS(closure) +
                        closure->n_fnotifiers + n;
                closure->marshal = (GClosureMarshal)ndata->notify;
                closure->data = ndata->data;
                ndata->notify(ndata->data, closure);
            }
...
static void g_closure_set_meta_va_marshal(GClosure* closure,
 ...
 */
void g_closure_set_meta_marshal(GClosure* closure, gpointer marshal_data,
 ...
 */
void g_closure_add_marshal_guards(GClosure* closure, gpointer pre_marshal_data,
                                  ...
                                  GClosureNotify post_marshal_notify)
{
    ...
    closure->notifiers = g_renew(GClosureNotifyData, closure->notifiers,
                                 CLOSURE_N_NOTIFIERS(closure) + 2);
    if (closure->n_inotifiers)
        closure->notifiers[(CLOSURE_N_MFUNCS(closure) + closure->n_fnotifiers +
    ...
    if (closure->n_fnotifiers)
        closure->notifiers[(CLOSURE_N_MFUNCS(closure) + closure->n_fnotifiers +
                            1)] =
            closure->notifiers[CLOSURE_N_MFUNCS(closure) + 0];
    if (closure->n_fnotifiers > 1)
        closure
            ->notifiers[(CLOSURE_N_MFUNCS(closure) + closure->n_fnotifiers)] =
            closure->notifiers[CLOSURE_N_MFUNCS(closure) + 1];
    if (closure->n_guards)
        closure->notifiers[(closure->n_guards + closure->n_guards + 1)] =
 ...
 */
void g_closure_add_finalize_notifier(GClosure* closure, gpointer notify_data,
 ...
 * unspecified order.
 */
void g_closure_add_invalidate_notifier(GClosure* closure, gpointer notify_data,
                                       GClosureNotify notify_func)
{
    guint i;
...
static inline gboolean closure_try_remove_inotify(GClosure* closure,
...
static inline gboolean closure_try_remove_fnotify(GClosure* closure,
                                                  gpointer notify_data,
                                                  GClosureNotify notify_func)
{
    GClosureNotifyData *ndata, *nlast;
                                          ...
                                          GClosureNotify notify_func)
{
    ...
    if (closure->is_invalid &&
        closure->in_inotify && /* account removal of notify_func() while it's
                                  called */
 ...
 * Notice that notifiers are automatically removed after they are run.
 */
void g_closure_remove_finalize_notifier(GClosure* closure, gpointer notify_data,
                                        GClosureNotify notify_func)
{
    g_return_if_fail(closure != NULL);
    g_return_if_fail(notify_func != NULL);
 ...
 */
void g_closure_invoke(GClosure* closure, GValue /*out*/* return_value,
                      ...
                      gpointer invocation_hint)
{
    ...
    if (!closure->is_invalid)
    {
        ...
        marshal(closure, return_value, n_param_values, param_values,
...
void _g_closure_invoke_va(GClosure* closure, GValue /*out*/* return_value,
                          gpointer instance, va_list args, int n_params,
                          GType* param_types)
{
    GRealClosure* real_closure;
    ...
    if (!closure->is_invalid)
    {
        ...
        if (!in_marshal)
            closure_invoke_notifiers(closure, PRE_NOTIFY);
        marshal(closure, return_value, instance, args, marshal_data, n_params,
                param_types);
        if (!in_marshal)
            closure_invoke_notifiers(closure, POST_NOTIFY);
        SET(closure, in_marshal, in_marshal);
...
void g_closure_set_marshal(GClosure* closure, GClosureMarshal marshal)
{
    ...
    if (closure->marshal && closure->marshal != marshal)
        g_warning(
            "attempt to override closure->marshal (%p) with new marshal (%p)",
            closure->marshal, marshal);
    else
        closure->marshal = marshal;
}

void _g_closure_set_va_marshal(GClosure* closure, GVaClosureMarshal marshal)
{
    GRealClosure* real_closure;
    ...
    if (real_closure->va_marshal && real_closure->va_marshal != marshal)
        g_warning("attempt to override closure->va_marshal (%p) with new "
                  "marshal (%p)",
 ...
 * Returns: (transfer none): a floating reference to a new #GCClosure
 */
GClosure* g_cclosure_new_swap(GCallback callback_func, gpointer user_data,
                              GClosureNotify destroy_data)
{
    GClosure* closure;
...
static void
    g_type_class_meta_marshal(GClosure* closure, GValue /*out*/* return_value,
                              guint n_param_values, const GValue* param_values,
                              gpointer invocation_hint, gpointer marshal_data)
{
    ...
    class = G_TYPE_INSTANCE_GET_CLASS(g_value_peek_pointer(param_values + 0),
                                      itype, GTypeClass);
    callback = G_STRUCT_MEMBER(gpointer, class, offset);
    if (callback)
        closure->marshal(closure, return_value, n_param_values, param_values,
                         invocation_hint, callback);
}

static void g_type_class_meta_marshalv(GClosure* closure, GValue* return_value,
                                       ...
                                       GType* param_types)
{
    ...
    callback = G_STRUCT_MEMBER(gpointer, class, offset);
    if (callback)
        real_closure->va_marshal(closure, return_value, instance, args,
                                 callback, n_params, param_types);
}

static void
    g_type_iface_meta_marshal(GClosure* closure, GValue /*out*/* return_value,
                              guint n_param_values, const GValue* param_values,
                              gpointer invocation_hint, gpointer marshal_data)
{
    GTypeClass* class;
    ...
    class = G_TYPE_INSTANCE_GET_INTERFACE(
        g_value_peek_pointer(param_values + 0), itype, GTypeClass);
    callback = G_STRUCT_MEMBER(gpointer, class, offset);
    if (callback)
        closure->marshal(closure, return_value, n_param_values, param_values,
                         invocation_hint, callback);
...
gboolean _g_closure_is_void(GClosure* closure, gpointer instance)
{
    GRealClosure* real_closure;
    GTypeClass* class;
    gpointer callback;
...
static void g_type_iface_meta_marshalv(GClosure* closure, GValue* return_value,
                                       gpointer instance, va_list args,
                                       gpointer marshal_data, int n_params,
                                       GType* param_types)
{
...
GClosure* g_signal_type_cclosure_new(GType itype, guint struct_offset)
{
    ...
    g_return_val_if_fail(G_TYPE_IS_CLASSED(itype) || G_TYPE_IS_INTERFACE(itype),
                         NULL);
    ...
    {
        g_closure_set_meta_marshal(closure, GUINT_TO_POINTER(struct_offset),
...
#include <ffi.h>
static ffi_type* value_to_ffi_type(const GValue* gvalue, gpointer* value,
                                   gint* enum_tmpval, gboolean* tmpval_used)
{
    ...
    switch (type)
    {
        ...
        case G_TYPE_INT:
            ...
            *value = (gpointer) & (gvalue->data[0].v_int);
            break;
        case G_TYPE_ENUM:
            /* enums are stored in v_long even though they are integers, which
             * makes marshalling through libffi somewhat complicated.  They need
             * to be marshalled as signed ints, but we need to use a temporary
             * int sized value to pass to libffi otherwise it'll pull the wrong
             * value on BE machines with 32-bit integers when treating v_long as
             * 32-bit int.
             */
...
static void value_from_ffi_type(GValue* gvalue, gpointer* value)
{
...
static ffi_type* va_to_ffi_type(GType gtype, va_list* va,
                                va_arg_storage* storage)
{
    ...
    switch (type)
    {
        ...
        default:
            rettype = &ffi_type_pointer;
            storage->_guint64 = 0;
            g_warning("va_to_ffi_type: Unsupported fundamental type: %s",
                      g_type_name(type));
            break;
    }
    return rettype;
 ...
 * Since: 2.30
 */
void g_cclosure_marshal_generic(GClosure* closure, GValue* return_gvalue,
                                ...
                                gpointer invocation_hint, gpointer marshal_data)
{
    ...
    {
        rtype = value_to_ffi_type(return_gvalue, &rvalue, enum_tmpval,
                                  &tmpval_used);
    }
    else
    {
    ...
    {
        atypes[n_args - 1] = value_to_ffi_type(
            param_values + 0, &args[n_args - 1], enum_tmpval, &tmpval_used);
        atypes[0] = &ffi_type_pointer;
        args[0] = &closure->data;
    }
    else
    {
        atypes[0] = value_to_ffi_type(param_values + 0, &args[0], enum_tmpval,
                                      &tmpval_used);
    ...
    for (i = 1; i < n_args - 1; i++)
    {
        ...
        atypes[i] = value_to_ffi_type(param_values + i, &args[i], enum_tmpval,
                                      &tmpval_used);
    }
 ...
 * Since: 2.30
 */
void g_cclosure_marshal_generic_va(GClosure* closure, GValue* return_value,
                                   ...
                                   GType* param_types)
{
    ...
    {
        rtype =
            value_to_ffi_type(return_value, &rvalue, enum_tmpval, &tmpval_used);
    }
    else
    {
        rtype = &ffi_type_void;
 ...
 *
 * A marshaller for a #GCClosure with a callback of type
 * `void (*callback) (gpointer instance, guint arg1, gpointer arg2, gpointer
 * user_data)`.
 */
 ...
 *
 * A marshaller for a #GCClosure with a callback of type
 * `gboolean (*callback) (gpointer instance, GBoxed *arg1, GBoxed *arg2,
 * gpointer user_data)`.
 *
 * Since: 2.26
 */
