...
int main(int argc, char* argv[])
{
    ...
    if (jffs2)
    {
        ...
        cleanmarker.hdr_crc =
            cpu_to_je32(mtd_crc32(0, &cleanmarker, sizeof(cleanmarker) - 4));
    }
    ...
    for (eb = eb_start; eb < eb_start + eb_cnt; eb++)
    {
        ...
        if (!noskipbad)
        {
            ...
            if (ret > 0)
            {
                verbose(!quiet, "Skipping bad block at %08llx",
            ...
            else if (ret < 0)
            {
                if (errno == EOPNOTSUPP)
                {
                    noskipbad = 1;
                    if (isNAND)
                        return errmsg("%s: Bad block check not available",
                                      mtd_device);
                }
                else
                    return sys_errmsg("%s: MTD get bad block failed",
                                      mtd_device);
            }
        }
        ...
        if (isNAND)
        {
            if (mtd_write(mtd_desc, &mtd, fd, eb, 0, NULL, 0, &cleanmarker,
                          cmlen, MTD_OPS_AUTO_OOB) != 0)
            {
        ...
        {
            if (pwrite(fd, &cleanmarker, sizeof(cleanmarker), (loff_t)offset) !=
                sizeof(cleanmarker))
            {
