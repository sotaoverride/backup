...
#define BOOST_LOG_AUX_LOG_ATTRIBUTE_VALUE_TYPES()                              \
    (boost::log::attributes::named_scope_list)(boost::log::aux::process::id)   \
        BOOST_LOG_AUX_THREAD_ID_TYPE()

// The list of the attribute value types supported by the default formatter.
// Note that we have to exclude std::time_t as it is an integral type, as well
// as double from the native time duration types - these are part of arithmetic
// types already.
#define BOOST_LOG_AUX_LOG_DEFAULT_VALUE_TYPES()                                \
    BOOST_LOG_DEFAULT_ATTRIBUTE_VALUE_TYPES()                                  \
    (std::tm) BOOST_LOG_BOOST_DATE_TYPES()                                     \
    ...
    class default_formatter
    {
      ...
      private:
        ...
        struct visitor
        {
            ...
            explicit visitor(basic_formatting_ostream<CharT>& strm) :
                m_strm(strm)
            {
            }
            ...
            void operator()(boost::posix_time::ptime const& value) const
            {
                if (!value.is_special())
                {
                    ...
                    int res = boost::log::aux::snprintf(
                        buf + len, size, ".%.6u",
                        static_cast<unsigned int>(
                            value.time_of_day().total_microseconds() %
                            1000000));
                    if (res < 0)
                        buf[len] = '\0';
            ...
            void operator()(
                boost::local_time::local_date_time const& value) const
            {
                if (!value.is_special())
                {
            ...
            void operator()(boost::posix_time::time_duration const& value) const
            {
                if (!value.is_special())
                {
                    ...
                    unsigned long long total_useconds =
                        value.total_microseconds();
                    unsigned long long hours =
                        total_useconds / (3600ull * 1000000ull);
                    unsigned int minutes = static_cast<unsigned int>(
                        total_useconds / (60ull * 1000000ull) % 60ull);
                    unsigned int seconds = static_cast<unsigned int>(
                        total_useconds / 1000000ull % 60ull);
                    unsigned int useconds =
                        static_cast<unsigned int>(total_useconds % 1000000ull);
                    char buf[64];
                    int len = boost::log::aux::snprintf(
                        buf, sizeof(buf), "%.2llu:%.2u:%.2u.%.6u", hours,
                        minutes, seconds, useconds);
                    if (len > 0)
                    {
                        unsigned int size =
                            static_cast<unsigned int>(len) >= sizeof(buf)
            ...
            void operator()(
                boost::date_time::period<PointRepT, DurationRepT> const& value)
                const
            {
                m_strm << '[';
          ...
          private:
            ...
            void format_special_date_time(
                boost::date_time::special_values value) const
            {
                switch (value)
                {
      ...
      public:
        ...
        result_type operator()(record_view const& rec,
                               basic_formatting_ostream<CharT>& strm) const
        {
            ...
            boost::log::visit<value_types>(m_attribute_name, rec,
                                           visitor(strm));
        }
...
typename default_formatter_factory<CharT>::formatter_type
    default_formatter_factory<CharT>::create_formatter(
        attribute_name const& name, args_map const& args)
{
    // No user-defined factory, shall use the most generic formatter we can ever
