...
libc_hidden_proto(__readdir64) extern int __readdir_r(DIR* __dirp,
                                                      struct dirent* __entry,
                                                      struct dirent** __result);
extern int __readdir64_r(DIR* __dirp, struct dirent64* __entry,
                         struct dirent64** __result);
extern int __scandir64(const char* __dir, struct dirent64*** __namelist,
...
extern __ssize_t __getdirentries(int __fd, char* __restrict __buf,
    ...
    __nonnull((2, 4));
extern __ssize_t __getdents(int __fd, char* __buf,
                            size_t __nbytes) attribute_hidden;
extern __ssize_t __getdents64(int __fd, char* __buf,
                              size_t __nbytes) attribute_hidden;
extern int __alphasort64(const struct dirent64** a,
                         const struct dirent64** b) __attribute_pure__;
...
extern int __scandir_tail(DIR* dp, struct dirent*** namelist,
                          int (*select)(const struct dirent*),
                          int (*cmp)(const struct dirent**,
                                     const struct dirent**)) attribute_hidden;
...
#endif
extern int
    __scandir64_tail(DIR* dp, struct dirent64*** namelist,
                     int (*select)(const struct dirent64*),
                     int (*cmp)(const struct dirent64**,
                                const struct dirent64**)) attribute_hidden;

libc_hidden_proto(__rewinddir) extern __typeof(scandirat) __scandirat;
