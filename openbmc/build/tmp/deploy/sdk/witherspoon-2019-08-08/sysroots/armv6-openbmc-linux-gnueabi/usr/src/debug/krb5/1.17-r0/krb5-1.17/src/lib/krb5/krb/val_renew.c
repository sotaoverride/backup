 ...
 */
static krb5_error_code get_new_creds(krb5_context context, krb5_ccache ccache,
                                     krb5_creds* in_creds, krb5_flags kdcopt,
 ...
 * credential.
 */
static krb5_error_code gc_valrenew(krb5_context context, krb5_ccache ccache,
                                   krb5_creds* in_creds, krb5_flags kdcopt,
                                   krb5_creds** out_creds)
{
    krb5_error_code code;
...
krb5_error_code KRB5_CALLCONV krb5_get_credentials_renew(krb5_context context,
                                                         krb5_flags options,
                                                         krb5_ccache ccache,
                                                         krb5_creds* in_creds,
                                                         krb5_creds** out_creds)
                         ...
                         const char* in_tkt_service, int kdcopt)
{
    ...
    else
    {
        ...
        code =
            krb5int_tgtname(context, &client->realm, &client->realm, &server);
        if (code != 0)
            goto cleanup;
    }
...
{
    return get_valrenewed_creds(context, creds, client, ccache, in_tkt_service,
                                KDC_OPT_VALIDATE);
}

krb5_error_code KRB5_CALLCONV krb5_get_renewed_creds(krb5_context context,
                                                     krb5_creds* creds,
