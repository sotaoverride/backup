...
void _gcry_poly1305_amd64_sse2_init_ext(void* state,
                                        const poly1305_key_t* key) OPS_FUNC_ABI;
...
static const poly1305_ops_t poly1305_amd64_sse2_ops = {
    POLY1305_SSE2_BLOCKSIZE, _gcry_poly1305_amd64_sse2_init_ext,
    _gcry_poly1305_amd64_sse2_blocks, _gcry_poly1305_amd64_sse2_finish_ext};
...
static OPS_FUNC_ABI unsigned int poly1305_finish_ext_ref32
    /**/ (void* state, const byte* m, size_t remaining,
          byte mac[POLY1305_TAGLEN]);

static const poly1305_ops_t poly1305_default_ops = {
    POLY1305_REF_BLOCKSIZE, poly1305_init_ext_ref32, poly1305_blocks_ref32,
...
void _gcry_poly1305_amd64_avx2_init_ext(void* state,
                                        const poly1305_key_t* key) OPS_FUNC_ABI;
...
static const poly1305_ops_t poly1305_amd64_avx2_ops = {
    POLY1305_AVX2_BLOCKSIZE, _gcry_poly1305_amd64_avx2_init_ext,
    _gcry_poly1305_amd64_avx2_blocks, _gcry_poly1305_amd64_avx2_finish_ext};

#endif

#ifdef POLY1305_USE_NEON

void _gcry_poly1305_armv7_neon_init_ext(void* state,
                                        const poly1305_key_t* key) OPS_FUNC_ABI;
...
static const poly1305_ops_t poly1305_armv7_neon_ops = {
    POLY1305_NEON_BLOCKSIZE, _gcry_poly1305_armv7_neon_init_ext,
    _gcry_poly1305_armv7_neon_blocks, _gcry_poly1305_armv7_neon_finish_ext};
...
static OPS_FUNC_ABI void poly1305_init_ext_ref32(void* state,
                                                 const poly1305_key_t* key)
{
    ...
    poly1305_blocks_ref32(void* state, const byte* m, size_t bytes)
{
    ...
    while (bytes >= POLY1305_REF_BLOCKSIZE)
    {
        ...
        /* h *= r */
        d0 = ((u64)h0 * r0) + ((u64)h1 * s4) + ((u64)h2 * s3) + ((u64)h3 * s2) +
             ((u64)h4 * s1);
        d1 = ((u64)h0 * r1) + ((u64)h1 * r0) + ((u64)h2 * s4) + ((u64)h3 * s3) +
             ((u64)h4 * s2);
        d2 = ((u64)h0 * r2) + ((u64)h1 * r1) + ((u64)h2 * r0) + ((u64)h3 * s4) +
             ((u64)h4 * s3);
        d3 = ((u64)h0 * r3) + ((u64)h1 * r2) + ((u64)h2 * r1) + ((u64)h3 * r0) +
             ((u64)h4 * s4);
        d4 = ((u64)h0 * r4) + ((u64)h1 * r3) + ((u64)h2 * r2) + ((u64)h3 * r1) +
             ((u64)h4 * r0);
...
#ifndef POLY1305_USE_SSE2
static OPS_FUNC_ABI unsigned int
    poly1305_finish_ext_ref32(void* state, const byte* m, size_t remaining,
                              byte mac[POLY1305_TAGLEN])
{
    poly1305_state_ref32_t* st = (poly1305_state_ref32_t*)state;
    ...
    /* burn_stack */
    return (13 * sizeof(u32) + sizeof(u64) + POLY1305_REF_BLOCKSIZE +
            6 * sizeof(void*)) +
           burn;
}
#endif /* !POLY1305_USE_SSE2*/

static inline void* poly1305_get_state(poly1305_context_t* ctx)
{
    byte* c = ctx->state;
...
static void poly1305_init(poly1305_context_t* ctx, const poly1305_key_t* key)
{
...
void _gcry_poly1305_update(poly1305_context_t* ctx, const byte* m, size_t bytes)
{
    void* state = poly1305_get_state(ctx);
    unsigned int burn = 0;
...
void _gcry_poly1305_finish(poly1305_context_t* ctx, byte mac[POLY1305_TAGLEN])
{
    void* state = poly1305_get_state(ctx);
    unsigned int burn;
...
static void poly1305_auth(byte mac[POLY1305_TAGLEN], const byte* m,
                          size_t bytes, const byte* key)
{
...
static const char* selftest(void)
{
    ...
    static const byte nacl_mac[16] = {0xf3, 0xff, 0xc7, 0x70, 0x3f, 0x94,
    ...
    static const byte wrap_msg[16] = {0xff, 0xff, 0xff, 0xff, 0xff, 0xff,
    ...
    static const byte total_mac[16] = {0x64, 0xaf, 0xe2, 0xe8, 0xd6, 0xad,
                                       0x7b, 0xbd, 0xd2, 0x87, 0xf9, 0x7c,
                                       0x44, 0x62, 0x3d, 0x39};
