...
G_DEFINE_ABSTRACT_TYPE_WITH_PRIVATE(GOutputStream, g_output_stream,
                                    G_TYPE_OBJECT)
...
static gssize g_output_stream_real_splice(GOutputStream* stream,
                                          ...
                                          GError** error);
static void g_output_stream_real_write_async(
    GOutputStream* stream, const void* buffer, gsize count, int io_priority,
    GCancellable* cancellable, GAsyncReadyCallback callback, gpointer data);
static gssize g_output_stream_real_write_finish(GOutputStream* stream,
                                                GAsyncResult* result,
                                                GError** error);
static gboolean g_output_stream_real_writev(
    GOutputStream* stream, const GOutputVector* vectors, gsize n_vectors,
...
static void g_output_stream_real_writev_async(GOutputStream* stream,
                                              ...
                                              gsize n_vectors, int io_priority,
                                              GCancellable* cancellable,
...
static gboolean g_output_stream_real_writev_finish(GOutputStream* stream,
                                                   ...
                                                   gsize* bytes_written,
                                                   GError** error);
static void g_output_stream_real_splice_async(
    GOutputStream* stream, GInputStream* source, GOutputStreamSpliceFlags flags,
 ...
 * @buffer: (array length=count) (element-type guint8): the buffer containing
 *the data to write.
 ...
 **/
gssize g_output_stream_write(GOutputStream* stream, const void* buffer,
 ...
 **/
gboolean g_output_stream_write_all(GOutputStream* stream, const void* buffer,
                                   ...
                                   GCancellable* cancellable, GError** error)
{
    ...
    {
        res = g_output_stream_write(stream, (char*)buffer + _bytes_written,
                                    count - _bytes_written, cancellable, error);
        if (res == -1)
        {
            if (bytes_written)
 ...
 * @stream: a #GOutputStream.
 * @vectors: (array length=n_vectors): the buffer containing the #GOutputVectors
 * to write.
...
gboolean g_output_stream_writev(GOutputStream* stream,
                                const GOutputVector* vectors, gsize n_vectors,
                                ...
                                GError** error)
{
    ...
    res = class->writev_fn(stream, vectors, n_vectors, &_bytes_written,
                           cancellable, error);
...
gboolean g_output_stream_writev_all(GOutputStream* stream,
                                    GOutputVector* vectors, gsize n_vectors,
                                    gsize* bytes_written,
                                    GCancellable* cancellable, GError** error)
{
    gsize _bytes_written = 0;
    ...
    while (n_vectors > 0 && to_be_written > 0)
    {
        ...
        res = g_output_stream_writev(stream, vectors, n_vectors, &n_written,
                                     cancellable, error);
 ...
 * Returns: %TRUE on success, %FALSE if there was an error
 **/
gboolean g_output_stream_printf(GOutputStream* stream, gsize* bytes_written,
                                GCancellable* cancellable, GError** error,
                                const gchar* format, ...)
{
    va_list args;
    ...
    success = g_output_stream_vprintf(stream, bytes_written, cancellable, error,
                                      format, args);
    va_end(args);
 ...
 * Returns: %TRUE on success, %FALSE if there was an error
 **/
gboolean g_output_stream_vprintf(GOutputStream* stream, gsize* bytes_written,
                                 GCancellable* cancellable, GError** error,
                                 const gchar* format, va_list args)
{
    ...
    success = g_output_stream_write_all(stream, text, strlen(text),
 ...
 * Returns: Number of bytes written, or -1 on error
 **/
gssize g_output_stream_write_bytes(GOutputStream* stream, GBytes* bytes,
                                   GCancellable* cancellable, GError** error)
{
    gsize size;
    gconstpointer data;
    ...
    return g_output_stream_write(stream, data, size, cancellable, error);
}
 ...
 **/
gboolean g_output_stream_flush(GOutputStream* stream, GCancellable* cancellable,
                               GError** error)
{
    GOutputStreamClass* class;
    gboolean res;
 ...
 *     of bytes spliced.
 **/
gssize g_output_stream_splice(GOutputStream* stream, GInputStream* source,
                              GOutputStreamSpliceFlags flags,
                              GCancellable* cancellable, GError** error)
{
    GOutputStreamClass* class;
                                          ...
                                          GError** error)
{
    ...
    do
    {
        n_read = g_input_stream_read(source, buffer, sizeof(buffer),
                                     cancellable, error);
        if (n_read == -1)
        {
            res = FALSE;
 ...
 **/
gboolean g_output_stream_close(GOutputStream* stream, GCancellable* cancellable,
                               GError** error)
{
    gboolean res;
 ...
 */
void g_output_stream_write_async(GOutputStream* stream, const void* buffer,
 ...
 **/
gssize g_output_stream_write_finish(GOutputStream* stream, GAsyncResult* result,
                                    GError** error)
{
    g_return_val_if_fail(G_IS_OUTPUT_STREAM(stream), FALSE);
    g_return_val_if_fail(g_task_is_valid(result, stream), FALSE);
    g_return_val_if_fail(
        g_async_result_is_tagged(result, g_output_stream_write_async), FALSE);
...
static void free_async_write_all(gpointer data)
{
    g_slice_free(AsyncWriteAll, data);
}

static void write_all_callback(GObject* stream, GAsyncResult* result,
                               gpointer user_data)
{
    GTask* task = user_data;
    ...
    if (result)
    {
        ...
        nwritten = g_output_stream_write_finish(G_OUTPUT_STREAM(stream), result,
                                                &error);
    ...
    if (data->to_write == 0)
    {
        ...
        g_output_stream_write_async(
            G_OUTPUT_STREAM(stream), data->buffer + data->bytes_written,
            data->to_write, g_task_get_priority(task),
            g_task_get_cancellable(task), write_all_callback, task);
}

static void write_all_async_thread(GTask* task, gpointer source_object,
                                   gpointer task_data,
                                   GCancellable* cancellable)
{
    ...
    if (g_output_stream_write_all(stream, data->buffer, data->to_write,
                                  &data->bytes_written,
 ...
 * Since: 2.44
 */
void g_output_stream_write_all_async(GOutputStream* stream, const void* buffer,
                                     gsize count, int io_priority,
                                     GCancellable* cancellable,
                                     GAsyncReadyCallback callback,
                                     gpointer user_data)
 ...
 * can then call g_output_stream_writev_finish() to get the result of the
 * operation.
 *
 * During an async request no other sync and async calls are allowed,
 * and will result in %G_IO_ERROR_PENDING errors.
...
static void free_async_writev_all(gpointer data)
{
    g_slice_free(AsyncWritevAll, data);
}

static void writev_all_callback(GObject* stream, GAsyncResult* result,
                                gpointer user_data)
{
    GTask* task = user_data;
    ...
    if (result)
    {
        ...
        res = g_output_stream_writev_finish(G_OUTPUT_STREAM(stream), result,
                                            &n_written, &error);
        ...
        if (n_written > 0 && data->n_vectors > 0)
        {
            ...
            data->vectors[0].buffer =
                ((guint8*)data->vectors[0].buffer) + n_written;
        }
    }
    ...
    if (data->n_vectors == 0)
    {
        ...
        g_output_stream_writev_async(
            G_OUTPUT_STREAM(stream), data->vectors, data->n_vectors, priority,
            cancellable, writev_all_callback, g_steal_pointer(&task));
}

static void writev_all_async_thread(GTask* task, gpointer source_object,
                                    gpointer task_data,
                                    GCancellable* cancellable)
{
    GOutputStream* stream = G_OUTPUT_STREAM(source_object);
    ...
    if (g_output_stream_writev_all(stream, data->vectors, data->n_vectors,
                                   &data->bytes_written,
                                   g_task_get_cancellable(task), &error))
        g_task_return_boolean(task, TRUE);
    else
 ...
 * @vectors into the stream. When the operation is finished @callback will be
 * called. You can then call g_output_stream_writev_all_finish() to get the
...
static void write_bytes_callback(GObject* stream, GAsyncResult* result,
                                 gpointer user_data)
{
    GTask* task = user_data;
    GError* error = NULL;
 ...
 * g_output_stream_write_bytes().
 **/
void g_output_stream_write_bytes_async(GOutputStream* stream, GBytes* bytes,
                                       int io_priority,
                                       GCancellable* cancellable,
                                       GAsyncReadyCallback callback,
                                       gpointer user_data)
{
    ...
    g_output_stream_write_async(stream, data, size, io_priority, cancellable,
                                write_bytes_callback, task);
}
...
static void async_ready_splice_callback_wrapper(GObject* source_object,
                                                GAsyncResult* res,
                                                gpointer _data)
{
    GOutputStream* stream = G_OUTPUT_STREAM(source_object);
 ...
 * g_output_stream_splice().
 **/
void g_output_stream_splice_async(GOutputStream* stream, GInputStream* source,
                                  GOutputStreamSpliceFlags flags,
                                  int io_priority, GCancellable* cancellable,
                                  GAsyncReadyCallback callback,
                                  gpointer user_data)
                                     ...
                                     GAsyncResult* result, GError** error)
{
    ...
    g_return_val_if_fail(g_task_is_valid(result, stream), FALSE);
    g_return_val_if_fail(
        g_async_result_is_tagged(result, g_output_stream_splice_async), FALSE);

    /* @result is always the GTask created by g_output_stream_splice_async();
     * we called class->splice_finish() from
     * async_ready_splice_callback_wrapper.
     */
    return g_task_propagate_int(G_TASK(result), error);
}

static void async_ready_flush_callback_wrapper(GObject* source_object,
                                               GAsyncResult* res,
                                               gpointer user_data)
{
    GOutputStream* stream = G_OUTPUT_STREAM(source_object);
 ...
 * result of the operation.
 **/
void g_output_stream_flush_async(GOutputStream* stream, int io_priority,
                                 GCancellable* cancellable,
                                 GAsyncReadyCallback callback,
                                 gpointer user_data)
{
                                      ...
                                      GAsyncResult* result, GError** error)
{
    ...
    g_return_val_if_fail(g_task_is_valid(result, stream), FALSE);
    g_return_val_if_fail(
        g_async_result_is_tagged(result, g_output_stream_flush_async), FALSE);
...
static void async_ready_close_callback_wrapper(GObject* source_object,
                                               GAsyncResult* res,
                                               gpointer user_data)
{
    GOutputStream* stream = G_OUTPUT_STREAM(source_object);
    ...
    if (!error && !g_async_result_legacy_propagate_error(res, &error))
    {
        ...
        class->close_finish(stream, res, error ? NULL : &error);
    }
...
static void async_ready_close_flushed_callback_wrapper(GObject* source_object,
                                                       GAsyncResult* res,
                                                       gpointer user_data)
{
    GOutputStream* stream = G_OUTPUT_STREAM(source_object);
    ...
    class->close_async(stream, g_task_get_priority(task),
                       g_task_get_cancellable(task),
                       async_ready_close_callback_wrapper, task);
}

static void real_close_async_cb(GObject* source_object, GAsyncResult* res,
                                gpointer user_data)
{
    GOutputStream* stream = G_OUTPUT_STREAM(source_object);
    GTask* task = user_data;
 ...
 * classes. However, if you override one you must override all.
 **/
void g_output_stream_close_async(GOutputStream* stream, int io_priority,
                                 GCancellable* cancellable,
                                 GAsyncReadyCallback callback,
                                 gpointer user_data)
{
 ...
 * g_output_stream_set_pending()/g_output_stream_clear_pending().
 */
void g_output_stream_internal_close_async(GOutputStream* stream,
                                          int io_priority,
                                          GCancellable* cancellable,
                                          GAsyncReadyCallback callback,
                                          gpointer user_data)
                                                      ...
                                                      GError** error)
{
    ...
    g_return_val_if_fail(g_task_is_valid(result, stream), FALSE);
    g_return_val_if_fail(
        g_async_result_is_tagged(result, g_output_stream_internal_close_async),
        FALSE);
                                      ...
                                      GAsyncResult* result, GError** error)
{
    ...
    g_return_val_if_fail(g_task_is_valid(result, stream), FALSE);
    g_return_val_if_fail(
        g_async_result_is_tagged(result, g_output_stream_close_async), FALSE);
...
gboolean g_output_stream_set_pending(GOutputStream* stream, GError** error)
{
    ...
    if (stream->priv->pending)
    {
        g_set_error_literal(error, G_IO_ERROR, G_IO_ERROR_PENDING,
                             ...
                             * already an operation running against this stream
                             * when you try to start one */
...
gboolean g_output_stream_async_write_is_via_threads(GOutputStream* stream)
{
    ...
    return (
        class->write_async == g_output_stream_real_write_async &&
...
gboolean g_output_stream_async_writev_is_via_threads(GOutputStream* stream)
{
    ...
    return (
        class->writev_async == g_output_stream_real_writev_async &&
        !(G_IS_POLLABLE_OUTPUT_STREAM(stream) &&
          g_pollable_output_stream_can_poll(G_POLLABLE_OUTPUT_STREAM(stream))));
}
    ...
    gsize* bytes_written, GCancellable* cancellable, GError** error)
{
    ...
    for (i = 0; i < n_vectors; i++)
    {
        ...
        res = class->write_fn(stream, vectors[i].buffer, vectors[i].size,
                              cancellable, &err);
...
static void free_write_data(WriteData* op)
{
    g_slice_free(WriteData, op);
}

static void write_async_thread(GTask* task, gpointer source_object,
...
static void write_async_pollable(GPollableOutputStream* stream, GTask* task);

static gboolean write_async_pollable_ready(GPollableOutputStream* stream,
                                           gpointer user_data)
{
    GTask* task = user_data;
...
static void write_async_pollable(GPollableOutputStream* stream, GTask* task)
{
    ...
    count_written =
        G_POLLABLE_OUTPUT_STREAM_GET_INTERFACE(stream)->write_nonblocking(
            stream, op->buffer, op->count_requested, &error);
    ...
    if (g_error_matches(error, G_IO_ERROR, G_IO_ERROR_WOULD_BLOCK))
    {
        ...
        source = g_pollable_output_stream_create_source(
            stream, g_task_get_cancellable(task));
...
static void free_writev_data(WritevData* op)
{
    g_slice_free(WritevData, op);
}

static void writev_async_thread(GTask* task, gpointer source_object,
                                gpointer task_data, GCancellable* cancellable)
{
    GOutputStream* stream = source_object;
...
static void writev_async_pollable(GPollableOutputStream* stream, GTask* task);

static gboolean writev_async_pollable_ready(GPollableOutputStream* stream,
                                            gpointer user_data)
{
    GTask* task = user_data;
...
static void writev_async_pollable(GPollableOutputStream* stream, GTask* task)
{
    ...
    res = G_POLLABLE_OUTPUT_STREAM_GET_INTERFACE(stream)->writev_nonblocking(
        stream, op->vectors, op->n_vectors, &bytes_written, &error);
                                                   ...
                                                   GError** error)
{
    ...
    g_return_val_if_fail(g_task_is_valid(result, stream), FALSE);
    g_return_val_if_fail(
        g_async_result_is_tagged(result, g_output_stream_writev_async), FALSE);
...
static void free_splice_data(SpliceData* op)
{
...
static void real_splice_async_complete_cb(GTask* task)
{
...
static void real_splice_async_close_input_cb(GObject* source, GAsyncResult* res,
                                             gpointer user_data)
{
    GTask* task = user_data;
...
static void real_splice_async_close_output_cb(GObject* source,
                                              GAsyncResult* res,
                                              gpointer user_data)
{
...
static void real_splice_async_complete(GTask* task)
{
    SpliceData* op = g_task_get_task_data(task);
    gboolean done = TRUE;
    ...
    if (op->flags & G_OUTPUT_STREAM_SPLICE_CLOSE_TARGET)
    {
        ...
        g_output_stream_internal_close_async(
            g_task_get_source_object(task), g_task_get_priority(task),
            g_task_get_cancellable(task), real_splice_async_close_output_cb,
            task);
    }
...
static void real_splice_async_read_cb(GObject* source, GAsyncResult* res,
                                      gpointer user_data);

static void real_splice_async_write_cb(GObject* source, GAsyncResult* res,
                                       gpointer user_data)
{
    GOutputStreamClass* class;
    GTask* task = G_TASK(user_data);
    ...
    {
        class->write_async(
            g_task_get_source_object(task), op->buffer + op->n_written,
            op->n_read - op->n_written, g_task_get_priority(task),
            g_task_get_cancellable(task), real_splice_async_write_cb, task);
        return;
    }

    g_input_stream_read_async(
        op->source, op->buffer, 8192, g_task_get_priority(task),
        g_task_get_cancellable(task), real_splice_async_read_cb, task);
}
                                      ...
                                      gpointer user_data)
{
    ...
    class->write_async(g_task_get_source_object(task), op->buffer, op->n_read,
                       g_task_get_priority(task), g_task_get_cancellable(task),
                       real_splice_async_write_cb, task);
}

static void splice_async_thread(GTask* task, gpointer source_object,
                                gpointer task_data, GCancellable* cancellable)
{
    GOutputStream* stream = source_object;
    ...
    bytes_copied =
        class->splice(stream, op->source, op->flags, cancellable, &error);
    if (bytes_copied == -1)
        g_task_return_error(task, error);
    else
...
static void flush_async_thread(GTask* task, gpointer source_object,
                               gpointer task_data, GCancellable* cancellable)
{
    GOutputStream* stream = source_object;
    GOutputStreamClass* class;
...
static void close_async_thread(GTask* task, gpointer source_object,
                               gpointer task_data, GCancellable* cancellable)
{
    GOutputStream* stream = source_object;
    GOutputStreamClass* class;
