...
struct wdinfo
{
    ...
    unsigned int has_timeout : 1, has_timeleft : 1, has_pretimeout : 1;
};
...
static void __attribute__((__noreturn__)) usage(void)
{
    ...
    fputs(
        _(" -f, --flags <list>     print selected flags only\n"
...
static void add_flag_line(struct libscols_table* table, struct wdinfo* wd,
...
static int set_watchdog(struct wdinfo* wd, int timeout)
{
    ...
    {
        if (errno == EBUSY)
            warnx(_("%s: watchdog already in use, terminating."), wd->device);
...
static void print_oneline(struct wdinfo* wd, uint32_t wanted, int noident,
                          int notimeouts, int noflags)
{
    printf("%s:", wd->device);
    ...
    if (!noflags)
    {
        ...
        for (i = 0; i < ARRAY_SIZE(wdflags); i++)
        {
            ...
            printf(" %s=\"%s\"", fl->name, wd->status & fl->flag ? "1" : "0");
...
int main(int argc, char* argv[])
{
    ...
    while ((c = getopt_long(argc, argv, "d:f:hFnITo:s:OrVx", long_opts,
                            NULL)) != -1)
    {
        ...
        switch (c)
        {
            ...
            case 's':
                ...
                break;
            case 'f':
                if (string_to_bitmask(optarg, (unsigned long*)&wanted,
                                      name2bit) != 0)
                    return EXIT_FAILURE;
                break;
            case 'V':
    ...
    do
    {
        ...
        {
            printf("%-15s%s\n", _("Device:"), wd.device);
            printf("%-15s%s [%s %x]\n", _("Identity:"), wd.ident.identity,
                   _("version"), wd.ident.firmware_version);
        }
        if (!notimeouts)
            show_timeouts(&wd);
        if (!noflags)
