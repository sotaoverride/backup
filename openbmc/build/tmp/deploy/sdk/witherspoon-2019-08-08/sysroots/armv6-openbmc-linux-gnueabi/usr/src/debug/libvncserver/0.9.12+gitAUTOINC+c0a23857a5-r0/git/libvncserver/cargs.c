...
void rfbUsage(void)
{
    ...
    fprintf(stderr,
            ...
            "                       (use 'storepasswd' to create a password "
            "file)\n");
    fprintf(stderr, "-rfbversion 3.x        Set the version of the RFB we "
                    "choose to advertise\n");
    fprintf(stderr, "-permitfiletransfer    permit file transfer support\n");
    fprintf(stderr, "-passwd plain-password use authentication \n"
                    "                       (use plain-password as password, "
                    "USE AT YOUR RISK)\n");
    ...
    fprintf(stderr, "-sslkeyfile path       set path to private key file for "
                    "encrypted WebSockets connections\n");
    fprintf(stderr, "-sslcertfile path      set path to certificate file for "
                    "encrypted WebSockets connections\n");
#endif
    fprintf(stderr,
    ...
    fprintf(stderr, "-listen ipaddr         listen for connections only on "
                    "network interface with\n");
    fprintf(stderr, "                       addr ipaddr. '-listen localhost' "
                    "and hostname work too.\n");
#ifdef LIBVNCSERVER_IPv6
    fprintf(stderr, "-listenv6 ipv6addr     listen for IPv6 connections only "
                    "on network interface with\n");
    fprintf(stderr, "                       addr ipv6addr. '-listen localhost' "
                    "and hostname work too.\n");
#endif
...
void rfbPurgeArguments(int* argc, int* position, int count, char* argv[])
{
    ...
    if (amount)
        memmove(argv + (*position), argv + (*position) + count,
                sizeof(char*) * amount);
    (*argc) -= count;
}
...
rfbBool rfbProcessArguments(rfbScreenInfoPtr rfbScreen, int* argc, char* argv[])
{
    ...
    for (i = i1 = 1; i < *argc;)
    {
        ...
        else if (strcmp(argv[i], "-rfbport") == 0)
        { /* -rfbport port */
            if (i + 1 >= *argc)
            {
                rfbUsage();
        ...
        else if (strcmp(argv[i], "-rfbportv6") == 0)
        { /* -rfbportv6 port */
            if (i + 1 >= *argc)
            {
                rfbUsage();
        ...
        else if (strcmp(argv[i], "-rfbwait") == 0)
        { /* -rfbwait ms */
            if (i + 1 >= *argc)
            {
                rfbUsage();
        ...
        else if (strcmp(argv[i], "-rfbauth") == 0)
        { /* -rfbauth passwd-file */
            if (i + 1 >= *argc)
            {
                rfbUsage();
        ...
        else if (strcmp(argv[i], "-permitfiletransfer") == 0)
        { /* -permitfiletransfer  */
            rfbScreen->permitFileTransfer = TRUE;
        }
        else if (strcmp(argv[i], "-rfbversion") == 0)
        { /* -rfbversion 3.6  */
            if (i + 1 >= *argc)
            {
                rfbUsage();
            ...
            sscanf(argv[++i], "%d.%d", &rfbScreen->protocolMajorVersion,
        ...
        else if (strcmp(argv[i], "-deferptrupdate") == 0)
        { /* -deferptrupdate milliseconds */
            if (i + 1 >= *argc)
            {
                rfbUsage();
                return FALSE;
        ...
        else if (strcmp(argv[i], "-httpdir") == 0)
        ...
        else if (strcmp(argv[i], "-httpport") == 0)
        { /* -httpport portnum */
            if (i + 1 >= *argc)
            {
                rfbUsage();
        ...
        else if (strcmp(argv[i], "-httpportv6") == 0)
        { /* -httpportv6 portnum */
            if (i + 1 >= *argc)
            {
                rfbUsage();
        ...
        else if (strcmp(argv[i], "-listenv6") == 0)
        { /* -listenv6 ipv6addr */
            if (i + 1 >= *argc)
            {
                rfbUsage();
        ...
        else if (strcmp(argv[i], "-sslkeyfile") == 0)
        ...
        else if (strcmp(argv[i], "-sslcertfile") == 0)
        ...
        else
        {
            ...
            for (extension = rfbGetExtensionIterator();
                 handled == 0 && extension; extension = extension->next)
...
rfbBool rfbProcessSizeArguments(int* width, int* height, int* bpp, int* argc,
                                char* argv[])
{
    int i, i1;
