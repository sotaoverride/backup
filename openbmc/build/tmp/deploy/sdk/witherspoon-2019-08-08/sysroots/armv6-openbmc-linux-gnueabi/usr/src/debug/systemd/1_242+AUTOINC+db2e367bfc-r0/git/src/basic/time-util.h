...
triple_timestamp* triple_timestamp_from_realtime(triple_timestamp* ts,
                                                 usec_t u);
...
static inline usec_t usec_add(usec_t a, usec_t b)
{
    ...
    /* Adds two time values, and makes sure USEC_INFINITY as input results as
     * USEC_INFINITY in output, and doesn't overflow. */
...
#if SIZEOF_TIME_T == 8
/* The last second we can format is 31. Dec 9999, 1s before midnight, because
 * otherwise we'd enter 5 digit year territory. However, since we want to stay
 * away from this in all timezones we take one day off. */
