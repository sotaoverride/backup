...
static ssize_t fail_no_erange(void)
{
    if (errno == ERANGE)
        __set_errno(EINVAL);
    return -1;
...
static ssize_t readline_slow(FILE* fp, char* buffer, char* buffer_end)
{
    char* start = buffer;
...
ssize_t __libc_readline_unlocked(FILE* fp, char* buffer, size_t buffer_length)
{
