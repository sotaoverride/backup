...
static void debprret(char* w, update_status_t ret, kdb_sno_t sno)
{
    switch (ret)
    {
        case UPDATE_OK:
...
static char* replystr(update_status_t ret)
{
    switch (ret)
    {
        case UPDATE_OK:
...
static char* buf_to_string(gss_buffer_desc* b)
{
    OM_uint32 min_stat;
...
static krb5_boolean iprop_acl_check(krb5_context context,
                                    const char* client_name)
{
    ...
    if (krb5_parse_name(context, client_name, &client_princ) != 0)
        return FALSE;
    result =
        auth(context, OP_IPROP, client_princ, NULL, NULL, NULL, NULL, NULL, 0);
    krb5_free_principal(context, client_princ);
    return result;
}

kdb_incr_result_t* iprop_get_updates_1_svc(kdb_last_t* arg,
                                           struct svc_req* rqstp)
{
    ...
    DPRINT("%s: start, last_sno=%lu\n", whoami, (unsigned long)arg->last_sno);

    if (!handle)
    {
        krb5_klog_syslog(LOG_ERR, _("%s: server handle is NULL"), whoami);
        goto out;
    }

    {
        ...
        {
            krb5_klog_syslog(LOG_ERR, _("%s: setup_gss_names failed"), whoami);
            goto out;
        }
    ...
    if (!iprop_acl_check(handle->context, client_name))
    {
        ...
        krb5_klog_syslog(LOG_NOTICE, LOG_UNAUTH, whoami, client_name,
    ...
    if (ret.ret == UPDATE_OK)
    {
        (void)snprintf(obuf, sizeof(obuf),
                       ...
                       replystr(ret.ret), (unsigned long)arg->last_sno,
    ...
    DPRINT("%s: request %s %s\n\tclprinc=`%s'\n\tsvcprinc=`%s'\n", whoami, obuf,
           ((kret == 0) ? "success" : error_message(kret)), client_name,
           service_name);

    krb5_klog_syslog(
        LOG_NOTICE, _("Request: %s, %s, %s, client=%s, service=%s, addr=%s"),
        whoami, obuf, ((kret == 0) ? "success" : error_message(kret)),
        client_name, service_name, client_addr(rqstp->rq_xprt));
...
static kdb_fullresync_result_t* ipropx_resync(uint32_t vers,
                                              struct svc_req* rqstp)
{
    ...
    {
        krb5_klog_syslog(LOG_ERR, _("%s: getclhoststr failed"), whoami);
        goto out;
    }
    ...
    {
        krb5_klog_syslog(LOG_ERR,
                         _("%s: cannot construct kdb5 util dump string too "
                           "long; out of memory"),
                         whoami);
        goto out;
    }
    ...
    switch (fret)
    {
        case -1: /* error */
            ...
            krb5_klog_syslog(LOG_ERR, _("%s: fork failed: %s"), whoami,
                             error_message(errno));
            goto out;
        ...
        case 0: /* child */
            ...
            if (p == NULL)
            {
                krb5_klog_syslog(LOG_ERR, _("%s: popen failed: %s"), whoami,
                                 error_message(errno));
                _exit(1);
            }
            pret = pclose(p);
            ...
            if (pret != 0)
            {
                ...
                krb5_klog_syslog(LOG_ERR, _("%s: pclose(popen) failed: %s"),
            ...
            if (kprop_port != NULL)
            {
                DPRINT("%s: exec `kprop -r %s -f %s -P %s %s' ...\n", whoami,
                       handle->params.realm, dump_file, kprop_port, clhost);
            ...
            else
            {
                DPRINT("%s: exec `kprop -r %s -f %s %s' ...\n", whoami,
                       handle->params.realm, dump_file, clhost);
                pret = execl(kprop, "kprop", "-r", handle->params.realm, "-f",
                             dump_file, clhost, NULL);
            }
            perror(whoami);
            krb5_klog_syslog(LOG_ERR, _("%s: exec failed: %s"), whoami,
                             error_message(errno));
            _exit(1);
        ...
        default: /* parent */
            ...
            DPRINT("%s: spawned resync process %d, client=%s, "
                   "service=%s, addr=%s\n",
                   whoami, fret, client_name, service_name,
                   client_addr(rqstp->rq_xprt));
            krb5_klog_syslog(LOG_NOTICE,
                             _("Request: %s, spawned resync process %d, "
...
kdb_fullresync_result_t* iprop_full_resync_1_svc(/* LINTED */ void* argp,
                                                 struct svc_req* rqstp)
{
    return ipropx_resync(IPROPX_VERSION_0, rqstp);
}

kdb_fullresync_result_t* iprop_full_resync_ext_1_svc(uint32_t* argp,
                                                     struct svc_req* rqstp)
{
    return ipropx_resync(*argp, rqstp);
}

static int check_iprop_rpcsec_auth(struct svc_req* rqstp)
{
    ...
    maj_stat = gss_inquire_context(&min_stat, ctx, NULL, &name, NULL, NULL,
                                   NULL, NULL, NULL);
...
fail_princ:
    if (!success)
    {
        krb5_klog_syslog(LOG_ERR, _("bad service principal %.*s%s"), (int)slen,
                         (char*)gss_str.value, sdots);
    }
...
void krb5_iprop_prog_1(struct svc_req* rqstp, SVCXPRT* transp)
{
    union
    {
    ...
    {
        krb5_klog_syslog(LOG_ERR,
                         _("authentication attempt failed: %s, RPC "
                           "authentication flavor %d"),
                         client_addr(rqstp->rq_xprt), rqstp->rq_cred.oa_flavor);
    ...
    switch (rqstp->rq_proc)
    {
        ...
        default:
            krb5_klog_syslog(LOG_ERR, _("RPC unknown request: %d (%s)"),
    ...
    {
        krb5_klog_syslog(LOG_ERR, _("RPC svc_getargs failed (%s)"), whoami);
    ...
    {
        krb5_klog_syslog(LOG_ERR, _("RPC svc_sendreply failed (%s)"), whoami);
        svcerr_systemerr(transp);
    }
    if (!svc_freeargs(transp, _xdr_argument, (caddr_t)&argument))
    {
        krb5_klog_syslog(LOG_ERR, _("RPC svc_freeargs failed (%s)"), whoami);
