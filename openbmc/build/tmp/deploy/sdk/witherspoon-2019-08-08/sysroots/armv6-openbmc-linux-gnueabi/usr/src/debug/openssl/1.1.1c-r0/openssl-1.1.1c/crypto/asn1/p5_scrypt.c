...
ASN1_SEQUENCE(SCRYPT_PARAMS) =
    {
        ...
        IMPLEMENT_ASN1_FUNCTIONS(SCRYPT_PARAMS)
            ...
            static X509_ALGOR
    * pkcs5_scrypt_set(const unsigned char* salt, size_t saltlen, size_t keylen,
                                  ...
                                  uint64_t p)
{
    ...
    {
        ASN1err(ASN1_F_PKCS5_PBE2_SET_SCRYPT, ASN1_R_INVALID_SCRYPT_PARAMETERS);
        goto err;
    }
                             ...
                             const EVP_MD* md, int en_de)
{
    ...
    if (sparam->keyLength)
    {
        ...
        if ((ASN1_INTEGER_get_uint64(&spkeylen, sparam->keyLength) == 0) ||
            (spkeylen != keylen))
        {
            EVPerr(EVP_F_PKCS5_V2_SCRYPT_KEYIVGEN, EVP_R_UNSUPPORTED_KEYLENGTH);
            goto err;
        }
    }
    ...
    {
        EVPerr(EVP_F_PKCS5_V2_SCRYPT_KEYIVGEN, EVP_R_ILLEGAL_SCRYPT_PARAMETERS);
        goto err;
    }
