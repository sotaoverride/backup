...
static const char* default_config_paths[] = {
    SYSCONFDIR "/modprobe.d", "/run/modprobe.d", "/lib/modprobe.d", NULL};
...
_printf_format_(6, 0) static void log_filep(void* data, int priority,
 ...
 */
KMOD_EXPORT void kmod_set_log_fn(
    struct kmod_ctx* ctx,
    void (*log_fn)(void* data, int priority, const char* file, int line,
                   const char* fn, const char* format, va_list args),
    const void* data)
...
int kmod_lookup_alias_from_aliases_file(struct kmod_ctx* ctx, const char* name,
                                        struct kmod_list** list)
{
    return kmod_lookup_alias_from_alias_bin(ctx, KMOD_INDEX_MODULES_ALIAS, name,
                                            list);
}
...
KMOD_EXPORT int kmod_load_resources(struct kmod_ctx* ctx)
{
    ...
    for (i = 0; i < _KMOD_INDEX_MODULES_SIZE; i++)
    {
        ...
        snprintf(path, sizeof(path), "%s/%s.bin", ctx->dirname,
                 index_files[i].fn);
        ctx->indexes[i] = index_mm_open(ctx, path, &ctx->indexes_stamp[i]);
        if (ctx->indexes[i] == NULL)
            goto fail;
    }
