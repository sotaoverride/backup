...
static void check_tree(struct rb_root* root, const char* msg)
{
    ...
    for (node = ext2fs_rb_first(root); node; node = ext2fs_rb_next(node))
    {
        ext = node_to_extent(node);
        if (ext->count == 0)
        {
        ...
        {
            fprintf(stderr, "Tree Error: start or count is crazy\n");
            fprintf(stderr, "extent: %llu -> %llu (%llu)\n", ext->start,
                    ext->start + ext->count, ext->count);
            goto err_out;
        }
                              ...
                              __u64 count)
{
    ...
    retval = ext2fs_get_mem(sizeof(struct bmap_rb_extent), &new_ext);
    if (retval)
        abort();
...
inline static void rb_free_extent(struct ext2fs_rb_private* bp,
                                  struct bmap_rb_extent* ext)
{
    if (bp->wcursor == ext)
        bp->wcursor = NULL;
                              ...
                              ext2fs_generic_bitmap dest)
{
    ...
    while (src_node)
    {
        ...
        retval = ext2fs_get_mem(sizeof(struct bmap_rb_extent), &dest_ext);
        if (retval)
            break;
...
static errcode_t rb_resize_bmap(ext2fs_generic_bitmap bmap, __u64 new_end,
                                __u64 new_real_end)
{
    struct ext2fs_rb_private* bp;
...
inline static int rb_test_bit(struct ext2fs_rb_private* bp, __u64 bit)
{
...
inline static int rb_test_bmap(ext2fs_generic_bitmap bitmap, __u64 arg)
{
    struct ext2fs_rb_private* bp;
...
static int rb_test_clear_bmap_extent(ext2fs_generic_bitmap bitmap, __u64 start,
                                     unsigned int len)
{
...
static errcode_t rb_set_bmap_range(ext2fs_generic_bitmap bitmap, __u64 start,
                                   size_t num, void* in)
{
    struct ext2fs_rb_private* bp;
    unsigned char* cp = in;
    ...
    if (first_set != -1)
    {
        rb_insert_extent(start + first_set - bitmap->start, num - first_set,
                         bp);
        check_tree(&bp->root, __func__);
    }
...
static errcode_t rb_get_bmap_range(ext2fs_generic_bitmap bitmap, __u64 start,
                                   size_t num, void* out)
{
...
static errcode_t rb_find_first_zero(ext2fs_generic_bitmap bitmap, __u64 start,
                                    __u64 end, __u64* out)
{
    struct rb_node *parent = NULL, **n;
    struct ext2fs_rb_private* bp;
...
static errcode_t rb_find_first_set(ext2fs_generic_bitmap bitmap, __u64 start,
                                   __u64 end, __u64* out)
{
    struct rb_node *parent = NULL, **n;
    struct rb_node* node;
...
static void rb_print_stats(ext2fs_generic_bitmap bitmap)
{
    ...
    fprintf(stderr, "%16llu bits set in bitmap (out of %llu)\n", size,
            bitmap->real_end - bitmap->start);
    fprintf(stderr, "%16.4lf memory / bitmap bit memory ratio (bitarray = 1)\n",
            eff);
}
#else
