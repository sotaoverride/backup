...
DEFobjCurrIf(obj) DEFobjCurrIf(prop) DEFobjCurrIf(parser) DEFobjCurrIf(ruleset)
    DEFobjCurrIf(net) DEFobjCurrIf(rsconf) DEFobjCurrIf(module)
        DEFobjCurrIf(datetime) DEFobjCurrIf(glbl)
...
/* forward definitions */
void rsyslogd_submitErrMsg(const int severity, const int iErr,
                           const uchar* msg);
...
int MarkInterval =
    20 * 60; /* interval between marks in seconds - read-only after startup */
ratelimit_t* dflt_ratelimiter =
    NULL; /* ratelimiter for submits without explicit one */
uchar* ConfFile = (uchar*)PATH_CONFFILE;
int bHaveMainQueue =
    0; /* set to 1 if the main queue - in queueing mode - is available
        * If the main queue is either not yet ready or not running in
        * queueing mode (mode DIRECT!), then this is set to 0.
        */
qqueue_t* pMsgQueue = NULL;        /* default main message queue */
prop_t* pInternalInputName = NULL; /* there is only one global inputName for all
                                      internally-generated messages */
ratelimit_t* internalMsg_ratelimiter =
    NULL;            /* ratelimiter for rsyslog-own messages */
...
static int forkRsyslog(void)
{
    ...
    close(pipefd[1]);
    dbgprintf(
        "rsyslogd: parent waiting up to 60 seconds to read startup message\n");
...
static rsRetVal rsyslogd_InitStdRatelimiters(void)
{
    ...
    CHKiRet(ratelimitNew(&internalMsg_ratelimiter, "rsyslogd",
                         "internal_messages"));
    ratelimitSetThreadSafe(internalMsg_ratelimiter);
    ratelimitSetLinuxLike(internalMsg_ratelimiter, glblIntMsgRateLimitItv,
                          glblIntMsgRateLimitBurst);
 ...
 * rgerhards, 2008-04-16: the actual initialization is now carried out by the
 * runtime
 */
static rsRetVal rsyslogd_InitGlobalClasses(void)
{
    DEFiRet;
    const char* pErrObj; /* tells us which object failed if that happens (useful
                            for troubleshooting!) */

    /* Intialize the runtime system */
    pErrObj = "rsyslog runtime"; /* set in case the runtime errors before
                                    setting an object */
...
finalize_it:
    if (iRet != RS_RET_OK)
    {
        ...
        fprintf(stderr,
                ...
                pErrObj);
        fprintf(stderr, "rsyslogd initializiation failed - global classes "
                        "could not be initialized.\n"
                        "Did you do a \"make install\"?\n"
                        "Suggested action: run rsyslogd with -d -n options to "
                        "see what exactly "
                        "fails.\n");
    }
 ...
 * This is done as a first stage and totally in parallel to any other worker
 * active in the system. So it helps us keep up the overall concurrency level.
...
static rsRetVal preprocessBatch(batch_t* pBatch, int* pbShutdownImmediate)
{
    ...
    for (i = 0; i < pBatch->nElem && !*pbShutdownImmediate; i++)
    {
        ...
        {
            DBGPRINTF("msgConsumer: UDP ACL must be checked for message "
                      "(hostname-based)\n");
            ...
            if (!bIsPermitted)
            {
                DBGPRINTF("Message from '%s' discarded, not a permitted sender "
                          "host\n",
                          propGetSzStr(fqdn));
 ...
 * for the main queue.
 */
static rsRetVal msgConsumer(void __attribute__((unused)) * notNeeded,
                            batch_t* pBatch, wti_t* pWti)
{
    DEFiRet;
    assert(pBatch != NULL);
 ...
 * the time being (remember that we want to restructure config processing at
 * large!). rgerhards, 2009-10-27
 */
                         ...
                         struct nvlst* lst)
{
    ...
    CHKiRet_Hdlr(qqueueConstruct(ppQueue, ourConf->globals.mainQ.MainMsgQueType,
                                 ...
                                 ourConf->globals.mainQ.iMainMsgQueueSize,
                                 msgConsumer))
    {
...
#define setQPROP(func, directive, data)                                        \
    ...
    {                                                                          \
        LogError(0, NO_ERRCODE,                                                \
                 "Invalid " #directive ", error %d. Ignored, "                 \
                 "running with default setting",                               \
                 iRet);                                                        \
    }
#define setQPROPstr(func, directive, data)                                     \
    CHKiRet_Hdlr(                                                              \
        func(*ppQueue, data, (data == NULL) ? 0 : strlen((char*)data)))        \
    {                                                                          \
        LogError(0, NO_ERRCODE,                                                \
                 "Invalid " #directive ", error %d. Ignored, "                 \
                 "running with default setting",                               \
        ...
        if (ourConf->globals.mainQ.pszMainMsgQFName != NULL)
        {
            ...
            for (qfn = queuefilenames; qfn != NULL; qfn = qfn->next)
            {
                ...
                if (!ustrcmp(qfn->name,
                             ourConf->globals.mainQ.pszMainMsgQFName))
                {
                    snprintf((char*)qfrenamebuf, sizeof(qfrenamebuf),
                             "%d-%s-%s", ++qfn_renamenum,
                             ourConf->globals.mainQ.pszMainMsgQFName,
                             (pszQueueName == NULL) ? "NONAME"
                                                    : (char*)pszQueueName);
        ...
        setQPROP(qqueueSettoEnq, "$MainMsgQueueTimeoutEnqueue",
                 ourConf->globals.mainQ.iMainMsgQtoEnq);
 ...
 */
static rsRetVal logmsgInternalSubmit(const int iErr, const syslog_pri_t pri,
                                     const size_t lenMsg,
                                     const char* __restrict__ const msg,
                                     int flags)
{
    uchar pszTag[33];
    smsg_t* pMsg;
                        ...
                        const uchar* const msg, int flags)
{
    ...
    unsigned i;
    char* bufModMsg =
        NULL; /* buffer for modified message, should we need to modify */
    DEFiRet;
     ...
     * suggested by HKS as a way to help people troubleshoot rsyslog
     * configuration (by running it interactively. This makes an awful lot of
     * sense, so I add it here. -- rgerhards, 2008-07-28 Note that error
     * messages can not be disabled during a config verify. This permits us to
     * process unmodified config files which otherwise contain a supressor
     * statement.
     */
    int emit_to_stderr =
        (ourConf == NULL) ? 1 : ourConf->globals.bErrMsgToStderr;
...
rsRetVal submitMsg2(smsg_t* pMsg)
{
    ...
    if (getRawMsgLen(pMsg) > glblGetMaxLine())
    {
        ...
        else if (glblGetOversizeMsgInputMode() ==
                 glblOversizeMsgInputMode_Truncate)
        {
        ...
        else
        {
             ...
             * to use as-is.
             */
            assert(glblGetOversizeMsgInputMode() ==
                   glblOversizeMsgInputMode_Accept);
        }
    }
 ...
 * to prevent dependencies between the options. That, in turn, means we need to
 * have something that is capable of buffering options and there values. The
 * follwing functions handle that. rgerhards, 2008-04-04
 */
...
/* remove option buffer from top of list, return values and destruct buffer
 * itself. returns RS_RET_END_OF_LINKEDLIST when no more options are present.
 ...
 * may introduce subtle security issues: if we are in a jail, one may break out
 * of it via these descriptors. But if I close them earlier, error messages will
 * (once again) not be emitted to the user that starts the daemon. Given that
 * the risk of a break-in is very low in the startup phase, we decide it is more
 * important to emit error messages.
 */
static void initAll(int argc, char** argv)
{
     ...
     * work, just see what we should do. This relieves us from certain anomalies
     * and we can process the parameters down below in the correct order. For
     * example, we must know the value of -M before we can do the init, but at
     * the same time we need to have the base classes init before we can process
     * most of the options. Now, with the split of functionality, this is no
     * longer a problem. Thanks to varmofekoj for suggesting this algo. Note:
     * where we just need to set some flags and can do so without knowledge of
     * other options, we do this during the inital option processing. rgerhards,
     * 2008-04-04
     */
#if defined(_AIX)
    ...
    while ((ch = getopt(argc, argv, "46ACDdf:hi:l:M:nN:qQs:S:T:u:vwx")) != EOF)
    {
        ...
        switch ((char)ch)
        {
            ...
            case 's':
            case 'S': /* Source IP for local client to be used on multihomed
                         host */
            ...
            case 'R': /* This option is a no-op for AIX */
                break;
#endif
            case 'd': /* debug - must be handled now, so that debug is active
                         during init! */
                debugging_on = 1;
                Debug = 1;
                yydebug = 1;
            ...
            case 'D': /* BISON debug */
                yydebug = 1;
                break;
            case 'M': /* default module load path -- this MUST be carried out
                         immediately! */
    ...
    DBGPRINTF("rsyslogd %s startup, module path '%s', cwd:%s\n", VERSION,
              glblModPath == NULL ? "" : (char*)glblModPath,
              getcwd(cwdbuf, sizeof(cwdbuf)));

    /* we are done with the initial option parsing and processing. Now we init
     * the system. */
    ...
    {
        fprintf(stderr,
                "fatal error: could not initialize errbuf object (error code "
                "%d).\n",
                iRet);
     ...
     * 2008-04-04 But we need to have imInternal up first!
     */
    queryLocalHostname();
    ...
    if (getenv("TZ") == NULL)
    {
        const char* const tz = (access("/etc/localtime", R_OK) == 0)
                                   ? "TZ=/etc/localtime"
                                   : "TZ=UTC";
        putenv((char*)tz);
        if (emitTZWarning)
        ...
        {
            dbgprintf("environment variable TZ is not set, auto correcting "
                      "this to %s\n",
                      tz);
        }
    }

    /* END core initializations - we now come back to carrying out command line
     * options*/
    ...
    while ((iRet = bufOptRemove(&ch, &arg)) == RS_RET_OK)
    {
        ...
        switch ((char)ch)
        {
            case '4':
                fprintf(stderr,
                        "rsyslogd: the -4 command line option will go away "
                        "soon.\nPlease use the "
            ...
            case '6':
                fprintf(stderr,
                        "rsyslogd: the -6 command line option will go away "
                        "soon.\nPlease use the "
            ...
            case 'A':
                fprintf(stderr,
                        ...
                        "soon.\n"
                        "Please use the omfwd parameter \"upd.sendToAll\" "
                        "instead.\n");
                send_to_all++;
                break;
            case 'S': /* Source IP for local client to be used on multihomed
                ...
                fprintf(stderr,
                        ...
                        "soon.\n"
                        "Please use the omrelp parameter \"localClientIP\" "
                        "instead.\n");
            ...
            case 'l':
                fprintf(stderr,
                        "rsyslogd: the -l command line option will go away "
            ...
            case 'q': /* add hostname if DNS resolving has failed */
                fprintf(stderr,
                        "rsyslogd: the -q command line option will go away "
                        "soon.\nPlease use the "
            ...
            case 's':
                ...
                break;
            case 'T': /* chroot() immediately at program startup, but only for
                         testing, NOT security yet */
                if (arg == NULL)
                {
            ...
            case 'w': /* disable disallowed host warnigs */
                fprintf(
                    stderr,
                    "rsyslogd: the -w command line option will go away "
            ...
            case 'x': /* disable dns for remote messages */
                fprintf(stderr,
                        "rsyslogd: the -x command line option will go away "
    ...
    if (iConfigVerify)
    {
        doFork = 0;
        fprintf(stderr,
                "rsyslogd: version %s, config validation run (level %d), "
                "master config %s\n",
    ...
    if (hadErrMsgs())
    {
        if (loadConf->globals.bAbortOnUncleanConfig)
        {
            fprintf(stderr,
                    ...
                    "config is not clean.\n"
                    "Check error log for details, fix errors and restart. As a "
                    "last\n"
                    "resort, you may want to use "
                    "global(AbortOnUncleanConfig=\"off\") \n"
    ...
    if (ourConf->globals.bLogStatusMsgs)
    {
        ...
        snprintf(bufStartUpMsg, sizeof(bufStartUpMsg),
                 ...
                 (int)glblGetOurPid());
        logmsgInternal(NO_ERRCODE, LOG_SYSLOG | LOG_INFO, (uchar*)bufStartUpMsg,
                       0);
    }
    ...
    DBGPRINTF("rsyslogd: initialization completed, transitioning to regular "
              "run mode\n");
...
finalize_it:
    ...
    else if (iRet != RS_RET_OK)
    {
        fprintf(stderr,
                ...
                "or try https://www.rsyslog.com/e/%d to learn what that number "
                "means)\n",
                iRet, iRet * -1);
        exit(1);
    }
...
void processImInternal(void)
{
    ...
    {
        rsRetVal localRet =
            ratelimitMsg(internalMsg_ratelimiter, pMsg, &repMsg);
        if (repMsg != NULL)
        {
...
rsRetVal parseAndSubmitMessage(const uchar* const hname,
                               ...
                               const uchar* const msg, const int len,
 ...
 * record detected inside the mainloop and then this function is called to do
 * the real work. -- rgerhards, 2008-10-22 Note: there is a VERY slim chance of
 * a data race when the hostname is reset. We prefer to take this risk rather
 * than sync all accesses, because to the best of my analysis it can not really
 * hurt (the actual property is reference-counted) but the sync would require
 * some extra CPU for *each* message processed. rgerhards, 2012-04-11
 */
static void doHUP(void)
{
    ...
    if (ourConf->globals.bLogStatusMsgs)
    {
        snprintf(buf, sizeof(buf),
                 " [origin software=\"rsyslogd\" "
                 "swVersion=\"" VERSION
                 "\" x-pid=\"%d\" x-info=\"https://www.rsyslog.com\"] rsyslogd "
                 "was HUPed",
                 (int)glblGetOurPid());
        errno = 0;
...
/* rsyslogdDoDie() is a signal handler. If called, it sets the bFinished
 * variable to indicate the program should terminate. However, it does not
 * terminate it itself, because that causes issues with multi-threading. The
 * actual termination is then done on the main thread. This solution might
 * introduce a minimal delay, but it is much cleaner than the approach of doing
 * everything inside the signal handler. rgerhards, 2005-10-26 Note:
...
/* This is the main processing loop. It is called after successful
 * initialization. When it returns, the syslogd terminates. Its sole function is
 * to provide some housekeeping things. The real work is done by the other
 * threads spawned.
 */
...
static void rsyslogd_destructAllActions(void)
{
    ...
    bHaveMainQueue =
        0; /* flag that internal messages need to be temporarily stored */
}
...
static void deinitAll(void)
{
    ...
    /* IMPORTANT: we should close the inputs first, and THEN send our
     * termination message. If we do it the other way around, logmsgInternal()
     * may block on a full queue and the inputs still fill up that queue.
     * Depending on the scheduling order, we may end up with logmsgInternal
     * being held for a quite long time. When the inputs are terminated first,
     * that should not happen because the queue is drained in parallel. The
     * situation could only become an issue with extremely long running actions
     * in a queue full environment. However, such actions are at least
     * considered poorly written, if not outright wrong. So we do not care about
     * this very remote problem. rgerhards, 2008-01-11
     */
    ...
    processImInternal(); /* make sure not-yet written internal messages are
                            processed */
    /* we sleep a couple of ms to give the queue a chance to pick up the late
     * messages (including exit message); otherwise we have seen cases where the
     * message did not make it to log files, even on idle systems.
     */
    srSleep(0, 50);
    ...
    /* Free ressources and close connections. This includes flushing any
     * remaining repeated msgs.
     */
    DBGPRINTF("Terminating outputs...\n");
    rsyslogd_destructAllActions();

    DBGPRINTF("all primary multi-thread sources have been terminated - now "
              "doing aux cleanup...\n");
    ...
    /* the following line cleans up CfSysLineHandlers that were not based on
     * loadable modules. As such, they are not yet cleared.  */
    unregCfSysLineHdlrs();

    /* this is the last spot where this can be done - below output modules are
     * unloaded! */
    ...
    rsrtExit(); /* runtime MUST always be deinitialized LAST (except for debug
                   system) */
...
/* This is the main entry point into rsyslogd. This must be a function in its
 * own right in order to intialize the debug system in a portable way (otherwise
 * we would need to have a statement before variable definitions. rgerhards,
 * 20080-01-28
 */
