...
static const struct range high_key_blocks[] = {
    {KEY_OK, BTN_DPAD_UP}, {KEY_ALS_TOGGLE, BTN_TRIGGER_HAPPY}};
...
static void get_cap_mask(sd_device* pdev, const char* attr,
                         unsigned long* bitmask, size_t bitmask_size, bool test)
{
    const char* v;
    char text[4096];
    unsigned i;
    ...
    while ((word = strrchr(text, ' ')) != NULL)
    {
        ...
        if (i < bitmask_size / sizeof(unsigned long))
            ...
            log_device_debug(
                pdev, "Ignoring %s block %lX which is larger than maximum size",
                attr, val);
        *word = '\0';
        ++i;
    }
...
/* pointer devices */
static bool test_pointers(sd_device* dev, const unsigned long* bitmask_ev,
                          const unsigned long* bitmask_abs,
                          const unsigned long* bitmask_key,
                          const unsigned long* bitmask_rel,
                          const unsigned long* bitmask_props, bool test)
{
    int button, axis;
    ...
    finger_but_no_pen = test_bit(BTN_TOOL_FINGER, bitmask_key) &&
                        !test_bit(BTN_TOOL_PEN, bitmask_key);
    for (button = BTN_MOUSE; button < BTN_JOYSTICK && !has_mouse_button;
         button++)
        has_mouse_button = test_bit(button, bitmask_key);
    has_rel_coordinates = test_bit(EV_REL, bitmask_ev) &&
    ...
    {
        for (button = BTN_JOYSTICK;
             button < BTN_DIGI && !has_joystick_axes_or_buttons; button++)
        ...
        for (button = BTN_TRIGGER_HAPPY1;
             ...
             button++)
            has_joystick_axes_or_buttons = test_bit(button, bitmask_key);
        for (button = BTN_DPAD_UP;
             button <= BTN_DPAD_RIGHT && !has_joystick_axes_or_buttons;
             button++)
            has_joystick_axes_or_buttons = test_bit(button, bitmask_key);
    }
    for (axis = ABS_RX; axis < ABS_PRESSURE && !has_joystick_axes_or_buttons;
         axis++)
    ...
    return is_tablet || is_mouse || is_touchpad || is_touchscreen ||
           is_joystick || is_pointing_stick;
}

/* key like devices */
static bool test_key(sd_device* dev, const unsigned long* bitmask_ev,
                     const unsigned long* bitmask_key, bool test)
{
    unsigned i;
    unsigned long found;
    unsigned long mask;
    ...
    for (i = 0; i < BTN_MISC / BITS_PER_LONG; ++i)
    {
        found |= bitmask_key[i];
        log_device_debug(
            dev, "test_key: checking bit block %lu for any keys; found=%i",
    ...
    if (!found)
    {
        unsigned block;
        for (block = 0;
             block < (sizeof(high_key_blocks) / sizeof(struct range)); ++block)
        {
            for (i = high_key_blocks[block].start;
                 i < high_key_blocks[block].end; ++i)
            {
...
static int builtin_input_id(sd_device* dev, int argc, char* argv[], bool test)
{
    ...
    for (pdev = dev; pdev;)
    {
        ...
        if (sd_device_get_parent_with_subsystem_devtype(pdev, "input", NULL,
                                                        &pdev) >= 0)
            continue;
    ...
    if (pdev)
    {
        ...
        get_cap_mask(pdev, "capabilities/ev", bitmask_ev, sizeof(bitmask_ev),
                     test);
        get_cap_mask(pdev, "capabilities/abs", bitmask_abs, sizeof(bitmask_abs),
                     test);
        get_cap_mask(pdev, "capabilities/rel", bitmask_rel, sizeof(bitmask_rel),
                     test);
        get_cap_mask(pdev, "capabilities/key", bitmask_key, sizeof(bitmask_key),
                     test);
        get_cap_mask(pdev, "properties", bitmask_props, sizeof(bitmask_props),
                     test);
        is_pointer = test_pointers(dev, bitmask_ev, bitmask_abs, bitmask_key,
