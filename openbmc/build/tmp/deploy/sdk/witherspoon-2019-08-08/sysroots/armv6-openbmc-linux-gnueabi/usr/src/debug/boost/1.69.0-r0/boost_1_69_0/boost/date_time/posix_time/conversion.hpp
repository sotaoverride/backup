...
inline ptime from_time_t(std::time_t t)
{
...
inline std::tm to_tm(const boost::posix_time::ptime& t)
{
...
//! Convert a time_duration to a tm structure truncating any fractional seconds
//! and zeroing fields for date components
inline std::tm to_tm(const boost::posix_time::time_duration& td)
{
    std::tm timetm;
...
inline ptime ptime_from_tm(const std::tm& timetm)
{
    boost::gregorian::date d = boost::gregorian::date_from_tm(timetm);
    return ptime(d,
                 time_duration(timetm.tm_hour, timetm.tm_min, timetm.tm_sec));
}
...
inline TimeT from_ftime(const FileTimeT& ft)
{
