...
#define __libc_cleanup_end(DOIT)                                               \
    if (_avail)                                                                \
    {                                                                          \
        __libc_ptf_call_always(_pthread_cleanup_pop_restore,                   \
                               (&_buffer, DOIT));                              \
    }                                                                          \
    else if (DOIT)                                                             \
...
#define __libc_cleanup_push(fct, arg)                                          \
    ...
    {                                                                          \
        struct __pthread_cleanup_frame __clframe __attribute__(                \
            (__cleanup__(__libc_cleanup_routine))) = {                         \
...
extern int __pthread_setspecific(pthread_key_t __key, const void* __pointer);
...
#ifdef weak_extern
weak_extern(__pthread_mutex_init) weak_extern(
    __pthread_mutex_destroy) weak_extern(__pthread_mutex_lock)
    weak_extern(__pthread_mutex_trylock) weak_extern(
        __pthread_mutex_unlock) weak_extern(__pthread_mutexattr_init)
        weak_extern(__pthread_mutexattr_destroy) weak_extern(
            __pthread_mutexattr_settype) weak_extern(__pthread_rwlock_init)
            weak_extern(__pthread_rwlock_destroy) weak_extern(
                __pthread_rwlock_rdlock) weak_extern(__pthread_rwlock_tryrdlock)
                weak_extern(__pthread_rwlock_wrlock) weak_extern(
                    __pthread_rwlock_trywrlock)
                    weak_extern(__pthread_rwlock_unlock) weak_extern(
                        __pthread_key_create) weak_extern(__pthread_setspecific)
                        weak_extern(__pthread_getspecific) weak_extern(
                            __pthread_once) weak_extern(__pthread_initialize)
