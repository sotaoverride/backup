...
/* pointer                                                           */
static void cf2_hintmask_read(CF2_HintMask hintmask, CF2_Buffer charstring,
                              size_t bitCount)
{
    size_t i;
...
static void cf2_doStems(const CF2_Font font, CF2_Stack opStack,
                        CF2_ArrStack stemHintArray, CF2_Fixed* width,
                        FT_Bool* haveWidth, CF2_Fixed hintOffset)
{
    CF2_UInt i;
    ...
    for (i = hasWidthArg ? 1 : 0; i < count; i += 2)
    {
        ...
        stemhint.min = position =
            ADD_INT32(position, cf2_stack_getReal(opStack, i));
        stemhint.max = position =
            ADD_INT32(position, cf2_stack_getReal(opStack, i + 1));
...
static void cf2_doFlex(CF2_Stack opStack, CF2_Fixed* curX, CF2_Fixed* curY,
                       CF2_GlyphPath glyphPath, const FT_Bool* readFromStack,
                       FT_Bool doConditionalLastRead)
{
    ...
    for (i = 0; i < top; i++)
    {
        ...
        if (readFromStack[i])
            vals[i + 2] =
                ADD_INT32(vals[i + 2], cf2_stack_getReal(opStack, idx++));
    }
    ...
    {
        if (readFromStack[10])
            vals[12] = ADD_INT32(vals[10], cf2_stack_getReal(opStack, idx++));
        else
            vals[12] = *curX;

        if (readFromStack[11])
            vals[13] = ADD_INT32(vals[11], cf2_stack_getReal(opStack, idx));
        else
            vals[13] = *curY;
    }

    for (j = 0; j < 2; j++)
        cf2_glyphpath_curveTo(glyphPath, vals[j * 6 + 2], vals[j * 6 + 3],
                              vals[j * 6 + 4], vals[j * 6 + 5], vals[j * 6 + 6],
                              vals[j * 6 + 7]);
...
static void cf2_doBlend(const CFF_Blend blend, CF2_Stack opStack,
                        CF2_UInt numBlends)
{
    CF2_UInt delta;
    CF2_UInt base;
 ...
 */
FT_LOCAL_DEF(void)
cf2_interpT2CharString(CF2_Font font, CF2_Buffer buf,
                       CF2_OutlineCallbacks callbacks,
                       const FT_Vector* translation, FT_Bool doingSeac,
                       CF2_Fixed curX, CF2_Fixed curY, CF2_Fixed* width)
{
...
#define PS_STORAGE_SIZE 3
    ...
    cf2_arrstack_init(&subrStack, memory, error, sizeof(CF2_BufferRec));
    cf2_arrstack_init(&hStemHintArray, memory, error, sizeof(CF2_StemHintRec));
    ...
    cf2_glyphpath_init(&glyphPath, font, callbacks, scaleY,
                       /* hShift, */
                       &hStemHintArray, &vStemHintArray, &hintMask, hintOriginY,
                       &font->blues, translation);
    ...
    *width = cf2_getDefaultWidthX(decoder);
    ...
    stackSize =
        font->isCFF2 ? cf2_getMaxstack(decoder) : CF2_OPERAND_STACK_SIZE;
    ...
    *charstring = *buf;  /* structure copy     */
    ...
    {
        if (font->isT1)
            FT_ASSERT(known_othersubr_result_cnt == 0 || result_cnt == 0);
        ...
        if (cf2_buf_isEnd(charstring))
        {
            ...
            if (charstringIndex)
                ...
                op1 =
                    cf2_cmdENDCHAR; /* end of buffer for top level charstring */
        }
        else
        {
            ...
            if ((op1 == cf2_cmdRETURN || op1 == cf2_cmdENDCHAR) && font->isCFF2)
        ...
        if (font->isT1)
        {
            if (!initial_map_ready &&
                !(op1 == cf2_cmdHSTEM || op1 == cf2_cmdVSTEM ||
                  op1 == cf2_cmdHSBW || op1 == cf2_cmdCALLSUBR ||
                  op1 == cf2_cmdRETURN || op1 == cf2_cmdESC ||
                  op1 == cf2_cmdENDCHAR || op1 >= 32 /* Numbers */))
            {
            ...
            if (result_cnt > 0 &&
                !(op1 == cf2_cmdCALLSUBR || op1 == cf2_cmdRETURN ||
                  op1 == cf2_cmdESC || op1 >= 32 /* Numbers */))
            {
        ...
        switch (op1)
        {
            ...
            case cf2_cmdBLEND:
            {
                ...
                if (font->cffload->blend_check_vector(
                        &font->blend, font->vsindex, font->lenNDV, font->NDV))
                {
                    lastError = font->cffload->blend_build_vector(
                        &font->blend, font->vsindex, font->lenNDV, font->NDV);
                    if (lastError)
                        goto exit;
                }
            ...
            case cf2_cmdHSTEM:
                FT_TRACE4(
                    ("%s\n", op1 == cf2_cmdHSTEMHM ? " hstemhm" : " hstem"));
                ...
                cf2_doStems(font, opStack, &hStemHintArray, width, &haveWidth,
                            font->isT1 ? decoder->builder.left_bearing->y : 0);
            ...
            case cf2_cmdVSTEM:
                FT_TRACE4(
                    ("%s\n", op1 == cf2_cmdVSTEMHM ? " vstemhm" : " vstem"));
                ...
                /* add left-sidebearing correction in Type 1 mode */
                cf2_doStems(font, opStack, &vStemHintArray, width, &haveWidth,
                            font->isT1 ? decoder->builder.left_bearing->x : 0);
            ...
            case cf2_cmdVMOVETO:
                ...
                if (cf2_stack_count(opStack) > 1 && !haveWidth)
                    *width =
                        ADD_INT32(cf2_stack_getReal(opStack, 0), nominalWidthX);
            ...
            case cf2_cmdRLINETO:
            {
                ...
                {
                    curX = ADD_INT32(curX, cf2_stack_getReal(opStack, idx + 0));
                    curY = ADD_INT32(curY, cf2_stack_getReal(opStack, idx + 1));
            ...
            case cf2_cmdCALLSUBR:
            {
                ...
                charstring = (CF2_Buffer)cf2_arrstack_getPointer(
                    &subrStack, (size_t)charstringIndex + 1);
                ...
                {
                    size_t* val =
                        ft_hash_num_lookup(subrNum, decoder->locals_hash);
                ...
                switch (op1)
                {
                    case cf2_cmdCALLGSUBR:
                        ...
                        if (cf2_initGlobalRegionBuffer(decoder, subrNum,
                                                       charstring))
                        {
                            lastError = FT_THROW(Invalid_Glyph_Format);
                    ...
                    default:
                        ...
                        if (cf2_initLocalRegionBuffer(decoder, subrNum,
                                                      charstring))
                        {
                            lastError = FT_THROW(Invalid_Glyph_Format);
                            goto exit; /* subroutine lookup or stream error */
            ...
            case cf2_cmdRETURN:
                ...
                charstring = (CF2_Buffer)cf2_arrstack_getPointer(
                    &subrStack, (CF2_UInt)--charstringIndex);
                continue; /* do not clear the stack */
                ...
                switch (op2)
                {
                    case cf2_escHFLEX:
                    {
                        static const FT_Bool readFromStack[12] = {
                            TRUE /* dx1 */,  FALSE /* dy1 */, TRUE /* dx2 */,
                            TRUE /* dy2 */,  TRUE /* dx3 */,  FALSE /* dy3 */,
                            TRUE /* dx4 */,  FALSE /* dy4 */, TRUE /* dx5 */,
                            FALSE /* dy5 */, TRUE /* dx6 */,  FALSE /* dy6 */
                        };

                        FT_TRACE4((" hflex\n"));

                        cf2_doFlex(opStack, &curX, &curY, &glyphPath,
                                   readFromStack,
                    ...
                    {
                        static const FT_Bool readFromStack[12] = {
                            TRUE /* dx1 */, TRUE /* dy1 */, TRUE /* dx2 */,
                            TRUE /* dy2 */, TRUE /* dx3 */, TRUE /* dy3 */,
                            TRUE /* dx4 */, TRUE /* dy4 */, TRUE /* dx5 */,
                            TRUE /* dy5 */, TRUE /* dx6 */, TRUE /* dy6 */
                        };
                    ...
                    default:
                    {
                        if (font->isCFF2 || op2 >= cf2_escRESERVED_38)
                            FT_TRACE4((" unknown op (12, %d)\n", op2));
                        else if (font->isT1 && result_cnt > 0 &&
                                 op2 != cf2_escPOP)
                        {
                            /* all operands have been transferred by previous
                             * pops */
                            result_cnt = 0;
                        }
                        else
                        {
                            ...
                            switch (op2)
                            {
                                ...
                                case cf2_escDOTSECTION:
                                    /* something about `flip type of locking' --
                                     * ignore it */
                                ...
                                case cf2_escHSTEM3:
                                    /*
                                     * Type 1:                          Type 2:
                                     *   x0 dx0 x1 dx1 x2 dx2 vstem3      x dx
                                     * {dxa dxb}* vstem y0 dy0 y1 dy1 y2 dy2
                                     * hstem3      y dy {dya dyb}* hstem
                                ...
                                case cf2_escSEAC:
                                    ...
                                    else
                                    {
                                        ...
                                        /* `glyph_names' is set to 0 for CID
                                         * fonts which do */
                                        /* not include an encoding.  How can we
                                         * deal with   */
                                        /* these? */
#ifdef FT_CONFIG_OPTION_INCREMENTAL
...
#ifdef FT_CONFIG_OPTION_INCREMENTAL
                                        if (face->root.internal
                                                ->incremental_interface)
                                        {
                                            /* the caller must handle the font
                                             * encoding also */
                                            bchar_index = bchar;
                                            achar_index = achar;
                                        }
                                        ...
                                        /* if we are trying to load a composite
                                         * glyph, */
                                        /* do not load the accent character and
                                         * return */
                                        ...
                                        if (decoder->builder.no_recurse)
                                        {
                                            FT_GlyphSlot glyph =
                                                (FT_GlyphSlot)
                                                    decoder->builder.glyph;
                                            FT_GlyphLoader loader =
                                                glyph->internal->loader;
                                            FT_SubGlyph subg;

                                            /* reallocate subglyph array if
                                             * necessary */
                                            error2 =
                                            ...
                                            subg->flags =
                                                FT_SUBGLYPH_FLAG_ARGS_ARE_XY_VALUES;
                                            subg->arg1 =
                                                (FT_Int)FIXED_TO_INT(adx - asb);
                                            subg->arg2 =
                                                (FT_Int)FIXED_TO_INT(ady);
                                        ...
                                        FT_GlyphLoader_Prepare(
                                            decoder->builder.loader);

                                        error2 = cf2_getT1SeacComponent(
                                            decoder, (FT_UInt)bchar_index,
                                            &component);
                                        if (error2)
                                        {
                                            lastError =
                                                error2; /* pass FreeType error
                                                           through */
                                            goto exit;
                                        }
                                        cf2_interpT2CharString(
                                            font, &component, callbacks,
                                            translation, TRUE, 0, 0,
                                            &dummyWidth);
                                        cf2_freeT1SeacComponent(decoder,
                                                                &component);

                                        /* save the left bearing and width of
                                         * the base       */
                                        /* character as they will be erased by
                                         * the next load */
                                ...
                                case cf2_escSBW:
                                    ...
                                    else
                                    {
                                        ...
                                        builder->left_bearing->x = ADD_INT32(
                                            builder->left_bearing->x, lsb_x);
                                        builder->left_bearing->y = ADD_INT32(
                                            builder->left_bearing->y, lsb_y);

                                        haveWidth = TRUE;

                                        /* the `metrics_only' indicates that we
                                         * only want */
                                        /* to compute the glyph's metrics (lsb +
                                         * advance  */
                                        /* width), not load the  rest of it; so
                                         * exit      */
                                        /* immediately */
                                ...
                                case cf2_escADD:
                                {
                                    ...
                                    cf2_stack_pushFixed(
                                        opStack, ADD_INT32(summand1, summand2));
                                }
                                    continue; /* do not clear the stack */
                                ...
                                case cf2_escDIV:
                                {
                                    ...
                                    cf2_stack_pushFixed(
                                        opStack, FT_DivFix(dividend, divisor));
                                }
                                    continue; /* do not clear the stack */
                                ...
                                case cf2_escCALLOTHERSUBR:
                                    ...
                                    else
                                    {
                                        ...
                                        /********************************************************
                                         *
                                         * remove all operands to callothersubr
                                         * from the stack
                                         *
                                         * for handled othersubrs, where we know
                                         * the number of arguments, we increase
                                         * the stack by the value of
                                         * known_othersubr_result_cnt
                                         *
                                         * for unhandled othersubrs the
                                         * following pops adjust the stack
                                         ...
                                         * <whatever>'   */
                                        /* might not be correct; an othersubr
                                         * expects a        */
                                        /* certain number of operands on the
                                         * PostScript stack  */
                                        /* (as opposed to the T1 stack) but it
                                         * doesn't have to */
                                        /* put them there by itself; previous
                                         * othersubrs might */
                                        /* have left the operands there if they
                                         * were not       */
                                        /* followed by an appropriate number of
                                         * pops           */
                                        /*                                                     */
                                        /* On the other hand, Adobe Reader 7.0.8
                                         * for Linux     */
                                        /* doesn't accept a font that contains
                                         * charstrings     */
                                        /* like */
                                        /*                                                     */
                                        ...
                                        switch (subr_no)
                                        {
                                            ...
                                            case 2: /* add flex vectors */
                                            {
                                                ...
                                                /* note that we should not add a
                                                 * point for      */
                                                /* index 0; this will move our
                                                 * current position */
                                                /* to the flex point without
                                                 * adding any point   */
                                                /* to the outline */
                                                idx =
                                                    decoder->num_flex_vectors++;
                                                if (idx > 0 && idx < 7)
                                                {
                                                    /* in malformed fonts it is
                                                     * possible to have    */
                                                    /* other opcodes in the
                                                     * middle of a flex (which
                                                     */
                                                    /* don't increase
                                                     * `num_flex_vectors'); we
                                                     * thus  */
                                                    /* have to check whether we
                                                     * can add a point     */
                                            ...
                                            case 3: /* change hints */
                                                ...
                                                if (initial_map_ready)
                                                {
                                                    /* do not clear hints if
                                                     * initial hintmap */
                                                    /* is not ready - we need to
                                                     * collate all */
                                                    cf2_arrstack_clear(
                                                        &vStemHintArray);
                                                    cf2_arrstack_clear(
                                            ...
                                            case 18: /* multiple masters */
                                            {
                                                 ...
                                                 */
                                                /*                                                   */
                                                /* but we only have a0, a1-a0,
                                                 * a2-a0, ..., ak-a0.    */
                                                /*                                                   */
                                                /* However, given that w0 + w1 +
                                                 * ... + wk == 1, we   */
                                                 ...
                                                 * written in this `compact' */
                                                /* form. */
                                                /*                                                   */
                                                delta = opIdx + num_points;
                                                values = opIdx;
                                                for (nn = 0; nn < num_points;
                                                     nn++)
                                                {
                                                    CF2_Fixed tmp =
                                                        cf2_stack_getReal(
                    ...
                    case 26:
                      ...
                      break;
                                            ...
                                            case 27:
                                                /* <res1> <res2> <val1> <val2> 4
                                                 * 27 callothersubr pop */
                                                /* ==> push <res1> onto T1 stack
                                                 * if <val1> <= <val2>, */
                                                /*     otherwise push <res2> */
                                                {
                                                    CF2_F16Dot16 arg1;
                                                    ...
                                                    cond2 = cf2_stack_popFixed(
                                                        opStack);
                                                    cond1 = cf2_stack_popFixed(
                                                        opStack);
                                                    arg2 = cf2_stack_popFixed(
                                                        opStack);
                                            ...
                                            case 28:
                                                ...
                                                /*     onto stack */
                                                {
                                                    ...
                                                    /* only use the lower 16
                                                     * bits of `random'  */
                                                    /* to generate a number in
                                                     * the range (0;1] */
                                                    r = (CF2_F16Dot16)(
                                                        (decoder
                                                             ->current_subfont
                                ...
                                case cf2_escMUL:
                                {
                                    ...
                                    cf2_stack_pushFixed(
                                        opStack, FT_MulFix(factor1, factor2));
                                }
                                    continue; /* do not clear the stack */
                                ...
                                case cf2_escSETCURRENTPT:
                                    ...
                                    else
                                    {
                                        ...
                                        if (!initial_map_ready)
                                            ...
                                            /* From the T1 specification,
                                             * section 6.4:            */
                                            /*                                                    */
                                            /*   The setcurrentpoint command is
                                             * used only in      */
                                             ...
                                             * Ghostscript and Adobe     */
                                            /* Distiller handle this situation
                                             * by silently        */
            ...
            case cf2_cmdENDCHAR:
                ...
                if (!font->isCFF2 && !font->isT1 &&
                    cf2_stack_count(opStack) > 1)
                {
                    ...
                    cf2_interpT2CharString(font, &component, callbacks,
                                           translation, TRUE, curX, curY,
                                           &dummyWidth);
            ...
            case cf2_cmdHINTMASK:
                ...
                FT_TRACE4(
                    ("%s", op1 == cf2_cmdCNTRMASK ? " cntrmask" : " hintmask"));
                ...
                if (op1 == cf2_cmdHINTMASK)
                {
                    ...
                    cf2_hintmask_read(&hintMask, charstring,
                ...
                else
                {
                    ...
                    cf2_hintmap_init(&counterHintMap, font,
                                     &glyphPath.initialHintMap,
                                     &glyphPath.hintMoves, scaleY);
                    cf2_hintmask_init(&counterMask, error);

                    cf2_hintmask_read(&counterMask, charstring,
                                      cf2_arrstack_size(&hStemHintArray) +
                                          cf2_arrstack_size(&vStemHintArray));
                    cf2_hintmap_build(&counterHintMap, &hStemHintArray,
                                      &vStemHintArray, &counterMask, 0, FALSE);
                }
                break;
            ...
            case cf2_cmdHVCURVETO:
            {
                ...
                while (idx < count)
                {
                    ...
                    if (alternate)
                    {
                        x1 = ADD_INT32(cf2_stack_getReal(opStack, idx + 0),
                                       curX);
                        y1 = curY;
                        x2 = ADD_INT32(cf2_stack_getReal(opStack, idx + 1), x1);
                        y2 = ADD_INT32(cf2_stack_getReal(opStack, idx + 2), y1);
                        ...
                        if (count - idx == 5)
                        {
                            x3 = ADD_INT32(cf2_stack_getReal(opStack, idx + 4),
                                           x2);
                    ...
                    {
                        x1 = curX;
                        y1 = ADD_INT32(cf2_stack_getReal(opStack, idx + 0),
                                       curY);
                        x2 = ADD_INT32(cf2_stack_getReal(opStack, idx + 1), x1);
                        y2 = ADD_INT32(cf2_stack_getReal(opStack, idx + 2), y1);
                        x3 = ADD_INT32(cf2_stack_getReal(opStack, idx + 3), x2);
                        ...
                        if (count - idx == 5)
                        {
                            y3 = ADD_INT32(cf2_stack_getReal(opStack, idx + 4),
                                           y2);
            ...
            default:
                /* numbers */
                {
                    ...
                    else /* op1 == 255 */
                    {
                        ...
                        v = (CF2_Fixed)((byte1 << 24) | (byte2 << 16) |
                                        (byte3 << 8) | byte4);

                        /*
                         * For Type 1:
                         *
                         * According to the specification, values > 32000 or <
                         * -32000 must be followed by a `div' operator to make
                         * the result be in the range [-32000;32000].  We expect
                         * that the second argument of `div' is not a large
                         * number.  Additionally, we don't handle stuff like
                         * `<large1> <large2> <num> div <num> div' or <large1>
                         * <large2> <num> div div'.  This is probably not
                         * allowed anyway.
                         *
                         * <large> <num> <num>+ div is not checked but should
                         * not be allowed as the large value remains untouched.
                         *
                         */
                        if (font->isT1)
                        {
