        ...
        for (j = 0; j < n; j++)
        {
            ...
            for (k = 0; k < t; k++)
            {
                ...
                if (bit < C448_SCALAR_BITS)
                    tab |= (scalar1x->limb[bit / WBITS] >> (bit % WBITS) & 1)
                           << k;
            }
...
void curve448_point_mul_by_ratio_and_encode_like_eddsa(
    uint8_t enc[EDDSA_448_PUBLIC_BYTES], const curve448_point_t p)
{
    gf x, y, z, t;
    curve448_point_t q;
...
c448_error_t curve448_point_decode_like_eddsa_and_mul_by_ratio(
    curve448_point_t p, const uint8_t enc[EDDSA_448_PUBLIC_BYTES])
{
...
void curve448_point_mul_by_ratio_and_encode_like_x448(
    uint8_t out[X_PUBLIC_BYTES], const curve448_point_t p)
{
    curve448_point_t q;
                       ...
                       const curve448_scalar_t scalar, unsigned int table_bits)
{
    ...
    for (w = 1; w < (C448_SCALAR_BITS - 1) / 16 + 3; w++)
    {
        if (w < (C448_SCALAR_BITS - 1) / 16 + 1)
        {
            ...
            current += (uint32_t)(
                (scalar->limb[w / B_OVER_16] >> (16 * (w % B_OVER_16))) << 16);
        }
                                               ...
                                               const curve448_scalar_t scalar2)
{
    ...
    struct smvt_control
        control_var[C448_SCALAR_BITS / (C448_WNAF_VAR_TABLE_BITS + 1) + 3];
    struct smvt_control
        control_pre[C448_SCALAR_BITS / (C448_WNAF_FIXED_TABLE_BITS + 1) + 3];
    ...
    for (i--; i >= 0; i--)
    {
        ...
        if (cv)
        {
            ...
            if (control_var[contv].addend > 0)
                ...
                sub_pniels_from_pt(
                    combo, precmp_var[(-control_var[contv].addend) >> 1],
                    i && !cp);
            contv++;
        }
        ...
        if (cp)
        {
            ...
            if (control_pre[contp].addend > 0)
                add_niels_to_pt(
                    combo, curve448_wnaf_base[control_pre[contp].addend >> 1],
                    i);
            else
                sub_niels_from_pt(
                    combo,
                    curve448_wnaf_base[(-control_pre[contp].addend) >> 1], i);
            contp++;
        }
    }
