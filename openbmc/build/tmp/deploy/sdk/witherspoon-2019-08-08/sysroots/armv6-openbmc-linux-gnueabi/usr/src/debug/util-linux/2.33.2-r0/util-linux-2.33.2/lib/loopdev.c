...
#define loopcxt_sysfs_available(_lc)                                           \
    (!((_lc)->flags & LOOPDEV_FL_NOSYSFS)) && !loopcxt_ioctl_enabled(_lc)

/*
 * @lc: context
 * @device: device name, absolute device path or NULL to reset the current
 * setting
 *
...
int loopcxt_next(struct loopdev_cxt* lc)
{
    ...
    if (iter->default_check)
    {
        ...
        for (++iter->ncur; iter->ncur < LOOPDEV_DEFAULT_NNODES; iter->ncur++)
        {
            char name[16];
 ...
 */
int loopcxt_is_used(struct loopdev_cxt* lc, struct stat* st,
                    const char* backing_file, uint64_t offset,
                    uint64_t sizelimit, int flags)
{
    ino_t ino;
    dev_t dev;
    ...
    DBG(CXT, ul_debugobj(lc, "checking %s vs. %s", loopcxt_get_device(lc),
                         backing_file));
...
static int loopcxt_check_size(struct loopdev_cxt* lc, int file_fd)
{
    ...
    {
        if (blkdev_get_size(file_fd, (unsigned long long*)&expected_size))
        {
    ...
    {
        DBG(CXT,
            ul_debugobj(lc, "expected size misaligned to 512-byte sectors"));
        expected_size = (expected_size >> 9) << 9;
    }

    if (expected_size != size)
    {
        DBG(CXT, ul_debugobj(lc,
...
int loopcxt_setup_device(struct loopdev_cxt* lc)
{
    ...
    {
        DBG(SETUP,
            ul_debugobj(lc, "closing already open device (mode mismatch)"));
        close(lc->fd);
        lc->fd = -1;
        lc->mode = 0;
    }
 ...
 * Returns: TRUE/FALSE
 */
int loopdev_is_used(const char* device, const char* filename, uint64_t offset,
                    uint64_t sizelimit, int flags)
{
    struct loopdev_cxt lc;
    struct stat st;
    ...
    rc = !stat(filename, &st);
    rc = loopcxt_is_used(&lc, rc ? &st : NULL, filename, offset, sizelimit,
                         flags);
                                 ...
                                 uint64_t offset, uint64_t sizelimit, int flags)
{
    ...
    while ((rc = loopcxt_next(lc)) == 0)
    {
        ...
        if (loopcxt_is_used(lc, hasst ? &st : NULL, filename, offset, sizelimit,
                            flags))
            break;
    }
...
/*
 * Returns: 0 = not found, < 0 error, 1 found, 2 found full size and offset
 * match
 */
                         ...
                         uint64_t offset, uint64_t sizelimit)
{
    ...
    while ((rc = loopcxt_next(lc)) == 0)
    {
        ...
        rc = loopcxt_is_used(lc, hasst ? &st : NULL, filename, offset,
                             sizelimit, 0);
        if (!rc)
            continue; /* unused */
        if (rc < 0)
 ...
 */
char* loopdev_find_by_backing_file(const char* filename, uint64_t offset,
                                   uint64_t sizelimit, int flags)
{
    struct loopdev_cxt lc;
    char* res = NULL;
    ...
    if (loopcxt_init(&lc, 0))
        return NULL;
    if (loopcxt_find_by_backing_file(&lc, filename, offset, sizelimit, flags) ==
        0)
