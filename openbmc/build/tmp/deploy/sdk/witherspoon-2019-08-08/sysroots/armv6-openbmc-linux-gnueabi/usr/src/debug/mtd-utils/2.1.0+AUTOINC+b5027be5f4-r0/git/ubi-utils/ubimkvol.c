...
static const char doc[] =
    PROGRAM_NAME " version " VERSION " - a tool to create UBI volumes.";

static const char optionsstr[] =
    "-a, --alignment=<alignment>   volume alignment (default is 1)\n"
    "-n, --vol_id=<volume ID>      UBI volume ID, if not specified, the volume "
    "ID\n"
    ...
    "dynamic\n"
    "-k, --skipcheck               skip the CRC check done at volume open "
    "time\n"
    "-h, -?, --help                print help message\n"
    "-V, --version                 print program version";

static const char usage[] =
    "Usage: " PROGRAM_NAME " <UBI device node file name> [-h] [-a <alignment>] "
                           "[-n <volume ID>] [-N <name>]\n"
    "\t\t\t[-s <bytes>] [-S <LEBs>] [-t <static|dynamic>] [-V] [-m] [-k]\n"
    "\t\t\t[--alignment=<alignment>][--vol_id=<volume ID>] [--name=<name>]\n"
    "\t\t\t[--size=<bytes>] [--lebs=<LEBs>] [--type=<static|dynamic>] "
    "[--help]\n"
    "\t\t\t[--version] [--maxavsize] [--skipcheck]\n\n"
    "Example: " PROGRAM_NAME
    " /dev/ubi0 -s 20MiB -N config_data - create a 20 Megabytes volume\n"
...
static int param_sanity_check(void)
{
    ...
    if (len > UBI_MAX_VOLUME_NAME)
        return errmsg("too long name (%d symbols), max is %d", len,
                      UBI_MAX_VOLUME_NAME);
...
int main(int argc, char* const argv[])
{
    ...
    if (err)
    {
        sys_errmsg("cannot get information about UBI device \"%s\"", args.node);
        goto out_libubi;
    }
