...
struct bitset_cmp
{
    bool operator()(const ::std::bitset<32>& lhs,
                    const ::std::bitset<32>& rhs) const
    {
...
const ::std::map<::std::bitset<32>, int, bitset_cmp> reqflag_mapping = {
    {
        line_request::FLAG_ACTIVE_LOW,
        GPIOD_LINE_REQUEST_FLAG_ACTIVE_LOW,
    },
    {
        line_request::FLAG_OPEN_DRAIN,
        GPIOD_LINE_REQUEST_FLAG_OPEN_DRAIN,
    },
    {
        line_request::FLAG_OPEN_SOURCE,
...
line_bulk::line_bulk(const ::std::vector<line>& lines) : _m_bulk()
{
...
void line_bulk::append(const line& new_line)
{
    ...
    if (this->_m_bulk.size() >= 1 &&
        this->_m_bulk.begin()->get_chip() != new_line.get_chip())
...
void line_bulk::request(const line_request& config,
                        const std::vector<int> default_vals) const
{
    this->throw_if_empty();

    if (!default_vals.empty() && this->size() != default_vals.size())
        throw ::std::invalid_argument("the number of default values must "
                                      "correspond with the number of lines");
    ...
    rv = ::gpiod_line_request_bulk(
        ::std::addressof(bulk), ::std::addressof(conf),
...
void line_bulk::set_values(const ::std::vector<int>& values) const
{
    ...
    if (values.size() != this->_m_bulk.size())
        throw ::std::invalid_argument("the size of values array must "
                                      "correspond with the number of lines");
