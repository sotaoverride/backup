                           ...
                           int flags)
{
    ...
    {
        len =
            snprintf(deldelay_buf + deldelay_cnt, deldelay_size - deldelay_cnt,
...
static int read_delay_line(char* buf, int* flags_p)
{
    ...
    while (1)
    {
        ...
        {
            memmove(deldelay_buf, deldelay_buf + read_pos, deldelay_cnt);
        }
...
#ifdef SUPPORT_ACLS
static inline int acls_differ(const char* fname, struct file_struct* file,
                              stat_x* sxp)
{
    if (preserve_acls)
    {
...
static inline int xattrs_differ(const char* fname, struct file_struct* file,
                                stat_x* sxp)
{
    if (preserve_xattrs)
    {
...
void itemize(const char* fnamecmp, struct file_struct* file, int ndx,
             int statret, stat_x* sxp, int32 iflags, uchar fnamecmp_type,
             const char* xname)
{
    if (statret >= 0)
    { /* A from-dest-dir statret can == 1! */
        int keep_time = !preserve_times
                            ? 0
                            : S_ISDIR(file->mode)
        ...
        if (file->flags & FLAG_TIME_FAILED)
        { /* symlinks only */
            if (iflags & ITEM_LOCAL_CHANGE)
                iflags |= symlink_timeset_failed_flags;
        }
        else if (keep_time ? time_diff(&sxp->st, file)
                           : iflags & (ITEM_TRANSFER | ITEM_LOCAL_CHANGE) &&
                                 !(iflags & ITEM_MATCHED) &&
        ...
        else if (preserve_executability && ((sxp->st.st_mode & 0111 ? 1 : 0) ^
                                            (file->mode & 0111 ? 1 : 0)))
            iflags |= ITEM_REPORT_PERMS;
    ...
    iflags &= 0xffff;
    if ((iflags & (SIGNIFICANT_ITEM_FLAGS | ITEM_REPORT_XATTR) ||
        ...
        if (protocol_version >= 29)
        {
                ...
                iflags & (ITEM_REPORT_XATTR | ITEM_TRANSFER))
            {
                int fd = iflags & ITEM_REPORT_XATTR &&
                                 !(want_xattr_optim &&
                                   BITS_SET(iflags, ITEM_XNAME_FOLLOWS |
                                                        ITEM_LOCAL_CHANGE))
                             ? sock_f_out
                             : -1;
...
static void sum_sizes_sqroot(struct sum_struct* sum, int64 len)
{
    ...
    if (len < 0)
    {
        /* The file length overflowed our int64 var, so we can't process this
         * file. */
    ...
    {
        int32 max_blength =
            protocol_version < 30 ? OLD_MAX_BLOCK_SIZE : MAX_BLOCK_SIZE;
        int32 c;
        int cnt;
    ...
    {
        rprintf(FINFO, "count=%s rem=%ld blength=%ld s2length=%d flength=%s\n",
...
static int generate_and_send_sums(int fd, OFF_T len, int f_out, int f_copy)
{
    ...
    for (i = 0; i < sum.count; i++)
    {
        ...
        {
            rprintf(FINFO, "chunk[%s] offset=%s len=%ld sum1=%08lx\n",
...
static struct file_struct* find_fuzzy(struct file_struct* file,
                                      struct file_list* dirlist_array[],
                                      uchar* fnamecmp_type_ptr)
{
    ...
    for (i = 0; i < fuzzy_basis; i++)
    {
        ...
        for (j = 0; j < dirlist->used; j++)
        {
            ...
            if (F_LENGTH(fp) == F_LENGTH(file) &&
                cmp_time(fp->modtime, 0L, file->modtime, 0L) == 0)
            {
...
static int copy_altdest_file(const char* src, const char* dest,
                             struct file_struct* file)
{
    ...
    if (copy_file(src, copy_to, fd_w, file->mode) < 0)
    {
        if (INFO_GTE(COPY, 1))
        {
            rsyserr(FINFO, errno, "copy_file %s => %s", full_fname(src),
                    copy_to);
        }
        /* Try to clean up. */
        unlink(copy_to);
    ...
    if (match_level == 3 && !copy_dest)
    {
        ...
        if (link_dest)
        {
            ...
            if (preserve_hard_links && F_IS_HLINKED(file))
                finish_hard_link(file, fname, ndx, &sxp->st, itemizing, code,
                                 j);
                         ...
                         enum logcode code)
{
    ...
    if (match_level == 3)
    {
        ...
        if (itemizing && stdout_format_has_i &&
            (INFO_GTE(NAME, 2) || stdout_format_has_i > 1))
        {
            int chg = compare_dest && type != TYPE_DIR
                          ? 0
                          : ITEM_LOCAL_CHANGE +
...
static void list_file_entry(struct file_struct* f)
{
    ...
    if (preserve_links && S_ISLNK(f->mode))
    {
        rprintf(FINFO, "%s %*s %s %s -> %s\n", permbuf, colwidth,
                human_num(len), timestring(f->modtime), f_name(f, NULL),
                F_SYMLINK(f));
    }
    else
#endif
                           ...
                           int itemizing, enum logcode code, int f_out)
{
    ...
    int is_dir = !S_ISDIR(file->mode)
                     ? 0
                     : inc_recurse && ndx != cur_flist->ndx_start - 1 ? -1 : 1;
    ...
    if (daemon_filter_list.head && (*fname != '.' || fname[1]))
    {
        if (check_filter(&daemon_filter_list, FLOG, fname, is_dir) < 0)
        {
            ...
            if (F_IS_HLINKED(file))
                handle_skipped_hlink(file, itemizing, code, f_out);
#endif
            rprintf(FERROR_XFER, "ERROR: daemon refused to receive %s \"%s\"\n",
    ...
    else
    {
        ...
        {
            /* Each parent dir must be in the file list or the flist data is
             * bad. Optimization: most of the time the parent dir will be the
             * last dir this function was asked to process in the file list. */
            if (!inc_recurse &&
                (*dn != '.' ||
                 dn[1]) /* Avoid an issue with --relative and the "." dir. */
                && (!prior_dir_file ||
                    strcmp(dn, f_name(prior_dir_file, NULL)) != 0) &&
            ...
            if (relative_paths && !implied_dirs && do_stat(dn, &sx.st) < 0)
            {
                if (dry_run)
                ...
                if (make_path(fname, MKP_DROP_NAME | MKP_SKIP_SLASH) < 0)
                {
                    rsyserr(FERROR_XFER, errno,
                            "recv_generator: mkdir %s failed", full_fname(dn));
                }
            }
            if (fuzzy_basis)
            {
        ...
        if (need_fuzzy_dirlist && S_ISREG(file->mode))
        {
            ...
            for (i = 0; i < fuzzy_basis; i++)
            {
                if (i && pathjoin(fnamecmpbuf, MAXPATHLEN, basis_dir[i - 1],
                                  dn) >= MAXPATHLEN)
                    continue;
                fuzzy_dirlist[i] =
                    get_dirlist(fnamecmpbuf, -1, GDL_IGNORE_FILTER_RULES);
    ...
    if (statret == 0 && !(sx.st.st_mode & S_IWUSR) && !am_root &&
        sx.st.st_uid == our_uid)
    ...
    if (is_dir)
    {
        ...
        if (is_dir < 0)
        {
            ...
            if (statret == 0 && (S_ISDIR(sx.st.st_mode) ||
                                 delete_item(fname, sx.st.st_mode,
                                             del_opts | DEL_FOR_DIR) != 0))
        ...
        { /* See comment in non-dir code below. */
            file->mode =
                dest_mode(file->mode, sx.st.st_mode, dflt_perms, statret == 0);
        }
        if (statret != 0 && basis_dir[0] != NULL)
        {
            int j = try_dests_non(file, fname, ndx, fnamecmpbuf, &sx, itemizing,
                                  code);
            if (j == -2)
            {
                itemizing = 0;
            ...
            errno != EEXIST)
        {
            ...
            skipping_dir_contents:
                rprintf(FERROR, "*** Skipping any contents from this failed "
                                "directory ***\n");
                skip_dir = file;
                file->flags |= FLAG_MISSING_DIR;
                goto cleanup;
...
#ifdef SUPPORT_HARD_LINKS
                if (preserve_hard_links && F_IS_HLINKED(file))
                    finish_hard_link(file, fname, ndx, &sx.st, itemizing, code,
                                     -1);
#endif
        ...
        (preserve_specials && IS_SPECIAL(file->mode)))
    {
        ...
        if (DEBUG_GTE(GENR, 1))
        {
            rprintf(FINFO, "mknod(%s, 0%o, [%ld,%ld])\n", fname,
                    (int)file->mode, (long)major(rdev), (long)minor(rdev));
        }
    ...
    {
        int j = try_dests_reg(file, fname, ndx, fnamecmpbuf, &sx, statret == 0,
                              itemizing, code);
        if (j == -2)
        {
            if (remove_source_files == 1)
    ...
    init_stat_x(&real_sx);
    real_sx.st =
        sx.st; /* Don't copy xattr/acl pointers, as they would free wrong. */
    real_ret = statret;
    ...
    else if (unchanged_file(fnamecmp, file, &sx.st))
    {
        ...
        set_file_attrs(fname, file, &sx, NULL,
                       maybe_ATTRS_REPORT | maybe_ATTRS_SET_NANO);
        if (itemizing)
...
prepare_to_open:
    ...
    if (inplace && make_backups > 0 && fnamecmp_type == FNAMECMP_FNAME)
    {
        ...
        {
            rsyserr(FERROR_XFER, errno, "unlink %s", full_fname(backupptr));
            unmake_file(back_file);
            back_file = NULL;
            close(fd);
            goto cleanup;
        }
        if ((f_copy = do_open(backupptr, O_WRONLY | O_CREAT | O_TRUNC | O_EXCL,
                              0600)) < 0)
        {
...
int atomic_create(struct file_struct* file, char* fname, const char* slnk,
                  const char* hlnk, dev_t rdev, stat_x* sxp, int del_for_flag)
{
    char tmpname[MAXPATHLEN];
    const char* create_name;
    int skip_atomic, dir_in_the_way = del_for_flag && S_ISDIR(sxp->st.st_mode);

    if (!del_for_flag || dir_in_the_way || tmpdir ||
        !get_tmpname(tmpname, fname, True))
        skip_atomic = 1;
    else
        skip_atomic = 0;
    ...
    if (del_for_flag)
    {
        ...
        {
            int del_opts = delete_mode || force_delete ? DEL_RECURSE : 0;
            if (delete_item(fname, sxp->st.st_mode, del_opts | del_for_flag) !=
                0)
                return 0;
        }
    }
    ...
    while (1)
    {
        ...
        if (preserve_hard_links && (ndx = get_hlink_num()) != -1)
        {
            ...
            if (send_failed)
                ...
                finish_hard_link(file, f_name(file, fbuf), ndx, NULL, itemizing,
                                 code, -1);
            flist->in_progress--;
            continue;
        }
        ...
        if (!read_batch && !flist_eof)
        {
            ...
            maybe_flush_socket(!flist_eof && file_total - old_total <
...
void generate_files(int f_out, const char* local_name)
{
    ...
    implied_dirs_are_missing =
        relative_paths && !implied_dirs && protocol_version < 30;
    ...
    if (DEBUG_GTE(FLIST, 1))
    {
        rprintf(FINFO, "delta-transmission %s\n",
                whole_file ? "disabled for local transfer or --whole-file"
                           : "enabled");
    }
