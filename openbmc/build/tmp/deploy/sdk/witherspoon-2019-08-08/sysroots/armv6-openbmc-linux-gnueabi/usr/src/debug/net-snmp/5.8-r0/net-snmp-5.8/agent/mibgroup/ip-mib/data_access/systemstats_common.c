 ...
 */
netsnmp_container* netsnmp_access_systemstats_container_init(u_int flags)
{
                                              ...
                                              u_int load_flags)
{
    ...
    if (0 != rc)
    {
        netsnmp_access_systemstats_container_free(
            container, NETSNMP_ACCESS_SYSTEMSTATS_FREE_NOFLAGS);
        container = NULL;
    }
                                               ...
                                               u_int free_flags)
{
    ...
    {
        snmp_log(LOG_ERR,
                 "invalid container for netsnmp_access_systemstats_free\n");
        return;
    }
    ...
    if (!(free_flags & NETSNMP_ACCESS_SYSTEMSTATS_FREE_DONT_CLEAR))
    {
         ...
         * free all items.
         */
        CONTAINER_CLEAR(container, (netsnmp_container_obj_func*)_entry_release,
                        NULL);
    }
...
netsnmp_systemstats_entry*
    netsnmp_access_systemstats_entry_get_by_index(netsnmp_container* container,
                                                  oid index)
{
    netsnmp_index tmp;
 ...
 * \internal
 */
static void _entry_release(netsnmp_systemstats_entry* entry, void* context)
{
 ...
 */
static void _calculate_entries(netsnmp_systemstats_entry* entry)
{
    /*
        ...
        entry->stats.columnAvail[IPSYSTEMSTATSTABLE_HCINNOROUTES])
    {
        ...
        u64Incr(&entry->stats.HCInForwDatagrams,
                &entry->stats.HCOutForwDatagrams);
        ...
        entry->stats.columnAvail[IPSYSTEMSTATSTABLE_HCOUTDISCARDS])
    {
        ...
        u64Subtract(&tmp3, &entry->stats.HCOutDiscards,
                    &entry->stats.HCOutTransmits);
 ...
 * @retval <0 : error
 */
int netsnmp_access_systemstats_entry_update_stats(
    netsnmp_systemstats_entry* prev_vals, netsnmp_systemstats_entry* new_vals)
{
    ...
    else
    {
         ...
         */
        memcpy(&prev_vals->stats.columnAvail[0],
               &new_vals->stats.columnAvail[0],
               sizeof(new_vals->stats.columnAvail));
        ...
        if (new_vals->stats.columnAvail[IPSYSTEMSTATSTABLE_HCINNOROUTES])
            if (0 != netsnmp_c64_check32_and_update(
                         ...
                         &prev_vals->old_stats->HCInNoRoutes, &need_wrap_check))
                NETSNMP_LOGONCE(
                    (LOG_ERR, "Error expanding HCInNoRoutes to 64bits in %s\n",
                     prev_vals->tableName));
        ...
        if (new_vals->stats.columnAvail[IPSYSTEMSTATSTABLE_HCOUTNOROUTES])
            if (0 != netsnmp_c64_check32_and_update(
                         ...
                         &need_wrap_check))
                NETSNMP_LOGONCE(
                    (LOG_ERR, "Error expanding HCOutNoRoutes to 64bits in %s\n",
                     prev_vals->tableName));
        ...
        if (new_vals->stats.columnAvail[IPSYSTEMSTATSTABLE_HCOUTDISCARDS])
            if (0 != netsnmp_c64_check32_and_update(
                         ...
                         &need_wrap_check))
                NETSNMP_LOGONCE(
                    (LOG_ERR, "Error expanding HCOutDiscards to 64bits in %s\n",
                     prev_vals->tableName));
        ...
        if (new_vals->stats.columnAvail[IPSYSTEMSTATSTABLE_HCOUTFRAGREQDS])
            if (0 != netsnmp_c64_check32_and_update(
                         ...
                         &need_wrap_check))
                NETSNMP_LOGONCE((
                    LOG_ERR, "Error expanding HCOutFragReqds to 64bits in %s\n",
                    prev_vals->tableName));
        ...
        if (new_vals->stats.columnAvail[IPSYSTEMSTATSTABLE_HCOUTFRAGOKS])
            if (0 != netsnmp_c64_check32_and_update(
                         ...
                         &prev_vals->old_stats->HCOutFragOKs, &need_wrap_check))
                NETSNMP_LOGONCE(
                    (LOG_ERR, "Error expanding HCOutFragOKs to 64bits in %s\n",
                     prev_vals->tableName));
        ...
        if (new_vals->stats.columnAvail[IPSYSTEMSTATSTABLE_HCOUTFRAGFAILS])
            if (0 != netsnmp_c64_check32_and_update(
                         ...
                         &need_wrap_check))
                NETSNMP_LOGONCE((
                    LOG_ERR, "Error expanding HCOutFragFails to 64bits in %s\n",
                    prev_vals->tableName));
        ...
        if (new_vals->stats.columnAvail[IPSYSTEMSTATSTABLE_HCINRECEIVES])
            if (0 != netsnmp_c64_check32_and_update(
                         ...
                         &prev_vals->old_stats->HCInReceives, &need_wrap_check))
                NETSNMP_LOGONCE(
                    (LOG_ERR, "Error expanding HCInReceives to 64bits in %s\n",
                     prev_vals->tableName));
        ...
        if (new_vals->stats.columnAvail[IPSYSTEMSTATSTABLE_HCINOCTETS])
            if (0 != netsnmp_c64_check32_and_update(
                         ...
                         &prev_vals->old_stats->HCInOctets, &need_wrap_check))
        ...
        if (new_vals->stats.columnAvail[IPSYSTEMSTATSTABLE_HCINDELIVERS])
            if (0 != netsnmp_c64_check32_and_update(
                         ...
                         &prev_vals->old_stats->HCInDelivers, &need_wrap_check))
                NETSNMP_LOGONCE(
                    (LOG_ERR, "Error expanding HCInDelivers to 64bits in %s\n",
                     prev_vals->tableName));
        ...
        if (new_vals->stats.columnAvail[IPSYSTEMSTATSTABLE_HCOUTREQUESTS])
            if (0 != netsnmp_c64_check32_and_update(
                         ...
                         &need_wrap_check))
                NETSNMP_LOGONCE(
                    (LOG_ERR, "Error expanding HCOutRequests to 64bits in %s\n",
                     prev_vals->tableName));
        ...
        if (new_vals->stats.columnAvail[IPSYSTEMSTATSTABLE_HCOUTTRANSMITS])
            if (0 != netsnmp_c64_check32_and_update(
                         ...
                         &need_wrap_check))
                NETSNMP_LOGONCE((
                    LOG_ERR, "Error expanding HCOutTransmits to 64bits in %s\n",
                    prev_vals->tableName));
        ...
        if (new_vals->stats.columnAvail[IPSYSTEMSTATSTABLE_HCOUTOCTETS])
            if (0 != netsnmp_c64_check32_and_update(
                         ...
                         &prev_vals->old_stats->HCOutOctets, &need_wrap_check))
                NETSNMP_LOGONCE(
                    (LOG_ERR, "Error expanding HCOutOctets to 64bits in %s\n",
                     prev_vals->tableName));
        ...
        if (new_vals->stats.columnAvail[IPSYSTEMSTATSTABLE_HCINMCASTPKTS])
            if (0 != netsnmp_c64_check32_and_update(
                         ...
                         &need_wrap_check))
                NETSNMP_LOGONCE(
                    (LOG_ERR, "Error expanding HCInMcastPkts to 64bits in %s\n",
                     prev_vals->tableName));
        ...
        if (new_vals->stats.columnAvail[IPSYSTEMSTATSTABLE_HCOUTMCASTPKTS])
            if (0 != netsnmp_c64_check32_and_update(
                         ...
                         &need_wrap_check))
                NETSNMP_LOGONCE((
                    LOG_ERR, "Error expanding HCOutMcastPkts to 64bits in %s\n",
                    prev_vals->tableName));
        ...
        if (new_vals->stats.columnAvail[IPSYSTEMSTATSTABLE_HCINBCASTPKTS])
            if (0 != netsnmp_c64_check32_and_update(
                         ...
                         &need_wrap_check))
                NETSNMP_LOGONCE(
                    (LOG_ERR, "Error expanding HCInBcastPkts to 64bits in %s\n",
                     prev_vals->tableName));
        ...
        if (new_vals->stats.columnAvail[IPSYSTEMSTATSTABLE_HCOUTBCASTPKTS])
            if (0 != netsnmp_c64_check32_and_update(
                         ...
                         &need_wrap_check))
                NETSNMP_LOGONCE((
                    LOG_ERR, "Error expanding HCOutBcastPkts to 64bits in %s\n",
                    prev_vals->tableName));
    }
 ...
 * @retval  0 : no error
 */
int netsnmp_access_systemstats_entry_update(netsnmp_systemstats_entry* lhs,
                                            netsnmp_systemstats_entry* rhs)
{
    DEBUGMSGTL(("access:systemstats", "copy\n"));

    if ((NULL == lhs) || (NULL == rhs) || (lhs->index[0] != rhs->index[0]) ||
