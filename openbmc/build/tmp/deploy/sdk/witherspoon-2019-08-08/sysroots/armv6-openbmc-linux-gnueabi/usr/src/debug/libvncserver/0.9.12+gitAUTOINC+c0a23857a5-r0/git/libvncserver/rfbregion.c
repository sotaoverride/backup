...
static rfbBool sraSpanListEqual(const sraSpanList* s1, const sraSpanList* s2)
{
    ...
    {
        if ((sp1->start != sp2->start) || (sp1->end != sp2->end) ||
...
static void sraSpanMergePrevious(sraSpan* dest)
{
    ...
    while ((prev->_prev) && (prev->end == dest->start) &&
...
static void sraSpanMergeNext(sraSpan* dest)
{
    ...
    while ((next->_next) && (next->start == dest->end) &&
...
static void sraSpanListOr(sraSpanList* dest, const sraSpanList* src)
{
    ...
    while (s_curr != &(src->back))
    {
        ...
        {
            /* - Add the span */
            sraSpanInsertBefore(sraSpanCreate(s_start, s_end, s_curr->subspan),
                                d_curr);
            if (d_curr != &(dest->back))
        ...
        else
        {
            ...
            if ((s_start < d_curr->end) && (s_end > d_curr->start))
            {
                ...
                {
                    sraSpanInsertBefore(
                        sraSpanCreate(s_start, d_curr->start, s_curr->subspan),
                        d_curr);
                    sraSpanMergePrevious(d_curr);
                }
                ...
                {
                    sraSpanInsertAfter(
                        sraSpanCreate(s_end, d_curr->end, d_curr->subspan),
                        d_curr);
                    d_curr->end = s_end;
                }
                if (s_start > d_curr->start)
                {
                    sraSpanInsertBefore(
                        sraSpanCreate(d_curr->start, s_start, d_curr->subspan),
                        d_curr);
...
static rfbBool sraSpanListAnd(sraSpanList* dest, const sraSpanList* src)
{
    ...
    while ((s_curr != &(src->back)) && (d_curr != &(dest->back)))
    {
        ...
        if (s_curr->end < d_curr->end)
        {
            ...
            sraSpanInsertAfter(
                sraSpanCreate(s_curr->end, d_curr->end, d_curr->subspan),
                d_curr);
...
static rfbBool sraSpanListSubtract(sraSpanList* dest, const sraSpanList* src)
{
    ...
    while ((s_curr != &(src->back)) && (d_curr != &(dest->back)))
    {
        ...
        {
            sraSpanInsertBefore(
                sraSpanCreate(d_curr->start, s_curr->start, d_curr->subspan),
                d_curr);
            d_curr->start = s_curr->start;
        }
        if (s_curr->end < d_curr->end)
        ...
        /* - Now recursively process the affected span */
        if ((!d_curr->subspan) ||
            !sraSpanListSubtract(d_curr->subspan, s_curr->subspan))
        {
            /* - The destination subspan is now empty, so we should remove it */
            sraSpan* next = d_curr->_next;
            sraSpanRemove(d_curr);
...
{
    int xmin = ((unsigned int)(int)-1) >> 1, ymin = xmin, xmax = 1 - xmin,
        ymax = xmax;
    sraSpan *vcurr, *hcurr;
...
sraRectangleIterator* sraRgnGetReverseIterator(sraRegion* s, rfbBool reverseX,
                                               rfbBool reverseY)
{
...
rfbBool sraRgnIteratorNext(sraRectangleIterator* i, sraRect* r)
{
    ...
    while (i->sPtrs[i->ptrPos]->subspan)
    {
        ...
        { /* array is too small */
            i->ptrSize += DEFSTEP;
            i->sPtrs =
                (sraSpan**)realloc(i->sPtrs, sizeof(sraSpan*) * i->ptrSize);
        }
        i->ptrPos += 2;
        if (sraReverse(i))
        {
            ...
            i->sPtrs[i->ptrPos + 1] =
                &(i->sPtrs[i->ptrPos - 2]->subspan->front);
        }
        else
        {
...
int main(int argc, char** argv)
{
    ...
    printf(
        "\ntrue "
        "[(10-50)[(10-600)](50-200)[(10-40)(350-600)](200-300)[(10-600)]]\n\n");
    ...
    while (sraRgnIteratorNext(i, &rect))
        printf("%dx%d+%d+%d ", rect.x2 - rect.x1, rect.y2 - rect.y1, rect.x1,
               rect.y1);
    sraRgnReleaseIterator(i);
    printf("\n20x10+0+0 600x30+0+10 590x10+10+40 30x150+10+50 250x150+350+50 "
           "590x100+10+200 \n\n");
    ...
    while (sraRgnIteratorNext(i, &rect))
        printf("%dx%d+%d+%d ", rect.x2 - rect.x1, rect.y2 - rect.y1, rect.x1,
               rect.y1);
    sraRgnReleaseIterator(i);
    printf("\n20x10+0+0 600x30+0+10 590x10+10+40 250x150+350+50 30x150+10+50 "
           "590x100+10+200 \n\n");
    ...
    while (sraRgnIteratorNext(i, &rect))
        printf("%dx%d+%d+%d ", rect.x2 - rect.x1, rect.y2 - rect.y1, rect.x1,
               rect.y1);
    sraRgnReleaseIterator(i);
    printf("\n590x100+10+200 250x150+350+50 30x150+10+50 590x10+10+40 "
           "600x30+0+10 20x10+0+0 \n\n");
