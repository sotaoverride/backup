...
static const struct colinfo infos[] = {
    ...
    [COL_DISKSIZE] = {"DISKSIZE", 5, SCOLS_FL_RIGHT,
                      N_("limit on the uncompressed amount of data")},
    [COL_ORIG_SIZE] = {"DATA", 5, SCOLS_FL_RIGHT,
                       N_("uncompressed size of stored data")},
    [COL_COMP_SIZE] = {"COMPR", 5, SCOLS_FL_RIGHT,
                       N_("compressed size of stored data")},
    [COL_ALGORITHM] = {"ALGORITHM", 3, 0,
                       N_("the selected compression algorithm")},
    [COL_STREAMS] = {"STREAMS", 3, SCOLS_FL_RIGHT,
                     N_("number of concurrent compress operations")},
    [COL_ZEROPAGES] = {"ZERO-PAGES", 3, SCOLS_FL_RIGHT,
                       N_("empty pages with no allocated memory")},
    [COL_MEMTOTAL] = {"TOTAL", 5, SCOLS_FL_RIGHT,
                      N_("all memory including allocator fragmentation and "
                         "metadata overhead")},
    [COL_MEMLIMIT] = {"MEM-LIMIT", 5, SCOLS_FL_RIGHT,
    ...
    [COL_MIGRATED] = {"MIGRATED", 5, SCOLS_FL_RIGHT,
                      N_("number of objects migrated by compaction")},
    [COL_MOUNTPOINT] = {"MOUNTPOINT", 0.10, SCOLS_FL_TRUNC,
...
static const char* mm_stat_names[] = {[MM_ORIG_DATA_SIZE] = "orig_data_size",
...
struct zram
{
    ...
    unsigned int mm_stat_probed : 1, control_probed : 1,
...
static int zram_used(struct zram* z)
{
    ...
    if (sysfs && ul_path_read_u64(sysfs, &size, "disksize") == 0 && size > 0)
    {
...
static int zram_has_control(struct zram* z)
{
    ...
    {
        z->has_control =
            access(_PATH_SYS_CLASS "/zram-control/", F_OK) == 0 ? 1 : 0;
        z->control_probed = 1;
...
static void __attribute__((__noreturn__)) usage(void)
{
    ...
    fputs(_(" -a, --algorithm lzo|lz4|lz4hc|deflate|842   compression "
            "algorithm to use\n"),
          out);
    fputs(_(" -b, --bytes               print sizes in bytes rather than in "
...
int main(int argc, char** argv)
{
    ...
    static const ul_excl_t excl[] = {{'f', 'o', 'r'}, {'o', 'r', 's'}, {0}};
    ...
    while ((c = getopt_long(argc, argv, "a:bfho:nrs:t:V", longopts, NULL)) !=
           -1)
    {
        ...
        switch (c)
        {
            ...
            case 'f':
                ...
                break;
            case 'o':
                ncolumns = string_to_idarray(
                    optarg, columns, ARRAY_SIZE(columns), column_name_to_id);
                if (ncolumns < 0)
                    return EXIT_FAILURE;
                break;
            case OPT_LIST_TYPES:
                for (ncolumns = 0; (size_t)ncolumns < ARRAY_SIZE(infos);
                     ncolumns++)
            ...
            case 't':
                nstreams =
                    strtou64_or_err(optarg, _("failed to parse streams"));
                break;
            case 'r':
                act = A_RESET;
                break;
    ...
    switch (act)
    {
        ...
        case A_RESET:
            ...
            {
                zram = new_zram(argv[optind]);
                if (!zram_exist(zram) || zram_set_u64parm(zram, "reset", 1))
                {
        ...
        case A_CREATE:
            ...
            if (nstreams &&
                ...
                err(EXIT_FAILURE, _("%s: failed to set number of streams"),
                    zram->devname);

            if (algorithm &&
                zram_set_strparm(zram, "comp_algorithm", algorithm))
                err(EXIT_FAILURE, _("%s: failed to set algorithm"),
                    zram->devname);
