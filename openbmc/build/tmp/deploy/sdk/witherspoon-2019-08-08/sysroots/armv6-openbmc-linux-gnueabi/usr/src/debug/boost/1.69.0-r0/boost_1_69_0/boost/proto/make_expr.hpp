...
#define BOOST_PROTO_AS_CHILD_TYPE(Z, N, DATA)                                  \
    typename boost::proto::detail::protoify<                                   \
        ...
        BOOST_PP_TUPLE_ELEM(3, 2, DATA)>::result_type /**/

/// INTERNAL ONLY
///
#define BOOST_PROTO_AS_CHILD(Z, N, DATA)                                       \
    boost::proto::detail::protoify<BOOST_PP_CAT(                               \
...
template <typename Tag, BOOST_PP_ENUM_PARAMS(BOOST_PROTO_MAX_ARITY, typename A),
          typename Void1 // = void
          ,
          typename Void2 // = void
          >
struct make_expr
{
    ...
    typedef typename detail::make_expr_<
        Tag, deduce_domain BOOST_PP_ENUM_TRAILING_PARAMS(BOOST_PROTO_MAX_ARITY,
                                                         A)>::result_type type;
};
...
template <typename Tag, typename Domain BOOST_PP_ENUM_TRAILING_PARAMS(
                            BOOST_PROTO_MAX_ARITY, typename A)>
struct make_expr<Tag,
                 ...
                 typename Domain::proto_is_domain_>
{
    ...
    /// term\<A0\> \>)\>::type</tt>.
    ///
    /// Otherwise, \c type is a typedef for
    /// <tt>boost::result_of\<Domain(expr\<Tag, listN\< as_child\<A0\>::type,
    /// ... as_child\<AN\>::type\>)
    ...
    /// child type is \c A.
    /// \li If \c A is <tt>B &</tt> or <tt>cv
    /// boost::reference_wrapper\<B\></tt>, and <tt>is_expr\<B\>::value</tt> is
    /// \c true, then the child type is <tt>B &</tt>. \li If
    /// <tt>is_expr\<A\>::value</tt> is \c false, then the child type is
    /// <tt>boost::result_of\<Domain(expr\<tag::terminal, term\<A\> \>
    /// )\>::type</tt>.
    /// \li If \c A is <tt>B &</tt> or <tt>cv
    /// boost::reference_wrapper\<B\></tt>, and <tt>is_expr\<B\>::value</tt> is
    /// \c false, then the child type is
    /// <tt>boost::result_of\<Domain(expr\<tag::terminal, term\<B &\> \>
    /// )\>::type</tt>.
    typedef
        typename detail::make_expr_<Tag, Domain BOOST_PP_ENUM_TRAILING_PARAMS(
...
/// specialization is selected.)
template <typename Tag, typename Sequence, typename Void1 // = void
          ,
          typename Void2 // = void
          >
struct unpack_expr
{
    ...
    typedef typename detail::unpack_expr_<
        Tag, deduce_domain, Sequence,
        fusion::result_of::size<Sequence>::type::value>::type type;
};
...
struct make_expr
{
    ...
    BOOST_FORCEINLINE
        typename result_of::make_expr<Tag, Domain, A0 const>::type const
...
#include <boost/proto/detail/make_expr_funop.hpp>
    ...
    /// INTERNAL ONLY
    ///
    template <BOOST_PP_ENUM_BINARY_PARAMS(BOOST_PROTO_MAX_ARITY, typename A,
                                          = void BOOST_PP_INTERCEPT)>
    struct impl : detail::make_expr_<Tag, Domain BOOST_PP_ENUM_TRAILING_PARAMS(
                                              BOOST_PROTO_MAX_ARITY, A)>
    {
    };
};
...
struct unpack_expr
{
    ...
    {
        typedef typename result_of::unpack_expr<
            Tag, Domain, typename remove_reference<Sequence>::type>::type type;
    };
        ...
        operator()(Sequence const& sequence) const
    {
        return proto::detail::unpack_expr_<
            Tag, Domain, Sequence const,
            fusion::result_of::size<Sequence>::type::value>::call(sequence);
    }
};
...
BOOST_FORCEINLINE
    typename lazy_disable_if<is_domain<A0>,
                             result_of::make_expr<Tag, A0 const>>::type const
...
BOOST_FORCEINLINE
    typename result_of::make_expr<Tag, Domain, C0 const>::type const
...
BOOST_FORCEINLINE typename lazy_disable_if<
    is_domain<Sequence>,
    result_of::unpack_expr<Tag, Sequence const>>::type const
    unpack_expr(Sequence const& sequence)
{
    return proto::detail::unpack_expr_<
        Tag, deduce_domain, Sequence const,
        fusion::result_of::size<Sequence>::type::value>::call(sequence);
}
...
{
    return proto::detail::unpack_expr_<
        Tag, Domain, Sequence2 const,
        fusion::result_of::size<Sequence2>::type::value>::call(sequence2);
}
