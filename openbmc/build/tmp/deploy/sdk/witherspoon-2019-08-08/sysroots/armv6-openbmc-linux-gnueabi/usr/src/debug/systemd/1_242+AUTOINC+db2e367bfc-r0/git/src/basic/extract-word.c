...
int extract_first_word_and_warn(const char** p, char** ret,
                                const char* separators, ExtractFlags flags,
                                const char* unit, const char* filename,
                                unsigned line, const char* rvalue)
{
    ...
    if (r == -EINVAL && !(flags & EXTRACT_CUNESCAPE_RELAX))
    {
        ...
        *p = save;
        r = extract_first_word(p, ret, separators,
                               flags | EXTRACT_CUNESCAPE_RELAX);
        if (r >= 0)
        {
            /* It worked this time, hence it must have been an invalid escape
             * sequence. */
        ...
        /* If it's still EINVAL; then it must be unbalanced quoting, report
         * this. */
        if (r == -EINVAL)
...
/* We pass ExtractFlags as unsigned int (to avoid undefined behaviour when
 * passing an object that undergoes default argument promotion as an argument to
 * va_start). Let's make sure that ExtractFlags fits into an unsigned int. */
