...
static void lldp_neighbor_id_hash_func(const LLDPNeighborID* id,
                                       struct siphash* state)
{
    siphash24_compress(id->chassis_id, id->chassis_id_size, state);
    siphash24_compress(&id->chassis_id_size, sizeof(id->chassis_id_size),
                       state);
    siphash24_compress(id->port_id, id->port_id_size, state);
    siphash24_compress(&id->port_id_size, sizeof(id->port_id_size), state);
}

int lldp_neighbor_id_compare_func(const LLDPNeighborID* x,
                                  const LLDPNeighborID* y)
{
    return memcmp_nn(x->chassis_id, x->chassis_id_size, y->chassis_id,
                     y->chassis_id_size)
               ?: memcmp_nn(x->port_id, x->port_id_size, y->port_id,
                            y->port_id_size);
}

DEFINE_HASH_OPS_WITH_VALUE_DESTRUCTOR(lldp_neighbor_hash_ops, LLDPNeighborID,
...
_public_ sd_lldp_neighbor* sd_lldp_neighbor_unref(sd_lldp_neighbor* n)
{
    ...
    /* Drops one reference from the neighbor. Note that the object is not freed
     * unless it is already unlinked from the sd_lldp object. */
...
sd_lldp_neighbor* lldp_neighbor_unlink(sd_lldp_neighbor* n)
{
    ...
    /* Removes the neighbor object from the LLDP object, and frees it if it also
    ...
    /* Only remove the neighbor object from the hash table if it's in there,
     * don't complain if it isn't. This is because we are used as destructor
     * call for hashmap_clear() and thus sometimes are called to de-register
     * ourselves from the hashtable and sometimes are called after we already
     * are de-registered. */
...
int lldp_neighbor_parse(sd_lldp_neighbor* n)
{
    ...
    if (h.ether_dhost[0] != 0x01 || h.ether_dhost[1] != 0x80 ||
        h.ether_dhost[2] != 0xc2 || h.ether_dhost[3] != 0x00 ||
    ...
    for (;;)
    {
        ...
        switch (type)
        {
            ...
            case SD_LLDP_TYPE_END:
                ...
                /* Note that after processing the SD_LLDP_TYPE_END left could
                 * still be > 0 as the message may contain padding (see IEEE
                 * 802.1AB-2016, sec. 8.5.12) */
            ...
            case SD_LLDP_TYPE_CHASSIS_ID:
                ...
                { /* includes the chassis subtype, hence one extra byte */
                    log_lldp("Chassis ID field size out of range, ignoring "
                             "datagram.");
                    return -EBADMSG;
                }
                if (n->id.chassis_id)
            ...
            case SD_LLDP_TYPE_SYSTEM_CAPABILITIES:
                if (length != 4)
                    log_lldp(
                        "System capabilities field has wrong size, ignoring.");
                else
                {
...
void lldp_neighbor_start_ttl(sd_lldp_neighbor* n)
{
    ...
    if (n->ttl > 0)
    {
        ...
        base = triple_timestamp_by_clock(&n->timestamp,
                                         clock_boottime_or_monotonic());
        if (base <= 0 || base == USEC_INFINITY)
            base = now(clock_boottime_or_monotonic()); /* Otherwise, take the
                                                          current time */
...
_public_ int sd_lldp_neighbor_get_source_address(sd_lldp_neighbor* n,
...
_public_ int
    sd_lldp_neighbor_get_destination_address(sd_lldp_neighbor* n,
                                             struct ether_addr* address)
{
    assert_return(n, -EINVAL);
    assert_return(address, -EINVAL);
...
_public_ int sd_lldp_neighbor_get_raw(sd_lldp_neighbor* n, const void** ret,
                                      size_t* size)
{
    assert_return(n, -EINVAL);
...
_public_ int sd_lldp_neighbor_get_chassis_id(sd_lldp_neighbor* n, uint8_t* type,
                                             const void** ret, size_t* size)
{
    assert_return(n, -EINVAL);
...
_public_ int sd_lldp_neighbor_get_chassis_id_as_string(sd_lldp_neighbor* n,
                                                       const char** ret)
{
    char* k;
    int r;
    ...
    switch (*(uint8_t*)n->id.chassis_id)
    {
        ...
        case SD_LLDP_CHASSIS_SUBTYPE_LOCALLY_ASSIGNED:
            k = cescape_length((char*)n->id.chassis_id + 1,
        ...
        case SD_LLDP_CHASSIS_SUBTYPE_NETWORK_ADDRESS:
            r = format_network_address(n->id.chassis_id, n->id.chassis_id_size,
                                       &k);
            if (r < 0)
                return r;
            if (r > 0)
...
_public_ int sd_lldp_neighbor_get_port_id(sd_lldp_neighbor* n, uint8_t* type,
                                          const void** ret, size_t* size)
{
    assert_return(n, -EINVAL);
    assert_return(type, -EINVAL);
...
_public_ int sd_lldp_neighbor_get_port_id_as_string(sd_lldp_neighbor* n,
                                                    const char** ret)
{
    char* k;
    int r;
    ...
    switch (*(uint8_t*)n->id.port_id)
    {
        ...
        case SD_LLDP_PORT_SUBTYPE_LOCALLY_ASSIGNED:
            k = cescape_length((char*)n->id.port_id + 1,
                               n->id.port_id_size - 1);
            if (!k)
                return -ENOMEM;
...
_public_ int sd_lldp_neighbor_get_system_name(sd_lldp_neighbor* n,
                                              const char** ret)
{
    assert_return(n, -EINVAL);
    assert_return(ret, -EINVAL);
...
_public_ int sd_lldp_neighbor_get_system_description(sd_lldp_neighbor* n,
                                                     const char** ret)
{
    assert_return(n, -EINVAL);
    assert_return(ret, -EINVAL);
...
_public_ int sd_lldp_neighbor_get_port_description(sd_lldp_neighbor* n,
                                                   const char** ret)
{
    assert_return(n, -EINVAL);
    assert_return(ret, -EINVAL);
...
_public_ int sd_lldp_neighbor_get_system_capabilities(sd_lldp_neighbor* n,
                                                      uint16_t* ret)
{
    assert_return(n, -EINVAL);
    assert_return(ret, -EINVAL);
...
_public_ int sd_lldp_neighbor_get_enabled_capabilities(sd_lldp_neighbor* n,
                                                       uint16_t* ret)
{
    assert_return(n, -EINVAL);
    assert_return(ret, -EINVAL);
...
_public_ int sd_lldp_neighbor_from_raw(sd_lldp_neighbor** ret, const void* raw,
                                       size_t raw_size)
{
...
_public_ int sd_lldp_neighbor_tlv_get_oui(sd_lldp_neighbor* n,
                                          uint8_t oui[_SD_ARRAY_STATIC 3],
                                          uint8_t* subtype)
{
    const uint8_t* d;
    size_t length;
...
_public_ int sd_lldp_neighbor_tlv_is_oui(sd_lldp_neighbor* n,
                                         const uint8_t oui[_SD_ARRAY_STATIC 3],
                                         uint8_t subtype)
{
    uint8_t k[3], st;
    int r;
...
_public_ int sd_lldp_neighbor_tlv_get_raw(sd_lldp_neighbor* n, const void** ret,
                                          size_t* size)
{
    size_t length;
...
_public_ int sd_lldp_neighbor_get_timestamp(sd_lldp_neighbor* n,
                                            clockid_t clock, uint64_t* ret)
{
    assert_return(n, -EINVAL);
