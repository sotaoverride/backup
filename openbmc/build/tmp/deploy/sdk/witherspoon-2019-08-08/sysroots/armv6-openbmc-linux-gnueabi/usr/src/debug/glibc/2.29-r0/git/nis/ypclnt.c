...
__libc_lock_define_initialized(
    static, ypbindlist_lock) static dom_binding* ypbindlist = NULL;
                                  ...
                                  struct ypbind_resp* ypbr)
{
    ...
    ysd->dom_client = __libc_clntudp_bufcreate(
        &ysd->dom_server_addr, YPPROG, YPVERS, UDPTIMEOUT, &ysd->dom_socket,
        UDPMSGSIZE, UDPMSGSIZE, SOCK_CLOEXEC);
}
...
static int yp_bind_ypbindprog(const char* domain, dom_binding* ysd)
{
    ...
    client =
        clnttcp_create(&clnt_saddr, YPBINDPROG, YPBINDVERS, &clnt_sock, 0, 0);
    if (client == NULL)
        return YPERR_YPBIND;
    ...
    if (clnt_call(client, YPBINDPROC_DOMAIN, (xdrproc_t)xdr_domainname,
                  (caddr_t)&domain, (xdrproc_t)xdr_ypbind_resp, (caddr_t)&ypbr,
                  RPCTIMEOUT) != RPC_SUCCESS)
    {
        clnt_destroy(client);
        return YPERR_YPBIND;
    }
...
libnsl_hidden_nolink_def(yp_bind, GLIBC_2_0)
    ...
    static void yp_unbind_locked(const char* indomain)
{
...
libnsl_hidden_nolink_def(yp_unbind, GLIBC_2_0)
    ...
    static int __ypclnt_call(const char* domain, u_long prog, xdrproc_t xargs,
                             caddr_t req, xdrproc_t xres, caddr_t resp,
                             dom_binding** ydb, int print_error)
{
    enum clnt_stat result;

    result =
        clnt_call((*ydb)->dom_client, prog, xargs, req, xres, resp, RPCTIMEOUT);
                     ...
                     caddr_t req, xdrproc_t xres, caddr_t resp)
{
    ...
    while (ydb != NULL)
    {
        if (strcmp(domain, ydb->dom_domain) == 0)
        {
            if (__yp_bind(domain, &ydb) == 0)
            {
                ...
                status = __ypclnt_call(domain, prog, xargs, req, xres, resp,
                                       &ydb, 0);
    ...
    if (__yp_bind(domain, &ydb) == 0)
    {
        status = __ypclnt_call(domain, prog, xargs, req, xres, resp, &ydb, 1);
        __yp_unbind(ydb);
    }
    ...
    if (status != YPERR_SUCCESS)
    {
        ...
        {
            status =
                __ypclnt_call(domain, prog, xargs, req, xres, resp, &ydb, 1);
            __yp_unbind(ydb);
        }
        else
            free(ydb);
...
libnsl_hidden_nolink_def(__yp_check, GLIBC_2_0)
    ...
    int yp_match(const char* indomain, const char* inmap, const char* inkey,
                 const int inkeylen, char** outval, int* outvallen)
{
    ...
    if (indomain == NULL || indomain[0] == '\0' || inmap == NULL ||
        inmap[0] == '\0' || inkey == NULL || inkey[0] == '\0' || inkeylen <= 0)
        return YPERR_BADARGS;
    ...
    *outvallen = 0;
    ...
    result =
        do_ypcall_tr(indomain, YPPROC_MATCH, (xdrproc_t)xdr_ypreq_key,
                     (caddr_t)&req, (xdrproc_t)xdr_ypresp_val, (caddr_t)&resp);
...
libnsl_hidden_nolink_def(yp_match, GLIBC_2_0)
    ...
    int yp_first(const char* indomain, const char* inmap, char** outkey,
                 int* outkeylen, char** outval, int* outvallen)
{
    ...
    *outkeylen = *outvallen = 0;
    ...
    result =
        do_ypcall(indomain, YPPROC_FIRST, (xdrproc_t)xdr_ypreq_nokey,
                  (caddr_t)&req, (xdrproc_t)xdr_ypresp_key_val, (caddr_t)&resp);
    ...
    if (__builtin_expect((*outkey = malloc(resp.key.keydat_len + 1)) != NULL &&
                             (*outval = malloc(resp.val.valdat_len + 1)) !=
                                 NULL,
                         1))
    {
...
libnsl_hidden_nolink_def(yp_first, GLIBC_2_0)
    ...
    int yp_next(const char* indomain, const char* inmap, const char* inkey,
                const int inkeylen, char** outkey, int* outkeylen,
                char** outval, int* outvallen)
{
    ...
    if (indomain == NULL || indomain[0] == '\0' || inmap == NULL ||
        inmap[0] == '\0' || inkeylen <= 0 || inkey == NULL || inkey[0] == '\0')
        return YPERR_BADARGS;
...
libnsl_hidden_nolink_def(yp_master, GLIBC_2_0)
    ...
    int yp_order(const char* indomain, const char* inmap,
    ...
    struct ypresp_all_data
{
    ...
    void* data;
    int (*foreach)(int status, char* key, int keylen, char* val, int vallen,
                   char* data);
};
        ...
        switch (resp.ypresp_all_u.val.stat)
        {
            case YP_TRUE:
            {
                ...
                *((char*)__mempcpy(val, resp.ypresp_all_u.val.val.valdat_val,
                                   vallen)) = '\0';
                xdr_free((xdrproc_t)xdr_ypresp_all, (char*)&resp);
                if ((*objp->foreach)(objp->status, key, keylen, val, vallen,
                                     objp->data))
                    return TRUE;
            }
            break;
...
#define YP2YPERR(yp, yperr) [YP_##yp - YP_VERS] = YPERR_##yperr
        YP2YPERR(TRUE, SUCCESS), YP2YPERR(NOMORE, NOMORE),
        YP2YPERR(FALSE, YPERR),  YP2YPERR(NOMAP, MAP),
        YP2YPERR(NODOM, DOMAIN), YP2YPERR(NOKEY, KEY),
        YP2YPERR(BADOP, YPERR),  YP2YPERR(BADDB, BADDB),
        YP2YPERR(YPERR, YPERR),  YP2YPERR(BADARGS, BADARGS),
        YP2YPERR(VERS, VERS)};
...
libnsl_hidden_nolink_def(ypprot_err, GLIBC_2_0)
    ...
    const char* ypbinderr_string(const int error)
{
    const char* str;
    switch (error)
    {
