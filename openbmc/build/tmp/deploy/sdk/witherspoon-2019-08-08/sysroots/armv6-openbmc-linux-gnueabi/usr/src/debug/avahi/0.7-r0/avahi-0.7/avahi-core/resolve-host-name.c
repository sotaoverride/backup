...
static void finish(AvahiSHostNameResolver* r, AvahiResolverEvent event)
{
    ...
    switch (event)
    {
        case AVAHI_RESOLVER_FOUND:
        {
            ...
            r->callback(r, r->interface, r->protocol, AVAHI_RESOLVER_FOUND,
                        r->address_record->key->name, &a, r->flags,
                        r->userdata);
            break;
        }
                            ...
                            void* userdata)
{
    ...
    switch (event)
    {
        case AVAHI_BROWSER_NEW:
            assert(record);
            assert(record->key->type == AVAHI_DNS_TYPE_A ||
                   record->key->type == AVAHI_DNS_TYPE_AAAA);
    ...
    AvahiSHostNameResolverCallback callback, void* userdata)
{
    ...
    AVAHI_CHECK_VALIDITY_RETURN_NULL(server, avahi_is_valid_fqdn(host_name),
    ...
    AVAHI_LLIST_PREPEND(AvahiSHostNameResolver, resolver,
                        server->host_name_resolvers, r);
    ...
    if (aprotocol == AVAHI_PROTO_INET || aprotocol == AVAHI_PROTO_UNSPEC)
    {
        ...
        r->record_browser_a = avahi_s_record_browser_new(
            server, interface, protocol, k, flags, record_browser_callback, r);
        avahi_key_unref(k);
    ...
    if (aprotocol == AVAHI_PROTO_INET6 || aprotocol == AVAHI_PROTO_UNSPEC)
    {
        ...
        r->record_browser_aaaa = avahi_s_record_browser_new(
            server, interface, protocol, k, flags, record_browser_callback, r);
        avahi_key_unref(k);
...
void avahi_s_host_name_resolver_free(AvahiSHostNameResolver* r)
{
    ...
    AVAHI_LLIST_REMOVE(AvahiSHostNameResolver, resolver,
                       r->server->host_name_resolvers, r);
