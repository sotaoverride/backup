...
static int dns_query_candidate_new(DnsQueryCandidate** ret, DnsQuery* q,
                                   DnsScope* s)
{
    DnsQueryCandidate* c;
...
static int dns_query_candidate_add_transaction(DnsQueryCandidate* c,
                                               DnsResourceKey* key)
{
    DnsTransaction* t;
    int r;
...
static bool dns_query_candidate_is_routable(DnsQueryCandidate* c, uint16_t type)
{
    ...
    /* Checks whether the specified RR type matches an address family that is
     * routable on the link(s) the scope of this candidate belongs to.
     * Specifically, whether there's a routable IPv4 address on it if we query
     * an A RR, or a routable IPv6 address if we query an AAAA RR. */
...
static int dns_query_candidate_setup_transactions(DnsQueryCandidate* c)
{
    ...
    DNS_QUESTION_FOREACH(key, question)
    {
        ...
        if (c->search_domain)
        {
            r = dns_resource_key_new_append_suffix(&new_key, key,
                                                   c->search_domain->name);
            if (r < 0)
                goto fail;
...
DnsQuery* dns_query_free(DnsQuery* q)
{
    ...
    {
        /* Detach the stream from our query, in case something else keeps a
         * reference to it. */
...
int dns_query_new(Manager* m, DnsQuery** ret, DnsQuestion* question_utf8,
                  DnsQuestion* question_idna, int ifindex, uint64_t flags)
{
    ...
    /* And then dump the IDNA question, but only what hasn't been dumped already
     * through the UTF8 question. */
...
static int dns_query_add_candidate(DnsQuery* q, DnsScope* s)
{
    ...
    /* If this a single-label domain on DNS, we might append a suitable search
     * domain first. */
            ...
            s, dns_question_first_name(q->question_idna)))
    {
        ...
        r = dns_query_candidate_next_search_domain(c);
        if (r <= 0) /* if there's no search domain, then we won't add any
                       transaction. */
            goto fail;
    }
...
static int dns_query_synthesize_reply(DnsQuery* q, DnsTransactionState* state)
{
     ...
     * Note that this is done *after* the the normal lookup finished. The data
     * from the network hence takes precedence over the data we
     * synthesize. (But note that many scopes refuse to resolve certain domain
     * names) */

    if (!IN_SET(*state, DNS_TRANSACTION_RCODE_FAILURE,
                DNS_TRANSACTION_NO_SERVERS, DNS_TRANSACTION_TIMEOUT,
                DNS_TRANSACTION_ATTEMPTS_MAX_REACHED,
                DNS_TRANSACTION_NETWORK_DOWN, DNS_TRANSACTION_NOT_FOUND))
        return 0;

    r = dns_synthesize_answer(q->manager, q->question_utf8, q->ifindex,
                              &answer);
    if (r == -ENXIO)
    {
...
static int dns_query_try_etc_hosts(DnsQuery* q)
{
    ...
    /* Looks in /etc/hosts for matching entries. Note that this is done *before*
     * the normal lookup is done. The data from /etc/hosts hence takes
     * precedence over the network. */
...
int dns_query_go(DnsQuery* q)
{
    ...
    LIST_FOREACH(scopes, s, q->manager->dns_scopes)
    {
        ...
        name = dns_question_first_name(
            dns_query_question_for_protocol(q, s->protocol));
        if (!name)
            continue;
        ...
        if (match > found)
        { /* Does this match better? If so, remember how well it matched, and
           * the first one that matches this well */
            found = match;
            first = s;
        }
    }
    ...
    r = sd_event_add_time(q->manager->event, &q->timeout_event_source,
    ...
    (void)sd_event_source_set_description(q->timeout_event_source,
                                          "query-timeout");
...
static void dns_query_accept(DnsQuery* q, DnsQueryCandidate* c)
{
    ...
    SET_FOREACH(t, c->transactions, i)
    {
        ...
        switch (t->state)
        {
            ...
            default:
                /* Any kind of failure? Store the data away, if there's nothing
                 * stored yet. */
                if (state == DNS_TRANSACTION_SUCCESS)
                    continue;

                /* If there's already an authenticated negative reply stored,
                 * then prefer that over any unauthenticated one */
    ...
    if (state == DNS_TRANSACTION_SUCCESS)
    {
        ...
        q->answer_dnssec_result = q->answer_authenticated
...
static int dns_query_cname_redirect(DnsQuery* q, const DnsResourceRecord* cname)
{
    ...
    {
        /* Don't permit CNAME redirects from unicast DNS to LLMNR or
         * MulticastDNS, so that global resources cannot invade the local
         * namespace. The opposite way we permit: local names may redirect to
         * global ones. */

        q->flags &= ~(SD_RESOLVED_LLMNR |
                      SD_RESOLVED_MDNS); /* mask away the local protocols */
    }
...
int dns_query_process_cname(DnsQuery* q)
{
    ...
    DNS_ANSWER_FOREACH(rr, q->answer)
    {
        ...
        if (r < 0)
            return r;
        if (r > 0)
            return DNS_QUERY_MATCH; /* The answer matches directly, no need to
                                       follow cnames */
...
int dns_query_bus_track(DnsQuery* q, sd_bus_message* m)
{
    ...
    if (!q->bus_track)
    {
        r = sd_bus_track_new(sd_bus_message_get_bus(m), &q->bus_track,
                             on_bus_track, q);
        if (r < 0)
            return r;
    }
...
const char* dns_query_string(DnsQuery* q)
{
    ...
    {
        r = in_addr_to_string(q->request_family, &q->request_address,
                              &q->request_address_string);
        if (r >= 0)
            return q->request_address_string;
    }
