...
int ima_setup(void)
{
    ...
    if (access(IMA_SECFS_DIR, F_OK) < 0)
    {
        log_debug_errno(errno,
                        "IMA support is disabled in the kernel, ignoring: %m");
        return 0;
    }
    ...
    if (access(IMA_SECFS_POLICY, W_OK) < 0)
    {
        log_warning_errno(
            errno,
            "Another IMA custom policy has already been loaded, ignoring: %m");
        return 0;
    }

    if (access(IMA_POLICY_PATH, F_OK) < 0)
    {
        log_debug_errno(errno, "No IMA custom policy file " IMA_POLICY_PATH
                               ", ignoring: %m");
        return 0;
    }
    ...
    {
        log_error_errno(
            errno, "Failed to open the IMA kernel interface " IMA_SECFS_POLICY
                   ", ignoring: %m");
        return 0;
    }
    ...
    {
        log_warning_errno(
            errno, "Failed to open the IMA custom policy file " IMA_POLICY_PATH
                   ", ignoring: %m");
        return 0;
    }
    ...
    for (;;)
    {
        ...
        if (r < 0)
            return log_error_errno(
                r, "Failed to read the IMA custom policy file " IMA_POLICY_PATH
                   ": %m");
        if (r == 0)
            break;
