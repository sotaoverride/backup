             ...
             int verbose)
{
    ...
    for (eb = 0; eb < mtd->eb_cnt; eb++)
    {
        ...
        if (pr)
        {
            printf("\r" PROGRAM_NAME
                   ": scanning eraseblock %d -- %2lld %% complete  ",
        ...
        else
        {
            if ((int)be32_to_cpu(ech.vid_hdr_offset) != si->vid_hdr_offs)
            {
                ...
                warnmsg("inconsistent VID header offset: was "
                        ...
                        si->vid_hdr_offs, be32_to_cpu(ech.vid_hdr_offset), eb);
                warnmsg("treat eraseblock %d as corrupted", eb);
                si->corrupted_cnt += 1;
                si->ec[eb] = EB_CORRUPTED;
                continue;
            ...
            if ((int)be32_to_cpu(ech.data_offset) != si->data_offs)
            {
                ...
                warnmsg("inconsistent data offset: was %d, but"
                        ...
                        si->data_offs, be32_to_cpu(ech.data_offset), eb);
                warnmsg("treat eraseblock %d as corrupted", eb);
                si->corrupted_cnt += 1;
                si->ec[eb] = EB_CORRUPTED;
                continue;
    ...
    verbose(v,
            "finished, mean EC %lld, %d OK, %d corrupted, %d empty, %d "
            "alien, bad %d",
            si->mean_ec, si->ok_cnt, si->corrupted_cnt, si->empty_cnt,
            si->alien_cnt, si->bad_cnt);
