...
static void* (*old_memalign_hook)(size_t alignment, size_t size, const void*);
...
static void link_blk(struct hdr* hdr)
{
    ...
    if (hdr->next != NULL)
    {
        hdr->next->prev = hdr;
        hdr->next->magic =
            MAGICWORD ^ ((uintptr_t)hdr + (uintptr_t)hdr->next->next);
    }
}
...
static void* reallochook(void* ptr, size_t size, const void* caller)
{
    ...
    if (old_realloc_hook != NULL)
        hdr = (struct hdr*)(*old_realloc_hook)(
            (void*)hdr, sizeof(struct hdr) + size + 1, caller);
    else
        hdr = (struct hdr*)realloc((void*)hdr, sizeof(struct hdr) + size + 1);
    __free_hook = freehook;
    __malloc_hook = mallochook;
    __memalign_hook = memalignhook;
...
__attribute__((noreturn)) static void mabort(enum mcheck_status status)
{
    const char* msg;
    switch (status)
    {
