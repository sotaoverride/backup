...
int nl_addr_parse(const char* addrstr, int hint, struct nl_addr** result)
{
    ...
    if (!strcasecmp(str, "default") || !strcasecmp(str, "all") ||
        !strcasecmp(str, "any"))
    {
    ...
    if ((hint == AF_LLC || hint == AF_UNSPEC) && strchr(str, ':'))
    {
        ...
        if (sscanf(str, "%02x:%02x:%02x:%02x:%02x:%02x", &a, &b, &c, &d, &e,
                   &f) == 6)
        {
            family = AF_LLC;
            len = 6;
...
int nl_addr_cmp_prefix(const struct nl_addr* a, const struct nl_addr* b)
{
    ...
    if (d == 0)
    {
        ...
        if (d == 0 && (len % 8) != 0)
        {
            ...
            d = (a->a_addr[bytes] & mask) - (b->a_addr[bytes] & mask);
        }
    }
...
int nl_addr_resolve(const struct nl_addr* addr, char* host, size_t hostlen)
{
    ...
    err = getnameinfo((struct sockaddr*)&buf, salen, host, hostlen, NULL, 0,
                      NI_NAMEREQD);
    if (err < 0)
...
char* nl_addr2str(const struct nl_addr* addr, char* buf, size_t size)
{
    ...
    switch (addr->a_family)
    {
        ...
        case AF_LLC:
        default:
            snprintf(buf, size, "%02x", (unsigned char)addr->a_addr[0]);
...
prefix:
    if (addr->a_family != AF_MPLS && addr->a_prefixlen != (8 * addr->a_len))
    {
