...
{
    ostream_wrapper(std::ostream& sout) : sout_(sout)
    {
    }
...
{
    template <typename T>
    named_any(T const&) : name_(BOOST_SP_TYPEID(T).name())
    {
    }
...
struct display_expr_impl
{
    explicit display_expr_impl(std::ostream& sout, int depth = 0) :
        depth_(depth), first_(true), sout_(sout)
    {
    }
...
///
/// \note Equivalent to <tt>BOOST_MPL_ASSERT((proto::matches\<Expr,
/// Grammar\>))</tt> \param expr The Proto expression to check againts
/// <tt>Grammar</tt>
...
///
/// \note Equivalent to <tt>BOOST_MPL_ASSERT_NOT((proto::matches\<Expr,
/// Grammar\>))</tt> \param expr The Proto expression to check againts
/// <tt>Grammar</tt>
template <typename Grammar, typename Expr>
