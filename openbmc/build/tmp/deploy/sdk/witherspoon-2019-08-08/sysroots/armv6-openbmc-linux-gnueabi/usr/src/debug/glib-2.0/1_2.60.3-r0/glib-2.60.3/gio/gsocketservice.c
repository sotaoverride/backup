...
G_DEFINE_TYPE_WITH_PRIVATE(GSocketService, g_socket_service,
...
static void g_socket_service_ready(GObject* object, GAsyncResult* result,
                                   gpointer user_data);

static gboolean g_socket_service_real_incoming(GSocketService* service,
...
static gboolean get_active(GSocketService* service)
{
    gboolean active;
...
static void set_active(GSocketService* service, gboolean active)
{
...
static void g_socket_service_get_property(GObject* object, guint prop_id,
...
static void g_socket_service_set_property(GObject* object, guint prop_id,
                                          const GValue* value,
                                          GParamSpec* pspec)
{
    GSocketService* service = G_SOCKET_SERVICE(object);
...
static gboolean g_socket_service_incoming(GSocketService* service,
                                          GSocketConnection* connection,
                                          GObject* source_object)
{
    gboolean result;

    g_signal_emit(service, g_socket_service_incoming_signal, 0, connection,
...
static void g_socket_service_class_init(GSocketServiceClass* class)
{
     ...
     * Since: 2.22
     */
    g_socket_service_incoming_signal = g_signal_new(
        I_("incoming"), G_TYPE_FROM_CLASS(class), G_SIGNAL_RUN_LAST,
    ...
    g_object_class_install_property(
        ...
        g_param_spec_boolean(
            "active", P_("Active"),
            P_("Whether the service is currently accepting connections"), TRUE,
                                   ...
                                   gpointer user_data)
{
    ...
    connection = g_socket_listener_accept_finish(listener, result,
                                                 &source_object, &error);
    if (error)
    {
