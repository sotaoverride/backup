...
static unsigned int find_media_headers(void)
{
    ...
    {
        pread_nocheck(fd, &MedHead[NumMedHeads], sizeof(struct NFTLMediaHeader),
                      ofs);
        ...
        if (NumMedHeads == 2)
        {
            if (strncmp((char*)&MedHead[0], (char*)&MedHead[1],
                        sizeof(struct NFTLMediaHeader)) != 0)
            {
...
static void dump_erase_units(void)
{
    ...
    for (i = MedHead[0].FirstPhysicalEUN;
         i < MedHead[0].FirstPhysicalEUN + MedHead[0].NumEraseUnits; i++)
    {
        /* For each Erase Unit */
        ...
        else
        {
            ...
            {
                /* If this is the first in a chain, store the EUN in the VUC
                 * table */
                if (VUCtable[UCItable[i][0].a.VirtUnitNum & 0x7fff])
                {
                    printf("Duplicate start of chain for VUC %d: "
                           "Unit %d replaces Unit %d\n",
                           UCItable[i][0].a.VirtUnitNum & 0x7fff, i,
                           VUCtable[UCItable[i][0].a.VirtUnitNum & 0x7fff]);
                }
