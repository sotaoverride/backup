...
static const struct colinfo infos[] = {
    ...
    [COL_NPROCS] = {"NPROCS", 5, SCOLS_FL_RIGHT,
...
static char* ns_names[] = {[LSNS_ID_MNT] = "mnt",      [LSNS_ID_NET] = "net",
                           [LSNS_ID_PID] = "pid",      [LSNS_ID_UTS] = "uts",
                           [LSNS_ID_IPC] = "ipc",      [LSNS_ID_USER] = "user",
...
struct lsns
{
    ...
    unsigned int raw : 1, json : 1, tree : 1, list : 1, no_trunc : 1,
        no_headings : 1, no_wrap : 1;
...
static int netnsid_cache_find(ino_t netino, int* netnsid)
{
    ...
    {
        struct netnsid_cache* e = list_entry(p, struct netnsid_cache, netnsids);
        if (e->ino == netino)
        {
            *netnsid = e->id;
            return 1;
...
{
    unsigned char
        req[NLMSG_SPACE(sizeof(struct rtgenmsg)) + RTA_SPACE(sizeof(int32_t))];

    struct nlmsghdr* nlh = (struct nlmsghdr*)req;
    struct rtgenmsg* rt = NLMSG_DATA(req);
    struct rtattr* rta =
        (struct rtattr*)(req + NLMSG_SPACE(sizeof(struct rtgenmsg)));
...
{
    unsigned char
        res[NLMSG_SPACE(sizeof(struct rtgenmsg)) +
            ((RTA_SPACE(sizeof(int32_t)) < RTA_SPACE(sizeof(struct nlmsgerr)))
...
static struct lsns_namespace* get_namespace(struct lsns* ls, ino_t ino)
{
    ...
    list_for_each(p, &ls->namespaces)
    {
        struct lsns_namespace* ns =
            list_entry(p, struct lsns_namespace, namespaces);
...
static int namespace_has_process(struct lsns_namespace* ns, pid_t pid)
{
    ...
    {
        struct lsns_process* proc =
            list_entry(p, struct lsns_process, ns_siblings[ns->type]);
...
static struct lsns_namespace* add_namespace(struct lsns* ls, int type,
                                            ino_t ino)
{
...
static int add_process_to_namespace(struct lsns* ls, struct lsns_namespace* ns,
                                    struct lsns_process* proc)
{
    struct list_head* p;

    DBG(NS, ul_debugobj(ns, "add process [%p] pid=%d to %s[%ju]", proc,
                        proc->pid, ns_names[ns->type], (uintmax_t)ns->id));

    list_for_each(p, &ls->processes)
    {
        struct lsns_process* xproc =
            list_entry(p, struct lsns_process, processes);
...
{
    struct lsns_namespace *xa =
                              list_entry(a, struct lsns_namespace, namespaces),
                          *xb =
                              list_entry(b, struct lsns_namespace, namespaces);
                            ...
                            const char sep)
{
     ...
     */
    if (tmp == path_set &&
        ((path_set_len == elt_len) || (path_set[elt_len] == sep)))
        return 1;
     ...
     */
    if ((*(tmp - 1) == sep) &&
        ((*(tmp + elt_len) == sep) || (*(tmp + elt_len) == '\0')))
        return 1;

    return 0;
}

static int nsfs_xasputs(char** str, struct lsns_namespace* ns,
                        struct libmnt_table* tab, char sep)
{
    struct libmnt_iter* itr = mnt_new_iter(MNT_ITER_FORWARD);
    char* expected_root;
    struct libmnt_fs* fs = NULL;

    xasprintf(&expected_root, "%s:[%ju]", ns_names[ns->type],
              (uintmax_t)ns->id);
    *str = NULL;

    while (mnt_table_find_next_fs(tab, itr, is_nsfs_root, expected_root, &fs) ==
           0)
    {
                           ...
                           struct lsns_namespace* ns, struct lsns_process* proc)
{
    ...
    line = scols_table_new_line(
        table, ls->tree && proc->parent ? proc->parent->outline : NULL);
    if (!line)
    {
    ...
    for (i = 0; i < ncolumns; i++)
    {
        ...
        switch (get_column_id(i))
        {
            ...
            case COL_COMMAND:
                ...
                break;
            case COL_PATH:
                xasprintf(&str, "/proc/%d/ns/%s", (int)proc->pid,
                          ns_names[ns->type]);
                break;
            case COL_UID:
                         ...
                         struct lsns_process* proc, struct lsns_namespace* ns)
{
     ...
     */
    if (ls->tree && proc->parent && !proc->parent->outline &&
...
static void __attribute__((__noreturn__)) usage(void)
{
    ...
    fprintf(out, _(" %s [options] [<namespace>]\n"),
    ...
    fputs(_(" -W, --nowrap           don't use multi-line representation\n"),
          out);
    fputs(_(" -t, --type <name>      namespace type (mnt, net, ipc, user, pid, "
            "uts, cgroup)\n"),
          out);
...
int main(int argc, char* argv[])
{
    ...
    while ((c = getopt_long(argc, argv, "Jlp:o:nruhVt:W", long_opts, NULL)) !=
           -1)
    {
        ...
        switch (c)
        {
            ...
            case 'p':
                ls.fltr_pid =
                    strtos32_or_err(optarg, _("invalid PID argument"));
                break;
            case 'h':
                usage();
            case 'n':
    ...
    if (optind < argc)
    {
        ...
        ls.fltr_ns =
            strtou64_or_err(argv[optind], _("invalid namespace argument"));
        ls.tree = ls.list ? 0 : 1;
    ...
    if (!r)
    {
        if (ls.fltr_ns)
        {
            ...
            if (!ns)
                errx(EXIT_FAILURE, _("not found namespace: %ju"),
                     (uintmax_t)ls.fltr_ns);
