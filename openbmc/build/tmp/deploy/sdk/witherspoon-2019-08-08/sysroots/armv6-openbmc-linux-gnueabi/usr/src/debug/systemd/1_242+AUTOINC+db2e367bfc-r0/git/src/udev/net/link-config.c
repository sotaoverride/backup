...
int link_load_one(link_config_ctx* ctx, const char* filename)
{
    ...
    *link = (link_config){
    ...
    r = config_parse(NULL, filename, file, "Match\0Link\0",
    ...
    if (set_isempty(link->match_mac) && strv_isempty(link->match_path) &&
        ...
        log_warning("%s: No valid settings found in the [Match] section. "
                    ...
                    "If that is intended, please add OriginalName=* in the "
                    "[Match] section.",
                    filename);
    ...
    {
        log_debug(
            "%s: Conditions do not match the system environment, skipping.",
            filename);
        return 0;
    }
...
static int link_unsigned_attribute(sd_device* device, const char* attr,
                                   unsigned* type)
{
    const char* s;
    int r;
    ...
    if (r < 0)
        return log_device_debug_errno(device, r, "Failed to query %s: %m",
                                      attr);

    r = safe_atou(s, type);
    if (r < 0)
        return log_device_warning_errno(
            device, r, "Failed to parse %s \"%s\": %m", attr, s);
...
int link_config_load(link_config_ctx* ctx)
{
    ...
    if (!enable_name_policy())
    {
        ...
        log_info("Network interface NamePolicy= disabled on kernel command "
                 "line, ignoring.");
    }
...
{
    return paths_check_timestamp(NETWORK_DIRS, &ctx->network_dirs_ts_usec,
                                 false);
}
...
int link_config_get(link_config_ctx* ctx, sd_device* device, link_config** ret)
{
    ...
    {
        const char *address = NULL, *id_path = NULL, *id_net_driver = NULL,
        ...
        (void)sd_device_get_property_value(device, "ID_NET_DRIVER",
                                           &id_net_driver);
        ...
        if (net_match_config(link->match_mac, link->match_path,
                             ...
                             link->match_name,
                             address ? ether_aton(address) : NULL, id_path,
                             id_net_driver, devtype, sysname))
        {
            if (link->match_name)
            {
                unsigned name_assign_type = NET_NAME_UNKNOWN;

                (void)link_unsigned_attribute(device, "name_assign_type",
                                              &name_assign_type);

                if (name_assign_type == NET_NAME_ENUM)
                {
                    log_warning("Config file %s applies to device based on "
                                "potentially unpredictable interface name '%s'",
                ...
                {
                    log_warning("Config file %s matches device based on "
                                "renamed interface name '%s', ignoring",
                                link->filename, sysname);
            ...
            log_debug("Config file %s applies to device %s", link->filename,
                      sysname);
...
static int get_mac(sd_device* device, MACAddressPolicy policy,
                   struct ether_addr* mac)
{
    ...
    assert(IN_SET(policy, MAC_ADDRESS_POLICY_RANDOM,
    ...
    {
        case NET_ADDR_SET:
            return log_device_debug(
                device, "MAC on the device already set by userspace");
        ...
        case NET_ADDR_PERM:
            break;
        default:
            return log_device_warning(
                device, "Unknown addr_assign_type %u, ignoring", addr_type);
    }

    if (want_random == (addr_type == NET_ADDR_RANDOM))
        return log_device_debug(device,
    ...
    else
    {
        ...
        if (r < 0)
            return log_device_warning_errno(
                device, r, "Could not generate persistent MAC: %m");
                      ...
                      sd_device* device, const char** name)
{
    ...
    if (r < 0)
    {
        ...
        if (config->port != _NET_DEV_PORT_INVALID)
            log_warning_errno(r, "Could not set port (%s) of %s: %m",
                              port_to_string(config->port), old_name);

        if (!eqzero(config->advertise))
            log_warning_errno(
                r,
                "Could not set advertise mode: %m"); /* TODO: include modes in
                                                        the log message. */
        ...
        if (config->speed)
        {
            ...
            if (r == -EOPNOTSUPP)
            {
                r = ethtool_set_speed(&ctx->ethtool_fd, old_name, speed,
                                      config->duplex);
                if (r < 0)
                    log_warning_errno(
                        r, "Could not set speed of %s to %u Mbps: %m", old_name,
                        speed);
            }
        }

        if (config->duplex != _DUP_INVALID)
            log_warning_errno(r, "Could not set duplex of %s to (%s): %m",
                              old_name, duplex_to_string(config->duplex));
    }
    ...
    if (r < 0)
        log_warning_errno(r, "Could not set offload features of %s: %m",
                          old_name);
    ...
    if (ctx->enable_name_policy && config->name_policy)
        for (NamePolicy* p = config->name_policy;
             !new_name && *p != _NAMEPOLICY_INVALID; p++)
        {
            policy = *p;
            ...
            switch (policy)
            {
                case NAMEPOLICY_KERNEL:
                     ...
                     * it. */
                    log_device_debug(
                        device, "Policy *%s*: keeping predictable kernel name",
                ...
                case NAMEPOLICY_KEEP:
                    ...
                    log_device_debug(
                        device, "Policy *%s*: keeping existing userspace name",
                        name_policy_to_string(policy));
                    goto no_rename;
                ...
                case NAMEPOLICY_ONBOARD:
                    (void)sd_device_get_property_value(
                        device, "ID_NET_NAME_ONBOARD", &new_name);
                    break;
                case NAMEPOLICY_SLOT:
                    (void)sd_device_get_property_value(
                        device, "ID_NET_NAME_SLOT", &new_name);
                    break;
                case NAMEPOLICY_PATH:
                    (void)sd_device_get_property_value(
                        device, "ID_NET_NAME_PATH", &new_name);
                    break;
                case NAMEPOLICY_MAC:
                    (void)sd_device_get_property_value(
                        device, "ID_NET_NAME_MAC", &new_name);
                    break;
                default:
    ...
    if (new_name)
        log_device_debug(device, "Policy *%s* yields \"%s\".",
                         name_policy_to_string(policy), new_name);
    else if (config->name)
    {
        new_name = config->name;
        log_device_debug(
            device, "Policies didn't yield a name, using specified Name=%s.",
            new_name);
    }
    else
        log_device_debug(device, "Policies didn't yield a name and Name= is "
                                 "not given, not renaming.");
no_rename:

    if (IN_SET(config->mac_address_policy, MAC_ADDRESS_POLICY_PERSISTENT,
    ...
    if (r < 0)
        return log_warning_errno(
            r, "Could not set Alias=, MACAddress= or MTU= on %s: %m", old_name);
...
DEFINE_CONFIG_PARSE_ENUM(config_parse_mac_address_policy, mac_address_policy,
                         MACAddressPolicy,
                         "Failed to parse MAC address policy");

static const char* const name_policy_table[_NAMEPOLICY_MAX] = {
    [NAMEPOLICY_KERNEL] = "kernel",     [NAMEPOLICY_KEEP] = "keep",
    [NAMEPOLICY_DATABASE] = "database", [NAMEPOLICY_ONBOARD] = "onboard",
    [NAMEPOLICY_SLOT] = "slot",         [NAMEPOLICY_PATH] = "path",
    [NAMEPOLICY_MAC] = "mac",
};
