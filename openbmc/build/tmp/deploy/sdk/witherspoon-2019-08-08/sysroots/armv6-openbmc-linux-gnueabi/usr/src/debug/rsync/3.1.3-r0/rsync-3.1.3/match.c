                    ...
                    OFF_T offset, int32 i)
{
    ...
    {
        rprintf(FINFO, "match at %s last_match=%s j=%d len=%ld n=%ld\n",
                big_num(offset), big_num(last_match), i, (long)s->sums[i].len,
                (long)n);
    }
...
static void hash_search(int f, struct sum_struct* s, struct map_struct* buf,
                        OFF_T len)
{
    ...
    do
    {
        ...
        do
        {
            ...
            if (DEBUG_GTE(DELTASUM, 3))
            {
                rprintf(FINFO, "potential match at %s i=%ld sum=%08x\n",
            ...
            if (updating_basis_file)
            {
                ...
                if ((offset == aligned_offset || (sum == 0 && l == s->blength &&
                                                  aligned_offset + l <= len)) &&
                    aligned_i < s->count)
                {
                    if (i != aligned_i)
                    {
                        if (sum != s->sums[aligned_i].sum1 ||
                            ...
                            memcmp(sum2, s->sums[aligned_i].sum2,
                                   s->s2length) != 0)
                            goto check_want_i;
                        i = aligned_i;
                    }
                    if (offset != aligned_offset)
                    {
                         ...
                         * further along in the basis file, if we find zeros
                         * ahead in the sender's file, we'll output enough
                         * literal data to re-align with the basis file, and get
                         * back to seeking instead of writing. */
                        backup = (int32)(aligned_offset - last_match);
                        if (backup < 0)
                            backup = 0;
                        map = (schar*)map_ptr(buf, aligned_offset - backup,
                                              l + backup) +
                              backup;
                    ...
                    /* This identical chunk is in the same spot in the old and
                     * new file. */
                    s->sums[i].flags |= SUMFLG_SAME_OFFSET;
                    want_i = i;
                }
    ...
    if (append_mode > 0)
    {
        if (append_mode == 2)
        {
            ...
            for (j = CHUNK_SIZE; j < s->flength; j += CHUNK_SIZE)
            {
                if (buf && INFO_GTE(PROGRESS, 1))
                    show_progress(last_match, buf->file_size);
                sum_update(map_ptr(buf, last_match, CHUNK_SIZE), CHUNK_SIZE);
                last_match = j;
            }
...
void match_report(void)
{
    ...
    rprintf(FINFO, "total: matches=%d  hash_hits=%d  false_alarms=%d data=%s\n",
