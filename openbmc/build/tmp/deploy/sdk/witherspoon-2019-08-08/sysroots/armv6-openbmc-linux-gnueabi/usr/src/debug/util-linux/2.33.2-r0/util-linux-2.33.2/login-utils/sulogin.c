...
static void usage(void)
{
    ...
    fprintf(out, _(" %s [options] [tty device]\n"),
    ...
    fputs(_(" -p, --login-shell        start a login shell\n"
            " -t, --timeout <seconds>  max time to wait for a password "
            "(default: no limit)\n"
            " -e, --force              examine password files directly if "
            "getpwnam(3) fails\n"),
          out);
        ...
        switch ((con->pid = fork()))
        {
            ...
            nofork:
                ...
                while (1)
                {
                    ...
                    int doshell = 0;
                    int deny =
                        !opt_e && locked_account_password(pwd->pw_passwd);
