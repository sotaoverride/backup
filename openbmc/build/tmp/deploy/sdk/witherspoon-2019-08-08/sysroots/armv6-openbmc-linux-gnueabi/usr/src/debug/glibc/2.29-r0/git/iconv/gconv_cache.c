...
static int find_module_idx(const char* str, size_t* idxp)
{
    ...
    hashtab = (struct hash_entry*)((char*)gconv_cache + header->hash_offset);
...
int __gconv_compare_alias_cache(const char* name1, const char* name2,
                                int* result)
{
    size_t name1_idx;
    size_t name2_idx;
        ...
        __builtin_expect(from_module->extra_offset, 0) != 0)
    {
       ...
       See there for more explanations.  */
        ...
        while (extra->module_cnt != 0 &&
            ...
            extra =
                (const struct extra_entry*)((char*)extra +
        ...
        if (extra->module_cnt != 0)
        {
            ...
            *handle = result = (struct __gconv_step*)malloc(
                extra->module_cnt * sizeof(struct __gconv_step));
            if (result == NULL)
            ...
            do
            {
                ...
                fromname = result[idx].__to_name =
                    (char*)strtab +
                    modtab[extra->module[idx].outname_offset].canonname_offset;
                ...
                if (strtab[extra->module[idx].dir_offset] != '\0')
                {
                    ...
                    __gconv_get_builtin_trans(
                        strtab + extra->module[idx].name_offset, &result[idx]);
...
try_internal:
    ...
    if ((fromidx != 0 &&
         __builtin_expect(from_module->fromname_offset, 1) == 0) ||
    ...
    *nsteps = 0;
    ...
    if (fromidx != 0)
    {
        ...
        if (strtab[from_module->todir_offset] != '\0')
        {
            /* Load the module, return handle for it.  */
            int res =
                find_module(strtab + from_module->todir_offset,
                            strtab + from_module->toname_offset, &result[0]);
            if (__builtin_expect(res, __GCONV_OK) != __GCONV_OK)
            {
                /* Something went wrong.  */
                free(result);
    ...
    if (toidx != 0)
    {
        ...
        if (strtab[to_module->fromdir_offset] != '\0')
        {
            /* Load the module, return handle for it.  */
            int res =
                find_module(strtab + to_module->fromdir_offset,
                            strtab + to_module->fromname_offset, &result[idx]);
            if (__builtin_expect(res, __GCONV_OK) != __GCONV_OK)
            {
                /* Something went wrong.  */
                if (idx != 0)
