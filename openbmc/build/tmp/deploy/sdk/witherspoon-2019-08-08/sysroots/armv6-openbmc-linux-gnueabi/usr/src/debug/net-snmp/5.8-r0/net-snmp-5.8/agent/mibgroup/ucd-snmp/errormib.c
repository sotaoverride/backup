...
void init_errormib(void)
{
     ...
     * information at
     */
    struct variable2 extensible_error_variables[] = {{MIBINDEX,
                                                      ASN_INTEGER,
     ...
     */
    oid extensible_error_variables_oid[] = {NETSNMP_UCDAVIS_MIB,
                                            NETSNMP_ERRORMIBNUM};

    /*
     * register ourselves with the agent to handle our mib tree
     */
    REGISTER_MIB("ucd-snmp/errormib", extensible_error_variables, variable2,
                 extensible_error_variables_oid);
}
 ...
 *
 */
u_char* var_extensible_errors(struct variable* vp, oid* name, size_t* length,
                              ...
                              WriteMethod** write_method)
{
    ...
    switch (vp->magic)
    {
        ...
        case ERRORFLAG:
            long_ret = (NETSNMP_ERRORTIMELENGTH >= time(NULL) - errorstatustime)
                           ? 1
                           : 0;
            return ((u_char*)(&long_ret));
        case ERRORMSG:
            if ((NETSNMP_ERRORTIMELENGTH >= time(NULL) - errorstatustime) ? 1
                                                                          : 0)
            {
