...
#define OPB_STAT_ERR_BASE                                                      \
    (OPB_STAT_ANY_ERR | OPB_STAT_ERR_OPB | OPB_STAT_ERRACK)
...
static int fsi2pib_putscom(struct pib* pib, uint64_t addr, uint64_t value)
{
    ...
    CHECK_ERR(
        fsi_write(&pib->target, FSI_DATA0_REG, (value >> 32) & 0xffffffff));
...
__attribute__((constructor)) static void register_cfam(void)
{
