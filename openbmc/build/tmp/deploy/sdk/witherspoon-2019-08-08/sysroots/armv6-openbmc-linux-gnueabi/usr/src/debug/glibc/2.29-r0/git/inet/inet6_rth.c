...
void* inet6_rth_init(void* bp, socklen_t bp_len, int type, int segments)
{
    ...
    {
        case IPV6_RTHDR_TYPE_0:
            /* Make sure the parameters are valid and the buffer is large
             * enough.  */
            if (segments < 0 || segments > 127)
                break;
...
int inet6_rth_add(void* bp, const struct in6_addr* addr)
{
    ...
    switch (rthdr->ip6r_type)
    {
        ...
        case IPV6_RTHDR_TYPE_0:
            ...
            memcpy(&rthdr0->ip6r0_addr[rthdr0->ip6r0_segleft++], addr,
    ...
    switch (in_rthdr->ip6r_type)
    {
        ...
        case IPV6_RTHDR_TYPE_0:
            ...
            for (int i = 0; i < total / 2; ++i)
            {
                ...
                out_rthdr0->ip6r0_addr[i] =
                    in_rthdr0->ip6r0_addr[total - 1 - i];
                out_rthdr0->ip6r0_addr[total - 1 - i] = temp;
            }
            if (total % 2 != 0 && in != out)
                out_rthdr0->ip6r0_addr[total / 2] =
                    in_rthdr0->ip6r0_addr[total / 2];
