...
static char doc[] = "Check the randomness of data using FIPS 140-2 RNG tests.\n"
                    "\v"
                    "FIPS tests operate on 20000-bit blocks.  Data is read "
                    "from stdin.  Statistics "
                    "and messages are sent to stderr.\n\n"
                    "If no errors happen nor any blocks fail the FIPS tests, "
                    "the program will return "
                    "exit status 0.  If any blocks fail the tests, the exit "
                    "status will be 1.\n";

static struct argp_option options[] = {
    {"blockcount", 'c', "n", 0, "Exit after processing n blocks (default: 0)"},

    {"pipe", 'p', 0, 0,
     "Enable pipe mode: work silently, and echo to stdout all good blocks"},

    {"timedstats", 't', "n", 0, "Dump statistics every n secods (default: 0)"},

    {"blockstats", 'b', "n", 0, "Dump statistics every n blocks (default: 0)"},
...
static void init_sighandlers(void)
{
    ...
    {
        fprintf(stderr, "unable to install signal handler for SIGTERM: %s",
                strerror(errno));
        exit(EXIT_OSERR);
    }
    if (sigaction(SIGINT, &action, NULL) < 0)
    {
        fprintf(stderr, "unable to install signal handler for SIGINT: %s",
                strerror(errno));
        exit(EXIT_OSERR);
    }
}
...
static int xwrite(void* buf, size_t size)
{
    ...
    while (size)
    {
        ...
        {
            fprintf(stderr, "%swrite channel stuck\n", logprefix);
            exitstatus = EXIT_IOERR;
            return -1;
        }
        off += r;
    ...
    if (size)
    {
        fprintf(stderr, "%serror writing to output: %s\n", logprefix,
                strerror(errno));
        exitstatus = EXIT_IOERR;
        return -1;
    }
...
static void dump_rng_stats(void)
{
    ...
    fprintf(stderr, "%s\n",
            dump_stat_counter(buf, sizeof(buf), "bits received from input",
                              rng_stats.bytes_received * 8));
    if (arguments->pipemode)
        fprintf(stderr, "%s\n",
                dump_stat_counter(buf, sizeof(buf), "bits sent to output",
                                  rng_stats.bytes_sent * 8));
    fprintf(stderr, "%s\n",
            dump_stat_counter(buf, sizeof(buf), "FIPS 140-2 successes",
                              rng_stats.good_fips_blocks));
    fprintf(stderr, "%s\n",
            dump_stat_counter(buf, sizeof(buf), "FIPS 140-2 failures",
    ...
    fprintf(stderr, "%s\n",
            dump_stat_bw(buf, sizeof(buf), "input channel speed", "bits",
                         &rng_stats.source_blockfill,
                         FIPS_RNG_BUFFER_SIZE * 8));
    fprintf(stderr, "%s\n",
            dump_stat_bw(buf, sizeof(buf), "FIPS tests speed", "bits",
                         &rng_stats.fips_blockfill, FIPS_RNG_BUFFER_SIZE * 8));
    if (arguments->pipemode)
        fprintf(stderr, "%s\n",
                dump_stat_bw(buf, sizeof(buf), "output channel speed", "bits",
...
static int discard_initial_data(void)
{
    ...
    if (arguments->pipemode)
        if (xread(tempbuf, sizeof tempbuf))
            exit(EXIT_FAIL);
...
static void do_rng_fips_test_loop(void)
{
    ...
    while (!gotsigterm)
    {
        ...
        if (xread(rng_buffer, sizeof(rng_buffer)))
            return;
        gettimeofday(&stop, 0);
        ...
        gettimeofday(&stop, 0);
        update_usectimer_stat(&rng_stats.fips_blockfill, &start, &stop);
        ...
        else
        {
            ...
            if (arguments->pipemode)
            {
                ...
                if (xwrite(rng_buffer, sizeof(rng_buffer)))
                    return;
                gettimeofday(&stop, 0);
                update_usectimer_stat(&rng_stats.sink_blockfill, &start, &stop);
            }
        }

        if (arguments->blockcount && (++runs >= arguments->blockcount))
            break;

        gettimeofday(&now, 0);
        if ((arguments->blockstats && (++statruns >= arguments->blockstats)) ||
