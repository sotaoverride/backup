...
static int tls1_prf_alg(const EVP_MD* md, const unsigned char* sec, size_t slen,
...
static int pkey_tls1_prf_ctrl_str(EVP_PKEY_CTX* ctx, const char* type,
                                  const char* value)
{
    if (value == NULL)
    {
                                ...
                                size_t* keylen)
{
    ...
    return tls1_prf_alg(kctx->md, kctx->sec, kctx->seclen, kctx->seed,
                        kctx->seedlen, key, *keylen);
}

const EVP_PKEY_METHOD tls1_prf_pkey_meth = {EVP_PKEY_TLS1_PRF,
                                            0,
                                            pkey_tls1_prf_init,
                                            0,
...
static int tls1_prf_P_hash(const EVP_MD* md, const unsigned char* sec,
                           size_t sec_len, const unsigned char* seed,
                           size_t seed_len, unsigned char* out, size_t olen)
{
    int chunk;
                        ...
                        unsigned char* out, size_t olen)
{
    ...
    if (EVP_MD_type(md) == NID_md5_sha1)
    {
        size_t i;
        unsigned char* tmp;
        if (!tls1_prf_P_hash(EVP_md5(), sec, slen / 2 + (slen & 1), seed,
                             seed_len, out, olen))
            return 0;
