 ...
 *(IDNA)](http://www.ietf.org/rfc/rfc3490.txt) standards allow for the use of
 *Unicode domain names in applications, while providing backward-compatibility
 *with the old ASCII-only DNS, by defining an ASCII-Compatible Encoding of any
 *given Unicode name, which can be used with non-IDN-aware applications and
 *protocols. (For example, "Παν語.org" maps to "xn--4wa8awb4637h.org".)
 **/
...
static inline gchar encode_digit(guint dig)
{
    if (dig < 26)
        return dig + 'a';
    else
 ...
 * here.
 */
static gboolean punycode_encode(const gchar* input_utf8,
                                gsize input_utf8_length, GString* output)
{
    ...
    input = g_utf8_to_ucs4(input_utf8, input_utf8_length, NULL, &input_length,
                           NULL);
    if (!input)
        return FALSE;
    ...
    while (handled_chars < input_length)
    {
        ...
        for (j = 0; j < input_length; j++)
        {
            ...
            else if (input[j] == n)
            {
                ...
                bias = adapt(delta, handled_chars + 1,
                             handled_chars == num_basic_chars);
                delta = 0;
                handled_chars++;
            }
...
#define idna_is_junk(ch)                                                       \
    ((ch) == 0x00AD || (ch) == 0x1806 || (ch) == 0x200B || (ch) == 0x2060 ||   \
     (ch) == 0xFEFF || (ch) == 0x034F || (ch) == 0x180B || (ch) == 0x180C ||   \
     (ch) == 0x180D || (ch) == 0x200C || (ch) == 0x200D ||                     \
     ((ch) >= 0xFE00 && (ch) <= 0xFE0F))
...
#define idna_is_dot(str)                                                       \
    (((guchar)(str)[0] == '.') ||                                              \
     ((guchar)(str)[0] == 0xE3 && (guchar)(str)[1] == 0x80 &&                  \
      (guchar)(str)[2] == 0x82) ||                                             \
     ((guchar)(str)[0] == 0xEF && (guchar)(str)[1] == 0xBC &&                  \
      (guchar)(str)[2] == 0x8E) ||                                             \
     ((guchar)(str)[0] == 0xEF && (guchar)(str)[1] == 0xBD &&                  \
      (guchar)(str)[2] == 0xA1))
 ...
 */
static gboolean punycode_decode(const gchar* input, gsize input_length,
                                GString* output)
{
    GArray* output_chars;
    gunichar n;
    ...
    {
        output_chars =
            g_array_sized_new(FALSE, FALSE, sizeof(gunichar), split - input);
    ...
    for (i = 0; i < output_chars->len; i++)
        g_string_append_unichar(output,
                                g_array_index(output_chars, gunichar, i));
...
gchar* g_hostname_to_unicode(const gchar* hostname)
{
    ...
    do
    {
        ...
        if (!g_ascii_strncasecmp(hostname, IDNA_ACE_PREFIX,
                                 IDNA_ACE_PREFIX_LEN))
        {
