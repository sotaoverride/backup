...
krb5_error_code kg_make_seq_num(context, key, direction, seqnum, cksum,
                                buf) krb5_context context;
krb5_key key;
int direction;
krb5_ui_4 seqnum;
unsigned char* cksum;
unsigned char* buf;
{
        ...
        key->keyblock.enctype == ENCTYPE_ARCFOUR_HMAC_EXP)
    {
        ...
        return kg_arcfour_docrypt(&key->keyblock, 0, cksum, 8, &plain[0], 8,
                                  buf);
    }
...
krb5_error_code kg_get_seq_num(context, key, cksum, buf, direction,
                               seqnum) krb5_context context;
krb5_key key;
unsigned char* cksum;
unsigned char* buf;
int* direction;
krb5_ui_4* seqnum;
{
    ...
    if (key->keyblock.enctype == ENCTYPE_ARCFOUR_HMAC ||
        key->keyblock.enctype == ENCTYPE_ARCFOUR_HMAC_EXP)
    {
        code = kg_arcfour_docrypt(&key->keyblock, 0, cksum, 8, buf, 8, plain);
    }
    else
    {
    ...
    *direction = plain[4];
    ...
    {
        *seqnum =
            (plain[3] | (plain[2] << 8) | (plain[1] << 16) | (plain[0] << 24));
    }
    else
    {
        *seqnum = ((plain[0]) | (plain[1] << 8) | (plain[2] << 16) |
                   (plain[3] << 24));
    }
