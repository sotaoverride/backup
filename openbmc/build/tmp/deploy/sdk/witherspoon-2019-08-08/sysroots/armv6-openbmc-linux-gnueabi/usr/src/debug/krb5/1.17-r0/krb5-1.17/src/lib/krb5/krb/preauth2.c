...
/*
 * Copyright 1995, 2003, 2008, 2012 by the Massachusetts Institute of
...
static void free_handles(krb5_context context, clpreauth_handle* handles)
{
 ...
 * found found. */
static int search_module_list(clpreauth_handle* handles,
...
static clpreauth_handle find_module(krb5_context context,
...
static int clpreauth_is_real(krb5_context context, clpreauth_handle h,
                             krb5_preauthtype pa_type)
{
    if (h->vt.flags == NULL)
        return 1;
...
static krb5_error_code clpreauth_prep_questions(
    krb5_context context, clpreauth_handle h, krb5_clpreauth_modreq modreq,
    krb5_get_init_creds_opt* opt, krb5_clpreauth_callbacks cb,
    krb5_clpreauth_rock rock, krb5_kdc_req* req, krb5_data* req_body,
...
static krb5_error_code clpreauth_process(
    krb5_context context, clpreauth_handle h, krb5_clpreauth_modreq modreq,
    krb5_get_init_creds_opt* opt, krb5_clpreauth_callbacks cb,
    krb5_clpreauth_rock rock, krb5_kdc_req* req, krb5_data* req_body,
    krb5_data* prev_req, krb5_pa_data* pa_data, krb5_prompter_fct prompter,
    void* prompter_data, krb5_pa_data*** pa_data_out)
{
    return h->vt.process(context, h->data, modreq, opt, cb, rock, req, req_body,
                         prev_req, pa_data, prompter, prompter_data,
                         pa_data_out);
}

static krb5_error_code clpreauth_tryagain(
    krb5_context context, clpreauth_handle h, krb5_clpreauth_modreq modreq,
    krb5_get_init_creds_opt* opt, krb5_clpreauth_callbacks cb,
    krb5_clpreauth_rock rock, krb5_kdc_req* req, krb5_data* req_body,
    krb5_data* prev_req, krb5_preauthtype pa_type, krb5_error* error,
...
static krb5_error_code clpreauth_gic_opts(krb5_context context,
                                          clpreauth_handle h,
...
static void grow_ktypes(krb5_enctype** out_ktypes, int* out_nktypes,
                        krb5_enctype ktype)
{
    int i;
    krb5_enctype* ktypes;
...
static int grow_pa_list(krb5_pa_data*** out_pa_list, int* out_pa_list_size,
...
static krb5_enctype get_etype(krb5_context context, krb5_clpreauth_rock rock)
{
...
static krb5_keyblock* fast_armor(krb5_context context, krb5_clpreauth_rock rock)
{
    return ((krb5_init_creds_context)rock)->fast_state->armor_key;
}

static krb5_error_code get_as_key(krb5_context context,
...
static krb5_error_code set_as_key(krb5_context context,
                                  krb5_clpreauth_rock rock,
...
static krb5_error_code get_preauth_time(krb5_context context,
                                        krb5_clpreauth_rock rock,
...
static krb5_error_code responder_ask_question(krb5_context context,
                                              krb5_clpreauth_rock rock,
                                              const char* question,
                                              const char* challenge)
{
    ...
    if (strcmp(KRB5_RESPONDER_QUESTION_PASSWORD, question) == 0)
        return EINVAL;
    return k5_response_items_ask_question(ctx->rctx.items, question, challenge);
}

static const char* responder_get_answer(krb5_context context,
                                        krb5_clpreauth_rock rock,
                                        const char* question)
{
    krb5_init_creds_context ctx = (krb5_init_creds_context)rock;
...
static void need_as_key(krb5_context context, krb5_clpreauth_rock rock)
{
    krb5_init_creds_context ctx = (krb5_init_creds_context)rock;
...
static const char* get_cc_config(krb5_context context, krb5_clpreauth_rock rock,
                                 const char* key)
{
    krb5_init_creds_context ctx = (krb5_init_creds_context)rock;
    k5_json_value value;
...
static krb5_error_code set_cc_config(krb5_context context,
...
static void disable_fallback(krb5_context context, krb5_clpreauth_rock rock)
{
    ((krb5_init_creds_context)rock)->fallback_disabled = TRUE;
}

static struct krb5_clpreauth_callbacks_st callbacks = {3,
                                                       get_etype,
                                                       fast_armor,
                                                       get_as_key,
                                                       set_as_key,
...
void k5_preauth_prepare_request(krb5_context context,
                                krb5_get_init_creds_opt* opt, krb5_kdc_req* req)
{
    krb5_preauth_context pctx = context->preauth_context;
    clpreauth_handle *hp, h;
    krb5_enctype* ep;
...
static inline krb5_boolean pa_type_allowed(krb5_init_creds_context ctx,
                                           krb5_preauthtype pa_type)
{
...
static krb5_boolean previously_failed(krb5_init_creds_context ctx,
                                      krb5_preauthtype pa_type)
{
    krb5_preauth_req_context reqctx = ctx->preauth_reqctx;
    size_t i;
                    ...
                    krb5_preauthtype* out_type)
{
    ...
    for (real = 0; real <= 1; real++)
    {
        for (pa_ptr = in_pa_list; *pa_ptr != NULL; pa_ptr++)
        {
            ...
            TRACE_PREAUTH_PROCESS(context, h->vt.name, pa->pa_type, real, ret);
            if (mod_pa != NULL)
            {
...
static inline krb5_data padata2data(krb5_pa_data p)
{
    krb5_data d;
    d.magic = KV5M_DATA;
    d.length = p.length;
...
static krb5_error_code get_salt(krb5_context context,
                                krb5_init_creds_context ctx,
                                krb5_pa_data** padata)
{
    krb5_error_code ret;
                                  ...
                                  krb5_pa_data** padata)
{
    ...
    if (entry == NULL)
    {
        ret = (valid_found) ? KRB5_CONFIG_ETYPE_NOSUPP : KRB5_PROG_ETYPE_NOSUPP;
        goto cleanup;
    }
...
static krb5_error_code copy_cookie(krb5_context context,
...
static krb5_error_code add_s4u_x509_user_padata(krb5_context context,
                                    ...
                                    krb5_pa_data*** padata_out)
{
    ...
    *padata_out = NULL;
    ...
    mod_pa = NULL;
    ret = clpreauth_tryagain(
        context, h, modreq, ctx->opt, &callbacks, (krb5_clpreauth_rock)ctx,
        ctx->request, ctx->inner_request_body, ctx->encoded_previous_request,
        pa_type, err, err_padata, ctx->prompter, ctx->prompter_data, &mod_pa);
...
static krb5_error_code fill_response_items(krb5_context context,
                                           krb5_init_creds_context ctx,
                                           krb5_pa_data** in_padata)
{
    krb5_error_code ret;
    ...
    for (i = 0; in_padata[i] != NULL; i++)
    {
        ...
        if (h == NULL)
            continue;
        ret = clpreauth_prep_questions(context, h, modreq, ctx->opt, &callbacks,
                                       (krb5_clpreauth_rock)ctx, ctx->request,
                                       ctx->inner_request_body,
                                       ctx->encoded_previous_request, pa);
        if (ret)
                           ...
                           krb5_preauthtype* pa_type_out)
{
    ...
    *pa_type_out = KRB5_PADATA_NONE;
    ...
    ret = process_pa_data(context, ctx, in_padata, must_preauth, &out_pa_list,
                          &out_pa_list_size, pa_type_out);
    if (ret)
        goto error;
