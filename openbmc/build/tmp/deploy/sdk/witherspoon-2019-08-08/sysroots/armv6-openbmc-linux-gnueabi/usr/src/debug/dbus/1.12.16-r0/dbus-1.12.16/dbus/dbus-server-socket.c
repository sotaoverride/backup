...
struct DBusServerSocket
{
    ...
    DBusSocket*
        fds; /**< File descriptor or DBUS_SOCKET_INVALID if disconnected. */
...
static dbus_bool_t handle_new_client_fd_and_unlock(DBusServer* server,
                                                   DBusSocket client_fd)
{
    ...
    if (!_dbus_transport_set_auth_mechanisms(
            transport, (const char**)server->auth_mechanisms))
    {
...
static dbus_bool_t socket_handle_watch(DBusWatch* watch, unsigned int flags,
                                       void* data)
{
    ...
    if (flags & DBUS_WATCH_READABLE)
    {
        ...
        if (socket_server->noncefile)
            client_fd = _dbus_accept_with_noncefile(listen_fd,
                                                    socket_server->noncefile);
        else
...
static const DBusServerVTable socket_vtable = {socket_finalize,
                                               socket_disconnect};
 ...
 *
 */
DBusServer* _dbus_server_new_for_socket(DBusSocket* fds, int n_fds,
                                        ...
                                        DBusError* error)
{
    ...
    for (i = 0; i < n_fds; i++)
    {
        ...
        watch = _dbus_watch_new(_dbus_socket_get_pollable(fds[i]),
                                DBUS_WATCH_READABLE, TRUE, socket_handle_watch,
                                socket_server, NULL);
        if (watch == NULL)
            goto failed_2;
    ...
    if (!_dbus_server_init_base(&socket_server->base, &socket_vtable, address,
                                error))
        goto failed_2;
 ...
 * @param family
 * @param error location to store reason for failure.
 * @param use_nonce whether to use a nonce for low-level authentication
 * (nonce-tcp transport) or not (tcp transport)
 * @returns the new server, or #NULL on failure.
 */
DBusServer* _dbus_server_new_for_tcp_socket(const char* host, const char* bind,
                                            const char* port,
                                            const char* family,
                                            DBusError* error,
                                            dbus_bool_t use_nonce)
{
    ...
    nlisten_fds = _dbus_listen_tcp_socket(bind, port, family, &port_str,
                                          &listen_fds, error);
    if (nlisten_fds <= 0)
    {
    ...
    if (!_dbus_string_append(&address,
                             use_nonce ? "nonce-tcp:host=" : "tcp:host=") ||
    ...
    if (family && (!_dbus_string_append(&address, ",family=") ||
    ...
    if (use_nonce)
    {
        ...
        if (!_dbus_string_append(&address, ",noncefile=") ||
            !_dbus_address_append_escaped(&address,
                                          _dbus_noncefile_get_path(noncefile)))
        {
            dbus_set_error(error, DBUS_ERROR_NO_MEMORY, NULL);
            goto failed_4;
        }
    ...
    *server_p = NULL;
    ...
    if (strcmp(method, "tcp") == 0 || strcmp(method, "nonce-tcp") == 0)
    {
        ...
        *server_p = _dbus_server_new_for_tcp_socket(
            host, bind, port, family, error,
            strcmp(method, "nonce-tcp") == 0 ? TRUE : FALSE);
