...
static GSequenceNode* node_find(GSequenceNode* haystack, GSequenceNode* needle,
                                GSequenceNode* end,
...
static void node_insert_sorted(GSequenceNode* node, GSequenceNode* new,
                               GSequenceNode* end,
...
static gboolean seq_is_end(GSequence* seq, GSequenceIter* iter)
{
 ...
 */
static gint iter_compare(GSequenceIter* node1, GSequenceIter* node2,
                         gpointer data)
{
 ...
 * Since: 2.14
 */
void g_sequence_foreach_range(GSequenceIter* begin, GSequenceIter* end,
 ...
 * Since: 2.14
 */
void g_sequence_move_range(GSequenceIter* dest, GSequenceIter* begin,
                           GSequenceIter* end)
{
 ...
 * Since: 2.14
 */
void g_sequence_sort(GSequence* seq, GCompareDataFunc cmp_func,
                     gpointer cmp_data)
{
    SortInfo info;
 ...
 * Since: 2.14
 */
void g_sequence_sort_changed(GSequenceIter* iter, GCompareDataFunc cmp_func,
                             gpointer cmp_data)
{
    GSequence* seq;
    SortInfo info;
 ...
 * unsorted.
 *
 * Returns: (transfer none): an #GSequenceIter pointing to the position where
 * @data would have been inserted according to @cmp_func and @cmp_data
 *
 * Since: 2.14
 */
GSequenceIter* g_sequence_search(GSequence* seq, gpointer data,
 ...
 * unsorted.
 *
 * Returns: (transfer none) (nullable): an #GSequenceIter pointing to the
 * position of the first item found equal to @data according to @cmp_func and
 ...
 * Since: 2.14
 */
void g_sequence_sort_iter(GSequence* seq, GSequenceIterCompareFunc cmp_func,
                          gpointer cmp_data)
{
    GSequence* tmp;
    ...
    while (!g_sequence_is_empty(tmp))
    {
        ...
        node_insert_sorted(seq->end_node, node, seq->end_node, cmp_func,
                           cmp_data);
    }
 ...
 * Since: 2.14
 */
GSequenceIter* g_sequence_insert_sorted_iter(GSequence* seq, gpointer data,
                                             GSequenceIterCompareFunc iter_cmp,
                                             gpointer cmp_data)
{
    ...
    node_insert_sorted(seq->end_node, new_node, seq->end_node, iter_cmp,
                       cmp_data);
 ...
 * Since: 2.14
 */
GSequenceIter* g_sequence_search_iter(GSequence* seq, gpointer data,
                                      GSequenceIterCompareFunc iter_cmp,
                                      gpointer cmp_data)
{
    GSequenceNode* node;
    ...
    node = node_find_closest(seq->end_node, dummy, seq->end_node, iter_cmp,
                             cmp_data);
 ...
 *
 * Returns: (transfer none) (nullable): an #GSequenceIter pointing to the
 * position of the first item found equal to @data according to @iter_cmp and
 * @cmp_data, or %NULL if no such item exists
 *
 * Since: 2.28
 */
GSequenceIter* g_sequence_lookup_iter(GSequence* seq, gpointer data,
                                      GSequenceIterCompareFunc iter_cmp,
                                      gpointer cmp_data)
{
    GSequenceNode* node;
 ...
 *
 * Returns: (transfer none): a #GSequenceIter pointing to the next position
 * after @iter
 *
 * Since: 2.14
 */
 ...
 *
 * Returns: (transfer none): a #GSequenceIter which is @delta positions away
 * from @iter
 *
 * Since: 2.14
 */
