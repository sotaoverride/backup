...
/* Initial delay and the interval for printing status messages about running
 * jobs */
#define JOBS_IN_PROGRESS_WAIT_USEC (5 * USEC_PER_SEC)
#define JOBS_IN_PROGRESS_PERIOD_USEC (USEC_PER_SEC / 3)
#define JOBS_IN_PROGRESS_PERIOD_DIVISOR 3

/* If there are more than 1K bus messages queue across our API and direct
 * busses, then let's not add more on top until the queue gets more empty. */
#define MANAGER_BUS_BUSY_THRESHOLD 1024LU

/* How many units and jobs to process of the bus queue before returning to the
 * event loop. */
...
static int manager_dispatch_idle_pipe_fd(sd_event_source* source, int fd,
                                         uint32_t revents, void* userdata);
static int manager_dispatch_user_lookup_fd(sd_event_source* source, int fd,
                                           uint32_t revents, void* userdata);
static int manager_dispatch_jobs_in_progress(sd_event_source* source,
                                             usec_t usec, void* userdata);
static int manager_dispatch_run_queue(sd_event_source* source, void* userdata);
static int manager_dispatch_sigchld(sd_event_source* source, void* userdata);
static int manager_dispatch_timezone_change(sd_event_source* source,
...
static void manager_watch_jobs_in_progress(Manager* m)
{
    ...
    r = sd_event_add_time(m->event, &m->jobs_in_progress_event_source,
                          CLOCK_MONOTONIC, next, 0,
                          manager_dispatch_jobs_in_progress, m);
    if (r < 0)
        return;

    (void)sd_event_source_set_description(m->jobs_in_progress_event_source,
                                          "manager-jobs-in-progress");
}

#define CYLON_BUFFER_EXTRA                                                     \
    (2 * STRLEN(ANSI_RED) + STRLEN(ANSI_HIGHLIGHT_RED) +                       \
     2 * STRLEN(ANSI_NORMAL))
                       ...
                       unsigned pos)
{
    ...
    assert(pos <= width + 1); /* 0 or width+1 mean that the center light is
                                 behind the corner */
...
static void manager_print_jobs_in_progress(Manager* m)
{
    ...
    print_nr =
        (m->jobs_in_progress_iteration / JOBS_IN_PROGRESS_PERIOD_DIVISOR) %
        m->n_running_jobs;
    ...
    format_timespan(time, sizeof(time), now(CLOCK_MONOTONIC) - j->begin_usec,
                    1 * USEC_PER_SEC);
    ...
    manager_status_printf(m, STATUS_TYPE_EPHEMERAL, cylon,
                          ...
                          strempty(job_of_n), job_type_to_string(j->type),
                          unit_description(j->unit), time, limit);
}
...
static int manager_dispatch_ask_password_fd(sd_event_source* source, int fd,
                                            uint32_t revents, void* userdata)
{
    Manager* m = userdata;
...
static int manager_check_ask_password(Manager* m)
{
    ...
    if (!m->ask_password_event_source)
    {
        ...
        {
            log_error_errno(
                errno, "Failed to watch \"/run/systemd/ask-password\": %m");
        ...
        (void)sd_event_source_set_description(m->ask_password_event_source,
...
static int manager_watch_idle_pipe(Manager* m)
{
    ...
    r = sd_event_add_io(m->event, &m->idle_pipe_event_source, m->idle_pipe[2],
                        EPOLLIN, manager_dispatch_idle_pipe_fd, m);
    if (r < 0)
        return log_error_errno(r, "Failed to watch idle pipe: %m");

    (void)sd_event_source_set_description(m->idle_pipe_event_source,
                                          "manager-idle-pipe");
...
static int manager_setup_time_change(Manager* m)
{
    ...
    if (m->time_change_fd < 0)
        return log_error_errno(m->time_change_fd,
                               "Failed to create timer change timer fd: %m");

    r = sd_event_add_io(m->event, &m->time_change_event_source,
                        m->time_change_fd, EPOLLIN,
                        manager_dispatch_time_change_fd, m);
    if (r < 0)
        return log_error_errno(r,
                               "Failed to create time change event source: %m");

    /* Schedule this slightly earlier than the .timer event sources */
    r = sd_event_source_set_priority(m->time_change_event_source,
                                     SD_EVENT_PRIORITY_NORMAL - 1);
    if (r < 0)
        return log_error_errno(
            r, "Failed to set priority of time change event sources: %m");

    (void)sd_event_source_set_description(m->time_change_event_source,
                                          "manager-time-change");
...
static int manager_setup_timezone_change(Manager* m)
{
    ...
    /* We watch /etc/localtime for three events: change of the link count (which
     * might mean removal from /etc even though another link might be kept),
     * renames, and file close operations after writing. Note we don't bother
     * with IN_DELETE_SELF, as that would just report when the inode is removed
     * entirely, i.e. after the link count went to zero and all fds to it are
     * closed.
     *
     * Note that we never follow symlinks here. This is a simplification, but
     * should cover almost all cases correctly.
     *
     * Note that we create the new event source first here, before releasing the
     * old one. This should optimize behaviour as this way sd-event can reuse
     * the old watch in case the inode didn't change. */

    r = sd_event_add_inotify(m->event, &new_event, "/etc/localtime",
                             IN_ATTRIB | IN_MOVE_SELF | IN_CLOSE_WRITE |
                                 IN_DONT_FOLLOW,
                             manager_dispatch_timezone_change, m);
    if (r == -ENOENT)
    {
        /* If the file doesn't exist yet, subscribe to /etc instead, and wait
         * until it is created either by O_CREATE or by rename() */

        log_debug_errno(
            r, "/etc/localtime doesn't exist yet, watching /etc instead.");
    ...
    if (r < 0)
        return log_error_errno(
            r, "Failed to create timezone change event source: %m");

    /* Schedule this slightly earlier than the .timer event sources */
    r = sd_event_source_set_priority(new_event, SD_EVENT_PRIORITY_NORMAL - 1);
    if (r < 0)
        return log_error_errno(
            r, "Failed to set priority of timezone change event sources: %m");
...
static int manager_setup_signals(Manager* m)
{
    ...
    (void)sd_event_source_set_description(m->signal_event_source,
                                          "manager-signal");

    /* Process signals a bit earlier than the rest of things, but later than
     * notify_fd processing, so that the notify processing can still figure out
     * to which process/service a message belongs, before we reap the process.
     * Also, process this before handling cgroup notifications, so that we
     * always collect child exit status information before detecting that
     * there's no process in a cgroup. */
    r = sd_event_source_set_priority(m->signal_event_source,
                                     SD_EVENT_PRIORITY_NORMAL - 6);
    if (r < 0)
        return r;
...
static char** sanitize_environment(char** l)
{
    ...
    /* Let's remove some environment variables that we need ourselves to
     * communicate with our clients */
    strv_env_unset_many(
        l, "EXIT_CODE", "EXIT_STATUS", "INVOCATION_ID", "JOURNAL_STREAM",
        "LISTEN_FDNAMES", "LISTEN_FDS", "LISTEN_PID", "MAINPID", "MANAGERPID",
        "NOTIFY_SOCKET", "PIDFILE", "REMOTE_ADDR", "REMOTE_PORT",
        "SERVICE_RESULT", "WATCHDOG_PID", "WATCHDOG_USEC", NULL);
...
static int manager_setup_run_queue(Manager* m)
{
    ...
    (void)sd_event_source_set_description(m->run_queue_event_source,
                                          "manager-run-queue");
...
static int manager_setup_sigchld_event_source(Manager* m)
{
    ...
    r = sd_event_add_defer(m->event, &m->sigchld_event_source,
                           manager_dispatch_sigchld, m);
    if (r < 0)
        return r;

    r = sd_event_source_set_priority(m->sigchld_event_source,
                                     SD_EVENT_PRIORITY_NORMAL - 7);
    if (r < 0)
        return r;
    ...
    (void)sd_event_source_set_description(m->sigchld_event_source,
                                          "manager-sigchld");
                ...
                Manager** _m)
{
    ...
    *m = (Manager){
    ...
    /* Note that we do not set up the notify fd here. We do that after
     * deserialization, since they might have gotten serialized across the
     * reexec. */
...
static int manager_setup_notify(Manager* m)
{
    ...
    if (m->notify_fd < 0)
    {
        ...
        if (fd < 0)
            return log_error_errno(
                errno, "Failed to allocate notification socket: %m");

        fd_inc_rcvbuf(fd, NOTIFY_RCVBUF_SIZE);

        m->notify_socket =
            strappend(m->prefix[EXEC_DIRECTORY_RUNTIME], "/systemd/notify");
        if (!m->notify_socket)
            return log_oom();

        salen = sockaddr_un_set_path(&sa.un, m->notify_socket);
        if (salen < 0)
            return log_error_errno(salen,
                                   "Notify socket '%s' not valid for AF_UNIX "
                                   "socket address, refusing.",
                                   m->notify_socket);
    ...
    if (!m->notify_event_source)
    {
        r = sd_event_add_io(m->event, &m->notify_event_source, m->notify_fd,
                            EPOLLIN, manager_dispatch_notify_fd, m);
        if (r < 0)
            return log_error_errno(
                r, "Failed to allocate notify event source: %m");

        /* Process notification messages a bit earlier than SIGCHLD, so that we
         * can still identify to which service an exit message belongs. */
        r = sd_event_source_set_priority(m->notify_event_source,
                                         SD_EVENT_PRIORITY_NORMAL - 8);
        if (r < 0)
            return log_error_errno(
                r, "Failed to set priority of notify event source: %m");

        (void)sd_event_source_set_description(m->notify_event_source,
                                              "manager-notify");
    }
...
static int manager_setup_cgroups_agent(Manager* m)
{
    ...
    /* This creates a listening socket we receive cgroups agent messages on. We
     * do not use D-Bus for delivering these messages from the cgroups agent
     * binary to PID 1, as the cgroups agent binary is very short-living, and
     * each instance of it needs a new D-Bus connection. Since D-Bus connections
     * are SOCK_STREAM/AF_UNIX, on overloaded systems the backlog of the D-Bus
     * socket becomes relevant, as not more than the configured number of D-Bus
     * connections may be queued until the kernel will start dropping further
     * incoming connections, possibly resulting in lost cgroups agent messages.
     * To avoid this, we'll use a private SOCK_DGRAM/AF_UNIX socket, where no
     * backlog is relevant as communication may take place without an actual
     * connect() cycle, and we thus won't lose messages.
     *
     * Note that PID 1 will forward the agent message to system bus, so that the
     * user systemd instance may listen to it. The system instance hence listens
     * on this special socket, but the user instances listen on the system bus
     * for these messages. */
    ...
    if (r < 0)
        return log_error_errno(r, "Failed to determine whether unified cgroups "
                                  "hierarchy is used: %m");
    ...
    if (m->cgroups_agent_fd < 0)
    {
        ...
        if (fd < 0)
            return log_error_errno(
                errno, "Failed to allocate cgroups agent socket: %m");
    ...
    if (!m->cgroups_agent_event_source)
    {
        ...
        if (r < 0)
            return log_error_errno(
                r, "Failed to allocate cgroups agent event source: %m");

        /* Process cgroups notifications early, but after having processed
         * service notification messages or SIGCHLD signals, so that a cgroup
         * running empty is always just the last safety net of notification, and
         * we collected the metadata the notification and SIGCHLD stuff offers
         * first. Also see handling of cgroup inotify for the unified cgroup
         * stuff. */
        r = sd_event_source_set_priority(m->cgroups_agent_event_source,
                                         SD_EVENT_PRIORITY_NORMAL - 4);
        if (r < 0)
            return log_error_errno(
                r, "Failed to set priority of cgroups agent event source: %m");

        (void)sd_event_source_set_description(m->cgroups_agent_event_source,
...
static int manager_setup_user_lookup_fd(Manager* m)
{
    ...
    /* Set up the socket pair used for passing UID/GID resolution results from
     * forked off processes to PID
     * 1. Background: we can't do name lookups (NSS) from PID 1, since it might
     * involve IPC and thus activation, and we might hence deadlock on
     * ourselves. Hence we do all user/group lookups asynchronously from the
     * forked off processes right before executing the binaries to start. In
     * order to be able to clean up any IPC objects created by a unit (see
     * RemoveIPC=) we need to know in PID 1 the used UID/GID of the executed
     * processes, hence we establish this communication channel so that forked
     * off processes can pass their UID/GID information back to PID 1. The
     * forked off processes send their resolved UID/GID to PID 1 in a simple
     * datagram, along with their unit name, so that we can share one
     * communication socket pair among all units for this purpose.
     *
     * You might wonder why we need a communication channel for this that is
     * independent of the usual notification socket scheme (i.e.
     * $NOTIFY_SOCKET). The primary difference is about trust: data sent via the
     * $NOTIFY_SOCKET channel is only accepted if it originates from the right
     * unit and if reception was enabled for it. The user lookup socket OTOH is
     * only accessible by PID 1 and its children until they exec(), and always
     * available.
     *
     * Note that this function is called under two circumstances: when we first
     * initialize (in which case we allocate both the socket pair and the event
     * source to listen on it), and when we deserialize after a reload
     * (in which case the socket pair already exists but we still need to
     * allocate the event source for it). */
    ...
    {
        r = sd_event_add_io(m->event, &m->user_lookup_event_source,
                            m->user_lookup_fds[0], EPOLLIN,
                            manager_dispatch_user_lookup_fd, m);
        if (r < 0)
            return log_error_errno(
                errno, "Failed to allocate user lookup event source: %m");

        /* Process even earlier than the notify event source, so that we always
         * know first about valid UID/GID resolutions */
        r = sd_event_source_set_priority(m->user_lookup_event_source,
                                         SD_EVENT_PRIORITY_NORMAL - 11);
        if (r < 0)
            return log_error_errno(
                errno,
                "Failed to set priority ot user lookup event source: %m");

        (void)sd_event_source_set_description(m->user_lookup_event_source,
                                              "user-lookup");
    }
...
static void unit_gc_sweep(Unit* u, unsigned gc_marker)
{
    ...
    if (IN_SET(u->gc_marker - gc_marker, GC_OFFSET_GOOD, GC_OFFSET_BAD,
               GC_OFFSET_UNSURE, GC_OFFSET_IN_PATH))
        return;
...
static unsigned manager_dispatch_stop_when_unneeded_queue(Manager* m)
{
    ...
    while ((u = m->stop_when_unneeded_queue))
    {
        ...
        /* If stopping a unit fails continuously we might enter a stop loop
         * here, hence stop acting on the service being unnecessary after a
         * while. */

        if (!ratelimit_below(&u->auto_stop_ratelimit))
        {
            log_unit_warning(u, "Unit not needed anymore, but not stopping "
                                "since we tried this too often recently.");
            continue;
        }

        /* Ok, nobody needs us anymore. Sniff. Then let's commit suicide */
        r = manager_add_job(u->manager, JOB_STOP, u, JOB_FAIL, NULL, &error,
                            NULL);
        if (r < 0)
...
Manager* manager_free(Manager* m)
{
    ...
    /* Keep the cgroup hierarchy in place except when we know we are going down
     * for good */
    manager_shutdown_cgroup(m, IN_SET(m->objective, MANAGER_EXIT,
                                      MANAGER_REBOOT, MANAGER_POWEROFF,
                                      MANAGER_HALT, MANAGER_KEXEC));
...
static void manager_enumerate_perpetual(Manager* m)
{
    ...
    /* Let's ask every type to load all units from disk/kernel that it might
     * know */
    for (c = 0; c < _UNIT_TYPE_MAX; c++)
    {
        if (!unit_type_supported(c))
        {
            log_debug("Unit type .%s is not supported on this system.",
...
static void manager_coldplug(Manager* m)
{
    ...
    HASHMAP_FOREACH_KEY(u, k, m->units, i)
    {
        ...
        if (r < 0)
            log_warning_errno(
                r, "We couldn't coldplug %s, proceeding anyway: %m", u->id);
    }
}
...
static void manager_catchup(Manager* m)
{
    ...
    /* Let's catch up on any state changes that happened while we were
     * reloading/reexecing */
    HASHMAP_FOREACH_KEY(u, k, m->units, i)
    {
...
static void manager_build_unit_path_cache(Manager* m)
{
    ...
    STRV_FOREACH(i, m->lookup_paths.search_path)
    {
        ...
        if (!d)
        {
            if (errno != ENOENT)
                log_warning_errno(
                    errno, "Failed to open directory %s, ignoring: %m", *i);
            continue;
        }
...
fail:
    log_warning_errno(
        r, "Failed to build unit path cache, proceeding without: %m");
...
static bool manager_dbus_is_running(Manager* m, bool deserialized)
{
    ...
    /* This checks whether the dbus instance we are supposed to expose our APIs
     * on is up. We check both the socket and the service unit. If the
     * 'deserialized' parameter is true we'll check the deserialized state of
     * the unit rather than the current one. */
    ...
    if (!u)
        return false;
    if ((deserialized ? SOCKET(u)->deserialized_state : SOCKET(u)->state) !=
        SOCKET_RUNNING)
        return false;
    ...
    if (!u)
        return false;
    if (!IN_SET(
            (deserialized ? SERVICE(u)->deserialized_state : SERVICE(u)->state),
...
static void manager_preset_all(Manager* m)
{
    ...
    /* If this is the first boot, and we are in the host system, then preset
     * everything */
    r = unit_file_preset_all(UNIT_FILE_SYSTEM, 0, NULL,
                             UNIT_FILE_PRESET_ENABLE_ONLY, NULL, 0);
    if (r < 0)
        log_full_errno(
            r == -EEXIST ? LOG_NOTICE : LOG_WARNING, r,
...
static void manager_vacuum(Manager* m)
{
    ...
    /* Release any references to UIDs/GIDs no longer referenced, and destroy any
     * IPC owned by them */
...
static void manager_ready(Manager* m)
{
    ...
    /* After having loaded everything, do the final round of catching up with
    ...
    /* Let's finally catch up with any changes that took place while we were
     * reloading/reexecing */
    manager_catchup(m);
...
int manager_startup(Manager* m, FILE* serialization, FDSet* fds)
{
    ...
    r = lookup_paths_init(
        &m->lookup_paths, m->unit_file_scope,
        MANAGER_IS_TEST_RUN(m) ? LOOKUP_PATHS_TEMPORARY_GENERATED : 0, NULL);
    if (r < 0)
        return log_error_errno(r, "Failed to initialize path lookup table: %m");

    dual_timestamp_get(m->timestamps + manager_timestamp_initrd_mangle(
...

    {
        ...
        /* If we will deserialize make sure that during enumeration this is
         * already known, so we increase the counter here already */
        if (serialization)
            reloading = manager_reloading_start(m);

        /* First, enumerate what we can from all config files */
        dual_timestamp_get(m->timestamps +
        ...
        /* Any fds left? Find some unit which wants them. This is useful to
         * allow container managers to pass some file descriptors to us
         * pre-initialized. This enables socket-based activation of entire
         * containers. */
        manager_distribute_fds(m, fds);

        /* We might have deserialized the notify fd, but if we didn't then let's
         * create the bus now */
        r = manager_setup_notify(m);
        if (r < 0)
            /* No sense to continue without notifications, our children would
             * fail anyway. */
            return r;

        r = manager_setup_cgroups_agent(m);
        if (r < 0)
            /* Likewise, no sense to continue without empty cgroup
             * notifications. */
            return r;
        ...
        /* Now that we are connected to all possible busses, let's deserialize
         * who is tracking us. */
        r = bus_track_coldplug(m, &m->subscribed, false,
                               m->deserialized_subscribed);
        if (r < 0)
            log_warning_errno(
                r, "Failed to deserialized tracked clients, ignoring: %m");
        ...
        if (serialization)
            /* Let's wait for the UnitNew/JobNew messages being sent, before we
             * notify that the reload is finished */
...
int manager_add_job(Manager* m, JobType type, Unit* unit, JobMode mode,
                    Set* affected_jobs, sd_bus_error* error, Job** ret)
{
    ...
    if (mode == JOB_ISOLATE && !unit->allow_isolate)
        return sd_bus_error_setf(
            error, BUS_ERROR_NO_ISOLATION,
            "Operation refused, unit may not be isolated.");

    log_unit_debug(unit, "Trying to enqueue job %s/%s/%s", unit->id,
    ...
    log_unit_debug(unit, "Enqueued job %s/%s as %u", unit->id,
...
int manager_add_job_by_name(Manager* m, JobType type, const char* name,
                            JobMode mode, Set* affected_jobs, sd_bus_error* e,
                            Job** ret)
{
    Unit* unit =
        NULL; /* just to appease gcc, initialization is not really necessary */
    int r;
...
int manager_add_job_by_name_and_warn(Manager* m, JobType type, const char* name,
                                     JobMode mode, Set* affected_jobs,
                                     Job** ret)
{
    _cleanup_(sd_bus_error_free) sd_bus_error error = SD_BUS_ERROR_NULL;
    int r;
    ...
    r = manager_add_job_by_name(m, type, name, mode, affected_jobs, &error,
                                ret);
    if (r < 0)
        return log_warning_errno(r, "Failed to enqueue %s job for %s: %s",
                                 job_mode_to_string(mode), name,
                                 bus_error_message(&error, r));

    return r;
}

int manager_propagate_reload(Manager* m, Unit* unit, JobMode mode,
                             sd_bus_error* e)
{
    int r;
    Transaction* tr;
    ...
    r = transaction_add_job_and_dependencies(tr, JOB_NOP, unit, NULL, false,
                                             false, true, true, e);
    if (r < 0)
        goto tr_abort;

    /* Failure in adding individual dependencies is ignored, so this always
     * succeeds. */
    transaction_add_propagate_reload_jobs(tr, unit, tr->anchor_job,
...
static int manager_dispatch_target_deps_queue(Manager* m)
{
    ...
    static const UnitDependency deps[] = {UNIT_REQUIRED_BY, UNIT_REQUISITE_OF,
                                          UNIT_WANTED_BY, UNIT_BOUND_BY};
...
unsigned manager_dispatch_load_queue(Manager* m)
{
    ...
    /* Dispatch the units waiting for their target dependencies to be added now,
     * as all targets that we know about should be loaded and have aliases
     * resolved */
                              ...
                              sd_bus_error* e, Unit** _ret)
{
    ...
    if (t == _UNIT_TYPE_INVALID ||
        !unit_name_is_valid(name, UNIT_NAME_PLAIN | UNIT_NAME_INSTANCE))
    {
        if (unit_name_is_valid(name, UNIT_NAME_TEMPLATE))
            return sd_bus_error_setf(
                e, SD_BUS_ERROR_INVALID_ARGS,
...
int manager_load_startable_unit_or_warn(Manager* m, const char* name,
                                        const char* path, Unit** ret)
{
...
static unsigned manager_dispatch_dbus_queue(Manager* m)
{
    ...
    /* When we are reloading, let's not wait with generating signals, since we
     * need to exit the manager as quickly as we can. There's no point in
     * throttling generation of signals in that case. */
    if (MANAGER_IS_RELOADING(m) || m->send_reloading_done ||
        m->pending_reload_message)
    ...
    else
    {
        ...
        /* Do we have overly many messages queued at the moment? If so, let's
         * not enqueue more on top, let's
         * sit this cycle out, and process things in a later cycle when the
         * queues got a bit emptier. */
        if (manager_bus_n_queued_write(m) > MANAGER_BUS_BUSY_THRESHOLD)
            return 0;

        /* Only process a certain number of units/jobs per event loop iteration.
         * Even if the bus queue wasn't overly full before this call we
         * shouldn't increase it in size too wildly in one step, and we
         * shouldn't monopolize CPU time with generating these messages. Note
         * the difference in counting of this "budget" and the "threshold"
         * above: the "budget" is decreased only once per generated message,
         * regardless how many busses/direct connections it is enqueued on,
         * while the "threshold" is applied to each queued instance of bus
         * message, i.e. if the same message is enqueued to five busses/direct
         * connections it will be counted five times. This difference in
         * counting ("references" vs. "instances") is primarily a result of the
         * fact that it's easier to implement it this way, however it also
         * reflects the thinking that the "threshold" should put a limit on used
         * queue memory, i.e. space, while the "budget" should put a limit on
         * time. Also note that the "threshold" is
...
static void manager_invoke_notify_message(Manager* m, Unit* u,
                                          ...
                                          const char* buf, FDSet* fds)
{
    ...
    if (u->notifygen == m->notifygen) /* Already invoked on this same unit in
                                         this same iteration? */
        return;
    ...
    else if (DEBUG_LOGGING)
    {
        ...
        log_unit_debug(u, "Got notification message \"%s\", ignoring.",
                       strnull(y));
    }
}
                                      ...
                                      uint32_t revents, void* userdata)
{
    ...
    if (n < 0)
    {
        ...
        /* If this is any other, real error, then let's stop processing this
         * socket. This of course means we won't take notification messages
         * anymore, but that's still better than busy looping around this:
         * being woken up over and over again but being unable to actually read
    ...
    /* As extra safety check, let's make sure the string we get doesn't contain
     * embedded NUL bytes. We permit one trailing NUL byte in the message, but
     * don't expect it. */
    if (n > 1 && memchr(buf, 0, n - 1))
    {
        log_warning(
            "Received notify message with embedded NUL bytes. Ignoring.");
        return 0;
    }
    ...
    /* Increase the generation counter used for filtering out duplicate unit
     * invocations. */
    m->notifygen++;
    ...
    /* And now invoke the per-unit callbacks. Note that
     * manager_invoke_notify_message() will handle duplicate units
    ...
    if (!found)
        log_warning("Cannot find unit for notify message of PID " PID_FMT
                    ", ignoring.",
                    ucred->pid);
...
static void manager_invoke_sigchld_event(Manager* m, Unit* u,
                                         const siginfo_t* si)
{
    ...
    /* Already invoked the handler of this unit in this iteration? Then don't
     * process this again */
    ...
    if (UNIT_VTABLE(u)->sigchld_event)
        UNIT_VTABLE(u)->sigchld_event(u, si->si_pid, si->si_code,
                                      si->si_status);
}
...
static int manager_dispatch_sigchld(sd_event_source* source, void* userdata)
{
    ...
    /* First we call waitd() for a PID and do not reap the zombie. That way we
     * can still access /proc/$PID for it while it is a zombie. */
    ...
    if (waitid(P_ALL, 0, &si, WEXITED | WNOHANG | WNOWAIT) < 0)
    {
        ...
        if (errno != ECHILD)
            log_error_errno(
                errno, "Failed to peek for child with waitid(), ignoring: %m");
    ...
    if (IN_SET(si.si_code, CLD_EXITED, CLD_KILLED, CLD_DUMPED))
    {
        ...
        log_debug(
            "Child " PID_FMT " (%s) died (code=%s, status=%i/%s)", si.si_pid,
            strna(name), sigchld_code_to_string(si.si_code), si.si_status,
        ...
        if (array)
        {
            ...
            /* Make a copy of the array so that we don't trip up on the array
             * changing beneath us */
        ...
        /* Finally, execute them all. Note that u1, u2 and the array might
         * contain duplicates, but that's fine, manager_invoke_sigchld_event()
         * will ensure we only invoke the handlers once for each iteration. */
        if (u1)
...
static void manager_start_target(Manager* m, const char* name, JobMode mode)
{
    ...
    if (r < 0)
        log_error("Failed to enqueue %s job: %s", name,
                  bus_error_message(&error, r));
}
                                      ...
                                      uint32_t revents, void* userdata)
{
    ...
    if (n != sizeof(sfsi))
    {
        if (n >= 0)
        {
            log_warning("Truncated read from signal fd (%zu bytes), ignoring!",
                        n);
            return 0;
        }
    ...
    switch (sfsi.ssi_signo)
    {
        ...
        case SIGCHLD:
            r = sd_event_source_set_enabled(m->sigchld_event_source,
                                            SD_EVENT_ON);
            if (r < 0)
                log_warning_errno(
                    r, "Failed to enable SIGCHLD event source, ignoring: %m");
        ...
        default:
        {
            ...
            /* Starting SIGRTMIN+13, so that target halt and system halt are 10
             * apart */
            ...
            if ((int)sfsi.ssi_signo >= SIGRTMIN + 13 &&
                (int)sfsi.ssi_signo <
                    SIGRTMIN + 13 + (int)ELEMENTSOF(objective_table))
            {
            ...
            switch (sfsi.ssi_signo - SIGRTMIN)
            {
                ...
                case 26:
                case 29: /* compatibility: used to be mapped to
                                           ...
                                           uint32_t revents, void* userdata)
{
    ...
    log_struct(LOG_DEBUG, "MESSAGE_ID=" SD_MESSAGE_TIME_CHANGE_STR,
                                            ...
                                            void* userdata)
{
    ...
    /* Something changed, restart the watch, to ensure we watch the new
     * /etc/localtime if it changed */
    (void)manager_setup_timezone_change(m);
                                         ...
                                         uint32_t revents, void* userdata)
{
    ...
    /* There's at least one Type=idle child that just gave up on us waiting for
     * the boot process to complete. Let's now turn off any further console
     * output if there's at least one service that needs console access, so that
     * from now on our own output should not spill into that service's output
     * anymore. After all, we support Type=idle only to beautify console output
     * and it generally is set on services that want to own the console
     * exclusively without our interference. */
    m->no_console_output = m->n_on_console > 0;

    /* Acknowledge the child's request, and let all all other children know too
     * that they shouldn't wait any longer by closing the pipes towards them,
     * which is what they are waiting for. */
    manager_close_idle_pipe(m);
...
int manager_loop(Manager* m)
{
    ...
    assert(m);
    assert(m->objective ==
           MANAGER_OK); /* Ensure manager_startup() has been called */
    ...
    /* There might still be some zombies hanging around from before we were
     * exec()'ed. Let's reap them. */
                                     ...
                                     Unit** _u)
{
    ...
    /* Permit addressing units by invocation ID: if the passed bus path is
     * suffixed by a 128bit ID then we use it as invocation ID. */
    ...
    if (r >= 0)
    {
        ...
        return sd_bus_error_setf(
            e, BUS_ERROR_NO_UNIT_FOR_INVOCATION_ID,
    ...
    if (!unit_name_is_valid(n, UNIT_NAME_PLAIN | UNIT_NAME_INSTANCE))
    {
        ...
        return sd_bus_error_setf(
            e, SD_BUS_ERROR_INVALID_ARGS,
            "Unit name %s is neither a valid invocation ID nor unit name.",
            strnull(nn));
    }
...
void manager_send_unit_audit(Manager* m, Unit* u, int type, bool success)
{
    ...
    {
        log_error_errno(
            r, "Failed to extract prefix and instance of unit name: %m");
        return;
    }

    msg = strjoina("unit=", p);
    if (audit_log_user_comm_message(audit_fd, type, msg, "systemd", NULL, NULL,
                                    NULL, success) < 0)
    {
        if (errno == EPERM)
...
void manager_send_unit_plymouth(Manager* m, Unit* u)
{
    ...
    if (asprintf(&message, "U\002%c%s%n", (int)(strlen(u->id) + 1), u->id, &n) <
        0)
    {
        log_oom();
        return;
...
static bool manager_timestamp_shall_serialize(ManagerTimestamp t)
{
    ...
    /* The following timestamps only apply to the host system, hence only
     * serialize them there */
    return !IN_SET(
        t, MANAGER_TIMESTAMP_USERSPACE, MANAGER_TIMESTAMP_FINISH,
...
int manager_serialize(Manager* m, FILE* f, FDSet* fds, bool switching_root)
{
    ...
    _cleanup_(manager_reloading_stopp) _unused_ Manager* reloading =
        manager_reloading_start(m);

    (void)serialize_item_format(f, "current-job-id", "%" PRIu32,
                                m->current_job_id);
    (void)serialize_item_format(f, "n-installed-jobs", "%u",
                                m->n_installed_jobs);
    ...
    /* After switching root, udevd has not been started yet. So, enumeration
     * results should not be emitted. */
    ...
    if (m->log_level_overridden)
        (void)serialize_item_format(f, "log-level-override", "%i",
                                    log_get_max_level());
    ...
    if (m->user_lookup_fds[0] >= 0)
    {
        ...
        if (copy0 < 0)
            return log_error_errno(
                copy0, "Failed to add user lookup fd to serialization: %m");

        copy1 = fdset_put_dup(fds, m->user_lookup_fds[1]);
        if (copy1 < 0)
            return log_error_errno(
                copy1, "Failed to add user lookup fd to serialization: %m");
    ...
    if (r < 0)
        return log_error_errno(
            r, "Failed to add bus sockets to serialization: %m");

    return 0;
}

static int manager_deserialize_one_unit(Manager* m, const char* name, FILE* f,
                                        FDSet* fds)
{
    Unit* u;
    int r;
    ...
    if (r < 0)
    {
        ...
        return log_notice_errno(
            r, "Failed to load unit \"%s\", skipping deserialization: %m",
            name);
    }
    ...
    if (r < 0)
    {
        if (r == -ENOMEM)
            return r;
        return log_notice_errno(
            r, "Failed to deserialize unit \"%s\", skipping: %m", name);
    }
...
int manager_deserialize(Manager* m, FILE* f, FDSet* fds)
{
    ...
    /* If we are not in reload mode yet, enter it now. Not that this is
     * recursive, a caller might already have increased it to non-zero, which is
     * why we just increase it by one here and down again at the end of this
     * call. */
    _cleanup_(manager_reloading_stopp) _unused_ Manager* reloading =
        manager_reloading_start(m);
    ...
    for (;;)
    {
        ...
        if ((val = startswith(l, "current-job-id=")))
        {
            ...
            if (safe_atou32(val, &id) < 0)
                log_notice(
                    "Failed to parse current job id value '%s', ignoring.",
                    val);
            else
        ...
        else if ((val = startswith(l, "n-installed-jobs=")))
        {
            ...
            if (safe_atou32(val, &n) < 0)
                log_notice(
                    "Failed to parse installed jobs counter '%s', ignoring.",
                    val);
            else
        ...
        else if ((val = startswith(l, "n-failed-jobs=")))
        {
            ...
            if (safe_atou32(val, &n) < 0)
                log_notice(
                    "Failed to parse failed jobs counter '%s', ignoring.", val);
            else
                m->n_failed_jobs += n;
        }
        else if ((val = startswith(l, "taint-usr=")))
        {
            ...
            b = parse_boolean(val);
            if (b < 0)
                log_notice("Failed to parse taint /usr flag '%s', ignoring.",
                           val);
            else
        ...
        else if ((val = startswith(l, "ready-sent=")))
        {
            ...
            b = parse_boolean(val);
            if (b < 0)
                log_notice("Failed to parse ready-sent flag '%s', ignoring.",
                           val);
            else
        ...
        else if ((val = startswith(l, "taint-logged=")))
        {
            ...
            b = parse_boolean(val);
            if (b < 0)
                log_notice("Failed to parse taint-logged flag '%s', ignoring.",
                           val);
            else
        ...
        else if ((val = startswith(l, "service-watchdogs=")))
        {
            ...
            if (b < 0)
                log_notice(
                    "Failed to parse service-watchdogs flag '%s', ignoring.",
                    val);
            else
        ...
        else if ((val = startswith(l, "honor-device-enumeration=")))
        {
            ...
            b = parse_boolean(val);
            if (b < 0)
                log_notice("Failed to parse honor-device-enumeration flag "
                           "'%s', ignoring.",
                           val);
            else
        ...
        else if ((val = startswith(l, "show-status=")))
        {
            ...
            if (s < 0)
                log_notice("Failed to parse show-status flag '%s', ignoring.",
                           val);
            else
        ...
        else if ((val = startswith(l, "log-level-override=")))
        {
            ...
            if (level < 0)
                log_notice(
                    "Failed to parse log-level-override value '%s', ignoring.",
                    val);
            else
        ...
        else if ((val = startswith(l, "log-target-override=")))
        {
            ...
            if (target < 0)
                log_notice(
                    "Failed to parse log-target-override value '%s', ignoring.",
                    val);
            else
        ...
        else if (startswith(l, "env="))
        {
            ...
            if (r < 0)
                log_notice_errno(
                    r,
                    "Failed to parse environment entry: \"%s\", ignoring: %m",
                    l);
        }
        ...
        else if ((val = startswith(l, "cgroups-agent-fd=")))
        {
            ...
            if (safe_atoi(val, &fd) < 0 || fd < 0 || !fdset_contains(fds, fd))
                log_notice("Failed to parse cgroups agent fd, ignoring.: %s",
                           val);
            else
            {
                m->cgroups_agent_event_source =
        ...
        else if ((val = startswith(l, "user-lookup=")))
        {
            ...
            if (sscanf(val, "%i %i", &fd0, &fd1) != 2 || fd0 < 0 || fd1 < 0 ||
                fd0 == fd1 || !fdset_contains(fds, fd0) ||
                !fdset_contains(fds, fd1))
...
int manager_reload(Manager* m)
{
    ...
    if (fseeko(f, 0, SEEK_SET) < 0)
        return log_error_errno(
            errno, "Failed to seek to beginning of serialization: %m");
    ...
    /* Start by flushing out all jobs and units, all generated units, all
     * runtime environments, all dynamic users and everything else that is worth
     * flushing out. We'll get it all back from the serialization — if we need
     * it.*/
    ...
    if (r < 0)
        log_warning_errno(
            r, "Failed to initialize path lookup table, ignoring: %m");
    ...
    /* Re-register notify_fd as event source, and set up other
     * sockets/communication channels we might need */
    ...
    /* On manager reloading, device tag data should exists, thus, we should
     * honor the results of device enumeration. The flag should be always set
     * correctly by the serialized data, but it may fail. So, let's always set
     * the flag here for safety. */
    m->honor_device_enumeration = true;
...
static void log_taint_string(Manager* m)
{
    ...
    log_struct(LOG_NOTICE, LOG_MESSAGE("System is tainted: %s", taint),
               "TAINT=%s", taint, "MESSAGE_ID=" SD_MESSAGE_TAINTED_STR);
}
...
static void manager_notify_finished(Manager* m)
{
    ...
    usec_t firmware_usec, loader_usec, kernel_usec, initrd_usec, userspace_usec,
        total_usec;
    ...
    if (MANAGER_IS_SYSTEM(m) && detect_container() <= 0)
    {
        ...
        char buf[FORMAT_TIMESPAN_MAX + STRLEN(" (firmware) + ") +
                 FORMAT_TIMESPAN_MAX + STRLEN(" (loader) + ")] = {};
        char* p = buf;
        size_t size = sizeof buf;

        /* Note that MANAGER_TIMESTAMP_KERNEL's monotonic value is always at 0,
         * and MANAGER_TIMESTAMP_FIRMWARE's and MANAGER_TIMESTAMP_LOADER's
         * monotonic value should be considered negative values. */

        firmware_usec = m->timestamps[MANAGER_TIMESTAMP_FIRMWARE].monotonic -
                        m->timestamps[MANAGER_TIMESTAMP_LOADER].monotonic;
        loader_usec = m->timestamps[MANAGER_TIMESTAMP_LOADER].monotonic -
                      m->timestamps[MANAGER_TIMESTAMP_KERNEL].monotonic;
        userspace_usec = m->timestamps[MANAGER_TIMESTAMP_FINISH].monotonic -
                         m->timestamps[MANAGER_TIMESTAMP_USERSPACE].monotonic;
        total_usec = m->timestamps[MANAGER_TIMESTAMP_FIRMWARE].monotonic +
                     m->timestamps[MANAGER_TIMESTAMP_FINISH].monotonic;

        if (firmware_usec > 0)
            size = strpcpyf(
                &p, size, "%s (firmware) + ",
                format_timespan(ts, sizeof(ts), firmware_usec, USEC_PER_MSEC));
        if (loader_usec > 0)
            size = strpcpyf(
                &p, size, "%s (loader) + ",
                format_timespan(ts, sizeof(ts), loader_usec, USEC_PER_MSEC));

        if (dual_timestamp_is_set(&m->timestamps[MANAGER_TIMESTAMP_INITRD]))
        {

            /* The initrd case on bare-metal*/
            kernel_usec = m->timestamps[MANAGER_TIMESTAMP_INITRD].monotonic -
                          m->timestamps[MANAGER_TIMESTAMP_KERNEL].monotonic;
            initrd_usec = m->timestamps[MANAGER_TIMESTAMP_USERSPACE].monotonic -
                          m->timestamps[MANAGER_TIMESTAMP_INITRD].monotonic;

            log_struct(
                LOG_INFO, "MESSAGE_ID=" SD_MESSAGE_STARTUP_FINISHED_STR,
                "KERNEL_USEC=" USEC_FMT, kernel_usec, "INITRD_USEC=" USEC_FMT,
                initrd_usec, "USERSPACE_USEC=" USEC_FMT, userspace_usec,
                LOG_MESSAGE("Startup finished in %s%s (kernel) + %s (initrd) + "
                            "%s (userspace) = %s.",
                            buf,
        ...
        else
        {
            ...
            kernel_usec = m->timestamps[MANAGER_TIMESTAMP_USERSPACE].monotonic -
                          m->timestamps[MANAGER_TIMESTAMP_KERNEL].monotonic;
            initrd_usec = 0;
            ...
            log_struct(
                ...
                LOG_MESSAGE(
                    "Startup finished in %s%s (kernel) + %s (userspace) = %s.",
                    buf,
                    format_timespan(kernel, sizeof(kernel), kernel_usec,
                                    USEC_PER_MSEC),
                    format_timespan(userspace, sizeof(userspace),
    ...
    else
    {
        ...
        total_usec = userspace_usec =
            m->timestamps[MANAGER_TIMESTAMP_FINISH].monotonic -
            m->timestamps[MANAGER_TIMESTAMP_USERSPACE].monotonic;

        log_struct(LOG_INFO, "MESSAGE_ID=" SD_MESSAGE_USER_STARTUP_FINISHED_STR,
                   "USERSPACE_USEC=" USEC_FMT, userspace_usec,
...
static void manager_send_ready(Manager* m)
{
    ...
    /* We send READY=1 on reaching basic.target only when running in --user
     * mode. */
...
void manager_check_finished(Manager* m)
{
    ...
    /* Verify that we have entered the event loop already, and not left it
     * again. */
...
static bool generator_path_any(const char* const* paths)
{
    ...
    if (access(*path, F_OK) == 0)
        ...
        log_warning_errno(errno, "Failed to open generator directory %s: %m",
                          *path);
...
static const char* const system_env_generator_binary_paths[] = {
    ...
    SYSTEM_ENV_GENERATOR_PATH, NULL};

static const char* const user_env_generator_binary_paths[] = {
    "/run/systemd/user-environment-generators",
    "/etc/systemd/user-environment-generators",
    "/usr/local/lib/systemd/user-environment-generators",
    USER_ENV_GENERATOR_PATH, NULL};

static int manager_run_environment_generators(Manager* m)
{
    char** tmp =
        NULL; /* this is only used in the forked process, no cleanup here */
    ...
    if (MANAGER_IS_TEST_RUN(m) &&
        !(m->test_run_flags & MANAGER_TEST_RUN_ENV_GENERATORS))
        return 0;
...
static int manager_run_generators(Manager* m)
{
    ...
    if (MANAGER_IS_TEST_RUN(m) &&
        !(m->test_run_flags & MANAGER_TEST_RUN_GENERATORS))
        return 0;
...
void manager_recheck_dbus(Manager* m)
{
    ...
    /* Connects to the bus if the dbus service and socket are running. If we are
     * running in user mode this is all it does. In system mode we'll also
     * connect to the system bus (which will most likely just reuse the
     * connection of the API bus). That's because the system bus after all runs
     * as service of the system instance, while in the user instance we can
...
void manager_recheck_journal(Manager* m)
{
    ...
    /* Don't bother with this unless we are in the special situation of being
     * PID 1 */
    if (getpid_cached() != 1)
        return;
    ...
    /* The journal is fully and entirely up? If so, let's permit logging to it,
     * if that's configured. If the journal is down, don't ever log to it,
     * otherwise we might end up deadlocking ourselves as we might trigger an
     * activation ourselves we can't fulfill. */
...
const char* manager_get_confirm_spawn(Manager* m)
{
    ...
    /* Here's the deal: we want to test the validity of the console but don't
     * want PID1 to go through the whole console process which might block. But
     * we also want to warn the user only once if something is wrong with the
     * console so we cannot do the sanity checks after spawning our children. So
     * here we simply do really basic tests to hopefully trap common errors.
     *
     ...
     *
     * Note: TTYs are devices that can come and go any time, and frequently
     * aren't available yet during early boot (consider a USB rs232 dongle...).
     * If for any reason the configured console is not ready, we fallback to the
     * default console. */
...
fail:
    ...
    {
        last_errno = errno;
        log_warning_errno(errno, "Failed to open %s: %m, using default console",
                          vc);
    }
    return "/dev/console";
}
                           ...
                           const char* format, ...)
{
    ...
    status_vprintf(
        ...
        SHOW_STATUS_ELLIPSIZE |
            (type == STATUS_TYPE_EPHEMERAL ? SHOW_STATUS_EPHEMERAL : 0),
        format, ap);
    va_end(ap);
}
...
ManagerState manager_state(Manager* m)
{
    ...
    /* Is the special shutdown target active or queued? If so, we are in
     * shutdown state */
    ...
    if (MANAGER_IS_SYSTEM(m))
    {
        /* Are the rescue or emergency targets active or queued? If so we are in
         * maintenance state */
...
static void manager_unref_uid_internal(Manager* m, Hashmap** uid_refs,
                                       uid_t uid, bool destroy_now,
                                       int (*_clean_ipc)(uid_t uid))
{
    ...
    /* A generic implementation, covering both manager_unref_uid() and
     * manager_unref_gid(), under the assumption that uid_t and gid_t are
     * actually defined the same way, with the same validity rules.
     *
     * We store a hashmap where the UID/GID is they key and the value is a 32bit
     * reference counter, whose highest bit is used as flag for marking
     * UIDs/GIDs whose IPC objects to remove when the last reference to the
     * UID/GID is dropped. The flag is set to on, once at least one reference
     * from a unit where RemoveIPC= is set is added on a UID/GID. It is reset
     * when the UID's/GID's reference counter drops to 0 again. */
    ...
    if (destroy_now && n == 0)
    {
        ...
        if (c & DESTROY_IPC_FLAG)
        {
            log_debug("%s " UID_FMT
                      ...
                      _clean_ipc == clean_ipc_by_uid ? "UID" : "GID", uid);
            (void)_clean_ipc(uid);
        }
    }
    else
    {
        ...
        assert_se(
            hashmap_update(*uid_refs, UID_TO_PTR(uid), UINT32_TO_PTR(c)) >= 0);
    }
}

void manager_unref_uid(Manager* m, uid_t uid, bool destroy_now)
{
    manager_unref_uid_internal(m, &m->uid_refs, uid, destroy_now,
                               clean_ipc_by_uid);
}

void manager_unref_gid(Manager* m, gid_t gid, bool destroy_now)
{
    manager_unref_uid_internal(m, &m->gid_refs, (uid_t)gid, destroy_now,
                               clean_ipc_by_gid);
}

static int manager_ref_uid_internal(Manager* m, Hashmap** uid_refs, uid_t uid,
                                    bool clean_ipc)
{
    ...
    /* A generic implementation, covering both manager_ref_uid() and
     * manager_ref_gid(), under the assumption
     * that uid_t and gid_t are actually defined the same way, with the same
     * validity rules. */
...
static void manager_vacuum_uid_refs_internal(Manager* m, Hashmap** uid_refs,
                                             int (*_clean_ipc)(uid_t uid))
{
    ...
    HASHMAP_FOREACH_KEY(p, k, *uid_refs, i)
    {
        ...
        if (c & DESTROY_IPC_FLAG)
        {
            log_debug("Found unreferenced %s " UID_FMT
...
static void manager_serialize_uid_refs_internal(Manager* m, FILE* f,
                                                Hashmap** uid_refs,
                                                const char* field_name)
{
    ...
    /* Serialize the UID reference table. Or actually, just the IPC destruction
     * flag of it, as the actual counter of it is better rebuild after a
     * reload/reexec. */
...
static void manager_deserialize_uid_refs_one_internal(Manager* m,
                                                      Hashmap** uid_refs,
                                                      const char* value)
{
...
int manager_dispatch_user_lookup_fd(sd_event_source* source, int fd,
                                    uint32_t revents, void* userdata)
{
    struct buffer
    {
    ...
    /* Invoked whenever a child process succeeded resolving its user/group to
     * use and sent us the resulting UID/GID in a datagram. We parse the
     * datagram here and pass it off to the unit, so that it can add a reference
     * to the
     * UID/GID so that it can destroy the UID/GID's IPC objects when the
    ...
    {
        log_warning(
            "Got user lookup message with invalid UID/GID pair, ignoring.");
        return 0;
    }
    ...
    {
        log_warning(
            "Received lookup message with embedded NUL character, ignoring.");
        return 0;
    }
    ...
    log_unit_debug(u, "User lookup succeeded: uid=" UID_FMT " gid=" GID_FMT,
                   buffer.uid, buffer.gid);
...
{
    _cleanup_free_ char *destination = NULL, *overflowuid = NULL,
                        *overflowgid = NULL;
    char *buf, *e;
    int r;
...
void manager_unref_console(Manager* m)
{
    ...
    if (m->n_on_console == 0)
        m->no_console_output = false; /* unset no_console_output flag, since the
                                         console is definitely free now */
}
...
void manager_restore_original_log_target(Manager* m)
{
    ...
    log_info("Restoring log target to original %s.",
             log_target_to_string(m->original_log_target));
...
{
    if (in_initrd() && s >= MANAGER_TIMESTAMP_SECURITY_START &&
        s <= MANAGER_TIMESTAMP_UNITS_LOAD_FINISH)
        return s - MANAGER_TIMESTAMP_SECURITY_START +
               MANAGER_TIMESTAMP_INITRD_SECURITY_START;
    return s;
}

static const char* const manager_state_table[_MANAGER_STATE_MAX] = {
    [MANAGER_INITIALIZING] = "initializing", [MANAGER_STARTING] = "starting",
    [MANAGER_RUNNING] = "running",           [MANAGER_DEGRADED] = "degraded",
    [MANAGER_MAINTENANCE] = "maintenance",   [MANAGER_STOPPING] = "stopping",
};
