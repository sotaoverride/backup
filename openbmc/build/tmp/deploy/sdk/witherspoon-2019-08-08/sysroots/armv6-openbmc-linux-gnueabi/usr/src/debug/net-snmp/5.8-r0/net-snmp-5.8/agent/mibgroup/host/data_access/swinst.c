 ...
 */
netsnmp_swinst_entry* netsnmp_swinst_entry_create(int32_t swIndex)
{
...
{
    DEBUGMSGTL(
        ("swinst:entry:free", "index %" NETSNMP_PRIo "u\n", entry->swIndex));
 ...
 * free a row
 */
static void netsnmp_swinst_entry_free_cb(netsnmp_swinst_entry* entry,
                                         void* context)
{
    free(entry);
}
