...
#define DNSSD_SERVICE_DIRS                                                     \
    ((const char* const*)CONF_PATHS_STRV("systemd/dnssd"))
...
static int dnssd_service_load(Manager* manager, const char* filename)
{
    ...
    *d = '\0';
    ...
    r = config_parse_many(filename, DNSSD_SERVICE_DIRS, dropin_dirname,
                          "Service\0", config_item_perf_lookup,
                          resolved_dnssd_gperf_lookup, false, service);
    if (r < 0)
        return r;
...
static int specifier_dnssd_host_name(char specifier, const void* data,
...
int dnssd_render_instance_name(DnssdService* s, char** ret_name)
{
    ...
    if (!dns_service_name_is_valid(name))
        return log_debug_errno(SYNTHETIC_ERRNO(EINVAL),
                               "Service instance name '%s' is invalid.", name);
...
int dnssd_update_rrs(DnssdService* s)
{
    ...
    {
        txt_data->rr =
            dns_resource_record_new_full(DNS_CLASS_IN, DNS_TYPE_TXT, full_name);
        if (!txt_data->rr)
            goto oom;
    ...
    s->ptr_rr =
        dns_resource_record_new_full(DNS_CLASS_IN, DNS_TYPE_PTR, service_name);
    if (!s->ptr_rr)
        goto oom;
    ...
    s->srv_rr =
        dns_resource_record_new_full(DNS_CLASS_IN, DNS_TYPE_SRV, full_name);
    if (!s->srv_rr)
        goto oom;
...
int dnssd_txt_item_new_from_string(const char* key, const char* value,
                                   DnsTxtItem** ret_item)
{
    size_t length;
    DnsTxtItem* i;
    ...
    i = malloc0(offsetof(DnsTxtItem, data) + length +
...
int dnssd_txt_item_new_from_data(const char* key, const void* data,
                                 const size_t size, DnsTxtItem** ret_item)
{
    size_t length;
    DnsTxtItem* i;
...
void dnssd_signal_conflict(Manager* manager, const char* name)
{
    ...
    HASHMAP_FOREACH(s, manager->dnssd_services, i)
    {
        ...
        if (dns_name_equal(dns_resource_key_name(s->srv_rr->key), name))
        {
            ...
            r = sd_bus_path_encode("/org/freedesktop/resolve1/dnssd", s->name,
                                   &path);
            if (r < 0)
            {
            ...
            r = sd_bus_emit_signal(manager->bus, path,
