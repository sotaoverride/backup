...
void* weak_function malloc(size_t n)
{
    if (alloc_end == 0)
    {
        ...
        alloc_ptr = &_end;
        alloc_end =
            (void*)0 + (((alloc_ptr - (void*)0) + GLRO(dl_pagesize) - 1) &
                        ~(GLRO(dl_pagesize) - 1));
    }
    ...
    if (alloc_ptr + n >= alloc_end || n >= -(uintptr_t)alloc_ptr)
    {
       ...
       page to reduce number of mmap calls.  */
        ...
        page = __mmap(0, nup, PROT_READ | PROT_WRITE, MAP_ANON | MAP_PRIVATE,
                      -1, 0);
        if (page == MAP_FAILED)
            return NULL;
        if (page != alloc_end)
...
#define HALF_SIZE_T (((size_t)1) << (8 * sizeof(size_t) / 2))
    if (__builtin_expect((nmemb | size) >= HALF_SIZE_T, 0) && size != 0 &&
        bytes / size != nmemb)
        return NULL;
...
rtld_hidden_def(__chk_fail)
    ...
    void weak_function __assert_fail(const char* assertion, const char* file,
...
weak_alias(__strsep, strsep) strong_alias(__strsep, __strsep_g)
