...
struct su_context
{
    ...
    struct sigaction
        oldact[SIGNALS_IDX_COUNT]; /* original sigactions indexed by SIG*_IDX */

#ifdef USE_PTY
    struct termios
        stdin_attrs; /* stdin and slave terminal runtime attributes */
    int pty_master;
    int pty_slave;
    ...
    unsigned int runuser : 1,   /* flase=su, true=runuser */
        ...
        simulate_login : 1,     /* simulate a login instead of just starting a
                                   shell. */
        change_environment : 1, /* change some environment vars to indicate the
                                   user su'd to.*/
        same_session : 1,       /* don't call setsid() with a command. */
        suppress_pam_info : 1,  /* don't print PAM info messages (Last login,
                                   etc.). */
...
static void init_tty(struct su_context* su)
{
    ...
    DBG(TTY,
        ul_debug("initialize [is-term=%s]", su->isterm ? "true" : "false"));
    if (su->isterm)
...
static void pty_create(struct su_context* su)
{
    ...
    DBG(PTY, ul_debug("pty setup done [master=%d, slave=%d]", su->pty_master,
                      su->pty_slave));
}
...
static int write_to_child(struct su_context* su, char* buf, size_t bufsz)
{
...
static void write_eof_to_child(struct su_context* su)
{
    ...
    struct pollfd fds[] = {{.fd = su->pty_slave, .events = POLLIN}};
    char c = DEF_EOF;
    ...
    switch (info.ssi_signo)
    {
        case SIGCHLD:
            ...
            if (info.ssi_code == CLD_EXITED || info.ssi_code == CLD_KILLED ||
                info.ssi_code == CLD_DUMPED || info.ssi_status == SIGSTOP)
                wait_for_child(su);
...
static void pty_proxy_master(struct su_context* su)
{
    ...
    struct pollfd pfd[] = {
        ...
        [POLLFD_MASTER] = {.fd = su->pty_master,
                           .events = POLLIN | POLLERR | POLLHUP},
        [POLLFD_STDIN] = {.fd = STDIN_FILENO,
                          .events = POLLIN | POLLERR | POLLHUP}};
    ...
    while (!caught_signal)
    {
        ...
        {
            DBG(PTY,
                ul_debug("leaving poll() loop [timeout=%d]", su->poll_timeout));
            break;
        }
        ...
        for (i = 0; i < ARRAY_SIZE(pfd); i++)
        {
            ...
            DBG(PTY,
                ul_debug(" active pfd[%s].fd=%d %s %s %s",
                         ...
                         pfd[i].fd, pfd[i].revents & POLLIN ? "POLLIN" : "",
...
static void log_syslog(struct su_context* su, bool successful)
{
    ...
    syslog(LOG_NOTICE, "%s(to %s) %s on %s",
           successful ? "" : su->runuser ? "FAILED RUNUSER " : "FAILED SU ",
           su->new_user, su->old_user ?: "", su->tty_name ?: "none");
    closelog();
}
...
static int supam_conv(int num_msg, const struct pam_message** msg,
                      struct pam_response** resp, void* data)
{
    struct su_context* su = (struct su_context*)data;

    if (su->suppress_pam_info && num_msg == 1 && msg &&
...
static void parent_setup_signals(struct su_context* su)
{
    ...
    if (!caught_signal)
    {
        ...
        if (!su->same_session &&
            (sigaddset(&ourset, SIGINT) || sigaddset(&ourset, SIGQUIT)))
        {
        ...
        if (!caught_signal &&
            (sigaddset(&ourset, SIGTERM) || sigaddset(&ourset, SIGALRM)))
        {
        ...
        if (!caught_signal && sigprocmask(SIG_UNBLOCK, &ourset, NULL))
        {
...
{
    DBG(MISC,
        ul_debug("changing identity [GID=%d, UID=%d]", pw->pw_gid, pw->pw_uid));
 ...
 */
static void run_shell(struct su_context* su, char const* shell,
                      char const* command, char** additional_args,
                      size_t n_additional_args)
{
    ...
    DBG(MISC, ul_debug("starting shell [shell=%s, command=\"%s\"%s%s]", shell,
                       command, su->simulate_login ? " login" : "",
...
{
    fputs(_(" -m, -p, --preserve-environment      do not reset environment "
            "variables\n"),
          stdout);
    fputs(_(" -w, --whitelist-environment <list>  don't reset specified "
            "variables\n"),
          stdout);
    ...
    fputs(_(" -, -l, --login                  make the shell a login shell\n"),
          stdout);
    fputs(_(" -c, --command <command>         pass a single command to the "
            "shell with -c\n"),
          stdout);
    fputs(_(" --session-command <command>     pass a single command to the "
            "shell with -c\n"
            "                                   and do not create a new "
            "session\n"),
          stdout);
    fputs(_(" -f, --fast                      pass -f to the shell (for csh or "
            "tcsh)\n"),
          stdout);
    fputs(_(" -s, --shell <shell>             run <shell> if /etc/shells "
            "allows it\n"),
          stdout);
...
static void usage_runuser(void)
{
    ...
    fputs(USAGE_SEPARATOR, stdout);
    fputs(_("Run <command> with the effective user ID and group ID of <user>.  "
            "If -u is\n"
...
{
    fputs(USAGE_HEADER, stdout);
    fprintf(stdout, _(" %s [options] [-] [<user> [<argument>...]]\n"),
            program_invocation_short_name);
...
static gid_t add_supp_group(const char* name, gid_t** groups, size_t* ngroups)
{
    ...
    DBG(MISC, ul_debug("add %s group [name=%s, GID=%d]", name, gr->gr_name,
                       (int)gr->gr_gid));
...
int su_main(int argc, char** argv, int mode)
{
    ...
    while ((optc = getopt_long(argc, argv, "c:fg:G:lmpPs:u:hVw:", longopts,
                               NULL)) != -1)
    {
    ...
    {
        warnx(_("ignoring --preserve-environment, it's mutually exclusive with "
                "--login"));
    ...
    switch (mode)
    {
        case RUNUSER_MODE:
             ...
             *
             * If -u <user> is not specified, then follow traditional su(1)
             * behavior and fallthrough
             */
            if (su->runuser_uopt)
            {
    ...
    if (!su->pwd || !su->pwd->pw_passwd || !su->pwd->pw_name ||
        !*su->pwd->pw_name || !su->pwd->pw_dir || !*su->pwd->pw_dir)
    ...
    else
    {
        ...
        if (shell && getuid() != 0 && is_restricted_shell(su->pwd->pw_shell))
        {
