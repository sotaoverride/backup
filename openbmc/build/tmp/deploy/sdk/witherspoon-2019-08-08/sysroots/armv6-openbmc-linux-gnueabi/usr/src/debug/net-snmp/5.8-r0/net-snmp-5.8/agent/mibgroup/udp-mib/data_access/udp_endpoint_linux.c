 ...
 * @retval !0 errors
 */
int netsnmp_arch_udp_endpoint_container_load(netsnmp_container* container,
                                             u_int load_flags)
{
    int rc = 0;
 ...
 */
static int _process_line_udp_ep(netsnmp_line_info* line_info, void* mem,
                                struct netsnmp_line_process_info_s* lpi)
{
    ...
    {
        DEBUGMSGTL(
            ("text:util:tvi", "no space after sl '%s'\n", line_info->start));
        return PMLP_RC_MEMORY_UNUSED;
    }
...
static int _load4(netsnmp_container* container, u_int load_flags)
{
    ...
    container =
        netsnmp_file_text_parse(fp, container, PM_USER_FUNCTION, 0, &lpi);
