...
IMPLEMENT_BLOCK_CIPHER(aria_128, ks, aria, EVP_ARIA_KEY, NID_aria_128, 16, 16,
                       16, 128, 0, aria_init_key, NULL, EVP_CIPHER_set_asn1_iv,
                       EVP_CIPHER_get_asn1_iv, NULL)
IMPLEMENT_BLOCK_CIPHER(aria_192, ks, aria, EVP_ARIA_KEY, NID_aria_192, 16, 24,
                       16, 128, 0, aria_init_key, NULL, EVP_CIPHER_set_asn1_iv,
                       EVP_CIPHER_get_asn1_iv, NULL)
IMPLEMENT_BLOCK_CIPHER(aria_256, ks, aria, EVP_ARIA_KEY, NID_aria_256, 16, 32,
                       16, 128, 0, aria_init_key, NULL, EVP_CIPHER_set_asn1_iv,
                       EVP_CIPHER_get_asn1_iv, NULL)
                           ...
                           const unsigned char* in, size_t len)
{
    ...
    CRYPTO_ctr128_encrypt(
        ...
        EVP_CIPHER_CTX_buf_noconst(ctx), &num, (block128_f)aria_encrypt);
                             ...
                             const unsigned char* iv, int enc)
{
    ...
    if (key)
    {
        ret = aria_set_encrypt_key(key, EVP_CIPHER_CTX_key_length(ctx) * 8,
                                   &gctx->ks.ks);
        CRYPTO_gcm128_init(&gctx->gcm, &gctx->ks, (block128_f)aria_encrypt);
        if (ret < 0)
        {
                           ...
                           const unsigned char* in, size_t len)
{
    ...
    if (!EVP_CIPHER_CTX_encrypting(ctx))
    {
        if (gctx->taglen < 0)
            return -1;
        if (CRYPTO_gcm128_finish(&gctx->gcm, EVP_CIPHER_CTX_buf_noconst(ctx),
                                 gctx->taglen) != 0)
            return -1;
        gctx->iv_set = 0;
        return 0;
...
#define ARIA_AUTH_FLAGS                                                        \
    (EVP_CIPH_FLAG_DEFAULT_ASN1 | EVP_CIPH_CUSTOM_IV |                         \
     ...
     EVP_CIPH_CTRL_INIT | EVP_CIPH_CUSTOM_COPY | EVP_CIPH_FLAG_AEAD_CIPHER)

#define BLOCK_CIPHER_aead(nid, keylen, blocksize, ivlen, nmode, mode, MODE,    \
                          flags)                                               \
    static const EVP_CIPHER aria_##keylen##_##mode = {                         \
        nid##_##keylen##_##nmode,                                              \
        blocksize,                                                             \
