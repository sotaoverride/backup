...
struct logState
{
    char* fn;
    struct tm
        lastRotated; /* only tm_hour, tm_mday, tm_mon, tm_year are good! */
    struct stat sb;
    int doRotate;
...
static int allocateHash(unsigned int hs)
{
    ...
    message(MESS_DEBUG,
            "Allocating hash table for state file, size %u entries\n", hs);
...
static int runScript(struct logInfo* log, char* logfn, char* logrotfn,
                     char* script)
{
    int rc;
    ...
    if (!fork())
    {
        ...
        execl("/bin/sh", "sh", "-c", script, "logrotate_script", logfn,
              logrotfn, (char*)NULL);
        exit(1);
    }
...
static int is_acl_well_supported(int err)
{
    switch (err)
    {
        ...
        case ENOSYS:  /* compatibility - acl_(g|s)et_fd(3) should never return
                         this */
        case EBUSY:   /* compatibility - acl_(g|s)et_fd(3) should never return
                         this */
            return 0;
        default:
            return 1;
                            ...
                            acl_type acl, int force_mode)
{
    ...
    for (i = 0; i < 2; ++i)
    {
        ...
        {
            message(MESS_ERROR,
                    "error renaming already existing output file"
                    " %s to %s: %s\n",
    ...
    {
        message(MESS_ERROR,
                "error setting owner of %s to uid %u and gid %u: %s\n",
...
static int compressLogFile(char* name, struct logInfo* log, struct stat* sb)
{
    ...
    fullCommand =
        alloca(sizeof(*fullCommand) * (log->compress_options_count + 2));
    ...
    if (!fork())
    {
        ...
        envInFilename =
            alloca(strlen("LOGROTATE_COMPRESSED_FILENAME=") + strlen(name) + 2);
    ...
    while ((i = read(compressPipe[0], buff, sizeof(buff) - 1)) > 0)
    {
        if (!error_printed)
        {
            error_printed = 1;
            message(MESS_ERROR,
                    "Compressing program wrote following message "
                   ...
                   char* uncompressCommand, char* address, char* subject)
{
    ...
    char* const mailArgv[] = {(char*)mailComm, (char*)"-s", subject, address,
                              NULL};
    int rc = 0;
...
static int mailLogWrapper(char* mailFilename, const char* mailComm, int logNum,
                          struct logInfo* log)
{
    /* uncompress already compressed log files before mailing them */
    char* uncompress_prog =
        (log->flags & LOG_FLAG_COMPRESS) ? log->uncompress_prog : NULL;
                        ...
                        int flags, int skip_copy)
{
    ...
    if (!debug)
    {
        ...
        const int read_only =
            (flags & LOG_FLAG_COPY) && !(flags & LOG_FLAG_COPYTRUNCATE);
        if ((fdcurr = open(currLog,
                           ((read_only) ? O_RDONLY : O_RDWR) | O_NOFOLLOW)) < 0)
        {
            message(MESS_ERROR, "error opening %s: %s\n", currLog,
                    strerror(errno));
            goto fail;
...
static int findNeedRotating(struct logInfo* log, int logNum, int force)
{
    ...
    if ((log->flags & LOG_FLAG_SU) == 0 && getuid() == 0)
    {
        ...
        if (stat(ld, &sb))
        {
            ...
            if (errno != ENOENT ||
                (errno == ENOENT && (log->flags & LOG_FLAG_MISSINGOK) == 0))
            {
        ...
        /* Don't rotate in directories writable by others or group which is not
         * "root"  */
        if ((sb.st_gid != 0 && sb.st_mode & S_IWGRP) || sb.st_mode & S_IWOTH)
        {
            message(MESS_ERROR,
                    "skipping \"%s\" because parent directory has insecure "
                    "permissions"
                    " (It's world writable or writable by group which is not "
                    "\"root\")"
    ...
    {
        message(MESS_DEBUG,
                "  log %s is symbolic link. Rotation of symbolic"
    ...
    message(MESS_DEBUG, "  Last rotated at %d-%02d-%02d %02d:%02d\n",
            1900 + state->lastRotated.tm_year, 1 + state->lastRotated.tm_mon,
            state->lastRotated.tm_mday, state->lastRotated.tm_hour,
             ...
             state->lastRotated.tm_hour != now.tm_hour)
    {
        ...
        switch (log->criterium)
        {
            ...
            case ROT_HOURLY:
                state->doRotate =
                    ((now.tm_hour != state->lastRotated.tm_hour) ||
        ...
        if (log->rotateMinAge &&
            log->rotateMinAge * DAY_SECONDS >= nowSecs - sb.st_mtime)
        {
            state->doRotate = 0;
            message(MESS_DEBUG, "  log does not need rotating "
...
int mvFile(char* oldName, char* newName, struct logInfo* log, acl_type acl)
{
    ...
    if (errno == EXDEV)
    {
        ...
        {
            if ((fd_new =
                     createOutputFile(newName, O_WRONLY | O_CREAT | O_TRUNC,
                                      &sbprev, acl, 0)) < 0)
                return 1;
        }
        ...
        if ((close(fd_old) < 0) || removeLogFile(oldName, log) ||
            (close(fd_new) < 0))
            return 1;
...
static int findLastRotated(const struct logNames* rotNames, const char* fileext,
                           const char* compext)
{
    char* pattern;
    int glob_rc;
    ...
    if (asprintf(&pattern, "%s/%s.*%s%s", rotNames->dirName, rotNames->baseName,
                 fileext, compext) < 0)
        /* out of memory */
        return -1;
...
#define PATTERN_LEN (DATEEXT_LEN * 2)
    ...
    if (log->addextension)
    {
        ...
        if (baseLen >= extLen &&
            strncmp(&(rotNames->baseName[baseLen - extLen]), log->addextension,
                    extLen) == 0)
        {
            char* tempstr;
    ...
    if (log->extension &&
        strncmp(&(rotNames->baseName[strlen(rotNames->baseName) -
                                     strlen(log->extension)]),
    ...
    if (log->flags & LOG_FLAG_COMPRESS && log->flags & LOG_FLAG_DELAYCOMPRESS)
    {
        if (log->flags & LOG_FLAG_DATEEXT)
        {
            ...
            {
                size_t glob_count;
                sortGlobResult(&globResult,
                               strlen(rotNames->dirName) + 1 +
                                   strlen(rotNames->baseName),
                               dformat);
                for (glob_count = 0;
                     glob_count < globResult.gl_pathc && !hasErrors;
                     glob_count++)
                {
                    struct stat sbprev;
                    ...
                    {
                        message(MESS_FATAL,
                                "could not allocate glob result memory\n");
                    }
                    if (stat(oldName, &sbprev))
                    {
                        if (errno == ENOENT)
    ...
    if (log->flags & LOG_FLAG_DATEEXT)
    {
            ...
            for (glob_count = 0; glob_count < globResult.gl_pathc; glob_count++)
            {
                if (!stat((globResult.gl_pathv)[glob_count], &fst_buf))
                {
                    if (((globResult.gl_pathc >= (size_t)rotateCount) &&
                         (glob_count <= (globResult.gl_pathc - rotateCount))) ||
                        ((log->rotateAge > 0) &&
                          ...
                          log->rotateAge)))
                    {
                        if (mail_out != -1)
                        {
                            ...
                            if (!hasErrors && log->logAddress)
                                hasErrors = mailLogWrapper(
                                    mailFilename, mailCommand, logNum, log);
                            if (!hasErrors)
                            {
        ...
        sprintf(rotNames->firstRotated, "%s/%s%s%s%s", rotNames->dirName,
                rotNames->baseName, dext_str, fileext,
    ...
    else
    {
        ...
        if (log->rotateAge)
        {
            ...
            for (i = 1; i <= rotateCount + 1; i++)
            {
                ...
                {
                    message(MESS_FATAL,
                            "could not allocate mailFilename memory\n");
                }
                           ...
                           struct logState* state, struct logNames* rotNames)
{
    ...
    if (!hasErrors)
    {
        ...
        if (!(log->flags & (LOG_FLAG_COPYTRUNCATE | LOG_FLAG_COPY)))
        {
...
#ifdef WITH_ACL
            if ((prev_acl =
                     acl_get_file(log->files[logNum], ACL_TYPE_ACCESS)) == NULL)
            {
                if (is_acl_well_supported(errno))
                {
                    message(MESS_ERROR, "getting file ACL %s: %s\n",
            ...
            {
                if (asprintf(&tmpFilename, "%s%s", log->files[logNum], ".tmp") <
                    0)
                {
                    message(MESS_FATAL,
                            "could not allocate tmpFilename memory\n");
                           ...
                           prev_acl))
                {
                    message(MESS_ERROR, "failed to move %s to %s: %s\n",
                            log->files[logNum], tmpFilename, strerror(errno));
                    hasErrors = 1;
                }
            }
            else
            ...
            if (!log->rotateCount)
            {
                rotNames->disposeName = realloc(
                    rotNames->disposeName, strlen(rotNames->dirName) +
                                               strlen(rotNames->baseName) +
                                               strlen(log->files[logNum]) + 10);
                sprintf(rotNames->disposeName, "%s%s", rotNames->finalName,
                        (log->compress_ext && (log->flags & LOG_FLAG_COMPRESS))
                            ? log->compress_ext
                            : "");
                               ...
                               struct logNames* rotNames)
{
    ...
    if (!hasErrors && log->flags & LOG_FLAG_TMPFILENAME)
    {
        ...
        hasErrors = copyTruncate(tmpFilename, rotNames->finalName, &state->sb,
                                 LOG_FLAG_COPY, /* skip_copy */ 0);
...
static int rotateLogSet(struct logInfo* log, int force)
{
    ...
    else
    {
        switch (log->criterium)
        {
            case ROT_HOURLY:
                ...
                break;
            case ROT_DAYS:
                message(MESS_DEBUG, "after %jd days ",
            ...
            default:
                message(MESS_DEBUG,
                        "rotateLogSet() does not have case for: %u ",
    ...
    if (log->rotateMinAge)
        message(MESS_DEBUG, "only log files older than %d days are rotated, ",
                log->rotateMinAge);
    ...
    if (log->first)
    {
        ...
        else
        {
            ...
            {
                message(MESS_ERROR,
                        "error running first action script "
                        "for %s\n",
                        log->pattern);
                hasErrors = 1;
                if (log->flags & LOG_FLAG_SU)
    ...
    for (j = 0; (!(log->flags & LOG_FLAG_SHAREDSCRIPTS) && j < log->numFiles) ||
                ((log->flags & LOG_FLAG_SHAREDSCRIPTS) && j < 1);
         j++)
    {

        for (i = j;
             ((log->flags & LOG_FLAG_SHAREDSCRIPTS) && i < log->numFiles) ||
             (!(log->flags & LOG_FLAG_SHAREDSCRIPTS) && i == j);
             i++)
        {
               ...
               (hasErrors && (log->flags & LOG_FLAG_SHAREDSCRIPTS)))))
        {
            ...
            {
                char* logfn = (log->flags & LOG_FLAG_SHAREDSCRIPTS)
                                  ? log->pattern
                                  : log->files[j];

                /* It only makes sense to pass in a final rotated filename if
                 * scripts are not shared */
                char* logrotfn = (log->flags & LOG_FLAG_SHAREDSCRIPTS)
                                     ? NULL
                                     : rotNames[j]->finalName;
    ...
    if (log->last)
    {
        ...
        else
        {
            ...
            {
                message(MESS_ERROR,
                        "error running last action script "
                        "for %s\n",
                        log->pattern);
                hasErrors = 1;
            }
...
static int writeState(struct logInfo* log, char* stateFilename)
{
    ...
    fdsave = createOutputFile(tmpFilename, O_RDWR | O_CREAT | O_TRUNC, &sb,
                              prev_acl, 0);
#ifdef WITH_ACL
    if (prev_acl)
    {
    ...
    if (bytes < 0)
         ...
         * astronomical measurement (solar year) rather than something derived
         * from a convention (calendar year).
         */
#define SECONDS_IN_YEAR 31556926
    ...
    for (i = 0; i < hashSize && error == 0; i++)
    {
             ...
             p = p->list.le_next)
        {
            ...
            {
                message(MESS_DEBUG,
                        "Removing %s from state file, "
                        "because it does not exist and has not been rotated "
                        "for one year\n",
                        p->fn);
                continue;
            }
            ...
            if (error == 0)
            {
                bytes = fprintf(f, " %d-%d-%d-%d:%d:%d\n",
                                ...
                                p->lastRotated.tm_mday, p->lastRotated.tm_hour,
                                p->lastRotated.tm_min, p->lastRotated.tm_sec);
                if (bytes < 0)
                    error = bytes;
            }
        }
...
static int readState(const char* stateFilename)
{
    ...
    while (fgets(buf, sizeof(buf) - 1, f))
    {
        ...
        if (poptParseArgvString(buf, &argc, &argv) || (argc != 2) ||
            (sscanf(argv[1], "%d-%d-%d-%d:%d:%d", &year, &month, &day, &hour,
                    &minute, &second) < 3))
        {
        ...
        if (hour < 0 || hour > 23)
        {
            message(MESS_ERROR, "bad hour %d for file %s in state file %s\n",
                    hour, argv[0], stateFilename);
            free(argv);
            fclose(f);
            return 1;
        }
        ...
        {
            message(MESS_ERROR, "bad minute %d for file %s in state file %s\n",
                    minute, argv[0], stateFilename);
            free(argv);
            fclose(f);
            return 1;
        }
        ...
        {
            message(MESS_ERROR, "bad second %d for file %s in state file %s\n",
                    second, argv[0], stateFilename);
            free(argv);
            fclose(f);
            return 1;
        }
...
int main(int argc, const char** argv)
{
    ...
    struct poptOption options[] = {
        ...
        {"verbose", 'v', 0, NULL, 'v', "Display messages during rotation",
         NULL},
        {"log", 'l', POPT_ARG_STRING, &logFile, 'l',
         "Log file or 'syslog' to log to syslog", "logfile"},
    ...
    while ((arg = poptGetNextOpt(optCon)) >= 0)
    {
        switch (arg)
        {
            case 'd':
                ...
                message(
                    MESS_NORMAL,
                    "WARNING: logrotate in debug mode does nothing"
