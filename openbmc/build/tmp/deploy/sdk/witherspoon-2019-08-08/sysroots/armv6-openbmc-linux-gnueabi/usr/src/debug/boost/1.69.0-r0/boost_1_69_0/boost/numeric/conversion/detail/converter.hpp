...
typedef mpl::integral_c<std::float_round_style, std::round_toward_zero>
    round2zero_c;
typedef mpl::integral_c<std::float_round_style, std::round_to_nearest>
    round2nearest_c;
typedef mpl::integral_c<std::float_round_style, std::round_toward_infinity>
    round2inf_c;
typedef mpl::integral_c<std::float_round_style, std::round_toward_neg_infinity>
    round2neg_inf_c;
...
//
template <class RoundStyle, class RoundToZero, class RoundToNearest,
          class RoundToInf, class RoundToNegInf>
struct for_round_style
{
    typedef ct_switch4<RoundStyle, round2zero_c, round2nearest_c,
                       round2inf_c // round2neg_inf_c
                       ,
...
// Because a particular logic can combine either 1 or two predicates, the
// following tags are used to allow the predicate applier to receive 2 preds,
// but optimize away one of them if it is 'non-applicable'
...
//
// The following classes, collectivelly named 'Predicates', are instantiated
// within the corresponding range checkers. Their static member function 'apply'
// is called to perform the actual range checking logic.
//--------------------------------------------------------------------------
...
struct LT_LoT : applicable
{
    ...
    {
        return s < static_cast<S>(bounds<T>::lowest()) ? cNegOverflow
                                                       : cInRange;
    }
};
...
struct GT_HiT : applicable
{
    ...
    static range_check_result apply(argument_type s)
    {
        return s > static_cast<S>(bounds<T>::highest()) ? cPosOverflow
                                                        : cInRange;
    }
};
...
struct combine
{
    ...
    typedef void
        NNone; // 'None' is defined as a macro in (/usr/X11R6/include/X11/X.h)

    typedef typename PredA::do_apply do_applyA;
    typedef typename PredB::do_apply do_applyB;

    typedef
        typename for_both<do_applyA, do_applyB, Both, PredA, PredB, NNone>::type
            type;
};
...
//
// The following classes are VISIBLE base classes of the user-level converter<>
// class. They supply the optimized 'out_of_range()' and 'validate_range()'
// static member functions visible in the user interface.
//
//--------------------------------------------------------------------------
...
//
// All the range checking logic for all possible combinations of source and
// target can be arranged in terms of one or two predicates, which test overflow
// on both neg/pos 'sides' of the ranges.
//
// These predicates are given here as IsNegOverflow and IsPosOverflow.
//
template <class Traits, class IsNegOverflow, class IsPosOverflow,
...
struct GetRC_Sig2Unsig
{
    ...
    typedef generic_range_checker<Traits, Pred1, Pred2, OverflowHandler>
        ChoiceA;

    typedef generic_range_checker<Traits, Pred1, non_applicable,
                                  OverflowHandler>
        ChoiceB;
    ...
    typedef
        typename mpl::if_<positively_subranged, ChoiceA, ChoiceB>::type type;
};
...
struct GetRC_Unsig2Sig
{
    ...
    typedef generic_range_checker<Traits, non_applicable, Pred1,
                                  OverflowHandler>
        type;
};
...
struct GetRC_Int2Int
{
    ...
    typedef typename for_sign_mixture<sign_mixture, Sig2SigQ, Sig2UnsigQ,
                                      Unsig2SigQ, Unsig2UnsigQ>::type selector;
...
struct GetRC_Float2Int
{
    ...
    typedef generic_range_checker<Traits, Pred3, Pred4, OverflowHandler>
        ToNearest;
    typedef generic_range_checker<Traits, Pred1, Pred5, OverflowHandler> ToInf;
    typedef generic_range_checker<Traits, Pred6, Pred2, OverflowHandler>
        ToNegInf;

    typedef typename Float2IntRounder::round_style round_style;

    typedef typename for_round_style<round_style, ToZero, ToNearest, ToInf,
                                     ToNegInf>::type type;
};
...
struct GetRC_BuiltIn2BuiltIn
{
    ...
    typedef GetRC_Float2Int<Traits, OverflowHandler, Float2IntRounder>
        Float2IntQ;
    ...
    typedef
        typename for_int_float_mixture<int_float_mixture, Int2IntQ, Int2FloatQ,
                                       Float2IntQ, Float2FloatQ>::type selector;
...
struct GetRC
{
    typedef GetRC_BuiltIn2BuiltIn<Traits, OverflowHandler, Float2IntRounder>
        BuiltIn2BuiltInQ;
    ...
    typedef
        typename for_udt_builtin_mixture<udt_builtin_mixture, BuiltIn2BuiltInQ,
                                         DummyQ, DummyQ, DummyQ>::type selector;
...
//
// The following classes are VISIBLE base classes of the user-level converter<>
// class. They supply the optimized 'nearbyint()' and 'convert()' static member
// functions visible in the user interface.
//
//--------------------------------------------------------------------------
...
template <class Traits, class OverflowHandler, class Float2IntRounder,
          class RawConverter, class UserRangeChecker>
struct get_non_trivial_converter
{
    typedef GetRC<Traits, OverflowHandler, Float2IntRounder>
        InternalRangeCheckerQ;
    ...
    typedef typename mpl::eval_if<use_internal_RC, InternalRangeCheckerQ,
                                  UserRangeCheckerQ>::type RangeChecker;

    typedef non_rounding_converter<Traits, RangeChecker, RawConverter>
        NonRounding;
    typedef rounding_converter<Traits, RangeChecker, RawConverter,
                               Float2IntRounder>
        Rounding;
    ...
    typedef typename for_int_float_mixture<int_float_mixture, NonRoundingQ,
                                           NonRoundingQ, RoundingQ,
                                           NonRoundingQ>::type selector;
...
struct get_converter_impl
{
    ...
    typedef get_non_trivial_converter<Traits, OverflowHandler, Float2IntRounder,
                                      RawConverter, UserRangeChecker>
        NonTrivialQ;
