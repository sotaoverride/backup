...
int CMS_SignerInfo_sign(CMS_SignerInfo* si)
{
    ...
    if (EVP_PKEY_CTX_ctrl(pctx, -1, EVP_PKEY_OP_SIGN, EVP_PKEY_CTRL_CMS_SIGN, 0,
                          si) <= 0)
    {
    ...
    if (EVP_PKEY_CTX_ctrl(pctx, -1, EVP_PKEY_OP_SIGN, EVP_PKEY_CTRL_CMS_SIGN, 1,
                          si) <= 0)
    {
        CMSerr(CMS_F_CMS_SIGNERINFO_SIGN, CMS_R_CTRL_ERROR);
        goto err;
...
int CMS_SignerInfo_verify(CMS_SignerInfo* si)
{
    ...
    r = EVP_DigestVerifyFinal(mctx, si->signature->data, si->signature->length);
    if (r <= 0)
...
int CMS_SignerInfo_verify_content(CMS_SignerInfo* si, BIO* chain)
{
    ...
    else
    {
        ...
        if (!cms_sd_asn1_ctrl(si, 1))
            goto err;
        r = EVP_PKEY_verify(pkctx, si->signature->data, si->signature->length,
                            mval, mlen);
        if (r <= 0)
        {
            CMSerr(CMS_F_CMS_SIGNERINFO_VERIFY_CONTENT,
...
int CMS_add_smimecap(CMS_SignerInfo* si, STACK_OF(X509_ALGOR) * algs)
{
    ...
    if (smderlen <= 0)
        return 0;
    r = CMS_signed_add1_attr_by_NID(si, NID_SMIMECapabilities, V_ASN1_SEQUENCE,
                                    smder, smderlen);
    OPENSSL_free(smder);
    return r;
}
