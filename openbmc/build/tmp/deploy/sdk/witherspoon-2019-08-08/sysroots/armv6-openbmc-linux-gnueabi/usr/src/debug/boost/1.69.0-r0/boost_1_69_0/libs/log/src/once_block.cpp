 ...
 * \date   23.06.2010
 *
 * \brief  This file is the Boost.Log library implementation, see the library
 * documentation at
 * http://www.boost.org/doc/libs/release/libs/log/doc/html/index.html.
 *
 * The code in this file is based on the \c call_once function implementation in
 * Boost.Thread.
 */
...
BOOST_LOG_ANONYMOUS_NAMESPACE
{
    ...
    boost::winapi::CONDITION_VARIABLE_ g_OnceBlockCond =
        ...
        else
        {
            while (flag.status == once_block_flag::being_initialized)
            {
                BOOST_VERIFY(boost::winapi::SleepConditionVariableSRW(
                    &g_OnceBlockCond, &g_OnceBlockMutex,
                    boost::winapi::INFINITE_, 0));
            }
        }
    }
...
BOOST_LOG_ANONYMOUS_NAMESPACE
{
    ...
    class once_block_impl_nt6 : public once_block_impl_base
    {
      public:
        struct winapi_srwlock
        {
            void* p;
        };
        struct winapi_condition_variable
        {
            void* p;
        };

        typedef void(BOOST_WINAPI_WINAPI_CC* InitializeSRWLock_t)(
            winapi_srwlock*);
        typedef void(BOOST_WINAPI_WINAPI_CC* AcquireSRWLockExclusive_t)(
            winapi_srwlock*);
        typedef void(BOOST_WINAPI_WINAPI_CC* ReleaseSRWLockExclusive_t)(
            winapi_srwlock*);
        typedef void(BOOST_WINAPI_WINAPI_CC* InitializeConditionVariable_t)(
            winapi_condition_variable*);
        typedef boost::winapi::BOOL_(
            BOOST_WINAPI_WINAPI_CC* SleepConditionVariableSRW_t)(
            winapi_condition_variable*, winapi_srwlock*, boost::winapi::DWORD_,
            boost::winapi::ULONG_);
        typedef void(BOOST_WINAPI_WINAPI_CC* WakeAllConditionVariable_t)(
            winapi_condition_variable*);
      ...
      public:
        ...
        void rollback(once_block_flag& flag)
        {
            ...
            // The initializer failed, marking the flag as if it hasn't run at
            // all
    ...
    class once_block_impl_nt5 : public once_block_impl_base
    {
      private:
        mutex m_Mutex;
    ...
    once_block_impl_base* create_once_block_impl()
    {
        ...
        if (hKernel32)
        {
            once_block_impl_nt6::InitializeSRWLock_t pInitializeSRWLock =
                (once_block_impl_nt6::InitializeSRWLock_t)
                    boost::winapi::get_proc_address(hKernel32,
                                                    "InitializeSRWLock");
            if (pInitializeSRWLock)
            {
                once_block_impl_nt6::AcquireSRWLockExclusive_t
                    pAcquireSRWLockExclusive =
                        (once_block_impl_nt6::AcquireSRWLockExclusive_t)
                            boost::winapi::get_proc_address(
                                hKernel32, "AcquireSRWLockExclusive");
                if (pAcquireSRWLockExclusive)
                {
                    once_block_impl_nt6::ReleaseSRWLockExclusive_t
                        pReleaseSRWLockExclusive =
                            (once_block_impl_nt6::ReleaseSRWLockExclusive_t)
                                boost::winapi::get_proc_address(
                                    hKernel32, "ReleaseSRWLockExclusive");
                    if (pReleaseSRWLockExclusive)
                    {
                        once_block_impl_nt6::InitializeConditionVariable_t
                        ...
                        {
                            once_block_impl_nt6::SleepConditionVariableSRW_t
                            ...
                            {
                                once_block_impl_nt6::WakeAllConditionVariable_t
    ...
    {
        once_block_impl_base* impl =
            (once_block_impl_base*)BOOST_INTERLOCKED_EXCHANGE_POINTER(
                (void**)&g_pOnceBlockImpl, NULL);
        delete impl;
    }
        ...
        if (!impl)
            try
            {
                ...
                impl = (once_block_impl_base*)
                    BOOST_INTERLOCKED_COMPARE_EXCHANGE_POINTER(
                        (void**)&g_pOnceBlockImpl, (void*)new_impl, NULL);
                if (impl)
                {
                    delete new_impl;
                }
            ...
            catch (...)
            {
                BOOST_ASSERT_MSG(false,
                                 "Boost.Log: Failed to initialize the once "
                                 "block thread synchronization structures");
                std::abort();
            }
...
BOOST_LOG_API bool once_block_sentry::enter_once_block() const BOOST_NOEXCEPT
{
    ...
    while (flag.status != once_block_flag::initialized)
    {
        ...
        else
        {
            ...
            {
                BOOST_VERIFY(
                    !pthread_cond_wait(&g_OnceBlockCond, &g_OnceBlockMutex));
            }
        }
    }
