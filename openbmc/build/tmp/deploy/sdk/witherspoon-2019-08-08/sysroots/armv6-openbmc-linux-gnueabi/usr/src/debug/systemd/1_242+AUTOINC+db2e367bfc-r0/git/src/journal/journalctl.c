...
static int pattern_compile(const char* pattern, unsigned flags,
                           pcre2_code** out)
{
    int errorcode, r;
    ...
    p = pcre2_compile((PCRE2_SPTR8)pattern, PCRE2_ZERO_TERMINATED, flags,
...
static int add_matches_for_device(sd_journal* j, const char* devpath)
{
    ...
    if (r < 0)
        return log_error_errno(r, "Failed to get device from devnum %u:%u: %m",
        ...
        if (sd_device_get_devname(d, &devnode) >= 0)
        {
            ...
            if (r < 0)
                return log_error_errno(
                    r, "Failed to stat() device node \"%s\": %m", devnode);

            r = asprintf(&match1, "_KERNEL_DEVICE=%c%u:%u",
                         S_ISBLK(st.st_mode) ? 'b' : 'c', major(st.st_rdev),
                         minor(st.st_rdev));
            if (r < 0)
                return log_oom();
...
static int parse_boot_descriptor(const char* x, sd_id128_t* boot_id,
                                 int* offset)
{
...
static int help(void)
{
    ...
    printf(
        ...
        "     --user                  Show the user journal for the current "
        "user\n"
        ...
        "specified date\n"
        "  -c --cursor=CURSOR         Show entries starting at the specified "
        "cursor\n"
        "     --after-cursor=CURSOR   Show entries after the specified cursor\n"
        "     --show-cursor           Print the cursor after all the entries\n"
        "     --cursor-file=FILE      Show entries after cursor in FILE and "
        "update FILE\n"
        ...
        "  -t --identifier=STRING     Show entries with the specified syslog "
        "identifier\n"
        "  -p --priority=RANGE        Show entries with the specified "
        "priority\n"
        "  -g --grep=PATTERN          Show entries with MESSAGE matching "
        "PATTERN\n"
        "     --case-sensitive[=BOOL] Force case sensitive or insenstive "
        "matching\n"
        "  -e --pager-end             Immediately jump to the end in the "
        "pager\n"
        ...
        "short-full,\n"
        "                               short-monotonic, short-unix, verbose, "
        "export,\n"
        "                               json, json-pretty, json-sse, json-seq, "
        "cat,\n"
        "                               with-unit)\n"
        "     --output-fields=LIST    Select fields to print in "
        "verbose/export/json modes\n"
        ...
        "     --interval=TIME         Time interval for changing the FSS "
        "sealing key\n"
        "     --verify-key=KEY        Specify FSS verification key\n"
        "     --force                 Override of the FSS key pair with "
        "--setup-keys\n"
        "\nCommands:\n"
        "  -h --help                  Show this help text\n"
        "     --version               Show package version\n"
        "  -N --fields                List all field names currently used\n"
        "  -F --field=FIELD           List all values that a specified field "
        "takes\n"
        "     --disk-usage            Show total disk usage of all journal "
        "files\n"
        ...
        "     --sync                  Synchronize unwritten journal messages "
        "to disk\n"
...
static int parse_argv(int argc, char* argv[])
{
    ...
    while ((c = getopt_long(argc, argv,
        ...
        switch (c)
        {
            ...
            case 'o':
                ...
                if (IN_SET(arg_output, OUTPUT_EXPORT, OUTPUT_JSON,
                           OUTPUT_JSON_PRETTY, OUTPUT_JSON_SSE, OUTPUT_JSON_SEQ,
                           OUTPUT_CAT))
                    arg_quiet = true;
            ...
            case 'b':
                ...
                if (optarg)
                {
                    r = parse_boot_descriptor(optarg, &arg_boot_id,
                                              &arg_boot_offset);
                    if (r < 0)
                        return log_error_errno(
                            r, "Failed to parse boot descriptor '%s'", optarg);
                ...
                {
                    r = parse_boot_descriptor(argv[optind], &arg_boot_id,
                                              &arg_boot_offset);
                    if (r >= 0)
                    {
                        arg_boot = r;
            ...
            case ARG_FILE:
                if (streq(optarg, "-"))
                    /* An undocumented feature: we can read journal files from
                     * STDIN. We don't document this though, since after all we
                     * only support this for mmap-able, seekable files, and not
                     * for example pipes which are probably the primary usecase
                     * for reading things from STDIN. To avoid confusion we
                     * hence don't document this feature. */
                    arg_file_stdin = true;
                else
                {
            ...
            case ARG_VACUUM_SIZE:
                ...
                arg_action = arg_action == ACTION_ROTATE
                                 ? ACTION_ROTATE_AND_VACUUM
                                 : ACTION_VACUUM;
                break;
            ...
            case ARG_INTERVAL:
                ...
                {
                    log_error("Failed to parse sealing key change interval: %s",
                              optarg);
                    return -EINVAL;
                }
                break;
            ...
            case ARG_CASE_SENSITIVE:
                if (optarg)
                {
                    ...
                    if (r < 0)
                        return log_error_errno(
                            r, "Bad --case-sensitive= argument \"%s\": %m",
                            optarg);
            ...
            case ARG_ROTATE:
                arg_action = arg_action == ACTION_VACUUM
                                 ? ACTION_ROTATE_AND_VACUUM
                                 : ACTION_ROTATE;
                break;
    ...
    {
        log_error("Please specify at most one of -D/--directory=, --file=, "
                  "-M/--machine=, --root.");
        return -EINVAL;
    }
    ...
    {
        log_error("Please specify only one of --since=, --cursor=, and "
                  "--after-cursor.");
        return -EINVAL;
    }
    ...
    if (!IN_SET(arg_action, ACTION_SHOW, ACTION_DUMP_CATALOG,
                ACTION_LIST_CATALOG) &&
        optind < argc)
    {
    ...
    if (!strv_isempty(arg_system_units) &&
        arg_journal_type == SD_JOURNAL_CURRENT_USER)
    {
        /* Specifying --user and --unit= at the same time makes no sense (as the
         * former excludes the user journal, but the latter excludes the system
         * journal, thus resulting in empty output). Let's be nice
         * to users, and automatically turn --unit= into --user-unit= if
         * combined with --user. */
    ...
    if (arg_pattern)
    {
        ...
        else
        {
            ...
            r = pcre2_match(cs, (PCRE2_SPTR8)arg_pattern, PCRE2_ZERO_TERMINATED,
                            0, 0, md, NULL);
            has_case = r >= 0;
    ...
    STRV_FOREACH(i, args)
    {
        ...
        else if (path_is_absolute(*i))
        {
            ...
            else if (S_ISCHR(st.st_mode) || S_ISBLK(st.st_mode))
            {
                ...
                return log_error_errno(SYNTHETIC_ERRNO(EINVAL),
                                       "File is neither a device node, nor "
                                       "regular file, nor executable: %s",
                                       *i);
...
static int discover_next_boot(sd_journal* j, sd_id128_t previous_boot_id,
                              bool advance_older, BootId** ret)
{
    ...
    do
    {
        ...
        /* We iterate through this in a loop, until the boot ID differs from the
         * previous one. Note that normally, this will only require a single
         * iteration, as we seeked to the last entry of the previous boot entry
         * already. However, it might happen that the per-journal-field entry
         * arrays are less complete than the main entry array, and hence might
         * reference an entry that's not actually the last one of the boot ID as
         * last one. Let's hence use the per-field array is initial seek
         * position to speed things up, but let's not trust that it is complete,
         * and hence, manually advance as necessary. */
    ...
    if (r < 0)
        ...
        return log_debug_errno(SYNTHETIC_ERRNO(ENODATA),
                               ...
                               "its last entry."); /* This shouldn't happen. We
                                                      just came from this very
                                                      boot ID. */
...
static int get_boots(sd_journal* j, BootId** boots, sd_id128_t* boot_id,
                     int offset)
{
    ...
    if (boot_id && !sd_id128_is_null(*boot_id))
    {
        ...
        /* At this point the read pointer is positioned at the oldest/newest
         * occurence of the reference boot ID. After flushing the matches, one
         * more invocation of _previous()/_next() will hence place us at the
         * following entry, which must then have an older/newer boot ID */
    }
    else
    {
         ...
         *
         * At this point the read pointer is positioned after the newest/before
         * the oldest entry in the whole journal. The next invocation of
         * _previous()/_next() will hence position us at the newest/oldest
         * entry we have. */
    }
    ...
    for (;;)
    {
        ...
        else
        {
            LIST_FOREACH(boot_list, id, head)
            {
                ...
                {
                    /* boot id already stored, something wrong with the journal
                     * files */
                    /* exiting as otherwise this problem would cause forever
                     * loop */
                    goto finish;
                }
            }
...
static int add_boot(sd_journal* j)
{
    ...
    /* Take a shortcut and use the current boot_id, which we can do very
     * quickly. We can do this only when we logs are coming from the current
     * machine, so take the slow path if log location is specified. */
    ...
    {
        const char* reason =
            (r == 0) ? "No such boot ID in journal" : strerror(-r);

        if (sd_id128_is_null(arg_boot_id))
            log_error("Data from the specified boot (%+i) is not available: %s",
                      arg_boot_offset, reason);
        else
            log_error("Data from the specified boot (" SD_ID128_FORMAT_STR
                      ") is not available: %s",
...
static int get_possible_units(sd_journal* j, const char* fields,
                              char** patterns, Set** units)
{
...
static int add_units(sd_journal* j)
{
    ...
    STRV_FOREACH(i, arg_system_units)
    {
        ...
        r = unit_name_mangle(
            *i, UNIT_NAME_MANGLE_GLOB | (arg_quiet ? 0 : UNIT_NAME_MANGLE_WARN),
            &u);
        if (r < 0)
            return r;
...
static int setup_keys(void)
{
    ...
    r = chattr_fd(fd, FS_SECRM_FL | FS_NODUMP_FL | FS_SYNC_FL | FS_NOCOW_FL,
                  FS_SECRM_FL | FS_NODUMP_FL | FS_SYNC_FL | FS_NOCOW_FL, NULL);
    if (r < 0)
    ...
    if (on_tty())
    {
        fprintf(stderr,
                "\n"
                "The new key pair has been generated. The %ssecret sealing "
                "key%s has been written to\n"
                "the following local file. This key file is automatically "
                "updated when the\n"
                "sealing key is advanced. It should not be used on multiple "
                "hosts.\n"
                "\n"
                "\t%s\n"
                "\n"
                "Please write down the following %ssecret verification key%s. "
                "It should be stored\n"
                "at a safe location and should not be saved locally on disk.\n"
                "\n\t%s",
                ansi_highlight(), ansi_normal(), p, ansi_highlight(),
                ansi_normal(), ansi_highlight_red());
        fflush(stderr);
    }
    ...
    if (on_tty())
    {
        ...
        if (is_locale_utf8())
        {
            fputs("\nTo transfer the verification key to your phone please "
                  "scan the QR code below:\n\n",
                  stderr);
            print_qr_code(stderr, seed, seed_size, n, arg_interval, hn,
                          machine);
        }
#endif
        free(hn);
...
static int verify(sd_journal* j)
{
    ...
    ORDERED_HASHMAP_FOREACH(f, j->files, i)
    {
        ...
        if (!arg_verify_key && JOURNAL_HEADER_SEALED(f->header))
            log_notice("Journal file %s has sealing enabled but verification "
                       "key has not been passed using --verify-key=.",
                       f->path);
#endif

        k = journal_file_verify(f, arg_verify_key, &first, &validated, &last,
                                true);
        if (k == -EINVAL)
        {
...
static int watch_run_systemd_journal(uint32_t mask)
{
    ...
    if (inotify_add_watch(watch_fd, "/run/systemd/journal", mask) < 0)
        return log_error_errno(errno,
                               "Failed to watch \"/run/systemd/journal\": %m");
...
static int flush_to_var(void)
{
    ...
    r = sd_bus_call_method(
        ...
        "systemd-journald.service", "main", SIGUSR1);
    if (r < 0)
        return log_error_errno(r, "Failed to kill journal service: %s",
                               bus_error_message(&error, r));

    watch_fd =
        watch_run_systemd_journal(IN_CREATE | IN_DONT_FOLLOW | IN_ONLYDIR);
    if (watch_fd < 0)
        return watch_fd;
    ...
    for (;;)
    {
        ...
        if (errno != ENOENT)
            return log_error_errno(errno, "Failed to check for existence of "
                                          "/run/systemd/journal/flushed: %m");
...
static int send_signal_and_wait(int sig, const char* watch_path)
{
    ...
    if (arg_machine)
    {
        log_error("--sync and --rotate are not supported in conjunction with "
                  "--machine=.");
        return -EOPNOTSUPP;
    }
    ...
    for (;;)
    {
        ...
        if (!bus)
        {
            ...
            r = sd_bus_call_method(
                bus, "org.freedesktop.systemd1", "/org/freedesktop/systemd1",
                "org.freedesktop.systemd1.Manager", "KillUnit", &error, NULL,
        ...
        if (watch_fd < 0)
        {
            watch_fd = watch_run_systemd_journal(IN_MOVED_TO | IN_DONT_FOLLOW |
                                                 IN_ONLYDIR);
            if (watch_fd < 0)
                return watch_fd;

            /* Recheck the flag file immediately, so that we don't miss any
             * event since the last check. */
            continue;
        }

        /* OK, all preparatory steps done, let's wait until inotify reports an
         * event. */
...
static int wait_for_change(sd_journal* j, int poll_fd)
{
    ...
    /* Much like sd_journal_wait() but also keeps an eye on STDOUT, and exits as
     * soon as we see a POLLHUP on that, i.e. when it is closed. */
    ...
    if (ppoll(pollfds, ELEMENTSOF(pollfds),
              timeout == USEC_INFINITY ? NULL : timespec_store(&ts, timeout),
              NULL) < 0)
    {
        if (errno == EINTR)
            return 0;
...
{
    bool previous_boot_id_valid = false, first_line = true, ellipsized = false,
         need_seek = false;
    ...
    /* Increase max number of open files if we can, we might needs this when
     * browsing journal files, which might be split up into many files. */
    ...
    switch (arg_action)
    {
        ...
        case ACTION_UPDATE_CATALOG:
        {
            ...
            else
            {
                ...
                if (optind < argc)
                    r = catalog_list_items(stdout, database, oneline,
                                           argv + optind);
                else
    ...
    if (arg_directory)
        ...
        r = sd_journal_open_directory(&j, arg_root,
                                      arg_journal_type | SD_JOURNAL_OS_ROOT);
    ...
    else if (arg_machine)
    {
        ...
        if (geteuid() != 0)
        {
            /* The file descriptor returned by OpenMachineRootDirectory() will
             * be owned by users/groups of the container, thus we need root
        ...
        r = sd_bus_call_method(
            bus, "org.freedesktop.machine1", "/org/freedesktop/machine1",
            "org.freedesktop.machine1.Manager", "OpenMachineRootDirectory",
            &error, &reply, "s", arg_machine);
        if (r < 0)
        {
            log_error_errno(r, "Failed to open root directory: %s",
                            bus_error_message(&error, r));
            goto finish;
        }
        ...
        r = sd_journal_open(&j, !arg_merge * SD_JOURNAL_LOCAL_ONLY +
                                    arg_journal_type);
    if (r < 0)
    {
    ...
    r = journal_access_check_and_warn(
        j, arg_quiet,
    ...
    switch (arg_action)
    {
        ...
        case ACTION_VACUUM:
        {
            ...
            HASHMAP_FOREACH(d, j->directories_by_path, i)
            {
                ...
                q = journal_directory_vacuum(d->path, arg_vacuum_size,
                                             arg_vacuum_n_files,
                                             arg_vacuum_time, NULL, !arg_quiet);
                if (q < 0)
                {
    ...
    if (arg_boot_offset != 0 && sd_journal_has_runtime_files(j) > 0 &&
        sd_journal_has_persistent_files(j) == 0)
    {
        log_info("Specifying boot ID or boot offset has no effect, no "
                 "persistent journal was found.");
        r = 0;
        goto finish;
    }
    ...
    if (arg_action == ACTION_LIST_FIELDS)
    {
        ...
        SD_JOURNAL_FOREACH_UNIQUE(j, data, size)
        {
            ...
            if (eq)
                printf("%.*s\n",
                       (int)(size -
                             ((const uint8_t*)eq - (const uint8_t*)data + 1)),
                       (const char*)eq + 1);
            else
    ...
    if (arg_follow)
    {
        ...
        {
            log_error_errno(poll_fd, "The --follow switch is not supported in "
                                     "conjunction with reading from STDIN.");
            goto finish;
        }
        else if (poll_fd < 0)
        {
    ...
    if (arg_cursor || arg_after_cursor || arg_cursor_file)
    {
        ...
        if (arg_cursor_file)
        {
            ...
            if (r < 0 && r != -ENOENT)
            {
                log_error_errno(r, "Failed to read cursor file %s: %m",
                                arg_cursor_file);
                goto finish;
            }
        ...
        if (r > 0)
        {
            if (arg_follow)
                printf("-- Logs begin at %s. --\n",
                       format_timestamp_maybe_utc(start_buf, sizeof(start_buf),
                                                  start));
            else
                printf(
                    "-- Logs begin at %s, end at %s. --\n",
                    format_timestamp_maybe_utc(start_buf, sizeof(start_buf),
                                               start),
    ...
    for (;;)
    {
               ...
               (arg_follow && !first_line))
        {
            ...
            if (arg_compiled_pattern)
            {
                ...
                r = pcre2_match(arg_compiled_pattern, message,
                                len - strlen("MESSAGE="),
            ...
            flags = arg_all * OUTPUT_SHOW_ALL | arg_full * OUTPUT_FULL_WIDTH |
                    colors_enabled() * OUTPUT_COLOR |
                    arg_catalog * OUTPUT_CATALOG | arg_utc * OUTPUT_UTC |
            ...
            /* If journalctl take a long time to process messages, and during
             * that time journal file rotation occurs, a journalctl client will
             * keep those rotated files open until it calls
             * sd_journal_process(), which typically happens as a result of
             * calling sd_journal_wait() below in the "following" case.  By
             * periodically calling sd_journal_process() during the processing
            ...
            if (arg_show_cursor || arg_cursor_file)
            {
                ...
                else if (r >= 0)
                {
                    ...
                    if (arg_cursor_file)
                    {
                        ...
                        if (r < 0)
                            log_error_errno(
                                r, "Failed to write new cursor to %s: %m",
                                arg_cursor_file);
                    }
                }
            }
