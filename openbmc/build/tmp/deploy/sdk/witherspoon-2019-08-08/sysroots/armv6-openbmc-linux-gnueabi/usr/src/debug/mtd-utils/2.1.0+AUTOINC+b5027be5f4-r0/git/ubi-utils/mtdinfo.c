...
static void display_help(void)
{
    printf(
        ...
        "\n"
        "Options:\n"
        "-u, --ubi-info                  print what would UBI layout be if it "
        "was put\n"
        "                                on this MTD device\n"
        "-M, --map                       print eraseblock map\n"
        "-a, --all                       print information about all MTD "
        "devices\n"
        "                                Note: `--all' may give less info per "
        "device\n"
        ...
        "  %1$s -a                    print information about all MTD "
        "devices\n",
                           ...
                           const struct mtd_dev_info* mtd)
{
    ...
    ubigen_info_init(&ui, mtd->eb_size, mtd->min_io_size, mtd->subpage_size, 0,
                     1, 0);
        ...
        for (r = 0; r < mtd->region_cnt; ++r)
        {
            ...
            if (mtd_regioninfo(fd, r, &reginfo) == 0)
            {
                printf(" offset: %#x size: %#x numblocks: %#x\n",
                       reginfo.offset, reginfo.erasesize, reginfo.numblocks);
                if (args.map)
...
static int print_dev_info(libmtd_t libmtd, const struct mtd_info* mtd_info,
                          int mtdn)
{
    int err;
    struct mtd_dev_info mtd;
    ...
    if (err)
    {
        if (errno == ENODEV)
            return errmsg("mtd%d does not correspond to any "
                          "existing MTD device",
                          mtdn);
        return sys_errmsg("cannot get information about MTD device %d", mtdn);
    }
