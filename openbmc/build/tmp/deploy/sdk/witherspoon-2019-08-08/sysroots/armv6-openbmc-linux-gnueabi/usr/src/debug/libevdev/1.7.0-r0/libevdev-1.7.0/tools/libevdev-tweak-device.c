...
{
    printf("%s --abs <axis> [--min min] [--max max] [--res res] [--fuzz fuzz] "
           "[--flat flat] /dev/input/eventXYZ\n"
...
static bool parse_resolution_argument(const char* arg, int* xres, int* yres)
{
    int matched;
...
static int parse_event_code(int type, const char* str)
{
    int code;
...
static int parse_options_abs(int argc, char** argv, unsigned int* changes,
                             int* axis, struct input_absinfo* absinfo)
{
    ...
    static struct option opts[] = {
        {"abs", 1, 0, OPT_ABS},   {"min", 1, 0, OPT_MIN},
        {"max", 1, 0, OPT_MAX},   {"fuzz", 1, 0, OPT_FUZZ},
        {"flat", 1, 0, OPT_FLAT}, {"res", 1, 0, OPT_RES},
        {NULL, 0, 0, 0},
    };
...
static int parse_options_led(int argc, char** argv, int* led, int* led_state)
{
    int rc = 1;
    int c;
    int option_index = 0;
...
static int parse_options_resolution(int argc, char** argv, int* xres, int* yres)
{
    int rc = 1;
    int c;
    int option_index = 0;
    ...
    while (1)
    {
        ...
        {
            case OPT_RESOLUTION:
                if (!parse_resolution_argument(optarg, xres, yres))
                    goto error;
                break;
            default:
                goto error;
...
static enum mode parse_options_mode(int argc, char** argv)
{
    int c;
    int option_index = 0;
...
static void set_abs(struct libevdev* dev, unsigned int changes,
                    unsigned int axis, struct input_absinfo* absinfo)
{
    ...
    {
        fprintf(stderr, "Device '%s' doesn't have axis %s\n",
    ...
    if (rc != 0)
        fprintf(stderr, "Failed to set absinfo %s: %s",
                libevdev_event_code_get_name(EV_ABS, axis), strerror(-rc));
}

static void set_led(struct libevdev* dev, unsigned int led, int led_state)
{
    int rc;
    ...
    {
        fprintf(stderr, "Device '%s' doesn't have %s\n", libevdev_get_name(dev),
    ...
    if (rc != 0)
        fprintf(stderr, "Failed to set LED %s: %s",
                libevdev_event_code_get_name(EV_LED, led), strerror(-rc));
}

static void set_resolution(struct libevdev* dev, int xres, int yres)
{
    struct input_absinfo abs;
...
int main(int argc, char** argv)
{
    ...
    switch (mode)
    {
        ...
        case MODE_NONE:
            ...
            goto out;
        case MODE_ABS:
            rc = parse_options_abs(argc, argv, &changes, &axis, &absinfo);
            break;
        case MODE_LED:
            rc = parse_options_led(argc, argv, &led, &led_state);
            break;
        case MODE_RESOLUTION:
            rc = parse_options_resolution(argc, argv, &xres, &yres);
            break;
        default:
            fprintf(stderr,
