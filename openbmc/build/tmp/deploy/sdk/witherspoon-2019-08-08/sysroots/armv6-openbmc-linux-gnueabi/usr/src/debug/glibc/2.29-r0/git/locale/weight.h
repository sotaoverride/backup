...
findidx(const int32_t* table, const int32_t* indirect,
        const unsigned char* extra, const unsigned char** cpp, size_t len)
{
            ...
            if (cnt != nhere)
            {
                if (cnt == len || cp[cnt] > usrc[cnt])
                {
                    ...
                    if (!LOCFILE_ALIGNED_P(1 + 2 * nhere))
                        cp += (LOCFILE_ALIGN - (1 + 2 * nhere) % LOCFILE_ALIGN);
                    continue;
                }
