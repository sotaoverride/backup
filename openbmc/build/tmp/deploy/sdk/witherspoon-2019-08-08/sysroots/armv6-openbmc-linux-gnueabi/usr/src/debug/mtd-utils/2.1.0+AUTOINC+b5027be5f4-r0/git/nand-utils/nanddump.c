...
static void display_help(int status)
{
    fprintf(
        ...
        "           --bb=METHOD          Choose bad block handling method (see "
        "below).\n"
        ...
        "                                Skip bad blocks when seeking to the "
        "start address\n"
        "\n"
...
static void process_options(int argc, char* const argv[])
{
            ...
            case 0:
                switch (option_index)
                {
                    ...
                    case 2: /* --omitoob */
                        ...
                        {
                            errmsg_die(
                                "--oob and --oomitoob are mutually exclusive");
                        }
                        break;
    ...
    if (length < 0)
        errmsg_die("Can't specify negative length with option -l: %lld",
                   length);
...
int main(int argc, char* const argv[])
{
    ...
    for (ofs = start_addr; ofs < end_addr; ofs += bs)
    {
        ...
        else if (blockstart != (ofs & (~mtd.eb_size + 1)) || firstblock)
        {
        ...
        else
        {
            ...
            if (mtd_read(&mtd, fd, ofs / mtd.eb_size, ofs % mtd.eb_size,
                         readbuf, bs))
            {
                errmsg("mtd_read");
                goto closeall;
        ...
        if (eccstats)
        {
            ...
            if (stat1.failed != stat2.failed)
                fprintf(stderr,
                        "ECC: %d uncorrectable bitflip(s)"
                        " at offset 0x%08llx\n",
                        stat2.failed - stat1.failed, ofs);
            if (stat1.corrected != stat2.corrected)
                fprintf(stderr,
                        "ECC: %d corrected bitflip(s) at"
                        " offset 0x%08llx\n",
        ...
        if (pretty_print)
        {
            ...
            {
                pretty_dump_to_buffer(readbuf + i, PRETTY_ROW_SIZE, pretty_buf,
                                      PRETTY_BUF_LEN, true, canonical, ofs + i);
        ...
        if (pretty_print)
        {
            ...
            {
                pretty_dump_to_buffer(oobbuf + i, mtd.oob_size - i, pretty_buf,
                                      PRETTY_BUF_LEN, false, canonical, 0);
                err = ofd_write(ofd, pretty_buf, strlen(pretty_buf));
                if (err)
                    goto closeall;
            }
