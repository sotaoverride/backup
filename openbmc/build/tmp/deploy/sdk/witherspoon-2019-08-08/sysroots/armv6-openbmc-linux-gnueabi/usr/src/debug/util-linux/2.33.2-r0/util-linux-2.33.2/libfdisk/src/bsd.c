...
static const char* bsd_dktypenames[] = {
    ...
    "ST506",   "HP-IB", "HP-FL", "type 9",  "floppy", NULL};
#define BSD_DKMAXTYPES (ARRAY_SIZE(bsd_dktypenames) - 1)

static struct fdisk_parttype bsd_fstypes[] = {{BSD_FS_UNUSED, "unused"},
                                              {BSD_FS_SWAP, "swap"},
                                              {BSD_FS_V6, "Version 6"},
                                              {BSD_FS_V7, "Version 7"},
...
static struct fdisk_parttype* bsd_partition_parttype(struct fdisk_context* cxt,
...
static int bsd_assign_dos_partition(struct fdisk_context* cxt)
{
    ...
    for (i = 0; i < 4; i++)
    {
        ...
        {
            fdisk_warnx(cxt,
                        _("Partition %zd: has invalid starting "
                          "sector 0."),
                        i + 1);
            return -1;
        }
        ...
        {
            free(cxt->dev_path);
            cxt->dev_path = fdisk_partname(cxt->parent->dev_path, i + 1);
        }
...
static int set_parttype(struct fdisk_context* cxt, size_t partnum,
                        struct fdisk_parttype* t)
{
    struct bsd_partition* p;
                             ...
                             struct fdisk_partition* pa, size_t* partno)
{
    ...
    else
    {
        ...
        {
            fdisk_ask_set_query(
                ask, _("Last cylinder, +/-cylinders or +/-size{K,M,G,T,P}"));
                             ...
                             struct fdisk_partition* pa)
{
        ...
        (fdisk_partition_has_start(pa) || fdisk_partition_has_size(pa)))
    {
        ...
        fdisk_sector_t dosend =
            dosbegin + dos_partition_get_size(l->dos_part) - 1;
        fdisk_sector_t begin =
            fdisk_partition_has_start(pa) ? pa->start : p->p_offset;
        fdisk_sector_t end =
            begin + (fdisk_partition_has_size(pa) ? pa->size : p->p_size) - 1;
...
static int bsd_create_disklabel(struct fdisk_context* cxt)
{
    ...
    fdisk_info(cxt, _("The device %s does not contain BSD disklabel."),
               cxt->dev_path);
    rc =
        fdisk_ask_yesno(cxt, _("Do you want to create a BSD disklabel?"), &yes);
    if (rc)
        return rc;
    if (!yes)
        return 1;
...
static int bsd_delete_part(struct fdisk_context* cxt, size_t partnum)
{
    struct bsd_disklabel* d = self_disklabel(cxt);
...
static int bsd_get_disklabel_item(struct fdisk_context* cxt,
...
static uint32_t ask_uint32(struct fdisk_context* cxt, uint32_t dflt, char* mesg)
{
    uintmax_t res;

    if (fdisk_ask_number(cxt, min(dflt, (uint32_t)1), dflt, UINT32_MAX, mesg,
                         &res) == 0)
        return res;
    return dflt;
}

static uint16_t ask_uint16(struct fdisk_context* cxt, uint16_t dflt, char* mesg)
{
    uintmax_t res;

    if (fdisk_ask_number(cxt, min(dflt, (uint16_t)1), dflt, UINT16_MAX, mesg,
                         &res) == 0)
        return res;
    return dflt;
}
...
int fdisk_bsd_edit_disklabel(struct fdisk_context* cxt)
{
    ...
    if (fdisk_ask_number(cxt, DEFAULT_SECTOR_SIZE, d->d_secsize, UINT32_MAX,
                         _("bytes/sector"), &res) == 0)
        d->d_secsize = res;
    ...
    if (fdisk_ask_number(cxt, 1, d->d_nsectors * d->d_ntracks,
                         d->d_nsectors * d->d_ntracks, _("sectors/cylinder"),
                         &res) == 0)
        d->d_secpercyl = res;
...
static int bsd_get_bootstrap(struct fdisk_context* cxt, char* path, void* ptr,
                             int size)
{
    int fd;
...
int fdisk_bsd_write_bootstrap(struct fdisk_context* cxt)
{
    ...
    snprintf(buf, sizeof(buf),
             _("Bootstrap: %1$sboot -> boot%1$s (default %1$s)"), name);
    ...
    rc = bsd_get_bootstrap(cxt, buf, &l->bsdbuffer[d->d_secsize],
...
static int bsd_readlabel(struct fdisk_context* cxt)
{
    ...
    memmove(
        d,
        &l->bsdbuffer[BSD_LABELSECTOR * DEFAULT_SECTOR_SIZE + BSD_LABELOFFSET],
        sizeof(*d));
...
static int bsd_write_disklabel(struct fdisk_context* cxt)
{
    ...
    if (cxt->parent && fdisk_label_is_changed(cxt->parent->label))
        fdisk_info(cxt,
                   _("Disklabel written to %s.  (Don't forget to write the %s "
                     "disklabel too.)"),
...
static int bsd_partition_is_used(struct fdisk_context* cxt, size_t partnum)
{
    struct bsd_disklabel* d = self_disklabel(cxt);
