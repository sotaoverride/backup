...
/* The first DS RR from https://data.iana.org/root-anchors/root-anchors.xml,
 * retrieved December 2015 */
static const uint8_t root_digest1[] = {
    0x49, 0xAA, 0xC1, 0x1D, 0x7B, 0x6F, 0x64, 0x46, 0x70, 0x2E, 0x54,
    0xA1, 0x60, 0x73, 0x71, 0x60, 0x7A, 0x1A, 0x41, 0x85, 0x52, 0x00,
    0xFD, 0x2C, 0xE1, 0xCD, 0xDE, 0x32, 0xF2, 0x4E, 0x8F, 0xB5};

/* The second DS RR from https://data.iana.org/root-anchors/root-anchors.xml,
 * retrieved February 2017 */
static const uint8_t root_digest2[] = {
    0xE0, 0x6D, 0x44, 0xB8, 0x0B, 0x8F, 0x1D, 0x39, 0xA9, 0x5C, 0x0B,
    0x0D, 0x7C, 0x65, 0xD0, 0x84, 0x58, 0xE8, 0x80, 0x40, 0x9B, 0xBC,
    0x68, 0x34, 0x57, 0x10, 0x42, 0x37, 0xC7, 0xF8, 0xEC, 0x8D};

static bool dns_trust_anchor_knows_domain_positive(DnsTrustAnchor* d,
                                                   const char* name)
{
    assert(d);
    ...
    return hashmap_contains(
               d->positive_by_key,
               &DNS_RESOURCE_KEY_CONST(DNS_CLASS_IN, DNS_TYPE_DNSKEY, name)) ||
           hashmap_contains(
               d->positive_by_key,
               &DNS_RESOURCE_KEY_CONST(DNS_CLASS_IN, DNS_TYPE_DS, name));
}

static int add_root_ksk(DnsAnswer* answer, DnsResourceKey* key,
                        uint16_t key_tag, uint8_t algorithm,
                        uint8_t digest_type, const void* digest,
                        size_t digest_size)
{
...
static int dns_trust_anchor_add_builtin_positive(DnsTrustAnchor* d)
{
    ...
    r = hashmap_ensure_allocated(&d->positive_by_key,
                                 &dns_resource_key_hash_ops);
    if (r < 0)
        return r;

    /* Only add the built-in trust anchor if there's neither a DS nor a DNSKEY
     * defined for the root domain. That
     ...
     */
    r = add_root_ksk(answer, key, 19036, DNSSEC_ALGORITHM_RSASHA256,
                     DNSSEC_DIGEST_SHA256, root_digest1, sizeof(root_digest1));
    if (r < 0)
        return r;

    r = add_root_ksk(answer, key, 20326, DNSSEC_ALGORITHM_RSASHA256,
                     DNSSEC_DIGEST_SHA256, root_digest2, sizeof(root_digest2));
    if (r < 0)
        return r;
...
static int dns_trust_anchor_load_positive(DnsTrustAnchor* d, const char* path,
                                          unsigned line, const char* s)
{
    _cleanup_(dns_resource_record_unrefp) DnsResourceRecord* rr = NULL;
    ...
    if (r < 0)
        return log_warning_errno(r, "Unable to parse domain in line %s:%u: %m",
                                 path, line);

    r = dns_name_is_valid(domain);
    if (r < 0)
        return log_warning_errno(r,
                                 "Failed to chack validity of domain name "
                                 "'%s', at line %s:%u, ignoring line: %m",
                                 domain, path, line);
    if (r == 0)
    {
        log_warning("Domain name %s is invalid, at line %s:%u, ignoring line.",
                    domain, path, line);
        return -EINVAL;
    }
    ...
    if (r < 0)
        return log_warning_errno(
            r, "Unable to parse class and type in line %s:%u: %m", path, line);
    if (r != 2)
    {
    ...
    {
        log_warning("RR class %s is not supported, ignoring line %s:%u.", class,
                    path, line);
        return -EINVAL;
    }
    ...
    {
        _cleanup_free_ char *key_tag = NULL, *algorithm = NULL,
                            *digest_type = NULL;
        ...
        {
            log_warning_errno(r,
                              "Failed to parse DS parameters on line %s:%u: %m",
                              path, line);
            return -EINVAL;
        }
        if (r != 3)
        ...
        if (r < 0)
            return log_warning_errno(
                r, "Failed to parse DS key tag %s on line %s:%u: %m", key_tag,
                path, line);

        a = dnssec_algorithm_from_string(algorithm);
        if (a < 0)
        {
            log_warning("Failed to parse DS algorithm %s on line %s:%u",
                        algorithm, path, line);
            return -EINVAL;
        }
        ...
        if (dt < 0)
        {
            log_warning("Failed to parse DS digest type %s on line %s:%u",
                        digest_type, path, line);
            return -EINVAL;
        }
        ...
        if (r < 0)
        {
            log_warning("Failed to parse DS digest %s on line %s:%u", p, path,
                        line);
            return -EINVAL;
        }
    ...
    else if (strcaseeq(type, "DNSKEY"))
    {
        ...
        if (r < 0)
            return log_warning_errno(
                r, "Failed to parse DNSKEY parameters on line %s:%u: %m", path,
                line);
        if (r != 3)
        {
        ...
        if (r < 0)
            return log_warning_errno(
                r, "Failed to parse DNSKEY flags field %s on line %s:%u", flags,
                path, line);
        if ((f & DNSKEY_FLAG_ZONE_KEY) == 0)
        {
            log_warning("DNSKEY lacks zone key bit set on line %s:%u", path,
                        line);
            return -EINVAL;
        }
        ...
        if (a < 0)
        {
            log_warning("Failed to parse DNSKEY algorithm %s on line %s:%u",
                        algorithm, path, line);
            return -EINVAL;
        }
        ...
        if (r < 0)
            return log_warning_errno(
                r, "Failed to parse DNSKEY key data %s on line %s:%u", p, path,
                line);

        rr =
            dns_resource_record_new_full(DNS_CLASS_IN, DNS_TYPE_DNSKEY, domain);
        if (!rr)
            return log_oom();
    ...
    {
        log_warning("RR type %s is not supported, ignoring line %s:%u.", type,
                    path, line);
        return -EINVAL;
    }
...
static int dns_trust_anchor_load_negative(DnsTrustAnchor* d, const char* path,
                                          unsigned line, const char* s)
{
    ...
    r = extract_first_word(&p, &domain, NULL, EXTRACT_QUOTES);
    if (r < 0)
        return log_warning_errno(r, "Unable to parse line %s:%u: %m", path,
                                 line);
    ...
    if (!isempty(p))
    {
        log_warning("Trailing garbage at line %s:%u, ignoring line.", path,
                    line);
        return -EINVAL;
    }
...
static int dns_trust_anchor_load_files(DnsTrustAnchor* d, const char* suffix,
                                       int (*loader)(DnsTrustAnchor* d,
                                                     const char* path,
                                                     unsigned n,
                                                     const char* line))
{
    ...
    if (r < 0)
        return log_error_errno(
            r, "Failed to enumerate %s trust anchor files: %m", suffix);
...
int dns_trust_anchor_load(DnsTrustAnchor* d)
{
    ...
    (void)dns_trust_anchor_load_files(d, ".positive",
                                      dns_trust_anchor_load_positive);
    (void)dns_trust_anchor_load_files(d, ".negative",
                                      dns_trust_anchor_load_negative);

    /* However, if the built-in DS fails, then we have a problem. */
    r = dns_trust_anchor_add_builtin_positive(d);
    if (r < 0)
        return log_error_errno(
            r, "Failed to add built-in positive trust anchor: %m");

    r = dns_trust_anchor_add_builtin_negative(d);
    if (r < 0)
        return log_error_errno(
            r, "Failed to add built-in negative trust anchor: %m");
...
void dns_trust_anchor_flush(DnsTrustAnchor* d)
{
    ...
    d->positive_by_key =
        hashmap_free_with_destructor(d->positive_by_key, dns_answer_unref);
    d->revoked_by_rr =
        set_free_with_destructor(d->revoked_by_rr, dns_resource_record_unref);
    d->negative_by_name = set_free_free(d->negative_by_name);
}

int dns_trust_anchor_lookup_positive(DnsTrustAnchor* d,
                                     const DnsResourceKey* key, DnsAnswer** ret)
{
    DnsAnswer* a;
...
int dns_trust_anchor_lookup_negative(DnsTrustAnchor* d, const char* name)
{
    ...
    for (;;)
    {
        ...
        /* If the domain isn't listed as NTA, but is listed as positive trust
         * anchor, then that counts. See RFC 7646, section 1.1 */
        if (hashmap_contains(
                d->positive_by_key,
                &DNS_RESOURCE_KEY_CONST(DNS_CLASS_IN, DNS_TYPE_DS, name)))
            return false;
        ...
        if (hashmap_contains(
                d->positive_by_key,
                &DNS_RESOURCE_KEY_CONST(DNS_CLASS_IN, DNS_TYPE_KEY, name)))
            return false;
...
static int dns_trust_anchor_revoked_put(DnsTrustAnchor* d,
                                        DnsResourceRecord* rr)
{
    int r;
...
static int dns_trust_anchor_remove_revoked(DnsTrustAnchor* d,
                                           DnsResourceRecord* rr)
{
    ...
    log_struct(
        LOG_WARNING, "MESSAGE_ID=" SD_MESSAGE_DNSSEC_TRUST_ANCHOR_REVOKED_STR,
        LOG_MESSAGE(
            "DNSSEC trust anchor %s has been revoked.\n"
    ...
    r = hashmap_replace(d->positive_by_key, new_answer->items[0].rr->key,
                        new_answer);
    if (r < 0)
        return r;
...
static int dns_trust_anchor_check_revoked_one(DnsTrustAnchor* d,
                                              DnsResourceRecord* revoked_dnskey)
{
    DnsAnswer* a;
    int r;
    ...
    if (a)
    {
        ...
        DNS_ANSWER_FOREACH(anchor, a)
        {
            ...
            if (((anchor->dnskey.flags ^ revoked_dnskey->dnskey.flags) |
                 DNSKEY_FLAG_REVOKE) != DNSKEY_FLAG_REVOKE)
                continue;

            if (memcmp(anchor->dnskey.key, revoked_dnskey->dnskey.key,
                       anchor->dnskey.key_size) != 0)
                continue;
    ...
    a = hashmap_get(
        d->positive_by_key,
        &DNS_RESOURCE_KEY_CONST(revoked_dnskey->key->class, DNS_TYPE_DS,
                                dns_resource_key_name(revoked_dnskey->key)));
    if (a)
    {
        DnsResourceRecord* anchor;

        /* Second, look for DS RRs matching this DNSKEY in our trust anchor
         * database */
...
int dns_trust_anchor_check_revoked(DnsTrustAnchor* d, DnsResourceRecord* dnskey,
                                   DnsAnswer* rrs)
{
    ...
    if (!dns_trust_anchor_knows_domain_positive(
            d, dns_resource_key_name(dnskey->key)))
        return 0;
    ...
    DNS_ANSWER_FOREACH(rrsig, rrs)
    {
        ...
        r = dnssec_verify_rrset(rrs, dnskey->key, rrsig, dnskey, USEC_INFINITY,
                                &result);
        if (r < 0)
            return r;
