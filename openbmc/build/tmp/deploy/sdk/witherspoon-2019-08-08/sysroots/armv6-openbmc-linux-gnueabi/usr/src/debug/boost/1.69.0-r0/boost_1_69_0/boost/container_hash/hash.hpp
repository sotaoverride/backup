...
#if BOOST_WORKAROUND(__GNUC__, < 3) && !defined(__SGI_STL_PORT) &&             \
...
template <>
struct ulong_numbers2<boost::uint128_type>
    : boost::hash_detail::enable_hash_value
{
};
#endif
...
template <typename T>
typename boost::enable_if<boost::is_enum<T>, std::size_t>::type hash_value(T);
...
inline std::size_t hash_value_signed(T val)
{
    ...
    const int length =
        (std::numeric_limits<T>::digits - 1) / static_cast<int>(size_t_bits);
...
#endif
{
    ...
    std::size_t x =
        static_cast<std::size_t>(reinterpret_cast<long long int>(v));
#else
    std::size_t x =
...
#define BOOST_HASH_SPECIALIZE(type)                                            \
    template <>                                                                \
    struct hash<type> : public boost::hash_detail::hash_base<type>             \
    {                                                                          \
...
// Specializing boost::hash for pointers.
        ...
        template <class T>
        struct hash<T*> : public boost::hash_detail::hash_base<T*>
{
...
{
    template <class T>
    struct inner : public boost::hash_detail::hash_base<T>
    {
...
#else
            std::size_t x =
                static_cast<std::size_t>(reinterpret_cast<std::ptrdiff_t>(val));
