...
#define INPUT_JSON 2
    uchar* outputFileName; /* name of file for std[out/err] or NULL if to
                              discard */
...
BEGINinitConfVars /* (re)set config variables to default values */
    CODESTARTinitConfVars cs.szBinary = NULL; /* name of binary to call */
ENDinitConfVars

    /* config settings */

    BEGINcreateInstance CODESTARTcreateInstance pData->inputProp = INPUT_MSG;
pthread_mutex_init(&pData->mut, NULL);
ENDcreateInstance

    BEGINcreateWrkrInstance CODESTARTcreateWrkrInstance pWrkrData->fdPipeIn =
    -1;
...
ENDisCompatibleWithFeature
    ...
    BEGINfreeInstance int i;
CODESTARTfreeInstance pthread_mutex_destroy(&pData->mut);
free(pData->szBinary);
...
ENDfreeInstance
    ...
    BEGINfreeWrkrInstance CODESTARTfreeWrkrInstance free(pWrkrData->respBuf);
ENDfreeWrkrInstance

    BEGINdbgPrintInstInfo CODESTARTdbgPrintInstInfo ENDdbgPrintInstInfo
    ...
    writeOutputDebug(wrkrInstanceData_t* __restrict__ const pWrkrData,
                     const char* __restrict__ const buf, const ssize_t lenBuf)
{
    char errStr[1024];
    ssize_t r;
    ...
    if (r != lenBuf)
    {
        DBGPRINTF("mmexternal: problem writing output file %s: bytes "
                  ...
                  pWrkrData->pData->outputFileName, (long long)lenBuf,
                  (long long)r, rs_strerror_r(errno, errStr, sizeof(errStr)));
    }
done:
    return;
}
 ...
 */
static void
    processProgramReply(wrkrInstanceData_t* __restrict__ const pWrkrData,
                        smsg_t* const pMsg)
{
    rsRetVal iRet;
    char errStr[1024];
    ...
    do
    {
        if (pWrkrData->maxLenRespBuf < numCharsRead + 256)
        { /* 256 to permit at least a decent read */
            pWrkrData->maxLenRespBuf += 4096;
            if ((newptr = realloc(pWrkrData->respBuf,
                                  pWrkrData->maxLenRespBuf)) == NULL)
            {
        ...
        {
            numCharsRead += r;
            pWrkrData->respBuf[numCharsRead] =
                '\0'; /* space reserved in read! */
        }
        else
        {
            /* emergency - fake no update */
...
execBinary(wrkrInstanceData_t* pWrkrData, const int fdStdin,
           const int fdStdOutErr)
{
    ...
    /* we need to block SIGINT, otherwise the external program is cancelled when
     * we are stopped in debug mode.
     */
    ...
    /* finally exec child */
    execve((char*)pWrkrData->pData->szBinary, pWrkrData->pData->aParams,
           newenviron);
...
static rsRetVal cleanup(wrkrInstanceData_t* pWrkrData)
{
    ...
    {
        glblReportChildProcessExit(pWrkrData->pData->szBinary, pWrkrData->pid,
                                   status);
    }
 ...
 * own action queue.
 */
static rsRetVal callExtProg(wrkrInstanceData_t* __restrict__ const pWrkrData,
                            smsg_t* __restrict__ const pMsg)
{
    ...
    struct iovec iov[2];
    int bFreeInputstr =
        1; /* we must only free if it does not point to msg-obj mem! */
    const uchar* inputstr =
        NULL; /* string to be processed by external program */
    DEFiRet;
            ...
            switch (errno)
            {
                case EPIPE:
                    LogMsg(0, RS_RET_ERR_WRITE_PIPE, LOG_WARNING,
                           "mmexternal: program '%s' (pid %d) terminated; will "
                           "be restarted",
                    ...
                    break;
                default:
                    LogError(errno, RS_RET_ERR_WRITE_PIPE,
                             "mmexternal: error sending message to program");
...
BEGINdoAction_NoStrings smsg_t** ppMsg = (smsg_t**)pMsgData;
smsg_t* pMsg = ppMsg[0];
instanceData* pData;
...
CODE_STD_STRING_REQUESTnewActInst(1) for (i = 0; i < actpblk.nParams; ++i)
{
    ...
    {
        CHKiRet(split_binary_parameters(&pData->szBinary, &pData->aParams,
                                        &pData->iParams, pvals[i].val.d.estr));
    }
...
DBGPRINTF("mmexternal: interface.input '%s', mode %d\n", cstr,
          pData->inputProp);
CODE_STD_FINALIZERnewActInst free((void*)cstr);
cnfparamvalsDestruct(pvals, &actpblk);
ENDnewActInst

    NO_LEGACY_CONF_parseSelectorAct

        BEGINmodExit CODESTARTmodExit free(cs.szBinary);
cs.szBinary = NULL;
ENDmodExit

    BEGINqueryEtryPt CODESTARTqueryEtryPt CODEqueryEtryPt_STD_OMOD_QUERIES
        CODEqueryEtryPt_STD_OMOD8_QUERIES
            CODEqueryEtryPt_STD_CONF2_CNFNAME_QUERIES
                CODEqueryEtryPt_STD_CONF2_OMOD_QUERIES ENDqueryEtryPt

    BEGINmodInit() CODESTARTmodInit INITLegCnfVars* ipIFVersProvided =
        CURR_MOD_IF_VERSION; /* we only support the current interface
                                specification */
CODEmodInit_QueryRegCFSLineHdlr CODEmodInit_QueryRegCFSLineHdlr ENDmodInit
