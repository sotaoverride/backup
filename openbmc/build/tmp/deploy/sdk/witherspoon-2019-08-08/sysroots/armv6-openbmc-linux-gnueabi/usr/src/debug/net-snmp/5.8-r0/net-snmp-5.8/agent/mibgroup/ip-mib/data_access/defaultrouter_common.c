...
static int _access_defaultrouter_entry_compare_addr(const void* lhs,
                                                    const void* rhs);
static void
    _access_defaultrouter_entry_release(netsnmp_defaultrouter_entry* entry,
                                        void* unused);
...
/**
 */
netsnmp_container* netsnmp_access_defaultrouter_container_init(u_int flags)
{
    ...
    if (flags & NETSNMP_ACCESS_DEFAULTROUTER_INIT_ADDL_IDX_BY_ADDR)
    {
        netsnmp_container* container2 = netsnmp_container_find(
            "defaultrouter_addr:access_defaultrouter:table_container");
        if (NULL == container2)
        {
                                                ...
                                                u_int load_flags)
{
    ...
    if (0 != rc)
    {
        netsnmp_access_defaultrouter_container_free(
            container, NETSNMP_ACCESS_DEFAULTROUTER_FREE_NOFLAGS);
        container = NULL;
    }
...
void netsnmp_access_defaultrouter_container_free(netsnmp_container* container,
                                                 u_int free_flags)
{
...
/**
 */
netsnmp_defaultrouter_entry* netsnmp_access_defaultrouter_entry_create(void)
{
    int rc = 0;
    ...
    {
        DEBUGMSGT(
            ("access:defaultrouter:create", "error %d in arch init\n", rc));
...
/**
 */
void _access_defaultrouter_entry_release(netsnmp_defaultrouter_entry* entry,
                                         void* context)
{
    netsnmp_access_defaultrouter_entry_free(entry);
}
...
static int _access_defaultrouter_entry_compare_addr(const void* lhs,
                                                    const void* rhs)
{
    const netsnmp_defaultrouter_entry* lh =
        (const netsnmp_defaultrouter_entry*)lhs;
    const netsnmp_defaultrouter_entry* rh =
        (const netsnmp_defaultrouter_entry*)rhs;
