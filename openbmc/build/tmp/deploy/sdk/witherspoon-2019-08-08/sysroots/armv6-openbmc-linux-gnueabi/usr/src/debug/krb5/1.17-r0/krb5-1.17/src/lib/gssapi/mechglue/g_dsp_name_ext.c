...
static OM_uint32 val_dsp_name_ext_args(OM_uint32* minor_status,
                                             ...
                                             gss_buffer_t output_name_buffer)
{
    ...
    status = val_dsp_name_ext_args(minor_status, input_name,
                                   display_as_name_type, output_name_buffer);
    ...
    if (union_name->mech_type)
    {
        ...
        {
            status = (*mech->gss_display_name_ext)(
                minor_status, union_name->mech_name, display_as_name_type,
                output_name_buffer);
            if (status != GSS_S_COMPLETE)
                map_error(minor_status, mech);
        }
    ...
    (void)memcpy(output_name_buffer->value, union_name->external_name->value,
