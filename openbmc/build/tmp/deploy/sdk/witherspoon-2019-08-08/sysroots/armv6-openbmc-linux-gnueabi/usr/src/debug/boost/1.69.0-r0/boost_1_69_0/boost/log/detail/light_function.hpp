 ...
 * \date   20.06.2010
 *
 * \brief  This header is the Boost.Log library impl, see the library
 * documentation at
 * http://www.boost.org/doc/libs/release/libs/log/doc/html/index.html.
 *
 * The file contains a lightweight alternative of Boost.Function. It does not
 * provide all features of Boost.Function but doesn't introduce dependency on
 * Boost.Bind.
 */
...
template <typename T>
struct is_cv_same<T, volatile T>
{
    enum _
    {
        value = true
...
template <typename T>
struct is_cv_same<T, const volatile T>
{
    enum _
    {
        value = true
...
template <typename T, typename ThisT>
struct is_rv_or_same<boost::rv<T>, ThisT>
{
    enum _
    {
        value = true
...
class light_function<ResultT(ArgsT...)>
{
  ...
  private:
    struct impl_base
    {
        ...
        impl_base(invoke_type inv, clone_type cl, destroy_type dstr) :
            invoke(inv), clone(cl), destroy(dstr)
        {
        }
    ...
    class impl : public impl_base
    {
      ...
      public:
        explicit impl(FunT const& fun) :
            impl_base(&this_type::invoke_impl, &this_type::clone_impl,
                      &this_type::destroy_impl),
            m_Function(fun)
        {
        }
        ...
        {
            return new impl(
                static_cast<const impl*>(static_cast<const impl_base*>(self))
                    ->m_Function);
        }
        static result_type invoke_impl(void* self, ArgsT... args)
        {
            return static_cast<impl*>(static_cast<impl_base*>(self))
                ->m_Function(args...);
        }
  ...
  public:
    ...
    light_function(FunT&& fun) :
        m_pImpl(new impl<typename remove_cv<
                    typename remove_reference<FunT>::type>::type>(
            boost::forward<FunT>(fun)))
    {
    }
#else
    template <typename FunT>
    light_function(
        FunT const& fun,
        typename boost::disable_if_c<is_rv_or_same<FunT, this_type>::value,
    ...
    template <typename FunT>
    light_function(
        BOOST_RV_REF(FunT) fun,
        typename boost::disable_if_c<is_cv_same<FunT, this_type>::value,
                                     boost::log::aux::sfinae_dummy>::type =
            boost::log::aux::sfinae_dummy()) :
  ...
  private:
    ...
    class impl : public impl_base
    {
      ...
      public:
        ...
        static result_type invoke_impl(void* self, ArgsT... args)
        {
            static_cast<impl*>(static_cast<impl_base*>(self))
                ->m_Function(args...);
        }
...
template <typename SignatureT>
inline void swap(light_function<SignatureT>& left,
                 light_function<SignatureT>& right)
{
    left.swap(right);
}
