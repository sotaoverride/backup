...
static gcry_err_code_t do_setkey(RIJNDAEL_context* ctx, const byte* key,
...
static gcry_err_code_t rijndael_setkey(void* context, const byte* key,
                                       const unsigned keylen)
{
...
static unsigned int do_encrypt_fn(const RIJNDAEL_context* ctx, unsigned char* b,
                               ...
                               const unsigned char* ax)
{
    ...
    asm volatile("movq %[encT], %%r8\n\t"
                 "callq *%[ret]\n\t"
                 :
                 [ ret ] "=a"(ret), "+D"(key), "+S"(bx), "+d"(ax), "+c"(rounds)
                 : "0"(_gcry_aes_amd64_encrypt_block), [ encT ] "r"(encT)
...
static unsigned int rijndael_encrypt(void* context, byte* b, const byte* a)
{
    RIJNDAEL_context* ctx = context;
...
/* Bulk encryption of complete blocks in CFB mode.  Caller needs to
   ...
   cipher.c. */
void _gcry_aes_cfb_enc(void* context, unsigned char* iv, void* outbuf_arg,
                       const void* inbuf_arg, size_t nblocks)
{
    RIJNDAEL_context* ctx = context;
...
/* Bulk encryption of complete blocks in CBC mode.  Caller needs to
   ...
   function is only intended for the bulk encryption feature of
   cipher.c. */
void _gcry_aes_cbc_enc(void* context, unsigned char* iv, void* outbuf_arg,
...
/* Bulk encryption of complete blocks in CTR mode.  Caller needs to
   ...
   of size BLOCKSIZE. */
void _gcry_aes_ctr_enc(void* context, unsigned char* ctr, void* outbuf_arg,
                       const void* inbuf_arg, size_t nblocks)
{
    RIJNDAEL_context* ctx = context;
    unsigned char* outbuf = outbuf_arg;
    ...
    else
    {
        union
        {
            unsigned char x1[16] ATTR_ALIGNED_16;
            u32 x32[4];
        } tmp;
        rijndael_cryptfn_t encrypt_fn = ctx->encrypt_fn;
...
static unsigned int do_decrypt_fn(const RIJNDAEL_context* ctx, unsigned char* b,
                                  const unsigned char* a)
{
                               ...
                               const unsigned char* ax)
{
    ...
    asm volatile(
        ...
        "callq *%[ret]\n\t"
        : [ ret ] "=a"(ret), "+D"(key), "+S"(bx), "+d"(ax), "+c"(rounds)
        : "0"(_gcry_aes_amd64_decrypt_block), [ dectabs ] "r"(&dec_tables)
        : "cc", "memory", "r8", "r9", "r10", "r11");
    return ret;
#endif /* HAVE_COMPATIBLE_GCC_AMD64_PLATFORM_AS */
#elif defined(USE_ARM_ASM)
...
static inline void check_decryption_preparation(RIJNDAEL_context* ctx)
{
...
static unsigned int rijndael_decrypt(void* context, byte* b, const byte* a)
{
    RIJNDAEL_context* ctx = context;
...
/* Bulk decryption of complete blocks in CFB mode.  Caller needs to
   ...
   function is only intended for the bulk encryption feature of
   cipher.c. */
void _gcry_aes_cfb_dec(void* context, unsigned char* iv, void* outbuf_arg,
                       const void* inbuf_arg, size_t nblocks)
{
    RIJNDAEL_context* ctx = context;
    unsigned char* outbuf = outbuf_arg;
...
/* Bulk decryption of complete blocks in CBC mode.  Caller needs to
   ...
   function is only intended for the bulk encryption feature of
   cipher.c. */
void _gcry_aes_cbc_dec(void* context, unsigned char* iv, void* outbuf_arg,
                       const void* inbuf_arg, size_t nblocks)
{
    RIJNDAEL_context* ctx = context;
    unsigned char* outbuf = outbuf_arg;
...
static const char* selftest_basic_128(void)
{
...
static const char* selftest_basic_192(void)
{
    RIJNDAEL_context* ctx;
    unsigned char* ctxmem;
    unsigned char scratch[16];
    ...
    static unsigned char key_192[24] = {
        0x04, 0x05, 0x06, 0x07, 0x09, 0x0A, 0x0B, 0x0C, 0x0E, 0x0F, 0x10, 0x11,
        0x13, 0x14, 0x15, 0x16, 0x18, 0x19, 0x1A, 0x1B, 0x1D, 0x1E, 0x1F, 0x20};
...
static const char* selftest_basic_256(void)
{
    RIJNDAEL_context* ctx;
    unsigned char* ctxmem;
    unsigned char scratch[16];
    ...
    static unsigned char key_256[32] = {
        0x08, 0x09, 0x0A, 0x0B, 0x0D, 0x0E, 0x0F, 0x10, 0x12, 0x13, 0x14,
        0x15, 0x17, 0x18, 0x19, 0x1A, 0x1C, 0x1D, 0x1E, 0x1F, 0x21, 0x22,
        0x23, 0x24, 0x26, 0x27, 0x28, 0x29, 0x2B, 0x2C, 0x2D, 0x2E};
...
static const char* selftest_ctr_128(void)
{
    const int nblocks = 8 + 1;
    const int blocksize = BLOCKSIZE;
    const int context_size = sizeof(RIJNDAEL_context);

    return _gcry_selftest_helper_ctr("AES", &rijndael_setkey, &rijndael_encrypt,
                                     &_gcry_aes_ctr_enc, nblocks, blocksize,
                                     context_size);
}

/* Run the self-tests for AES-CBC-128, tests bulk CBC decryption.
   Returns NULL on success. */
static const char* selftest_cbc_128(void)
{
    const int nblocks = 8 + 2;
    const int blocksize = BLOCKSIZE;
    const int context_size = sizeof(RIJNDAEL_context);

    return _gcry_selftest_helper_cbc("AES", &rijndael_setkey, &rijndael_encrypt,
                                     &_gcry_aes_cbc_dec, nblocks, blocksize,
                                     context_size);
}

/* Run the self-tests for AES-CFB-128, tests bulk CFB decryption.
   Returns NULL on success. */
static const char* selftest_cfb_128(void)
{
    const int nblocks = 8 + 2;
    const int blocksize = BLOCKSIZE;
    const int context_size = sizeof(RIJNDAEL_context);

    return _gcry_selftest_helper_cfb("AES", &rijndael_setkey, &rijndael_encrypt,
                                     &_gcry_aes_cfb_dec, nblocks, blocksize,
                                     context_size);
}
...
static const char* selftest(void)
{
    ...
    if ((r = selftest_basic_128()) || (r = selftest_basic_192()) ||
...
static const char* selftest_fips_128_38a(int requested_mode)
{
            ...
            const unsigned char output[16];
    ...
    } tv[2] = {{GCRY_CIPHER_MODE_CFB, /* F.3.13, CFB128-AES128 */
                ...
                {{{0x6b, 0xc1, 0xbe, 0xe2, 0x2e, 0x40, 0x9f, 0x96, 0xe9, 0x3d,
                  ...
                  {0x3b, 0x3f, 0xd9, 0x2e, 0xb7, 0x2d, 0xad, 0x20, 0x33, 0x34,
                   0x49, 0xf8, 0xe8, 0x3c, 0xfb, 0x4a}},

                 {{0xae, 0x2d, 0x8a, 0x57, 0x1e, 0x03, 0xac, 0x9c, 0x9e, 0xb7,
                   0x6f, 0xac, 0x45, 0xaf, 0x8e, 0x51},
                  {0xc8, 0xa6, 0x45, 0x37, 0xa0, 0xb3, 0xa9, 0x3f, 0xcd, 0xe3,
                   0xcd, 0xad, 0x9f, 0x1c, 0xe5, 0x8b}},

                 {{0x30, 0xc8, 0x1c, 0x46, 0xa3, 0x5c, 0xe4, 0x11, 0xe5, 0xfb,
                   0xc1, 0x19, 0x1a, 0x0a, 0x52, 0xef},
                  {0x26, 0x75, 0x1f, 0x67, 0xa3, 0xcb, 0xb1, 0x40, 0xb1, 0x80,
                   0x8c, 0xf1, 0x87, 0xa4, 0xf4, 0xdf}},

                 {{0xf6, 0x9f, 0x24, 0x45, 0xdf, 0x4f, 0x9b, 0x17, 0xad, 0x2b,
                   0x41, 0x7b, 0xe6, 0x6c, 0x37, 0x10},
                  {0xc0, 0x4b, 0x05, 0x35, 0x7c, 0x5d, 0x1c, 0x0e, 0xea, 0xc4,
                   0xc6, 0x6f, 0x9f, 0xf7, 0xf2, 0xe6}}}},
               {GCRY_CIPHER_MODE_OFB,
                 ...
                 0x0a, 0x0b, 0x0c, 0x0d, 0x0e, 0x0f},
                {
                    {{0x6b, 0xc1, 0xbe, 0xe2, 0x2e, 0x40, 0x9f, 0x96, 0xe9,
                     ...
                     {0x3b, 0x3f, 0xd9, 0x2e, 0xb7, 0x2d, 0xad, 0x20, 0x33,
                      0x34, 0x49, 0xf8, 0xe8, 0x3c, 0xfb, 0x4a}},

                    {{0xae, 0x2d, 0x8a, 0x57, 0x1e, 0x03, 0xac, 0x9c, 0x9e,
                      0xb7, 0x6f, 0xac, 0x45, 0xaf, 0x8e, 0x51},
                     {0x77, 0x89, 0x50, 0x8d, 0x16, 0x91, 0x8f, 0x03, 0xf5,
                      0x3c, 0x52, 0xda, 0xc5, 0x4e, 0xd8, 0x25}},

                    {{0x30, 0xc8, 0x1c, 0x46, 0xa3, 0x5c, 0xe4, 0x11, 0xe5,
                      0xfb, 0xc1, 0x19, 0x1a, 0x0a, 0x52, 0xef},
                     {0x97, 0x40, 0x05, 0x1e, 0x9c, 0x5f, 0xec, 0xf6, 0x43,
                      0x44, 0xf7, 0xa8, 0x22, 0x60, 0xed, 0xcc}},

                    {{0xf6, 0x9f, 0x24, 0x45, 0xdf, 0x4f, 0x9b, 0x17, 0xad,
                      0x2b, 0x41, 0x7b, 0xe6, 0x6c, 0x37, 0x10},
                     {0x30, 0x4c, 0x65, 0x28, 0xf6, 0x59, 0xc7, 0x78, 0x66,
                      0xa5, 0x10, 0xd9, 0xc1, 0xd6, 0xae, 0x5e}},
                }}};
    unsigned char scratch[16];
    gpg_error_t err;
    int tvi, idx;
...
static gpg_err_code_t selftest_fips_128(int extended,
...
static gpg_err_code_t selftest_fips_192(int extended,
                                        selftest_report_func_t report)
{
    const char* what;
    const char* errtxt;
...
static gpg_err_code_t selftest_fips_256(int extended,
                                        selftest_report_func_t report)
{
    const char* what;
    const char* errtxt;
...
static const char* rijndael_names[] = {"RIJNDAEL", "AES128", "AES-128", NULL};
...
gcry_cipher_spec_t _gcry_cipher_spec_aes = {GCRY_CIPHER_AES,
                                            {0, 1},
                                            "AES",
                                            rijndael_names,
                                            rijndael_oids,
...
static const char* rijndael192_names[] = {"RIJNDAEL192", "AES-192", NULL};
...
gcry_cipher_spec_t _gcry_cipher_spec_aes192 = {GCRY_CIPHER_AES192,
                                               {0, 1},
                                               "AES192",
                                               rijndael192_names,
                                               rijndael192_oids,
...
static const char* rijndael256_names[] = {"RIJNDAEL256", "AES-256", NULL};
...
gcry_cipher_spec_t _gcry_cipher_spec_aes256 = {GCRY_CIPHER_AES256,
                                               {0, 1},
                                               "AES256",
                                               rijndael256_names,
                                               rijndael256_oids,
