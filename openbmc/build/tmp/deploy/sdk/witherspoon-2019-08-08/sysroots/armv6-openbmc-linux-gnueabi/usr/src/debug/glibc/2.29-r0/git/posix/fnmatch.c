...
static wchar_t* __wcschrnul(const wchar_t* s, wint_t c)
{
    ...
    if (__builtin_expect(MB_CUR_MAX, 1) != 1)
    {
        ...
        if (__glibc_likely(n < 1024))
        {
            ...
            if (__glibc_unlikely(n == (size_t)-1))
                /* Something wrong.
                   XXX Do we have to set `errno' to something which mbsrtows
                   hasn't already done?  */
                return -1;
            if (p)
            {
        ...
        int res =
            internal_fnwmatch(wpattern, wstring, wstring + n,
                              flags & FNM_PERIOD, flags, NULL, alloca_used);
