...
class collate_impl : public collator<CharType>
{
  public:
...
#if U_ICU_VERSION_MAJOR_NUM * 100 + U_ICU_VERSION_MINOR_NUM >= 402
    int do_utf8_compare(level_type level, char const* b1, char const* e1,
    ...
    int do_ustring_compare(level_type level, CharType const* b1,
                           CharType const* e1, CharType const* b2,
                           CharType const* e2, UErrorCode& status) const
    {
    ...
    int do_real_compare(level_type level, CharType const* b1,
                        CharType const* e1, CharType const* b2,
                        CharType const* e2, UErrorCode& status) const
    {
    ...
    virtual int do_compare(level_type level, CharType const* b1,
                           CharType const* e1, CharType const* b2,
                           CharType const* e2) const
    {
        ...
        if (U_FAILURE(status))
            throw std::runtime_error(std::string("Collation failed:") +
                                     u_errorName(status));
        if (res < 0)
            return -1;
        else if (res > 0)
    ...
    std::vector<uint8_t> do_basic_transform(level_type level, CharType const* b,
                                            CharType const* e) const
    {
    ...
    std::basic_string<CharType> do_transform(level_type level,
                                             CharType const* b,
                                             CharType const* e) const
    {
    ...
    long do_hash(level_type level, CharType const* b, CharType const* e) const
    {
        std::vector<uint8_t> tmp = do_basic_transform(level, b, e);
        tmp.push_back(0);
        return gnu_gettext::pj_winberger_hash_function(
            reinterpret_cast<char*>(&tmp.front()));
    }

    collate_impl(cdata const& d) :
        cvt_(d.encoding), locale_(d.locale), is_utf8_(d.utf8)
    {
    }
    icu::Collator* get_collator(level_type ilevel) const
    {
        ...
        static const icu::Collator::ECollationStrength levels[level_count] = {
            ...
            icu::Collator::TERTIARY, icu::Collator::QUATERNARY,
        ...
        if (U_FAILURE(status))
            throw std::runtime_error(
                std::string("Creation of collate failed:") +
                u_errorName(status));
...
#if U_ICU_VERSION_MAJOR_NUM * 100 + U_ICU_VERSION_MINOR_NUM >= 402
template <>
int collate_impl<char>::do_real_compare(level_type level, char const* b1,
                                        char const* e1, char const* b2,
                                        char const* e2,
                                        UErrorCode& status) const
{
    if (is_utf8_)
...
std::locale create_collate(std::locale const& in, cdata const& cd,
