...
static void forward_syslog_iovec(Server* s, const struct iovec* iovec,
                                 unsigned n_iovec, const struct ucred* ucred,
...
static void forward_syslog_raw(Server* s, int priority, const char* buffer,
                               size_t buffer_len, const struct ucred* ucred,
                               const struct timeval* tv)
{
    struct iovec iovec;
...
static int syslog_skip_timestamp(const char** buf)
{
    enum
    {
        ...
        LETTER, LETTER,          LETTER, SPACE, SPACE_OR_NUMBER, NUMBER,
...
void server_process_syslog_message(Server* s, const char* buf, size_t raw_len,
                                   ...
                                   size_t label_len)
{
    ...
    _cleanup_free_ char *identifier = NULL, *pid = NULL, *dummy = NULL,
                        *msg_msg = NULL, *msg_raw = NULL;
    ...
    /* We are creating a copy of the message because we want to forward the
       original message verbatim to the legacy syslog implementation */
    for (i = raw_len; i > 0; i--)
...
int server_open_syslog_socket(Server* s)
{
    ...
    if (r < 0)
        return log_error_errno(
            r, "Failed to add syslog server fd to event loop: %m");

    r = sd_event_source_set_priority(s->syslog_event_source,
                                     SD_EVENT_PRIORITY_NORMAL + 5);
    if (r < 0)
        return log_error_errno(
            r, "Failed to adjust syslog event source priority: %m");
...
void server_maybe_warn_forward_syslog_missed(Server* s)
{
    ...
    if (s->last_warn_forward_syslog_missed + WARN_FORWARD_SYSLOG_MISSED_USEC >
        n)
        return;

    server_driver_message(
        s, 0, "MESSAGE_ID=" SD_MESSAGE_FORWARD_SYSLOG_MISSED_STR,
