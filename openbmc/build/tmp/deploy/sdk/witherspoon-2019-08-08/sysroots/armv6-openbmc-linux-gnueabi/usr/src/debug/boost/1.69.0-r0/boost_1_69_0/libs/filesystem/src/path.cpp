...
//  Old standard library configurations, particularly MingGW, don't support wide
//  strings. Report this with an explicit error message.
...
void first_element(const string_type& src, size_type& element_pos,
...
#ifdef BOOST_WINDOWS_API
             || itr.m_element.m_pathname[itr.m_element.m_pathname.size() - 1] ==
                    colon
#endif
             ))
               ? itr.m_element
...
namespace detail
{
// C++14 provide a mismatch algorithm with four iterator arguments(), but
// earlier standard libraries didn't, so provide this needed functionality.
inline std::pair<path::iterator, path::iterator> mismatch(path::iterator it1,
                                                          path::iterator it1end,
                                                          path::iterator it2,
    ...
    for (iterator itr(start); itr != stop; ++itr)
    {
            ...
            (itr->native())[0] == dot && (itr->native())[1] == dot) // dot dot
        {
                                    ...
                                    )))
            {
                ...
                //    root_directory_start(temp.native(),
                //    temp.native().size()));
...
// pos is position of the separator
{
    BOOST_ASSERT_MSG(!str.empty() &&
                         fs::detail::is_directory_separator(str[pos]),
...
{
    // case: "//"
    if (end_pos == 2 && fs::detail::is_directory_separator(str[0]) &&
        fs::detail::is_directory_separator(str[1]))
        return 0;
...
// return npos if no root_directory found
{
...
#ifdef BOOST_WINDOWS_API
    // case "c:/"
    if (size > 2 && path[1] == colon &&
        fs::detail::is_directory_separator(path[2]))
        return 2;
#endif

    // case "//"
    if (size == 2 && fs::detail::is_directory_separator(path[0]) &&
        fs::detail::is_directory_separator(path[1]))
        return string_type::npos;

#ifdef BOOST_WINDOWS_API
    // case "\\?\"
    if (size > 4 && fs::detail::is_directory_separator(path[0]) &&
        fs::detail::is_directory_separator(path[1]) &&
        path[2] == questionmark && fs::detail::is_directory_separator(path[3]))
    {
    ...
    // case "//net {/}"
    if (size > 3 && fs::detail::is_directory_separator(path[0]) &&
        fs::detail::is_directory_separator(path[1]) &&
        !fs::detail::is_directory_separator(path[2]))
    {
...
void first_element(const string_type& src, size_type& element_pos,
                   size_type& element_size, size_type size)
{
    ...
    if (size >= 2 && fs::detail::is_directory_separator(src[0]) &&
        ...
        (size == 2 || !fs::detail::is_directory_separator(src[2])))
    {
        cur += 2;
        element_size += 2;
    }
...
path::iterator path::begin() const
{
    ...
    if (itr.m_element.m_pathname == preferred_separator_string)
        itr.m_element.m_pathname =
            separator_string; // needed for Windows, harmless on POSIX
    return itr;
}
...
void path::m_path_iterator_increment(path::iterator& it)
{
    ...
    // increment to position past current element; if current element is
    // implicit dot, this will cause it.m_pos to represent the end iterator
    ...
    if (detail::is_directory_separator(it.m_path_ptr->m_pathname[it.m_pos]))
    {
        ...
        // skip separators until it.m_pos points to the start of the next
        // element
        while (
            it.m_pos != it.m_path_ptr->m_pathname.size() &&
    ...
    // get m_element
    size_type end_pos(
        it.m_path_ptr->m_pathname.find_first_of(separators, it.m_pos));
    if (end_pos == string_type::npos)
        end_pos = it.m_path_ptr->m_pathname.size();
    it.m_element =
        it.m_path_ptr->m_pathname.substr(it.m_pos, end_pos - it.m_pos);
}
...
void path::m_path_iterator_decrement(path::iterator& it)
{
    ...
    if (it.m_pos == it.m_path_ptr->m_pathname.size() &&
        ...
        detail::is_directory_separator(
            it.m_path_ptr->m_pathname[it.m_pos - 1]) &&
        !is_root_separator(it.m_path_ptr->m_pathname, it.m_pos - 1))
    {
        --it.m_pos;
        it.m_element = detail::dot_path();
    ...
    size_type root_dir_pos(
        root_directory_start(it.m_path_ptr->m_pathname, end_pos));

    // skip separators unless root directory
    for (;
         end_pos > 0 && (end_pos - 1) != root_dir_pos &&
    ...
    it.m_element =
        it.m_path_ptr->m_pathname.substr(it.m_pos, end_pos - it.m_pos);
    if (it.m_element.m_pathname ==
        preferred_separator_string) // needed for Windows, harmless on POSIX
...
//  Prior versions of these locale and codecvt implementations tried to take
//  advantage of static initialization where possible, kept a local copy of the
//  current codecvt facet (to avoid codecvt() having to call use_facet()), and
//  was not multi-threading safe (again for efficiency).
//
//  This was error prone, and required different implementation techniques
//  depending on the compiler and also whether static or dynamic linking was
//  used. Furthermore, users could not easily provide their multi-threading safe
//  wrappers because the path interface requires the implementation itself to
//  call codecvt() to obtain the default facet, and the initialization of the
//  static within path_locale() could race.
//
//  The code below is portable to all platforms, is much simpler, and hopefully
//  will be much more robust. Timing tests (on Windows, using a Visual C++
//  release build) indicated the current code is roughly 9% slower than the
//  previous code, and that seems a small price to pay for better code that is
//  easier to use.
...
std::locale default_locale()
{
    ...
    // encoding and nothing else." See
    // http://developer.apple.com/mac/library/documentation/MacOSX/Conceptual/BPInternational/Articles/FileEncodings.html
    //
    // "The kernel will reject any filename that is not a valid UTF-8 string,
    // and it will even be normalized (to Unicode NFD) before stored on disk, at
    // least when using HFS. The right way to deal with it would be to always
    // convert the filename to UTF-8 before trying to open/create a file." See
    ...
    // Terminal. How it's stored on disk depends on the disk format; HFS+ uses
    // UTF-16, but that's not important in most cases." See
    // http://lists.apple.com/archives/applescript-users/2002/Sep/msg00319.html
    //
    // Many thanks to Peter Dimov for digging out the above references!

    std::locale global_loc = std::locale();
    return std::locale(global_loc,
                       new boost::filesystem::detail::utf8_codecvt_facet);
#else // Other POSIX
    // ISO C calls std::locale("") "the locale-specific native environment", and
    // this locale is the default for many POSIX-based operating systems such as
    // Linux.
    return std::locale("");
#endif
}
...
// path_locale() provides lazy initialization via a local static to ensure that
// any exceptions occur after main() starts and so can be caught. Furthermore,
// path_locale() is only called if path::codecvt() or path::imbue() are
// themselves actually called, ensuring that an exception will only be thrown if
// std::locale("") is really needed.
{
    // [locale] paragraph 6: Once a facet reference is obtained from a locale
    // object by calling use_facet<>, that reference remains usable, and the
    // results from member functions of it may be cached and re-used, as long as
