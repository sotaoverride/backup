...
class thread_data : public detail::thread_data_base
{
  public:
    ...
    thread_data(BOOST_THREAD_RV_REF(F) f_,
                BOOST_THREAD_RV_REF(ArgTypes)... args_) :
    ...
    void run2(tuple_indices<Indices...>)
    {
        ...
        detail::invoke(std::move(std::get<0>(fp)),
                       std::move(std::get<Indices>(fp))...);
    }
    void run()
    {
...
class thread_data : public detail::thread_data_base
{
  public:
    ...
    thread_data(BOOST_THREAD_RV_REF(F) f_) : f(boost::forward<F>(f_))
    {
    }
// This overloading must be removed if we want the packaged_task's tests to
// pass.
//            thread_data(F& f_):
//                f(f_)
//            {}
    ...
    thread_data(BOOST_THREAD_RV_REF(F) f_) : f(f_)
    {
    }
...
class thread_data<boost::reference_wrapper<F>> : public detail::thread_data_base
{
  ...
  public:
    BOOST_THREAD_NO_COPYABLE(thread_data)
    thread_data(boost::reference_wrapper<F> f_) : f(f_)
    {
    }
    void run()
    {
    ...
    : public detail::thread_data_base
{
  ...
  public:
    BOOST_THREAD_NO_COPYABLE(thread_data)
    thread_data(const boost::reference_wrapper<F> f_) : f(f_)
    {
    }
    void run()
    {
...
class BOOST_THREAD_DECL thread
{
  ...
  private:
                         ...
                         BOOST_THREAD_RV_REF(ArgTypes)... args)
    {
        return detail::thread_data_ptr(
            detail::heap_new<detail::thread_data<
                typename boost::remove_reference<F>::type, ArgTypes...>>(
        ...
        make_thread_info(BOOST_THREAD_RV_REF(F) f)
    {
        return detail::thread_data_ptr(
            detail::heap_new<
                detail::thread_data<typename boost::remove_reference<F>::type>>(
                boost::forward<F>(f)));
    }
#endif
    ...
    {
        return detail::thread_data_ptr(
            detail::heap_new<detail::thread_data<void (*)()>>(
    ...
    static inline detail::thread_data_ptr make_thread_info(
        ...
        typename disable_if_c<
            ...
            // ||
            is_same<typename decay<F>::type, thread>::value, dummy*>::type = 0)
    {
        return detail::thread_data_ptr(
            detail::heap_new<detail::thread_data<F>>(f));
    }
    template <typename F>
    static inline detail::thread_data_ptr
        make_thread_info(BOOST_THREAD_RV_REF(F) f)
  ...
  public:
    ...
    explicit thread(BOOST_THREAD_RV_REF(F) f
        ...
        thread_info(
            make_thread_info(thread_detail::decay_copy(boost::forward<F>(f))))
    {
        start_thread();
    }
    template <class F>
    ...
    template <class F>
    thread(attributes const& attrs, F f) : thread_info(make_thread_info(f))
    {
        start_thread(attrs);
    }
#else
    ...
    explicit thread(
        F f,
        typename disable_if_c<
            boost::thread_detail::is_rv<F>::value // todo as a
                                                  // thread_detail::is_rv
            // boost::thread_detail::is_convertible<F&,BOOST_THREAD_RV_REF(F)>::value
    ...
    thread(attributes const& attrs, F f,
           ...
           //, typename
           //disable_if<boost::thread_detail::is_convertible<F&,BOOST_THREAD_RV_REF(F)
           //>, dummy* >::type=0
           ) :
        thread_info(make_thread_info(f))
    {
    ...
    thread(F f, A1 a1,
           typename disable_if<
               boost::thread_detail::is_convertible<F&, thread_attributes>,
               dummy*>::type = 0) :
    ...
    thread(F f, A1 a1, A2 a2) :
        thread_info(
            make_thread_info(boost::bind(boost::type<void>(), f, a1, a2)))
    {
        start_thread();
    }
    ...
    thread(F f, A1 a1, A2 a2, A3 a3) :
        thread_info(
            make_thread_info(boost::bind(boost::type<void>(), f, a1, a2, a3)))
    {
        start_thread();
    }
    ...
    thread(F f, A1 a1, A2 a2, A3 a3, A4 a4) :
        thread_info(make_thread_info(
            boost::bind(boost::type<void>(), f, a1, a2, a3, a4)))
    {
        start_thread();
    }
    ...
    thread(F f, A1 a1, A2 a2, A3 a3, A4 a4, A5 a5) :
        thread_info(make_thread_info(
            boost::bind(boost::type<void>(), f, a1, a2, a3, a4, a5)))
    {
        start_thread();
    }
    ...
    thread(F f, A1 a1, A2 a2, A3 a3, A4 a4, A5 a5, A6 a6) :
        thread_info(make_thread_info(
            boost::bind(boost::type<void>(), f, a1, a2, a3, a4, a5, a6)))
    {
        start_thread();
    }
    ...
    thread(F f, A1 a1, A2 a2, A3 a3, A4 a4, A5 a5, A6 a6, A7 a7) :
        thread_info(make_thread_info(
            boost::bind(boost::type<void>(), f, a1, a2, a3, a4, a5, a6, a7)))
    {
        start_thread();
    }
  ...
  private:
    bool join_noexcept();
    bool do_try_join_until_noexcept(
...
class BOOST_SYMBOL_VISIBLE thread::id
{
  private:
    ...
    id(data thread_data_) : thread_data(thread_data_)
    {
    }
    friend class thread;
  ...
  public:
    ...
    id(const id& other) BOOST_NOEXCEPT : thread_data(other.thread_data)
    {
    }
...
inline thread::id get_id() BOOST_NOEXCEPT
{
    ...
    return (thread_info ? thread::id(thread_info->shared_from_this())
                        : thread::id());
#endif
}
} // namespace this_thread
...
inline bool thread::do_try_join_until(
...
struct thread_exit_function : thread_exit_function_base
{
    ...
    thread_exit_function(F f_) : f(f_)
    {
    }
