 ...
 * images to the GdkPixdata format, which allows you to create pixbufs directly
 * using the data inside the resource file, rather than an (uncompressed) copy
 * if it. For this, the gdk-pixbuf-pixdata program must be in the PATH, or the
 ...
 * its more common to use the --generate-source and
 * --generate-header arguments to create a source file and header to link
 ...
 * to perform during resource lookups.
 *
 ...
 * resource path
 * `/org/gtk/libgtk/ui/gtkdialog.ui` then GResource would check the filesystem
 * path
 * `/home/desrt/gtk-overlay/ui/gtkdialog.ui`.  If a file was found there, it
 * would be used instead.  This is an overlay, not an outright replacement,
 * which means that if a file is not found at that path, the built-in version
 * will be used instead.  Whiteouts are not currently supported.
 *
...
static gboolean open_overlay_stream(const gchar* candidate, gpointer user_data)
{
    ...
    *res = (GInputStream*)g_file_read(file, NULL, &error);
    ...
    else
    {
        if (!g_error_matches(error, G_IO_ERROR, G_IO_ERROR_NOT_FOUND))
            g_warning("Can't open overlay file '%s': %s", candidate,
                      error->message);
        g_error_free(error);
    }
...
static gboolean get_overlay_bytes(const gchar* candidate, gpointer user_data)
{
    ...
    else
    {
        if (!g_error_matches(error, G_FILE_ERROR, G_FILE_ERROR_NOENT))
            g_warning("Can't mmap overlay file '%s': %s", candidate,
                      error->message);
        g_error_free(error);
    }
...
static gboolean enumerate_overlay_dir(const gchar* candidate,
                                      gpointer user_data)
{
    ...
    {
        if (!g_error_matches(error, G_FILE_ERROR, G_FILE_ERROR_NOENT))
            g_warning("Can't enumerate overlay directory '%s': %s", candidate,
                      error->message);
        g_error_free(error);
        return FALSE;
    }
...
static gboolean get_overlay_info(const gchar* candidate, gpointer user_data)
{
...
static gboolean g_resource_find_overlay(const gchar* path, CheckCandidate check,
                                        gpointer user_data)
{
    ...
    if (g_once_init_enter(&overlay_dirs))
    {
        ...
        if (envvar != NULL)
        {
            ...
            for (i = j = 0; parts[i]; i++)
            {
                ...
                if (eq == NULL)
                {
                    g_critical("G_RESOURCE_OVERLAYS segment '%s' lacks '='.  "
                               "Ignoring.",
                               part);
                    g_free(part);
                    continue;
                ...
                if (eq == part)
                {
                    g_critical("G_RESOURCE_OVERLAYS segment '%s' lacks path "
                               "before '='.  Ignoring.",
                               part);
                    g_free(part);
                    continue;
                ...
                if (part[0] != '/')
                {
                    g_critical("G_RESOURCE_OVERLAYS segment '%s' lacks leading "
                               "'/'.  Ignoring.",
                               part);
                    g_free(part);
                    continue;
                ...
                if (eq[-1] == '/')
                {
                    g_critical("G_RESOURCE_OVERLAYS segment '%s' has trailing "
                               "'/' before '='.  Ignoring",
                               part);
                    g_free(part);
                    continue;
                ...
                {
                    g_critical("G_RESOURCE_OVERLAYS segment '%s' does not have "
                               "an absolute path after '='.  Ignoring",
                               part);
                    g_free(part);
                    continue;
                }
    ...
    for (i = 0; overlay_dirs[i]; i++)
    {
...

        {
            ...
            src_len = eq - src;
            dst = eq + 1;
            /* hold off on dst_len because we will probably fail the checks
             * below */
        }
 ...
 */
static GResource* g_resource_new_from_table(GvdbTable* table)
{
    GResource* resource;
...
static void g_resource_error_from_gvdb_table_error(
    GError** g_resource_error, GError* gvdb_table_error /* (transfer full) */)
{
    if (g_error_matches(gvdb_table_error, G_FILE_ERROR, G_FILE_ERROR_INVAL))
        g_set_error_literal(g_resource_error, G_RESOURCE_ERROR,
...
GResource* g_resource_new_from_data(GBytes* data, GError** error)
{
    ...
    {
        data =
            g_bytes_new(g_bytes_get_data(data, NULL), g_bytes_get_size(data));
        unref_data = TRUE;
    }
    ...
    if (table == NULL)
    {
        g_resource_error_from_gvdb_table_error(error,
...
/**
 * g_resource_load:
 * @filename: (type filename): the path of a filename to load, in the GLib
 *filename encoding
 * @error: return location for a #GError, or %NULL
 *
...
static gboolean do_lookup(GResource* resource, const gchar* path,
                          GResourceLookupFlags lookup_flags, gsize* size,
                          guint32* flags, const void** data, gsize* data_size,
                          GError** error)
{
    char* free_path = NULL;
    gsize path_len;
    ...
    if (value == NULL)
    {
        g_set_error(error, G_RESOURCE_ERROR, G_RESOURCE_ERROR_NOT_FOUND,
                    _("The resource at “%s” does not exist"), path);
    }
    else
    {
        ...
        g_variant_get(value, "(uu@ay)", &_size, &_flags, &array);
 ...
 * Since: 2.32
 **/
GInputStream* g_resource_open_stream(GResource* resource, const gchar* path,
                                     ...
                                     GError** error)
{
    ...
    if (!do_lookup(resource, path, lookup_flags, NULL, &flags, &data,
                   &data_size, error))
        return NULL;
 ...
 * Since: 2.32
 **/
GBytes* g_resource_lookup_data(GResource* resource, const gchar* path,
                               GResourceLookupFlags lookup_flags,
                               GError** error)
{
    const void* data;
    ...
    if (!do_lookup(resource, path, lookup_flags, &size, &flags, &data,
                   &data_size, error))
        return NULL;
    ...
    if (flags & G_RESOURCE_FLAGS_COMPRESSED)
    {
        ...
        do
        {
            res = g_converter_convert(G_CONVERTER(decompressor), s, s_size, d,
                                      d_size, G_CONVERTER_INPUT_AT_END,
        ...
        return g_bytes_new_with_free_func(data, data_size,
                                          (GDestroyNotify)g_resource_unref,
                                          g_resource_ref(resource));
}
 ...
 * @path: A pathname inside the resource
 * @lookup_flags: A #GResourceLookupFlags
 * @size:  (out) (optional): a location to place the length of the contents of
 ...
 * Since: 2.32
 **/
gboolean g_resource_get_info(GResource* resource, const gchar* path,
                             GResourceLookupFlags lookup_flags, gsize* size,
                             guint32* flags, GError** error)
{
    return do_lookup(resource, path, lookup_flags, size, flags, NULL, NULL,
                     error);
}
 ...
 * @lookup_flags controls the behaviour of the lookup.
 *
 * Returns: (array zero-terminated=1) (transfer full): an array of constant
 *strings
 *
 * Since: 2.32
 **/
gchar** g_resource_enumerate_children(GResource* resource, const gchar* path,
                                      GResourceLookupFlags lookup_flags,
                                      GError** error)
{
    gchar local_str[256];
...
/* This is updated atomically, so we can append to it and check for NULL outside
   the lock, but all other accesses are done under the write lock */
static GStaticResource* lazy_register_resources;

static void g_resources_register_unlocked(GResource* resource)
{
    registered_resources =
        g_list_prepend(registered_resources, g_resource_ref(resource));
}

static void g_resources_unregister_unlocked(GResource* resource)
{
 ...
 * Since: 2.32
 **/
GInputStream* g_resources_open_stream(const gchar* path,
                                      GResourceLookupFlags lookup_flags,
                                      GError** error)
{
    ...
    for (l = registered_resources; l != NULL; l = l->next)
    {
        ...
        if (stream == NULL && g_error_matches(my_error, G_RESOURCE_ERROR,
 ...
 * Since: 2.32
 **/
GBytes* g_resources_lookup_data(const gchar* path,
                                GResourceLookupFlags lookup_flags,
                                GError** error)
{
    GBytes* res = NULL;
    ...
    for (l = registered_resources; l != NULL; l = l->next)
    {
        ...
        if (data == NULL && g_error_matches(my_error, G_RESOURCE_ERROR,
                                            G_RESOURCE_ERROR_NOT_FOUND))
        {
            g_clear_error(&my_error);
        }
 ...
 * Since: 2.32
 **/
gchar** g_resources_enumerate_children(const gchar* path,
                                       GResourceLookupFlags lookup_flags,
                                       GError** error)
{
 ...
 * @size:  (out) (optional): a location to place the length of the contents of
 *the file, or %NULL if the length is not needed
 * @flags:  (out) (optional): a location to place the #GResourceFlags about the
 *file, or %NULL if the flags are not needed
 * @error: return location for a #GError, or %NULL
 *
 * Looks for a file at the specified @path in the set of
 ...
 * Since: 2.32
 **/
gboolean g_resources_get_info(const gchar* path,
                              GResourceLookupFlags lookup_flags, gsize* size,
                              guint32* flags, GError** error)
{
    gboolean res = FALSE;
    ...
    for (l = registered_resources; l != NULL; l = l->next)
    {
        ...
        r_res =
            g_resource_get_info(r, path, lookup_flags, size, flags, &my_error);
        if (!r_res && g_error_matches(my_error, G_RESOURCE_ERROR,
                                      G_RESOURCE_ERROR_NOT_FOUND))
        {
            g_clear_error(&my_error);
        }
...
/* This code is to handle registration of resources very early, from a
 * constructor. At that point we'd like to do minimal work, to avoid ordering
 * issues. For instance, we're not allowed to use g_malloc, as the user need to
 * be able to call g_mem_set_vtable before the first call to g_malloc.
 *
 * So, what we do at construction time is that we just register a static
 * structure on a list of resources that need to be initialized, and then later,
 * when doing any lookups in the global list of registered resources, or when
 * getting a reference to the lazily initialized resource we lazily create and
 * register all the GResources on the lazy list.
 *
 * To avoid having to use locks in the constructor, and having to grab the
 * writer lock when checking the lazy registering list we update
 * lazy_register_resources in a lock-less fashion (atomic prepend-only, atomic
 * replace with NULL). However, all operations except:
...
static void register_lazy_static_resources_unlocked(void)
{
    GStaticResource* list;

    do
        list = lazy_register_resources;
    while (!g_atomic_pointer_compare_and_exchange(&lazy_register_resources,
                                                  list, NULL));
