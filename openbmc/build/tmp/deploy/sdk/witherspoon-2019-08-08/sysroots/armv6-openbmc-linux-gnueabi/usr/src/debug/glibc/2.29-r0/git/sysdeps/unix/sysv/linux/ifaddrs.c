...
int __netlink_request(struct netlink_handle* h, int type)
{
    ...
    while (!done)
    {
       ...
       there is no point to record it.  */
        ...
        nlm_next =
            (struct netlink_res*)malloc(sizeof(struct netlink_res) + read_len);
        if (nlm_next == NULL)
            goto out_fail;
        nlm_next->next = NULL;
...
static int map_newlink(int index, struct ifaddrs_storage* ifas, int* map,
                       int max)
{
    int i;
                ...
                while (RTA_OK(rta, rtasize))
                {
                    ...
                    switch (rta->rta_type)
                    {
                        case IFLA_ADDRESS:
                            if (rta_payload <= sizeof(ifas[ifa_index].addr))
                            {
                                ...
                                ifas[ifa_index].addr.sl.sll_ifindex =
                                    ifim->ifi_index;
                                ifas[ifa_index].addr.sl.sll_hatype =
                                    ifim->ifi_type;
                        ...
                        case IFLA_BROADCAST:
                            ...
                            {
                                ifas[ifa_index].broadaddr.sl.sll_family =
                                    AF_PACKET;
                                memcpy(ifas[ifa_index].broadaddr.sl.sll_addr,
                                       (char*)rta_data, rta_payload);
                                ifas[ifa_index].broadaddr.sl.sll_halen =
                                    rta_payload;
                        ...
                        case IFLA_IFNAME: /* Name of Interface */
                                ...
                                sizeof(ifas[ifa_index].name))
                            {
                                ifas[ifa_index].ifa.ifa_name =
                                    ifas[ifa_index].name;
                                *(char*)__mempcpy(ifas[ifa_index].name,
                                                  rta_data, rta_payload) = '\0';
                            }
                            break;
                ...
                while (RTA_OK(rta, rtasize))
                {
                    ...
                    switch (rta->rta_type)
                    {
                        case IFA_ADDRESS:
                        {
                            ...
                            {
                                case AF_INET:
                                    /* Size must match that of an address for
                                     * IPv4.  */
                                    if (rta_payload == 4)
                        ...
                        case IFA_LOCAL:
                            ...
                            switch (ifam->ifa_family)
                            {
                                case AF_INET:
                                    ...
                                    if (rta_payload == 4)
                                        memcpy(
                                            &ifas[ifa_index].addr.s4.sin_addr,
                                            rta_data, rta_payload);
                                    break;
                                ...
                                case AF_INET6:
                                    ...
                                    {
                                        memcpy(
                                            &ifas[ifa_index].addr.s6.sin6_addr,
                                            rta_data, rta_payload);
                                        if (IN6_IS_ADDR_LINKLOCAL(rta_data) ||
                                            IN6_IS_ADDR_MC_LINKLOCAL(rta_data))
                                            ifas[ifa_index]
                                                .addr.s6.sin6_scope_id =
                                                ifam->ifa_index;
                                    }
                                    break;
                        ...
                        case IFA_BROADCAST:
                            ...
                            switch (ifam->ifa_family)
                            {
                                case AF_INET:
                                     ...
                                     * IPv4.  */
                                    if (rta_payload == 4)
                                        memcpy(&ifas[ifa_index]
                                                    .broadaddr.s4.sin_addr,
                                               rta_data, rta_payload);
                                    break;
                                ...
                                case AF_INET6:
                                    ...
                                    {
                                        memcpy(&ifas[ifa_index]
                                                    .broadaddr.s6.sin6_addr,
                                               rta_data, rta_payload);
                                        if (IN6_IS_ADDR_LINKLOCAL(rta_data) ||
                                            IN6_IS_ADDR_MC_LINKLOCAL(rta_data))
                                            ifas[ifa_index]
                                                .broadaddr.s6.sin6_scope_id =
                                                ifam->ifa_index;
                                    }
                                    break;
