...
int RAND_DRBG_set_defaults(int type, unsigned int flags)
{
    ...
    {
        default:
            RANDerr(RAND_F_RAND_DRBG_SET_DEFAULTS,
                          ...
                          size_t perslen)
{
    ...
    if (!drbg->meth->instantiate(drbg, entropy, entropylen, nonce, noncelen,
                                 pers, perslen))
    {
                     ...
                     int prediction_resistance)
{
    ...
    if (drbg->get_entropy != NULL)
        entropylen = drbg->get_entropy(
            drbg, &entropy, drbg->strength, drbg->min_entropylen,
            drbg->max_entropylen, prediction_resistance);
 ...
 */
int rand_drbg_restart(RAND_DRBG* drbg, const unsigned char* buffer, size_t len,
                      size_t entropy)
{
    int reseeded = 0;
    ...
    {
        /* reinstantiate drbg */
        RAND_DRBG_instantiate(drbg, (const unsigned char*)ossl_pers_string,
                       ...
                       size_t adinlen)
{
    ...
    if (drbg->parent != NULL)
    {
        ...
        if (reseed_counter > 0 &&
            tsan_load(&drbg->parent->reseed_prop_counter) != reseed_counter)
            reseed_required = 1;
    }
...
int RAND_DRBG_bytes(RAND_DRBG* drbg, unsigned char* out, size_t outlen)
{
    ...
    additional_len =
        rand_drbg_get_additional_data(drbg->adin_pool, &additional);
    ...
    for (; outlen > 0; outlen -= chunk, out += chunk)
    {
        ...
        ret =
            RAND_DRBG_generate(drbg, out, chunk, 0, additional, additional_len);
        if (!ret)
            goto err;
    }
    ret = 1;
...
static RAND_DRBG* drbg_setup(RAND_DRBG* parent)
{
     ...
     */
    (void)RAND_DRBG_instantiate(drbg, (const unsigned char*)ossl_pers_string,
                                sizeof(ossl_pers_string) - 1);
    return drbg;
...
static int drbg_add(const void* buf, int num, double randomness)
{
    ...
    if (buflen < seedlen || randomness < (double)seedlen)
    {
         ...
         * as additional data by setting randomness to zero and trigger a
         * regular reseeding.
         */
        randomness = 0.0;
#endif
...
RAND_METHOD rand_meth = {drbg_seed, drbg_bytes, NULL,
                         drbg_add,  drbg_bytes, drbg_status};
