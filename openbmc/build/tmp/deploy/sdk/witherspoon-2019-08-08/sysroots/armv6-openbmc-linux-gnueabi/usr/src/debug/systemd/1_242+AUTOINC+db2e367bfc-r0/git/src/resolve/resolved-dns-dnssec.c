...
/* Permit a maximum clock skew of 1h 10min. This should be enough to deal with
 * DST confusion */
#define SKEW_MAX (1 * USEC_PER_HOUR + 10 * USEC_PER_MINUTE)

/* Maximum number of NSEC3 iterations we'll do. RFC5155 says 2500 shall be the
 ...
 *
 *            Normal RRs are protected via RRSIG RRs in combination with DNSKEY
 * RRs, all in the same zone DNSKEY RRs are either protected like normal RRs, or
 * via a DS from a zone "higher" up the tree DS RRs are protected like normal
 * RRs
 *
 * Example chain:
 *            Normal RR → RRSIG/DNSKEY+ → DS → RRSIG/DNSKEY+ → DS → ... → DS →
 * RRSIG/DNSKEY+ → DS
 */
...
uint16_t dnssec_keytag(DnsResourceRecord* dnskey, bool mask_revoke)
{
    ...
    sum = f + ((((uint32_t)dnskey->dnskey.protocol) << 8) +
               (uint32_t)dnskey->dnskey.algorithm);
...
static int rr_compare(DnsResourceRecord* const* a, DnsResourceRecord* const* b)
{
    ...
    m = MIN(DNS_RESOURCE_RECORD_RDATA_SIZE(x),
    ...
    return CMP(DNS_RESOURCE_RECORD_RDATA_SIZE(x),
               DNS_RESOURCE_RECORD_RDATA_SIZE(y));
}
                                 ...
                                 const void* modulus, size_t modulus_size)
{
    ...
    ge = gcry_sexp_build(&data_sexp, NULL, "(data (flags pkcs1) (hash %s %b))",
                         hash_algorithm, (int)data_size, data);
    if (ge != 0)
    {
        r = -EIO;
        goto finish;
    ...
    ge = gcry_sexp_build(&public_key_sexp, NULL,
...
static int dnssec_rsa_verify(const char* hash_algorithm, const void* hash,
                             size_t hash_size, DnsResourceRecord* rrsig,
                             DnsResourceRecord* dnskey)
{
    ...
    return dnssec_rsa_verify_raw(
        hash_algorithm, rrsig->rrsig.signature, rrsig->rrsig.signature_size,
        hash, hash_size, exponent, exponent_size, modulus, modulus_size);
}

static int dnssec_ecdsa_verify_raw(
    const char* hash_algorithm, const char* curve, const void* signature_r,
    size_t signature_r_size, const void* signature_s, size_t signature_s_size,
    const void* data, size_t data_size, const void* key, size_t key_size)
{
    ...
    ge =
        gcry_mpi_scan(&r, GCRYMPI_FMT_USG, signature_r, signature_r_size, NULL);
    if (ge != 0)
    {
        k = -EIO;
        goto finish;
    ...
    ge =
        gcry_mpi_scan(&s, GCRYMPI_FMT_USG, signature_s, signature_s_size, NULL);
    if (ge != 0)
    {
        k = -EIO;
        goto finish;
    ...
    ge = gcry_sexp_build(&signature_sexp, NULL,
    ...
    ge =
        gcry_sexp_build(&data_sexp, NULL, "(data (flags rfc6979) (hash %s %b))",
                        hash_algorithm, (int)data_size, data);
    if (ge != 0)
    {
        k = -EIO;
    ...
    ge = gcry_sexp_build(&public_key_sexp, NULL,
                         "(public-key (ecc (curve %s) (q %m)))", curve, q);
    if (ge != 0)
    {
        k = -EIO;
        goto finish;
...
static int dnssec_ecdsa_verify(const char* hash_algorithm, int algorithm,
                               ...
                               DnsResourceRecord* dnskey)
{
    ...
    return dnssec_ecdsa_verify_raw(
        hash_algorithm, curve, rrsig->rrsig.signature, key_size,
        (uint8_t*)rrsig->rrsig.signature + key_size, key_size, hash, hash_size,
        q, key_size * 2 + 1);
}

#if GCRYPT_VERSION_NUMBER >= 0x010600
static int dnssec_eddsa_verify_raw(const char* curve, const void* signature_r,
                                   size_t signature_r_size,
                                   const void* signature_s,
                                   size_t signature_s_size, const void* data,
                                   size_t data_size, const void* key,
                                   size_t key_size)
{
    ...
    ge = gcry_sexp_build(
        &signature_sexp, NULL, "(sig-val (eddsa (r %b) (s %b)))",
        (int)signature_r_size, signature_r, (int)signature_s_size, signature_s);
    if (ge != 0)
    {
        k = -EIO;
        goto finish;
    ...
    ge = gcry_sexp_build(&data_sexp, NULL,
...
static int dnssec_eddsa_verify(int algorithm, const void* data,
                               size_t data_size, DnsResourceRecord* rrsig,
                               DnsResourceRecord* dnskey)
{
    const char* curve;
    size_t key_size;
    ...
    return dnssec_eddsa_verify_raw(curve, rrsig->rrsig.signature, key_size,
                                   (uint8_t*)rrsig->rrsig.signature + key_size,
                                   key_size, data, data_size,
...
static int dnssec_rrsig_prepare(DnsResourceRecord* rrsig)
{
    ...
    /* Checks whether the specified RRSIG RR is somewhat valid, and initializes
     * the .n_skip_labels_source and
...
static void dnssec_fix_rrset_ttl(DnsResourceRecord* list[], unsigned n,
                                 DnsResourceRecord* rrsig, usec_t realtime)
{
    ...
    for (k = 0; k < n; k++)
    {
        ...
        /* Copy over information about the signer and wildcard source of
         * synthesis */
...
int dnssec_verify_rrset(DnsAnswer* a, const DnsResourceKey* key,
                        DnsResourceRecord* rrsig, DnsResourceRecord* dnskey,
                        usec_t realtime, DnssecResult* result)
{
    ...
    /* Some keys may only appear signed in the zone apex, and are invalid
     * anywhere else. (SOA, NS...) */
    ...
    /* OTOH DS RRs may not appear in the zone apex, but are valid everywhere
     * else. */
    ...
    {
        /* We refuse to validate NSEC3 or SOA RRs that are synthesized from
         * wildcards */
        *result = DNSSEC_INVALID;
        return 0;
    }
    if (r == 1)
    {
        /* If we stripped a single label, then let's see if that maybe was "*".
         * If so, we are not really synthesized from a wildcard, we are the
         * wildcard itself. Treat that like a normal name. */
    ...
    /* Collect all relevant RRs in a single array, so that we can look at the
     * RRset */
    ...
    r = dns_name_to_wire_format(rrsig->rrsig.signer, wire_format_name,
                                sizeof(wire_format_name), true);
    if (r < 0)
        return r;
    fwrite(wire_format_name, 1, r, f);

    /* Convert the source of synthesis into wire format */
    r = dns_name_to_wire_format(source, wire_format_name,
                                sizeof(wire_format_name), true);
    if (r < 0)
        return r;
    ...
    for (k = 0; k < n; k++)
    {
        ...
        /* Hash the source of synthesis. If this is a wildcard, then prefix it
         * with the *. label */
        if (wildcard)
    ...
    switch (rrsig->rrsig.algorithm)
    {
        ...
        case DNSSEC_ALGORITHM_ED448:
            *result = DNSSEC_UNSUPPORTED_ALGORITHM;
            return 0;
        default:
            /* OK, the RRs are now in canonical order. Let's calculate the
             * digest */
    ...
    switch (rrsig->rrsig.algorithm)
    {
        ...
        case DNSSEC_ALGORITHM_RSASHA256:
        case DNSSEC_ALGORITHM_RSASHA512:
            r = dnssec_rsa_verify(gcry_md_algo_name(md_algorithm), hash,
                                  hash_size, rrsig, dnskey);
            break;

        case DNSSEC_ALGORITHM_ECDSAP256SHA256:
        case DNSSEC_ALGORITHM_ECDSAP384SHA384:
            r = dnssec_ecdsa_verify(gcry_md_algo_name(md_algorithm),
                                    rrsig->rrsig.algorithm, hash, hash_size,
                                    rrsig, dnskey);
            break;
#if GCRYPT_VERSION_NUMBER >= 0x010600
        case DNSSEC_ALGORITHM_ED25519:
            r = dnssec_eddsa_verify(rrsig->rrsig.algorithm, sig_data, sig_size,
                                    rrsig, dnskey);
            break;
#endif
    }
    ...
    /* Now, fix the ttl, expiry, and remember the synthesizing source and the
     * signer */
    if (r > 0)
...
int dnssec_rrsig_match_dnskey(DnsResourceRecord* rrsig,
                              DnsResourceRecord* dnskey, bool revoked_ok)
{
    ...
    return dns_name_equal(dns_resource_key_name(dnskey->key),
                          rrsig->rrsig.signer);
}
...
int dnssec_key_match_rrsig(const DnsResourceKey* key, DnsResourceRecord* rrsig)
{
    ...
    /* Checks if the specified RRSIG RR protects the RRSet of the specified RR
     * key. */
    ...
    return dns_name_equal(dns_resource_key_name(rrsig->key),
                          dns_resource_key_name(key));
}

int dnssec_verify_rrset_search(DnsAnswer* a, const DnsResourceKey* key,
                               DnsAnswer* validated_dnskeys, usec_t realtime,
                               DnssecResult* result,
                               DnsResourceRecord** ret_rrsig)
{

    bool found_rrsig = false, found_invalid = false,
         found_expired_rrsig = false, found_unsupported_algorithm = false;
    DnsResourceRecord* rrsig;
    int r;
    ...
    /* Verifies all RRs from "a" that match the key "key" against DNSKEYs in
     * "validated_dnskeys" */
...
int dnssec_has_rrsig(DnsAnswer* a, const DnsResourceKey* key)
{
    ...
    /* Checks whether there's at least one RRSIG in 'a' that proctects RRs of
     * the specified key */
...
int dnssec_verify_dnskey_by_ds(DnsResourceRecord* dnskey, DnsResourceRecord* ds,
                               bool mask_revoke)
{
    ...
    /* Implements DNSKEY verification by a DS, according to RFC 4035,
     * section 5.2 */
...
int dnssec_verify_dnskey_by_ds_search(DnsResourceRecord* dnskey,
                                      DnsAnswer* validated_ds)
{
    ...
    DNS_ANSWER_FOREACH_FLAGS(ds, flags, validated_ds)
    {
        ...
        r = dns_name_equal(dns_resource_key_name(dnskey->key),
                           dns_resource_key_name(ds->key));
        if (r < 0)
            return r;
        if (r == 0)
            continue;
        ...
        if (IN_SET(r, -EKEYREJECTED, -EOPNOTSUPP))
            return 0; /* The DNSKEY is revoked or otherwise invalid, or we don't
                         support the digest algorithm */
        if (r < 0)
            return r;
        if (r > 0)
            return 1;
...
static int nsec3_is_good(DnsResourceRecord* rr, DnsResourceRecord* nsec3)
{
    ...
    /* Ignore NSEC3 RRs generated from wildcards. If these NSEC3 RRs weren't
     * correctly signed we can't make this
     * check (since rr->n_skip_labels_source is -1), but that's OK, as we won't
     * trust them anyway in that case. */
    if (!IN_SET(rr->n_skip_labels_source, 0, (unsigned)-1))
        return 0;
    /* Ignore NSEC3 RRs that are located anywhere else than one label below the
     * zone */
    ...
    /* If a second NSEC3 RR is specified, also check if they are from the same
     * zone. */
    ...
    if (rr->nsec3.salt_size != nsec3->nsec3.salt_size)
        return 0;
    if (memcmp_safe(rr->nsec3.salt, nsec3->nsec3.salt, rr->nsec3.salt_size) !=
        0)
        return 0;
...
static int nsec3_hashed_domain_format(const uint8_t* hashed, size_t hashed_size,
                                      const char* zone, char** ret)
{
...
static int nsec3_hashed_domain_make(DnsResourceRecord* nsec3,
                                    const char* domain, const char* zone,
                                    char** ret)
{
 ...
 * First try to find a NSEC3 record that matches our query precisely, if that
 * fails, find the closest enclosure. Secondly, find a proof that there is no
 * closer enclosure and either a proof that there is no wildcard domain as a
 * direct descendant of the closest enclosure, or find an NSEC3 record that
 * matches the wildcard domain.
 *
 * Based on this we can prove either the existence of the record in @key, or
 * NXDOMAIN or NODATA, or that there is no proof either way. The latter is the
 * case if a the proof of non-existence of a given name uses an NSEC3 record
 * with the opt-out bit set. Lastly, if we are given insufficient NSEC3 records
 * to conclude anything we indicate this by returning NO_RR. */
static int dnssec_test_nsec3(DnsAnswer* answer, DnsResourceKey* key,
                             DnssecNsecResult* result, bool* authenticated,
                             uint32_t* ttl)
{
    ...
    for (;;)
    {
        DNS_ANSWER_FOREACH_FLAGS(zone_rr, flags, answer)
        {
            ...
            r = dns_name_equal_skip(dns_resource_key_name(zone_rr->key), 1,
                                    zone);
            if (r < 0)
                return r;
            if (r > 0)
...
found_zone:
    /* Second step, find the closest encloser NSEC3 RR in 'answer' that matches
     * 'key' */
    ...
    for (;;)
    {
        ...
        hashed_size =
            nsec3_hashed_domain_make(zone_rr, p, zone, &hashed_domain);
        ...
        DNS_ANSWER_FOREACH_FLAGS(enclosure_rr, flags, answer)
        {
            ...
            if (enclosure_rr->nsec3.next_hashed_name_size !=
                (size_t)hashed_size)
                continue;

            r = dns_name_equal(dns_resource_key_name(enclosure_rr->key),
                               hashed_domain);
            if (r < 0)
                return r;
            if (r > 0)
...
found_closest_encloser:
    ...
    if (!pp)
    {
        /* We have an exact match! If we area looking for a DS RR, then we must
         * insist that we got the NSEC3 RR from the parent. Otherwise the one
         * from the child. Do so, by checking whether SOA and NS are
         * appropriately set. */
        ...
        /* No next closer NSEC3 RR. That means there's a direct NSEC3 RR for our
         * key. */
    ...
    /* Prove that there is no next closer and whether or not there is a wildcard
     * domain. */

    wildcard = strjoina("*.", p);
    r = nsec3_hashed_domain_make(enclosure_rr, wildcard, zone,
                                 &wildcard_domain);
    if (r < 0)
        return r;
    if (r != hashed_size)
    ...
    DNS_ANSWER_FOREACH_FLAGS(rr, flags, answer)
    {
        ...
        r = nsec3_hashed_domain_format(rr->nsec3.next_hashed_name,
                                       rr->nsec3.next_hashed_name_size, zone,
                                       &next_hashed_domain);
        if (r < 0)
            return r;

        r = dns_name_between(dns_resource_key_name(rr->key), next_closer_domain,
                             next_hashed_domain);
        if (r < 0)
            return r;
        if (r > 0)
        ...
        r = dns_name_between(dns_resource_key_name(rr->key), wildcard_domain,
                             next_hashed_domain);
        if (r < 0)
            return r;
        if (r > 0)
        {
            if (rr->nsec3.flags & 1)
                /* This only makes sense if we have a wildcard delegation, which
                 * is very unlikely, see RFC 4592, Section 4.2, but we cannot
                 * rely on this not happening, so hence cannot simply conclude
                 * NXDOMAIN as we would wish */
                optout = true;
    ...
    {
        if (optout)
            /* The RFC only specifies that we have to care for optout for NODATA
             * for DS records. However, children of an insecure opt-out
             * delegation should also be considered opt-out, rather than
             * verified NXDOMAIN. Note that we do not require a proof of
             * wildcard non-existence if the next closer domain is covered by an
             * opt-out, as that would not provide any additional information. */
            *result = DNSSEC_NSEC_OPTOUT;
        else if (no_wildcard)
...
static int dnssec_nsec_wildcard_equal(DnsResourceRecord* rr, const char* name)
{
    ...
    /* Checks whether the specified RR has a name beginning in "*.", and if the
     * rest is a suffix of our name */
...
static int dnssec_nsec_in_path(DnsResourceRecord* rr, const char* name)
{
    ...
    /* Checks whether the specified nsec RR indicates that name is an empty
     * non-terminal (ENT)
     *
     * A couple of examples:
     *
     *      NSEC             bar →   waldo.foo.bar: indicates that foo.bar
     * exists and is an ENT NSEC   waldo.foo.bar → yyy.zzz.xoo.bar: indicates
     * that xoo.bar and zzz.xoo.bar exist and are ENTs NSEC yyy.zzz.xoo.bar →
     * bar: indicates pretty much nothing about ENTs
     */
    ...
    /* If the name we just determined is not equal or child of the name we are
     * interested in, then we can't say anything at all. */
    r = dns_name_endswith(nn, name);
    if (r <= 0)
        return r;

    /* If the name we are interested in is not a prefix of the common suffix of
     * the NSEC RR's owner and next domain names, then we can't say anything
     * either. */
    r = dns_name_common_suffix(dns_resource_key_name(rr->key),
                               rr->nsec.next_domain_name, &common_suffix);
    if (r < 0)
        return r;
...
static int dnssec_nsec_covers(DnsResourceRecord* rr, const char* name)
{
    ...
    /* Checks whether the name is covered by this NSEC RR. This means, that the
     * name is somewhere below the NSEC's signer name, and between the NSEC's
     * two names. */
    ...
    r = dns_name_endswith(name, signer); /* this NSEC isn't suitable the name is
                                            not in the signer's domain */
    if (r <= 0)
        return r;

    return dns_name_between(dns_resource_key_name(rr->key), name,
                            rr->nsec.next_domain_name);
}

static int dnssec_nsec_generate_wildcard(DnsResourceRecord* rr,
                                         const char* name, char** wc)
{
    ...
    /* Generates "Wildcard at the Closest Encloser" for the given name and NSEC
     * RR. */
    ...
    r = dns_name_common_suffix(name, dns_resource_key_name(rr->key),
                               &common_suffix1);
    if (r < 0)
        return r;
...
int dnssec_nsec_test(DnsAnswer* answer, DnsResourceKey* key,
                     DnssecNsecResult* result, bool* authenticated,
                     uint32_t* ttl)
{
    bool have_nsec3 = false, covering_rr_authenticated = false,
         wildcard_rr_authenticated = false;
    ...
    DNS_ANSWER_FOREACH_FLAGS(rr, flags, answer)
    {
        ...
        /* The following checks only make sense for NSEC RRs that are not
         * expanded from a wildcard */
        ...
        /* Check if this is a direct match. If so, we have encountered a NODATA
         * case */
        ...
        if (r > 0)
        {
            if (key->type == DNS_TYPE_DS)
            {
                /* If we look for a DS RR and the server sent us the NSEC RR of
                 * the child zone
                 * we have a problem. For DS RRs we want the NSEC RR from the
                 * parent */
            ...
            {
                /* For all RR types, ensure that if NS is set SOA is set too, so
                 * that we know we got the child's NSEC. */
                if (bitmap_isset(rr->nsec.types, DNS_TYPE_NS) &&
                    !bitmap_isset(rr->nsec.types, DNS_TYPE_SOA))
                    continue;
            }
        ...
        /* Check if the name we are looking for is an empty non-terminal within
         * the owner or next name of the NSEC RR. */
        ...
        /* The following two "covering" checks, are not useful if the NSEC is
         * from the parent */
        ...
        /* Check if this NSEC RR proves the absence of an explicit RR under this
         * name */
    ...
    {
        /* If we could prove that neither the name itself, nor the wildcard at
         * the closest encloser exists, we proved the NXDOMAIN case. */
        *result = DNSSEC_NSEC_NXDOMAIN;

        if (authenticated)
            *authenticated =
                covering_rr_authenticated && wildcard_rr_authenticated;
        if (ttl)
...
static int dnssec_nsec_test_enclosed(DnsAnswer* answer, uint16_t type,
                                     ...
                                     bool* authenticated)
{
    ...
    /* Checks whether there's an NSEC/NSEC3 that proves that the specified
     * 'name' is non-existing in the specified 'zone'. The 'zone' must be a
     * suffix of the 'name'. */
    ...
    DNS_ANSWER_FOREACH_FLAGS(rr, flags, answer)
    {
        ...
        switch (rr->key->type)
        {
            ...
            case DNS_TYPE_NSEC:
                ...
                r = dns_name_between(dns_resource_key_name(rr->key), name,
                                     rr->nsec.next_domain_name);
                if (r < 0)
                    return r;
            ...
            case DNS_TYPE_NSEC3:
            {
                ...
                /* Format the domain we are testing with the NSEC3 RR's hash
                 * function */
                ...
                r = nsec3_hashed_domain_format(rr->nsec3.next_hashed_name,
                                               ...
                                               zone, &next_hashed_domain);
                if (r < 0)
                    return r;

                r = dns_name_between(dns_resource_key_name(rr->key),
                                     hashed_domain, next_hashed_domain);
                if (r < 0)
                    return r;
...
static int dnssec_test_positive_wildcard_nsec3(DnsAnswer* answer,
                                               const char* name,
                                               const char* source,
                                               const char* zone,
                                               bool* authenticated)
{
     ...
     *
     * Note a key difference between the NSEC3 and NSEC versions of the proof.
     * NSEC RRs don't have to exist for empty non-transients. NSEC3 RRs however
     * have to. This means it's sufficient to check if the next closer name
     * exists for the NSEC3 RR and we are done.
     *
     * To prove that a.b.c.d.e.f is rightfully synthesized from a wildcard
     * *.d.e.f all we have to check is that c.d.e.f does not exist. */
    ...
    return dnssec_nsec_test_enclosed(answer, DNS_TYPE_NSEC3, next_closer, zone,
                                     authenticated);
}

static int dnssec_test_positive_wildcard_nsec(DnsAnswer* answer,
                                              const char* name,
                                              const char* source,
                                              const char* zone,
                                              bool* _authenticated)
{
     ...
     *
     * A proof that there's neither a wildcard name nor a non-wildcard name that
     * is a suffix of the name "name" and a prefix of the synthesizing source
     * "source" in the zone "zone".
     *
     * See RFC 5155, Section 8.8 and RFC 4035, Section 5.3.4
     *
     * Note that if we want to prove that a.b.c.d.e.f is rightfully synthesized
     * from a wildcard *.d.e.f, then we have to prove that none of the following
     * exist:
     *
     *      1) a.b.c.d.e.f
     *      2) *.b.c.d.e.f
    ...
    for (;;)
    {
        ...
        /* Check if there's an NSEC or NSEC3 RR that proves that the mame we
         * determined is really non-existing,
...
int dnssec_test_positive_wildcard(DnsAnswer* answer, const char* name,
                                  const char* source, const char* zone,
                                  bool* authenticated)
{
    ...
    if (r < 0)
        return r;
    if (r > 0)
        return dnssec_test_positive_wildcard_nsec3(answer, name, source, zone,
                                                   authenticated);
    else
        return dnssec_test_positive_wildcard_nsec(answer, name, source, zone,
                                                  authenticated);
}
