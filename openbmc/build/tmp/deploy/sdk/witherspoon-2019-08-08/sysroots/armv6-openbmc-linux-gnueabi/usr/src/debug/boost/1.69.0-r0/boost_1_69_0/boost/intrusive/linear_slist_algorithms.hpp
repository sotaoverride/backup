...
//!
//! linear_slist_algorithms is configured with a NodeTraits class, which
...
/// @endcond
{
  ...
  public:
    ...
    //! circular list.
    //!
    //! <b>Effects</b>: Returns true is "this_node" is the only node of a
    //! circular list:
    //!  or it's a not inserted node:
    //!  <tt>return node_ptr() == NodeTraits::get_next(this_node) ||
    //!  NodeTraits::get_next(this_node) == this_node</tt>
    //!
    //! <b>Complexity</b>: Constant
    //!
    //! <b>Throws</b>: Nothing.
    ...
    //! <b>Requires</b>: prev_node must be in a circular list or be an empty
    //! circular list.
    //!
    //! <b>Effects</b>: Unlinks the next node of prev_node from the circular
    //! list.
    //!
    //! <b>Complexity</b>: Constant
    //!
    ...
    //!
    //! <b>Effects</b>: Unlinks the range (prev_node, last_node) from the linear
    //! list.
    //!
    //! <b>Complexity</b>: Constant
    //!
    ...
    //!
    //! <b>Throws</b>: Nothing.
    static void link_after(const node_ptr& prev_node,
                           const node_ptr& this_node);

    //! <b>Requires</b>: b and e must be nodes of the same linear list or an
    //! empty range.
    //!   and p must be a node of a different linear list.
    //!
    //! <b>Effects</b>: Removes the nodes from (b, e] range from their linear
    //! list and inserts
    ...
    BOOST_INTRUSIVE_FORCEINLINE static void
        init_header(const node_ptr& this_node)
    {
        NodeTraits::set_next(this_node, node_ptr());
    }

    //! <b>Requires</b>: this_node and prev_init_node must be in the same linear
    //! list.
    //!
    //! <b>Effects</b>: Returns the previous node of this_node in the linear
    //! list starting.
    //!   the search from prev_init_node. The first node checked for equality
    //!   is NodeTraits::get_next(prev_init_node).
    //!
    //! <b>Complexity</b>: Linear to the number of elements between
    //! prev_init_node and this_node.
    //!
    //! <b>Throws</b>: Nothing.
    BOOST_INTRUSIVE_FORCEINLINE static node_ptr
        get_previous_node(const node_ptr& prev_init_node,
                          const node_ptr& this_node)
    {
        return base_t::get_previous_node(prev_init_node, this_node);
    }

    //! <b>Requires</b>: this_node must be in a linear list or be an empty
    //! linear list.
    //!
    //! <b>Effects</b>: Returns the number of nodes in a linear list. If the
    //! linear list
    //!  is empty, returns 1.
    //!
    ...
    //!
    //! <b>Effects</b>: Moves all the nodes previously chained after this_node
    //! after other_node
    //!   and vice-versa.
    //!
    //! <b>Complexity</b>: Constant
    ...
    //! <b>Effects</b>: Moves the first n nodes starting at p to the end of the
    //! list.
    //!
    //! <b>Returns</b>: A pair containing the new first and last node of the
    //! list or
    //!   if there has been any movement, a null pair if n leads to no movement.
    //!
    //! <b>Throws</b>: Nothing.
    //!
    //! <b>Complexity</b>: Linear to the number of elements plus the number
    //! moved positions.
    static std::pair<node_ptr, node_ptr> move_first_n_backwards(node_ptr p,
                                                                std::size_t n)
    {
    ...
    //! <b>Effects</b>: Moves the first n nodes starting at p to the beginning
    //! of the list.
    //!
    //! <b>Returns</b>: A pair containing the new first and last node of the
    //! list or
    ...
    //! <b>Complexity</b>: Linear to the number of elements plus the number
    //! moved positions.
    static std::pair<node_ptr, node_ptr> move_first_n_forward(node_ptr p,
                                                              std::size_t n)
    {
        std::pair<node_ptr, node_ptr> ret;
        // Null shift, or count() == 0 or 1, nothing to do
        ...
        // If the shift count is less than the size of the list, we can also
        // obtain the position of the new last node after the shift.
        ...
        if (distance <= n)
        {
            ...
            // Shortcut the shift with the modulo of the size of the list
            std::size_t new_before_last_pos =
                (distance - (n % distance)) % distance;
            // If the shift is a multiple of the size there is nothing to do
            if (!new_before_last_pos)
                return ret;

            for (new_last = p; --new_before_last_pos;
                 new_last = node_traits::get_next(new_last))
            {
                // empty
            }
