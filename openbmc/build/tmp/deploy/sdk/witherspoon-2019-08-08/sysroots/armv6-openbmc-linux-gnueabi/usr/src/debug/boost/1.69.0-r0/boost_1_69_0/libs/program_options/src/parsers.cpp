...
woption woption_from_option(const option& opt)
{
    ...
    std::transform(opt.value.begin(), opt.value.end(),
                   back_inserter(result.value), boost::bind(from_utf8, _1));
...
basic_parsed_options<wchar_t>::basic_parsed_options(const parsed_options& po) :
    description(po.description), utf8_encoded_options(po),
...
basic_parsed_options<charT> parse_config_file(std::basic_istream<charT>& is,
    ...
    for (unsigned i = 0; i < options.size(); ++i)
    {
        ...
        if (d.long_name().empty())
            boost::throw_exception(
                error("abbreviated option names are not permitted in options "
                      "configuration files"));
...
template BOOST_PROGRAM_OPTIONS_DECL basic_parsed_options<char>
...
template <class charT>
basic_parsed_options<charT> parse_config_file(const char* filename,
                                              const options_description& desc,
                                              bool allow_unregistered)
{
    // Parser return char strings
...
template BOOST_PROGRAM_OPTIONS_DECL basic_parsed_options<char>
    parse_config_file(const char* filename, const options_description& desc,
                      bool allow_unregistered);
...
{
  public:
    prefix_name_mapper(const std::string& prefix) : prefix(prefix)
    {
    }
...
BOOST_PROGRAM_OPTIONS_DECL parsed_options parse_environment(
