...
BOOST_LOG_ANONYMOUS_NAMESPACE
{
    ...
    class writeable_named_scope_list : public named_scope_list
    {
        //! Base type
    ...
    class named_scope_value : public attribute_value::impl
    {
        //! Scope names stack
      ...
      public:
        ...
        //! The method dispatches the value to the given object. It returns true
        //! if the object was capable to consume the real attribute value type
        //! and false otherwise.
        ...
        {
            return typeindex::type_id<scope_stack>();
        }
...
struct BOOST_SYMBOL_VISIBLE named_scope::impl
    : public attribute::impl,
      public log::aux::singleton<impl, intrusive_ptr<impl>>
{
    //! Singleton base type
    typedef log::aux::singleton<impl, intrusive_ptr<impl>> singleton_base_type;
...
#if defined(BOOST_LOG_USE_COMPILER_TLS)
//! Cached pointer to the thread-specific scope stack
BOOST_LOG_TLS named_scope::impl::scope_list* named_scope::impl::pScopesCache =
    NULL;
...
BOOST_LOG_API named_scope_list::named_scope_list(named_scope_list const& that) :
    ...
    m_Size(that.size()), m_fNeedToDeallocate(!that.empty())
{
    if (m_Size > 0)
    {
        ...
        for (const_iterator src = that.begin(), end = that.end(); src != end;
             ++src, ++p)
        {
...
BOOST_LOG_API named_scope_list::~named_scope_list()
{
    if (m_fNeedToDeallocate)
    {
        ...
        allocator_type::deallocate(static_cast<pointer>(m_RootNode._m_pNext),
                                   m_Size);
    }
}
...
BOOST_LOG_API void named_scope_list::swap(named_scope_list& that)
{
    if (!this->empty())
    {
        if (!that.empty())
        {
            ...
            std::swap(m_RootNode._m_pNext->_m_pPrev,
                      that.m_RootNode._m_pNext->_m_pPrev);
            std::swap(m_RootNode._m_pPrev->_m_pNext,
                      that.m_RootNode._m_pPrev->_m_pNext);
        ...
        {
            // this is not empty
            m_RootNode._m_pNext->_m_pPrev = m_RootNode._m_pPrev->_m_pNext =
                &that.m_RootNode;
    ...
    else if (!that.empty())
    {
        // that is not empty
        that.m_RootNode._m_pNext->_m_pPrev =
            that.m_RootNode._m_pPrev->_m_pNext = &m_RootNode;
...
//! Constructor
named_scope::named_scope() : attribute(impl::instance)
{
}
