...
static int tty_find_device(struct console* console)
{
    ...
    rc = asprintf(&tty_class_device_link, "/sys/class/tty/%s", tty_kname_real);
    if (rc < 0)
        goto out_free;
...
static void tty_change_baudrate(struct console* console)
{
    ...
    for (i = 0; i < console->n_handlers; i++)
    {
        ...
        if (rc)
            warnx("Can't set terminal baudrate for handler %s", handler->name);
    }
}
...
static int tty_init_io(struct console* console)
{
    ...
    if (console->tty_lpc_addr)
        tty_set_sysfs_attr(console, "lpc_address", console->tty_lpc_addr);
...
struct ringbuffer_consumer* console_ringbuffer_consumer_register(
    struct console* console, ringbuffer_poll_fn_t poll_fn, void* data)
{
...
int main(int argc, char** argv)
{
    ...
    console->pollfds = calloc(MAX_INTERNAL_POLLFD, sizeof(*console->pollfds));
