...
typedef struct netsnmp_tdata_row_s
{
    ...
    netsnmp_variable_list*
        indexes; /* stored permanently if store_indexes = 1 */
...
void netsnmp_tdata_replace_row(netsnmp_tdata* table, netsnmp_tdata_row* origrow,
...
#define netsnmp_tdata_add_index(thetable, type)                                \
    snmp_varlist_add_variable(&thetable->indexes_template, NULL, 0, type,      \
                              NULL, 0)
#define netsnmp_tdata_row_add_index(row, type, value, value_len)               \
    snmp_varlist_add_variable(&row->indexes, NULL, 0, type,                    \
                              (const u_char*)value, value_len)
