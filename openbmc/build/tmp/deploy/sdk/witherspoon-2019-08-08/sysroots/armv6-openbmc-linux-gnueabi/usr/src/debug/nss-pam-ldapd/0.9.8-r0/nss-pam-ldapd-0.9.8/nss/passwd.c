...
static nss_status_t read_passwd(TFILE* fp, struct passwd* result, char* buffer,
...
{
    NSS_GETONE(NSLCD_ACTION_PASSWD_BYNAME, WRITE_STRING(fp, name),
...
nss_status_t NSS_NAME(getpwuid_r)(uid_t uid, struct passwd* result,
                                  char* buffer, size_t buflen, int* errnop)
{
    NSS_GETONE(NSLCD_ACTION_PASSWD_BYUID, WRITE_INT32(fp, uid),
               read_passwd(fp, result, buffer, buflen, errnop));
}
...
{
    int res;
    res = snprintf(buffer, buflen, "%s:%s:%d:%d:%s:%s:%s", result->pw_name,
                   result->pw_passwd, (int)result->pw_uid, (int)result->pw_gid,
...
static nss_status_t passwd_getpwuid(nss_backend_t UNUSED(*be), void* args)
{
    NSS_GETONE(NSLCD_ACTION_PASSWD_BYUID,
               WRITE_INT32(fp, NSS_ARGS(args)->key.uid), read_result(fp, args));
}
...
static nss_backend_op_t passwd_ops[] = {nss_ldap_destructor, passwd_endpwent,
                                        passwd_setpwent,     passwd_getpwent,
                                        passwd_getpwnam,     passwd_getpwuid};
