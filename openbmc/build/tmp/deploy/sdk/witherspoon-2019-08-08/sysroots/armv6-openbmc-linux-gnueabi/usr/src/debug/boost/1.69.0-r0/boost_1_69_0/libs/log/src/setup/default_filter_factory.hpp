...
struct predicate_wrapper
{
    ...
    explicit predicate_wrapper(attribute_name const& name,
    ...
    {
        bool res = false;
        boost::log::visit<ValueT>(
            m_name, arg,
            save_result_wrapper<PredicateT const&, bool>(m_visitor, res));
        return res;
    }
...
//! The default filter factory that supports creating filters for the standard
//! types (see utility/type_dispatch/standard_types.hpp)
template <typename CharT>
class default_filter_factory : public filter_factory<CharT>
{
  private:
    //! Base type
  ...
  private:
    //! The function parses the argument value for a binary relation and
    //! constructs the corresponding filter
    template <typename RelationT>
    static filter parse_argument(attribute_name const& name,
                                 string_type const& arg);
};
