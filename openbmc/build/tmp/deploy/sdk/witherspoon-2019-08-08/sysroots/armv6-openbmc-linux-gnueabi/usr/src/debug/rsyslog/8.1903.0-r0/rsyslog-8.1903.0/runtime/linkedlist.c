 ...
 */
rsRetVal llGetNextElt(linkedList_t* pThis, linkedListCookie_t* ppElt,
                      void** ppUsr)
{
    llElt_t* pElt;
    DEFiRet;
...
/* append a user element to the end of the linked list. This includes setting a
 * key. If no key is desired, simply pass in a NULL pointer for it.
 */
 ...
 */
static rsRetVal llUnlinkElt(linkedList_t* pThis, llElt_t* pElt,
                            llElt_t* pEltPrev)
{
    assert(pElt != NULL);
 ...
 * rgerhards, 2007-11-21
 */
static rsRetVal llUnlinkAndDelteElt(linkedList_t* pThis, llElt_t* pElt,
                                    llElt_t* pEltPrev)
{
    DEFiRet;
 ...
 * rgerhards, 2007-11-21
 */
static rsRetVal llFindElt(linkedList_t* pThis, void* pKey, llElt_t** ppElt,
                          llElt_t** ppEltPrev)
{
    DEFiRet;
    llElt_t* pElt;
 ...
 * If the called user function returns RS_RET_OK_DELETE_LISTENTRY the current
 * element is deleted.
 */
rsRetVal llExecFunc(linkedList_t* pThis, rsRetVal (*pFunc)(void*, void*),
                    void* pParam)
{
    DEFiRet;
    rsRetVal iRetLL;
    ...
    linkedListCookie_t llCookiePrev =
        NULL; /* previous list element (needed for deletion, NULL = at root) */

    assert(pThis != NULL);
    assert(pFunc != NULL);

    while ((iRetLL = llGetNextElt(pThis, &llCookie, (void**)&pData)) ==
           RS_RET_OK)
    {
        ...
        if (iRet == RS_RET_OK_DELETE_LISTENTRY)
        {
            ...
            /* we need to revert back, as we have just deleted the current
             * element. So the actual current element is the one before it,
             * which happens to be stored in llCookiePrev. -- rgerhards,
             * 2007-11-21
             */
