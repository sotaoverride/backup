...
static void g_inet_socket_address_connectable_iface_init(
    GSocketConnectableIface* iface);
static gchar* g_inet_socket_address_connectable_to_string(
    GSocketConnectable* connectable);

G_DEFINE_TYPE_WITH_CODE(
    GInetSocketAddress, g_inet_socket_address, G_TYPE_SOCKET_ADDRESS,
...
static void g_inet_socket_address_get_property(GObject* object, guint prop_id,
                                               GValue* value, GParamSpec* pspec)
{
    ...
    switch (prop_id)
    {
        ...
        case PROP_FLOWINFO:
            g_return_if_fail(
                g_inet_address_get_family(address->priv->address) ==
...
static void g_inet_socket_address_set_property(GObject* object, guint prop_id,
                                               const GValue* value,
                                               GParamSpec* pspec)
{
    GInetSocketAddress* address = G_INET_SOCKET_ADDRESS(object);
...
static gboolean g_inet_socket_address_to_native(GSocketAddress* address,
                                                ...
                                                GError** error)
{
    ...
    if (family == AF_INET)
    {
        ...
        memcpy(&(sock->sin_addr.s_addr),
               g_inet_address_to_bytes(addr->priv->address),
    ...
    else if (family == AF_INET6)
    {
        ...
        memcpy(&(sock->sin6_addr.s6_addr),
               g_inet_address_to_bytes(addr->priv->address),
               sizeof(sock->sin6_addr));
        return TRUE;
    }
    else
    {
...
static void g_inet_socket_address_class_init(GInetSocketAddressClass* klass)
{
    ...
    g_object_class_install_property(
        ...
        g_param_spec_object("address", P_("Address"), P_("The address"),
                            G_TYPE_INET_ADDRESS,
    ...
    g_object_class_install_property(
        ...
        g_param_spec_uint("port", P_("Port"), P_("The port"), 0, 65535, 0,
                          G_PARAM_CONSTRUCT_ONLY | G_PARAM_READWRITE |
                              G_PARAM_STATIC_STRINGS));
     ...
     */
    g_object_class_install_property(
        gobject_class, PROP_FLOWINFO,
        g_param_spec_uint("flowinfo", P_("Flow info"), P_("IPv6 flow info"), 0,
                          G_MAXUINT32, 0,
                          G_PARAM_CONSTRUCT_ONLY | G_PARAM_READWRITE |
                              G_PARAM_STATIC_STRINGS));
     ...
     */
    g_object_class_install_property(
        gobject_class, PROP_SCOPE_ID,
        g_param_spec_uint("scope-id", P_("Scope ID"), P_("IPv6 scope ID"), 0,
                          G_MAXUINT32, 0,
                          G_PARAM_CONSTRUCT_ONLY | G_PARAM_READWRITE |
                              G_PARAM_STATIC_STRINGS));
}
...
{
    return g_object_new(G_TYPE_INET_SOCKET_ADDRESS, "address", address, "port",
                        port, NULL);
}
                                                      ...
                                                      guint port)
{
    ...
    if (strchr(address, ':'))
    {
            ...
            res->ai_addrlen == sizeof(struct sockaddr_in6))
        {
            ...
            saddr =
                g_socket_address_new_from_native(res->ai_addr, res->ai_addrlen);
        }
        else
            saddr = NULL;
    ...
    else
    {
        ...
        g_warn_if_fail(g_inet_address_get_family(iaddr) ==
...
{
    g_return_val_if_fail(G_IS_INET_SOCKET_ADDRESS(address), 0);
    g_return_val_if_fail(g_inet_address_get_family(address->priv->address) ==
                             G_SOCKET_FAMILY_IPV6,
                         0);
