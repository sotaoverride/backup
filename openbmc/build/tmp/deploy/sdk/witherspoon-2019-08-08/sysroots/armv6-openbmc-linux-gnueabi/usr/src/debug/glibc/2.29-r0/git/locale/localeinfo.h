...
#define _NL_CURRENT_DEFINE(category)                                           \
    ...
    asm(".globl " __SYMBOL_PREFIX "_nl_current_" #category                     \
...
{
    return (wchar_t*)l->__locales[category]->values[_NL_ITEM_INDEX(item)].wstr;
}
...
extern struct __locale_data*
    _nl_find_locale(const char* locale_path, size_t locale_path_len,
                    int category, const char** name) attribute_hidden;

/* Try to load the file described by FILE.  */
extern void _nl_load_locale(struct loaded_l10nfile* file,
                            int category) attribute_hidden;

/* Free all resource.  */
extern void _nl_unload_locale(struct __locale_data* locale) attribute_hidden;

/* Free the locale and give back all memory if the usage count is one.  */
extern void _nl_remove_locale(int locale,
                              struct __locale_data* data) attribute_hidden;
...
extern struct __locale_data*
    _nl_load_locale_from_archive(int category,
                                 const char** namep) attribute_hidden;
...
extern struct __locale_data*
    _nl_intern_locale_data(int category, const void* data,
                           size_t datasize) attribute_hidden;

/* Return `era' entry which corresponds to TP.  Used in strftime.  */
extern struct era_entry*
    _nl_get_era_entry(const struct tm* tp,
                      struct __locale_data* lc_time) attribute_hidden;
