                                       ...
                                       const char* keyword, int condition)
{
    if (!condition)
    {
        log_log(LOG_ERR, "%s:%d: %s: wrong number of arguments", filename, lnr,
                keyword);
        exit(EXIT_FAILURE);
    }
}
...
static char* get_strdup(const char* filename, int lnr, const char* keyword,
                        char** line)
{
...
static char* get_linedup(const char* filename, int lnr, const char* keyword,
                         char** line)
{
    char* var;
    check_argumentcount(filename, lnr, keyword,
...
static void get_eol(const char* filename, int lnr, const char* keyword,
                    char** line)
{
    if ((line != NULL) && (*line != NULL) && (**line != '\0'))
    {
        log_log(LOG_ERR, "%s:%d: %s: too many arguments", filename, lnr,
                keyword);
        exit(EXIT_FAILURE);
    }
}

static int get_int(const char* filename, int lnr, const char* keyword,
                   char** line)
{
    char token[32];
    check_argumentcount(filename, lnr, keyword,
...
static int parse_boolean(const char* filename, int lnr, const char* value)
{
    ...
    {
        log_log(LOG_ERR, "%s:%d: not a boolean argument: '%s'", filename, lnr,
                value);
        exit(EXIT_FAILURE);
    }
}

static int get_boolean(const char* filename, int lnr, const char* keyword,
                       char** line)
{
    char token[32];
    check_argumentcount(filename, lnr, keyword,
...
static time_t parse_time(const char* filename, int lnr, const char* value)
{
    ...
    if (errno != 0)
    {
        log_log(LOG_ERR, "%s:%d: value out of range: '%s'", filename, lnr,
                value);
        exit(EXIT_FAILURE);
    }
    ...
    {
        log_log(LOG_ERR, "%s:%d: invalid time value: '%s'", filename, lnr,
                value);
        exit(EXIT_FAILURE);
    }
}

static time_t get_time(const char* filename, int lnr, const char* keyword,
                       char** line)
{
    char token[32];
    check_argumentcount(filename, lnr, keyword,
...
static void handle_uid(const char* filename, int lnr, const char* keyword,
                       char* line, struct ldap_config* cfg)
{
    ...
    /* log an error */
    log_log(LOG_ERR, "%s:%d: %s: not a valid uid: '%s'", filename, lnr, keyword,
            token);
    exit(EXIT_FAILURE);
}

static void handle_gid(const char* filename, int lnr, const char* keyword,
                       char* line, gid_t* gid)
{
    char token[32];
    struct group* grent;
    ...
    *gid = strtogid(token, &tmp, 10);
    ...
    /* log an error */
    log_log(LOG_ERR, "%s:%d: %s: not a valid gid: '%s'", filename, lnr, keyword,
            token);
    exit(EXIT_FAILURE);
}
...
static int parse_loglevel(const char* filename, int lnr, const char* value)
{
    ...
    {
        log_log(LOG_ERR, "%s:%d: not a log level '%s'", filename, lnr, value);
        exit(EXIT_FAILURE);
    }
}

static void handle_log(const char* filename, int lnr, const char* keyword,
                       char* line)
{
    int level = LOG_INFO;
    char scheme[64];
    ...
    {
        log_log(LOG_ERR, "%s:%d: %s: invalid argument '%s'", filename, lnr,
                keyword, scheme);
        exit(EXIT_FAILURE);
    }
}
...
static void add_uri(const char* filename, int lnr, struct ldap_config* cfg,
                    const char* uri)
{
    int i;
    ...
    {
        log_log(LOG_ERR, "%s:%d: maximum number of URIs exceeded", filename,
                lnr);
        exit(EXIT_FAILURE);
    }
    /* append URI to list */
...
static const char* cfg_getdomainname(const char* filename, int lnr)
{
    ...
    fqdn = getfqdn();
    if ((fqdn != NULL) && ((domain = strchr(fqdn, '.')) != NULL) &&
        (domain[1] != '\0'))
        return domain + 1;
    log_log(LOG_ERR, "%s:%d: unable to determinate a domain name", filename,
            lnr);
    exit(EXIT_FAILURE);
}
                              ...
                              struct ldap_config* cfg, const char* domain)
{
    ...
    if (rc != LDAP_SUCCESS)
    {
        log_log(LOG_ERR, "%s:%d: no servers found in DNS zone %s: %s", filename,
                lnr, domain, ldap_err2string(rc));
        exit(EXIT_FAILURE);
    }
    if ((hostlist == NULL) || (*hostlist == '\0'))
    {
        log_log(LOG_ERR, "%s:%d: no servers found in DNS zone %s", filename,
                lnr, domain);
        exit(EXIT_FAILURE);
    }
    ...
    while (hostlist != NULL)
    {
        ...
        if ((strlen(hostlist) > 4) &&
            (strcmp(hostlist + strlen(hostlist) - 4, ":636") == 0))
        {
            ...
            if (mysnprintf(buf, sizeof(buf), "ldaps://%s", hostlist))
            {
                log_log(
                    LOG_ERR,
                    "add_uris_from_dns(): buf buffer too small (%lu required)",
        ...
        else
        {
            ...
            if ((strlen(hostlist) > 4) &&
                (strcmp(hostlist + strlen(hostlist) - 4, ":389") == 0))
                hostlist[strlen(hostlist) - 4] = '\0';
...
static void check_permissions(const char* filename, const char* keyword)
{
    ...
    if ((sb.st_mode & 0007) != 0)
    {
        if (keyword != NULL)
            log_log(LOG_ERR,
                    "%s: file should not be world readable if %s is set",
                    filename, keyword);
        else
...
static void check_readable(const char* filename, int lnr, const char* keyword,
                           const char* path)
{
    if (access(path, R_OK) != 0)
    {
        log_log(LOG_ERR, "%s:%d: %s: error accessing %s: %s", filename, lnr,
                keyword, path, strerror(errno));
        exit(EXIT_FAILURE);
    }
}

/* check whether the specified path is a directory */
static void check_dir(const char* filename, int lnr, const char* keyword,
                      const char* path)
{
    struct stat sb;
    if (stat(path, &sb))
    {
        log_log(LOG_ERR, "%s:%d: %s: cannot stat() %s: %s", filename, lnr,
                keyword, path, strerror(errno));
        exit(EXIT_FAILURE);
    }
    if (!S_ISDIR(sb.st_mode))
    {
        log_log(LOG_ERR, "%s:%d: %s: %s is not a directory", filename, lnr,
                keyword, path);
        exit(EXIT_FAILURE);
    }
}
...
static enum ldap_map_selector parse_map(const char* value)
{
    if ((strcasecmp(value, "alias") == 0) ||
        (strcasecmp(value, "aliases") == 0))
        return LM_ALIASES;
    else if ((strcasecmp(value, "ether") == 0) ||
             (strcasecmp(value, "ethers") == 0))
        return LM_ETHERS;
    else if (strcasecmp(value, "group") == 0)
        return LM_GROUP;
    else if ((strcasecmp(value, "host") == 0) ||
             (strcasecmp(value, "hosts") == 0))
        return LM_HOSTS;
...
static void handle_base(const char* filename, int lnr, const char* keyword,
                        char* line, struct ldap_config* cfg)
{
    const char** bases;
    int i;
    ...
    /* no free spot found */
    log_log(LOG_ERR,
            "%s:%d: maximum number of base options per map (%d) exceeded",
            filename, lnr, NSS_LDAP_CONFIG_MAX_BASES);
    exit(EXIT_FAILURE);
}

static void handle_scope(const char* filename, int lnr, const char* keyword,
                         char* line, struct ldap_config* cfg)
{
    char token[32];
    int* var;
    ...
    else if ((strcasecmp(token, "one") == 0) ||
             (strcasecmp(token, "onelevel") == 0))
    ...
    {
        log_log(LOG_ERR, "%s:%d: not a scope argument: '%s'", filename, lnr,
                token);
        exit(EXIT_FAILURE);
    }
}
...
static void handle_deref(const char* filename, int lnr, const char* keyword,
                         char* line, struct ldap_config* cfg)
{
    char token[32];
    check_argumentcount(filename, lnr, keyword,
...
static void handle_filter(const char* filename, int lnr, const char* keyword,
                          char* line)
{
    const char** var;
    const char* map = line;
...
static void handle_map(const char* filename, int lnr, const char* keyword,
                       char* line)
{
    enum ldap_map_selector map;
    const char** var;
    ...
    if (var == NULL)
    {
        log_log(LOG_ERR, "%s:%d: unknown attribute to map: '%s'", filename, lnr,
                oldatt);
        exit(EXIT_FAILURE);
    }
...
static void handle_nss_initgroups_ignoreusers(const char* filename, int lnr,
                                              const char* keyword, char* line,
                                              struct ldap_config* cfg)
{
    ...
    while (get_token(&line, token, sizeof(token)) != NULL)
    {
        ...
        {
            /* go over all users (this will work because nslcd is not yet
             * running) */
            setpwent();
                              ...
                              struct ldap_config* cfg)
{
    ...
    if ((i = regcomp(&cfg->validnames, value + 1, flags)) != 0)
    {
        ...
        value = malloc(l);
        if (value == NULL)
            log_log(LOG_ERR, "%s:%d: invalid regular expression", filename,
                    lnr);
        else
        {
            regerror(i, &cfg->validnames, value, l);
            log_log(LOG_ERR, "%s:%d: invalid regular expression: %s", filename,
                    lnr, value);
        }
        exit(EXIT_FAILURE);
    }
...
static void check_search_variables(const char* filename, int lnr,
                                   const char* expression)
{
    ...
    for (i = 0; list[i] != NULL; i++)
    {
        if ((strcmp(list[i], "username") != 0) &&
            ...
            (strcmp(list[i], "rhost") != 0) && (strcmp(list[i], "tty") != 0) &&
            (strcmp(list[i], "hostname") != 0) &&
            (strcmp(list[i], "fqdn") != 0) && (strcmp(list[i], "dn") != 0) &&
            (strcmp(list[i], "uid") != 0))
        {
            log_log(LOG_ERR, "%s:%d: unknown variable $%s", filename, lnr,
                    list[i]);
            exit(EXIT_FAILURE);
        }
    }
...
static void handle_pam_authc_search(const char* filename, int lnr,
                                    const char* keyword, char* line,
                                    struct ldap_config* cfg)
{
    check_argumentcount(filename, lnr, keyword,
...
static void handle_pam_authz_search(const char* filename, int lnr,
                                    const char* keyword, char* line,
                                    struct ldap_config* cfg)
{
    int i;
    ...
    if (i >= NSS_LDAP_CONFIG_MAX_AUTHZ_SEARCHES)
    {
        log_log(
            LOG_ERR,
            "%s:%d: maximum number of pam_authz_search options (%d) exceeded",
...
static void handle_pam_password_prohibit_message(const char* filename, int lnr,
                                                 const char* keyword,
                                                 char* line,
                                                 struct ldap_config* cfg)
{
...
static void handle_reconnect_invalidate(const char* filename, int lnr,
                                        const char* keyword, char* line,
                                        struct ldap_config* cfg)
{
    char token[MAX_LINE_LENGTH];
    ...
    while (get_token(&line, token, sizeof(token)) != NULL)
    {
        ...
        while (*next != '\0')
        {
            ...
            if (map == LM_NONE)
            {
                log_log(LOG_ERR, "%s:%d: unknown map: '%s'", filename, lnr,
                        name);
                exit(EXIT_FAILURE);
            }
...
static void handle_cache(const char* filename, int lnr, const char* keyword,
                         char* line, struct ldap_config* cfg)
{
    char cache[16];
...
static MUST_USE char* get_base_from_rootdse(void)
{
    ...
    for (i = 0; (entry = myldap_get_entry(search, &rc)) != NULL; i++)
    {
        ...
        if ((values != NULL) && (values[0] != NULL))
        {
            ...
            log_log(LOG_DEBUG,
                    "get_basedn_from_rootdse(): found attribute "
        ...
        if ((values != NULL) && (values[0] != NULL))
        {
            base = xstrdup(values[0]);
            log_log(LOG_DEBUG,
                    "get_basedn_from_rootdse(): found attribute namingContexts "
                    "with value %s",
                    values[0]);
            break;
        }
...
static void cfg_read(const char* filename, struct ldap_config* cfg)
{
    ...
    {
        log_log(LOG_ERR, "cannot open config file (%s): %s", filename,
                strerror(errno));
        exit(EXIT_FAILURE);
    }
    ...
    while (fgets(linebuf, sizeof(linebuf), fp) != NULL)
    {
        ...
        {
            log_log(LOG_ERR,
                    "%s:%d: line too long or last line missing newline",
                    filename, lnr);
            exit(EXIT_FAILURE);
        }
        line[i - 1] = '\0';
        ...
        else if (strcasecmp(keyword, "uri") == 0)
        {
            ...
            while (get_token(&line, token, sizeof(token)) != NULL)
            {
                if (strcasecmp(token, "dns") == 0)
                {
                    ...
                    log_log(LOG_ERR,
                            "%s:%d: value %s not supported on platform",
                            filename, lnr, token);
                    exit(EXIT_FAILURE);
        ...
        else if (strcasecmp(keyword, "tls_cacertdir") == 0)
        {
            ...
            log_log(LOG_DEBUG,
                    "ldap_set_option(LDAP_OPT_X_TLS_CACERTDIR,\"%s\")", value);
                 ...
                 (strcasecmp(keyword, "tls_cacert") == 0))
        {
            ...
            log_log(LOG_DEBUG,
                    "ldap_set_option(LDAP_OPT_X_TLS_CACERTFILE,\"%s\")", value);
        ...
        else if (strcasecmp(keyword, "tls_randfile") == 0)
        {
            ...
            check_readable(filename, lnr, keyword, value);
            log_log(LOG_DEBUG,
                    "ldap_set_option(LDAP_OPT_X_TLS_RANDOM_FILE,\"%s\")",
                    value);
        ...
        else if (strcasecmp(keyword, "tls_cert") == 0)
        {
            ...
            check_readable(filename, lnr, keyword, value);
            log_log(LOG_DEBUG,
                    "ldap_set_option(LDAP_OPT_X_TLS_CERTFILE,\"%s\")", value);
        ...
        {
            cfg->nss_getgrent_skipmembers =
                get_boolean(filename, lnr, keyword, &line);
            get_eol(filename, lnr, keyword, &line);
        }
        else if (strcasecmp(keyword, "nss_disable_enumeration") == 0)
        {
            cfg->nss_disable_enumeration =
                get_boolean(filename, lnr, keyword, &line);
            get_eol(filename, lnr, keyword, &line);
        }
        ...
        {
            handle_pam_password_prohibit_message(filename, lnr, keyword, line,
                                                 cfg);
        }
        ...
        {
            log_log(LOG_ERR, "%s:%d: unknown keyword: '%s'", filename, lnr,
                    keyword);
            exit(EXIT_FAILURE);
        }
#endif
...
static void bindpw_read(const char* filename, struct ldap_config* cfg)
{
    ...
    if ((fp = fopen(filename, "r")) == NULL)
    {
        ...
        {
            log_log(LOG_ERR, "cannot open bindpw file (%s): %s", filename,
                    strerror(errno));
            exit(EXIT_FAILURE);
        }
    }
...
static void cfg_dump(void)
{
    ...
    if (nslcd_cfg->sasl_canonicalize >= 0)
        log_log(LOG_DEBUG, "CFG: sasl_canonicalize %s",
                print_boolean(nslcd_cfg->sasl_canonicalize));
#endif /* LDAP_OPT_X_SASL_NOCANON */
    ...
    for (map = LM_ALIASES; map < LM_NONE; map++)
    {
        ...
        if (strp != NULL)
            for (i = 0; i < NSS_LDAP_CONFIG_MAX_BASES; i++)
                if (strp[i] != NULL)
                    log_log(LOG_DEBUG, "CFG: base %s %s", print_map(map),
                            strp[i]);
    }
    ...
    for (map = LM_ALIASES; map < LM_NONE; map++)
    {
        ...
        if ((scopep != NULL) && (*scopep != LDAP_SCOPE_DEFAULT))
            log_log(LOG_DEBUG, "CFG: scope %s %s", print_map(map),
                    print_scope(*scopep));
    }
    log_log(LOG_DEBUG, "CFG: deref %s", print_deref(nslcd_cfg->deref));
    log_log(LOG_DEBUG, "CFG: referrals %s",
            print_boolean(nslcd_cfg->referrals));
    for (map = LM_ALIASES; map < LM_NONE; map++)
    {
...
#define LOG_ATTMAP(map, mapl, att)                                             \
    if (strcmp(attmap_##mapl##_##att, __STRING(att)) != 0)                     \
        log_log(LOG_DEBUG, "CFG: map %s %s %s", print_map(map), __STRING(att), \
                attmap_##mapl##_##att);
    ...
    log_log(LOG_DEBUG, "CFG: reconnect_sleeptime %d",
            nslcd_cfg->reconnect_sleeptime);
    log_log(LOG_DEBUG, "CFG: reconnect_retrytime %d",
...
#define LOG_LDAP_OPT_STRING(cfg, option)                                       \
    ...
    log_log(LOG_DEBUG, "CFG: nss_min_uid %lu",
            (unsigned long int)nslcd_cfg->nss_min_uid);
    log_log(LOG_DEBUG, "CFG: nss_uid_offset %lu",
            (unsigned long int)nslcd_cfg->nss_uid_offset);
    log_log(LOG_DEBUG, "CFG: nss_gid_offset %lu",
            (unsigned long int)nslcd_cfg->nss_gid_offset);
    ...
    print_time(nslcd_cfg->cache_dn2uid_positive, buffer + (sizeof(buffer) / 2),
               sizeof(buffer) / 2);
    log_log(LOG_DEBUG, "CFG: cache dn2uid %s %s", buffer,
...
void cfg_init(const char* fname)
{
    ...
    if (nslcd_cfg->ssl == SSL_LDAPS)
    {
        for (i = 0; nslcd_cfg->uris[i].uri != NULL; i++)
        {
            if (strncasecmp(nslcd_cfg->uris[i].uri, "ldaps://", 8) != 0)
                log_log(LOG_WARNING,
                        "%s doesn't start with ldaps:// and \"ssl on\" is "
                        "specified",
    ...
    {
        log_log(LOG_ERR,
                "no base defined in config and couldn't get one from server");
        exit(EXIT_FAILURE);
    }
    /* dump configuration */
    cfg_dump();
