...
/*
 * Copyright (C) 2001,2002,2003,2004,2005,2006 by the Massachusetts Institute of
 * Technology, Cambridge, MA, USA.  All Rights Reserved.
 *
...
#ifdef S_SPLINT_S
/*@-incondefs@*/
extern int getaddrinfo(/*@in@*/ /*@null@*/ const char*,
                       /*@in@*/ /*@null@*/ const char*,
                       /*@in@*/ /*@null@*/ const struct addrinfo*,
                       /*@out@*/ struct addrinfo**);
extern void freeaddrinfo(/*@only@*/ /*@out@*/ struct addrinfo*);
...
#define GET_HOST_BY_ADDR(ADDR, ADDRLEN, FAMILY, HP, ERR, TMP)                  \
    {                                                                          \
        TMP = gethostbyaddr((ADDR), (ADDRLEN), (FAMILY));                      \
        (ERR) = h_errno;                                                       \
        (HP) = TMP;                                                            \
    }
#else
...
#define GET_HOST_BY_NAME(NAME, HP, ERR, TMP)                                   \
    {                                                                          \
        (HP) = (gethostbyname_r((NAME), &TMP.ent, &TMP.data) ? 0 : &TMP.ent);  \
        (ERR) = h_errno;                                                       \
    }
/*
...
#define GET_HOST_BY_NAME(NAME, HP, ERR, TMP)                                   \
    {                                                                          \
        ...
        int my_h_err, my_ret;                                                  \
        my_ret = gethostbyname_r((NAME), &TMP.ent, TMP.buf, sizeof(TMP.buf),   \
                                 &my_hp, &my_h_err);                           \
        (HP) = (((my_ret != 0) || (my_hp != &TMP.ent)) ? 0 : &TMP.ent);        \
        (ERR) = my_h_err;                                                      \
    }
#define GET_HOST_BY_ADDR(ADDR, ADDRLEN, FAMILY, HP, ERR, TMP)                  \
    {                                                                          \
        ...
        my_ret = gethostbyaddr_r((ADDR), (ADDRLEN), (FAMILY), &TMP.ent,        \
                                 TMP.buf, sizeof(TMP.buf), &my_hp, &my_h_err); \
        (HP) = (((my_ret != 0) || (my_hp != &TMP.ent)) ? 0 : &TMP.ent);        \
        (ERR) = my_h_err;                                                      \
    }
#else
...
#define GET_HOST_BY_NAME(NAME, HP, ERR, TMP)                                   \
    {                                                                          \
        int my_h_err;                                                          \
        (HP) = gethostbyname_r((NAME), &TMP.ent, TMP.buf, sizeof(TMP.buf),     \
                               &my_h_err);                                     \
        (ERR) = my_h_err;                                                      \
    }
#define GET_HOST_BY_ADDR(ADDR, ADDRLEN, FAMILY, HP, ERR, TMP)                  \
    {                                                                          \
        int my_h_err;                                                          \
        (HP) = gethostbyaddr_r((ADDR), (ADDRLEN), (FAMILY), &TMP.ent, TMP.buf, \
                               sizeof(TMP.buf), &my_h_err);                    \
        (ERR) = my_h_err;                                                      \
    }
#endif /* returns int? */
#endif /* _AIX */
...
#define GET_SERV_BY_PORT(PORT, PROTO, SP, ERR, TMP)                            \
    {                                                                          \
        struct servent* my_sp;                                                 \
        int my_s_err;                                                          \
        (SP) = (getservbyport_r((PORT), (PROTO), &TMP.ent, TMP.buf,            \
                                sizeof(TMP.buf), &my_sp, &my_s_err)            \
                    ? 0                                                        \
                    : &TMP.ent);                                               \
        (ERR) = my_s_err;                                                      \
...
#define GET_SERV_BY_PORT(PORT, PROTO, SP, ERR, TMP)                            \
    {                                                                          \
        struct servent* my_sp;                                                 \
        my_sp = getservbyport_r((PORT), (PROTO), &TMP.ent, TMP.buf,            \
                                sizeof(TMP.buf));                              \
        (SP) = my_sp;                                                          \
        (ERR) = my_sp == 0;                                                    \
...
static inline int system_getaddrinfo(const char* name, const char* serv,
...
static inline void system_freeaddrinfo(struct addrinfo* ai)
{
    freeaddrinfo(ai);
}
                                        ...
                                        const struct addrinfo* hint)
{
    ...
    if (hint)
    {
        ...
        sep = "";
#define Z(FLAG)                                                                \
    if (hint->ai_flags & AI_##FLAG)                                            \
    fprintf(stderr, "%s%s", sep, #FLAG), sep = "|"
        Z(CANONNAME);
        Z(PASSIVE);
        ...
        if (hint->ai_family)
            fprintf(stderr, " %s",
                    familyname(hint->ai_family, buf, sizeof(buf)));
        if (hint->ai_socktype)
            fprintf(stderr, " SOCK_%s",
                    socktypename(hint->ai_socktype, buf, sizeof(buf)));
        if (hint->ai_protocol)
            fprintf(stderr, " IPPROTO_%s",
                    protoname(hint->ai_protocol, buf, sizeof(buf)));
    }
    else
        fprintf(stderr, "(null)");
    fprintf(stderr, " }):\n");
...
static void debug_dump_addrinfos(const struct addrinfo* ai)
{
    ...
    while (ai)
    {
        ...
        fprintf(stderr, " socktype=%s",
                socktypename(ai->ai_socktype, buf, sizeof(buf)));
        fprintf(stderr, " ai_family=%s",
                familyname(ai->ai_family, buf, sizeof(buf)));
...
static int getaddrinfo(const char* name, const char* serv,
...
#if !defined(HAVE_GETADDRINFO)
static int getnameinfo(const struct sockaddr* addr, socklen_t len, char* host,
                       socklen_t hostlen, char* service, socklen_t servicelen,
                       int flags);
#endif
...
static inline int fai_add_entry(struct addrinfo** result, void* addr, int port,
                                const struct addrinfo* template)
{
    struct addrinfo* n = malloc(sizeof(struct addrinfo));
    if (n == 0)
        return EAI_MEMORY;
    if (template->ai_family != AF_INET && template->ai_family != AF_INET6)
        return EAI_FAMILY;
    *n = *template;
...
static void plant_face(const char* name, struct face* entry)
{
    ...
    printf("added cache entry '%s' at %p: %d ipv4, %d ipv6; expire %d\n", name,
           entry, entry->naddrs4, entry->naddrs6, entry->expiration);
#endif
}
...
static inline int fai_add_hosts_by_name(const char* name,
                                        struct addrinfo* template, int portnum,
                                        int flags, struct addrinfo** result)
{
#ifdef FAI_CACHE
    ...
    if (!find_face(name, &ce))
    {
        ...
        for (ai2 = ai, i4 = i6 = 0; ai2; ai2 = ai2->ai_next)
        {
            switch (ai2->ai_family)
            {
                case AF_INET:
                    ce->addrs4[i4++] =
                        ((struct sockaddr_in*)ai2->ai_addr)->sin_addr;
                    break;
                case AF_INET6:
                    ce->addrs6[i6++] =
                        ((struct sockaddr_in6*)ai2->ai_addr)->sin6_addr;
                    break;
                default:
                    break;
            }
    ...
    if (*result && (flags & AI_CANONNAME))
        (*result)->ai_canonname =
            (ce->canonname ? strdup(ce->canonname) : NULL);
    krb5int_unlock_fac();
    return 0;
...
static inline void fake_freeaddrinfo(struct addrinfo* ai)
{
...
static inline int fake_getaddrinfo(const char* name, const char* serv,
                                   const struct addrinfo* hint,
                                   struct addrinfo** result)
{
    ...
    if (serv)
    {
        ...
        try_service_lookup:
            GET_SERV_BY_NAME(serv, socktype == SOCK_STREAM ? "tcp" : "udp", sp,
                             s_err, stmp);
            if (sp == 0)
            {
                if (try_dgram_too)
    ...
    {
        ret = fai_add_hosts_by_name(name, &template, port, flags, &res);
    }
...
static inline int fake_getnameinfo(const struct sockaddr* sa, socklen_t len,
                                   char* host, socklen_t hostlen, char* service,
                                   socklen_t servicelen, int flags)
{
    ...
    if (host)
    {
        ...
        numeric_host:
            ...
            snprintf(tmpbuf, sizeof(tmpbuf), "%d.%d.%d.%d", uc[0], uc[1], uc[2],
                     uc[3]);
        ...
        else
        {
            ...
            GET_HOST_BY_ADDR((const char*)&sinp->sin_addr,
                             sizeof(struct in_addr), sa->sa_family, hp, herr,
                             htmp);
            if (hp == 0)
            {
    ...
    if (service)
    {
        ...
        else
        {
            ...
            GET_SERV_BY_PORT(sinp->sin_port, (flags & NI_DGRAM) ? "udp" : "tcp",
                             sp, serr, stmp);
            if (sp == 0)
...
static inline char* gai_strerror(int code)
{
    switch (code)
    {
        ...
        default:
            return "bogus getaddrinfo error?";
    }
}
#endif
...
#if defined(HAVE_FAKE_GETADDRINFO) || defined(FAI_CACHE)
static inline int getaddrinfo(const char* name, const char* serv,
                              const struct addrinfo* hint,
                              struct addrinfo** result)
{
...
#ifdef NEED_FAKE_GETNAMEINFO
static inline int getnameinfo(const struct sockaddr* sa, socklen_t len,
                              char* host, socklen_t hostlen, char* service,
                              socklen_t servicelen, int flags)
{
    return fake_getnameinfo(sa, len, host, hostlen, service, servicelen, flags);
}
...
int krb5int_getnameinfo(const struct sockaddr* sa, socklen_t salen, char* hbuf,
                        size_t hbuflen, char* sbuf, size_t sbuflen, int flags)
{
