...
/* Open the database.  */
enum nss_status CONCAT(_nss_db_set, ENTNAME)(int stayopen)
{
...
/* Close it again.  */
enum nss_status CONCAT(_nss_db_end, ENTNAME)(void)
{
    __libc_lock_lock(lock);
...
#define DB_LOOKUP(name, db_char, keysize, keypattern, break_if_match,          \
                  proto...)                                                    \
    enum nss_status _nss_db_get##name##_r(proto, struct STRUCTURE* result,     \
...
enum nss_status CONCAT(_nss_db_get, ENTNAME_r)(struct STRUCTURE* result,
                                               char* buffer, size_t buflen,
                                               int* errnop H_ERRNO_PROTO)
{
    ...
    if (state.header != MAP_FAILED)
    {
        const char* const end =
            ((const char*)state.header + state.header->valstroffset +
