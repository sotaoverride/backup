...
static uint32_t find_hardlink(struct filesystem_entry* e)
{
    ...
    while (*n)
    {
        ...
        if ((f->sb.st_dev < e->sb.st_dev) ||
            (f->sb.st_dev == e->sb.st_dev && f->sb.st_ino < e->sb.st_ino))
            n = &parent->rb_left;
...
static struct filesystem_entry*
    find_filesystem_entry(struct filesystem_entry* dir, char* fullname,
                          uint32_t type)
{
...
static struct filesystem_entry* add_host_filesystem_entry(
    const char* name, const char* path, unsigned long uid, unsigned long gid,
    unsigned long mode, dev_t rdev, struct filesystem_entry* parent)
{
    ...
    entry->sb.st_atime = entry->sb.st_ctime = entry->sb.st_mtime = timestamp;
    if (S_ISREG(mode))
    {
...
static struct filesystem_entry*
    recursive_add_host_directory(struct filesystem_entry* parent,
                                 const char* targetpath, const char* hostpath)
{
    int i, n;
    struct stat sb;
    char *hpath, *tpath;
    ...
    entry = add_host_filesystem_entry(targetpath, hostpath, sb.st_uid,
                                      sb.st_gid, sb.st_mode, 0, parent);
    ...
    for (i = 0; i < n; i++)
    {
        ...
        switch (sb.st_mode & S_IFMT)
        {
            ...
            case S_IFCHR:
            case S_IFBLK:
                add_host_filesystem_entry(tpath, hpath, sb.st_uid, sb.st_gid,
                                          sb.st_mode, sb.st_rdev, entry);
                break;
...
/*  device table entries take the form of:
   ...
   <count> /dev/mem     c    640       0       0         1       1       0     0
   -
...
static int interpret_table_entry(struct filesystem_entry* root, char* line)
{
    ...
    else
    {
        ...
        if (parent == NULL)
        {
            errmsg("skipping device_table entry '%s': no parent directory!",
                   name);
            free(name);
            free(hostpath);
            return 1;
            ...
            case 'b':
                if (count > 0)
                {
                    ...
                    for (i = start; i < (start + count); i++)
                    {
                        ...
                        add_host_filesystem_entry(dname, hpath, uid, gid, mode,
                                                  rdev, parent);
                        free(dname);
                        free(hpath);
                    }
                ...
                else
                {
                    ...
                    add_host_filesystem_entry(name, hostpath, uid, gid, mode,
                                              rdev, parent);
                }
                break;
            default:
...
static uint8_t* file_buffer =
    NULL; /* file buffer contains the actual erase block*/
...
static unsigned char ffbuf[16] = {0xff, 0xff, 0xff, 0xff, 0xff, 0xff,
...
static void write_dirent(struct filesystem_entry* e)
{
    ...
    rd.hdr_crc =
        cpu_to_je32(mtd_crc32(0, &rd, sizeof(struct jffs2_unknown_node) - 4));
...
static unsigned int write_regular_file(struct filesystem_entry* e)
{
    ...
    e->ino = ++ino;
    mkfs_debug_msg("writing file '%s'  ino=%lu  parent_ino=%lu", e->name,
                   (unsigned long)e->ino, (unsigned long)e->parent->ino);
    write_dirent(e);
    ...
    while ((len = read(fd, buf, page_size)))
    {
        ...
        while (len)
        {
            ...
            dsize = len;
            space =
                erase_block_size - (out_ofs % erase_block_size) - sizeof(ri);
            if (space > dsize)
                space = dsize;
            ...
            ri.hdr_crc = cpu_to_je32(
                mtd_crc32(0, &ri, sizeof(struct jffs2_unknown_node) - 4));
...
static void write_symlink(struct filesystem_entry* e)
{
    ...
    statbuf = &(e->sb);
    e->ino = ++ino;
    mkfs_debug_msg("writing symlink '%s'  ino=%lu  parent_ino=%lu", e->name,
                   (unsigned long)e->ino, (unsigned long)e->parent->ino);
    write_dirent(e);
    ...
    ri.hdr_crc =
        cpu_to_je32(mtd_crc32(0, &ri, sizeof(struct jffs2_unknown_node) - 4));
...
static void write_pipe(struct filesystem_entry* e)
{
    ...
    {
        mkfs_debug_msg("writing dir '%s'  ino=%lu  parent_ino=%lu", e->name,
                       (unsigned long)e->ino,
...
static void write_special_file(struct filesystem_entry* e)
{
    ...
    kdev =
        cpu_to_je16((major(statbuf->st_rdev) << 8) + minor(statbuf->st_rdev));
...
static struct
{
    ...
    {JFFS2_XPREFIX_ACL_ACCESS, POSIX_ACL_XATTR_ACCESS,
     POSIX_ACL_XATTR_ACCESS_LEN},
    {JFFS2_XPREFIX_ACL_DEFAULT, POSIX_ACL_XATTR_DEFAULT,
     POSIX_ACL_XATTR_DEFAULT_LEN},
...
static void formalize_posix_acl(void* xvalue, int* value_len)
{
    ...
    if (offset > *value_len)
    {
        printf("Length of JFFS2 ACL expression(%u) is longer than general "
               "one(%u).\n",
               offset, *value_len);
        exit(EXIT_FAILURE);
    }
...
static xattr_entry_t* create_xattr_entry(int xprefix, char* xname, char* xvalue,
                                         int value_len)
{
    xattr_entry_t* xe;
    ...
    rx.hdr_crc =
        cpu_to_je32(mtd_crc32(0, &rx, sizeof(struct jffs2_unknown_node) - 4));
    ...
    rx.data_crc =
        cpu_to_je32(mtd_crc32(0, xe->xname, xe->name_len + 1 + xe->value_len));
...
static xattr_entry_t* find_xattr_entry(int xprefix, char* xname, char* xvalue,
                                       int value_len)
{
    ...
    name_len = strlen(xname);
    index = (mtd_crc32(0, xname, name_len) ^ mtd_crc32(0, xvalue, value_len)) %
            XATTRENTRY_HASHSIZE;
    for (xe = xentry_hash[index]; xe; xe = xe->next)
    {
        if (xe->xprefix == xprefix && xe->value_len == value_len &&
            !strcmp(xe->xname, xname) && !memcmp(xe->xvalue, xvalue, value_len))
            break;
    }
    if (!xe)
    {
...
static void write_xattr_entry(struct filesystem_entry* e)
{
    ...
    for (offset = 0; offset < list_sz; offset += name_len)
    {
        ...
        ref.hdr_crc = cpu_to_je32(
            mtd_crc32(0, &ref, sizeof(struct jffs2_unknown_node) - 4));
                ...
                case S_IFDIR:
                    if (verbose)
                    {
                        printf("\td %04o %9lld             %5d:%-3d %s\n",
                               e->sb.st_mode & ~S_IFMT,
                               (long long)e->sb.st_size, (int)(e->sb.st_uid),
...
static void create_target_filesystem(struct filesystem_entry* root)
{
    ...
    cleanmarker.hdr_crc = cpu_to_je32(
        mtd_crc32(0, &cleanmarker, sizeof(struct jffs2_unknown_node) - 4));
    ...
    else
    {
        ...
        else
        {
            ...
            {
                full_write(out_fd, ffbuf,
                           min(sizeof(ffbuf), pad_fs_size - out_ofs));
            }
        }
    }
}

static struct option long_options[] = {{"pad", 2, NULL, 'p'},
                                       {"root", 1, NULL, 'r'},
                                       {"pagesize", 1, NULL, 's'},
                                       {"eraseblock", 1, NULL, 'e'},
                                       {"ignore", 1, NULL, 'I'},
...
static const char helptext[] =
    ...
    "  -r, -d, --root=DIR      Build file system from directory DIR (default: "
    "cwd)\n"
    "  -s, --pagesize=SIZE     Use page size (max data node size) SIZE.\n"
    "                          Set according to target system's memory "
    "management\n"
    ...
    "  -t, --test-compression  Call decompress and compare with the original "
    "(for test)\n"
    "  -n, --no-cleanmarkers   Don't add a cleanmarker to every eraseblock\n"
    "  -I, --ignore=PATH       Ignore sub directory and file tree below PATH "
    "when recursing over the file system\n"
    ...
    while (p < (file_buffer + inp_size))
    {
        ...
        switch (je16_to_cpu(node->u.nodetype))
        {
            ...
            case JFFS2_NODETYPE_INODE:
                if (verbose)
                    printf(
                        "%8s Inode      node at 0x%08zx, totlen 0x%08x, #ino  "
                        ...
                        je32_to_cpu(node->i.isize), je32_to_cpu(node->i.csize),
            ...
            case JFFS2_NODETYPE_DIRENT:
                ...
                if (verbose)
                    printf(
                        "%8s Dirent     node at 0x%08zx, totlen 0x%08x, #pino "
...
int main(int argc, char** argv)
{
                              ...
                              long_options, &c)) >= 0)
    {
        switch (opt)
        {
            ...
            case 'e':
            {
                ...
                {
                    fprintf(stderr,
                            "Erase size 0x%x too small. Increasing to 8KiB "
                            "minimum\n",
                            erase_block_size);
            ...
            case 'i':
                ...
                {
                    errmsg_die(
                        "(incremental) filename specified more than once");
                }
    ...
    if ((verbose) || (jffs2_compression_check_get() &&
