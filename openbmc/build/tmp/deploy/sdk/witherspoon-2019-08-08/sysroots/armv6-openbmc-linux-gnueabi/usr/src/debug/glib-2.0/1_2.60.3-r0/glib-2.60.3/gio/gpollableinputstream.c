...
G_DEFINE_INTERFACE(GPollableInputStream, g_pollable_input_stream,
                   G_TYPE_INPUT_STREAM)

static gboolean
    g_pollable_input_stream_default_can_poll(GPollableInputStream* stream);
static gssize g_pollable_input_stream_default_read_nonblocking(
                                               ...
                                               GCancellable* cancellable)
{
    ...
    return G_POLLABLE_INPUT_STREAM_GET_INTERFACE(stream)->create_source(
        stream, cancellable);
}
    ...
    GPollableInputStream* stream, void* buffer, gsize count, GError** error)
{
    ...
    return G_INPUT_STREAM_GET_CLASS(stream)->read_fn(
        G_INPUT_STREAM(stream), buffer, count, NULL, error);
}
                                                ...
                                                GError** error)
{
    ...
    res = G_POLLABLE_INPUT_STREAM_GET_INTERFACE(stream)->read_nonblocking(
