...
static void job_set_elapse_time(AvahiQueryScheduler* s, AvahiQueryJob* qj,
                                unsigned msec, unsigned jitter)
{
    ...
    if (qj->time_event)
        ...
        qj->time_event =
            avahi_time_event_new(s->time_event_queue, &tv, elapse_callback, qj);
}
...
static void* known_answer_walk_callback(AvahiCache* c, AvahiKey* pattern,
...
static int packet_add_query_job(AvahiQueryScheduler* s, AvahiDnsPacket* p,
                                AvahiQueryJob* qj)
{
    assert(s);
    assert(p);
    ...
    avahi_cache_walk(s->interface->cache, qj->key, known_answer_walk_callback,
                     s);
...
static void append_known_answers_and_send(AvahiQueryScheduler* s,
                                          AvahiDnsPacket* p)
{
    AvahiKnownAnswer* ka;
    unsigned n;
    ...
    while ((ka = s->known_answers))
    {
        ...
        AVAHI_LLIST_REMOVE(AvahiKnownAnswer, known_answer, s->known_answers,
                           ka);
...
int avahi_query_scheduler_post(AvahiQueryScheduler* s, AvahiKey* key,
                               int immediately, unsigned* ret_id)
{
    ...
    else
    {
        ...
        qj->time_event = avahi_time_event_new(
            s->time_event_queue, &qj->delivery, elapse_callback, qj);
    }
