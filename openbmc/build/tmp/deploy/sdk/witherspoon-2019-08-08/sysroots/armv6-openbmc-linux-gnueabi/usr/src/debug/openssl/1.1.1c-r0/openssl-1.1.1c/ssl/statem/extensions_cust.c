                     ...
                     size_t chainidx)
{
    ...
    if ((context & (SSL_EXT_TLS1_2_SERVER_HELLO | SSL_EXT_TLS1_3_SERVER_HELLO |
    ...
    for (i = 0; i < exts->meths_count; i++)
    {
        ...
        if ((context &
             (SSL_EXT_TLS1_2_SERVER_HELLO | SSL_EXT_TLS1_3_SERVER_HELLO |
              SSL_EXT_TLS1_3_ENCRYPTED_EXTENSIONS | SSL_EXT_TLS1_3_CERTIFICATE |
        ...
        {
            int cb_retval =
                meth->add_cb(s, meth->ext_type, context, &out, &outlen, x,
                             chainidx, &al, meth->add_arg);
                           ...
                           const custom_ext_methods* src)
{
    ...
    for (i = 0; i < src->meths_count; i++, methsrc++)
    {
        custom_ext_method* methdst =
            custom_ext_find(dst, methsrc->role, methsrc->ext_type, NULL);
...
int custom_exts_copy(custom_ext_methods* dst, const custom_ext_methods* src)
{
    ...
    {
        dst->meths =
            OPENSSL_memdup(src->meths, sizeof(*src->meths) * src->meths_count);
        if (dst->meths == NULL)
            return 0;
        ...
        for (i = 0; i < src->meths_count; i++)
        {
            ...
            methdst->parse_arg = OPENSSL_memdup(
                methsrc->parse_arg, sizeof(custom_ext_parse_cb_wrap));
...
static int
    add_custom_ext_intern(SSL_CTX* ctx, ENDPOINT role, unsigned int ext_type,
                          ...
                          SSL_custom_ext_parse_cb_ex parse_cb, void* parse_arg)
{
    ...
    if (ext_type == TLSEXT_TYPE_signed_certificate_timestamp &&
        (context & SSL_EXT_CLIENT_HELLO) != 0 && SSL_CTX_ct_is_enabled(ctx))
        return 0;
#endif
...
{
    custom_ext_add_cb_wrap* add_cb_wrap = OPENSSL_malloc(sizeof(*add_cb_wrap));
    custom_ext_parse_cb_wrap* parse_cb_wrap =
    ...
    ret = add_custom_ext_intern(ctx, role, ext_type, context,
