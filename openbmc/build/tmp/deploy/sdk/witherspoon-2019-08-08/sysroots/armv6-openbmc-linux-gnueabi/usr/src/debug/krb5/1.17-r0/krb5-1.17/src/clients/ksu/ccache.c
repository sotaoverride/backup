...
krb5_error_code krb5_store_all_creds(context, cc, creds_def,
                                     creds_other) krb5_context context;
krb5_ccache cc;
...
krb5_creds** creds_other;
{
    ...
    { /* both arrays have elements in them */
        ...
        /************   while(creds_other[i]){
                                ...
                                while(creds_def[j]){
                                   cmp =
        compare_creds(creds_other[i],creds_def[j]);
        ...
        cc, creds_other[i])){ return retval;
                                        }
                                }
                                i ++;
                        }
...
krb5_boolean compare_creds(context, cred1, cred2) krb5_context context;
krb5_creds* cred1;
krb5_creds* cred2;
{
    krb5_boolean retval;
...
krb5_error_code krb5_get_nonexp_tkts(context, cc,
                                     creds_array) krb5_context context;
krb5_ccache cc;
    ...
    while (!(retval = krb5_cc_next_cred(context, cc, &cur, &creds)))
    {
            ...
            (retval = krb5_check_exp(context, creds.times)))
        {
            ...
            if (auth_debug)
            {
                ...
                fputs("  Valid starting         Expires         Service "
                      "principal\n",
                      stdout);
        ...
        { /* these credentials didn't expire */
            ...
            if ((retval = krb5_copy_creds(context, &creds, &temp_creds[count])))
            {
                return retval;
            }
            count++;
            ...
            {
                chunk_count++;
                if (!(temp_creds = (krb5_creds**)realloc(
                          temp_creds,
...
krb5_error_code krb5_check_exp(context, tkt_time) krb5_context context;
...
char* flags_string(cred) krb5_creds* cred;
{
    static char buf[32];
    int i = 0;
...
krb5_error_code krb5_get_login_princ(luser, princ_list) const char* luser;
char*** princ_list;
{
    struct stat sbuf;
    struct passwd* pwd;
    ...
    while (fgets(linebuf, BUFSIZ, fp) != NULL)
    {
        ...
        {
            chunk_count++;
            if (!(buf_out = (char**)realloc(buf_out, chunk_count * CHUNK *
                                                         sizeof(char*))))
            {
                return ENOMEM;
            }
...
void show_credential(context, cred, cc) krb5_context context;
krb5_creds* cred;
krb5_ccache cc;
{
...
krb5_error_code gen_sym(krb5_context context, char** sym_out)
{
    krb5_error_code retval;
    char bytes[6], *p, *sym;
...
krb5_error_code krb5_ccache_overwrite(context, ccs, cct,
                                      primary_principal) krb5_context context;
krb5_ccache ccs;
krb5_ccache cct;
krb5_principal primary_principal;
{
...
krb5_boolean* stored;
{
    ...
    if (temp_creds)
    {
        while (temp_creds[i])
        {
            if (krb5_principal_compare(context, temp_creds[i]->client, prst) ==
                TRUE)
            {
...
krb5_error_code krb5_ccache_filter(context, cc, prst) krb5_context context;
krb5_ccache cc;
krb5_principal prst;
{
    ...
    if (ks_ccache_is_initialized(context, cc))
    {
        ...
        if ((retval = krb5_store_some_creds(context, cc, cc_creds_arr, NULL,
                                            prst, &stored)))
        {
            return retval;
        }
...
krb5_boolean krb5_find_princ_in_cred_list(context, creds_list,
                                          princ) krb5_context context;
...
krb5_principal princ;
{
    ...
    if (creds_list)
    {
        while (creds_list[i])
        {
            if (krb5_principal_compare(context, creds_list[i]->client, princ) ==
                TRUE)
            {
                temp_stored = TRUE;
                break;
...
krb5_boolean ks_ccache_name_is_initialized(krb5_context context,
                                           const char* cctag)
{
    krb5_boolean result;
    krb5_ccache cc;
...
krb5_boolean ks_ccache_is_initialized(krb5_context context, krb5_ccache cc)
{
    krb5_principal princ;
    krb5_error_code retval;
