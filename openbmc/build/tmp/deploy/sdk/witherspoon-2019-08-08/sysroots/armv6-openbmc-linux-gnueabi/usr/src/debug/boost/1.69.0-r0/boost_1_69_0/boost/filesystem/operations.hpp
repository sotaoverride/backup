...
class BOOST_SYMBOL_VISIBLE filesystem_error : public system::system_error
{
  ...
  public:
    ...
    filesystem_error(const std::string& what_arg, const path& path1_arg,
                     system::error_code ec) BOOST_NOEXCEPT
        : system::system_error(ec, what_arg)
    {
        try
...
enum perms
{
    ...
    remove_perms = 0x2000, // removes the given permission bits from the current
                           // bits; choose add_perms or remove_perms, not both;
                           // if neither add_perms nor remove_perms is given,
                           // replace the current bits with the given bits.

    symlink_perms =
        0x4000, // on POSIX, don't resolve symlinks; implied on Windows
...
{
  public:
    file_status() BOOST_NOEXCEPT : m_value(status_error),
                                   m_perms(perms_not_known)
    {
    }
    explicit file_status(file_type v) BOOST_NOEXCEPT : m_value(v),
                                                       m_perms(perms_not_known)
    {
    }
    file_status(file_type v, perms prms) BOOST_NOEXCEPT : m_value(v),
                                                          m_perms(prms)
    {
    }

    //  As of October 2015 the interaction between noexcept and =default is so
    //  troublesome for VC++, GCC, and probably other compilers, that =default
    //  is not used with noexcept functions. GCC is not even consistent for the
    //  same release on different platforms.

    file_status(const file_status& rhs) BOOST_NOEXCEPT : m_value(rhs.m_value),
                                                         m_perms(rhs.m_perms)
    {
    }
...
{
    return exists(f) && !is_regular_file(f) && !is_directory(f) &&
           !is_symlink(f);
}
...
BOOST_SCOPED_ENUM_START(copy_option){none = 0, fail_if_exists = none,
                                     overwrite_if_exists};
...
//  result in an undefined reference if the library is compled with -std=c++0x
//  but the use is compiled in C++03 mode, or visa versa. See tickets 6124,
//  6779, 10038.
enum copy_option
{
    none = 0,
...
inline file_status status(const path& p, system::error_code& ec)
{
...
inline file_status symlink_status(const path& p, system::error_code& ec)
{
...
inline bool is_other(const path& p, system::error_code& ec)
{
...
inline bool is_empty(const path& p, system::error_code& ec)
{
...
inline path canonical(const path& p, system::error_code& ec)
{
    return detail::canonical(p, current_path(), &ec);
}
inline path canonical(const path& p, const path& base, system::error_code& ec)
{
...
inline path current_path(system::error_code& ec) BOOST_NOEXCEPT
{
...
inline void current_path(const path& p, system::error_code& ec) BOOST_NOEXCEPT
{
...
inline boost::uintmax_t file_size(const path& p)
{
...
inline boost::uintmax_t hard_link_count(const path& p)
{
...
inline path read_symlink(const path& p, system::error_code& ec)
{
...
inline bool remove(const path& p, system::error_code& ec) BOOST_NOEXCEPT
{
    return detail::remove(p, &ec);
}

inline boost::uintmax_t remove_all(const path& p)
{
...
inline path relative(const path& p, system::error_code& ec)
{
    return detail::relative(p, current_path(), &ec);
}
inline path relative(const path& p, const path& base, system::error_code& ec)
{
...
inline space_info space(const path& p, system::error_code& ec) BOOST_NOEXCEPT
{
...
inline path system_complete(const path& p, system::error_code& ec)
{
...
inline path unique_path(const path& p, system::error_code& ec)
{
...
inline path weakly_canonical(const path& p, system::error_code& ec)
{
...
class BOOST_FILESYSTEM_DECL directory_entry
{
  public:
    typedef boost::filesystem::path::value_type
        value_type; // enables class path ctor taking directory_entry
    ...
    directory_entry(const boost::filesystem::path& p, file_status st,
                    file_status symlink_st = file_status()) :
        m_path(p),
        m_status(st), m_symlink_status(symlink_st)
    {
    }

    directory_entry(const directory_entry& rhs) :
        m_path(rhs.m_path), m_status(rhs.m_status),
...
#ifndef BOOST_FILESYSTEM_NO_DEPRECATED
    void replace_leaf(const boost::filesystem::path& p, file_status st,
...
class directory_iterator
    : public boost::iterator_facade<directory_iterator, directory_entry,
                                    boost::single_pass_traversal_tag>
{
  public:
    ...
    explicit directory_iterator(const path& p) : m_imp(new detail::dir_itr_imp)
    {
  ...
  private:
    ...
    // shared_ptr provides the shallow-copy semantics required for single pass
    // iterators (i.e. InputIterators). The end iterator is indicated by !m_imp
    // || !m_imp->handle
    boost::shared_ptr<detail::dir_itr_imp> m_imp;

    friend class boost::iterator_core_access;

    boost::iterator_facade<directory_iterator, directory_entry,
    ...
    void increment()
    {
        detail::directory_iterator_increment(*this, 0);
    }
...
//  begin() and end() are only used by a range-based for statement in the
//  context of auto - thus the top-level const is stripped - so returning const
//  is harmless and emphasizes begin() is just a pass through.
inline const directory_iterator&
    begin(const directory_iterator& iter) BOOST_NOEXCEPT
{
    return iter;
}
...
inline directory_iterator
    range_begin(const directory_iterator& iter) BOOST_NOEXCEPT
{
    return iter;
}
...
inline directory_iterator range_end(const directory_iterator&) BOOST_NOEXCEPT
{
    return directory_iterator();
}
} // namespace filesystem
...
inline bool
    recur_dir_itr_imp::push_directory(system::error_code& ec) BOOST_NOEXCEPT
// Returns: true if push occurs, otherwise false. Always returns false on error.
{
    ec.clear();

    //  Discover if the iterator is for a directory that needs to be recursed
    //  into, taking symlinks and options into account.
    ...
    // if we are not recursing into symlinks, we are going to have to know if
    // the stack top is a symlink, so get symlink_status and verify no error
    // occurred
    ...
    // Logic for following predicate was contributed by Daniel Aarno to handle
    // cyclic symlinks correctly and efficiently, fixing ticket #5652.
    //   if (((m_options & symlink_option::recurse) == symlink_option::recurse
...
inline void recur_dir_itr_imp::increment(system::error_code* ec)
// ec == 0 means throw on error
//
// Invariant: On return, the top of the iterator stack is the next valid
// (possibly end) iterator, regardless of whether or not an error is reported,
// and regardless of whether any error is reported by exception or error code.
// In other words, progress is always made so a loop on the iterator will always
// eventually terminate regardless of errors.
{
    system::error_code ec_push_directory;

    //  if various conditions are met, push a directory_iterator into the
    //  iterator stack
    ...
    //  stack, popping the stack if necessary, until either the stack is empty
    //  or a non-end iterator is reached.
...
class recursive_directory_iterator
    : public boost::iterator_facade<recursive_directory_iterator,
                                    directory_entry,
                                    boost::single_pass_traversal_tag>
{
  public:
    ...
    explicit recursive_directory_iterator(
        const path& dir_path) // throws if !exists()
        :
    ...
    recursive_directory_iterator(const path& dir_path,
                                 BOOST_SCOPED_ENUM(symlink_option)
                                     opt) // throws if !exists()
        :
        m_imp(new detail::recur_dir_itr_imp)
    {
        m_imp->m_options = opt;
    ...
    recursive_directory_iterator&
        increment(system::error_code& ec) BOOST_NOEXCEPT
    {
    ...
    bool no_push_pending() const BOOST_NOEXCEPT
    {
        return recursion_pending();
    }

#ifndef BOOST_FILESYSTEM_NO_DEPRECATED
    bool no_push_request() const BOOST_NOEXCEPT
    {
  ...
  private:
    ...
    boost::iterator_facade<recursive_directory_iterator, directory_entry,
                           boost::single_pass_traversal_tag>::reference
        dereference() const
    {
        BOOST_ASSERT_MSG(m_imp.get(),
...
inline recursive_directory_iterator
    end(const recursive_directory_iterator&) BOOST_NOEXCEPT
{
...
inline recursive_directory_iterator
    range_end(recursive_directory_iterator&) BOOST_NOEXCEPT
{
    return recursive_directory_iterator();
}
inline recursive_directory_iterator
    range_end(const recursive_directory_iterator&) BOOST_NOEXCEPT
{
    return recursive_directory_iterator();
}
} // namespace filesystem
