...
static int print_filename(FILE* f, struct ext2_dir_entry* dirent, int options)
{
    ...
    if ((options & ENCRYPT_OPT) && !(options & RAW_OPT))
    {
        ...
        {
            char tmp[1];
            return snprintf(tmp, sizeof(tmp), "<encrypted (%d)>", len);
        }
    }
    while (len--)
    {
...
static int list_dir_proc(ext2_ino_t dir EXT2FS_ATTR((unused)), int entry,
                         ...
                         char* buf EXT2FS_ATTR((unused)), void* private)
{
    ...
    if (options & PARSE_OPT)
    {
        ...
        fprintf(ls->f, "/%u/%06o/%d/%d/%.*s/", ino, inode.i_mode, inode.i_uid,
                inode.i_gid, thislen, dirent->name);
    ...
    else if (options & LONG_OPT)
    {
        if (ino)
        {
            ...
            sprintf(datestr, "%2d-%s-%4d %02d:%02d", tm_p->tm_mday,
                    monstr[tm_p->tm_mon], 1900 + tm_p->tm_year, tm_p->tm_hour,
                    tm_p->tm_min);
        }
        else
        {
        ...
        fprintf(ls->f, "(%d)  %5d  %5d   ", ext2fs_dirent_file_type(dirent),
    ...
    else
    {
        if (entry == DIRENT_CHECKSUM)
        {
            sprintf(tmp, "%c%u%c (dirblock checksum: 0x%08x)   ", lbr,
                    dirent->inode, rbr, t->det_checksum);
            thislen = strlen(tmp);
        ...
        sprintf(tmp, "%c%u%c (%d) ", lbr, dirent->inode, rbr, dirent->rec_len);
...
void do_list_dir(int argc, char* argv[])
{
    ...
    retval = ext2fs_dir_iterate2(current_fs, ino, flags, 0, list_dir_proc, &ls);
    fprintf(ls.f, "\n");
    close_pager(ls.f);
    if (retval)
