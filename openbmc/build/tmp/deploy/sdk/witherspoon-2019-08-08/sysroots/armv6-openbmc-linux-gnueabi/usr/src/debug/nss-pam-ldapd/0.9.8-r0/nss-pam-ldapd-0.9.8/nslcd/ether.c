...
static int mkfilter_ether_byname(const char* name, char* buffer, size_t buflen)
{
    ...
    /* build filter */
    return mysnprintf(buffer, buflen, "(&%s(%s=%s))", ether_filter,
...
static int mkfilter_ether_byether(const struct ether_addr* addr, char* buffer,
                                  size_t buflen)
{
...
static int write_ether(TFILE* fp, MYLDAP_ENTRY* entry, const char* reqname,
                       const char* reqether)
{
    int32_t tmpint32;
...
NSLCD_HANDLE(ether, byname, NSLCD_ACTION_ETHER_BYNAME,
...
NSLCD_HANDLE(ether, byether, NSLCD_ACTION_ETHER_BYETHER, struct ether_addr addr;
             char addrstr[20]; char filter[BUFLEN_FILTER];
...
NSLCD_HANDLE(ether, all, NSLCD_ACTION_ETHER_ALL, const char* filter;
