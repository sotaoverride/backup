...
blkid_probe get_swap_prober(const char* devname)
{
    ...
    blkid_probe_set_superblocks_flags(
        pr, BLKID_SUBLKS_LABEL | BLKID_SUBLKS_UUID | BLKID_SUBLKS_VERSION);
    ...
    if (rc == 0)
    {
        ...
        if (blkid_probe_lookup_value(pr, "VERSION", &version, NULL) == 0 &&
            version && strcmp(version, stringify_value(SWAP_VERSION)))
            warnx(_("%s: unsupported swap version '%s'"), devname, version);
        else
            return pr;
    }
