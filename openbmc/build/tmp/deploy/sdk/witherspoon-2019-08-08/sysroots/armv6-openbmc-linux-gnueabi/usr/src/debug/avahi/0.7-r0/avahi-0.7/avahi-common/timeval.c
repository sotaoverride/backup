...
AvahiUsec avahi_timeval_diff(const struct timeval* a, const struct timeval* b)
{
    ...
    return ((AvahiUsec)a->tv_sec - b->tv_sec) * 1000000 + a->tv_usec -
           b->tv_usec;
}
...
struct timeval* avahi_elapse_time(struct timeval* tv, unsigned msec,
                                  unsigned jitter)
{
    assert(tv);
    ...
    if (jitter)
    {
        ...
        avahi_timeval_add(tv,
                          (AvahiUsec)(jitter * 1000.0 * r / (RAND_MAX + 1.0)));
    }
