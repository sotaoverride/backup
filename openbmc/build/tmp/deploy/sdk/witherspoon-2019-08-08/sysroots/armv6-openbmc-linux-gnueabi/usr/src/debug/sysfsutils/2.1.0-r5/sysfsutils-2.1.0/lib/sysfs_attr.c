...
{
    return ((strncmp((char*)new, (char*)old, strlen((char*)new))) < 0 ? 1 : 0);
}
...
{
    return (struct sysfs_attribute*)calloc(1, sizeof(struct sysfs_attribute));
}
...
struct sysfs_attribute* sysfs_open_attribute(const char* path)
{
    ...
    if (sysfs_get_name_from_path(path, sysattr->name, SYSFS_NAME_LEN) != 0)
    {
                                                     ...
                                                     const char* path)
{
    ...
    if (!alist)
    {
        alist = dlist_new_with_delete(sizeof(struct sysfs_attribute),
                                      sysfs_del_attribute);
    }
...
static struct sysfs_attribute* add_attribute(void* dev, const char* path)
{
    ...
    dlist_unshift_sorted(((struct sysfs_device*)dev)->attrlist, attr,
                         sort_list);
...
struct sysfs_attribute* get_attribute(void* dev, const char* name)
{
    ...
    if (((struct sysfs_device*)dev)->attrlist)
    {
        ...
        cur = (struct sysfs_attribute*)dlist_find_custom(
            (((struct sysfs_device*)dev)->attrlist), (void*)name,
            attr_name_equal);
        if (cur)
            return cur;
    }
    safestrcpymax(path, ((struct sysfs_device*)dev)->path, SYSFS_PATH_MAX);
...
struct dlist* read_dir_links(const char* path)
{
    ...
    while ((dirent = readdir(dir)) != NULL)
    {
        ...
        if (!sysfs_path_is_link(file_path))
        {
            ...
            {
                linklist =
                    dlist_new_with_delete(SYSFS_NAME_LEN, sysfs_del_name);
                if (!linklist)
                {
...
int add_subdirectory(struct sysfs_device* dev, char* path)
{
    ...
    if (dev->children == NULL)
        dev->children = dlist_new_with_delete(sizeof(struct sysfs_device),
...
struct dlist* read_dir_subdirs(const char* path)
{
    ...
    while ((dirent = readdir(dir)) != NULL)
    {
        ...
        if (!sysfs_path_is_dir(file_path))
        {
            if (!dirlist)
            {
                dirlist = dlist_new_with_delete(SYSFS_NAME_LEN, sysfs_del_name);
                if (!dirlist)
                {
                    dprintf("Error creating list\n");
                    return NULL;
