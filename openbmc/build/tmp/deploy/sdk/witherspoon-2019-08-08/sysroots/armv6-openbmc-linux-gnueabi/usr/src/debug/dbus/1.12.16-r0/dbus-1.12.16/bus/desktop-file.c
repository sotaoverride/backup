...
static unsigned char valid[256] = {
    ...
    0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
    0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
    0x3, 0x2, 0x0, 0x3, 0x3, 0x3, 0x3, 0x3, 0x3, 0x3, 0x3, 0x3, 0x3, 0x0, 0x0,
    0x0, 0x0, 0x0, 0x0, 0x0, 0x3, 0x3, 0x3, 0x3, 0x3, 0x3, 0x3, 0x3, 0x3, 0x3,
    0x3, 0x3, 0x3, 0x3, 0x3, 0x3, 0x3, 0x3, 0x3, 0x3, 0x3, 0x3, 0x3, 0x3, 0x3,
    0x3, 0x0, 0x0, 0x0, 0x0, 0x2, 0x0, 0x3, 0x3, 0x3, 0x3, 0x3, 0x3, 0x3, 0x3,
    0x3, 0x3, 0x3, 0x3, 0x3, 0x3, 0x3, 0x3, 0x3, 0x3, 0x3, 0x3, 0x3, 0x3, 0x3,
...
static void report_error(BusDesktopFileParser* parser, const char* message,
                         const char* error_name, DBusError* error);
...
static dbus_bool_t grow_lines_in_section(BusDesktopFileSection* section)
{
    ...
    lines =
        dbus_realloc(section->lines, sizeof(BusDesktopFileLine) * new_n_lines);
...
static char* unescape_string(BusDesktopFileParser* parser,
                             const DBusString* str, int pos, int end_pos,
                             DBusError* error)
{
    char *retval, *q;
...
static BusDesktopFileSection* new_section(BusDesktopFile* desktop_file,
                                          const char* name)
{
    int n;
    char* name_copy;
...
static BusDesktopFileSection* open_section(BusDesktopFileParser* parser,
                                           char* name)
{
    ...
    _dbus_assert(&parser->desktop_file->sections[parser->current_section] ==
                 section);
...
{
    /* 5. Group names may contain all ASCII characters except for control
     * characters and '[' and ']'.
     *
...
static dbus_bool_t parse_section_start(BusDesktopFileParser* parser,
                                       DBusError* error)
{
    int line_end, eol_len;
    char* section_name;
    ...
    {
        report_error(parser, "Invalid syntax for section header",
                     BUS_DESKTOP_PARSE_ERROR_INVALID_SYNTAX, error);
        parser_free(parser);
        return FALSE;
    }

    section_name = unescape_string(parser, &parser->data, parser->pos + 1,
                                   line_end - 1, error);
    ...
    {
        report_error(parser, "Invalid characters in section name",
                     BUS_DESKTOP_PARSE_ERROR_INVALID_CHARS, error);
        parser_free(parser);
...
static dbus_bool_t parse_key_value(BusDesktopFileParser* parser,
                                   DBusError* error)
{
    int line_end, eol_len;
    ...
    {
        report_error(parser, "Empty key name",
                     BUS_DESKTOP_PARSE_ERROR_INVALID_SYNTAX, error);
        parser_free(parser);
        return FALSE;
    }
    ...
    if (p < line_end && _dbus_string_get_byte(&parser->data, p) != '=')
    {
        report_error(parser, "Invalid characters in key name",
                     BUS_DESKTOP_PARSE_ERROR_INVALID_CHARS, error);
        parser_free(parser);
        return FALSE;
    }
    ...
    if (p == line_end)
    {
        report_error(parser, "No '=' in key/value pair",
                     BUS_DESKTOP_PARSE_ERROR_INVALID_SYNTAX, error);
        parser_free(parser);
        return FALSE;
    }
    ...
    value =
        unescape_string(parser, &parser->data, value_start, line_end, error);
    if (value == NULL)
    {
        parser_free(parser);
        return FALSE;
                         ...
                         const char* error_name, DBusError* error)
{
    ...
    if (parser->current_section != -1)
        section_name = parser->desktop_file->sections[parser->current_section]
                           .section_name;
...
static BusDesktopFileSection* lookup_section(BusDesktopFile* desktop_file,
...
static BusDesktopFileLine* lookup_line(BusDesktopFile* desktop_file,
                                       BusDesktopFileSection* section,
                                       const char* keyname)
{
    BusDesktopFileLine* line;
