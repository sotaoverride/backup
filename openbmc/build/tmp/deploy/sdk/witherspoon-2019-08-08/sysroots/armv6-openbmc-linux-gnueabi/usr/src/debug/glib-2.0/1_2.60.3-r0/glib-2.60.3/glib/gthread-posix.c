...
static pthread_mutex_t* g_mutex_impl_new(void)
{
    ...
    if
        G_UNLIKELY((status = pthread_mutex_init(mutex, pattr)) != 0)
...
void g_mutex_lock(GMutex* mutex)
{
    ...
    if
        G_UNLIKELY((status = pthread_mutex_lock(g_mutex_get_impl(mutex))) != 0)
...
void g_mutex_unlock(GMutex* mutex)
{
    ...
    if
        G_UNLIKELY((status = pthread_mutex_unlock(g_mutex_get_impl(mutex))) !=
                   0)
...
gboolean g_mutex_trylock(GMutex* mutex)
{
    ...
    if
        G_LIKELY((status = pthread_mutex_trylock(g_mutex_get_impl(mutex))) == 0)
    return TRUE;

    if
        G_UNLIKELY(status != EBUSY)
...
static inline pthread_mutex_t* g_rec_mutex_get_impl(GRecMutex* rec_mutex)
{
    ...
    if
        G_UNLIKELY(impl == NULL)
        {
            ...
            if (!g_atomic_pointer_compare_and_exchange(&rec_mutex->p, NULL,
                                                       impl))
...
static pthread_rwlock_t* g_rw_lock_impl_new(void)
{
    ...
    if
        G_UNLIKELY(rwlock == NULL)
    g_thread_abort(errno, "malloc");

    if
        G_UNLIKELY((status = pthread_rwlock_init(rwlock, NULL)) != 0)
    g_thread_abort(status, "pthread_rwlock_init");

    return rwlock;
}

static void g_rw_lock_impl_free(pthread_rwlock_t* rwlock)
{
    pthread_rwlock_destroy(rwlock);
    free(rwlock);
}

static inline pthread_rwlock_t* g_rw_lock_get_impl(GRWLock* lock)
{
...
static pthread_cond_t* g_cond_impl_new(void)
{
    ...
    if
        G_UNLIKELY(
            (status = pthread_condattr_setclock(&attr, CLOCK_MONOTONIC)) != 0)
...
#error Cannot support GCond on your platform.
    ...
    if
        G_UNLIKELY(cond == NULL)
    g_thread_abort(errno, "malloc");

    if
        G_UNLIKELY((status = pthread_cond_init(cond, &attr)) != 0)
...
static inline pthread_cond_t* g_cond_get_impl(GCond* cond)
{
...
void g_cond_wait(GCond* cond, GMutex* mutex)
{
    ...
    if
        G_UNLIKELY((status = pthread_cond_wait(g_cond_get_impl(cond),
                                               g_mutex_get_impl(mutex))) != 0)
...
void g_cond_signal(GCond* cond)
{
    ...
    if
        G_UNLIKELY((status = pthread_cond_signal(g_cond_get_impl(cond))) != 0)
...
void g_cond_broadcast(GCond* cond)
{
    ...
    if
        G_UNLIKELY((status = pthread_cond_broadcast(g_cond_get_impl(cond))) !=
                   0)
...
gboolean g_cond_wait_until(GCond* cond, GMutex* mutex, gint64 end_time)
{
     ...
     */
    {
        ...
        if ((status = pthread_cond_timedwait_relative_np(
                 g_cond_get_impl(cond), g_mutex_get_impl(mutex), &ts)) == 0)
            return TRUE;
    }
#elif defined(HAVE_PTHREAD_CONDATTR_SETCLOCK) && defined(CLOCK_MONOTONIC)
     ...
     */
    {
        ...
        if ((status = pthread_cond_timedwait(
                 g_cond_get_impl(cond), g_mutex_get_impl(mutex), &ts)) == 0)
            return TRUE;
    }
#else
...
void g_private_set(GPrivate* key, gpointer value)
{
    ...
    if
        G_UNLIKELY((status = pthread_setspecific(*g_private_get_impl(key),
                                                 value)) != 0)
...
void g_private_replace(GPrivate* key, gpointer value)
{
    ...
    if
        G_UNLIKELY((status = pthread_setspecific(*impl, value)) != 0)
    g_thread_abort(status, "pthread_setspecific");
}
...
#define posix_check_err(err, name)                                             \
    ...
    {                                                                          \
        int error = (err);                                                     \
        if (error)                                                             \
            g_error("file %s: line %d (%s): error '%s' during '%s'", __FILE__, \
                    __LINE__, G_STRFUNC, g_strerror(error), name);             \
    }                                                                          \
    G_STMT_END
                                 ...
                                 gpointer data, GError** error)
{
    ...
    ret = pthread_create(&thread->system_thread, &attr, (void* (*)(void*))proxy,
                         thread);
...
#define compare_exchange_acquire(ptr, old, new)                                \
    __atomic_compare_exchange_4((ptr), (old), (new), 0, __ATOMIC_ACQUIRE,      \
                                __ATOMIC_RELAXED)
...
static void __attribute__((noinline)) g_mutex_lock_slowpath(GMutex* mutex)
{
    ...
    while (exchange_acquire(&mutex->i[0], 2) != 0)
        syscall(__NR_futex, &mutex->i[0], (gsize)FUTEX_WAIT_PRIVATE, (gsize)2,
                NULL);
}

static void __attribute__((noinline))
g_mutex_unlock_slowpath(GMutex* mutex, guint prev)
{
    ...
    syscall(__NR_futex, &mutex->i[0], (gsize)FUTEX_WAKE_PRIVATE, (gsize)1,
            NULL);
}
...
void g_mutex_lock(GMutex* mutex)
{
    ...
    if
        G_UNLIKELY(g_atomic_int_add(&mutex->i[0], 1) != 0)
...
void g_cond_wait(GCond* cond, GMutex* mutex)
{
    ...
    syscall(__NR_futex, &cond->i[0], (gsize)FUTEX_WAIT_PRIVATE, (gsize)sampled,
            NULL);
    g_mutex_lock(mutex);
}
...
void g_cond_broadcast(GCond* cond)
{
    ...
    syscall(__NR_futex, &cond->i[0], (gsize)FUTEX_WAKE_PRIVATE, (gsize)INT_MAX,
            NULL);
}
