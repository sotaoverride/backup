...
const s8* IoPipe::localCodec()
{
    ...
    if (!local_codec[0])
    {
        // The output character set of gettext is, by default, the value of
        // nl_langinfo(CODESET), which depends on the LC_CTYPE part of the
        // current locale.
        setlocale(LC_CTYPE, "");
...
void IoPipe::setCodec(const s8* up, const s8* down)
{
    ...
    if (!strcasecmp(up, down))
        return;

    mCodecRead = iconv_open(up, down);
    if (mCodecRead == (void*)-1)
        mCodecRead = 0;

    mCodecWrite = iconv_open(down, up);
    if (mCodecWrite == (void*)-1)
        mCodecWrite = 0;
}
