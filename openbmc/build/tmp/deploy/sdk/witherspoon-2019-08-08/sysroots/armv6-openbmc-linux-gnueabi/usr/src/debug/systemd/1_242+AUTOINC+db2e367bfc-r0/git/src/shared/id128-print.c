...
int id128_pretty_print(sd_id128_t id, bool pretty)
{
    ...
    if (terminal_urlify("man:systemd-id128(1)", "systemd-id128(1)", &man_link) <
        0)
        return log_oom();

    if (terminal_urlify("https://docs.python.org/3/library/uuid.html", "uuid",
                        &mod_link) < 0)
        return log_oom();
    ...
    printf("As string:\n"
           ...
           "As UUID:\n"
           "%s%02x%02x%02x%02x-%02x%02x-%02x%02x-%02x%02x-%02x%02x%02x%02x%02x%"
           "02x%s\n\n"
           "As %s macro:\n"
    ...
    printf("As Python constant:\n"
           ...
           mod_link, on, SD_ID128_FORMAT_VAL(id), off);
