 ...
 *
 * This structure should be used in conjunction with the \c
 * BOOST_LOG_ONCE_BLOCK_FLAG macro. Usage example:
 *
 * <code>
...
struct once_block_flag
{
    ...
    {
        uninitialized =
            0, // this must be zero, so that zero-initialized once_block_flag is
               // equivalent to the one initialized with uninitialized
        being_initialized,
        initialized
    };
 ...
 */
#define BOOST_LOG_ONCE_BLOCK_INIT                                              \
    {                                                                          \
...
class once_block_sentry
{
  ...
  public:
    ...
    {
        return (m_flag.status == once_block_flag::initialized ||
                enter_once_block());
    }
...
// NOTE: flag_var deliberately doesn't have an initializer so that it is
// zero-initialized at the static initialization stage
 ...
 *
 * Begins a code block to be executed only once, with protection against thread
 * concurrency. User has to provide the flag variable that controls whether the
