...
#define AVAHI_DNS_PACKET_SIZE_MAX                                              \
    (AVAHI_DNS_PACKET_HEADER_SIZE + 256 + 2 + 2 + 4 + 2 + AVAHI_DNS_RDATA_MAX)
...
#define AVAHI_DNS_PACKET_DATA(p)                                               \
    ((p)->data ? (p)->data : ((uint8_t*)p) + sizeof(AvahiDnsPacket))
...
uint8_t* avahi_dns_packet_append_bytes(AvahiDnsPacket* p, const void* d,
                                       size_t l);
...
#define AVAHI_DNS_FLAGS(qr, opcode, aa, tc, rd, ra, z, ad, cd, rcode)          \
    (((uint16_t) !!qr << 15) | ((uint16_t)(opcode & 15) << 11) |               \
     ((uint16_t) !!aa << 10) | ((uint16_t) !!tc << 9) |                        \
     ((uint16_t) !!rd << 8) | ((uint16_t) !!ra << 7) |                         \
     ((uint16_t) !!ad << 5) | ((uint16_t) !!cd << 4) |                         \
     ((uint16_t)(rcode & 15)))
