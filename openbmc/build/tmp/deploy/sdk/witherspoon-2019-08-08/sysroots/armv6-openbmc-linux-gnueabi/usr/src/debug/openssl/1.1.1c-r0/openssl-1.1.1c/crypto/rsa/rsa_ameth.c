...
static int rsa_param_encode(const EVP_PKEY* pkey, ASN1_STRING** pstr,
                            int* pstrtype)
{
...
static int rsa_pub_encode(X509_PUBKEY* pk, const EVP_PKEY* pkey)
{
    ...
    if (penclen <= 0)
        return 0;
    if (X509_PUBKEY_set0_param(pk, OBJ_nid2obj(pkey->ameth->pkey_id), strtype,
                               str, penc, penclen))
        return 1;
...
static int rsa_priv_encode(PKCS8_PRIV_KEY_INFO* p8, const EVP_PKEY* pkey)
{
    ...
    if (!PKCS8_pkey_set0(p8, OBJ_nid2obj(pkey->ameth->pkey_id), 0, strtype, str,
                         rk, rklen))
    {
        RSAerr(RSA_F_RSA_PRIV_ENCODE, ERR_R_MALLOC_FAILURE);
        ASN1_STRING_free(str);
                         ...
                         ASN1_BIT_STRING* sig)
{
    ...
    if (pad_mode == RSA_PKCS1_PSS_PADDING)
    {
        ...
        X509_ALGOR_set0(alg1, OBJ_nid2obj(EVP_PKEY_RSA_PSS), V_ASN1_SEQUENCE,
                        os1);
        return 3;
    }
    return 2;
...
const EVP_PKEY_ASN1_METHOD rsa_asn1_meths[2] = {
    ...
    {EVP_PKEY_RSA2, EVP_PKEY_RSA, ASN1_PKEY_ALIAS}};

const EVP_PKEY_ASN1_METHOD rsa_pss_asn1_meth = {EVP_PKEY_RSA_PSS,
                                                EVP_PKEY_RSA_PSS,
                                                ASN1_PKEY_SIGPARAM_NULL,
