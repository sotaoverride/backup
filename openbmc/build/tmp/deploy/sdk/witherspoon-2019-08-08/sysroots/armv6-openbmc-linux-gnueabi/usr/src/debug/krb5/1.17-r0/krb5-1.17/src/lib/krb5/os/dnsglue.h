...
#define INCR_OK(base, max, ptr, incr)                                          \
    ((incr) <=                                                                 \
     (max) - ((const unsigned char*)(ptr) - (const unsigned char*)(base)))
...
#define SAFE_GETUINT16(base, max, ptr, incr, s, label)                         \
    do                                                                         \
    {                                                                          \
        if (!INCR_OK(base, max, ptr, incr))                                    \
            goto label;                                                        \
        (s) = (unsigned short)(ptr)[0] << 8 | (unsigned short)(ptr)[1];        \
        (ptr) += (incr);                                                       \
    } while (0)
...
int krb5int_dns_nextans(struct krb5int_dns_state*, const unsigned char**, int*);
int krb5int_dns_expand(struct krb5int_dns_state*, const unsigned char*, char*,
                       int);
