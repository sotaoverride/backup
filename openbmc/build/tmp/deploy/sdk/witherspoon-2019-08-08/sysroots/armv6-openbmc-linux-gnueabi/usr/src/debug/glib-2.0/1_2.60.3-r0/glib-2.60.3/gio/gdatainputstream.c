...
static void g_data_input_stream_set_property(GObject* object, guint prop_id,
                                             const GValue* value,
                                             GParamSpec* pspec);
static void g_data_input_stream_get_property(GObject* object, guint prop_id,
                                             GValue* value, GParamSpec* pspec);

G_DEFINE_TYPE_WITH_PRIVATE(GDataInputStream, g_data_input_stream,
...
static void g_data_input_stream_class_init(GDataInputStreamClass* klass)
{
    ...
    g_object_class_install_property(
        object_class, PROP_BYTE_ORDER,
        g_param_spec_enum(
            "byte-order", P_("Byte order"), P_("The byte order"),
            G_TYPE_DATA_STREAM_BYTE_ORDER, G_DATA_STREAM_BYTE_ORDER_BIG_ENDIAN,
     ...
     */
    g_object_class_install_property(
        object_class, PROP_NEWLINE_TYPE,
        g_param_spec_enum(
            "newline-type", P_("Newline type"),
            P_("The accepted types of line ending"),
            G_TYPE_DATA_STREAM_NEWLINE_TYPE, G_DATA_STREAM_NEWLINE_TYPE_LF,
            G_PARAM_READWRITE | G_PARAM_STATIC_NAME | G_PARAM_STATIC_BLURB));
}
                                             ...
                                             GParamSpec* pspec)
{
    ...
    switch (prop_id)
    {
        case PROP_BYTE_ORDER:
            g_data_input_stream_set_byte_order(dstream,
...
GDataInputStream* g_data_input_stream_new(GInputStream* base_stream)
{
    ...
    stream = g_object_new(G_TYPE_DATA_INPUT_STREAM, "base-stream", base_stream,
                          NULL);
...
{
    g_return_val_if_fail(G_IS_DATA_INPUT_STREAM(stream),
...
static gboolean read_data(GDataInputStream* stream, void* buffer, gsize size,
                          GCancellable* cancellable, GError** error)
{
    gsize available;
    gssize res;

    while ((available = g_buffered_input_stream_get_available(
                G_BUFFERED_INPUT_STREAM(stream))) < size)
    {
        res =
            g_buffered_input_stream_fill(G_BUFFERED_INPUT_STREAM(stream),
                                         size - available, cancellable, error);
        if (res < 0)
            return FALSE;
        if (res == 0)
        {
 ...
 * see g_data_input_stream_get_byte_order() and
 *g_data_input_stream_set_byte_order().
 *
    ...
    for (i = 0; checked < available && i < peeked; i++)
    {
        switch (priv->newline_type)
        {
            ...
            case G_DATA_STREAM_NEWLINE_TYPE_ANY:
                ...
                /* Don't check for CR here, instead look at last_saw_cr on next
                 * byte */
                break;
        }
 ...
 * was cancelled, the error %G_IO_ERROR_CANCELLED will be returned.
 *
 * Returns: (nullable) (transfer full) (array zero-terminated=1) (element-type
 *guint8): a NUL terminated byte array with the line that was read in (without
 *the newlines).  Set @length to a #gsize to get the length of the read line. On
 *an error, it will return %NULL and @error will be set. If there's no content
 *to read, it will still return %NULL, but @error won't be set.
 **/
char* g_data_input_stream_read_line(GDataInputStream* stream, gsize* length,
                                    GCancellable* cancellable, GError** error)
{
    GBufferedInputStream* bstream;
    gsize checked;
    ...
    while ((found_pos = scan_for_newline(stream, &checked, &last_saw_cr,
                                         &newline_len)) == -1)
    {
        if (g_buffered_input_stream_get_available(bstream) ==
            g_buffered_input_stream_get_buffer_size(bstream))
            g_buffered_input_stream_set_buffer_size(
                bstream, 2 * g_buffered_input_stream_get_buffer_size(bstream));
    ...
    res = g_input_stream_read(G_INPUT_STREAM(stream), line,
                              found_pos + newline_len, NULL, NULL);
    if (length)
...
static gssize scan_for_chars(GDataInputStream* stream, gsize* checked_out,
                             const char* stop_chars, gssize stop_chars_len)
{
    GBufferedInputStream* bstream;
    const char* buffer;
    gsize start, end, peeked;
...
char* g_data_input_stream_read_until(GDataInputStream* stream,
                                     const gchar* stop_chars, gsize* length,
                                     GCancellable* cancellable, GError** error)
{
    GBufferedInputStream* bstream;
    gchar* result;
    ...
    result = g_data_input_stream_read_upto(stream, stop_chars, -1, length,
                                           cancellable, error);
...
static void g_data_input_stream_read_complete(GTask* task, gsize read_length,
                                              gsize skip_length)
{
...
static void g_data_input_stream_read_line_ready(GObject* object,
                                                GAsyncResult* result,
                                                gpointer user_data)
{
    ...
    if (data->stop_chars)
    {
        found_pos = scan_for_chars(G_DATA_INPUT_STREAM(buffer), &data->checked,
                                   data->stop_chars, data->stop_chars_len);
        newline_len = 0;
    }
    else
        found_pos =
            scan_for_newline(G_DATA_INPUT_STREAM(buffer), &data->checked,
    ...
    /* didn't find a full line; need to buffer some more bytes */
    {
        ...
        g_buffered_input_stream_fill_async(
            buffer, -1, g_task_get_priority(task), g_task_get_cancellable(task),
            g_data_input_stream_read_line_ready, user_data);
    }
    else
    {
...
static void g_data_input_stream_read_async(
    GDataInputStream* stream, const gchar* stop_chars, gssize stop_chars_len,
    gint io_priority, GCancellable* cancellable, GAsyncReadyCallback callback,
    gpointer user_data)
{
...
static gchar* g_data_input_stream_read_finish(GDataInputStream* stream,
                                              GAsyncResult* result,
                                         ...
                                         gpointer user_data)
{
    ...
    g_data_input_stream_read_async(stream, NULL, 0, io_priority, cancellable,
                                   callback, user_data);
}
...
void g_data_input_stream_read_until_async(
    GDataInputStream* stream, const gchar* stop_chars, gint io_priority,
 ...
 * Returns: (nullable) (transfer full) (array zero-terminated=1) (element-type
 * guint8): a NUL-terminated byte array with the line that was read in (without
 * the newlines).  Set @length to a #gsize to get the length of the read line.
 ...
 * Since: 2.20
 */
gchar* g_data_input_stream_read_line_finish(GDataInputStream* stream,
 ...
 * Since: 2.30
 */
gchar* g_data_input_stream_read_line_finish_utf8(GDataInputStream* stream,
                                                 GAsyncResult* result,
                                                 gsize* length, GError** error)
{
    gchar* res;
 ...
 *     has more consistent behaviour regarding the stop character.
 */
gchar* g_data_input_stream_read_until_finish(GDataInputStream* stream,
                                             GAsyncResult* result,
                                             gsize* length, GError** error)
{
    g_return_val_if_fail(g_task_is_valid(result, stream), NULL);
                                    ...
                                    GCancellable* cancellable, GError** error)
{
    ...
    res = g_input_stream_read(G_INPUT_STREAM(stream), data_until, found_pos,
                              NULL, NULL);
    if (length)
        *length = (gsize)found_pos;
    ...
    gpointer user_data)
{
    ...
    g_data_input_stream_read_async(stream, stop_chars, stop_chars_len,
 ...
 * Since: 2.24
 */
gchar* g_data_input_stream_read_upto_finish(GDataInputStream* stream,
                                            GAsyncResult* result, gsize* length,
                                            GError** error)
{
    g_return_val_if_fail(g_task_is_valid(result, stream), NULL);
