...
static char* gsserrstr(char* buf, ber_len_t buf_len, gss_OID mech, int gss_rc,
                                       ...
                                       Sockbuf_Buf* dst)
{
    ...
    if (conf_req_flag && conf_state == 0)
    {
        ber_log_printf(LDAP_DEBUG_ANY, p->sbiod->sbiod_sb->sb_debug,
                       "sb_sasl_gssapi_encode: GSS_C_CONF_FLAG was ignored by "
                       "our gss_wrap()\n");
        return -1;
    }
                                       ...
                                       const Sockbuf_Buf* src, Sockbuf_Buf* dst)
{
    ...
    gss_rc = gss_unwrap(&minor_status, gss_ctx, &wrapped, &unwrapped,
                           ...
                           OM_uint32 minor_status)
{
    ...
    Debug(LDAP_DEBUG_ANY, "%s\n",
          gsserrstr(msg, sizeof(msg), mech, gss_rc, minor_status), NULL, NULL);
                                         ...
                                         char** pdnsHostName)
{
    ...
    LDAPMessage *res, *e;
    char* attrs[] = {"supportedSASLMechanisms", "ldapServiceName",
                     "dnsHostName", NULL};
    ...
    rc = ldap_search_s(ld, "", LDAP_SCOPE_BASE, NULL, attrs, 0, &res);
...
void ldap_int_gssapi_close(LDAP* ld, LDAPConn* lc)
{
    if (lc && lc->lconn_gss_ctx)
    {
        ...
        {
            gss_delete_sec_context(&minor_status, &old_gss_ctx,
                                   GSS_C_NO_BUFFER);
        }
...
static int ldap_int_gss_spnego_bind_s(LDAP* ld)
{
    int rc;
    int gss_rc;
    OM_uint32 minor_status;
...
#define __SPNEGO_OID "\053\006\001\005\005\002"
    ...
    gss_rc = gss_test_oid_set_member(&minor_status, &spnego_oid,
                                     supported_mechs, &spnego_support);
    ...
    while (1)
    {
        ...
        rc =
            ldap_sasl_bind_s(ld, NULL, "GSS-SPNEGO", &cred, NULL, NULL, &scred);
        ...
        gss_rc = gss_init_sec_context(&minor_status, GSS_C_NO_CREDENTIAL,
                                      &gss_ctx, principal, req_mech, req_flags,
                                      0, NULL, &input_token, &ret_mech,
...
int ldap_int_gssapi_get_option(LDAP* ld, int option, void* arg)
{
    if (ld == NULL)
        return -1;
    ...
    switch (option)
    {
        ...
        case LDAP_OPT_SECURITY_CONTEXT:
            ...
            {
                *(gss_ctx_id_t*)arg =
                    (gss_ctx_id_t)ld->ld_defconn->lconn_gss_ctx;
            }
            else
            {
...
int ldap_int_gssapi_set_option(LDAP* ld, int option, void* arg)
{
    if (ld == NULL)
        return -1;
    ...
    switch (option)
    {
        ...
        case LDAP_OPT_ENCRYPT:
            if (arg != LDAP_OPT_OFF)
            {
                ld->ld_options.ldo_gssapi_flags |=
        ...
        case LDAP_OPT_SECURITY_CONTEXT:
            ...
            {
                ldap_int_gssapi_setup(ld, ld->ld_defconn, (gss_ctx_id_t)arg);
            }
            break;
...
int ldap_gssapi_bind(LDAP* ld, LDAP_CONST char* dn, LDAP_CONST char* creds)
{
    return LDAP_NOT_SUPPORTED;
}

int ldap_gssapi_bind_s(LDAP* ld, LDAP_CONST char* dn, LDAP_CONST char* creds)
{
    if (dn != NULL)
    {
        return LDAP_NOT_SUPPORTED;
