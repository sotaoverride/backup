    ...
    void init_mteEventConf(void)
{
    ...
    snmpd_register_config_handler("setEvent", parse_setEvent, NULL,
     ...
     */
    snmpd_register_config_handler("_mteETable", parse_mteETable, NULL, NULL);
    snmpd_register_config_handler("_mteENotTable", parse_mteENotTable, NULL,
                                  NULL);
    snmpd_register_config_handler("_mteESetTable", parse_mteESetTable, NULL,
                                  NULL);
    ...
    snmp_register_callback(SNMP_CALLBACK_APPLICATION,
                           SNMPD_CALLBACK_PRE_UPDATE_CONFIG, clear_mteETable,
                           NULL);
}
 ...
 */
static struct mteEvent* _find_mteEvent_entry(const char* owner,
                                             const char* ename)
{
    ...
    snmp_set_var_typed_value(&ename_var, ASN_PRIV_IMPLIED_OCTET_STR, ename,
                             strlen(ename));
...
static struct mteEvent* _find_typed_mteEvent_entry(const char* owner,
                                                   const char* ename, int type)
{
...
void parse_notificationEvent(const char* token, char* line)
{
    ...
    if (cp && *cp == '-' && *(cp + 1) == 'm')
    {
        ...
        for (var = tp->varbinds; var; var = var->next)
        {
            idx++;
            object =
                mteObjects_addOID("snmpd.conf", ename, idx, var->vblabel, wild);
    ...
    entry->flags |= MTE_EVENT_FLAG_ENABLED | MTE_EVENT_FLAG_ACTIVE |
                    MTE_EVENT_FLAG_FIXED | MTE_EVENT_FLAG_VALID;
    return;
}
...
void parse_mteESetTable(const char* token, char* line)
{
    ...
    vp = entry->mteSetOID;
    line =
        read_config_read_data(ASN_OBJECT_ID, line, &vp, &entry->mteSetOID_len);
    line = read_config_read_data(ASN_UNSIGNED, line, &entry->mteSetValue, &len);
    len = MTE_STR2_LEN;
...
int clear_mteETable(int majorID, int minorID, void* serverarg, void* clientarg)
{
     ...
     */
    memset(&owner_var, 0, sizeof(netsnmp_variable_list));
    snmp_set_var_typed_value(&owner_var, ASN_OCTET_STR, "snmpd.conf",
                             strlen("snmpd.conf"));
    while ((row = netsnmp_tdata_row_next_byidx(event_table_data, &owner_var)))
    {
        /*
