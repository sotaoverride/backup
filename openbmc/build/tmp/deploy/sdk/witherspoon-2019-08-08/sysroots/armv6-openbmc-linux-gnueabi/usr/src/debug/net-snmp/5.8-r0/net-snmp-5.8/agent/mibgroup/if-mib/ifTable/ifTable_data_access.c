...
static void _delete_missing_interface(ifTable_rowreq_ctx* rowreq_ctx,
...
static void parse_interface_fadeout(const char* token, char* line)
{
    fadeout = atoi(line);
}
static void parse_interface_replace_old(const char* token, char* line)
{
...
int ifTable_init_data(ifTable_registration* ifTable_reg)
{
     ...
     */
    snmpd_register_config_handler("interface_fadeout", parse_interface_fadeout,
                                  NULL, "interface_fadeout seconds");
    snmpd_register_config_handler("interface_replace_old",
                                  parse_interface_replace_old, NULL,
                                  "interface_replace_old yes|no");
                            ...
                            netsnmp_cache* cache)
{
    ...
    *container_ptr_ptr = NULL;
     ...
     * often the cache is reloaded. A 10 Mbps stream can wrap if*Octets in ~57
     * minutes. At 100 Mbps it is ~5 minutes, and at 1 Gbps, ~34 seconds.
     */
...
void send_linkUpDownNotifications(oid* notification_oid,
                                  size_t notification_oid_len, int if_index,
                                  int if_admin_status, int if_oper_status)
{
    /*
     ...
     */
    snmp_varlist_add_variable(&notification_vars, if_index_oid,
                              if_index_oid_len, ASN_INTEGER, (u_char*)&if_index,
                              sizeof(if_index));
    ...
    snmp_varlist_add_variable(
        &notification_vars, if_admin_status_oid, if_admin_status_oid_len,
        ASN_INTEGER, (u_char*)&if_admin_status, sizeof(if_admin_status));

    snmp_varlist_add_variable(&notification_vars, if_oper_status_oid,
                              if_oper_status_oid_len, ASN_INTEGER,
                              (u_char*)&if_oper_status, sizeof(if_oper_status));
 ...
 *
 */
static void _check_interface_entry_for_updates(ifTable_rowreq_ctx* rowreq_ctx,
                                               cd_container* cdc)
{
    char oper_changed = 0;
    ...
    if (NULL == ifentry)
    {
        ...
        if (rowreq_ctx->known_missing)
        {
            ...
            DEBUGMSGTL(("verbose:ifTable:access",
                        "missing entry for %ld seconds\n", diff));
            if (diff >= fadeout)
            {
                DEBUGMSGTL(("ifTable:access",
                            "marking missing entry %s for "
                            "removal after %d seconds\n",
                            rowreq_ctx->data.ifName, fadeout));
                if (NULL == cdc->deleted)
                    cdc->deleted =
                        netsnmp_container_find("ifTable_deleted:linked_list");
                if (NULL == cdc->deleted)
                    snmp_log(
                        LOG_ERR,
                        "couldn't create container for deleted interface\n");
                else
                {
    ...
    else
    {
...
#ifdef USING_IF_MIB_IFXTABLE_IFXTABLE_MODULE
        {
            ...
            {
                NETSNMP_LOGONCE((
                    LOG_ERR, "Name of an interface changed. Such "
                             "interfaces will keep its old name in IF-MIB.\n"));
                DEBUGMSGTL(("ifTable:access",
                            "interface %s changed name to %s, ignoring\n",
        ...
        if (rowreq_ctx->data.ifLinkUpDownTrapEnable == 1)
        {
            if (rowreq_ctx->data.ifOperStatus == IFOPERSTATUS_UP)
            {
                ...
                send_linkUpDownNotifications(
                    ...
                    rowreq_ctx->tbl_idx.ifIndex, rowreq_ctx->data.ifAdminStatus,
 ...
 */
static void _check_and_replace_old(netsnmp_interface_entry* ifentry,
                                   netsnmp_container* container)
{
    netsnmp_iterator* it;
    ...
    for (rowreq_ctx = ITERATOR_FIRST(it); rowreq_ctx;
         rowreq_ctx = ITERATOR_NEXT(it))
    {
 ...
 * add new entry
 */
static void _add_new_interface(netsnmp_interface_entry* ifentry,
                               netsnmp_container* container)
{
    ifTable_rowreq_ctx* rowreq_ctx;
...
int ifTable_container_load(netsnmp_container* container)
{
     ...
     */
    cdc.current = netsnmp_access_interface_container_load(
        NULL, NETSNMP_ACCESS_INTERFACE_INIT_NOFLAGS);
    if (NULL == cdc.current)
    ...
    CONTAINER_FOR_EACH(
        container,
        (netsnmp_container_obj_func*)_check_interface_entry_for_updates, &cdc);
    ...
    {
        CONTAINER_FOR_EACH(
            cdc.deleted, (netsnmp_container_obj_func*)_delete_missing_interface,
            container);
    ...
    netsnmp_access_interface_container_free(
        cdc.current, NETSNMP_ACCESS_INTERFACE_FREE_DONT_CLEAR);

    DEBUGMSGT(("verbose:ifTable:ifTable_cache_load", "%lu records\n",
