...
static errcode_t alloc_icount(ext2_filsys fs, int flags, ext2_icount_t* ret)
{
    ...
    *ret = 0;
    ...
    {
        retval =
            ext2fs_allocate_inode_bitmap(fs, "icount_inc", &icount->multiple);
        if (retval)
            goto errout;
    }
    else
...
errcode_t ext2fs_create_icount(ext2_filsys fs, int flags, unsigned int size,
                               ext2_icount_t* ret)
{
                                               ...
                                               ext2_ino_t ino, int pos)
{
    ...
    if (icount->count >= icount->size)
    {
        ...
        retval = ext2fs_resize_mem(
            (size_t)icount->size * sizeof(struct ext2_icount_el),
            (size_t)new_size * sizeof(struct ext2_icount_el), &icount->list);
        if (retval)
            return 0;
    ...
    for (i = 1; i < icount->count; i++)
    {
        if (icount->list[i - 1].ino >= icount->list[i].ino)
        {
            fprintf(out, "%s: list[%d].ino=%u, list[%d].ino=%u\n", bad, i - 1,
                    icount->list[i - 1].ino, i, icount->list[i].ino);
                                  ...
                                  __u16* ret)
{
    ...
    if (icount->multiple && !ext2fs_test_inode_bitmap2(icount->multiple, ino))
        return EXT2_ET_INVALID_ARGUMENT;
...
errcode_t ext2fs_icount_store(ext2_icount_t icount, ext2_ino_t ino, __u16 count)
{
    if (!ino || (ino > icount->num_inodes))
        return EXT2_ET_INVALID_ARGUMENT;
...
struct test_program prog[] = {
    {STORE, 42, 42, 42},  {STORE, 1, 1, 1},     {STORE, 2, 2, 2},
    {STORE, 3, 3, 3},     {STORE, 10, 1, 1},    {STORE, 42, 0, 0},
    {INCREMENT, 5, 0, 1}, {INCREMENT, 5, 0, 2}, {INCREMENT, 5, 0, 3},
    {INCREMENT, 5, 0, 4}, {DECREMENT, 5, 0, 3}, {DECREMENT, 5, 0, 2},
    {DECREMENT, 5, 0, 1}, {DECREMENT, 5, 0, 0}, {FETCH, 10, 0, 1},
    {FETCH, 1, 0, 1},     {FETCH, 2, 0, 2},     {FETCH, 3, 0, 3},
    {INCREMENT, 1, 0, 2}, {DECREMENT, 2, 0, 1}, {DECREMENT, 2, 0, 0},
    {FETCH, 12, 0, 0},    {EXIT, 0, 0, 0}};

struct test_program extended[] = {
    {STORE, 1, 1, 1},  {STORE, 2, 2, 2},  {STORE, 3, 3, 3},  {STORE, 4, 4, 4},
    {STORE, 5, 5, 5},  {STORE, 6, 1, 1},  {STORE, 7, 2, 2},  {STORE, 8, 3, 3},
    {STORE, 9, 4, 4},  {STORE, 10, 5, 5}, {STORE, 11, 1, 1}, {STORE, 12, 2, 2},
    {STORE, 13, 3, 3}, {STORE, 14, 4, 4}, {STORE, 15, 5, 5}, {STORE, 16, 1, 1},
    {STORE, 17, 2, 2}, {STORE, 18, 3, 3}, {STORE, 19, 4, 4}, {STORE, 20, 5, 5},
    {STORE, 21, 1, 1}, {STORE, 22, 2, 2}, {STORE, 23, 3, 3}, {STORE, 24, 4, 4},
    {STORE, 25, 5, 5}, {STORE, 26, 1, 1}, {STORE, 27, 2, 2}, {STORE, 28, 3, 3},
    {STORE, 29, 4, 4}, {STORE, 30, 5, 5}, {EXIT, 0, 0, 0}};
...
static void setup(void)
{
    ...
    if (retval)
    {
        com_err("setup", retval, "while allocating tables for test filesystem");
        exit(1);
    }
}
...
int run_test(int flags, int size, char* dir, struct test_program* prog)
{
    ...
    {
#ifdef CONFIG_TDB
        retval = ext2fs_create_icount_tdb(test_fs, dir, flags, &icount);
        if (retval)
        {
            com_err("run_test", retval, "while creating icount using tdb");
            exit(1);
        }
#else
        printf("Skipped\n");
    ...
    for (pc = prog; pc->cmd != EXIT; pc++)
    {
        switch (pc->cmd)
        {
            ...
            case STORE:
                ...
                if (retval)
                {
                    com_err("run_test", retval, "while calling icount_store");
                    exit(1);
                }
        ...
        if (retval)
        {
            com_err("run_test", retval, "while calling icount_fetch");
            exit(1);
        }
        printf("%u (%s)\n", result, (result == pc->expected) ? "OK" : "NOT OK");
