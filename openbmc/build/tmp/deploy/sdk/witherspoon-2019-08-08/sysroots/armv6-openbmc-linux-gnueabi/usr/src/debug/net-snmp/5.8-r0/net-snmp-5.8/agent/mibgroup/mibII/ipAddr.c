...
u_char* var_ipAddrEntry(struct variable* vp, oid* name, size_t* length,
...
#else
        cp = (u_char*)&(
            ((struct sockaddr_in*)&(in_ifaddr.ia_addr))->sin_addr.s_addr);
#endif
    ...
    switch (vp->magic)
    {
        ...
        case IPADNETMASK:
            *var_len = sizeof(addr_ret);
            ...
            return (u_char*)&addr_ret;
#elif defined(linux)
            return (u_char*)&((struct sockaddr_in*)&lowin_ifnet.ia_subnetmask)
                ->sin_addr.s_addr;
#elif !defined(sunV3)
        ...
        case IPADBCASTADDR:
            ...
            *var_len = sizeof(long_return);
            ...
            long_return = ((struct sockaddr_in*)&lowin_ifaddr.ia_broadaddr)
                              ->sin_addr.s_addr &
                          1;
#else
            long_return =
        ...
        default:
            DEBUGMSGTL(
                ("snmpd", "unknown sub-id %d in var_ipAddrEntry\n", vp->magic));
    }
    return NULL;
}
 ...
 */
static int Address_Scan_Next(Index, Retin_ifaddr) short* Index;
struct in_ifaddr* Retin_ifaddr;
{
    ...
    while (in_ifaddraddr)
    {
         ...
         */
        if (!NETSNMP_KLOOKUP(in_ifaddraddr, (char*)&in_ifaddr,
                             sizeof in_ifaddr))
        {
 ...
 * NB: Index is the number of the corresponding interface, not of the address
 */
static int Address_Scan_Next(short* Index, struct ifnet* Retifnet)
{
    ...
    while (ifr)
    {
        ...
        if (Retifnet)
        {
            ...
            {
                memset((char*)&Retifnet->ifu_broadaddr, 0,
                       sizeof(Retifnet->ifu_broadaddr));
            }
            else
            ...
            {
                memset((char*)&Retifnet->ia_subnetmask, 0,
                       sizeof(Retifnet->ia_subnetmask));
            }
            else
    ...
    static int IP_Cmp(void* addr, void* ep)
{
...
#define IP_ADDRINDEX_OFF 10
    ...
    memcpy((char*)current, (char*)vp->name, (int)vp->namelen * sizeof(oid));
    if (*length ==
        IP_ADDRNAME_LENGTH) /* Assume that the input name is the lowest */
         ...
         NextAddr = entry.ipAdEntAddr, req_type = GET_NEXT)
    {
        ...
        if (exact)
        {
            if (snmp_oid_compare(current, IP_ADDRNAME_LENGTH, name, *length) ==
                0)
            {
        ...
        else
        {
            if ((snmp_oid_compare(current, IP_ADDRNAME_LENGTH, name, *length) >
                 0) &&
                (((NextAddr == (u_long)-1)) ||
                 (snmp_oid_compare(current, IP_ADDRNAME_LENGTH, lowest,
                                   IP_ADDRNAME_LENGTH) < 0) ||
                 (snmp_oid_compare(name, *length, lowest, IP_ADDRNAME_LENGTH) ==
                  0)))
            {
                /*
                 * if new one is greater than input and closer to input than
                 * previous lowest, and is not equal to it, save this one as the
                 * "next" one.
                 */
                Lowentry = entry;
                Found++;
    ...
    *var_len = sizeof(long_return);
    switch (vp->magic)
    {
        ...
        case IPADIFINDEX:
            ...
            Lowentry.ipAdEntIfIndex.o_bytes[Lowentry.ipAdEntIfIndex.o_length] =
                '\0';
            long_return = netsnmp_access_interface_index_find(
                Lowentry.ipAdEntIfIndex.o_bytes);
#else
            long_return =
...
loop:
    ...
    while (cp < &ifbuf[len])
    {
        ...
        while (cp < &ifbuf[len] && rtm->rtm_type == RTM_NEWADDR)
        {
             ...
             * from route.c
             */
#define ROUND(a) ((a) > 0 ? (1 + (((a)-1) | (sizeof(long) - 1))) : sizeof(long))
            ...
            *op++ = *cp++;
            ...
            else
            {
                if (snmp_oid_compare(current, 14, name, *length) > 0)
                {
                    lowinterface = i;
                    memcpy(lowest, current, 14 * sizeof(oid));
                    break; /* Since the table is sorted, no need to search
                              further  */
                }
            }
        }
