...
static void (*getfnc_fast_random_poll(void))(void (*)(const void*, size_t,
                                                      enum random_origins),
                                             enum random_origins);
static void read_random_source(enum random_origins origin, size_t length,
                               int level);
...
/* Basic initialization which is required to initialize mutexes and
   ...
   happens. */
static void initialize_basics(void)
{
...
static void initialize(void)
{
    ...
    if (!rndpool)
    {
        ...
        rndpool = (secure_alloc ? xcalloc_secure(1, POOLSIZE + BLOCKLEN)
                                : xcalloc(1, POOLSIZE + BLOCKLEN));
        keypool = (secure_alloc ? xcalloc_secure(1, POOLSIZE + BLOCKLEN)
                                : xcalloc(1, POOLSIZE + BLOCKLEN));
...
void _gcry_rngcsprng_dump_stats(void)
{
    ...
    log_info("random usage: poolsize=%d mixed=%lu polls=%lu/%lu added=%lu/%lu\n"
             ...
             rndstats.naddbytes, rndstats.addbytes, rndstats.mixkey,
             rndstats.ngetbytes1, rndstats.getbytes1, rndstats.ngetbytes2,
             rndstats.getbytes2,
 ...
 */
static void mix_pool(unsigned char* pool)
{
...
static int lock_seed_file(int fd, const char* fname, int for_write)
{
#ifdef __GCC__
...
static int read_seed_file(void)
{
    int fd;
    struct stat sb;
...
void _gcry_rngcsprng_update_seed_file(void)
{
    ...
    for (i = 0, dp = (unsigned long*)(void*)keypool,
        sp = (unsigned long*)(void*)rndpool;
         i < POOLWORDS; i++, dp++, sp++)
    {
        *dp = *sp + ADD_VALUE;
    }
...
static void random_poll()
{
    rndstats.slowpolls++;
    read_random_source(RANDOM_ORIGIN_SLOWPOLL, POOLSIZE / 5,
                       GCRY_STRONG_RANDOM);
}
...
#if USE_RNDLINUX
    if (!access(NAME_OF_DEV_RANDOM, R_OK) && !access(NAME_OF_DEV_URANDOM, R_OK))
    {
