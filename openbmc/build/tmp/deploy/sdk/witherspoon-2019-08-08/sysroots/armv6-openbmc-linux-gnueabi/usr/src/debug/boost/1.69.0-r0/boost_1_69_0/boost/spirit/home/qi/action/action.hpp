...
struct action : unary_parser<action<Subject, Action>>
{
    ...
    action(Subject const& subject_, Action f_) : subject(subject_), f(f_)
    {
    }
    ...
    {
        Iterator save = first;
        if (subject.parse(first, last, context, skipper,
                          attr)) // Use the attribute as-is
        {
            // call the function, passing the attribute, the context.
            // The client can return false to fail parsing.
            if (traits::action_dispatch<Subject>()(f, attr, context))
    ...
    template <typename Iterator, typename Context, typename Skipper>
    bool parse(Iterator& first, Iterator const& last, Context& context,
               Skipper const& skipper, unused_type) const
    {
        typedef typename attribute<Context, Iterator>::type attr_type;
...
struct make_component<qi::domain, tag::action>
{
    ...
    {
        typedef typename remove_const<typename Elements::car_type>::type
            subject_type;
    ...
    {
        typename result<make_component(Elements, unused_type)>::type result(
            elements.car, elements.cdr.car);
        return result;
    }
};
} // namespace spirit
...
struct handles_container<qi::action<Subject, Action>, Attribute, Context,
                         Iterator>
