...
#define G_REGEX_COMPILE_MASK                                                   \
    (G_REGEX_CASELESS | G_REGEX_MULTILINE | G_REGEX_DOTALL |                   \
     G_REGEX_EXTENDED | G_REGEX_ANCHORED | G_REGEX_DOLLAR_ENDONLY |            \
     G_REGEX_UNGREEDY | G_REGEX_RAW | G_REGEX_NO_AUTO_CAPTURE |                \
     G_REGEX_OPTIMIZE | G_REGEX_FIRSTLINE | G_REGEX_DUPNAMES |                 \
     G_REGEX_NEWLINE_CR | G_REGEX_NEWLINE_LF | G_REGEX_NEWLINE_CRLF |          \
     G_REGEX_NEWLINE_ANYCRLF | G_REGEX_BSR_ANYCRLF |                           \
     G_REGEX_JAVASCRIPT_COMPAT)

/* Mask of all GRegexCompileFlags values that are (not) passed trough to PCRE */
#define G_REGEX_COMPILE_PCRE_MASK                                              \
    (G_REGEX_COMPILE_MASK & ~G_REGEX_COMPILE_NONPCRE_MASK)
#define G_REGEX_COMPILE_NONPCRE_MASK (G_REGEX_RAW | G_REGEX_OPTIMIZE)

/* Mask of all the possible values for GRegexMatchFlags. */
#define G_REGEX_MATCH_MASK                                                     \
    (G_REGEX_MATCH_ANCHORED | G_REGEX_MATCH_NOTBOL | G_REGEX_MATCH_NOTEOL |    \
     G_REGEX_MATCH_NOTEMPTY | G_REGEX_MATCH_PARTIAL |                          \
     G_REGEX_MATCH_NEWLINE_CR | G_REGEX_MATCH_NEWLINE_LF |                     \
     G_REGEX_MATCH_NEWLINE_CRLF | G_REGEX_MATCH_NEWLINE_ANY |                  \
     G_REGEX_MATCH_NEWLINE_ANYCRLF | G_REGEX_MATCH_BSR_ANYCRLF |               \
     G_REGEX_MATCH_BSR_ANY | G_REGEX_MATCH_PARTIAL_SOFT |                      \
     G_REGEX_MATCH_PARTIAL_HARD | G_REGEX_MATCH_NOTEMPTY_ATSTART)
 ...
 * use just +/- 1. */
#define NEXT_CHAR(re, s)                                                       \
    (((re)->compile_opts & G_REGEX_RAW) ? ((s) + 1) : g_utf8_next_char(s))
#define PREV_CHAR(re, s)                                                       \
    (((re)->compile_opts & G_REGEX_RAW) ? ((s)-1) : g_utf8_prev_char(s))
...
struct _GRegex
{
    ...
    GRegexCompileFlags
        compile_opts; /* options used at compile time on the pattern */
    GRegexMatchFlags match_opts; /* options used at match time on the regex */
    pcre_extra* extra;           /* data stored when G_REGEX_OPTIMIZE is used */
};

/* TRUE if ret is an error code, FALSE otherwise. */
#define IS_PCRE_ERROR(ret)                                                     \
    ((ret) < PCRE_ERROR_NOMATCH && (ret) != PCRE_ERROR_PARTIAL)
...
static const gchar* match_error(gint errcode)
{
    switch (errcode)
    {
        ...
        case PCRE_ERROR_BADPARTIAL:
            return _("the pattern contains items not supported for partial "
                     "matching");
        ...
        case PCRE_ERROR_DFA_UCOND:
            return _("back references as conditions are not supported for "
                     "partial matching");
...
static void translate_compile_error(gint* errcode, const gchar** errmsg)
{
    ...
    *errcode += 100;
    ...
    switch (*errcode)
    {
        ...
        case 129:
            *errcode = G_REGEX_ERROR_UNMATCHED_PARENTHESIS;
            /* translators: '(?R' and '(?[+-]digits' are both meant as (groups
             * of) sequences here, '(?-54' would be an example for the second
             * group.
             */
        ...
        case 152: /* internal error: overran compiling workspace */
            ...
            *errmsg = _("overran compiling workspace");
            break;
        case 153: /* internal error: previously-checked referenced subpattern
                     not found */
            *errcode = G_REGEX_ERROR_INTERNAL;
        ...
        case G_REGEX_ERROR_INVALID_DATA_CHARACTER:
            *errmsg = _("] is an invalid data character in JavaScript "
                        "compatibility mode");
            break;
        case G_REGEX_ERROR_EXTRA_SUBPATTERN_NAME:
            *errmsg = _("different names for subpatterns of the same number "
                        "are not allowed");
            break;
        ...
        case G_REGEX_ERROR_MISSING_NAME:
            *errmsg = _("\\k is not followed by a braced, angle-bracketed, or "
                        "quoted name");
            break;
        ...
        case 167: /* this version of PCRE is not compiled with Unicode property
                     support */
        case 173: /* disallowed Unicode code point (>= 0xd800 && <= 0xdfff) */
        case 174: /* invalid UTF-16 string */
                  /* These errors should not happen as we are using an UTF-8 and
                   * UCP-enabled PCRE       and we do not check if strings are valid */
...
static GMatchInfo* match_info_new(const GRegex* regex, const gchar* string,
                                  gint string_len, gint start_position,
                                  gint match_options, gboolean is_dfa)
{
    ...
    {
        gint capture_count;
        pcre_fullinfo(regex->pcre_re, regex->extra, PCRE_INFO_CAPTURECOUNT,
                      &capture_count);
...
gboolean g_match_info_next(GMatchInfo* match_info, GError** error)
{
    ...
    match_info->matches =
        pcre_exec(match_info->regex->pcre_re, match_info->regex->extra,
                  match_info->string, match_info->string_len, match_info->pos,
                  match_info->regex->match_opts | match_info->match_opts,
                  match_info->offsets, match_info->n_offsets);
    if (IS_PCRE_ERROR(match_info->matches))
    {
        g_set_error(error, G_REGEX_ERROR, G_REGEX_ERROR_MATCH,
                    _("Error while matching regular expression %s: %s"),
                    match_info->regex->pattern,
                    match_error(match_info->matches));
        return FALSE;
    }
    ...
    if (match_info->pos == match_info->offsets[1])
    {
        ...
        match_info->pos =
            NEXT_CHAR(match_info->regex, &match_info->string[match_info->pos]) -
            match_info->string;
    }
    else
    {
 ...
 * Since: 2.14
 */
gboolean g_match_info_fetch_pos(const GMatchInfo* match_info, gint match_num,
 ...
 */
static gint get_matched_substring_number(const GMatchInfo* match_info,
                                         const gchar* name)
{
    gint entrysize;
    gchar *first, *last;
    ...
    entrysize = pcre_get_stringtable_entries(match_info->regex->pcre_re, name,
                                             &first, &last);
 ...
 * Since: 2.14
 */
GRegex* g_regex_new(const gchar* pattern, GRegexCompileFlags compile_options,
                    GRegexMatchFlags match_options, GError** error)
{
    GRegex* regex;
    pcre* re;
    ...
    if (g_once_init_enter(&initialised))
    {
        ...
        if (!supports_ucp)
            g_critical(
                _("PCRE library is compiled without UTF8 properties support"));
    ...
    if (optimize)
    {
        ...
        {
            GError* tmp_error =
                g_error_new(G_REGEX_ERROR, G_REGEX_ERROR_OPTIMIZE,
                            _("Error while optimizing "
                              "regular expression %s: %s"),
                            regex->pattern, errmsg);
            g_propagate_error(error, tmp_error);
                           ...
                           GRegexMatchFlags* match_options, GError** error)
{
    ...
    /* compile the pattern */
    re = pcre_compile2(pattern, compile_options, &errcode, &errmsg, &erroffset,
                       NULL);
...
gboolean g_regex_get_has_cr_or_lf(const GRegex* regex)
{
    ...
    pcre_fullinfo(regex->pcre_re, regex->extra, PCRE_INFO_HASCRORLF, &value);
...
gint g_regex_get_max_lookbehind(const GRegex* regex)
{
    ...
    pcre_fullinfo(regex->pcre_re, regex->extra, PCRE_INFO_MAXLOOKBEHIND,
                  &max_lookbehind);
 ...
 * Since: 2.14
 */
gboolean g_regex_match_simple(const gchar* pattern, const gchar* string,
                              GRegexCompileFlags compile_options,
                              GRegexMatchFlags match_options)
{
    ...
    if (!regex)
        return FALSE;
    result =
        g_regex_match_full(regex, string, -1, 0, match_options, NULL, NULL);
    g_regex_unref(regex);
    return result;
}
...
{
    return g_regex_match_full(regex, string, -1, 0, match_options, match_info,
                              NULL);
}
 ...
 * @string_len: the length of @string, in bytes, or -1 if @string is
 * nul-terminated
 ...
 * Since: 2.14
 */
gboolean g_regex_match_full(const GRegex* regex, const gchar* string,
                                ...
                                GMatchInfo** match_info, GError** error)
{
    ...
    pcre_re = regex_compile(regex->pattern,
                            regex->compile_opts | PCRE_NO_AUTO_POSSESS, NULL,
                            NULL, error);
    ...
    while (!done)
    {
        ...
        info->matches = pcre_dfa_exec(
            pcre_re, extra, info->string, info->string_len, info->pos,
            regex->match_opts | match_options, info->offsets, info->n_offsets,
        ...
        if (info->matches == PCRE_ERROR_DFA_WSSIZE)
        {
            ...
            info->n_workspace *= 2;
            info->workspace =
                g_realloc(info->workspace, info->n_workspace * sizeof(gint));
            done = FALSE;
        }
        else if (info->matches == 0)
        {
            ...
            info->n_offsets *= 2;
            info->offsets =
                g_realloc(info->offsets, info->n_offsets * sizeof(gint));
            done = FALSE;
        }
 ...
 * Since: 2.14
 **/
gchar** g_regex_split_simple(const gchar* pattern, const gchar* string,
                             GRegexCompileFlags compile_options,
                             GRegexMatchFlags match_options)
{
    GRegex* regex;
...
gchar** g_regex_split_full(const GRegex* regex, const gchar* string,
                           gssize string_len, gint start_position,
                           GRegexMatchFlags match_options, gint max_tokens,
                           GError** error)
{
    GError* tmp_error = NULL;
    GMatchInfo* match_info;
    ...
    if (max_tokens == 1)
    {
        ...
        string_list[0] =
            g_strndup(&string[start_position], string_len - start_position);
        return string_list;
    }
    ...
    while (tmp_error == NULL)
    {
            ...
            if (last_separator_end != match_info->offsets[1])
            {
                ...
                if (match_count > 1)
                {
                    for (i = 1; i < match_count; i++)
                        list = g_list_prepend(
                            list, g_match_info_fetch(match_info, i));
                }
            }
        }
        else
        {
            ...
            {
                gchar* token =
                    g_strndup(string + last_separator_end,
        ...
        if (token_count >= max_tokens - 1)
        {
            ...
            if (last_match_is_empty)
            {
                 ...
                 * same position. */
                match_info->pos =
                    PREV_CHAR(regex, &string[match_info->pos]) - string;
            }
        ...
        if (last_match_is_empty)
             ...
             * character. */
            last_separator_end =
                PREV_CHAR(regex, &string[last_separator_end]) - string;
...
typedef enum
{
    ...
    CHANGE_CASE_SINGLE_MASK =
        CHANGE_CASE_UPPER_SINGLE | CHANGE_CASE_LOWER_SINGLE,
...
static const gchar* expand_escape(const gchar* replacement, const gchar* p,
                                  InterpolationData* data, GError** error)
{
    const gchar *q, *r;
    gint x, d, h, i;
    ...
    tmp_error =
        g_error_new(G_REGEX_ERROR, G_REGEX_ERROR_REPLACE,
                    _("Error while parsing replacement "
                      "text “%s” at char %lu: %s"),
                    replacement, (gulong)(p - replacement), error_detail);
    g_propagate_error(error, tmp_error);
...
#define CHANGE_CASE(c, change_case)                                            \
    (((change_case)&CHANGE_CASE_LOWER_MASK) ? g_unichar_tolower(c)             \
                                            : g_unichar_toupper(c))

static void string_append(GString* string, const gchar* text,
                          ChangeCase* change_case)
{
    gunichar c;
 ...
 * Since: 2.14
 */
gchar* g_regex_replace(const GRegex* regex, const gchar* string,
                       gssize string_len, gint start_position,
                       const gchar* replacement, GRegexMatchFlags match_options,
                       GError** error)
{
    gchar* result;
    GList* list;
    ...
    result = g_regex_replace_eval(regex, string, string_len, start_position,
                                  match_options, interpolate_replacement,
                                  (gpointer)list, &tmp_error);
    if (tmp_error != NULL)
        g_propagate_error(error, tmp_error);
...
static gboolean literal_replacement(const GMatchInfo* match_info,
                                    GString* result, gpointer data)
{
                               ...
                               GRegexMatchFlags match_options, GError** error)
{
    ...
    return g_regex_replace_eval(regex, string, string_len, start_position,
                                match_options, literal_replacement,
                                (gpointer)replacement, error);
}
                            ...
                            GError** error)
{
    ...
    g_regex_match_full(regex, string, string_len, start_position, match_options,
                       &match_info, &tmp_error);
    while (!done && g_match_info_matches(match_info))
    {
        g_string_append_len(result, string + str_pos,
...
gboolean g_regex_check_replacement(const gchar* replacement,
                                   gboolean* has_references, GError** error)
{
    GList* list;
    GError* tmp = NULL;
