...
static void getRegKeyValue(HKEY key, const char* keyPath, const char* valueName,
 ...
 */
OM_uint32 KRB5_CALLCONV gss_release_oid(minor_status,
                                        oid) OM_uint32* minor_status;
gss_OID* oid;
{
    OM_uint32 major;
    gss_mech_info aMech;
    ...
    *minor_status = gssint_mechglue_initialize_library();
    ...
    while (aMech != NULL)
    {
        ...
        {
            major = aMech->mech->gss_internal_release_oid(minor_status, oid);
 ...
 */
static int is_deprecated(gss_OID element)
{
 ...
 */
static void prune_deprecated(gss_OID_set mech_set)
{
    OM_uint32 i, j;
 ...
 */
OM_uint32 KRB5_CALLCONV gss_indicate_mechs(minorStatus,
                                           mechSet_out) OM_uint32* minorStatus;
...
static OM_uint32 build_mechSet(void)
{
    ...
    if (count > 0)
    {
        g_mechSet.elements = (gss_OID)calloc(count, sizeof(gss_OID_desc));
        ...
        (void)memset(g_mechSet.elements, 0, count * sizeof(gss_OID_desc));
        ...
        for (mList = g_mechList; mList != NULL; mList = mList->next)
        {
            ...
            curItem->elements = (void*)malloc(mList->mech_type->length);
 ...
 */
char* gssint_get_modOptions(oid) const gss_OID oid;
{
    gss_mech_info aMech;
...
static time_t check_link_mtime(const char* filename, time_t* mtime_out)
{
    struct stat st1, st2;
...
static void load_if_changed(const char* pathname, time_t last, time_t* highest)
{
    time_t mtime;
...
static void releaseMechInfo(gss_mech_info* pCf)
{
    gss_mech_info cf;
    ...
    if (cf->optionStr != NULL)
        free(cf->optionStr);
    if (cf->mech_type != GSS_C_NO_OID && cf->mech_type != &cf->mech->mech_type)
...
int gssint_register_mechinfo(gss_mech_info template)
{
    ...
    *new_cf->mech = *template->mech;
    ...
    {
        if (cf->next == NULL || new_cf->priority < cf->next->priority)
        {
            new_cf->next = cf->next;
            cf->next = new_cf;
...
#define GSS_ADD_DYNAMIC_METHOD(_dl, _mech, _symbol)                            \
    ...
    {                                                                          \
        ...
        if (krb5int_get_plugin_func(                                           \
                _dl, #_symbol, (void (**)()) & (_mech)->_symbol, &errinfo) ||  \
            errinfo.code)                                                      \
        {                                                                      \
            (_mech)->_symbol = NULL;                                           \
...
static gss_mechanism build_dynamicMech(void* dl, const gss_OID mech_type)
{
    gss_mechanism mech;
...
#define RESOLVE_GSSI_SYMBOL(_dl, _mech, _psym, _nsym)                          \
    ...
    {                                                                          \
        struct errinfo errinfo;                                                \
        memset(&errinfo, 0, sizeof(errinfo));                                  \
        if (krb5int_get_plugin_func(_dl, "gssi" #_nsym,                        \
...
static gss_mechanism build_interMech(void* dl, const gss_OID mech_type)
{
    gss_mechanism mech;
 ...
 */
static gss_OID interposed_oid(gss_OID pre, gss_OID real)
{
    gss_OID o;
    ...
    memcpy((char*)o->elements + pre->length, real->elements, real->length);

    return o;
}

static void loadInterMech(gss_mech_info minfo)
{
    ...
    if (krb5int_get_plugin_func(dl, MECH_INTERPOSER_SYM, (void (**)()) & isym,
                                &errinfo) != 0)
        goto cleanup;
    ...
    *public_set = GSS_C_NO_OID_SET;
    ...
    for (i = 0; i < count; i++)
    {
        ...
        if (public_oid == GSS_C_NO_OID)
            continue;
        status = generic_gss_add_oid_set_member(minor_status, public_oid, &set);
        if (GSS_ERROR(status))
        {
 ...
 */
gss_mechanism gssint_get_mechanism(gss_const_OID oid)
{
    gss_mech_info aMech;
    ...
    if (krb5int_get_plugin_func(dl, MECH_SYM, (void (**)()) & sym, &errinfo) ==
        0)
    {
...
static char* delimit_ws(char* str)
{
 ...
 * non-whitespace character after that point. */
static char* delimit(char* str, char delimiter)
{
...
static void getRegKeyValue(HKEY hKey, const char* keyPath,
                           const char* valueName, void** data, DWORD* dataLen)
{
    DWORD sizeRequired = *dataLen;
    HRESULT hr;
    /* Get data length required */
    if ((hr = RegGetValue(hKey, keyPath, valueName, RRF_RT_REG_SZ, NULL, NULL,
                          &sizeRequired)) != ERROR_SUCCESS)
    {
    ...
    /* get data */
    if ((hr = RegGetValue(hKey, keyPath, valueName, RRF_RT_REG_SZ, NULL, *data,
                          &sizeRequired)) != ERROR_SUCCESS)
    {
        /* LOG registry error */
        return;
...
static void loadConfigFromRegistry(HKEY hBaseKey, const char* keyPath)
{
    HKEY hConfigKey;
    ...
    if ((rc = RegQueryInfoKey(hConfigKey, NULL, /* lpClass */
                              NULL,             /* lpcClass */
                              NULL,             /* lpReserved */
                              &nSubKeys, &maxSubKeyNameLen,
                              NULL, /* lpcMaxClassLen */
                              NULL, /* lpcValues */
    ...
    for (iSubKey = 0; iSubKey < nSubKeys; iSubKey++)
    {
        ...
        if ((rc = RegEnumKeyEx(hConfigKey, iSubKey, oidStr, &oidStrLen, NULL,
        ...
        getRegKeyValue(hConfigKey, oidStr, "Shared Library", &sharedLib,
                       &sharedLibLen);
        getRegKeyValue(hConfigKey, oidStr, "Kernel Module", &kernMod,
                       &kernModLen);
        getRegKeyValue(hConfigKey, oidStr, "Options", &modOptions,
                       &modOptionsLen);
        getRegKeyValue(hConfigKey, oidStr, "Type", &modType, &modTypeLen);
        addConfigEntry(oidStr, oid, sharedLib, kernMod, modOptions, modType);
    }
cleanup:
    RegCloseKey(hConfigKey);
                           ...
                           const char* modOptions, const char* modType)
{
    ...
    if (generic_gss_str_to_oid(&minor, &oidBuf, &mechOid) != GSS_S_COMPLETE)
    {
        return;
    }
    ...
    if (sharedLib[0] == '/')
        ...
        snprintf(sharedPath, sizeof(sharedPath), "%s%s", MECH_LIB_PREFIX,
                 sharedLib);
#endif
    /*
