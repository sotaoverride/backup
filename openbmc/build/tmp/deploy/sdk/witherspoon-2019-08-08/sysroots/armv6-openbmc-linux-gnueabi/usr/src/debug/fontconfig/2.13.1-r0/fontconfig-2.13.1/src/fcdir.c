...
static FcBool FcFileScanFontConfig(FcFontSet* set, const FcChar8* file,
                                   FcConfig* config)
{
    int i;
    FcBool ret = FcTrue;
    ...
    for (i = old_nfont; i < set->nfont; i++)
    {
        ...
        /*
         * Get rid of sysroot here so that targeting scan rule may contains
         * FC_FILE pattern and they should usually expect without sysroot.
         */
        if (sysroot)
        {
            ...
            if (FcPatternObjectGetString(font, FC_FILE_OBJECT, 0, &f) ==
                    FcResultMatch &&
...
FcBool FcFileScanConfig(FcFontSet* set, FcStrSet* dirs, const FcChar8* file,
                        FcConfig* config)
{
    if (FcFileIsDir(file))
    {
...
FcBool FcFileScan(FcFontSet* set, FcStrSet* dirs, FcFileCache* cache FC_UNUSED,
                  FcBlanks* blanks FC_UNUSED, const FcChar8* file,
...
FcBool FcDirScanConfig(FcFontSet* set, FcStrSet* dirs, const FcChar8* dir,
...
FcBool FcDirScan(FcFontSet* set, FcStrSet* dirs, FcFileCache* cache FC_UNUSED,
                 FcBlanks* blanks FC_UNUSED, const FcChar8* dir,
                 FcBool force FC_UNUSED)
{
    if (cache || !force)
        return FcFalse;
...
FcBool FcDirSave(FcFontSet* set FC_UNUSED, FcStrSet* dirs FC_UNUSED,
                 const FcChar8* dir FC_UNUSED)
{
