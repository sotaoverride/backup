...
static inline unsigned int nl_calc_hash(int cat, int id1, int id2, int id3,
                                        int id4)
{
    unsigned int h;
...
static struct nl_entry* nl_lookup(struct pci_access* a, int num, int cat,
                                  int id1, int id2, int id3, int id4)
{
    ...
    n = a->nl_hash[h];
    while (n && (n->id1 != id1 || n->id2 != id2 || n->id3 != id3 ||
                 n->id4 != id4 || n->cat != cat))
        n = n->next;
...
static int nl_add(struct pci_access* a, int cat, int id1, int id2, int id3,
                  int id4, unsigned char* text)
{
...
static void err_name_list(struct pci_access* a, unsigned char* msg)
{
    fprintf(stderr, "%s: %s: %s\n", a->pci_id_file_name, msg, strerror(errno));
}

static void parse_name_list(struct pci_access* a)
{
    ...
    while (*p)
    {
        ...
        if (q == r)
        {
            ...
            {
                if (strlen(q) < 5 || q[4] != ' ' || sscanf(q, "%x", &id1) != 1)
                    goto parserr;
                cat = NL_VENDOR;
            }
            id2 = id3 = id4 = 0;
        ...
        if (nl_add(a, cat, id1, id2, id3, id4, q))
            fprintf(stderr, "%s, line %d: duplicate entry", a->pci_id_file_name,
                    lino);
    }
    return;
...
static void load_name_list(struct pci_access* a)
{
    int fd;
    struct stat st;
...
unsigned char* pci_lookup_name(struct pci_access* a, unsigned char* buf,
                               int size, int flags, unsigned int arg1,
    ...
    switch (flags)
    {
        ...
        case PCI_LOOKUP_VENDOR | PCI_LOOKUP_DEVICE:
            if (!num)
            {
                ...
                if (!e)
                    res = snprintf(buf, size, "Unknown device %04x:%04x", arg1,
                                   arg2);
                else if (!e2)
                    res = snprintf(buf, size, "%s: Unknown device %04x",
                                   e->name, arg2);
                else
        ...
        case PCI_LOOKUP_DEVICE | PCI_LOOKUP_SUBSYSTEM:
            if ((n = nl_lookup(a, num, NL_SUBSYSTEM, arg1, arg2, arg3, arg4)))
                return n->name;
                     ...
                     (n = nl_lookup(a, num, NL_DEVICE, arg1, arg2, 0, 0)))
                return n->name;
            else
        ...
        case PCI_LOOKUP_VENDOR | PCI_LOOKUP_DEVICE | PCI_LOOKUP_SUBSYSTEM:
            if (!num)
            {
                ...
                if (!e2 && arg1 == arg3 && arg2 == arg4)
                    /* Cheat for vendors blindly setting subsystem ID same as
                     * device ID */
                    e2 = nl_lookup(a, 0, NL_DEVICE, arg1, arg2, 0, 0);
                if (!e)
                    res = snprintf(buf, size, "Unknown device %04x:%04x", arg3,
                                   arg4);
                else if (!e2)
                    res = snprintf(buf, size, "%s: Unknown device %04x",
                                   e->name, arg4);
        ...
        case PCI_LOOKUP_CLASS:
            if ((n = nl_lookup(a, num, NL_SUBCLASS, arg1 >> 8, arg1 & 0xff, 0,
                               0)))
                return n->name;
        ...
        case PCI_LOOKUP_PROGIF:
            if ((n = nl_lookup(a, num, NL_PROGIF, arg1 >> 8, arg1 & 0xff, arg2,
                               0)))
                return n->name;
            if (arg1 == 0x0101)
            {
                ...
                if (arg2 & 0x70)
                    return NULL;
                res = snprintf(
                    buf, size, "%s%s%s%s%s", (arg2 & 0x80) ? "Master " : "",
                    (arg2 & 0x08) ? "SecP " : "", (arg2 & 0x04) ? "SecO " : "",
                    (arg2 & 0x02) ? "PriP " : "", (arg2 & 0x01) ? "PriO " : "");
                if (res)
                    buf[--res] = 0;
                break;
            }
