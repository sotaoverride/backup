 ...
 * the same as smtradfile.c, except that we do have a RFC3339 timestamp.
 * However, we have copied over the code from there, it is too simple to go
 * through all the hassle of having a single code base.
 *
...
/* This strgen tries to minimize the amount of reallocs be first obtaining
 * pointers to all strings needed (including their length) and then calculating
 * the actual space required. So when we finally copy, we know exactly what we
 * need. So we do at most one alloc.
 */
    ...
    : ENDstrgen
          ...
          BEGINmodExit CODESTARTmodExit ENDmodExit
              ...
              BEGINqueryEtryPt CODESTARTqueryEtryPt
                  CODEqueryEtryPt_STD_SMOD_QUERIES ENDqueryEtryPt

                      BEGINmodInit(smfile) CODESTARTmodInit* ipIFVersProvided =
                          CURR_MOD_IF_VERSION; /* we only support the current
                                                  interface specification */
CODEmodInit_QueryRegCFSLineHdlr

    dbgprintf("rsyslog standard file format strgen init called, compiled with "
              "version %s\n",
              VERSION);
ENDmodInit
