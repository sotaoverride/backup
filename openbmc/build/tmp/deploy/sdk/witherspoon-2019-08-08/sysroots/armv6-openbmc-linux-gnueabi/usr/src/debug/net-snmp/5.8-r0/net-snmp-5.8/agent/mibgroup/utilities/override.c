 ...
 *  read-write */
int override_handler(netsnmp_mib_handler* handler,
...
void netsnmp_parse_override(const char* token, char* line)
{
...

    {
        struct
        {
            const char* key;
            int value;
        } const strings[] = {{"counter", ASN_COUNTER},
    ...
    the_reg->handler = netsnmp_create_handler("override", override_handler);
...
void init_override(void)
{
    ...
    snmpd_register_config_handler("override", netsnmp_parse_override,
                                  NULL, /* XXX: free func */
