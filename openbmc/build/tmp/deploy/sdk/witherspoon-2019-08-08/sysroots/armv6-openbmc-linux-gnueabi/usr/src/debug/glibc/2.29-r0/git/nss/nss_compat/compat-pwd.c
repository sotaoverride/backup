...
static enum nss_status (*nss_getpwnam_r)(const char* name, struct passwd* pwd,
...
static enum nss_status internal_endpwent(ent_t* ent)
{
    if (ent->stream != NULL)
    {
        fclose(ent->stream);
...
static enum nss_status getpwent_next_nss_netgr(const char* name,
                                               struct passwd* result,
                                               ent_t* ent, char* group,
                                               char* buffer, size_t buflen,
                                               int* errnop)
{
    ...
    while (1)
    {
        status = __internal_getnetgrent_r(
            &host, &user, &domain, &ent->netgrdata, buffer, buflen, errnop);
        if (status != 1)
        {
            __internal_endnetgrent(&ent->netgrdata);
            ent->netgroup = 0;
        ...
        if (domain != NULL)
        {
            if (curdomain == NULL && __nss_get_default_domain(&curdomain) != 0)
            {
                __internal_endnetgrent(&ent->netgrdata);
                ent->netgroup = false;
                give_pwd_free(&ent->pwd);
...
static enum nss_status getpwent_next_nss(struct passwd* result, ent_t* ent,
                                         char* buffer, size_t buflen,
                                         int* errnop)
{
...
static enum nss_status getpwnam_plususer(const char* name,
                                         struct passwd* result, ent_t* ent,
                                         char* buffer, size_t buflen,
                                         int* errnop)
{
    if (!nss_getpwnam_r)
    ...
    enum nss_status status =
        nss_getpwnam_r(name, result, buffer, buflen, errnop);
    ...
    while (1)
    {
        ...
        do
        {
            ...
            while (isspace(*p))
                 ...
                 *p == '#' || /* Ignore empty and comment lines.  */
            ...
            result->pw_name[2] != '\0')
        {
            ...
            status = getpwent_next_nss_netgr(
                NULL, result, ent, &result->pw_name[2], buffer, buflen, errnop);
            ...
            result->pw_name[1] != '@')
        {
            ...
            status = getpwnam_plususer(&result->pw_name[1], result, ent, buffer,
                                       buflen, errnop);
            ...
            if (status == NSS_STATUS_SUCCESS) /* We found the entry. */
                break;
                         ...
                         NSS_STATUS_RETURN /* We couldn't parse the entry */
...
static enum nss_status internal_getpwent_r(struct passwd* pw, ent_t* ent,
                                           char* buffer, size_t buflen,
                                           int* errnop)
{
    if (ent->netgroup)
...
enum nss_status _nss_compat_getpwent_r(struct passwd* pwd, char* buffer,
                                       size_t buflen, int* errnop)
{
...
static enum nss_status internal_getpwnam_r(const char* name,
                                           struct passwd* result, ent_t* ent,
                                           char* buffer, size_t buflen,
                                           int* errnop)
{
    ...
    while (1)
    {
        ...
        if (result->pw_name[0] == '+' && result->pw_name[1] == '\0')
        {
            ...
            status =
                getpwnam_plususer(name, result, ent, buffer, buflen, errnop);
            if (status == NSS_STATUS_SUCCESS) /* We found the entry. */
                break;
            else if (status ==
                     NSS_STATUS_RETURN) /* We couldn't parse the entry */
...
static enum nss_status getpwuid_plususer(uid_t uid, struct passwd* result,
                                         char* buffer, size_t buflen,
                                         int* errnop)
{
    struct passwd pwd;
...
static enum nss_status internal_getpwuid_r(uid_t uid, struct passwd* result,
                                           ent_t* ent, char* buffer,
                                           size_t buflen, int* errnop)
{
    struct parser_data* data = (void*)buffer;
