...
struct use_lazy_directive<qi::domain, tag::repeat, 1 // arity
                          > : mpl::true_
{
};
...
{
    exact_iterator(T const exact_) : exact(exact_)
    {
    }
...
struct finite_iterator // handles repeat(min, max)[p]
{
    finite_iterator(T const min_, T const max_) :
        min BOOST_PREVENT_MACRO_SUBSTITUTION(min_), max
        BOOST_PREVENT_MACRO_SUBSTITUTION(max_)
    {
    }
...
{
    infinite_iterator(T const min_) : min BOOST_PREVENT_MACRO_SUBSTITUTION(min_)
    {
    }
...
struct repeat_parser : unary_parser<repeat_parser<Subject, LoopIter>>
{
    ...
    struct attribute
    {
        ...
        typedef typename traits::build_std_vector<typename traits::attribute_of<
            Subject, Context, Iterator>::type>::type type;
    };
...
struct make_directive<terminal_ex<tag::repeat, fusion::vector1<T>>, Subject,
                      Modifiers>
{
...
template <typename T, typename Subject, typename Modifiers>
struct make_directive<terminal_ex<tag::repeat, fusion::vector2<T, T>>, Subject,
                      Modifiers>
{
    ...
    result_type operator()(Terminal const& term, Subject const& subject,
                           unused_type) const
    {
        return result_type(subject, iterator_type(fusion::at_c<0>(term.args),
                                                  fusion::at_c<1>(term.args)));
    }
};

template <typename T, typename Subject, typename Modifiers>
struct make_directive<terminal_ex<tag::repeat, fusion::vector2<T, inf_type>>,
                      Subject, Modifiers>
{
