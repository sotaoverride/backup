...
DEFobjCurrIf(glbl) DEFobjCurrIf(parser) DEFobjCurrIf(datetime)
    ...
    /* static data */
    static int bParseHOSTNAMEandTAG; /* cache for the equally-named global param
...
BEGINnewParserInst struct cnfparamvals* pvals = NULL;
int i;
...
for (i = 0; i < parserpblk.nParams; ++i)
{
    ...
    else if (!strcmp(parserpblk.descr[i].name,
...
finalize_it : CODE_STD_FINALIZERnewParserInst if (lst != NULL)
                  cnfparamvalsDestruct(pvals, &parserpblk);
if (iRet != RS_RET_OK)
    free(inst);
ENDnewParserInst

    BEGINfreeParserInst CODESTARTfreeParserInst
        dbgprintf("pmrfc3164: free parser instance %p\n", pInst);
ENDfreeParserInst

    /* parse a legay-formatted syslog message.
     */
    BEGINparse2 uchar* p2parse;
int lenMsg;
int i; /* general index for parsing */
uchar bufParseTAG[CONF_TAG_MAXSIZE];
uchar bufParseHOSTNAME[CONF_HOSTNAME_MAXSIZE];
CODESTARTparse assert(pMsg != NULL);
assert(pMsg->pszRawMsg != NULL);
lenMsg = pMsg->iLenRawMsg - pMsg->offAfterPRI;
DBGPRINTF("Message will now be parsed by the legacy syslog parser "
          "(offAfterPRI=%d, lenMsg=%d.\n",
          pMsg->offAfterPRI, lenMsg);
/* note: offAfterPRI is already the number of PRI chars (do not add one!) */
p2parse =
    pMsg->pszRawMsg + pMsg->offAfterPRI; /* point to start of text, after PRI */
 ...
 */
if (datetime.ParseTIMESTAMP3339(&(pMsg->tTIMESTAMP), &p2parse, &lenMsg) ==
    RS_RET_OK)
{
    /* we are done - parse pointer is moved by ParseTIMESTAMP3339 */;
}
else if (datetime.ParseTIMESTAMP3164(
             &(pMsg->tTIMESTAMP), &p2parse, &lenMsg, NO_PARSE3164_TZSTRING,
...
{
    /* try to see if it is slighly malformed - HP procurve seems to do that
     * sometimes */
...
if (bParseHOSTNAMEandTAG && !(pMsg->msgFlags & INTERNAL_MSG))
{
     ...
     * rger, 2005-11-14: we still have a problem with BSD messages. These
     * messages do NOT include a host name. In most cases, this leads to the TAG
     * to be treated as hostname and the first word of the message as the TAG.
     * Clearly, this is not of advantage ;) I think I have now found a way to
     * handle this situation: there are certain characters which are frequently
     * used in TAG (e.g. ':'), which are *invalid* in host names. So while
     * parsing the hostname, I check for these characters. If I find them, I set
     * a simple flag but continue. After parsing, I check the flag. If it was
     * set, then we most probably do not have a hostname but a TAG. Thus, I
     * change the fields. I think this logic shall work with any type of syslog
     * message. rgerhards, 2009-06-23: and I now have extended this logic to
     * every character that is not a valid hostname. A "hostname" can validly
     * include "[]" at the beginning and end. This sometimes happens with IP
     * address (e.g. "[192.168.0.1]"). This must be turned on via an option as
     * it may interfere with non-hostnames in some message formats. rgerhards,
     * 2015-04-20
     */
    if (lenMsg > 0 && pMsg->msgFlags & PARSE_HOSTNAME)
    {
        ...
        while (i < lenMsg &&
               (isalnum(p2parse[i]) || p2parse[i] == '.' || p2parse[i] == '_' ||
                p2parse[i] == '-' || (p2parse[i] == ']' && bHadSBracket) ||
    ...
    /* no TAG can only be detected if the message immediatly ends, in which case
     * an empty TAG is considered OK. So we do not need to check for empty TAG.
...
finalize_it : if (pInst->bRemoveMsgFirstSpace && *p2parse == ' ')
{
...
ENDmodExit
    ...
    BEGINqueryEtryPt CODESTARTqueryEtryPt CODEqueryEtryPt_STD_PMOD2_QUERIES
        CODEqueryEtryPt_IsCompatibleWithFeature_IF_OMOD_QUERIES ENDqueryEtryPt

            BEGINmodInit(pmrfc3164)
                CODESTARTmodInit* ipIFVersProvided = CURR_MOD_IF_VERSION;
