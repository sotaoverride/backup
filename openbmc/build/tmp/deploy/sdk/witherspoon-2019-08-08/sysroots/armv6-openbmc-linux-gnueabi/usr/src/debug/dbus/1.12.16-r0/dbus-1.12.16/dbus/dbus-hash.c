...
typedef DBusHashEntry* (*DBusFindEntryFunction)(
    DBusHashTable* table, void* key, dbus_bool_t create_if_not_found,
...
typedef struct
{
    ...
    DBusHashEntry*
        next_entry;  /**< Next entry to be iterated onto in current bucket */
...
static DBusHashEntry* find_direct_function(DBusHashTable* table, void* key,
                                           dbus_bool_t create_if_not_found,
                                           DBusHashEntry*** bucket,
                                           DBusPreallocatedHash* preallocated);
static DBusHashEntry* find_string_function(DBusHashTable* table, void* key,
                                           dbus_bool_t create_if_not_found,
                                           DBusHashEntry*** bucket,
                                           DBusPreallocatedHash* preallocated);
...
static void remove_entry(DBusHashTable* table, DBusHashEntry** bucket,
                         DBusHashEntry* entry);
                                    ...
                                    DBusFreeFunction value_free_function)
{
    ...
    _dbus_assert(DBUS_SMALL_HASH_TABLE ==
                 _DBUS_N_ELEMENTS(table->static_buckets));
 ...
 */
dbus_bool_t _dbus_hash_iter_lookup(DBusHashTable* table, void* key,
                                   dbus_bool_t create_if_not_found,
                                   DBusHashIter* iter)
{
    DBusRealHashIter* real;
    ...
    entry =
        (*table->find_function)(table, key, create_if_not_found, &bucket, NULL);
...
static void add_allocated_entry(DBusHashTable* table, DBusHashEntry* entry,
...
static DBusHashEntry* add_entry(DBusHashTable* table, unsigned int idx,
                                void* key, DBusHashEntry*** bucket,
                                DBusPreallocatedHash* preallocated)
{
    DBusHashEntry* entry;
...
static DBusHashEntry* find_generic_function(DBusHashTable* table, void* key,
                                            unsigned int idx,
                                           ...
                                           DBusPreallocatedHash* preallocated)
{
    ...
    return find_generic_function(table, key, idx, (KeyCompareFunc)strcmp,
                                 create_if_not_found, bucket, preallocated);
}
                                           ...
                                           DBusPreallocatedHash* preallocated)
{
    ...
    return find_generic_function(table, key, idx, NULL, create_if_not_found,
                                 bucket, preallocated);
}
...
void* _dbus_hash_table_lookup_int(DBusHashTable* table, int key)
{
    ...
    entry = (*table->find_function)(table, _DBUS_INT_TO_POINTER(key), FALSE,
                                    NULL, NULL);
 ...
 */
dbus_bool_t _dbus_hash_table_insert_string(DBusHashTable* table, char* key,
                                           void* value)
{
    DBusPreallocatedHash* preallocated;
    ...
    _dbus_hash_table_insert_string_preallocated(table, preallocated, key,
                                                value);
 ...
 * @param value the hash entry value.
 */
dbus_bool_t _dbus_hash_table_insert_int(DBusHashTable* table, int key,
                                        void* value)
{
    DBusHashEntry* entry;
    ...
    entry = (*table->find_function)(table, _DBUS_INT_TO_POINTER(key), TRUE,
                                    NULL, NULL);
 ...
 * @param value the hash entry value.
 */
dbus_bool_t _dbus_hash_table_insert_uintptr(DBusHashTable* table, uintptr_t key,
                                            void* value)
{
    DBusHashEntry* entry;
...
void _dbus_hash_table_free_preallocated_entry(
    DBusHashTable* table, DBusPreallocatedHash* preallocated)
{
    DBusHashEntry* entry;
...
void _dbus_hash_table_insert_string_preallocated(
    DBusHashTable* table, DBusPreallocatedHash* preallocated, char* key,
    void* value)
{
    DBusHashEntry* entry;
...
dbus_bool_t _dbus_hash_table_from_array(DBusHashTable* table, char** array,
                                        char delimiter)
{
    DBusString key;
    DBusString value;
    ...
    for (i = 0; array[i] != NULL; i++)
    {
        ...
        if (_dbus_string_split_on_byte(&key, delimiter, &value))
        {
            ...
            if (!_dbus_hash_table_insert_string(table, hash_key, hash_value))
                break;
        }
...
char** _dbus_hash_table_to_array(DBusHashTable* table, char delimiter)
{
    ...
    while (_dbus_hash_iter_next(&iter))
    {
        ...
        if (!_dbus_string_append_printf(&entry, "%s%c%s", key, delimiter,
                                        value))
            break;
...
#define N_HASH_KEYS 5000
    ...
    while (i < 3000)
    {
        ...
        if (!_dbus_hash_table_insert_string(table1, steal(&str_key),
                                            steal(&str_value)))
            goto out;
        ...
        if (!_dbus_hash_table_insert_int(table2, i, steal(&str_value)))
            goto out;
        ...
        if (!_dbus_hash_table_insert_uintptr(table3, i, steal(&str_value)))
            goto out;
    ...
    while (i < 3000)
    {
        ...
        if (!_dbus_hash_iter_lookup(table1, steal(&str_key), TRUE, &iter))
            goto out;
        ...
        if (!_dbus_hash_iter_lookup(table2, _DBUS_INT_TO_POINTER(i), TRUE,
                                    &iter))
            goto out;
        _dbus_assert(_dbus_hash_iter_get_value(&iter) == NULL);
        _dbus_hash_iter_set_value(&iter, steal(&str_value));
        ...
        if (!_dbus_hash_iter_lookup(table2, _DBUS_INT_TO_POINTER(i), FALSE,
                                    &iter))
            goto out;
