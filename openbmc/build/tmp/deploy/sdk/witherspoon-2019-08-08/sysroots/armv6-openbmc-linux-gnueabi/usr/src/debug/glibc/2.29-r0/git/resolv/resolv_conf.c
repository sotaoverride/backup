...
static bool same_address(const struct sockaddr* left,
                                ...
                                const struct resolv_conf* conf)
{
    ...
    /* Check that the sort list has not been modified.  */
    {
        ...
        for (size_t i = 0; i < nsort; ++i)
            if (resp->sort_list[i].addr.s_addr !=
                    conf->sort_list[i].addr.s_addr ||
...
struct resolv_conf* __resolv_conf_get(struct __res_state* resp)
{
...
struct resolv_conf* __resolv_conf_allocate(const struct resolv_conf* init)
{
    ...
    struct alloc_buffer buffer = alloc_buffer_allocate(
        sizeof(struct resolv_conf) +
            ...
            init->sort_list_size * sizeof(init->sort_list[0]) + string_space,
        &ptr);
    struct resolv_conf* conf = alloc_buffer_alloc(&buffer, struct resolv_conf);
    if (conf == NULL)
    ...
    const char** search_array =
        alloc_buffer_alloc_array(&buffer, const char*, init->search_list_size);
    ...
    for (size_t i = 0; i < init->nameserver_list_size; ++i)
        if (init->nameserver_list[i]->sa_family == AF_INET)
        {
            struct sockaddr_in* sa =
                alloc_buffer_alloc(&buffer, struct sockaddr_in);
        ...
        else
        {
            struct sockaddr_in6* sa =
                alloc_buffer_alloc(&buffer, struct sockaddr_in6);
       ...
       strings are the least aligned part of the allocation.  */
    {
        for (size_t i = 0; i < init->search_list_size; ++i)
            search_array[i] =
                alloc_buffer_copy_string(&buffer, init->search_list[i]);
    }
    ...
    update_from_conf(struct __res_state* resp, const struct resolv_conf* conf)
{
       ...
       MAXDNSRCH, or if reps->defdname has insufficient space.  */
    {
        ...
        {
            resp->dnsrch[i] =
                alloc_buffer_copy_string(&buffer, conf->search_list[i]);
            if (resp->dnsrch[i] == NULL)
...
/* Decrement the configuration object at INDEX and free it if the
   ...
   remains so.  */
static void decrement_at_index(struct resolv_conf_global* global_copy,
                               size_t index)
{
    if (index < resolv_conf_array_size(&global_copy->array))
    {
