...
static int apply_all(OrderedHashmap* sysctl_options)
{
    ...
    ORDERED_HASHMAP_FOREACH_KEY(value, property, sysctl_options, i)
    {
        ...
        if (k < 0)
        {
            /* If the sysctl is not available in the kernel or we are running
             * with reduced privileges and cannot write it, then log about the
             * issue at LOG_NOTICE level, and proceed without failing. (EROFS is
             * treated as a permission problem here, since that's how container
             * managers
             * usually protected their sysctls.) In all other cases log an error
             * and make the tool fail. */

            if (IN_SET(k, -EPERM, -EACCES, -EROFS, -ENOENT))
                log_notice_errno(k, "Couldn't write '%s' to '%s', ignoring: %m",
                                 value, property);
            else
            {
                log_error_errno(k, "Couldn't write '%s' to '%s': %m", value,
                                property);
                if (r == 0)
                    r = k;
            }
...
static int parse_file(OrderedHashmap* sysctl_options, const char* path,
                      bool ignore_enoent)
{
    ...
    r = search_and_fopen(path, "re", NULL,
    ...
    if (r < 0)
    {
        ...
        return log_error_errno(r, "Failed to open file '%s', ignoring: %m",
                               path);
    }
    ...
    for (;;)
    {
        ...
        if (k == 0)
            break;
        if (k < 0)
            return log_error_errno(k, "Failed to read file '%s', ignoring: %m",
                                   path);
        ...
        *value = 0;
        ...
        if (existing)
        {
            ...
            log_debug("Overwriting earlier assignment of %s at '%s:%u'.", p,
                      path, c);
        ...
        {
            log_error_errno(
                k, "Failed to add sysctl variable %s to hashmap: %m", property);
            free(property);
            free(new_value);
            return k;
        }
...
{
    _cleanup_(ordered_hashmap_free_free_freep) OrderedHashmap* sysctl_options =
        NULL;
    int r, k;
