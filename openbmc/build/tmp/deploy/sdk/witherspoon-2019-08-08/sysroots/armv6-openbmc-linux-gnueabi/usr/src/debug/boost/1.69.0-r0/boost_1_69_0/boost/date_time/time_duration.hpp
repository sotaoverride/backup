...
{
  public:
    // A tag for type categorization. Can be used to detect Boost.DateTime
    // duration types in generic code.
    ...
    time_duration(hour_type hours_in, min_type minutes_in,
        ...
        ticks_(rep_type::to_tick_count(hours_in, minutes_in, seconds_in,
                                       frac_sec_in))
    {
    }
    ...
    {
        return static_cast<min_type>((ticks() / (60 * ticks_per_second())) %
                                     60);
    }
    ...
    tick_type total_milliseconds() const
    {
        ...
        {
            return ticks() *
                   (static_cast<tick_type>(1000) / ticks_per_second());
        }
        return ticks() / (ticks_per_second() / static_cast<tick_type>(1000));
    }
    //! Returns total number of nanoseconds truncating any sub millisecond
    //! values
    tick_type total_nanoseconds() const
    {
        ...
        {
            return ticks() *
                   (static_cast<tick_type>(1000000000) / ticks_per_second());
        }
        return ticks() /
               (ticks_per_second() / static_cast<tick_type>(1000000000));
    }
    //! Returns total number of microseconds truncating any sub microsecond
    //! values
    tick_type total_microseconds() const
    {
        ...
        {
            return ticks() *
                   (static_cast<tick_type>(1000000) / ticks_per_second());
        }
...
class BOOST_SYMBOL_VISIBLE subsecond_duration : public base_duration
{
  ...
  private:
    // To avoid integer overflow we precompute the duration resolution
    // conversion coefficient (ticket #3471)
    BOOST_STATIC_ASSERT_MSG(
        (traits_type::ticks_per_second >= frac_of_second
             ? traits_type::ticks_per_second % frac_of_second
             : frac_of_second % traits_type::ticks_per_second) == 0,
        "The base duration resolution must be a multiple of the subsecond "
        "duration resolution");
    BOOST_STATIC_CONSTANT(
        boost::int64_t,
        adjustment_ratio = (traits_type::ticks_per_second >= frac_of_second
                                ? traits_type::ticks_per_second / frac_of_second
                                : frac_of_second /
                                      traits_type::ticks_per_second));

  public:
    // The argument (ss) must be an integral type
    template <typename T>
    explicit subsecond_duration(
        T const& ss,
        typename boost::enable_if<boost::is_integral<T>, void>::type* =
            BOOST_DATE_TIME_NULLPTR) :
        base_duration(impl_type(traits_type::ticks_per_second >= frac_of_second
                                    ? ss * adjustment_ratio
                                    : ss / adjustment_ratio))
    {
    }
