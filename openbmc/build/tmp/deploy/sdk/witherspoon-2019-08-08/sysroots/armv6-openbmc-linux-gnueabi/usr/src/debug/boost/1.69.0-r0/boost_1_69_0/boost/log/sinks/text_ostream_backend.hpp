...
class basic_text_ostream_backend
    : public basic_formatted_sink_backend<
          CharT, combine_requirements<synchronized_feeding, flushing>::type>
{
    //! Base type
  ...
  public:
    ...
    /*!
     * The method removes a stream from the sink. If the stream is not attached
     * to the sink, the method has no effect.
     *
     * \param strm Pointer to the stream. Must not be NULL.
     */
    ...
    /*!
     * Sets the flag to automatically flush buffers of all attached streams
...
typedef basic_text_ostream_backend<char>
    text_ostream_backend; //!< Convenience typedef for narrow-character logging
#endif
#ifdef BOOST_LOG_USE_WCHAR_T
typedef basic_text_ostream_backend<wchar_t>
    wtext_ostream_backend; //!< Convenience typedef for wide-character logging
#endif
