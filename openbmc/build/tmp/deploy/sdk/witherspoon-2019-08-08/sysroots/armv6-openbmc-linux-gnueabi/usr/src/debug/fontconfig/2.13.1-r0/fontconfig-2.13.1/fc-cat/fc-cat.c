...
const struct option longopts[] = {
    {"version", 0, 0, 'V'}, {"verbose", 0, 0, 'v'}, {"recurse", 0, 0, 'r'},
...
static FcBool write_chars(FILE* f, const FcChar8* chars)
{
    FcChar8 c;
    while ((c = *chars++))
    {
...
static FcBool write_ulong(FILE* f, unsigned long t)
{
    int pow;
...
static FcBool write_int(FILE* f, int i)
{
    return write_ulong(f, (unsigned long)i);
}

static FcBool write_string(FILE* f, const FcChar8* string)
{
...
#if HAVE_GETOPT_LONG
    fprintf(file,
            _("usage: %s [-rv] [--recurse] [--verbose] [*-%s" FC_CACHE_SUFFIX
              "|directory]...\n"),
            program, FC_ARCHITECTURE);
    fprintf(file, "       %s [-Vh] [--version] [--help]\n", program);
#else
    fprintf(file,
            _("usage: %s [-rvVh] [*-%s" FC_CACHE_SUFFIX "|directory]...\n"),
            program, FC_ARCHITECTURE);
#endif
...
static const FcChar8* file_base_name(const FcChar8* cache, const FcChar8* file)
{
    int cache_len = strlen((char*)cache);

    if (!strncmp((char*)cache, (char*)file, cache_len) &&
        file[cache_len] == '/')
...
static FcBool cache_print_set(FcFontSet* set, FcStrSet* dirs,
                              const FcChar8* base_name, FcBool verbose)
{
    FcChar8* dir;
    const FcChar8* base;
    int n;
