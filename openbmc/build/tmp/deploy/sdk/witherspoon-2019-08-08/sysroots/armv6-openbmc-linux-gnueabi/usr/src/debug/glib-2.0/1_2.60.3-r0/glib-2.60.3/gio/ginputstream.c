...
static gssize g_input_stream_real_skip(GInputStream* stream, gsize count,
                                       ...
                                       GError** error);
static void g_input_stream_real_read_async(GInputStream* stream, void* buffer,
...
static gssize g_input_stream_real_read_finish(GInputStream* stream,
                                              GAsyncResult* result,
                                              GError** error);
static void g_input_stream_real_skip_async(GInputStream* stream, gsize count,
                                           int io_priority,
                                           GCancellable* cancellable,
                                           GAsyncReadyCallback callback,
                                           gpointer data);
 ...
 *     read data into (which should be at least count bytes long).
 * @count: the number of bytes that will be read from the stream
 * @bytes_read: (out): location to store the number of bytes that was read from
 *the stream
 * @cancellable: (nullable): optional #GCancellable object, %NULL to ignore.
 * @error: location to store the error occurring, or %NULL to ignore
 *
 ...
 **/
gboolean g_input_stream_read_all(GInputStream* stream, void* buffer,
                                 ...
                                 GCancellable* cancellable, GError** error)
{
    ...
    {
        res = g_input_stream_read(stream, (char*)buffer + _bytes_read,
                                  count - _bytes_read, cancellable, error);
        if (res == -1)
        {
            if (bytes_read)
 ...
 * Since: 2.34
 **/
GBytes* g_input_stream_read_bytes(GInputStream* stream, gsize count,
                                  GCancellable* cancellable, GError** error)
{
    guchar* buf;
    gssize nread;
 ...
 **/
gssize g_input_stream_skip(GInputStream* stream, gsize count,
                           GCancellable* cancellable, GError** error)
{
    GInputStreamClass* class;
    gssize res;
                                       ...
                                       GError** error)
{
    ...
    {
        if (g_seekable_seek(G_SEEKABLE(stream), count, G_SEEK_CUR, cancellable,
                            NULL))
            return count;
    }
    ...
    while (1)
    {
        ...
        if (ret == -1)
        {
            if (read_bytes > 0 && my_error->domain == G_IO_ERROR &&
 ...
 **/
gboolean g_input_stream_close(GInputStream* stream, GCancellable* cancellable,
                              GError** error)
{
    GInputStreamClass* class;
    gboolean res;
...
static void async_ready_callback_wrapper(GObject* source_object,
                               ...
                               GAsyncReadyCallback callback, gpointer user_data)
{
    ...
    if (((gssize)count) < 0)
    {
        g_task_report_new_error(
            stream, callback, user_data, g_input_stream_read_async, G_IO_ERROR,
    ...
    if (!g_input_stream_set_pending(stream, &error))
    {
        g_task_report_error(stream, callback, user_data,
                            g_input_stream_read_async, error);
        return;
    }
 ...
 **/
gssize g_input_stream_read_finish(GInputStream* stream, GAsyncResult* result,
                                  GError** error)
{
    GInputStreamClass* class;
...
static void free_async_read_all(gpointer data)
{
    g_slice_free(AsyncReadAll, data);
}

static void read_all_callback(GObject* stream, GAsyncResult* result,
                              gpointer user_data)
{
    ...
    if (got_eof || data->to_read == 0)
    {
        ...
        g_input_stream_read_async(
            G_INPUT_STREAM(stream), data->buffer + data->bytes_read,
            data->to_read, g_task_get_priority(task),
            g_task_get_cancellable(task), read_all_callback, task);
}

static void read_all_async_thread(GTask* task, gpointer source_object,
                                  gpointer task_data, GCancellable* cancellable)
{
    ...
    if (g_input_stream_read_all(stream, data->buffer, data->to_read,
                                &data->bytes_read, g_task_get_cancellable(task),
                                &error))
        g_task_return_boolean(task, TRUE);
    else
        g_task_return_error(task, error);
 ...
 * Since: 2.44
 **/
void g_input_stream_read_all_async(GInputStream* stream, void* buffer,
                                   gsize count, int io_priority,
                                   GCancellable* cancellable,
                                   GAsyncReadyCallback callback,
                                   gpointer user_data)
...
gboolean g_input_stream_read_all_finish(GInputStream* stream,
                                        GAsyncResult* result, gsize* bytes_read,
                                        GError** error)
{
    GTask* task;
...
static void read_bytes_callback(GObject* stream, GAsyncResult* result,
                                gpointer user_data)
{
    GTask* task = user_data;
                                     ...
                                     gpointer user_data)
{
    ...
    g_input_stream_read_async(stream, buf, count, io_priority, cancellable,
                               ...
                               GAsyncReadyCallback callback, gpointer user_data)
{
    ...
    {
        g_task_report_new_error(
            stream, callback, user_data, g_input_stream_skip_async, G_IO_ERROR,
            G_IO_ERROR_INVALID_ARGUMENT,
            _("Too large count value passed to %s"), G_STRFUNC);
        return;
    }
    ...
    {
        g_task_report_error(stream, callback, user_data,
                            g_input_stream_skip_async, error);
        return;
    }
 ...
 **/
gssize g_input_stream_skip_finish(GInputStream* stream, GAsyncResult* result,
                                  GError** error)
{
    GInputStreamClass* class;
 ...
 * @user_data: (closure): the data to pass to callback function
 *
 * Requests an asynchronous closes of the stream, releasing resources related to
 *it. When the operation is finished @callback will be called. You can then call
 ...
 *However, if you override one you must override all.
 **/
void g_input_stream_close_async(GInputStream* stream, int io_priority,
                                GCancellable* cancellable,
                                GAsyncReadyCallback callback,
                                gpointer user_data)
{
    ...
    {
        g_task_report_error(stream, callback, user_data,
                            g_input_stream_close_async, error);
        return;
    }
 ...
 **/
gboolean g_input_stream_close_finish(GInputStream* stream, GAsyncResult* result,
                                     GError** error)
{
    GInputStreamClass* class;
...
gboolean g_input_stream_async_read_is_via_threads(GInputStream* stream)
{
    ...
    return (
        class->read_async == g_input_stream_real_read_async &&
...
static void free_read_data(ReadData* op)
{
    g_slice_free(ReadData, op);
}

static void read_async_thread(GTask* task, gpointer source_object,
                              gpointer task_data, GCancellable* cancellable)
{
    GInputStream* stream = source_object;
    ...
    nread = class->read_fn(stream, op->buffer, op->count,
...
static void read_async_pollable(GPollableInputStream* stream, GTask* task);

static gboolean read_async_pollable_ready(GPollableInputStream* stream,
                                          gpointer user_data)
{
    GTask* task = user_data;
...
static void read_async_pollable(GPollableInputStream* stream, GTask* task)
{
    ...
    nread = G_POLLABLE_INPUT_STREAM_GET_INTERFACE(stream)->read_nonblocking(
        stream, op->buffer, op->count, &error);
...
static void skip_async_thread(GTask* task, gpointer source_object,
                              gpointer task_data, GCancellable* cancellable)
{
    GInputStream* stream = source_object;
    ...
    class = G_INPUT_STREAM_GET_CLASS(stream);
    ret = class->skip(stream, count, g_task_get_cancellable(task), &error);
    if (ret == -1)
        g_task_return_error(task, error);
    else
...
static void skip_callback_wrapper(GObject* source_object, GAsyncResult* res,
                                  gpointer user_data)
{
    GInputStreamClass* class;
    GTask* task = user_data;
    ...
    if (ret > 0)
    {
        ...
        if (data->count > 0)
        {
            ...
            class->read_async(G_INPUT_STREAM(source_object), data->buffer,
                              MIN(8192, data->count), g_task_get_priority(task),
    ...
    if (ret == -1 && g_error_matches(error, G_IO_ERROR, G_IO_ERROR_CANCELLED) &&
        data->count_skipped)
    {
                                           ...
                                           gpointer user_data)
{
    ...
    {
        /* TODO: Skip fallback uses too much memory, should do multiple read
         * calls */
        ...
        class->read_async(stream, data->buffer, MIN(8192, count), io_priority,
                          cancellable, skip_callback_wrapper, task);
    }
}
                               ...
                               gpointer task_data, GCancellable* cancellable)
{
    ...
    if (class->close_fn)
    {
        result = class->close_fn(stream, g_task_get_cancellable(task), &error);
        if (!result)
        {
            g_task_return_error(task, error);
            return;
