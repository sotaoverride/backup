...
static krb5_error_code k5_insert_client_info(krb5_context context, krb5_pac pac,
                                             ...
                                             krb5_boolean with_realm)
{
    ...
    ret = k5_pac_add_buffer(context, pac, KRB5_PAC_CLIENT_INFO, &client_info,
                            TRUE, &client_info);
    if (ret != 0)
        goto cleanup;
...
static krb5_error_code k5_insert_checksum(krb5_context context, krb5_pac pac,
                                          krb5_ui_4 type,
                                          ...
                                          krb5_cksumtype* cksumtype)
{
    ...
    else
    {
        ...
        ret =
            k5_pac_add_buffer(context, pac, type, &cksumdata, TRUE, &cksumdata);
        if (ret != 0)
            return ret;
    }
...
static krb5_error_code k5_pac_encode_header(krb5_context context, krb5_pac pac)
{
    size_t i;
    unsigned char* p;
    size_t header_len;
    ...
    const krb5_keyblock* privsvr_key, krb5_boolean with_realm, krb5_data* data)
{
    ...
    ret = k5_insert_checksum(context, pac, KRB5_PAC_SERVER_CHECKSUM, server_key,
                             &server_cksumtype);
    if (ret != 0)
        return ret;
    ...
    ret = krb5_c_make_checksum_iov(context, server_cksumtype, server_key,
                                   KRB5_KEYUSAGE_APP_DATA_CKSUM, iov,
    ...
    ret = krb5_c_make_checksum_iov(context, privsvr_cksumtype, privsvr_key,
                                   KRB5_KEYUSAGE_APP_DATA_CKSUM, iov,
                                   sizeof(iov) / sizeof(iov[0]));
    if (ret != 0)
        return ret;
