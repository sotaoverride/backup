...
static void show_nodes(GType type, GType sibling, const gchar* indent)
{
    ...
    g_fprintf(f_out, "%s%s%s%s", indent,
              sibling ? O_BRANCH : (type != root ? O_LLEAF : O_SPACE), O_ESPACE,
              g_type_name(type));
...
{
    g_fprintf(stderr, "usage: gobject-query <qualifier> [-r <type>] [-{i|b} "
                      "\"\"] [-s #] [-{h|x|y}]\n");
...
int main(gint argc, gchar* argv[])
{
    ...
    {
        root = ~0;
        for (i = 0; i <= G_TYPE_FUNDAMENTAL_MAX;
             i += G_TYPE_MAKE_FUNDAMENTAL(1))
        {
