...
#define SEQUENCES(io, s)                                                       \
    {                                                                          \
        {3 * (s) - (io)-1, 1}, {512}, {666}, {2048},                           \
            {(io), (io), MAX_NAND_PAGE_SIZE},                                  \
            {(io) + 1, (io) + 1, MAX_NAND_PAGE_SIZE}, {MAX_NAND_PAGE_SIZE},    \
            {MAX_NAND_PAGE_SIZE - 1}, {MAX_NAND_PAGE_SIZE + (io)}, {(s)},      \
            {(s)-1}, {(s) + 1}, {(io), (s) + 1},                               \
            {(s) + (io), MAX_NAND_PAGE_SIZE}, {2 * (s), MAX_NAND_PAGE_SIZE},   \
            {MAX_NAND_PAGE_SIZE, 2 * (s), 1}, {MAX_NAND_PAGE_SIZE, 2 * (s)},   \
            {2 * (s)-1, 2 * (s)-1}, {3 * (s), MAX_NAND_PAGE_SIZE + 1},         \
...
{
    long long total_len =
        leb_change ? vol_info->leb_size : vol_info->rsvd_bytes;
    int sequences[SEQ_SZ][3] =
        SEQUENCES(dev_info.min_io_size,
                  leb_change ? dev_info.min_io_size * 2 : vol_info->leb_size);
    ...
    sprintf(vol_node, UBI_VOLUME_PATTERN, dev_info.dev_num, vol_info->vol_id);
