                            ...
                            void* userdata)
{
    ...
    if (event == AVAHI_BROWSER_ALL_FOR_NOW && b->defer_event)
    {
...
static void defer_callback(AvahiTimeEvent* e, void* userdata)
{
    ...
    for (l = b->server->config.browse_domains; l; l = l->next)
    {
        ...
        b->callback(b, AVAHI_IF_UNSPEC, AVAHI_PROTO_UNSPEC, AVAHI_BROWSER_NEW,
                    (char*)l->text, AVAHI_LOOKUP_RESULT_STATIC, b->userdata);
    }
    ...
    if (b->ref > 1)
    {
        ...
        if (b->all_for_now_scheduled)
            b->callback(b, AVAHI_IF_UNSPEC, AVAHI_PROTO_UNSPEC,
                        AVAHI_BROWSER_ALL_FOR_NOW, NULL, 0, b->userdata);
    }
...
AvahiSDomainBrowser* avahi_s_domain_browser_new(
    ...
    const char* domain, AvahiDomainBrowserType type, AvahiLookupFlags flags,
    AvahiSDomainBrowserCallback callback, void* userdata)
{
    ...
    AVAHI_CHECK_VALIDITY_RETURN_NULL(server, type < AVAHI_DOMAIN_BROWSER_MAX,
    ...
    if ((r = avahi_service_name_join(n, sizeof(n), type_table[type],
                                     "_dns-sd._udp", domain)) < 0)
    {
    ...
    AVAHI_LLIST_PREPEND(AvahiSDomainBrowser, browser, server->domain_browsers,
                        b);
    ...
    if (type == AVAHI_DOMAIN_BROWSER_BROWSE && b->server->config.browse_domains)
        b->defer_event = avahi_time_event_new(server->time_event_queue, NULL,
                                              defer_callback, b);
...
void avahi_s_domain_browser_free(AvahiSDomainBrowser* b)
{
    ...
    AVAHI_LLIST_REMOVE(AvahiSDomainBrowser, browser, b->server->domain_browsers,
                       b);
