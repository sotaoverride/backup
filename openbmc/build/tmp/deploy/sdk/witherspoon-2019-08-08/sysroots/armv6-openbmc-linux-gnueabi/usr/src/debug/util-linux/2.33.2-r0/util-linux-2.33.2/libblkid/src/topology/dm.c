...
{
    const char* paths[] = {"/usr/local/sbin/dmsetup", "/usr/sbin/dmsetup",
                           "/sbin/dmsetup"};
    ...
    switch (fork())
    {
        case 0:
        {
            ...
            DBG(LOWPROBE,
                ul_debug("Failed to execute %s: errno=%d", cmd, errno));
            exit(1);
        }
        case -1:
    ...
    if (fscanf(stream, "%lld %lld striped %d %d ", &offset, &size, &stripes,
               &stripesize) != 0)
        goto nothing;
...
const struct blkid_idinfo dm_tp_idinfo = {
    .name = "dm", .probefunc = probe_dm_tp, .magics = BLKID_NONE_MAGIC};
