...
class ecv2_record
{
  ...
  public:
    ecv2_record(stack_context sctx, StackAlloc&& salloc, Fn&& fn,
                Params&&... params) noexcept :
        salloc_(std::forward<StackAlloc>(salloc)),
        sctx_(sctx), fn_(std::forward<Fn>(fn)),
    ...
    {
        Ctx from{t.fctx};
        typename Ctx::args_tpl_t args = std::move(std::get<1>(
            *static_cast<
                std::tuple<std::exception_ptr, typename Ctx::args_tpl_t>*>(
                t.data)));
        auto tpl = std::tuple_cat(
            params_, std::forward_as_tuple(std::move(from)), std::move(args));
...
class execution_context
{
  private:
    ...
    execution_context(detail::fcontext_t fctx) noexcept : fctx_(fctx)
    {
    }
  ...
  public:
    ...
    // segmented-stack requires to preserve the segments of the `current`
    // context which is not possible (no global pointer to current context)
    template <typename Fn, typename... Params>
    execution_context(std::allocator_arg_t, segmented_stack, Fn&&,
                      Params&&...) = delete;

    template <typename Fn, typename... Params>
    execution_context(std::allocator_arg_t, preallocated, segmented_stack, Fn&&,
                      Params&&...) = delete;
#else
    template <typename Fn, typename... Params,
    ...
    execution_context(Fn&& fn, Params&&... params) :
        ...
        fctx_(detail::ecv2_context_create<execution_context>(
            fixedsize_stack(), std::forward<Fn>(fn),
            std::forward<Params>(params)...))
    {
    }

    template <typename StackAlloc, typename Fn, typename... Params>
    execution_context(std::allocator_arg_t, StackAlloc&& salloc, Fn&& fn,
                      Params&&... params) :
        // deferred execution of fn and its arguments
        // arguments are stored in std::tuple<>
        // non-type template parameter pack via std::index_sequence_for<>
    ...
    execution_context(std::allocator_arg_t, preallocated palloc,
                      StackAlloc&& salloc, Fn&& fn, Params&&... params) :
        // deferred execution of fn and its arguments
        // arguments are stored in std::tuple<>
        // non-type template parameter pack via std::index_sequence_for<>
        // preserves the number of arguments
    ...
    ~execution_context()
    {
        if (nullptr != fctx_)
        {
            detail::ontop_fcontext(detail::exchange(fctx_, nullptr), nullptr,
                                   detail::ecv2_context_unwind);
        }
    }

    execution_context(execution_context&& other) noexcept : fctx_(other.fctx_)
    {
        other.fctx_ = nullptr;
    }
    ...
    execution_context&
        operator=(execution_context const& other) noexcept = delete;
...
class ontop_error : public std::exception
{
  ...
  public:
    ontop_error(detail::fcontext_t fctx) noexcept : fctx_{fctx}
    {
    }
...
template <typename... Args>
typename execution_context<Args...>::ret_tpl_t execution_context<Args...>::
    operator()(Args... args)
{
    ...
    detail::transfer_t t =
        detail::jump_fcontext(detail::exchange(fctx_, nullptr), &p);
    if (nullptr != t.data)
    {
        auto p =
            static_cast<std::tuple<std::exception_ptr, args_tpl_t>*>(t.data);
    ...
    return std::tuple_cat(std::forward_as_tuple(execution_context(t.fctx)),
                          std::move(data));
}
    ...
    operator()(exec_ontop_arg_t, Fn&& fn, Args... args)
{
    ...
    auto p = std::make_tuple(
        fn, std::make_tuple(std::exception_ptr{}, std::move(data)));
    detail::transfer_t t = detail::ontop_fcontext(
        detail::exchange(fctx_, nullptr), &p,
...
inline transfer_t ecv2_context_unwind(transfer_t t)
{
...
{
    auto p = static_cast<
        std::tuple<Fn, std::tuple<std::exception_ptr, std::tuple<Args...>>>*>(
        t.data);
    ...
    try
    {
        // execute function
#if defined(BOOST_NO_CXX17_STD_APPLY)
        std::get<1>(std::get<1>(*p)) = helper<sizeof...(Args)>::convert(
            boost::context::detail::apply(fn, std::move(args)));
#else
        std::get<1>(std::get<1>(*p)) =
            helper<sizeof...(Args)>::convert(std::apply(fn, std::move(args)));
#endif
    }
    catch (...)
    {
...
fcontext_t ecv2_context_create(StackAlloc&& salloc, Fn&& fn, Params&&... params)
{
    ...
    const std::size_t size =
        sctx.size - (static_cast<char*>(sctx.sp) - static_cast<char*>(sp));
#endif
    // create fast-context
    const fcontext_t fctx =
        make_fcontext(sp, size, &ecv2_context_etry<ecv2_record_t>);
    BOOST_ASSERT(nullptr != fctx);
    // placment new for control structure on context-stack
    auto rec = ::new (sp)
        ecv2_record_t{sctx, std::forward<StackAlloc>(salloc),
...
template <typename Ctx, typename StackAlloc, typename Fn, typename... Params>
fcontext_t ecv2_context_create(preallocated palloc, StackAlloc&& salloc,
                               Fn&& fn, Params&&... params)
{
    typedef ecv2_record<Ctx, StackAlloc, Fn, Params...> ecv2_record_t;
    ...
    // calculate remaining size
    const std::size_t size =
        palloc.size - (static_cast<char*>(palloc.sp) - static_cast<char*>(sp));
#endif
    // create fast-context
    const fcontext_t fctx =
        make_fcontext(sp, size, &ecv2_context_etry<ecv2_record_t>);
    ...
    // placment new for control structure on context-stack
    auto rec = ::new (sp)
        ecv2_record_t{palloc.sctx, std::forward<StackAlloc>(salloc),
                      std::forward<Fn>(fn), std::forward<Params>(params)...};
    // transfer control structure to context-stack
    return jump_fcontext(fctx, rec).fctx;
}
