...
ftc_basic_family_init(FTC_MruNode ftcfamily, FT_Pointer ftcquery,
                      FT_Pointer ftccache)
{
    FTC_BasicFamily family = (FTC_BasicFamily)ftcfamily;
    FTC_BasicQuery query = (FTC_BasicQuery)ftcquery;
...
ftc_basic_family_get_count(FTC_Family ftcfamily, FTC_Manager manager)
{
    ...
    error =
        FTC_Manager_LookupFace(manager, family->attrs.scaler.face_id, &face);
...
FT_CALLBACK_DEF(FT_Error)
ftc_basic_family_load_bitmap(FTC_Family ftcfamily, FT_UInt gindex,
                             FTC_Manager manager, FT_Face* aface)
{
    FTC_BasicFamily family = (FTC_BasicFamily)ftcfamily;
    FT_Error error;
    FT_Size size;
    ...
    if (!error)
    {
        ...
        error = FT_Load_Glyph(
            face, gindex, (FT_Int)family->attrs.load_flags | FT_LOAD_RENDER);
        if (!error)
            *aface = face;
    }
...
FT_CALLBACK_DEF(FT_Error)
ftc_basic_family_load_glyph(FTC_Family ftcfamily, FT_UInt gindex,
                            FTC_Cache cache, FT_Glyph* aglyph)
{
    FTC_BasicFamily family = (FTC_BasicFamily)ftcfamily;
    FT_Error error;
...
FT_CALLBACK_DEF(FT_Bool)
ftc_basic_gnode_compare_faceid(FTC_Node ftcgnode, FT_Pointer ftcface_id,
                               FTC_Cache cache, FT_Bool* list_changed)
{
...
static const FTC_GCacheClassRec ftc_basic_image_cache_class = {
    {
        ...
        ftc_basic_gnode_compare_faceid, /* FTC_Node_CompareFunc
                                           node_remove_faceid */
                      ...
                      FT_Glyph* aglyph, FTC_Node* anode)
{
    ...
    *aglyph = NULL;
    ...
    FTC_GCACHE_LOOKUP_CMP(cache, ftc_basic_family_compare, FTC_GNode_Compare,
                          hash, gindex, &query, node, error);
#else
...
FTC_SBitCache_Lookup(FTC_SBitCache cache, FTC_ImageType type, FT_UInt gindex,
                     FTC_SBit* ansbit, FTC_Node* anode)
{
    FT_Error error;
    FTC_BasicQueryRec query;
    FTC_Node node = 0; /* make compiler happy */
