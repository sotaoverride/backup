...
af_axis_hints_new_segment(AF_AxisHints axis, FT_Memory memory,
                          AF_Segment* asegment)
{
...
FT_LOCAL(FT_Error)
af_axis_hints_new_edge(AF_AxisHints axis, FT_Int fpos, AF_Direction dir,
                       FT_Bool top_to_bottom_hinting, FT_Memory memory,
                       AF_Edge* anedge)
{
    FT_Error error = FT_Err_Ok;
    AF_Edge edge = NULL;
...
static char* af_print_idx(char* p, int idx)
{
    if (idx == -1)
    {
        p[0] = '-';
...
static int af_get_segment_index(AF_GlyphHints hints, int point_idx,
                                int dimension)
{
...
static int af_get_edge_index(AF_GlyphHints hints, int segment_idx,
                             int dimension)
{
    AF_AxisHints axis = &hints->axis[dimension];
...
static int af_get_strong_edge_index(AF_GlyphHints hints, AF_Edge* strong_edges,
                                    int dimension)
{
    AF_AxisHints axis = &hints->axis[dimension];
    AF_Edge edges = axis->edges;
...
void af_glyph_hints_dump_points(AF_GlyphHints hints, FT_Bool to_stdout)
{
    ...
    for (point = points; point < limit; point++)
    {
        ...
        AF_DUMP(("  %5d  %5s %5s  %5s %5s  %s"
                 ...
                 " %5s %5s %5s %5s\n",
                 point_idx,
                 af_print_idx(buf1, af_get_edge_index(hints, segment_idx_1, 1)),
                 af_print_idx(buf2, segment_idx_1),
                 af_print_idx(buf3, af_get_edge_index(hints, segment_idx_0, 0)),
                 ...
                 point->fx, point->fy, point->ox / 64.0, point->oy / 64.0,
                 point->x / 64.0, point->y / 64.0,

                 af_print_idx(
                     buf5, af_get_strong_edge_index(hints, point->before, 1)),
                 af_print_idx(buf6,
                              af_get_strong_edge_index(hints, point->after, 1)),
                 af_print_idx(
                     buf7, af_get_strong_edge_index(hints, point->before, 0)),
                 af_print_idx(
                     buf8, af_get_strong_edge_index(hints, point->after, 0))));
    }
    AF_DUMP(("\n"));
}
#ifdef __cplusplus
...
static const char* af_edge_flags_to_string(FT_UInt flags)
{
    static char temp[32];
    int pos = 0;
    ...
    for (dimension = 1; dimension >= 0; dimension--)
    {
        ...
        for (seg = segments; seg < limit; seg++)
            AF_DUMP(("  %5d  %5d  %5d  %5s  %4d  %4d"
                     "  %4s  %5s  %4s"
                     "  %6d  %5d  %11s\n",
                     AF_INDEX_NUM(seg, segments), seg->pos, seg->delta,
...
FT_Error af_glyph_hints_get_segment_offset(AF_GlyphHints hints,
                                           FT_Int dimension, FT_Int idx,
                                           FT_Pos* offset, FT_Bool* is_blue,
                                           FT_Pos* blue_offset)
{
    AF_Dimension dim;
    AF_AxisHints axis;
    ...
    *offset = (dim == AF_DIMENSION_HORZ) ? seg->first->fx : seg->first->fy;
    if (seg->edge)
...
void af_glyph_hints_dump_edges(AF_GlyphHints hints, FT_Bool to_stdout)
{
    ...
    for (dimension = 1; dimension >= 0; dimension--)
    {
        ...
        if (dimension == AF_DIMENSION_HORZ)
            ...
            AF_DUMP(("Table of %s edges (1px=%.2fu, 10u=%.2fpx):\n",
                     "horizontal", 65536.0 * 64.0 / hints->y_scale,
        ...
        for (edge = edges; edge < limit; edge++)
            AF_DUMP(("  %5d  %7.2f  %5s  %4s  %5s"
                     "    %c   %7.2f  %7.2f  %11s\n",
                     AF_INDEX_NUM(edge, edges), (int)edge->opos / 64.0,
                     af_dir_str((AF_Direction)edge->dir),
                     af_print_idx(buf1, AF_INDEX_NUM(edge->link, edges)),
                     af_print_idx(buf2, AF_INDEX_NUM(edge->serif, edges)),

                     edge->blue_edge ? 'y' : 'n', edge->opos / 64.0,
                     edge->pos / 64.0, af_edge_flags_to_string(edge->flags)));
        AF_DUMP(("\n"));
    }
}
#ifdef __cplusplus
...

        {
            ...
            for (contour = hints->contours; contour < contour_limit; contour++)
            {
                ...
                while (prev != first)
                {
                     ...
                     * opposite direction of the distance measured here.  For
                     * this reason we use `near_limit2' for the comparison to
                     * get a non-near point even in the worst case.
                     */
                 ...
                 *
                 * To avoid problems with not having non-near points, we point
                 * to `first' by default as the next non-near point.
                 *
                 */
                ...
                else if (point->out_dir == point->in_dir)
                {
...

                    {
                        ...
                        if (ft_corner_is_flat(
                                point->fx - prev_v->fx, point->fy - prev_v->fy,
                                next_u->fx - point->fx, next_u->fy - point->fy))
                        {
        ...
        for (point = points; point < point_limit; point++)
        {
...

            {
                ...
                /* point is not on an edge */
                {
                    ...
                    u = before->pos +
                        FT_MulFix(fu - before->fpos, before->scale);
                }
            }
...
static void af_iup_interp(AF_Point p1, AF_Point p2, AF_Point ref1,
                          AF_Point ref2)
{
    AF_Point p;
...
af_glyph_hints_align_weak_points(AF_GlyphHints hints, AF_Dimension dim)
{
    ...
    for (; contour < contour_limit; contour++)
    {
        ...
        for (;;)
        {
            ...
            af_iup_interp(last_touched + 1, point - 1, last_touched, point);
        }
    ...
    EndContour:
        ...
        else /* interpolate the last part */
        {
            if (last_touched < end_point)
                af_iup_interp(last_touched + 1, end_point, last_touched,
                              first_touched);

            if (first_touched > points)
                af_iup_interp(first_point, first_touched - 1, last_touched,
                              first_touched);
        }
...
FT_LOCAL_DEF(void)
af_glyph_hints_scale_dim(AF_GlyphHints hints, AF_Dimension dim, FT_Fixed scale,
                         FT_Pos delta)
{
    AF_Point points = hints->points;
