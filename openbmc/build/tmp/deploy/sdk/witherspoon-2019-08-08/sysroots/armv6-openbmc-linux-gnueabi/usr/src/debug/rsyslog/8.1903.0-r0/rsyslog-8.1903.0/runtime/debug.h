...
extern int stddbg; /* the handle for regular debug output, set to stdout if not
                      forking, -1 otherwise */
...
/* external data */
extern char*
    pszAltDbgFileName; /* if set, debug output is *also* sent to here */
...
static inline void r_dbgprintf(const char DBGL_UNUSED* srcname,
                               const char DBGL_UNUSED* fmt, ...)
{
}
#else
...
#define d_pthread_cond_timedwait(cond, mut, to)                                \
    pthread_cond_timedwait(cond, mut, to)
