...
void kdb5_stash(argc, argv) int argc;
char* argv[];
{
    extern char* optarg;
    extern int optind;
    ...
    {
        char tmp[32];
        if (krb5_enctype_to_name(master_keyblock.enctype, FALSE, tmp,
                                 sizeof(tmp)))
    ...
    if (!valid_master_key)
    {
        ...
        retval = krb5_db_fetch_mkey(
            util_context, master_princ, master_keyblock.enctype, TRUE, FALSE,
            (char*)NULL, &mkey_kvno, NULL, &master_keyblock);
        if (retval)
        {
