...
OM_uint32 generic_gss_release_oid(OM_uint32* minor_status, gss_OID* oid)
{
    if (minor_status)
        *minor_status = 0;
...
OM_uint32 generic_gss_copy_oid(OM_uint32* minor_status,
                               const gss_OID_desc* const oid, gss_OID* new_oid)
{
    gss_OID p;
...
OM_uint32 generic_gss_create_empty_oid_set(OM_uint32* minor_status,
                                           gss_OID_set* oid_set)
{
    *minor_status = 0;
...
OM_uint32 generic_gss_add_oid_set_member(OM_uint32* minor_status,
                                         ...
                                         gss_OID_set* oid_set)
{
    ...
    *minor_status = 0;
    ...
    if (((*oid_set)->elements = (gss_OID)gssalloc_malloc(
             ((*oid_set)->count + 1) * sizeof(gss_OID_desc))))
    {
...
OM_uint32 generic_gss_test_oid_set_member(OM_uint32* minor_status,
...
OM_uint32 generic_gss_oid_to_str(OM_uint32* minor_status,
...
static size_t arc_encoded_length(unsigned long arc)
{
    size_t len = 1;
...
static void arc_encode(unsigned long arc, unsigned char** bufp)
{
    unsigned char* p;
...
static int get_arc(const unsigned char** bufp, const unsigned char* end,
 ...
 * 2.
 */
OM_uint32 generic_gss_str_to_oid(OM_uint32* minor_status, gss_buffer_t oid_str,
                                 gss_OID* oid_out)
{
...
OM_uint32 generic_gss_oid_compose(OM_uint32* minor_status, const char* prefix,
                                  size_t prefix_len, int suffix,
                                  gss_OID_desc* oid)
{
    int osuffix, i;
    size_t nbytes;
...
OM_uint32 generic_gss_oid_decompose(OM_uint32* minor_status, const char* prefix,
                                    size_t prefix_len, gss_OID_desc* oid,
                                    int* suffix)
{
    size_t i, slen;
    unsigned char* op;
...
OM_uint32 generic_gss_copy_oid_set(OM_uint32* minor_status,
                                   ...
                                   gss_OID_set* new_oidset)
{
    ...
    if ((copy->elements = (gss_OID_desc*)gssalloc_calloc(
             oidset->count, sizeof(*copy->elements))) == NULL)
    {
        major = GSS_S_FAILURE;
        goto done;
    }
