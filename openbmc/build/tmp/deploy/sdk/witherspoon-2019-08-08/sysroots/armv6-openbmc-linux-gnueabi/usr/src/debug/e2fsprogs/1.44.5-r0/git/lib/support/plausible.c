...
static int magic_library_available(void)
{
    if (!magic_handle)
    {
        ...
        dl_magic_open = (magic_t(*)(int))dlsym(magic_handle, "magic_open");
        dl_magic_file = (const char* (*)(magic_t, const char*))dlsym(
            magic_handle, "magic_file");
        dl_magic_load =
            (int (*)(magic_t, const char*))dlsym(magic_handle, "magic_load");
        dl_magic_close = (void (*)(magic_t))dlsym(magic_handle, "magic_close");
    }

    if (!dl_magic_open || !dl_magic_file || !dl_magic_load || !dl_magic_close)
        return 0;
    return 1;
}
#else
...

{
    ...
    retval =
        ext2fs_open2(device, 0, EXT2_FLAG_64BITS, 0, 0, unix_io_manager, &fs);
    if (retval)
        return;
    sb = fs->super;
    ...
    if (sb->s_mtime)
    {
        ...
        if (sb->s_last_mounted[0])
        {
            ...
            printf(_("\tlast mounted on %s on %s"), buf, ctime(&tm));
        }
        else
...
#ifdef HAVE_MAGIC_H
    if ((flags & CHECK_FS_EXIST) && !getenv("E2FSPROGS_LIBMAGIC_SUPPRESS") &&
