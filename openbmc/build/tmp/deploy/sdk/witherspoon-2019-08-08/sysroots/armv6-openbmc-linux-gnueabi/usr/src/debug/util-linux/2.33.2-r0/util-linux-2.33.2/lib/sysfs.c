...
static int sysfs_blkdev_enoent_redirect(struct path_cxt* pc, const char* path,
                                        int* dirfd);
...
struct path_cxt* ul_new_sysfs_path(dev_t devno, struct path_cxt* parent,
                                   const char* prefix)
{
 ...
 *
 * The function is possible to call in loop and without
 * sysfs_blkdev_deinit_path(). The sysfs_blkdev_deinit_path() is automatically
 * called by ul_unref_path().
 *
 */
int sysfs_blkdev_init_path(struct path_cxt* pc, dev_t devno,
                           struct path_cxt* parent)
{
    struct sysfs_blkdev* blk;
    int rc;
    char buf[sizeof(_PATH_SYS_DEVBLOCK) +
             sizeof(stringify_value(UINT32_MAX)) * 2 + 3];

    /* define path to devno stuff */
    snprintf(buf, sizeof(buf), _PATH_SYS_DEVBLOCK "/%d:%d", major(devno),
             minor(devno));
...
int sysfs_blkdev_is_partition_dirent(DIR* dir, struct dirent* d,
...
dev_t sysfs_blkdev_partno_to_devno(struct path_cxt* pc, int partno)
{
    ...
    closedir(dir);
    DBG(CXT,
        ul_debugobj(pc, "partno (%d) -> devno (%d)", (int)partno, (int)devno));
    return devno;
}
...
char* sysfs_blkdev_get_devchain(struct path_cxt* pc, char* buf, size_t bufsz)
{
    ...
    memcpy(buf + psz, _PATH_SYS_DEVBLOCK "/",
...
{
    static const char* const hotplug_subsystems[] = {"usb", "ieee1394",
                                                     "pcmcia", "mmc", "ccw"};
    size_t i;
...
static int get_dm_wholedisk(struct path_cxt* pc, char* diskname, size_t len,
                            dev_t* diskdevno)
{
    int rc = 0;
    char* name;
    ...
    {
        *diskdevno =
            __sysfs_devname_to_devno(ul_path_get_prefix(pc), name, NULL);
        if (!*diskdevno)
            rc = -1;
    }
 ...
 */
int sysfs_blkdev_get_wholedisk(struct path_cxt* pc, char* diskname, size_t len,
                               dev_t* diskdevno)
{
    int is_part = 0;
    ...
    if (!is_part)
    {
        ...
        if (is_part && get_dm_wholedisk(pc, diskname, len, diskdevno) == 0)
            /*
...
int sysfs_devno_to_wholedisk(dev_t devno, char* diskname, size_t len,
                             dev_t* diskdevno)
{
    struct path_cxt* pc;
    int rc = 0;
...
int sysfs_blkdev_scsi_get_hctl(struct path_cxt* pc, int* h, int* c, int* t,
                               int* l)
{
...
static char* scsi_host_attribute_path(struct path_cxt* pc, const char* type,
                                      char* buf, size_t bufsz, const char* attr)
{
    ...
    if (attr)
        len = snprintf(buf, bufsz, "%s%s/%s_host/host%d/%s", prefix,
                       _PATH_SYS_CLASS, type, host, attr);
    else
        len = snprintf(buf, bufsz, "%s%s/%s_host/host%d", prefix,
                       _PATH_SYS_CLASS, type, host);
...
int sysfs_blkdev_scsi_host_is(struct path_cxt* pc, const char* type)
{
    ...
    if (!type || !scsi_host_attribute_path(pc, type, buf, sizeof(buf), NULL))
        return 0;

    return stat(buf, &st) == 0 && S_ISDIR(st.st_mode);
}

static char* scsi_attribute_path(struct path_cxt* pc, char* buf, size_t bufsz,
                                 const char* attr)
{
    int len, h, c, t, l;
    const char* prefix;
    ...
    if (attr)
        len = snprintf(buf, bufsz, "%s%s/devices/%d:%d:%d:%d/%s", prefix,
                       _PATH_SYS_SCSI, h, c, t, l, attr);
    else
        len = snprintf(buf, bufsz, "%s%s/devices/%d:%d:%d:%d", prefix,
                       _PATH_SYS_SCSI, h, c, t, l);
    return (len < 0 || (size_t)len >= bufsz) ? NULL : buf;
}
...
dev_t __sysfs_devname_to_devno(const char* prefix, const char* name,
                               const char* parent)
{
    char buf[PATH_MAX];
    ...
    if (parent && strncmp("dm-", name, 3))
    {
        ...
        len = snprintf(buf, sizeof(buf), "%s" _PATH_SYS_BLOCK "/%s/%s/dev",
     ...
     */
    len = snprintf(buf, sizeof(buf), "%s" _PATH_SYS_BLOCK "/%s/dev", prefix,
                   _name);
    if (len < 0 || (size_t)len >= sizeof(buf))
        goto done;
    dev = read_devno(buf);
    ...
    if (!dev)
    {
         ...
         */
        len = snprintf(buf, sizeof(buf), "%s" _PATH_SYS_BLOCK "/%s/device/dev",
                       prefix, _name);
        if (len < 0 || (size_t)len >= sizeof(buf))
            goto done;
        dev = read_devno(buf);
...
int main(int argc, char* argv[])
{
    ...
    printf("non-context:\n");
    printf(" DEVNO:   %u (%d:%d)\n", (unsigned int)devno, major(devno),
           minor(devno));
    ...
    printf(" WHOLEDISK-DEVNO:   %u (%d:%d)\n", (unsigned int)disk_devno,
           major(disk_devno), minor(disk_devno));
