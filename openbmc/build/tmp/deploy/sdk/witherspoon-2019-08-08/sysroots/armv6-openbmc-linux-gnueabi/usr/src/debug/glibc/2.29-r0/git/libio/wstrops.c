...
void _IO_wstr_init_static(FILE* fp, wchar_t* ptr, size_t size, wchar_t* pstart)
{
    wchar_t* end;
...
ssize_t _IO_wstr_count(FILE* fp)
{
    ...
    return ((wd->_IO_write_ptr > wd->_IO_read_end ? wd->_IO_write_ptr
                                                  : wd->_IO_read_end) -
            wd->_IO_read_base);
}
...
off64_t _IO_wstr_seekoff(FILE* fp, off64_t offset, int dir, int mode)
{
    ...
    else
    {
        ...
        if (mode & _IOS_INPUT)
        {
            ...
            base += offset;
            if (base > cur_size && enlarge_userbuf(fp, base, 1) != 0)
                return EOF;
            fp->_wide_data->_IO_read_ptr =
                (fp->_wide_data->_IO_read_base + base);
            fp->_wide_data->_IO_read_end =
                (fp->_wide_data->_IO_read_base + cur_size);
            new_pos = offset;
        }
        ...
        if (mode & _IOS_OUTPUT)
        {
            ...
            base += offset;
            if (base > cur_size && enlarge_userbuf(fp, base, 0) != 0)
                return EOF;
            fp->_wide_data->_IO_write_ptr =
                (fp->_wide_data->_IO_write_base + base);
            new_pos = base;
        }
    }
    return new_pos;
