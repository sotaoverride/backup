...
static void __attribute__((__noreturn__)) usage(void)
{
    ...
    fputs(_(" -M, --shmem <size>       create shared memory segment of size "
            "<size>\n"),
          out);
    fputs(_(" -S, --semaphore <number> create semaphore array with <number> "
            "elements\n"),
          out);
    fputs(_(" -Q, --queue              create message queue\n"), out);
    fputs(_(" -p, --mode <mode>        permission for the resource (default is "
            "0644)\n"),
          out);
