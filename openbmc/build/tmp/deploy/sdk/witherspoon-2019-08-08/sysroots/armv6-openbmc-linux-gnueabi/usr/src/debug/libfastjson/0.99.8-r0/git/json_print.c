...
/* Note: on OpenCSW, we have vasprintf() inside the headers, but not inside the
 * lib. So we need to use a different name, else we get issues with
 * redefinitions. We we solve this by using the macro below, which just renames
 * the function BUT does not affect the (variadic) arguments. rgerhards,
 * 2017-04-11
 */
...
static int rs_vasprintf(char** buf, const char* fmt, va_list ap)
{
    ...
    /* CAW: RAWR! We have to hope to god here that vsnprintf doesn't overwrite
       our buffer like on some 64bit sun systems.... but hey, its time to move
       on */
...
static size_t buffer_flush(struct buffer* buffer)
{
    ...
    size_t result =
        buffer->overflow(buffer->ptr, buffer->buffer, buffer->filled);
 ...
 *  @return size_t
 */
static size_t buffer_append(struct buffer* buffer, const char* data,
                            size_t size)
{
    // return value
    size_t result = 0;
    ...
    if (buffer->filled + size > buffer->size)
    {
        ...
        if (size > buffer->size)
            return result + buffer->overflow(buffer->ptr, data, size);
    }
 ...
 *  @return size_t
 */
__attribute__((__format__(__printf__, 2, 3))) static size_t
    buffer_printf(struct buffer* buffer, const char* format, ...)
{
    // return value
    size_t result = 0;
    ...
    if (buffer->size - buffer->filled < 32)
        result += buffer_flush(buffer);
    ...
    size = vsnprintf(buffer->buffer + buffer->filled,
                     buffer->size - buffer->filled - 1, format, arguments);

    // clean up varargs (it is not possible to reuse the vararg arguments later
    // on, the have to be reset and possible reinitialized later on)
    va_end(arguments);
    ...
    {
        // there was not enough room in the buffer, but it would have been
        // enough if we would have been able to use the entire buffer, so we
        // reset the buffer, and retry the whole procedure
        result += buffer_flush(buffer);
        ...
        buffer->size +=
            vsnprintf(buffer->buffer + buffer->filled,
                      buffer->size - buffer->filled - 1, format, arguments);
    ...
    else
    {
        ...
        // was this a success?
        if (size > 0)
            result += buffer_append(buffer, tmp, size);
...
static size_t escape(const char* str, struct buffer* buffer)
{
    ...
    { /* broken below on 0-byte */
        if (char_needsEscape[*((unsigned char*)str)])
        {
            ...
            if (str != start_offset)
                result +=
                    buffer_append(buffer, start_offset, str - start_offset);
            switch (*str)
            {
                case '\b':
                    result += buffer_append(buffer, "\\b", 2);
                    break;
                case '\n':
                    result += buffer_append(buffer, "\\n", 2);
                    break;
                case '\r':
                    result += buffer_append(buffer, "\\r", 2);
                    break;
                case '\t':
                    result += buffer_append(buffer, "\\t", 2);
                    break;
                case '\f':
                    result += buffer_append(buffer, "\\f", 2);
                    break;
                case '"':
                    result += buffer_append(buffer, "\\\"", 2);
                    break;
    ...
    if (str != start_offset)
        result += buffer_append(buffer, start_offset, str - start_offset);
    return result;
}
                           ...
                           struct buffer* buffer)
{
    ...
    while (!fjson_object_iter_equal(&it, &itEnd))
    {
        ...
        if (flags & FJSON_TO_STRING_SPACED)
            result += buffer_append(buffer, "\": ", 3);
        else
            result += buffer_append(buffer, "\":", 2);
        result +=
            write(fjson_object_iter_peek_value(&it), level + 1, flags, buffer);
...
{
    if (jso->o.c_boolean)
        return buffer_append(buffer, "true", 4);
    else
        return buffer_append(buffer, "false", 5);
}
...
static size_t write_double(struct fjson_object* jso, int flags,
                           struct buffer* buffer)
{
    ...
    if (jso->o.c_double.source)
        return buffer_append(buffer, jso->o.c_double.source,
    ...
    if (isnan(jso->o.c_double.value))
        return buffer_append(buffer, "NaN", 3);
    if (isinf(jso->o.c_double.value))
        return buffer_printf(buffer, jso->o.c_double.value > 0 ? "Infinity"
                                                               : "-Infinity");

    // store the beginning of the buffer (this is where buffer_printf() will
    // most likely write)
    buf = buffer->buffer + buffer->filled;

    // write to the buffer
    result = buffer_printf(
        buffer,
        (modf(jso->o.c_double.value, &dummy) == 0) ? "%.17g.0" : "%.17g",
        jso->o.c_double.value);

    // if the buffer got flushed
    if (buffer->buffer + buffer->filled < buf)
        buf = buffer->buffer;
...
static size_t write_string(struct fjson_object* jso, struct buffer* buffer)
{
    return buffer_append(buffer, "\"", 1) +
           escape(get_string_component(jso), buffer) +
                          ...
                          struct buffer* buffer)
{
    ...
    for (ii = 0; ii < fjson_object_array_length(jso); ii++)
    {
        ...
        if (flags & FJSON_TO_STRING_SPACED)
            result += buffer_append(buffer, " ", 1);
        result += indent(level + 1, flags, buffer);
        result += write(fjson_object_array_get_idx(jso, ii), level + 1, flags,
                        buffer);
    }
    if (flags & FJSON_TO_STRING_PRETTY)
    {
                    ...
                    struct buffer* buffer)
{
    ...
    if (!jso)
        return buffer_append(buffer, "null", 4);
    ...
    switch (jso->o_type)
    {
        ...
        case fjson_type_boolean:
            return write_boolean(jso, buffer);
        case fjson_type_double:
            return write_double(jso, flags, buffer);
        case fjson_type_int:
            return write_int(jso, buffer);
        case fjson_type_object:
            return write_object(jso, level, flags, buffer);
        case fjson_type_array:
            return write_array(jso, level, flags, buffer);
        case fjson_type_string:
            return write_string(jso, buffer);
        default:
            return 0;
    }
}
...
static size_t calculate(void __attribute__((unused)) * ptr,
                        const char __attribute__((unused)) * buffer,
                        size_t size)
{
    return size;
}
...
size_t fjson_object_dump_buffered(struct fjson_object* jso, int flags,
                                  char* temp, size_t size, fjson_write_fn* func,
                                  void* ptr)
{
    // construct a buffer
...
size_t fjson_object_dump(struct fjson_object* jso, fjson_write_fn* func,
                         void* ptr)
{
    // write the value
...
{
    // write the value with a dummy function (this is a simple implementation
    // that can later be optimized in a pure size-calculating function)
...
{
    return fjson_object_dump_ext(obj, FJSON_TO_STRING_SPACED, fwrite_wrapper,
                                 fp);
}
