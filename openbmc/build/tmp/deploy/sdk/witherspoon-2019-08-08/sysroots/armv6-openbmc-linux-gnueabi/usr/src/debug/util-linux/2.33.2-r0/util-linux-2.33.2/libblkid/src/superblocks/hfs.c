...
static int probe_hfsplus(blkid_probe pr, const struct blkid_idmag* mag)
{
    ...
    if (memcmp(sbd->signature, "BD", 2) == 0)
    {
        ...
        off =
            (alloc_first_block * 512) + (embed_first_block * alloc_block_size);

        buf = blkid_probe_get_buffer(pr, off + (mag->kboff * 1024),
                                     sizeof(struct hfsplus_vol_header));
        hfsplus = (struct hfsplus_vol_header*)buf;
    }
    else
        hfsplus = blkid_probe_get_sb(pr, mag, struct hfsplus_vol_header);
    ...
    buf = blkid_probe_get_buffer(pr, off + ((uint64_t)cat_block * blocksize),
                                 0x2000);
    if (!buf)
        return errno ? -errno : 0;

    bnode = (struct hfsplus_bheader_record*)&buf[sizeof(
        struct hfsplus_bnode_descriptor)];
    ...
    buf = blkid_probe_get_buffer(pr, (uint64_t)off + leaf_off, leaf_node_size);
    if (!buf)
        return errno ? -errno : 0;
    ...
    key = (struct hfsplus_catalog_key*)&buf[sizeof(
        struct hfsplus_bnode_descriptor)];
    ...
    blkid_probe_set_utf8label(
        pr, key->unicode, be16_to_cpu(key->unicode_len) * 2, BLKID_ENC_UTF16BE);
    return 0;
}
...
const struct blkid_idinfo hfs_idinfo = {
    ...
    .magics = {{.magic = "BD", .len = 2, .kboff = 1}, {NULL}}};

const struct blkid_idinfo hfsplus_idinfo = {
    .name = "hfsplus",
    .usage = BLKID_USAGE_FILESYSTEM,
    .probefunc = probe_hfsplus,
    .magics = {{.magic = "BD", .len = 2, .kboff = 1},
