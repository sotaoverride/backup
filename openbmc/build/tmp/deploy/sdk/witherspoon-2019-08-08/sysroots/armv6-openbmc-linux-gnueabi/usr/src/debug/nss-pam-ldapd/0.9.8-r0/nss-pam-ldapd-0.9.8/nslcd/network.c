...
static int mkfilter_network_byname(const char* name, char* buffer,
                                   size_t buflen)
{
    ...
    {
        log_log(LOG_ERR,
                "mkfilter_network_byname(): safename buffer too small");
        return -1;
    }
    /* build filter */
    return mysnprintf(buffer, buflen, "(&%s(%s=%s))", network_filter,
                      attmap_network_cn, safename);
}

static int mkfilter_network_byaddr(const char* addrstr, char* buffer,
                                   size_t buflen)
{
    char safeaddr[64];
    /* escape attribute */
    ...
    {
        log_log(LOG_ERR,
                "mkfilter_network_byaddr(): safeaddr buffer too small");
        return -1;
    }
    /* build filter */
    return mysnprintf(buffer, buflen, "(&%s(%s=%s))", network_filter,
...
NSLCD_HANDLE(network, byname, NSLCD_ACTION_NETWORK_BYNAME,
...
NSLCD_HANDLE(
    network, byaddr, NSLCD_ACTION_NETWORK_BYADDR, int af; char addr[64];
    int len = sizeof(addr); char addrstr[64]; char filter[BUFLEN_FILTER];
...
NSLCD_HANDLE(network, all, NSLCD_ACTION_NETWORK_ALL, const char* filter;
             log_setrequest("network(all)");
             , (filter = network_filter, 0), write_network(fp, entry))
