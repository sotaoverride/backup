...
static FT_Error check_type1_format(FT_Stream stream, const char* header_string,
...
T1_New_Parser(T1_Parser parser, FT_Stream stream, FT_Memory memory,
              PSAux_Service psaux)
{
    FT_Error error;
    FT_UShort tag;
...
T1_Get_Private_Dict(T1_Parser parser, PSAux_Service psaux)
{
    ...
    Found:
        ...
        test_cr = FT_BOOL(!pos_lf ||
                          pos_lf > ft_memchr(cur, '\r', (size_t)(limit - cur)));
        ...
        if (cur + 3 < limit && ft_isxdigit(cur[0]) && ft_isxdigit(cur[1]) &&
            ft_isxdigit(cur[2]) && ft_isxdigit(cur[3]))
        {
            ...
            (void)psaux->ps_parser_funcs->to_bytes(
                &parser->root, parser->private_dict, parser->private_len, &len,
                0);
