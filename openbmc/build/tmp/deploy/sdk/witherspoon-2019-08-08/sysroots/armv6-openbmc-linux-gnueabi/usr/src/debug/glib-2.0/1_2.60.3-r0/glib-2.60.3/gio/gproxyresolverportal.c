...
G_DEFINE_TYPE_WITH_CODE(
    GProxyResolverPortal, g_proxy_resolver_portal, G_TYPE_OBJECT,
    G_IMPLEMENT_INTERFACE(G_TYPE_PROXY_RESOLVER,
                          g_proxy_resolver_portal_iface_init)
        _g_io_modules_ensure_extension_points_registered();
    g_io_extension_point_implement(G_PROXY_RESOLVER_EXTENSION_POINT_NAME,
                                   g_define_type_id, "portal", 90))

static gboolean ensure_resolver_proxy(GProxyResolverPortal* resolver)
{
...
static void g_proxy_resolver_portal_init(GProxyResolverPortal* resolver)
{
}

static gboolean g_proxy_resolver_portal_is_supported(GProxyResolver* object)
{
...
static gchar** g_proxy_resolver_portal_lookup(GProxyResolver* proxy_resolver,
                                              const gchar* uri,
                                              ...
                                              GError** error)
{
    ...
    if (!gxdp_proxy_resolver_call_lookup_sync(resolver->resolver, uri, &proxy,
                                              cancellable, error))
        return NULL;
...
static void g_proxy_resolver_portal_lookup_async(GProxyResolver* proxy_resolver,
                                                 const gchar* uri,
                                                 GCancellable* cancellable,
                                                 ...
                                                 gpointer user_data)
{
    ...
    gxdp_proxy_resolver_call_lookup(resolver->resolver, uri, cancellable,
                                    lookup_done, g_object_ref(task));
    g_object_unref(task);
}
...
static void g_proxy_resolver_portal_finalize(GObject* object)
{
    GProxyResolverPortal* resolver = G_PROXY_RESOLVER_PORTAL(object);
...
static void g_proxy_resolver_portal_class_init(
    GProxyResolverPortalClass* resolver_class)
{
