...
static void verify_mesg(struct verify_context* vfy, char type, const char* fmt,
                        va_list ap)
{
    ...
    while (mnt_table_next_fs(vfy->tb, itr, &next) == 0)
    {
        ...
        if (strncmp(n_tgt, tgt, len) == 0)
        {
            if (*(tgt + len) == '\0')
                ...
                verify_err(vfy, _("wrong order: %s specified before %s"), tgt,
                           n_tgt);
        }
    }
done:
                        ...
                        const char* value)
{
    ...
    {
        if (mnt_fs_get_option(vfy->fs, "noauto", NULL, NULL) == 1)
            verify_err(vfy, _("unreachable on boot required source: %s=%s"),
                       name, value);
        else
...
static int verify_source(struct verify_context* vfy)
{
    ...
    if (mnt_fs_is_pseudofs(vfy->fs) || mnt_fs_is_netfs(vfy->fs))
        ...
        verify_warn(
            vfy, _("non-bind mount source %s is a directory or regular file"),
            src);
...
static int verify_swaparea(struct verify_context* vfy)
{
    ...
    if (mnt_fs_get_option(vfy->fs, "discard", &arg, &argsz) == 0 && arg &&
        strncmp(arg, "once", argsz) != 0 && strncmp(arg, "pages", argsz) != 0)
...
static int verify_fstype(struct verify_context* vfy)
{
    ...
    if (type)
    {
        ...
        if (none && mnt_fs_get_option(vfy->fs, "bind", NULL, NULL) == 1 &&
            mnt_fs_get_option(vfy->fs, "move", NULL, NULL) == 1)
            return verify_warn(vfy, _("\"none\" FS type is recommended for "
                                      "bind or move oprations only"));
        ...
        if (!isswap && !isauto && !none && !is_supported_filesystem(vfy, type))
            verify_warn(vfy, _("%s seems unsupported by the current kernel"),
                        type);
    }
    ...
    if (realtype)
    {
        ...
        if (type && !isauto && strcmp(type, realtype) != 0)
            return verify_err(vfy, _("%s does not match with on-disk %s"), type,
                              realtype);

        if (!isswap && !is_supported_filesystem(vfy, realtype))
            return verify_err(
                vfy, _("on-disk %s seems unsupported by the current kernel"),
                realtype);
...
static int verify_passno(struct verify_context* vfy)
{
    ...
    if (tgt && strcmp("/", tgt) == 0 && passno != 1)
        return verify_warn(
            vfy, _("recommended root FS passno is 1 (current is %d)"), passno);
...
int verify_table(struct libmnt_table* tb)
{
    ...
    {
        fputc('\n', stderr);
        fprintf(stderr, P_("%d parse error", "%d parse errors", parse_nerrors),
                parse_nerrors);
        fprintf(stderr, P_(", %d error", ", %d errors", vfy.nerrors),
                vfy.nerrors);
        fprintf(stderr, P_(", %d warning", ", %d warnings", vfy.nwarnings),
                vfy.nwarnings);
        fputc('\n', stderr);
    }
    else
