...
static int verify_tag(const char* devname, const char* name, const char* value)
{
    ...
    DBG(EVALUATE, ul_debug("%s: %s verification %s", devname, name,
                           rc == 0 ? "PASS" : "FAILED"));
    if (fd >= 0)
        close(fd);
    blkid_free_probe(pr);
...
int blkid_send_uevent(const char* devname, const char* action)
{
    ...
    DBG(EVALUATE,
        ul_debug("%s: send uevent %s", uevent, rc == 0 ? "SUCCESS" : "FAILED"));
    return rc;
}
 ...
 */
char* blkid_evaluate_tag(const char* token, const char* value,
                         blkid_cache* cache)
{
