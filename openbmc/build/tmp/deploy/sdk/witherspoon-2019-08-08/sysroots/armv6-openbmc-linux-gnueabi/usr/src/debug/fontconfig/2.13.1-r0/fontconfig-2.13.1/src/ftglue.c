...
static FT_Pointer ftglue_qalloc(FT_Memory memory, FT_ULong size,
                                FT_Error* perror)
{
    FT_Error error = 0;
...
static void ftglue_free(FT_Memory memory, FT_Pointer block)
{
    if (block)
...
ftglue_face_goto_table(FT_Face face, FT_ULong the_tag, FT_Stream stream)
{
    ...
    LOG(("ftglue_face_goto_table( %p, %c%c%c%c, %p )\n", face,
         (int)((the_tag >> 24) & 0xFF), (int)((the_tag >> 16) & 0xFF),
         (int)((the_tag >> 8) & 0xFF), (int)(the_tag & 0xFF), stream));
    ...
    else
    {
        ...
        if (sig == FT_MAKE_TAG('t', 't', 'c', 'f'))
        {
            ...
            if (FILE_Seek(12 + face->face_index * 4) || ACCESS_Frame(4))
                goto Exit;
        ...
        if (FILE_Seek(offset + 4) || ACCESS_Frame(2))
            goto Exit;
        ...
        if (FILE_Seek(offset + 12) || ACCESS_Frame(count * 16))
            goto Exit;
