...
static int Debug =
    0; /* Debug level. Each "-d" on the cmd line increases the level */
...
{
    printf("Usage: plotJittervsFill [options] -f [--file] <input log file "
           "name> -t [--jitter_threshold] <jitter threshold in ms>\n");
    printf("[options]:\n-v [--version] Print version and exit\n");
    printf("-d Debug. Prints input file line number for each data point picked "
           "up.\n");
 ...
 ***********************************************************************/
static void
    HandleCmdLineArgs(int argc,     /* number of command-line arguments */
    ...
    if (argc > (int)1)
    {
        ...
        for (argNum = (int)1; argNum < argc; argNum++)
        {
                     ...
                     (strcmp(argv[argNum], "-t") == 0))
            {
                ...
                {
                    printf("A jitter threshold less than %i ms is not allowed. "
                           "Bye.\n",
...
#define MAX_SAVE_BUFFER                                                        \
    1000 /* How many values will be picked up while searching for              \
      a % disk full line (i.e. before they can be printed out)                 \
    */
    int saveJitter[MAX_SAVE_BUFFER]; /* lets us record multiple jitter values
                            that exceed our threshold till we find a "df" field-
                            which is when we can o/p all these values.
                         */
    int dataLineNo[MAX_SAVE_BUFFER]; /* The saved line #'s for the above.
                                        Printed if debug specified. */

    int saveJitterCnt = 0;
    int lookFor_df = FALSE;
    int dfPercent =
        -1; /* will be >= 0 if at least one found. The init value is a flag. */
    ...
    while (fgets(lineBuf, sizeof(lineBuf), fp) != NULL)
    {
        ...
        if ((strstr(lineBuf, "%") != NULL) && (lookFor_df))
        {
            converted = sscanf(lineBuf, "%s %i %i %i %i\n", junkStr1, &junkInt1,
                               &junkInt2, &junkInt3, &dfPercent);
            if (converted < 5)
            {
                printf("Line %i contains \"%%\", but expected fields not "
                       "found. Skipping.\n",
                       lineNo);
            }
            else
            {
                /* Now print out the saved jitter values (in col2) with this
                 * dfPercent value as the col1. */
        ...
        if (converted < 4)
        {
            printf("Line %i contains \"ms\", but expected fields not found. "
                   "Converted %i, Skipping.",
                   lineNo, converted);
            printf("1=%i, 2=%s.\n", junkInt1, junkStr1);
        ...
        if (abs((int)jitter_ms) > JitterThreshold_ms)
        {
            ...
            */
            ...
            {
                saveJitter[saveJitterCnt] =
                    abs((int)jitter_ms); /* why keep the (ms) jitter in float */
            ...
            {
                printf("Oops! I've run out of buffer space before I found a %% "
                       "use line. Dropping itter value. Increase "
                       "MAX_SAVE_BUFFER and recompile.\n");
            }
        }
    }

    /* Now print out any saved jitter values that were not printed out because
       we did not find and "df" after these were picked up. Only print if a "df"
       disk usage was ever found.
     */
    if (lookFor_df && (dfPercent >= 0))
    {
        /* Now print out the saved jitter values (in col2) with this dfPercent
         * value as the col1. */
        for (cnt = 0; cnt < saveJitterCnt; cnt++)
        {
            fprintf(stderr, "%i\t%i\n", dfPercent, (int)saveJitter[cnt]);
