...
static int pss_verify_cmp(void* opaque, gcry_mpi_t tmp)
{
    ...
    return _gcry_rsa_pss_verify(hash, tmp, ctx->nbits - 1, ctx->hash_algo,
                                ctx->saltlen);
}
...
static int get_hash_algo(const char* s, size_t n)
{
    static const struct
    {
        const char* name;
    ...
    {
        if (strlen(hashnames[i].name) == n && !memcmp(hashnames[i].name, s, n))
            break;
    }
    if (hashnames[i].name)
 ...
 * stores 0 at R_E.
 */
gpg_err_code_t _gcry_pk_util_get_rsa_use_e(gcry_sexp_t list, unsigned long* r_e)
{
    char buf[50];
    const char* s;
    size_t n;
                                          ...
                                          struct pk_encoding_ctx* ctx)
{
    ...
    *ret_mpi = NULL;
    ...
    /* See whether there is a flags list.  */
    {
        ...
        if (lflags)
        {
            if (_gcry_pk_util_parse_flaglist(lflags, &parsed_flags,
                                             &ctx->encoding))
                unknown_flag = 1;
            sexp_release(lflags);
        }
             ...
             ctx->op == PUBKEY_OP_ENCRYPT)
    {
        ...
        else
        {
            ...
            rc = _gcry_rsa_pkcs1_encode_for_enc(ret_mpi, ctx->nbits, value,
                                                valuelen, random_override,
                                                random_override_len);
             ...
             (ctx->op == PUBKEY_OP_SIGN || ctx->op == PUBKEY_OP_VERIFY))
    {
        ...
        else
        {
            ...
            if (!ctx->hash_algo)
                rc = GPG_ERR_DIGEST_ALGO;
            else if (!(value = sexp_nth_data(lhash, 2, &valuelen)) || !valuelen)
                rc = GPG_ERR_INV_OBJ;
            else
                rc = _gcry_rsa_pkcs1_encode_for_sig(ret_mpi, ctx->nbits, value,
                                                    valuelen, ctx->hash_algo);
        }
    }
             ...
             (ctx->op == PUBKEY_OP_SIGN || ctx->op == PUBKEY_OP_VERIFY))
    {
        ...
        if (sexp_length(lvalue) != 2)
            ...
            rc = _gcry_rsa_pkcs1_encode_raw_for_sig(ret_mpi, ctx->nbits, value,
                                                    valuelen);
    }
             ...
             ctx->op == PUBKEY_OP_ENCRYPT)
    {
        ...
        else
        {
            ...
            rc = _gcry_rsa_oaep_encode(ret_mpi, ctx->nbits, ctx->hash_algo,
                                       value, valuelen, ctx->label,
                                       ctx->labellen, random_override,
                                       random_override_len);
             ...
             ctx->op == PUBKEY_OP_SIGN)
    {
        ...
        else
        {
            ...
            else
            {
                ...
                rc = _gcry_rsa_pss_encode(
                    ret_mpi, ctx->nbits - 1, ctx->hash_algo, value, valuelen,
                    ctx->saltlen, random_override, random_override_len);
