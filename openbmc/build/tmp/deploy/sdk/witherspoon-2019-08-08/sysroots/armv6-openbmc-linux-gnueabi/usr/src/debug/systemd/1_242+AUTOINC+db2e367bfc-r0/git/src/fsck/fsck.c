...
static void start_target(const char* target, const char* mode)
{
    ...
    r = sd_bus_call_method(
        ...
        "org.freedesktop.systemd1.Manager", "StartUnitReplace", &error, NULL,
    ...
    else if (streq(key, "fsck.repair"))
    {
        ...
        else
        {
            ...
            if (r > 0)
                ...
                log_warning("Invalid fsck.repair= parameter '%s'. Ignoring.",
                            value);
        }
    }
...
static void test_files(void)
{
    ...
    {
        log_error("Please pass 'fsck.mode=skip' on the kernel command line "
                  "rather than creating /fastboot on the root file system.");
        arg_skip = true;
    }

    if (access("/forcefsck", F_OK) >= 0)
    {
        log_error("Please pass 'fsck.mode=force' on the kernel command line "
                  "rather than creating /forcefsck on the root file system.");
        arg_force = true;
    }
#endif
...
static int fsck_progress_socket(void)
{
    ...
    if (connect(fd, &sa.sa, SOCKADDR_UN_LEN(sa.un)) < 0)
        return log_full_errno(
            IN_SET(errno, ECONNREFUSED, ENOENT) ? LOG_DEBUG : LOG_WARNING,
            errno, "Failed to connect to progress socket %s, ignoring: %m",
            sa.un.sun_path);
...
static int run(int argc, char* argv[])
{
    ...
    if (sd_device_get_property_value(dev, "ID_FS_TYPE", &type) >= 0)
    {
        ...
        {
            log_device_info(
                dev, "fsck.%s doesn't exist, not checking file system.", type);
            return 0;
        }
    }
    ...
    r = safe_fork("(fsck)", FORK_RESET_SIGNALS | FORK_DEATHSIG | FORK_LOG,
                  &pid);
    if (r < 0)
        return r;
    if (r == 0)
    {
        ...
        {
            /* If this worked we close the progress pipe early, and just use the
             * socket */
        ...
        {
            /* Otherwise if we have the progress pipe to our own local handle,
             * we use it */
