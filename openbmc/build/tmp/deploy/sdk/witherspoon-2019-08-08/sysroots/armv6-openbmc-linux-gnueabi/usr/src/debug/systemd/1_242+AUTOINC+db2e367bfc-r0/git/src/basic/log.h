...
#define log_set_max_level(level) log_set_max_level_realm(LOG_REALM, (level))
...
#define log_get_max_level() log_get_max_level_realm(LOG_REALM)
...
#define log_parse_environment() log_parse_environment_realm(LOG_REALM)

int log_dispatch_internal(int level, int error, const char* file, int line,
                          const char* func, const char* object_field,
                          const char* object, const char* extra,
                          const char* extra_field, char* buffer);
...
int log_struct_iovec_internal(int level, int error, const char* file, int line,
                              const char* func, const struct iovec* input_iovec,
...
#define log_dispatch(level, error, buffer)                                     \
    log_dispatch_internal(level, error, __FILE__, __LINE__, __func__, NULL,    \
...
#define log_emergency_errno(error, ...)                                        \
    log_full_errno(log_emergency_level(), error, __VA_ARGS__)
...
#define log_struct_errno(level, error, ...)                                    \
    log_struct_internal(LOG_REALM_PLUS_LEVEL(LOG_REALM, level), error,         \
                        __FILE__, __LINE__, __func__, __VA_ARGS__, NULL)
#define log_struct(level, ...) log_struct_errno(level, 0, __VA_ARGS__)

#define log_struct_iovec_errno(level, error, iovec, n_iovec)                   \
    log_struct_iovec_internal(LOG_REALM_PLUS_LEVEL(LOG_REALM, level), error,   \
                              __FILE__, __LINE__, __func__, iovec, n_iovec)
#define log_struct_iovec(level, iovec, n_iovec)                                \
    log_struct_iovec_errno(level, 0, iovec, n_iovec)

/* This modifies the buffer passed! */
#define log_dump(level, buffer)                                                \
    log_dump_internal(LOG_REALM_PLUS_LEVEL(LOG_REALM, level), 0, __FILE__,     \
...
/* If turned on, any requests for a log target involving "syslog" will be
 * implicitly upgraded to the equivalent journal target */
void log_set_upgrade_syslog_to_journal(bool b);

/* If turned on, and log_open() is called, we'll not use STDERR_FILENO for
 * logging ever, but rather open /dev/console */
void log_set_always_reopen_console(bool b);

/* If turned on, we'll open the log stream implicitly if needed on each
 * individual log call. This is normally not desired as we want to reuse our
 * logging streams. It is useful however  */
...
#define log_syntax(unit, level, config_file, config_line, error, ...)          \
    ({                                                                         \
        ...
        (log_get_max_level() >= LOG_PRI(_level))                               \
            ? log_syntax_internal(unit, _level, config_file, config_line, _e,  \
                                  __FILE__, __LINE__, __func__, __VA_ARGS__)   \
            : -ERRNO_VALUE(_e);                                                \
    })
...
#define log_syntax_invalid_utf8(unit, level, config_file, config_line, rvalue) \
    ({                                                                         \
        int _level = (level);                                                  \
        (log_get_max_level() >= LOG_PRI(_level))                               \
            ? log_syntax_invalid_utf8_internal(unit, _level, config_file,      \
                                               config_line, __FILE__,          \
                                               __LINE__, __func__, rvalue)     \
            : -EINVAL;                                                         \
    })
