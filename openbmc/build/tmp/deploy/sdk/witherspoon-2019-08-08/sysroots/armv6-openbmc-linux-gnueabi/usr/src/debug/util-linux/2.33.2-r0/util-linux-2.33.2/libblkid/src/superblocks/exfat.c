...
static uint32_t next_cluster(blkid_probe pr, const struct exfat_super_block* sb,
                             uint32_t cluster)
{
    uint32_t* next;
    uint64_t fat_offset;
    ...
    next = (uint32_t*)blkid_probe_get_buffer(pr, fat_offset, sizeof(uint32_t));
    if (!next)
        return 0;
                                            ...
                                            const struct exfat_super_block* sb)
{
    ...
    for (; i < max_iter; i++)
    {
        entry = (uint8_t*)blkid_probe_get_buffer(pr, offset, EXFAT_ENTRY_SIZE);
        if (!entry)
            return NULL;
...
static int probe_exfat(blkid_probe pr, const struct blkid_idmag* mag)
{
    ...
    if (label)
        blkid_probe_set_utf8label(pr, label->name, min(label->length * 2, 30),
                                  BLKID_ENC_UTF16LE);
    else if (errno)
        return -errno;

    blkid_probe_sprintf_uuid(pr, sb->volume_serial, 4,
                             "%02hhX%02hhX-%02hhX%02hhX", sb->volume_serial[3],
                             sb->volume_serial[2], sb->volume_serial[1],
                             sb->volume_serial[0]);

    blkid_probe_sprintf_version(pr, "%u.%u", sb->version.vermaj,
                                sb->version.vermin);
...
const struct blkid_idinfo exfat_idinfo = {
    ...
    .magics = {{.magic = "EXFAT   ", .len = 8, .sboff = 3}, {NULL}}};
