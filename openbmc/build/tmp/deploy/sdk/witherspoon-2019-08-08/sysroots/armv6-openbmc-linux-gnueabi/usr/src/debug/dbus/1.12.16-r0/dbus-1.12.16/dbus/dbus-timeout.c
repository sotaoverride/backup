...
struct DBusTimeout
{
    ...
    DBusFreeFunction
        free_handler_data_function; /**< Free the timeout handler data. */

    void* data;                          /**< Application data. */
    DBusFreeFunction free_data_function; /**< Free the application data. */
    unsigned int enabled : 1;            /**< True if timeout is active. */
    unsigned int needs_restart : 1; /**< Flag that timeout should be restarted
                                       after re-enabling. */
};
 ...
 */
DBusTimeout* _dbus_timeout_new(int interval, DBusTimeoutHandler handler,
...
void _dbus_timeout_unref(DBusTimeout* timeout)
{
    ...
    {
        dbus_timeout_set_data(timeout, NULL,
                              NULL); /* call free_data_function */
...
struct DBusTimeoutList
{
    ...
    DBusAddTimeoutFunction
        add_timeout_function; /**< Callback for adding a timeout. */
    DBusRemoveTimeoutFunction
        remove_timeout_function; /**< Callback for removing a timeout. */
    DBusTimeoutToggledFunction
        timeout_toggled_function; /**< Callback when timeout is enabled/disabled
                                     or changes interval */
    void* timeout_data;           /**< Data for timeout callbacks */
    DBusFreeFunction timeout_free_data_function; /**< Free function for timeout
                                                    callback data */
};
...
void _dbus_timeout_list_free(DBusTimeoutList* timeout_list)
{
    ...
    _dbus_timeout_list_set_functions(timeout_list, NULL, NULL, NULL, NULL,
                                     NULL);

    _dbus_list_foreach(&timeout_list->timeouts,
                       (DBusForeachFunction)_dbus_timeout_unref, NULL);
...
dbus_bool_t _dbus_timeout_list_set_functions(
    DBusTimeoutList* timeout_list, DBusAddTimeoutFunction add_function,
    ...
    DBusFreeFunction free_data_function)
{
    ...
    if (add_function != NULL)
    {
        ...
        {
            DBusList* next =
                _dbus_list_get_next_link(&timeout_list->timeouts, link);
            ...
            if (!(*add_function)(link->data, data))
            {
                ...
                while (link2 != link)
                {
                    DBusList* next2 = _dbus_list_get_next_link(
                        &timeout_list->timeouts, link2);
 ...
 *
 * Use dbus_connection_set_timeout_functions() or
 * dbus_server_set_timeout_functions() to be notified when libdbus needs to add
 * or remove timeouts.
 *
 * @{
 */
