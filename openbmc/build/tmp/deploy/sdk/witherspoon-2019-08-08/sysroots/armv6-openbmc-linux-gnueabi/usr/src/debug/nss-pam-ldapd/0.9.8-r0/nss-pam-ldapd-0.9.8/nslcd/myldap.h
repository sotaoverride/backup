...
MUST_USE int myldap_bind(MYLDAP_SESSION* session, const char* dn,
                         const char* password, int* response,
                         const char** message);
...
MUST_USE MYLDAP_SEARCH* myldap_search(MYLDAP_SESSION* session, const char* base,
...
/* Get the attribute values from a certain entry as a NULL terminated list.
   May return NULL or an empty array. */
MUST_USE const char** myldap_get_values_len(MYLDAP_ENTRY* entry,
                                            const char* attr);

/* Checks to see if the entry has the specified object class. */
MUST_USE int myldap_has_objectclass(MYLDAP_ENTRY* entry,
...
MUST_USE const char* myldap_get_rdn_value(MYLDAP_ENTRY* entry,
                                          const char* attr);
