...
static int _pam_add_handler(pam_handle_t* pamh, int must_fail, int other,
                            int stack_level, int type, int* actions,
                            const char* mod_path, int argc, char** argv,
                            int argvlen);
    ...
    while ((x = _pam_assemble_line(f, buf, BUF_SIZE)) > 0)
    {
        ...
        if (!strcasecmp(this_service, pamh->service_name) || other)
        {
            ...
            if (tok == NULL)
            {
                ...
                D(("_pam_init_handlers: empty module type for %s",
                   this_service));
                pam_syslog(pamh, LOG_ERR, "(%s) empty module type",
                           this_service);
                module_type = (requested_module_type != PAM_T_ANY)
                                  ? requested_module_type
                                  : PAM_T_AUTH;  /* most sensitive */
                handler_type = PAM_HT_MUST_FAIL; /* install as normal but fail
                                                    when dispatched */
            }
            else
            {
                if (tok[0] == '-')
                { /* do not log module load errors */
                ...
                else
                {
                    ...
                    module_type = (requested_module_type != PAM_T_ANY)
                                      ? requested_module_type
                                      : PAM_T_AUTH;  /* most sensitive */
                    handler_type = PAM_HT_MUST_FAIL; /* install as normal but
            ...
            {
                D(("Skipping config entry: %s (requested=%d, found=%d)", tok,
            ...
            if (tok == NULL)
            {
                ...
                pam_syslog(pamh, LOG_ERR, "(%s) no control flag supplied",
                           this_service);
            ...
            if (pam_include)
            {
                if (substack)
                {
                    res = _pam_add_handler(pamh, PAM_HT_SUBSTACK, other,
                                           stack_level, module_type, actions,
                                           tok, 0, NULL, 0);
                    if (res != PAM_SUCCESS)
                    {
                        pam_syslog(pamh, LOG_ERR, "error adding substack %s",
                                   tok);
            ...
            else
            {
                ...
                pam_syslog(pamh, LOG_ERR, "(%s) no module name supplied",
                           this_service);
                mod_path = NULL;
                handler_type = PAM_HT_MUST_FAIL;
            }
            ...
            { /* there are no arguments so fix by hand */
...
#ifdef PAM_DEBUG
            {
                ...
                D(("CONF%s: %s%s %d %s %d",
                   ...
                   this_service, other ? "(backup)" : "", module_type, mod_path,
                   argc));
                for (y = 0; y < argc; y++)
                {
                    D(("CONF: %s", argv[y]));
                ...
                {
                    D(("RETURN %s(%d) -> %d %s", _pam_token_returns[y], y,
                       actions[y],
                       actions[y] > 0 ? "jump"
            ...
            res = _pam_add_handler(pamh, handler_type, other, stack_level,
                                   module_type, actions, mod_path, argc, argv,
                                   argvlen);
            if (res != PAM_SUCCESS)
            {
...
static int _pam_open_config_file(pam_handle_t* pamh, const char* service,
                                 char** path, FILE** file)
{
    char* p;
    FILE* f;
    ...
    if (_pam_open_config_file(pamh, config_name, &path, &f) == PAM_SUCCESS)
    {
        ...
        if (retval != PAM_SUCCESS)
            pam_syslog(pamh, LOG_ERR,
                       "_pam_load_conf_file: error reading %s: %s", path,
...
int _pam_init_handlers(pam_handle_t* pamh)
{
    ...
    /* Is the PAM subsystem locked? */
    {
        ...
        {
            pam_syslog(pamh, LOG_ERR,
                       "_pam_init_handlers: PAM lockfile (" PAM_LOCK_FILE
                       ") exists - aborting");
            (void)close(fd_tmp);
            /*
     ...
     */
    {
            ...
            (stat(PAM_CONFIG_DIST_D, &test_d) == 0 && S_ISDIR(test_d.st_mode)))
        {
            ...
            if (_pam_open_config_file(pamh, pamh->service_name, &path, &f) ==
                PAM_SUCCESS)
            {
                ...
                {
                    pam_syslog(pamh, LOG_ERR,
                               "_pam_init_handlers: error reading %s", path);
            ...
            else
            {
                ...
                {
                    retval =
                        _pam_parse_conf_file(pamh, f, NULL, PAM_T_ANY, 0, 1);
                    fclose(f);
                }
                else
#endif /* PAM_READ_BOTH_CONFS */
            ...
            if (retval == PAM_SUCCESS)
            {
                ...
                if (_pam_open_config_file(pamh, PAM_DEFAULT_SERVICE, &path,
                                          &f) == PAM_SUCCESS)
                {
        ...
        else
        {
            ...
            {
                pam_syslog(pamh, LOG_ERR,
                           "_pam_init_handlers: could not open " PAM_CONFIG);
                return PAM_ABORT;
            }
...
static char* extract_modulename(const char* mod_path)
{
    ...
    _pam_load_module(pam_handle_t* pamh, const char* mod_path, int handler_type)
{
    ...
    if (x == pamh->handlers.modules_used)
    {
        ...
        if (mod->dl_handle == NULL)
        {
            ...
            {
                mod_full_isa_path =
                    malloc(strlen(mod_path) + strlen(_PAM_ISA) + 1);
                ...
                else
                {
                    ...
                    if (isa)
                    {
                        memmove(isa + strlen(_PAM_ISA), isa + 4,
                                strlen(isa + 4) + 1);
...
int _pam_add_handler(pam_handle_t* pamh, int handler_type, int other,
                     int stack_level, int type, int* actions,
                     const char* mod_path, int argc, char** argv, int argvlen)
{
    ...
    D(("_pam_add_handler: adding type %d, handler_type %d, module `%s'", type,
       handler_type, mod_path));
        ...
        mod_path != NULL)
    {
        ...
        else if (asprintf(&mod_full_path, "%s%s", DEFAULT_MODULE_PATH,
                          mod_path) >= 0)
        {
    ...
    if (mod_type != PAM_MT_DYNAMIC_MOD && mod_type != PAM_MT_FAULTY_MOD)
    {
        ...
        pam_syslog(pamh, LOG_ERR,
                   "internal error: module library type not known: %s;%d", sym,
                   mod_type);
        return PAM_ABORT;
    }
