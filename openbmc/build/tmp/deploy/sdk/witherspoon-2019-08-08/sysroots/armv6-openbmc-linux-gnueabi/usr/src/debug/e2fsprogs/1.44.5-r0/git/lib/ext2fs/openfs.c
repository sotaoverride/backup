...
errcode_t ext2fs_open(const char* name, int flags, int superblock,
                      ...
                      ext2_filsys* ret_fs)
{
    return ext2fs_open2(name, 0, flags, superblock, block_size, manager,
                        ret_fs);
}
 ...
 *				filesystems)
 */
errcode_t ext2fs_open2(const char* name, const char* io_options, int flags,
                       ...
                       io_manager manager, ext2_filsys* ret_fs)
{
    ...
    if (retval)
        goto cleanup;
    if (io_options && (retval = io_channel_set_options(fs->io, io_options)))
        goto cleanup;
    fs->image_io = fs->io;
    fs->io->app_data = fs;
    ...
    {
        retval =
            ext2fs_get_mem(sizeof(struct ext2_image_hdr), &fs->image_header);
        if (retval)
            goto cleanup;
        retval = io_channel_read_blk(
            fs->io, 0, -(int)sizeof(struct ext2_image_hdr), fs->image_header);
        if (retval)
            goto cleanup;
        if (ext2fs_le32_to_cpu(fs->image_header->magic_number) !=
    ...
    retval =
        io_channel_read_blk(fs->io, superblock, -SUPERBLOCK_SIZE, fs->super);
    if (retval)
        goto cleanup;
    if (fs->orig_super)
     ...
     */
    if (ext2fs_has_feature_bigalloc(fs->super) && !(flags & EXT2_FLAG_64BITS))
    {
    ...
    if ((inode_size < EXT2_GOOD_OLD_INODE_SIZE) ||
        (inode_size > fs->blocksize) || (inode_size & (inode_size - 1)))
    {
        retval = EXT2_ET_CORRUPT_SUPERBLOCK;
        goto cleanup;
    }
    ...
    if (EXT2_BLOCKS_PER_GROUP(fs->super) != EXT2_CLUSTERS_PER_GROUP(fs->super)
                                                << fs->cluster_ratio_bits)
    {
        retval = EXT2_ET_CORRUPT_SUPERBLOCK;
        goto cleanup;
    ...
    retval = ext2fs_get_array(fs->desc_blocks, fs->blocksize, &fs->group_desc);
    if (retval)
        goto cleanup;
    if (!group_block)
    ...
    {
        retval = io_channel_read_blk(
            fs->io, group_block + group_zero_adjust + 1, first_meta_bg, dest);
        if (retval)
            goto cleanup;
#ifdef WORDS_BIGENDIAN
    ...
    for (i = first_meta_bg; i < fs->desc_blocks; i++)
    {
        ...
        {
            gdp =
                ext2fs_group_desc(fs, fs->group_desc, i * groups_per_block + j);
            ext2fs_swap_group_desc2(fs, gdp);
        }
#endif
        dest += fs->blocksize;
    ...
    {
        fs->block_sha_map = ext2fs_hashmap_create(
            ext2fs_djb2_hash, block_sha_map_free_entry, 4096);
        if (!fs->block_sha_map)
        {
...
errcode_t ext2fs_rewrite_to_io(ext2_filsys fs, io_channel new_io)
{
    ...
    if ((new_io == fs->image_io) || (new_io == fs->io))
        return 0;
    if ((fs->image_io != fs->io) && fs->image_io)
        io_channel_close(fs->image_io);
    if (fs->io)
        io_channel_close(fs->io);
    fs->io = fs->image_io = new_io;
    fs->flags |= EXT2_FLAG_DIRTY | EXT2_FLAG_RW | EXT2_FLAG_BB_DIRTY |
                 EXT2_FLAG_IB_DIRTY;
