...
static krb5_key_salt_tuple* copy_key_salt_tuple(krb5_key_salt_tuple* ksalt,
                                                krb5_int32 len)
{
 ...
 */
krb5_error_code krb5_aprof_init(char* fname, char* envname,
 ...
 */
krb5_error_code krb5_aprof_getvals(krb5_pointer acontext,
...
static krb5_error_code string_to_boolean(const char* string, krb5_boolean* out)
{
    static const char* const yes[] = {"y", "yes", "true", "t", "1", "on"};
    static const char* const no[] = {"n",   "no", "false", "f",
                                     "nil", "0",  "off"};
    unsigned int i;
...
krb5_error_code krb5_aprof_get_boolean(krb5_pointer acontext,
                                       const char** hierarchy, int uselast,
                                       krb5_boolean* retdata)
{
    krb5_error_code ret;
    char **values, *valp;
    int idx;
 ...
 */
krb5_error_code krb5_aprof_get_deltat(krb5_pointer acontext,
 ...
 *         error codes from profile_get_values()
 */
krb5_error_code krb5_aprof_get_string(krb5_pointer acontext,
                                      const char** hierarchy,
 ...
 */
krb5_error_code krb5_aprof_get_string_all(krb5_pointer acontext,
                                          const char** hierarchy,
                                          char** stringp)
{
    krb5_error_code ret;
 ...
 */
krb5_error_code krb5_aprof_get_int32(krb5_pointer acontext,
                                     const char** hierarchy,
 ...
 */
krb5_error_code krb5_aprof_finish(krb5_pointer acontext)
{
 ...
 */
static int get_string_param(char** param_out, char* param_in, long* mask_out,
 ...
 */
static void get_port_param(int* param_out, int param_in, long* mask_out,
                           long mask_in, long mask_bit, krb5_pointer aprofile,
                           const char** hierarchy, const char* config_name,
                           int default_value)
{
 ...
 */
static void get_deltat_param(krb5_deltat* param_out, krb5_deltat param_in,
                             long* mask_out, long mask_in, long mask_bit,
                             krb5_pointer aprofile, const char** hierarchy,
                             const char* config_name, krb5_deltat default_value)
{
    krb5_deltat dtvalue;
 ...
 */
static void parse_admin_server_port(char* server, int* port, long* mask)
{
    char *end, *portstr;
...
#define GET_STRING_PARAM(FIELD, BIT, CONFTAG, DEFAULT)                         \
    get_string_param(&params.FIELD, params_in->FIELD, &params.mask,            \
                     params_in->mask, BIT, aprofile, hierarchy, CONFTAG,       \
                     DEFAULT)
...
#define GET_PORT_PARAM(FIELD, BIT, CONFTAG, DEFAULT)                           \
    get_port_param(&params.FIELD, params_in->FIELD, &params.mask,              \
                   params_in->mask, BIT, aprofile, hierarchy, CONFTAG,         \
                   DEFAULT)
...
#define GET_DELTAT_PARAM(FIELD, BIT, CONFTAG, DEFAULT)                         \
    get_deltat_param(&params.FIELD, params_in->FIELD, &params.mask,            \
                     params_in->mask, BIT, aprofile, hierarchy, CONFTAG,       \
                     DEFAULT)
    ...
    else
    {
        ...
        krb5_string_to_keysalts(svalue, NULL, /* Tuple separators */
                                NULL,         /* Key/salt separators */
                                0,            /* No duplicates */
                                &params.keysalts, &params.num_keysalts);
...
krb5_error_code kadm5_get_admin_service_name(krb5_context ctx, char* realm_in,
