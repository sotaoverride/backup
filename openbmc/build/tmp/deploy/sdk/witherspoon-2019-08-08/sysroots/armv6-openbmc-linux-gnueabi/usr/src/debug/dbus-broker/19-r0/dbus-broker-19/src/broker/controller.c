...
static int controller_name_new(ControllerName** namep, Controller* controller,
                               const char* path)
{
    CRBNode **slot, *parent;
    ControllerName* name;
    ...
    slot = c_rbtree_find_slot(&controller->name_tree, controller_name_compare,
                              path, &parent);
    if (!slot)
...
int controller_name_reset(ControllerName* name)
{
    ...
    r = driver_name_activation_failed(&name->controller->broker->bus,
                                      &name->activation);
    if (r)
        return error_fold(r);
...
static int controller_reload_compare(CRBTree* t, void* k, CRBNode* rb)
{
    ControllerReload* reload =
        c_container_of(rb, ControllerReload, controller_node);
...
static int controller_reload_new(ControllerReload** reloadp, User* user,
                                 Controller* controller)
{
    ...
    for (uint32_t i = 0; i < UINT32_MAX; i++)
    {
        ...
        slot = c_rbtree_find_slot(&controller->reload_tree,
                                  controller_reload_compare, &serial, &parent);
        if (slot)
            break;
    }
    if (!slot)
    ...
    *reload = (ControllerReload)CONTROLLER_RELOAD_NULL(*reload);
    ...
    r = user_charge(controller->broker->bus.user, &reload->charge, user,
                    USER_SLOT_BYTES, sizeof(ControllerReload));
    if (r)
        return (r == USER_E_QUOTA) ? CONTROLLER_E_QUOTA : error_fold(r);

    c_rbtree_add(&controller->reload_tree, parent, slot,
                 &reload->controller_node);
    *reloadp = reload;
    return 0;
}
...
{
    return error_fold(driver_reload_config_completed(
        &reload->controller->broker->bus, reload->sender_id,
        reload->sender_serial));
}
...
{
    return error_fold(
        driver_reload_config_invalid(&reload->controller->broker->bus,
                                     reload->sender_id, reload->sender_serial));
}

static int controller_listener_compare(CRBTree* t, void* k, CRBNode* rb)
{
    ControllerListener* listener =
        c_container_of(rb, ControllerListener, controller_node);
...
static int controller_listener_new(ControllerListener** listenerp,
                                   Controller* controller, const char* path)
{
    ...
    slot = c_rbtree_find_slot(&controller->listener_tree,
                              controller_listener_compare, path, &parent);
    if (!slot)
    ...
    listener->controller_node =
        (CRBNode)C_RBNODE_INIT(listener->controller_node);
    memcpy(listener->path, path, n_path + 1);

    c_rbtree_add(&controller->listener_tree, parent, slot,
                 &listener->controller_node);
    *listenerp = listener;
    return 0;
}
 ...
 */
int controller_listener_set_policy(ControllerListener* listener,
...
{
    Controller* controller =
        c_container_of(file, Controller, connection.socket_file);
    int r;

    r = connection_dispatch(&controller->connection,
    ...
    if (r == CONTROLLER_E_EOF)
    {
        ...
        return connection_is_running(&controller->connection) ? 0
                                                              : DISPATCH_E_EXIT;
    }
...
int controller_init(Controller* c, Broker* broker, int controller_fd)
{
    ...
    *controller = (Controller)CONTROLLER_NULL(*controller);
    ...
    r = connection_init_server(&controller->connection, &broker->dispatcher,
                               controller_dispatch_connection, broker->bus.user,
                               "0123456789abcdef", controller_fd);
    if (r)
        return error_fold(r);
...
void controller_deinit(Controller* controller)
{
    ...
    c_rbtree_for_each_entry_safe_postorder_unlink(
        reload, reload_safe, &controller->reload_tree, controller_node)
        controller_reload_free(reload);

    c_rbtree_for_each_entry_safe_postorder_unlink(
        name, name_safe, &controller->name_tree, controller_node)
        controller_name_free(name);

    c_rbtree_for_each_entry_safe_postorder_unlink(
        listener, listener_safe, &controller->listener_tree, controller_node)
 ...
 */
int controller_add_name(Controller* controller, ControllerName** namep,
                        const char* path, const char* name_str, uid_t uid)
{
    ...
    r = name_registry_ref_name(&controller->broker->bus.names, &name_entry,
                               name_str);
    if (r)
        return error_fold(r);

    r = user_registry_ref_user(&controller->broker->bus.users, &user_entry,
                               uid);
    if (r)
        return error_fold(r);
    ...
    if (r)
        return (r == ACTIVATION_E_ALREADY_ACTIVATABLE)
...
int controller_add_listener(Controller* controller,
                            ControllerListener** listenerp, const char* path,
                            int listener_fd, PolicyRegistry* policy)
{
    ...
    r = listener_init_with_fd(&listener->listener, &controller->broker->bus,
                              &controller->broker->dispatcher, listener_fd,
                              policy);
    if (r)
        return error_fold(r);
 ...
 */
int controller_request_reload(Controller* controller, User* sender_user,
                              uint64_t sender_id, uint32_t sender_serial)
{
...
ControllerName* controller_find_name(Controller* controller, const char* path)
{
    return c_container_of(c_rbtree_find_node(&controller->name_tree,
                                             controller_name_compare, path),
                          ControllerName, controller_node);
}

/**
 * controller_find_listener() - XXX
 */
ControllerListener* controller_find_listener(Controller* controller,
                                             const char* path)
{
    return c_container_of(c_rbtree_find_node(&controller->listener_tree,
                                             controller_listener_compare, path),
                          ControllerListener, controller_node);
}

/**
 * controller_find_reload() - XXX
 */
ControllerReload* controller_find_reload(Controller* controller,
                                         uint32_t serial)
{
    return c_container_of(c_rbtree_find_node(&controller->reload_tree,
                                             controller_reload_compare,
                                             &serial),
                          ControllerReload, controller_node);
}
