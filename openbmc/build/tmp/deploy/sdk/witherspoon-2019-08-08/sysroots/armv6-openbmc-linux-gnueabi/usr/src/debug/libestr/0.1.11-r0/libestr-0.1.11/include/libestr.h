 ...
 * The string object.
 * @note
 * We do not use es_size_t, because that tends to be 64 bits on 64 bit
 * platforms. In almost all cases I can think of, 4GB is a sufficient upper
 * limit on string size. So we use unsigned ints, which means we save a lot of
 * space and efficieny, what is especially important if there is a large number
 * of strings inside a process. For the same reason, we do \b not provide a way
 * to create and automatically free a traditional C string. That would requre
 * another pointer (8 bytes of overhead on a 64 bit machine!).
 */
typedef struct
{
    /* word-aligned items */
 ...
 * @returns address of buffer <b>Note: this is NOT a zero-terminated C
 * string!</b>
 */
 ...
 * @param[in] lenhint expected max length of string. Do \b not use too large
 * value.
 ...
 */
static inline void es_emptyStr(es_str_t* str)
{
    str->lenStr = 0;
}
 ...
 * @returns pointer to new object or NULL on error
 */
static inline es_str_t* es_strdup(es_str_t* str)
{
 ...
 */
static inline int es_strcmp(es_str_t* s1, es_str_t* s2)
{
 ...
 * @returns 0 on success, something else otherwise
 */
static inline int es_addStr(es_str_t** ps1, es_str_t* s2)
{
 ...
 *
 * @param[in] s string object
 * @param[in] nulEsc escape sequence for NULs. If NULL, NUL characters will be
 * dropped.
 *
 * @returns NULL in case of error, otherwise a suitably-encoded standard C
 * string. This string is allocated from the dynamic memory pool and must be
 * freed by the caller.
 */
