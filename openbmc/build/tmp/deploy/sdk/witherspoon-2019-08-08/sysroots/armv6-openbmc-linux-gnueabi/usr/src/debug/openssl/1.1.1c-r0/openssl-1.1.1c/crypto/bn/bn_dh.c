...
#define make_dh_bn(x)                                                          \
    extern const BIGNUM _bignum_##x;                                           \
    const BIGNUM _bignum_##x = {(BN_ULONG*)x, OSSL_NELEM(x), OSSL_NELEM(x), 0, \
                                BN_FLG_STATIC_DATA};

static const BN_ULONG value_2 = 2;

const BIGNUM _bignum_const_2 = {(BN_ULONG*)&value_2, 1, 1, 0,
                                BN_FLG_STATIC_DATA};

make_dh_bn(dh1024_160_p) make_dh_bn(dh1024_160_g) make_dh_bn(dh1024_160_q)
    make_dh_bn(dh2048_224_p) make_dh_bn(dh2048_224_g) make_dh_bn(dh2048_224_q)
        make_dh_bn(dh2048_256_p) make_dh_bn(dh2048_256_g)
            make_dh_bn(dh2048_256_q)

                make_dh_bn(ffdhe2048_p) make_dh_bn(ffdhe3072_p)
                    make_dh_bn(ffdhe4096_p) make_dh_bn(ffdhe6144_p)
