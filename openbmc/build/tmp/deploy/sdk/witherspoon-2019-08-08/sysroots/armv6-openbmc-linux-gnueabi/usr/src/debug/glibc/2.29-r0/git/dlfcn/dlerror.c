...
char* __dlerror(void)
{
    ...
    else if (result->errstring != NULL)
    {
        ...
        int n;
        if (result->errcode == 0)
            n = __asprintf(&buf, "%s%s%s", result->objname,
                           result->objname[0] == '\0' ? "" : ": ",
                           _(result->errstring));
        else
            n = __asprintf(&buf, "%s%s%s: %s", result->objname,
                           result->objname[0] == '\0' ? "" : ": ",
                           _(result->errstring), strerror(result->errcode));
        if (n != -1)
        {
...
{
    if (rec->errstring != NULL && strcmp(rec->errstring, "out of memory") != 0)
    {
...
void __dlerror_main_freeres(void)
{
    void* mem;
...
static struct dlfcn_hook _dlfcn_hooks = {.dlopen = __dlopen,
                                         .dlclose = __dlclose,
                                         .dlsym = __dlsym,
                                         .dlvsym = __dlvsym,
                                         .dlerror = __dlerror,
