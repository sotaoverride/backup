...
typedef int64
    number_t; /* type to use for numbers - TODO: maybe an autoconf option? */
...
/* this is a trick to store a function ptr to a function returning a function
 * ptr... */
...
typedef int rs_size_t; /* we do never need more than 2Gig strings, signed
                        * permits to use -1 as a special flag. */
typedef rsRetVal (*prsf_t)(struct vmstk_s*,
                           int); /* pointer to a RainerScript function */
typedef uint64
    qDeqID; /* queue Dequeue order ID. 32 bits is considered dangerously few */
...
typedef signed char sbool; /* (small bool) I intentionally use char, to keep it
               slim so that many fit into the CPU cache! */
...
typedef enum
{
    ...
    eFLOWCTL_LIGHT_DELAY =
        1, /**< some light delay possible, but no extended period of time */
...
/* properties are now encoded as (tiny) integers. I do not use an enum as I
 * would like to keep the memory footprint small (and thus cache hits high).
