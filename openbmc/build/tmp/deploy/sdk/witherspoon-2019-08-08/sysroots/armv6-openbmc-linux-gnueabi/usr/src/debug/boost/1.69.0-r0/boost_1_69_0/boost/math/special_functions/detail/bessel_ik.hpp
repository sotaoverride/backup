...
int temme_ik(T v, T x, T* K, T* K1, const Policy& pol)
{
    ...
    c = abs(v) < tools::epsilon<T>()
            ? T(1)
            : T(boost::math::sin_pi(v) / (v * pi<T>()));
    d = abs(sigma) < tools::epsilon<T>() ? T(1) : T(sinh(sigma) / sigma);
    gamma1 = abs(v) < tools::epsilon<T>() ? T(-euler<T>())
                                          : T((0.5f / v) * (gp - gm) * c);
    gamma2 = (2 + gp + gm) * c / 2;
    ...
    policies::check_series_iterations<T>(
        "boost::math::bessel_ik<%1%>(%1%,%1%) in temme_ik", k, pol);
...
int CF1_ik(T v, T x, T* fv, const Policy& pol)
{
    ...
    policies::check_series_iterations<T>(
        "boost::math::bessel_ik<%1%>(%1%,%1%) in CF1_ik", k, pol);
...
int CF2_ik(T v, T x, T* Kv, T* Kv1, const Policy& pol)
{
    ...
    for (k = 2; k < policies::get_max_series_iterations<Policy>();
         k++) // starting from 2
    {
    ...
    policies::check_series_iterations<T>(
        "boost::math::bessel_ik<%1%>(%1%,%1%) in CF2_ik", k, pol);
...
int bessel_ik(T v, T x, T* I, T* K, int kind, const Policy& pol)
{
    ...
    if (x < 0)
    {
        *I = *K = policies::raise_domain_error<T>(
            function,
            "Got x = %1% but real argument x must be non-negative, complex "
            "number result not supported.",
            x, pol);
        return 1;
    }
    if (x == 0)
        ...
        if (reflect && (kind & need_i))
        {
            ...
            Iv = boost::math::sin_pi(z, pol) == 0
                     ...
                     : policies::raise_overflow_error<T>(
                           function, 0, pol); // reflection formula
        }
