...
#define HAVE_LOCK_CHECK(connection)                                            \
    _dbus_assert((connection)->have_connection_lock)
/* A "DO_NOT_HAVE_LOCK_CHECK" is impossible since we need the lock to check the
 * flag */
#endif
...
#define DISPATCH_STATUS_NAME(s)                                                \
    ((s) == DBUS_DISPATCH_COMPLETE                                             \
         ? "complete"                                                          \
         : (s) == DBUS_DISPATCH_DATA_REMAINS                                   \
               ? "data remains"                                                \
 ...
 * <ul>
 * <li><b>read</b> means to fill the incoming message queue by reading from the
 * socket</li> <li><b>write</b> means to drain the outgoing queue by writing to
 * the socket</li> <li><b>dispatch</b> means to drain the incoming queue by
 * invoking application-provided message handlers</li>
 * </ul>
 *
...
static void _dbus_connection_trace_ref(DBusConnection* connection,
                                       int old_refcount, int new_refcount,
                                       const char* why)
{
...
struct DBusMessageFilter
{
    ...
    DBusFreeFunction
        free_user_data_function; /**< Function to free the user data */
};
...
struct DBusConnection
{
    ...
    DBusCondVar*
        dispatch_cond; /**< Notify when dispatch_acquired is available */
    DBusCMutex* io_path_mutex; /**< Protects io_path_acquired */
    DBusCondVar* io_path_cond; /**< Notify when io_path_acquired is available */

    DBusList* outgoing_messages; /**< Queue of messages we need to send, send
                                    the end of the list first. */
    DBusList* incoming_messages; /**< Queue of messages we have received, end of
                                    the list received most recently. */
    DBusList* expired_messages; /**< Messages that will be released when we next
                                   unlock. */

    DBusMessage* message_borrowed; /**< Filled in if the first incoming message
                                    * has been borrowed; dispatch_acquired will
                                    * be set by the borrower
                                    */
    ...
    DBusTransport*
        transport; /**< Object that sends/receives messages over network. */
    DBusWatchList* watches;    /**< Stores active watches. */
    DBusTimeoutList* timeouts; /**< Stores active timeouts. */

    DBusList* filter_list; /**< List of filters. */

    DBusRMutex* slot_mutex; /**< Lock on slot_list so overall connection lock
                               need not be taken */
    DBusDataSlotList slot_list; /**< Data stored by allocated integer ID */

    DBusHashTable*
        pending_replies; /**< Hash of message serials to #DBusPendingCall. */

    dbus_uint32_t client_serial; /**< Client serial. Increments each time a
                                    message is sent  */
    DBusList* disconnect_message_link; /**< Preallocated list node for queueing
    ...
    DBusDispatchStatusFunction
        dispatch_status_function; /**< Function on dispatch status changes  */
    void* dispatch_status_data;   /**< Application data for
                                     dispatch_status_function */
    DBusFreeFunction
        free_dispatch_status_data; /**< free dispatch_status_data */

    DBusDispatchStatus last_dispatch_status; /**< The last dispatch status we
    ...
    char*
        server_guid; /**< GUID of server if we are in shared_connections, #NULL
                        if server GUID is unknown or connection is private */

    /* These two MUST be bools and not bitfields, because they are protected by
     * a separate lock from connection->mutex and all bitfields in a word have
     * to be read/written together. So you can't have a different lock for
     * different bitfields in the same word.
     */
    dbus_bool_t dispatch_acquired; /**< Someone has dispatch path (can drain
                                      incoming queue) */
    dbus_bool_t io_path_acquired;  /**< Someone has transport io path (can use
                                      the transport to read/write messages) */

    unsigned int shareable : 1; /**< #TRUE if libdbus owns a reference to the
                                   connection and can return it from
    ...
    unsigned int
        route_peer_messages : 1; /**< If #TRUE, if org.freedesktop.DBus.Peer
                                    messages have a bus name, don't handle them
                                    automatically */

    unsigned int
        disconnected_message_arrived : 1; /**< We popped or are dispatching the
                                           * disconnected message. if the
                                           * disconnect_message_link is NULL
                                           * then we queued it, but this flag is
                                           * whether it got to the head of the
                                           * queue.
                                           */
    unsigned int
        disconnected_message_processed : 1; /**< We did our default handling of
                                             * the disconnected message, such as
    ...
    int generation; /**< _dbus_current_generation that should correspond to this
                       connection */
#endif
};
 ...
 *        variable pointer
 */
void _dbus_connection_test_get_locks(DBusConnection* connection,
                                     DBusMutex** mutex_loc,
                                                  ...
                                                  DBusList* link)
{
    ...
    _dbus_verbose(
        "Message %p (%s %s %s %s '%s' reply to %u) added to incoming queue %p, "
        "%d incoming\n",
        ...
        dbus_message_get_reply_serial(message), connection,
                                            ...
                                            DBusMessage* message)
{
    ...
    _dbus_verbose(
        "Message %p (%s %s %s %s '%s') removed from outgoing queue %p, %d left "
        "to send\n",
        message, dbus_message_type_to_string(dbus_message_get_type(message)),
        dbus_message_get_path(message) ? dbus_message_get_path(message)
                                       : "no path",
        ...
        dbus_message_get_member(message) ? dbus_message_get_member(message)
                                         : "no member",
        dbus_message_get_signature(message), connection,
...
/** Function to be called in protected_change_watch() with refcount held */
typedef void (*DBusWatchToggleFunction)(DBusWatchList* list, DBusWatch* watch,
                                        dbus_bool_t enabled);

static dbus_bool_t protected_change_watch(
    DBusConnection* connection, DBusWatch* watch,
    DBusWatchAddFunction add_function, DBusWatchRemoveFunction remove_function,
...
dbus_bool_t _dbus_connection_add_watch_unlocked(DBusConnection* connection,
                                                DBusWatch* watch)
{
    return protected_change_watch(connection, watch, _dbus_watch_list_add_watch,
                                  NULL, NULL, FALSE);
}
...
void _dbus_connection_remove_watch_unlocked(DBusConnection* connection,
                                            DBusWatch* watch)
{
    protected_change_watch(connection, watch, NULL,
                           _dbus_watch_list_remove_watch, NULL, FALSE);
}
                                            ...
                                            dbus_bool_t enabled)
{
    ...
    protected_change_watch(connection, watch, NULL, NULL,
                           _dbus_watch_list_toggle_watch, enabled);
}
...
static dbus_bool_t
    protected_change_timeout(DBusConnection* connection, DBusTimeout* timeout,
...
dbus_bool_t _dbus_connection_add_timeout_unlocked(DBusConnection* connection,
                                                  DBusTimeout* timeout)
{
    return protected_change_timeout(
        connection, timeout, _dbus_timeout_list_add_timeout, NULL, NULL, FALSE);
}
...
void _dbus_connection_remove_timeout_unlocked(DBusConnection* connection,
                                              DBusTimeout* timeout)
{
    protected_change_timeout(connection, timeout, NULL,
                             _dbus_timeout_list_remove_timeout, NULL, FALSE);
}
...
void _dbus_connection_toggle_timeout_unlocked(DBusConnection* connection,
                                              ...
                                              dbus_bool_t enabled)
{
    protected_change_timeout(connection, timeout, NULL, NULL,
                             _dbus_timeout_list_toggle_timeout, enabled);
}
...
static void free_pending_call_on_hash_removal(void* data)
{
    ...
    {
        _dbus_connection_remove_timeout_unlocked(
            connection, _dbus_pending_call_get_timeout_unlocked(pending));
...
static void
    _dbus_connection_detach_pending_call_unlocked(DBusConnection* connection,
                                                  DBusPendingCall* pending)
{
    /* This ends up unlocking to call the pending call finalizer, which is
 ...
 */
static dbus_bool_t _dbus_connection_acquire_io_path(DBusConnection* connection,
                                            ...
                                            int timeout_milliseconds)
{
    ...
    if (_dbus_connection_acquire_io_path(
            connection,
            (flags & DBUS_ITERATION_BLOCK) ? timeout_milliseconds : 0))
    {
        ...
        if ((pending != NULL) &&
            _dbus_pending_call_get_completed_unlocked(pending))
        {
            _dbus_verbose("pending call completed while acquiring I/O path");
        }
        else if ((pending != NULL) &&
                 _dbus_connection_peek_for_reply_unlocked(
                     connection,
                     _dbus_pending_call_get_reply_serial_unlocked(pending)))
        {
            _dbus_verbose("pending call completed while acquiring I/O path "
                          "(reply found in queue)");
        }
        else
        {
            _dbus_transport_do_iteration(connection->transport, flags,
                                         timeout_milliseconds);
        }
...
DBusConnection* _dbus_connection_new_for_transport(DBusTransport* transport)
{
    ...
    pending_replies = _dbus_hash_table_new(
        DBUS_HASH_INT, NULL,
    ...
    disconnect_message = dbus_message_new_signal(
    ...
    connection->last_dispatch_status =
        DBUS_DISPATCH_COMPLETE; /* so we're notified first time there's data */
 ...
 *
 * @todo This is basically a hack - we could delete
 * _dbus_transport_handle_watch() and the virtual handle_watch in DBusTransport
 * if we got rid of it. The reason this is some work is threading, see the
 * _dbus_connection_handle_watch() implementation.
 *
 ...
 * @returns #FALSE if the IO condition may not have been fully handled due to
 * lack of memory
 */
        ...
        while (connection != NULL)
        {
            ...
            if (!_DBUS_LOCK(shared_connections))
                _dbus_assert_not_reached(
                    "global locks were already initialized");
            connection = _dbus_list_pop_first(&shared_connections_no_guid);
        }
    }
...
static dbus_bool_t connection_lookup_shared(DBusAddressEntry* entry,
                                            DBusConnection** result)
{
    ...
    *result = NULL;
    ...
    if (shared_connections == NULL)
    {
        ...
        shared_connections =
            _dbus_hash_table_new(DBUS_HASH_STRING, dbus_free, NULL);
        if (shared_connections == NULL)
        {
            _DBUS_UNLOCK(shared_connections);
            return FALSE;
    ...
    else
    {
        ...
        if (guid != NULL)
        {
            ...
            connection =
                _dbus_hash_table_lookup_string(shared_connections, guid);
            ...
            if (connection)
            {
                 ...
                 * drop our shared ref to the connection on connection
                 * disconnect, and then assert here that the connection is
                 * connected, but that causes reentrancy headaches.
                 */
                CONNECTION_LOCK(connection);
                ...
                {
                    _dbus_verbose(
                        "looked up existing connection to server guid %s but "
                        "it was disconnected so ignoring it\n",
                        guid);
                }
                CONNECTION_UNLOCK(connection);
            }
...
static dbus_bool_t connection_record_shared_unlocked(DBusConnection* connection,
                                                     const char* guid)
{
    char* guid_key;
    ...
    if (!_dbus_hash_table_insert_string(shared_connections, guid_key,
                                        connection))
    {
        dbus_free(guid_key);
        dbus_free(guid_in_connection);
...
static DBusConnection*
    connection_try_from_address_entry(DBusAddressEntry* entry, DBusError* error)
{
 ...
 */
static DBusConnection* _dbus_connection_open_internal(const char* address,
                                                      dbus_bool_t shared,
                                                      DBusError* error)
{
    DBusConnection* connection;
    ...
    _dbus_connection_preallocate_send_unlocked(DBusConnection* connection)
{
    ...
    preallocated->counter_link =
        _dbus_list_alloc_link(connection->outgoing_counter);
...
static void _dbus_connection_send_preallocated_unlocked_no_update(
    ...
    DBusMessage* message, dbus_uint32_t* client_serial)
{
    dbus_uint32_t serial;
    ...
    _dbus_verbose(
        "Message %p (%s %s %s %s '%s') for %s added to outgoing queue %p, %d "
        "pending to send\n",
        message, dbus_message_type_to_string(dbus_message_get_type(message)),
        dbus_message_get_path(message) ? dbus_message_get_path(message)
                                       : "no path",
     ...
     */
    _dbus_connection_do_iteration_unlocked(connection, NULL,
                                           DBUS_ITERATION_DO_WRITING, -1);
    ...
    DBusMessage* message, dbus_uint32_t* client_serial)
{
    ...
    _dbus_connection_send_preallocated_unlocked_no_update(
        connection, preallocated, message, client_serial);
                                             ...
                                             dbus_uint32_t* client_serial)
{
    ...
    _dbus_connection_send_preallocated_and_unlock(connection, preallocated,
                                                  message, client_serial);
    return TRUE;
}

/**
 * Used internally to handle the semantics of
 * dbus_server_set_new_connection_function(). If the new connection function
 * does not ref the connection, we want to close it.
 *
 * A bit of a hack, probably the new connection function should have returned a
 * value for whether to close, or should have had to close the connection itself
 * if it didn't want it.
 *
...
static DBusMessage* generate_local_error_message(dbus_uint32_t serial,
 ...
 * the dispatch lock.
 */
static DBusMessage* check_for_reply_unlocked(DBusConnection* connection,
                                             dbus_uint32_t client_serial)
{
    DBusList* link;
...
static void connection_timeout_and_complete_all_pending_calls_unlocked(
    DBusConnection* connection)
{
...
static void complete_pending_call_and_unlock(DBusConnection* connection,
                                             DBusPendingCall* pending,
                                             DBusMessage* message)
{
    _dbus_pending_call_set_reply_unlocked(pending, message);
    _dbus_pending_call_ref_unlocked(
        pending); /* in case there's no app with a ref held */
                                                 ...
                                                 DBusPendingCall* pending)
{
    ...
    reply = check_for_reply_unlocked(
...
void _dbus_connection_block_pending_call(DBusPendingCall* pending)
{
    ...
    dbus_pending_call_ref(
        pending); /* necessary because the call could be canceled */
    ...
    if (timeout)
    {
        ...
        _dbus_verbose(
            "dbus_connection_send_with_reply_and_block(): will block %d "
            "milliseconds for reply serial %u from %ld sec %ld usec\n",
            timeout_milliseconds, client_serial, start_tv_sec, start_tv_usec);
    }
    else
    {
        timeout_milliseconds = -1;

        _dbus_verbose("dbus_connection_send_with_reply_and_block(): will block "
                      "for reply serial %u\n",
                      client_serial);
    }
    ...
    _dbus_connection_do_iteration_unlocked(
        connection, pending, DBUS_ITERATION_DO_READING | DBUS_ITERATION_BLOCK,
        timeout_milliseconds);
...
recheck_status:
    ...
    if (!_dbus_connection_get_is_connected_unlocked(connection))
    {
        ...
        error_msg = generate_local_error_message(
            client_serial, DBUS_ERROR_DISCONNECTED,
    ...
    else if (timeout == NULL)
    {
        ...
        {
            /* Try sleeping a bit, as we aren't sure we need to block for
             * reading, we may already have a reply in the buffer and just can't
             * process it.
             */
        ...
        _dbus_verbose("dbus_connection_send_with_reply_and_block(): clock set "
                      "backward\n");
    else if (elapsed_milliseconds < timeout_milliseconds)
    {
        _dbus_verbose("dbus_connection_send_with_reply_and_block(): %d "
                      "milliseconds remain\n",
                      timeout_milliseconds - elapsed_milliseconds);
    ...
    _dbus_verbose("dbus_connection_send_with_reply_and_block(): Waited %d "
                  "milliseconds and got no reply\n",
                  elapsed_milliseconds);
...
void _dbus_connection_set_pending_fds_function(
    DBusConnection* connection, DBusPendingFdsChangeFunction callback,
    void* data)
{
    _dbus_transport_set_pending_fds_function(connection->transport, callback,
                                             data);
}
...
DBusConnection* dbus_connection_ref(DBusConnection* connection)
{
    ...
    _dbus_return_val_if_fail(connection->generation == _dbus_current_generation,
                             NULL);
    old_refcount = _dbus_atomic_inc(&connection->refcount);
    _dbus_connection_trace_ref(connection, old_refcount, old_refcount + 1,
                               "ref");
...
static void _dbus_connection_last_unref(DBusConnection* connection)
{
    ...
    /* ---- We're going to call various application callbacks here, hope it
    ...
    while (link != NULL)
    {
        ...
        DBusList* next =
            _dbus_list_get_next_link(&connection->filter_list, link);
    ...
    _dbus_list_foreach(&connection->outgoing_messages, free_outgoing_message,
                       connection);
    _dbus_list_clear(&connection->outgoing_messages);

    _dbus_list_foreach(&connection->incoming_messages,
                       (DBusForeachFunction)dbus_message_unref, NULL);
 ...
 * Private connections come from dbus_connection_open_private() or
    ...
    if (old_refcount == 1)
    {
        ...
        if (_dbus_transport_get_is_connected(connection->transport))
        {
            _dbus_warn_check_failed(
                "The last reference on a connection was dropped without "
                ...
                connection->shareable
                    ? "Most likely, the application called unref() too many "
                      "times and removed a reference belonging to libdbus, "
                      "since this is a shared connection."
                    : "Most likely, the application was supposed to call "
                      "dbus_connection_close(), since this is a private "
                      "connection.");
            return;
        }
#endif
    ...
    DBusConnection* connection)
{
     ...
     * After the disconnect message link is queued,
     * dbus_bus_get/dbus_connection_open should never again return the
     * newly-disconnected connection.
     *
...
void dbus_connection_close(DBusConnection* connection)
{
    ...
    if (connection->shareable)
    {
        ...
        _dbus_warn_check_failed(
            "Applications must not close shared connections - see "
 ...
 *
 * @param connection the connection
 * @param exit_on_disconnect #TRUE if _exit() should be called after a
 * disconnect signal
 */
 ...
 * @param message the message to send
 * @param client_serial return location for client serial assigned to the
 * message
 */
                                       ...
                                       dbus_uint32_t* client_serial)
{
    ...
    _dbus_return_if_fail(dbus_message_get_type(message) !=
                                            ...
                                            int timeout_milliseconds)
{
    ...
    _dbus_return_val_if_fail(
        timeout_milliseconds >= 0 || timeout_milliseconds == -1, FALSE);
    ...
    pending = _dbus_pending_call_new_unlocked(connection, timeout_milliseconds,
    ...
    if (!_dbus_pending_call_set_timeout_error_unlocked(pending, message,
                                                       serial))
        goto error;
     ...
     */
    if (!_dbus_connection_attach_pending_call_unlocked(connection, pending))
        goto error;
    ...
    DBusError* error)
{
    ...
    _dbus_return_val_if_fail(
        timeout_milliseconds >= 0 || timeout_milliseconds == -1, NULL);
    _dbus_return_val_if_error_is_set(error, NULL);
...
static dbus_bool_t _dbus_connection_read_write_dispatch(
    DBusConnection* connection, int timeout_milliseconds, dbus_bool_t dispatch)
{
    ...
    else
    {
        ...
        if (_dbus_connection_get_is_connected_unlocked(connection))
        {
            ...
            _dbus_connection_do_iteration_unlocked(
                connection, NULL,
                DBUS_ITERATION_DO_READING | DBUS_ITERATION_DO_WRITING |
                    DBUS_ITERATION_BLOCK,
                timeout_milliseconds);
        }
    }
                                                ...
                                                int timeout_milliseconds)
{
    ...
    _dbus_return_val_if_fail(
        timeout_milliseconds >= 0 || timeout_milliseconds == -1, FALSE);
    return _dbus_connection_read_write_dispatch(connection,
                                                ...
                                                DBusMessage* head_of_queue)
{
    ...
    /* checking that the link is NULL is an optimization to avoid the is_signal
     * call */
    if (connection->disconnect_message_link == NULL &&
        dbus_message_is_signal(head_of_queue, DBUS_INTERFACE_LOCAL,
                               "Disconnected"))
    {
...
DBusMessage* dbus_connection_borrow_message(DBusConnection* connection)
{
    ...
    connection->message_borrowed =
        _dbus_list_get_first(&connection->incoming_messages);
    ...
    _dbus_connection_pop_message_link_unlocked(DBusConnection* connection)
{
    ...
    if (connection->n_incoming > 0)
    {
        ...
        _dbus_verbose(
            "Message %p (%s %s %s %s sig:'%s' serial:%u) removed from incoming "
            "queue %p, %d incoming\n",
            link->data,
            ...
            dbus_message_get_serial(link->data), connection,
            connection->n_incoming);
                                                   ...
                                                   DBusList* message_link)
{
    ...
    _dbus_verbose(
        ...
        dbus_message_get_member(message_link->data)
            ? dbus_message_get_member(message_link->data)
            : "no member",
        dbus_message_get_signature(message_link->data), connection,
        connection->n_incoming);
...
static void _dbus_connection_failed_pop(DBusConnection* connection,
                                        DBusList* message_link)
{
    _dbus_list_prepend_link(&connection->incoming_messages, message_link);
    connection->n_incoming += 1;
...
/* Note this may be called multiple times since we don't track whether we
 * already did it */
...
/* Note this may be called multiple times since we don't track whether we
 * already did it */
static DBusDispatchStatus notify_disconnected_and_dispatch_complete_unlocked(
    DBusConnection* connection)
{
    HAVE_LOCK_CHECK(connection);
    ...
    if (connection->disconnect_message_link != NULL)
    {
        ...
        /* If we have pending calls, queue their timeouts - we want the
         * Disconnected to be the last message, after these timeouts.
         */
         ...
         */
        _dbus_connection_queue_synthesized_message_link(
            connection, connection->disconnect_message_link);
        connection->disconnect_message_link = NULL;
    ...
    _dbus_connection_get_dispatch_status_unlocked(DBusConnection* connection)
{
    ...
    else
    {
        ...
        if (!is_connected)
        {
             ...
             * status check. However, probably not good to change until it
             * causes a problem.
             */
                                                    ...
                                                    DBusMessage* message)
{
    ...
    if (connection->route_peer_messages &&
    ...
    else if (dbus_message_is_method_call(message, DBUS_INTERFACE_PEER,
                                         "GetMachineId"))
    {
        DBusString uuid;
        ...
        if (_dbus_get_local_machine_uuid_encoded(&uuid, &error))
        {
            ...
            if (dbus_message_append_args(ret, DBUS_TYPE_STRING, &v_STRING,
                                         DBUS_TYPE_INVALID))
            {
        ...
        else
        {
            ...
            sent =
                _dbus_connection_send_unlocked_no_update(connection, ret, NULL);
        }
 ...
 **/
static DBusHandlerResult
    _dbus_connection_run_builtin_filters_unlocked_no_update(
...
DBusDispatchStatus dbus_connection_dispatch(DBusConnection* connection)
{
    ...
    if (message_link == NULL)
    {
        ...
        _dbus_verbose("another thread dispatched message (during "
                      "acquire_dispatch above)\n");
    ...
    result = _dbus_connection_run_builtin_filters_unlocked_no_update(connection,
                                                                     message);
    ...
    _dbus_list_foreach(&filter_list_copy,
                       (DBusForeachFunction)_dbus_message_filter_ref, NULL);
    ...
    _dbus_list_foreach(&filter_list_copy,
                       (DBusForeachFunction)_dbus_message_filter_unref, NULL);
    ...
    HAVE_LOCK_CHECK(connection);
    result = _dbus_object_tree_dispatch_and_unlock(connection->objects, message,
                                                   &found_object);
    ...
    if (dbus_message_get_type(message) == DBUS_MESSAGE_TYPE_METHOD_CALL)
    {
        ...
        if (!_dbus_string_append_printf(&str,
                                        "Method \"%s\" with signature \"%s\" "
                                        "on interface \"%s\" doesn't exist\n",
                                        dbus_message_get_member(message),
                                        dbus_message_get_signature(message),
                                        dbus_message_get_interface(message)))
        {
        ...
        reply = dbus_message_new_error(message,
                                       found_object ? DBUS_ERROR_UNKNOWN_METHOD
        ...
        _dbus_connection_send_preallocated_unlocked_no_update(
            connection, preallocated, reply, NULL);
 ...
 * See the comments in the code or
 * http://lists.freedesktop.org/archives/dbus/2007-July/tread.html#8144 if you
    ...
    DBusFreeFunction free_data_function)
{
    ...
    retval = _dbus_watch_list_set_functions(connection->watches, add_function,
                                            remove_function, toggled_function,
    ...
    DBusFreeFunction free_data_function)
{
    ...
    retval = _dbus_timeout_list_set_functions(
        connection->timeouts, add_function, remove_function, toggled_function,
        data, free_data_function);
 ...
 * of the connection, if any. DO NOT read or write to the file descriptor, or
 * try to select() on it; use DBusWatch for main loop integration. Not all
                                                ...
                                                unsigned long* pid)
{
    ...
    if (!_dbus_transport_try_to_authenticate(connection->transport))
        ...
        result =
            _dbus_transport_get_unix_process_id(connection->transport, pid);
...
dbus_bool_t dbus_connection_get_adt_audit_session_data(
    DBusConnection* connection, void** data, dbus_int32_t* data_size)
{
    dbus_bool_t result;
    ...
    if (!_dbus_transport_try_to_authenticate(connection->transport))
        ...
        result = _dbus_transport_get_adt_audit_session_data(
            connection->transport, data, data_size);
    CONNECTION_UNLOCK(connection);
                                            ...
                                            DBusFreeFunction free_data_function)
{
    ...
    CONNECTION_LOCK(connection);
    _dbus_transport_set_unix_user_function(connection->transport, function,
                                           data, free_data_function, &old_data,
                                           &old_free_function);
    CONNECTION_UNLOCK(connection);
 ...
 *
 * @param connection the connection
 * @param windows_sid_p return location for an allocated copy of the user ID, or
 * #NULL if no memory
 * @returns #TRUE if user is available (returned value may be #NULL anyway if no
 * memory)
 */
    ...
    void* data, DBusFreeFunction free_data_function)
{
    ...
    CONNECTION_LOCK(connection);
    _dbus_transport_set_windows_user_function(connection->transport, function,
                                              data, free_data_function,
 ...
 *
 * @param connection the connection
 * @param value #TRUE to pass through org.freedesktop.DBus.Peer messages with a
 * bus name set
 */
                                   ...
                                   void* user_data)
{
    ...
    if (filter == NULL)
    {
        _dbus_warn_check_failed("Attempt to remove filter function %p user "
                                "data %p, but no such filter has been added",
                                function, user_data);
        return;
    }
#endif
...
static dbus_bool_t _dbus_connection_register_object_path(
    DBusConnection* connection, dbus_bool_t fallback, const char* path,
    const DBusObjectPathVTable* vtable, void* user_data, DBusError* error)
{
    ...
    retval = _dbus_object_tree_register(connection->objects, fallback,
    ...
    const DBusObjectPathVTable* vtable, void* user_data, DBusError* error)
{
    ...
    return _dbus_connection_register_object_path(connection, FALSE, path,
                                                 vtable, user_data, error);
}
    ...
    const DBusObjectPathVTable* vtable, void* user_data)
{
    ...
    retval = _dbus_connection_register_object_path(connection, FALSE, path,
                                                   vtable, user_data, &error);
    ...
    const DBusObjectPathVTable* vtable, void* user_data, DBusError* error)
{
    ...
    return _dbus_connection_register_object_path(connection, TRUE, path, vtable,
                                                 user_data, error);
}
    ...
    const DBusObjectPathVTable* vtable, void* user_data)
{
    ...
    retval = _dbus_connection_register_object_path(connection, TRUE, path,
                                                   vtable, user_data, &error);
                                                   ...
                                                   const char* path)
{
    ...
    _dbus_object_tree_unregister_and_unlock(connection->objects,
                                                 ...
                                                 void** data_p)
{
    ...
    *data_p = _dbus_object_tree_get_user_data_unlocked(
        connection->objects, (const char**)decomposed_path);
                                            ...
                                            char*** child_entries)
{
    ...
    if (!_dbus_decompose_path(parent_path, strlen(parent_path),
                              &decomposed_path, NULL))
        return FALSE;

    CONNECTION_LOCK(connection);

    retval = _dbus_object_tree_list_registered_and_unlock(
        connection->objects, (const char**)decomposed_path, child_entries);
    dbus_free_string_array(decomposed_path);
                                     ...
                                     DBusFreeFunction free_data_func)
{
    ...
    retval = _dbus_data_slot_list_set(&slot_allocator, &connection->slot_list,
...
void* dbus_connection_get_data(DBusConnection* connection, dbus_int32_t slot)
{
    ...
    res =
        _dbus_data_slot_list_get(&slot_allocator, &connection->slot_list, slot);
 ...
 * not read more data until some messages are finalized.
 *
 * The semantics are analogous to those of
...
void _dbus_connection_get_stats(
    DBusConnection* connection, dbus_uint32_t* in_messages,
    dbus_uint32_t* in_bytes, dbus_uint32_t* in_fds,
    dbus_uint32_t* in_peak_bytes, dbus_uint32_t* in_peak_fds,
    dbus_uint32_t* out_messages, dbus_uint32_t* out_bytes,
    dbus_uint32_t* out_fds, dbus_uint32_t* out_peak_bytes,
    dbus_uint32_t* out_peak_fds)
{
    ...
    if (out_fds != NULL)
        *out_fds =
            _dbus_counter_get_unix_fd_value(connection->outgoing_counter);

    if (out_peak_bytes != NULL)
        *out_peak_bytes =
            _dbus_counter_get_peak_size_value(connection->outgoing_counter);

    if (out_peak_fds != NULL)
        *out_peak_fds =
            _dbus_counter_get_peak_unix_fd_value(connection->outgoing_counter);
