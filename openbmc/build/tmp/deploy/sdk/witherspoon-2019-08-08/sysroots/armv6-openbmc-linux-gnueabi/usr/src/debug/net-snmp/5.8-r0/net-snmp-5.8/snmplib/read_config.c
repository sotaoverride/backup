...
netsnmp_feature_child_of(read_config_all, libnetsnmp)
    ...
    netsnmp_feature_child_of(unregister_app_config_handler, read_config_all)
        netsnmp_feature_child_of(
            read_config_register_app_prenetsnmp_mib_handler, netsnmp_unused)
...
static struct config_line*
    internal_register_config_handler(const char* type_param, const char* token,
                                     ...
                                     int when)
{
    ...
    if (strchr(type, ':'))
    {
        ...
        while (cptr)
        {
            ...
            ltmp2 = internal_register_config_handler(c, token, parser, releaser,
                                                     help, when);
        }
        return ltmp2;
    }
    ...
    if (*ctmp == NULL)
    {
        *ctmp = (struct config_files*)calloc(1, sizeof(struct config_files));
        if (!*ctmp)
        {
            return NULL;
        }
    ...
    DEBUGMSGTL(
        ("9:read_config:register_handler", "registering %s %s\n", type, token));
    /*
    ...
    if (*ltmp == NULL)
    {
        *ltmp = (struct config_line*)calloc(1, sizeof(struct config_line));
        if (!*ltmp)
        {
            return NULL;
        }
...
struct config_line*
    register_prenetsnmp_mib_handler(const char* type, const char* token,
                                    void (*parser)(const char*, char*),
                                    void (*releaser)(void), const char* help)
{
    return internal_register_config_handler(type, token, parser, releaser, help,
                                            PREMIB_CONFIG);
}
...
{
    return (
        register_prenetsnmp_mib_handler(NULL, token, parser, releaser, help));
}
 ...
 */
struct config_line* register_config_handler(const char* type, const char* token,
                                            void (*parser)(const char*, char*),
                                            void (*releaser)(void),
                                            const char* help)
{
...
struct config_line*
    register_const_config_handler(const char* type, const char* token,
                                  void (*parser)(const char*, const char*),
                                  void (*releaser)(void), const char* help)
{
    return internal_register_config_handler(
        type, token, (void (*)(const char*, char*))parser, releaser, help,
        NORMAL_CONFIG);
}
...
struct config_line* read_config_find_handler(struct config_line* line_handlers,
                                             const char* token)
{
 ...
 */
int run_config_handler(struct config_line* lptr, const char* token, char* cptr,
                       int when)
{
    char* cp;
...
void netsnmp_config_remember_in_list(char* line,
...
void netsnmp_config_process_memory_list(struct read_config_memory** memp,
                                        int when, int clear)
{
 ...
 */
int read_config(const char* filename, struct config_line* line_handler,
                int when)
{
    static int depth = 0;
    static int files = 0;
        ...
        if ((cptr = skip_white(line)))
        {
                     ...
                     (strncasecmp(token, "include", 7) == 0))
            {
                if (strcasecmp(token, "include") == 0)
                {
                    ...
                    {
                        netsnmp_config_warn(
                            "Ambiguous token '%s' - use 'includeSearch' (or "
                            "'includeFile') instead.",
                            token);
                    }
                    continue;
                ...
                else if (strcasecmp(token, "includedir") == 0)
                {
                    ...
                    {
                        if (when != PREMIB_CONFIG)
                            netsnmp_config_error("Can't open include dir '%s'.",
                                                 cptr);
                        continue;
                    }
                    while ((entry = readdir(d)) != NULL)
                    {
                        ...
                        {
                            len = NAMLEN(entry);
                            if ((len > 5) && (strcmp(&(entry->d_name[len - 5]),
                                                     ".conf") == 0))
                            {
                                snprintf(fname, SNMP_MAXPATH, "%s/%s", cptr,
                                         entry->d_name);
                ...
                else if (strcasecmp(token, "includesearch") == 0)
                {
                    ...
                    if ((ret != SNMPERR_SUCCESS) && (when != PREMIB_CONFIG))
                        netsnmp_config_error("Included config '%s' not found.",
                                             cptr);
                    continue;
                }
                else
...
int read_configs_optional(const char* optional_config, int when)
{
    ...
    while (cp)
    {
        ...
        {
            DEBUGMSGTL(
                ("read_config", "Optional File \"%s\" does not exist.\n", cp));
            snmp_log_perror(cp);
        }
        else
        {
...
const char* get_configuration_directory(void)
{
                                      ...
                                      NETSNMP_DS_LIB_CONFIGURATION_DIR))
    {
        ...
        snprintf(defaultPath, sizeof(defaultPath), "%s%c%s%c%s%s%s%s",
                 SNMPCONFPATH, ENV_SEPARATOR_CHAR, SNMPSHAREPATH,
                 ENV_SEPARATOR_CHAR, SNMPLIBPATH,
 ...
 *    Whether this is actually an error is left to the application
 */
static int read_config_files_in_path(const char* path,
                                     struct config_files* ctmp, int when,
                                     const char* perspath, const char* persfile)
{
    int done, j;
    char configfile[300];
    ...
    DEBUGMSGTL(("read_config:path",
                " config path used for %s:%s (persistent path:%s)\n",
    ...
    while ((!done) && (*cptr2 != 0))
    {
        ...
        if (stat(cptr2, &statbuf) != 0)
        {
             ...
             */
            DEBUGMSGTL(
                ("read_config:dir", " Directory not present: %s\n", cptr2));
            cptr2 = ++cptr1;
            continue;
        }
#ifdef S_ISDIR
             ...
             strncmp(cptr2, persfile, strlen(persfile)) == 0))
        {
            ...
            {
                snprintf(configfile, sizeof(configfile), "%s/%s.%d.conf", cptr2,
                         ctmp->fileHeader, j);
                ...
                else
                {
                    ...
                    DEBUGMSGTL(("read_config_files",
                                ...
                                configfile));
                    if (read_config(configfile, ctmp->start, when) ==
                        SNMPERR_SUCCESS)
                        ret = SNMPERR_SUCCESS;
                }
            }
        }
        snprintf(configfile, sizeof(configfile), "%s/%s.conf", cptr2,
                 ctmp->fileHeader);
        configfile[sizeof(configfile) - 1] = 0;
        if (read_config(configfile, ctmp->start, when) == SNMPERR_SUCCESS)
            ret = SNMPERR_SUCCESS;
        snprintf(configfile, sizeof(configfile), "%s/%s.local.conf", cptr2,
                 ctmp->fileHeader);
        configfile[sizeof(configfile) - 1] = 0;
        if (read_config(configfile, ctmp->start, when) == SNMPERR_SUCCESS)
            ret = SNMPERR_SUCCESS;
...
void read_config_store(const char* type, const char* line)
{
    ...
    {
        snprintf(file, sizeof(file), "%s/%s.conf", get_persistent_directory(),
                 type);
 ...
 * incrementing number on each call, but less than
...
void snmp_save_persistent(const char* type)
{
    ...
    if (stat(file, &statbuf) == 0)
    {
        for (j = 0; j <= NETSNMP_MAX_PERSISTENT_BACKUPS; j++)
        {
            snprintf(fileold, sizeof(fileold), "%s/%s.%d.conf",
                     get_persistent_directory(), type, j);
     ...
     */
    snprintf(
        fileold, sizeof(fileold),
        "%s%s# Please save normal configuration tokens for %s in "
        "SNMPCONFPATH/%s.conf.\n# Only \"createUser\" tokens should be placed "
        "here by %s administrators.\n%s",
        "#\n# net-snmp (or ucd-snmp) persistent data "
        "file.\n#\n############################################################"
        ...
        type, type, type,
        "# (Did I mention: do not edit this "
        "file?)\n#\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    fileold[sizeof(fileold) - 1] = 0;
 ...
 *
 *
 * Unlink all backup files called
 *"<NETSNMP_PERSISTENT_DIRECTORY>/<type>.%d.conf".
 *
...
void snmp_clean_persistent(const char* type)
{
    ...
    if (stat(file, &statbuf) == 0)
    {
        for (j = 0; j <= NETSNMP_MAX_PERSISTENT_BACKUPS; j++)
        {
            snprintf(file, sizeof(file), "%s/%s.%d.conf",
                     get_persistent_directory(), type, j);
            file[sizeof(file) - 1] = 0;
            if (stat(file, &statbuf) == 0)
            {
 ...
 */
static void config_vlog(int level, const char* levelmsg, const char* str,
                        va_list args)
{
    char tmpbuf[256];
    char* buf = tmpbuf;
    ...
    if (len >= (int)sizeof(tmpbuf))
    {
        ...
        sprintf(buf, "%s: line %d: %s: %s\n", curfilename, linecount, levelmsg,
                str);
    }
...
char* read_config_save_octet_string(char* saveto, const u_char* str, size_t len)
{
     ...
     */
    for (i = 0, cp = str;
         i < len && cp && (isalpha(*cp) || isdigit(*cp) || *cp == ' ');
         cp++, i++)
        ;
 ...
 */
char* read_config_read_octet_string(const char* readfrom, u_char** str,
                                    size_t* len)
{
    return NETSNMP_REMOVE_CONST(
        char*, read_config_read_octet_string_const(readfrom, str, len));
}
    ...
    if (strncasecmp(readfrom, "0x", 2) == 0)
    {
        ...
        else
        {
            ...
            if (ilen >= *len)
            {
                ...
                DEBUGMSGTL(("read_config_read_octet_string",
                            "buffer too small (%lu < %lu)", (unsigned long)*len,
                            (unsigned long)ilen));
                *len = 0;
                cptr1 = skip_not_white_const(readfrom);
        ...
        for (i = 0; i < ilen; i++)
        {
            ...
            {
                /*
                 * we may lose memory, but don't know caller's buffer XX
                 * free(cptr);
                 */
                return (NULL);
            }
...
{
    return NETSNMP_REMOVE_CONST(
        char*, read_config_read_objid_const(readfrom, objid, len));
}

const char* read_config_read_objid_const(const char* readfrom, oid** objid,
                                         size_t* len)
{
 ...
 *
 */
char* read_config_read_data(int type, char* readfrom, void* dataptr,
                            size_t* len)
{
    int* intp;
    char** charpp;
    ...
    if (dataptr && readfrom)
        switch (type)
        {
            ...
            case ASN_BIT_STR:
                charpp = (char**)dataptr;
                return read_config_read_octet_string(readfrom, (u_char**)charpp,
                                                     len);
            ...
            default:
                DEBUGMSGTL(
                    ("read_config_read_data", "Fail: Unknown type: %d", type));
                return NULL;
        }
    return NULL;
}
 ...
 */
char* read_config_read_memory(int type, char* readfrom, char* dataptr,
                              size_t* len)
{
    int* intp;
    unsigned int* uintp;
    ...
    switch (type)
    {
        ...
        case ASN_PRIV_IMPLIED_OCTET_STR:
            return read_config_read_octet_string(readfrom, (u_char**)&dataptr,
                                                 len);
 ...
 */
char* read_config_store_data(int type, char* storeto, void* dataptr,
                             size_t* len)
{
...
char* read_config_store_data_prefix(char prefix, int type, char* storeto,
