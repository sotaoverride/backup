...
{
    fprintf(stderr,
            _("Usage: %s [-f] [-h] [-n] [-o offset] [-v] [-z undo_file] "
              "<transaction file> <filesystem>\n"),
            prg_name);
    exit(1);
}
                                ...
                                struct ext2_super_block* undo_super)
{
    ...
    if (memcmp(fs_super->s_uuid, undo_super->s_uuid, sizeof(fs_super->s_uuid)))
...
static int check_filesystem(struct undo_context* ctx, io_channel channel)
{
    ...
    if (retval)
    {
        com_err(prg_name, retval, "%s",
                _("while reading filesystem superblock."));
        return retval;
    }
    ...
    {
        fprintf(stderr, _("Undo file superblock checksum doesn't match.\n"));
        retval = -1;
        goto out;
    }
...
static int e2undo_setup_tdb(const char* name, io_manager* io_ptr)
{
    ...
    {
        retval = errno;
        com_err(prg_name, retval, _("while trying to delete %s"), tdb_file);
        goto errout;
    }
...
int main(int argc, char* argv[])
{
    ...
    while ((c = getopt(argc, argv, "fhno:vz:")) != EOF)
    {
        switch (c)
        {
            ...
            case 'o':
                ...
                if (*buf)
                {
                    com_err(prg_name, 0, _("illegal offset - %s"), optarg);
                    exit(1);
                }
    ...
    retval = manager->open(tdb_file, IO_FLAG_EXCLUSIVE, &undo_ctx.undo_file);
    if (retval)
    {
        com_err(prg_name, errno, _("while opening undo file `%s'\n"), tdb_file);
        exit(1);
    }
    retval = io_channel_read_blk64(undo_ctx.undo_file, 0,
                                   -(int)sizeof(undo_ctx.hdr), &undo_ctx.hdr);
    if (retval)
    {
        com_err(prg_name, retval, _("while reading undo file"));
        exit(1);
    }
    if (memcmp(undo_ctx.hdr.magic, E2UNDO_MAGIC, sizeof(undo_ctx.hdr.magic)))
    {
    ...
    retval = manager->open(
        device_name, IO_FLAG_EXCLUSIVE | (dry_run ? 0 : IO_FLAG_RW), &channel);
    if (retval)
    {
        com_err(prg_name, retval, _("while opening `%s'"), device_name);
        exit(1);
    }
    ...
    retval =
        ext2fs_get_mem(E2UNDO_MAX_EXTENT_BLOCKS * undo_ctx.blocksize, &buf);
    if (retval)
    {
        com_err(prg_name, retval, "%s", _("while allocating memory"));
        exit(1);
    ...
    dbg_printf("nr_keys=%lu, kpb=%zu, blksz=%u\n", undo_ctx.num_keys,
               keys_per_block, undo_ctx.blocksize);
    for (i = 0; i < undo_ctx.num_keys; i += keys_per_block)
    {
        size_t j, max_j;
        __le32 crc;

        retval = io_channel_read_blk64(undo_ctx.undo_file, lblk, 1, keyb);
        if (retval)
        {
        ...
        /* check keys */
        if (!force && ext2fs_le32_to_cpu(keyb->magic) != KEYBLOCK_MAGIC)
        {
        ...
        keyb->crc = 0;
        key_crc =
            ext2fs_crc32c_le(~0, (unsigned char*)keyb, undo_ctx.blocksize);
        if (!force && ext2fs_le32_to_cpu(crc) != key_crc)
        {
            fprintf(stderr, _("%s: key block checksum error at %llu.\n"),
                    tdb_file, lblk);
            exit(1);
        }
        ...
        for (j = 0, dkey = keyb->keys; j < max_j; j++, ikey++, dkey++)
        {
            ...
            lblk += (ikey->size + undo_ctx.blocksize - 1) / undo_ctx.blocksize;

            if (E2UNDO_MAX_EXTENT_BLOCKS * undo_ctx.blocksize < ikey->size)
            {
                com_err(prg_name, retval, _("%s: block %llu is too long."),
                        tdb_file, ikey->fsblk);
                exit(1);
            }

            /* check each block's crc */
            retval = io_channel_read_blk64(undo_ctx.undo_file, ikey->fileblk,
                                           -(int)ikey->size, buf);
            if (retval)
            {
                com_err(prg_name, retval, _("while fetching block %llu."),
                        ikey->fileblk);
                if (!force)
                    exit(1);
                io_error = 1;
            ...
            blk_crc = ext2fs_crc32c_le(~0, (unsigned char*)buf, ikey->size);
    ...
    for (i = 0, ikey = undo_ctx.keys; i < undo_ctx.num_keys; i++, ikey++)
    {
        ...
        if (verbose)
            printf("Replayed block of size %u from %llu to %llu\n", ikey->size,
                   ikey->fileblk, ikey->fsblk);
        if (dry_run)
            continue;
        retval =
            io_channel_write_blk64(channel, ikey->fsblk, -(int)ikey->size, buf);
        if (retval)
        {
            com_err(prg_name, retval, _("while writing block %llu."),
                    ikey->fsblk);
            io_error = 1;
        }
    }
    ...
    {
        retval =
            ext2fs_open2(device_name, NULL, EXT2_FLAG_RW | EXT2_FLAG_64BITS, 0,
                         0, manager, &fs);
        if (retval)
            goto out;
