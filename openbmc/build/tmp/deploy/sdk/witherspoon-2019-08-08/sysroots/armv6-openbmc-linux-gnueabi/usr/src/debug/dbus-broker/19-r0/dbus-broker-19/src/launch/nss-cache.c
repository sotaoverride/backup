...
static int nss_cache_node_new(NSSCacheNode** nodep, const char* name,
                              uint32_t id)
{
    NSSCacheNode* node;
...
void nss_cache_deinit(NSSCache* cache)
{
    ...
    c_rbtree_for_each_entry_safe_postorder_unlink(
        node, safe, &cache->user_tree, rb_by_name) nss_cache_node_free(node);

    c_rbtree_for_each_entry_safe_postorder_unlink(
        node, safe, &cache->group_tree, rb_by_name) nss_cache_node_free(node);
...
static int nss_cache_add(CRBTree* tree_by_name, CRBTree* tree_by_id,
                         NSSCacheNode** nodep, const char* name, uint32_t id)
{
    ...
    if (name)
    {
         ...
         */
        slot_by_name = c_rbtree_find_slot(
            tree_by_name, nss_cache_node_compare_name, name, &parent_by_name);

        if (!slot_by_name)
        {
            node_by_name =
                c_rbnode_entry(parent_by_name, NSSCacheNode, rb_by_name);
            node = node_by_name;
        }
    }
    ...
    if (id != (uint32_t)-1)
    {
         ...
         */
        slot_by_id =
            c_rbtree_find_slot(tree_by_id, nss_cache_node_compare_id,
                               (void*)(unsigned long)id, &parent_by_id);
        if (!slot_by_id)
        {
    ...
    if (!node)
    {
        ...
        if (name)
            c_rbtree_add(tree_by_name, parent_by_name, slot_by_name,
                         &node->rb_by_name);
        if (id != (uint32_t)-1)
            c_rbtree_add(tree_by_id, parent_by_id, slot_by_id, &node->rb_by_id);
    }
...
static int nss_cache_add_user(NSSCache* cache, struct passwd* pw)
{
    ...
    r = nss_cache_add(&cache->user_tree, &cache->uid_tree, &node, pw->pw_name,
                      pw->pw_uid);
    if (r)
        return error_trace(r);
...
static int nss_cache_add_group(NSSCache* cache, struct group* gr)
{
    ...
    r = nss_cache_add(&cache->group_tree, &cache->gid_tree, &node, gr->gr_name,
                      gr->gr_gid);
    if (r)
        return error_trace(r);
...
int nss_cache_populate(NSSCache* cache)
{
    ...
    r = nss_cache_add_user(cache, &(struct passwd){
                                      .pw_name = (char[]){"root"},
                                      .pw_uid = 0,
                                      .pw_gid = 0,
                                  });
    ...
    r = nss_cache_add_group(cache, &(struct group){
                                       .gr_name = (char[]){"root"},
                                       .gr_gid = 0,
                                   });
    if (r)
...
int nss_cache_get_uid(NSSCache* cache, uint32_t* uidp, uint32_t* gidp,
                      const char* name)
{
     ...
     */
    if (by_id)
        node = c_rbtree_find_entry(&cache->uid_tree, nss_cache_node_compare_id,
                                   (void*)(unsigned long)id, NSSCacheNode,
                                   rb_by_id);
    else
        node =
            c_rbtree_find_entry(&cache->user_tree, nss_cache_node_compare_name,
                                name, NSSCacheNode, rb_by_name);
    if (node)
    {
        pw = &node->pw;
...
int nss_cache_get_gid(NSSCache* cache, uint32_t* gidp, const char* name)
{
     ...
     */
    if (by_id)
        node = c_rbtree_find_entry(&cache->gid_tree, nss_cache_node_compare_id,
                                   (void*)(unsigned long)id, NSSCacheNode,
                                   rb_by_id);
    else
        node =
            c_rbtree_find_entry(&cache->group_tree, nss_cache_node_compare_name,
                                name, NSSCacheNode, rb_by_name);
    if (node)
    {
        gr = &node->gr;
...
int nss_cache_resolve_system_console_users(NSSCache* nss_cache,
                                           uint32_t** uidsp, size_t* n_uidsp)
{
