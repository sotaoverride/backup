...
static rfbBool rfbSendOneRectEncodingUltra(rfbClientPtr cl, int x, int y, int w,
                                           int h)
{
    ...
    if (cl->compStreamInitedLZO == FALSE)
    {
         ...
         * of `lzo_align_t' (instead of `char') to make sure it is properly
         * aligned.
         */
        cl->lzoWrkMem =
            malloc(sizeof(lzo_align_t) *
    ...
    deflateResult = lzo1x_1_compress(
        (unsigned char*)cl->beforeEncBuf,
        (lzo_uint)(w * h * (cl->format.bitsPerPixel / 8)),
        (unsigned char*)cl->afterEncBuf, &maxCompSize, cl->lzoWrkMem);
    ...
    /* Update statics */
    rfbStatRecordEncodingSent(cl, rfbEncodingUltra,
                              sz_rfbFramebufferUpdateRectHeader +
                                  sz_rfbZlibHeader + cl->afterEncBufLen,
                              maxRawSize);
...
rfbBool rfbSendRectEncodingUltra(rfbClientPtr cl, int x, int y, int w, int h)
{
    ...
    while (linesRemaining > 0)
    {
        ...
        if (!rfbSendOneRectEncodingUltra(cl, partialRect.x, partialRect.y,
                                         partialRect.w, partialRect.h))
        {
         ...
         */
        if ((cl->ublen > 0) && (linesToComp == maxLines))
        {
            if (!rfbSendUpdateBuf(cl))
            {
