...
#define HASH_KEY                                                               \
    SD_ID128_MAKE(d3, 1e, 48, fa, 90, fe, 4b, 4c, 9d, af, d5, d7, a1, b1, 2e,  \
                  8a)
...
int net_get_unique_predictable_data(sd_device* device, uint64_t* result)
{
    ...
    /* net_get_name() will return one of the device names based on stable
     * information about the device. If this is not available, we fall back to
    ...
    if (!name)
        return log_device_debug_errno(
            device, SYNTHETIC_ERRNO(ENODATA),
            "No stable identifying information found");

    log_device_debug(device, "Using \"%s\" as stable identifying information",
                     name);
    l = strlen(name);
...
bool net_match_config(Set* match_mac, char* const* match_paths,
                      char* const* match_drivers, char* const* match_types,
                      char* const* match_names,
                      const struct ether_addr* dev_mac, const char* dev_path,
                      const char* dev_driver, const char* dev_type,
                      const char* dev_name)
{
...
int config_parse_net_condition(const char* unit, const char* filename,
                         ...
                         void* data, void* userdata)
{
    ...
    for (;;)
    {
        ...
        {
            log_syntax(unit, LOG_ERR, filename, line, 0,
                       "Interface name is not valid or too long, ignoring "
                       "assignment: %s",
                       rvalue);
            return 0;
        }
                         ...
                         void* data, void* userdata)
{
    ...
    {
        log_syntax(unit, LOG_ERR, filename, line, 0,
                   "Interface alias is not ASCII clean or is too long, "
...
size_t serialize_in_addrs(FILE* f, const struct in_addr* addresses, size_t size,
...
int deserialize_in_addrs(struct in_addr** ret, const char* string)
{
    ...
    for (;;)
    {
        ...
        new_addresses =
            reallocarray(addresses, size + 1, sizeof(struct in_addr));
        if (!new_addresses)
            return -ENOMEM;
        else
...
int deserialize_in6_addrs(struct in6_addr** ret, const char* string)
{
    ...
    for (;;)
    {
        ...
        new_addresses =
            reallocarray(addresses, size + 1, sizeof(struct in6_addr));
        if (!new_addresses)
            return -ENOMEM;
        else
            addresses = new_addresses;
...
void serialize_dhcp_routes(FILE* f, const char* key, sd_dhcp_route** routes,
                           size_t size)
{
    unsigned i;
    ...
    for (i = 0; i < size; i++)
    {
        ...
        assert_se(sd_dhcp_route_get_destination_prefix_length(routes[i],
                                                              &length) >= 0);

        fprintf(f, "%s/%" PRIu8, inet_ntop(AF_INET, &dest, sbuf, sizeof(sbuf)),
                length);
        fprintf(f, ",%s%s", inet_ntop(AF_INET, &gw, sbuf, sizeof(sbuf)),
                (i < (size - 1)) ? " " : "");
    }
...
int deserialize_dhcp_routes(struct sd_dhcp_route** ret, size_t* ret_size,
                            size_t* ret_allocated, const char* string)
{
...
int serialize_dhcp_option(FILE* f, const char* key, const void* data,
                          size_t size)
{
