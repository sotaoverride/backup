...
rsRetVal (*CreateOrReuseStringProp)(prop_t** ppThis, const uchar* psz,
                                    const int len);
ENDinterface(prop)
#define propCURR_IF_VERSION                                                    \
     ...
     * obviously results in linker errors. Using "static inline" as below
     * together with "__attribute__((unused))" works in all cases. Note also
     * that we cannot work around this as we would otherwise need to evaluate
     * pThis more than once.
     */
    static inline uchar* __attribute__((unused)) ATTR_NONNULL(1)
        propGetSzStr(prop_t* pThis)
{
    return (pThis->len < CONF_PROP_BUFSIZE) ? pThis->szVal.sz
                                            : pThis->szVal.psz;
}
