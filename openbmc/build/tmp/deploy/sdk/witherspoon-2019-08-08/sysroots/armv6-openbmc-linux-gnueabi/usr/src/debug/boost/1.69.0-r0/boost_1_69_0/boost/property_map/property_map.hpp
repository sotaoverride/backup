...
template <class PA>
struct is_property_map : boost::mpl::and_<has_key_type<PA>, has_value_type<PA>,
                                          has_reference<PA>, has_category<PA>>
{
};
...
struct property_traits
    : boost::mpl::if_<is_property_map<PA>, default_property_traits<PA>,
...
{
    // BOOST_STATIC_ASSERT(boost::is_same<T, T*>::value && !"Using pointers as
    // property maps is deprecated");
...
struct LvaluePropertyMapConcept
{
    ...
    void constraints()
    {
        ...
        BOOST_MPL_ASSERT(
            (boost::mpl::or_<boost::is_same<const value_type&, reference>,
...
inline Reference get(const put_get_helper<Reference, PropertyMap>& pa,
                     const K& k)
{
...
class iterator_property_map
    : public boost::put_get_helper<
          R, iterator_property_map<RandomAccessIterator, IndexMap, T, R>>
{
  public:
...
inline iterator_property_map<RAIter, ID,
    ...
    make_iterator_property_map(RAIter iter, ID id)
{
    ...
    typedef iterator_property_map<
        RAIter, ID, typename std::iterator_traits<RAIter>::value_type,
        typename std::iterator_traits<RAIter>::reference>
        PA;
    return PA(iter, id);
}
...
class safe_iterator_property_map
    : public boost::put_get_helper<
          R, safe_iterator_property_map<RandomAccessIterator, IndexMap, T, R>>
{
  public:
    typedef typename property_traits<IndexMap>::key_type key_type;
    typedef T value_type;
    ...
    typename property_traits<IndexMap>::value_type size() const
    {
        return n;
    }
...
template <class RAIter, class ID>
inline safe_iterator_property_map<
    RAIter, ID, typename boost::detail::iterator_traits<RAIter>::value_type,
...
class const_associative_property_map
    : public boost::put_get_helper<
          const typename UniquePairAssociativeContainer::value_type::
              second_type&,
...
class static_property_map
    : public boost::put_get_helper<ValueType, static_property_map<ValueType>>
{
    ValueType value;
                                   ...
                                   dummy_property_map>
{
  public:
    ...
    inline dummy_property_map(const dummy_property_map& x) : c(x.c)
    {
    }
