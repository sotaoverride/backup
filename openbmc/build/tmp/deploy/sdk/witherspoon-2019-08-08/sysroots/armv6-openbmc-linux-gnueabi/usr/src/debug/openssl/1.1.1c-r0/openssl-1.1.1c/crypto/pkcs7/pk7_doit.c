                               ...
                               PKCS7_RECIP_INFO* ri, EVP_PKEY* pkey)
{
    ...
    if (EVP_PKEY_decrypt(pctx, NULL, &eklen, ri->enc_key->data,
                         ri->enc_key->length) <= 0)
        goto err;
...
BIO* PKCS7_dataDecode(PKCS7* p7, EVP_PKEY* pkey, BIO* in_bio, X509* pcert)
{
    ...
    switch (i)
    {
        case NID_pkcs7_signed:
            /*
             * p7->d.sign->contents is a PKCS7 structure consisting of a
             * contentType field and optional content. data_body is NULL if that
             * structure has no (=detached) content or if the contentType is
             * wrong (i.e., not "data").
             */
...
int PKCS7_SIGNER_INFO_sign(PKCS7_SIGNER_INFO* si)
{
    ...
    if (EVP_PKEY_CTX_ctrl(pctx, -1, EVP_PKEY_OP_SIGN, EVP_PKEY_CTRL_PKCS7_SIGN,
                          0, si) <= 0)
    {
                                ...
                                STACK_OF(X509_ATTRIBUTE) * sk)
{
    ...
    for (i = 0; i < sk_X509_ATTRIBUTE_num(sk); i++)
    {
        if ((sk_X509_ATTRIBUTE_set(
                p7si->auth_attr, i,
                X509_ATTRIBUTE_dup(sk_X509_ATTRIBUTE_value(sk, i)))) == NULL)
            return 0;
    }
    return 1;
}
