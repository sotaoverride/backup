...
static int is_xrealm_tgt(krb5_context, krb5_const_principal);

krb5_error_code krb5_ktkdb_close(krb5_context, krb5_keytab);

krb5_error_code krb5_ktkdb_get_entry(krb5_context, krb5_keytab,
                                     krb5_const_principal, krb5_kvno,
                                     krb5_enctype, krb5_keytab_entry*);

static krb5_error_code krb5_ktkdb_get_name(krb5_context context,
...
krb5_error_code krb5_ktkdb_resolve(context, name, id) krb5_context context;
const char* name;
krb5_keytab* id;
{
...
krb5_error_code krb5_ktkdb_close(context, kt) krb5_context context;
krb5_keytab kt;
{
    /*
 ...
 */
krb5_error_code krb5_ktkdb_set_context(krb5_context ctx)
{
    ktkdb_ctx = ctx;
    return 0;
}

krb5_error_code krb5_ktkdb_get_entry(in_context, id, principal, kvno, enctype,
                                     entry) krb5_context in_context;
krb5_keytab id;
...
krb5_keytab_entry* entry;
{
    ...
    kerror = krb5_dbe_find_enctype(context, db_entry, xrealm_tgt ? enctype : -1,
                                   -1, kvno, &key_data);
    ...
    kerror =
        krb5_dbe_decrypt_key_data(context, NULL, key_data, &entry->key, NULL);
    if (kerror)
        goto error;

    if (enctype > 0)
    {
        kerror = krb5_c_enctype_compare(context, enctype, entry->key.enctype,
                                        &similar);
        if (kerror)
            goto error;
 ...
 */
static int is_xrealm_tgt(krb5_context context, krb5_const_principal princ)
{
    krb5_data* dat;
