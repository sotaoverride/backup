...
static void __attribute__((__noreturn__)) usage(void)
{
    ...
    fprintf(out,
            _(" %1$s %2$srawN <major> <minor>\n"
              ...
              " %1$s -q %2$srawN\n"
              " %1$s -qa\n"),
            program_invocation_short_name, _PATH_RAWDEVDIR);
...
int main(int argc, char* argv[])
{
    ...
    switch (argc - optind)
    {
        case 1:
            ...
            if (retval)
                err(EXIT_RAW_ACCESS, _("Cannot locate block device '%s'"),
                    block_name);
            if (!S_ISBLK(statbuf.st_mode))
                errx(EXIT_RAW_ACCESS, _("Device '%s' is not a block device"),
                     block_name);
        ...
        case 2:
            block_major = strtol_octal_or_err(argv[optind],
                                              _("failed to parse argument"));
            block_minor = strtol_octal_or_err(argv[optind + 1],
                                              _("failed to parse argument"));
            break;
...
void open_raw_ctl(void)
{
    ...
    if (master_fd < 0)
    {
        ...
        if (master_fd < 0)
            err(EXIT_RAW_ACCESS, _("Cannot open master raw device '%s'"),
                _PATH_RAWDEVCTL);
    }
}
...
static int query(int minor_raw, const char* raw_name, int quiet)
{
    ...
    if (raw_name)
    {
        ...
        if (stat(raw_name, &statbuf) != 0)
            err(EXIT_RAW_ACCESS, _("Cannot locate raw device '%s'"), raw_name);
        if (!S_ISCHR(statbuf.st_mode))
            errx(EXIT_RAW_ACCESS, _("Raw device '%s' is not a character dev"),
                 raw_name);
        if (major(statbuf.st_rdev) != RAW_MAJOR)
            errx(EXIT_RAW_ACCESS, _("Device '%s' is not a raw dev"), raw_name);
    ...
    printf(_("%sraw%d:  bound to major %d, minor %d\n"), _PATH_RAWDEVDIR,
           minor_raw, (int)rq.block_major, (int)rq.block_minor);
    return 0;
}
