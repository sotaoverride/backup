...
typedef int (*DriverMethodFn)(Peer* peer, const char* path, CDVar* var_in,
                              uint32_t serial, CDVar* var_out);
...
#define DRIVER_T_MESSAGE(_body)                                                \
    C_DVAR_T_TUPLE2(C_DVAR_T_TUPLE7(C_DVAR_T_y, C_DVAR_T_y, C_DVAR_T_y,        \
                                    C_DVAR_T_y, C_DVAR_T_u, C_DVAR_T_u,        \
                                    C_DVAR_T_ARRAY(C_DVAR_T_TUPLE2(            \
                                        C_DVAR_T_y, C_DVAR_T_v))),             \
                    _body)

static const CDVarType driver_type_in_s[] = {
    C_DVAR_T_INIT(C_DVAR_T_TUPLE1(C_DVAR_T_s))};
static const CDVarType driver_type_in_su[] = {
    C_DVAR_T_INIT(C_DVAR_T_TUPLE2(C_DVAR_T_s, C_DVAR_T_u))};
static const CDVarType driver_type_in_ss[] = {
    C_DVAR_T_INIT(C_DVAR_T_TUPLE2(C_DVAR_T_s, C_DVAR_T_s))};
static const CDVarType driver_type_in_ssv[] = {
    C_DVAR_T_INIT(C_DVAR_T_TUPLE3(C_DVAR_T_s, C_DVAR_T_s, C_DVAR_T_v))};
static const CDVarType driver_type_in_apss[] = {C_DVAR_T_INIT(
    C_DVAR_T_TUPLE1(C_DVAR_T_ARRAY(C_DVAR_T_PAIR(C_DVAR_T_s, C_DVAR_T_s))))};
static const CDVarType driver_type_in_asu[] = {
    C_DVAR_T_INIT(C_DVAR_T_TUPLE2(C_DVAR_T_ARRAY(C_DVAR_T_s), C_DVAR_T_u))};
...
static const CDVarType driver_type_out_s[] = {
    C_DVAR_T_INIT(DRIVER_T_MESSAGE(C_DVAR_T_TUPLE1(C_DVAR_T_s)))};
static const CDVarType driver_type_out_b[] = {
    C_DVAR_T_INIT(DRIVER_T_MESSAGE(C_DVAR_T_TUPLE1(C_DVAR_T_b)))};
static const CDVarType driver_type_out_u[] = {
    C_DVAR_T_INIT(DRIVER_T_MESSAGE(C_DVAR_T_TUPLE1(C_DVAR_T_u)))};
static const CDVarType driver_type_out_v[] = {
    C_DVAR_T_INIT(DRIVER_T_MESSAGE(C_DVAR_T_TUPLE1(C_DVAR_T_v)))};
static const CDVarType driver_type_out_as[] = {C_DVAR_T_INIT(
    DRIVER_T_MESSAGE(C_DVAR_T_TUPLE1(C_DVAR_T_ARRAY(C_DVAR_T_s))))};
static const CDVarType driver_type_out_ay[] = {C_DVAR_T_INIT(
    DRIVER_T_MESSAGE(C_DVAR_T_TUPLE1(C_DVAR_T_ARRAY(C_DVAR_T_y))))};
static const CDVarType driver_type_out_apsv[] = {C_DVAR_T_INIT(DRIVER_T_MESSAGE(
    C_DVAR_T_TUPLE1(C_DVAR_T_ARRAY(C_DVAR_T_PAIR(C_DVAR_T_s, C_DVAR_T_v)))))};
...
{
    c_dvar_write(var, "s",
                 address_to_string(&(Address)ADDRESS_INIT_ID(peer->id)));
}
...
static void driver_dvar_write_signature_out(CDVar* var, const CDVarType* type)
{
    ...
    for (unsigned int i = strlen("((yyyyuua(yv))("), j = 0;
         i < type->length - strlen("))"); i++, j++)
...
static int driver_dvar_verify_signature_in(const CDVarType* type,
...
static void driver_write_reply_header(CDVar* var, Peer* peer, uint32_t serial,
                                      const CDVarType* type)
{
    c_dvar_write(
        var, "(yyyyuu[(y<u>)(y<s>)(y<", c_dvar_is_big_endian(var) ? 'B' : 'l',
        DBUS_MESSAGE_TYPE_METHOD_RETURN, DBUS_HEADER_FLAG_NO_REPLY_EXPECTED, 1,
        0, (uint32_t)-1, DBUS_MESSAGE_FIELD_REPLY_SERIAL, c_dvar_type_u, serial,
...
static void driver_write_signal_header(CDVar* var, Peer* peer,
                                       const char* member,
                                       const char* signature)
{
    c_dvar_write(var, "(yyyyuu[(y<s>)", c_dvar_is_big_endian(var) ? 'B' : 'l',
                 DBUS_MESSAGE_TYPE_SIGNAL, DBUS_HEADER_FLAG_NO_REPLY_EXPECTED,
                 1, 0, (uint32_t)-1, DBUS_MESSAGE_FIELD_SENDER, c_dvar_type_s,
    ...
    c_dvar_write(var, "(y<o>)(y<s>)(y<s>)(y<g>)])", DBUS_MESSAGE_FIELD_PATH,
                 c_dvar_type_o, "/org/freedesktop/DBus",
                 DBUS_MESSAGE_FIELD_INTERFACE, c_dvar_type_s,
                 "org.freedesktop.DBus", DBUS_MESSAGE_FIELD_MEMBER,
                 c_dvar_type_s, member, DBUS_MESSAGE_FIELD_SIGNATURE,
                 c_dvar_type_g, signature);
}
...
static const char* driver_error_to_string(int r)
{
    static const char* error_strings[_DRIVER_E_MAX] = {
        [DRIVER_E_PEER_NOT_REGISTERED] =
            "Message forwarding attempted without calling Hello()",
        ...
        [DRIVER_E_PEER_NOT_PRIVILEGED] =
            "The caller does not have the necessary privileged to call this "
            "method",
        [DRIVER_E_MONITOR_READ_ONLY] = "Monitor attempted to send message",
        [DRIVER_E_UNEXPECTED_FDS] =
            "Peer does not support file descriptor passing.",
        ...
        [DRIVER_E_FORWARD_FAILED] =
            "Request could not be forwarded to the parent process",
        ...
        [DRIVER_E_UNEXPECTED_ENVIRONMENT_UPDATE] =
            "User is not authorized to update environment variables",
        [DRIVER_E_SEND_DENIED] = "Sender is not authorized to send message",
        [DRIVER_E_RECEIVE_DENIED] =
            "Receiver is not authorized to receive message",
...
static int driver_monitor(Bus* bus, Peer* sender, Message* message)
{
    ...
    bus_get_monitor_destinations(bus, &destinations, sender,
                                 &message->metadata);

    while ((match_owner = c_list_first_entry(&destinations, MatchOwner,
                                             destinations_link)))
    {
        ...
        if (r)
        {
            ...
            {
                NameSet sender_names = NAME_SET_INIT_FROM_OWNER(
                ...
                bus_log_append_transaction(
                    bus, sender ? sender->id : ADDRESS_ID_INVALID, receiver->id,
                    &sender_names, NULL,
                    sender ? sender->policy->seclabel : bus->seclabel,
                    receiver->policy->seclabel, message);
                if (r == CONNECTION_E_QUOTA)
                    r = log_commitf(bus->log,
                                    "Monitor :1.%llu is being disconnected as "
                                    "it does not have the resources to receive "
                                    "a message it subscribed to.",
                                    receiver->id);
                else
                    r = log_commitf(
                        bus->log,
                        "Monitor :1.%llu is being disconnected as it does not "
                        "support receiving file descriptors it subscribed to.",
                        receiver->id);
                if (r)
                    return error_fold(r);
            }
...
static int driver_send_unicast(Peer* receiver, Message* message)
{
    ...
    if (r)
    {
        if (r == CONNECTION_E_QUOTA || r == CONNECTION_E_UNEXPECTED_FDS)
        {
            ...
            bus_log_append_transaction(receiver->bus, ADDRESS_ID_INVALID,
                                       receiver->id, NULL, &receiver_names,
                                       receiver->bus->seclabel,
                                       receiver->policy->seclabel, message);
            if (r == CONNECTION_E_QUOTA)
                r = log_commitf(receiver->bus->log,
                                "Peer :1.%llu is being disconnected as it does "
                                "not have the resources to receive a reply or "
                                "unicast signal it expects.",
                                receiver->id);
            else
                r = log_commitf(
                    receiver->bus->log,
                    "Peer :1.%llu is being disconnected as it does not support "
                    "receiving file descriptors it expects.",
                    receiver->id);
            if (r)
                return error_fold(r);
        }
...
static int driver_send_error(Peer* receiver, uint32_t serial, const char* error,
                             const char* error_message)
{
    ...
    c_dvar_write(&var, "((yyyyuu[(y<u>)(y<s>)(y<s>)(y<g>)(y<",
                 ...
                 DBUS_MESSAGE_TYPE_ERROR, DBUS_HEADER_FLAG_NO_REPLY_EXPECTED, 1,
                 0, (uint32_t)-1, DBUS_MESSAGE_FIELD_REPLY_SERIAL,
                 c_dvar_type_u, serial, DBUS_MESSAGE_FIELD_SENDER,
                 c_dvar_type_s, "org.freedesktop.DBus",
...
static int driver_notify_name_owner_changed(Bus* bus, MatchRegistry* matches,
                                            const char* name,
                                            ...
                                            const char* new_owner)
{
    ...
    bus_get_broadcast_destinations(bus, &destinations, matches, NULL,
                                   &metadata);

    if (!c_list_is_empty(&destinations))
    {
        static const CDVarType type[] = {C_DVAR_T_INIT(DRIVER_T_MESSAGE(
                                                 ...
                                                 destinations_link)))
        {
            ...
            r = policy_snapshot_check_receive(
                receiver->policy, NULL, metadata.fields.interface,
                metadata.fields.member, metadata.fields.path,
                metadata.header.type, true, 0);
            if (r)
            {
            ...
            if (r)
            {
                if (r == CONNECTION_E_QUOTA || r == CONNECTION_E_UNEXPECTED_FDS)
                {
                    ...
                    log_append_here(bus->log, LOG_WARNING, 0);
                    bus_log_append_transaction(
                        bus, ADDRESS_ID_INVALID, receiver->id, NULL,
                        &receiver_names, receiver->bus->seclabel,
                        receiver->policy->seclabel, message);
                    if (r == CONNECTION_E_QUOTA)
                        r = log_commitf(bus->log,
                                        "Peer :1.%llu is being disconnected as "
                                        "it does not have the resources to "
                                        "receive a signal it subscribed to.",
                                        receiver->id);
                    else
                        r = log_commitf(bus->log,
                                        "Peer :1.%llu is being disconnected as "
                                        "it does not support receiving the "
                                        "file descriptors it subscribed to.",
                                        receiver->id);
                    if (r)
                        return error_fold(r);
                }
...
static int driver_name_owner_changed(Bus* bus, MatchRegistry* matches,
                                     const char* name, Peer* old_owner,
                                     Peer* new_owner)
{
    ...
    old_owner_str =
        old_owner ? address_to_string(&(Address)ADDRESS_INIT_ID(old_owner->id))
                  : "";
    new_owner_str =
        new_owner ? address_to_string(&(Address)ADDRESS_INIT_ID(new_owner->id))
                  : "";
    ...
    r = driver_notify_name_owner_changed(bus, matches, name, old_owner_str,
                                         new_owner_str);
    if (r)
        return error_trace(r);
...
int driver_name_activation_failed(Bus* bus, Activation* activation)
{
    ...
    /* in case the name is activated again in the future, we should request it
     * again */
    activation->requested = false;

    c_list_for_each_entry_safe(request, request_safe,
                               &activation->activation_requests, link)
    {
        Peer* sender;
        ...
        if (sender)
        {
            r = driver_send_error(sender, request->serial,
    ...
    c_list_for_each_entry_safe(message, message_safe,
                               &activation->activation_messages, link)
    {
        Peer* sender;

        sender = peer_registry_find_peer(&bus->peers,
...
static int driver_name_activated(Activation* activation, Peer* receiver)
{
    ...
    /* in case the name is dropped again in the future, we should request it
     * again */
    activation->requested = false;
                               ...
                               &activation->activation_requests, link)
    {
        ...
        if (sender)
        {
            ...
            c_dvar_begin_write(&var, (__BYTE_ORDER == __BIG_ENDIAN),
                               driver_type_out_u, 1);
            c_dvar_write(&var, "(");
            driver_write_reply_header(&var, sender, request->serial,
                                      driver_type_out_u);
                               ...
                               &activation->activation_messages, link)
    {
        ...
        sender = peer_registry_find_peer(&receiver->bus->peers,
                                         message->message->metadata.sender_id);

        r = peer_queue_unicast(
            message->senders_policy, &sender_names,
            sender ? &sender->owned_replies : NULL, message->user,
            message->message->metadata.sender_id, receiver, message->message);
        if (r)
        {
            switch (r)
            {
                case PEER_E_QUOTA:
                    if (sender)
                        r = driver_send_error(
                            sender, message_read_serial(message->message),
...
static int driver_method_hello(Peer* peer, const char* path, CDVar* in_v,
                               uint32_t serial, CDVar* out_v)
{
    ...
    r = driver_name_owner_changed(peer->bus, &peer->name_owner_changed_matches,
                                  NULL, NULL, peer);
    if (r)
        return error_trace(r);
...
static int driver_method_request_name(Peer* peer, const char* path, CDVar* in_v,
                                      uint32_t serial, CDVar* out_v)
{
    ...
    if (change.name)
    {
        r = driver_name_owner_changed(
            peer->bus, &change.name->name_owner_changed_matches,
            change.name->name,
        ...
        r = driver_name_activated(
            change.name->activation,
            c_container_of(change.new_owner, Peer, owned_names));
        if (r)
            return error_trace(r);
    }
...
static int driver_method_release_name(Peer* peer, const char* path, CDVar* in_v,
                                      uint32_t serial, CDVar* out_v)
{
    NameChange change = NAME_CHANGE_INIT;
    const char* name;
...
static int driver_method_list_queued_owners(Peer* peer, const char* path,
                                            CDVar* in_v, uint32_t serial,
                                            CDVar* out_v)
{
        ...
        if (name)
        {
            c_list_for_each_entry(ownership, &name->ownership_list, name_link)
                driver_dvar_write_unique_name(
                    out_v, c_container_of(ownership->owner, Peer, owned_names));
        }
        else
        {
...
static int driver_method_list_names(Peer* peer, const char* path, CDVar* in_v,
                                    uint32_t serial, CDVar* out_v)
{
    Peer* p;
    Name* name;
...
static int driver_method_list_activatable_names(Peer* peer, const char* path,
                                                CDVar* in_v, uint32_t serial,
                                                CDVar* out_v)
{
    Name* name;
...
static int driver_method_name_has_owner(Peer* peer, const char* path,
                                        CDVar* in_v, uint32_t serial,
                                        CDVar* out_v)
{
    Peer* connection;
...
static int driver_method_start_service_by_name(Peer* peer, const char* path,
                                               CDVar* in_v, uint32_t serial,
                                               CDVar* out_v)
{
    const char* service;
    ...
    {
        r = activation_queue_request(name->activation, peer->user, peer->id,
                                     serial);
        if (r)
        {
...
static int driver_method_update_activation_environment(
    Peer* peer, const char* path, CDVar* in_v, uint32_t serial, CDVar* out_v)
{
...
static int driver_method_get_name_owner(Peer* peer, const char* path,
                                        CDVar* in_v, uint32_t serial,
                                        CDVar* out_v)
{
...
static int driver_method_get_connection_unix_user(Peer* peer, const char* path,
                                                  CDVar* in_v, uint32_t serial,
                                                  CDVar* out_v)
{
    Peer* connection;
...
static int driver_method_get_connection_unix_process_id(
    Peer* peer, const char* path, CDVar* in_v, uint32_t serial, CDVar* out_v)
{
    Peer* connection;
    const char* name;
...
static int driver_method_get_connection_credentials(
    Peer* peer, const char* path, CDVar* in_v, uint32_t serial, CDVar* out_v)
{
    Peer* connection;
    ...
    c_dvar_write(out_v, "([{s<u>}{s<u>}", "UnixUserID", c_dvar_type_u, uid,
    ...
    if (n_seclabel)
    {
         ...
         */
        c_dvar_write(
            out_v, "{s<", "LinuxSecurityLabel",
            (const CDVarType[]){C_DVAR_T_INIT(C_DVAR_T_ARRAY(C_DVAR_T_y))});
...
static int driver_method_get_adt_audit_session_data(
    Peer* peer, const char* path, CDVar* in_v, uint32_t serial, CDVar* out_v)
{
    Peer* connection;
    const char* name;
...
static int driver_method_get_connection_selinux_security_context(
    Peer* peer, const char* path, CDVar* in_v, uint32_t serial, CDVar* out_v)
{
    const char *name, *seclabel;
    size_t n_seclabel;
...
static int driver_method_add_match(Peer* peer, const char* path, CDVar* in_v,
                                   uint32_t serial, CDVar* out_v)
{
...
static int driver_method_remove_match(Peer* peer, const char* path, CDVar* in_v,
                                      uint32_t serial, CDVar* out_v)
{
    const char* rule_string;
    int r;
...
int driver_reload_config_completed(Bus* bus, uint64_t sender_id,
                                   uint32_t reply_serial)
{
    ...
    if (sender)
    {
        ...
        c_dvar_begin_write(&var, (__BYTE_ORDER == __BIG_ENDIAN),
                           driver_type_out_unit, 1);
        c_dvar_write(&var, "(");
        driver_write_reply_header(&var, sender, reply_serial,
                                  driver_type_out_unit);
        c_dvar_write(&var, "()");
...
int driver_reload_config_invalid(Bus* bus, uint64_t sender_id,
                                 uint32_t reply_serial)
{
    Peer* sender;
    int r;
    ...
    if (sender)
    {
        r = driver_send_error(sender, reply_serial,
                              "org.freedesktop.DBus.Error.Failed",
...
static int driver_method_reload_config(Peer* peer, const char* path,
                                       CDVar* in_v, uint32_t serial,
                                       CDVar* out_v)
{
    int r;
...
static int driver_method_get_id(Peer* peer, const char* path, CDVar* in_v,
                                uint32_t serial, CDVar* out_v)
{
...
static int driver_method_introspect(Peer* peer, const char* path, CDVar* in_v,
                                    uint32_t serial, CDVar* out_v)
{
    ...
    /*
     * We return the same intropsection data for the canonical path as the
     * reference implementation does, and all methods behave the same. However,
     * we only expose simplified intropsection data for all other paths, as if
     * all interfaces were only implemented on the canonical path. The only
     * reason they are implemented on arbitrary paths is for backwards
     * compatibility.
     */
...
static int driver_method_become_monitor(Peer* peer, const char* path,
                                        CDVar* in_v, uint32_t serial,
                                        CDVar* out_v)
{
...
error:
    while (owned_matches.rule_tree.root)
        match_rule_user_unref(c_container_of(owned_matches.rule_tree.root,
                                             MatchRule, owner_node));

    return r;
}

static int driver_method_ping(Peer* peer, const char* path, CDVar* in_v,
                              uint32_t serial, CDVar* out_v)
{
    int r;
...
static int driver_method_get_machine_id(Peer* peer, const char* path,
                                        CDVar* in_v, uint32_t serial,
                                        CDVar* out_v)
{
    int r;
...
static void driver_append_property_features(CDVar* v)
{
    static const CDVarType variant_type[] = {
        C_DVAR_T_INIT(C_DVAR_T_ARRAY(C_DVAR_T_s))};
...
static int driver_method_get(Peer* peer, const char* path, CDVar* in_v,
                             uint32_t serial, CDVar* out_v)
{
...
static int driver_method_set(Peer* peer, const char* path, CDVar* in_v,
                             uint32_t serial, CDVar* out_v)
{
    const char *interface, *property;
    int r;
    ...
    if (strcmp(property, "Features") != 0 &&
        strcmp(property, "Interfaces") != 0)
        return DRIVER_E_UNEXPECTED_PROPERTY;

    return DRIVER_E_READONLY_PROPERTY;
}

static int driver_method_get_all(Peer* peer, const char* path, CDVar* in_v,
                                 uint32_t serial, CDVar* out_v)
{
...
static int driver_handle_method(const DriverMethod* method, Peer* peer,
                                const char* path, uint32_t serial,
                                const char* signature_in, Message* message_in)
{
    _c_cleanup_(c_dvar_deinit) CDVar var_in = C_DVAR_INIT,
                                     var_out = C_DVAR_INIT;
    int r;
    ...
    c_dvar_begin_read(&var_in, message_in->big_endian, method->in, 1,
                      message_in->body, message_in->n_body);
    c_dvar_begin_write(&var_out, (__BYTE_ORDER == __BIG_ENDIAN), method->out,
                       1);
...
static const DriverMethod driver_methods[] = {
    {"Hello", false, NULL, driver_method_hello, c_dvar_type_unit,
     driver_type_out_s},
    {"AddMatch", true, NULL, driver_method_add_match, driver_type_in_s,
     driver_type_out_unit},
    {"RemoveMatch", true, NULL, driver_method_remove_match, driver_type_in_s,
     driver_type_out_unit},
    {"RequestName", true, NULL, driver_method_request_name, driver_type_in_su,
     driver_type_out_u},
    {"ReleaseName", true, NULL, driver_method_release_name, driver_type_in_s,
     driver_type_out_u},
    {"GetConnectionCredentials", true, NULL,
     driver_method_get_connection_credentials, driver_type_in_s,
     driver_type_out_apsv},
    {"GetConnectionUnixUser", true, NULL,
     driver_method_get_connection_unix_user, driver_type_in_s,
     driver_type_out_u},
    {"GetConnectionUnixProcessID", true, NULL,
     driver_method_get_connection_unix_process_id, driver_type_in_s,
     driver_type_out_u},
    {"GetAdtAuditSessionData", true, NULL,
     driver_method_get_adt_audit_session_data, driver_type_in_s,
     driver_type_out_ay},
    {"GetConnectionSELinuxSecurityContext", true, NULL,
     driver_method_get_connection_selinux_security_context, driver_type_in_s,
     driver_type_out_ay},
    {"StartServiceByName", true, NULL, driver_method_start_service_by_name,
     driver_type_in_su, driver_type_out_u},
    {"ListQueuedOwners", true, NULL, driver_method_list_queued_owners,
     driver_type_in_s, driver_type_out_as},
    {"ListNames", true, NULL, driver_method_list_names, c_dvar_type_unit,
     driver_type_out_as},
    {"ListActivatableNames", true, NULL, driver_method_list_activatable_names,
     c_dvar_type_unit, driver_type_out_as},
    {"NameHasOwner", true, NULL, driver_method_name_has_owner, driver_type_in_s,
     driver_type_out_b},
    {"UpdateActivationEnvironment", true, "/org/freedesktop/DBus",
     driver_method_update_activation_environment, driver_type_in_apss,
     driver_type_out_unit},
    {"GetNameOwner", true, NULL, driver_method_get_name_owner, driver_type_in_s,
     driver_type_out_s},
    {"ReloadConfig", true, NULL, driver_method_reload_config, c_dvar_type_unit,
     driver_type_out_unit},
    {"GetId", true, NULL, driver_method_get_id, c_dvar_type_unit,
     driver_type_out_s},
    {},
};

static const DriverMethod monitoring_methods[] = {
    {"BecomeMonitor", true, "/org/freedesktop/DBus",
     driver_method_become_monitor, driver_type_in_asu, driver_type_out_unit},
    {},
};

static const DriverMethod introspectable_methods[] = {
    {"Introspect", true, NULL, driver_method_introspect, c_dvar_type_unit,
     driver_type_out_s},
    {},
};

static const DriverMethod peer_methods[] = {
    {"Ping", true, NULL, driver_method_ping, c_dvar_type_unit,
     driver_type_out_unit},
    {"GetMachineId", true, NULL, driver_method_get_machine_id, c_dvar_type_unit,
     driver_type_out_s},
    {},
};

static const DriverMethod properties_methods[] = {
    {"Get", true, "/org/freedesktop/DBus", driver_method_get, driver_type_in_ss,
     driver_type_out_v},
    {"Set", true, "/org/freedesktop/DBus", driver_method_set,
     driver_type_in_ssv, driver_type_out_unit},
    {"GetAll", true, "/org/freedesktop/DBus", driver_method_get_all,
     driver_type_in_s, driver_type_out_apsv},
    {},
};

static int driver_dispatch_method(Peer* peer, const DriverMethod* methods,
                                  uint32_t serial, const char* method,
                                  ...
                                  Message* message)
{
    for (size_t i = 0; methods[i].name; i++)
    {
        ...
        if (_c_likely_(peer_is_registered(peer)) ||
            !methods[i].needs_registration)
            return driver_handle_method(methods + i, peer, path, serial,
                                        signature, message);
    }
...
static int driver_dispatch_interface(Peer* peer, uint32_t serial,
                                     ...
                                     Message* message)
{
    ...
    r = policy_snapshot_check_send(peer->policy, NULL, NULL, interface, member,
                                   path, message->header->type, false,
    ...
    if (r)
    {
        if (r == POLICY_E_ACCESS_DENIED || r == POLICY_E_SELINUX_ACCESS_DENIED)
        {
            ...
            bus_log_append_policy_send(
                peer->bus,
                (r == POLICY_E_ACCESS_DENIED ? BUS_LOG_POLICY_TYPE_INTERNAL
                                             : BUS_LOG_POLICY_TYPE_SELINUX),
                peer->id, ADDRESS_ID_INVALID, &names, NULL,
                peer->policy->seclabel, peer->bus->seclabel, message);
            r = log_commitf(peer->bus->log,
                            "A security policy denied :1.%llu to send method "
                            "call %s:%s.%s to org.freedesktop.DBus.",
    ...
    if (interface)
    {
        for (size_t i = 0; i < C_ARRAY_SIZE(interfaces); ++i)
        {
            ...
            return error_trace(
                driver_dispatch_method(peer, interfaces[i].methods, serial,
                                       member, path, signature, message));
        }
    ...
    else
    {
        for (size_t i = 0; i < C_ARRAY_SIZE(interfaces); ++i)
        {
            r = driver_dispatch_method(peer, interfaces[i].methods, serial,
                                       member, path, signature, message);
...
int driver_goodbye(Peer* peer, bool silent)
{
    ...
    c_list_for_each_entry_safe(reply, reply_safe,
                               &peer->owned_replies.reply_list, owner_link)
        reply_slot_free(reply);

    match_registry_flush(&peer->sender_matches);

    c_rbtree_for_each_entry_safe_postorder_unlink(
        ownership, ownership_safe, &peer->owned_names.ownership_tree,
        owner_node)
    {
        NameChange change;
    ...
    if (peer_is_registered(peer))
    {
        ...
        {
            r = driver_name_owner_changed(
                peer->bus, &peer->name_owner_changed_matches, NULL, peer, NULL);
            if (r)
                return error_trace(r);
        }
    ...
    c_rbtree_for_each_entry_safe_postorder_unlink(
        reply, reply_safe, &peer->replies.reply_tree, registry_node)
    {
        Peer* sender = c_container_of(reply->owner, Peer, owned_replies);

        if (!silent)
        {
            r = driver_send_error(sender, reply->serial,
                                  ...
                                  Message* message)
{
    ...
    if (!receiver)
    {
        ...
        r = activation_queue_message(name->activation, sender->user,
                                     &sender->owned_names, sender->policy,
                                     message);
        if (r)
        {
            if (r == ACTIVATION_E_QUOTA)
    ...
    r = peer_queue_unicast(sender->policy, &sender_names,
                           &sender->owned_replies, sender->user, sender->id,
                           receiver, message);
    if (r)
    {
...
static int driver_forward_broadcast(Peer* sender, Message* message)
{
    ...
    bus_get_broadcast_destinations(sender->bus, &destinations,
                                   &sender->sender_matches, sender,
                                   &message->metadata);
                                             ...
                                             destinations_link)))
    {
        ...
        r = policy_snapshot_check_send(
            sender->policy, receiver->seclabel, &receiver_names,
            message->metadata.fields.interface, message->metadata.fields.member,
            message->metadata.fields.path, message->metadata.header.type, true,
            message->metadata.fields.unix_fds);
        if (r)
        {
            if (r == POLICY_E_ACCESS_DENIED ||
        ...
        r = policy_snapshot_check_receive(
            receiver->policy, &sender_names, message->metadata.fields.interface,
            message->metadata.fields.member, message->metadata.fields.path,
            message->metadata.header.type, true,
            message->metadata.fields.unix_fds);
        if (r)
        {
            if (r == POLICY_E_ACCESS_DENIED)
        ...
        if (r)
        {
            if (r == CONNECTION_E_QUOTA || r == CONNECTION_E_UNEXPECTED_FDS)
            {
                ...
                bus_log_append_transaction(
                    sender->bus, sender->id, receiver->id, &sender_names,
                    &receiver_names, sender->policy->seclabel,
                    receiver->policy->seclabel, message);
                if (r == CONNECTION_E_QUOTA)
                    r = log_commitf(sender->bus->log,
                                    "Peer :1.%llu is being disconnected as it "
                                    "does not have the resources to receive a "
                                    "signal it subscribed to.",
                                    receiver->id);
                else
                    r = log_commitf(sender->bus->log,
                                    "Peer :1.%llu is being disconnected as it "
                                    "does not support receiving the file "
                                    "descriptors it subscribed to.",
                                    receiver->id);
                if (r)
                    return error_fold(r);
            }
...
static int driver_dispatch_internal(Peer* peer, Message* message)
{
    ...
    if (_c_unlikely_(message->metadata.header.type ==
                         DBUS_MESSAGE_TYPE_METHOD_CALL &&
                     !message->metadata.fields.destination))
    {
        /*
         * The empty destination is treated as a special peer, only implementing
         * the Peer interface.
         */
        if (message->metadata.fields.interface &&
            strcmp(message->metadata.fields.interface,
                   "org.freedesktop.DBus.Peer") != 0)
            return DRIVER_E_UNEXPECTED_METHOD;

        return error_trace(driver_dispatch_method(
            peer, peer_methods, message_read_serial(message),
            message->metadata.fields.member, message->metadata.fields.path,
            message->metadata.fields.signature, message));
    }

    if (c_string_equal(message->metadata.fields.destination,
                       "org.freedesktop.DBus"))
    {
        r = driver_dispatch_interface(
            peer, message_read_serial(message),
            message->metadata.fields.interface, message->metadata.fields.member,
            message->metadata.fields.path, message->metadata.fields.signature,
            message);
        if (r)
        {
            if (_c_unlikely_(!peer_is_registered(peer)) &&
    ...
    switch (message->metadata.header.type)
    {
        ...
        case DBUS_MESSAGE_TYPE_METHOD_CALL:
            return error_trace(driver_forward_unicast(
                peer, message->metadata.fields.destination, message));
        case DBUS_MESSAGE_TYPE_METHOD_RETURN:
        case DBUS_MESSAGE_TYPE_ERROR:
            r = peer_queue_reply(peer, message->metadata.fields.destination,
...
int driver_dispatch(Peer* peer, Message* message)
{
    ...
    switch (r)
    {
        ...
        case DRIVER_E_UNEXPECTED_FDS:
            r = driver_send_error(peer, message_read_serial(message),
                                  "org.freedesktop.DBus.Error.Failed",
