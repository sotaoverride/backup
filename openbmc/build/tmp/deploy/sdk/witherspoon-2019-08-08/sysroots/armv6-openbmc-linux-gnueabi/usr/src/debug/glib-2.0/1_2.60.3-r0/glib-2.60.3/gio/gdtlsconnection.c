...
static void g_dtls_connection_default_init(GDtlsConnectionInterface* iface)
{
    /**
     * GDtlsConnection:base-socket:
     *
    ...
    g_object_interface_install_property(
        iface,
        g_param_spec_object("base-socket", P_("Base Socket"),
     ...
     */
    g_object_interface_install_property(
        iface, g_param_spec_object("database", P_("Database"),
                                   P_("Certificate database to use for looking "
                                      "up or verifying certificates"),
                                   G_TYPE_TLS_DATABASE,
     ...
     */
    g_object_interface_install_property(
        iface, g_param_spec_object("interaction", P_("Interaction"),
    ...
    g_object_interface_install_property(
        ...
        g_param_spec_enum("rehandshake-mode", P_("Rehandshake mode"),
                          ...
                          G_TYPE_TLS_REHANDSHAKE_MODE, G_TLS_REHANDSHAKE_NEVER,
     ...
     */
    g_object_interface_install_property(
        iface, g_param_spec_object("certificate", P_("Certificate"),
     ...
     */
    g_object_interface_install_property(
        iface, g_param_spec_object("peer-certificate", P_("Peer Certificate"),
     ...
     */
    g_object_interface_install_property(
        iface, g_param_spec_flags(
                   "peer-certificate-errors", P_("Peer Certificate Errors"),
    ...
    signals[ACCEPT_CERTIFICATE] = g_signal_new(
        I_("accept-certificate"), G_TYPE_DTLS_CONNECTION, G_SIGNAL_RUN_LAST,
 ...
 * Since: 2.48
 */
void g_dtls_connection_handshake_async(GDtlsConnection* conn, int io_priority,
                                       ...
                                       gpointer user_data)
{
    ...
    G_DTLS_CONNECTION_GET_INTERFACE(conn)->handshake_async(
                                            ...
                                            GError** error)
{
    ...
    return G_DTLS_CONNECTION_GET_INTERFACE(conn)->handshake_finish(conn, result,
                                                                   error);
}
                                    ...
                                    GCancellable* cancellable, GError** error)
{
    ...
    return iface->shutdown(conn, shutdown_read, shutdown_write, cancellable,
                           error);
}
                                      ...
                                      gpointer user_data)
{
    ...
    iface->shutdown_async(conn, TRUE, TRUE, io_priority, cancellable, callback,
                          user_data);
}
 ...
 * Since: 2.48
 */
void g_dtls_connection_close_async(GDtlsConnection* conn, int io_priority,
                                   GCancellable* cancellable,
                                   GAsyncReadyCallback callback,
                                   gpointer user_data)
{
    ...
    G_DTLS_CONNECTION_GET_INTERFACE(conn)->shutdown_async(
        conn, TRUE, TRUE, io_priority, cancellable, callback, user_data);
}
