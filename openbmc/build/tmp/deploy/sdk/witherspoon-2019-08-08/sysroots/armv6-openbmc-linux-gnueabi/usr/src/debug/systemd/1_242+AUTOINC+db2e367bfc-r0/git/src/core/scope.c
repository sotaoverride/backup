...
static int scope_dispatch_timer(sd_event_source* source, usec_t usec,
                                void* userdata);
...
static int scope_arm_timer(Scope* s, usec_t usec)
{
    ...
    r = sd_event_add_time(UNIT(s)->manager->event, &s->timer_event_source,
                          CLOCK_MONOTONIC, usec, 0, scope_dispatch_timer, s);
    if (r < 0)
        return r;
...
static int scope_verify(Scope* s)
{
    ...
    if (set_isempty(UNIT(s)->pids) && !MANAGER_IS_RELOADING(UNIT(s)->manager) &&
...
static int scope_load_init_scope(Unit* u)
{
    ...
    /* init.scope is a bit special, as it has to stick around forever. Because
     * of its special semantics we
...
static int scope_load(Unit* u)
{
    ...
    if (!u->transient && !MANAGER_IS_RELOADING(u->manager))
        /* Refuse to load non-transient scope units, but allow them while
         * reloading. */
        return -ENOENT;
...
static int scope_coldplug(Unit* u)
{
    ...
    {
        r = scope_arm_timer(s, usec_add(u->state_change_timestamp.monotonic,
                                        s->timeout_stop_usec));
        if (r < 0)
            return r;
    }
...
static void scope_dump(Unit* u, FILE* f, const char* prefix)
{
    ...
    fprintf(f,
            "%sScope State: %s\n"
            "%sResult: %s\n",
            prefix, scope_state_to_string(s->state), prefix,
...
static void scope_enter_dead(Scope* s, ScopeResult f)
{
    ...
    unit_log_result(UNIT(s), s->result == SCOPE_SUCCESS,
                    scope_result_to_string(s->result));
...
static void scope_enter_signal(Scope* s, ScopeState state, ScopeResult f)
{
    ...
    /* If we have a controller set let's ask the controller nicely to terminate
     * the scope, instead of us going directly into SIGTERM berserk mode */
...
static int scope_start(Unit* u)
{
    ...
    if (r < 0)
    {
        log_unit_warning_errno(
            u, r, "Failed to add PIDs to scope's control group: %m");
        scope_enter_dead(s, SCOPE_FAILURE_RESOURCES);
        return r;
    }
...
static int scope_deserialize_item(Unit* u, const char* key, const char* value,
                                  FDSet* fds)
{
    Scope* s = SCOPE(u);
    int r;
...
static void scope_notify_cgroup_empty_event(Unit* u)
{
    ...
    if (IN_SET(s->state, SCOPE_RUNNING, SCOPE_ABANDONED, SCOPE_STOP_SIGTERM,
               SCOPE_STOP_SIGKILL))
        scope_enter_dead(s, SCOPE_SUCCESS);
}
...
static void scope_sigchld_event(Unit* u, pid_t pid, int code, int status)
{
    ...
    /* If we get a SIGCHLD event for one of the processes we were interested in,
     * then we look for others to watch, under the assumption that we'll sooner
     * or later get a SIGCHLD for them, as the original process we watched was
     * probably the parent of them, and they are hence now our children. */
    ...
    switch (s->state)
    {
        ...
        case SCOPE_STOP_SIGTERM:
            if (s->kill_context.send_sigkill)
            {
                ...
                scope_enter_signal(s, SCOPE_STOP_SIGKILL,
...
int scope_abandon(Scope* s)
{
    ...
    /* The client is no longer watching the remaining processes, so let's step
     * in here, under the assumption that
     * the remaining processes will be sooner or later reassigned to us as
     * parent. */
    (void)unit_enqueue_rewatch_pids(UNIT(s));
