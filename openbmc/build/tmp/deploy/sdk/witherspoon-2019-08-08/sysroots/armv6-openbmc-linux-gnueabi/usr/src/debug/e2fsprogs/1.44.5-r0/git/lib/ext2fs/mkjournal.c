                                    ...
                                    blk_t num_blocks, int flags)
{
    ...
    if ((retval =
             ext2fs_create_journal_superblock(fs, num_blocks, flags, &buf)))
        return retval;
...
errcode_t ext2fs_zero_blocks(ext2_filsys fs, blk_t blk, int num, blk_t* ret_blk,
                             int* ret_count)
{
    blk64_t ret_blk2;
    errcode_t retval;
...
static blk64_t get_midpoint_journal_block(ext2_filsys fs)
{
    ...
    group = ext2fs_group_of_blk2(
        fs,
        (ext2fs_blocks_count(fs->super) - fs->super->s_first_data_block) / 2);
                                     ...
                                     blk_t num_blocks, blk64_t goal, int flags)
{
    ...
    retval = ext2fs_fallocate(fs, falloc_flags, journal_ino, &inode, goal, 0,
                              num_blocks);
    if (retval)
        goto out2;
                                    ...
                                    blk64_t goal, int flags)
{
    ...
    if (flags & EXT2_MKJOURNAL_NO_MNT_CHECK)
        mount_flags = 0;
    else if ((retval = ext2fs_check_mount_point(fs->device_name, &mount_flags,
    ...
    {
        if ((mount_flags & EXT2_MF_BUSY) && !(fs->flags & EXT2_FLAG_EXCLUSIVE))
        {
            retval = EBUSY;
            goto errout;
        }
        ...
        if ((retval =
                 write_journal_inode(fs, journal_ino, num_blocks, goal, flags)))
            return retval;
    }

    fs->super->s_journal_inum = journal_ino;
    fs->super->s_journal_dev = 0;
    memset(fs->super->s_journal_uuid, 0, sizeof(fs->super->s_journal_uuid));
    ext2fs_set_feature_journal(fs->super);
...
main(int argc, char** argv)
{
    ...
    retval = ext2fs_open(device_name, EXT2_FLAG_RW, 0, 0, unix_io_manager, &fs);
    if (retval)
    {
    ...
    if (retval)
    {
        com_err(argv[0], retval, "while adding journal to %s", device_name);
        exit(1);
    }
