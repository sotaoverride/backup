...
DEFINE_HASH_OPS_WITH_VALUE_DESTRUCTOR(image_hash_ops, char, string_hash_func,
                                      string_compare_func, Image, image_unref);
...
static int image_new(ImageType t, const char* pretty, const char* path,
                     const char* filename, bool read_only, usec_t crtime,
                     usec_t mtime, Image** ret)
{
...
static int image_make(const char* pretty, int dfd, const char* path,
                      const char* filename, const struct stat* st, Image** ret)
{
     ...
     * symlinking trees, raw files and block devices into /var/lib/machines/,
     * and treat them normally.
     *
     * This function returns -ENOENT if we can't find the image after all, and
     * -EMEDIUMTYPE if it's not a file we recognize. */
    ...
    if (S_ISDIR(st->st_mode))
    {
        ...
        if (st->st_ino == 256)
        {
            ...
            if (r)
            {
                ...
                r = image_new(IMAGE_SUBVOLUME, pretty, path, filename,
                              info.read_only || read_only, info.otime, 0, ret);
                if (r < 0)
                    return r;
        ...
        r = image_new(IMAGE_DIRECTORY, pretty, path, filename,
                      read_only || (file_attr & FS_IMMUTABLE_FL), 0, 0, ret);
        if (r < 0)
            return r;
    ...
    else if (S_ISREG(st->st_mode) && endswith(filename, ".raw"))
    {
        ...
        r = image_new(IMAGE_RAW, pretty, path, filename,
                      !(st->st_mode & 0222) || read_only, crtime,
                      timespec_load(&st->st_mtim), ret);
        if (r < 0)
            return r;
    ...
    else if (S_ISBLK(st->st_mode))
    {
        ...
        block_fd =
            openat(dfd, filename, O_RDONLY | O_NONBLOCK | O_CLOEXEC | O_NOCTTY);
        if (block_fd < 0)
        ...
        else
        {
            ...
            if (!S_ISBLK(st->st_mode)) /* Verify that what we opened is actually
                                          what we think it is */
                return -ENOTTY;
            ...
            if (!read_only)
            {
                ...
                if (ioctl(block_fd, BLKROGET, &state) < 0)
                    log_debug_errno(errno,
                                    "Failed to issue BLKROGET on device %s/%s, "
                                    "ignoring: %m",
                                    path, filename);
                else if (state)
                    read_only = true;
            ...
            if (ioctl(block_fd, BLKGETSIZE64, &size) < 0)
                log_debug_errno(errno,
                                "Failed to issue BLKGETSIZE64 on device %s/%s, "
                                "ignoring: %m",
                                path, filename);
        ...
        r = image_new(IMAGE_BLOCK, pretty, path, filename,
                      !(st->st_mode & 0222) || read_only, 0, 0, ret);
        if (r < 0)
            return r;

        if (size != 0 && size != UINT64_MAX)
            (*ret)->usage = (*ret)->usage_exclusive = (*ret)->limit =
                (*ret)->limit_exclusive = size;
...
int image_find(ImageClass class, const char* name, Image** ret)
{
    ...
    NULSTR_FOREACH(path, image_search_path[class])
    {
        ...
        /* As mentioned above, we follow symlinks on this fstatat(), because we
         * want to permit people to symlink block devices into the search path
         */
...
int image_from_path(const char* path, Image** ret)
{
    ...
    /* Note that we don't set the 'discoverable' field of the returned object,
     * because we don't check here whether the image is in the image search
     * path. And if it is we don't know if the path we used is actually not
...
int image_discover(ImageClass class, Hashmap* h)
{
    ...
    NULSTR_FOREACH(path, image_search_path[class])
    {
        ...
        FOREACH_DIRENT_ALL(de, d, return -errno)
        {
            ...
            /* As mentioned above, we follow symlinks on this fstatat(), because
             * we want to permit people
...
int image_remove(Image* i)
{
    ...
    switch (i->type)
    {
        ...
        case IMAGE_SUBVOLUME:
            ...
            /* Let's unlink first, maybe it is a symlink? If that works we are
             * happy. Otherwise, let's get out the big guns */
            if (unlink(i->path) < 0)
            {
                r = btrfs_subvol_remove(i->path, BTRFS_REMOVE_RECURSIVE |
                                                     BTRFS_REMOVE_QUOTA);
                if (r < 0)
                    return r;
            }
        ...
        case IMAGE_BLOCK:
            ...
            /* If this is inside of /dev, then it's a real block device, hence
             * let's not touch the device node itself (but let's remove the
             * stuff stored alongside it). If it's anywhere else, let's try to
             * unlink the thing (it's most likely a symlink after all). */
...
static int rename_auxiliary_file(const char* path, const char* new_name,
                                 const char* suffix)
{
...
int image_rename(Image* i, const char* new_name)
{
    ...
    switch (i->type)
    {
        ...
        case IMAGE_DIRECTORY:
            /* Turn of the immutable bit while we rename the image, so that we
             * can rename it */
        ...
        case IMAGE_BLOCK:
            ...
            /* Refuse renaming raw block devices in /dev, the names are picked
             * by udev after all. */
            if (path_startswith(i->path, "/dev"))
                return -EROFS;
    ...
    STRV_FOREACH(j, settings)
    {
        ...
        if (r < 0 && r != -ENOENT)
            log_debug_errno(
                r, "Failed to rename settings file %s, ignoring: %m", *j);
    }

    r = rename_auxiliary_file(roothash, new_name, ".roothash");
    if (r < 0 && r != -ENOENT)
        log_debug_errno(r, "Failed to rename roothash file %s, ignoring: %m",
                        roothash);

    return 0;
}

static int clone_auxiliary_file(const char* path, const char* new_name,
                                const char* suffix)
{
    _cleanup_free_ char* rs = NULL;
    const char* fn;
...
int image_clone(Image* i, const char* new_name, bool read_only)
{
    ...
    switch (i->type)
    {
        ...
        case IMAGE_SUBVOLUME:
        case IMAGE_DIRECTORY:
            /* If we can we'll always try to create a new btrfs subvolume here,
             * even if the source is a plain directory. */
            ...
            if (r >= 0)
                /* Enable "subtree" quotas for the copy, if we didn't copy any
                 * quota from the source. */
    ...
    if (r < 0 && r != -ENOENT)
        log_debug_errno(r, "Failed to clone root hash file %s, ignoring: %m",
                        roothash);
                    ...
                    LockFile* local)
{
    ...
    {
        if (S_ISBLK(st.st_mode))
            r = asprintf(&p, "/run/systemd/nspawn/locks/block-%u:%u",
                         major(st.st_rdev), minor(st.st_rdev));
        else if (S_ISDIR(st.st_mode) || S_ISREG(st.st_mode))
            r = asprintf(&p, "/run/systemd/nspawn/locks/inode-%lu:%lu",
    ...
    /* For block devices we don't need the "local" lock, as the major/minor lock
     * above should be sufficient, since
    ...
    if (!path_startswith(path, "/dev"))
    {
        ...
        if (r < 0)
        {
            if ((operation & LOCK_SH) && r == -EROFS)
                log_debug_errno(
                    r, "Failed to create shared lock for '%s', ignoring: %m",
                    path);
            else
                return r;
        }
...
int image_read_metadata(Image* i)
{
    ...
    switch (i->type)
    {
        ...
        case IMAGE_DIRECTORY:
        {
            ...
            r = chase_symlinks("/etc/hostname", i->path,
                               CHASE_PREFIX_ROOT | CHASE_TRAIL_SLASH, &path);
            if (r < 0 && r != -ENOENT)
                log_debug_errno(r,
                                "Failed to chase /etc/hostname in image %s: %m",
                                i->name);
            else if (r >= 0)
            {
                ...
                if (r < 0)
                    log_debug_errno(
                        errno, "Failed to read /etc/hostname of image %s: %m",
                        i->name);
            }

            path = mfree(path);

            r = chase_symlinks("/etc/machine-id", i->path,
                               CHASE_PREFIX_ROOT | CHASE_TRAIL_SLASH, &path);
            if (r < 0 && r != -ENOENT)
                log_debug_errno(
                    r, "Failed to chase /etc/machine-id in image %s: %m",
                    i->name);
            else if (r >= 0)
            {
                _cleanup_close_ int fd = -1;
                ...
                else
                {
                    ...
                    if (r < 0)
                        log_debug_errno(r,
                                        "Image %s contains invalid machine ID.",
                                        i->name);
                }
            }
            ...
            r = chase_symlinks("/etc/machine-info", i->path,
                               CHASE_PREFIX_ROOT | CHASE_TRAIL_SLASH, &path);
            if (r < 0 && r != -ENOENT)
                log_debug_errno(
                    r, "Failed to chase /etc/machine-info in image %s: %m",
                    i->name);
            else if (r >= 0)
            {
                ...
                if (r < 0)
                    log_debug_errno(
                        r, "Failed to parse machine-info data of %s: %m",
                        i->name);
            }
            ...
            if (r < 0)
                log_debug_errno(
                    r, "Failed to read os-release in image, ignoring: %m");
