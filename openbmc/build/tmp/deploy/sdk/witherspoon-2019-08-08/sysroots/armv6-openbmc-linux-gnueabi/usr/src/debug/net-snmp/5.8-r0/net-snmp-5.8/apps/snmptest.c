        ...
        else
        {
            ...
            if (status == STAT_SUCCESS)
            {
                ...
                else
                {
                    ...
                    {
                        char* addr_string = transport->f_fmtaddr(
                            transport, response->transport_data,
...
int input_variable(netsnmp_variable_list* vp)
{
    char buf[256];
    size_t val_len;
    u_char ch;
    ...
    if (*buf == '$')
    {
        switch (toupper((unsigned char)(buf[1])))
        {
            ...
            case 'B':
                ...
                printf("Enter a blank line to terminate the list of "
                       "non-repeaters\n");
    ...
    if (command == SNMP_MSG_INFORM || command == SNMP_MSG_TRAP2
#ifndef NETSNMP_NO_WRITE_SUPPORT
        || command == SNMP_MSG_SET
#endif /* NETSNMP_NO_WRITE_SUPPORT */
    )
