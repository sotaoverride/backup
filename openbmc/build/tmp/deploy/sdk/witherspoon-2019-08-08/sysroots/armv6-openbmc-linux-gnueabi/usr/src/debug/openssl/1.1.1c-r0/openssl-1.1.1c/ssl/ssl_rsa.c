...
int SSL_use_RSAPrivateKey_file(SSL* ssl, const char* file, int type)
{
    ...
    {
        j = ERR_R_PEM_LIB;
        rsa = PEM_read_bio_RSAPrivateKey(in, NULL, ssl->default_passwd_callback,
                                         ssl->default_passwd_callback_userdata);
    }
    else
    {
...
int SSL_use_PrivateKey_file(SSL* ssl, const char* file, int type)
{
    ...
    {
        j = ERR_R_PEM_LIB;
        pkey = PEM_read_bio_PrivateKey(in, NULL, ssl->default_passwd_callback,
                                       ssl->default_passwd_callback_userdata);
    }
    ...
    while (PACKET_remaining(&pkt))
    {
        ...
        if (version == SSL_SERVERINFOV1 || context == SYNTHV1CONTEXT)
        {
            if (!SSL_CTX_add_server_custom_ext(
                    ctx, ext_type, serverinfo_srv_add_cb, NULL, NULL,
                    serverinfo_srv_parse_cb, NULL))
                return 0;
        }
        else
        {
                              ...
                              size_t serverinfo_length)
{
     ...
     * register callbacks.
     */
    if (!serverinfo_process_buffer(version, serverinfo, serverinfo_length, ctx))
    {
        SSLerr(SSL_F_SSL_CTX_USE_SERVERINFO_EX, SSL_R_INVALID_SERVERINFO_DATA);
        return 0;
    }
    ...
    for (num_extensions = 0;; num_extensions++)
    {
        ...
        else
        {
            ...
            if (extension_length < 8 ||
                (extension[6] << 8) + extension[7] != extension_length - 8)
            {
                SSLerr(SSL_F_SSL_CTX_USE_SERVERINFO_FILE, SSL_R_BAD_DATA);
                goto end;
            }
        ...
        memcpy(serverinfo + serverinfo_length + contextoff, extension,
               extension_length);
