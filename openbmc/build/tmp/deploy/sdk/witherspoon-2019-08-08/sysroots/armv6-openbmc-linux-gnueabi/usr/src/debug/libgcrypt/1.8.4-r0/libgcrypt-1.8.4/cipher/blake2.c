...
static byte zero_block[BLAKE2B_BLOCKBYTES] = {
    0,
};

static void blake2_write(void* S, const void* inbuf, size_t inlen, byte* tmpbuf,
                         size_t* tmpbuflen, size_t blkbytes,
                         blake2_transform_t transform_fn)
{
    ...
    if (inlen > 0)
    {
        ...
        if (inlen > fill)
        {
            ...
            burn =
                transform_fn(S, tmpbuf, 1); /* Increment counter + Compress */
...
static void blake2b_final(void* ctx)
{
    ...
    if (c->buflen < BLAKE2B_BLOCKBYTES)
        memset(c->buf + c->buflen, 0,
               BLAKE2B_BLOCKBYTES - c->buflen); /* Padding */
...
static void blake2s_final(void* ctx)
{
    ...
    if (c->buflen < BLAKE2S_BLOCKBYTES)
        memset(c->buf + c->buflen, 0,
               BLAKE2S_BLOCKBYTES - c->buflen); /* Padding */
...
static gpg_err_code_t selftests_blake2b(int algo, int extended,
...
static gpg_err_code_t selftests_blake2s(int algo, int extended,
                                        selftest_report_func_t report)
{
...
#define DEFINE_BLAKE2_VARIANT(bs, BS, dbits, oid_branch)                       \
    ...
    static gcry_md_oid_spec_t oid_spec_blake2##bs##_##dbits[] = {              \
        {" 1.3.6.1.4.1.1722.12.2." oid_branch}, {NULL}};                       \
    gcry_md_spec_t _gcry_digest_spec_blake2##bs##_##dbits = {                  \
        GCRY_MD_BLAKE2##BS##_##dbits,                                          \
        {0, 0},                                                                \
        "BLAKE2" #BS "_" #dbits,                                               \
        blake2##bs##_##dbits##_asn,                                            \
        DIM(blake2##bs##_##dbits##_asn),                                       \
        oid_spec_blake2##bs##_##dbits,                                         \
        dbits / 8,                                                             \
        blake2##bs##_##dbits##_init,                                           \
        blake2##bs##_write,                                                    \
        blake2##bs##_final,                                                    \
        blake2##bs##_read,                                                     \
        NULL,                                                                  \
