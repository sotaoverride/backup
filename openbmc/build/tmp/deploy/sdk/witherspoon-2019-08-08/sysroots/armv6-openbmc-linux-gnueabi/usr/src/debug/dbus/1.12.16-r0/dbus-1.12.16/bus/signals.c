...
static dbus_bool_t append_key_and_escaped_value(DBusString* str,
                                                const char* token,
                                                const char* value)
{
    const char* p = value;
...
static char* match_rule_to_string(BusMatchRule* rule)
{
    ...
    if (rule->flags & BUS_MATCH_MESSAGE_TYPE)
    {
        if (!append_key_and_escaped_value(
                &str, "type", dbus_message_type_to_string(rule->message_type)))
            goto nomem;
    }
    ...
    if (rule->flags & BUS_MATCH_DESTINATION)
    {
        ...
        if (!append_key_and_escaped_value(&str, "destination",
                                          rule->destination))
            goto nomem;
    }
    ...
    if (rule->flags & BUS_MATCH_CLIENT_IS_EAVESDROPPING)
    {
        ...
        if (!append_key_and_escaped_value(
                &str, "eavesdrop",
                (rule->flags & BUS_MATCH_CLIENT_IS_EAVESDROPPING) ? "true"
                                                                  : "false"))
            goto nomem;
    }
    ...
    if (rule->flags & BUS_MATCH_ARGS)
    {
        ...
        while (i < rule->args_len)
        {
            if (rule->args[i] != NULL)
            {
                ...
                is_namespace =
                    (rule->arg_lens[i] & BUS_MATCH_ARG_NAMESPACE) != 0;

                if (!_dbus_string_append_printf(
                        &str, "arg%d%s", i,
                        is_path ? "path" : is_namespace ? "namespace" : ""))
                    goto nomem;
...
dbus_bool_t bus_match_rule_set_path(BusMatchRule* rule, const char* path,
...
dbus_bool_t bus_match_rule_set_arg(BusMatchRule* rule, int arg,
                                   const DBusString* value, dbus_bool_t is_path,
                                   dbus_bool_t is_namespace)
{
    int length;
    char* new;
    ...
    if (arg >= rule->args_len)
    {
        ...
        new_arg_lens =
            dbus_realloc(rule->arg_lens, sizeof(int) * (new_args_len + 1));
...
static dbus_bool_t find_key(const DBusString* str, int start, DBusString* key,
                            int* value_pos, DBusError* error)
{
    const char* p;
    const char* s;
...
static dbus_bool_t find_value(const DBusString* str, int start, const char* key,
                              DBusString* value, int* value_end,
                              DBusError* error)
{
    const char* p;
    const char* s;
...
static dbus_bool_t tokenize_rule(const DBusString* rule_text,
...
static dbus_bool_t bus_match_rule_parse_arg_match(BusMatchRule* rule,
                                                  const char* key,
                                                  ...
                                                  DBusError* error)
{
    ...
    {
        dbus_set_error(error, DBUS_ERROR_MATCH_RULE_INVALID,
                       "Key '%s' in match rule starts with 'arg' but lacks an "
                       "arg number. Should be 'arg0' or 'arg7' for example.\n",
                       key);
        goto failed;
    }
    ...
    if (!_dbus_string_parse_uint(&key_str, 3, &arg, &end))
    {
        dbus_set_error(
            ...
            "Key '%s' in match rule starts with 'arg' but could not parse arg "
            "number. Should be 'arg0' or 'arg7' for example.\n",
            key);
        goto failed;
    }
    ...
    if (end != length)
    {
        ...
        {
            dbus_set_error(error, DBUS_ERROR_MATCH_RULE_INVALID,
                           "Key '%s' in match rule contains junk after "
                           "argument number (%lu). Only 'arg%lupath' (for "
                           "example) or 'arg0namespace' are valid",
                           key, arg, arg);
            goto failed;
        }
    }
    ...
    {
        dbus_set_error(error, DBUS_ERROR_MATCH_RULE_INVALID,
                       "Key '%s' in match rule has arg number %lu but the "
                       "maximum is %d.\n",
    ...
    if ((rule->flags & BUS_MATCH_ARGS) && rule->args_len > (int)arg &&
        rule->args[arg] != NULL)
    {
        dbus_set_error(error, DBUS_ERROR_MATCH_RULE_INVALID,
                                   ...
                                   DBusError* error)
{
    ...
    while (tokens[i].key != NULL)
    {
        ...
        {
            dbus_set_error(error, DBUS_ERROR_MATCH_RULE_INVALID,
                           "Unknown key \"%s\" in match rule", key);
            goto failed;
        }
                                ...
                                DBusMessageIter* arr_iter)
{
    ...
    for (i = 0; i < DBUS_NUM_MESSAGE_TYPES; i++)
    {
        ...
        _dbus_hash_iter_init(matchmaker->rules_by_type[i].rules_by_iface,
                             &iter);
        while (_dbus_hash_iter_next(&iter))
        {
            list = _dbus_hash_iter_get_value(&iter);
            for (link = _dbus_list_get_first_link(list); link != NULL;
...
BusMatchmaker* bus_matchmaker_new(void)
{
    ...
    for (i = DBUS_MESSAGE_TYPE_INVALID; i < DBUS_NUM_MESSAGE_TYPES; i++)
    {
        ...
        p->rules_by_iface = _dbus_hash_table_new(
            DBUS_HASH_STRING, dbus_free, (DBusFreeFunction)rule_list_ptr_free);
...
static DBusList** bus_matchmaker_get_rules(BusMatchmaker* matchmaker,
                                           int message_type,
                                           const char* interface,
                                           dbus_bool_t create)
{
...
static void bus_matchmaker_gc_rules(BusMatchmaker* matchmaker, int message_type,
                                    const char* interface, DBusList** rules)
{
    RulePool* p;
...
static void rule_list_remove_by_connection(DBusList** rules,
...
static dbus_bool_t connection_is_primary_owner(DBusConnection* connection,
...
static dbus_bool_t match_rule_matches(BusMatchRule* rule,
                                      ...
                                      BusMatchFlags already_matched)
{
    ...
    if (flags & BUS_MATCH_DESTINATION)
    {
        ...
        if (addressed_recipient == NULL)
        {
             ...
             * the message (for instance because they haven't been activated
             * yet), assume they will own the requested destination name and no
             * other, and match the rule's destination against that.
             */
        ...
        {
            if (!connection_is_primary_owner(addressed_recipient,
                                             rule->destination))
                return FALSE;
        }
    }
    else
    { /* no destination in rule */
        ...
        while (i < rule->args_len)
        {
            ...
            if (expected_arg != NULL)
            {
                ...
                else if (is_namespace)
                {
                    ...
                    {
                        /* Check that the actual argument is within the expected
                         * namespace, rather than just starting with that
                         * string, by checking that the matched prefix ends in a
                         * '.'.
                         *
...
static dbus_bool_t get_recipients_from_list(DBusList** rules,
                                            DBusConnection* sender,
                                            DBusConnection* addressed_recipient,
                                            DBusMessage* message,
                                            DBusList** recipients_p)
{
    ...
    while (link != NULL)
    {
                               ...
                               BUS_MATCH_MESSAGE_TYPE | BUS_MATCH_INTERFACE))
        {
            ...
            {
                _dbus_verbose("Connection already receiving this message, so "
                              "not adding again\n");
            }
        }
                                          ...
                                          DBusList** recipients_p)
{
    ...
    if (interface != NULL)
        just_iface = bus_matchmaker_get_rules(
            matchmaker, DBUS_MESSAGE_TYPE_INVALID, interface, FALSE);
    ...
    if (!(get_recipients_from_list(neither, sender, addressed_recipient,
          ...
          get_recipients_from_list(just_type, sender, addressed_recipient,
                                   message, recipients_p) &&
          get_recipients_from_list(both, sender, addressed_recipient, message,
                                   recipients_p)))
    {
...
static BusMatchRule* check_parse(dbus_bool_t should_succeed, const char* text)
{
    ...
    {
        _dbus_warn("Failed to parse: %s: %s: \"%s\"", error.name, error.message,
...
static dbus_bool_t test_parsing(void* data)
{
    ...
    rule =
        check_parse(TRUE, "type='signal',sender='org.freedesktop.DBusSender',"
                          "interface='org.freedesktop.DBusInterface',member='"
                          "Foo',path='/bar/foo',destination=':452345.34'");
    if (rule != NULL)
    {
    ...
    rule = check_parse(
        TRUE, "    type='signal',  \tsender='org.freedes''ktop.DBusSender',   "
              "interface='org.freedesktop.DBusInterface''''', "
...
static struct
{
    ...
    {"type='method_call',destination=':1.0'",
     "destination=':1.0',type='method_call'"},
    {"type='method_call',path='/foo/bar'",
     "path='/foo/bar',type='method_call'"},
    {"type='method_call',arg0='blah'", "arg0='blah',type='method_call'"},
    {"type='method_call',arg0='boo'", "arg0='boo',type='method_call'"},
    {"type='method_call',arg0='blah',arg1='baz'",
     "arg0='blah',arg1='baz',type='method_call'"},
    ...
    {"arg0='comma,type=comma',type=signal",
     "type=signal,arg0='comma,type=comma'"},
    ...
    while (i < _DBUS_N_ELEMENTS(equality_tests))
    {
        ...
        if (!match_rule_equal(first, second))
        {
            _dbus_warn("rule %s and %s should have been equal",
                       equality_tests[i].first, equality_tests[i].second);
            exit(1);
        }
...
static const char* should_match_message_1[] = {
    ...
    "type='signal',member='Frobated',arg0='foobar'",
    "member='Frobated',arg0='foobar'", "type='signal',arg0='foobar'",
    /* The definition of argXpath matches says: "As with normal argument
     * matches, if the argument is exactly equal to the string given in the
     * match rule then the rule is satisfied." So this should match (even though
     * the argument is not a valid path)!
     */
...
static const char* should_not_match_message_1[] = {
    ...
    "arg0='foobar',arg1='abcdef',arg2='abcdefghi',arg3='abcdefghi',arg4='"
    "abcdefghi'",
    "arg0='foobar',arg1='abcdef',arg4='abcdefghi',arg3='abcdefghi',arg2='"
    "abcdefghi'",
    "arg0path='foo'",
    "arg0path='foobar/'",
    "arg1path='3'",
...
static const char* should_match_message_2[] = {
    ...
    "arg0namespace='com.example.backend'", "arg0namespace='com.example'",
    "arg0namespace='com'",
...
static const char* should_not_match_message_2[] = {
    ...
    "arg0namespace='com.example.backen'", "arg0namespace='com.exampl'",
    "arg0namespace='co'",

    NULL};

static void check_matches(dbus_bool_t expected_to_match, int number,
                          DBusMessage* message, const char* rule_text)
{
    BusMatchRule* rule;
    dbus_bool_t matched;
    ...
    {
        _dbus_warn("Expected rule %s to %s message %d, failed", rule_text,
                   expected_to_match ? "match" : "not match", number);
        exit(1);
    }
...
static void check_matching(DBusMessage* message, int number,
...
static void test_matching(void)
{
    ...
    v_STRING = "foobar";
    v_INT32 = 3;
    if (!dbus_message_append_args(message1, DBUS_TYPE_STRING, &v_STRING,
                                  DBUS_TYPE_INT32, &v_INT32, NULL))
        _dbus_assert_not_reached("oom");

    check_matching(message1, 1, should_match_message_1,
    ...
    check_matching(message2, 2, should_match_message_2,
...
static void test_path_match(int type, const char* path, const char* rule_text,
                            BusMatchRule* rule, dbus_bool_t should_match)
{
    ...
    if (matched != should_match)
    {
        _dbus_warn("Expected rule %s to %s message "
                   ...
                   rule_text, should_match ? "match" : "not match", path,
                   (char)type);
        exit(1);
    }
...
static void test_path_matching(void)
{
    ...
    for (s = paths_that_should_not_be_matched; *s != NULL; s++)
    {
        ...
        test_path_match(DBUS_TYPE_OBJECT_PATH, *s, PATH_MATCH_RULE, rule,
                        FALSE);
    }
...
static const char* path_namespace_should_match_message_1[] = {
    "type='signal',path_namespace='/'", "type='signal',path_namespace='/foo'",
...
static const char* path_namespace_should_not_match_message_1[] = {
    ...
    "type='signal',path_namespace='/bar/TheObjectManager'", NULL};
...
static const char* path_namespace_should_match_message_3[] = {
    "type='signal',path_namespace='/'", NULL};

static const char* path_namespace_should_not_match_message_3[] = {
    "type='signal',path_namespace='/foo/TheObjectManager'", NULL};

static const char* path_namespace_should_match_message_4[] = {
    "type='signal',path_namespace='/'", NULL};
...
static void test_matching_path_namespace(void)
{
    ...
    check_matching(message1, 1, path_namespace_should_match_message_1,
                   path_namespace_should_not_match_message_1);
    check_matching(message2, 2, path_namespace_should_match_message_2,
                   path_namespace_should_not_match_message_2);
    check_matching(message3, 3, path_namespace_should_match_message_3,
                   path_namespace_should_not_match_message_3);
    check_matching(message4, 4, path_namespace_should_match_message_4,
