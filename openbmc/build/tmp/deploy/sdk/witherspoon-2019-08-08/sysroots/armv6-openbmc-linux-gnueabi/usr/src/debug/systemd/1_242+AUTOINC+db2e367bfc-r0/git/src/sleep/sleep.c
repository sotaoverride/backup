...
static int write_hibernate_location_info(void)
{
    ...
    {
        r = write_string_file("/sys/power/resume", device,
    ...
    if (access("/sys/power/resume_offset", W_OK) < 0)
    {
        ...
        {
            log_debug(
                "Kernel too old, can't configure resume offset, ignoring.");
            return 0;
        }
    ...
    r = write_string_file("/sys/power/resume_offset", offset_str,
                          WRITE_STRING_FILE_DISABLE_BUFFER);
    if (r < 0)
    ...
    r = write_string_file("/sys/power/resume", device_str,
                          WRITE_STRING_FILE_DISABLE_BUFFER);
    if (r < 0)
...
static int execute(char** modes, char** states)
{
    ...
    if (!strv_isempty(modes))
    {
        ...
        if (r < 0)
            return log_error_errno(
                r, "Failed to write hibernation disk offset: %m");
        r = write_mode(modes);
        if (r < 0)
            return log_error_errno(
                r, "Failed to write mode to /sys/power/disk: %m");
        ;
    }
    ...
    log_struct(LOG_INFO, "MESSAGE_ID=" SD_MESSAGE_SLEEP_START_STR,
               LOG_MESSAGE("Suspending system..."), "SLEEP=%s", arg_verb);

    r = write_state(&f, states);
    if (r < 0)
        log_struct_errno(
            LOG_ERR, r, "MESSAGE_ID=" SD_MESSAGE_SLEEP_STOP_STR,
            LOG_MESSAGE("Failed to suspend system. System resumed again: %m"),
            "SLEEP=%s", arg_verb);
    else
        log_struct(LOG_INFO, "MESSAGE_ID=" SD_MESSAGE_SLEEP_STOP_STR,
                   LOG_MESSAGE("System resumed."), "SLEEP=%s", arg_verb);
...
static int rtc_write_wake_alarm(uint64_t sec)
{
    ...
    r = write_string_file("/sys/class/rtc/rtc0/wakealarm", buf,
                          WRITE_STRING_FILE_DISABLE_BUFFER);
    if (r < 0)
        return log_error_errno(
            r, "Failed to write '%s' to /sys/class/rtc/rtc0/wakealarm: %m",
            buf);
...
static int execute_s2h(usec_t hibernate_delay_sec)
{
    ...
    _cleanup_strv_free_ char **hibernate_modes = NULL,
                             **hibernate_states = NULL, **suspend_modes = NULL,
                             **suspend_states = NULL;
    usec_t original_time, wake_time, cmp_time;
    int r;

    r = parse_sleep_config("suspend", NULL, &suspend_modes, &suspend_states,
                           NULL);
    if (r < 0)
        return r;

    r = parse_sleep_config("hibernate", NULL, &hibernate_modes,
    ...
    if (cmp_time <
        wake_time) /* We woke up before the alarm time, we are done. */
        return 0;
...
static int parse_argv(int argc, char* argv[])
{
    ...
    if (argc - optind != 1)
        return log_error_errno(SYNTHETIC_ERRNO(EINVAL), "Usage: %s COMMAND",
                               program_invocation_short_name);

    arg_verb = argv[optind];

    if (!STR_IN_SET(arg_verb, "suspend", "hibernate", "hybrid-sleep",
