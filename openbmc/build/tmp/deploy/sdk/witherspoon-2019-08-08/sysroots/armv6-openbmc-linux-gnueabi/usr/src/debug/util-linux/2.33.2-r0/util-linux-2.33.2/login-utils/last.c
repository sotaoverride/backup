...
static struct last_timefmt timefmts[] = {
    ...
    [LAST_TIMEFTM_SHORT] = {.name = "short",
                            .in_len = 16,
                            .out_len = 7,
                            .in_fmt = LAST_TIMEFTM_CTIME,
                            .out_fmt = LAST_TIMEFTM_HHMM},
    [LAST_TIMEFTM_CTIME] = {.name = "full",
                            .in_len = 24,
                            .out_len = 26,
                            .in_fmt = LAST_TIMEFTM_CTIME,
                            .out_fmt = LAST_TIMEFTM_CTIME},
    [LAST_TIMEFTM_ISO8601] = {.name = "iso",
                              .in_len = 25,
                              .out_len = 27,
 ...
 */
static int list(const struct last_control* ctl, struct utmpx* p,
                time_t logout_time, int what)
{
    ...
    /* log-in time */
    if (time_formatter(fmt->in_fmt, logintime, sizeof(logintime), &utmp_time) <
        0)
        errx(EXIT_FAILURE, _("preallocation size exceeded"));

    /* log-out time */
    secs = logout_time -
           utmp_time; /* Under strange circumstances, secs < 0 can happen */
    mins = (secs / 60) % 60;
    hours = (secs / 3600) % 24;
    days = secs / 86400;

    strcpy(logouttime, "- ");
    if (time_formatter(fmt->out_fmt, logouttime + 2, sizeof(logouttime) - 2,
                       &logout_time) < 0)
        errx(EXIT_FAILURE, _("preallocation size exceeded"));
    ...
    {
        sprintf(length, "(%d+%02d:%02d)", days, abs(hours),
                abs(mins)); /* hours and mins always shown as positive (w/o
                               minus sign!) even if secs < 0 */
    }
    else if (hours)
    {
        sprintf(length, " (%02d:%02d)", hours,
                abs(mins)); /* mins always shown as positive (w/o minus sign!)
                               even if secs < 0 */
    }
    else if (secs >= 0)
    {
    ...
    if (ctl->usedns || ctl->useip)
        r = dns_lookup(domain, sizeof(domain), ctl->useip,
                       (int32_t*)p->ut_addr_v6);
    if (r < 0)
    ...
    if (ctl->showhost)
    {
        if (!ctl->altlist)
        {
            len = snprintf(final, sizeof(final),
                           ...
                           ctl->name_len, p->ut_user, utline, ctl->domain_len,
                           domain, fmt->in_len, fmt->in_len, logintime,
                           fmt->out_len, fmt->out_len, logouttime, length);
        }
        else
        {
            len = snprintf(final, sizeof(final),
                           "%-8.*s %-12.12s %-*.*s %-*.*s %-12.12s %s\n",
                           ctl->name_len, p->ut_user, utline, fmt->in_len,
                           fmt->in_len, logintime, fmt->out_len, fmt->out_len,
        ...
        len = snprintf(final, sizeof(final),
                       "%-8.*s %-12.12s %-*.*s %-*.*s %s\n", ctl->name_len,
                       p->ut_user, utline, fmt->in_len, fmt->in_len, logintime,
                       fmt->out_len, fmt->out_len, logouttime, length);
...
static void __attribute__((__noreturn__)) usage(const struct last_control* ctl)
{
    ...
    fputs(_(" -a, --hostlast       display hostnames in the last column\n"),
          out);
    fputs(_(" -d, --dns            translate the IP number back into a "
            "hostname\n"),
          out);
    fprintf(out, _(" -f, --file <file>    use a specific file instead of %s\n"),
            ctl->lastb ? _PATH_BTMP : _PATH_WTMP);
    fputs(_(" -F, --fulltimes      print full login and logout times and "
            "dates\n"),
          out);
    fputs(_(" -i, --ip             display IP numbers in numbers-and-dots "
            "notation\n"),
          out);
    fputs(_(" -n, --limit <number> how many lines to show\n"), out);
    fputs(_(" -R, --nohostname     don't display the hostname field\n"), out);
    fputs(
        _(" -s, --since <time>   display the lines since the specified time\n"),
        out);
    fputs(
        _(" -t, --until <time>   display the lines until the specified time\n"),
        out);
    fputs(_(" -p, --present <time> display who were present at the specified "
            "time\n"),
          out);
    fputs(_(" -w, --fullnames      display full user and domain names\n"), out);
    fputs(_(" -x, --system         display system shutdown entries and run "
            "level changes\n"),
          out);
    fputs(_("     --time-format <format>  show timestamps in the specified "
            "<format>:\n"
                              ...
                              const char* filename)
{
    ...
    if (ctl->time_fmt != LAST_TIMEFTM_NONE)
    {
        ...
        fmt = &timefmts[ctl->time_fmt];
        if (time_formatter(fmt->in_fmt, timestr, sizeof(timestr), &begintime) <
            0)
            errx(EXIT_FAILURE, _("preallocation size exceeded"));
...
{
    struct last_control ctl = {.showhost = TRUE,
    ...
    while ((c = getopt_long(argc, argv, "hVf:n:RxadFit:p:s:0123456789w",
                            long_opts, NULL)) != -1)
    {
        ...
        switch (c)
        {
            ...
            case 'n':
                ctl.maxrecs =
                    strtos32_or_err(optarg, _("failed to parse number"));
                break;
            case 'f':
                if (!files)
