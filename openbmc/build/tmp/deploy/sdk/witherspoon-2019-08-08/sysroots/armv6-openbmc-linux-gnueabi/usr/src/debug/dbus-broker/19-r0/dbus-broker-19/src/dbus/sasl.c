 ...
 * The broker knows the UID of a peer from the peer credentials obtained from
 * its socket, which is used for authenticating. For the purposes of the broker
 * SASL is a no-op needed only for compatibility.
 *
...
static void sasl_split(const char* input, size_t n_input, const char** cmd,
                       size_t* n_cmd, const char** arg, size_t* n_arg)
{
    /*
 ...
 * @sasl:               the client to operate on
 *
 * This deinitializes the SASL client object @sasl. It is safe to call deinit
 * several times.
 */
 ...
 *
 * This dispatches the SASL object, consuming the given input string and
 * providing a pointer to an output string to send, if applicable. Neither input
 * nor output strings are nul-terminated.
 *
 * We hardcode the EXTERNAL mechanism, relying on the UDS providing the
 * credentials, so not attaching them ourselves. Note that we do not necessarily
 * know the UID the remote end will see us as, in the case of crossing a
 * namespace boundary, so providing an UID is not straight-forward. We also
 * hard-code the requirement for unix FD support.
 *
 * If either the EXTERNAL mechanism or FD support is not supported by the remote
 * side, we fail the negotiation, we do not fall back to anything else.
 *
 ...
 *         0 on success.
 */
int sasl_client_dispatch(SASLClient* sasl, const char* input, size_t n_input,
                         const char** outputp, size_t* n_outputp)
{
    static const char request[] = {"\0AUTH EXTERNAL\r\n"
                                   "DATA\r\n"
                                   "NEGOTIATE_UNIX_FD\r\n"
                                   "BEGIN"};
    ...
    switch (sasl->state)
    {
        ...
        case SASL_CLIENT_STATE_UNIX_FD:
            ...
            if (n_cmd != strlen("AGREE_UNIX_FD") ||
                strncmp(cmd, "AGREE_UNIX_FD", n_cmd))
                return SASL_E_FAILURE;
            if (n_arg)
                return SASL_E_PROTOCOL_VIOLATION;
 ...
 * @sasl:               the server to operate on
 *
 * This deinitializes the SASL server object @sasl. It is safe to call deinit
 * several times.
 */
...
static void sasl_server_handle_data(SASLServer* sasl, const char* input,
                                    size_t n_input, const char** outputp,
                                    size_t* n_outputp)
{
...
static void sasl_server_handle_auth(SASLServer* sasl, const char* input,
                                    size_t n_input, const char** outputp,
                                    size_t* n_outputp)
{
    ...
    if (n_protocol == strlen("EXTERNAL") &&
        !strncmp(protocol, "EXTERNAL", n_protocol))
    {
        if (n_arg)
            return sasl_server_handle_data(sasl, arg, n_arg, outputp,
                                           n_outputp);
 ...
 * nor output strings are nul-terminated.
 *
 * Only the EXTERNAL mechanism is supported, so if the client tries to use
 * anything else, we reject that and inform them that only EXTERNAL is
 * supported. If the client provides their UID, then we verify that this is as
 * expected, but if it is ommitted, we use the expected one.
 *
 * We will optionally allow unix FD support to be negotiated if the client
 * requests that.
 *
 * Note that even though each output string has a fixed size, the server will
 * potentially produce an unbounded amount of output over all, as it will only
 * terminate the exchange in case it succeeded or the protocol was violated.
 *
 * Return: SASL_E_PROTOCOL_VIOLATION if the client violated the protocol, or 0
 * on success.
 */
int sasl_server_dispatch(SASLServer* sasl, const char* input, size_t n_input,
                         const char** outputp, size_t* n_outputp)
{
    const char *cmd, *arg;
    size_t n_cmd, n_arg;
    ...
    switch (sasl->state)
    {
        ...
        case SASL_SERVER_STATE_AUTH:
            ...
            else if (n_cmd == strlen("BEGIN") &&
                     !strncmp(cmd, "BEGIN", n_cmd) && !n_arg)
            {
                return SASL_E_PROTOCOL_VIOLATION;
            }
            else
        ...
        case SASL_SERVER_STATE_CHALLENGE:
            ...
            else if ((n_cmd == strlen("ERROR") &&
                      !strncmp(cmd, "ERROR", n_cmd)) ||
                     (n_cmd == strlen("CANCEL") &&
                      !strncmp(cmd, "CANCEL", n_cmd) && !n_arg))
            {
                *outputp = "REJECTED EXTERNAL";
                *n_outputp = strlen("REJECTED EXTERNAL");
                sasl->state = SASL_SERVER_STATE_AUTH;
        ...
        case SASL_SERVER_STATE_NEGOTIATED_FDS:
            if (n_cmd == strlen("NEGOTIATE_UNIX_FD") &&
                !strncmp(cmd, "NEGOTIATE_UNIX_FD", n_cmd) && !n_arg)
            {
