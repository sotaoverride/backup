...
netsnmp_feature_child_of(usm_support, libnetsnmp)
     ...
     * algorithm given in RFC 2274 concerning the SNMPv3 User Security Model
     *(USM) as follows:
     *
     ...
     */
    int generate_Ku(const oid* hashtype, u_int hashtype_len, const u_char* P,
                    size_t pplen, u_char* Ku, size_t* kulen)
    ...
    defined(NETSNMP_USE_INTERNAL_CRYPTO)
{
    int rval = SNMPERR_SUCCESS, nbytes = USM_LENGTH_EXPANDED_PASSPHRASE,
        auth_type;
#if !defined(NETSNMP_USE_OPENSSL) && defined(NETSNMP_USE_INTERNAL_MD5) ||      \
    defined(NETSNMP_USE_INTERNAL_CRYPTO)
    int ret;
#endif
...
int generate_kul(const oid* hashtype, u_int hashtype_len,
                 const u_char* engineID, size_t engineID_len, const u_char* Ku,
                 size_t ku_len, u_char* Kul, size_t* kul_len)
#if defined(NETSNMP_USE_OPENSSL) || defined(NETSNMP_USE_INTERNAL_MD5) ||       \
...
static int _kul_extend_blumenthal(int needKeyLen, oid* hashoid,
                                  u_int hashoid_len, u_char* origKul,
                                  size_t* origKulLen, int origKulSize)
{
...
static int _kul_extend_reeder(int needKeyLen, oid* hashoid, u_int hashoid_len,
                              u_char* engineID, int engineIDLen,
                              u_char* origKul, size_t* origKulLen,
                              int origKulSize)
{
    ...
    while (needKeyLen > 0)
    {
         ...
         * [ Ku' = Ku(kul) ]
         */
        if (generate_Ku(hashoid, hashoid_len, origKul, *origKulLen, newKu,
                        &newKuLen) != SNMPERR_SUCCESS)
            return SNMPERR_GENERR;
         ...
         */
        newKulLen = sizeof(newKul);
        if (generate_kul(hashoid, hashoid_len, engineID, engineIDLen, newKu,
                         newKuLen, newKul, &newKulLen) != SNMPERR_SUCCESS)
            return SNMPERR_GENERR;
                       ...
                       u_char** kulBuf, size_t* kulBufLen, u_int kulBufSize)
{
    ...
    {
        int i;
        DEBUGMSG(("9:usm:extend_kul", "key: key=0x"));
    ...
    switch (privType & (USM_PRIV_MASK_ALG | USM_PRIV_MASK_VARIANT))
    {
        ...
        case USM_CREATE_USER_PRIV_AES256:
        {
            ...
            if (reeder)
                ret = _kul_extend_reeder(needKeyLen, hashoid, hashoid_len,
                                         engineID, engineIDLen, newKul,
                                         &newKulLen, kulBufSize);
            else
        ...
        case USM_CREATE_USER_PRIV_3DES:
            ret =
                _kul_extend_reeder(needKeyLen, hashoid, hashoid_len, engineID,
                                   engineIDLen, newKul, &newKulLen, kulBufSize);
            break;
#endif
        default:
            DEBUGMSGTL(
                ("usm:extend_kul", "unknown priv type 0x%x\n", privType));
            ret = SNMPERR_GENERR;
    }
    ...
    {
        int i;
        DEBUGMSG(("usm:extend_kul", "key: key=0x"));
        for (i = 0; i < newKulLen; i++)
 ...
 *
 */
int encode_keychange(const oid* hashtype, u_int hashtype_len, u_char* oldkey,
                     size_t oldkey_len, u_char* newkey, size_t newkey_len,
    ...
    defined(NETSNMP_USE_PKCS11) || defined(NETSNMP_USE_INTERNAL_CRYPTO)
{
    ...
    DEBUGMSGTL(("encode_keychange",
                "oldkey_len %" NETSNMP_PRIz "d, newkey_len %" NETSNMP_PRIz
                "d, auth_length %" NETSNMP_PRIz "d\n",
    ...
    {
        int i;
        DEBUGMSG(("encode_keychange", "rand: key=0x"));
    ...
    while (delta_len < newkey_len)
    {
        ...
        rval = sc_hash(hashtype, hashtype_len, tmpbuf, tmp_len, digest,
                       &digest_len);
        QUITFUN(rval, encode_keychange_quit);
        ...
        DEBUGMSGTL(
            ("encode_keychange", "delta_len %" NETSNMP_PRIz "d\n", delta_len));
    }
 ...
 */
int decode_keychange(const oid* hashtype, u_int hashtype_len, u_char* oldkey,
                     size_t oldkey_len, u_char* kcstring, size_t kcstring_len,
    ...
    defined(NETSNMP_USE_PKCS11) || defined(NETSNMP_USE_INTERNAL_CRYPTO)
{
    ...
    DEBUGMSGTL(("decode_keychange",
                "oldkey_len %" NETSNMP_PRIz "d, newkey_len %" NETSNMP_PRIz
                "d, kcstring_len %" NETSNMP_PRIz "d, hash_len %" NETSNMP_PRIz
                "d\n",
    ...
    *newkey_len = oldkey_len;
    ...
    {
        int i;
        DEBUGMSG(("decode_keychange", "oldkey: key=0x"));
        for (i = 0; i < oldkey_len; i++)
            DEBUGMSG(("decode_keychange", "%02x", oldkey[i] & 0xff));
        DEBUGMSG(("decode_keychange", " (%ld)\n", oldkey_len));

        DEBUGMSG(("decode_keychange", "kcstring: key=0x"));
        for (i = 0; i < kcstring_len; i++)
    ...
    while (delta_len < key_len)
    {
         ...
         */
        DEBUGMSGTL(("decode_keychange",
                    "append random tmpbuf_len %" NETSNMP_PRIz
        ...
        rval = sc_hash(hashtype, hashtype_len, tmpbuf, tmpbuf_len, hash,
                       &hash_len);
        QUITFUN(rval, decode_keychange_quit);
        if (hash_len > key_len)
        {
            DEBUGMSGTL(("decode_keychange", "truncating hash to key_len\n"));
            hash_len = key_len;
        }
         ...
         */
        DEBUGMSGTL(("decode_keychange",
                    "copy %" NETSNMP_PRIz "d hash bytes to tmp\n", hash_len));
         ...
         */
        DEBUGMSGTL(("decode_keychange",
                    "xor to get new key; hash_len %" NETSNMP_PRIz
                    "d delta_len %" NETSNMP_PRIz "d\n",
                    hash_len, delta_len));
        nbytes = 0;
    ...
    {
        int i;
        DEBUGMSG(("decode_keychange", "newkey: key=0x"));
        for (i = 0; i < *newkey_len; i++)
