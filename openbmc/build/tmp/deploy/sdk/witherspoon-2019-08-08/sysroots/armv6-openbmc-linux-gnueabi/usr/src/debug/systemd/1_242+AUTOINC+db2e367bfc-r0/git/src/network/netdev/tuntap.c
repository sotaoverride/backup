...
static int netdev_tuntap_add(NetDev* netdev, struct ifreq* ifr)
{
    ...
    if (fd < 0)
        return log_netdev_error_errno(netdev, -errno,
...
static int tuntap_verify(NetDev* netdev, const char* filename)
{
    ...
    if (netdev->mtu != 0)
        log_netdev_warning(netdev, "MTU configured for %s, ignoring",
                           netdev_kind_to_string(netdev->kind));

    if (netdev->mac)
        log_netdev_warning(netdev, "MAC configured for %s, ignoring",
                           netdev_kind_to_string(netdev->kind));
