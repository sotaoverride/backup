...
static uint32_t none_stat_compr_blocks = 0, none_stat_decompr_blocks = 0,
                none_stat_compr_size = 0;
                                     ...
                                     uint32_t buf_size)
{
    ...
    for (i = buf_size; i > cdatalen; i--)
    {
        ...
        {
            fprintf(stderr,
                    "COMPR_ERROR: buffer overflow at %s. "
                    "(bs=%d csize=%d b[%d]=%d)\n",
                    compr->name, buf_size, cdatalen, i, (int)(output_buf[i]));
            jffs2_error_cnt++;
            return;
        }
    }
    ...
    if (!jffs2_compression_check_buf)
    {
        ...
        if (!jffs2_compression_check_buf)
        {
            fprintf(stderr, "No memory for buffer allocation. Compression "
                            "check disabled.\n");
    ...
    if (!compr->decompress)
    {
        fprintf(
            stderr,
            "JFFS2 compression check: there is no decompress function at %s.\n",
            compr->name);
        jffs2_error_cnt++;
        return;
    }
    if (compr->decompress(output_buf, jffs2_compression_check_buf, cdatalen,
                          datalen))
    {
        fprintf(stderr,
                "JFFS2 compression check: decompression failed at %s.\n",
                compr->name);
        jffs2_error_cnt++;
    }
    ...
    else
    {
        for (i = 0; i < datalen; i++)
        {
            if (data_in[i] != jffs2_compression_check_buf[i])
            {
                fprintf(
                    stderr,
                    "JFFS2 compression check: data mismatch at %s (pos %d).\n",
                    compr->name, i);
                jffs2_error_cnt++;
                break;
            }
...
static int jffs2_is_best_compression(struct jffs2_compressor* this,
                                     struct jffs2_compressor* best,
                                     uint32_t size, uint32_t bestsize)
{
                        ...
                        uint32_t* datalen, uint32_t* cdatalen)
{
    ...
    switch (jffs2_compression_mode)
    {
        ...
        case JFFS2_COMPR_MODE_PRIORITY:
            ...
            if (!output_buf)
            {
                fprintf(stderr, "mkfs.jffs2: No memory for compressor "
                                "allocation. Compression failed.\n");
                goto out;
            }
            list_for_each_entry(this, &jffs2_compressor_list, list)
            {
                /* Skip decompress-only backwards-compatibility and disabled
                 * modules */
                ...
                if (jffs2_compression_check) /*preparing output buffer for
                ...
                *cdatalen = orig_dlen;
                compr_ret =
                    this->compress(data_in, output_buf, datalen, cdatalen);
                this->usecount--;
                if (!compr_ret)
                {
                    ret = this->compr;
                    ...
                    if (jffs2_compression_check)
                        jffs2_decompression_test(this, data_in, output_buf,
                                                 *cdatalen, *datalen,
                                                 orig_dlen);
                    break;
                }
        ...
        case JFFS2_COMPR_MODE_SIZE:
            ...
            list_for_each_entry(this, &jffs2_compressor_list, list)
            {
                ...
                if ((this->compr_buf_size < needed_buf_size) &&
                    (this->compr_buf))
                {
                    free(this->compr_buf);
                ...
                if (!this->compr_buf)
                {
                    ...
                    {
                        fprintf(stderr,
                                "mkfs.jffs2: No memory for compressor "
                                "allocation. (%d bytes)\n",
                                orig_dlen);
                        continue;
                    }
                ...
                if (jffs2_compression_check) /*preparing output buffer for
                                                testing buffer overflow */
                    jffs2_decompression_test_prepare(this->compr_buf,
                                                     this->compr_buf_size);
                *datalen = orig_slen;
                *cdatalen = orig_dlen;
                compr_ret =
                ...
                {
                    if (jffs2_compression_check)
                        jffs2_decompression_test(this, data_in, this->compr_buf,
                                                 *cdatalen, *datalen,
                                                 this->compr_buf_size);
                    if (((!best_dlen) ||
                         jffs2_is_best_compression(this, best, *cdatalen,
                                                   best_dlen)) &&
                        (*cdatalen < *datalen))
                    {
                        best_dlen = *cdatalen;
...
int jffs2_unregister_compressor(struct jffs2_compressor* comp)
{
    ...
    {
        fprintf(stderr,
                "mkfs.jffs2: Compressor modul is in use. Unregister failed.\n");
        return -1;
    }
    list_del(&comp->list);
...
char* jffs2_list_compressors(void)
{
    ...
    {
        act_buf +=
            sprintf(act_buf, "%10s priority:%d ", this->name, this->priority);
...
char* jffs2_stats(void)
{
    ...
    {
        act_buf +=
            sprintf(act_buf, "%10s (prio:%d) ", this->name, this->priority);
        if ((this->disabled) || (!this->compress))
            act_buf += sprintf(act_buf, "- ");
        else
            act_buf += sprintf(act_buf, "+ ");
        act_buf +=
            sprintf(act_buf, "compr: %d blocks (%d/%d)  decompr: %d blocks ",
                    this->stat_compr_blocks, this->stat_compr_new_size,
                    this->stat_compr_orig_size, this->stat_decompr_blocks);
        act_buf += sprintf(act_buf, "\n");
    }
    return buf;
}
