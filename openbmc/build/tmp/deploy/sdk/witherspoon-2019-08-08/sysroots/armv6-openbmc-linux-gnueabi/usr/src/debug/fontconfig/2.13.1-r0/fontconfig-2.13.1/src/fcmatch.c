...
static double FcCompareNumber(const FcValue* value1, const FcValue* value2,
                              FcValue* bestValue)
{
    double v1, v2, v;
...
static double FcCompareString(const FcValue* v1, const FcValue* v2,
                              FcValue* bestValue)
{
    *bestValue = FcValueCanonicalize(v2);
    return (double)FcStrCmpIgnoreCase(FcValueString(v1), FcValueString(v2)) !=
           0;
}

static double FcCompareFamily(const FcValue* v1, const FcValue* v2,
                              FcValue* bestValue)
{
    ...
    *bestValue = FcValueCanonicalize(v2);
    ...
    if (FcToLower(*v1_string) != FcToLower(*v2_string) && *v1_string != ' ' &&
        *v2_string != ' ')
        return 1.0;
...
static double FcComparePostScript(const FcValue* v1, const FcValue* v2,
                                  FcValue* bestValue)
{
    const FcChar8* v1_string = FcValueString(v1);
    const FcChar8* v2_string = FcValueString(v2);
    ...
    *bestValue = FcValueCanonicalize(v2);
    ...
    n = FcStrMatchIgnoreCaseAndDelims(v1_string, v2_string,
                                      (const FcChar8*)" -");
...
static double FcCompareLang(const FcValue* v1, const FcValue* v2,
                            FcValue* bestValue)
{
    FcLangResult result;
...
static double FcCompareBool(const FcValue* v1, const FcValue* v2,
                            FcValue* bestValue)
{
...
static double FcCompareCharSet(const FcValue* v1, const FcValue* v2,
                               FcValue* bestValue)
{
...
static double FcCompareRange(const FcValue* v1, const FcValue* v2,
                             FcValue* bestValue)
{
...
static double FcCompareSize(const FcValue* v1, const FcValue* v2,
                            FcValue* bestValue)
{
    FcValue value1 = FcValueCanonicalize(v1);
    FcValue value2 = FcValueCanonicalize(v2);
...
static double FcCompareFilename(const FcValue* v1, const FcValue* v2,
                                FcValue* bestValue)
{
...
#define PRI_NULL(n) PRI_##n##_STRONG = -1, PRI_##n##_WEAK = -1,
#define PRI1(n)
...
#undef PRI1
#define PRI1(n) PRI_##n, PRI_##n##_STRONG = PRI_##n, PRI_##n##_WEAK = PRI_##n
...
#define FC_OBJECT(NAME, Type, Cmp)                                             \
    {FC_##NAME##_OBJECT, Cmp, PRI_##NAME##_STRONG, PRI_##NAME##_WEAK},
...
static const FcMatcher* FcObjectToMatcher(FcObject object, FcBool include_lang)
{
    if (include_lang)
    {
        switch (object)
    ...
    if (object > FC_MAX_BASE_OBJECT || !_FcMatchers[object].compare ||
        _FcMatchers[object].strong == -1 || _FcMatchers[object].weak == -1)
        return NULL;

    return _FcMatchers + object;
}

static FcBool FcCompareValueList(FcObject object, const FcMatcher* match,
                                 FcValueListPtr v1orig, /* pattern */
                                 FcValueListPtr v2orig, /* target */
                                 FcValue* bestValue, double* value, int* n,
                                 FcResult* result)
{
...
static FcBool FcCompare(FcPattern* pat, FcPattern* fnt, double* value,
                        FcResult* result)
{
    int i, i1, i2;
    ...
    while (i1 < pat->num && i2 < fnt->num)
    {
        ...
        else
        {
            ...
            if (!FcCompareValueList(
                    elt_i1->object, match, FcPatternEltValues(elt_i1),
                    FcPatternEltValues(elt_i2), NULL, value, NULL, result))
                return FcFalse;
            i1++;
            i2++;
        }
                               ...
                               FcPattern* font)
{
    ...
    for (i = 0; i < font->num; i++)
    {
        ...
        if (fe->object == FC_FAMILY_OBJECT || fe->object == FC_STYLE_OBJECT ||
            fe->object == FC_FULLNAME_OBJECT)
        {
            ...
            FC_ASSERT_STATIC((FC_FULLNAME_OBJECT + 1) ==
            ...
            if (fel && pel)
            {
                /* The font has name languages, and pattern asks for specific
                 * language(s). Match on language and and prefer that result.
                 * Note:  Currently the code only give priority to first
                 * matching language.
                 */
                int n = 1, j;
                ...
                if (!FcCompareValueList(
                        pel->object, match, FcPatternEltValues(pel),
                ...
                for (j = 0, l1 = FcPatternEltValues(fe),
                    l2 = FcPatternEltValues(fel);
                     l1 != NULL || l2 != NULL;
                     j++, l1 = l1 ? FcValueListNext(l1) : NULL,
                    l2 = l2 ? FcValueListNext(l2) : NULL)
                {
                    if (j == n)
                    {
                        if (l1)
                            ln = FcValueListPrepend(
                                ln, FcValueCanonicalize(&l1->value),
                                FcValueBindingStrong);
                        if (l2)
                            ll = FcValueListPrepend(
                                ll, FcValueCanonicalize(&l2->value),
                                FcValueBindingStrong);
                    }
                    else
                    {
                        if (l1)
                            ln = FcValueListAppend(
                                ln, FcValueCanonicalize(&l1->value),
                                FcValueBindingStrong);
                        if (l2)
                            ll = FcValueListAppend(
                                ll, FcValueCanonicalize(&l2->value),
                                FcValueBindingStrong);
                    }
                }
            ...
            {
                /* Pattern doesn't ask for specific language.  Copy all for name
                 * and lang. */
        ...
        if (pe)
        {
            ...
            if (!FcCompareValueList(pe->object, match, FcPatternEltValues(pe),
                                    FcPatternEltValues(fe), &v, NULL, NULL,
                                    &result))
            {
                FcPatternDestroy(new);
                return NULL;
             ...
             */
            if (variable && FcPatternEltValues(fe)->value.type == FcTypeRange &&
                (fe->object == FC_WEIGHT_OBJECT ||
                 fe->object == FC_WIDTH_OBJECT || fe->object == FC_SIZE_OBJECT))
            {
                double num;
                FcChar8 temp[128];
                const char* tag = "    ";
    ...
    for (i = 0; i < pat->num; i++)
    {
        ...
        if (!fe && pe->object != FC_FAMILYLANG_OBJECT &&
    ...
    {
        FcChar8* vars = NULL;
        if (FcPatternObjectGetString(new, FC_FONT_VARIATIONS_OBJECT, 0,
                                     &vars) == FcResultMatch)
        {
            FcStrBufChar(&variations, ',');
...
static FcPattern* FcFontSetMatchInternal(FcFontSet** sets, int nsets,
...
static int FcSortCompare(const void* aa, const void* ab)
{
...
static FcBool FcSortWalk(FcSortNode** n, int nnode, FcFontSet* fs,
                         FcCharSet** csp, FcBool trim)
{
    FcBool ret = FcFalse;
    FcCharSet* cs;
    int i;
...
FcFontSet* FcFontSetSort(FcConfig* config FC_UNUSED, FcFontSet** sets,
                         int nsets, FcPattern* p, FcBool trim, FcCharSet** csp,
                         FcResult* result)
{
    FcFontSet* ret;
    FcFontSet* s;
    ...
    qsort(nodeps, nnodes, sizeof(FcSortNode*), FcSortCompare);
    ...
    for (f = 0; f < nnodes; f++)
    {
        ...
        if (nodeps[f]->score[PRI_LANG] < 2000)
        {
            for (i = 0; i < nPatternLang; i++)
            {
                ...
                if (!patternLangSat[i] &&
                    FcPatternGet(p, FC_LANG, i, &patternLang) ==
                        FcResultMatch &&
                    FcPatternGet(nodeps[f]->pattern, FC_LANG, 0, &nodeLang) ==
                        FcResultMatch)
                {
                    FcValue matchValue;
                    double compare =
                        FcCompareLang(&patternLang, &nodeLang, &matchValue);
                    if (compare >= 0 && compare < 2)
                    {
                        if (FcDebug() & FC_DBG_MATCHV)
                        {
                            ...
                            if (FcPatternGetString(nodeps[f]->pattern,
                                                   FC_FAMILY, 0,
                                                   &family) == FcResultMatch &&
                                FcPatternGetString(nodeps[f]->pattern, FC_STYLE,
