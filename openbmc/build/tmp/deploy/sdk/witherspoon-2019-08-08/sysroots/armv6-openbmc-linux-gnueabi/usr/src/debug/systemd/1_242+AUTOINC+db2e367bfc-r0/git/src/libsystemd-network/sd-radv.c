...
_public_ int sd_radv_attach_event(sd_radv* ra, sd_event* event,
                                  int64_t priority)
{
    int r;
...
static void radv_reset(sd_radv* ra)
{
    ...
    ra->recv_event_source = sd_event_source_unref(ra->recv_event_source);
    ...
    LIST_FOREACH(prefix, p, ra->prefixes)
    {
        if (p->valid_until)
        {
            ...
            if (time_now > p->valid_until)
                ...
                p->opt.valid_lifetime =
                    htobe32((p->valid_until - time_now) / USEC_PER_SEC);

            if (time_now > p->preferred_until)
                p->opt.preferred_lifetime = 0;
            else
                p->opt.preferred_lifetime =
                    htobe32((p->preferred_until - time_now) / USEC_PER_SEC);
        }
...
static int radv_recv(sd_event_source* s, int fd, uint32_t revents,
                     void* userdata)
{
    sd_radv* ra = userdata;
    ...
    if (r < 0)
    {
        ...
        {
            case -EADDRNOTAVAIL:
                (void)in_addr_to_string(AF_INET6, (union in_addr_union*)&src,
                                        &addr);
                log_radv("Received RS from non-link-local address %s. Ignoring",
                         addr);
                break;
            ...
            case -EPFNOSUPPORT:
                log_radv("Received invalid source address from ICMPv6 socket. "
                         "Ignoring.");
                break;
            ...
            default:
                log_radv_errno(
                    r, "Unexpected error receiving from ICMPv6 socket: %m");
                break;
        }
    ...
    if (r < 0)
        log_radv_errno(
            r, "Unable to send solicited Router Advertisement to %s: %m",
            strnull(addr));
    else
...
static int radv_timeout(sd_event_source* s, uint64_t usec, void* userdata)
{
    ...
    log_radv("Next Router Advertisement in %s",
             format_timespan(time_string, FORMAT_TIMESPAN_MAX, timeout,
                             USEC_PER_SEC));

    r = event_reset_time(ra->event, &ra->timeout_event_source,
                         clock_boottime_or_monotonic(), time_now + timeout,
                         MSEC_PER_SEC, radv_timeout, ra, ra->event_priority,
                         "radv-timeout", true);
    if (r < 0)
        goto fail;
...
_public_ int sd_radv_stop(sd_radv* ra)
{
    ...
    if (r < 0)
        log_radv_errno(r, "Unable to send last Router Advertisement with "
                          "router lifetime set to zero: %m");
...
_public_ int sd_radv_start(sd_radv* ra)
{
    ...
    r = event_reset_time(ra->event, &ra->timeout_event_source,
                         clock_boottime_or_monotonic(), 0, 0, radv_timeout, ra,
    ...
    r = sd_event_add_io(ra->event, &ra->recv_event_source, ra->fd, EPOLLIN,
                        radv_recv, ra);
    if (r < 0)
        goto fail;
    ...
    (void)sd_event_source_set_description(ra->recv_event_source,
...
_public_ int sd_radv_set_preference(sd_radv* ra, unsigned preference)
{
    ...
    assert_return(ra, -EINVAL);
    assert_return(IN_SET(preference, SD_NDISC_PREFERENCE_LOW,
                         SD_NDISC_PREFERENCE_MEDIUM, SD_NDISC_PREFERENCE_HIGH),
                  -EINVAL);
...
_public_ int sd_radv_add_prefix(sd_radv* ra, sd_radv_prefix* p, int dynamic)
{
    ...
    LIST_FOREACH(prefix, cur, ra->prefixes)
    {
        ...
        r = in_addr_prefix_intersect(
            AF_INET6, (union in_addr_union*)&cur->opt.in6_addr,
            cur->opt.prefixlen, (union in_addr_union*)&p->opt.in6_addr,
            p->opt.prefixlen);
        if (r > 0)
        {
            ...
            (void)in_addr_to_string(
                AF_INET6, (union in_addr_union*)&p->opt.in6_addr, &addr_p);

            if (dynamic && cur->opt.prefixlen == p->opt.prefixlen)
                goto update;

            (void)in_addr_to_string(
                AF_INET6, (union in_addr_union*)&cur->opt.in6_addr, &addr_cur);
            log_radv("IPv6 prefix %s/%u already configured, ignoring %s/%u",
                     addr_cur, cur->opt.prefixlen, addr_p, p->opt.prefixlen);
    ...
    (void)in_addr_to_string(AF_INET6, (union in_addr_union*)&p->opt.in6_addr,
                            &addr_p);
...
update:
    ...
    log_radv("%s prefix %s/%u preferred %s valid %s", cur ? "Updated" : "Added",
             addr_p, p->opt.prefixlen,
             format_timespan(time_string_preferred, FORMAT_TIMESPAN_MAX,
                             preferred, USEC_PER_SEC),
             format_timespan(time_string_valid, FORMAT_TIMESPAN_MAX, valid,
                             USEC_PER_SEC));
                                               ...
                                               unsigned char prefixlen)
{
    ...
    LIST_FOREACH_SAFE(prefix, cur, next, ra->prefixes)
    {
        ...
        if (!in_addr_equal(AF_INET6, (union in_addr_union*)prefix,
...
_public_ int sd_radv_prefix_set_prefix(sd_radv_prefix* p,
...
_public_ int
    sd_radv_prefix_set_address_autoconfiguration(sd_radv_prefix* p,
