...
void shutdown_snmpNotifyFilterProfileTable_data(void)
{
    ...
    DEBUGMSGTL(
        ("trap:notifyFilterProfile:shutdown", "active count %d\n", _active));
    if (_active != 0)
    {
 ...
 *
 */
struct snmpNotifyFilterProfileTable_data* snmpNotifyFilterProfileTable_create(
    const char* paramsName, size_t paramsName_len, const char* profileName,
    size_t profileName_len)
{
    ...
    {
        DEBUGMSGTL(
            ("snmpNotifyFilterProfileTable", "bad params or profile name\n"));
        return NULL;
    }
 ...
 */
int snmpNotifyFilterProfileTable_add(
    struct snmpNotifyFilterProfileTable_data* thedata)
{
    ...
    snmpNotifyFilterProfileTable_find(const char* name, size_t len)
{
     ...
     */
    data = (struct snmpNotifyFilterProfileTable_data*)header_complex_get(
        snmpNotifyFilterProfileTableStorage, vars);
...
struct snmpNotifyFilterProfileTable_data*
    snmpNotifyFilterProfileTable_oldapi_find(struct variable* vp, oid* name,
                                             size_t* nameLen, int exact,
                                             size_t* var_len,
                                             WriteMethod** write_method)
{
    return header_complex(
        (struct header_complex_index*)snmpNotifyFilterProfileTableStorage, vp,
        name, nameLen, exact, var_len, write_method);
}
         ...
         hcindex = hcindex->next)
    {
            ...
            (StorageTmp->snmpNotifyFilterProfileStorType == ST_PERMANENT))
        {
            ...
            cptr = read_config_store_data(
                ASN_OCTET_STR, cptr, &StorageTmp->snmpNotifyFilterProfileName,
                &StorageTmp->snmpNotifyFilterProfileNameLen);
            cptr = read_config_store_data(
                ASN_INTEGER, cptr, &StorageTmp->snmpNotifyFilterProfileStorType,
                NULL);
            cptr = read_config_store_data(
                ASN_INTEGER, cptr,
                &StorageTmp->snmpNotifyFilterProfileRowStatus, NULL);
