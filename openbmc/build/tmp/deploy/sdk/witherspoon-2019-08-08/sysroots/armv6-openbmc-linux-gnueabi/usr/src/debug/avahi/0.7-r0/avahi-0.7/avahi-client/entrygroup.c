...
void avahi_entry_group_set_state(AvahiEntryGroup* group,
...
static int retrieve_state(AvahiEntryGroup* group)
{
    ...
    if (!(message = dbus_message_new_method_call(
              AVAHI_DBUS_NAME, group->path, AVAHI_DBUS_INTERFACE_ENTRY_GROUP,
              "GetState")))
    {
...
static int entry_group_simple_method_call(AvahiEntryGroup* group,
                                          const char* method)
{
    DBusMessage *message = NULL, *reply = NULL;
    DBusError error;
...
int avahi_entry_group_is_empty(AvahiEntryGroup* group)
{
    ...
    if (!dbus_message_get_args(reply, &error, DBUS_TYPE_BOOLEAN, &b,
                               DBUS_TYPE_INVALID) ||
        dbus_error_is_set(&error))
    {
        r = avahi_client_set_errno(client, AVAHI_ERR_DBUS_ERROR);
...
static int append_rdata(DBusMessage* message, const void* rdata, size_t size)
{
    ...
    if (!(dbus_message_iter_open_container(&iter, DBUS_TYPE_ARRAY,
        ...
        !(dbus_message_iter_append_fixed_array(&sub, DBUS_TYPE_BYTE, &rdata,
                                               size)) ||
...
static int append_string_list(DBusMessage* message, AvahiStringList* txt)
{
    ...
    /* Assemble the AvahiStringList into an Array of Array of Bytes to send over
     * dbus */
    for (p = txt; p != NULL; p = p->next)
    {
        ...
        if (!(dbus_message_iter_open_container(&sub, DBUS_TYPE_ARRAY, "y",
                                               &sub2)) ||
            !(dbus_message_iter_append_fixed_array(&sub2, DBUS_TYPE_BYTE, &data,
                                                   p->size)) ||
...
int avahi_entry_group_add_service_strlst(
    ...
    const char* domain, const char* host, uint16_t port, AvahiStringList* txt)
{
    ...
    if (!dbus_message_append_args(
            ...
            &i_protocol, DBUS_TYPE_UINT32, &u_flags, DBUS_TYPE_STRING, &name,
                                  ...
                                  const char* host, uint16_t port, ...)
{
    ...
    r = avahi_entry_group_add_service_strlst(
        group, interface, protocol, flags, name, type, domain, host, port, txt);
...
int avahi_entry_group_update_service_txt(AvahiEntryGroup* group,
                                         AvahiIfIndex interface,
                                         AvahiProtocol protocol,
                                         ...
                                         const char* domain, ...)
{
    ...
    va_start(va, domain);
    txt = avahi_string_list_new_va(va);
    r = avahi_entry_group_update_service_txt_strlst(
        group, interface, protocol, flags, name, type, domain, txt);
    avahi_string_list_free(txt);
    va_end(va);
    return r;
}
...
int avahi_entry_group_update_service_txt_strlst(
    AvahiEntryGroup* group, AvahiIfIndex interface, AvahiProtocol protocol,
    AvahiPublishFlags flags, const char* name, const char* type,
    const char* domain, AvahiStringList* txt)
{
...
int avahi_entry_group_add_address(AvahiEntryGroup* group,
                                  AvahiIfIndex interface,
                                  AvahiProtocol protocol,
                                  AvahiPublishFlags flags, const char* name,
                                  const AvahiAddress* a)
{
    ...
    if (!dbus_message_append_args(
            message, DBUS_TYPE_INT32, &i_interface, DBUS_TYPE_INT32,
            &i_protocol, DBUS_TYPE_UINT32, &u_flags, DBUS_TYPE_STRING, &name,
            DBUS_TYPE_STRING, &p_address, DBUS_TYPE_INVALID))
    {
        r = avahi_client_set_errno(group->client, AVAHI_ERR_NO_MEMORY);
        goto fail;
    }
                                 ...
                                 const void* rdata, size_t size)
{
    ...
    if (!dbus_message_append_args(
            message, DBUS_TYPE_INT32, &i_interface, DBUS_TYPE_INT32,
            &i_protocol, DBUS_TYPE_UINT32, &u_flags, DBUS_TYPE_STRING, &name,
            DBUS_TYPE_UINT16, &clazz, DBUS_TYPE_UINT16, &type, DBUS_TYPE_UINT32,
            &ttl, DBUS_TYPE_INVALID) ||
