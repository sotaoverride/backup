...
int manager_parse_config_file(Manager* m)
{
    ...
    return config_parse_many_nulstr(
        ...
        CONF_PATHS_NULSTR("systemd/networkd.conf.d"), "DHCP\0",
        config_item_perf_lookup, networkd_gperf_lookup, CONFIG_PARSE_WARN, m);
}
...
int config_parse_duid_type(const char* unit, const char* filename,
...
int config_parse_duid_rawdata(const char* unit, const char* filename,
                              unsigned line, const char* section,
    ...
    for (;;)
    {
        ...
        if (count >= MAX_DUID_LEN)
        {
            log_syntax(unit, LOG_ERR, filename, line, 0,
                       "Max DUID length exceeded, ignoring assignment: %s.",
                       rvalue);
            return 0;
        }
        ...
        if (!IN_SET(len, 1, 2))
        {
            log_syntax(
                ...
                "Invalid length - DUID byte: %s, ignoring assignment: %s.",
                cbyte, rvalue);
            return 0;
        }
        ...
        {
            log_syntax(unit, LOG_ERR, filename, line, 0,
                       "Invalid DUID byte: %s. Ignoring assignment: %s.", cbyte,
                       rvalue);
            return 0;
        }
