...
static int help(void)
{
    ...
    printf("%s [OPTIONS...] {COMMAND} ...\n\n"
           ...
           "     --stderr-priority=PRIORITY  Set priority value (0..7) used "
           "for stderr\n"
           "     --level-prefix=BOOL         Control whether level prefix "
           "shall be parsed\n"
    ...
    while ((c = getopt_long(argc, argv, "+ht:p:", options, NULL)) >= 0)
        ...
        switch (c)
        {
            ...
            case ARG_LEVEL_PREFIX:
            {
                ...
                if (k < 0)
                    return log_error_errno(
                        k, "Failed to parse level prefix value.");
...
static int run(int argc, char* argv[])
{
    ...
    outfd =
        sd_journal_stream_fd(arg_identifier, arg_priority, arg_level_prefix);
    if (outfd < 0)
        return log_error_errno(outfd, "Failed to create stream fd: %m");

    if (arg_stderr_priority >= 0 && arg_stderr_priority != arg_priority)
    {
        errfd = sd_journal_stream_fd(arg_identifier, arg_stderr_priority,
                                     arg_level_prefix);
        if (errfd < 0)
    ...
    r = rearrange_stdio(
        STDIN_FILENO, outfd,
        errfd < 0 ? outfd : errfd); /* Invalidates fd on succcess + error! */
    TAKE_FD(outfd);
    TAKE_FD(errfd);
    if (r < 0)
