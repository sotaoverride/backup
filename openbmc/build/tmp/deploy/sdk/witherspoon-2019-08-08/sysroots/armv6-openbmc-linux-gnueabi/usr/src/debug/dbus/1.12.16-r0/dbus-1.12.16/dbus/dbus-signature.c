...
typedef struct
{
    ...
    unsigned int in_array : 1; /**< true if we are a subiterator pointing to an
                                  array's element type */
} DBusSignatureRealIter;

/** macro that checks whether a typecode is a container type */
#define TYPE_IS_CONTAINER(typecode)                                            \
    ((typecode) == DBUS_TYPE_STRUCT || (typecode) == DBUS_TYPE_DICT_ENTRY ||   \
     (typecode) == DBUS_TYPE_VARIANT || (typecode) == DBUS_TYPE_ARRAY)
 ...
 *
 * @param iter pointer to an iterator
 * @returns current signature; or #NULL if no memory.  Should be freed with
 * dbus_free()
 */
...
int dbus_signature_iter_get_element_type(const DBusSignatureIter* iter)
{
    ...
    _dbus_return_val_if_fail(dbus_signature_iter_get_current_type(iter) ==
                                 DBUS_TYPE_ARRAY,
                             DBUS_TYPE_INVALID);
...

{
    ...
    reason = _dbus_validate_signature_with_reason(
...
dbus_bool_t dbus_type_is_container(int typecode)
{
    ...
    _dbus_return_val_if_fail(
        dbus_type_is_valid(typecode) || typecode == DBUS_TYPE_INVALID, FALSE);
...
dbus_bool_t _dbus_signature_test(void)
{
    ...
    _DBUS_STATIC_ASSERT(sizeof(DBusSignatureIter) >=
                        sizeof(DBusSignatureRealIter));
    ...
    _dbus_assert(dbus_signature_iter_get_current_type(&iter) ==
                 DBUS_TYPE_INVALID);
    ...
    sig = DBUS_TYPE_UINT16_AS_STRING DBUS_STRUCT_BEGIN_CHAR_AS_STRING
        DBUS_TYPE_STRING_AS_STRING DBUS_TYPE_UINT32_AS_STRING
            DBUS_TYPE_VARIANT_AS_STRING DBUS_TYPE_DOUBLE_AS_STRING
    ...
    _dbus_assert(dbus_signature_iter_get_current_type(&subiter) ==
                 DBUS_TYPE_STRING);
    ...
    sig = DBUS_TYPE_UINT16_AS_STRING DBUS_STRUCT_BEGIN_CHAR_AS_STRING
        DBUS_TYPE_UINT32_AS_STRING DBUS_TYPE_BYTE_AS_STRING
            DBUS_TYPE_ARRAY_AS_STRING DBUS_TYPE_ARRAY_AS_STRING
                DBUS_TYPE_DOUBLE_AS_STRING DBUS_STRUCT_BEGIN_CHAR_AS_STRING
                    DBUS_TYPE_BYTE_AS_STRING DBUS_STRUCT_END_CHAR_AS_STRING
                        DBUS_STRUCT_END_CHAR_AS_STRING;
    _dbus_assert(dbus_signature_validate(sig, NULL));
    dbus_signature_iter_init(&iter, sig);
    _dbus_assert(dbus_signature_iter_get_current_type(&iter) ==
    ...
    _dbus_assert(dbus_signature_iter_get_current_type(&subiter) ==
                 DBUS_TYPE_ARRAY);
    _dbus_assert(dbus_signature_iter_get_element_type(&subiter) ==
                 DBUS_TYPE_ARRAY);

    dbus_signature_iter_recurse(&subiter, &subsubiter);
    _dbus_assert(dbus_signature_iter_get_current_type(&subsubiter) ==
                 DBUS_TYPE_ARRAY);
    _dbus_assert(dbus_signature_iter_get_element_type(&subsubiter) ==
                 DBUS_TYPE_DOUBLE);

    dbus_signature_iter_recurse(&subsubiter, &subsubsubiter);
    _dbus_assert(dbus_signature_iter_get_current_type(&subsubsubiter) ==
                 DBUS_TYPE_DOUBLE);
    boolres = dbus_signature_iter_next(&subiter);
    _dbus_assert(boolres);
    _dbus_assert(dbus_signature_iter_get_current_type(&subiter) ==
    ...
    sig = DBUS_TYPE_ARRAY_AS_STRING DBUS_DICT_ENTRY_BEGIN_CHAR_AS_STRING
        DBUS_TYPE_INT16_AS_STRING DBUS_TYPE_STRING_AS_STRING
            DBUS_DICT_ENTRY_END_CHAR_AS_STRING DBUS_TYPE_VARIANT_AS_STRING;
    _dbus_assert(dbus_signature_validate(sig, NULL));
    _dbus_assert(!dbus_signature_validate_single(sig, NULL));
    dbus_signature_iter_init(&iter, sig);
    _dbus_assert(dbus_signature_iter_get_current_type(&iter) ==
                 DBUS_TYPE_ARRAY);
    _dbus_assert(dbus_signature_iter_get_element_type(&iter) ==
