...
#define SAMDATA(kdata, str, maxsize)                                           \
    (int)((kdata.length) ? ((((kdata.length) <= (maxsize)) ? (kdata.length)    \
                                                           : strlen(str)))     \
                         : strlen(str)),                                       \
        (kdata.length)                                                         \
            ? ((((kdata.length) <= (maxsize)) ? (kdata.data) : (str)))         \
            : (str)
static char* sam_challenge_banner(krb5_int32 sam_type)
{
    char* label;
                 ...
                 krb5_pa_data*** out_padata)
{
    ...
    retval =
        decode_krb5_sam_challenge_2_body(&sc2->sam_challenge_2_body, &sc2b);
    ...
    if (!(sc2b->sam_flags & KRB5_SAM_USE_SAD_AS_KEY))
    {
        ...
        retval =
            ctx->gak_fct(context, request->client, sc2b->sam_etype, prompter,
                         prompter_data, &ctx->salt, &ctx->s2kparams,
    ...
    snprintf(banner, sizeof(banner), "%.*s",
             SAMDATA(sc2b->sam_challenge_label,
                     sam_challenge_banner(sc2b->sam_type), sizeof(banner) - 1));
    ...
    if ((retval =
             ((*prompter)(context, prompter_data, name, banner, 1, &kprompt))))
    {
        krb5_free_sam_challenge_2(context, sc2);
        krb5_free_sam_challenge_2_body(context, sc2b);
    ...
    if (!(sc2b->sam_flags & KRB5_SAM_USE_SAD_AS_KEY))
    {
        ...
        gakpw = ctx->gak_data;
        retval = krb5_c_string_to_key(context, sc2b->sam_etype, gakpw->password,
                                      salt, &ctx->as_key);
    ...
    else
    {
        ...
        /* generate a key using the supplied password */
        retval = krb5_c_string_to_key(context, sc2b->sam_etype, &response_data,
                                      salt, &ctx->as_key);
    ...
    for (; *cksum; cksum++)
    {
        ...
        retval = krb5_c_verify_checksum(
            context, &ctx->as_key, KRB5_KEYUSAGE_PA_SAM_CHALLENGE_CKSUM,
            &sc2->sam_challenge_2_body, *cksum, &valid_cksum);
        if (retval)
        {
    ...
    retval =
        encode_krb5_enc_sam_response_enc_2(&enc_sam_response_enc_2, &scratch);
    if (retval)
    {
        krb5_free_sam_challenge_2(context, sc2);
        krb5_free_sam_challenge_2_body(context, sc2b);
    ...
    retval =
        krb5_c_encrypt(context, &ctx->as_key, KRB5_KEYUSAGE_PA_SAM_RESPONSE,
...
static krb5_preauthtype sam2_pa_types[] = {KRB5_PADATA_SAM_CHALLENGE_2, 0};
