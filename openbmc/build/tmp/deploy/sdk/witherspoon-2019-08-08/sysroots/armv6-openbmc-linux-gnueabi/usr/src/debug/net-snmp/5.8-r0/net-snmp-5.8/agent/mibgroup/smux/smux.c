...
static u_char* smux_parse_var(u_char*, size_t*, oid*, size_t*, size_t*,
                              u_char*);
...
static int smux_build(u_char, long, oid*, size_t*, u_char, u_char*, size_t,
                      u_char*, size_t*);
...
u_char* var_smux_get(oid*, size_t, oid*, size_t*, int, size_t*, u_char*);
...
void smux_parse_smux_socket(const char* token, char* cptr)
{
    DEBUGMSGTL(("smux", "port spec: %s\n", cptr));
    netsnmp_ds_set_string(NETSNMP_DS_APPLICATION_ID, NETSNMP_DS_SMUX_SOCKET,
                          cptr);
}

void smux_parse_peer_auth(const char* token, char* cptr)
{
    smux_peer_auth* aptr;
    char* password_cptr;
    int rv;
...
void init_smux(void)
{
    snmpd_register_config_handler("smuxpeer", smux_parse_peer_auth,
                                  smux_free_peer_auth, "OID-IDENTITY PASSWORD");
    snmpd_register_config_handler("smuxsocket", smux_parse_smux_socket, NULL,
                                  "SMUX bind address");
}

void real_init_smux(void)
{
    ...
    if (bind(smux_listen_sd, (struct sockaddr*)&lo_socket, sizeof(lo_socket)) <
        0)
    {
...
static int smux_handler(netsnmp_mib_handler* handler,
    ...
    for (; requests; requests = requests->next)
    {
        switch (reqinfo->mode)
        {
            ...
            case MODE_SET_RESERVE1:
                access = var_smux_get(reginfo->rootoid, reginfo->rootoid_len,
                                      requests->requestvb->name,
                                      &requests->requestvb->name_length, exact,
                                      &var_len, &var_type);
                if (access)
                    if (reqinfo->mode != MODE_SET_RESERVE1)
                        snmp_set_var_typed_value(requests->requestvb, var_type,
                                                 access, var_len);
                if (reqinfo->mode != MODE_SET_RESERVE1)
                    break;
            ...
            default:
                /* SET processing */
                status = var_smux_write(
                    reqinfo->mode, requests->requestvb->val.string,
                    requests->requestvb->type, requests->requestvb->val_len,
                    requests->requestvb->name,
...
u_char* var_smux_get(oid* root, size_t root_len, oid* name, size_t* length,
                     int exact, size_t* var_len, u_char* var_type)
{
    ...
    valptr =
        smux_snmp_process(exact, name, length, var_len, var_type, rptr->sr_fd);
...
int var_smux_write(int action, u_char* var_val, u_char var_val_type,
                   size_t var_val_len, oid* name, size_t name_len)
{
    smux_reg* rptr;
    ...
    switch (action)
    {
        case RESERVE1:
            ...
            switch (var_val_type)
            {
                ...
                default:
                    DEBUGMSGTL(
                        ("smux", "[var_smux_write] variable not supported\n"));
                    return SNMP_ERR_GENERR;
                    break;
            }

            if ((smux_build((u_char)SMUX_SET, smux_reqid, name, &name_length,
                            var_val_type, dataptr, datalen, buf, &len)) < 0)
            {
            ...
            while (1)
            {
                 ...
                 */
                if ((len = recvfrom(rptr->sr_fd, (void*)buf, SMUXMAXPKTSIZE,
                ...
                DEBUGMSGTL(("smux",
                            "[var_smux_write] Peeked at %" NETSNMP_PRIz
                            "d bytes\n",
                            len));
                ...
                DEBUGMSGTL(("smux",
                            "[var_smux_write] Received %" NETSNMP_PRIz
                            "d bytes\n",
                            len));
                ...
                if (buf[0] == SMUX_TRAP)
                {
                    ...
                    DEBUGMSGTL(
                        ("smux", "Got trap from peer on fd %d\n", rptr->sr_fd));
                ...
                else
                {
                    ...
                    ptr = asn_parse_int(ptr, &len, &type, &errsts,
                                        sizeof(errsts));
                    if ((ptr == NULL) || type != ASN_INTEGER)
                        return SNMP_ERR_GENERR;
                    ...
                    {
                        DEBUGMSGTL(
                            ("smux", "[var_smux_write] errsts returned\n"));
                        return (errsts);
                    }

                    ptr = asn_parse_int(ptr, &len, &type, &erridx,
                                        sizeof(erridx));
                    if ((ptr == NULL) || type != ASN_INTEGER)
                        return SNMP_ERR_GENERR;
        ...
        case COMMIT:
            ...
            *(ptr++) = (u_char)1;
            ...
            {
                DEBUGMSGTL(
                    ("smux", "[var_smux_write] send rollback/commit failed\n"));
                return SNMP_ERR_GENERR;
            }
...
int smux_accept(int sd)
{
    ...
    else
    {
        DEBUGMSGTL(("smux", "[smux_accept] accepted fd %d from %s:%d\n", fd,
                    inet_ntoa(in_socket.sin_addr), ntohs(in_socket.sin_port)));
        if (npeers + 1 == SMUXMAXPEERS)
        {
            snmp_log(LOG_ERR,
                     "[smux_accept] denied peer on fd %d, limit %d reached", fd,
                     SMUXMAXPEERS);
            close(fd);
            return -1;
        }
...
int smux_process(int fd)
{
    ...
    {
        length =
            recvfrom(fd, (char*)data, SMUXMAXPKTSIZE, MSG_PEEK, NULL, NULL);
    } while ((length == -1) && ((errno == EINTR) || (errno == EAGAIN)));
    ...
    if (length <= 0)
    {
         ...
         */
        DEBUGMSGTL(
            ("smux", "[smux_process] peer on fd %d died or timed out\n", fd));
        smux_peer_cleanup(fd);
        return -1;
    }
...
static int smux_pdu_process(int fd, u_char* data, size_t length)
{
    int error;
    size_t len;
    u_char *ptr, type;
    ...
    DEBUGMSGTL(("smux",
                "[smux_pdu_process] Processing %" NETSNMP_PRIz "d bytes\n",
                length));
        ...
        switch (type)
        {
            ...
            case SMUX_TRAP:
                ...
                if (ptr)
                {
                    DEBUGMSGTL(
                        ("smux",
                         "[smux_pdu_process] call smux_trap_process.\n"));
                    ptr = smux_trap_process(ptr, &len);
                }
                else
                {
                    DEBUGMSGTL(("smux", "[smux_pdu_process] smux_trap_process "
                                        "not called: ptr=NULL.\n"));
            ...
            default:
                ...
                smux_peer_cleanup(fd);
                DEBUGMSGTL(
                    ("smux", "[smux_pdu_process] Wrong type %d\n", (int)type));
                error = -1;
                break;
        }
    }
...
static u_char* smux_open_process(int fd, u_char* ptr, size_t* len, int* fail)
{
    u_char type;
    long version;
    ...
    if ((ptr = asn_parse_objid(ptr, len, &type, oid_name, &oid_name_len)) ==
        NULL)
    {
    ...
    if (snmp_get_do_debugging())
    {
        ...
        DEBUGMSGTL(("smux",
                    "[smux_open_process] len %" NETSNMP_PRIz "u, type %d\n",
                    *len, (int)type));
    }
    ...
    {
        snmp_log(LOG_WARNING, "refused smux peer: oid %s, descr %s\n",
                 oid_print, descr);
        *fail = TRUE;
        return ptr;
    }
    DEBUGMSGTL(
        ("smux", "accepted smux peer: oid %s, descr %s\n", oid_print, descr));
    *fail = FALSE;
    return ptr;
}

static void smux_send_close(int fd, int reason)
{
...
static int smux_auth_peer(oid* name, size_t namelen, char* passwd, int fd)
{
    int i;
    char oid_print[SMUXMAXSTRLEN];
    ...
    for (i = 0; i < nauths; i++)
    {
        if (snmp_get_do_debugging())
        {
            snprint_objid(oid_print, sizeof(oid_print), Auths[i]->sa_oid,
                          Auths[i]->sa_oid_len);
            DEBUGMSGTL(("smux:auth", "[smux_auth_peer] Checking OID: %s (%d)\n",
                        oid_print, i));
        }
        if (snmp_oid_compare(Auths[i]->sa_oid, Auths[i]->sa_oid_len, name,
                             namelen) == 0)
        {
            if (snmp_get_do_debugging())
            {
                DEBUGMSGTL(("smux:auth",
                            "[smux_auth_peer] Checking P/W: %s (%d)\n",
 ...
 */
static u_char* smux_close_process(int fd, u_char* ptr, size_t* len)
{
    long down = 0;
    int length = *len;
...
static u_char* smux_rreq_process(int sd, u_char* ptr, size_t* len)
{
    ...
    if ((ptr = asn_parse_int(ptr, len, &type, &priority, sizeof(priority))) ==
        NULL)
    {
    ...
    if ((ptr = asn_parse_int(ptr, len, &type, &operation, sizeof(operation))) ==
        NULL)
    {
    ...
    if (operation == SMUX_REGOP_DELETE)
    {
         ...
         */
        rptr =
            smux_find_match(ActiveRegs, sd, oid_name, oid_name_len, priority);
        if (rptr)
        {
             ...
             * find a replacement
             */
            nrptr = smux_find_replacement(rptr->sr_name, rptr->sr_name_len);
            if (nrptr)
            {
                /*
                 * found one
         ...
         */
        rptr =
            smux_find_match(PassiveRegs, sd, oid_name, oid_name_len, priority);
        if (rptr)
        {
            rpriority = rptr->sr_priority;
             ...
             (operation == SMUX_REGOP_REGISTER_RW))
    {
        ...
        reg = netsnmp_create_handler_registration(
            "smux", smux_handler, nrptr->sr_name, nrptr->sr_name_len,
            HANDLER_CAN_RWRITE);
        if (reg == NULL)
        {
 ...
 */
static smux_reg* smux_find_match(smux_reg* regs, int sd, oid* oid_name,
                                 size_t oid_name_len, long priority)
{
    ...
    for (rptr = regs; rptr; rptr = rptr->sr_next)
    {
        if (rptr->sr_fd != sd)
            continue;
        if (snmp_oid_compare(rptr->sr_name, rptr->sr_name_len, oid_name,
                             oid_name_len))
            continue;
...
static void smux_replace_active(smux_reg* actptr, smux_reg* pasptr)
{
    netsnmp_handler_registration* reg;
    ...
    reg = netsnmp_create_handler_registration(
        "smux", smux_handler, pasptr->sr_name, pasptr->sr_name_len,
        HANDLER_CAN_RWRITE);
    if (reg == NULL)
    {
...
static void smux_list_detach(smux_reg** head, smux_reg* m_remove)
{
    smux_reg *rptr, *rptr2;
 ...
 * priority) return -1.
 */
static int smux_list_add(smux_reg** head, smux_reg* add)
{
    smux_reg *rptr, *prev;
    int result;
    ...
    {
        result = snmp_oid_compare(add->sr_name, add->sr_name_len, rptr->sr_name,
                                  rptr->sr_name_len);
        if (result == 0)
        {
            /*
 ...
 *
 */
static smux_reg* smux_find_replacement(oid* name, size_t name_len)
{
    ...
    {
        if (!snmp_oidtree_compare(rptr->sr_name, rptr->sr_name_len, name,
                                  name_len))
        {
            if ((difflen = rptr->sr_name_len - name_len) < bestlen || !bestptr)
            {
                bestlen = difflen;
                bestptr = rptr;
            }
...
u_char* smux_snmp_process(int exact, oid* objid, size_t* len,
                          size_t* return_len, u_char* return_type, int sd)
{
    ...
    if (smux_build(type, smux_reqid, objid, len, 0, NULL, *len, packet,
                   (size_t*)&length) < 0)
    {
    ...
    DEBUGMSGTL(("smux",
                "[smux_snmp_process] Sent %d request to peer; %" NETSNMP_PRIz
                "d bytes\n",
                (int)type, length));
    ...
    while (1)
    {
         ...
         */
        length =
            recvfrom(sd, (char*)result, SMUXMAXPKTSIZE, MSG_PEEK, NULL, NULL);
        if (length <= 0)
        {
        ...
        DEBUGMSGTL(("smux",
                    "[smux_snmp_process] Peeked at %" NETSNMP_PRIz "d bytes\n",
                    length));
        ...
        DEBUGMSGTL(("smux",
                    "[smux_snmp_process] Received %" NETSNMP_PRIz "d bytes\n",
                    length));
...
static u_char* smux_parse(u_char* rsp, oid* objid, size_t* oidlen,
                          size_t* return_len, u_char* return_type)
{
    ...
    if ((ptr = asn_parse_int(ptr, &length, &type, &reqid, sizeof(reqid))) ==
        NULL)
    {
        DEBUGMSGTL(("smux", "[smux_parse] parse of reqid failed\n"));
        return NULL;
    }
    if ((ptr = asn_parse_int(ptr, &length, &type, &errstat, sizeof(errstat))) ==
        NULL)
    {
     ...
     */
    return (
        smux_parse_var(ptr, &length, objid, oidlen, return_len, return_type));
}

static u_char* smux_parse_var(u_char* varbind, size_t* varbindlength,
                              oid* objid, size_t* oidlen, size_t* varlength,
                              u_char* vartype)
{
    ...
    ptr = snmp_parse_var_op(ptr, var_name, &var_name_len, vartype, &var_val_len,
                            &var_val, &len);
    ...
    switch ((short)*vartype)
    {
        case ASN_INTEGER:
            *varlength = sizeof(long);
            asn_parse_int(var_val, &len, vartype, (long*)&smux_long,
                          *varlength);
        ...
        case ASN_UINTEGER:
            *varlength = sizeof(u_long);
            asn_parse_unsigned_int(var_val, &len, vartype, (u_long*)&smux_ulong,
                                   *varlength);
        ...
        case ASN_IPADDRESS:
            *varlength = 4;
            ...
            if ((var_val = asn_parse_header(var_val, &len, &type)) == NULL)
                return NULL;
            memcpy((u_char*)&(smux_sa.sin_addr.s_addr), var_val, *varlength);
 ...
 */
static int smux_build(u_char type, long reqid, oid* objid, size_t* oidlen,
                      u_char val_type, u_char* val, size_t val_len,
                      u_char* packet, size_t* length)
{
     ...
     */
    ptr = snmp_build_var_op(ptr, objid, oidlen, val_type, val_len, val, &len);
    if (ptr == NULL)
    {
        return -1;
    }
    ...
    len = ptr - save2;
    asn_build_sequence(save2, &len, (ASN_SEQUENCE | ASN_CONSTRUCTOR),
                       (ptr - save2 - 4));
...
static void smux_peer_cleanup(int sd)
{
...
int smux_send_rrsp(int sd, int pri)
{
...
static u_char* smux_trap_process(u_char* rsp, size_t* len)
{
    ...
    if ((ptr = asn_parse_objid(ptr, len, &vartype, (oid*)&sa_enterpriseoid,
    ...
    if (((ptr = asn_parse_string(ptr, len, &vartype, smux_str, &datalen)) ==
         NULL) ||
    ...
    {
        DEBUGMSGTL(
            ("smux", "[smux_trap_process] asn_parse_int generic failed\n"));
        return NULL;
    }
     ...
     */
    datalen = sizeof(long);
    if ((ptr = asn_parse_int(ptr, len, &vartype, &specific, datalen)) == NULL)
    {
        DEBUGMSGTL(
            ("smux", "[smux_trap_process] asn_parse_int specific failed\n"));
        return NULL;
    }
    ...
    if (((ptr = asn_parse_unsigned_int(ptr, len, &vartype, (u_long*)&timestamp,
                                       datalen)) == NULL) ||
    ...
    while (ptr && *len)
    {
        ...
        switch ((short)vartype)
        {
            case ASN_INTEGER:
                ...
                asn_parse_int(var_val, &maxlen, &vartype, (long*)&smux_long,
                              var_val_len);
            ...
            case ASN_IPADDRESS:
                 ...
                 * because we know it is an ip address
                 */
                if ((var_val = asn_parse_header(var_val, &maxlen, &vartype)) ==
                    NULL)
                    goto err;
            ...
            case ASN_OCTET_STR:
                ...
                asn_parse_string(var_val, &maxlen, &vartype, smux_str,
                                 &var_val_len);
                var_val = smux_str;
                break;
            case ASN_OBJECT_ID:
                var_val_len = MAX_OID_LEN;
                asn_parse_objid(var_val, &maxlen, &vartype, smux_objid,
                                &var_val_len);
            ...
            case ASN_BIT_STR:
                 ...
                 */
                var_val_len = SMUXMAXSTRLEN;
                asn_parse_bitstring(var_val, &maxlen, &vartype, smux_str,
                                    &var_val_len);
