...
#if 0 /* debugging */
#define LOCK(mutex)                                                            \
    (rfbLog("%s:%d LOCK(%s,0x%x)\n", __FILE__, __LINE__, #mutex, &(mutex)),    \
     pthread_mutex_lock(&(mutex)))
#define UNLOCK(mutex)                                                          \
    (rfbLog("%s:%d UNLOCK(%s,0x%x)\n", __FILE__, __LINE__, #mutex, &(mutex)),  \
     pthread_mutex_unlock(&(mutex)))
#define MUTEX(mutex) pthread_mutex_t(mutex)
#define INIT_MUTEX(mutex)                                                      \
    (rfbLog("%s:%d INIT_MUTEX(%s,0x%x)\n", __FILE__, __LINE__, #mutex,         \
            &(mutex)),                                                         \
     pthread_mutex_init(&(mutex), NULL))
#define TINI_MUTEX(mutex)                                                      \
    (rfbLog("%s:%d TINI_MUTEX(%s)\n", __FILE__, __LINE__, #mutex),             \
     pthread_mutex_destroy(&(mutex)))
#define TSIGNAL(cond)                                                          \
    (rfbLog("%s:%d TSIGNAL(%s)\n", __FILE__, __LINE__, #cond),                 \
...
#define INIT_COND(cond)                                                        \
    (rfbLog("%s:%d INIT_COND(%s)\n", __FILE__, __LINE__, #cond),               \
     pthread_cond_init(&(cond), NULL))
#define TINI_COND(cond)                                                        \
    (rfbLog("%s:%d TINI_COND(%s)\n", __FILE__, __LINE__, #cond),               \
...
/* if you use pthreads, but don't define LIBVNCSERVER_HAVE_LIBPTHREAD, the
   structs get all mixed up. So this gives a linker error reminding you to
   compile the library and your application (at least the parts including rfb.h)
...
typedef void (*rfbKbdAddEventProcPtr)(rfbBool down, rfbKeySym keySym,
...
typedef void (*rfbPtrAddEventProcPtr)(int buttonMask, int x, int y,
                                      struct _rfbClientRec* cl);
typedef void (*rfbSetXCutTextProcPtr)(char* str, int len,
                                      struct _rfbClientRec* cl);
...
typedef rfbBool (*rfbPasswordCheckProcPtr)(struct _rfbClientRec* cl,
                                           const char* encryptedPassWord,
                                           int len);
typedef enum rfbNewClientAction (*rfbNewClientHookPtr)(
    struct _rfbClientRec* cl);
 ...
 * else find the window underneath x and y and set the framebuffer to the
 * dimensions of that window
 */
typedef void (*rfbSetSingleWindowProcPtr)(struct _rfbClientRec* cl, int x,
                                          int y);
/**
 ...
 * It is called when a client initiates a connection to determine if it is
 * permitted.
 */
typedef int (*rfbFileTransferPermitted)(struct _rfbClientRec* cl);
/** Handle the textchat messages */
typedef void (*rfbSetTextChat)(struct _rfbClientRec* cl, int length,
                               char* string);
...
typedef struct _rfbProtocolExtension
{
    ...
    rfbBool (*enablePseudoEncoding)(struct _rfbClientRec* client, void** data,
                                    int encodingNumber);
    /** returns TRUE if message was handled */
    rfbBool (*handleMessage)(struct _rfbClientRec* client, void* data,
...
typedef struct _rfbScreenInfo
{
    ...
    rfbColourMap
        colourMap; /**< set this if rfbServerFormat.trueColour==FALSE */
...
typedef void (*rfbTranslateFnType)(char* table, rfbPixelFormat* in,
                                   ...
                                   int bytesBetweenInputLines, int width,
                                   int height);
...
typedef struct _rfbClientRec
{
    ...
    /** how did the client tell us it wanted the screen changed?  Ultra style or
     * palm style? */
    rfbBool PalmVNC;
...
#define UPDATE_BUF_SIZE 30000
    ...
    int lastKeyboardLedState; /**< keep track of last value so we can send
                                 *change* events */
    rfbBool enableSupportedMessages;  /**< client supports SupportedMessages
                                         encoding */
    rfbBool enableSupportedEncodings; /**< client supports SupportedEncodings
                                         encoding */
    rfbBool
        enableServerIdentity; /**< client supports ServerIdentity encoding */
    rfbBool
        enableKeyboardLedState; /**< client supports KeyboardState encoding */
    rfbBool enableLastRectEncoding; /**< client supports LastRect encoding */
    rfbBool
        enableCursorShapeUpdates; /**< client supports cursor shape updates */
    rfbBool
        enableCursorPosUpdates; /**< client supports cursor position updates */
...
#define Swap32(l)                                                              \
    ((((l) >> 24) & 0x000000ff) | (((l)&0x00ff0000) >> 8) |                    \
...
/* UltraVNC uses some windows structures unmodified, so the viewer expects
 * LittleEndian Data */
...
extern int rfbReadExactTimeout(rfbClientPtr cl, char* buf, int len,
                               int timeout);
extern int rfbPeekExactTimeout(rfbClientPtr cl, char* buf, int len,
                               int timeout);
...
extern rfbClientPtr rfbReverseConnection(rfbScreenInfoPtr rfbScreen, char* host,
                                         int port);
...
extern rfbBool rfbSendFramebufferUpdate(rfbClientPtr cl,
                                        sraRegionPtr updateRegion);
extern rfbBool rfbSendRectEncodingRaw(rfbClientPtr cl, int x, int y, int w,
                                      int h);
extern rfbBool rfbSendUpdateBuf(rfbClientPtr cl);
extern void rfbSendServerCutText(rfbScreenInfoPtr rfbScreen, char* str,
                                 int len);
extern rfbBool rfbSendCopyRegion(rfbClientPtr cl, sraRegionPtr reg, int dx,
                                 int dy);
extern rfbBool rfbSendLastRectMarker(rfbClientPtr cl);
extern rfbBool rfbSendNewFBSize(rfbClientPtr cl, int w, int h);
extern rfbBool rfbSendSetColourMapEntries(rfbClientPtr cl, int firstColour,
                                          int nColours);
...
extern char* rfbProcessFileTransferReadBuffer(rfbClientPtr cl, uint32_t length);
extern rfbBool rfbProcessFileTransfer(rfbClientPtr cl, uint8_t contentType,
                                      uint8_t contentParam, uint32_t size,
                                      uint32_t length);
...
extern void rfbTranslateNone(char* table, rfbPixelFormat* in,
                             rfbPixelFormat* out, char* iptr, char* optr,
                             int bytesBetweenInputLines, int width, int height);
extern rfbBool rfbSetTranslateFunction(rfbClientPtr cl);
extern rfbBool rfbSetClientColourMap(rfbClientPtr cl, int firstColour,
                                     int nColours);
extern void rfbSetClientColourMaps(rfbScreenInfoPtr rfbScreen, int firstColour,
                                   int nColours);
...
extern rfbBool rfbSendRectEncodingRRE(rfbClientPtr cl, int x, int y, int w,
                                      int h);

/* corre.c */

extern rfbBool rfbSendRectEncodingCoRRE(rfbClientPtr cl, int x, int y, int w,
                                        int h);
...
extern rfbBool rfbSendRectEncodingUltra(rfbClientPtr cl, int x, int y, int w,
                                        int h);
...
extern rfbBool rfbSendRectEncodingTight(rfbClientPtr cl, int x, int y, int w,
                                        int h);
...
extern rfbBool rfbSendRectEncodingTightPng(rfbClientPtr cl, int x, int y, int w,
                                           int h);
#endif
...
typedef struct rfbCursor
{
    ...
    unsigned short foreRed, foreGreen,
        foreBlue; /**< device-independent colour */
    unsigned short backRed, backGreen,
        backBlue;               /**< device-independent colour */
...
extern rfbBool
    rfbSendCursorShape(rfbClientPtr cl /*, rfbScreenInfoPtr pScreen*/);
extern rfbBool rfbSendCursorPos(rfbClientPtr cl);
extern void rfbConvertLSBCursorBitmapOrMask(int width, int height,
                                            unsigned char* bitmap);
extern rfbCursorPtr rfbMakeXCursor(int width, int height, char* cursorString,
                                   char* maskString);
extern char* rfbMakeMaskForXCursor(int width, int height, char* cursorString);
extern char* rfbMakeMaskFromAlphaSource(int width, int height,
                                        unsigned char* alphaSource);
extern void rfbMakeXCursorFromRichCursor(rfbScreenInfoPtr rfbScreen,
                                         rfbCursorPtr cursor);
extern void rfbMakeRichCursorFromXCursor(rfbScreenInfoPtr rfbScreen,
                                         rfbCursorPtr cursor);
extern void rfbFreeCursor(rfbCursorPtr cursor);
extern void rfbSetCursor(rfbScreenInfoPtr rfbScreen, rfbCursorPtr c);

/** cursor handling for the pointer */
extern void rfbDefaultPtrAddEvent(int buttonMask, int x, int y,
                                  rfbClientPtr cl);

/* zrle.c */
#ifdef LIBVNCSERVER_HAVE_LIBZ
extern rfbBool rfbSendRectEncodingZRLE(rfbClientPtr cl, int x, int y, int w,
                                       int h);
#endif
...
int rfbDrawChar(rfbScreenInfoPtr rfbScreen, rfbFontDataPtr font, int x, int y,
                unsigned char c, rfbPixel colour);
void rfbDrawString(rfbScreenInfoPtr rfbScreen, rfbFontDataPtr font, int x,
                   int y, const char* string, rfbPixel colour);
/** if colour==backColour, background is transparent */
int rfbDrawCharWithClip(rfbScreenInfoPtr rfbScreen, rfbFontDataPtr font, int x,
                        int y, unsigned char c, int x1, int y1, int x2, int y2,
                        rfbPixel colour, rfbPixel backColour);
void rfbDrawStringWithClip(rfbScreenInfoPtr rfbScreen, rfbFontDataPtr font,
                           int x, int y, const char* string, int x1, int y1,
                           int x2, int y2, rfbPixel colour,
                           rfbPixel backColour);
int rfbWidthOfString(rfbFontDataPtr font, const char* string);
int rfbWidthOfChar(rfbFontDataPtr font, unsigned char c);
void rfbFontBBox(rfbFontDataPtr font, unsigned char c, int* x1, int* y1,
                 int* x2, int* y2);
...
void rfbFillRect(rfbScreenInfoPtr s, int x1, int y1, int x2, int y2,
                 rfbPixel col);
void rfbDrawPixel(rfbScreenInfoPtr s, int x, int y, rfbPixel col);
void rfbDrawLine(rfbScreenInfoPtr s, int x1, int y1, int x2, int y2,
                 rfbPixel col);
...
extern int rfbSelectBox(rfbScreenInfoPtr rfbScreen, rfbFontDataPtr font,
                        char** list, int x1, int y1, int x2, int y2,
...
extern void rfbPurgeArguments(int* argc, int* position, int count,
                              char* argv[]);
extern rfbBool rfbProcessArguments(rfbScreenInfoPtr rfbScreen, int* argc,
                                   char* argv[]);
extern rfbBool rfbProcessSizeArguments(int* width, int* height, int* bpp,
                                       int* argc, char* argv[]);
...
void rfbScheduleCopyRect(rfbScreenInfoPtr rfbScreen, int x1, int y1, int x2,
                         int y2, int dx, int dy);
void rfbScheduleCopyRegion(rfbScreenInfoPtr rfbScreen, sraRegionPtr copyRegion,
                           int dx, int dy);

void rfbDoCopyRect(rfbScreenInfoPtr rfbScreen, int x1, int y1, int x2, int y2,
                   int dx, int dy);
void rfbDoCopyRegion(rfbScreenInfoPtr rfbScreen, sraRegionPtr copyRegion,
                     int dx, int dy);

void rfbMarkRectAsModified(rfbScreenInfoPtr rfbScreen, int x1, int y1, int x2,
                           int y2);
...
extern rfbScreenInfoPtr rfbGetScreen(int* argc, char** argv, int width,
                                     int height, int bitsPerSample,
                                     int samplesPerPixel, int bytesPerPixel);
extern void rfbInitServer(rfbScreenInfoPtr rfbScreen);
extern void rfbShutdownServer(rfbScreenInfoPtr rfbScreen,
...
extern void rfbRunEventLoop(rfbScreenInfoPtr screenInfo, long usec,
...
extern void rfbStatRecordEncodingSent(rfbClientPtr cl, uint32_t type,
                                      int byteCount, int byteIfRaw);
extern void rfbStatRecordEncodingSentAdd(
    rfbClientPtr cl, uint32_t type,
    int byteCount); /* Specifically for tight encoding */
extern void rfbStatRecordEncodingRcvd(rfbClientPtr cl, uint32_t type,
                                      int byteCount, int byteIfRaw);
extern void rfbStatRecordMessageSent(rfbClientPtr cl, uint32_t type,
                                     int byteCount, int byteIfRaw);
extern void rfbStatRecordMessageRcvd(rfbClientPtr cl, uint32_t type,
                                     int byteCount, int byteIfRaw);
extern void rfbResetStats(rfbClientPtr cl);
extern void rfbPrintStats(rfbClientPtr cl);
...
/** Set which version you want to advertise 3.3, 3.6, 3.7 and 3.8 are currently
 * supported*/
extern void rfbSetProtocolVersion(rfbScreenInfoPtr rfbScreen, int major_,
                                  int minor_);

/** send a TextChat message to a client */
extern rfbBool rfbSendTextChatMessage(rfbClientPtr cl, uint32_t length,
                                      char* buffer);
...
/**
 ...
 You then can set hooks and io functions (see @ref making_it_interactive) or
 other options (see @ref server_options).
 ...
 rfbScreenInfo::kbdAddEvent(), rfbScreenInfo::kbdReleaseAllKeys(),
 rfbScreenInfo::ptrAddEvent() and rfbScreenInfo::setXCutText()
 ...
 The rfbCursor structure consists mainly of a mask and a source. The
 rfbCursor::mask describes, which pixels are drawn for the cursor (a cursor
 needn't be rectangular). The rfbCursor::source describes, which colour those
 pixels should have.
 ...
 @endcode
 You can even set rfbCursor::mask to NULL in this call and LibVNCServer will
 calculate a mask for you (dynamically, so you have to free it yourself).

 There is also an array named rfbCursor::richSource for colourful cursors. They
 have the same format as the frameBuffer (i.e. if the server is 32 bit, a 10x4
 cursor has 4x10x4 bytes).

 @section screen_client_difference What is the difference between
 rfbScreenInfoPtr and rfbClientPtr?
 ...
 frame buffer etc. The rfbClientPtr is a pointer to an rfbClientRec structure,
 which holds information about a client, like pixel format, socket of the
 connection, etc. A server can have several clients, but needn't have any. So,
 if you have a server and three clients are connected, you have one instance of
 a rfbScreenInfo and three instances of rfbClientRec's.

 The rfbClientRec structure holds a member rfbClientRec::screen which points to
 the server. So, to access the server from the client structure, you use
 client->screen.
