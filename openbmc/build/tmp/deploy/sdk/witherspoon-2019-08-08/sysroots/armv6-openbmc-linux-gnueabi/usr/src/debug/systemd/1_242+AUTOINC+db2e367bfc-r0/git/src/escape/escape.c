...
static int help(void)
{
    ...
    printf(
        ...
        "     --instance           With --unescape, show just the instance "
        "part\n"
        "  -u --unescape           Unescape strings\n"
        "  -m --mangle             Mangle strings\n"
        "  -p --path               When escaping/unescaping assume the string "
        "is a path\n"
        ...
        switch (arg_action)
        {
            ...
            case ACTION_ESCAPE:
                ...
                if (arg_template)
                {
                    ...
                    if (r < 0)
                        return log_error_errno(
                            r, "Failed to replace instance: %m");
            ...
            case ACTION_UNESCAPE:
            {
                ...
                if (arg_template || arg_instance)
                {
                    ...
                    if (r < 0)
                        return log_error_errno(
                            r, "Failed to extract instance: %m");
                    if (isempty(name))
                        return log_error(
                    ...
                    if (r < 0)
                        return log_error_errno(
                            r, "Failed to extract template: %m");
                    if (arg_template && !streq(arg_template, template))
                        return log_error("Unit %s template %s does not match "
                                         "specified template %s.",
