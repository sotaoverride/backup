...
{
    int err =
        mtd_write(mtd_desc, &mtd, fd, ebnum, 0, iobuf, mtd.eb_size, NULL, 0, 0);
    if (err)
...
static int write_eraseblock_by_page(int ebnum)
{
    ...
    for (i = 0; i < pgcnt; ++i)
    {
        err = mtd_write(mtd_desc, &mtd, fd, ebnum, i * pgsize, buf, pgsize,
                        NULL, 0, 0);
        if (err)
        {
...
static int write_eraseblock_by_2pages(int ebnum)
{
    ...
    for (i = 0; i < n; ++i)
    {
        err = mtd_write(mtd_desc, &mtd, fd, ebnum, i * sz, buf, sz, NULL, 0, 0);
        if (err)
        {
    ...
    if (pgcnt % 2)
    {
        err = mtd_write(mtd_desc, &mtd, fd, ebnum, i * sz, buf, pgsize, NULL, 0,
                        0);
        if (err)
        {
