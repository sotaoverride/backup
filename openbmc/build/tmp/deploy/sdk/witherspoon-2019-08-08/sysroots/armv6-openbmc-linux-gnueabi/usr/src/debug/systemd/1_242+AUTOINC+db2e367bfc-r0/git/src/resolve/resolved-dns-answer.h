...
typedef enum DnsAnswerFlags
{
    ...
    DNS_ANSWER_CACHE_FLUSH = 1 << 3, /* For mDNS: sets cache-flush bit in the
                                        rrclass of response records */
...
#define _DNS_ANSWER_FOREACH(q, kk, a)                                          \
    for (size_t UNIQ_T(i, q) = ({                                              \
         ...
         UNIQ_T(i, q)++, (kk) = (UNIQ_T(i, q) < (a)->n_rrs                     \
                                     ? (a)->items[UNIQ_T(i, q)].rr             \
                                     : NULL))
...
#define _DNS_ANSWER_FOREACH_IFINDEX(q, kk, ifi, a)                             \
    for (size_t UNIQ_T(i, q) = ({                                              \
         ...
         (a) && (UNIQ_T(i, q) < (a)->n_rrs); UNIQ_T(i, q)++,                   \
                          (kk) = ((UNIQ_T(i, q) < (a)->n_rrs)                  \
                                      ? (a)->items[UNIQ_T(i, q)].rr            \
                                      : NULL),                                 \
                          (ifi) = ((UNIQ_T(i, q) < (a)->n_rrs)                 \
                                       ? (a)->items[UNIQ_T(i, q)].ifindex      \
                                       : 0))

#define DNS_ANSWER_FOREACH_IFINDEX(kk, ifindex, a)                             \
    _DNS_ANSWER_FOREACH_IFINDEX(UNIQ, kk, ifindex, a)
    ...
    for (size_t UNIQ_T(i, q) = ({                                              \
         ...
         (a) && (UNIQ_T(i, q) < (a)->n_rrs); UNIQ_T(i, q)++,                   \
                          (kk) = ((UNIQ_T(i, q) < (a)->n_rrs)                  \
                                      ? (a)->items[UNIQ_T(i, q)].rr            \
                                      : NULL),                                 \
                          (fl) = ((UNIQ_T(i, q) < (a)->n_rrs)                  \
                                      ? (a)->items[UNIQ_T(i, q)].flags         \
                                      : 0))

#define DNS_ANSWER_FOREACH_FLAGS(kk, flags, a)                                 \
    _DNS_ANSWER_FOREACH_FLAGS(UNIQ, kk, flags, a)
...
#define DNS_ANSWER_FOREACH_FULL(kk, ifindex, flags, a)                         \
    _DNS_ANSWER_FOREACH_FULL(UNIQ, kk, ifindex, flags, a)
