...
static const char* rtcwake_mode_string[] = {[OFF_MODE] = "off",
                                            [NO_MODE] = "no",
                                            [ON_MODE] = "on",
...
static void __attribute__((__noreturn__)) usage(void)
{
    ...
    fputs(_(" -a, --auto               reads the clock mode from adjust file "
            "(default)\n"),
          out);
    fprintf(
        out,
...
static int get_basetimes(struct rtcwake_control* ctl, int fd)
{
    ...
    if (ctl->verbose)
    {
        ...
        printf("\tsystime = %ld, (UTC) %s", (long)ctl->sys_time,
               asctime(gmtime(&ctl->sys_time)));
        printf("\trtctime = %ld, (UTC) %s", (long)ctl->rtc_time, asctime(&tm));
    }
    return 0;
}
...
{
    struct rtcwake_control ctl = {.mode_str = "suspend", /* default mode */
    ...
    while ((t = getopt_long(argc, argv, "A:ahd:lm:ns:t:uVv", long_options,
                            NULL)) != EOF)
    {
        ...
        switch (t)
        {
            ...
            case 's':
                ...
                seconds =
                    strtou32_or_err(optarg, _("invalid seconds argument"));
                break;
            case 't':
    ...
    if (!alarm && !seconds && suspend != DISABLE_MODE && suspend != SHOW_MODE)
        errx(EXIT_FAILURE, _("must provide wake time (see --seconds, --time "
                             "and --date options)"));
    ...
    if (ctl.verbose)
        printf(_("alarm %ld, sys_time %ld, rtc_time %ld, seconds %u\n"), alarm,
               ctl.sys_time, ctl.rtc_time, seconds);
