...
class attribute_set_reference_proxy
{
  ...
  public:
    //! Constructor
    explicit attribute_set_reference_proxy(attribute_set* pContainer,
                                           key_type const& key) BOOST_NOEXCEPT
        : m_pContainer(pContainer),
          m_key(key)
    {
    }

    //! Conversion operator (would be invoked in case of reading from the
    //! container)
    BOOST_FORCEINLINE operator mapped_type() const BOOST_NOEXCEPT
    {
        return read_mapped_value();
    }
    //! Assignment operator (would be invoked in case of writing to the
    //! container)
 ...
 * pointer to the attribute as a mapped value. The container allows storing only
 * one element for each distinct key value. In most regards attribute set
 * container provides interface similar to \c std::unordered_map. However, there
 * are differences in \c operator[] semantics and a number of optimizations with
 * regard to iteration. Besides, attribute names are stored as a read-only
 * <tt>attribute_name</tt>'s instead of \c std::string, which saves memory and
 * CPU time.
 */
class attribute_set
{
  ...
  private:
    ...
    class iter
    {
      ...
      public:
        ...
        typedef typename mpl::if_c<fConstV, attribute_set::const_reference,
                                   attribute_set::reference>::type reference;
        typedef typename mpl::if_c<fConstV, attribute_set::const_pointer,
                                   attribute_set::pointer>::type pointer;
      ...
      public:
        ...
        //  Comparison
        template <bool f>
        bool operator==(iter<f> const& that) const BOOST_NOEXCEPT
        {
        ...
        node_base* base() const BOOST_NOEXCEPT
        {
            return m_pNode;
        }
...
#else
    /*!
     * Iterator type. The iterator complies to the bidirectional iterator
     * requirements.
     */
    typedef implementation_defined iterator;
    /*!
     * Constant iterator type. The iterator complies to the bidirectional
     * iterator requirements with read-only capabilities.
     */
  ...
  public:
     ...
     * Copy constructor.
     *
     * \post <tt>size() == that.size() && std::equal(begin(), end(),
     * that.begin()) == true</tt>
     */
     ...
     * \return Iterator to the found element or end() if the attribute with such
     * name is not found.
     */
    ...
    /*!
     * The method counts the number of the attribute occurrences in the
     * container. Since there can be only one attribute with a particular key,
    ...
    /*!
     * Combined lookup/insertion operator. The operator semantics depends on the
     * further usage of the returned reference. \li If the reference is used as
     * an assignment target, the assignment expression is equivalent to element
     * insertion, where the element is composed of the second argument of the \c
     * operator[] as a key and the second argument of assignment as a mapped
     * value. \li If the returned reference is used in context where a
     * conversion to the mapped type is required, the result of the conversion
     * is equivalent to the mapped value found with the second argument of the
     * \c operator[] as a key, if such an element exists in the container, or a
     * default-constructed mapped value, if an element does not exist in the
     *     container.
     *
     * \param key Attribute name.
     ...
     *
     * \param key Attribute name.
     * \return If an element with the corresponding attribute name is found in
     * the container, its mapped value is returned. Otherwise a
     * default-constructed mapped value is returned.
     */
     ...
     * \returns A pair of values. If second is true, the insertion succeeded and
     * the first component points to the inserted element. Otherwise the first
     * component points to the element that prevents insertion.
     */
    BOOST_LOG_API std::pair<iterator, bool> insert(key_type key,
     ...
     *
     * \pre \a end is reachable from \a begin with a finite number of
     * increments. \post All iterators to the erased elements become invalid.
     * \param begin An iterator that points to the first element to be erased.
     * \param end An iterator that points to the after-the-last element to be
     * erased.
     */
...
//! Reads the referenced mapped value from the container
inline attribute_set_reference_proxy::mapped_type
    attribute_set_reference_proxy::read_mapped_value() const BOOST_NOEXCEPT
{
...
inline attribute_set_reference_proxy::mapped_type&
    attribute_set_reference_proxy::operator=(mapped_type const& val) const
{
    std::pair<attribute_set::iterator, bool> res =
        m_pContainer->insert(m_key, val);
    if (!res.second)
...
inline attribute& attribute::
    operator=(aux::attribute_set_reference_proxy const& that) BOOST_NOEXCEPT
{
    attribute attr = that;
    this->swap(attr);
    return *this;
