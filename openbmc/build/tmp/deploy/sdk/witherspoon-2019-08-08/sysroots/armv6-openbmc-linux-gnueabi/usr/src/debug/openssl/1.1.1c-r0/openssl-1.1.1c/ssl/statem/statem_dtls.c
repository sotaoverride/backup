...
static unsigned char bitmask_start_values[] = {0xff, 0xfe, 0xfc, 0xf8,
                                               0xf0, 0xe0, 0xc0, 0x80};
static unsigned char bitmask_end_values[] = {0xff, 0x01, 0x03, 0x07,
                                             0x0f, 0x1f, 0x3f, 0x7f};

static void dtls1_fix_message_header(SSL* s, size_t frag_off, size_t frag_len);
static unsigned char* dtls1_write_message_header(SSL* s, unsigned char* p);
static void dtls1_set_message_header_int(SSL* s, unsigned char mt, size_t len,
...
int dtls1_do_write(SSL* s, int type)
{
    ...
    while (s->init_num > 0)
    {
        ...
        used_len = BIO_wpending(s->wbio) + DTLS1_RT_HEADER_LENGTH + mac_size +
                   blocksize;
        ...
        if (ret < 0)
        {
             ...
             */
            if (retry && BIO_ctrl(SSL_get_wbio(s), BIO_CTRL_DGRAM_MTU_EXCEEDED,
                                  0, NULL) > 0)
            {
...
again:
    ...
    *len = s->init_num;
    ...
    if (*mt == SSL3_MT_CHANGE_CIPHER_SPEC)
    {
        if (s->msg_callback)
        {
            s->msg_callback(0, s->version, SSL3_RT_CHANGE_CIPHER_SPEC, p, 1, s,
                            s->msg_callback_arg);
        }
        /*
    ...
    if (frag->reassembly == NULL)
    {
        ...
        while (frag_len)
        {
            i = s->method->ssl_read_bytes(
                s, SSL3_RT_HANDSHAKE, NULL, devnull,
...
redo:
    ...
    if (!s->server && s->d1->r_msg_hdr.frag_off == 0 &&
        s->statem.hand_state != TLS_ST_OK && wire[0] == SSL3_MT_HELLO_REQUEST)
    {
        /*
    ...
    if (frag_len > 0)
    {
        ...
        i = s->method->ssl_read_bytes(s, SSL3_RT_HANDSHAKE, NULL, &p[frag_off],
...
int dtls1_read_failed(SSL* s, int code)
{
    if (code > 0)
    {
        SSLfatal(s, SSL_AD_INTERNAL_ERROR, SSL_F_DTLS1_READ_FAILED,
                 ERR_R_INTERNAL_ERROR);
        return 0;
    }
...
int dtls1_buffer_message(SSL* s, int is_ccs)
{
    ...
    {
        if (!ossl_assert(s->d1->w_msg_hdr.msg_len + DTLS1_HM_HEADER_LENGTH ==
                         (unsigned int)s->init_num))
            return 0;
    }
    ...
    memset(seq64be, 0, sizeof(seq64be));
    seq64be[6] =
        (unsigned char)(dtls1_get_queue_priority(frag->msg_header.seq,
...
int dtls1_retransmit_message(SSL* s, unsigned short seq, int* found)
{
    ...
    *found = 1;
    ...
    DTLS_RECORD_LAYER_set_saved_w_epoch(
        &s->rlayer, frag->msg_header.saved_retransmit_state.epoch);
...
int dtls1_close_construct_packet(SSL* s, WPACKET* pkt, int htype)
{
    ...
    if (htype != DTLS1_MT_HELLO_VERIFY_REQUEST)
    {
        ...
        if (!dtls1_buffer_message(s,
                                  htype == SSL3_MT_CHANGE_CIPHER_SPEC ? 1 : 0))
            return 0;
    }
