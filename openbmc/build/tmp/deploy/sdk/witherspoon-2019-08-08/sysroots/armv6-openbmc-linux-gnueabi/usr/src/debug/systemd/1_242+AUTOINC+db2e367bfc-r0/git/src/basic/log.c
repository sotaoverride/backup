...
static int create_log_socket(int type)
{
    ...
    /* We need a blocking fd here since we'd otherwise lose messages way too
     * early. However, let's not hang forever in the unlikely case of a
     * deadlock. */
    if (getpid_cached() == 1)
...
int log_open(void)
{
    ...
    if (log_target != LOG_TARGET_AUTO || getpid_cached() == 1 ||
        isatty(STDERR_FILENO) <= 0)
    {
        ...
        if (!prohibit_ipc &&
            IN_SET(log_target, LOG_TARGET_SYSLOG_OR_KMSG, LOG_TARGET_SYSLOG))
        {
            r = log_open_syslog();
            if (r >= 0)
            {
        ...
        if (IN_SET(log_target, LOG_TARGET_AUTO, LOG_TARGET_JOURNAL_OR_KMSG,
                   LOG_TARGET_SYSLOG_OR_KMSG, LOG_TARGET_KMSG))
        {
            r = log_open_kmsg();
            if (r >= 0)
            {
...
static int write_to_console(int level, int error, const char* file, int line,
                            const char* func, const char* buffer)
{
...
static int write_to_syslog(int level, int error, const char* file, int line,
                           const char* func, const char* buffer)
{

    char header_priority[2 + DECIMAL_STR_MAX(int) + 1], header_time[64],
...
static int write_to_kmsg(int level, int error, const char* file, int line,
                         const char* func, const char* buffer)
{
...
static int log_do_header(char* header, size_t size, int level, int error,
                         ...
                         const char* extra_field, const char* extra)
{
    ...
    r = snprintf(
        ...
        LOG_PRI(level), LOG_FAC(level),
        isempty(file) ? "" : "CODE_FILE=", isempty(file) ? "" : file,
        isempty(file) ? "" : "\n", line ? "CODE_LINE=" : "", line ? 1 : 0,
        line, /* %.0d means no output too, special case for 0 */
        line ? "\n" : "",
        isempty(func) ? "" : "CODE_FUNC=", isempty(func) ? "" : func,
        isempty(func) ? "" : "\n", error ? "ERRNO=" : "", error ? 1 : 0, error,
        error ? "\n" : "", isempty(object) ? "" : object_field,
        isempty(object) ? "" : object, isempty(object) ? "" : "\n",
        isempty(extra) ? "" : extra_field, isempty(extra) ? "" : extra,
...
static int write_to_journal(int level, int error, const char* file, int line,
                            const char* func, const char* object_field,
                            const char* object, const char* extra_field,
                            const char* extra, const char* buffer)
{
    ...
    log_do_header(header, sizeof(header), level, error, file, line, func,
                  object_field, object, extra_field, extra);
...
int log_dispatch_internal(int level, int error, const char* file, int line,
                          const char* func, const char* object_field,
                          const char* object, const char* extra_field,
                          const char* extra, char* buffer)
{
    ...
    do
    {
                   ...
                   LOG_TARGET_JOURNAL))
        {
            ...
            k = write_to_journal(level, error, file, line, func, object_field,
                                 object, extra_field, extra, buffer);
            if (k < 0 && k != -EAGAIN)
                log_close_journal();
        }
        ...
        if (k <= 0 &&
            IN_SET(log_target, LOG_TARGET_AUTO, LOG_TARGET_SYSLOG_OR_KMSG,
                   LOG_TARGET_JOURNAL_OR_KMSG, LOG_TARGET_KMSG))
        {
                      ...
                      const char* func, char* buffer)
{
    ...
    return log_dispatch_internal(level, error, file, line, func, NULL, NULL,
                                 NULL, NULL, buffer);
}
...
int log_object_internalv(int level, int error, const char* file, int line,
                         const char* func, const char* object_field,
                         const char* object, const char* extra_field,
                         const char* extra, const char* format, va_list ap)
{
    ...
    return log_dispatch_internal(level, error, file, line, func, object_field,
                                 object, extra_field, extra, buffer);
}
...
int log_object_internal(int level, int error, const char* file, int line,
                        const char* func, const char* object_field,
                        const char* object, const char* extra_field,
                        const char* extra, const char* format, ...)
{
    ...
    va_start(ap, format);
    r = log_object_internalv(level, error, file, line, func, object_field,
                             object, extra_field, extra, format, ap);
    va_end(ap);

    return r;
}

static void log_assert(int level, const char* text, const char* file, int line,
                       const char* func, const char* format)
{
    ...
    log_dispatch_internal(level, 0, file, line, func, NULL, NULL, NULL, NULL,
                          buffer);
}
                                                    ...
                                                    const char* func)
{
    log_open();
    log_assert(
        LOG_REALM_PLUS_LEVEL(realm, LOG_CRIT), text, file, line, func,
...
int log_oom_internal(LogRealm realm, const char* file, int line,
                     const char* func)
{
    return log_internal_realm(LOG_REALM_PLUS_LEVEL(realm, LOG_ERR), ENOMEM,
                              file, line, func, "Out of memory.");
}

int log_format_iovec(struct iovec* iovec, size_t iovec_len, size_t* n,
                     bool newline_separator, int error, const char* format,
                     va_list ap)
{
                        ...
                        const char* func, const char* format, ...)
{
               ...
               LOG_TARGET_JOURNAL))
    {
        ...
        if (journal_fd >= 0)
        {
            ...
            log_do_header(header, sizeof(header), level, error, file, line,
                          func, NULL, NULL, NULL, NULL);
            iovec[n++] = IOVEC_MAKE_STRING(header);

            va_start(ap, format);
            r = log_format_iovec(iovec, ELEMENTSOF(iovec), &n, true, error,
                                 format, ap);
            if (r < 0)
                fallback = true;
            else
                              ...
                              size_t n_input_iovec)
{
    ...
    for (i = 0; i < n_input_iovec; i++)
        if (memory_startswith(input_iovec[i].iov_base, input_iovec[i].iov_len,
                              "MESSAGE="))
            break;
                                   ...
                                   void* data)
{
    ...
    else if (proc_cmdline_key_streq(key, "systemd.log_color"))
    {
        ...
        if (log_show_color_from_string(value ?: "1") < 0)
            log_warning("Failed to parse log color setting '%s'. Ignoring.",
                        value);
    }
    else if (proc_cmdline_key_streq(key, "systemd.log_location"))
    {
        ...
        if (log_show_location_from_string(value ?: "1") < 0)
            log_warning("Failed to parse log location setting '%s'. Ignoring.",
                        value);
    }
...
void log_parse_environment_realm(LogRealm realm)
{
    ...
    if (get_ctty_devnr(0, NULL) < 0)
        /* Only try to read the command line in daemons.  We assume that
           anything that has a controlling tty is user stuff. */
        (void)proc_cmdline_parse(parse_proc_cmdline_item, NULL,
...
void log_received_signal(int level, const struct signalfd_siginfo* si)
{
    ...
    if (pid_is_valid(si->ssi_pid))
    {
        ...
        log_full(level, "Received SIG%s from PID %" PRIu32 " (%s).",
                 signal_to_string(si->ssi_signo), si->ssi_pid, strna(p));
    }
    else
                        ...
                        int line, const char* func, const char* format, ...)
{
    ...
    if (config_file)
        return log_struct_internal(
            ...
            "CONFIG_FILE=%s", config_file, "CONFIG_LINE=%u", config_line,
        ...
        return log_struct_internal(
            LOG_REALM_PLUS_LEVEL(LOG_REALM_SYSTEMD, level), error, file, line,
            func, "MESSAGE_ID=" SD_MESSAGE_INVALID_CONFIGURATION_STR,
            LOG_MESSAGE("%s: %s", unit, buffer), unit_fmt, unit, NULL);
    else
        return log_struct_internal(
            LOG_REALM_PLUS_LEVEL(LOG_REALM_SYSTEMD, level), error, file, line,
            func, "MESSAGE_ID=" SD_MESSAGE_INVALID_CONFIGURATION_STR,
...
int log_syntax_invalid_utf8_internal(const char* unit, int level,
                                     const char* config_file,
                                     unsigned config_line, const char* file,
                                     ...
                                     const char* rvalue)
{
    ...
    log_syntax_internal(
        unit, level, config_file, config_line, 0, file, line, func,
...
{
    /* Returns the log level to use for log_emergency() logging. We use
     * LOG_EMERG only when we are PID 1, as only then the system of the whole
     * system is obviously affected. */
...
int log_dup_console(void)
{
    ...
    /* Duplicate the fd we use for fd logging if it's < 3 and use the copy from
     * now on. This call is useful
     * whenever we want to continue logging through the original fd, but want to
     * rearrange stderr. */
...
{
    /* Sets up logging the way it is most appropriate for running a program as a
     * service. Note that using this doesn't make the binary unsuitable for
     * invocation on the command line, as log output will still go to the
