 ...
 * SECTION: ask
 * @title: Ask
 * @short_description: interface for dialog driven partitioning, warning and
 * info messages
 *
 */
...
int fdisk_do_ask(struct fdisk_context* cxt, struct fdisk_ask* ask)
{
    ...
    if (!fdisk_has_dialogs(cxt) &&
        !(ask->type == FDISK_ASKTYPE_INFO || ask->type == FDISK_ASKTYPE_WARNX ||
          ask->type == FDISK_ASKTYPE_WARN))
    {
...
static char* mk_string_list(char* ptr, size_t* len, size_t* begin, size_t* run,
                            ssize_t cur, int inchar)
{
    int rlen;
    ...
    if (!*run)
        ...
        rlen = inchar ? snprintf(ptr, *len, "%c,%c,", tochar(*begin),
                                 tochar(*begin + 1))
...
int fdisk_ask_partnum(struct fdisk_context* cxt, size_t* partnum, int wantnew)
{
    ...
    DBG(ASK, ul_debugobj(ask,
                         "ask limits: low: %" PRIu64 ", high: %" PRIu64
                         ", default: %" PRIu64 "",
...
int fdisk_ask_menu_add_item(struct fdisk_ask* ask, int key, const char* name,
                            const char* desc)
{
    struct ask_menuitem* mi;
...
#define is_print_ask(a)                                                        \
    (fdisk_is_ask(a, WARN) || fdisk_is_ask(a, WARNX) || fdisk_is_ask(a, INFO))
...
int fdisk_info_new_partition(struct fdisk_context* cxt, int num,
                             fdisk_sector_t start, fdisk_sector_t stop,
                             struct fdisk_parttype* t)
{
    ...
    rc = fdisk_info(
        cxt, _("Created a new partition %d of type '%s' and of size %s."), num,
        t ? t->name : _("Unknown"), str);
    free(str);
    return rc;
}
...
{
    struct fdisk_test tss[] = {{"--ranges", test_ranges, "generates ranges"},
                               {NULL}};
