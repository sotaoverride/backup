...
static void extended_com_err_fn(const char* prog, errcode_t code,
...
int main(int argc, char* argv[])
{
    ...
    {
        if (show_flags || status_only || show_addresses || show_all || list_all)
            usage();
    }
...
static void do_keytab(const char* name)
{
    krb5_error_code ret;
    krb5_keytab kt;
...
static int list_ccache(krb5_ccache cache)
{
    krb5_error_code ret;
...
static int show_ccache(krb5_ccache cache)
{
    krb5_cc_cursor cur;
    krb5_creds creds;
...
static int check_ccache(krb5_ccache cache)
{
    krb5_error_code ret;
    krb5_cc_cursor cur;
    krb5_creds creds;
...
static void printtime(krb5_timestamp ts)
{
    char timestring[BUFSIZ], fill = ' ';

    if (!krb5_timestamp_to_sfstring(ts, timestring, timestamp_width + 1, &fill))
        printf("%s", timestring);
}

static void print_config_data(int col, krb5_data* data)
{
    unsigned int i;
...
static void show_credential(krb5_creds* cred)
{
    krb5_error_code ret;
    krb5_ticket* tkt;
...
static void one_addr(krb5_address* a)
{
    ...
    switch (a->addrtype)
    {
        case ADDRTYPE_INET:
            if (a->length != 4)
            {
                printf(_("broken address (type %d length %d)"), a->addrtype,
                       a->length);
                return;
            }
            sinp = ss2sin(&ss);
    ...
    err = getnameinfo(ss2sa(&ss), sa_socklen(ss2sa(&ss)), namebuf,
                      sizeof(namebuf), 0, 0, no_resolve ? NI_NUMERICHOST : 0U);
    if (err)
    {
...
static void fillit(FILE* f, unsigned int num, int c)
{
    unsigned int i;
