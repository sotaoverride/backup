...
#define LENGTHES(io, s)                                                        \
    {1,                                                                        \
     ...
     2 * MAX_NAND_PAGE_SIZE,                                                   \
     2 * MAX_NAND_PAGE_SIZE - (io),                                            \
     (s) / 2 - 1,                                                              \
     (s) / 2,                                                                  \
     (s) / 2 + 1,                                                              \
     (s)-1,                                                                    \
...
static int test_read3(const struct ubi_vol_info* vol_info, int len, off_t off)
{
    ...
    if (new_off != off + len1)
    {
        if (new_off == -1)
            ...
            errorm("read %d bytes from %lld, but resulting "
                   "offset is %lld",
                   len1, (long long)off, (long long)new_off);
        goto out;
    }
    ...
    {
        errorm("incorrect data read from offset %lld", (long long)off);
        errorm("len = %d", len);
        goto out;
    }
...
static int test_read2(const struct ubi_vol_info* vol_info, int len)
{
    int i;
    off_t offsets[] =
        OFFSETS(dev_info.min_io_size, vol_info->leb_size, vol_info->data_bytes);
