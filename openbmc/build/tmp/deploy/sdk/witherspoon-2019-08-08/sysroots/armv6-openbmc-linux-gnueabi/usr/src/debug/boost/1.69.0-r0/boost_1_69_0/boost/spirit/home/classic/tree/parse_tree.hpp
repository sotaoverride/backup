...
//////////////////////////////////
// pt_match_policy is simply an id so the correct specialization of tree_policy
// can be found.
template <typename IteratorT, typename NodeFactoryT, typename T>
struct pt_match_policy
    : public common_tree_match_policy<
          pt_match_policy<IteratorT, NodeFactoryT, T>, IteratorT, NodeFactoryT,
          pt_tree_policy<pt_match_policy<IteratorT, NodeFactoryT, T>,
                         NodeFactoryT, T>,
          T>
{
    ...
    : public common_tree_tree_policy<MatchPolicyT, NodeFactoryT>
{
                            ...
                            Iterator1T const& first, Iterator2T const& last)
    {
        ...
        for (cont_iterator_t i = newmatch.trees.begin()->children.begin();
             i != newmatch.trees.begin()->children.end(); ++i)
        {
            if (i->value.id() == 0)
                i->value.id(id);
        }
...
{
    typedef pt_tree_policy<pt_match_policy<IteratorT, NodeFactoryT, T>,
                           NodeFactoryT, T>
        type;
};
...
inline tree_parse_info<IteratorT, NodeFactoryT>
    pt_parse(IteratorT const& first_, IteratorT const& last,
             parser<ParserT> const& p, SkipT const& skip,
...
inline tree_parse_info<IteratorT>
    pt_parse(IteratorT const& first, IteratorT const& last,
             parser<ParserT> const& p, SkipT const& skip)
{
...
inline tree_parse_info<IteratorT> pt_parse(IteratorT const& first_,
                                           IteratorT const& last,
                                           parser<ParserT> const& parser)
{
    typedef pt_match_policy<IteratorT> pt_match_policy_t;
    IteratorT first = first_;
    scanner<IteratorT, scanner_policies<iteration_policy, pt_match_policy_t>>
        scan(first, last);
...
inline tree_parse_info<CharT const*>
    pt_parse(CharT const* str, parser<ParserT> const& p, SkipT const& skip)
{
...
inline tree_parse_info<CharT const*> pt_parse(CharT const* str,
                                              parser<ParserT> const& parser)
{
    CharT const* last = str;
    while (*last)
