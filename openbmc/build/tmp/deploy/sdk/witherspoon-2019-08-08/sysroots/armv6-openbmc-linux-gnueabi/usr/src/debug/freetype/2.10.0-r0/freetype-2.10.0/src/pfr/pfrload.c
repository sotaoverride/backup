...
FT_LOCAL_DEF(FT_Error)
pfr_extra_items_parse(FT_Byte** pp, FT_Byte* limit, PFR_ExtraItem item_list,
...
pfr_header_load(PFR_Header header, FT_Stream stream)
{
    ...
    if (!FT_STREAM_SEEK(0) && !FT_STREAM_READ_FIELDS(pfr_header_fields, header))
    {
        /* make a few adjustments to the header */
        header->phy_font_max_size += (FT_UInt32)header->phy_font_max_size_high
                                     << 16;
    }
...
pfr_header_check(PFR_Header header)
{
    ...
    if (header->signature != 0x50465230L || /* "PFR0" */
        header->version > 4 || header->header_size < 58 ||
...
FT_LOCAL_DEF(FT_Error)
pfr_log_font_load(PFR_LogFont log_font, FT_Stream stream, FT_UInt idx,
                  FT_UInt32 section_offset, FT_Bool size_increment)
{
    ...
    if (FT_STREAM_SKIP(idx * 5) || FT_READ_USHORT(size) ||
...
pfr_extra_item_load_bitmap_info(FT_Byte* p, FT_Byte* limit,
                                PFR_PhyFont phy_font)
{
    ...
    {
        strike->x_ppm = (flags0 & PFR_STRIKE_2BYTE_XPPM) ? PFR_NEXT_USHORT(p)
                                                         : PFR_NEXT_BYTE(p);

        strike->y_ppm = (flags0 & PFR_STRIKE_2BYTE_YPPM) ? PFR_NEXT_USHORT(p)
                                                         : PFR_NEXT_BYTE(p);
...
FT_CALLBACK_DEF(FT_Error)
pfr_extra_item_load_kerning_pairs(FT_Byte* p, FT_Byte* limit,
                                  PFR_PhyFont phy_font)
{
    ...
    item->offset = phy_font->offset + (FT_Offset)(p - phy_font->cursor);
 ...
 */
static FT_Error pfr_aux_name_load(FT_Byte* p, FT_UInt len, FT_Memory memory,
                                  FT_String** astring)
{
    FT_Error error = FT_Err_Ok;
...
FT_LOCAL_DEF(FT_Error)
pfr_phy_font_load(PFR_PhyFont phy_font, FT_Stream stream, FT_UInt32 offset,
                  FT_UInt32 size)
{
    FT_Error error;
    ...
    {
        error = pfr_extra_items_parse(&p, limit, pfr_phy_font_extra_items,
                                      phy_font);
        ...
        while (num_aux > 0)
        {
            ...
            switch (type)
            {
                case 1:
                     ...
                     * padded to */
                    /* an even number of bytes with a zero byte appended if
                     * needed   */
                ...
                case 3:
                    /* this seems to correspond to the font's style name, padded
                     * to */
                    /* an even number of bytes with a zero byte appended if
                     * needed  */
                    error = pfr_aux_name_load(q, length - 4U, memory,
