...
static inline void fputs_quoted_case_json(const char* data, FILE* out, int dir)
{
    ...
    for (p = data; p && *p; p++)
    {
        ...
        if (c >= 0x20)
        {
            fputc(dir == 1 ? toupper(c) : dir == -1 ? tolower(c) : *p, out);
            continue;
        }
...
static inline void fputs_quoted_case(const char* data, FILE* out, int dir)
{
    ...
    for (p = data; p && *p; p++)
    {
        if ((unsigned char)*p == 0x22 || /* " */
            ...
            !isprint((unsigned char)*p) || iscntrl((unsigned char)*p))
        {

            fprintf(out, "\\x%02x", (unsigned char)*p);
        }
        else
            fputc(dir == 1 ? toupper(*p) : dir == -1 ? tolower(*p) : *p, out);
    }
    fputc('"', out);
}
...
static inline void fputs_nonblank(const char* data, FILE* out)
{
    ...
    for (p = data; p && *p; p++)
    {
        if (isblank((unsigned char)*p) || (unsigned char)*p == 0x5c || /* \ */
            !isprint((unsigned char)*p) || iscntrl((unsigned char)*p))
        {
