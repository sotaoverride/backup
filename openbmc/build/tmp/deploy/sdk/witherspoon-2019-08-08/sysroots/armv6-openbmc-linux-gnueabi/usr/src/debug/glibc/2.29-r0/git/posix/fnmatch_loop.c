                ...
                else
                {
                    ...
                    else if (c == L('/') && (flags & FNM_FILE_NAME))
                    {
                        while (n < string_end && *n != L('/'))
                            ++n;
                        if (n < string_end && *n == L('/') &&
                            (FCT(p, n + 1, string_end, flags & FNM_PERIOD,
                                 flags, NULL, alloca_used) == 0))
                            return 0;
                    }
                    else
                    {
                        ...
                        for (--p; n < endp; ++n, no_leading_period = 0)
                            if (FOLD((UCHAR)*n) == c &&
                                (FCT(p, n, string_end, no_leading_period,
                                     flags2, &end, alloca_used) == 0))
                            {
                            found:
                                if (end.pattern == NULL)
                                    return 0;
                    ...
                    else if (c == L('[') && *p == L(':'))
                    {
                        ...
                        for (;;)
                        {
                            if (c1 == CHAR_CLASS_MAX_LENGTH)
                                /* The name is too long and therefore the
                            ...
                            if (c < L('a') || c >= L('z'))
                            {
                                /* This cannot possibly be a character class
                                   name. Match it as a normal range.  */
                                p = startp;
                                c = L('[');
                                goto normal_bracket;
                            }
...
#define str (startp + 1)
                                ...
                                if (symb_table[2 * elem] != 0)
                                {
                                    ...
                                    {
#if WIDE_CHAR_VERSION
                                        for (c1 = 0; (int32_t)c1 < wextra[idx];
                                             ++c1)
                                            if (n[c1] != wextra[1 + c1])
                                                break;
...
#undef str
                        ...
                        if (c == L('-') && *p != L(']'))
                        {
                            ...
                            if (fcollseq == ~((uint32_t)0))
                                /* XXX We don't know anything about the
                                   character we are supposed to match.  This
                                   means we are failing.  */
                            ...
                            if (is_seqval)
                                ...
                                lcollseq =
                                    __collseq_table_lookup(collseq, cold);
#else
                            fcollseq = collseq[fn];
...
#if WIDE_CHAR_VERSION
                                    /* We have to convert the name to a
                                       single-byte string.  This is possible
                                       since the names consist of ASCII
                                       characters and the internal
                                       representation is UCS4.  */
                                    for (strcnt = 0; strcnt < c1; ++strcnt)
                                        str[strcnt] = startp[1 + strcnt];
#endif
...
#define NEW_PATTERN                                                            \
    ...
    size_t slen =                                                              \
        (opt == L('?') || opt == L('@') ? pattern_len : (p - startp + 1));     \
            ...
            do
            {
                for (rs = string; rs <= string_end; ++rs)
                    ...
                    if (FCT(list->str, string, rs, no_leading_period,
                            ...
                            NULL, alloca_used) == 0
                        /* This was successful.  Now match the rest with the
                           rest of the pattern.  */
                        &&
                        (FCT(p, rs, string_end,
                             rs == string
            ...
            for (rs = string; rs <= string_end; ++rs)
            {
                ...
                if (runp == NULL &&
                    (FCT(p, rs, string_end,
                         rs == string
                             ? no_leading_period
                             : rs[-1] == '/' && NO_LEADING_PERIOD(flags) ? 1
                                                                         : 0,
                         flags & FNM_FILE_NAME ? flags : flags & ~FNM_PERIOD,
                         NULL, alloca_used) == 0))
                    /* This is successful.  */
