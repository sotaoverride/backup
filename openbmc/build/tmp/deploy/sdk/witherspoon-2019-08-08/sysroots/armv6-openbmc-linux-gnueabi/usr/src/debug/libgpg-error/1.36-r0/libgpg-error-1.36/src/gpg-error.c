...
#define SLDIR "\\share\\locale"
    if (*moddir)
    {
        ...
        if (result)
        {
            nbytes = WideCharToMultiByte(CP_UTF8, 0, moddir, -1, result, nbytes,
                                         NULL, NULL);
            if (nbytes < 0)
            {
                free(result);
...
static int get_err_from_number(char* str, gpg_error_t* err)
{
    unsigned long nr;
    char* tail;
...
static int get_err_from_symbol_one(char* str, gpg_error_t* err,
...
static int get_err_from_symbol(char* str, gpg_error_t* err)
{
    char* str2 = str;
    int have_source = 0;
    int have_code = 0;
...
static int get_err_from_str_one(char* str, gpg_error_t* err, int* have_source,
                                int* have_code)
{
    gpg_err_source_t src;
    gpg_err_code_t code;
...
static int get_err_from_str(char* str, gpg_error_t* err)
{
    char* str2 = str;
    int have_source = 0;
    int have_code = 0;
...
static void print_desc(const char* symbol)
{
...
static const char* my_strusage(int level)
{
    ...
    switch (level)
    {
        ...
        case 13:
            ...
            break;
        case 14:
            p = "Copyright (C) 2019 g10 Code GmbH";
            break;
        case 19:
            p = _("Please report bugs to <https://bugs.gnupg.org>.\n");
            break;
...
int main(int argc, char* argv[])
{
    ...
    static gpgrt_opt_t opts[] = {
        ARGPARSE_c(CMD_LIB_VERSION, "lib-version", "Print library version"),
        ARGPARSE_c(CMD_LIST, "list", "Print all error codes"),
    ...
    else if (listmode == 1)
    {
        for (i = 0; i < GPG_ERR_SOURCE_DIM; i++)
        {
            ...
            {
                printf("%u = (%u, -) = (%s, -) = (%s, -)\n", err,
                       gpg_err_source(err), source_sym, gpg_strsource(err));
                if (desc)
        ...
        for (i = 0; i < GPG_ERR_CODE_DIM; i++)
        {
            ...
            {
                printf("%u = (-, %u) = (-, %s) = (-, %s)\n", err,
                       gpg_err_code(err), error_sym, gpg_strerror(err));
                if (desc)
        ...
        for (i = 0; i < argc; i++)
        {
                ...
                get_err_from_str(argv[i], &err))
            {
                ...
                printf("%u = (%u, %u) = (%s, %s) = (%s, %s)\n", err,
                       ...
                       error_sym ? error_sym : "-", gpg_strsource(err),
                       gpg_strerror(err));
                if (desc)
                    print_desc(error_sym);
            }
