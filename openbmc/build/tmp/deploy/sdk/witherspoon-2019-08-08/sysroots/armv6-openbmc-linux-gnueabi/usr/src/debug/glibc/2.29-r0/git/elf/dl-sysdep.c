...
rtld_hidden_data_def(__libc_enable_secure) int __libc_multiple_libcs =
    0; /* Defining this here avoids the inclusion
of init-first.  */
/* This variable contains the lowest stack address ever used.  */
void* __libc_stack_end attribute_relro = NULL;
rtld_hidden_data_def(__libc_stack_end) void* _dl_random attribute_relro = NULL;
...
void _dl_show_auxv(void)
{
    ...
    for (av = GLRO(dl_auxv); av->a_type != AT_NULL; ++av)
    {
        ...
        const char* val =
            _itoa((unsigned long int)av->a_type, buf + sizeof buf - 1, 16, 0);
