...
rsRetVal (*Connect)(nsd_t* pThis, int family, unsigned char* port,
                    unsigned char* host, char* device);
...
rsRetVal (*SetMode)(
    nsd_t* pThis,
    int mode); /* sets a driver specific mode - see driver doc for details */
rsRetVal (*SetAuthMode)(
    nsd_t* pThis,
    uchar*); /* sets a driver specific mode - see driver doc for details */
rsRetVal (*SetPermitExpiredCerts)(
    nsd_t* pThis, uchar*); /* sets a driver specific permitexpiredcerts mode */
rsRetVal (*SetPermPeers)(
    nsd_t* pThis,
    permittedPeers_t*); /* sets driver permitted peers for auth needs */
rsRetVal (*CheckConnection)(
    nsd_t* pThis); /* This is a trick mostly for plain tcp syslog */
 ...
 * peer's socket addr structure, so that the legacy matching functions can work
 * on it. Note that this ties netstream drivers to things that can be
 * implemented over sockets - not really desirable, but not the end of the
 * world... TODO: should be reconsidered when a new ACL system is build. --
...
rsRetVal (*LstnInit)(netstrms_t* pNS, void* pUsr,
...
#define nsdCURR_IF_VERSION                                                     \
    11 /* increment whenever you change the interface structure! */
    /* interface version 4 added GetRemAddr()
     * interface version 5 added EnableKeepAlive() -- rgerhards, 2009-06-02
     * interface version 6 changed return of CheckConnection from void to
     * rsRetVal -- alorbach, 2012-09-06 interface version 7 changed signature
     * ofGetRempoteIP() -- rgerhards, 2013-01-21 interface version 8 added keep
     * alive parameter set functions interface version 9 changed signature of
     * Connect() -- dsa, 2016-11-14 interface version 10 added
     * SetGnutlsPriorityString() -- PascalWithopf, 2017-08-08 interface version
     * 11 added oserr to Rcv() signature -- rgerhards, 2017-09-04
     */

    /* interface  for the select call */
    BEGINinterface(
        nsdsel) /* name must also be changed in ENDinterface macro! */
...
rsRetVal (*IsReady)(nsdsel_t* pNsdsel, nsd_t* pNsd, nsdsel_waitOp_t waitOp,
                    int* pbIsReady);
ENDinterface(nsdsel)
#define nsdselCURR_IF_VERSION                                                  \
    1 /* increment whenever you change the interface structure! */

    /* interface  for the epoll call */
    BEGINinterface(
        nsdpoll) /* name must also be changed in ENDinterface macro! */
    rsRetVal (*Construct)(nsdpoll_t** ppThis);
rsRetVal (*Destruct)(nsdpoll_t** ppThis);
rsRetVal (*Ctl)(nsdpoll_t* pNsdpoll, nsd_t* pNsd, int id, void* pUsr, int mode,
                int op);
rsRetVal (*Wait)(nsdpoll_t* pNsdpoll, int timeout, int* numReady,
                 nsd_epworkset_t workset[]);
ENDinterface(nsdpoll)
#define nsdpollCURR_IF_VERSION                                                 \
    1 /* increment whenever you change the interface structure! */
