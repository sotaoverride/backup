...
static int lsdel_proc(ext2_filsys fs, blk64_t* block_nr,
...
void do_lsdel(int argc, char** argv)
{
    ...
    if (common_args_process(argc, argv, 1, 2, "list_deleted_inodes", "[secs]",
                            0))
        return;
    ...
    if (retval)
    {
        com_err("ls_deleted_inodes", retval, "while opening inode scan");
        goto error_out;
    }
    ...
    if (retval)
    {
        com_err("ls_deleted_inodes", retval, "while starting inode scan");
        goto error_out;
    }
    ...
    while (ino)
    {
        ...
        {
            retval =
                ext2fs_block_iterate3(current_fs, ino, BLOCK_FLAG_READ_ONLY,
                                      block_buf, lsdel_proc, &lsd);
            if (retval)
            {
    ...
    next:
        ...
        if (retval)
        {
            com_err("ls_deleted_inodes", retval, "while doing inode scan");
            goto error_out;
        }
    }
    ...
    {
        fprintf(out, "%6u %6d %6o %6llu %6lld/%6lld %s", delarray[i].ino,
