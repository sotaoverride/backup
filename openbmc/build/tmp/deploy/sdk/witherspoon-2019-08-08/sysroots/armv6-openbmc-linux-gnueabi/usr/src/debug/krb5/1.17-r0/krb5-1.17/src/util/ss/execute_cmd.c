...
static int check_request_table(rqtbl, argc, argv,
                               sci_idx) ss_request_table* rqtbl;
int argc;
char* argv[];
int sci_idx;
{
    ...
    for (i = 0; (request = get_request(rqtbl, i))->command_names; i++)
    {
        for (name = request->command_names; *name; name++)
            if (!strcmp(*name, string))
            {
                ...
                (request->function)(argc, (const char* const*)argv, sci_idx,
                                    info->info_ptr);
...
static int really_execute_command(sci_idx, argc, argv) int sci_idx;
int argc;
char** argv[];
{
...
int ss_execute_command(sci_idx, argv) int sci_idx;
char* argv[];
{
    unsigned int i, argc;
    char** argp;
...
int ss_execute_line(sci_idx, line_ptr) int sci_idx;
char* line_ptr;
{
    char** argv;
    int argc, ret;
