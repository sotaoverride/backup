 ...
 * mnt_new_table:
 *
 * The tab is a container for struct libmnt_fs entries that usually represents a
 * fstab, mtab or mountinfo file from your system.
 *
...
int mnt_reset_table(struct libmnt_table* tb)
{
    ...
    {
        struct libmnt_fs* fs =
            list_entry(tb->ents.next, struct libmnt_fs, ents);
 ...
 * @tb: tab pointer
 *
 * Deallocates the table. This function does not care about reference count.
 * Don't use this function directly -- it's better to use mnt_unref_table().
 *
...
int mnt_table_add_fs(struct libmnt_table* tb, struct libmnt_fs* fs)
{
    ...
    DBG(TAB, ul_debugobj(tb, "add entry: %s %s", mnt_fs_get_source(fs),
 ...
 * @fs: new entry
 *
 * Removes the @fs from the table and de-increment reference counter of the @fs.
 * The filesystem with zero reference counter will be deallocated. Don't forget
 * to use mnt_ref_fs() before call mnt_table_remove_fs() if you want to use @fs
 * later.
 *
 * Returns: 0 on success or negative number in case of error.
 */
...
static inline struct libmnt_fs* get_parent_fs(struct libmnt_table* tb,
                                              struct libmnt_fs* fs)
{
...
int mnt_table_get_root_fs(struct libmnt_table* tb, struct libmnt_fs** root)
{
    ...
    *root = NULL;
    ...
    while (*root)
    {
        ...
        if (!x || x == *root)
            break;
        DBG(TAB, ul_debugobj(tb, " messy mountinfo, walk to %s",
                             mnt_fs_get_target(x)));
        *root = x;
    }
 ...
 *
 * Returns: 0 on success, negative number in case of error or 1 at the end of
 * list.
 */
                            ...
                            struct libmnt_fs* parent, struct libmnt_fs** chld)
{
    ...
    *chld = NULL;
    ...
    while (mnt_table_next_fs(tb, itr, &fs) == 0)
    {
        ...
        if ((!lastchld_id || id > lastchld_id) && (!*chld || id < chld_id))
        {
            *chld = fs;
            chld_id = id;
        }
 ...
 */
int mnt_table_next_fs(struct libmnt_table* tb, struct libmnt_iter* itr,
                      struct libmnt_fs** fs)
{
    int rc = 1;
...
int mnt_table_find_next_fs(struct libmnt_table* tb, struct libmnt_iter* itr,
                           ...
                           void* userdata, struct libmnt_fs** fs)
{
...
int mnt_table_uniq_fs(struct libmnt_table* tb, int flags,
                      int (*cmp)(struct libmnt_table*, struct libmnt_fs*,
                                 struct libmnt_fs*))
{
    struct libmnt_iter itr;
    struct libmnt_fs* fs;
    ...
    while (mnt_table_next_fs(tb, &itr, &fs) == 0)
    {
        ...
        if (!want)
        {
            ...
            DBG(TAB,
                ul_debugobj(tb, "remove duplicate %s", mnt_fs_get_target(fs)));
            mnt_table_remove_fs(tb, fs);
        }
    }
 ...
 */
int mnt_table_set_iter(struct libmnt_table* tb, struct libmnt_iter* itr,
                       struct libmnt_fs* fs)
{
    if (!tb || !itr || !fs)
        return -EINVAL;
 ...
 * @direction: MNT_ITER_{FORWARD,BACKWARD}
 *
 * Try to lookup an entry in the given tab, three iterations are possible, the
 * first with @path, the second with realpath(@path) and the third with
 * realpath(@path) against realpath(fs->target). The 2nd and 3rd iterations are
 * not performed when the @tb cache is not set (see mnt_table_set_cache()). If
                                        ...
                                        const char* path, int direction)
{
    ...
    while (mnt_table_next_fs(tb, &itr, &fs) == 0)
    {
        ...
        if (!fs->target || mnt_fs_is_swaparea(fs) || mnt_fs_is_kernel(fs) ||
 ...
 * @direction: MNT_ITER_{FORWARD,BACKWARD}
 *
 * Try to lookup an entry in the given tab, four iterations are possible, the
 * first with @path, the second with realpath(@path), the third with tags
 * (LABEL, UUID, ..) from @path and the fourth with realpath(@path) against
 * realpath(entry->srcpath).
 *
 ...
 * @direction: MNT_ITER_{FORWARD,BACKWARD}
 *
 * Try to lookup an entry in the given tab, the first attempt is to lookup by
 @tag and
                                     ...
                                     const char* val, int direction)
{
    ...
    while (mnt_table_next_fs(tb, &itr, &fs) == 0)
    {
        if (fs->tagname && fs->tagval && strcmp(fs->tagname, tag) == 0 &&
                                                    ...
                                                    int direction)
{
    ...
    DBG(TAB, ul_debugobj(tb, "lookup TARGET: '%s' with OPTION %s %s", path,
                         option, val));
    ...
    while (mnt_table_next_fs(tb, &itr, &fs) == 0)
    {
        if (mnt_fs_streq_target(fs, path) &&
            ...
            (!val ||
             (optvalsz == valsz && strncmp(optval, val, optvalsz) == 0)))
            return fs;
    }
    return NULL;
}
 ...
 * others mnt_table_find_* function, because every @tab entry is fully
 * evaluated.
 *
 * Returns: a tab entry or NULL.
 */
 ...
 * @direction: MNT_ITER_{FORWARD,BACKWARD}
 *
 * Note that zero could be a valid device number for the root pseudo filesystem
 * (e.g. tmpfs).
 *
 * Returns: a tab entry or NULL.
 */
struct libmnt_fs* mnt_table_find_devno(struct libmnt_table* tb, dev_t devno,
                                       int direction)
{
    struct libmnt_fs* fs = NULL;
    struct libmnt_iter itr;
                             ...
                             char** root)
{
    ...
    *root = NULL;
    ...
    if (mnt_fs_get_option(fs, "subvolid", &vol, &volsz) == 0)
    {
        ...
        DBG(BTRFS,
            ul_debug(" trying target=%s subvolid=%s", target, subvolidstr));
        f = mnt_table_find_target_with_option(tb, target, "subvolid",
                                              subvolidstr, MNT_ITER_BACKWARD);
        if (!tb->cache)
            free(target);
        if (!f)
            goto not_found;
    ...
    else if (mnt_fs_get_option(fs, "subvol", &vol, &volsz) != 0)
    {
        ...
        DBG(BTRFS, ul_debug(" trying target=%s default subvolid=%s", target,
                            default_id_str));

        f = mnt_table_find_target_with_option(
            tb, target, "subvolid", default_id_str, MNT_ITER_BACKWARD);
        if (!tb->cache)
            free(target);
        if (!f)
            goto not_found;
...
struct libmnt_fs* mnt_table_get_fs_root(struct libmnt_table* tb,
                                        struct libmnt_fs* fs,
                                        unsigned long mountflags, char** fsroot)
{
    char* root = NULL;
    const char* mnt = NULL;
    ...
    if (tb && (mountflags & MS_BIND))
    {
        ...
        {
            struct libmnt_fs* f =
                mnt_table_find_mountpoint(tb, src, MNT_ITER_BACKWARD);
            if (f)
        ...
        DBG(FS, ul_debugobj(fs, "source root: %s, source FS root: %s", root,
                            src_root));
...
int mnt_table_is_fs_mounted(struct libmnt_table* tb, struct libmnt_fs* fstab_fs)
{
    ...
    DBG(FS,
        ul_debugobj(fstab_fs, "mnt_table_is_fs_mounted: target=%s, source=%s",
                    mnt_fs_get_target(fstab_fs), mnt_fs_get_source(fstab_fs)));
    ...
    while (mnt_table_next_fs(tb, &itr, &fs) == 0)
    {
        ...
        if (!eq)
        {
            ...
            DBG(FS, ul_debugobj(fs, "checking for loop: src=%s",
        ...
        if (root)
        {
            ...
            if (fstype && strcmp(fstype, "cifs") == 0)
            {
                ...
                if (!unc_subdir || !path_on_fs ||
                    !streq_paths(unc_subdir, path_on_fs))
                    continue;
            }
            else
            {
