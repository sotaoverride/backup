...
class severity_level : public attribute
{
  ...
  public:
    ...
    BOOST_STATIC_ASSERT_MSG(sizeof(value_type) <= sizeof(uintmax_t),
                            "Boost.Log: Unsupported severity level type, the "
                            "severity level must fit into uintmax_t");

  protected:
    //! Factory implementation
    class BOOST_SYMBOL_VISIBLE impl : public attribute_value::impl
    {
      public:
        ...
        intrusive_ptr<attribute_value::impl> detach_from_thread()
        {
            ...
            return new attributes::attribute_value_impl<value_type>(
                reinterpret_cast<value_type const&>(get_severity_level()));
#else
            // With multithreading disabled we may safely return this here. This
            // method will not be called anyway.
            return this;
#endif
        }
    };
  ...
  public:
    ...
    severity_level(severity_level const& that) :
        attribute(static_cast<attribute const&>(that))
    {
    }
    //! Move constructor
    severity_level(BOOST_RV_REF(severity_level) that) :
        attribute(boost::move(static_cast<attribute&>(that)))
    {
    }
...
class basic_severity_logger : public BaseT
{
  ...
  public:
    ...
    typedef typename strictest_lock<typename base_type::open_record_lock,
                                    no_lock<threading_model>>::type
        open_record_lock;
  ...
  public:
    /*!
     * Default constructor. The constructed logger will have a severity
     * attribute registered. The default level for log records will be 0.
     */
    basic_severity_logger() :
        base_type(), m_DefaultSeverity(static_cast<severity_level>(0))
    {
    ...
    /*!
     * Constructor with named arguments. Allows to setup the default level for
     * log records.
     *
     * \param args A set of named arguments. The following arguments are
     * supported: \li \c severity - default severity value
     */
 ...
 *
 * The logger with this feature registers a special attribute with an integral
 * value type on construction. This attribute will provide severity level for
 * each log record being made through the logger. The severity level can be
 * omitted on logging record construction, in which case the default level will
 * be used. The default level can also be customized by passing it to the logger
 * constructor.
 *
 * The type of the severity level attribute can be specified as a template
 * parameter for the feature template. By default, \c int will be used.
 */
