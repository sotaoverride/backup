...
static FT_Error ft_bzip2_check_header(FT_Stream stream)
{
...
static FT_Error ft_bzip2_file_init(FT_BZip2File zip, FT_Stream stream,
                                   FT_Stream source)
{
...
static void ft_bzip2_file_done(FT_BZip2File zip)
{
    bz_stream* bzstream = &zip->bzstream;
...
static FT_Error ft_bzip2_file_reset(FT_BZip2File zip)
{
...
static FT_Error ft_bzip2_file_fill_input(FT_BZip2File zip)
{
    bz_stream* bzstream = &zip->bzstream;
    FT_Stream stream = zip->source;
    FT_ULong size;
    ...
    {
        size =
            stream->read(stream, stream->pos, zip->input, FT_BZIP2_BUFFER_SIZE);
        if (size == 0)
        {
...
static FT_Error ft_bzip2_file_fill_output(FT_BZip2File zip)
{
    bz_stream* bzstream = &zip->bzstream;
    FT_Error error = FT_Err_Ok;
...
static FT_Error ft_bzip2_file_skip_output(FT_BZip2File zip, FT_ULong count)
{
    FT_Error error = FT_Err_Ok;
    FT_ULong delta;
...
static FT_ULong ft_bzip2_file_io(FT_BZip2File zip, FT_ULong pos,
...
static unsigned long ft_bzip2_stream_io(FT_Stream stream, unsigned long offset,
