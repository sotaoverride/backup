...
/***
  Copyright © 2015-2017 Jason A. Donenfeld <Jason@zx2c4.com>. All Rights
Reserved.
***/
...
static int wireguard_peer_new_static(Wireguard* w, const char* filename,
                                     unsigned section_line, WireguardPeer** ret)
{
    ...
    *peer = (WireguardPeer){
    ...
    r = hashmap_ensure_allocated(&w->peers_by_section,
...
static int wireguard_set_ipmask_one(NetDev* netdev, sd_netlink_message* message,
                                    const WireguardIPmask* mask, uint16_t index)
{
    int r;
    ...
    /* This returns 1 on success, 0 on recoverable error, and negative errno on
     * failure. */
    ...
    r = sd_netlink_message_append_u16(message, WGALLOWEDIP_A_FAMILY,
                                      mask->family);
    if (r < 0)
        goto cancel;

    r = netlink_message_append_in_addr_union(message, WGALLOWEDIP_A_IPADDR,
                                             mask->family, &mask->ip);
    if (r < 0)
        goto cancel;

    r = sd_netlink_message_append_u8(message, WGALLOWEDIP_A_CIDR_MASK,
                                     mask->cidr);
    if (r < 0)
        goto cancel;
...
static int wireguard_set_peer_one(NetDev* netdev, sd_netlink_message* message,
                                  const WireguardPeer* peer, uint16_t index,
                                  WireguardIPmask** mask_start)
{
    ...
    r = sd_netlink_message_append_data(message, WGPEER_A_PUBLIC_KEY,
                                       &peer->public_key,
    ...
    if (!*mask_start)
    {
        r = sd_netlink_message_append_data(message, WGPEER_A_PRESHARED_KEY,
                                           &peer->preshared_key, WG_KEY_LEN);
        if (r < 0)
            goto cancel;
        ...
        r = sd_netlink_message_append_u16(
            message, WGPEER_A_PERSISTENT_KEEPALIVE_INTERVAL,
        ...
        {
            r = netlink_message_append_sockaddr_union(
                message, WGPEER_A_ENDPOINT, &peer->endpoint);
            if (r < 0)
                goto cancel;
        }
    }
...
static int wireguard_set_interface(NetDev* netdev)
{
    ...
    for (peer_start = w->peers; peer_start;)
    {
        ...
        r = sd_genl_message_new(netdev->manager->genl, SD_GENL_WIREGUARD,
                                WG_CMD_SET_DEVICE, &message);
        if (r < 0)
            return log_netdev_error_errno(
                netdev, r, "Failed to allocate generic netlink message: %m");

        r = sd_netlink_message_append_string(message, WGDEVICE_A_IFNAME,
                                             netdev->ifname);
        if (r < 0)
            return log_netdev_error_errno(
        ...
        {
            r = sd_netlink_message_append_data(message, WGDEVICE_A_PRIVATE_KEY,
                                               &w->private_key, WG_KEY_LEN);
            if (r < 0)
                return log_netdev_error_errno(
                    netdev, r, "Could not append wireguard private key: %m");

            r = sd_netlink_message_append_u16(message, WGDEVICE_A_LISTEN_PORT,
                                              w->port);
            if (r < 0)
                return log_netdev_error_errno(
                    netdev, r, "Could not append wireguard port: %m");

            r = sd_netlink_message_append_u32(message, WGDEVICE_A_FWMARK,
                                              w->fwmark);
            if (r < 0)
                return log_netdev_error_errno(
                    netdev, r, "Could not append wireguard fwmark: %m");

            r = sd_netlink_message_append_u32(message, WGDEVICE_A_FLAGS,
                                              w->flags);
            if (r < 0)
                return log_netdev_error_errno(
        ...
        if (r < 0)
            return log_netdev_error_errno(
                netdev, r, "Could not close wireguard container: %m");
 ...
 * increasing time in milliseconds to wait starting at 200ms and capped at 25
 * seconds.
 */
...
static int wireguard_resolve_handler(sd_resolve_query* q, int ret,
                                     ...
                                     WireguardPeer* peer)
{
    ...
    if (ret != 0)
    {
        log_netdev_error(netdev, "Failed to resolve host '%s:%s': %s",
        ...
        log_netdev_error(netdev,
                         "Neither IPv4 nor IPv6 address found for peer "
                         "endpoint %s:%s, ignoring the address.",
                         peer->endpoint_host, peer->endpoint_port);
...
resolve_next:
    ...
    if (!set_isempty(w->peers_with_failed_endpoint))
    {
        ...
        w->n_retries++;
        usec = usec_add(now(CLOCK_MONOTONIC),
                        exponential_backoff_milliseconds(w->n_retries));
        r = event_reset_time(netdev->manager->event,
                             &w->resolve_retry_event_source, CLOCK_MONOTONIC,
                             usec, 0, on_resolve_retry, netdev, 0,
                             "wireguard-resolve-retry", true);
        if (r < 0)
        {
...
static void resolve_endpoints(NetDev* netdev)
{
    ...
    {
        r = resolve_getaddrinfo(netdev->manager->resolve, NULL,
                                peer->endpoint_host, peer->endpoint_port,
                                &hints, wireguard_resolve_handler,
                                wireguard_peer_destroy_callback, peer);
        if (r == -ENOBUFS)
            break;
        if (r < 0)
        {
...
int config_parse_wireguard_listen_port(const char* unit, const char* filename,
...
static int wireguard_decode_key_and_warn(const char* rvalue,
                                         uint8_t ret[static WG_KEY_LEN],
                                         const char* unit, const char* filename,
                                         unsigned line, const char* lvalue)
{
    ...
    if (r < 0)
        return log_syntax(unit, LOG_ERR, filename, line, r,
                          "Failed to decode wireguard key provided by %s=, "
                          "ignoring assignment: %m",
                          lvalue);
    if (len != WG_KEY_LEN)
    {
        explicit_bzero_safe(key, len);
        return log_syntax(unit, LOG_ERR, filename, line, 0,
                          "Wireguard key provided by %s= has invalid length "
                          "(%zu bytes), ignoring assignment.",
                          lvalue, len);
    }
...
int config_parse_wireguard_private_key(const char* unit, const char* filename,
                                       unsigned line, const char* section,
                                       unsigned section_line,
                                       const char* lvalue, int ltype,
                                       const char* rvalue, void* data,
                                       void* userdata)
{
    ...
    (void)wireguard_decode_key_and_warn(rvalue, w->private_key, unit, filename,
                                        line, lvalue);
    return 0;
}
    ...
    void* data, void* userdata)
{
    ...
    if (path_simplify_and_warn(path, PATH_CHECK_ABSOLUTE, unit, filename, line,
                               lvalue) < 0)
        return 0;
...
int config_parse_wireguard_preshared_key(const char* unit, const char* filename,
                                         unsigned line, const char* section,
                                         unsigned section_line,
                                         const char* lvalue, int ltype,
                                         const char* rvalue, void* data,
                                         void* userdata)
{
    ...
    r = wireguard_decode_key_and_warn(rvalue, peer->preshared_key, unit,
...
int config_parse_wireguard_public_key(const char* unit, const char* filename,
                                      unsigned line, const char* section,
                                      ...
                                      void* userdata)
{
    ...
    r = wireguard_decode_key_and_warn(rvalue, peer->public_key, unit, filename,
                                      line, lvalue);
    if (r < 0)
        return r;
...
int config_parse_wireguard_allowed_ips(const char* unit, const char* filename,
                                       unsigned line, const char* section,
                                       unsigned section_line,
                                       const char* lvalue, int ltype,
                                       const char* rvalue, void* data,
...
int config_parse_wireguard_endpoint(const char* unit, const char* filename,
                                    unsigned line, const char* section,
                                    unsigned section_line, const char* lvalue,
                                    int ltype, const char* rvalue, void* data,
                                    void* userdata)
    ...
    if (rvalue[0] == '[')
    {
        ...
        if (!end)
        {
            log_syntax(unit, LOG_ERR, filename, line, 0,
                       "Unable to find matching brace of endpoint, ignoring "
                       "assignment: %s",
                       rvalue);
            return 0;
        }
...
int config_parse_wireguard_keepalive(const char* unit, const char* filename,
                                     unsigned line, const char* section,
                                     unsigned section_line, const char* lvalue,
                                     int ltype, const char* rvalue, void* data,
                                     void* userdata)
{
    ...
    else
    {
        ...
        {
            log_syntax(unit, LOG_ERR, filename, line, r,
                       "The persistent keepalive interval must be 0-65535. "
                       "Ignore assignment: %s",
                       rvalue);
            return 0;
        }
...
static int wireguard_read_key_file(const char* filename,
                                   uint8_t dest[static WG_KEY_LEN])
{
    ...
    r = read_full_file_full(filename,
