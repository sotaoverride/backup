                                 ...
                                 AvahiEntryGroupState state, void* userdata)
{
    ...
    switch (state)
    {
        ...
        case AVAHI_ENTRY_GROUP_COLLISION:
            avahi_log_error("Host name conflict for \"%s\", not established.",
                            h->host);
            break;

        case AVAHI_ENTRY_GROUP_ESTABLISHED:
            avahi_log_notice(
                "Static host name \"%s\" successfully established.", h->host);
            break;

        case AVAHI_ENTRY_GROUP_FAILURE:
            avahi_log_notice("Failed to establish static host name \"%s\": %s.",
                             h->host, avahi_strerror(avahi_server_errno(s)));
            break;
...
static void add_static_host_to_server(StaticHost* h)
{
    ...
    if (!h->group)
        if (!(h->group = avahi_s_entry_group_new(avahi_server,
                                                 entry_group_callback, h)))
        {
            avahi_log_error("avahi_s_entry_group_new() failed: %s",
    ...
    if (avahi_s_entry_group_is_empty(h->group))
    {
        ...
        p = (h->address.proto == AVAHI_PROTO_INET &&
             config->publish_a_on_ipv6) ||
                    (h->address.proto == AVAHI_PROTO_INET6 &&
                     config->publish_aaaa_on_ipv4)
                ? AVAHI_PROTO_UNSPEC
                : h->address.proto;

        if ((err = avahi_server_add_address(avahi_server, h->group,
                                            AVAHI_IF_UNSPEC, p, 0, h->host,
                                            &h->address)) < 0)
        {
            avahi_log_error(
                "Static host name %s: avahi_server_add_address failure: %s",
                h->host, avahi_strerror(err));
            return;
        }
...
void static_hosts_load(int in_chroot)
{
    ...
    {
        if (errno != ENOENT)
            avahi_log_error("Failed to open static hosts file: %s",
                            strerror(errno));
        return;
    }
    ...
    while (!feof(f))
    {
        ...
        if (*host == 0)
        {
            avahi_log_error("%s:%d: Error, unexpected end of line!", filename,
                            line);
            avahi_free(host);
            avahi_free(ip);
            goto fail;
        ...
        if (*s != 0)
        {
            avahi_log_error("%s:%d: Junk on the end of the line!", filename,
                            line);
            avahi_free(host);
            avahi_free(ip);
            goto fail;
        ...
        {
            avahi_log_error("Static host name %s: failed to parse address %s",
                            host, ip);
            avahi_free(host);
            avahi_free(ip);
            goto fail;
