...
BOOST_LOG_ANONYMOUS_NAMESPACE
{
    ...
    class settings_parser
    {
      ...
      public:
        //! Constructor
        explicit settings_parser(settings_type& setts,
                                 unsigned int& line_counter,
                                 std::locale const& loc) :
            m_Settings(setts),
            m_Locale(loc), m_LineCounter(line_counter)
        {
        }
        ...
        void parse_line(iterator_type& begin, iterator_type end)
        {
            ...
            if (p != end)
            {
                ...
                if (c == constants::char_section_bracket_left)
                {
                    ...
                    iterator_type stop = std::find(
                        start, end, constants::char_section_bracket_right);
                    if (stop == end)
                        BOOST_LOG_THROW_DESCR_PARAMS(
                            parse_error, "Section header is invalid",
                            (m_LineCounter));
                ...
                else if (c != constants::char_comment)
                {
                    ...
                    if (eq == end)
                        BOOST_LOG_THROW_DESCR_PARAMS(
                            parse_error, "Parameter description is invalid",
                            (m_LineCounter));
                    ...
                    if (p == end || *p == constants::char_comment)
                        BOOST_LOG_THROW_DESCR_PARAMS(
                            parse_error, "Parameter value is not specified",
                            (m_LineCounter));
                ...
                if (p != end)
                {
                    ...
                    else
                    {
                        BOOST_LOG_THROW_DESCR_PARAMS(
                            parse_error,
                            "Unexpected characters in the end of the line",
                            (m_LineCounter));
                    }
                }
            }
      ...
      private:
        ...
        void set_section_name(iterator_type begin, iterator_type end)
        {
            ...
            if (begin == end)
                BOOST_LOG_THROW_DESCR_PARAMS(
                    parse_error, "Section name is empty", (m_LineCounter));
            ...
            m_SectionName =
                log::aux::to_narrow(string_type(begin, end), m_Locale);

            // For compatibility with Boost.Log v1, we replace the "Sink:"
            // prefix with "Sinks." so that all sink parameters are placed in
            // the common Sinks section.
        ...
        void set_parameter_name(iterator_type begin, iterator_type end)
        {
            if (m_SectionName.empty())
            {
                ...
                BOOST_LOG_THROW_DESCR_PARAMS(
                    parse_error, "Parameters are only allowed within sections",
                    (m_LineCounter));
            }
            ...
            if (begin == end)
                BOOST_LOG_THROW_DESCR_PARAMS(
                    parse_error, "Parameter name is empty", (m_LineCounter));

            iterator_type p = begin;
            if (!encoding::isalpha(*p))
                BOOST_LOG_THROW_DESCR_PARAMS(
                    parse_error, "Parameter name is invalid", (m_LineCounter));
            for (++p; p != end; ++p)
            {
                char_type c = *p;
        ...
        BOOST_DELETED_FUNCTION(
            settings_parser& operator=(settings_parser const&))
    };
...
template <typename CharT>
BOOST_LOG_SETUP_API basic_settings<CharT>
    parse_settings(std::basic_istream<CharT>& strm)
{
    ...
    io::basic_ios_exception_saver<char_type> exceptions_guard(
        strm, std::ios_base::badbit);
...
template BOOST_LOG_SETUP_API basic_settings<char>
    parse_settings<char>(std::basic_istream<char>& strm);
#endif
#ifdef BOOST_LOG_USE_WCHAR_T
template BOOST_LOG_SETUP_API basic_settings<wchar_t>
    parse_settings<wchar_t>(std::basic_istream<wchar_t>& strm);
#endif
