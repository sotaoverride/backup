 ...
 * final class name */
static struct pdbg_target* get_class_target_addr(struct pdbg_target* target,
                                                 const char* name,
                                                 uint64_t* addr)
{
    /* Check class */
...
static int pib_indirect_read(struct pib* pib, uint64_t addr, uint64_t* data)
{
    ...
    *data = PIB_IND_READ | (addr & PIB_IND_ADDR);
    ...
    for (retries = 0; retries < PIB_IND_MAX_RETRIES; retries++)
    {
        ...
        if ((*data & PIB_DATA_IND_COMPLETE) || (retries >= PIB_IND_MAX_RETRIES))
        {
...
static int pib_indirect_write(struct pib* pib, uint64_t addr, uint64_t data)
{
    ...
    for (retries = 0; retries < PIB_IND_MAX_RETRIES; retries++)
    {
        ...
        if ((data & PIB_DATA_IND_COMPLETE) && ((data & PIB_DATA_IND_ERR) == 0))
            break;
...
struct pdbg_target_class* find_target_class(const char* name)
{
    ...
    list_for_each(&target_classes, target_class,
                  class_head_link) if (!strcmp(target_class->name,
                                               name)) return target_class;
...
void pdbg_target_release(struct pdbg_target* target)
{
    ...
    pdbg_for_each_child_target(target, child) pdbg_target_release(child);
