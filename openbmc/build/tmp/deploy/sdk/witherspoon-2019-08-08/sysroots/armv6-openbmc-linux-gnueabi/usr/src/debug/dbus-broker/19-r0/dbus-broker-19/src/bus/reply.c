...
int reply_slot_new(ReplySlot** replyp, ReplyRegistry* registry,
                   ReplyOwner* owner, User* user, User* actor, uint64_t id,
                   uint32_t serial)
{
    ReplySlot* reply;
    CRBNode **slot, *parent;
    ...
    slot = c_rbtree_find_slot(&registry->reply_tree, reply_slot_compare, &key,
                              &parent);
    if (!slot)
...
ReplySlot* reply_slot_get_by_id(ReplyRegistry* registry, uint64_t id,
                                uint32_t serial)
{
    ReplySlotKey key = {
        .id = id,
    ...
    return c_rbtree_find_entry(&registry->reply_tree, reply_slot_compare, &key,
                               ReplySlot, registry_node);
}
