...
//!   global `operator new` and global `operator delete`. The same value is
//!   returned every time this function is called. For return value p and memory
...
//!   deallocate() has no effect. The same value is returned every time this
//!   function is called. For return value p and memory resource r,
//!   p->is_equal(r) returns &r == p.
BOOST_CONTAINER_DECL memory_resource* null_memory_resource() BOOST_NOEXCEPT;

//! <b>Effects</b>: If r is non-null, sets the value of the default memory
//! resource
...
//!
//! <b>Remarks</b>: Calling the set_default_resource and get_default_resource
//! functions shall
//!   not incur a data race. A call to the set_default_resource function shall
//!   synchronize with subsequent calls to the set_default_resource and
