...
static int process_block(ext2_filsys fs, blk64_t* blocknr, e2_blkcnt_t blockcnt,
...
{
    ext2_ino_t ino;
    unsigned is_dir : 1, is_reg : 1, clear : 1, suppress : 1, fragmented : 1,
        compressed : 1, bbcheck : 1, inode_modified : 1;
    blk64_t num_blocks;
    blk64_t max_blocks;
    blk64_t last_block;
...
static __u64
    ext2_max_sizes[EXT2_MAX_BLOCK_LOG_SIZE - EXT2_MIN_BLOCK_LOG_SIZE + 1];
    ...
    else
    {
        ...
        if (inode->i_flags & EXT4_EXTENTS_FL)
        {
            ...
            if (ext2fs_extent_get_info(handle, &info) ||
                (info.num_entries != 1) || (info.max_depth != 0))
            {
                ext2fs_extent_free(handle);
                return 0;
            }
            if (ext2fs_extent_get(handle, EXT2_EXTENT_ROOT, &extent) ||
                (extent.e_lblk != 0) || (extent.e_len != 1))
            {
                ext2fs_extent_free(handle);
                return 0;
            }
...
static void check_extents_inlinedata(e2fsck_t ctx, struct problem_context* pctx)
{
                                      ...
                                      blk64_t* quota_blocks)
{
    ...
    {
        *quota_blocks = size_to_quota_blocks(ctx->fs, entry->e_value_size);
    }
    else
    {
                              ...
                              struct ext2_ext_attr_entry* first, void* end)
{
    ...
    for (entry = first; (void*)entry < end && !EXT2_EXT_IS_LAST_ENTRY(entry);
                              ...
                              struct ea_quota* ea_ibody_quota)
{
    ...
    storage_size = EXT2_INODE_SIZE(ctx->fs->super) - EXT2_GOOD_OLD_INODE_SIZE -
                   inode->i_extra_isize;
    header = ((char*)inode) + EXT2_GOOD_OLD_INODE_SIZE + inode->i_extra_isize;
           ...
           !EXT2_EXT_IS_LAST_ENTRY(entry))
    {
        ...
        if (entry->e_value_inum == 0)
        {
            ...
            hash =
                ext2fs_ext_attr_hash_entry(entry, start + entry->e_value_offs);
        ...
        else
        {
            ...
            problem = check_large_ea_inode(ctx, entry, pctx, &quota_blocks);
            if (problem != 0)
                goto fix;
...
fix:
    ...
    {
        inc_ea_inode_refs(ctx, pctx, (struct ext2_ext_attr_entry*)start, end);
        return;
    }

    /* simply remove all possible EA(s) */
    *((__u32*)header) = 0UL;
    e2fsck_write_inode_full(ctx, pctx->ino, pctx->inode, EXT2_INODE_SIZE(sb),
                            "pass1");
    ea_ibody_quota->blocks = 0;
    ea_ibody_quota->inodes = 0;
}
...
#define CHECK_INODE_EXTRA_NEGATIVE_EPOCH(inode, xtime)                         \
    check_inode_extra_negative_epoch(inode->i_##xtime, inode->i_##xtime##_extra)
                                ...
                                char* buf)
{
    ...
    if (inlinedata_fs && (inode->i_flags & EXT4_INLINE_DATA_FL))
    {
        ...
        if (dotdot >= ctx->fs->super->s_inodes_count ||
            ...
            (de.inode < EXT2_FIRST_INO(ctx->fs->super) && de.inode != 0) ||
            rec_len2 > EXT4_MIN_INLINE_DATA_SIZE - EXT4_INLINE_DATA_DOTDOT_SIZE)
            return;
    ...
    if ((ext2fs_dirent_name_len(dirent) != 1) || (dirent->name[0] != '.') ||
        (dirent->inode != pctx->ino) || (rec_len < 12) || (rec_len % 4) ||
    ...
    if ((ext2fs_dirent_name_len(dirent) != 2) || (dirent->name[0] != '.') ||
        (dirent->name[1] != '.') || (rec_len < 12) || (rec_len % 4))
        return;
                                     ...
                                     ext2_icount_t* ret)
{
    ...
    *ret = 0;
    ...
    profile_get_string(ctx->profile, "scratch_files", "directory", 0, 0,
                       &tdb_dir);
    profile_get_uint(ctx->profile, "scratch_files", "numdirs_threshold", 0, 0,
                     &threshold);
    profile_get_boolean(ctx->profile, "scratch_files", "icount", 0, 1, &enable);
 ...
 */
static int fix_inline_data_extents_file(e2fsck_t ctx, ext2_ino_t ino,
                                        struct ext2_inode* inode,
                                        int inode_size,
                                        struct problem_context* pctx)
{
    ...
    if (ext2fs_extent_header_verify(inode->i_block, sizeof(inode->i_block)) ==
            0 &&
    ...
    if (EXT2_I_SIZE(inode) < EXT4_MIN_INLINE_DATA_SIZE + max_inline_ea_size &&
...
static void pass1_readahead(e2fsck_t ctx, dgrp_t* group, ext2_ino_t* next_ino)
{
    ...
    for (grp = start; grp < ctx->fs->group_desc_count; grp++)
    {
        ...
        blocks_to_read +=
            (inodes_in_group + inodes_per_block - 1) / inodes_per_block;
        if (blocks_to_read * ctx->fs->blocksize > ctx->readahead_kb * 1024)
            break;
    }

    err =
        e2fsck_readahead(ctx->fs, E2FSCK_READA_ITABLE, start, grp - start + 1);
    if (err == EAGAIN)
    {
        ctx->readahead_kb /= 2;
        err = 0;
...
out:
    ...
    else
    {
        ...
        *group = grp + 1;
        inodes_per_buffer =
            (ctx->inode_buffer_blocks ? ctx->inode_buffer_blocks
                                      : EXT2_INODE_SCAN_DEFAULT_BUFFER_BLOCKS) *
            ctx->fs->blocksize / EXT2_INODE_SIZE(ctx->fs->super);
        inodes_in_group--;
        *next_ino = inodes_in_group - (inodes_in_group % inodes_per_buffer) +
                    1 + (grp * ctx->fs->super->s_inodes_per_group);
    }
}
...
void e2fsck_pass1(e2fsck_t ctx)
{
    ...
    if (ext2fs_has_feature_dir_index(fs->super) && !(ctx->options & E2F_OPT_NO))
    {
...
#undef EXT2_BPP
    ...
    pctx.errcode = e2fsck_allocate_inode_bitmap(
        fs, _("in-use inode map"), EXT2FS_BMAP64_RBTREE, "inode_used_map",
        &ctx->inode_used_map);
    if (pctx.errcode)
    {
        pctx.num = 1;
    ...
    pctx.errcode = e2fsck_allocate_inode_bitmap(
        fs, _("directory inode map"), EXT2FS_BMAP64_AUTODIR, "inode_dir_map",
        &ctx->inode_dir_map);
    if (pctx.errcode)
    {
        pctx.num = 2;
    ...
    pctx.errcode = e2fsck_allocate_inode_bitmap(
        fs, _("regular file inode map"), EXT2FS_BMAP64_RBTREE, "inode_reg_map",
        &ctx->inode_reg_map);
    if (pctx.errcode)
    {
        pctx.num = 6;
    ...
    pctx.errcode = e2fsck_allocate_subcluster_bitmap(
        fs, _("in-use block map"), EXT2FS_BMAP64_RBTREE, "block_found_map",
        &ctx->block_found_map);
    if (pctx.errcode)
    {
        pctx.num = 1;
    ...
    pctx.errcode = e2fsck_allocate_block_bitmap(
        fs, _("metadata block map"), EXT2FS_BMAP64_RBTREE, "block_metadata_map",
    ...
    inode = (struct ext2_inode*)e2fsck_allocate_memory(ctx, bufsize,
                                                       "scratch inode");

    inodes_to_process = (struct process_inode_block*)e2fsck_allocate_memory(
        ctx, (ctx->process_inode_size * sizeof(struct process_inode_block)),
    ...
    pctx.errcode = ext2fs_convert_subcluster_bitmap(fs, &ctx->block_found_map);
    if (pctx.errcode)
    {
    ...
    ext2fs_inode_scan_flags(
        scan, EXT2_SF_SKIP_MISSING_ITABLE | EXT2_SF_WARN_GARBAGE_INODES, 0);
    ...
    if (ext2fs_has_feature_mmp(fs->super) &&
        ...
        ext2fs_mark_block_bitmap2(ctx->block_found_map, fs->super->s_mmp_block);
    ...
    while (1)
    {
        ...
        pctx.errcode =
            ext2fs_get_next_inode_full(scan, &ino, inode, inode_size);
        ...
        if (pctx.errcode == EXT2_ET_BAD_BLOCK_IN_INODE_TABLE)
        {
             ...
             */
            if (ino == EXT2_BAD_INO &&
                fix_problem(ctx, PR_1_BADBLOCKS_IN_BADBLOCKS, &pctx))
            {
                errcode_t err;
                ...
                err = ext2fs_read_bb_inode(ctx->fs, &ctx->fs->badblocks);
                if (err)
                {
                    fix_problem(ctx, PR_1_ISCAN_ERROR, &pctx);
                    ctx->flags |= E2F_FLAG_ABORT;
                ...
                err = ext2fs_inode_scan_goto_blockgroup(scan, 0);
                if (err)
                {
                    fix_problem(ctx, PR_1_ISCAN_ERROR, &pctx);
                    ctx->flags |= E2F_FLAG_ABORT;
        ...
        if (pctx.errcode && pctx.errcode != EXT2_ET_INODE_CSUM_INVALID &&
            ...
            inode->i_dtime < ctx->fs->super->s_inodes_count)
        {
            ...
            {
                inode->i_dtime = inode->i_links_count ? 0 : ctx->now;
                e2fsck_write_inode(ctx, ino, inode, "pass1");
                failed_csum = 0;
            }
        }
        ...
        {
            pctx.errcode = ext2fs_icount_store(ctx->inode_link_info, ino,
                                               inode->i_links_count);
            if (pctx.errcode)
            {
                 ...
                 !quota_inum_is_reserved(fs, ino))
        {
            ...
            {
                if (fix_problem(ctx, PR_1_ZERO_DTIME, &pctx))
                {
        ...
        if ((inode->i_flags & EXT4_INLINE_DATA_FL) &&
            (inode->i_flags & EXT4_EXTENTS_FL))
        {
            int res = fix_inline_data_extents_file(ctx, ino, inode, inode_size,
                                                   &pctx);
            if (res < 0)
            {
            ...
            (ino >= EXT2_FIRST_INODE(fs->super)))
        {
            ...
            switch (err)
            {
                ...
                case EXT2_ET_EA_INODE_CORRUPTED:
                    ...
                    if (fix_problem(ctx, PR_1_INLINE_DATA_NO_ATTR, &pctx))
                    {
            ...
            (LINUX_S_ISREG(inode->i_mode) || LINUX_S_ISDIR(inode->i_mode)))
        {
            ...
            ehp = inode->i_block;
#endif
            if ((ext2fs_extent_header_verify(ehp, sizeof(inode->i_block)) ==
                 0) &&
                (fix_problem(ctx, PR_1_UNSET_EXTENT_FL, &pctx)))
            {
                inode->i_flags |= EXT4_EXTENTS_FL;
#ifdef WORDS_BIGENDIAN
                memcpy(inode->i_block, tmp_block, sizeof(inode->i_block));
#endif
                e2fsck_write_inode(ctx, ino, inode, "pass1");
                failed_csum = 0;
            }
        ...
        if (ino == EXT2_BAD_INO)
        {
            ...
            if ((failed_csum || inode->i_mode || inode->i_uid || inode->i_gid ||
                 inode->i_links_count ||
                 (inode->i_flags & EXT4_INLINE_DATA_FL) || inode->i_file_acl) &&
                fix_problem(ctx, PR_1_INVALID_BAD_INODE, &pctx))
            {
                memset(inode, 0, sizeof(struct ext2_inode));
                e2fsck_write_inode(ctx, ino, inode, "clear bad inode");
                failed_csum = 0;
            }

            pctx.errcode =
                ext2fs_copy_bitmap(ctx->block_found_map, &pb.fs_meta_blocks);
            if (pctx.errcode)
            {
                pctx.num = 4;
                fix_problem(ctx, PR_1_ALLOCATE_BBITMAP_ERROR, &pctx);
        ...
        else if (ino == EXT2_JOURNAL_INO)
        {
            ...
            if (fs->super->s_journal_inum == EXT2_JOURNAL_INO)
            {
                if (!LINUX_S_ISREG(inode->i_mode) &&
                    fix_problem(ctx, PR_1_JOURNAL_BAD_MODE, &pctx))
                {
            ...
            if ((inode->i_links_count || inode->i_blocks ||
                 inode->i_block[0]) &&
                fix_problem(ctx, PR_1_JOURNAL_INODE_NOT_CLEAR, &pctx))
            {
                memset(inode, 0, inode_size);
                ext2fs_icount_store(ctx->inode_link_info, ino, 0);
                e2fsck_write_inode_full(ctx, ino, inode, inode_size, "pass1");
                failed_csum = 0;
            }
        }
        else if (quota_inum_is_reserved(fs, ino))
        {
            ...
            {
                if (!LINUX_S_ISREG(inode->i_mode) &&
                    fix_problem(ctx, PR_1_QUOTA_BAD_MODE, &pctx))
                {
                    inode->i_mode = LINUX_S_IFREG;
                    e2fsck_write_inode(ctx, ino, inode, "pass1");
                    failed_csum = 0;
            ...
            if ((inode->i_links_count || inode->i_blocks ||
                 inode->i_block[0]) &&
                fix_problem(ctx, PR_1_QUOTA_INODE_NOT_CLEAR, &pctx))
            {
                memset(inode, 0, inode_size);
                ext2fs_icount_store(ctx->inode_link_info, ino, 0);
                e2fsck_write_inode_full(ctx, ino, inode, inode_size, "pass1");
        ...
        else if (ino < EXT2_FIRST_INODE(fs->super))
        {
            ...
            {
                if (inode->i_mode && !LINUX_S_ISREG(inode->i_mode))
                    problem = PR_1_RESERVED_BAD_MODE;
            }
            else
            {
                 ...
                 e2fsck_pass1_check_device_inode(fs, inode))
        {
            ...
            ctx->fs_blockdev_count++;
                 ...
                 e2fsck_pass1_check_symlink(fs, ino, inode, block_buf))
        {
            check_immutable(ctx, &pctx);
    ...
    {
        adjust_extattr_refcount(ctx, ctx->refcount_extra, block_buf, +1);
    ...
    if (ctx->flags & E2F_FLAG_RESIZE_INODE)
    {
        ...
        if (pctx.errcode)
        {
            if (!fix_problem(ctx, PR_1_RESIZE_INODE_CREATE, &pctx))
            {
                ctx->flags |= E2F_FLAG_ABORT;
                goto endit;
            }
        ...
        if (!pctx.errcode)
        {
            e2fsck_read_inode(ctx, EXT2_RESIZE_INO, inode, "recreate inode");
            inode->i_mtime = ctx->now;
            e2fsck_write_inode(ctx, EXT2_RESIZE_INO, inode, "recreate inode");
        }
...
static void process_inodes(e2fsck_t ctx, char* block_buf)
{
    ...
    for (i = 0; i < process_inode_count; i++)
    {
		...
		printf("%u ", pctx.ino);
#endif
        sprintf(buf, _("reading indirect blocks of inode %u"), pctx.ino);
...
static void mark_inode_bad(e2fsck_t ctx, ino_t ino)
{
    ...
    if (!ctx->inode_bad_map)
    {
        ...
        pctx.errcode = e2fsck_allocate_inode_bitmap(
            ctx->fs, _("bad inode map"), EXT2FS_BMAP64_RBTREE, "inode_bad_map",
            &ctx->inode_bad_map);
        if (pctx.errcode)
        {
            pctx.num = 3;
...
static void alloc_bb_map(e2fsck_t ctx)
{
    ...
    clear_problem_context(&pctx);
    pctx.errcode = e2fsck_allocate_inode_bitmap(
        ctx->fs, _("inode in bad block map"), EXT2FS_BMAP64_RBTREE,
...
static void alloc_imagic_map(e2fsck_t ctx)
{
    ...
    clear_problem_context(&pctx);
    pctx.errcode = e2fsck_allocate_inode_bitmap(
        ctx->fs, _("imagic inode map"), EXT2FS_BMAP64_RBTREE,
...
static _INLINE_ void mark_block_used(e2fsck_t ctx, blk64_t block)
{
    ...
    if (ext2fs_fast_test_block_bitmap2(ctx->block_found_map, block))
    {
        ...
        {
            pctx.errcode = e2fsck_allocate_block_bitmap(
                ctx->fs, _("multiply claimed block map"), EXT2FS_BMAP64_RBTREE,
                "block_dup_map", &ctx->block_dup_map);
            if (pctx.errcode)
            {
                pctx.num = 3;
                fix_problem(ctx, PR_1_ALLOCATE_BBITMAP_ERROR, &pctx);
                                    ...
                                    char* block_buf, int adjust_sign)
{
    ...
    while (1)
    {
        ...
        if (fix_problem(ctx, PR_1_EXTATTR_REFCOUNT, &pctx))
        {
            ...
            if (pctx.errcode)
            {
                fix_problem(ctx, PR_1_EXTATTR_WRITE_ABORT, &pctx);
                continue;
            }
        }
    }
                          ...
                          char* block_buf, struct ea_quota* ea_block_quota)
{
    ...
    {
        pctx->errcode = e2fsck_allocate_block_bitmap(
            fs, _("ext attr block map"), EXT2FS_BMAP64_RBTREE, "block_ea_map",
            &ctx->block_ea_map);
        if (pctx->errcode)
        {
            pctx->num = 2;
    ...
    if (ext2fs_fast_test_block_bitmap2(ctx->block_ea_map, blk))
    {
        ...
        if (ctx->ea_block_quota_blocks)
        {
            ea_refcount_fetch(ctx->ea_block_quota_blocks, blk, &quota_blocks);
            if (quota_blocks)
    ...
    if (((ctx->ext_attr_ver == 1) &&
        ...
        ((ctx->ext_attr_ver == 2) && (header->h_magic != EXT2_EXT_ATTR_MAGIC)))
    {
    ...
    while ((char*)entry < end && *(__u32*)entry)
    {
        ...
        else
        {
            ...
            problem =
                check_large_ea_inode(ctx, entry, pctx, &entry_quota_blocks);
     ...
     */
    if (failed_csum && fix_problem(ctx, PR_1_EA_BLOCK_ONLY_CSUM_INVALID, pctx))
    {
        pctx->errcode = ext2fs_write_ext_attr3(fs, blk, block_buf, pctx->ino);
        if (pctx->errcode)
            return 0;
...
void e2fsck_clear_inode(e2fsck_t ctx, ext2_ino_t ino, struct ext2_inode* inode,
                        int restart_flag, const char* source)
{
    inode->i_flags = 0;
    inode->i_links_count = 0;
    ext2fs_icount_store(ctx->inode_link_info, ino, 0);
...
static void scan_extent_node(e2fsck_t ctx, struct problem_context* pctx,
                             ...
                             blk64_t eof_block, ext2_extent_handle_t ehandle,
                             int try_repairs)
{
    struct ext2fs_extent extent;
    ...
    if (pctx->errcode)
        return;
    if (!(ctx->options & E2F_OPT_FIXES_ONLY) && !pb->eti.force_rebuild)
    {
    ...
    while (
        (pctx->errcode == 0 || pctx->errcode == EXT2_ET_EXTENT_CSUM_INVALID) &&
        info.num_entries-- > 0)
    {
        ...
        else if (is_leaf && (extent.e_pblk + extent.e_len) >
            ...
            if (fix_problem(ctx, problem, pctx))
            {
                if (ctx->invalid_bitmaps)
                {
                    ...
                    pctx->errcode = ext2fs_extent_get(
                        ehandle, EXT2_EXTENT_NEXT_SIB, &extent);
                ...
                pb->inode_modified = 1;
                pctx->errcode = ext2fs_extent_delete(ehandle, 0);
                if (pctx->errcode)
                {
                ...
                if (pctx->errcode && pctx->errcode != EXT2_ET_NO_CURRENT_NODE)
                {
                    pctx->str = "ext2fs_extent_fix_parents";
                    return;
                }
                pctx->errcode =
                    ext2fs_extent_get(ehandle, EXT2_EXTENT_CURRENT, &extent);
                if (pctx->errcode == EXT2_ET_NO_CURRENT_NODE)
                {
                    pctx->errcode = 0;
                    break;
        ...
        if (!is_leaf)
        {
            ...
            pctx->errcode =
                ext2fs_extent_get(ehandle, EXT2_EXTENT_DOWN, &extent);
            if (pctx->errcode && pctx->errcode != EXT2_ET_EXTENT_CSUM_INVALID)
            {
            ...
            scan_extent_node(ctx, pctx, pb, extent.e_lblk, last_lblk, eof_block,
                             ehandle, next_try_repairs);
            if (pctx->errcode)
                return;
            pctx->errcode = ext2fs_extent_get(ehandle, EXT2_EXTENT_UP, &extent);
            if (pctx->errcode)
            {
         ...
         */
        if (try_repairs && is_dir && pb->last_block + 1 < extent.e_lblk)
        {
            blk64_t new_lblk;

            new_lblk = pb->last_block + 1;
            if (EXT2FS_CLUSTER_RATIO(ctx->fs) > 1)
                new_lblk = ((new_lblk + EXT2FS_CLUSTER_RATIO(ctx->fs) - 1) &
                            ~EXT2FS_CLUSTER_MASK(ctx->fs)) |
                           (extent.e_pblk & EXT2FS_CLUSTER_MASK(ctx->fs));
            ...
            if (fix_problem(ctx, PR_1_COLLAPSE_DBLOCK, pctx))
            {
                ...
                pctx->errcode = ext2fs_extent_goto(ehandle, extent.e_lblk);
                if (pctx->errcode)
                    goto failed_add_dir_block;
                last_lblk = extent.e_lblk + extent.e_len - 1;
                failed_csum = 0;
    ...
    alloc_later:
        if (is_dir)
        {
            while (++pb->last_db_block < (e2_blkcnt_t)extent.e_lblk)
            {
                pctx->errcode = ext2fs_add_dir_block2(ctx->fs->dblist, pb->ino,
                                                      0, pb->last_db_block);
                if (pctx->errcode)
                {
                    pctx->blk = 0;
            ...
            {
                pctx->errcode =
                    ext2fs_add_dir_block2(ctx->fs->dblist, pctx->ino,
                                          extent.e_pblk + i, extent.e_lblk + i);
                if (pctx->errcode)
                {
        ...
        if (has_unaligned_cluster_map(ctx, pb->previous_block, pb->last_block,
                                      extent.e_pblk, extent.e_lblk))
        {
            for (i = 0; i < extent.e_len; i++)
            {
         ...
         */
        if (ctx->fs->cluster_ratio_bits && pb->previous_block &&
            (EXT2FS_B2C(ctx->fs, extent.e_pblk) ==
             EXT2FS_B2C(ctx->fs, pb->previous_block)))
            /* Set blk to the beginning of next cluster. */
            blk = EXT2FS_C2B(ctx->fs, EXT2FS_B2C(ctx->fs, extent.e_pblk) + 1);
        else
            /* Set blk to the beginning of current cluster. */
            blk = EXT2FS_C2B(ctx->fs, EXT2FS_B2C(ctx->fs, extent.e_pblk));

        if (blk < extent.e_pblk + extent.e_len)
        {
            mark_blocks_used(ctx, blk, extent.e_pblk + extent.e_len - blk);
        ...
        if (is_leaf && !is_dir && !(extent.e_flags & EXT2_EXTENT_FLAGS_UNINIT))
            pb->last_init_lblock = last_lblk;
    next:
        pctx->errcode =
            ext2fs_extent_get(ehandle, EXT2_EXTENT_NEXT_SIB, &extent);
    }

    /* Failed csum but passes checks?  Ask to fix checksum. */
    if (failed_csum && fix_problem(ctx, PR_1_EXTENT_ONLY_CSUM_INVALID, pctx))
    {
        pb->inode_modified = 1;
        pctx->errcode = ext2fs_extent_replace(ehandle, 0, &extent);
        if (pctx->errcode)
                                 ...
                                 struct process_block_struct* pb)
{
    ...
    if (pctx->errcode && fix_problem(ctx, PR_1_EXTENT_ITERATE_FAILURE, pctx))
    {
        pb->num_blocks = 0;
        inode->i_blocks = 0;
    ...
    if (LINUX_S_ISDIR(inode->i_mode) && e2fsck_dir_will_be_rehashed(ctx, ino))
        return;
                                     ...
                                     struct process_block_struct* pb)
{
    ...
    pctx->errcode =
        ext2fs_inline_data_size(ctx->fs, pb->ino, &inline_data_size);
                         ...
                         char* block_buf, const struct ea_quota* ea_ibody_quota)
{
    ...
    pb.ino = ino;
    pb.num_blocks =
        EXT2FS_B2C(ctx->fs, ea_ibody_quota ? ea_ibody_quota->blocks : 0);
    pb.last_block = ~0;
    ...
    else if (ext2fs_inode_has_valid_blocks2(fs, inode))
    {
        ...
        else
        {
            ...
            fs->flags |= EXT2_FLAG_IGNORE_CSUM_ERRORS;
            pctx->errcode =
                ext2fs_block_iterate3(fs, ino, pb.is_dir ? BLOCK_FLAG_HOLE : 0,
    ...
    if (pb.clear)
    {
        e2fsck_clear_inode(ctx, ino, inode, E2F_FLAG_RESTART, "check_blocks");
        return;
    }
    ...
    if (!pb.num_blocks && pb.is_dir && !(inode->i_flags & EXT4_INLINE_DATA_FL))
    {
    ...
    if (ino != quota_type2inum(PRJQUOTA, fs->super) &&
        ...
        !(inode->i_flags & EXT4_EA_INODE_FL))
    {
        quota_data_add(ctx->qctx, (struct ext2_inode_large*)inode, ino,
                       pb.num_blocks * EXT2_CLUSTER_SIZE(fs->super));
        quota_data_inodes(ctx->qctx, (struct ext2_inode_large*)inode, ino,
                          (ea_ibody_quota ? ea_ibody_quota->inodes : 0) +
    ...
    if (pb.is_dir)
    {
        ...
        if (inode->i_flags & EXT4_INLINE_DATA_FL)
        {
            ...
            flags = ctx->fs->flags;
            ctx->fs->flags |= EXT2_FLAG_IGNORE_CSUM_ERRORS;
            err = ext2fs_inline_data_size(ctx->fs, pctx->ino, &sz);
            ctx->fs->flags = (flags & EXT2_FLAG_IGNORE_CSUM_ERRORS) |
                             (ctx->fs->flags & ~EXT2_FLAG_IGNORE_CSUM_ERRORS);
    ...
    else
    {
        ...
        if ((pb.last_init_lblock >= 0) &&
            ...
            bad_size = 4;
                 ...
                 size > ((1ULL << (32 + EXT2_BLOCK_SIZE_BITS(fs->super))) - 1))
                         ...
                         int ref_offset EXT2FS_ATTR((unused)), void* priv_data)
{
        ...
        if (p->previous_block + 1 != blk)
        {
            if (ctx->options & E2F_OPT_FRAGCHECK)
            {
                ...
                printf(_("%6lu(%c): expecting %6lu "
                       ...
                       (unsigned long)pctx->ino, type,
                       (unsigned long)p->previous_block + 1, (unsigned long)blk,
                       blockcnt);
            }
            p->fragmented = 1;
        }
     ...
     */
    if (blockcnt < 0 && p->ino != EXT2_RESIZE_INO &&
    ...
    if (problem)
    {
         ...
         */
        if (p->ino != EXT2_RESIZE_INO && ref_block != 0 &&
            ext2fs_test_block_bitmap2(ctx->block_metadata_map, ref_block))
        {
            *block_nr = 0;
            return 0;
        }
        if (!p->suppress && (p->num_illegal_blocks % 12) == 0)
        {
            ...
            {
                p->suppress = 1;
                set_latch_flags(PR_LATCH_BLOCK, PRL_SUPPRESS, 0);
            }
        }
        pctx->blk = blk;
    ...
    else if (!(ctx->fs->cluster_ratio_bits && p->previous_block &&
    ...
    else if (has_unaligned_cluster_map(ctx, p->previous_block, p->last_block,
                                       blk, blockcnt))
    {
        pctx->blk = blockcnt;
        pctx->blk2 = blk;
...
mark_dir:
    if (p->is_dir && (blockcnt >= 0))
    {
        ...
        {
            pctx->errcode =
                ext2fs_add_dir_block2(fs->dblist, p->ino, 0, p->last_db_block);
            if (pctx->errcode)
            {
                pctx->blk = 0;
        ...
        pctx->errcode =
            ext2fs_add_dir_block2(fs->dblist, p->ino, blk, blockcnt);
        if (pctx->errcode)
        {
            pctx->blk = blk;
            pctx->num = blockcnt;
                             ...
                             void* priv_data)
{
    ...
    for (i = 0; i < fs->group_desc_count; i++)
    {
        ...
        if (blk == first_block)
        {
            if (i == 0)
            {
                if (fix_problem(ctx, PR_1_BAD_PRIMARY_SUPERBLOCK, pctx))
                {
                    *block_nr = 0;
                    return BLOCK_CHANGED;
                }
        ...
        if ((blk > first_block) && (blk <= first_block + fs->desc_blocks))
        {
            ...
            {
                pctx->blk = *block_nr;
                if (fix_problem(ctx, PR_1_BAD_PRIMARY_GROUP_DESCRIPTOR, pctx))
                {
                    *block_nr = 0;
                    return BLOCK_CHANGED;
                }
    ...
    skip_super:
        ...
        if ((blk >= ext2fs_inode_table_loc(fs, i)) &&
            (blk <
             (ext2fs_inode_table_loc(fs, i) + fs->inode_blocks_per_group)))
        {
            /*
                            ...
                            const char* name, int num, blk64_t* new_block)
{
    ...
    if (is_flexbg)
    {
        ...
        first_block = ext2fs_group_first_block2(fs, flexbg_size * flexbg);
    ...
    pctx.errcode = ext2fs_get_free_blocks2(fs, first_block, last_block, num,
                                           ctx->block_found_map, new_block);
    if (is_flexbg && (pctx.errcode == EXT2_ET_BLOCK_ALLOC_FAIL))
        pctx.errcode = ext2fs_get_free_blocks2(
            fs, fs->super->s_first_data_block, ext2fs_blocks_count(fs->super),
    ...
    pctx.blk2 = *new_block;
    fix_problem(ctx, (old_block ? PR_1_RELOC_FROM_TO : PR_1_RELOC_TO), &pctx);
    pctx.blk2 = 0;
    for (i = 0; i < num; i++)
    {
        pctx.blk = i;
        ...
        if (old_block)
        {
            pctx.errcode = io_channel_read_blk64(fs->io, old_block + i, 1, buf);
            if (pctx.errcode)
                fix_problem(ctx, PR_1_RELOC_READ_ERR, &pctx);
            pctx.blk = (*new_block) + i;
            pctx.errcode = io_channel_write_blk64(fs->io, pctx.blk, 1, buf);
        }
        else
        {
            pctx.blk = (*new_block) + i;
            pctx.errcode = ext2fs_zero_blocks2(fs, pctx.blk, 1, NULL, NULL);
        }
...
static void handle_fs_bad_blocks(e2fsck_t ctx)
{
    ...
    for (i = 0; i < fs->group_desc_count; i++)
    {
        ...
        if (ctx->invalid_block_bitmap_flag[i])
        {
            ...
            new_table_block(ctx, first_block, i, _("block bitmap"), 1,
                            &new_blk);
        ...
        if (ctx->invalid_inode_bitmap_flag[i])
        {
            ...
            new_table_block(ctx, first_block, i, _("inode bitmap"), 1,
                            &new_blk);
    ...
    for (i = 0; i < fs->group_desc_count; i++)
    {
        ...
        if (ext2fs_inode_table_loc(fs, i))
        {
            for (j = 0, b = ext2fs_inode_table_loc(fs, i);
                 j < fs->inode_blocks_per_group; j++, b++)
            {
                if (ext2fs_test_block_bitmap2(ctx->block_found_map, b))
                {
                    pctx.blk = b;
                    if (!ctx->invalid_inode_table_flag[i] &&
                        fix_problem(ctx, PR_1_ITABLE_CONFLICT, &pctx))
                    {
                ...
                {
                    ext2fs_mark_block_bitmap2(ctx->block_found_map, b);
                    ext2fs_mark_block_bitmap2(ctx->block_metadata_map, b);
                }
            }
        }
 ...
 * the inode again.
 */
static errcode_t pass1_get_blocks(ext2_filsys fs, ext2_ino_t ino, blk_t* blocks)
{
                                        ...
                                        blk64_t* ret)
{
    ...
    {
        retval = ext2fs_new_block2(fs, goal, ctx->block_found_map, &new_block);
        if (retval)
            return retval;
        if (fs->block_map)
        {
                                  ...
                                  blk64_t len, blk64_t* pblk, blk64_t* plen)
{
    ...
    if (ctx->block_found_map)
        return ext2fs_new_range(fs, flags, goal, len, ctx->block_found_map,
                                pblk, plen);
    ...
    return ext2fs_new_range(fs, flags, goal, len, fs->block_map, pblk, plen);
}
                                           ...
                                           blk_t num, int inuse)
{
    ...
    {
        if (inuse > 0)
            ext2fs_mark_block_bitmap_range2(ctx->block_found_map, blk, num);
        else
            ext2fs_unmark_block_bitmap_range2(ctx->block_found_map, blk, num);
    }
}
...
void e2fsck_intercept_block_allocations(e2fsck_t ctx)
{
    ...
    ext2fs_set_block_alloc_stats_callback(ctx->fs, e2fsck_block_alloc_stats, 0);
