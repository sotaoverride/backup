...
static int write_access2_rules(const char* srcdir)
{
    ...
    load2_fd = open("/sys/fs/smackfs/load2",
    ...
    change_fd = open("/sys/fs/smackfs/change-rule",
                     O_RDWR | O_CLOEXEC | O_NONBLOCK | O_NOCTTY);
    if (change_fd < 0)
    {
        if (errno != ENOENT)
            log_warning_errno(
                errno, "Failed to open '/sys/fs/smackfs/change-rule': %m");
        return -errno; /* negative error */
    }
    ...
    FOREACH_DIRENT(entry, dir, return 0)
    {
        ...
        for (;;)
        {
            _cleanup_free_ char *buf = NULL, *sbj = NULL, *obj = NULL,
                                *acc1 = NULL, *acc2 = NULL;
            int q;

            q = read_line(policy, NAME_MAX, &buf);
            if (q < 0)
                return log_error_errno(q, "Failed to read line from '%s': %m",
                                       entry->d_name);
            if (q == 0)
                break;
            ...
            if (write(isempty(acc2) ? load2_fd : change_fd, buf, strlen(buf)) <
                0)
            {
                if (r == 0)
                    r = -errno;
                log_error_errno(errno,
                                "Failed to write '%s' to '%s' in '%s': %m", buf,
...
static int write_cipso2_rules(const char* srcdir)
{
    ...
    cipso2_fd = open("/sys/fs/smackfs/cipso2",
                     O_RDWR | O_CLOEXEC | O_NONBLOCK | O_NOCTTY);
    if (cipso2_fd < 0)
    {
        if (errno != ENOENT)
...
static int write_onlycap_list(void)
{
    ...
    for (;;)
    {
        ...
        if (r < 0)
            return log_error_errno(
                r, "Failed to read line from /etc/smack/onlycap: %m");
        if (r == 0)
            break;
    ...
    onlycap_fd = open("/sys/fs/smackfs/onlycap",
    ...
    if (r < 0)
        return log_error_errno(
            errno,
            "Failed to write onlycap list(%s) to '/sys/fs/smackfs/onlycap': %m",
            list);
...
int mac_smack_setup(bool* loaded_policy)
{
    ...
    switch (r)
    {
        case -ENOENT:
            ...
            return 0;
        case ENOENT:
            log_debug("Smack access rules directory '/etc/smack/accesses.d/' "
                      "not found");
            return 0;
        case 0:
        ...
        default:
            log_warning_errno(
                r, "Failed to load Smack access rules, ignoring: %m");
            return 0;
    }
    ...
    if (r < 0)
        log_warning_errno(r, "Failed to set SMACK label \"" SMACK_RUN_LABEL
                             "\" on self: %m");
    r = write_string_file("/sys/fs/smackfs/ambient", SMACK_RUN_LABEL,
                          WRITE_STRING_FILE_DISABLE_BUFFER);
    if (r < 0)
        log_warning_errno(
            r, "Failed to set SMACK ambient label \"" SMACK_RUN_LABEL "\": %m");
    r = write_string_file("/sys/fs/smackfs/netlabel",
                          "0.0.0.0/0 " SMACK_RUN_LABEL,
                          WRITE_STRING_FILE_DISABLE_BUFFER);
    if (r < 0)
        log_warning_errno(
            r, "Failed to set SMACK netlabel rule \"0.0.0.0/0 " SMACK_RUN_LABEL
               "\": %m");
    r = write_string_file("/sys/fs/smackfs/netlabel", "127.0.0.1 -CIPSO",
                          WRITE_STRING_FILE_DISABLE_BUFFER);
    if (r < 0)
        log_warning_errno(
            r, "Failed to set SMACK netlabel rule \"127.0.0.1 -CIPSO\": %m");
#endif
    ...
    switch (r)
    {
        case -ENOENT:
            ...
            return 0;
        case ENOENT:
            log_debug("Smack/CIPSO access rules directory "
                      "'/etc/smack/cipso.d/' not found");
            break;
        case 0:
        ...
        default:
            log_warning_errno(
                r, "Failed to load Smack/CIPSO access rules, ignoring: %m");
            break;
    }
    ...
    switch (r)
    {
        case -ENOENT:
            ...
            return 0;
        case ENOENT:
            log_debug("Smack network host rules directory "
                      "'/etc/smack/netlabel.d/' not found");
            break;
        case 0:
        ...
        default:
            log_warning_errno(
                r, "Failed to load Smack network host rules: %m, ignoring.");
            break;
    }
