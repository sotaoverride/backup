...
#define EXFS_MIN_DBLOCKS(s)                                                    \
    ((uint64_t)((s)->sb_agcount - 1) * (s)->sb_agblocks + EXFS_MIN_AG_BLOCKS)
...
static int exfs_verify_sb(struct exfs_super_block* ondisk)
{
    ...
    if (sbp->sb_agcount <= 0 || sbp->sb_sectsize < EXFS_MIN_SECTORSIZE ||
        ...
        sbp->sb_dblocks == 0 || sbp->sb_dblocks > EXFS_MAX_DBLOCKS(sbp) ||
