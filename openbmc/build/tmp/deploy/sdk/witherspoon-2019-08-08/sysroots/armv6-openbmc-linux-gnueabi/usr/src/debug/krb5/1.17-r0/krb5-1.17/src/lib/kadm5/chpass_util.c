 ...
 *
 * Purpose: Wrapper around chpass_principal. We can read new pw, change pw and
...
kadm5_ret_t _kadm5_chpass_principal_util(void* server_handle, void* lhandle,
                                         ...
                                         unsigned int msg_len)
{
    ...
    { /* read the password */
        ...
        if ((code = (int)kadm5_init_krb5_context(&context)) == 0)
        {
            ...
            code = krb5_read_password(context, KADM5_PW_FIRST_PROMPT,
                                      KADM5_PW_SECOND_PROMPT, buffer, &pwsize);
        ...
        if (pwsize == 0)
        {
            ...
            memset(buffer, 0, sizeof(buffer));
#endif
            strncpy(msg_ret, string_text(CHPASS_UTIL_NO_PASSWORD_READ),
                    msg_len - 1);
            msg_ret[msg_len - 1] = '\0';
    ...
    if (code == KADM5_OK)
    {
        strncpy(msg_ret, string_text(CHPASS_UTIL_PASSWORD_CHANGED),
                msg_len - 1);
        msg_ret[msg_len - 1] = '\0';
        return (0);
    }

    if ((code != KADM5_PASS_Q_TOOSHORT) && (code != KADM5_PASS_REUSE) &&
        (code != KADM5_PASS_Q_CLASS) && (code != KADM5_PASS_Q_DICT) &&
    ...
    {
        snprintf(msg_ret, msg_len, "%s %s\n%s %s\n\n%s\n", error_message(code2),
                 string_text(CHPASS_UTIL_GET_PRINC_INFO), error_message(code),
                 string_text(CHPASS_UTIL_WHILE_TRYING_TO_CHANGE),
                 string_text(CHPASS_UTIL_PASSWORD_NOT_CHANGED));
        msg_ret[msg_len - 1] = '\0';
        return (code);
    ...
    code2 = kadm5_get_policy(lhandle, princ_ent.policy, &policy_ent);
    if (code2 != 0)
    {
        snprintf(msg_ret, msg_len, "%s %s\n%s %s\n\n%s\n ",
                 error_message(code2), string_text(CHPASS_UTIL_GET_POLICY_INFO),
                 error_message(code),
                 string_text(CHPASS_UTIL_WHILE_TRYING_TO_CHANGE),
                 string_text(CHPASS_UTIL_PASSWORD_NOT_CHANGED));
        (void)kadm5_free_principal_ent(lhandle, &princ_ent);
    ...
    snprintf(msg_ret, msg_len, "%s\n%s %s\n",
             string_text(CHPASS_UTIL_PASSWORD_NOT_CHANGED), error_message(code),
             string_text(CHPASS_UTIL_WHILE_TRYING_TO_CHANGE));
    (void)kadm5_free_principal_ent(lhandle, &princ_ent);
    (void)kadm5_free_policy_ent(lhandle, &policy_ent);
    return (code);
