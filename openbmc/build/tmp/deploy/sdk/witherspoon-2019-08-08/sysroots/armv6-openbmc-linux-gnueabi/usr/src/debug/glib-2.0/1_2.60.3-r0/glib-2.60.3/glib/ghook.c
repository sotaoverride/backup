...
/* --- functions --- */
static void default_finalize_hook(GHookList* hook_list, GHook* hook)
{
 ...
 */
void g_hook_list_marshal_check(GHookList* hook_list, gboolean may_recurse,
    ...
    if (hook_list->is_setup)
    {
        ...
        if (hook)
        {
            ...
            if (G_HOOK_IS_VALID(hook) &&
                (may_be_in_call || !G_HOOK_IN_CALL(hook)))
                return hook;
            else
 ...
 */
GHook* g_hook_next_valid(GHookList* hook_list, GHook* hook,
                   ...
                   GHookFindFunc func, gpointer data)
{
    ...
    while (hook)
    {
        ...
        if (func(hook, data) && hook->hook_id &&
                        ...
                        gpointer data)
{
    ...
    {
        /* test only non-destroyed hooks */
        if (hook->data == data && hook->hook_id &&
            (!need_valids || G_HOOK_ACTIVE(hook)))
            return hook;
                        ...
                        gpointer func)
{
    ...
    {
        /* test only non-destroyed hooks */
        if (hook->func == func && hook->hook_id &&
            (!need_valids || G_HOOK_ACTIVE(hook)))
            return hook;
 ...
 */
GHook* g_hook_find_func_data(GHookList* hook_list, gboolean need_valids,
                             gpointer func, gpointer data)
{
    ...
    {
        /* test only non-destroyed hooks */
        if (hook->data == data && hook->func == func && hook->hook_id &&
            (!need_valids || G_HOOK_ACTIVE(hook)))
            return hook;
