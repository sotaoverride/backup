...
static GDBusInterfaceInfo* org_gtk_Menus_get_interface(void)
{
    ...
    if (interface_info == NULL)
    {
        ...
        interface_info =
            g_dbus_node_info_lookup_interface(info, "org.gtk.Menus");
...
static GMenuExporterGroup* g_menu_exporter_lookup_group(GMenuExporter* exporter,
                                                        guint group_id);
static void g_menu_exporter_report(GMenuExporter* exporter, GVariant* report);
static void g_menu_exporter_remove_group(GMenuExporter* exporter, guint id);
...
static void g_menu_exporter_menu_free(GMenuExporterMenu* menu)
{
...
static void g_menu_exporter_link_free(gpointer data)
{
...
static GMenuExporterLink*
    g_menu_exporter_menu_create_links(GMenuExporterMenu* menu, gint position)
{
    ...
    while (g_menu_link_iter_get_next(iter, &name, &model))
    {
        ...
        if (!g_str_equal(name, "section"))
            group = g_menu_exporter_create_group(
                g_menu_exporter_group_get_exporter(menu->group));
        else
            group = menu->group;
...
static GVariant* g_menu_exporter_menu_describe_item(GMenuExporterMenu* menu,
                                                    gint position)
{
    ...
    for (link = g_sequence_get(iter); link; link = link->next)
        g_variant_builder_add(
            &builder, "{sv}", link->name,
            g_variant_new("(uu)",
                          g_menu_exporter_group_get_id(link->menu->group),
                          link->menu->id));

    return g_variant_builder_end(&builder);
}

static GVariant* g_menu_exporter_menu_list(GMenuExporterMenu* menu)
{
    GVariantBuilder builder;
    gint i, n;
    ...
    for (i = 0; i < n; i++)
        g_variant_builder_add_value(
            &builder, g_menu_exporter_menu_describe_item(menu, i));

    return g_variant_builder_end(&builder);
}

static void g_menu_exporter_menu_items_changed(GMenuModel* model, gint position,
                                               ...
                                               gpointer user_data)
{
    ...
    g_sequence_remove_range(
        g_sequence_get_iter_at_pos(menu->item_links, position), point);

    for (i = position; i < position + added; i++)
        g_sequence_insert_before(point,
                                 g_menu_exporter_menu_create_links(menu, i));
    ...
    if (g_menu_exporter_group_is_subscribed(menu->group))
    {
        ...
        g_variant_builder_add(&builder, "u",
                              g_menu_exporter_group_get_id(menu->group));
        ...
        g_menu_exporter_report(g_menu_exporter_group_get_exporter(menu->group),
                               g_variant_builder_end(&builder));
    }
}

static void g_menu_exporter_menu_prepare(GMenuExporterMenu* menu)
{
    gint n_items;
    ...
    if (g_menu_model_is_mutable(menu->model))
        menu->handler_id = g_signal_connect(
...
static GMenuExporterMenu* g_menu_exporter_menu_new(GMenuExporterGroup* group,
                                                   guint id, GMenuModel* model)
{
    GMenuExporterMenu* menu;
...
static void g_menu_exporter_group_check_if_useless(GMenuExporterGroup* group)
{
...
static void g_menu_exporter_group_subscribe(GMenuExporterGroup* group,
                                            GVariantBuilder* builder)
{
    GHashTableIter iter;
    gpointer key, val;
...
static void g_menu_exporter_group_unsubscribe(GMenuExporterGroup* group,
                                              gint count)
{
...
static GMenuExporterMenu*
    g_menu_exporter_group_add_menu(GMenuExporterGroup* group, GMenuModel* model)
{
    GMenuExporterMenu* menu;
    guint id;
...
static GMenuExporterGroup* g_menu_exporter_group_new(GMenuExporter* exporter,
                                                     guint id)
{
    GMenuExporterGroup* group;
...
static void g_menu_exporter_remote_subscribe(GMenuExporterRemote* remote,
                                             guint group_id,
                                             GVariantBuilder* builder)
{
    GMenuExporterGroup* group;
    ...
    count =
        (gsize)g_hash_table_lookup(remote->watches, GINT_TO_POINTER(group_id));
    g_hash_table_insert(remote->watches, GINT_TO_POINTER(group_id),
...
static void g_menu_exporter_remote_unsubscribe(GMenuExporterRemote* remote,
                                               guint group_id)
{
    GMenuExporterGroup* group;
    guint count;
...
static void g_menu_exporter_remote_free(gpointer data)
{
    ...
    while (g_hash_table_iter_next(&iter, &key, &val))
    {
        ...
        group = g_menu_exporter_lookup_group(remote->exporter,
                                             GPOINTER_TO_INT(key));
...
static GMenuExporterRemote* g_menu_exporter_remote_new(GMenuExporter* exporter,
                                                       guint watch_id)
{
...
static void g_menu_exporter_name_vanished(GDBusConnection* connection,
...
static GVariant* g_menu_exporter_subscribe(GMenuExporter* exporter,
                                           const gchar* sender,
                                           GVariant* group_ids)
{
    GMenuExporterRemote* remote;
    ...
    if (remote == NULL)
    {
        if (sender != NULL)
        {
            ...
            watch_id = g_bus_watch_name_on_connection(
                exporter->connection, sender, G_BUS_NAME_WATCHER_FLAGS_NONE,
...
static void g_menu_exporter_unsubscribe(GMenuExporter* exporter,
                                        const gchar* sender,
                                        GVariant* group_ids)
{
    GMenuExporterRemote* remote;
    ...
    if (!g_menu_exporter_remote_has_subscriptions(remote))
    {
        if (sender != NULL)
            ...
            g_clear_pointer(&exporter->peer_remote,
...
static void g_menu_exporter_report(GMenuExporter* exporter, GVariant* report)
{
    ...
    g_dbus_connection_emit_signal(
        exporter->connection, NULL, exporter->object_path, "org.gtk.Menus",
        "Changed", g_variant_builder_end(&builder), NULL);
}
...
static void g_menu_exporter_free(gpointer user_data)
{
    GMenuExporter* exporter = user_data;
...
static void g_menu_exporter_method_call(
    ...
    GDBusMethodInvocation* invocation, gpointer user_data)
{
    ...
    if (g_str_equal(method_name, "Start"))
        g_dbus_method_invocation_return_value(
            invocation, g_menu_exporter_subscribe(exporter, sender, group_ids));
                                          ...
                                          GMenuModel* menu, GError** error)
{
    ...
    id = g_dbus_connection_register_object(
        connection, object_path, org_gtk_Menus_get_interface(), &vtable,
        exporter, g_menu_exporter_free, error);
    ...
    exporter->remotes = g_hash_table_new_full(g_str_hash, g_str_equal, g_free,
                                              g_menu_exporter_remote_free);
    exporter->root = g_menu_exporter_group_add_menu(
        g_menu_exporter_create_group(exporter), menu);
