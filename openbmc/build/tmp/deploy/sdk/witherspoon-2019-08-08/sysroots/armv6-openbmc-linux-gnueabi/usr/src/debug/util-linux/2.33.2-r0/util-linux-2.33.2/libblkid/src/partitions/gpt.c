...
{
    return (ul_crc32_exclude_offset(~0L, buf, len, exclude_off, exclude_len) ^
            ~0L);
}

static inline unsigned char* get_lba_buffer(blkid_probe pr, uint64_t lba,
                                            size_t bytes)
{
    return blkid_probe_get_buffer(pr, blkid_probe_get_sectorsize(pr) * lba,
                                  bytes);
}
 ...
 * disabled by BLKID_PARTS_FOPCE_GPT flag (see also
 * blkid_partitions_set_flags()).
 */
 ...
 */
static struct gpt_header* get_gpt_header(blkid_probe pr, struct gpt_header* hdr,
                                         ...
                                         uint64_t lastlba)
{
    ...
    {
        DBG(LOWPROBE, ul_debug("GPT->MyLBA mismatch with real position"));
        return NULL;
    }
    ...
    {
        DBG(LOWPROBE, ul_debug("GPT->{First,Last}UsableLBA out of range"));
        return NULL;
    }
    ...
    /* Read GPT entries */
    *ents = (struct gpt_entry*)get_lba_buffer(
        pr, le64_to_cpu(h->partition_entries_lba), esz);
    if (!*ents)
    {
                        ...
                        __attribute__((__unused__)))
{
    ...
    for (i = 0; i < le32_to_cpu(h->num_partition_entries); i++, e++)
    {
        ...
        uint64_t size =
            le64_to_cpu(e->ending_lba) - le64_to_cpu(e->starting_lba) + 1ULL;
        ...
        {
            DBG(LOWPROBE,
                ul_debug("GPT entry[%d] overflows usable area - ignore", i));
            blkid_partlist_increment_partno(ls);
            continue;
        }

        par = blkid_partlist_add_partition(ls, tab, start * ssf, size * ssf);
        if (!par)
            goto err;

        blkid_partition_set_utf8name(par, (unsigned char*)e->partition_name,
                                     sizeof(e->partition_name),
                                     BLKID_ENC_UTF16LE);
...
static int probe_pmbr_pt(blkid_probe pr, const struct blkid_idmag* mag
                         __attribute__((__unused__)))
{
    int has = 0;
    struct gpt_entry* e;
