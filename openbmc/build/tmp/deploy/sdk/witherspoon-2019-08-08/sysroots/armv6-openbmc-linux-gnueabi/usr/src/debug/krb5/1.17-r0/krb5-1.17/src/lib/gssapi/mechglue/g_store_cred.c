...
static OM_uint32 store_cred_fallback(
    OM_uint32* minor_status, gss_mechanism mech, gss_cred_id_t mech_cred,
    gss_cred_usage_t cred_usage, gss_OID desired_mech, OM_uint32 overwrite_cred,
    ...
    gss_OID_set* elements_stored, gss_cred_usage_t* cred_usage_stored)
{
    ...
    {
        return mech->gss_store_cred_into(
            minor_status, mech_cred, cred_usage, public_mech, overwrite_cred,
            default_cred, cred_store, elements_stored, cred_usage_stored);
    }
    else if (cred_store == GSS_C_NO_CRED_STORE)
    {
        return mech->gss_store_cred(minor_status, mech_cred, cred_usage,
                                    public_mech, overwrite_cred, default_cred,
...
static OM_uint32 val_store_cred_args(
    OM_uint32* minor_status, const gss_cred_id_t input_cred_handle,
...
OM_uint32 KRB5_CALLCONV gss_store_cred(minor_status, input_cred_handle,
                                       cred_usage, desired_mech, overwrite_cred,
                                       default_cred, elements_stored,
                                       cred_usage_stored)
...
OM_uint32 KRB5_CALLCONV gss_store_cred_into(minor_status, input_cred_handle,
...

{
    ...
    if (desired_mech != GSS_C_NULL_OID)
    {
        ...
        if (mech->gss_store_cred == NULL && mech->gss_store_cred_into == NULL)
            return (major_status);
    ...
    *minor_status = 0;
    ...
    for (i = 0; i < union_cred->count; i++)
    {
        ...
        major_status = store_cred_fallback(
            minor_status, mech, mech_cred, cred_usage, dmech, overwrite_cred,
            default_cred, cred_store, NULL, cred_usage_stored);
        if (major_status != GSS_S_COMPLETE)
        {
            map_error(minor_status, mech);
            continue;
        ...
        {
            major_status =
                gss_create_empty_oid_set(minor_status, elements_stored);
        ...
        major_status =
            gss_add_oid_set_member(minor_status, dmech, elements_stored);
