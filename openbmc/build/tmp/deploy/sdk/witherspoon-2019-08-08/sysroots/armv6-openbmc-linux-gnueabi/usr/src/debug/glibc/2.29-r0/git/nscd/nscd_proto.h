...
extern int __nscd_getpwuid_r(uid_t uid, struct passwd* resultbuf, char* buffer,
                             size_t buflen,
                             struct passwd** result) attribute_hidden;
extern int __nscd_getgrnam_r(const char* name, struct group* resultbuf,
                             char* buffer, size_t buflen,
                             struct group** result) attribute_hidden;
extern int __nscd_getgrgid_r(gid_t gid, struct group* resultbuf, char* buffer,
                             size_t buflen,
                             struct group** result) attribute_hidden;
extern int __nscd_gethostbyname_r(const char* name, struct hostent* resultbuf,
                                  char* buffer, size_t buflen,
...
extern int __nscd_innetgr(const char* netgroup, const char* host,
                          const char* user,
                          const char* domain) attribute_hidden;
extern int __nscd_setnetgrent(const char* group,
                              struct __netgrent* datap) attribute_hidden;
