...
struct pthread
{
    union
    {
        ...
        struct
        {
            /* multiple_threads is enabled either when the process has spawned
           at least one thread or when a single-threaded process cancels itself.
...
#define DEQUEUE_MUTEX(mutex)                                                   \
    ...
    {                                                                          \
        __pthread_list_t* next =                                               \
            (__pthread_list_t*)((char*)(((uintptr_t)                           \
                                             mutex->__data.__list.__next) &    \
                                        ~1ul) -                                \
                                QUEUE_PTR_ADJUST);                             \
    ...
    {                                                                          \
        ...
        {                                                                      \
            __pthread_slist_t* next =                                          \
                (__pthread_slist_t*)(((uintptr_t)runp->__next) & ~1ul);        \
