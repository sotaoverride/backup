...
#define G_DBUS_CONNECTION(o)                                                   \
    (G_TYPE_CHECK_INSTANCE_CAST((o), G_TYPE_DBUS_CONNECTION, GDBusConnection))
#define G_IS_DBUS_CONNECTION(o)                                                \
    (G_TYPE_CHECK_INSTANCE_TYPE((o), G_TYPE_DBUS_CONNECTION))
 ...
 * @invocation: (transfer full): A #GDBusMethodInvocation object that must be
 * used to return a value or error.
 ...
 *
 * Returns: A newly allocated array of strings for node names that are children
 * of @object_path.
 *
 * Since: 2.26
 */
 ...
 * @object_path: The object path that was registered with
 * g_dbus_connection_register_subtree().
 * @node: A node that is a child of @object_path (relative to @object_path) or
 * %NULL for the root of the subtree.
 * @user_data: The @user_data #gpointer passed to
 * g_dbus_connection_register_subtree().
 *
 ...
 * case.
 *
 * Returns: A %NULL-terminated array of pointers to #GDBusInterfaceInfo, or
 * %NULL.
 *
 * Since: 2.26
 */
typedef GDBusInterfaceInfo** (*GDBusSubtreeIntrospectFunc)(
    GDBusConnection* connection, const gchar* sender, const gchar* object_path,
    const gchar* node, gpointer user_data);
 ...
 * @object_path: The object path that was registered with
 * g_dbus_connection_register_subtree().
 * @interface_name: The D-Bus interface name that the method call or property
 * access is for.
 * @node: A node that is a child of @object_path (relative to @object_path) or
 * %NULL for the root of the subtree.
 * @out_user_data: (nullable) (not optional): Return location for user data to
 * pass to functions in the returned #GDBusInterfaceVTable (never %NULL).
 * @user_data: The @user_data #gpointer passed to
 * g_dbus_connection_register_subtree().
 *
 ...
 * segment of the object path (ie: it never contains a slash).
 *
 * Returns: A #GDBusInterfaceVTable or %NULL if you don't want to handle the
 * methods.
 *
 * Since: 2.26
 */
typedef const GDBusInterfaceVTable* (*GDBusSubtreeDispatchFunc)(
    GDBusConnection* connection, const gchar* sender, const gchar* object_path,
    const gchar* interface_name, const gchar* node, gpointer* out_user_data,
    gpointer user_data);
 ...
 * @message: (transfer full): A locked #GDBusMessage that the filter function
 * takes ownership of.
 * @incoming: %TRUE if it is a message received from the other peer, %FALSE if
 * it is a message to be sent to the other peer.
