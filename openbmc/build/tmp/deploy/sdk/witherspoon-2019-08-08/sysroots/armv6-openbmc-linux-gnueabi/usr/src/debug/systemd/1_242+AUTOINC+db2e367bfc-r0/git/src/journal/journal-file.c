...
/* This is the default minimal use limit, how much we'll use even if keep_free
 * suggests otherwise. */
...
/* How much to increase the journal file size at once each time we allocate
 * something new. */
...
/* The mmap context to use for the header we pick as one above the last defined
 * typed */
...
/* This may be called from a separate thread to prevent blocking the caller for
 * the duration of fsync(). As a result we use atomic operations on
 * f->offline_state for inter-thread communications with
...
static void journal_file_set_offline_internal(JournalFile* f)
{
    ...
    for (;;)
    {
        ...
        {
            case OFFLINE_CANCEL:
                if (!__sync_bool_compare_and_swap(&f->offline_state,
                                                  OFFLINE_CANCEL, OFFLINE_DONE))
                    continue;
                return;
            ...
            case OFFLINE_SYNCING:
                ...
                if (!__sync_bool_compare_and_swap(
                        &f->offline_state, OFFLINE_SYNCING, OFFLINE_OFFLINING))
                    continue;
            ...
            case OFFLINE_OFFLINING:
                if (!__sync_bool_compare_and_swap(
                        &f->offline_state, OFFLINE_OFFLINING, OFFLINE_DONE))
                    continue;
                _fallthrough_;
            case OFFLINE_DONE:
                return;
            ...
            case OFFLINE_JOINED:
                log_debug("OFFLINE_JOINED unexpected offline state for "
                          "journal_file_set_offline_internal()");
                return;
        }
    }
}
...
int journal_file_set_offline(JournalFile* f, bool wait)
{
    ...
    /* An offlining journal is implicitly online and may modify
     * f->header->state, we must also join any potentially lingering offline
     * thread when not online. */
    if (!journal_file_is_offlining(f) && f->header->state != STATE_ONLINE)
        return journal_file_set_offline_thread_join(f);

    /* Restart an in-flight offline thread and wait if needed, or join a
     * lingering done one. */
    ...
    else
    {
        ...
        /* Don't block SIGBUS since the offlining thread accesses a memory
         * mapped file. Asynchronous SIGBUS signals can safely be handled by
         * either thread. */
        ...
        r = pthread_create(&f->offline_thread, NULL,
                           journal_file_set_offline_thread, f);
...
static int journal_file_set_online(JournalFile* f)
{
    ...
    while (wait)
    {
        switch (f->offline_state)
        {
            ...
            case OFFLINE_SYNCING:
                if (!__sync_bool_compare_and_swap(
                        &f->offline_state, OFFLINE_SYNCING, OFFLINE_CANCEL))
                    continue;
            ...
            case OFFLINE_AGAIN_FROM_OFFLINING:
                if (!__sync_bool_compare_and_swap(&f->offline_state,
                                                  ...
                                                  OFFLINE_CANCEL))
                    continue;
                /* Canceled restart from offlining, must wait for offlining to
                 * complete however. */
                _fallthrough_;
            default:
            {
...
static int journal_file_init_header(JournalFile* f, JournalFile* template)
{
    ...
    h.incompatible_flags |=
        htole32(f->compress_xz * HEADER_INCOMPATIBLE_COMPRESSED_XZ |
                f->compress_lz4 * HEADER_INCOMPATIBLE_COMPRESSED_LZ4);

    h.compatible_flags = htole32(f->seal * HEADER_COMPATIBLE_SEALED);
...
static int journal_file_refresh_header(JournalFile* f)
{
    ...
    /* We likely just created a new file, also sync the directory this file is
     * located in. */
...
{
    const uint32_t any = compatible ? HEADER_COMPATIBLE_ANY
                                    : HEADER_INCOMPATIBLE_ANY,
                   supported = compatible ? HEADER_COMPATIBLE_SUPPORTED
                                          : HEADER_INCOMPATIBLE_SUPPORTED;
    const char* type = compatible ? "compatible" : "incompatible";
    uint32_t flags;

    flags = le32toh(compatible ? f->header->compatible_flags
                               : f->header->incompatible_flags);
    ...
    if (flags & ~supported)
    {
        ...
        if (flags)
        {
            ...
            log_debug(
                "Journal file %s uses %s %s %s disabled at compilation time.",
...
static int journal_file_verify_header(JournalFile* f)
{
    ...
    if (UINT64_MAX - header_size < arena_size ||
        header_size + arena_size > (uint64_t)f->last_stat.st_size)
        return -ENODATA;
    ...
    if (f->writable)
    {
        ...
        /* Don't permit appending to files from the future. Because otherwise
         * the realtime timestamps wouldn't be strictly ordered in the entries
         * in the file anymore, and we can't have that since it breaks
         * bisection. */
        if (le64toh(f->header->tail_entry_realtime) > now(CLOCK_REALTIME))
            return log_debug_errno(
                SYNTHETIC_ERRNO(ETXTBSY),
                "Journal file %s is from the future, refusing to append new "
                "data to it that'd be older.",
                f->path);
    }
...
static int journal_file_allocate(JournalFile* f, uint64_t offset, uint64_t size)
{
    ...
    old_size = le64toh(f->header->header_size) + le64toh(f->header->arena_size);
    ...
    if (new_size > f->metrics.min_size && f->metrics.keep_free > 0)
    {
        ...
        if (fstatvfs(f->fd, &svfs) >= 0)
        {
            ...
            available = LESS_BY((uint64_t)svfs.f_bfree * (uint64_t)svfs.f_bsize,
                                f->metrics.keep_free);
...
static int journal_file_move_to(JournalFile* f, ObjectType type,
                                bool keep_always, uint64_t offset,
                                uint64_t size, void** ret, size_t* ret_size)
{
    int r;
    ...
    return mmap_cache_get(f->mmap, f->cache_fd, f->prot, type_to_context(type),
...
static int journal_file_check_object(JournalFile* f, uint64_t offset, Object* o)
{
    ...
    switch (o->object.type)
    {
        ...
        case OBJECT_DATA:
        {
            ...
            if (!VALID64(le64toh(o->data.next_hash_offset)) ||
                ...
                return log_debug_errno(
                    ...
                    ", next_field_offset=" OFSfmt ", entry_offset=" OFSfmt
                    ", entry_array_offset=" OFSfmt ": %" PRIu64,
                    le64toh(o->data.next_hash_offset),
                    le64toh(o->data.next_field_offset),
                    le64toh(o->data.entry_offset),
                    le64toh(o->data.entry_array_offset), offset);
        ...
        case OBJECT_ENTRY:
            ...
            if ((le64toh(o->object.size) - offsetof(EntryObject, items)) /
                ...
                return log_debug_errno(
                    ...
                    (le64toh(o->object.size) - offsetof(EntryObject, items)) /
                        sizeof(EntryItem),
                    offset);
            ...
            if (le64toh(o->entry.seqnum) <= 0)
                return log_debug_errno(SYNTHETIC_ERRNO(EBADMSG),
                                       ...
                                       ": %" PRIu64,
                                       le64toh(o->entry.seqnum), offset);

            if (!VALID_REALTIME(le64toh(o->entry.realtime)))
                return log_debug_errno(
                    SYNTHETIC_ERRNO(EBADMSG),
                    "Invalid entry realtime timestamp: %" PRIu64 ": %" PRIu64,
                    le64toh(o->entry.realtime), offset);

            if (!VALID_MONOTONIC(le64toh(o->entry.monotonic)))
                return log_debug_errno(
                    SYNTHETIC_ERRNO(EBADMSG),
                    "Invalid entry monotonic timestamp: %" PRIu64 ": %" PRIu64,
                    le64toh(o->entry.monotonic), offset);

            break;

        case OBJECT_DATA_HASH_TABLE:
        case OBJECT_FIELD_HASH_TABLE:
            if ((le64toh(o->object.size) - offsetof(HashTableObject, items)) %
                        sizeof(HashItem) !=
                    0 ||
        ...
        case OBJECT_ENTRY_ARRAY:
            if ((le64toh(o->object.size) - offsetof(EntryArrayObject, items)) %
                        sizeof(le64_t) !=
                    0 ||
                (le64toh(o->object.size) - offsetof(EntryArrayObject, items)) /
                        sizeof(le64_t) <=
                    0)
                return log_debug_errno(
                    SYNTHETIC_ERRNO(EBADMSG),
            ...
            if (!VALID64(le64toh(o->entry_array.next_entry_array_offset)))
                return log_debug_errno(
                    SYNTHETIC_ERRNO(EBADMSG),
                    "Invalid object entry array "
                    "next_entry_array_offset: " OFSfmt ": %" PRIu64,
                    le64toh(o->entry_array.next_entry_array_offset), offset);
...
int journal_file_move_to_object(JournalFile* f, ObjectType type,
                                uint64_t offset, Object** ret)
{
    int r;
    void* t;
    size_t tsize;
    ...
    if (s == 0)
        return log_debug_errno(
            SYNTHETIC_ERRNO(EBADMSG),
            "Attempt to move to uninitialized object: %" PRIu64, offset);
    if (s < sizeof(ObjectHeader))
        return log_debug_errno(
            SYNTHETIC_ERRNO(EBADMSG),
            "Attempt to move to overly short object: %" PRIu64, offset);

    if (o->object.type <= OBJECT_UNUSED)
        return log_debug_errno(
            SYNTHETIC_ERRNO(EBADMSG),
            "Attempt to move to object with invalid type: %" PRIu64, offset);
    ...
    if (type > OBJECT_UNUSED && o->object.type != type)
        return log_debug_errno(
            SYNTHETIC_ERRNO(EBADMSG),
            "Attempt to move to object of unexpected type: %" PRIu64, offset);
...
int journal_file_append_object(JournalFile* f, ObjectType type, uint64_t size,
...
static int journal_file_setup_data_hash_table(JournalFile* f)
{
    ...
    log_debug("Reserving %" PRIu64 " entries in hash table.",
              s / sizeof(HashItem));

    r = journal_file_append_object(f, OBJECT_DATA_HASH_TABLE,
                                   offsetof(Object, hash_table.items) + s, &o,
                                   &p);
    if (r < 0)
        return r;
    ...
    f->header->data_hash_table_offset =
        htole64(p + offsetof(Object, hash_table.items));
...
static int journal_file_setup_field_hash_table(JournalFile* f)
{
    ...
    r = journal_file_append_object(f, OBJECT_FIELD_HASH_TABLE,
                                   offsetof(Object, hash_table.items) + s, &o,
                                   &p);
    if (r < 0)
        return r;
    ...
    f->header->field_hash_table_offset =
        htole64(p + offsetof(Object, hash_table.items));
...
int journal_file_map_data_hash_table(JournalFile* f)
{
    ...
    r = journal_file_move_to(f, OBJECT_DATA_HASH_TABLE, true, p, s, &t, NULL);
    if (r < 0)
        return r;
...
int journal_file_map_field_hash_table(JournalFile* f)
{
    ...
    r = journal_file_move_to(f, OBJECT_FIELD_HASH_TABLE, true, p, s, &t, NULL);
    if (r < 0)
        return r;
...
static int journal_file_link_field(JournalFile* f, Object* o, uint64_t offset,
                                   uint64_t hash)
{
...
static int journal_file_link_data(JournalFile* f, Object* o, uint64_t offset,
                                  uint64_t hash)
{
...
int journal_file_find_field_object_with_hash(JournalFile* f, const void* field,
...
int journal_file_find_field_object(JournalFile* f, const void* field,
                                   uint64_t size, Object** ret,
                                   uint64_t* offset)
{
    ...
    return journal_file_find_field_object_with_hash(f, field, size, hash, ret,
                                                    offset);
}

int journal_file_find_data_object_with_hash(JournalFile* f, const void* data,
                                            uint64_t size, uint64_t hash,
                                            Object** ret, uint64_t* offset)
{
    ...
    while (p > 0)
    {
        ...
        if (o->object.flags & OBJECT_COMPRESSION_MASK)
        {
            ...
            r = decompress_blob(o->object.flags & OBJECT_COMPRESSION_MASK,
                                ...
                                &f->compress_buffer_size, &rsize, 0);
            if (r < 0)
                return r;

            if (rsize == size && memcmp(f->compress_buffer, data, size) == 0)
            {
...
int journal_file_find_data_object(JournalFile* f, const void* data,
                                  uint64_t size, Object** ret, uint64_t* offset)
{
    ...
    return journal_file_find_data_object_with_hash(f, data, size, hash, ret,
                                                   offset);
}

static int journal_file_append_field(JournalFile* f, const void* field,
                                     uint64_t size, Object** ret,
                                     uint64_t* offset)
{
...
static int journal_file_append_data(JournalFile* f, const void* data,
                                    uint64_t size, Object** ret,
                                    uint64_t* offset)
{
    ...
    if (JOURNAL_FILE_COMPRESS(f) && size >= f->compress_threshold_bytes)
    {
        ...
        if (compression >= 0)
        {
            ...
            /* Compression didn't work, we don't really care why, let's continue
             * without compression */
            compression = 0;
    }
#endif
    ...
    if (eq && eq > data)
    {
        ...
        /* Create field object ... */
        r = journal_file_append_field(f, data, (uint8_t*)eq - (uint8_t*)data,
                                      &fo, &fp);
        if (r < 0)
            return r;
...
uint64_t journal_file_entry_n_items(Object* o)
{
    ...
    return (le64toh(o->object.size) - offsetof(Object, entry.items)) /
           sizeof(EntryItem);
}
...
uint64_t journal_file_entry_array_n_items(Object* o)
{
    ...
    return (le64toh(o->object.size) - offsetof(Object, entry_array.items)) /
           sizeof(uint64_t);
}
...
uint64_t journal_file_hash_table_n_items(Object* o)
{
    ...
    if (!IN_SET(o->object.type, OBJECT_DATA_HASH_TABLE,
                OBJECT_FIELD_HASH_TABLE))
        return 0;

    return (le64toh(o->object.size) - offsetof(Object, hash_table.items)) /
           sizeof(HashItem);
}

static int link_entry_into_array(JournalFile* f, le64_t* first, le64_t* idx,
                                 uint64_t p)
{
    int r;
    ...
    r = journal_file_append_object(
        f, OBJECT_ENTRY_ARRAY,
        offsetof(Object, entry_array.items) + n * sizeof(uint64_t), &o, &q);
    if (r < 0)
        return r;
    ...
    if (JOURNAL_HEADER_CONTAINS(f->header, n_entry_arrays))
        f->header->n_entry_arrays =
            htole64(le64toh(f->header->n_entry_arrays) + 1);
...
static int link_entry_into_array_plus_one(JournalFile* f, le64_t* extra,
                                          le64_t* first, le64_t* idx,
                                          uint64_t p)
{
...
static int journal_file_link_entry_item(JournalFile* f, Object* o,
                                        uint64_t offset, uint64_t i)
{
    uint64_t p;
    int r;
    assert(f);
    ...
    return link_entry_into_array_plus_one(f, &o->data.entry_offset,
...
static int journal_file_link_entry(JournalFile* f, Object* o, uint64_t offset)
{
    ...
    r = link_entry_into_array(f, &f->header->entry_array_offset,
                              &f->header->n_entries, offset);
    if (r < 0)
        return r;

    /* log_debug("=> %s seqnr=%"PRIu64" n_entries=%"PRIu64, f->path,
     * o->entry.seqnum, f->header->n_entries); */
...
static int journal_file_append_entry_internal(
    ...
    uint64_t xor_hash, const EntryItem items[], unsigned n_items,
...
static int post_change_thunk(sd_event_source* timer, uint64_t usec,
                             void* userdata)
{
    assert(userdata);
...
static void schedule_post_change(JournalFile* f)
{
    ...
    r = sd_event_now(sd_event_source_get_event(f->post_change_timer),
                     CLOCK_MONOTONIC, &now);
    if (r < 0)
    {
        log_debug_errno(
            r, "Failed to get clock's now for scheduling ftruncate: %m");
        goto fail;
    }

    r = sd_event_source_set_time(f->post_change_timer,
                                 now + f->post_change_timer_period);
    if (r < 0)
    {
...
int journal_file_enable_post_change_timer(JournalFile* f, sd_event* e, usec_t t)
{
    ...
    r = sd_event_add_time(e, &timer, CLOCK_MONOTONIC, 0, 0, post_change_thunk,
                          f);
    if (r < 0)
        return r;
                              ...
                              uint64_t* seqnum, Object** ret, uint64_t* offset)
{
    ...
    if (ts)
    {
        if (!VALID_REALTIME(ts->realtime))
            return log_debug_errno(SYNTHETIC_ERRNO(EBADMSG),
                                   "Invalid realtime timestamp %" PRIu64
                                   ", refusing entry.",
                                   ts->realtime);
        if (!VALID_MONOTONIC(ts->monotonic))
            return log_debug_errno(SYNTHETIC_ERRNO(EBADMSG),
                                   "Invalid monotomic timestamp %" PRIu64
                                   ", refusing entry.",
                                   ts->monotonic);
    }
    else
    ...
    for (i = 0; i < n_iovec; i++)
    {
        ...
        r = journal_file_append_data(f, iovec[i].iov_base, iovec[i].iov_len, &o,
                                     &p);
        if (r < 0)
            return r;
    ...
    r = journal_file_append_entry_internal(f, ts, boot_id, xor_hash, items,
                                           n_iovec, seqnum, ret, offset);
...
typedef struct ChainCacheItem
{
    ...
    uint64_t total; /* the total number of items in all arrays before this one
                       in the chain */
    uint64_t last_index; /* the last index we looked at, to optimize locality
                            when bisecting */
} ChainCacheItem;

static void chain_cache_put(OrderedHashmap* h, ChainCacheItem* ci,
                            uint64_t first, uint64_t array, uint64_t begin,
                            uint64_t total, uint64_t last_index)
{
...
static int generic_array_get(JournalFile* f, uint64_t first, uint64_t i,
                             Object** ret, uint64_t* offset)
{
...
static int generic_array_get_plus_one(JournalFile* f, uint64_t extra,
                                      uint64_t first, uint64_t i, Object** ret,
                                      uint64_t* offset)
{
...
static int generic_array_bisect(
    JournalFile* f, uint64_t first, uint64_t n, uint64_t needle,
    int (*test_object)(JournalFile* f, uint64_t p, uint64_t needle),
    direction_t direction, Object** ret, uint64_t* offset, uint64_t* idx)
{
    ...
    while (a > 0)
    {
        ...
        if (r == -EBADMSG)
        {
            log_debug_errno(r, "Encountered invalid entry while bisecting, "
                               "cutting algorithm short. (1)");
            n = i;
            continue;
        }
        if (r < 0)
        ...
        if (r == TEST_RIGHT)
        {
            ...
            if (last_index != (uint64_t)-1)
            {
                ...
                if (last_index > 0)
                {
                    ...
                    if (r == TEST_FOUND)
                        r = direction == DIRECTION_DOWN ? TEST_RIGHT
                                                        : TEST_LEFT;
            ...
            for (;;)
            {
                ...
                {
                    log_debug_errno(r,
                                    "Encountered invalid entry while "
                                    "bisecting, cutting algorithm short. (2)");
                    right = n = i;
                    continue;
                }
                if (r < 0)
...
found:
    ...
    chain_cache_put(f->chain_cache, ci, first, a,
                    ...
                    subtract_one ? (i > 0 ? i - 1 : (uint64_t)-1) : i);
...
static int generic_array_bisect_plus_one(
    JournalFile* f, uint64_t extra, uint64_t first, uint64_t n, uint64_t needle,
    int (*test_object)(JournalFile* f, uint64_t p, uint64_t needle),
    direction_t direction, Object** ret, uint64_t* offset, uint64_t* idx)
{
    ...
    r = generic_array_bisect(f, first, n - 1, needle, test_object, direction,
                             ret, offset, idx);
...
_pure_ static int test_object_offset(JournalFile* f, uint64_t p,
                                     uint64_t needle)
{
    assert(f);
    assert(p > 0);
...
int journal_file_move_to_entry_by_seqnum(JournalFile* f, uint64_t seqnum,
                                         direction_t direction, Object** ret,
                                         uint64_t* offset)
{
    assert(f);
    assert(f->header);

    return generic_array_bisect(f, le64toh(f->header->entry_array_offset),
                                le64toh(f->header->n_entries), seqnum,
                                test_object_seqnum, direction, ret, offset,
                                NULL);
}
...
int journal_file_move_to_entry_by_realtime(JournalFile* f, uint64_t realtime,
                                           direction_t direction, Object** ret,
                                           uint64_t* offset)
{
    assert(f);
    ...
    return generic_array_bisect(f, le64toh(f->header->entry_array_offset),
                                le64toh(f->header->n_entries), realtime,
                                test_object_realtime, direction, ret, offset,
                                NULL);
}
...
static int find_data_object_by_boot_id(JournalFile* f, sd_id128_t boot_id,
                                       Object** o, uint64_t* b)
{
...
int journal_file_move_to_entry_by_monotonic(JournalFile* f, sd_id128_t boot_id,
                                            uint64_t monotonic,
                                            direction_t direction, Object** ret,
                                            uint64_t* offset)
{
    ...
    return generic_array_bisect_plus_one(
        ...
        le64toh(o->data.n_entries), monotonic, test_object_monotonic, direction,
        ret, offset, NULL);
}
...
static bool check_properly_ordered(uint64_t new_offset, uint64_t old_offset,
                                   direction_t direction)
{
    ...
    return direction == DIRECTION_DOWN ? new_offset > old_offset
                                       : new_offset < old_offset;
}

int journal_file_next_entry(JournalFile* f, uint64_t p, direction_t direction,
                            Object** ret, uint64_t* offset)
{
    ...
    {
        r = generic_array_bisect(f, le64toh(f->header->entry_array_offset),
                                 le64toh(f->header->n_entries), p,
                                 test_object_offset, DIRECTION_DOWN, NULL, NULL,
                                 &i);
        if (r <= 0)
            return r;
    ...
    for (;;)
    {
        r = generic_array_get(f, le64toh(f->header->entry_array_offset), i, ret,
                              &ofs);
        if (r > 0)
            break;
        if (r != -EBADMSG)
        ...
        /* OK, so this entry is borked. Most likely some entry didn't get synced
         * to disk properly, let's see if the next one might work for us
         * instead. */
        log_debug_errno(r, "Entry item %" PRIu64 " is bad, skipping over it.",
                        i);
    ...
    if (p > 0 && !check_properly_ordered(ofs, p, direction))
        return log_debug_errno(
            SYNTHETIC_ERRNO(EBADMSG),
            "%s: entry array not properly ordered at entry %" PRIu64, f->path,
            i);
                                     ...
                                     uint64_t* offset)
{
    ...
    else
    {
        ...
        r = generic_array_bisect_plus_one(
            f, le64toh(d->data.entry_offset),
            le64toh(d->data.entry_array_offset), le64toh(d->data.n_entries), p,
            test_object_offset, DIRECTION_DOWN, NULL, NULL, &i);
    ...
    for (;;)
    {
        r = generic_array_get_plus_one(f, le64toh(d->data.entry_offset),
                                       le64toh(d->data.entry_array_offset), i,
                                       ret, &ofs);
        if (r > 0)
            break;
        if (r != -EBADMSG)
        ...
        log_debug_errno(
            r, "Data entry item %" PRIu64 " is bad, skipping over it.", i);
...
int journal_file_move_to_entry_by_offset_for_data(
    JournalFile* f, uint64_t data_offset, uint64_t p, direction_t direction,
    Object** ret, uint64_t* offset)
{
    ...
    return generic_array_bisect_plus_one(
        ...
        le64toh(d->data.n_entries), p, test_object_offset, direction, ret,
        offset, NULL);
}

int journal_file_move_to_entry_by_monotonic_for_data(
    JournalFile* f, uint64_t data_offset, sd_id128_t boot_id,
    uint64_t monotonic, direction_t direction, Object** ret, uint64_t* offset)
{
    ...
    for (;;)
    {
        ...
        r = generic_array_bisect_plus_one(
            f, le64toh(d->data.entry_offset),
            le64toh(d->data.entry_array_offset), le64toh(d->data.n_entries), z,
            test_object_offset, direction, NULL, &p, NULL);
        if (r <= 0)
            return r;
        ...
        r = generic_array_bisect_plus_one(
            f, le64toh(o->data.entry_offset),
            le64toh(o->data.entry_array_offset), le64toh(o->data.n_entries), p,
            test_object_offset, direction, &qo, &q, NULL);
...
int journal_file_move_to_entry_by_seqnum_for_data(
    JournalFile* f, uint64_t data_offset, uint64_t seqnum,
    direction_t direction, Object** ret, uint64_t* offset)
{
    ...
    return generic_array_bisect_plus_one(
        f, le64toh(d->data.entry_offset), le64toh(d->data.entry_array_offset),
        le64toh(d->data.n_entries), seqnum, test_object_seqnum, direction, ret,
        offset, NULL);
}

int journal_file_move_to_entry_by_realtime_for_data(
    JournalFile* f, uint64_t data_offset, uint64_t realtime,
    direction_t direction, Object** ret, uint64_t* offset)
{
    ...
    return generic_array_bisect_plus_one(
        f, le64toh(d->data.entry_offset), le64toh(d->data.entry_array_offset),
        le64toh(d->data.n_entries), realtime, test_object_realtime, direction,
        ret, offset, NULL);
}
...
void journal_file_dump(JournalFile* f)
{
    ...
    while (p != 0)
    {
        ...
        switch (o->object.type)
        {
            ...
            case OBJECT_ENTRY:
                printf("Type: OBJECT_ENTRY seqnum=%" PRIu64
                       " monotonic=%" PRIu64 " realtime=%" PRIu64 "\n",
                       le64toh(o->entry.seqnum), le64toh(o->entry.monotonic),
...
void journal_file_print_header(JournalFile* f)
{
    ...
    printf(
        ...
        f->path, sd_id128_to_string(f->header->file_id, a),
        ...
        (le32toh(f->header->incompatible_flags) & ~HEADER_INCOMPATIBLE_ANY)
            ? " ???"
            : "",
        le64toh(f->header->header_size), le64toh(f->header->arena_size),
        ...
        le64toh(f->header->tail_entry_seqnum),
        format_timestamp_safe(x, sizeof(x),
                              le64toh(f->header->head_entry_realtime)),
        le64toh(f->header->head_entry_realtime),
        format_timestamp_safe(y, sizeof(y),
                              le64toh(f->header->tail_entry_realtime)),
        le64toh(f->header->tail_entry_realtime),
        format_timespan(z, sizeof(z), le64toh(f->header->tail_entry_monotonic),
                        USEC_PER_MSEC),
        le64toh(f->header->tail_entry_monotonic), le64toh(f->header->n_objects),
    ...
    if (JOURNAL_HEADER_CONTAINS(f->header, n_data))
        printf("Data Objects: %" PRIu64 "\n"
               ...
               100.0 * (double)le64toh(f->header->n_data) /
    ...
    if (JOURNAL_HEADER_CONTAINS(f->header, n_fields))
        printf("Field Objects: %" PRIu64 "\n"
               ...
               100.0 * (double)le64toh(f->header->n_fields) /
                   ((double)(le64toh(f->header->field_hash_table_size) /
                             sizeof(HashItem))));
...
static int journal_file_warn_btrfs(JournalFile* f)
{
    ...
    if (r < 0)
        return log_warning_errno(
            r, "Failed to determine if journal is on btrfs: %m");
    if (!r)
        return 0;
    ...
    {
        log_debug("Detected btrfs file system with copy-on-write disabled, all "
                  "is good.");
        return 0;
    }

    log_notice("Creating journal file %s on a btrfs file system, and "
               "copy-on-write is enabled. "
               "This is likely to slow down journal access substantially, "
               "please consider turning "
               "off the copy-on-write file attribute on the journal directory, "
               "using chattr +C.",
               f->path);
...
int journal_file_open(int fd, const char* fname, int flags, mode_t mode,
                      bool compress, uint64_t compress_threshold_bytes,
                      bool seal, JournalMetrics* metrics, MMapCache* mmap_cache,
                      Set* deferred_closes, JournalFile* template,
                      JournalFile** ret)
{
    ...
    *f = (JournalFile)
    {
        ...
        .flags = flags, .prot = prot_from_flags(flags),
        ...
        .compress_threshold_bytes =
            compress_threshold_bytes == (uint64_t)-1
    ...
    log_debug("Journal effective settings seal=%s compress=%s "
              "compress_threshold_bytes=%s",
    ...
    else
    {
        ...
        /* If we don't know the path, fill in something explanatory and vaguely
         * useful */
    ...
    if (f->fd < 0)
    {
        /* We pass O_NONBLOCK here, so that in case somebody pointed us to some
         * character device node or FIFO or so, we likely fail quickly than
         * block for long. For regular files O_NONBLOCK has no effect, hence
    ...
    if (f->last_stat.st_size == 0 && f->writable)
    {
        ...
        /* Let's attach the creation time to the journal file, so that the
         * vacuuming code knows the age of this file even if the file might end
         * up corrupted one day... Ideally we'd just use the creation time many
         * file systems maintain for each file, but the API to query this is
         * very new, hence let's emulate this via extended attributes. If
         * extended attributes are not supported we'll just skip this, and rely
    ...
    r = mmap_cache_get(f->mmap, f->cache_fd, f->prot, CONTEXT_HEADER, true, 0,
                       PAGE_ALIGN(sizeof(Header)), &f->last_stat, &h, NULL);
    if (r < 0)
        goto fail;
    ...
    if (template && template->post_change_timer)
    {
        r = journal_file_enable_post_change_timer(
            f, sd_event_source_get_event(template->post_change_timer),
    ...
    /* The file is opened now successfully, thus we take possession of any
     * passed in fd. */
    f->close_fd = true;
...
int journal_file_archive(JournalFile* f)
{
    ...
    /* Is this a journal file that was passed to us as fd? If so, we synthesized
     * a path name for it, and we refuse rotation, since we don't know the
     * actual path, and couldn't rename the file hence. */
     ...
     * deleted, we'll get ENOENT, let's ignore that case. */
    ...
    /* Set as archive so offlining commits w/state=STATE_ARCHIVED. Previously we
     * would set old_file->header->state to STATE_ARCHIVED directly here, but
     * journal_file_set_offline() short-circuits when state != STATE_ONLINE,
     * which would result in the rotated journal never getting fsync() called
     * before closing.  Now we simply queue the archive state by setting an
     * archive bit, leaving the state as STATE_ONLINE so proper offlining
     * occurs. */
    f->archive = true;

    /* Currently, btrfs is not very good with out write patterns and fragments
     * heavily. Let's defrag our journal files when we archive them */
...
JournalFile* journal_initiate_close(JournalFile* f, Set* deferred_closes)
{
    ...
    if (deferred_closes)
    {
        ...
        if (r < 0)
            log_debug_errno(r, "Failed to add file to deferred close set, "
                               "closing immediately.");
        else
        {
...
int journal_file_rotate(JournalFile** f, bool compress,
                        uint64_t compress_threshold_bytes, bool seal,
                        Set* deferred_closes)
{
    ...
    r = journal_file_open(-1, (*f)->path, (*f)->flags, (*f)->mode, compress,
                          compress_threshold_bytes, seal, NULL, /* metrics */
                          (*f)->mmap, deferred_closes, *f,      /* template */
                          &new_file);
...
int journal_file_dispose(int dir_fd, const char* fname)
{
    ...
    /* Renames a journal file to *.journal~, i.e. to mark it as corruped or
     * otherwise uncleanly shutdown. Note that this is done without looking into
     * the file or changing any of its contents. The idea is that this is called
     * whenever something is suspicious and we want to move the file away and
     * make clear that it is not accessed for writing anymore. */

    if (!endswith(fname, ".journal"))
        return -EINVAL;

    if (asprintf(&p, "%.*s@%016" PRIx64 "-%016" PRIx64 ".journal~",
                 (int)strlen(fname) - 8, fname, now(CLOCK_REALTIME),
                 random_u64()) < 0)
        return -ENOMEM;
    ...
    /* btrfs doesn't cope well with our write pattern and fragments heavily.
     * Let's defrag all files we rotate */
    fd = openat(dir_fd, p, O_RDONLY | O_CLOEXEC | O_NOCTTY | O_NOFOLLOW);
    if (fd < 0)
        log_debug_errno(errno, "Failed to open file for "
                               "defragmentation/FS_NOCOW_FL, ignoring: %m");
    else
    {
...
int journal_file_open_reliably(const char* fname, int flags, mode_t mode,
                               bool compress, uint64_t compress_threshold_bytes,
                               bool seal, JournalMetrics* metrics,
                               MMapCache* mmap_cache, Set* deferred_closes,
                               JournalFile* template, JournalFile** ret)
{

    int r;

    r = journal_file_open(-1, fname, flags, mode, compress,
                          compress_threshold_bytes, seal, metrics, mmap_cache,
                          deferred_closes, template, ret);
    if (!IN_SET(r, -EBADMSG,      /* Corrupted */
                -ENODATA,         /* Truncated */
    ...
    log_warning_errno(
        r, "File %s corrupted or uncleanly shut down, renaming and replacing.",
        fname);
    ...
    return journal_file_open(-1, fname, flags, mode, compress,
                             compress_threshold_bytes, seal, metrics,
...
void journal_default_metrics(JournalMetrics* m, int fd)
{
    char a[FORMAT_BYTES_MAX], b[FORMAT_BYTES_MAX], c[FORMAT_BYTES_MAX],
        d[FORMAT_BYTES_MAX], e[FORMAT_BYTES_MAX];
    struct statvfs ss;
    uint64_t fs_size;
    ...
    if (m->keep_free == (uint64_t)-1)
    {
        ...
        {
            m->keep_free =
                PAGE_ALIGN(fs_size * 3 / 20); /* 15% of file system size */
    ...
    log_debug("Fixed min_use=%s max_use=%s max_size=%s min_size=%s "
              "keep_free=%s n_max_files=%" PRIu64,
              ...
              format_bytes(e, sizeof(e), m->keep_free), m->n_max_files);
}

int journal_file_get_cutoff_realtime_usec(JournalFile* f, usec_t* from,
                                          usec_t* to)
{
    assert(f);
    assert(f->header);
...
int journal_file_get_cutoff_monotonic_usec(JournalFile* f, sd_id128_t boot_id,
                                           usec_t* from, usec_t* to)
{
    ...
    if (to)
    {
        ...
        r = generic_array_get_plus_one(f, le64toh(o->data.entry_offset),
                                       le64toh(o->data.entry_array_offset),
                                       le64toh(o->data.n_entries) - 1, &o,
                                       NULL);
        if (r <= 0)
            return r;
...
bool journal_file_rotate_suggested(JournalFile* f, usec_t max_file_usec)
{
    ...
    if (JOURNAL_HEADER_CONTAINS(f->header, n_data))
        if (le64toh(f->header->n_data) * 4ULL >
            (le64toh(f->header->data_hash_table_size) / sizeof(HashItem)) *
                3ULL)
        {
            log_debug("Data hash table of %s has a fill level at %.1f (%" PRIu64
                      " of %" PRIu64 " items, %llu file size, %" PRIu64
    ...
    if (JOURNAL_HEADER_CONTAINS(f->header, n_fields))
        if (le64toh(f->header->n_fields) * 4ULL >
            (le64toh(f->header->field_hash_table_size) / sizeof(HashItem)) *
                3ULL)
        {
            log_debug(
                "Field hash table of %s has a fill level at %.1f (%" PRIu64
                " of %" PRIu64 " items), suggesting rotation.",
                f->path,
                100.0 * (double)le64toh(f->header->n_fields) /
                    ((double)(le64toh(f->header->field_hash_table_size) /
                              sizeof(HashItem))),
    ...
    if (JOURNAL_HEADER_CONTAINS(f->header, n_data) &&
        ...
        le64toh(f->header->n_data) > 0 && le64toh(f->header->n_fields) == 0)
        return true;
