...
class basic_oarchive_impl
{
    ...
    struct aobject
    {
        ...
        aobject(const void* a, class_id_type class_id_,
                object_id_type object_id_) :
            address(a),
            class_id(class_id_), object_id(object_id_)
        {
        }
    ...
    struct cobject_type
    {
        ...
        cobject_type(const basic_oserializer& bos) :
            m_bos_ptr(&bos), m_initialized(false)
        {
        }
        cobject_type(const cobject_type& rhs) :
            m_bos_ptr(rhs.m_bos_ptr), m_class_id(rhs.m_class_id),
...
inline void
    basic_oarchive_impl::save_pointer(basic_oarchive& ar, const void* t,
...
BOOST_ARCHIVE_DECL void
    basic_oarchive::save_object(const void* x, const basic_oserializer& bos)
{
    pimpl->save_object(*this, x, bos);
}

BOOST_ARCHIVE_DECL void
    basic_oarchive::save_pointer(const void* t,
                                 const basic_pointer_oserializer* bpos_ptr)
{
