                         ...
                         uint32_t table_sz)
{
    ...
    if (addr)
        memcpy(nkey->n_addr, nl_addr_get_binary_addr(addr),
    ...
    *hashkey = nl_hash(nkey, nkey_sz, 0) % table_sz;
    ...
    NL_DBG(5, "neigh %p key (fam %d dev %d addr %s) keysz %d hash 0x%x\n",
           ...
           nl_addr2str(addr, buf, sizeof(buf)), nkey_sz, *hashkey);
...
#define NEIGH_DIFF(ATTR, EXPR) ATTR_DIFF(attrs, NEIGH_ATTR_##ATTR, a, b, EXPR)
    ...
    {
        diff |= NEIGH_DIFF(STATE, (a->n_state ^ b->n_state) & b->n_state_mask);
        diff |= NEIGH_DIFF(FLAGS, (a->n_flags ^ b->n_flags) & b->n_flag_mask);
    }
    else
    {
...
static const struct trans_tbl neigh_attrs[] = {
    __ADD(NEIGH_ATTR_FLAGS, flags),         __ADD(NEIGH_ATTR_STATE, state),
    __ADD(NEIGH_ATTR_LLADDR, lladdr),       __ADD(NEIGH_ATTR_DST, dst),
    __ADD(NEIGH_ATTR_CACHEINFO, cacheinfo), __ADD(NEIGH_ATTR_IFINDEX, ifindex),
    __ADD(NEIGH_ATTR_FAMILY, family),       __ADD(NEIGH_ATTR_TYPE, type),
    __ADD(NEIGH_ATTR_PROBES, probes),       __ADD(NEIGH_ATTR_MASTER, master),
    __ADD(NEIGH_ATTR_VLAN, vlan),
};

static char* neigh_attrs2str(int attrs, char* buf, size_t len)
{
    return __flags2str(attrs, buf, len, neigh_attrs, ARRAY_SIZE(neigh_attrs));
}
...
int rtnl_neigh_parse(struct nlmsghdr* n, struct rtnl_neigh** result)
{
    ...
    neigh->ce_mask |= (NEIGH_ATTR_FAMILY | NEIGH_ATTR_IFINDEX |
                       NEIGH_ATTR_STATE | NEIGH_ATTR_FLAGS | NEIGH_ATTR_TYPE);
        ...
        else
        {
            ...
            if (lcache)
            {
                struct rtnl_link* link =
                    rtnl_link_get(lcache, neigh->n_ifindex);
                if (link)
                {
...
static void neigh_dump_line(struct nl_object* a, struct nl_dump_params* p)
{
    ...
    if (link_cache)
        nl_dump(
            p, "dev %s ",
            rtnl_link_i2name(link_cache, n->n_ifindex, state, sizeof(state)));
    else
...
/**
 * Build a neighbour cache including all neighbours currently configured in the
 * kernel.
                                  ...
                                  struct nl_addr* dst)
{
    ...
    {
        if (neigh->n_ifindex == ifindex && !nl_addr_cmp(neigh->n_dst, dst))
        {
                                          ...
                                          struct nl_addr* lladdr, int vlan)
{
    ...
    nl_list_for_each_entry(neigh, &cache->c_items, ce_list)
    {
        if (neigh->n_ifindex == ifindex && neigh->n_vlan == vlan &&
...
{
    return __flags2str(flags, buf, len, neigh_flags, ARRAY_SIZE(neigh_flags));
}
...
{
    return __assign_addr(neigh, &neigh->n_dst, addr, NEIGH_ATTR_DST, 0);
}
