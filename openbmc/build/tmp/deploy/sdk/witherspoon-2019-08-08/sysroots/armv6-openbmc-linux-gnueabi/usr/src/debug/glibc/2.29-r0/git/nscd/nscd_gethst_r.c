                           ...
                           int* h_errnop)
{
    ...
    return nscd_gethst_r(name, strlen(name) + 1, reqtype, resultbuf, buffer,
...
uint32_t __nscd_get_nl_timestamp(void)
{
    ...
    if (map == NULL ||
        (map != NO_MAPPING && map->head->nscd_certainly_running == 0 &&
                         ...
                         struct hostent** result, int* h_errnop)
{
    ...
    mapped =
        __nscd_get_map_ref(GETFDHST, "hosts", &__hst_map_handle, &gc_cycle);
...
retry:;
    ...
    if (mapped != NO_MAPPING)
    {
        ...
        struct datahead* found =
            __nscd_cache_search(type, key, keylen, mapped, sizeof hst_resp);
        if (found != NULL)
        {
            ...
            if (((uintptr_t)aliases_len & (__alignof__(*aliases_len) - 1)) != 0)
            {
                uint32_t* tmp =
                    alloca(hst_resp.h_aliases_cnt * sizeof(uint32_t));
            ...
            if (__builtin_expect(
                    (const char*)addr_list + addr_list_len > recend, 0))
                goto out;
        }
    }
    ...
    {
        sock =
            __nscd_open_socket(key, keylen, type, &hst_resp, sizeof(hst_resp));
        if (sock == -1)
        {
            __nss_not_use_nscd_hosts = 1;
            goto out;
    ...
    if (hst_resp.found == 1)
    {
       ...
       align the pointer and the base of the h_addr_list pointers.  */
        align1 = ((__alignof__(char*) - (cp - ((char*)0))) &
                  (__alignof__(char*) - 1));
        align2 = ((__alignof__(char*) -
                   ((cp + align1 + hst_resp.h_name_len) - ((char*)0))) &
                  (__alignof__(char*) - 1));
        if (buflen < (align1 + hst_resp.h_name_len + align2 +
                      ((hst_resp.h_aliases_cnt + hst_resp.h_addr_list_cnt + 2) *
                       sizeof(char*)) +
                      hst_resp.h_addr_list_cnt *
                          (type == AF_INET ? INADDRSZ : IN6ADDRSZ)))
        {
        no_room:
        ...
        if (__builtin_expect(
                (const char*)addr_list + addr_list_len + total_len > recend, 0))
        {
