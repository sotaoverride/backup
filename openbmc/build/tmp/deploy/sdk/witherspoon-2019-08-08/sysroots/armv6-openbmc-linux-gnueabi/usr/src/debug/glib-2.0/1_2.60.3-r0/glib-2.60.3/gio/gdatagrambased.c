...
static void g_datagram_based_default_init(GDatagramBasedInterface* iface)
{
    /* Nothing here. */
}
    ...
    gint flags, gint64 timeout, GCancellable* cancellable, GError** error)
{
    ...
    g_return_val_if_fail(
        timeout > 0 ||
            !g_error_matches(child_error, G_IO_ERROR, G_IO_ERROR_TIMED_OUT),
        -1);
                                         ...
                                         GError** error)
{
    ...
    out = iface->condition_wait(datagram_based, condition, timeout, cancellable,
                                &child_error);
