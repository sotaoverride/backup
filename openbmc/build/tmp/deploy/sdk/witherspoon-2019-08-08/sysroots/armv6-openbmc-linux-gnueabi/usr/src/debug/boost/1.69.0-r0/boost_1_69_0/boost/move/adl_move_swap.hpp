...
// For non-GCC compilers, where GNUC version is not very reliable, or old GCC
// versions use the good old stl_algobase header, which is quite lightweight
...
struct has_member_swap_impl<Fun, true>
{
    ...
    static bool const value =
        sizeof(no_type) ==
        sizeof(is_private_type((declval_wrap().swap(declval_fun()), 0)));
};

template <typename Fun>
struct has_member_swap
    : public has_member_swap_impl<Fun,
                                  has_member_function_named_swap<Fun>::value>
{
};
...
template <class P1, class P2>
struct and_op : and_op_impl<P1, P2>
{
};
...
template <class P1, class P2>
struct and_op_not : and_op_not_impl<P1, P2>
{
};

template <class T>
BOOST_MOVE_FORCEINLINE void swap_proxy(
    T& x, T& y,
    typename boost::move_detail::enable_if_c<
        !boost::move_detail::has_move_emulation_enabled_impl<T>::value>::type* =
        0)
{
    // use std::swap if argument dependent lookup fails
    // Use using directive ("using namespace xxx;") instead as some older
    // compilers don't do ADL with using declarations ("using ns::func;").
    using namespace std;
    swap(x, y);
}
...
BOOST_MOVE_FORCEINLINE void swap_proxy(
    T& x, T& y,
    typename boost::move_detail::enable_if<
        and_op_not_impl<boost::move_detail::has_move_emulation_enabled_impl<T>,
                        boost_move_member_swap::has_member_swap<T>>>::type* = 0)
{
    T t(::boost::move(x));
    x = ::boost::move(y);
    y = ::boost::move(t);
...
BOOST_MOVE_FORCEINLINE void swap_proxy(
    T& x, T& y,
    typename boost::move_detail::enable_if<
        and_op_impl<boost::move_detail::has_move_emulation_enabled_impl<T>,
                    boost_move_member_swap::has_member_swap<T>>>::type* = 0)
{
    x.swap(y);
}
...
//! Exchanges the values of a and b, using Argument Dependent Lookup (ADL) to
//! select a specialized swap function if available. If no specialized swap
//! function is available, std::swap is used.
//!
...
//!      <code>T t(::boost::move(x)); x = ::boost::move(y); y =
//!      ::boost::move(t);</code>.
template <class T>
...
//! Exchanges elements between range [first1, last1) and another range starting
//! at first2 using boost::adl_move_swap.
//!
//! Parameters:
...
//!
//! Return value: Iterator to the element past the last element exchanged in the
