...
static void show_pid_array(pid_t pids[], unsigned n_pids, const char* prefix,
                           unsigned n_columns, bool extra, bool more,
                           OutputFlags flags)
{
...
static int show_cgroup_one_by_path(const char* path, const char* prefix,
                                   unsigned n_columns, bool more,
                                   OutputFlags flags)
{
...
int show_cgroup_by_path(const char* path, const char* prefix,
                        unsigned n_columns, OutputFlags flags)
{
    ...
    while ((r = cg_read_subgroup(d, &gn)) > 0)
    {
        ...
        if (last)
        {
            printf("%s%s%s\n", prefix, special_glyph(SPECIAL_GLYPH_TREE_BRANCH),
    ...
    if (last)
    {
        printf("%s%s%s\n", prefix, special_glyph(SPECIAL_GLYPH_TREE_RIGHT),
               cg_unescape(basename(last)));
...
int show_cgroup(const char* controller, const char* path, const char* prefix,
                unsigned n_columns, OutputFlags flags)
{
    _cleanup_free_ char* p = NULL;
    int r;
...
static int show_extra_pids(const char* controller, const char* path,
                           const char* prefix, unsigned n_columns,
                           const pid_t pids[], unsigned n_pids,
                           OutputFlags flags)
{
...
int show_cgroup_and_extra(const char* controller, const char* path,
                          const char* prefix, unsigned n_columns,
...
int show_cgroup_get_unit_path_and_warn(sd_bus* bus, const char* unit,
                                       char** ret)
{
...
int show_cgroup_get_path_and_warn(const char* machine, const char* prefix,
                                  char** ret)
{
    ...
    if (machine)
    {
        ...
        r = bus_connect_transport_systemd(BUS_TRANSPORT_LOCAL, NULL, false,
                                          &bus);
        if (r < 0)
    ...
    else
    {
        ...
        if (r == -ENOMEDIUM)
            return log_error_errno(
                r, "Failed to get root control group path.\n"
