...
OM_uint32 KRB5_CALLCONV gss_set_sec_context_option(OM_uint32* minor_status,
                                                   ...
                                                   const gss_buffer_t value)
{
    ...
    *minor_status = 0;
    ...
    status = mech->gss_set_sec_context_option(
        minor_status, ctx ? &ctx->internal_ctx_id : &internal_ctx,
    ...
    if (status == GSS_S_COMPLETE)
    {
        if (ctx == NULL && internal_ctx != GSS_C_NO_CONTEXT)
        {
            ...
            if (ctx == NULL)
            {
                *minor_status = ENOMEM;
                gssint_delete_internal_sec_context(
                    &minor, &mech->mech_type, &internal_ctx, GSS_C_NO_BUFFER);
            ...
            {
                gssint_delete_internal_sec_context(
                    &minor, ctx->mech_type, &internal_ctx, GSS_C_NO_BUFFER);
                free(ctx);
                return status;
            }
