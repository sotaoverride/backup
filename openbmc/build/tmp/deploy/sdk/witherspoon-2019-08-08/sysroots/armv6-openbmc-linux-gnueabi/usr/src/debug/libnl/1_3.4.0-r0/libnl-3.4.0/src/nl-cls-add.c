...
static void print_usage(void)
{
    printf(
        ...
        " -d, --dev=DEV             Network device the classifier should be "
        "attached to.\n"
        " -i, --id=ID               ID of new classifier (default: "
        "auto-generated)\n"
        " -p, --parent=ID           ID of parent { root | ingress | class-ID "
        "}\n"
        "     --proto=PROTO         Protocol to match (default: all)\n"
        "     --prio=PRIO           Priority (default: 0)\n"
        "     --mtu=SIZE            Overwrite MTU (default: MTU of network "
        "device)\n"
        "     --mpu=SIZE            Minimum packet size on the link (default: "
        "0).\n"
        "     --overhead=SIZE       Overhead in bytes per packet (default: "
        "0).\n"
        "     --linktype=TYPE       Overwrite linktype (default: type of "
        "network device)\n"
        "\n"
        "CONFIGURATION\n"
        " -h, --help                Show help text of classifier specific "
        "options.\n"
        "\n"
        "EXAMPLE\n"
...
int main(int argc, char* argv[])
{
    ...
    for (;;)
    {
        ...
        c = getopt_long(argc, argv, "+qhvd:p:i:", long_opts, &optidx);
        if (c == -1)
            break;
        ...
        switch (c)
        {
            ...
            case ARG_UPDATE:
                flags = NLM_F_CREATE;
                break;
            case ARG_UPDATE_ONLY:
                flags = 0;
                break;
            case ARG_MTU:
                nl_cli_tc_parse_mtu(tc, optarg);
                break;
            case ARG_MPU:
                nl_cli_tc_parse_mpu(tc, optarg);
                break;
            case ARG_OVERHEAD:
                nl_cli_tc_parse_overhead(tc, optarg);
                break;
            case ARG_LINKTYPE:
                nl_cli_tc_parse_linktype(tc, optarg);
                break;
            case ARG_PROTO:
                nl_cli_cls_parse_proto(cls, optarg);
                break;
            case ARG_PRIO:
