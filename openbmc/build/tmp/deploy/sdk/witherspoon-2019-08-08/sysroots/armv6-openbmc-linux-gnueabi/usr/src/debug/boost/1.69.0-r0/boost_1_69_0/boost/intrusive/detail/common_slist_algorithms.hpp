...
class common_slist_algorithms
{
  public:
    ...
    {
        for (node_ptr p_next; this_node != (p_next = NodeTraits::get_next(p));
             p = p_next)
        {
    ...
    BOOST_INTRUSIVE_FORCEINLINE static void
        incorporate_after(node_ptr bp, node_ptr b, node_ptr be)
    {
    ...
    template <class Pred>
    static void stable_partition(node_ptr before_beg, node_ptr end, Pred pred,
