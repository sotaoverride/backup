...
static const char* const
    bond_xmit_hash_policy_table[_NETDEV_BOND_XMIT_HASH_POLICY_MAX] = {
...
DEFINE_CONFIG_PARSE_ENUM(config_parse_bond_lacp_rate, bond_lacp_rate,
                         BondLacpRate, "Failed to parse bond lacp rate")
...
DEFINE_CONFIG_PARSE_ENUM(config_parse_bond_ad_select, bond_ad_select,
                         BondAdSelect, "Failed to parse bond AD select");

static const char* const
    bond_fail_over_mac_table[_NETDEV_BOND_FAIL_OVER_MAC_MAX] = {
...
DEFINE_CONFIG_PARSE_ENUM(config_parse_bond_fail_over_mac, bond_fail_over_mac,
                         BondFailOverMac, "Failed to parse bond fail over MAC");

static const char* const
    bond_arp_validate_table[_NETDEV_BOND_ARP_VALIDATE_MAX] = {
...
DEFINE_CONFIG_PARSE_ENUM(config_parse_bond_arp_validate, bond_arp_validate,
                         BondArpValidate, "Failed to parse bond arp validate");

static const char* const
    bond_arp_all_targets_table[_NETDEV_BOND_ARP_ALL_TARGETS_MAX] = {
...
DEFINE_CONFIG_PARSE_ENUM(config_parse_bond_arp_all_targets,
                         bond_arp_all_targets, BondArpAllTargets,
                         "Failed to parse bond Arp all targets");

static const char* const
    bond_primary_reselect_table[_NETDEV_BOND_PRIMARY_RESELECT_MAX] = {
...
DEFINE_CONFIG_PARSE_ENUM(config_parse_bond_primary_reselect,
                         bond_primary_reselect, BondPrimaryReselect,
...
static int netdev_bond_fill_message_create(NetDev* netdev, Link* link,
                                           sd_netlink_message* m)
{
    ...
    {
        r = sd_netlink_message_append_u8(m, IFLA_BOND_AD_LACP_RATE,
                                         b->lacp_rate);
        if (r < 0)
            return log_netdev_error_errno(
                netdev, r,
    ...
    if (b->miimon != 0)
    {
        r = sd_netlink_message_append_u32(m, IFLA_BOND_MIIMON,
                                          b->miimon / USEC_PER_MSEC);
        if (r < 0)
    ...
    if (b->downdelay != 0)
    {
        r = sd_netlink_message_append_u32(m, IFLA_BOND_DOWNDELAY,
                                          b->downdelay / USEC_PER_MSEC);
        if (r < 0)
            return log_netdev_error_errno(
                netdev, r,
    ...
    if (b->updelay != 0)
    {
        r = sd_netlink_message_append_u32(m, IFLA_BOND_UPDELAY,
                                          b->updelay / USEC_PER_MSEC);
        if (r < 0)
            return log_netdev_error_errno(
    ...
    if (b->arp_interval != 0)
    {
        r = sd_netlink_message_append_u32(m, IFLA_BOND_ARP_INTERVAL,
        ...
        {
            r = sd_netlink_message_append_u32(m, IFLA_BOND_LP_INTERVAL,
                                              b->lp_interval / USEC_PER_SEC);
            if (r < 0)
                return log_netdev_error_errno(
                    netdev, r,
    ...
    {
        r = sd_netlink_message_append_u32(m, IFLA_BOND_ARP_VALIDATE,
                                          b->arp_validate);
        if (r < 0)
            return log_netdev_error_errno(
                netdev, r,
    ...
    {
        r = sd_netlink_message_append_u32(m, IFLA_BOND_ARP_ALL_TARGETS,
                                          b->arp_all_targets);
        if (r < 0)
            return log_netdev_error_errno(
                netdev, r,
    ...
    {
        r = sd_netlink_message_append_u8(m, IFLA_BOND_PRIMARY_RESELECT,
                                         b->primary_reselect);
        if (r < 0)
            return log_netdev_error_errno(
                netdev, r,
    ...
    {
        r = sd_netlink_message_append_u32(m, IFLA_BOND_RESEND_IGMP,
                                          b->resend_igmp);
        if (r < 0)
            return log_netdev_error_errno(
                netdev, r,
    ...
    {
        r = sd_netlink_message_append_u32(m, IFLA_BOND_PACKETS_PER_SLAVE,
                                          b->packets_per_slave);
        if (r < 0)
            return log_netdev_error_errno(
                netdev, r,
    ...
    {
        r = sd_netlink_message_append_u8(m, IFLA_BOND_NUM_PEER_NOTIF,
                                         b->num_grat_arp);
        if (r < 0)
            return log_netdev_error_errno(
                netdev, r,
    ...
    {
        r = sd_netlink_message_append_u16(m, IFLA_BOND_AD_ACTOR_SYS_PRIO,
                                          b->ad_actor_sys_prio);
        if (r < 0)
            return log_netdev_error_errno(
                netdev, r,
    ...
    {
        r = sd_netlink_message_append_u16(m, IFLA_BOND_AD_USER_PORT_KEY,
                                          b->ad_user_port_key);
        if (r < 0)
            return log_netdev_error_errno(
                netdev, r,
    ...
    {
        r = sd_netlink_message_append_ether_addr(m, IFLA_BOND_AD_ACTOR_SYSTEM,
                                                 b->ad_actor_system);
        if (r < 0)
            return log_netdev_error_errno(
                netdev, r,
    ...
    r = sd_netlink_message_append_u8(m, IFLA_BOND_ALL_SLAVES_ACTIVE,
                                     b->all_slaves_active);
    if (r < 0)
        return log_netdev_error_errno(
            netdev, r,
    ...
    {
        r = sd_netlink_message_append_u8(m, IFLA_BOND_TLB_DYNAMIC_LB,
                                         b->tlb_dynamic_lb);
        if (r < 0)
            return log_netdev_error_errno(
                netdev, r,
...
int config_parse_arp_ip_target_address(const char* unit, const char* filename,
                                       ...
                                       void* userdata)
{
    ...
    for (;;)
    {
        ...
        if (r < 0)
        {
            log_syntax(unit, LOG_ERR, filename, line, r,
                       "Failed to parse Bond ARP ip target address, ignoring "
                       "assignment: %s",
                       rvalue);
            return 0;
        }
        ...
        {
            log_syntax(unit, LOG_ERR, filename, line, r,
                       "Bond ARP ip target address is invalid, ignoring "
                       "assignment: %s",
                       n);
            return 0;
        }
    ...
    if (b->n_arp_ip_targets > NETDEV_BOND_ARP_TARGETS_MAX)
        log_syntax(unit, LOG_WARNING, filename, line, 0,
                   "More than the maximum number of kernel-supported ARP ip "
                   "targets specified: %d > %d",
                   b->n_arp_ip_targets, NETDEV_BOND_ARP_TARGETS_MAX);

    return 0;
}

int config_parse_ad_actor_sys_prio(const char* unit, const char* filename,
                                   unsigned line, const char* section,
                                   ...
                                   void* userdata)
{
    ...
    {
        log_syntax(unit, LOG_ERR, filename, line, 0,
                   "Failed to parse actor system priority '%s'. Range is "
                   "[1,65535], ignoring.",
                   rvalue);
        return 0;
    }
...
int config_parse_ad_user_port_key(const char* unit, const char* filename,
                                  unsigned line, const char* section,
                                  unsigned section_line, const char* lvalue,
                                  int ltype, const char* rvalue, void* data,
                                  void* userdata)
...
int config_parse_ad_actor_system(const char* unit, const char* filename,
                                 unsigned line, const char* section,
                                 unsigned section_line, const char* lvalue,
                                 int ltype, const char* rvalue, void* data,
                                 void* userdata)
{
    ...
    {
        log_syntax(unit, LOG_ERR, filename, line, 0,
                   "Not a valid MAC address %s, can not be null or multicast. "
                   "Ignoring assignment.",
                   rvalue);
        return 0;
    }
