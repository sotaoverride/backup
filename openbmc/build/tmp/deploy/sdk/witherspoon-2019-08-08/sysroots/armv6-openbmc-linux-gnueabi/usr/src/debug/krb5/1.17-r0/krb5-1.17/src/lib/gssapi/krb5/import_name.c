 ...
 */
static krb5_error_code import_name_composite(krb5_context context,
                                             ...
                                             krb5_authdata_context* pad_context)
{
    ...
    code = krb5_authdata_import_attributes(context, ad_context, AD_USAGE_MASK,
                                           &data);
    if (code != 0)
    {
...
static krb5_error_code parse_hostbased(const char* str, size_t len,
    ...
    {                                                                          \
        if ((end) - (cp) < (n))                                                \
            ...
            if (is_composite)
            {
                ...
                BOUNDS_CHECK(cp, end, length);
                code = import_name_composite(context, cp, length, &ad_context);
                if (code != 0)
                    goto fail_name;
                cp += length;
            }
