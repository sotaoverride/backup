...
void TUNABLE_CALLBACK(set_mallopt_check)(tunable_val_t* valp)
{
...
#define TUNABLE_CALLBACK_FNDECL(__name, __type)                                \
    ...
    void TUNABLE_CALLBACK(__name)(tunable_val_t * valp)                        \
    {                                                                          \
...
static char* next_env_entry(char*** position)
{
    ...
    while (*current != NULL)
    {
        if (__builtin_expect((*current)[0] == 'M', 0) && (*current)[1] == 'A' &&
            (*current)[2] == 'L' && (*current)[3] == 'L' &&
            (*current)[4] == 'O' && (*current)[5] == 'C' &&
            (*current)[6] == '_')
        {
            result = &(*current)[7];
...
#ifdef SHARED
static void* __failing_morecore(ptrdiff_t d)
{
...
static void ptmalloc_init(void)
{
    ...
    if (_dl_open_hook != NULL ||
        (_dl_addr(ptmalloc_init, &di, &l, NULL) != 0 && l->l_ns != LM_ID_BASE))
                ...
                case 15:
                    if (!__builtin_expect(__libc_enable_secure, 0))
                    {
                        if (memcmp(envline, "TRIM_THRESHOLD_", 15) == 0)
                            __libc_mallopt(M_TRIM_THRESHOLD,
                                           atoi(&envline[16]));
                        else if (memcmp(envline, "MMAP_THRESHOLD_", 15) == 0)
                            __libc_mallopt(M_MMAP_THRESHOLD,
                                           atoi(&envline[16]));
                    }
                    break;
                default:
...
static int shrink_heap(heap_info* h, long diff)
{
    ...
    {
        if ((char*)MMAP((char*)h + new_size, diff, PROT_NONE, MAP_FIXED) ==
            (char*)MAP_FAILED)
            return -2;
