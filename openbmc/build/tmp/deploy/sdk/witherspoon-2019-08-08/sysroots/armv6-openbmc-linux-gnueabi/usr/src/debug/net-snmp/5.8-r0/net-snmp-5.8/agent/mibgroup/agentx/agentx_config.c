...
void agentx_parse_master(const char* token, char* cptr)
{
    ...
    if (!strcmp(cptr, "agentx") || !strcmp(cptr, "all") ||
    ...
    if (i < 0 || i > 1)
    {
        ...
        netsnmp_ds_set_boolean(NETSNMP_DS_APPLICATION_ID,
                               NETSNMP_DS_AGENT_AGENTX_MASTER, i);
}
...
void agentx_parse_agentx_perms(const char* token, char* cptr)
{
    ...
    if (socket_perm)
    {
        ...
        DEBUGMSGTL(
            ("agentx/config", "socket permissions: %o (%d)\n", s_perm, s_perm));
    }
    if (dir_perm)
    {
    ...
    if (socket_user)
    {
        ...
        DEBUGMSGTL(
            ("agentx/config", "socket owner: %s (%d)\n", socket_user, uid));
    }
    ...
    if (socket_group)
    {
        ...
        DEBUGMSGTL(
            ("agentx/config", "socket group: %s (%d)\n", socket_group, gid));
    }
}
...
void agentx_parse_agentx_retries(const char* token, char* cptr)
{
    int x = atoi(cptr);
 ...
 */
void agentx_register_config_handler(const char* token,
...
netsnmp_feature_child_of(agentx_unregister_config_handler, netsnmp_unused)
    ...
    void agentx_unregister_config_handler(const char* token)
{
...
#undef val
    agentx_register_config_handler("agentxsocket", agentx_parse_agentx_socket,
                                   NULL, "AgentX bind address");
#ifdef USING_AGENTX_MASTER_MODULE
    /*
    ...
    {
        snmpd_register_config_handler("master", agentx_parse_master, NULL,
                                      "specify 'agentx' for AgentX support");
        agentx_register_config_handler(
            "agentxperms", agentx_parse_agentx_perms, NULL,
    ...
    if (SUB_AGENT == agent_role)
    {
        ...
        netsnmp_ds_register_config(
            ...
            "agentxPingInterval", NETSNMP_DS_APPLICATION_ID,
