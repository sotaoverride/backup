...
struct msdos_super_block
{
    ...
    /* 20*/ uint32_t ms_total_sect; /* iff ms_sectors == 0 */
                                    /* V4 BPB */
    /* 24*/ unsigned char
        ms_unknown[3]; /* Phys drive no., resvd, V4 sig (0x29) */
 ...
 */
static unsigned char* search_fat_label(blkid_probe pr, uint64_t offset,
                                       uint32_t entries)
{
    ...
    DBG(LOWPROBE, ul_debug("\tlook for label in root-dir "
                           "(entries: %" PRIu32 ", offset: %" PRIu64 ")",
                           entries, offset));

    if (!blkid_probe_is_tiny(pr))
    {
        /* large disk, read whole root directory */
        dir = (struct vfat_dir_entry*)blkid_probe_get_buffer(
            pr, offset, (uint64_t)entries * sizeof(struct vfat_dir_entry));
        if (!dir)
            return NULL;
    }
    ...
    for (i = 0; i < entries; i++)
    {
         ...
         */
        if (!dir)
            ent = (struct vfat_dir_entry*)blkid_probe_get_buffer(
                pr, (uint64_t)offset + (i * sizeof(struct vfat_dir_entry)),
...
static int fat_valid_superblock(blkid_probe pr, const struct blkid_idmag* mag,
                                ...
                                uint32_t* cluster_count, uint32_t* fat_size)
{
    ...
    if (!is_power_of_2(sector_size) || sector_size < 512 || sector_size > 4096)
        return 0;
    ...
    dir_size =
        ((dir_entries * sizeof(struct vfat_dir_entry)) + (sector_size - 1)) /
        sector_size;

    __cluster_count =
        (sect_count - (reserved + __fat_size + dir_size)) / ms->ms_cluster_size;
...
static int probe_vfat(blkid_probe pr, const struct blkid_idmag* mag)
{
    ...
    else if (vs->vs_fat32_length)
    {
        ...
        while (next && next < entries && --maxloop)
        {
            ...
            next_off =
                (uint64_t)(start_data_sect + next_sect_off) * sector_size;
            ...
            /* get FAT entry */
            fat_entry_off =
                ((uint64_t)reserved * sector_size) + (next * sizeof(uint32_t));
        ...
        if (fsinfo_sect)
        {
            ...
            buf =
                blkid_probe_get_buffer(pr, (uint64_t)fsinfo_sect * sector_size,
    ...
    if (vol_serno)
        blkid_probe_sprintf_uuid(pr, vol_serno, 4, "%02X%02X-%02X%02X",
                                 vol_serno[3], vol_serno[2], vol_serno[1],
                                 vol_serno[0]);
    if (version)
...
const struct blkid_idinfo vfat_idinfo = {
    ...
    .magics = {{.magic = "MSWIN", .len = 5, .sboff = 0x52},
