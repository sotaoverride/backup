...
static void SHA256_Update_recycled(SHA256_CTX* ctx, unsigned char block[32],
                                   size_t len)
{
    size_t cnt;
                          ...
                          size_t scr_size)
{
    ...
    {
        int n =
            snprintf(cp, SHA256_HASH_LENGTH - (sizeof(sha256_salt_prefix) - 1),
...
void gensalt_sha256crypt_rn(unsigned long count, const uint8_t* rbytes,
