                                   ...
                                   char* buffer, size_t buflen)
{
    ...
    {
        log_log(LOG_ERR,
                "mkfilter_service_byname(): safename buffer too small");
        return -1;
    }
    /* build filter */
    if (*protocol != '\0')
    {
        ...
        {
            log_log(LOG_ERR,
                    "mkfilter_service_byname(): safeprotocol buffer too small");
            return -1;
        }
        return mysnprintf(buffer, buflen, "(&%s(%s=%s)(%s=%s))", service_filter,
                          attmap_service_cn, safename,
                          attmap_service_ipServiceProtocol, safeprotocol);
    }
    else
        return mysnprintf(buffer, buflen, "(&%s(%s=%s))", service_filter,
                          attmap_service_cn, safename);
}
                                     ...
                                     char* buffer, size_t buflen)
{
    ...
    if (*protocol != '\0')
    {
        if (myldap_escape(protocol, safeprotocol, sizeof(safeprotocol)))
        {
            log_log(
                LOG_ERR,
                "mkfilter_service_bynumber(): safeprotocol buffer too small");
            return -1;
        }
        return mysnprintf(buffer, buflen, "(&%s(%s=%d)(%s=%s))", service_filter,
                          attmap_service_ipServicePort, number,
                          attmap_service_ipServiceProtocol, safeprotocol);
    }
    else
        return mysnprintf(buffer, buflen, "(&%s(%s=%d))", service_filter,
                          attmap_service_ipServicePort, number);
}
...
static int write_service(TFILE* fp, MYLDAP_ENTRY* entry, const char* reqname,
                         const char* reqprotocol)
{
    ...
    {
        log_log(LOG_WARNING, "%s: %s: multiple values", myldap_get_dn(entry),
                attmap_service_ipServicePort);
    }
    errno = 0;
    ...
    if ((*(ports[0]) == '\0') || (*tmp != '\0'))
    {
        log_log(LOG_WARNING, "%s: %s: non-numeric value", myldap_get_dn(entry),
                attmap_service_ipServicePort);
        return 0;
    }
    ...
    for (i = 0; protocols[i] != NULL; i++)
            ...
            (STR_CMP(reqprotocol, protocols[i]) == 0))
        {
            ...
            /* port number is actually a 16-bit value but we write 32 bits
             * anyway */
            WRITE_INT32(fp, port);
...
NSLCD_HANDLE(service, byname, NSLCD_ACTION_SERVICE_BYNAME,
             char name[BUFLEN_NAME];
             char protocol[BUFLEN_NAME]; char filter[BUFLEN_FILTER];
             READ_STRING(fp, name); READ_STRING(fp, protocol);
...
NSLCD_HANDLE(service, bynumber, NSLCD_ACTION_SERVICE_BYNUMBER, int number;
             char protocol[BUFLEN_NAME]; char filter[BUFLEN_FILTER];
             READ_INT32(fp, number); READ_STRING(fp, protocol);
             log_setrequest("service=%lu%s%s", (unsigned long int)number,
                            *protocol != '\0' ? "/" : "", protocol);
             ,
             mkfilter_service_bynumber(number, protocol, filter,
                                       sizeof(filter)),
             write_service(fp, entry, NULL, protocol))

NSLCD_HANDLE(service, all, NSLCD_ACTION_SERVICE_ALL, const char* filter;
