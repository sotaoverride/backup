 ...
 * always has to be parsed by private libmount routines with an explicitly
 * defined format.
 *
...
static int append_comment(struct libmnt_table* tb, struct libmnt_fs* fs,
                          const char* comm, int eof)
{
    ...
    DBG(TAB, ul_debugobj(tb, "appending %s comment",
                         intro ? "intro" : eof ? "trailing" : "fs"));
    if (intro)
        rc = mnt_table_append_intro_comment(tb, comm);
    else if (eof)
    {
        rc = mnt_table_set_trailing_comment(tb, mnt_fs_get_comment(fs));
        if (!rc)
...
next_line:
    do
    {
        ...
        if (!s)
        {
            ...
            if (feof(pa->f))
            {
                DBG(TAB, ul_debugobj(tb, "%s: no final newline", pa->filename));
                s = strchr(pa->buf, '\0');
            }
            else
            {
                DBG(TAB, ul_debugobj(tb, "%s:%zu: missing newline at line",
                                     pa->filename, pa->line));
                goto err;
            }
        }
...
err:
    DBG(TAB,
        ul_debugobj(tb, "%s:%zu: %s parse error", pa->filename, pa->line,
...
static int kernel_fs_postparse(struct libmnt_table* tb, struct libmnt_fs* fs,
                               pid_t* tid, const char* filename)
{
    int rc = 0;
 ...
 */
int mnt_table_parse_stream(struct libmnt_table* tb, FILE* f,
                           const char* filename)
{
    int rc = -1;
    int flags = 0;
    ...
    DBG(TAB,
        ul_debugobj(tb, "%s: start parsing [entries=%d, filter=%s]", filename,
    ...
    DBG(TAB, ul_debugobj(tb, "%s: stop parsing (%d entries)", filename,
 ...
 * reported to the caller by the errcb() function (see
...
int mnt_table_parse_file(struct libmnt_table* tb, const char* filename)
{
    ...
    DBG(TAB,
        ul_debugobj(tb, "parsing done [filename=%s, rc=%d]", filename, rc));
    return rc;
}
...
#define MNT_MNTTABDIR_EXTSIZ (sizeof(MNT_MNTTABDIR_EXT) - 1)
    ...
    if (!namesz || namesz < MNT_MNTTABDIR_EXTSIZ + 1 ||
        strcmp(d->d_name + (namesz - MNT_MNTTABDIR_EXTSIZ), MNT_MNTTABDIR_EXT))
        return 0;
...
static int __mnt_table_parse_dir(struct libmnt_table* tb, const char* dirname)
{
    ...
    for (i = 0; i < n; i++)
    {
        ...
        if (fstatat(dd, d->d_name, &st, 0) || !S_ISREG(st.st_mode))
            continue;
...
static int __mnt_table_parse_dir(struct libmnt_table* tb, const char* dirname)
{
    ...
    for (i = 0; i < n; i++)
    {
        ...
        if (fstatat(dirfd(dir), d->d_name, &st, 0) || !S_ISREG(st.st_mode))
            continue;

        f = fopen_at(dirfd(dir), d->d_name, O_RDONLY | O_CLOEXEC,
                     "r" UL_CLOEXECSTR);
        if (f)
        {
            mnt_table_parse_stream(tb, f, d->d_name);
 ...
 *
 * Same as mnt_new_table() + mnt_table_parse_file(). Use this function for
 * private files only. This function does not allow using the error callback, so
 * you cannot provide any feedback to end-users about broken records in files
 * (e.g. fstab).
 *
 ...
 *
 * The error callback function is called by table parser
 * (mnt_table_parse_file()) in case of a syntax error. The callback function
 * could be used for error evaluation, libmount will continue/stop parsing
 * according to callback return codes:
 *
...
int mnt_table_set_parser_fltrcb(struct libmnt_table* tb,
                                int (*cb)(struct libmnt_fs*, void*), void* data)
{
    if (!tb)
        return -EINVAL;
 ...
 */
static struct libmnt_fs* mnt_table_merge_user_fs(struct libmnt_table* tb,
                                                 struct libmnt_fs* uf)
{
    struct libmnt_fs* fs;
    ...
    while (mnt_table_next_fs(tb, &itr, &fs) == 0)
    {
        ...
        if (r && strcmp(r, root) == 0 && mnt_fs_streq_target(fs, target) &&
