...
#pragma warning(                                                               \
    disable : 4390) /*empty controlled statement found; is this the intent?*/
...
static mchunkptr try_realloc_chunk_with_min(mstate m, mchunkptr p,
                                            size_t min_nb, size_t des_nb,
                                            int can_move)
{
    mchunkptr newp = 0;
                ...
                ok_pinuse(next)))
    {
        if (is_mmapped(p))
        {
            ...
            if (!newp) // mmap does not return how many bytes we could
                       // reallocate, so go the minimum
        ...
        { /* extend into top */
            ...
            {
                size_t nb = (oldsize + m->topsize) > des_nb
                                ? des_nb
                                : (oldsize + m->topsize - MALLOC_ALIGNMENT);
        ...
        { /* extend into dv */
            ...
            {
                size_t nb =
                    (oldsize + dvs) >= des_nb ? des_nb : (oldsize + dvs);
        ...
        { /* extend into next free chunk */
            ...
            {
                size_t nb = (oldsize + nextsize) >= des_nb
                                ? des_nb
                                : (oldsize + nextsize);
...
#define GET_TRUNCATED_SIZE(ORIG_SIZE, ROUNDTO)                                 \
    ((ORIG_SIZE) / (ROUNDTO) * (ROUNDTO))
#define GET_ROUNDED_SIZE(ORIG_SIZE, ROUNDTO)                                   \
    ((((ORIG_SIZE)-1) / (ROUNDTO) + 1) * (ROUNDTO))
#define GET_TRUNCATED_PO2_SIZE(ORIG_SIZE, ROUNDTO)                             \
    ((ORIG_SIZE) & (~(ROUNDTO - 1)))
#define GET_ROUNDED_PO2_SIZE(ORIG_SIZE, ROUNDTO)                               \
    (((ORIG_SIZE - 1) & (~(ROUNDTO - 1))) + ROUNDTO)
    ...
    size_t* plcm, size_t* pneeds_backwards_lcmed)
{
    ...
    if ((backwards_multiple & (backwards_multiple - 1)) == 0)
    {
        ...
        current_forward =
            GET_TRUNCATED_PO2_SIZE(received_size, backwards_multiple);
    ...
    else if ((backwards_multiple & (MALLOC_ALIGNMENT - 1u)) == 0)
    {
        ...
        needs_backwards_lcmed = needs_backwards =
            size_to_achieve - current_forward;
    ...
    else if ((backwards_multiple & ((MALLOC_ALIGNMENT / 4u) - 1u)) == 0)
    {
        ...
        // needs_backwards_lcmed += backwards_multiple;
        if (0 !=
            (remainder = ((needs_backwards_lcmed & (MALLOC_ALIGNMENT - 1)) >>
                          (MALLOC_ALIGNMENT / 8u))))
        {
...
static void* internal_grow_both_sides(mstate m, allocation_type command,
                                      void* oldmem, size_t minbytes,
                                      size_t maxbytes, size_t* received_size,
                                      ...
                                      int only_preferred_backwards)
{
    ...
    *received_size = oldsize - overhead_for(oldp);
    ...
    if (RTCHECK(ok_address(m, oldp) && ok_inuse(oldp)))
    {
        ...
        {
            if (try_realloc_chunk_with_min(m, oldp, request2size(minbytes),
                                           request2size(maxbytes), 0))
            {
        ...
        }
        {
            ...
            if ((prevsize & USE_MMAP_BIT) != 0)
            {
                /*Return failure the previous chunk was mmapped.
                  mremap does not allow expanding to a fixed address
                  (MREMAP_MAYMOVE) without
            ...
            else
            {
                ...
                /* Let's calculate the number of extra bytes of data before the
                current block's begin. The value is a multiple of
                backwards_multiple and the alignment*/
                if (!calculate_lcm_and_needs_backwards_lcmed(
                        backwards_multiple, *received_size,
                        only_preferred_backwards ? maxbytes : minbytes, &lcm,
                ...
                if (command & BOOST_CONTAINER_EXPAND_FWD)
                {
                    if (!try_realloc_chunk_with_min(
                            m, oldp, request2size(*received_size),
                            request2size(*received_size), 0))
                    {
                        assert(0);
                    }
                    check_inuse_chunk(m, oldp);
                ...
                {
                    mchunkptr r =
                        chunk_minus_offset(oldp, needs_backwards_lcmed);
                ...
                /* Check if there is no place to create a new block and
                   the whole new block is multiple of the backwards expansion
                   multiple */
...
*/
static int internal_mmap_shrink_in_place(mstate m, mchunkptr oldp, size_t nbmin,
                                         size_t nbmax, size_t* received_size,
                                         int do_commit)
{
    size_t oldsize = chunksize(oldp);
    *received_size = oldsize;
        ...
        return 0;
    {
        ...
        /* Now calculate new sizes */
        {
            ...
            size_t newmmsize =
                mmap_align(effective_min + SIX_SIZE_T_SIZES + CHUNK_ALIGN_MASK);
            *received_size = newmmsize;
            if (!do_commit)
            {
                ...
                }
                {
                    ...
                    if (cp < m->least_addr)
                        m->least_addr = cp;
                    if ((m->footprint += newmmsize - oldmmsize) >
                        m->max_footprint)
...
static int internal_shrink(mstate m, void* oldmem, size_t minbytes,
                           size_t maxbytes, size_t* received_size,
                           int do_commit)
{
    *received_size =
        chunksize(mem2chunk(oldmem)) - overhead_for(mem2chunk(oldmem));
...

    {
                    ...
                    ok_next(oldp, next) && ok_pinuse(next)))
        {
            ...
            {
                return internal_mmap_shrink_in_place(m, oldp, nbmin, nbmax,
                                                     received_size, do_commit);
            }
            else
...
static int internal_node_multialloc(mstate m, size_t n_elements,
                                    size_t element_size,
                                    ...
                                    boost_cont_memchain* pchain)
{
    ...
    boost_cont_memchain_it prev_last_it =
    ...
    if (!element_size ||
        ...
        ((contiguous_elements + 1) > (DL_MULTIALLOC_DEFAULT_CONTIGUOUS + 1) &&
         n_elements < contiguous_elements) ||
    ...
    switch (contiguous_elements)
    {
        ...
        {
            /* Default contiguous, just check that we can store at least one
             * element */
            elements_per_segment =
                INTERNAL_MULTIALLOC_DEFAULT_CONTIGUOUS_MEM / element_req_size;
...

    {
        ...
        */
        ...
        for (i = 0; i != n_elements; i = next_i)
        {
            ...
            next_i = i + ((n_elements_left < elements_per_segment)
                              ? n_elements_left
                              : elements_per_segment);
            ...
            {   /* split out elements */
                // void *mem_orig = mem;
                // boost_cont_memchain_it last_it =
                // BOOST_CONTAINER_MEMCHAIN_LAST_IT(pchain);
                ...
                BOOST_CONTAINER_MEMCHAIN_PUSH_BACK(pchain, mem);
                // BOOST_CONTAINER_MEMCHAIN_INCORPORATE_AFTER(pchain, last_it,
                // mem_orig, mem, num_elements);
            }
        }
        if (was_enabled)
            enable_mmap(m);
...
// Instead of returning the unallocated nodes, returns a chain of
// non-deallocated nodes. After forward merging, backwards merging is also tried
static void internal_multialloc_free(mstate m, boost_cont_memchain* pchain)
{
    ...
    if (!PREACTION(m))
    {
        ...
        { /* Iterate though all memory holded by the chain */
            ...
            {
                while (1)
                { /* Internal loop to speed up forward and backward merging
                    ...
                    if (!BOOST_CONTAINER_MEMCHAIN_IS_END_IT(pchain, b_it))
                    {
                        ...
                        { /* b chunk is contiguous and next so b's size can be
                             added to a */
                            psize += chunksize(b_p);
                            set_inuse(m, a_p, psize);
                            BOOST_ALLOC_PLUS_MEMCHAIN_MEM_JUMP_NEXT(a_mem,
                                                                    b_mem);
                            continue;
                        }
                        if (RTCHECK(ok_address(m, b_p) && ok_inuse(b_p)))
                        {
                            /* b chunk is contiguous and previous so a's size
                             * can be added to b */
...
#if FOOTERS
    {
        boost_cont_memchain_it last_pchain =
            BOOST_CONTAINER_MEMCHAIN_LAST_IT(pchain);
        BOOST_CONTAINER_MEMCHAIN_INIT(pchain);
        BOOST_CONTAINER_MEMCHAIN_INCORPORATE_AFTER(
            pchain, last_pchain, BOOST_CONTAINER_MEMCHAIN_FIRSTMEM(&ret_chain),
...
static int internal_multialloc_arrays(mstate m, size_t n_elements,
                                      const size_t* sizes, size_t element_size,
                                      size_t contiguous_elements,
                                      boost_cont_memchain* pchain)
{
    void* mem;             /* malloced aggregate space */
    ...
    switch (contiguous_elements)
    {
        case DL_MULTIALLOC_DEFAULT_CONTIGUOUS:
            ...
            boost_cont_multialloc_segmented_malloc_size =
        ...
        default:
            ...
            {
                /* The suggested buffer is just the the element count by the
                 * size */
                boost_cont_multialloc_segmented_malloc_size =
                    element_size * contiguous_elements;
            }
    }
        ...
        */
        ...
        for (i = 0, next_i = 0; i != n_elements; i = next_i)
        {
            ...
            for (accum_size = 0; next_i != n_elements; ++next_i)
            {
                ...
                {
                    size_t reqsize =
                        request2size(cur_array_size * element_size);
                    if (((boost_cont_multialloc_segmented_malloc_size -
                          CHUNK_OVERHEAD) -
                         accum_size) < reqsize)
                    {
                        if (!accum_size)
            ...
            mem = error
                      ? 0
                      : mspace_malloc_lockless(m, accum_size - CHUNK_OVERHEAD);
            if (mem == 0)
            {
                boost_cont_memchain_it it =
            ...
            { /* split out elements */
                ...
                set_size_and_pinuse_of_inuse_chunk(m, p, remainder_size);
                BOOST_CONTAINER_MEMCHAIN_INCORPORATE_AFTER(
                    pchain, last_it, mem_orig, mem, num_elements);
            }
        }
        if (was_enabled)
            enable_mmap(m);
...
int boost_cont_multialloc_arrays(size_t n_elements, const size_t* sizes,
                                 size_t element_size,
                                 size_t contiguous_elements,
                                 boost_cont_memchain* pchain)
{
    ...
    {
        ret = internal_multialloc_arrays(ms, n_elements, sizes, element_size,
                                         contiguous_elements, pchain);
        POSTACTION(ms);
    }
    return ret;
}
...
void* boost_cont_malloc(size_t bytes)
{
    ...
    ensure_initialization();
    return boost_cont_allocation_command(BOOST_CONTAINER_ALLOCATE_NEW, 1, bytes,
                                         bytes, &received_bytes, 0)
        .first;
}
...
int boost_cont_multialloc_nodes(size_t n_elements, size_t elem_size,
                                size_t contiguous_elements,
                                boost_cont_memchain* pchain)
{
    int ret = 0;
    ...
    {
        ret = internal_node_multialloc(ms, n_elements, elem_size,
                                       contiguous_elements, pchain);
        POSTACTION(ms);
    }
    return ret;
    ...
    if (!PREACTION(m))
    {
        ...
        if (is_initialized(m))
        {
            ...
            }
            {
                ...
                if (nfree)
                    alloc_mem =
                        (size_t)(uordblks - (nfree - 1) * TOP_FOOT_SIZE);
                else
                    alloc_mem = uordblks;
            }
        }
...
int boost_cont_grow(void* oldmem, size_t minbytes, size_t maxbytes,
                    size_t* received)
{
    mstate ms = (mstate)gm;
    if (!ok_magic(ms))
    ...
    if (!PREACTION(ms))
    {
        ...
        size_t oldsize = chunksize(p);
        p = try_realloc_chunk_with_min(ms, p, request2size(minbytes),
                                       request2size(maxbytes), 0);
        POSTACTION(ms);
        if (p)
        {
...
int boost_cont_shrink(void* oldmem, size_t minbytes, size_t maxbytes,
                      size_t* received, int do_commit)
{
    mstate ms = (mstate)gm;
    if (!ok_magic(ms))
    {
    ...
    if (!PREACTION(ms))
    {
        int ret = internal_shrink(ms, oldmem, minbytes, maxbytes, received,
                                  do_commit);
        POSTACTION(ms);
        return 0 != ret;
    }
...
void* boost_cont_alloc(size_t minbytes, size_t preferred_bytes,
                       size_t* received_bytes)
{
    // ensure_initialization provided by boost_cont_allocation_command
    return boost_cont_allocation_command(BOOST_CONTAINER_ALLOCATE_NEW, 1,
                                         minbytes, preferred_bytes,
                                         received_bytes, 0)
        .first;
}
...
boost_cont_command_ret_t
    boost_cont_allocation_command(allocation_type command, size_t sizeof_object,
                                  size_t limit_size, size_t preferred_size,
                                  size_t* received_size, void* reuse_ptr)
{
    boost_cont_command_ret_t ret = {0, 0};
    ensure_initialization();
    if (command &
        (BOOST_CONTAINER_SHRINK_IN_PLACE | BOOST_CONTAINER_TRY_SHRINK_IN_PLACE))
    {
        int success = boost_cont_shrink(
            reuse_ptr, preferred_size, limit_size, received_size,
            (command & BOOST_CONTAINER_SHRINK_IN_PLACE));
...
#endif
            if (reuse_ptr && (command & (BOOST_CONTAINER_EXPAND_FWD |
                                         BOOST_CONTAINER_EXPAND_BWD)))
            {
                void* r = internal_grow_both_sides(
                    ms, command, reuse_ptr, limit_size, preferred_size,
                    received_size, sizeof_object, 1);
                if (r)
                {
                    ret.first = r;
                    ret.second = 1;
