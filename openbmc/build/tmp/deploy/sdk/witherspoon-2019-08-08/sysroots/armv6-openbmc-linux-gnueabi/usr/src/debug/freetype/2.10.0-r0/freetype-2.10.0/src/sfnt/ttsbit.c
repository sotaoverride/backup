...
tt_face_load_sbit(TT_Face face, FT_Stream stream)
{
    ...
    switch ((FT_UInt)face->sbit_table_type)
    {
        ...
        case TT_SBIT_TABLE_TYPE_SBIX:
        {
            ...
            if (!(flags == 1 || flags == 3) || num_strikes >= 0x10000UL)
            {
                error = FT_THROW(Invalid_File_Format);
                goto Exit;
            }
...
FT_LOCAL_DEF(FT_Error)
tt_face_set_sbit_strike(TT_Face face, FT_Size_Request req,
...
FT_LOCAL_DEF(FT_Error)
tt_face_load_strike_metrics(TT_Face face, FT_ULong strike_index,
    ...
    switch ((FT_UInt)face->sbit_table_type)
    {
        ...
        case TT_SBIT_TABLE_TYPE_CBLC:
        {
            ...
            else if (metrics->descender == 0)
            {
                if (metrics->ascender == 0)
                {
                    FT_TRACE2(("tt_face_load_strike_metrics:"
                               ...
                               strike_index, metrics->x_ppem, metrics->y_ppem));
          ...
          ; /* if we have a negative descender, simply use it */
            ...
            metrics->x_scale = FT_MulDiv(metrics->x_ppem, 64 * 0x10000,
                                         face->header.Units_Per_EM);
            metrics->y_scale = FT_MulDiv(metrics->y_ppem, 64 * 0x10000,
                                         face->header.Units_Per_EM);
        ...
        case TT_SBIT_TABLE_TYPE_SBIX:
        {
            ...
            if (FT_STREAM_SEEK(face->ebdt_start + offset) || FT_FRAME_ENTER(4))
                return error;
            ...
            metrics->ascender = FT_MulDiv(hori->Ascender, ppem_ * 64, upem);
            metrics->descender = FT_MulDiv(hori->Descender, ppem_ * 64, upem);
            metrics->height =
...
static FT_Error tt_sbit_decoder_init(TT_SBitDecoder decoder, TT_Face face,
...
static FT_Error tt_sbit_decoder_load_image(TT_SBitDecoder decoder,
                                           FT_UInt glyph_index, FT_Int x_pos,
                                           FT_Int y_pos, FT_UInt recurse_count,
                                           FT_Bool metrics_only);

typedef FT_Error (*TT_SBitDecoder_LoadFunc)(TT_SBitDecoder decoder, FT_Byte* p,
                                            FT_Byte* plimit, FT_Int x_pos,
                                            FT_Int y_pos,
                                            FT_UInt recurse_count);

static FT_Error tt_sbit_decoder_load_byte_aligned(TT_SBitDecoder decoder,
                                                  FT_Byte* p, FT_Byte* limit,
                                                  FT_Int x_pos, FT_Int y_pos,
                                                  FT_UInt recurse_count)
{
    FT_Error error = FT_Err_Ok;
    FT_Byte* line;
    ...
    if (x_pos < 0 || (FT_UInt)(x_pos + width) > bit_width || y_pos < 0 ||
        (FT_UInt)(y_pos + height) > bit_height)
    {
                                                 ...
                                                 FT_UInt recurse_count)
{
    ...
    for (h = height; h > 0; h--, line += pitch)
    {
        ...
        if (x_pos)
        {
            ...
            *pwrite++ |=
                ((rval >> nbits) & 0xFF) & (~(0xFFU << w) << (8 - w - x_pos));
            rval <<= 8;
                                              ...
                                              FT_UInt recurse_count)
{
    ...
    FT_TRACE3(("tt_sbit_decoder_load_compound: loading %d component%s\n",
               num_components, num_components == 1 ? "" : "s"));
    ...
    for (nn = 0; nn < num_components; nn++)
    {
        ...
        /* NB: a recursive call */
        error = tt_sbit_decoder_load_image(decoder, gindex, x_pos + dx,
                                           y_pos + dy, recurse_count + 1,
...
static FT_Error tt_sbit_decoder_load_png(TT_SBitDecoder decoder, FT_Byte* p,
                                         FT_Byte* limit, FT_Int x_pos,
                                         FT_Int y_pos, FT_UInt recurse_count)
{
    FT_Error error = FT_Err_Ok;
    FT_ULong png_len;
    ...
    error = Load_SBit_Png(decoder->face->root.glyph, x_pos, y_pos,
                          decoder->bit_depth, decoder->metrics,
                          decoder->stream->memory, p, png_len, FALSE, FALSE);
...
static FT_Error tt_sbit_decoder_load_bitmap(TT_SBitDecoder decoder,
                                            ...
                                            FT_ULong glyph_size, FT_Int x_pos,
                                            FT_Int y_pos, FT_UInt recurse_count,
                                            FT_Bool metrics_only)
{
    FT_Error error;
    ...
    if (!glyph_size || glyph_start + glyph_size > decoder->ebdt_size)
    {
        error = FT_THROW(Invalid_Argument);
        goto Exit;
    }
...

    {
        ...
        switch (glyph_format)
        {
            ...
            case 7:
            {
                ...
                if (bit_size < byte_size && byte_size == (FT_UInt)(p_limit - p))
                    loader = tt_sbit_decoder_load_byte_aligned;
                else
...
FoundRange:
    ...
    return tt_sbit_decoder_load_bitmap(decoder, image_format, image_start,
                                       image_end, x_pos, y_pos, recurse_count,
                                       metrics_only);
...
static FT_Error tt_face_load_sbix_image(TT_Face face, FT_ULong strike_index,
                                        FT_UInt glyph_index, FT_Stream stream,
                                        FT_Bitmap* map,
                                        TT_SBit_MetricsRec* metrics,
                                        FT_Bool metrics_only)
{
...
retry:
    ...
    if (FT_STREAM_SEEK(face->ebdt_start + strike_offset + 4 +
                       glyph_index * 4) ||
        FT_FRAME_ENTER(8))
        return error;
    ...
    if (glyph_start == glyph_end)
        return FT_THROW(Missing_Bitmap);
    if (glyph_start > glyph_end || glyph_end - glyph_start < 8 ||
...
#ifdef FT_CONFIG_OPTION_USE_PNG
            error =
                Load_SBit_Png(face->root.glyph, 0, 0, 32, metrics,
                              stream->memory, stream->cursor,
                              glyph_end - glyph_start - 8, TRUE, metrics_only);
#else
            error = FT_THROW(Unimplemented_Feature);
#endif
            break;
    ...
    if (!error)
    {
        ...
        metrics->horiAdvance =
            (FT_UShort)(aadvance * face->root.size->metrics.x_ppem /
                        face->header.Units_Per_EM);
    }
...
tt_face_load_sbit_image(TT_Face face, FT_ULong strike_index,
                        ...
                        FT_Stream stream, FT_Bitmap* map,
                        TT_SBit_MetricsRec* metrics)
{
    FT_Error error = FT_Err_Ok;
    ...
    if (!error && !(load_flags & FT_LOAD_COLOR) &&
