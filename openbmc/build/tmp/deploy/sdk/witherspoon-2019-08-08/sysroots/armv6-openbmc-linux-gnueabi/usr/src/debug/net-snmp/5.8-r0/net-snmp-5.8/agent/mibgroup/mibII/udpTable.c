...
#else
#ifdef openbsd4
#define INP_NEXT_SYMBOL                                                        \
    inp_queue.cqe_next /* or set via <net-snmp/system/openbsd.h> */
#endif
...
void init_udpTable(void)
{
    ...
    netsnmp_table_helper_add_indexes(table_info, ASN_IPADDRESS, ASN_INTEGER, 0);
    ...
    reginfo = netsnmp_create_handler_registration(
        "udpTable", udpTable_handler, udpTable_oid, OID_LENGTH(udpTable_oid),
        HANDLER_CAN_RONLY),
    ...
    netsnmp_inject_handler(reginfo, netsnmp_get_cache_handler(
                                        UDP_STATS_CACHE_TIMEOUT, udpTable_load,
                                        udpTable_free, udpTable_oid,
                                        OID_LENGTH(udpTable_oid)));
}

int udpTable_handler(netsnmp_mib_handler* handler,
                     netsnmp_handler_registration* reginfo,
    ...
    switch (reqinfo->mode)
    {
        case MODE_GET:
            for (request = requests; request; request = request->next)
            {
                ...
                entry = (UDPTABLE_ENTRY_TYPE*)netsnmp_extract_iterator_context(
                    request);
                if (!entry)
                    continue;
                ...
                switch (subid)
                {
                    case UDPLOCALADDRESS:
                        ...
                        snmp_set_var_typed_value(requestvb, ASN_IPADDRESS,
                                                 ...
                                                 sizeof(uint32_t));
#else
                        addr = UDP_ADDRESS_TO_HOST_ORDER(
                            entry->UDPTABLE_LOCALADDRESS);
                        snmp_set_var_typed_value(requestvb, ASN_IPADDRESS,
                                                 (u_char*)&addr,
                                                 sizeof(uint32_t));
#endif
                        break;
                    case UDPLOCALPORT:
                        port = UDP_PORT_TO_HOST_ORDER(
                            (u_short)entry->UDPTABLE_LOCALPORT);
...
netsnmp_variable_list* udpTable_first_entry(void** loop_context,
                                            void** data_context,
...
netsnmp_variable_list* udpTable_next_entry(void** loop_context,
                                           void** data_context,
                                           netsnmp_variable_list* index,
                                           netsnmp_iterator_info* data)
{
    ...
    snmp_set_var_value(index->next_variable, (u_char*)&port, sizeof(port));
    /*
...
void udpTable_free(netsnmp_cache* cache, void* magic)
{
#if defined(WIN32) || defined(cygwin)
    if (udp_head)
    {
...
#else
    addr =
        UDP_ADDRESS_TO_NETWORK_ORDER((in_addr_t)entry->UDPTABLE_LOCALADDRESS);
    snmp_set_var_value(index, (u_char*)&addr, sizeof(addr));
#endif
...
#ifdef hpux11
int udpTable_load(netsnmp_cache* cache, void* vmagic)
{
    int fd;
    struct nmparms p;
    int val = 0;
...
int udpTable_load(netsnmp_cache* cache, void* vmagic)
{
    ...
    while (line == fgets(line, sizeof(line), in))
    {
        ...
        if (3 != sscanf(line, "%*d: %x:%x %*x:%*x %x", &pcb.inp_laddr.s_addr,
                        &lport, &state))
            continue;
...
{
    int count;
    udp_head = kvm_getfiles(kd, KERN_FILE_BYFILE, DTYPE_SOCKET,
...
#elif defined(solaris2)
static int UDP_Cmp(void* addr, void* ep)
{
...
int udpTable_load(netsnmp_cache* cache, void* vmagic)
{
    ...
    if (getMibstat(MIB_UDP_LISTEN, &entry, sizeof(mib2_udpEntry_t), GET_FIRST,
                   &UDP_Cmp, &entry) != 0)
    {
