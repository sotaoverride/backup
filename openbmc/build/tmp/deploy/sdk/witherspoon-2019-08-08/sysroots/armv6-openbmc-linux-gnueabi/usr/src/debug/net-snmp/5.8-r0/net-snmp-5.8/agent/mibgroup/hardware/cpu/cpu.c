...
void init_cpu(void)
{
    ...
    if (_cpuAutoUpdate)
    {
        ...
        _cpu_cache = netsnmp_cache_create(5, netsnmp_cpu_arch_load, NULL, nsCPU,
                                          OID_LENGTH(nsCPU));
}
...
int netsnmp_cpu_load(void)
{
     ...
     */
    return (_cpuAutoUpdate ? 1 : netsnmp_cache_check_and_reload(_cpu_cache));
}
 ...
 */
static void _cpu_update_stats(unsigned int reg, void* magic)
{
    netsnmp_cpu_info* cpu;
    int i;
    ...
    for (cpu = _cpu_head; cpu; cpu = cpu->next)
    {
        if (!cpu->history)
        {
             ...
             */
            cpu->history = (struct netsnmp_cpu_history*)calloc(
                _cpuHistoryLen, sizeof(struct netsnmp_cpu_history));
        }
        else
        {
            /*
    ...
    for (cpu = _cpu_head; cpu; cpu = cpu->next)
    {
        cpu->total_ticks = cpu->user_ticks + cpu->nice_ticks + cpu->sys_ticks +
                           cpu->idle_ticks + cpu->wait_ticks + cpu->kern_ticks +
                           cpu->intrpt_ticks + cpu->sirq_ticks +
                           cpu->steal_ticks + cpu->guest_ticks +
                           cpu->guestnice_ticks;
    }
}
