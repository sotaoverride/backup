...
static void g_enumerable_thread_remove(gpointer data)
{
...
static void g_enumerable_thread_add(GRealThread* thread)
{
    G_LOCK(g_thread);
...
static gpointer g_deprecated_thread_proxy(gpointer data)
{
 ...
 */
GThread* g_thread_create(GThreadFunc func, gpointer data, gboolean joinable,
                         GError** error)
{
 ...
 * Use g_thread_new().
 */
GThread* g_thread_create_full(GThreadFunc func, gpointer data,
                              ...
                              GError** error)
{
    ...
    thread = g_thread_new_internal(NULL, g_deprecated_thread_proxy, func, data,
                                   stack_size, error);
...
static GRecMutex* g_static_rec_mutex_get_rec_mutex_impl(GStaticRecMutex* mutex)
{
    GRecMutex* result;
...
static void g_static_private_cleanup(gpointer data)
{
    GArray* array = data;
    guint i;
...
gpointer g_static_private_get(GStaticPrivate* private_key)
{
    ...
    if (array && private_key->index != 0 && private_key->index <= array->len)
    {
        ...
        node =
            &g_array_index(array, GStaticPrivateNode, private_key->index - 1);
    ...
    if (!private_key->index)
    {
        ...
        if (!private_key->index)
        {
            if (g_thread_free_indices)
            {
                ...
                g_thread_free_indices = g_slist_delete_link(
                    g_thread_free_indices, g_thread_free_indices);
            }
            else
...
void g_static_private_free(GStaticPrivate* private_key)
{
    ...
    g_thread_free_indices =
        g_slist_prepend(g_thread_free_indices, GUINT_TO_POINTER(idx));
    G_UNLOCK(g_thread);
}
