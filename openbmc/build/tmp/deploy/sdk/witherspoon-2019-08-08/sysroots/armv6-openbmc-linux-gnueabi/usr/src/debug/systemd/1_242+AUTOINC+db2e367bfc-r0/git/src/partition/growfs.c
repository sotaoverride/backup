...
static int resize_ext4(const char* path, int mountfd, int devfd,
                       uint64_t numblocks, uint64_t blocksize)
{
    ...
    if (ioctl(mountfd, EXT4_IOC_RESIZE_FS, &numblocks) != 0)
        return log_error_errno(
            errno, "Failed to resize \"%s\" to %" PRIu64 " blocks (ext4): %m",
            path, numblocks);

    return 0;
}

static int resize_btrfs(const char* path, int mountfd, int devfd,
                        uint64_t numblocks, uint64_t blocksize)
{
    ...
    {
        log_warning(
            "%s: resizing of btrfs volumes smaller than 256M is not supported",
            path);
        return -EOPNOTSUPP;
    }

    r = snprintf(args.name, sizeof(args.name), "%" PRIu64,
                 numblocks * blocksize);
    ...
    if (ioctl(mountfd, BTRFS_IOC_RESIZE, &args) != 0)
        return log_error_errno(
            errno, "Failed to resize \"%s\" to %" PRIu64 " blocks (btrfs): %m",
            path, numblocks);
...
static int resize_crypt_luks_device(dev_t devno, const char* fstype,
                                    dev_t main_devno)
{
    ...
    if (ioctl(main_devfd, BLKGETSIZE64, &size) != 0)
        return log_error_errno(
            errno, "Failed to query size of \"%s\" (before resize): %m",
            main_devpath);
    ...
    if (r < 0)
        return log_debug_errno(r, "Failed to load LUKS metadata for %s: %m",
                               devpath);
...
static int maybe_resize_slave_device(const char* mountpath, dev_t main_devno)
{
    ...
    if (r < 0)
        return log_error_errno(
            r, "Failed to determine underlying block device of \"%s\": %m",
            mountpath);

    log_debug("Underlying device %d:%d, main dev %d:%d, %s", major(devno),
              minor(devno), major(main_devno), minor(main_devno),
    ...
    if (r == -EUCLEAN)
        return log_warning_errno(
            r, "Cannot reliably determine probe \"%s\", refusing to proceed.",
            devpath);
    if (r < 0)
    ...
    log_debug("Don't know how to resize %s of type %s, ignoring", devpath,
              strnull(fstype));
    return 0;
}
...
int main(int argc, char* argv[])
{
    ...
    if (r < 0)
    {
        log_error_errno(r, "Failed to check if \"%s\" is a mount point: %m",
                        arg_target);
        return EXIT_FAILURE;
    }
    if (r == 0)
    ...
    if (r < 0)
    {
        log_error_errno(r, "Failed to determine block device of \"%s\": %m",
                        arg_target);
        return EXIT_FAILURE;
    }
    ...
    {
        log_error_errno(errno, "Failed to query block size of \"%s\": %m",
                        devpath);
        return EXIT_FAILURE;
    }
    ...
    {
        log_error_errno(errno, "Failed to stat file system \"%s\": %m",
                        arg_target);
        return EXIT_FAILURE;
    }
    ...
    log_info("Successfully resized \"%s\" to %s bytes (%" PRIu64
             " blocks of %d bytes).",
             arg_target, format_bytes(fb, sizeof fb, size), numblocks,
             blocksize);
    return EXIT_SUCCESS;
}
