...
static void g_unix_mount_finalize(GObject* object)
{
    GUnixMount* mount;
...
static void g_unix_mount_class_init(GUnixMountClass* klass)
{
...
static void g_unix_mount_init(GUnixMount* unix_mount)
{
}

GUnixMount* _g_unix_mount_new(GVolumeMonitor* volume_monitor,
                              GUnixMountEntry* mount_entry, GUnixVolume* volume)
{
    GUnixMount* mount;
...
void _g_unix_mount_unset_volume(GUnixMount* mount, GUnixVolume* volume)
{
    if (mount->volume == volume)
    {
        ...
        g_signal_emit_by_name(mount, "changed");
        if (mount->volume_monitor != NULL)
            g_signal_emit_by_name(mount->volume_monitor, "mount-changed",
                                  mount);
    }
}

static GFile* g_unix_mount_get_root(GMount* mount)
{
...
static GIcon* g_unix_mount_get_icon(GMount* mount)
{
    GUnixMount* unix_mount = G_UNIX_MOUNT(mount);
...
static GIcon* g_unix_mount_get_symbolic_icon(GMount* mount)
{
    GUnixMount* unix_mount = G_UNIX_MOUNT(mount);
...
static char* g_unix_mount_get_uuid(GMount* mount)
{
    return NULL;
}

static char* g_unix_mount_get_name(GMount* mount)
{
    GUnixMount* unix_mount = G_UNIX_MOUNT(mount);
...
static GDrive* g_unix_mount_get_drive(GMount* mount)
{
    GUnixMount* unix_mount = G_UNIX_MOUNT(mount);
...
static GVolume* g_unix_mount_get_volume(GMount* mount)
{
    GUnixMount* unix_mount = G_UNIX_MOUNT(mount);
...
static gboolean g_unix_mount_can_unmount(GMount* mount)
{
    return TRUE;
}

static gboolean g_unix_mount_can_eject(GMount* mount)
{
    GUnixMount* unix_mount = G_UNIX_MOUNT(mount);
    return unix_mount->can_eject;
}

static void eject_unmount_done(GObject* source, GAsyncResult* result,
                               gpointer user_data)
{
...
static gboolean eject_unmount_do_cb(gpointer user_data)
{
    GTask* task = user_data;
    GError* error = NULL;
...
static void eject_unmount_do(GMount* mount, GCancellable* cancellable,
                             ...
                             char** argv)
{
    ...
    if (unix_mount->volume_monitor != NULL)
        g_signal_emit_by_name(unix_mount->volume_monitor, "mount-pre-unmount",
                              mount);
...
static void g_unix_mount_unmount(GMount* mount, GMountUnmountFlags flags,
...
static gboolean g_unix_mount_unmount_finish(GMount* mount, GAsyncResult* result,
                                            GError** error)
{
    return g_task_propagate_boolean(G_TASK(result), error);
}

static void g_unix_mount_eject(GMount* mount, GMountUnmountFlags flags,
                               GCancellable* cancellable,
                               GAsyncReadyCallback callback, gpointer user_data)
{
    GUnixMount* unix_mount = G_UNIX_MOUNT(mount);
...
static gboolean g_unix_mount_eject_finish(GMount* mount, GAsyncResult* result,
                                          GError** error)
{
    return g_task_propagate_boolean(G_TASK(result), error);
}
