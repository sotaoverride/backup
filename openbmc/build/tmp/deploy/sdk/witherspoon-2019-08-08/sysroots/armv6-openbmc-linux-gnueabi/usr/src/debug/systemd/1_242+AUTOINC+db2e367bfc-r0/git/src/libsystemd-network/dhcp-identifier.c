...
#define HASH_KEY                                                               \
    SD_ID128_MAKE(80, 11, 8c, c2, fe, 4a, 03, ee, 3e, d6, 0c, 6f, 36, 39, 14,  \
                  09)
#define APPLICATION_ID                                                         \
    SD_ID128_MAKE(a5, 0a, d1, 12, bf, 60, 45, 77, a2, fb, 74, 1a, b1, 95, 5b,  \
                  03)
...
int dhcp_identifier_set_duid_llt(struct duid* duid, usec_t t,
                                 const uint8_t* addr, size_t addr_len,
                                 uint16_t arp_type, size_t* len)
{
    ...
    if (t < USEC_2000)
        ...
        time_from_2000y =
            (uint16_t)(((t - USEC_2000) / USEC_PER_SEC) & 0xffffffff);
    ...
    *len = sizeof(duid->type) + sizeof(duid->llt.htype) +
           sizeof(duid->llt.time) + addr_len;

    return 0;
}

int dhcp_identifier_set_duid_ll(struct duid* duid, const uint8_t* addr,
                                size_t addr_len, uint16_t arp_type, size_t* len)
{
    assert(duid);
    assert(len);
    assert(addr);
...
int dhcp_identifier_set_duid_en(struct duid* duid, size_t* len)
{
    ...
    if (r < 0)
    {
        ...
        machine_id = SD_ID128_MAKE(01, 02, 03, 04, 05, 06, 07, 08, 09, 0a, 0b,
                                   0c, 0d, 0e, 0f, 10);
#else
        return r;
#endif
...
int dhcp_identifier_set_iaid(int ifindex, const uint8_t* mac, size_t mac_len,
                             bool legacy_unstable_byteorder, void* _id)
{
