                        ...
                        __attribute__((__unused__)))
{
    ...
    jm = (struct jm_metadata*)blkid_probe_get_buffer(
        pr, off, sizeof(struct jm_metadata));
    if (!jm)
        return errno ? -errno : 1;

    if (memcmp(jm->signature, JM_SIGNATURE, sizeof(JM_SIGNATURE) - 1) != 0)
        return 1;
    if (blkid_probe_sprintf_version(pr, "%u.%u", jm->major_version,
                                    jm->minor_version) != 0)
        return 1;
...
const struct blkid_idinfo jmraid_idinfo = {.name = "jmicron_raid_member",
