                        ...
                        MkdirFlags flags, mkdir_func_t _mkdir)
{
    ...
    if ((flags & MKDIR_FOLLOW_SYMLINK) && S_ISLNK(st.st_mode))
    {
        ...
        if (r == 0)
            return mkdir_safe_internal(p, mode, uid, gid,
                                       flags & ~MKDIR_FOLLOW_SYMLINK, _mkdir);
    ...
    {
        log_full(flags & MKDIR_WARN_MODE ? LOG_WARNING : LOG_DEBUG,
                 "Directory \"%s\" already exists, but has mode %04o that is "
                 "too permissive (%04o was requested), refusing.",
        ...
        (gid != GID_INVALID && st.st_gid != gid))
    {
        ...
        log_full(flags & MKDIR_WARN_MODE ? LOG_WARNING : LOG_DEBUG,
                 "Directory \"%s\" already exists, but is owned by " UID_FMT
                 ":" GID_FMT " (%s:%s was requested), refusing.",
...
int mkdir_safe(const char* path, mode_t mode, uid_t uid, gid_t gid,
               MkdirFlags flags)
{
    return mkdir_safe_internal(path, mode, uid, gid, flags,
                               mkdir_errno_wrapper);
}
