...
#define G_VALUE_COLLECT_INIT(value, _value_type, var_args, flags, __error)     \
    ...
    {                                                                          \
        ...
        GTypeCValue g_vci_cvalues[G_VALUE_COLLECT_FORMAT_MAX_LENGTH] = {       \
            {                                                                  \
                0,                                                             \
            },                                                                 \
        };                                                                     \
        ...
        *(__error) = g_vci_vtab->collect_value(g_vci_val, g_vci_n_values,      \
                                               g_vci_cvalues, g_vci_flags);    \
    }                                                                          \
    G_STMT_END
...
#define G_VALUE_COLLECT(value, var_args, flags, __error)                       \
    ...
    {                                                                          \
        ...
        GTypeValueTable* g_vc_vtable =                                         \
            g_type_value_table_peek(g_vc_value_type);                          \
                                                                               \
        if (g_vc_vtable->value_free)                                           \
            g_vc_vtable->value_free(g_vc_value);                               \
        memset(g_vc_value->data, 0, sizeof(g_vc_value->data));                 \
                                                                               \
        G_VALUE_COLLECT_INIT(value, g_vc_value_type, var_args, flags,          \
                             __error);                                         \
    }                                                                          \
    G_STMT_END
...
#define G_VALUE_LCOPY(value, var_args, flags, __error)                         \
    ...
    {                                                                          \
        ...
        *(__error) = g_vl_vtable->lcopy_value(g_vl_value, g_vl_n_values,       \
                                              g_vl_cvalues, g_vl_flags);       \
    }                                                                          \
    G_STMT_END
