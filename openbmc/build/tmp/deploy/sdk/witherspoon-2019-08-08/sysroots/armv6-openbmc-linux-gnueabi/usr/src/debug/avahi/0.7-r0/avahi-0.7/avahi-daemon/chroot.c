    ...
    for (;;)
    {
        ...
        switch (command)
        {
            ...
            case AVAHI_CHROOT_GET_RESOLV_CONF:
            {
                ...
                if ((payload =
                         open(get_file_name_table[(int)command], O_RDONLY)) < 0)
                {
                    uint8_t c = AVAHI_CHROOT_FAILURE;

                    avahi_log_error(__FILE__ ": open() failed: %s",
                                    strerror(errno));
...
fail:
    ...
    avahi_log_debug(__FILE__ ": chroot() helper exiting with return value %i",
                    ret);
