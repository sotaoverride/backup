 ...
 *     {
 *	   fprintf(stderr, "An error in the DES/Triple-DES implementation occurred:
 *%s\n", error_msg); abort();
 *     }
 */
...
static int tripledes_set3keys(struct _tripledes_ctx*, const byte*, const byte*,
                              const byte*);
 ...
 */
static u32 leftkey_swap[16] = {0x00000000, 0x00000001, 0x00000100, 0x00000101,
 ...
 */
static byte encrypt_rotate_tab[16] = {1, 1, 2, 2, 2, 2, 2, 2,
                                      1, 2, 2, 2, 2, 2, 2, 1};
 ...
 *
 */
static void des_key_schedule(const byte* rawkey, u32* subkey)
{
    u32 left, right, work;
    int round;
 ...
 * to 'mode'.
 */
static int des_ecb_crypt(struct _des_ctx* ctx, const byte* from, byte* to,
                         int mode)
{
    u32 left, right, work;
    u32* keys;
    ...
    DES_ROUND(left, right, work, keys) DES_ROUND(right, left, work, keys)
        DES_ROUND(left, right, work, keys) DES_ROUND(right, left, work, keys)
            DES_ROUND(left, right, work, keys) DES_ROUND(
                right, left, work, keys) DES_ROUND(left, right, work, keys)
 ...
 * Does not check for weak keys.
 */
static int tripledes_set2keys(struct _tripledes_ctx* ctx, const byte* key1,
                              const byte* key2)
{
    int i;
...
static inline int tripledes_ecb_crypt(struct _tripledes_ctx* ctx,
                                      const byte* from, byte* to, int mode)
{
    u32* keys;
...
static inline void tripledes_amd64_ctr_enc(const void* keys, byte* out,
                                           const byte* in, byte* ctr)
{
#ifdef HAVE_COMPATIBLE_GCC_WIN64_PLATFORM_AS
...
static inline void tripledes_amd64_cbc_dec(const void* keys, byte* out,
                                           const byte* in, byte* iv)
{
#ifdef HAVE_COMPATIBLE_GCC_WIN64_PLATFORM_AS
...
static inline void tripledes_amd64_cfb_dec(const void* keys, byte* out,
                                           const byte* in, byte* iv)
{
#ifdef HAVE_COMPATIBLE_GCC_WIN64_PLATFORM_AS
 ...
 * (Encryption-Decryption-Encryption).
 */
static int tripledes_ecb_crypt(struct _tripledes_ctx* ctx, const byte* from,
                               byte* to, int mode)
{
    u32 left, right, work;
    u32* keys;
...
void _gcry_3des_ctr_enc(void* context, unsigned char* ctr, void* outbuf_arg,
...
void _gcry_3des_cbc_dec(void* context, unsigned char* iv, void* outbuf_arg,
                        const void* inbuf_arg, size_t nblocks)
{
    struct _tripledes_ctx* ctx = context;
    unsigned char* outbuf = outbuf_arg;
...
/* Bulk decryption of complete blocks in CFB mode.  This function is only
   intended for the bulk encryption feature of cipher.c. */
void _gcry_3des_cfb_dec(void* context, unsigned char* iv, void* outbuf_arg,
                        const void* inbuf_arg, size_t nblocks)
{
    struct _tripledes_ctx* ctx = context;
    unsigned char* outbuf = outbuf_arg;
...
static gcry_err_code_t bulk_selftest_setkey(void* context, const byte* __key,
                                            unsigned __keylen)
{
    static const unsigned char key[24] ATTR_ALIGNED_16 = {
        0x66, 0x9A, 0x00, 0x7F, 0xC7, 0x6A, 0x45, 0x9F, 0x98, 0xBA, 0xF9, 0x17,
        0xFE, 0xDF, 0x95, 0x22, 0x18, 0x2A, 0x39, 0x47, 0x5E, 0x6F, 0x75, 0x82};
...
static const char* selftest_ctr(void)
{
    ...
    return _gcry_selftest_helper_ctr("3DES", &bulk_selftest_setkey,
                                     &do_tripledes_encrypt, &_gcry_3des_ctr_enc,
...
static const char* selftest_cbc(void)
{
    ...
    return _gcry_selftest_helper_cbc("3DES", &bulk_selftest_setkey,
                                     &do_tripledes_encrypt, &_gcry_3des_cbc_dec,
                                     nblocks, blocksize, context_size);
}
...
static const char* selftest_cfb(void)
{
    ...
    return _gcry_selftest_helper_cfb("3DES", &bulk_selftest_setkey,
                                     &do_tripledes_encrypt, &_gcry_3des_cfb_dec,
                                     nblocks, blocksize, context_size);
}
...
static const char* selftest(void)
{
    ...
    {
        int i;
        byte key[8] = {0x55, 0x55, 0x55, 0x55, 0x55, 0x55, 0x55, 0x55};
        byte input[8] = {0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff};
        byte result[8] = {0x24, 0x6e, 0x9d, 0xb9, 0xc5, 0x50, 0x38, 0x1a};
    ...
    {
        int i;
        byte input[8] = {0xfe, 0xdc, 0xba, 0x98, 0x76, 0x54, 0x32, 0x10};
        byte key1[8] = {0x12, 0x34, 0x56, 0x78, 0x9a, 0xbc, 0xde, 0xf0};
        byte key2[8] = {0x11, 0x22, 0x33, 0x44, 0xff, 0xaa, 0xcc, 0xdd};
        byte result[8] = {0x7b, 0x38, 0x3b, 0x23, 0xa2, 0x7d, 0x26, 0xd3};
     ...
     */
    {
        static const struct
        {
            ...
            byte plain[8];
            byte cipher[8];
        } testdata[] = {{{0x01, 0x01, 0x01, 0x01, 0x01, 0x01, 0x01, 0x01,
        ...
        {
            tripledes_set3keys(des3, testdata[i].key, testdata[i].key + 8,
                               testdata[i].key + 16);
...
static gcry_err_code_t do_tripledes_set_extra_info(void* context, int what,
                                                   const void* buffer,
                                                   size_t buflen)
{
    struct _tripledes_ctx* ctx = (struct _tripledes_ctx*)context;
...
static gcry_err_code_t do_des_setkey(void* context, const byte* key,
                                     unsigned keylen)
{
...
static unsigned int do_des_encrypt(void* context, byte* outbuf,
                                   const byte* inbuf)
{
    struct _des_ctx* ctx = (struct _des_ctx*)context;
...
static unsigned int do_des_decrypt(void* context, byte* outbuf,
                                   const byte* inbuf)
{
    struct _des_ctx* ctx = (struct _des_ctx*)context;
...
static gpg_err_code_t selftest_fips(int extended, selftest_report_func_t report)
{
    const char* what;
    const char* errtxt;
...
gcry_cipher_spec_t _gcry_cipher_spec_des = {GCRY_CIPHER_DES,
                                            {0, 0},
                                            "DES",
                                            NULL,
                                            NULL,
...
gcry_cipher_spec_t _gcry_cipher_spec_tripledes = {GCRY_CIPHER_3DES,
                                                  {0, 1},
                                                  "3DES",
                                                  NULL,
                                                  oids_tripledes,
