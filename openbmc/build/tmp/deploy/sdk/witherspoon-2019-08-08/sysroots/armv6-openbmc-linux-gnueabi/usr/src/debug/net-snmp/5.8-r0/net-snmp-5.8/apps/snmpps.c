...
int command_args = 0, command_path = 0;
int topsort = 'c';
char* progname;
...
netsnmp_variable_list* collect_procs(netsnmp_session* ss, netsnmp_pdu* pdu,
                                     oid* base, size_t base_length)
{
    ...
    while (running)
    {
        ...
        if (snmp_oid_compare(
                ...
                SNMP_MIN(base_length, response->variables->name_length), base,
                base_length) != 0)
            running = 0;
        ...
        else
        {
            ...
            *vlpp = response->variables;
            (*vlpp)->next_variable =
                NULL; /* shouldn't be any, but just in case */
...
int collect_perf(netsnmp_session* ss, struct hrSWRunTable** fproc)
{
    netsnmp_pdu* pdu;
    netsnmp_pdu* response;
    ...
    pdu = snmp_pdu_create(SNMP_MSG_GETNEXT);
    base_length = add(pdu, "HOST-RESOURCES-MIB:hrSWRunIndex", NULL, 0);
    ...
    while (vlp)
    {
        ...
        add(pdu, "HOST-RESOURCES-MIB:hrSWRunName", &vlp->name[base_length],
            vlp->name_length - base_length);
        add(pdu, "HOST-RESOURCES-MIB:hrSWRunID", &vlp->name[base_length],
            vlp->name_length - base_length);
        add(pdu, "HOST-RESOURCES-MIB:hrSWRunPath", &vlp->name[base_length],
            vlp->name_length - base_length);
        add(pdu, "HOST-RESOURCES-MIB:hrSWRunParameters",
            &vlp->name[base_length], vlp->name_length - base_length);
        add(pdu, "HOST-RESOURCES-MIB:hrSWRunType", &vlp->name[base_length],
            vlp->name_length - base_length);
        add(pdu, "HOST-RESOURCES-MIB:hrSWRunStatus", &vlp->name[base_length],
            vlp->name_length - base_length);
        add(pdu, "HOST-RESOURCES-MIB:hrSWRunPerfCPU", &vlp->name[base_length],
            vlp->name_length - base_length);
        add(pdu, "HOST-RESOURCES-MIB:hrSWRunPerfMem", &vlp->name[base_length],
            vlp->name_length - base_length);
    ...
    next:
        if (proc.hrSWRunName)
            free(proc.hrSWRunName);
        if (proc.hrSWRunPath)
            free(proc.hrSWRunPath);
        if (proc.hrSWRunParameters)
            free(proc.hrSWRunParameters);
        snmp_free_pdu(response);
        vlp2 = vlp;
        vlp = vlp->next_variable;
        free(vlp2);
...
char* format_centisec(char* buf, size_t len, unsigned long csec)
{
    ...
    if (day)
        snprintf(buf, len, "%ldd%02ld:%02ld:%02ld.%02lu", day, hour, min, sec,
                 csec);
    else if (hour)
...
int snmpps(int argc, char* argv[])
{
    ...
    while (pinx < count)
    {
        ...
        printf("%7lu %4s %6s %10s %11.11s %s %s\n", proc->hrSWRunIndex, hr_type,
               hr_status, format_humanmem(b1, sizeof b1, proc->hrSWRunPerfMem),
               format_centisec(b2, sizeof b2, proc->hrSWRunPerfCPU),
               command_path && proc->hrSWRunPath[0] ? proc->hrSWRunPath
                                                    : proc->hrSWRunName,
...
int snmptop(int argc, char** argv)
{
    ...
    while ((ch = getch()) != 'q')
    {
        ...
        while (oinx < ocount && ninx < ncount)
        {
            ...
            {
                nproc[ninx].hrSWRunPerfCPUInc =
                    nproc[ninx].hrSWRunPerfCPU - oproc[oinx].hrSWRunPerfCPU;
                ninx++;
                oinx++;
            }
        ...
        if (has_cpu)
        {
            ...
            oldCpu = newCpu;
            sumCpu = deltaCpu.user + deltaCpu.nice + deltaCpu.system +
                     deltaCpu.idle + deltaCpu.wait + deltaCpu.kernel +
                     deltaCpu.steal + deltaCpu.intr + deltaCpu.softintr +
                     deltaCpu.guest + deltaCpu.guestnice;

            printw("\nCPU%%: %4.1fUs %4.1fSy %4.1fId %3.1fWa %3.1fNi %3.1fKe "
                   "%3.1fHi %3.1fSi %3.1fSt %3.1fGu %3.1fGN",
        ...
        printw("\n%7s %4s %6s %10s %11s %5s %-10s", "Index", "Type", "Status",
               "Memory", "Total CPU", "%CPU", "Command");
        line++;
        ninx = 0;
        while (line < LINES && ninx < ncount)
        {
            ...
            printw("\n%7lu %4s %6s %10s %11s %5.1f %s %s", proc->hrSWRunIndex,
                   hr_type, hr_status,
...
int main(int argc, char** argv)
{
    ...
    if (strcmp(progname, "snmpps") == 0)
        return snmpps(argc, argv);
    if (strcmp(progname, "snmptop") == 0)
        return snmptop(argc, argv);
