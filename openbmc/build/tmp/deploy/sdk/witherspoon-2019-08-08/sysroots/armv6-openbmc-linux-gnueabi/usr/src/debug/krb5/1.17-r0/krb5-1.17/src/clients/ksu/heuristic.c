...
krb5_error_code get_all_princ_from_file(fp, plist) FILE* fp;
char*** plist;
{
    ...
    while (line)
    {
        ...
        {
            chunk_count++;
            if (!(temp_list = (char**)realloc(temp_list, chunk_count * CHUNK *
                                                             sizeof(char*))))
            {
                return ENOMEM;
            }
...
krb5_error_code list_union(list1, list2, combined_list) char** list1;
char** list2;
char*** combined_list;
{
...
krb5_error_code filter(fp, cmd, k5users_list, k5users_filt_list) FILE* fp;
char* cmd;
char** k5users_list;
...
krb5_error_code get_authorized_princ_names(luser, cmd, princ_list) const
    char* luser;
char* cmd;
char*** princ_list;
{
    ...
    *princ_list = NULL;
    ...
    if (!k5login_flag)
    {
        ...
        {
            close_time(1 /*k5users_flag*/, (FILE*)0 /*users_fp*/, k5login_flag,
                       login_fp);
            return 0;
        }
    }
...
static void close_time(k5users_flag, users_fp, k5login_flag,
                       login_fp) int k5users_flag;
FILE* users_fp;
int k5login_flag;
FILE* login_fp;
{
...
static krb5_boolean find_str_in_list(list, elm) char** list;
char* elm;
{
...
krb5_error_code get_closest_principal(context, plist, client,
                                      found) krb5_context context;
char** plist;
...
krb5_boolean* found;
{
    ...
    if (ks_ccache_is_initialized(context, cc))
    {
        ...
        {
            retval =
                ksu_tgtname(context, krb5_princ_realm(context, client),
                            krb5_princ_realm(context, client), &kdc_server);
            if (retval)
                return retval;
...
krb5_error_code find_ticket(context, cc, client, server,
                            found) krb5_context context;
krb5_ccache cc;
krb5_principal client;
...
krb5_boolean* found;
{
    ...
    *found = FALSE;
    ...
    retval = krb5_cc_retrieve_cred(
        context, cc, KRB5_TC_MATCH_SRV_NAMEONLY | KRB5_TC_SUPPORTED_KTYPES,
        &tgtq, &tgt);
...
krb5_error_code find_princ_in_list(context, princ, plist,
                                   found) krb5_context context;
...
krb5_error_code get_best_princ_for_target(context, source_uid, target_uid,
                                          ...
                                          options, cmd, hostname, client,
                                          path_out) krb5_context context;
uid_t source_uid;
uid_t target_uid;
char* source_user;
char* target_user;
...
int* path_out;
{
    ...
    *path_out = 0;
    ...
    for (i = 0; i < count; i++)
    {
        ...
        {
            retval =
                find_princ_in_list(context, princ_trials[i].p, aplist, &found);
            if (retval)
                return retval;
            ...
            if (found == TRUE)
            {
                ...
                retval = find_either_ticket(
                    context, cc_source, princ_trials[i].p, end_server, &found);
                if (retval)
                    return retval;
                if (found == TRUE)
                {
                    *client = princ_trials[i].p;
                    if (auth_debug)
                        printf("GET_best_princ_for_target: via ticket file, "
                               "choice #%d\n",
                               i);
                    return 0;
                }
    ...
    while (aplist[i])
    {
        ...
        retval = find_either_ticket(context, cc_source, temp_client, end_server,
                                    &found);
        if (retval)
            return retval;
        ...
        {
            if (auth_debug)
                printf("GET_best_princ_for_target: via ticket file, choice: "
                       "any ok ticket \n");
            *client = temp_client;
            return 0;
        }
    ...
    for (i = 0; i < count; i++)
    {
        if (princ_trials[i].found == TRUE)
        {
            *client = princ_trials[i].p;
            ...
            if (auth_debug)
                printf("GET_best_princ_for_target: via prompt passwd list "
                       "choice #%d \n",
                       i);
            return 0;
        }
    ...
    for (i = 0; i < count; i++)
    {
        ...
        {
            retval =
                krb5_copy_principal(context, princ_trials[i].p, &temp_client);
            if (retval)
                return retval;
            ...
            retval =
                get_closest_principal(context, aplist, &temp_client, &found);
            if (retval)
                return retval;
            ...
            if (found == TRUE)
            {
                *client = temp_client;
                if (auth_debug)
                    printf("GET_best_princ_for_target: via prompt passwd list "
                           "choice: approximation of princ in trials # %d \n",
                           i);
                return 0;
            }
            krb5_free_principal(context, temp_client);
    ...
    if (auth_debug)
        printf("GET_best_princ_for_target: out of luck, can't get appropriate "
               "default principal\n");
