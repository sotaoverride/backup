...
static int service_update(Service* service, const char* unit, size_t argc,
                          char** argv, const char* user, uid_t uid)
{
...
static int service_new(Service** servicep, Manager* manager, const char* name,
                       CRBNode** slot_by_name, CRBNode* parent_by_name,
                       const char* unit, size_t argc, char** argv,
                       const char* user, uid_t uid)
{
    ...
    slot = c_rbtree_find_slot(&manager->services, service_compare, service->id,
                              &parent);
    assert(slot);
    c_rbtree_add(&manager->services, parent, slot, &service->rb);
    c_rbtree_add(&manager->services_by_name, parent_by_name, slot_by_name,
                 &service->rb_by_name);
...
static Manager* manager_free(Manager* manager)
{
    ...
    c_rbtree_for_each_entry_safe_postorder_unlink(
        service, safe, &manager->services, rb) service_free(service);
...
static int manager_on_sighup(sd_event_source* s,
...
static int manager_on_dirwatch(sd_event_source* s, int fd, uint32_t revents,
                               void* userdata)
{
    Manager* manager = userdata;
    int r;
...
static int manager_new(Manager** managerp)
{
    ...
    r = sd_event_add_signal(manager->event, NULL, SIGHUP, manager_on_sighup,
                            manager);
    if (r < 0)
        return error_origin(r);
...
static noreturn void manager_run_child(Manager* manager, int fd_log,
                                       int fd_controller)
{
    sd_id128_t machine_id;
    char str_log[C_DECIMAL_MAX(int) + 1],
        str_controller[C_DECIMAL_MAX(int) + 1], str_machine_id[33],
    ...
    const char* const argv[] = {
        ...
        main_arg_audit
            ? "--audit"
            : NULL, /* note that this needs to be the last argument to work */
        NULL,
    };
    int r;
    ...
    r = snprintf(str_max_bytes, sizeof(str_max_bytes), "%" PRIu64,
                 manager->max_bytes);
    assert(r < (ssize_t)sizeof(str_max_bytes));

    r = snprintf(str_max_fds, sizeof(str_max_fds), "%" PRIu64,
                 manager->max_fds);
    assert(r < (ssize_t)sizeof(str_max_fds));

    r = snprintf(str_max_matches, sizeof(str_max_matches), "%" PRIu64,
                 manager->max_matches);
...
static int manager_on_child_exit(sd_event_source* source, const siginfo_t* si,
                                 void* userdata)
{
    fprintf(stderr, "Caught SIGCHLD of broker\n");

    return sd_event_exit(sd_event_source_get_event(source),
                         (si->si_code == CLD_EXITED) ? si->si_status
                                                     : EXIT_FAILURE);
}
...
static int manager_fork(Manager* manager, int fd_controller)
{
    ...
    r = sd_event_add_child(manager->event, NULL, pid, WEXITED,
                           manager_on_child_exit, manager);
    if (r < 0)
        return error_origin(-errno);
...
static int manager_start_unit_handler(sd_bus_message* message, void* userdata,
                                      sd_bus_error* errorp)
{
    ...
    if (strcmp(error->name,
               "org.freedesktop.systemd1.TransactionIsDestructive") != 0)
    {
        /*
    ...
    r = sd_bus_call_method(service->manager->bus_controller, NULL, object_path,
                           "org.bus1.DBus.Name", "Reset", NULL, NULL, "");
    if (r < 0)
        return error_origin(r);
...
static int manager_start_unit(Manager* manager, Service* service)
{
    ...
    r = sd_bus_message_new_method_call(
        manager->bus_regular, &method_call, "org.freedesktop.systemd1",
    ...
    r = sd_bus_call_async(manager->bus_regular, &service->slot, method_call,
                          manager_start_unit_handler, service, -1);
    if (r < 0)
        return error_origin(r);
...
static int manager_start_transient_unit(Manager* manager, Service* service)
{
    ...
    r = asprintf(&unit, "dbus-%s-%s@%" PRIu64 ".service", unique_name,
...

        {
...

            {
...

                {
                    ...
                    {
                        r = sd_bus_message_append(method_call, "s",
                                                  service->argv[0]);
                        if (r < 0)
                            return error_origin(r);

                        r = sd_bus_message_open_container(method_call, 'a',
                                                          "s");
                        if (r < 0)
                            return error_origin(r);
...
static int manager_on_name_activate(Manager* manager, sd_bus_message* m,
                                    const char* id)
{
    Service* service;
    int r;

    service = c_rbtree_find_entry(&manager->services, service_compare, id,
                                  Service, rb);
    if (!service)
    {
...
static int manager_set_environment_handler(sd_bus_message* message,
                                           void* userdata, sd_bus_error* errorp)
{
    const sd_bus_error* error;
...
static int manager_on_set_activation_environment(Manager* manager,
                                                 sd_bus_message* m)
{
    _c_cleanup_(sd_bus_message_unrefp) sd_bus_message* method_call = NULL;
    int r;
    ...
    r = sd_bus_call_async(manager->bus_regular, NULL, method_call,
                          manager_set_environment_handler, NULL, -1);
    if (r < 0)
        return error_origin(r);
...
static int manager_on_message(sd_bus_message* m, void* userdata,
                              sd_bus_error* error)
{
    Manager* manager = userdata;
...
static int manager_ini_reader_parse_file(CIniGroup** groupp, const char* path)
{
    ...
    c_ini_reader_set_mode(reader, C_INI_MODE_EXTENDED_WHITESPACE |
    ...
    {
        fprintf(stderr,
                "Missing 'D-Bus Service' section in service file '%s'\n", path);
        return MANAGER_E_INVALID_SERVICE_FILE;
    }
                                     ...
                                     NSSCache* nss_cache)
{
    ...
    _c_cleanup_(service_freep) Service* service = NULL;
    CIniEntry *name_entry = NULL, *unit_entry = NULL, *exec_entry = NULL,
              *user_entry = NULL;
    ...
    slot = c_rbtree_find_slot(&manager->services_by_name,
                              service_compare_by_name, name, &parent);
    if (slot)
    {
        r = service_new(&service, manager, name, slot, parent, unit, argc, argv,
                        user, uid);
        if (r)
            return error_trace(r);
    }
    else
    {
        ...
        {
            fprintf(stderr,
                    "Ignoring duplicate name '%s' in service file '%s'\n", name,
                    path);
            return MANAGER_E_INVALID_SERVICE_FILE;
        }
    }
...
static int manager_load_service_dir(Manager* manager, const char* dirpath,
                                    NSSCache* nss_cache)
{
    ...
    for (errno = 0, de = readdir(dir); de; errno = 0, de = readdir(dir))
    {
        if (de->d_name[0] == '.')
            continue;
...
static int manager_add_services(Manager* manager)
{
    ...
    c_rbtree_for_each_entry(service, &manager->services, rb)
    {
        ...
        r = sd_bus_call_method(manager->bus_controller, NULL,
                               "/org/bus1/DBus/Broker", "org.bus1.DBus.Broker",
                               "AddName", NULL, NULL, "osu", object_path,
...
static int manager_remove_services(Manager* manager)
{
    ...
    c_rbtree_for_each_entry_safe(service, service_safe, &manager->services, rb)
    {
        ...
        r = sd_bus_call_method(manager->bus_controller, NULL, object_path,
                               "org.bus1.DBus.Name", "Release", NULL, NULL, "");
        if (r < 0)
            return error_origin(r);
...
static int manager_load_standard_session_services(Manager* manager,
                                                  NSSCache* nss_cache)
{
     ...
     */
    {
        ...
        else
        {
            ...
            {
                r = asprintf(&data_home_dir, "%s/.local/share/%s",
                             passwd->pw_dir, suffix);
                if (r < 0)
                    return error_origin(-ENOMEM);
            }
        }
     ...
     */
    {
        ...
        data_dirs = getenv("XDG_DATA_DIRS") ?: "/usr/local/share:/usr/share";
...
static int manager_load_standard_system_services(Manager* manager,
                                                 NSSCache* nss_cache)
{
...
static int manager_load_services(Manager* manager, ConfigRoot* config,
                                 NSSCache* nss_cache)
{
    ConfigNode* cnode;
    int r;
    ...
    c_list_for_each_entry(cnode, &config->node_list, root_link)
    {
        switch (cnode->type)
        {
            ...
            case CONFIG_NODE_SERVICEDIR:
                r = manager_load_service_dir(manager, cnode->servicedir.path,
                                             nss_cache);
                if (r)
                    return error_trace(r);
...
static int manager_parse_config(Manager* manager, ConfigRoot** rootp,
                                NSSCache* nss_cache)
{
    _c_cleanup_(config_parser_deinit) ConfigParser parser =
        CONFIG_PARSER_NULL(parser);
    _c_cleanup_(dirwatch_freep) Dirwatch* dirwatch = NULL;
    uint64_t max_match_rules_per_connection =
        main_max_match_rules_per_connection;
    ...
    r = sd_event_add_io(manager->event, &manager->dirwatch_src,
                        dirwatch_get_fd(manager->dirwatch), EPOLLIN,
                        manager_on_dirwatch, manager);
    if (r)
        return error_origin(r);
    ...
    manager->max_bytes =
        util_umul64_saturating(max_connections_per_user, max_outgoing_bytes);
    manager->max_fds =
        util_umul64_saturating(max_connections_per_user, max_outgoing_unix_fds);
    manager->max_matches = util_umul64_saturating(
        max_connections_per_user, max_match_rules_per_connection);

    return 0;
}

static int manager_load_policy(Manager* manager, ConfigRoot* root,
                               Policy* policy)
{
    int r;
...
static int manager_add_listener(Manager* manager, Policy* policy,
                                uint32_t* system_console_users,
                                size_t n_system_console_users)
{
    _c_cleanup_(sd_bus_message_unrefp) sd_bus_message* m = NULL;
    int r;

    r = sd_bus_message_new_method_call(manager->bus_controller, &m, NULL,
                                       "/org/bus1/DBus/Broker",
                                       "org.bus1.DBus.Broker", "AddListener");
    if (r < 0)
        return error_origin(r);

    r = sd_bus_message_append(m, "oh", "/org/bus1/DBus/Listener/0",
                              manager->fd_listen);
    if (r < 0)
        return error_origin(r);
                              ...
                              size_t n_system_console_users)
{
    ...
    r = sd_bus_message_new_method_call(manager->bus_controller, &m, NULL,
                                       ...
                                       "org.bus1.DBus.Listener", "SetPolicy");
    if (r < 0)
        return error_origin(r);
...
static int manager_reload_config(Manager* manager)
{
    ...
    c_rbtree_for_each_entry(service, &manager->services, rb) service->state =
        SERVICE_STATE_DEFUNCT;
    ...
    r = nss_cache_resolve_system_console_users(
        &nss_cache, &system_console_users, &n_system_console_users);
    if (r)
        return error_trace(r);
    ...
    switch (policy.apparmor_mode)
    {
        case CONFIG_APPARMOR_ENABLED:
        {
            ...
            if (enabled)
                fprintf(stderr,
                        "AppArmor enabled, but not supported. Ignoring.\n");
    ...
    r = manager_set_policy(manager, &policy, system_console_users,
...
static int bus_method_reload_config(sd_bus_message* message, void* userdata,
                                    sd_bus_error* error)
{
    Manager* manager = userdata;
    int r;
    ...
    if (r)
    {
        if (r == MANAGER_E_INVALID_CONFIG)
            return sd_bus_reply_method_errorf(
                message, "org.bus1.DBus.Controller.Error.InvalidConfig",
                "Invalid configuration. Reload ignored.");
        else
            return error_fold(r);
    }
...
static int manager_run(Manager* manager)
{
    ...
    switch (policy.apparmor_mode)
    {
        case CONFIG_APPARMOR_ENABLED:
        {
            ...
            /* XXX: once the broker supports AppArmor, set this to DISABLED if
             * and only if it is disabled in the kernel. */
            policy.apparmor_mode = CONFIG_APPARMOR_DISABLED;
            break;
        }
        case CONFIG_APPARMOR_REQUIRED:
            ...
            /* XXX: once the broker supports AppArmor, set this to enabled if
             * and only if it is enabled in the kernel, and exit the launcher
             * otherwise. */
            return 0;
    }
    ...
    r = socketpair(PF_UNIX, SOCK_STREAM | SOCK_CLOEXEC | SOCK_NONBLOCK, 0,
                   controller);
    if (r < 0)
        return error_origin(-errno);
    ...
    r = sd_bus_add_object_vtable(
        manager->bus_controller, NULL, "/org/bus1/DBus/Controller",
        "org.bus1.DBus.Controller", manager_vtable, manager);
    if (r < 0)
        return error_origin(r);

    r = sd_bus_add_filter(manager->bus_controller, NULL, manager_on_message,
                          manager);
    if (r < 0)
        return error_origin(r);
    ...
    r = manager_add_listener(manager, &policy, system_console_users,
                             n_system_console_users);
    if (r)
        return error_trace(r);
    ...
    r = sd_bus_attach_event(manager->bus_controller, manager->event,
                            SD_EVENT_PRIORITY_NORMAL);
    if (r < 0)
        return error_origin(r);

    r = sd_bus_attach_event(manager->bus_regular, manager->event,
                            SD_EVENT_PRIORITY_NORMAL);
    if (r < 0)
        return error_origin(r);
...
static int open_log(void)
{
    ...
    r = connect(fd, (struct sockaddr*)&address,
