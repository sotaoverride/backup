...
netsnmp_feature_require(iquery) netsnmp_feature_require(iquery_pdu_session)
    netsnmp_feature_require(table_tdata) netsnmp_feature_require(date_n_time)
...
void init_schedTable(void)
{
    ...
    reg = netsnmp_create_handler_registration(
        "schedTable", schedTable_handler, schedTable_oid, schedTable_oid_len,
        HANDLER_CAN_RWRITE);
...
int schedTable_handler(netsnmp_mib_handler* handler,
                       ...
                       netsnmp_request_info* requests)
{
    ...
    DEBUGMSGTL(
        ("disman:schedule:mib", "Schedule handler (%d)\n", reqinfo->mode));
    switch (reqinfo->mode)
    {
            /*
        ...
        case MODE_GET:
            for (request = requests; request; request = request->next)
            {
                ...
                entry = (struct schedTable_entry*)netsnmp_tdata_extract_entry(
                    request);
                ...
                switch (tinfo->colnum)
                {
                    case COLUMN_SCHEDDESCR:
                        snmp_set_var_typed_value(
                            ...
                            entry->schedDescr, strlen(entry->schedDescr));
                        break;
                    ...
                    case COLUMN_SCHEDWEEKDAY:
                        snmp_set_var_typed_value(
                            request->requestvb, ASN_OCTET_STR,
                            &entry->schedWeekDay, sizeof(entry->schedWeekDay));
                        break;
                    case COLUMN_SCHEDMONTH:
                        snmp_set_var_typed_value(
                            request->requestvb, ASN_OCTET_STR,
                            entry->schedMonth, sizeof(entry->schedMonth));
                        break;
                    case COLUMN_SCHEDDAY:
                        snmp_set_var_typed_value(request->requestvb,
                                                 ASN_OCTET_STR, entry->schedDay,
                                                 sizeof(entry->schedDay));
                        break;
                    case COLUMN_SCHEDHOUR:
                        snmp_set_var_typed_value(
                            request->requestvb, ASN_OCTET_STR, entry->schedHour,
                            sizeof(entry->schedHour));
                        break;
                    case COLUMN_SCHEDMINUTE:
                        snmp_set_var_typed_value(
                            request->requestvb, ASN_OCTET_STR,
                            entry->schedMinute, sizeof(entry->schedMinute));
                        break;
                    ...
                    case COLUMN_SCHEDVALUE:
                        snmp_set_var_typed_integer(
                            request->requestvb, ASN_INTEGER, entry->schedValue);
                        break;
                    case COLUMN_SCHEDTYPE:
                        snmp_set_var_typed_integer(
                            request->requestvb, ASN_INTEGER, entry->schedType);
                        break;
                    case COLUMN_SCHEDADMINSTATUS:
                        ret = (entry->flags & SCHEDULE_FLAG_ENABLED) ? TV_TRUE
                                                                     : TV_FALSE;
                        snmp_set_var_typed_integer(request->requestvb,
                                                   ASN_INTEGER, ret);
                        break;
                    ...
                    case COLUMN_SCHEDROWSTATUS:
                        ret = (entry->flags & SCHEDULE_FLAG_ACTIVE) ? TV_TRUE
                                                                    : TV_FALSE;
                        snmp_set_var_typed_integer(request->requestvb,
                                                   ASN_INTEGER, ret);
                        break;
            ...
            for (request = requests; request; request = request->next)
            {
                ...
                switch (tinfo->colnum)
                {
                    ...
                    case COLUMN_SCHEDHOUR:
                        ...
                        break;
                    case COLUMN_SCHEDMINUTE:
                        memset(entry->schedMinute, 0,
                               sizeof(entry->schedMinute));
                        memcpy(entry->schedMinute,
                               request->requestvb->val.string,
                               request->requestvb->val_len);
                        recalculate = 1;
                        break;
                    case COLUMN_SCHEDCONTEXTNAME:
                        memset(entry->schedContextName, 0,
                               sizeof(entry->schedContextName));
                        memcpy(entry->schedContextName,
                               request->requestvb->val.string,
                               request->requestvb->val_len);
                        break;
                    case COLUMN_SCHEDVARIABLE:
                        memset(entry->schedVariable, 0,
                               sizeof(entry->schedVariable));
                    ...
                    case COLUMN_SCHEDROWSTATUS:
                        switch (*request->requestvb->val.integer)
                        {
                            ...
                            case RS_CREATEANDGO:
                                entry->flags |= SCHEDULE_FLAG_ACTIVE;
                                entry->flags |= SCHEDULE_FLAG_VALID;
                                entry->session = netsnmp_iquery_pdu_session(
                                    reqinfo->asp->pdu);
                                break;
                            case RS_CREATEANDWAIT:
                                entry->flags |= SCHEDULE_FLAG_VALID;
