...
extern LZMA_API(void) lzma_index_hash_end(lzma_index_hash* index_hash,
                                          const lzma_allocator* allocator)
{
...
static lzma_ret hash_append(lzma_index_hash_info* info, lzma_vli unpadded_size,
                            lzma_vli uncompressed_size)
{
    ...
    lzma_check_update(&info->check, LZMA_CHECK_BEST, (const uint8_t*)(sizes),
                      sizeof(sizes));
                           ...
                           lzma_vli uncompressed_size)
{
    ...
    if (index_hash->blocks.blocks_size > LZMA_VLI_MAX ||
        ...
        index_stream_size(index_hash->blocks.blocks_size,
                          ...
                          index_hash->blocks.index_list_size) > LZMA_VLI_MAX)
                           ...
                           size_t* in_pos, size_t in_size)
{
    ...
    while (*in_pos < in_size)
        switch (index_hash->sequence)
        {
            ...
            case SEQ_COUNT:
            {
                ret = lzma_vli_decode(&index_hash->remaining, &index_hash->pos,
                                      in, in_pos, in_size);
            ...
            case SEQ_UNCOMPRESSED:
            {
                ...
                ret = lzma_vli_decode(size, &index_hash->pos, in, in_pos,
                                      in_size);
                if (ret != LZMA_STREAM_END)
                    goto out;
            ...
            case SEQ_PADDING_INIT:
                index_hash->pos =
                    (LZMA_VLI_C(4) -
                     index_size_unpadded(index_hash->records.count,
                                         index_hash->records.index_list_size)) &
                    3;
            ...
            case SEQ_PADDING:
                ...
                index_hash->crc32 = lzma_crc32(
                    in + in_start, *in_pos - in_start, index_hash->crc32);
            ...
            case SEQ_CRC32:
                do
                {
                    ...
                    if (((index_hash->crc32 >> (index_hash->pos * 8)) & 0xFF) !=
                        in[(*in_pos)++])
                        return LZMA_DATA_ERROR;
...
out:
    // Update the CRC32,
    index_hash->crc32 =
        lzma_crc32(in + in_start, *in_pos - in_start, index_hash->crc32);
