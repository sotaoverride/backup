...
static errcode_t inode_read_blk64(io_channel channel, unsigned long long block,
                                  int count, void* data);
static errcode_t inode_write_blk64(io_channel channel, unsigned long long block,
                                   int count, const void* data);
                                  ...
                                  struct ext2_inode* inode, char** name)
{
    ...
    if ((retval = ext2fs_get_mem(sizeof(struct inode_private_data), &data)))
        return retval;
...
errcode_t ext2fs_inode_io_intern(ext2_filsys fs, ext2_ino_t ino, char** name)
{
...
static errcode_t inode_open(const char* name, int flags, io_channel* channel)
{
    ...
    for (data = top_intern, prev = NULL; data; prev = data, data = data->next)
    ...
    retval =
        ext2fs_file_open2(data->fs, data->ino,
                          (data->flags & CHANNEL_HAS_INODE) ? &data->inode : 0,
                          open_flags, &data->file);
    if (retval)
        goto cleanup;
                                  ...
                                  int count, void* buf)
{
    ...
    if ((retval = ext2fs_file_lseek(data->file, block * channel->block_size,
                                    EXT2_SEEK_SET, 0)))
        return retval;
                                  ...
                                  int size, const void* buf)
{
    ...
    if ((retval = ext2fs_file_lseek(data->file, offset, EXT2_SEEK_SET, 0)))
        return retval;
