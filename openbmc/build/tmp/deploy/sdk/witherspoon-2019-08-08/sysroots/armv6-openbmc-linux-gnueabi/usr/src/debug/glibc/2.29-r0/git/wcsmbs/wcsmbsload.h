...
extern void
    __wcsmbs_load_conv(struct __locale_data* new_category) attribute_hidden;

/* Clone the current `__wcsmbs_load_conv' value.  */
extern void __wcsmbs_clone_conv(struct gconv_fcts* copy) attribute_hidden;

/* Find the conversion functions for converting to and from NAME.  */
extern int __wcsmbs_named_conv(struct gconv_fcts* copy,
...
extern struct __gconv_step* __wcsmbs_getfct(const char* to, const char* from,
                                            size_t* nstepsp) attribute_hidden;
