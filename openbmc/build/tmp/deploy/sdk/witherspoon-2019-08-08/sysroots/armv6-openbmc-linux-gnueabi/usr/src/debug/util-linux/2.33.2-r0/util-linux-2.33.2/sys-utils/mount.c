...
static void __attribute__((__noreturn__)) print_version(void)
{
    ...
    printf(_("%s from %s (libmount %s"), program_invocation_short_name,
           PACKAGE_STRING, ver);
    p = features;
    while (p && *p)
    {
    ...
    while (mnt_context_next_mount(cxt, itr, &fs, &mntrc, &ignored) == 0)
    {
        ...
        else
        {
            if (mk_exit_code(cxt, mntrc) == MNT_EX_SUCCESS)
            {
                 ...
                 * (e.g. nofail option) */
                if (mnt_context_get_status(cxt) && mnt_context_is_verbose(cxt))
...
static void success_message(struct libmnt_context* cxt)
{
    ...
    if (mnt_context_helper_executed(cxt) || mnt_context_get_status(cxt) != 1)
        return;
            ...
            security_get_initial_context("file", &def) == 0)
        {
            ...
            if (!selinux_file_context_cmp(raw, def))
                printf(_("mount: %s does not contain SELinux labels.\n"
                         "       You just mounted an file system that supports "
                         "labels which does not\n"
                         "       contain labels, onto an SELinux box. It is "
                         "likely that confined\n"
                         "       applications will generate AVC messages and "
                         "not be allowed access to\n"
                         "       this file system.  For more details see "
                         "restorecon(8) and mount(8).\n"),
                       tgt);
        }
        freecon(raw);
        freecon(def);
...
static void __attribute__((__noreturn__)) usage(void)
{
    ...
    fprintf(
        out,
        _(" -a, --all               mount all filesystems mentioned in fstab\n"
    ...
    fprintf(
        out,
        _(" -i, --internal-only     don't call the mount.<type> helpers\n"));
    ...
    fprintf(out,
            _(" -o, --options <list>    comma-separated list of mount options\n"
              " -O, --test-opts <list>  limit the set of filesystems (use with "
              "-a)\n"
              " -r, --read-only         mount the filesystem read-only (same "
              "as -o ro)\n"
              " -t, --types <list>      limit the set of filesystem types\n"));
    fprintf(out,
            _("     --source <src>      explicitly specifies source (path, "
              "label, uuid)\n"
              "     --target <target>   explicitly specifies mountpoint\n"));
    fprintf(out, _(" -v, --verbose           say what is being done\n"));
    fprintf(out, _(" -w, --rw, --read-write  mount the filesystem read-write "
                   "(default)\n"));
    fprintf(out,
            _(" -N, --namespace <ns>    perform mount in another namespace\n"));
    ...
    fprintf(out,
            _(" <device>                specifies device by path\n"
              " <directory>             mountpoint for bind mounts (see "
              "--bind/rbind)\n"
              " <file>                  regular file for loopdev setup\n"));

    fprintf(out,
            _("\nOperations:\n"
              " -B, --bind              mount a subtree somewhere else (same "
              "as -o bind)\n"
              " -M, --move              move a subtree to some other place\n"
              " -R, --rbind             mount a subtree and all submounts "
              "somewhere else\n"));
    ...
    fprintf(
        out,
        _(" --make-rshared          recursively mark a whole subtree as "
          "shared\n"
          " --make-rslave           recursively mark a whole subtree as slave\n"
          " --make-rprivate         recursively mark a whole subtree as "
          "private\n"
          " --make-runbindable      recursively mark a whole subtree as "
          "unbindable\n"));
...
int main(int argc, char** argv)
{
                            ...
                            longopts, NULL)) != -1)
    {
        ...
        if (mnt_context_is_restricted(cxt) && !strchr("hlLUVvrist", c) &&
            c != MOUNT_OPT_TARGET && c != MOUNT_OPT_SOURCE)
        ...
        switch (c)
        {
            ...
            case 'N':
            {
                ...
                if (mnt_context_set_target_ns(cxt, pid ? path : optarg))
                    err(MNT_EX_SYSERR,
                        _("failed to set target namespace to %s"),
                        pid ? path : optarg);
                break;
            }
    ...
    if (!mnt_context_get_source(cxt) && !mnt_context_get_target(cxt) && !argc &&
        !all)
    {
    ...
    if (types && (all || strchr(types, ',') || strncmp(types, "no", 2) == 0))
    ...
    if (all)
    {
        ...
        goto done;
             ...
             (mnt_context_get_source(cxt) || mnt_context_get_target(cxt)))
    {
        /*
         ...
         */
        if (mnt_context_is_restricted(cxt) && mnt_context_get_source(cxt) &&
