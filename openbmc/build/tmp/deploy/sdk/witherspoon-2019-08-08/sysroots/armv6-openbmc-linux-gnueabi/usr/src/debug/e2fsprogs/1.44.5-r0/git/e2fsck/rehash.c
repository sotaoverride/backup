...
static int fill_dir_block(ext2_filsys fs, blk64_t* block_nr,
                          ...
                          int ref_offset EXT2FS_ATTR((unused)), void* priv_data)
{
    ...
    else
    {
        ...
        fd->err = ext2fs_read_dir_block4(fs, *block_nr, dir, 0, fd->dir);
    ...
    while (dir_offset < fs->blocksize)
    {
        ...
        if (((dir_offset + rec_len) > fs->blocksize) || (rec_len < 8) ||
            ((rec_len % 4) != 0) || (name_len + 8 > rec_len))
        {
            fd->err = EXT2_ET_DIR_CORRUPTED;
            return BLOCK_ABORT;
        }
        ...
        if (dirent->inode == 0)
            continue;
        if (!fd->compress && (name_len == 1) && (dirent->name[0] == '.'))
            continue;
        if (!fd->compress && (name_len == 2) && (dirent->name[0] == '.') &&
            (dirent->name[1] == '.'))
        {
        ...
        {
            new_array = realloc(fd->harray, sizeof(struct hash_entry) *
                                                (fd->max_array + 500));
            if (!new_array)
            {
                fd->err = ENOMEM;
        ...
        {
            fd->err = ext2fs_dirhash(hash_alg, dirent->name, name_len,
                                     fs->super->s_hash_seed, &ent->hash,
                                     &ent->minor_hash);
            if (fd->err)
                return BLOCK_ABORT;
        }
                                ...
                                int blocks)
{
    ...
    if (outdir->max)
    {
        ...
        if (!new_mem)
            return ENOMEM;
        outdir->buf = new_mem;
        new_mem = realloc(outdir->hashes, blocks * sizeof(ext2_dirhash_t));
        if (!new_mem)
            return ENOMEM;
...
static int duplicate_search_and_fix(e2fsck_t ctx, ext2_filsys fs,
                                    ext2_ino_t ino, struct fill_dir_struct* fd)
{
    ...
    for (i = 1; i < fd->num_array; i++)
    {
        ...
        if (fix_problem(ctx, PR_2_NON_UNIQUE_FILE, &pctx))
        {
            ...
            ext2fs_dirhash(hash_alg, new_name, new_len, fs->super->s_hash_seed,
...
static errcode_t copy_dir_entries(e2fsck_t ctx, struct fill_dir_struct* fd,
                                  struct out_dir* outdir)
{
    ...
    if (ctx->htree_slack_percentage == 255)
    {
        profile_get_uint(ctx->profile, "options",
                         "indexed_dir_slack_percentage", 0, 20,
    ...
    outdir->max = 0;
    retval = alloc_size_dir(fs, outdir, (fd->dir_size / fs->blocksize) + 2);
    if (retval)
        return retval;
    ...
    for (i = 0; i < fd->num_array; i++)
    {
        ...
        ext2fs_dirent_set_name_len(dirent, ext2fs_dirent_name_len(ent->dir));
        ext2fs_dirent_set_file_type(dirent, ext2fs_dirent_file_type(ent->dir));
        retval = ext2fs_set_rec_len(fs, rec_len, dirent);
        if (retval)
            return retval;
        prev_rec_len = rec_len;
        memcpy(dirent->name, ent->dir->name, ext2fs_dirent_name_len(dirent));
        offset += rec_len;
        left -= rec_len;
        if (left < slack)
        {
...
static struct ext2_dx_root_info*
    set_root_node(ext2_filsys fs, char* buf, ext2_ino_t ino, ext2_ino_t parent)
{
    struct ext2_dir_entry* dir;
    ...
    limits->limit =
        (fs->blocksize - (32 + csum_size)) / sizeof(struct ext2_dx_entry);
    limits->count = 0;
...
static struct ext2_dx_entry* set_int_node(ext2_filsys fs, char* buf)
{
    ...
    limits->limit =
        (fs->blocksize - (8 + csum_size)) / sizeof(struct ext2_dx_entry);
    limits->count = 0;

    return (struct ext2_dx_entry*)limits;
}

static int alloc_blocks(ext2_filsys fs, struct ext2_dx_countlimit** limit,
                        struct ext2_dx_entry** prev_ent,
                        struct ext2_dx_entry** next_ent, int* prev_offset,
                        int* next_offset, struct out_dir* outdir, int i,
                        int* prev_count, int* next_count)
{
    ...
    if (*limit)
        (*limit)->limit = (*limit)->count = ext2fs_cpu_to_le16((*limit)->limit);
    *prev_ent = (struct ext2_dx_entry*)(outdir->buf + *prev_offset);
    (*prev_ent)->block = ext2fs_cpu_to_le32(outdir->num);

    if (i != 1)
        (*prev_ent)->hash = ext2fs_cpu_to_le32(outdir->hashes[i]);
                                ...
                                ext2_ino_t ino, ext2_ino_t parent)
{
    ...
    if (nblks - 1 <= c1)
    {
        ...
        for (i = 1; i < nblks; i++)
        {
            ...
            if (i != 1)
                root->hash = ext2fs_cpu_to_le32(outdir->hashes[i]);
            root++;
            c1--;
        }
    }
    else if (nblks - 1 <= ext2fs_htree_intnode_maxrecs(fs, c1))
    {
        ...
        for (i = 1; i < nblks; i++)
        {
            ...
            if (c2 != limit->limit)
                dx_ent->hash = ext2fs_cpu_to_le32(outdir->hashes[i]);
            dx_ent++;
            c2--;
        }
    ...
    else
    {
        ...
        for (i = 1; i < nblks; i++)
        {
            ...
            {
                retval =
                    alloc_blocks(fs, &int_limit, &root, &int_ent, &root_offset,
            ...
            if (c3 == 0)
            {
                retval = alloc_blocks(fs, &limit, &int_ent, &dx_ent,
                                      &int_offset, NULL, outdir, i, &c2, &c3);
                if (retval)
                    return retval;
            }
            dx_ent->block = ext2fs_cpu_to_le32(i);
 ...
 */
static int write_dir_block(ext2_filsys fs, blk64_t* block_nr,
                           e2_blkcnt_t blockcnt,
                           blk64_t ref_block EXT2FS_ATTR((unused)),
...
static errcode_t write_directory(e2fsck_t ctx, ext2_filsys fs,
                                 ...
                                 struct ext2_inode* inode, int compress)
{
    ...
    retval = ext2fs_block_iterate3(fs, ino, 0, NULL, write_dir_block, &wd);
    if (retval)
        return retval;
    if (wd.err)
        return wd.err;
    ...
    retval = ext2fs_inode_size_set(fs, inode,
                                   (ext2_off64_t)outdir->num * fs->blocksize);
    if (retval)
        return retval;
                            ...
                            struct problem_context* pctx)
{
    ...
    struct ext2_inode inode;
    char* dir_buf = 0;
    struct fill_dir_struct fd = {NULL, NULL, 0, 0, 0, NULL, 0, 0, 0, 0, 0, 0};
