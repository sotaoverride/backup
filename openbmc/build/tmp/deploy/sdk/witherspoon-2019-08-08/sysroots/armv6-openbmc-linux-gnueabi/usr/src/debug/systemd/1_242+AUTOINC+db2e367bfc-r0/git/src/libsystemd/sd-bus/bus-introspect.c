...
int introspect_write_default_interfaces(struct introspect* i,
                                        bool object_manager)
{
    assert(i);
...
int introspect_write_child_nodes(struct introspect* i, Set* s,
                                 const char* prefix)
{
    char* node;
...
static void introspect_write_flags(struct introspect* i, int type,
                                   uint64_t flags)
{
    if (flags & SD_BUS_VTABLE_DEPRECATED)
        fputs("   <annotation name=\"org.freedesktop.DBus.Deprecated\" "
              "value=\"true\"/>\n",
              i->f);

    if (type == _SD_BUS_VTABLE_METHOD &&
        (flags & SD_BUS_VTABLE_METHOD_NO_REPLY))
        fputs("   <annotation name=\"org.freedesktop.DBus.Method.NoReply\" "
              "value=\"true\"/>\n",
              i->f);
    ...
    if (IN_SET(type, _SD_BUS_VTABLE_PROPERTY, _SD_BUS_VTABLE_WRITABLE_PROPERTY))
    {
        if (flags & SD_BUS_VTABLE_PROPERTY_EXPLICIT)
            fputs("   <annotation name=\"org.freedesktop.systemd1.Explicit\" "
                  "value=\"true\"/>\n",
                  i->f);
    ...
    if (!i->trusted &&
        ...
        fputs("   <annotation name=\"org.freedesktop.systemd1.Privileged\" "
              "value=\"true\"/>\n",
              i->f);
}

/* Note that "names" is both an input and an output parameter. It initially
   points to the first argument name in a
   NULL-separated list of strings, and is then advanced with each argument, and
   the resulting pointer is returned. */
static int introspect_write_arguments(struct introspect* i,
                                      const char* signature, const char** names,
...
int introspect_write_interface(struct introspect* i, const sd_bus_vtable* v)
{
    ...
    for (; v->type != _SD_BUS_VTABLE_END; v = bus_vtable_next(vtable, v))
    {
        ...
        if (v->type != _SD_BUS_VTABLE_START &&
            (v->flags & SD_BUS_VTABLE_HIDDEN))
            continue;
        ...
        switch (v->type)
        {
            ...
            case _SD_BUS_VTABLE_METHOD:
                ...
                introspect_write_arguments(i, strempty(v->x.method.signature),
                                           &names, "in");
                introspect_write_arguments(i, strempty(v->x.method.result),
                                           &names, "out");
            ...
            case _SD_BUS_VTABLE_WRITABLE_PROPERTY:
                fprintf(i->f,
                        ...
                        v->x.property.member, v->x.property.signature,
            ...
            case _SD_BUS_VTABLE_SIGNAL:
                ...
                if (bus_vtable_has_names(vtable))
                    names = strempty(v->x.method.names);
                introspect_write_arguments(i, strempty(v->x.signal.signature),
                                           &names, NULL);
                introspect_write_flags(i, v->type, v->flags);
                fputs("  </signal>\n", i->f);
                break;
...
int introspect_finish(struct introspect* i, sd_bus* bus, sd_bus_message* m,
