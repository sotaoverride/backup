...
static void __attribute__((__noreturn__)) usage(void)
{
    ...
    fputs(_(" -y                       for compatibility only, ignored\n"),
          out);
    fputs(_(" -b, --blocksize <size>   use this blocksize, defaults to page "
            "size\n"),
          out);
    fputs(_("     --extract[=<dir>]    test uncompression, optionally extract "
            "into <dir>\n"),
          out);
    fputs(USAGE_SEPARATOR, out);
...
static void test_crc(int start)
{
    ...
    if (buf == MAP_FAILED)
    {
        buf = mmap(NULL, super.size, PROT_READ | PROT_WRITE,
        ...
        if (buf != MAP_FAILED)
        {
            ...
            if (tmp != (ssize_t)super.size)
                errx(FSCK_EX_ERROR,
                     _("failed to read %" PRIu32 " bytes from file %s"),
                     super.size, filename);
        }
    }
    if (buf != MAP_FAILED)
...
static void print_node(char type, struct cramfs_inode* i, char* name)
{
    ...
    printf("%c %04o %s %5d:%-3d %s\n", type, i->mode & ~S_IFMT, info, i->uid,
           i->gid, !*name && type == 'd' ? "/" : name);
}
...
static int uncompress_block(void* src, size_t len)
{
    ...
    if (err != Z_STREAM_END)
        errx(FSCK_EX_UNCORRECTED, _("decompression error: %s"), zError(err));
                          ...
                          unsigned long size)
{
    ...
    do
    {
        ...
        unsigned long next = u32_toggle_endianness(
            cramfs_is_big_endian, *(uint32_t*)romfs_read(offset));
        ...
        {
            if (out != blksize)
                errx(FSCK_EX_UNCORRECTED, _("non-block (%ld) bytes"), out);
        }
        else
        {
            if (out != size)
                errx(FSCK_EX_UNCORRECTED, _("non-size (%ld vs %ld) bytes"), out,
                     size);
        }
        size -= out;
...
static void do_directory(char* path, struct cramfs_inode* i)
{
    ...
    if (offset == 0 && count != 0)
        errx(FSCK_EX_UNCORRECTED,
             _("directory inode has zero offset and non-zero size: %s"), path);
...
static void do_special_inode(char* path, struct cramfs_inode* i)
{
    ...
    if (i->offset)
        ...
        errx(FSCK_EX_UNCORRECTED, _("special file has non-zero offset: %s"),
             path);
    ...
    {
        if (i->size != 0)
            errx(FSCK_EX_UNCORRECTED, _("fifo has non-zero size: %s"), path);
        type = 'p';
    }
    else if (S_ISSOCK(i->mode))
    {
        if (i->size != 0)
            errx(FSCK_EX_UNCORRECTED, _("socket has non-zero size: %s"), path);
        type = 's';
    }
    else
    {
...
static void test_fs(int start)
{
    ...
    if (start_data != ~0UL)
    {
        if (start_data < (sizeof(struct cramfs_super) + start))
            errx(FSCK_EX_UNCORRECTED,
                 _("directory data start (%lu) < sizeof(struct cramfs_super) + "
                   "start (%zu)"),
