...
static void __attribute__((__noreturn__)) usage(void)
{
    ...
    fputs(_(" -F, --no-fork          do not fork before exec'ing <program>\n"),
          out);
#ifdef HAVE_LIBSELINUX
    fputs(_(" -Z, --follow-context   set SELinux context according to --target "
            "PID\n"),
          out);
#endif
...
static void open_target_fd(int* fd, const char* type, const char* path)
{
    ...
    {
        snprintf(pathbuf, sizeof(pathbuf), "/proc/%u/%s", namespace_target_pid,
                 type);
        path = pathbuf;
    }
    if (!path)
...
int main(int argc, char* argv[])
{
    ...
    while (
        (c = getopt_long(argc, argv, "+ahVt:m::u::i::n::p::C::U::S:G:r::w::FZ",
                         longopts, NULL)) != -1)
    {
        switch (c)
        {
    ...
    {
        if (fchdir(wd_fd) < 0)
            err(EXIT_FAILURE, _("change directory by working directory file "
                                "descriptor failed"));
