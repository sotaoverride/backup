        ...
        netsnmp_mode_handler_list* endlist)
{
    netsnmp_mib_handler* me =
        netsnmp_create_handler("mode_end_call", netsnmp_mode_end_call_helper);
 ...
 * @param endlist the information structure for the mode_end_call helper.  Can
 * be NULL to create a new list.
 * @param mode the mode to be called upon.  A mode of NETSNMP_MODE_END_ALL_MODES
 * = all modes.
...
int netsnmp_mode_end_call_helper(netsnmp_mib_handler* handler,
                                 ...
                                 netsnmp_request_info* requests)
{
    ...
    for (ptr = (netsnmp_mode_handler_list*)handler->myvoid; ptr;
         ptr = ptr->next)
    {
        if (ptr->mode == NETSNMP_MODE_END_ALL_MODES ||
            reqinfo->mode == ptr->mode)
        {
            ret2 = netsnmp_call_handler(ptr->callback_handler, reginfo, reqinfo,
                                        requests);
