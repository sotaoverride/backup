    ...
    netsnmp_feature_child_of(table_tdata, table_tdata_all)
        netsnmp_feature_child_of(table_tdata_delete_table, table_tdata_all)
            netsnmp_feature_child_of(table_tdata_extract_table, table_tdata_all)
                netsnmp_feature_child_of(table_tdata_remove_row,
                                         table_tdata_all)
                    netsnmp_feature_child_of(table_tdata_insert_row,
                                             table_tdata_all)
...
netsnmp_mib_handler* netsnmp_get_tdata_handler(netsnmp_tdata* table)
{
    ...
    ret =
        netsnmp_create_handler(TABLE_TDATA_NAME, _netsnmp_tdata_helper_handler);
    if (ret)
    {
    ...
    switch (reqinfo->mode)
    {
        ...
        case MODE_SET_RESERVE1:
            ...
            for (request = requests; request; request = request->next)
            {
                ...
                row = (netsnmp_tdata_row*)netsnmp_container_table_row_extract(
                    request);
                ...
                netsnmp_request_add_list_data(
                    request,
                    netsnmp_create_data_list(TABLE_TDATA_TABLE, table, NULL));
                netsnmp_request_add_list_data(
                    request,
                    netsnmp_create_data_list(TABLE_TDATA_ROW, row, NULL));
            }
...
netsnmp_feature_child_of(tdata_extract_container, table_tdata_all)
    ...
    netsnmp_container* netsnmp_tdata_extract_container(
        netsnmp_request_info* request)
{
    netsnmp_tdata* tdata = (netsnmp_tdata*)netsnmp_request_get_list_data(
        request, TABLE_TDATA_TABLE);
...
int netsnmp_tdata_compare_oid(netsnmp_tdata_row* row, oid* compareto,
                              size_t compareto_len)
{
    netsnmp_index* index = (netsnmp_index*)row;
    return snmp_oid_compare(index->oids, index->len, compareto, compareto_len);
}
...
int netsnmp_tdata_compare_subtree_oid(netsnmp_tdata_row* row, oid* compareto,
                                      size_t compareto_len)
{
    netsnmp_index* index = (netsnmp_index*)row;
    return snmp_oidtree_compare(index->oids, index->len, compareto,
                                compareto_len);
}
