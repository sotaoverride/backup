...
#pragma warning(disable : 4351) // vc8 new behavior: elements of array 'foo'
...
struct set_matcher : quant_style_fixed_width<1>
{
    ...
    set_matcher() : set_(), not_(false), icase_(false)
    {
    }
    ...
    {
        if (state.eos() ||
            this->not_ == this->in_set(traits_cast<Traits>(state), *state.cur_))
        {
            return false;
        }
