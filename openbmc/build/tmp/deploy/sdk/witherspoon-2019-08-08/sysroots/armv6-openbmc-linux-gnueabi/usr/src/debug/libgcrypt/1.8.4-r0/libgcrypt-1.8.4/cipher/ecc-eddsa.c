...
static void reverse_buffer(unsigned char* buffer, unsigned int length)
{
    unsigned int tmp, i;
...
static gpg_err_code_t eddsa_encodempi(gcry_mpi_t mpi, unsigned int minlen,
...
/* Encode (X,Y) using the EdDSA scheme.  MINLEN is the required length
   ...
   malloced buffer with the encoded point is stored at R_BUFFER; the
   length of this buffer is stored at R_BUFLEN.  */
static gpg_err_code_t eddsa_encode_x_y(gcry_mpi_t x, gcry_mpi_t y,
                                       unsigned int minlen, int with_prefix,
                                       unsigned char** r_buffer,
                                       unsigned int* r_buflen)
{
    unsigned char* rawmpi;
    ...
    rawmpi =
        _gcry_mpi_get_buffer_extra(y, minlen, off ? -1 : 0, &rawmpilen, NULL);
    if (!rawmpi)
        return gpg_err_code_from_syserror();
...
gpg_err_code_t _gcry_ecc_eddsa_encodepoint(mpi_point_t point, mpi_ec_t ec,
                                           ...
                                           unsigned int* r_buflen)
{
    ...
    if (_gcry_mpi_ec_get_affine(x, y, point, ec))
    {
        ...
        rc = eddsa_encode_x_y(x, y, ec->nbits / 8, with_prefix, r_buffer,
                              r_buflen);
    ...
    if (rawmpilen > 1 && (rawmpilen % 2))
    {
        if (buf[0] == 0x04)
        {
            /* Buffer is in SEC1 uncompressed format.  Extract y and
               compress.  */
            rc = _gcry_mpi_scan(&x, GCRYMPI_FMT_STD, buf + 1,
                                (rawmpilen - 1) / 2, NULL);
            if (rc)
                return rc;
    ...
    if (mpi_is_opaque(pk))
    {
        ...
        if (rawmpilen > 1 && (rawmpilen % 2))
        {
            ...
            if (buf[0] == 0x04)
            {
                ...
                if (r_encpk)
                {
                    rc = eddsa_encode_x_y(x, y, ctx->nbits / 8, 0, r_encpk,
                                          r_encpklen);
                    if (rc)
                    {
                        mpi_free(x);
 ...
 * to use a faster RNG.
 */
gpg_err_code_t _gcry_ecc_eddsa_genkey(ECC_secret_key* sk, elliptic_curve_t* E,
 ...
 */
gpg_err_code_t _gcry_ecc_eddsa_sign(gcry_mpi_t input, ECC_secret_key* skey,
 ...
 */
gpg_err_code_t _gcry_ecc_eddsa_verify(gcry_mpi_t input, ECC_public_key* pkey,
