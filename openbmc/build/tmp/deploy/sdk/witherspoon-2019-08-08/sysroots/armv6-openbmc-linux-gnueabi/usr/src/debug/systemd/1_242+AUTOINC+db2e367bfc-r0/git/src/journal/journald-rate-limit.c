...
static const int priority_map[] = {
    [LOG_EMERG] = 0,   [LOG_ALERT] = 0,  [LOG_CRIT] = 0, [LOG_ERR] = 1,
...
_pure_ static bool journal_rate_limit_group_expired(JournalRateLimitGroup* g,
                                                    usec_t ts)
{
    unsigned i;
...
static JournalRateLimitGroup* journal_rate_limit_group_new(JournalRateLimit* r,
                                                           const char* id,
                                                           usec_t interval,
                                                           usec_t ts)
{
...
int journal_rate_limit_test(JournalRateLimit* r, const char* id,
                            usec_t rl_interval, unsigned rl_burst, int priority,
                            uint64_t available)
{
    uint64_t h;
    JournalRateLimitGroup* g;
     ...
     * 1 + n → the log message shall be permitted, and n messages were dropped
     * from the peer before < 0   → error
     */
