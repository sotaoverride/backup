...
#define set_field(field, default)                                              \
    (super->field = param->field ? param->field : (default))
#define assign_field(field) (super->field = param->field)
    ...
    if (bigalloc_flag)
    {
        ...
        if (param->s_blocks_per_group && param->s_clusters_per_group &&
        ...
        if (param->s_clusters_per_group)
            ...
            super->s_clusters_per_group =
                param->s_blocks_per_group / EXT2FS_CLUSTER_RATIO(fs);
...
retry:
    ...
    set_field(s_desc_size,
              ext2fs_has_feature_64bit(super) ? EXT2_MIN_DESC_SIZE_64BIT : 0);

    fs->desc_blocks =
        ext2fs_div_ceil(fs->group_desc_count, EXT2_DESC_PER_BLOCK(super));
...
ipg_retry:
    ...
    fs->inode_blocks_per_group =
        (((super->s_inodes_per_group * EXT2_INODE_SIZE(super)) +
          EXT2_BLOCK_SIZE(super) - 1) /
         EXT2_BLOCK_SIZE(super));
    super->s_inodes_per_group =
        ((fs->inode_blocks_per_group * EXT2_BLOCK_SIZE(super)) /
    ...
    super->s_inodes_count = super->s_inodes_per_group * fs->group_desc_count;
     ...
     */
    overhead =
        (int)(3 + fs->inode_blocks_per_group + super->s_reserved_gdt_blocks);
    ...
    {
        ext2fs_blocks_count_set(super, ext2fs_blocks_count(super) - rem);
        /*
         * If blocks count is changed, we need to recalculate
         * reserved blocks count not to exceed 50%.
         */
        reserved_ratio =
            100.0 * ext2fs_r_blocks_count(param) / ext2fs_blocks_count(param);
        ext2fs_r_blocks_count_set(
            super, reserved_ratio * ext2fs_blocks_count(super) / 100.0);
