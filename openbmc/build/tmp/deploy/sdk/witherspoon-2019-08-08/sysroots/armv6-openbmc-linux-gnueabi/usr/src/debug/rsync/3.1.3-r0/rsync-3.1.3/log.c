...
static void logit(int priority, const char* buf)
{
    ...
    if (logfile_fp)
    {
        fprintf(logfile_fp, "%s [%d] %s", timestring(time(NULL)), (int)getpid(),
                buf);
        fflush(logfile_fp);
    }
    else
...
static void logfile_open(void)
{
    ...
    if (!logfile_fp)
    {
        ...
        syslog_init();
        rsyserr(FERROR, fopen_errno, "failed to open log-file %s",
                logfile_name);
...
void log_init(int restart)
{
    if (log_initialised)
    {
        if (!restart) /* Note: a restart only happens with am_daemon */
            return;
        assert(logfile_name); /* all am_daemon procs got at least an empty
                                 string */
...
static void filtered_fwrite(FILE* f, const char* buf, int len, int use_isprint)
{
    ...
    for (s = buf; s < end; s++)
    {
        if ((s < end - 4 && *s == '\\' && s[1] == '#' && isDigit(s + 2) &&
             isDigit(s + 3) && isDigit(s + 4)) ||
            (*s != '\t' && ((use_isprint && !isPrint(s)) || *(uchar*)s < ' ')))
        {
        ...
        *c = '\0';
        ...
        switch (*p)
        {
            case 'h':
                if (am_daemon)
                {
                    n = lp_reverse_lookup(module_id) ? client_name(0)
            ...
            case 'U':
                ...
                snprintf(buf2, sizeof buf2, fmt, uid_ndx ? F_OWNER(file) : 0);
                n = buf2;
                break;
            case 'G':
                ...
                {
                    strlcat(fmt, "u", sizeof fmt);
                    snprintf(buf2, sizeof buf2, fmt, F_GROUP(file));
                    n = buf2;
                }
                break;
            case 'p':
            ...
            case 'f':
                ...
                {
                    pathjoin(buf2, sizeof buf2, F_PATHNAME(file), c);
                    clean_fname(buf2, 0);
                    if (fmt[1])
                    {
                ...
                {
                    pathjoin(buf2, sizeof buf2, curr_dir + module_dirlen, c);
                    clean_fname(buf2, 0);
                    if (fmt[1])
                    {
                        strlcpy(c, buf2, MAXPATHLEN);
            ...
            case 'c':
                ...
                strlcat(fmt, "s", sizeof fmt);
                snprintf(buf2, sizeof buf2, fmt, do_big_num(b, humanize, NULL));
                n = buf2;
                break;
            case 'C':
                n = NULL;
                if (S_ISREG(file->mode))
                {
                    if (always_checksum && canonical_checksum(checksum_type))
                        n = sum_as_hex(checksum_type, F_SUM(file), 1);
                             ...
                             canonical_checksum(xfersum_type))
                        n = sum_as_hex(xfersum_type, sender_file_sum, 0);
                }
                if (!n)
                {
                    int sum_len = csum_len_for_type(
                        always_checksum ? checksum_type : xfersum_type,
                        always_checksum);
                ...
                if (S_ISLNK(file->mode))
                {
                    ...
                    c[4] = !(iflags & ITEM_REPORT_TIME)
                               ...
                               : !preserve_times || !receiver_symlink_times ||
                                         (iflags & ITEM_REPORT_TIMEFAIL)
                                     ? 'T'
                                     : 't';
                }
                else
                {
                    c[1] = S_ISDIR(file->mode)
                               ? 'd'
                               : IS_SPECIAL(file->mode)
                                     ? 'S'
 ...
 * output to both. */
void log_item(enum logcode code, struct file_struct* file, int iflags,
              const char* hlink)
{
    const char* s_or_r = am_sender ? "send" : "recv";

    if (code != FLOG && stdout_format && !am_server)
        log_formatted(FCLIENT, stdout_format, s_or_r, file, NULL, iflags,
                      hlink);
                    ...
                    const char* buf)
{
    ...
    int see_item =
        itemizing && (significant_flags || *buf || stdout_format_has_i > 1 ||
                      (INFO_GTE(NAME, 2) && stdout_format_has_i));
    ...
    {
        enum logcode code =
            significant_flags || logfile_format_has_i ? FINFO : FCLIENT;
...
void log_exit(int code, const char* file, int line)
{
    if (code == 0)
    {
        rprintf(FLOG, "sent %s bytes  received %s bytes  total size %s\n",
                big_num(stats.total_written), big_num(stats.total_read),
