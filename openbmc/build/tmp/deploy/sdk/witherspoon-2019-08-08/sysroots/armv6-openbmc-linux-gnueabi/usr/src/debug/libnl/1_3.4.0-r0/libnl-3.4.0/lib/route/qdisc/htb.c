...
static int htb_class_msg_parser(struct rtnl_tc* tc, void* data)
{
    ...
    if (tb[TCA_HTB_PARMS])
    {
        ...
        htb->ch_rbuffer =
            rtnl_tc_calc_bufsize(nl_ticks2us(opts.buffer), opts.rate.rate);
        htb->ch_cbuffer =
            rtnl_tc_calc_bufsize(nl_ticks2us(opts.cbuffer), opts.ceil.rate);
        ...
        htb->ch_mask = (SCH_HTB_HAS_PRIO | SCH_HTB_HAS_RATE | SCH_HTB_HAS_CEIL |
                        SCH_HTB_HAS_RBUFFER | SCH_HTB_HAS_CBUFFER |
                        SCH_HTB_HAS_QUANTUM | SCH_HTB_HAS_LEVEL);
    }
                                ...
                                struct nl_dump_params* p)
{
    ...
    if (htb->ch_mask & SCH_HTB_HAS_RATE)
    {
        ...
        nl_dump(p, " rate %.2f%s/s (%.0f%s) log %u", r, ru, rbit, rubit,
                1 << htb->ch_rate.rs_cell_log);
    }
}
                                   ...
                                   struct nl_dump_params* p)
{
    ...
    if (htb->ch_mask & SCH_HTB_HAS_CEIL)
    {
        ...
        nl_dump(p, " ceil %.2f%s/s (%.0f%s) log %u", r, ru, rbit, rubit,
                1 << htb->ch_ceil.rs_cell_log);
    }
