                                      ...
                                      const gss_buffer_set_t src)
{
    ...
    {
        status =
            gss_add_buffer_set_member(minor_status, &src->elements[i], dst);
        if (status != GSS_S_COMPLETE)
            break;
    }
...
OM_uint32 KRB5_CALLCONV gss_inquire_cred_by_oid(OM_uint32* minor_status,
                                                ...
                                                gss_buffer_set_t* data_set)
{
    ...
    *data_set = GSS_C_NO_BUFFER_SET;
    ...
    for (i = 0; i < union_cred->count; i++)
    {
        ...
        status = (mech->gss_inquire_cred_by_oid)(
            minor_status, union_cred->cred_array[i], desired_object, &ret_set);
        if (status != GSS_S_COMPLETE)
        {
