 ...
 */
krb5_error_code krb5_dbe_def_search_enctype(kcontext, dbentp, start, ktype,
                                            stype, kvno,
                                            kdatap) krb5_context kcontext;
...
krb5_error_code krb5_def_store_mkey_list(krb5_context context, char* keyfile,
    ...
    if ((statrc = stat(keyfile, &stb)) >= 0)
    {
        ...
        if (!S_ISREG(stb.st_mode))
        {
            ...
            k5_setmsg(context, retval,
                      _("keyfile (%s) is not a regular file: %s"), keyfile,
...
static krb5_error_code krb5_db_def_fetch_mkey_stash(krb5_context context,
                                                    const char* keyfile,
                                                    krb5_keyblock* key,
                                                    krb5_kvno* kvno)
{
    ...
    if (fread((krb5_pointer)&keylength, sizeof(keylength), 1, kf) != 1)
    {
        retval = KRB5_KDB_CANTREAD_STORED;
        goto errout;
    }
...
static krb5_error_code krb5_db_def_fetch_mkey_keytab(krb5_context context,
                                                     const char* keyfile,
                                                     krb5_principal mname,
                                                     krb5_keyblock* key,
                                                     krb5_kvno* kvno)
{
    ...
    if ((retval =
             krb5_kt_get_entry(context, kt, mname, kvno ? *kvno : IGNORE_VNO,
                               enctype, &kt_ent)) == 0)
    {
         ...
         */
        key->contents =
            k5memdup(kt_ent.key.contents, kt_ent.key.length, &retval);
...
krb5_error_code krb5_db_def_fetch_mkey(krb5_context context,
                                       krb5_principal mname, krb5_keyblock* key,
                                       krb5_kvno* kvno, char* db_args)
{
    ...
    {
        (void)snprintf(keyfile, sizeof(keyfile), "%s%s", DEFAULT_KEYFILE_STUB,
                       realm->data);
    }
...
krb5_error_code krb5_def_fetch_mkey_list(krb5_context context,
                                         ...
                                         krb5_keylist_node** mkeys_list)
{
    ...
    for (i = 1; i < master_entry->n_key_data; i++)
    {
        if (*mkey_list_node == NULL)
        {
            ...
            *mkey_list_node =
                (krb5_keylist_node*)malloc(sizeof(krb5_keylist_node));
            if (*mkey_list_node == NULL)
            {
                retval = ENOMEM;
                goto clean_n_exit;
        ...
        retval = krb5_dbe_decrypt_key_data(
            context, &cur_mkey, key_data, &((*mkey_list_node)->keyblock), NULL);
        if (retval)
            goto clean_n_exit;
...
krb5_error_code krb5_db_def_rename_principal(krb5_context kcontext,
                                             ...
                                             krb5_const_principal target)
{
    ...
    ret = krb5_db_get_principal(kcontext, source, KRB5_KDB_FLAG_ALIAS_OK, &kdb);
    if (ret)
        goto cleanup;
