...
struct resolv_conf*
    __resolv_conf_load(struct __res_state* preinit) attribute_hidden
    __attribute__((warn_unused_result));

/* Return a configuration object for the current /etc/resolv.conf
   settings, or NULL on failure.  The object is cached.  */
struct resolv_conf* __resolv_conf_get_current(void) attribute_hidden
    __attribute__((warn_unused_result));
...
/* Allocate a new struct resolv_conf object and copy the
   ...
   __resolv_conf_put.  */
struct resolv_conf*
    __resolv_conf_allocate(const struct resolv_conf* init) attribute_hidden
...
bool __resolv_conf_attach(struct __res_state*,
                          struct resolv_conf*) attribute_hidden;
