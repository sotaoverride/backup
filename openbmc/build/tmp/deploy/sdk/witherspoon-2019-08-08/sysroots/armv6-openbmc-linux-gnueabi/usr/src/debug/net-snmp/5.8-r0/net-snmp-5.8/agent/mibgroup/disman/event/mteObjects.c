...
void _init_default_mteObject(const char* oname, const char* object, int index,
                             int wcard)
{
    ...
    if (entry)
    {
        entry->flags |= MTE_OBJECT_FLAG_ACTIVE | MTE_OBJECT_FLAG_FIXED |
...
int _init_default_mteObject_lists(int majorID, int minorID, void* serverargs,
                                  void* clientarg)
{
 ...
 */
netsnmp_tdata_row* mteObjects_createEntry(const char* owner, const char* oname,
                                          int index, int flags)
{
    struct mteObject* entry;
    ...
    netsnmp_tdata_row_add_index(row, ASN_OCTET_STR, entry->mteOwner, owner_len);
    if (oname)
        memcpy(entry->mteOName, oname, oname_len);
    netsnmp_tdata_row_add_index(row, ASN_OCTET_STR, entry->mteOName, oname_len);
    entry->mteOIndex = index;
    netsnmp_tdata_row_add_index(row, ASN_INTEGER, &entry->mteOIndex,
                                sizeof(long));
     ...
     */
    row2 = netsnmp_tdata_row_get_byoid(objects_table_data, row->oid_index.oids,
                                       row->oid_index.len);
    if (row2)
    {
 ...
 */
struct mteObject* mteObjects_addOID(const char* owner, const char* oname,
                                    int index, const char* oid_name_buf,
                                    int wild)
{
...
void mteObjects_removeEntry(netsnmp_tdata_row* row)
{
    ...
    entry = (struct mteObject*)netsnmp_tdata_remove_and_delete_row(
        objects_table_data, row);
    SNMP_FREE(entry);
}
...
void mteObjects_removeEntries(const char* owner, char* oname)
{
    ...
    snmp_set_var_typed_value(&oname_var, ASN_OCTET_STR, oname, strlen(oname));
...
int mteObjects_vblist(netsnmp_variable_list* vblist, char* owner, char* oname,
                      oid* suffix, size_t sfx_len)
{
    ...
    DEBUGMSGTL(
        ("disman:event:objects", "Objects add (%s, %s)\n", owner, oname));
    ...
    while (row && !netsnmp_tdata_compare_subtree_idx(row, &owner_var))
    {
        ...
        memcpy(name, entry->mteObjectID, entry->mteObjectID_len * sizeof(oid));
...
int mteObjects_internal_vblist(netsnmp_variable_list* vblist, char* oname,
                               ...
                               netsnmp_session* sess)
{
    ...
    if (!strcmp(oname, "_triggerFire"))
    {
        ...
        snmp_varlist_add_variable(&var, mteHotTrigger,
                                  OID_LENGTH(mteHotTrigger), ASN_OCTET_STR,
                                  trigger->mteTName, strlen(trigger->mteTName));
        snmp_varlist_add_variable(&var, mteHotTarget, OID_LENGTH(mteHotTarget),
                                  ASN_OCTET_STR, trigger->mteTriggerTarget,
                                  strlen(trigger->mteTriggerTarget));
        snmp_varlist_add_variable(
            &var, mteHotContext, OID_LENGTH(mteHotContext), ASN_OCTET_STR,
            trigger->mteTriggerContext, strlen(trigger->mteTriggerContext));
        snmp_varlist_add_variable(
            &var, mteHotOID, OID_LENGTH(mteHotOID), ASN_OBJECT_ID,
            (char*)trigger->mteTriggerFired->name,
            trigger->mteTriggerFired->name_length * sizeof(oid));
        snmp_varlist_add_variable(&var, mteHotValue, OID_LENGTH(mteHotValue),
    ...
    else if ((!strcmp(oname, "_linkUpDown")))
    {
        ...
        snmp_varlist_add_variable(&var, ifIndexOid, OID_LENGTH(ifIndexOid),
                                  ASN_INTEGER, &if_index, sizeof(if_index));

        /* Set up a dummy varbind for ifAdminStatus... */
        snmp_varlist_add_variable(&var, ifAdminStatus,
                                  OID_LENGTH(ifAdminStatus), ASN_INTEGER,
                                  trigger->mteTriggerFired->val.integer,
                                  trigger->mteTriggerFired->val_len);
        /* ... then retrieve the actual value */
        netsnmp_query_get(var->next_variable, sess);

        snmp_varlist_add_variable(&var, ifOperStatus, OID_LENGTH(ifOperStatus),
                                  ASN_INTEGER,
                                  trigger->mteTriggerFired->val.integer,
                                  trigger->mteTriggerFired->val_len);
    }
