 ...
 */
static int get_file_options(const char* vendor, const char* model, int* argc,
                            char*** newargv)
{
...
static void help(void)
{
    printf(
        ...
        "  -p --page=0x80|0x83|pre-spc3-83  SCSI page (0x80, 0x83, "
        "pre-spc3-83)\n"
...
static int set_options(int argc, char** argv, char* maj_min_dev)
{
    ...
    optind = 1;
    while ((option =
                getopt_long(argc, argv, "d:f:gp:uvVxhbs:", options, NULL)) >= 0)
        switch (option)
        {
            case 'b':
                all_good = false;
...
static int per_dev_options(struct scsi_id_device* dev_scsi, int* good_bad,
                           int* page_code)
{
    int retval;
    int newargc;
...
static int set_inq_values(struct scsi_id_device* dev_scsi, const char* path)
{
    ...
    util_replace_whitespace(dev_scsi->vendor, vendor_str,
                            sizeof(vendor_str) - 1);
    util_replace_chars(vendor_str, NULL);
    util_replace_whitespace(dev_scsi->model, model_str, sizeof(model_str) - 1);
    util_replace_chars(model_str, NULL);
    set_type(dev_scsi->type, type_str, sizeof(type_str));
    util_replace_whitespace(dev_scsi->revision, revision_str,
...
static int scsi_id(char* maj_min_dev)
{
    ...
    if (export)
    {
        ...
        if (dev_scsi.serial[0] != '\0')
        {
            ...
            printf("ID_SERIAL=%s\n", serial_str);
            util_replace_whitespace(dev_scsi.serial_short, serial_str,
                                    sizeof(serial_str) - 1);
            util_replace_chars(serial_str, NULL);
