...
{
    return nscd_getgr_r(name, strlen(name) + 1, GETGRBYNAME, resultbuf, buffer,
                        buflen, result);
}
                        ...
                        struct group** result)
{
    ...
    struct mapped_database* mapped =
        __nscd_get_map_ref(GETFDGR, "group", &__gr_map_handle, &gc_cycle);
retry:;
    ...
    if (mapped != NO_MAPPING)
    {
        struct datahead* found =
            __nscd_cache_search(type, key, keylen, mapped, sizeof gr_resp);
        if (found != NULL)
        {
            len = (const uint32_t*)(&found->data[0].grdata + 1);
            gr_resp = found->data[0].grdata;
            gr_name =
                ((const char*)len + gr_resp.gr_mem_cnt * sizeof(uint32_t));
    ...
    if (gr_name == NULL)
    {
        sock = __nscd_open_socket(key, keylen, type, &gr_resp, sizeof(gr_resp));
        if (sock == -1)
        {
       ...
       align the pointer.  */
        ...
        if (gr_name == NULL)
        {
            ...
            else
            {
                ...
                if (!scratch_buffer_set_array_size(&lenbuf, gr_resp.gr_mem_cnt,
                                                   sizeof(uint32_t)))
                    goto out_close;
                len = lenbuf.data;
