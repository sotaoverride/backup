...
static void g_file_icon_load_async(GLoadableIcon* icon, int size,
...
G_DEFINE_TYPE_WITH_CODE(
    GFileIcon, g_file_icon, G_TYPE_OBJECT,
...
static void g_file_icon_get_property(GObject* object, guint prop_id,
...
static void g_file_icon_set_property(GObject* object, guint prop_id,
...
static void g_file_icon_finalize(GObject* object)
{
    GFileIcon* icon;
...
static void g_file_icon_class_init(GFileIconClass* klass)
{
    ...
    g_object_class_install_property(
        gobject_class, PROP_FILE,
        g_param_spec_object(
            "file", P_("file"), P_("The file containing the icon"), G_TYPE_FILE,
            G_PARAM_CONSTRUCT_ONLY | G_PARAM_READWRITE | G_PARAM_STATIC_NAME |
                G_PARAM_STATIC_BLURB | G_PARAM_STATIC_NICK));
}

static void g_file_icon_init(GFileIcon* file)
{
}
...
static guint g_file_icon_hash(GIcon* icon)
{
...
static gboolean g_file_icon_equal(GIcon* icon1, GIcon* icon2)
{
...
static gboolean g_file_icon_to_tokens(GIcon* icon, GPtrArray* tokens,
                                      gint* out_version)
{
    GFileIcon* file_icon = G_FILE_ICON(icon);
...
static GIcon* g_file_icon_from_tokens(gchar** tokens, gint num_tokens,
...
static GVariant* g_file_icon_serialize(GIcon* icon)
{
    GFileIcon* file_icon = G_FILE_ICON(icon);

    return g_variant_new(
        "(sv)", "file",
        g_variant_new_take_string(g_file_get_uri(file_icon->file)));
}
...
static GInputStream* g_file_icon_load(GLoadableIcon* icon, int size,
                                      char** type, GCancellable* cancellable,
                                      GError** error)
{
...
static void load_async_callback(GObject* source_object, GAsyncResult* res,
                                gpointer user_data)
{
    GFileInputStream* stream;
    GError* error = NULL;
                                   ...
                                   gpointer user_data)
{
    ...
    g_file_read_async(file_icon->file, 0, cancellable, load_async_callback,
                      task);
}

static GInputStream* g_file_icon_load_finish(GLoadableIcon* icon,
