...
struct template
{
    ...
    sbool bHaveSubtree;
    msgPropDescr_t
        subtree;      /* subtree property name for subtree-type templates */
...
#define JSONF 4 /* 4 - not a real escape - template contains json fields only  \
     ...
     * than short...
     */
    char optCaseSensitive; /* case-sensitive variable property references,
                              default False, 0 */
};
...
struct templateEntry
{
    ...
    int lenFieldName;
    sbool bComplexProcessing; /**< set if complex processing (options, etc) is
                                 required */
    union
    {
        struct
        ...
        struct
        {
            ...
            unsigned iFromPos;      /* for partial strings only chars from this
                                       position ... */
            ...
            short
                iSubMatchToUse; /* which submatch should be obtained (10 max) */
            ...
            enum tlpRegexNoMatchType
            {
                ...
                TPL_REGEX_NOMATCH_USE_WHOLE_FIELD = 2, /* use the full field
                                    contents that we were searching in*/
                TPL_REGEX_NOMATCH_USE_ZERO =
                    3        /* use  0 (useful for numerical values) */
            } nomatchAction; /**< what to do if we do not have a match? */

#endif
            unsigned
                has_fields; /* support for field-counting: field to extract */
            unsigned char field_delim; /* support for field-counting: field
                                          delemiter char */
#ifdef STRICT_GPLV3
            int field_expand; /* use multiple instances of the field delimiter
                                 as a single one? */
#endif
            ...
            {                              /* bit fields! */
                ...
                unsigned bSpaceCC : 1;     /* change control characters to
                                              spaceescape? */
                unsigned bEscapeCC : 1;    /* escape control characters? */
                unsigned bCompressSP : 1;  /* compress multiple spaces to a
                                              single one? */
                unsigned bDropLastLF : 1;  /* drop last LF char in msg (PIX!) */
                unsigned bSecPathDrop : 1; /* drop slashes, replace dots, empty
                                              string */
                unsigned bSecPathReplace : 1; /* replace slashes, replace dots,
                                                 empty string */
                unsigned bSPIffNo1stSP : 1; /* be a space if 1st pos if field is
                                               no space*/
                ...
                unsigned bJSONfr : 1; /* format field JSON *field* non escaped
                                         (n/v pair) */
                unsigned
                    bMandatory : 1; /* mandatory field - emit even if empty */
                unsigned bFromPosEndRelative : 1; /* is From/To-Pos relative to
                                                     end of string? */
                unsigned bFixedWidth : 1; /* space pad to toChar if string is
                                             shorter */
 ...
 * code line below, the actual cause is that you currently MUST include
 * template.h BEFORE msg.h, even if your code file does not actually need it.
 * rgerhards, 2007-08-06
 */
rsRetVal tplToJSON(struct template* pTpl, smsg_t* pMsg, struct json_object**,
