...
static lzma_ret decode_buffer(lzma_coder* coder, const uint8_t* restrict in,
                              ...
                              size_t out_size)
{
    while (true)
    {
        ...
        coder->dict.limit =
            coder->dict.pos +
            my_min(out_size - *out_pos, coder->dict.size - coder->dict.pos);
              ...
              size_t out_size, lzma_action action)
{
    ...
    if (coder->next.code == NULL)
        return decode_buffer(coder, in, in_pos, in_size, out, out_pos,
                             out_size);
    ...
    while (*out_pos < out_size)
    {
        ...
        if (!coder->next_finished && coder->temp.pos == coder->temp.size)
        {
            ...
            const lzma_ret ret =
                coder->next.code(coder->next.coder, allocator, in, in_pos,
                                 in_size, coder->temp.buffer, &coder->temp.size,
        ...
        const lzma_ret ret =
            decode_buffer(coder, coder->temp.buffer, &coder->temp.pos,
                        ...
                        const void* options, lzma_lz_options* lz_options))
{
    ...
    {
        lzma_free(coder->dict.buf, allocator);
        coder->dict.buf = lzma_alloc(lz_options.dict_size, allocator);
    ...
    if (lz_options.preset_dict != NULL && lz_options.preset_dict_size > 0)
    {
        ...
        const size_t copy_size =
            my_min(lz_options.preset_dict_size, lz_options.dict_size);
...
extern void lzma_lz_decoder_uncompressed(void* coder_ptr,
