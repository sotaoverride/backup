...
{
    //  The NewLexer function allows the opaque generation of a new lexer
    //  object. It is coupled to the token type to allow to decouple the
...
struct lex_input_interface_generator : lex_input_interface<TokenT>
{
    ...
    //  The new_lexer function allows the opaque generation of a new lexer
    //  object. It is coupled to the token type to allow to distinguish
    //  different lexer/token configurations at compile time.
    template <typename IteratorT>
    static lex_input_interface<TokenT>*
        new_lexer(IteratorT const& first, IteratorT const& last,
