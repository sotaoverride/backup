...
static krb5_error_code kdcrep2creds(krb5_context context, krb5_kdc_rep* pkdcrep,
                                    krb5_address* const* address,
                                    krb5_boolean is_skey, krb5_data* psectkt,
                                    krb5_creds** ppcreds)
{
    ...
    if ((retval = krb5_copy_keyblock_contents(
             context, pkdcrep->enc_part2->session, &(*ppcreds)->keyblock)))
        goto cleanup;
    ...
    else
    {
        ...
        if ((retval =
                 krb5_copy_addresses(context, address, &(*ppcreds)->addresses)))
            goto cleanup_keyblock;
    }
...
static krb5_error_code check_reply_server(krb5_context context,
...
static inline int tgt_is_local_realm(krb5_creds* tgt)
{
...
krb5_error_code krb5_get_cred_via_tkt(krb5_context context, krb5_creds* tkt,
                                      krb5_flags kdcoptions,
                                      krb5_address* const* address,
                                      krb5_creds* in_cred,
                                      krb5_creds** out_cred)
{
    return krb5_get_cred_via_tkt_ext(context, tkt, kdcoptions, address, NULL,
                                     in_cred, NULL, NULL, NULL, NULL, out_cred,
                                     NULL);
}

krb5_error_code krb5int_process_tgs_reply(
    krb5_context context, struct krb5int_fast_request_state* fast_state,
    ...
    krb5_creds** out_cred)
{
    ...
    if (krb5_is_krb_error(response_data))
    {
        ...
        if (retval != 0)
            goto cleanup;
        retval = krb5int_fast_process_error(context, fast_state, &err_reply,
                                            NULL, NULL);
        if (retval)
            goto cleanup;
        ...
        if (err_reply->text.length > 0)
        {
            switch (err_reply->error)
            {
                ...
                case KDC_ERR_S_PRINCIPAL_UNKNOWN:
                {
                    char* s_name;
                    if (err_reply->server &&
                        krb5_unparse_name(context, err_reply->server,
                                          &s_name) == 0)
                    {
                        k5_setmsg(context, retval,
    ...
    if (retval)
    {
        ...
        if ((krb5int_decode_tgs_rep(
                context, fast_state, response_data, &tkt->keyblock,
    ...
    if ((kdcoptions & KDC_OPT_POSTDATED) && (in_cred->times.starttime != 0) &&
    ...
    if ((kdcoptions & KDC_OPT_RENEWABLE) && (in_cred->times.renew_till != 0) &&
...
krb5_error_code krb5_get_cred_via_tkt_ext(
    ...
    krb5_pa_data*** out_enc_padata, krb5_creds** out_cred,
...
send_again:
    ...
    retval = krb5int_process_tgs_reply(context, fast_state, &response_data, tkt,
                                       kdcoptions, address, in_padata, in_cred,
                                       timestamp, nonce, subkey, out_padata,
