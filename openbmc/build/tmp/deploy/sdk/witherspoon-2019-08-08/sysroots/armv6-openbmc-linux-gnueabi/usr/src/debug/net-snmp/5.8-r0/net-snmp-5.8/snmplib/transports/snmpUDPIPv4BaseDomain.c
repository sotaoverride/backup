...
int netsnmp_udpipv4base_transport_bind(netsnmp_transport* t,
                                       ...
                                       int flags)
{
...
#ifndef WIN32
#if defined(HAVE_IP_PKTINFO)
        if (setsockopt(t->sock, SOL_IP, IP_PKTINFO, &sockopt, sizeof sockopt) ==
            -1)
        {
    ...
    DEBUGIF("netsnmp_udpbase")
    {
        ...
        str = netsnmp_udp_fmtaddr(NULL, (void*)&addr_pair,
                                  sizeof(netsnmp_indexed_addr_pair));
        DEBUGMSGTL(
            ("netsnmp_udpbase", "binding socket: %d to %s\n", t->sock, str));
        free(str);
    }
...
void netsnmp_udpipv4base_transport_get_bound_addr(netsnmp_transport* t)
{
    ...
    DEBUGIF("netsnmp_udpbase")
    {
        ...
        str = netsnmp_udp_fmtaddr(NULL, (void*)addr_pair,
                                  sizeof(netsnmp_indexed_addr_pair));
        DEBUGMSGTL(
            ("netsnmp_udpbase", "socket %d bound to %s\n", t->sock, str));
        free(str);
    }
}
                                                 ...
                                                 int local)
{
    if (!local)
    {
        ...
        if (client_socket)
        {
            ...
            int have_port, rc;
            int uses_port = netsnmp_ds_get_boolean(
