...
DEFINE_CONFIG_PARSE_ENUM(config_parse_fou_encap_type, fou_encap_type,
                         FooOverUDPEncapType, "Failed to parse Encapsulation=");

static int netdev_fill_fou_tunnel_message(NetDev* netdev,
                                          sd_netlink_message** ret)
{
    ...
    r = sd_genl_message_new(netdev->manager->genl, SD_GENL_FOU, FOU_CMD_ADD,
                            &m);
    if (r < 0)
    ...
    if (r < 0)
        return log_netdev_error_errno(
            netdev, r, "Could not append FOU_ATTR_PORT attribute: %m");
    ...
    if (r < 0)
        return log_netdev_error_errno(
            netdev, r, "Could not append FOU_ATTR_TYPE attribute: %m");

    r = sd_netlink_message_append_u8(m, FOU_ATTR_AF, AF_INET);
    if (r < 0)
        return log_netdev_error_errno(
            netdev, r, "Could not append FOU_ATTR_AF attribute: %m");

    r = sd_netlink_message_append_u8(m, FOU_ATTR_IPPROTO, t->fou_protocol);
    if (r < 0)
        return log_netdev_error_errno(
            netdev, r, "Could not append FOU_ATTR_IPPROTO attribute: %m");

    *ret = TAKE_PTR(m);
    return 0;
}

static int fou_tunnel_create_handler(sd_netlink* rtnl, sd_netlink_message* m,
                                     NetDev* netdev)
{
    int r;
...
static int netdev_fou_tunnel_create(NetDev* netdev)
{
    ...
    r = netlink_call_async(netdev->manager->genl, NULL, m,
                           fou_tunnel_create_handler, netdev_destroy_callback,
                           netdev);
    if (r < 0)
...
int config_parse_ip_protocol(const char* unit, const char* filename,
        ...
        case NETDEV_FOO_OVER_UDP_ENCAP_DIRECT:
            if (t->fou_protocol <= 0)
                return log_netdev_error_errno(
                    ...
                    "FooOverUDP protocol not configured in %s. Rejecting "
                    "configuration.",
                    filename);
            break;
        case NETDEV_FOO_OVER_UDP_ENCAP_GUE:
            if (t->fou_protocol > 0)
                return log_netdev_error_errno(
                    netdev, SYNTHETIC_ERRNO(EINVAL),
                    "FooOverUDP GUE can't be set with protocol configured in "
                    "%s. Rejecting configuration.",
                    filename);
            break;
        default:
