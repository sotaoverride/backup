...
void _init_builtin_mteEvent(const char* event, const char* oname, oid* trapOID,
                            size_t trapOID_len);
                            ...
                            size_t trapOID_len)
{
    ...
    entry->flags |=
        MTE_EVENT_FLAG_ENABLED | MTE_EVENT_FLAG_ACTIVE | MTE_EVENT_FLAG_VALID;
}
...
void _mteEvent_dump(void)
{
    ...
    for (row = netsnmp_tdata_row_first(event_table_data); row;
...
void mteEvent_removeEntry(netsnmp_tdata_row* row)
{
    ...
    entry = (struct mteEvent*)netsnmp_tdata_remove_and_delete_row(
        event_table_data, row);
    SNMP_FREE(entry);
}
...
int _mteEvent_fire_notify(struct mteEvent* event, struct mteTrigger* trigger,
                          oid* suffix, size_t sfx_len);
#ifndef NETSNMP_NO_WRITE_SUPPORT
int _mteEvent_fire_set(struct mteEvent* event, struct mteTrigger* trigger,
                       oid* suffix, size_t sfx_len);
                  ...
                  oid* suffix, size_t s_len)  /* Matching instance  */
{
    ...
    snmp_set_var_typed_value(&event_var, ASN_PRIV_IMPLIED_OCTET_STR, event,
                             strlen(event));
    owner_var.next_variable = &event_var;
    entry = (struct mteEvent*)netsnmp_tdata_row_entry(
...
#ifdef __NOT_NEEDED
void _insert_internal_objects(netsnmp_variable_list* vblist, char* oname,
                              struct mteTrigger* trigger)
{
    ...
    {
        DEBUGMSGTL(("disman:event:fire", "Unknown internal objects tag (%s)\n",
                    oname));
        return;
    }
