...
static void all_for_now_callback(AvahiTimeEvent* e, void* userdata)
{
    ...
    l->callback(l->engine, l->interface, l->protocol, AVAHI_BROWSER_ALL_FOR_NOW,
                AVAHI_LOOKUP_RESULT_MULTICAST, NULL, l->userdata);
}

AvahiMulticastLookup* avahi_multicast_lookup_new(
    AvahiMulticastLookupEngine* e, AvahiIfIndex interface,
    AvahiProtocol protocol, AvahiKey* key,
    AvahiMulticastLookupCallback callback, void* userdata)
{
    ...
    /* Issue the ALL_FOR_NOW event one second after the querier was initially
     * created */
    l->all_for_now_event = avahi_time_event_new(e->server->time_event_queue,
                                                &tv, all_for_now_callback, l);
...
static void lookup_stop(AvahiMulticastLookup* l)
{
    ...
    if (l->queriers_added)
    {
        avahi_querier_remove_for_all(l->engine->server, l->interface,
                                     l->protocol, l->key);
        l->queriers_added = 0;
    }
...
static void* scan_cache_callback(AvahiCache* c, AvahiKey* pattern,
                                 AvahiCacheEntry* e, void* userdata)
{
    ...
    cbdata->callback(cbdata->engine, cbdata->interface->hardware->index,
                     cbdata->interface->protocol, AVAHI_BROWSER_NEW,
                     AVAHI_LOOKUP_RESULT_CACHED | AVAHI_LOOKUP_RESULT_MULTICAST,
                     e->record, cbdata->userdata);
...
static void scan_interface_callback(AvahiInterfaceMonitor* m, AvahiInterface* i,
                                    void* userdata)
{
    struct cbdata* cbdata = userdata;
    ...
    if (cbdata->cname_key)
        avahi_cache_walk(i->cache, cbdata->cname_key, scan_cache_callback,
                         cbdata);
    ...
    AvahiMulticastLookupCallback callback, void* userdata)
{
    ...
    avahi_interface_monitor_walk(e->server->monitor, interface, protocol,
                                 scan_interface_callback, &cbdata);
...
void avahi_multicast_lookup_engine_new_interface(AvahiMulticastLookupEngine* e,
                                                 AvahiInterface* i)
{
    ...
    for (l = e->lookups; l; l = l->lookups_next)
    {
        ...
        if (l->queriers_added &&
            avahi_interface_match(i, l->interface, l->protocol))
            avahi_querier_add(i, l->key, NULL);
    }
}

void avahi_multicast_lookup_engine_notify(AvahiMulticastLookupEngine* e,
                                          AvahiInterface* i,
                                          AvahiRecord* record,
                                          AvahiBrowserEvent event)
{
    ...
    for (l = avahi_hashmap_lookup(e->lookups_by_key, record->key); l;
         l = l->by_key_next)
    {
        if (l->dead || !l->callback)
            continue;
        ...
        if (avahi_interface_match(i, l->interface, l->protocol))
            l->callback(e, i->hardware->index, i->protocol, event,
                        AVAHI_LOOKUP_RESULT_MULTICAST, record, l->userdata);
    }

    if (record->key->clazz == AVAHI_DNS_CLASS_IN &&
        record->key->type == AVAHI_DNS_TYPE_CNAME)
    {
        ...
        for (l = e->lookups; l; l = l->lookups_next)
        {
            ...
            if ((key = avahi_key_new_cname(l->key)))
            {
                if (avahi_key_equal(record->key, key))
                    l->callback(e, i->hardware->index, i->protocol, event,
                                AVAHI_LOOKUP_RESULT_MULTICAST, record,
                                l->userdata);
