...
struct sequence_stack
{
  ...
  private:
    ...
    {
        allocate_guard_t guard = {0, (T*)::operator new(size * sizeof(T)),
                                  false};
    ...
    struct chunk
    {
        chunk(std::size_t size, T const& t, std::size_t count, chunk* back,
              chunk* next) :
            begin_(allocate(size, t)),
            curr_(begin_ + count), end_(begin_ + size), back_(back), next_(next)
        {
            if (this->back_)
    ...
    T* grow_(std::size_t count, T const& t)
    {
        if (this->current_chunk_)
        {
            ...
            if (this->current_chunk_->next_ &&
                count <= this->current_chunk_->next_->size())
            {
                this->current_chunk_ = this->current_chunk_->next_;
                this->curr_ = this->current_chunk_->curr_ =
                    this->current_chunk_->begin_ + count;
            ...
            this->current_chunk_ =
                new chunk(new_size, t, count, this->current_chunk_,
                          this->current_chunk_->next_);
        }
        else
        {
            // first chunk is 256
            std::size_t new_size =
                (std::max)(count, static_cast<std::size_t>(256U));
    ...
    {
        return !std::less<void*>()(ptr, this->begin_) &&
               std::less<void*>()(ptr, this->end_);
    }

  public:
    sequence_stack() : current_chunk_(0), begin_(0), curr_(0), end_(0)
    {
    }
    ...
    void unwind()
    {
        if (this->current_chunk_)
        {
            ...
            this->begin_ = this->curr_ = this->current_chunk_->curr_ =
                this->current_chunk_->begin_;
            this->end_ = this->current_chunk_->end_;
        }
    }
    ...
    void conserve()
    {
        if (this->current_chunk_)
        {
            for (chunk* next; this->current_chunk_->next_;
                 this->current_chunk_->next_ = next)
            {
