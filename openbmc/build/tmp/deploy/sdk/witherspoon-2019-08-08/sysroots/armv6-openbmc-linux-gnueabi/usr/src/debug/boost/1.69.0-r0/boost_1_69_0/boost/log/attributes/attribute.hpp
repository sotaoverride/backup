 ...
 *
 * Every attribute is represented with a factory that is basically an attribute
 * value generator. The sole purpose of an attribute is to return an actual
 * value when requested. A simplest attribute can always return the same value
 * that it stores internally, but more complex ones can perform a considerable
 * amount of work to return a value, and the returned values may differ each
 * time requested.
 *
 * A word about thread safety. An attribute should be prepared to be requested a
 * value from multiple threads concurrently.
 */
class attribute
{
  ...
  public:
        ...
        : public boost::intrusive_ref_counter<impl>
    {
        ...
        /*!
         * \return The actual attribute value. It shall not return empty values
         * (exceptions shall be used to indicate errors).
         */
        virtual attribute_value get_value() = 0;

        BOOST_LOG_API static void* operator new(std::size_t size);
        BOOST_LOG_API static void
            operator delete(void* p, std::size_t size)BOOST_NOEXCEPT;
    };
  ...
  public:
    /*!
     * Default constructor. Creates an empty attribute value factory, which is
     * not usable until \c set_impl is called.
     */
     ...
     */
    explicit attribute(intrusive_ptr<impl> p) BOOST_NOEXCEPT
    {
        m_pImpl.swap(p);
    }
