...
flush_underlying_parser const flush_underlying_parser_p =
    flush_underlying_parser();
    ...
    : public boost::spirit::classic::grammar<cpp_grammar<TokenT, ContainerT>>
{
    ...
    struct definition
    {
        ...
        typedef boost::spirit::classic::scanner_policies<
            ...
            policies_t;
        typedef boost::spirit::classic::scanner<typename ScannerT::iterator_t,
                                                policies_t>
            non_tree_scanner_t;
        ...
        definition(cpp_grammar const& self)
        {
            ...
            // C++ standard 16.1: A preprocessing directive consists of a
            // sequence of preprocessing tokens. The first token in the sequence
            // is # preprocessing token that is either the first character in
            // the source file (optionally after white space containing no
            // new-line characters) or that follows white space containing at
            // least one new-line character. The last token in the sequence is
            // the first new-line character that follows the first token in the
            // sequence.
...
//  Note: this may break the parser, though.
      ...
      (BOOST_SPIRIT_DEBUG_FLAGS_CPP & BOOST_SPIRIT_DEBUG_FLAGS_CPP_GRAMMAR))
            ...
            macro_include_file // include ...anything else...
                = no_node_d[ch_p(T_PP_INCLUDE)[store_found_directive_type(
                                self.found_directive)]
#if BOOST_WAVE_SUPPORT_INCLUDE_NEXT != 0
                            |
                            ch_p(T_PP_INCLUDE_NEXT)[store_found_directive_type(
                                self.found_directive)]
#endif
            ] >> *(anychar_p -
            ...
            plain_define =
                no_node_d[ch_p(T_PP_DEFINE)[store_found_directive_type(
                              self.found_directive)] >>
                          +ppsp] >>
                (ch_p(T_IDENTIFIER) |
                ...
                ((no_node_d[eps_p(ch_p(T_LEFTPAREN))] >> macro_parameters >>
                  !macro_definition) |
            ...
            // #undef FOO
            undefine =
                no_node_d[ch_p(T_PP_UNDEF)[store_found_directive_type(
                              self.found_directive)] >>
                          +ppsp] >>
                (ch_p(T_IDENTIFIER) |
                 pattern_p(KeywordTokenType, TokenTypeMask | PPTokenFlag) |
            ...
            // #ifdef et.al.
            ppifdef = no_node_d[ch_p(T_PP_IFDEF)[store_found_directive_type(
                                    self.found_directive)] >>
                                +ppsp] >>
                      ppqualifiedname;

            ppifndef = no_node_d[ch_p(T_PP_IFNDEF)[store_found_directive_type(
                                     self.found_directive)] >>
                                 +ppsp] >>
                       ppqualifiedname;
            ...
            ppif =
                no_node_d[ch_p(
                    T_PP_IF)[store_found_directive_type(self.found_directive)]
                          //                        >> *ppsp
            ] >> +(anychar_p -
                   (ch_p(T_NEWLINE) | ch_p(T_CPPCOMMENT) | ch_p(T_EOF)));
            ...
            ppelif =
                no_node_d[ch_p(
                    T_PP_ELIF)[store_found_directive_type(self.found_directive)]
                          //                        >> *ppsp
            ] >> +(anychar_p -
                   (ch_p(T_NEWLINE) | ch_p(T_CPPCOMMENT) | ch_p(T_EOF)));
            ...
            // #line ...
            ppline = no_node_d[ch_p(T_PP_LINE)[store_found_directive_type(
                                   self.found_directive)] >>
                               *ppsp] >>
                     +(anychar_p -
                       (ch_p(T_NEWLINE) | ch_p(T_CPPCOMMENT) | ch_p(T_EOF)));
            ...
            // #region ...
            ppregion =
                no_node_d[ch_p(T_MSEXT_PP_REGION)[store_found_directive_type(
                              self.found_directive)] >>
                          +ppsp] >>
                ppqualifiedname;
            ...
            // #endregion
            ppendregion =
                no_node_d[ch_p(T_MSEXT_PP_ENDREGION)[store_found_directive_type(
                    self.found_directive)]];
#endif
            ...
            illformed // for error reporting
                = no_node_d[pattern_p(T_POUND, MainTokenMask) >> *ppsp] >>
                  (anychar_p -
                   (ch_p(T_NEWLINE) | ch_p(T_CPPCOMMENT) | ch_p(T_EOF))) >>
                  no_node_d[*(anychar_p - (ch_p(T_NEWLINE) |
                                           ch_p(T_CPPCOMMENT) | ch_p(T_EOF)))];

            // #error
            pperror = no_node_d[ch_p(T_PP_ERROR)[store_found_directive_type(
                                    self.found_directive)] >>
                                *ppsp] >>
                      *(anychar_p -
                        (ch_p(T_NEWLINE) | ch_p(T_CPPCOMMENT) | ch_p(T_EOF)));
            ...
            // #warning
            ppwarning = no_node_d[ch_p(T_PP_WARNING)[store_found_directive_type(
                                      self.found_directive)] >>
                                  *ppsp] >>
                        *(anychar_p -
                          (ch_p(T_NEWLINE) | ch_p(T_CPPCOMMENT) | ch_p(T_EOF)));
            ...
            // end of line tokens
            eol_tokens =
                no_node_d[*(ch_p(T_SPACE) | ch_p(T_CCOMMENT)) >>
            ...
            //             BOOST_SPIRIT_DEBUG_TRACE_RULE(include_file,
            //             TRACE_CPP_GRAMMAR);
            //             BOOST_SPIRIT_DEBUG_TRACE_RULE(system_include_file,
            //             TRACE_CPP_GRAMMAR);
            BOOST_SPIRIT_DEBUG_TRACE_RULE(macro_include_file,
                                          TRACE_CPP_GRAMMAR);
            ...
            //             BOOST_SPIRIT_DEBUG_TRACE_RULE(ppelse,
            //             TRACE_CPP_GRAMMAR);
            //             BOOST_SPIRIT_DEBUG_TRACE_RULE(ppelif,
            //             TRACE_CPP_GRAMMAR);
    ...
    cpp_grammar(bool& found_eof_, TokenT& found_directive_,
        ...
        found_directive(found_directive_), found_eoltokens(found_eoltokens_)
    {
            ...
            for (int i = 0; 0 != init_ruleid_name_map[i].parser_id; ++i)
                base_type::insert(base_type::value_type(
                    boost::spirit::classic::parser_id(
                        init_ruleid_name_map[i].parser_id),
        ...
        token_type& found_directive, token_container_type& found_eoltokens)
{
    ...
    cpp_grammar<token_type, TokenContainerT> g(found_eof, found_directive,
                                               found_eoltokens);
