...
static BUS_DEFINE_PROPERTY_GET(property_get_description, "s", Unit,
                               unit_description);
static BUS_DEFINE_PROPERTY_GET2(property_get_active_state, "s", Unit,
                                unit_active_state, unit_active_state_to_string);
static BUS_DEFINE_PROPERTY_GET(property_get_sub_state, "s", Unit,
                               unit_sub_state_to_string);
static BUS_DEFINE_PROPERTY_GET2(property_get_unit_file_state, "s", Unit,
                                unit_get_unit_file_state,
                                unit_file_state_to_string);
static BUS_DEFINE_PROPERTY_GET(property_get_can_reload, "b", Unit,
                               unit_can_reload);
static BUS_DEFINE_PROPERTY_GET(property_get_can_start, "b", Unit,
                               unit_can_start_refuse_manual);
static BUS_DEFINE_PROPERTY_GET(property_get_can_stop, "b", Unit,
                               unit_can_stop_refuse_manual);
static BUS_DEFINE_PROPERTY_GET(property_get_can_isolate, "b", Unit,
                               unit_can_isolate_refuse_manual);
static BUS_DEFINE_PROPERTY_GET(property_get_need_daemon_reload, "b", Unit,
                               unit_need_daemon_reload);
static BUS_DEFINE_PROPERTY_GET_GLOBAL(property_get_empty_strv, "as", 0);

static int property_get_names(sd_bus* bus, const char* path,
...
static int property_get_following(sd_bus* bus, const char* path,
                                  const char* interface, const char* property,
                                  sd_bus_message* reply, void* userdata,
                                  sd_bus_error* error)
{
...
static int property_get_dependencies(sd_bus* bus, const char* path,
...
static int property_get_requires_mounts_for(sd_bus* bus, const char* path,
                                            const char* interface,
                                            const char* property,
...
static int property_get_unit_file_preset(sd_bus* bus, const char* path,
                                         const char* interface,
                                         const char* property,
                                         sd_bus_message* reply, void* userdata,
                                         sd_bus_error* error)
{
    ...
    return sd_bus_message_append(reply, "s",
                                 r < 0 ? NULL : r > 0 ? "enabled" : "disabled");
}

static int property_get_job(sd_bus* bus, const char* path,
                            const char* interface, const char* property,
                            sd_bus_message* reply, void* userdata,
                            sd_bus_error* error)
{
...
static int property_get_conditions(sd_bus* bus, const char* path,
                                   const char* interface, const char* property,
                                   sd_bus_message* reply, void* userdata,
                                   sd_bus_error* error)
{
    ...
    LIST_FOREACH(conditions, c, *list)
    {
        ...
        tristate = c->result == CONDITION_UNTESTED
                       ? 0
...
static int property_get_load_error(sd_bus* bus, const char* path,
                                   const char* interface, const char* property,
                                   sd_bus_message* reply, void* userdata,
                                   sd_bus_error* error)
{
...
static int bus_verify_manage_units_async_full(
    Unit* u, const char* verb, int capability, const char* polkit_message,
    bool interactive, sd_bus_message* call, sd_bus_error* error)
{
    ...
    return bus_verify_polkit_async(
        call, capability, "org.freedesktop.systemd1.manage-units", details,
        interactive, UID_INVALID, &u->manager->polkit_registry, error);
}
...
int bus_unit_method_start_generic(sd_bus_message* message, Unit* u,
                                  ...
                                  sd_bus_error* error)
{
    ...
    r = mac_selinux_unit_access_check(
        u, message, job_type_to_access_method(job_type), error);
    if (r < 0)
        return r;
    ...
    r = bus_verify_manage_units_async_full(u, verb, CAP_SYS_ADMIN,
    ...
    return bus_unit_queue_job(
        message, u, job_type, mode,
        reload_if_possible ? BUS_UNIT_QUEUE_RELOAD_IF_POSSIBLE : 0, error);
}

static int method_start(sd_bus_message* message, void* userdata,
                        sd_bus_error* error)
{
    return bus_unit_method_start_generic(message, userdata, JOB_START, false,
                                         error);
}

static int method_stop(sd_bus_message* message, void* userdata,
                       sd_bus_error* error)
{
    return bus_unit_method_start_generic(message, userdata, JOB_STOP, false,
                                         error);
}
...
static int method_reload(sd_bus_message* message, void* userdata,
                         sd_bus_error* error)
{
    return bus_unit_method_start_generic(message, userdata, JOB_RELOAD, false,
                                         error);
}

static int method_restart(sd_bus_message* message, void* userdata,
                          sd_bus_error* error)
{
    return bus_unit_method_start_generic(message, userdata, JOB_RESTART, false,
                                         error);
}

static int method_try_restart(sd_bus_message* message, void* userdata,
                              sd_bus_error* error)
{
    return bus_unit_method_start_generic(message, userdata, JOB_TRY_RESTART,
                                         false, error);
}

static int method_reload_or_restart(sd_bus_message* message, void* userdata,
                                    sd_bus_error* error)
{
    return bus_unit_method_start_generic(message, userdata, JOB_RESTART, true,
                                         error);
}

static int method_reload_or_try_restart(sd_bus_message* message, void* userdata,
                                        sd_bus_error* error)
{
    return bus_unit_method_start_generic(message, userdata, JOB_TRY_RESTART,
                                         true, error);
...
int bus_unit_method_enqueue_job(sd_bus_message* message, void* userdata,
                                sd_bus_error* error)
{
    ...
    r = bus_verify_manage_units_async_full(u, jtype, CAP_SYS_ADMIN,
                                           polkit_message_for_job[type], true,
                                           message, error);
    if (r < 0)
        return r;
    if (r == 0)
...
int bus_unit_method_kill(sd_bus_message* message, void* userdata,
                         sd_bus_error* error)
{
    Unit* u = userdata;
    const char* swho;
    ...
    r = bus_verify_manage_units_async_full(
        u, "kill", CAP_KILL,
        N_("Authentication is required to send a UNIX signal to the processes "
           "of '$(unit)'."),
        true, message, error);
    if (r < 0)
        return r;
...
int bus_unit_method_reset_failed(sd_bus_message* message, void* userdata,
                                 sd_bus_error* error)
{
    Unit* u = userdata;
    int r;
    ...
    r = bus_verify_manage_units_async_full(
        u, "reset-failed", CAP_SYS_ADMIN,
        N_("Authentication is required to reset the \"failed\" state of "
           "'$(unit)'."),
        true, message, error);
    if (r < 0)
        return r;
...
int bus_unit_method_set_properties(sd_bus_message* message, void* userdata,
                                   sd_bus_error* error)
{
    Unit* u = userdata;
    int runtime, r;
    ...
    r = bus_verify_manage_units_async_full(
        u, "set-property", CAP_SYS_ADMIN,
        N_("Authentication is required to set properties on '$(unit)'."), true,
        message, error);
    if (r < 0)
        return r;
    if (r == 0)
    ...
    r = bus_unit_set_properties(
        u, message, runtime ? UNIT_RUNTIME : UNIT_PERSISTENT, true, error);
    if (r < 0)
        return r;
...
int bus_unit_method_ref(sd_bus_message* message, void* userdata,
                        sd_bus_error* error)
{
    Unit* u = userdata;
    int r;
    ...
    r = bus_verify_manage_units_async_full(u, "ref", CAP_SYS_ADMIN, NULL, false,
                                           message, error);
    if (r < 0)
        return r;
    if (r == 0)
...
int bus_unit_method_unref(sd_bus_message* message, void* userdata,
                          sd_bus_error* error)
{
    Unit* u = userdata;
    int r;
    ...
    if (r == -EUNATCH)
        return sd_bus_error_setf(error, BUS_ERROR_NOT_REFERENCED,
...
static int property_get_refs(sd_bus* bus, const char* path,
                             const char* interface, const char* property,
                             sd_bus_message* reply, void* userdata,
                             sd_bus_error* error)
{
    ...
    for (i = sd_bus_track_first(u->bus_track); i;
         i = sd_bus_track_next(u->bus_track))
    {
        int c, k;
...
const sd_bus_vtable bus_unit_vtable[] = {
    ...
    SD_BUS_PROPERTY("Names", "as", property_get_names, offsetof(Unit, names),
                    SD_BUS_VTABLE_PROPERTY_CONST),
    ...
    SD_BUS_PROPERTY("RequiredBy", "as", property_get_dependencies,
                    ...
                    SD_BUS_VTABLE_PROPERTY_CONST),
    SD_BUS_PROPERTY("RequisiteOf", "as", property_get_dependencies,
    ...
    SD_BUS_PROPERTY("JoinsNamespaceOf", "as", property_get_dependencies,
                    ...
                    SD_BUS_VTABLE_PROPERTY_CONST),
    SD_BUS_PROPERTY("RequiresMountsFor", "as", property_get_requires_mounts_for,
                    offsetof(Unit, requires_mounts_for),
                    SD_BUS_VTABLE_PROPERTY_CONST),
    SD_BUS_PROPERTY("Documentation", "as", NULL, offsetof(Unit, documentation),
                    SD_BUS_VTABLE_PROPERTY_CONST),
    ...
    SD_BUS_PROPERTY("SubState", "s", property_get_sub_state, 0,
                    SD_BUS_VTABLE_PROPERTY_EMITS_CHANGE),
    SD_BUS_PROPERTY("FragmentPath", "s", NULL, offsetof(Unit, fragment_path),
                    SD_BUS_VTABLE_PROPERTY_CONST),
    ...
    SD_BUS_PROPERTY("CanIsolate", "b", property_get_can_isolate, 0,
                    SD_BUS_VTABLE_PROPERTY_CONST),
    SD_BUS_PROPERTY("Job", "(uo)", property_get_job, offsetof(Unit, job),
                    SD_BUS_VTABLE_PROPERTY_EMITS_CHANGE),
    ...
    SD_BUS_PROPERTY("IgnoreOnIsolate", "b", bus_property_get_bool,
                    ...
                    SD_BUS_VTABLE_PROPERTY_CONST),
    SD_BUS_PROPERTY("NeedDaemonReload", "b", property_get_need_daemon_reload, 0,
                    SD_BUS_VTABLE_PROPERTY_CONST),
    SD_BUS_PROPERTY("JobTimeoutUSec", "t", bus_property_get_usec,
                    offsetof(Unit, job_timeout), SD_BUS_VTABLE_PROPERTY_CONST),
    SD_BUS_PROPERTY("JobRunningTimeoutUSec", "t", bus_property_get_usec,
    ...
    SD_BUS_PROPERTY("Perpetual", "b", bus_property_get_bool,
                    offsetof(Unit, perpetual), SD_BUS_VTABLE_PROPERTY_CONST),
    SD_BUS_PROPERTY("StartLimitIntervalUSec", "t", bus_property_get_usec,
    ...
    SD_BUS_PROPERTY("StartLimitBurst", "u", bus_property_get_unsigned,
                    ...
                    SD_BUS_VTABLE_PROPERTY_CONST),
    SD_BUS_PROPERTY("StartLimitAction", "s", property_get_emergency_action,
                    offsetof(Unit, start_limit_action),
                    SD_BUS_VTABLE_PROPERTY_CONST),
    SD_BUS_PROPERTY("FailureAction", "s", property_get_emergency_action,
                    offsetof(Unit, failure_action),
                    SD_BUS_VTABLE_PROPERTY_CONST),
    SD_BUS_PROPERTY("FailureActionExitStatus", "i", bus_property_get_int,
                    offsetof(Unit, failure_action_exit_status),
                    SD_BUS_VTABLE_PROPERTY_CONST),
    SD_BUS_PROPERTY("SuccessAction", "s", property_get_emergency_action,
                    offsetof(Unit, success_action),
                    SD_BUS_VTABLE_PROPERTY_CONST),
    SD_BUS_PROPERTY("SuccessActionExitStatus", "i", bus_property_get_int,
                    offsetof(Unit, success_action_exit_status),
                    SD_BUS_VTABLE_PROPERTY_CONST),
    SD_BUS_PROPERTY("RebootArgument", "s", NULL, offsetof(Unit, reboot_arg),
                    SD_BUS_VTABLE_PROPERTY_CONST),
    SD_BUS_PROPERTY("InvocationID", "ay", bus_property_get_id128,
    ...
    SD_BUS_METHOD("Reload", "s", "o", method_reload,
                  SD_BUS_VTABLE_UNPRIVILEGED),
    SD_BUS_METHOD("Restart", "s", "o", method_restart,
                  SD_BUS_VTABLE_UNPRIVILEGED),
    SD_BUS_METHOD("TryRestart", "s", "o", method_try_restart,
                  SD_BUS_VTABLE_UNPRIVILEGED),
    SD_BUS_METHOD("ReloadOrRestart", "s", "o", method_reload_or_restart,
                  SD_BUS_VTABLE_UNPRIVILEGED),
    SD_BUS_METHOD("ReloadOrTryRestart", "s", "o", method_reload_or_try_restart,
                  SD_BUS_VTABLE_UNPRIVILEGED),
    SD_BUS_METHOD("EnqueueJob", "ss", "uososa(uosos)",
                  bus_unit_method_enqueue_job, SD_BUS_VTABLE_UNPRIVILEGED),
    SD_BUS_METHOD("Kill", "si", NULL, bus_unit_method_kill,
                  SD_BUS_VTABLE_UNPRIVILEGED),
    SD_BUS_METHOD("ResetFailed", NULL, NULL, bus_unit_method_reset_failed,
                  SD_BUS_VTABLE_UNPRIVILEGED),
    SD_BUS_METHOD("SetProperties", "ba(sv)", NULL,
                  bus_unit_method_set_properties, SD_BUS_VTABLE_UNPRIVILEGED),
    SD_BUS_METHOD("Ref", NULL, NULL, bus_unit_method_ref,
                  SD_BUS_VTABLE_UNPRIVILEGED),
    SD_BUS_METHOD("Unref", NULL, NULL, bus_unit_method_unref,
                  SD_BUS_VTABLE_UNPRIVILEGED),

    /* For dependency types we don't support anymore always return an empty
       array */
    SD_BUS_PROPERTY("RequiresOverridable", "as", property_get_empty_strv, 0,
                    SD_BUS_VTABLE_HIDDEN),
    SD_BUS_PROPERTY("RequisiteOverridable", "as", property_get_empty_strv, 0,
                    SD_BUS_VTABLE_HIDDEN),
    SD_BUS_PROPERTY("RequiredByOverridable", "as", property_get_empty_strv, 0,
                    SD_BUS_VTABLE_HIDDEN),
    SD_BUS_PROPERTY("RequisiteOfOverridable", "as", property_get_empty_strv, 0,
                    SD_BUS_VTABLE_HIDDEN),
    ...
    SD_BUS_PROPERTY("StartLimitIntervalSec", "t", bus_property_get_usec,
                    offsetof(Unit, start_limit.interval),
                    SD_BUS_VTABLE_PROPERTY_CONST | SD_BUS_VTABLE_HIDDEN),
    SD_BUS_VTABLE_END};

static int property_get_slice(sd_bus* bus, const char* path,
                              const char* interface, const char* property,
                              sd_bus_message* reply, void* userdata,
                              sd_bus_error* error)
{
...
static int property_get_current_memory(sd_bus* bus, const char* path,
                                       const char* interface,
                                       const char* property,
                                       sd_bus_message* reply, void* userdata,
                                       sd_bus_error* error)
{
    ...
    if (r < 0 && r != -ENODATA)
        log_unit_warning_errno(
            u, r, "Failed to get memory.usage_in_bytes attribute: %m");

    return sd_bus_message_append(reply, "t", sz);
}

static int property_get_current_tasks(sd_bus* bus, const char* path,
                                      const char* interface,
                                      const char* property,
                                      sd_bus_message* reply, void* userdata,
                                      sd_bus_error* error)
...
static int property_get_cpu_usage(sd_bus* bus, const char* path,
                                  const char* interface, const char* property,
                                  sd_bus_message* reply, void* userdata,
                                  sd_bus_error* error)
{
...
static int property_get_cgroup(sd_bus* bus, const char* path,
                               const char* interface, const char* property,
                               sd_bus_message* reply, void* userdata,
                               sd_bus_error* error)
{
...
static int append_process(sd_bus_message* reply, const char* p, pid_t pid,
                          Set* pids)
{
...
int bus_unit_method_get_processes(sd_bus_message* message, void* userdata,
                                  sd_bus_error* error)
{
    ...
    /* The main and control pids might live outside of the cgroup, hence fetch
     * them separately */
    pid = unit_main_pid(u);
    if (pid > 0)
    {
...
static int property_get_ip_counter(sd_bus* bus, const char* path,
                                   const char* interface, const char* property,
                                   sd_bus_message* reply, void* userdata,
                                   sd_bus_error* error)
{
...
int bus_unit_method_attach_processes(sd_bus_message* message, void* userdata,
                                     sd_bus_error* error)
{
    ...
    /* This migrates the processes with the specified PIDs into the cgroup of
     * this unit, optionally below a specified cgroup path. Obviously this only
     * works for units that actually maintain a cgroup representation. If a
     * process is already in the cgroup no operation is executed – in this case
     * the specified subcgroup path has no effect! */
    ...
    r = sd_bus_query_sender_creds(message, SD_BUS_CREDS_EUID | SD_BUS_CREDS_PID,
                                  &creds);
    if (r < 0)
        return r;
    ...
    for (;;)
    {
        ...
        /* Let's query the sender's UID, so that we can make our security
         * decisions */
        r = sd_bus_creds_get_euid(creds, &sender_uid);
        if (r < 0)
            return r;

        /* Let's validate security: if the sender is root, then all is OK. If
         * the sender is any other unit, then the process' UID and the target
         * unit's UID have to match the sender's UID */
        if (sender_uid != 0 && sender_uid != getuid())
        {
            ...
            if (process_uid != sender_uid)
                return sd_bus_error_setf(
                    ...
                    "Process " PID_FMT " not owned by client's UID. Refusing.",
                    pid);
    ...
    if (r < 0)
        return sd_bus_error_set_errnof(
            error, r, "Failed to attach processes to control group: %m");
...
const sd_bus_vtable bus_unit_cgroup_vtable[] = {
    ...
    SD_BUS_METHOD("GetProcesses", NULL, "a(sus)", bus_unit_method_get_processes,
                  SD_BUS_VTABLE_UNPRIVILEGED),
    SD_BUS_METHOD("AttachProcesses", "sau", NULL,
                  bus_unit_method_attach_processes, SD_BUS_VTABLE_UNPRIVILEGED),
    SD_BUS_VTABLE_END};
...
static int send_changed_signal(sd_bus* bus, void* userdata)
{
    ...
    r = sd_bus_emit_properties_changed_strv(
        bus, p, unit_dbus_interface_from_type(u->type), NULL);
    if (r < 0)
        return r;

    return sd_bus_emit_properties_changed_strv(
        bus, p, "org.freedesktop.systemd1.Unit", NULL);
}
...
void bus_unit_send_pending_change_signal(Unit* u, bool including_new)
{
    ...
    /* Sends out any pending change signals, but only if they really are
     * pending. This call is used when we are about to change state in order to
     * force out a PropertiesChanged signal beforehand if there was one pending
    ...
    if (!u->sent_dbus_new_signal &&
        !including_new) /* If the unit was never announced, don't bother, it's
                         * fine if the unit appears in the new state right-away
                         * (except if the
                         * caller explicitly asked us to send it anyway) */
        return;

    if (MANAGER_IS_RELOADING(
            u->manager)) /* Don't generate unnecessary PropertiesChanged signals
                          * for the same unit when we are reloading. */
        return;
...
void bus_unit_send_removed_signal(Unit* u)
{
    ...
    if (r < 0)
        log_unit_debug_errno(
            u, r, "Failed to send unit remove signal for %s: %m", u->id);
}

int bus_unit_queue_job(sd_bus_message* message, Unit* u, JobType type,
                       JobMode mode, BusUnitQueueFlags flags,
                       sd_bus_error* error)
{
    ...
    if (FLAGS_SET(flags, BUS_UNIT_QUEUE_RELOAD_IF_POSSIBLE) &&
        unit_can_reload(u))
    {
        if (type == JOB_RESTART)
    ...
    if ((type == JOB_START && u->refuse_manual_start) ||
        ...
        (IN_SET(type, JOB_RESTART, JOB_TRY_RESTART) &&
         (u->refuse_manual_start || u->refuse_manual_stop)) ||
        (type == JOB_RELOAD_OR_START &&
         job_type_collapse(type, u) == JOB_START && u->refuse_manual_start))
        return sd_bus_error_setf(
            error, BUS_ERROR_ONLY_BY_DEPENDENCY,
            "Operation refused, unit %s may be requested by dependency only "
    ...
    /* Before we send the method reply, force out the announcement JobNew for
     * this job */
    ...
    /* In verbose mode respond with the anchor job plus everything that has been
     * affected */
    r = sd_bus_message_new_method_return(message, &reply);
    if (r < 0)
        return r;
    ...
    r = sd_bus_message_append(reply, "uosos", j->id, job_path, j->unit->id,
                              unit_path, job_type_to_string(j->type));
    if (r < 0)
        return r;
    ...
    SET_FOREACH(a, affected, i)
    {
        ...
        r = sd_bus_message_append(reply, "(uosos)", a->id, job_path,
                                  a->unit->id, unit_path,
...
static int bus_unit_set_live_property(Unit* u, const char* name,
                                      ...
                                      UnitWriteFlags flags, sd_bus_error* error)
{
    ...
    /* Handles setting properties both "live" (i.e. at any time during runtime),
     * and during creation (for transient units that are being created). */
...
static int bus_set_transient_emergency_action(Unit* u, const char* name,
                                              EmergencyAction* p,
                                              sd_bus_message* message,
...
static int bus_set_transient_exit_status(Unit* u, const char* name, int* p,
                                         sd_bus_message* message,
                                         UnitWriteFlags flags,
                                         sd_bus_error* error)
{
...
static BUS_DEFINE_SET_TRANSIENT_PARSE(collect_mode, CollectMode,
                                      collect_mode_from_string);
static BUS_DEFINE_SET_TRANSIENT_PARSE(job_mode, JobMode, job_mode_from_string);

static int bus_set_transient_conditions(Unit* u, const char* name,
                                        Condition** list, bool is_condition,
                                        sd_bus_message* message,
                                        UnitWriteFlags flags,
                                        sd_bus_error* error)
{
    ...
    while ((r = sd_bus_message_read(message, "(sbbs)", &type_name, &trigger,
                                    &negate, &param)) > 0)
    {
        ConditionType t;

        t = is_condition ? condition_type_from_string(type_name)
        ...
        if (!UNIT_WRITE_FLAGS_NOOP(flags))
        {
            ...
            if (t != CONDITION_NULL)
                unit_write_settingf(u, flags | UNIT_ESCAPE_SPECIFIERS, name,
                                    "%s=%s%s%s", type_name, trigger ? "|" : "",
                                    negate ? "!" : "", param);
            else
                unit_write_settingf(u, flags, name, "%s=%s%s", type_name,
    ...
    if (!UNIT_WRITE_FLAGS_NOOP(flags) && empty)
    {
        *list = condition_free_list(*list);
        unit_write_settingf(u, flags, name,
                            "%sNull=", is_condition ? "Condition" : "Assert");
    }

    return 1;
}

static int bus_unit_set_transient_property(Unit* u, const char* name,
                                           sd_bus_message* message,
                                           UnitWriteFlags flags,
                                           sd_bus_error* error)
{
    ...
    /* Handles settings when transient units are created. This settings cannot
     * be altered anymore after the unit has been created. */

    if (streq(name, "SourcePath"))
        return bus_set_transient_path(u, name, &u->source_path, message, flags,
                                      error);

    if (streq(name, "StopWhenUnneeded"))
        return bus_set_transient_bool(u, name, &u->stop_when_unneeded, message,
                                      flags, error);

    if (streq(name, "RefuseManualStart"))
        return bus_set_transient_bool(u, name, &u->refuse_manual_start, message,
                                      flags, error);

    if (streq(name, "RefuseManualStop"))
        return bus_set_transient_bool(u, name, &u->refuse_manual_stop, message,
                                      flags, error);

    if (streq(name, "AllowIsolate"))
        return bus_set_transient_bool(u, name, &u->allow_isolate, message,
                                      flags, error);

    if (streq(name, "DefaultDependencies"))
        return bus_set_transient_bool(u, name, &u->default_dependencies,
                                      message, flags, error);

    if (streq(name, "OnFailureJobMode"))
        return bus_set_transient_job_mode(u, name, &u->on_failure_job_mode,
                                          message, flags, error);

    if (streq(name, "IgnoreOnIsolate"))
        return bus_set_transient_bool(u, name, &u->ignore_on_isolate, message,
                                      flags, error);

    if (streq(name, "JobTimeoutUSec"))
    {
        r = bus_set_transient_usec_fix_0(u, name, &u->job_timeout, message,
                                         flags, error);
        if (r >= 0 && !UNIT_WRITE_FLAGS_NOOP(flags) &&
            !u->job_running_timeout_set)
            u->job_running_timeout = u->job_timeout;
    }

    if (streq(name, "JobRunningTimeoutUSec"))
    {
        r = bus_set_transient_usec_fix_0(u, name, &u->job_running_timeout,
                                         message, flags, error);
        if (r >= 0 && !UNIT_WRITE_FLAGS_NOOP(flags))
    ...
    if (streq(name, "JobTimeoutRebootArgument"))
        return bus_set_transient_string(u, name, &u->job_timeout_reboot_arg,
                                        message, flags, error);

    if (streq(name, "StartLimitIntervalUSec"))
        return bus_set_transient_usec(u, name, &u->start_limit.interval,
                                      message, flags, error);

    if (streq(name, "StartLimitBurst"))
        return bus_set_transient_unsigned(u, name, &u->start_limit.burst,
                                          message, flags, error);
    ...
    if (streq(name, "FailureAction"))
        return bus_set_transient_emergency_action(u, name, &u->failure_action,
                                                  message, flags, error);

    if (streq(name, "SuccessAction"))
        return bus_set_transient_emergency_action(u, name, &u->success_action,
                                                  message, flags, error);
    ...
    if (streq(name, "RebootArgument"))
        return bus_set_transient_string(u, name, &u->reboot_arg, message, flags,
                                        error);

    if (streq(name, "CollectMode"))
        return bus_set_transient_collect_mode(u, name, &u->collect_mode,
                                              message, flags, error);

    if (streq(name, "Conditions"))
        return bus_set_transient_conditions(u, name, &u->conditions, true,
                                            message, flags, error);

    if (streq(name, "Asserts"))
        return bus_set_transient_conditions(u, name, &u->asserts, false,
                                            message, flags, error);
    ...
    else if (streq(name, "Slice"))
    {
        ...
        if (!UNIT_HAS_CGROUP_CONTEXT(u))
            return sd_bus_error_setf(error, SD_BUS_ERROR_INVALID_ARGS,
                                     "The slice property is only available for "
                                     "units with control groups.");
        ...
        /* Note that we do not dispatch the load queue here yet, as we don't
         * want our own transient unit to be loaded while we are still setting
         * it up. Or in other words, we use manager_load_unit_prepare()
    ...
    else if (streq(name, "RequiresMountsFor"))
    {
        ...
        STRV_FOREACH(p, l)
        {
            ...
            if (!path_is_absolute(*p))
                return sd_bus_error_setf(
                    error, SD_BUS_ERROR_INVALID_ARGS,
                    "Path specified in %s is not absolute: %s", name, *p);

            if (!path_is_valid(*p))
                return sd_bus_error_setf(
                    error, SD_BUS_ERROR_INVALID_ARGS,
                    "Path specified in %s has invalid length: %s", name, *p);

            if (!path_is_normalized(*p))
                return sd_bus_error_setf(
                    error, SD_BUS_ERROR_INVALID_ARGS,
                    "Path specified in %s is not normalized: %s", name, *p);
    ...
    if (d >= 0)
    {
        ...
        while ((r = sd_bus_message_read(message, "s", &other)) > 0)
        {
            ...
            if (!UNIT_WRITE_FLAGS_NOOP(flags))
            {
                ...
                r = unit_add_dependency_by_name(u, d, other, true,
                ...
                unit_write_settingf(u, flags, label, "%s=%s",
                                    unit_dependency_to_string(d), other);
            }
        }
        if (r < 0)
            return r;
    ...
    else if (streq(name, "AddRef"))
    {
        ...
        /* Why is this called "AddRef" rather than just "Ref", or "Reference"?
         * There's already a "Ref()" method on the Unit interface, and it's
         * probably not a good idea to expose a property and a method on the
         * same interface (well, strictly speaking AddRef isn't exposed as full
         * property, we just read it for transient units, but still). And
         * "References" and "ReferencedBy" is already used as unit reference
         * dependency type, hence let's not confuse things with that.
         *
         * Note that we don't acually add the reference to the bus track. We do
         * that only after the setup of the transient unit is complete, so that
         * setting this property multiple times in the same transient unit
         * creation call doesn't count as individual references. */
...
int bus_unit_set_properties(Unit* u, sd_bus_message* message,
                            UnitWriteFlags flags, bool commit,
                            sd_bus_error* error)
{
    ...
    for (;;)
    {
        ...
        if (!UNIT_VTABLE(u)->bus_set_property)
            return sd_bus_error_setf(
                error, SD_BUS_ERROR_PROPERTY_READ_ONLY,
...
int bus_unit_validate_load_state(Unit* u, sd_bus_error* error)
{
    ...
    switch (u->load_state)
    {
        ...
        case UNIT_ERROR: /* Only show .load_error in UNIT_ERROR state */
            return sd_bus_error_set_errnof(
                error, u->load_error, "Unit %s failed to load properly: %m.",
                u->id);
        ...
        default:
            return sd_bus_error_setf(error, BUS_ERROR_NO_SUCH_UNIT,
                                     "Unexpected load state of unit %s", u->id);
    }
}
...
static int bus_unit_track_handler(sd_bus_track* t, void* userdata)
{
    ...
    u->bus_track =
        sd_bus_track_unref(u->bus_track); /* make sure we aren't called again */

    /* If the client that tracks us disappeared, then there's reason to believe
     * that the cgroup is empty now too, let's see */
    unit_add_to_cgroup_empty_queue(u);

    /* Also add the unit to the GC queue, after all if the client left it might
     * be time to GC this unit */
...
static int bus_unit_allocate_bus_track(Unit* u)
{
    ...
    r = sd_bus_track_new(u->manager->api_bus, &u->bus_track,
                         bus_unit_track_handler, u);
    if (r < 0)
        return r;
...
int bus_unit_track_remove_sender(Unit* u, sd_bus_message* m)
{
    ...
    /* If we haven't allocated the bus track object yet, then there's definitely
     * no reference taken yet, return an error */
    if (!u->bus_track)
        return -EUNATCH;
