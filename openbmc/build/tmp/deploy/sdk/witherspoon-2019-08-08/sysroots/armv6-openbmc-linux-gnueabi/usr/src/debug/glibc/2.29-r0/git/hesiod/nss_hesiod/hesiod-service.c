...
LINE_PARSER("#", STRING_FIELD(result->s_name, ISSC_OR_SPACE, 1);
...
static enum nss_status lookup(const char* name, const char* type,
                              const char* protocol, struct servent* serv,
                              char* buffer, size_t buflen, int* errnop)
{
    ...
    do
    {
        ...
        if (parse_res > 0)
            found =
                protocol == NULL || strcasecmp(serv->s_proto, protocol) == 0;
...
enum nss_status _nss_hesiod_getservbyname_r(const char* name,
                                            const char* protocol,
...
enum nss_status _nss_hesiod_getservbyport_r(const int port,
                                            const char* protocol,
                                            struct servent* serv, char* buffer,
                                            size_t buflen, int* errnop)
{
