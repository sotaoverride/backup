...
{
    if (pool->sort_func)
        g_async_queue_push_sorted_unlocked(pool->queue, data, pool->sort_func,
                                           pool->sort_user_data);
    else
    ...
    do
    {
        ...
        if (pool == wakeup_thread_marker)
        {
            ...
            else
            {
                ...
                DEBUG_MSG(
                    ("thread %p updating to new limits.", g_thread_self()));
...
static gpointer g_thread_pool_wait_for_new_task(GRealThreadPool* pool)
{
    ...
    if (pool->running ||
        (!pool->immediate && g_async_queue_length_unlocked(pool->queue) > 0))
    {
        ...
        if (pool->num_threads > pool->max_threads && pool->max_threads != -1)
        {
            ...
            DEBUG_MSG(
                ("superfluous thread %p in pool %p.", g_thread_self(), pool));
        }
        ...
        else
        {
             ...
             */
            DEBUG_MSG(
                ("thread %p in pool %p waits for up to a 1/2 second for task "
                 "(%d running, %d unprocessed).",
                 g_thread_self(), pool, pool->num_threads,
                 g_async_queue_length_unlocked(pool->queue)));
    ...
    else
    {
        ...
        DEBUG_MSG(("pool %p not active, thread %p will go to global pool "
                   ...
                   pool, g_thread_self(), pool->running ? "true" : "false",
                                           ...
                                           GError** error)
{
    ...
    if (!success)
    {
        ...
        thread =
            g_thread_try_new(name, g_thread_pool_thread_proxy, pool, error);
 ...
 */
GThreadPool* g_thread_pool_new(GFunc func, gpointer user_data, gint max_threads,
 ...
 * Since: 2.10
 */
void g_thread_pool_set_sort_function(GThreadPool* pool, GCompareDataFunc func,
                                     gpointer user_data)
{
    GRealThreadPool* real;
    ...
    if (func)
        g_async_queue_sort_unlocked(real->queue, real->sort_func,
                                    real->sort_user_data);
