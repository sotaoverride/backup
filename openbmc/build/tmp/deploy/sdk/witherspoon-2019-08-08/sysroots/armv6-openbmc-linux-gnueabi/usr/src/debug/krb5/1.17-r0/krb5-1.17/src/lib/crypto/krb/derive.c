...
static krb5_key find_cached_dkey(struct derived_key* list,
...
static krb5_error_code add_cached_dkey(krb5_key key, const krb5_data* constant,
                                       const krb5_keyblock* dkeyblock,
                                       krb5_key* cached_dkey)
{
    krb5_key dkey;
    krb5_error_code ret;
...
static krb5_error_code
    derive_random_rfc3961(const struct krb5_enc_provider* enc, krb5_key inkey,
                          krb5_data* outrnd, const krb5_data* in_constant)
{
    ...
    else
    {
        krb5int_nfold(in_constant->length * 8,
                      (unsigned char*)in_constant->data, blocksize * 8,
                                   ...
                                   enum deriv_alg alg)
{
    ...
    *outkey = NULL;
    ...
    if (keyblock.contents == NULL)
        return ENOMEM;
    ret =
        krb5int_derive_keyblock(enc, hash, inkey, &keyblock, in_constant, alg);
    if (ret)
        goto cleanup;
