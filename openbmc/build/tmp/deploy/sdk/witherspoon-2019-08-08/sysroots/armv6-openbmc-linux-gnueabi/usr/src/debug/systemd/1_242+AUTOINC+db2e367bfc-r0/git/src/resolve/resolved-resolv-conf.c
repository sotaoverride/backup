 ...
 * uplink, i.e. the full uplink data */
#define PRIVATE_UPLINK_RESOLV_CONF "/run/systemd/resolve/resolv.conf"

/* A resolv.conf file containing the domain data we learnt from uplink, but our
 * own DNS server address. */
#define PRIVATE_STUB_RESOLV_CONF "/run/systemd/resolve/stub-resolv.conf"

/* A static resolv.conf file containing no domains, but only our own DNS server
 * address */
...
int manager_check_resolv_conf(const Manager* m)
{
    ...
    /* This warns only when our stub listener is disabled and /etc/resolv.conf
     * is a symlink to PRIVATE_STATIC_RESOLV_CONF or PRIVATE_STUB_RESOLV_CONF.
     */
    ...
    FOREACH_STRING(path, PRIVATE_STUB_RESOLV_CONF, PRIVATE_STATIC_RESOLV_CONF)
    {
        ...
        if (stat(path, &own) >= 0 && st.st_dev == own.st_dev &&
            st.st_ino == own.st_ino)
        {
            log_warning("DNSStubListener= is disabled, but /etc/resolv.conf is "
                        "a symlink to %s "
...
static bool file_is_our_own(const struct stat* st)
{
    ...
    FOREACH_STRING(path, PRIVATE_UPLINK_RESOLV_CONF, PRIVATE_STUB_RESOLV_CONF,
...
int manager_read_resolv_conf(Manager* m)
{
    ...
    for (;;)
    {
        ...
        if (a)
        {
            r = manager_parse_dns_server_string_and_warn(m, DNS_SERVER_SYSTEM,
                                                         a);
            if (r < 0)
                log_warning_errno(
                    r, "Failed to parse DNS server address '%s', ignoring.", a);

            continue;
        }

        a = first_word(l, "domain");
        if (!a) /* We treat "domain" lines, and "search" lines as equivalent,
                   and add both to our list. */
        ...
        if (a)
        {
            ...
            if (r < 0)
                log_warning_errno(
                    r, "Failed to parse search domain string '%s', ignoring.",
                    a);
        }

        log_syntax(NULL, LOG_DEBUG, "/etc/resolv.conf", n, 0,
                   "Ignoring resolv.conf line: %s", l);
    }
    ...
    /* If /etc/resolv.conf changed, make sure to forget everything we learned
     * about the DNS servers. After all we might now talk to a very different
     * DNS server that just happens to have the same IP address as an old one
     * (think 192.168.1.1). */
...
static void write_resolv_conf_server(DnsServer* s, FILE* f, unsigned* count)
{
    ...
    /* Check if the scope this DNS server belongs to is suitable as 'default'
     * route for lookups; resolv.conf does not have a syntax to express that, so
     * it must not appear as a global name server to avoid routing unrelated
     * domains to it (which is a privacy violation, will most probably fail
     * anyway, and adds unnecessary load) */
    scope = dns_server_scope(s);
    if (scope && !dns_scope_is_default_route(scope))
    {
        log_debug("Scope of DNS server %s has only route-only domains, not "
                  "using as global name server",
                  dns_server_string(s));
        return;
    }

    if (*count == MAXNS)
        fputs("# Too many DNS servers configured, the following entries may be "
              "ignored.\n",
              f);
    (*count)++;
...
static void write_resolv_conf_search(OrderedSet* domains, FILE* f)
{
    ...
    ORDERED_SET_FOREACH(domain, domains, i)
    {
        if (++count > MAXDNSRCH)
        {
            fputs("\n# Too many search domains configured, remaining ones "
                  "ignored.",
                  f);
            break;
        }
        ...
        if (length > 256)
        {
            fputs("\n# Total length of all search domains is too long, "
                                             ...
                                             OrderedSet* domains)
{
    ...
    fputs("# This file is managed by man:systemd-resolved(8). Do not edit.\n"
          ...
          "directly to\n"
          "# all known uplink DNS servers. This file lists all configured "
          "search domains.\n"
          "#\n"
...
static int write_stub_resolv_conf_contents(FILE* f, OrderedSet* dns,
                                           OrderedSet* domains)
{
    fputs_unlocked(
...
int manager_write_resolv_conf(Manager* m)
{
    ...
    if (r < 0)
        return log_warning_errno(r,
                                 "Failed to compile list of DNS servers: %m");

    r = manager_compile_search_domains(m, &domains, false);
    if (r < 0)
        return log_warning_errno(
            r, "Failed to compile list of search domains: %m");

    r = fopen_temporary_label(PRIVATE_UPLINK_RESOLV_CONF,
                              PRIVATE_UPLINK_RESOLV_CONF, &f_uplink,
                              &temp_path_uplink);
    if (r < 0)
        return log_warning_errno(
            r, "Failed to open private resolv.conf file for writing: %m");

    (void)__fsetlocking(f_uplink, FSETLOCKING_BYCALLER);
    (void)fchmod(fileno(f_uplink), 0644);

    r = fopen_temporary_label(PRIVATE_STUB_RESOLV_CONF,
                              PRIVATE_STUB_RESOLV_CONF, &f_stub,
                              &temp_path_stub);
    if (r < 0)
        return log_warning_errno(
            r, "Failed to open private stub-resolv.conf file for writing: %m");
    ...
    {
        r = log_error_errno(
            errno, "Failed to move private resolv.conf file into place: %m");
        goto fail;
    }
    ...
    {
        log_error_errno(
            r, "Failed to write private stub-resolv.conf contents: %m");
        goto fail;
    }
    ...
    if (rename(temp_path_stub, PRIVATE_STUB_RESOLV_CONF) < 0)
    {
        r = log_error_errno(
            errno,
            "Failed to move private stub-resolv.conf file into place: %m");
        goto fail;
    }
