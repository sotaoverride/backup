...
static int help(void)
{
    ...
    printf("%s [OPTIONS...] [VARIABLE=VALUE...]\n\n"
           ...
           "     --ready           Inform the init system about service "
           "start-up completion\n"
           "     --pid[=PID]       Set main PID of daemon\n"
           "     --uid=USER        Set user to send from\n"
           "     --status=TEXT     Set status text\n"
           "     --booted          Check if the system was booted up with "
           "systemd\n"
    ...
    while ((c = getopt_long(argc, argv, "h", options, NULL)) >= 0)
    {
        ...
        switch (c)
        {
            ...
            case ARG_UID:
            {
                ...
                if (r == -ESRCH) /* If the user doesn't exist, then accept it
                                    anyway as numeric */
                    r = parse_uid(u, &arg_uid);
                if (r < 0)
                    return log_error_errno(r, "Can't resolve user %s: %m",
                                           optarg);
...
static int run(int argc, char* argv[])
{
    ...
    /* If this is requested change to the requested UID/GID. Note thta we only
       change the real UID here, and leave the effective UID in effect (which is
       0 for this to work). That's because we want the privileges to fake the
