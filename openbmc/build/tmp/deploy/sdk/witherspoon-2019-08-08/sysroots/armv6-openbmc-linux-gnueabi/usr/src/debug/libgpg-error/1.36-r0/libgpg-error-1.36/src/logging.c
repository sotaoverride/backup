...
static int writen(int fd, const void* buffer, size_t nbytes, int is_socket)
{
    ...
    if (cookie->want_socket && cookie->fd == -1)
    {
        ...
        if (af == AF_LOCAL)
        {
            ...
            if (!*name && (name = socket_dir_cb()) && *name)
            {
                ...
                {
                    strncpy(srvr_addr_un.sun_path, name,
                            sizeof(srvr_addr_un.sun_path) - 1);
                    strcat(srvr_addr_un.sun_path, "/S.log");
                    srvr_addr_un.sun_path[sizeof(srvr_addr_un.sun_path) - 1] =
                        0;
...
/* Common function to either set the logging to a file or a file
   descriptor. */
static void set_file_fd(const char* name, int fd, estream_t stream)
{
    estream_t fp;
    int want_socket = 0;
#ifdef HAVE_W32CE_SYSTEM
    ...
    else
    {
        do
            cookie->fd = open(
                name, O_WRONLY | O_APPEND | O_CREAT,
...
int _gpgrt_log_get_fd()
{
    return logstream ? _gpgrt_fileno(logstream) : -1;
}

estream_t _gpgrt_log_get_stream()
{
    if (!logstream)
    {
 ...
 * "%s" format specifiers.  */
static char* fmt_string_filter(const char* string, int no, void* opaque)
{
...
static int print_prefix(int level, int leading_backspace)
{
    int rc;
    int length = 0;
    ...
    { /* Note this does not work for multiple line logging as we would
       * need to print to a buffer first */
        if (with_time && !force_prefixes)
        {
            ...
            tp = localtime(&atime);
            rc = _gpgrt_fprintf_unlocked(
                logstream, "%04d-%02d-%02d %02d:%02d:%02d ", 1900 + tp->tm_year,
    ...
    switch (level)
    {
        case GPGRT_LOGLVL_BEGIN:
            break;
        case GPGRT_LOGLVL_CONT:
            break;
        case GPGRT_LOGLVL_INFO:
            break;
        case GPGRT_LOGLVL_WARN:
            break;
        case GPGRT_LOGLVL_ERROR:
            break;
 ...
 * line ends in a LF. */
int _gpgrt_logv_internal(int level, int ignore_arg_ptr, const char* extrastring,
                         const char* prefmt, const char* fmt, va_list arg_ptr)
{
    ...
    if (!logstream)
    {
        ...
        tmp = (no_registry
                   ? NULL
                   : _gpgrt_w32_reg_query_string(
                         NULL, "Software\\\\GNU\\\\GnuPG", "DefaultLogFile"));
...
void _gpgrt_log(int level, const char* fmt, ...)
{
    va_list arg_ptr;
...
void _gpgrt_logv_prefix(int level, const char* prefix, const char* fmt,
                        va_list arg_ptr)
{
...
void _gpgrt_log_info(const char* fmt, ...)
{
    va_list arg_ptr;
...
void _gpgrt_log_error(const char* fmt, ...)
{
    va_list arg_ptr;
...
void _gpgrt_log_fatal(const char* fmt, ...)
{
    va_list arg_ptr;
...
void _gpgrt_log_bug(const char* fmt, ...)
{
    va_list arg_ptr;
...
void _gpgrt_log_debug(const char* fmt, ...)
{
    va_list arg_ptr;
...
void _gpgrt_log_printf(const char* fmt, ...)
{
    va_list arg_ptr;

    va_start(arg_ptr, fmt);
    _gpgrt_logv_internal(fmt ? GPGRT_LOGLVL_CONT : GPGRT_LOGLVL_BEGIN, 0, NULL,
                         NULL, fmt, arg_ptr);
    va_end(arg_ptr);
}
...
void _gpgrt_logv_printhex(const void* buffer, size_t length, const char* fmt,
                          va_list arg_ptr)
{
    int wrap = 0;
    int cnt = 0;
...
void _gpgrt_log_printhex(const void* buffer, size_t length, const char* fmt,
                         ...)
{
    va_list arg_ptr;
...
void _gpgrt_logv_clock(const char* fmt, va_list arg_ptr)
{
    ...
    _gpgrt_logv_internal(GPGRT_LOGLVL_DEBUG, 0, NULL, "[no clock] ", fmt,
                         arg_ptr);
...
/* Print a microsecond timestamp followed by FMT.  */
void _gpgrt_log_clock(const char* fmt, ...)
{
    va_list arg_ptr;
