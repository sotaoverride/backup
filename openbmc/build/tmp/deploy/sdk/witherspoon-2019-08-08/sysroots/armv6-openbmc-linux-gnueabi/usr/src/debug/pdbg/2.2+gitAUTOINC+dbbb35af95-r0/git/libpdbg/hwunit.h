...
#define DECLARE_HW_UNIT(name)                                                  \
    ...
    {                                                                          \
        ((void)sizeof(                                                         \
            char[1 - 2 * (container_off(typeof(name), target) != 0)]));        \
    }                                                                          \
    const struct hw_unit_info __used name##_hw_unit = {.hw_unit = &name,       \
                                                       .size = sizeof(name)};
...
struct sbefifo
{
    ...
    int (*chipop)(struct sbefifo*, uint32_t*, uint32_t, uint8_t**, uint32_t*,
                  uint32_t*);
