...
#define zrleOutStreamWRITE_PIXEL                                               \
    __RFB_CONCAT2E(zrleOutStreamWriteOpaque, CPIXEL)
...
static const int bitsPerPackedPixel[] = {0, 1, 2, 2, 4, 4, 4, 4,
                                         4, 4, 4, 4, 4, 4, 4, 4};
...
static void ZRLE_ENCODE(int x, int y, int w, int h, zrleOutStream* os,
                        void* buf EXTRA_ARGS)
{
    int ty;
    for (ty = y; ty < y + h; ty += rfbZRLETileHeight)
    {
        ...
        for (tx = x; tx < x + w; tx += rfbZRLETileWidth)
        {
            ...
            ZRLE_ENCODE_TILE((PIXEL_T*)buf, tw, th, os, cl->zywrleLevel,
                             cl->zywrleBuf, cl->paletteHelper);
        }
    }
                      ...
                      int zywrle_level, int* zywrleBuf, void* paletteHelper)
{
    ...
    PIXEL_T* end = ptr + h * w;
    *end =
        ~*(end - 1); /* one past the end is different so the while loop ends */
