...
struct ext2_resize_struct
{
     ...
     */
    errcode_t (*progress)(ext2_resize_t rfs, int pass, unsigned long cur,
                          unsigned long max);
    void* prog_data;
};
...
extern errcode_t ext2fs_add_extent_entry(ext2_extent extent, __u64 old_loc,
                                         __u64 new_loc);
...
extern errcode_t ext2fs_progress_init(ext2_sim_progmeter* ret_prog,
                                      const char* label, int labelwidth,
                                      int barwidth, __u32 maxdone, int flags);
extern void ext2fs_progress_update(ext2_sim_progmeter prog, __u32 current);
