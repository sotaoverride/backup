...
static void do_encrypt(BLOWFISH_context* bc, u32* ret_xl, u32* ret_xr)
{
#ifdef HAVE_COMPATIBLE_GCC_WIN64_PLATFORM_AS
...
static void do_encrypt_block(BLOWFISH_context* context, byte* outbuf,
                             const byte* inbuf)
{
#ifdef HAVE_COMPATIBLE_GCC_WIN64_PLATFORM_AS
...
static void do_decrypt_block(BLOWFISH_context* context, byte* outbuf,
                             const byte* inbuf)
{
#ifdef HAVE_COMPATIBLE_GCC_WIN64_PLATFORM_AS
...
static inline void blowfish_amd64_ctr_enc(BLOWFISH_context* ctx, byte* out,
                                          const byte* in, byte* ctr)
{
#ifdef HAVE_COMPATIBLE_GCC_WIN64_PLATFORM_AS
...
static inline void blowfish_amd64_cbc_dec(BLOWFISH_context* ctx, byte* out,
                                          const byte* in, byte* iv)
{
#ifdef HAVE_COMPATIBLE_GCC_WIN64_PLATFORM_AS
...
static inline void blowfish_amd64_cfb_dec(BLOWFISH_context* ctx, byte* out,
                                          const byte* in, byte* iv)
{
#ifdef HAVE_COMPATIBLE_GCC_WIN64_PLATFORM_AS
...
static unsigned int encrypt_block(void* context, byte* outbuf,
                                  const byte* inbuf)
{
...
static unsigned int decrypt_block(void* context, byte* outbuf,
                                  const byte* inbuf)
{
    BLOWFISH_context* c = (BLOWFISH_context*)context;
...
static inline u32 function_F(BLOWFISH_context* bc, u32 x)
{
    u16 a, b, c, d;
...
#ifdef WORDS_BIGENDIAN
#define F(x)                                                                   \
    (((s0[((byte*)&x)[0]] + s1[((byte*)&x)[1]]) ^ s2[((byte*)&x)[2]]) +        \
     s3[((byte*)&x)[3]])
#else
#define F(x)                                                                   \
    (((s0[((byte*)&x)[3]] + s1[((byte*)&x)[2]]) ^ s2[((byte*)&x)[1]]) +        \
     s3[((byte*)&x)[0]])
#endif
#define R(l, r, i)                                                             \
    do                                                                         \
...
static void decrypt(BLOWFISH_context* bc, u32* ret_xl, u32* ret_xr)
{
#if BLOWFISH_ROUNDS == 16
    u32 xl, xr, *s0, *s1, *s2, *s3, *p;
...
static void do_encrypt_block(BLOWFISH_context* bc, byte* outbuf,
                             const byte* inbuf)
{
    u32 d1, d2;
...
static void do_decrypt_block(BLOWFISH_context* bc, byte* outbuf,
                             const byte* inbuf)
{
    u32 d1, d2;
...
void _gcry_blowfish_ctr_enc(void* context, unsigned char* ctr, void* outbuf_arg,
...
void _gcry_blowfish_cbc_dec(void* context, unsigned char* iv, void* outbuf_arg,
                            const void* inbuf_arg, size_t nblocks)
{
    BLOWFISH_context* ctx = context;
    unsigned char* outbuf = outbuf_arg;
...
/* Bulk decryption of complete blocks in CFB mode.  This function is only
   intended for the bulk encryption feature of cipher.c. */
void _gcry_blowfish_cfb_dec(void* context, unsigned char* iv, void* outbuf_arg,
                            const void* inbuf_arg, size_t nblocks)
{
    BLOWFISH_context* ctx = context;
    unsigned char* outbuf = outbuf_arg;
...
static const char* selftest_ctr(void)
{
    ...
    return _gcry_selftest_helper_ctr("BLOWFISH", &bf_setkey, &encrypt_block,
                                     &_gcry_blowfish_ctr_enc, nblocks,
                                     blocksize, context_size);
}
...
static const char* selftest_cbc(void)
{
    ...
    return _gcry_selftest_helper_cbc("BLOWFISH", &bf_setkey, &encrypt_block,
                                     &_gcry_blowfish_cbc_dec, nblocks,
                                     blocksize, context_size);
}
...
static const char* selftest_cfb(void)
{
    ...
    return _gcry_selftest_helper_cfb("BLOWFISH", &bf_setkey, &encrypt_block,
...
static const char* selftest(void)
{
    ...
    byte plain[] = "BLOWFISH";
    byte buffer[8];
    static const byte plain3[] = {0xFE, 0xDC, 0xBA, 0x98,
                                  0x76, 0x54, 0x32, 0x10};
    static const byte key3[] = {0x41, 0x79, 0x6E, 0xA0, 0x52, 0x61, 0x6E, 0xE4};
    static const byte cipher3[] = {0xE1, 0x13, 0xF4, 0x10,
                                   0x2C, 0xFC, 0xCE, 0x43};
    const char* r;

    bf_setkey((void*)&c, (const unsigned char*)"abcdefghijklmnopqrstuvwxyz",
              26);
...
static inline byte val_to_hidx(u32 val)
{
    /* bf sboxes are quite random already. */
    return (val >> 24) ^ (val >> 16) ^ (val >> 8) ^ val;
}

static inline int add_val(struct hashset_elem hset[256], u32 val, int* midx,
...
static gcry_err_code_t do_bf_setkey(BLOWFISH_context* c, const byte* key,
                                    unsigned keylen)
{
    struct hashset_elem
        mempool[4 * 255]; /* Enough entries for the worst case. */
    ...
    {
        data = ((u32)key[j] << 24) | ((u32)key[(j + 1) % keylen] << 16) |
               ((u32)key[(j + 2) % keylen] << 8) | ((u32)key[(j + 3) % keylen]);
        c->p[i] ^= data;
        j = (j + 4) % keylen;
    }
...
static gcry_err_code_t bf_setkey(void* context, const byte* key,
                                 unsigned keylen)
{
    BLOWFISH_context* c = (BLOWFISH_context*)context;
...
gcry_cipher_spec_t _gcry_cipher_spec_blowfish = {GCRY_CIPHER_BLOWFISH,
                                                 {0, 0},
                                                 "BLOWFISH",
                                                 NULL,
                                                 NULL,
