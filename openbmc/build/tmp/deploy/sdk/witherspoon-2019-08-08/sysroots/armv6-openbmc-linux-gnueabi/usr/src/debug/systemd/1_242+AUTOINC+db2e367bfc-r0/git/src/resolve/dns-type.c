...
static const struct dns_type_name* lookup_dns_type(register const char* str,
...
bool dns_type_is_pseudo(uint16_t type)
{
    ...
    return IN_SET(type, 0, /* A Pseudo RR type, according to RFC 2931 */
                  DNS_TYPE_ANY, DNS_TYPE_AXFR, DNS_TYPE_IXFR, DNS_TYPE_OPT,
                  DNS_TYPE_TSIG, DNS_TYPE_TKEY);
}
...
bool dns_type_is_valid_query(uint16_t type)
{
    ...
    return !IN_SET(type, 0, DNS_TYPE_OPT, DNS_TYPE_TSIG, DNS_TYPE_TKEY,

                   /* RRSIG are technically valid as questions, but we refuse
                    * doing explicit queries for them, as they aren't really
                    * payload, but signatures for payload, and cannot be
                    * validated on their own. After all they are the signatures,
                    * and have no signatures of their own validating them. */
                   DNS_TYPE_RRSIG);
}
...
bool dns_type_may_redirect(uint16_t type)
{
    ...
    return !IN_SET(type, DNS_TYPE_CNAME, DNS_TYPE_DNAME, DNS_TYPE_NSEC3,
                   DNS_TYPE_NSEC, DNS_TYPE_RRSIG, DNS_TYPE_NXT, DNS_TYPE_SIG,
                   DNS_TYPE_KEY);
}
...
bool dns_type_may_wildcard(uint16_t type)
{
    ...
    return !IN_SET(
        type, DNS_TYPE_NSEC3, DNS_TYPE_SOA,
...
bool dns_type_apex_only(uint16_t type)
{
    ...
    return IN_SET(
        ...
        DNS_TYPE_DNSKEY, DNS_TYPE_NSEC3PARAM);
}

bool dns_type_is_dnssec(uint16_t type)
{
    return IN_SET(type, DNS_TYPE_DS, DNS_TYPE_DNSKEY, DNS_TYPE_RRSIG,
                  DNS_TYPE_NSEC, DNS_TYPE_NSEC3, DNS_TYPE_NSEC3PARAM);
}
...
bool dns_type_is_obsolete(uint16_t type)
{
    return IN_SET(
        ...
        DNS_TYPE_MD, DNS_TYPE_MF, DNS_TYPE_MAILA,

        /* Kinda obsoleted by RFC 2505 */
        DNS_TYPE_MB, DNS_TYPE_MG, DNS_TYPE_MR, DNS_TYPE_MINFO, DNS_TYPE_MAILB,
...
bool dns_type_needs_authentication(uint16_t type)
{
    ...
    /* Returns true for all (non-obsolete) RR types where records are not useful
     * if they aren't
     * authenticated. I.e. everything that contains crypto keys. */

    return IN_SET(type, DNS_TYPE_CERT, DNS_TYPE_SSHFP, DNS_TYPE_IPSECKEY,
                  DNS_TYPE_DS, DNS_TYPE_DNSKEY, DNS_TYPE_TLSA, DNS_TYPE_CDNSKEY,
                  DNS_TYPE_OPENPGPKEY, DNS_TYPE_CAA);
}
