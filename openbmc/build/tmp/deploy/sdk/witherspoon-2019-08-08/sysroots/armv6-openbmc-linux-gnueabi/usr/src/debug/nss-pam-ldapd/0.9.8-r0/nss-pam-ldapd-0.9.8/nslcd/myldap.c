...
#define MYLDAP_SCOPE_BINDONLY                                                  \
    0x1972 /* magic number: should never be a real scope */
...
static void myldap_err(int pri, LDAP* ld, int rc, const char* format, ...)
{
    ...
    /* log the message */
    log_log(pri, "%s%s%s%s%s%s%s", message, (msg_ldap == NULL) ? "" : ": ",
            (msg_ldap == NULL) ? "" : msg_ldap, (msg_diag == NULL) ? "" : ": ",
            (msg_diag == NULL) ? "" : msg_diag, (msg_errno == NULL) ? "" : ": ",
            (msg_errno == NULL) ? "" : msg_errno);
...
static MYLDAP_ENTRY* myldap_entry_new(MYLDAP_SEARCH* search)
{
    ...
    {
        log_log(LOG_CRIT,
                "myldap_entry_new(): malloc() failed to allocate memory");
        exit(EXIT_FAILURE);
    }
    /* fill in fields */
    entry->search = search;
                                        ...
                                        const char* filter, const char** attrs)
{
    ...
    {
        log_log(LOG_CRIT,
                "myldap_search_new(): malloc() failed to allocate memory");
        exit(EXIT_FAILURE);
    }
    /* initialize struct */
...
static MYLDAP_SESSION* myldap_session_new(void)
{
    ...
    {
        log_log(LOG_CRIT,
                "myldap_session_new(): malloc() failed to allocate memory");
        exit(EXIT_FAILURE);
    }
...
PURE static inline int is_valid_entry(MYLDAP_ENTRY* entry)
{
    return (entry != NULL) && (entry->search != NULL) &&
           (entry->search->session != NULL) &&
           (entry->search->session->ld != NULL) && (entry->search->msg != NULL);
}
    ...
    while (interact->id != SASL_CB_LIST_END)
    {
        switch (interact->id)
        {
            case SASL_CB_GETREALM:
                ...
                {
                    log_log(LOG_DEBUG,
                            "do_sasl_interact(): returning sasl_realm \"%s\"",
                            cfg->sasl_realm);
                    interact->result = cfg->sasl_realm;
                    interact->len = strlen(cfg->sasl_realm);
                }
                else
                    log_log(LOG_DEBUG, "do_sasl_interact(): were asked for "
                                       "sasl_realm but we don't have any");
                break;
            case SASL_CB_AUTHNAME:
                if (cfg->sasl_authcid)
                {
                    log_log(LOG_DEBUG,
                            "do_sasl_interact(): returning sasl_authcid \"%s\"",
                            cfg->sasl_authcid);
                    ...
                    log_log(LOG_DEBUG, "do_sasl_interact(): were asked for "
                                       "sasl_authcid but we don't have any");
                break;
            case SASL_CB_USER:
                if (cfg->sasl_authzid)
                {
                    log_log(LOG_DEBUG,
                            "do_sasl_interact(): returning sasl_authzid \"%s\"",
                            cfg->sasl_authzid);
                    ...
                    log_log(LOG_DEBUG, "do_sasl_interact(): were asked for "
                                       "sasl_authzid but we don't have any");
                break;
            case SASL_CB_PASS:
                if (cfg->bindpw)
                {
                    log_log(LOG_DEBUG,
                            "do_sasl_interact(): returning bindpw \"***\"");
                    interact->result = cfg->bindpw;
                    interact->len = strlen(cfg->bindpw);
                }
                else
                    log_log(LOG_DEBUG, "do_sasl_interact(): were asked for "
                                       "bindpw but we don't have any");
                break;
            default:
                /* just ignore */
                break;
                                    ...
                                    LDAPControl** ctrls)
{
    ...
    for (i = 0; ctrls[i] != NULL; i++)
    {
        ...
        {
            /* check for expired control: force the user to change their
             * password */
            log_log(LOG_DEBUG, "got LDAP_CONTROL_PWEXPIRED (password expired, "
                               "user should change)");
        ...
        {
            /* check for password expiration warning control: the password is
               about to expire (returns the number of seconds remaining until
            ...
            sec = atol(seconds);
            log_log(LOG_DEBUG,
                    "got LDAP_CONTROL_PWEXPIRING (password will expire in %ld "
                    "seconds)",
                    sec);
                        ...
                        LDAP_CONTROL_PASSWORDPOLICYRESPONSE) == 0)
        {
            ...
            rc = ldap_parse_passwordpolicy_control(ld, ctrls[i], &expire,
                                                   &grace, &error);
            if (rc != LDAP_SUCCESS)
                myldap_err(
                    LOG_WARNING, ld, rc,
                    "ldap_parse_passwordpolicy_control() failed (ignored)");
            else
            {
                /* log returned control information */
                log_log(LOG_DEBUG,
                        "got LDAP_CONTROL_PASSWORDPOLICYRESPONSE (%s)",
                        ldap_passwordpolicy_err2txt(error));
                if (expire >= 0)
                    log_log(LOG_DEBUG,
                            "got LDAP_CONTROL_PASSWORDPOLICYRESPONSE (password "
                            "will expire in %d seconds)",
                            expire);
                if (grace >= 0)
                    log_log(LOG_DEBUG,
                            "got LDAP_CONTROL_PASSWORDPOLICYRESPONSE (%d grace "
                            "logins left)",
                            grace);
                ...
                if ((error == PP_passwordExpired) &&
                    ((session->policy_response == NSLCD_PAM_SUCCESS) ||
                     (session->policy_response == NSLCD_PAM_NEW_AUTHTOK_REQD)))
                {
                    /* this means that the password has expired and must be
                     * reset */
                    session->policy_response = NSLCD_PAM_NEW_AUTHTOK_REQD;
                    mysnprintf(session->policy_message,
                               sizeof(session->policy_message), "%s",
                               ldap_passwordpolicy_err2txt(error));
                }
                else if ((error == PP_accountLocked) &&
                         ((session->policy_response == NSLCD_PAM_SUCCESS) ||
                          (session->policy_response ==
                           NSLCD_PAM_NEW_AUTHTOK_REQD)))
                {
                    /* this means that the account is locked and the user cannot
                       log in (the bind probably failed already) */
                    ...
                    mysnprintf(session->policy_message,
                         ...
                         (session->policy_response == NSLCD_PAM_SUCCESS))
                {
                    /* this indicates that the password must be changed before
                       the user is allowed to perform any other operation */
                    session->policy_response = NSLCD_PAM_NEW_AUTHTOK_REQD;
                    mysnprintf(session->policy_message,
                               sizeof(session->policy_message), "%s",
                               ldap_passwordpolicy_err2txt(error));
                         ...
                         ((session->policy_response == NSLCD_PAM_SUCCESS) ||
                          (session->policy_response ==
                           NSLCD_PAM_NEW_AUTHTOK_REQD)))
                {
                    /* any other error is assumed to mean that the operation
                     * failed */
                ...
                {
                    /* if no other error has happened, this indicates that the
                       password will soon expire (number of seconds) */
                    print_ppolicy_expiry(session, (unsigned int)expire);
                }
                else if ((grace >= 0) && (strlen(session->policy_message) == 0))
                {
                    /* this indicates the number of grace logins that are left
                       before no further login attempts will be allowed */
                    mysnprintf(session->policy_message,
...
static int do_ppolicy_bind(MYLDAP_SESSION* session, LDAP* ld, const char* uri)
{
    ...
    if (msgid == -1)
    {
        myldap_err(LOG_WARNING, ld, rc,
                   "ldap_sasl_bind() failed (msgid=-1, uri=%s)", uri);
    ...
    if (nslcd_cfg->pam_authc_ppolicy)
    {
        /* parse the result from the bind operation (frees result, gets
         * controls) */
...
static int do_bind(MYLDAP_SESSION* session, LDAP* ld, const char* uri)
{
    ...
    if (nslcd_cfg->ssl == SSL_START_TLS)
    {
        ...
        {
            myldap_err(LOG_WARNING, ld, rc,
                       "ldap_start_tls_s() failed (uri=%s)", uri);
            return rc;
        }
    }
    ...
    if (nslcd_cfg->sasl_mech != NULL)
    {
        ...
        {
            log_log(LOG_DEBUG,
                    "ldap_set_option(LDAP_OPT_X_SASL_SECPROPS,\"%s\")",
                    nslcd_cfg->sasl_secprops);
            LDAP_SET_OPTION(ld, LDAP_OPT_X_SASL_SECPROPS,
                            (void*)nslcd_cfg->sasl_secprops);
        }
#ifdef HAVE_SASL_INTERACT_T
        if (nslcd_cfg->binddn != NULL)
            log_log(LOG_DEBUG,
                    "ldap_sasl_interactive_bind_s(\"%s\",\"%s\") (uri=\"%s\")",
                    nslcd_cfg->binddn, nslcd_cfg->sasl_mech, uri);
        else
            log_log(LOG_DEBUG,
                    "ldap_sasl_interactive_bind_s(NULL,\"%s\") (uri=\"%s\")",
                    nslcd_cfg->sasl_mech, uri);
        return ldap_sasl_interactive_bind_s(
            ld, nslcd_cfg->binddn, nslcd_cfg->sasl_mech, NULL, NULL,
            LDAP_SASL_QUIET, do_sasl_interact, (void*)nslcd_cfg);
        ...
        if (nslcd_cfg->binddn != NULL)
            ...
            log_log(LOG_DEBUG, "ldap_sasl_bind_s(NULL,\"%s\",%s) (uri=\"%s\")",
                    nslcd_cfg->sasl_mech,
                    nslcd_cfg->bindpw ? "\"***\"" : "NULL", uri);
        return ldap_sasl_bind_s(ld, nslcd_cfg->binddn, nslcd_cfg->sasl_mech,
                                &cred, NULL, NULL, NULL);
#endif /* not HAVE_SASL_INTERACT_T */
    }
#endif /* HAVE_LDAP_SASL_INTERACTIVE_BIND_S */
    ...
    if (nslcd_cfg->binddn)
        log_log(LOG_DEBUG, "ldap_simple_bind_s(\"%s\",%s) (uri=\"%s\")",
                nslcd_cfg->binddn, nslcd_cfg->bindpw ? "\"***\"" : "NULL", uri);
    else
...
static int do_rebind(LDAP* ld, LDAP_CONST char* url, ber_tag_t UNUSED(request),
        ...
        *authmethodp = LDAP_AUTH_SIMPLE;
        if ((*dnp == NULL) || (*passwdp == NULL))
        {
            if (*dnp != NULL)
                free(*dnp);
            log_log(LOG_CRIT,
                    "do_rebind(): strdup() failed to allocate memory");
...
static int set_socket_timeout(LDAP* ld, time_t sec, suseconds_t usec)
{
    ...
    int rc = LDAP_SUCCESS;
    int sd;
    log_log(LOG_DEBUG, "set_socket_timeout(%lu,%lu)", (unsigned long)sec,
            (unsigned long)usec);
    /* get the socket */
    ...
    {
        log_log(LOG_ERR, "setsockopt(%d,SO_RCVTIMEO) failed: %s", sd,
                strerror(errno));
        rc = LDAP_LOCAL_ERROR;
    }
...
/* This function is called by the LDAP library once a connection was made to the
   server. We set a timeout on the socket here, to catch network timeouts during
   the ssl handshake phase. It is configured with LDAP_OPT_CONNECT_CB. */
...
static int do_set_options(MYLDAP_SESSION* session)
{
    ...
    log_log(LOG_DEBUG, "ldap_set_option(LDAP_OPT_DEREF,%d)", nslcd_cfg->deref);
    ...
    if ((nslcd_cfg->ssl == SSL_LDAPS) ||
        (strncasecmp(nslcd_cfg->uris[session->current_uri].uri, "ldaps://",
                     8) == 0))
    {
        /* use tls */
    ...
    if (nslcd_cfg->sasl_canonicalize >= 0)
    {
        ...
        LDAP_SET_OPTION(session->ld, LDAP_OPT_X_SASL_NOCANON,
                        nslcd_cfg->sasl_canonicalize ? LDAP_OPT_OFF
                                                     : LDAP_OPT_ON);
    }
        ...
        for (i = 0; i < MAX_SEARCHES_IN_SESSION; i++)
        {
            if (session->searches[i] != NULL)
            {
                ...
                if (session->searches[i]->msgid != -1)
                {
                    ...
                    {
                        if (ldap_get_option(session->ld, LDAP_OPT_ERROR_NUMBER,
                                            &rc) != LDAP_SUCCESS)
                            rc = LDAP_OTHER;
    ...
    if (session->ld != NULL)
    {
        ...
        else
        {
            ...
            if (getpeername(sd, &sa, &salen) == -1)
            {
                ...
                {
                    log_log(LOG_DEBUG,
                            "myldap_session_check(): connection reset by peer");
                    do_close(session);
                    return;
                }
            }
        ...
        if (nslcd_cfg->idle_timelimit > 0)
        {
            ...
            for (i = 0; i < MAX_SEARCHES_IN_SESSION; i++)
                if ((session->searches[i] != NULL) &&
                    (session->searches[i]->valid))
                    return;
            /* consider timeout (there are no running searches) */
            time(&current_time);
            if ((session->lastactivity + nslcd_cfg->idle_timelimit) <
                current_time)
            {
                log_log(LOG_DEBUG,
                        "myldap_session_check(): idle_timelimit reached");
                do_close(session);
            }
        }
    }
                ...
                int* response, const char** message)
{
    ...
    {
        log_log(LOG_ERR,
                "myldap_bind(): binddn buffer too small (%lu required)",
    ...
    {
        log_log(LOG_ERR,
                "myldap_bind(): bindpw buffer too small (%lu required)",
...
static int do_try_search(MYLDAP_SEARCH* search)
{
    ...
    rc = ldap_search_ext(search->session->ld, search->base, search->scope,
                         search->filter, (char**)(search->attrs), 0,
                         serverctrls[0] == NULL ? NULL : serverctrls, NULL,
    ...
    while (1)
    {
        ...
        do
        {
            ...
            { /* skip this URI */
                      ...
                      (current_uri->firstfail +
                       nslcd_cfg->reconnect_retrytime)) &&
                     ((t = time(NULL)) <
                      (current_uri->lastfail + nslcd_cfg->reconnect_retrytime)))
            {
                /* we are in a hard fail state and have retried not long ago */
                log_log(LOG_DEBUG,
                        "not retrying server %s which failed just %d second(s) "
                        "ago and has been failing for %d seconds",
            ...
            else
            {
                ...
                if ((rc == LDAP_SUCCESS) &&
                    (search->scope != MYLDAP_SCOPE_BINDONLY))
                    rc = do_try_search(search);
                /* if we are authenticating a user and get an error regarding
                   failed password we should error out instead of trying all
                   servers */
                ...
                if (rc == LDAP_SUCCESS)
                {
                    ...
                    if ((current_uri->lastfail > 0) ||
                        (search->session->current_uri != start_uri))
                    {
        ...
        {
            if (search->session->binddn[0] == '\0')
                myldap_err(LOG_ERR, search->session->ld, rc,
        ...
        {
            log_log(LOG_WARNING,
                    "no available LDAP server found, sleeping %d seconds",
                    sleeptime);
...
void myldap_immediate_reconnect(void)
{
    ...
    for (i = 0; i < (NSS_LDAP_CONFIG_MAX_URIS + 1); i++)
    {
        ...
        if ((nslcd_cfg->uris[i].lastfail > t) &&
            (nslcd_cfg->uris[i].lastfail >
             (nslcd_cfg->uris[i].firstfail + nslcd_cfg->reconnect_retrytime)))
        {
            /* move lastfail back to ensure quick retry */
            log_log(LOG_DEBUG,
                    "moving lastfail of %s %d second(s) back to force retry",
                    nslcd_cfg->uris[i].uri,
                    (int)(nslcd_cfg->uris[i].lastfail - t));
...
MYLDAP_SEARCH* myldap_search(MYLDAP_SESSION* session, const char* base,
                             int scope, const char* filter, const char** attrs,
                             int* rcp)
{
    MYLDAP_SEARCH* search;
    int i;
    int rc;
    /* check parameters */
    if ((session == NULL) || (base == NULL) || (filter == NULL) ||
        (attrs == NULL))
    {
    ...
    /* log the call */
    log_log(LOG_DEBUG, "myldap_search(base=\"%s\", filter=\"%s\")", base,
            filter);
    ...
    {
        log_log(LOG_ERR,
                "myldap_search(): too many searches registered with session "
                "(max %d)",
    ...
    while (1)
    {
        ...
        switch (rc)
        {
            case LDAP_RES_SEARCH_ENTRY:
                /* we have a normal search entry, update timestamp and return
                 * result */
                time(&(search->session->lastactivity));
            ...
            case LDAP_RES_SEARCH_RESULT:
                ...
                parserc =
                    ldap_parse_result(search->session->ld, search->msg, &rc,
                ...
                if (resultcontrols != NULL)
                {
                    ...
                    if (rc != LDAP_SUCCESS)
                    {
                        ...
                        /* clear error flag */
                        rc = LDAP_SUCCESS;
                        if (ldap_set_option(search->session->ld,
                                            LDAP_OPT_ERROR_NUMBER,
                                            &rc) != LDAP_SUCCESS)
                            log_log(LOG_WARNING,
                                    "failed to clear the error flag");
                    }
                ...
                if ((search->cookie == NULL) || (search->cookie->bv_len == 0))
                {
                    ...
                    log_log(LOG_DEBUG,
                            "ldap_result(): end of results (%d total)",
                            search->count);
            ...
            default:
                ...
                switch (rc)
                {
                    case -1:
                        ...
                        if (ldap_get_option(search->session->ld,
                                            LDAP_OPT_ERROR_NUMBER,
                                            &rc) != LDAP_SUCCESS)
                            rc = LDAP_UNAVAILABLE;
                        myldap_err(LOG_ERR, search->session->ld, rc,
                    ...
                    default:
                        ...
                        log_log(
                            LOG_WARNING,
                            "ldap_result() returned unexpected result type");
                ...
                if ((rc == LDAP_UNAVAILABLE) || (rc == LDAP_SERVER_DOWN) ||
                    ...
                    (rc == LDAP_PROTOCOL_ERROR) || (rc == LDAP_BUSY) ||
                    (rc == LDAP_UNWILLING_TO_PERFORM) || (rc == LDAP_TIMEOUT) ||
                    (rc == LDAP_CONNECT_ERROR) || (rc == LDAP_NOT_SUPPORTED))
                {
                    do_close(search->session);
...
const char* myldap_get_dn(MYLDAP_ENTRY* entry)
{
    ...
    if ((entry->dn == NULL) && (entry->search->valid))
    {
        ...
        if (entry->dn == NULL)
        {
            if (ldap_get_option(entry->search->session->ld,
                                LDAP_OPT_ERROR_NUMBER, &rc) != LDAP_SUCCESS)
                rc = LDAP_UNAVAILABLE;
            myldap_err(LOG_WARNING, entry->search->session->ld, rc,
                       "ldap_get_dn() returned NULL");
            /* close connection on connection problems */
            if ((rc == LDAP_UNAVAILABLE) || (rc == LDAP_SERVER_DOWN))
...
static char** myldap_get_ranged_values(MYLDAP_ENTRY* entry, const char* attr)
{
    ...
    {
        log_log(LOG_ERR,
                "myldap_get_ranged_values(): attbuf buffer too small (%lu "
                "required)",
    ...
    while (1)
    {
        ...
        if (set == NULL)
        {
            ...
            if (set == NULL)
            {
                ...
                log_log(LOG_CRIT, "myldap_get_ranged_values(): set_new() "
        ...
        {
            log_log(LOG_ERR,
                    "myldap_get_ranged_values(): attbuf buffer too small");
            break;
        }
        attrs[0] = attbuf;
        attrs[1] = NULL;
    ...
    if (values == NULL)
        log_log(
            LOG_CRIT,
            "myldap_get_ranged_values(): malloc() failed to allocate memory");
    return values;
}
...
const char** myldap_get_values(MYLDAP_ENTRY* entry, const char* attr)
{
    ...
    /* get from LDAP */
    values =
        ldap_get_values(entry->search->session->ld, entry->search->msg, attr);
    if (values == NULL)
    {
        ...
        {
            rc = LDAP_SUCCESS;
            if (ldap_set_option(entry->search->session->ld,
                                LDAP_OPT_ERROR_NUMBER, &rc) != LDAP_SUCCESS)
                log_log(LOG_WARNING, "failed to clear the error flag");
        }
        else if (rc == LDAP_SUCCESS)
        {
            ...
            log_log(LOG_ERR, "ldap_get_values() couldn't store results, "
...
static const char** bervalues_to_values(struct berval** bvalues)
{
    ...
    {
        log_log(LOG_CRIT,
                "bervalues_to_values(): malloc() failed to allocate memory");
        return NULL;
    }
    buf = (char*)values;
...
const char** myldap_get_values_len(MYLDAP_ENTRY* entry, const char* attr)
{
    ...
    {
        log_log(LOG_ERR,
                "myldap_get_values_len(): invalid result entry passed");
        errno = EINVAL;
        return NULL;
    }
    else if (attr == NULL)
    {
        log_log(LOG_ERR,
                "myldap_get_values_len(): invalid attribute name passed");
        errno = EINVAL;
        return NULL;
    }
    if (!entry->search->valid)
    ...
    if (bvalues == NULL)
    {
        ...
        {
            myldap_err(LOG_WARNING, entry->search->session->ld, rc,
                       "myldap_get_values_len() of attribute \"%s\" on entry "
                       "\"%s\" returned NULL",
                       attr, myldap_get_dn(entry));
            return NULL;
        }
...
static char** get_exploded_rdn(const char* dn)
{
    ...
    {
        log_log(LOG_WARNING, "ldap_explode_dn(%s) returned NULL: %s", dn,
                strerror(errno));
        return NULL;
    }
...
const char* myldap_get_rdn_value(MYLDAP_ENTRY* entry, const char* attr)
{
    ...
    {
        log_log(LOG_ERR,
                "myldap_get_rdn_value(): invalid attribute name passed");
        errno = EINVAL;
        return NULL;
    }
...
const char* myldap_cpy_rdn_value(const char* dn, const char* attr, char* buf,
                                 size_t buflen)
{
    char** exploded_rdn;
    const char* value;
    ...
    for (pass = 0; pass < 2; pass++)
    {
        ...
        for (d = deref; d != NULL; d = d->next)
                ...
                (strcasecmp(derefattr, d->derefAttr) == 0))
            {
                ...
                found = 0;
                /* go over deref'd attribute values to find the ones we're
                 * looking for */
        ...
        if (results == NULL)
        {
            ...
            if (buffer == NULL)
            {
                log_log(LOG_CRIT, "myldap_get_deref_values(): malloc() failed "
                                  "to allocate memory");
                return NULL;
            }
...
const char*** myldap_get_deref_values(MYLDAP_ENTRY UNUSED(*entry),
                                      const char UNUSED(*derefattr),
                                      const char UNUSED(*getattr))
{
    return NULL;
}
...
int myldap_set_debuglevel(int level)
{
    ...
    if (level > 1)
    {
        ...
        if (rc != LDAP_SUCCESS)
        {
            myldap_err(LOG_ERR, NULL, rc,
                       "ber_set_option(LBER_OPT_LOG_PRINT_FILE) failed");
            return rc;
        }
        ...
        if (level > 2)
        {
            ...
            {
                myldap_err(LOG_ERR, NULL, rc,
                           "ber_set_option(LBER_OPT_DEBUG_LEVEL) failed");
                return rc;
            }
        }
        ...
        {
            myldap_err(LOG_ERR, NULL, rc,
                       "ldap_set_option(LDAP_OPT_DEBUG_LEVEL) failed");
            return rc;
        }
...
int myldap_passwd(MYLDAP_SESSION* session, const char* userdn,
                  const char* oldpassword, const char* newpasswd)
{
    int rc;
    ...
    /* perform request */
    log_log(LOG_DEBUG,
            "myldap_passwd(): try ldap_passwd_s() without old password");
    rc = ldap_passwd_s(session->ld, &ber_userdn, NULL, &ber_newpassword,
                       &ber_retpassword, NULL, NULL);
    if (rc != LDAP_SUCCESS)
        myldap_err(LOG_ERR, session->ld, rc,
                   "ldap_passwd_s() without old password failed");
    ...
    if ((rc != LDAP_SUCCESS) && (oldpassword != NULL))
    {
        ...
        log_log(LOG_DEBUG,
                "myldap_passwd(): try ldap_passwd_s() with old password");
        ...
        if (rc != LDAP_SUCCESS)
            myldap_err(LOG_ERR, session->ld, rc,
                       "ldap_passwd_s() with old password failed");
        /* free returned data if needed */
        if (ber_retpassword.bv_val != NULL)
            ldap_memfree(ber_retpassword.bv_val);
    }
...
int myldap_error_message(MYLDAP_SESSION* session, int rc, char* buffer,
                         size_t buflen)
{
    char* msg_diag = NULL;
