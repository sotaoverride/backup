...
void init_pass(void)
{
    snmpd_register_config_handler("pass", pass_parse_config, pass_free_config,
                                  "miboid command");
}
    ...
    *ppass = calloc(1, sizeof(**ppass));
    ...
    if (numpassthrus > 1)
    {
        etmp = (struct extensible**)malloc(
            ((sizeof(struct extensible*)) * numpassthrus));
        if (etmp == NULL)
            return;
...
u_char* var_extensible_pass(struct variable* vp, oid* name, size_t* length,
                            ...
                            WriteMethod** write_method)
{
    ...
    for (i = 1; i <= numpassthrus; i++)
    {
        ...
        rtest = snmp_oidtree_compare(name, *length, passthru->miboid,
                                     passthru->miblen);
            ...
            size_t var_val_len, u_char* statP, oid* name, size_t name_len)
{
    ...
    {
        passthru = get_exten_instance(passthrus, i);
        rtest = snmp_oidtree_compare(name, name_len, passthru->miboid,
                                     passthru->miblen);
        if (rtest <= 0)
        {
            if (action != ACTION)
            ...
            DEBUGMSGTL(
                ("ucd-snmp/pass", "pass-running:  %s", passthru->command));
