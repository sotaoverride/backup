...
class shared_array
{
  ...
  public:
    ...
    shared_array(Y* p, D d) : px(p), pn(p, d)
    {
        boost::detail::sp_assert_convertible<Y[], T[]>();
    }
    ...
    shared_array(Y* p, D d, A a) : px(p), pn(p, d, a)
    {
        boost::detail::sp_assert_convertible<Y[], T[]>();
    }
    ...
    template <class Y>
    shared_array(shared_array<Y> const& r, element_type* p) BOOST_SP_NOEXCEPT
        : px(p),
          pn(r.pn)
    {
    }
    ...
    template <class Y>
    void reset(shared_array<Y> const& r, element_type* p) BOOST_SP_NOEXCEPT
    {
...
template <class T>
inline bool operator==(shared_array<T> const& a,
                       shared_array<T> const& b) BOOST_SP_NOEXCEPT
{
    return a.get() == b.get();
}
...
template <class T>
inline bool operator==(shared_array<T> const& p,
...
inline bool operator==(boost::detail::sp_nullptr_t,
                       shared_array<T> const& p) BOOST_SP_NOEXCEPT
{
    return p.get() == 0;
}
...
template <class T>
void swap(shared_array<T>& a, shared_array<T>& b) BOOST_SP_NOEXCEPT
{
    a.swap(b);
}

template <class D, class T>
D* get_deleter(shared_array<T> const& p) BOOST_SP_NOEXCEPT
{
