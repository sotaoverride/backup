                                          ...
                                          uint8_t events)
{
    ...
    rc =
        sd_bus_emit_properties_changed_strv(context->bus, MBOX_DBUS_OBJECT,
                                            /* FIXME: Hard-coding v2 */
                                            MBOX_DBUS_PROTOCOL_IFACE_V2, props);
                                   ...
                                   sd_bus_error* ret_error)
{
    ...
    rc = sd_bus_message_append(n, "yyq", io.resp.api_version,
                               io.resp.v2.block_size_shift, io.resp.v2.timeout);
    if (rc < 0)
    {
...
static int transport_dbus_get_property(sd_bus* bus, const char* path,
...
static const sd_bus_vtable protocol_v2_vtable[] = {
    ...
    SD_BUS_METHOD("GetInfo", "y", "yyq", &transport_dbus_get_info,
                  SD_BUS_VTABLE_UNPRIVILEGED),
    SD_BUS_METHOD("GetFlashInfo", NULL, "qq", &transport_dbus_get_flash_info,
                  SD_BUS_VTABLE_UNPRIVILEGED),
