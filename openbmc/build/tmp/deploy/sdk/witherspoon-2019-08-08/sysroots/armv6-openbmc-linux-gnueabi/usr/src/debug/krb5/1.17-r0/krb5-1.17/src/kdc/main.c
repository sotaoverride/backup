 ...
 */
kdc_realm_t* find_realm_data(struct server_handle* handle, char* rname,
                             krb5_ui_4 rsize)
{
    int i;
...
kdc_realm_t* setup_server_realm(struct server_handle* handle,
                                krb5_principal sprinc)
{
    ...
    {
        newrealm =
            find_realm_data(handle, sprinc->realm.data, sprinc->realm.length);
    }
    else
    {
...
static void finish_realm(kdc_realm_t* rdp)
{
    if (rdp->realm_name)
        free(rdp->realm_name);
    if (rdp->realm_mpname)
...
static krb5_error_code combine(const char* val1, const char* val2,
                               char** val_out)
{
...
static krb5_error_code
    init_realm(kdc_realm_t* rdp, krb5_pointer aprof, char* realm,
               ...
               krb5_boolean def_restrict_anon, char** db_args,
               char* no_referral, char* hostbased)
{
    ...
    {
        rdp->realm_mpname =
            (def_mpname) ? strdup(def_mpname) : strdup(KRB5_KDB_M_NAME);
    }
    if (!rdp->realm_mpname)
    {
        kret = ENOMEM;
    ...
    if (krb5_aprof_get_string(aprof, hierarchy, TRUE, &rdp->realm_tcp_listen))
    {
     ...
     */
    if ((kret = krb5_db_fetch_mkey(
             rdp->realm_context, rdp->realm_mprinc, rdp->realm_mkey.enctype,
             manual, FALSE, rdp->realm_stash, &mkvno, NULL, &rdp->realm_mkey)))
    {
        kdc_err(rdp->realm_context, kret,
    ...
    /* Set up the keytab */
    if ((kret =
             krb5_ktkdb_resolve(rdp->realm_context, NULL, &rdp->realm_keytab)))
    {
        kdc_err(rdp->realm_context, kret,
    ...
    if ((kret = krb5_build_principal(rdp->realm_context, &rdp->realm_tgsprinc,
                                     strlen(realm), realm, KRB5_TGS_NAME, realm,
                                     (char*)NULL)))
    {
        kdc_err(rdp->realm_context, kret,
    ...
    if (!rkey_init_done)
    {
        ...
        if ((kret = krb5_c_random_add_entropy(
                 rdp->realm_context, KRB5_C_RANDSOURCE_TRUSTEDPARTY, &seed)))
            goto whoops;
...
static krb5_sigtype on_monitor_signal(int signo)
{
...
static krb5_sigtype on_monitor_sighup(int signo)
{
    sighup_received = 1;
 ...
 * longer in use).
 */
static void terminate_workers(pid_t* pids, int bound)
{
 ...
 */
static krb5_error_code create_workers(verto_ctx* ctx, int num)
{
                              ...
                              int* tcp_listen_backlog_out)
{
        ...
        switch (c)
        {
            case 'x':
                db_args_size++;
                {
                    ...
                    {
                        fprintf(stderr,
                                _("%s: KDC cannot initialize. Not enough "
                                  "memory\n"),
                                argv[0]);
                        exit(1);
                    }
            ...
            case 'r': /* realm name for db */
                if (!find_realm_data(&shandle, optarg,
                                     (krb5_ui_4)strlen(optarg)))
                {
                    if ((rdatap = (kdc_realm_t*)malloc(sizeof(kdc_realm_t))))
                    {
                        retval = init_realm(
                            rdatap, aprof, optarg, mkey_name, menctype,
                            def_udp_listen, def_tcp_listen, manual,
                            def_restrict_anon, db_args, no_referral, hostbased);
                        if (retval)
                        {
                            fprintf(stderr,
                                    _("%s: cannot initialize realm %s - "
                    ...
                    {
                        fprintf(stderr,
                                _("%s: cannot initialize realm %s. Not "
                                  "enough memory\n"),
                                argv[0], optarg);
                        exit(1);
                    }
    ...
    if (shandle.kdc_numrealms == 0)
    {
        ...
        if ((rdatap = (kdc_realm_t*)malloc(sizeof(kdc_realm_t))))
        {
            ...
            {
                fprintf(stderr,
                        _("%s: cannot initialize realm %s - see log "
                          "file for details\n"),
                        argv[0], lrealm);
                exit(1);
            }
...
int main(int argc, char** argv)
{
    ...
    shandle.kdc_realmlist = malloc(sizeof(kdc_realm_t*) * KRB5_KDC_MAX_REALMS);
    ...
    for (i = 0; i < shandle.kdc_numrealms; i++)
    {
        ...
        {
            retval =
                loop_add_udp_address(KRB5_DEFAULT_PORT, realm->realm_listen);
            if (retval)
                goto net_init_error;
        }
