...
{
template <class T>
inline BOOST_DEDUCED_TYPENAME boost::make_unsigned<T>::type
    lcast_to_unsigned(const T value) BOOST_NOEXCEPT
{
...
class lcast_put_unsigned : boost::noncopyable
{
    ...
    BOOST_DEDUCED_TYPENAME boost::conditional<(sizeof(unsigned) > sizeof(T)),
                                              unsigned, T>::type m_value;
    CharT* m_finish;
    CharT const m_czero;
        ...
        do
        {
            if (left == 0)
            {
                ...
                if (group < grouping_size)
                {
                    ...
                    last_grp_size = (grp_size <= 0 ? static_cast<char>(CHAR_MAX)
                                                   : grp_size);
                }
...
class lcast_ret_unsigned : boost::noncopyable
{
  ...
  public:
    lcast_ret_unsigned(T& value, const CharT* const begin,
                       const CharT* end) BOOST_NOEXCEPT
          ...
          m_end(end)
    {
        ...
        BOOST_STATIC_ASSERT_MSG(std::numeric_limits<T>::is_specialized,
                                "std::numeric_limits are not specialized for "
                                "integral type passed to boost::lexical_cast");
#endif
    }
    ...
    inline bool convert()
    {
        ...
        for (; m_end >= m_begin; --m_end)
        {
            ...
            else
            {
                if (!Traits::eq(
                        *m_end,
                        thousands_sep)) //|| begin == end ) return false;
                {
                    /*
                     * According to Programming languages - C++
                     * Digit grouping is checked. That is, the positions of
                     * discarded separators is examined for consistency with
  ...
  private:
    // Iteration that does not care about grouping/separators and assumes that
    // all input characters are digits
    inline bool main_convert_iteration() BOOST_NOEXCEPT
    {
        CharT const czero = lcast_char_constants<CharT>::zero;
        ...
        // We must correctly handle situations like
        // `000000000000000000000000000001`. So we take care of overflow only if
        // `dig_value` is not '0'.
