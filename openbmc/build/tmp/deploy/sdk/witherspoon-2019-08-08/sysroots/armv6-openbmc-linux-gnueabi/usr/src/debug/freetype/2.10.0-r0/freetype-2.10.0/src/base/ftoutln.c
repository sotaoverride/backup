...
{
#undef SCALED
#define SCALED(x) (((x) < 0 ? -(-(x) << shift) : ((x) << shift)) - delta)
               ...
               FT_Outline* anoutline)
{
    ...
    *anoutline = null_outline;
    ...
    if (numContours < 0 || (FT_UInt)numContours > numPoints)
        return FT_THROW(Invalid_Argument);
...
#define FT_OUTLINE_GET_CONTOUR(outline, c, first, last)                        \
    do                                                                         \
    {                                                                          \
        (first) = (c > 0) ? (outline)->points + (outline)->contours[c - 1] + 1 \
                          : (outline)->points;                                 \
...
FT_Outline_EmboldenXY(FT_Outline* outline, FT_Pos xstrength, FT_Pos ystrength)
{
    ...
    for (c = 0; c < outline->n_contours; c++)
    {
        ...
        for (i = last, j = first, k = -1; j != i && i != k;
             j = j < last ? j + 1 : first)
        {
            if (j != k)
            {
            ...
            if (l_in != 0)
            {
                ...
                if (d > -0xF000L)
                {
                    ...
                    /* shift components along lateral bisector in proper
                     * orientation */
                    shift.x = in.y + out.y;
                    shift.y = in.x + out.x;
...
FT_Outline_Get_Orientation(FT_Outline* outline)
{
    ...
    xshift = FT_MSB((FT_UInt32)(FT_ABS(cbox.xMax) | FT_ABS(cbox.xMin))) - 14;
    ...
    for (c = 0; c < outline->n_contours; c++)
    {
        ...
        for (n = first; n <= last; n++)
        {
            ...
            area = ADD_LONG(area,
                            MUL_LONG(v_cur.y - v_prev.y, v_cur.x + v_prev.x));
