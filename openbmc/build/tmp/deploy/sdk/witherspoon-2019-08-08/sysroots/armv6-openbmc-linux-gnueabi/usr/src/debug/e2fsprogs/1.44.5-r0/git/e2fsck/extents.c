...
errcode_t e2fsck_rebuild_extents_later(e2fsck_t ctx, ext2_ino_t ino)
{
    ...
    if (!ctx->inodes_to_rebuild)
        retval = e2fsck_allocate_inode_bitmap(
            ctx->fs, _("extent rebuild inode map"), EXT2FS_BMAP64_RBTREE,
            "inodes_to_rebuild", &ctx->inodes_to_rebuild);
    if (retval)
        return retval;
...
static errcode_t load_extents(e2fsck_t ctx, struct extent_list* list)
{
    ...
    do
    {
        ...
        if (list->count)
        {
            struct ext2fs_extent* last = list->extents + list->count - 1;
        ...
        if (list->count == list->size)
        {
            unsigned int new_size =
                (list->size + NUM_EXTENTS) * sizeof(struct ext2fs_extent);
                       ...
                       int ref_offset EXT2FS_ATTR((unused)), void* priv_data)
{
    ...
    if (list->count)
    {
        ...
        if (last->e_lblk + last->e_len == (__u64)blockcnt &&
            last->e_pblk + last->e_len == *blocknr && end < (1ULL << 32))
        {
            last->e_len++;
#ifdef DEBUG
            printf("R: ino=%d len=%u\n", list->ino, last->e_len);
...
extents_loaded:
    ...
    retval =
        ext2fs_iblk_sub_blocks(ctx->fs, EXT2_INODE(&inode), list->blocks_freed);
    if (retval)
        goto err;
                                       ...
                                       struct problem_context* pctx)
{
    ...
    do
    {
        ...
        if (extent.e_flags & EXT2_EXTENT_FLAGS_LEAF)
        {
            retval = ext2fs_extent_get(ehandle, EXT2_EXTENT_LAST_SIB, &extent);
            if (retval)
                break;
        }
                                        ...
                                        struct ext2_extent_info* info)
{
    ...
    extents_per_block =
        (ctx->fs->blocksize - sizeof(struct ext3_extent_header)) /
