...
static void free_enum_list(struct snmp_enum_list* list);
...
int init_snmp_enum(const char* type)
{
    ...
    snmp_enum_lists = (struct snmp_enum_list***)calloc(
        1, sizeof(struct snmp_enum_list**) * SE_MAX_IDS);
    if (!snmp_enum_lists)
        return SE_NOMEM;
    ...
    for (i = 0; i < SE_MAX_IDS; i++)
    {
        if (!snmp_enum_lists[i])
            snmp_enum_lists[i] = (struct snmp_enum_list**)calloc(
                1, sizeof(struct snmp_enum_list*) * SE_MAX_SUBIDS);
        if (!snmp_enum_lists[i])
            return SE_NOMEM;
    }
...
int se_store_in_list(struct snmp_enum_list* new_list, unsigned int major,
                     unsigned int minor)
{
    int ret = SE_OK;
...
void se_store_enum_list(struct snmp_enum_list* new_list, const char* token,
                        const char* type)
{
...
void se_store_list(unsigned int major, unsigned int minor, const char* type)
{
    char token[32];
...
int se_find_value_in_list(struct snmp_enum_list* list, const char* label)
{
    if (!list)
...
int se_find_casevalue_in_list(struct snmp_enum_list* list, const char* label)
{
    if (!list)
        return SE_DNE; /* XXX: um, no good solution here */
    while (list)
...
int se_find_value(unsigned int major, unsigned int minor, const char* label)
{
...
int se_add_pair_to_list(struct snmp_enum_list** list, char* label, int value)
{
 ...
 */
static struct snmp_enum_list** se_find_slist_ptr(const char* listname)
{
...
int se_find_value_in_slist(const char* listname, const char* label)
{
    return (se_find_value_in_list(se_find_slist(listname), label));
}

int se_find_casevalue_in_slist(const char* listname, const char* label)
{
...
void se_clear_list(struct snmp_enum_list** list)
{
...
int se_store_slist_callback(int majorID, int minorID, void* serverargs,
                            void* clientargs)
{
    char* appname =
