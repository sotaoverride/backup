...
#define C_RBNODE_INIT(_var)                                                    \
    {                                                                          \
        .__parent_and_flags = (unsigned long)&(_var)                           \
    }
...
#define c_rbtree_for_each_entry(_iter, _tree, _m)                              \
    for (_iter =                                                               \
             c_rbnode_entry(c_rbtree_first(_tree), __typeof__(*_iter), _m);    \
         _iter; _iter = c_rbnode_entry(c_rbnode_next(&_iter->_m),              \
                                       __typeof__(*_iter), _m))

#define c_rbtree_for_each_safe(_iter, _safe, _tree)                            \
    for (_iter = c_rbtree_first(_tree), _safe = c_rbnode_next(_iter); _iter;   \
...
#define c_rbtree_for_each_entry_postorder(_iter, _tree, _m)                    \
    for (_iter = c_rbnode_entry(c_rbtree_first_postorder(_tree),               \
                                __typeof__(*_iter), _m);                       \
         _iter; _iter = c_rbnode_entry(c_rbnode_next_postorder(&_iter->_m),    \
                                       __typeof__(*_iter), _m))
