...
static FT_Error T42_Open_Face(T42_Face face)
{
    ...
    error = t42_parser_init(parser, face->root.stream, memory, psaux);
    if (error)
        goto Exit;
...
T42_Face_Init(FT_Stream stream, FT_Face t42face, /* T42_Face */
              FT_Int face_index, FT_Int num_params, FT_Parameter* params)
{
    ...
    root->face_flags |= FT_FACE_FLAG_SCALABLE | FT_FACE_FLAG_HORIZONTAL |
    ...
    /* Load the TTF font embedded in the T42 font */
    {
        ...
        args.driver = FT_Get_Module(FT_FACE_LIBRARY(face), "truetype");
                   ...
                   FT_Int32 load_flags)
{
    ...
    glyph_index = (FT_UInt)ft_strtol(
        (const char*)t42face->type1.charstrings[glyph_index], NULL, 10);

    t42_glyphslot_clear(t42slot->ttslot);
    error = ttclazz->load_glyph(t42slot->ttslot, t42size->ttsize, glyph_index,
