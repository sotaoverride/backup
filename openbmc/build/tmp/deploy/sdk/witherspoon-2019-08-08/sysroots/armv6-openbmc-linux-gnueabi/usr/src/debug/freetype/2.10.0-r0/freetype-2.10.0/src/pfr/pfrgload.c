...
static FT_Error pfr_glyph_curve_to(PFR_Glyph glyph, FT_Vector* control1,
...
/* load a simple glyph */
static FT_Error pfr_glyph_load_simple(PFR_Glyph glyph, FT_Byte* p,
                                      FT_Byte* limit)
{
...
static FT_Error pfr_glyph_load_compound(PFR_Glyph glyph, FT_Byte* p,
                                        FT_Byte* limit)
{
    FT_Error error = FT_Err_Ok;
    FT_GlyphLoader loader = glyph->loader;
...
static FT_Error pfr_glyph_load_rec(PFR_Glyph glyph, FT_Stream stream,
                                   FT_ULong gps_offset, FT_ULong offset,
                                   FT_ULong size)
{
    FT_Error error;
    FT_Byte* p;
    ...
    if (FT_STREAM_SEEK(gps_offset + offset) || FT_FRAME_ENTER(size))
        goto Exit;
    ...
    if (size > 0 && *p & PFR_GLYPH_IS_COMPOUND)
    {
        ...
        for (n = 0; n < count; n++)
        {
            ...
            error =
                pfr_glyph_load_rec(glyph, stream, gps_offset,
...
pfr_glyph_load(PFR_Glyph glyph, FT_Stream stream, FT_ULong gps_offset,
               FT_ULong offset, FT_ULong size)
{
