...
kadm5_ret_t schpw_util_wrapper(void* server_handle, krb5_principal client,
                               krb5_principal target, krb5_boolean initial_flag,
                               char* new_pw, char** ret_pw, char* msg_ret,
                               unsigned int msg_len)
{
    ...
    if (krb5_principal_compare(handle->context, client, target))
    {
        if (!initial_flag)
        {
            strlcpy(msg_ret, "Ticket must be derived from a password", msg_len);
    ...
    if (auth(handle->context, OP_CPW, client, target, NULL, NULL, NULL, NULL,
             0))
    {
        ret = kadm5_chpass_principal_util(server_handle, target, new_pw, ret_pw,
                                          msg_ret, msg_len);
    }
    else
    {
                           ...
                           char* msg_ret, unsigned int msg_len)
{
    ...
    ret = kadm5_get_principal(handle->lhandle, principal, &princ,
    ...
    if (princ.aux_attributes & KADM5_POLICY)
    {
         ...
         * it had no policy. */
        if ((ret = kadm5_get_policy(handle->lhandle, princ.policy, &pol)) !=
            KADM5_OK)
        {
...
krb5_error_code make_toolong_error(void* handle, krb5_data** out)
{
    ...
    retval =
        krb5_us_timeofday(server_handle->context, &errpkt.stime, &errpkt.susec);
    if (retval)
        return retval;
    ...
    retval = krb5_build_principal(server_handle->context, &errpkt.server,
                                  ...
                                  server_handle->params.realm, "kadmin",
                                  "changepw", NULL);
    if (retval)
        return retval;
    errpkt.client = NULL;
