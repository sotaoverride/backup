...
#define svg_bar(class, x1, x2, y)                                              \
    svg("  <rect class=\"%s\" x=\"%.03f\" y=\"%.03f\" width=\"%.03f\" "        \
        "height=\"%.03f\" />\n",                                               \
        (class), SCALE_X*(x1), SCALE_Y*(y), SCALE_X*((x2) - (x1)),             \
        SCALE_Y - 1.0)

#define svg_text(b, x, y, format, ...)                                         \
    do                                                                         \
    {                                                                          \
        svg("  <text class=\"%s\" x=\"%.03f\" y=\"%.03f\">",                   \
            (b) ? "left" : "right", SCALE_X*(x) + (b ? 5.0 : -5.0),            \
            SCALE_Y * (y) + 14.0);                                             \
        svg(format, ##__VA_ARGS__);                                            \
        svg("</text>\n");                                                      \
    } while (false)

static enum dot { DEP_ALL, DEP_ORDER, DEP_REQUIRE } arg_dot = DEP_ALL;
...
static int bus_get_uint64_property(sd_bus* bus, const char* path,
                                   ...
                                   uint64_t* val)
{
    ...
    if (r < 0)
        return log_error_errno(r, "Failed to parse reply: %s",
                               bus_error_message(&error, r));

    return 0;
}

static int bus_get_unit_property_strv(sd_bus* bus, const char* path,
                                      const char* property, char*** strv)
{
    _cleanup_(sd_bus_error_free) sd_bus_error error = SD_BUS_ERROR_NULL;
    int r;
    ...
    r = sd_bus_get_property_strv(bus, "org.freedesktop.systemd1", path,
                                 ...
                                 &error, strv);
    if (r < 0)
        return log_error_errno(r, "Failed to get unit property %s: %s",
                               property, bus_error_message(&error, r));

    return 0;
}

static int compare_unit_time(const struct unit_times* a,
                             const struct unit_times* b)
{
    return CMP(b->time, a->time);
}

static int compare_unit_start(const struct unit_times* a,
                              const struct unit_times* b)
{
...
static int acquire_boot_times(sd_bus* bus, struct boot_times** bt)
{
    static const struct bus_properties_map property_map[] = {
        {"FirmwareTimestampMonotonic", "t", NULL,
         offsetof(struct boot_times, firmware_time)},
        {"LoaderTimestampMonotonic", "t", NULL,
         offsetof(struct boot_times, loader_time)},
        {"KernelTimestamp", "t", NULL,
         offsetof(struct boot_times, kernel_time)},
        {"InitRDTimestampMonotonic", "t", NULL,
         offsetof(struct boot_times, initrd_time)},
        {"UserspaceTimestampMonotonic", "t", NULL,
         offsetof(struct boot_times, userspace_time)},
        {"FinishTimestampMonotonic", "t", NULL,
         offsetof(struct boot_times, finish_time)},
        {"SecurityStartTimestampMonotonic", "t", NULL,
         offsetof(struct boot_times, security_start_time)},
        {"SecurityFinishTimestampMonotonic", "t", NULL,
         offsetof(struct boot_times, security_finish_time)},
        {"GeneratorsStartTimestampMonotonic", "t", NULL,
         offsetof(struct boot_times, generators_start_time)},
        {"GeneratorsFinishTimestampMonotonic", "t", NULL,
         offsetof(struct boot_times, generators_finish_time)},
        {"UnitsLoadStartTimestampMonotonic", "t", NULL,
         offsetof(struct boot_times, unitsload_start_time)},
        {"UnitsLoadFinishTimestampMonotonic", "t", NULL,
         offsetof(struct boot_times, unitsload_finish_time)},
        {"InitRDSecurityStartTimestampMonotonic", "t", NULL,
         offsetof(struct boot_times, initrd_security_start_time)},
        {"InitRDSecurityFinishTimestampMonotonic", "t", NULL,
         offsetof(struct boot_times, initrd_security_finish_time)},
        {"InitRDGeneratorsStartTimestampMonotonic", "t", NULL,
         offsetof(struct boot_times, initrd_generators_start_time)},
        {"InitRDGeneratorsFinishTimestampMonotonic", "t", NULL,
         offsetof(struct boot_times, initrd_generators_finish_time)},
        {"InitRDUnitsLoadStartTimestampMonotonic", "t", NULL,
         offsetof(struct boot_times, initrd_unitsload_start_time)},
        {"InitRDUnitsLoadFinishTimestampMonotonic", "t", NULL,
         offsetof(struct boot_times, initrd_unitsload_finish_time)},
        {},
    };
    _cleanup_(sd_bus_error_free) sd_bus_error error = SD_BUS_ERROR_NULL;
    ...
    r = bus_map_all_properties(bus, "org.freedesktop.systemd1",
                               "/org/freedesktop/systemd1", property_map,
                               BUS_MAP_STRDUP, &error, NULL, &times);
    if (r < 0)
        return log_error_errno(r, "Failed to get timestamp properties: %s",
                               bus_error_message(&error, r));

    if (times.finish_time <= 0)
        return log_error_errno(
            SYNTHETIC_ERRNO(EINPROGRESS),
            "Bootup is not yet finished "
            "(org.freedesktop.systemd1.Manager.FinishTimestampMonotonic="
            "%" PRIu64 ").\n"
            "Please try again later.\n"
            "Hint: Use 'systemctl%s list-jobs' to see active jobs",
            times.finish_time, arg_scope == UNIT_FILE_SYSTEM ? "" : " --user");

    if (arg_scope == UNIT_FILE_SYSTEM && times.security_start_time > 0)
    {
        /* security_start_time is set when systemd is not running under
    ...
    {
        /*
         * User-instance-specific or container-system-specific timestamps
         * processing (see comment to reverse_offset in struct boot_times).
         */
        times.reverse_offset = times.userspace_time;

        times.firmware_time = times.loader_time = times.kernel_time =
            times.initrd_time = times.userspace_time =
...
static int acquire_time_data(sd_bus* bus, struct unit_times** out)
{
    static const struct bus_properties_map property_map[] = {
        {"InactiveExitTimestampMonotonic", "t", NULL,
         offsetof(struct unit_times, activating)},
        {"ActiveEnterTimestampMonotonic", "t", NULL,
         offsetof(struct unit_times, activated)},
        {"ActiveExitTimestampMonotonic", "t", NULL,
         offsetof(struct unit_times, deactivating)},
        {"InactiveEnterTimestampMonotonic", "t", NULL,
         offsetof(struct unit_times, deactivated)},
        {},
    };
    ...
    r = sd_bus_call_method(
        ...
        "org.freedesktop.systemd1.Manager", "ListUnits", &error, &reply, NULL);
    if (r < 0)
    ...
    while ((r = bus_parse_unit_info(reply, &u)) > 0)
    {
        ...
        r = bus_map_all_properties(bus, "org.freedesktop.systemd1", u.unit_path,
                                   property_map, BUS_MAP_STRDUP, &error, NULL,
                                   t);
        if (r < 0)
            return log_error_errno(
                r, "Failed to get timestamp properties of unit %s: %s", u.id,
                bus_error_message(&error, r));
...
static int acquire_host_info(sd_bus* bus, struct host_info** hi)
{
    static const struct bus_properties_map hostname_map[] = {
        ...
        {"KernelRelease", "s", NULL,
         offsetof(struct host_info, kernel_release)},
        {"KernelVersion", "s", NULL,
         offsetof(struct host_info, kernel_version)},
        {"OperatingSystemPrettyName", "s", NULL,
         offsetof(struct host_info, os_pretty_name)},
        {}};

    static const struct bus_properties_map manager_map[] = {
        {"Virtualization", "s", NULL,
         offsetof(struct host_info, virtualization)},
    ...
    r = bus_map_all_properties(system_bus ?: bus, "org.freedesktop.hostname1",
                               "/org/freedesktop/hostname1", hostname_map,
                               BUS_MAP_STRDUP, &error, NULL, host);
    if (r < 0)
    {
        log_debug_errno(r,
                        "Failed to get host information from "
                        "systemd-hostnamed, ignoring: %s",
                        bus_error_message(&error, r));
...
manager:
    r = bus_map_all_properties(bus, "org.freedesktop.systemd1",
                               "/org/freedesktop/systemd1", manager_map,
                               BUS_MAP_STRDUP, &error, NULL, host);
    if (r < 0)
        return log_error_errno(
            r, "Failed to get host information from systemd: %s",
            bus_error_message(&error, r));
...
static int pretty_boot_time(sd_bus* bus, char** _buf)
{
    ...
    if (r < 0)
    {
        log_error_errno(r, "default.target doesn't seem to exist: %s",
                        bus_error_message(&error, r));
        unit_id = NULL;
    }

    r = bus_get_uint64_property(bus, path, "org.freedesktop.systemd1.Unit",
                                "ActiveEnterTimestampMonotonic",
                                &activated_time);
    if (r < 0)
    {
        log_info_errno(
            r, "Could not get time to reach default.target, ignoring: %m");
    ...
    if (t->firmware_time > 0)
        size = strpcpyf(&ptr, size, "%s (firmware) + ",
                        format_timespan(ts, sizeof(ts),
                                        t->firmware_time - t->loader_time,
                                        USEC_PER_MSEC));
    if (t->loader_time > 0)
        size = strpcpyf(
            &ptr, size, "%s (loader) + ",
            format_timespan(ts, sizeof(ts), t->loader_time, USEC_PER_MSEC));
    if (t->kernel_done_time > 0)
        size = strpcpyf(&ptr, size, "%s (kernel) + ",
                        format_timespan(ts, sizeof(ts), t->kernel_done_time,
                                        USEC_PER_MSEC));
    if (t->initrd_time > 0)
        size = strpcpyf(&ptr, size, "%s (initrd) + ",
                        format_timespan(ts, sizeof(ts),
                                        t->userspace_time - t->initrd_time,
                                        USEC_PER_MSEC));

    size = strpcpyf(&ptr, size, "%s (userspace) ",
                    format_timespan(ts, sizeof(ts),
                                    t->finish_time - t->userspace_time,
                                    USEC_PER_MSEC));
    if (t->kernel_done_time > 0)
        strpcpyf(&ptr, size, "= %s ",
                 format_timespan(ts, sizeof(ts),
                                 t->firmware_time + t->finish_time,
                                 USEC_PER_MSEC));

    if (unit_id && activated_time > 0 && activated_time != USEC_INFINITY)
    {
        usec_t base =
            t->userspace_time > 0 ? t->userspace_time : t->reverse_offset;

        size =
            strpcpyf(&ptr, size, "\n%s reached after %s in userspace", unit_id,
                     format_timespan(ts, sizeof(ts), activated_time - base,
                                     USEC_PER_MSEC));
    }
...
static void svg_graph_box(double height, double begin, double end)
{
    ...
    for (i = ((long long)(begin / 100000)) * 100000; i <= end; i += 100000)
    {
        ...
        if (i % 5000000 == 0)
            svg("  <line class=\"sec5\" x1=\"%.03f\" y1=\"0\" x2=\"%.03f\" "
                "y2=\"%.03f\" />\n"
                "  <text class=\"sec\" x=\"%.03f\" y=\"%.03f\" "
                ">%.01fs</text>\n",
                SCALE_X * i, SCALE_X * i, SCALE_Y * height, SCALE_X * i, -5.0,
                0.000001 * i);
        else if (i % 1000000 == 0)
            svg("  <line class=\"sec1\" x1=\"%.03f\" y1=\"0\" x2=\"%.03f\" "
                "y2=\"%.03f\" />\n"
                "  <text class=\"sec\" x=\"%.03f\" y=\"%.03f\" "
                ">%.01fs</text>\n",
                SCALE_X * i, SCALE_X * i, SCALE_Y * height, SCALE_X * i, -5.0,
            ...
            svg("  <line class=\"sec01\" x1=\"%.03f\" y1=\"0\" x2=\"%.03f\" "
                "y2=\"%.03f\" />\n",
...
static int plot_unit_times(struct unit_times* u, double width, int y)
{
    ...
    if (u->time)
        svg_text(b, u->activating, y, "%s (%s)", u->name,
                 format_timespan(ts, sizeof(ts), u->time, USEC_PER_MSEC));
    else
...
static int analyze_plot(int argc, char* argv[], void* userdata)
{
    ...
    for (u = times; u->has_data; u++)
    {
        ...
        if (u->deactivated > u->activating &&
            u->deactivated <= boot->finish_time && u->activated == 0 &&
            u->deactivating == 0)
            u->activated = u->deactivating = u->deactivated;
        if (u->activated < u->activating || u->activated > boot->finish_time)
            u->activated = boot->finish_time;
        if (u->deactivating < u->activated ||
            u->deactivating > boot->finish_time)
            u->deactivating = boot->finish_time;
        if (u->deactivated < u->deactivating ||
    ...
    svg("<svg width=\"%.0fpx\" height=\"%.0fpx\" version=\"1.1\" "
        ...
        80.0 + width, 150.0 + (m * SCALE_Y) + 5 * SCALE_Y /* legend */);

    /* write some basic info as a comment, including some help */
    svg("<!-- This file is a systemd-analyze SVG file. It is best rendered in "
        "a   -->\n"
        "<!-- browser such as Chrome, Chromium or Firefox. Other applications  "
        "   -->\n"
        "<!-- that render these files properly but much slower are "
        "ImageMagick,   -->\n"
    ...
    svg("<defs>\n  <style type=\"text/css\">\n    <![CDATA[\n"
        ...
        "}\n"
        "      text.left  { font-family: Verdana, Helvetica; font-size: 14px; "
        "text-anchor: start; }\n"
        "      text.right { font-family: Verdana, Helvetica; font-size: 14px; "
        "text-anchor: end; }\n"
    ...
    if (host)
        svg("<text x=\"20\" y=\"30\">%s %s (%s %s %s) %s %s</text>",
            ...
            strempty(host->hostname), strempty(host->kernel_name),
            strempty(host->kernel_release), strempty(host->kernel_version),
            strempty(host->architecture), strempty(host->virtualization));
    ...
    {
        svg_bar("firmware", -(double)boot->firmware_time,
                -(double)boot->loader_time, y);
    ...
    if (boot->initrd_time > 0)
    {
        ...
        if (boot->initrd_security_start_time <
            boot->initrd_security_finish_time)
            svg_bar("security", boot->initrd_security_start_time,
                    boot->initrd_security_finish_time, y);
        if (boot->initrd_generators_start_time <
            boot->initrd_generators_finish_time)
            svg_bar("generators", boot->initrd_generators_start_time,
                    boot->initrd_generators_finish_time, y);
        if (boot->initrd_unitsload_start_time <
            boot->initrd_unitsload_finish_time)
            svg_bar("unitsload", boot->initrd_unitsload_start_time,
                    boot->initrd_unitsload_finish_time, y);
    ...
    if (boot->security_start_time > 0)
        svg_bar("security", boot->security_start_time,
                boot->security_finish_time, y);
...
static int list_dependencies_print(const char* name, unsigned level,
                                   ...
                                   struct boot_times* boot)
{
    ...
    for (i = level; i != 0; i--)
        printf("%s", special_glyph(branches & (1 << (i - 1))
    ...
    if (times)
    {
        if (times->time > 0)
            printf(
                "%s%s @%s +%s%s", ansi_highlight_red(), name,
                format_timespan(ts, sizeof(ts),
                                times->activating - boot->userspace_time,
                                USEC_PER_MSEC),
                format_timespan(ts2, sizeof(ts2), times->time, USEC_PER_MSEC),
                ansi_normal());
        else if (times->activated > boot->userspace_time)
            printf("%s @%s", name,
                   format_timespan(ts, sizeof(ts),
                                   times->activated - boot->userspace_time,
                                   USEC_PER_MSEC));
        else
            printf("%s", name);
    }
...
static bool times_in_range(const struct unit_times* times,
                           const struct boot_times* boot)
{
    return times && times->activated > 0 &&
           times->activated <= boot->finish_time;
}

static int list_dependencies_one(sd_bus* bus, const char* name, unsigned level,
                                 char*** units, unsigned branches)
{
    ...
    STRV_FOREACH(c, deps)
    {
        ...
        r = list_dependencies_print(*c, level, branches, to_print == 0, times,
                                    boot);
        if (r < 0)
            return r;
...
static int list_dependencies(sd_bus* bus, const char* name)
{
    ...
    r = sd_bus_get_property(bus, "org.freedesktop.systemd1", path,
                            "org.freedesktop.systemd1.Unit", "Id", &error,
                            &reply, "s");
    if (r < 0)
        return log_error_errno(r, "Failed to get ID: %s",
                               bus_error_message(&error, r));
    ...
    if (times)
    {
        if (times->time)
            printf("%s%s +%s%s\n", ansi_highlight_red(), id,
                   format_timespan(ts, sizeof(ts), times->time, USEC_PER_MSEC),
                   ansi_normal());
        else if (times->activated > boot->userspace_time)
            printf("%s @%s\n", id,
                   format_timespan(ts, sizeof(ts),
...
static int analyze_critical_chain(int argc, char* argv[], void* userdata)
{
    ...
    puts("The time when unit became active or started is printed after the "
         "\"@\" character.\n"
...
static int graph_one_property(sd_bus* bus, const UnitInfo* u, const char* prop,
                              const char* color, char* patterns[],
                              char* from_patterns[], char* to_patterns[])
{
    _cleanup_strv_free_ char** units = NULL;
    char** unit;
    int r;
    ...
    if (!strv_isempty(from_patterns) && !match_patterns &&
    ...
    STRV_FOREACH(unit, units)
    {
        ...
        if (!strv_isempty(to_patterns) && !match_patterns2 &&
                     ...
                     char* from_patterns[], char* to_patterns[])
{
    ...
    if (IN_SET(arg_dot, DEP_REQUIRE, DEP_ALL))
    {
        ...
        if (r < 0)
            return r;
        r = graph_one_property(bus, u, "Requisite", "darkblue", patterns,
                               from_patterns, to_patterns);
        if (r < 0)
            return r;
...
static int dot(int argc, char* argv[], void* userdata)
{
    ...
    r = sd_bus_call_method(
        bus, "org.freedesktop.systemd1", "/org/freedesktop/systemd1",
        "org.freedesktop.systemd1.Manager", "ListUnits", &error, &reply, "");
    if (r < 0)
        log_error_errno(r, "Failed to list units: %s",
                        bus_error_message(&error, r));
    ...
    while ((r = bus_parse_unit_info(reply, &u)) > 0)
    {
        ...
        r = graph_one(bus, &u, expanded_patterns, expanded_from_patterns,
    ...
    if (on_tty())
        log_notice("-- You probably want to process this output with graphviz' "
                   "dot tool.\n"
                   "-- Try a shell pipeline like 'systemd-analyze dot | dot "
                   "-Tsvg > systemd.svg'!\n");
...
static int dump_fallback(sd_bus* bus)
{
    ...
    r = sd_bus_call_method(
        bus, "org.freedesktop.systemd1", "/org/freedesktop/systemd1",
        "org.freedesktop.systemd1.Manager", "Dump", &error, &reply, NULL);
    if (r < 0)
        return log_error_errno(r, "Failed to issue method call Dump: %s",
                               bus_error_message(&error, r));
...
static int dump(int argc, char* argv[], void* userdata)
{
    ...
    r = sd_bus_call_method(bus, "org.freedesktop.systemd1",
                           ...
                           "DumpByFileDescriptor", &error, &reply, NULL);
    if (r < 0)
    {
        /* fall back to Dump if DumpByFileDescriptor is not supported */
        if (!IN_SET(r, -EACCES, -EBADR))
            return log_error_errno(
                r, "Failed to issue method call DumpByFileDescriptor: %s",
                bus_error_message(&error, r));
...
static int set_log_level(int argc, char* argv[], void* userdata)
{
    ...
    r = sd_bus_set_property(
        bus, "org.freedesktop.systemd1", "/org/freedesktop/systemd1",
        "org.freedesktop.systemd1.Manager", "LogLevel", &error, "s", argv[1]);
    if (r < 0)
        return log_error_errno(r, "Failed to issue method call: %s",
                               bus_error_message(&error, r));
...
static int get_log_level(int argc, char* argv[], void* userdata)
{
    ...
    r = sd_bus_get_property_string(
        bus, "org.freedesktop.systemd1", "/org/freedesktop/systemd1",
        "org.freedesktop.systemd1.Manager", "LogLevel", &error, &level);
    if (r < 0)
        return log_error_errno(r, "Failed to get log level: %s",
                               bus_error_message(&error, r));
...
{
    return (argc == 1) ? get_log_level(argc, argv, userdata)
                       : set_log_level(argc, argv, userdata);
}
...
static int set_log_target(int argc, char* argv[], void* userdata)
{
    ...
    r = sd_bus_set_property(
        bus, "org.freedesktop.systemd1", "/org/freedesktop/systemd1",
        "org.freedesktop.systemd1.Manager", "LogTarget", &error, "s", argv[1]);
    if (r < 0)
        return log_error_errno(r, "Failed to issue method call: %s",
                               bus_error_message(&error, r));
...
static int get_log_target(int argc, char* argv[], void* userdata)
{
    ...
    r = sd_bus_get_property_string(
        bus, "org.freedesktop.systemd1", "/org/freedesktop/systemd1",
        "org.freedesktop.systemd1.Manager", "LogTarget", &error, &target);
    if (r < 0)
        return log_error_errno(r, "Failed to get log target: %s",
                               bus_error_message(&error, r));
...
{
    return (argc == 1) ? get_log_target(argc, argv, userdata)
                       : set_log_target(argc, argv, userdata);
}
...
static int load_kernel_syscalls(Set** ret)
{
    ...
    /* Let's read the available system calls from the list of available tracing
     * events. Slightly dirty, but good enough for analysis purposes. */

    f = fopen("/sys/kernel/tracing/available_events", "re");
    if (!f)
    {
        /* We tried the non-debugfs mount point and that didn't work. If it
         * wasn't mounted, maybe the old debugfs mount point works? */
        f = fopen("/sys/kernel/debug/tracing/available_events", "re");
        if (!f)
            return log_full_errno(
                IN_SET(errno, EPERM, EACCES, ENOENT) ? LOG_DEBUG : LOG_WARNING,
                errno, "Can't read open tracefs' available_events file: %m");
    }
    ...
    for (;;)
    {
        ...
        /* These are named differently inside the kernel than their external
         * name for historical reasons. Let's hide them here. */
        if (STR_IN_SET(e, "newuname", "newfstat", "newstat", "newlstat",
                       "sysctl"))
            continue;
...
static void dump_syscall_filter(const SyscallFilterSet* set)
{
    ...
    printf("%s%s%s\n"
           "    # %s\n",
           ansi_highlight(), set->name, ansi_normal(), set->help);
...
static int dump_syscall_filters(int argc, char* argv[], void* userdata)
{
    ...
    if (strv_isempty(strv_skip(argv, 1)))
    {
        ...
        if (k < 0)
        {
            fputc('\n', stdout);
            fflush(stdout);
            log_notice_errno(k, "# Not showing unlisted system calls, couldn't "
                                "retrieve kernel system call list: %m");
        }
        else if (!set_isempty(kernel))
        {
            ...
            printf(
                "\n"
                "# %sUnlisted System Calls%s (supported by the local kernel, "
                "but not included in any of the groups listed above):\n",
...
static int dump_timespan(int argc, char* argv[], void* userdata)
{
    ...
    STRV_FOREACH(input_timespan, strv_skip(argv, 1))
    {
        ...
        if (r < 0)
            return log_error_errno(r, "Failed to parse time span '%s': %m",
                                   *input_timespan);

        printf("Original: %s\n", *input_timespan);
        printf("      %ss: %" PRIu64 "\n", special_glyph(SPECIAL_GLYPH_MU),
               output_usecs);
        printf("   Human: %s\n", format_timespan(ft_buf, sizeof(ft_buf),
                                                 output_usecs, usec_magnitude));
...
static int test_calendar_one(usec_t n, const char* p)
{
    ...
    if (r < 0)
        return log_error_errno(
            r, "Failed to parse calendar specification '%s': %m", p);

    r = calendar_spec_normalize(spec);
    if (r < 0)
        return log_error_errno(
            r, "Failed to normalize calendar specification '%s': %m", p);

    r = calendar_spec_to_string(spec, &t);
    if (r < 0)
        return log_error_errno(
            r, "Failed to format calendar specification '%s': %m", p);
    ...
    {
        char buffer[CONST_MAX(FORMAT_TIMESTAMP_MAX,
        ...
        if (r < 0)
            return log_error_errno(
                r, "Failed to determine next elapse for '%s': %m", p);
        ...
        else
        {
            ...
            printf(
                "%*sIter. #%u: %s%s%s\n", k, "", i + 1, ansi_highlight_blue(),
                format_timestamp(buffer, sizeof(buffer), next), ansi_normal());
        }
...
static int test_calendar(int argc, char* argv[], void* userdata)
{
    ...
    n = now(CLOCK_REALTIME); /* We want to use the same "base" for all
                                expressions */
...
static int service_watchdogs(int argc, char* argv[], void* userdata)
{
    ...
    if (argc == 1)
    {
        r = sd_bus_get_property_trivial(bus, "org.freedesktop.systemd1",
                                        "/org/freedesktop/systemd1",
                                        "org.freedesktop.systemd1.Manager",
                                        "ServiceWatchdogs", &error, 'b', &b);
        if (r < 0)
            return log_error_errno(r,
                                   "Failed to get service-watchdog state: %s",
                                   bus_error_message(&error, r));
    ...
    r = sd_bus_set_property(
        bus, "org.freedesktop.systemd1", "/org/freedesktop/systemd1",
        "org.freedesktop.systemd1.Manager", "ServiceWatchdogs", &error, "b", b);
    if (r < 0)
        return log_error_errno(r, "Failed to set service-watchdog state: %s",
                               bus_error_message(&error, r));
...
static int help(int argc, char* argv[], void* userdata)
{
    ...
    printf(
        ...
        "     --fuzz=SECONDS        Also print also services which finished "
        "SECONDS\n"
        "                           earlier than the latest in the branch\n"
        "     --man[=BOOL]          Do [not] check for existence of man pages\n"
        "     --generators[=BOOL]   Do [not] run unit generators (requires "
        "privileges)\n"
        ...
        "time to init\n"
        "  critical-chain [UNIT...] Print a tree of the time critical chain of "
        "units\n"
        "  plot                     Output SVG graphic showing service "
        "initialization\n"
        "  dot [UNIT...]            Output dependency graph in man:dot(1) "
        "format\n"
     ...
     * shell-completion/bash/systemd-analyze and
...
static int parse_argv(int argc, char* argv[])
{
    ...
    while ((c = getopt_long(argc, argv, "hH:M:", options, NULL)) >= 0)
        switch (c)
        {
            ...
            case ARG_ITERATIONS:
                ...
                if (r < 0)
                    return log_error_errno(r, "Failed to parse iterations: %s",
                                           optarg);
    ...
    if (arg_scope == UNIT_FILE_GLOBAL &&
        ...
        return log_error_errno(SYNTHETIC_ERRNO(EINVAL),
                               "Option --global only makes sense with verbs "
                               "dot, unit-paths, verify.");
