...
static int retrieve_key(key_serial_t serial, char*** ret)
{
    ...
    for (;;)
    {
        ...
        n = keyctl(KEYCTL_READ, (unsigned long)serial, (unsigned long)p,
                   (unsigned long)m, 0);
        if (n < 0)
            return -errno;
        if (n < m)
        ...
        m *= 2;
        if ((long)(size_t)m !=
            m) /* make sure that this still fits if converted to size_t */
            return -ENOMEM;
...
static int add_to_keyring(const char* keyname, AskPasswordFlags flags,
                          char** passwords)
{
    ...
    if (keyctl(KEYCTL_SET_TIMEOUT, (unsigned long)serial,
...
static int add_to_keyring_and_log(const char* keyname, AskPasswordFlags flags,
                                  char** passwords)
{
    int r;
...
static int ask_password_keyring(const char* keyname, AskPasswordFlags flags,
                                char*** ret)
{
...
static void backspace_string(int ttyfd, const char* str)
{
    ...
    /* Backspaces through enough characters to entirely undo printing of the
     * specified string. */

    m = utf8_n_codepoints(str);
    if (m == (size_t)-1)
        m = strlen(
            str); /* Not a valid UTF-8 string? If so, let's backspace the number
                   * of bytes output. Most likely this happened because we are
                   * not in an UTF-8 locale, and in that case that is the
                   * correct thing to do. And even if it's not, terminals tend
                   * to stop backspacing at the leftmost column, hence
                   * backspacing too much should be mostly OK. */

    backspace_chars(ttyfd, m);
}

int ask_password_tty(int ttyfd, const char* message, const char* keyname,
                     usec_t until, AskPasswordFlags flags,
                     const char* flag_file, char*** ret)
{
    ...
    if (flag_file)
    {
        if (inotify_add_watch(notify, flag_file,
                              IN_ATTRIB /* for the link count */) < 0)
            return -errno;
    }
    if ((flags & ASK_PASSWORD_ACCEPT_CACHED) && keyname)
    {
        ...
        if (inotify_add_watch(notify, "/run/systemd/ask-password",
                              IN_ATTRIB /* for mtime */) < 0)
            return -errno;
    }

    /* If the caller didn't specify a TTY, then use the controlling tty, if we
     * can. */
    if (ttyfd < 0)
    ...
    if (ttyfd >= 0)
    {
        ...
        if (use_color)
            (void)loop_write(ttyfd, ANSI_HIGHLIGHT, STRLEN(ANSI_HIGHLIGHT),
                             false);
    ...
    for (;;)
    {
        ...
        else if (IN_SET(c, '\b', 127))
        {
            ...
            if (p > 0)
            {
                ...
                /* Remove a full UTF-8 codepoint from the end. For that, figure
                 * out where the last one begins */
                q = 0;
                for (;;)
                {
                    size_t z;
            ...
            else if (!dirty && !(flags & ASK_PASSWORD_SILENT))
            {
                ...
                /* There are two ways to enter silent mode. Either by pressing
                 * backspace as first key (and only as first key), or ... */
        ...
        else
        {
            ...
            {
                /* Check if we got a complete UTF-8 character now. If so, let's
                 * output one '*'. */
...
static int create_socket(char** ret)
{
    ...
    if (asprintf(&path, "/run/systemd/ask-password/sck.%" PRIx64,
                 random_u64()) < 0)
        return -ENOMEM;
...
int ask_password_agent(const char* message, const char* icon, const char* id,
                       const char* keyname, usec_t until,
                       AskPasswordFlags flags, char*** ret)
{
    ...
    fprintf(f,
            ...
            "Echo=%i\n"
            "NotAfter=" USEC_FMT "\n",
            getpid_cached(), socket_name,
            (flags & ASK_PASSWORD_ACCEPT_CACHED) ? 1 : 0,
            (flags & ASK_PASSWORD_ECHO) ? 1 : 0, until);
    ...
    for (;;)
    {
        ...
        k = poll(pollfd, notify >= 0 ? _FD_MAX : _FD_MAX - 1,
...
int ask_password_auto(const char* message, const char* icon, const char* id,
                      const char* keyname, usec_t until, AskPasswordFlags flags,
                      char*** ret)
{
    ...
    if (!(flags & ASK_PASSWORD_NO_AGENT))
        return ask_password_agent(message, icon, id, keyname, until, flags,
                                  ret);
