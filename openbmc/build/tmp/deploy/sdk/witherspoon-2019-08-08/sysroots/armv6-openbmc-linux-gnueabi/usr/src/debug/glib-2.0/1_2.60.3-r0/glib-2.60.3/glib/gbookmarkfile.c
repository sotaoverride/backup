...
static gboolean g_bookmark_file_parse(GBookmarkFile* bookmark,
                                      ...
                                      GError** error);
static gchar* g_bookmark_file_dump(GBookmarkFile* bookmark, gsize* length,
                                   GError** error);
...
static void g_bookmark_file_add_item(GBookmarkFile* bookmark,
                                     BookmarkItem* item, GError** error);
...
static void bookmark_metadata_free(BookmarkMetadata* metadata)
{
    ...
    g_list_free_full(metadata->applications,
                     (GDestroyNotify)bookmark_app_info_free);
...
static gchar* bookmark_metadata_dump(BookmarkMetadata* metadata)
{
    ...
    g_string_append(retval, "      "
                            "<" XBEL_METADATA_ELEMENT " " XBEL_OWNER_ATTRIBUTE
    ...
    if (metadata->icon_href)
    {
        ...
        buffer = g_strconcat(
            ...
            metadata->icon_href, "\" " BOOKMARK_TYPE_ATTRIBUTE "=\"",
            metadata->icon_mime, "\"/>\n", NULL);
        g_string_append(retval, buffer);
...
static gchar* bookmark_item_dump(BookmarkItem* item)
{
    ...
    /* at this point, we must have at least a registered application; if we
     * don't we don't screw up the bookmark file, and just skip this item
     */
    ...
    buffer =
        g_strconcat("  <" XBEL_BOOKMARK_ELEMENT " " XBEL_HREF_ATTRIBUTE "=\"",
                    escaped_uri, "\" " XBEL_ADDED_ATTRIBUTE "=\"", added,
...
static BookmarkAppInfo* bookmark_item_lookup_app_info(BookmarkItem* item,
...
static void parse_bookmark_element(GMarkupParseContext* context,
                                   ...
                                   GError** error)
{
    ...
    for (attr = attribute_names[i]; attr != NULL; attr = attribute_names[++i])
    {
        ...
        else
        {
            ...
            g_set_error(error, G_MARKUP_ERROR, G_MARKUP_ERROR_UNKNOWN_ATTRIBUTE,
                        _("Unexpected attribute “%s” for element “%s”"), attr,
    ...
    if (!uri)
    {
        g_set_error(error, G_MARKUP_ERROR, G_MARKUP_ERROR_INVALID_CONTENT,
                    ...
                    XBEL_HREF_ATTRIBUTE, XBEL_BOOKMARK_ELEMENT);
        return;
    }
...
static void parse_application_element(GMarkupParseContext* context,
                                      ParseData* parse_data,
                                      const gchar** attribute_names,
                                      const gchar** attribute_values,
                                      GError** error)
{
    ...
    if (!name)
    {
        g_set_error(error, G_MARKUP_ERROR, G_MARKUP_ERROR_INVALID_CONTENT,
                    _("Attribute “%s” of element “%s” not found"),
                    BOOKMARK_NAME_ATTRIBUTE, BOOKMARK_APPLICATION_ELEMENT);
        return;
    }
    ...
    if (!exec)
    {
        g_set_error(error, G_MARKUP_ERROR, G_MARKUP_ERROR_INVALID_CONTENT,
                    _("Attribute “%s” of element “%s” not found"),
                    BOOKMARK_EXEC_ATTRIBUTE, BOOKMARK_APPLICATION_ELEMENT);
        return;
    }
    ...
    if (!ai)
    {
        ...
        item->metadata->applications =
            g_list_prepend(item->metadata->applications, ai);
...
static void parse_mime_type_element(GMarkupParseContext* context,
                                    ParseData* parse_data,
                                    const gchar** attribute_names,
                                    const gchar** attribute_values,
                                    GError** error)
...
static void parse_icon_element(GMarkupParseContext* context,
                               ParseData* parse_data,
                               const gchar** attribute_names,
                               const gchar** attribute_values, GError** error)
{
    ...
    if (!href)
    {
        g_set_error(error, G_MARKUP_ERROR, G_MARKUP_ERROR_INVALID_CONTENT,
                    _("Attribute “%s” of element “%s” not found"),
                    BOOKMARK_HREF_ATTRIBUTE, BOOKMARK_ICON_ELEMENT);
        return;
    }
 ...
 */
static void map_namespace_to_name(ParseData* parse_data,
                                  const gchar** attribute_names,
                                  const gchar** attribute_values)
{
    const gchar* attr;
    ...
    for (attr = attribute_names[i]; attr; attr = attribute_names[++i])
    {
        if (g_str_has_prefix(attr, "xmlns"))
        {
            ...
            g_hash_table_replace(parse_data->namespaces, namespace_name,
                                 namespace_uri);
        }
    }
}
 ...
 */
static gboolean is_element_full(ParseData* parse_data,
                                const gchar* element_full,
                                const gchar* namespace, const gchar* element,
                                const gchar sep)
{
    ...
    retval =
        (0 == strcmp(ns_uri, namespace) && 0 == strcmp(element_name, element));
...
static const gchar* parser_state_to_element_name(ParserState state)
{
    switch (state)
    {
        case STATE_STARTED:
...
static void start_element_raw_cb(GMarkupParseContext* context,
    ...
    switch (parse_data->state)
    {
        case STATE_STARTED:
            if (IS_ELEMENT(parse_data, element_name, XBEL_ROOT_ELEMENT))
            {
                ...
                i = 0;
                for (attr = attribute_names[i]; attr;
        ...
        case STATE_ROOT:
            ...
            else if (IS_ELEMENT(parse_data, element_name,
                                XBEL_BOOKMARK_ELEMENT))
            {
                ...
                parse_bookmark_element(context, parse_data, attribute_names,
                                       attribute_values, &inner_error);
                if (inner_error)
                ...
                g_set_error(error, G_MARKUP_ERROR,
                            G_MARKUP_ERROR_INVALID_CONTENT,
                            _("Unexpected tag “%s” inside “%s”"), element_name,
                            XBEL_ROOT_ELEMENT);
            break;
        case STATE_BOOKMARK:
            if (IS_ELEMENT(parse_data, element_name, XBEL_TITLE_ELEMENT))
            ...
            if (IS_ELEMENT(parse_data, element_name, XBEL_METADATA_ELEMENT))
            {
                     ...
                     attr = attribute_names[++i])
                {
                    if ((IS_ATTRIBUTE(attr, XBEL_OWNER_ATTRIBUTE)) &&
                        (0 ==
                         strcmp(attribute_values[i], BOOKMARK_METADATA_OWNER)))
                    {
                ...
                g_set_error(error, G_MARKUP_ERROR,
                            G_MARKUP_ERROR_INVALID_CONTENT,
                            _("Unexpected tag “%s”, tag “%s” expected"),
                            element_name, XBEL_METADATA_ELEMENT);
            break;
        case STATE_METADATA:
            if (IS_ELEMENT_NS(parse_data, element_name, BOOKMARK_NAMESPACE_URI,
                              BOOKMARK_APPLICATIONS_ELEMENT))
                                   ...
                                   BOOKMARK_ICON_ELEMENT))
            {
                ...
                parse_icon_element(context, parse_data, attribute_names,
                                   attribute_values, &inner_error);
                if (inner_error)
                    g_propagate_error(error, inner_error);
            }
            else if (IS_ELEMENT_NS(parse_data, element_name, MIME_NAMESPACE_URI,
                                   MIME_TYPE_ELEMENT))
            {
                GError* inner_error = NULL;
                ...
                parse_mime_type_element(context, parse_data, attribute_names,
                                        attribute_values, &inner_error);
                if (inner_error)
                    g_propagate_error(error, inner_error);
            }
        ...
        case STATE_APPLICATIONS:
                              ...
                              BOOKMARK_APPLICATION_ELEMENT))
            {
                ...
                parse_application_element(context, parse_data, attribute_names,
                                          attribute_values, &inner_error);
                if (inner_error)
                    g_propagate_error(error, inner_error);
            }
                ...
                g_set_error(error, G_MARKUP_ERROR,
                            G_MARKUP_ERROR_INVALID_CONTENT,
                            _("Unexpected tag “%s”, tag “%s” expected"),
                            element_name, BOOKMARK_APPLICATION_ELEMENT);
            break;
        case STATE_GROUPS:
            if (IS_ELEMENT_NS(parse_data, element_name, BOOKMARK_NAMESPACE_URI,
                ...
                g_set_error(error, G_MARKUP_ERROR,
                            G_MARKUP_ERROR_INVALID_CONTENT,
                            _("Unexpected tag “%s”, tag “%s” expected"),
                            element_name, BOOKMARK_GROUP_ELEMENT);
            break;
...
static void end_element_raw_cb(GMarkupParseContext* context,
                               ...
                               GError** error)
{
             ...
             (IS_ELEMENT(parse_data, element_name, XBEL_DESC_ELEMENT)))
    {
        ...
        parse_data->state = STATE_INFO;
    else if (IS_ELEMENT_NS(parse_data, element_name, BOOKMARK_NAMESPACE_URI,
                           BOOKMARK_APPLICATION_ELEMENT))
        parse_data->state = STATE_APPLICATIONS;
    else if (IS_ELEMENT_NS(parse_data, element_name, BOOKMARK_NAMESPACE_URI,
                           BOOKMARK_GROUP_ELEMENT))
        parse_data->state = STATE_GROUPS;
                            ...
                            BOOKMARK_APPLICATIONS_ELEMENT)) ||
             (IS_ELEMENT_NS(parse_data, element_name, BOOKMARK_NAMESPACE_URI,
                            BOOKMARK_GROUPS_ELEMENT)) ||
             (IS_ELEMENT_NS(parse_data, element_name, BOOKMARK_NAMESPACE_URI,
                            BOOKMARK_PRIVATE_ELEMENT)) ||
             (IS_ELEMENT_NS(parse_data, element_name, BOOKMARK_NAMESPACE_URI,
                            BOOKMARK_ICON_ELEMENT)) ||
             (IS_ELEMENT_NS(parse_data, element_name, MIME_NAMESPACE_URI,
                            MIME_TYPE_ELEMENT)))
        parse_data->state = STATE_METADATA;
}

static void text_raw_cb(GMarkupParseContext* context, const gchar* text,
    ...
    switch (parse_data->state)
    {
        ...
        case STATE_GROUP:
        {
            ...
            parse_data->current_item->metadata->groups =
                g_list_prepend(groups, g_strdup(payload));
        }
        break;
        case STATE_ROOT:
        case STATE_BOOKMARK:
                                      ...
                                      GError** error)
{
    ...
    context = g_markup_parse_context_new(&markup_parser, 0, parse_data,
                                         (GDestroyNotify)parse_data_free);

    retval =
        g_markup_parse_context_parse(context, buffer, length, &parse_error);
    if (!retval)
                                   ...
                                   GError** error)
{
    ...
    g_string_append(retval, "<?xml version=\"1.0\" encoding=\"UTF-8\"?>\n"
		   ...
		   "         \"" XBEL_DTD_URI "\">\n"
#endif
                            "<" XBEL_ROOT_ELEMENT " " XBEL_VERSION_ATTRIBUTE
                            "=\"" XBEL_VERSION "\"\n"
                            "      xmlns:" BOOKMARK_NAMESPACE_NAME
                            "=\"" BOOKMARK_NAMESPACE_URI "\"\n"
                            "      xmlns:" MIME_NAMESPACE_NAME
                            "=\"" MIME_NAMESPACE_URI "\"\n>");
 ...
 */
static gchar* find_file_in_data_dirs(const gchar* file, gchar*** dirs,
                                     GError** error)
{
                                             ...
                                             gchar** full_path, GError** error)
{
    ...
    all_data_dirs =
        g_new0(gchar*, g_strv_length((gchar**)system_data_dirs) + 2);
    ...
    while (*data_dirs != NULL && !found_file)
    {
        ...
        found_file =
            g_bookmark_file_load_from_file(bookmark, output_path, &file_error);
        if (file_error)
        {
            g_propagate_error(error, file_error);
            break;
 ...
 * Since: 2.12
 */
gchar* g_bookmark_file_to_data(GBookmarkFile* bookmark, gsize* length,
                               GError** error)
{
 ...
 * Since: 2.12
 */
gboolean g_bookmark_file_to_file(GBookmarkFile* bookmark, const gchar* filename,
                                 GError** error)
{
    gchar* data;
                                     ...
                                     BookmarkItem* item, GError** error)
{
    ...
    if (G_UNLIKELY(g_bookmark_file_has_item(bookmark, item->uri)))
    {
        g_set_error(error, G_BOOKMARK_FILE_ERROR,
                    ...
                    _("A bookmark for URI “%s” already exists"), item->uri);
        return;
    }
 ...
 * Since: 2.12
 */
gboolean g_bookmark_file_remove_item(GBookmarkFile* bookmark, const gchar* uri,
                                     GError** error)
{
    BookmarkItem* item;
    ...
    if (!item)
    {
        g_set_error(error, G_BOOKMARK_FILE_ERROR,
                    ...
                    _("No bookmark found for URI “%s”"), uri);
        return FALSE;
    }
 ...
 * @bookmark: a #GBookmarkFile
 * @length: (out) (optional): return location for the number of returned URIs,
 * or %NULL
 *
 ...
 * Since: 2.12
 */
void g_bookmark_file_set_title(GBookmarkFile* bookmark, const gchar* uri,
                               const gchar* title)
{
 ...
 * Since: 2.12
 */
gchar* g_bookmark_file_get_title(GBookmarkFile* bookmark, const gchar* uri,
                                 GError** error)
{
    BookmarkItem* item;
 ...
 * Since: 2.12
 */
void g_bookmark_file_set_description(GBookmarkFile* bookmark, const gchar* uri,
 ...
 * Since: 2.12
 */
void g_bookmark_file_set_mime_type(GBookmarkFile* bookmark, const gchar* uri,
 ...
 * Since: 2.12
 */
gchar* g_bookmark_file_get_mime_type(GBookmarkFile* bookmark, const gchar* uri,
                                     GError** error)
{
    BookmarkItem* item;
    ...
    {
        g_set_error(
            error, G_BOOKMARK_FILE_ERROR, G_BOOKMARK_FILE_ERROR_INVALID_VALUE,
            _("No MIME type defined in the bookmark for URI “%s”"), uri);
        return NULL;
    }
 ...
 * Since: 2.12
 */
void g_bookmark_file_set_is_private(GBookmarkFile* bookmark, const gchar* uri,
                                    gboolean is_private)
{
    BookmarkItem* item;
 ...
 * Since: 2.12
 */
void g_bookmark_file_set_added(GBookmarkFile* bookmark, const gchar* uri,
                               time_t added)
{
    BookmarkItem* item;
 ...
 * Since: 2.12
 */
time_t g_bookmark_file_get_added(GBookmarkFile* bookmark, const gchar* uri,
                                 GError** error)
{
    BookmarkItem* item;
 ...
 * Since: 2.12
 */
void g_bookmark_file_set_modified(GBookmarkFile* bookmark, const gchar* uri,
                                  time_t modified)
{
    BookmarkItem* item;
 ...
 * Since: 2.12
 */
time_t g_bookmark_file_get_modified(GBookmarkFile* bookmark, const gchar* uri,
                                    GError** error)
{
    BookmarkItem* item;
 ...
 * Since: 2.12
 */
void g_bookmark_file_set_visited(GBookmarkFile* bookmark, const gchar* uri,
                                 time_t visited)
{
    BookmarkItem* item;
 ...
 * Since: 2.12
 */
time_t g_bookmark_file_get_visited(GBookmarkFile* bookmark, const gchar* uri,
                                   GError** error)
{
    BookmarkItem* item;
 ...
 * Since: 2.12
 */
gboolean g_bookmark_file_has_group(GBookmarkFile* bookmark, const gchar* uri,
 ...
 * Since: 2.12
 */
void g_bookmark_file_add_group(GBookmarkFile* bookmark, const gchar* uri,
                               const gchar* group)
{
    BookmarkItem* item;
    ...
    if (!g_bookmark_file_has_group(bookmark, uri, group, NULL))
    {
        item->metadata->groups =
            g_list_prepend(item->metadata->groups, g_strdup(group));
 ...
 * Since: 2.12
 */
gboolean g_bookmark_file_remove_group(GBookmarkFile* bookmark, const gchar* uri,
                                      const gchar* group, GError** error)
{
    BookmarkItem* item;
    GList* l;
    ...
    if (!item->metadata)
    {
        g_set_error(error, G_BOOKMARK_FILE_ERROR,
                    ...
                    _("No groups set in bookmark for URI “%s”"), uri);
        return FALSE;
    }
    ...
    for (l = item->metadata->groups; l != NULL; l = l->next)
    {
        ...
        {
            item->metadata->groups =
                g_list_remove_link(item->metadata->groups, l);
            g_free(l->data);
            g_list_free_1(l);
 ...
 * Since: 2.12
 */
void g_bookmark_file_set_groups(GBookmarkFile* bookmark, const gchar* uri,
                                const gchar** groups, gsize length)
{
    BookmarkItem* item;
    gsize i;
    ...
    if (groups)
    {
        for (i = 0; i < length && groups[i] != NULL; i++)
            item->metadata->groups =
                g_list_append(item->metadata->groups, g_strdup(groups[i]));
    }
 ...
 *
 * Returns: (array length=length) (transfer full): a newly allocated
 * %NULL-terminated array of group names. Use g_strfreev() to free it.
 *
 * Since: 2.12
 */
gchar** g_bookmark_file_get_groups(GBookmarkFile* bookmark, const gchar* uri,
 ...
 * Since: 2.12
 */
void g_bookmark_file_add_application(GBookmarkFile* bookmark, const gchar* uri,
                                     const gchar* name, const gchar* exec)
{
    BookmarkItem* item;
    ...
    g_bookmark_file_set_app_info(bookmark, uri, app_name, app_exec, -1,
                                 (time_t)-1, NULL);
                                            ...
                                            GError** error)
{
    ...
    set_error = NULL;
    retval = g_bookmark_file_set_app_info(bookmark, uri, name, "", 0,
                                          (time_t)-1, &set_error);
    if (set_error)
    {
 ...
 * Since: 2.12
 */
gboolean g_bookmark_file_set_app_info(GBookmarkFile* bookmark, const gchar* uri,
                                      const gchar* name, const gchar* exec,
                                      gint count, time_t stamp, GError** error)
{
    BookmarkItem* item;
    BookmarkAppInfo* ai;
    ...
    if (!ai)
    {
        if (count == 0)
        {
            g_set_error(error, G_BOOKMARK_FILE_ERROR,
                        G_BOOKMARK_FILE_ERROR_APP_NOT_REGISTERED,
                        _("No application with name “%s” registered a bookmark "
                          "for “%s”"),
                        name, uri);
            return FALSE;
        }
    ...
    {
        item->metadata->applications =
            g_list_remove(item->metadata->applications, ai);
 ...
 * @uri: a valid URI
 * @name: an application's name
 * @exec: (out) (optional): return location for the command line of the
 * application, or %NULL
 * @count: (out) (optional): return location for the registration count, or
 * %NULL
 * @stamp: (out) (optional): return location for the last registration time, or
 * %NULL
 * @error: return location for a #GError, or %NULL
 *
 ...
 * Since: 2.12
 */
gboolean g_bookmark_file_get_app_info(GBookmarkFile* bookmark, const gchar* uri,
                                      const gchar* name, gchar** exec,
                                      guint* count, time_t* stamp,
                                      GError** error)
{
    BookmarkItem* item;
    BookmarkAppInfo* ai;
 ...
 * @bookmark: a #GBookmarkFile
 * @uri: a valid URI
 * @length: (out) (optional): return location of the length of the returned
 * list, or %NULL
 * @error: return location for a #GError, or %NULL
 *
                                         ...
                                         GError** error)
{
    ...
    for (l = g_list_last(item->metadata->applications), i = 0; l != NULL;
         l = l->prev)
    {
        BookmarkAppInfo* ai;
                                   ...
                                   GError** error)
{
    ...
    if (!item)
    {
        g_set_error(error, G_BOOKMARK_FILE_ERROR,
                    G_BOOKMARK_FILE_ERROR_URI_NOT_FOUND,
                    _("No bookmark found for URI “%s”"), old_uri);
        return FALSE;
    }
 ...
 * Since: 2.12
 */
void g_bookmark_file_set_icon(GBookmarkFile* bookmark, const gchar* uri,
 ...
 * @mime_type: (out) (optional): return location for the icon's MIME type or
 * %NULL
 * @error: return location for a #GError or %NULL
 *
 ...
 * Since: 2.12
 */
gboolean g_bookmark_file_get_icon(GBookmarkFile* bookmark, const gchar* uri,
                                  gchar** href, gchar** mime_type,
                                  GError** error)
{
    BookmarkItem* item;
