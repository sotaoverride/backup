...
int dns_server_new(Manager* m, DnsServer** ret, DnsServerType type, Link* l,
                   int family, const union in_addr_union* in_addr, int ifindex)
{
    ...
    *s = (DnsServer){
     ...
     * the new one */
    if (type != DNS_SERVER_FALLBACK && m->current_dns_server &&
...
void dns_server_move_back_and_unmark(DnsServer* s)
{
    ...
    switch (s->type)
    {
        ...
        case DNS_SERVER_FALLBACK:
            LIST_FIND_TAIL(servers, s, tail);
            LIST_REMOVE(servers, s->manager->fallback_dns_servers, s);
            LIST_INSERT_AFTER(servers, s->manager->fallback_dns_servers, tail,
                              s);
            break;
...
static void dns_server_verified(DnsServer* s, DnsServerFeatureLevel level)
{
    ...
    if (s->verified_feature_level != level)
    {
        log_debug("Verified we get a response at feature level %s from DNS "
                  "server %s.",
    ...
    assert_se(sd_event_now(s->manager->event, clock_boottime_or_monotonic(),
                           &s->verified_usec) >= 0);
}
...
static void dns_server_reset_counters(DnsServer* s)
{
    ...
    /* Note that we do not reset s->packet_bad_opt and s->packet_rrsig_missing
     * here. We reset them only when the grace period ends, but not when
     * lowering the possible feature level, as a lower level feature level
     * should not make RRSIGs appear or OPT appear, but rather make them
     * disappear. If the reappear anyway, then that's indication for a
     * differently broken OPT/RRSIG implementation, and we really don't want to
     * support that either.
     *
     * This is particularly important to deal with certain Belkin routers which
     * break OPT for certain lookups (A), but pass traffic through for others
     * (AAAA). If we detect the broken behaviour on one lookup we should not
     * reenable it for another, because we cannot validate things anyway, given
     * that the RRSIG/OPT data will be incomplete. */
}

void dns_server_packet_received(DnsServer* s, int protocol,
                                DnsServerFeatureLevel level, size_t size)
{
    assert(s);
    ...
    else if (protocol == IPPROTO_TCP)
    {
        ...
        else
        {
            ...
            /* Successful TCP connections are only useful to verify the TCP
             * feature level. */
    ...
    if (s->packet_rrsig_missing && level >= DNS_SERVER_FEATURE_LEVEL_DO)
        level = DNS_SERVER_FEATURE_LEVEL_IS_TLS(level)
    ...
    /* Even if we successfully receive a reply to a request announcing support
       for large packets, that does not mean we can necessarily receive large
       packets. */
...
void dns_server_packet_lost(DnsServer* s, int protocol,
...
void dns_server_packet_rrsig_missing(DnsServer* s, DnsServerFeatureLevel level)
{
    ...
    /* If the RRSIG RRs are missing, we have to downgrade what we previously
     * verified */
    if (s->verified_feature_level >= DNS_SERVER_FEATURE_LEVEL_DO)
        s->verified_feature_level = DNS_SERVER_FEATURE_LEVEL_IS_TLS(level)
                                        ? DNS_SERVER_FEATURE_LEVEL_TLS_PLAIN
                                        : DNS_SERVER_FEATURE_LEVEL_EDNS0;
...
void dns_server_packet_rcode_downgrade(DnsServer* s,
                                       DnsServerFeatureLevel level)
{
    assert(s);

    /* Invoked whenever we got a FORMERR, SERVFAIL or NOTIMP rcode from a server
     * and downgrading the feature level for the transaction made it go away. In
     * this case we immediately downgrade to the feature level that made
     * things work. */
    ...
    log_debug("Downgrading transaction feature level fixed an RCODE error, "
              "downgrading server %s too.",
              dns_server_string(s));
}
...
static bool dns_server_grace_period_expired(DnsServer* s)
{
    ...
    s->features_grace_period_usec =
        MIN(s->features_grace_period_usec * 2,
...
DnsServerFeatureLevel dns_server_possible_feature_level(DnsServer* s)
{
    ...
    /* Determine the best feature level we care about. If DNSSEC mode is off
     * there's no point in using anything better than EDNS0, hence don't even
     * try. */
    ...
    /* Clamp the feature level the highest level we care about. The DNSSEC mode
     * might have changed since the last time, hence let's downgrade if we are
    ...
    if (s->possible_feature_level < best && dns_server_grace_period_expired(s))
    {
        ...
        log_info("Grace period over, resuming full feature set (%s) for DNS "
                 "server %s.",
    ...
    else
    {
            ...
            s->possible_feature_level == DNS_SERVER_FEATURE_LEVEL_TCP)
        {
            ...
            /* We are at the TCP (lowest) level, and we tried a couple of TCP
             * connections, and it didn't work. Upgrade back to UDP again. */
            log_debug("Reached maximum number of failed TCP connection "
                      "attempts, trying UDP again...");
                 ...
                 DNS_SERVER_FEATURE_LEVEL_IS_TLS(s->possible_feature_level))
        {
            ...
            /* We tried to connect using DNS-over-TLS, and it didn't work.
             * Downgrade to plaintext UDP if we don't require DNS-over-TLS */

            log_debug(
                "Server doesn't support DNS-over-TLS, downgrading protocol...");
                 ...
                 s->possible_feature_level >= DNS_SERVER_FEATURE_LEVEL_EDNS0)
        {
            ...
            /* A reply to one of our EDNS0 queries didn't carry a valid OPT RR,
             * then downgrade to below EDNS0 levels. After all, some records
             * generate different responses with and without OPT RR in the
             * request. Example:
             * https://open.nlnetlabs.nl/pipermail/dnssec-trigger/2014-November/000376.html
             */

            log_debug("Server doesn't support EDNS(0) properly, downgrading "
                      "feature level...");
            s->possible_feature_level = DNS_SERVER_FEATURE_LEVEL_UDP;
        }
                 ...
                 s->possible_feature_level >= DNS_SERVER_FEATURE_LEVEL_DO)
        {
            ...
            /* RRSIG data was missing on a EDNS0 packet with DO bit set. This
             * means the server doesn't augment responses with DNSSEC RRs. If
             * so, let's better not ask the server for it anymore, after all
             * some servers generate different replies depending if an OPT RR is
             * in the query or not. */

            log_debug("Detected server responses lack RRSIG records, "
                      "downgrading feature level...");
            s->possible_feature_level =
                DNS_SERVER_FEATURE_LEVEL_IS_TLS(s->possible_feature_level)
                    ? DNS_SERVER_FEATURE_LEVEL_TLS_PLAIN
                    : DNS_SERVER_FEATURE_LEVEL_EDNS0;
                          ...
                          : DNS_SERVER_FEATURE_LEVEL_UDP))
        {
            ...
            /* We lost too many UDP packets in a row, and are on a feature level
             * of UDP or higher. If the packets are lost, maybe the server
             * cannot parse them, hence downgrading sounds like a good idea. We
             * might downgrade all the way down to TCP this way.
             *
             * If strict DNSSEC mode is used we won't downgrade below DO level
             * however, as packet loss might have many reasons, a broken DNSSEC
             * implementation being only one reason. And if the user is strict
             * on DNSSEC, then let's assume that DNSSEC is not the fault here.
             */
                          ...
                          : DNS_SERVER_FEATURE_LEVEL_UDP))
        {
            ...
            /* We got too many TCP connection failures in a row, we had at least
             * one truncated packet, and are on a feature level above UDP. By
             * downgrading things and getting rid of DNSSEC or EDNS0 data we
             * hope to make the packet smaller, so that it still works via UDP
             * given that TCP appears not to be a fallback. Note that if we are
             * already at the lowest UDP level, we don't
             * go further down, since that's TCP, and TCP failed too often after
             * all. */

            log_debug("Got too many failed TCP connection failures and "
                      "truncated UDP packets, downgrading feature level...");
            s->possible_feature_level--;
        }
        ...
        if (p != s->possible_feature_level)
        {
            ...
            log_warning(
                "Using degraded feature set (%s) for DNS server %s.",
                dns_server_feature_level_to_string(s->possible_feature_level),
                dns_server_string(s));
        }
    }
...
int dns_server_adjust_opt(DnsServer* server, DnsPacket* packet,
                          DnsServerFeatureLevel level)
{
    size_t packet_size;
    bool edns_do;
...
const char* dns_server_string(DnsServer* server)
{
    ...
    if (!server->server_string)
        (void)in_addr_ifindex_to_string(server->family, &server->address,
...
bool dns_server_dnssec_supported(DnsServer* server)
{
    ...
    /* Returns whether the server supports DNSSEC according to what we know
     * about it */
    ...
    /* DNSSEC servers need to support TCP properly (see RFC5966), if they don't,
     * we assume DNSSEC is borked too */
...
void dns_server_warn_downgrade(DnsServer* server)
{
    ...
    log_struct(
        LOG_NOTICE, "MESSAGE_ID=" SD_MESSAGE_DNSSEC_DOWNGRADE_STR,
        LOG_MESSAGE("Server %s does not support DNSSEC, downgrading to "
                    "non-DNSSEC mode.",
...
DEFINE_HASH_OPS(dns_server_hash_ops, DnsServer, dns_server_hash_func,
...
DnsServer* dns_server_find(DnsServer* first, int family,
                           const union in_addr_union* in_addr, int ifindex)
{
    DnsServer* s;
...
DnsServer* manager_set_dns_server(Manager* m, DnsServer* s)
{
    ...
    if (s)
        log_debug("Switching to %s DNS server %s.",
                  dns_server_type_to_string(s->type), dns_server_string(s));
...
bool dns_server_address_valid(int family, const union in_addr_union* sa)
{
    ...
    /* Refuses the 0 IP addresses as well as 127.0.0.53 (which is our own DNS
     * stub) */
...
void dns_server_flush_cache(DnsServer* s)
{
    ...
    current =
        s->link ? s->link->current_dns_server : s->manager->current_dns_server;
    if (current != s)
        return;
...
void dns_server_dump(DnsServer* s, FILE* f)
{
    ...
    fputs(strna(dns_server_feature_level_to_string(s->verified_feature_level)),
          f);
    fputc('\n', f);

    fputs("\tPossible feature level: ", f);
    fputs(strna(dns_server_feature_level_to_string(s->possible_feature_level)),
          f);
    fputc('\n', f);
    ...
    fprintf(f,
            ...
            yes_no(s->packet_truncated), yes_no(s->packet_bad_opt),
...
void dns_server_unref_stream(DnsServer* s)
{
    ...
    /* Detaches the default stream of this server. Some special care needs to be
     * taken here, as that stream and this server reference each other. First,
     * take the stream out of the server. It's destructor will check if it is
     * registered with us, hence let's invalidate this separatly, so that it is
...
static const char* const
    dns_server_feature_level_table[_DNS_SERVER_FEATURE_LEVEL_MAX] = {
