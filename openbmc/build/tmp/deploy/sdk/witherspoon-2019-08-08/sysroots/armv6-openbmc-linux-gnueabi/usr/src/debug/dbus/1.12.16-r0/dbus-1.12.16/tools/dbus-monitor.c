...
{
    printf("#type\ttimestamp\tserial\tsender\tdestination\tpath\tinterface\tmem"
           "ber\n");
    printf("#\t\t\t\t\tin_reply_to\n");
}

static void profile_print_with_attrs(const char* type, DBusMessage* message,
                                     long sec, long usec,
...
static DBusHandlerResult binary_filter_func(DBusConnection* connection,
                                            DBusMessage* message,
                                            void* user_data)
{
...
{
    fprintf(stderr,
            "Usage: %s [--system | --session | --address ADDRESS] [--monitor | "
            "--profile | --pcap | --binary ] [watch expressions]\n",
            name);
    exit(ecode);
}
...
int main(int argc, char* argv[])
{
    ...
    switch (binary_mode)
    {
        ...
        case BINARY_MODE_PCAP:
        {
            ...
            } header = {0xA1B2C3D4U, /* magic number */
                        2,
                        4,         /* v2.4 */
                        0,         /* capture in GMT */
