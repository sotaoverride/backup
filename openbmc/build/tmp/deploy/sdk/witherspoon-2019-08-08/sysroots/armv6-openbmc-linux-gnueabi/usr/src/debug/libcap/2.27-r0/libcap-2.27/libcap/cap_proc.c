...
{
    int result;
    result = prctl(PR_CAP_AMBIENT, pr_arg(PR_CAP_AMBIENT_IS_SET), pr_arg(cap),
                   pr_arg(0), pr_arg(0));
    if (result < 0)
    {
        errno = -result;
...
int cap_set_ambient(cap_value_t cap, cap_flag_value_t set)
{
    ...
    result =
        prctl(PR_CAP_AMBIENT, pr_arg(val), pr_arg(cap), pr_arg(0), pr_arg(0));
    if (result < 0)
    {
        errno = -result;
        return -1;
...
int cap_reset_ambient()
{
    ...
    result = prctl(PR_CAP_AMBIENT, pr_arg(PR_CAP_AMBIENT_CLEAR_ALL), pr_arg(0),
                   pr_arg(0), pr_arg(0));
    if (result < 0)
    {
        errno = -result;
