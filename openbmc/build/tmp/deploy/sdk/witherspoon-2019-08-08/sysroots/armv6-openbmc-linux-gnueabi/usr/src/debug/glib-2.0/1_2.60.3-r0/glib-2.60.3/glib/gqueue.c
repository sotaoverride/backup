 ...
 * Since: 2.4
 */
void g_queue_sort(GQueue* queue, GCompareDataFunc compare_func,
                  gpointer user_data)
{
    g_return_if_fail(queue != NULL);
...
void g_queue_insert_before(GQueue* queue, GList* sibling, gpointer data)
{
    ...
    if (sibling == NULL)
    {
         ...
         * would be a O(n) operation and we would need to update manually the
         * tail pointer.
         */
        g_queue_push_tail(queue, data);
    }
 ...
 * Since: 2.4
 */
void g_queue_insert_sorted(GQueue* queue, gpointer data, GCompareDataFunc func,
                           gpointer user_data)
{
    GList* list;
