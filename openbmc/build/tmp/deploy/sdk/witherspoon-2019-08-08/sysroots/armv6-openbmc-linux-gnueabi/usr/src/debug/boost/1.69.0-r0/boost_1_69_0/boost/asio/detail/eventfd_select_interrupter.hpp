...
class eventfd_select_interrupter
{
  ...
  private:
    ...
    // descriptor is passed to select such that when it is time to stop, a
    // single 64bit value will be written on the other end of the connection and
    // this descriptor will become readable.
    int read_descriptor_;

    // The write end of a connection used to interrupt the select call. A single
    // 64bit non-zero value may be written to this to wake up the select which
    // is waiting for the other end to become readable. This descriptor will
