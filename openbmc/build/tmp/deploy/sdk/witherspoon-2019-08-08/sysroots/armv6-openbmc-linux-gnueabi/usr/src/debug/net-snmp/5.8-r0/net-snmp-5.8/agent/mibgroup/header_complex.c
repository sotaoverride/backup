        ...
        switch (var->type)
        {
            ...
            case ASN_TIMETICKS:
                ...
                *var->val.integer = (long)*oidIndex++;
                ...
                oidLen--;
                DEBUGMSGTL(("header_complex_parse_oid", "Parsed int(%d): %ld\n",
                                  ...
                                  oid* searchfor, size_t searchfor_len)
{
    ...
    {
        if (netsnmp_oid_equals(searchfor, searchfor_len, nptr->name,
                               nptr->namelen) == 0)
            return nptr->data;
    }
    return NULL;
                                  ...
                                  int dont_allow_duplicates)
{
    ...
    ret = header_complex_maybe_add_data_by_oid(thedata, newoid, newoid_len,
                                               data, dont_allow_duplicates);
    /*
                                ...
                                oid* newoid, size_t newoid_len, void* data)
{
     ...
     */
    ourself =
        (struct header_complex_index*)SNMP_MALLOC_STRUCT(header_complex_index);
    if (ourself == NULL)
        return NULL;
...
struct header_complex_index*
    header_complex_maybe_add_data_by_oid(struct header_complex_index** thedata,
                                         oid* newoid, size_t newoid_len,
                                         void* data, int dont_allow_duplicates)
{
         ...
         hciptrp = hciptrn, hciptrn = hciptrn->next)
    {
         ...
         */
        rc = snmp_oid_compare(hciptrn->name, hciptrn->namelen, newoid,
                              newoid_len);
        if (rc > 0)
            break;
        else if (0 == rc)
    ...
    return _header_complex_add_between(thedata, hciptrp, hciptrn, newoid,
                                       newoid_len, data);
}
...
void header_complex_dump(struct header_complex_index* thestuff)
{
    struct header_complex_index* hciptr;
...
main()
{
    ...
    ret = header_complex_parse_oid(testparse, sizeof(testparse) / sizeof(oid),
                                   vars);
