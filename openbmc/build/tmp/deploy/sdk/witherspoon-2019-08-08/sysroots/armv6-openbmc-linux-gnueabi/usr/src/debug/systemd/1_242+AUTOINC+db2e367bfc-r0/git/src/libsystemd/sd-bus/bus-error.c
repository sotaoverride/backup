...
BUS_ERROR_MAP_ELF_REGISTER const sd_bus_error_map bus_standard_errors[] = {
    ...
    SD_BUS_ERROR_MAP("org.freedesktop.DBus.Error.SELinuxSecurityContextUnknown",
                     ESRCH),
...
static int bus_error_name_to_errno(const char* name)
{
    ...
    if (additional_error_maps)
        for (map = additional_error_maps; *map; map++)
            for (m = *map;; m++)
            {
                /* For additional error maps the end marker is actually the end
                 * marker */
...
static sd_bus_error errno_to_bus_error_const(int error)
{
    ...
    switch (error)
    {
        ...
        case EPERM:
        case EACCES:
            return SD_BUS_ERROR_MAKE_CONST(SD_BUS_ERROR_ACCESS_DENIED,
                                           "Access denied");

        case EINVAL:
            return SD_BUS_ERROR_MAKE_CONST(SD_BUS_ERROR_INVALID_ARGS,
                                           "Invalid argument");

        case ESRCH:
            return SD_BUS_ERROR_MAKE_CONST(SD_BUS_ERROR_UNIX_PROCESS_ID_UNKNOWN,
                                           "No such process");

        case ENOENT:
            return SD_BUS_ERROR_MAKE_CONST(SD_BUS_ERROR_FILE_NOT_FOUND,
                                           "File not found");

        case EEXIST:
            return SD_BUS_ERROR_MAKE_CONST(SD_BUS_ERROR_FILE_EXISTS,
                                           "File exists");
        ...
        case EIO:
            return SD_BUS_ERROR_MAKE_CONST(SD_BUS_ERROR_IO_ERROR,
                                           "Input/output error");

        case ENETRESET:
        case ECONNABORTED:
        case ECONNRESET:
            return SD_BUS_ERROR_MAKE_CONST(SD_BUS_ERROR_DISCONNECTED,
                                           "Disconnected");

        case EOPNOTSUPP:
            return SD_BUS_ERROR_MAKE_CONST(SD_BUS_ERROR_NOT_SUPPORTED,
                                           "Not supported");

        case EADDRNOTAVAIL:
            return SD_BUS_ERROR_MAKE_CONST(SD_BUS_ERROR_BAD_ADDRESS,
                                           "Address not available");

        case ENOBUFS:
            return SD_BUS_ERROR_MAKE_CONST(SD_BUS_ERROR_LIMITS_EXCEEDED,
                                           "Limits exceeded");

        case EADDRINUSE:
            return SD_BUS_ERROR_MAKE_CONST(SD_BUS_ERROR_ADDRESS_IN_USE,
                                           "Address in use");

        case EBADMSG:
            return SD_BUS_ERROR_MAKE_CONST(SD_BUS_ERROR_INCONSISTENT_MESSAGE,
...
_public_ int sd_bus_error_set(sd_bus_error* e, const char* name,
                              const char* message)
{
...
_public_ int sd_bus_error_setf(sd_bus_error* e, const char* name,
                               const char* format, ...)
{
...
_public_ int sd_bus_error_copy(sd_bus_error* dest, const sd_bus_error* e)
{
    ...
    /*
     * _need_free  < 0 indicates that the error is temporarily const, needs deep
     * copying _need_free == 0 indicates that the error is perpetually const,
     * needs no deep copying _need_free  > 0 indicates that the error is fully
     * dynamic, needs deep copying
     */
...
_public_ int sd_bus_error_set_const(sd_bus_error* e, const char* name,
                                    const char* message)
{
    if (!name)
        return 0;
...
_public_ int sd_bus_error_set_errnofv(sd_bus_error* e, int error,
...
_public_ int sd_bus_error_set_errnof(sd_bus_error* e, int error,
                                     const char* format, ...)
{
    int r;
...
_public_ int sd_bus_error_add_map(const sd_bus_error_map* map)
{
    ...
    maps = reallocarray(additional_error_maps, n + 2,
