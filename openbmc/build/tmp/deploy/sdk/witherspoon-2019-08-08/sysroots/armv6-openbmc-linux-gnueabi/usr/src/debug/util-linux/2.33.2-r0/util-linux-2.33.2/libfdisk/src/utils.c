...
static int read_from_device(struct fdisk_context* cxt, unsigned char* buf,
                            uintmax_t start, size_t size)
{
    ...
    DBG(CXT, ul_debugobj(cxt, "reading: offset=%ju, size=%zu", start, size));
                                  ...
                                  unsigned int protect_size)
{
    ...
    if (protect_size)
    {
         ...
         */
        DBG(CXT,
            ul_debugobj(cxt, "first sector protection enabled -- re-reading"));
...
{
    struct fdisk_test tss[] = {{"--partnames", test_partnames, "<diskname>"},
                               {NULL}};
