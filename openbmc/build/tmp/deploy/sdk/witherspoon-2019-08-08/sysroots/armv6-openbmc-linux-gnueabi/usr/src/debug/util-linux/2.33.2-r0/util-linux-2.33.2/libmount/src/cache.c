 ...
 *
 * Deallocates the cache. This function does not care about reference count.
 * Don't use this function directly -- it's better to use mnt_unref_cache().
 */
 ...
 */
int mnt_cache_set_targets(struct libmnt_cache* cache, struct libmnt_table* mtab)
{
    if (!cache)
        return -EINVAL;
...
static int cache_add_entry(struct libmnt_cache* cache, char* key, char* value,
                           int flag)
{
    ...
    DBG(CACHE,
        ul_debugobj(cache, "add entry [%2zd] (%s): %s: %s", cache->nents,
                    (flag & MNT_CACHE_ISPATH) ? "path" : "tag", value, key));
    return 0;
}
 ...
 */
static const char* cache_find_tag(struct libmnt_cache* cache, const char* token,
                                  const char* value)
{
    size_t i;
    size_t tksz;
    ...
    for (i = 0; i < cache->nents; i++)
    {
        ...
        if (!(e->flag & MNT_CACHE_ISTAG))
            continue;
        if (strcmp(token, e->key) == 0 && strcmp(value, e->key + tksz + 1) == 0)
            return e->value;
    }
    return NULL;
}
...
int mnt_cache_read_tags(struct libmnt_cache* cache, const char* devname)
{
    ...
    const char* blktags[] = {"LABEL", "UUID", "TYPE", "PART_ENTRY_UUID",
                             "PART_ENTRY_NAME"};
    ...
    for (i = 0; i < ARRAY_SIZE(tags); i++)
    {
        ...
        if (!dev)
            goto error;
        if (cache_add_tag(cache, tags[i], data, dev, MNT_CACHE_TAGREAD))
        {
            free(dev);
            goto error;
        }
 ...
 */
char* mnt_cache_find_tag_value(struct libmnt_cache* cache, const char* devname,
                               const char* token)
{
    char* data = NULL;
                                         ...
                                         struct libmnt_cache* cache)
{
    ...
    if (p && cache)
    {
        ...
        if (cache_add_entry(cache, key, value, MNT_CACHE_ISPATH))
            goto error;
    }
...
char* mnt_resolve_target(const char* path, struct libmnt_cache* cache)
{
    ...
    else
    {
        ...
        while (mnt_table_next_fs(cache->mtab, &itr, &fs) == 0)
        {
            ...
            if (!mnt_fs_is_kernel(fs) || mnt_fs_is_swaparea(fs) ||
 ...
 *
 * Returns: newly allocated string with path, result always has to be
 *deallocated by free().
 */
                      ...
                      struct libmnt_cache* cache)
{
    ...
    if (!p)
    {
        ...
        if (p && cache && cache_add_tag(cache, token, value, p, 0))
            goto error;
    }
