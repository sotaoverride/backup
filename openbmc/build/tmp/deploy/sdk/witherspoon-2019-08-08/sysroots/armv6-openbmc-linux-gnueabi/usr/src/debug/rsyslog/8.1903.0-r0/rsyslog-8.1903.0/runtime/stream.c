 ...
 * Large modifications in 2009-06 to support using it with omfile, including zip
 * writer. Note that this file obtains the zlib wrapper object is needed, but it
 * never frees it again. While this sounds like a leak (and one may argue it
 * actually is), there is no harm associated with that. The reason is that strm
 * is a core object, so it is terminated only when rsyslogd exists. As we could
 * only release on termination (or else bear more overhead for keeping track of
 * how many users we have), not releasing zlibw is OK, because it will be
 * released when rsyslogd terminates. We may want to revisit this decision if it
 * turns out to be problematic. Then, we need to quasi-refcount the number of
...
/* static data */
DEFobjStaticHelpers DEFobjCurrIf(zlibw)

    /* forward definitions */
    static rsRetVal strmFlushInternal(strm_t* pThis, int bFlushZip);
static rsRetVal strmWrite(strm_t* __restrict__ const pThis,
                          const uchar* __restrict__ const pBuf,
                          const size_t lenBuf);
...
/* Try to resolve a size limit situation. This is used to support custom-file
 * size handlers for omfile. It first runs the command, and then checks if we
 * are still above the size treshold. Note that this works only with single file
 * names, NOT with circular names. Note that pszCurrFName can NOT be taken from
 * pThis, because the stream is closed when we are called (and that destroys
 * pszCurrFName, as there is NO CURRENT file name!). So we need to receive the
 * name as a parameter. initially wirtten 2005-06-21, moved to this class &
 * updates 2009-06-01, both rgerhards
 */
static rsRetVal resolveFileSizeLimit(strm_t* pThis, uchar* pszCurrFName)
{
    ...
    {
        ABORT_FINALIZE(
            RS_RET_NON_SIZELIMITCMD); /* nothing we can do in this case... */
    }
    ...
    {
        ABORT_FINALIZE(
            RS_RET_SIZELIMITCMD_DIDNT_RESOLVE); /* OK, it didn't work out... */
    }
 ...
 * but for this specific type of strm. The mapping to these handlers happens
 * during strm construction. Later on, handlers are called by pointers present
 * in the strm instance object.
 */
...
static rsRetVal doPhysOpen(strm_t* pThis)
{
    ...
    pThis->fd = open((char*)pThis->pszCurrFName, iFlags | O_LARGEFILE,
                     pThis->tOpenMode);
    ...
    if (pThis->fd == -1)
    {
        ...
        if (pThis->fileNotFoundError)
        {
            ...
            {
                LogError(errno_save, errcode, "file '%s': open error",
                         pThis->pszCurrFName);
    ...
    if (!ustrcmp(pThis->pszCurrFName, UCHAR_CONSTANT(_PATH_CONSOLE)) ||
        isatty(pThis->fd))
    {
    ...
    {
        CHKiRet(pThis->cryprov->OnFileOpen(
            pThis->cryprovData, pThis->pszCurrFName, &pThis->cryprovFileData,
            (pThis->tOperationsMode == STREAMMODE_READ) ? 'r' : 'w'));
        pThis->cryprov->SetDeleteOnClose(pThis->cryprovFileData,
...
static rsRetVal strmSetCurrFName(strm_t* pThis)
{
    ...
    else
    {
        ...
        {
            CHKiRet(genFileName(&pThis->pszCurrFName, pThis->pszDir,
                                pThis->lenDir, pThis->pszFName, pThis->lenFName,
                                -1, 0));
        }
    }
finalize_it:
...
finalize_it:
    ...
    else
    {
        if (pThis->pszCurrFName != NULL)
        {
            free(pThis->pszCurrFName);
            pThis->pszCurrFName =
                NULL; /* just to prevent mis-adressing down the road... */
        }
        if (pThis->fd != -1)
        {
            close(pThis->fd);
...
/* wait for the output writer thread to be done. This must be called before
 * actions that require data to be persisted. May be called in non-async mode
 * and is a null operation than. Must be called with the mutex locked.
 */
 ...
 * Note: it is valid to call this function when the physical file is closed. If
 * so, strmCloseFile() will still check if there is any unwritten data inside
 * buffers (this may be the case) and, if so, will open the file, write the
 * data, and then close it again (this is done via strmFlushInternal and
 * friends).
 */
static rsRetVal strmCloseFile(strm_t* pThis)
{
    ...
    DBGOPRINT((obj_t*)pThis, "file %d(%s) closing, bDeleteOnClose %d\n",
              pThis->fd, getFileDebugName(pThis), pThis->bDeleteOnClose);
...
static rsRetVal strmNextFile(strm_t* pThis)
{
    ...
    /* we do modulo operation to ensure we obey the iMaxFile property. This will
     * always result in a file number lower than iMaxFile, so it if wraps, the
     * name is back to 0, which results in the first file being overwritten. Not
     * desired for queues, so make sure their iMaxFiles is large enough. But it
     * is well-desired for other use cases, e.g. a circular output log file. --
 ...
 */
static rsRetVal ATTR_NONNULL() strmHandleEOFMonitor(strm_t* const pThis)
{
    DEFiRet;
    struct stat statName;
    ...
    if (stat((char*)pThis->pszCurrFName, &statName) == -1)
        ABORT_FINALIZE(RS_RET_IO_ERROR);
    DBGPRINTF("strmHandleEOFMonitor: stream checking for file change on '%s', "
              "inode %u/%u size %llu/%llu\n",
              pThis->pszCurrFName, (unsigned)pThis->inode,
              (unsigned)statName.st_ino, (long long unsigned)pThis->iCurrOffs,
              (long long unsigned)statName.st_size);
...
static rsRetVal ATTR_NONNULL()
    rereadTruncated(strm_t* const pThis, const int err_no,
                    const char* const reason, const long long data)
{
    DEFiRet;
...
static rsRetVal ATTR_NONNULL() checkTruncation(strm_t* const pThis)
{
    ...
    { /* costly operation, so make sure we need it */
    ...
    if (ret < 0)
    {
        iRet = rereadTruncated(pThis, errno,
                               "cannot seek backward to begin of last block",
                               backseek);
        FINALIZE;
    }

    const ssize_t lenRead =
        read(pThis->fd, pThis->pIOBuf_truncation, pThis->iBufPtrMax);
    if (lenRead != (ssize_t)pThis->iBufPtrMax)
    {
        iRet = rereadTruncated(pThis, errno, "last block could not be re-read",
                               lenRead);
        FINALIZE;
    }
...
static rsRetVal strmReadBuf(strm_t* pThis, int* padBytes)
{
    ...
    ISOBJ_TYPE_assert(pThis, strm);
    /* We need to try read at least twice because we may run into EOF and need
     * to switch files. */
    bRun = 1;
    while (bRun)
    {
        /* first check if we need to (re)open the file. We may have switched to
         * a new one in circular mode or it may have been rewritten (rotated) if
         * we monitor a file rgerhards, 2008-02-13
         */
        CHKiRet(strmOpenFile(pThis));
        if (pThis->cryprov == NULL)
        {
        ...
        {
            CHKiRet(pThis->cryprov->GetBytesLeftInBlock(pThis->cryprovFileData,
                                                        &bytesLeft));
        ...
        { /* good read */
            ...
            if (pThis->cryprov != NULL)
            {
                ...
                pThis->cryprov->Decrypt(pThis->cryprovFileData, pThis->pIOBuf,
                                        &actualDataLen);
...
void strmDebugOutBuf(const strm_t* const pThis)
{
    ...
    DBGOPRINT(
        (obj_t*)pThis,
        "strmRead ungetc %d, index %zd, max %zd, buf '%.*s', CURR: '%.*s'\n",
        pThis->iUngetC, pThis->iBufPtr, pThis->iBufPtrMax,
        (int)pThis->iBufPtrMax - strtIdx, pThis->pIOBuf + strtIdx,
        (int)(pThis->iBufPtrMax - pThis->iBufPtr),
...
{
    int padBytes =
        0; /* in crypto mode, we may have some padding (non-data) bytes */
    DEFiRet;

    assert(pThis != NULL);
    assert(pC != NULL);

    /* DEV debug only: DBGOPRINT((obj_t*) pThis, "strmRead index %zd, max
    %zd\n", pThis->iBufPtr, pThis->iBufPtrMax); */
    if (pThis->iUngetC != -1)
    { /* do we have an "unread" char that we need to provide? */
        *pC = pThis->iUngetC;
...
/* unget a single character just like ungetc(). As with that call, there is only
 * a single character buffering capability. rgerhards, 2008-01-07
 */
static rsRetVal strmUnreadChar(strm_t* pThis, uchar c)
{
    ...
    pThis->iUngetC = c;
    --pThis->iCurrOffs; /* one less octet read - NOTE: this can cause problems
    if we got a file change and immediately do an unread and the file is on a
    buffer boundary and the stream is then persisted. With the queue, this can
    not happen as an Unread is only done on record begin, which is never split
    accross files. For other cases we accept the very remote risk. -- rgerhards,
    2008-01-12 */
 ...
 * A paragraph may be terminated by a LF, by a LFLF, or by LF<not whitespace>
 * depending on the option set. The termination LF characters are read, but are
 ...
 */
static rsRetVal strmReadLine(strm_t* const pThis, cstr_t** ppCStr, uint8_t mode,
                             sbool bEscapeLF, uint32_t trimLineOverBytes,
                             int64* const strtOffs)
{
    ...
    if (mode == 0)
    {
        ...
        if (trimLineOverBytes > 0 &&
            (uint32_t)cstrLen(*ppCStr) > trimLineOverBytes)
        {
            ...
            else
            {
                if ((((*ppCStr)->iStrLen) > 0))
                {
                    ...
                    else
                    {
                        if (bEscapeLF)
                        {
                            CHKiRet(rsCStrAppendStrWithLen(
                                *ppCStr, (uchar*)"#012", sizeof("#012") - 1));
                        }
                        else
                        {
                            CHKiRet(cstrAppendChar(*ppCStr, c));
                ...
                else
                {
                    finished = 1; /* this is a blank line, a \n with nothing
                             since the last complete record */
                }
            }
        }
        cstrFinalize(*ppCStr);
    ...
    else if (mode == 2)
    {
        ...
        while (finished == 0)
        {
            if ((*ppCStr)->iStrLen == 0)
            {
                if (c != '\n')
                {
                    /* nothing in the buffer, and it's not a newline, add it to
                     * the buffer */
                    CHKiRet(cstrAppendChar(*ppCStr, c));
                    CHKiRet(strmReadChar(pThis, &c));
                }
            ...
            else
            {
                if (pThis->bPrevWasNL)
                {
                    ...
                    {
                        /* clean things up by putting the character we just read
                         * back into the input buffer and removing the LF
                         * character that is currently at the
                ...
                { /* not the first character after a newline, add it to the
                     buffer */
                    if (c == '\n')
                    {
                        pThis->bPrevWasNL = 1;
...
finalize_it:
    ...
    else
    {
        if (*ppCStr != NULL)
        {
            if (cstrLen(*ppCStr) > 0)
            {
                /* we may have an empty string in an unsuccesfull poll or after
                 * restart! */
                if (rsCStrConstructFromCStr(&pThis->prevLineSegment, *ppCStr) !=
                    RS_RET_OK)
                {
 ...
 */
rsRetVal strmReadMultiLine(strm_t* pThis, cstr_t** ppCStr, regex_t* start_preg,
                           regex_t* end_preg, const sbool bEscapeLF,
                           ...
                           int64* const strtOffs)
{
    ...
    do
    {
        ...
        while (c != '\n')
        {
            ...
            { /* end of file reached without \n? */
                CHKiRet(
                    rsCStrConstructFromCStr(&pThis->prevLineSegment, thisLine));
            }
            CHKiRet(readCharRet);
        }
        ...
        const int isStartMatch =
            start_preg
                ? !regexec(start_preg, (char*)rsCStrGetSzStrNoNULL(thisLine), 0,
                           NULL, 0)
                : 0;
        const int isEndMatch =
            end_preg ? !regexec(end_preg, (char*)rsCStrGetSzStrNoNULL(thisLine),
                                0, NULL, 0)
                     : 0;
        ...
        else
        {
            if (pThis->ignoringMsg == 0)
            {
                if (pThis->prevMsgSegment == NULL)
                {
                    ...
                    CHKiRet(rsCStrConstructFromCStr(&pThis->prevMsgSegment,
                                                    thisLine));
                }
                else
                {
                    if (bEscapeLF)
                    {
                        rsCStrAppendStrWithLen(pThis->prevMsgSegment,
                                               (uchar*)"\\n", 2);
                    }
                    else
                    {
                    ...
                    {
                        int len;
                        if ((len = cstrLen(pThis->prevMsgSegment) +
                                   currLineLen) < maxMsgSize)
                        {
                            CHKiRet(cstrAppendCStr(pThis->prevMsgSegment,
                                                   thisLine));
                            /* we could do this faster, but for now keep it
                             * simple */
                        }
                        else
                        {
                            ...
                            finished = 1;
                            *ppCStr = pThis->prevMsgSegment;
                            CHKiRet(rsCStrConstructFromszStr(
                                &pThis->prevMsgSegment, thisLine->pBuf + len));
...
finalize_it:
    *strtOffs = pThis->strtOffs;
    ...
    {
        if (pThis->readTimeout && (pThis->prevMsgSegment != NULL) &&
            (tCurr > pThis->lastRead + pThis->readTimeout))
        {
            if (rsCStrConstructFromCStr(ppCStr, pThis->prevMsgSegment) ==
                RS_RET_OK)
            {
                cstrFinalize(*ppCStr);
                ...
                pThis->lastRead = tCurr;
                pThis->strtOffs =
                    pThis->iCurrOffs; /* we are at begin of next line */
 ...
 */
BEGINobjConstruct(
    strm) /* be sure to specify the object type also in END macro! */
    pThis->iCurrFNum = 1;
pThis->fd = -1;
pThis->fdDir = -1;
pThis->iUngetC = -1;
    ...
    static rsRetVal strmConstructFinalize(strm_t* pThis)
{
    ...
    if (pThis->iZipLevel)
    { /* do we need a zip buf? */
        localRet = objUse(zlibw, LM_ZLIBW_FILENAME);
        if (localRet != RS_RET_OK)
        {
            pThis->iZipLevel = 0;
            DBGPRINTF("stream was requested with zip mode, but zlibw module "
                      "unavailable (%d) - using "
                      "without zip\n",
                      localRet);
        }
        else
        {
             ...
             * We add another 128 bytes to take care of the gzip header and "all
             * eventualities".
             */
    ...
    /* if we are set to sync, we must obtain a file handle to the directory for
     * fsync() purposes */
    if (pThis->bSync && !pThis->bIsTTY && pThis->pszDir != NULL)
    {
        pThis->fdDir =
            open((char*)pThis->pszDir, O_RDONLY | O_CLOEXEC | O_NOCTTY);
        if (pThis->fdDir == -1)
        {
            char errStr[1024];
            int err = errno;
            rs_strerror_r(err, errStr, sizeof(errStr));
            DBGPRINTF("error %d opening directory file for fsync() use - fsync "
                      "for directory "
                      "disabled: %s\n",
                      errno, errStr);
        }
    ...
    DBGPRINTF("file stream %s params: flush interval %d, async write %d\n",
              getFileDebugName(pThis), pThis->iFlushInterval,
              pThis->bAsyncWrite);
    ...
    if (pThis->bAsyncWrite)
    {
        ...
        {
            CHKmalloc(pThis->asyncBuf[i].pBuf =
                          (uchar*)malloc(pThis->sIOBufSize));
        }
        pThis->pIOBuf = pThis->asyncBuf[0].pBuf;
        pThis->bStopWriter = 0;
        if (pthread_create(&pThis->writerThreadID, &default_thread_attr,
 ...
 */
static void stopWriter(strm_t* pThis)
{
    pThis->bStopWriter = 1;
    pthread_cond_signal(&pThis->notEmpty);
...
BEGINobjDestruct(strm) /* be sure to specify the object type also in END and
                          CODESTART macros! */
    int i;
 ...
 * IMPORTANT: we MUST free this only AFTER the ansyncWriter has been stopped,
 * else we get random errors...
 */
    ...
    static rsRetVal strmCheckNextOutputFile(strm_t* pThis)
{
    ...
    if (pThis->iCurrOffs >= pThis->iMaxFileSize)
    {
        DBGOPRINT(
            (obj_t*)pThis,
            "max file size %ld reached for %d, now %ld - starting new file\n",
 ...
 */
static rsRetVal doWriteCall(strm_t* pThis, uchar* pBuf, size_t* pLenBuf)
{
    ssize_t lenBuf;
    ...
    do
    {
        ...
        if (pThis->bIsTTY && !pThis->iZipLevel && !pThis->cryprov)
        {
            ...
            if (pNl == pWriteBuf)
            {
                ...
                iWritten =
                    write(pThis->fd, pWriteBuf, pNl ? pNl - pWriteBuf : lenBuf);
        }
        else
#endif /* __FreeBSD__ */
        ...
        if (iWritten < 0)
        {
            ...
            if (err == EBADF)
            {
                LogError(err, RS_RET_IO_ERROR,
                         "file %s: fd %d no longer valid, recovery by "
                         "reopen; if you see this, consider reporting at "
                         "https://github.com/rsyslog/rsyslog/issues/3404 "
                         "so that we know when it happens. Include output of "
                         "uname -a. "
                         "OS error reason",
            ...
            else
            {
                if (err != EINTR)
                {
                    LogError(err, RS_RET_IO_ERROR, "file '%d' write error",
                             pThis->fd);
                }
                if (err == EINTR)
                {
                ...
                {
                    /* Failure for network file system, thus file needs to be
                     * closed and reopened. */
                    close(pThis->fd);
                    pThis->fd = -1;
                    CHKiRet(doPhysOpen(pThis));
                ...
                else
                {
                    ...
                    {
                        ABORT_FINALIZE(RS_RET_IO_ERROR);
                        /* Would it make sense to cover more error cases? So
                         * far, I do not see good reason to do so.
                         */
                    }
                }
            }
 ...
 */
static rsRetVal doWriteInternal(strm_t* pThis, uchar* pBuf, const size_t lenBuf,
                                const int bFlush)
{
    DEFiRet;
...
/* This function is called to "do" an async write call, what primarily means
 * that the data is handed over to the writer thread (which will then do the
 * actual write in parallel). Note that the stream mutex has already been locked
 * by the strmWrite...() calls. Also note that we always have only a single
 * producer, so we can simply serially assign the next free buffer to it and be
 * sure that the very some producer comes back in sequence to submit the
 * then-filled buffers. This also enables us to timout on partially written
 * buffers. -- rgerhards, 2009-07-06
 */
static rsRetVal doAsyncWriteInternal(strm_t* pThis, size_t lenBuf,
                                     const int bFlushZip)
{
    DEFiRet;
    ISOBJ_TYPE_assert(pThis, strm);
    ...
    DBGOPRINT((obj_t*)pThis,
              ...
              pThis->fd, getFileDebugName(pThis), pThis->iCnt, pThis->iEnq,
              bFlushZip);
    /* the -1 below is important, because we need one buffer for the main
     * thread! */
    ...
    pThis->bDoTimedWait =
        0; /* everything written, no need to timeout partial buffer writes */
    if (++pThis->iCnt == 1)
    {
        pthread_cond_signal(&pThis->notEmpty);
 ...
 */
static rsRetVal strmSchedWrite(strm_t* pThis, uchar* pBuf, size_t lenBuf,
                               const int bFlushZip)
{
    DEFiRet;
    ...
    { /* loop broken inside */
        while (pThis->iCnt == 0)
        {
            DBGOPRINT((obj_t*)pThis,
                      ...
                      pThis->fd, getFileDebugName(pThis), pThis->iCnt,
                      bTimedOut, pThis->iFlushInterval);
            ...
            {
                timeoutComp(&t, pThis->iFlushInterval *
                                    1000); /* 1000 *millisconds* */
                if ((err = pthread_cond_timedwait(&pThis->notEmpty, &pThis->mut,
                                                  &t)) != 0)
                {
                    DBGOPRINT((obj_t*)pThis,
                              "file %d(%s) asyncWriterThread timed out\n",
        ...
        d_pthread_mutex_unlock(&pThis->mut);
        doWriteInternal(pThis, pThis->asyncBuf[iDeq].pBuf,
                        pThis->asyncBuf[iDeq].lenBuf, bFlush);
                           ...
                           const int bFlush)
{
    ...
    if (!pThis->bzInitDone)
    {
        ...
        zRet = zlibw.DeflateInit2(&pThis->zstrm, pThis->iZipLevel, Z_DEFLATED,
                                  31, 9, Z_DEFAULT_STRATEGY);
        if (zRet != Z_OK)
        {
            LogError(0, RS_RET_ZLIB_ERR,
                     "error %d returned from zlib/deflateInit2()", zRet);
    ...
    do
    {
        ...
        zRet = zlibw.Deflate(&pThis->zstrm,
                             bFlush ? Z_SYNC_FLUSH
                                    : Z_NO_FLUSH); /* no bad return value */
        DBGPRINTF("after deflate, ret %d, avail_out %d, to write %d\n", zRet,
                  pThis->zstrm.avail_out, outavail);
        if (zRet != Z_OK)
        {
            LogError(0, RS_RET_ZLIB_ERR,
                     "error %d returned from zlib/Deflate()", zRet);
            ABORT_FINALIZE(RS_RET_ZLIB_ERR);
        }
...
finalize_it:
    ...
    if (zRet != Z_OK)
    {
        LogError(0, RS_RET_ZLIB_ERR, "error %d returned from zlib/deflateEnd()",
                 zRet);
    }
...
/* flush stream output buffer to persistent storage. This can be called at any
 * time and is automatically called when the output buffer is full. rgerhards,
 * 2008-01-10
 */
static rsRetVal strmFlushInternal(strm_t* pThis, int bFlushZip)
{
    ...
    assert(pThis != NULL);
    DBGOPRINT((obj_t*)pThis,
              "strmFlushinternal: file %d(%s) flush, buflen %ld%s\n", pThis->fd,
              getFileDebugName(pThis), (long)pThis->iBufPtr,
              (pThis->iBufPtr == 0) ? " (no need to flush)" : "");
...
/* flush stream output buffer to persistent storage. This can be called at any
 * time and is automatically called when the output buffer is full. This
 * function is for use by EXTERNAL callers. Do NOT use it internally. It locks
 * the async writer mutex if ther is need to do so. rgerhards, 2010-03-18
 */
 ...
 */
rsRetVal strmMultiFileSeek(strm_t* pThis, unsigned int FNum, off64_t offs,
                           off64_t* bytesDel)
{
    struct stat statBuf;
    DEFiRet;
    ...
    if (pThis->iCurrFNum != FNum)
    {
        ...
        *bytesDel = statBuf.st_size;
        DBGPRINTF("strmMultiFileSeek: detected new filenum, was %u, new %u, "
                  ...
                  pThis->iCurrFNum, FNum, pThis->pszCurrFName,
                  (long long)*bytesDel);
...
/* write memory buffer to a stream object.
 * process the data in chunks and copy it over to our buffer. The
 * caller-provided data may theoritically be larger than our buffer. In that
 * case, we do multiple copies. One may argue if it were more efficient to write
 * out the caller-provided buffer in that case and earlier versions of rsyslog
 * did this. However, this introduces a lot of complexity inside the buffered
 * writer and potential performance bottlenecks when trying to solve it. Now
 * keep in mind that we actually do (almost?) never have a case where the
 * caller-provided buffer is larger than our one. So instead of optimizing a
 * case which normally does not exist, we expect some degradation in its case
 * but make us perform better in the regular cases. -- rgerhards, 2009-07-07
 ...
 */
static rsRetVal strmWrite(strm_t* __restrict__ const pThis,
                          const uchar* __restrict__ const pBuf, size_t lenBuf)
{
    DEFiRet;
    size_t iWrite;
    size_t iOffset;
  ...
  */
    ...
    do
    {
        if (pThis->iBufPtr == pThis->sIOBufSize)
        {
            CHKiRet(strmFlushInternal(
                pThis, 0)); /* get a new buffer for rest of data */
        }
        iWrite =
            pThis->sIOBufSize - pThis->iBufPtr; /* this fits in current buf */
        if (iWrite > lenBuf)
            iWrite = lenBuf;
     ...
     * write it. This seems more natural than waiting (hours?) for the next
     * message...
     */
    if (pThis->iBufPtr == pThis->sIOBufSize)
    {
        CHKiRet(strmFlushInternal(pThis,
                                  0)); /* get a new buffer for rest of data */
    }
...
/* simple ones first */
DEFpropSetMeth(strm, iMaxFileSize, int64) DEFpropSetMeth(strm, iFileNumDigits,
                                                         int)
    DEFpropSetMeth(strm, tOperationsMode, int) DEFpropSetMeth(strm, tOpenMode,
                                                              mode_t)
        DEFpropSetMeth(strm, sType, strmType_t) DEFpropSetMeth(strm, iZipLevel,
                                                               int)
            DEFpropSetMeth(strm, bVeryReliableZip,
                           int) DEFpropSetMeth(strm, bSync, int)
...
static rsRetVal strmSetFName(strm_t* pThis, uchar* pszName, size_t iLenName)
{
    ...
    memcpy(pThis->pszFName, pszName,
           iLenName + 1); /* always think about the \0! */
...
static rsRetVal strmRecordEnd(strm_t* pThis)
{
    ...
    pThis->bInRecord = 0;
    iRet =
        strmCheckNextOutputFile(pThis); /* check if we need to switch files */
...
static rsRetVal strmSerialize(strm_t* pThis, strm_t* pStrm)
{
    ...
    objSerializeSCALAR(pStrm, iCurrFNum,
                       INT); /* implicit cast is OK for persistance */
 ...
 */
#define isProp(name)                                                           \
    !rsCStrSzStrCmp(pProp->pcsName, UCHAR_CONSTANT(name), sizeof(name) - 1)
static rsRetVal strmSetProperty(strm_t* pThis, var_t* pProp)
{
    ...
    {
        CHKiRet(strmSetFName(pThis, rsCStrGetSzStrNoNULL(pProp->val.pStr),
                             rsCStrLen(pProp->val.pStr)));
    }
    ...
    {
        CHKiRet(
            rsCStrConstructFromCStr(&pThis->prevLineSegment, pProp->val.pStr));
    }
    else if (isProp("prevMsgSegment"))
    {
        CHKiRet(
            rsCStrConstructFromCStr(&pThis->prevMsgSegment, pProp->val.pStr));
    }
...
/* return the current offset inside the stream. Note that on two consequtive
 * calls, the offset reported on the second call may actually be lower than on
 * the first call. This is due to file circulation. A caller must deal with
 * that. -- rgerhards, 2008-01-30
 */
...
finalize_it : ENDobjQueryInterface(strm)
              ...
              /* Initialize the stream class. Must be called as the very first
               * method before anything else is called inside this class.
               * rgerhards, 2008-01-09
               */
