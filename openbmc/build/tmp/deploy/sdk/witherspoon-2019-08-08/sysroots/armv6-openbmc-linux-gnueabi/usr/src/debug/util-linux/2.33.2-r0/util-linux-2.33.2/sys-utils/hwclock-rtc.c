...
static int do_rtc_read_ioctl(int rtc_fd, struct tm* tm)
{
    ...
    if (rc == -1)
    {
        warn(_("ioctl(%s) to %s to read the time failed"), ioctlname,
             rtc_dev_name);
        return -1;
    }
...
static int synchronize_to_clock_tick_rtc(const struct hwclock_control* ctl)
{
    ...
    else
    {
        ...
        else if (errno == ENOTTY || errno == EINVAL)
        {
            ...
            warn(_("ioctl(%d, RTC_UIE_ON, 0) to %s failed"), rtc_fd,
                 rtc_dev_name);
    }
    return ret;
}
                                  ...
                                  const struct tm* new_broken_time)
{
    ...
    if (rc == -1)
    {
        warn(_("ioctl(%s) to %s to set the time failed"), ioctlname,
             rtc_dev_name);
        hwclock_exit(ctl, EXIT_FAILURE);
    }
...
int get_epoch_rtc(const struct hwclock_control* ctl, unsigned long* epoch_p)
{
    ...
    {
        warn(_("ioctl(%d, RTC_EPOCH_READ, epoch_p) to %s failed"), rtc_fd,
             rtc_dev_name);
        return 1;
    }
...
int set_epoch_rtc(const struct hwclock_control* ctl)
{
    ...
    {
        warn(_("ioctl(%d, RTC_EPOCH_SET, %lu) to %s failed"), rtc_fd, epoch,
             rtc_dev_name);
        return 1;
    }
