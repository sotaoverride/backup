  ...
  public:
    ...
    duration_type get_offset(const date_type& d) const
    {
        ...
        if (origDayOfMonth_ == 0)
        {
            ...
            day_type endOfMonthDay(
                cal_type::end_of_month_day(ymd.year, ymd.month));
        ...
        const typename ymd_type::year_type year(
            static_cast<typename ymd_type::year_type::value_type>(ymd.year +
                                                                  wi.add(f_)));
        //       std::cout << "trace wi: " << wi.as_int() << std::endl;
        //       std::cout << "trace year: " << year << std::endl;
        // find the last day for the new month
        day_type resultingEndOfMonthDay(
            cal_type::end_of_month_day(year, wi.as_int()));
    ...
    duration_type get_neg_offset(const date_type& d) const
    {
        ...
        // calc the year wrap around, add() returns 0 or 1 if wrapped
        const typename ymd_type::year_type year(
            static_cast<typename ymd_type::year_type::value_type>(
                ymd.year + wi.subtract(f_)));
        // find the last day for the new month
        day_type resultingEndOfMonthDay(
            cal_type::end_of_month_day(year, wi.as_int()));
        // original was the end of month -- force to last day of month
...
class week_functor
{
  public:
    ...
    {
        return duration_type(f_ *
                             static_cast<int>(calendar_type::days_in_week()));
    }
    duration_type get_neg_offset(const date_type&) const
    {
        return duration_type(-f_ *
