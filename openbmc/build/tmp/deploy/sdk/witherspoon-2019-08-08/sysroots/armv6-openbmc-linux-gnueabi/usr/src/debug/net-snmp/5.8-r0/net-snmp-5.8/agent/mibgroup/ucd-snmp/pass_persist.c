...
void init_pass_persist(void)
{
    ...
    pipe_check_alarm_id =
        snmp_alarm_register(10, SA_REPEAT, check_persist_pipes, NULL);
}
...
void pass_persist_group(struct extensible* persistpassthrus)
{
    ...
    *ppass = calloc(1, sizeof(**ppass));
    ...
    else
    {
        for (tcptr = cptr; *tcptr != 0 && *tcptr != '#' && *tcptr != ';';
             tcptr++)
            ;
        if (asprintf(&(*ppass)->command, "%.*s", (int)(tcptr - cptr), cptr) < 0)
        {
        }
    }
    ...
    register_mib_priority(
        "pass_persist", (struct variable*)extensible_persist_passthru_variables,
        sizeof(struct variable2), 1, (*ppass)->miboid, (*ppass)->miblen,
        (*ppass)->mibpriority);
    ...
    {
        etmp = (struct extensible**)malloc(
            ((sizeof(struct extensible*)) * numpersistpassthrus));
        if (etmp == NULL)
            return;
...
void pass_persist_free_config(void)
{
    ...
    for (etmp = persistpassthrus; etmp != NULL;)
    {
        etmp2 = etmp;
        etmp = etmp->next;
        unregister_mib_priority(etmp2->miboid, etmp2->miblen,
                                etmp2->mibpriority);
        free(etmp2);
    }
    if (persist_pipes)
...
#ifdef USING_SINGLE_COMMON_PASSPERSIST_INSTANCE
int get_exten_group_id(struct extensible* persistpassthru, int current_id)
{
    ...
    for (idx = 1, ptmp = persistpassthrus; ptmp != NULL;
         ptmp = ptmp->next, idx++)
    {
...
u_char* var_extensible_pass_persist(struct variable* vp, oid* name,
                                    ...
                                    WriteMethod** write_method)
{
    ...
    for (i = 1; i <= numpersistpassthrus; i++)
    {
        ...
        rtest = snmp_oidtree_compare(name, *length, persistpassthru->miboid,
        ...
        if ((exact && rtest == 0) || (!exact && rtest <= 0))
        {
            ...
            pipe_idx = i;
#ifdef USING_SINGLE_COMMON_PASSPERSIST_INSTANCE
            pipe_idx = get_exten_group_id(persistpassthru->passpersist_inst, i);
...
int setPassPersist(int action, u_char* var_val, u_char var_val_type,
                   ...
                   size_t name_len)
{
    ...
    {
        persistpassthru = get_exten_instance(persistpassthrus, i);
        rtest = snmp_oidtree_compare(name, name_len, persistpassthru->miboid,
                                     persistpassthru->miblen);
        pipe_idx = i;
#ifdef USING_SINGLE_COMMON_PASSPERSIST_INSTANCE
        pipe_idx = get_exten_group_id(persistpassthru->passpersist_inst, i);
    ...
    if (snmp_get_do_debugging())
    {
        ...
        DEBUGMSGTL(
            ("ucd-snmp/pass_persist", "persistpass-notfound:  %s\n", buf2));
    }
...
static int init_persist_pipes(void)
{
     ...
     */
    persist_pipes = (struct persist_pipe_type*)malloc(
        sizeof(struct persist_pipe_type) * (numpersistpassthrus + 1));
    if (persist_pipes)
    {
        for (i = 0; i <= numpersistpassthrus; i++)
        {
...
static void check_persist_pipes(unsigned clientreg, void* clientarg)
{
    ...
    for (i = 0; i <= numpersistpassthrus; i++)
    {
        ...
        {
            snmp_log(LOG_INFO,
                     "pass_persist[%d]: child process stopped - closing pipe\n",
                     i);
            close_persist_pipe(i);
        }
    }
...
static int open_persist_pipe(int iindex, char* command)
{
    ...
    DEBUGMSGTL(("ucd-snmp/pass_persist",
                "open_persist_pipe(%d,'%s') recurse=%d\n", iindex, command,
                recurse));
    /*
    ...
    if (persist_pipes[iindex].pid == NETSNMP_NO_SUCH_PROCESS)
    {
        ...
        {
            DEBUGMSGTL(
                ("ucd-snmp/pass_persist", "open_persist_pipe: pid == -1\n"));
            recurse = 0;
            return 0;
        }
