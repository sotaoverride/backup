...
retry:;
    ...
    if (mapped != NO_MAPPING)
    {
        struct datahead* found =
            __nscd_cache_search(GETAI, key, keylen, mapped, sizeof ai_resp);
        if (found != NULL)
        {
    ...
    {
        sock =
            __nscd_open_socket(key, keylen, GETAI, &ai_resp, sizeof(ai_resp));
        if (sock == -1)
        {
    ...
    if (ai_resp.found == 1)
    {
       ...
       comes from the mapped cache.  */
        ...
        /* Create result.  */
        resultbuf =
            (struct nscd_ai_result*)malloc(sizeof(*resultbuf) + datalen);
        if (resultbuf == NULL)
        {
