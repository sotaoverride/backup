...
int device_add_property_aux(sd_device* device, const char* _key,
                            const char* _value, bool db)
{
...
int device_add_property_internal(sd_device* device, const char* key,
                                 const char* value)
{
...
int device_set_syspath(sd_device* device, const char* _syspath, bool verify)
{
    ...
    if (!path_startswith(_syspath, "/sys/"))
        return log_debug_errno(
            ...
            "sd-device: Syspath '%s' is not a subdirectory of /sys", _syspath);
    ...
    if (verify)
    {
        ...
        if (r < 0)
            return log_debug_errno(
                r, "sd-device: Failed to get target of '%s': %m", _syspath);
        ...
        if (!path_startswith(syspath, "/sys"))
        {
            ...
            /* /sys is a symlink to somewhere sysfs is mounted on? In that case,
             * we convert the path to real sysfs to "/sys". */
            r = chase_symlinks("/sys", NULL, 0, &real_sys);
            if (r < 0)
                return log_debug_errno(
                    r, "sd-device: Failed to chase symlink /sys: %m");

            p = path_startswith(syspath, real_sys);
            if (!p)
                return log_debug_errno(
                    SYNTHETIC_ERRNO(ENODEV),
                    "sd-device: Canonicalized path '%s' does not starts with "
                    "sysfs mount point '%s'",
                    syspath, real_sys);
        ...
        if (path_startswith(syspath, "/sys/devices/"))
        {
            ...
            if (r < 0)
            {
                ...
                return log_debug_errno(
                    errno, "sd-device: %s does not have an uevent file: %m",
                    syspath);
            }
        }
        else
...
_public_ int sd_device_new_from_subsystem_sysname(sd_device** ret,
...
static int handle_uevent_line(sd_device* device, const char* key,
                              const char* value, const char** major,
                              const char** minor)
{
    int r;
...
int device_read_uevent_file(sd_device* device)
{
    ...
    const char *syspath, *key = NULL, *value = NULL, *major = NULL,
                         *minor = NULL;
    char* path;
    size_t uevent_len;
    unsigned i;
        ...
        switch (state)
        {
            ...
            case KEY:
                ...
                else if (strchr(NEWLINE, uevent[i]))
                {
                    uevent[i] = '\0';
                    log_device_debug(
                        device, "sd-device: Invalid uevent line '%s', ignoring",
                        key);
                ...
                if (strchr(NEWLINE, uevent[i]))
                {
                    ...
                    if (r < 0)
                        log_device_debug_errno(
                            device, r,
                            "sd-device: Failed to handle uevent entry '%s=%s', "
                            "ignoring: %m",
                            key, value);
    ...
    if (major)
    {
        ...
        if (r < 0)
            log_device_debug_errno(device, r,
                                   "sd-device: Failed to set 'MAJOR=%s' or "
                                   "'MINOR=%s' from '%s', ignoring: %m",
                                   major, minor, path);
    }
...
_public_ int sd_device_new_from_device_id(sd_device** ret, const char* id)
{
    ...
    switch (id[0])
    {
        ...
        case 'n':
        {
            ...
            r = sd_device_new_from_subsystem_sysname(&device, "net",
                                                     ifr.ifr_name);
            if (r < 0)
                return r;
    ...
    if (!device->subsystem_set)
    {
        ...
        /* use implicit names */
            ...
            r = device_set_subsystem(device, "module");
                 ...
                 PATH_STARTSWITH_SET(device->devpath, "/subsystem/", "/class/",
                                     "/bus/"))
            r = device_set_subsystem(device, "subsystem");
        if (r < 0 && r != -ENOENT)
            return log_device_debug_errno(
                device, r, "sd-device: Failed to set subsystem for %s: %m",
                device->devpath);
...
_public_ int sd_device_get_driver(sd_device* device, const char** ret)
{
    ...
    if (!device->driver_set)
    {
        ...
        if (r >= 0)
        {
            ...
            if (r < 0)
                return log_device_debug_errno(
                    device, r, "sd-device: Failed to set driver for %s: %m",
                    device->devpath);
        }
        else if (r == -ENOENT)
            device->driver_set = true;
...
static int device_add_property_internal_from_string(sd_device* device,
                                                    const char* str)
{
    _cleanup_free_ char* key = NULL;
    char* value;
    ...
    *value = '\0';
    ...
    /* Add the property to both sd_device::properties and
     * sd_device::properties_db, as this is called by only handle_db_line(). */
...
static int handle_db_line(sd_device* device, char key, const char* value)
{
    ...
    switch (key)
    {
        ...
        default:
            log_device_debug(
                device, "sd-device: Unknown key '%c' in device db, ignoring",
                key);
    }
...
int device_get_id_filename(sd_device* device, const char** ret)
{
    ...
    if (!device->id_filename)
    {
        ...
        if (sd_device_get_devnum(device, &devnum) >= 0)
        {
            ...
            r = asprintf(&id, "%c%u:%u", streq(subsystem, "block") ? 'b' : 'c',
        ...
        else
        {
            ...
            if (streq(subsystem, "drivers"))
            {
                ...
                r = asprintf(&id, "+drivers:%s:%s", device->driver_subsystem,
                             sysname);
                if (r < 0)
                    return -ENOMEM;
            }
...
int device_read_db_internal_filename(sd_device* device, const char* filename)
{
    ...
    if (r < 0)
    {
        ...
        return log_device_debug_errno(
            device, r, "sd-device: Failed to read db '%s': %m", filename);
    }
        ...
        switch (state)
        {
            ...
            case KEY:
                if (db[i] != ':')
                {
                    log_device_debug(
                        device,
                        "sd-device: Invalid db entry with key '%c', ignoring",
                        key);
            ...
            case VALUE:
                if (strchr(NEWLINE, db[i]))
                {
                    ...
                    if (r < 0)
                        log_device_debug_errno(device, r,
                                               "sd-device: Failed to handle db "
                                               "entry '%c:%s', ignoring: %m",
                                               key, value);
...
_public_ int sd_device_get_usec_since_initialized(sd_device* device,
                                                  uint64_t* usec)
{
    usec_t now_ts;
    int r;
...
int device_properties_prepare(sd_device* device)
{
    ...
    if (device->property_devlinks_outdated)
    {
             ...
             devlink = sd_device_get_devlink_next(device))
        {
            ...
            if (!GREEDY_REALLOC(devlinks, devlinks_allocated,
                                devlinks_len + strlen(devlink) + 2))
                return -ENOMEM;
            if (devlinks_len > 0)
    ...
    if (device->property_tags_outdated)
    {
        ...
        for (tag = sd_device_get_tag_first(device); tag;
             tag = sd_device_get_tag_next(device))
        {
            char* e;

            if (!GREEDY_REALLOC(tags, tags_allocated,
                                tags_len + strlen(tag) + 2))
                return -ENOMEM;
...
_public_ const char* sd_device_get_property_first(sd_device* device,
                                                  const char** _value)
{
    const char* key;
    const char* value;
    ...
    ordered_hashmap_iterate(device->properties, &device->properties_iterator,
...
_public_ const char* sd_device_get_property_next(sd_device* device,
                                                 const char** _value)
{
    const char* key;
    const char* value;
...
_public_ int sd_device_get_property_value(sd_device* device, const char* key,
                                          const char** _value)
{
    char* value;
    int r;
...
static int device_add_sysattr_value(sd_device* device, const char* _key,
                                    char* value)
{
    _cleanup_free_ char* key = NULL;
...
static int device_get_sysattr_value(sd_device* device, const char* _key,
                                    const char** _value)
{
...
_public_ int sd_device_get_sysattr_value(sd_device* device, const char* sysattr,
                                         const char** _value)
{
    _cleanup_free_ char* value = NULL;
