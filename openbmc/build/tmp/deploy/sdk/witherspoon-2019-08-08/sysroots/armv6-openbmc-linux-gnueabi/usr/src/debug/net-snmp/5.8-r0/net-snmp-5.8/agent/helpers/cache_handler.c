...
static void _cache_free(netsnmp_cache* cache);
...
netsnmp_cache* netsnmp_cache_find_by_oid(const oid* rootoid, int rootoid_len)
{
    ...
    {
        if (0 == netsnmp_oid_equals(cache->rootoid, cache->rootoid_len, rootoid,
                                    rootoid_len))
            return cache;
    }
...
/** returns a cache
 */
netsnmp_cache* netsnmp_cache_create(int timeout, NetsnmpCacheLoad* load_hook,
...
static netsnmp_cache* netsnmp_cache_ref(netsnmp_cache* cache)
{
    cache->refcnt++;
    return cache;
}

static void netsnmp_cache_deref(netsnmp_cache* cache)
{
    if (--cache->refcnt == 0)
    {
...
static void _timer_reload(unsigned int regNo, void* clientargs)
{
...
unsigned int netsnmp_cache_timer_start(netsnmp_cache* cache)
{
    ...
    cache->timer_id =
        snmp_alarm_register(cache->timeout, SA_REPEAT, _timer_reload, cache);
    if (0 == cache->timer_id)
    {
    ...
    DEBUGMSGT(("cache_timer:start", "starting timer %lu for cache %p\n",
...
void netsnmp_cache_timer_stop(netsnmp_cache* cache)
{
    ...
    DEBUGMSGT(("cache_timer:stop", "stopping timer %lu for cache %p\n",
               cache->timer_id, cache));
...
netsnmp_mib_handler* netsnmp_cache_handler_get(netsnmp_cache* cache)
{
    ...
    ret = netsnmp_create_handler("cache_handler", netsnmp_cache_helper_handler);
    if (ret)
    {
                                               ...
                                               int rootoid_len)
{
    ...
    if (ret)
    {
        cache = netsnmp_cache_create(timeout, load_hook, free_hook, rootoid,
                                     rootoid_len);
        ret->myvoid = (void*)cache;
...
#if !defined(NETSNMP_FEATURE_REMOVE_NETSNMP_CACHE_HANDLER_REGISTER) ||         \
    !defined(NETSNMP_FEATURE_REMOVE_NETSNMP_REGISTER_CACHE_HANDLER)
static int _cache_handler_register(netsnmp_handler_registration* reginfo,
...
static char* _build_cache_name(const char* name)
{
                                  ...
                                  const char* name)
{
    ...
    if (NULL == netsnmp_agent_get_list_data(reqinfo, cache_name))
    {
        ...
        netsnmp_agent_add_list_data(
            reqinfo, netsnmp_create_data_list(cache_name, cache, NULL));
    }
...
int netsnmp_cache_check_expired(netsnmp_cache* cache)
{
    ...
    if (!cache->valid || (NULL == cache->timestampM) || (-1 == cache->timeout))
        ...
        cache->expired =
            netsnmp_ready_monotonic(cache->timestampM, 1000 * cache->timeout);
                                 ...
                                 netsnmp_request_info* requests)
{
    ...
    DEBUGMSGOID(
        ("helper:cache_handler", reginfo->rootoid, reginfo->rootoid_len));
    ...
    {
        DEBUGMSGT(("helper:cache_handler",
                   " caching disabled or "
...
static int _cache_load(netsnmp_cache* cache)
{
     ...
     */
    if (cache->valid && (!(cache->flags & NETSNMP_CACHE_DONT_FREE_BEFORE_LOAD)))
        _cache_free(cache);
    ...
    {
        snmp_alarm_register(CACHE_RELEASE_FREQUENCY, 0,
                            release_cached_resources, NULL);
...
void release_cached_resources(unsigned int regNo, void* clientargs)
{
    ...
    for (cache = cache_head; cache; cache = cache->next)
    {
        DEBUGMSGTL(("helper:cache_handler", " checking %p (flags 0x%x)\n",
                    cache, cache->flags));
        if (cache->valid && !(cache->flags & NETSNMP_CACHE_DONT_AUTO_RELEASE))
        {
