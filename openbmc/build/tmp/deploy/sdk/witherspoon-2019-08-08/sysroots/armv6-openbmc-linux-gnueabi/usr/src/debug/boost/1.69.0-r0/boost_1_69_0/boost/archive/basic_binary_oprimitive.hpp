...
class BOOST_SYMBOL_VISIBLE basic_binary_oprimitive
{
  ...
  public:
    ...
    basic_binary_oprimitive(std::basic_streambuf<Elem, Tr>& sb,
                            bool no_codecvt);
    BOOST_ARCHIVE_OR_WARCHIVE_DECL
  ...
  public:
    ...
    struct use_array_optimization
    {
        ...
        {
            typedef
                typename boost::serialization::is_bitwise_serializable<T>::type
                    type;
        };
#else
...
template <class Archive, class Elem, class Tr>
inline void
    basic_binary_oprimitive<Archive, Elem, Tr>::save_binary(const void* address,
                                                            std::size_t count)
{
    // BOOST_ASSERT(count <=
    // std::size_t(boost::integer_traits<std::streamsize>::const_max)); note: if
    // the following assertions fail a likely cause is that the output stream is
    // set to "text" mode where by cr characters recieve special treatment. be
    // sure that the output stream is opened with ios::binary
    // if(os.fail())
    ...
    std::streamsize scount = m_sb.sputn(static_cast<const Elem*>(address),
