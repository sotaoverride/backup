...
///
/// \brief This class holds an external data - beyond existing fmtflags that
/// std::ios_base holds
///
/// You should almost never create this object directly. Instead, you should
/// access it via ios_info::get(stream_object) static member function. It
/// automatically creates default formatting data for that stream
///
class BOOST_LOCALE_DECL ios_info
{
  public:
    ...
    ///
    /// Set a flags that define a way for format data like number, spell,
    /// currency etc.
    ///
    ...
    ///
    /// Get a flags that define a way for format data like number, spell,
    /// currency etc.
    ///
...
///
/// \brief This namespace includes all manipulators that can be used on IO
/// streams
///
namespace as
{
...
///
/// Format values with "POSIX" or "C"  locale. Note, if locale was created with
/// additional non-classic locale then These numbers may be localized
///
...
///
/// Format a number. Note, unlike standard number formatting, integers would be
/// treated like real numbers when std::fixed or std::scientific manipulators
/// were applied
///
...
///
/// Create formatted date time, Please note, this manipulator only changes
/// formatting mode, and not format itself, so you are probably looking for
/// ftime manipulator
///
...
///
/// Please note, formatting flags are very similar but not exactly the same as
/// flags for C function strftime. Differences: some flags as "%e" do not add
/// blanks to fill text up to two spaces, not all flags supported.
///
/// Flags:
