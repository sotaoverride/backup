...
static int parse_qtd_backslash(char** word, size_t* word_length,
...
static int do_parse_glob(const char* glob_word, char** word,
                         size_t* word_length, size_t* max_length,
                         wordexp_t* pwordexp, const char* ifs,
                         const char* ifs_white)
{
    int error;
    ...
    for (; words[*offset]; ++(*offset))
    {
        switch (words[*offset])
        {
            ...
            case ')':
                ...
                expr =
                    w_addchar(expr, &expr_length, &expr_maxlen, words[*offset]);
                if (expr == NULL)
                    return WRDE_NOSPACE;
...
exec_comm_child(char* comm, int* fildes, int showerr, int noexec)
{
    ...
    if (showerr == 0)
    {
       ...
       device.  */
        if (__builtin_expect(__fxstat64(_STAT_VER, STDERR_FILENO, &st), 0) !=
                0 ||
...
static int exec_comm(char* comm, char** word, size_t* word_length,
...
again:
    ...
    if (pid == 0)
        exec_comm_child(comm, fildes, noexec ? 0 : flags & WRDE_SHOWERR,
                        noexec);
    ...
    /* Quoted - no field splitting */
    {
        while (1)
        {
            if ((buflen = TEMP_FAILURE_RETRY(
                     __read(fildes[0], buffer, bufsize))) < 1)
            {
               ...
               looping until the process eventually exits.  */
                if (TEMP_FAILURE_RETRY(__waitpid(
                        pid, &status, buflen == 0 ? 0 : WNOHANG)) == 0)
                    continue;
                if ((buflen = TEMP_FAILURE_RETRY(
                         __read(fildes[0], buffer, bufsize))) < 1)
                    break;
        ...
        while (1)
        {
            ...
            for (i = 0; i < buflen; ++i)
            {
                if (strchr(ifs, buffer[i]) != NULL)
                {
                     ...
                     * non-whitespace. Delimit the field.  Nulls are converted
                     * by w_addword. */
...
#ifdef __libc_ptf_call
                        __libc_ptf_call(__pthread_setcancelstate, (state, NULL),
                                        0);
#endif
        ...
        for (; words[*offset]; ++(*offset))
        {
            switch (words[*offset])
            {
                ...
                case '\\':
                    ...
                    pattern =
                        w_addchar(pattern, &pat_length, &pat_maxlen, '\\');
                    if (pattern == NULL)
                        goto no_space;
        ...
        switch (action)
        {
            ...
            case ACT_NULL_ASSIGN:
                if (!value || (!*value && colon_seen))
                    /* If param is unset, or set but null and a colon has been
                       seen, the expansion of the pattern will be needed. */
                    expand_pattern = 1;

                break;

            case ACT_NONNULL_SUBST:
                /* Expansion of word will be needed if parameter is set and not
                   null, or set null but no colon has been seen. */
                if (value && (*value || !colon_seen))
                    expand_pattern = 1;
            ...
            for (p = pattern; p && *p; p++)
            {
                ...
                switch (*p)
                {
                    ...
                    case '$':
                        offset = 0;
                        error =
                            parse_dollars(&expanded, &exp_len, &exp_maxl, p,
                                          &offset, flags, NULL, NULL, NULL, 1);
                        if (error)
                        {
                            if (free_value)
                    ...
                    case ACT_RP_SHORT_RIGHT:
                        for (p = end; p >= value; --p)
                        {
                            if (fnmatch(pattern, p, 0) != FNM_NOMATCH)
                            {
                                ...
                                *(char*)__mempcpy(newval, value, p - value) =
                                    '\0';
                                if (free_value)
                                    free(value);
                                value = newval;
    ...
    for (; words[*offset]; ++(*offset))
    {
        switch (words[*offset])
        {
            ...
            case '\\':
                if (squoting)
                {
                    error = parse_qtd_backslash(&comm, &comm_length,
                                                &comm_maxlen, words, offset);
                ...
                error = parse_backslash(&comm, &comm_length, &comm_maxlen,
                                        words, offset);
            ...
            default:
                comm =
                    w_addchar(comm, &comm_length, &comm_maxlen, words[*offset]);
                if (comm == NULL)
                    return WRDE_NOSPACE;
        }
    }
                        ...
                        const char* ifs_white)
{
    ...
    for (; words[*offset]; ++(*offset))
    {
        switch (words[*offset])
        {
            ...
            default:
                *word =
                    w_addchar(*word, word_length, max_length, words[*offset]);
                if (*word == NULL)
                    return WRDE_NOSPACE;
        }
    }
    ...
    int wordexp(const char* words, wordexp_t* pwordexp, int flags)
{
    ...
    for (words_offset = 0; words[words_offset]; ++words_offset)
        switch (words[words_offset])
        {
            ...
            case '?':
                error =
                    parse_glob(&word, &word_length, &max_length, words,
