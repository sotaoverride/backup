...
/*
 * Copyright 1990,1991,1995,2007,2008 by the Massachusetts Institute of
...
#define KTCHECKLOCK(id)                                                        \
    k5_mutex_assert_locked(&((krb5_ktfile_data*)(id)->data)->lock)

extern const struct _krb5_kt_ops krb5_ktf_ops;
extern const struct _krb5_kt_ops krb5_ktf_writable_ops;

static krb5_error_code KRB5_CALLCONV krb5_ktfile_resolve(krb5_context,
                                                         const char*,
                                                         krb5_keytab*);

static krb5_error_code KRB5_CALLCONV krb5_ktfile_get_name(krb5_context,
                                                          krb5_keytab, char*,
                                                          unsigned int);

static krb5_error_code KRB5_CALLCONV krb5_ktfile_close(krb5_context,
                                                       krb5_keytab);
...
static krb5_error_code KRB5_CALLCONV krb5_ktfile_start_seq_get(krb5_context,
                                                               krb5_keytab,
                                                               krb5_kt_cursor*);

static krb5_error_code KRB5_CALLCONV krb5_ktfile_get_next(krb5_context,
                                                          krb5_keytab,
                                                          krb5_keytab_entry*,
                                                          krb5_kt_cursor*);

static krb5_error_code KRB5_CALLCONV krb5_ktfile_end_get(krb5_context,
                                                         krb5_keytab,
                                                         krb5_kt_cursor*);

/* routines to be included on extended version (write routines) */
static krb5_error_code KRB5_CALLCONV krb5_ktfile_add(krb5_context, krb5_keytab,
                                                     krb5_keytab_entry*);

static krb5_error_code KRB5_CALLCONV krb5_ktfile_remove(krb5_context,
                                                        krb5_keytab,
                                                        krb5_keytab_entry*);

static krb5_error_code krb5_ktfileint_openr(krb5_context, krb5_keytab);

static krb5_error_code krb5_ktfileint_openw(krb5_context, krb5_keytab);

static krb5_error_code krb5_ktfileint_close(krb5_context, krb5_keytab);

static krb5_error_code krb5_ktfileint_read_entry(krb5_context, krb5_keytab,
                                                 krb5_keytab_entry*);

static krb5_error_code krb5_ktfileint_write_entry(krb5_context, krb5_keytab,
                                                  krb5_keytab_entry*);

static krb5_error_code krb5_ktfileint_delete_entry(krb5_context, krb5_keytab,
                                                   krb5_int32);

static krb5_error_code krb5_ktfileint_internal_read_entry(krb5_context,
                                                          krb5_keytab,
                                                          krb5_keytab_entry*,
                                                          krb5_int32*);
...
static krb5_error_code krb5_ktfileint_find_slot(krb5_context, krb5_keytab,
...
static krb5_error_code KRB5_CALLCONV krb5_ktfile_resolve(krb5_context context,
                                                         const char* name,
                                                         krb5_keytab* id_out)
{
...
static krb5_error_code KRB5_CALLCONV krb5_ktfile_close(krb5_context context,
                                                       krb5_keytab id)
/*
...
static krb5_boolean more_recent(const krb5_keytab_entry* k1,
                                const krb5_keytab_entry* k2)
{
    /*
     ...
     */
    if (!ts_after(k2->timestamp, k1->timestamp) && k1->vno < 128 &&
        k2->vno > 240)
        return TRUE;
    if (!ts_after(k1->timestamp, k2->timestamp) && k1->vno > 240 &&
        k2->vno < 128)
        return FALSE;
    ...
    krb5_kvno kvno, krb5_enctype enctype, krb5_keytab_entry* entry)
{
    ...
    while (TRUE)
    {
        ...
        {
            if ((kerror = krb5_c_enctype_compare(
                     context, enctype, new_entry.key.enctype, &similar)))
            {
                krb5_kt_free_entry(context, &new_entry);
                break;
            }
    ...
    if (kerror == KRB5_KT_END)
    {
        ...
        else
        {
            ...
            {
                k5_setmsg(context, kerror, _("No key table entry found for %s"),
                          princname);
                free(princname);
            }
        }
...
static krb5_error_code KRB5_CALLCONV krb5_ktfile_get_name(krb5_context context,
                                                          krb5_keytab id,
                                                          char* name,
                                                          unsigned int len)
/*
...
static krb5_error_code KRB5_CALLCONV krb5_ktfile_start_seq_get(
    krb5_context context, krb5_keytab id, krb5_kt_cursor* cursorp)
{
...
static krb5_error_code KRB5_CALLCONV
    krb5_ktfile_get_next(krb5_context context, krb5_keytab id,
...
static krb5_error_code KRB5_CALLCONV krb5_ktfile_end_get(krb5_context context,
                                                         krb5_keytab id,
 ...
 */
static krb5_error_code krb5_ktf_keytab_size(krb5_context, krb5_pointer,
                                            size_t*);

static krb5_error_code krb5_ktf_keytab_externalize(krb5_context, krb5_pointer,
                                                   krb5_octet**, size_t*);

static krb5_error_code krb5_ktf_keytab_internalize(krb5_context, krb5_pointer*,
                                                   krb5_octet**, size_t*);
 ...
 */
static krb5_error_code krb5_ktf_keytab_size(krb5_context kcontext,
                                            krb5_pointer arg, size_t* sizep)
{
    ...
    if ((keytab = (krb5_keytab)arg))
    {
        ...
        required +=
            strlen((ktdata && ktdata->name) ? ktdata->name : ktfile_def_name);
        kret = 0;
 ...
 */
static krb5_error_code krb5_ktf_keytab_externalize(krb5_context kcontext,
                                                   krb5_pointer arg,
                                                   krb5_octet** buffer,
                                                   size_t* lenremain)
{
    ...
    if ((keytab = (krb5_keytab)arg))
    {
            ...
            (required <= remain))
        {
            ...
            if (ktname)
            {
                ...
                (void)krb5_ser_pack_int32((krb5_int32)strlen(ktname), &bp,
                                          &remain);

                /* Put the name */
                (void)krb5_ser_pack_bytes((krb5_octet*)ktname, strlen(ktname),
                                          &bp, &remain);
                ...
                (void)krb5_ser_pack_int32(
                    (krb5_int32)((ktdata) ? ktdata->version : 0), &bp, &remain);
 ...
 */
static krb5_error_code krb5_ktf_keytab_internalize(krb5_context kcontext,
                                                   krb5_pointer* argp,
                                                   krb5_octet** buffer,
                                                   size_t* lenremain)
{
    ...
    *argp = NULL;
    ...
    if (!ktname)
        return ENOMEM;
    kret =
        krb5_ser_unpack_bytes((krb5_octet*)ktname, (size_t)ibuf, &bp, &remain);
    if (kret)
        goto cleanup;
    ktname[ibuf] = '\0';
...
static krb5_error_code KRB5_CALLCONV krb5_ktfile_add(krb5_context context,
                                                     krb5_keytab id,
    ...
    krb5_context context, krb5_keytab id, krb5_keytab_entry* entry)
{
    ...
    while (TRUE)
    {
        if ((kerror = krb5_ktfileint_internal_read_entry(
                 context, id, &cur_entry, &delete_point)))
            break;
...
static krb5_error_code krb5_ktfileint_open(krb5_context context, krb5_keytab id,
                                           int mode)
{
    krb5_error_code kerror;
    krb5_kt_vno kt_vno;
    ...
    errno = 0;
    KTFILEP(id) =
        fopen(KTFILENAME(id), (mode == KRB5_LOCKMODE_EXCLUSIVE) ? "rb+" : "rb");
    if (!KTFILEP(id))
    {
    ...
    else
    {
        ...
        if ((kt_vno != KRB5_KT_VNO) && (kt_vno != KRB5_KT_VNO_1))
        {
            (void)krb5_unlock_file(context, fileno(KTFILEP(id)));
            (void)fclose(KTFILEP(id));
            KTFILEP(id) = 0;
...
static krb5_error_code krb5_ktfileint_openr(krb5_context context,
                                            krb5_keytab id)
{
    return krb5_ktfileint_open(context, id, KRB5_LOCKMODE_SHARED);
}

static krb5_error_code krb5_ktfileint_openw(krb5_context context,
                                            krb5_keytab id)
{
    return krb5_ktfileint_open(context, id, KRB5_LOCKMODE_EXCLUSIVE);
}

static krb5_error_code krb5_ktfileint_close(krb5_context context,
                                            krb5_keytab id)
{
    krb5_error_code kerror;
...
static krb5_error_code krb5_ktfileint_delete_entry(krb5_context context,
                                                   krb5_keytab id,
                                                   krb5_int32 delete_point)
{
    krb5_int32 size;
...
static krb5_error_code
    krb5_ktfileint_internal_read_entry(krb5_context context, krb5_keytab id,
                                       ...
                                       krb5_int32* delete_point)
{
    ...
    ret_entry->principal->data = (krb5_data*)calloc(u_count, sizeof(krb5_data));
    ...
    if (!fread(&ret_entry->timestamp, sizeof(ret_entry->timestamp), 1,
               KTFILEP(id)))
    {
        error = KRB5_KT_END;
        goto fail;
    ...
    if (!fread(ret_entry->key.contents, sizeof(krb5_octet), count, KTFILEP(id)))
    {
        error = KRB5_KT_END;
        goto fail;
    }
...
static krb5_error_code krb5_ktfileint_read_entry(krb5_context context,
                                                 krb5_keytab id,
...
static krb5_error_code krb5_ktfileint_write_entry(krb5_context context,
                                                  krb5_keytab id,
                                                  krb5_keytab_entry* entry)
{
    krb5_octet vno;
    ...
    if (!fwrite(entry->key.contents, sizeof(krb5_octet), entry->key.length,
                KTFILEP(id)))
    {
        goto abend;
    }
 ...
 * keytab entry.
 */
static krb5_error_code krb5_ktfileint_size_entry(krb5_context context,
                                                 krb5_keytab_entry* entry,
                                                 krb5_int32* size_needed)
{
    krb5_int16 count;
 ...
 */
static krb5_error_code krb5_ktfileint_find_slot(krb5_context context,
                                                krb5_keytab id,
                                                krb5_int32* size_needed,
                                                krb5_int32* commit_point_ptr)
{
    FILE* fp;
