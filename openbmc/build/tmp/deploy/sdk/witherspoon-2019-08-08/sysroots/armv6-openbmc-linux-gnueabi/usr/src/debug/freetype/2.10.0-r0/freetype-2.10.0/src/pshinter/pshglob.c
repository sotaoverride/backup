...
static void psh_blues_set_zones_0(PSH_Blues target, FT_Bool is_others,
                                  FT_UInt read_count, FT_Short* read,
...
static void psh_blues_set_zones(PSH_Blues target, FT_UInt count,
                                FT_Short* blues, FT_UInt count_others,
                                FT_Short* other_blues, FT_Int fuzz,
                                FT_Int family)
{
    ...
    psh_blues_set_zones_0(target, 1, count_others, other_blues, top_table,
                          bot_table);
...
FT_LOCAL_DEF(void)
psh_blues_snap_stem(PSH_Blues blues, FT_Int stem_top, FT_Int stem_bot,
...
static FT_Error psh_globals_new(FT_Memory memory, T1_Private* priv,
    ...
    if (!FT_NEW(globals))
    {
        ...
        /* limit the BlueScale value to `1 / max_of_blue_zone_heights' */
        {
            ...
            max_height = psh_calc_max_height(priv->num_blue_values,
                                             priv->blue_values, max_height);
            max_height = psh_calc_max_height(priv->num_other_blues,
                                             priv->other_blues, max_height);
            max_height = psh_calc_max_height(priv->num_family_blues,
                                             priv->family_blues, max_height);
            max_height =
                psh_calc_max_height(priv->num_family_other_blues,
                                    priv->family_other_blues, max_height);

            /* BlueScale is scaled 1000 times */
            max_scale = FT_DivFix(1000, max_height);
            globals->blues.blue_scale =
                priv->blue_scale < max_scale ? priv->blue_scale : max_scale;
        }
...
FT_LOCAL_DEF(void)
psh_globals_set_scale(PSH_Globals globals, FT_Fixed x_scale, FT_Fixed y_scale,
                      FT_Fixed x_delta, FT_Fixed y_delta)
{
    PSH_Dimension dim;
