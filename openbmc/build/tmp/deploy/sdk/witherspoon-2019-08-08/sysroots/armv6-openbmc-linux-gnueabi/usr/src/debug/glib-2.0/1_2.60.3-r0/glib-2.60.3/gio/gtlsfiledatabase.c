...
static void g_tls_file_database_default_init(GTlsFileDatabaseInterface* iface)
{
    /**
    ...
    g_object_interface_install_property(
        iface,
        g_param_spec_string("anchors", P_("Anchors"),
                            P_("The certificate authority anchor file"), NULL,
...
GTlsDatabase* g_tls_file_database_new(const gchar* anchors, GError** error)
{
    ...
    database = g_initable_new(g_tls_backend_get_file_database_type(backend),
                              NULL, error, "anchors", anchors, NULL);
