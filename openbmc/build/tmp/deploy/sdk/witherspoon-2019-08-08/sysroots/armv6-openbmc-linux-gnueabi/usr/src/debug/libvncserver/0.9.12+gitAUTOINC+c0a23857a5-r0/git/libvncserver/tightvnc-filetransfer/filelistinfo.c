...
{
    int i = 0;
    if ((fli.pEntries == NULL) || (fli.numEntries == 0))
        return;
...
int AddFileListItemInfo(FileListInfoPtr fileListInfoPtr, char* name,
                        unsigned int size, unsigned int data)
{
    FileListItemInfoPtr fileListItemInfoPtr = (FileListItemInfoPtr)calloc(
        (fileListInfoPtr->numEntries + 1), sizeof(FileListItemInfo));
