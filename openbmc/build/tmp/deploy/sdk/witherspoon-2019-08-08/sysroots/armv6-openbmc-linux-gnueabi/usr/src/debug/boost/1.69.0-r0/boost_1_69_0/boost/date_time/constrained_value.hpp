...
class BOOST_SYMBOL_VISIBLE simple_exception_policy
{
    ...
    {
        // In order to support throw_exception mechanism in the
        // BOOST_NO_EXCEPTIONS mode, we'll have to provide a way to acquire
        // std::exception from the exception being thrown. However, we cannot
        // derive from it, since it would make it interceptable by this class,
        // which might not be what the user wanted.
        operator std::out_of_range() const
        {
            // TODO: Make the message more descriptive by using arguments to
            // on_error
            return std::out_of_range(
                "constrained value boundary has been violated");
        }
    };

    typedef typename mpl::if_<is_base_of<std::exception, exception_type>,
                              exception_type, exception_wrapper>::type
