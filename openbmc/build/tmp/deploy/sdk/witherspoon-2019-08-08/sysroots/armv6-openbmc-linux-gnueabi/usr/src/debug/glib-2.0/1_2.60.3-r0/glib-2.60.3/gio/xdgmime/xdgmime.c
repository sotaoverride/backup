...
static void xdg_dir_time_list_add(char* file_name, time_t mtime)
{
    XdgDirTimeList* list;
...
static void xdg_init_dirs(void)
{
    ...
    else if (home != NULL)
    {
        ...
        guessed_xdg_home =
            malloc(strlen(home) + strlen("/.local/share/mime/") + 1);
 ...
 */
static int xdg_check_file(const char* file_path, int* exists)
{
    struct stat st;
...
static int xdg_check_dirs(void)
{
    ...
    xdg_run_command_on_dirs((XdgDirectoryFunc)xdg_check_dir, &invalid_dir_list);
                                            ...
                                            int* result_prio)
{
    ...
    if (_caches)
        mime_type =
            _xdg_mime_cache_get_mime_type_for_data(data, len, result_prio);
    else
                                           ...
                                           int n_mime_types)
{
    ...
    if (_caches)
        return _xdg_mime_cache_get_mime_types_from_file_name(
            file_name, mime_types, n_mime_types);

    return _xdg_glob_hash_lookup_file_name(global_hash, file_name, mime_types,
                                           n_mime_types);
}
...
static int xdg_mime_is_super_type(const char* mime)
{
...
/* Registers a function to be called every time the mime database reloads its
 * files
 */
int xdg_mime_register_reload_callback(XdgMimeCallback callback, void* data,
                                      XdgMimeDestroy destroy)
{
