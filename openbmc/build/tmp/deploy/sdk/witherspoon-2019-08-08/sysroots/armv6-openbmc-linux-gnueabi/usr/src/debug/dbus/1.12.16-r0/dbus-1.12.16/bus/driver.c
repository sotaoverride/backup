...
static DBusConnection* bus_driver_get_owner_of_name(DBusConnection* connection,
                                                    const char* name)
{
...
BusDriverFound
    bus_driver_get_conn_helper(DBusConnection* connection, DBusMessage* message,
                               const char* what_we_want, const char** name_p,
                               DBusConnection** peer_conn_p, DBusError* error)
{
    DBusConnection* conn;
    const char* name;
...
static dbus_bool_t bus_driver_check_caller_is_privileged(
    DBusConnection* connection, BusTransaction* transaction,
    DBusMessage* message, DBusError* error)
{
    ...
    if (!dbus_connection_get_unix_user(connection, &uid))
    {
        ...
        bus_context_log_and_set_error(
            bus_transaction_get_context(transaction), DBUS_SYSTEM_LOG_SECURITY,
            error, DBUS_ERROR_ACCESS_DENIED,
                                                  ...
                                                  DBusError* error)
{
    ...
    if (!dbus_message_append_args(
            message, DBUS_TYPE_STRING, &service_name, DBUS_TYPE_STRING,
                                         ...
                                         DBusError* error)
{
    ...
    if (!dbus_message_set_destination(message,
        ...
        !dbus_message_append_args(message, DBUS_TYPE_STRING, &service_name,
                                  DBUS_TYPE_INVALID))
    {
        dbus_message_unref(message);
        BUS_SET_OOM(error);
        ...
        if (next_minor_number <= 0)
        {
            ...
            if (next_major_number <= 0)
                _dbus_assert_not_reached(
                    "INT_MAX * INT_MAX clients were added");
        }
                                           ...
                                           DBusError* error)
{
    ...
    if (!bus_connections_check_limits(connections, connection, &limit_name,
                                      &limit, &tmp_error))
    {
        BusContext* context;
    ...
    if (!bus_driver_send_welcome_message(connection, message, transaction,
                                         error))
        goto out_0;

    /* Create the service */
    service = bus_registry_ensure(registry, &unique_name, connection, 0,
                                  transaction, error);
    if (service == NULL)
        goto out_0;
                                                   ...
                                                   DBusError* error)
{
    ...
    if (!dbus_message_append_args(welcome, DBUS_TYPE_STRING, &name,
                                  DBUS_TYPE_INVALID))
    {
    ...
    _dbus_assert(
        dbus_message_has_signature(welcome, DBUS_TYPE_STRING_AS_STRING));
...
static dbus_bool_t bus_driver_handle_list_services(DBusConnection* connection,
                                                   BusTransaction* transaction,
                                                   DBusMessage* message,
                                                   DBusError* error)
{
                                      ...
                                      DBusMessage* message, DBusError* error)
{
    ...
    if (!bus_registry_acquire_service(registry, connection, &service_name,
                                      flags, &service_reply, transaction,
                                      error))
        goto out;
    ...
    if (!dbus_message_append_args(reply, DBUS_TYPE_UINT32, &service_reply,
                                  DBUS_TYPE_INVALID))
    {
        BUS_SET_OOM(error);
        goto out;
                                      ...
                                      DBusMessage* message, DBusError* error)
{
    ...
    if (!bus_registry_release_service(registry, connection, &service_name,
                                      &service_reply, transaction, error))
        goto out;
...
static dbus_bool_t bus_driver_handle_service_exists(DBusConnection* connection,
                                                    BusTransaction* transaction,
                                                    DBusMessage* message,
                                                    DBusError* error)
{
 ...
 */
static dbus_bool_t bus_driver_send_or_activate(BusTransaction* transaction,
                                               DBusMessage* message,
                                               DBusError* error)
{
    BusContext* context;
    ...
    service =
        bus_registry_lookup(bus_context_get_registry(context), &service_string);
    ...
    if (service == NULL)
    {
        ...
        if (!bus_activation_activate_service(activation, NULL, transaction,
                                             TRUE, message, service_name,
                                             error))
        {
            _DBUS_ASSERT_ERROR_IS_SET(error);
            _dbus_verbose("bus_activation_activate_service() failed");
    ...
    else
    {
        ...
        if (!bus_transaction_send_from_driver(transaction, service_conn,
                                              message))
        {
            BUS_SET_OOM(error);
    ...
    DBusMessage* message, DBusError* error)
{
    ...
    keys = NULL;
    values = NULL;
    while ((array_type = dbus_message_iter_get_arg_type(&dict_iter)) ==
           DBUS_TYPE_DICT_ENTRY)
    {
        dbus_message_iter_recurse(&dict_iter, &dict_entry_iter);

        while ((key_type = dbus_message_iter_get_arg_type(&dict_entry_iter)) ==
               DBUS_TYPE_STRING)
        {
            char* key;
            char* value;
    ...
    _dbus_assert(_dbus_list_get_length(&keys) ==
                 _dbus_list_get_length(&values));

    if (bus_context_get_systemd_activation(
    ...
    while (key_link != NULL)
    {
        ...
        if (!bus_activation_set_environment_variable(activation, key, value,
                                                     error))
        {
            _DBUS_ASSERT_ERROR_IS_SET(error);
        ...
        if (systemd_message != NULL)
        {
            ...
            if (!dbus_message_iter_append_basic(&systemd_iter, DBUS_TYPE_STRING,
                                                &s))
            {
                BUS_SET_OOM(error);
    ...
    /* FIXME: We can fail early having set only some of the environment
     * variables, (because of OOM failure).  It's sort of hard to fix and it
     * doesn't really matter, so we're punting for now.
     */
    if (key_link != NULL)
    {
        if (systemd_message != NULL)
...
static dbus_bool_t bus_driver_handle_add_match(DBusConnection* connection,
                                               BusTransaction* transaction,
                                               DBusMessage* message,
                                               DBusError* error)
{
...
static dbus_bool_t bus_driver_handle_remove_match(DBusConnection* connection,
                                                  BusTransaction* transaction,
                                                  DBusMessage* message,
                                                  DBusError* error)
{
                                        ...
                                        DBusMessage* message, DBusError* error)
{
    ...
    {
        base_name = bus_connection_get_name(
    ...
    if (!dbus_message_append_args(reply, DBUS_TYPE_STRING, &base_name,
                                  DBUS_TYPE_INVALID))
        goto oom;
                                         ...
                                         DBusMessage* message, DBusError* error)
{
    ...
    {
        if (!bus_service_list_queued_owners(service, &base_names, error))
            goto failed;
    }
    ...
    DBusMessage* message, DBusError* error)
{
    ...
    uid32 = uid;
    if (!dbus_message_append_args(reply, DBUS_TYPE_UINT32, &uid32,
                                  DBUS_TYPE_INVALID))
        goto oom;
    ...
    DBusMessage* message, DBusError* error)
{
    ...
    {
        dbus_set_error(error, DBUS_ERROR_UNIX_PROCESS_ID_UNKNOWN,
    ...
    pid32 = pid;
    if (!dbus_message_append_args(reply, DBUS_TYPE_UINT32, &pid32,
                                  DBUS_TYPE_INVALID))
        goto oom;
    ...
    DBusMessage* message, DBusError* error)
{
    ...
    found = bus_driver_get_conn_helper(
        connection, message, "audit session data", &service, &conn, error);
    ...
    if (found != BUS_DRIVER_FOUND_PEER ||
        ...
        data == NULL)
    {
        dbus_set_error(error, DBUS_ERROR_ADT_AUDIT_DATA_UNKNOWN,
                       "Could not determine audit session data for '%s'",
                       service);
        goto failed;
    }

    if (!dbus_message_append_args(reply, DBUS_TYPE_ARRAY, DBUS_TYPE_BYTE, &data,
                                  data_size, DBUS_TYPE_INVALID))
        goto oom;
    ...
    DBusMessage* message, DBusError* error)
{
        ...
        _dbus_connection_get_linux_security_label(conn, &s))
    {
        ...
        if (!_dbus_asv_add_byte_array(&array_iter, "LinuxSecurityLabel", s,
                                      strlen(s) + 1))
        {
            dbus_free(s);
            goto oom;
...
static dbus_bool_t bus_driver_handle_reload_config(DBusConnection* connection,
                                                   BusTransaction* transaction,
                                                   DBusMessage* message,
                                                   DBusError* error)
{
...
static dbus_bool_t bus_driver_handle_enable_verbose(DBusConnection* connection,
                                                    BusTransaction* transaction,
                                                    DBusMessage* message,
                                                    DBusError* error)
{
...
static dbus_bool_t bus_driver_handle_get_id(DBusConnection* connection,
                                            BusTransaction* transaction,
                                            DBusMessage* message,
                                            DBusError* error)
{
    ...
    if (!dbus_message_append_args(reply, DBUS_TYPE_STRING, &v_STRING,
                                  DBUS_TYPE_INVALID))
        goto oom;
...
static dbus_bool_t bus_driver_handle_become_monitor(DBusConnection* connection,
                                                    BusTransaction* transaction,
                                                    DBusMessage* message,
                                                    DBusError* error)
{
    ...
    if (!dbus_message_get_args(message, error, DBUS_TYPE_ARRAY,
                               DBUS_TYPE_STRING, &match_rules, &n_match_rules,
                               DBUS_TYPE_UINT32, &flags, DBUS_TYPE_INVALID))
        goto out;
...
out:
    ...
    for (iter = _dbus_list_get_first_link(&rules); iter != NULL;
...
static dbus_bool_t bus_driver_handle_get_machine_id(DBusConnection* connection,
                                                    BusTransaction* transaction,
                                                    DBusMessage* message,
                                                    DBusError* error)
{
    ...
    if (!dbus_message_append_args(reply, DBUS_TYPE_STRING, &str,
                                  DBUS_TYPE_INVALID))
        goto oom;
...
static dbus_bool_t bus_driver_handle_ping(DBusConnection* connection,
                                          BusTransaction* transaction,
                                          DBusMessage* message,
                                          DBusError* error)
{
...
static const MessageHandler dbus_message_handlers[] = {
    {"Hello", "", DBUS_TYPE_STRING_AS_STRING, bus_driver_handle_hello,
     METHOD_FLAG_ANY_PATH},
    {"RequestName", DBUS_TYPE_STRING_AS_STRING DBUS_TYPE_UINT32_AS_STRING,
     DBUS_TYPE_UINT32_AS_STRING, bus_driver_handle_acquire_service,
     METHOD_FLAG_ANY_PATH},
    {"ReleaseName", DBUS_TYPE_STRING_AS_STRING, DBUS_TYPE_UINT32_AS_STRING,
     bus_driver_handle_release_service, METHOD_FLAG_ANY_PATH},
    {"StartServiceByName",
     DBUS_TYPE_STRING_AS_STRING DBUS_TYPE_UINT32_AS_STRING,
     DBUS_TYPE_UINT32_AS_STRING, bus_driver_handle_activate_service,
     METHOD_FLAG_ANY_PATH},
    ...
    {"NameHasOwner", DBUS_TYPE_STRING_AS_STRING, DBUS_TYPE_BOOLEAN_AS_STRING,
     bus_driver_handle_service_exists, METHOD_FLAG_ANY_PATH},
    {"ListNames", "", DBUS_TYPE_ARRAY_AS_STRING DBUS_TYPE_STRING_AS_STRING,
     bus_driver_handle_list_services, METHOD_FLAG_ANY_PATH},
    {"ListActivatableNames", "",
     DBUS_TYPE_ARRAY_AS_STRING DBUS_TYPE_STRING_AS_STRING,
     bus_driver_handle_list_activatable_services, METHOD_FLAG_ANY_PATH},
    {"AddMatch", DBUS_TYPE_STRING_AS_STRING, "", bus_driver_handle_add_match,
     METHOD_FLAG_ANY_PATH},
    {"RemoveMatch", DBUS_TYPE_STRING_AS_STRING, "",
     bus_driver_handle_remove_match, METHOD_FLAG_ANY_PATH},
    {"GetNameOwner", DBUS_TYPE_STRING_AS_STRING, DBUS_TYPE_STRING_AS_STRING,
     bus_driver_handle_get_service_owner, METHOD_FLAG_ANY_PATH},
    ...
    {"GetConnectionUnixUser", DBUS_TYPE_STRING_AS_STRING,
     DBUS_TYPE_UINT32_AS_STRING, bus_driver_handle_get_connection_unix_user,
     METHOD_FLAG_ANY_PATH},
    {"GetConnectionUnixProcessID", DBUS_TYPE_STRING_AS_STRING,
    ...
    {"ReloadConfig", "", "", bus_driver_handle_reload_config,
     METHOD_FLAG_ANY_PATH},
    {"GetId", "", DBUS_TYPE_STRING_AS_STRING, bus_driver_handle_get_id,
     METHOD_FLAG_ANY_PATH},
    {"GetConnectionCredentials", "s", "a{sv}",
     bus_driver_handle_get_connection_credentials, METHOD_FLAG_ANY_PATH},
    {NULL, NULL, NULL, NULL}};
...
static dbus_bool_t write_args_for_direction(DBusString* xml,
                                            ...
                                            dbus_bool_t in)
{
    ...
    while ((current_type = _dbus_type_reader_get_current_type(&typereader)) !=
           DBUS_TYPE_INVALID)
    {
        ...
        if (!_dbus_string_append_printf(
                xml, "      <arg direction=\"%s\" type=\"", in ? "in" : "out"))
            goto oom;
        if (!_dbus_string_append_len(
                xml, _dbus_string_get_const_data(subsig) + start, len))
            goto oom;
    ...
    for (ih = interface_handlers; ih->name != NULL; ih++)
    {
        ...
        for (ph = ih->property_handlers; ph != NULL && ph->name != NULL; ph++)
        {
            ...
            if (!_dbus_string_append(xml,
                                     ...
                                     "EmitsChangedSignal\" value=\"const\"/>\n"
                                     "    </property>\n"))
                return FALSE;
        }
    ...
    if (message != NULL)
    {
        ...
        if (dbus_message_has_path(message, "/"))
        {
            if (!_dbus_string_append(
                    xml, "  <node name=\"org/freedesktop/DBus\"/>\n"))
                return FALSE;
        }
...
static dbus_bool_t bus_driver_handle_introspect(DBusConnection* connection,
                                                BusTransaction* transaction,
                                                DBusMessage* message,
                                                DBusError* error)
{
                                      ...
                                      DBusMessage* message, DBusError* error)
{
    ...
    if (dbus_message_is_signal(message, "org.freedesktop.systemd1.Activator",
                               "ActivationFailure"))
    {
        BusContext* context;
    ...
    for (ih = interface_handlers; ih->name != NULL; ih++)
    {
        ...
        for (mh = ih->message_handlers; mh->name != NULL; mh++)
        {
            ...
            if (!(is_canonical_path || (mh->flags & METHOD_FLAG_ANY_PATH)))
            {
                _DBUS_ASSERT_ERROR_IS_CLEAR(error);
                dbus_set_error(error, DBUS_ERROR_ACCESS_DENIED,
                               "Method '%s' is only available at the canonical "
                               "object path '%s'",
    ...
    dbus_set_error(error,
                   found_interface ? DBUS_ERROR_UNKNOWN_METHOD
                                   : DBUS_ERROR_UNKNOWN_INTERFACE,
                   "%s does not understand message %s", DBUS_SERVICE_DBUS,
                   name);
...
static const InterfaceHandler*
    bus_driver_find_interface(const char* name, dbus_bool_t canonical_path,
                              DBusError* error)
{
    const InterfaceHandler* ih;
                                         ...
                                         DBusMessage* message, DBusError* error)
{
    ...
    if (!dbus_message_get_args(message, error, DBUS_TYPE_STRING, &iface,
                                             ...
                                             DBusError* error)
{
    ...
    for (ph = ih->property_handlers; ph != NULL && ph->name != NULL; ph++)
    {
        ...
        if (!_dbus_asv_open_entry(&array_iter, &entry_iter, ph->name, ph->type,
                                  &var_iter))
                                         ...
                                         DBusMessage* message, DBusError* error)
{
    ...
    if (!dbus_message_iter_init(message, &iter))
        _dbus_assert_not_reached(
            "Message type was already checked to be 'ssv'");
