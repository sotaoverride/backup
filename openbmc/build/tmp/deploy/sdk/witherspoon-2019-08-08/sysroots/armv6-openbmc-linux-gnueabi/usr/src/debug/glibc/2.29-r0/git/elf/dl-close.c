...
static bool remove_slotinfo(size_t idx, struct dtv_slotinfo_list* listp,
                            size_t disp, bool should_be_there)
{
...
retry:
    ...
    while (++done_index < nloaded)
    {
        ...
        if (l->l_type == lt_loaded && l->l_direct_opencount == 0 &&
            ...
            for (size_t cnt = 0; imap->l_scope[cnt] != NULL; ++cnt)
                ...
                if (imap->l_scope[cnt] != &imap->l_symbolic_searchlist)
                {
                    struct link_map* tmap =
                        (struct link_map*)((char*)imap->l_scope[cnt] -
...
#define SCOPE_ELEMS(imap)                                                      \
    ...
    if (!RTLD_SINGLE_THREAD_P &&
        (unload_global || scope_mem_left ||
         (GL(dl_scope_free_list) != NULL && GL(dl_scope_free_list)->count)))
    {
        THREAD_GSCOPE_WAIT();
...
#error "Either TLS_TCB_AT_TP or TLS_DTV_AT_TP must be defined"
    ...
    if (any_tls)
    {
        if (__glibc_unlikely(++GL(dl_tls_generation) == 0))
            _dl_fatal_printf("TLS generation counter wrapped!  Please report "
    ...
    if (__builtin_expect(ns->_ns_loaded == NULL, 0) && nsid == GL(dl_nns) - 1)
        do
            --GL(dl_nns);
