...
nss_status_t NSS_NAME(gethostton_r)(const char* name, struct etherent* result,
                                    char* buffer, size_t buflen, int* errnop)
{
    NSS_GETONE(NSLCD_ACTION_ETHER_BYNAME, WRITE_STRING(fp, name),
...
{
    NSS_GETONE(NSLCD_ACTION_ETHER_BYETHER, WRITE(fp, addr, sizeof(uint8_t[6])),
               read_etherent(fp, result, buffer, buflen, errnop));
}
...
static nss_backend_op_t ethers_ops[] = {nss_ldap_destructor, ethers_gethostton,
                                        ethers_getntohost};
