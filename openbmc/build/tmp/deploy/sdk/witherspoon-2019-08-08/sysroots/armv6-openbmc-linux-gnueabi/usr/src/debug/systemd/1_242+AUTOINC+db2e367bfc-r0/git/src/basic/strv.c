...
int strv_split_extract(char*** t, const char* s, const char* separators,
                       ExtractFlags flags)
{
...
char** strv_parse_nulstr(const char* s, size_t l)
{
     ...
     * elements of the resulting strv. Hence, the number of items in the
     * resulting strv will be equal to one plus the number of NUL bytes in the l
     * bytes starting at s, unless s[l-1] is NUL, in which case the final empty
     * string is not stored in the resulting strv, and length is equal to the
     * number of NUL bytes.
     *
     * Note that contrary to a normal nulstr which cannot contain empty strings,
     * because the input data is terminated by any two consequent NUL bytes,
     * this parser accepts empty strings in s.
     */
