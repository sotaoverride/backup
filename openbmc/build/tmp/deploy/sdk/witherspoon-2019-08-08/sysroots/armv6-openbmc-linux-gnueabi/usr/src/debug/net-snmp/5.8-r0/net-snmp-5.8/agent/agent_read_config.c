...
netsnmp_feature_child_of(agent_read_config_all, libnetsnmpagent)
    ...
    netsnmp_feature_child_of(snmpd_unregister_config_handler,
                             agent_read_config_all)
...
void init_agent_read_config(const char* app)
{
    ...
    register_app_config_handler("authtrapenable", snmpd_parse_config_authtrap,
                                NULL, "1 | 2\t\t(1 = enable, 2 = disable)");
    register_app_config_handler("pauthtrapenable", snmpd_parse_config_authtrap,
                                NULL, NULL);
                               ...
                               NETSNMP_DS_AGENT_ROLE) == MASTER_AGENT)
    {
        ...
        register_app_config_handler(
            "trap2sink", snmpd_parse_config_trap2sink, snmpd_free_trapsinks,
            "[-profile p] [-name n] [-tag t] host [community] [port]");
        register_app_config_handler(
            "informsink", snmpd_parse_config_informsink, snmpd_free_trapsinks,
            "[-profile p] [-name n] [-tag t] host [community] [port]");
#endif
        register_app_config_handler(
            "trapsess", snmpd_parse_config_trapsess, snmpd_free_trapsinks,
    ...
    register_app_config_handler("trapcommunity",
                                ...
                                snmpd_free_trapcommunity, "community-string");
#endif /* support for community based SNMP */
    netsnmp_ds_register_config(ASN_OCTET_STR, app, "v1trapaddress",
                               NETSNMP_DS_APPLICATION_ID,
                               NETSNMP_DS_AGENT_TRAP_ADDR);
#ifdef HAVE_UNISTD_H
    register_app_config_handler("agentuser", snmpd_set_agent_user, NULL,
                                "userid");
    register_app_config_handler("agentgroup", snmpd_set_agent_group, NULL,
                                "groupid");
#endif
#ifndef NETSNMP_NO_LISTEN_SUPPORT
    register_app_config_handler("agentaddress", snmpd_set_agent_address, NULL,
                                "SNMP bind address");
#endif /* NETSNMP_NO_LISTEN_SUPPORT */
...
void snmpd_register_const_config_handler(
    const char* token, void (*parser)(const char*, const char*),
    void (*releaser)(void), const char* help)
{
    DEBUGMSGTL(("snmpd_register_app_config_handler",
                "registering .conf token for \"%s\"\n", token));
