...
static void send_property_change(GApplicationImpl* impl)
{
    GVariantBuilder builder;

    g_variant_builder_init(&builder, G_VARIANT_TYPE_ARRAY);
    g_variant_builder_add(&builder, "{sv}", "Busy",
                          g_variant_new_boolean(impl->busy));

    g_dbus_connection_emit_signal(
        impl->session_bus, NULL, impl->object_path,
        "org.freedesktop.DBus.Properties", "PropertiesChanged",
        g_variant_new("(sa{sv}as)", "org.gtk.Application", &builder, NULL),
        NULL);
}
    ...
    GDBusMethodInvocation* invocation, gpointer user_data)
{
    ...
    else if (strcmp(method_name, "Open") == 0)
    {
        ...
        if (g_str_equal(interface_name, "org.freedesktop.Application"))
        {
            ...
            g_variant_get(parameters, "(@as&s@a{sv})", &array, &hint,
                          &platform_data);
...
static gchar* application_path_from_appid(const gchar* appid)
{
...
static void name_lost(GDBusConnection* bus, const char* sender_name,
 ...
 * successful.
 */
static gboolean g_application_impl_attempt_primary(GApplicationImpl* impl,
                                                   GCancellable* cancellable,
                                                   GError** error)
{
    const static GDBusInterfaceVTable vtable = {
        g_application_impl_method_call, g_application_impl_get_property,
        NULL /* set_property */
    };
    ...
    if (org_gtk_Application == NULL)
    {
        ...
        if
            G_UNLIKELY(info == NULL)
        g_error("%s", error->message);
        org_gtk_Application =
            g_dbus_node_info_lookup_interface(info, "org.gtk.Application");
        g_assert(org_gtk_Application != NULL);
        g_dbus_interface_info_ref(org_gtk_Application);
        g_dbus_node_info_unref(info);

        info = g_dbus_node_info_new_for_xml(org_freedesktop_Application_xml,
                                            &error);
        if
            G_UNLIKELY(info == NULL)
        g_error("%s", error->message);
        org_freedesktop_Application = g_dbus_node_info_lookup_interface(
            info, "org.freedesktop.Application");
     ...
     * for the same reason.
     */
    impl->object_id = g_dbus_connection_register_object(
        impl->session_bus, impl->object_path, org_gtk_Application, &vtable,
        impl, NULL, error);

    if (impl->object_id == 0)
        return FALSE;

    impl->fdo_object_id = g_dbus_connection_register_object(
        impl->session_bus, impl->object_path, org_freedesktop_Application,
        &vtable, impl, NULL, error);

    if (impl->fdo_object_id == 0)
        return FALSE;

    impl->actions_id = g_dbus_connection_export_action_group(
        impl->session_bus, impl->object_path, impl->exported_actions, error);

    if (impl->actions_id == 0)
        return FALSE;

    impl->registered = TRUE;
    if (!app_class->dbus_register(impl->app, impl->session_bus,
                                  impl->object_path, error))
        return FALSE;
    ...
    {
        impl->name_lost_signal = g_dbus_connection_signal_subscribe(
            impl->session_bus, "org.freedesktop.DBus", "org.freedesktop.DBus",
            "NameLost", "/org/freedesktop/DBus", impl->bus_name,
            G_DBUS_SIGNAL_FLAGS_NONE, name_lost, impl, NULL);
    ...
    reply = g_dbus_connection_call_sync(
        ...
        G_VARIANT_TYPE("(u)"), 0, -1, cancellable, error);
    ...
    {
        g_dbus_connection_signal_unsubscribe(impl->session_bus,
                                             impl->name_lost_signal);
...
static void g_application_impl_stop_primary(GApplicationImpl* impl)
{
    ...
    if (impl->registered)
    {
        app_class->dbus_unregister(impl->app, impl->session_bus,
                                   impl->object_path);
    ...
    if (impl->actions_id)
    {
        g_dbus_connection_unexport_action_group(impl->session_bus,
                                                impl->actions_id);
        impl->actions_id = 0;
    }
    ...
    if (impl->primary && impl->bus_name)
    {
        g_dbus_connection_call(impl->session_bus, "org.freedesktop.DBus",
                               ...
                               "ReleaseName",
                               g_variant_new("(s)", impl->bus_name), NULL,
                               G_DBUS_CALL_FLAGS_NONE, -1, NULL, NULL, NULL);
        impl->primary = FALSE;
    }
}

void g_application_impl_set_busy_state(GApplicationImpl* impl, gboolean busy)
{
    if (impl->busy != busy)
    {
        impl->busy = busy;
...
GApplicationImpl* g_application_impl_register(
    GApplication* application, const gchar* appid, GApplicationFlags flags,
    GActionGroup* exported_actions, GRemoteActionGroup** remote_actions,
    GCancellable* cancellable, GError** error)
{
     ...
     */
    actions = g_dbus_action_group_get(impl->session_bus, impl->bus_name,
                                      impl->object_path);
                                 ...
                                 GVariant* platform_data)
{
    g_dbus_connection_call(impl->session_bus, impl->bus_name, impl->object_path,
                           ...
                           g_variant_new("(@a{sv})", platform_data), NULL, 0,
                           -1, NULL, NULL, NULL);
}

void g_application_impl_open(GApplicationImpl* impl, GFile** files,
                             ...
                             GVariant* platform_data)
{
    ...
    g_dbus_connection_call(impl->session_bus, impl->bus_name, impl->object_path,
                           ...
                           g_variant_builder_end(&builder), NULL, 0, -1, NULL,
                           NULL, NULL);
}

static void g_application_impl_cmdline_method_call(
    GDBusConnection* connection, const gchar* sender, const gchar* object_path,
    const gchar* interface_name, const gchar* method_name, GVariant* parameters,
    GDBusMethodInvocation* invocation, gpointer user_data)
{
...
static void g_application_impl_cmdline_done(GObject* source,
                                            GAsyncResult* result,
                                            gpointer user_data)
{
    ...
    reply = g_dbus_connection_call_with_unix_fd_list_finish(
        G_DBUS_CONNECTION(source), NULL, result, &error);
#else
                                    ...
                                    GVariant* platform_data)
{
    ...
    if (org_gtk_private_CommandLine == NULL)
    {
        ...
        info = g_dbus_node_info_new_for_xml(org_gtk_private_CommandLine_xml,
                                            &error);
        if
            G_UNLIKELY(info == NULL)
        g_error("%s", error->message);
        org_gtk_private_CommandLine = g_dbus_node_info_lookup_interface(
            info, "org.gtk.private.CommandLine");
    ...
    object_id = g_dbus_connection_register_object(
        impl->session_bus, object_path, org_gtk_private_CommandLine, &vtable,
        &data, NULL, NULL);
...
#ifdef G_OS_UNIX
    {
        ...
        g_dbus_connection_call_with_unix_fd_list(
            impl->session_bus, impl->bus_name, impl->object_path,
...
G_DEFINE_TYPE(GDBusCommandLine, g_dbus_command_line,
              G_TYPE_APPLICATION_COMMAND_LINE)

static void g_dbus_command_line_print_literal(GApplicationCommandLine* cmdline,
                                              const gchar* message)
{
    GDBusCommandLine* gdbcl = (GDBusCommandLine*)cmdline;

    g_dbus_connection_call(gdbcl->connection, gdbcl->bus_name,
                           gdbcl->object_path, "org.gtk.private.CommandLine",
                           "Print", g_variant_new("(s)", message), NULL, 0, -1,
                           NULL, NULL, NULL);
}
                                         ...
                                         const gchar* message)
{
    ...
    g_dbus_connection_call(gdbcl->connection, gdbcl->bus_name,
                           gdbcl->object_path, "org.gtk.private.CommandLine",
                           "PrintError", g_variant_new("(s)", message), NULL, 0,
                           -1, NULL, NULL, NULL);
}
...
static void g_dbus_command_line_finalize(GObject* object)
{
    ...
    G_OBJECT_CLASS(g_dbus_command_line_parent_class)->finalize(object);
}

static void g_dbus_command_line_init(GDBusCommandLine* gdbcl)
{
}

static void g_dbus_command_line_class_init(GApplicationCommandLineClass* class)
{
    ...
    g_dbus_command_line_new(GDBusMethodInvocation* invocation)
{
    ...
    gdbcl = g_object_new(g_dbus_command_line_get_type(), "arguments", arguments,
                         "platform-data", platform_data, NULL);
