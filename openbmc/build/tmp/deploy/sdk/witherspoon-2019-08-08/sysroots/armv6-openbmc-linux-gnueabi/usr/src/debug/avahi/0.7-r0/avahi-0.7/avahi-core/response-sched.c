...
static AvahiResponseJob* job_new(AvahiResponseScheduler* s, AvahiRecord* record,
                                 AvahiResponseJobState state)
{
    AvahiResponseJob* rj;
...
static void job_set_elapse_time(AvahiResponseScheduler* s, AvahiResponseJob* rj,
                                unsigned msec, unsigned jitter)
{
    ...
    if (rj->time_event)
        ...
        rj->time_event =
            avahi_time_event_new(s->time_event_queue, &tv, elapse_callback, rj);
}
...
static void enumerate_aux_records_callback(AVAHI_GCC_UNUSED AvahiServer* s,
                                           AvahiRecord* r, int flush_cache,
                                           void* userdata)
{
    ...
    avahi_response_scheduler_post(rj->scheduler, r, flush_cache,
                                  rj->querier_valid ? &rj->querier : NULL, 0);
}

static int packet_add_response_job(AvahiResponseScheduler* s, AvahiDnsPacket* p,
                                   AvahiResponseJob* rj)
{
    assert(s);
    assert(p);
    ...
    avahi_server_enumerate_aux_records(s->interface->monitor->server,
                                       s->interface, rj->record,
                                       enumerate_aux_records_callback, rj);
    job_mark_done(s, rj);

    return 1;
}

static void send_response_packet(AvahiResponseScheduler* s,
                                 AvahiResponseJob* rj)
{
    AvahiDnsPacket* p;
    unsigned n;
    ...
    else
    {
        ...
        if (!(p = avahi_dns_packet_new_response(
                  size + AVAHI_DNS_PACKET_EXTRA_SIZE, 1)))
            return; /* OOM */
...
static AvahiResponseJob* find_scheduled_job(AvahiResponseScheduler* s,
                                            AvahiRecord* record)
{
    AvahiResponseJob* rj;
...
static AvahiResponseJob* find_history_job(AvahiResponseScheduler* s,
                                          AvahiRecord* record)
{
    AvahiResponseJob* rj;
    ...
    for (rj = s->history; rj; rj = rj->jobs_next)
    {
        ...
        if (avahi_record_equal_no_ttl(rj->record, record))
        {
            ...
            /*             avahi_log_debug("history age: %u", (unsigned)
             * (avahi_age(&rj->delivery)/1000)); */
...
static AvahiResponseJob* find_suppressed_job(AvahiResponseScheduler* s,
                                             AvahiRecord* record,
                                             const AvahiAddress* querier)
{
    AvahiResponseJob* rj;
...
int avahi_response_scheduler_post(AvahiResponseScheduler* s,
                                  AvahiRecord* record, int flush_cache,
                                  const AvahiAddress* querier, int immediately)
{
    AvahiResponseJob* rj;
    struct timeval tv;
    /*     char *t; */
    ...
    if (querier && (rj = find_suppressed_job(s, record, querier)) &&
        avahi_record_is_goodbye(record) ==
            avahi_record_is_goodbye(rj->record) &&
        rj->record->ttl >= record->ttl / 2)
    {

        /*         avahi_log_debug("Response suppressed by known answer
         * suppression.");  */
        return 0;
    }
    ...
    if ((rj = find_history_job(s, record)))
    {
        ...
        {
            /*             avahi_log_debug("Response suppressed by local
    ...
    avahi_elapse_time(&tv, immediately ? 0 : AVAHI_RESPONSE_DEFER_MSEC,
                      immediately ? 0 : AVAHI_RESPONSE_JITTER_MSEC);

    if ((rj = find_scheduled_job(s, record)))
    {
        /*          avahi_log_debug("Response suppressed by local duplicate
        ...
        if (!querier || (rj->querier_valid &&
                         avahi_address_cmp(querier, &rj->querier) != 0))
            rj->querier_valid = 0;
    ...
    else
    {
        ...
        rj->delivery = tv;
        rj->time_event = avahi_time_event_new(
            s->time_event_queue, &rj->delivery, elapse_callback, rj);
        rj->flush_cache = flush_cache;
...
void avahi_response_scheduler_incoming(AvahiResponseScheduler* s,
                                       AvahiRecord* record, int flush_cache)
{
    AvahiResponseJob* rj;
    assert(s);
    ...
    if ((rj = find_scheduled_job(s, record)))
    {
        ...
        if ((!rj->flush_cache ||
             flush_cache) && /* flush cache bit was set correctly */
            avahi_record_is_goodbye(record) ==
                avahi_record_is_goodbye(
                    rj->record) && /* both goodbye packets, or both not */
            record->ttl >= rj->record->ttl / 2)
        { /* sensible TTL */

            /* A matching entry was found, so let's mark it done */
            /*             avahi_log_debug("Response suppressed by distributed
...
void avahi_response_scheduler_suppress(AvahiResponseScheduler* s,
                                       AvahiRecord* record,
                                       const AvahiAddress* querier)
{
    AvahiResponseJob* rj;
    ...
    if ((rj = find_scheduled_job(s, record)))
    {
        ...
        if (rj->querier_valid &&
            avahi_address_cmp(querier, &rj->querier) ==
                0 && /* same originator */
            avahi_record_is_goodbye(record) ==
                avahi_record_is_goodbye(
                    rj->record) && /* both goodbye packets, or both not */
