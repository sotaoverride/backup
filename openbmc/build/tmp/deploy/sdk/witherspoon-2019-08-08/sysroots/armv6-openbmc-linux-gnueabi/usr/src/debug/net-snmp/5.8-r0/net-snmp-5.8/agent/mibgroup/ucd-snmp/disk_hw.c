...
static void disk_free_config(void)
{
    ...
    for (entry = netsnmp_fsys_get_first(); entry != NULL;
...
static void disk_parse_config(const char* token, char* cptr)
{
    char path[STRMAX];
    int minpercent;
    int minspace;
 ...
 *
 */
u_char* var_extensible_disk(struct variable* vp, oid* name, size_t* length,
...
tryAgain:
    ...
    switch (vp->magic)
    {
        ...
        case DISKPERCENTNODE:
            long_ret = _percent(entry->inums_total - entry->inums_avail,
                                entry->inums_total);
            return ((u_char*)(&long_ret));
            ...
            if ((entry->minspace >= 0 && val < entry->minspace &&
                ...
                (entry->minpercent >= 0 &&
                 ...
                 asprintf(&errmsg, "%s: less than %d%% free (= %d%%)",
                          ...
                          _percent(entry->avail, entry->size)) >= 0))
            {
