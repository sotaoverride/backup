...
char* avahi_get_host_name(char* ret_s, size_t size)
{
    ...
    if (strcmp(ret_s, "localhost") == 0 ||
        strncmp(ret_s, "localhost.", 10) == 0)
    {
        *ret_s = 0;
        avahi_log_warn("System host name is set to 'localhost'. This is not a "
                       "suitable mDNS host name, looking for alternatives.");
    }
