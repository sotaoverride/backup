...
GType g_binding_flags_get_type(void)
{
    ...
    if (g_once_init_enter(&g_define_type_id__volatile))
    {
        ...
        GType g_define_type_id = g_flags_register_static(
            g_intern_static_string("GBindingFlags"), values);
...
#define G_BINDING_CLASS(klass)                                                 \
    (G_TYPE_CHECK_CLASS_CAST((klass), G_TYPE_BINDING, GBindingClass))
#define G_IS_BINDING_CLASS(klass)                                              \
    (G_TYPE_CHECK_CLASS_TYPE((klass), G_TYPE_BINDING))
#define G_BINDING_GET_CLASS(obj)                                               \
    (G_TYPE_INSTANCE_GET_CLASS((obj), G_TYPE_BINDING, GBindingClass))
...
static void weak_unbind(gpointer user_data, GObject* where_the_object_was)
{
    ...
    else
    {
        if (binding->source_notify != 0)
            g_signal_handler_disconnect(binding->source,
                                        binding->source_notify);
    ...
    else
    {
        if (binding->target_notify != 0)
            g_signal_handler_disconnect(binding->target,
                                        binding->target_notify);
...
static gboolean default_transform(GBinding* binding, const GValue* value_a,
                                  GValue* value_b,
                                  gpointer user_data G_GNUC_UNUSED)
{
    ...
    if (!g_type_is_a(G_VALUE_TYPE(value_a), G_VALUE_TYPE(value_b)))
    {
        ...
        g_warning("%s: Unable to convert a value of type %s to a "
                  "value of type %s",
                  G_STRLOC, g_type_name(G_VALUE_TYPE(value_a)),
...
static gboolean
    default_invert_boolean_transform(GBinding* binding, const GValue* value_a,
                                     GValue* value_b,
                                     gpointer user_data G_GNUC_UNUSED)
{
    gboolean value;
...
static void on_source_notify(GObject* gobject, GParamSpec* pspec,
                             GBinding* binding)
{
    ...
    g_object_get_property(binding->source, binding->source_pspec->name,
                          &from_value);

    res = binding->transform_s2t(binding, &from_value, &to_value,
    ...
    if (res)
    {
        ...
        g_object_set_property(binding->target, binding->target_pspec->name,
                              &to_value);
...
static void on_target_notify(GObject* gobject, GParamSpec* pspec,
                             GBinding* binding)
{
    GValue from_value = G_VALUE_INIT;
    GValue to_value = G_VALUE_INIT;
    ...
    g_object_get_property(binding->target, binding->target_pspec->name,
                          &from_value);

    res = binding->transform_t2s(binding, &from_value, &to_value,
                                 binding->transform_data);
    if (res)
    {
        binding->is_frozen = TRUE;
        ...
        g_object_set_property(binding->source, binding->source_pspec->name,
                              &to_value);
...
static inline void g_binding_unbind_internal(GBinding* binding,
...
static void g_binding_set_property(GObject* gobject, guint prop_id,
...
static void g_binding_get_property(GObject* gobject, guint prop_id,
...
static void g_binding_constructed(GObject* gobject)
{
    ...
    binding->source_pspec = g_object_class_find_property(
        G_OBJECT_GET_CLASS(binding->source), binding->source_property);
    binding->target_pspec = g_object_class_find_property(
        G_OBJECT_GET_CLASS(binding->target), binding->target_property);
    ...
    source_notify_closure =
        g_cclosure_new(G_CALLBACK(on_source_notify), binding, NULL);
    binding->source_notify = g_signal_connect_closure_by_id(
        binding->source, gobject_notify_signal_id, source_property_detail,
        source_notify_closure, FALSE);
    ...
    if (binding->flags & G_BINDING_BIDIRECTIONAL)
    {
        ...
        target_notify_closure =
            g_cclosure_new(G_CALLBACK(on_target_notify), binding, NULL);
        binding->target_notify = g_signal_connect_closure_by_id(
            binding->target, gobject_notify_signal_id, target_property_detail,
            target_notify_closure, FALSE);
    }
...
static void g_binding_class_init(GBindingClass* klass)
{
    ...
    g_object_class_install_property(
        ...
        g_param_spec_object("source", P_("Source"),
                            P_("The source of the binding"), G_TYPE_OBJECT,
    ...
    g_object_class_install_property(
        ...
        g_param_spec_object("target", P_("Target"),
                            P_("The target of the binding"), G_TYPE_OBJECT,
                            G_PARAM_CONSTRUCT_ONLY | G_PARAM_READWRITE |
                                G_PARAM_STATIC_STRINGS));
    /**
     ...
     */
    g_object_class_install_property(
        gobject_class, PROP_SOURCE_PROPERTY,
        g_param_spec_string("source-property", P_("Source Property"),
                            P_("The property on the source to bind"), NULL,
                            G_PARAM_CONSTRUCT_ONLY | G_PARAM_READWRITE |
                                G_PARAM_STATIC_STRINGS));
    /**
     ...
     */
    g_object_class_install_property(
        gobject_class, PROP_TARGET_PROPERTY,
        g_param_spec_string("target-property", P_("Target Property"),
                            P_("The property on the target to bind"), NULL,
                            G_PARAM_CONSTRUCT_ONLY | G_PARAM_READWRITE |
                                G_PARAM_STATIC_STRINGS));
    /**
     * GBinding:flags:
    ...
    g_object_class_install_property(
        ...
        g_param_spec_flags("flags", P_("Flags"), P_("The binding flags"),
                           G_TYPE_BINDING_FLAGS, G_BINDING_DEFAULT,
                           G_PARAM_CONSTRUCT_ONLY | G_PARAM_READWRITE |
                               G_PARAM_STATIC_STRINGS));
}
...
GBinding* g_object_bind_property_full(
    gpointer source, const gchar* source_property, gpointer target,
    const gchar* target_property, GBindingFlags flags,
    ...
    gpointer user_data, GDestroyNotify notify)
{
    ...
    {
        g_warning(
            "%s: The source object of type %s has no property called '%s'",
            G_STRLOC, G_OBJECT_TYPE_NAME(source), source_property);
        return NULL;
    }

    if (!(pspec->flags & G_PARAM_READABLE))
    {
        g_warning("%s: The source object of type %s has no readable property "
                  "called '%s'",
                  G_STRLOC, G_OBJECT_TYPE_NAME(source), source_property);
        return NULL;
    }
    ...
    {
        g_warning("%s: The source object of type %s has no writable property "
                  "called '%s'",
                  G_STRLOC, G_OBJECT_TYPE_NAME(source), source_property);
        return NULL;
    }
        ...
        !(G_PARAM_SPEC_VALUE_TYPE(pspec) == G_TYPE_BOOLEAN))
    {
        g_warning("%s: The G_BINDING_INVERT_BOOLEAN flag can only be used "
                  ...
                  "is of type '%s'",
                  G_STRLOC, source_property,
                  g_type_name(G_PARAM_SPEC_VALUE_TYPE(pspec)));
        return NULL;
    }

    pspec = g_object_class_find_property(G_OBJECT_GET_CLASS(target),
                                         target_property);
    if (pspec == NULL)
    {
        g_warning(
            "%s: The target object of type %s has no property called '%s'",
            G_STRLOC, G_OBJECT_TYPE_NAME(target), target_property);
        return NULL;
    }
    ...
    if ((pspec->flags & G_PARAM_CONSTRUCT_ONLY) ||
        !(pspec->flags & G_PARAM_WRITABLE))
    {
        g_warning("%s: The target object of type %s has no writable property "
                  "called '%s'",
                  G_STRLOC, G_OBJECT_TYPE_NAME(target), target_property);
        return NULL;
    }
    ...
    {
        g_warning("%s: The target object of type %s has no readable property "
                  "called '%s'",
                  G_STRLOC, G_OBJECT_TYPE_NAME(target), target_property);
        return NULL;
    }
        ...
        !(G_PARAM_SPEC_VALUE_TYPE(pspec) == G_TYPE_BOOLEAN))
    {
        g_warning("%s: The G_BINDING_INVERT_BOOLEAN flag can only be used "
                  ...
                  "is of type '%s'",
                  G_STRLOC, target_property,
                  g_type_name(G_PARAM_SPEC_VALUE_TYPE(pspec)));
        return NULL;
    }

    binding = g_object_new(G_TYPE_BINDING, "source", source, "source-property",
                           source_property, "target", target, "target-property",
                           target_property, "flags", flags, NULL);
...
GBinding* g_object_bind_property(gpointer source, const gchar* source_property,
                                 gpointer target, const gchar* target_property,
                                 GBindingFlags flags)
{
    /* type checking is done in g_object_bind_property_full() */

    return g_object_bind_property_full(source, source_property, target,
                                       target_property, flags, NULL, NULL, NULL,
                                       NULL);
}
...
static gboolean bind_with_closures_transform_to(GBinding* binding,
                                                const GValue* source,
...
static gboolean bind_with_closures_transform_from(GBinding* binding,
                                                  const GValue* source,
                                                  GValue* target, gpointer data)
{
    TransformData* t_data = data;
...
GBinding* g_object_bind_property_with_closures(
    gpointer source, const gchar* source_property, gpointer target,
    const gchar* target_property, GBindingFlags flags, GClosure* transform_to,
    GClosure* transform_from)
{
    ...
    if (transform_to != NULL)
    {
        if (G_CLOSURE_NEEDS_MARSHAL(transform_to))
            g_closure_set_marshal(transform_to,
    ...
    if (transform_from != NULL)
    {
        if (G_CLOSURE_NEEDS_MARSHAL(transform_from))
            g_closure_set_marshal(transform_from,
                                  g_cclosure_marshal_BOOLEAN__BOXED_BOXED);
    ...
    return g_object_bind_property_full(
        ...
        transform_from != NULL ? bind_with_closures_transform_from : NULL, data,
