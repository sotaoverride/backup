...
static OM_uint32 val_canon_name_args(OM_uint32* minor_status,
...
OM_uint32
    KRB5_CALLCONV gss_canonicalize_name(minor_status, input_name, mech_type,
                                        output_name) OM_uint32* minor_status;
const gss_name_t input_name;
const gss_OID mech_type;
gss_name_t* output_name;
{
    ...
    major_status =
        gssint_select_mech_type(minor_status, mech_type, &selected_mech);
    if (major_status != GSS_S_COMPLETE)
        return (major_status);
    ...
    if (output_name)
    {
        ...
        if (in_union->name_type != GSS_C_NULL_OID)
        {
            major_status = generic_gss_copy_oid(
                minor_status, in_union->name_type, &out_union->name_type);
            if (major_status)
            {
    ...
    if (!output_name)
    {
        if (in_union->mech_type)
        {
            (void)gssint_release_internal_name(
                minor_status, in_union->mech_type, &in_union->mech_name);
    ...
    if ((major_status = gssint_import_internal_name(
             minor_status, selected_mech, in_union, &dest_union->mech_name)))
        goto allocation_failure;
...
allocation_failure:
    ...
    else if (!output_name)
    {
        ...
        {
            (void)gssint_release_internal_name(&tmpmin, dest_union->mech_type,
