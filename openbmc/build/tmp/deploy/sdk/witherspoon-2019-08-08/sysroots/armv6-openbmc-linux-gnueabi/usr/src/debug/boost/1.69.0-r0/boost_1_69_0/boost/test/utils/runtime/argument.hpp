...
class argument
{
  public:
    // Constructor
    argument(rtti::id_t value_type) : p_value_type(value_type)
    {
    }
...
class typed_argument : public argument
{
  public:
    ...
    explicit typed_argument(T const& v) :
        argument(rtti::type_id<T>()), p_value(v)
    {
    }
...
class arguments_store
{
  public:
    ...
    /// Returns true if there is an argument corresponding to the specified
    /// parameter name
    ...
    T& get(cstring parameter_name)
    {
        ...
        BOOST_TEST_I_ASSRT(arg->p_value_type == rtti::type_id<T>(),
                           arg_type_mismatch()
                               << "Access with invalid type for argument "
    ...
    {
        m_arguments[parameter_name] =
            argument_ptr(new typed_argument<T>(value));
    }
