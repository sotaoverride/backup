...
int ec_GF2m_simple_group_set_curve(EC_GROUP* group, const BIGNUM* p,
 ...
 */
int ec_GF2m_simple_group_get_curve(const EC_GROUP* group, BIGNUM* p, BIGNUM* a,
                                   BIGNUM* b, BN_CTX* ctx)
{
    int ret = 0;
...
int ec_GF2m_simple_field_mul(const EC_GROUP* group, BIGNUM* r, const BIGNUM* a,
...
int ec_GF2m_simple_field_sqr(const EC_GROUP* group, BIGNUM* r, const BIGNUM* a,
                             BN_CTX* ctx)
{
...
int ec_GF2m_simple_field_div(const EC_GROUP* group, BIGNUM* r, const BIGNUM* a,
                             const BIGNUM* b, BN_CTX* ctx)
{
 ...
 * s:= p, r := 2p
 */
static int ec_GF2m_simple_ladder_pre(const EC_GROUP* group, EC_POINT* r,
                                     EC_POINT* s, EC_POINT* p, BN_CTX* ctx)
{
    ...
    do
    {
        if (!BN_priv_rand(s->Z, BN_num_bits(group->field) - 1, BN_RAND_TOP_ANY,
                          BN_RAND_BOTTOM_ANY))
        {
    ...
    do
    {
        if (!BN_priv_rand(r->Y, BN_num_bits(group->field) - 1, BN_RAND_TOP_ANY,
                          BN_RAND_BOTTOM_ANY))
        {
            ECerr(EC_F_EC_GF2M_SIMPLE_LADDER_PRE, ERR_R_BN_LIB);
            return 0;
 ...
 * s := r + s, r := 2r
 */
static int ec_GF2m_simple_ladder_step(const EC_GROUP* group, EC_POINT* r,
                                      EC_POINT* s, EC_POINT* p, BN_CTX* ctx)
{
 ...
 * Appendix Alg Mxy.
 */
static int ec_GF2m_simple_ladder_post(const EC_GROUP* group, EC_POINT* r,
                                      EC_POINT* s, EC_POINT* p, BN_CTX* ctx)
{
    int ret = 0;
    ...
    if (BN_is_zero(s->Z))
    {
        if (!EC_POINT_copy(r, p) || !EC_POINT_invert(group, r, ctx))
        {
    ...
    if (!group->meth->field_mul(group, t0, r->Z, s->Z, ctx) ||
        ...
        !BN_GF2m_add(r->Y, p->Y, t2) || !BN_one(r->Z))
        goto err;
