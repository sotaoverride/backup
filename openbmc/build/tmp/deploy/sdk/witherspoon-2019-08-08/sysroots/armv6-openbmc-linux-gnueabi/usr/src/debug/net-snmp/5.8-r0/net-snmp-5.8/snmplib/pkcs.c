...
int pkcs_init(void)
{
    ...
    {
        DEBUGMSGTL(
            ("pkcs_init", "C_Initialize failed: %s", pkcserr_string(rv)));
    ...
    {
        DEBUGMSGTL(
            ("pkcs_init", "C_GetSlotList failed: %s", pkcserr_string(rv)));
    ...
    if (pSlotList == NULL_PTR || pSlot == NULL_PTR ||
 ...
 */
static int free_slots(int majorID, int minorID, void* serverarg,
                      void* clientarg)
{
    int slotcount, i;
 ...
 * mechanism.
 */
static CK_RV get_session_handle(CK_MECHANISM_TYPE mech_type, CK_FLAGS flag,
                                CK_SESSION_HANDLE_PTR sess)
{
    ...
    {
        DEBUGMSGTL(
            ("pkcs_init", "No cryptographic provider for %s", mech_type));
 ...
 */
static CK_RV get_slot_session_handle(netsnmp_pkcs_slot_session* p,
                                     CK_SESSION_HANDLE_PTR sess)
{
    CK_RV rv = CKR_OK;
    if (p == NULL)
    ...
    {
        /* Open a session */
        rv = C_OpenSession(p->sid, CKF_SERIAL_SESSION, NULL_PTR, NULL, &p->hdl);

        if (rv != CKR_OK)
        {
            DEBUGMSGTL(("get_slot_session_handle",
                        "can not open PKCS #11 session: %s",
                        pkcserr_string(rv)));
        }
    }
    *sess = p->hdl;
 ...
 */
int pkcs_sign(CK_MECHANISM_TYPE mech_type, u_char* key, u_int keylen,
              u_char* msg, u_int msglen, u_char* mac, size_t* maclen)
{
    ...
    CK_ATTRIBUTE template[] = {{CKA_CLASS, &class, sizeof(class)},
 ...
 */
int pkcs_digest(CK_MECHANISM_TYPE mech_type, u_char* msg, u_int msglen,
 ...
 */
int pkcs_encrpyt(CK_MECHANISM_TYPE mech_type, u_char* key, u_int keylen,
                 u_char* iv, u_int ivlen, u_char* plaintext, u_int ptlen,
                 u_char* ciphertext, size_t* ctlen)
{
    ...
    if (get_session_handle(mech_type, CKF_ENCRYPT, &hSession) != CKR_OK ||
        hSession == NULL)
    {
 ...
 */
int pkcs_decrpyt(CK_MECHANISM_TYPE mech_type, u_char* key, u_int keylen,
                 u_char* iv, u_int ivlen, u_char* ciphertext, u_int ctlen,
                 u_char* plaintext, size_t* ptlen)
{
    ...
    if (C_DecryptUpdate(hSession, ciphertext, ctlen, plaintext, ptlen) !=
        CKR_OK)
    {
        QUITFUN(SNMPERR_GENERR, pkcs_decrypt_quit);
    }
 ...
 */
int pkcs_generate_Ku(CK_MECHANISM_TYPE mech_type, u_char* passphrase,
                     u_int pplen, u_char* Ku, size_t* kulen)
{
 ...
 * return code.
 */
static char* pkcserr_string(CK_RV rv)
{
    ...
    switch (rv)
    {
        ...
        default:
            /* rv not found */
            snprintf(errstr, sizeof(errstr), "Unknown return code: 0x%x", rv);
            return (errstr);
            break;
    }
}
