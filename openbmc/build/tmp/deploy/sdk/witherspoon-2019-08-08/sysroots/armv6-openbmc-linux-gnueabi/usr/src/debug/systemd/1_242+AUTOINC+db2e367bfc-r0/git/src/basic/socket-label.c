                          ...
                          const char* label)
{
    ...
    {
        r = setsockopt_int(fd, IPPROTO_IPV6, IPV6_V6ONLY,
                           only == SOCKET_ADDRESS_IPV6_ONLY);
        if (r < 0)
            return r;
    }
    ...
    /* Let's trigger an inotify event on the socket node, so that anyone waiting
     * for this socket to be connectable gets notified */
    if (p)
        (void)touch(p);
...
int make_socket_fd(int log_level, const char* address, int type, int flags)
{
    ...
    if (r < 0)
        return log_error_errno(r, "Failed to parse socket address \"%s\": %m",
                               address);

    a.type = type;

    fd = socket_address_listen(&a, type | flags, SOMAXCONN,
                               SOCKET_ADDRESS_DEFAULT, NULL, false, false,
                               false, 0755, 0644, NULL);
