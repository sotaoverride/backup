...
#ifdef NETSNMP_USE_INTERNAL_CRYPTO
        static int SHA1_hmac(const u_char* data, size_t len, u_char* mac,
                             size_t maclen, const u_char* secret,
                             size_t secretlen);

static int MD5_hmac(const u_char* data, size_t len, u_char* mac, size_t maclen,
...
static netsnmp_auth_alg_info _auth_alg_info[] = {
    ...
    {NETSNMP_USMAUTH_HMACMD5, "usmHMACMD5AuthProtocol", usmHMACMD5AuthProtocol,
     OID_LENGTH(usmHMACMD5AuthProtocol), BYTESIZE(SNMP_TRANS_AUTHLEN_HMACMD5),
...
static netsnmp_priv_alg_info _priv_alg_info[] = {
    {USM_CREATE_USER_PRIV_NONE, "usmNoPrivProtocol", usmNoPrivProtocol,
     OID_LENGTH(usmNoPrivProtocol), 0, 0, 0},
#ifndef NETSNMP_DISABLE_DES
    {USM_CREATE_USER_PRIV_DES, "usmDESPrivProtocol", usmDESPrivProtocol,
     OID_LENGTH(usmDESPrivProtocol), BYTESIZE(SNMP_TRANS_PRIVLEN_1DES),
     BYTESIZE(SNMP_TRANS_PRIVLEN_1DES_IV), BYTESIZE(SNMP_TRANS_PRIVLEN_1DES)},
#endif
#ifdef HAVE_AES
    {USM_CREATE_USER_PRIV_AES, "usmAESPrivProtocol", usmAESPrivProtocol,
     OID_LENGTH(usmAESPrivProtocol), BYTESIZE(SNMP_TRANS_PRIVLEN_AES),
     BYTESIZE(SNMP_TRANS_PRIVLEN_AES_IV), 0},
    ...
    {USM_CREATE_USER_PRIV_AES192, "usmAES192PrivProtocol",
     ...
     BYTESIZE(SNMP_TRANS_PRIVLEN_AES192_IV), 0},
    ...
    {USM_CREATE_USER_PRIV_AES256, "usmAES256PrivProtocol",
     ...
     BYTESIZE(SNMP_TRANS_PRIVLEN_AES256_IV), 0},
 ...
 */
netsnmp_priv_alg_info* sc_get_priv_alg_byoid(const oid* privoid, u_int len)
{
    int i = 0;
    ...
    for (; _priv_alg_info[i].type != -1; ++i)
    {
        ...
        if (snmp_oid_compare(_priv_alg_info[i].alg_oid,
                             _priv_alg_info[i].oid_len, privoid, len) == 0)
 ...
 */
netsnmp_auth_alg_info* sc_find_auth_alg_byoid(const oid* authoid, u_int len)
{
    int i = 0;
    ...
    for (; _auth_alg_info[i].type != -1; ++i)
    {
        ...
        if (snmp_oid_compare(_auth_alg_info[i].alg_oid,
                             _auth_alg_info[i].oid_len, authoid, len) == 0)
 ...
 */
int sc_get_auth_maclen(int hashtype)
{
    netsnmp_auth_alg_info* aai;
 ...
 */
int sc_get_proper_auth_length_bytype(int hashtype)
{
    netsnmp_auth_alg_info* aai;
 ...
 */
oid* sc_get_auth_oid(int type, size_t* oid_len)
{
 ...
 */
const char* sc_get_auth_name(int type)
{
    netsnmp_auth_alg_info* ai;
...
netsnmp_feature_child_of(scapi_get_proper_priv_length, netsnmp_unused)
    ...
    int sc_get_proper_priv_length(const oid* privtype, u_int privtype_len)
{
    netsnmp_priv_alg_info* pai;
...
int sc_random(u_char* buf, size_t* buflen)
    ...
    defined(NETSNMP_USE_PKCS11) || defined(NETSNMP_USE_INTERNAL_CRYPTO)
{
    int rval = SNMPERR_SUCCESS;
 ...
 */
int sc_generate_keyed_hash(const oid* authtypeOID, size_t authtypeOIDlen,
                           const u_char* key, u_int keylen,
                           const u_char* message, u_int msglen, u_char* MAC,
                           size_t* maclen)
#if defined(NETSNMP_USE_INTERNAL_MD5) || defined(NETSNMP_USE_OPENSSL) ||       \
    defined(NETSNMP_USE_PKCS11) || defined(NETSNMP_USE_INTERNAL_CRYPTO)
{
...
#ifdef NETSNMP_ENABLE_TESTING_CODE
    {
        int i;
        DEBUGMSG(
            ("sc_generate_keyed_hash", "sc_generate_keyed_hash(): key=0x"));
        for (i = 0; i < keylen; i++)
     ...
     * Sanity check.
     */
    if (!authtypeOID || !key || !message || !MAC || !maclen || (keylen <= 0) ||
        (msglen <= 0) || (*maclen <= 0))
    {
    ...
    {
        if (pkcs_sign(CKM_MD5_HMAC, key, keylen, message, msglen, buf,
                      &buf_len) != SNMPERR_SUCCESS)
        {
            QUITFUN(SNMPERR_GENERR, sc_generate_keyed_hash_quit);
        }
    }
    ...
    {
        if (pkcs_sign(CKM_SHA_1_HMAC, key, keylen, message, msglen, buf,
                      &buf_len) != SNMPERR_SUCCESS)
        {
            QUITFUN(SNMPERR_GENERR, sc_generate_keyed_hash_quit);
        }
 ...
 */
int sc_hash(const oid* hashtype, size_t hashtypelen, const u_char* buf,
 ...
 * SNMPERR_SC_NOT_CONFIGURED    Hash type not supported.
 */
int sc_hash_type(int auth_type, const u_char* buf, size_t buf_len, u_char* MAC,
                 size_t* MAC_len)
#if defined(NETSNMP_USE_INTERNAL_MD5) || defined(NETSNMP_USE_OPENSSL) ||       \
    defined(NETSNMP_USE_PKCS11) || defined(NETSNMP_USE_INTERNAL_CRYPTO)
{
 ...
 */
int sc_check_keyed_hash(const oid* authtypeOID, size_t authtypeOIDlen,
                        const u_char* key, u_int keylen, const u_char* message,
                        u_int msglen, const u_char* MAC, u_int maclen)
#if defined(NETSNMP_USE_INTERNAL_MD5) || defined(NETSNMP_USE_OPENSSL) ||       \
    defined(NETSNMP_USE_PKCS11) || defined(NETSNMP_USE_INTERNAL_CRYPTO)
{
     ...
     * Sanity check.
     */
    if (!authtypeOID || !key || !message || !MAC || (keylen <= 0) ||
        (msglen <= 0) || (maclen <= 0))
    {
 ...
 * successful return.
 */
int sc_encrypt(const oid* privtype, size_t privtypelen, u_char* key,
               u_int keylen, u_char* iv, u_int ivlen, const u_char* plaintext,
...
#if defined(NETSNMP_USE_OPENSSL) || defined(NETSNMP_USE_INTERNAL_CRYPTO)
{
...
#ifdef NETSNMP_ENABLE_TESTING_CODE
    {
        ...
        if (buf != NULL)
        {
            if (sprint_realloc_hexstring(&buf, &buf_len, &out_len, 1, iv,
                                         ivlen))
            {
            ...
            out_len = 0;
            if (sprint_realloc_hexstring(&buf, &buf_len, &out_len, 1, key,
                                         keylen))
            {
            ...
            out_len = 0;
            if (sprint_realloc_hexstring(&buf, &buf_len, &out_len, 1, plaintext,
                                         16))
            {
    ...
    if (NULL == pai || (keylen < pai->proper_length) ||
        (ivlen < pai->iv_length))
    {
    ...
    if (USM_CREATE_USER_PRIV_DES == (pai->type & USM_PRIV_MASK_ALG))
    {
        ...
        { /* copy data into pad block if needed */
            ...
            memset(&pad_block[pad_size - pad], pad,
                   pad); /* filling in padblock */
        }
        ...
        if (pad > 0)
        {
             ...
             */
            DES_ncbc_encrypt(pad_block, ciphertext + plast, pad_size, key_sch,
                             (DES_cblock*)my_iv, DES_ENCRYPT);
...
#elif defined(NETSNMP_USE_PKCS11)
{
    int rval = SNMPERR_SUCCESS, priv_type u_char pkcs_des_key[8];
    netsnmp_priv_alg_info* pai;
    ...
    rval = pkcs_encrpyt(CKM_DES_CBC, pkcs_des_key, sizeof(pkcs_des_key), iv,
                        ivlen, plaintext, ptlen, ciphertext, ctlen);
 ...
 * successful return.
 */
int sc_decrypt(const oid* privtype, size_t privtypelen, u_char* key,
               u_int keylen, u_char* iv, u_int ivlen, u_char* ciphertext,
...
#if defined(NETSNMP_USE_OPENSSL) || defined(NETSNMP_USE_INTERNAL_CRYPTO)
{
...
#ifdef NETSNMP_ENABLE_TESTING_CODE
    {
        ...
        {
            DEBUGMSGTL(("scapi", "decrypt: malloc fail for debug output\n"));
        }
    }
#endif /* NETSNMP_ENABLE_TESTING_CODE */
...
#elif NETSNMP_USE_PKCS11 /* USE PKCS */
{
    ...
    memset(pkcs_des_key, 0, sizeof(pkcs_des_key));
    memcpy(pkcs_des_key, key, sizeof(pkcs_des_key));
    rval = pkcs_decrpyt(CKM_DES_CBC, pkcs_des_key, sizeof(pkcs_des_key), iv,
                        ivlen, ciphertext, ctlen, plaintext, ptlen);
    *ptlen = ctlen;
...
#define MD5_SECRETKEYLEN 16
    ...
    if (secretlen != MD5_SECRETKEYLEN || secret == NULL || mac == NULL ||
        data == NULL || len <= 0 || maclen <= 0)
    {
        /*
...
static int SHA1_hmac(const u_char* data, size_t len, u_char* mac, size_t maclen,
                     const u_char* secret, size_t secretlen)
{
...
#define SHA1_SECRETKEYLEN 20
    ...
    if (secretlen != SHA1_SECRETKEYLEN || secret == NULL || mac == NULL ||
        data == NULL || len <= 0 || maclen <= 0)
    {
        /*
