 ...
 */
    ...
    !defined(BOOST_LOG_WITHOUT_DEFAULT_FACTORIES)
...
BOOST_LOG_ANONYMOUS_NAMESPACE
{
    ...
    //! A special filtering predicate that adopts the string operand to the
    //! attribute value character type
    template <typename RelationT>
    class string_predicate : public RelationT
    {
      private:
      ...
      public:
        ...
        template <typename StringT>
        string_predicate(relation_type const& rel, StringT const& operand) :
            relation_type(rel)
        {
        ...
        result_type operator()(T const& val) const
        {
            ...
            return relation_type::operator()(
                val, fusion::at_key<operand_type>(m_operands));
        }
...
#error Boost.Log: Inconsistent character configuration
      ...
      public:
        string_predicate(relation_type const& rel, string_type const& operand) :
            relation_type(rel), m_operand(operand)
        {
        }
    ...
    class numeric_predicate : public string_predicate<RelationT>
    {
      ...
      public:
        template <typename StringT>
        numeric_predicate(relation_type const& rel,
                          StringT const& string_operand,
                          numeric_type numeric_operand) :
        ...
        result_type operator()(
            std::basic_string<CharT, TraitsT, AllocatorT> const& val) const
        {
        ...
        result_type
            operator()(basic_string_literal<CharT, TraitsT> const& val) const
        {
            return base_type::operator()(val);
        }
    ...
    typedef mpl::vector<BOOST_PP_SEQ_ENUM(
...
filter default_filter_factory<CharT>::on_equality_relation(
    attribute_name const& name, string_type const& arg)
{
...
template <typename CharT>
filter default_filter_factory<CharT>::on_inequality_relation(
    attribute_name const& name, string_type const& arg)
{
...
template <typename CharT>
filter
    default_filter_factory<CharT>::on_less_relation(attribute_name const& name,
...
template <typename CharT>
filter default_filter_factory<CharT>::on_greater_relation(
    attribute_name const& name, string_type const& arg)
{
...
template <typename CharT>
filter default_filter_factory<CharT>::on_less_or_equal_relation(
    attribute_name const& name, string_type const& arg)
{
...
template <typename CharT>
filter default_filter_factory<CharT>::on_greater_or_equal_relation(
    attribute_name const& name, string_type const& arg)
{
...
template <typename CharT>
filter default_filter_factory<CharT>::on_custom_relation(
    attribute_name const& name, string_type const& rel, string_type const& arg)
{
    ...
    if (rel == constants::begins_with_keyword())
    {
        ...
        return predicate_wrapper<log::string_types, predicate>(
            name, predicate(begins_with_fun(), arg));
    }
    else if (rel == constants::ends_with_keyword())
    {
        typedef string_predicate<ends_with_fun> predicate;
        return predicate_wrapper<log::string_types, predicate>(
            name, predicate(ends_with_fun(), arg));
    }
    else if (rel == constants::contains_keyword())
    {
        typedef string_predicate<contains_fun> predicate;
        return predicate_wrapper<log::string_types, predicate>(
            name, predicate(contains_fun(), arg));
    }
    else if (rel != constants::matches_keyword())
    {
        BOOST_LOG_THROW_DESCR(parse_error, "The custom attribute relation \"" +
                                               log::aux::to_narrow(rel) +
                                               "\" is not supported");
    }
...
template <typename CharT>
template <typename RelationT>
filter default_filter_factory<CharT>::parse_argument(attribute_name const& name,
                                                     string_type const& arg)
{
    ...
    if (qi::parse(begin, end, real_, real_val) && begin == end)
    {
        ...
        return predicate_wrapper<value_types, predicate>(
            name, predicate(RelationT(), arg, real_val));
    }
    else
    {
        begin = arg.c_str();
        ...
        if (qi::parse(begin, end, qi::long_, int_val) && begin == end)
        {
            ...
            return predicate_wrapper<value_types, predicate>(
                name, predicate(RelationT(), arg, int_val));
        }
        else
        {
            typedef string_predicate<RelationT> predicate;
            return predicate_wrapper<log::string_types, predicate>(
                name, predicate(RelationT(), arg));
        }
    }
}
