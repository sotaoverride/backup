...
static inline void _elf_dbg(const struct kmod_elf* elf, const char* fname,
                                    ...
                                    uint16_t size)
{
    ...
    {
        ELFDBG(elf,
               "out of bounds: %" PRIu64 " + %" PRIu16 " = %" PRIu64
               "> %" PRIu64 " (ELF size)\n",
...
static inline int elf_set_uint(struct kmod_elf* elf, uint64_t offset,
                               uint64_t size, uint64_t value)
{
    ...
    ELFDBG(elf,
           "size=%" PRIu16 " offset=%" PRIu64 " value=%" PRIu64
           " write memory=%p\n",
...
static inline const void* elf_get_mem(const struct kmod_elf* elf,
                                      uint64_t offset)
{
...
static inline const void* elf_get_section_header(const struct kmod_elf* elf,
                                                 uint16_t idx)
{
...
#undef READV
    ...
    if (elf_get_section_info(
            elf, elf->header.strings.section, &elf->header.strings.offset,
            &elf->header.strings.size, &elf->header.strings.nameoff) < 0)
    {
...
static int kmod_elf_crc_find(const struct kmod_elf* elf, const void* versions,
                             uint64_t versionslen, const char* name,
                             uint64_t* crc)
{
