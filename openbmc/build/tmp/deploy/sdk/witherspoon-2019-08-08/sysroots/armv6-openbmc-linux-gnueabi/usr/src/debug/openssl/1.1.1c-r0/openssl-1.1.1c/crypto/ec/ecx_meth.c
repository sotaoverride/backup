...
static int ecx_priv_encode(PKCS8_PRIV_KEY_INFO* p8, const EVP_PKEY* pkey)
{
    ...
    if (!PKCS8_pkey_set0(p8, OBJ_nid2obj(pkey->ameth->pkey_id), 0, V_ASN1_UNDEF,
                         NULL, penc, penclen))
    {
                         ...
                         ASN1_PCTX* ctx, ecx_key_op_t op)
{
    ...
    if (op == KEY_OP_PRIVATE)
    {
        ...
        if (BIO_printf(bp, "%*spriv:\n", indent, "") <= 0)
            return 0;
        if (ASN1_buf_print(bp, ecxkey->privkey, KEYLEN(pkey), indent + 4) == 0)
            return 0;
    }
    else
    {
    ...
    if (ASN1_buf_print(bp, ecxkey->pubkey, KEYLEN(pkey), indent + 4) == 0)
        return 0;
    return 1;
}
...
static int validate_ecx_derive(EVP_PKEY_CTX* ctx, unsigned char* key,
                               size_t* keylen, const unsigned char** privkey,
                                ...
                                size_t* keylen)
{
    ...
    if (!validate_ecx_derive(ctx, key, keylen, &privkey, &pubkey) ||
        (key != NULL && X25519(key, privkey, pubkey) == 0))
        return 0;
                              ...
                              size_t* keylen)
{
    ...
    if (!validate_ecx_derive(ctx, key, keylen, &privkey, &pubkey) ||
        (key != NULL && X448(key, privkey, pubkey) == 0))
        return 0;
    *keylen = X448_KEYLEN;
    return 1;
}
...
const EVP_PKEY_METHOD ecx25519_pkey_meth = {EVP_PKEY_X25519,
                                            0,
                                            0,
                                            0,
                                            0,
...
const EVP_PKEY_METHOD ecx448_pkey_meth = {EVP_PKEY_X448,
                                          0,
                                          0,
                                          0,
                                          0,
                                  ...
                                  size_t tbslen)
{
    ...
    if (ED448_sign(sig, tbs, tbslen, edkey->pubkey, edkey->privkey, NULL, 0) ==
        0)
        return 0;
    *siglen = ED448_SIGSIZE;
    return 1;
...
const EVP_PKEY_METHOD ed25519_pkey_meth = {EVP_PKEY_ED25519,
