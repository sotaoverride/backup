...
template <typename T>
struct use_directive<qi::domain, terminal_ex<tag::skip // enables skip(s)[p]
                                             ,
                                             fusion::vector1<T>>>
...
struct use_lazy_directive<qi::domain, tag::skip, 1 // arity
                          > : mpl::true_
{
};
} // namespace spirit
...
struct reskip_parser : unary_parser<reskip_parser<Subject>>
{
    ...
    struct attribute
    {
        typedef typename traits::attribute_of<Subject, Context, Iterator>::type
            type;
    };

    reskip_parser(Subject const& subject_) : subject(subject_)
    {
    }
    ...
    typename enable_if<detail::is_unused_skipper<Skipper>, bool>::type
        parse(Iterator& first, Iterator const& last, Context& context,
              Skipper const& u // --> The skipper is reintroduced
              ,
              Attribute& attr_) const
    {
        return subject.parse(first, last, context, detail::get_skipper(u),
                             attr_);
    }
    template <typename Iterator, typename Context, typename Skipper,
              typename Attribute>
...
struct skip_parser : unary_parser<skip_parser<Subject, Skipper>>
{
    ...
    template <typename Iterator, typename Context, typename Skipper_,
              typename Attribute>
...
struct make_directive<terminal_ex<tag::skip, fusion::vector1<Skipper>>, Subject,
                      Modifiers>
{
    typedef typename result_of::compile<qi::domain, Skipper, Modifiers>::type
        skipper_type;
                           ...
                           Modifiers const& modifiers) const
    {
        return result_type(subject, compile<qi::domain>(
                                        fusion::at_c<0>(term.args), modifiers));
    }
};
...
template <typename Subject, typename Attribute, typename Context,
          typename Iterator>
struct handles_container<qi::reskip_parser<Subject>, Attribute, Context,
                         Iterator>
...
struct handles_container<qi::skip_parser<Subject, Skipper>, Attribute, Context,
                         Iterator>
    : unary_handles_container<Subject, Attribute, Context, Iterator>
{
};
