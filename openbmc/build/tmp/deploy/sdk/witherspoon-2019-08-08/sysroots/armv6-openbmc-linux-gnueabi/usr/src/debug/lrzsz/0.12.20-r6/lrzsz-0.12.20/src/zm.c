...
void zshhdr(int type, char* hdr)
{
    ...
    VPRINTF(3, ("zshhdr: %s %lx", frametypes[(type & 0x7f) + FTOFFSET],
                rclhdr(hdr)));
    s[0] = ZPAD;
    s[1] = ZPAD;
    s[2] = ZDLE;
...
static void printout_blocksizes(void)
{
    ...
    if (blocksizes[i].count)
    {
        lsyslog(LOG_DEBUG, "unk. byte: %ld blocks", blocksizes[i].count);
    }
}
            ...
            switch (c)
            {
                ...
                case GOTCRCW:
                    ...
                    *bytes_received = length - (end - buf);
                    COUNT_BLK(*bytes_received);
                    VPRINTF(3,
                            ("zrdat32: %lu %s", (unsigned long)*bytes_received,
                             Zendnames[(d - GOTCRCE) & 3]));
                    return d;
                case GOTCAN:
                    zperr(_("Sender Canceled"));
...
fifi:
    switch (c)
    {
        ...
        default:
            if (c >= -3 && c <= FRTYPES)
                VPRINTF(3, ("zgethdr: %s %lx", frametypes[c + FTOFFSET],
                            (unsigned long)rxpos));
            else
