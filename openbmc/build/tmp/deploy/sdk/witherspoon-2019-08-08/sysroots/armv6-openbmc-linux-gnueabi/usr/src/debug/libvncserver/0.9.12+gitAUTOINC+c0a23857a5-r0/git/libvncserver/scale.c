...
/****************************/
#define CEIL(x)                                                                \
    ((double)((int)(x)) == (x) ? (double)((int)(x)) : (double)((int)(x) + 1))
 ...
 */
void rfbScaledCorrection(rfbScreenInfoPtr from, rfbScreenInfoPtr to, int* x,
                         int* y, int* w, int* h, const char* function)
{
     ...
     * from, to, from->width, from->height, to->width, to->height, *x, *y, *w,
     * *h);
     */
    ...
    /*
     * rfbLog("%s (%dXx%dY-%dWx%dH  ->  %fXx%fY-%fWx%fH) {%dWx%dH ->
     * %dWx%dH}\n", function, *x, *y, *w, *h, x2, y2, w2, h2, from->width,
     * from->height, to->width, to->height);
     */
...
void rfbScaledScreenUpdateRect(rfbScreenInfoPtr screen, rfbScreenInfoPtr ptr,
                               int x0, int y0, int w0, int h0)
{
    ...
    rfbScaledCorrection(screen, ptr, &x1, &y1, &w1, &h1,
    ...
    srcptr = (unsigned char*)(screen->frameBuffer +
                              (y0 * screen->paddedWidthInBytes +
                               x0 * bytesPerPixel));
    dstptr = (unsigned char*)(ptr->frameBuffer + (y1 * ptr->paddedWidthInBytes +
                                                  x1 * bytesPerPixel));
    ...
    /*
     * rfbLog("rfbScaledScreenUpdateRect(%dXx%dY-%dWx%dH  ->  %dXx%dY-%dWx%dH
     * <%dx%d>) {%dWx%dH -> %dWx%dH} 0x%p\n", x0, y0, w0, h0, x1, y1, w1, h1,
     * areaX, areaY, screen->width, screen->height, ptr->width, ptr->height,
     * ptr->frameBuffer);
     */

    if (screen->serverFormat.trueColour)
    { /* Blend neighbouring pixels together */
            ...
            for (x = 0; x < w1; x++)
            {
                ...
                for (w = 0; w < areaX; w++)
                {
                    for (v = 0; v < areaY; v++)
                    {
                        ...
                        {
                            case 4:
                                pixel_value = *((unsigned int*)srcptr2);
                                break;
                            case 2:
                                pixel_value = *((unsigned short*)srcptr2);
                                break;
                            case 1:
                                pixel_value = *((unsigned char*)srcptr2);
                                break;
                            default:
                ...
                pixel_value = ((red & redMax) << redShift) |
    ...
    { /* Not truecolour, so we can't blend. Just use the top-left pixel instead
       */
        ...
        {
            for (x = x1; x < (x1 + w1); x++)
                memcpy(&ptr->frameBuffer[(y * ptr->paddedWidthInBytes) +
                                         (x * bytesPerPixel)],
                       &screen->frameBuffer[(y * areaY *
                                             screen->paddedWidthInBytes) +
                                            (x * areaX * bytesPerPixel)],
                       bytesPerPixel);
        }
    }
}
...
void rfbScaledScreenUpdate(rfbScreenInfoPtr screen, int x1, int y1, int x2,
                           int y2)
{
    /* ok, now the task is to update each and every scaled version of the
     * framebuffer and we only have to do this for this specific changed
     * rectangle!
     */
    ...
    for (ptr = screen->scaledScreenNext; ptr != NULL;
...
rfbScreenInfoPtr rfbScaledScreenAllocate(rfbClientPtr cl, int width, int height)
{
    ...
    if (ptr != NULL)
    {
         ...
         */
        allocSize =
            pad4(width * (ptr->bitsPerPixel /
                          8)); /* per protocol, width<2**16 and bpp<256 */
        if (height == 0 || allocSize >= SIZE_MAX / height)
        {
            free(ptr);
            return NULL; /* malloc() will allocate an incorrect buffer size -
                            early abort */
        }
        ...
        if (ptr->frameBuffer != NULL)
        {
            ...
            rfbScaledScreenUpdateRect(cl->screen, ptr, 0, 0, cl->screen->width,
                                      cl->screen->height);
...
{
    rfbScreenInfoPtr ptr;
    /* include the original in the search (ie: fine 1:1 scaled version of the
     * frameBuffer) */
...
int rfbSendNewScaleSize(rfbClientPtr cl)
{
    ...
    if (cl->PalmVNC == TRUE)
    {
        ...
        rfbLog("Sending a response to a PalmVNC style frameuffer resize event "
               "(%dx%d)\n",
               cl->scaledScreen->width, cl->scaledScreen->height);
        if (rfbWriteExact(cl, (char*)&pmsg, sz_rfbPalmVNCReSizeFrameBufferMsg) <
            0)
        {
    ...
    else
    {
        ...
        rfbLog("Sending a response to a UltraVNC style frameuffer resize event "
               "(%dx%d)\n",
               cl->scaledScreen->width, cl->scaledScreen->height);
