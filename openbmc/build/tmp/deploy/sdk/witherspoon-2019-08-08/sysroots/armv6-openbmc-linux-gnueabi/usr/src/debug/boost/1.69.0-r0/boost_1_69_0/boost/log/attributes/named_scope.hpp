 ...
 * \date   24.06.2007
 *
 * The header contains implementation of named scope container and an attribute
 * that allows to put the named scope to log. A number of convenience macros are
 * also provided.
 */
...
struct named_scope_list_node
{
    ...
    named_scope_list_node() BOOST_NOEXCEPT
    {
        _m_pPrev = _m_pNext = this;
    }
};
 ...
 *
 * The named scope entries are stored as elements of \c basic_named_scope_list
 * container, which in turn can be acquired either from the \c basic_named_scope
...
//! \endcond
{
    ...
    {
        general, //!< The scope name contains some unstructured string that
                 //!< should not be interpreted by the library
    ...
    named_scope_entry(string_literal const& sn, string_literal const& fn,
                      unsigned int ln,
                      scope_name_type t = general) BOOST_NOEXCEPT
        : scope_name(sn),
          file_name(fn),
          line(ln),
          type(t)
...
//! \endcond
{
  ...
  protected:
    ...
    class iter
    {
      ...
      public:
        ...
        typedef typename mpl::if_c<fConstV, named_scope_list::const_reference,
                                   named_scope_list::reference>::type reference;
        typedef typename mpl::if_c<fConstV, named_scope_list::const_pointer,
                                   named_scope_list::pointer>::type pointer;
  ...
  protected:
    ...
    /*!
     * A constant iterator to the sequence of scopes. Complies to bidirectional
     * iterator requirements.
     */
    typedef implementation_defined const_iterator;
    /*!
     * An iterator to the sequence of scopes. Complies to bidirectional iterator
     * requirements.
     */
    typedef implementation_defined iterator;
    /*!
     * A constant reverse iterator to the sequence of scopes. Complies to
     * bidirectional iterator requirements.
     */
    typedef implementation_defined const_reverse_iterator;
    /*!
     * A reverse iterator to the sequence of scopes. Complies to bidirectional
     * iterator requirements.
     */
  ...
  public:
    ...
    const_iterator end() const
    {
        return const_iterator(
            const_cast<aux::named_scope_list_node*>(&m_RootNode));
    }
    /*!
    ...
    /*!
     * \return Constant iterator to the before-the-first element of the
     * container.
     */
...
/*!
 * \brief A class of an attribute that holds stack of named scopes of the
 * current thread
 *
 * The basic_named_scope attribute is essentially a hook to the thread-specific
 * instance of scope list. This means that the attribute will generate different
 * values if get_value is called in different threads. The attribute generates
 * value with stored type <tt>basic_named_scope_list< CharT ></tt>.
 *
 * The attribute class can also be used to gain access to the scope stack
 * instance, e.g. to get its copy or to push or pop a scope entry. However, it
 * is highly not recommended to maintain scope list manually. Use \c
 * BOOST_LOG_NAMED_SCOPE or \c BOOST_LOG_FUNCTION macros instead.
 */
class BOOST_LOG_API named_scope : public attribute
{
  public:
    ...
    {
        /*!
         * Constructor. Pushes the specified scope to the end of the
         * thread-local list of scopes.
         *
         * \param sn Scope name.
         * \param fn File name, in which the scope is located.
         * \param ln Line number in the file.
         */
        sentry(string_literal const& sn, string_literal const& fn,
               unsigned int ln,
               scope_entry::scope_name_type t = scope_entry::general)
            BOOST_NOEXCEPT : m_Entry(sn, fn, ln, t)
        {
            named_scope::push_scope(m_Entry);
        }

        /*!
         * Destructor. Removes the last pushed scope from the thread-local list
         * of scopes.
         */
  ...
  public:
    ...
    /*!
     * The method pushes the scope to the back of the current thread's scope
     * list
     *
     * \b Throws: Nothing.
     */
    ...
    /*!
     * The method pops the last pushed scope from the current thread's scope
     * list
     *
     * \b Throws: Nothing.
     */
     ...
     *
     *  \note The returned reference is only valid until the current thread
     * ends. The scopes in the returned container may change if the execution
     * scope is changed (i.e. either \c push_scope or \c pop_scope is called).
     * User has to copy the stack if he wants to keep it intact regardless of
     * the execution scope.
     */
...
#define BOOST_LOG_NAMED_SCOPE_INTERNAL(var, name, file, line, type)            \
    BOOST_LOG_UNUSED_VARIABLE(::boost::log::attributes::named_scope::sentry,   \
                              var, (name, file, line, type));

#endif // BOOST_LOG_DOXYGEN_PASS

/*!
 * Macro for scope markup. The specified scope name is pushed to the end of the
 * current thread scope list.
 */
#define BOOST_LOG_NAMED_SCOPE(name)                                            \
    BOOST_LOG_NAMED_SCOPE_INTERNAL(                                            \
        ...
        ::boost::log::attributes::named_scope_entry::general)
...
/*!
 * Macro for function scope markup. The scope name is constructed with help of
 * compiler and contains the current function name. It may be shorter than what
 * \c BOOST_LOG_FUNCTION macro produces. The scope name is pushed to the end of
 * the current thread scope list.
 *
 * Not all compilers have support for this macro. The exact form of the scope
 * name may vary from one compiler to another.
 */
