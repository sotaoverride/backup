                    ...
                    uint32_t count_bytes)
{
    ...
    low_mem.flash_offset =
        align_down(flash_offset, 1 << context->backend.erase_size_shift);
    ...
    if (low_mem.flash_offset < context->current->flash_offset)
    {
        ...
        rc = backend_copy(&context->backend, low_mem.flash_offset, low_mem.mem,
                          low_mem.size);
        if (rc < 0)
        {
            goto out;
    ...
    if (low_mem.mem)
    {
        ...
        rc = backend_write(&context->backend, low_mem.flash_offset, low_mem.mem,
                           low_mem.size);
        if (rc < 0)
        {
            goto out;
                 ...
                 uint8_t type)
{
    ...
    switch (type)
    {
        case WINDOW_ERASED: /* >= V2 ONLY -> block_size == erasesize */
            flash_offset = context->current->flash_offset + offset_bytes;
            rc = backend_erase(&context->backend, flash_offset, count_bytes);
            if (rc < 0)
            {
                MSG_ERR("Couldn't erase flash\n");
                return rc;
        ...
        case WINDOW_DIRTY:
            ...
            {
                return window_flush_v1(context, offset_bytes, count_bytes);
            }
            flash_offset = context->current->flash_offset + offset_bytes;
