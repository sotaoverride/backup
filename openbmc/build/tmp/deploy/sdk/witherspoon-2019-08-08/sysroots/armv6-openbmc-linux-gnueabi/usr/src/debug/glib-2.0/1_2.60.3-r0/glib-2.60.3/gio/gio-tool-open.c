...
static void launch_default_for_uri_cb(GObject* source_object, GAsyncResult* res,
                                      gpointer user_data)
{
    GError* error = NULL;
    gchar* uri = user_data;
...
int handle_open(int argc, char* argv[], gboolean do_help)
{
    ...
    g_option_context_set_summary(
        context, _("Open files with the default application that\n"
    ...
    for (i = 1; i < argc; i++)
    {
        ...
        g_app_info_launch_default_for_uri_async(
            uri, NULL, NULL, launch_default_for_uri_cb, g_strdup(uri));
