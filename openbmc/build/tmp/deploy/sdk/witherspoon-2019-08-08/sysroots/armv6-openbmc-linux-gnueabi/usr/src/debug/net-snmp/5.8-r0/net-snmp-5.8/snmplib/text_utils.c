...
netsnmp_feature_child_of(text_utils, libnetsnmp)
     ...
     * parse methods
     */
    static void _pm_save_index_string_string(FILE* f, netsnmp_container* cin,
                                             int flags);
static void _pm_save_everything(FILE* f, netsnmp_container* cin, int flags);

static void _pm_user_function(FILE* f, netsnmp_container* cin,
                              netsnmp_line_process_info* lpi, int flags);

/*
 * line processors
 */
static int _process_line_tvi(netsnmp_line_info* line_info, void* mem,
 ...
 */
netsnmp_container* netsnmp_file_text_parse(netsnmp_file* f,
                                           netsnmp_container* cin,
                                           int parse_mode, u_int flags,
                                           void* context)
{
...
netsnmp_feature_child_of(text_token_container_from_file, netsnmp_unused)
    ...
    netsnmp_container* netsnmp_text_token_container_from_file(
        const char* file, u_int flags, netsnmp_container* cin, void* context)
{
                             ...
                             struct netsnmp_line_process_info_s* lpi)
{
    ...
    {
        DEBUGMSGTL(
            ("text:util:tvi", "no value after token '%s'\n", line_info->start));
    ...
    *(ptr++) = 0;
    ...
    switch ((int)(intptr_t)lpi->user_context)
    {
        ...
        case PMLP_TYPE_INTEGER:
            ...
            if ((errno == ERANGE) &&
                ((LONG_MAX == tvi->value.sl) || (LONG_MIN == tvi->value.sl)))
