...
class counting_sink : boost::noncopyable
{
  public:
    counting_sink(OutputIterator& sink_, std::size_t count_ = 0,
                  bool enabled = true) :
        count(count_),
        initial_count(count), prev_count(0), sink(sink_)
    {
...
struct counting_policy
{
  public:
    ...
    counting_sink<OutputIterator>*
        chain_counting(counting_sink<OutputIterator>* count_data)
    {
...
struct make_output_iterator
{
    ...
    typedef typename mpl::if_<traits::not_is_unused<Derived>, Derived,
                              output_iterator<OutputIterator, Properties,
                                              Derived>>::type most_derived_type;
    ...
    : public make_output_iterator<OutputIterator, Properties, Derived>::type
{
  private:
    // base iterator type
    typedef
        typename make_output_iterator<OutputIterator, Properties, Derived>::type
            base_iterator;
  ...
  public:
    ...
    explicit output_iterator(OutputIterator& sink_) : sink(&sink_)
    {
    }
...
class output_iterator<karma::ostream_iterator<T, Elem, Traits>, Properties>
    : public output_iterator<
          karma::ostream_iterator<T, Elem, Traits>, Properties,
          output_iterator<karma::ostream_iterator<T, Elem, Traits>, Properties>>
{
  ...
  public:
    output_iterator(base_iterator_type& sink) : base_type(sink)
    {
    }

    ostream_type& get_ostream()
    {
        return (*this->sink).get_ostream();
    }
    ...
    bool good() const
    {
        return (*this->sink).get_ostream().good();
    }
};
...
{
    disable_counting(OutputIterator& sink_) : count_data(sink_, 0, false)
    {
    }
...
struct enable_buffering
{
    enable_buffering(OutputIterator& sink_,
        ...
        sink(sink_),
        prev_buffer(NULL), enabled(false)
    {
