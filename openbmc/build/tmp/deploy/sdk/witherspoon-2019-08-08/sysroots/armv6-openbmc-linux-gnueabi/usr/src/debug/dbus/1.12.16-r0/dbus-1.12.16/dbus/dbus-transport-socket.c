...
static void free_watches(DBusTransport* transport)
{
    ...
    if (socket_transport->read_watch)
    {
        if (transport->connection)
            _dbus_connection_remove_watch_unlocked(
                transport->connection, socket_transport->read_watch);
    ...
    if (socket_transport->write_watch)
    {
        if (transport->connection)
            _dbus_connection_remove_watch_unlocked(
                transport->connection, socket_transport->write_watch);
...
static void check_write_watch(DBusTransport* transport)
{
    ...
    if (_dbus_transport_try_to_authenticate(transport))
        needed = _dbus_connection_has_messages_to_send_unlocked(
    ...
    _dbus_verbose(
        "check_write_watch(): needed = %d on connection %p watch %p fd = "
        "%" DBUS_SOCKET_FORMAT " outgoing messages exist %d\n",
    ...
    _dbus_connection_toggle_watch_unlocked(
        transport->connection, socket_transport->write_watch, needed);
...
static void check_read_watch(DBusTransport* transport)
{
    ...
    if (_dbus_transport_try_to_authenticate(transport))
        need_read_watch =
            (_dbus_counter_get_size_value(transport->live_messages) <
             transport->max_live_messages_size) &&
            (_dbus_counter_get_unix_fd_value(transport->live_messages) <
    ...
    _dbus_connection_toggle_watch_unlocked(
        transport->connection, socket_transport->read_watch, need_read_watch);
...
static dbus_bool_t read_data_into_auth(DBusTransport* transport,
                                       dbus_bool_t* oom)
{
    DBusTransportSocket* socket_transport = (DBusTransportSocket*)transport;
    DBusString* buffer;
    ...
    *oom = FALSE;
    ...
    bytes_read =
        _dbus_read_socket(socket_transport->fd, buffer,
...
static dbus_bool_t write_data_from_auth(DBusTransport* transport)
{
    ...
    if (!_dbus_auth_get_bytes_to_send(transport->auth, &buffer))
        return FALSE;

    bytes_written = _dbus_write_socket(socket_transport->fd, buffer, 0,
...
/* FALSE on OOM */
static dbus_bool_t exchange_credentials(DBusTransport* transport,
...
static dbus_bool_t do_authentication(DBusTransport* transport,
                                     dbus_bool_t do_reading,
                                     dbus_bool_t do_writing,
                                     dbus_bool_t* auth_completed)
{
           ...
           _dbus_transport_get_is_connected(transport))
    {
        ...
        if (transport->send_credentials_pending ||
            transport->receive_credentials_pending)
        {
            _dbus_verbose("send_credentials_pending = %d "
                          "receive_credentials_pending = %d\n",
        ...
        _dbus_connection_has_messages_to_send_unlocked(transport->connection))
    {
      ...
      _dbus_verbose ("writing message %p\n", message);
        ...
        if (_dbus_auth_needs_encoding(transport->auth))
        {
            ...
            if (_dbus_string_get_length(&socket_transport->encoded_outgoing) ==
                0)
            {
                if (!_dbus_auth_encode_data(
            ...
            total_bytes_to_write =
                _dbus_string_get_length(&socket_transport->encoded_outgoing);
        ...
        else
        {
          ...
          _dbus_verbose ("message is %d bytes\n",
            ...
            if (socket_transport->message_bytes_written <= 0 &&
                DBUS_TRANSPORT_CAN_SEND_UNIX_FD(transport))
            {
                ...
                bytes_written = _dbus_write_socket_with_unix_fds_two(
                    socket_transport->fd, header,
                    socket_transport->message_bytes_written,
                    header_len - socket_transport->message_bytes_written, body,
                    0, body_len, unix_fds, n);
                saved_errno = _dbus_save_socket_errno();
...
#endif
            {
                ...
                {
                    bytes_written = _dbus_write_socket(
                        socket_transport->fd, body,
                        (socket_transport->message_bytes_written - header_len),
                        body_len - (socket_transport->message_bytes_written -
                                    header_len));
                }
        ...
        if (bytes_written < 0)
        {
            ...
            if (_dbus_get_is_errno_eagain_or_ewouldblock(saved_errno) ||
                _dbus_get_is_errno_epipe(saved_errno))
                goto out;
...
again:
    ...
    if (_dbus_auth_needs_decoding(transport->auth))
    {
        ...
        if (_dbus_string_get_length(&socket_transport->encoded_incoming) > 0)
            bytes_read =
                _dbus_string_get_length(&socket_transport->encoded_incoming);
        else
            bytes_read = _dbus_read_socket(
                socket_transport->fd, &socket_transport->encoded_incoming,
                socket_transport->max_bytes_read_per_iteration);

        saved_errno = _dbus_save_socket_errno();

        _dbus_assert(_dbus_string_get_length(
                         &socket_transport->encoded_incoming) == bytes_read);

        if (bytes_read > 0)
        {
            _dbus_message_loader_get_buffer(transport->loader, &buffer, NULL,
                                            NULL);
    ...
    else
    {
        ...
        if (DBUS_TRANSPORT_CAN_SEND_UNIX_FD(transport) && may_read_unix_fds)
        {
            ...
            if (!_dbus_message_loader_get_unix_fds(transport->loader, &fds,
                                                   &n_fds))
            {
            ...
            bytes_read = _dbus_read_socket_with_unix_fds(
                socket_transport->fd, buffer, max_to_read, fds, &n_fds);
            saved_errno = _dbus_save_socket_errno();

            if (bytes_read >= 0 && n_fds > 0)
                _dbus_verbose("Read %i unix fds\n", n_fds);

            _dbus_message_loader_return_unix_fds(transport->loader, fds,
                                                 bytes_read < 0 ? 0 : n_fds);
        }
        else
#endif
        {
            bytes_read =
                _dbus_read_socket(socket_transport->fd, buffer, max_to_read);
            saved_errno = _dbus_save_socket_errno();
        }
    ...
    else
    {
        ...
        {
            oom = TRUE;
            _dbus_verbose(" out of memory when queueing messages we just read "
                          "in the transport\n");
            goto out;
        }
...
static dbus_bool_t unix_error_with_read_to_come(DBusTransport* itransport,
                                                DBusWatch* watch,
                                                unsigned int flags)
{
...
static dbus_bool_t socket_handle_watch(DBusTransport* transport,
                                       DBusWatch* watch, unsigned int flags)
{
    ...
    /* If we hit an error here on a write watch, don't disconnect the transport
     * yet because data can still be in the buffer and do_reading may need
     * several iteration to read it all (because of its
     * max_bytes_read_per_iteration limit).
     */
    if (!(flags & DBUS_WATCH_READABLE) &&
        unix_error_with_read_to_come(transport, watch, flags))
    {
    ...
    if (watch == socket_transport->read_watch && (flags & DBUS_WATCH_READABLE))
    {
        ...
        {
            _dbus_verbose("Not reading anything since we just completed the "
                          "authentication\n");
        }
    }
 ...
 * messages).
 */
static void socket_do_iteration(DBusTransport* transport, unsigned int flags,
                                int timeout_milliseconds)
{
    ...
    _dbus_verbose(" iteration flags = %s%s timeout = %d read_watch = %p "
                  "write_watch = %p fd = %" DBUS_SOCKET_FORMAT "\n",
                  flags & DBUS_ITERATION_DO_READING ? "read" : "",
                  flags & DBUS_ITERATION_DO_WRITING ? "write" : "",
                  timeout_milliseconds, socket_transport->read_watch,
                  socket_transport->write_watch,
                  _dbus_socket_printable(socket_transport->fd));
    ...
    again:
        ...
        if (poll_res >= 0)
        {
            if (poll_res == 0)
                poll_fd.revents =
                    0; /* some concern that posix does not guarantee this;
...
static dbus_bool_t socket_get_socket_fd(DBusTransport* transport,
                                        DBusSocket* fd_p)
{
    DBusTransportSocket* socket_transport = (DBusTransportSocket*)transport;
...
static const DBusTransportVTable socket_vtable = {
    socket_finalize,       socket_handle_watch, socket_disconnect,
    socket_connection_set, socket_do_iteration, socket_live_messages_changed,
 ...
 * @param server_guid non-#NULL if this transport is on the server side of a
 * connection
                                              ...
                                              const DBusString* address)
{
    ...
    socket_transport->write_watch =
        _dbus_watch_new(_dbus_socket_get_pollable(fd), DBUS_WATCH_WRITABLE,
                        FALSE, NULL, NULL, NULL);
    if (socket_transport->write_watch == NULL)
        goto failed_2;

    socket_transport->read_watch =
        _dbus_watch_new(_dbus_socket_get_pollable(fd), DBUS_WATCH_READABLE,
                        FALSE, NULL, NULL, NULL);
    if (socket_transport->read_watch == NULL)
        goto failed_3;

    if (!_dbus_transport_init_base(&socket_transport->base, &socket_vtable,
                                   server_guid, address))
        goto failed_4;

#ifdef HAVE_UNIX_FD_PASSING
    _dbus_auth_set_unix_fd_possible(socket_transport->base.auth,
                                    _dbus_socket_can_pass_unix_fd(fd));
#endif
                                                  ...
                                                  DBusError* error)
{
    ...
    if (family != NULL && (!_dbus_string_append(&address, ",family=") ||
                           !_dbus_string_append(&address, family)))
        goto error;

    if (noncefile != NULL && (!_dbus_string_append(&address, ",noncefile=") ||
                              !_dbus_string_append(&address, noncefile)))
        goto error;

    fd = _dbus_connect_tcp_socket_with_nonce(host, port, family, noncefile,
                                             error);
                                                    ...
                                                    DBusError* error)
{
    ...
    if (isTcp || isNonceTcp)
    {
        ...
        const char* noncefile =
            dbus_address_entry_get_value(entry, "noncefile");
        ...
        *transport_p = _dbus_transport_new_for_tcp_socket(host, port, family,
                                                          noncefile, error);
        if (*transport_p == NULL)
        {
            _DBUS_ASSERT_ERROR_IS_SET(error);
