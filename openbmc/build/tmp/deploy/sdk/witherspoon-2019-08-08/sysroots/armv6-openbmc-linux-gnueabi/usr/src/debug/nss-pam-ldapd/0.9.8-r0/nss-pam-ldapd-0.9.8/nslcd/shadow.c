...
static int mkfilter_shadow_byname(const char* name, char* buffer, size_t buflen)
{
    ...
    /* build filter */
    return mysnprintf(buffer, buflen, "(&%s(%s=%s))", shadow_filter,
...
#define GET_OPTIONAL_LONG(var, att, fallback)                                  \
    tmpvalue =                                                                 \
        attmap_get_value(entry, attmap_shadow_##att, buffer, sizeof(buffer));  \
    if (tmpvalue == NULL)                                                      \
        tmpvalue = "";                                                         \
    errno = 0;                                                                 \
                           ...
                           unsigned long* flag)
{
    ...
    char* tmp;
    /* get lastchange date */
    tmpvalue = attmap_get_value(entry, attmap_shadow_shadowLastChange, buffer,
                                sizeof(buffer));
    if (tmpvalue == NULL)
        tmpvalue = "";
    *lastchangedate =
        to_date(myldap_get_dn(entry), tmpvalue, attmap_shadow_shadowLastChange);
                        ...
                        uid_t calleruid)
{
    ...
    /* get password */
    passwd = get_userpassword(entry, attmap_shadow_userPassword, passbuffer,
                              sizeof(passbuffer));
                               ...
                               int* rcp)
{
    ...
    for (i = 0;
         (i < NSS_LDAP_CONFIG_MAX_BASES) && ((base = shadow_bases[i]) != NULL);
         i++)
    {
        search = myldap_search(session, base, shadow_scope, filter,
                               shadow_attrs, rcp);
        if (search == NULL)
        {
...
NSLCD_HANDLE_UID(
    shadow, byname, NSLCD_ACTION_SHADOW_BYNAME, char name[BUFLEN_NAME];
...
NSLCD_HANDLE_UID(shadow, all, NSLCD_ACTION_SHADOW_ALL, const char* filter;
