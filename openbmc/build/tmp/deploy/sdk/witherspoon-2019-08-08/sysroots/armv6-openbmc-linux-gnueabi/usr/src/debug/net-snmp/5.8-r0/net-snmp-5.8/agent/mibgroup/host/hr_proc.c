...
struct variable4 hrproc_variables[] = {
    {HRPROC_ID, ASN_OBJECT_ID, NETSNMP_OLDAPI_RONLY, var_hrproc, 2, {1, 1}},
    {HRPROC_LOAD, ASN_INTEGER, NETSNMP_OLDAPI_RONLY, var_hrproc, 2, {1, 2}}};
...
int header_hrproc(struct variable* vp, oid* name, size_t* length, int exact,
...
#define HRPROC_ENTRY_NAME_LENGTH 11
    ...
    for (;;)
    {
        ...
        if ((!exact && (result < 0)) && (LowIndex == -1 || proc_idx < LowIndex))
        {
            LowIndex = proc_idx;
            /*
             * Save processor status information
...
u_char* var_hrproc(struct variable* vp, oid* name, size_t* length, int exact,
                   size_t* var_len, WriteMethod** write_method)
{
    int proc_idx;
