...
struct standard_wide
{
        ...
        return (std::size_t(0) ==
                ...
                std::size_t(~0) ==
                    std::size_t(ch |
                                traits::wchar_t_size<sizeof(wchar_t)>::mask))
                   ? true
                   : false; // any wchar_t, but no other bits set
    }
    ...
    {
        using namespace std;
        return isupper(ch) ? to_char_type<wchar_t>(towlower(to_int_type(ch)))
                           : ch;
    }
    ...
    {
        using namespace std;
        return islower(ch) ? to_char_type<wchar_t>(towupper(to_int_type(ch)))
                           : ch;
    }
