...
#define XFS_MIN_DBLOCKS(s)                                                     \
    ((uint64_t)((s)->sb_agcount - 1) * (s)->sb_agblocks + XFS_MIN_AG_BLOCKS)
...
static int xfs_verify_sb(struct xfs_super_block* ondisk)
{
    ...
    if (sbp->sb_agcount <= 0 || sbp->sb_sectsize < XFS_MIN_SECTORSIZE ||
        ...
        sbp->sb_dblocks == 0 || sbp->sb_dblocks > XFS_MAX_DBLOCKS(sbp) ||
                         ...
                         __attribute__((__unused__)))
{
    ...
    for (i = 0; i < 512; i++)
    {
        ...
        if (xlog_valid_rec_header(rhead))
        {
            ...
            if (blkid_probe_set_magic(pr, i * 512, sizeof(rhead->h_magicno),
