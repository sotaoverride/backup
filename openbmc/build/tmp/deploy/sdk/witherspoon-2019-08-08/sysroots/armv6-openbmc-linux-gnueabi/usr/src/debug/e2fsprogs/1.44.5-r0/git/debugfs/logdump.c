...
static void dump_descriptor_block(FILE*, struct journal_source*, char*,
                                  journal_superblock_t*, unsigned int*, int,
                                  tid_t);

static void dump_revoke_block(FILE*, char*, journal_superblock_t*, unsigned int,
                              int, tid_t);

static void dump_metadata_block(FILE*, struct journal_source*,
                                journal_superblock_t*, unsigned int,
...
#define WRAP(jsb, blocknr)                                                     \
    if (blocknr >= be32_to_cpu((jsb)->s_maxlen))                               \
        blocknr -= (be32_to_cpu((jsb)->s_maxlen) - be32_to_cpu((jsb)->s_first));
...
void do_logdump(int argc, char** argv)
{
    ...
    while ((c = getopt(argc, argv, "ab:ci:f:OsS")) != EOF)
    {
        switch (c)
        {
            ...
            case 'b':
                ...
                if (*tmp)
                {
                    com_err(argv[0], 0, "Bad block number - %s", optarg);
                    return;
                }
                dump_descriptors = 0;
                break;
    ...
    if (inode_spec)
    {
        ...
        inode_group = ((inode_to_dump - 1) / es->s_inodes_per_group);
        group_offset = ((inode_to_dump - 1) % es->s_inodes_per_group);
        inodes_per_block = (current_fs->blocksize / sizeof(struct ext2_inode));

        inode_block_to_dump = ext2fs_inode_table_loc(current_fs, inode_group) +
                              (group_offset / inodes_per_block);
        inode_offset_to_dump =
            ((group_offset % inodes_per_block) * sizeof(struct ext2_inode));
    ...
    else
    {
        ...
        if (!out_file)
        {
            com_err(argv[0], errno, "while opening %s for logdump", out_fn);
            goto errout;
        }
    }
    ...
    if (journal_fn)
    {
        ...
        if (journal_fd < 0)
        {
            com_err(argv[0], errno, "while opening %s for logdump", journal_fn);
            goto errout;
        }
    ...
    else if ((journal_inum = es->s_journal_inum))
    {
        if (use_sb)
        {
            ...
            {
                com_err(argv[0], 0, "no journal backup in super block\n");
                goto errout;
            }
        ...
        retval = ext2fs_file_open2(current_fs, journal_inum, &journal_inode, 0,
                                   &journal_file);
        if (retval)
        {
                              ...
                              ext2_loff_t offset, char* buf, unsigned int size)
{
    ...
    {
        retval = ext2fs_file_llseek(source->file, offset, EXT2_SEEK_SET, NULL);
        if (retval)
        {
        seek_err:
                         ...
                         struct journal_source* source)
{
    ...
    if ((be32_to_cpu(jsb->s_header.h_magic) != JFS_MAGIC_NUMBER) &&
        (sb->s_magic == EXT2_SUPER_MAGIC) && ext2fs_has_feature_journal_dev(sb))
    {
    ...
    retval = read_journal_block(
        cmdname, source, ((ext2_loff_t)blocknr) * blocksize, jsb_buffer, 1024);
    if (retval)
        return;

    if (dump_super)
    {
        e2p_list_journal_super(out_file, jsb_buffer, current_fs->blocksize, 0);
        fputc('\n', out_file);
    }
    ...
    if (be32_to_cpu(jsb->s_header.h_magic) != JFS_MAGIC_NUMBER)
    {
        fprintf(out_file, "Journal superblock magic number invalid!\n");
        return;
    }
    ...
    while (1)
    {
        retval = read_journal_block(cmdname, source,
                                    ((ext2_loff_t)blocknr) * blocksize, buf,
                                    blocksize);
        if (retval)
            return;
        ...
        {
            case JFS_DESCRIPTOR_BLOCK:
                dump_descriptor_block(out_file, source, buf, jsb, &blocknr,
                                      blocksize, transaction);
                continue;
            ...
            case JFS_REVOKE_BLOCK:
                dump_revoke_block(out_file, buf, jsb, blocknr, blocksize,
                                  transaction);
                blocknr++;
                WRAP(jsb, blocknr);
                continue;
...
static void dump_descriptor_block(FILE* out_file, struct journal_source* source,
                                  char* buf, journal_superblock_t* jsb,
                                  ...
                                  tid_t transaction)
{
    ...
    do
    {
        ...
        dump_metadata_block(out_file, source, jsb, blocknr, tag_block,
                            tag_flags, blocksize, transaction);
                        ...
                        __u32 first_block)
{
    if (start_extent >= 0 && first_block != 0)
        fprintf(out_file, "(%d+%u): %u ", start_extent,
                end_extent - start_extent, first_block);
}
...
static void dump_metadata_block(FILE* out_file, struct journal_source* source,
                                ...
                                unsigned int log_tag_flags, int blocksize,
                                tid_t transaction)
{
    int retval;
    char buf[8192];

    if (!(dump_all || (fs_blocknr == block_to_dump) ||
    ...
    if (!(dump_contents && dump_all) && fs_blocknr != block_to_dump &&
        fs_blocknr != bitmap_to_dump && fs_blocknr != inode_block_to_dump)
        return;

    retval = read_journal_block("logdump", source,
                                ((ext2_loff_t)log_blocknr) * blocksize, buf,
                                blocksize);
    if (retval)
        return;
