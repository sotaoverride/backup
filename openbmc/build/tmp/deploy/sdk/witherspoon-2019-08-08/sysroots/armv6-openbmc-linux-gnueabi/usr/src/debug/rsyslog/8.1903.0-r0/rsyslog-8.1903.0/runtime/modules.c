...
/* static data */
DEFobjStaticHelpers DEFobjCurrIf(strgen)

    static modInfo_t* pLoadedModules =
        NULL; /* list of currently-loaded modules */
...
typedef rsRetVal (*pModInit_t)(int, int*, rsRetVal (**)(), rsRetVal (*)(),
                               modInfo_t*);

/* we provide a set of dummy functions for modules that do not support the
 * some interfaces.
 * On the commit feature: As the modules do not support it, they commit each
 * message they receive, and as such the dummies can always return RS_RET_OK
 * without causing harm. This simplifies things as in action processing we do
 * not need to check if the transactional entry points exist.
 */
...
static rsRetVal dummyIsCompatibleWithFeature(__attribute__((unused))
                                             syslogFeature eFeat)
{
...
static rsRetVal dummynewActInst(uchar* modName,
                                ...
                                omodStringRequest_t __attribute__((unused)) *
                                    *dummy3)
{
...
#ifdef DEBUG
/* we add some home-grown support to track our users (and detect who does not
 * free us). In the long term, this should probably be migrated into debug.c
 * (TODO). -- rgerhards, 2008-03-11
 */
...
static void modUsrDel(modInfo_t* pThis, const char* pszUsr)
{
    ...
    if (pUsr == NULL)
    {
        dbgprintf("oops - tried to delete user %s from module %s and it wasn't "
                  "registered as one...\n",
...
/* print all loaded modules and who is accessing them. This is primarily
 * intended to be called at end of run to detect "module leaks" and who is
 * causing them. rgerhards, 2008-03-11
 */
static void modUsrPrintAll(void)
{
    ...
    {
        dbgprintf("printing users of loadable module %s, refcount %u, ptr %p, "
                  "type %d\n",
 ...
 */
static void addModToGlblList(modInfo_t* pThis)
{
    assert(pThis != NULL);
 ...
 */
rsRetVal readyModForCnf(modInfo_t* pThis, cfgmodules_etry_t** ppNew,
                        cfgmodules_etry_t** ppLast)
{
        ...
        { /* loop broken inside */
            if (pLast->pMod == pThis)
            {
                DBGPRINTF("module '%s' already in this config\n",
                          modGetName(pThis));
                if (strncmp((char*)modGetName(pThis),
                            "builtin:", sizeof("builtin:") - 1))
                {
                    LogError(
                        0, RS_RET_MODULE_ALREADY_IN_CONF,
     ...
     * pass it a pointer which it can populate with a pointer to its module
     * conf.
     */
 ...
 */
rsRetVal ATTR_NONNULL(1) addModToCnfList(cfgmodules_etry_t** const pNew,
                                         cfgmodules_etry_t* const pLast)
{
    DEFiRet;
    assert(*pNew != NULL);
 ...
 */
static cfgmodules_etry_t* GetNxtCnfType(rsconf_t* cnf, cfgmodules_etry_t* node,
                                        eModType_t rqtdType)
{
    if (node == NULL)
    { /* start at beginning of module list */
    ...
    if (rqtdType != eMOD_ANY)
    { /* if any, we already have the right one! */
 ...
 */
static modInfo_t* FindWithCnfName(rsconf_t* cnf, uchar* name,
                                  eModType_t rqtdType)
{
...
static rsRetVal modPrepareUnload(modInfo_t* pThis)
{
    ...
    {
        dbgprintf(
            "rejecting unload of module '%s' because it has a refcount of %d\n",
 ...
 */
static rsRetVal doModInit(pModInit_t modInit, uchar* name, void* pModHdlr,
                          modInfo_t** pNewModule)
{
    rsRetVal localRet;
    modInfo_t* pNew = NULL;
    uchar* pName;
    ...
    CHKiRet((*modInit)(CURR_MOD_IF_VERSION, &pNew->iIFVers,
                       &pNew->modQueryEtryPt, queryHostEtryPt, pNew));
    ...
    /* We now poll the module to see what type it is. We do this only once as
     * this can never change in the lifetime of an module. -- rgerhards,
     * 2007-12-14
     */
    ...
    localRet = (*pNew->modQueryEtryPt)((uchar*)"isCompatibleWithFeature",
                                       &pNew->isCompatibleWithFeature);
    ...
    localRet =
        (*pNew->modQueryEtryPt)((uchar*)"beginCnfLoad", &pNew->beginCnfLoad);
    if (localRet == RS_RET_OK)
    {
        dbgprintf("module %s supports rsyslog v6 config interface\n", name);
        CHKiRet(
            (*pNew->modQueryEtryPt)((uchar*)"endCnfLoad", &pNew->endCnfLoad));
        CHKiRet((*pNew->modQueryEtryPt)((uchar*)"freeCnf", &pNew->freeCnf));
        CHKiRet((*pNew->modQueryEtryPt)((uchar*)"checkCnf", &pNew->checkCnf));
        CHKiRet(
            (*pNew->modQueryEtryPt)((uchar*)"activateCnf", &pNew->activateCnf));
        localRet = (*pNew->modQueryEtryPt)((uchar*)"activateCnfPrePrivDrop",
                                           &pNew->activateCnfPrePrivDrop);
        if (localRet == RS_RET_MODULE_ENTRY_POINT_NOT_FOUND)
        {
    ...
    {
        case eMOD_IN:
            CHKiRet((*pNew->modQueryEtryPt)((uchar*)"runInput",
                                            &pNew->mod.im.runInput));
            CHKiRet((*pNew->modQueryEtryPt)((uchar*)"willRun",
                                            &pNew->mod.im.willRun));
            CHKiRet((*pNew->modQueryEtryPt)((uchar*)"afterRun",
                                            &pNew->mod.im.afterRun));
            pNew->mod.im.bCanRun = 0;
            localRet = (*pNew->modQueryEtryPt)((uchar*)"newInpInst",
                                               &pNew->mod.im.newInpInst);
            if (localRet == RS_RET_MODULE_ENTRY_POINT_NOT_FOUND)
            {
            ...
            if (localRet != RS_RET_OK &&
                localRet != RS_RET_MODULE_ENTRY_POINT_NOT_FOUND)
                ABORT_FINALIZE(localRet);

            break;
        case eMOD_OUT:
            CHKiRet((*pNew->modQueryEtryPt)((uchar*)"freeInstance",
                                            &pNew->freeInstance));
            CHKiRet((*pNew->modQueryEtryPt)((uchar*)"dbgPrintInstInfo",
                                            &pNew->dbgPrintInstInfo));
            CHKiRet((*pNew->modQueryEtryPt)((uchar*)"parseSelectorAct",
                                            &pNew->mod.om.parseSelectorAct));
            CHKiRet(
                (*pNew->modQueryEtryPt)((uchar*)"tryResume", &pNew->tryResume));
            ...
            localRet =
                (*pNew->modQueryEtryPt)((uchar*)"doHUPWrkr", &pNew->doHUPWrkr);
            if (localRet != RS_RET_OK &&
                localRet != RS_RET_MODULE_ENTRY_POINT_NOT_FOUND)
                ABORT_FINALIZE(localRet);

            localRet =
                (*pNew->modQueryEtryPt)((uchar*)"SetShutdownImmdtPtr",
            ...
            localRet = (*pNew->modQueryEtryPt)((uchar*)"beginTransaction",
                                               &pNew->mod.om.beginTransaction);
            if (localRet == RS_RET_MODULE_ENTRY_POINT_NOT_FOUND)
            {
            ...
            localRet = (*pNew->modQueryEtryPt)((uchar*)"newActInst",
                                               &pNew->mod.om.newActInst);
            if (localRet == RS_RET_MODULE_ENTRY_POINT_NOT_FOUND)
            {
        ...
        case eMOD_PARSER:
            localRet =
                (*pNew->modQueryEtryPt)((uchar*)"parse2", &pNew->mod.pm.parse2);
            if (localRet == RS_RET_OK)
            {
            ...
            else if (localRet == RS_RET_MODULE_ENTRY_POINT_NOT_FOUND)
            {
                ...
                CHKiRet((*pNew->modQueryEtryPt)((uchar*)"parse",
                                                &pNew->mod.pm.parse));
            }
            else
            {
            ...
            break;
        case eMOD_STRGEN:
            /* first, we need to obtain the strgen object. We could not do that
             * during init as that would have caused class bootstrap issues
             * which are not absolutely necessary. Note that we can call
             * objUse() multiple times, it handles that.
             */
            CHKiRet(objUse(strgen, CORE_COMPONENT));
            CHKiRet((*pNew->modQueryEtryPt)((uchar*)"strgen",
                                            &pNew->mod.sm.strgen));
            ...
            break;
        case eMOD_FUNCTION:
            CHKiRet((*pNew->modQueryEtryPt)((uchar*)"getFunctArray",
                                            &pNew->mod.fm.getFunctArray));
            int version;
    ...
    pNew->pszName = (uchar*)strdup(
        (char*)name); /* we do not care if strdup() fails, we can accept that */
    ...
    /* we initialized the structure, now let's add it to the linked list of
     * modules */
...
static rsRetVal modUnlinkAndDestroy(modInfo_t** ppThis)
{
    ...
    if (pThis->eType == eMOD_LIB)
    {
        if (pThis->uRefCnt > 0)
        {
            dbgprintf("module %s NOT unloaded because it still has a refcount "
                      "of %u\n",
                      pThis->pszName, pThis->uRefCnt);
            ABORT_FINALIZE(RS_RET_MODULE_STILL_REFERENCED);
        }
    ...
    /* we need to unlink the module before we can destruct it -- rgerhards,
     * 2008-02-26 */
    if (pThis->pPrev == NULL)
    {
...
static rsRetVal modUnloadAndDestructAll(eModLinkType_t modLinkTypesToUnload)
{
    ...
    while (pModCurr != NULL)
    {
        if (modLinkTypesToUnload == eMOD_LINK_ALL ||
            pModCurr->eLinkType == modLinkTypesToUnload)
        {
            if (modUnlinkAndDestroy(&pModCurr) ==
                RS_RET_MODULE_STILL_REFERENCED)
            {
                pModCurr = GetNxt(pModCurr);
            }
            else
            {
                /* Note: if the module was successfully unloaded, it has updated
                 * the pModCurr pointer to the next module. However, the unload
                 * process may still have indirectly referenced the pointer list
                 * in a way that the unloaded module is not aware of. So we
                 * restart the unload process to make sure we do not fall into a
                 * trap (what we did ;)). The performance toll is minimal. --
...
static rsRetVal findModule(uchar* pModName, int iModNameLen, modInfo_t** pMod)
{
    ...
    while (pModInfo != NULL)
    {
        if (!strncmp((char*)pModName,
                     (char*)(pModNameCmp = modGetName(pModInfo)),
                     iModNameLen) &&
            (!*(pModNameCmp + iModNameLen) ||
             !strcmp((char*)pModNameCmp + iModNameLen, ".so")))
        {
 ...
 * rgerhards, 2007-09-25: please note that the non-threadsafe function dlerror()
 * is called below. This is ok because modules are currently only loaded during
 * configuration file processing, which is executed on a single thread. Should
 * we change that design at any stage (what is unlikely), we need to find a
 * replacement.
 * rgerhards, 2011-04-27:
 * Parameter "bConfLoad" tells us if the load was triggered by a config handler,
 * in which case we need to tie the loaded module to the current config. If
 * bConfLoad == 0, the system loads a module for internal reasons, this is not
 * directly tied to a configuration. We could also think if it would be useful
 * to add only certain types of modules, but the current implementation at least
 * looks simpler. Note: pvals = NULL means legacy config system
 */
...
#define PATHBUF_OVERHEAD 1 + iModNameLen + 3 + 1
                             ...
                             "builtin:", sizeof("builtin:") - 1))
                {
                    if (pModInfo->bSetModCnfCalled)
                    {
                        LogError(0, RS_RET_DUP_PARAM,
                                 "parameters for built-in module %s already "
                                 "set - ignored\n",
                                 pModName);
    ...
    { /* now build our load module name */
        if (*pModName == '/' || *pModName == '.')
        {
            if (lenPathBuf < PATHBUF_OVERHEAD)
            {
                if (pPathBuf != pathBuf) /* already malloc()ed memory? */
                    free(pPathBuf);
                /* we always alloc enough memory for everything we potentiall
                 * need to add */
                lenPathBuf = PATHBUF_OVERHEAD;
                CHKmalloc(pPathBuf = malloc(lenPathBuf));
            }
            *pPathBuf = '\0'; /* we do not need to append the path - its already
                                 in the module name */
            iPathLen = 0;
        }
        else
        ...
        if (!bHasExtension)
        {
             ...
             */
            strncat((char*)pPathBuf, ".so",
                    lenPathBuf - strlen((char*)pPathBuf) - 1);
        }
    ...
    if (!pModHdlr)
    {
        LogError(0, RS_RET_MODULE_LOAD_ERR_DLOPEN,
                 "could not load module '%s', errors: %s", pModName,
                 (load_err_msg == NULL)
                     ? "NONE SEEN???"
                     : (const char*)cstrGetSzStrNoNULL(load_err_msg));
    ...
    if ((iRet = doModInit(pModInit, (uchar*)pModName, pModHdlr, &pModInfo)) !=
        RS_RET_OK)
    {
 ...
 * which case any previous value is deleted but no new one set. The
 * caller-provided string is duplicated. If it needs to be freed, that's the
 * caller's duty. rgerhards, 2008-03-07
 */
...
/* Reference-Counting object access: add 1 to the current reference count. Must
 * be called by anyone interested in using a module. -- rgerhards, 20080-03-10
 */
static rsRetVal Use(const char* srcFile, modInfo_t* pThis)
{
    ...
    assert(pThis != NULL);
    pThis->uRefCnt++;
    dbgprintf("source file %s requested reference for module '%s', reference "
              "count now %u\n",
...
static rsRetVal Release(const char* srcFile, modInfo_t** ppThis)
{
    ...
    if (pThis->uRefCnt == 0)
    {
        ...
        dbgprintf("internal error: module '%s' already has a refcount of 0 "
                  "(released by %s)!\n",
    ...
    if (pThis->uRefCnt == 0)
    {
        ...
        /* we must NOT do a *ppThis = NULL, because ppThis now points into freed
         * memory! If in doubt, see obj.c::ReleaseObj() for how we are called.
         */
    }
 ...
 * rgerhards, 2008-03-11
 */
BEGINObjClassExit(module,
                  OBJ_IS_LOADABLE_MODULE) /* CHANGE class also in END MACRO! */
    CODESTARTObjClassExit(module)
    /* release objects we no longer need */
    free(pModDir);
#ifdef DEBUG
modUsrPrintAll(); /* debug aid - TODO: integrate with debug.c, at least the
                     settings! */
#endif
ENDObjClassExit(module)
    ...
    BEGINobjQueryInterface(module)
        CODESTARTobjQueryInterface(module) if (pIf->ifVersion !=
                                               moduleCURR_IF_VERSION)
...
pIf->Use = Use;
pIf->Release = Release;
finalize_it : ENDobjQueryInterface(module)
              ...
              BEGINAbstractObjClassInit(
                  module, 1, OBJ_IS_CORE_MODULE) /* class, version - CHANGE
                                                    class also in END MACRO! */
              uchar* pModPath;
