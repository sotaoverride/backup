...
#define is_cpu_online(cpu)                                                     \
    (CPU_ISSET_S((cpu), CPU_ALLOC_SIZE(maxcpus), onlinecpus))
 ...
 *          > 0 = partial success
 */
static int cpu_enable(struct path_cxt* sys, cpu_set_t* cpu_set, size_t setsize,
                      int enable)
{
    int cpu;
    int online, rc;
 ...
 *          > 0 = partial success
 */
static int cpu_configure(struct path_cxt* sys, cpu_set_t* cpu_set,
                         size_t setsize, int configure)
{
    int cpu;
    int rc, current;
    int fails = 0;
    ...
    for (cpu = 0; cpu < maxcpus; cpu++)
    {
        ...
        if (current == 1 && configure == 0 && onlinecpus && is_cpu_online(cpu))
        {
...
int main(int argc, char* argv[])
{
    ...
    return rc == 0 ? EXIT_SUCCESS : rc < 0 ? EXIT_FAILURE : CHCPU_EXIT_SOMEOK;
}
