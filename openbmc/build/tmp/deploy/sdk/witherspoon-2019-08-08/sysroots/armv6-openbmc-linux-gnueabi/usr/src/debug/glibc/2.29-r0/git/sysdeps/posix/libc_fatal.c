...
static bool writev_for_fatal(int fd, const struct iovec* iov, size_t niov,
                             size_t total)
{
...
static void before_abort(int do_abort __attribute__((unused)),
    ...
    if (nlist > 0)
    {
        ...
        if ((action & do_abort))
        {
            total = ((total + 1 + GLRO(dl_pagesize) - 1) &
                     ~(GLRO(dl_pagesize) - 1));
            struct abort_msg_s* buf =
                __mmap(NULL, total, PROT_READ | PROT_WRITE,
