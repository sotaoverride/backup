...
static const OPT_PAIR dsa_choices[] = {
    {"dsa512", R_DSA_512}, {"dsa1024", R_DSA_1024}, {"dsa2048", R_DSA_2048}};
...
static const OPT_PAIR rsa_choices[] = {
    {"rsa512", R_RSA_512},    {"rsa1024", R_RSA_1024}, {"rsa2048", R_RSA_2048},
    {"rsa3072", R_RSA_3072},  {"rsa4096", R_RSA_4096}, {"rsa7680", R_RSA_7680},
...
#define R_EC_Ed448 1
static OPT_PAIR eddsa_choices[] = {{"ed25519", R_EC_Ed25519},
                                   {"ed448", R_EC_Ed448}};
...
static int EVP_Digest_RMD160_loop(void* args)
{
    ...
    {
        if (!EVP_Digest(buf, (size_t)lengths[testnum], &(rmd160[0]), NULL,
                        EVP_ripemd160(), NULL))
            return -1;
    }
    return count;
...
static int DES_ncbc_encrypt_loop(void* args)
{
    ...
    for (count = 0; COND(c[D_CBC_DES][testnum]); count++)
        DES_ncbc_encrypt(buf, buf, lengths[testnum], &sch, &DES_iv,
                         DES_ENCRYPT);
    return count;
}
...
static int DES_ede3_cbc_encrypt_loop(void* args)
{
    ...
    for (count = 0; COND(c[D_EDE3_DES][testnum]); count++)
        DES_ede3_cbc_encrypt(buf, buf, lengths[testnum], &sch, &sch2, &sch3,
                             &DES_iv, DES_ENCRYPT);
    return count;
}
#endif
...
static int AES_cbc_128_encrypt_loop(void* args)
{
    ...
    for (count = 0; COND(c[D_CBC_128_AES][testnum]); count++)
        AES_cbc_encrypt(buf, buf, (size_t)lengths[testnum], &aes_ks1, iv,
                        AES_ENCRYPT);
    return count;
}
...
static int AES_cbc_192_encrypt_loop(void* args)
{
    ...
    for (count = 0; COND(c[D_CBC_192_AES][testnum]); count++)
        AES_cbc_encrypt(buf, buf, (size_t)lengths[testnum], &aes_ks2, iv,
                        AES_ENCRYPT);
    return count;
}
...
static int AES_cbc_256_encrypt_loop(void* args)
{
    ...
    for (count = 0; COND(c[D_CBC_256_AES][testnum]); count++)
        AES_cbc_encrypt(buf, buf, (size_t)lengths[testnum], &aes_ks3, iv,
                        AES_ENCRYPT);
    return count;
}
...
static int AES_ige_128_encrypt_loop(void* args)
{
    ...
    for (count = 0; COND(c[D_IGE_128_AES][testnum]); count++)
        AES_ige_encrypt(buf, buf2, (size_t)lengths[testnum], &aes_ks1, iv,
                        AES_ENCRYPT);
    return count;
}
...
static int AES_ige_192_encrypt_loop(void* args)
{
    ...
    for (count = 0; COND(c[D_IGE_192_AES][testnum]); count++)
        AES_ige_encrypt(buf, buf2, (size_t)lengths[testnum], &aes_ks2, iv,
                        AES_ENCRYPT);
    return count;
}
...
static int AES_ige_256_encrypt_loop(void* args)
{
    ...
    for (count = 0; COND(c[D_IGE_256_AES][testnum]); count++)
        AES_ige_encrypt(buf, buf2, (size_t)lengths[testnum], &aes_ks3, iv,
                        AES_ENCRYPT);
    return count;
}
...
static int EVP_Update_loop_aead(void* args)
{
    ...
    if (decrypt)
    {
        ...
        {
            EVP_DecryptInit_ex(ctx, NULL, NULL, NULL, iv);
            EVP_CIPHER_CTX_ctrl(ctx, EVP_CTRL_AEAD_SET_TAG, sizeof(faketag),
                                faketag);
...
int speed_main(int argc, char** argv)
{
    ...
    openssl_speed_sec_t seconds = {SECONDS,       RSA_SECONDS,  DSA_SECONDS,
                                   ECDSA_SECONDS, ECDH_SECONDS, EdDSA_SECONDS};
...
#endif
#ifndef OPENSSL_NO_RSA
    static const unsigned int rsa_bits[RSA_NUM] = {512,  1024, 2048, 3072,
                                                   4096, 7680, 15360};
    static const unsigned char* rsa_data[RSA_NUM] = {
        test512, test1024, test2048, test3072, test4096, test7680, test15360};
    static const int rsa_data_length[RSA_NUM] = {
        sizeof(test512),  sizeof(test1024), sizeof(test2048), sizeof(test3072),
        sizeof(test4096), sizeof(test7680), sizeof(test15360)};
    ...
    while ((o = opt_next()) != OPT_EOF)
    {
        switch (o)
        {
            ...
            case OPT_MISALIGN:
                ...
                {
                    BIO_printf(bio_err, "%s: Maximum offset is %d\n", prog,
                               MISALIGN);
                    goto opterr;
                }
                break;
    ...
    for (; *argv; argv++)
    {
        ...
        {
            dsa_doit[R_DSA_512] = dsa_doit[R_DSA_1024] = dsa_doit[R_DSA_2048] =
                1;
            continue;
        }
    ...
    if (aead)
    {
        ...
        else if (!(EVP_CIPHER_flags(evp_cipher) & EVP_CIPH_FLAG_AEAD_CIPHER))
        {
...
#error "You cannot disable DES on systems without SIGALRM."
        ...
        for (testnum = 0; testnum < size_num && async_init == 0; testnum++)
        {
            ...
            for (count = 0, run = 1; COND(c[D_CBC_IDEA][testnum]); count++)
                IDEA_cbc_encrypt(loopargs[0].buf, loopargs[0].buf,
                                 (size_t)lengths[testnum], &idea_ks, iv,
                                 IDEA_ENCRYPT);
            d = Time_F(STOP);
        ...
        for (testnum = 0; testnum < size_num && async_init == 0; testnum++)
        {
            ...
            for (count = 0, run = 1; COND(c[D_CBC_RC2][testnum]); count++)
                RC2_cbc_encrypt(loopargs[0].buf, loopargs[0].buf,
                                (size_t)lengths[testnum], &rc2_ks, iv,
                                RC2_ENCRYPT);
            d = Time_F(STOP);
        ...
        for (testnum = 0; testnum < size_num && async_init == 0; testnum++)
        {
            ...
            for (count = 0, run = 1; COND(c[D_CBC_RC5][testnum]); count++)
                RC5_32_cbc_encrypt(loopargs[0].buf, loopargs[0].buf,
                                   (size_t)lengths[testnum], &rc5_ks, iv,
                                   RC5_ENCRYPT);
            d = Time_F(STOP);
        ...
        for (testnum = 0; testnum < size_num && async_init == 0; testnum++)
        {
            ...
            for (count = 0, run = 1; COND(c[D_CBC_BF][testnum]); count++)
                BF_cbc_encrypt(loopargs[0].buf, loopargs[0].buf,
                               (size_t)lengths[testnum], &bf_ks, iv,
                               BF_ENCRYPT);
            d = Time_F(STOP);
        ...
        for (testnum = 0; testnum < size_num && async_init == 0; testnum++)
        {
            ...
            for (count = 0, run = 1; COND(c[D_CBC_CAST][testnum]); count++)
                CAST_cbc_encrypt(loopargs[0].buf, loopargs[0].buf,
                                 (size_t)lengths[testnum], &cast_ks, iv,
                                 CAST_ENCRYPT);
            d = Time_F(STOP);
        ...
        if (evp_cipher != NULL)
        {
            ...
            if (EVP_CIPHER_mode(evp_cipher) == EVP_CIPH_CCM_MODE)
            {
                loopfunc = EVP_Update_loop_ccm;
                     ...
                     (EVP_CIPHER_flags(evp_cipher) & EVP_CIPH_FLAG_AEAD_CIPHER))
            {
    ...
    for (testnum = 0; testnum < RSA_NUM; testnum++)
    {
        ...
        {
            pkey_print_message("private", "rsa", rsa_c[testnum][0],
                               rsa_bits[testnum], seconds.rsa);
        ...
        {
            pkey_print_message("public", "rsa", rsa_c[testnum][1],
                               rsa_bits[testnum], seconds.rsa);
            Time_F(START);
    ...
    for (testnum = 0; testnum < DSA_NUM; testnum++)
    {
        ...
        {
            pkey_print_message("sign", "dsa", dsa_c[testnum][0],
        ...
        {
            pkey_print_message("verify", "dsa", dsa_c[testnum][1],
                               dsa_bits[testnum], seconds.dsa);
            Time_F(START);
    ...
    for (testnum = 0; testnum < ECDSA_NUM; testnum++)
    {
        ...
        else
        {
            for (i = 0; i < loopargs_len; i++)
            {
                ...
                st =
                    ECDSA_sign(0, loopargs[i].buf, 20, loopargs[i].buf2,
            ...
            {
                pkey_print_message("sign", "ecdsa", ecdsa_c[testnum][0],
            ...
            {
                pkey_print_message("verify", "ecdsa", ecdsa_c[testnum][1],
                                   test_curves[testnum].bits, seconds.ecdsa);
                Time_F(START);
    ...
    for (testnum = 0; testnum < EC_NUM; testnum++)
    {
        ...
        for (i = 0; i < loopargs_len; i++)
        {
            ...
            if (ERR_peek_error())
            {
                BIO_printf(bio_err, "WARNING: the error queue contains "
            ...
            kctx = EVP_PKEY_CTX_new_id(test_curves[testnum].nid,
                                       NULL); /* keygen ctx from NID */
            if (!kctx)
            {
                ...
                if (error == ERR_peek_last_error() && /* oldest and latest
                                                         errors match */
                ...
                if (ERR_peek_error())
                {
                    BIO_printf(bio_err, "Unhandled error in the error queue "
                                        "during ECDH init.\n");
                    ERR_print_errors(bio_err);
                    rsa_count = 1;
                    break;
            ...
            if (!EVP_PKEY_keygen(kctx, &key_A) || /* generate secret key A */
                ...
                !(ctx = EVP_PKEY_CTX_new(
                      key_A, NULL)) ||        /* derivation ctx from skeyA */
                !EVP_PKEY_derive_init(ctx) || /* init derivation ctx */
                !EVP_PKEY_derive_set_peer(ctx,
                                          key_B) || /* set peer pubkey in ctx */
                !EVP_PKEY_derive(ctx, NULL,
                                 &outlen) || /* determine max length */
            ...
            if (!(test_ctx = EVP_PKEY_CTX_new(
                      key_B, NULL)) || /* test ctx from skeyB */
                !EVP_PKEY_derive_init(
                    test_ctx) || /* init derivation test_ctx */
                !EVP_PKEY_derive_set_peer(
                    test_ctx, key_A) || /* set peer pubkey in test_ctx */
            ...
            /* Compare the computation results: CRYPTO_memcmp() returns 0 if
             * equal */
            if (CRYPTO_memcmp(loopargs[i].secret_a, loopargs[i].secret_b,
                              outlen))
            {
                ecdh_checks = 0;
        ...
        if (ecdh_checks != 0)
        {
            pkey_print_message("", "ecdh", ecdh_c[testnum][0],
    ...
    for (testnum = 0; testnum < EdDSA_NUM; testnum++)
    {
        ...
        for (i = 0; i < loopargs_len; i++)
        {
            ...
            if ((ed_pctx = EVP_PKEY_CTX_new_id(test_ed_curves[testnum].nid,
                                               NULL)) == NULL ||
...
show_res:
    ...
    for (k = 0; k < EC_NUM; k++)
    {
        ...
        if (mr)
            ...
            printf("%4u bits ecdh (%s) %8.4fs %8.1f\n", test_curves[k].bits,
                   test_curves[k].name, 1.0 / ecdh_results[k][0],
                   ecdh_results[k][0]);
    }
    ...
    for (k = 0; k < OSSL_NELEM(eddsa_doit); k++)
    {
        ...
        if (mr)
            printf("+F6:%u:%u:%s:%f:%f\n", k, test_ed_curves[k].bits,
                   test_ed_curves[k].name, eddsa_results[k][0],
                   eddsa_results[k][1]);
        else
...
static void multiblock_speed(const EVP_CIPHER* evp_cipher, int lengths_single,
                             const openssl_speed_sec_t* seconds)
{
    static const int mblengths_list[] = {8 * 1024, 2 * 8 * 1024, 4 * 8 * 1024,
                                         8 * 8 * 1024, 8 * 16 * 1024};
