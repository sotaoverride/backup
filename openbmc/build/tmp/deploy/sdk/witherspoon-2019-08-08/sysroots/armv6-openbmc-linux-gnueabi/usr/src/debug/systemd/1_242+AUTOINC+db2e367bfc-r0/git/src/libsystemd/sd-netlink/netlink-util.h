...
int rtnl_message_new_synthetic_error(sd_netlink* rtnl, int error,
                                     uint32_t serial, sd_netlink_message** ret);
...
#define netlink_call_async(nl, ret_slot, message, callback, destroy_callback,  \
                           userdata)                                           \
    ({                                                                         \
        int (*_callback_)(sd_netlink*, sd_netlink_message*,                    \
                          typeof(userdata)) = callback;                        \
        void (*_destroy_)(typeof(userdata)) = destroy_callback;                \
        sd_netlink_call_async(                                                 \
            nl, ret_slot, message, (sd_netlink_message_handler_t)_callback_,   \
            (sd_netlink_destroy_t)_destroy_, userdata, 0, __func__);           \
    })
...
#define netlink_add_match(nl, ret_slot, metch, callback, destroy_callback,     \
    ...
    ({                                                                         \
        ...
        sd_netlink_add_match(                                                  \
            nl, ret_slot, match, (sd_netlink_message_handler_t)_callback_,     \
            (sd_netlink_destroy_t)_destroy_, userdata, __func__);              \
    })
