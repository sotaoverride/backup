...
static void routing_policy_rule_hash_func(const RoutingPolicyRule* rule,
                                          struct siphash* state)
{
    ...
    switch (rule->family)
    {
        ...
        case AF_INET6:
            ...
            siphash24_compress(&rule->from, FAMILY_ADDRESS_SIZE(rule->family),
                               state);
            siphash24_compress(&rule->from_prefixlen,
                               sizeof(rule->from_prefixlen), state);

            siphash24_compress(&rule->to, FAMILY_ADDRESS_SIZE(rule->family),
                               state);
            siphash24_compress(&rule->to_prefixlen, sizeof(rule->to_prefixlen),
                               state);
...
static int routing_policy_rule_compare_func(const RoutingPolicyRule* a,
                                            const RoutingPolicyRule* b)
{
    int r;
...
DEFINE_PRIVATE_HASH_OPS(routing_policy_rule_hash_ops, RoutingPolicyRule,
                        routing_policy_rule_hash_func,
                        routing_policy_rule_compare_func);

int routing_policy_rule_get(
    Manager* m, int family, const union in_addr_union* from,
    uint8_t from_prefixlen, const union in_addr_union* to, uint8_t to_prefixlen,
    uint8_t tos, uint32_t fwmark, uint32_t table, const char* iif,
    const char* oif, uint8_t protocol, struct fib_rule_port_range* sport,
...
static int routing_policy_rule_add_internal(
    Manager* m, Set** rules, int family, const union in_addr_union* from,
    uint8_t from_prefixlen, const union in_addr_union* to, uint8_t to_prefixlen,
    uint8_t tos, uint32_t fwmark, uint32_t table, const char* _iif,
    const char* _oif, uint8_t protocol, const struct fib_rule_port_range* sport,
...
int routing_policy_rule_add(
    ...
    uint8_t from_prefixlen, const union in_addr_union* to, uint8_t to_prefixlen,
    uint8_t tos, uint32_t fwmark, uint32_t table, const char* iif,
    const char* oif, uint8_t protocol, const struct fib_rule_port_range* sport,
    const struct fib_rule_port_range* dport, RoutingPolicyRule** ret)
{

    return routing_policy_rule_add_internal(
        m, &m->rules, family, from, from_prefixlen, to, to_prefixlen, tos,
        fwmark, table, iif, oif, protocol, sport, dport, ret);
}

int routing_policy_rule_add_foreign(
    Manager* m, int family, const union in_addr_union* from,
    uint8_t from_prefixlen, const union in_addr_union* to, uint8_t to_prefixlen,
    uint8_t tos, uint32_t fwmark, uint32_t table, const char* iif,
    const char* oif, uint8_t protocol, const struct fib_rule_port_range* sport,
...
{
    return routing_policy_rule_add_internal(
        m, &m->rules_foreign, family, from, from_prefixlen, to, to_prefixlen,
        tos, fwmark, table, iif, oif, protocol, sport, dport, ret);
}

static int routing_policy_rule_remove_handler(sd_netlink* rtnl,
                                              sd_netlink_message* m, Link* link)
{
    int r;
                               ...
                               link_netlink_message_handler_t callback)
{
    ...
    r = sd_rtnl_message_new_routing_policy_rule(
        link->manager->rtnl, &m, RTM_DELRULE, routing_policy_rule->family);
    if (r < 0)
        return log_error_errno(r, "Could not allocate RTM_DELRULE message: %m");

    if (in_addr_is_null(routing_policy_rule->family,
                        &routing_policy_rule->from) == 0)
    {
        r = netlink_message_append_in_addr_union(m, FRA_SRC,
                                                 routing_policy_rule->family,
                                                 &routing_policy_rule->from);
        if (r < 0)
            return log_error_errno(r, "Could not append FRA_SRC attribute: %m");

        r = sd_rtnl_message_routing_policy_rule_set_rtm_src_prefixlen(
            m, routing_policy_rule->from_prefixlen);
        if (r < 0)
            return log_error_errno(r, "Could not set source prefix length: %m");
    }

    if (in_addr_is_null(routing_policy_rule->family,
                        &routing_policy_rule->to) == 0)
    {
        r = netlink_message_append_in_addr_union(
            m, FRA_DST, routing_policy_rule->family, &routing_policy_rule->to);
        if (r < 0)
            return log_error_errno(r, "Could not append FRA_DST attribute: %m");

        r = sd_rtnl_message_routing_policy_rule_set_rtm_dst_prefixlen(
            m, routing_policy_rule->to_prefixlen);
        if (r < 0)
            return log_error_errno(
                r, "Could not set destination prefix length: %m");
    }
...
static int routing_policy_rule_new_static(Network* network,
                                          const char* filename,
                                          ...
                                          RoutingPolicyRule** ret)
{
    ...
    if (filename)
    {
        ...
        r = hashmap_ensure_allocated(&network->rules_by_section,
...
static int routing_policy_rule_handler(sd_netlink* rtnl, sd_netlink_message* m,
                                       Link* link)
{
    int r;
...
int routing_policy_rule_configure(RoutingPolicyRule* rule, Link* link,
                                  link_netlink_message_handler_t callback,
                                  bool update)
{
    _cleanup_(sd_netlink_message_unrefp) sd_netlink_message* m = NULL;
    ...
    r = sd_rtnl_message_new_routing_policy_rule(link->manager->rtnl, &m,
                                                RTM_NEWRULE, rule->family);
    if (r < 0)
        return log_error_errno(r, "Could not allocate RTM_NEWRULE message: %m");

    if (in_addr_is_null(rule->family, &rule->from) == 0)
    {
        r = netlink_message_append_in_addr_union(m, FRA_SRC, rule->family,
                                                 &rule->from);
        if (r < 0)
            return log_error_errno(r, "Could not append FRA_SRC attribute: %m");

        r = sd_rtnl_message_routing_policy_rule_set_rtm_src_prefixlen(
            m, rule->from_prefixlen);
        if (r < 0)
            return log_error_errno(r, "Could not set source prefix length: %m");
    }
    ...
    {
        r = netlink_message_append_in_addr_union(m, FRA_DST, rule->family,
                                                 &rule->to);
        if (r < 0)
            return log_error_errno(r, "Could not append FRA_DST attribute: %m");
    ...
    if (rule->oif)
    {
        ...
        if (r < 0)
            return log_error_errno(
                r, "Could not append FRA_OIFNAME attribute: %m");
    }
    ...
    {
        r = sd_netlink_message_append_data(m, FRA_SPORT_RANGE, &rule->sport,
                                           sizeof(rule->sport));
        if (r < 0)
            return log_error_errno(
                r, "Could not append FRA_SPORT_RANGE attribute: %m");
    }

    if (rule->dport.start != 0 || rule->dport.end != 0)
    {
        r = sd_netlink_message_append_data(m, FRA_DPORT_RANGE, &rule->dport,
                                           sizeof(rule->dport));
        if (r < 0)
            return log_error_errno(
                r, "Could not append FRA_DPORT_RANGE attribute: %m");
    }
    ...
    if (rule->invert_rule)
    {
        ...
        if (r < 0)
            return log_error_errno(
                r, "Could not append FIB_RULE_INVERT attribute: %m");
    }
...
static int parse_fwmark_fwmask(const char* s, uint32_t* fwmark,
                               uint32_t* fwmask)
{
    ...
    if (r < 0)
        return log_error_errno(
            r, "Failed to parse RPDB rule firewall mark, ignoring: %s", f);
    ...
    if (p)
    {
        ...
        if (r < 0)
            return log_error_errno(
                r, "Failed to parse RPDB rule mask, ignoring: %s", f);
    }

    return 0;
}

int config_parse_routing_policy_rule_tos(const char* unit, const char* filename,
                                         ...
                                         void* userdata)
{
    ...
    _cleanup_(routing_policy_rule_free_or_set_invalidp) RoutingPolicyRule* n =
        NULL;
    ...
    void* data, void* userdata)
{
    ...
    {
        log_syntax(unit, LOG_ERR, filename, line, r,
                   "RPDB rule prefix is invalid, ignoring assignment: %s",
                   rvalue);
        return 0;
    }
    ...
    void* data, void* userdata)
{
    ...
    {
        log_syntax(unit, LOG_ERR, filename, line, r,
                   "Failed to parse routing policy rule port range '%s'",
                   rvalue);
        return 0;
    }
    ...
    void* data, void* userdata)
{
    ...
    {
        log_syntax(unit, LOG_ERR, filename, line, r,
                   "Failed to parse IP protocol '%s' for routing policy rule, "
                   "ignoring: %m",
                   rvalue);
        return 0;
    }
...
static int routing_policy_rule_read_full_file(const char* state_file,
                                              char** ret)
{
...
int routing_policy_serialize_rules(Set* rules, FILE* f)
{
    ...
    SET_FOREACH(rule, rules, i)
    {
        ...
        if (!in_addr_is_null(rule->family, &rule->from))
        {
            ...
            fprintf(f, "from=%s/%hhu", from_str, rule->from_prefixlen);
            space = true;
        }
        ...
        if (!in_addr_is_null(rule->family, &rule->to))
        {
            ...
            fprintf(f, "%sto=%s/%hhu", space ? " " : "", to_str,
                    rule->to_prefixlen);
            space = true;
        }
        ...
        if (rule->tos != 0)
        {
            fprintf(f, "%stos=%hhu", space ? " " : "", rule->tos);
            space = true;
        }

        if (rule->fwmark != 0)
        {
            fprintf(f, "%sfwmark=%" PRIu32 "/%" PRIu32, space ? " " : "",
        ...
        if (rule->iif)
        {
            fprintf(f, "%siif=%s", space ? " " : "", rule->iif);
            space = true;
        }

        if (rule->oif)
        {
            fprintf(f, "%soif=%s", space ? " " : "", rule->oif);
            space = true;
        }

        if (rule->protocol != 0)
        {
            fprintf(f, "%sprotocol=%hhu", space ? " " : "", rule->protocol);
            space = true;
        }

        if (rule->sport.start != 0 || rule->sport.end != 0)
        {
            fprintf(f, "%ssourcesport=%" PRIu16 "-%" PRIu16, space ? " " : "",
        ...
        if (rule->dport.start != 0 || rule->dport.end != 0)
        {
            fprintf(f, "%sdestinationport=%" PRIu16 "-%" PRIu16,
                    space ? " " : "", rule->dport.start, rule->dport.end);
            space = true;
        }
    ...
    STRV_FOREACH(i, l)
    {
        ...
        for (;;)
        {
            ...
            if (STR_IN_SET(a, "from", "to"))
            {
                ...
                r = in_addr_prefix_from_string_auto(b, &rule->family, buffer,
                                                    prefixlen);
                if (r < 0)
                {
                    log_error_errno(
            ...
            else if (streq(a, "tos"))
            {
                ...
                {
                    log_error_errno(
                        r, "Failed to parse RPDB rule tos, ignoring: %s", b);
                    continue;
                }
            }
            else if (streq(a, "table"))
            {
                ...
                {
                    log_error_errno(
                        r, "Failed to parse RPDB rule table, ignoring: %s", b);
                    continue;
                }
            }
            else if (streq(a, "fwmark"))
            {
                ...
                {
                    log_error_errno(r,
                                    "Failed to parse RPDB rule firewall mark "
                                    "or mask, ignoring: %s",
                                    a);
                    continue;
                }
            ...
            else if (streq(a, "protocol"))
            {
                ...
                {
                    log_error_errno(
                        r, "Failed to parse RPDB rule protocol, ignoring: %s",
                        b);
                    continue;
                }
            }
            else if (streq(a, "sourceport"))
            {
                ...
                {
                    log_error_errno(r,
                                    "Invalid routing policy rule source port "
                                    "range, ignoring assignment:'%s'",
                                    b);
                    continue;
                }
            ...
            else if (streq(a, "destinationport"))
            {
                ...
                {
                    log_error_errno(r,
                                    "Invalid routing policy rule destination "
                                    "port range, ignoring assignment:'%s'",
                                    b);
                    continue;
                }
        ...
        {
            log_warning_errno(
                r, "Failed to add RPDB rule to saved DB, ignoring: %s", p);
            continue;
        }
...
void routing_policy_rule_purge(Manager* m, Link* link)
{
    ...
    SET_FOREACH(rule, m->rules_saved, i)
    {
        ...
        /* Existing links do not have the saved rule. Let's drop the rule now,
         * and re-configure it later when it is requested. */
