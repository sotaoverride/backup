...
#define snmpTargetAddrOIDLen                                                   \
    11 /*This is base+column,                                                  \
        * i.e. everything but index */

static oid snmpTargetAddrOID[snmpTargetAddrOIDLen] = {1,  3, 6, 1, 6, 3,
                                                      12, 1, 2, 1, 0};
 ...
 */
static const oid snmpTargetAddrEntry_variables_oid[] = {1, 3,  6, 1, 6,
                                                        3, 12, 1, 2, 1};
...
void init_snmpTargetAddrEntry(void)
{
    ...
    REGISTER_MIB("target/snmpTargetSpinLock", snmpTargetSpinLock_var, variable2,
                 snmpTargetSpinLock_oid);
...
u_char* var_snmpTargetAddrEntry(struct variable* vp, oid* name, size_t* length,
                                ...
                                WriteMethod** write_method)
{
    ...
    *var_len = sizeof(long_ret);
     ...
     */
    if ((temp_struct = search_snmpTargetAddrTable(vp->name, vp->namelen, name,
                                                  length, exact)) == NULL)
    {
        return NULL;
    }
...
#ifndef NETSNMP_NO_WRITE_SUPPORT
int write_snmpTargetAddrTDomain(int action, u_char* var_val,
                                ...
                                u_char* statP, oid* name, size_t name_len)
{
    ...
    else if (action == RESERVE2)
    {
        ...
        else
        {
            ...
            {
                DEBUGMSGTL(
                    ("snmpTargetAddrEntry", "write to snmpTargetAddrTDomain: "
...
int write_snmpTargetAddrTAddress(int action, u_char* var_val,
                                 u_char var_val_type, size_t var_val_len,
                                 u_char* statP, oid* name, size_t name_len)
{
    struct targetAddrTable_struct* target = NULL;
    ...
    else if (action == RESERVE2)
    {
        ...
        else
        {
            ...
            {
                DEBUGMSGTL(
                    ("snmpTargetAddrEntry", "write to snmpTargetAddrTAddress: "
                                            "not allowed in active row.\n"));
                return SNMP_ERR_INCONSISTENTVALUE;
            }
...
int write_snmpTargetAddrTimeout(int action, u_char* var_val,
                                u_char var_val_type, size_t var_val_len,
                                u_char* statP, oid* name, size_t name_len)
{
    /*
    ...
    else if (action == RESERVE2)
    {
        ...
        if ((temp_struct = search_snmpTargetAddrTable(
                 snmpTargetAddrOID, snmpTargetAddrOIDLen, name, &name_len,
                 1)) == NULL)
        {
            DEBUGMSGTL(("snmpTargetAddrEntry",
...
int write_snmpTargetAddrRetryCount(int action, u_char* var_val,
                                   u_char var_val_type, size_t var_val_len,
                                   u_char* statP, oid* name, size_t name_len)
{
    static long long_ret;
...
int write_snmpTargetAddrTagList(int action, u_char* var_val,
                                u_char var_val_type, size_t var_val_len,
                                u_char* statP, oid* name, size_t name_len)
{
    struct targetAddrTable_struct* target = NULL;
    ...
    else if (action == RESERVE2)
    {
        ...
        else
        {
            ...
            target->tagListData =
                netsnmp_memdup_nt(var_val, var_val_len, &target->tagListLen);
...
int write_snmpTargetAddrParams(int action, u_char* var_val, u_char var_val_type,
                               ...
                               size_t name_len)
{
    ...
    else if (action == RESERVE2)
    {
        ...
        else
        {
            ...
            {
                DEBUGMSGTL(
                    ("snmpTargetAddrEntry", "write to snmpTargetAddrParams: "
                                            "not allowed in active row.\n"));
                return SNMP_ERR_INCONSISTENTVALUE;
            }
            if (old_params != NULL)
            ...
            target->paramsData =
                netsnmp_memdup_nt(var_val, var_val_len, &target->paramsLen);
...
int write_snmpTargetAddrStorageType(int action, u_char* var_val,
                                    u_char var_val_type, size_t var_val_len,
                                    u_char* statP, oid* name, size_t name_len)
{
    ...
    if (action == RESERVE1)
    {
        ...
        {
            DEBUGMSGTL(
                ("snmpTargetAddrEntry", "write to snmpTargetAddrStorageType: "
                                        "attempted storage type not a valid"));
    ...
    else if (action == RESERVE2)
    {
        ...
        else
        {
            ...
            {
                DEBUGMSGTL(("snmpTargetAddrEntry",
                            "write to snmpTargetAddrStorageType: row has "
 ...
 */
int write_snmpTargetAddrRowStatus(int action, u_char* var_val,
                                  u_char var_val_type, size_t var_val_len,
                                  u_char* statP, oid* name, size_t name_len)
{
    static long value;
    ...
    if (action == RESERVE1)
    {
        ...
        snmpTargetAddrOID[snmpTargetAddrOIDLen - 1] =
            SNMPTARGETADDRROWSTATUSCOLUMN;
        target = search_snmpTargetAddrTable(
            snmpTargetAddrOID, snmpTargetAddrOIDLen, name, &name_len, 1);
        ...
        else
        {
            ...
            if (value == RS_CREATEANDGO || value == RS_CREATEANDWAIT)
            {
                ...
                {
                    DEBUGMSGTL(
                        ("snmpTargetAddrEntry", "couldn't malloc() new row\n"));
                    return SNMP_ERR_RESOURCEUNAVAILABLE;
                }
            }
        }
...
int write_targetSpinLock(int action, u_char* var_val, u_char var_val_type,
                         size_t var_val_len, u_char* statP, oid* name,
                         size_t name_len)
{
    if (action == RESERVE1)
...
u_char* var_targetSpinLock(struct variable* vp, oid* name, size_t* length,
                           int exact, size_t* var_len,
                           WriteMethod** write_method)
{
