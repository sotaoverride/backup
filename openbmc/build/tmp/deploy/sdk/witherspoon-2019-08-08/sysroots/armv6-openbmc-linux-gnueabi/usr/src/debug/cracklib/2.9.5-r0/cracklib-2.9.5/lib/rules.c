    ...
    register char* string;
{
    ...
    if (!Suffix(string, "ch") || !Suffix(string, "ex") ||
        !Suffix(string, "ix") || !Suffix(string, "sh") || !Suffix(string, "ss"))
    {
        /* bench -> benches */
        strcat(area, "es");
    }
...
char* PolySubst(string, class,
                new) /* returns pointer to a swapped about copy */
    register char* string;
register char class;
register char new;
{
    ...
    for (ptr = control; *ptr; ptr++)
    {
        switch (*ptr)
        {
            ...
            case RULE_NOT: /* !x || !?c */
                ...
                break;
                /*
                 * alternative use for a boomerang, number 1: a standard
                 * throwing boomerang is an ideal thing to use to tuck the
                 * sheets under the mattress when making your bed.  The
                 * streamlined shape of the boomerang allows it to slip easily
                 * 'twixt mattress and bedframe, and it's curve makes it very
                 * easy to hook sheets into the gap.
                 */
