...
static void server_process_entry_meta(const char* p, size_t l,
                                      const struct ucred* ucred, int* priority,
                                      char** identifier, char** message,
                                      pid_t* object_pid)
{

    /* We need to determine the priority of this entry for the rate limiting
     * logic */

    if (l == 10 && startswith(p, "PRIORITY=") && p[9] >= '0' && p[9] <= '9')
        *priority = (*priority & LOG_FACMASK) | (p[9] - '0');

    else if (l == 17 && startswith(p, "SYSLOG_FACILITY=") && p[16] >= '0' &&
             p[16] <= '9')
        *priority = (*priority & LOG_PRIMASK) | ((p[16] - '0') << 3);

    else if (l == 18 && startswith(p, "SYSLOG_FACILITY=") && p[16] >= '0' &&
             p[16] <= '9' && p[17] >= '0' && p[17] <= '9')
...
static int server_process_entry(Server* s, const void* buffer,
                                size_t* remaining, ClientContext* context,
                                const struct ucred* ucred,
                                const struct timeval* tv, const char* label,
                                size_t label_len)
{

    /* Process a single entry from a native message. Returns 0 if nothing
     * special happened and the message processing should continue, and a
     * negative or positive value otherwise.
     *
    ...
    while (*remaining > 0)
    {
        ...
        {
            log_debug("Received an entry that has more than " STRINGIFY(
                ENTRY_FIELD_COUNT_MAX) " fields, ignoring entry.");
            goto finish;
        }
        ...
        if (q)
        {
            if (journal_field_valid(p, q - p, false))
            {
                ...
                if (l > DATA_SIZE_MAX)
                {
                    log_debug("Received text block of %zu bytes is too large, "
                              "ignoring entry.",
                              l);
                    goto finish;
                }

                if (entry_size + l + n + 1 > ENTRY_SIZE_MAX)
                { /* data + separators + trailer */
                    log_debug("Entry is too big (%zu bytes after processing "
                              "%zu entries), ignoring entry.",
                              entry_size + l, n + 1);
                    goto finish;
                }

                /* If the field name starts with an underscore, skip the
                 * variable, since that indicates a trusted field */
                iovec[n++] = IOVEC_MAKE((char*)p, l);
                entry_size += l;

                server_process_entry_meta(p, l, ucred, &priority, &identifier,
                                          &message, &object_pid);
            }
        ...
        else
        {
            ...
            if (l > DATA_SIZE_MAX)
            {
                log_debug("Received binary data block of %" PRIu64
                          " bytes is too large, ignoring entry.",
                          l);
                goto finish;
            }

            total = (e - p) + 1 + l;
            if (entry_size + total + n + 1 > ENTRY_SIZE_MAX)
            { /* data + separators + trailer */
                log_debug("Entry is too big (%" PRIu64
            ...
            if (journal_field_valid(p, e - p, false))
            {
                ...
                server_process_entry_meta(k, (e - p) + 1 + l, ucred, &priority,
                                          &identifier, &message, &object_pid);
            }
            else
                free(k);
    ...
    { /* data + separators + trailer */
        log_debug(
            "Entry is too big with %zu properties and %zu bytes, ignoring.", n,
            entry_size);
        goto finish;
    }
    ...
    if (message)
    {
        if (s->forward_to_syslog)
            server_forward_syslog(s, syslog_fixup_facility(priority),
...
void server_process_native_message(Server* s, const char* buffer,
                                   size_t buffer_size,
                                   const struct ucred* ucred,
                                   const struct timeval* tv, const char* label,
                                   size_t label_len)
{
    ...
    {
        r = client_context_get(s, ucred->pid, ucred, label, label_len, NULL,
                               &context);
        if (r < 0)
            log_warning_errno(r,
    ...
    do
    {
        r = server_process_entry(
            s, (const uint8_t*)buffer + (buffer_size - remaining), &remaining,
            context, ucred, tv, label, label_len);
    } while (r == 0);
}

void server_process_native_file(Server* s, int fd, const struct ucred* ucred,
                                const struct timeval* tv, const char* label,
                                size_t label_len)
{
    ...
    if (!sealed && (!ucred || ucred->uid != 0))
    {
        ...
        {
            log_error("Received file in subdirectory of allowed directories. "
                      "Refusing.");
            return;
        }
    }
    ...
    {
        log_error("File passed too large (%" PRIu64 " bytes). Ignoring.",
                  (uint64_t)st.st_size);
        return;
    }
    ...
    if (sealed)
    {
        ...
        server_process_native_message(s, p, st.st_size, ucred, tv, label,
                                      label_len);
    ...
    else
    {
        ...
        {
            log_error_errno(errno, "Failed to stat file system of passed file, "
                                   "not processing it: %m");
            return;
        }
        ...
        {
            log_error("Received file descriptor from file system with "
                      "mandatory locking enabled, not processing it.");
            return;
        }
        ...
        {
            log_error_errno(
                r, "Failed to make fd non-blocking, not processing it: %m");
            return;
        }
...
int server_open_native_socket(Server* s)
{
    ...
    r = sd_event_add_io(s->event, &s->native_event_source, s->native_fd,
                        EPOLLIN, server_process_datagram, s);
    if (r < 0)
        return log_error_errno(
            r, "Failed to add native server fd to event loop: %m");

    r = sd_event_source_set_priority(s->native_event_source,
                                     SD_EVENT_PRIORITY_NORMAL + 5);
    if (r < 0)
        return log_error_errno(
            r, "Failed to adjust native event source priority: %m");
