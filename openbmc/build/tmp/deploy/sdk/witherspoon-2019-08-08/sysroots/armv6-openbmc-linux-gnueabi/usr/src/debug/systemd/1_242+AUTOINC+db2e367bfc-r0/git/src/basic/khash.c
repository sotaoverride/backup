...
/* On current kernels the maximum digest (according to "grep digestsize
 * /proc/crypto | sort -u") is actually 32, but let's add some extra room, the
 * few wasted bytes don't really matter... */
...
int khash_supported(void)
{
    ...
    if (cached < 0)
    {
        ...
        {
            /* The kernel returns ENOENT if the selected algorithm is not
             * supported at all. We use a check for SHA256 as a proxy for
             * whether the whole API is supported at all. After all it's one of
             * the most common hash functions, and if it isn't supported, that's
             * ample indication that something is really off. */
        ...
        {
            /* On some kernels we get ENOKEY for non-keyed hash functions (such
             * as sha256), let's refuse using the API in those cases, since the
             * kernel is
...
int khash_new_with_key(khash** ret, const char* algorithm, const void* key,
                       size_t key_size)
{
    union
    {
    ...
    /* Filter out an empty algorithm early, as we do not support an algorithm by
     * that name. */
    ...
    /* Temporary fix for rc kernel bug:
    ...
    if (n < 0)
        return -errno;
    if (n >= LONGEST_DIGEST) /* longer than what we expected? If so, we don't
                                support this */
        return -EOPNOTSUPP;
