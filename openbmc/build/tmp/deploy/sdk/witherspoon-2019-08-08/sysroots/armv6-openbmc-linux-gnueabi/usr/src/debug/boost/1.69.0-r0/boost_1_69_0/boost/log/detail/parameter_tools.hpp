...
//! The maximum number of named arguments that are accepted by constructors and
//! functions
#define BOOST_LOG_MAX_PARAMETER_ARGS 16
#endif

// The macro applies the passed macro with the specified arguments
// BOOST_LOG_MAX_PARAMETER_ARGS times
...
#define BOOST_LOG_CTOR_FORWARD_1(n, types)                                     \
    ...
    explicit BOOST_PP_TUPLE_ELEM(2, 0, types)(                                 \
...
#define BOOST_LOG_CTOR_FORWARD(z, n, types)                                    \
    BOOST_PP_IF(BOOST_PP_EQUAL(n, 1), BOOST_LOG_CTOR_FORWARD_1,                \
                BOOST_LOG_CTOR_FORWARD_N)                                      \
    (n, types)

// The macro expands to a number of templated constructors that aggregate their
// named arguments into an ArgumentsPack and pass it to the base class
// constructor.
#define BOOST_LOG_PARAMETRIZED_CONSTRUCTORS_FORWARD(class_type, base_type)     \
    BOOST_LOG_PARAMETRIZED_CONSTRUCTORS_GEN(BOOST_LOG_CTOR_FORWARD,            \
...
#define BOOST_LOG_CTOR_CALL(z, n, types)                                       \
    BOOST_PP_IF(BOOST_PP_EQUAL(n, 1), BOOST_LOG_CTOR_CALL_1,                   \
                BOOST_LOG_CTOR_CALL_N)                                         \
    (n, types)

// The macro expands to a number of templated constructors that aggregate their
// named arguments into an ArgumentsPack and pass it to a function call.
#define BOOST_LOG_PARAMETRIZED_CONSTRUCTORS_CALL(class_type, fun)              \
    BOOST_LOG_PARAMETRIZED_CONSTRUCTORS_GEN(BOOST_LOG_CTOR_CALL,               \
                                            (class_type, fun))
...
typedef boost::parameter::aux::empty_arg_list empty_arg_list;
      ...
      defined(BOOST_LOG_NO_CXX11_ARG_PACKS_TO_NON_VARIADIC_ARGS_EXPANSION))
...
struct make_arg_list
{
    typedef boost::parameter::aux::arg_list<
        ArgT0, typename make_arg_list<ArgsT...>::type>
        type;
};
...
//! The metafunction generates argument pack
template <typename ArgT0, BOOST_PP_ENUM_BINARY_PARAMS(
                              BOOST_PP_DEC(BOOST_LOG_MAX_PARAMETER_ARGS),
                              typename T, = void BOOST_PP_INTERCEPT)>
struct make_arg_list
{
    typedef boost::parameter::aux::arg_list<
        ArgT0, typename make_arg_list<BOOST_PP_ENUM_PARAMS(
                   BOOST_PP_DEC(BOOST_LOG_MAX_PARAMETER_ARGS), T)>::type>
        type;
};

template <typename ArgT0>
struct make_arg_list<ArgT0, BOOST_PP_ENUM_PARAMS(
                                BOOST_PP_DEC(BOOST_LOG_MAX_PARAMETER_ARGS),
...
struct enable_if_named_parameters<
    boost::parameter::aux::tagged_argument<Keyword, Arg>, R>
{
    typedef R type;
};
...
struct enable_if_named_parameters<
    boost::parameter::aux::arg_list<TaggedArg, Next>, R>
{
    typedef R type;
};
