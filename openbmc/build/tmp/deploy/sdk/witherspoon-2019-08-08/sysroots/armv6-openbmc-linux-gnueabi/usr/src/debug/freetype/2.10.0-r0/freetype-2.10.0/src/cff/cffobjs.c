...
cff_size_init(FT_Size cffsize) /* CFF_Size */
{
    ...
    if (funcs)
    {
        ...
        error = funcs->create(cffsize->face->memory, &priv, &internal->topfont);
        if (error)
            goto Exit;
...
cff_size_select(FT_Size size, FT_ULong strike_index)
{
    ...
    if (funcs)
    {
        ...
        funcs->set_scale(internal->topfont, size->metrics.x_scale,
        ...
        for (i = font->num_subfonts; i > 0; i--)
        {
            ...
            funcs->set_scale(internal->subfonts[i - 1], x_scale, y_scale, 0, 0);
        }
    }
...
static void remove_style(FT_String* family_name, const FT_String* style_name)
{
    ...
    if (family_name_length > style_name_length)
    {
        ...
        if (idx > style_name_length)
        {
            ...
            while (idx > 0 &&
                   (family_name[idx] == '-' || family_name[idx] == ' ' ||
                    family_name[idx] == '_' || family_name[idx] == '+'))
                idx--;
              ...
              FT_Int face_index, FT_Int num_params, FT_Parameter* params)
{
    ...
    pshinter = (PSHinter_Service)FT_Get_Module_Interface(library, "pshinter");
    ...
    /* now load and parse the CFF table in the file */
    {
        ...
        face->extra.data = cff;
        error = cff_font_load(library, stream, face_index, cff, face, pure_cff,
                              cff2);
        if (error)
            goto Exit;
...
#ifdef FT_DEBUG_LEVEL_TRACE
        {
            ...
            for (idx = 1; idx < cff->num_strings; idx++)
            {
                ...
                FT_PtrDist s1len =
                    s2 - s1 - 1; /* without the final NULL byte */
                FT_PtrDist l;
...
#ifdef TT_CONFIG_OPTION_GX_VAR_SUPPORT
        {
            ...
            if (FT_HAS_MULTIPLE_MASTERS(cffface) && mm && instance_index > 0)
            {
...

        {
            ...
            temp = matrix->yy ? FT_ABS(matrix->yy) : FT_ABS(matrix->yx);
            ...
            if (sub->has_font_matrix)
            {
                ...
                if (top->has_font_matrix)
                {
                    ...
                    FT_Matrix_Multiply_Scaled(&top->font_matrix,
                                              &sub->font_matrix, scaling);
                    FT_Vector_Transform_Scaled(&sub->font_offset,
                                               &top->font_matrix, scaling);

                    sub->units_per_em = (FT_ULong)FT_MulDiv(
                        (FT_Long)sub->units_per_em, (FT_Long)top->units_per_em,
                        scaling);
                }
            }
            else
        ...
        if (pure_cff)
        {
            ...
            {
                cffface->family_name =
                    cff_index_get_name(cff, (FT_UInt)(face_index & 0xFFFF));
                if (cffface->family_name)
            ...
            if (cffface->family_name)
            {
                char* full = cff_index_get_sid_string(cff, dict->full_name);
                char* fullp = full;
                ...
                if (full && family)
                {
                    while (*fullp)
                    {
                        ...
                        if (!*family && *fullp)
                        {
                            ...
                            /* remove the style part from the family name (if
                             * present) */
            ...
            {
                char* cid_font_name =
                    cff_index_get_sid_string(cff, dict->cid_font_name);
            ...
            if (dict->is_fixed_pitch)
                ...
                /* XXX: WE DO NOT SUPPORT KERNING METRICS IN THE GPOS TABLE FOR
                 * NOW */
#if 0
        /* kerning available? */
        if ( face->kern_pairs )
          flags |= FT_FACE_FLAG_KERNING;
            ...
            {
                char* weight = cff_index_get_sid_string(cff, dict->weight);
...
cff_driver_init(FT_Module module) /* CFF_Driver */
{
    ...
    seed = (FT_UInt32)((FT_Offset)(char*)&seed ^ (FT_Offset)(char*)&module ^
