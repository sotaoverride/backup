...
FcLangSet* FcFreeTypeLangSet(const FcCharSet* charset,
                             const FcChar8* exclusiveLang)
{
    ...
    for (i = 0; i < NUM_LANG_CHAR_SET; i++)
    {
        ...
        if (FcDebug() & FC_DBG_SCANV)
        {
            ...
            {
                FcCharSet* missed =
                    FcCharSetSubtract(&fcLangCharSets[i].charset, charset);
                FcChar32 ucs4;
...
FcChar8* FcLangNormalize(const FcChar8* lang)
{
    ...
    {
        fprintf(stderr,
                "Fontconfig warning: ignoring %s: not a valid language tag\n",
                lang);
        goto bail0;
    }
    ...
    {
        fprintf(stderr,
                "Fontconfig warning: ignoring %s: not a valid region tag\n",
                lang);
        goto bail0;
    }
    if (territory)
    ...
    if (territory)
    {
        ...
        {
            memmove(territory - 1, territory + tlen,
                    (mlen > 0 ? mlen + 1 : 0) + 1);
            if (modifier)
                modifier = territory;
        }
...
FcLangResult FcLangCompare(const FcChar8* s1, const FcChar8* s2)
{
    ...
    is_und = FcToLower(s1[0]) == 'u' && FcToLower(s1[1]) == 'n' &&
             FcToLower(s1[2]) == 'd' && FcLangEnd(s1[3]);
...
FcLangSet* FcLangSetCopy(const FcLangSet* ls)
{
    ...
    memcpy(new->map, ls->map,
           FC_MIN(sizeof(new->map), ls->map_size * sizeof(ls->map[0])));
    if (ls->extra)
    {
        FcStrList* list;
        FcChar8* extra;
...
static int FcLangSetIndex(const FcChar8* lang)
{
    ...
    while (low <= high)
    {
        ...
        { /* fast path for resolving 2-letter languages (by far the most common)
           * after
           * finding the first char (probably already true because of the hash
           * table) */
            cmp = fcLangCharSets[mid].lang[1] - secondChar;
            if (cmp == 0 &&
                (fcLangCharSets[mid].lang[2] != '\0' || lang[2] != '\0'))
            {
                cmp =
                    FcStrCmpIgnoreCase(fcLangCharSets[mid].lang + 2, lang + 2);
            }
        }
        if (cmp == 0)
            return mid;
...
FcLangSet* FcLangSetPromote(const FcChar8* lang, FcValuePromotionBuffer* vbuf)
{
    ...
    FC_ASSERT_STATIC(sizeof(FcLangSetPromotionBuffer) <=
                     sizeof(FcValuePromotionBuffer));
        ...
        if ((bits = ls->map[i]))
        {
            for (bit = 0; bit <= 31; bit++)
                if (bits & (1U << bit))
                {
                    ...
                    if (!FcStrBufString(
                            buf,
                            fcLangCharSets[fcLangCharSetIndicesInv[id]].lang))
                        return FcFalse;
                    first = FcFalse;
                }
        }
    ...
    for (i = 0; i < count; i++)
    {
        ...
        if (missing)
        {
            for (j = 0; j < 32; j++)
                if (missing & (1U << j))
                {
                    if (!FcLangSetContainsLang(
                            lsa,
                            fcLangCharSets[fcLangCharSetIndicesInv[i * 32 + j]]
                                .lang))
                    {
                        if (FcDebug() & FC_DBG_MATCHV)
                            printf("\tMissing bitmap %s\n",
                                   fcLangCharSets
                                       [fcLangCharSetIndicesInv[i * 32 + j]]
                                           .lang);
                        return FcFalse;
                    }
                }
...
FcLangSet* FcLangSetSerialize(FcSerialize* serialize, const FcLangSet* l)
{
    ...
    memcpy(l_serialize->map, l->map,
           FC_MIN(sizeof(l_serialize->map), l->map_size * sizeof(l->map[0])));
...
static FcLangSet* FcLangSetOperate(const FcLangSet* a, const FcLangSet* b,
