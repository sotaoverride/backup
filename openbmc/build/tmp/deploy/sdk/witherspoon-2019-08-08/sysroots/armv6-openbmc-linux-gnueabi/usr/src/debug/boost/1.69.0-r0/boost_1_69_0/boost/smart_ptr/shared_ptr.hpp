...
class shared_ptr
{
  ...
  public:
    ...
    BOOST_CONSTEXPR
        shared_ptr(boost::detail::sp_internal_constructor_tag,
                   element_type* px_,
...
#include <boost/smart_ptr/detail/operator_bool.hpp>
    ...
    void* _internal_get_local_deleter(
        boost::detail::sp_typeinfo const& ti) const BOOST_SP_NOEXCEPT
    {
...
template <class T, class U>
shared_ptr<T> reinterpret_pointer_cast(shared_ptr<U> const& r) BOOST_SP_NOEXCEPT
{
...
class esft2_deleter_wrapper
{
  ...
  public:
    ...
    template <typename D>
    D* get_deleter() const BOOST_SP_NOEXCEPT
    {
...
template <class T, class M>
shared_ptr<T> inline atomic_exchange_explicit(
    shared_ptr<T>* p, shared_ptr<T> r, /*memory_order mo*/ M) BOOST_SP_NOEXCEPT
{
...
inline bool atomic_compare_exchange_explicit(
    ...
    /*memory_order success*/ M, /*memory_order failure*/ M) BOOST_SP_NOEXCEPT
{
...
D* basic_get_local_deleter(D*, shared_ptr<T> const& p) BOOST_SP_NOEXCEPT
{
    return static_cast<D*>(
        p._internal_get_local_deleter(BOOST_SP_TYPEID(local_sp_deleter<D>)));
}
