...
static TIGHT_PNG_CONF tightPngConf[10] = {
    {0, PNG_NO_FILTERS},  {1, PNG_NO_FILTERS},  {2, PNG_NO_FILTERS},
    {3, PNG_NO_FILTERS},  {4, PNG_NO_FILTERS},  {5, PNG_ALL_FILTERS},
    {6, PNG_ALL_FILTERS}, {7, PNG_ALL_FILTERS}, {8, PNG_ALL_FILTERS},
    {9, PNG_ALL_FILTERS},
};
#endif
...
static const int subsampLevel2tjsubsamp[4] = {TJ_444, TJ_420, TJ_422,
                                              TJ_GRAYSCALE};
...
static rfbBool SendRectEncodingTight(rfbClientPtr cl, int x, int y, int w,
                                     int h);
...
static void FastFillPalette16(rfbClientPtr cl, uint16_t* data, int w, int pitch,
                              int h);
static void FastFillPalette32(rfbClientPtr cl, uint32_t* data, int w, int pitch,
                              int h);
...
static rfbBool SendJpegRect(rfbClientPtr cl, int x, int y, int w, int h,
                            int quality);
static void PrepareRowForImg(rfbClientPtr cl, uint8_t* dst, int x, int y,
                             int count);
static void PrepareRowForImg24(rfbClientPtr cl, uint8_t* dst, int x, int y,
                               int count);
static void PrepareRowForImg16(rfbClientPtr cl, uint8_t* dst, int x, int y,
                               int count);
static void PrepareRowForImg32(rfbClientPtr cl, uint8_t* dst, int x, int y,
                               int count);
...
rfbBool SendRectEncodingTight(rfbClientPtr cl, int x, int y, int w, int h)
{
    int nMaxRows;
    uint32_t colorValue;
    int dx, dy, dw, dh;
    ...
    if (tightBeforeBufSize < 4)
    {
        ...
        if (tightBeforeBuf == NULL)
            ...
            tightBeforeBuf = (char*)realloc(tightBeforeBuf, tightBeforeBufSize);
    }
    ...
    for (dy = y; dy < y + h; dy += MAX_SPLIT_TILE_SIZE)
    {
        ...
        for (dx = x; dx < x + w; dx += MAX_SPLIT_TILE_SIZE)
        {
            ...
            dw = (dx + MAX_SPLIT_TILE_SIZE <= x + w) ? MAX_SPLIT_TILE_SIZE
                                                     : (x + w - dx);
            ...
            if (CheckSolidTile(cl, dx, dy, dw, dh, &colorValue, FALSE))
            {
                ...
                if (y_best != y && !SendRectSimple(cl, x, y, w, y_best - y))
                    return FALSE;
                if (x_best != x &&
                    !SendRectEncodingTight(cl, x, y_best, x_best - x, h_best))
                    return FALSE;
                ...
                if (y_best + h_best != y + h &&
                    !SendRectEncodingTight(cl, x, y_best + h_best, w,
                                           h - (y_best - y) - h_best))
                    return FALSE;
                              ...
                              uint32_t colorValue, int* w_ptr, int* h_ptr)
{
    ...
    for (dy = y; dy < y + h; dy += MAX_SPLIT_TILE_SIZE)
    {
        ...
        {
            dw = (dx + MAX_SPLIT_TILE_SIZE <= x + w_prev) ? MAX_SPLIT_TILE_SIZE
                                                          : (x + w_prev - dx);
                            ...
                            int* w_ptr, int* h_ptr)
{
    ...
    for (cy = *y_ptr - 1; cy >= y && CheckSolidTile(cl, *x_ptr, cy, *w_ptr, 1,
                                                    &colorValue, TRUE);
         cy--)
        ;
    *h_ptr += *y_ptr - (cy + 1);
    ...
    *h_ptr += cy - (*y_ptr + *h_ptr);
    ...
    /* ... to the left. */
    for (cx = *x_ptr - 1; cx >= x && CheckSolidTile(cl, cx, *y_ptr, 1, *h_ptr,
                                                    &colorValue, TRUE);
         cx--)
        ;
    *w_ptr += *x_ptr - (cx + 1);
...
#define DEFINE_CHECK_SOLID_FUNCTION(bpp)                                       \
                                                                               \
    static rfbBool CheckSolidTile##bpp(rfbClientPtr cl, int x, int y, int w,   \
    ...
    {                                                                          \
        ...
        int dx, dy;                                                            \
                                                                               \
        fbptr = (uint##bpp##_t*)&cl->scaledScreen                              \
                    ->frameBuffer[y * cl->scaledScreen->paddedWidthInBytes +   \
                                  x * (bpp / 8)];                              \
                                                                               \
        colorValue = *fbptr;                                                   \
...
static rfbBool SendRectSimple(rfbClientPtr cl, int x, int y, int w, int h)
{
    ...
    if (tightAfterBufSize < maxAfterSize)
    {
        ...
        if (tightAfterBuf == NULL)
            ...
            tightAfterBuf = (char*)realloc(tightAfterBuf, tightAfterBufSize);
    }
...
static rfbBool SendIndexedRect(rfbClientPtr cl, int x, int y, int w, int h)
{
    ...
    switch (cl->format.bitsPerPixel)
    {
        ...
        case 32:
            ...
            {
                ((uint32_t*)tightAfterBuf)[i] = palette.entry[i].listNode->rgb;
            }
            if (usePixelFormat24)
            {
...
#define DEFINE_FILL_PALETTE_FUNCTION(bpp)                                      \
                                                                               \
    static void FillPalette##bpp(int count)                                    \
    {                                                                          \
...
#define DEFINE_FAST_FILL_PALETTE_FUNCTION(bpp)                                 \
                                                                               \
    static void FastFillPalette##bpp(rfbClientPtr cl, uint##bpp##_t* data,     \
    ...
    done2:                                                                     \
        (*cl->translateFn)(cl->translateLookupTable,                           \
                           ...
                           (char*)&c0t, bpp / 8, 1, 1);                        \
        (*cl->translateFn)(cl->translateLookupTable,                           \
                           &cl->screen->serverFormat, &cl->format, (char*)&c1, \
                           (char*)&c1t, bpp / 8, 1, 1);                        \
        if (j2 >= h)                                                           \
        {                                                                      \
            if (n0 > n1)                                                       \
        ...
        {                                                                      \
            ...
            {                                                                  \
                ...
                {                                                              \
                    (*cl->translateFn)(                                        \
                        cl->translateLookupTable, &cl->screen->serverFormat,   \
                        &cl->format, (char*)&ci, (char*)&cit, bpp / 8, 1, 1);  \
            ...
            i2 = 0;                                                            \
                                                                               ...
                                                                               \
        (*cl->translateFn)(cl->translateLookupTable,                           \
                           &cl->screen->serverFormat, &cl->format, (char*)&ci, \
                           (char*)&cit, bpp / 8, 1, 1);                        \
...
static int PaletteInsert(uint32_t rgb, int numPixels, int bpp)
{
    ...
    for (idx = paletteNumColors;
         idx > 0 && palette.entry[idx - 1].numPixels < numPixels; idx--)
    {
...
#define DEFINE_IDX_ENCODE_FUNCTION(bpp)                                        \
                                                                               \
    static void EncodeIndexedRect##bpp(uint8_t* buf, int count)                \
    {                                                                          \
        COLOR_LIST* pnode;                                                     \
        uint##bpp##_t* src;                                                    \
        uint##bpp##_t rgb;                                                     \
...
#define DEFINE_MONO_ENCODE_FUNCTION(bpp)                                       \
                                                                               \
    static void EncodeMonoRect##bpp(uint8_t* buf, int w, int h)                \
    {                                                                          \
        uint##bpp##_t* ptr;                                                    \
        uint##bpp##_t bg;                                                      \
                            ...
                            int quality)
{
    ...
    if (tightAfterBufSize < TJBUFSIZE(w, h))
    {
        if (tightAfterBuf == NULL)
            ...
            tightAfterBuf = (char*)realloc(tightAfterBuf, TJBUFSIZE(w, h));
        if (!tightAfterBuf)
        {
    ...
    if (ps == 2)
    {
        ...
        for (j = 0; j < h; j++)
        {
            ...
            for (i = 0; i < w; i++)
            {
                ...
                *dst2++ = (uint8_t)(
                    (inRed * 255 + cl->screen->serverFormat.redMax / 2) /
                    cl->screen->serverFormat.redMax);
                *dst2++ = (uint8_t)(
                    (inGreen * 255 + cl->screen->serverFormat.greenMax / 2) /
                    cl->screen->serverFormat.greenMax);
                *dst2++ = (uint8_t)(
                    (inBlue * 255 + cl->screen->serverFormat.blueMax / 2) /
                    cl->screen->serverFormat.blueMax);
            }
    ...
    else
    {
        ...
        srcbuf =
            (unsigned char*)&cl->scaledScreen->frameBuffer[y * pitch + x * ps];
    }
                               ...
                               int count)
{
    ...
    fbptr = (uint32_t*)&cl->scaledScreen
                ->frameBuffer[y * cl->scaledScreen->paddedWidthInBytes + x * 4];
...
#define DEFINE_JPEG_GET_ROW_FUNCTION(bpp)                                      \
                                                                               \
    static void PrepareRowForImg##bpp(rfbClientPtr cl, uint8_t* dst, int x,    \
                                      int y, int count)                        \
    {                                                                          \
        uint##bpp##_t* fbptr;                                                  \
        uint##bpp##_t pix;                                                     \
