...
static gboolean poll_pgood(gpointer user_data)
{
    ...
    if (difftime(current_time, pgood_timeout_start) >
    ...
    if (rc == GPIO_OK)
    {
        ...
        if (pgood_state != control_power_get_pgood(control_power))
        {
            ...
            for (i = 0; i < g_gpio_configs.power_gpio.num_reset_outs; i++)
            {
                ...
                reset_state =
                    pgood_state ^ g_gpio_configs.power_gpio.reset_pols[i];
                rc = gpio_open(reset_out, reset_state);
                if (rc != GPIO_OK)
                {
                    gpio_close(reset_out);
                    g_print(
                        "ERROR PowerControl: GPIO open error (gpio=%s,rc=%d)\n",
                        reset_out->name, rc);
                    continue;
                }
            ...
            {
                GPIO* pci_reset_out =
                    &g_gpio_configs.power_gpio.pci_reset_outs[i];
                if (pgood_state == 1)
                {
                    /*
                ...
                reset_state =
                    pgood_state ^ g_gpio_configs.power_gpio.pci_reset_pols[i];
                ...
                g_print("PowerControl: pgood: %d, setting pci reset %s to %d\n",
                        (int)pgood_state, pci_reset_out->name,
                        (int)reset_state);
...
static void on_boot_progress(GDBusConnection* connection,
                             ...
                             gpointer user_data)
{
    ...
    for (i = 0; g_variant_iter_next(properties, "{&sv}", &property, &value);
         i++)
    {
    ...
    ignore = strcmp(boot_progress, "xyz.openbmc_project.State.Boot.Progress."
                                   "ProgressStages.MotherboardInit") == 0;
    ...
    {
        gpio_close(&g_gpio_configs.power_gpio.power_good_in);
        g_print("ERROR PowerControl: on_boot_progress(): GPIO open error "
                "(gpio=%s,rc=%d)\n",
                g_gpio_configs.power_gpio.power_good_in.name, rc);
        return;
    }
                                   ...
                                   guint state, gpointer user_data)
{
    ...
    if (state > 1)
    {
        g_dbus_method_invocation_return_dbus_error(
            invocation, "org.openbmc.ControlPower.Error.Failed",
            "Invalid power state");
        return TRUE;
    }
...
static gboolean on_get_power_state(ControlPower* pwr,
                                   GDBusMethodInvocation* invocation,
                                   gpointer user_data)
{
...
static int set_up_gpio(PowerGpio* power_gpio, ControlPower* control_power)
{
    int error = GPIO_OK;
    int rc;
    int i;
                            ...
                            gpointer user_data)
{
    ...
    g_signal_connect(control_power, "handle-set-power-state",
                     G_CALLBACK(on_set_power_state), object); /* user_data */

    g_signal_connect(control_power, "handle-get-power-state",
                     G_CALLBACK(on_get_power_state), NULL); /* user_data */

    g_signal_connect(control, "handle-init", G_CALLBACK(on_init),
                     object); /* user_data */

    /* Listen for BootProgress signal from BootProgress sensor */
    g_dbus_connection_signal_subscribe(
        connection, NULL,                   /* service */
        ...
        NULL,                               /* arg0 */
        G_DBUS_SIGNAL_FLAGS_NONE, (GDBusSignalCallback)on_boot_progress,
        object, /* user data */
        NULL);
    ...
    {
        g_print(
            "ERROR PowerControl: could not read power GPIO configuration\n");
        exit(-1);
    }
    ...
    {
        g_print(
            "ERROR PowerControl: poll_interval < 500 or pgood_timeout < 5\n");
        exit(-1);
    }
    else
    {
...
gint main(gint argc, gchar* argv[])
{
    ...
    id = g_bus_own_name(DBUS_TYPE, dbus_name,
                        G_BUS_NAME_OWNER_FLAGS_ALLOW_REPLACEMENT |
                            G_BUS_NAME_OWNER_FLAGS_REPLACE,
                        on_bus_acquired, on_name_acquired, on_name_lost, &cmd,
                        NULL);
