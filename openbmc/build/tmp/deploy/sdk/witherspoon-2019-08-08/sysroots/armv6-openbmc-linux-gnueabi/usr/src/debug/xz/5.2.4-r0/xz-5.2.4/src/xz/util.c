...
extern uint64_t str_to_uint64(const char* name, const char* value, uint64_t min,
                              uint64_t max)
{
    uint64_t result = 0;
    ...
    if (*value != '\0')
    {
        ...
        if (*value != '\0' && strcmp(value, "i") != 0 &&
            strcmp(value, "iB") != 0 && strcmp(value, "B") != 0)
            multiplier = 0;

        if (multiplier == 0)
        {
            message(V_ERROR, _("%s: Invalid multiplier suffix"), value - 1);
...
extern const char* uint64_to_nicestr(uint64_t value, enum nicestr_unit unit_min,
                                     ...
                                     bool always_also_bytes, uint32_t slot)
{
    ...
    if ((unit_min == NICESTR_B && value < 10000) || unit_max == NICESTR_B)
    {
