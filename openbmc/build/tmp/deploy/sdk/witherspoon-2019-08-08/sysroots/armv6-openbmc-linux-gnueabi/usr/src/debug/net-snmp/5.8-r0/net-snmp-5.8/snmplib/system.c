...
{
    long return_value = 0;
    DWORD buffersize = (sizeof(PERF_DATA_BLOCK) + sizeof(PERF_OBJECT_TYPE)),
          type = REG_EXPAND_SZ;
    ...
    RegQueryValueEx(HKEY_PERFORMANCE_DATA, "Global", NULL, &type,
    ...
    if (wcsncmp(perfdata->Signature, L"PERF", 4) == 0)
    {
         ...
         */
        return_value =
            (long)((perfdata->PerfTime100nSec.QuadPart / (LONGLONG)100000));
    }
    else
...
in_addr_t get_myaddr(void)
{
    ...
    for (ifrp = ifc.ifc_req; (char*)ifrp < (char*)ifc.ifc_req + ifc.ifc_len;
...
#endif /* IFF_RUNNING */
            && !(ifrp->ifr_flags & IFF_LOOPBACK) && addr != LOOPBACK)
        {
            /*
...
static val_context_t* netsnmp_validator_context(void)
{
    if (NULL == _val_context)
    {
        ...
        DEBUGMSGTL(
            ("dns:sec:context", "creating dnssec context for %s\n", apptype));
...
int netsnmp_gethostbyname_v4(const char* name, in_addr_t* addr_out)
{
...

    {
        ...
        if (addrs != NULL)
        {
            memcpy(addr_out, &((struct sockaddr_in*)addrs->ai_addr)->sin_addr,
                   sizeof(in_addr_t));
            freeaddrinfo(addrs);
        }
        else
        {
            DEBUGMSGTL(("get_thisaddr", "Failed to resolve IPv4 hostname\n"));
        }
        return 0;
        ...
        if (hp == NULL)
        {
            DEBUGMSGTL(("get_thisaddr", "hostname (couldn't resolve)\n"));
            return -1;
        }
        else if (hp->h_addrtype != AF_INET)
        {
            DEBUGMSGTL(("get_thisaddr", "hostname (not AF_INET!)\n"));
            return -1;
        }
        else
        {
            DEBUGMSGTL(("get_thisaddr", "hostname (resolved okay)\n"));
        ...
        {
            DEBUGMSGTL(
                ("get_thisaddr", "hostname (couldn't resolve = %d)\n", err));
            return -1;
        }
        DEBUGMSGTL(("get_thisaddr", "hostname (resolved okay)\n"));
        memcpy(addr_out, hp->h_addr, sizeof(in_addr_t));
...
struct hostent* netsnmp_gethostbyname(const char* name)
{
    ...
    DEBUGMSGTL(("dns:sec:val", "val_status %d / %s; trusted: %d\n", val_status,
                p_val_status(val_status), val_istrusted(val_status)));
    if (!val_istrusted(val_status))
    {
        snmp_log(LOG_WARNING,
                 "The authenticity of DNS response is not trusted (%s)\n",
    ...
    {
        DEBUGMSGTL(("dns:gethostbyname", "%s resolved okay\n", name));
    }
    return hp;
#else
 ...
 */
struct hostent* netsnmp_gethostbyaddr(const void* addr, socklen_t len, int type)
{
    ...
    {
        DEBUGMSGTL(
            ("dns:gethostbyaddr", "warning: response for addr not AF_INET!\n"));
    }
    else
    {
...
netsnmp_feature_child_of(calculate_time_diff, netsnmp_unused)
     ...
     */
    int calculate_time_diff(const struct timeval* now,
 ...
 *
 * This function can be used to test kernels on any platform that supports
 * uname().
 *
 ...
 */
int netsnmp_os_prematch(const char* ospmname, const char* ospmrelprefix)
{
#if HAVE_SYS_UTSNAME_H
...
netsnmp_feature_child_of(str_to_gid, user_information)
     ...
     *
     * @param[in] grouporgid Either a Unix group name or the ASCII
     * representation of a group number.
     *
