                ...
                krb5_error_code errcode, const char* emsg)
{
    ...
    fromstring = inet_ntop(ADDRTYPE2FAMILY(remote_addr->address->addrtype),
                           remote_addr->address->contents, fromstringbuf,
    ...
    if (status == NULL)
    {
        ...
        krb5_klog_syslog(LOG_INFO,
                         _("AS_REQ (%s) %s: ISSUE: authtime %u, %s, "
                           "%s for %s"),
    ...
    else
    {
        ...
        krb5_klog_syslog(LOG_INFO, _("AS_REQ (%s) %s: %s: %s for %s%s%s"),
                         ktypestr, fromstring, status, cname2, sname2,
                         emsg ? ", " : "", emsg ? emsg : "");
    }
    krb5_db_audit_as_req(context, request, local_addr->address,
                         remote_addr->address, client, server, authtime,
                         errcode);
}
 ...
 */
static void unparse_and_limit(krb5_context ctx, krb5_principal princ,
                              char** str)
{
    /* Ignore errors */
                 ...
                 krb5_error_code errcode, const char* emsg)
{
    ...
    fromstring = inet_ntop(ADDRTYPE2FAMILY(from->address->addrtype),
                           from->address->contents, fromstringbuf,
                           sizeof(fromstringbuf));
    if (!fromstring)
        fromstring = "<unknown>";
    ktypes2str(ktypestr, sizeof(ktypestr), request->nktypes, request->ktype);
    ...
    {
        krb5_klog_syslog(LOG_INFO,
                         _("TGS_REQ (%s) %s: %s: authtime %u, %s%s "
                           "%s for %s%s%s"),
        ...
        krb5_klog_syslog(LOG_INFO,
                         _("TGS_REQ %s: %s: authtime %u, %s for %s, "
                           "2nd tkt client %s"),
                         fromstring, status, (unsigned int)authtime, logcname,
                         logsname, logaltcname);
                      ...
                      krb5_error_code errcode)
{
    ...
    if (errcode == KRB5KRB_AP_ERR_ILL_CR_TKT)
        krb5_klog_syslog(LOG_INFO,
                         _("bad realm transit path from '%s' "
                           "to '%s' via '%.*s%s'"),
                         logcname, logsname, tlen, trcont->data, tdots);
    else
    {
        emsg = krb5_get_error_message(ctx, errcode);
        krb5_klog_syslog(LOG_ERR,
                         _("unexpected error checking transit "
                           "from '%s' to '%s' via '%.*s%s': %s"),
                         logcname, logsname, tlen, trcont->data, tdots, emsg);
