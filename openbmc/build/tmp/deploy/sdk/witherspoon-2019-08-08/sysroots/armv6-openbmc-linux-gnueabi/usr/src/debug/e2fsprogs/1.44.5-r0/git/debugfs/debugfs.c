                             ...
                             io_manager* io_ptr)
{
    ...
    {
        retval = errno;
        com_err("debugfs", retval, "while trying to delete %s", tdb_file);
        goto errout;
    }
                            ...
                            char* data_filename, char* undo_file)
{
    ...
    {
        com_err(device, 0,
                "if you specify the superblock, you must also specify the "
                "block size");
        current_fs = NULL;
        return;
    }
...
try_open_again:
    retval = ext2fs_open(device, open_flags, superblock, blocksize, io_ptr,
                         &current_fs);
    ...
    if (retval)
    {
        com_err(debug_prog_name, retval, "while trying to open %s", device);
    ...
    else
    {
        ...
        if (retval)
        {
            com_err(device, retval, "while reading allocation bitmaps");
            goto errout;
        }
    }
    ...
    if (data_io)
    {
        ...
        if (retval)
        {
            com_err(device, retval, "while setting data source");
            goto errout;
        }
    }
...
void do_open_filesys(int argc, char** argv)
{
    ...
    while ((c = getopt(argc, argv, "iwfecb:s:d:Dz:")) != EOF)
    {
        switch (c)
        {
            ...
            case 'D':
                ...
                break;
            case 'b':
                blocksize = parse_ulong(optarg, argv[0], "block size", &err);
                if (err)
                    return;
                break;
            case 's':
                err = strtoblk(argv[0], optarg, "superblock block number",
                               &superblock);
                if (err)
                    return;
                break;
    ...
    if (check_fs_not_open(argv[0]))
        return;
    open_filesystem(argv[optind], open_flags, superblock, blocksize,
                    catastrophic, data_filename, undo_file);
    return;
...
void do_lcd(int argc, char** argv)
{
    ...
    if (chdir(argv[1]) == -1)
    {
        com_err(argv[0], errno, "while trying to change native directory to %s",
                argv[1]);
        return;
    }
}
...
void do_init_filesys(int argc, char** argv)
{
    ...
    if (common_args_process(argc, argv, 3, 3, "initialize", "<device> <blocks>",
                            CHECK_FS_NOTOPEN))
        return;
    ...
    retval =
        ext2fs_initialize(argv[1], 0, &param, unix_io_manager, &current_fs);
    if (retval)
    {
...
void do_show_super_stats(int argc, char* argv[])
{
    ...
    for (i = 0; i < current_fs->group_desc_count; i++)
    {
        fprintf(out,
                ...
                ext2fs_bg_free_blocks_count(current_fs, i) != 1 ? "s" : "",
                ext2fs_bg_free_inodes_count(current_fs, i),
                ext2fs_bg_free_inodes_count(current_fs, i) != 1 ? "inodes"
                                                                : "inode",
        ...
        if (gdt_csum)
            fprintf(out, "%u unused %s\n",
                    ...
                    ext2fs_bg_itable_unused(current_fs, i) != 1 ? "inodes"
                                                                : "inode");
        first = 1;
        ...
        if (gdt_csum)
        {
            fprintf(out, "%sChecksum 0x%04x", first ? "           [" : ", ",
                    ext2fs_bg_checksum(current_fs, i));
            first = 0;
        }
        if (!first)
            fputs("]\n", out);
...
static void finish_range(struct list_blocks_struct* lb)
{
    ...
    if (lb->first_block == lb->last_block)
        fprintf(lb->f, "(%lld):%llu", (long long)lb->first_bcnt,
                lb->first_block);
    else
        fprintf(lb->f, "(%lld-%lld):%llu-%llu", (long long)lb->first_bcnt,
                (long long)lb->last_bcnt, lb->first_block, lb->last_block);
    lb->first_block = 0;
}
...
static int list_blocks_proc(ext2_filsys fs EXT2FS_ATTR((unused)),
                            ...
                            int ref_offset EXT2FS_ATTR((unused)), void* private)
{
    ...
    if (blockcnt >= 0)
    {
         ...
         */
        if (lb->first_block && (lb->last_block + 1 == *blocknr) &&
...
static void internal_dump_inode_extra(
    FILE* out, const char* prefix EXT2FS_ATTR((unused)),
    ext2_ino_t inode_num EXT2FS_ATTR((unused)), struct ext2_inode_large* inode)
{
    fprintf(out, "Size of extra inode fields: %u\n", inode->i_extra_isize);
    if (inode->i_extra_isize >
        EXT2_INODE_SIZE(current_fs->super) - EXT2_GOOD_OLD_INODE_SIZE)
    {
...
static void dump_extents(FILE* f, const char* prefix, ext2_ino_t ino, int flags,
                         int logical_width, int physical_width)
{
    ...
    if (flags & DUMP_EXTENT_TABLE)
        fprintf(f, "Level Entries %*s %*s Length Flags\n",
                (logical_width * 2) + 3, "Logical", (physical_width * 2) + 3,
                "Physical");
    else
        fprintf(f, "%sEXTENTS:\n%s", prefix, prefix);
    ...
    while (1)
    {
        ...
        if (!(extent.e_flags & EXT2_EXTENT_FLAGS_LEAF))
        {
            ...
            if (flags & DUMP_EXTENT_TABLE)
            {
                fprintf(f,
                        "%2d/%2d %3d/%3d %*llu - %*llu "
                        "%*llu%*s %6u\n",
                        info.curr_level, info.max_depth, info.curr_entry,
                        info.num_entries, logical_width, extent.e_lblk,
                        logical_width, extent.e_lblk + (extent.e_len - 1),
                        physical_width, extent.e_pblk, physical_width + 3, "",
                        extent.e_len);
                continue;
            }

            fprintf(f, "%s(ETB%d):%lld", printed ? ", " : "", info.curr_level,
                    extent.e_pblk);
            printed = 1;
            continue;
        }
        ...
        if (flags & DUMP_EXTENT_TABLE)
        {
            fprintf(f,
                    ...
                    info.num_entries, logical_width, extent.e_lblk,
                    logical_width, extent.e_lblk + (extent.e_len - 1),
                    physical_width, extent.e_pblk, physical_width,
                    extent.e_pblk + (extent.e_len - 1), extent.e_len,
                    extent.e_flags & EXT2_EXTENT_FLAGS_UNINIT ? "Uninit" : "");
            continue;
        }
        ...
        if (extent.e_len == 0)
            ...
            fprintf(f, "%s(%lld%s):%lld", printed ? ", " : "", extent.e_lblk,
                    extent.e_flags & EXT2_EXTENT_FLAGS_UNINIT ? "[u]" : "",
                    extent.e_pblk);
        else
            fprintf(f, "%s(%lld-%lld%s):%lld-%lld", printed ? ", " : "",
                    extent.e_lblk, extent.e_lblk + (extent.e_len - 1),
                    extent.e_flags & EXT2_EXTENT_FLAGS_UNINIT ? "[u]" : "",
                    extent.e_pblk, extent.e_pblk + (extent.e_len - 1));
        printed = 1;
    }
    if (printed)
        fprintf(f, "\n");
...
static void dump_inline_data(FILE* out, const char* prefix,
                                ...
                                struct ext2_inode* inode, const char* prefix)
{
    ...
    retval = ext2fs_inline_data_get(current_fs, inode_num, inode, buf, &size);
    if (retval)
        goto out;
...
void internal_dump_inode(FILE* out, const char* prefix, ext2_ino_t inode_num,
                         struct ext2_inode* inode, int do_dump_blocks)
{
    const char* i_type;
    char frag, fsize;
    ...
    if (LINUX_S_ISDIR(inode->i_mode))
        ...
        i_type = "block special";
    else if (LINUX_S_ISCHR(inode->i_mode))
        i_type = "character special";
    ...
    fprintf(out, "%sMode:  0%03o   Flags: 0x%x\n", prefix,
            inode->i_mode & 07777, inode->i_flags);
    if (is_large_inode && large_inode->i_extra_isize >= 24)
    {
        fprintf(out, "%sGeneration: %u    Version: 0x%08x:%08x\n", prefix,
                inode->i_generation, large_inode->i_version_hi,
    ...
    fprintf(out, "%sUser: %5d   Group: %5d", prefix, inode_uid(*inode),
            inode_gid(*inode));
    ...
    if (LINUX_S_ISREG(inode->i_mode))
        ...
        fprintf(out, "%d\n", inode->i_size);
    if (os == EXT2_OS_HURD)
        fprintf(out, "%sFile ACL: %d Translator: %d\n", prefix,
                inode->i_file_acl, inode->osd1.hurd1.h_i_translator);
    else
        fprintf(out, "%sFile ACL: %llu\n", prefix,
                inode->i_file_acl |
                    ((long long)(inode->osd2.linux2.l_i_file_acl_high) << 32));
    if (os != EXT2_OS_HURD)
    ...
    {
        fprintf(
            out, "%s ctime: 0x%08x:%08x -- %s", prefix, inode->i_ctime,
            large_inode->i_ctime_extra,
            inode_time_to_string(inode->i_ctime, large_inode->i_ctime_extra));
        fprintf(
            out, "%s atime: 0x%08x:%08x -- %s", prefix, inode->i_atime,
            large_inode->i_atime_extra,
            inode_time_to_string(inode->i_atime, large_inode->i_atime_extra));
        fprintf(
            out, "%s mtime: 0x%08x:%08x -- %s", prefix, inode->i_mtime,
            large_inode->i_mtime_extra,
            inode_time_to_string(inode->i_mtime, large_inode->i_mtime_extra));
    ...
    if (ext2fs_has_feature_metadata_csum(current_fs->super))
    {
        ...
        if (is_large_inode &&
            large_inode->i_extra_isize >=
                (offsetof(struct ext2_inode_large, i_checksum_hi) -
...
void do_dump_extents(int argc, char** argv)
{
    ...
    logical_width =
        int_log10((EXT2_I_SIZE(&inode) + current_fs->blocksize - 1) /
...
void do_freei(int argc, char* argv[])
{
    ...
    if (len == 1 && !ext2fs_test_inode_bitmap2(current_fs->inode_map, inode))
...
void do_freeb(int argc, char* argv[])
{
    ...
    while (count-- > 0)
    {
        if (!ext2fs_test_block_bitmap2(current_fs->block_map, block))
            com_err(argv[0], 0, "Warning: block %llu already clear", block);
...
void do_setb(int argc, char* argv[])
{
    ...
    {
        if (ext2fs_test_block_bitmap2(current_fs->block_map, block))
            com_err(argv[0], 0, "Warning: block %llu already set", block);
...
#ifndef READ_ONLY
static void modify_u8(char* com, const char* prompt, const char* format,
                      __u8* val)
{
    char buf[200];
    unsigned long v;
...
static void modify_u16(char* com, const char* prompt, const char* format,
                       __u16* val)
{
    char buf[200];
    unsigned long v;
...
static void modify_u32(char* com, const char* prompt, const char* format,
                       __u32* val)
{
    char buf[200];
    unsigned long v;
...
void do_modify_inode(int argc, char* argv[])
{
    ...
    if (os == EXT2_OS_HURD)
        modify_u32(argv[0], "Translator Block", decimal_format,
...
void do_print_working_directory(int argc, char* argv[])
{
    ...
    if (common_args_process(argc, argv, 1, 1, "print_working_directory", "", 0))
        return;

    retval = ext2fs_get_pathname(current_fs, cwd, 0, &pathname);
    if (retval)
    {
        com_err(argv[0], retval, "while trying to get pathname of cwd");
    }
    ...
    if (retval)
    {
        com_err(argv[0], retval, "while trying to get pathname of root");
    }
...
static void make_link(char* sourcename, char* destname)
{
    ...
    retval =
        ext2fs_link(current_fs, dir, dest, ino, ext2_file_type(inode.i_mode));
    if (retval)
...
{
    if (common_args_process(argc, argv, 2, 2, "link", "<pathname>",
                            CHECK_FS_RW))
        return;
...
void do_copy_inode(int argc, char* argv[])
{
    ...
    if (debugfs_read_inode_full(src_ino, (struct ext2_inode*)buf, argv[0],
                                sizeof(buf)))
        return;

    if (debugfs_write_inode_full(dest_ino, (struct ext2_inode*)buf, argv[0],
                                 sizeof(buf)))
        return;
}
...
void do_find_free_block(int argc, char* argv[])
{
    ...
    while (count-- > 0)
    {
        retval = ext2fs_new_block2(current_fs, free_blk + 1, 0, &free_blk);
        if (first_free)
        {
...
void do_mknod(int argc, char* argv[])
{
    ...
    retval =
        do_mknod_internal(current_fs, cwd, argv[1], st.st_mode, st.st_rdev);
    if (retval)
        com_err(argv[0], retval, 0);
}
...
void do_mkdir(int argc, char* argv[])
{
    ...
    if (common_args_process(argc, argv, 2, 2, "mkdir", "<filename>",
                            CHECK_FS_RW))
        return;
...
static void kill_file_by_inode(ext2_ino_t inode)
{
    ...
    if (ext2fs_inode_has_valid_blocks2(current_fs, &inode_buf))
    {
        ...
        ext2fs_block_iterate3(current_fs, inode, BLOCK_FLAG_READ_ONLY, NULL,
                              release_blocks_proc, &last_cluster);
    }
    printf("\n");
...
void do_rm(int argc, char* argv[])
{
    ...
    if (common_args_process(argc, argv, 2, 2, "rm", "<filename>", CHECK_FS_RW))
        return;
...
void do_rmdir(int argc, char* argv[])
{
    ...
    if (common_args_process(argc, argv, 2, 2, "rmdir", "<filename>",
                            CHECK_FS_RW))
        return;
    ...
    retval = ext2fs_dir_iterate2(current_fs, inode_num, 0, 0, rmdir_proc, &rds);
    if (retval)
    {
...
void do_features(int argc, char* argv[])
{
    ...
    for (i = 1; i < argc; i++)
    {
        if (e2p_edit_feature(argv[i], &current_fs->super->s_feature_compat, 0))
...
void do_bmap(int argc, char* argv[])
{
    ...
    {
    print_usage:
        com_err(0, 0, "Usage: bmap [-a] <file> logical_blk [physical_blk]");
        return;
    }
    ...
    if (argc == optind + 1)
    {
        err = strtoblk(argv[0], argv[optind++], "physical block", &pblk);
        if (err)
            return;
        if (flags & BMAP_ALLOC)
        {
    ...
    errcode =
        ext2fs_bmap2(current_fs, ino, 0, 0, flags, blk, &ret_flags, &pblk);
    if (errcode)
    {
        com_err(argv[0], errcode, "while mapping logical block %llu\n", blk);
        return;
    }
    printf("%llu", pblk);
...
void do_imap(int argc, char* argv[])
{
    ...
    if (common_args_process(argc, argv, 2, 2, argv[0], "<file>", 0))
        return;
    ino = string_to_inode(argv[1]);
    if (!ino)
        return;
    ...
    block_nr = ext2fs_inode_table_loc(current_fs, (unsigned)group) + block;
...
void do_idump(int argc, char* argv[])
{
    ...
    while ((c = getopt(argc, argv, "bex")) != EOF)
    {
        if (mode || c == '?')
        {
            com_err(argv[0], 0, "Usage: inode_dump [-b]|[-e] <file>");
            return;
        }
        mode = c;
    }
    ...
    err =
        ext2fs_read_inode_full(current_fs, ino, (struct ext2_inode*)buf, isize);
    if (err)
    {
...
void do_set_current_time(int argc, char* argv[])
{
    ...
    if (common_args_process(argc, argv, 2, 2, argv[0], "<time>", 0))
        return;
...
static int find_supp_feature(__u32* supp, int feature_type, char* name)
{
    ...
    if (name)
    {
        if (feature_type == E2P_FS_FEATURE)
            ...
            ret = e2p_jrnl_string2feature(name, &compat, &feature_mask);
        if (ret)
            return ret;
    ...
    else
    {
        ...
        {
            for (bit = 0, feature_mask = 1; bit < 32; bit++, feature_mask <<= 1)
            {
                if (supp[compat] & feature_mask)
                {
                    if (feature_type == E2P_FS_FEATURE)
                        fprintf(stdout, " %s",
                                e2p_feature2string(compat, feature_mask));
                    else
                        fprintf(stdout, " %s",
                                e2p_jrnl_feature2string(compat, feature_mask));
                }
            }
        }
        fprintf(stdout, "\n");
...
void do_supported_features(int argc, char* argv[])
{
    ...
    if (argc > 1)
    {
        ...
        if (ret)
        {
            ret = find_supp_feature(jrnl_supp, E2P_JOURNAL_FEATURE, argv[1]);
        }
        if (ret)
            com_err(argv[0], 0, "Unknown feature: %s\n", argv[1]);
        else
...
void do_fallocate(int argc, char* argv[])
{
    ...
    errcode = ext2fs_fallocate(current_fs, EXT2_FALLOCATE_INIT_BEYOND_EOF, ino,
                               NULL, ~0ULL, start, end - start + 1);
...
void do_symlink(int argc, char* argv[])
{
    ...
    if (common_args_process(argc, argv, 3, 3, "symlink", "<filename> <target>",
                            CHECK_FS_RW))
        return;
...
void do_dump_mmp(int argc EXT2FS_ATTR((unused)), char* argv[])
{
    ...
    {
        retval = ext2fs_get_mem(current_fs->blocksize, &current_fs->mmp_buf);
        if (retval)
        {
...
int main(int argc, char** argv)
{
    ...
    fprintf(stderr, "%s %s (%s)\n", debug_prog_name, E2FSPROGS_VERSION,
            E2FSPROGS_DATE);
    ...
    if (jbd_debug)
    {
        ...
        if (res != 1)
        {
            fprintf(stderr, "DEBUGFS_JBD_DEBUG \"%s\" not an integer\n\n",
                    jbd_debug);
            exit(1);
        }
    }
    ...
    while ((c = getopt(argc, argv, opt_string)) != EOF)
    {
        switch (c)
        {
            ...
            case 'D':
                ...
                break;
            case 'b':
                blocksize = parse_ulong(optarg, argv[0], "block size", 0);
                break;
            case 's':
                retval = strtoblk(argv[0], optarg, "superblock block number",
                                  &superblock);
                if (retval)
                    return 1;
                break;
