...
BOOST_LOG_ANONYMOUS_NAMESPACE
{
    ...
    struct string_constants<char>
    {
        ...
        static const char_type* iso_date_format()
        {
            return "%Y%m%d";
        }
        static const char_type* extended_iso_date_format()
        {
            return "%Y-%m-%d";
        }

        static const char_type* iso_time_format()
        {
            return "%H%M%S";
        }
        static const char_type* extended_iso_time_format()
        {
            return "%H:%M:%S";
        }
    ...
    struct common_flags
    {
        ...
        const char_type* parse(const char_type* begin, const char_type* end,
                               callback_type& callback)
        {
            switch (begin[1])
            {
                ...
                default:
                    flush(callback);
                    callback.on_placeholder(
                        iterator_range<const char_type*>(begin, begin + 2));
                    break;
            }
        ...
        void flush(callback_type& callback)
        {
            if (!m_literal.empty())
            {
                ...
                callback.on_literal(
                    iterator_range<const char_type*>(p, p + m_literal.size()));
                m_literal.clear();
            }
        }
    ...
    struct date_flags : public BaseT
    {
                               ...
                               callback_type& callback)
        {
            ...
            switch (begin[1])
            {
                case 'Y':
                {
                    ...
                    if (len >= 8 &&
                        std::memcmp(begin,
                                    constants::extended_iso_date_format(),
                                    8 * sizeof(char_type)) == 0)
                    {
                        callback.on_extended_iso_date();
                        return begin + 8;
                    }
                    else if (len >= 6 &&
                             std::memcmp(begin, constants::iso_date_format(),
                                         6 * sizeof(char_type)) == 0)
                    {
    ...
    template <typename BaseT>
    struct time_flags : public BaseT
    {
        typedef typename BaseT::callback_type callback_type;
        typedef typename BaseT::char_type char_type;
                               ...
                               callback_type& callback)
        {
            ...
            switch (begin[1])
            {
                ...
                case 'H':
                {
                    ...
                    std::size_t len = end - begin;
                    if (len >= 11 &&
                        std::memcmp(begin, constants::default_time_format(),
                                    11 * sizeof(char_type)) == 0)
                    {
                        ...
                        return begin + 11;
                             ...
                             std::memcmp(begin,
                                         constants::extended_iso_time_format(),
                                         8 * sizeof(char_type)) == 0)
                    {
                        ...
                        return begin + 8;
                             ...
                             std::memcmp(begin, constants::iso_time_format(),
                                         6 * sizeof(char_type)) == 0)
                    {
    ...
    inline void parse_format(const CharT* begin, const CharT* end,
                             ParserT& parser, CallbackT& callback)
    {
        ...
        while (begin != end)
        {
            ...
            else
            {
                if (p != end)
                    parser.add_literal(
                        p,
                        end); // a single '%' character at the end of the string
                begin = end;
            }
        }
...
template <typename CharT>
BOOST_LOG_API void
    put_integer(boost::log::aux::basic_ostringstreambuf<CharT>& strbuf,
                uint32_t value, unsigned int width, CharT fill_char)
{
    typedef CharT char_type;
...
template BOOST_LOG_API void
    parse_date_format(const char* begin, const char* end,
                      date_format_parser_callback<char>& callback);
template BOOST_LOG_API void
    parse_time_format(const char* begin, const char* end,
                      time_format_parser_callback<char>& callback);
template BOOST_LOG_API void
    parse_date_time_format(const char* begin, const char* end,
                           date_time_format_parser_callback<char>& callback);
template BOOST_LOG_API void
    put_integer(boost::log::aux::basic_ostringstreambuf<char>& strbuf,
                uint32_t value, unsigned int width, char fill_char);
...
template BOOST_LOG_API void
    parse_date_format(const wchar_t* begin, const wchar_t* end,
                      date_format_parser_callback<wchar_t>& callback);
template BOOST_LOG_API void
    parse_time_format(const wchar_t* begin, const wchar_t* end,
                      time_format_parser_callback<wchar_t>& callback);
template BOOST_LOG_API void
    parse_date_time_format(const wchar_t* begin, const wchar_t* end,
                           date_time_format_parser_callback<wchar_t>& callback);
template BOOST_LOG_API void
    put_integer(boost::log::aux::basic_ostringstreambuf<wchar_t>& strbuf,
                uint32_t value, unsigned int width, wchar_t fill_char);
