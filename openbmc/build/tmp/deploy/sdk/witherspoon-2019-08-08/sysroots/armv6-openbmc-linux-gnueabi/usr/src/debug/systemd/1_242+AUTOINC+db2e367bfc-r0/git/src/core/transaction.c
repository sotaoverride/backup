...
static void transaction_unlink_job(Transaction* tr, Job* j,
                                   bool delete_dependencies);

static void transaction_delete_job(Transaction* tr, Job* j,
                                   bool delete_dependencies)
{
    assert(tr);
    assert(j);
...
static void transaction_find_jobs_that_matter_to_anchor(Job* j,
                                                        unsigned generation)
{
    JobDependency* l;
...
static void transaction_merge_and_delete_job(Transaction* tr, Job* j,
                                             Job* other, JobType t)
{
...
static int delete_one_unmergeable_job(Transaction* tr, Job* j)
{
    ...
    LIST_FOREACH(transaction, k, j->transaction_next)
    {
        ...
        if (!j->matters_to_anchor && !k->matters_to_anchor)
        {
            ...
            log_unit_debug(
                j->unit, "Looking at job %s/%s conflicted_by=%s", j->unit->id,
                job_type_to_string(j->type),
                yes_no(j->type == JOB_STOP && job_is_conflicted_by(j)));
            log_unit_debug(
                k->unit, "Looking at job %s/%s conflicted_by=%s", k->unit->id,
        ...
        log_unit_debug(
            ...
            j->unit->id, job_type_to_string(j->type), k->unit->id,
            job_type_to_string(k->type), d->unit->id,
...
static void transaction_drop_redundant(Transaction* tr)
{
    ...
    /* Goes through the transaction and removes all jobs of the units whose jobs
     * are all noops. If not
    ...
    do
    {
        ...
        HASHMAP_FOREACH(j, tr->jobs, i)
        {
            ...
            if (tr->anchor_job == k ||
                ...
                (k->unit->job &&
                 job_type_is_conflicting(k->type, k->unit->job->type)))
            {
                keep = true;
                break;
            }
            ...
            {
                log_trace(
                    "Found redundant job %s/%s, dropping from transaction.",
...
static int transaction_verify_order_one(Transaction* tr, Job* j, Job* from,
                                        unsigned generation, sd_bus_error* e)
{
    Iterator i;
    Unit* u;
    void* v;
    ...
    if (j->generation == generation)
    {
        ...
        for (k = from; k;
             k = ((k->generation == generation && k->marker != k) ? k->marker
                                                                  : NULL))
        {

            /* For logging below */
            if (strv_push_pair(&array, k->unit->id,
                               (char*)job_type_to_string(k->type)) < 0)
                log_oom();
        ...
        unit_ids = merge_unit_ids(j->manager->unit_log_field,
                                  array); /* ignore error */

        STRV_FOREACH_PAIR(unit_id, job_type, array)
        /* logging for j not k here to provide a consistent narrative */
        log_struct(LOG_WARNING, "MESSAGE=%s: Found %s on %s/%s", j->unit->id,
                   unit_id == array ? "ordering cycle" : "dependency", *unit_id,
                   *job_type, unit_ids);
        ...
        if (delete)
        {
            ...
            /* logging for j not k here to provide a consistent narrative */
            log_struct(LOG_ERR,
                       "MESSAGE=%s: Job %s/%s deleted to break ordering cycle "
                       "starting with %s/%s",
                       j->unit->id, delete->unit->id,
                       job_type_to_string(delete->type), j->unit->id,
                       job_type_to_string(j->type), unit_ids);
        ...
        log_struct(
            LOG_ERR, "MESSAGE=%s: Unable to break cycle starting with %s/%s",
            j->unit->id, j->unit->id, job_type_to_string(j->type), unit_ids);
...
static void transaction_collect_garbage(Transaction* tr)
{
    ...
    do
    {
        ...
        HASHMAP_FOREACH(j, tr->jobs, i)
        {
            ...
            if (!j->object_list)
            {
                log_trace("Garbage collecting job %s/%s", j->unit->id,
                          job_type_to_string(j->type));
                transaction_delete_job(tr, j, true);
                again = true;
                break;
            ...
            log_trace("Keeping job %s/%s because of %s/%s", j->unit->id,
                      job_type_to_string(j->type),
                      j->object_list->subject
                          ? j->object_list->subject->unit->id
                          : "root",
                      j->object_list->subject
                          ? job_type_to_string(j->object_list->subject->type)
                          : "root");
        }
                                      ...
                                      sd_bus_error* e)
{
    ...
    HASHMAP_FOREACH(j, tr->jobs, i)
    {
        ...
        if (j->unit->job && (mode == JOB_FAIL || j->unit->job->irreversible) &&
            ...
            return sd_bus_error_setf(
                e, BUS_ERROR_TRANSACTION_IS_DESTRUCTIVE,
                "Transaction for %s/%s is destructive (%s has '%s' job queued, "
                "but '%s' is included in transaction).",
                tr->anchor_job->unit->id,
                job_type_to_string(tr->anchor_job->type), j->unit->id,
    ...
    HASHMAP_FOREACH(j, tr->jobs, i)
    {
        LIST_FOREACH(transaction, j, j)
        {
            ...
            if (stops_running_service)
                log_unit_debug(j->unit, "%s/%s would stop a running service.",
                               j->unit->id, job_type_to_string(j->type));

            if (changes_existing_job)
                log_unit_debug(j->unit, "%s/%s would change existing job.",
                               j->unit->id, job_type_to_string(j->type));

            /* Ok, let's get rid of this */
            log_unit_debug(j->unit, "Deleting %s/%s to minimize impact.",
                           j->unit->id, job_type_to_string(j->type));
                             ...
                             Set* affected_jobs)
{
    ...
    while ((j = hashmap_steal_first(tr->jobs)))
    {
        ...
        /* When 'affected' is specified, let's track all in it all jobs that
         * were touched because of this transaction. */
        if (affected_jobs)
...
int transaction_activate(Transaction* tr, Manager* m, JobMode mode,
                         Set* affected_jobs, sd_bus_error* e)
{
    ...
    /* Reset the generation counter of all installed jobs. The detection of
     * cycles looks at installed jobs. If they had a non-zero generation from
     * some previous walk of the graph, the algorithm would break. */
    HASHMAP_FOREACH(j, m->jobs, i)
    j->generation = 0;
    ...
    for (;;)
    {
        ...
        if (r != -EAGAIN)
            return log_warning_errno(r,
                                     "Requested transaction contains an "
                                     "unfixable cyclic ordering dependency: %s",
    ...
    for (;;)
    {
        ...
        if (r != -EAGAIN)
            return log_warning_errno(
                r, "Requested transaction contains unmergeable jobs: %s",
                bus_error_message(e, r));
    ...
    /* Eights step: Drop redundant jobs again, if the merging now allows us to
     * drop more. */
    transaction_drop_redundant(tr);
    ...
    if (r < 0)
        return log_notice_errno(
            r, "Requested transaction contradicts existing jobs: %s",
            bus_error_message(e, r));
    ...
    if (!hashmap_isempty(m->jobs))
    {
        ...
        if (m->idle_pipe[0] < 0 && m->idle_pipe[1] < 0 && m->idle_pipe[2] < 0 &&
            m->idle_pipe[3] < 0)
        {
...
static Job* transaction_add_one_job(Transaction* tr, JobType type, Unit* unit,
                                    bool* is_new)
{
    Job *j, *f;
    ...
    log_trace("Added job %s/%s to transaction.", unit->id,
...
void transaction_add_propagate_reload_jobs(Transaction* tr, Unit* unit, Job* by,
                                           bool ignore_order, sd_bus_error* e)
{
    Iterator i;
    JobType nt;
    Unit* dep;
                        ...
                        i)
    {
        ...
        r = transaction_add_job_and_dependencies(tr, nt, dep, by, false, false,
                                         ...
                                         bool ignore_order, sd_bus_error* e)
{
    ...
    /* Before adding jobs for this unit, let's ensure that its state has been
     * loaded This matters when jobs are spawned as part of coldplugging itself
     * (see e. g. path_coldplug()). This way, we "recursively" coldplug units,
     * ensuring that we do not look at state of not-yet-coldplugged units. */
    if (MANAGER_IS_RELOADING(unit->manager))
        unit_coldplug(unit);

    if (by)
        log_trace("Pulling in %s/%s from %s/%s", unit->id,
                  job_type_to_string(type), by->unit->id,
                  job_type_to_string(by->type));

    /* Safety check that the unit is a valid state, i.e. not in UNIT_STUB or
     * UNIT_MERGED which should only be set temporarily. */
    if (!IN_SET(unit->load_state, UNIT_LOADED, UNIT_ERROR, UNIT_NOT_FOUND,
                UNIT_BAD_SETTING, UNIT_MASKED))
    ...
    if (is_new && !ignore_requirements && type != JOB_NOP)
    {
        ...
        if (unit_following_set(ret->unit, &following) > 0)
        {
            SET_FOREACH(dep, following, i)
            {
                ...
                if (r < 0)
                {
                    log_unit_full(dep, r == -ERFKILL ? LOG_INFO : LOG_WARNING,
        ...
        if (IN_SET(type, JOB_START, JOB_RESTART))
        {
            ...
            {
                r = transaction_add_job_and_dependencies(
                    tr, JOB_START, dep, ret, true, false, false, ignore_order,
                    e);
                if (r < 0)
                {
            ...
            {
                r = transaction_add_job_and_dependencies(
                    tr, JOB_START, dep, ret, false, false, false, ignore_order,
                    e);
                if (r < 0)
                {
                    /* unit masked, job type not applicable and unit not found
                     * are not considered as errors. */
                    log_unit_full(dep,
                                  IN_SET(r, -ERFKILL, -EBADR, -ENOENT)
                                      ? LOG_DEBUG
                                      : LOG_WARNING,
                                  r, "Cannot add dependency job, ignoring: %s",
                                  bus_error_message(e, r));
            ...
            {
                r = transaction_add_job_and_dependencies(
                    tr, JOB_VERIFY_ACTIVE, dep, ret, true, false, false,
                    ignore_order, e);
                if (r < 0)
                {
                    if (r != -EBADR) /* job type not applicable */
            ...
            HASHMAP_FOREACH_KEY(v, dep,
                                ret->unit->dependencies[UNIT_CONFLICTED_BY], i)
            {
                r = transaction_add_job_and_dependencies(tr, JOB_STOP, dep, ret,
                                                         false, false, false,
                                                         ignore_order, e);
                if (r < 0)
                {
        ...
        if (IN_SET(type, JOB_STOP, JOB_RESTART))
        {
            ...
            for (j = 0; j < ELEMENTSOF(propagate_deps); j++)
                HASHMAP_FOREACH_KEY(
                    v, dep, ret->unit->dependencies[propagate_deps[j]], i)
                {
                    JobType nt;
        ...
        if (type == JOB_RELOAD)
            transaction_add_propagate_reload_jobs(tr, ret->unit, ret,
                                                  ignore_order, e);
