...
void lzma_base::after(const char*& src_begin, char*& dest_begin, bool)
{
    lzma_stream* s = static_cast<lzma_stream*>(stream_);
    src_begin =
        const_cast<const char*>(reinterpret_cast<const char*>(s->next_in));
    dest_begin = reinterpret_cast<char*>(s->next_out);
}

int lzma_base::deflate(int action)
{
    return lzma_code(static_cast<lzma_stream*>(stream_),
                     static_cast<lzma_action>(action));
}
...
void lzma_base::do_init(const lzma_params& p, bool compress, lzma::alloc_func,
                        lzma::free_func, void*)
{
    lzma_stream* s = static_cast<lzma_stream*>(stream_);
