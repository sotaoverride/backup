...
netsnmp_feature_child_of(table_dataset_all, mib_helpers) netsnmp_feature_child_of(
    table_dataset,
    table_dataset_all) netsnmp_feature_child_of(table_dataset_remove_row,
                                                table_dataset_all)
    netsnmp_feature_child_of(
        table_data_set_column,
        table_dataset_all) netsnmp_feature_child_of(table_dataset_get_newrow,
                                                    table_dataset_all)
        netsnmp_feature_child_of(
            table_set_add_indexes,
            table_dataset_all) netsnmp_feature_child_of(delete_table_data_set,
                                                        table_dataset_all)
            netsnmp_feature_child_of(table_set_multi_add_default_row,
                                     table_dataset_all)
                netsnmp_feature_child_of(
                    table_dataset_unregister_auto_data_table, table_dataset_all)
...
{
#ifndef NETSNMP_DISABLE_MIB_LOADING
    register_app_config_handler("table", netsnmp_config_parse_table_set, NULL,
                                "tableoid");
#endif /* NETSNMP_DISABLE_MIB_LOADING */
    register_app_config_handler("add_row", netsnmp_config_parse_add_row, NULL,
                                "table_name indexes... values...");
}
...
netsnmp_table_row* netsnmp_table_data_set_clone_row(netsnmp_table_row* row)
{
    ...
    if (data)
    {
        for (newrowdata = (netsnmp_table_data_set_storage**)&(newrow->data);
             data; newrowdata = &((*newrowdata)->next), data = data->next)
        {

            *newrowdata =
                netsnmp_memdup(data, sizeof(netsnmp_table_data_set_storage));
            if (!*newrowdata)
            {
...
int netsnmp_table_set_add_default_row(netsnmp_table_data_set* table_set,
                                      unsigned int column, int type,
                                      int writable, void* default_value,
                                      size_t default_value_len)
{
    ...
    else
    {
        ...
        for (ptr = table_set->default_row, pptr = NULL; ptr;
        ...
        if (pptr)
            ...
            snmp_log(LOG_ERR,
                     "Shouldn't have gotten here: table_dataset/add_row");
    }
    return SNMPERR_SUCCESS;
}
...
void netsnmp_table_set_multi_add_default_row(netsnmp_table_data_set* tset, ...)
{
    ...
    while ((column = va_arg(debugargs, unsigned int)) != 0)
    {
        ...
        netsnmp_table_set_add_default_row(tset, column, type, writable, data,
                                          data_len);
    }
    ...
    netsnmp_get_table_data_set_handler(netsnmp_table_data_set* data_set)
{
    ...
    ret = netsnmp_create_handler(TABLE_DATA_SET_NAME,
                                    ...
                                    netsnmp_table_registration_info* table_info)
{
    ...
    ret = netsnmp_register_table_data(reginfo, data_set->table, table_info);
...
newrow_stash* netsnmp_table_data_set_create_newrowstash(
    netsnmp_table_data_set* datatable, netsnmp_table_request_info* table_info)
{
...
int netsnmp_table_data_set_helper_handler(netsnmp_mib_handler* handler,
                                          ...
                                          netsnmp_request_info* requests)
{
    ...
    for (request = requests; request; request = request->next)
    {
        ...
        if (MODE_IS_SET(reqinfo->mode))
        {
             ...
             */
            stashp = netsnmp_table_dataset_get_or_create_stash(
                reqinfo, datatable, table_info);
            if (NULL == stashp)
            {
            ...
            newrowstash = (newrow_stash*)netsnmp_oid_stash_get_data(
                *stashp, suffix, suffix_len);
            ...
            if (!newrowstash)
            {
                if (!row)
                {
                    if (datatable->allow_creation)
                    {
                         ...
                         */
                        newrowstash = netsnmp_table_data_set_create_newrowstash(
        ...
        switch (reqinfo->mode)
        {
            ...
            case MODE_GETBULK: /* XXXWWW */
                ...
                {
                    /*
                     * Note: data->data.voidp can be NULL, e.g. when a
                     * zero-length octet string has been stored in the table
                     * cache.
                     */
                    netsnmp_table_data_build_result(
                        reginfo, reqinfo, request, row, table_info->colnum,
                        data->type, (u_char*)data->data.voidp, data->data_len);
                }
                break;
                ...
                else if (datatable->rowstatus_column == table_info->colnum)
                {
                    ...
                    switch (*(request->requestvb->val.integer))
                    {
                        ...
                        case RS_CREATEANDWAIT:
                            ...
                            newrow = newrowstash->newrow;
                            row = newrow;
                            netsnmp_oid_stash_add_data(stashp, suffix,
                                                       suffix_len, newrowstash);
                    }
                }
                break;
            ...
            case MODE_SET_RESERVE2:
                /*
                 * If the agent receives a SET request for an object in a
                 * non-existant row, then the RESERVE1 pass will create the row
                 * automatically.
                 *
                 * But since the row doesn't exist at that point, the test for
                 * whether the object is writable or not will be skipped.  So we
                 * need to check for this possibility again here.
                 *
                 * Similarly, if row creation is under the control of the
                 * RowStatus object (i.e. allow_creation == 0), but this
                 * particular request doesn't include such an object, then the
                 * row won't have been created, and the writable check will also
                 * have been skipped.  Again - check here.
                 */
                ...
                data->data.string = (u_char*)netsnmp_strdup_and_null(
            ...
            case MODE_SET_ACTION:
                ...
                if (newrowstash->state != STATE_ACTION)
                {
                    ...
                    {
                        netsnmp_table_dataset_replace_row(datatable, row,
                                                          newrow);
                    }
                }
                /*
                ...
                if (newrowstash->created)
                {
                    netsnmp_request_add_list_data(
                        request, netsnmp_create_data_list(TABLE_DATA_NAME,
                                                          newrow, NULL));
                }
                break;
            ...
            case MODE_SET_UNDO:
                ...
                if (newrowstash->state != STATE_UNDO)
                {
                    ...
                    {
                        netsnmp_table_dataset_replace_row(datatable, newrow,
                                                          row);
                        netsnmp_table_dataset_delete_row(newrow);
                    }
                    newrow = NULL;
            ...
            case MODE_SET_COMMIT:
                if (newrowstash->state != STATE_COMMIT)
                {
                    ...
                    if (!newrowstash->created)
                    {
                        ...
                        /* Walk the request list to update the reference to the
                         * old row w/ th new one */
                        for (req = requests; req; req = req->next)
                        {
                             ...
                             */
                            if ((netsnmp_table_row*)netsnmp_extract_table_row(
                                    req) == row)
                            {
...
{
    return (netsnmp_table_data_set*)netsnmp_request_get_list_data(
...
{
    netsnmp_table_data_set_storage* data =
        (netsnmp_table_data_set_storage*)netsnmp_extract_table_row_data(
            request);
    if (data)
    {
                                      ...
                                      char* registration_name)
{
    ...
    netsnmp_add_list_data(
        &auto_tables,
        netsnmp_create_data_list(registration_name, tables, free)); /* XXX */
}
...
#ifndef NETSNMP_DISABLE_MIB_LOADING
static void _table_set_add_indexes(netsnmp_table_data_set* table_set,
                                   struct tree* tp)
{
    ...
    for (index = tp->indexes; index; index = index->next)
    {
        ...
        DEBUGMSGTL(
            ("table_set_add_table", "adding default index of type %d\n", type));
...
void netsnmp_config_parse_table_set(const char* token, char* line)
{
    ...
    if (NULL == (tp = get_tree(table_name, table_name_length, get_tree_head())))
    {
        config_pwarn("can't instatiate table since "
    ...
    netsnmp_register_table_data_set(
        netsnmp_create_handler_registration(
            line, NULL, table_name, table_name_length, HANDLER_CAN_RWRITE),
        table_set, NULL);
...
void netsnmp_config_parse_add_row(const char* token, char* line)
{
         ...
         vb = vb->next_variable)
    {
        ...
        DEBUGMSGTL(
            ("table_set_add_row", "adding index of type %d\n", vb->type));
        buf_size = sizeof(buf);
...
#ifndef NETSNMP_NO_WRITE_SUPPORT
        if (dr->writable)
            netsnmp_mark_row_column_writable(row, dr->column,
                                             1); /* make writable */
#endif                                           /* !NETSNMP_NO_WRITE_SUPPORT */
    }
...
#ifndef NETSNMP_NO_WRITE_SUPPORT
netsnmp_oid_stash_node** netsnmp_table_dataset_get_or_create_stash(
    netsnmp_agent_request_info* reqinfo, netsnmp_table_data_set* datatable,
    netsnmp_table_request_info* table_info)
{
    netsnmp_oid_stash_node** stashp = NULL;
    ...
    rc = snprintf(buf, sizeof(buf),
                  "dataset_row_stash:%s:", datatable->table->name);
    ...
    stashp = (netsnmp_oid_stash_node**)netsnmp_table_get_or_create_row_stash(
        reqinfo, (u_char*)buf);
    return stashp;
}

#ifndef NETSNMP_FEATURE_REMOVE_TABLE_DATASET_GET_NEWROW
netsnmp_table_row* netsnmp_table_dataset_get_newrow(
    netsnmp_request_info* request, netsnmp_agent_request_info* reqinfo,
    int rootoid_len, netsnmp_table_data_set* datatable,
    netsnmp_table_request_info* table_info)
{
                           ...
                           int type, const void* value, size_t value_len)
{
    ...
    /* Transitions from / to SNMP_NOSUCHINSTANCE are allowed, but no other
     * transitions. */
