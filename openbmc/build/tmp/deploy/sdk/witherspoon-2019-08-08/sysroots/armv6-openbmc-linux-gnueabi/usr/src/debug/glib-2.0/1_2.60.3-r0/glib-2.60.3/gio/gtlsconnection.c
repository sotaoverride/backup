...
G_DEFINE_ABSTRACT_TYPE_WITH_PRIVATE(GTlsConnection, g_tls_connection,
                                    G_TYPE_IO_STREAM)

static void g_tls_connection_get_property(GObject* object, guint prop_id,
                                          GValue* value, GParamSpec* pspec);
static void g_tls_connection_set_property(GObject* object, guint prop_id,
                                          const GValue* value,
                                          GParamSpec* pspec);
...
static void g_tls_connection_class_init(GTlsConnectionClass* klass)
{
     ...
     *
     * The #GIOStream that the connection wraps. The connection holds a
     * reference to this stream, and may run operations on the stream from other
     * threads throughout its lifetime. Consequently, after the #GIOStream has
     * been constructed, application code may only run its own operations on
     * this stream when no #GIOStream operations are running.
     *
     * Since: 2.28
     */
    g_object_class_install_property(
        gobject_class, PROP_BASE_IO_STREAM,
        g_param_spec_object("base-io-stream", P_("Base IOStream"),
     ...
     */
    g_object_class_install_property(
        gobject_class, PROP_USE_SYSTEM_CERTDB,
        g_param_spec_boolean(
            "use-system-certdb", P_("Use system certificate database"),
            P_("Whether to verify peer certificates against the system "
    ...
    g_object_class_install_property(
        ...
        g_param_spec_object("database", P_("Database"),
                            P_("Certificate database to use for looking up or "
     ...
     */
    g_object_class_install_property(
        gobject_class, PROP_REQUIRE_CLOSE_NOTIFY,
        g_param_spec_boolean(
            "require-close-notify", P_("Require close notify"),
            P_("Whether to require proper TLS close notification"), TRUE,
            G_PARAM_READWRITE | G_PARAM_CONSTRUCT | G_PARAM_STATIC_STRINGS));
    /**
     ...
     */
    g_object_class_install_property(
        gobject_class, PROP_REHANDSHAKE_MODE,
        g_param_spec_enum("rehandshake-mode", P_("Rehandshake mode"),
                          P_("When to allow rehandshaking"),
                          G_TYPE_TLS_REHANDSHAKE_MODE, G_TLS_REHANDSHAKE_SAFELY,
     ...
     */
    g_object_class_install_property(
        gobject_class, PROP_CERTIFICATE,
        g_param_spec_object("certificate", P_("Certificate"),
     ...
     */
    g_object_class_install_property(
        gobject_class, PROP_PEER_CERTIFICATE,
        g_param_spec_object("peer-certificate", P_("Peer Certificate"),
     ...
     */
    g_object_class_install_property(
        gobject_class, PROP_PEER_CERTIFICATE_ERRORS,
     ...
     */
    g_object_class_install_property(
        gobject_class, PROP_ADVERTISED_PROTOCOLS,
        g_param_spec_boxed(
            "advertised-protocols", P_("Advertised Protocols"),
     ...
     */
    g_object_class_install_property(
        gobject_class, PROP_NEGOTIATED_PROTOCOL,
        g_param_spec_string(
            "negotiated-protocol", P_("Negotiated Protocol"),
     ...
     * Since: 2.28
     */
    signals[ACCEPT_CERTIFICATE] = g_signal_new(
        I_("accept-certificate"), G_TYPE_TLS_CONNECTION, G_SIGNAL_RUN_LAST,
...
void g_tls_connection_set_database(GTlsConnection* conn, GTlsDatabase* database)
{
    ...
    g_object_set(G_OBJECT(conn), "database", database, NULL);
}
...
GTlsDatabase* g_tls_connection_get_database(GTlsConnection* conn)
{
    ...
    g_object_get(G_OBJECT(conn), "database", &database, NULL);
    if (database)
                                               ...
                                               gboolean require_close_notify)
{
    ...
    g_object_set(G_OBJECT(conn), "require-close-notify", require_close_notify,
                 NULL);
}
...
gboolean g_tls_connection_get_require_close_notify(GTlsConnection* conn)
{
    ...
    g_object_get(G_OBJECT(conn), "require-close-notify", &require_close_notify,
                 NULL);
                                               ...
                                               const gchar* const* protocols)
{
    ...
    g_object_set(G_OBJECT(conn), "advertised-protocols", protocols, NULL);
}
...
const gchar* g_tls_connection_get_negotiated_protocol(GTlsConnection* conn)
{
    ...
    g_object_get(G_OBJECT(conn), "negotiated-protocol", &protocol, NULL);
                                    ...
                                    GCancellable* cancellable, GError** error)
{
    ...
    return G_TLS_CONNECTION_GET_CLASS(conn)->handshake(conn, cancellable,
                                                       error);
}
 ...
 * Since: 2.28
 */
void g_tls_connection_handshake_async(GTlsConnection* conn, int io_priority,
                                      ...
                                      gpointer user_data)
{
    ...
    G_TLS_CONNECTION_GET_CLASS(conn)->handshake_async(
                                           ...
                                           GAsyncResult* result, GError** error)
{
    ...
    return G_TLS_CONNECTION_GET_CLASS(conn)->handshake_finish(conn, result,
                                                              error);
}
                                                  ...
                                                  GTlsCertificateFlags errors)
{
    ...
    g_signal_emit(conn, signals[ACCEPT_CERTIFICATE], 0, peer_cert, errors,
                  &accept);
    return accept;
}
