...
class nullary_function<void()>
{
...
#endif
        impl_type(BOOST_THREAD_RV_REF(F) f_) : f(boost::move(f_))
        {
        }
  ...
  public:
    ...
    explicit nullary_function(void (*f)()) : impl(new impl_type_ptr(f))
    {
    }
    ...
    explicit nullary_function(
        F& f,
        typename disable_if<is_same<typename decay<F>::type, nullary_function>,
                            int*>::type = 0) :
    ...
    nullary_function(
        ...
        typename disable_if<is_same<typename decay<F>::type, nullary_function>,
                            int*>::type = 0) :
        impl(new impl_type<typename decay<F>::type>(
            thread_detail::decay_copy(boost::forward<F>(f))))
    {
    }

    nullary_function() : impl()
    {
    }
    ...
    nullary_function& operator=(BOOST_THREAD_COPY_ASSIGN_REF(nullary_function)
                                    other) BOOST_NOEXCEPT
    {
        impl = other.impl;
        return *this;
    }
    nullary_function& operator=(BOOST_THREAD_RV_REF(nullary_function)
                                    other) BOOST_NOEXCEPT
    {
#if defined BOOST_NO_CXX11_SMART_PTR
        impl = BOOST_THREAD_RV(other).impl;
...
class nullary_function<R()>
{
  ...
  public:
    ...
    nullary_function(R (*f)()) : impl(new impl_type_ptr(f))
    {
    }
#ifdef BOOST_NO_CXX11_RVALUE_REFERENCES
    template <typename F>
    nullary_function(F& f) : impl(new impl_type<F>(f))
    {
    }
#endif
    template <typename F>
...
detail::nullary_function<F> BOOST_THREAD_DCL_MOVABLE_END
} // namespace boost
