...
static int mount_dispatch_timer(sd_event_source* source, usec_t usec,
                                void* userdata);
...
{
    return IN_SET(state, MOUNT_MOUNTING, MOUNT_MOUNTING_DONE, MOUNT_REMOUNTING,
                  MOUNT_REMOUNTING_SIGTERM, MOUNT_REMOUNTING_SIGKILL,
                  MOUNT_UNMOUNTING, MOUNT_UNMOUNTING_SIGTERM,
...
static bool mount_is_bind(const MountParameters* p)
{
    ...
    if (fstab_test_option(p->options, "bind\0"
                                      "rbind\0"))
        return true;
...
static bool mount_needs_quota(const MountParameters* p)
{
    ...
    /* Quotas are not enabled on network filesystems, but we want them, for
     * example, on storage connected via iscsi. We hence don't use
     * mount_is_network() here, as that would also return true for _netdev
     * devices. */
    if (p->fstype && fstype_is_network(p->fstype))
        return false;
    ...
    return fstab_test_option(p->options, "usrquota\0"
                                         "grpquota\0"
                                         "quota\0"
                                         "usrjquota\0"
                                         "grpjquota\0");
...
static int mount_arm_timer(Mount* m, usec_t usec)
{
    ...
    if (m->timer_event_source)
    {
        ...
        return sd_event_source_set_enabled(m->timer_event_source,
                                           SD_EVENT_ONESHOT);
    }
    ...
    r = sd_event_add_time(UNIT(m)->manager->event, &m->timer_event_source,
                          CLOCK_MONOTONIC, usec, 0, mount_dispatch_timer, m);
    if (r < 0)
        return r;
...
static int update_parameters_proc_self_mount_info(Mount* m, const char* what,
                                                  const char* options,
                                                  const char* fstype)
{
...
static int mount_add_mount_dependencies(Mount* m)
{
    ...
    if (!path_equal(m->where, "/"))
    {
        ...
        /* Adds in links to other mount points that might lie further up in the
         * hierarchy */
    ...
    /* Adds in dependencies to other mount points that might be needed for the
     * source path (if this is a bind mount or a loop mount) to be available. */
    ...
    /* Adds in dependencies to other units that use this path or paths further
    ...
    SET_FOREACH(other, s, i)
    {
        ...
        {
            /* If we have fragment configuration, then make this dependency
             * required */
...
static int mount_add_device_dependencies(Mount* m)
{
    ...
    mask = m->from_proc_self_mountinfo ? UNIT_DEPENDENCY_MOUNTINFO_IMPLICIT
...
static int mount_add_quota_dependencies(Mount* m)
{
    ...
    mask = m->from_fragment ? UNIT_DEPENDENCY_FILE
                            : UNIT_DEPENDENCY_MOUNTINFO_IMPLICIT;

    r = unit_add_two_dependencies_by_name(UNIT(m), UNIT_BEFORE, UNIT_WANTS,
                                          SPECIAL_QUOTACHECK_SERVICE, true,
                                          mask);
    if (r < 0)
        return r;

    r = unit_add_two_dependencies_by_name(UNIT(m), UNIT_BEFORE, UNIT_WANTS,
                                          SPECIAL_QUOTAON_SERVICE, true, mask);
    if (r < 0)
        return r;
...
static bool mount_is_extrinsic(Mount* m)
{
     ...
     * the usual start-up and shutdown dependencies. We call them "extrinsic"
     * here, as they are generally mounted outside of the systemd dependency
     * logic. We shouldn't attempt to manage them ourselves but it's fine if the
     * user operates on them with us. */

    if (!MANAGER_IS_SYSTEM(
            UNIT(m)->manager)) /* We only automatically manage mounts if we are
                                  in system mode */
        return true;
    ...
    if (PATH_STARTSWITH_SET(
            m->where, "/run/initramfs", /* This should stay around from before
                                           we boot until after we shutdown */
            "/proc",                    /* All of this is API VFS */
            "/sys",                     /* … dito … */
            "/dev"))                    /* … dito … */
        return true;

    /* If this is an initrd mount, and we are not in the initrd, then leave this
     * around forever, too. */
    p = get_mount_parameters(m);
...
static int mount_add_default_dependencies(Mount* m)
{
    ...
    /* We do not add any default dependencies to /, /usr or /run/initramfs/,
     * since they are guaranteed to stay mounted the whole time, since our
     * system is on it.  Also, don't bother with anything mounted below virtual
     * file systems, it's also going to be virtual, and hence not worth the
     * effort. */
    ...
    nofail = m->from_fragment
                 ? fstab_test_yes_no_option(m->parameters_fragment.options,
                                            "nofail\0"
                                            "fail\0")
                 : false;
    ...
    if (mount_is_network(p))
    {
        ...
        r = unit_add_dependency_by_name(UNIT(m), UNIT_AFTER,
                                        SPECIAL_NETWORK_TARGET, true, mask);
        if (r < 0)
            return r;
    ...
    r = unit_add_two_dependencies_by_name(UNIT(m), UNIT_BEFORE, UNIT_CONFLICTS,
                                          SPECIAL_UMOUNT_TARGET, true, mask);
    if (r < 0)
        return r;

    /* If this is a tmpfs mount then we have to unmount it before we try to
     * deactivate swaps */
    if (streq_ptr(p->fstype, "tmpfs"))
    {
        r = unit_add_dependency_by_name(UNIT(m), UNIT_AFTER,
                                        SPECIAL_SWAP_TARGET, true, mask);
        if (r < 0)
            return r;
    }
...
static int mount_verify(Mount* m)
{
    ...
    if (r < 0)
        return log_unit_error_errno(
            UNIT(m), r, "Failed to generate unit name from mount path: %m");

    if (!unit_has_name(UNIT(m), e))
    {
        log_unit_error(UNIT(m),
    ...
    if (m->exec_context.pam_name &&
        m->kill_context.kill_mode != KILL_CONTROL_GROUP)
    {
        log_unit_error(UNIT(m), "Unit has PAM enabled. Kill mode must be set "
...
static int mount_add_extras(Mount* m)
{
    ...
    /* Note: this call might be called after we already have been loaded once
     * (and even when it has already been activated), in case data from
     * /proc/self/mountinfo has changed. This means all code here needs to be
     * ready to run with an already set up unit. */
...
static int mount_load_root_mount(Unit* u)
{
    ...
    /* The stdio/kmsg bridge socket is on /, in order to avoid a dep loop, don't
     * use kmsg logging for -.mount */
...
static int mount_load(Unit* u)
{
    ...
    /* Add in some extras. Note we do this in all cases (even if we failed to
     * load the unit) when announced by the kernel, because we need some things
     * to be set up no matter what when the kernel establishes a mount and thus
     * we need to update the state in our unit to track it. After all, consider
     * that we don't allow changing the 'slice' field for a unit once it is
     * active. */
    if (u->load_state == UNIT_LOADED || m->from_proc_self_mountinfo ||
        u->perpetual)
...
static int mount_coldplug(Unit* u)
{
    ...
    if (m->control_pid > 0 && pid_is_unwaited(m->control_pid) &&
        MOUNT_STATE_WITH_PROCESS(new_state))
    {
        ...
        r = mount_arm_timer(
            m, usec_add(u->state_change_timestamp.monotonic, m->timeout_usec));
        if (r < 0)
            return r;
    }
...
static void mount_dump(Unit* u, FILE* f, const char* prefix)
{
    ...
    fprintf(f,
            ...
            "%sForceUnmount: %s\n"
            "%sTimeoutSec: %s\n",
            prefix, mount_state_to_string(m->state), prefix,
            mount_result_to_string(m->result), prefix, m->where, prefix,
            p ? strna(p->what) : "n/a", prefix, p ? strna(p->fstype) : "n/a",
            prefix, p ? strna(p->options) : "n/a", prefix,
            yes_no(m->from_proc_self_mountinfo), prefix,
            yes_no(m->from_fragment), prefix, yes_no(mount_is_extrinsic(m)),
            prefix, m->directory_mode, prefix, yes_no(m->sloppy_options),
            prefix, yes_no(m->lazy_unmount), prefix, yes_no(m->force_unmount),
            prefix,
            format_timespan(buf, sizeof(buf), m->timeout_usec, USEC_PER_SEC));
...
static int mount_spawn(Mount* m, ExecCommand* c, pid_t* _pid)
{
    ...
    r = exec_spawn(UNIT(m), c, &m->exec_context, &exec_params, m->exec_runtime,
                   &m->dynamic_creds, &pid);
    if (r < 0)
        return r;
...
static void mount_enter_dead(Mount* m, MountResult f)
{
    ...
    unit_log_result(UNIT(m), m->result == MOUNT_SUCCESS,
    ...
    exec_context_destroy_runtime_directory(
        &m->exec_context, UNIT(m)->manager->prefix[EXEC_DIRECTORY_RUNTIME]);
...
static void mount_enter_dead_or_mounted(Mount* m, MountResult f)
{
    ...
    /* Enter DEAD or MOUNTED state, depending on what the kernel currently says
     * about the mount point. We use this whenever we executed an operation, so
     * that our internal state reflects what the kernel says again, after all
...
static void mount_enter_signal(Mount* m, MountState state, MountResult f)
{
    ...
    r = unit_kill_context(UNIT(m), &m->kill_context,
                          state_to_kill_operation(state), -1, m->control_pid,
                          false);
    if (r < 0)
        goto fail;
...
static void mount_enter_unmounting(Mount* m)
{
    ...
    if (!IN_SET(m->state, MOUNT_UNMOUNTING, MOUNT_UNMOUNTING_SIGTERM,
                MOUNT_UNMOUNTING_SIGKILL))
        m->n_retry_umount = 0;
...
static void mount_enter_mounting(Mount* m)
{
    ...
    if (p)
    {
        ...
        r = fstab_filter_options(p->options,
                                 "nofail\0"
                                 "noauto\0"
                                 "auto\0",
                                 NULL, NULL, &opts);
        ...
        r = exec_command_set(m->control_command, MOUNT_PATH, p->what, m->where,
                             NULL);
...
static int mount_stop(Unit* u)
{
    ...
    switch (m->state)
    {
        ...
        case MOUNT_REMOUNTING:
            /* If we are still waiting for /bin/mount, we go directly into kill
             * mode. */
            mount_enter_signal(m, MOUNT_UNMOUNTING_SIGTERM, MOUNT_SUCCESS);
            return 0;

        case MOUNT_REMOUNTING_SIGTERM:
            /* If we are already waiting for a hung remount, convert this to the
             * matching unmounting state */
...
static int mount_serialize(Unit* u, FILE* f, FDSet* fds)
{
    ...
    (void)serialize_item(f, "reload-result",
                         mount_result_to_string(m->reload_result));
...
static int mount_deserialize_item(Unit* u, const char* key, const char* value,
                                  FDSet* fds)
{
    Mount* m = MOUNT(u);
    int r;
    ...
    else if (streq(key, "n-retry-umount"))
    {
        ...
        if (r < 0)
            log_unit_debug(u, "Failed to parse n-retry-umount value: %s",
                           value);
    }
...
static void mount_sigchld_event(Unit* u, pid_t pid, int code, int status)
{
    ...
    if (IN_SET(m->state, MOUNT_REMOUNTING, MOUNT_REMOUNTING_SIGKILL,
    ...
    {
        exec_status_exit(&m->control_command->exec_status, &m->exec_context,
                         pid, code, status);
    ...
    unit_log_process_exit(
        u, f == MOUNT_SUCCESS ? LOG_DEBUG : LOG_NOTICE, "Mount process",
        mount_exec_command_to_string(m->control_command_id), code, status);

    /* Note that due to the io event priority logic, we can be sure the new
     * mountinfo is loaded
    ...
    switch (m->state)
    {
        ...
        case MOUNT_MOUNTING:
            /* Our mount point has not appeared in mountinfo.  Something went
             * wrong. */
            ...
            if (f == MOUNT_SUCCESS)
            {
                ...
                log_unit_warning(
                    UNIT(m), "Mount process finished, but there is no mount.");
        ...
        case MOUNT_UNMOUNTING:
            ...
            if (f == MOUNT_SUCCESS && m->from_proc_self_mountinfo)
            {
                ...
                /* Still a mount point? If so, let's try again. Most likely
                 * there were multiple mount points stacked on top of each
                 * other. We might exceed the timeout specified by the user
                 * overall, but we will stop as soon as any one umount times
                 * out. */
                ...
                {
                    log_unit_warning(u,
                                     "Mount still present after %u attempts to "
                                     "unmount, giving up.",
                                     m->n_retry_umount);
                    mount_enter_mounted(m, f);
                }
                                ...
                                void* userdata)
{
    ...
    switch (m->state)
    {
        ...
        case MOUNT_MOUNTING_DONE:
            ...
            mount_enter_signal(m, MOUNT_UNMOUNTING_SIGTERM,
                               MOUNT_FAILURE_TIMEOUT);
            break;

        case MOUNT_REMOUNTING:
            log_unit_warning(
                UNIT(m), "Remounting timed out. Terminating remount process.");
        ...
        case MOUNT_REMOUNTING_SIGKILL:
            ...
            log_unit_warning(
                UNIT(m), "Mount process still around after SIGKILL. Ignoring.");
            mount_enter_dead_or_mounted(m, MOUNT_SUCCESS);
            break;
        ...
        case MOUNT_UNMOUNTING_SIGTERM:
            if (m->kill_context.send_sigkill)
            {
                ...
                mount_enter_signal(m, MOUNT_UNMOUNTING_SIGKILL,
                                   MOUNT_FAILURE_TIMEOUT);
            }
            else
            {
...
static int mount_setup_new_unit(Manager* m, const char* name, const char* what,
                                const char* where, const char* options,
                                const char* fstype, MountProcFlags* ret_flags,
                                Unit** ret)
{
     ...
     * Remember this, so that by the time we load the unit file for it (and thus
     * add in extra deps right after) we know what source to attributes the deps
     * to.*/
    MOUNT(u)->from_proc_self_mountinfo = true;

    /* We have only allocated the stub now, let's enqueue this unit for loading
     * now, so that everything else is loaded in now. */
    unit_add_to_load_queue(u);

    *ret_flags = MOUNT_PROC_IS_MOUNTED | MOUNT_PROC_JUST_MOUNTED |
                 MOUNT_PROC_JUST_CHANGED;
    *ret = TAKE_PTR(u);
    return 0;
}

static int mount_setup_existing_unit(Unit* u, const char* what,
                                     const char* where, const char* options,
                                     const char* fstype,
                                     MountProcFlags* ret_flags)
{
    ...
    if (!MOUNT(u)->from_proc_self_mountinfo ||
        FLAGS_SET(MOUNT(u)->proc_flags, MOUNT_PROC_JUST_MOUNTED))
    ...
    {
        /* The unit was previously not found or otherwise not loaded. Now that
         * the unit shows up in /proc/self/mountinfo we should reconsider it
         * this, hence set it to UNIT_LOADED. */
...
static int mount_setup_unit(Manager* m, const char* what, const char* where,
                            const char* options, const char* fstype,
                            bool set_flags)
{
    ...
    if (r < 0)
        return log_error_errno(
            r, "Failed to generate unit name from path '%s': %m", where);
    ...
    if (u)
         ...
         * initiated by a mount unit but rather by the sysadmin having called
         * mount(8) directly. */
        r = mount_setup_new_unit(m, e, what, where, options, fstype, &flags,
                                 &u);
    if (r < 0)
...
static int mount_load_proc_self_mountinfo(Manager* m, bool set_flags)
{
    ...
    for (;;)
    {
        ...
        if (k < 0)
            return log_error_errno(
                k, "Failed to get next entry from /proc/self/mountinfo: %m");
...
static void mount_enumerate_perpetual(Manager* m)
{
    ...
    /* Whatever happens, we know for sure that the root directory is around, and
     * cannot go away. Let's unconditionally synthesize it here and mark it as
     * perpetual. */
...
static bool mount_is_mounted(Mount* m)
{
    ...
    return UNIT(m)->perpetual ||
           FLAGS_SET(m->proc_flags, MOUNT_PROC_IS_MOUNTED);
}
...
static void mount_enumerate(Manager* m)
{
    ...
    if (!m->mount_monitor)
    {
        ...
        {
            log_error_errno(
                r, "Failed to enable watching of kernel mount events: %m");
            goto fail;
        }
        ...
        {
            log_error_errno(
                r, "Failed to enable watching of userspace mount events: %m");
            goto fail;
        }
        ...
        r = sd_event_add_io(m->event, &m->mount_event_source, fd, EPOLLIN,
                            mount_dispatch_io, m);
        if (r < 0)
        {
        ...
        r = sd_event_source_set_priority(m->mount_event_source,
                             ...
                             void* userdata)
{
    ...
    LIST_FOREACH(units_by_type, u, m->units_by_type[UNIT_MOUNT])
    {
        ...
        if (!mount_is_mounted(mount))
        {
                ...
                mount->parameters_proc_self_mountinfo.what)
            {
                ...
                if (set_ensure_allocated(&gone, &path_hash_ops) < 0 ||
                    set_put_strdup(
                        gone, mount->parameters_proc_self_mountinfo.what) < 0)
                    log_oom(); /* we don't care too much about OOM here... */
            }

            mount->from_proc_self_mountinfo = false;
            assert_se(update_parameters_proc_self_mount_info(mount, NULL, NULL,
                                                             NULL) >= 0);
            ...
            switch (mount->state)
            {
                ...
                case MOUNT_MOUNTED:
                    /* This has just been unmounted by somebody else, follow the
                     * state change. */
        ...
        else if (mount->proc_flags &
                 (MOUNT_PROC_JUST_MOUNTED | MOUNT_PROC_JUST_CHANGED))
        {
            ...
            switch (mount->state)
            {
                ...
                case MOUNT_FAILED:
                    ...
                    /* This has just been mounted by somebody else, follow the
                     * state change, but let's
                     * generate a new invocation ID for this implicitly and
                     * automatically. */
        ...
        if (mount_is_mounted(mount) && mount->from_proc_self_mountinfo &&
            mount->parameters_proc_self_mountinfo.what)
        {
            /* Track devices currently used */

            if (set_ensure_allocated(&around, &path_hash_ops) < 0 ||
                set_put_strdup(around,
                               mount->parameters_proc_self_mountinfo.what) < 0)
                log_oom();
        }
