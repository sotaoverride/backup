...
#define XMIT_HLINK_FIRST (1 << 12) /* protocols 30 - now (HLINKED files only)  \
                                    */
#define XMIT_IO_ERROR_ENDLIST                                                  \
    (1 << 12) /* protocols 31*- now (w/XMIT_EXTENDED_FLAGS) (also protocol 30  \
                 w/'f' compat flag) */
...
#define FLAG_OWNED_BY_US                                                       \
    (1 << 0) /* generator: set by make_file() for aux flists only */
...
#define BITS_SETnUNSET(val, onbits, offbits)                                   \
    (((val) & ((onbits) | (offbits))) == (onbits))
#define BITS_EQUAL(b1, b2, mask)                                               \
    (((unsigned)(b1) & (unsigned)(mask)) == ((unsigned)(b2) & (unsigned)(mask)))
...
#define ROUND_UP_1024(siz)                                                     \
    ((siz) & (1024 - 1) ? ((siz) | (1024 - 1)) + 1 : (siz))
...
enum logcode
{
    FNONE = 0, /* never sent */
    FERROR_XFER = 1,
    FINFO = 2, /* sent over socket for any protocol */
    FERROR = 3,
    FWARNING = 4, /* sent over socket for protocols >= 30 */
    FERROR_SOCKET = 5,
    FLOG = 6,        /* only sent via receiver -> generator pipe */
...
enum msgcode
{
    ...
    MSG_ERROR_XFER = FERROR_XFER,
    MSG_INFO = FINFO, /* remote logging */
    MSG_ERROR = FERROR,
    MSG_WARNING = FWARNING,           /* protocol-30 remote logging */
    MSG_ERROR_SOCKET = FERROR_SOCKET, /* sibling logging */
    MSG_ERROR_UTF8 = FERROR_UTF8,     /* sibling logging */
    MSG_LOG = FLOG,
    MSG_CLIENT = FCLIENT, /* sibling logging */
    ...
    MSG_ERROR_EXIT =
        86, /* synchronize an error exit (siblings and protocol >= 31) */
...
#define DEL_MAKE_ROOM                                                          \
    (DEL_FOR_FILE | DEL_FOR_DIR | DEL_FOR_SYMLINK | DEL_FOR_DEVICE |           \
     DEL_FOR_SPECIAL)
...
#define HT_KEY(node, k64)                                                      \
    ((k64) ? ((struct ht_int64_node*)(node))->key                              \
...
#define HLINK_BUMP(f)                                                          \
    ((f)->flags & (FLAG_HLINKED | FLAG_HLINK_DONE) ? inc_recurse + 1 : 0)
...
#else
#define F_LENGTH(f)                                                            \
    ((int64)(f)->len32 + ((f)->flags & FLAG_LENGTH64                           \
                              ? (int64)OPT_EXTRA(f, NSEC_BUMP(f))->unum << 32  \
                              : 0))
#endif
...
#define F_HL_PREV(f)                                                           \
    OPT_EXTRA(f, START_BUMP(f) + inc_recurse)->num /* non-dirs */
#define F_DIR_NODE_P(f)                                                        \
    (&OPT_EXTRA(f, START_BUMP(f) + DIRNODE_EXTRA_CNT - 1)                      \
          ->num) /* sender dirs */
#define F_DIR_RELNAMES_P(f)                                                    \
    (&OPT_EXTRA(f, START_BUMP(f) + DIRNODE_EXTRA_CNT + PTR_EXTRA_CNT - 1)      \
          ->num)                                          /* sender dirs */
#define F_DIR_DEFACL(f) OPT_EXTRA(f, START_BUMP(f))->unum /* receiver dirs */
#define F_DIR_DEV_P(f)                                                         \
    (&OPT_EXTRA(f, START_BUMP(f) + ACL_BUMP(f) + DEV_EXTRA_CNT - 1)            \
          ->unum) /* receiver dirs */

/* This optional item might follow an F_HL_*() item. */
#define F_RDEV_P(f)                                                            \
    (&OPT_EXTRA(f, START_BUMP(f) + HLINK_BUMP(f) + DEV_EXTRA_CNT - 1)->unum)

/* The sum is only present on regular files. */
#define F_SUM(f)                                                               \
    ((char*)OPT_EXTRA(f, START_BUMP(f) + HLINK_BUMP(f) + SUM_EXTRA_CNT - 1))

/* Some utility defines: */
#define F_IS_ACTIVE(f) (f)->basename[0]
#define F_IS_HLINKED(f) ((f)->flags & FLAG_HLINKED)

#define F_HLINK_NOT_FIRST(f)                                                   \
    BITS_SETnUNSET((f)->flags, FLAG_HLINKED, FLAG_HLINK_FIRST)
#define F_HLINK_NOT_LAST(f)                                                    \
    BITS_SETnUNSET((f)->flags, FLAG_HLINKED, FLAG_HLINK_LAST)
...
#define FILTRULE_RECEIVER_SIDE                                                 \
    (1 << 17)                         /* rule applies to the receiving side */
...
struct stats
{
    ...
    int created_files, created_dirs, created_symlinks, created_devices,
        created_specials;
    int deleted_files, deleted_dirs, deleted_symlinks, deleted_devices,
        deleted_specials;
    int xferred_files;
};
...
#define INIT_XBUF(xb, str, ln, sz)                                             \
    (xb).buf = (str), (xb).len = (ln), (xb).size = (sz), (xb).pos = 0
#define INIT_XBUF_STRLEN(xb, str)                                              \
    (xb).buf = (str), (xb).len = strlen((xb).buf), (xb).size = (size_t)-1,     \
    (xb).pos = 0
/* This one is used to make an output xbuf based on a char[] buffer: */
#define INIT_CONST_XBUF(xb, bf)                                                \
    (xb).buf = (bf), (xb).size = sizeof(bf), (xb).len = (xb).pos = 0
