...
#define NEW_ARGV(old, n)                                                       \
    (char**)realloc((char*)old, (unsigned)(n + 2) * sizeof(char*))

char** ss_parse(sci_idx, line_ptr, argc_ptr) int sci_idx;
char* line_ptr;
int* argc_ptr;
{
    char **argv, *cp;
    ...
    *argv = (char*)NULL;
    ...
    while (1)
    {
        ...
        while (parse_mode == QUOTED_STRING)
        {
            if (*line_ptr == '\0')
            {
                ss_error(sci_idx, 0, "Unbalanced quotes in command line");
                free(argv);
                *argc_ptr = 0;
                return NULL;
            }
