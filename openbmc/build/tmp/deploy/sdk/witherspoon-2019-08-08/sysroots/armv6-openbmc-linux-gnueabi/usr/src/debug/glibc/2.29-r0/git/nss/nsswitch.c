...
#define DEFINE_DATABASE(name) {#name, &__nss_##name##_database},
#include "databases.def"
#undef DEFINE_DATABASE
};
...
int __nss_database_lookup(const char* database, const char* alternate_name,
       ...
       only requested once and so this might not be critical.  */
        ...
        if (*ni == NULL && alternate_name != NULL)
            /* We haven't found an entry so far.  Try to find it with the
               alternative name.  */
            for (entry = service_table->entry; entry != NULL;
                 entry = entry->next)
...
libc_hidden_def(__nss_database_lookup)
    ...
    /* -1 == not found
        0 == function found
        1 == finished */
    int __nss_lookup(service_user** ni, const char* fct_name,
...
libc_hidden_def(__nss_lookup)
    ...
    /* -1 == not found
        ...
        1 == finished */
    int __nss_next2(service_user** ni, const char* fct_name,
                    ...
                    int all_values)
{
    ...
    else
    {
        ...
        if (__builtin_expect(
                NSS_STATUS_TRYAGAIN > status || status > NSS_STATUS_RETURN, 0))
...
{
    return p1 == p2 ? 0
                    : strcmp(*(const char* const*)p1, *(const char* const*)p2);
}
...
static int nss_load_library(service_user* ni)
{
    ...
    if (ni->library->lib_handle == NULL)
    {
        ...
        size_t shlen =
            (7 + strlen(ni->name) + 3 + strlen(__nss_shlib_revision) + 1);
        int saved_errno = errno;
        char shlib_name[shlen];

        /* Construct shared object name.  */
        __stpcpy(__stpcpy(__stpcpy(__stpcpy(shlib_name, "libnss_"), ni->name),
                          ".so"),
        ...
        else if (is_nscd)
        {
            ...
            size_t initlen = (5 + strlen(ni->name) + strlen("_init") + 1);
            char init_name[initlen];

            /* Construct the init function name.  */
            __stpcpy(__stpcpy(__stpcpy(init_name, "_nss_"), ni->name), "_init");
        ...
        else
        {
            ...
            *found = known;
            ...
            else
            {
                ...
                size_t namlen =
                    (5 + strlen(ni->name) + 1 + strlen(fct_name) + 1);
                char name[namlen];

                /* Construct the function name.  */
                __stpcpy(
                    __stpcpy(__stpcpy(__stpcpy(name, "_nss_"), ni->name), "_"),
                    fct_name);
...
#define DEFINE_GET(h, nm) {#h "_get" #nm "_r", _nss_##h##_get##nm##_r},
#define DEFINE_GETBY(h, nm, ky)                                                \
    {#h "_get" #nm "by" #ky "_r", _nss_##h##_get##nm##by##ky##_r},
                static struct fct_tbl
                {
                    const char* fname;
                    void* fp;
                } * tp, tbl[] = {
...
static service_user* nss_parse_service_list(const char* line)
{
    ...
    while (1)
    {
        ...
        new_service =
            (service_user*)malloc(sizeof(service_user) + (line - name + 1));
        if (new_service == NULL)
            return result;
...
static void nss_load_all_libraries(const char* service, const char* def)
{
...
static void free_database_entries(name_database_entry* entry)
{
    while (entry != NULL)
    {
