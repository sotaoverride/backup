 ...
 * \date   24.06.2007
 *
 * The header contains an implementation of a basic attribute value
 ...
 *
 * This class can be used as a boilerplate for simple attribute values. The
 * class implements all needed interfaces of attribute values and allows to
 * store a single value of the type specified as a template parameter. The
 * stored value can be dispatched with type dispatching mechanism.
 */
template <typename T>
class attribute_value_impl : public attribute_value::impl
{
  ...
  public:
     ...
     * Constructor with initialization of the stored value
     */
    explicit attribute_value_impl(BOOST_RV_REF(value_type) v) :
...
{
    typedef
        typename remove_cv<typename remove_reference<T>::type>::type value_type;
    return attribute_value(
        new attribute_value_impl<value_type>(boost::forward<T>(v)));
}
