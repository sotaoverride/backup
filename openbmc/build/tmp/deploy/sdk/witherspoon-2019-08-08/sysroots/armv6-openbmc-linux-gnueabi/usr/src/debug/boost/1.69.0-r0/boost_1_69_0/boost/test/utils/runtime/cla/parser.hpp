...
typedef std::vector<boost::reference_wrapper<parameter_cla_id const>>
    param_cla_id_list;
...
struct parameter_trie
{
    ...
    /// If subtrie corresponding to the char c exists returns it otherwise
    /// creates new
        ...
        if (it == m_subtrie.end())
            it = m_subtrie
                     .insert(std::make_pair(
                         c, parameter_trie_ptr(new parameter_trie)))
                     .first;
    ...
    /// Registers candidate parameter for this subtrie. If final, it needs to be
    /// unique
    void add_candidate_id(parameter_cla_id const& param_id,
                          basic_param_ptr param_candidate, bool final)
    {
        BOOST_TEST_I_ASSRT(
            !m_has_final_candidate && (!final || m_id_candidates.empty()),
            conflicting_param()
                << "Parameter cla id " << param_id.m_tag
                << " conflicts with the "
...
static void report_foreing_token(cstring program_name, cstring token)
{
    std::cerr << "Boost.Test WARNING: token \"" << token
              << "\" does not correspond to the Boost.Test argument \n"
              << "                    and should be placed after all "
                 "Boost.Test arguments and the -- separator.\n"
              << "                    For example: " << program_name
              << " --random -- " << token << "\n";
}
...
class parser
{
  public:
    ...
    parser(parameters_store const& parameters,
           Modifiers const& m = nfp::no_params)
#else
...
#endif
    {
        ...
        BOOST_TEST_I_ASSRT(
            algorithm::all_of(m_end_of_param_indicator.begin(),
                              m_end_of_param_indicator.end(),
                              parameter_cla_id::valid_prefix_char),
            invalid_cla_id() << "End of parameters indicator can only consist "
                                "of prefix characters.");

        BOOST_TEST_I_ASSRT(
            algorithm::all_of(m_negation_prefix.begin(),
                              m_negation_prefix.end(),
                              parameter_cla_id::valid_name_char),
            invalid_cla_id()
                << "Negation prefix can only consist of prefix characters.");
    ...
    int parse(int argc, char** argv, runtime::arguments_store& res)
    {
        ...
        cstring::iterator it = unit_test::utils::find_last_of(
            m_program_name.begin(), m_program_name.end(), path_sep.begin(),
            path_sep.end());
        ...
        while (!tr.eoi())
        {
            ...
            // Perform format validations and split the argument into prefix,
            // name and separator False return value indicates end of params
            // indicator is met
            if (!validate_token_format(curr_token, prefix, name,
                                       value_separator, negative_form))
            {
            ...
            // Locate trie corresponding to found prefix and skip it in the
            // input
            trie_ptr curr_trie = m_param_trie[prefix];

            if (!curr_trie)
            {
                //  format_error() << "Unrecognized parameter prefix in the
                //  argument " << tr.current_token()
            ...
            locate_result locate_res =
                locate_parameter(curr_trie, name, curr_token);
            ...
            // Skip validations if parameter has optional value and we are at
            // the end of token
            if (!value_separator.is_empty() ||
                !found_param->p_has_optional_value)
            {
                ...
                BOOST_TEST_I_ASSRT(
                    found_id.m_value_separator == value_separator,
                    format_error(found_param->p_name)
                        << "Invalid separator for the parameter "
                        << found_param->p_name << " in the argument "
                        << tr.current_token());
            ...
            else if ((value_separator.is_empty() &&
                      found_id.m_value_separator.empty()))
            {
                // Deduce value source
                value = curr_token;
                if (value.is_empty())
                {
                    tr.next_token(); // tokenization broke the value, we check
                                     // the next one
                    ...
                    else
                    {
                        ...
                        if (validate_token_format(
                                value_check, prefix_test, name_test,
                                value_separator_test, negative_form_test) &&
            ...
            BOOST_TEST_I_ASSRT(
                !res.has(found_param->p_name) || found_param->p_repeatable,
    ...
    void version(std::ostream& ostr)
    {
        ...
        ostr << "Compiled from Boost version " << BOOST_VERSION / 100000 << "."
    ...
    void usage(std::ostream& ostr, cstring param_name = cstring(),
               bool use_color = true)
    {
        ...
        if (!param_name.is_empty())
        {
            basic_param_ptr param =
                locate_parameter(m_param_trie[help_prefix], param_name, "")
                    .second;
            param->usage(ostr, m_negation_prefix);
        }
        else
        {
            ostr << "\n  The program '" << m_program_name
                 << "' is a Boost.test module containing unit tests.";

            {
                BOOST_TEST_SCOPE_SETCOLOR(use_color, ostr, term_attr::BRIGHT,
                                          term_color::ORIGINAL);
                ostr << "\n\n  Usage\n    ";
            }
    ...
    void help(std::ostream& ostr, parameters_store const& parameters,
              cstring param_name, bool use_color = true)
    {
        namespace utils = unit_test::utils;
        namespace ut_detail = unit_test::ut_detail;
        ...
        runtime::commandline_pretty_print(
            ...
            "All parameters are optional. You can specify parameter value "
            "either "
            "as a command line argument or as a value of its corresponding "
            "environment "
            "variable. If a flag is specified as a command line argument and "
            "an environment variable "
            "at the same time, the command line takes precedence. "
            "The command line argument "
            "support name guessing, and works with shorter names as long as "
        ...
        runtime::commandline_pretty_print(
            ...
            "environment"
            "variable. For an argument '--argument_x=<value>', the "
            "corresponding "
            "environment variable is 'BOOST_TEST_ARGUMENT_X=value");

        ostr << "\n\n  The following parameters are supported:\n";

        BOOST_TEST_FOREACH(parameters_store::storage_type::value_type const&, v,
                           parameters.all())
        {
  ...
  private:
    ...
    void build_trie(parameters_store const& parameters)
    {
                           ...
                           parameters.all())
        {
            ...
            BOOST_TEST_FOREACH(parameter_cla_id const&, id, param->cla_ids())
            {
                ...
                if (!next_trie)
                    next_trie = m_param_trie[id.m_prefix] =
                        trie_ptr(new rt_cla_detail::parameter_trie);
                ...
                for (size_t index = 0; index < id.m_tag.size(); ++index)
                {
                    ...
                    next_trie->add_candidate_id(id, param,
                                                index == (id.m_tag.size() - 1));
                }
            }
        }
    }

    bool validate_token_format(cstring token, cstring& prefix, cstring& name,
                               cstring& separator, bool& negative_form)
    {
        // Match prefix
        ...
        if (name.empty())
        {
            ...
            BOOST_TEST_I_THROW(format_error()
                               << "Invalid format for an actual argument "
                               << token);
        }
        ...
        negative_form =
            !m_negation_prefix.empty() &&
            (name.substr(0, m_negation_prefix.size()) == m_negation_prefix);
        if (negative_form)
    ...
    locate_result locate_parameter(trie_ptr curr_trie, cstring name,
                                   cstring token)
    {
        ...
        BOOST_TEST_FOREACH(char, c, name)
        {
            if (curr_trie)
            {
                ...
                {
                    // Initiate search for typo candicates. We will account for
                    // 'wrong char' typo 'missing char' typo and 'extra char'
                    // typo
                    BOOST_TEST_FOREACH(trie_per_char::value_type const&,
                                       typo_cand, curr_trie->m_subtrie)
                    {
                        // 'wrong char' typo
            ...
            {
                // go over existing typo candidates and see if they are still
                // viable
        ...
        if (!curr_trie)
        {
            ...
            std::set<parameter_cla_id const*>
                unique_typo_candidate; // !! ?? unordered_set
            typo_candidate_names.reserve(typo_candidates.size());
            // !! ??            unique_typo_candidate.reserve(
            // typo_candidates.size() );
            ...
            BOOST_TEST_FOREACH(trie_ptr, trie_cand, typo_candidates)
            {
                ...
                BOOST_TEST_FOREACH(parameter_cla_id const&, param_cand,
                                   trie_cand->m_id_candidates)
                {
            ...
            BOOST_TEST_I_THROW(
                unrecognized_param(std::move(typo_candidate_names))
                << "An unrecognized parameter in the argument " << token);
#else
        ...
        if (curr_trie->m_id_candidates.size() > 1)
        {
            ...
            BOOST_TEST_FOREACH(parameter_cla_id const&, param_id,
                               curr_trie->m_id_candidates)
        ...
        return locate_result(curr_trie->m_id_candidates.back().get(),
                             curr_trie->m_param_candidate);
    }
