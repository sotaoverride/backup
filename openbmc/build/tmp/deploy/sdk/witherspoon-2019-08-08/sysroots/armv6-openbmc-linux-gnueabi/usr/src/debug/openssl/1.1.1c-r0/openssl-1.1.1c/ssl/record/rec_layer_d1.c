...
void DTLS_RECORD_LAYER_set_saved_w_epoch(RECORD_LAYER* rl, unsigned short e)
{
    if (e == rl->d->w_epoch - 1)
    {
        memcpy(rl->d->curr_write_sequence, rl->write_sequence,
               sizeof(rl->write_sequence));
        memcpy(rl->write_sequence, rl->d->last_write_sequence,
               sizeof(rl->write_sequence));
    }
    else if (e == rl->d->w_epoch + 1)
    {
        memcpy(rl->d->last_write_sequence, rl->write_sequence,
               sizeof(unsigned char[8]));
        memcpy(rl->write_sequence, rl->d->curr_write_sequence,
               sizeof(rl->write_sequence));
    }
    rl->d->w_epoch = e;
}
...
#define dtls1_get_unprocessed_record(s)                                        \
    dtls1_retrieve_buffered_record((s), &((s)->rlayer.d->unprocessed_rcds))
...
int do_dtls1_write(SSL* s, int type, const unsigned char* buf, size_t len,
                   int create_empty_fragment, size_t* written)
{
    ...
    *(p++) = type & 0xff;
    ...
    {
        if (!s->method->ssl3_enc->mac(
                s, &wr, &(p[SSL3_RECORD_get_length(&wr) + eivlen]), 1))
        {
            SSLfatal(s, SSL_AD_INTERNAL_ERROR, SSL_F_DO_DTLS1_WRITE,
                     ERR_R_INTERNAL_ERROR);
            return -1;
    ...
    if (SSL_WRITE_ETM(s) && mac_size != 0)
    {
        if (!s->method->ssl3_enc->mac(s, &wr, &(p[SSL3_RECORD_get_length(&wr)]),
                                      1))
        {
            SSLfatal(s, SSL_AD_INTERNAL_ERROR, SSL_F_DO_DTLS1_WRITE,
                     ERR_R_INTERNAL_ERROR);
