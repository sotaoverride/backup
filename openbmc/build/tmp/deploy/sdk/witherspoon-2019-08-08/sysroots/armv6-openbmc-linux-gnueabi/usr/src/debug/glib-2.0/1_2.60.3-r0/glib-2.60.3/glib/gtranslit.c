...
#define get_src_char(array, encoded, index)                                    \
    ((encoded & 0x8000) ? (array)[((encoded)&0xfff) + index] : encoded)
#define get_length(encoded)                                                    \
    ((encoded & 0x8000) ? ((encoded & 0x7000) >> 12) : 1)

#if G_BYTE_ORDER == G_BIG_ENDIAN
#define get_ascii_item(array, encoded)                                         \
    ((encoded & 0x8000) ? &(array)[(encoded)&0xfff]                            \
                        : (gpointer)(((char*)&(encoded)) + 1))
#else
#define get_ascii_item(array, encoded)                                         \
    ((encoded & 0x8000) ? &(array)[(encoded)&0xfff] : (gpointer) & (encoded))
#endif

static const gchar* lookup_in_item(guint item_id, const gunichar* key,
                                   gint* result_len, gint* key_consumed);

static gint compare_mapping_entry(gconstpointer user_data, gconstpointer data)
{
...
static const gchar* lookup_in_mapping(const struct mapping_entry* mapping,
                                      gint mapping_size, const gunichar* key,
                                      gint* result_len, gint* key_consumed)
{
    const struct mapping_entry* hit;

    hit = bsearch(key, mapping, mapping_size, sizeof(struct mapping_entry),
...
static const gchar* lookup_in_chain(const guint8* chain, const gunichar* key,
                                    gint* result_len, gint* key_consumed)
{
    const gchar* result;
                                   ...
                                   gint* result_len, gint* key_consumed)
{
    ...
    else
    {
        ...
        return lookup_in_mapping(mappings_table + range->start, range->length,
                                 key, result_len, key_consumed);
    }
}

static gint compare_locale_entry(gconstpointer user_data, gconstpointer data)
{
...
static gboolean lookup_item_id_for_one_locale(const gchar* key, guint* item_id)
{
    const struct locale_entry* hit;

    hit = bsearch(key, locale_index, G_N_ELEMENTS(locale_index),
                  sizeof(struct locale_entry), compare_locale_entry);
...
static guint lookup_item_id_for_locale(const gchar* locale)
{
...
static guint get_default_item_id(void)
{
