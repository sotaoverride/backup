...
static int generate_machine_id(const char* root, sd_id128_t* ret)
{
    ...
    if (isempty(root))
    {
        ...
        else if (detect_vm() == VIRTUALIZATION_KVM)
        {
            ...
            if (id128_read("/sys/class/dmi/id/product_uuid", ID128_UUID, ret) >=
                0)
            {
...
int machine_id_setup(const char* root, sd_id128_t machine_id, sd_id128_t* ret)
{
            ...
            if (fd < 0)
            {
                if (old_errno == EROFS && errno == ENOENT)
                    return log_error_errno(
                        errno, "System cannot boot: Missing /etc/machine-id "
                               "and /etc is mounted read-only.\n"
                               "Booting up is supported only when:\n"
                               "1) /etc/machine-id exists and is populated.\n"
                               "2) /etc/machine-id exists and is empty.\n"
                               "3) /etc/machine-id is missing and /etc is "
                               "writable.\n");
                else
    ...
    if (sd_id128_is_null(machine_id))
    {
        ...
        /* Hmm, so, the id currently stored is not useful, then let's generate
         * one */
    ...
    if (writable)
    {
        if (lseek(fd, 0, SEEK_SET) == (off_t)-1)
            return log_error_errno(errno, "Failed to seek %s: %m",
                                   etc_machine_id);

        if (ftruncate(fd, 0) < 0)
            return log_error_errno(errno, "Failed to truncate %s: %m",
                                   etc_machine_id);
    ...
    /* Hmm, we couldn't write it? So let's write it to /run/machine-id as a
     * replacement */
    ...
    if (mount(NULL, etc_machine_id, NULL, MS_BIND | MS_RDONLY | MS_REMOUNT,
        ...
        log_warning_errno(errno,
                          "Failed to make transient %s read-only, ignoring: %m",
                          etc_machine_id);
...
int machine_id_commit(const char* root)
{
    ...
    /* Replaces a tmpfs bind mount of /etc/machine-id by a proper file,
     * atomically. For this, the umount is removed in a mount namespace, a new
     * file is created at the right place. Afterwards the mount is also removed
     * in the original mount namespace, thus revealing the file that was just
     * created. */
    ...
    if (r < 0)
        return log_error_errno(r, "We didn't find a valid machine ID in %s: %m",
                               etc_machine_id);
    ...
    if (r < 0)
        return log_error_errno(r,
                               "Cannot write %s. This is mandatory to get a "
