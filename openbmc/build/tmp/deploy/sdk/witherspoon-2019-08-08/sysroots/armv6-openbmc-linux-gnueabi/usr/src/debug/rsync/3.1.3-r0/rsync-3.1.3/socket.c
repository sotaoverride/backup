                                      ...
                                      char* proxy_user, char* proxy_pass)
{
    ...
    {
        stringjoin(buffer, sizeof buffer, proxy_user, ":", proxy_pass, NULL);
        len = strlen(buffer);

        if ((len * 8 + 5) / 6 >= (int)sizeof authbuf - 3)
        {
            rprintf(FERROR, "authentication information is too long\n");
            return -1;
        }
...
int open_socket_out(char* host, int port, const char* bind_addr, int af_hint)
{
    ...
    if (proxied)
    {
        ...
        if ((cp = strrchr(buffer, '@')) != NULL)
        {
            *cp++ = '\0';
            ...
            {
                rprintf(FERROR, "invalid proxy specification: should be "
                                "USER:PASS@HOST:PORT\n");
                return -1;
            }
            *cp++ = '\0';
    ...
    for (res = res0, j = 0; res; res = res->ai_next, j++)
    {
        ...
        if (bind_addr &&
            try_bind_local(s, res->ai_family, type, bind_addr) == -1)
        {
            close(s);
            s = -1;
            continue;
        ...
        if (proxied && establish_proxy_connection(s, host, port, proxy_user,
                                                  proxy_pass) != 0)
        {
            close(s);
            s = -1;
        ...
        {
            char buf[2048];
            if ((error = getnameinfo(res->ai_addr, res->ai_addrlen, buf,
                                     sizeof buf, NULL, 0, NI_NUMERICHOST)) != 0)
                snprintf(buf, sizeof buf, "*getnameinfo failure: %s*",
                         gai_strerror(error));
                            ...
                            int af_hint)
{
    ...
    if (DEBUG_GTE(CONNECT, 1))
    {
        rprintf(FINFO, "%sopening tcp connection to %s port %d\n",
                prog ? "Using RSYNC_CONNECT_PROG instead of " : "", host, port);
    }
    if (prog)
                           ...
                           int af_hint)
{
    ...
    for (resp = all_ai, i = ecnt = 0; resp; resp = resp->ai_next)
    {
        ...
        if (s == -1)
        {
            int r = asprintf(&errmsgs[ecnt++], "socket(%d,%d,%d) failed: %s\n",
        ...
        {
            if (setsockopt(s, IPPROTO_IPV6, IPV6_V6ONLY, (char*)&one,
                           sizeof one) < 0 &&
    ...
    if (!i)
    {
        rprintf(FERROR, "unable to bind any inbound sockets on port %d\n",
                port);
        free(socks);
        return NULL;
    }
...
void start_accept_loop(int port, int (*fn)(int, int))
{
    ...
    for (i = 0, maxfd = -1; sp[i] >= 0; i++)
    {
        if (listen(sp[i], lp_listen_backlog()) < 0)
        {
            ...
            {
                rprintf(FINFO, "Try using --ipv4 or --ipv6 to avoid this "
                               "listen() error.\n");
            }
#endif
            exit_cleanup(RERR_SOCKETIO);
    ...
    while (1)
    {
        ...
        {
            rsyserr(FERROR, errno, "could not create child server process");
            close(fd);
...
void set_socket_options(int fd, char* options)
{
    ...
    for (tok = strtok(options, " \t,"); tok; tok = strtok(NULL, " \t,"))
    {
        ...
        switch (socket_options[i].opttype)
        {
            ...
            case OPT_INT:
                ret = setsockopt(fd, socket_options[i].level,
                                 socket_options[i].option, (char*)&value,
                                 sizeof(int));
                break;
            ...
            case OPT_ON:
...

                {
                    ...
                    ret = setsockopt(fd, socket_options[i].level,
                                     socket_options[i].option, (char*)&on,
                                     sizeof(int));
                }
                break;
        }
        ...
        if (ret != 0)
        {
            rsyserr(FERROR, errno, "failed to set socket option %s", tok);
        }
    }
...
static int sock_exec(const char* prog)
{
    ...
    {
        close(fd[0]);
        if (dup2(fd[1], STDIN_FILENO) < 0 || dup2(fd[1], STDOUT_FILENO) < 0)
        {
