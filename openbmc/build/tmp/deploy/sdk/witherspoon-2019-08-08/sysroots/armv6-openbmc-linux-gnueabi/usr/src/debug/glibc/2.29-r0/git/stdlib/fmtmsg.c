...
__libc_lock_define_initialized(static, lock)
    ...
    enum {
        ...
        action_mask = 0x08,
        tag_mask = 0x10,
        all_mask = label_mask | severity_mask | text_mask | action_mask |
                   tag_mask
    };
...
static const struct
{
    ...
    {5, "label"}, {8, "severity"}, {4, "text"}, {6, "action"}, {3, "tag"}};
...
static const struct severity_info nosev = {MM_NOSEV, "", NULL};
static const struct severity_info haltsev = {MM_HALT, "HALT",
                                             (struct severity_info*)&nosev};
static const struct severity_info errorsev = {MM_ERROR, "ERROR",
                                              (struct severity_info*)&haltsev};
           ...
           const char* text, const char* action, const char* tag)
{
    ...
    if (severity_rec != NULL)
    {
        ...
        if (classification & MM_PRINT)
        {
            ...
            if (__fxprintf(
                    stderr, "%s%s%s%s%s%s%s%s%s%s\n", do_label ? label : "",
                    need_colon ? ": " : "",
                    do_severity ? severity_rec->string : "",
                    do_severity && (do_text | do_action | do_tag) ? ": " : "",
                    do_text ? text : "",
                    do_text && (do_action | do_tag) ? "\n" : "",
                    do_action ? "TO FIX: " : "", do_action ? action : "",
                    do_action && do_tag ? "  " : "", do_tag ? tag : "") < 0)
    ...
    if (msgverb_var != NULL && msgverb_var[0] != '\0')
    {
        ...
        do
        {
            ...
            for (cnt = 0; cnt < NKEYWORDS; ++cnt)
                if (memcmp(msgverb_var, keywords[cnt].name,
                           keywords[cnt].len) == 0 &&
        ...
        while (sevlevel_var[0] != '\0')
        {
            ...
            if (sevlevel_var < end)
            {
                    ...
                    level > MM_INFO)
                {
                    ...
                    if (new_string != NULL &&
                        (internal_addseverity(level, new_string) != MM_OK))
