 ...
 *  - selection of long, short strings, or numerical month representation
...
{
    typedef boost::date_time::date_names_put<greg_facet_config, charT>
        facet_def;
    typedef boost::date_time::ostream_date_formatter<date, facet_def, charT>
 ...
 * or short strings. Default if no facet is installed is to output a 2 wide
 * numeric value for the month eg: 01 == Jan, 02 == Feb, ... 12 == Dec.
 */
template <class charT, class traits>
inline std::basic_ostream<charT, traits>&
    operator<<(std::basic_ostream<charT, traits>& os, const greg_month& m)
{
    typedef boost::date_time::date_names_put<greg_facet_config, charT>
        facet_def;
    typedef boost::date_time::ostream_month_formatter<facet_def, charT>
...
//! ...
/*! Uses the date facet to determine output string as well as selection of long
 * or short string. Default if no facet is installed is to output a 3 char
 * english string for the day of the week.
 */
template <class charT, class traits>
inline std::basic_ostream<charT, traits>&
    operator<<(std::basic_ostream<charT, traits>& os, const greg_weekday& wd)
{
    typedef boost::date_time::date_names_put<greg_facet_config, charT>
        facet_def;
    typedef boost::date_time::ostream_weekday_formatter<greg_weekday, facet_def,
                                                        charT>
    ...
    { // default to short English string eg: Sun, Mon, Tue, Wed...
...
//! operator<< for gregorian::date_period typical output:
//! [2002-Jan-01/2002-Jan-31]
/*! Uses the date facet to determine output string as well as selection of long
 *  or short string fr dates.
 *  Default if no facet is installed is to output a 3 char english string for
 * the day of the week.
 */
template <class charT, class traits>
inline std::basic_ostream<charT, traits>&
...
{
    os << nkd.nth_week_as_str() << ' ' << nkd.day_of_week() << " of "
...
inline std::basic_istream<charT>& operator>>(std::basic_istream<charT>& is,
                                             date& d)
{
...
//! operator>> for gregorian::greg_month - throws exception if invalid month
//! given
template <class charT>
inline std::basic_istream<charT>& operator>>(std::basic_istream<charT>& is,
                                             greg_month& m)
{
    typedef boost::date_time::all_date_names_put<greg_facet_config, charT>
        facet_def;
    ...
    try
    {
        ...
        num = date_time::find_match(
            f.get_short_month_names(), f.get_long_month_names(),
            (greg_month::max)(),
            s); // greg_month spans 1..12, so max returns the array size,
                                             ...
                                             greg_weekday& wd)
{
    ...
    try
    {
        const facet_def& f = std::use_facet<facet_def>(is.getloc());
        num = date_time::find_match(
            f.get_short_weekday_names(), f.get_long_weekday_names(),
            (greg_weekday::max)() + 1,
            s); // greg_weekday spans 0..6, so increment is needed
