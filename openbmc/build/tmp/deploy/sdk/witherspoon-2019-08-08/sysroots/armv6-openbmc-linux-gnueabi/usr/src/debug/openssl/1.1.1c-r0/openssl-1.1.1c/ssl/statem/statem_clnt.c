...
static int key_exchange_expected(SSL* s)
{
     ...
     */
    if (alg_k &
        (SSL_kDHE | SSL_kECDHE | SSL_kDHEPSK | SSL_kECDHEPSK | SSL_kSRP))
    {
        return 1;
    }
...
static int ossl_statem_client13_read_transition(SSL* s, int mt)
{
    ...
    switch (st->hand_state)
    {
        ...
        case TLS_ST_CW_CLNT_HELLO:
            /*
             * This must a ClientHello following a HelloRetryRequest, so the
             * only thing we can get now is a ServerHello.
             */
...
#error TODO(DTLS1.3): Restore digest for PHA before adding message.
                    ...
                    s->post_handshake_auth == SSL_PHA_EXT_SENT)
                {
                     ...
                     * digest. In DTLS, this is expected to be called after
                     * adding to the digest. Either move the digest restore, or
                     * add the message here after the swap, or do it after the
                     * clientFinished?
                     */
    ...
    switch (st->hand_state)
    {
        ...
        case TLS_ST_CR_SRVR_HELLO:
            ...
            else
            {
                ...
                {
                    /*
                     * Normally, we can tell if the server is resuming the
                     * session from the session ID. EAP-FAST (RFC 4851),
                     * however, relies on the next server message after the
                     * ServerHello to determine if the server is resuming.
                     */
                    s->hit = 1;
                    st->hand_state = TLS_ST_CR_CHANGE;
                    return 1;
    ...
    switch (st->hand_state)
    {
        ...
        case TLS_ST_CR_FINISHED:
            if (s->early_data_state == SSL_EARLY_DATA_WRITE_RETRY ||
                ...
                st->hand_state = (s->s3->tmp.cert_req != 0)
                                     ? TLS_ST_CW_CERT
                                     : TLS_ST_CW_FINISHED;
            return WRITE_TRAN_CONTINUE;
...
WRITE_TRAN ossl_statem_client_write_transition(SSL* s)
{
    ...
    switch (st->hand_state)
    {
        ...
        case TLS_ST_OK:
            ...
            {
                /*
                 * We haven't requested a renegotiation ourselves so we must
                 * have received a message from the server. Better read it.
                 */
                return WRITE_TRAN_FINISHED;
            }
            /* Renegotiation */
        ...
        case TLS_ST_CW_CLNT_HELLO:
            ...
            {
                /*
                 * We are assuming this is a TLSv1.3 connection, although we
                 * haven't actually selected a version yet.
                 */
...
WORK_STATE ossl_statem_client_pre_work(SSL* s, WORK_STATE wst)
{
    ...
    switch (st->hand_state)
    {
        ...
        case TLS_ST_PENDING_EARLY_DATA_END:
            /*
             * If we've been called by SSL_do_handshake()/SSL_write(), or we did
             * not attempt to write early data before calling SSL_read() then we
             * press on with the handshake. Otherwise we pause here.
             */
...
WORK_STATE ossl_statem_client_post_work(SSL* s, WORK_STATE wst)
{
    ...
    switch (st->hand_state)
    {
                ...
                s->max_early_data > 0)
            {
                ...
                if ((s->options & SSL_OP_ENABLE_MIDDLEBOX_COMPAT) == 0)
                {
                    if (!tls13_change_cipher_state(
                            s, SSL3_CC_EARLY | SSL3_CHANGE_CIPHER_CLIENT_WRITE))
                    {
                        /* SSLfatal() already called */
                        return WORK_ERROR;
                    }
                }
                /* else we're in compat mode so we delay flushing until after
                 * CCS */
            }
        ...
        case TLS_ST_CW_END_OF_EARLY_DATA:
            /*
             * We set the enc_write_ctx back to NULL because we may end up
             * writing in cleartext again if we get a HelloRetryRequest from the
             * server.
             */
...
int tls_construct_client_hello(SSL* s, WPACKET* pkt)
{
    ...
    if (sess == NULL || !ssl_version_supported(s, sess->ssl_version, NULL) ||
...
static int set_client_ciphersuite(SSL* s, const unsigned char* cipherchars)
{
    ...
    if (s->hit && (s->session->cipher_id != c->id))
    {
        if (SSL_IS_TLS13(s))
        {
             ...
             */
            if (ssl_md(c->algorithm2) != ssl_md(s->session->cipher->algorithm2))
            {
...
MSG_PROCESS_RETURN tls_process_server_hello(SSL* s, PACKET* pkt)
{
    ...
    if (SSL_IS_TLS13(s))
    {
        ...
        if (!tls_parse_extension(s, TLSEXT_IDX_psk, SSL_EXT_TLS1_3_SERVER_HELLO,
                                 extensions, NULL, 0))
        {
            /* SSLfatal() already called */
            goto err;
    ...
    else
    {
         ...
         */
        if (s->version >= TLS1_VERSION && s->ext.session_secret_cb != NULL &&
            s->session->ext.tick)
        {
            ...
            if (s->ext.session_secret_cb(s, s->session->master_key,
                                         &master_key_length, NULL, &pref_cipher,
    ...
    if (SSL_IS_DTLS(s) && s->hit)
    {
        ...
        if (SSL_export_keying_material(s, sctpauthkey, sizeof(sctpauthkey),
                                       labelbuffer, labellen, NULL, 0, 0) <= 0)
        {
    ...
    if (SSL_IS_TLS13(s) &&
        (!s->method->ssl3_enc->setup_key_block(s) ||
         !s->method->ssl3_enc->change_cipher_state(
             s, SSL3_CC_HANDSHAKE | SSL3_CHANGE_CIPHER_CLIENT_READ)))
    {
        /* SSLfatal() already called */
        goto err;
    }
...
MSG_PROCESS_RETURN tls_process_server_certificate(SSL* s, PACKET* pkt)
{
    ...
    if ((SSL_IS_TLS13(s) && !PACKET_get_1(pkt, &context)) || context != 0 ||
        !PACKET_get_net_3(pkt, &cert_list_len) ||
        PACKET_remaining(pkt) != cert_list_len || PACKET_remaining(pkt) == 0)
    {
    ...
    if (SSL_IS_TLS13(s) &&
        !ssl_handshake_hash(s, s->cert_verify_hash, sizeof(s->cert_verify_hash),
...
static int tls_process_ske_srp(SSL* s, PACKET* pkt, EVP_PKEY** pkey)
{
    ...
    if ((s->srp_ctx.N = BN_bin2bn(PACKET_data(&prime),
        ...
        (s->srp_ctx.g = BN_bin2bn(PACKET_data(&generator),
            ...
            NULL ||
        (s->srp_ctx.s = BN_bin2bn(
             PACKET_data(&salt), (int)PACKET_remaining(&salt), NULL)) == NULL ||
...
static int tls_process_ske_dhe(SSL* s, PACKET* pkt, EVP_PKEY** pkey)
{
    ...
    g = BN_bin2bn(PACKET_data(&generator), (int)PACKET_remaining(&generator),
                  NULL);
    bnpub_key =
        BN_bin2bn(PACKET_data(&pub_key), (int)PACKET_remaining(&pub_key), NULL);
...
static int tls_process_ske_ecdhe(SSL* s, PACKET* pkt, EVP_PKEY** pkey)
{
     ...
     */
    if (curve_type != NAMED_CURVE_TYPE || !tls1_check_group_id(s, curve_id, 1))
    {
...
MSG_PROCESS_RETURN tls_process_certificate_request(SSL* s, PACKET* pkt)
{
    ...
    if (SSL_IS_TLS13(s))
    {
        ...
        {
            SSLfatal(s, SSL_AD_DECODE_ERROR,
                     SSL_F_TLS_PROCESS_CERTIFICATE_REQUEST, SSL_R_BAD_LENGTH);
            return MSG_PROCESS_ERROR;
        }
        if (!tls_collect_extensions(s, &extensions,
    ...
    if (PACKET_remaining(pkt) != 0)
    {
        SSLfatal(s, SSL_AD_DECODE_ERROR, SSL_F_TLS_PROCESS_CERTIFICATE_REQUEST,
                 SSL_R_LENGTH_MISMATCH);
        return MSG_PROCESS_ERROR;
    }
...
MSG_PROCESS_RETURN tls_process_new_session_ticket(SSL* s, PACKET* pkt)
{
    ...
    if (SSL_IS_TLS13(s))
    {
        ...
        if (!tls_collect_extensions(s, &extpkt,
                                    ...
                                    NULL, 1) ||
            !tls_parse_all_extensions(s, SSL_EXT_TLS1_3_NEW_SESSION_TICKET,
                                      exts, NULL, 0, 1))
        {
            /* SSLfatal() already called */
            goto err;
     ...
     */
    if (!EVP_Digest(s->session->ext.tick, ticklen, s->session->session_id,
                    &sess_len, EVP_sha256(), NULL))
    {
        SSLfatal(s, SSL_AD_INTERNAL_ERROR, SSL_F_TLS_PROCESS_NEW_SESSION_TICKET,
                 ERR_R_EVP_LIB);
        goto err;
    ...
    if (SSL_IS_TLS13(s))
    {
        ...
        if (!tls13_hkdf_expand(s, md, s->resumption_master_secret, nonce_label,
                               sizeof(nonce_label) - 1, PACKET_data(&nonce),
                               PACKET_remaining(&nonce), s->session->master_key,
                               hashlen, 1))
        {
            /* SSLfatal() already called */
            goto err;
...
static int tls_construct_cke_psk_preamble(SSL* s, WPACKET* pkt)
{
    ...
    psklen = s->psk_client_callback(s, s->session->psk_identity_hint, identity,
                                    sizeof(identity) - 1, psk, sizeof(psk));
...
static int tls_construct_cke_srp(SSL* s, WPACKET* pkt)
{
    ...
    if (s->srp_ctx.A == NULL || !WPACKET_sub_allocate_bytes_u16(
                                    pkt, BN_num_bytes(s->srp_ctx.A), &abytes))
    {
...
int tls_construct_client_key_exchange(SSL* s, WPACKET* pkt)
{
     ...
     */
    if ((alg_k & SSL_PSK) && !tls_construct_cke_psk_preamble(s, pkt))
        goto err;
...
int tls_construct_client_certificate(SSL* s, WPACKET* pkt)
{
    if (SSL_IS_TLS13(s))
    {
        ...
        else if (!WPACKET_sub_memcpy_u8(pkt, s->pha_context,
                                        s->pha_context_len))
        {
            SSLfatal(s, SSL_AD_INTERNAL_ERROR,
                     SSL_F_TLS_CONSTRUCT_CLIENT_CERTIFICATE,
    ...
    if (!ssl3_output_cert_chain(
            s, pkt, (s->s3->tmp.cert_req == 2) ? NULL : s->cert->key))
    {
        /* SSLfatal() already called */
        return 0;
    }
    ...
    if (SSL_IS_TLS13(s) && SSL_IS_FIRST_HANDSHAKE(s) &&
        (!s->method->ssl3_enc->change_cipher_state(
            s, SSL3_CC_HANDSHAKE | SSL3_CHANGE_CIPHER_CLIENT_WRITE)))
    {
        /*
...
int ssl_do_client_cert_cb(SSL* s, X509** px509, EVP_PKEY** ppkey)
{
    ...
    if (s->ctx->client_cert_engine)
    {
        i = ENGINE_load_ssl_client_cert(s->ctx->client_cert_engine, s,
                                        SSL_get_client_CA_list(s), px509, ppkey,
                                        NULL, NULL, NULL);
        if (i != 0)
            return i;
    }
...
#error Max cipher length too short
    ...
    if (totlen != 0)
    {
        ...
        {
            static SSL_CIPHER scsv = {
                0, NULL, NULL, SSL3_CK_FALLBACK_SCSV, 0, 0, 0, 0, 0, 0,
                0, 0,    0};
            if (!s->method->put_cipher_by_char(&scsv, pkt, &len))
            {
                SSLfatal(s, SSL_AD_INTERNAL_ERROR,
