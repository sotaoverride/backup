...
static int node_vtable_get_userdata(sd_bus* bus, const char* path,
                                    struct node_vtable* c, void** userdata,
                                    sd_bus_error* error)
{
...
static int vtable_property_get_userdata(sd_bus* bus, const char* path,
...
static int add_enumerated_to_set(sd_bus* bus, const char* prefix,
                                 struct node_enumerator* first, Set* s,
                                 sd_bus_error* error)
{
...
static int add_subtree_to_set(sd_bus* bus, const char* prefix, struct node* n,
                              unsigned flags, Set* s, sd_bus_error* error)
{
...
static int get_child_nodes(sd_bus* bus, const char* prefix, struct node* n,
                           unsigned flags, Set** _s, sd_bus_error* error)
{
...
static int node_callbacks_run(sd_bus* bus, sd_bus_message* m,
...
#define CAPABILITY_SHIFT(x)                                                    \
    (((x) >> __builtin_ctzll(_SD_BUS_VTABLE_CAPABILITY_MASK)) & 0xFFFF)

static int check_access(sd_bus* bus, sd_bus_message* m, struct vtable_member* c,
                        sd_bus_error* error)
{
    uint64_t cap;
    int r;
    ...
    return sd_bus_error_setf(error, SD_BUS_ERROR_ACCESS_DENIED,
                             "Access to %s.%s() not permitted.", c->interface,
                             c->member);
}

static int method_callbacks_run(sd_bus* bus, sd_bus_message* m,
                                struct vtable_member* c, bool require_fallback,
                                bool* found_object)
{
    ...
    *found_object = true;
    ...
    if (!streq(strempty(c->vtable->x.method.signature), signature))
        return sd_bus_reply_method_errorf(
            m, SD_BUS_ERROR_INVALID_ARGS,
            "Invalid arguments '%s' to call %s.%s(), expecting '%s'.",
            signature, c->interface, c->member,
            strempty(c->vtable->x.method.signature));
...
static int invoke_property_get(sd_bus* bus, sd_bus_slot* slot,
                               const sd_bus_vtable* v, const char* path,
                               ...
                               sd_bus_error* error)
{
    ...
    if (v->x.property.get)
    {
        ...
        r = v->x.property.get(bus, path, interface, property, reply, userdata,
                              error);
        bus->current_userdata = NULL;
        bus->current_slot = sd_bus_slot_unref(slot);
...
static int invoke_property_set(sd_bus* bus, sd_bus_slot* slot,
                               const sd_bus_vtable* v, const char* path,
                               const char* interface, const char* property,
                               ...
                               sd_bus_error* error)
{
    ...
    if (v->x.property.set)
    {
        ...
        bus->current_slot = sd_bus_slot_ref(slot);
        bus->current_userdata = userdata;
        r = v->x.property.set(bus, path, interface, property, value, userdata,
                              error);
        bus->current_userdata = NULL;
        bus->current_slot = sd_bus_slot_unref(slot);
    ...
    switch (v->x.property.signature[0])
    {
        ...
        case SD_BUS_TYPE_SIGNATURE:
        {
            ...
            r = sd_bus_message_read_basic(value, v->x.property.signature[0],
                                          &p);
            if (r < 0)
                return r;
...
static int property_get_set_callbacks_run(sd_bus* bus, sd_bus_message* m,
                                          struct vtable_member* c,
                                          bool require_fallback, bool is_get,
                                          bool* found_object)
{
    ...
    *found_object = true;
    ...
    if (is_get)
    {
        ...
        r = sd_bus_message_open_container(reply, 'v',
                                          c->vtable->x.property.signature);
        if (r < 0)
            return r;
        ...
        r = invoke_property_get(bus, slot, c->vtable, m->path, c->interface,
                                c->member, reply, u, &error);
        if (r < 0)
            return bus_maybe_reply_error(m, r, &error);
    ...
    else
    {
        ...
        if (c->vtable->type != _SD_BUS_VTABLE_WRITABLE_PROPERTY)
            return sd_bus_reply_method_errorf(
                m, SD_BUS_ERROR_PROPERTY_READ_ONLY,
                "Property '%s' is not writable.", c->member);
        ...
        if (type != 'v' || !streq(strempty(signature),
                                  strempty(c->vtable->x.property.signature)))
            return sd_bus_reply_method_errorf(
                m, SD_BUS_ERROR_INVALID_ARGS,
                "Incorrect parameters for property '%s', expected '%s', got "
                "'%s'.",
                c->member, strempty(c->vtable->x.property.signature),
                strempty(signature));
        ...
        r = invoke_property_set(bus, slot, c->vtable, m->path, c->interface,
                                c->member, m, u, &error);
        if (r < 0)
            return bus_maybe_reply_error(m, r, &error);
...
static int vtable_append_one_property(sd_bus* bus, sd_bus_message* reply,
                                      const char* path, struct node_vtable* c,
                                      const sd_bus_vtable* v, void* userdata,
                                      sd_bus_error* error)
{
    ...
    r = invoke_property_get(
        bus, slot, v, path, c->interface, v->x.property.member, reply,
        vtable_property_convert_userdata(v, userdata), error);
    if (r < 0)
        return r;
    if (bus->nodes_modified)
        return 0;
...
static int vtable_append_all_properties(sd_bus* bus, sd_bus_message* reply,
                                        const char* path, struct node_vtable* c,
                                        void* userdata, sd_bus_error* error)
{
    ...
    v = c->vtable;
    for (v = bus_vtable_next(c->vtable, v); v->type != _SD_BUS_VTABLE_END;
         v = bus_vtable_next(c->vtable, v))
    {
        if (!IN_SET(v->type, _SD_BUS_VTABLE_PROPERTY,
...
static int property_get_all_callbacks_run(sd_bus* bus, sd_bus_message* m,
                                          struct node_vtable* first,
                                          bool require_fallback,
                                          const char* iface, bool* found_object)
{
    ...
    if (!found_interface)
    {
        r = sd_bus_reply_method_errorf(m, SD_BUS_ERROR_UNKNOWN_INTERFACE,
...
static int bus_node_exists(sd_bus* bus, struct node* n, const char* path,
                           bool require_fallback)
{
...
static int process_introspect(sd_bus* bus, sd_bus_message* m, struct node* n,
                              bool require_fallback, bool* found_object)
{
    ...
    r = introspect_write_default_interfaces(&intro, !require_fallback &&
                                                        n->object_managers);
    if (r < 0)
        return r;
...
static int object_manager_serialize_path(sd_bus* bus, sd_bus_message* reply,
...
static int object_manager_serialize_path_and_fallbacks(sd_bus* bus,
                                                       ...
                                                       sd_bus_error* error)
{
    ...
    {
        r = object_manager_serialize_path(bus, reply, prefix, path, true,
                                          error);
        if (r < 0)
            return r;
        if (bus->nodes_modified)
...
static int process_get_managed_objects(sd_bus* bus, sd_bus_message* m,
                                       struct node* n, bool require_fallback,
                                       bool* found_object)
{
    ...
    {
        r = object_manager_serialize_path_and_fallbacks(bus, reply, path,
                                                        &error);
        if (r < 0)
            return bus_maybe_reply_error(m, r, &error);
...
static int object_find_and_run(sd_bus* bus, sd_bus_message* m, const char* p,
                               bool require_fallback, bool* found_object)
{
    ...
    if (streq(m->interface, "org.freedesktop.DBus.Properties"))
    {
        ...
        if (get || streq(m->member, "Set"))
        {
            ...
            r = sd_bus_message_read(m, "ss", &vtable_key.interface,
                                    &vtable_key.member);
            if (r < 0)
                return sd_bus_reply_method_errorf(
                    m, SD_BUS_ERROR_INVALID_ARGS,
            ...
            if (v)
            {
                r = property_get_set_callbacks_run(bus, m, v, require_fallback,
                                                   get, found_object);
                if (r != 0)
                    return r;
            }
        ...
        else if (streq(m->member, "GetAll"))
        {
            ...
            r = property_get_all_callbacks_run(
                bus, m, n->vtables, require_fallback, iface, found_object);
            if (r != 0)
                return r;
        }
    }
                 ...
                 m, "org.freedesktop.DBus.ObjectManager", "GetManagedObjects"))
    {
        ...
        r = process_get_managed_objects(bus, m, n, require_fallback,
                                        found_object);
        if (r != 0)
            return r;
    }
...
int bus_process_object(sd_bus* bus, sd_bus_message* m)
{
    ...
    if (sd_bus_message_is_method_call(m, "org.freedesktop.DBus.Properties",
                                      "Get") ||
        sd_bus_message_is_method_call(m, "org.freedesktop.DBus.Properties",
                                      "Set"))
        r = sd_bus_reply_method_errorf(m, SD_BUS_ERROR_UNKNOWN_PROPERTY,
                                       "Unknown property or interface.");
    else
        r = sd_bus_reply_method_errorf(m, SD_BUS_ERROR_UNKNOWN_METHOD,
                                       "Unknown method '%s' or interface '%s'.",
                                       m->member, m->interface);
...
static int bus_find_parent_object_manager(sd_bus* bus, struct node** out,
                                          const char* path)
{
    struct node* n;
...
static int bus_add_object(sd_bus* bus, sd_bus_slot** slot, bool fallback,
                          const char* path, sd_bus_message_handler_t callback,
                          void* userdata)
{
    ...
    s = bus_slot_allocate(bus, !slot, BUS_NODE_CALLBACK,
                          sizeof(struct node_callback), userdata);
    if (!s)
    {
        r = -ENOMEM;
        goto fail;
...
_public_ int sd_bus_add_object(sd_bus* bus, sd_bus_slot** slot,
                               const char* path,
...
_public_ int sd_bus_add_fallback(sd_bus* bus, sd_bus_slot** slot,
                                 const char* prefix,
                                 sd_bus_message_handler_t callback,
                                 void* userdata)
{
...
static void vtable_member_hash_func(const struct vtable_member* m,
...
static int vtable_member_compare_func(const struct vtable_member* x,
                                      const struct vtable_member* y)
{
    int r;
...
DEFINE_PRIVATE_HASH_OPS(vtable_member_hash_ops, struct vtable_member,
                        vtable_member_hash_func, vtable_member_compare_func);

typedef enum
{
    NAMES_FIRST_PART = 1 << 0, /* first part of argument name list (input
                                  names). It is reset by names_are_valid() */
    NAMES_PRESENT = 1 << 1,    /* at least one argument name is present, so the
                                  names will checked.    This flag is set and used
                                  internally by names_are_valid(), but needs to be
                                  stored across calls for 2-parts list  */
    NAMES_SINGLE_PART =
        1 << 2, /* argument name list consisting of a single part */
} names_flags;

static bool names_are_valid(const char* signature, const char** names,
                            names_flags* flags)
{
    int r;

    if ((*flags & NAMES_FIRST_PART || *flags & NAMES_SINGLE_PART) &&
        **names != '\0')
        *flags |= NAMES_PRESENT;
    ...
    /* let's check if there are more argument names specified than the signature
     * allows */
    if (*flags & NAMES_PRESENT && **names != '\0' &&
...
/* the current version of this struct is defined in sd-bus-vtable.h, but we need
   to list here the historical versions to make sure the calling code is
   compatible with one of these */
...
const sd_bus_vtable* bus_vtable_next(const sd_bus_vtable* vtable,
                                     const sd_bus_vtable* v)
{
    if (vtable[0].x.start.element_size == VTABLE_ELEMENT_SIZE_ORIGINAL)
    {
        const struct sd_bus_vtable_original* v2 =
            (const struct sd_bus_vtable_original*)v;
        v2++;
...
static int add_object_vtable_internal(sd_bus* bus, sd_bus_slot** slot,
                                      const char* path, const char* interface,
                                      const sd_bus_vtable* vtable,
                                      bool fallback, sd_bus_object_find_t find,
                                      void* userdata)
{
    ...
    assert_return(vtable[0].x.start.element_size ==
    ...
    r = hashmap_ensure_allocated(&bus->vtable_properties,
    ...
    s = bus_slot_allocate(bus, !slot, BUS_NODE_VTABLE,
                          sizeof(struct node_vtable), userdata);
    if (!s)
    {
        r = -ENOMEM;
        goto fail;
    ...
    for (v = bus_vtable_next(vtable, v); v->type != _SD_BUS_VTABLE_END;
         v = bus_vtable_next(vtable, v))
    {
        ...
        switch (v->type)
        {
            ...
            case _SD_BUS_VTABLE_METHOD:
            {
                ...
                if (!member_name_is_valid(v->x.method.member) ||
                    !signature_is_valid(strempty(v->x.method.signature),
                                        false) ||
                    !signature_is_valid(strempty(v->x.method.result), false) ||
                    !names_are_valid(strempty(v->x.method.signature), &names,
                                     &nf) ||
                    !names_are_valid(strempty(v->x.method.result), &names,
                                     &nf) ||
                    !(v->x.method.handler || (isempty(v->x.method.signature) &&
                                              isempty(v->x.method.result))) ||
            ...
            case _SD_BUS_VTABLE_WRITABLE_PROPERTY:
                ...
                if (!(v->x.property.set ||
                      bus_type_is_basic(v->x.property.signature[0])))
                {
                    r = -EINVAL;
                    goto fail;
                }
            ...
            case _SD_BUS_VTABLE_PROPERTY:
            {
                ...
                if (!member_name_is_valid(v->x.property.member) ||
                    ...
                    !(v->x.property.get ||
                      bus_type_is_basic(v->x.property.signature[0]) ||
                      streq(v->x.property.signature, "as")) ||
                    (v->flags & SD_BUS_VTABLE_METHOD_NO_REPLY) ||
                    (!!(v->flags & SD_BUS_VTABLE_PROPERTY_CONST) +
                     !!(v->flags & SD_BUS_VTABLE_PROPERTY_EMITS_CHANGE) +
                     !!(v->flags & SD_BUS_VTABLE_PROPERTY_EMITS_INVALIDATION)) >
                        1 ||
                    ((v->flags & SD_BUS_VTABLE_PROPERTY_EMITS_CHANGE) &&
                     (v->flags & SD_BUS_VTABLE_PROPERTY_EXPLICIT)) ||
            ...
            case _SD_BUS_VTABLE_SIGNAL:
                ...
                if (!member_name_is_valid(v->x.signal.member) ||
                    !signature_is_valid(strempty(v->x.signal.signature),
                                        false) ||
                    !names_are_valid(strempty(v->x.signal.signature), &names,
                                     &nf) ||
                    v->flags & SD_BUS_VTABLE_UNPRIVILEGED)
                {
                    r = -EINVAL;
...
/* This symbol exists solely to tell the linker that the "new" vtable format is
 * used. */
_public_ const unsigned sd_bus_object_vtable_format = 242;

_public_ int sd_bus_add_object_vtable(sd_bus* bus, sd_bus_slot** slot,
                                      const char* path, const char* interface,
                                      const sd_bus_vtable* vtable,
                                      void* userdata)
{

    return add_object_vtable_internal(bus, slot, path, interface, vtable, false,
                                      NULL, userdata);
}

_public_ int sd_bus_add_fallback_vtable(
    sd_bus* bus, sd_bus_slot** slot, const char* prefix, const char* interface,
    const sd_bus_vtable* vtable, sd_bus_object_find_t find, void* userdata)
{

    return add_object_vtable_internal(bus, slot, prefix, interface, vtable,
                                      true, find, userdata);
}

_public_ int sd_bus_add_node_enumerator(sd_bus* bus, sd_bus_slot** slot,
                                        const char* path,
                                        ...
                                        void* userdata)
{
    ...
    s = bus_slot_allocate(bus, !slot, BUS_NODE_ENUMERATOR,
                          sizeof(struct node_enumerator), userdata);
    if (!s)
    {
        r = -ENOMEM;
        goto fail;
...
static int emit_properties_changed_on_interface(
    sd_bus* bus, const char* prefix, const char* path, const char* interface,
    bool require_fallback, bool* found_interface, char** names)
{
    ...
    r = sd_bus_message_new_signal(
        bus, &m, path, "org.freedesktop.DBus.Properties", "PropertiesChanged");
    if (r < 0)
        return r;
        ...
        *found_interface = true;
        ...
        if (names)
        {
            ...
            STRV_FOREACH(property, names)
            {
                ...
                assert_return(v->vtable->flags &
                                      SD_BUS_VTABLE_PROPERTY_EMITS_CHANGE ||
                                  v->vtable->flags &
                                      SD_BUS_VTABLE_PROPERTY_EMITS_INVALIDATION,
                              -EDOM);

                assert_return(!(v->vtable->flags & SD_BUS_VTABLE_HIDDEN),
                              -EDOM);
                ...
                r = vtable_append_one_property(bus, m, m->path, c, v->vtable, u,
                                               &error);
                if (r < 0)
                    return r;
                if (bus->nodes_modified)
        ...
        else
        {
            ...
            v = c->vtable;
            for (v = bus_vtable_next(c->vtable, v);
                 ...
                 v = bus_vtable_next(c->vtable, v))
            {
                ...
                r = vtable_append_one_property(bus, m, m->path, c, v, u,
                                               &error);
                if (r < 0)
                    return r;
                if (bus->nodes_modified)
...
_public_ int sd_bus_emit_properties_changed_strv(sd_bus* bus, const char* path,
                                                 ...
                                                 char** names)
{
    ...
    do
    {
        ...
        r = emit_properties_changed_on_interface(
            bus, path, path, interface, false, &found_interface, names);
        if (r != 0)
            return r;
        if (bus->nodes_modified)
            continue;
        ...
        {
            r = emit_properties_changed_on_interface(
                bus, prefix, path, interface, true, &found_interface, names);
            if (r != 0)
                return r;
            if (bus->nodes_modified)
                break;
...
_public_ int sd_bus_emit_properties_changed(sd_bus* bus, const char* path,
                                            const char* interface,
                                            const char* name, ...)
{
...
static int object_added_append_all_prefix(sd_bus* bus, sd_bus_message* m,
                                          Set* s, const char* prefix,
                                          const char* path,
                                          bool require_fallback)
{
...
static int object_added_append_all(sd_bus* bus, sd_bus_message* m,
                                   const char* path)
{
    ...
    if (r < 0)
        return r;
    r = sd_bus_message_append(m, "{sa{sv}}",
                              "org.freedesktop.DBus.Introspectable", 0);
    if (r < 0)
        return r;
    r = sd_bus_message_append(m, "{sa{sv}}", "org.freedesktop.DBus.Properties",
                              0);
    if (r < 0)
        return r;
...
_public_ int sd_bus_emit_object_added(sd_bus* bus, const char* path)
{
    ...
    do
    {
        ...
        r = sd_bus_message_new_signal(bus, &m, object_manager->path,
...
static int object_removed_append_all_prefix(sd_bus* bus, sd_bus_message* m,
                                            Set* s, const char* prefix,
                                            const char* path,
                                            bool require_fallback)
{
...
static int interfaces_added_append_one_prefix(sd_bus* bus, sd_bus_message* m,
                                              const char* prefix,
                                              const char* path,
                                              const char* interface,
                                              bool require_fallback)
...
static int interfaces_added_append_one(sd_bus* bus, sd_bus_message* m,
                                       const char* path, const char* interface)
{
    ...
    r = interfaces_added_append_one_prefix(bus, m, path, path, interface,
                                           false);
    if (r != 0)
        return r;
    if (bus->nodes_modified)
    ...
    OBJECT_PATH_FOREACH_PREFIX(prefix, path)
    {
        r = interfaces_added_append_one_prefix(bus, m, prefix, path, interface,
                                               true);
        if (r != 0)
            return r;
        if (bus->nodes_modified)
...
_public_ int sd_bus_emit_interfaces_added(sd_bus* bus, const char* path,
                                          const char* interface, ...)
{
    char** interfaces;
...
_public_ int sd_bus_emit_interfaces_removed_strv(sd_bus* bus, const char* path,
                                                 char** interfaces)
{
    _cleanup_(sd_bus_message_unrefp) sd_bus_message* m = NULL;
    struct node* object_manager;
...
_public_ int sd_bus_emit_interfaces_removed(sd_bus* bus, const char* path,
                                            const char* interface, ...)
{
    char** interfaces;
...
_public_ int sd_bus_add_object_manager(sd_bus* bus, sd_bus_slot** slot,
                                       const char* path)
{
    sd_bus_slot* s;
    struct node* n;
    ...
    s = bus_slot_allocate(bus, !slot, BUS_NODE_OBJECT_MANAGER,
                          sizeof(struct node_object_manager), NULL);
    if (!s)
    {
        r = -ENOMEM;
        goto fail;
