...
/** Initialize the hrSWInstalledTable table by defining its contents and how
 * it's structured */
void initialize_table_hrSWInstalledTable(void)
{
    static oid hrSWInstalledTable_oid[] = {1, 3, 6, 1, 2, 1, 25, 6, 3};
    size_t hrSWInstalledTable_oid_len = OID_LENGTH(hrSWInstalledTable_oid);
    ...
    reg = netsnmp_create_handler_registration(
        ...
        hrSWInstalledTable_oid, hrSWInstalledTable_oid_len, HANDLER_CAN_RONLY);
    if (NULL == reg)
    {
        snmp_log(LOG_ERR,
                 "error creating handler registration for " MYTABLE "\n");
        goto bail;
    }
    ...
    {
        snmp_log(LOG_ERR,
                 "error allocating table registration for " MYTABLE "\n");
        goto bail;
    }

    netsnmp_table_helper_add_indexes(
        table_info, ASN_INTEGER, /* index: hrSWInstalledIndex */
        0);
    ...
    handler = netsnmp_container_table_handler_get(
        table_info, container, TABLE_CONTAINER_KEY_NETSNMP_INDEX);
    if (NULL == handler)
    {
        snmp_log(LOG_ERR,
                 "error allocating table registration for " MYTABLE "\n");
    ...
    {
        snmp_log(LOG_ERR,
                 "error injecting container_table handler for " MYTABLE "\n");
        goto bail;
    }
    ...
    cache =
        netsnmp_cache_create(30, /* timeout in seconds */
                             _cache_load, _cache_free, hrSWInstalledTable_oid,
    ...
    if (SNMPERR_SUCCESS != netsnmp_inject_handler(reg, handler))
    {
        snmp_log(LOG_ERR, "error injecting cache handler for " MYTABLE "\n");
        goto bail;
    }
    ...
    {
        snmp_log(LOG_ERR, "error registering table handler for " MYTABLE "\n");
...
int hrSWInstalledTable_handler(netsnmp_mib_handler* handler,
        ...
        case MODE_GET:
            for (request = requests; request; request = request->next)
            {
                ...
                if ((NULL == table_entry) || (NULL == table_info))
                {
                    snmp_log(
                        LOG_ERR,
                        "could not extract table entry or info for " MYTABLE
                        "\n");
