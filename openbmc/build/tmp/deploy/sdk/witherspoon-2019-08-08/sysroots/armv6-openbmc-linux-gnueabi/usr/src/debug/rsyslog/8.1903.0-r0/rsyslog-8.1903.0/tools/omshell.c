...
ENDisCompatibleWithFeature
    ...
    BEGINfreeInstance CODESTARTfreeInstance ENDfreeInstance
...
if (execProg((uchar*)pWrkrData->pData->progName, 1, ppString[0]) == 0)
    LogError(0, NO_ERRCODE, "Executing program '%s' failed",
             (char*)pWrkrData->pData->progName);
ENDdoAction
...
switch (*p)
{
    case '^': /* bkalkbrenner 2005-09-20: execute shell command */
        dbgprintf("exec\n");
        ++p;
        iRet = cflineParseFileName(p, (uchar*)pData->progName, *ppOMSR, 0,
...
CODE_STD_FINALIZERparseSelectorAct ENDparseSelectorAct
    ...
    BEGINmodExit CODESTARTmodExit ENDmodExit
        ...
        BEGINqueryEtryPt CODESTARTqueryEtryPt CODEqueryEtryPt_STD_OMOD_QUERIES
            CODEqueryEtryPt_STD_OMOD8_QUERIES ENDqueryEtryPt

                BEGINmodInit(Shell) CODESTARTmodInit* ipIFVersProvided =
