...
static int context_parse_ntp_services(Context* c)
{
    ...
    if (!env)
    {
        ...
        if (r < 0)
            log_warning_errno(r, "Failed to add NTP service "
                                 "\"systemd-timesyncd.service\", ignoring: %m");
    ...
    for (p = env;;)
    {
        ...
        if (r < 0)
            log_warning_errno(
                r, "Failed to add NTP service \"%s\", ignoring: %m", word);
    }
...
static int context_ntp_service_is_active(Context* c)
{
    ...
    /* Call context_update_ntp_status() to update UnitStatusInfo before calling
     * this. */
...
static int context_ntp_service_is_enabled(Context* c)
{
    ...
    LIST_FOREACH(units, info, c->units)
    count += !STRPTR_IN_SET(info->unit_file_state, "masked", "masked-runtime",
                            "disabled", "bad");
...
static int context_read_data(Context* c)
{
    ...
    r = get_timezone(&t);
    if (r == -EINVAL)
        log_warning_errno(r, "/etc/localtime should be a symbolic link to a "
                             "time zone data file in /usr/share/zoneinfo/.");
    else if (r < 0)
    ...
    else
    {
        ...
        else
        {
            ...
            else
            {
                ...
                /* if we actually have a fourth line, use that as suffix "e",
        ...
        *(char*)mempcpy(stpcpy(stpcpy(mempcpy(w, s, a), prepend),
...
static int context_update_ntp_status(Context* c, sd_bus* bus, sd_bus_message* m)
{
    ...
    /* Suppress calling context_update_ntp_status() multiple times within single
     * DBus transaction. */
    if (m)
    {
        if (m == c->cache)
    ...
    LIST_FOREACH(units, u, c->units)
    {
        ...
        r = bus_map_all_properties(bus, "org.freedesktop.systemd1", path, map,
                                   BUS_MAP_STRDUP, &error, NULL, u);
        if (r < 0)
                             ...
                             sd_bus_error* error)
{
    ...
    if (n == 0)
    {
        ...
        (void)sd_bus_emit_properties_changed(
            sd_bus_message_get_bus(m), "/org/freedesktop/timedate1",
...
static int unit_start_or_stop(UnitStatusInfo* u, sd_bus* bus,
                              sd_bus_error* error, bool start)
{
    ...
    r = sd_bus_call_method(
        ...
        "org.freedesktop.systemd1.Manager", start ? "StartUnit" : "StopUnit",
        error, &reply, "ss", u->name, "replace");
    if (r < 0)
        return r;
...
static int unit_enable_or_disable(UnitStatusInfo* u, sd_bus* bus,
                                  sd_bus_error* error, bool enable)
{
    int r;
    ...
    if (enable)
        r = sd_bus_call_method(
            bus, "org.freedesktop.systemd1", "/org/freedesktop/systemd1",
            "org.freedesktop.systemd1.Manager", "EnableUnitFiles", error, NULL,
            "asbb", 1, u->name, false, true);
    else
        r = sd_bus_call_method(
            bus, "org.freedesktop.systemd1", "/org/freedesktop/systemd1",
            "org.freedesktop.systemd1.Manager", "DisableUnitFiles", error, NULL,
            "asb", 1, u->name, false);
    if (r < 0)
        return r;
    ...
    r = sd_bus_call_method(
        bus, "org.freedesktop.systemd1", "/org/freedesktop/systemd1",
        "org.freedesktop.systemd1.Manager", "Reload", error, NULL, NULL);
    if (r < 0)
        return r;
...
static BUS_DEFINE_PROPERTY_GET_GLOBAL(property_get_time, "t",
                                      now(CLOCK_REALTIME));
static BUS_DEFINE_PROPERTY_GET_GLOBAL(property_get_ntp_sync, "b", ntp_synced());

static int property_get_rtc_time(sd_bus* bus, const char* path,
                                 ...
                                 sd_bus_error* error)
{
    ...
    {
        log_warning(
            "/dev/rtc is busy. Is somebody keeping it open continuously? "
            "That's not a good idea... Returning a bogus RTC timestamp.");
        t = 0;
    }
    else if (r == -ENOENT)
    {
...
static int property_get_can_ntp(sd_bus* bus, const char* path,
                                const char* interface, const char* property,
                                sd_bus_message* reply, void* userdata,
                                sd_bus_error* error)
{
    ...
    if (c->slot_job_removed)
        /* When the previous request is not finished, then assume NTP is
         * enabled. */
...
static int property_get_ntp(sd_bus* bus, const char* path,
                            const char* interface, const char* property,
                            sd_bus_message* reply, void* userdata,
                            sd_bus_error* error)
{
    ...
    return sd_bus_message_append(reply, "b",
                                 context_ntp_service_is_active(c) > 0);
}

static int method_set_timezone(sd_bus_message* m, void* userdata,
                               sd_bus_error* error)
{
    Context* c = userdata;
    int interactive, r;
    ...
    r = bus_verify_polkit_async(
        m, CAP_SYS_TIME, "org.freedesktop.timedate1.set-timezone", NULL,
    ...
    if (c->local_rtc)
    {
        ...
        if (r < 0)
            log_debug_errno(
                r, "Failed to sync time to hardware clock, ignoring: %m");
    }

    log_struct(LOG_INFO, "MESSAGE_ID=" SD_MESSAGE_TIMEZONE_CHANGE_STR,
               "TIMEZONE=%s", c->zone, "TIMEZONE_SHORTNAME=%s",
               tzname[daylight], "DAYLIGHT=%i", daylight,
               LOG_MESSAGE("Changed time zone to '%s' (%s).", c->zone,
                           tzname[daylight]));
...
static int method_set_local_rtc(sd_bus_message* m, void* userdata,
                                sd_bus_error* error)
{
    ...
    r = bus_verify_polkit_async(
        m, CAP_SYS_TIME, "org.freedesktop.timedate1.set-local-rtc", NULL,
        interactive, UID_INVALID, &c->polkit_registry, error);
    if (r < 0)
        return r;
    if (r == 0)
...
static int method_set_time(sd_bus_message* m, void* userdata,
                           sd_bus_error* error)
{
    ...
    if (c->slot_job_removed)
        return sd_bus_error_set(error, BUS_ERROR_AUTOMATIC_TIME_SYNC_ENABLED,
    ...
    r = bus_verify_polkit_async(
        m, CAP_SYS_TIME, "org.freedesktop.timedate1.set-time", NULL,
        interactive, UID_INVALID, &c->polkit_registry, error);
    if (r < 0)
        return r;
    if (r == 0)
    ...
    log_struct(LOG_INFO, "MESSAGE_ID=" SD_MESSAGE_TIME_CHANGE_STR,
...
static int method_set_ntp(sd_bus_message* m, void* userdata,
                          sd_bus_error* error)
{
    ...
    if (context_ntp_service_exists(c) <= 0)
        return sd_bus_error_set(error, BUS_ERROR_NO_NTP_SUPPORT,
                                "NTP not supported");

    r = bus_verify_polkit_async(
        m, CAP_SYS_TIME, "org.freedesktop.timedate1.set-ntp", NULL, interactive,
        UID_INVALID, &c->polkit_registry, error);
    if (r < 0)
        return r;
    if (r == 0)
        return 1;

    /* This method may be called frequently. Forget the previous job if it has
     * not completed yet. */
    LIST_FOREACH(units, u, c->units)
    u->path = mfree(u->path);
    ...
    if (!c->slot_job_removed)
    {
        r = sd_bus_match_signal_async(bus, &slot, "org.freedesktop.systemd1",
                                      "/org/freedesktop/systemd1",
                                      "org.freedesktop.systemd1.Manager",
                                      "JobRemoved", match_job_removed, NULL, c);
        if (r < 0)
            return r;
    }
...
static int method_list_timezones(sd_bus_message* m, void* userdata,
                                 sd_bus_error* error)
{
    _cleanup_(sd_bus_message_unrefp) sd_bus_message* reply = NULL;
    ...
    if (r < 0)
        return sd_bus_error_set_errnof(error, r,
                                       "Failed to read list of time zones: %m");
...
static const sd_bus_vtable timedate_vtable[] = {
    ...
    SD_BUS_PROPERTY("NTP", "b", property_get_ntp, 0,
                    SD_BUS_VTABLE_PROPERTY_EMITS_CHANGE),
    ...
    SD_BUS_METHOD("SetNTP", "bb", NULL, method_set_ntp,
                  SD_BUS_VTABLE_UNPRIVILEGED),
    SD_BUS_METHOD("ListTimezones", NULL, "as", method_list_timezones,
                  SD_BUS_VTABLE_UNPRIVILEGED),
    SD_BUS_VTABLE_END,
};
...
static int connect_bus(Context* c, sd_event* event, sd_bus** _bus)
{
    ...
    r = sd_bus_request_name_async(bus, NULL, "org.freedesktop.timedate1", 0,
                                  NULL, NULL);
    if (r < 0)
