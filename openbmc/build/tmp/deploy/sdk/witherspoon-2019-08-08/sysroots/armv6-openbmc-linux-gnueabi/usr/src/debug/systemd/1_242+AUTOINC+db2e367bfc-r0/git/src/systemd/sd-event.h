...
/*
  ...
  - Scales better with a large number of time events because it does not require
  one timerfd each
...
int sd_event_add_inotify(sd_event* e, sd_event_source** s, const char* path,
                         uint32_t mask, sd_event_inotify_handler_t callback,
                         void* userdata);
...
int sd_event_source_set_destroy_callback(sd_event_source* s,
                                         sd_event_destroy_t callback);
int sd_event_source_get_destroy_callback(sd_event_source* s,
...
/* Define helpers so that __attribute__((cleanup(sd_event_unrefp))) and similar
 * may be used. */
