...
struct filter_factory
{
     ...
     */
    virtual filter on_equality_relation(attribute_name const& name,
                                        string_type const& arg)
    {
        BOOST_LOG_THROW_DESCR_PARAMS(
            parse_error,
            "The equality attribute value relation is not supported", (name));
        BOOST_LOG_UNREACHABLE_RETURN(filter());
    }
    /*!
     * The callback for inequality relation filter
     */
    virtual filter on_inequality_relation(attribute_name const& name,
                                          string_type const& arg)
    {
        BOOST_LOG_THROW_DESCR_PARAMS(
            parse_error,
            "The inequality attribute value relation is not supported", (name));
        BOOST_LOG_UNREACHABLE_RETURN(filter());
    }
    /*!
     * The callback for less relation filter
     */
    virtual filter on_less_relation(attribute_name const& name,
                                    string_type const& arg)
    {
        BOOST_LOG_THROW_DESCR_PARAMS(
            parse_error, "The less attribute value relation is not supported",
            (name));
        BOOST_LOG_UNREACHABLE_RETURN(filter());
    }
    /*!
     ...
     */
    virtual filter on_greater_relation(attribute_name const& name,
                                       string_type const& arg)
    {
        BOOST_LOG_THROW_DESCR_PARAMS(
            parse_error,
            "The greater attribute value relation is not supported", (name));
        BOOST_LOG_UNREACHABLE_RETURN(filter());
    }
    /*!
     * The callback for less or equal relation filter
     */
    virtual filter on_less_or_equal_relation(attribute_name const& name,
                                             string_type const& arg)
    {
        BOOST_LOG_THROW_DESCR_PARAMS(
            parse_error,
            "The less-or-equal attribute value relation is not supported",
            (name));
        BOOST_LOG_UNREACHABLE_RETURN(filter());
    }
    /*!
     ...
     */
    virtual filter on_greater_or_equal_relation(attribute_name const& name,
                                                string_type const& arg)
    {
        BOOST_LOG_THROW_DESCR_PARAMS(
            parse_error,
            "The greater-or-equal attribute value relation is not supported",
            (name));
        BOOST_LOG_UNREACHABLE_RETURN(filter());
    }
     ...
     */
    virtual filter on_custom_relation(attribute_name const& name,
                                      string_type const& rel,
                                      string_type const& arg)
    {
        BOOST_LOG_THROW_DESCR_PARAMS(parse_error,
                                     "The custom attribute value relation \"" +
                                         boost::log::aux::to_narrow(rel) +
                                         "\" is not supported",
                                     (name));
        BOOST_LOG_UNREACHABLE_RETURN(filter());
    }
...
/*!
 * The base class for filter factories. The class defines default
 * implementations for most filter expressions. In order to be able to construct
 * filters, the attribute value type must support reading from a stream. Also,
 * the default filters will rely on relational operators for the type, so these
 * operators must also be defined.
 */
template <typename CharT, typename AttributeValueT>
class basic_filter_factory : public filter_factory<CharT>
{
    //! Base type
  ...
  public:
    ...
    virtual filter on_equality_relation(attribute_name const& name,
                                        string_type const& arg)
    {
        return filter(expressions::attr<value_type>(name) ==
                      parse_argument(arg));
    }
    /*!
     * The callback for inequality relation filter
 ...
 * name. The factory will be used to construct a filter during parsing the
 * filter string.
 *
 * \pre <tt>name != NULL && factory != NULL</tt>, <tt>name</tt> points to a
 * zero-terminated string \param name Attribute name to associate the factory
 * with \param factory The filter factory
 */
template <typename CharT>
...
inline typename boost::enable_if_c<is_base_and_derived<
    filter_factory<typename FactoryT::char_type>, FactoryT>::value>::type
    register_filter_factory(attribute_name const& name,
                            shared_ptr<FactoryT> const& factory)
{
    typedef filter_factory<typename FactoryT::char_type> factory_base;
    register_filter_factory(name,
                            boost::static_pointer_cast<factory_base>(factory));
}

/*!
 * The function registers a simple filter factory object for the specified
 * attribute name. The factory will support attribute values of type \c
 * AttributeValueT, which must support all relation operations, such as equality
 * comparison and less/greater ordering, and also extraction from stream.
 *
...
/*!
 * The function registers a simple filter factory object for the specified
 * attribute keyword. The factory will support attribute values described by the
 * keyword. The values must support all relation operations, such as equality
 * comparison and less/greater ordering, and also extraction from stream.
 *
 * \pre <tt>name != NULL</tt>, <tt>name</tt> points to a zero-terminated string
...
inline void register_simple_filter_factory(
    expressions::attribute_keyword<DescriptorT, ActorT> const& keyword)
{
    register_simple_filter_factory<typename DescriptorT::value_type, CharT>(
        keyword.get_name());
}
 ...
 *
 * \b Throws: An <tt>std::exception</tt>-based exception, if a filter cannot be
 * recognized in the character sequence.
 */
template <typename CharT>
