...
struct DBusWatch
{
    ...
    DBusFreeFunction
        free_handler_data_function; /**< Free the watch handler data. */
 ...
 */
DBusWatch* _dbus_watch_new(DBusPollable fd, unsigned int flags,
                           dbus_bool_t enabled, DBusWatchHandler handler,
...
struct DBusWatchList
{
    ...
    DBusAddWatchFunction
        add_watch_function; /**< Callback for adding a watch. */
    DBusRemoveWatchFunction
        remove_watch_function; /**< Callback for removing a watch. */
    DBusWatchToggledFunction
        watch_toggled_function; /**< Callback on toggling enablement */
    void* watch_data;           /**< Data for watch callbacks */
    DBusFreeFunction
        watch_free_data_function; /**< Free function for watch callback data */
};
...
void _dbus_watch_list_free(DBusWatchList* watch_list)
{
    ...
    _dbus_list_foreach(&watch_list->watches,
                       (DBusForeachFunction)_dbus_watch_unref, NULL);
    ...
    if (add_function != NULL)
    {
        ...
        while (link != NULL)
        {
            ...
            if (!(*add_function)(link->data, data))
            {
                ...
                {
                    DBusList* next2 =
                        _dbus_list_get_next_link(&watch_list->watches, link2);
#ifdef DBUS_ENABLE_VERBOSE_MODE
                    DBusWatch* watch2 = link2->data;

                    _dbus_verbose("Removing watch on fd %" DBUS_POLLABLE_FORMAT
                                  " using newly-set remove function because "
                                  "initial add failed\n",
 ...
 */
void _dbus_watch_list_toggle_watch(DBusWatchList* watch_list, DBusWatch* watch,
                                   dbus_bool_t enabled)
{
    enabled = !!enabled;
 ...
 */
void _dbus_watch_set_handler(DBusWatch* watch, DBusWatchHandler handler,
                             void* data, DBusFreeFunction free_data_function)
{
    if (watch->free_handler_data_function)
        (*watch->free_handler_data_function)(watch->handler_data);
 ...
 *
 * Use dbus_connection_set_watch_functions() or
 * dbus_server_set_watch_functions() to be notified when libdbus needs to add or
 * remove watches.
 *
 * @{
 */
                         ...
                         DBusFreeFunction free_data_function)
{
    ...
    _dbus_verbose(
        ...
        _dbus_pollable_printable(watch->fd), data, free_data_function,
        watch->data, watch->free_data_function);
...
dbus_bool_t dbus_watch_handle(DBusWatch* watch, unsigned int flags)
{
    ...
    _dbus_return_val_if_fail(
        _dbus_pollable_is_valid(watch->fd) /* fails if watch was removed */,
        TRUE);
