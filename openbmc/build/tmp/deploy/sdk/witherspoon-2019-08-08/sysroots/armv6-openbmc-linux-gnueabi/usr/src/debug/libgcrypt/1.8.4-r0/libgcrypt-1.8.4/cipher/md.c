...
static gcry_md_spec_t* spec_from_algo(int algo)
{
    int idx;
    gcry_md_spec_t* spec;
...
static gcry_md_spec_t* spec_from_name(const char* name)
{
    gcry_md_spec_t* spec;
    int idx;
...
static gcry_md_spec_t* spec_from_oid(const char* oid)
{
    gcry_md_spec_t* spec;
...
static gcry_md_spec_t* search_oid(const char* oid, gcry_md_oid_spec_t* oid_spec)
{
    gcry_md_spec_t* spec;
    int i;
 ...
 */
int _gcry_md_map_name(const char* string)
{
    gcry_md_spec_t* spec;
 ...
 * is valid.
 */
const char* _gcry_md_algo_name(int algorithm)
{
    gcry_md_spec_t* spec;
...
static gcry_err_code_t check_digest_algo(int algorithm)
{
    gcry_md_spec_t* spec;
 ...
 * may be 0.
 */
static gcry_err_code_t md_open(gcry_md_hd_t* h, int algo, unsigned int flags)
{
...
static gcry_err_code_t md_enable(gcry_md_hd_t hd, int algorithm)
{
    ...
    {
        size_t size =
            (sizeof(*entry) + spec->contextsize * (h->flags.hmac ? 3 : 1) -
...
gcry_err_code_t _gcry_md_enable(gcry_md_hd_t hd, int algorithm)
{
    return md_enable(hd, algorithm);
}

static gcry_err_code_t md_copy(gcry_md_hd_t ahd, gcry_md_hd_t* b_hd)
{
    gcry_err_code_t err = 0;
...
gcry_err_code_t _gcry_md_copy(gcry_md_hd_t* handle, gcry_md_hd_t hd)
{
    gcry_err_code_t rc;
...
/* Note that this function may be used after finalize and read to keep
   ...
   attacks.  */
void _gcry_md_write(gcry_md_hd_t hd, const void* inbuf, size_t inlen)
{
...
static gcry_err_code_t md_setkey(gcry_md_hd_t h, const unsigned char* key,
                                 size_t keylen)
{
    ...
    for (r = h->ctx->list; r; r = r->next)
    {
        switch (r->spec->algo)
        {
            ...
            case GCRY_MD_BLAKE2S_128:
                algo_had_setkey = 1;
                memset(r->context.c, 0, r->spec->contextsize);
                rc = _gcry_blake2_init_with_key(
                    r->context.c,
                    h->ctx->flags.bugemu1 ? GCRY_MD_FLAG_BUGEMU1 : 0, key,
                    keylen, r->spec->algo);
                break;
            default:
...
static gcry_err_code_t prepare_macpads(gcry_md_hd_t a, const unsigned char* key,
                                       size_t keylen)
{
    GcryDigestEntry* r;
...
gcry_err_code_t _gcry_md_ctl(gcry_md_hd_t hd, int cmd, void* buffer,
                             size_t buflen)
{
    gcry_err_code_t rc = 0;
...
gcry_err_code_t _gcry_md_setkey(gcry_md_hd_t hd, const void* key, size_t keylen)
{
    gcry_err_code_t rc;
...
static byte* md_read(gcry_md_hd_t a, int algo)
{
    ...
    if (r && !r->spec->read)
        ...
        _gcry_fatal_error(GPG_ERR_DIGEST_ALGO,
                          "requested algo not in md context");
    return NULL;
}
 ...
 * the hash.
 */
byte* _gcry_md_read(gcry_md_hd_t hd, int algo)
{
 ...
 * only one)
 */
static gcry_err_code_t md_extract(gcry_md_hd_t a, int algo, void* out,
                                  size_t outlen)
{
    GcryDigestEntry* r = a->ctx->list;
 ...
 * the hash.
 */
gcry_err_code_t _gcry_md_extract(gcry_md_hd_t hd, int algo, void* out,
                                 size_t outlen)
{
    _gcry_md_ctl(hd, GCRYCTL_FINALIZE, NULL, 0);
 ...
 */
gcry_err_code_t _gcry_md_get(gcry_md_hd_t hd, int algo, byte* buffer,
                             int buflen)
{
    (void)hd;
    (void)algo;
                          ...
                          size_t length)
{
    ...
    else
    {
       ...
       normal functions. */
        ...
        if (err)
            log_bug("gcry_md_open failed for algo %d: %s", algo,
                    gpg_strerror(gcry_error(err)));
                                     ...
                                     const gcry_buffer_t* iov, int iovcnt)
{
    ...
    else
    {
       ...
       normal functions.  */
        ...
        if (hmac)
        {
            rc = _gcry_md_setkey(h, (const char*)iov[0].data + iov[0].off,
                                 iov[0].len);
            if (rc)
            {
                md_close(h);
 ...
 */
static int md_digest_length(int algorithm)
{
    gcry_md_spec_t* spec;
 ...
 */
unsigned int _gcry_md_get_algo_dlen(int algorithm)
{
...
static const byte* md_asn_oid(int algorithm, size_t* asnlen, size_t* mdlen)
{
    gcry_md_spec_t* spec;
 ...
 * the block size)
 */
gcry_err_code_t _gcry_md_algo_info(int algo, int what, void* buffer,
                                   size_t* nbytes)
{
    gcry_err_code_t rc;
    ...
    switch (what)
    {
        ...
        case GCRYCTL_SELFTEST:
            ...
            rc = gpg_err_code(
                _gcry_md_selftest(algo, nbytes ? (int)*nbytes : 0, NULL));
            break;
...
static void md_stop_debug(gcry_md_hd_t md)
{
    if (md->ctx->debug)
    {
        if (md->bufpos)
 ...
 */
gcry_err_code_t _gcry_md_info(gcry_md_hd_t h, int cmd, void* buffer,
                              size_t* nbytes)
{
    gcry_err_code_t rc = 0;
