...
static void ghash_setup_armv8_ce_pmull(gcry_cipher_hd_t c)
{
    _gcry_ghash_setup_armv8_ce_pmull(c->u_mode.gcm.u_ghash_key.key,
                                     c->u_mode.gcm.gcm_table);
}

static unsigned int ghash_armv8_ce_pmull(gcry_cipher_hd_t c, byte* result,
                                         const byte* buf, size_t nblocks)
{
    return _gcry_ghash_armv8_ce_pmull(c->u_mode.gcm.u_ghash_key.key, result,
...
static void bshift(u64* b0, u64* b1)
{
    u64 t[2], mask;
...
static void do_fillM(unsigned char* h, u64* M)
{
    int i, j;
...
static inline unsigned int do_ghash(unsigned char* result,
                                    const unsigned char* buf, const u64* gcmM)
{
    u64 V[2];
    u64 tmp[2];
    const u64* M;
    ...
    return (sizeof(V) + sizeof(T) + sizeof(tmp) + sizeof(int) * 2 +
            sizeof(void*) * 5);
}

#else  /*!GCM_TABLES_USE_U64*/

static void bshift(u32* M, int i)
{
    u32 t[4], mask;
...
static void do_fillM(unsigned char* h, u32* M)
{
    int i, j;
...
static inline unsigned int do_ghash(unsigned char* result,
                                    const unsigned char* buf, const u32* gcmM)
{
    byte V[16];
    u32 tmp[4];
    u32 v;
...
static unsigned int do_ghash(unsigned char* hsub, unsigned char* result,
...
#define GHASH(c, result, buf)                                                  \
    do_ghash(c->u_mode.gcm.u_ghash_key.key, result, buf)

#endif /* !GCM_USE_TABLES */

static unsigned int ghash_internal(gcry_cipher_hd_t c, byte* result,
                                   const byte* buf, size_t nblocks)
{
...
static void setupM(gcry_cipher_hd_t c)
{
...
static inline void gcm_bytecounter_add(u32 ctr[2], size_t add)
{
...
static inline u32 gcm_add32_be128(byte* ctr, unsigned int add)
{
...
static inline int gcm_check_datalen(u32 ctr[2])
{
...
static inline int gcm_check_aadlen_or_ivlen(u32 ctr[2])
{
...
static void do_ghash_buf(gcry_cipher_hd_t c, byte* hash, const byte* buf,
...
static gcry_err_code_t gcm_ctr_encrypt(gcry_cipher_hd_t c, byte* outbuf,
...
gcry_err_code_t _gcry_cipher_gcm_encrypt(gcry_cipher_hd_t c, byte* outbuf,
                                         size_t outbuflen, const byte* inbuf,
                                         size_t inbuflen)
{
...
gcry_err_code_t _gcry_cipher_gcm_decrypt(gcry_cipher_hd_t c, byte* outbuf,
                                         size_t outbuflen, const byte* inbuf,
                                         size_t inbuflen)
{
    static const unsigned char zerobuf[MAX_BLOCKSIZE];
...
gcry_err_code_t _gcry_cipher_gcm_authenticate(gcry_cipher_hd_t c,
                                              const byte* aadbuf,
                                              size_t aadbuflen)
{
    static const unsigned char zerobuf[MAX_BLOCKSIZE];
    ...
    if (c->marks.tag || c->u_mode.gcm.ghash_aad_finalized ||
        c->u_mode.gcm.ghash_data_finalized || !c->u_mode.gcm.ghash_fn)
        return GPG_ERR_INV_STATE;
...
void _gcry_cipher_gcm_setkey(gcry_cipher_hd_t c)
{
...
static gcry_err_code_t _gcry_cipher_gcm_initiv(gcry_cipher_hd_t c,
                                               const byte* iv, size_t ivlen)
{
    ...
    if (ivlen != GCRY_GCM_BLOCK_LEN - 4)
    {
        ...
        bitlengths[1][0] = be_bswap32((iv_bytes[0] >> 29) | (iv_bytes[1] << 3));
        /* zeros, 64-bit */
        bitlengths[0][1] = 0;
        bitlengths[0][0] = 0;
...
gcry_err_code_t _gcry_cipher_gcm_setiv(gcry_cipher_hd_t c, const byte* iv,
                                       size_t ivlen)
{
    c->marks.iv = 0;
    c->marks.tag = 0;
...
static gcry_err_code_t _gcry_cipher_gcm_tag(gcry_cipher_hd_t c, byte* outbuf,
...
gcry_err_code_t _gcry_cipher_gcm_get_tag(gcry_cipher_hd_t c,
...
gcry_err_code_t _gcry_cipher_gcm_check_tag(gcry_cipher_hd_t c,
