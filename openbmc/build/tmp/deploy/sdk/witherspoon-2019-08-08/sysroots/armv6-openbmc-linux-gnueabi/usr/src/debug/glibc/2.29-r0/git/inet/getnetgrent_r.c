...
/* Set up NIP to run through the services.  Return nonzero if there are no
   services (left).  */
static int setup(void** fctp, service_user** nipp)
{
...
static int __internal_setnetgrent_reuse(const char* group,
                                        struct __netgrent* datap, int* errnop)
{
    union
    {
    ...
    while (!no_more)
    {
        ...
        no_more =
            __nss_next2(&datap->nip, "setnetgrent", NULL, &fct.ptr, status, 0);
    ...
    new_elem = (struct name_list*)malloc(sizeof(struct name_list) + group_len);
    if (new_elem == NULL)
    {
        *errnop = errno;
...
libc_hidden_def(__internal_setnetgrent)
    ...
    static int nscd_setnetgrent(const char* group)
{
#ifdef USE_NSCD
...
static enum nss_status nscd_getnetgrent(struct __netgrent* datap, char* buffer,
...
int __internal_getnetgrent_r(char** hostp, char** userp, char** domainp,
                             struct __netgrent* datap, char* buffer,
                             size_t buflen, int* errnop)
{
       ...
       before considering any modules.  */
        ...
        while (!no_more)
        {
            ...
            if (status == NSS_STATUS_RETURN
                /* The service returned a NOTFOUND, but there are more groups
               that we need to resolve before we give up.  */
                ||
                (status == NSS_STATUS_NOTFOUND && datap->needed_groups != NULL))
            {
               ...
               if available.  */
                ...
                while (datap->needed_groups != NULL && !found)
                {
                    ...
                    found = __internal_setnetgrent_reuse(
                        datap->known_groups->name, datap, errnop);
                }
...
libc_hidden_def(__internal_getnetgrent_r)
    ...
    int __getnetgrent_r(char** hostp, char** userp, char** domainp,
                        char* buffer, size_t buflen)
{
    ...
    status = __internal_getnetgrent_r(hostp, userp, domainp, &dataset, buffer,
                                      buflen, &errno);
                ...
                const char* domain)
{
    ...
    while (1)
    {
        ...
        while (!no_more)
        {
            ...
            /* Open netgroup.  */
            enum nss_status status =
                DL_CALL_FCT(*setfct.f, (current_group, &entry));
                    ...
                    NULL)
            {
                ...
                while (DL_CALL_FCT(*getfct, (&entry, buffer, sizeof buffer,
                                             &errno)) == NSS_STATUS_SUCCESS)
                {
                    if (entry.type == group_val)
                    {
                            ...
                            strcmp(netgroup, entry.val.group) != 0)
                        {
                            ...
                            namep = (struct name_list*)malloc(sizeof(*namep) +
                                                              group_len);
                            if (namep == NULL)
                            {
                    ...
                    else
                    {
                        if ((entry.val.triple.host == NULL || host == NULL ||
                            ...
                            (entry.val.triple.domain == NULL ||
                             domain == NULL ||
                             __strcasecmp(entry.val.triple.domain, domain) ==
                                 0))
                        {
                            result = 1;
                            break;
            ...
            no_more = __nss_next2(&entry.nip, "setnetgrent", NULL, &setfct.ptr,
                                  status, 0);
        }
