...
{
    return socket_address_family(a) == AF_UNIX
               ? sockaddr_un_unlink(&a->sockaddr.un)
               : 0;
}
...
int socket_address_listen(const SocketAddress* a, int flags, int backlog,
                          ...
                          const char* bind_to_device, bool reuse_port,
                          bool free_bind, bool transparent,
                          mode_t directory_mode, mode_t socket_mode,
                          const char* label);
...
SocketAddressBindIPv6Only
    socket_address_bind_ipv6_only_from_string(const char* s) _pure_;
SocketAddressBindIPv6Only
...
#define send_one_fd(transport_fd, fd, flags)                                   \
    send_one_fd_iov_sa(transport_fd, fd, NULL, 0, NULL, 0, flags)
 ...
 * (8 bytes) and run over the structure. This macro returns the correct size
 * that must be passed to kernel.
 */
...
/* Covers only file system and abstract AF_UNIX socket addresses, but not
 * unnamed socket addresses. */
