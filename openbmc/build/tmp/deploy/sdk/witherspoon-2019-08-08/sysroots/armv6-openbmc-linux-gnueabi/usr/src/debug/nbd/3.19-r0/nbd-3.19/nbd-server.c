 ...
 * Version 1.1 - merging enhancements from Josh Parsons,
 *<josh@coombs.anu.edu.au> Version 1.2 - autodetect size of block devices, thanx
 *to Peter T. Breuer" <ptb@it.uc3m.es> Version 1.5 - can compile on Unix systems
 *that don't have 64 bit integer type, or don't have 64 bit file offsets by
 *defining FS_32BIT in compile options for nbd-server *only*. This can be done
...
char default_authname[] =
    SYSCONFDIR "/nbd-server/allow"; /**< default name of allow file */
...
static int writeit_tls(gnutls_session_t s, void* buf, size_t len)
{
    ...
    {
        DEBUG("+");
        if ((res = gnutls_record_send(s, buf, len)) < 0 &&
            !gnutls_error_is_fatal(res))
        {
            m = g_strdup_printf("issue while sending data: %s",
                                gnutls_strerror(res));
            err_nonfatal(m);
            g_free(m);
        }
        ...
        {
            m = g_strdup_printf("could not send data: %s",
                                gnutls_strerror(res));
            err_nonfatal(m);
            g_free(m);
            return -1;
...
static int readit_tls(gnutls_session_t s, void* buf, size_t len)
{
    ...
    {
        DEBUG("*");
        if ((res = gnutls_record_recv(s, buf, len)) < 0 &&
            !gnutls_error_is_fatal(res))
        {
            m = g_strdup_printf("issue while receiving data: %s",
                                gnutls_strerror(res));
            err_nonfatal(m);
            g_free(m);
        }
        ...
        {
            m = g_strdup_printf("could not receive data: %s",
                                gnutls_strerror(res));
            err_nonfatal(m);
            g_free(m);
            return -1;
...
void usage()
{
    ...
    printf("Usage: [ip:|ip6@]port file_to_export [size][kKmM] [-l "
           "authorize_file] [-r] [-m] [-c] [-C configuration file] [-p PID "
           "file name] [-o section name] [-M max connections] [-V]\n"
           ...
           "\t-M|--max-connections\tspecify the maximum number of opened "
           "connections\n"
           ...
           "listening.\n\tif not, the server will listen on all local IP "
           "addresses\n");
...
SERVER* cmdline(int argc, char* argv[], struct generic_conf* genconf)
{
            ...
            case 1:
                ...
                switch (nonspecial++)
                {
                    ...
                    case 1:
                        ...
                        {
                            fprintf(stderr, "E: The to be exported file needs "
    ...
    if (do_output)
    {
        if (!serve)
        {
            g_critical("Need a complete configuration on the command line to "
                       "output a config file section!");
            exit(EXIT_FAILURE);
        }
...
GArray* parse_cfile(gchar* f, struct generic_conf* genconf, bool expect_generic,
                    GError** e);
...
GArray* do_cfile_dir(gchar* dir, struct generic_conf* const genconf, GError** e)
{
    ...
    if (!dirh)
    {
        g_set_error(e, NBDS_ERR, NBDS_ERR_CFILE_DIR_UNKNOWN,
    ...
    if (errno)
    {
        g_set_error(e, NBDS_ERR, NBDS_ERR_CFILE_READDIR_ERR,
 ...
 **/
GArray* parse_cfile(gchar* f, struct generic_conf* const genconf,
                    bool expect_generic, GError** e)
{
    const char* DEFAULT_ERROR = "Could not parse %s in group %s: %s";
    const char* MISSING_REQUIRED_ERROR =
    ...
    PARAM gp[] = {
        ...
        {"oldstyle", FALSE, PARAM_BOOL, &(genconftmp.flags),
         F_OLDSTYLE}, // only left here so we can issue an appropriate error
                      // message when the option is used
        ...
        {"duallisten", FALSE, PARAM_BOOL, &(genconftmp.flags),
         F_DUAL_LISTEN}, // Used to listen on both TCP and unix socket
    ...
    if (!g_key_file_load_from_file(
            cfile, f, G_KEY_FILE_KEEP_COMMENTS | G_KEY_FILE_KEEP_TRANSLATIONS,
            &err))
    {
        g_set_error(e, NBDS_ERR, NBDS_ERR_CFILE_NOTFOUND,
                    "Could not open config file %s: %s", f, err->message);
    ...
    {
        g_set_error(e, NBDS_ERR, NBDS_ERR_CFILE_MISSING_GENERIC,
    ...
    for (i = 0; groups[i]; i++)
    {
        ...
        for (j = 0; j < p_size; j++)
        {
            ...
            assert(p[j].ptype == PARAM_INT || p[j].ptype == PARAM_STRING ||
                   p[j].ptype == PARAM_BOOL || p[j].ptype == PARAM_INT64);
            switch (p[j].ptype)
            {
                case PARAM_INT:
                    ival = g_key_file_get_integer(cfile, groups[i],
                                                  p[j].paramname, &err);
                    if (!err)
                    {
                    ...
                    break;
                case PARAM_INT64:
                    i64val = g_key_file_get_int64(cfile, groups[i],
                                                  p[j].paramname, &err);
                    if (!err)
                    {
                    ...
                    break;
                case PARAM_STRING:
                    sval = g_key_file_get_string(cfile, groups[i],
                                                 p[j].paramname, &err);
                    if (!err)
                    {
                    ...
                    break;
                case PARAM_BOOL:
                    bval = g_key_file_get_boolean(cfile, groups[i],
                                                  p[j].paramname, &err);
                    if (!err)
                    {
                        if (bval)
            ...
            if (err)
            {
                ...
                g_set_error(e, NBDS_ERR, NBDS_ERR_CFILE_VALUE_INVALID, err_msg,
                            p[j].paramname, groups[i], err->message);
                g_array_free(retval, TRUE);
                g_error_free(err);
                g_key_file_free(cfile);
                return NULL;
        ...
        if (virtstyle)
        {
            ...
            else if (!strncmp(virtstyle, "cidrhash", 8))
            {
                ...
                {
                    g_set_error(e, NBDS_ERR, NBDS_ERR_CFILE_VALUE_INVALID,
                                "Invalid value %s for parameter virtstyle in "
                                "group %s: missing length",
                                virtstyle, groups[i]);
                    g_array_free(retval, TRUE);
                    g_key_file_free(cfile);
                    return NULL;
            ...
            {
                g_set_error(
                    e, NBDS_ERR, NBDS_ERR_CFILE_VALUE_INVALID,
                    "Invalid value %s for parameter virtstyle in group %s",
                    virtstyle, groups[i]);
                g_array_free(retval, TRUE);
                g_key_file_free(cfile);
                return NULL;
        ...
        if (genconftmp.flags & F_OLDSTYLE)
        {
            g_message("Since 3.10, the oldstyle protocol is no longer "
                      "supported. Please migrate to the newstyle protocol.");
            g_message("Exiting.");
            return NULL;
        }
#ifndef HAVE_SPLICE
        if (s.flags & F_SPLICE)
        {
            g_set_error(e, NBDS_ERR, NBDS_ERR_CFILE_VALUE_UNSUPPORTED,
                        "This nbd-server was built without splice support, yet "
                        "group %s uses it",
                        groups[i]);
            g_array_free(retval, TRUE);
            g_key_file_free(cfile);
        ...
        {
            g_set_error(
                e, NBDS_ERR, NBDS_ERR_CFILE_INVALID_SPLICE,
        ...
        if ((s.flags & F_COPYONWRITE) && (s.flags & F_WAIT))
        {
            g_set_error(e, NBDS_ERR, NBDS_ERR_CFILE_INVALID_WAIT,
                        "Cannot mix copyonwrite with waitfile for an export in "
                        "group %s",
                        groups[i]);
            g_array_free(retval, TRUE);
            g_key_file_free(cfile);
    ...
    if (i == 1 && expect_generic)
    {
        g_set_error(e, NBDS_ERR, NBDS_ERR_CFILE_NO_EXPORTS,
 ...
 **/
int get_filepos(CLIENT* client, off_t a, int* fhandle, off_t* foffset,
                size_t* maxbytes)
{
    ...
    if (client->server->flags & F_TREEFILES)
    {
        ...
        *maxbytes = ((1 + (a / TREEPAGESIZE)) * TREEPAGESIZE) -
                    a; // start position of next block
        *fhandle = open_treefile(
            client->exportname,
            ((client->server->flags & F_READONLY) ? O_RDONLY : O_RDWR),
            client->exportsize, a, &client->lock);
        return 0;
    }
...
ssize_t rawexpwrite(off_t a, char* buf, size_t len, CLIENT* client, int fua)
{
    ...
    DEBUG("(WRITE to fd %d offset %llu len %u fua %d), ", fhandle,
          (long long unsigned)foffset, (unsigned int)len, fua);
    ...
    else if (fua)
    {
         ...
         * non-essential metadata (basically just timestamps in practice), but
         * it does flush metadata requried to find the data again, e.g.
         * allocation information and extent maps.  sync_file_range does nothing
         * but flush out pagecache content - it means you basically won't get
         * your data back in case of a crash if you either:
         *
         *  a) have a volatile write cache in your disk (e.g. any normal SATA
         * disk) b) are using a sparse file on a filesystem c) are using a
         * fallocate-preallocated file on a filesystem d) use any file on a COW
         * filesystem like btrfs
         *
         * e.g. it only does anything useful for you if you do not have a
         * volatile write cache, and either use a raw block device node, or just
         * overwrite an already fully allocated (and not preallocated) file on a
         * non-COW filesystem. [ENDS]
         *
...
ssize_t rawexpread(off_t a, char* buf, size_t len, CLIENT* client)
{
    ...
    DEBUG("(READ from fd %d offset %llu len %u), ", fhandle,
          (long long unsigned int)foffset, (unsigned int)len);
                 ...
                 int fua)
{
    ...
    DEBUG("(SPLICE %s fd %d offset %llu len %u), ",
          (dir == SPLICE_IN) ? "from" : "to", fhandle, (unsigned long long)a,
          (unsigned)len);
...
int expread(off_t a, char* buf, size_t len, CLIENT* client)
{
    ...
    if (!(client->server->flags & F_COPYONWRITE) &&
        !((client->server->flags & F_WAIT) && (client->export == NULL)))
        return (rawexpread_fully(a, buf, len, client));

    mapl = a / DIFFPAGESIZE;
    maph = (a + len - 1) / DIFFPAGESIZE;
    ...
    for (mapcnt = mapl; mapcnt <= maph; mapcnt++)
    {
        ...
        offset = a - pagestart;
        rdlen =
            (0 < DIFFPAGESIZE - offset && len < (size_t)(DIFFPAGESIZE - offset))
                ? len
        ...
        { /* the block is already there */
            ...
            if (pread(client->difffile, buf, rdlen,
                      client->difmap[mapcnt] * DIFFPAGESIZE + offset) != rdlen)
                goto fail;
        }
        else
        { /* the block is not there */
            ...
            else
            {
                DEBUG("Page %llu is not here, we read the original one\n",
                      (unsigned long long)mapcnt);
                if (rawexpread_fully(a, buf, rdlen, client))
                    goto fail;
            }
        }
        if (!(client->server->flags & F_COPYONWRITE))
...
int expwrite(off_t a, char* buf, size_t len, CLIENT* client, int fua)
{
    ...
    for (mapcnt = mapl; mapcnt <= maph; mapcnt++)
    {
        ...
        { /* the block is already there */
            DEBUG("Page %llu is at %lu\n", (unsigned long long)mapcnt,
                  (unsigned long)(client->difmap[mapcnt]));
            if (pwrite(client->difffile, buf, wrlen,
                       client->difmap[mapcnt] * DIFFPAGESIZE + offset) != wrlen)
                goto fail;
        }
        else
        { /* the block is not there */
            client->difmap[mapcnt] = (client->server->flags & F_SPARSE)
                                         ? mapcnt
                                         : client->difffilelen++;
            ...
            if ((offset != 0) || (wrlen != DIFFPAGESIZE))
            {
                if ((client->server->flags & F_WAIT) &&
                    (client->export == NULL))
                {
                    DEBUG("error: we can write only whole page while waiting "
                          "for file\n");
                    goto fail;
                }
                rdlen = DIFFPAGESIZE;
...
void punch_hole(int fd, off_t off, off_t len)
{
    ...
    DWORD bytesret;
    DeviceIoControl(w32handle, FSCTL_SET_ZERO_DATA, &zerodata, sizeof(zerodata),
...
int set_peername(int net, CLIENT* client)
{
    ...
    switch (client->server->virtstyle)
    {
        ...
        case VIRT_IPLIT:
            ...
            client->exportname =
                g_strdup_printf(client->server->exportname, peername);
            break;
        case VIRT_CIDR:
            ...
            else
            {
                ...
                uint8_t* addrptr =
                    (uint8_t*)(((struct sockaddr*)&netaddr)->sa_data);
                ...
                getnameinfo((struct sockaddr*)&netaddr, addrinlen, netname,
            ...
            {
                client->exportname =
                    g_strdup_printf(client->server->exportname, tmp);
                g_free(tmp);
            }
    ...
    freeaddrinfo(ai);
    msg(LOG_INFO, "connect from %s, assigned file is %s", peername,
        client->exportname);
...
int commit_diff(CLIENT* client, bool lock, int fhandle)
{
    ...
    for (int i = 0; i < pagecount; i++)
    {
        ...
        if (client->difmap[i] != (u32)-1)
        {
            ...
            if (pread(client->difffile, buf, DIFFPAGESIZE,
...
void* wait_file(void* void_ptr)
{
    ...
    msg(LOG_INFO, "File %s appeared, fd %d", client->server->exportname,
        fi.fhandle);
    ...
    // last time we lock export for the whole time until we switch write
    // destination
    pthread_rwlock_wrlock(&client->export_lock);
    do
    {
    ...
    pthread_rwlock_unlock(&client->export_lock);
    msg(LOG_INFO, "Waiting for file ended, switching to exported file %s",
        client->server->exportname);
...
bool setupexport(CLIENT* client)
{
    ...
    {
        client->export = NULL; // this could be thousands of files so we open
                               // handles on demand although its slower
        client->exportsize =
            client->server
                ->expected_size; // available space is not checked, as it could
    ...
    else
    {
        ...
        for (i = 0;; i++)
        {
            ...
            if (i)
                cancreate = 0;
            /* if expected_size is specified, and this is the first file, we can
             * create the file */
            mode_t mode = (client->server->flags & F_READONLY)
                              ? O_RDONLY
                              : (O_RDWR | (cancreate ? O_CREAT : 0));

            if (temporary)
            {
                tmpname =
                    g_strdup_printf("%s.%d-XXXXXX", client->exportname, i);
                DEBUG("Opening %s\n", tmpname);
            ...
            if (fi.fhandle == -1)
            {
                ...
                error_string = g_strdup_printf(
                    "Could not open exported file %s: %%m", tmpname);
    ...
    msg(LOG_INFO, "Size of exported file/device is %llu",
    ...
    if (treefile)
    {
        msg(LOG_INFO, "Total number of (potential) files: %" PRId64,
            (client->exportsize + TREEPAGESIZE - 1) / TREEPAGESIZE);
    }
    return true;
}
...
bool copyonwrite_prepare(CLIENT* client)
{
    ...
    client->difffilename = g_strdup_printf("%s/%s-%s-%d.diff", dir, export_base,
                                           client->clientname, (int)getpid());
    g_free(dir);
    g_free(export_base);
    msg(LOG_INFO, "About to create map and diff file %s", client->difffilename);
    client->difffile =
        open(client->difffilename, O_RDWR | O_CREAT | O_TRUNC, 0600);
    ...
    if ((client->difmap =
             calloc(client->exportsize / DIFFPAGESIZE, sizeof(u32))) == NULL)
    {
        err("Could not allocate memory");
        return false;
    }
    for (i = 0; i < client->exportsize / DIFFPAGESIZE; i++)
        client->difmap[i] = (u32)-1;
...
static bool commit_client(CLIENT* client, SERVER* server)
{
    ...
    if (client->server->transactionlog && (client->transactionlogfd == -1))
    {
        if ((client->transactionlogfd =
                 open(client->server->transactionlog, O_WRONLY | O_CREAT,
                      S_IRUSR | S_IWUSR)) == -1)
        {
            msg(LOG_INFO,
...
static CLIENT* handle_export_name(CLIENT* client, uint32_t opt, GArray* servers,
                                  uint32_t cflags)
{
    uint32_t namelen;
    char* name;
    ...
    free(name);
    err("Negotiation failed/8a: Requested export not found, or is TLS-only and "
        "client did not negotiate TLS");
}

static void handle_list(CLIENT* client, uint32_t opt, GArray* servers,
                        uint32_t cflags)
{
    uint32_t len;
    int i;
    ...
    {
        send_reply(
            client, opt, NBD_REP_ERR_INVALID, -1,
    ...
    if (!(glob_flags & F_LIST))
    {
        send_reply(client, opt, NBD_REP_ERR_POLICY, -1,
                   "Listing of exports denied by server configuration");
    ...
    for (i = 0; i < servers->len; i++)
    {
        ...
        send_reply(client, opt, NBD_REP_SERVER,
...
CLIENT* handle_starttls(CLIENT* client, int opt, GArray* servers,
                        uint32_t cflags, struct generic_conf* genconf)
{
#define check_rv(c)                                                            \
    if ((c) < 0)                                                               \
    {                                                                          \
    ...
    if (G_UNLIKELY(len != 0))
    {
        ...
        send_reply(client, opt, NBD_REP_ERR_INVALID, -1,
                   "Sending a STARTTLS command with data is invalid");
        return NULL;
    }
    ...
    check_rv(gnutls_certificate_set_x509_trust_file(
        x509_cred, genconf->cacertfile, GNUTLS_X509_FMT_PEM));
    check_rv(gnutls_certificate_set_x509_key_file(
        x509_cred, genconf->certfile, genconf->keyfile, GNUTLS_X509_FMT_PEM));
    ...
    gnutls_transport_set_ptr(*session,
                             (gnutls_transport_ptr_t)(intptr_t)client->net);
#endif
    do
    {
 ...
 */
static bool handle_info(CLIENT* client, uint32_t opt, GArray* servers,
                        uint32_t cflags)
{
    ...
    {
        send_reply(client, opt, NBD_REP_ERR_INVALID, -1,
                   "An OPT_INFO request cannot be smaller than the length of "
                   "the name + 6");
...
CLIENT* negotiate(int net, GArray* servers, struct generic_conf* genconf)
{
    ...
    do
    {
        ...
        opt = ntohl(opt);
        if (client->tls_session == NULL && glob_flags & F_FORCEDTLS &&
            opt != NBD_OPT_STARTTLS)
        {
            ...
            consume_len(client);
            send_reply(client, opt, NBD_REP_ERR_TLS_REQD, -1,
                       "TLS is required on this server");
            continue;
        }
        switch (opt)
        {
            ...
            case NBD_OPT_STARTTLS:
#if !HAVE_GNUTLS
                consume_len(client);
                send_reply(client, opt, NBD_REP_ERR_PLATFORM, -1,
                           "This nbd-server was compiled without TLS support");
#else
                if (client->tls_session != NULL)
                {
                    consume_len(client);
                    send_reply(client, opt, NBD_REP_ERR_INVALID, -1,
                               "Invalid STARTTLS request: TLS has already been "
                               "negotiated!");
                    continue;
                }
                ...
                if (handle_starttls(client, opt, servers, cflags, genconf) ==
                    NULL)
                {
            ...
            case NBD_OPT_GO:
            case NBD_OPT_INFO:
                if (handle_info(client, opt, servers, cflags) &&
                    opt == NBD_OPT_GO)
                {
                    return client;
                }
            ...
            default:
                ...
                send_reply(client, opt, NBD_REP_ERR_UNSUP, -1,
                           "The given option is unknown to this server "
                           "implementation");
                break;
        }
...
static void handle_write(struct work_package* pkg)
{
    ...
    if (!pkg->data)
    {
        if (expsplice(pkg->pipefd[0], req->from, req->len, client, SPLICE_OUT,
                      fua))
        {
...
static void handle_request(gpointer data, gpointer user_data)
{
    ...
    {
        msg(LOG_ERR, "E: received invalid flag %d on command %d, ignoring",
            flags, type);
        goto error;
    }
    ...
    switch (type)
    {
        ...
        case NBD_CMD_WRITE_ZEROES:
            ...
            break;
        default:
            msg(LOG_ERR, "E: received unknown command %d of type, ignoring",
                package->req->type);
            goto error;
    }
    goto end;
    ...
    for (i = 0; i < servers->len; i++)
    {
        ...
        if (strcmp(srv->servename, buf) == 0)
        {
            if (srv->max_connections == 0 ||
                srv->max_connections > srv->numclients)
            {
                writeit(socket, "Y", 1);
                srv->numclients++;
            }
...
static int append_new_servers(GArray* const servers, GError** const gerror)
{
    ...
    for (i = 0; i < new_servers->len; ++i)
    {
        ...
        if (new_server.servename &&
            -1 == get_index_by_servename(new_server.servename, servers))
        {
                   ...
                   const gint flags)
{
    ...
    children =
        g_hash_table_new_full(g_int_hash, g_int_equal, NULL, destroy_pid_t);
...
int main(int argc, char* argv[])
{
    ...
    if (serve)
    {
        g_warning("Specifying an export on the command line no longer uses the "
                  "oldstyle protocol.");
    }
...
#endif
    tpool =
        g_thread_pool_new(handle_request, NULL, genconf.threads, FALSE, NULL);
    ...
    gnutls_dh_params_generate2(
        dh_params, gnutls_sec_param_to_pk_bits(GNUTLS_PK_DH,
