...
static inline time_t datahead_init_common(struct datahead* head,
...
static inline time_t datahead_init_pos(struct datahead* head,
                                       nscd_ssize_t allocsize,
                                       nscd_ssize_t recsize, uint8_t nreloads,
                                       uint32_t ttl)
{
...
static inline time_t datahead_init_neg(struct datahead* head,
                                       nscd_ssize_t allocsize,
                                       nscd_ssize_t recsize, uint32_t ttl)
{
    time_t ret = datahead_init_common(head, allocsize, recsize, ttl);
    ...
    __nscd_acquire_maplock(volatile struct locked_map_ptr* mapptr)
{
    ...
    while (__builtin_expect(
        atomic_compare_and_exchange_val_acq(&mapptr->lock, 1, 0) != 0, 0))
    {
...
extern int __nscd_open_socket(const char* key, size_t keylen, request_type type,
                              void* response,
                              size_t responselen) attribute_hidden;

/* Try to get a file descriptor for the shared meory segment
   containing the database.  */
extern struct mapped_database*
...
extern struct mapped_database*
    __nscd_get_map_ref(request_type type, const char* name,
                       ...
                       int* gc_cyclep) attribute_hidden;

/* Unmap database.  */
extern void __nscd_unmap(struct mapped_database* mapped) attribute_hidden;
...
extern struct datahead*
    __nscd_cache_search(request_type type, const char* key, size_t keylen,
                        const struct mapped_database* mapped,
                        size_t datalen) attribute_hidden;

/* Wrappers around read, readv and write that only read/write less than LEN
   bytes on error or EOF.  */
extern ssize_t __readall(int fd, void* buf, size_t len) attribute_hidden;
extern ssize_t __readvall(int fd, const struct iovec* iov,
                          int iovcnt) attribute_hidden;
extern ssize_t writeall(int fd, const void* buf, size_t len) attribute_hidden;

/* Get netlink timestamp counter from mapped area or zero.  */
extern uint32_t __nscd_get_nl_timestamp(void) attribute_hidden;
