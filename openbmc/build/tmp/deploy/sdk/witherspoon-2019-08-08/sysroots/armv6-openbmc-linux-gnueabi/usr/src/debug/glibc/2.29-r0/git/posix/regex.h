...
#elif (                                                                        \
    (199901L <= __STDC_VERSION__ || 3 < __GNUC__ + (1 <= __GNUC_MINOR__)) &&   \
    !defined __GNUG__)
...
/* POSIX compatibility.  */
extern int regcomp(regex_t* _Restrict_ __preg, const char* _Restrict_ __pattern,
                   int __cflags);

extern int regexec(const regex_t* _Restrict_ __preg,
                   const char* _Restrict_ __String, size_t __nmatch,
                   regmatch_t __pmatch[_Restrict_arr_], int __eflags);
