...
static inline int isControlFrame(ws_ctx_t* wsctx)
{
    return 0 != (wsctx->header.opcode & 0x08);
}

static uint64_t hybiRemaining(ws_ctx_t* wsctx)
{
    return wsctx->header.payloadLen - wsctx->nReadPayload;
}

static void hybiDecodeCleanupBasics(ws_ctx_t* wsctx)
{
...
static void hybiDecodeCleanupForContinuation(ws_ctx_t* wsctx)
{
 ...
 */
static int hybiReturnData(char* dst, int len, ws_ctx_t* wsctx, int* nWritten)
{
 ...
 */
static int hybiReadHeader(ws_ctx_t* wsctx, int* sockRet, int* nPayload)
{
    int ret;
    ...
    wsctx->readPos =
        (unsigned char*)(wsctx->codeBufDecode + wsctx->header.headerLen);

    *nPayload = wsctx->header.nRead - wsctx->header.headerLen;
    wsctx->nReadPayload = *nPayload;

    ws_dbg("header complete: state=%d headerlen=%d payloadlen=%llu writeTo=%p "
           "nPayload=%d\n",
           wsctx->hybiDecodeState, wsctx->header.headerLen,
           wsctx->header.payloadLen, wsctx->writePos, *nPayload);
 ...
 *  @param[out] sockRet emulated recv return value
 *  @param[in]  nInBuf number of undecoded bytes before writePos from header
 * read
 *  @return next hybi decode state
 */
static int hybiReadAndDecode(ws_ctx_t* wsctx, char* dst, int len, int* sockRet,
                             int nInBuf)
{
    int n;
    int i;
    ...
    bufsize = wsctx->codeBufDecode + ARRAYSIZE(wsctx->codeBufDecode) -
              wsctx->writePos - 1;
    ws_dbg("bufsize=%d\n", bufsize);
    ...
    ws_dbg("calling read with buf=%p and len=%d (decodebuf=%p headerLen=%d)\n",
           wsctx->writePos, nextRead, wsctx->codeBufDecode,
           wsctx->header.headerLen);

    if (nextRead > 0)
    {
        /* decode more data */
        if (-1 == (n = wsctx->ctxInfo.readFunc(wsctx->ctxInfo.ctxPtr,
                                               wsctx->writePos, nextRead)))
        {
            int olderrno = errno;
    ...
    ws_dbg("toDecode=%d from n=%d carrylen=%d headerLen=%d\n", toDecode, n,
           wsctx->carrylen, wsctx->header.headerLen);
    if (toDecode < 0)
    {
        rfbErr("%s: internal error; negative number of bytes to decode: %d",
               __func__, toDecode);
        errno = EIO;
        *sockRet = -1;
        return WS_HYBI_STATE_ERR;
    ...
    else
    {
        ...
        {
            rfbErr("%s: internal error, invalid carry over size: carrylen=%d, "
                   "toDecode=%d, i=%d",
                   __func__, wsctx->carrylen, toDecode, i);
            *sockRet = -1;
            errno = EIO;
            return WS_HYBI_STATE_ERR;
        }
        ws_dbg("carrying over %d bytes from %p to %p\n", wsctx->carrylen,
               wsctx->writePos + (i * 4), wsctx->carryBuf);
    ...
    switch (wsctx->header.opcode)
    {
            ...
            if (hybiWsFrameComplete(wsctx))
            {
                *(wsctx->writePos) = '\0';
                ws_dbg("got close cmd %d, reason %d: %s\n",
                       (int)(wsctx->writePos - hybiPayloadStart(wsctx)),
                       WS_NTOH16(((uint16_t*)hybiPayloadStart(wsctx))[0]),
                       &hybiPayloadStart(wsctx)[2]);
                errno = ECONNRESET;
                *sockRet = -1;
        ...
        case WS_OPCODE_TEXT_FRAME:
            data[toReturn] = '\0';
            ws_dbg("Initiate Base64 decoding in %p with max size %d and '\\0' "
                   "at %p\n",
                   data, bufsize, data + toReturn);
            if (-1 ==
                (wsctx->readlen = rfbBase64PtoN((char*)data, data, bufsize)))
            {
        ...
        default:
            rfbErr("%s: unhandled opcode %d, b0: %02x, b1: %02x\n", __func__,
                   (int)wsctx->header.opcode, wsctx->header.data->b0,
                   wsctx->header.data->b1);
    }
    wsctx->readPos = data;
...
int webSocketsDecodeHybi(ws_ctx_t* wsctx, char* dst, int len)
{
    ...
    ws_dbg("%s_enter: len=%d; "
           ...
           __func__, len, wsctx->readlen, wsctx->readPos, wsctx->writePos,
           wsctx->hybiDecodeState, wsctx->header.payloadLen,
           hybiRemaining(wsctx), wsctx->nReadPayload, wsctx->carrylen,
           wsctx->carryBuf);
    ...
    switch (wsctx->hybiDecodeState)
    {
        ...
        case WS_HYBI_STATE_HEADER_PENDING:
            ...
            if (wsctx->hybiDecodeState != WS_HYBI_STATE_HEADER_PENDING)
            {
                ...
                wsctx->hybiDecodeState =
                    hybiReadAndDecode(wsctx, dst, len, &result, nInBuf);
            }
            break;
        ...
        case WS_HYBI_STATE_DATA_NEEDED:
            wsctx->hybiDecodeState =
                hybiReadAndDecode(wsctx, dst, len, &result, 0);
            break;
...
spor:
    if (wsctx->hybiDecodeState == WS_HYBI_STATE_FRAME_COMPLETE)
    {
        ws_dbg("frame received successfully, cleaning up: read=%d hlen=%d "
               "plen=%d\n",
               wsctx->header.nRead, wsctx->header.headerLen,
               wsctx->header.payloadLen);
