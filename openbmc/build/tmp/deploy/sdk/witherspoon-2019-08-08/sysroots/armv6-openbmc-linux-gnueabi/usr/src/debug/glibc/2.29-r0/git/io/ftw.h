...
#define FTW_SKIP_SUBTREE FTW_SKIP_SUBTREE
    FTW_SKIP_SIBLINGS = 3, /* Continue with FTW_DP callback for current
                   directory (if FTW_DEPTH) and then its siblings.  */
...
typedef int (*__ftw_func_t)(const char* __filename, const struct stat* __status,
                            int __flag);
...
#ifdef __REDIRECT
extern int
    __REDIRECT(ftw, (const char* __dir, __ftw_func_t __func, int __descriptors),
               ftw64) __nonnull((1, 2));
#else
#define ftw ftw64
#endif
...
extern int __REDIRECT(nftw,
                      ...
                      nftw64) __nonnull((1, 2));
#else
#define nftw nftw64
#endif
#endif
#ifdef __USE_LARGEFILE64
extern int nftw64(const char* __dir, __nftw64_func_t __func, int __descriptors,
                  int __flag) __nonnull((1, 2));
#endif
#endif
