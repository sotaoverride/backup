...
static void rfbInitOneRGBTable24(uint8_t* table, int inMax, int outMax,
                                 int outShift, int swap);

static void rfbInitColourMapSingleTable24(char** table, rfbPixelFormat* in,
                                          rfbPixelFormat* out,
...
static void rfbInitTrueColourSingleTable24(char** table, rfbPixelFormat* in,
                                           rfbPixelFormat* out)
{
    int i, outValue;
    ...
    *table = (char*)malloc(nEntries * 3 + 1);
    ...
    for (i = 0; i < nEntries; i++)
    {
        ...
        outValue = ((outRed << out->redShift) | (outGreen << out->greenShift) |
...
static void rfbInitTrueColourRGBTables24(char** table, rfbPixelFormat* in,
                                         rfbPixelFormat* out)
{
    uint8_t* redTable;
    uint8_t* greenTable;
    ...
    if (*table)
        free(*table);
    *table =
        (char*)malloc((in->redMax + in->greenMax + in->blueMax + 3) * 3 + 1);
    redTable = (uint8_t*)*table;
    greenTable = redTable + 3 * (in->redMax + 1);
    blueTable = greenTable + 3 * (in->greenMax + 1);

    rfbInitOneRGBTable24(redTable, in->redMax, out->redMax, out->redShift,
                         (out->bigEndian != in->bigEndian));
    rfbInitOneRGBTable24(greenTable, in->greenMax, out->greenMax,
                         out->greenShift, (out->bigEndian != in->bigEndian));
    rfbInitOneRGBTable24(blueTable, in->blueMax, out->blueMax, out->blueShift,
                         (out->bigEndian != in->bigEndian));
}
