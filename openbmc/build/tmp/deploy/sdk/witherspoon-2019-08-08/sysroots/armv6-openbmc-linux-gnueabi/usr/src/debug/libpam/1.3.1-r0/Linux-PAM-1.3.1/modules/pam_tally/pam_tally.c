...
static void _cleanup(pam_handle_t* pamh UNUSED, void* data,
...
static int get_tally(pam_handle_t* pamh, tally_t* tally, uid_t uid,
                     const char* filename, FILE** TALLY, struct fail_s* fsp)
{
    ...
    {
        pam_syslog(pamh, LOG_ALERT, "Error opening %s for %s", filename,
                   *tally != TALLY_HI ? "update" : "read");
    ...
    else if ((fread((char*)&fsp->fs_faillog, sizeof(struct faillog), 1,
                    *TALLY)) == 0)
    {
...
static int set_tally(pam_handle_t* pamh, tally_t tally, uid_t uid,
                     const char* filename, FILE** TALLY, struct fail_s* fsp)
{
    ...
    if (tally != TALLY_HI)
    {
        ...
        else
        {
            ...
            if (fwrite((char*)&fsp->fs_faillog, sizeof(struct faillog), 1,
                       *TALLY) == 0)
            {
                pam_syslog(pamh, LOG_ALERT, "update (fwrite) failed for %s",
                           filename);
                retval = PAM_AUTH_ERR;
            }
        }
...
{
    tally_t tally = 0; /* !TALLY_HI --> Log opened for update */

    FILE* TALLY = NULL;
    const void *remote_host = NULL, *cur_tty = NULL;
    ...
    if (!remote_host)
    {
        ...
        if (!cur_tty)
        {
            ...
            fsp->fs_faillog.fail_line[sizeof(fsp->fs_faillog.fail_line) - 1] =
                0;
        }
        else
        {
...
static int tally_check(time_t oldtime, pam_handle_t* pamh, uid_t uid,
                       const char* user, struct tally_options* opts)
{
    tally_t deny = opts->deny;
    tally_t tally = TALLY_HI;
    long lock_time = opts->lock_time;
    ...
    { /* magic_root skips tally check */
        ...
        if (lock_time && oldtime && !(opts->ctrl & OPT_NO_LOCK_TIME))
        {
        ...
        if ((deny != 0) &&    /* deny==0 means no deny        */
            (tally > deny) && /* tally>deny means exceeded    */
            (((opts->ctrl & OPT_DENY_ROOT) ||
              uid)) /* even_deny stops uid check    */
        )
        {
            if (!(opts->ctrl & OPT_SILENT))
...
int main(int argc UNUSED, char** argv)
{
    ...
    else /* !cline_user (ie, operate on all users) */
    {
        ...
        for (; !feof(TALLY); uid++)
        {
            tally_t tally;
            struct passwd* pw;
            if (!fread((char*)&fsp->fs_faillog, sizeof(struct faillog), 1,
                       TALLY) ||
