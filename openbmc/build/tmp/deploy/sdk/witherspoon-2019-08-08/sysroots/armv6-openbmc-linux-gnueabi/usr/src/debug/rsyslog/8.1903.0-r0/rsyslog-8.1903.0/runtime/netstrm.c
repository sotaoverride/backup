...
/* static data */
DEFobjStaticHelpers DEFobjCurrIf(netstrms)

    /* Standard-Constructor */
    BEGINobjConstruct(
        netstrm) /* be sure to specify the object type also in END macro! */
    ENDobjConstruct(netstrm)

    /* destructor for the netstrm object */
    BEGINobjDestruct(netstrm) /* be sure to specify the object type also in END
                                 and CODESTART macros! */
    CODESTARTobjDestruct(netstrm) if (pThis->pDrvrData != NULL) iRet =
 ...
 */
static rsRetVal AcceptConnReq(netstrm_t* pThis, netstrm_t** ppNew)
{
    nsd_t* pNewNsd = NULL;
    DEFiRet;
    ...
    CHKiRet(objUse(
        netstrms, DONT_LOAD_LIB)); /* use netstrms obj if not already done so */
 ...
 */
static rsRetVal LstnInit(netstrms_t* pNS, void* pUsr,
 ...
 */
static rsRetVal Rcv(netstrm_t* pThis, uchar* pBuf, ssize_t* pLenBuf,
                    int* const oserr)
{
    DEFiRet;
    ISOBJ_TYPE_assert(pThis, netstrm);
...
static rsRetVal SetGnutlsPriorityString(netstrm_t* pThis,
                                        uchar* gnutlsPriorityString)
{
    DEFiRet;
    ISOBJ_TYPE_assert(pThis, netstrm);
    iRet = pThis->Drvr.SetGnutlsPriorityString(pThis->pDrvrData,
 ...
 * rgerhards, 2008-03-19
 */
static rsRetVal Connect(netstrm_t* pThis, int family, uchar* port, uchar* host,
                        char* device)
{
    DEFiRet;
    ISOBJ_TYPE_assert(pThis, netstrm);
...
BEGINobjQueryInterface(netstrm)
    CODESTARTobjQueryInterface(netstrm) if (pIf->ifVersion !=
                                            netstrmCURR_IF_VERSION)
...
finalize_it
    : ENDobjQueryInterface(netstrm)
       ...
       */
      BEGINObjClassExit(
          netstrm, OBJ_IS_LOADABLE_MODULE) /* CHANGE class also in END MACRO! */
...
ENDObjClassExit(netstrm)
     ...
     */
    BEGINAbstractObjClassInit(netstrm, 1,
