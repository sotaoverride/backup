 ...
 * `HKEY_CLASSES_ROOT`.
 * On macOS it is a [Uniform Type
 *Identifier](https://en.wikipedia.org/wiki/Uniform_Type_Identifier) such as
...
gchar* _g_unix_content_type_unalias(const gchar* type)
{
    gchar* res;
...
gchar** _g_unix_content_type_get_parents(const gchar* type)
{
    const gchar* umime;
    gchar** parents;
    GPtrArray* array;
...
static void _g_content_type_set_mime_dirs_locked(const char* const* dirs)
{
    ...
    else
    {
        ...
        g_ptr_array_add(mime_dirs,
                        g_build_filename(g_get_user_data_dir(), "mime", NULL));
        for (; *system_dirs != NULL; system_dirs++)
            g_ptr_array_add(mime_dirs,
                            g_build_filename(*system_dirs, "mime", NULL));
...
static int language_level(const char* lang)
{
...
static void mime_info_start_element(GMarkupParseContext* context,
...
static void mime_info_end_element(GMarkupParseContext* context,
...
static void mime_info_text(GMarkupParseContext* context, const gchar* text,
...
static char* load_comment_for_mime_helper(const char* dir, const char* basename)
{
    ...
    GMarkupParser parser = {mime_info_start_element, mime_info_end_element,
                            mime_info_text};
...
static char* load_comment_for_mime(const char* mimetype)
{
...
gchar* g_content_type_get_description(const gchar* type)
{
    ...
    G_LOCK(gio_xdgmime);
    g_hash_table_insert(type_comment_cache, g_strdup(type), g_strdup(comment));
    G_UNLOCK(gio_xdgmime);
...
static GIcon* g_content_type_get_icon_internal(const gchar* type,
                                               gboolean symbolic)
{
    char* mimetype_icon;
 ...
 *
 * Returns: (transfer full): #GIcon corresponding to the content type. Free the
...
static gboolean looks_like_text(const guchar* data, gsize data_size)
{
    gsize i;
    char c;
 ...
 */
gchar* g_content_type_guess(const gchar* filename, const guchar* data,
                            gsize data_size, gboolean* result_uncertain)
{
    ...
    if (filename)
    {
        ...
        else
        {
            ...
            n_name_mimetypes = xdg_mime_get_mime_types_from_file_name(
                basename, name_mimetypes, 10);
            g_free(basename);
        }
    }
    ...
    {
        sniffed_mimetype =
            xdg_mime_get_mime_type_for_data(data, data_size, &sniffed_prio);
        if (sniffed_mimetype == XDG_MIME_TYPE_UNKNOWN && data &&
    ...
    {
        if (sniffed_mimetype == XDG_MIME_TYPE_UNKNOWN && result_uncertain)
            *result_uncertain = TRUE;
    ...
    else
    {
        ...
        if (sniffed_mimetype != XDG_MIME_TYPE_UNKNOWN)
        {
            ...
            else
            {
                ...
                {
                    if (xdg_mime_mime_type_subclass(name_mimetypes[i],
                                                    sniffed_mimetype))
                    {
...
static void enumerate_mimetypes_subdir(const char* dir, const char* prefix,
            ...
            if (g_str_has_suffix(ent->d_name, ".xml"))
            {
                mimetype =
                    g_strdup_printf("%s/%.*s", prefix,
                                    (int)strlen(ent->d_name) - 4, ent->d_name);
...
static void enumerate_mimetypes_dir(const char* dir, GHashTable* mimetypes)
{
    DIR* d;
    struct dirent* ent;
    const char* mimedir;
...
static void tree_matchlet_free(TreeMatchlet* matchlet)
{
...
static TreeMatch* parse_header(gchar* line)
{
    gint len;
    gchar* s;
    TreeMatch* match;
...
static TreeMatchlet* parse_match_line(gchar* line, gint* depth)
{
    gchar *s, *p;
...
static gint cmp_match(gconstpointer a, gconstpointer b)
{
...
static void insert_match(TreeMatch* match)
{
    tree_matches = g_list_insert_sorted(tree_matches, match, cmp_match);
}

static void insert_matchlet(TreeMatch* match, TreeMatchlet* matchlet,
                            gint depth)
{
    if (depth == 0)
...
static void read_tree_magic_from_directory(const gchar* prefix)
{
    gchar* filename;
    gchar* text;
    gsize len;
...
static void xdg_mime_reload(void* user_data)
{
    tree_magic_schedule_reload();
}

static void tree_magic_shutdown(void)
{
    g_list_free_full(tree_matches, (GDestroyNotify)tree_match_free);
    tree_matches = NULL;
}

static void tree_magic_init(void)
{
...
static gboolean component_match(Enumerator* e, gint depth, const gchar* name)
{
...
static GFile* next_match_recurse(Enumerator* e, gint depth)
{
    GFile* file;
    GFileInfo* info;
    const gchar* name;
    ...
    while (TRUE)
    {
        if (e->enumerators[depth] == NULL)
        {
            if (depth > 0)
            {
                ...
                if (file)
                {
                    ...
                    e->enumerators[depth] = g_file_enumerate_children(
        ...
        while ((info = g_file_enumerator_next_file(e->enumerators[depth], NULL,
                                                   NULL)))
        {
...
static GFile* enumerator_next(Enumerator* e)
{
    return next_match_recurse(e, e->depth - 1);
}

static Enumerator* enumerator_new(GFile* root, const char* path,
                                  gboolean ignore_case)
{
    Enumerator* e;
    gint i;
    ...
    e->enumerators[0] =
        g_file_enumerate_children(root, G_FILE_ATTRIBUTE_STANDARD_NAME,
                                  G_FILE_QUERY_INFO_NONE, NULL, NULL);

    return e;
}

static void enumerator_free(Enumerator* e)
{
    gint i;
...
static gboolean matchlet_match(TreeMatchlet* matchlet, GFile* root)
{
    GFile* file;
    GFileInfo* info;
    gboolean result;
    ...
    do
    {
        ...
        if (result && matchlet->non_empty)
        {
            ...
            {
                child_info =
                    g_file_enumerator_next_file(child_enum, NULL, NULL);
                if (child_info)
        ...
        {
            if (strcmp(matchlet->mimetype,
                       g_file_info_get_content_type(info)) != 0)
                result = FALSE;
        }
...
static void match_match(TreeMatch* match, GFile* root, GPtrArray* types)
{
    GList* l;
