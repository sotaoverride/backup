...
enum nss_status _nss_dns_getcanonname_r(const char* name, char* buffer,
                                        ...
                                        int* errnop, int* h_errnop)
{
    ...
    {
        int r =
            __res_context_query(ctx, name, ns_c_in, qtypes[i], buf, sizeof(buf),
                                &ansp.ptr, NULL, NULL, NULL, NULL);
        if (r > 0)
        {
            ...
            while (ancount-- > 0)
            {
               ...
               metadata.  */
                ...
                {
                    /* We found the record.  */
                    s = __dn_expand(ansp.buf->buf, endptr, namestart, buffer,
                                    buflen);
                    if (s < 0)
                    {
                        if (errno != EMSGSIZE)
