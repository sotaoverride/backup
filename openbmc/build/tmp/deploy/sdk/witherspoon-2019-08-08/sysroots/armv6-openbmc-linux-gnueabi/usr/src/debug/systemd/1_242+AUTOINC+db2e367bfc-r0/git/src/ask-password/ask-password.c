...
static int help(void)
{
    ...
    printf("%s [OPTIONS...] MESSAGE\n\n"
           "Query the user for a system passphrase, via the TTY or an UI "
           "agent.\n\n"
           "  -h --help           Show this help\n"
           "     --icon=NAME      Icon name\n"
           "     --id=ID          Query identifier (e.g. "
           "\"cryptsetup:/dev/sda5\")\n"
           "     --keyname=NAME   Kernel key name for caching passwords (e.g. "
           "\"cryptsetup\")\n"
...
static int run(int argc, char* argv[])
{
    ...
    r = ask_password_auto(arg_message, arg_icon, arg_id, arg_keyname, timeout,
                          arg_flags, &l);
    if (r < 0)
