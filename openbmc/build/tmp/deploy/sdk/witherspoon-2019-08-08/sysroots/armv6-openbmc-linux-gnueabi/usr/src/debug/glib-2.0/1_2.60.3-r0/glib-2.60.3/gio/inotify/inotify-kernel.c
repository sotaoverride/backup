...
static ik_event_t* ik_event_new(struct inotify_event* kevent, gint64 now)
{
...
static gint64 ik_source_get_dispatch_time(InotifyKernelSource* iks)
{
    ik_event_t* head;
...
static gboolean ik_source_can_dispatch_now(InotifyKernelSource* iks, gint64 now)
{
...
static gsize ik_source_read_some_events(InotifyKernelSource* iks, gchar* buffer,
                                        gsize buffer_len)
{
    gssize result;
    int errsv;
...
static gchar* ik_source_read_all_the_events(InotifyKernelSource* iks,
                                            ...
                                            gsize* length_out)
{
    ...
    if (n_read + MAX_EVENT_SIZE > buffer_len)
    {
        ...
        if (n_readable != 0)
        {
            ...
            n_read += ik_source_read_some_events(iks, new_buffer + n_read,
                                                 n_readable);
...
static gboolean ik_source_dispatch(GSource* source, GSourceFunc func,
                                   gpointer user_data)
{
    ...
    if (iks->is_bored || g_source_query_unix_fd(source, iks->fd_tag))
    {
         ...
         * buffer.
         */
        buffer = ik_source_read_all_the_events(
            iks, stack_buffer, sizeof stack_buffer, &buffer_len);

        offset = 0;

        while (offset < buffer_len)
        {
            struct inotify_event* kevent =
            ...
            if (event->mask & IN_MOVED_TO)
            {
                ...
                pair = g_hash_table_lookup(iks->unmatched_moves,
                                           GUINT_TO_POINTER(event->cookie));
                if (pair != NULL)
                {
                    g_assert(!pair->pair);

                    g_hash_table_remove(iks->unmatched_moves,
                                        GUINT_TO_POINTER(event->cookie));
            ...
            else if (event->mask & IN_MOVED_FROM)
            {
                ...
                new =
                    g_hash_table_insert(iks->unmatched_moves,
                                        GUINT_TO_POINTER(event->cookie), event);
                if
                    G_UNLIKELY(!new)
                g_warning("inotify: got IN_MOVED_FROM event with "
                          "already-pending cookie %#x",
                          event->cookie);
    ...
    g_assert((iks->queue.length > 0) ==
             (g_hash_table_size(iks->unmatched_moves) > 0));
