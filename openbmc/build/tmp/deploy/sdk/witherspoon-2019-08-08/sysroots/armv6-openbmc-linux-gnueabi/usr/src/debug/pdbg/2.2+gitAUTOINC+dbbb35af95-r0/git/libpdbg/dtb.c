...
static void* bmc_target(void)
{
    ...
    else
    {
        if (!strcmp(pdbg_backend_option, "p9"))
            ...
            pdbg_log(PDBG_WARNING,
                     "Invalid OpenBMC system type '%s' specified\n",
                     pdbg_backend_option);
    }
    ...
    switch (chip_id)
    {
        ...
        default:
            pdbg_log(PDBG_ERROR,
                     "Unrecognised Chip ID register 0x%08" PRIx32 "\n",
                     chip_id);
            return NULL;
    }
}
...
void* pdbg_default_dtb(void)
{
    ...
    switch (pdbg_backend)
    {
        ...
        default:
            pdbg_log(PDBG_WARNING,
                     "Unable to determine a valid default backend, using fake "
