    ...
    while (true)
        switch (coder->sequence)
        {
            ...
            case SEQ_BLOCK_HEADER:
            {
                ...
                return_if_error(lzma_block_header_decode(
                    &coder->block_options, allocator, coder->buffer));
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        ret = lzma_block_decoder_init(&coder->block_decoder,
                                                      allocator,
                                                      &coder->block_options);
                    }
                }
            ...
            case SEQ_BLOCK:
            {
                const lzma_ret ret = coder->block_decoder.code(
                    coder->block_decoder.coder, allocator, in, in_pos, in_size,
                ...
                // the Index hash.
                return_if_error(lzma_index_hash_append(
                    coder->index_hash,
            ...
            case SEQ_INDEX:
            {
                ...
                const lzma_ret ret = lzma_index_hash_decode(
                    coder->index_hash, in, in_pos, in_size);
                if (ret != LZMA_STREAM_END)
                    return ret;
            ...
            case SEQ_STREAM_FOOTER:
            {
                ...
                const lzma_ret ret =
                    lzma_stream_footer_decode(&footer_flags, coder->buffer);
                if (ret != LZMA_OK)
                ...
                return_if_error(lzma_stream_flags_compare(&coder->stream_flags,
                                                          &footer_flags));
            ...
            case SEQ_STREAM_PADDING:
                ...
                while (true)
                {
                    if (*in_pos >= in_size)
                    {
                        ...
                        // four bytes.
                        return coder->pos == 0 ? LZMA_STREAM_END
                                               : LZMA_DATA_ERROR;
                    }
...
static lzma_ret stream_decoder_memconfig(void* coder_ptr, uint64_t* memusage,
