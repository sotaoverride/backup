...
#define log_debug_bus_message(m)                                               \
    ...
    {                                                                          \
        ...
        log_debug(                                                             \
            "Got message type=%s sender=%s destination=%s path=%s "            \
            "interface=%s member=%s cookie=%" PRIu64 " reply_cookie=%" PRIu64  \
            ...
            strna(sd_bus_message_get_member(_mm)), BUS_MESSAGE_COOKIE(_mm),    \
            _mm->reply_cookie, strna(_mm->root_container.signature),           \
...
_public_ int sd_bus_negotiate_timestamp(sd_bus* bus, int b)
{
    ...
    /* This is not actually supported by any of our transports these days, but
     * we do honour it for synthetic replies, and maybe one day classic D-Bus
     * learns this too */
...
static int synthesize_connected_signal(sd_bus* bus)
{
    ...
    /* If enabled, synthesizes a local "Connected" signal mirroring the local
     * "Disconnected" signal. This is called whenever we fully established a
     * connection, i.e. after the authorization phase, and after receiving the
     * Hello() reply. Or in other words, whenver we enter BUS_RUNNING state.
     *
     * This is useful so that clients can start doing stuff whenver the
     * connection is fully established in a way that works independently from
     * whether we connected to a full bus or just a direct connection. */

    if (!bus->connected_signal)
        return 0;

    r = sd_bus_message_new_signal(bus, &m, "/org/freedesktop/DBus/Local",
                                  "org.freedesktop.DBus.Local", "Connected");
    if (r < 0)
        return r;
    ...
    memmove(bus->rqueue + 1, bus->rqueue,
            sizeof(sd_bus_message*) * bus->rqueue_size);
...
void bus_set_state(sd_bus* bus, enum bus_state state)
{
    ...
    log_debug("Bus %s: changing state %s → %s", strna(bus->description),
              table[bus->state], table[state]);
    bus->state = state;
}
...
int bus_start_running(sd_bus* bus)
{
    ...
    /* We start all method call timeouts when we enter BUS_HELLO or BUS_RUNNING
     * mode. At this point let's convert all relative to absolute timestamps.
     * Note that we do not reshuffle the reply callback priority queue since
...
static int parse_unix_address(sd_bus* b, const char** p, char** guid)
{
    ...
    if (path)
    {
        l = strlen(path);
        if (l >=
            sizeof(b->sockaddr.un.sun_path)) /* We insist on NUL termination */
            return -E2BIG;
...
static int parse_container_unix_address(sd_bus* b, const char** p, char** guid)
{
    ...
    b->sockaddr.un = (struct sockaddr_un){
        .sun_family = AF_UNIX,
        /* Note that we use the old /var/run prefix here, to increase
           compatibility with really old containers */
...
static int bus_start_address(sd_bus* b)
{
    ...
    next:
        ...
        if (r < 0)
            return r;
        if (r == 0)
            return b->last_connect_error > 0 ? -b->last_connect_error
                                             : -ECONNREFUSED;
    }
}
...
_public_ int sd_bus_open_system_with_description(sd_bus** ret,
...
_public_ int sd_bus_open_user_with_description(sd_bus** ret,
                                               const char* description)
{
    _cleanup_(bus_freep) sd_bus* b = NULL;
    int r;
...
int bus_set_address_system_remote(sd_bus* b, const char* host)
{
    ...
    a = strjoin("unixexec:path=ssh,argv1=-xT", p ? ",argv2=-p,argv3=" : "",
                strempty(p), ",argv", p ? "4" : "2", "=--,argv", p ? "5" : "3",
                "=", e, ",argv", p ? "6" : "4", "=systemd-stdio-bridge", c);
    if (!a)
        return -ENOMEM;
...
_public_ int sd_bus_open_system_machine(sd_bus** ret, const char* machine)
{
    ...
    assert_return(ret, -EINVAL);
    assert_return(streq(machine, ".host") || machine_name_is_valid(machine),
                  -EINVAL);
...
static uint64_t cookie_inc(uint64_t cookie)
{
    ...
    if (cookie >= UINT32_MAX)
        return COOKIE_CYCLED; /* Don't go back to zero, but use the highest bit
                               * for checking whether we are looping. */
...
static int next_cookie(sd_bus* b)
{
    ...
    /* Small optimization: don't bother with checking for cookie reuse until we
     * overran cookiespace at least once, but then do it thorougly. */
...
static int bus_write_message(sd_bus* bus, sd_bus_message* m, size_t* idx)
{
    ...
    if (*idx >= BUS_MESSAGE_SIZE(m))
        log_debug("Sent message type=%s sender=%s destination=%s path=%s "
                  "interface=%s member=%s cookie=%" PRIu64
                  " reply_cookie=%" PRIu64
                  " signature=%s error-name=%s error-message=%s",
                  ...
                  m->reply_cookie, strna(m->root_container.signature),
                  strna(m->error.name), strna(m->error.message));
...
static int dispatch_wqueue(sd_bus* bus)
{
    ...
    while (bus->wqueue_size > 0)
    {
        ...
        else if (bus->windex >= BUS_MESSAGE_SIZE(bus->wqueue[0]))
        {
            ...
            memmove(bus->wqueue, bus->wqueue + 1,
                    sizeof(sd_bus_message*) * bus->wqueue_size);
            bus->windex = 0;
...
static int dispatch_rqueue(sd_bus* bus, bool hint_priority, int64_t priority,
                           sd_bus_message** m)
{
    int r, ret = 0;
...
_public_ int sd_bus_send_to(sd_bus* bus, sd_bus_message* m,
...
static usec_t calc_elapse(sd_bus* bus, uint64_t usec)
{
     ...
     * so that the don't run in parallel with any connection setup states.
     * Hence, if a method callback is started earlier than that we just store
     * the relative timestamp, and afterwards the absolute one. */
...
_public_ int sd_bus_call_async(sd_bus* bus, sd_bus_slot** slot,
                               sd_bus_message* _m,
                               ...
                               void* userdata, uint64_t usec)
{
    ...
    assert_return(
        !m->sealed ||
            (!!callback == !(m->header->flags & BUS_MESSAGE_NO_REPLY_EXPECTED)),
        -EINVAL);
    ...
    /* If no callback is specified and there's no interest in a slot, then
     * there's no reason to ask for a reply */
    if (!callback && !slot && !m->sealed)
        m->header->flags |= BUS_MESSAGE_NO_REPLY_EXPECTED;

    r = ordered_hashmap_ensure_allocated(&bus->reply_callbacks,
                                         &uint64_hash_ops);
    if (r < 0)
        return r;
    ...
    if (slot || callback)
    {
        s = bus_slot_allocate(bus, !slot, BUS_REPLY_CALLBACK,
                              sizeof(struct reply_callback), userdata);
        if (!s)
            return -ENOMEM;
        ...
        r = ordered_hashmap_put(bus->reply_callbacks, &s->reply_callback.cookie,
                                &s->reply_callback);
        if (r < 0)
        {
        ...
        {
            r = prioq_put(bus->reply_callbacks_prioq, &s->reply_callback,
                          &s->reply_callback.prioq_idx);
            if (r < 0)
            {
...
_public_ int sd_bus_call(sd_bus* bus, sd_bus_message* _m, uint64_t usec,
                         sd_bus_error* error, sd_bus_message** reply)
{
    ...
    bus_assert_return(m->header->type == SD_BUS_MESSAGE_METHOD_CALL, -EINVAL,
                      error);
    bus_assert_return(!(m->header->flags & BUS_MESSAGE_NO_REPLY_EXPECTED),
                      -EINVAL, error);
            ...
            if (incoming->reply_cookie == cookie)
            {
                ...
                if (incoming->header->type == SD_BUS_MESSAGE_METHOD_RETURN)
                {
                    ...
                    return sd_bus_error_setf(
                        ...
                        "Reply message contained file descriptors which I "
                        "couldn't accept. Sorry.");
                }
                ...
                else
                {
                    ...
                    goto fail;
                     ...
                     bus->unique_name && incoming->sender &&
                     streq(bus->unique_name, incoming->sender))
            {

                rqueue_drop_one(bus, i);

                /* Our own message? Somebody is trying to send its own client a
                 * message, let's not dead-lock, let's fail immediately. */
...
static int process_timeout(sd_bus* bus)
{
    ...
    r = bus_message_new_synthetic_error(
        bus, c->cookie,
        &SD_BUS_ERROR_MAKE_CONST(SD_BUS_ERROR_NO_REPLY,
    ...
    /* When this is the hello message and it timed out, then make sure to
     * propagate the error up, don't just log
...
static int process_hello(sd_bus* bus, sd_bus_message* m)
{
    ...
    if (!IN_SET(m->header->type, SD_BUS_MESSAGE_METHOD_RETURN,
...
static int process_reply(sd_bus* bus, sd_bus_message* m)
{
    ...
    if (m->destination && bus->unique_name &&
        !streq_ptr(m->destination, bus->unique_name))
        return 0;
    ...
    /* When this is the hello message and it failed, then make sure to propagate
     * the error up, don't just log and ignore the callback handler's return
     * value. */
    if (is_hello)
        return r;
...
static int process_builtin(sd_bus* bus, sd_bus_message* m)
{
    ...
    else
    {
        r = sd_bus_message_new_method_errorf(
            m, &reply, SD_BUS_ERROR_UNKNOWN_METHOD,
...
static int process_fd_check(sd_bus* bus, sd_bus_message* m)
{
    ...
    return sd_bus_reply_method_errorf(
        m, SD_BUS_ERROR_INCONSISTENT_MESSAGE,
...
static int process_running(sd_bus* bus, bool hint_priority, int64_t priority,
                           sd_bus_message** ret)
{
    _cleanup_(sd_bus_message_unrefp) sd_bus_message* m = NULL;
    int r;
    ...
    if (m->header->type == SD_BUS_MESSAGE_METHOD_CALL)
    {
        ...
        log_debug("Unprocessed message call sender=%s object=%s interface=%s "
                  "member=%s",
                  strna(sd_bus_message_get_sender(m)),
                  strna(sd_bus_message_get_path(m)),
                  strna(sd_bus_message_get_interface(m)),
...
static int bus_exit_now(sd_bus* bus)
{
    ...
    /* Exit due to close, if this is requested. If this is bus object is
     * attached to an event source, invokes sd_event_exit(), otherwise invokes
     * libc exit(). */
...
static int bus_process_internal(sd_bus* bus, bool hint_priority,
                                int64_t priority, sd_bus_message** ret)
{
    int r;
    ...
    assert(!bus->current_slot); /* This should be NULL whenever
                                   bus->current_message is */
    ...
    switch (bus->state)
    {
        ...
        case BUS_HELLO:
            ...
            /* This branch initializes *ret, hence we don't use the generic
...
_public_ int sd_bus_process_priority(sd_bus* bus, int64_t priority,
                                     sd_bus_message** ret)
{
...
static int bus_poll(sd_bus* bus, bool need_more, uint64_t timeout_usec)
{
    ...
    if (timeout_usec != (uint64_t)-1 &&
        (m == USEC_INFINITY || timeout_usec < m))
        m = timeout_usec;
...
_public_ int sd_bus_flush(sd_bus* bus)
{
    ...
    /* We never were connected? Don't hang in inotify for good, as there's no
     * timeout set for it */
    if (bus->state == BUS_WATCH_BIND)
        return -EUNATCH;
...
_public_ int sd_bus_add_filter(sd_bus* bus, sd_bus_slot** slot,
                               sd_bus_message_handler_t callback,
                               void* userdata)
{
    ...
    s = bus_slot_allocate(bus, !slot, BUS_FILTER_CALLBACK,
                          sizeof(struct filter_callback), userdata);
    if (!s)
        return -ENOMEM;
...
static int add_match_callback(sd_bus_message* m, void* userdata,
                              sd_bus_error* ret_error)
{
    ...
    if (sd_bus_message_is_method_error(m, NULL))
    {
        ...
        log_debug("Match %s successfully installed.",
                  match_slot->match_callback.match_string);
    ...
    if (match_slot->match_callback.install_callback)
    {
        ...
        /* This function has been called as slot handler, and we want to call
         * another slot handler. Let's update the slot callback metadata
         * temporarily with our own data, and then revert back to the old
         * values. */
        ...
        r = match_slot->match_callback.install_callback(m, match_slot->userdata,
                                                        ret_error);
        ...
        match_slot->match_callback.install_slot =
            sd_bus_slot_unref(match_slot->match_callback.install_slot);
    }
    else
    {
...
static int bus_add_match_full(sd_bus* bus, sd_bus_slot** slot,
                              bool asynchronous, const char* match,
                              sd_bus_message_handler_t callback,
                              ...
                              void* userdata)
{
    ...
    if (bus->bus_client)
    {
        ...
        /* Do not install server-side matches for matches against the local
         * service, interface or bus path. */
        if (scope != BUS_MATCH_LOCAL)
        {

            /* We store the original match string, so that we can use it to
             * remove the match again. */
            ...
            if (asynchronous)
            {
                r = bus_add_match_internal_async(
                    bus, &s->match_callback.install_slot,
                    s->match_callback.match_string, add_match_callback, s);

                if (r < 0)
                    return r;

                /* Make the slot of the match call floating now. We need the
                 * reference, but we don't want that this match pins the bus
                 * object, hence we first create it non-floating, but
                 * then make it floating. */
                r = sd_bus_slot_set_floating(s->match_callback.install_slot,
                                             true);
            }
            else
...
_public_ int sd_bus_add_match(sd_bus* bus, sd_bus_slot** slot,
                              const char* match,
                              sd_bus_message_handler_t callback, void* userdata)
{

    return bus_add_match_full(bus, slot, false, match, callback, NULL,
                              userdata);
}

_public_ int sd_bus_add_match_async(sd_bus* bus, sd_bus_slot** slot,
                                    const char* match,
                                    sd_bus_message_handler_t callback,
                                    sd_bus_message_handler_t install_callback,
                                    void* userdata)
{
    ...
    return bus_add_match_full(bus, slot, true, match, callback,
...
static int io_callback(sd_event_source* s, int fd, uint32_t revents,
                       void* userdata)
{
    sd_bus* bus = userdata;
    int r;
    ...
    /* Note that this is called both on input_fd, output_fd as well as
     * inotify_fd events */
...
int bus_attach_io_events(sd_bus* bus)
{
    ...
    if (bus->output_fd != bus->input_fd)
    {
        ...
        {
            r = sd_event_add_io(bus->event, &bus->output_io_event_source,
                                bus->output_fd, 0, io_callback, bus);
            if (r < 0)
                return r;
...
int bus_attach_inotify_event(sd_bus* bus)
{
    ...
    {
        r = sd_event_add_io(bus->event, &bus->inotify_event_source,
                            bus->inotify_fd, EPOLLIN, io_callback, bus);
        if (r < 0)
            return r;

        r = sd_event_source_set_priority(bus->inotify_event_source,
                                         bus->event_priority);
        if (r < 0)
            return r;

        r = sd_event_source_set_description(bus->inotify_event_source,
                                            "bus-inotify");
    }
    else
        r = sd_event_source_set_io_fd(bus->inotify_event_source,
                                      bus->inotify_fd);
    if (r < 0)
        return r;
...
static void bus_detach_inotify_event(sd_bus* bus)
{
    ...
    if (bus->inotify_event_source)
    {
        ...
        bus->inotify_event_source =
            sd_event_source_unref(bus->inotify_event_source);
    }
}
...
_public_ int sd_bus_attach_event(sd_bus* bus, sd_event* event, int priority)
{
    ...
    r = sd_event_add_time(bus->event, &bus->time_event_source, CLOCK_MONOTONIC,
                          0, 0, time_callback, bus);
    if (r < 0)
        goto fail;
    ...
    r = sd_event_add_exit(bus->event, &bus->quit_event_source, quit_callback,
                          bus);
    if (r < 0)
        goto fail;
...
static int bus_default(int (*bus_open)(sd_bus**), sd_bus** default_bus,
                       sd_bus** ret)
{
    sd_bus* b = NULL;
    int r;
...
_public_ int sd_bus_path_encode(const char* prefix, const char* external_id,
                                char** ret_path)
{
    _cleanup_free_ char* e = NULL;
    char* ret;
...
_public_ int sd_bus_path_decode(const char* path, const char* prefix,
                                char** external_id)
{
    const char* e;
    char* ret;
...
_public_ int sd_bus_path_decode_many(const char* path,
...
_public_ int sd_bus_set_exit_on_disconnect(sd_bus* bus, int b)
{
    ...
    /* Turns on exit-on-disconnect, and triggers it immediately if the bus
     * connection was already disconnected. Note that this is triggered
     * exclusively on disconnections triggered by the server side, never from
     * the client side. */
...
_public_ int sd_bus_get_method_call_timeout(sd_bus* bus, uint64_t* ret)
{
    ...
    {
        /* Save the parsed value to avoid multiple parsing. To change the
         * timeout value, use sd_bus_set_method_call_timeout() instead of
         * setenv(). */
