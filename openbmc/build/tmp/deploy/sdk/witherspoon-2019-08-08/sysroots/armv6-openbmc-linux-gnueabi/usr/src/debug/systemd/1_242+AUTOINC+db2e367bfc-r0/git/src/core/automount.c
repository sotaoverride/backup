...
static int automount_add_trigger_dependencies(Automount* a)
{
    ...
    return unit_add_two_dependencies(UNIT(a), UNIT_BEFORE, UNIT_TRIGGERS, x,
...
static int automount_add_default_dependencies(Automount* a)
{
    ...
    r = unit_add_two_dependencies_by_name(UNIT(a), UNIT_BEFORE, UNIT_CONFLICTS,
...
static int automount_verify(Automount* a)
{
    ...
    if (r < 0)
        return log_unit_error_errno(
            UNIT(a), r, "Failed to generate unit name from path: %m");

    if (!unit_has_name(UNIT(a), e))
    {
        log_unit_error(UNIT(a),
...
static void automount_dump(Unit* u, FILE* f, const char* prefix)
{
    ...
    fprintf(f,
            ...
            "%sDirectoryMode: %04o\n"
            "%sTimeoutIdleUSec: %s\n",
            prefix, automount_state_to_string(a->state), prefix,
            automount_result_to_string(a->result), prefix, a->where, prefix,
            a->directory_mode, prefix,
...
static void automount_enter_dead(Automount* a, AutomountResult f)
{
    ...
    unit_log_result(UNIT(a), a->result == AUTOMOUNT_SUCCESS,
                    automount_result_to_string(a->result));
    automount_set_state(a, a->result != AUTOMOUNT_SUCCESS ? AUTOMOUNT_FAILED
                                                          : AUTOMOUNT_DEAD);
}
...
static int autofs_send_ready(int dev_autofs_fd, int ioctl_fd, uint32_t token,
                             int status)
{
    struct autofs_dev_ioctl param;
    ...
    if (ioctl(dev_autofs_fd,
              status ? AUTOFS_DEV_IOCTL_FAIL : AUTOFS_DEV_IOCTL_READY,
              &param) < 0)
        return -errno;
...
static int automount_send_ready(Automount* a, Set* tokens, int status)
{
    ...
    ioctl_fd =
        open_ioctl_fd(UNIT(a)->manager->dev_autofs_fd, a->where, a->dev_id);
    if (ioctl_fd < 0)
        return ioctl_fd;
    ...
    while ((token = PTR_TO_UINT(set_steal_first(tokens))))
    {
        ...
        k = autofs_send_ready(UNIT(a)->manager->dev_autofs_fd, ioctl_fd, token,
                              status);
        if (k < 0)
            r = k;
    }
...
static void automount_trigger_notify(Unit* u, Unit* other)
{
    ...
    if (IN_SET(MOUNT(other)->state, MOUNT_MOUNTING, MOUNT_MOUNTING_DONE,
               MOUNT_MOUNTED, MOUNT_REMOUNTING, MOUNT_REMOUNTING_SIGTERM,
               MOUNT_REMOUNTING_SIGKILL, MOUNT_UNMOUNTING_SIGTERM,
               MOUNT_UNMOUNTING_SIGKILL, MOUNT_FAILED))
    {
    ...
    /* The mount is in some unhappy state now, let's unfreeze any waiting
     * clients */
    if (IN_SET(MOUNT(other)->state, MOUNT_DEAD, MOUNT_UNMOUNTING,
...
static void automount_enter_waiting(Automount* a)
{
    ...
    r = sd_event_add_io(UNIT(a)->manager->event, &a->pipe_event_source, p[0],
                        EPOLLIN, automount_dispatch_io, a);
    if (r < 0)
        goto fail;
...
{
    struct autofs_dev_ioctl param;
    _cleanup_(expire_data_freep) struct expire_data* data =
        (struct expire_data*)p;
    int r;
                                     ...
                                     void* userdata)
{
    ...
    data->dev_autofs_fd =
        fcntl(UNIT(a)->manager->dev_autofs_fd, F_DUPFD_CLOEXEC, 3);
    if (data->dev_autofs_fd < 0)
        return log_unit_error_errno(UNIT(a), errno,
                                    "Failed to duplicate autofs fd: %m");

    data->ioctl_fd =
        open_ioctl_fd(UNIT(a)->manager->dev_autofs_fd, a->where, a->dev_id);
    if (data->ioctl_fd < 0)
        return log_unit_error_errno(UNIT(a), data->ioctl_fd,
                                    "Couldn't open autofs ioctl fd: %m");

    r = asynchronous_job(expire_thread, data);
    if (r < 0)
        return log_unit_error_errno(UNIT(a), r,
...
static int automount_start_expire(Automount* a)
{
    ...
    timeout =
        now(CLOCK_MONOTONIC) + MAX(a->timeout_idle_usec / 3, USEC_PER_SEC);
    ...
    r = sd_event_add_time(UNIT(a)->manager->event, &a->expire_event_source,
                          CLOCK_MONOTONIC, timeout, 0,
                          automount_dispatch_expire, a);
    if (r < 0)
        return r;

    (void)sd_event_source_set_description(a->expire_event_source,
                                          "automount-expire");
...
static void automount_enter_running(Automount* a)
{
    ...
    {
        log_unit_warning_errno(UNIT(a), errno,
    ...
    r = manager_add_job(UNIT(a)->manager, JOB_START, trigger, JOB_REPLACE, NULL,
                        &error, NULL);
    if (r < 0)
    {
        log_unit_warning(UNIT(a), "Failed to queue mount startup job: %s",
...
static int automount_start(Unit* u)
{
    ...
    {
        log_unit_error(u, "Path %s is already a mount point, refusing start.",
                       a->where);
        return -EEXIST;
    }
...
static int automount_deserialize_item(Unit* u, const char* key,
                                      const char* value, FDSet* fds)
{
    Automount* a = AUTOMOUNT(u);
    int r;
                                 ...
                                 void* userdata)
{
    ...
    {
        log_unit_error(UNIT(a),
                       "Got invalid poll event %" PRIu32 " on pipe (fd=%d)",
                       events, fd);
        goto fail;
    }
    ...
    switch (packet.hdr.type)
    {
        ...
        case autofs_ptype_missing_direct:
            ...
            if (packet.v5_packet.pid > 0)
            {
                ...
                log_unit_debug(UNIT(a), "Got direct mount request on %s",
                               a->where);
        ...
        case autofs_ptype_expire_direct:
            log_unit_debug(UNIT(a), "Got direct umount request on %s",
                           a->where);
            ...
            r = manager_add_job(UNIT(a)->manager, JOB_STOP, trigger,
        ...
        default:
            log_unit_error(UNIT(a), "Received unknown automount request %i",
                           packet.hdr.type);
            break;
    }
