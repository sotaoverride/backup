...
#define DBUS_NAME_FLAG_REPLACE_EXISTING                                        \
    0x2 /**< Request to replace the current primary owner */
#define DBUS_NAME_FLAG_DO_NOT_QUEUE                                            \
    0x4 /**< If we can not become the primary owner do not place us in the     \
           queue */
...
#define DBUS_REQUEST_NAME_REPLY_IN_QUEUE                                       \
    2 /**< Service could not become the primary owner and has been placed in   \
         the queue */
...
#define DBUS_RELEASE_NAME_REPLY_NON_EXISTENT                                   \
    2 /**< The given name does not exist on the bus */
#define DBUS_RELEASE_NAME_REPLY_NOT_OWNER                                      \
    3 /**< Service is not an owner of the given name */
...
G_DEFINE_TYPE_WITH_CODE(GDBusDaemon, g_dbus_daemon,
...
static NameOwner* name_owner_new(Client* client, guint32 flags)
{
    NameOwner* owner;
...
static Name* name_new(GDBusDaemon* daemon, const char* str)
{
    Name* name;
...
static Name* name_ref(Name* name)
{
    name->refcount++;
    return name;
}

static void name_unref(Name* name)
{
    if (--name->refcount == 0)
    {
...
static Name* name_ensure(GDBusDaemon* daemon, const char* str)
{
    Name* name;
...
static Name* name_lookup(GDBusDaemon* daemon, const char* str)
{
    return g_hash_table_lookup(daemon->names, str);
}

static gboolean is_key(const char* key_start, const char* key_end,
                       const char* value)
{
...
static gboolean parse_key(MatchElement* element, const char* key_start,
                          const char* key_end)
{
    gboolean res = TRUE;
...
static const char* parse_value(MatchElement* element, const char* s)
{
    char quote_char;
    GString* value;
...
static Match* match_new(const char* str)
{
    Match* match;
    GArray* elements;
    const char* p;
...
static gboolean match_equal(Match* a, Match* b)
{
    int i;
...
static const gchar* message_get_argN(GDBusMessage* message, int n,
                                     gboolean allow_path)
{
    const gchar* ret;
    GVariant* body;
    ...
    if (body != NULL && g_variant_is_of_type(body, G_VARIANT_TYPE_TUPLE))
    {
        ...
        if (g_variant_is_of_type(item, G_VARIANT_TYPE_STRING) ||
            (allow_path &&
             g_variant_is_of_type(item, G_VARIANT_TYPE_OBJECT_PATH)))
...
static gboolean match_matches(GDBusDaemon* daemon, Match* match,
                              GDBusMessage* message, gboolean has_destination)
{
    MatchElement* element;
    Name* name;
    int i, len, len2;
    ...
    for (i = 0; i < match->n_elements; i++)
    {
        ...
        switch (check_type)
        {
            ...
            case CHECK_TYPE_PATH_PREFIX:
                ...
                /* Fail if there's no prefix match, or if the prefix match
                 * doesn't
            ...
            case CHECK_TYPE_PATH_RELATED:
                ...
                if (!(strcmp(value, element->value) == 0 ||
                      (len2 > 0 && value[len2 - 1] == '/' &&
                       g_str_has_prefix(element->value, value)) ||
                      (len > 0 && element->value[len - 1] == '/' &&
                       g_str_has_prefix(value, element->value))))
                    return FALSE;
                break;
...
static void broadcast_message(GDBusDaemon* daemon, GDBusMessage* message,
                              gboolean has_destination,
                              gboolean preserve_serial, Client* not_to)
{
    GList *clients, *l, *ll;
    GDBusMessage* copy;
...
static void send_name_owner_changed(GDBusDaemon* daemon, const char* name,
                                    ...
                                    const char* new_owner)
{
    ...
    g_dbus_message_set_body(
        signal_message, g_variant_new("(sss)", name, old_owner ? old_owner : "",
                                      new_owner ? new_owner : ""));
...
static gboolean name_unqueue_owner(Name* name, Client* client)
{
    GList* l;
...
static void name_replace_owner(Name* name, NameOwner* owner)
{
    ...
    if (old_owner)
    {
        ...
        g_dbus_connection_emit_signal(old_client->connection, NULL,
                                      "/org/freedesktop/DBus",
                                      "org.freedesktop.DBus", "NameLost",
                                      g_variant_new("(s)", name->name), NULL);
    ...
    if (owner)
    {
        ...
        g_dbus_connection_emit_signal(new_client->connection, NULL,
                                      "/org/freedesktop/DBus",
...
static void name_release_owner(Name* name)
{
...
static void name_queue_owner(Name* name, NameOwner* owner)
{
    GList* l;
...
static Client* client_new(GDBusDaemon* daemon, GDBusConnection* connection)
{
    Client* client;
    GError* error = NULL;
    ...
    client->id =
        g_strdup_printf(":%d.%d", daemon->next_major_id, daemon->next_minor_id);
    ...
    g_signal_connect(connection, "closed", G_CALLBACK(connection_closed),
                     client);
    g_dbus_connection_add_filter(connection, filter_function, client, NULL);
...
static void client_free(Client* client)
{
    ...
    g_dbus_interface_skeleton_unexport_from_connection(
        G_DBUS_INTERFACE_SKELETON(daemon), client->connection);
...
static gboolean idle_timeout_cb(gpointer user_data)
{
    GDBusDaemon* daemon = user_data;

    daemon->timeout = 0;

    g_signal_emit(daemon, g_dbus_daemon_signals[SIGNAL_IDLE_TIMEOUT], 0);
                              ...
                              Client* client)
{
    ...
    if (g_hash_table_size(daemon->clients) == 0)
        daemon->timeout =
            g_timeout_add(IDLE_TIMEOUT_MSEC, idle_timeout_cb, daemon);
}

static gboolean handle_add_match(_GFreedesktopDBus* object,
                                 GDBusMethodInvocation* invocation,
                                 const gchar* arg_rule)
{
    Client* client = g_object_get_data(
        G_OBJECT(g_dbus_method_invocation_get_connection(invocation)),
        "client");
    Match* match;
...
static gboolean handle_get_connection_selinux_security_context(
    _GFreedesktopDBus* object, GDBusMethodInvocation* invocation,
    const gchar* arg_name)
{
    g_dbus_method_invocation_return_error(
        invocation, G_DBUS_ERROR, G_DBUS_ERROR_SELINUX_SECURITY_CONTEXT_UNKNOWN,
...
static gboolean handle_get_id(_GFreedesktopDBus* object,
                              GDBusMethodInvocation* invocation)
{
...
static gboolean handle_get_name_owner(_GFreedesktopDBus* object,
                                      GDBusMethodInvocation* invocation,
                                      const gchar* arg_name)
{
    GDBusDaemon* daemon = G_DBUS_DAEMON(object);
    ...
    {
        _g_freedesktop_dbus_complete_get_name_owner(object, invocation,
                                                    DBUS_SERVICE_NAME);
        return TRUE;
    }
    ...
    if (arg_name[0] == ':')
    {
        if (g_hash_table_lookup(daemon->clients, arg_name) == NULL)
            g_dbus_method_invocation_return_error(
                invocation, G_DBUS_ERROR, G_DBUS_ERROR_NAME_HAS_NO_OWNER,
...
static gboolean handle_hello(_GFreedesktopDBus* object,
                             GDBusMethodInvocation* invocation)
{
    Client* client = g_object_get_data(
        G_OBJECT(g_dbus_method_invocation_get_connection(invocation)),
    ...
    g_dbus_connection_emit_signal(client->connection, NULL,
                                  "/org/freedesktop/DBus",
                                  "org.freedesktop.DBus", "NameAcquired",
                                  g_variant_new("(s)", client->id), NULL);

    return TRUE;
}

static gboolean handle_list_activatable_names(_GFreedesktopDBus* object,
                                              GDBusMethodInvocation* invocation)
{
    const char* names[] = {NULL};

    _g_freedesktop_dbus_complete_list_activatable_names(object, invocation,
                                                        names);
    return TRUE;
}

static gboolean handle_list_names(_GFreedesktopDBus* object,
                                  GDBusMethodInvocation* invocation)
{
    GDBusDaemon* daemon = G_DBUS_DAEMON(object);
    GPtrArray* array;
    ...
    _g_freedesktop_dbus_complete_list_names(object, invocation,
                                            (const gchar* const*)array->pdata);
    g_ptr_array_free(array, TRUE);
    return TRUE;
}

static gboolean handle_list_queued_owners(_GFreedesktopDBus* object,
                                          GDBusMethodInvocation* invocation,
                                          const gchar* arg_name)
{
    GDBusDaemon* daemon = G_DBUS_DAEMON(object);
    ...
    _g_freedesktop_dbus_complete_list_queued_owners(
        object, invocation, (const gchar* const*)array->pdata);
    g_ptr_array_free(array, TRUE);
    return TRUE;
}
...
static gboolean handle_release_name(_GFreedesktopDBus* object,
                                    GDBusMethodInvocation* invocation,
                                    const gchar* arg_name)
{
    Client* client = g_object_get_data(
    ...
    if (strcmp(arg_name, DBUS_SERVICE_NAME) == 0)
    {
        g_dbus_method_invocation_return_error(
            invocation, G_DBUS_ERROR, G_DBUS_ERROR_INVALID_ARGS,
            "Cannot release a service named " DBUS_SERVICE_NAME
...
static gboolean handle_reload_config(_GFreedesktopDBus* object,
                                     GDBusMethodInvocation* invocation)
{
...
static gboolean handle_remove_match(_GFreedesktopDBus* object,
                                    GDBusMethodInvocation* invocation,
                                    const gchar* arg_rule)
{
    Client* client = g_object_get_data(
...
static gboolean handle_request_name(_GFreedesktopDBus* object,
                                    GDBusMethodInvocation* invocation,
                                    const gchar* arg_name, guint flags)
{
    Client* client = g_object_get_data(
        G_OBJECT(g_dbus_method_invocation_get_connection(invocation)),
        "client");
    ...
    if (strcmp(arg_name, DBUS_SERVICE_NAME) == 0)
    {
        g_dbus_method_invocation_return_error(
            invocation, G_DBUS_ERROR, G_DBUS_ERROR_INVALID_ARGS,
            "Cannot acquire a service named " DBUS_SERVICE_NAME
...
static gboolean handle_start_service_by_name(_GFreedesktopDBus* object,
                                             GDBusMethodInvocation* invocation,
                                             const gchar* arg_name,
                                             guint arg_flags)
{
...
G_GNUC_PRINTF(5, 6)
static void return_error(Client* client, GDBusMessage* message, GQuark domain,
                         gint code, const gchar* format, ...)
{
    ...
    reply = g_dbus_message_new_method_error_literal(message, dbus_error_name,
                                                    error_message);
    ...
    if (!g_dbus_connection_send_message(client->connection, reply,
...
static GDBusMessage* route_message(Client* source_client, GDBusMessage* message)
{
    const char* dest;
    Client* dest_client;
    GDBusDaemon* daemon;
    ...
    if (dest != NULL && strcmp(dest, DBUS_SERVICE_NAME) != 0)
    {
        ...
        if (dest_client == NULL)
        {
            if (g_dbus_message_get_message_type(message) ==
                ...
                return_error(source_client, message, G_DBUS_ERROR,
        ...
        else
        {
            ...
            if (!g_dbus_connection_send_message(
                    dest_client->connection, message,
...
static GDBusMessage* copy_if_locked(GDBusMessage* message)
{
                                     ...
                                     gpointer user_data)
{
    ...
    const char* types[] = {"invalid", "method_call", "method_return", "error",
                           "signal"};
...
static void g_dbus_daemon_finalize(GObject* object)
{
    GDBusDaemon* daemon = G_DBUS_DAEMON(object);
    GList *clients, *l;
...
static void g_dbus_daemon_init(GDBusDaemon* daemon)
{
                              ...
                              GError** error)
{
    ...
    daemon->server = g_dbus_server_new_sync(
        daemon->address, flags, daemon->guid, observer, cancellable, error);
    ...
    g_signal_connect(daemon->server, "new-connection",
                     G_CALLBACK(on_new_connection), daemon);
    g_signal_connect(observer, "authorize-authenticated-peer",
                     G_CALLBACK(on_authorize_authenticated_peer), daemon);
...
static void g_dbus_daemon_set_property(GObject* object, guint prop_id,
...
static void g_dbus_daemon_get_property(GObject* object, guint prop_id,
...
static void g_dbus_daemon_class_init(GDBusDaemonClass* klass)
{
    ...
    g_dbus_daemon_signals[SIGNAL_IDLE_TIMEOUT] = g_signal_new(
        I_("idle-timeout"), G_TYPE_DBUS_DAEMON, G_SIGNAL_RUN_LAST, 0, NULL,
        NULL, g_cclosure_marshal_VOID__VOID, G_TYPE_NONE, 0);

    g_object_class_install_property(
        gobject_class, PROP_ADDRESS,
        g_param_spec_string("address", "Bus Address",
                            "The address the bus should use", NULL,
...
static void g_dbus_daemon_iface_init(_GFreedesktopDBusIface* iface)
{
    ...
    iface->handle_get_connection_selinux_security_context =
        handle_get_connection_selinux_security_context;
    iface->handle_get_connection_unix_process_id =
    ...
    iface->handle_update_activation_environment =
...
GDBusDaemon* _g_dbus_daemon_new(const char* address, GCancellable* cancellable,
                                GError** error)
{
    return g_initable_new(G_TYPE_DBUS_DAEMON, cancellable, error, "address",
                          address, NULL);
}
