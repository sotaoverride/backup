...
static void print_usage(void)
{
    printf(
        ...
        "     --buffer              create a new buffer(plug) and queue "
        "incoming traffic into it.\n"
        "     --release-one         release traffic from previous buffer.\n"
        "     --release-indefinite  stop buffering and release all (buffered "
        "and new) packets.\n"
        "\n"
        "EXAMPLE"
        ...
        "    # at time t=t0, create a new output buffer b0 to hold network "
        "output\n"
        "    nl-qdisc-add --dev=ifb0 --parent=root --update plug --buffer\n\n"
        "    # at time t=t1, take a checkpoint c0, create a new output buffer "
        "b1\n"
        "    nl-qdisc-add --dev=ifb0 --parent=root --update plug --buffer\n"
        "    # at time t=t1+r, after c0 is committed, release b0\n"
        "    nl-qdisc-add --dev=ifb0 --parent=root --update plug "
        "--release-one\n\n"
        "    # at time t=t2, take a checkpoint c1, create a new output buffer "
        "b2\n"
        "    nl-qdisc-add --dev=ifb0 --parent=root --update plug --buffer\n"
