...
std::vector<function const*>
    function_doc_signature_generator::flatten(function const* f)
{
    object name = f->name();
                                                       ...
                                                       bool cpp_types)
{
    ...
    {
        if (n) // we are processing an argument and trying to come up with a
               // name for it
        {
            object kv;
            if (arg_names && (kv = arg_names[n - 1]))
                ...
                param =
                    str(" (%s)%s%d" % make_tuple(py_type_str(s[n]), "arg", n));
        }
                                                       ...
                                                       bool cpp_types)
{
    ...
    {
        return str(
            "%s %s(%s%s%s%s)" % boost::python::make_tuple // workaround, see top
            (ret_type, f->m_name,
             str(",").join(formal_params.slice(0, arity - n_overloads)),
             n_overloads ? (n_overloads != arity ? str(" [,") : str("[ "))
                         : str(),
    ...
    return str(
        "%s %s(%s%s%s%s) %s" % boost::python::make_tuple // workaround, see top
        ...
        if (*sfi == *fi)
        {
            if ((*fi)->doc())
            {
                ...
                bool show_py_signature =
                    doc_len >=
                        int(sizeof(detail::py_signature_tag) / sizeof(char) -
                            1) &&
                ...
                bool show_cpp_signature =
                    doc_len >=
                        int(sizeof(detail::cpp_signature_tag) / sizeof(char) -
                            1) &&
                ...
                {
                    func_doc = str(func_doc.slice(
                        _, 1 - int(sizeof(detail::cpp_signature_tag) /
                                   sizeof(char))));
                    doc_len = len(func_doc);
                }
