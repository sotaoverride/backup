...
#define NL_DBG(LVL, FMT, ARG...)                                               \
    ...
    {                                                                          \
        ...
        {                                                                      \
            int _errsv = errno;                                                \
            fprintf(stderr, "DBG<" #LVL ">%20s:%-4u %s: " FMT, __FILE__,       \
                    __LINE__, __PRETTY_FUNCTION__, ##ARG);                     \
            errno = _errsv;                                                    \
        }                                                                      \
    } while (0)
#else /* NL_DEBUG */
...
static inline int nl_cb_call(struct nl_cb* cb, enum nl_cb_type type,
                             struct nl_msg* msg)
{
    int ret;
...
#define GENL_FAMILY(id, name)                                                  \
    {                                                                          \
        {id, NL_ACT_UNSPEC, name}, END_OF_MSGTYPES_LIST,                       \
    }
