...
long KRB5_CALLCONV krb5int_open_plugin(const char* filepath,
                                       ...
                                       struct errinfo* ep)
{
    ...
    if (!err)
    {
        if (stat(filepath, &statbuf) < 0)
        {
            ...
            k5_set_error(ep, err, _("unable to find plugin [%s]: %s"), filepath,
                         strerror(err));
        }
    }
                 ...
                 ))
    {
        ...
        if ((statbuf.st_mode & S_IFMT) == S_IFDIR)
        {
            ...
            if (!err)
            {
                pluginString = CFStringCreateWithCString(
                    kCFAllocatorDefault, filepath, kCFStringEncodingASCII);
            ...
            if (!err)
            {
                pluginURL = CFURLCreateWithFileSystemPath(
                    kCFAllocatorDefault, pluginString, kCFURLPOSIXPathStyle,
                    true);
                if (pluginURL == NULL)
                {
                    err = ENOMEM;
            ...
            if (!err)
            {
                if (!CFURLGetFileSystemRepresentation(
                        executableURL, true, /* absolute */
                        (UInt8*)executablepath, sizeof(executablepath)))
                {
                    err = ENOMEM;
                }
            }
            ...
            if (!lock_err)
            {
                pthread_mutex_unlock(&krb5int_bundle_mutex);
            }
        }
#endif /* USE_CFBUNDLE */
...
static long krb5int_get_plugin_sym(struct plugin_file_handle* h,
                                   ...
                                   struct errinfo* ep)
{
    ...
    if (!err && !sym && (h->hinstPlugin != NULL))
    {
        ...
        {
            const char* e =
                "unable to get dll symbol"; /* XXX copy and save away */
...
long KRB5_CALLCONV krb5int_get_plugin_data(struct plugin_file_handle* h,
                                           const char* csymname, void** ptr,
                                           struct errinfo* ep)
{
    return krb5int_get_plugin_sym(h, csymname, 0, ptr, ep);
}

long KRB5_CALLCONV krb5int_get_plugin_func(struct plugin_file_handle* h,
                                           const char* csymname, void (**ptr)(),
                                           struct errinfo* ep)
{
    void* dptr = NULL;
...
void KRB5_CALLCONV krb5int_close_plugin(struct plugin_file_handle* h)
{
#if USE_DLOPEN
    if (h->dlhandle != NULL)
    {
...
#endif
#ifdef _WIN32
    if (h->hinstPlugin != NULL)
    {
        FreeLibrary(h->hinstPlugin);
    }
#endif
    free(h);
}
...
static long
    krb5int_plugin_file_handle_array_add(struct plugin_file_handle*** harray,
                                         size_t* count,
                                         struct plugin_file_handle* p)
{
    ...
    newharray =
        realloc(*harray, ((newcount + 1) * sizeof(**harray))); /* +1 for NULL */
    if (newharray == NULL)
    {
        err = ENOMEM;
    }
...
static void krb5int_free_plugin_filenames(char** filenames)
{
    if (filenames != NULL)
    {
        int i;
...
static long krb5int_get_plugin_filenames(const char* const* filebases,
                                         char*** filenames)
{
    long err = 0;
...
/* Takes a NULL-terminated list of directories.  If filebases is NULL, filebases
 * is ignored all plugins in the directories are loaded.  If filebases is a
 * NULL-terminated array of names, only plugins in the directories with those
 * name (plus any platform extension) are loaded. */

long KRB5_CALLCONV krb5int_open_plugin_dirs(const char* const* dirnames,
    ...
    for (i = 0; !err && dirnames[i] != NULL; i++)
    {
        if (filenames != NULL)
        {
            ...
            for (j = 0; !err && filenames[j] != NULL; j++)
            {
                ...
                if (!err)
                {
                    if (asprintf(&filepath, "%s/%s", dirnames[i],
                                 filenames[j]) < 0)
                    {
                        filepath = NULL;
                        err = ENOMEM;
        ...
        else
        {
            ...
            while (dir != NULL && !err)
            {
                ...
                {
                    int len = NAMELEN(d);
                    if (asprintf(&filepath, "%s/%*s", dirnames[i], len,
                                 d->d_name) < 0)
                    {
                        filepath = NULL;
                        err = ENOMEM;
...
void KRB5_CALLCONV krb5int_free_plugin_dir_data(void** ptrs)
{
    /* Nothing special to be done per pointer.  */
    free(ptrs);
}

long KRB5_CALLCONV krb5int_get_plugin_dir_data(
    struct plugin_dir_handle* dirhandle, const char* symname, void*** ptrs,
    struct errinfo* ep)
{
    long err = 0;
    void** p = NULL;
    ...
    if (!err && (dirhandle != NULL) && (dirhandle->files != NULL))
    {
        ...
        for (i = 0; !err && (dirhandle->files[i] != NULL); i++)
        {
            ...
            if (krb5int_get_plugin_data(dirhandle->files[i], symname, &sym,
                                        ep) == 0)
            {
                void** newp = NULL;
...
void KRB5_CALLCONV krb5int_free_plugin_dir_func(void (**ptrs)(void))
{
    /* Nothing special to be done per pointer.  */
    free(ptrs);
}

long KRB5_CALLCONV krb5int_get_plugin_dir_func(
    struct plugin_dir_handle* dirhandle, const char* symname,
    void (***ptrs)(void), struct errinfo* ep)
{
    long err = 0;
    void (**p)() = NULL;
    size_t count = 0;
    ...
    if (!err && (dirhandle != NULL) && (dirhandle->files != NULL))
    {
        ...
        for (i = 0; !err && (dirhandle->files[i] != NULL); i++)
        {
            ...
            if (krb5int_get_plugin_func(dirhandle->files[i], symname, &sym,
                                        ep) == 0)
            {
                void (**newp)() = NULL;
