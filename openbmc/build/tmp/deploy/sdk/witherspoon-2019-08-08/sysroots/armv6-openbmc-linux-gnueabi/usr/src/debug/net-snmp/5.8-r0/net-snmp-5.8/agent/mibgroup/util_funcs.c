...
netsnmp_feature_child_of(util_funcs, libnetsnmpmibs)
    ...
    netsnmp_feature_child_of(
        shell_command, util_funcs) netsnmp_feature_child_of(get_exten_instance,
                                                            util_funcs)
        netsnmp_feature_child_of(clear_cache, util_funcs)
            netsnmp_feature_child_of(
                find_field, util_funcs) netsnmp_feature_child_of(parse_miboid,
                                                                 util_funcs)
                netsnmp_feature_child_of(string_append_int, util_funcs)
                    netsnmp_feature_child_of(internal_mib_table, util_funcs)
                        ...
                        netsnmp_feature_child_of(prefix_info_all, util_funcs)
                            netsnmp_feature_child_of(prefix_info,
                                                     prefix_info_all)
                                netsnmp_feature_child_of(update_prefix_info,
                                                         prefix_info_all)
                                    netsnmp_feature_child_of(delete_prefix_info,
                                                             prefix_info_all)
                                        netsnmp_feature_child_of(
                                            find_prefix_info, prefix_info_all)
                                            netsnmp_feature_child_of(
                                                create_prefix_info,
                                                prefix_info_all)
...
struct extensible* get_exten_instance(struct extensible* exten, size_t inst)
{
    int i;
...
int get_exec_output(struct extensible* ex)
{
    ...
    ex->result = -1;
    NETSNMP_LOGONCE(
        (LOG_WARNING, "support for run_exec_command not available\n"));
#else
#if HAVE_EXECV
    ...
    /* Child temporary output pipe with Inheritance on (sa.bInheritHandle is
     * true) */
    ...
    /* Copy the stdout handle to the stderr handle in case the child closes one
     * of its stdout handles. */
    ...
    /* Create new copies of the input and output handles but set bInheritHandle
     * to FALSE so the new handle can not be inherited.  Otherwise the handles
     * can not be closed.  */
                         ...
                         DUPLICATE_SAME_ACCESS))
    {
        DEBUGMSGTL(("util_funcs",
                    "get_exec_output DupliateHandle ChildOut: %lu\n",
                    GetLastError()));
    ...
    {
        DEBUGMSGTL(("util_funcs",
                    "get_exec_output CloseHandle (hOutputReadTmp): %lu\n",
                    GetLastError()));
        CloseHandle(hErrorWrite);
    ...
    /* Associates a C run-time file descriptor with an existing operating-system
     * file handle. */
    fd = _open_osfhandle((intptr_t)hOutputRead, 0);

    /* Set up STARTUPINFO for CreateProcess with the handles and have it hide
     * the window for the new process. */
    ...
    /* Launch the process that you want to redirect.  Example snmpd.conf
     * pass_persist: pass_persist    .1.3.6.1.4.1.2021.255  c:/perl/bin/perl
     * c:/temp/pass_persisttest
     */
    if (!CreateProcess(NULL, ex->command, NULL, NULL, TRUE, CREATE_NEW_CONSOLE,
                       NULL, NULL, &si, &pi))
    {
    ...
    {
        DEBUGMSGTL(("util_funcs",
                    "get_exec_output CloseHandle hOutputWrite: %lu\n",
                    GetLastError()));
        return -1;
    }
    ...
    {
        DEBUGMSGTL(("util_funcs",
                    "get_exec_output CloseHandle hErrorWrite: %lu\n",
                    GetLastError()));
        return -1;
    }
    return fd;
...
/* 	Alexander Prömel, alexander@proemel.de 08/24/2006
    ...
    I think, it will be better to put the named pipes, into /var/run or make it
   selectable via CONFIG file. If the pipe file already exist, the creation will
   fail. I put the pipes into /flash, the pipepath has to change in
   ucd-snmp/pass_persist.c too, if you change it here.
*/
#if HAVE_EXECV
    ...
    else
    {
        if (tpid > 0)
        {
            ...
            out = open(fifo_out_path,
                       O_WRONLY); /*open the Output Pipe write Only*/
            if (out < 0)
            {
    ...
    /* MSVC (MinGW not working but should use this code).  Cygwin already works
     * as it has execv and fork */
    /* Reference:  MS tech note: 190351 */
    ...
    /* Child temporary input pipe with Inheritance on (sa.bInheritHandle is
     * true) */
    ...
    if (!DuplicateHandle(GetCurrentProcess(), hInputWriteTmp,
                         GetCurrentProcess(), &hInputWrite, 0, FALSE,
                         DUPLICATE_SAME_ACCESS))
    {
    ...
    *fdOut = _open_osfhandle((intptr_t)hInputWrite, 0);
     ...
     * c:/temp/pass_persisttest
     */
    if (!CreateProcess(NULL, cmd, NULL, NULL, TRUE, CREATE_NEW_CONSOLE, NULL,
                       NULL, &si, &pi))
    {
    ...
    DEBUGMSGTL(
        ("util_funcs", "child hProcess (stored in pid): %p\n", pi.hProcess));
    ...
    *pid = pi.hProcess;
    ...
    if (!CloseHandle(pi.hThread))
        DEBUGMSGTL(
            ("util_funcs", "get_exec_pipes CloseHandle pi.hThread: %d\n", cmd));
    ...
    {
        DEBUGMSGTL(("util_funcs",
                    "get_exec_pipes CloseHandle hOutputWrite: %d\n", cmd,
                    GetLastError()));
        return 0;
    }
    ...
    {
        DEBUGMSGTL(("util_funcs",
                    "get_exec_pipes CloseHandle hErrorWrite: %d\n", cmd,
                    GetLastError()));
        return 0;
    }
    return 1;
...
int clear_cache(int action, u_char* var_val, u_char var_val_type,
...
void print_mib_oid(oid name[], size_t len)
{
    char* buffer;
    buffer =
        (char*)malloc(11 * len); /* maximum digit lengths for int32 + a '.' */
    if (!buffer)
    {
 ...
 */
int checkmib(struct variable* vp, oid* name, size_t* length, int exact,
             size_t* var_len, WriteMethod** write_method, int max)
{
    /*
     ...
     * header_generic().
     */
    return (!header_simple_table(vp, name, length, exact, var_len, write_method,
                                 max));
}
...
int parse_miboid(const char* buf, oid* oidout)
{
    int i;
...
void string_append_int(char* s, int val)
{
    char textVal[16];
...
mib_table_t Initialise_Table(int size, int timeout, RELOAD* reload,
                             COMPARE* compare)
{
...
int check_and_reload_table(struct internal_mib_table* table)
{
    ...
    if (table->compare != NULL) /* Sort the table */
        qsort(TABLE_START(table), table->next_index - 1, table->data_size,
              table->compare);
    return 1;
}

int Search_Table(mib_table_t t, void* entry, int exact)
{
...
int net_snmp_find_prefix_info(prefix_cbx** head, char* address,
...
int net_snmp_update_prefix_info(prefix_cbx** head, prefix_cbx* node_to_update)
{
    int iret;
    ...
    {
        DEBUGMSGTL(
            ("util_funcs:prefix", "Unable to find the node to update\n"));
        return -1;
    }
    else
        return 0;
                                       ...
                                       int functionality)
{
    ...
    /* We define functionality based on need * 0 - Need to do a search and
     * update. We have to provide the node_to_use structure filled fully * 1 -
     * Need to do only search. Provide the node_to_use with in6p value filled */
    ...
    if (!functionality)
    {
        ...
        for (temp_node = *head; temp_node->next_info != NULL;
             temp_node = temp_node->next_info)
        {
            if (0 == strcmp(temp_node->in6p, node_to_use->in6p))
            {
                temp_node->ipAddressPrefixOnLinkFlag =
                    node_to_use->ipAddressPrefixOnLinkFlag;
                temp_node->ipAddressPrefixAutonomousFlag =
                    node_to_use->ipAddressPrefixAutonomousFlag;
                return 2;
            }
        }
    ...
    {
        for (temp_node = *head; temp_node != NULL;
             temp_node = temp_node->next_info)
        {
            if (0 == strcmp(temp_node->in6p, node_to_use->in6p))
            {
...
int net_snmp_delete_prefix_info(prefix_cbx** head, char* address)
{
