...
AvahiIniFile* avahi_ini_file_load(const char* fname)
{
    ...
    while (!feof(fo))
    {
        ...
        if (*s == '[')
        {
            ...
            if (!(e = strchr(s, ']')))
            {
                avahi_log_error("Unclosed group header in %s:%u: <%s>", fname,
                                line, s);
                goto fail;
            }
        ...
        else
        {
            ...
            if (!(e = strchr(s, '=')))
            {
                avahi_log_error("Missing assignment in %s:%u: <%s>", fname,
                                line, s);
                goto fail;
            }
            ...
            if (!group)
            {
                avahi_log_error("Assignment outside group in %s:%u <%s>", fname,
                                line, s);
                goto fail;
            }
