...
struct uuidd_cxt_t
{
    ...
    uint32_t timeout;
    unsigned int debug : 1, quiet : 1, no_fork : 1, no_sock : 1;
};
                       ...
                       size_t buflen, int* num, const char** err_context)
{
    ...
    op_buf[0] = op;
    op_len = 1;
    if ((op == UUIDD_OP_BULK_TIME_UUID) || (op == UUIDD_OP_BULK_RANDOM_UUID))
    {
 ...
 */
static int create_socket(struct uuidd_cxt_t* uuidd_cxt, const char* socket_path,
                         int will_fork)
{
    ...
    unlink(socket_path);
    save_umask = umask(0);
    if (bind(s, (const struct sockaddr*)&my_addr, sizeof(struct sockaddr_un)) <
        0)
    {
        if (!uuidd_cxt->quiet)
...
all_done(const struct uuidd_cxt_t* uuidd_cxt, int ret)
{
                        ...
                        struct uuidd_cxt_t* uuidd_cxt)
{
...
#endif
    {
        ...
        if (pidfile_path)
        {
            ...
            if (ftruncate(fd_pidfile, 0))
                err(EXIT_FAILURE, _("could not truncate file: %s"),
                    pidfile_path);
    ...
    if (uuidd_cxt->no_sock)
    {
        ...
        if (r < 0)
        {
            ...
            errx(EXIT_FAILURE, _("no file descriptors received, check "
    ...
    pfd[POLLFD_SIGNAL].events = pfd[POLLFD_SOCKET].events =
        POLLIN | POLLERR | POLLHUP;

    while (1)
    {
        ret = poll(pfd, ARRAY_SIZE(pfd),
                   uuidd_cxt->timeout ? (int)uuidd_cxt->timeout * 1000 : -1);
        if (ret < 0)
        {
            if (errno == EAGAIN)
                continue;
        ...
        if (len != 1)
        {
            if (len < 0)
                ...
                warnx(_("error reading from client, len = %d"), len);
        ...
        switch (op)
        {
            ...
            case UUIDD_OP_BULK_RANDOM_UUID:
                ...
                __uuid_generate_random((unsigned char*)reply_buf + sizeof(num),
                                       &num);
                if (uuidd_cxt->debug)
                {
                    fprintf(stderr,
                            P_("Generated %d UUID:\n", "Generated %d UUIDs:\n",
                               num),
                            num);
                    for (i = 0, cp = reply_buf + sizeof(num); i < num;
                         i++, cp += UUID_LEN)
                    {
...
int main(int argc, char** argv)
{
    ...
    static const struct option longopts[] = {
        ...
        {NULL, 0, NULL, 0}};
    static const ul_excl_t excl[] = {{'P', 'p'}, {'d', 'q'}, {'r', 't'}, {0}};
    ...
    while ((c = getopt_long(argc, argv, "p:s:T:krtn:PFSdqVh", longopts,
                            NULL)) != -1)
    {
        ...
        switch (c)
        {
            ...
            case 'k':
                ...
                break;
            case 'n':
                num = strtou32_or_err(optarg, _("failed to parse --uuids"));
                break;
            case 'p':
                pidfile_path = optarg;
                break;
            ...
            case 'T':
                uuidd_cxt.timeout =
                    strtou32_or_err(optarg, _("failed to parse --timeout"));
                break;
            case 'V':
    ...
    if (num && do_type)
    {
        ret = call_daemon(socket_path, do_type + 2, buf, sizeof(buf), &num,
                          &err_context);
        if (ret < 0)
    ...
    if (do_type)
    {
        ret = call_daemon(socket_path, do_type, (char*)&uu, sizeof(uu), 0,
                          &err_context);
        if (ret < 0)
            err(EXIT_FAILURE, _("error calling uuidd daemon (%s)"),
                err_context ?: _("unexpected error"));
    ...
    if (do_kill)
    {
        ret = call_daemon(socket_path, UUIDD_OP_GETPID, buf, sizeof(buf), 0,
                          NULL);
