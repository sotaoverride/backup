...
static void __attribute__((__noreturn__)) oom(void)
{
...
void _dl_determine_tlsoffset(void)
{
    ...
    GL(dl_tls_static_size) =
        (roundup(offset + TLS_STATIC_SURPLUS, max_align) + TLS_TCB_SIZE);
#elif TLS_DTV_AT_TP
    ...
    for (size_t cnt = 0; slotinfo[cnt].map != NULL; ++cnt)
    {
        ...
        if (slotinfo[cnt].map->l_tls_blocksize <= freetop - freebottom)
        {
            ...
            if (off + slotinfo[cnt].map->l_tls_blocksize - firstbyte <= freetop)
            {
                ...
                freebottom =
                    (off + slotinfo[cnt].map->l_tls_blocksize - firstbyte);
                continue;
            }
        }
    ...
    GL(dl_tls_static_used) = offset;
    GL(dl_tls_static_size) =
        roundup(offset + TLS_STATIC_SURPLUS, TLS_TCB_ALIGN);
#else
...
void* _dl_allocate_tls_storage(void)
{
    ...
    /* Pre-TCB and TCB come before the TLS blocks.  The layout computed
       ...
       blocks.  */
    result = (void*)roundup(
        sizeof(void*) + TLS_PRE_TCB_SIZE + (uintptr_t)allocated, alignment);
...
static dtv_t* _dl_resize_dtv(dtv_t* dtv)
{
    /* Resize the dtv.  */
    dtv_t* newp;
    /* Load GL(dl_tls_max_dtv_idx) atomically since it may be written to by
       other threads concurrently.  */
    size_t newsize = atomic_load_acquire(&GL(dl_tls_max_dtv_idx)) + DTV_SURPLUS;
...
void* _dl_allocate_tls_init(void* result)
{
    if (result == NULL)
...
{
    return _dl_allocate_tls_init(mem == NULL ? _dl_allocate_tls_storage()
                                             : allocate_dtv(mem));
}
...
{
    struct dtv_pointer result =
        allocate_dtv_entry(map->l_tls_align, map->l_tls_blocksize);
    if (result.val == NULL)
        oom();

    /* Initialize the memory.  */
    memset(
        __mempcpy(result.val, map->l_tls_initimage, map->l_tls_initimage_size),
        '\0', map->l_tls_blocksize - map->l_tls_initimage_size);
...
static void* __attribute_noinline__ tls_get_addr_tail(GET_ADDR_ARGS, dtv_t* dtv,
                                                      struct link_map* the_map)
{
    ...
    if (__glibc_unlikely(the_map->l_tls_offset != FORCED_DYNAMIC_TLS_OFFSET))
    {
...
#elif TLS_DTV_AT_TP
            void* p =
                (char*)THREAD_SELF + the_map->l_tls_offset + TLS_PRE_TCB_SIZE;
#else
#error "Either TLS_TCB_AT_TP or TLS_DTV_AT_TP must be defined"
#endif
            __rtld_lock_unlock_recursive(GL(dl_load_lock));
...
static struct link_map* __attribute_noinline__ update_get_addr(GET_ADDR_ARGS)
{
...
rtld_hidden_proto(__tls_get_addr) rtld_hidden_def(__tls_get_addr)
#endif
...
void _dl_add_to_slotinfo(struct link_map* l)
{
    ...
    if (listp == NULL)
    {
       ...
       the first slot.  */
        ...
        listp = prevp->next = (struct dtv_slotinfo_list*)malloc(
