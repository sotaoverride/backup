...
/*! \file
    ...
    the iterator yields the current match or a range between the last and the
   current match depending on the iterator used.
*/
                      ...
                      private detail::find_iterator_base<IteratorT>
{
  ...
  private:
    ...
    typedef BOOST_STRING_TYPENAME base_type::input_iterator_type
        input_iterator_type;
  ...
  public:
    ...
    */
    find_iterator(const find_iterator& Other) :
        base_type(Other), m_Match(Other.m_Match), m_End(Other.m_End)
    {
    }
    ...
    */
    ...
    find_iterator(IteratorT Begin, IteratorT End, FinderT Finder) :
        detail::find_iterator_base<IteratorT>(Finder, 0), m_Match(Begin, Begin),
        m_End(End)
    {
        increment();
    }
  ...
  public:
    ...
    */
    split_iterator(const split_iterator& Other) :
        base_type(Other), m_Match(Other.m_Match), m_Next(Other.m_Next),
        m_End(Other.m_End), m_bEof(Other.m_bEof)
    {
    }
    ...
    */
    ...
    {
        // force the correct behavior for empty sequences and yield at least one
        // token
        if (Begin != End)
        {
            increment();
    ...
    */
    ...
    split_iterator(RangeT& Col, FinderT Finder) :
        detail::find_iterator_base<IteratorT>(Finder, 0), m_bEof(false)
    {
        iterator_range<BOOST_STRING_TYPENAME range_iterator<RangeT>::type>
            lit_col(::boost::as_literal(Col));
        m_Match = make_iterator_range(::boost::begin(lit_col),
                                      ::boost::begin(lit_col));
