...
#error                                                                         \
    "Sorry, can't mix <windows.h> with STL code and gcc compiler: if you ran configure, try again with configure --disable-ms-windows"
...
class BOOST_REGEX_DECL mapfile_iterator
{
  ...
  public:
    ...
    friend inline bool operator==(const mapfile_iterator& i,
                                  const mapfile_iterator& j)
    {
        return (i.file == j.file) && (i.node == j.node) &&
               (i.offset == j.offset);
    }
    ...
    friend mapfile_iterator operator-(const mapfile_iterator& i, long off);
    friend inline long operator-(const mapfile_iterator& i,
                                 const mapfile_iterator& j)
    {
        return i.position() - j.position();
    }
...
class BOOST_REGEX_DECL file_iterator
{
  ...
  public:
    ...
    friend inline bool operator==(const file_iterator& f1,
                                  const file_iterator& f2)
    {
        return ((f1.ref->hf == _fi_invalid_handle) &&
                (f2.ref->hf == _fi_invalid_handle));
    }
...
class BOOST_REGEX_DECL directory_iterator
{
  ...
  public:
    ...
    friend inline bool operator==(const directory_iterator& f1,
