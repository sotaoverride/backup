...
int main(int argc, char* argv[])
{
    ...
    struct option lopts[] = {{"u2u", 1, NULL, OPTION_U2U}, {NULL, 0, NULL, 0}};
...
static krb5_error_code kvno(const char* name, krb5_ccache ccache,
                            krb5_principal me, krb5_enctype etype,
                            krb5_keytab keytab, const char* sname,
                            krb5_flags options, int unknown,
                            krb5_principal for_user_princ, int proxy,
                            krb5_data* u2u_ticket)
{
    ...
    {
        com_err(prog, ret,
                _("while formatting parsed principal name for "
                  "'%s'"),
                name);
        goto cleanup;
    }
    ...
    if (for_user_princ != NULL)
    {
        ...
        in_creds.server = me;
        ret = krb5_get_credentials_for_user(context, options, ccache, &in_creds,
                                            NULL, &out_creds);
    }
    else
    {
...
static krb5_error_code get_u2u_ticket(const char* ccname,
                                      krb5_data** ticket_out)
{
    krb5_error_code ret;
    krb5_ccache cc = NULL;
    ...
    *ticket_out = NULL;
    ...
    ret = krb5_build_principal_ext(
        context, &mcred.server, mcred.client->realm.length,
        mcred.client->realm.data, KRB5_TGS_NAME_SIZE, KRB5_TGS_NAME,
        mcred.client->realm.length, mcred.client->realm.data, 0);
    if (ret)
        goto cleanup;
...
static void do_v5_kvno(int count, char* names[], char* ccachestr,
                       char* etypestr, char* keytab_name, char* sname,
                       int canon, int unknown, char* for_user, int proxy,
