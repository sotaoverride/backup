...
static int ndisc_netlink_message_handler(sd_netlink* rtnl,
...
static int ndisc_router_process_default(Link* link, sd_ndisc_router* rt)
{
    ...
    if (r < 0)
        return log_link_warning_errno(
            link, r, "Failed to get gateway address from RA: %m");
    ...
    if (!memcmp(&gateway, &address->in_addr.in6, sizeof(address->in_addr.in6)))
    {
        ...
        log_link_debug(
            link, "No NDisc route added, gateway %s matches local address",
    ...
    if (r == -ENODATA)
        ...
        return log_link_warning_errno(
            link, r, "Failed to get default router MTU from RA: %m");
                                                  ...
                                                  sd_ndisc_router* rt)
{
    ...
    if (r < 0)
        return log_link_error_errno(
            link, r, "Failed to get prefix preferred lifetime: %m");
    ...
    if (in_addr_is_null(
            AF_INET6, (const union in_addr_union*)&link->network->ipv6_token) ==
        0)
        memcpy(((char*)&address->in_addr.in6) + 8,
               ((char*)&link->network->ipv6_token) + 8, 8);
    else
    {
    ...
    r = address_get(link, address->family, &address->in_addr,
                    address->prefixlen, &existing_address);
    if (r > 0)
    {
        lifetime_remaining = existing_address->cinfo.tstamp / 100 +
                             existing_address->cinfo.ifa_valid -
                             time_now / USEC_PER_SEC;
        if (lifetime_valid > NDISC_PREFIX_LFT_MIN ||
            lifetime_valid > lifetime_remaining)
...
DEFINE_PRIVATE_HASH_OPS(ndisc_rdnss_hash_ops, NDiscRDNSS, ndisc_rdnss_hash_func,
...
static int ndisc_router_process_rdnss(Link* link, sd_ndisc_router* rt)
{
    ...
    if (n < 0)
        return log_link_warning_errno(link, n,
...
{
    siphash24_compress(NDISC_DNSSL_DOMAIN(x), strlen(NDISC_DNSSL_DOMAIN(x)),
                       state);
}
...
DEFINE_PRIVATE_HASH_OPS(ndisc_dnssl_hash_ops, NDiscDNSSL, ndisc_dnssl_hash_func,
...
static int ndisc_router_process_options(Link* link, sd_ndisc_router* rt)
{
    ...
    for (;;)
    {
        ...
        if (r < 0)
            return log_link_warning_errno(
                link, r, "Failed to iterate through options: %m");
        if (r == 0) /* EOF */
            break;
        ...
        switch (type)
        {
            ...
            case SD_NDISC_OPTION_PREFIX_INFORMATION:
            {
                ...
                if (r < 0)
                    return log_link_warning_errno(
                        link, r, "Failed to get RA prefix flags: %m");
...
static int ndisc_router_handler(Link* link, sd_ndisc_router* rt)
{
    ...
    {
        /* (re)start DHCPv6 client in stateful or stateless mode according to RA
         * flags */
...
static void ndisc_handler(sd_ndisc* nd, sd_ndisc_event event,
                          sd_ndisc_router* rt, void* userdata)
{
    Link* link = userdata;
    ...
    switch (event)
    {
        ...
        case SD_NDISC_EVENT_TIMEOUT:
            ...
            break;
        default:
            log_link_warning(link, "IPv6 Neighbor Discovery unknown event: %d",
                             event);
    }
}
