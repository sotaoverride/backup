 ...
 * @param start the place to start scanning (will not find the byte at this
 * point)
...
dbus_bool_t _dbus_string_test(void)
{
    ...
    int i, a, end;
    long v;
    int lens[] = {0,  1,   2,   3,   4,   5,   10,  16,  17,  18,  25, 31,
                  32, 33,  34,  35,  63,  64,  65,  66,  67,  68,  69, 70,
                  71, 127, 128, 129, 130, 131, 132, 133, 134, 135, 136};
    char* s;
    ...
    if (!_dbus_string_move(&str, 0, &other,
                           _dbus_string_get_length(&other) / 2))
        _dbus_assert_not_reached("could not move");
    ...
    if (!_dbus_string_copy(&str, 0, &other,
                           _dbus_string_get_length(&other) / 2))
        _dbus_assert_not_reached("could not copy");
    ...
    _dbus_assert(_dbus_string_get_length(&other) == i * 3);
    _dbus_assert(
        _dbus_string_equal_c_str(&other, "Hello WorldHello WorldHello World"));
    ...
    if (!_dbus_string_replace_len(&str, 1, 1, &other,
                                  _dbus_string_get_length(&other) - 1, 1))
    ...
    if (!_dbus_string_replace_len(&str, 0, 6, &other, 4, 0))
    ...
    if (!_dbus_string_replace_len(&str, 5, 6, &other,
                                  _dbus_string_get_length(&other), 0))
    ...
    if (!_dbus_string_replace_len(&str, 0, 5, &other,
                                  _dbus_string_get_length(&other) - 5, 5))
    ...
    if (!_dbus_string_replace_len(&str, 6, 5, &other,
                                  _dbus_string_get_length(&other) - 5, 5))
        _dbus_assert_not_reached("could not replace same length");
    ...
    if (!_dbus_string_find_byte_backward(&str, _dbus_string_get_length(&str),
                                         'H', &i))
    ...
    if (_dbus_string_find_byte_backward(&str, _dbus_string_get_length(&str) - 1,
                                        'o', &i))
...

    {
        ...
        if (!_dbus_string_find_eol(&str, 0, &found, &found_len) || found != 3 ||
            found_len != 2)
