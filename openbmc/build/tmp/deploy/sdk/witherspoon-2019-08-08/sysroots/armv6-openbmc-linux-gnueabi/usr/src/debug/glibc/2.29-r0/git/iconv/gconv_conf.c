                       ...
                       void* modules)
{
    ...
    struct gconv_alias* new_alias =
        (struct gconv_alias*)malloc(sizeof(struct gconv_alias) + (wp - from));
    if (new_alias != NULL)
    {
        void** inserted;

        new_alias->fromname = memcpy(
            (char*)new_alias + sizeof(struct gconv_alias), from, wp - from);
...
/* Add new alias.  */
static void add_alias(char* rp, void* modules)
{
...
/* Add new module.  */
static void add_module(char* rp, const char* directory, size_t dir_len,
                       void** modules, size_t* nmodules, int modcounter)
{
    /* We expect now
       1. `from' name
       2. `to' name
    ...
    *wp++ = '\0';
    ...
    new_module = (struct gconv_module*)calloc(
        1, sizeof(struct gconv_module) + (wp - from) + dir_len + need_ext);
    if (new_module != NULL)
    {
        char* tmp;
...
static void read_conf_file(const char* filename, const char* directory,
                           size_t dir_len, void** modules, size_t* nmodules)
{
...
static void __gconv_get_path(void)
{
    ...
    else
    {
        ...
        __mempcpy(
            __mempcpy(__mempcpy(gconv_path, __gconv_path_envvar, user_len), ":",
                      1),
    ...
    result = malloc((nelems + 1) * sizeof(struct path_elem) + gconv_path_len +
                    nelems + (nelems - 1) * (cwdlen + 1));
    if (result != NULL)
    {
...
static void __gconv_read_conf(void)
{
    ...
    for (cnt = 0; __gconv_path_elem[cnt].name != NULL; ++cnt)
    {
       ...
       elem already ends in a slash.  */
        ...
        __mempcpy(__mempcpy(filename, elem, elem_len), gconv_conf_filename,
