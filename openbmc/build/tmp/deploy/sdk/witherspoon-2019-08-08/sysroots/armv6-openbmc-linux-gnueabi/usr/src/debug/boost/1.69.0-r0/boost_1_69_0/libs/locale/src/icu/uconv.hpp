...
class icu_std_converter
{
  public:
    ...
    size_t cut(icu::UnicodeString const& str, char_type const* begin,
               char_type const* end, size_t n, size_t from_u = 0,
...
class icu_std_converter<CharType, 1>
{
  public:
    ...
    icu::UnicodeString icu_checked(char_type const* vb,
                                   char_type const* ve) const
    {
      ...
      public:
        uconv(std::string const& charset, cpcvt_type cvt_type = cvt_skip)
        {
            ...
            try
            {
                ...
                {
                    ucnv_setFromUCallBack(cvt_, UCNV_FROM_U_CALLBACK_SKIP, 0, 0,
                                          0, &err);
                    check_and_throw_icu_error(err);

                    err = U_ZERO_ERROR;
                    ucnv_setToUCallBack(cvt_, UCNV_TO_U_CALLBACK_SKIP, 0, 0, 0,
                                        &err);
                    check_and_throw_icu_error(err);
                }
                else
                {
                    ucnv_setFromUCallBack(cvt_, UCNV_FROM_U_CALLBACK_STOP, 0, 0,
                                          0, &err);
                    check_and_throw_icu_error(err);

                    err = U_ZERO_ERROR;
                    ucnv_setToUCallBack(cvt_, UCNV_TO_U_CALLBACK_STOP, 0, 0, 0,
                                        &err);
                    check_and_throw_icu_error(err);
                }
            }
...
class icu_std_converter<CharType, 2>
{
  public:
    ...
    icu::UnicodeString icu_checked(char_type const* begin,
    ...
    {
        char_type const* ptr =
            reinterpret_cast<char_type const*>(str.getBuffer());
...
class icu_std_converter<CharType, 4>
{
  public:
    ...
    string_type std(icu::UnicodeString const& str) const
    {
        ...
        u_strToUTF32(ptr, tmp.size(), &len, str.getBuffer(), str.length(),
                     &code);
