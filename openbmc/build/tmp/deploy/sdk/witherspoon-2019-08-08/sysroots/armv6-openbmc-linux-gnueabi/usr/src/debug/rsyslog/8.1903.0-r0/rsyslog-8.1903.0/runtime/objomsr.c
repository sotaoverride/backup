...
rsRetVal OMSRconstruct(omodStringRequest_t** ppThis, int iNumEntries)
{
    ...
    if (iNumEntries > 0)
    {
        ...
        /* allocate string for template name array. The individual strings will
         * be allocated as the code progresses (we do not yet know the string
         * sizes)
         */
...
/* set a template name and option to the object. Index must be given. The
 * pTplName must be pointing to memory that can be freed. If in doubt, the
 * caller must strdup() the value.
 */
rsRetVal OMSRsetEntry(omodStringRequest_t* pThis, int iEntry, uchar* pTplName,
                      int iTplOpts)
{
    assert(pThis != NULL);
 ...
 */
int OMSRgetEntry(omodStringRequest_t* pThis, int iEntry, uchar** ppTplName,
                 int* piTplOpts)
{
    assert(pThis != NULL);
 ...
 * Note that this interface was added in 4.1.6, so a plugin must obtain a
 * pointer to this interface via queryHostEtryPt(). rgerhards, 2009-04-03
 */
