...
#define pcv(fmt, ...)                                                          \
    ...
    {                                                                          \
        ...
        if (!args.power_cut_mode || args.verbose || __err)                     \
            normsg(fmt " (line %d, error %d (%s))", ##__VA_ARGS__, __LINE__,   \
 ...
 * mount_opts: non-standard mount options of the tested file-system
 * (non-standard options are stored in string form as a comma-separated list)
                         ...
                         int line)
{
    ...
    fflush(stdout);
    fflush(stderr);
    errmsg("condition '%s' failed in %s() at %s:%d", cond, func, file, line);
                    ...
                    struct file_info* file)
{
    ...
    if (ret != 0)
    {
        if (errno == ENOSPC)
        {
            ...
            pcv("cannot create hardlink %s in directory %s to file %s", path,
                parent->entry->name, target);
        free(target);
        free(path);
        return ret;
    }
...
static void file_info_display(struct file_info* file)
{
    ...
    normsg("    File was open: %s", (file->fds == NULL) ? "false" : "true");
                               ...
                               size_t size, unsigned int seed)
{
    ...
    v("write %zd bytes, offset %lld, file %s", size, (long long)offset,
      get_file_name(file));
    while (remains)
    {
        ...
        if (written < 0)
        {
            ...
            pcv("failed to write %zu bytes to offset %llu of file %s", block,
                (unsigned long long)(offset + actual), get_file_name(file));
            return -1;
        }
        remains -= written;
        actual += written;
...
static void get_offset_and_size(struct file_info* file, off_t* offset,
                                size_t* size)
{
    unsigned int r, n;
                               ...
                               size_t new_length)
{
    ...
    if (args.verify_ops && !args.power_cut_mode && new_length > file->length)
        file_check_hole(file, fd, file->length, new_length - file->length);
...
static int file_ftruncate(struct file_info* file, int fd, off_t new_length)
{
    if (ftruncate(fd, new_length) != 0)
    {
        if (errno == ENOSPC)
        {
            ...
            pcv("cannot truncate file %s to %llu", get_file_name(file),
...
static int file_write(struct file_info* file, int fd)
{
    ...
    if (!args.power_cut_mode && fsinfo.can_mmap && !full && file->link_count &&
        random_no(100) == 1)
    {
        /*
    ...
    if (random_no(100) >= 99)
    {
        ...
        else
        {
            ...
            ret = fdatasync(fd);
            if (ret)
                pcv("fdatasync failed for %s", get_file_name(file));
        }
        if (ret)
            return -1;
        file->clean = 1;
        ...
        for (i = 0; i < block; ++i)
        {
            if (buf[i] != 0)
            {
                errmsg("file_check_hole failed at %zu checking "
                       "hole at %llu size %zu",
                       size - remains + i, (unsigned long long)offset, size);
                            ...
                            struct write_info* w)
{
    ...
    while (remains)
    {
        ...
        if (memcmp(check_buf, read_buf, block) != 0)
        {
            ...
            for (r = 0; r < block; ++r)
                fprintf(stderr, "%02x%c", read_buf[r],
                        ((r + 1) % 16) ? ' ' : '\n');
            fprintf(stderr, "\nExpected data:\n");
            for (r = 0; r < block; ++r)
                fprintf(stderr, "%02x%c", check_buf[r],
                        ((r + 1) % 16) ? ' ' : '\n');
            fprintf(stderr, " \n");
...
static void dir_entry_check(struct dir_entry_info** entry_array,
                            size_t number_of_entries, struct dirent* ent)
{
                              ...
                              struct dir_entry_info** rename_entry, int isdir)
{
    ...
    *rename_entry = NULL;
    ...
    if (!entry)
        entry = dir->first;
    if (!entry || (entry->type == 'd' && entry->dir->number_of_entries != 0))
        return dup_string(make_name(dir));

    if ((isdir && entry->type != 'd') || (!isdir && entry->type == 'd'))
        return dup_string(make_name(dir));
...
static int rename_entry(struct dir_entry_info* entry)
{
    ...
    if (rename_entry)
        v("moving %s (type %c) inoto %s (type %c)", path, entry->type, to,
          rename_entry->type);
    else
...
static int symlink_new(struct dir_info* dir, const char* nm)
{
    ...
    if (symlink(target, path) != 0)
    {
        ...
        {
            pcv("cannot create symlink %s in directory %s to file %s", path,
                dir->entry->name, target);
            ret = -1;
        }
        free(target);
        free(name);
...
static int operate_on_entry(struct dir_entry_info* entry)
{
    ...
    else if (entry->type == 'f')
    {
        ...
        else if (!grow && entry->file->link_count > 1 && random_no(10) == 0)
...
static int remount_tested_fs(void)
{
    ...
    if (rorw1)
    {
        ...
        ret = mount(fsinfo.fsdev, fsinfo.mount_point, fsinfo.fstype, flags,
                    fsinfo.mount_opts);
        if (ret)
        {
            pcv("cannot remount %s R/O (1)", fsinfo.mount_point);
            return -1;
        }
    ...
    if (um)
    {
        if (um_ro)
        {
            ...
            if (ret)
            {
                pcv("cannot remount %s R/O (2)", fsinfo.mount_point);
                return -1;
            }
        }
        ...
        if (!um_rorw)
        {
            ret = mount(fsinfo.fsdev, fsinfo.mount_point, fsinfo.fstype,
                        fsinfo.mount_flags, fsinfo.mount_opts);
            if (ret)
            {
        ...
        {
            ret = mount(fsinfo.fsdev, fsinfo.mount_point, fsinfo.fstype,
                        fsinfo.mount_flags | MS_RDONLY, fsinfo.mount_opts);
            if (ret)
            {
...
static void get_tested_fs_info(void)
{
    ...
    *(p + 1) = '\0';
    ...
    normsg("pid %u, testing \"%s\" at \"%s\"", pid, fsinfo.fstype,
           fsinfo.mount_point);
}

static const char doc[] = PROGRAM_NAME
    " version " VERSION
    " - a stress test which checks the file-system integrity.\n"
    "\n"
    "The test creates a directory named \"integck_test_dir_<pid>\", where "
    "where\n"
    "<pid> is the process id. Then it randomly creates and deletes files,\n"
    "directories, symlinks, and hardlinks, randomly writes and truncate "
    "files,\n"
    "sometimes makes holes in files, sometimes fsync()'s them. Then it "
    "un-mounts and\n"
    "re-mounts the tested file-system and checks the contents - everything "
    "(files,\n"
    "directories, etc) should be there and the contents of the files should "
    "be\n"
    "correct. This is repeated a number of times (set with -n, default 1).\n"
    "\n"
    "By default the test does not verify file-system modifications and assumes "
    "they\n"
    "are done correctly if the file-system returns success. However, the -e "
    "flag\n"
    "enables additional verifications and the test verifies all the "
    "file-system\n"
    "operations it performs.\n"
    "\n"
    "This test is also able to perform power cut testing. The underlying "
    "file-system\n"
    "or the device driver should be able to emulate power-cuts, by switching "
    "to R/O\n"
    "mode at random moments. And the file-system should return EROFS "
    "(read-only\n"
    "file-system error) for all operations which modify it. In this case this "
    "test\n"
    "program re-mounts the file-system and checks that all files and "
    "directories\n"
    "which have been successfully synchronized before the power cut. And the "
    "test\n"
    "continues forever.\n";

static const char optionsstr[] =
    "-n, --repeat=<count> repeat count, default is 1; zero value - repeat "
    "forever\n"
    "-p, --power-cut      power cut testing mode (-n parameter is ignored and "
    "the\n"
    "                     test continues forever)\n"
    "-e, --verify-ops     verify all operations, e.g., every time a file is "
    "written\n"
    "                     to, read the data back and verify it, every time a\n"
    "                     directory is created, check that it exists, etc\n"
    "-v, --verbose        be verbose\n"
    "-m, --reattach=<mtd> re-attach mtd device number <mtd> (only if doing "
    "UBIFS power\n"
...
static int parse_opts(int argc, char* const argv[])
{
    ...
    if (optind == argc)
        ...
        return errmsg(
            "more then one test file-system specified (use -h for help)");
...
int main(int argc, char* argv[])
{
    ...
    {
        warnmsg("file-system %s cannot be unmounted (%s)", fsinfo.mount_point,
                strerror(errno));
        if (args.power_cut_mode)
        {
            /*
