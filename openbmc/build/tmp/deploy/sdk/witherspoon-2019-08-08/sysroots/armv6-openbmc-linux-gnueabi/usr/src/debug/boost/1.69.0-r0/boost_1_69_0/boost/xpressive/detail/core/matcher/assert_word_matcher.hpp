...
struct word_boundary
{
    ...
    static bool eval(bool prevword, bool thisword, match_state<BidiIter>& state)
    {
        if ((state.flags_.match_not_bow_ && state.bos()) ||
            (state.flags_.match_not_eow_ && state.eos()))
        {
...
struct assert_word_matcher : quant_style_assertion
{
    ...
    assert_word_matcher(Traits const& tr) : word_(lookup_classname(tr, "w"))
    {
        BOOST_ASSERT(0 != this->word_);
    }

    assert_word_matcher(char_class_type word) : word_(word)
    {
    }
    ...
    bool match(match_state<BidiIter>& state, Next const& next) const
    {
        ...
        bool const thisword =
            !state.eos() && this->is_word(traits_cast<Traits>(state), *cur);
        bool const prevword =
            (!state.bos() || state.flags_.match_prev_avail_) &&
