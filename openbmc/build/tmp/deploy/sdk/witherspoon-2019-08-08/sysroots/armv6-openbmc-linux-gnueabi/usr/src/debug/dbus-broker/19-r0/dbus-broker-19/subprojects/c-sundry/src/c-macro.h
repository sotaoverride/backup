...
#define C_DECL(_expr, _assertion)                                              \
    ...
    (__builtin_choose_expr(!!(1 + 0 * sizeof(struct { _assertion; })),         \
                           (_expr), ((void)0)))
...
#define C_VAR2(_x, _uniq)                                                      \
    C_CONCATENATE(c_internal_var_unique_, C_CONCATENATE(_uniq, _x))
 ...
 */
#define C_CC_MACRO1(_call, _x, ...)                                            \
    C_INTERNAL_CC_MACRO1(_call, __COUNTER__, (_x), ##__VA_ARGS__)
#define C_INTERNAL_CC_MACRO1(_call, _xq, _x, ...)                              \
    __builtin_choose_expr(C_CC_IS_CONST(_x), _call(_x, ##__VA_ARGS__),         \
                          __extension__({                                      \
 ...
 * Return: Result of @_call is returned.
 */
#define C_CC_MACRO2(_call, _x, _y, ...)                                        \
    C_INTERNAL_CC_MACRO2(_call, __COUNTER__, (_x), __COUNTER__, (_y),          \
                         ##__VA_ARGS__)
#define C_INTERNAL_CC_MACRO2(_call, _xq, _x, _yq, _y, ...)                     \
    __builtin_choose_expr((C_CC_IS_CONST(_x) && C_CC_IS_CONST(_y)),            \
                          _call((_x), (_y), ##__VA_ARGS__), __extension__({    \
                              const __auto_type C_VAR(X, _xq) = (_x);          \
 ...
 * Return: Result of @_call is returned.
 */
#define C_CC_MACRO3(_call, _x, _y, _z, ...)                                    \
    C_INTERNAL_CC_MACRO3(_call, __COUNTER__, (_x), __COUNTER__, (_y),          \
                         __COUNTER__, (_z), ##__VA_ARGS__)
#define C_INTERNAL_CC_MACRO3(_call, _xq, _x, _yq, _y, _zq, _z, ...)            \
    __builtin_choose_expr(                                                     \
        (C_CC_IS_CONST(_x) && C_CC_IS_CONST(_y) && C_CC_IS_CONST(_z)),         \
        _call((_x), (_y), (_z), ##__VA_ARGS__), __extension__({                \
            const __auto_type C_VAR(X, _xq) = (_x);                            \
            const __auto_type C_VAR(Y, _yq) = (_y);                            \
...
#define C_ARRAY_SIZE(_x)                                                       \
    C_DECL(sizeof(_x) /                                                        \
               sizeof((_x)[0]), /*                                             \
                                 * Verify that `_x' is an array, not a         \
                                 * pointer. Rely on                            \
                                 * `&_x[0]' degrading arrays to pointers.      \
                                 */                                            \
           _Static_assert(                                                     \
               !__builtin_types_compatible_p(                                  \
                   __typeof__(_x), __typeof__(&(*(__typeof__(_x)*)0)[0])),     \
...
#define C_DECIMAL_MAX(_arg)                                                    \
    (_Generic(                                                                 \
        (__typeof__(_arg)){0}, char                                            \
        : C_INTERNAL_DECIMAL_MAX(sizeof(char)), signed char                    \
        : C_INTERNAL_DECIMAL_MAX(sizeof(signed char)), unsigned char           \
        : C_INTERNAL_DECIMAL_MAX(sizeof(unsigned char)), signed short          \
        : C_INTERNAL_DECIMAL_MAX(sizeof(signed short)), unsigned short         \
        : C_INTERNAL_DECIMAL_MAX(sizeof(unsigned short)), signed int           \
        : C_INTERNAL_DECIMAL_MAX(sizeof(signed int)), unsigned int             \
        : C_INTERNAL_DECIMAL_MAX(sizeof(unsigned int)), signed long            \
        : C_INTERNAL_DECIMAL_MAX(sizeof(signed long)), unsigned long           \
        : C_INTERNAL_DECIMAL_MAX(sizeof(unsigned long)), signed long long      \
        : C_INTERNAL_DECIMAL_MAX(sizeof(signed long long)), unsigned long long \
        : C_INTERNAL_DECIMAL_MAX(sizeof(unsigned long long))))
#define C_INTERNAL_DECIMAL_MAX(_bytes)                                         \
    C_DECL(                                                                    \
        1 + ((_bytes) <= 1 ? 3 : (_bytes) <= 2 ? 5 : (_bytes) <= 4 ? 10 : 20), \
        _Static_assert((_bytes) <= 8,                                          \
                       "Invalid use of C_INTERNAL_DECIMAL_MAX()"))

/**
 * c_container_of() - cast a member of a structure out to the containing
 * structure
...
#define c_container_of(_ptr, _type, _member)                                   \
    C_CC_MACRO1(C_CONTAINER_OF, (_ptr), _type, _member)
#define C_CONTAINER_OF(_ptr, _type, _member)                                   \
    __extension__({                                                            \
...
#define C_CLZ(_val)                                                            \
    (_Generic((_val), unsigned int                                             \
              : __builtin_clz, unsigned long                                   \
              : __builtin_clzl, unsigned long long                             \
              : __builtin_clzll) /* clz(0) is undefined behavior, so work      \
                                    around it */                               \
     ((_val) | !(_val)) +                                                      \
     !_c_likely_(_val))
