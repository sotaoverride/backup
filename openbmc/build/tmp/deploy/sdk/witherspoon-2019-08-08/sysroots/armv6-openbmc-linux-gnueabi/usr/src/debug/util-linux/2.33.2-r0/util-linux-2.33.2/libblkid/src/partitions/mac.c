...
static inline unsigned char* get_mac_block(blkid_probe pr, uint16_t block_size,
                                           uint32_t num)
{
                        ...
                        __attribute__((__unused__)))
{
    ...
    for (i = 1; i <= nblks; ++i)
    {
        ...
        {
            DBG(LOWPROBE,
                ul_debug("mac: inconsistent map_count in partition map, "
                         "entry[0]: %d, entry[%d]: %d",
        ...
        blkid_partition_set_name(par, (unsigned char*)p->name, sizeof(p->name));
...
const struct blkid_idinfo mac_pt_idinfo = {
    ...
    .magics = {/* big-endian magic string */
               {.magic = "\x45\x52", .len = 2},
               {NULL}}};
