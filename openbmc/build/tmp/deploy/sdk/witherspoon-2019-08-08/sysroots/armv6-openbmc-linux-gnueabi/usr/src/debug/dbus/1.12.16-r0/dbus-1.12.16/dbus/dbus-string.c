...
static void fixup_alignment(DBusRealString* real)
{
    ...
    /* we have to have extra space in real->allocated for the align offset and
     * nul byte */
    _dbus_assert(real->len <=
                 real->allocated - _DBUS_STRING_ALLOCATION_PADDING);
    ...
    {
        /* Here comes the suck */
        memmove(real_block + real->align_offset, real_block + old_align_offset,
                real->len + 1);
    }
...
void _dbus_string_init_const_len(DBusString* str, const char* value, int len)
{
    ...
    real->allocated =
        real->len +
        _DBUS_STRING_ALLOCATION_PADDING; /* a lie, just to avoid special-case
                                            assertions... */
    real->constant = TRUE;
    real->locked = TRUE;
    real->invalid = FALSE;
...
static dbus_bool_t reallocate_for_length(DBusRealString* real, int new_length)
{
     ...
     */
    if (real->allocated >
        (_DBUS_STRING_MAX_LENGTH + _DBUS_STRING_ALLOCATION_PADDING) / 2)
        new_allocated =
            _DBUS_STRING_MAX_LENGTH + _DBUS_STRING_ALLOCATION_PADDING;
    else
    ...
    new_allocated =
        MAX(new_allocated, new_length + _DBUS_STRING_ALLOCATION_PADDING);
...
static dbus_bool_t open_gap(int len, DBusRealString* dest, int insert_at)
{
    ...
    memmove(dest->str + insert_at + len, dest->str + insert_at,
 ...
 * @returns the string data
 */
const char* _dbus_string_get_const_data_len(const DBusString* str, int start,
                                            int len)
{
    DBUS_CONST_STRING_PREAMBLE(str);
    _dbus_assert(start >= 0);
 ...
 */
dbus_bool_t _dbus_string_insert_bytes(DBusString* str, int i, int n_bytes,
                                      unsigned char byte)
{
    DBUS_STRING_PREAMBLE(str);
    _dbus_assert(i <= real->len);
 ...
 */
void _dbus_string_copy_to_buffer(const DBusString* str, char* buffer,
                                 int avail_len)
{
    DBUS_CONST_STRING_PREAMBLE(str);
 ...
 * @param avail_len maximum length of C buffer
 */
void _dbus_string_copy_to_buffer_with_nul(const DBusString* str, char* buffer,
                                          int avail_len)
{
    DBUS_CONST_STRING_PREAMBLE(str);
...
static dbus_bool_t align_insert_point_then_open_gap(DBusString* str,
                                                    int* insert_at_p,
                                                    int alignment, int gap_size)
{
    ...
    if (delta ==
        0) /* only happens if gap_size == 0 and insert_at is aligned already */
    {
        _dbus_assert(((unsigned long)*insert_at_p) == gap_pos);
        return TRUE;
    }

    if (_DBUS_UNLIKELY(!open_gap(new_len - real->len, real, insert_at)))
        return FALSE;
...
static dbus_bool_t align_length_then_lengthen(DBusString* str, int alignment,
                                              int then_lengthen_by)
{
    ...
    return align_insert_point_then_open_gap(str, &insert_at, alignment,
                                            then_lengthen_by);
}
...
static dbus_bool_t append(DBusRealString* real, const char* buffer,
                          int buffer_len)
{
    if (buffer_len == 0)
        return TRUE;
 ...
 * @returns #FALSE if not enough memory.
 */
dbus_bool_t _dbus_string_insert_2_aligned(DBusString* str, int insert_at,
 ...
 * @returns #FALSE if not enough memory.
 */
dbus_bool_t _dbus_string_insert_4_aligned(DBusString* str, int insert_at,
 ...
 * @returns #FALSE if not enough memory.
 */
dbus_bool_t _dbus_string_insert_8_aligned(DBusString* str, int insert_at,
 ...
 * @returns #FALSE if not enough memory.
 */
dbus_bool_t _dbus_string_insert_alignment(DBusString* str, int* insert_at,
                                          int alignment)
{
    DBUS_STRING_PREAMBLE(str);
    ...
    _dbus_assert(_DBUS_ALIGN_VALUE(*insert_at, alignment) ==
                 (unsigned)*insert_at);
 ...
 * @returns #FALSE if not enough memory.
 */
dbus_bool_t _dbus_string_append_len(DBusString* str, const char* buffer,
                                    int len)
{
    DBUS_STRING_PREAMBLE(str);
    _dbus_assert(buffer != NULL);
...
static void delete (DBusRealString* real, int start, int len)
{
    ...
    memmove(real->str + start, real->str + start + len,
            real->len - (start + len));
    real->len -= len;
    real->str[real->len] = '\0';
}
...
static dbus_bool_t copy(DBusRealString* source, int start, int len,
                        DBusRealString* dest, int insert_at)
{
    if (len == 0)
        return TRUE;
 ...
 * @returns #FALSE if not enough memory
 */
dbus_bool_t _dbus_string_move(DBusString* source, int start, DBusString* dest,
                              int insert_at)
{
    DBusRealString* real_source = (DBusRealString*)source;
    _dbus_assert(start <= real_source->len);

    return _dbus_string_move_len(source, start, real_source->len - start, dest,
                                 insert_at);
}
 ...
 * @returns #FALSE if not enough memory
 */
dbus_bool_t _dbus_string_copy(const DBusString* source, int start,
                              DBusString* dest, int insert_at)
{
    DBUS_STRING_COPY_PREAMBLE(source, start, dest, insert_at);

    return copy(real_source, start, real_source->len - start, real_dest,
                insert_at);
}
 ...
 * @returns #FALSE if not enough memory
 */
dbus_bool_t _dbus_string_move_len(DBusString* source, int start, int len,
                                  DBusString* dest, int insert_at)
...
#define ASSIGN_DATA(a, b)                                                      \
    ...
    {
        if (!copy(real_source, start, len, real_dest, insert_at))
            return FALSE;
 ...
 * @returns #FALSE if not enough memory
 */
dbus_bool_t _dbus_string_copy_len(const DBusString* source, int start, int len,
                                  DBusString* dest, int insert_at)
{
    DBUS_STRING_COPY_PREAMBLE(source, start, dest, insert_at);
    _dbus_assert(len >= 0);
 ...
 *
 */
dbus_bool_t _dbus_string_replace_len(const DBusString* source, int start,
                                     int len, DBusString* dest, int replace_at,
                                     int replace_len)
{
    ...
    else
    {
        ...
        if (!copy(real_source, start + replace_len, diff, real_dest,
                  replace_at + replace_len))
            return FALSE;

        memmove(real_dest->str + replace_at, real_source->str + start,
                replace_len);
    }
 ...
 *
 */
dbus_bool_t _dbus_string_split_on_byte(DBusString* source, unsigned char byte,
                                       DBusString* tail)
{
    int byte_position;
    char byte_string[2] = "";
    ...
    if (!_dbus_string_move_len(source, byte_position + 1, tail_length, tail, 0))
        return FALSE;
 ...
 */
dbus_bool_t _dbus_string_find(const DBusString* str, int start,
                              const char* substr, int* found)
{
    return _dbus_string_find_to(str, start, ((const DBusRealString*)str)->len,
                                substr, found);
}
 ...
 * @param str the string
 * @param start where to start looking
 * @param found return location for where eol was found or string length
 * otherwise
 * @param found_len return length of found eol string or zero otherwise
 * @returns #TRUE if found
 */
dbus_bool_t _dbus_string_find_eol(const DBusString* str, int start, int* found,
                                  int* found_len)
{
    int i;
 ...
 * @returns #TRUE if found
 */
dbus_bool_t _dbus_string_find_to(const DBusString* str, int start, int end,
                                 const char* substr, int* found)
{
    int i;
    DBUS_CONST_STRING_PREAMBLE(str);
 ...
 */
dbus_bool_t _dbus_string_find_blank(const DBusString* str, int start,
                                    int* found)
{
    int i;
    DBUS_CONST_STRING_PREAMBLE(str);
...
void _dbus_string_skip_white_reverse(const DBusString* str, int end, int* start)
{
    ...
    _dbus_assert(i >= 0 &&
                 (i == 0 || !(DBUS_IS_ASCII_WHITE(real->str[i - 1]))));
...
dbus_bool_t _dbus_string_pop_line(DBusString* source, DBusString* dest)
{
    ...
    /* remember eol can be 0 if it's an empty line, but eol_len should not be
     * zero also since find_eol returned TRUE
     */
 ...
 */
dbus_bool_t _dbus_string_equal_len(const DBusString* a, const DBusString* b,
                                   int len)
{
    const unsigned char* ap;
    const unsigned char* bp;
    ...
    if (real_a->len != real_b->len && (real_a->len < len || real_b->len < len))
        return FALSE;
 ...
 */
dbus_bool_t _dbus_string_equal_substring(const DBusString* a, int a_start,
                                         int a_len, const DBusString* b,
                                         int b_start)
{
    const unsigned char* ap;
    const unsigned char* bp;
...
dbus_bool_t _dbus_string_append_byte_as_hex(DBusString* str, unsigned char byte)
{
    ...
    if (!_dbus_string_append_byte(str, hexdigits[(byte >> 4)]))
        return FALSE;
 ...
 */
dbus_bool_t _dbus_string_hex_encode(const DBusString* source, int start,
                                    DBusString* dest, int insert_at)
{
    DBusString result;
 ...
 */
dbus_bool_t _dbus_string_hex_decode(const DBusString* source, int start,
                                    int* end_return, DBusString* dest,
                                    int insert_at)
{
    DBusString result;
    const unsigned char* p;
    ...
    while (p != end)
    {
        ...
        if (high_bits)
        {
            if (!_dbus_string_append_byte(&result, val << 4))
                goto out;
        }
        else
        {
 ...
 */
dbus_bool_t _dbus_string_validate_ascii(const DBusString* str, int start,
                                        int len)
{
 ...
 */
dbus_bool_t _dbus_string_validate_utf8(const DBusString* str, int start,
                                       int len)
{
    const unsigned char* p;
    const unsigned char* end;
    ...
    while (p < end)
    {
        ...
        /* check that the expected number of bytes exists in the remaining
         * length */
        if (_DBUS_UNLIKELY((end - p) <
                           char_len)) /* ASCII: p < end and char_len == 1 */
            break;

        UTF8_GET(result, p, i, mask, char_len);

        /* Check for overlong UTF-8 */
        if (_DBUS_UNLIKELY(UTF8_LENGTH(result) !=
                           char_len)) /* ASCII: UTF8_LENGTH == 1 */
            break;
#if 0
