...
static int match_hostname(const char** host_ptr, const char* addr,
                          const char* tok)
{
    struct hostent* hp;
    unsigned int i;
    ...
    /* Fail quietly if tok is an address or wildcarded entry, not a simple
     * hostname. */
    if (!tok[strspn(tok, ".0123456789")] || tok[strcspn(tok, ":/*?[")])
        return 0;

    /* Now try forward-DNS on the token (config-specified hostname) and see if
     * the IP matches. */
    ...
    {
        if (strcmp(addr, inet_ntoa(*(struct in_addr*)(hp->h_addr_list[i]))) ==
            0)
        {
...
static int match_binary(const char* b1, const char* b2, const char* mask,
                        int addrlen)
{
    int i;
...
static int access_match(const char* list, const char* addr,
                        const char** host_ptr)
{
    char* tok;
