...
// error checking API:
//
BOOST_REGEX_DECL void BOOST_REGEX_CALL verify_options(
    boost::regex_constants::syntax_option_type ef, match_flag_type mf);
//
// function can_start:
//
...
inline bool can_start(charT c, const unsigned char* map, unsigned char mask)
{
    return ((c < static_cast<charT>(0))
                ? true
                : ((c >= static_cast<charT>(1 << CHAR_BIT)) ? true
                                                            : map[c] & mask));
}
...
inline bool can_start(signed char c, const unsigned char* map,
                      unsigned char mask)
{
    return map[(unsigned char)c] & mask;
}
inline bool can_start(unsigned char c, const unsigned char* map,
                      unsigned char mask)
{
    return map[c] & mask;
}
inline bool can_start(unsigned short c, const unsigned char* map,
                      unsigned char mask)
{
    return ((c >= (1 << CHAR_BIT)) ? true : map[c] & mask);
}
#if !defined(__hpux) &&                                                        \
    !defined(__WINSCW__) // WCHAR_MIN not usable in pp-directives.
...
inline bool can_start(unsigned int c, const unsigned char* map,
                      unsigned char mask)
{
    return (((c >= static_cast<unsigned int>(1u << CHAR_BIT)) ? true
                                                              : map[c] & mask));
}
#endif
...
iterator BOOST_REGEX_CALL re_is_set_member(
    iterator next, iterator last, const re_set_long<char_classT>* set_,
    const regex_data<charT, traits_type>& e, bool icase)
{
    ...
    const ::boost::regex_traits_wrapper<traits_type>& traits_inst =
        *(e.m_ptraits);
    ...
    if (set_->cranges || set_->cequivalents)
    {
        ...
        //
        // try and match an equivalence class, NB only a single character can
        // match
    ...
    if (traits_inst.isctype(col, set_->cclasses) == true)
        return set_->isnot ? next : ++next;
    if ((set_->cnclasses != 0) &&
        (traits_inst.isctype(col, set_->cnclasses) == false))
        return set_->isnot ? next : ++next;
    return set_->isnot ? ++next : next;
}
...
class repeater_count
{
    ...
    repeater_count* unwind_until(int n, repeater_count* p,
  ...
  public:
    repeater_count(repeater_count** s) :
        stack(s), next(0), state_id(-1), count(0), start_pos()
    {
    }
...
class perl_matcher
{
  public:
    ...
    typedef typename regex_iterator_traits<BidiIterator>::difference_type
        difference_type;
    ...
    perl_matcher(BidiIterator first, BidiIterator end,
                 ...
                 BidiIterator l_base) :
        m_result(what),
        base(first), last(end), position(first), backstop(l_base), re(e),
        traits_inst(e.get_traits()), m_independent(false), next_count(&rep_obj),
        rep_obj(&next_count)
  ...
  private:
    void construct_init(const basic_regex<char_type, traits>& e,
                        match_flag_type f);
    ...
    {
        return ::boost::is_random_access_iterator<BidiIterator>::value
  ...
  private:
    ...
    bool icase;
    // set to true when (position == last), indicates that we may have a partial
    // match:
    ...
    // the bitmask to use when determining whether a match_any matches a newline
    // or not:
    ...
    void push_single_repeat(std::size_t c, const re_repeat* r,
                            BidiIterator last_position, int state_id);
    void push_non_greedy_repeat(const re_syntax_base* ps);
    void push_recursion(int idx, const re_syntax_base* p,
                        results_type* presults, results_type* presults2);
    ...
    bool m_unwound_alt;
    // We are unwinding a commit - used by independent subs to determine whether
    // to stop there or carry on unwinding:
    ...
    perl_matcher(const perl_matcher& that) :
        m_result(that.m_result), re(that.re), traits_inst(that.traits_inst),
        rep_obj(0)
    {
    }
};
