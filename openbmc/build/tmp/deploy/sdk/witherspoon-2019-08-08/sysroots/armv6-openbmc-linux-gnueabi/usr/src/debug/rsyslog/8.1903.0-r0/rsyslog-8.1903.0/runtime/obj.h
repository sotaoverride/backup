 ...
 * Destruct(pThis);
 *
 * A constructor is not necessary (except for some features, e.g.
 * de-serialization). If it is provided, it is a three-part constructor (to
 * handle all cases with a generic interface):
 *
 * Construct(&pThis);
...
#define objSerializeSCALAR_VAR(strm, propName, propType, var)                  \
    CHKiRet(obj.SerializeProp(strm, (uchar*)#propName, PROPTYPE_##propType,    \
                              (void*)&var));
...
#define objConstructSetObjInfo(pThis)                                          \
    ...
    ((obj_t*)(pThis))->pszName = NULL
#endif
#define objSerialize(pThis)                                                    \
    (((obj_t*)(pThis))->pObjInfo->objMethods[objMethod_SERIALIZE])
...
BEGINinterface(obj) /* name must also be changed in ENDinterface macro! */
    rsRetVal (*UseObj)(const char* srcFile, uchar* pObjName, uchar* pObjFile,
                       interface_t* pIf);
rsRetVal (*ReleaseObj)(const char* srcFile, uchar* pObjName, uchar* pObjFile,
                       interface_t* pIf);
...
rsRetVal (*InfoSetMethod)(objInfo_t* pThis, objMethod_t objMethod,
                          rsRetVal (*pHandler)(void*));
rsRetVal (*BeginSerialize)(strm_t* pStrm, obj_t* pObj);
rsRetVal (*SerializeProp)(strm_t* pStrm, uchar* pszPropName,
...
rsRetVal (*Deserialize)(void* ppObj, uchar* pszTypeExpected, strm_t* pStrm,
...
ENDinterface(obj)
#define objCURR_IF_VERSION                                                     \
    2 /* increment whenever you change the interface structure! */

    /* prototypes */
    /* the following define *is* necessary, because it provides the root way of
     * obtaining interfaces (at some place we need to start our query...
     */
