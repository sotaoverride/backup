...
int qcow2_write_raw_image(int qcow2_fd, int raw_fd, struct ext2_qcow2_hdr* hdr)
{
    ...
    max_l1_size =
        (img.image_size >> ((2 * img.cluster_bits) - 3)) + img.cluster_size;
    ...
    for (l1_index = 0; l1_index < img.l1_size; l1_index++)
    {
        ...
        if ((offset > img.image_size) || (offset <= 0))
            continue;
        ...
        {
            offset =
                ext2fs_be64_to_cpu(l2_table[l2_index]) & ~QCOW_OFLAG_COPIED;

            if (offset == 0)
                continue;

            off_out = (l1_index * img.l2_size) + l2_index;
            off_out <<= img.cluster_bits;
            ret = qcow2_copy_data(qcow2_fd, raw_fd, offset, off_out, copy_buf,
                                  img.cluster_size);
            if (ret)
                goto out;
        }
