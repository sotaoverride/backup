...
const char_constants<char>::char_type
    char_constants<char>::char_section_bracket_left;
const char_constants<char>::char_type
    char_constants<char>::char_section_bracket_right;
const char_constants<char>::char_type
    char_constants<char>::char_paren_bracket_left;
const char_constants<char>::char_type
    char_constants<char>::char_paren_bracket_right;

#endif // BOOST_LOG_BROKEN_STATIC_CONSTANTS_LINKAGE

//! Skips spaces in the beginning of the input
const char* char_constants<char>::trim_spaces_left(const char_type* begin,
                                                   const char_type* end)
{
    using namespace std;
...
const char* char_constants<char>::trim_spaces_right(const char_type* begin,
                                                    const char_type* end)
{
    using namespace std;
...
const char* char_constants<char>::scan_attr_placeholder(const char_type* begin,
                                                        const char_type* end)
{
    using namespace std;
    while (begin != end)
...
const char* char_constants<char>::parse_operand(const char_type* begin,
                                                const char_type* end,
                                                string_type& operand)
{
    ...
    if (c == char_quote)
    {
        ...
        if (p == end)
            BOOST_LOG_THROW_DESCR(
                parse_error,
                "Unterminated quoted string in the argument value");
            ...
            switch (*it)
            {
                ...
                case 'x':
                {
                    ...
                    if (std::distance(++b, str.end()) >= 2)
                    {
                        ...
                        {
                            *it++ =
                                char_type((to_number(c1) << 4) | to_number(c2));
                            it = str.erase(it, b);
                        }
                    }
                    break;
...
const char_constants<wchar_t>::char_type
    char_constants<wchar_t>::char_exclamation;
...
const char_constants<wchar_t>::char_type
    char_constants<wchar_t>::char_underline;
const char_constants<wchar_t>::char_type
    char_constants<wchar_t>::char_backslash;
const char_constants<wchar_t>::char_type
    char_constants<wchar_t>::char_section_bracket_left;
const char_constants<wchar_t>::char_type
    char_constants<wchar_t>::char_section_bracket_right;
const char_constants<wchar_t>::char_type
    char_constants<wchar_t>::char_paren_bracket_left;
const char_constants<wchar_t>::char_type
    char_constants<wchar_t>::char_paren_bracket_right;

#endif // BOOST_LOG_BROKEN_STATIC_CONSTANTS_LINKAGE

//! Skips spaces in the beginning of the input
const wchar_t* char_constants<wchar_t>::trim_spaces_left(const char_type* begin,
                                                         const char_type* end)
{
    using namespace std;
...
//! Skips spaces in the end of the input
const wchar_t*
    char_constants<wchar_t>::trim_spaces_right(const char_type* begin,
                                               const char_type* end)
{
    using namespace std;
...
//! Scans for the attribute name placeholder in the input
const wchar_t*
    char_constants<wchar_t>::scan_attr_placeholder(const char_type* begin,
                                                   const char_type* end)
{
    using namespace std;
    while (begin != end)
...
//! Parses an operand string (possibly quoted) from the input
const wchar_t* char_constants<wchar_t>::parse_operand(const char_type* begin,
                                                      const char_type* end,
                                                      string_type& operand)
{
    using namespace std; // to make sure we can use C functions unqualified
            ...
            switch (*it)
            {
                ...
                case L'u':
                {
                    ...
                    if (std::distance(++b, str.end()) >= 4)
                    {
                        ...
                        if (iswxdigit(c1) && iswxdigit(c2) && iswxdigit(c3) &&
                            iswxdigit(c4))
                        {
                            *it++ = char_type(
                                (to_number(c1) << 12) | (to_number(c2) << 8) |
                                (to_number(c3) << 4) | to_number(c4));
                            it = str.erase(it, b);
                        }
                    }
                    break;
                ...
                case L'U':
                {
                    ...
                    if (std::distance(++b, str.end()) >= 8)
                    {
                        ...
                        if (iswxdigit(c1) && iswxdigit(c2) && iswxdigit(c3) &&
                            iswxdigit(c4) && iswxdigit(c5) && iswxdigit(c6) &&
                            iswxdigit(c7) && iswxdigit(c8))
                        {
                            *it++ = char_type(
                                (to_number(c1) << 28) | (to_number(c2) << 24) |
                                (to_number(c3) << 20) | (to_number(c4) << 16) |
                                (to_number(c5) << 12) | (to_number(c6) << 8) |
                                (to_number(c7) << 4) | to_number(c8));
                            it = str.erase(it, b);
                        }
                    }
                    break;
