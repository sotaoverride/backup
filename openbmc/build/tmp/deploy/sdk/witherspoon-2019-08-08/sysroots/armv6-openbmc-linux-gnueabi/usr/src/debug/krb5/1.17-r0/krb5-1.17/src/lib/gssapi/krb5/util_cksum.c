...
krb5_error_code kg_checksum_channel_bindings(context, cb,
                                             cksum) krb5_context context;
...
krb5_checksum* cksum;
{
    ...
    len = (sizeof(krb5_int32) * 5 + cb->initiator_address.length +
           cb->acceptor_address.length + cb->application_data.length);
    ...
    code =
        krb5_c_make_checksum(context, CKSUMTYPE_RSA_MD5, 0, 0, &plaind, cksum);
    if (code)
        goto cleanup;
...
krb5_error_code kg_make_checksum_iov_v1(krb5_context context,
                                        krb5_cksumtype type, size_t cksum_len,
                                        krb5_key seq, krb5_key enc,
                                        ...
                                        int toktype, krb5_checksum* checksum)
{
    ...
    kiov[i].data.data =
        (char*)header->buffer.value + header->buffer.length - token_header_len;
    i++;
...
static krb5_error_code checksum_iov_v3(krb5_context context,
                                       krb5_cksumtype type, size_t rrc,
                                       krb5_key key, krb5_keyusage sign_usage,
                                       gss_iov_buffer_desc* iov, int iov_count,
                                       int toktype, krb5_boolean verify,
                                       krb5_boolean* valid)
{
    krb5_error_code code;
    gss_iov_buffer_desc* header;
    ...
    if (verify)
        code = krb5_k_verify_checksum_iov(context, type, key, sign_usage, kiov,
                                          kiov_count, valid);
    else
        code = krb5_k_make_checksum_iov(context, type, key, sign_usage, kiov,
                                        kiov_count);
...
krb5_error_code kg_make_checksum_iov_v3(krb5_context context,
                                        ...
                                        int toktype)
{
    return checksum_iov_v3(context, type, rrc, key, sign_usage, iov, iov_count,
                           toktype, 0, NULL);
}
