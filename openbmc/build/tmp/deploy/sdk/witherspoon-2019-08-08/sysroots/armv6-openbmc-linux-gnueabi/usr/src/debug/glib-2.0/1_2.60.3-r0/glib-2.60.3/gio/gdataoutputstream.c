...
static void g_data_output_stream_set_property(GObject* object, guint prop_id,
                                              const GValue* value,
                                              GParamSpec* pspec);
static void g_data_output_stream_get_property(GObject* object, guint prop_id,
...
static gboolean g_data_output_stream_seek(GSeekable* seekable, goffset offset,
                                          GSeekType type,
...
G_DEFINE_TYPE_WITH_CODE(
    GDataOutputStream, g_data_output_stream, G_TYPE_FILTER_OUTPUT_STREAM,
                                              ...
                                              GParamSpec* pspec)
{
    ...
    switch (prop_id)
    {
        case PROP_BYTE_ORDER:
            g_data_output_stream_set_byte_order(dstream,
...
GDataOutputStream* g_data_output_stream_new(GOutputStream* base_stream)
{
    ...
    stream = g_object_new(G_TYPE_DATA_OUTPUT_STREAM, "base-stream", base_stream,
                          NULL);
...
{
    g_return_val_if_fail(G_IS_DATA_OUTPUT_STREAM(stream),
 ...
 **/
gboolean g_data_output_stream_put_byte(GDataOutputStream* stream, guchar data,
                                       GCancellable* cancellable,
                                       GError** error)
{
    gsize bytes_written;
    ...
    return g_output_stream_write_all(G_OUTPUT_STREAM(stream), &data, 1,
 ...
 * Returns: %TRUE if @data was successfully added to the @stream.
 **/
gboolean g_data_output_stream_put_int16(GDataOutputStream* stream, gint16 data,
                                        GCancellable* cancellable,
                                        GError** error)
{
    gsize bytes_written;
    ...
    return g_output_stream_write_all(G_OUTPUT_STREAM(stream), &data, 2,
                                     &bytes_written, cancellable, error);
}
 ...
 * Returns: %TRUE if @data was successfully added to the @stream.
 **/
gboolean g_data_output_stream_put_int32(GDataOutputStream* stream, gint32 data,
                                        GCancellable* cancellable,
                                        GError** error)
{
    gsize bytes_written;
    ...
    return g_output_stream_write_all(G_OUTPUT_STREAM(stream), &data, 4,
                                     &bytes_written, cancellable, error);
}
 ...
 * Returns: %TRUE if @data was successfully added to the @stream.
 **/
gboolean g_data_output_stream_put_int64(GDataOutputStream* stream, gint64 data,
                                        GCancellable* cancellable,
                                        GError** error)
{
    gsize bytes_written;
    ...
    return g_output_stream_write_all(G_OUTPUT_STREAM(stream), &data, 8,
                                     &bytes_written, cancellable, error);
}
                                         ...
                                         GError** error)
{
    ...
    return g_output_stream_write_all(G_OUTPUT_STREAM(stream), str, strlen(str),
                                     &bytes_written, cancellable, error);
}
                                          ...
                                          GError** error)
{
    ...
    base_stream_seekable = G_SEEKABLE(base_stream);
    return g_seekable_seek(base_stream_seekable, offset, type, cancellable,
                           error);
}
                                              ...
                                              GError** error)
{
    ...
    base_stream_seekable = G_SEEKABLE(base_stream);
    return g_seekable_truncate(base_stream_seekable, offset, cancellable,
                               error);
}
