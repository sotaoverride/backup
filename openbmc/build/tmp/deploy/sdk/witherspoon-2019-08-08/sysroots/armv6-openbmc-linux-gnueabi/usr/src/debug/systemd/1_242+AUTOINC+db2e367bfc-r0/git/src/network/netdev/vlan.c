...
static int netdev_vlan_fill_message_create(NetDev* netdev, Link* link,
                                           sd_netlink_message* req)
{
    ...
    if (r < 0)
        return log_netdev_error_errno(
            netdev, r, "Could not append IFLA_VLAN_ID attribute: %m");
    ...
    r = sd_netlink_message_append_data(req, IFLA_VLAN_FLAGS, &flags,
                                       sizeof(struct ifla_vlan_flags));
    if (r < 0)
        return log_netdev_error_errno(
            netdev, r, "Could not append IFLA_VLAN_FLAGS attribute: %m");
...
static int netdev_vlan_verify(NetDev* netdev, const char* filename)
{
    ...
    {
        log_warning("VLAN without valid Id (%" PRIu16 ") configured in %s.",
                    v->id, filename);
        return -EINVAL;
    }
