...
#if defined(BOOST_THREAD_POSIX) // This one can be defined by users, so it
                                // should go first
...
class adaptive_mutex
{
  ...
  public:
    ...
    defined(BOOST_LOG_COMPILER_BARRIER)
...
class adaptive_mutex
{
  ...
  public:
    adaptive_mutex()
    {
        ...
        if (BOOST_UNLIKELY(err != 0))
            throw_exception<thread_resource_error>(
                err, "Failed to initialize an adaptive mutex",
                "adaptive_mutex::adaptive_mutex()", __FILE__, __LINE__);
    }
    ...
    bool try_lock()
    {
        ...
        if (BOOST_UNLIKELY(err != EBUSY))
            throw_exception<lock_error>(err, "Failed to lock an adaptive mutex",
                                        "adaptive_mutex::try_lock()", __FILE__,
                                        __LINE__);
        return false;
    }
  ...
  private:
    ...
    static BOOST_NOINLINE BOOST_LOG_NORETURN void
        throw_exception(int err, const char* descr, const char* func,
                        const char* file, int line)
    {
#if !defined(BOOST_EXCEPTION_DISABLE)
        boost::exception_detail::throw_exception_(ExceptionT(err, descr), func,
                                                  file, line);
#else
