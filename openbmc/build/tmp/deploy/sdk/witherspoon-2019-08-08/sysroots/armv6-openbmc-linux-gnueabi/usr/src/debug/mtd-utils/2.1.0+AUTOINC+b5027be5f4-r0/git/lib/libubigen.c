...
struct ubi_vtbl_record* ubigen_create_empty_vtbl(const struct ubigen_info* ui)
{
    ...
    {
        uint32_t crc =
            mtd_crc32(UBI_CRC32_INIT, &vtbl[i], UBI_VTBL_RECORD_SIZE_CRC);
                      ...
                      struct ubi_vtbl_record* vtbl)
{
    ...
    {
        errmsg("too high volume id %d, max. volumes is %d", vi->id,
               ui->max_volumes);
        errno = EINVAL;
        return -1;
    }
    ...
    {
        errmsg("too large alignment %d, max is %d (LEB size)", vi->alignment,
               ui->leb_size);
        errno = EINVAL;
        return -1;
    }
...
void ubigen_init_ec_hdr(const struct ubigen_info* ui, struct ubi_ec_hdr* hdr,
                        long long ec)
{
    uint32_t crc;
...
void ubigen_init_vid_hdr(const struct ubigen_info* ui,
                         const struct ubigen_vol_info* vi,
                         struct ubi_vid_hdr* hdr, int lnum, const void* data,
                         int data_size)
{
    uint32_t crc;
                        ...
                        long long bytes, int in, int out)
{
    ...
    if (!inbuf)
        return sys_errmsg("cannot allocate %d bytes of memory", ui->leb_size);
                            ...
                            struct ubi_vtbl_record* vtbl, int fd)
{
    ...
    outbuf = malloc(ui->peb_size);
    if (!outbuf)
        return sys_errmsg("failed to allocate %d bytes", ui->peb_size);
