...
static bool match_uid_gid(uid_t subject_uid, gid_t subject_gid,
                          uid_t delete_uid, gid_t delete_gid)
{
...
static int clean_sysvipc_shm(uid_t delete_uid, gid_t delete_gid, bool rm)
{
    ...
    for (;;)
    {
        ...
        if (sscanf(line,
                   ...
                   &shmid, &cpid, &lpid, &n_attached, &uid, &gid, &cuid,
                   &cgid) != 8)
            continue;
        ...
        if (shmctl(shmid, IPC_RMID, NULL) < 0)
        {
            ...
            ret = log_warning_errno(
                errno, "Failed to remove SysV shared memory segment %i: %m",
                shmid);
        }
        else
        {
...
static int clean_sysvipc_sem(uid_t delete_uid, gid_t delete_gid, bool rm)
{
    ...
    for (;;)
    {
        ...
        if (semctl(semid, 0, IPC_RMID) < 0)
        {
            ...
            ret = log_warning_errno(
                errno, "Failed to remove SysV semaphores object %i: %m", semid);
        }
        else
        {
...
static int clean_sysvipc_msg(uid_t delete_uid, gid_t delete_gid, bool rm)
{
    ...
    for (;;)
    {
        ...
        if (sscanf(line,
                   "%*i %i %*o %*u %*u " PID_FMT " " PID_FMT " " UID_FMT
                   " " GID_FMT " " UID_FMT " " GID_FMT,
        ...
        if (msgctl(msgid, IPC_RMID, NULL) < 0)
        {
            ...
            ret = log_warning_errno(
                errno, "Failed to remove SysV message queue %i: %m", msgid);
        }
        else
        {
...
static int clean_posix_shm_internal(DIR* dir, uid_t uid, gid_t gid, bool rm)
{
    ...
    FOREACH_DIRENT_ALL(de, dir, goto fail)
    {
        ...
        if (fstatat(dirfd(dir), de->d_name, &st, AT_SYMLINK_NOFOLLOW) < 0)
        {
            ...
            ret = log_warning_errno(
                errno, "Failed to stat() POSIX shared memory segment %s: %m",
                de->d_name);
            continue;
        }
        ...
        if (S_ISDIR(st.st_mode))
        {
            ...
            if (!kid)
            {
                if (errno != ENOENT)
                    ret = log_warning_errno(
                        errno, "Failed to enter shared memory directory %s: %m",
                        de->d_name);
            }
            else
            {
            ...
            {
                log_debug("Removed POSIX shared memory directory %s",
                          de->d_name);
                if (ret == 0)
                    ret = 1;
            }
...
static int clean_posix_mq(uid_t uid, gid_t gid, bool rm)
{
    ...
    FOREACH_DIRENT_ALL(de, dir, goto fail)
    {
        ...
        if (fstatat(dirfd(dir), de->d_name, &st, AT_SYMLINK_NOFOLLOW) < 0)
        {
            ...
            ret = log_warning_errno(errno, "Failed to stat() MQ segment %s: %m",
                                    de->d_name);
            continue;
        }
        ...
        if (mq_unlink(fn) < 0)
        {
            ...
            ret = log_warning_errno(
                errno, "Failed to unlink POSIX message queue %s: %m", fn);
        }
        else
        {
...
int clean_ipc_internal(uid_t uid, gid_t gid, bool rm)
{
    ...
    /* If 'rm' is true, clean all IPC objects owned by either the specified UID
     * or the specified GID. Return the last error encountered or == 0 if no
     * matching IPC objects have been found or > 0 if matching IPC objects have
     * been found and have been removed.
     *
     * If 'rm' is false, just search for IPC objects owned by either the
     * specified UID or the specified GID. In this case we return < 0 on error,
     * > 0 if we found a matching object, == 0 if we didn't.
     *
     * As special rule: if UID/GID is specified as root we'll silently not clean
     * up things, and always claim that there are IPC objects for it. */
