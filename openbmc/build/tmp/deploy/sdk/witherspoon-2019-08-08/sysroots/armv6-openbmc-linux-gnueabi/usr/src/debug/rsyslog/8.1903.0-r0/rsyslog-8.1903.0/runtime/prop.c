...
DEFobjStaticHelpers
    ...
    // */
         ...
         */
        BEGINobjConstruct(
            prop) /* be sure to specify the object type also in END macro! */
    pThis->iRefCount = 1;
INIT_ATOMIC_HELPER_MUT(pThis->mutRefCount);
ENDobjConstruct(prop)

    /* destructor for the prop object */
    BEGINobjDestruct(prop) /* be sure to specify the object type also in END and
                              CODESTART macros! */
    int currRefCount;
 ...
 */
static rsRetVal CreateStringProp(prop_t** ppThis, const uchar* psz,
                                 const int len)
{
    prop_t* pThis = NULL;
    DEFiRet;
 ...
 * a string. If the string is already contained in the property, nothing
 * happens. If the string is different (or the pointer NULL), the current
 * property is destructed and a new one created. This can be used to get a
 * specific name in those cases where there is a good chance that the property
 ...
 * rgerhards, 2009-07-01
 */
static rsRetVal CreateOrReuseStringProp(prop_t** ppThis, const uchar* psz,
                                        const int len)
{
    uchar* pszPrev;
    int lenPrev;
...
BEGINobjDebugPrint(prop) /* be sure to specify the object type also in END and
                            CODESTART macros! */
...
ENDobjDebugPrint(prop)
    ...
    BEGINobjQueryInterface(prop)
        CODESTARTobjQueryInterface(prop) if (pIf->ifVersion !=
                                             propCURR_IF_VERSION)
