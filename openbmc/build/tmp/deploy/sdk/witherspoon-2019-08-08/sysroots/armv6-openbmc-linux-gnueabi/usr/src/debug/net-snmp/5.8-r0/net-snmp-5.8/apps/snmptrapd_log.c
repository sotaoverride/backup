...
#define is_cur_time_cmd(chr)                                                   \
    ((((chr) == CHR_CUR_TIME) || ((chr) == CHR_CUR_YEAR) ||                    \
      ((chr) == CHR_CUR_MONTH) || ((chr) == CHR_CUR_MDAY) ||                   \
      ((chr) == CHR_CUR_HOUR) || ((chr) == CHR_CUR_MIN) ||                     \
      ((chr) == CHR_CUR_SEC))                                                  \
         ? TRUE                                                                \
         : FALSE)
/*
...
#define is_up_time_cmd(chr)                                                    \
    ((((chr) == CHR_UP_TIME) || ((chr) == CHR_UP_YEAR) ||                      \
      ((chr) == CHR_UP_MONTH) || ((chr) == CHR_UP_MDAY) ||                     \
      ((chr) == CHR_UP_HOUR) || ((chr) == CHR_UP_MIN) ||                       \
      ((chr) == CHR_UP_SEC))                                                   \
         ? TRUE                                                                \
         : FALSE)
/*
...
#define is_auth_cmd(chr)                                                       \
    ((((chr) == CHR_SNMP_VERSION || (chr) == CHR_SNMP_SECMOD ||                \
       (chr) == CHR_SNMP_USER))                                                \
         ? TRUE                                                                \
         : FALSE)
...
#define is_trap_cmd(chr)                                                       \
    ((((chr) == CHR_TRAP_NUM) || ((chr) == CHR_TRAP_DESC) ||                   \
      ((chr) == CHR_TRAP_STYPE) || ((chr) == CHR_TRAP_VARS))                   \
         ? TRUE                                                                \
         : FALSE)
...
#define is_fmt_cmd(chr)                                                        \
    ((is_cur_time_cmd(chr) || is_up_time_cmd(chr) || is_auth_cmd(chr) ||       \
      is_agent_cmd(chr) || is_pdu_ip_cmd(chr) || ((chr) == CHR_PDU_ENT) ||     \
      ((chr) == CHR_TRAP_CONTEXTID) || ((chr) == CHR_PDU_WRAP) ||              \
      is_trap_cmd(chr))                                                        \
         ? TRUE                                                                \
         : FALSE)
/*
...
#define is_numeric_cmd(chr)                                                    \
    ((is_cur_time_cmd(chr) || is_up_time_cmd(chr) || (chr) == CHR_TRAP_NUM)    \
         ? TRUE                                                                \
         : FALSE)
/*
 * Function:
...
static int realloc_output_temp_bfr(u_char** buf, size_t* buf_len,
 ...
 */
{
    ...
    if ((!options->left_justify) && (temp_to_write < options->width))
    {
        ...
        for (char_to_write = options->width - temp_to_write; char_to_write > 0;
             char_to_write--)
        {
...
static int realloc_handle_time_fmt(u_char** buf, size_t* buf_len,
                                   size_t* out_len, int allow_realloc,
 ...
 */
{
    ...
    else if (fmt_cmd == CHR_UP_TIME)
    {
        ...
        {
            case 0:
                sprintf(safe_bfr, "%u:%02u:%02u.%02u", hours, minutes, seconds,
                        centisecs);
                break;
            case 1:
                sprintf(safe_bfr, "1 day, %u:%02u:%02u.%02u", hours, minutes,
                        seconds, centisecs);
                break;
            default:
                sprintf(safe_bfr, "%u days, %u:%02u:%02u.%02u", days, hours,
...
static int realloc_handle_ip_fmt(u_char** buf, size_t* buf_len, size_t* out_len,
                                 int allow_realloc, options_type* options,
 ...
 */
{
    ...
    switch (fmt_cmd)
    {
        ...
        case CHR_AGENT_NAME:
            ...
            {
                host =
                    netsnmp_gethostbyaddr((char*)pdu->agent_addr, 4, AF_INET);
            }
            if (host != NULL)
            {
                if (!snmp_strcat(&temp_buf, &temp_buf_len, &temp_out_len, 1,
        ...
        case CHR_PDU_NAME:
            ...
            /*
             * Don't know how to handle this command - write the character
             * itself.
             */
        default:
            temp_buf[0] = fmt_cmd;
...
static int realloc_handle_ent_fmt(u_char** buf, size_t* buf_len,
                                  size_t* out_len, int allow_realloc,
                                  options_type* options, netsnmp_pdu* pdu)
 ...
 */
{
    ...
    switch (fmt_cmd)
    {
        case CHR_PDU_ENT:
             ...
             */
            if (!sprint_realloc_objid(&temp_buf, &temp_buf_len, &temp_out_len,
                                      1, pdu->enterprise,
...
static int realloc_handle_trap_fmt(u_char** buf, size_t* buf_len,
                                   size_t* out_len, int allow_realloc,
                                   options_type* options, netsnmp_pdu* pdu)
 ...
 */
{
    ...
    switch (fmt_cmd)
    {
        ...
        case CHR_TRAP_DESC:
             ...
             */
            tout_len = sprintf((char*)temp_buf, "%s",
...
static int realloc_handle_auth_fmt(u_char** buf, size_t* buf_len,
                                   size_t* out_len, int allow_realloc,
                                   options_type* options, netsnmp_pdu* pdu)
/*
 * Function:
...
#endif
                default:
                    snprintf((char*)temp_buf, tbuf_len, "%s",
                             pdu->securityName);
            }
            break;
...
static int realloc_handle_wrap_fmt(u_char** buf, size_t* buf_len,
                                   size_t* out_len, int allow_realloc,
                                   netsnmp_pdu* pdu)
{
    size_t i = 0;
...
static int realloc_dispatch_format_cmd(u_char** buf, size_t* buf_len,
                                       size_t* out_len, int allow_realloc,
                                       options_type* options, netsnmp_pdu* pdu,
 ...
 */
{
    ...
    {
        return realloc_handle_auth_fmt(buf, buf_len, out_len, allow_realloc,
                                       options, pdu);
    }
...
static int realloc_handle_backslash(u_char** buf, size_t* buf_len,
                                    size_t* out_len, int allow_realloc,
                                    char fmt_cmd)
 ...
 */
{
    ...
    sprintf(safe_bfr, "%.4d-%.2d-%.2d %.2d:%.2d:%.2d ",
            ...
            now_parsed->tm_mday, now_parsed->tm_hour, now_parsed->tm_min,
            now_parsed->tm_sec);
    if (!snmp_strcat(buf, buf_len, out_len, allow_realloc,
    ...
    if (pdu->trap_type == SNMP_TRAP_ENTERPRISESPECIFIC)
    {
         ...
         * Find the element after the last dot.
         */
        if (!sprint_realloc_objid(&obuf, &obuf_len, &oout_len, 1, trap_oid,
                                  trap_oid_len))
        {
            if (obuf != NULL)
            {
    ...
    for (fmt_idx = 0; format_str[fmt_idx] != '\0'; fmt_idx++)
    {
        ...
        switch (state)
        {
                 ...
                 */
                {
                    ...
                    while (j < i && next_chr && next_chr != CHR_FMT_DELIM)
                    {
                        if (next_chr == '\\')
                        {
                             ...
                             * Print to "separator" string rather than the
                             * output buffer (a bit of a hack, but it should
                             * work!)
                             */
                            next_chr = format_str[++fmt_idx];
                            if (!realloc_handle_backslash((u_char**)&sep, &i,
                                                          &j, 0, next_chr))
                            {
                                return 0;
                            }
            ...
            case PARSE_BACKSLASH:
                 ...
                 * Found a backslash.
                 */
                if (!realloc_handle_backslash(buf, buf_len, out_len,
                                              allow_realloc, next_chr))
                {
                    return 0;
                }
                state = PARSE_NORMAL;
            ...
            case PARSE_IN_FORMAT:
                ...
                else if (is_fmt_cmd(next_chr))
                {
                    ...
                    if (!realloc_dispatch_format_cmd(buf, buf_len, out_len,
                                                     allow_realloc, &options,
                                                     pdu, transport))
                    {
                        return 0;
                    }
