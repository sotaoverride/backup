/* nspoll.c
 *
 * This is an io waiter interface utilizing the much-more-efficient poll/epoll
 * API. Note that it may not always be available for a given driver. If so, that
 * is reported back to the upper peer which then should consult a nssel-based io
 * waiter.
 *
...
DEFobjStaticHelpers DEFobjCurrIf(glbl)
     ...
     */
    static rsRetVal loadDrvr(nspoll_t* pThis)
{
    DEFiRet;
    uchar* pBaseDrvrName;
    ...
    if (snprintf((char*)szDrvrName, sizeof(szDrvrName), "lmnsdpoll_%s",
...
BEGINobjConstruct(
    nspoll) /* be sure to specify the object type also in END macro! */
    ENDobjConstruct(nspoll)

    /* destructor for the nspoll object */
    BEGINobjDestruct(nspoll) /* be sure to specify the object type also in END
                                and CODESTART macros! */
    CODESTARTobjDestruct(nspoll) if (
...
ENDobjDestruct(nspoll)
    ...
    static rsRetVal ConstructFinalize(nspoll_t* pThis)
{
    DEFiRet;
 ...
 */
static rsRetVal Wait(nspoll_t* pThis, int timeout, int* numEntries,
 ...
 */
static rsRetVal SetDrvrName(nspoll_t* pThis, uchar* pszName)
{
    DEFiRet;
    ISOBJ_TYPE_assert(pThis, nspoll);
 ...
 */
static rsRetVal Ctl(nspoll_t* pThis, netstrm_t* pStrm, int id, void* pUsr,
                    int mode, int op)
{
    DEFiRet;
    ISOBJ_TYPE_assert(pThis, nspoll);
    iRet =
        pThis->Drvr.Ctl(pThis->pDrvrData, pStrm->pDrvrData, id, pUsr, mode, op);
    RETiRet;
}

/* queryInterface function */
BEGINobjQueryInterface(nspoll)
    CODESTARTobjQueryInterface(nspoll) if (pIf->ifVersion !=
                                           nspollCURR_IF_VERSION)
...
finalize_it
    : ENDobjQueryInterface(nspoll)
       ...
       */
      BEGINObjClassExit(
          nspoll, OBJ_IS_LOADABLE_MODULE) /* CHANGE class also in END MACRO! */
