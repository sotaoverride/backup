...
#define FILESIZE_MAX                                                           \
    20000.0 /* for each file in sizeof(int). Must be a float #                 \
...
static void check_crc_ccitt(char* filename)
{
    ...
    if (!fp)
    {
        printf("Verify checksum:Error! Cannot open filename passed for verify "
               "checksum: %s\n",
               filename);
        exit(1);
    }
...
static void checksum(char* filename)
{
    ...
    /*the CRC has been calculated. now close the file and open it in append
     * mode*/
    fclose(fp);
...
int main(void)
{
    ...
    for (counter = 0; counter < numberFiles; counter++)
    {
        ...
        while (((data_size = (int)(1 + (int)(FILESIZE_MAX * rand() /
                                             (RAND_MAX + 1.0)))) <
                100)) /*file should not be less than 100 ints long. (so that we
                         have decent length files, that's all)*/
