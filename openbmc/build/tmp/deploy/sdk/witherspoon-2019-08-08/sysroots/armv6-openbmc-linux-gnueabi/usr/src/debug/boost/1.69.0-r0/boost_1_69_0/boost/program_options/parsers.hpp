...
#pragma warning(push)
#pragma warning(                                                               \
    disable : 4251) // class 'std::vector<_Ty>' needs to have dll-interface to
...
class basic_parsed_options
{
  public:
     ...
     *  The canonical option prefix  for the parser which generated these
     * results, depending on the settings for basic_command_line_parser::style()
     * or cmdline::style(). In order of precedence of command_line_style enums:
     *      allow_long
     *      allow_long_disguise
...
basic_parsed_options<charT> parse_command_line(
    int argc, const charT* const argv[], const options_description&,
    int style = 0,
...
BOOST_PROGRAM_OPTIONS_DECL std::vector<std::wstring> split_unix(
    const std::wstring& cmdline, const std::wstring& seperator = L" \t",
