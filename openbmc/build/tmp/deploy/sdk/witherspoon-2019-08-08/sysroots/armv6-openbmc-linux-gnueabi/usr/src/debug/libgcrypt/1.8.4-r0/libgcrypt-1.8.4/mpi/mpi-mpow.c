...
#ifdef USE_BARRETT
static void barrett_mulm(gcry_mpi_t w, gcry_mpi_t u, gcry_mpi_t v, gcry_mpi_t m,
                         gcry_mpi_t y, int k, gcry_mpi_t r1, gcry_mpi_t r2);
static gcry_mpi_t init_barrett(gcry_mpi_t m, int* k, gcry_mpi_t* r1,
                               gcry_mpi_t* r2);
static int calc_barrett(gcry_mpi_t r, gcry_mpi_t x, gcry_mpi_t m, gcry_mpi_t y,
                        int k, gcry_mpi_t r1, gcry_mpi_t r2);
#else
#define barrett_mulm(w, u, v, m, y, k, r1, r2)                                 \
    _gcry_mpi_mulm((w), (u), (v), (m))
#endif

static int build_index(gcry_mpi_t* exparray, int k, int i, int t)
{
    int j, bitno;
    int idx = 0;
...
void _gcry_mpi_mulpowm(gcry_mpi_t res, gcry_mpi_t* basearray,
                       gcry_mpi_t* exparray, gcry_mpi_t m)
{
    ...
    for (i = 1; i <= t; i++)
    {
        barrett_mulm(tmp, res, res, m, barrett_y, barrett_k, barrett_r1,
                     barrett_r2);
                ...
                for (j = 0; j < k; j++)
                {
                    if ((idx & (1 << j)))
                    {
                        if (!G[idx])
                            ...
                            barrett_mulm(G[idx], G[idx], basearray[j], m,
                                         barrett_y, barrett_k, barrett_r1,
                                         barrett_r2);
                    }
                }
                if (!G[idx])
        ...
        barrett_mulm(res, tmp, G[idx], m, barrett_y, barrett_k, barrett_r1,
                     barrett_r2);
    }
