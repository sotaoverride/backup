...
static errcode_t ext2fs_ext_attr_block_csum(
    ext2_filsys fs, ext2_ino_t inum EXT2FS_ATTR((unused)), blk64_t block,
    struct ext2_ext_attr_header* hdr, __u32* crc)
{
    char* buf = (char*)hdr;
    ...
    *crc =
        ext2fs_crc32c_le(fs->csum_seed, (unsigned char*)&block, sizeof(block));
                                     ...
                                     int* offset, int need_swab)
{
    ...
    max_sane_entries =
        (fs->blocksize - count_offset) / sizeof(struct ext2_dx_entry);
...
errcode_t ext2fs_get_dx_countlimit(ext2_filsys fs,
                                   struct ext2_dir_entry* dirent,
                                   struct ext2_dx_countlimit** cc, int* offset)
{
                                    ...
                                    int size)
{
    ...
    *crc = ext2fs_crc32c_le(fs->csum_seed, (unsigned char*)&inum, sizeof(inum));
                                        ...
                                        struct ext2_dir_entry* dirent)
{
    ...
    retval =
        ext2fs_dirent_csum(fs, inum, dirent, &crc, (char*)t - (char*)dirent);
    if (retval)
        return retval;
                                ...
                                struct ext2_dx_tail* t)
{
    ...
    *crc = ext2fs_crc32c_le(*crc, (unsigned char*)buf, size);
    *crc =
        ext2fs_crc32c_le(*crc, (unsigned char*)t, sizeof(struct ext2_dx_tail));
                                 ...
                                 struct ext2_dir_entry* dirent)
{
    ...
    retval =
        ext2fs_dx_csum(fs, inum, dirent, &calculated, count_offset, count, t);
    if (retval)
        return 0;
...
#define EXT3_EXTENT_TAIL_OFFSET(hdr)                                           \
...
{
    return (struct ext3_extent_tail*)(((char*)h) + EXT3_EXTENT_TAIL_OFFSET(h));
}
                                          ...
                                          __u32* crc)
{
    ...
    size = EXT3_EXTENT_TAIL_OFFSET(eh) +
...
int ext2fs_inode_bitmap_csum_verify(ext2_filsys fs, dgrp_t group, char* bitmap,
                                    int size)
{
    struct ext4_group_desc* gdp =
        (struct ext4_group_desc*)ext2fs_group_desc(fs, fs->group_desc, group);
    __u32 provided, calculated;

    if (!ext2fs_has_feature_metadata_csum(fs->super))
        return 1;
    provided = gdp->bg_inode_bitmap_csum_lo;
    calculated = ext2fs_crc32c_le(fs->csum_seed, (unsigned char*)bitmap, size);
...
int ext2fs_block_bitmap_csum_verify(ext2_filsys fs, dgrp_t group, char* bitmap,
                                    int size)
{
    struct ext4_group_desc* gdp =
        (struct ext4_group_desc*)ext2fs_group_desc(fs, fs->group_desc, group);
...
static errcode_t ext2fs_inode_csum(ext2_filsys fs, ext2_ino_t inum,
                                   struct ext2_inode_large* inode, __u32* crc,
                                   int has_hi)
{
    __u32 gen;
                             ...
                             struct ext2_inode_large* inode)
{
     ...
     */
    for (cp = (char*)inode, i = 0; i < sizeof(struct ext2_inode); cp++, i++)
        if (*cp)
            return 0;
...
__u16 ext2fs_group_desc_csum(ext2_filsys fs, dgrp_t group)
{
    ...
    if (ext2fs_has_feature_metadata_csum(fs->super))
    {
        ...
        crc32 = ext2fs_crc32c_le(fs->csum_seed, (unsigned char*)&group,
                                 sizeof(group));
        crc32 = ext2fs_crc32c_le(crc32, (unsigned char*)desc, size);
    ...
    crc = ext2fs_crc16(~0, fs->super->s_uuid, sizeof(fs->super->s_uuid));
    ...
    if (offset < size)
    {
        crc = ext2fs_crc16(crc, (char*)desc + offset, size - offset);
    }
#ifdef WORDS_BIGENDIAN
    /*
...
int ext2fs_group_desc_csum_verify(ext2_filsys fs, dgrp_t group)
{
    if (ext2fs_has_group_desc_csum(fs) &&
        (ext2fs_bg_checksum(fs, group) != ext2fs_group_desc_csum(fs, group)))
        return 0;
    ...
    for (i = 0; i < fs->group_desc_count; i++)
    {
        ...
        else
        {
            int unused =
                ...
                find_last_inode_ingrp(fs->inode_map, sb->s_inodes_per_group, i);
...
void print_csum(const char* msg, ext2_filsys fs, dgrp_t group)
{
    ...
    printf("%s UUID %s=%04x, grp %u=%04x: %04x=%04x\n", msg,
           e2p_uuid2str(sb->s_uuid), crc1, group, crc2, crc3,
