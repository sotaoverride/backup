...
DEFINE_PRIVATE_STRING_TABLE_LOOKUP_FROM_STRING(l2tp_l2spec_type,
                                               L2tpL2specType);
...
DEFINE_CONFIG_PARSE_ENUM(config_parse_l2tp_encap_type, l2tp_encap_type,
                         L2tpEncapType,
                         "Failed to parse L2TP Encapsulation Type");

static const char* const
    l2tp_local_address_type_table[_NETDEV_L2TP_LOCAL_ADDRESS_MAX] = {
        [NETDEV_L2TP_LOCAL_ADDRESS_AUTO] = "auto",
        [NETDEV_L2TP_LOCAL_ADDRESS_STATIC] = "static",
        [NETDEV_L2TP_LOCAL_ADDRESS_DYNAMIC] = "dynamic",
};

DEFINE_PRIVATE_STRING_TABLE_LOOKUP_FROM_STRING(l2tp_local_address_type,
...
static int l2tp_session_new_static(L2tpTunnel* t, const char* filename,
                                   unsigned section_line, L2tpSession** ret)
{
    ...
    *s = (L2tpSession){
    ...
    r = ordered_hashmap_ensure_allocated(&t->sessions_by_section,
...
static int netdev_l2tp_fill_message_tunnel(NetDev* netdev,
                                           ...
                                           sd_netlink_message** ret)
{
    ...
    r = sd_genl_message_new(netdev->manager->genl, SD_GENL_L2TP,
                            L2TP_CMD_TUNNEL_CREATE, &m);
    if (r < 0)
        return log_netdev_error_errno(
            netdev, r, "Failed to create generic netlink message: %m");

    r = sd_netlink_message_append_u32(m, L2TP_ATTR_CONN_ID, t->tunnel_id);
    if (r < 0)
        return log_netdev_error_errno(
            netdev, r, "Could not append L2TP_ATTR_CONN_ID attribute: %m");

    r = sd_netlink_message_append_u32(m, L2TP_ATTR_PEER_CONN_ID,
                                      t->peer_tunnel_id);
    if (r < 0)
        return log_netdev_error_errno(
            netdev, r, "Could not append L2TP_ATTR_PEER_CONN_ID attribute: %m");
    ...
    if (r < 0)
        return log_netdev_error_errno(
            netdev, r, "Could not append L2TP_ATTR_ENCAP_TYPE attribute: %m");

    if (t->family == AF_INET)
    {
        r = sd_netlink_message_append_in_addr(m, L2TP_ATTR_IP_SADDR,
                                              &local_address->in);
        if (r < 0)
            return log_netdev_error_errno(
                netdev, r, "Could not append L2TP_ATTR_IP_SADDR attribute: %m");

        r = sd_netlink_message_append_in_addr(m, L2TP_ATTR_IP_DADDR,
                                              &t->remote.in);
        if (r < 0)
            return log_netdev_error_errno(
                netdev, r, "Could not append L2TP_ATTR_IP_DADDR attribute: %m");
    }
    else
    {
        r = sd_netlink_message_append_in6_addr(m, L2TP_ATTR_IP6_SADDR,
                                               &local_address->in6);
        if (r < 0)
            return log_netdev_error_errno(
                netdev, r,
        ...
        r = sd_netlink_message_append_in6_addr(m, L2TP_ATTR_IP6_DADDR,
                                               &t->remote.in6);
        if (r < 0)
            return log_netdev_error_errno(
                netdev, r,
    ...
    {
        r = sd_netlink_message_append_u16(m, L2TP_ATTR_UDP_SPORT,
                                          t->l2tp_udp_sport);
        if (r < 0)
            return log_netdev_error_errno(
                netdev, r,
        ...
        r = sd_netlink_message_append_u16(m, L2TP_ATTR_UDP_DPORT,
                                          t->l2tp_udp_dport);
        if (r < 0)
            return log_netdev_error_errno(
                netdev, r,
        ...
        if (t->udp_csum)
        {
            r = sd_netlink_message_append_u8(m, L2TP_ATTR_UDP_CSUM,
                                             t->udp_csum);
            if (r < 0)
                return log_netdev_error_errno(
                    netdev, r,
        ...
        if (t->udp6_csum_tx)
        {
            ...
            if (r < 0)
                return log_netdev_error_errno(
                    netdev, r,
                    "Could not append L2TP_ATTR_UDP_ZERO_CSUM6_TX attribute: "
                    "%m");
        }
        ...
        if (t->udp6_csum_rx)
        {
            ...
            if (r < 0)
                return log_netdev_error_errno(
                    netdev, r,
                    "Could not append L2TP_ATTR_UDP_ZERO_CSUM6_RX attribute: "
                    "%m");
        }
    }
...
static int netdev_l2tp_fill_message_session(NetDev* netdev,
                                            L2tpSession* session,
                                            sd_netlink_message** ret)
{
    _cleanup_(sd_netlink_message_unrefp) sd_netlink_message* m = NULL;
    ...
    r = sd_genl_message_new(netdev->manager->genl, SD_GENL_L2TP,
                            L2TP_CMD_SESSION_CREATE, &m);
    if (r < 0)
        return log_netdev_error_errno(
            netdev, r, "Failed to create generic netlink message: %m");

    r = sd_netlink_message_append_u32(m, L2TP_ATTR_CONN_ID,
                                      session->tunnel->tunnel_id);
    if (r < 0)
        return log_netdev_error_errno(
            netdev, r, "Could not append L2TP_ATTR_CONN_ID attribute: %m");

    r = sd_netlink_message_append_u32(m, L2TP_ATTR_PEER_CONN_ID,
                                      session->tunnel->peer_tunnel_id);
    if (r < 0)
        return log_netdev_error_errno(
            netdev, r, "Could not append L2TP_ATTR_PEER_CONN_ID attribute: %m");

    r = sd_netlink_message_append_u32(m, L2TP_ATTR_SESSION_ID,
                                      session->session_id);
    if (r < 0)
        return log_netdev_error_errno(
            netdev, r, "Could not append L2TP_ATTR_SESSION_ID attribute: %m");

    r = sd_netlink_message_append_u32(m, L2TP_ATTR_PEER_SESSION_ID,
    ...
    if (r < 0)
        return log_netdev_error_errno(
            netdev, r, "Could not append L2TP_ATTR_PW_TYPE attribute: %m");
    ...
    if (r < 0)
        return log_netdev_error_errno(
            netdev, r, "Could not append L2TP_ATTR_L2SPEC_TYPE attribute: %m");

    r = sd_netlink_message_append_u8(m, L2TP_ATTR_L2SPEC_LEN, l2_spec_len);
    if (r < 0)
        return log_netdev_error_errno(
            netdev, r, "Could not append L2TP_ATTR_L2SPEC_LEN attribute: %m");

    r = sd_netlink_message_append_string(m, L2TP_ATTR_IFNAME, session->name);
    if (r < 0)
        return log_netdev_error_errno(
            netdev, r, "Could not append L2TP_ATTR_IFNAME attribute: %m");
...
static int l2tp_acquire_local_address_one(L2tpTunnel* t, Address* a,
...
static int l2tp_acquire_local_address(L2tpTunnel* t, Link* link,
                                      union in_addr_union* ret)
{
    Address* a;
    Iterator i;
...
static int l2tp_create_session_handler(sd_netlink* rtnl, sd_netlink_message* m,
                                       L2tpSession* session)
{
    NetDev* netdev;
    int r;
    ...
    if (r == -EEXIST)
        log_netdev_info(netdev,
                        "L2TP session %s exists, using existing without "
                        "changing its parameters",
                        session->name);
    else if (r < 0)
    {
        log_netdev_warning_errno(netdev, r,
                                 "L2TP session %s could not be created: %m",
                                 session->name);
        return 1;
    }
...
static int l2tp_create_session(NetDev* netdev, L2tpSession* session)
{
    ...
    r = netlink_call_async(netdev->manager->genl, NULL, n,
                           l2tp_create_session_handler,
                           l2tp_session_destroy_callback, session);
    if (r < 0)
        return log_netdev_error_errno(
            netdev, r, "Failed to create L2TP session %s: %m", session->name);

    netdev_ref(netdev);
    return 0;
}

static int l2tp_create_tunnel_handler(sd_netlink* rtnl, sd_netlink_message* m,
                                      NetDev* netdev)
{
    L2tpSession* session;
    L2tpTunnel* t;
...
static int l2tp_create_tunnel(NetDev* netdev, Link* link)
{
    ...
    r = netlink_call_async(netdev->manager->genl, NULL, m,
                           l2tp_create_tunnel_handler, netdev_destroy_callback,
                           netdev);
    if (r < 0)
        return log_netdev_error_errno(netdev, r,
...
int config_parse_l2tp_tunnel_address(const char* unit, const char* filename,
                                     ...
                                     void* userdata)
{
    ...
    if (r < 0)
    {
        log_syntax(unit, LOG_ERR, filename, line, r,
                   "Invalid L2TP Tunnel address specified in %s='%s', ignoring "
                   "assignment: %m",
                   lvalue, rvalue);
        return 0;
    }
...
int config_parse_l2tp_tunnel_id(const char* unit, const char* filename,
                                unsigned line, const char* section,
                                unsigned section_line, const char* lvalue,
                                int ltype, const char* rvalue, void* data,
                                void* userdata)
{
    ...
    {
        log_syntax(unit, LOG_ERR, filename, line, r,
                   "Failed to parse L2TP tunnel id. Ignoring assignment: %s",
                   rvalue);
        return 0;
    }
...
int config_parse_l2tp_session_id(const char* unit, const char* filename,
                                 unsigned line, const char* section,
                                 unsigned section_line, const char* lvalue,
                                 int ltype, const char* rvalue, void* data,
                                 void* userdata)
{
    ...
    {
        log_syntax(unit, LOG_ERR, filename, line, r,
                   "Failed to parse L2TP session id. Ignoring assignment: %s",
                   rvalue);
        return 0;
    }
...
int config_parse_l2tp_session_l2spec(const char* unit, const char* filename,
                                     unsigned line, const char* section,
                                     unsigned section_line, const char* lvalue,
                                     int ltype, const char* rvalue, void* data,
                                     void* userdata)
{
    ...
    if (spec < 0)
    {
        log_syntax(unit, LOG_ERR, filename, line, 0,
                   "Failed to parse layer2 specific header type. Ignoring "
                   "assignment: %s",
                   rvalue);
        return 0;
    }
...
int config_parse_l2tp_session_name(const char* unit, const char* filename,
                                   unsigned line, const char* section,
                                   unsigned section_line, const char* lvalue,
                                   int ltype, const char* rvalue, void* data,
                                   void* userdata)
{
    ...
    if (!ifname_valid(rvalue))
    {
        log_syntax(
            ...
            "Failed to parse L2TP tunnel session name. Ignoring assignment: %s",
            rvalue);
        return 0;
    }
...
static void l2tp_tunnel_done(NetDev* netdev)
{
    ...
    ordered_hashmap_free_with_destructor(t->sessions_by_section,
                                         l2tp_session_free);
}
