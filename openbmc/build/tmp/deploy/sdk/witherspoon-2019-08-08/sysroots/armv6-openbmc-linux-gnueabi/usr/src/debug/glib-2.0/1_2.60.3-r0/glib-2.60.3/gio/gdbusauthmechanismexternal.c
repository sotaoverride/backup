...
G_DEFINE_TYPE_WITH_PRIVATE(GDBusAuthMechanismExternal,
...
{
    // GDBusAuthMechanismExternal *mechanism = G_DBUS_AUTH_MECHANISM_EXTERNAL
    // (object);

    if (G_OBJECT_CLASS(_g_dbus_auth_mechanism_external_parent_class)
            ->finalize != NULL)
        G_OBJECT_CLASS(_g_dbus_auth_mechanism_external_parent_class)
            ->finalize(object);
}

static void _g_dbus_auth_mechanism_external_class_init(
    GDBusAuthMechanismExternalClass* klass)
{
...
static gint mechanism_get_priority(void)
{
    /* We prefer EXTERNAL to most other mechanism (DBUS_COOKIE_SHA1 and
     * ANONYMOUS) */
    return 100;
}
    ...
    mechanism_server_get_state(GDBusAuthMechanism* mechanism)
{
    ...
    g_return_val_if_fail(G_IS_DBUS_AUTH_MECHANISM_EXTERNAL(mechanism),
...
static gboolean data_matches_credentials(const gchar* data, gsize data_len,
...
#else
        /* TODO: Dont know how to compare credentials on this OS. Please
         * implement. */
#endif
                                      ...
                                      gsize initial_response_len)
{
    ...
    {
        if (data_matches_credentials(
                initial_response, initial_response_len,
                                          ...
                                          const gchar* data, gsize data_len)
{
    ...
    if (data_matches_credentials(
            data, data_len, _g_dbus_auth_mechanism_get_credentials(mechanism)))
    {
        m->priv->state = G_DBUS_AUTH_MECHANISM_STATE_ACCEPTED;
    }
    else
    ...
    *out_initial_response_len = 0;
    ...
    initial_response =
        g_strdup_printf("%" G_GINT64_FORMAT,
                        (gint64)g_credentials_get_unix_user(credentials, NULL));
