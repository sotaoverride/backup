...
G_DEFINE_TYPE_WITH_CODE(
    GSocks5Proxy, g_socks5_proxy, G_TYPE_OBJECT,
    G_IMPLEMENT_INTERFACE(G_TYPE_PROXY, g_socks5_proxy_iface_init)
        _g_io_modules_ensure_extension_points_registered();
    g_io_extension_point_implement(G_PROXY_EXTENSION_POINT_NAME,
                                   g_define_type_id, "socks5", 0))
...
static gboolean parse_nego_reply(const guint8* data, gboolean has_auth,
                                 gboolean* must_auth, GError** error)
{
    ...
    switch (data[1])
    {
        ...
        case SOCKS5_AUTH_USR_PASS:
            ...
            {
                g_set_error_literal(
                    error, G_IO_ERROR, G_IO_ERROR_PROXY_NEED_AUTH,
        ...
        default:
            g_set_error_literal(
                error, G_IO_ERROR, G_IO_ERROR_PROXY_AUTH_FAILED,
...
static gint set_auth_msg(guint8* msg, const gchar* username,
...
static gint set_connect_msg(guint8* msg, const gchar* hostname, guint16 port,
                            GError** error)
{
    guint len = 0;
...
static gboolean parse_connect_reply(const guint8* data, gint* atype,
                                    GError** error)
{
    if (data[0] != SOCKS5_VERSION)
    {
    ...
    switch (data[1])
    {
        case SOCKS5_REP_SUCCEEDED:
            ...
            {
                g_set_error_literal(
                    error, G_IO_ERROR, G_IO_ERROR_PROXY_FAILED,
                    _("The server is not a SOCKSv5 proxy server."));
                return FALSE;
            }
            ...
            switch (data[3])
            {
                ...
                default:
                    g_set_error_literal(error, G_IO_ERROR,
                                        ...
                                        _("The SOCKSv5 proxy server uses "
        ...
        case SOCKS5_REP_NOT_ALLOWED:
            g_set_error_literal(
                error, G_IO_ERROR, G_IO_ERROR_PROXY_NOT_ALLOWED,
        ...
        case SOCKS5_REP_NET_UNREACH:
            g_set_error_literal(
                error, G_IO_ERROR, G_IO_ERROR_NETWORK_UNREACHABLE,
...
static GIOStream* g_socks5_proxy_connect(GProxy* proxy, GIOStream* io_stream,
                                         ...
                                         GError** error)
{
    ...
    /* Send SOCKS5 handshake */
    {
        ...
        if (!g_output_stream_write_all(out, msg, len, NULL, cancellable, error))
            goto error;
    }
    ...
    /* Receive SOCKS5 response and reply with authentication if required */
    {
        ...
        if (!g_input_stream_read_all(in, data, sizeof(data), NULL, cancellable,
                                     error))
            goto error;
        ...
        if (must_auth)
        {
            ...
            if (!g_output_stream_write_all(out, msg, len, NULL, cancellable,
                                           error))
                goto error;
    ...
    /* Read SOCKS5 response */
    {
        ...
        if (!g_input_stream_read_all(in, data, 4, NULL, cancellable, error))
            goto error;
        ...
        {
            case SOCKS5_ATYP_IPV4:
                if (!g_input_stream_read_all(in, data, 6, NULL, cancellable,
                                             error))
                    goto error;
                break;

            case SOCKS5_ATYP_IPV6:
                if (!g_input_stream_read_all(in, data, 18, NULL, cancellable,
                                             error))
                    goto error;
                break;

            case SOCKS5_ATYP_DOMAINNAME:
                if (!g_input_stream_read_all(in, data, 1, NULL, cancellable,
                                             error))
                    goto error;
...
static void nego_msg_write_cb(GObject* source, GAsyncResult* res,
                              gpointer user_data);
static void nego_reply_read_cb(GObject* source, GAsyncResult* res,
                               gpointer user_data);
static void auth_msg_write_cb(GObject* source, GAsyncResult* res,
                              gpointer user_data);
static void auth_reply_read_cb(GObject* source, GAsyncResult* result,
                               gpointer user_data);
static void send_connect_msg(GTask* task);
static void connect_msg_write_cb(GObject* source, GAsyncResult* result,
                                 gpointer user_data);
static void connect_reply_read_cb(GObject* source, GAsyncResult* result,
                                  gpointer user_data);
static void connect_addr_len_read_cb(GObject* source, GAsyncResult* result,
                                     gpointer user_data);
static void connect_addr_read_cb(GObject* source, GAsyncResult* result,
                                 gpointer user_data);
...
static void do_read(GAsyncReadyCallback callback, GTask* task,
                    ConnectAsyncData* data)
{
    GInputStream* in;
    in = g_io_stream_get_input_stream(data->io_stream);
    g_input_stream_read_async(in, data->buffer + data->offset,
                              data->length - data->offset,
                              g_task_get_priority(task),
                              g_task_get_cancellable(task), callback, task);
}

static void do_write(GAsyncReadyCallback callback, GTask* task,
                     ConnectAsyncData* data)
{
    GOutputStream* out;
    out = g_io_stream_get_output_stream(data->io_stream);
    g_output_stream_write_async(out, data->buffer + data->offset,
                                data->length - data->offset,
                                g_task_get_priority(task),
                                g_task_get_cancellable(task), callback, task);
}

static void g_socks5_proxy_connect_async(GProxy* proxy, GIOStream* io_stream,
                                         GProxyAddress* proxy_address,
                                         GCancellable* cancellable,
                                         ...
                                         gpointer user_data)
{
    ...
    g_object_get(G_OBJECT(proxy_address), "destination-hostname",
                 &data->hostname, "destination-port", &data->port, "username",
                              ...
                              gpointer user_data)
{
    ...
    written =
        g_output_stream_write_finish(G_OUTPUT_STREAM(source), res, &error);
                               ...
                               gpointer user_data)
{
    ...
    if (data->offset == data->length)
    {
        ...
        if (must_auth)
        {
            ...
            data->length = set_auth_msg(data->buffer, data->username,
                                        data->password, &error);
            data->offset = 0;
...
static void auth_msg_write_cb(GObject* source, GAsyncResult* result,
                              gpointer user_data)
{
    GTask* task = user_data;
    ConnectAsyncData* data = g_task_get_task_data(task);
    ...
    written =
        g_output_stream_write_finish(G_OUTPUT_STREAM(source), result, &error);
...
static void send_connect_msg(GTask* task)
{
    ...
    data->length =
        set_connect_msg(data->buffer, data->hostname, data->port, &error);
    data->offset = 0;
                                 ...
                                 gpointer user_data)
{
    ...
    if (data->offset == data->length)
    {
        g_task_return_pointer(task, g_object_ref(data->io_stream),
                              g_object_unref);
        g_object_unref(task);
        return;
    }
...
static GIOStream* g_socks5_proxy_connect_finish(GProxy* proxy,
                                                GAsyncResult* result,
                                                GError** error)
{
