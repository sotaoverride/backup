        ...
        if (__glibc_unlikely(debug_level >= 3))
        {
            for (size_t idx = 0; idx < db->head->module; ++idx)
            {
                ...
                while (run != ENDREF)
                {
                    ...
                    {
                        struct datahead* dh =
                            (struct datahead*)(db->data + he->packet);
                        if (he->packet + dh->allocsize > db->head->first_free)
    ...
    if (__glibc_unlikely(db->head->first_free + len > db->head->data_size))
    {
        if (!tried_resize)
        {
            ...
            size_t new_data_size =
                (db->head->data_size + MAX(2 * len, db->head->data_size / 8));
            size_t newtotal =
                (sizeof(struct database_pers_head) +
                 roundup(db->head->module * sizeof(ref_t), ALIGN) +
                 new_data_size);
            ...
            if (db->mmap_used &&
               ...
               become magically available.  */
                && TEMP_FAILURE_RETRY_VAL(posix_fallocate(
                       db->wr_fd, oldtotal, newtotal - oldtotal)) == 0)
            {
