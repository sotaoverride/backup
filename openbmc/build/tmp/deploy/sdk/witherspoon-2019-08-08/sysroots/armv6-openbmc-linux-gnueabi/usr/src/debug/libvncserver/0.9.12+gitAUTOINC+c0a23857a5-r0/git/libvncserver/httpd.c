...
void rfbHttpInitSockets(rfbScreenInfoPtr rfbScreen)
{
    ...
    if ((rfbScreen->httpListenSock = rfbListenOnTCPPort(
             rfbScreen->httpPort, rfbScreen->listenInterface)) < 0)
    {
    ...
    if ((rfbScreen->httpListen6Sock = rfbListenOnTCP6Port(
             rfbScreen->http6Port, rfbScreen->listen6Interface)) < 0)
    {
...
void rfbHttpCheckFds(rfbScreenInfoPtr rfbScreen)
{
    ...
    tv.tv_usec = 0;
    nfds =
        select(rfbMax(rfbScreen->httpListen6Sock,
                      rfbMax(rfbScreen->httpSock, rfbScreen->httpListenSock)) +
                   1,
               &fds, NULL, NULL, &tv);
    if (nfds == 0)
    {
    ...
    if (FD_ISSET(rfbScreen->httpListenSock, &fds) ||
        FD_ISSET(rfbScreen->httpListen6Sock, &fds))
    {
        if (rfbScreen->httpSock >= 0)
            close(rfbScreen->httpSock);
        ...
        if (FD_ISSET(rfbScreen->httpListenSock, &fds))
        {
            if ((rfbScreen->httpSock =
                     accept(rfbScreen->httpListenSock, (struct sockaddr*)&addr,
                            &addrlen)) < 0)
            {
        ...
        {
            if ((rfbScreen->httpSock =
                     accept(rfbScreen->httpListen6Sock, (struct sockaddr*)&addr,
                            &addrlen)) < 0)
            {
                rfbLogPerror("httpCheckFds: accept");
                return;
...
#endif
        if (!hosts_ctl("vnc", STRING_UNKNOWN, host, STRING_UNKNOWN))
        {
...
static void httpCloseSock(rfbScreenInfoPtr rfbScreen)
{
    close(rfbScreen->httpSock);
    rfbScreen->httpSock = -1;
    buf_filled = 0;
...
static void httpProcessInput(rfbScreenInfoPtr rfbScreen)
{
    ...
    {
        const static char* PROXY_OK_STR =
        ...
        if (!strncmp(buf, "CONNECT ", 8))
        {
            if (atoi(strchr(buf, ':') + 1) != rfbScreen->port)
            {
                ...
                rfbWriteExact(&cl, INVALID_REQUEST_STR,
                              strlen(INVALID_REQUEST_STR));
                httpCloseSock(rfbScreen);
                return;
            }
            /* proxy connection */
        ...
        if (!strncmp(buf, "GET ", 4) &&
            !strncmp(strchr(buf, '/'), "/proxied.connection HTTP/1.", 27))
        {
            /* proxy connection */
...
#else
    rfbLog("httpd: get '%s' for %s\n", fname + 1, inet_ntoa(addr.sin_addr));
#endif
        ...
        if (performSubstitutions)
        {
            ...
            while ((dollar = strchr(ptr, '$')) != NULL)
            {
                ...
                else if (compareAndSkip(&ptr, "$DESKTOP"))
                {
                    ...
                    rfbWriteExact(&cl, rfbScreen->desktopName,
                                  strlen(rfbScreen->desktopName));
                }
                else if (compareAndSkip(&ptr, "$DISPLAY"))
                {

                    sprintf(str, "%s:%d", rfbScreen->thisHost,
                            rfbScreen->port - 5900);
                    rfbWriteExact(&cl, str, strlen(str));
                }
                else if (compareAndSkip(&ptr, "$USER"))
                {
                    ...
                    if (user)
                    {
                        rfbWriteExact(&cl, user, strlen(user));
                    }
                    else
#endif
                        rfbWriteExact(&cl, "?", 1);
