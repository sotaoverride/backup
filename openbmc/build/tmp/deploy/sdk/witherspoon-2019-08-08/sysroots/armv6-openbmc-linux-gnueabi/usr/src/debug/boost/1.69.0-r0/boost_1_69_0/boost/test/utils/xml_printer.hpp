...
inline void print_escaped(std::ostream& where_to, const_string value)
{
#if !defined(BOOST_NO_CXX11_HDR_INITIALIZER_LIST) &&                           \
    !defined(BOOST_NO_CXX11_UNIFIED_INITIALIZATION_SYNTAX)
    static std::map<char, char const*> const char_type{{{'<', "lt"},
                                                        {'>', "gt"},
                                                        {'&', "amp"},
                                                        {'\'', "apos"},
                                                        {'"', "quot"}}};
    ...
    {
        std::map<char, char const*>::const_iterator found_ref =
            char_type.find(c);
...
inline void print_escaped(std::ostream& where_to, std::string const& value)
{
...
template <typename T>
inline void print_escaped(std::ostream& where_to, T const& value)
{
    where_to << value;
}
...
inline void print_escaped_cdata(std::ostream& where_to, const_string value)
{
...
template <typename T>
inline std::ostream& operator<<(custom_printer<attr_value> const& p,
                                T const& value)
{
    *p << "=\"";
...
inline std::ostream& operator<<(custom_printer<cdata> const& p,
                                const_string value)
{
