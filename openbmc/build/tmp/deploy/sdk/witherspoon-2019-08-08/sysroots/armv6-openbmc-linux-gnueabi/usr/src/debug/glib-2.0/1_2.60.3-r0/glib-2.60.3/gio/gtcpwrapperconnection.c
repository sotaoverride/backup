...
G_DEFINE_TYPE_WITH_PRIVATE(GTcpWrapperConnection, g_tcp_wrapper_connection,
...
static void g_tcp_wrapper_connection_get_property(GObject* object,
...
static void g_tcp_wrapper_connection_set_property(GObject* object,
                                                  guint prop_id,
                                                  const GValue* value,
                                                  GParamSpec* pspec)
{
    ...
    g_tcp_wrapper_connection_class_init(GTcpWrapperConnectionClass* klass)
{
    ...
    stream_class->get_output_stream =
        g_tcp_wrapper_connection_get_output_stream;

    g_object_class_install_property(
        gobject_class, PROP_BASE_IO_STREAM,
        g_param_spec_object("base-io-stream", P_("Base IO Stream"),
                            P_("The wrapped GIOStream"), G_TYPE_IO_STREAM,
                                                ...
                                                GSocket* socket)
{
    ...
    g_return_val_if_fail(
        g_socket_get_socket_type(socket) == G_SOCKET_TYPE_STREAM, NULL);

    return g_object_new(G_TYPE_TCP_WRAPPER_CONNECTION, "base-io-stream",
                        base_io_stream, "socket", socket, NULL);
}
