...
krb5_error_code krb5_encode_princ_dbkey(krb5_context context, krb5_data* key,
...
krb5_error_code krb5_encode_princ_entry(krb5_context context,
                                        krb5_data* content,
                                        krb5_db_entry* entry)
{
    int i, j;
    ...
    for (i = 0; i < entry->n_key_data; i++)
    {
        ...
        for (j = 0; j < entry->key_data[i].key_data_ver; j++)
        {
            ...
            if (length)
            {
                memcpy(nextloc, entry->key_data[i].key_data_contents[j],
                       length);
                nextloc += length;
            }
        }
...
krb5_error_code krb5_decode_princ_entry(krb5_context context,
                                        krb5_data* content,
    ...
    for (i = 0; i < entry->n_key_data; i++)
    {
            ...
            key_data->key_data_ver <= KRB5_KDB_V1_KEY_DATA_ARRAY)
        {
            for (j = 0; j < key_data->key_data_ver; j++)
            {
                ...
                {
                    key_data->key_data_contents[j] = k5memdup(
                        nextloc, key_data->key_data_length[j], &retval);
