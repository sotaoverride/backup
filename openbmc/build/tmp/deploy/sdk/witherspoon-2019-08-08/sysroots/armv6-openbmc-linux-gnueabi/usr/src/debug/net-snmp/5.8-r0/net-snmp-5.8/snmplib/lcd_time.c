    ...
    *engine_time = *engineboot = 0;
    ...
    if (!authenticated || e->authenticatedFlag)
    {
        ...
        *engineboot = e->engineBoot;
        ...
        timediff =
            (int)(snmpv3_local_snmpEngineTime() - e->lastReceivedEngineTime);
 ...
 *            this is another matter.
 */
int get_enginetime_ex(u_char* engineID, u_int engineID_len, u_int* engineboot,
                      u_int* engine_time, u_int* last_engine_time,
                      u_int authenticated)
{
    int rval = SNMPERR_SUCCESS;
    int timediff = 0;
 ...
 */
int set_enginetime(const u_char* engineID, u_int engineID_len, u_int engineboot,
...
int hash_engineID(const u_char* engineID, u_int engineID_len)
{
    ...
    rval = sc_hash(usmHMACMD5AuthProtocol,
                   sizeof(usmHMACMD5AuthProtocol) / sizeof(oid), engineID,
                   engineID_len, buf, &buf_len);
    if (rval == SNMPERR_SC_NOT_CONFIGURED)
    {
        /* fall back to sha1 */
        rval = sc_hash(usmHMACSHA1AuthProtocol,
                       sizeof(usmHMACSHA1AuthProtocol) / sizeof(oid), engineID,
                       engineID_len, buf, &buf_len);
    }
#else
    rval = sc_hash(usmHMACSHA1AuthProtocol,
...
void dump_etimelist_entry(Enginetime e, int count)
{
    ...
    DEBUGMSGTL(("dump_etimelist", "%s%s (len=%d) <%d,%d>\n", tabs, s,
                e->engineID_len, e->engineTime, e->engineBoot));
