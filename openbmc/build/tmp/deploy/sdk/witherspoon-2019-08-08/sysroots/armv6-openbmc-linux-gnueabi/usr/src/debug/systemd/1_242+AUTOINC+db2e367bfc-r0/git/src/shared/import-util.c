...
int import_url_change_last_component(const char* url, const char* suffix,
                                     char** ret)
{
    const char* e;
    char* s;
...
int import_assign_pool_quota_and_warn(const char* path)
{
    ...
    if (r == -ENOTTY)
    {
        log_debug_errno(
            r,
            "Failed to set up default quota hierarchy for /var/lib/machines, "
            "as directory is not on btrfs or not a subvolume. Ignoring.");
        return 0;
    }
    if (r < 0)
        return log_error_errno(r, "Failed to set up default quota hierarchy "
                                  "for /var/lib/machines: %m");
    if (r > 0)
    ...
    {
        log_debug_errno(r,
                        "Failed to set up quota hierarchy for %s, as directory "
                        "is not on btrfs or not a subvolume. Ignoring.",
                        path);
        return 0;
    }
    if (r < 0)
        return log_error_errno(
            r, "Failed to set up default quota hierarchy for %s: %m", path);
    if (r > 0)
