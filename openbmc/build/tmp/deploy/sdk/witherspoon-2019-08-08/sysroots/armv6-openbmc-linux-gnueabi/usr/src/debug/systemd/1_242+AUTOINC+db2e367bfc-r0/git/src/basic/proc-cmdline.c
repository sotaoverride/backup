...
int proc_cmdline(char** ret)
{
    ...
    /* For testing purposes it is sometimes useful to be able to override what
     * we consider /proc/cmdline to be */
...
static int proc_cmdline_extract_first(const char** p, char** ret_word,
                                      ProcCmdlineFlags flags)
{
    ...
    for (;;)
    {
        ...
        if (c)
        {
            ...
            continue; /* And optionally filter out arguments that are intended
                         only for the host */
...
int proc_cmdline_parse_given(const char* line, proc_cmdline_parse_t parse_item,
                             void* data, ProcCmdlineFlags flags)
{
    const char* p;
    int r;
    ...
    /* The PROC_CMDLINE_VALUE_OPTIONAL flag doesn't really make sense for
...
int proc_cmdline_parse(proc_cmdline_parse_t parse_item, void* data,
                       ProcCmdlineFlags flags)
{
...
int proc_cmdline_get_key(const char* key, ProcCmdlineFlags flags,
                         char** ret_value)
{
    ...
    /* Looks for a specific key on the kernel command line. Supports three
     * modes:
     *
     * a) The "ret_value" parameter is used. In this case a parameter beginning
     * with the "key" string followed by
     *    "=" is searched for, and the value following it is returned in
     * "ret_value".
     *
     * b) as above, but the PROC_CMDLINE_VALUE_OPTIONAL flag is set. In this
     * case if the key is found as a separate word (i.e. not followed by "=" but
     * instead by whitespace or the end of the command line), then this is also
     * accepted, and "value" is returned as NULL.
     *
     * c) The "ret_value" parameter is NULL. In this case a search for the exact
     * "key" parameter is performed.
     *
...
int proc_cmdline_get_key_many_internal(ProcCmdlineFlags flags, ...)
{
    ...
    /* The PROC_CMDLINE_VALUE_OPTIONAL flag doesn't really make sense for
     * proc_cmdline_get_key_many(), let's make this clear. */
    assert(!FLAGS_SET(flags, PROC_CMDLINE_VALUE_OPTIONAL));
...
static const char* const rlmap_initrd[] = {"emergency",
                                           SPECIAL_EMERGENCY_TARGET, "rescue",
                                           SPECIAL_RESCUE_TARGET, NULL};
