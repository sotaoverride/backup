...
static krb5_error_code kg_oid_externalize(kcontext, arg, buffer,
                                          lenremain) krb5_context kcontext;
krb5_pointer arg;
krb5_octet** buffer;
size_t* lenremain;
{
    ...
    if (err)
        return err;
    err = krb5_ser_pack_int32((krb5_int32)oid->length, buffer, lenremain);
    if (err)
        return err;
    err = krb5_ser_pack_bytes((krb5_octet*)oid->elements, oid->length, buffer,
                              lenremain);
    if (err)
        return err;
    err = krb5_ser_pack_int32(KV5M_GSS_OID, buffer, lenremain);
...
static krb5_error_code kg_oid_internalize(kcontext, argp, buffer,
                                          lenremain) krb5_context kcontext;
krb5_pointer* argp;
krb5_octet** buffer;
size_t* lenremain;
{
    ...
    if (krb5_ser_unpack_bytes((krb5_octet*)oid->elements, oid->length, &bp,
                              &remain))
    {
        free(oid->elements);
        free(oid);
...
static krb5_error_code kg_oid_size(kcontext, arg, sizep) krb5_context kcontext;
krb5_pointer arg;
size_t* sizep;
{
...
static krb5_error_code kg_seqstate_externalize(kcontext, arg, buffer,
                                               lenremain) krb5_context kcontext;
g_seqnum_state arg;
krb5_octet** buffer;
size_t* lenremain;
...
static krb5_error_code kg_seqstate_internalize(kcontext, argp, buffer,
                                               lenremain) krb5_context kcontext;
g_seqnum_state* argp;
krb5_octet** buffer;
size_t* lenremain;
...
static krb5_error_code kg_seqstate_size(kcontext, arg,
                                        sizep) krb5_context kcontext;
g_seqnum_state arg;
size_t* sizep;
{
    krb5_error_code kret;
 ...
 */
krb5_error_code kg_ctx_size(kcontext, arg, sizep) krb5_context kcontext;
krb5_pointer arg;
size_t* sizep;
{
    krb5_error_code kret;
    ...
    if ((ctx = (krb5_gss_ctx_id_rec*)arg))
    {
        ...
        if (!kret && ctx->here)
            kret = krb5_size_opaque(kcontext, KV5M_PRINCIPAL,
                                    (krb5_pointer)ctx->here->princ, &required);

        if (!kret && ctx->there)
            kret = krb5_size_opaque(kcontext, KV5M_PRINCIPAL,
                                    (krb5_pointer)ctx->there->princ, &required);
        ...
        if (!kret && ctx->enc)
            kret =
                krb5_size_opaque(kcontext, KV5M_KEYBLOCK,
                                 (krb5_pointer)&ctx->enc->keyblock, &required);

        if (!kret && ctx->seq)
            kret =
                krb5_size_opaque(kcontext, KV5M_KEYBLOCK,
                                 (krb5_pointer)&ctx->seq->keyblock, &required);

        if (!kret)
            kret =
                kg_oid_size(kcontext, (krb5_pointer)ctx->mech_used, &required);
        ...
        if (!kret)
            kret = krb5_size_opaque(kcontext, KV5M_CONTEXT,
                                    (krb5_pointer)ctx->k5_context, &required);
        if (!kret)
            kret = krb5_size_opaque(kcontext, KV5M_AUTH_CONTEXT,
                                    (krb5_pointer)ctx->auth_context, &required);
        if (!kret && ctx->acceptor_subkey)
            kret = krb5_size_opaque(
                kcontext, KV5M_KEYBLOCK,
                (krb5_pointer)&ctx->acceptor_subkey->keyblock, &required);
        if (!kret && ctx->authdata)
        {
            ...
            {
                kret =
                    krb5_size_opaque(kcontext, KV5M_AUTHDATA,
                                     (krb5_pointer)ctx->authdata[i], &required);
            }
        }
        if (!kret)
        {
            ...
            if (initiator_name && initiator_name->ad_context)
            {
                kret = krb5_size_opaque(kcontext, KV5M_AUTHDATA_CONTEXT,
                                        initiator_name->ad_context, &required);
            }
        }
        *sizep += required;
    }
 ...
 */
krb5_error_code kg_ctx_externalize(kcontext, arg, buffer,
                                   lenremain) krb5_context kcontext;
krb5_pointer arg;
krb5_octet** buffer;
size_t* lenremain;
{
    ...
    {
        kret = ENOMEM;
        if (!kg_ctx_size(kcontext, arg, &required) && (required <= remain))
        {
            /* Our identifier */
            (void)krb5_ser_pack_int32(KG_CONTEXT, &bp, &remain);

            /* Now static data */
            (void)krb5_ser_pack_int32((krb5_int32)ctx->initiate, &bp, &remain);
            (void)krb5_ser_pack_int32((krb5_int32)ctx->established, &bp,
                                      &remain);
            (void)krb5_ser_pack_int32((krb5_int32)ctx->have_acceptor_subkey,
                                      &bp, &remain);
            (void)krb5_ser_pack_int32((krb5_int32)ctx->seed_init, &bp, &remain);
            (void)krb5_ser_pack_int32((krb5_int32)ctx->gss_flags, &bp, &remain);
            (void)krb5_ser_pack_bytes((krb5_octet*)ctx->seed, sizeof(ctx->seed),
                                      &bp, &remain);
            (void)krb5_ser_pack_int32((krb5_int32)ctx->signalg, &bp, &remain);
            (void)krb5_ser_pack_int32((krb5_int32)ctx->cksum_size, &bp,
                                      &remain);
            (void)krb5_ser_pack_int32((krb5_int32)ctx->sealalg, &bp, &remain);
            (void)krb5_ser_pack_int32((krb5_int32)ctx->krb_times.authtime, &bp,
                                      &remain);
            (void)krb5_ser_pack_int32((krb5_int32)ctx->krb_times.starttime, &bp,
                                      &remain);
            (void)krb5_ser_pack_int32((krb5_int32)ctx->krb_times.endtime, &bp,
                                      &remain);
            (void)krb5_ser_pack_int32((krb5_int32)ctx->krb_times.renew_till,
                                      &bp, &remain);
            (void)krb5_ser_pack_int32((krb5_int32)ctx->krb_flags, &bp, &remain);
            (void)(*kaccess.ser_pack_int64)((int64_t)ctx->seq_send, &bp,
                                            &remain);
            (void)(*kaccess.ser_pack_int64)((int64_t)ctx->seq_recv, &bp,
                                            &remain);
            ...
            if (!kret && ctx->mech_used)
                kret =
                    kg_oid_externalize(kcontext, ctx->mech_used, &bp, &remain);
            ...
            if (!kret && ctx->subkey)
                kret = krb5_externalize_opaque(
                    kcontext, KV5M_KEYBLOCK,
                    (krb5_pointer)&ctx->subkey->keyblock, &bp, &remain);

            if (!kret && ctx->enc)
                kret = krb5_externalize_opaque(
                    kcontext, KV5M_KEYBLOCK, (krb5_pointer)&ctx->enc->keyblock,
                    &bp, &remain);

            if (!kret && ctx->seq)
                kret = krb5_externalize_opaque(
                    kcontext, KV5M_KEYBLOCK, (krb5_pointer)&ctx->seq->keyblock,
                    &bp, &remain);

            if (!kret && ctx->seqstate)
                kret = kg_seqstate_externalize(kcontext, ctx->seqstate, &bp,
                                               &remain);
            ...
            if (!kret)
                kret =
                    krb5_ser_pack_int32((krb5_int32)ctx->proto, &bp, &remain);
            if (!kret)
                kret = krb5_ser_pack_int32((krb5_int32)ctx->cksumtype, &bp,
                                           &remain);
            if (!kret && ctx->acceptor_subkey)
                kret = krb5_externalize_opaque(
                    kcontext, KV5M_KEYBLOCK,
                    (krb5_pointer)&ctx->acceptor_subkey->keyblock, &bp,
                    &remain);
            if (!kret)
                kret = krb5_ser_pack_int32(
                    (krb5_int32)ctx->acceptor_subkey_cksumtype, &bp, &remain);

            if (!kret)
                kret = krb5_ser_pack_int32((krb5_int32)ctx->cred_rcache, &bp,
                                           &remain);
            if (!kret)
            {
                krb5_int32 i = 0;
                ...
                if (!kret && ctx->authdata)
                {
                    /* authdata */
                    for (i = 0; !kret && ctx->authdata[i]; i++)
                        kret = krb5_externalize_opaque(kcontext, KV5M_AUTHDATA,
                                                       ctx->authdata[i], &bp,
                                                       &remain);
                }
            }
            ...
            if (!kret)
            {
                ...
                {
                    kret = krb5_externalize_opaque(
                        kcontext, KV5M_AUTHDATA_CONTEXT,
                        initiator_name->ad_context, &bp, &remain);
                }
            }
            /* trailer */
            if (!kret)
...
static krb5_error_code intern_key(krb5_context ctx, krb5_key* key,
                                  krb5_octet** bp, size_t* sp)
{
    krb5_keyblock* keyblock;
    krb5_error_code ret;

    ret = krb5_internalize_opaque(ctx, KV5M_KEYBLOCK, (krb5_pointer*)&keyblock,
                                  bp, sp);
    if (ret != 0)
        return ret;
 ...
 */
krb5_error_code kg_ctx_internalize(kcontext, argp, buffer,
                                   lenremain) krb5_context kcontext;
krb5_pointer* argp;
krb5_octet** buffer;
size_t* lenremain;
{
    ...
    if (ibuf == KG_CONTEXT)
    {
        ...
        /* Get a context */
        if ((remain >= (17 * sizeof(krb5_int32) + 2 * sizeof(int64_t) +
                        sizeof(ctx->seed))) &&
            (ctx = (krb5_gss_ctx_id_rec*)xmalloc(sizeof(krb5_gss_ctx_id_rec))))
        {
            ...
            (void)(*kaccess.ser_unpack_int64)((int64_t*)&ctx->seq_send, &bp,
                                              &remain);
            kret = (*kaccess.ser_unpack_int64)((int64_t*)&ctx->seq_recv, &bp,
                                               &remain);
            if (kret)
            {
                free(ctx);
            ...
            {
                krb5_pointer tmp;
                kret = kg_oid_internalize(kcontext, &tmp, &bp, &remain);
                if (kret == 0)
                    ctx->mech_used = tmp;
                else if (kret == EINVAL)
                    kret = 0;
            ...
            kret = krb5_internalize_opaque(kcontext, KV5M_PRINCIPAL,
                                           (krb5_pointer*)&princ, &bp, &remain);
            if (kret == 0)
            {
            ...
            if (!kret && (kret = intern_key(kcontext, &ctx->enc, &bp, &remain)))
            {
                if (kret == EINVAL)
                    kret = 0;
            }
            if (!kret && (kret = intern_key(kcontext, &ctx->seq, &bp, &remain)))
            {
                if (kret == EINVAL)
                    kret = 0;
            }
            ...
            if (!kret)
            {
                kret = kg_seqstate_internalize(kcontext, &ctx->seqstate, &bp,
                                               &remain);
                if (kret == EINVAL)
                    kret = 0;
            }
            ...
            if (!kret)
                kret = krb5_internalize_opaque(kcontext, KV5M_CONTEXT,
                                               (krb5_pointer*)&ctx->k5_context,
                                               &bp, &remain);

            if (!kret)
                kret = krb5_internalize_opaque(
                    kcontext, KV5M_AUTH_CONTEXT,
                    (krb5_pointer*)&ctx->auth_context, &bp, &remain);
            ...
            if (!kret)
                kret = krb5_ser_unpack_int32(&ibuf, &bp, &remain);
            ctx->cksumtype = ibuf;
            if (!kret && (kret = intern_key(kcontext, &ctx->acceptor_subkey,
                                            &bp, &remain)))
            {
                if (kret == EINVAL)
                    kret = 0;
            ...
            if (!kret)
            {
                ...
                if (nadata > 0)
                {
                    ctx->authdata = (krb5_authdata**)calloc(
                        (size_t)nadata + 1, sizeof(krb5_authdata*));
                    ...
                    else
                    {
                        for (i = 0; !kret && i < nadata; i++)
                            kret = krb5_internalize_opaque(
                                kcontext, KV5M_AUTHDATA,
                                (krb5_pointer*)&ctx->authdata[i], &bp, &remain);
                    }
                }
            }
            /* authdata context */
            if (!kret)
            {
                ...
                else
                {
                    kret = krb5_internalize_opaque(
                        kcontext, KV5M_AUTHDATA_CONTEXT,
                        (krb5_pointer*)&initiator_name->ad_context, &bp,
                        &remain);
                    if (kret == EINVAL)
                        kret = 0;
                }
