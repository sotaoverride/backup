...
class copy_map : private noncopyable
{
  public:
    ...
    copy_map(const Allocator& al, std::size_t size, Node* header_org,
             Node* header_cpy) :
        al_(al),
        size_(size), spc(al_, size_), n(0), header_org_(header_org),
        header_cpy_(header_cpy), released(false)
    {
    }
    ...
    void clone(Node* node)
    {
        ...
        BOOST_TRY
        {
            boost::detail::allocator::construct(
                boost::addressof((spc.data() + n)->second->value()),
                node->value());
        }
        BOOST_CATCH(...)
        {
    ...
    Node* find(Node* node) const
    {
        if (node == header_org_)
            return header_cpy_;
        return std::lower_bound(begin(), end(), copy_map_entry<Node>(node, 0))
            ->second;
    }
  ...
  private:
    typedef typename boost::detail::allocator::rebind_to<Allocator, Node>::type
        allocator_type;
...
#else
        allocator_traits::deallocate(al_, static_cast<allocator_pointer>(node),
                                     1);
#endif
    }
};
