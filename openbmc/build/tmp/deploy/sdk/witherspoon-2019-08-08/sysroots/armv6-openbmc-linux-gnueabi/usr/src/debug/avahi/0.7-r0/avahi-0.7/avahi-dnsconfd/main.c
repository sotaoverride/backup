...
static enum { ACKWAIT, BROWSING } state = ACKWAIT;
...
static DNSServerInfo* get_server_info(AvahiIfIndex interface,
                                      AvahiProtocol protocol,
                                      const char* address)
{
    DNSServerInfo* i;
...
static DNSServerInfo* new_server_info(AvahiIfIndex interface,
                                      AvahiProtocol protocol,
                                      const char* address)
{
    DNSServerInfo* i;
...
static void set_env(const char* name, const char* value)
{
    ...
    for (e = environ; *e; e++)
    {
        ...
        /* We simply free the record, sicne we know that we created it
         * previously */
        avahi_free(*e);
...
static void run_script(int new, AvahiIfIndex interface, AvahiProtocol protocol,
                       const char* address)
{
    char* p;
    int ret;
    ...
    if (daemon_exec("/", &ret, AVAHI_DNSCONF_SCRIPT, AVAHI_DNSCONF_SCRIPT,
                    new ? "+" : "-", address, ia, pa,
                    avahi_proto_to_string(protocol), NULL) < 0)
        daemon_log(LOG_WARNING, "Failed to run script");
    else if (ret != 0)
        daemon_log(LOG_WARNING, "Script returned with non-zero exit code %i",
                   ret);
}
...
static int new_line(const char* l)
{
    ...
    else
    {
        ...
        if (*l != '<' && *l != '>')
        {
            daemon_log(LOG_ERR, "Avahi sent us an invalid browsing line: %s",
                       l);
            return -1;
        }

        if (sscanf(l + 1, "%i %i %39s %i", &i_interface, &i_protocol, a,
                   &port) != 4)
        {
        ...
        if (*l == '>')
        {
            if (port != 53)
                daemon_log(
                    LOG_WARNING,
                    "DNS server with port address != 53 found, ignoring");
            else
            {
                daemon_log(LOG_INFO, "New DNS Server %s (interface: %i.%s)", a,
                           interface, avahi_proto_to_string(protocol));
        ...
        else
        {
            ...
            if (port == 53)
                ...
                {
                    daemon_log(LOG_INFO,
                               "DNS Server %s removed (interface: %i.%s)", a,
                               interface, avahi_proto_to_string(protocol));
                    server_info_free(i);
...
static void help(FILE* f, const char* argv0)
{
    fprintf(f,
            ...
            "    -r --refresh     Request a running daemon to refresh DNS "
            "server data\n"
