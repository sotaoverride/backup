...
#define SPACES                                                                 \
    ...
    {
        snmp_log(LOG_ERR,
                 "Too deep indentation for debug_indent. "
...
NETSNMP_IMPORT void debug_config_register_tokens(const char* configtoken,
                                                 char* tokens);
...
NETSNMP_IMPORT void debug_config_turn_on_debugging(const char* configtoken,
                                                   char* line);
...
static void debug_config_debug_log_level(const char* configtoken, char* line)
{
    ...
    static const struct strval_s
    {
        ...
        {"notice", LOG_NOTICE}, {"warning", LOG_WARNING}, {NULL, 0}};
#endif
    int i = 0, len_l, len_p;
...
void debug_print_registered_tokens(void)
{
    ...
    {
        snmp_log(LOG_INFO, "%d) %s : %d\n", i, dbg_tokens[i].token_name,
                 dbg_tokens[i].enabled);
    }
}
...
void debugmsg_suboid(const char* token, const oid* theoid, size_t len)
{
    u_char* buf = NULL;
    size_t buf_len = 0, out_len = 0;
    int buf_overflow = 0;

    netsnmp_sprint_realloc_objid(&buf, &buf_len, &out_len, 1, &buf_overflow,
                                 theoid, len);
    if (buf_overflow)
    {
        if (buf != NULL)
...
void debugmsg_var(const char* token, netsnmp_variable_list* var)
{
    ...
    if (sprint_realloc_variable(&buf, &buf_len, &out_len, 1, var->name,
                                var->name_length, var))
    {
        if (buf != NULL)
        {
...
void debugmsg_hextli(const char* token, const u_char* thedata, size_t len)
{
    ...
    DEBUGIF(token2)
    {
        for (incr = 16; len > 0; len -= incr, thedata += incr)
        {
            ...
            if (sprint_realloc_hexstring(&b3, &b3_len, &o3_len, 1, thedata,
                                         incr))
            {
                if (b3 != NULL)
                {
 ...
 */
void snmp_set_do_debugging(int val)
{
    dodebug = val;
}
...
#ifndef NETSNMP_DISABLE_DYNAMIC_LOG_LEVEL
static void debug_config_debug_log_level(const char* configtoken
...
NETSNMP_IMPORT int netsnmp_get_debug_log_level(void)
{
    return 0;
}
#endif /* NETSNMP_DISABLE_DYNAMIC_LOG_LEVEL */
...
void debug_config_turn_on_debugging(const char* configtokenu, char* line)
{
}
...
void snmp_debug_init(void)
{
    register_prenetsnmp_mib_handler(
        "snmp", "doDebugging", debug_config_turn_on_debugging, NULL, "(1|0)");
    register_prenetsnmp_mib_handler("snmp", "debugTokens",
                                    debug_config_register_tokens, NULL,
                                    "token[,token...]");
#ifndef NETSNMP_DISABLE_DYNAMIC_LOG_LEVEL
    register_prenetsnmp_mib_handler(
        "snmp", "debugLogLevel", debug_config_debug_log_level, NULL,
