...
static int add_cryptsetup(const char* id, const char* what, bool rw,
                          bool require, char** device)
{
    _cleanup_free_ char *e = NULL, *n = NULL, *d = NULL, *id_escaped = NULL,
                        *what_escaped = NULL;
    ...
    fprintf(f,
            ...
            "KeyringMode=shared\n" /* make sure we can share cached keys among
                                      instances */
            "ExecStart=" SYSTEMD_CRYPTSETUP_PATH " attach '%s' '%s' '' '%s'\n"
            "ExecStop=" SYSTEMD_CRYPTSETUP_PATH " detach '%s'\n",
            d, d, id_escaped, what_escaped, rw ? "" : "read-only", id_escaped);
    ...
    p = strjoina(arg_dest, "/dev-mapper-", e,
...
static int add_mount(const char* id, const char* what, const char* where,
                     const char* fstype, bool rw, const char* options,
                     const char* description, const char* post)
{
    ...
    /* Note that we don't apply specifier escaping on the input strings here,
     * since we know they are not configured externally, but all originate from
     * our own sources here, and hence we know they contain no % characters that
...
static int path_is_busy(const char* where)
{
    ...
    if (r < 0)
        return log_warning_errno(
            r, "Cannot check if \"%s\" is a mount point: %m", where);

    /* not a mountpoint but it contains files */
    r = dir_is_empty(where);
    if (r < 0)
        return log_warning_errno(r, "Cannot check if \"%s\" is empty: %m",
                                 where);
    if (r > 0)
        return false;
...
static int add_partition_mount(DissectedPartition* p, const char* id,
                               const char* where, const char* description)
{
    ...
    return add_mount(id, p->node, where, p->fstype, p->rw, NULL, description,
...
static int add_automount(const char* id, const char* what, const char* where,
                         const char* fstype, bool rw, const char* options,
                         const char* description, usec_t timeout)
{
    ...
    r = add_mount(id, what, where, fstype, rw, opt, description, NULL);
    if (r < 0)
        return r;
    ...
    fprintf(f,
            ...
            description, where, timeout / USEC_PER_SEC);

    r = fflush_and_check(f);
    if (r < 0)
        return log_error_errno(r, "Failed to write unit file %s: %m", p);

    return generator_add_symlink(arg_dest, SPECIAL_LOCAL_FS_TARGET, "wants",
                                 unit);
}
...
static int add_xbootldr(DissectedPartition* p)
{
    ...
    return add_automount("boot", p->node, "/boot", p->fstype, true,
                         "umask=0077", "Boot Loader Partition",
                         120 * USEC_PER_SEC);
}
...
static int add_esp(DissectedPartition* p, bool has_xbootldr)
{
    ...
    /* If /efi exists we'll use that. Otherwise we'll use /boot, as that's
     * usually the better choice, but only if there's no explicit XBOOTLDR
     * partition around. */
    if (access("/efi", F_OK) < 0)
    {
        if (errno != ENOENT)
            return log_error_errno(
                errno, "Failed to determine whether /efi exists: %m");
    ...
    /* We create an .automount which is not overridden by the .mount from the
     * fstab generator. */
    ...
    if (is_efi_boot())
    {
        ...
        /* If this is an EFI boot, be extra careful, and only mount the ESP if
         * it was the ESP used for booting. */
        ...
        {
            log_debug("Partition for %s does not appear to be the partition we "
                      "are booted from.",
                      p->node);
            return 0;
        }
    ...
    return add_automount(id, p->node, esp_path, p->fstype, true, "umask=0077",
                         "EFI System Partition Automount", 120 * USEC_PER_SEC);
}
#else
static int add_esp(DissectedPartition* p, bool has_xbootldr)
{
...
static int add_root_rw(DissectedPartition* p)
{
    ...
    if (in_initrd())
    {
        log_debug("In initrd, not generating drop-in for "
    ...
    if (arg_root_rw >= 0)
    {
        log_debug("Parameter ro/rw specified on kernel command line, not "
                  "generating drop-in for systemd-remount-fs.service.");
        return 0;
    }

    if (!p->rw)
    {
        log_debug("Root partition marked read-only in GPT partition table, not "
                  "generating drop-in for systemd-remount-fs.service.");
        return 0;
    }
    ...
    path =
        strjoina(arg_dest, "/systemd-remount-fs.service.d/50-remount-rw.conf");
...
static int open_parent_devno(dev_t devnum, int* ret)
{
    ...
    if (sd_device_get_devname(d, &name) < 0)
    {
        ...
        if (r < 0)
        {
            log_device_debug_errno(
                d, r, "Device %u:%u does not have a name, ignoring: %m",
    ...
    {
        log_device_debug_errno(
            parent, r, "Parent doesn't have a device type, ignoring: %m");
        return 0;
    }
    ...
    {
        log_device_debug_errno(
            parent, r, "Parent device does not have device node, ignoring: %m");
        return 0;
    }
...
static int enumerate_partitions(dev_t devnum)
{
    ...
    r = dissect_image(fd, NULL, 0,
                      DISSECT_IMAGE_GPT_ONLY | DISSECT_IMAGE_NO_UDEV, &m);
    if (r == -ENOPKG)
    {
    ...
    if (m->partitions[PARTITION_ESP].found)
    {
        k = add_esp(m->partitions + PARTITION_ESP,
                    m->partitions[PARTITION_XBOOTLDR].found);
        if (k < 0)
            r = k;
    }
    ...
    {
        k = add_partition_mount(m->partitions + PARTITION_HOME, "home", "/home",
                                "Home Partition");
        if (k < 0)
            r = k;
    }
    ...
    {
        k = add_partition_mount(m->partitions + PARTITION_SRV, "srv", "/srv",
                                   ...
                                   void* data)
{
        ...
        proc_cmdline_key_streq(key, "rd.systemd.gpt_auto"))
    {
        ...
        if (r < 0)
            log_warning_errno(
                r, "Failed to parse gpt-auto switch \"%s\", ignoring: %m",
                value);
        else
            arg_enabled = r;
    }
    ...
    else if (proc_cmdline_key_streq(key, "roothash"))
    {
        ...
        /* Disable root disk logic if there's roothash= defined (i.e. verity
         * enabled) */
...
static int add_root_mount(void)
{
    ...
    if (in_initrd())
    {
        r = generator_write_initrd_root_device_deps(arg_dest,
                                                    "/dev/gpt-auto-root");
        if (r < 0)
            return 0;
    ...
    return add_mount(
        "root", "/dev/gpt-auto-root", in_initrd() ? "/sysroot" : "/", NULL,
        arg_root_rw > 0, NULL, "Root Partition",
...
static int add_mounts(void)
{
    ...
    if (r < 0)
        return log_error_errno(
            r, "Failed to determine block device of root file system: %m");
    if (r == 0)
    {
        r = get_block_device_harder("/usr", &devno);
        if (r < 0)
            return log_error_errno(
                r, "Failed to determine block device of /usr file system: %m");
        if (r == 0)
        {
            _cleanup_free_ char* p = NULL;
            mode_t m;

            /* If the root mount has been replaced by some form of volatile file
             * system (overlayfs), the original root block device node is
             * symlinked in /run/systemd/volatile-root. Let's read that here. */
            r = readlink_malloc("/run/systemd/volatile-root", &p);
            if (r == -ENOENT)
            {
                log_debug("Neither root nor /usr file system are on a (single) "
                          "block device.");
                return 0;
            }
            if (r < 0)
                return log_error_errno(
                    r, "Failed to read symlink /run/systemd/volatile-root: %m");

            r = device_path_parse_major_minor(p, &m, &devno);
            if (r < 0)
                return log_error_errno(
                    r, "Failed to parse major/minor device node: %m");
            if (!S_ISBLK(m))
                return log_error_errno(
