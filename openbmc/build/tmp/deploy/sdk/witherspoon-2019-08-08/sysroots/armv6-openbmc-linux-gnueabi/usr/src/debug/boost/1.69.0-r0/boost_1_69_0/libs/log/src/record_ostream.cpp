...
BOOST_LOG_API void basic_record_ostream<CharT>::init_stream()
{
    ...
    if (m_record)
    {
        ...
        intrusive_ptr<message_impl_type> p =
            new message_impl_type(string_type());
        ...
        std::pair<attribute_value_set::const_iterator, bool> res =
            m_record->attribute_values().insert(
                expressions::tag::message::get_name(), value);
        if (!res.second)
...
template <typename CharT>
BOOST_LOG_API void
    basic_record_ostream<CharT>::detach_from_record() BOOST_NOEXCEPT
{
    if (m_record)
    {
        base_type::detach();
...
#endif
        //! Singleton base type
        typedef log::aux::lazy_singleton<this_type, tls_ptr_type> base_type;
        //! Stream compound type
        typedef
            typename stream_provider<CharT>::stream_compound stream_compound_t;
...
{
    stream_compound_pool<char_type>& pool =
        stream_compound_pool<char_type>::get();
    if (pool.m_Top)
    {
...
template <typename CharT>
BOOST_LOG_API void stream_provider<CharT>::release_compound(
    stream_compound* compound) BOOST_NOEXCEPT
{
    stream_compound_pool<char_type>& pool =
        stream_compound_pool<char_type>::get();
