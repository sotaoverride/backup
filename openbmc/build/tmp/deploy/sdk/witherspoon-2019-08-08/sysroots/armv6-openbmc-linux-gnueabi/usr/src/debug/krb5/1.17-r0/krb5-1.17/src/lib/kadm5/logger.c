...
static struct log_control log_control = {(struct log_entry*)NULL, 0,
...
static void klog_com_err_proc(const char* whoami, long int code,
 ...
 */
krb5_error_code krb5_klog_init(krb5_context kcontext, char* ename, char* whoami,
                               krb5_boolean do_com_err)
{
    ...
    if (!profile_get_values(kcontext->profile, logging_profent,
                            &logging_specs) ||
        !profile_get_values(kcontext->profile, logging_defent, &logging_specs))
    {
        /*
         ...
         */
        log_control.log_entries = (struct log_entry*)malloc(
        ...
        if (log_control.log_entries)
        {
            ...
            for (i = 0; i < log_control.log_nentries; i++)
            {
                ...
                *cp2 = '\0';
                ...
                else if (!strncasecmp(cp, "SYSLOG", 6))
                {
                    ...
                    if (cp[6] == ':')
                    {
                        ...
                        if (cp2)
                        {
                            ...
                            for (j = 0;
                                 j < sizeof(facilities) / sizeof(facilities[0]);
                                 j++)
                ...
                {
                    log_control.log_entries[i].ldu_filep = CONSOLE_OPEN("a+");
...
void krb5_klog_set_context(krb5_context kcontext)
{
    err_context = kcontext;
}
 ...
 */
void krb5_klog_close(krb5_context kcontext)
{
    int lindex;
 ...
 */
static const char* severity2string(int severity)
{
    int s;
    const char* ss;
 ...
 *                        by krb5_klog_init().
 */
static int klog_vsyslog(int priority, const char* format, va_list arglist)
#if !defined(__cplusplus) && (__GNUC__ > 2)
...
static int klog_vsyslog(int priority, const char* format, va_list arglist)
{
    ...
    snprintf(cp, sizeof(outbuf) - (cp - outbuf), " %s %s[%ld](%s): ",
             ...
             (long)getpid(), severity2string(priority));
#else
...
int krb5_klog_syslog(int priority, const char* format, ...)
{
    int retval;
    va_list pvar;
 ...
 */
void krb5_klog_reopen(krb5_context kcontext)
{
    int lindex;
    FILE* f;
