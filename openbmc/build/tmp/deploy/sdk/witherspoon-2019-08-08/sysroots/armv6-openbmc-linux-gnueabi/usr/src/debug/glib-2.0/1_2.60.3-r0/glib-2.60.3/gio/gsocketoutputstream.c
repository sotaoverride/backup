...
static void g_socket_output_stream_pollable_iface_init(
    GPollableOutputStreamInterface* iface);
#ifdef G_OS_UNIX
static void g_socket_output_stream_file_descriptor_based_iface_init(
...
G_DEFINE_TYPE_WITH_CODE(
    GSocketOutputStream, g_socket_output_stream, G_TYPE_OUTPUT_STREAM,
...
static void g_socket_output_stream_get_property(GObject* object, guint prop_id,
                                                GValue* value,
                                                GParamSpec* pspec)
{
...
static void g_socket_output_stream_set_property(GObject* object, guint prop_id,
                                                const GValue* value,
                                                GParamSpec* pspec)
{
    GSocketOutputStream* stream = G_SOCKET_OUTPUT_STREAM(object);
                                           ...
                                           GError** error)
{
    ...
    return g_socket_send_with_blocking(output_stream->priv->socket, buffer,
                                       count, TRUE, cancellable, error);
}

static gboolean g_socket_output_stream_writev(
    ...
    gsize* bytes_written, GCancellable* cancellable, GError** error)
{
    ...
    res = g_socket_send_message_with_timeout(
        output_stream->priv->socket, NULL, vectors, n_vectors, NULL, 0,
        G_SOCKET_MSG_NONE, -1, bytes_written, cancellable, error);
...
static gssize g_socket_output_stream_pollable_write_nonblocking(
    GPollableOutputStream* pollable, const void* buffer, gsize size,
    GError** error)
{
    GSocketOutputStream* output_stream = G_SOCKET_OUTPUT_STREAM(pollable);

    return g_socket_send_with_blocking(output_stream->priv->socket, buffer,
                                       size, FALSE, NULL, error);
}

static GPollableReturn g_socket_output_stream_pollable_writev_nonblocking(
    GPollableOutputStream* pollable, const GOutputVector* vectors,
    gsize n_vectors, gsize* bytes_written, GError** error)
{
    GSocketOutputStream* output_stream = G_SOCKET_OUTPUT_STREAM(pollable);
    ...
    return g_socket_send_message_with_timeout(
        output_stream->priv->socket, NULL, vectors, n_vectors, NULL, 0,
        G_SOCKET_MSG_NONE, 0, bytes_written, NULL, error);
}

static GSource* g_socket_output_stream_pollable_create_source(
    GPollableOutputStream* pollable, GCancellable* cancellable)
{
    GSocketOutputStream* output_stream = G_SOCKET_OUTPUT_STREAM(pollable);
...
#ifdef G_OS_UNIX
static int g_socket_output_stream_get_fd(GFileDescriptorBased* fd_based)
{
...
static void g_socket_output_stream_class_init(GSocketOutputStreamClass* klass)
{
    ...
    g_object_class_install_property(
        ...
        g_param_spec_object("socket", P_("socket"),
    ...
    GPollableOutputStreamInterface* iface)
{
    ...
    iface->writev_nonblocking =
        g_socket_output_stream_pollable_writev_nonblocking;
}
