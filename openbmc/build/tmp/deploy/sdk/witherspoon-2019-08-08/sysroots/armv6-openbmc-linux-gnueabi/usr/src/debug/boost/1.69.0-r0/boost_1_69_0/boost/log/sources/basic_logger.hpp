 ...
 * \date   08.03.2007
 *
 * The header contains implementation of a base class for loggers. Convenience
 * macros for defining custom loggers are also provided.
 */
...
class basic_logger : public ThreadingModelT
{
  ...
  public:
    ...
    typedef boost::log::aux::exclusive_lock_guard<threading_model>
        add_attribute_lock;
    ...
    typedef boost::log::aux::shared_lock_guard<const threading_model>
        get_attributes_lock;
    //! Lock requirement for the open_record_unlocked method
    typedef boost::log::aux::shared_lock_guard<threading_model>
        open_record_lock;
  ...
  public:
    /*!
     * Constructor. Initializes internal data structures of the basic logger
     * class, acquires reference to the logging core.
     */
    basic_logger() : threading_model(), m_pCore(core::get())
    {
    }
    /*!
     * Copy constructor. Copies all attributes from the source logger.
     *
     * \note Not thread-safe. The source logger must be locked in the final
     * class before copying.
     *
     * \param that Source logger
     */
    basic_logger(basic_logger const& that) :
        threading_model(static_cast<threading_model const&>(that)),
        m_pCore(core::get()), m_Attributes(that.m_Attributes)
    {
    }
    /*!
    ...
    /*!
     * Constructor with named arguments. The constructor ignores all arguments.
     * The result of construction is equivalent to default construction.
     */
    template <typename ArgsT>
    explicit basic_logger(ArgsT const&) :
        threading_model(), m_pCore(core::get())
    {
    }
  ...
  protected:
    ...
    std::pair<attribute_set::iterator, bool>
        add_attribute_unlocked(attribute_name const& name,
    ...
    //! Assignment is closed (should be implemented through copy and swap in the
    //! final class)
 ...
 */
template <typename CharT, typename FinalT, typename ThreadingModelT>
inline void swap(basic_logger<CharT, FinalT, ThreadingModelT>& left,
 ...
 *
 * The composite logger is a helper class that simplifies feature composition
 * into the final logger. The user's logger class is expected to derive from the
 * composite logger class, instantiated with the character type, the user's
 * logger class, the threading model and the list of the required features. The
 * former three parameters are passed to the \c basic_logger class template. The
 * feature list must be an MPL type sequence, where each element is a unary MPL
 * metafunction class, that upon applying on its argument results in a logging
 * feature class that derives from the argument. Every logger feature provided
 * by the library can participate in the feature list.
 */
template <typename CharT, typename FinalT, typename ThreadingModelT,
          typename FeaturesT>
class basic_composite_logger
    : public boost::log::sources::aux::inherit_features<
          basic_logger<CharT, FinalT, ThreadingModelT>, FeaturesT>::type
{
  private:
  ...
  public:
    ...
    basic_composite_logger(basic_composite_logger const& that) :
        base_type((boost::log::aux::shared_lock_guard<const threading_model>(
                       that.get_threading_model()),
    ...
    /*!
     * The method adds an attribute to the source-specific attribute set. The
     * attribute will be implicitly added to every log record made with the
     * current logger.
     *
    ...
    {
        typename base_type::add_attribute_lock lock(
            base_type::get_threading_model());
        return base_type::add_attribute_unlocked(name, attr);
    }
    /*!
     * The method removes an attribute from the source-specific attribute set.
     *
     * \pre The attribute was added with the add_attribute call for this
     * instance of the logger. \post The attribute is no longer registered as a
     * source-specific attribute for this logger. The iterator is invalidated
     * after removal.
     *
    ...
    {
        typename base_type::remove_attribute_lock lock(
            base_type::get_threading_model());
        base_type::remove_attribute_unlocked(it);
    }

    /*!
     * The method removes all attributes from the logger. All iterators and
     * references to the removed attributes are invalidated.
     */
    void remove_all_attributes()
    {
        typename base_type::remove_all_attributes_lock lock(
            base_type::get_threading_model());
    ...
    {
        typename base_type::get_attributes_lock lock(
            base_type::get_threading_model());
        return base_type::get_attributes_unlocked();
    }

    /*!
     * The method installs the whole attribute set into the logger. All
     * iterators and references to elements of the previous set are invalidated.
     * Iterators to the \a attrs set are not valid to be used with the logger
     * (that is, the logger owns a copy of \a attrs after completion).
     *
     * \param attrs The set of attributes to install into the logger. Attributes
     * are shallow-copied.
     */
    void set_attributes(attribute_set const& attrs)
    {
        typename base_type::set_attributes_lock lock(
            base_type::get_threading_model());
     ...
     *
     * \return A valid record handle if the logging record is opened
     * successfully, an invalid handle otherwise.
     */
    record open_record()
    {
        ...
        {
            typename base_type::open_record_lock lock(
                base_type::get_threading_model());
            return base_type::open_record_unlocked(
                boost::log::aux::empty_arg_list());
        }
        else
            return record();
    }
     ...
     * The method opens a new log record in the logging core.
     *
     * \param args A set of additional named arguments. The parameter is
     * ignored. \return A valid record handle if the logging record is opened
     * successfully, an invalid handle otherwise.
     */
    template <typename ArgsT>
    ...
    {
        typename base_type::push_record_lock lock(
            base_type::get_threading_model());
  ...
  protected:
    /*!
     * Assignment for the final class. Threadsafe, provides strong exception
     * guarantee.
     */
    FinalT& assign(FinalT const& that)
    {
        ...
        {
            // We'll have to explicitly create the copy in order to make sure
            // it's unlocked when we attempt to lock *this
            FinalT tmp(that);
            boost::log::aux::exclusive_lock_guard<threading_model> lock(
                base_type::get_threading_model());
...
class basic_composite_logger<CharT, FinalT, single_thread_model, FeaturesT>
    : public boost::log::sources::aux::inherit_features<
          basic_logger<CharT, FinalT, single_thread_model>, FeaturesT>::type
{
  private:
    typedef typename boost::log::sources::aux::inherit_features<
        basic_logger<CharT, FinalT, single_thread_model>, FeaturesT>::type
...
#define BOOST_LOG_FORWARD_LOGGER_CONSTRUCTORS_IMPL(class_type,                 \
  ...
  public:                                                                      \
    ...
    class_type(class_type const& that) :                                       \
        class_type::logger_base(                                               \
            static_cast<typename_keyword() class_type::logger_base const&>(    \
                that))                                                         \
    {                                                                          \
    }                                                                          \
    class_type(BOOST_RV_REF(class_type) that) :                                \
        class_type::logger_base(::boost::move(                                 \
            static_cast<typename_keyword() class_type::logger_base&>(that)))   \
    {                                                                          \
    }                                                                          \
    BOOST_LOG_PARAMETRIZED_CONSTRUCTORS_FORWARD(class_type,                    \
                                                class_type::logger_base)
...
#define BOOST_LOG_FORWARD_LOGGER_CONSTRUCTORS_TEMPLATE(class_type)             \
    BOOST_LOG_FORWARD_LOGGER_CONSTRUCTORS_IMPL(class_type,                     \
...
#define BOOST_LOG_FORWARD_LOGGER_ASSIGNMENT(class_type)                        \
  public:                                                                      \
    ...
    {                                                                          \
        return class_type::logger_base::assign(                                \
            static_cast<class_type const&>(that));                             \
    }                                                                          \
    class_type& operator=(BOOST_RV_REF(class_type) that)                       \
    {                                                                          \
        BOOST_LOG_EXPR_IF_MT(::boost::log::aux::exclusive_lock_guard<          \
                                 class_type::threading_model>                  \
...
#define BOOST_LOG_FORWARD_LOGGER_ASSIGNMENT_TEMPLATE(class_type)               \
  public:                                                                      \
    ...
    {                                                                          \
        BOOST_LOG_EXPR_IF_MT(::boost::log::aux::exclusive_lock_guard<          \
                                 typename class_type::threading_model>         \
                                 lock(this->get_threading_model());)           \
        this->swap_unlocked(that);                                             \
        return *this;                                                          \
    }
...
/*!
 *  \brief The macro declares a logger class that inherits a number of base
 * classes
 *
 *  \param type_name The name of the logger class to declare
 *  \param char_type The character type of the logger. Either char or wchar_t
 * expected. \param base_seq A Boost.Preprocessor sequence of type identifiers
 * of the base classes templates \param threading A threading model class
 */
#define BOOST_LOG_DECLARE_LOGGER_TYPE(type_name, char_type, base_seq,          \
                                      threading)                               \
    class type_name                                                            \
        : public ::boost::log::sources::basic_composite_logger<                \
              char_type, type_name, threading,                                 \
...
/*!
 *  \brief The macro declares a narrow-char logger class that inherits a number
 * of base classes
 *
 ...
 *
 *  \param type_name The name of the logger class to declare
 *  \param base_seq A Boost.Preprocessor sequence of type identifiers of the
...
/*!
 *  \brief The macro declares a narrow-char thread-safe logger class that
 * inherits a number of base classes
 *
 *  Equivalent to <tt>BOOST_LOG_DECLARE_LOGGER_TYPE(type_name, char, base_seq,
 * multi_thread_model< shared_mutex >)</tt>
 *
 *  \param type_name The name of the logger class to declare
 *  \param base_seq A Boost.Preprocessor sequence of type identifiers of the
 * base classes templates
...
#define BOOST_LOG_DECLARE_LOGGER_MT(type_name, base_seq)                       \
    BOOST_LOG_DECLARE_LOGGER_TYPE(                                             \
        type_name, char, base_seq,                                             \
...
/*!
 *  \brief The macro declares a wide-char logger class that inherits a number of
 * base classes
 *
...
/*!
 *  \brief The macro declares a wide-char thread-safe logger class that inherits
 * a number of base classes
 *
 *  Equivalent to <tt>BOOST_LOG_DECLARE_LOGGER_TYPE(type_name, wchar_t,
 * base_seq, multi_thread_model< shared_mutex >)</tt>
 *
 *  \param type_name The name of the logger class to declare
 *  \param base_seq A Boost.Preprocessor sequence of type identifiers of the
 * base classes templates
