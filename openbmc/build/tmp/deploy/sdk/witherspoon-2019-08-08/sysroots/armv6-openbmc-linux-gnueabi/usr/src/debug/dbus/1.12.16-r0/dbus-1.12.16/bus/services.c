                                ...
                                const DBusString* service_name)
{
    ...
    service = _dbus_hash_table_lookup_string(
        registry->service_hash, _dbus_string_get_const_data(service_name));

    return service;
}

static DBusList* _bus_service_find_owner_link(BusService* service,
...
static BusOwner* bus_owner_new(BusService* service, DBusConnection* conn,
                               dbus_uint32_t flags)
{
    BusOwner* result;
                                ...
                                BusTransaction* transaction, DBusError* error)
{
    ...
    _dbus_verbose("copied string %p '%s' to '%s'\n", service_name,
                  _dbus_string_get_const_data(service_name), service->name);

    if (!bus_driver_send_service_owner_changed(
            service->name, NULL,
            bus_connection_get_name(owner_connection_if_created), transaction,
            error))
    {
    ...
    if (!bus_activation_service_created(
            bus_context_get_activation(registry->context), service->name,
            transaction, error))
    {
        bus_service_unref(service);
        return NULL;
    ...
    if (!_dbus_hash_table_insert_string(registry->service_hash, service->name,
                                        service))
    {
...
void bus_registry_foreach(BusRegistry* registry,
                          BusServiceForeachFunction function, void* data)
{
    DBusHashIter iter;
...
dbus_bool_t bus_registry_list_services(BusRegistry* registry, char*** listp,
                                       int* array_len)
{
    int i, j, len;
    char** retval;
...
dbus_bool_t bus_registry_acquire_service(
    BusRegistry* registry, DBusConnection* connection,
    const DBusString* service_name, dbus_uint32_t flags, dbus_uint32_t* result,
    BusTransaction* transaction, DBusError* error)
{
    dbus_bool_t retval;
     ...
     */
    sid =
        bus_selinux_id_table_lookup(registry->service_sid_table, service_name);

    if (!bus_selinux_allows_acquire_service(
            connection, sid, _dbus_string_get_const_data(service_name), error))
    {
    ...
    if (!bus_apparmor_allows_acquire_service(
            connection, bus_context_get_type(registry->context),
    ...
    if (bus_connection_get_n_services_owned(connection) >= limit)
    {
        ...
        dbus_set_error(&tmp_error, DBUS_ERROR_LIMITS_EXCEEDED,
                       ...
                       bus_connection_is_active(connection)
                           ...
                           : "(inactive)",
                       limit);
        bus_context_log(registry->context, DBUS_SYSTEM_LOG_WARNING, "%s",
                        tmp_error.message);
              ...
              !(bus_service_get_allow_replacement(service))))
    {
        ...
        if (!bus_service_add_owner(service, connection, flags, transaction,
                                   error))
            goto out;
    ...
    else
    {
        ...
        {
            if (!bus_service_remove_owner(service, old_owner_conn, transaction,
                                          error))
                goto out;
        }
        else
        {
            if (!bus_service_swap_owner(service, old_owner_conn, transaction,
                                        error))
                goto out;
        }

        _dbus_assert(connection ==
                     bus_service_get_primary_owner(service)->conn);
        *result = DBUS_REQUEST_NAME_REPLY_PRIMARY_OWNER;
    }

    activation = bus_context_get_activation(registry->context);
    retval = bus_activation_send_pending_auto_activation_messages(
        activation, service, transaction);
    if (!retval)
        BUS_SET_OOM(error);
                                                   ...
                                                   DBusHashTable* table)
{
    ...
    while (_dbus_hash_iter_next(&iter))
    {
        ...
        if (!bus_selinux_id_table_insert(new_table, service, context))
            return FALSE;
    }
...
static void bus_service_relink(BusService* service,
                               DBusPreallocatedHash* preallocated)
{
    ...
    _dbus_hash_table_insert_string_preallocated(
        service->registry->service_hash, preallocated, service->name, service);
...
static dbus_bool_t
    add_cancel_ownership_to_transaction(BusTransaction* transaction,
                                        BusService* service, BusOwner* owner)
{
                                  ...
                                  BusTransaction* transaction, DBusError* error)
{
    ...
    {
        if (!bus_driver_send_service_acquired(connection, service->name,
                                              transaction, error))
            return FALSE;
    }
    ...
    if (bus_owner_link == NULL)
    {
        ...
        {
            if (!_dbus_list_insert_after(
                    &service->owners,
                    _dbus_list_get_first_link(&service->owners), bus_owner))
            {
                bus_owner_unref(bus_owner);
                BUS_SET_OOM(error);
                return FALSE;
    ...
    else
    {
        ...
        if (flags & DBUS_NAME_FLAG_REPLACE_EXISTING)
        {
            ...
            _dbus_list_insert_after_link(&service->owners, link,
                                         bus_owner_link);
        }
...
typedef struct
{
    BusOwner* owner;
    BusService* service;
    BusOwner* before_owner; /* restore to position before this connection in
                               owners list */
    DBusList* owner_link;
...
static void free_ownership_restore_data(void* data)
{
    ...
    if (d->hash_entry)
        _dbus_hash_table_free_preallocated_entry(
            d->service->registry->service_hash, d->hash_entry);
                                         ...
                                         BusService* service, BusOwner* owner)
{
    ...
    d->hash_entry =
        _dbus_hash_table_preallocate_entry(service->registry->service_hash);
    ...
    if (d->service_link == NULL || d->owner_link == NULL ||
        d->hash_entry == NULL ||
                                   ...
                                   DBusError* error)
{
    ...
    if (!bus_driver_send_service_lost(connection, service->name, transaction,
                                      error))
        return FALSE;
    ...
    if (service->owners == NULL)
    {
        _dbus_assert_not_reached(
            "Tried to swap owner of a service that has no owners");
    }
    else if (_dbus_list_length_is_one(&service->owners))
    {
        _dbus_assert_not_reached("Tried to swap owner of a service that has no "
    ...
    else
    {
        ...
        if (!bus_driver_send_service_owner_changed(
                service->name, bus_connection_get_name(connection),
                bus_connection_get_name(new_owner_conn), transaction, error))
            return FALSE;

        /* This will be our new owner */
        if (!bus_driver_send_service_acquired(new_owner_conn, service->name,
                                              transaction, error))
            return FALSE;
    }

    if (!add_restore_ownership_to_transaction(transaction, service,
                                              primary_owner))
    {
        BUS_SET_OOM(error);
        return FALSE;
                                     ...
                                     DBusError* error)
{
    ...
    {
        _dbus_assert_not_reached(
            "Tried to remove owner of a service that has no owners");
    }
    else if (_dbus_list_length_is_one(&service->owners))
    {
        if (!bus_driver_send_service_owner_changed(
                service->name, bus_connection_get_name(connection), NULL,
                transaction, error))
            return FALSE;
    }
    else
