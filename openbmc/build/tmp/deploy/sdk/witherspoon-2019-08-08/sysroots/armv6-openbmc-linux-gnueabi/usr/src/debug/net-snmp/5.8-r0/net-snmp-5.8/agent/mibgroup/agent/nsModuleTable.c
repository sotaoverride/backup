...
/** Initialize the nsModuleTable table by defining it's contents and how it's
 * structured */
void initialize_table_nsModuleTable(void)
{
    ...
    my_handler = netsnmp_create_handler_registration(
        "nsModuleTable", nsModuleTable_handler, nsModuleTable_oid,
...
netsnmp_variable_list* nsModuleTable_get_first_data_point(
    void** my_loop_context, void** my_data_context,
    netsnmp_variable_list* put_index_data, netsnmp_iterator_info* otherstuff)
{
    ...
    *my_data_context = ctree->tree;
    ...
    snmp_set_var_value(vptr, ctree->context_ptr->first_subtree->name_a,
...
struct variable_list* nsModuleTable_get_next_data_point(
    void** my_loop_context, void** my_data_context,
    struct variable_list* put_index_data, netsnmp_iterator_info* otherstuff)
{
...
/** handles requests for the nsModuleTable table, if anything else needs to be
 * done */
int nsModuleTable_handler(netsnmp_mib_handler* handler,
            ...
            case MODE_GET:
                switch (table_info->colnum)
                {
                    case COLUMN_NSMODULENAME:
                        if (tree->reginfo->handlerName)
                        {
                            snmp_set_var_typed_value(
                                var, ASN_OCTET_STR, tree->reginfo->handlerName,
                    ...
                    case COLUMN_NSMODULEMODES:
                         ...
                         */
                        modes[0] =
                            ((HANDLER_CAN_GETANDGETNEXT & tree->reginfo->modes)
                             << 7) |
                    ...
                    case COLUMN_NSMODULETIMEOUT:
                        ultmp = tree->timeout;
                        snmp_set_var_typed_value(
                            var, ASN_INTEGER, (u_char*)&ultmp, sizeof(u_long));
                        break;
                    ...
                    default:
                        ...
                        snmp_log(LOG_ERR,
                                 ...
                                 "nsModuleTable_handler: unknown column\n");
                }
                break;

            default:
                snmp_log(LOG_ERR, "problem encountered in "
                                  "nsModuleTable_handler: unsupported mode\n");
        }
    }
