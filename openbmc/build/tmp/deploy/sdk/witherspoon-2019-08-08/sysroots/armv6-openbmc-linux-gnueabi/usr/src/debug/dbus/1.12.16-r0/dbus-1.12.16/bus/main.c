...
static void signal_handler(int sig)
{
    ...
    switch (sig)
    {
        case SIGHUP:
        {
            ...
            {
                /* If we receive SIGHUP often enough to fill the pipe buffer
                 * (4096 times on old Linux, 65536 on modern Linux) before it
                 * can be drained, let's just warn and ignore. The configuration
                 * will be reloaded while draining the pipe buffer, which is
                 * what we wanted. It's harmless that it will be reloaded fewer
                 * times than we asked for, since the reload is delayed anyway,
                 * so new changes will be picked up.
                 *
        ...
        case SIGTERM:
        {
                ...
                !_dbus_write_socket(reload_pipe[RELOAD_WRITE_END], &str, 0, 1))
            {
                 ...
                 * so we exit. We'd use exit(), but that's not
                 * async-signal-safe, so we'll have to resort to _exit(). */
                static const char message[] = "Unable to write termination "
                                              "signal to pipe - buffer full?\n"
                                              "Will exit instead.\n";
...
{
    fprintf(stderr, DBUS_DAEMON_NAME " [--version]"
                                     " [--session]"
                                     " [--system]"
                                     " [--config-file=FILE]"
...
static void check_two_config_files(const DBusString* config_file,
...
static void check_two_addresses(const DBusString* address,
                                const char* extra_arg)
{
...
static void check_two_addr_descriptors(const DBusString* addr_fd,
                                       const char* extra_arg)
{
...
static void check_two_pid_descriptors(const DBusString* pid_fd,
                                      const char* extra_arg)
{
...
#ifdef DBUS_UNIX
static dbus_bool_t handle_reload_watch(DBusWatch* watch, unsigned int flags,
                                       void* data)
{
    DBusError error;
    DBusString str;
...
static void setup_reload_pipe(DBusLoop* loop)
{
    ...
    watch = _dbus_watch_new(
        ...
        DBUS_WATCH_READABLE, TRUE, handle_reload_watch, NULL, NULL);
...
int main(int argc, char** argv)
{
    ...
    while (i < argc)
    {
        ...
        else if (strcmp(arg, "--syslog-only") == 0)
        {
            flags &= ~BUS_CONTEXT_FLAG_SYSLOG_NEVER;
            flags |=
                (BUS_CONTEXT_FLAG_SYSLOG_ALWAYS | BUS_CONTEXT_FLAG_SYSLOG_ONLY);
        }
        ...
        {
            print_address =
                TRUE; /* and we'll get the next arg if appropriate */
        }
    ...
    if (print_address)
    {
        ...
        if (_dbus_string_get_length(&addr_fd) > 0)
        {
            ...
            if (!_dbus_string_parse_int(&addr_fd, 0, &val, &end) ||
                end != _dbus_string_get_length(&addr_fd) || val < 0 ||
                val > _DBUS_INT_MAX)
            {
    ...
    if (print_pid)
    {
        ...
        if (_dbus_string_get_length(&pid_fd) > 0)
        {
            ...
            if (!_dbus_string_parse_int(&pid_fd, 0, &val, &end) ||
                end != _dbus_string_get_length(&pid_fd) || val < 0 ||
                val > _DBUS_INT_MAX)
            {
                fprintf(stderr, "Invalid file descriptor: \"%s\"\n",
    ...
    context = bus_context_new(
        &config_file, flags, &print_addr_pipe, &print_pid_pipe,
