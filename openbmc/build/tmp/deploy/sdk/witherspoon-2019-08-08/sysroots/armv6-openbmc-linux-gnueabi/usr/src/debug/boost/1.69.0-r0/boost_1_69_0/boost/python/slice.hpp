...
class slice : public detail::slice_base
{
  ...
  public:
                    ...
                    const RandomAccessIterator& end) const
    {
        ...
        typedef typename iterator_difference<RandomAccessIterator>::type
            difference_type;
        ...
        else
        {
            ...
            if (i >= 0)
            {
                ret.start = begin;
                BOOST_USING_STD_MIN();
                std::advance(ret.start, min BOOST_PREVENT_MACRO_SUBSTITUTION(
                                            i, max_dist - 1));
            }
            else
            {
        ...
        else
        {
            ...
            if (ret.step < 0)
            {
                ...
                { // i is negative, but more negative than -1.
                    ret.stop = end;
            ...
            { // stepping forward
                ...
                { // i is negative, but not more negative than -max_dist
                    ret.stop = end;
                    std::advance(ret.stop, i - 1);
                }
            }
