...
static int manager_link_is_online(Manager* m, Link* l, LinkOperationalState s)
{
    ...
    if (s < 0)
        s = m->required_operstate >= 0 ? m->required_operstate
                                       : l->required_operstate;
            ...
            if (!l)
            {
                ...
                if (r < 0)
                    return log_error_errno(r,
                                           "Failed to create link object: %m");
            }

            r = link_update_rtnl(l, mm);
            if (r < 0)
                log_link_warning_errno(
                    l, r, "Failed to process RTNL link message, ignoring: %m");

            r = link_update_monitor(l);
            if (r < 0)
                log_link_warning_errno(
                    l, r, "Failed to update link state, ignoring: %m");
...
static int on_rtnl_event(sd_netlink* rtnl, sd_netlink_message* mm,
                         void* userdata)
{
    Manager* m = userdata;
    int r;
    ...
    HASHMAP_FOREACH(l, m->links, i)
    {
        ...
        if (r < 0)
            log_link_warning_errno(l, r,
                                   "Failed to update monitor information: %m");
    }
...
static int manager_network_monitor_listen(Manager* m)
{
    ...
    r = sd_event_add_io(m->event, &m->network_monitor_event_source, fd, events,
                        &on_network_event, m);
    if (r < 0)
        return r;
...
int manager_new(Manager** ret, Hashmap* interfaces, char** ignore,
                LinkOperationalState required_operstate, bool any,
                usec_t timeout)
{
    ...
    *m = (Manager){
    ...
    if (timeout > 0)
    {
        ...
        r = sd_event_add_time(m->event, NULL, clock_boottime_or_monotonic(),
                              usec, 0, NULL, INT_TO_PTR(-ETIMEDOUT));
        if (r < 0)
            return r;
    }
