...
VTerm::CharAttr VTerm::default_char_attr = {0, 0, 1, 0,
                                            0, 0, 0, VTerm::CharAttr::Single};
...
u8 VTerm::control_map[MAX_CONTROL_CODE],
    VTerm::escape_map[NR_STATES][MAX_ESCAPE_CODE];
...
void VTerm::resize(u16 w, u16 h)
{
    ...
    {
        u16* new_text =
            new u16[new_max_width * (history_lines + new_max_height)];
        CharAttr* new_attrs =
            new CharAttr[new_max_width * (history_lines + new_max_height)];

        if (text)
        {
            u32 start, new_start;
            u16 history_copy_lines =
                (history_full ? history_lines : history_save_line);
            for (u16 i = 0; i < history_copy_lines; i++)
            {
                start = i * max_width;
                new_start = i * new_max_width;
                memcpy(&new_text[new_start], &text[start],
                       sizeof(*text) * max_width);
                memcpy(&new_attrs[new_start], &attrs[start],
                       sizeof(*attrs) * max_width);
...
#define CTRL_ALWAYS 0x0800f501 /* Cannot be overridden by disp_ctrl */
         ...
         */
        bool ok = tc &&
                  (c >= 32 || !(mode_flags.display_ctrl
                                    ? ((CTRL_ALWAYS >> c) & 1)
                                    : (utf8 || ((CTRL_ACTION >> c) & 1)))) &&
                  (c != 127 || mode_flags.display_ctrl) && (c != 128 + 27);
...
void VTerm::do_control_char()
{
    ...
    if (!index && esc_state != ESnormal && cur_char)
    {
        seq =
            escape_sequences +
            (cur_char < MAX_ESCAPE_CODE ? escape_map[esc_state][cur_char] : 0);
    }
...
void VTerm::update()
{
    ...
    for (u16 i = 0; i < height; i++)
    {
        ...
        {
            requestUpdate(dirty_startx[i], i,
                          dirty_endx[i] - dirty_startx[i] + 1, 1);
...
void VTerm::expose(u16 x, u16 y, u16 w, u16 h)
{
    ...
    for (; h--; y++)
    {
        ...
        if (attrs[yp + startx].type == CharAttr::DoubleRight)
            startx--;
        if (attrs[yp + endx].type == CharAttr::DoubleLeft)
            endx++;
        ...
        {
            if (attrs[yp + cur].type == CharAttr::DoubleRight)
                continue;
...
void VTerm::inverse(u16 sx, u16 sy, u16 ex, u16 ey)
{
    if (sy > ey)
        return;
    if (charAttr(sx, sy).type == CharAttr::DoubleRight)
        sx--;
    if (charAttr(ex, ey).type == CharAttr::DoubleLeft)
        ex++;
    if (sy == ey && sx > ex)
        return;
...
void VTerm::shift_text(u16 y, u16 start_x, u16 end_x, s16 num)
{
    ...
    if (num < mx && -num < mx)
    {
        if (num < 0)
        {
            memmove(text + yp + start_x, text + yp + start_x - num,
                    sizeof(*text) * (mx + num));
            memmove(attrs + yp + start_x, attrs + yp + start_x - num,
                    sizeof(*attrs) * (mx + num));
        }
        else
        {
            memmove(text + yp + start_x + num, text + yp + start_x,
                    sizeof(*text) * (mx - num));
            memmove(attrs + yp + start_x + num, attrs + yp + start_x,
                    sizeof(*attrs) * (mx - num));
        }
    }
...
{
    if (!history_lines || (absolute && num == (s32)visual_start_line) ||
        (!absolute && !num))
        return;
    ...
    u32 scroll_lines = (visual_start_line > bak_line)
                           ? (visual_start_line - bak_line)
                           : (bak_line - visual_start_line);
...
u16 VTerm::get_line(u16 y)
{
    ...
    if (line >= total_history_lines())
        return linenumbers[line - total_history_lines()];
