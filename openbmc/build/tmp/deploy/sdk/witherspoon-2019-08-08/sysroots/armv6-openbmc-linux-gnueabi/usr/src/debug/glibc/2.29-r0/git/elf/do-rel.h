...
elf_dynamic_do_Rel(struct link_map* map, ElfW(Addr) reladdr, ElfW(Addr) relsize,
            ...
            if (r2 != NULL)
                for (; r2 <= end2; ++r2)
                    if (ELFW(R_TYPE)(r2->r_info) == ELF_MACHINE_IRELATIVE)
                    {
                        ...
                        elf_machine_rel(
                            map, r2, &symtab[ELFW(R_SYM)(r2->r_info)],
                            &map->l_versions[ndx],
                            (void*)(l_addr + r2->r_offset), skip_ifunc);
                    }
#endif
        }
#ifndef RTLD_BOOTSTRAP
        else
        {
            ...
            if (r2 != NULL)
                for (; r2 <= end2; ++r2)
                    if (ELFW(R_TYPE)(r2->r_info) == ELF_MACHINE_IRELATIVE)
                        elf_machine_rel(
                            map, r2, &symtab[ELFW(R_SYM)(r2->r_info)], NULL,
                            (void*)(l_addr + r2->r_offset), skip_ifunc);
#endif
        }
#endif
