    ...
    if (access("/proc/self/ns/cgroup", F_OK) < 0)
    {
        if (errno != ENOENT)
            log_debug_errno(errno,
                            "Failed to check whether /proc/self/ns/cgroup is "
                            "available, assuming not: %m");
        enabled = false;
    }
    else
        enabled = true;
...
int cg_rmdir(const char* controller, const char* path)
{
    ...
    if (streq(controller, SYSTEMD_CGROUP_CONTROLLER))
    {
        ...
        if (r < 0)
            log_warning_errno(
                r, "Failed to remove compat systemd cgroup %s: %m", path);
    }
...
int cg_kill(const char* controller, const char* path, int sig,
            CGroupFlags flags, Set* s, cg_kill_log_func_t log_kill,
            void* userdata)
{
    ...
    /* Don't send SIGCONT twice. Also, SIGKILL always works even when process is
     * suspended, hence don't send SIGCONT on SIGKILL. */
...
int cg_migrate(const char* cfrom, const char* pfrom, const char* cto,
               const char* pto, CGroupFlags flags)
{
...
int cg_migrate_recursive(const char* cfrom, const char* pfrom, const char* cto,
                         const char* pto, CGroupFlags flags)
{
...
int cg_migrate_recursive_fallback(const char* cfrom, const char* pfrom,
                                  const char* cto, const char* pto,
                                  CGroupFlags flags)
{
...
static int join_path_legacy(const char* controller, const char* path,
...
int cg_get_path_and_check(const char* controller, const char* path,
                          const char* suffix, char** fs)
{
    int r;
...
static int trim_cb(const char* path, const struct stat* sb, int typeflag,
                   struct FTW* ftwbuf)
{
    assert(path);
    assert(sb);
...
int cg_trim(const char* controller, const char* path, bool delete_root)
{
    ...
    if (q > 0 && streq(controller, SYSTEMD_CGROUP_CONTROLLER))
    {
        ...
        if (q < 0)
            log_warning_errno(q, "Failed to trim compat systemd cgroup %s: %m",
                              path);
    }
...
int cg_create(const char* controller, const char* path)
{
    ...
    if (r > 0 && streq(controller, SYSTEMD_CGROUP_CONTROLLER))
    {
        ...
        if (r < 0)
            log_warning_errno(
                r, "Failed to create compat systemd cgroup %s: %m", path);
    }
...
int cg_set_access(const char* controller, const char* path, uid_t uid,
                  gid_t gid)
{
    ...
    for (i = attributes[unified]; i->name; i++)
    {
        ...
        if (r < 0)
        {
            ...
            log_debug_errno(
                r, "Failed to set access on cgroup %s, ignoring: %m", fs);
        }
    }
    ...
    if (streq(controller, SYSTEMD_CGROUP_CONTROLLER))
    {
        ...
        if (r > 0)
        {
            ...
            if (r < 0)
                log_debug_errno(r,
                                "Failed to set access on compatibility systemd "
                                "cgroup %s, ignoring: %m",
                                path);
        }
    }
...
int cg_set_xattr(const char* controller, const char* path, const char* name,
...
int cg_get_xattr(const char* controller, const char* path, const char* name,
...
int cg_set_attribute(const char* controller, const char* path,
...
int cg_get_attribute(const char* controller, const char* path,
...
int cg_get_keyed_attribute(const char* controller, const char* path,
                           const char* attribute, char** keys,
                           char** ret_values)
{
    ...
    /* Reads one or more fields of a cgroup v2 keyed attribute file. The 'keys'
     * parameter should be an strv with all keys to retrieve. The 'ret_values'
     * parameter should be passed as string size with the same number of entries
     * as 'keys'. On success each entry will be set to the value of the matching
     * key.
     *
     * If the attribute file doesn't exist at all returns ENOENT, if any key is
     * not found returns ENXIO. */
    ...
    /* Let's build this up in a temporary array for now in order not to clobber
     * the return parameter on failure */
    v = newa0(char*, n);
...
int cg_migrate_everywhere(CGroupMask supported, const char* from,
                          const char* to, cg_migrate_callback_t to_callback,
                          void* userdata)
{
    CGroupController c;
    CGroupMask done;
    ...
    if (!path_equal(from, to))
    {
        r = cg_migrate_recursive(SYSTEMD_CGROUP_CONTROLLER, from,
                                 SYSTEMD_CGROUP_CONTROLLER, to, CGROUP_REMOVE);
        if (r < 0)
            return r;
    }
    ...
    for (c = 0; c < _CGROUP_CONTROLLER_MAX; c++)
    {
        ...
        (void)cg_migrate_recursive_fallback(SYSTEMD_CGROUP_CONTROLLER, to,
                                            cgroup_controller_to_string(c), p,
                                            0);
        done |= CGROUP_MASK_EXTEND_JOINED(bit);
    }
...
int cg_mask_supported(CGroupMask* ret)
{
    ...
    /* Determines the mask of supported cgroup controllers. Only includes
     * controllers we can make sense of and that are actually accessible. Only
     * covers real controllers, i.e. not the CGROUP_CONTROLLER_BPF_xyz
     * pseudo-controllers. */
    ...
    if (r > 0)
    {
        ...
        r = cg_get_path(SYSTEMD_CGROUP_CONTROLLER, root, "cgroup.controllers",
                        &path);
        if (r < 0)
            return r;
        ...
        /* Currently, we support the cpu, memory, io and pids controller in the
         * unified hierarchy, mask everything else off. */
        mask &= CGROUP_MASK_V2;
    }
    else
    {
...
int cg_kernel_controllers(Set** ret)
{
    ...
    /* Determines the full list of kernel-known controllers. Might include
     * controllers we don't actually support and controllers that aren't
     * currently accessible (because not mounted). This does not include "name="
     * pseudo-controllers. */
...
/* The hybrid mode was initially implemented in v232 and simply mounted cgroup2
 * on /sys/fs/cgroup/systemd.  This unfortunately broke other tools (such as
 * docker) which expected the v1 "name=systemd" hierarchy on
 * /sys/fs/cgroup/systemd.  From v233 and on, the hybrid mode mountnbs v2 on
 * /sys/fs/cgroup/unified and maintains "name=systemd" hierarchy on
 * /sys/fs/cgroup/systemd for compatibility with other tools.
 *
 * To keep live upgrade working, we detect and support v232 layout.  When v232
 * layout is detected, to keep cgroup v2 process management but disable the
 * compat dual layout, we return %true on
 * cg_unified_controller(SYSTEMD_CGROUP_CONTROLLER) and %false on
 * cg_hybrid_unified().
 */
...
static int cg_unified_update(void)
{
    ...
    else if (F_TYPE_EQUAL(fs.f_type, TMPFS_MAGIC))
    {
        ...
        {
            log_debug("Found cgroup2 on /sys/fs/cgroup/unified, unified "
                      "hierarchy for systemd controller");
        ...
        else
        {
            if (statfs("/sys/fs/cgroup/systemd/", &fs) < 0)
                return log_debug_errno(
                    errno, "statfs(\"/sys/fs/cgroup/systemd\" failed: %m");

            if (F_TYPE_EQUAL(fs.f_type, CGROUP2_SUPER_MAGIC))
            {
                log_debug("Found cgroup2 on /sys/fs/cgroup/systemd, unified "
                          "hierarchy for systemd controller (v232 variant)");
                unified_cache = CGROUP_UNIFIED_SYSTEMD;
            ...
            {
                log_debug("Unexpected filesystem type %llx mounted on "
                         ...
                         CGroupMask* ret_result_mask)
{
    ...
    if (r == 0)
    {
        /* On the legacy hiearchy there's no concept of "enabling" controllers
         * in cgroups defined. Let's claim complete success right away. (If you
         * wonder why we return the full mask here, rather than zero: the caller
         * tends to use the returned mask later on to compare if all controllers
         * where properly joined, and if not requeues realization. This use is
         * the primary purpose of the return value, hence let's minimize
         * surprises here and reduce triggers for re-realization by always
         * saying we fully succeeded.) */
        if (ret_result_mask)
            *ret_result_mask =
                mask & supported &
    ...
    r = cg_get_path(SYSTEMD_CGROUP_CONTROLLER, p, "cgroup.subtree_control",
                    &fs);
    if (r < 0)
        return r;
        ...
        n = cgroup_controller_to_string(c);
        {
            ...
            if (r < 0)
            {
                log_debug_errno(r, "Failed to %s controller %s for %s (%s): %m",
                                FLAGS_SET(mask, bit) ? "enable" : "disable", n,
                                p, fs);
                clearerr(f);

                /* If we can't turn off a controller, leave it on in the
                 * reported resulting mask. This happens for example when we
                 * attempt to turn off a controller up in the tree that is
            ...
            {
                /* Otherwise, if we managed to turn on a controller, set the bit
                 * reflecting that. */
                if (FLAGS_SET(mask, bit))
                    ret |= bit;
            }
