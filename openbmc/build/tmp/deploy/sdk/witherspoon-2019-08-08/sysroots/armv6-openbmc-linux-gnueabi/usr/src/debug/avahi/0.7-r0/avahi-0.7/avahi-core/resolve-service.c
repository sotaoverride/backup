...
static void finish(AvahiSServiceResolver* r, AvahiResolverEvent event)
{
    ...
    flags = r->txt_flags | r->srv_flags | r->address_flags;

    switch (event)
    {
        case AVAHI_RESOLVER_FAILURE:

            r->callback(r, r->interface, r->protocol, event, r->service_name,
                        r->service_type, r->domain_name, NULL, NULL, 0, NULL,
                        flags, r->userdata);
        ...
        case AVAHI_RESOLVER_FOUND:
        {
            ...
            r->callback(
                r, r->interface, r->protocol, event, r->service_name,
                r->service_type, r->domain_name, r->srv_record->data.srv.name,
                r->address_record ? &a : NULL, r->srv_record->data.srv.port,
...
static void start_timeout(AvahiSServiceResolver* r)
{
    ...
    r->time_event = avahi_time_event_new(r->server->time_event_queue, &tv,
                                         time_event_callback, r);
}

static void
    record_browser_callback(AvahiSRecordBrowser* rr, AvahiIfIndex interface,
                            AvahiProtocol protocol, AvahiBrowserEvent event,
                            AvahiRecord* record, AvahiLookupResultFlags flags,
                            void* userdata)
{
    ...
    switch (event)
    {
        ...
        case AVAHI_BROWSER_NEW:
        {
            ...
            if (r->protocol != AVAHI_PROTO_UNSPEC &&
                protocol != AVAHI_PROTO_UNSPEC && protocol != r->protocol)
                return;
                ...
                case AVAHI_DNS_TYPE_SRV:
                    if (!r->srv_record)
                    {
                        ...
                        if (!(r->user_flags & AVAHI_LOOKUP_NO_ADDRESS))
                        {
                            ...
                            if (r->address_protocol == AVAHI_PROTO_INET ||
                                r->address_protocol == AVAHI_PROTO_UNSPEC)
                            {
                                AvahiKey* k = avahi_key_new(
                                    r->srv_record->data.srv.name,
                                    AVAHI_DNS_CLASS_IN, AVAHI_DNS_TYPE_A);
                                r->record_browser_a =
                                    avahi_s_record_browser_new(
                                        r->server, r->interface, r->protocol, k,
                                        r->user_flags &
                                            ~(AVAHI_LOOKUP_NO_TXT |
                                              AVAHI_LOOKUP_NO_ADDRESS),
                                        record_browser_callback, r);
                                avahi_key_unref(k);
                            }

                            if (r->address_protocol == AVAHI_PROTO_INET6 ||
                                r->address_protocol == AVAHI_PROTO_UNSPEC)
                            {
                                AvahiKey* k = avahi_key_new(
                                    r->srv_record->data.srv.name,
                                    AVAHI_DNS_CLASS_IN, AVAHI_DNS_TYPE_AAAA);
        ...
        case AVAHI_BROWSER_REMOVE:
            ...
            switch (record->key->type)
            {
                case AVAHI_DNS_TYPE_SRV:
                    ...
                    if (r->srv_record &&
                        avahi_record_equal_no_ttl(record, r->srv_record))
                    {
                ...
                case AVAHI_DNS_TYPE_TXT:
                    ...
                    if (r->txt_record &&
                        avahi_record_equal_no_ttl(record, r->txt_record))
                    {
                ...
                case AVAHI_DNS_TYPE_AAAA:
                    ...
                    if (r->address_record &&
                        avahi_record_equal_no_ttl(record, r->address_record))
                    {
                        ...
                        /** Look for a replacement */
                        if (r->record_browser_aaaa)
                            avahi_s_record_browser_restart(
                                r->record_browser_aaaa);
                        if (r->record_browser_a)
        ...
        case AVAHI_BROWSER_FAILURE:
            ...
            {
                /* We were looking for both AAAA and A, and the other query is
                 * still living, so we'll not die */
                avahi_s_record_browser_free(r->record_browser_a);
                r->record_browser_a = NULL;
                break;
            }
            ...
            r->record_browser_srv = r->record_browser_txt =
...
AvahiSServiceResolver* avahi_s_service_resolver_new(
    ...
    AvahiSServiceResolverCallback callback, void* userdata)
{
    ...
    AVAHI_CHECK_VALIDITY_RETURN_NULL(server, AVAHI_PROTO_VALID(aprotocol),
                                     AVAHI_ERR_INVALID_PROTOCOL);
    AVAHI_CHECK_VALIDITY_RETURN_NULL(
        server, !domain || avahi_is_valid_domain_name(domain),
    ...
    AVAHI_CHECK_VALIDITY_RETURN_NULL(server,
                                     avahi_is_valid_service_type_strict(type),
                                     AVAHI_ERR_INVALID_SERVICE_TYPE);
    AVAHI_CHECK_VALIDITY_RETURN_NULL(
        server,
        AVAHI_FLAGS_VALID(
            flags, AVAHI_LOOKUP_USE_WIDE_AREA | AVAHI_LOOKUP_USE_MULTICAST |
                       AVAHI_LOOKUP_NO_TXT | AVAHI_LOOKUP_NO_ADDRESS),
    ...
    r->protocol = protocol;
    r->user_flags = flags;
    r->record_browser_a = r->record_browser_aaaa = r->record_browser_srv =
        r->record_browser_txt = NULL;
    r->time_event = NULL;
    AVAHI_LLIST_PREPEND(AvahiSServiceResolver, resolver,
                        server->service_resolvers, r);

    k = avahi_key_new(n, AVAHI_DNS_CLASS_IN, AVAHI_DNS_TYPE_SRV);
    r->record_browser_srv = avahi_s_record_browser_new(
        server, interface, protocol, k,
        flags & ~(AVAHI_LOOKUP_NO_TXT | AVAHI_LOOKUP_NO_ADDRESS),
        record_browser_callback, r);
    avahi_key_unref(k);
    ...
    if (!(flags & AVAHI_LOOKUP_NO_TXT))
    {
        ...
        r->record_browser_txt = avahi_s_record_browser_new(
            server, interface, protocol, k,
            flags & ~(AVAHI_LOOKUP_NO_TXT | AVAHI_LOOKUP_NO_ADDRESS),
            record_browser_callback, r);
        avahi_key_unref(k);
...
void avahi_s_service_resolver_free(AvahiSServiceResolver* r)
{
    ...
    AVAHI_LLIST_REMOVE(AvahiSServiceResolver, resolver,
                       r->server->service_resolvers, r);
