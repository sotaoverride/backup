...
static const struct flags_char flags_array[] = {
    {EXT2_NOATIME_FL, 'A'},      {EXT2_SYNC_FL, 'S'},
    {EXT2_DIRSYNC_FL, 'D'},      {EXT2_APPEND_FL, 'a'},
    {EXT2_COMPR_FL, 'c'},        {EXT2_NODUMP_FL, 'd'},
    {EXT4_EXTENTS_FL, 'e'},      {EXT2_IMMUTABLE_FL, 'i'},
    {EXT3_JOURNAL_DATA_FL, 'j'}, {EXT4_PROJINHERIT_FL, 'P'},
    {EXT2_SECRM_FL, 's'},        {EXT2_UNRM_FL, 'u'},
    {EXT2_NOTAIL_FL, 't'},       {EXT2_TOPDIR_FL, 'T'},
    {FS_NOCOW_FL, 'C'},          {0, 0}};
        ...
        case '-':
            for (p = &argv[*i][1]; *p; p++)
            {
                ...
                if (*p == 'p')
                {
                    ...
                    if (*tmp)
                    {
                        com_err(program_name, 0, _("bad project - %s\n"),
                                argv[*i]);
                        usage();
                    }
                    set_project = 1;
                ...
                if (*p == 'v')
                {
                    ...
                    if (*tmp)
                    {
                        com_err(program_name, 0, _("bad version - %s\n"),
                                argv[*i]);
                        usage();
                    }
                    set_version = 1;
...
static int change_attributes(const char* name)
{
    ...
    {
        if (!silent)
            com_err(program_name, errno, _("while trying to stat %s"), name);
        return -1;
    }
    ...
    {
        if (!silent)
            com_err(program_name, errno, _("while reading flags on %s"), name);
        return -1;
    }
    if (set)
    {
    ...
    else
    {
        ...
        if (fsetflags(name, flags) == -1)
        {
            if (!silent)
            {
                com_err(program_name, errno, _("while setting flags on %s"),
                        name);
            }
            return -1;
        }
    ...
    if (set_version)
    {
        ...
        {
            if (!silent)
                com_err(program_name, errno, _("while setting version on %s"),
                        name);
            return -1;
        }
    }
    if (set_project)
    {
        ...
        {
            if (!silent)
                com_err(program_name, errno, _("while setting project on %s"),
                        name);
            return -1;
        }
    }
