...
DEFINE_CONFIG_PARSE_ENUM(config_parse_dns_stub_listener_mode,
                         dns_stub_listener_mode, DnsStubListenerMode,
                         "Failed to parse DNS stub listener mode setting");

static const char* const
    dns_stub_listener_mode_table[_DNS_STUB_LISTENER_MODE_MAX] = {
        [DNS_STUB_LISTENER_NO] = "no",
        [DNS_STUB_LISTENER_UDP] = "udp",
        [DNS_STUB_LISTENER_TCP] = "tcp",
        [DNS_STUB_LISTENER_YES] = "yes",
};
DEFINE_STRING_TABLE_LOOKUP_WITH_BOOLEAN(dns_stub_listener_mode,
                                        DnsStubListenerMode,
                                        DNS_STUB_LISTENER_YES);

int manager_add_dns_server_by_string(Manager* m, DnsServerType type,
                                     const char* word)
{
                                             ...
                                             const char* string)
{
    ...
    for (;;)
    {
        ...
        if (r < 0)
            log_warning_errno(
                r, "Failed to add DNS server address '%s', ignoring: %m", word);
    }
...
int manager_add_search_domain_by_string(Manager* m, const char* domain)
{
    ...
    {
        r = dns_search_domain_new(m, &d, DNS_SEARCH_DOMAIN_SYSTEM, NULL,
                                  domain);
        if (r < 0)
            return r;
    }
...
int manager_parse_search_domains_and_warn(Manager* m, const char* string)
{
    ...
    for (;;)
    {
        ...
        if (r < 0)
            log_warning_errno(
                r, "Failed to add search domain '%s', ignoring: %m", word);
    }

    return 0;
}

int config_parse_dns_servers(const char* unit, const char* filename,
...
int config_parse_dnssd_service_name(const char* unit, const char* filename,
                                    unsigned line, const char* section,
                                    unsigned section_line, const char* lvalue,
                                    int ltype, const char* rvalue, void* data,
                                    void* userdata)
{
    ...
    if (isempty(rvalue))
    {
        log_syntax(unit, LOG_ERR, filename, line, 0,
                   "Service instance name can't be empty. Ignoring.");
        return -EINVAL;
    }
    ...
    {
        log_syntax(unit, LOG_ERR, filename, line, 0,
                   "Service instance name template renders to invalid name "
                   "'%s'. Ignoring.",
                   name);
        return -EINVAL;
    }
...
int config_parse_dnssd_service_type(const char* unit, const char* filename,
                                    unsigned line, const char* section,
                                    unsigned section_line, const char* lvalue,
                                    int ltype, const char* rvalue, void* data,
                                    void* userdata)
{
    ...
    {
        log_syntax(unit, LOG_ERR, filename, line, 0,
                   "Service type can't be empty. Ignoring.");
        return -EINVAL;
    }
    ...
    {
        log_syntax(unit, LOG_ERR, filename, line, 0,
                   "Service type is invalid. Ignoring.");
        return -EINVAL;
    }
...
int config_parse_dnssd_txt(const char* unit, const char* filename,
                           unsigned line, const char* section,
...
int manager_parse_config_file(Manager* m)
{
    ...
    r = config_parse_many_nulstr(PKGSYSCONFDIR "/resolved.conf",
                                 ...
                                 resolved_gperf_lookup, CONFIG_PARSE_WARN, m);
    if (r < 0)
        return r;
    ...
    {
        log_warning("DNSSEC option cannot be enabled or set to allow-downgrade "
                    "when systemd-resolved is built without gcrypt support. "
    ...
    {
        log_warning("DNS-over-TLS option cannot be set to opportunistic when "
                    "systemd-resolved is built without DNS-over-TLS support. "
