...
/* create hints table */
static FT_Error psh_hint_table_init(PSH_Hint_Table table, PS_Hint_Table hints,
...
#if 1
static FT_Pos psh_dimension_quantize_len(PSH_Dimension dim, FT_Pos len,
                                         FT_Bool do_snapping)
{
    if (len <= 64)
        len = 64;
...
static void ps_simple_scale(PSH_Hint_Table table, FT_Fixed scale,
                            FT_Fixed delta, FT_Int dimension)
{
    FT_UInt count;
...
static void psh_hint_align(PSH_Hint hint, PSH_Globals globals, FT_Int dimension,
                           PSH_Glyph glyph)
{
            ...
            default:
            {
                ...
                if (glyph->do_stem_adjust)
                {
                    if (len <= 64)
                    {
                        ...
                        if (len >= 32)
                        {
                            /* This is a special case where we also widen the
                             * stem and align it to the pixel grid.
                             *
                             *   stem_center          = pos + (len/2)
                             *   nearest_pixel_center =
                             * FT_ROUND(stem_center-32)+32 new_pos =
                             * nearest_pixel_center-32 =
                             * FT_ROUND(stem_center-32) =
                             * FT_FLOOR(stem_center-32+32) =
                             * FT_FLOOR(stem_center) new_len              = 64
                             */
                        ...
                        {
                            /* This is a very small stem; we simply align it to
                             * the pixel grid, trying to find the minimum
                             * displacement.
                             *
                             * left               = pos
                             * right              = pos + len
...
static void psh_hint_table_align_hints(PSH_Hint_Table table,
                                       PSH_Globals globals, FT_Int dimension,
                                       PSH_Glyph glyph)
{
    PSH_Hint hint;
    FT_UInt count;
...
static FT_Error psh_glyph_init(PSH_Glyph glyph, FT_Outline* outline,
                               PS_Hints ps_hints, PSH_Globals globals)
{
    FT_Error error;
    FT_Memory memory;
    ...
    error = psh_hint_table_init(&glyph->hint_tables[0],
                                ...
                                &ps_hints->dimension[0].counters, memory);
    if (error)
        goto Exit;
    ...
    error = psh_hint_table_init(&glyph->hint_tables[1],
                                ...
                                &ps_hints->dimension[1].counters, memory);
    if (error)
        goto Exit;
...
static void psh_glyph_compute_extrema(PSH_Glyph glyph)
{
    ...
    for (n = 0; n < glyph->num_points; n++)
    {
        ...
        if (before->org_v < point->org_v && after->org_v > point->org_v)
        {
            psh_point_set_positive(point);
        }
        else if (before->org_v > point->org_v && after->org_v < point->org_v)
        {
...
static void psh_glyph_find_strong_points(PSH_Glyph glyph, FT_Int dimension)
{
    ...
    FT_UInt first = 0;
    FT_Int major_dir = (dimension == 0) ? PSH_DIR_VERTICAL : PSH_DIR_HORIZONTAL;
    ...
    if (num_masks > 1 && glyph->num_points > 0)
    {
        ...
        first = mask->end_point > glyph->num_points ? glyph->num_points
                                                    : mask->end_point;
        mask++;
    ...
    if (num_masks == 1)
    {
        ...
        psh_hint_table_find_strong_points(table, point, count, threshold,
                                          major_dir);
    }
                                                ...
                                                FT_Int dimension)
{
    ...
    for (point = points; point < points_end; point++)
    {
        ...
        if (psh_point_is_smooth(point))
        {
            ...
            if (!psh_point_is_extremum(point) && !psh_point_is_inflex(point))
                continue;
            ...
            /* now interpolate between them */
            {
                ...
                do
                {
                    ...
                    else if (org_ac >= org_ab)
                    {
                        ...
                        cur_c =
                            cur_a + cur_ab + FT_MulFix(org_ac - org_ab, scale);
                    }
                    else
                    {
...
FT_Error ps_hints_apply(PS_Hints ps_hints, FT_Outline* outline,
                        PSH_Globals globals, FT_Render_Mode hint_mode)
{
     ...
     */
    {
        ...
        for (dimension = 0; dimension < 2; dimension++)
        {
            ...
            if (rescale)
                psh_globals_set_scale(glyph->globals, old_x_scale, old_y_scale,
                                      0, 0);
        }
    }
