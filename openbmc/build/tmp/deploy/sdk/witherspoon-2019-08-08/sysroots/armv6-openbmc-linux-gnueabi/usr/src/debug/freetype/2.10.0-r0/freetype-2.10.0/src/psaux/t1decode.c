...
t1_lookup_glyph_by_stdcharcode_ps(PS_Decoder* decoder, FT_Int charcode)
{
    ...
    glyph_name =
        psnames->adobe_std_strings(psnames->adobe_std_encoding[charcode]);
 ...
 */
static FT_Error t1operator_seac(T1_Decoder decoder, FT_Pos asb, FT_Pos adx,
 ...
 */
FT_LOCAL_DEF(FT_Error)
t1_decoder_parse_charstrings(T1_Decoder decoder, FT_Byte* charstring_base,
                             FT_UInt charstring_len)
{
    ...
    seed = (FT_Fixed)(((FT_Offset)(char*)&seed ^ (FT_Offset)(char*)&decoder ^
    ...
    FT_ASSERT((decoder->len_buildchar == 0) == (decoder->buildchar == NULL));
    ...
    while (ip < limit)
    {
        ...
        switch (*ip++)
        {
            ...
            case 255: /* four bytes integer */
                ...
                /* <large1> <large2> <num> div div'.  This is probably not
                 * allowed */
                /* anyway. */
        ...
        else if (op == op_callothersubr) /* callothersubr */
        {
            ...
            switch (subr_no)
            {
                case 0: /* end flex feature */
                    ...
                    if (!decoder->flex_state || decoder->num_flex_vectors != 7)
                    {
                        FT_ERROR(("t1_decoder_parse_charstrings:"
                                  " unexpected flex end\n"));
                        goto Syntax_Error;
                ...
                case 19:
                    ...
                    /*    of BuildCharArray with WeightVector               */
                    {
                        ...
                        if (idx < 0 || (FT_UInt)idx + blend->num_designs >
            ...
            switch (op)
            {
                case op_endchar:
                    ...
                    if (hinter)
                    {
                        ...
                        error =
                            hinter->apply(hinter->hints, builder->current,
                ...
                case op_hsbw:
                    ...
                    builder->left_bearing.x =
                        ADD_LONG(builder->left_bearing.x, top[0]);
                ...
                case op_sbw:
                    ...
                    builder->left_bearing.y =
                        ADD_LONG(builder->left_bearing.y, top[1]);
                ...
                case op_callsubr:
                {
                    ...
                    {
                        size_t* val =
                            ft_hash_num_lookup(idx, decoder->subrs_hash);
                    ...
                    /* The Type 1 driver stores subroutines without the seed
                     * bytes. */
                    ...
                    {
                        /* We are using subroutines from a CID font.  We must
                         * adjust */
                        /* for the seed bytes. */
                        zone->base +=
                            (decoder->lenIV >= 0 ? decoder->lenIV : 0);
 ...
 */
FT_LOCAL_DEF(FT_Error)
t1_decoder_parse_metrics(T1_Decoder decoder, FT_Byte* charstring_base,
                         FT_UInt charstring_len)
{
    T1_Decoder_Zone zone;
    FT_Byte* ip;
...
FT_LOCAL_DEF(FT_Error)
t1_decoder_init(T1_Decoder decoder, FT_Face face, FT_Size size,
