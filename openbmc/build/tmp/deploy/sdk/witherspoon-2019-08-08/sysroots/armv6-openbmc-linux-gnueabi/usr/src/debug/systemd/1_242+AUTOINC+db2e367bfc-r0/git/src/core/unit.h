...
/* Stores the 'reason' a dependency was created as a bit mask, i.e. due to which
 * configuration source it came to be. We use this so that we can selectively
 * flush out parts of dependencies again. Note that the same dependency might be
 * created as a result of multiple "reasons", hence the bitmask. */
typedef enum UnitDependencyMask
{
    /* Configured directly by the unit file, .wants/.requries symlink or
     * drop-in, or as an immediate result of a
     * non-dependency option configured that way.  */
    UNIT_DEPENDENCY_FILE = 1 << 0,

    /* As unconditional implicit dependency (not affected by unit configuration
     * — except by the unit name and type) */
    UNIT_DEPENDENCY_IMPLICIT = 1 << 1,

    /* A dependency effected by DefaultDependencies=yes. Note that dependencies
     * marked this way are conceptually just a subset of UNIT_DEPENDENCY_FILE,
     * as DefaultDependencies= is itself a unit file setting that can only be
     * set in unit files. We make this two separate bits only to help debugging
    ...
    /* A dependency created because of data read from /proc/self/mountinfo and
       no other configuration source */
    UNIT_DEPENDENCY_MOUNTINFO_IMPLICIT = 1 << 5,

    /* A dependency created because of data read from /proc/self/mountinfo, but
     * conditionalized by DefaultDependencies= and thus also involving
     * configuration from UNIT_DEPENDENCY_FILE sources */
    UNIT_DEPENDENCY_MOUNTINFO_DEFAULT = 1 << 6,

    /* A dependency created because of data read from /proc/swaps and no other
...
/* The Unit's dependencies[] hashmaps use this structure as value. It has the
 * same size as a void pointer, and thus can be stored directly as hashmap
 * value, without any indirection. Note that this stores two masks, as both the
 * origin and the destination of a dependency might have created it. */
...
typedef struct Unit
{
    ...
    char* id; /* One name is special because we use it for identification.
                 Points to an entry in the names set */
    char* instance;

    Set* names;

    /* For each dependency type we maintain a Hashmap whose key is the Unit*
     * object, and the value encodes why the dependency exists, using the
     * UnitDependencyInfo type */
    Hashmap* dependencies[_UNIT_DEPENDENCY_MAX];

    /* Similar, for RequiresMountsFor= path dependencies. The key is the path,
     * the value the UnitDependencyInfo type */
    Hashmap* requires_mounts_for;

    char* description;
    char** documentation;

    char* fragment_path; /* if loaded from a config file this is the primary
                            path to it */
    ...
    /* If this is a transient unit we are currently writing, this is where we
     * are writing it to */
    FILE* transient_file;

    /* If there is something to do with this unit, then this is the installed
     * job for it */
    Job* job;

    /* JOB_NOP jobs are special and can be installed without disturbing the real
     * job. */
    Job* nop_job;
    ...
    /* Updated whenever the (high-level) active state enters or leaves the
    ...
    /* Queue of units with StopWhenUnneeded set that shell be checked for
     * clean-up. */
    ...
    /* Used in SIGCHLD and sd_notify() message event invocation logic to avoid
     * that we dispatch the same event multiple times on the same unit. */
    unsigned sigchldgen;
    unsigned notifygen;
    ...
    /* Make sure we never enter endless loops with the check unneeded logic, or
     * the BindsTo= logic */
    ...
    char* cgroup_path;
    CGroupMask
        cgroup_realized_mask; /* In which hierarchies does this unit's cgroup
                                 exist? (only relevant on cgroup v1) */
    CGroupMask
        cgroup_enabled_mask; /* Which controllers are enabled (or more
                                correctly: enabled for the children) for this
                                unit's cgroup? (only relevant on cgroup v2) */
    CGroupMask cgroup_invalidated_mask; /* A mask specifiying controllers which
                                           shall be considered invalidated, and
                                           require re-realization */
    CGroupMask cgroup_members_mask; /* A cache for the controllers required by
                                       all children of this cgroup (only
                                       relevant for slice units) */
    ...
    /* Low-priority event source which is used to remove watched PIDs that have
     * gone away, and subscribe to any new ones which might have appeared. */
    ...
    /* Don't allow the user to stop this unit manually, allow stopping only
    ...
    /* For transient units: whether to add a bus track reference after creating
     * the unit */
    ...
    /* When writing transient unit files, stores which section we stored last.
     * If < 0, we didn't write any yet. If
     * == 0 we are in the [Unit] section, if > 0 we are in the unit
...
/* Returns true if neither persistent, nor runtime storage is requested, i.e.
 * this is a check invocation only */
...
typedef struct UnitVTable
{
    ...
    /* If greater than 0, the offset into the object where the pointer to
     * DynamicCreds is found, if the unit type has that. */
    size_t dynamic_creds_offset;

    /* The name of the configuration file section with the private settings of
     * this unit */
    ...
    /* During deserialization we only record the intended state to return to.
     * With coldplug() we actually put the deserialized state in effect. This is
     * where unit_notify() should be called to start things up. Note that this
     * callback is invoked *before* we leave the reloading state of the manager,
     * i.e. *before* we consider the reloading to be complete. Thus, this
     * callback should just restore the exact same state for any unit that was
     * in effect before the reload, i.e. units should not catch up with changes
     * happened during the reload. That's what catchup() below is for. */
    int (*coldplug)(Unit* u);

    /* This is called shortly after all units' coldplug() call was invoked, and
     * *after* the manager left the reloading state. It's supposed to catch up
     * with state changes due to external events we missed so far (for example
     * because they took place while we were reloading/reexecing) */
    void (*catchup)(Unit* u);
    ...
    /* When the unit is not running and no job for it queued we shall release
    ...
    int (*bus_set_property)(Unit* u, const char* name, sd_bus_message* message,
                            UnitWriteFlags flags, sd_bus_error* error);

    /* Called after at least one property got changed to apply the necessary
     * change */
    ...
    /* Returns the exit status to propagate in case of
     * FailureAction=exit/SuccessAction=exit; usually returns the exit code of
     * the "main" process of the service or similar. */
    int (*exit_status)(Unit* u);

    /* Like the enumerate() callback further down, but only enumerates the
     * perpetual units, i.e. all units that unconditionally exist and are always
     * active. The main reason to keep both enumeration functions separate is
     * philosophical: the state of perpetual units should be put in place by
     * coldplug(), while the state of those discovered through regular
     * enumeration should be put in place by catchup(), see below. */
    void (*enumerate_perpetual)(Manager* m);

    /* This is called for each unit type and should be used to enumerate units
     * already existing in the system internally and load them. However,
     * everything that is loaded here should still stay in inactive state. It is
     * the job of the catchup() call above to put the units into the discovered
     * state. */
    ...
    /* True if units of this type shall be startable only once and then never
     * again */
    bool once_only : 1;

    /* True if queued jobs of this type should be GC'ed if no other job needs
     * them anymore */
    bool gc_jobs : 1;
} UnitVTable;
...
int unit_add_node_dependency(Unit* u, const char* what, bool wants,
                             UnitDependency d, UnitDependencyMask mask);
...
int unit_write_setting(Unit* u, UnitWriteFlags flags, const char* name,
                       const char* data);
int unit_write_settingf(Unit* u, UnitWriteFlags mode, const char* name,
...
#define log_unit_full(unit, level, error, ...)                                 \
    ({                                                                         \
        const Unit* _u = (unit);                                               \
        _u ? log_object_internal(level, error, __FILE__, __LINE__, __func__,   \
                                 _u->manager->unit_log_field, _u->id,          \
                                 _u->manager->invocation_log_field,            \
                                 _u->invocation_id_string, ##__VA_ARGS__)      \
           : log_internal(level, error, __FILE__, __LINE__, __func__,          \
                          ##__VA_ARGS__);                                      \
    })

#define log_unit_debug(unit, ...)                                              \
    log_unit_full(unit, LOG_DEBUG, 0, ##__VA_ARGS__)
#define log_unit_info(unit, ...) log_unit_full(unit, LOG_INFO, 0, ##__VA_ARGS__)
#define log_unit_notice(unit, ...)                                             \
    log_unit_full(unit, LOG_NOTICE, 0, ##__VA_ARGS__)
#define log_unit_warning(unit, ...)                                            \
    log_unit_full(unit, LOG_WARNING, 0, ##__VA_ARGS__)
#define log_unit_error(unit, ...) log_unit_full(unit, LOG_ERR, 0, ##__VA_ARGS__)

#define log_unit_debug_errno(unit, error, ...)                                 \
    log_unit_full(unit, LOG_DEBUG, error, ##__VA_ARGS__)
#define log_unit_info_errno(unit, error, ...)                                  \
    log_unit_full(unit, LOG_INFO, error, ##__VA_ARGS__)
#define log_unit_notice_errno(unit, error, ...)                                \
    log_unit_full(unit, LOG_NOTICE, error, ##__VA_ARGS__)
#define log_unit_warning_errno(unit, error, ...)                               \
    log_unit_full(unit, LOG_WARNING, error, ##__VA_ARGS__)
#define log_unit_error_errno(unit, error, ...)                                 \
    log_unit_full(unit, LOG_ERR, error, ##__VA_ARGS__)

#define LOG_UNIT_MESSAGE(unit, fmt, ...)                                       \
    "MESSAGE=%s: " fmt, (unit)->id, ##__VA_ARGS__
