...
static const struct argp_option options[] = {
    ...
    {"invalidate", 'i', N_("TABLE"), 0, N_("Invalidate the specified cache")},
...
static struct argp argp = {options, parse_opt, NULL, doc, NULL, more_help};
...
static void __attribute__((noreturn)) invalidate_db(const char* dbname)
{
    ...
    ssize_t nbytes =
        TEMP_FAILURE_RETRY(send(sock, reqdata, reqlen, MSG_NOSIGNAL));
...
static void __attribute__((noreturn)) send_shutdown(void)
{
    int sock = nscd_open_socket();
    ...
    ssize_t nbytes =
        TEMP_FAILURE_RETRY(send(sock, &req, sizeof req, MSG_NOSIGNAL));
    close(sock);
...
static int monitor_child(int fd)
{
    ...
    /* The child terminated with an error, either via exit or some other
       abnormal method, like a segfault.  */
