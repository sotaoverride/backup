...
static int probe_ntfs(blkid_probe pr, const struct blkid_idmag* mag)
{
    ...
    if ((uint16_t)le16_to_cpu(ns->bpb.sector_size) * sectors_per_cluster >
        NTFS_MAX_CLUSTER_SIZE)
        return 1;

    /* Unused fields must be zero */
    if (le16_to_cpu(ns->bpb.reserved_sectors) ||
        le16_to_cpu(ns->bpb.root_entries) || le16_to_cpu(ns->bpb.sectors) ||
        le16_to_cpu(ns->bpb.sectors_per_fat) ||
        le32_to_cpu(ns->bpb.large_sectors) || ns->bpb.fats)
        return 1;
    ...
    if (ns->clusters_per_mft_record > 0)
        mft_record_size =
            ns->clusters_per_mft_record * sectors_per_cluster * sector_size;
    else
    ...
    DBG(LOWPROBE,
        ul_debug("NTFS: sector_size=%" PRIu16 ", mft_record_size=%" PRIu32 ", "
                 "sectors_per_cluster=%" PRIu32 ", nr_clusters=%" PRIu64 " "
                 "cluster_offset=%" PRIu64 "",
                 sector_size, mft_record_size, sectors_per_cluster, nr_clusters,
                 off));
    ...
    blkid_probe_sprintf_uuid(pr, (unsigned char*)&ns->volume_serial,
                             sizeof(ns->volume_serial), "%016" PRIX64,
                             le64_to_cpu(ns->volume_serial));
    return 0;
}
...
const struct blkid_idinfo ntfs_idinfo = {
    ...
    .magics = {{.magic = "NTFS    ", .len = 8, .sboff = 3}, {NULL}}};
