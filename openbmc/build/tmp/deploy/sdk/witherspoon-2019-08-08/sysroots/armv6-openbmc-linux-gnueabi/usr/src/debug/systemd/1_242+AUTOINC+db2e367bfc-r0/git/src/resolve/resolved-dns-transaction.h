...
enum DnsTransactionState
{
    ...
    DNS_TRANSACTION_NOT_FOUND, /* like NXDOMAIN, but when LLMNR/TCP connections
                                  fail */
...
#define DNS_TRANSACTION_IS_LIVE(state)                                         \
    IN_SET((state), DNS_TRANSACTION_NULL, DNS_TRANSACTION_PENDING,             \
           DNS_TRANSACTION_VALIDATING)
...
struct DnsTransaction
{
    ...
    /* If we got SERVFAIL back, we retry the lookup, using a lower feature level
     * than we used before. */
...
#define TRANSACTION_ATTEMPTS_MAX(p)                                            \
    (((p) == DNS_PROTOCOL_LLMNR)                                               \
         ...
         : (((p) == DNS_PROTOCOL_MDNS) ? MDNS_TRANSACTION_ATTEMPTS_MAX         \
