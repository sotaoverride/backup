...
errcode_t online_resize_fs(ext2_filsys fs, const char* mtpt, blk64_t* new_size,
                           int flags EXT2FS_ATTR((unused)))
{
        ...
        (access("/sys/fs/ext4/features/sparse_super2", R_OK) != 0))
    {
        com_err(program_name, 0,
                _("kernel does not support online "
    ...
    new_desc_blocks = ext2fs_div_ceil(
        ext2fs_div64_ceil(*new_size - fs->super->s_first_data_block,
        ...
        no_meta_bg_resize)
    {
        ...
        if ((ext2fs_blocks_count(sb) > MAX_32_NUM) || (*new_size > MAX_32_NUM))
        {
            com_err(
                program_name, 0,
    ...
    if (fd < 0)
    {
        com_err(program_name, errno, _("while trying to open mountpoint %s"),
                mtpt);
        exit(1);
    }
    ...
    percent = (ext2fs_r_blocks_count(sb) * 100.0) / ext2fs_blocks_count(sb);
    ...
    size =
        fs->group_desc_count * sb->s_blocks_per_group + sb->s_first_data_block;
    if (size > *new_size)
        size = *new_size;
    ...
    for (i = fs->group_desc_count; i < new_fs->group_desc_count; i++)
    {
        input.group = i;
        ...
        input.reserved_blocks = (blk_t)(percent * input.blocks_count / 100.0);
        ...
        if (use_old_ioctl && ioctl(fd, EXT2_IOC_GROUP_ADD, &input) == 0)
            continue;
        else
            use_old_ioctl = 0;
        ...
        {
            com_err(program_name, errno, _("While trying to add group #%d"),
                    input.group);
            exit(1);
        }
    }
