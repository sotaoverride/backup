...
/* --- prototypes --- */
static int main_selftest(int argc, char** argv);
...
/* --- functions --- */
static const char* sindent(guint n)
{
...
static void G_GNUC_PRINTF(1, 2) test_log_printfe(const char* format, ...)
{
    char* result;
    int r;
    va_list args;
...
static void testcase_close(long double duration, gint exit_status,
                           guint n_forks)
{
    gboolean success;
    ...
    test_log_printfe("%s<duration>%.6Lf</duration>\n", sindent(log_indent),
                     duration);
    success =
        exit_status == G_TEST_RUN_SUCCESS || exit_status == G_TEST_RUN_SKIPPED;
    test_log_printfe(
...
static void test_log_msg(GTestLogMsg* msg)
{
    switch (msg->log_type)
    {
        guint i;
        ...
        case G_TEST_LOG_SKIP_CASE:
            if (FALSE &&
                gtester_verbose) /* enable to debug test case skipping logic */
            {
                gchar* sc = g_strconcat(msg->strings[0], ":", NULL);
                gchar* sleft = g_strdup_printf("%-68s", sc);
                g_free(sc);
        ...
        case G_TEST_LOG_MAX_RESULT:
            test_log_printfe(
                "%s<performance minimize=\"%d\" maximize=\"%d\" "
                "value=\"%.16Lg\">\n",
                sindent(log_indent), msg->log_type == G_TEST_LOG_MIN_RESULT,
                msg->log_type == G_TEST_LOG_MAX_RESULT, msg->nums[0]);
            test_log_printfe("%s%s\n", sindent(log_indent + 2),
                             msg->strings[0]);
...
static gboolean child_report_cb(GIOChannel* source, GIOCondition condition,
                                gpointer data)
{
    ...
    do
    {
        ...
        GError* error = NULL;
        status = g_io_channel_read_chars(source, (gchar*)buffer, sizeof(buffer),
                                         &length, &error);
        ...
        g_clear_error(&error);
        /* ignore the io channel status, which will report intermediate EOFs for
         * non blocking fds */
        (void)status;
    } while (length > 0);
...
static gchar* queue_gfree(GSList** slistp, gchar* string)
{
    *slistp = g_slist_prepend(*slistp, string);
    return string;
}

static void unset_cloexec_fdp(gpointer fdp_data)
{
    int r, *fdp = fdp_data;
    do
...
static gboolean launch_test_binary(const char* binary, guint skip_tests)
{
    GTestLogBuffer* tlb;
    ...
    if (!g_unix_open_pipe(report_pipe, FD_CLOEXEC, &error))
    {
        if (subtest_mode_fatal)
            g_error("Failed to open pipe for test binary: %s: %s", binary,
                    error->message);
        else
            g_warning("Failed to open pipe for test binary: %s: %s", binary,
                      error->message);
        g_clear_error(&error);
        return FALSE;
    }
    ...
    if (gtester_list_tests)
        argv[i++] = "-l";
    if (subtest_seedstr)
        argv[i++] = queue_gfree(&free_list,
                                g_strdup_printf("--seed=%s", subtest_seedstr));
    argv[i++] = queue_gfree(&free_list,
                            g_strdup_printf("--GTestLogFD=%u", report_pipe[1]));
    if (skip_tests)
        argv[i++] = queue_gfree(
            &free_list, g_strdup_printf("--GTestSkipCount=%u", skip_tests));
    for (slist = subtest_paths; slist; slist = slist->next)
        argv[i++] = queue_gfree(&free_list,
                                g_strdup_printf("-p=%s", (gchar*)slist->data));
    for (slist = skipped_paths; slist; slist = slist->next)
        argv[i++] = queue_gfree(&free_list,
                                g_strdup_printf("-s=%s", (gchar*)slist->data));
    argv[i++] = NULL;

    g_spawn_async_with_pipes(
        NULL,                               /* g_get_current_dir() */
        (gchar**)argv, NULL,                /* envp */
        G_SPAWN_DO_NOT_REAP_CHILD,          /* G_SPAWN_SEARCH_PATH */
        unset_cloexec_fdp, &report_pipe[1], /* pre-exec callback */
        &pid, NULL,                         /* standard_input */
    ...
    if (error)
    {
        close(report_pipe[0]);
        if (subtest_mode_fatal)
            g_error("Failed to execute test binary: %s: %s", argv[0],
                    error->message);
        else
    ...
    if (report_pipe[0] >= 0)
    {
        ...
        child_report_cb_id = g_io_add_watch_full(
            ioc_report, G_PRIORITY_DEFAULT - 1, G_IO_IN | G_IO_ERR | G_IO_HUP,
            child_report_cb, tlb, NULL);
        g_io_channel_unref(ioc_report);
    }
    g_child_watch_add_full(G_PRIORITY_DEFAULT + 1, pid, child_watch_cb, NULL,
                           NULL);
...
static void launch_test(const char* binary)
{
...
static void usage(gboolean just_version)
{
    if (just_version)
    {
    ...
    g_print("  -k, --keep-going              Continue running after tests "
            "failed\n");
    g_print(
    ...
    g_print("  -p=TESTPATH                   Only start test cases matching "
            "TESTPATH\n");
    g_print(
        "  -s=TESTPATH                   Skip test cases matching TESTPATH\n");
    g_print("  --seed=SEEDSTRING             Start tests with random seed "
            "SEEDSTRING\n");
...
static void parse_args(gint* argc_p, gchar*** argv_p)
{
    ...
    for (i = 1; i < argc; i++)
    {
        ...
        else if (strcmp("--test-arg", argv[i]) == 0 ||
                 strncmp("--test-arg=", argv[i], 11) == 0)
        {
...
int main(int argc, char** argv)
{
    ...
    if (output_filename)
    {
        ...
        errsv = errno;
        if (log_fd < 0)
            g_error("Failed to open log file '%s': %s", output_filename,
                    g_strerror(errsv));
    }
...
static void fixture_setup(guint* fix, gconstpointer test_data)
{
    g_assert_cmphex(*fix, ==, 0);
    *fix = 0xdeadbeef;
}
static void fixture_test(guint* fix, gconstpointer test_data)
{
...
static void fixture_teardown(guint* fix, gconstpointer test_data)
{
    g_assert_cmphex(*fix, ==, 0xdeadbeef);
}
...
static int main_selftest(int argc, char** argv)
{
    ...
    g_test_add("/gtester/fixture-test", guint, NULL, fixture_setup,
               fixture_test, fixture_teardown);
    return g_test_run();
}
