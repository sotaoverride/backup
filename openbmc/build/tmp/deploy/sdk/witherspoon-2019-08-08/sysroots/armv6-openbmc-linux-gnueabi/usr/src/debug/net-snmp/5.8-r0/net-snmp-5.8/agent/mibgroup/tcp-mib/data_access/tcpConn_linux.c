...
static int linux_states[12] = {1, 5, 3, 4, 6, 7, 11, 1, 8, 9, 2, 10};
 ...
 * delete an entry
 */
int netsnmp_arch_tcpconn_entry_delete(netsnmp_tcpconn_entry* entry)
{
    if (NULL == entry)
        return -1;
 ...
 * @retval !0 errors
 */
static int _load4(netsnmp_container* container, u_int load_flags)
{
    int rc = 0;
    FILE* in;
    char line[160];
...
#define PROCFILE "/proc/net/tcp"
     ...
     * retrnsmt   uid  timeout inode 0: 00000000:8000 00000000:0000 0A
     * 00000000:00000000 00:00000000 00000000    29        0 1028 1 df7b1b80 300
     * 0 0 2 -1
     */
    while (fgets(line, sizeof(line), in))
    {
        ...
        if (6 != (rc = sscanf(line,
                              "%*d: %8[0-9A-Z]:%x %8[0-9A-Z]:%x %x %*x:%*x "
                              "%*x:%*x %*x %*x %*x %llu",
                              local_addr, &local_port, remote_addr,
                              &remote_port, &tmp_state, &inode)))
        {
        ...
        if ((8 != buf_len) || (-1 == netsnmp_addrstr_hton(local_addr, 8)))
        {
            DEBUGMSGT(("verbose:access:tcpconn:container",
        ...
        rc = netsnmp_hex_to_binary(&tmp_ptr, &buf_len, &offset, 0, local_addr,
                                   NULL);
        ...
        if ((8 != buf_len) || (-1 == netsnmp_addrstr_hton(remote_addr, 8)))
        {
            DEBUGMSGT(("verbose:access:tcpconn:container",
        ...
        rc = netsnmp_hex_to_binary(&tmp_ptr, &buf_len, &offset, 0, remote_addr,
                                   NULL);
 ...
 * @retval !0 errors
 */
static int _load6(netsnmp_container* container, u_int load_flags)
{
    int rc = 0;
    FILE* in;
    char line[360];
...
#define PROCFILE "/proc/net/tcp6"
    ...
    {
        DEBUGMSGTL(
            ("access:tcpconn:container", "could not open " PROCFILE "\n"));
        return -2;
    }
     ...
     *
     *   sl  local_address rem_address   st tx_queue rx_queue tr tm->when
     * retrnsmt   uid  timeout inode 0: 00000000000000000000000000000001:1466
     * 00000000000000000000000000000000:0000 0A 00000000:00000000 00:00000000
     * 00000000   500        0 326699 1 efb81580 3000 0 0 2 -1
     */
    while (fgets(line, sizeof(line), in))
    {
        netsnmp_tcpconn_entry* entry;
        ...
        if (6 != (rc = sscanf(line,
                              "%*d: %47[0-9A-Z]:%x %47[0-9A-Z]:%x %x %*x:%*x "
                              "%*x:%*x %*x %*x %*x %llu",
                              local_addr, &local_port, remote_addr,
                              &remote_port, &tmp_state, &inode)))
        {
        ...
        /** the addr string may need work */
        buf_len = strlen(local_addr);
        if ((32 != buf_len) || (-1 == netsnmp_addrstr_hton(local_addr, 32)))
        {
            DEBUGMSGT(("verbose:access:tcpconn:container",
                       " error processing local address\n"));
            netsnmp_access_tcpconn_entry_free(entry);
        ...
        buf_len = strlen((char*)remote_addr);
        if ((32 != buf_len) || (-1 == netsnmp_addrstr_hton(remote_addr, 32)))
        {
            DEBUGMSGT(("verbose:access:tcpconn:container",
                       " error processing remote address\n"));
            netsnmp_access_tcpconn_entry_free(entry);
