...
static void xmit_database(krb5_context context, krb5_auth_context auth_context,
...
static void get_tickets(krb5_context context)
{
    ...
    retval =
        sn2princ_realm(context, NULL, KPROP_SERVICE_NAME, realm, &my_principal);
    if (retval)
    {
                                  ...
                                  krb5_principal me, krb5_creds** new_creds)
{
    ...
    retval = krb5_sendauth(context, auth_context, &fd, kprop_version, me,
                           creds.server, AP_OPTS_MUTUAL_REQUIRED, NULL, &creds,
                           NULL, &error, &rep_result, new_creds);
    if (retval)
    {
                          ...
                          int in_database_size)
{
    ...
    if (retval)
    {
        send_error(context, my_creds, fd, "failed while initializing i_vector",
                   retval);
    ...
    while ((n = read(database_fd, buf, sizeof(buf))))
    {
        ...
        if (retval)
        {
            snprintf(buf, sizeof(buf),
                     "while encoding database block starting at %d", sent_size);
...
static void update_last_prop_file(char* hostname, char* file_name)
{
    ...
    if (asprintf(&file_last_prop, "%s.%s%s", file_name, hostname, last_prop) <
        0)
    {
