                ...
                if (t == XML_ATTRIBUTE_NAME)
                {
                    ...
                    if (streq_ptr(name, "name"))
                        ...
                        return log_error_errno(
                            ...
                            "Unexpected <annotation> attribute %s.", name);
                }
...
static int parse_xml_node(Context* context, const char* prefix,
                          unsigned n_depth)
{
    ...
    _cleanup_free_ char *node_path = NULL, *argument_type = NULL,
        ...
        switch (state)
        {
            ...
            case STATE_NODE:
                         ...
                         (t == XML_TAG_CLOSE && streq_ptr(name, "node")))
                {
                    ...
                    {
                        r = context->ops->on_path(node_path ? node_path : np,
                                                  context->userdata);
                        if (r < 0)
                            return r;
                    }
            ...
            case STATE_INTERFACE:
                ...
                if (t == XML_ATTRIBUTE_NAME)
                {
                    if (streq_ptr(name, "name"))
                        ...
                        return log_error_errno(
                            SYNTHETIC_ERRNO(EBADMSG),
                            "Unexpected <interface> attribute %s.", name);
                }
                else if (t == XML_TAG_OPEN)
                {
                    ...
                    {
                        context->member_flags |=
                            SD_BUS_VTABLE_PROPERTY_EMITS_CHANGE;
                        state = STATE_PROPERTY;
                    }
                    else if (streq_ptr(name, "annotation"))
                    {
                        r = parse_xml_annotation(context,
                                                 &context->interface_flags);
                        if (r < 0)
                            return r;
                    }
                    else
                        return log_error_errno(SYNTHETIC_ERRNO(EINVAL),
                                               "Unexpected <interface> tag %s.",
                                               name);
                }
                else if (t == XML_TAG_CLOSE_EMPTY ||
                    ...
                    if (n_depth == 0)
                    {
                        if (context->ops->on_interface)
                        {
                            r = context->ops->on_interface(
                                ...
                                context->interface_flags, context->userdata);
                            if (r < 0)
                                return r;
                        }
            ...
            case STATE_METHOD:
                ...
                if (t == XML_ATTRIBUTE_NAME)
                {
                    if (streq_ptr(name, "name"))
                        ...
                        return log_error_errno(
                            SYNTHETIC_ERRNO(EBADMSG),
                            "Unexpected <method> attribute %s", name);
                }
                else if (t == XML_TAG_OPEN)
                {
                         ...
                         (t == XML_TAG_CLOSE && streq_ptr(name, "method")))
                {
                    ...
                    if (n_depth == 0)
                    {
                        ...
                        {
                            r = context->ops->on_method(
                                context->interface_name, context->member_name,
                                context->member_signature,
                                context->member_result, context->member_flags,
                                context->userdata);
                            if (r < 0)
                                return r;
                        }
            ...
            case STATE_METHOD_ARG:
                ...
                if (t == XML_ATTRIBUTE_NAME)
                {
                    if (streq_ptr(name, "name"))
                        ...
                        return log_error_errno(
                            SYNTHETIC_ERRNO(EBADMSG),
                            "Unexpected method <arg> attribute %s.", name);
                }
                else if (t == XML_TAG_OPEN)
                {
                    if (streq_ptr(name, "annotation"))
                    {
                        ...
                        return log_error_errno(
                            SYNTHETIC_ERRNO(EINVAL),
                            "Unexpected method <arg> tag %s.", name);
                }
                else if (t == XML_TAG_CLOSE_EMPTY ||
                         (t == XML_TAG_CLOSE && streq_ptr(name, "arg")))
                {
                    ...
                    if (n_depth == 0)
                    {
                        ...
                        if (argument_type)
                        {
                            if (!argument_direction ||
                                streq(argument_direction, "in"))
                            {
                                if (!strextend(&context->member_signature,
                                               argument_type, NULL))
                                    return log_oom();
                            }
                            else if (streq(argument_direction, "out"))
                            {
                                if (!strextend(&context->member_result,
                                               argument_type, NULL))
                                    return log_oom();
                            }
                            else
                                log_error("Unexpected method <arg> direction "
                                          "value '%s'.",
                                          argument_direction);
                        }
            ...
            case STATE_SIGNAL:
                ...
                if (t == XML_ATTRIBUTE_NAME)
                {
                    if (streq_ptr(name, "name"))
                        ...
                        return log_error_errno(
                            SYNTHETIC_ERRNO(EBADMSG),
                            "Unexpected <signal> attribute %s.", name);
                }
                else if (t == XML_TAG_OPEN)
                {
                    if (streq_ptr(name, "arg"))
                         ...
                         (t == XML_TAG_CLOSE && streq_ptr(name, "signal")))
                {
                    ...
                    if (n_depth == 0)
                    {
                        ...
                        {
                            r = context->ops->on_signal(
                                context->interface_name, context->member_name,
                                context->member_signature,
                                context->member_flags, context->userdata);
                            if (r < 0)
                                return r;
                        }
            ...
            case STATE_SIGNAL_ARG:
                ...
                if (t == XML_ATTRIBUTE_NAME)
                {
                    if (streq_ptr(name, "name"))
                        ...
                        return log_error_errno(
                            SYNTHETIC_ERRNO(EBADMSG),
                            "Unexpected signal <arg> attribute %s.", name);
                }
                else if (t == XML_TAG_OPEN)
                {
                    if (streq_ptr(name, "annotation"))
                    {
                        ...
                        return log_error_errno(
                            SYNTHETIC_ERRNO(EINVAL),
                            "Unexpected signal <arg> tag %s.", name);
                }
                else if (t == XML_TAG_CLOSE_EMPTY ||
                         (t == XML_TAG_CLOSE && streq_ptr(name, "arg")))
                {
                    ...
                    if (argument_type)
                    {
                            ...
                            streq(argument_direction, "out"))
                        {
                            ...
                            log_error(
                                "Unexpected signal <arg> direction value '%s'.",
                                argument_direction);
            ...
            case STATE_PROPERTY:
                ...
                if (t == XML_ATTRIBUTE_NAME)
                {
                    if (streq_ptr(name, "name"))
                        ...
                        return log_error_errno(
                            SYNTHETIC_ERRNO(EBADMSG),
                            "Unexpected <property> attribute %s.", name);
                }
                else if (t == XML_TAG_OPEN)
                {
                         ...
                         (t == XML_TAG_CLOSE && streq_ptr(name, "property")))
                {
                    ...
                    if (n_depth == 0)
                    {
                        ...
                        {
                            r = context->ops->on_property(
                                context->interface_name, context->member_name,
                                context->member_signature,
                                context->member_writable, context->member_flags,
                                context->userdata);
                            if (r < 0)
                                return r;
                        }
...
int parse_xml_introspect(const char* prefix, const char* xml,
                         const XMLIntrospectOps* ops, void* userdata)
{
    Context context = {
        .ops = ops,
        .userdata = userdata,
