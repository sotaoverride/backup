...
static gcry_err_code_t gost_setkey(void* c, const byte* key, unsigned keylen)
{
    int i;
...
static u32 gost_val(GOST28147_context* ctx, u32 cm1, int subkey)
{
    cm1 += ctx->key[subkey];
...
static unsigned int _gost_encrypt_data(void* c, u32* o1, u32* o2, u32 n1,
                                       u32 n2)
{
    GOST28147_context* ctx = c;

    n2 ^= gost_val(ctx, n1, 0);
    n1 ^= gost_val(ctx, n2, 1);
    n2 ^= gost_val(ctx, n1, 2);
    n1 ^= gost_val(ctx, n2, 3);
    n2 ^= gost_val(ctx, n1, 4);
    n1 ^= gost_val(ctx, n2, 5);
    n2 ^= gost_val(ctx, n1, 6);
    n1 ^= gost_val(ctx, n2, 7);
    ...
    n2 ^= gost_val(ctx, n1, 7);
    n1 ^= gost_val(ctx, n2, 6);
    n2 ^= gost_val(ctx, n1, 5);
    n1 ^= gost_val(ctx, n2, 4);
    n2 ^= gost_val(ctx, n1, 3);
    n1 ^= gost_val(ctx, n2, 2);
    n2 ^= gost_val(ctx, n1, 1);
    n1 ^= gost_val(ctx, n2, 0);
...
static unsigned int gost_encrypt_block(void* c, byte* outbuf, const byte* inbuf)
{
    GOST28147_context* ctx = c;
    u32 n1, n2;
    unsigned int burn;
...
unsigned int _gcry_gost_enc_data(GOST28147_context* c, const u32* key, u32* o1,
                                 u32* o2, u32 n1, u32 n2, int cryptopro)
{
    if (cryptopro)
...
static unsigned int gost_decrypt_block(void* c, byte* outbuf, const byte* inbuf)
{
    GOST28147_context* ctx = c;
    u32 n1, n2;
...
static gpg_err_code_t gost_set_sbox(GOST28147_context* ctx, const char* oid)
{
    int i;
...
static gpg_err_code_t gost_set_extra_info(void* c, int what, const void* buffer,
                                          size_t buflen)
{
    GOST28147_context* ctx = c;
    gpg_err_code_t ec = 0;
