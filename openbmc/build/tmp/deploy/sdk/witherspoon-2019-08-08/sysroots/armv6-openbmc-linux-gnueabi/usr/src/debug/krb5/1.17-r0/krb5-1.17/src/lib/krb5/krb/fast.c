                          ...
                          krb5_ccache ccache, krb5_principal target_principal)
{
    ...
    if (retval == 0)
    {
        ...
        retval = krb5_mk_req_extended(context, &authcontext, AP_OPTS_USE_SUBKEY,
                                      NULL /*data*/, out_creds,
...
krb5_error_code krb5int_fast_tgs_armor(krb5_context context,
                                       struct krb5int_fast_request_state* state,
                                       ...
                                       krb5_data* target_realm)
{
    ...
    if (ccache)
    {
        ...
        if (retval == 0)
            retval =
                fast_armor_ap_request(context, state, ccache, target_principal);
        if (retval == 0)
        {
            existing_armor = state->armor_key;
            state->armor_key = NULL;
            retval =
                krb5_c_fx_cf2_simple(context, existing_armor, "explicitarmor",
                                     subkey, "tgsarmor", &state->armor_key);
        }
    }
    else
    {
        retval =
            krb5_c_fx_cf2_simple(context, subkey, "subkeyarmor", session_key,
...
krb5_error_code krb5int_fast_prep_req_body(
...
krb5_error_code krb5int_fast_as_armor(krb5_context context,
                                      struct krb5int_fast_request_state* state,
 ...
 */
static krb5_error_code make_tgs_outer_padata(krb5_pa_data* tgs,
                                             krb5_pa_data* fast,
                                             krb5_pa_data** other,
...
krb5_error_code krb5int_fast_prep_req(krb5_context context,
                                      struct krb5int_fast_request_state* state,
                                      krb5_kdc_req* request,
                                      ...
                                      krb5_data** encoded_request)
{
    ...
    if (retval == 0)
        retval = krb5_c_make_checksum(
            context, 0, state->armor_key, KRB5_KEYUSAGE_FAST_REQ_CHKSUM,
            to_be_checksummed, &armored_req->req_checksum);
    if (retval == 0)
    ...
    if (retval == 0)
        retval =
            encode_krb5_pa_fx_fast_request(armored_req, &encoded_armored_req);
    if (retval == 0)
    {
        ...
        {
            retval =
                make_tgs_outer_padata(tgs, pa, request->padata, &pa_tgs_array);
...
static krb5_error_code
    decrypt_fast_reply(krb5_context context,
                       struct krb5int_fast_request_state* state,
                       krb5_pa_data** in_padata, krb5_fast_response** response)
{
    krb5_error_code retval = 0;
    krb5_data scratch;
    ...
    if (retval == 0)
        retval =
            krb5_c_decrypt(context, state->armor_key, KRB5_KEYUSAGE_FAST_REP,
                           NULL, encrypted_response, &scratch);
    if (retval != 0)
...
krb5_error_code krb5int_fast_process_error(
    krb5_context context, struct krb5int_fast_request_state* state,
    krb5_error** err_replyptr, krb5_pa_data*** out_padata, krb5_boolean* retry)
{
    krb5_error_code retval = 0;
    ...
    { /*not FAST*/
        ...
        {
            retval =
                decode_krb5_padata_sequence(&err_reply->e_data, out_padata);
            if (retval != 0)
            {
...
krb5_error_code krb5int_fast_process_response(
    krb5_context context, struct krb5int_fast_request_state* state,
    krb5_kdc_rep* resp, krb5_keyblock** strengthen_key)
{
    krb5_error_code retval = 0;
    krb5_fast_response* fast_response = NULL;
    ...
    *strengthen_key = NULL;
    if (state->armor_key == 0)
        return 0;
    retval = decrypt_fast_reply(context, state, resp->padata, &fast_response);
    if (retval == 0)
    {
    ...
    if (retval == 0)
        retval = krb5_c_verify_checksum(
            context, state->armor_key, KRB5_KEYUSAGE_FAST_FINISHED,
            encoded_ticket, &fast_response->finished->ticket_checksum,
            &cksum_valid);
                                       ...
                                       krb5_keyblock* out_key)
{
    ...
    if (strengthen_key)
    {
        retval = krb5_c_fx_cf2_simple(
            context, (krb5_keyblock*)strengthen_key, "strengthenkey",
            (krb5_keyblock*)existing_key, "replykey", &key);
        if (retval == 0)
        {
...
void krb5int_fast_free_state(krb5_context context,
                             struct krb5int_fast_request_state* state)
{
    if (state == NULL)
        return;
...
krb5_pa_data* krb5int_find_pa_data(krb5_context context,
...
krb5_error_code krb5int_fast_verify_nego(
    krb5_context context, struct krb5int_fast_request_state* state,
    krb5_kdc_rep* rep, krb5_data* request, krb5_keyblock* decrypting_key,
...
krb5_boolean k5_upgrade_to_fast_p(krb5_context context,
                                  struct krb5int_fast_request_state* state,
                                  krb5_pa_data** padata)
{
    if (state->armor_key != NULL)
