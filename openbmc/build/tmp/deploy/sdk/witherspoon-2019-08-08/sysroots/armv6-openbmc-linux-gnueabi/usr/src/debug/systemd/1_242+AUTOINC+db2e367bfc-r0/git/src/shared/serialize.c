...
int serialize_item(FILE* f, const char* key, const char* value)
{
    ...
    /* Make sure that anything we serialize we can also read back again with
     * read_line() with a maximum line size of LONG_LINE_MAX. This is a safety
     * net only. All code calling us should filter this out earlier anyway. */
    if (strlen(key) + 1 + strlen(value) + 1 > LONG_LINE_MAX)
    {
        log_warning("Attempted to serialize overly long item '%s', refusing.",
                    key);
        return -EINVAL;
    }
...
int serialize_item_format(FILE* f, const char* key, const char* format, ...)
{
    ...
    if (k < 0 || (size_t)k >= sizeof(buf) ||
        strlen(key) + 1 + k + 1 > LONG_LINE_MAX)
    {
        log_warning("Attempted to serialize overly long item '%s', refusing.",
                    key);
        return -EINVAL;
...
int serialize_fd(FILE* f, FDSet* fds, const char* key, int fd)
{
    ...
    if (copy < 0)
        return log_error_errno(
            copy, "Failed to add file descriptor to serialization set: %m");
...
int serialize_dual_timestamp(FILE* f, const char* name, const dual_timestamp* t)
{
    ...
    return serialize_item_format(f, name, USEC_FMT " " USEC_FMT, t->realtime,
                                 t->monotonic);
}
...
int serialize_strv(FILE* f, const char* key, char** l)
{
    ...
    /* Returns the first error, or positive if anything was serialized, 0
     * otherwise. */
...
int deserialize_usec(const char* value, usec_t* ret)
{
    ...
    if (r < 0)
        return log_debug_errno(r, "Failed to parse usec value \"%s\": %m",
                               value);
