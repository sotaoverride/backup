                   ...
                   const void* const* args)
{
    ...
    else
    {
        ...
        decimalwc =
            _NL_CURRENT_WORD(LC_MONETARY, _NL_MONETARY_DECIMAL_POINT_WC);
    }
...
#undef PRINTF_FPHEX_FETCH
        ...
        if (info->is_long_double == 0 || sizeof(double) == sizeof(long double))
    {
       ...
       the decimal point.  */
        ...
        {
            wnumstr =
                _itowa_word(num, wnumbuf + (sizeof wnumbuf) / sizeof(wchar_t),
                            16, info->spec == 'A');
            numstr =
                _itoa_word(num, numbuf + sizeof numbuf, 16, info->spec == 'A');
        }
        else
        {
            wnumstr = _itowa(num, wnumbuf + sizeof wnumbuf / sizeof(wchar_t),
                             16, info->spec == 'A');
        ...
        if (precision != -1 && precision < numend - numstr)
        {
            ...
            do_round_away =
                round_away(negative, last_digit_value & 1,
                           next_digit_value >= 8, more_bits, rounding_mode);
        }
    ...
    wexpstr = _itowa_word(exponent, wexpbuf + sizeof wexpbuf / sizeof(wchar_t),
                          10, 0);
