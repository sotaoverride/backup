...
#if !defined(                                                                  \
    CPPLEXER_EXCEPTIONS_HPP_1A09DE1A_6D1F_4091_AF7F_5F13AB0D31AB_INCLUDED)
...
#define BOOST_WAVE_LEXER_THROW(cls, code, msg, line, column, name)             \
    {                                                                          \
        ...
        boost::throw_exception(                                                \
            cls(throwmsg.c_str(), cls::code, line, column, name));             \
    }                                                                          \
    /**/
#else
#include <sstream>
#define BOOST_WAVE_LEXER_THROW(cls, code, msg, line, column, name)             \
    {                                                                          \
        ...
        stream << std::ends;                                                   \
        boost::throw_exception(                                                \
            cls(stream.str().c_str(), cls::code, line, column, name));         \
    }                                                                          \
    /**/
...
#define BOOST_WAVE_LEXER_THROW_VAR(cls, codearg, msg, line, column, name)      \
    {                                                                          \
        ...
        stream.freeze(false);                                                  \
        boost::throw_exception(                                                \
            cls(throwmsg.c_str(), code, line, column, name));                  \
    }                                                                          \
    /**/
#else
#include <sstream>
#define BOOST_WAVE_LEXER_THROW_VAR(cls, codearg, msg, line, column, name)      \
    {                                                                          \
        ...
        stream << std::ends;                                                   \
        boost::throw_exception(                                                \
            cls(stream.str().c_str(), code, line, column, name));              \
    }                                                                          \
    /**/
#endif // BOOST_NO_STRINGSTREAM
#endif // BOOST_WAVE_LEXER_THROW
...
class BOOST_SYMBOL_VISIBLE lexing_exception : public cpplexer_exception
{
  public:
    ...
    {
        // error texts in this array must appear in the same order as the items
        // in the error enum above
        static char const*
            preprocess_exception_errors
                [] =
                    {
