...
#define FT_STRUCTURE WinMZ_HeaderRec
    ...
    FT_FRAME_START(64), FT_FRAME_USHORT_LE(magic), FT_FRAME_SKIP_BYTES(29 * 2),
    FT_FRAME_ULONG_LE(lfanew), FT_FRAME_END};
...
#define FT_STRUCTURE WinPE_RsrcDirEntryRec
    ...
    FT_FRAME_START(8), FT_FRAME_ULONG_LE(name), FT_FRAME_ULONG_LE(offset),
    FT_FRAME_END};
...
#define FT_STRUCTURE WinPE_RsrcDataEntryRec
    ...
    FT_FRAME_START(16),          FT_FRAME_ULONG_LE(offset_to_data),
    FT_FRAME_ULONG_LE(size),     FT_FRAME_ULONG_LE(code_page),
    FT_FRAME_ULONG_LE(reserved), FT_FRAME_END};
...
static FT_Error fnt_face_get_dll_font(FNT_Face face, FT_Int face_instance_index)
{
    ...
    if (mz_header.magic == WINFNT_MZ_MAGIC)
    {
        ...
        if (ne_header.magic == WINFNT_NE_MAGIC)
        {
            ...
            FT_ULong res_offset =
                mz_header.lfanew + ne_header.resource_tab_offset;
        ...
        else if (ne_header.magic == WINFNT_PE_MAGIC)
        {
            ...
            FT_TRACE2(("magic %04lx, machine %02x, number_of_sections %u, "
                       ...
                       pe32_header.size_of_optional_header, pe32_header.magic32,
                       pe32_header.rsrc_virtual_address,
        ...
        Found_rsrc_section:
                 ...
                 i++)
            {
                ...
                name_dir_offset =
                    pe32_section.pointer_to_raw_data + dir_entry1.offset;
                     ...
                     j++)
                {
                    ...
                    if (!(dir_entry2.offset &
                          0x80000000UL) /* DataIsDirectory */)
                    {
                        error = FT_THROW(Invalid_File_Format);
                        goto Exit;
                    }
                    ...
                    lang_dir_offset =
                        pe32_section.pointer_to_raw_data + dir_entry2.offset;
                         ...
                         k++)
                    {
                        ...
                        {
                            if (FT_STREAM_SEEK(root_dir_offset +
                                               dir_entry3.offset) ||
                                FT_STREAM_READ_FIELDS(
                                    winpe_rsrc_data_entry_fields, &data_entry))
                                goto Exit;
...
static FT_Error FNT_Face_Init(FT_Stream stream, FT_Face fntface, /* FNT_Face */
                              FT_Int face_instance_index, FT_Int num_params,
                              FT_Parameter* params)
{
    FNT_Face face = (FNT_Face)fntface;
    FT_Error error;
    ...
    /* with relevant information                   */
    {
        ...
        root->num_glyphs =
            font->header.last_char - font->header.first_char + 1 + 1;
        ...
        if (FT_REALLOC(font->family_name, family_size,
...
static FT_Error FNT_Load_Glyph(FT_GlyphSlot slot, FT_Size size,
                               FT_UInt glyph_index, FT_Int32 load_flags)
{
    ...
    /* allocate and build bitmap */
    {
        ...
        if (!pitch || offset + pitch * bitmap->rows > font->header.file_size)
        {
