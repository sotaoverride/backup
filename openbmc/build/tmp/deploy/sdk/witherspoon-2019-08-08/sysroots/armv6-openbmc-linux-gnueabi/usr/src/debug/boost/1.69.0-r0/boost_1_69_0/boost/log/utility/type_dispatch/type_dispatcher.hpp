...
class type_dispatcher
{
  public:
    ...
    class callback_base
    {
      ...
      public:
        explicit callback_base(void* visitor = 0,
                               void* tramp = 0) BOOST_NOEXCEPT
            : m_pVisitor(visitor),
              m_pTrampoline(tramp)
        {
        }
        template <typename ValueT>
        explicit callback_base(void* visitor,
                               void (*tramp)(void*,
                                             ValueT const&)) BOOST_NOEXCEPT
            : m_pVisitor(visitor)
        {
            typedef void (*trampoline_t)(void*, ValueT const&);
            BOOST_STATIC_ASSERT_MSG(
                sizeof(trampoline_t) == sizeof(void*),
                "Boost.Log: Unsupported platform, the size of a function "
                "pointer differs from the size of a pointer");
            union
            {
                void* as_pvoid;
    ...
    template <typename T>
    class callback : private callback_base
    {
      private:
      ...
      public:
        ...
        explicit callback(callback_base const& base) BOOST_NOEXCEPT
            : callback_base(base)
        {
        }
    ...
    /*!
     * This interface is used by type dispatchers to consume the dispatched
     * value.
     */
    template <typename T>
    class callback
  ...
  protected:
    ...
    typedef callback_base (*get_callback_impl_type)(type_dispatcher*,
  ...
  protected:
     ...
     */
    explicit type_dispatcher(get_callback_impl_type get_callback_impl)
        BOOST_NOEXCEPT : m_get_callback_impl(get_callback_impl)
    {
    }

    // Destructor and copying can only be called from the derived classes
    BOOST_DEFAULTED_FUNCTION(~type_dispatcher(), {})
    BOOST_DEFAULTED_FUNCTION(type_dispatcher(type_dispatcher const& that),
                             : m_get_callback_impl(that.m_get_callback_impl){})
    BOOST_DEFAULTED_FUNCTION(
        type_dispatcher& operator=(type_dispatcher const& that), {
            m_get_callback_impl = that.m_get_callback_impl;
            return *this;
        })
  ...
  public:
     ...
     *
     * \return The type-specific callback or an empty value, if the type is not
     * supported
     */
    template <typename T>
    ...
    {
        return callback<T>(
            (this->m_get_callback_impl)(this, typeindex::type_id<T>()));
    }
};
