...
static int usage(void)
{
    ...
    printf("\n");
    printf(
        "This tool reads relative events from the kernel and calculates\n"
...
static inline double get_frequency(uint64_t last, uint64_t current)
{
    return 1000000.0 / (current - last);
}

static inline void push_frequency(struct measurements* m, double freq)
{
    if (m->nfrequencies == m->frequencies_sz)
    {
        m->frequencies_sz += 100;
        m->frequencies =
            realloc(m->frequencies, m->frequencies_sz * sizeof *m->frequencies);
        if (!m->frequencies)
            abort();
    }
...
static int print_current_values(const struct measurements* m)
{
...
static int handle_event(struct measurements* m, const struct input_event* ev)
{
    if (ev->type == EV_SYN)
    {
...
static int mainloop(struct libevdev* dev, struct measurements* m)
{
    struct pollfd fds[2];
    sigset_t mask;
...
static inline double mean_frequency(struct measurements* m)
{
    int idx;
...
static inline const char* bustype(int bustype)
{
    const char* bus;
...
static void print_summary(struct libevdev* dev, struct measurements* m)
{
    int res;
    ...
    printf("Entry for hwdb match (replace XXX with the resolution in DPI):\n"
           ...
           " MOUSE_DPI=XXX@%d\n",
           bustype(libevdev_get_id_bustype(dev)), libevdev_get_id_vendor(dev),
           libevdev_get_id_product(dev), libevdev_get_name(dev),
           (int)m->max_frequency);
}
...
int main(int argc, char** argv)
{
    ...
    if (libevdev_grab(dev, LIBEVDEV_GRAB) != 0)
    {
        fprintf(
            stderr,
            "Error: cannot grab the device, something else is grabbing it.\n");
