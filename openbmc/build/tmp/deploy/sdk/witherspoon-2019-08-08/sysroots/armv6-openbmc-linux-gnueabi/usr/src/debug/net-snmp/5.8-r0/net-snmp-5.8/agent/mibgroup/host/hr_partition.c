...
#endif
#ifdef darwin
extern int Get_HR_Disk_Label(char* string, size_t str_len, const char* devfull);
#endif
...
struct variable4 hrpartition_variables[] = {{HRPART_INDEX,
                                             ASN_INTEGER,
...
int header_hrpartition(struct variable* vp, oid* name, size_t* length,
...
#define HRPART_ENTRY_NAME_LENGTH 12
    ...
    {
        LowDiskIndex =
            (name[HRPART_DISK_NAME_LENGTH] & ((1 << HRDEV_TYPE_SHIFT) - 1));
...
u_char* var_hrpartition(struct variable* vp, oid* name, size_t* length,
                        int exact, size_t* var_len, WriteMethod** write_method)
{
    int part_idx;
    ...
    switch (vp->magic)
    {
        ...
        default:
            DEBUGMSGTL(
                ("snmpd", "unknown sub-id %d in var_hrpartition\n", vp->magic));
    }
    return NULL;
}
...
static int Get_Next_HR_Partition(void)
{
    ...
    DEBUGMSGTL(
        ("host/hr_partition", "Get_Next_HR_Partition %d\n", HRP_DiskIndex));
    if (HRP_DiskIndex == -1)
    {
        return 0;
    }
    ...
    {
        DEBUGMSGTL(("host/hr_partition", "Get_Next_HR_Partition: %s (:%d)\n",
                    string, HRP_index));
...
static void Save_HR_Partition(int disk_idx, int part_idx)
{
    HRP_savedDiskIndex = disk_idx;
    HRP_savedPartIndex = part_idx;
    (void)Get_Next_HR_Disk_Partition(HRP_savedName, sizeof(HRP_savedName),
                                     HRP_index);
#ifdef NETSNMP_CAN_GET_DISK_LABEL
    (void)Get_HR_Disk_Label(HRP_savedLabel, sizeof(HRP_savedLabel),
                            HRP_savedName);
#endif
}
