...
int udev_builtin_hwdb_lookup(sd_device* dev, const char* prefix,
                             const char* modalias, const char* filter,
                             bool test)
{
                                    ...
                                    const char* filter, bool test)
{
    ...
    for (d = srcdev; d;)
    {
            ...
            streq(devtype, "usb_device"))
        {
            ...
            /* avoid looking at any parent device, they are usually just a USB
             * hub */
            last = true;
        }
...
static int builtin_hwdb(sd_device* dev, int argc, char* argv[], bool test)
{
    ...
    if (argv[optind])
    {
        ...
        if (r == 0)
            return log_device_debug_errno(dev, SYNTHETIC_ERRNO(ENODATA),
...
/* called every couple of seconds during event activity; 'true' if config has
 * changed */
