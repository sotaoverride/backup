...
static const GOptionEntry entries[] = {
    ...
    {"append", 'a', 0, G_OPTION_ARG_NONE, &append, N_("Append to end of file"),
     NULL},
    {"private", 'p', 0, G_OPTION_ARG_NONE, &priv,
     N_("When creating, restrict access to the current user"), NULL},
    {"unlink", 'u', 0, G_OPTION_ARG_NONE, &replace_dest,
    ...
    /* Translators: The "etag" is a token allowing to verify whether a file has
       been modified */
    {"print-etag", 'v', 0, G_OPTION_ARG_NONE, &print_etag,
     N_("Print new etag at end"), NULL},
    /* Translators: The "etag" is a token allowing to verify whether a file has
       been modified */
    {"etag", 'e', 0, G_OPTION_ARG_STRING, &etag,
...
static gboolean save(GFile* file)
{
    GOutputStream* out;
    ...
    if (create)
        ...
        out = (GOutputStream*)g_file_replace(file, etag, backup, flags, NULL,
                                             &error);
    if (out == NULL)
    {
...
out:
    ...
    if (close_res && print_etag)
    {
        ...
        if (etag)
            ...
            /* Translators: The "etag" is a token allowing to verify whether a
...
int handle_save(int argc, char* argv[], gboolean do_help)
{
    ...
    g_option_context_set_summary(
        context, _("Read from standard input and save to DEST."));
