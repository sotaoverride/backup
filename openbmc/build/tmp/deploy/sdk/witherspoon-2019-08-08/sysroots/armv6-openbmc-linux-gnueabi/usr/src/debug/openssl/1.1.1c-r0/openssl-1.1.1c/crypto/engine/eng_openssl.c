...
static int openssl_digests(ENGINE* e, const EVP_MD** digest, const int** nids,
                           int nid);
#endif
...
static int ossl_pkey_meths(ENGINE* e, EVP_PKEY_METHOD** pmeth, const int** nids,
                           int nid);
#endif
...
static const EVP_CIPHER* test_r4_40_cipher(void)
{
    if (r4_40_cipher == NULL)
    {
        ...
        if ((cipher = EVP_CIPHER_meth_new(NID_rc4, 1, 5 /* 40 bits */)) ==
                NULL ||
            !EVP_CIPHER_meth_set_iv_length(cipher, 0) ||
            !EVP_CIPHER_meth_set_flags(cipher, EVP_CIPH_VARIABLE_LENGTH) ||
            !EVP_CIPHER_meth_set_init(cipher, test_rc4_init_key) ||
...
static const EVP_MD* test_sha_md(void)
{
    if (sha1_md == NULL)
    {
        ...
        if ((md = EVP_MD_meth_new(NID_sha1, NID_sha1WithRSAEncryption)) ==
                NULL ||
...
static int ossl_hmac_ctrl_str(EVP_PKEY_CTX* ctx, const char* type,
                              const char* value)
{
    if (!value)
    {
...
static int ossl_pkey_meths(ENGINE* e, EVP_PKEY_METHOD** pmeth, const int** nids,
                           int nid)
{
    static int ossl_pkey_nids[] = {EVP_PKEY_HMAC, 0};
    if (!pmeth)
    {
        *nids = ossl_pkey_nids;
        return 1;
