/* nssel.c
 *
 * The io waiter is a helper object enabling us to wait on a set of streams to
 * become ready for IO - this is modelled after select(). We need this, because
...
DEFobjStaticHelpers DEFobjCurrIf(glbl)
     ...
     */
    static rsRetVal loadDrvr(nssel_t* pThis)
{
    DEFiRet;
    uchar* pBaseDrvrName;
    ...
    if (snprintf((char*)szDrvrName, sizeof(szDrvrName), "lmnsdsel_%s",
                 pBaseDrvrName) == sizeof(szDrvrName))
     ...
     */
    CHKiRet(obj.UseObj(__FILE__, szDrvrName + 2, DONT_LOAD_LIB,
                       (void*)&pThis->Drvr));
...
BEGINobjConstruct(
    nssel) /* be sure to specify the object type also in END macro! */
    ENDobjConstruct(nssel)

    /* destructor for the nssel object */
    BEGINobjDestruct(nssel) /* be sure to specify the object type also in END
                               and CODESTART macros! */
    CODESTARTobjDestruct(nssel) if (
        pThis->pDrvrData != NULL) pThis->Drvr.Destruct(&pThis->pDrvrData);
...
if (pThis->pDrvrName != NULL)
{
    obj.ReleaseObj(__FILE__, pThis->pDrvrName + 2, DONT_LOAD_LIB,
                   (void*)&pThis->Drvr);
    free(pThis->pDrvrName);
}
ENDobjDestruct(nssel)
    ...
    static rsRetVal ConstructFinalize(nssel_t* pThis)
{
    DEFiRet;
 ...
 */
static rsRetVal SetDrvrName(nssel_t* pThis, uchar* pszName)
{
    DEFiRet;
    ISOBJ_TYPE_assert(pThis, nssel);
 ...
 */
static rsRetVal Add(nssel_t* pThis, netstrm_t* pStrm, nsdsel_waitOp_t waitOp)
{
    DEFiRet;
 ...
 */
static rsRetVal Wait(nssel_t* pThis, int* piNumReady)
{
    DEFiRet;
    ISOBJ_TYPE_assert(pThis, nssel);
 ...
 * level driver requires some IO which is hidden from the upper layer point of
 * view. rgerhards, 2008-04-23
 */
static rsRetVal IsReady(nssel_t* pThis, netstrm_t* pStrm,
                        nsdsel_waitOp_t waitOp, int* pbIsReady,
                        int __attribute__((unused)) * piNumReady)
{
    ...
    assert(piNumReady != NULL);
    iRet = pThis->Drvr.IsReady(pThis->pDrvrData, pStrm->pDrvrData, waitOp,
                               pbIsReady);
    RETiRet;
}
...
BEGINobjQueryInterface(nssel)
    CODESTARTobjQueryInterface(nssel) if (pIf->ifVersion !=
                                          nsselCURR_IF_VERSION)
...
finalize_it
    : ENDobjQueryInterface(nssel)
       ...
       */
      BEGINObjClassExit(
          nssel, OBJ_IS_LOADABLE_MODULE) /* CHANGE class also in END MACRO! */
