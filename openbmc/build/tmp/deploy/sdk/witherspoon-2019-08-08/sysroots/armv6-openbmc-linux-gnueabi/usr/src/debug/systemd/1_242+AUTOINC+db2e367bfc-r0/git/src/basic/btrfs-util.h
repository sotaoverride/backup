...
typedef enum BtrfsSnapshotFlags
{
    ...
    BTRFS_SNAPSHOT_FALLBACK_DIRECTORY =
        1 << 4, /* If the destination doesn't support subvolumes, reflink/copy
                   instead */
    BTRFS_SNAPSHOT_FALLBACK_IMMUTABLE =
        1 << 5, /* When we can't create a subvolume, use the FS_IMMUTABLE
...
int btrfs_subvol_snapshot_fd_full(int old_fd, const char* new_path,
                                  ...
                                  void* userdata);
static inline int btrfs_subvol_snapshot_fd(int old_fd, const char* new_path,
                                           BtrfsSnapshotFlags flags)
{
    return btrfs_subvol_snapshot_fd_full(old_fd, new_path, flags, NULL, NULL,
                                         NULL);
}
...
int btrfs_subvol_snapshot_full(const char* old_path, const char* new_path,
                               ...
                               copy_progress_bytes_t progress_bytes,
                               void* userdata);
static inline int btrfs_subvol_snapshot(const char* old_path,
                                        const char* new_path,
                                        BtrfsSnapshotFlags flags)
{
    return btrfs_subvol_snapshot_full(old_path, new_path, flags, NULL, NULL,
                                      NULL);
}
