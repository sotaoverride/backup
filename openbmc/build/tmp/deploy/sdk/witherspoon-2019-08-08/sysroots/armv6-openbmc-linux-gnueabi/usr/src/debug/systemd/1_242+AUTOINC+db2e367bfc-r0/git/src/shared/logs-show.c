...
/* up to three lines (each up to 100 characters) or 300 characters, whichever is
 * less */
...
static int parse_field(const void* data, size_t length, const char* field,
                       size_t field_len, char** target, size_t* target_len)
{
    size_t nl;
    char* buf;
...
#define PARSE_FIELD_VEC_ENTRY(_field, _target, _target_len)                    \
    {                                                                          \
        .field = _field, .field_len = strlen(_field), .target = _target,       \
        .target_len = _target_len                                              \
    }

static int parse_fieldv(const void* data, size_t length,
                        const ParseFieldVec* fields, unsigned n_fields)
{
    unsigned i;
    ...
    for (i = 0; i < n_fields; i++)
    {
        ...
        r = parse_field(data, length, f->field, f->field_len, f->target,
                        f->target_len);
        if (r < 0)
            return r;
        else if (r > 0)
...
static bool print_multiline(FILE* f, unsigned prefix, unsigned n_columns,
                            OutputFlags flags, int priority,
                            ...
                            size_t highlight[2])
{
    ...
    for (pos = message; pos < message + message_len; pos = end + 1, line++)
    {
         ...
         * or not at all.
         */
        tail_line = line + 1 == PRINT_LINE_THRESHOLD ||
                    end + 1 >= message + PRINT_CHAR_THRESHOLD;

        if (flags & (OUTPUT_FULL_WIDTH | OUTPUT_SHOW_ALL) ||
            (prefix + len + 1 < n_columns && !tail_line))
        {
            if (highlight && (size_t)(pos - message) <= highlight[0] &&
                highlight[0] < (size_t)len)
            {

                fprintf(f, "%*s%s%.*s", continuation * prefix, "", color_on,
                        (int)highlight[0], pos);
                fprintf(f, "%s%.*s", highlight_on,
                        (int)(MIN((size_t)len, highlight[1]) - highlight[0]),
                        pos + highlight[0]);
                if ((size_t)len > highlight[1])
                    fprintf(f, "%s%.*s", color_on, (int)(len - highlight[1]),
                            pos + highlight[1]);
                fprintf(f, "%s\n", color_off);
            }
            else
                fprintf(f, "%*s%s%.*s%s\n", continuation * prefix, "", color_on,
                        len, pos, color_off);
            continue;
        }
...
static int output_timestamp_monotonic(FILE* f, sd_journal* j,
                                      const char* monotonic)
{
    ...
    fprintf(f, "[%5" PRI_USEC ".%06" PRI_USEC "]", t / USEC_PER_SEC,
            t % USEC_PER_SEC);
    return 1 + 5 + 1 + 6 + 1;
}

static int output_timestamp_realtime(FILE* f, sd_journal* j, OutputMode mode,
                                     OutputFlags flags, const char* realtime)
{
    ...
    else
    {
        ...
        switch (mode)
        {
            ...
            case OUTPUT_SHORT_UNIX:
                xsprintf(buf, "%10" PRI_TIME ".%06" PRIu64, t,
                         x % USEC_PER_SEC);
                break;
            ...
            case OUTPUT_SHORT_PRECISE:
                ...
                if (mode == OUTPUT_SHORT_PRECISE)
                {
                    ...
                    r = snprintf(buf + strlen(buf), k, ".%06" PRIu64,
                                 x % USEC_PER_SEC);
...
static int output_short(FILE* f, sd_journal* j, OutputMode mode,
                        unsigned n_columns, OutputFlags flags,
                        Set* output_fields, const size_t highlight[2])
{
    ...
    _cleanup_free_ char *hostname = NULL, *identifier = NULL, *comm = NULL,
                        *pid = NULL, *fake_pid = NULL, *message = NULL,
                        *realtime = NULL, *monotonic = NULL, *priority = NULL,
                        *unit = NULL, *user_unit = NULL;
    size_t hostname_len = 0, identifier_len = 0, comm_len = 0, pid_len = 0,
           fake_pid_len = 0, message_len = 0, realtime_len = 0,
           monotonic_len = 0, priority_len = 0, unit_len = 0, user_unit_len = 0;
    int p = LOG_INFO;
    bool ellipsized = false;
    const ParseFieldVec fields[] = {
        ...
        PARSE_FIELD_VEC_ENTRY("SYSLOG_IDENTIFIER=", &identifier,
                              &identifier_len),
        PARSE_FIELD_VEC_ENTRY("_SOURCE_REALTIME_TIMESTAMP=", &realtime,
                              &realtime_len),
        PARSE_FIELD_VEC_ENTRY("_SOURCE_MONOTONIC_TIMESTAMP=", &monotonic,
                              &monotonic_len),
        PARSE_FIELD_VEC_ENTRY("_SYSTEMD_UNIT=", &unit, &unit_len),
        PARSE_FIELD_VEC_ENTRY("_SYSTEMD_USER_UNIT=", &user_unit,
                              &user_unit_len),
    };
    size_t highlight_shifted[] = {highlight ? highlight[0] : 0,
    ...
    sd_journal_set_data_threshold(j,
                                  flags & (OUTPUT_SHOW_ALL | OUTPUT_FULL_WIDTH)
                                      ? 0
                                      : PRINT_CHAR_THRESHOLD + 1);
    ...
    if (mode == OUTPUT_WITH_UNIT &&
        ((unit && shall_print(unit, unit_len, flags)) ||
    ...
    {
        fputs(": ", f);
        ellipsized |= print_multiline(f, n + 2, n_columns, flags, p, message,
                                      message_len, highlight_shifted);
    }
...
static int output_verbose(FILE* f, sd_journal* j, OutputMode mode,
                          unsigned n_columns, OutputFlags flags,
                          Set* output_fields, const size_t highlight[2])
{
    ...
    if (r == -ENOENT)
        ...
        return log_full_errno(r == -EADDRNOTAVAIL ? LOG_DEBUG : LOG_ERR, r,
    ...
    else
    {
        ...
        r = parse_field(data, length, "_SOURCE_REALTIME_TIMESTAMP=",
                        STRLEN("_SOURCE_REALTIME_TIMESTAMP="), &value, NULL);
        if (r < 0)
            return r;
        assert(r > 0);
    ...
    timestamp = flags & OUTPUT_UTC
                    ? format_timestamp_us_utc(ts, sizeof ts, realtime)
                    : format_timestamp_us(ts, sizeof ts, realtime);
    fprintf(f, "%s [%s]\n", timestamp ?: "(no timestamp)", cursor);
    ...
    JOURNAL_FOREACH_DATA_RETVAL(j, data, length, r)
    {
             ...
             utf8_is_printable(data, length)))
        {
            ...
            print_multiline(f, 4 + fieldlen + 1, 0, OUTPUT_FULL_WIDTH, 0, c + 1,
                            length - fieldlen - 1, NULL);
            fputs(off, f);
        }
        else
        {
            ...
            fprintf(f, "    %s%.*s=[%s blob data]%s\n", on,
                    (int)(c - (const char*)data), (const char*)data,
                    format_bytes(bytes, sizeof(bytes),
                                 length - (c - (const char*)data) - 1),
                    off);
        }
    }
...
static int output_export(FILE* f, sd_journal* j, OutputMode mode,
                         unsigned n_columns, OutputFlags flags,
                         Set* output_fields, const size_t highlight[2])
{
    ...
    fprintf(f,
            ...
            "_BOOT_ID=%s\n",
            cursor, realtime, monotonic, sd_id128_to_string(boot_id, sid));

    JOURNAL_FOREACH_DATA_RETVAL(j, data, length, r)
    {
        const char* c;

        /* We already printed the boot id from the data in the header, hence
...
static int update_json_data(Hashmap* h, OutputFlags flags, const char* name,
                            const void* value, size_t size)
{
    ...
    if (d)
    {
        ...
        w = realloc(d, offsetof(struct json_data, values) +
                           sizeof(JsonVariant*) * (d->n_values + 1));
        if (!w)
            return log_oom();
    ...
    else
    {
        ...
        if (r < 0)
        {
            ...
            return log_error_errno(
                r, "Failed to insert JSON name into hashmap: %m");
        }
...
static int update_json_data_split(Hashmap* h, OutputFlags flags,
                                  Set* output_fields, const void* data,
                                  size_t size)
{
    ...
    return update_json_data(h, flags, name, eq + 1,
                            size - (eq - (const char*)data) - 1);
}

static int output_json(FILE* f, sd_journal* j, OutputMode mode,
                       unsigned n_columns, OutputFlags flags,
                       Set* output_fields, const size_t highlight[2])
{
    ...
    (void)sd_journal_set_data_threshold(
        j, flags & OUTPUT_SHOW_ALL ? 0 : JSON_THRESHOLD);
    ...
    r = update_json_data(h, flags, "__REALTIME_TIMESTAMP", usecbuf,
                         strlen(usecbuf));
    if (r < 0)
        goto finish;

    xsprintf(usecbuf, USEC_FMT, monotonic);
    r = update_json_data(h, flags, "__MONOTONIC_TIMESTAMP", usecbuf,
                         strlen(usecbuf));
    if (r < 0)
        goto finish;
...
static int output_cat(FILE* f, sd_journal* j, OutputMode mode,
                      unsigned n_columns, OutputFlags flags, Set* output_fields,
                      const size_t highlight[2])
{
    ...
    if (highlight && (flags & OUTPUT_COLOR))
    {
        ...
        fwrite((const char*)data + 8 + highlight[0], 1,
               highlight[1] - highlight[0], f);
        fwrite(highlight_off, 1, strlen(highlight_off), f);
        fwrite((const char*)data + 8 + highlight[1], 1, l - 8 - highlight[1],
               f);
    }
    else
...
static int (*output_funcs[_OUTPUT_MODE_MAX])(
    FILE* f, sd_journal* j, OutputMode mode, unsigned n_columns,
    OutputFlags flags, Set* output_fields, const size_t highlight[2]) = {
...
int show_journal_entry(FILE* f, sd_journal* j, OutputMode mode,
                       unsigned n_columns, OutputFlags flags,
                       char** output_fields, const size_t highlight[2],
                       bool* ellipsized)
{
...
int show_journal(FILE* f, sd_journal* j, OutputMode mode, unsigned n_columns,
                 usec_t not_before, unsigned how_many, OutputFlags flags,
                 bool* ellipsized)
{
    ...
    for (;;)
    {
        for (;;)
        {
            ...
            if (need_seek)
            {
                ...
                if (r < 0)
                    return log_error_errno(
                        r, "Failed to iterate through journal: %m");
            }
            ...
            r = show_journal_entry(f, j, mode, n_columns, flags, NULL, NULL,
                                   ellipsized);
            if (r < 0)
                return r;
        }
        ...
        if (warn_cutoff && line < how_many && not_before > 0)
        {
            ...
            if (r > 0 && not_before < cutoff)
            {
                ...
                fprintf(f,
                        ...
                        "started. Log output is incomplete or unavailable.\n");
            }
...
int add_matches_for_unit(sd_journal* j, const char* unit)
{
    ...
    (void)(
        ...
        (r = sd_journal_add_match(
             j, "MESSAGE_ID=fc2e22bc6ee647b6b90729ab34a250b1", 0)) ||
...
static int get_boot_id_for_machine(const char* machine, sd_id128_t* boot_id)
{
    ...
    r = namespace_fork("(sd-bootidns)", "(sd-bootid)", NULL, 0,
...
int add_match_this_boot(sd_journal* j, const char* machine)
{
    ...
    if (machine)
    {
        ...
        if (r < 0)
            return log_error_errno(
                r, "Failed to get boot id of container %s: %m", machine);
    }
    else
    {
        r = sd_id128_get_boot(&boot_id);
...
int show_journal_by_unit(FILE* f, const char* unit, OutputMode mode,
                         unsigned n_columns, usec_t not_before,
                         unsigned how_many, uid_t uid, OutputFlags flags,
                         int journal_open_flags, bool system_unit,
                         bool* ellipsized)
{
