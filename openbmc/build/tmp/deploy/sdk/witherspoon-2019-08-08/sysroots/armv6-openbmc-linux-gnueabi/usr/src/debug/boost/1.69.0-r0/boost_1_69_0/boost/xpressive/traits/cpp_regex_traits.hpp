...
// From John Maddock:
// Fix for gcc prior to 3.4: std::ctype<wchar_t> doesn't allow masks to be
// combined, for example: std::use_facet<std::ctype<wchar_t>
// >(locale()).is(std::ctype_base::lower|std::ctype_base::upper, L'a');
...
// Calculate what the size of the umaskex_t type should be to fix the 3 extra
// bitmasks
//   11 char categories in ctype_base
// +  3 extra categories for xpressive
// = 14 total bits needed
int const umaskex_bits =
    (14 > (sizeof(umask_t) * CHAR_BIT)) ? 14 : sizeof(umask_t) * CHAR_BIT;
...
template <umaskex_t In, umaskex_t Out = highest_bit,
          bool Done = (0 == (Out & In))>
struct unused_mask
{
    BOOST_STATIC_ASSERT(1 != Out);
    BOOST_STATIC_CONSTANT(umaskex_t,
...
umaskex_t const all_ctype_masks =
    std_ctype_reserved | std_ctype_alnum | std_ctype_alpha | std_ctype_cntrl |
...
umaskex_t const non_std_ctype_blank =
    unused_mask<all_ctype_masks | non_std_ctype_underscore>::value;

// define a new mask for "newline"
umaskex_t const non_std_ctype_newline =
    unused_mask<all_ctype_masks | non_std_ctype_underscore |
                non_std_ctype_blank>::value;
...
//   an array of underscore characters and an array of line separator
//   characters.
...
// HACKHACK Cygwin and mingw have buggy ctype facets for wchar_t
        ...
        if (std::ctype_base::xdigit ==
            ((std::ctype_base::mask)(umask_t)mask & std::ctype_base::xdigit))
        {
            typename std::char_traits<Char>::int_type i =
        ...
        for (umaskex_t i; 0 != (i = (tmp & (~tmp + 1))); tmp &= ~i)
        {
            std::ctype_base::mask m =
                (std::ctype_base::mask)(umask_t)std_masks[mylog2(i)];
            if (ct.is(m, ch))
            {
                return true;
            }
        ...
        return ((mask & non_std_ctype_blank) &&
                cpp_regex_traits_base::is_blank(ch)) ||
               ((mask & non_std_ctype_underscore) &&
                cpp_regex_traits_base::is_underscore(ch)) ||
               ((mask & non_std_ctype_newline) &&
                cpp_regex_traits_base::is_newline(ch));
    }
  ...
  private:
    ...
    static bool is_newline(Char ch)
    {
        ...
        return L'\r' == ch || L'\n' == ch || L'\f' == ch ||
               (1 < SizeOfChar &&
                (0x2028u == ch || 0x2029u == ch || 0x85u == ch));
    }
};
...
struct cpp_regex_traits_base<Char, 1>
{
  protected:
    void imbue(std::locale const& loc)
    {
        ...
        this->masks_[static_cast<unsigned char>('_')] |=
...
struct cpp_regex_traits : detail::cpp_regex_traits_base<Char>
{
    ...
    /// \return std::use_facet\<std::ctype\<char_type\>
    /// \>(this->getloc()).widen(ch).
    ...
    /// Returns a \c string_type containing all the characters that compare
    /// equal disregrarding case to the one passed in. This function can only be
    /// called if <tt>has_fold_case\<cpp_regex_traits\<Char\> \>::value</tt> is
    /// \c true.
    ///
    /// \param ch The source character.
    /// \return \c string_type containing all chars which are equal to \c ch
    /// when disregarding
    ///     case
    string_type fold_case(char_type ch) const
    {
        BOOST_MPL_ASSERT((is_same<char_type, char>));
        char_type ntcs[] = {this->ctype_->tolower(ch),
                            this->ctype_->toupper(ch), 0};
        if (ntcs[1] == ntcs[0])
            ntcs[1] = 0;
    ...
    /// Checks to see if a character is within a character range, irregardless
    /// of case.
    ///
    /// \param first The bottom of the range, inclusive.
    /// \param last The top of the range, inclusive.
    /// \param ch The source character.
    /// \return in_range(first, last, ch) || in_range(first, last, tolower(ch,
    /// this->getloc())) ||
    ...
    /// Returns a sort key for the character sequence designated by the iterator
    /// range [F1, F2) such that if the character sequence [G1, G2) sorts before
    /// the character sequence [H1, H2) when character case is not considered
    ...
    /// [F1, F2). Returns an empty string if the character sequence is not a
    ...
    /// For the character class name represented by the specified character
    /// sequence, return the corresponding bitmask representation.
    ///
    /// \param begin A forward iterator to the start of the character sequence
    /// representing
    ///     the name of the character class.
    /// \param end The end of the character sequence.
    /// \param icase Specifies whether the returned bitmask should represent the
    /// case-insensitive
    ///     version of the character class.
    /// \return A bitmask representing the character class.
    template <typename FwdIter>
    char_class_type lookup_classname(FwdIter begin, FwdIter end,
                                     bool icase) const
    {
        ...
        if (0 == char_class)
        {
            ...
            for (typename string_type::size_type i = 0, len = classname.size();
                 i < len; ++i)
            {
                classname[i] = this->translate_nocase(classname[i]);
            }
            char_class = this->lookup_classname_impl_(classname.begin(),
                                                      classname.end());
        }
    ...
    /// \pre mask is a bitmask returned by lookup_classname, or is several such
    /// masks bit-or'ed
    ///     together.
    /// \return true if the character is a member of any of the specified
    /// character classes, false
    ///     otherwise.
    ...
    /// \return -1 if ch is not a digit character, the integer value of the
    /// character otherwise.
    ///     The conversion is performed by imbueing a std::stringstream with
    ///     this-\>getloc(); setting the radix to one of oct, hex or dec;
    ///     inserting ch into the stream; and extracting an int.
    ...
    locale_type imbue(locale_type loc)
    {
        ...
        // this->collate_ = &BOOST_USE_FACET(std::collate<char_type>,
        // this->loc_);
  ...
  private:
    ...
    static char_class_pair const& char_class(std::size_t j)
    {
        static BOOST_CONSTEXPR_OR_CONST char_class_pair s_char_class_map[] = {
            ...
            {BOOST_XPR_CSTR_(char_type, "newline"),
             detail::non_std_ctype_newline},
            ...
            {BOOST_XPR_CSTR_(char_type, "w"),
             detail::std_ctype_alnum | detail::non_std_ctype_underscore},
