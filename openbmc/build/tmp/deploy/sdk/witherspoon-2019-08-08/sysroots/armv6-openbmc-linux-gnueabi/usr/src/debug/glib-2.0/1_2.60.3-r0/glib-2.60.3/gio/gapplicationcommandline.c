...
G_DEFINE_TYPE_WITH_PRIVATE(GApplicationCommandLine, g_application_command_line,
                           G_TYPE_OBJECT)

/* All subclasses represent remote invocations of some kind. */
#define IS_REMOTE(cmdline)                                                     \
    (G_TYPE_FROM_INSTANCE(cmdline) != G_TYPE_APPLICATION_COMMAND_LINE)
...
static void g_application_command_line_real_print_literal(
    GApplicationCommandLine* cmdline, const gchar* message)
{
    g_print("%s", message);
}

static void g_application_command_line_real_printerr_literal(
    GApplicationCommandLine* cmdline, const gchar* message)
{
...
static void g_application_command_line_get_property(GObject* object,
                                                    guint prop_id,
                                                    GValue* value,
                                                    GParamSpec* pspec)
{
...
static void g_application_command_line_set_property(GObject* object,
                                                    guint prop_id,
                                                    const GValue* value,
                                                    GParamSpec* pspec)
{
...
static void g_application_command_line_constructed(GObject* object)
{
    GApplicationCommandLine* cmdline = G_APPLICATION_COMMAND_LINE(object);
    ...
    g_application_command_line_class_init(GApplicationCommandLineClass* class)
{
    ...
    g_object_class_install_property(
        object_class, PROP_ARGUMENTS,
        g_param_spec_variant("arguments", P_("Commandline arguments"),
                             P_("The commandline that caused this "
                                "::command-line signal emission"),
    ...
    g_object_class_install_property(
        ...
        g_param_spec_variant("options", P_("Options"),
    ...
    g_object_class_install_property(
        object_class, PROP_PLATFORM_DATA,
        g_param_spec_variant("platform-data", P_("Platform data"),
    ...
    g_object_class_install_property(
        ...
        g_param_spec_boolean("is-remote", P_("Is remote"),
                             P_("TRUE if this is a remote commandline"), FALSE,
    ...
    GApplicationCommandLine* cmdline)
{
    ...
    if (!cmdline->priv->options_dict)
        cmdline->priv->options_dict =
            g_variant_dict_new(cmdline->priv->options);
                                      ...
                                      const gchar* format, ...)
{
    ...
    G_APPLICATION_COMMAND_LINE_GET_CLASS(cmdline)->print_literal(cmdline,
                                                                 message);
    g_free(message);
}
                                         ...
                                         const gchar* format, ...)
{
    ...
    G_APPLICATION_COMMAND_LINE_GET_CLASS(cmdline)->printerr_literal(cmdline,
                                                                    message);
    g_free(message);
}
    ...
    GApplicationCommandLine* cmdline, const gchar* arg)
{
    ...
    g_warning("Requested creation of GFile for commandline invocation that did "
              "not send cwd. "
