 ...
 *
 * Janson, S. 2005. Individual displacements for linear probing hashing with
 * different insertion policies. ACM Trans. Algorithms 1, 2 (October 2005),
 * 177-213. DOI=10.1145/1103963.1103964
 * http://doi.acm.org/10.1145/1103963.1103964
 ...
 *
 * Viola, A. 2005. Exact distribution of individual displacements in linear
 * probing hashing. ACM Trans. Algorithms 1, 2 (October 2005), 214-242.
 ...
 * - Short summary of random vs. linear probing, and tombstones vs. backward
 * shift.
 */
...
#define IDX_FIRST                                                              \
    (UINT_MAX - 1)       /* special index for freshly initialized iterators */
...
#define DIB_RAW_OVERFLOW                                                       \
    ((dib_raw_t)0xfdU) /* indicates DIB value is greater than representable */
#define DIB_RAW_REHASH                                                         \
    ((dib_raw_t)0xfeU) /* entry yet to be rehashed during in-place resize */
#define DIB_RAW_FREE ((dib_raw_t)0xffU) /* a free bucket */
#define DIB_RAW_INIT                                                           \
    ((char)DIB_RAW_FREE) /* a byte to memset a DIB store with when             \
                            initializing */
...
struct _packed_ indirect_storage
{
    ...
    unsigned
        idx_lowest_entry; /* Index below which all buckets are free.
...
{
    return (
        struct hashmap_base_entry*)((uint8_t*)storage_ptr(h) +
                                    idx *
                                        hashmap_type_info[h->type].entry_size);
}
...
static struct ordered_hashmap_entry* ordered_bucket_at(OrderedHashmap* h,
                                                       unsigned idx)
{
...
static struct ordered_hashmap_entry* bucket_at_swap(struct swap_entries* swap,
                                                    unsigned idx)
{
...
{
    return (dib_raw_t*)((uint8_t*)storage_ptr(h) +
                        hashmap_type_info[h->type].entry_size * n_buckets(h));
}
                              ...
                              unsigned from, unsigned to)
{
    ...
    if (h->type == HASHMAP_TYPE_ORDERED)
    {
        ...
        {
            le = (struct ordered_hashmap_entry*)bucket_at_virtual(
...
static void base_remove_entry(HashmapBase* h, unsigned idx)
{
    ...
    if (h->type == HASHMAP_TYPE_ORDERED)
    {
        ...
        if (le->iterate_next != IDX_NIL)
            ordered_bucket_at(lh, le->iterate_next)->iterate_previous =
                le->iterate_previous;
        else
...
static unsigned hashmap_iterate_in_insertion_order(OrderedHashmap* h,
                                                   Iterator* i)
{
    ...
    else
    {
        ...
        e = ordered_bucket_at(h, idx);
        /*
         * We allow removing the current entry while iterating, but removal may
         * cause a backward shift. The next entry may thus move one bucket to
         * the left. To detect when it happens, we remember the key pointer of
         * the entry we were going to iterate next. If it does not match, there
         * was a backward shift.
         */
...
bool internal_hashmap_iterate(HashmapBase* h, Iterator* i, void** value,
                              const void** key)
{
    struct hashmap_base_entry* e;
    void* data;
...
{
    return (Hashmap*)hashmap_base_new(
        hash_ops, HASHMAP_TYPE_PLAIN HASHMAP_DEBUG_PASS_ARGS);
}

OrderedHashmap* internal_ordered_hashmap_new(
    const struct hash_ops* hash_ops HASHMAP_DEBUG_PARAMS)
{
    return (OrderedHashmap*)hashmap_base_new(
        hash_ops, HASHMAP_TYPE_ORDERED HASHMAP_DEBUG_PASS_ARGS);
}
...
int internal_hashmap_ensure_allocated(
    Hashmap** h, const struct hash_ops* hash_ops HASHMAP_DEBUG_PARAMS)
{
    return hashmap_base_ensure_allocated(
        (HashmapBase**)h, hash_ops, HASHMAP_TYPE_PLAIN HASHMAP_DEBUG_PASS_ARGS);
}

int internal_ordered_hashmap_ensure_allocated(
    OrderedHashmap** h, const struct hash_ops* hash_ops HASHMAP_DEBUG_PARAMS)
{
    return hashmap_base_ensure_allocated(
        (HashmapBase**)h, hash_ops,
        HASHMAP_TYPE_ORDERED HASHMAP_DEBUG_PASS_ARGS);
}

int internal_set_ensure_allocated(
    Set** s, const struct hash_ops* hash_ops HASHMAP_DEBUG_PARAMS)
{
    return hashmap_base_ensure_allocated(
        (HashmapBase**)s, hash_ops, HASHMAP_TYPE_SET HASHMAP_DEBUG_PASS_ARGS);
}
...
HashmapBase* internal_hashmap_free(HashmapBase* h, free_func_t default_free_key,
...
void internal_hashmap_clear(HashmapBase* h, free_func_t default_free_key,
                            free_func_t default_free_value)
{
    ...
    if (free_key || free_value)
    {
        ...
        /* If destructor calls are defined, let's destroy things defensively:
         * let's take the item out of the hash table, and only then call the
         * destructor functions. If these destructors then try to unregister
         * themselves from our hash table a second time, the entry is already
         * gone. */
...
static int resize_buckets(HashmapBase* h, unsigned entries_add)
{
    ...
    if (_unlikely_(new_n_buckets >
                   UINT_MAX / (hi->entry_size + sizeof(dib_raw_t))))
        return -ENOMEM;
    ...
    new_shift =
        log2u_round_up(MAX(new_n_buckets * (hi->entry_size + sizeof(dib_raw_t)),
                           2 * sizeof(struct direct_storage)));

    /* Realloc storage (buckets and DIB array). */
    new_storage =
        realloc(h->has_indirect ? h->indirect.storage : NULL, 1U << new_shift);
    if (!new_storage)
        return -ENOMEM;
    ...
    h->indirect.n_buckets =
        (1U << new_shift) / (hi->entry_size + sizeof(dib_raw_t));

    old_dibs =
        (dib_raw_t*)((uint8_t*)new_storage + hi->entry_size * old_n_buckets);
    ...
    for (idx = 0; idx < old_n_buckets; idx++)
    {
        ...
        new_dibs[idx] =
            old_dibs[idx] == DIB_RAW_FREE ? DIB_RAW_FREE : DIB_RAW_REHASH;
    }
...
void* internal_hashmap_remove_value(HashmapBase* h, const void* key,
                                    void* value)
{
    struct hashmap_base_entry* e;
    unsigned hash, idx;
...
void* internal_hashmap_first_key_and_value(HashmapBase* h, bool remove,
                                           void** ret_key)
{
    struct hashmap_base_entry* e;
    void *key, *data;
...
int set_put_strsplit(Set* s, const char* v, const char* separators,
                     ExtractFlags flags)
{
    const char* p = v;
    int r;
...
int iterated_cache_get(IteratedCache* cache, const void*** res_keys,
                       const void*** res_values, unsigned* res_n_entries)
{
