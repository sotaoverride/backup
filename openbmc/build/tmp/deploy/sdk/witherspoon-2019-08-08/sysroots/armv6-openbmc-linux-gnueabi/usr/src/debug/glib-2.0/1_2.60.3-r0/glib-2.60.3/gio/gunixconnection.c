...
G_DEFINE_TYPE_WITH_CODE(GUnixConnection, g_unix_connection,
                        ...
                        g_socket_connection_factory_register_type(
                            g_define_type_id, G_SOCKET_FAMILY_UNIX,
 ...
 * Since: 2.22
 */
gboolean g_unix_connection_send_fd(GUnixConnection* connection, gint fd,
                                   GCancellable* cancellable, GError** error)
{
    ...
    if (g_socket_send_message(socket, NULL, NULL, 0, &scm, 1, 0, cancellable,
                              error) != 1)
                                  ...
                                  GCancellable* cancellable, GError** error)
{
    ...
    g_object_get(connection, "socket", &socket, NULL);
    if (g_socket_receive_message(socket, NULL, NULL, 0, &scms, &nscm, NULL,
                                 cancellable, error) != 1)
...
void                    g_unix_connection_send_fd_async (GUnixConnection
*connection, gint                  fd, gboolean              close, gint
io_priority, GAsyncReadyCallback   callback, gpointer              user_data);
gboolean                g_unix_connection_send_fd_finish (GUnixConnection
*connection, GError              **error);

gboolean                g_unix_connection_send_fds (GUnixConnection *connection,
                                                                         gint
*fds, gint                  nfds, GError              **error); void
g_unix_connection_send_fds_async                (GUnixConnection *connection,
                                                                         gint
*fds, gint                  nfds, gint                  io_priority,
                                                                         GAsyncReadyCallback
callback, gpointer              user_data); gboolean
g_unix_connection_send_fds_finish               (GUnixConnection *connection,
                                                                         GError
**error);

void                    g_unix_connection_receive_fd_async (GUnixConnection
*connection, gint                  io_priority, GAsyncReadyCallback   callback,
                                                                         gpointer
user_data); gint                    g_unix_connection_receive_fd_finish
(GUnixConnection      *connection, GError              **error);


gboolean                g_unix_connection_send_fake_credentials (GUnixConnection
*connection, guint64               pid, guint64               uid, guint64 gid,
                                                                         GError
**error); void                    g_unix_connection_send_fake_credentials_async
(GUnixConnection      *connection, guint64               pid, guint64 uid,
                                                                         guint64
gid, gint                  io_priority, GAsyncReadyCallback   callback, gpointer
user_data); gboolean g_unix_connection_send_fake_credentials_finish
(GUnixConnection      *connection, GError              **error);
                                            ...
                                            GError** error)
{
    ...
    g_object_get(connection, "socket", &socket, NULL);
    if (g_socket_send_message(socket, NULL, /* address */
                              &vector, 1, &scm, num_messages, G_SOCKET_MSG_NONE,
                              cancellable, error) != 1)
    {
...
static void send_credentials_async_thread(GTask* task, gpointer source_object,
                                          gpointer task_data,
                                          GCancellable* cancellable)
{
    GError* error = NULL;
                                                    ...
                                                    GError** error)
{
...
#ifdef __linux__
    {
        ...
        opt_val = 0;
        if (!g_socket_get_option(socket, SOL_SOCKET, SO_PASSCRED, &opt_val,
                                 NULL))
        {
            int errsv = errno;
            g_set_error(
        ...
        if (opt_val == 0)
        {
            if (!g_socket_set_option(socket, SOL_SOCKET, SO_PASSCRED, TRUE,
                                     NULL))
            {
                int errsv = errno;
    ...
    num_bytes_read = g_socket_receive_message(
        socket, NULL, /* GSocketAddress **address */
        NULL, 0, &scms, &nscm, NULL, cancellable, error);
    if (num_bytes_read != 1)
    {
        ...
        {
            g_set_error_literal(error, G_IO_ERROR, G_IO_ERROR_FAILED,
                                _("Expecting to read a single byte for "
                                  "receiving credentials but read zero bytes"));
        }
        goto out;
    }

    if (g_unix_credentials_message_is_supported() &&
        /* Fall back on get_credentials if the other side didn't send the
           credentials */
        nscm > 0)
    {
        if (nscm != 1)
        ...
        ret = g_unix_credentials_message_get_credentials(
            G_UNIX_CREDENTIALS_MESSAGE(scms[0]));
        g_object_ref(ret);
    }
    else
    {
        ...
        {
            g_set_error(error, G_IO_ERROR, G_IO_ERROR_FAILED,
                        _("Not expecting control message, but got %d"), nscm);
            goto out;
        }
        else
        {
...
out:
    ...
    {
        if (!g_socket_set_option(socket, SOL_SOCKET, SO_PASSCRED, FALSE, NULL))
        {
            int errsv = errno;
            g_set_error(error, G_IO_ERROR, g_io_error_from_errno(errsv),
...
static void receive_credentials_async_thread(GTask* task,
                                             ...
                                             GCancellable* cancellable)
{
    ...
    creds = g_unix_connection_receive_credentials(
        G_UNIX_CONNECTION(source_object), cancellable, &error);
    if (creds)
