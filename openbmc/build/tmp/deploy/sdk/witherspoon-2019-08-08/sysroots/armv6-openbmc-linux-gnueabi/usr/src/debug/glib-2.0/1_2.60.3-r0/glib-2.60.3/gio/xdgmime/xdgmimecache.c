...
static int cache_magic_matchlet_compare_to_data(XdgMimeCache* cache,
                                                xdg_uint32_t offset,
...
static int cache_magic_matchlet_compare(XdgMimeCache* cache,
                                        xdg_uint32_t offset, const void* data,
                                        size_t len)
{
    ...
    if (cache_magic_matchlet_compare_to_data(cache, offset, data, len))
    {
        ...
        {
            if (cache_magic_matchlet_compare(cache, child_offset + 32 * i, data,
                                             len))
                return TRUE;
        }
    }
...
static const char* cache_magic_compare_to_data(XdgMimeCache* cache,
                                               xdg_uint32_t offset,
                                               const void* data, size_t len,
                                               int* prio)
{
    ...
    {
        if (cache_magic_matchlet_compare(cache, matchlet_offset + i * 32, data,
                                         len))
        {
            *prio = priority;
...
static const char* cache_magic_lookup_data(XdgMimeCache* cache,
                                           const void* data, size_t len,
                                           int* prio, const char* mime_types[],
                                           int n_mime_types)
{
    ...
    *prio = 0;
    ...
    for (j = 0; j < n_entries; j++)
    {
        ...
        match = cache_magic_compare_to_data(cache, offset + 16 * j, data, len,
                                            prio);
        if (match)
            return match;
        else
...
static const char* cache_alias_lookup(const char* alias)
{
    const char* ptr;
    ...
    for (i = 0; _caches[i]; i++)
    {
        ...
        while (max >= min)
        {
            ...
            {
                offset =
                    GET_UINT32(cache->buffer, list_offset + 4 + 8 * mid + 4);
...
static int cache_glob_lookup_literal(const char* file_name,
                                     const char* mime_types[], int n_mime_types,
                                     int case_sensitive_check)
{
    ...
    for (i = 0; _caches[i]; i++)
    {
        ...
        while (max >= min)
        {
            ...
            {
                int weight =
                    GET_UINT32(cache->buffer, list_offset + 4 + 12 * mid + 8);
                int case_sensitive = weight & 0x100;
                weight = weight & 0xff;

                if (case_sensitive_check || !case_sensitive)
                {
                    offset = GET_UINT32(cache->buffer,
                                        list_offset + 4 + 12 * mid + 4);
...
static int cache_glob_lookup_fnmatch(const char* file_name,
                                     MimeWeight mime_types[], int n_mime_types)
{
    ...
    for (i = 0; _caches[i]; i++)
    {
        ...
        {
            xdg_uint32_t offset =
                GET_UINT32(cache->buffer, list_offset + 4 + 12 * j);
            xdg_uint32_t mimetype_offset =
                GET_UINT32(cache->buffer, list_offset + 4 + 12 * j + 4);
            int weight =
                GET_UINT32(cache->buffer, list_offset + 4 + 12 * j + 8);
            weight = weight & 0xff;
            ptr = cache->buffer + offset;
...
static int
    cache_glob_node_lookup_suffix(XdgMimeCache* cache, xdg_uint32_t n_entries,
                                  xdg_uint32_t offset, const char* file_name,
                                  int len, int case_sensitive_check,
                                  MimeWeight mime_types[], int n_mime_types)
{
        ...
        else
        {
            ...
            if (len > 0)
            {
                n = cache_glob_node_lookup_suffix(
                    cache, n_children, child_offset, file_name, len,
                    case_sensitive_check, mime_types, n_mime_types);
            }
            if (n == 0)
            {
                i = 0;
                ...
                {
                    match_char =
                        GET_UINT32(cache->buffer, child_offset + 12 * i);
                    if (match_char != 0)
                        break;

                    mimetype_offset =
                        GET_UINT32(cache->buffer, child_offset + 12 * i + 4);
                    weight =
                        GET_UINT32(cache->buffer, child_offset + 12 * i + 8);
...
static int cache_glob_lookup_suffix(const char* file_name, int len,
                                    int ignore_case, MimeWeight mime_types[],
                                    int n_mime_types)
{
    int i, n;
    ...
    for (i = 0; _caches[i]; i++)
    {
        ...
        n += cache_glob_node_lookup_suffix(cache, n_entries, offset, file_name,
                                           len, ignore_case, mime_types + n,
                                           n_mime_types - n);
        if (n == n_mime_types)
            break;
    }
...
static char* ascii_tolower(const char* str)
{
    char *p, *lower;
...
static int filter_out_dupes(MimeWeight mimes[], int n_mimes)
{
    int last;
    int i, j;
...
static int cache_glob_lookup_file_name(const char* file_name,
                                       ...
                                       int n_mime_types)
{
    ...
    if (n < 2)
        n += cache_glob_lookup_suffix(file_name, len, TRUE, mimes + n,
                                      n_mimes - n);
...
static const char* cache_get_mime_type_for_data(const void* data, size_t len,
                                                int* result_prio,
                                                const char* mime_types[],
                                                int n_mime_types)
{
    ...
    for (i = 0; _caches[i]; i++)
    {
        ...
        match = cache_magic_lookup_data(cache, data, len, &prio, mime_types,
                                        n_mime_types);
        if (prio > priority)
        {
            priority = prio;
                                                   ...
                                                   struct stat* statbuf)
{
     ...
     * could be large and need getting from a stream instead of just reading it
     * all in. */
    ...
    mime_type =
        cache_get_mime_type_for_data(data, bytes_read, NULL, mime_types, n);
...
static int is_super_type(const char* mime)
{
...
int _xdg_mime_cache_mime_type_subclass(const char* mime, const char* base)
{
    ...
    for (i = 0; _caches[i]; i++)
    {
        ...
        while (max >= min)
        {
            ...
            {
                offset =
                    GET_UINT32(cache->buffer, list_offset + 4 + 8 * med + 4);
                n_parents = GET_UINT32(cache->buffer, offset);

                for (j = 0; j < n_parents; j++)
                {
                    parent_offset =
                        GET_UINT32(cache->buffer, offset + 4 + 4 * j);
                    if (_xdg_mime_cache_mime_type_subclass(
                            cache->buffer + parent_offset, ubase))
                        return 1;
                }
...
char** _xdg_mime_cache_list_mime_parents(const char* mime)
{
    ...
    for (i = 0; _caches[i]; i++)
    {
        ...
        {
            xdg_uint32_t mimetype_offset =
                GET_UINT32(cache->buffer, list_offset + 4 + 8 * j);
            xdg_uint32_t parents_offset =
                GET_UINT32(cache->buffer, list_offset + 4 + 8 * j + 4);

            if (strcmp(cache->buffer + mimetype_offset, mime) == 0)
            {
                xdg_uint32_t parent_mime_offset;
                xdg_uint32_t n_parents =
                    GET_UINT32(cache->buffer, parents_offset);

                for (k = 0; k < n_parents && p < 127; k++)
                {
                    parent_mime_offset =
                        GET_UINT32(cache->buffer, parents_offset + 4 + 4 * k);

                    /* Don't add same parent multiple times.
                     * This can happen for instance if the same type is listed
                     * in multiple directories
                     */
                    for (l = 0; l < p; l++)
                    {
                        if (strcmp(all_parents[l],
                                   cache->buffer + parent_mime_offset) == 0)
                            break;
                    }
...
static const char* cache_lookup_icon(const char* mime, int header)
{
    const char* ptr;
    int i, min, max, mid, cmp;
...
static void dump_glob_node(XdgMimeCache* cache, xdg_uint32_t offset, int depth)
{
    xdg_unichar_t character;
