...
static netsnmp_scalar_group* clone_scalar_group(netsnmp_scalar_group* src)
{
...
int netsnmp_scalar_group_helper_handler(netsnmp_mib_handler* handler,
                                        ...
                                        netsnmp_request_info* requests)
{
    ...
    namelen = SNMP_MIN(requests->requestvb->name_length, reginfo->rootoid_len);
    cmp = snmp_oid_compare(requests->requestvb->name, namelen, reginfo->rootoid,
                           reginfo->rootoid_len);
    ...
    switch (reqinfo->mode)
    {
        ...
        case MODE_GET:
            ret = reqinfo->mode == MODE_GET ? SNMP_NOSUCHOBJECT
                                            : SNMP_ERR_NOCREATION;
            if (cmp != 0 ||
            ...
            else
            {
                ...
                if (subid < sgroup->lbound || subid > sgroup->ubound)
                {
                    netsnmp_set_request_error(reqinfo, requests, ret);
                    return SNMP_ERR_NOERROR;
                }
        ...
        case MODE_GETNEXT:
            ...
            else if (requests->requestvb->name_length ==
