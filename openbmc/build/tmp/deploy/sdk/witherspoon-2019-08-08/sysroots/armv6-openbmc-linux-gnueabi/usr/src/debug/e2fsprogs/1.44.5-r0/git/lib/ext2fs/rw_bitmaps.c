...
static errcode_t write_bitmaps(ext2_filsys fs, int do_inode, int do_block)
{
    ...
    if (do_inode)
    {
        inode_nbytes = (size_t)((EXT2_INODES_PER_GROUP(fs->super) + 7) / 8);
    ...
    for (i = 0; i < fs->group_desc_count; i++)
    {
        ...
        retval = ext2fs_get_block_bitmap_range2(fs->block_map, blk_itr,
                                                block_nbytes << 3, block_buf);
        if (retval)
            goto errout;
        ...
        if (i == fs->group_desc_count - 1)
        {
            ...
            nbits =
                EXT2FS_NUM_B2C(fs, ((ext2fs_blocks_count(fs->super) -
        ...
        retval = ext2fs_block_bitmap_csum_set(fs, i, block_buf, block_nbytes);
        if (retval)
            return retval;
    ...
    skip_block_bitmap:
        ...
        retval = ext2fs_get_inode_bitmap_range2(fs->inode_map, ino_itr,
                                                inode_nbytes << 3, inode_buf);
        if (retval)
            goto errout;

        retval = ext2fs_inode_bitmap_csum_set(fs, i, inode_buf, inode_nbytes);
        if (retval)
            goto errout;
        ext2fs_group_desc_csum_set(fs, i);
        fs->flags |= EXT2_FLAG_DIRTY;
        ...
        if (blk)
        {
            retval = io_channel_write_blk64(fs->io, blk, 1, inode_buf);
            if (retval)
            {
...
static errcode_t read_bitmaps(ext2_filsys fs, int do_inode, int do_block)
{
    ...
    {
        blk = (ext2fs_le32_to_cpu(fs->image_header->offset_inodemap) /
               fs->blocksize);
        ino_cnt = fs->super->s_inodes_count;
        while (inode_bitmap && ino_cnt > 0)
        {
            retval =
                io_channel_read_blk64(fs->image_io, blk++, 1, inode_bitmap);
            if (retval)
                goto cleanup;
            cnt = fs->blocksize << 3;
            if (cnt > ino_cnt)
                cnt = ino_cnt;
            retval = ext2fs_set_inode_bitmap_range2(fs->inode_map, ino_itr, cnt,
                                                    inode_bitmap);
            if (retval)
                goto cleanup;
            ino_itr += cnt;
        ...
        blk = (ext2fs_le32_to_cpu(fs->image_header->offset_blockmap) /
               fs->blocksize);
        blk_cnt = EXT2_GROUPS_TO_CLUSTERS(fs->super, fs->group_desc_count);
        while (block_bitmap && blk_cnt > 0)
        {
            retval =
                io_channel_read_blk64(fs->image_io, blk++, 1, block_bitmap);
            if (retval)
                goto cleanup;
            cnt = fs->blocksize << 3;
            if (cnt > blk_cnt)
                cnt = blk_cnt;
            retval = ext2fs_set_block_bitmap_range2(fs->block_map, blk_itr, cnt,
                                                    block_bitmap);
            if (retval)
                goto cleanup;
            blk_itr += cnt;
    ...
    for (i = 0; i < fs->group_desc_count; i++)
    {
        if (block_bitmap)
        {
            ...
            if (blk)
            {
                retval = io_channel_read_blk64(fs->io, blk, 1, block_bitmap);
                if (retval)
                {
                ...
                if (!(fs->flags & EXT2_FLAG_IGNORE_CSUM_ERRORS) &&
                    !ext2fs_block_bitmap_csum_verify(fs, i, block_bitmap,
                                                     block_nbytes))
                {
        ...
        if (inode_bitmap)
        {
            ...
            if (blk)
            {
                retval = io_channel_read_blk64(fs->io, blk, 1, inode_bitmap);
                if (retval)
                {
                ...
                if (!(fs->flags & EXT2_FLAG_IGNORE_CSUM_ERRORS) &&
                    !ext2fs_inode_bitmap_csum_verify(fs, i, inode_bitmap,
                                                     inode_nbytes))
                {
