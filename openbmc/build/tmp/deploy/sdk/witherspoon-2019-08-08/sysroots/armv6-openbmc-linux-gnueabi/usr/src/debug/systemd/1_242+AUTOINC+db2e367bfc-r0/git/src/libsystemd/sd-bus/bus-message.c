...
static int message_append_basic(sd_bus_message* m, char type, const void* p,
                                const void** stored);

static void* adjust_pointer(const void* p, void* old_base, size_t sz,
                            void* new_base)
{
...
static sd_bus_message* message_free(sd_bus_message* m)
{
    ...
    /* Note that we don't unref m->bus here. That's already done by
     * sd_bus_message_unref() as each user
     * reference to the bus message also is considered a reference to the bus
     * connection itself. */
                                   ...
                                   bool add_offset)
{
    ...
    if (add_offset)
    {
        ...
        m->header_offsets[m->n_header_offsets++] =
            new_size - sizeof(struct bus_header);
    }
...
static int message_append_field_string(sd_bus_message* m, uint64_t h, char type,
                                       const char* s, const char** ret)
{
    ...
    {
        /* (field id byte + (signature length + signature 's' + NUL) + (string
         * length + string + NUL)) */
...
static int message_append_field_signature(sd_bus_message* m, uint64_t h,
                                          const char* s, const char** ret)
{
    ...
    {
        /* (field id byte + (signature length + signature 'g' + NUL) + (string
         * length + string + NUL)) */
...
static int message_append_field_uint32(sd_bus_message* m, uint64_t h,
                                       uint32_t x)
{
    uint8_t* p;
...
static int message_append_field_uint64(sd_bus_message* m, uint64_t h,
                                       uint64_t x)
{
    uint8_t* p;
    ...
    {
        /* (field id byte + (signature length + signature 't' + NUL) + 4 byte
         * padding + value) */
...
static int message_append_reply_cookie(sd_bus_message* m, uint64_t cookie)
{
    ...
    if (BUS_MESSAGE_IS_GVARIANT(m))
        return message_append_field_uint64(m, BUS_MESSAGE_HEADER_REPLY_SERIAL,
                                           cookie);
    else
    {
        ...
        return message_append_field_uint32(m, BUS_MESSAGE_HEADER_REPLY_SERIAL,
                                           (uint32_t)cookie);
    }
}

int bus_message_from_header(sd_bus* bus, void* header, size_t header_accessible,
                            void* footer, size_t footer_accessible,
                            size_t message_size, int* fds, size_t n_fds,
                            const char* label, size_t extra,
                            sd_bus_message** ret)
{
    ...
    if (BUS_MESSAGE_IS_GVARIANT(m))
    {
        ...
        m->fields_size = bus_gvariant_read_word_le(
            (uint8_t*)footer + footer_accessible - ws, ws);
        ...
        m->body_size =
            message_size - (sizeof(struct bus_header) + ALIGN8(m->fields_size));
    }
    else
    {
        if (h->dbus1.serial == 0)
        ...
        if (sizeof(struct bus_header) + ALIGN8(m->fields_size) + m->body_size !=
            message_size)
            return -EBADMSG;
    }
    ...
    {
        m->creds.label =
            (char*)m + ALIGN(sizeof(sd_bus_message)) + ALIGN(extra);
                            ...
                            sd_bus_message** ret)
{
    ...
    r = bus_message_from_header(
        bus, buffer, length, /* in this case the initial bytes and the final
                                bytes are the same */
        buffer, length, length, fds, n_fds, label, 0, &m);
    if (r < 0)
        return r;
    ...
    {
        m->n_body_parts = 1;
        m->body.data = (uint8_t*)buffer + sizeof(struct bus_header) +
...
_public_ int sd_bus_message_new(sd_bus* bus, sd_bus_message** m, uint8_t type)
{
    ...
    t->header = (struct bus_header*)((uint8_t*)t +
...
_public_ int sd_bus_message_new_signal(sd_bus* bus, sd_bus_message** m,
                                       ...
                                       const char* member)
{
    ...
    r = message_append_field_string(t, BUS_MESSAGE_HEADER_PATH,
                                    SD_BUS_TYPE_OBJECT_PATH, path, &t->path);
    if (r < 0)
        return r;
    r = message_append_field_string(t, BUS_MESSAGE_HEADER_INTERFACE,
                                    SD_BUS_TYPE_STRING, interface,
                                    &t->interface);
    if (r < 0)
        return r;
    r = message_append_field_string(t, BUS_MESSAGE_HEADER_MEMBER,
                                    SD_BUS_TYPE_STRING, member, &t->member);
    if (r < 0)
        return r;
...
_public_ int sd_bus_message_new_method_call(sd_bus* bus, sd_bus_message** m,
                                            ...
                                            const char* member)
{
    ...
    if (destination)
    {
        r = message_append_field_string(t, BUS_MESSAGE_HEADER_DESTINATION,
                                        SD_BUS_TYPE_STRING, destination,
                                        &t->destination);
        if (r < 0)
            return r;
    }
...
static int message_new_reply(sd_bus_message* call, uint8_t type,
                             sd_bus_message** m)
{
    ...
    {
        r = message_append_field_string(t, BUS_MESSAGE_HEADER_DESTINATION,
                                        SD_BUS_TYPE_STRING, call->sender,
                                        &t->destination);
        if (r < 0)
            return r;
    }
...
_public_ int sd_bus_message_new_method_return(sd_bus_message* call,
                                              sd_bus_message** m)
{
...
_public_ int sd_bus_message_new_method_error(sd_bus_message* call,
                                             sd_bus_message** m,
                                             const sd_bus_error* e)
{
    ...
    r = message_append_field_string(t, BUS_MESSAGE_HEADER_ERROR_NAME,
                                    SD_BUS_TYPE_STRING, e->name,
                                    &t->error.name);
    if (r < 0)
        return r;
    ...
    if (e->message)
    {
        r = message_append_basic(t, SD_BUS_TYPE_STRING, e->message,
                                 (const void**)&t->error.message);
        if (r < 0)
            return r;
    }
...
_public_ int sd_bus_message_new_method_errorf(sd_bus_message* call,
                                              sd_bus_message** m,
                                              const char* name,
                                              const char* format, ...)
{
...
_public_ int sd_bus_message_new_method_errnof(sd_bus_message* call,
                                              sd_bus_message** m, int error,
                                              const char* format, ...)
{
...
int bus_message_new_synthetic_error(sd_bus* bus, uint64_t cookie,
                                    const sd_bus_error* e, sd_bus_message** m)
{
    ...
    {
        r = message_append_field_string(t, BUS_MESSAGE_HEADER_DESTINATION,
                                        SD_BUS_TYPE_STRING, bus->unique_name,
                                        &t->destination);
        if (r < 0)
            return r;
    }
...
_public_ sd_bus_message* sd_bus_message_ref(sd_bus_message* m)
{
    ...
    /* We are fine if this message so far was either explicitly reffed or not
     * reffed but queued into at least one bus connection object. */
    assert(m->n_ref > 0 || m->n_queued > 0);

    m->n_ref++;

    /* Each user reference to a bus message shall also be considered a ref on
     * the bus */
    sd_bus_ref(m->bus);
    return m;
}
...
_public_ sd_bus_message* sd_bus_message_unref(sd_bus_message* m)
{
    ...
    sd_bus_unref(
        m->bus); /* Each regular ref is also a ref on the bus connection. Let's
                  * hence drop it here. Note we have to do this before
                  * decrementing our own n_ref here, since otherwise, if this
                  * message is currently queued sd_bus_unref() might call
                  * bus_message_unref_queued() for this which might then destroy
                  * the message while we are still processing it. */
    m->n_ref--;

    if (m->n_ref > 0 || m->n_queued > 0)
        return NULL;

    /* Unset the bus field if neither the user has a reference nor this message
     * is queued. We are careful to reset the field only after the last
     * reference to the bus is dropped, after all we might keep multiple
     * references to the bus, once for each reference kept on outselves. */
    m->bus = NULL;
...
sd_bus_message* bus_message_ref_queued(sd_bus_message* m, sd_bus* bus)
{
    ...
    /* If this is a different bus than the message is associated with, then
     * implicitly turn this into a regular reference. This means that you can
     * create a memory leak by enqueuing a message generated on one bus onto
     * another at the same time as enqueueing a message from the second one on
     * the first, as we'll not detect the cyclic references there. */
    if (bus != m->bus)
...
_public_ int sd_bus_message_get_reply_cookie(sd_bus_message* m,
                                             uint64_t* cookie)
{
    assert_return(m, -EINVAL);
    assert_return(cookie, -EINVAL);
...
_public_ int
    sd_bus_message_get_allow_interactive_authorization(sd_bus_message* m)
{
    assert_return(m, -EINVAL);
...
_public_ int sd_bus_message_get_monotonic_usec(sd_bus_message* m,
                                               uint64_t* usec)
{
    assert_return(m, -EINVAL);
...
_public_ int sd_bus_message_is_signal(sd_bus_message* m, const char* interface,
                                      const char* member)
{
...
_public_ int
    sd_bus_message_set_allow_interactive_authorization(sd_bus_message* m, int b)
{
    assert_return(m, -EINVAL);
    assert_return(!m->sealed, -EPERM);
...
static int part_make_space(struct sd_bus_message* m, struct bus_body_part* part,
                           size_t sz, void** q)
{
...
static void* message_extend_body(sd_bus_message* m, size_t align, size_t sz,
                                 bool add_offset, bool force_inline)
{
    ...
    if (added > 0)
    {
        ...
        add_new_part = m->n_body_parts <= 0 || m->body_end->sealed ||
                       (padding != ALIGN_TO(m->body_end->size, align) -
                                       m->body_end->size) ||
        ...
        else
        {
            ...
            for (c = m->containers; c < m->containers + m->n_containers; c++)
                c->array_size =
                    adjust_pointer(c->array_size, op, os, part->data);

            m->error.message = (const char*)adjust_pointer(m->error.message, op,
                                                           os, part->data);
        }
    }
    else
...
int message_append_basic(sd_bus_message* m, char type, const void* p,
                         const void** stored)
{
    ...
    else
    {
        ...
        /* Maybe we can append to the signature? But only if this is the
         * top-level container */
        if (c->enclosing != 0)
            return -ENXIO;
...
_public_ int sd_bus_message_append_basic(sd_bus_message* m, char type,
                                         const void* p)
{
    return message_append_basic(m, type, p, NULL);
}

_public_ int sd_bus_message_append_string_space(sd_bus_message* m, size_t size,
                                                char** s)
{
...
_public_ int sd_bus_message_append_string_iovec(
    sd_bus_message* m, const struct iovec* iov,
...
static int bus_message_open_array(sd_bus_message* m, struct bus_container* c,
                                  const char* contents, uint32_t** array_size,
                                  size_t* begin, bool* need_offsets)
{
...
static int bus_message_open_variant(sd_bus_message* m, struct bus_container* c,
                                    const char* contents)
{
...
static int bus_message_open_struct(sd_bus_message* m, struct bus_container* c,
                                   const char* contents, size_t* begin,
                                   bool* need_offsets)
{
    ...
    else
    {
        ...
        e = strextend(&c->signature, CHAR_TO_STR(SD_BUS_TYPE_STRUCT_BEGIN),
                      contents, CHAR_TO_STR(SD_BUS_TYPE_STRUCT_END), NULL);
        if (!e)
        {
            m->poisoned = true;
            return -ENOMEM;
...
static int bus_message_open_dict_entry(sd_bus_message* m,
                                       struct bus_container* c,
                                       const char* contents, size_t* begin,
                                       bool* need_offsets)
{
...
_public_ int sd_bus_message_open_container(sd_bus_message* m, char type,
                                           const char* contents)
{
...
static int bus_message_close_array(sd_bus_message* m, struct bus_container* c)
{
    ...
    if (c->need_offsets)
    {
        ...
        payload =
            c->n_offsets > 0 ? c->offsets[c->n_offsets - 1] - c->begin : 0;
        ...
        for (i = 0; i < c->n_offsets; i++)
            bus_gvariant_write_word_le(a + sz * i, sz,
                                       c->offsets[i] - c->begin);
    }
    else
    {
        ...
        a = message_extend_body(m, 1, 0, true,
                                false); /* let's add offset to parent */
        if (!a)
            return -ENOMEM;
    }
                                    ...
                                    bool add_offset)
{
    ...
    else
    {
        ...
        sz = bus_gvariant_determine_word_size(m->body_size - c->begin,
                                              n_variable);
        ...
        for (i = 0, j = 0; i < c->n_offsets; i++)
        {
            ...
            bus_gvariant_write_word_le(a + k * sz, sz,
                                       c->offsets[i] - c->begin);
...
static int type_stack_push(TypeStack* stack, unsigned max, unsigned* i,
                           const char* types, unsigned n_struct,
                           unsigned n_array)
{
    assert(stack);
    assert(max > 0);
...
static int type_stack_pop(TypeStack* stack, unsigned max, unsigned* i,
                          const char** types, unsigned* n_struct,
                          unsigned* n_array)
{
    assert(stack);
...
_public_ int sd_bus_message_appendv(sd_bus_message* m, const char* types,
                                    va_list ap)
{
    ...
    for (;;)
    {
        ...
        {
            r = type_stack_pop(stack, ELEMENTSOF(stack), &stack_ptr, &types,
                               &n_struct, &n_array);
            if (r < 0)
                return r;
            if (r == 0)
        ...
        switch (*t)
        {
            ...
            case SD_BUS_TYPE_ARRAY:
            {
                ...
                r = type_stack_push(stack, ELEMENTSOF(stack), &stack_ptr, types,
                                    n_struct, n_array);
                if (r < 0)
                    return r;
            ...
            case SD_BUS_TYPE_DICT_ENTRY_BEGIN:
            {
...

                {
                    ...
                    r = sd_bus_message_open_container(
                        m,
                        *t == SD_BUS_TYPE_STRUCT_BEGIN ? SD_BUS_TYPE_STRUCT
...
_public_ int sd_bus_message_append_array_space(sd_bus_message* m, char type,
                                               size_t size, void** ptr)
{
    ...
    assert_return(m, -EINVAL);
    assert_return(!m->sealed, -EPERM);
    assert_return(bus_type_is_trivial(type) && type != SD_BUS_TYPE_BOOLEAN,
                  -EINVAL);
...
_public_ int sd_bus_message_append_array(sd_bus_message* m, char type,
...
_public_ int sd_bus_message_append_array_iovec(
    sd_bus_message* m, char type, const struct iovec* iov,
    unsigned n /* should be size_t, but is API now… 😞 */)
{
...
_public_ int sd_bus_message_append_array_memfd(sd_bus_message* m, char type,
...
static int bus_message_close_header(sd_bus_message* m)
{
    ...
    if (BUS_MESSAGE_IS_GVARIANT(m))
    {
        ...
        if (m->n_header_offsets >= 1)
        {
            ...
            assert(m->fields_size ==
                   m->header_offsets[m->n_header_offsets - 1]);

            sz = bus_gvariant_determine_word_size(m->fields_size,
                                                  m->n_header_offsets);
        ...
        sz = bus_gvariant_determine_word_size(sizeof(struct bus_header) +
                                                  ALIGN8(m->fields_size) +
                                                  m->body_size + 1 + l + 2,
                                              1);
        ...
        *((uint8_t*)d + 1 + l + 1) = SD_BUS_TYPE_STRUCT_END;
        ...
        bus_gvariant_write_word_le((uint8_t*)d + 1 + l + 2, sz,
...
_public_ int sd_bus_message_seal(sd_bus_message* m, uint64_t cookie,
                                 uint64_t timeout_usec)
{
    ...
    if (m->header->type == SD_BUS_MESSAGE_METHOD_RETURN &&
        ...
        !streq(strempty(m->root_container.signature),
               m->enforced_reply_signature))
        return -ENOMSG;
    ...
    {
        r = message_append_field_signature(m, BUS_MESSAGE_HEADER_SIGNATURE,
    ...
    if (m->n_fds > 0)
    {
        r = message_append_field_uint32(m, BUS_MESSAGE_HEADER_UNIX_FDS,
                                        m->n_fds);
        if (r < 0)
            return r;
    }
    ...
    m->timeout =
        m->header->flags & BUS_MESSAGE_NO_REPLY_EXPECTED ? 0 : timeout_usec;
    ...
    m->root_container.item_size =
        m->root_container.n_offsets > 0 ? m->root_container.offsets[0] : 0;
...
int bus_body_part_map(struct bus_body_part* part)
{
    ...
    /* For smaller zero parts (as used for padding) we don't need to map
     * anything... */
    ...
    shift =
        part->memfd_offset - ((part->memfd_offset / page_size()) * page_size());
    psz = PAGE_ALIGN(part->size + shift);

    if (part->memfd >= 0)
        p = mmap(NULL, psz, PROT_READ, MAP_PRIVATE, part->memfd,
                 part->memfd_offset - shift);
...
static int buffer_peek(const void* p, uint32_t sz, size_t* rindex, size_t align,
                       size_t nbytes, void** r)
{
    size_t k, start, end;
...
static struct bus_body_part* find_part(sd_bus_message* m, size_t index,
                                       size_t sz, void** p)
{
    struct bus_body_part* part;
    size_t begin;
...
static int container_next_item(sd_bus_message* m, struct bus_container* c,
                               size_t* rindex)
{
    int r;
    ...
    else if (IN_SET(c->enclosing, 0, SD_BUS_TYPE_STRUCT,
                    SD_BUS_TYPE_DICT_ENTRY))
    {
...
static int message_peek_body(sd_bus_message* m, size_t* rindex, size_t align,
                             size_t nbytes, void** ret)
{
...
_public_ int sd_bus_message_read_basic(sd_bus_message* m, char type, void* p)
{
    ...
    if (BUS_MESSAGE_IS_GVARIANT(m))
    {
                   ...
                   SD_BUS_TYPE_SIGNATURE))
        {
            ...
            /* D-Bus spec: The marshalling formats for the string-like types all
             * end with a single zero (NUL) byte, but that byte is not
...
static int bus_message_enter_array(sd_bus_message* m, struct bus_container* c,
                                   const char* contents, uint32_t** array_size,
                                   size_t* item_size, size_t** offsets,
                                   size_t* n_offsets)
{
...
static int bus_message_enter_variant(sd_bus_message* m, struct bus_container* c,
                                     const char* contents, size_t* item_size)
{
...
static int build_struct_offsets(sd_bus_message* m, const char* signature,
            ...
            if (k < 0)
            {
                ...
                if (offset < start)
                    return log_debug_errno(
                        ...
                        "For type %s with alignment %zu, message specifies "
                        "offset %zu which is smaller than previous end %zu + "
                        "alignment = %zu",
                                      ...
                                      size_t** offsets, size_t* n_offsets)
{
    ...
    if (!BUS_MESSAGE_IS_GVARIANT(m))
    {
        ...
        return build_struct_offsets(m, contents, c->item_size, item_size,
                                    offsets, n_offsets);

    return 0;
}

static int bus_message_enter_struct(sd_bus_message* m, struct bus_container* c,
                                    const char* contents, size_t* item_size,
                                    size_t** offsets, size_t* n_offsets)
{
    ...
    r = enter_struct_or_dict_entry(m, c, contents, item_size, offsets,
                                   n_offsets);
    if (r < 0)
        return r;
...
_public_ int sd_bus_message_enter_container(sd_bus_message* m, char type,
                                            const char* contents)
{
    struct bus_container* c;
    uint32_t* array_size = NULL;
    ...
    if (type == SD_BUS_TYPE_ARRAY)
        r = bus_message_enter_array(m, c, contents, &array_size, &item_size,
                                    &offsets, &n_offsets);
    else if (type == SD_BUS_TYPE_VARIANT)
        r = bus_message_enter_variant(m, c, contents, &item_size);
    else if (type == SD_BUS_TYPE_STRUCT)
        r = bus_message_enter_struct(m, c, contents, &item_size, &offsets,
                                     &n_offsets);
    else if (type == SD_BUS_TYPE_DICT_ENTRY)
        r = bus_message_enter_dict_entry(m, c, contents, &item_size, &offsets,
                                         &n_offsets);
    else
        r = -EINVAL;
    if (r <= 0)
    ...
    /* OK, let's fill it in */
    if (BUS_MESSAGE_IS_GVARIANT(m) && type == SD_BUS_TYPE_STRUCT &&
        isempty(signature))
        end = m->rindex + 0;
    else
...
_public_ int sd_bus_message_peek_type(sd_bus_message* m, char* type,
                                      const char** contents)
{
    struct bus_container* c;
    int r;
    ...
    if (IN_SET(c->signature[c->index], SD_BUS_TYPE_STRUCT_BEGIN,
               SD_BUS_TYPE_DICT_ENTRY_BEGIN))
    {
        ...
        if (type)
            *type = c->signature[c->index] == SD_BUS_TYPE_STRUCT_BEGIN
                        ? SD_BUS_TYPE_STRUCT
                        : SD_BUS_TYPE_DICT_ENTRY;
    ...
    if (c->signature[c->index] == SD_BUS_TYPE_VARIANT)
    {
        if (contents)
        {
            ...
            if (BUS_MESSAGE_IS_GVARIANT(m))
            {
                ...
                if (free_and_strndup(&c->peeked_signature, (char*)q + 1,
                                     k - 1) < 0)
                    return -ENOMEM;
...
_public_ int sd_bus_message_readv(sd_bus_message* m, const char* types,
                                  va_list ap)
{
    ...
    switch (*types)
    {
        ...
        case SD_BUS_TYPE_DICT_ENTRY_BEGIN:
        {
...

            {
                ...
                r = sd_bus_message_enter_container(
                    m,
                    *types == SD_BUS_TYPE_STRUCT_BEGIN ? SD_BUS_TYPE_STRUCT
                                                       : SD_BUS_TYPE_DICT_ENTRY,
                    s);
                if (r <= 0)
                    return r;
...
_public_ int sd_bus_message_read_array(sd_bus_message* m, char type,
                                       const void** ptr, size_t* size)
{
...
static int message_peek_fields(sd_bus_message* m, size_t* rindex, size_t align,
                               size_t nbytes, void** ret)
{
    ...
    return buffer_peek(BUS_MESSAGE_FIELDS(m), m->fields_size, rindex, align,
                       nbytes, ret);
}

static int message_peek_field_uint32(sd_bus_message* m, size_t* ri,
                                     size_t item_size, uint32_t* ret)
{
...
static int message_peek_field_uint64(sd_bus_message* m, size_t* ri,
                                     size_t item_size, uint64_t* ret)
{
...
static int message_peek_field_string(sd_bus_message* m,
                                     bool (*validate)(const char* p),
                                     size_t* ri, size_t item_size,
                                     const char** ret)
{
...
static int message_peek_field_signature(sd_bus_message* m, size_t* ri,
                                        size_t item_size, const char** ret)
{
...
static int message_skip_fields(sd_bus_message* m, size_t* ri,
                               uint32_t array_size, const char** signature)
{
...
int bus_message_parse_fields(sd_bus_message* m)
{
    ...
    if (BUS_MESSAGE_IS_GVARIANT(m))
    {
         ...
         * table */
        m->user_body_size =
            m->body_size - ((char*)m->footer + m->footer_accessible - p);
        ...
        if (sz > 0)
        {
            ...
            ri = framing;
            r = message_peek_fields(m, &ri, 1, m->fields_size - framing,
                                    &offsets);
            if (r < 0)
                return r;
    ...
    while (ri < m->fields_size)
    {
        ...
        if (BUS_MESSAGE_IS_GVARIANT(m))
        {
            ...
            if (i == 0)
                ...
                ri = ALIGN_TO(bus_gvariant_read_word_le(
                                  (uint8_t*)offsets + (i - 1) * sz, sz),
                              8);
        ...
        switch (field_type)
        {
            ...
            case BUS_MESSAGE_HEADER_PATH:
                ...
                r = message_peek_field_string(m, object_path_is_valid, &ri,
                                              item_size, &m->path);
                break;
            ...
            case BUS_MESSAGE_HEADER_INTERFACE:
                ...
                r = message_peek_field_string(m, interface_name_is_valid, &ri,
                                              item_size, &m->interface);
                break;
            ...
            case BUS_MESSAGE_HEADER_MEMBER:
                ...
                r = message_peek_field_string(m, member_name_is_valid, &ri,
                                              item_size, &m->member);
                break;
            ...
            case BUS_MESSAGE_HEADER_ERROR_NAME:
                ...
                r = message_peek_field_string(m, error_name_is_valid, &ri,
                                              item_size, &m->error.name);
                if (r >= 0)
            ...
            case BUS_MESSAGE_HEADER_DESTINATION:
                ...
                r = message_peek_field_string(m, service_name_is_valid, &ri,
                                              item_size, &m->destination);
                break;
            ...
            case BUS_MESSAGE_HEADER_SENDER:
                ...
                if (r >= 0 && m->sender[0] == ':' && m->bus->bus_client)
                {
                    ...
                    m->creds.mask |=
                        SD_BUS_CREDS_UNIQUE_NAME & m->bus->creds_mask;
                }
            ...
            case BUS_MESSAGE_HEADER_REPLY_SERIAL:
                ...
                if (BUS_MESSAGE_IS_GVARIANT(m))
                {
                    ...
                    r = message_peek_field_uint64(m, &ri, item_size,
                                                  &m->reply_cookie);
                    if (r < 0)
                        return r;
                }
            ...
            default:
                if (!BUS_MESSAGE_IS_GVARIANT(m))
                    r = message_skip_fields(m, &ri, (uint32_t)-1,
                                            (const char**)&signature);
        }
    ...
    if (BUS_MESSAGE_IS_GVARIANT(m))
    {
        r = build_struct_offsets(
            m, m->root_container.signature, m->user_body_size,
            &m->root_container.item_size, &m->root_container.offsets,
...
_public_ int sd_bus_message_set_destination(sd_bus_message* m,
                                            const char* destination)
{
    assert_return(m, -EINVAL);
...
_public_ int sd_bus_message_set_sender(sd_bus_message* m, const char* sender)
{
    ...
    return message_append_field_string(m, BUS_MESSAGE_HEADER_SENDER,
...
static int bus_message_get_arg_skip(sd_bus_message* m, unsigned i, char* _type,
...
_public_ const char* sd_bus_message_get_signature(sd_bus_message* m,
                                                  int complete)
{
    struct bus_container* c;
...
_public_ int sd_bus_message_has_signature(sd_bus_message* m,
                                          const char* signature)
{
    assert_return(m, -EINVAL);
...
_public_ int sd_bus_message_copy(sd_bus_message* m, sd_bus_message* source,
                                 int all)
{
    ...
    do
    {
        ...
        if (IN_SET(type, SD_BUS_TYPE_OBJECT_PATH, SD_BUS_TYPE_SIGNATURE,
                   SD_BUS_TYPE_STRING))
...
_public_ int sd_bus_message_verify_type(sd_bus_message* m, char type,
                                        const char* contents)
{
    const char* c;
    char t;
...
int bus_message_remarshal(sd_bus* bus, sd_bus_message** m)
{
    ...
    switch ((*m)->header->type)
    {
        ...
        case SD_BUS_MESSAGE_SIGNAL:
            r = sd_bus_message_new_signal(bus, &n, (*m)->path, (*m)->interface,
                                          (*m)->member);
            if (r < 0)
                return r;
        ...
        case SD_BUS_MESSAGE_METHOD_CALL:
            r = sd_bus_message_new_method_call(bus, &n, (*m)->destination,
                                               (*m)->path, (*m)->interface,
                                               (*m)->member);
            if (r < 0)
                return r;
        ...
        case SD_BUS_MESSAGE_METHOD_ERROR:
                ...
                (*m)->error.name)
            {
                r = message_append_field_string(
                    n, BUS_MESSAGE_HEADER_ERROR_NAME, SD_BUS_TYPE_STRING,
                    (*m)->error.name, &n->error.message);
                if (r < 0)
                    return r;
    ...
    {
        r = message_append_field_string(n, BUS_MESSAGE_HEADER_DESTINATION,
                                        SD_BUS_TYPE_STRING, (*m)->destination,
                                        &n->destination);
        if (r < 0)
            return r;
    }
    ...
    {
        r = message_append_field_string(n, BUS_MESSAGE_HEADER_SENDER,
                                        SD_BUS_TYPE_STRING, (*m)->sender,
                                        &n->sender);
        if (r < 0)
            return r;
    }

    n->header->flags |= (*m)->header->flags & (BUS_MESSAGE_NO_REPLY_EXPECTED |
