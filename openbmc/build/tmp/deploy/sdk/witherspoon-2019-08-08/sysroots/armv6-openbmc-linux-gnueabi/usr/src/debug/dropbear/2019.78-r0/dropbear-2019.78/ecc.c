...
/* For the "ephemeral public key octet string" in ECDH (rfc5656 section 4) */
ecc_key* buf_get_ecc_raw_pubkey(buffer* buf,
                                const struct dropbear_ecc_curve* curve)
{
    ...
    if (mp_read_unsigned_bin(key->pubkey.x, buf_getptr(buf, size), size) !=
        MP_OKAY)
    {
    ...
    if (mp_read_unsigned_bin(key->pubkey.y, buf_getptr(buf, size), size) !=
        MP_OKAY)
    {
...
/* a modified version of libtomcrypt's "ecc_shared_secret" to output
   a mp_int instead. */
mp_int* dropbear_ecc_shared_secret(ecc_key* public_key,
                                   const ecc_key* private_key)
{
    ...
    if (ltc_mp.ecc_ptmul(private_key->k, &public_key->pubkey, result, prime,
                         1) != CRYPT_OK)
    {
        goto out;
    }
