 ...
 * [D-Bus
 * specification](http://dbus.freedesktop.org/doc/dbus-specification.html#introspection-format)
 */
...
static void free_null_terminated_array(gpointer array,
...
void g_dbus_method_info_unref(GDBusMethodInfo* info)
{
    ...
    {
        g_free(info->name);
        free_null_terminated_array(info->in_args,
                                   (GDestroyNotify)g_dbus_arg_info_unref);
        free_null_terminated_array(info->out_args,
                                   (GDestroyNotify)g_dbus_arg_info_unref);
        free_null_terminated_array(
            info->annotations, (GDestroyNotify)g_dbus_annotation_info_unref);
        g_free(info);
...
void g_dbus_signal_info_unref(GDBusSignalInfo* info)
{
    ...
    {
        g_free(info->name);
        free_null_terminated_array(info->args,
                                   (GDestroyNotify)g_dbus_arg_info_unref);
        free_null_terminated_array(
            info->annotations, (GDestroyNotify)g_dbus_annotation_info_unref);
        g_free(info);
...
void g_dbus_interface_info_unref(GDBusInterfaceInfo* info)
{
    ...
    {
        g_free(info->name);
        free_null_terminated_array(info->methods,
                                   (GDestroyNotify)g_dbus_method_info_unref);
        free_null_terminated_array(info->signals,
                                   (GDestroyNotify)g_dbus_signal_info_unref);
        free_null_terminated_array(info->properties,
                                   (GDestroyNotify)g_dbus_property_info_unref);
        free_null_terminated_array(
            info->annotations, (GDestroyNotify)g_dbus_annotation_info_unref);
        g_free(info);
    }
...
void g_dbus_node_info_unref(GDBusNodeInfo* info)
{
    ...
    {
        g_free(info->path);
        free_null_terminated_array(info->interfaces,
                                   (GDestroyNotify)g_dbus_interface_info_unref);
        free_null_terminated_array(info->nodes,
...
static void
    g_dbus_annotation_info_set(ParseData* data, GDBusAnnotationInfo* info,
...
static void g_dbus_arg_info_set(ParseData* data, GDBusArgInfo* info,
...
static void g_dbus_method_info_set(ParseData* data, GDBusMethodInfo* info,
                                   const gchar* name, GDBusArgInfo** in_args,
...
static void g_dbus_signal_info_set(ParseData* data, GDBusSignalInfo* info,
                                   const gchar* name, GDBusArgInfo** args,
                                   GDBusAnnotationInfo** annotations)
{
    info->ref_count = 1;
...
static void g_dbus_property_info_set(ParseData* data, GDBusPropertyInfo* info,
                                     const gchar* name, const gchar* signature,
...
static void g_dbus_interface_info_set(ParseData* data, GDBusInterfaceInfo* info,
                                      const gchar* name,
...
static void g_dbus_node_info_set(ParseData* data, GDBusNodeInfo* info,
                                 const gchar* path,
...
static void g_dbus_annotation_info_generate_xml(GDBusAnnotationInfo* info,
                                                guint indent,
                                                GString* string_builder)
{
    ...
    else
    {
        ...
        for (n = 0; info->annotations != NULL && info->annotations[n] != NULL;
             n++)
...
static void g_dbus_arg_info_generate_xml(GDBusArgInfo* info, guint indent,
                                         const gchar* extra_attributes,
                                         GString* string_builder)
{
    guint n;

    g_string_append_printf(string_builder, "%*s<arg type=\"%s\"", indent, "",
                           info->signature);
...
static void g_dbus_method_info_generate_xml(GDBusMethodInfo* info, guint indent,
                                            GString* string_builder)
{
    guint n;

    g_string_append_printf(string_builder, "%*s<method name=\"%s\"", indent, "",
                           info->name);

    if (info->annotations == NULL && info->in_args == NULL &&
        info->out_args == NULL)
    {
        g_string_append(string_builder, "/>\n");
    }
    else
    {
        ...
        for (n = 0; info->in_args != NULL && info->in_args[n] != NULL; n++)
            g_dbus_arg_info_generate_xml(info->in_args[n], indent + 2,
                                         "direction=\"in\"", string_builder);

        for (n = 0; info->out_args != NULL && info->out_args[n] != NULL; n++)
            g_dbus_arg_info_generate_xml(info->out_args[n], indent + 2,
                                         "direction=\"out\"", string_builder);

        g_string_append_printf(string_builder, "%*s</method>\n", indent, "");
    }
}

static void g_dbus_signal_info_generate_xml(GDBusSignalInfo* info, guint indent,
                                            GString* string_builder)
{
    guint n;

    g_string_append_printf(string_builder, "%*s<signal name=\"%s\"", indent, "",
                           info->name);
    ...
    else
    {
        ...
        for (n = 0; info->args != NULL && info->args[n] != NULL; n++)
            g_dbus_arg_info_generate_xml(info->args[n], indent + 2, NULL,
                                         string_builder);
...
static void g_dbus_property_info_generate_xml(GDBusPropertyInfo* info,
                                              guint indent,
                                              GString* string_builder)
{
    guint n;
    ...
    g_string_append_printf(
        string_builder, "%*s<property type=\"%s\" name=\"%s\" access=\"%s\"",
        indent, "", info->signature, info->name, access_string);
 ...
 * Since: 2.26
 */
void g_dbus_interface_info_generate_xml(GDBusInterfaceInfo* info, guint indent,
                                        GString* string_builder)
{
    guint n;
    ...
    for (n = 0; info->annotations != NULL && info->annotations[n] != NULL; n++)
        g_dbus_annotation_info_generate_xml(info->annotations[n], indent + 2,
                                            string_builder);

    for (n = 0; info->methods != NULL && info->methods[n] != NULL; n++)
        g_dbus_method_info_generate_xml(info->methods[n], indent + 2,
                                        string_builder);

    for (n = 0; info->signals != NULL && info->signals[n] != NULL; n++)
        g_dbus_signal_info_generate_xml(info->signals[n], indent + 2,
                                        string_builder);

    for (n = 0; info->properties != NULL && info->properties[n] != NULL; n++)
        g_dbus_property_info_generate_xml(info->properties[n], indent + 2,
                                          string_builder);
 ...
 * Since: 2.26
 */
void g_dbus_node_info_generate_xml(GDBusNodeInfo* info, guint indent,
                                   GString* string_builder)
{
    guint n;
    ...
    if (info->interfaces == NULL && info->nodes == NULL &&
    ...
    else
    {
        ...
        for (n = 0; info->interfaces != NULL && info->interfaces[n] != NULL;
             n++)
            g_dbus_interface_info_generate_xml(info->interfaces[n], indent + 2,
                                               string_builder);

        for (n = 0; info->nodes != NULL && info->nodes[n] != NULL; n++)
            g_dbus_node_info_generate_xml(info->nodes[n], indent + 2,
                                          string_builder);
...
static GDBusArgInfo** parse_data_steal_args(ParseData* data,
...
static GDBusArgInfo** parse_data_steal_out_args(ParseData* data,
                                                guint* out_num_elements)
{
    GDBusArgInfo** ret;
    if (out_num_elements != NULL)
...
static GDBusMethodInfo** parse_data_steal_methods(ParseData* data,
                                                  guint* out_num_elements)
{
    GDBusMethodInfo** ret;
    if (out_num_elements != NULL)
...
static GDBusSignalInfo** parse_data_steal_signals(ParseData* data,
                                                  guint* out_num_elements)
{
    GDBusSignalInfo** ret;
    if (out_num_elements != NULL)
...
static GDBusPropertyInfo** parse_data_steal_properties(ParseData* data,
                                                       guint* out_num_elements)
{
...
static GDBusInterfaceInfo** parse_data_steal_interfaces(ParseData* data,
                                                        guint* out_num_elements)
{
...
static GDBusNodeInfo** parse_data_steal_nodes(ParseData* data,
                                              guint* out_num_elements)
{
    GDBusNodeInfo** ret;
    if (out_num_elements != NULL)
...
static void parse_data_free_annotations(ParseData* data)
{
    if (data->annotations == NULL)
        return;
    g_ptr_array_foreach(data->annotations, (GFunc)g_dbus_annotation_info_unref,
                        NULL);
...
static void parse_data_free_properties(ParseData* data)
{
    if (data->properties == NULL)
        return;
    g_ptr_array_foreach(data->properties, (GFunc)g_dbus_property_info_unref,
                        NULL);
...
static void parse_data_free_interfaces(ParseData* data)
{
    if (data->interfaces == NULL)
        return;
    g_ptr_array_foreach(data->interfaces, (GFunc)g_dbus_interface_info_unref,
                        NULL);
...
static GDBusAnnotationInfo* parse_data_get_annotation(ParseData* data,
                                                      gboolean create_new)
{
    if (create_new)
...
static GDBusArgInfo* parse_data_get_out_arg(ParseData* data,
                                            gboolean create_new)
{
    if (create_new)
...
static GDBusMethodInfo* parse_data_get_method(ParseData* data,
                                              gboolean create_new)
{
    if (create_new)
...
static GDBusSignalInfo* parse_data_get_signal(ParseData* data,
                                              gboolean create_new)
{
    if (create_new)
...
static GDBusPropertyInfo* parse_data_get_property(ParseData* data,
                                                  gboolean create_new)
{
    if (create_new)
...
static GDBusInterfaceInfo* parse_data_get_interface(ParseData* data,
                                                    gboolean create_new)
{
    if (create_new)
...
static void parse_data_free(ParseData* data)
{
    ...
    for (l = data->annotations_stack; l != NULL; l = l->next)
    {
        ...
        g_ptr_array_foreach(annotations, (GFunc)g_dbus_annotation_info_unref,
                            NULL);
    ...
    for (l = data->interfaces_stack; l != NULL; l = l->next)
    {
        ...
        g_ptr_array_foreach(interfaces, (GFunc)g_dbus_interface_info_unref,
                            NULL);
    ...
    /* free arrays (data->annotations, data->interfaces and data->nodes have
     * been freed above) */
                                 ...
                                 gpointer user_data, GError** error)
{
    ...
    if (strcmp(element_name, "node") == 0)
    {
              ...
              strcmp(stack->next->data, "node") != 0))
        {
            g_set_error_literal(error, G_MARKUP_ERROR,
                                ...
                                "<node> elements can only be top-level or "
                                "embedded in other <node> elements");
            goto out;
        }

        if (!g_markup_collect_attributes(
                element_name, attribute_names, attribute_values, error,
                G_MARKUP_COLLECT_STRING | G_MARKUP_COLLECT_OPTIONAL, "name",
                &name,
                /* some hand-written introspection XML documents use this */
                G_MARKUP_COLLECT_STRING | G_MARKUP_COLLECT_OPTIONAL,
                "xmlns:doc", NULL, G_MARKUP_COLLECT_INVALID))
            goto out;

        g_dbus_node_info_set(data, parse_data_get_node(data, TRUE), name, NULL,
                             NULL, NULL);

        /* push the currently retrieved interfaces and nodes on the stack and
         * prepare new arrays */
        data->interfaces_stack =
            g_slist_prepend(data->interfaces_stack, data->interfaces);
        data->interfaces = NULL;
        parse_data_steal_interfaces(data, NULL);
    ...
    else if (strcmp(element_name, "interface") == 0)
    {
        ...
        if (!g_markup_collect_attributes(
                ...
                /* seen in the wild */
                G_MARKUP_COLLECT_STRING | G_MARKUP_COLLECT_OPTIONAL, "version",
                NULL, G_MARKUP_COLLECT_INVALID))
            goto out;

        g_dbus_interface_info_set(data, parse_data_get_interface(data, TRUE),
    ...
    else if (strcmp(element_name, "method") == 0)
    {
            ...
            strcmp(stack->next->data, "interface") != 0)
        {
            g_set_error_literal(error, G_MARKUP_ERROR,
                                G_MARKUP_ERROR_INVALID_CONTENT,
                                "<method> elements can only be embedded in "
                                "<interface> elements");
            goto out;
        }
        ...
        g_dbus_method_info_set(data, parse_data_get_method(data, TRUE), name,
                               NULL, NULL, NULL);
    ...
    else if (strcmp(element_name, "signal") == 0)
    {
            ...
            strcmp(stack->next->data, "interface") != 0)
        {
            g_set_error_literal(error, G_MARKUP_ERROR,
                                G_MARKUP_ERROR_INVALID_CONTENT,
                                "<signal> elements can only be embedded in "
                                "<interface> elements");
            goto out;
        }
        ...
        g_dbus_signal_info_set(data, parse_data_get_signal(data, TRUE), name,
                               NULL, NULL);
    ...
    else if (strcmp(element_name, "property") == 0)
    {
            ...
            strcmp(stack->next->data, "interface") != 0)
        {
            g_set_error_literal(error, G_MARKUP_ERROR,
                                G_MARKUP_ERROR_INVALID_CONTENT,
                                "<property> elements can only be embedded in "
                                "<interface> elements");
            goto out;
        }
        ...
        if (!g_markup_collect_attributes(
                element_name, attribute_names, attribute_values, error,
                G_MARKUP_COLLECT_STRING, "name", &name, G_MARKUP_COLLECT_STRING,
                "type", &type, G_MARKUP_COLLECT_STRING, "access", &access,
                G_MARKUP_COLLECT_INVALID))
            goto out;
        ...
        g_dbus_property_info_set(data, parse_data_get_property(data, TRUE),
                                 name, type, flags, NULL);
    }
    /* ----------------------------------------------------------------------------------------------------
     */
    else if (strcmp(element_name, "arg") == 0)
    {
             ...
             strcmp(stack->next->data, "signal") != 0))
        {
            g_set_error_literal(error, G_MARKUP_ERROR,
                                G_MARKUP_ERROR_INVALID_CONTENT,
                                "<arg> elements can only be embedded in "
                                "<method> or <signal> elements");
            goto out;
        }
        ...
        if (!g_markup_collect_attributes(
                element_name, attribute_names, attribute_values, error,
                G_MARKUP_COLLECT_STRING | G_MARKUP_COLLECT_OPTIONAL, "name",
                &name, G_MARKUP_COLLECT_STRING | G_MARKUP_COLLECT_OPTIONAL,
                "direction", &direction, G_MARKUP_COLLECT_STRING, "type", &type,
                G_MARKUP_COLLECT_INVALID))
            goto out;
        ...
        if (direction != NULL)
        {
            ...
            else
            {
                g_set_error(
                    error, G_MARKUP_ERROR, G_MARKUP_ERROR_INVALID_CONTENT,
                    "Unknown value '%s' of direction attribute", direction);
                goto out;
            }
        }
        ...
        if (is_in && strcmp(stack->next->data, "signal") == 0)
        {
            g_set_error_literal(error, G_MARKUP_ERROR,
                                G_MARKUP_ERROR_INVALID_CONTENT,
                                "Only direction 'out' is allowed for <arg> "
                                "elements embedded in <signal>");
            goto out;
        }
        ...
        if (is_in)
        {
            g_dbus_arg_info_set(data, parse_data_get_arg(data, TRUE),
                                name_to_use, type, NULL);
            data->last_arg_was_in = TRUE;
        }
        else
        {
            g_dbus_arg_info_set(data, parse_data_get_out_arg(data, TRUE),
                                name_to_use, type, NULL);
    ...
    else if (strcmp(element_name, "annotation") == 0)
    {
             ...
             strcmp(stack->next->data, "annotation") != 0))
        {
            g_set_error_literal(error, G_MARKUP_ERROR,
                                G_MARKUP_ERROR_INVALID_CONTENT,
                                "<annotation> elements can only be embedded in "
                                "<node>, <interface>, <signal>, <method>, "
                                "<property>, <arg> or <annotation> elements");
            goto out;
        }
        ...
        if (!g_markup_collect_attributes(
                element_name, attribute_names, attribute_values, error,
                G_MARKUP_COLLECT_STRING, "name", &name, G_MARKUP_COLLECT_STRING,
                "value", &value, G_MARKUP_COLLECT_INVALID))
            goto out;

        g_dbus_annotation_info_set(data, parse_data_get_annotation(data, TRUE),
                                   name, value, NULL);
    }
    /* ----------------------------------------------------------------------------------------------------
     */
    ...
    /* push the currently retrieved annotations on the stack and prepare a new
     * one */
    data->annotations_stack =
        g_slist_prepend(data->annotations_stack, data->annotations);
    data->annotations = NULL;
    parse_data_steal_annotations(data, NULL);
                               ...
                               GError** error)
{
    ...
    if (strcmp(element_name, "node") == 0)
    {
        ...
        /* destroy the nodes, interfaces for scope we're exiting and and pop the
         * nodes, interfaces from the scope we're reentering
         */
        parse_data_free_interfaces(data);
        data->interfaces = (GPtrArray*)data->interfaces_stack->data;
        data->interfaces_stack = g_slist_remove(data->interfaces_stack,
                                                data->interfaces_stack->data);

        parse_data_free_nodes(data);
        data->nodes = (GPtrArray*)data->nodes_stack->data;
        data->nodes_stack =
            g_slist_remove(data->nodes_stack, data->nodes_stack->data);

        g_dbus_node_info_set(data, parse_data_get_node(data, FALSE), NULL,
                             interfaces, nodes, steal_annotations(data));
    }
    else if (strcmp(element_name, "interface") == 0)
    {
        guint num_methods;
        ...
        g_dbus_interface_info_set(data, parse_data_get_interface(data, FALSE),
                                  NULL, methods, signals, properties,
    ...
    else if (strcmp(element_name, "method") == 0)
    {
        ...
        g_dbus_method_info_set(data, parse_data_get_method(data, FALSE), NULL,
                               in_args, out_args, steal_annotations(data));
    }
    else if (strcmp(element_name, "signal") == 0)
    {
        guint num_args;
        ...
        g_dbus_signal_info_set(data, parse_data_get_signal(data, FALSE), NULL,
                               args, steal_annotations(data));
    }
    else if (strcmp(element_name, "property") == 0)
    {
        g_dbus_property_info_set(data, parse_data_get_property(data, FALSE),
                                 NULL, NULL, G_DBUS_PROPERTY_INFO_FLAGS_NONE,
                                 steal_annotations(data));
    }
    else if (strcmp(element_name, "arg") == 0)
    {
        g_dbus_arg_info_set(data,
                            data->last_arg_was_in
                                ? parse_data_get_arg(data, FALSE)
                                : parse_data_get_out_arg(data, FALSE),
    ...
    else if (strcmp(element_name, "annotation") == 0)
    {
        ...
        /* destroy the annotations for scope we're exiting and and pop the
         * annotations from the scope we're reentering */
        parse_data_free_annotations(data);
        data->annotations = (GPtrArray*)data->annotations_stack->data;
        data->annotations_stack = g_slist_remove(data->annotations_stack,
                                                 data->annotations_stack->data);

        have_popped_annotations = TRUE;

        g_dbus_annotation_info_set(data, parse_data_get_annotation(data, FALSE),
                                   NULL, NULL, embedded_annotations);
    }
    else
    {
        /* don't bail on unknown elements; just ignore them */
...
static void parser_error(GMarkupParseContext* context, GError* error,
                         gpointer user_data)
{
    gint line_number;
    gint char_number;
                                            ...
                                            GError** error)
{
    ...
    data = parse_data_new();
    context = g_markup_parse_context_new(parser, G_MARKUP_IGNORE_QUALIFIED,
                                         data, (GDestroyNotify)parse_data_free);

    if (!g_markup_parse_context_parse(context, xml_data, strlen(xml_data),
                                      error))
        goto out;
 ...
 * @annotations: (array zero-terminated=1) (nullable): A %NULL-terminated array
 * of annotations or %NULL.
 ...
 *
 * Returns: The value or %NULL if not found. Do not free, it is owned by
 * @annotations.
 *
 * Since: 2.26
 */
...
static void info_cache_free(InfoCacheEntry* cache)
{
 ...
 *
 * Returns: (transfer none): A #GDBusMethodInfo or %NULL if not found. Do not
 * free, it is owned by @info.
 *
 * Since: 2.26
 */
 ...
 * g_dbus_interface_info_cache_build() has been used on @info.
 *
 * Returns: (transfer none): A #GDBusSignalInfo or %NULL if not found. Do not
 * free, it is owned by @info.
 *
 * Since: 2.26
 */
 ...
 * g_dbus_interface_info_cache_build() has been used on @info.
 *
 * Returns: (transfer none): A #GDBusPropertyInfo or %NULL if not found. Do not
 * free, it is owned by @info.
 *
 * Since: 2.26
 */
...
void g_dbus_interface_info_cache_build(GDBusInterfaceInfo* info)
{
    ...
    G_LOCK(info_cache_lock);
    if (info_cache == NULL)
        info_cache = g_hash_table_new_full(g_direct_hash, g_direct_equal, NULL,
                                           (GDestroyNotify)info_cache_free);
    cache = g_hash_table_lookup(info_cache, info);
    if (cache != NULL)
    {
        cache->use_count += 1;
    ...
    for (n = 0; info->methods != NULL && info->methods[n] != NULL; n++)
        g_hash_table_insert(cache->method_name_to_data, info->methods[n]->name,
                            info->methods[n]);
    for (n = 0; info->signals != NULL && info->signals[n] != NULL; n++)
        g_hash_table_insert(cache->signal_name_to_data, info->signals[n]->name,
                            info->signals[n]);
    for (n = 0; info->properties != NULL && info->properties[n] != NULL; n++)
        g_hash_table_insert(cache->property_name_to_data,
                            info->properties[n]->name, info->properties[n]);
...
void g_dbus_interface_info_cache_release(GDBusInterfaceInfo* info)
{
    ...
    {
        g_warning("%s called for interface %s but there is no cache",
                  info->name, G_STRFUNC);
        goto out;
    }
    ...
    {
        g_warning("%s called for interface %s but there is no cache entry",
                  info->name, G_STRFUNC);
        goto out;
    }
    cache->use_count -= 1;
 ...
 *
 * Returns: (transfer none): A #GDBusInterfaceInfo or %NULL if not found. Do not
 * free, it is owned by @info.
 *
 * Since: 2.26
 */
