...
G_DEFINE_TYPE_WITH_CODE(GDBusObjectSkeleton, g_dbus_object_skeleton,
                        G_TYPE_OBJECT,
                        G_ADD_PRIVATE(GDBusObjectSkeleton)
                            G_IMPLEMENT_INTERFACE(G_TYPE_DBUS_OBJECT,
                                                  dbus_object_interface_init))

static void g_dbus_object_skeleton_finalize(GObject* _object)
{
...
static void g_dbus_object_skeleton_get_property(GObject* _object, guint prop_id,
                                                GValue* value,
                                                GParamSpec* pspec)
{
    GDBusObjectSkeleton* object = G_DBUS_OBJECT_SKELETON(_object);
...
static void g_dbus_object_skeleton_set_property(GObject* _object, guint prop_id,
                                                const GValue* value,
                                                GParamSpec* pspec)
{
    GDBusObjectSkeleton* object = G_DBUS_OBJECT_SKELETON(_object);
...
static gboolean g_dbus_object_skeleton_authorize_method_default(
    GDBusObjectSkeleton* object, GDBusInterfaceSkeleton* interface,
    GDBusMethodInvocation* invocation)
{
    return TRUE;
}

static void g_dbus_object_skeleton_class_init(GDBusObjectSkeletonClass* klass)
{
    ...
    g_object_class_install_property(
        ...
        g_param_spec_string("g-object-path", "Object Path",
                            "The object path where the object is exported",
                            NULL,
     ...
     * Since: 2.30
     */
    signals[AUTHORIZE_METHOD_SIGNAL] = g_signal_new(
        I_("authorize-method"), G_TYPE_DBUS_OBJECT_SKELETON, G_SIGNAL_RUN_LAST,
        G_STRUCT_OFFSET(GDBusObjectSkeletonClass, authorize_method),
        _g_signal_accumulator_false_handled, NULL, NULL, G_TYPE_BOOLEAN, 2,
        G_TYPE_DBUS_INTERFACE_SKELETON, G_TYPE_DBUS_METHOD_INVOCATION);
}

static void g_dbus_object_skeleton_init(GDBusObjectSkeleton* object)
{
...
GDBusObjectSkeleton* g_dbus_object_skeleton_new(const gchar* object_path)
{
    ...
    return G_DBUS_OBJECT_SKELETON(g_object_new(
        G_TYPE_DBUS_OBJECT_SKELETON, "g-object-path", object_path, NULL));
}
...
static const gchar* g_dbus_object_skeleton_get_object_path(GDBusObject* _object)
{
    GDBusObjectSkeleton* object = G_DBUS_OBJECT_SKELETON(_object);
    const gchar* ret;
    g_mutex_lock(&object->priv->lock);
                                          ...
                                          GDBusInterfaceSkeleton* interface_)
{
    ...
    interface_to_remove =
        g_hash_table_lookup(object->priv->map_name_to_iface, info->name);
    if (interface_to_remove != NULL)
    {
        g_object_ref(interface_to_remove);
        g_warn_if_fail(
            g_hash_table_remove(object->priv->map_name_to_iface, info->name));
    }
    g_hash_table_insert(object->priv->map_name_to_iface, g_strdup(info->name),
                        g_object_ref(interface_));
    g_dbus_interface_set_object(G_DBUS_INTERFACE(interface_),
    ...
    if (interface_to_remove != NULL)
    {
        ...
        g_signal_emit_by_name(object, "interface-removed", interface_to_remove);
        g_object_unref(interface_to_remove);
    }

    g_signal_emit_by_name(object, "interface-added", interface_);
                                             ...
                                             GDBusInterfaceSkeleton* interface_)
{
    ...
    else if (other_interface != interface_)
    {
        ...
        g_warning("Tried to remove interface %p with name %s from object "
                  ...
                  interface_, info->name, object->priv->object_path,
                  other_interface);
    }
    else
    {
        ...
        g_signal_emit_by_name(object, "interface-removed", interface_);
        g_object_unref(interface_);
    }
}
    ...
    GDBusObjectSkeleton* object, const gchar* interface_name)
{
    ...
    g_mutex_lock(&object->priv->lock);
    interface =
        g_hash_table_lookup(object->priv->map_name_to_iface, interface_name);
    if (interface != NULL)
    {
        g_object_ref(interface);
        g_warn_if_fail(g_hash_table_remove(object->priv->map_name_to_iface,
                                           interface_name));
        g_mutex_unlock(&object->priv->lock);
        g_dbus_interface_set_object(interface, NULL);
        g_signal_emit_by_name(object, "interface-removed", interface);
...
static GList* g_dbus_object_skeleton_get_interfaces(GDBusObject* _object)
{
    GDBusObjectSkeleton* object = G_DBUS_OBJECT_SKELETON(_object);
    GList* ret;
...
gboolean _g_dbus_object_skeleton_has_authorize_method_handlers(
    GDBusObjectSkeleton* object)
{
    ...
    has_handlers = g_signal_has_handler_pending(
        object, signals[AUTHORIZE_METHOD_SIGNAL], 0, TRUE);
    has_default_class_handler =
        (G_DBUS_OBJECT_SKELETON_GET_CLASS(object)->authorize_method ==
