...
static int path_dispatch_io(sd_event_source* source, int fd, uint32_t revents,
                            void* userdata);
...
int path_spec_watch(PathSpec* s, sd_event_io_handler_t handler)
{
    ...
    static const int flags_table[_PATH_TYPE_MAX] = {
        ...
        [PATH_MODIFIED] = IN_DELETE_SELF | IN_MOVE_SELF | IN_ATTRIB |
                          IN_CLOSE_WRITE | IN_CREATE | IN_DELETE |
    ...
    r = sd_event_add_io(s->unit->manager->event, &s->event_source,
                        s->inotify_fd, EPOLLIN, handler, s);
    if (r < 0)
        goto fail;
    ...
    for (slash = strchr(s->path, '/');; slash = strchr(slash + 1, '/'))
    {
        ...
        if (slash)
        {
            ...
            *cut = '\0';
            ...
            flags = IN_MOVE_SELF | IN_DELETE_SELF | IN_ATTRIB | IN_CREATE |
                    IN_MOVED_TO;
        }
        else
        ...
        if (r < 0)
        {
            ...
            r = log_warning_errno(
                errno, "Failed to add watch on %s: %s", s->path,
        ...
        else
        {
            ...
            if (oldslash)
            {
                ...
                *cut2 = '\0';
                ...
                /* Error is ignored, the worst can happen is we get spurious
                 * events. */
    ...
    {
        r = log_error_errno(
            errno, "Failed to add watch on any of the components of %s: %m",
            s->path);
...
static int path_add_default_dependencies(Path* p)
{
    ...
    r = unit_add_dependency_by_name(UNIT(p), UNIT_BEFORE, SPECIAL_PATHS_TARGET,
    ...
    if (MANAGER_IS_SYSTEM(UNIT(p)->manager))
    {
        r = unit_add_two_dependencies_by_name(
            UNIT(p), UNIT_AFTER, UNIT_REQUIRES, SPECIAL_SYSINIT_TARGET, true,
    ...
    return unit_add_two_dependencies_by_name(
        UNIT(p), UNIT_BEFORE, UNIT_CONFLICTS, SPECIAL_SHUTDOWN_TARGET, true,
        UNIT_DEPENDENCY_DEFAULT);
}
...
static int path_add_trigger_dependencies(Path* p)
{
    ...
    return unit_add_two_dependencies(UNIT(p), UNIT_BEFORE, UNIT_TRIGGERS, x,
...
static void path_dump(Unit* u, FILE* f, const char* prefix)
{
    ...
    fprintf(f,
            ...
            "%sMakeDirectory: %s\n"
            "%sDirectoryMode: %04o\n",
            prefix, path_state_to_string(p->state), prefix,
            path_result_to_string(p->result), prefix,
            trigger ? trigger->id : "n/a", prefix, yes_no(p->make_directory),
...
static void path_enter_dead(Path* p, PathResult f)
{
    ...
    unit_log_result(UNIT(p), p->result == PATH_SUCCESS,
...
static void path_enter_running(Path* p)
{
    ...
    r = manager_add_job(UNIT(p)->manager, JOB_START, trigger, JOB_REPLACE, NULL,
                        &error, NULL);
    if (r < 0)
        goto fail;
...
fail:
    log_unit_warning(UNIT(p), "Failed to queue unit startup job: %s",
...
static int path_deserialize_item(Unit* u, const char* key, const char* value,
                                 FDSet* fds)
{
    Path* p = PATH(u);
