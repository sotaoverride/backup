...
#define NH_DIFF(ATTR, EXPR) ATTR_DIFF(attrs, NH_ATTR_##ATTR, a, b, EXPR)
    ...
    diff |= NH_DIFF(GATEWAY, nl_addr_cmp(a->rtnh_gateway, b->rtnh_gateway));
    diff |= NH_DIFF(NEWDST, nl_addr_cmp(a->rtnh_newdst, b->rtnh_newdst));
    diff |= NH_DIFF(VIA, nl_addr_cmp(a->rtnh_via, b->rtnh_via));
    diff |= NH_DIFF(ENCAP, nh_encap_compare(a->rtnh_encap, b->rtnh_encap));

    if (loose)
        diff |=
            NH_DIFF(FLAGS, (a->rtnh_flags ^ b->rtnh_flags) & b->rtnh_flag_mask);
    else
...
static void nh_dump_line(struct rtnl_nexthop* nh, struct nl_dump_params* dp)
{
    ...
    if (nh->ce_mask & NH_ATTR_GATEWAY)
        nl_dump(dp, " %s", nl_addr2str(nh->rtnh_gateway, buf, sizeof(buf)));
    ...
    if (nh->ce_mask & NH_ATTR_IFINDEX)
    {
        ...
        {
            nl_dump(dp, " dev %s",
                    rtnl_link_i2name(link_cache, nh->rtnh_ifindex, buf,
                                     sizeof(buf)));
        }
        else
...
static void nh_dump_details(struct rtnl_nexthop* nh, struct nl_dump_params* dp)
{
    ...
    if (nh->ce_mask & NH_ATTR_VIA)
        nl_dump(dp, " via %s", nl_addr2str(nh->rtnh_via, buf, sizeof(buf)));

    if (nh->ce_mask & NH_ATTR_GATEWAY)
        nl_dump(dp, " via %s", nl_addr2str(nh->rtnh_gateway, buf, sizeof(buf)));
    ...
    if (nh->ce_mask & NH_ATTR_REALMS)
        nl_dump(dp, " realm %04x:%04x", RTNL_REALM_FROM(nh->rtnh_realms),
                RTNL_REALM_TO(nh->rtnh_realms));

    if (nh->ce_mask & NH_ATTR_FLAGS)
        nl_dump(dp, " <%s>",
                rtnl_route_nh_flags2str(nh->rtnh_flags, buf, sizeof(buf)));
...
int rtnl_route_nh_set_newdst(struct rtnl_nexthop* nh, struct nl_addr* addr)
{
    ...
    if (!nl_addr_valid(nl_addr_get_binary_addr(addr), nl_addr_get_len(addr)))
        return -NLE_INVAL;
