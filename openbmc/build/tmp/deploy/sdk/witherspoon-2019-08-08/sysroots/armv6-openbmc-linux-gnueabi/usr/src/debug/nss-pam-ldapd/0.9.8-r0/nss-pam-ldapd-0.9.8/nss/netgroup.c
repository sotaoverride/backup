                                                     ...
                                                     void* args)
{
    ...
    while (1)
    {
        ...
        retv = read_netgrent_line(NETGROUP_BE(be)->fp, &result,
        ...
        else if ((retv == NSS_STATUS_SUCCESS) && (result.type == triple_val))
        {
            ...
            GETNETGRENT_ARGS(args)->retp[NSS_NETGR_MACHINE] =
                (char*)result.val.triple.host;
            GETNETGRENT_ARGS(args)->retp[NSS_NETGR_USER] =
                (char*)result.val.triple.user;
            GETNETGRENT_ARGS(args)->retp[NSS_NETGR_DOMAIN] =
                (char*)result.val.triple.domain;
            return NSS_STATUS_SUCCESS;
        }
        ...
        {
            /* some error occurred when reading the line (stream should be
             * closed by now) */
            NETGROUP_BE(be)->fp = NULL;
            GETNETGRENT_ARGS(args)->status = NSS_NETGR_NO;
            return retv;
                                                     ...
                                                     void* args)
{
    ...
    ngbe =
        (struct setnetgrent_backend*)malloc(sizeof(struct setnetgrent_backend));
    if (ngbe == NULL)
        return NSS_STATUS_UNAVAIL;
...
static nss_status_t netgroup_innetgr(nss_backend_t UNUSED(*be), void* args)
{
    unsigned int i;
    ...
    for (i = 0; i < INNETGR_ARGS(args)->groups.argc; i++)
    {
        ...
        {
            res =
                netgroup_setnetgrent_getnetgrent(set_args.iterator, &get_args);
            /* see if we have an error or are at the end of the results */
            if ((res != NSS_SUCCESS) || (get_args.status != NSS_NETGR_FOUND))
                break;
            /* see if we have a match */
            if (((host == NULL) ||
                 (strcmp(host, get_args.retp[NSS_NETGR_MACHINE]) == 0)) &&
                ((user == NULL) ||
                 (strcmp(user, get_args.retp[NSS_NETGR_USER]) == 0)) &&
                ((domain == NULL) ||
                 (strcmp(domain, get_args.retp[NSS_NETGR_DOMAIN]) == 0)))
            {
...
static nss_backend_op_t netgroup_ops[] = {nss_ldap_destructor,
                                          NULL,
                                          NULL,
                                          NULL,
                                          netgroup_innetgr,
