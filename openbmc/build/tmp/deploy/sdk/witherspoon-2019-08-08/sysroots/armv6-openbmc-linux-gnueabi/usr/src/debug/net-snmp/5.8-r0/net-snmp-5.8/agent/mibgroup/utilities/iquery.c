        ...
        netsnmp_feature_child_of(iquery_community_session, iquery_all)
            netsnmp_feature_child_of(iquery_pdu_session, iquery_all)
                ...
                netsnmp_feature_require(query_set_default_session)
                    ...
                    void netsnmp_parse_iquerySecLevel(const char* token,
                                                      char* line)
{
    int secLevel;
...
void netsnmp_parse_iqueryVersion(const char* token, char* line)
{
 ...
 */
int _init_default_iquery_session(int majorID, int minorID, void* serverargs,
                                 void* clientarg)
{
    char* secName = netsnmp_ds_get_string(NETSNMP_DS_APPLICATION_ID,
                                          NETSNMP_DS_AGENT_INTERNAL_SECNAME);
    if (secName)
        netsnmp_query_set_default_session(netsnmp_iquery_user_session(secName));
 ...
 */
int _tweak_default_iquery_session(int majorID, int minorID, void* serverargs,
                                  void* clientarg)
{
...
void init_iquery(void)
{
    ...
    snmpd_register_config_handler("iqueryVersion", netsnmp_parse_iqueryVersion,
                                  NULL, "1 | 2c | 3");
...
netsnmp_session* netsnmp_iquery_pdu_session(netsnmp_pdu* pdu)
{
    ...
    if (pdu->version == SNMP_VERSION_3)
        return netsnmp_iquery_session(pdu->securityName, pdu->version,
                                      pdu->securityModel, pdu->securityLevel,
                                      pdu->securityEngineID,
                                      pdu->securityEngineIDLen);
    else
        return netsnmp_iquery_session(
            (char*)pdu->community,

            pdu->version, pdu->version + 1, SNMP_SEC_LEVEL_NOAUTH,
...
netsnmp_session* netsnmp_iquery_user_session(char* secName)
{
    ...
    return netsnmp_iquery_session(secName, SNMP_VERSION_3, SNMP_SEC_MODEL_USM,
