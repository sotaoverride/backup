...
int parse_cpu_set_internal(const char* rvalue, cpu_set_t** cpu_set, bool warn,
    ...
    for (;;)
    {
        ...
        if (r < 0)
            return warn ? log_syntax(unit, LOG_ERR, filename, line, r,
                                     "Invalid value for %s: %s", lvalue, rvalue)
                        : r;
        if (r == 0)
            break;
        ...
        if (cpu_lower >= ncpus || cpu_upper >= ncpus)
            return warn ? log_syntax(unit, LOG_ERR, filename, line, EINVAL,
        ...
        if (cpu_lower > cpu_upper)
        {
            if (warn)
                log_syntax(unit, LOG_WARNING, filename, line, 0,
                           "Range '%s' is invalid, %u > %u, ignoring", word,
                           cpu_lower, cpu_upper);
            continue;
        }
