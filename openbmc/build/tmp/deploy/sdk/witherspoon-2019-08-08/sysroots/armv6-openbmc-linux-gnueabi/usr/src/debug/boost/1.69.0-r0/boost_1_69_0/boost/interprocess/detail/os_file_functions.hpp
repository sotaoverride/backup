...
inline bool get_temporary_path(char* buffer, std::size_t buf_len,
                               std::size_t& required_len)
{
    required_len = 0;
    // std::size_t is always bigger or equal than unsigned long in Windows
    // systems In case std::size_t is bigger than unsigned long
...
inline file_handle_t
    create_or_open_file(const char* name, mode_t mode,
                        const permissions& perm = permissions(),
                        bool temporary = false)
{
    unsigned long attr = temporary ? winapi::file_attribute_temporary : 0;
...
inline bool try_acquire_file_lock(file_handle_t hnd, bool& acquired)
{
    ...
    winapi::interprocess_overlapped overlapped;
    std::memset(&overlapped, 0, sizeof(overlapped));
    if (!winapi::lock_file_ex(hnd,
                              winapi::lockfile_exclusive_lock |
                                  winapi::lockfile_fail_immediately,
...
inline bool
    delete_subdirectories_recursive(const std::string& refcstrRootDirectory,
