...
void nl_cli_route_parse_src(struct rtnl_route* route, char* arg)
{
    ...
    if ((err = rtnl_route_set_src(route, addr)) < 0)
        nl_cli_fatal(err, "Unable to set source address: %s", nl_geterror(err));
...
void nl_cli_route_parse_metric(struct rtnl_route* route, char* subopts)
{
    ...
    static char* const tokens[] = {
        "unspec",   "lock",     "mtu",      "window", "rtt",
        "rttvar",   "sstresh",  "cwnd",     "advmss", "reordering",
        "hoplimit", "initcwnd", "features", NULL,
    };
    unsigned long lval;
    char *arg, *endptr;
    ...
    while (*subopts != '\0')
    {
        ...
        if (endptr == arg)
            nl_cli_fatal(EINVAL, "Metric \"%s\", value not numeric",
                         tokens[ret]);

        if ((ret = rtnl_route_set_metric(route, ret, lval)) < 0)
            nl_cli_fatal(ret, "Unable to set metric: %s", nl_geterror(ret));
    }
}
    ...
    while (*subopts != '\0')
    {
        ...
        switch (ret)
        {
            ...
            case NH_WEIGHT:
                lval = strtoul(arg, &endptr, 0);
                if (endptr == arg)
                    nl_cli_fatal(EINVAL, "Invalid weight \"%s\", not numeric",
                                 arg);
...
void nl_cli_route_parse_protocol(struct rtnl_route* route, char* arg)
{
    ...
    if (endptr == arg)
    {
        if ((proto = rtnl_route_str2proto(arg)) < 0)
            nl_cli_fatal(EINVAL, "Unknown routing protocol name \"%s\"", arg);
    }
    else
    {
        proto = lval;
...
void nl_cli_route_parse_type(struct rtnl_route* route, char* arg)
{
    ...
    if ((ival = rtnl_route_set_type(route, ival)) < 0)
        nl_cli_fatal(ival, "Unable to set routing type: %s", nl_geterror(ival));
}

void nl_cli_route_parse_iif(struct rtnl_route* route, char* arg,
                            struct nl_cache* link_cache)
{
    int ival;
