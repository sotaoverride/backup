...
static int set_usb_mass_storage_ifsubtype(char* to, const char* from,
                                          size_t len)
{
    int type_num = 0;
    char* eptr;
...
static int dev_if_packed_info(sd_device* dev, char* ifs_str, size_t len)
{
    ...
    if (fd < 0)
        return log_device_debug_errno(dev, errno, "Failed to open \"%s\": %m",
                                      filename);
           ...
           strpos + 7 < len - 2)
    {
        ...
        if (snprintf(if_str, 8, ":%02x%02x%02x", desc->bInterfaceClass,
...
static int builtin_usb_id(sd_device* dev, int argc, char* argv[], bool test)
{
    ...
    if (r < 0)
        return log_device_debug_errno(dev_interface, r,
                                      "Failed to get syspath: %m");
    (void)sd_device_get_sysattr_value(dev_interface, "bInterfaceNumber",
                                      &ifnum);
    (void)sd_device_get_sysattr_value(dev_interface, "driver", &driver);

    r = sd_device_get_sysattr_value(dev_interface, "bInterfaceClass",
                                    &if_class);
    if (r < 0)
    ...
    {
        /* mass storage */
        if (sd_device_get_sysattr_value(dev_interface, "bInterfaceSubClass",
                                        &if_subclass) >= 0)
            protocol = set_usb_mass_storage_ifsubtype(type_str, if_subclass,
                                                      sizeof(type_str) - 1);
    }
    else
        set_usb_iftype(type_str, if_class_num, sizeof(type_str) - 1);

    log_device_debug(dev_interface, "if_class:%d protocol:%d", if_class_num,
                     protocol);

    /* usb device directory */
    r = sd_device_get_parent_with_subsystem_devtype(dev_interface, "usb",
                                                    "usb_device", &dev_usb);
    if (r < 0)
        return log_device_debug_errno(dev_interface, r,
    ...
    {
        sd_device* dev_scsi;
        const char *scsi_sysname, *scsi_model, *scsi_vendor, *scsi_type,
            *scsi_rev;
        int host, bus, target, lun;

        /* get scsi device */
        r = sd_device_get_parent_with_subsystem_devtype(
            dev, "scsi", "scsi_device", &dev_scsi);
        if (r < 0)
        {
        ...
        if (sd_device_get_sysname(dev_scsi, &scsi_sysname) < 0)
            goto fallback;
        if (sscanf(scsi_sysname, "%d:%d:%d:%d", &host, &bus, &target, &lun) !=
            4)
        {
        ...
        util_replace_whitespace(scsi_rev, revision_str,
...
fallback:
    ...
    if (r < 0)
        return log_device_debug_errno(dev_usb, r,
    ...
    if (vendor_str[0] == '\0')
    {
        ...
        if (sd_device_get_sysattr_value(dev_usb, "manufacturer", &usb_vendor) <
            0)
    ...
    if (revision_str[0] == '\0')
    {
        ...
        {
            util_replace_whitespace(usb_rev, revision_str,
                                    sizeof(revision_str) - 1);
            util_replace_chars(revision_str, NULL);
        }
    }
    ...
    if (serial_str[0] == '\0')
    {
        ...
        if (sd_device_get_sysattr_value(dev_usb, "serial", &usb_serial) >= 0)
        {
            ...
            if (usb_serial)
            {
                util_replace_whitespace(usb_serial, serial_str,
                                        sizeof(serial_str) - 1);
    ...
    if (!isempty(packed_if_str))
        udev_builtin_add_property(dev, test, "ID_USB_INTERFACES",
                                  packed_if_str);
    if (ifnum)
