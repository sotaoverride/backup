...
static dbus_bool_t service_dirs_find_dir(DBusList** service_dirs,
                                         const char* dir)
{
    DBusList* link;
...
void bus_config_parser_unref(BusConfigParser* parser)
{
    ...
    _dbus_list_foreach(&parser->service_dirs, (DBusForeachFunction)dbus_free,
                       NULL);
                                      ...
                                      DBusError* error)
{
    ...
    switch (parser->type)
    {
        ...
        case ELEMENT_SERVICEHELPER:
        {
            if (!_dbus_string_copy(&content_sane, 0, &parser->service_helper,
                                   0))
            {
                BUS_SET_OOM(error);
                goto out_content;
...
static dbus_bool_t make_full_path(const DBusString* test_data_dir,
                                  const char* subdir, const char* file,
                                  DBusString* full_path)
{
    DBusString filename;
    dbus_bool_t retval;
...
dbus_bool_t bus_config_parser_trivial_test(const DBusString* test_data_dir)
{
    ...
    /* We already test default_session_servicedirs and
     * default_system_servicedirs in bus_config_parser_test() */
