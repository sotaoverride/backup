                                 ...
                                 gid_t** groupsp, long int limit)
{
    ...
    if (!__nss_not_use_nscd_group && !__nss_database_custom[NSS_DBSIDX_group])
    {
    ...
    while (!no_more)
    {
        ...
        if (fct == NULL)
            status = compat_call(nip, user, group, &start, size, groupsp, limit,
                                 &errno);
        else
            status = DL_CALL_FCT(
                fct, (user, group, &start, size, groupsp, limit, &errno));
