...
static void file_data_free(FileData* data)
{
...
#define NO_ATTRS() COLLECT(G_MARKUP_COLLECT_INVALID, NULL)
    ...
    else if (strcmp(container, "gresources") == 0)
    {
        ...
        {
            COLLECT(OPTIONAL | STRDUP, "prefix", &state->prefix);
            return;
        }
    }
    else if (strcmp(container, "gresource") == 0)
    {
        ...
        {
            COLLECT(OPTIONAL | STRDUP, "alias", &state->alias, OPTIONAL | BOOL,
                    "compressed", &state->compressed, OPTIONAL | STRDUP,
                    "preprocess", &state->preproc_options);
...
static GvdbItem* get_parent(GHashTable* table, gchar* key, gint length)
{
...
static gchar* find_file(const gchar* filename)
{
    guint i;
    gchar* real_file;
    gboolean exists;
                        ...
                        gpointer user_data, GError** error)
{
    ...
    else if (strcmp(element_name, "file") == 0)
    {
        ...
        if (sourcedirs != NULL)
        {
            ...
            if (real_file == NULL && state->collect_data)
            {
                g_set_error(error, G_IO_ERROR, G_IO_ERROR_FAILED,
                            _("Failed to locate “%s” in any source directory"),
                            file);
                return;
            }
        }
        else
        {
            ...
            {
                g_set_error(error, G_IO_ERROR, G_IO_ERROR_FAILED,
                            _("Failed to locate “%s” in current directory"),
                            file);
                return;
            }
        }
        ...
        if (state->preproc_options)
        {
            ...
            if (xml_stripblanks)
            {
                ...
                if (xmllint == NULL)
                {
                    ...
                    if (!xmllint_warned)
                    {
                        /* Translators: the first %s is a gresource XML
                         * attribute, the second %s is an environment variable,
                         * and the third %s is a command line tool
                         */
                        char* warn = g_strdup_printf(
                            _("%s preprocessing requested, but %s is not set, "
                              "and %s is not in PATH"),
                            "xml-stripblanks", "XMLLINT", "xmllint");
                        g_printerr("%s\n", warn);
                        g_free(warn);
                ...
                if (jsonformat == NULL)
                {
                    ...
                    if (!jsonformat_warned)
                    {
                        ...
                        char* warn = g_strdup_printf(
                            _("%s preprocessing requested, but %s is not set, "
                              "and %s is not in PATH"),
                            "json-stripblanks", "JSON_GLIB_FORMAT",
                            "json-glib-format");
                        g_printerr("%s\n", warn);
                        g_free(warn);
            ...
            if (to_pixdata)
            {
                ...
                if (gdk_pixbuf_pixdata == NULL)
                {
                     ...
                     */
                    g_set_error(error, G_IO_ERROR, G_IO_ERROR_FAILED,
                                _("%s preprocessing requested, but %s is not "
                                  "set, and %s is not in PATH"),
                                "to-pixdata", "GDK_PIXBUF_PIXDATA",
                                "gdk-pixbuf-pixdata");
                    goto cleanup;
                }
                ...
                proc = g_subprocess_new(G_SUBPROCESS_FLAGS_STDOUT_SILENCE,
                                        error, gdk_pixbuf_pixdata, real_file,
                                        tmp_file, NULL);
                g_free(real_file);
                real_file = NULL;
        ...
        if (!g_file_get_contents(real_file, &data->content, &data->size,
                                 &my_error))
        {
            g_set_error(error, G_MARKUP_ERROR, G_MARKUP_ERROR_INVALID_CONTENT,
                        _("Error reading file %s: %s"), real_file,
                        my_error->message);
            g_clear_error(&my_error);
            goto cleanup;
        }
        /* Include zero termination in content_size for uncompressed files (but
         * not in size) */
        ...
        if (state->compressed)
        {
            ...
            GOutputStream* out2 =
                g_converter_output_stream_new(out, G_CONVERTER(compressor));
                ...
                !g_output_stream_close(out2, NULL, NULL))
            {
                g_set_error(error, G_MARKUP_ERROR,
                            ...
                            _("Error compressing file %s"), real_file);
                goto cleanup;
            }

            g_free(data->content);
            data->content_size = g_memory_output_stream_get_data_size(
...
static GHashTable* parse_resource_file(const gchar* filename,
                                       gboolean collect_data, GHashTable* files)
{
    ...
    context = g_markup_parse_context_new(
        &parser, G_MARKUP_TREAT_CDATA_AS_TEXT | G_MARKUP_PREFIX_ERROR_POSITION,
        &state, NULL);
    ...
    else
    {
        ...
        while (g_hash_table_iter_next(&iter, (gpointer*)&key, (gpointer*)&data))
        {
            ...
            gvdb_item_set_parent(item, get_parent(table, mykey, key_len));
            ...
            v_data = g_variant_new_from_data(G_VARIANT_TYPE("ay"),
                                             data->content, data->content_size,
                                             TRUE, g_free, data->content);
...
static gboolean write_to_file(GHashTable* table, const gchar* filename,
                              GError** error)
{
    gboolean success;
...
static gboolean extension_in_set(const char* str, ...)
{
    va_list list;
 ...
 */
static char* escape_makefile_string(const char* string)
{
    GString* str;
    const char *p, *q;
...
int main(int argc, char** argv)
{
    ...
    GOptionEntry entries[] = {
        ...
        {"target", 0, 0, G_OPTION_ARG_FILENAME, &target,
         N_("Name of the output file"), N_("FILE")},
        {"sourcedir", 0, 0, G_OPTION_ARG_FILENAME_ARRAY, &sourcedirs,
         N_("The directories to load files referenced in FILE from (default: "
            "current directory)"),
         N_("DIRECTORY")},
        {"generate", 0, 0, G_OPTION_ARG_NONE, &generate_automatic,
         N_("Generate output in the format selected for by the target filename "
            "extension"),
         NULL},
        {"generate-header", 0, 0, G_OPTION_ARG_NONE, &generate_header,
         N_("Generate source header"), NULL},
        {"generate-source", 0, 0, G_OPTION_ARG_NONE, &generate_source,
         N_("Generate source code used to link in the resource file into your "
            "code"),
         NULL},
        {"generate-dependencies", 0, 0, G_OPTION_ARG_NONE,
         &generate_dependencies, N_("Generate dependency list"), NULL},
        {"dependency-file", 0, 0, G_OPTION_ARG_FILENAME, &dependency_file,
         N_("Name of the dependency file to generate"), N_("FILE")},
        {"generate-phony-targets", 0, 0, G_OPTION_ARG_NONE,
         &generate_phony_targets,
         N_("Include phony targets in the generated dependency file"), NULL},
        {"manual-register", 0, 0, G_OPTION_ARG_NONE, &manual_register,
         N_("Don’t automatically create and register resource"), NULL},
        {"internal", 0, 0, G_OPTION_ARG_NONE, &internal,
         N_("Don’t export functions; declare them G_GNUC_INTERNAL"), NULL},
        {"external-data", 0, 0, G_OPTION_ARG_NONE, &external_data,
         N_("Don’t embed resource data in the C file; assume it's linked "
            "externally instead"),
         NULL},
        {"c-name", 0, 0, G_OPTION_ARG_STRING, &c_name,
    ...
    files = g_hash_table_new_full(g_str_hash, g_str_equal, g_free,
                                  (GDestroyNotify)file_data_free);

    if ((table = parse_resource_file(srcfile, !generate_dependencies, files)) ==
        NULL)
    {
        g_free(target);
        g_free(c_name);
    ...
    if (dependency_file != NULL)
    {
        ...
        {
            if (!g_file_set_contents(dependency_file, dep_string->str,
                                     dep_string->len, &error))
            {
    ...
    if (binary_target != NULL && !write_to_file(table, binary_target, &error))
    {
        g_printerr("%s\n", error->message);
        g_free(target);
        g_free(c_name);
    ...
    else if (generate_source)
    {
        ...
        if (!g_file_get_contents(binary_target, (char**)&data, &data_size,
                                 NULL))
        {
        ...
        g_fprintf(file,
                  "#include <gio/gio.h>\n"
                  "\n"
                  "#if defined (__ELF__) && ( __GNUC__ > 2 || (__GNUC__ == 2 "
                  "&& __GNUC_MINOR__ >= 6))\n"
                  "# define SECTION __attribute__ ((section "
                  "(\".gresource.%s\"), aligned (8)))\n"
                  "#else\n"
                  "# define SECTION\n"
                  "#endif\n"
                  "\n",
        ...
        {
            g_fprintf(file,
                      "extern const SECTION union { const guint8 "
                      "data[%" G_GSIZE_FORMAT "]; const double alignment; void "
        ...
        {
            /* For Visual Studio builds: Avoid surpassing the 65535-character
             * limit for a string, GitLab issue #1580 */
            g_fprintf(file, "#ifdef _MSC_VER\n");
            g_fprintf(file,
                      "static const SECTION union { const guint8 "
                      "data[%" G_GSIZE_FORMAT
                      "]; const double alignment; void * const ptr;}  "
                      "%s_resource_data = { {\n",
        ...
        g_fprintf(
            file,
            "\n"
            "static GStaticResource static_resource = { %s_resource_data.data, "
            "sizeof (%s_resource_data.data)%s, NULL, NULL, NULL };\n"
            "%s GResource *%s_get_resource (void);\n"
        ...
        else
        {
            ...
            g_fprintf(
                ...
                "#pragma "
                "G_DEFINE_CONSTRUCTOR_PRAGMA_ARGS(resource_constructor)\n"
                "#endif\n"
                ...
                "\n"
                "#else\n"
                "#warning \"Constructor not supported on this compiler, "
                "linking in resources will not work\"\n"
                "#endif\n"
                "\n"
