...
static int generate_output(struct getopt_control* ctl, char* argv[], int argc)
{
    ...
    while ((opt = (getopt_long_fp(argc, argv, ctl->optstr,
                                  (const struct option*)ctl->long_options,
                                  &longindex))) != EOF)
    {
        if (opt == '?' || opt == ':')
...
static void add_longopt(struct getopt_control* ctl, const char* name,
                        int has_arg)
{
    static int flag;
    ...
    if (ctl->long_options_nr == ctl->long_options_length)
    {
        ...
        ctl->long_options =
            xrealloc(ctl->long_options,
                     sizeof(struct option) * ctl->long_options_length);
    }
    if (name)
    {
        /* Not for init! */
...
static void __attribute__((__noreturn__)) usage(void)
{
    ...
    printf(_(" %1$s <optstring> <parameters>\n"
             ...
             " %1$s [options] -o|--options <optstring> [options] [--] "
             "<parameters>\n"),
    ...
    fputs(_(" -a, --alternative             allow long options starting with "
            "single -\n"),
          stdout);
    fputs(
        _(" -l, --longoptions <longopts>  the long options to be recognized\n"),
        stdout);
    fputs(_(" -n, --name <progname>         the name under which errors are "
            "reported\n"),
          stdout);
    fputs(_(" -o, --options <optstring>     the short options to be "
            "recognized\n"),
          stdout);
    fputs(_(" -q, --quiet                   disable error reporting by "
            "getopt(3)\n"),
          stdout);
    fputs(_(" -Q, --quiet-output            no normal output\n"), stdout);
    fputs(_(" -s, --shell <shell>           set quoting conventions to those "
            "of <shell>\n"),
          stdout);
