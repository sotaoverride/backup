...
static GObject* g_object_constructor(GType type, guint n_construct_properties,
...
static void g_object_do_set_property(GObject* object, guint property_id,
                                     const GValue* value, GParamSpec* pspec);
static void g_object_do_get_property(GObject* object, guint property_id,
...
static gulong gobject_signals[LAST_SIGNAL] = {
    0,
};
static guint (*floating_flag_handler)(GObject*,
                                      gint) = object_floating_flag_handler;
                                                        ...
                                                        gboolean conditional)
{
    ...
    if (!nqueue)
    {
        ...
        g_datalist_id_set_data_full(&object->qdata, quark_notify_queue, nqueue,
                                       ...
                                       GObjectNotifyQueue* nqueue)
{
    ...
    pspecs = nqueue->n_pspecs > 16
                 ? free_me = g_new(GParamSpec*, nqueue->n_pspecs)
                 : pspecs_mem;
    ...
    if (n_pspecs)
        G_OBJECT_GET_CLASS(object)->dispatch_properties_changed(
            object, n_pspecs, pspecs);
    g_free(free_me);
}

static void g_object_notify_queue_add(GObject* object,
                                      GObjectNotifyQueue* nqueue,
                                      GParamSpec* pspec)
{
    G_LOCK(notify_lock);
...
static void debug_objects_foreach(gpointer key, gpointer value,
                                  gpointer user_data)
{
    GObject* object = value;

    g_message("[%p] stale %s\tref_count=%u", object, G_OBJECT_TYPE_NAME(object),
              object->ref_count);
}
...
void _g_object_type_init(void)
{
    ...
    type = g_type_register_fundamental(
        G_TYPE_OBJECT, g_intern_static_string("GObject"), &info, &finfo, 0);
    g_assert(type == G_TYPE_OBJECT);
    g_value_register_transform_func(G_TYPE_OBJECT, G_TYPE_OBJECT,
...
static void g_object_base_class_init(GObjectClass* class)
{
    ...
    class->construct_properties =
        pclass ? g_slist_copy(pclass->construct_properties) : NULL;
...
static void g_object_do_class_init(GObjectClass* class)
{
    /* read the comment about typedef struct CArray; on why not to change this
     * quark */
    ...
    quark_in_construction =
        g_quark_from_static_string("GObject-in-construction");
     ...
     * the property has actually changed. When it is emitted is determined by
     * the derived GObject class. If the implementor did not create the property
     * with %G_PARAM_EXPLICIT_NOTIFY, then any call to g_object_set_property()
     * results in ::notify being emitted, even if the new value is the same as
     * the old. If they did pass %G_PARAM_EXPLICIT_NOTIFY, then this signal is
     * emitted only when they explicitly call g_object_notify() or
     * g_object_notify_by_pspec(), and common practice is to do that only when
     ...
     */
    gobject_signals[NOTIFY] = g_signal_new(
        g_intern_static_string("notify"), G_TYPE_FROM_CLASS(class),
        G_SIGNAL_RUN_FIRST | G_SIGNAL_NO_RECURSE | G_SIGNAL_DETAILED |
            G_SIGNAL_NO_HOOKS | G_SIGNAL_ACTION,
        G_STRUCT_OFFSET(GObjectClass, notify), NULL, NULL,
...
static inline gboolean install_property_internal(GType g_type,
                                                 guint property_id,
                                                 GParamSpec* pspec)
{
    if (g_param_spec_pool_lookup(pspec_pool, pspec->name, g_type, FALSE))
    {
        g_warning("When installing property: type '%s' already has a property "
                  "named '%s'",
                  g_type_name(g_type), pspec->name);
        return FALSE;
    }
...
static gboolean validate_pspec_to_install(GParamSpec* pspec)
{
    g_return_val_if_fail(G_IS_PARAM_SPEC(pspec), FALSE);
    g_return_val_if_fail(PARAM_SPEC_PARAM_ID(pspec) == 0, FALSE); /* paranoid */

    g_return_val_if_fail(pspec->flags & (G_PARAM_READABLE | G_PARAM_WRITABLE),
                         FALSE);

    if (pspec->flags & G_PARAM_CONSTRUCT)
        g_return_val_if_fail((pspec->flags & G_PARAM_CONSTRUCT_ONLY) == 0,
                             FALSE);
...
static gboolean validate_and_install_class_property(GObjectClass* class,
                                                    GType oclass_type,
                                                    GType parent_type,
                                                    guint property_id,
                                                    GParamSpec* pspec)
{
    ...
    if (install_property_internal(oclass_type, property_id, pspec))
    {
        if (pspec->flags & (G_PARAM_CONSTRUCT | G_PARAM_CONSTRUCT_ONLY))
            class->construct_properties =
                g_slist_append(class->construct_properties, pspec);

        /* for property overrides of construct properties, we have to get rid
         * of the overidden inherited construct property
         */
        pspec = g_param_spec_pool_lookup(pspec_pool, pspec->name, parent_type,
                                         TRUE);
        if (pspec &&
            pspec->flags & (G_PARAM_CONSTRUCT | G_PARAM_CONSTRUCT_ONLY))
            class->construct_properties =
                g_slist_remove(class->construct_properties, pspec);
 ...
 */
void g_object_class_install_property(GObjectClass* class, guint property_id,
                                     GParamSpec* pspec)
{
    ...
    (void)validate_and_install_class_property(class, oclass_type, parent_type,
                                              property_id, pspec);
}
 ...
 * Since: 2.26
 */
void g_object_class_install_properties(GObjectClass* oclass, guint n_pspecs,
                                       GParamSpec** pspecs)
{
    GType oclass_type, parent_type;
    gint i;
    ...
    for (i = 1; i < n_pspecs; i++)
    {
        ...
        if (!validate_and_install_class_property(oclass, oclass_type,
                                                 parent_type, i, pspec))
        {
            break;
        }
 ...
 */
GParamSpec* g_object_class_find_property(GObjectClass* class,
                                         const gchar* property_name)
{
    ...
    pspec = g_param_spec_pool_lookup(pspec_pool, property_name,
                                     G_OBJECT_CLASS_TYPE(class), TRUE);
    if (pspec)
    {
                                             ...
                                             const gchar* property_name)
{
    ...
    return g_param_spec_pool_lookup(pspec_pool, property_name,
 ...
 * Since: 2.4
 */
void g_object_class_override_property(GObjectClass* oclass, guint property_id,
                                      const gchar* name)
{
    ...
    if (parent_type != G_TYPE_NONE)
        overridden =
            g_param_spec_pool_lookup(pspec_pool, name, parent_type, TRUE);
    if (!overridden)
    {
        GType* ifaces;
        guint n_ifaces;
        ...
        {
            overridden = g_param_spec_pool_lookup(pspec_pool, name,
                                                  ifaces[n_ifaces], FALSE);
        }
    ...
    if (!overridden)
    {
        g_warning("%s: Can't find property to override for '%s::%s'", G_STRFUNC,
                  G_OBJECT_CLASS_NAME(oclass), name);
        return;
    }
...
GParamSpec** /* free result */
    g_object_class_list_properties(GObjectClass* class, guint* n_properties_p)
{
    GParamSpec** pspecs;
    guint n;
...
static inline gboolean object_in_construction(GObject* object)
{
    return g_datalist_id_get_data(&object->qdata, quark_in_construction) !=
           NULL;
}
...
static void g_object_init(GObject* object, GObjectClass* class)
{
    ...
    {
        /* freeze object's notification queue, g_object_newv() preserves
         * pairedness */
    ...
    {
        /* mark object in-construction for notify_queue_thaw() and to allow
                                                 ...
                                                 GParamSpec** pspecs)
{
    ...
    for (i = 0; i < n_pspecs; i++)
        g_signal_emit(object, gobject_signals[NOTIFY],
...
static GParamSpec* get_notify_pspec(GParamSpec* pspec)
{
...
static inline void g_object_notify_by_spec_internal(GObject* object,
                                                    GParamSpec* pspec)
{
    ...
    consider_issuing_property_deprecation_warning(const GParamSpec* pspec)
{
    ...
    already =
        g_hash_table_contains(already_warned_table, (gpointer)pspec->name);
    if (!already)
...
static inline void object_get_property(GObject* object, GParamSpec* pspec,
                                       GValue* value)
{
    ...
    if (class == NULL)
    {
        g_warning("'%s::%s' is not a valid property name; '%s' is not a "
                  "GObject subtype",
                  g_type_name(pspec->owner_type), pspec->name,
...
static inline void object_set_property(GObject* object, GParamSpec* pspec,
                                       const GValue* value,
                                       GObjectNotifyQueue* nqueue)
{
             ...
             !(pspec->flags & G_PARAM_LAX_VALIDATION))
    {
        ...
        g_warning("value \"%s\" of type '%s' is invalid or out of range for "
                                              ...
                                              gpointer g_iface)
{
    ...
    while (n--)
    {
        GParamSpec* class_pspec = g_param_spec_pool_lookup(
            pspec_pool, pspecs[n]->name, G_OBJECT_CLASS_TYPE(class), TRUE);

        if (!class_pspec)
        {
            g_critical("Object class %s doesn't implement property "
                       "'%s' from interface '%s'",
                       g_type_name(G_OBJECT_CLASS_TYPE(class)), pspecs[n]->name,
...
#define SUBSET(a, b, mask) (((a) & ~(b) & (mask)) == 0)
         ...
         */
        if (!SUBSET(pspecs[n]->flags, class_pspec->flags,
                    G_PARAM_READABLE | G_PARAM_WRITABLE))
        {
            g_critical("Flags for property '%s' on class '%s' remove "
                       "functionality compared with the "
                       "property on interface '%s'\n",
                       pspecs[n]->name, g_type_name(G_OBJECT_CLASS_TYPE(class)),
                       g_type_name(iface_type));
            continue;
        }
        ...
        {
            if (!SUBSET(class_pspec->flags, pspecs[n]->flags,
                        G_PARAM_CONSTRUCT_ONLY))
            {
                g_critical("Flags for property '%s' on class '%s' introduce "
                           "additional restrictions on "
                           "writability compared with the property on "
                           "interface '%s'\n",
                           pspecs[n]->name,
...
#undef SUBSET
        ...
        switch (pspecs[n]->flags & (G_PARAM_READABLE | G_PARAM_WRITABLE))
        {
            case G_PARAM_READABLE | G_PARAM_WRITABLE:
                /* class pspec value type must have exact equality with
                 * interface */
                if (pspecs[n]->value_type != class_pspec->value_type)
                    g_critical(
                        "Read/writable property '%s' on class '%s' has type "
                        "'%s' which is not exactly equal to the "
            ...
            case G_PARAM_READABLE:
                /* class pspec value type equal or more restrictive than
                 * interface */
                if (!g_type_is_a(class_pspec->value_type,
                                 pspecs[n]->value_type))
                    g_critical(
                        "Read-only property '%s' on class '%s' has type '%s' "
                        "which is not equal to or more "
                        "restrictive than the type '%s' of the property on the "
                        "interface '%s'\n",
                        pspecs[n]->name,
                        g_type_name(G_OBJECT_CLASS_TYPE(class)),
                        g_type_name(G_PARAM_SPEC_VALUE_TYPE(class_pspec)),
            ...
            case G_PARAM_WRITABLE:
                /* class pspec value type equal or less restrictive than
                 * interface */
                if (!g_type_is_a(pspecs[n]->value_type,
                                 class_pspec->value_type))
                    g_critical(
                        "Write-only property '%s' on class '%s' has type '%s' "
                        "which is not equal to or less "
                        "restrictive than the type '%s' of the property on the "
                        "interface '%s' \n",
                        pspecs[n]->name,
                        g_type_name(G_OBJECT_CLASS_TYPE(class)),
...
static gpointer g_object_new_with_custom_constructor(
    GObjectClass* class, GObjectConstructParam* params, guint n_params)
{
    ...
    {
        g_critical(
            "Custom constructor for class %s returned NULL (which is invalid). "
            "Please use GInitable instead.",
     ...
     * already-existing singleton (in which case we should not do
     * 'constructed').
     */
    ...
    for (i = 0; i < n_params; i++)
        if (!(params[i].pspec->flags &
              (G_PARAM_CONSTRUCT | G_PARAM_CONSTRUCT_ONLY)))
        {
            consider_issuing_property_deprecation_warning(params[i].pspec);
            object_set_property(object, params[i].pspec, params[i].value,
                                nqueue);
        }
...
static gpointer g_object_new_internal(GObjectClass* class,
                                      ...
                                      guint n_params)
{
    ...
    if
        G_UNLIKELY(CLASS_HAS_CUSTOM_CONSTRUCTOR(class))
...
static inline gboolean
    g_object_new_is_valid_property(GType object_type, GParamSpec* pspec,
                                   const char* name,
                                   GObjectConstructParam* params, int n_params)
{
    gint i;
 ...
 * Since: 2.54
 */
GObject* g_object_new_with_properties(GType object_type, guint n_properties,
                                      const char* names[],
                                      const GValue values[])
{
    ...
    if (n_properties > 0)
    {
        ...
        {
            GParamSpec* pspec;
            pspec = g_param_spec_pool_lookup(pspec_pool, names[i], object_type,
                                             TRUE);
                       ...
                       GParameter* parameters)
{
    ...
    if (n_parameters)
    {
        ...
        for (i = 0; i < n_parameters; i++)
        {
            ...
            pspec = g_param_spec_pool_lookup(pspec_pool, parameters[i].name,
                                             object_type, TRUE);
            if (!g_object_new_is_valid_property(object_type, pspec,
                                                parameters[i].name, cparams, j))
                continue;
                             ...
                             const gchar* first_property_name, va_list var_args)
{
    ...
    if (first_property_name)
    {
        ...
        do
        {
            ...
            pspec =
                g_param_spec_pool_lookup(pspec_pool, name, object_type, TRUE);

            if (!g_object_new_is_valid_property(object_type, pspec, name,
                                                params, n_params))
                break;
...
static inline gboolean g_object_set_is_valid_property(GObject* object,
                                                      GParamSpec* pspec,
                                                      const char* property_name)
{
    ...
    {
        g_warning("%s: construct property \"%s\" for object '%s' can't be set "
                  "after construction",
                  G_STRFUNC, pspec->name, G_OBJECT_TYPE_NAME(object));
        return FALSE;
    }
 ...
 * Since: 2.54
 */
void g_object_setv(GObject* object, guint n_properties, const gchar* names[],
                   const GValue values[])
{
    guint i;
    GObjectNotifyQueue* nqueue;
...
static inline gboolean g_object_get_is_valid_property(GObject* object,
                                                      GParamSpec* pspec,
                                                      const char* property_name)
{
    if (G_UNLIKELY(pspec == NULL))
 ...
 * Since: 2.54
 */
void g_object_getv(GObject* object, guint n_properties, const gchar* names[],
                   GValue values[])
{
    guint i;
    GParamSpec* pspec;
                           ...
                           GValue* value)
{
    ...
    if (g_object_get_is_valid_property(object, pspec, property_name))
    {
        ...
        else if (!g_value_type_transformable(pspec->value_type,
                                             G_VALUE_TYPE(value)))
        {
            g_warning("%s: can't retrieve property '%s' of type '%s' as value "
                      "of type '%s'",
                      G_STRFUNC, pspec->name, g_type_name(pspec->value_type),
                      G_VALUE_TYPE_NAME(value));
            g_object_unref(object);
            return;
        }
 ...
 * - object-signal, object_signal: equivalent to g_signal_connect_object (...,
 * 0)
 * - swapped-signal, swapped_signal: equivalent to g_signal_connect_data (...,
 * NULL, G_CONNECT_SWAPPED)
 * - swapped_object_signal, swapped-object-signal: equivalent to
 * g_signal_connect_object (..., G_CONNECT_SWAPPED)
 * - signal_after, signal-after: equivalent to g_signal_connect_data (..., NULL,
 * G_CONNECT_AFTER)
 * - object_signal_after, object-signal-after: equivalent to
 * g_signal_connect_object (..., G_CONNECT_AFTER)
 * - swapped_signal_after, swapped-signal-after: equivalent to
 * g_signal_connect_data (..., NULL, G_CONNECT_SWAPPED | G_CONNECT_AFTER)
 * - swapped_object_signal_after, swapped-object-signal-after: equivalent to
 * g_signal_connect_object (..., G_CONNECT_SWAPPED | G_CONNECT_AFTER)
 *
 * |[<!-- language="C" -->
...
void g_object_disconnect(gpointer _object, const gchar* signal_spec, ...)
{
    ...
    while (signal_spec)
    {
        ...
        if ((mask & G_SIGNAL_MATCH_ID) &&
            !g_signal_parse_name(signal_spec, G_OBJECT_TYPE(object), &sid,
                                 &detail, FALSE))
            g_warning("%s: invalid signal name \"%s\"", G_STRFUNC, signal_spec);
        else if (!g_signal_handlers_disconnect_matched(
                     object, mask | (detail ? G_SIGNAL_MATCH_DETAIL : 0), sid,
                     detail, NULL, (gpointer)callback, data))
            g_warning("%s: signal handler %p(%p) is not connected", G_STRFUNC,
                      callback, data);
        signal_spec = va_arg(var_args, gchar*);
    }
    va_end(var_args);
...
void g_object_weak_ref(GObject* object, GWeakNotify notify, gpointer data)
{
    ...
    g_datalist_id_set_data_full(&object->qdata, quark_weak_refs, wstack,
                                weak_refs_notify);
        ...
        for (i = 0; i < wstack->n_weak_refs; i++)
                ...
                wstack->weak_refs[i].data == data)
            {
                ...
                if (i != wstack->n_weak_refs)
                    wstack->weak_refs[i] =
                        wstack->weak_refs[wstack->n_weak_refs];
...
void g_object_add_weak_pointer(GObject* object, gpointer* weak_pointer_location)
{
    ...
    g_object_weak_ref(object, (GWeakNotify)g_nullify_pointer,
                                  ...
                                  gpointer* weak_pointer_location)
{
    ...
    g_object_weak_unref(object, (GWeakNotify)g_nullify_pointer,
                        weak_pointer_location);
}
...
static guint object_floating_flag_handler(GObject* object, gint job)
{
    switch (job)
    {
        ...
        default: /* check floating */
            return 0 != ((gsize)g_atomic_pointer_get(&object->qdata) &
...
static void toggle_refs_notify(GObject* object, gboolean is_last_ref)
{
    ...
    tstack.toggle_refs[0].notify(tstack.toggle_refs[0].data, tstack.object,
                                 is_last_ref);
}
 ...
 * Since: 2.8
 */
void g_object_add_toggle_ref(GObject* object, GToggleNotify notify,
                             gpointer data)
{
    ...
    if (tstack)
    {
        ...
        /* allocate i = tstate->n_toggle_refs - 1 positions beyond the 1
         * declared in tstate->toggle_refs */
        tstack = g_realloc(tstack, sizeof(*tstack) +
                                       sizeof(tstack->toggle_refs[0]) * i);
    }
    else
    {
 ...
 * Since: 2.8
 */
void g_object_remove_toggle_ref(GObject* object, GToggleNotify notify,
                                gpointer data)
{
    ToggleRefStack* tstack;
    gboolean found_one = FALSE;
        ...
        for (i = 0; i < tstack->n_toggle_refs; i++)
                ...
                tstack->toggle_refs[i].data == data)
            {
                ...
                if (i != tstack->n_toggle_refs)
                    tstack->toggle_refs[i] =
                        tstack->toggle_refs[tstack->n_toggle_refs];

                if (tstack->n_toggle_refs == 0)
                    g_datalist_unset_flags(&object->qdata,
    ...
    if (found_one)
        ...
        g_warning("%s: couldn't find toggle ref %p(%p)", G_STRFUNC, notify,
                  data);
}
...
retry_atomic_decrement1:
    ...
    if (old_ref > 1)
    {
        /* valid if last 2 refs are owned by this call to unref and the
         * toggle_ref */
        gboolean has_toggle_ref = OBJECT_HAS_TOGGLE_REF(object);

        if (!g_atomic_int_compare_and_exchange((int*)&object->ref_count,
                                               old_ref, old_ref - 1))
            goto retry_atomic_decrement1;

        TRACE(GOBJECT_OBJECT_UNREF(object, G_TYPE_FROM_INSTANCE(object),
                                   old_ref));

        /* if we went from 2->1 we need to notify toggle refs if any */
        if (old_ref == 2 &&
            has_toggle_ref) /* The last ref being held in this case is owned by
                               the toggle_ref */
    ...
    else
    {
         ...
         */
        weak_locations =
            g_datalist_id_get_data(&object->qdata, quark_weak_locations);
        ...
        if (weak_locations != NULL)
        {
            ...
            while (*weak_locations)
            {
                ...
                *weak_locations =
                    g_slist_delete_link(*weak_locations, *weak_locations);
            }
        ...
        G_OBJECT_GET_CLASS(object)->dispose(object);
        TRACE(GOBJECT_OBJECT_DISPOSE_END(object, G_TYPE_FROM_INSTANCE(object),
                                         1));
    ...
    retry_atomic_decrement2:
        ...
        if (old_ref > 1)
        {
            ...
            /* if we went from 2->1 we need to notify toggle refs if any */
            if (old_ref == 2 &&
                has_toggle_ref) /* The last ref being held in this case is owned
                                   by the toggle_ref */
                toggle_refs_notify(object, TRUE);
        ...
        {
            TRACE(
                GOBJECT_OBJECT_FINALIZE(object, G_TYPE_FROM_INSTANCE(object)));
            G_OBJECT_GET_CLASS(object)->finalize(object);

            TRACE(GOBJECT_OBJECT_FINALIZE_END(object,
 ...
 * Since: 2.34
 */
gpointer g_object_dup_qdata(GObject* object, GQuark quark,
 ...
 * Since: 2.34
 */
gboolean g_object_replace_qdata(GObject* object, GQuark quark, gpointer oldval,
                                ...
                                GDestroyNotify* old_destroy)
{
    ...
    return g_datalist_id_replace_data(&object->qdata, quark, oldval, newval,
                                      destroy, old_destroy);
}
 ...
 * with the same @quark.
 */
void g_object_set_qdata_full(GObject* object, GQuark quark, gpointer data,
 ...
 * Since: 2.34
 */
gpointer g_object_dup_data(GObject* object, const gchar* key,
                           GDuplicateFunc dup_func, gpointer user_data)
{
    g_return_val_if_fail(G_IS_OBJECT(object), NULL);
    g_return_val_if_fail(key != NULL, NULL);

    return g_datalist_id_dup_data(&object->qdata, g_quark_from_string(key),
                                  dup_func, user_data);
}
 ...
 * Since: 2.34
 */
gboolean g_object_replace_data(GObject* object, const gchar* key,
                               ...
                               GDestroyNotify* old_destroy)
{
    ...
    return g_datalist_id_replace_data(&object->qdata, g_quark_from_string(key),
                                      oldval, newval, destroy, old_destroy);
}
 ...
 */
void g_object_set_data_full(GObject* object, const gchar* key, gpointer data,
                            GDestroyNotify destroy)
{
    g_return_if_fail(G_IS_OBJECT(object));
    g_return_if_fail(key != NULL);
                                           ...
                                           guint collect_flags)
{
    if (collect_values[0].v_pointer)
    {
        ...
        else if (!g_value_type_compatible(G_OBJECT_TYPE(object),
                                          G_VALUE_TYPE(value)))
            return g_strconcat("invalid object type '",
                               G_OBJECT_TYPE_NAME(object), "' for value type '",
                               G_VALUE_TYPE_NAME(value), "'", NULL);
...
void g_value_set_object(GValue* value, gpointer v_object)
{
    ...
    if (v_object)
    {
        ...
        g_return_if_fail(g_value_type_compatible(G_OBJECT_TYPE(v_object),
                                                 G_VALUE_TYPE(value)));
...
gpointer g_value_dup_object(const GValue* value)
{
    ...
    return value->data[0].v_pointer ? g_object_ref(value->data[0].v_pointer)
                                    : NULL;
}
 ...
 */
gulong g_signal_connect_object(gpointer instance, const gchar* detailed_signal,
                               ...
                               GConnectFlags connect_flags)
{
    ...
    if (gobject)
    {
        ...
        closure = ((connect_flags & G_CONNECT_SWAPPED)
                       ? g_cclosure_new_object_swap
                       : g_cclosure_new_object)(c_handler, gobject);

        return g_signal_connect_closure(instance, detailed_signal, closure,
                                        connect_flags & G_CONNECT_AFTER);
    }
    else
        return g_signal_connect_data(instance, detailed_signal, c_handler, NULL,
                                     NULL, connect_flags);
}
...
static void destroy_closure_array(gpointer data)
{
    ...
    for (i = 0; i < n; i++)
    {
         ...
         */
        g_closure_remove_invalidate_notifier(closure, object,
...
void g_object_watch_closure(GObject* object, GClosure* closure)
{
    ...
    g_closure_add_marshal_guards(closure, object, (GClosureNotify)g_object_ref,
                                 object, (GClosureNotify)g_object_unref);
    G_LOCK(closure_array_mutex);
    carray =
        g_datalist_id_remove_no_notify(&object->qdata, quark_closure_array);
    if (!carray)
    {
    ...
    else
    {
        ...
        carray = g_realloc(carray,
                           sizeof(*carray) + sizeof(carray->closures[0]) * i);
    }
    carray->closures[i] = closure;
    g_datalist_id_set_data_full(&object->qdata, quark_closure_array, carray,
...
GClosure* g_closure_new_object(guint sizeof_closure, GObject* object)
{
    ...
    g_return_val_if_fail(G_IS_OBJECT(object), NULL);
    g_return_val_if_fail(object->ref_count > 0,
                         NULL); /* this doesn't work on finalizing objects */
...
void g_weak_ref_set(GWeakRef* weak_ref, gpointer object)
{
    ...
    if (new_object != old_object)
    {
        ...
        if (old_object != NULL)
        {
            weak_locations = g_datalist_id_get_data(&old_object->qdata,
                                                    quark_weak_locations);
            /* for it to point to an object, the object must have had it added
             * once */
        ...
        if (new_object != NULL)
        {
            weak_locations = g_datalist_id_get_data(&new_object->qdata,
                                                    quark_weak_locations);

            if (weak_locations == NULL)
            {
                weak_locations = g_new0(GSList*, 1);
                g_datalist_id_set_data_full(&new_object->qdata,
                                            quark_weak_locations,
                                            weak_locations, g_free);
            }
