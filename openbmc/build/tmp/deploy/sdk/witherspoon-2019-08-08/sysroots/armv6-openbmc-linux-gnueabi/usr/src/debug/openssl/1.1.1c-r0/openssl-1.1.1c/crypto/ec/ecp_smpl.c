                              ...
                              BN_CTX* ctx)
{
     ...
     * into Y^2 = X^3 + a*X*Z^4 + b*Z^6. To test this, we add up the right-hand
     * side in 'rh'.
     */
                              ...
                              EC_POINT* p, BN_CTX* ctx)
{
    ...
    if (t7 == NULL || !group->meth->field_mul(group, t0, r->X, s->X, ctx) ||
 ...
 * Y4 = two*b*Z3*SQR(Z2*Z1) + Z3*(a*Z2*Z1+X1*X2)*(X1*Z2+X2*Z1) -
 * X3*SQR(X1*Z2-X2*Z1); Z4 = two*Y1*Z3*SQR(Z2)*Z1;
 *
 * Z4 != 0 because:
                              ...
                              EC_POINT* p, BN_CTX* ctx)
{
    ...
    if (BN_is_zero(s->Z))
    {
        ...
        if (!group->meth->field_mul(group, r->X, p->X, p->Z, ctx) ||
            ...
            !BN_copy(r->Z, p->Z) || !EC_POINT_invert(group, r, ctx))
            return 0;
        return 1;
    }
    ...
    if (t6 == NULL || !BN_mod_lshift1_quick(t0, p->Y, group->field) ||
