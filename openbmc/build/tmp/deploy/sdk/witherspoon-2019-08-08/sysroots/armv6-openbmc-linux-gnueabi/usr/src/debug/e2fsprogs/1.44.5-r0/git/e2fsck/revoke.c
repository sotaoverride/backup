...
static void write_one_revoke_record(journal_t*, transaction_t*,
                                    struct list_head*, struct buffer_head**,
                                    int*, struct jbd2_revoke_record_s*, int);
...
int __init journal_init_revoke_caches(void)
{
    ...
    jbd2_revoke_record_cache =
        KMEM_CACHE(jbd2_revoke_record_s, SLAB_HWCACHE_ALIGN | SLAB_TEMPORARY);
    if (!jbd2_revoke_record_cache)
        goto record_cache_failure;

    jbd2_revoke_table_cache = KMEM_CACHE(jbd2_revoke_table_s, SLAB_TEMPORARY);
    if (!jbd2_revoke_table_cache)
                   ...
                   struct buffer_head* bh_in)
{
    ...
    if (bh)
    {
        if (!J_EXPECT_BH(bh, !buffer_revoked(bh), "inconsistent data on disk"))
        {
            if (!bh_in)
                brelse(bh);
            return -EIO;
    ...
    err = insert_revoke_hash(journal, blocknr, handle->h_transaction->t_tid);
...
void jbd2_clear_buffer_revoked_flags(journal_t* journal)
{
    ...
    for (i = 0; i < revoke->hash_size; i++)
    {
        ...
        list_for_each(list_entry, hash_list)
        {
            ...
            bh = __find_get_block(journal->j_fs_dev, record->blocknr,
                                  journal->j_blocksize);
            if (bh)
            {
                clear_buffer_revoked(bh);
                                  ...
                                  struct list_head* log_bufs, int write_op)
{
    ...
    for (i = 0; i < revoke->hash_size; i++)
    {
        ...
        {
            record = (struct jbd2_revoke_record_s*)hash_list->next;
            write_one_revoke_record(journal, transaction, log_bufs, &descriptor,
                                    &offset, record, write_op);
            count++;
            list_del(&record->hash);
            kmem_cache_free(jbd2_revoke_record_cache, record);
        }
...
static void
    write_one_revoke_record(journal_t* journal, transaction_t* transaction,
                            struct list_head* log_bufs,
                            struct buffer_head** descriptorp, int* offsetp,
                            struct jbd2_revoke_record_s* record, int write_op)
{
    ...
    if (JBD2_HAS_INCOMPAT_FEATURE(journal, JBD2_FEATURE_INCOMPAT_64BIT))
    {
        *((__be64*)(&descriptor->b_data[offset])) =
        ...
        else*((__be32*)(&descriptor->b_data[offset])) =
    ...
    static void jbd2_revoke_csum_set(journal_t * j, struct buffer_head * bh)
    {
        ...
        tail =
            (struct journal_revoke_tail*)(bh->b_data + j->j_blocksize -
    ...
    static void flush_descriptor(journal_t * journal,
                                 struct buffer_head * descriptor, int offset,
                                 int write_op)
    {
    ...
    int journal_set_revoke(journal_t * journal, unsigned long long blocknr,
                           tid_t sequence)
    {
        struct jbd2_revoke_record_s* record;
    ...
    int journal_test_revoke(journal_t * journal, unsigned long long blocknr,
                            tid_t sequence)
    {
        struct jbd2_revoke_record_s* record;
