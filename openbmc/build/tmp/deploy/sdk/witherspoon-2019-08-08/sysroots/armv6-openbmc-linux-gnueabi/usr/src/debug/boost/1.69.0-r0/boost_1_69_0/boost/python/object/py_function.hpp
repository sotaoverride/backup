...
{
    caller_py_function_impl(Caller const& caller) : m_caller(caller)
    {
    }
...
{
    signature_py_function_impl(Caller const& caller) : m_caller(caller)
    {
    }
    ...
    virtual python::detail::py_func_sig_info signature() const
    {
        python::detail::signature_element const* sig =
            python::detail::signature<Sig>::elements();
...
{
    full_py_function_impl(Caller const& caller, unsigned min_arity,
                          unsigned max_arity) :
        m_caller(caller),
...
struct py_function
{
    ...
    py_function(Caller const& caller, Sig, int min_arity, int max_arity = 0) :
        m_impl(new full_py_function_impl<Caller, Sig>(caller, min_arity,
                                                      max_arity))
    {
    }
