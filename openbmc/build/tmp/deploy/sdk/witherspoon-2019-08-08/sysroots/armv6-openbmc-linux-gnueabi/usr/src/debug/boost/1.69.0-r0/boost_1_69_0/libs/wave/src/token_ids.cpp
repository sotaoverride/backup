...
get_token_name(token_id tokid)
{
    ...
    //
    //      Please note that the sequence of token names must match the sequence
    //      of token id's defined in then enum token_id above.
    ...
    BOOST_STATIC_ASSERT(sizeof(tok_names) / sizeof(tok_names[0]) ==
...
char const* get_token_value(token_id tokid)
{
    ...
    // make sure, I have not forgotten any commas (as I did more than once)
    BOOST_STATIC_ASSERT(sizeof(tok_values) / sizeof(tok_values[0]) ==
                        T_LAST_TOKEN - T_FIRST_TOKEN);
