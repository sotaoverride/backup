...
int dns_question_matches_cname_or_dname(DnsQuestion* q, DnsResourceRecord* rr,
                                        const char* search_domain)
{
    size_t i;
    int r;
    ...
    for (i = 0; i < q->n_keys; i++)
    {
        ...
        r = dns_resource_key_match_cname_or_dname(q->keys[i], rr->key,
                                                  search_domain);
        if (r != 0)
            return r;
    }
...
int dns_question_cname_redirect(DnsQuestion* q, const DnsResourceRecord* cname,
                                DnsQuestion** ret)
{
...
int dns_question_new_reverse(DnsQuestion** ret, int family,
...
int dns_question_new_service(DnsQuestion** ret, const char* service,
                             const char* type, const char* domain,
                             bool with_txt, bool convert_idna)
{
     ...
     *
     * 1. Only a domain is specified, in which case we assume a properly encoded
     * SRV RR name, including service type and possibly a service name. If
     * specified in this way we assume it's already IDNA converted if that's
     * necessary.
     *
     * 2. Both service type and a domain specified, in which case a normal SRV
     * RR is assumed, without a DNS-SD style prefix. In this case we'll IDNA
     * convert the domain, if that's requested.
     *
     * 3. All three of service name, type and domain are specified, in which
     * case a DNS-SD service is put together. The service name is never IDNA
     * converted, and the domain is if requested.
     *
     * It's not supported to specify a service name without a type, or no domain
     * name.
     */
