...
static void expiration_event(AVAHI_GCC_UNUSED AvahiTimeout* timeout,
                             void* userdata)
{
    ...
    avahi_log_debug(
        __FILE__
        ": Strange, expiration_event() called, but nothing really happened.");
    update_timeout(q);
}
...
AvahiTimeEventQueue* avahi_time_event_queue_new(const AvahiPoll* poll_api)
{
    ...
    if (!(q->timeout =
              poll_api->timeout_new(poll_api, NULL, expiration_event, q)))
        goto oom;
...
AvahiTimeEvent* avahi_time_event_new(AvahiTimeEventQueue* q,
