...
#define AUXTAG                                                                 \
    (DT_NUM + DT_THISPROCNUM + DT_VERSIONTAGNUM + DT_EXTRATAGIDX(DT_AUXILIARY))
/* Whether an shared object references one or more auxiliary objects
   is signaled by the AUXTAG entry in l_info.  */
#define FILTERTAG                                                              \
    (DT_NUM + DT_THISPROCNUM + DT_VERSIONTAGNUM + DT_EXTRATAGIDX(DT_FILTER))
...
static void openaux(void* a)
{
    ...
    args->aux = _dl_map_object(
        args->map, args->name,
        (args->map->l_type == lt_executable ? lt_library : args->map->l_type),
        args->trace_mode, args->open_mode, args->map->l_ns);
}

static ptrdiff_t _dl_build_local_scope(struct link_map** list,
                                       struct link_map* map)
{
...
DST not allowed in SUID/SGID programs"));                                      \
                                                                               \
            __newp = (char*)alloca(                                            \
                DL_DST_REQUIRED(l, __str, strlen(__str), __dst_cnt));          \
                                                                               \
...
static void preload(struct list* known, unsigned int* nlist,
                    struct link_map* map)
{
    known[*nlist].done = 0;
    known[*nlist].map = map;
...
void _dl_map_object_deps(struct link_map* map, struct link_map** preloads,
    ...
    for (runp = known; runp;)
    {
       ...
       dependencies of this object.  */
            ...
            l != map && l->l_ldnum > 0)
        {
            ...
            if (!scratch_buffer_set_array_size(&needed_space, l->l_ldnum,
            ...
            for (d = l->l_ld; d->d_tag != DT_NULL; ++d)
                ...
                else if (d->d_tag == DT_AUXILIARY || d->d_tag == DT_FILTER)
                {
                    ...
                    /* We must be prepared that the addressed shared
                       object is not available.  For filter objects the
        ...
        if (needed != NULL)
        {
            ...
            struct link_map** l_initfini =
                (struct link_map**)malloc((2 * nneeded + 1) * sizeof needed[0]);
            if (l_initfini == NULL)
            {
        ...
        map == GL(dl_ns)[LM_ID_BASE]._ns_loaded)
    {
       ...
       for each library, not just the ultimate loader.  */
        for (i = 0; i < nlist; ++i)
        {
            ...
            if (l == map ||
                (l->l_local_scope[0] && l->l_local_scope[0]->r_nlist) != 0)
                continue;
...
Filters not supported with LD_TRACE_PRELINKING"));
            ...
            for (j = 0; j < cnt; j++)
            {
                ...
                if (j && __builtin_expect(
                             l_initfini[j]->l_info[DT_SYMBOLIC] != NULL, 0))
                    l->l_symbolic_in_local_scope = true;
            }

            l->l_local_scope[0] = (struct r_scope_elem*)malloc(
                sizeof(struct r_scope_elem) + (cnt * sizeof(struct link_map*)));
    ...
    if (errno_reason)
        _dl_signal_exception(errno_reason == -1 ? 0 : errno_reason, &exception,
                             NULL);
}
