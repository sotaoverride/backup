...
libnsl_hidden_nolink_def(nis_leaf_of, GLIBC_2_1)
    ...
    nis_name nis_leaf_of_r(const_nis_name name, char* buffer, size_t buflen)
{
    size_t i = 0;
...
libnsl_hidden_nolink_def(nis_leaf_of_r, GLIBC_2_1)
    ...
    nis_name nis_name_of(const_nis_name name)
{
    static char result[NIS_MAXNAMELEN + 1];
...
libnsl_hidden_nolink_def(nis_name_of, GLIBC_2_1)
    ...
    nis_name nis_name_of_r(const_nis_name name, char* buffer, size_t buflen)
{
    char* local_domain;
    int diff;
...
libnsl_hidden_nolink_def(nis_name_of_r, GLIBC_2_1)
    ...
    static int __always_inline count_dots(const_nis_name str)
{
    int count = 0;
...
nis_name* nis_getnames(const_nis_name name)
{
    ...
    while (cp)
    {
        if (strcmp(cp, "$") == 0)
        {
            ...
            while (*cptr != '\0' && count_dots(cptr) >= 2)
            {
                if (pos >= count)
                {
                    count += 5;
                    nis_name* newp =
                        realloc(getnames, (count + 1) * sizeof(char*));
        ...
        else
        {
            ...
            {
                count += 5;
                nis_name* newp = realloc(getnames, (count + 1) * sizeof(char*));
                if (__glibc_unlikely(newp == NULL))
                    goto free_null;
                getnames = newp;
            }
    ...
    if (pos == 0 &&
        __asprintf(&getnames[pos++], "%s%s%s%s", name,
                   name[name_len - 1] == '.' ? "" : ".", local_domain,
...
libnsl_hidden_nolink_def(nis_freenames, GLIBC_2_1)
    ...
    name_pos nis_dir_cmp(const_nis_name n1, const_nis_name n2)
{
    int len1, len2;
