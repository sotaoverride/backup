...
static void display_help(int status)
{
    fprintf(
        ...
        "  -O, --onlyoob           Input contains oob data and only write the "
        "oob part\n"
        "  -s addr, --start=addr   Set output start address (default is 0)\n"
        "  --skip-bad-blocks-to-start"
        "                          Skip bad blocks when seeking to the start "
        "address\n"
...
static int is_virt_block_bad(struct mtd_dev_info* mtd, int fd, long long offset)
{
    int i, ret = 0;
...
int main(int argc, char* const argv[])
{
    ...
    {
        fprintf(stderr,
                "Image %lld bytes, NAND page %d bytes, OOB area %d"
           ...
           mtdoffset < mtd.size)
    {
        ...
        if (writebuf + mtd.min_io_size > filebuf + filebuf_len)
        {
            ...
            if (tinycnt < readlen)
            {
                ...
                {
                    fprintf(stderr,
                            "Unexpected EOF. Expecting at least "
        ...
        if (writeoob)
        {
            ...
            /* Read more data for the OOB from the input if there isn't enough
             * in the buffer */
            if (oobbuf + mtd.oob_size > filebuf + filebuf_len)
            {
                ...
                {
                    /* No more bytes - we are done after writing the remaining
                     * bytes */
                    imglen = 0;
                }
            }
        ...
        ret = 0;
        if (!skipallffs ||
            !buffer_check_pattern(writebuf, mtd.min_io_size, 0xff))
        {
            /* Write out data */
            ret = mtd_write(mtd_desc, &mtd, fd, mtdoffset / mtd.eb_size,
                            mtdoffset % mtd.eb_size, onlyoob ? NULL : writebuf,
                            onlyoob ? 0 : mtd.min_io_size,
                            writeoob ? oobbuf : NULL,
                            writeoob ? mtd.oob_size : 0, write_mode);
        }
        ...
        if (ret)
        {
            ...
            if (mtd_erase_multi(mtd_desc, &mtd, fd, blockstart / mtd.eb_size,
                                blockalign))
            {
                int errno_tmp = errno;
