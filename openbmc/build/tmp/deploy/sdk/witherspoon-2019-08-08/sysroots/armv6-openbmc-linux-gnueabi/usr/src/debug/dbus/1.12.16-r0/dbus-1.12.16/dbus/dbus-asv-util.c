...
dbus_bool_t _dbus_asv_open_entry(DBusMessageIter* arr_iter,
                                 ...
                                 const char* type, DBusMessageIter* var_iter)
{
    if (!dbus_message_iter_open_container(arr_iter, DBUS_TYPE_DICT_ENTRY, NULL,
                                          entry_iter))
        return FALSE;
    ...
    if (!dbus_message_iter_open_container(entry_iter, DBUS_TYPE_VARIANT, type,
                                          var_iter))
    {
        dbus_message_iter_abandon_container(arr_iter, entry_iter);
        return FALSE;
 ...
 *
 * @param iter the iterator which is appending to the message or other data
 * structure containing the a{sv}
 ...
 * @returns #TRUE on success, or #FALSE if not enough memory
 */
dbus_bool_t _dbus_asv_add_uint32(DBusMessageIter* arr_iter, const char* key,
                                 dbus_uint32_t value)
{
 ...
 * @returns #TRUE on success, or #FALSE if not enough memory
 */
dbus_bool_t _dbus_asv_add_string(DBusMessageIter* arr_iter, const char* key,
                                 const char* value)
{
    DBusMessageIter entry_iter, var_iter;
 ...
 * @returns #TRUE on success, or #FALSE if not enough memory
 */
dbus_bool_t _dbus_asv_add_byte_array(DBusMessageIter* arr_iter, const char* key,
