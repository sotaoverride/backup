...
void server_forward_kmsg(Server* s, int priority, const char* identifier,
...
void dev_kmsg_record(Server* s, char* p, size_t l)
{
    ...
    _cleanup_free_ char *message = NULL, *syslog_priority = NULL,
                        *syslog_pid = NULL, *syslog_facility = NULL,
                        *syslog_identifier = NULL, *source_time = NULL,
                        *identifier = NULL, *pid = NULL;
    struct iovec iovec[N_IOVEC_META_FIELDS + 7 + N_IOVEC_KERNEL_FIELDS + 2 +
                       N_IOVEC_UDEV_FIELDS];
    ...
    if (s->kernel_seqnum)
    {
        ...
        if (serial > *s->kernel_seqnum)
            server_driver_message(
                s, 0, "MESSAGE_ID=" SD_MESSAGE_JOURNAL_MISSED_STR,
    ...
    *e = 0;
    ...
    for (j = 0; l > 0 && j < N_IOVEC_KERNEL_FIELDS; j++)
    {
        ...
        *e = 0;
        ...
        if (cunescape_length_with_prefix(k, e - k, "_KERNEL_", UNESCAPE_RELAX,
                                         &m) < 0)
            break;
    ...
    if (asprintf(&syslog_facility, "SYSLOG_FACILITY=%i", LOG_FAC(priority)) >=
        0)
    ...
    if (cunescape_length_with_prefix(p, pl, "MESSAGE=", UNESCAPE_RELAX,
                                     &message) >= 0)
        iovec[n++] = IOVEC_MAKE_STRING(message);
...
{
    char
        buffer[8192 + 1]; /* the kernel-side limit per record is 8K currently */
    ssize_t l;
...
static int dispatch_dev_kmsg(sd_event_source* es, int fd, uint32_t revents,
                             void* userdata)
{
    Server* s = userdata;
    ...
    if (!(revents & EPOLLIN))
        log_error("Got invalid event from epoll for /dev/kmsg: %" PRIx32,
                  revents);
...
int server_open_dev_kmsg(Server* s)
{
    ...
    r = sd_event_add_io(s->event, &s->dev_kmsg_event_source, s->dev_kmsg_fd,
                        EPOLLIN, dispatch_dev_kmsg, s);
    if (r < 0)
    {
    ...
    r = sd_event_source_set_priority(s->dev_kmsg_event_source,
...
int server_open_kernel_seqnum(Server* s)
{
    ...
    {
        log_error_errno(
            r, "Failed to allocate sequential number file, ignoring: %m");
        return 0;
    }
