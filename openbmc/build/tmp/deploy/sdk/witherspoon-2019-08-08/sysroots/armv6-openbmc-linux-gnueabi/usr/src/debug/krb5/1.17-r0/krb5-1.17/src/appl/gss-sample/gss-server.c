...
static OM_uint32 enumerateAttributes(OM_uint32* minor, gss_name_t name,
                                     int noisy);
static OM_uint32 showLocalIdentity(OM_uint32* minor, gss_name_t name);
...
static int server_acquire_creds(char* service_name, gss_OID mech,
 ...
 */
static int server_establish_context(int s, gss_cred_id_t server_creds,
    ...
    if (token_flags & TOKEN_CONTEXT_NEXT)
    {
        do
        {
            ...
            maj_stat = gss_accept_sec_context(
                &acc_sec_min_stat, context, server_creds, &recv_tok,
                GSS_C_NO_CHANNEL_BINDINGS, &client, &doid, &send_tok, ret_flags,
                NULL,  /* time_rec */
 ...
 */
static int create_socket(u_short port)
{
 ...
 *                                      -TYT
 */
static int test_import_export_context(gss_ctx_id_t* context)
{
...
{
    gss_buffer_desc client_name, recv_buf, unwrap_buf, mic_buf, *msg_buf,
        *send_buf;
    gss_ctx_id_t context;
    OM_uint32 maj_stat, min_stat;
    int i, conf_state;
...
static void worker_bee(void* param)
{
                          ...
                          gss_buffer_t attribute, int noisy)
{
    ...
    while (more != 0)
    {
        ...
        major =
            gss_get_name_attribute(minor, name, attribute, &authenticated,
        ...
        printf("Attribute %.*s %s %s\n\n%.*s\n", (int)attribute->length,
               (char*)attribute->value, authenticated ? "Authenticated" : "",
               complete ? "Complete" : "", (int)display_value.length,
