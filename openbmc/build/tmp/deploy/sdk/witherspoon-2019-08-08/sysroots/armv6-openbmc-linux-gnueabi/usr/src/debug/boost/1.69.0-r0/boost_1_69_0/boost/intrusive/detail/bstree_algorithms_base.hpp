...
class bstree_algorithms_base
{
  public:
    ...
    {
        for (node_ptr p_left = NodeTraits::get_left(node); p_left;
    ...
    {
        for (node_ptr p_right = NodeTraits::get_right(node); p_right;
    ...
    static node_ptr get_header(const const_node_ptr& node)
    {
        ...
        if (p)
        {
            ...
            // If granparent is not equal to n, then n is neither root nor
            // header, the try the fast path
            if (n != pp)
            {
                do
                {
