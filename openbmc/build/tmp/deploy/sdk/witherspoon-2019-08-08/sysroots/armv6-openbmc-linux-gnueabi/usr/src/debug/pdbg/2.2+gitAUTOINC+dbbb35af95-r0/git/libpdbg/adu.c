                    ...
                    uint64_t size, uint8_t block_size, bool ci)
{
    ...
    for (addr = addr0; addr < start_addr + size; addr += block_size)
    {
        ...
        if (addr < start_addr)
        {
            ...
            size_t n =
                (size <= block_size - offset ? size : block_size - offset);
                         ...
                         uint8_t block_size)
{
    ...
    if (ci)
    {
        ...
        ctrl_reg =
            SETFIELD(P8_FBC_ALTD_TTYPE, ctrl_reg, P8_TTYPE_CI_PARTIAL_READ);
        block_size = (blog2(block_size) + 1) << 1;
    }
    else
    {
        ctrl_reg =
            SETFIELD(P8_FBC_ALTD_TTYPE, ctrl_reg, P8_TTYPE_DMA_PARTIAL_READ);
        block_size = 0;
    }
...
retry:
    ...
    CHECK_ERR_GOTO(out,
                   rc = pib_write(&adu->target, P8_ALTD_CONTROL_REG, ctrl_reg));
    ...
    {
        CHECK_ERR_GOTO(out,
                       rc = pib_read(&adu->target, P8_ALTD_STATUS_REG, &val));
    } while (!val);

    if (!(val & FBC_ALTD_ADDR_DONE) || !(val & FBC_ALTD_DATA_DONE))
    {
                  ...
                  int ci, uint8_t block_size)
{
    ...
    if (ci)
    {
        /* Do cache inhibited access */
        ctrl_reg =
            SETFIELD(P8_FBC_ALTD_TTYPE, ctrl_reg, P8_TTYPE_CI_PARTIAL_WRITE);
        block_size = (blog2(block_size) + 1) << 1;
    }
    else
    {
        ctrl_reg =
            SETFIELD(P8_FBC_ALTD_TTYPE, ctrl_reg, P8_TTYPE_DMA_PARTIAL_WRITE);
        block_size <<= 1;
    }
    ctrl_reg = SETFIELD(P8_FBC_ALTD_TSIZE, ctrl_reg, block_size);
                         ...
                         uint8_t block_size)
{
    ...
    if (ci)
    {
        /* Do cache inhibited access */
        cmd_reg =
            SETFIELD(P9_FBC_ALTD_TTYPE, cmd_reg, P9_TTYPE_CI_PARTIAL_READ);
        block_size = (blog2(block_size) + 1) << 1;
    }
    else
    {
        cmd_reg =
            SETFIELD(P9_FBC_ALTD_TTYPE, cmd_reg, P9_TTYPE_DMA_PARTIAL_READ);
...
static int p9_adu_putmem(struct adu* adu, uint64_t addr, uint64_t data,
                         int size, int ci, uint8_t block_size)
{
    uint64_t ctrl_reg, cmd_reg, val;
    ...
    if (ci)
    {
        /* Do cache inhibited access */
        cmd_reg =
            SETFIELD(P9_FBC_ALTD_TTYPE, cmd_reg, P9_TTYPE_CI_PARTIAL_WRITE);
        block_size = (blog2(block_size) + 1) << 1;
    }
    else
    {
        cmd_reg =
            SETFIELD(P9_FBC_ALTD_TTYPE, cmd_reg, P9_TTYPE_DMA_PARTIAL_WRITE);
        block_size <<= 1;
    }
    cmd_reg = SETFIELD(P9_FBC_ALTD_TSIZE, cmd_reg, block_size);
    cmd_reg |= FBC_ALTD_START_OP;
...
__attribute__((constructor)) static void register_adu(void)
{
