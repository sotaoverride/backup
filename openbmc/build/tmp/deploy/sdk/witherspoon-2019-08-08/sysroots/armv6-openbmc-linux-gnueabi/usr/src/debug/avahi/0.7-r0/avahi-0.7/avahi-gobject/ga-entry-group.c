...
#define GA_ENTRY_GROUP_GET_PRIVATE(o)                                          \
    (G_TYPE_INSTANCE_GET_PRIVATE((o), GA_TYPE_ENTRY_GROUP, GaEntryGroupPrivate))
...
static void ga_entry_group_init(GaEntryGroup* obj)
{
    ...
    priv->client = NULL;
    priv->group = NULL;
    priv->services = g_hash_table_new_full(g_direct_hash, g_direct_equal, NULL,
                                           _free_service);
}
...
static void ga_entry_group_get_property(GObject* object, guint property_id,
...
static void ga_entry_group_class_init(GaEntryGroupClass* ga_entry_group_class)
{
    ...
    g_type_class_add_private(ga_entry_group_class, sizeof(GaEntryGroupPrivate));
    ...
    param_spec = g_param_spec_enum(
        "state", "Entry Group state", "The state of the avahi entry group",
        GA_TYPE_ENTRY_GROUP_STATE, GA_ENTRY_GROUP_STATE_UNCOMMITED,
        G_PARAM_READABLE | G_PARAM_STATIC_NAME | G_PARAM_STATIC_BLURB);
    g_object_class_install_property(object_class, PROP_STATE, param_spec);

    signals[STATE_CHANGED] =
        g_signal_new("state-changed", G_OBJECT_CLASS_TYPE(ga_entry_group_class),
...
{
    const GaEntryGroupServiceEntry* entry = (const GaEntryGroupServiceEntry*)v;
    guint32 h = 0;
    guint i;
...
{
    const GaEntryGroupServiceEntry* aentry = (const GaEntryGroupServiceEntry*)a;
    const GaEntryGroupServiceEntry* bentry = (const GaEntryGroupServiceEntry*)b;
...
static GHashTable* _string_list_to_hash(AvahiStringList* list)
{
    ...
    ret = g_hash_table_new_full(_entry_hash, _entry_equal, _free_entry,
                                _free_entry);
...
static void _hash_to_string_list_foreach(gpointer key, gpointer value,
                                         gpointer data)
{
    ...
    if (value != NULL)
    {
        *list = avahi_string_list_add_pair_arbitrary(
            *list, (gchar*)kentry->value, ventry->value, ventry->size);
    }
    else
    {
        *list =
            avahi_string_list_add_arbitrary(*list, kentry->value, kentry->size);
    }
}
...
GaEntryGroupService*
    ga_entry_group_add_service_strlist(GaEntryGroup* group, const gchar* name,
                                       const gchar* type, guint16 port,
                                       GError** error, AvahiStringList* txt)
{
    return ga_entry_group_add_service_full_strlist(
        group, AVAHI_IF_UNSPEC, AVAHI_PROTO_UNSPEC, 0, name, type, NULL, NULL,
        port, error, txt);
}

GaEntryGroupService* ga_entry_group_add_service_full_strlist(
    GaEntryGroup* group, AvahiIfIndex interface, AvahiProtocol protocol,
    AvahiPublishFlags flags, const gchar* name, const gchar* type,
    const gchar* domain, const gchar* host, guint16 port, GError** error,
    AvahiStringList* txt)
{
    GaEntryGroupPrivate* priv = GA_ENTRY_GROUP_GET_PRIVATE(group);
    ...
    ret = avahi_entry_group_add_service_strlst(priv->group, interface, protocol,
                                               flags, name, type, domain, host,
                                               port, txt);
    if (ret)
    {
        if (error != NULL)
        {
            *error =
                g_error_new(GA_ERROR, ret, "Adding service to group failed: %s",
                            avahi_strerror(ret));
        }
        goto out;
    }
                                                ...
                                                GError** error, ...)
{
    ...
    ret = ga_entry_group_add_service_full_strlist(
        group, AVAHI_IF_UNSPEC, AVAHI_PROTO_UNSPEC, 0, name, type, NULL, NULL,
        port, error, txt);
...
GaEntryGroupService* ga_entry_group_add_service_full(
    GaEntryGroup* group, AvahiIfIndex interface, AvahiProtocol protocol,
    AvahiPublishFlags flags, const gchar* name, const gchar* type,
    const gchar* domain, const gchar* host, guint16 port, GError** error, ...)
{
    ...
    ret = ga_entry_group_add_service_full_strlist(group, interface, protocol,
...
gboolean ga_entry_group_add_record(GaEntryGroup* group, AvahiPublishFlags flags,
                                   const gchar* name, guint16 type, guint32 ttl,
                                   const void* rdata, gsize size,
                                   GError** error)
{
    return ga_entry_group_add_record_full(
        group, AVAHI_IF_UNSPEC, AVAHI_PROTO_UNSPEC, flags, name,
        AVAHI_DNS_CLASS_IN, type, ttl, rdata, size, error);
}

gboolean ga_entry_group_add_record_full(
    GaEntryGroup* group, AvahiIfIndex interface, AvahiProtocol protocol,
    AvahiPublishFlags flags, const gchar* name, guint16 clazz, guint16 type,
    guint32 ttl, const void* rdata, gsize size, GError** error)
{
    int ret;
    GaEntryGroupPrivate* priv = GA_ENTRY_GROUP_GET_PRIVATE(group);
    g_assert(group != NULL && priv->group != NULL);

    ret = avahi_entry_group_add_record(priv->group, interface, protocol, flags,
                                       name, clazz, type, ttl, rdata, size);
    if (ret)
    {
        if (error != NULL)
        {
            *error = g_error_new(GA_ERROR, ret, "Setting raw record failed: %s",
                                 avahi_strerror(ret));
        }
        return FALSE;
    }
                                     ...
                                     GError** error)
{
    ...
    ret = avahi_entry_group_update_service_txt_strlst(
        GA_ENTRY_GROUP_GET_PRIVATE(priv->group)->group, service->interface,
        service->protocol, service->flags, service->name, service->type,
        service->domain, txt);
    if (ret)
    {
        if (error != NULL)
        {
            *error =
                g_error_new(GA_ERROR, ret, "Updating txt record failed: %s",
                            avahi_strerror(ret));
        }
        result = FALSE;
    }
...
gboolean ga_entry_group_service_set(GaEntryGroupService* service,
                                    ...
                                    GError** error)
{
    return ga_entry_group_service_set_arbitrary(
        service, key, (const guint8*)value, strlen(value), error);
}

gboolean ga_entry_group_service_set_arbitrary(GaEntryGroupService* service,
                                              const gchar* key,
                                              const guint8* value, gsize size,
                                              GError** error)
{
    GaEntryGroupServicePrivate* priv = (GaEntryGroupServicePrivate*)service;
...
gboolean ga_entry_group_attach(GaEntryGroup* group, GaClient* client,
                               GError** error)
{
    GaEntryGroupPrivate* priv = GA_ENTRY_GROUP_GET_PRIVATE(group);
    ...
    if (priv->group == NULL)
    {
        if (error != NULL)
        {
            ...
            *error = g_error_new(GA_ERROR, aerrno, "Attaching group failed: %s",
...
gboolean ga_entry_group_commit(GaEntryGroup* group, GError** error)
{
    ...
    if (ret)
    {
        if (error != NULL)
        {
            *error = g_error_new(GA_ERROR, ret, "Committing group failed: %s",
                                 avahi_strerror(ret));
        }
        return FALSE;
    }
...
gboolean ga_entry_group_reset(GaEntryGroup* group, GError** error)
{
    ...
    if (ret)
    {
        if (error != NULL)
        {
            *error = g_error_new(GA_ERROR, ret, "Resetting group failed: %s",
                                 avahi_strerror(ret));
        }
        return FALSE;
    }
