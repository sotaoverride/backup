...
static int address_compare(const struct local_address* a,
                           const struct local_address* b)
{
    int r;
...
int local_addresses(sd_netlink* context, int ifindex, int af,
                    struct local_address** ret)
{
    ...
    for (m = reply; m; m = sd_netlink_message_next(m))
    {
        ...
        switch (family)
        {
            ...
            case AF_INET:
                r = sd_netlink_message_read_in_addr(m, IFA_LOCAL,
                                                    &a->address.in);
                if (r < 0)
                {
                    r = sd_netlink_message_read_in_addr(m, IFA_ADDRESS,
                                                        &a->address.in);
                    if (r < 0)
                        continue;
                }
            ...
            case AF_INET6:
                r = sd_netlink_message_read_in6_addr(m, IFA_LOCAL,
                                                     &a->address.in6);
                if (r < 0)
                {
                    r = sd_netlink_message_read_in6_addr(m, IFA_ADDRESS,
                                                         &a->address.in6);
                    if (r < 0)
                        continue;
                }
...
int local_gateways(sd_netlink* context, int ifindex, int af,
                   struct local_address** ret)
{
    _cleanup_(sd_netlink_message_unrefp) sd_netlink_message *req = NULL,
                                                            *reply = NULL;
    ...
    for (m = reply; m; m = sd_netlink_message_next(m))
    {
        ...
        if (r == -ENODATA) /* Not all routes have an RTA_OIF attribute (for
                              example nexthop ones) */
            continue;
        if (r < 0)
            return r;
        ...
        {
            case AF_INET:
                r = sd_netlink_message_read_in_addr(m, RTA_GATEWAY,
                                                    &a->address.in);
                if (r < 0)
                    continue;
                ...
                break;
            case AF_INET6:
                r = sd_netlink_message_read_in6_addr(m, RTA_GATEWAY,
                                                     &a->address.in6);
                if (r < 0)
                    continue;
