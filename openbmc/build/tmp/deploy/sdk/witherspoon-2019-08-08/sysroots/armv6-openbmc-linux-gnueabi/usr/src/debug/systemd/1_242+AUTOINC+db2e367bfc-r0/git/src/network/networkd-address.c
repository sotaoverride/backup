...
static int address_new_static(Network* network, const char* filename,
                              unsigned section_line, Address** ret)
{
    ...
    if (filename)
    {
        ...
        r = hashmap_ensure_allocated(&network->addresses_by_section,
                                     &network_config_hash_ops);
        if (r < 0)
            return r;

        r = hashmap_put(network->addresses_by_section, address->section,
                        address);
        if (r < 0)
            return r;
    }
...
static void address_hash_func(const Address* a, struct siphash* state)
{
    ...
    switch (a->family)
    {
        case AF_INET:
            ...
            if (a->prefixlen != 0)
            {
                ...
                if (a->in_addr_peer.in.s_addr != 0)
                    prefix = be32toh(a->in_addr_peer.in.s_addr) >>
                             (32 - a->prefixlen);
                else
                    prefix =
                        be32toh(a->in_addr.in.s_addr) >> (32 - a->prefixlen);
        ...
        case AF_INET6:
            /* local address */
            siphash24_compress(&a->in_addr, FAMILY_ADDRESS_SIZE(a->family),
                               state);
...
static int address_compare_func(const Address* a1, const Address* a2)
{
    ...
    switch (a1->family)
    {
        ...
        case AF_INET:
            ...
            _fallthrough_;
        case AF_INET6:
            return memcmp(&a1->in_addr, &a2->in_addr,
                          FAMILY_ADDRESS_SIZE(a1->family));
        default:
            /* treat any other address family as AF_UNSPEC */
            return 0;
    }
...
DEFINE_PRIVATE_HASH_OPS(address_hash_ops, Address, address_hash_func,
...
static int address_establish(Address* address, Link* link)
{
    ...
    masq = link->network && link->network->ip_masquerade &&
           address->family == AF_INET && address->scope < RT_SCOPE_LINK;
    ...
    if (address->ip_masquerade_done != masq)
    {
        ...
        r = fw_add_masquerade(masq, AF_INET, 0, &masked, address->prefixlen,
                              NULL, NULL, 0);
        if (r < 0)
            return r;
...
static int address_add_internal(Link* link, Set** addresses, int family,
                                const union in_addr_union* in_addr,
                                unsigned char prefixlen, Address** ret)
{
    _cleanup_(address_freep) Address* address = NULL;
    int r;
...
int address_add_foreign(Link* link, int family,
                        const union in_addr_union* in_addr,
                        unsigned char prefixlen, Address** ret)
{
    return address_add_internal(link, &link->addresses_foreign, family, in_addr,
                                prefixlen, ret);
}

int address_add(Link* link, int family, const union in_addr_union* in_addr,
                unsigned char prefixlen, Address** ret)
{
    Address* address;
    int r;
    ...
    if (r == -ENOENT)
    {
        ...
        r = address_add_internal(link, &link->addresses, family, in_addr,
                                 prefixlen, &address);
        if (r < 0)
            return r;
    }
...
static int address_release(Address* address)
{
    ...
    if (address->ip_masquerade_done)
    {
        ...
        r = fw_add_masquerade(false, AF_INET, 0, &masked, address->prefixlen,
                              NULL, NULL, 0);
        if (r < 0)
            return r;
...
int address_update(Address* address, unsigned char flags, unsigned char scope,
                   const struct ifa_cacheinfo* cinfo)
{
    ...
    if (!ready && address_is_ready(address) && address->family == AF_INET6 &&
        in_addr_is_link_local(AF_INET6, &address->in_addr) > 0 &&
        in_addr_is_null(
            AF_INET6,
            (const union in_addr_union*)&address->link->ipv6ll_address) > 0)
    {
...
int address_drop(Address* address)
{
    ...
    if (r < 0)
        log_link_warning_errno(
            link, r, "Failed to disable IP masquerading, ignoring: %m");
...
int address_get(Link* link, int family, const union in_addr_union* in_addr,
                unsigned char prefixlen, Address** ret)
{
...
static int address_remove_handler(sd_netlink* rtnl, sd_netlink_message* m,
                                  Link* link)
{
    int r;
                   ...
                   link_netlink_message_handler_t callback)
{
    ...
    if (r < 0)
        return log_link_error_errno(
            link, r, "Could not allocate RTM_DELADDR message: %m");

    r = sd_rtnl_message_addr_set_prefixlen(req, address->prefixlen);
    if (r < 0)
        return log_link_error_errno(link, r, "Could not set prefixlen: %m");

    r = netlink_message_append_in_addr_union(req, IFA_LOCAL, address->family,
                                             &address->in_addr);
    if (r < 0)
...
static int address_acquire(Link* link, Address* original, Address** ret)
{
    ...
    r = manager_address_pool_acquire(link->manager, original->family,
                                     original->prefixlen, &in_addr);
    if (r < 0)
        return r;
    if (r == 0)
        return -EBUSY;
    ...
    if (original->family == AF_INET)
    {
        ...
        if (original->prefixlen > 30)
            ...
            broadcast.s_addr = in_addr.in.s_addr |
                               htobe32(0xFFFFFFFFUL >> original->prefixlen);
    }
...
int address_configure(Address* address, Link* link,
                      link_netlink_message_handler_t callback, bool update)
{
    ...
    if (address_get(link, address->family, &address->in_addr,
                    address->prefixlen, NULL) <= 0 &&
    ...
    if (r < 0)
        return log_link_error_errno(
            link, r, "Failed to acquire an address from pool: %m");
    ...
    if (r < 0)
        return log_link_error_errno(
            link, r, "Could not allocate RTM_NEWADDR message: %m");
    ...
    if (in_addr_is_null(address->family, &address->in_addr_peer) == 0)
    {
        ...
        if (r < 0)
            return log_link_error_errno(
                link, r, "Could not append IFA_ADDRESS attribute: %m");
    }
    else if (address->family == AF_INET && address->prefixlen <= 30)
    {
        r = sd_netlink_message_append_in_addr(req, IFA_BROADCAST,
                                              &address->broadcast);
        if (r < 0)
            return log_link_error_errno(
                link, r, "Could not append IFA_BROADCAST attribute: %m");
    }
    ...
    if (address->label)
    {
        ...
        if (r < 0)
            return log_link_error_errno(
                link, r, "Could not append IFA_LABEL attribute: %m");
    }
    ...
    if (r < 0)
        return log_link_error_errno(
            link, r, "Could not append IFA_CACHEINFO attribute: %m");

    r = address_establish(address, link);
    if (r < 0)
        log_link_warning_errno(
            link, r, "Could not enable IP masquerading, ignoring: %m");
...
int config_parse_broadcast(const char* unit, const char* filename,
                           ...
                           const char* rvalue, void* data, void* userdata)
{
    ...
    if (n->family == AF_INET6)
    {
        log_syntax(unit, LOG_ERR, filename, line, 0,
                   "Broadcast is not valid for IPv6 addresses, ignoring "
                   "assignment: %s",
                   rvalue);
        return 0;
    }

    r = in_addr_from_string(AF_INET, rvalue,
                            (union in_addr_union*)&n->broadcast);
    if (r < 0)
    {
                         ...
                         void* data, void* userdata)
{
    ...
    r = in_addr_prefix_from_string_auto_internal(rvalue, PREFIXLEN_REFUSE, &f,
                                                 &buffer, &prefixlen);
    if (r == -ENOANO)
    {
        log_syntax(unit, LOG_ERR, filename, line, r,
                   "An address '%s' is specified without prefix length. "
                   "The behavior of parsing addresses without prefix length "
                   "will be changed in the future release. "
                   "Please specify prefix length explicitly.",
                   rvalue);

        r = in_addr_prefix_from_string_auto_internal(rvalue, PREFIXLEN_LEGACY,
                                                     &f, &buffer, &prefixlen);
    }
    if (r < 0)
    {
    ...
    {
        log_syntax(unit, LOG_ERR, filename, line, 0,
                   "Address is incompatible, ignoring assignment: %s", rvalue);
        return 0;
    }

    if (in_addr_is_null(f, &buffer))
    {
        /* Will use address from address pool. Note that for ipv6 case, prefix
         * of the address pool is 8, but 40 bit is used by the global ID and 16
         * bit by the subnet ID. So, let's limit the prefix length to 64 or
         * larger. See RFC4193. */
        if ((f == AF_INET && prefixlen < 8) ||
            (f == AF_INET6 && prefixlen < 64))
        {
            log_syntax(unit, LOG_ERR, filename, line, 0,
                       "Null address with invalid prefixlen='%u', ignoring "
                       "assignment: %s",
                       prefixlen, rvalue);
            return 0;
        }
    ...
    if (n->family == AF_INET && n->broadcast.s_addr == 0)
        n->broadcast.s_addr =
            n->in_addr.in.s_addr | htonl(0xfffffffflu >> n->prefixlen);
...
int config_parse_lifetime(const char* unit, const char* filename, unsigned line,
                          const char* section, unsigned section_line,
                          const char* lvalue, int ltype, const char* rvalue,
                          void* data, void* userdata)
{
...
int config_parse_address_flags(const char* unit, const char* filename,
                               unsigned line, const char* section,
...
int config_parse_address_scope(const char* unit, const char* filename,
                               unsigned line, const char* section,
                               unsigned section_line, const char* lvalue,
                               int ltype, const char* rvalue, void* data,
                               void* userdata)
