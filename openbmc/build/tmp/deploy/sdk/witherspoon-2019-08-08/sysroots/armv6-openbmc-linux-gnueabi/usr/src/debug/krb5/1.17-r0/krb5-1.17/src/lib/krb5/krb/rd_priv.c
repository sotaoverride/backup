...
static krb5_error_code rd_priv_basic(krb5_context context, krb5_auth_context ac,
                                     ...
                                     krb5_data* outbuf)
{
    ...
    if ((retval = krb5_k_decrypt(context, key, KRB5_KEYUSAGE_KRB_PRIV_ENCPART,
                                 iv, &privmsg->enc_part, &scratch)))
                                           ...
                                           krb5_replay_data* outdata)
{
    ...
    retval =
        rd_priv_basic(context, auth_context, inbuf, key, &replaydata, outbuf);
    if (retval)
        return retval;
