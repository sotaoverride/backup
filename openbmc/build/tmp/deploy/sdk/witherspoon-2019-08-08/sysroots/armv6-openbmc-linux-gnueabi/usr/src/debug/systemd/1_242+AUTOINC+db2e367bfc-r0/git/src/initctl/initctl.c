...
static int change_runlevel(Server* s, int runlevel)
{
    ...
    r = sd_bus_call_method(s->bus, "org.freedesktop.systemd1",
                           ...
                           &error, NULL, "ss", target, mode);
    if (r < 0)
        return log_error_errno(r, "Failed to change runlevel: %s",
...
static void request_process(Server* s, const struct init_request* req)
{
    ...
    switch (req->cmd)
    {
        ...
        case INIT_CMD_CHANGECONS:
            log_warning("Received console change initctl request. This is not "
                        "implemented in systemd.");
            return;

        case INIT_CMD_SETENV:
        case INIT_CMD_UNSETENV:
            log_warning("Received environment initctl request. This is not "
                        "implemented in systemd.");
            return;
