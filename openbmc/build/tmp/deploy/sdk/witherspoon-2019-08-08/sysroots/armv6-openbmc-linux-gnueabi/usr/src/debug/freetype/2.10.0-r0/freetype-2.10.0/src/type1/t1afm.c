...
static FT_Int t1_get_index(const char* name, FT_Offset len, void* user_data)
{
    ...
    for (n = 0; n < type1->num_glyphs; n++)
    {
        ...
        if (gname && gname[0] == name[0] && ft_strlen(gname) == len &&
...
T1_Read_Metrics(FT_Face t1_face, FT_Stream stream)
{
    ...
    if (FT_NEW(fi) || FT_FRAME_ENTER(stream->size))
        goto Exit;
    ...
    {
        error = psaux->afm_parser_funcs->init(&parser, stream->memory,
...
FT_LOCAL_DEF(void)
T1_Get_Kerning(AFM_FontInfo fi, FT_UInt glyph1, FT_UInt glyph2,
               FT_Vector* kerning)
{
...
FT_LOCAL_DEF(FT_Error)
T1_Get_Track_Kerning(FT_Face face, FT_Fixed ptsize, FT_Int degree,
                     FT_Fixed* kerning)
{
    ...
    for (i = 0; i < fi->NumTrackKern; i++)
    {
        ...
        {
            *kerning =
                FT_MulDiv(ptsize - tk->min_ptsize, tk->max_kern - tk->min_kern,
