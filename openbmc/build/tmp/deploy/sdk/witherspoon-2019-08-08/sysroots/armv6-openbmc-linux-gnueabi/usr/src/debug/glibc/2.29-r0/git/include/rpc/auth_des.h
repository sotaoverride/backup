...
libc_hidden_proto(getpublickey) libc_hidden_proto(getsecretkey)
    libc_hidden_proto(rtime)

        extern bool_t xdr_authdes_cred(XDR* xdrs, struct authdes_cred* cred);
extern bool_t xdr_authdes_verf(XDR* xdrs, struct authdes_verf* verf);
struct svc_req;
struct rpc_msg;
...
#define DECLARE_NSS_PROTOTYPES(service)                                        \
    ...
    extern enum nss_status _nss_##service##_getsecretkey(                      \
        const char* netname, char* skey, char* passwd, int* errnop);
...
libc_hidden_proto(authdes_getucred) libc_hidden_proto(xdr_authdes_cred)
