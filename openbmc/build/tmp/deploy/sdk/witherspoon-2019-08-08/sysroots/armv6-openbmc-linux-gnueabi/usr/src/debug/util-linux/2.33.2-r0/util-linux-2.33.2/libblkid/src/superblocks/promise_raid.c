...
{
    size_t i;
    static unsigned int sectors[] = {63,  255, 256, 16,  399, 591, 675,
                                     735, 911, 974, 991, 951, 3087};
    uint64_t nsectors;
    ...
    for (i = 0; i < ARRAY_SIZE(sectors); i++)
    {
        ...
        pdc = (struct promise_metadata*)blkid_probe_get_buffer(
            pr, off, sizeof(struct promise_metadata));
        if (!pdc)
...
const struct blkid_idinfo pdcraid_idinfo = {.name =
