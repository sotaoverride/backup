/* omtesting.c
 *
 * This module is a testing aid. It is not meant to be used in production. I
 * have initially written it to introduce delays of custom length to action
 * processing. This is needed for development of new message queueing methods.
 * However, I think there are other uses for this module. For example, I can
 * envision that it is a good thing to have an output module that requests a
 * retry on every "n"th invocation and such things. I implement only what I
 * need. But should further testing needs arise, it makes much sense to add them
 * here.
 *
 * This module will become part of the CVS and the rsyslog project because I
 * think it is a generally useful debugging, testing and development aid for
 * everyone involved with rsyslog.
 *
...
typedef struct _instanceData
{
    ...
    int bEchoStdout;
    int iWaitSeconds;
    int iWaitUSeconds; /* micro-seconds (one millionth of a second, just to make
                          sure...) */
    int iCurrCallNbr;
    int iFailFrequency;
    int iResumeAfter;
    int iCurrRetries;
    int bFailed; /* indicates if we are already in failed state - this is
                  * necessary to work properly together with multiple worker
                  * instances.
                  */
    pthread_mutex_t mut;
} instanceData;
...
BEGINinitConfVars /* (re)set config variables to default values */
    CODESTARTinitConfVars cs.bEchoStdout = 0;
ENDinitConfVars

    BEGINcreateInstance CODESTARTcreateInstance pData->iWaitSeconds = 1;
...
ENDcreateInstance
    ...
    BEGINcreateWrkrInstance CODESTARTcreateWrkrInstance ENDcreateWrkrInstance
        ...
        BEGINdbgPrintInstInfo CODESTARTdbgPrintInstInfo dbgprintf(
            "Action delays rule by %d second(s) and %d microsecond(s)\n",
            pData->iWaitSeconds, pData->iWaitUSeconds);
/* do nothing */
ENDdbgPrintInstInfo

    BEGINisCompatibleWithFeature CODESTARTisCompatibleWithFeature
        /* we are not compatible with repeated msg reduction feature, so do not
           allow it */
            ENDisCompatibleWithFeature

    /* implement "fail" command in retry processing */
    static rsRetVal
    doFailOnResume(instanceData* pData)
{
    DEFiRet;
...
{
    DEFiRet;
    if ((randomNumber() >> 4) < (RAND_MAX >> 5))
    { /* rougly same probability */
        iRet = RS_RET_OK;
...
BEGINtryResume CODESTARTtryResume dbgprintf("omtesting tryResume() called\n");
...
CODESTARTdoAction dbgprintf("omtesting received msg '%s'\n", ppString[0]);
...
ENDdoAction
    ...
    BEGINfreeInstance CODESTARTfreeInstance pthread_mutex_destroy(&pData->mut);
ENDfreeInstance

    BEGINfreeWrkrInstance CODESTARTfreeWrkrInstance ENDfreeWrkrInstance

        BEGINparseSelectorAct int i;
uchar szBuf[1024];
CODESTARTparseSelectorAct CODE_STD_STRING_REQUESTparseSelectorAct(1)
...
{
    p += sizeof(":omtesting:") -
         1; /* eat indicator sequence (-1 because of '\0'!) */
}
else
{
    ...
    BEGINmodExit CODESTARTmodExit ENDmodExit
        ...
        BEGINqueryEtryPt CODESTARTqueryEtryPt CODEqueryEtryPt_STD_OMOD_QUERIES
            CODEqueryEtryPt_STD_OMOD8_QUERIES
                CODEqueryEtryPt_STD_CONF2_CNFNAME_QUERIES ENDqueryEtryPt

    BEGINmodInit() CODESTARTmodInit INITLegCnfVars* ipIFVersProvided =
        CURR_MOD_IF_VERSION; /* we only support the current interface
                                specification */
CODEmodInit_QueryRegCFSLineHdlr CHKiRet(
    omsdRegCFSLineHdlr((uchar*)"actionomtestingechostdout", 0, eCmdHdlrBinary,
