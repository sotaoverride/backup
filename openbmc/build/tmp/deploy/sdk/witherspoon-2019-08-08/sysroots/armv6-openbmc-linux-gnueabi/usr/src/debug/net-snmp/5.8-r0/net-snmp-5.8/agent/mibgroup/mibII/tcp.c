...
void init_tcp(void)
{
    ...
    reginfo = netsnmp_create_handler_registration(
        "tcp", tcp_handler, tcp_oid, OID_LENGTH(tcp_oid), HANDLER_CAN_RONLY);
    ...
    netsnmp_inject_handler(
        reginfo,
        netsnmp_get_cache_handler(TCP_STATS_CACHE_TIMEOUT, tcp_load, tcp_free,
...
int tcp_handler(netsnmp_mib_handler* handler,
                netsnmp_handler_registration* reginfo,
        ...
        case MODE_GET:
            for (request = requests; request; request = request->next)
            {
                ...
                switch (subid)
                {
                     ...
                     * together, so we need to isolate the TCP-specific bits.
                     */
#define tcpstat tcpstat.tcpstat
                    ...
                    case TCPOUTSEGS:
                         ...
                         * octets"
                         */
                        ret_value = (tcpstat.tcps_sndtotal -
                    ...
                    case TCPRETRANSSEGS:
                        ...
                        break;
                    case TCPINERRS:
                        ret_value = tcpstat.tcps_rcvbadsum +
...
#ifdef hpux11
int tcp_load(netsnmp_cache* cache, void* vmagic)
{
    int fd;
    struct nmparms p;
    unsigned int ulen;
    ...
    {
        DEBUGMSGTL(("mibII/tcpScalar",
                    "Failed to load TCP object %d (hpux11)\n", magic));
        return (-1); /* error */
    }
...
int tcp_load(netsnmp_cache* cache, void* vmagic)
{
    ...
    {
        DEBUGMSGTL(
            ("mibII/tcpScalar", "Failed to load TCP scalar Group (linux)\n"));
    }
    else
    {
...
int tcp_load(netsnmp_cache* cache, void* vmagic)
{
    ...
    if (magic == TCPINERRS)
    {
        if (getMibstat(MIB_IP, &ipstat, sizeof(mib2_ip_t), GET_FIRST,
                       &Get_everything, NULL) < 0)
        {
            DEBUGMSGTL(("mibII/tcpScalar",
                        "Failed to load TCP object %d (solaris)\n", magic));
            return -1;
        }
        else
        {
            DEBUGMSGTL(
                ("mibII/tcpScalar", "Loaded TCP object %d (solaris)\n", magic));
     ...
     */
    ret_value = getMibstat(MIB_TCP, &tcpstat, sizeof(mib2_tcp_t), GET_FIRST,
                           &Get_everything, NULL);

    if (ret_value < 0)
    {
        DEBUGMSGTL(
            ("mibII/tcpScalar", "Failed to load TCP scalar Group (solaris)\n"));
    }
    else
    {
...
int tcp_load(netsnmp_cache* cache, void* vmagic)
{
    ...
    {
        DEBUGMSGTL(
            ("mibII/tcpScalar", "Failed to load TCP scalar Group (netbsd)\n"));
    }
    else
    {
...
int tcp_load(netsnmp_cache* cache, void* vmagic)
{
    ...
    {
        DEBUGMSGTL(
            ("mibII/tcpScalar", "Failed to load TCP scalar Group (win32)\n"));
    }
    else
    {
...
int tcp_load(netsnmp_cache* cache, void* vmagic)
{
    ...
    {
        DEBUGMSGTL(
            ("mibII/tcpScalar", "Failed to load TCP scalar Group (AIX)\n"));
    }
    else
    {
        ret_value = 0;
...
int tcp_load(netsnmp_cache* cache, void* vmagic)
{
    ...
    {
        DEBUGMSGTL(
            ("mibII/tcpScalar", "Failed to load TCP scalar Group (sysctl)\n"));
    }
    else
    {
...
int tcp_load(netsnmp_cache* cache, void* vmagic)
{
    ...
    {
        DEBUGMSGTL(("mibII/tcpScalar",
                    "Failed to load TCP scalar Group (tcpipstats)\n"));
    }
    else
    {
        DEBUGMSGTL(
            ("mibII/tcpScalar", "Loaded TCP scalar Group (tcpipstats)\n"));
    }
    return ret_value;
}
...
int tcp_load(netsnmp_cache* cache, void* vmagic)
{
    ...
    {
        DEBUGMSGTL(
            ("mibII/tcpScalar", "Failed to load TCP scalar Group (tcpstat)\n"));
    }
    else
    {
...
void tcp_free(netsnmp_cache* cache, void* magic)
{
#if defined(_USE_FIRST_PROTOCOL)
