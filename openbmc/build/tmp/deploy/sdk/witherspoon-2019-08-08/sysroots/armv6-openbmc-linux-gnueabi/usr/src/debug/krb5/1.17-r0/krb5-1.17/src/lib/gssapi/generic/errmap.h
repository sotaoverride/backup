 ...
 * script: ../../../util/gen.pl
 * args:   bimap errmap.h NAME=mecherrmap LEFT=OM_uint32 RIGHT=struct mecherror
 * LEFTPRINT=print_OM_uint32 RIGHTPRINT=mecherror_print LEFTCMP=cmp_OM_uint32
 * RIGHTCMP=mecherror_cmp The rest of this file is copied from a template, with
...
static inline int mecherrmap__pairarray_init(mecherrmap__pairarray* arr)
{
...
static inline long mecherrmap__pairarray_size(mecherrmap__pairarray* arr)
{
...
static inline int mecherrmap__pairarray_grow(mecherrmap__pairarray* arr,
...
static inline void mecherrmap__pairarray_set(mecherrmap__pairarray* arr,
                                             long idx,
                                             struct mecherrmap__pair value)
{
...
static inline void mecherrmap__pairarray_destroy(mecherrmap__pairarray* arr)
{
    free(arr->elts);
    arr->elts = 0;
}
...
static inline int mecherrmap_init(mecherrmap* m)
{
    m->nextidx = 0;
    return mecherrmap__pairarray_init(&m->a);
}

static inline long mecherrmap_size(mecherrmap* m)
{
    return mecherrmap__pairarray_size(&m->a);
}

static inline void
    mecherrmap_foreach(mecherrmap* m,
                       int (*fn)(OM_uint32, struct mecherror, void*), void* p)
{
    long i, sz;
    sz = m->nextidx;
    for (i = 0; i < sz; i++)
...
static inline int mecherrmap_add(mecherrmap* m, OM_uint32 l, struct mecherror r)
{
    long i, sz;
...
static inline const mecherrmap__right_t* mecherrmap_findleft(mecherrmap* m,
                                                             OM_uint32 l)
{
    long i, sz;
...
static inline const mecherrmap__left_t* mecherrmap_findright(mecherrmap* m,
                                                             struct mecherror r)
{
    long i, sz;
    sz = mecherrmap_size(m);
...
static inline int mecherrmap__printone(OM_uint32 l, struct mecherror r, void* p)
{
...
static inline void mecherrmap_printmap(mecherrmap* m, FILE* f)
{
...
static inline void mecherrmap_destroy(mecherrmap* m)
{
