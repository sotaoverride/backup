...
///
/// This interface is used by observers in order to receive notifications from
/// the Boost.Test framework on the current execution state.
///
/// Several observers can be running at the same time, and it is not unusual to
/// have interactions among them. The @ref test_observer::priority member
/// function allows the specification of a particular order among them (lowest
/// priority executed first, except specified otherwise).
///
class BOOST_TEST_DECL test_observer
{
  public:
    ...
    //!
    //! @param[in] number_of_test_cases indicates the number of test cases. Only
    //! active test cases are taken into account.
    ...
    //!
    //! The critical errors are mainly the signals sent by the system and caught
    //! by the Boost.Test framework. Since the running binary may be in
    //! incoherent/instable state, the test execution is aborted and all
    //! remaining tests are discarded.
    //!
    ...
    virtual void test_unit_finish(test_unit const& /* test */,
    ...
    //! - a strong assertion (with @c REQUIRE) fails, which indicates that the
    //! test case cannot continue
    ...
    //! and tore down in the UTF framework. The order is lowest to highest
    //! priority.
