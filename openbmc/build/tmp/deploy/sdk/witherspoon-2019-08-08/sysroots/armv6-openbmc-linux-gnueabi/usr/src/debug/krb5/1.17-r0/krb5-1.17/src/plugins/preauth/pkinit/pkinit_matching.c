...
static char* keyword2string(unsigned int kw)
{
    switch (kw)
    {
        case kw_undefined:
...
static char* relation2string(unsigned int rel)
{
    switch (rel)
    {
        case relation_none:
...
static char* kwval2string(unsigned int kwval)
{
    switch (kwval)
    {
        case kwvaltype_undefined:
            return "NONE";
            break;
...
static krb5_error_code free_rule_component(krb5_context context,
                                           rule_component* rc)
{
    if (rc == NULL)
        return 0;
...
static krb5_error_code free_rule_set(krb5_context context, rule_set* rs)
{
...
static krb5_error_code parse_list_value(krb5_context context, keyword_type type,
                                        char* value, rule_component* rc)
{
...
static krb5_error_code parse_rule_component(krb5_context context,
                                            const char** rule, int* remaining,
...
static krb5_error_code parse_rule_set(krb5_context context, const char* rule_in,
                                      rule_set** out_rs)
{
    const char* rule;
    int remaining;
    ...
    while (remaining > 0)
    {
        ...
        {
            pkiDebug("%s: Assuming AND relation for multiple components in "
                     "rule '%s'\n",
...
static int regexp_match(krb5_context context, rule_component* rc, char* value)
{
    int code;

    pkiDebug("%s: checking %s rule '%s' with value '%s'\n", __FUNCTION__,
             keyword2string(rc->kw_type), rc->regsrc, value);
...
static int component_match(krb5_context context, rule_component* rc,
...
static krb5_error_code
    check_all_certs(krb5_context context,
                    ...
                    krb5_principal princ, rule_set* rs, /* rule to check */
                    pkinit_cert_matching_data** matchdata, int* match_found,
                    size_t* match_index)
{
    krb5_error_code retval;
    pkinit_cert_matching_data* md;
    ...
    pkinit_identity_crypto_context id_cryptoctx, krb5_principal princ)
{
    ...
    for (x = 0; rules[x] != NULL; x++)
    {
        ...
        retval =
            check_all_certs(context, plg_cryptoctx, req_cryptoctx, id_cryptoctx,
                            princ, rs, matchdata, &match_found, &match_index);
        if (retval)
        {
...
krb5_error_code pkinit_client_cert_match(krb5_context context,
