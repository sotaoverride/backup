...
static void
    dbg_print_extent(const char* desc EXT2FS_ATTR((unused)),
                             ...
                             blk64_t blk, blk64_t len)
{
    ...
    clusters = (len + EXT2FS_CLUSTER_RATIO(fs) - 1) / EXT2FS_CLUSTER_RATIO(fs);
    ext2fs_block_alloc_stats_range(fs, blk, clusters * EXT2FS_CLUSTER_RATIO(fs),
                                   +1);
    return ext2fs_iblk_add_blocks(fs, inode, clusters);
}

static errcode_t ext_falloc_helper(ext2_filsys fs, int flags, ext2_ino_t ino,
                                   ...
                                   blk64_t alloc_goal)
{
    ...
    if (left_ext && EXT2FS_CLUSTER_RATIO(fs) > 1)
    {
         ...
         */
        cluster_fill =
            EXT2FS_CLUSTER_RATIO(fs) - (range_start & EXT2FS_CLUSTER_MASK(fs));
        ...
        if (!(left_ext->e_flags & EXT2_EXTENT_FLAGS_UNINIT))
        {
            err = ext2fs_zero_blocks2(
                fs, left_ext->e_pblk + left_ext->e_len - cluster_fill,
...
expand_right:
    ...
    if (right_ext && EXT2FS_CLUSTER_RATIO(fs) > 1)
    {
        ...
        if (!(right_ext->e_flags & EXT2_EXTENT_FLAGS_UNINIT))
        {
            err = ext2fs_zero_blocks2(fs, right_ext->e_pblk, cluster_fill, NULL,
                                      NULL);
            if (err)
                goto out;
        }
...
try_merge:
    ...
    if (left_ext && right_ext)
    {
        ...
        if (x > (left_ext->e_flags & EXT2_EXTENT_FLAGS_UNINIT ? max_uninit_len
                                                              : max_init_len))
            goto try_left;
        ...
        err = ext2fs_new_range(
            fs, EXT2_NEWRANGE_FIXED_GOAL | EXT2_NEWRANGE_MIN_LENGTH, y,
            right_ext->e_pblk - y + 1, NULL, &pblk, &plen);
        if (err)
            goto try_left;
        ...
        *right_ext = *left_ext;
        ...
        if (!(left_ext->e_flags & EXT2_EXTENT_FLAGS_UNINIT) &&
            (flags & EXT2_FALLOCATE_ZERO_BLOCKS))
        {
            err = ext2fs_zero_blocks2(fs, range_start, range_len, NULL, NULL);
            if (err)
                goto out;
        }
...
try_left:
    ...
    if (left_ext)
    {
        ...
        {
            err = ext2fs_map_cluster_block(fs, ino, inode, x + fillable, &pblk);
            if (err)
                goto out;
            if (pblk)
                fillable -= 1 + ((x + fillable) & EXT2FS_CLUSTER_MASK(fs));
            if (fillable == 0)
                goto try_right;
        }
        ...
        err = ext2fs_new_range(
            fs, EXT2_NEWRANGE_FIXED_GOAL | EXT2_NEWRANGE_MIN_LENGTH, x,
...
try_right:
    ...
    if (right_ext)
    {
        ...
        if (EXT2FS_CLUSTER_RATIO(fs) > 1)
        {
            ...
            if (pblk)
                fillable -=
                    ...
                    ((right_ext->e_lblk - fillable) & EXT2FS_CLUSTER_MASK(fs));
            if (fillable == 0)
                goto try_anywhere;
        }
         ...
         */
        err = ext2fs_new_range(
            fs, EXT2_NEWRANGE_FIXED_GOAL | EXT2_NEWRANGE_MIN_LENGTH,
            right_ext->e_pblk - fillable, fillable, NULL, &pblk, &plen);
        if (err)
            goto try_anywhere;
        err = claim_range(fs, inode, pblk & ~EXT2FS_CLUSTER_MASK(fs),
        ...
        {
            err =
                ext2fs_zero_blocks2(fs, pblk, plen + cluster_fill, NULL, NULL);
            if (err)
                goto out;
        }
    }
...
try_anywhere:
    ...
    if (range_len > 0 && (range_start & EXT2FS_CLUSTER_MASK(fs)))
    {
        ...
        err = ext2fs_map_cluster_block(fs, ino, inode, newex.e_lblk, &pblk);
        if (err)
            goto out;
        if (pblk == 0)
        ...
        newex.e_flags =
            (flags & EXT2_FALLOCATE_FORCE_INIT ? 0 : EXT2_EXTENT_FLAGS_UNINIT);
        dbg_print_extent("ext_falloc iclus left+", &newex);
        ext2fs_extent_goto(handle, newex.e_lblk);
        err = ext2fs_extent_get(handle, EXT2_EXTENT_CURRENT, &ex);
        ...
        dbg_printf("%s: inserting %s lblk %llu newex=%llu\n", __func__,
                   op ? "after" : "before", ex.e_lblk, newex.e_lblk);
        ...
        {
            err =
                ext2fs_zero_blocks2(fs, newex.e_pblk, newex.e_len, NULL, NULL);
            if (err)
                goto out;
        }
...
no_implied:
    ...
    for (x = 0; x < y;)
    {
        ...
        err = ext2fs_new_range(fs, 0, pblk & ~EXT2FS_CLUSTER_MASK(fs), fillable,
                               NULL, &pblk, &plen);
        if (err)
            goto out;
        err = claim_range(fs, inode, pblk, plen);
...
start_again:
    ...
    if (err == EXT2_ET_NO_CURRENT_NODE)
    {
        ...
        err = ext2fs_find_first_zero_block_bitmap2(fs->block_map, goal,
                                                   max_blocks - 1, &goal);
        goal += start;
        err = ext_falloc_helper(fs, flags, ino, inode, handle, NULL, NULL,
                                start, len, goal);
        goto errout;
    }
    else if (err)
    ...
    if (left_extent.e_lblk > start)
    {
        ...
        err =
            ext_falloc_helper(fs, flags, ino, inode, handle, NULL, &left_extent,
                              start, left_extent.e_lblk - start, goal);
        if (err)
            goto errout;
    ...
    do
    {
        err = ext2fs_extent_get(handle, EXT2_EXTENT_NEXT_LEAF, &right_extent);
        dbg_printf("%s: ino=%d get next =%d\n", __func__, ino, (int)err);
        ...
        if (err && err != EXT2_ET_EXTENT_NO_NEXT)
            goto errout;
        if (err == EXT2_ET_EXTENT_NO_NEXT || right_extent.e_lblk > end + 1)
        {
            range_end = end;
            right_adjacent = NULL;
        }
        ...
        if (err != EXT2_ET_EXTENT_NO_NEXT &&
            ...
            goal = right_extent.e_pblk - (right_extent.e_lblk - range_start);
        ...
        if (range_start <= range_end)
        {
            ...
            err =
                ext_falloc_helper(fs, flags, ino, inode, handle, left_adjacent,
                                  right_adjacent, range_start, count, goal);
            if (err)
                goto errout;
        }
                           ...
                           blk64_t start, blk64_t len)
{
    ...
    for (blk = start; blk < start + len; blk++)
    {
        ...
        err = ext2fs_bmap2(fs, ino, inode, NULL,
                           BMAP_ALLOC | BMAP_UNINIT | BMAP_ZERO, blk, 0, &x);
        if (err)
            return err;
    }
