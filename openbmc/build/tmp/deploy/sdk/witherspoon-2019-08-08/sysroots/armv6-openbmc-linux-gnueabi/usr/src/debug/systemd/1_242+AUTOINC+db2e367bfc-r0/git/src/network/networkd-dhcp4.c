...
static int route_scope_from_address(const Route* route,
                                    const struct in_addr* self_addr)
{
    assert(route);
    assert(self_addr);
...
static int link_set_dhcp_routes(Link* link)
{
    ...
    if (!link->dhcp_lease) /* link went down while we configured the IP
                              addresses? */
        return 0;
    ...
    for (i = 0; i < n; i++)
    {
        ...
        /* if the DHCP server returns both a Classless Static Routes option and
           a Static Routes option,
           the DHCP client MUST ignore the Static Routes option. */
        if (classless_route && sd_dhcp_route_get_option(static_routes[i]) !=
        ...
        assert_se(sd_dhcp_route_get_gateway(static_routes[i], &route->gw.in) >=
                  0);
        assert_se(sd_dhcp_route_get_destination(static_routes[i],
                                                &route->dst.in) >= 0);
        assert_se(sd_dhcp_route_get_destination_prefix_length(
                      static_routes[i], &route->dst_prefixlen) >= 0);
    ...
    if (classless_route && static_route)
        log_link_warning(link,
                         "Classless static routes received from DHCP server: "
                         "ignoring static-route option and router option");
    ...
    if (link->network->dhcp_use_routes)
    {
        ...
        if (n >= 0)
        {
            for (i = 0; i < n; i++)
            {
                ...
                {
                    route->family = AF_INET;
                    assert_se(sd_dhcp_route_get_gateway(routes[i],
                                                        &route->gw.in) >= 0);
                    assert_se(sd_dhcp_route_get_destination(
                                  routes[i], &route->dst.in) >= 0);
                    assert_se(sd_dhcp_route_get_destination_prefix_length(
        ...
        if (hostname)
        {
            ...
            if (r < 0)
                log_link_warning_errno(
                    link, r, "Failed to reset transient hostname: %m");
        }
    }
...
static int dhcp4_update_address(Link* link, struct in_addr* address,
                                struct in_addr* netmask, uint32_t lifetime)
{
...
static int dhcp_lease_acquired(sd_dhcp_client* client, Link* link)
{
    ...
    if (r > 0 && !in4_addr_is_null(&router[0]))
        log_struct(LOG_INFO, LOG_LINK_INTERFACE(link),
                   LOG_LINK_MESSAGE(
                       link, "DHCPv4 address %u.%u.%u.%u/%u via %u.%u.%u.%u",
                       ADDRESS_FMT_VAL(address), prefixlen,
                       ADDRESS_FMT_VAL(router[0])),
                   "ADDRESS=%u.%u.%u.%u", ADDRESS_FMT_VAL(address),
                   "PREFIXLEN=%u", prefixlen, "GATEWAY=%u.%u.%u.%u",
                   ADDRESS_FMT_VAL(router[0]));
    else
        log_struct(LOG_INFO, LOG_LINK_INTERFACE(link),
        ...
        if (r >= 0)
        {
            ...
            if (r < 0)
                log_link_error_errno(
                    link, r, "Failed to set MTU to %" PRIu16 ": %m", mtu);
        }
    }
    ...
    if (link->network->dhcp_use_hostname)
    {
        ...
        if (dhcpname)
        {
            ...
            if (r < 0)
                log_link_warning_errno(link, r,
                                       "Unable to shorten overlong DHCP "
                                       "hostname '%s', ignoring: %m",
                                       dhcpname);
            if (r == 1)
                log_link_notice(link,
                                "Overlong DCHP hostname received, shortened "
                                "from '%s' to '%s'",
                                dhcpname, hostname);
        }
        ...
        if (hostname)
        {
            ...
            if (r < 0)
                log_link_error_errno(
                    link, r, "Failed to set transient hostname to '%s': %m",
                    hostname);
        }
    }
...
static void dhcp4_handler(sd_dhcp_client* client, int event, void* userdata)
{
    ...
    switch (event)
    {
        ...
        default:
            if (event < 0)
                log_link_warning_errno(link, event,
...
static int dhcp4_set_hostname(Link* link)
{
    ...
    else
    {
        ...
        if (r < 0 &&
            r != -ENXIO) /* ENXIO: no hostname set or hostname is "localhost" */
            return r;
    ...
    if (r == -EINVAL && hostname)
        /* Ignore error when the machine's hostname is not suitable to send in
         * DHCP packet. */
        log_link_warning_errno(link, r,
                               "DHCP4 CLIENT: Failed to set hostname from "
                               "kernel hostname, ignoring: %m");
    else if (r < 0)
        return log_link_error_errno(link, r,
...
static bool promote_secondaries_enabled(const char* ifname)
{
    ...
    promote_secondaries_path =
        strjoina("net/ipv4/conf/", ifname, "/promote_secondaries");
    ...
    if (r < 0)
        log_warning_errno(r,
                          ...
                          promote_secondaries_path, promote_secondaries_sysctl);
    return r > 0;
}
...
int dhcp4_set_promote_secondaries(Link* link)
{
     ...
     */
    if (!(promote_secondaries_enabled("all") ||
          promote_secondaries_enabled(link->ifname)))
    {
        char* promote_secondaries_path = NULL;

        log_link_debug(link, "promote_secondaries is unset, setting it");
        promote_secondaries_path =
            strjoina("net/ipv4/conf/", link->ifname, "/promote_secondaries");
        r = sysctl_write(promote_secondaries_path, "1");
        if (r < 0)
            log_link_warning_errno(link, r, "cannot set sysctl %s to 1",
                                   promote_secondaries_path);
        return r > 0;
    }
    ...
    switch (link->network->dhcp_client_identifier)
    {
        case DHCP_CLIENT_ID_DUID:
        {
            ...
            if (duid->type == DUID_TYPE_LLT && duid->raw_data_len == 0)
                r = sd_dhcp_client_set_iaid_duid_llt(
                    link->dhcp_client, link->network->iaid_set,
                    link->network->iaid, duid->llt_time);
            else
            ...
            if (r < 0)
                return log_link_error_errno(
                    link, r, "DHCP4 CLIENT: Failed to set IAID+DUID: %m");
            break;
        }
        case DHCP_CLIENT_ID_DUID_ONLY:
        {
            ...
            if (duid->type == DUID_TYPE_LLT && duid->raw_data_len == 0)
                ...
                r = sd_dhcp_client_set_duid(
                    link->dhcp_client, duid->type,
                    duid->raw_data_len > 0 ? duid->raw_data : NULL,
                    duid->raw_data_len);
            if (r < 0)
                return log_link_error_errno(
                    link, r, "DHCP4 CLIENT: Failed to set DUID: %m");
            break;
        }
        case DHCP_CLIENT_ID_MAC:
            r = sd_dhcp_client_set_client_id(link->dhcp_client, ARPHRD_ETHER,
                                             (const uint8_t*)&link->mac,
                                             sizeof(link->mac));
            if (r < 0)
                return log_link_error_errno(
                    link, r, "DHCP4 CLIENT: Failed to set client ID: %m");
            break;
        default:
...
int dhcp4_configure(Link* link)
{
    ...
    if (r < 0)
        return log_link_error_errno(link, r,
                                    "DHCP4 CLIENT: Failed to attach event: %m");

    r = sd_dhcp_client_set_mac(link->dhcp_client, (const uint8_t*)&link->mac,
                               sizeof(link->mac), ARPHRD_ETHER);
    if (r < 0)
        return log_link_error_errno(
            link, r, "DHCP4 CLIENT: Failed to set MAC address: %m");
    ...
    if (link->network->dhcp_use_routes && !link->network->dhcp_anonymize)
    {
        ...
        if (r < 0)
            return log_link_error_errno(link, r,
                                        "DHCP4 CLIENT: Failed to set request "
                                        "flag for static route: %m");

        r = sd_dhcp_client_set_request_option(
            link->dhcp_client, SD_DHCP_OPTION_CLASSLESS_STATIC_ROUTE);
        if (r < 0)
            return log_link_error_errno(link, r,
                                        "DHCP4 CLIENT: Failed to set request "
                                        "flag for classless static route: %m");
    }
    ...
    {
        r = sd_dhcp_client_set_vendor_class_identifier(
            link->dhcp_client, link->network->dhcp_vendor_class_identifier);
        if (r < 0)
            return log_link_error_errno(
                link, r,
    ...
    {
        r = sd_dhcp_client_set_user_class(
            link->dhcp_client, (const char**)link->network->dhcp_user_class);
        if (r < 0)
            return log_link_error_errno(
                link, r, "DHCP4 CLIENT: Failed to set user class: %m");
    }

    if (link->network->dhcp_client_port)
    {
        r = sd_dhcp_client_set_client_port(link->dhcp_client,
                                           link->network->dhcp_client_port);
        if (r < 0)
            return log_link_error_errno(
                link, r, "DHCP4 CLIENT: Failed to set listen port: %m");
    }
