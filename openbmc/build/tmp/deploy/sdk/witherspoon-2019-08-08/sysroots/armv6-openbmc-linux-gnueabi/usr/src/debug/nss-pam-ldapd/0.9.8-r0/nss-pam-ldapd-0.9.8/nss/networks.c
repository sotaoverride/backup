...
static nss_status_t read_netent(TFILE* fp, struct netent* result, char* buffer,
...
nss_status_t NSS_NAME(getnetbyname_r)(const char* name, struct netent* result,
                                      ...
                                      int* h_errnop)
{
    NSS_GETONE(NSLCD_ACTION_NETWORK_BYNAME, WRITE_STRING(fp, name),
...
nss_status_t NSS_NAME(getnetbyaddr_r)(uint32_t addr, int UNUSED(af),
                                      ...
                                      size_t buflen, int* errnop, int* h_errnop)
{
    NSS_GETONE(NSLCD_ACTION_NETWORK_BYADDR, WRITE_ADDRESS(fp, addr),
               read_netent(fp, result, buffer, buflen, errnop, h_errnop));
}
...
static nss_backend_op_t networks_ops[] = {
    nss_ldap_destructor, networks_endnetent,    networks_setnetent,
    networks_getnetent,  networks_getnetbyname, networks_getnetbyaddr};
