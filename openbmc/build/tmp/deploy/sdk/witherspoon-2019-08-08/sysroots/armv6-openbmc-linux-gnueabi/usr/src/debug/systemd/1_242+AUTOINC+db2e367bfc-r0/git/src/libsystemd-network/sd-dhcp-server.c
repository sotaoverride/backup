...
/* configures the server's address and subnet, and optionally the pool's size
 * and offset into the subnet the whole pool must fit into the subnet, and may
 * not contain the first (any) nor last (broadcast) address moreover, the
 * server's own address may be in the pool, and is in that case reserved in
 * order not to accidentally hand it out */
int sd_dhcp_server_configure_pool(sd_dhcp_server* server,
                                  struct in_addr* address,
                                  unsigned char prefixlen, uint32_t offset,
                                  uint32_t size)
{
    ...
    /* 0 offset means we should set a default, we skip the first (subnet)
       address and take the next one */
    if (offset == 0)
        offset = 1;
    ...
    if (server->address != address->s_addr || server->netmask != netmask ||
        server->pool_size != size || server->pool_offset != offset)
    {
...
DEFINE_PRIVATE_HASH_OPS_WITH_VALUE_DESTRUCTOR(dhcp_lease_hash_ops, DHCPClientId,
                                              client_id_hash_func,
                                              client_id_compare_func, DHCPLease,
                                              dhcp_lease_free);
...
int sd_dhcp_server_new(sd_dhcp_server** ret, int ifindex)
{
    ...
    server->default_lease_time =
        DIV_ROUND_UP(DHCP_DEFAULT_LEASE_TIME_USEC, USEC_PER_SEC);
    server->max_lease_time =
        DIV_ROUND_UP(DHCP_MAX_LEASE_TIME_USEC, USEC_PER_SEC);
...
static int dhcp_server_send_udp(sd_dhcp_server* server, be32_t destination,
                                uint16_t destination_port, DHCPMessage* message,
                                size_t len)
{
...
int dhcp_server_send_packet(sd_dhcp_server* server, DHCPRequest* req,
                            DHCPPacket* packet, int type, size_t optoffset)
{
    ...
    if (destination != INADDR_ANY)
        return dhcp_server_send_udp(server, destination, destination_port,
                                    &packet->dhcp,
                                    sizeof(DHCPMessage) + optoffset);
    else if (requested_broadcast(req) || type == DHCP_NAK)
        return dhcp_server_send_udp(server, INADDR_BROADCAST, destination_port,
                                    &packet->dhcp,
                                    sizeof(DHCPMessage) + optoffset);
    else
                                  ...
                                  be32_t gateway, uint8_t chaddr[])
{
    ...
    r = dhcp_message_init(&packet->dhcp, BOOTREPLY, 0, DHCP_FORCERENEW,
                          ARPHRD_ETHER, DHCP_MIN_OPTIONS_SIZE, &optoffset);
    if (r < 0)
        return r;

    r = dhcp_option_append(&packet->dhcp, DHCP_MIN_OPTIONS_SIZE, &optoffset, 0,
                           SD_DHCP_OPTION_END, 0, NULL);
    if (r < 0)
        return r;
    ...
    r = dhcp_server_send_udp(server, address, DHCP_PORT_CLIENT, &packet->dhcp,
                             sizeof(DHCPMessage) + optoffset);
    if (r < 0)
        return r;
    ...
    switch (code)
    {
        ...
        case SD_DHCP_OPTION_MAXIMUM_MESSAGE_SIZE:
            ...
            if (len == 2 && unaligned_read_be16(option) >= sizeof(DHCPPacket))
                req->max_optlen =
                    unaligned_read_be16(option) - sizeof(DHCPPacket);
...
static int ensure_sane_request(sd_dhcp_server* server, DHCPRequest* req,
                               DHCPMessage* message)
{
    assert(req);
    assert(message);
...
static int get_pool_offset(sd_dhcp_server* server, be32_t requested_ip)
{
    ...
    if (be32toh(requested_ip) <
            (be32toh(server->subnet) | server->pool_offset) ||
        be32toh(requested_ip) >= (be32toh(server->subnet) |
...
#define HASH_KEY                                                               \
    SD_ID128_MAKE(0d, 1d, fe, bd, f1, 24, bd, b3, 47, f1, dd, 6e, 73, 21, 93,  \
                  30)
                               ...
                               size_t length)
{
    ...
    if (message->op != BOOTREQUEST || message->htype != ARPHRD_ETHER ||
    ...
    type =
        dhcp_option_parse(message, length, parse_request, req, &error_message);
    if (type < 0)
        return 0;
    ...
    switch (type)
    {
        ...
        case DHCP_DISCOVER:
        {
            ...
            else
            {
                ...
                /* even with no persistence of leases, we try to offer the same
                   client the same IP address. we do this by using the hash of
                   the client id as the offset into the pool of leases when
                ...
                for (i = 0; i < server->pool_size; i++)
                {
                    if (!server->bound_leases[next_offer])
                    {
                        address = server->subnet |
                                  htobe32(server->pool_offset + next_offer);
                        break;
                    }
            ...
            if (r < 0)
                /* this only fails on critical errors */
                return log_dhcp_server_errno(server, r,
        ...
        case DHCP_DECLINE:
            log_dhcp_server(server, "DECLINE (0x%x): %s",
                            be32toh(req->message->xid), strna(error_message));
                ...
                server->bound_leases[pool_offset] == existing_lease)
            {
                ...
                if (!existing_lease)
                {
                    ...
                    lease->client_id.data =
                        memdup(req->client_id.data, req->client_id.length);
                ...
                r = sd_event_now(server->event, clock_boottime_or_monotonic(),
                                 &time_now);
                if (r < 0)
                {
                    if (!existing_lease)
                ...
                else
                {
                    ...
                    hashmap_put(server->leases_by_client_id, &lease->client_id,
                                lease);
...
static int server_receive_message(sd_event_source* s, int fd, uint32_t revents,
                                  void* userdata)
{
    ...
    if (r < 0)
        log_dhcp_server_errno(server, r,
...
int sd_dhcp_server_start(sd_dhcp_server* server)
{
    ...
    r = dhcp_network_bind_udp_socket(server->ifindex, INADDR_ANY,
                                     DHCP_PORT_SERVER);
    if (r < 0)
    {
        sd_dhcp_server_stop(server);
    ...
    r = sd_event_add_io(server->event, &server->receive_message, server->fd,
                        EPOLLIN, server_receive_message, server);
    if (r < 0)
    {
        sd_dhcp_server_stop(server);
        return r;
...
int sd_dhcp_server_forcerenew(sd_dhcp_server* server)
{
    ...
    for (i = 0; i < server->pool_size; i++)
    {
        ...
        r = server_send_forcerenew(server, lease->address, lease->gateway,
                                   lease->chaddr);
        if (r < 0)
            return r;
...
int sd_dhcp_server_set_dns(sd_dhcp_server* server, const struct in_addr dns[],
                           unsigned n)
{
    assert_return(server, -EINVAL);
...
int sd_dhcp_server_set_ntp(sd_dhcp_server* server, const struct in_addr ntp[],
                           unsigned n)
{
    assert_return(server, -EINVAL);
