...
// is asigned some bits or some pointer value, but that's ok, because we don't
// want to default initialize parts that are not being updated.
...
//! in a raw pointer. The extra bits are stored in the lower bits of the
//! pointer.
