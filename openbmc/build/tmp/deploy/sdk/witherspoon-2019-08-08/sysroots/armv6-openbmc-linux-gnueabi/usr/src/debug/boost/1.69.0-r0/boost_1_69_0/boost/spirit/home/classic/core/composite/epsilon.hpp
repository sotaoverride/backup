    ...
    : unary<SubjectT, parser<empty_match_parser<SubjectT>>>
{
        ...
        parse(ScannerT const& scan) const
    {
        ...
        bool matches =
            this->subject().parse(scan.change_policies(policies_t(scan)));
        if (matches)
        {
...
struct epsilon_selector
{
    ...
    typedef
        typename mpl::if_<is_parser<subject_t>, empty_match_parser<subject_t>,
                          condition_parser<subject_t>>::type type;
};
} // namespace impl
