...
{
    GDate* d =
        g_new0(GDate, 1); /* happily, 0 is the invalid flag for everything. */
 ...
 */
gboolean g_date_valid(const GDate* d)
{
...
{
    /* No need to check the upper bound of @y, because #GDateYear is 16 bits
     * wide, just like #GDate.year. */
    return (
        (m > G_DATE_BAD_MONTH) && (m < 13) && (d > G_DATE_BAD_DAY) &&
        (y >
         G_DATE_BAD_YEAR) && /* must check before using g_date_is_leap_year */
        (d <= (g_date_is_leap_year(y) ? days_in_months[1][m]
                                      : days_in_months[0][m])));
}
 ...
 *   Jan 1, Year 1
 */
static void g_date_update_julian(const GDate* const_d)
{
    ...
    d->julian_days +=
        year >> 2; /* divides by 4, which divides original by 400 */
...
static void g_date_update_dmy(const GDate* const_d)
{
    GDate* d = (GDate*)const_d;
    GDateYear y;
    GDateMonth m;
    ...
    if (!g_date_valid_dmy(day, m, y))
        g_warning("OOPS julian: %u  computed dmy: %u %u %u", d->julian_days,
                  day, m, y);
#endif
 ...
 */
GDateMonth g_date_get_month(const GDate* d)
{
 ...
 */
GDateYear g_date_get_year(const GDate* d)
{
 ...
 */
GDateDay g_date_get_day(const GDate* d)
{
 ...
 */
guint g_date_get_day_of_year(const GDate* d)
{
    gint idx;
 ...
 */
guint g_date_get_monday_week_of_year(const GDate* d)
{
    GDateWeekday wd;
    guint day;
    GDate first;
 ...
 * Returns: week number
 */
guint g_date_get_sunday_week_of_year(const GDate* d)
{
    GDateWeekday wd;
    guint day;
    GDate first;
 ...
 * Since: 2.6
 **/
guint g_date_get_iso8601_week_of_year(const GDate* d)
{
    guint j, d4, L, d1, w;
 ...
 */
gint g_date_days_between(const GDate* d1, const GDate* d2)
{
 ...
 */
void g_date_clear(GDate* d, guint ndates)
{
    g_return_if_fail(d != NULL);
...
static GDateDMY dmy_order[3] = {G_DATE_DAY, G_DATE_MONTH, G_DATE_YEAR};
...
static inline gboolean update_month_match(gsize* longest, const gchar* haystack,
                                          const gchar* needle)
{
    gsize length;
...
static void g_date_fill_parse_tokens(const gchar* str, GDateParseTokens* pt)
{
    ...
    if (pt->num_ints < 3)
    {
        ...
        for (i = 1; i < 13; ++i)
        {
             ...
             */
            if (update_month_match(&longest, normalized,
                                   long_month_names_alternative[i]))
                pt->month = i;
            ...
            if (update_month_match(&longest, normalized,
                                   short_month_names_alternative[i]))
                pt->month = i;
        }
...
/* HOLDS: g_date_global_lock */
static void g_date_prepare_to_parse(const gchar* str, GDateParseTokens* pt)
{
    ...
    if ((current_locale == NULL) || (strcmp(locale, current_locale) != 0))
        recompute_localeinfo =
            TRUE; /* Uh, there used to be a reason for the temporary */
    ...
    if (recompute_localeinfo)
    {
        ...
        while (i < 13)
        {
            ...
            short_month_names[i] =
                g_utf8_normalize(casefold, -1, G_NORMALIZE_ALL);
            g_free(casefold);
        ...
        while (i < 13)
        {
            DEBUG_MSG(("  %s   %s", long_month_names_alternative[i],
                       short_month_names_alternative[i]));
            ++i;
        }
        if (using_twodigit_years)
        {
            DEBUG_MSG(("**Using twodigit years with cutoff year: %u",
                       twodigit_start_year));
        }
        {
            gchar* strings[3];
            ...
            DEBUG_MSG(
                ("**Order: %s, %s, %s", strings[0], strings[1], strings[2]));
 ...
 * capacity).
 */
void g_date_set_parse(GDate* d, const gchar* str)
{
    GDateParseTokens pt;
    ...
    /* y < 8000 is to catch 19998 style typos; the library is OK up to 65535 or
     * so */
 ...
 */
void g_date_set_month(GDate* d, GDateMonth m)
{
    g_return_if_fail(d != NULL);
 ...
 * day-month-year triplet is invalid, the date will be invalid.
 */
void g_date_set_day(GDate* d, GDateDay day)
{
    g_return_if_fail(d != NULL);
 ...
 */
void g_date_set_year(GDate* d, GDateYear y)
{
    g_return_if_fail(d != NULL);
 ...
 * set it.
 */
void g_date_set_dmy(GDate* d, GDateDay day, GDateMonth m, GDateYear y)
{
    g_return_if_fail(d != NULL);
 ...
 */
void g_date_set_julian(GDate* d, guint32 j)
{
    g_return_if_fail(d != NULL);
 ...
 */
gboolean g_date_is_first_of_month(const GDate* d)
{
 ...
 * Returns: %TRUE if the date is the last day of the month
 */
gboolean g_date_is_last_of_month(const GDate* d)
{
    gint idx;
 ...
 * The date must be valid.
 */
void g_date_add_days(GDate* d, guint ndays)
{
    g_return_if_fail(g_date_valid(d));
 ...
 * The date must be valid.
 */
void g_date_subtract_days(GDate* d, guint ndays)
{
    g_return_if_fail(g_date_valid(d));
 ...
 */
void g_date_add_months(GDate* d, guint nmonths)
{
    guint years, months;
    gint idx;
 ...
 */
void g_date_subtract_months(GDate* d, guint nmonths)
{
    guint years, months;
    gint idx;
 ...
 */
void g_date_add_years(GDate* d, guint nyears)
{
    g_return_if_fail(g_date_valid(d));
 ...
 * must be valid.
 */
void g_date_subtract_years(GDate* d, guint nyears)
{
    g_return_if_fail(g_date_valid(d));
 ...
 */
void g_date_to_struct_tm(const GDate* d, struct tm* tm)
{
    GDateWeekday day;
    ...
    tm->tm_year =
        ((int)d->year) - 1900; /* X/Open says tm_year can be negative */

    day = g_date_get_weekday(d);
    if (day == 7)
        day = 0; /* struct tm wants days since Sunday, so Sunday is 0 */
...
#ifdef G_OS_WIN32
static void append_month_name(GArray* result, LCID lcid, SYSTEMTIME* systemtime,
                              gboolean abbreviated, gboolean alternative)
{
    int n;
    WORD base;
    LPCWSTR lpFormat;
...
static gsize win32_strftime_helper(const GDate* d, const gchar* format,
                                   const struct tm* tm, gchar* s, gsize slen)
{
        ...
        if (c == '%')
        {
            ...
            switch (c)
            {
                case 'a':
                    ...
                    n = GetLocaleInfoW(lcid, LOCALE_SABBREVDAYNAME1 + k, NULL,
                                       0);
                    g_array_set_size(result, result->len + n);
                    GetLocaleInfoW(lcid, LOCALE_SABBREVDAYNAME1 + k,
                                   ((wchar_t*)result->data) + result->len - n,
                                   n);
                    g_array_set_size(result, result->len - 1);
                    break;
                case 'A':
                    ...
                    g_array_set_size(result, result->len + n);
                    GetLocaleInfoW(lcid, LOCALE_SDAYNAME1 + k,
                                   ((wchar_t*)result->data) + result->len - n,
                                   n);
                    g_array_set_size(result, result->len - 1);
                    break;
                ...
                case 'C':
                    g_array_append_vals(result,
                                        digits + systemtime.wYear / 1000, 1);
                    g_array_append_vals(
                        result, digits + (systemtime.wYear / 1000) % 10, 1);
                    break;
                case 'd':
                    g_array_append_vals(result, digits + systemtime.wDay / 10,
                                        1);
                    g_array_append_vals(result, digits + systemtime.wDay % 10,
                                        1);
                    break;
                case 'D':
                    g_array_append_vals(result, digits + systemtime.wMonth / 10,
                                        1);
                    g_array_append_vals(result, digits + systemtime.wMonth % 10,
                                        1);
                    g_array_append_vals(result, L"/", 1);
                    ...
                    g_array_append_vals(result, L"/", 1);
                    g_array_append_vals(
                        result, digits + (systemtime.wYear / 10) % 10, 1);
                    g_array_append_vals(result, digits + systemtime.wYear % 10,
                                        1);
                    break;
                case 'e':
                    if (systemtime.wDay >= 10)
                        g_array_append_vals(result,
                                            digits + systemtime.wDay / 10, 1);
                    else
                        g_array_append_vals(result, L" ", 1);
                    g_array_append_vals(result, digits + systemtime.wDay % 10,
                                        1);
...
#else
                    g_array_append_vals(result, digits + systemtime.wHour / 10,
                                        1);
                    g_array_append_vals(result, digits + systemtime.wHour % 10,
                                        1);
#endif
                ...
                case 'I':
                    ...
                    {
                        g_array_append_vals(
                            result, digits + (systemtime.wHour % 12) / 10, 1);
                        g_array_append_vals(
                            result, digits + (systemtime.wHour % 12) % 10, 1);
                    }
#endif
                ...
                case 'j':
                    g_array_append_vals(result,
                                        digits + (tm->tm_yday + 1) / 100, 1);
                    g_array_append_vals(
                        result, digits + ((tm->tm_yday + 1) / 10) % 10, 1);
                    g_array_append_vals(result, digits + (tm->tm_yday + 1) % 10,
                                        1);
                    break;
                case 'm':
                    g_array_append_vals(result, digits + systemtime.wMonth / 10,
...
#else
                    g_array_append_vals(result,
                                        digits + systemtime.wMinute / 10, 1);
                    g_array_append_vals(result,
                                        digits + systemtime.wMinute % 10, 1);
#endif
                    break;
                ...
                case 'r':
                    ...
                    g_array_append_vals(result, L":", 1);
                    g_array_append_vals(result,
                                        digits + systemtime.wSecond / 10, 1);
                    g_array_append_vals(result,
                                        digits + systemtime.wSecond % 10, 1);
                    g_array_append_vals(result, L" ", 1);
#endif
                ...
                case 'u':
                    if (systemtime.wDayOfWeek == 0)
                        ...
                        g_array_append_vals(result,
                                            digits + systemtime.wDayOfWeek, 1);
                    break;
                case 'U':
                ...
                case 'V':
                    ...
                    break;
                case 'w':
                    g_array_append_vals(result, digits + systemtime.wDayOfWeek,
                                        1);
                    break;
                case 'W':
                ...
                case 'Y':
                    g_array_append_vals(result,
                                        digits + systemtime.wYear / 1000, 1);
                    g_array_append_vals(
                        result, digits + (systemtime.wYear / 100) % 10, 1);
                    g_array_append_vals(
                        result, digits + (systemtime.wYear / 10) % 10, 1);
                    g_array_append_vals(result, digits + systemtime.wYear % 10,
                                        1);
                ...
                case 'Z':
                    ...
                    if (n == TIME_ZONE_ID_UNKNOWN)
                        ...
                        g_array_append_vals(result, tzinfo.StandardName,
                                            wcslen(tzinfo.StandardName));
                    else if (n == TIME_ZONE_ID_DAYLIGHT)
                        g_array_append_vals(result, tzinfo.DaylightName,
    ...
    convbuf = g_utf16_to_utf8((wchar_t*)result->data, result->len, NULL,
                              &convlen, NULL);
    g_array_free(result, TRUE);
...
gsize g_date_strftime(gchar* s, gsize slen, const gchar* format, const GDate* d)
{
    struct tm tm;
#ifndef G_OS_WIN32
    ...
    locale_format =
        g_locale_from_utf8(format, -1, NULL, &locale_format_len, &error);
    ...
    while (TRUE)
    {
        ...
        if (tmplen == 0 && tmpbuf[0] != '\0')
        {
            ...
            if (tmpbufsize > 65536)
            {
                g_warning(G_STRLOC "Maximum buffer size for g_date_strftime "
                                   "exceeded: giving up");
