...
/* Initialize a `struct traced_file`.  As input we need the name
   ...
   not.  */
static inline void init_traced_file(struct traced_file* file, const char* fname,
                                    int crinit)
{
    char* dname;
    file->mtime = 0;
...
/* nscd_conf.c */
extern int nscd_parse_file(const char* fname, struct database_dyn dbs[lastdb]);
...
/* cache.c */
extern struct datahead* cache_search(request_type, const void* key, size_t len,
...
extern void addhstbyaddr(struct database_dyn* db, int fd, request_header* req,
                         void* key, uid_t uid);
extern void addhstbynamev6(struct database_dyn* db, int fd, request_header* req,
                           void* key, uid_t uid);
extern void addhstbyaddrv6(struct database_dyn* db, int fd, request_header* req,
                           void* key, uid_t uid);
...
/* initgrcache.c */
extern void addinitgroups(struct database_dyn* db, int fd, request_header* req,
                          void* key, uid_t uid);
extern time_t readdinitgroups(struct database_dyn* db, struct hashentry* he,
                              struct datahead* dh);

/* servicecache.c */
extern void addservbyname(struct database_dyn* db, int fd, request_header* req,
                          void* key, uid_t uid);
extern time_t readdservbyname(struct database_dyn* db, struct hashentry* he,
                              struct datahead* dh);
extern void addservbyport(struct database_dyn* db, int fd, request_header* req,
                          void* key, uid_t uid);
...
extern time_t readdinnetgr(struct database_dyn* db, struct hashentry* he,
                           struct datahead* dh);
extern void addgetnetgrent(struct database_dyn* db, int fd, request_header* req,
                           void* key, uid_t uid);
extern time_t readdgetnetgrent(struct database_dyn* db, struct hashentry* he,
                               struct datahead* dh);

/* mem.c */
extern void* mempool_alloc(struct database_dyn* db, size_t len, int data_alloc);
