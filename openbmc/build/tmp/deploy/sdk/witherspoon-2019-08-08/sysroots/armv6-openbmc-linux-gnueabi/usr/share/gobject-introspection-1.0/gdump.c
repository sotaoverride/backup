...
static void escaped_printf(GOutputStream* out, const char* fmt, ...)
    G_GNUC_PRINTF(2, 3);
...
static void goutput_write(GOutputStream* out, const char* str)
{
    gsize written;
    GError* error = NULL;
    if (!g_output_stream_write_all(out, str, strlen(str), &written, NULL,
...
static GType invoke_get_type(GModule* self, const char* symbol, GError** error)
{
    GetTypeFunc sym;
    GType ret;
...
static GQuark invoke_error_quark(GModule* self, const char* symbol,
                                 GError** error)
{
    ErrorQuarkFunc sym;
...
static void dump_properties(GType type, GOutputStream* out)
{
    guint i;
    guint n_properties;
    GParamSpec** props;
    ...
    for (i = 0; i < n_properties; i++)
    {
        ...
        escaped_printf(out,
                       "    <property name=\"%s\" type=\"%s\" flags=\"%d\"/>\n",
                       prop->name, g_type_name(prop->value_type), prop->flags);
    }
    g_free(props);
}

static void dump_signals(GType type, GOutputStream* out)
{
    guint i;
    guint n_sigs;
    guint* sig_ids;
...
static void dump_object_type(GType type, const char* symbol, GOutputStream* out)
{
    guint n_interfaces;
    guint i;
    GType* interfaces;
...
static void dump_interface_type(GType type, const char* symbol,
                                GOutputStream* out)
{
    guint n_interfaces;
    guint i;
...
static void dump_boxed_type(GType type, const char* symbol, GOutputStream* out)
{
    escaped_printf(out, "  <boxed name=\"%s\" get-type=\"%s\"/>\n",
                   g_type_name(type), symbol);
}

static void dump_flags_type(GType type, const char* symbol, GOutputStream* out)
{
    guint i;
    GFlagsClass* klass;
...
static void dump_enum_type(GType type, const char* symbol, GOutputStream* out)
{
    guint i;
    GEnumClass* klass;
...
static void dump_fundamental_type(GType type, const char* symbol,
                                  GOutputStream* out)
{
    guint n_interfaces;
    guint i;
...
static void dump_type(GType type, const char* symbol, GOutputStream* out)
{
...
static void dump_error_quark(GQuark quark, const char* symbol,
                             GOutputStream* out)
{
...
static gboolean dump_irepository(const char* arg, GError** error) G_GNUC_UNUSED;
static gboolean dump_irepository(const char* arg, GError** error)
#else
...
#endif
{
    ...
    {
        g_set_error(error, G_IO_ERROR, G_IO_ERROR_FAILED,
                    "failed to open self: %s", g_module_error());
        return FALSE;
    }
