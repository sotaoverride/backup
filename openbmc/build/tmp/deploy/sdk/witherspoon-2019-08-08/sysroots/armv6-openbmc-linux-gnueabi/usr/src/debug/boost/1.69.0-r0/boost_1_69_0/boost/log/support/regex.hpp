...
//! This tag type is used if an expression is recognized as a Boost.Regex
//! expression
struct boost_regex_expression_tag;

//! The metafunction detects the matching expression kind and returns a tag that
//! is used to specialize \c match_traits
...
struct match_traits<ExpressionT, boost_regex_expression_tag>
{
    ...
    static bool matches(StringT const& str,
                        boost::basic_regex<CharT, TraitsT> const& expr,
    ...
    template <typename CharT, typename StringTraitsT, typename AllocatorT,
              typename ReTraitsT>
    static bool
        matches(std::basic_string<CharT, StringTraitsT, AllocatorT> const& str,
