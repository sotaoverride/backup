...
static krb5_error_code make_etype_list(krb5_context context,
                                       krb5_enctype* desired_etypes,
                                       krb5_enctype tkt_enctype,
                                       krb5_authdata*** authdata);

static krb5_error_code generate_authenticator(
    krb5_context, krb5_authenticator*, krb5_principal, krb5_checksum*, krb5_key,
    krb5_ui_4, krb5_authdata**, krb5_authdata_context ad_context,
    krb5_enctype* desired_etypes, krb5_enctype tkt_enctype);

/* Return the checksum type for the AP request, or 0 to use the enctype's
 * mandatory checksum. */
static krb5_cksumtype ap_req_cksum(krb5_context context,
                         ...
                         krb5_creds* in_creds, krb5_data* outbuf)
{
    ...
    {
        if ((retval =
                 krb5_generate_seq_number(context, &in_creds->keyblock,
    ...
    if (!in_data && (*auth_context)->checksum_func)
    {
        retval =
            (*auth_context)
                ->checksum_func(context, *auth_context,
                                (*auth_context)->checksum_func_data, &in_data);
        if (retval)
            goto cleanup;
    }
    ...
    if (in_data)
    {
        ...
        else
        {
            ...
            krb5_cksumtype cksumtype =
                ap_req_cksum(context, *auth_context, enctype);
            if ((retval = krb5_k_make_checksum(
                     context, cksumtype, (*auth_context)->key,
                     KRB5_KEYUSAGE_AP_REQ_AUTH_CKSUM, in_data, &checksum)))
                goto cleanup_cksum;
        }
        checksump = &checksum;
    }
    ...
    if (((*auth_context)->authentp =
             (krb5_authenticator*)malloc(sizeof(krb5_authenticator))) == NULL)
    {
        retval = ENOMEM;
        goto cleanup_cksum;
    }
    ...
    if (ap_req_options & AP_OPTS_ETYPE_NEGOTIATION)
    {
        ...
        {
            retval =
                krb5_get_tgs_ktypes(context, in_creds->server, &desired_etypes);
            if (retval)
                goto cleanup_cksum;
        }
        else
    ...
    if ((retval = generate_authenticator(
             context, (*auth_context)->authentp, in_creds->client, checksump,
             (*auth_context)->send_subkey, (*auth_context)->local_seq_number,
    ...
    if ((retval =
             encode_krb5_authenticator((*auth_context)->authentp, &scratch)))
        goto cleanup_cksum;

    /* call the encryption routine */
    if ((retval = krb5_encrypt_helper(context, &in_creds->keyblock,
                                      KRB5_KEYUSAGE_AP_REQ_AUTH, scratch,
...
cleanup:
    if (desired_etypes && desired_etypes != (*auth_context)->permitted_etypes)
        free(desired_etypes);
    if (request.ticket)
...
static krb5_error_code generate_authenticator(
    krb5_context context, krb5_authenticator* authent, krb5_principal client,
    krb5_checksum* cksum, krb5_key key, krb5_ui_4 seq_number,
    krb5_authdata** authorization, krb5_authdata_context ad_context,
    krb5_enctype* desired_etypes, krb5_enctype tkt_enctype)
{
    krb5_error_code retval;
    ...
    if (ad_context != NULL)
    {
        retval = krb5_authdata_export_authdata(context, ad_context,
                                               AD_USAGE_AP_REQ, &ext_authdata);
        if (retval)
            return retval;
    }
    ...
    {
        retval = krb5_merge_authdata(context, authorization, ext_authdata,
                                       ...
                                       krb5_authdata*** authdata)
{
    ...
    for (etypes.length = 0; etypes.etypes[etypes.length] != ENCTYPE_NULL;
         etypes.length++)
    {
        /*
         * RFC 4537:
