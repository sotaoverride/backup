...
static void internal_endent(FILE** stream)
{
    if (*stream != NULL)
    {
        fclose(*stream);
...
static enum nss_status
    internal_getent(FILE* stream, struct STRUCTURE* result, char* buffer,
                    size_t buflen, int* errnop H_ERRNO_PROTO EXTRA_ARGS_DECL)
{
    char* p;
    ...
    {
        ssize_t r =
            __libc_readline_unlocked(stream, data->linebuffer, linebuflen);
        if (r < 0)
        {
            *errnop = errno;
            H_ERRNO_SET(NETDB_INTERNAL);
...
enum nss_status CONCAT(_nss_files_get, ENTNAME_r)(struct STRUCTURE* result,
                                                  ...
                                                  int* errnop H_ERRNO_PROTO)
{
    ...
    if (status == NSS_STATUS_SUCCESS)
        status = internal_getent(stream, result, buffer, buflen,
...
#define DB_LOOKUP(name, db_char, keysize, keypattern, break_if_match,          \
                  proto...)                                                    \
    enum nss_status _nss_files_get##name##_r(proto, struct STRUCTURE* result,  \
                                             char* buffer, size_t buflen,      \
                                             int* errnop H_ERRNO_PROTO)        \
    {                                                                          \
        enum nss_status status;                                                \
        ...
        if (status == NSS_STATUS_SUCCESS)                                      \
        {                                                                      \
            while ((status = internal_getent(                                  \
                        stream, result, buffer, buflen,                        \
                        errnop H_ERRNO_ARG EXTRA_ARGS_VALUE)) ==               \
                   NSS_STATUS_SUCCESS)                                         \
            {                                                                  \
                break_if_match                                                 \
