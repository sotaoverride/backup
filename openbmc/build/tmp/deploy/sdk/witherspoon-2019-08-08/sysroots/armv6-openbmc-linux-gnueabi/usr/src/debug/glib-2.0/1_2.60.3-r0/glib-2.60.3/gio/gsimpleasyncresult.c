...
G_DEFINE_TYPE_WITH_CODE(
    GSimpleAsyncResult, g_simple_async_result, G_TYPE_OBJECT,
...
GSimpleAsyncResult* g_simple_async_result_new_from_error(
    GObject* source_object, GAsyncReadyCallback callback, gpointer user_data,
    const GError* error)
{
    GSimpleAsyncResult* simple;
    ...
    simple =
        g_simple_async_result_new(source_object, callback, user_data, NULL);
...
static gboolean g_simple_async_result_is_tagged(GAsyncResult* res,
                                                gpointer source_tag)
{
 ...
 **/
void g_simple_async_result_set_error(GSimpleAsyncResult* simple, GQuark domain,
...
void g_simple_async_result_complete(GSimpleAsyncResult* simple)
{
    ...
    if (simple->callback)
    {
        ...
        simple->callback(simple->source_object, G_ASYNC_RESULT(simple),
                         simple->user_data);
...
static gboolean run_in_thread(GIOSchedulerJob* job, GCancellable* c,
                              gpointer _data)
{
    RunInThreadData* data = _data;
    ...
    if (simple->handle_cancellation && g_cancellable_is_cancelled(c))
        ...
        data->func(simple, simple->source_object, c);
                                         ...
                                         GCancellable* cancellable)
{
    ...
    G_GNUC_BEGIN_IGNORE_DEPRECATIONS;
    g_io_scheduler_push_job(run_in_thread, data, NULL, io_priority,
                            cancellable);
 ...
 **/
gboolean g_simple_async_result_is_valid(GAsyncResult* result, GObject* source,
                                        gpointer source_tag)
{
    GSimpleAsyncResult* simple;
    GObject* cmp_source;
                                          ...
                                          const GError* error)
{
    ...
    simple = g_simple_async_result_new_from_error(object, callback, user_data,
                                                  error);
    g_simple_async_result_complete_in_idle(simple);
    g_object_unref(simple);
}
                                               ...
                                               GError* error)
{
    ...
    simple = g_simple_async_result_new_take_error(object, callback, user_data,
                                                  error);
    g_simple_async_result_complete_in_idle(simple);
    g_object_unref(simple);
}
...
{
    g_return_if_fail(G_IS_SIMPLE_ASYNC_RESULT(simple));
    g_return_if_fail(check_cancellable == NULL ||
