 ...
 * \date   07.11.2008
 *
 * The header contains facilities that are used in different sinks to map
 * attribute values used throughout the application to values used with the
 * specific native logging API. These tools are mostly needed to map application
 * severity levels on native levels, required by OS-specific sink backends.
 */
...
class basic_direct_mapping : public basic_mapping<MappedT>
{
    //! Base type
  ...
  public:
     ...
     * \param default_value The default native value that is returned if the
 ...
 *
 * The class allows to setup a custom mapping between an attribute and native
 * values. The mapping should be initialized similarly to the standard \c map
 * container, by using indexing operator and assignment.
 *
 * \note Unlike many other components of the library, exact type of the
 * attribute value must be specified in the template parameter \c
 * AttributeValueT. Type sequences are not supported.
 */
template <typename MappedT, typename AttributeValueT = int>
class basic_custom_mapping : public basic_mapping<MappedT>
{
    //! Base type
  ...
  private:
    ...
    class reference_proxy
    {
      ...
      public:
        //! Constructor
        reference_proxy(mapping_type& mapping,
                        attribute_value_type const& key) :
            m_Mapping(mapping),
            m_Key(key)
        {
        }
    ...
    struct visitor
    {
        ...
        visitor(mapping_type const& mapping, mapped_type& extracted) :
            m_Mapping(mapping), m_Extracted(extracted)
        {
        }
        template <typename T>
        void operator()(T const& val) const
  ...
  public:
    ...
    /*!
     * Extraction operator. Extracts the attribute value and attempts to map it
     * onto the native value.
     *
     * \param rec A log record to extract value from
     * \return A mapped value, if mapping was successful, or the default value
     * if mapping did not succeed.
     */
    mapped_type operator()(record_view const& rec) const
    {
        mapped_type res = m_DefaultValue;
     ...
     * \return An object of unspecified type that allows to insert a new mapping
     * through assignment. The \a key argument becomes the key attribute value,
     * and the assigned value becomes the mapped native value.
     */
