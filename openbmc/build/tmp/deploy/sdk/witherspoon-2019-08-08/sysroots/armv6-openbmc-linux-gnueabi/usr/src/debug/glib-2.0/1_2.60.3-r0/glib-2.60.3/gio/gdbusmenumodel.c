...
static void g_dbus_menu_group_changed(GDBusMenuGroup* group, guint menu_id,
                                      gint position, gint removed,
                                      GVariant* added);
static void g_dbus_menu_model_changed(GDBusMenuModel* proxy, GSequence* items,
...
static guint path_identifier_hash(gconstpointer data)
{
...
static gboolean path_identifier_equal(gconstpointer a, gconstpointer b)
{
...
static void path_identifier_free(PathIdentifier* id)
{
...
static PathIdentifier* path_identifier_new(ConstPathIdentifier* cid)
{
    PathIdentifier* id;
...
static GDBusMenuPath* g_dbus_menu_path_ref(GDBusMenuPath* path)
{
    path->ref_count++;
...
static void g_dbus_menu_path_unref(GDBusMenuPath* path)
{
    if (--path->ref_count == 0)
    {
...
static void g_dbus_menu_path_signal(GDBusConnection* connection,
                                    ...
                                    GVariant* parameters, gpointer user_data)
{
    ...
    while (g_variant_iter_loop(iter, "(uuuu@aa{sv})", &group_id, &menu_id,
                               &position, &removes, &adds))
    {
...
static void g_dbus_menu_path_activate(GDBusMenuPath* path)
{
    if (path->active++ == 0)
...
static void g_dbus_menu_path_deactivate(GDBusMenuPath* path)
{
    if (--path->active == 0)
        g_dbus_connection_signal_unsubscribe(path->id->connection,
                                             path->watch_id);
}

static GDBusMenuPath* g_dbus_menu_path_get(GMainContext* context,
                                           GDBusConnection* connection,
                                           const gchar* bus_name,
                                           const gchar* object_path)
{
    ...
    if (g_dbus_menu_paths == NULL)
        g_dbus_menu_paths =
            g_hash_table_new(path_identifier_hash, path_identifier_equal);
...
static GDBusMenuGroup* g_dbus_menu_group_ref(GDBusMenuGroup* group)
{
    group->ref_count++;
...
static void g_dbus_menu_group_unref(GDBusMenuGroup* group)
{
...
static void g_dbus_menu_model_item_free(gpointer data)
{
...
static GDBusMenuModelItem* g_dbus_menu_group_create_item(GVariant* description)
{
    ...
    item->attributes = g_hash_table_new_full(g_str_hash, g_str_equal, g_free,
                                             (GDestroyNotify)g_variant_unref);
    item->links = g_hash_table_new_full(g_str_hash, g_str_equal, g_free,
                                        (GDestroyNotify)g_variant_unref);
    ...
    while (g_variant_iter_loop(&iter, "{&sv}", &key, &value))
        if (key[0] == ':')
            /* key + 1 to skip the ':' */
            g_hash_table_insert(item->links, g_strdup(key + 1),
                                g_variant_ref(value));
        else
            g_hash_table_insert(item->attributes, g_strdup(key),
                                g_variant_ref(value));
 ...
 */
static void g_dbus_menu_group_go_offline(GDBusMenuGroup* group)
{
    g_dbus_menu_path_deactivate(group->path);
    g_dbus_connection_call(group->path->id->connection,
                           group->path->id->bus_name,
                           group->path->id->object_path, "org.gtk.Menus", "End",
                           g_variant_new_parsed("([ %u ],)", group->id), NULL,
                                          ...
                                          gpointer user_data)
{
    ...
    if (group->active)
    {
        ...
        if (reply)
        {
            ...
            while (g_variant_iter_loop(iter, "(uu@aa{sv})", &group_id, &menu_id,
                                       &items))
...
static void g_dbus_menu_group_activate(GDBusMenuGroup* group)
{
    if (group->active++ == 0)
    {
        ...
        if (group->state == GROUP_OFFLINE)
        {
            ...
            g_dbus_connection_call(
                group->path->id->connection, group->path->id->bus_name,
                group->path->id->object_path, "org.gtk.Menus", "Start",
                g_variant_new_parsed("([ %u ],)", group->id),
                G_VARIANT_TYPE("(a(uuaa{sv}))"), G_DBUS_CALL_FLAGS_NONE, -1,
                NULL, g_dbus_menu_group_start_ready,
...
static void g_dbus_menu_group_deactivate(GDBusMenuGroup* group)
{
    if (--group->active == 0)
    {
        g_assert(group->state != GROUP_OFFLINE);
                                                       ...
                                                       guint group_id)
{
    ...
    if (group == NULL)
    {
        ...
        group->menus = g_hash_table_new_full(NULL, NULL, NULL,
...
static GDBusMenuGroup* g_dbus_menu_group_get(GMainContext* context,
                                             GDBusConnection* connection,
                                             const gchar* bus_name,
                                             const gchar* object_path,
                                             guint group_id)
...
static gboolean g_dbus_menu_model_is_mutable(GMenuModel* model)
{
    return TRUE;
}

static gint g_dbus_menu_model_get_n_items(GMenuModel* model)
{
...
static void g_dbus_menu_model_get_item_attributes(GMenuModel* model,
                                                  gint item_index,
                                                  GHashTable** table)
{
    GDBusMenuModel* proxy = G_DBUS_MENU_MODEL(model);
...
static void g_dbus_menu_model_get_item_links(GMenuModel* model, gint item_index,
                                             GHashTable** table)
{
    GDBusMenuModel* proxy = G_DBUS_MENU_MODEL(model);
    GDBusMenuModelItem* item;
        ...
        while (g_hash_table_iter_next(&tmp, &key, &value))
        {
            if (g_variant_is_of_type(value, G_VARIANT_TYPE("(uu)")))
            {
                ...
                if (proxy->group->id != group_id)
                    group = g_dbus_menu_group_get_from_path(proxy->group->path,
                                                            group_id);
                else
...
static void g_dbus_menu_model_finalize(GObject* object)
{
    ...
    G_OBJECT_CLASS(g_dbus_menu_model_parent_class)->finalize(object);
}

static void g_dbus_menu_model_init(GDBusMenuModel* proxy)
{
}

static void g_dbus_menu_model_class_init(GDBusMenuModelClass* class)
{
                                      ...
                                      gint position, gint removed, gint added)
{
    ...
    if (proxy->active && (removed || added))
        g_menu_model_items_changed(G_MENU_MODEL(proxy), position, removed,
                                   added);
}
                                                        ...
                                                        guint menu_id)
{
    ...
    if (proxy == NULL)
    {
        ...
        proxy->items =
            g_hash_table_lookup(group->menus, GINT_TO_POINTER(menu_id));
                                      ...
                                      const gchar* object_path)
{
    ...
    g_return_val_if_fail(bus_name != NULL || g_dbus_connection_get_unique_name(
                                                 connection) == NULL,
                         NULL);
    ...
    group =
        g_dbus_menu_group_get(context, connection, bus_name, object_path, 0);
