...
/* dbus-file-unix.c unix related file implementation (internal to D-Bus
 * implementation)
 *
 ...
 */
dbus_bool_t _dbus_file_get_contents(DBusString* str, const DBusString* filename,
                                    DBusError* error)
{
    int fd;
    struct stat sb;
    ...
    else if (sb.st_size != 0)
    {
        ...
        dbus_set_error(error, DBUS_ERROR_FAILED, "\"%s\" is not a regular file",
                       filename_c);
        _dbus_close(fd, NULL);
        return FALSE;
    }
...
#define N_TMP_FILENAME_RANDOM_BYTES 8
    ...
    if (world_readable)
    {
         ...
         * see
         * http://lists.freedesktop.org/archives/dbus/2010-September/013367.html
         */
        if (fchmod(fd, 0644) < 0)
        {
            dbus_set_error(error, _dbus_error_from_errno(errno),
    ...
    {
        dbus_set_error(error, _dbus_error_from_errno(errno),
                       "Could not synchronize file %s: %s", tmp_filename_c,
                       _dbus_strerror(errno));
    ...
    {
        dbus_set_error(error, _dbus_error_from_errno(errno),
                       "Could not close file %s: %s", tmp_filename_c,
                       _dbus_strerror(errno));
    ...
    {
        dbus_set_error(error, _dbus_error_from_errno(errno),
                       "Could not rename %s to %s: %s", tmp_filename_c,
