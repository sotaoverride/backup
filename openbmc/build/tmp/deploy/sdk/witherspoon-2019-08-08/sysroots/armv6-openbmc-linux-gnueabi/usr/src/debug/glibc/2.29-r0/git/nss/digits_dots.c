                               ...
                               enum nss_status* status, int af, int* h_errnop)
{
    ...
    int ret = __nss_hostname_digits_dots_context(ctx, name, resbuf, buffer,
                                                 buffer_size, buflen, result,
                                                 status, af, h_errnop);
    ...
    if (isdigit(name[0]) || isxdigit(name[0]) || name[0] == ':')
    {
        ...
        if (isdigit(name[0]))
        {
            for (cp = name;; ++cp)
            {
                if (*cp == '\0')
                {
                    ...
                    if (af == AF_INET)
                        ok =
                            __inet_aton_exact(name, (struct in_addr*)host_addr);
                    else
                    {
                        assert(af == AF_INET6);
        ...
        if ((isxdigit(name[0]) && strchr(name, ':') != NULL) || name[0] == ':')
        {
            switch (af)
            {
                ...
                case AF_INET:
                    /* This is not possible.  We cannot represent an IPv6
                   address in an `struct in_addr' variable.  */
                    *h_errnop = HOST_NOT_FOUND;
                    if (buffer_size == NULL)
                        *status = NSS_STATUS_NOTFOUND;
                    else
