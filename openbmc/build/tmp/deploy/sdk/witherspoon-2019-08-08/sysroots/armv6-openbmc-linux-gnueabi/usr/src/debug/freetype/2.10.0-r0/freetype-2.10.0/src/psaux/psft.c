                               ...
                               const CF2_CallbackParams params)
{
    ...
    if (!builder->path_begun)
    {
        ...
        /* `path_begun'                                               */
        error = ps_builder_start_point(builder, params->pt0.x, params->pt0.y);
        if (error)
        {
            if (!*callbacks->error)
    ...
    error = ps_builder_add_point1(builder, params->pt1.x, params->pt1.y);
    if (error)
    {
        if (!*callbacks->error)
            *callbacks->error = error;
...
static void cf2_outline_init(CF2_Outline outline, FT_Memory memory,
                             FT_Error* error)
{
    FT_ZERO(outline);
...
static void cf2_getScaleAndHintFlag(PS_Decoder* decoder, CF2_Fixed* x_scale,
                                    CF2_Fixed* y_scale, FT_Bool* hinted,
                                    FT_Bool* scaled)
{
...
cf2_decoder_parse_charstrings(PS_Decoder* decoder, FT_Byte* charstring_base,
                              FT_ULong charstring_len)
{
    ...
    FT_ASSERT(decoder && (is_t1 || decoder->cff));
    ...
    if (!decoder->cf2_instance->data)
    {
        ...
        if (FT_ALLOC(decoder->cf2_instance->data, sizeof(CF2_FontRec)))
...

    {
        ...
        FT_ZERO(&buf);
        buf.start = buf.ptr = charstring_base;
        buf.end = charstring_base + charstring_len;

        FT_ZERO(&transform);

        cf2_getScaleAndHintFlag(decoder, &transform.a, &transform.d, &hinted,
                                &scaled);
        ...
        if (scaled && (!no_stem_darkening_font || (no_stem_darkening_font < 0 &&
...
FT_LOCAL_DEF(FT_Error)
cf2_getNormalizedVector(PS_Decoder* decoder, CF2_UInt* len, FT_Fixed** vec)
{
    TT_Face face;
...
{
    FT_ASSERT(decoder && decoder->builder.face && decoder->builder.face->size);
     ...
     *
     */
    return cf2_intToFixed(decoder->builder.face->size->metrics.y_ppem);
}
...
FT_LOCAL_DEF(void)
cf2_getBlueMetrics(PS_Decoder* decoder, CF2_Fixed* blueScale,
                   CF2_Fixed* blueShift, CF2_Fixed* blueFuzz)
{
    FT_ASSERT(decoder && decoder->current_subfont);

    *blueScale = FT_DivFix(decoder->current_subfont->private_dict.blue_scale,
                           cf2_intToFixed(1000));
    *blueShift =
        cf2_intToFixed(decoder->current_subfont->private_dict.blue_shift);
    *blueFuzz =
        cf2_intToFixed(decoder->current_subfont->private_dict.blue_fuzz);
}
...
cf2_getBlueValues(PS_Decoder* decoder, size_t* count, FT_Pos** data)
{
    ...
    *data = (FT_Pos*)&decoder->current_subfont->private_dict.blue_values;
}
...
cf2_getFamilyBlues(PS_Decoder* decoder, size_t* count, FT_Pos** data)
{
    ...
    *data = (FT_Pos*)&decoder->current_subfont->private_dict.family_blues;
}
...
cf2_initGlobalRegionBuffer(PS_Decoder* decoder, CF2_Int subrNum, CF2_Buffer buf)
{
    ...
    buf->start = buf->ptr = decoder->globals[idx];
...
cf2_getT1SeacComponent(PS_Decoder* decoder, FT_UInt glyph_index, CF2_Buffer buf)
{
    ...
    if (inc)
        error =
            inc->funcs->get_glyph_data(inc->object, glyph_index, &glyph_data);
    else
#endif
...
cf2_getDefaultWidthX(PS_Decoder* decoder)
{
    ...
    return cf2_intToFixed(decoder->current_subfont->private_dict.default_width);
}
...
cf2_getNominalWidthX(PS_Decoder* decoder)
{
    ...
    return cf2_intToFixed(decoder->current_subfont->private_dict.nominal_width);
}
