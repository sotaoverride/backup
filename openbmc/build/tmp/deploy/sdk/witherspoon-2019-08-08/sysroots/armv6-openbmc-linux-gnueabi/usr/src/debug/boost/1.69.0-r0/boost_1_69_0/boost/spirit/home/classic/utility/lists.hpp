...
struct list_parser : public parser<list_parser<ItemT, DelimT, EndT, CategoryT>>
{
    ...
    {
        return impl::list_parser_type<CategoryT>::parse(scan, *this, item,
                                                        delim, end);
    }
    ...
    : public list_parser<kleene_star<anychar_parser>, chlit<CharT>>
{
    ...
    // construct the list_parser_gen object as an list parser for comma
    // separated lists without item formatting.
    list_parser_gen() :
    ...
    // The following generator functions should be used under normal
    // circumstances. (the operator()(...) functions)
    ...
    list_parser<kleene_star<anychar_parser>, typename as_parser<DelimT>::type,
                no_list_endtoken,
    ...
    list_parser<typename as_parser<ItemT>::type,
                typename as_parser<DelimT>::type, no_list_endtoken,
        ...
        operator()(ItemT const& item_, DelimT const& delim_) const
    {
        ...
        return return_t(as_parser<ItemT>::convert(item_),
    ...
    list_parser<typename as_parser<ItemT>::type,
                ...
                typename as_parser<ItemT>::type::parser_category_t>
        operator()(ItemT const& item_, DelimT const& delim_,
    ...
    // The following functions should be used, if the 'item' parser has an
    // attached semantic action or is a unary_parser_category type parser and
    // the structure of the resulting list parser should _not_ be refactored
    // during parser construction (see comment above).
