...
static int vacuum_compare(const struct vacuum_info* a,
                          const struct vacuum_info* b)
{
    int r;
...
static void patch_realtime(int fd, const char* fn, const struct stat* st,
...
static int journal_file_empty(int dir_fd, const char* name)
{
    ...
    fd = openat(dir_fd, name,
                O_RDONLY | O_CLOEXEC | O_NOFOLLOW | O_NONBLOCK | O_NOATIME);
    if (fd < 0)
    {
...
int journal_directory_vacuum(const char* directory, uint64_t max_use,
                             uint64_t n_max_files, usec_t max_retention_usec,
                             usec_t* oldest_usec, bool verbose)
{
    ...
    if (max_retention_usec > 0)
        retention_limit =
            usec_sub_unsigned(now(CLOCK_REALTIME), max_retention_usec);
    ...
    FOREACH_DIRENT_ALL(de, d, r = -errno; goto finish)
    {
        ...
        {
            log_debug_errno(
                errno, "Failed to stat file %s while vacuuming, ignoring: %m",
                de->d_name);
            continue;
        }
        ...
        if (endswith(de->d_name, ".journal"))
        {
            ...
            if (sd_id128_from_string(de->d_name + q - 8 - 16 - 1 - 16 - 1 - 32,
                                     &seqnum_id) < 0)
            {
                n_active_files++;
                continue;
            ...
            if (sscanf(de->d_name + q - 8 - 16 - 1 - 16,
                       "%16llx-%16llx.journal", &seqnum, &realtime) != 2)
            {
                n_active_files++;
                continue;
            }
        ...
        else if (endswith(de->d_name, ".journal~"))
        {
            ...
            if (sscanf(de->d_name + q - 1 - 8 - 16 - 1 - 16,
                       "%16llx-%16llx.journal~", &realtime, &tmp) != 2)
            {
                n_active_files++;
                continue;
            }
        ...
        if (r > 0)
        {
            ...
            if (r >= 0)
            {
                ...
                log_warning_errno(
                    r, "Failed to delete empty archived journal %s/%s: %m",
                    directory, p);
    ...
    for (i = 0; i < n_list; i++)
    {
        ...
        if (r >= 0)
        {
            ...
            log_warning_errno(r, "Failed to delete archived journal %s/%s: %m",
...
finish:
    ...
    log_full(verbose ? LOG_INFO : LOG_DEBUG,
             ...
             format_bytes(sbytes, sizeof(sbytes), freed), directory);
