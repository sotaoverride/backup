...
static __u32 e2fsck_journal_sb_csum(journal_superblock_t* jsb)
{
    ...
    jsb->s_checksum = 0;
    crc =
        ext2fs_crc32c_le(~0, (unsigned char*)jsb, sizeof(journal_superblock_t));
...
int journal_bmap(journal_t* journal, blk64_t block, unsigned long long* phys)
{
    ...
    *phys = block;
    ...
    retval = ext2fs_bmap2(inode->i_ctx->fs, inode->i_ino, &inode->i_ext2, NULL,
                          0, block, 0, &pblk);
    *phys = pblk;
    return -1 * ((int)retval);
#endif
...
struct buffer_head* getblk(kdev_t kdev, blk64_t blocknr, int blocksize)
{
    ...
    int bufsize = sizeof(*bh) + kdev->k_ctx->fs->blocksize - sizeof(bh->b_data);
...
void ll_rw_block(int rw, int nr, struct buffer_head* bhp[])
{
    ...
    for (; nr > 0; --nr)
    {
        ...
        if (rw == READ && !bh->b_uptodate)
        {
            ...
            retval =
                io_channel_read_blk64(bh->b_io, bh->b_blocknr, 1, bh->b_data);
            if (retval)
            {
        ...
        else if (rw == WRITE && bh->b_dirty)
        {
            ...
            retval =
                io_channel_write_blk64(bh->b_io, bh->b_blocknr, 1, bh->b_data);
            if (retval)
            {
                com_err(bh->b_ctx->device_name, retval,
...
static errcode_t e2fsck_get_journal(e2fsck_t ctx, journal_t** ret_journal)
{
    ...
    if (uuid_is_null(sb->s_journal_uuid))
    {
        ...
        if ((retval = ext2fs_read_inode(ctx->fs, sb->s_journal_inum,
                                        &j_inode->i_ext2)))
        {
        try_backup_journal:
        ...
        pb.last_block = -1;
        retval = ext2fs_block_iterate3(ctx->fs, j_inode->i_ino, BLOCK_FLAG_HOLE,
                                       0, process_journal_block, &pb);
        ...
        journal->j_maxlen =
            EXT2_I_SIZE(&j_inode->i_ext2) / journal->j_blocksize;

#ifdef USE_INODE_IO
        retval = ext2fs_inode_io_intern2(ctx->fs, sb->s_journal_inum,
                                         &j_inode->i_ext2, &journal_name);
        if (retval)
            goto errout;
...
#endif
    {
        ...
        retval = io_ptr->open(journal_name, flags, &ctx->journal_io);
    }
    if (retval)
        goto errout;
    ...
    if (ext_journal)
    {
        ...
        if (ext2fs_has_feature_metadata_csum(&jsuper))
        {
            ...
            if (!ext2fs_superblock_csum_verify(&fsx, p) &&
                fix_problem(ctx, PR_0_EXT_JOURNAL_SUPER_CSUM_INVALID, &pctx))
            {
...
static errcode_t e2fsck_journal_load(journal_t* journal)
{
    ...
    switch (ntohl(jsb->s_header.h_blocktype))
    {
        case JFS_SUPERBLOCK_V1:
            ...
            if (jsb->s_feature_compat || jsb->s_feature_incompat ||
                jsb->s_feature_ro_compat || jsb->s_nr_users)
    ...
    if (journal_has_csum_v2or3(journal) && jfs_has_feature_checksum(journal))
        return EXT2_ET_CORRUPT_JOURNAL_SB;
    ...
    if (journal_has_csum_v2or3(journal))
        journal->j_csum_seed =
            jbd2_chksum(journal, ~0, jsb->s_uuid, sizeof(jsb->s_uuid));
...
errcode_t e2fsck_check_ext3_journal(e2fsck_t ctx)
{
    ...
    if (!ext2fs_has_feature_journal(sb) && !recover &&
        sb->s_journal_inum == 0 && sb->s_journal_dev == 0 &&
    ...
    if (retval)
    {
        if ((retval == EXT2_ET_CORRUPT_JOURNAL_SB) ||
            ((retval == EXT2_ET_UNSUPP_FEATURE) &&
             (!fix_problem(ctx, PR_0_JOURNAL_UNSUPP_INCOMPAT, &pctx))) ||
            ((retval == EXT2_ET_RO_UNSUPP_FEATURE) &&
             (!fix_problem(ctx, PR_0_JOURNAL_UNSUPP_ROCOMPAT, &pctx))) ||
            ((retval == EXT2_ET_JOURNAL_UNSUPP_VERSION) &&
             (!fix_problem(ctx, PR_0_JOURNAL_UNSUPP_VERSION, &pctx))))
            retval = e2fsck_journal_fix_corrupt_super(ctx, journal, &pctx);
...
no_has_journal:
    if (!ext2fs_has_feature_journal(sb))
    {
        ...
        {
            if (recover && !fix_problem(ctx, PR_0_JOURNAL_RECOVER_SET, &pctx))
                goto no_has_journal;
            /*
             * Need a full fsck if we are releasing a
             * journal stored on a reserved inode.
             */
            force_fsck = recover || (sb->s_journal_inum < EXT2_FIRST_INODE(sb));
        ...
        journal->j_superblock->s_start != 0)
    {
        ...
        else if (fix_problem(ctx, PR_0_JOURNAL_RESET_JOURNAL, &pctx))
        {
            reset = 1;
...
errcode_t e2fsck_run_ext3_journal(e2fsck_t ctx)
{
     ...
     */
    if (ctx->fs->super->s_kbytes_written && ctx->fs->io->manager->get_stats)
    ...
    retval = ext2fs_open(ctx->filesystem_name, ctx->openfs_flags,
                         ctx->superblock, blocksize, io_ptr, &ctx->fs);
    if (retval)
    {
        com_err(ctx->program_name, retval, _("while trying to re-open %s"),
                ctx->device_name);
        fatal_error(ctx, 0);
    }
 ...
 */
static const char* const journal_names[] = {".journal", "journal",
                                            ".journal.dat", "journal.dat", 0};
...
void e2fsck_move_ext3_journal(e2fsck_t ctx)
{
     ...
     */
    if ((ctx->options & E2F_OPT_READONLY) || (sb->s_journal_inum == 0) ||
         ...
         memcmp(inode.i_block, sb->s_jnl_blocks, EXT2_N_BLOCKS * 4)))
    {
        ...
        {
            memcpy(sb->s_jnl_blocks, inode.i_block, EXT2_N_BLOCKS * 4);
    ...
    {
        retval = ext2fs_lookup(fs, EXT2_ROOT_INO, *cpp, strlen(*cpp), 0, &ino);
    ...
    ext2fs_bg_free_inodes_count_set(fs, group,
                                    ext2fs_bg_free_inodes_count(fs, group) + 1);
