...
static inline int __attribute((always_inline)) decode(const void* ptr)
{
...
static inline int64_t __attribute((always_inline)) decode64(const void* ptr)
{
...
void __tzfile_read(const char* file, size_t extra, char** extrap)
{
    ...
    _Static_assert(sizeof(__time64_t) == 8, "__time64_t must be eight bytes");
    ...
    else
    {
       ...
       and which is not the system wide default TZDEFAULT.  */
        if (__libc_enable_secure &&
            ((*file == '/' && memcmp(file, TZDEFAULT, sizeof TZDEFAULT) &&
    ...
    struct stat64 st;
    if (was_using_tzfile && stat64(file, &st) == 0 && tzfile_ino == st.st_ino &&
        tzfile_dev == st.st_dev && tzfile_mtime == st.st_mtime)
...
read_again:
    if (__builtin_expect(
            __fread_unlocked((void*)&tzhead, sizeof(tzhead), 1, f) != 1, 0) ||
    ...
    if (trans_width == 4 && tzhead.tzh_version[0] != '\0')
    {
        ...
        size_t to_skip = (num_transitions * (4 + 1) + num_types * 6 + chars +
    ...
    if (__builtin_expect(
            num_types > (SIZE_MAX - total_size) / sizeof(struct ttinfo), 0))
        goto lose;
    total_size += num_types * sizeof(struct ttinfo);
    if (__glibc_unlikely(chars > SIZE_MAX - total_size))
        goto lose;
    total_size += chars;
    if (__builtin_expect(__alignof__(struct leap) - 1 > SIZE_MAX - total_size,
                         0))
        goto lose;
    ...
    if (__builtin_expect(
            num_leaps > (SIZE_MAX - total_size) / sizeof(struct leap), 0))
        goto lose;
    ...
    if (trans_width == 8)
    {
        ...
        if (__builtin_expect(rem < 0 ||
            ...
            goto lose;
        tzspec_len =
            (size_t)rem - (num_transitions * (8 + 1) + num_types * 6 + chars);
        if (__builtin_expect(
                num_leaps > SIZE_MAX / 12 || tzspec_len < num_leaps * 12, 0))
            goto lose;
    ...
    type_idxs =
        (unsigned char*)transitions + (num_transitions * sizeof(__time64_t));
    ...
    {
        if (__builtin_expect(__fread_unlocked(transitions, 4, num_transitions,
                                              f) != num_transitions,
                             0) ||
    ...
    for (i = 0; i < num_types; ++i)
    {
        unsigned char x[4];
        int c;
        if (__builtin_expect(__fread_unlocked(x, 1, sizeof(x), f) != sizeof(x),
                             0))
            goto lose;
        c = __getc_unlocked(f);
    ...
    for (i = 0; i < num_leaps; ++i)
    {
        unsigned char x[8];
        if (__builtin_expect(
                __fread_unlocked(x, 1, trans_width, f) != trans_width, 0))
            goto lose;
        if (trans_width == 4)
    ...
    if (tzspec != NULL)
    {
        ...
        if (__getc_unlocked(f) != '\n' ||
            (__fread_unlocked(tzspec, 1, tzspec_len - 1, f) != tzspec_len - 1))
            tzspec = NULL;
        else
...
void __tzfile_default(const char* std, const char* dst, long int stdoff,
                      long int dstoff)
{
...
void __tzfile_compute(__time64_t timer, int use_localtime,
                      long int* leap_correct, int* leap_hit, struct tm* tp)
{
    size_t i;
