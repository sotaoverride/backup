...
static int do_keyop(EVP_PKEY_CTX* ctx, int pkey_op, unsigned char* out,
                    size_t* poutlen, const unsigned char* in, size_t inlen);
...
int pkeyutl_main(int argc, char** argv)
{
    ...
    ctx = init_ctx(kdfalg, &keysize, inkey, keyform, key_type, passinarg,
                   pkey_op, e, engine_impl);
    if (ctx == NULL)
    {
    ...
    {
        rv = EVP_PKEY_verify(ctx, sig, (size_t)siglen, buf_in,
                             (size_t)buf_inlen);
        if (rv == 1)
        {
    ...
    if (rv > 0 && buf_outlen != 0)
    {
        ...
        rv = do_keyop(ctx, pkey_op, buf_out, (size_t*)&buf_outlen, buf_in,
                      (size_t)buf_inlen);
    }
    if (rv <= 0)
    {
