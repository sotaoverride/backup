...
{
    DEBUGMSGTL(
        ("trap", "freeing callback trap session (%p, %p)\n", sp, sp->sesp));
    snmp_close(sp->sesp);
    free(sp);
}

static void _trap_version_incr(int version)
{
    switch (version)
    {
#ifndef NETSNMP_DISABLE_SNMPV1
...
static void _trap_version_decr(int version)
{
    switch (version)
    {
#ifndef NETSNMP_DISABLE_SNMPV1
...
static void _dump_trap_stats(netsnmp_session* sess)
{
...
int netsnmp_add_notification_session(netsnmp_session* ss, int pdutype,
 ...
 */
void netsnmp_unregister_notification(const char* name, u_char len)
{
    if (snmp_callback_available(SNMP_CALLBACK_APPLICATION,
        ...
        SNMPERR_SUCCESS)
    {
        ...
        NETSNMP_LOGONCE(
            (LOG_WARNING, "netsnmp_unregister_notification not supported\n"));
}

int add_trap_session(netsnmp_session* ss, int pdutype, int confirm, int version)
{
    return netsnmp_add_notification_session(ss, pdutype, confirm, version, NULL,
                                            NULL, NULL);
}
...
int remove_trap_session(netsnmp_session* ss)
{
    ...
    while (sp)
    {
        if (sp->sesp == ss)
        {
             ...
             */
            DEBUGMSGTL(
                ("trap", "removing trap session (%p, %p)\n", sp, sp->sesp));
            free(sp);
            return 1;
        }
        prev = sp;
...
#if !defined(NETSNMP_DISABLE_SNMPV1) || !defined(NETSNMP_DISABLE_SNMPV2C)
netsnmp_session* netsnmp_create_v1v2_notification_session(
    const char* sink, const char* sinkport, const char* com, const char* src,
    int version, int pdutype, const char* name, const char* tag,
    const char* profile)
{
    netsnmp_transport* t;
    ...
    else
    {
        ...
        if ((NULL == client_addr) && ((0 == strcmp("localhost", sink)) ||
    ...
    if ((NULL == t) || ((sesp = snmp_add(&session, t, NULL, NULL)) == NULL))
    {
...
int create_trap_session_with_src(const char* sink, const char* sinkport,
                                 const char* com, const char* src, int version,
                                 int pdutype)
{
    void* ss = netsnmp_create_v1v2_notification_session(
        sink, sinkport, com, src, version, pdutype, NULL, NULL, NULL);
    return (ss != NULL);
}

int create_trap_session2(const char* sink, const char* sinkport, char* com,
...
int create_trap_session(char* sink, u_short sinkport, char* com, int version,
                        int pdutype)
{
    void* ss;
...
netsnmp_pdu* convert_v2pdu_to_v1(netsnmp_pdu* template_v2pdu)
{
    ...
    if (!template_v1pdu)
    {
        snmp_log(LOG_WARNING, "send_trap: failed to copy v1 template PDU\n");
        return NULL;
    }
     ...
     */
    if (!vblist || snmp_oid_compare(vblist->name, vblist->name_length,
    ...
    {
        snmp_log(LOG_WARNING, "send_trap: no v2 trapOID varbind to set from\n");
        snmp_free_pdu(template_v1pdu);
        return NULL;
    }
                          ...
                          trap_prefix, OID_LENGTH(trap_prefix)))
    {
        ...
        var = find_varbind_in_list(vblist, snmptrapenterprise_oid,
                                   snmptrapenterprise_oid_len);
        if (var)
        {
            template_v1pdu->enterprise_length = var->val_len / sizeof(oid);
            template_v1pdu->enterprise = snmp_duplicate_objid(
                var->val.objid, template_v1pdu->enterprise_length);
        }
        else
        {
    ...
    else
    {
        ...
        if (len <= 2)
        {
            snmp_log(LOG_WARNING, "send_trap: v2 trapOID too short (%d)\n",
                     (int)len);
            snmp_free_pdu(template_v1pdu);
            return NULL;
        }
    ...
    var = find_varbind_in_list(vblist, agentaddr_oid, agentaddr_oid_len);
    if (var)
    {
        memcpy(template_v1pdu->agent_addr, var->val.string, 4);
    }
 ...
 */
int netsnmp_build_trap_oid(netsnmp_pdu* pdu, oid* t_oid, size_t* t_oid_len)
{
...
netsnmp_pdu* convert_v1pdu_to_v2(netsnmp_pdu* template_v1pdu)
{
    ...
    if (!template_v2pdu)
    {
        snmp_log(LOG_WARNING, "send_trap: failed to copy v2 template PDU\n");
        return NULL;
    }
    ...
    if ((netsnmp_build_trap_oid(template_v1pdu, enterprise, &enterprise_len) !=
         SNMPERR_SUCCESS) ||
        !snmp_varlist_add_variable(&var, snmptrap_oid, snmptrap_oid_len,
    ...
    if (!snmp_varlist_add_variable(
            &var, sysuptime_oid, sysuptime_oid_len, ASN_TIMETICKS,
            (u_char*)&(template_v1pdu->time), sizeof(template_v1pdu->time)))
    {
        snmp_log(LOG_WARNING,
     ...
     */
    var = find_varbind_in_list(template_v2pdu->variables, agentaddr_oid,
                               agentaddr_oid_len);
    if (!var &&
        (template_v1pdu->agent_addr[0] || template_v1pdu->agent_addr[1] ||
         template_v1pdu->agent_addr[2] || template_v1pdu->agent_addr[3]))
    {
        if (!snmp_varlist_add_variable(
                &(template_v2pdu->variables), agentaddr_oid, agentaddr_oid_len,
                ASN_IPADDRESS, (u_char*)&(template_v1pdu->agent_addr),
                sizeof(template_v1pdu->agent_addr)))
            snmp_log(LOG_WARNING,
                     "send_trap: failed to append snmpTrapAddr varbind\n");
    }
    var = find_varbind_in_list(template_v2pdu->variables, community_oid,
                               community_oid_len);
    if (!var && template_v1pdu->community)
    {
        if (!snmp_varlist_add_variable(&(template_v2pdu->variables),
                                       community_oid, community_oid_len,
                                       ASN_OCTET_STR, template_v1pdu->community,
                                       template_v1pdu->community_len))
            snmp_log(LOG_WARNING,
                     "send_trap: failed to append snmpTrapCommunity varbind\n");
    }
    var =
        find_varbind_in_list(template_v2pdu->variables, snmptrapenterprise_oid,
                             snmptrapenterprise_oid_len);
    if (!var)
    {
        if (!snmp_varlist_add_variable(
                &(template_v2pdu->variables), snmptrapenterprise_oid,
                snmptrapenterprise_oid_len, ASN_OBJECT_ID,
 ...
 * @see send_v2trap
 */
int netsnmp_send_traps(int trap, int specific, const oid* enterprise,
                       ...
                       const char* context, int flags)
{
    ...
    if (vars)
    {
        ...
        if (!vblist)
        {
            snmp_log(LOG_WARNING, "send_trap: failed to clone varbind list\n");
            return -1;
        }
    }
    ...
    if (trap == -1)
    {
         ...
         */
        if (!snmp_oid_compare(vblist->name, vblist->name_length, sysuptime_oid,
                              sysuptime_oid_len))
        {
        ...
        else
        {
            ...
            var = NULL;
            snmp_varlist_add_variable(&var, sysuptime_oid, sysuptime_oid_len,
                                      ASN_TIMETICKS, (u_char*)&uptime,
                                      sizeof(uptime));
            if (!var)
            {
                snmp_log(LOG_WARNING,
         ...
         */
        if (!trap_vb || snmp_oid_compare(trap_vb->name, trap_vb->name_length,
                                         snmptrap_oid, snmptrap_oid_len))
        {
            snmp_log(LOG_WARNING,
                              ...
                              trap_prefix, OID_LENGTH(trap_prefix)))
        {
            ...
            if (!var &&
                !snmp_varlist_add_variable(
                    &(template_v2pdu->variables), snmptrapenterprise_oid,
                    snmptrapenterprise_oid_len, ASN_OBJECT_ID, enterprise,
    ...
    else
    {
        ...
        if (snmp_clone_mem((void**)&template_v1pdu->enterprise, enterprise,
                           enterprise_length * sizeof(oid)))
        {
            snmp_log(LOG_WARNING,
    ...
    if (template_v1pdu)
    {
        ...
        {
            /* "v1trapaddress" was not specified in config or the resolution
             * failed, try any local address */
...
void send_enterprise_trap_vars(int trap, int specific, const oid* enterprise,
                               int enterprise_length,
                               netsnmp_variable_list* vars)
{
    netsnmp_send_traps(trap, specific, enterprise, enterprise_length, vars,
                       NULL, 0);
    return;
}
 ...
 */
int handle_trap_callback(int op, netsnmp_session* session, int reqid,
                         netsnmp_pdu* pdu, void* magic)
{
    ...
    switch (op)
    {
        ...
        case NETSNMP_CALLBACK_OP_SEND_FAILED:
            DEBUGMSGTL(
                ("trap", "failed to send an inform for reqid=%d\n", reqid));
#ifndef NETSNMP_NO_TRAP_STATS
            if (session->trap_stats)
            {
                session->trap_stats->sent_last_fail =
        ...
        case NETSNMP_CALLBACK_OP_SEC_ERROR:
            DEBUGMSGTL(
                ("trap", "sec error sending a trap for reqid=%d\n", reqid));
#ifndef NETSNMP_NO_TRAP_STATS
            if (session->trap_stats)
            {
 ...
 */
int handle_inform_response(int op, netsnmp_session* session, int reqid,
                           netsnmp_pdu* pdu, void* magic)
{
    if (NULL == session)
        return 0;
    ...
    switch (op)
    {
        ...
        case NETSNMP_CALLBACK_OP_RECEIVED_MESSAGE:
            ...
            {
                DEBUGMSGTL(("trap",
                            "received the inform response for reqid=%d\n",
                            reqid));
#ifndef NETSNMP_NO_TRAP_STATS
                if (session->trap_stats)
                {
                    ++session->trap_stats->ack_count;
                    session->trap_stats->ack_last_rcvd =
                        netsnmp_get_agent_uptime();
                }
#endif /* NETSNMP_NO_TRAP_STATS */
                break;
        ...
        case NETSNMP_CALLBACK_OP_SEC_ERROR:
            DEBUGMSGTL(
                ("trap", "sec error sending an inform for reqid=%d\n", reqid));
#ifndef NETSNMP_NO_TRAP_STATS
            if (session->trap_stats)
            {
                session->trap_stats->sec_err_last = netsnmp_get_agent_uptime();
...
#ifndef NETSNMP_NO_TRAP_STATS
            if (session->trap_stats)
                session->trap_stats->sent_last_sent =
                    netsnmp_get_agent_uptime();
#endif /* NETSNMP_NO_TRAP_STATS */
            break;
        ...
        default:
            DEBUGMSGTL(
                ("trap",
                 "received op=%d for reqid=%d when trying to send an inform\n",
                 op, reqid));
    }
...
void send_trap_to_sess(netsnmp_session* sess, netsnmp_pdu* template_pdu)
{
    ...
    {
        DEBUGMSGTL(("trap",
                    "not sending trap type=%d, version %02lx disabled\n",
    ...
    {
        result = snmp_async_send(sess, pdu, &handle_inform_response, NULL);
    }
    else
    {
                                 ...
                                 const char* context)
{
    if (trap == SNMP_TRAP_ENTERPRISESPECIFIC)
        netsnmp_send_traps(trap, specific, objid_enterprisetrap,
                           OID_LENGTH(objid_enterprisetrap), vars, context, 0);
    else
        netsnmp_send_traps(trap, specific, trap_version_id,
                           OID_LENGTH(trap_version_id), vars, context, 0);
}
...
{
    netsnmp_send_traps(-1, -1, trap_version_id, OID_LENGTH(trap_version_id),
                       vars, context, 0);
}
    ...
    else
    {
        if (strcmp(token, "pauthtrapenable") == 0)
        {
            if (snmp_enableauthentrapsset < 0)
            {
                 ...
                 * read-only.
                 */
                snmp_log(LOG_WARNING, "ignoring attempted override of "
                                      "read-only snmpEnableAuthenTraps.0\n");
                return;
            }
            else
            {
...
#if !defined(NETSNMP_DISABLE_SNMPV1) || !defined(NETSNMP_DISABLE_SNMPV2C)
static void _parse_config_sink(const char* token, char* cptr, int version,
                               int type)
{
    ...
    if (netsnmp_create_v1v2_notification_session(
            sp, pp, cp ? cp : snmp_trapcommunity, src, version, type, name, tag,
            profile) == NULL)
    {
...
netsnmp_session* netsnmp_create_v3user_notification_session(
    const char* dest, const char* user, int level, const char* context,
    int pdutype, const u_char* engineId, size_t engineId_len, const char* src,
    const char* notif_name, const char* notif_tag, const char* notif_profile)
{
                               ...
                               NETSNMP_DS_LIB_DISABLE_V3))
    {
        netsnmp_config_error(
            "SNMPv3 disabled, cannot create notification session");
        return NULL;
    }
    ...
    if (NULL != usmUser->authProtocol)
    {
        session.securityAuthProto = snmp_duplicate_objid(
            usmUser->authProtocol, usmUser->authProtocolLen);
    ...
    if (NULL != usmUser->privProtocol)
    {
        session.securityPrivProto = snmp_duplicate_objid(
            usmUser->privProtocol, usmUser->privProtocolLen);
    ...
    session.contextEngineID =
        netsnmp_memdup(usmUser->engineID, usmUser->engineIDLen);
    ...
    if (netsnmp_add_notification_session(
            ss, pdutype, (pdutype == SNMP_MSG_INFORM), ss->version, notif_name,
            notif_tag, notif_profile) != 1)
    {
...
void snmpd_parse_config_trapsess(const char* word, char* cptr)
{
    ...
    if (!ss)
    {
        config_perror("snmpd: failed to parse this line or the remote trap "
     ...
     */
    if (ss->version == SNMP_VERSION_3 && traptype != SNMP_MSG_INFORM &&
    ...
    netsnmp_add_notification_session(ss, traptype,
                                     (traptype == SNMP_MSG_INFORM), ss->version,
                                     name, tag, profile);
