...
Config::Config()
{
    ...
    if (stat(name, &cstat) == -1)
        return;
    if (cstat.st_size > MAX_CONFIG_FILE_SIZE)
        return;
...
void Config::getOption(const s8* key, bool& val)
{
    ...
    if (!strcmp(entry->val, "yes"))
        val = true;
    else if (!strcmp(entry->val, "no"))
        val = false;
}
...
void Config::checkConfigFile(const s8* name)
{
    static const s8 defaultConfig[] =
        ...
        "# Note that end-of-line comments are NOT supported, comments must be "
        "on a line of their own.\n"
        "\n\n"
        "# font family names/pixelsize used by fbterm, multiple font family "
        "names must be seperated by ','\n"
        "# and using a fixed width font as the first is strongly recommended\n"
        "font-names=mono\n"
        "font-size=12\n"
        "\n"
        "# force font width (and/or height), usually for non-fixed width "
        "fonts\n"
        "# legal value format: n (fw_new = n), +n (fw_new = fw_old + n), -n "
        "(fw_new = fw_old - n)\n"
        "#font-width=\n"
        "#font-height=\n"
        "\n"
        "# default color of foreground/background text\n"
        "# available colors: 0 = black, 1 = red, 2 = green, 3 = brown, 4 = "
        "blue, 5 = magenta, 6 = cyan, 7 = white\n"
        "color-foreground=7\n"
        "color-background=0\n"
        "\n"
        "# max scroll-back history lines of every window, value must be [0 - "
        "65535], 0 means disable it\n"
        "history-lines=1000\n"
        "\n"
        "# up to 5 additional text encodings, multiple encodings must be "
        "seperated by ','\n"
        ...
        "cursor-interval=500\n"
        "\n"
        "# additional ascii chars considered as part of a word while "
        "auto-selecting text, except ' ', 0-9, a-z, A-Z\n"
        "word-chars=._-\n"
        "\n"
        "# change the clockwise orientation angle of screen display\n"
        "# available values: 0 = 0 degree, 1 = 90 degrees, 2 = 180 degrees, 3 "
        "= 270 degrees\n"
        "screen-rotate=0\n"
        "\n"
...
bool Config::parseArgs(s32 argc, s8** argv)
{
    ...
    s32 index;
    while ((index = getopt_long(argc, argv, "Vvhn:s:f:b:e:r:i:m:a", options,
                                0)) != -1)
    {
        switch (index)
        {
            ...
            case '?':
                printf(
                    ...
                    "A fast framebuffer/vesa based terminal emulator for "
                    "linux\n"
                    "\n"
                    "  -h, --help                      display this help and "
                    "exit\n"
                    "  -V, --version                   display FbTerm version "
                    "and exit\n"
                    "  -v, --verbose                   display extra "
                    "information\n"
                    "  -n, --font-names=TEXT           specify font family "
                    "names\n"
                    "  -s, --font-size=NUM             specify font pixel "
                    "size\n"
                    "      --font-width=NUM            force font width\n"
                    "      --font-height=NUM           force font height\n"
                    "  -f, --color-foreground=NUM      specify foreground "
                    "color\n"
                    "  -b, --color-background=NUM      specify background "
                    "color\n"
                    "  -e, --text-encodings=TEXT       specify additional text "
                    "encodings\n"
                    "  -r, --screen-rotate=NUM         specify orientation of "
                    "screen display\n"
                    "  -a, --ambiguous-wide            treat ambiguous width "
                    "characters as wide\n"
                    "  -i, --input-method=TEXT         specify input method "
                    "program\n"
                    "  -m, --shared-mem=DESC           use shared-memory "
                    "framebuffer DESC\n"
                    "      --cursor-shape=NUM          specify default cursor "
                    "shape\n"
                    "      --cursor-interval=NUM       specify cursor flash "
                    "interval\n"
#ifdef ENABLE_VESA
                    "      --vesa-mode=NUM             force VESA video mode\n"
                    "                  list            display available VESA "
                    "video modes\n"
#endif
                    "\n"
                    "See comments in ~/.fbtermrc for details of these "
                    "options.\n");
                return false;
