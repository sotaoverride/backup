...
int mac_smack_apply(const char* path, SmackAttr attr, const char* label)
{
    ...
    if (label)
        r = lsetxattr(path, smack_attr_to_string(attr), label, strlen(label),
                      0);
    else
...
static int smack_fix_fd(int fd, const char* abspath, LabelFixFlags flags)
{
    ...
    if (setxattr(procfs_path, "security.SMACK64", label, strlen(label), 0) < 0)
    {
         ...
         * error */
        if (getxattr_malloc(procfs_path, "security.SMACK64", &old_label,
                            false) >= 0 &&
            streq(old_label, label))
            return 0;

        return log_debug_errno(r, "Unable to fix SMACK label of %s: %m",
                               abspath);
    }
