...
static errcode_t ind_punch(ext2_filsys fs, struct ext2_inode* inode,
                           char* block_buf, blk_t* p, int level, blk64_t start,
                           blk64_t count, int max)
{
    errcode_t retval;
    blk_t b;
    int i;
    ...
    for (i = 0, offset = 0; i < max; i++, p++, offset += incr)
    {
        ...
        if (level > 0)
        {
            ...
            retval = ind_punch(fs, inode, block_buf + fs->blocksize,
                               (blk_t*)block_buf, level - 1, start2,
                               count - offset, fs->blocksize >> 2);
            if (retval)
                return retval;
                                  ...
                                  char* block_buf, blk64_t start, blk64_t end)
{
    ...
    for (level = 0; level < 4; level++, max *= (blk64_t)addr_per_block)
    {
        ...
        {
            retval =
                ind_punch(fs, inode, block_buf, bp, level, start, count, num);
            if (retval)
                goto errout;
            if (count > max)
                count -= max - start;
                                     ...
                                     __u32 free_count, int* freed)
{
    ...
    {
        retval = ext2fs_map_cluster_block(fs, ino, inode, lfree_start, &pblk);
        if (retval)
            goto errout;
        if (!pblk)
        {
        ...
        cluster_freed =
            EXT2FS_CLUSTER_RATIO(fs) - (free_start & EXT2FS_CLUSTER_MASK(fs));
...
static errcode_t ext2fs_punch_extent(ext2_filsys fs, ext2_ino_t ino,
                                     struct ext2_inode* inode, blk64_t start,
                                     blk64_t end)
{
    ...
    while (1)
    {
        ...
        else
        {
            ...
            newex.e_pblk = extent.e_pblk + (end + 1 - extent.e_lblk);
            newex.e_lblk = end + 1;
            ...
            retval =
                ext2fs_extent_insert(handle, EXT2_EXTENT_INSERT_AFTER, &newex);
            if (retval)
                goto errout;
        ...
        else
        {
             ...
             */
            retval = ext2fs_extent_get(handle, EXT2_EXTENT_CURRENT, &newex);
            if (retval)
                goto errout;
            old_lblk = newex.e_lblk;

            retval = ext2fs_extent_get(handle, EXT2_EXTENT_NEXT_LEAF, &newex);
        ...
        dbg_printf("Free start %llu, free count = %u\n", free_start,
                   free_count);
        retval = punch_extent_blocks(fs, ino, inode, lfree_start, free_start,
                                     free_count, &freed);
        if (retval)
            goto errout;
    next_extent:
 ...
 */
errcode_t ext2fs_punch(ext2_filsys fs, ext2_ino_t ino, struct ext2_inode* inode,
                       char* block_buf, blk64_t start, blk64_t end)
{
    errcode_t retval;
