...
template <typename Traits, typename ICase,
          typename CharSet = compound_charset<Traits>>
...
typedef proto::expr<proto::tag::terminal,
                    proto::term<logical_newline_placeholder>, 0>
    logical_newline_xpression;

struct set_initializer;

typedef proto::expr<proto::tag::terminal, proto::term<set_initializer>, 0>
...
void set_char(compound_charset<Traits>& chset, Char ch, Traits const& tr,
              bool icase);

template <typename Char, typename Traits>
void set_range(compound_charset<Traits>& chset, Char from, Char to,
               Traits const& tr, bool icase);

template <typename Traits>
void set_class(compound_charset<Traits>& chset,
               typename Traits::char_class_type char_class, bool no,
               Traits const& tr);
...
template <typename Char, typename Traits>
void set_range(basic_chset<Char>& chset, Char from, Char to, Traits const& tr,
               bool icase);

template <typename Char, typename Traits>
void set_class(basic_chset<Char>& chset,
               typename Traits::char_class_type char_class, bool no,
               Traits const& tr);
...
template <typename Matcher, typename Next>
static_xpression<Matcher, Next> const make_static(Matcher const& matcher,
                                                  Next const& next);

int get_mark_number(basic_mark_tag const&);

template <typename Xpr, typename BidiIter>
void static_compile(Xpr const& xpr,
...
typename Traits::char_class_type lookup_classname(Traits const& traits,
                                                  char const (&cname)[N],
                                                  bool icase = false);
