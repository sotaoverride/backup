...
void CRYPTO_mem_debug_free(void* addr, int before_p, const char* file, int line)
{
    MEM m, *mp;
...
static void print_leak(const MEM* m, MEM_LEAK* l)
{
    ...
    n = BIO_snprintf(bufp, len, "[%02d:%02d:%02d] ", lcl->tm_hour, lcl->tm_min,
                     lcl->tm_sec);
    if (n <= 0)
    {
        bufp[0] = '\0';
    ...
    n = BIO_snprintf(bufp, len, "%5lu file=%s, line=%d, ", m->order, m->file,
                     m->line);
    if (n <= 0)
        return;
    bufp += n;
    ...
    if (amip)
    {
        ...
        do
        {
            ...
            n = BIO_snprintf(buf + ami_cnt, sizeof(buf) - ami_cnt,
                             " thread=%lu, file=%s, line=%d, info=\"", tid.ltid,
                             amip->file, amip->line);
            if (n <= 0)
                break;
            buf_len = ami_cnt + n;
