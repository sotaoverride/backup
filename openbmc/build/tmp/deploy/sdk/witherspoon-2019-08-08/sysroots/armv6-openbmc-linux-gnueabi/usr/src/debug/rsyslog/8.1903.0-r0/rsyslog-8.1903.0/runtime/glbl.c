...
/* static data */
DEFobjStaticHelpers DEFobjCurrIf(prop) DEFobjCurrIf(net)

    /* static data
     * For this object, these variables are obviously what makes the "meat" of
     * the class...
     */
...
static struct cnfobj* mainqCnfObj =
    NULL; /* main queue object, to be used later in startup sequence */
int bProcessInternalMessages =
    0; /* Should rsyslog itself process internal messages?
        * 1 - yes
...
static int bParseHOSTNAMEandTAG =
    1; /* parser modification (based on startup params!) */
static int bPreserveFQDN = 0; /* should FQDNs always be preserved? */
static int iMaxLine = 8096;   /* maximum length of a syslog message */
static uchar* oversizeMsgErrorFile =
    NULL; /* File where oversize messages are written to */
static int oversizeMsgInputMode =
    0; /* Mode which oversize messages will be forwarded */
static int reportOversizeMsg =
    1; /* shall error messages be generated for oversize messages? */
...
static int bDropMalPTRMsgs =
    0; /* Drop messages which have malicious PTR records during DNS lookup */
static int option_DisallowWarning =
    1; /* complain if message from disallowed sender is received */
static int bDisableDNS = 0; /* don't look up IP addresses of remote messages */
static prop_t* propLocalIPIF =
    NULL; /* IP address to report for the local host (default is 127.0.0.1) */
static int propLocalIPIF_set = 0; /* is propLocalIPIF already set? */
static prop_t* propLocalHostName =
    NULL; /* our hostname as FQDN - read-only after startup */
static prop_t* propLocalHostNameToDelete =
    NULL; /* see GenerateLocalHostName function hdr comment! */
static uchar* LocalHostName =
    NULL; /* our hostname  - read-only after startup, except HUP */
static uchar* LocalHostNameOverride =
    NULL; /* user-overridden hostname - read-only after startup */
static uchar* LocalFQDNName =
    NULL; /* our hostname as FQDN - read-only after startup, except HUP */
static uchar* LocalDomain =
    NULL; /* our local domain name  - read-only after startup, except HUP */
static char** StripDomains = NULL;
/* these domains may be stripped before writing logs  - r/o after s.u., never
 * touched by init */
static char** LocalHosts = NULL;
/* these hosts are logged with their hostname  - read-only after startup, never
 * touched by init */
static uchar* pszDfltNetstrmDrvr =
    NULL; /* module name of default netstream driver */
static uchar* pszDfltNetstrmDrvrCAF =
    NULL; /* default CA file for the netstrm driver */
static uchar* pszDfltNetstrmDrvrKeyFile =
    NULL; /* default key file for the netstrm driver (server) */
static uchar* pszDfltNetstrmDrvrCertFile =
    NULL; /* default cert file for the netstrm driver (server) */
static int bTerminateInputs =
    0; /* global switch that inputs shall terminate ASAP (1=> terminate) */
static uchar cCCEscapeChar = '#'; /* character to be used to start an escape
                                     sequence for control chars */
static int bDropTrailingLF = 1;   /* drop trailing LF's on reception? */
static int bEscapeCCOnRcv =
    1; /* escape control characters on reception: 0 - no, 1 - yes */
static int bSpaceLFOnRcv =
    0; /* replace newlines with spaces on reception: 0 - no, 1 - yes */
static int bEscape8BitChars =
    0; /* escape characters > 127 on reception: 0 - no, 1 - yes */
static int bEscapeTab =
    1; /* escape tab control character when doing CC escapes: 0 - no, 1 - yes */
static int bParserEscapeCCCStyle =
    0; /* escape control characters in c style: 0 - no, 1 - yes */
short janitorInterval =
    10; /* interval (in minutes) at which the janitor runs */
...
static int iFdSetSize = howmany(FD_SETSIZE, __NFDBITS) *
                        sizeof(fd_mask); /* size of select() bitmask in bytes */
#endif
static uchar* SourceIPofLocalClient =
    NULL; /* [ar] Source IP for local client to be used on multihomed host */
...
static struct cnfparamblk timezonepblk = {CNFPARAMBLK_VERSION,
                                          sizeof(timezonecnfparamdescr) /
                                              sizeof(struct cnfparamdescr),
...
SIMP_PROP_SET(DfltNetstrmDrvr, pszDfltNetstrmDrvr,
              uchar*) /* TODO: use custom function which frees existing value */
 ...
 */
static rsRetVal setLocalHostIPIF(void __attribute__((unused)) * pVal,
                                 uchar* pNewVal)
{
    uchar myIP[128];
    rsRetVal localRet;
...
static rsRetVal setWorkDir(void __attribute__((unused)) * pVal, uchar* pNewVal)
{
    ...
    {
        LogError(0, RS_RET_ERR_WRKDIR,
                 "$WorkDirectory: %s not a directory - directive ignored",
                 pNewVal);
        ABORT_FINALIZE(RS_RET_ERR_WRKDIR);
    }
...
static rsRetVal legacySetMaxMessageSize(void __attribute__((unused)) * pVal,
                                        int64_t iNew)
{
    setMaxLine(iNew);
    return RS_RET_OK;
...
static rsRetVal setDebugFile(void __attribute__((unused)) * pVal,
                             uchar* pNewVal)
{
    DEFiRet;
    ...
    setReportChildProcessExits(const uchar* const mode)
{
    ...
    else
    {
        LogError(0, RS_RET_CONF_PARAM_INVLD,
                 "invalid value '%s' for global parameter "
                 "reportChildProcessExits -- ignored",
                 mode);
...
static rsRetVal setDisableDNS(int val)
{
    bDisableDNS = val;
    return RS_RET_OK;
}

static int getDisableDNS(void)
{
    return bDisableDNS;
}

static rsRetVal setOption_DisallowWarning(int val)
{
    option_DisallowWarning = val;
    return RS_RET_OK;
}

static int getOption_DisallowWarning(void)
{
    return option_DisallowWarning;
}

static rsRetVal setParseHOSTNAMEandTAG(int val)
{
    bParseHOSTNAMEandTAG = val;
    return RS_RET_OK;
}

static int getParseHOSTNAMEandTAG(void)
{
    return bParseHOSTNAMEandTAG;
}

static rsRetVal setDefPFFamily(int level)
{
    DEFiRet;
    iDefPFFamily = level;
    RETiRet;
...
static int getDefPFFamily(void)
{
    return iDefPFFamily;
}
 ...
 * "once in a lifetime" action which can not be undone. -- gerhards, 2009-07-20
 */
static rsRetVal SetLocalHostName(uchar* newname)
{
    free(LocalHostName);
 ...
 */
static uchar* GetLocalHostName(void)
{
    uchar* pszRet;
    ...
    if (WIFEXITED(status))
    {
        ...
        if (name != NULL)
        {
            LogMsg(0, NO_ERRCODE, severity,
                   "program '%s' (pid %d) exited with status %d", name, pid,
                   WEXITSTATUS(status));
        }
        else
        {
            LogMsg(0, NO_ERRCODE, severity,
                   "child process (pid %d) exited with status %d", pid,
                   WEXITSTATUS(status));
        }
    }
    else if (WIFSIGNALED(status))
    {
        if (name != NULL)
        {
            LogMsg(0, NO_ERRCODE, LOG_WARNING,
                   "program '%s' (pid %d) terminated by signal %d", name, pid,
                   WTERMSIG(status));
        }
        else
        {
            LogMsg(0, NO_ERRCODE, LOG_WARNING,
                   "child process (pid %d) terminated by signal %d", pid,
                   WTERMSIG(status));
        }
    }
}
 ...
 * info has been set as well as PreserveFQDN. rgerhards, 2009-06-30 NOTE: This
 * function tries to avoid locking by not destructing the previous value
 * immediately. This is so that current readers can  continue to use the
 * previous name. Otherwise, we would need to use read/write locks to protect
 * the update process. In order to do so, we save the previous value and delete
 * it when we are called again the next time. Note that this in theory is racy
 * and can lead to a double-free. In practice, however, the window of exposure
 * to trigger this is extremely short and as this functions is very infrequently
 * being called (on HUP), the trigger condition for this bug is so highly
 * unlikely that it never occurs in practice. Probably if you HUP rsyslog every
 * few milliseconds, but who does that... To further reduce risk potential, we
 * do only update the property when there actually is a hostname change, which
 * makes it even less likely. rgerhards, 2013-10-28
 */
static rsRetVal GenerateLocalHostNameProperty(void)
{
    ...
    { /* local hostname is overriden via config */
...
#endif
#undef SIMP_PROP
finalize_it : ENDobjQueryInterface(glbl)
...
static void freeTimezoneInfo(void)
{
    int i;
...
static void displayTzinfos(void)
{
    int i;
    if (!Debug)
        return;
    for (i = 0; i < ntzinfos; ++i)
        dbgprintf("tzinfo: '%s':%c%2.2d:%2.2d\n", tzinfos[i].id,
                  tzinfos[i].offsMode, tzinfos[i].offsHour, tzinfos[i].offsMin);
}
 ...
 * initialization.
 */
static rsRetVal addTimezoneInfo(uchar* tzid, char offsMode, int8_t offsHour,
                                int8_t offsMin)
{
    DEFiRet;
    tzinfo_t* newti;
...
{
    return (tzinfo_t*)bsearch(id, tzinfos, ntzinfos, sizeof(tzinfo_t),
                              bs_arrcmp_tzinfo);
}
...
void glblProcessTimezone(struct cnfobj* o)
{
    ...
    {
        parser_errmsg(
            "offset parameter missing (logic error?), timezone config ignored");
        goto done;
    }
    if (id == NULL)
    {
        parser_errmsg(
            "id parameter missing (logic error?), timezone config ignored");
        goto done;
    }
    ...
    {
        parser_errmsg("timezone offset has invalid format. Must be +/-hh:mm, "
                      "e.g. \"-07:00\".");
        goto done;
    }
    ...
    {
        parser_errmsg("timezone offset outside of supported range (hours "
                      "0..12, minutes 0..59)");
        goto done;
    }
...
rsRetVal glblDoneLoadCnf(void)
{
    ...
    for (i = 0; i < paramblk.nParams; ++i)
    {
        ...
        else if (!strcmp(paramblk.descr[i].name, "debug.onshutdown"))
        {
            ...
            LogError(0, RS_RET_OK, "debug: onShutdown set to %d",
                     glblDebugOnShutdown);
        }
        ...
        else if (!strcmp(paramblk.descr[i].name, "parser.spacelfonreceive"))
        {
            bSpaceLFOnRcv = (int)cnfparamvals[i].val.d.n;
                         ...
                         "parser.escape8bitcharactersonreceive"))
        {
            bEscape8BitChars = (int)cnfparamvals[i].val.d.n;
        }
        else if (!strcmp(paramblk.descr[i].name,
                         "parser.escapecontrolcharactertab"))
        {
        ...
        else if (!strcmp(paramblk.descr[i].name, "debug.logfile"))
        {
            if (pszAltDbgFileName == NULL)
            {
                ...
                if ((altdbg = open(pszAltDbgFileName,
                                   O_WRONLY | O_CREAT | O_TRUNC | O_NOCTTY |
                                       O_CLOEXEC,
                                   S_IRUSR | S_IWUSR)) == -1)
                {
                    LogError(0, RS_RET_ERR,
                             "debug log file '%s' could not be opened",
                             pszAltDbgFileName);
                }
            }
            LogError(0, RS_RET_OK, "debug log file is '%s', fd %d",
                     pszAltDbgFileName, altdbg);
        }
        ...
        else if (!strcmp(paramblk.descr[i].name, "inputs.timeout.shutdown"))
        {
            glblInputTimeoutShutdown = (int)cnfparamvals[i].val.d.n;
                         ...
                         "privdrop.group.keepsupplemental"))
        {
            loadConf->globals.gidDropPrivKeepSupplemental =
                (int)cnfparamvals[i].val.d.n;
        }
        ...
        else if (!strcmp(paramblk.descr[i].name, "internalmsg.ratelimit.burst"))
        {
            glblIntMsgRateLimitBurst = (int)cnfparamvals[i].val.d.n;
                         ...
                         "internalmsg.ratelimit.interval"))
        {
        ...
        else if (!strcmp(paramblk.descr[i].name, "environment"))
        {
            for (int j = 0; j < cnfparamvals[i].val.d.ar->nmemb; ++j)
            {
                ...
                free(var);
                         ...
                         "errormessagestostderr.maxnumber"))
        {
        ...
        else if (!strcmp(paramblk.descr[i].name, "debug.files"))
        {
            ...
            glblDbgFiles =
                (char**)malloc(cnfparamvals[i].val.d.ar->nmemb * sizeof(char*));
            for (int j = 0; j < cnfparamvals[i].val.d.ar->nmemb; ++j)
            {
                glblDbgFiles[j] =
                    es_str2cstr(cnfparamvals[i].val.d.ar->arr[j], NULL);
            }
            qsort(glblDbgFiles, glblDbgFilesNum, sizeof(char*),
                  qs_arrcmp_glblDbgFiles);
        }
 ...
 * ;) */
CHKiRet(regCfSysLineHdlr((uchar*)"debugfile", 0, eCmdHdlrGetWord, setDebugFile,
                         NULL, NULL));
CHKiRet(regCfSysLineHdlr((uchar*)"debuglevel", 0, eCmdHdlrInt, setDebugLevel,
                         NULL, NULL));
CHKiRet(regCfSysLineHdlr((uchar*)"workdirectory", 0, eCmdHdlrGetWord,
                         setWorkDir, NULL, NULL));
CHKiRet(regCfSysLineHdlr((uchar*)"dropmsgswithmaliciousdnsptrrecords", 0,
                         eCmdHdlrBinary, NULL, &bDropMalPTRMsgs, NULL));
CHKiRet(regCfSysLineHdlr((uchar*)"defaultnetstreamdriver", 0, eCmdHdlrGetWord,
                         NULL, &pszDfltNetstrmDrvr, NULL));
CHKiRet(regCfSysLineHdlr((uchar*)"defaultnetstreamdrivercafile", 0,
                         eCmdHdlrGetWord, NULL, &pszDfltNetstrmDrvrCAF, NULL));
CHKiRet(regCfSysLineHdlr((uchar*)"defaultnetstreamdriverkeyfile", 0,
                         eCmdHdlrGetWord, NULL, &pszDfltNetstrmDrvrKeyFile,
                         NULL));
CHKiRet(regCfSysLineHdlr((uchar*)"defaultnetstreamdrivercertfile", 0,
                         eCmdHdlrGetWord, NULL, &pszDfltNetstrmDrvrCertFile,
                         NULL));
CHKiRet(regCfSysLineHdlr((uchar*)"localhostname", 0, eCmdHdlrGetWord, NULL,
                         &LocalHostNameOverride, NULL));
CHKiRet(regCfSysLineHdlr((uchar*)"localhostipif", 0, eCmdHdlrGetWord,
                         setLocalHostIPIF, NULL, NULL));
CHKiRet(regCfSysLineHdlr((uchar*)"optimizeforuniprocessor", 0, eCmdHdlrGoneAway,
                         NULL, NULL, NULL));
CHKiRet(regCfSysLineHdlr((uchar*)"preservefqdn", 0, eCmdHdlrBinary, NULL,
                         &bPreserveFQDN, NULL));
CHKiRet(regCfSysLineHdlr((uchar*)"maxmessagesize", 0, eCmdHdlrSize,
                         legacySetMaxMessageSize, NULL, NULL));

/* Deprecated parser config options */
CHKiRet(regCfSysLineHdlr((uchar*)"controlcharacterescapeprefix", 0,
                         eCmdHdlrGetChar, NULL, &cCCEscapeChar, NULL));
CHKiRet(regCfSysLineHdlr((uchar*)"droptrailinglfonreception", 0, eCmdHdlrBinary,
                         NULL, &bDropTrailingLF, NULL));
CHKiRet(regCfSysLineHdlr((uchar*)"escapecontrolcharactersonreceive", 0,
                         eCmdHdlrBinary, NULL, &bEscapeCCOnRcv, NULL));
CHKiRet(regCfSysLineHdlr((uchar*)"spacelfonreceive", 0, eCmdHdlrBinary, NULL,
                         &bSpaceLFOnRcv, NULL));
CHKiRet(regCfSysLineHdlr((uchar*)"escape8bitcharactersonreceive", 0,
                         eCmdHdlrBinary, NULL, &bEscape8BitChars, NULL));
CHKiRet(regCfSysLineHdlr((uchar*)"escapecontrolcharactertab", 0, eCmdHdlrBinary,
                         NULL, &bEscapeTab, NULL));
