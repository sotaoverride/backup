...
#ifdef WIN32
#define WIN32_LEAN_AND_MEAN /* Prevent loading any Winsock 1.x headers from    \
                               windows.h */
#endif
...
#define rfbClientSwap16IfLE(s)                                                 \
    (*(char*)&client->endianTest ? ((((s)&0xff) << 8) | (((s) >> 8) & 0xff))   \
                                 : (s))
...
/* When using OpenSSL, CRLs can be included in both the x509CACrlFile and
   appended to the x509CACertFile as is common with OpenSSL.  When
   rfbX509CrlVerifyAll is specified the CRL list must include CRLs for all
   certificates in the chain */
#define rfbX509CrlVerifyNone 0 /* No CRL checking is performed */
#define rfbX509CrlVerifyClient                                                 \
    1 /* Only the leaf server certificate is checked */
#define rfbX509CrlVerifyAll                                                    \
    2 /* All certificates in the server chain are checked */
 ...
 */
typedef void (*HandleTextChatProc)(struct _rfbClient* client, int value,
                                   char* text);
/**
 ...
 */
typedef void (*HandleXvpMsgProc)(struct _rfbClient* client, uint8_t version,
                                 uint8_t opcode);
typedef void (*HandleKeyboardLedStateProc)(struct _rfbClient* client, int value,
                                           int pad);
typedef rfbBool (*HandleCursorPosProc)(struct _rfbClient* client, int x, int y);
typedef void (*SoftCursorLockAreaProc)(struct _rfbClient* client, int x, int y,
                                       int w, int h);
typedef void (*SoftCursorUnlockScreenProc)(struct _rfbClient* client);
typedef void (*GotFrameBufferUpdateProc)(struct _rfbClient* client, int x,
                                         int y, int w, int h);
typedef void (*FinishedFrameBufferUpdateProc)(struct _rfbClient* client);
typedef char* (*GetPasswordProc)(struct _rfbClient* client);
typedef rfbCredential* (*GetCredentialProc)(struct _rfbClient* client,
                                            int credentialType);
typedef rfbBool (*MallocFrameBufferProc)(struct _rfbClient* client);
typedef void (*GotXCutTextProc)(struct _rfbClient* client, const char* text,
                                int textlen);
typedef void (*BellProc)(struct _rfbClient* client);
/**
    Called when a cursor shape update was received from the server. The decoded
   cursor shape will be in client->rcSource. It's up to the application to do
   something with this, e.g. draw into a viewer's window. If you want the server
   to draw the cursor into the framebuffer, be careful not to announce remote
   cursor support, i.e. not include rfbEncodingXCursor or rfbEncodingRichCursor
   in SetFormatAndEncodings().
*/
typedef void (*GotCursorShapeProc)(struct _rfbClient* client, int xhot,
                                   int yhot, int width, int height,
                                   int bytesPerPixel);
typedef void (*GotCopyRectProc)(struct _rfbClient* client, int src_x, int src_y,
                                int w, int h, int dest_x, int dest_y);
typedef void (*GotFillRectProc)(struct _rfbClient* client, int x, int y, int w,
                                int h, uint32_t colour);
typedef void (*GotBitmapProc)(struct _rfbClient* client, const uint8_t* buffer,
                              int x, int y, int w, int h);
typedef rfbBool (*GotJpegProc)(struct _rfbClient* client, const uint8_t* buffer,
...
typedef char* (*GetSASLMechanismProc)(struct _rfbClient* client,
                                      char* mechlist);
#endif /* LIBVNCSERVER_HAVE_SASL */
...
typedef struct _rfbClient
{
    ...
    /** Note that the CoRRE encoding uses this buffer and assumes it is big
       enough to hold 255 * 255 * 32 bits -> 260100 bytes.  640*480 = 307200
       bytes. Hextile also assumes it is big enough to hold 16 * 16 * 32 bits.
...
#define ZLIB_BUFFER_SIZE 30000
    ...
    /* Output Window ID. When set, client application enables libvncclient to
     * perform direct rendering in its window */
    ...
    /* Callback to allow the client to choose a preferred mechanism. The string
       returned will be freed once no longer required. */
 ...
 */
extern rfbBool HandleCursorShape(rfbClient* client, int xhot, int yhot,
                                 int width, int height, uint32_t enc);

/* listen.c */

extern void listenForIncomingConnections(rfbClient* viewer);
extern int listenForIncomingConnectionsNoFork(rfbClient* viewer,
                                              int usec_timeout);
...
extern rfbBool ConnectToRFBServer(rfbClient* client, const char* hostname,
                                  int port);
extern rfbBool ConnectToRFBRepeater(rfbClient* client, const char* repeaterHost,
                                    int repeaterPort, const char* destHost,
                                    int destPort);
extern void SetClientAuthSchemes(rfbClient* client, const uint32_t* authSchemes,
                                 int size);
 ...
 */
extern rfbBool SendFramebufferUpdateRequest(rfbClient* client, int x, int y,
                                            int w, int h, rfbBool incremental);
 ...
 */
extern rfbBool SendPointerEvent(rfbClient* client, int x, int y,
                                int buttonMask);
/**
...
rfbClient* rfbGetClient(int bitsPerSample, int samplesPerPixel,
                        int bytesPerPixel);
/**
 * Initializes the client. The format is {PROGRAM_NAME, [OPTIONS]..., HOST}.
 * This function does not initialize the program name if the rfbClient's program
 * name is set already. The options are as follows:
 * <table>
 * <tr><th>Option</th><th>Description</th></tr>
 * <tr><td>-listen</td><td>Listen for incoming connections.</td></tr>
 * <tr><td>-listennofork</td><td>Listen for incoming connections without
 * forking.
 * </td></tr>
 * <tr><td>-play</td><td>Set this client to replay a previously recorded
 * session.</td></tr> <tr><td>-encodings</td><td>Set the encodings to use. The
 * next item in the argv array is the encodings string, consisting of comma
 * separated encodings like 'tight,ultra,raw'.</td></tr>
 * <tr><td>-compress</td><td>Set the compression level. The next item in the
 * argv array is the compression level as an integer. Ranges from 0 (lowest) to
 * 9 (highest).
 * </td></tr>
 * <tr><td>-scale</td><td>Set the scaling level. The next item in the
 * argv array is the scaling level as an integer. The screen will be scaled down
 * by this factor.</td></tr> <tr><td>-qosdscp</td><td>Set the Quality of Service
 * Differentiated Services Code Point (QoS DSCP). The next item in the argv
 * array is the code point as an integer.</td></tr>
 * <tr><td>-repeaterdest</td><td>Set a VNC repeater address. The next item in
 * the argv array is the repeater's address as a string.</td></tr>
 * </table>
 *
