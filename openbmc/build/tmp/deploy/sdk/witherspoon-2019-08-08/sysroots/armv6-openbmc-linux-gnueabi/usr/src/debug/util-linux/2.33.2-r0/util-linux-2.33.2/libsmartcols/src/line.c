 ...
 * SECTION: line
 * @title: Line
 * @short_description: cells container, also keeps tree (parent->child)
 * information
 *
 ...
 * @ln: a pointer to a struct libscols_line instance
 *
 * Returns: a pointer to @ln's parent, NULL in case it has no parent or if there
 * was an error.
 */
 ...
 * Returns: 0, a negative value in case of an error.
 */
int scols_line_next_child(struct libscols_line* ln, struct libscols_iter* itr,
 ...
 * Returns: 0 or 1
 */
int scols_line_is_ancestor(struct libscols_line* ln,
 ...
 */
struct libscols_cell* scols_line_get_cell(struct libscols_line* ln, size_t n)
{
    if (!ln || n >= ln->ncells)
        return NULL;
    return &ln->cells[n];
...
int scols_line_set_column_data(struct libscols_line* ln,
                               struct libscols_column* cl, const char* data)
{
