...
#define ATTR_TABLE(Page)                                                       \
    (((Page) <= G_UNICODE_LAST_PAGE_PART1) ? attr_table_part1[Page]            \
                                           : attr_table_part2[(Page)-0xe00])

#define ATTTABLE(Page, Char)                                                   \
    ((ATTR_TABLE(Page) == G_UNICODE_MAX_TABLE_INDEX)                           \
         ? 0                                                                   \
         : (attr_data[ATTR_TABLE(Page)][Char]))
...
#define ISALPHA(Type)                                                          \
    IS((Type), OR(G_UNICODE_LOWERCASE_LETTER,                                  \
                  OR(G_UNICODE_UPPERCASE_LETTER,                               \
                     OR(G_UNICODE_TITLECASE_LETTER,                            \
                        OR(G_UNICODE_MODIFIER_LETTER,                          \
                           OR(G_UNICODE_OTHER_LETTER, 0))))))

#define ISALDIGIT(Type)                                                        \
    IS((Type), OR(G_UNICODE_DECIMAL_NUMBER,                                    \
...
#define ISMARK(Type)                                                           \
    IS((Type),                                                                 \
       OR(G_UNICODE_NON_SPACING_MARK,                                          \
          OR(G_UNICODE_SPACING_MARK, OR(G_UNICODE_ENCLOSING_MARK, 0))))

#define ISZEROWIDTHTYPE(Type)                                                  \
    IS((Type), OR(G_UNICODE_NON_SPACING_MARK,                                  \
                  OR(G_UNICODE_ENCLOSING_MARK, OR(G_UNICODE_FORMAT, 0))))
        ...
        OR(G_UNICODE_CONTROL,
           OR(G_UNICODE_FORMAT,
              OR(G_UNICODE_UNASSIGNED,
                 OR(G_UNICODE_SURROGATE, OR(G_UNICODE_SPACE_SEPARATOR, 0))))));
}
...
gboolean g_unichar_isprint(gunichar c)
{
    return !IS(TYPE(c),
               OR(G_UNICODE_CONTROL,
                  OR(G_UNICODE_FORMAT,
                     OR(G_UNICODE_UNASSIGNED, OR(G_UNICODE_SURROGATE, 0)))));
}
...
{
    return IS(TYPE(c), OR(G_UNICODE_CONNECT_PUNCTUATION,
...
gboolean g_unichar_isspace(gunichar c)
{
    switch (c)
    {
        ...
        default:
        {
            return IS(TYPE(c), OR(G_UNICODE_SPACE_SEPARATOR,
                                  OR(G_UNICODE_LINE_SEPARATOR,
                                     OR(G_UNICODE_PARAGRAPH_SEPARATOR, 0))))
                       ? TRUE
                       : FALSE;
        }
        break;
...
static inline gboolean g_unichar_iswide_bsearch(gunichar ch)
{
    int lower = 0;
...
gboolean g_unichar_iswide_cjk(gunichar c)
{
    ...
    if (bsearch(GUINT_TO_POINTER(c), g_unicode_width_table_ambiguous,
                G_N_ELEMENTS(g_unicode_width_table_ambiguous),
                sizeof g_unicode_width_table_ambiguous[0], interval_compare))
        return TRUE;
...
static gint output_marks(const char** p_inout, char* out_buffer,
                         gboolean remove_dot)
{
    ...
    while (*p)
    {
        ...
        if (ISMARK(TYPE(c)))
        {
            if (!remove_dot || c != 0x307 /* COMBINING DOT ABOVE */)
                len +=
                    g_unichar_to_utf8(c, out_buffer ? out_buffer + len : NULL);
...
static gint output_special_case(gchar* out_buffer, int offset, int type,
                                int which)
{
...
static gsize real_toupper(const gchar* str, gssize max_len, gchar* out_buffer,
                          LocaleType locale_type)
{
    ...
    while ((max_len < 0 || p < str + max_len) && *p)
    {
            ...
            else
            {
                if (last_was_i)
                {
                    ...
                    decomp_len = g_unichar_fully_decompose(
                        c, FALSE, decomp, G_N_ELEMENTS(decomp));
                    for (i = 0; i < decomp_len; i++)
                    {
                        if (decomp[i] != 0x307 /* COMBINING DOT ABOVE */)
                            len += g_unichar_to_utf8(
                                g_unichar_toupper(decomp[i]),
                                out_buffer ? out_buffer + len : NULL);
                    }

                    len += output_marks(
                        &p, out_buffer ? out_buffer + len : NULL, TRUE);
        ...
        if (locale_type == LOCALE_TURKIC && c == 'i')
        {
            ...
            len +=
                g_unichar_to_utf8(0x130, out_buffer ? out_buffer + len : NULL);
        }
        else if (c == 0x0345) /* COMBINING GREEK YPOGEGRAMMENI */
        {
            /* Nasty, need to move it after other combining marks .. this would
             * go away if we normalized first.
             */
            len +=
                output_marks(&p, out_buffer ? out_buffer + len : NULL, FALSE);

            /* And output as GREEK CAPITAL LETTER IOTA */
            len +=
                g_unichar_to_utf8(0x399, out_buffer ? out_buffer + len : NULL);
        }
        else if (IS(t, OR(G_UNICODE_LOWERCASE_LETTER,
                          OR(G_UNICODE_TITLECASE_LETTER, 0))))
        {
            val = ATTTABLE(c >> 8, c & 0xff);

            if (val >= 0x1000000)
            {
                len += output_special_case(
                    out_buffer ? out_buffer + len : NULL, val - 0x1000000, t,
            ...
            else
            {
                 ...
                 * INDICATOR, do not have an uppercase equivalent, in which case
                 * val will be zero. */
                len += g_unichar_to_utf8(val ? val : c,
                                         out_buffer ? out_buffer + len : NULL);
            }
        }
        else
...
static gsize real_tolower(const gchar* str, gssize max_len, gchar* out_buffer,
                          LocaleType locale_type)
{
    const gchar* p = str;
    const char* last = NULL;
    ...
    while ((max_len < 0 || p < str + max_len) && *p)
    {
        ...
        {
            len +=
                g_unichar_to_utf8(0x0069, out_buffer ? out_buffer + len : NULL);
            len +=
                g_unichar_to_utf8(0x0307, out_buffer ? out_buffer + len : NULL);

            switch (c)
            {
                case 0x00cc:
                    len += g_unichar_to_utf8(
                        0x0300, out_buffer ? out_buffer + len : NULL);
                    break;
                case 0x00cd:
                    len += g_unichar_to_utf8(
                        0x0301, out_buffer ? out_buffer + len : NULL);
                    break;
                case 0x0128:
                    len += g_unichar_to_utf8(
                        0x0303, out_buffer ? out_buffer + len : NULL);
                    break;
            }
        }
        else if (locale_type == LOCALE_LITHUANIAN &&
                 (c == 'I' || c == 'J' || c == 0x012e) && has_more_above(p))
        {
            len += g_unichar_to_utf8(g_unichar_tolower(c),
                                     out_buffer ? out_buffer + len : NULL);
            len +=
                g_unichar_to_utf8(0x0307, out_buffer ? out_buffer + len : NULL);
        }
        ...
        else if (IS(t, OR(G_UNICODE_UPPERCASE_LETTER,
                          OR(G_UNICODE_TITLECASE_LETTER, 0))))
        {
            val = ATTTABLE(c >> 8, c & 0xff);
...
#define PACK(a, b, c, d)                                                       \
    ((guint32)((((guint8)(a)) << 24) | (((guint8)(b)) << 16) |                 \
               (((guint8)(c)) << 8) | ((guint8)(d))))
 ...
 *   ISO 15924 code 'Zzzz' (script code for UNKNOWN) if @script is not
 * understood.
 *
 * Since: 2.30
 */
