...
void e2p_list_journal_super(FILE* f, char* journal_sb_buf, int exp_block_size,
                            int flags)
{
    ...
    if ((jsb->s_feature_incompat & e2p_be32(JFS_FEATURE_INCOMPAT_CSUM_V3)) ||
        (jsb->s_feature_incompat & e2p_be32(JFS_FEATURE_INCOMPAT_CSUM_V2)))
        fprintf(f,
                "Journal checksum type:    %s\n"
                "Journal checksum:         0x%08x\n",
                journal_checksum_type_str(jsb->s_checksum_type),
                e2p_be32(jsb->s_checksum));
    if ((nr_users > 1) || !e2p_is_null_uuid(&jsb->s_users[0]))
    {
