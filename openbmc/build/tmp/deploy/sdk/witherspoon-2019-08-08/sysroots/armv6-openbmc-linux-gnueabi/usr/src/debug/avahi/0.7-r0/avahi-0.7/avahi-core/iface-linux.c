...
static int netlink_list_items(AvahiNetlink* nl, uint16_t type,
                              unsigned* ret_seq)
{
    struct nlmsghdr* n;
    struct rtgenmsg* gen;
...
static void netlink_callback(AvahiNetlink* nl, struct nlmsghdr* n,
                             void* userdata)
{
    ...
    if (n->nlmsg_type == RTM_NEWLINK)
    {
        ...
        if (!(hw = avahi_interface_monitor_get_hw_interface(
                  m, ifinfomsg->ifi_index)))
             ...
             * ourselves */
            if (!(hw = avahi_hw_interface_new(
                      m, (AvahiIfIndex)ifinfomsg->ifi_index)))
                return; /* OOM */

        /* Check whether the flags of this interface are OK for us */
        hw->flags_ok = (ifinfomsg->ifi_flags & IFF_UP) &&
                       (!m->server->config.use_iff_running ||
                        (ifinfomsg->ifi_flags & IFF_RUNNING)) &&
                       !(ifinfomsg->ifi_flags & IFF_LOOPBACK) &&
                       (ifinfomsg->ifi_flags & IFF_MULTICAST) &&
                       (m->server->config.allow_point_to_point ||
    ...
    else if (n->nlmsg_type == RTM_NEWADDR || n->nlmsg_type == RTM_DELADDR)
    {
        ...
        if (!(i = avahi_interface_monitor_get_interface(
                  m, (AvahiIfIndex)ifaddrmsg->ifa_index,
                  avahi_af_to_proto(ifaddrmsg->ifa_family))))
            return;
        ...
        while (RTA_OK(a, l))
        {
            ...
            switch (a->rta_type)
            {
                ...
                case IFA_LOCAL:
                    ...
                    if ((rlocal.proto == AVAHI_PROTO_INET6 &&
                         RTA_PAYLOAD(a) != 16) ||
                        (rlocal.proto == AVAHI_PROTO_INET &&
                         RTA_PAYLOAD(a) != 4))
                        return;
        ...
        if (n->nlmsg_type == RTM_NEWADDR)
        {
            ...
            /* This address is new or has been modified, so let's get an object
             * for it */
            if (!(addr = avahi_interface_monitor_get_address(m, i, r)))

                /* Mmm, no object existing yet, so let's create a new one */
                if (!(addr = avahi_interface_address_new(
                          m, i, r, ifaddrmsg->ifa_prefixlen)))
                    return; /* OOM */

            /* Update the scope field for the address */
            addr->global_scope = ifaddrmsg->ifa_scope == RT_SCOPE_UNIVERSE ||
                                 ifaddrmsg->ifa_scope == RT_SCOPE_SITE;
        ...
        else
        {
            ...
            /* Try to get a reference to our AvahiInterfaceAddress object for
             * this address */
            if (!(addr = avahi_interface_monitor_get_address(m, i, r)))
                return;
    ...
    else if (n->nlmsg_type == NLMSG_DONE)
    {
        ...
        if (m->osdep.list == LIST_IFACE)
        {
            ...
            if (netlink_list_items(m->osdep.netlink, RTM_GETADDR,
                                   &m->osdep.query_addr_seq) < 0)
            {
                avahi_log_warn("NETLINK: Failed to list addrs: %s",
                               strerror(errno));
        ...
        if (m->osdep.list == LIST_DONE)
        {
            ...
            avahi_log_info("Network interface enumeration completed.");
             ...
             (n->nlmsg_seq == m->osdep.query_link_seq ||
              n->nlmsg_seq == m->osdep.query_addr_seq))
    {
        ...
        if (e->error)
            avahi_log_warn("NETLINK: Failed to browse: %s",
                           strerror(-e->error));
    }
}
...
int avahi_interface_monitor_init_osdep(AvahiInterfaceMonitor* m)
{
    ...
    if (netlink_list_items(m->osdep.netlink, RTM_GETLINK,
                           &m->osdep.query_link_seq) < 0)
        goto fail;
