                           ...
                           size_t maxsize, const char* prompt_info, void* data)
{
    ...
    else if (!UI_add_input_string(ui, prompt, UI_INPUT_FLAG_DEFAULT_PWD, pass,
                                  0, maxsize - 1))
    {
 ...
 *    a OSSL_STORE_INFO
 */
typedef OSSL_STORE_INFO* (*file_try_decode_fn)(
    const char* pem_name, const char* pem_header, const unsigned char* blob,
    size_t len, void** handler_ctx, int* matchcount, const UI_METHOD* ui_method,
    void* ui_data);
/*
            ...
            *matchcount = 1;
            ...
            else
            {
                if ((pass = file_get_pass(ui_method, tpass, PEM_BUFSIZE,
                                          "PKCS12 import password", ui_data)) ==
                    NULL)
                {
...
static FILE_HANDLER PKCS12_handler = {
    "PKCS12", try_decode_PKCS12, eof_PKCS12, destroy_ctx_PKCS12,
    1 /* repeatable */
};
...
static OSSL_STORE_INFO*
    try_decode_PKCS8Encrypted(const char* pem_name, const char* pem_header,
                              const unsigned char* blob, size_t len,
                              void** pctx, int* matchcount,
                              const UI_METHOD* ui_method, void* ui_data)
{
...
static FILE_HANDLER PKCS8Encrypted_handler = {"PKCS8Encrypted",
...
static OSSL_STORE_INFO*
    try_decode_PrivateKey(const char* pem_name, const char* pem_header,
                          const unsigned char* blob, size_t len, void** pctx,
                          int* matchcount, const UI_METHOD* ui_method,
                          void* ui_data)
{
    OSSL_STORE_INFO* store_info = NULL;
    EVP_PKEY* pkey = NULL;
...
static FILE_HANDLER PrivateKey_handler = {"PrivateKey", try_decode_PrivateKey};
...
static FILE_HANDLER PUBKEY_handler = {"PUBKEY", try_decode_PUBKEY};
...
static FILE_HANDLER params_handler = {"params", try_decode_params};

/*
 * X.509 certificate decoder.
 */
static OSSL_STORE_INFO*
    try_decode_X509Certificate(const char* pem_name, const char* pem_header,
                               const unsigned char* blob, size_t len,
                               void** pctx, int* matchcount,
                               const UI_METHOD* ui_method, void* ui_data)
{
...
static FILE_HANDLER X509Certificate_handler = {"X509Certificate",
 ...
 */
static OSSL_STORE_INFO*
    try_decode_X509CRL(const char* pem_name, const char* pem_header,
                       const unsigned char* blob, size_t len, void** pctx,
                       int* matchcount, const UI_METHOD* ui_method,
                       void* ui_data)
{
...
static FILE_HANDLER X509CRL_handler = {"X509CRL", try_decode_X509CRL};

/*
 * To finish it all off, we collect all the handlers.
 */
static const FILE_HANDLER* file_handlers[] = {
    &PKCS12_handler,     &PKCS8Encrypted_handler, &X509Certificate_handler,
    &X509CRL_handler,    &params_handler,         &PUBKEY_handler,
    &PrivateKey_handler,
};
...
again:
{
    ...
    const FILE_HANDLER** matching_handlers =
        OPENSSL_zalloc(sizeof(*matching_handlers) * OSSL_NELEM(file_handlers));

    if (matching_handlers == NULL)
    {
        OSSL_STOREerr(OSSL_STORE_F_FILE_LOAD_TRY_DECODE, ERR_R_MALLOC_FAILURE);
        goto err;
    }
    ...
    *matchcount = 0;
    for (i = 0; i < OSSL_NELEM(file_handlers); i++)
    {
        ...
        OSSL_STORE_INFO* tmp_result = handler->try_decode(
            pem_name, pem_header, data, len, &tmp_handler_ctx, &try_matchcount,
            ui_method, ui_data);
                                             ...
                                             void* ui_data)
{
    ...
    if (ctx->_.file.last_handler != NULL)
    {
        result = ctx->_.file.last_handler->try_decode(
            NULL, NULL, NULL, 0, &ctx->_.file.last_handler_ctx, &try_matchcount,
            ui_method, ui_data);
...
static int file_read_pem(BIO* bp, char** pem_name, char** pem_header,
                         ...
                         const UI_METHOD* ui_method, void* ui_data, int secure)
{
    int i = secure ? PEM_read_bio_ex(bp, pem_name, pem_header, data, len,
    ...
    if (strlen(*pem_header) > 10)
    {
        ...
        if (!PEM_get_EVP_CIPHER_INFO(*pem_header, &cipher) ||
            ...
            !PEM_do_header(&cipher, *data, len, file_get_pem_pass, &pass_data))
        {
            return 0;
        }
    }
...
static int file_name_check(OSSL_STORE_LOADER_CTX* ctx, const char* name)
{
    ...
    if (ctx->expected_type != 0 && ctx->expected_type != OSSL_STORE_INFO_CERT &&
                                  ...
                                  const UI_METHOD* ui_method, void* ui_data)
{
    ...
    if (ctx->type == is_dir)
    {
        do
        {
             ...
             * it our URI here.
             */
            ctx->_.dir.last_entry =
                OPENSSL_DIR_read(&ctx->_.dir.ctx, ctx->_.dir.uri);
            ctx->_.dir.last_errno = errno;
    ...
    again:
        ...
        if (result != NULL && ctx->expected_type != 0 &&
...
static OSSL_STORE_LOADER file_loader = {
    "file",    NULL,      file_open, file_ctrl,  file_expect,
    file_find, file_load, file_eof,  file_error, file_close};
