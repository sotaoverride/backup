 ...
 */
#define G_DEBUG_HERE()                                                         \
    g_log_structured(G_LOG_DOMAIN, G_LOG_LEVEL_DEBUG, "CODE_FILE", __FILE__,   \
                     "CODE_LINE", G_STRINGIFY(__LINE__), "CODE_FUNC",          \
                     G_STRFUNC, "MESSAGE", "%" G_GINT64_FORMAT ": %s",         \
...
void g_warn_message(const char* domain, const char* file, int line,
                    const char* func, const char* warnexpr) G_ANALYZER_NORETURN;
GLIB_DEPRECATED
...
#define g_error(...)                                                           \
    G_STMT_START                                                               \
    {                                                                          \
        g_log_structured_standard(G_LOG_DOMAIN, G_LOG_LEVEL_ERROR, __FILE__,   \
                                  G_STRINGIFY(__LINE__), G_STRFUNC,            \
                                  __VA_ARGS__);                                \
        for (;;)                                                               \
            ;                                                                  \
    }                                                                          \
    G_STMT_END
#define g_message(...)                                                         \
    g_log_structured_standard(G_LOG_DOMAIN, G_LOG_LEVEL_MESSAGE, __FILE__,     \
                              G_STRINGIFY(__LINE__), G_STRFUNC, __VA_ARGS__)
#define g_critical(...)                                                        \
    g_log_structured_standard(G_LOG_DOMAIN, G_LOG_LEVEL_CRITICAL, __FILE__,    \
                              G_STRINGIFY(__LINE__), G_STRFUNC, __VA_ARGS__)
#define g_warning(...)                                                         \
    g_log_structured_standard(G_LOG_DOMAIN, G_LOG_LEVEL_WARNING, __FILE__,     \
                              G_STRINGIFY(__LINE__), G_STRFUNC, __VA_ARGS__)
#define g_info(...)                                                            \
    g_log_structured_standard(G_LOG_DOMAIN, G_LOG_LEVEL_INFO, __FILE__,        \
                              G_STRINGIFY(__LINE__), G_STRFUNC, __VA_ARGS__)
#define g_debug(...)                                                           \
    g_log_structured_standard(G_LOG_DOMAIN, G_LOG_LEVEL_DEBUG, __FILE__,       \
                              G_STRINGIFY(__LINE__), G_STRFUNC, __VA_ARGS__)
#else
...
#define g_error(format...)                                                     \
    ...
    {                                                                          \
        g_log_structured_standard(G_LOG_DOMAIN, G_LOG_LEVEL_ERROR, __FILE__,   \
                                  G_STRINGIFY(__LINE__), G_STRFUNC, format);   \
        for (;;)                                                               \
            ;                                                                  \
    }                                                                          \
    G_STMT_END
...
static inline void g_error(const gchar* format, ...)
{
    va_list args;
...
static inline void g_message(const gchar* format, ...)
{
    va_list args;
    va_start(args, format);
...
static inline void g_critical(const gchar* format, ...)
{
    va_list args;
    va_start(args, format);
...
static inline void g_warning(const gchar* format, ...)
{
    va_list args;
    va_start(args, format);
