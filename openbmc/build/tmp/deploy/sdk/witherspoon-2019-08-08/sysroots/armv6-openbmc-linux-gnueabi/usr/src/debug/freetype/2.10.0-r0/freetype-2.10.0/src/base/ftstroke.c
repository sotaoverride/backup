...
static FT_Bool ft_conic_is_small_enough(FT_Vector* base, FT_Angle* angle_in,
                                        FT_Angle* angle_out)
{
    FT_Vector d1, d2;
    FT_Angle theta;
...
static FT_Bool ft_cubic_is_small_enough(FT_Vector* base, FT_Angle* angle_in,
                                        FT_Angle* angle_mid,
                                        FT_Angle* angle_out)
{
    FT_Vector d1, d2, d3;
...
static FT_Error ft_stroke_border_lineto(FT_StrokeBorder border, FT_Vector* to,
                                        FT_Bool movable)
{
    FT_Error error = FT_Err_Ok;
...
static FT_Error ft_stroke_border_cubicto(FT_StrokeBorder border,
                                         FT_Vector* control1,
                                         FT_Vector* control2, FT_Vector* to)
{
    FT_Error error;
...
static FT_Error ft_stroke_border_arcto(FT_StrokeBorder border,
                                       FT_Vector* center, FT_Fixed radius,
...
static void ft_stroke_border_export(FT_StrokeBorder border, FT_Outline* outline)
{
    ...
    if (border->num_points)
        FT_ARRAY_COPY(outline->points + outline->n_points, border->points,
                      border->num_points);
                                  ...
                                  FT_Fixed line_length)
{
    ...
    if (!border->movable || line_length == 0 || theta > 0x59C000 ||
        theta < -0x59C000)
        intersect = FALSE;
    else
    {
        ...
        FT_Fixed min_length =
            ft_pos_abs(FT_MulFix(stroker->radius, FT_Tan(theta)));

        intersect = FT_BOOL(min_length && stroker->line_length >= min_length &&
    ...
    else
    {
        ...
        if (bevel) /* this is a bevel (broken angle) */
        {
            if (fixed_bevel)
            {
                ...
                /* add bevel */
                FT_Vector_From_Polar(&delta, radius,
                                     stroker->angle_out + rotate);
                delta.x += stroker->center.x;
                delta.y += stroker->center.y;
            ...
            else /* variable bevel */
            {
                ...
                FT_Vector_From_Polar(
                    &middle, FT_MulFix(radius, stroker->miter_limit), phi);
...
FT_Stroker_ConicTo(FT_Stroker stroker, FT_Vector* control, FT_Vector* to)
{
    ...
    if (FT_IS_SMALL(stroker->center.x - control->x) &&
        ...
        FT_IS_SMALL(control->x - to->x) && FT_IS_SMALL(control->y - to->y))
    {
        stroker->center = *to;
        goto Exit;
    }
                   ...
                   FT_Vector* to)
{
    ...
    if (FT_IS_SMALL(stroker->center.x - control1->x) &&
        ...
        FT_IS_SMALL(control2->x - to->x) && FT_IS_SMALL(control2->y - to->y))
    {
        stroker->center = *to;
        goto Exit;
    }
...
FT_Stroker_EndSubPath(FT_Stroker stroker)
{
    ...
    else
    {
        ...
        if (turn != 0)
        {
            ...
            /* process the outside side */
            error = ft_stroker_outside(stroker, !inside_side,
                                       stroker->subpath_line_length);
            if (error)
                goto Exit;
        }
            ...
            switch (tag)
            {
                ...
                Do_Conic:
                    if (point < limit)
                    {
                        ...
                        error =
                            FT_Stroker_ConicTo(stroker, &v_control, &v_middle);
                        if (error)
                            goto Exit;
...
FT_Glyph_Stroke(FT_Glyph* pglyph, FT_Stroker stroker, FT_Bool destroy)
{
...

    {
        ...
        error = FT_Outline_New(glyph->library, num_points, (FT_Int)num_contours,
                               outline);
        if (error)
            goto Fail;
