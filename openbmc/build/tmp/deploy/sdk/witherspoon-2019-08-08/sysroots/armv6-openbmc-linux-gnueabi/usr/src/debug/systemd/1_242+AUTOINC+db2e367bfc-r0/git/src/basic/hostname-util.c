...
int gethostname_strict(char** ret)
{
    ...
    /* This call will rather fail than make up a name. It will not return
     * "localhost" either. */
...
int read_etc_hostname_stream(FILE* f, char** ret)
{
    ...
    for (;;)
    {
        ...
        if (r < 0)
            return r;
        if (r == 0) /* EOF without any hostname? the file is empty, let's treat
                       that exactly like no file at all: ENOENT */
            return -ENOENT;
        ...
        if (!IN_SET(*p, '\0', '#'))
        {
            ...
            if (!hostname_is_valid(
                    p, true)) /* check that the hostname we return is valid */
                return -EBADMSG;
