...
#define G_DATAGRAM_BASED(inst)                                                 \
    (G_TYPE_CHECK_INSTANCE_CAST((inst), G_TYPE_DATAGRAM_BASED, GDatagramBased))
#define G_IS_DATAGRAM_BASED(inst)                                              \
    (G_TYPE_CHECK_INSTANCE_TYPE((inst), G_TYPE_DATAGRAM_BASED))
#define G_DATAGRAM_BASED_GET_IFACE(inst)                                       \
    (G_TYPE_INSTANCE_GET_INTERFACE((inst), G_TYPE_DATAGRAM_BASED,              \
                                   GDatagramBasedInterface))
#define G_TYPE_IS_DATAGRAM_BASED(type)                                         \
    (g_type_is_a((type), G_TYPE_DATAGRAM_BASED))
...
struct _GDatagramBasedInterface
{
    ...
    gint (*receive_messages)(GDatagramBased* datagram_based,
                             GInputMessage* messages, guint num_messages,
                             gint flags, gint64 timeout,
                             GCancellable* cancellable, GError** error);
    gint (*send_messages)(GDatagramBased* datagram_based,
                          GOutputMessage* messages, guint num_messages,
                          gint flags, gint64 timeout, GCancellable* cancellable,
                          GError** error);
    ...
    gboolean (*condition_wait)(GDatagramBased* datagram_based,
                               GIOCondition condition, gint64 timeout,
                               GCancellable* cancellable, GError** error);
};
...
gint g_datagram_based_receive_messages(
    GDatagramBased* datagram_based, GInputMessage* messages, guint num_messages,
    gint flags, gint64 timeout, GCancellable* cancellable, GError** error);
