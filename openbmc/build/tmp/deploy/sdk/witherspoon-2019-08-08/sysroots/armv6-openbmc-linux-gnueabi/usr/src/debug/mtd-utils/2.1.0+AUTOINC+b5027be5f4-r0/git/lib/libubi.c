...
/**
 * dev_read_ll - read a positive 'long long' value from an UBI device sysfs
 * file.
 * @patt: file pattern to read from
 * @dev_num: UBI device number
 * @value: the result is stored here
...
/**
 * vol_read_ll - read a positive 'long long' value from an UBI volume sysfs
 * file.
 * @patt: file pattern to read from
 * @dev_num: UBI device number
 * @vol_id: volume ID
 ...
 */
static int dev_get_major(struct libubi* lib, int dev_num, int* major,
                         int* minor)
{
...
static int mtd_node_to_num(const char* mtd_dev_node)
{
    ...
    {
        errno = EINVAL;
        return sys_errmsg("\"%s\" is not a character device", mtd_dev_node);
    }
...
out_not_ubi:
    errmsg("\"%s\" has major:minor %d:%d, but this does not correspond to "
           "any existing UBI device or volume",
           node, major, minor);
    errno = ENODEV;
    return -1;
}
...
int ubi_get_info(libubi_t desc, struct ubi_info* info)
{
    ...
    while (1)
    {
        ...
        {
            errmsg("invalid entry in %s: \"%s\"", lib->sysfs_ubi,
                   dirent->d_name);
            errno = EINVAL;
            goto out_close;
        }

        ret = sscanf(dirent->d_name, UBI_DEV_NAME_PATT "%s", &dev_num, tmp_buf);
        if (ret == 1)
        {
            info->dev_count += 1;
                      ...
                      struct ubi_vol_info* info)
{
    ...
    ret = vol_read_int(lib->vol_alignment, dev_num, vol_id, &info->alignment);
    if (ret)
        return -1;
    ret = vol_read_ll(lib->vol_data_bytes, dev_num, vol_id, &info->data_bytes);
    if (ret)
        return -1;
    ...
    if (ret)
        return -1;
    ret = vol_read_int(lib->vol_corrupted, dev_num, vol_id, &info->corrupted);
    if (ret)
        return -1;
