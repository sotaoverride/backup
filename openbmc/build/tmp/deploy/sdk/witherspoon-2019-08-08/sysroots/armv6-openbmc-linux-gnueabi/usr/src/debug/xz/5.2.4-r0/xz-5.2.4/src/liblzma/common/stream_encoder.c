...
static lzma_ret stream_encode(void* coder_ptr, const lzma_allocator* allocator,
        ...
        switch (coder->sequence)
        {
            ...
            case SEQ_BLOCK_INIT:
            {
                if (*in_pos == in_size)
                {
                    ...
                    return_if_error(lzma_index_encoder_init(
                        &coder->index_encoder, allocator, coder->index));
            ...
            case SEQ_BLOCK_ENCODE:
            {
                static const lzma_action convert[LZMA_ACTION_MAX + 1] = {
                    LZMA_RUN,    LZMA_SYNC_FLUSH, LZMA_FINISH,
                    LZMA_FINISH, LZMA_FINISH,
                };

                const lzma_ret ret = coder->block_encoder.code(
                    coder->block_encoder.coder, allocator, in, in_pos, in_size,
...
extern LZMA_API(lzma_ret)
    lzma_stream_encoder(lzma_stream* strm, const lzma_filter* filters,
                        lzma_check check)
{
