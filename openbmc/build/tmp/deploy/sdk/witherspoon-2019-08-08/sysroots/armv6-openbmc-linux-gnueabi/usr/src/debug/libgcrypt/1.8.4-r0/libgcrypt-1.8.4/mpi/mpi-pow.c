 ...
 */
void _gcry_mpi_powm(gcry_mpi_t res, gcry_mpi_t base, gcry_mpi_t expo,
                    gcry_mpi_t mod)
{
 ...
 */
static void mul_mod(mpi_ptr_t xp, mpi_size_t* xsize_p, mpi_ptr_t rp,
                    ...
                    gcry_mpi_t mod)
{
    ...
    unsigned int xp_nlimbs = 0;
    mpi_ptr_t
        precomp[SIZE_PRECOMP]; /* Pre-computed array: BASE^1, ^3, ^5, ... */
    ...
    /* Main processing.  */
    {
        ...
        { /* PRECOMP[i] = BASE^(2 * i + 1) */
            if (xsize >= base_u_size)
                mul_mod(rp, &rsize, xp, xsize, base_u, base_u_size, mp, msize,
                        &karactx);
            else
                mul_mod(rp, &rsize, base_u, base_u_size, xp, xsize, mp, msize,
                        &karactx);
        ...
        for (;;)
            ...
            last_step:
                ...
                for (j += W - c0; j >= 0; j--)
                {
                    ...
                    mul_mod(xp, &xsize, rp, rsize, base_u, base_u_size, mp,
                            msize, &karactx);
                    tp = rp;
                    rp = xp;
                    xp = tp;
