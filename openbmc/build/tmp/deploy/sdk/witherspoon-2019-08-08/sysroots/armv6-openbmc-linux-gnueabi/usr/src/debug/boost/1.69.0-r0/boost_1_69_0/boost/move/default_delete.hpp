...
//!\file
//! Describes the default deleter (destruction policy) of <tt>unique_ptr</tt>:
//! <tt>default_delete</tt>.
...
// Avoid "defaulted on its first declaration must not have an
// exception-specification" error for GCC 4.6
#if !defined(BOOST_GCC) || (BOOST_GCC < 40600 && BOOST_GCC >= 40700) ||        \
    ...
    //! <b>Effects</b>: Constructs a default_delete object from another
    //! <tt>default_delete<U></tt> object.
    //!
    ...
    //!   - If T is an array type and U* is a more CV qualified pointer to
    //!   remove_extent<T>::type.
    template <class U>
    default_delete(const default_delete<U>& BOOST_MOVE_DOCIGN(
        BOOST_MOVE_I typename bmupd::enable_def_del<U BOOST_MOVE_I T>::type* =
            0)) BOOST_NOEXCEPT
    {
        // If T is not an array type, U derives from T
        // and T has no virtual destructor, then you have a problem
        BOOST_STATIC_ASSERT(
            (!::boost::move_upmu::missing_virtual_destructor<default_delete,
                                                             U>::value));
    }
    ...
    BOOST_MOVE_DOC1ST(default_delete&,
                      typename bmupd::enable_def_del<
                          U BOOST_MOVE_I T BOOST_MOVE_I default_delete&>::type)
    ...
    //! <b>Effects</b>: if T is not an array type, calls <tt>delete</tt> on
    //! static_cast<T*>(ptr),
    //!   otherwise calls <tt>delete[]</tt> on
    //!   static_cast<remove_extent<T>::type*>(ptr).
    //!
    ...
    //!         remove_cv<remove_extent<T>::type>::type and U* is convertible to
    //!         remove_extent<T>::type*.
    template <class U>
    BOOST_MOVE_DOC1ST(void, typename bmupd::enable_defdel_call<
                                U BOOST_MOVE_I T BOOST_MOVE_I void>::type)
    operator()(U* ptr) const BOOST_NOEXCEPT
    {
        ...
        move_upd::call_delete(
            p, move_upd::is_array_del<bmupmu::is_array<T>::value>());
    }

    //! <b>Effects</b>: Same as
    //! <tt>(*this)(static_cast<element_type*>(nullptr))</tt>.
    //!
    void
        operator()(BOOST_MOVE_DOC0PTR(bmupd::nullptr_type)) const BOOST_NOEXCEPT
    {
