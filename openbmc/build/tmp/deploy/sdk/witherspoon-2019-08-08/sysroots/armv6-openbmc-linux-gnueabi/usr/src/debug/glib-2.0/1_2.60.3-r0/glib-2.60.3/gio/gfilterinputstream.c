...
static void g_filter_input_stream_set_property(GObject* object, guint prop_id,
                                               const GValue* value,
                                               GParamSpec* pspec);

static void g_filter_input_stream_get_property(GObject* object, guint prop_id,
                                               GValue* value,
                                               GParamSpec* pspec);
static void g_filter_input_stream_finalize(GObject* object);

static gssize g_filter_input_stream_read(GInputStream* stream, void* buffer,
                                         gsize count, GCancellable* cancellable,
                                         GError** error);
static gssize g_filter_input_stream_skip(GInputStream* stream, gsize count,
...
G_DEFINE_ABSTRACT_TYPE_WITH_PRIVATE(GFilterInputStream, g_filter_input_stream,
                                    G_TYPE_INPUT_STREAM)
...
static void g_filter_input_stream_class_init(GFilterInputStreamClass* klass)
{
    ...
    g_object_class_install_property(
        ...
        g_param_spec_object(
            "base-stream", P_("The Filter Base Stream"),
    ...
    g_object_class_install_property(
        object_class, PROP_CLOSE_BASE,
        g_param_spec_boolean("close-base-stream", P_("Close Base Stream"),
                             P_("If the base stream should be closed when the "
    ...
    switch (prop_id)
    {
        ...
        case PROP_CLOSE_BASE:
            g_filter_input_stream_set_close_base_stream(
                filter_stream, g_value_get_boolean(value));
            break;
                                            ...
                                            GError** error)
{
    ...
    if (priv->close_base)
    {
        res = g_input_stream_close(filter_stream->base_stream, cancellable,
                                   error);
    }
