 ...
 * @authorize_authenticated_peer: Signal class handler for the
 * #GDBusAuthObserver::authorize-authenticated-peer signal.
 *
...
static gboolean g_dbus_auth_observer_authorize_authenticated_peer_real(
...
static void g_dbus_auth_observer_class_init(GDBusAuthObserverClass* klass)
{
     ...
     * Since: 2.26
     */
    signals[AUTHORIZE_AUTHENTICATED_PEER_SIGNAL] = g_signal_new(
        I_("authorize-authenticated-peer"), G_TYPE_DBUS_AUTH_OBSERVER,
        G_SIGNAL_RUN_LAST,
        G_STRUCT_OFFSET(GDBusAuthObserverClass, authorize_authenticated_peer),
        _g_signal_accumulator_false_handled, NULL, /* accu_data */
        NULL, G_TYPE_BOOLEAN, 2, G_TYPE_IO_STREAM, G_TYPE_CREDENTIALS);
     ...
     *
     * Returns: %TRUE if @mechanism can be used to authenticate the other peer,
     * %FALSE if not.
     *
     * Since: 2.34
     */
    signals[ALLOW_MECHANISM_SIGNAL] = g_signal_new(
        I_("allow-mechanism"), G_TYPE_DBUS_AUTH_OBSERVER, G_SIGNAL_RUN_LAST,
        G_STRUCT_OFFSET(GDBusAuthObserverClass, allow_mechanism),
        _g_signal_accumulator_false_handled, NULL, /* accu_data */
        NULL, G_TYPE_BOOLEAN, 1, G_TYPE_STRING);
}
    ...
    GDBusAuthObserver* observer, GIOStream* stream, GCredentials* credentials)
{
    ...
    denied = FALSE;
    g_signal_emit(observer, signals[AUTHORIZE_AUTHENTICATED_PEER_SIGNAL], 0,
                  stream, credentials, &denied);
    return denied;
}
                                              ...
                                              const gchar* mechanism)
{
    ...
    ret = FALSE;
    g_signal_emit(observer, signals[ALLOW_MECHANISM_SIGNAL], 0, mechanism,
                  &ret);
    return ret;
}
