...
krb5_kvno get_next_kvno(krb5_context context, krb5_db_entry* entry)
{
    krb5_kvno new_kvno;

    new_kvno =
        krb5_db_get_key_data_kvno(context, entry->n_key_data, entry->key_data);
    new_kvno++;
    /* deal with wrapping */
    if (new_kvno == 0)
        new_kvno =
            1; /* knvo must not be 0 as this is special value (IGNORE_VNO) */

    return (new_kvno);
}

krb5_error_code add_new_mkey(krb5_context context, krb5_db_entry* master_entry,
                             krb5_keyblock* new_mkey, krb5_kvno use_mkvno)
{
         ...
         keylist_node = keylist_node->next, i++)
    {
        ...
        retval = krb5_dbe_encrypt_key_data(
            context, new_mkey, &keylist_node->keyblock, NULL,
            (int)keylist_node->kvno, &master_entry->key_data[i]);
        if (retval)
            goto clean_n_exit;
    }
...
void kdb5_add_mkey(int argc, char* argv[])
{
    int optchar;
    ...
    retval =
        krb5_db_get_principal(util_context, master_princ, 0, &master_entry);
    if (retval != 0)
    {
    ...
    retval = krb5_c_string_to_key(util_context, new_master_enctype, &pwd,
                                  &master_salt, &new_mkeyblock);
    if (retval)
    {
        com_err(progname, retval,
    ...
    retval =
        add_new_mkey(util_context, master_entry, &new_mkeyblock, new_mkey_kvno);
    if (retval)
    {
        com_err(progname, retval,
    ...
    {
        com_err(progname, retval,
                _("while updating the master key principal "
                  "modification time"));
        exit_status++;
        goto cleanup_return;
    }
    ...
    {
        com_err(progname, retval,
                _("while adding master key entry to the "
                  "database"));
        exit_status++;
        goto cleanup_return;
    }
    ...
    if (do_stash)
    {
        retval = krb5_db_store_master_key(
            util_context, global_params.stash_file, master_princ, new_mkey_kvno,
            &new_mkeyblock, mkey_password);
        if (retval)
        {
...
void kdb5_use_mkey(int argc, char* argv[])
{
    krb5_error_code retval;
    krb5_kvno use_kvno;
     ...
     */
    for (prev_actkvno = NULL, cur_actkvno = actkvno_list; cur_actkvno != NULL;
    ...
    if ((retval =
             krb5_dbe_update_actkvno(util_context, master_entry, actkvno_list)))
    {
        com_err(progname, retval,
...
void kdb5_list_mkeys(int argc, char* argv[])
{
    krb5_error_code retval;
         ...
         cur_kb_node = cur_kb_node->next)
    {
        ...
        if ((retval = krb5_enctype_to_name(cur_kb_node->keyblock.enctype, FALSE,
        ...
        else
        {
            ...
            {
                retval = asprintf(
                    &output_str, _("KVNO: %d, Enctype: %s, Active on: %s\n"),
                    cur_kb_node->kvno, enctype, strdate(act_time));
            }
            else
            {
        ...
        if (retval == -1)
        {
            com_err(progname, ENOMEM,
                    _("asprintf could not allocate enough "
...
static int update_princ_encryption_1(void* cb, krb5_db_entry* ent)
{
    ...
    if ((retval = krb5_dbe_update_mod_princ_data(util_context, ent, now,
                                                 master_princ)))
    {
        com_err(progname, retval,
    ...
    {
        com_err(progname, retval,
                _("while updating principal '%s' key data "
                  "in the database"),
                pname);
        goto fail;
    }
...
int are_you_sure(const char* format, ...)
{
    va_list va;
    char ansbuf[100];
...
void kdb5_update_princ_encryption(int argc, char* argv[])
{
...
static krb5_error_code find_mkvnos_in_use(krb5_pointer ptr,
                                          krb5_db_entry* entry)
{
    krb5_error_code retval;
...
void kdb5_purge_mkeys(int argc, char* argv[])
{
    int optchar;
    krb5_error_code retval;
    krb5_timestamp now;
    ...
    krb5_mkey_aux_node *mkey_aux_list = NULL, *mkey_aux_entry,
                       *prev_mkey_aux_entry;
    ...
    args.kvnos = (struct kvnos_in_use*)malloc(sizeof(struct kvnos_in_use) *
                                              old_key_data_count);
    if (args.kvnos == NULL)
    {
        retval = ENOMEM;
    ...
    if ((retval = krb5_db_iterate(util_context, NULL, find_mkvnos_in_use,
                                  (krb5_pointer)&args, 0)))
    {
    ...
    retval =
        krb5_dbe_lookup_mkey_aux(util_context, master_entry, &mkey_aux_list);
    if (retval != 0)
    {
    ...
    master_entry->key_data =
        (krb5_key_data*)malloc(sizeof(krb5_key_data) * num_kvnos_inuse);
    if (master_entry->key_data == NULL)
    {
        retval = ENOMEM;
    ...
    master_entry->n_key_data =
        num_kvnos_inuse; /* there's only 1 mkey per kvno */
