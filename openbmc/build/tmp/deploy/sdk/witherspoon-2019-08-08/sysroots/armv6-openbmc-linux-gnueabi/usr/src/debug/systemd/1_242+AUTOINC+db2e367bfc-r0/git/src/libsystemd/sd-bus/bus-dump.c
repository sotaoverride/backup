...
int bus_message_dump(sd_bus_message* m, FILE* f, unsigned flags)
{
    ...
    if (flags & BUS_MESSAGE_DUMP_WITH_HEADER)
    {
                ...
                m->header->type == SD_BUS_MESSAGE_METHOD_ERROR
                    ...
                    : m->header->type == SD_BUS_MESSAGE_METHOD_RETURN
                          ...
                          : m->header->type != SD_BUS_MESSAGE_SIGNAL
                                ? ansi_highlight()
                                : "",
                special_glyph(SPECIAL_GLYPH_TRIANGULAR_BULLET), ansi_normal(),

                ansi_highlight(),
                bus_message_type_to_string(m->header->type) ?: "(unknown)",
                ansi_normal(),

                m->header->endian, m->header->flags, m->header->version,
                m->priority);
        ...
        if (m->sender)
            fprintf(f, "  Sender=%s%s%s", ansi_highlight(), m->sender,
                    ansi_normal());
        if (m->destination)
            fprintf(f, "  Destination=%s%s%s", ansi_highlight(), m->destination,
                    ansi_normal());
        if (m->path)
            fprintf(f, "  Path=%s%s%s", ansi_highlight(), m->path,
                    ansi_normal());
        if (m->interface)
            fprintf(f, "  Interface=%s%s%s", ansi_highlight(), m->interface,
                    ansi_normal());
        if (m->member)
            fprintf(f, "  Member=%s%s%s", ansi_highlight(), m->member,
                    ansi_normal());
        ...
        if (sd_bus_error_is_set(&m->error))
            fprintf(f,
                    ...
                    ansi_highlight_red(), strna(m->error.message),
                    ansi_normal());
        ...
        switch (type)
        {
            ...
            case SD_BUS_TYPE_BYTE:
                fprintf(f, "%sBYTE %s%u%s;\n", prefix, ansi_highlight(),
                        basic.u8, ansi_normal());
                break;

            case SD_BUS_TYPE_BOOLEAN:
                fprintf(f, "%sBOOLEAN %s%s%s;\n", prefix, ansi_highlight(),
                        true_false(basic.i), ansi_normal());
                break;

            case SD_BUS_TYPE_INT16:
                fprintf(f, "%sINT16 %s%i%s;\n", prefix, ansi_highlight(),
                        basic.s16, ansi_normal());
                break;

            case SD_BUS_TYPE_UINT16:
                fprintf(f, "%sUINT16 %s%u%s;\n", prefix, ansi_highlight(),
                        basic.u16, ansi_normal());
                break;

            case SD_BUS_TYPE_INT32:
                fprintf(f, "%sINT32 %s%i%s;\n", prefix, ansi_highlight(),
                        basic.s32, ansi_normal());
                break;

            case SD_BUS_TYPE_UINT32:
                fprintf(f, "%sUINT32 %s%u%s;\n", prefix, ansi_highlight(),
                        basic.u32, ansi_normal());
                break;

            case SD_BUS_TYPE_INT64:
                fprintf(f, "%sINT64 %s%" PRIi64 "%s;\n", prefix,
                        ansi_highlight(), basic.s64, ansi_normal());
                break;

            case SD_BUS_TYPE_UINT64:
                fprintf(f, "%sUINT64 %s%" PRIu64 "%s;\n", prefix,
                        ansi_highlight(), basic.u64, ansi_normal());
                break;

            case SD_BUS_TYPE_DOUBLE:
                fprintf(f, "%sDOUBLE %s%g%s;\n", prefix, ansi_highlight(),
                        basic.d64, ansi_normal());
                break;

            case SD_BUS_TYPE_STRING:
                fprintf(f, "%sSTRING \"%s%s%s\";\n", prefix, ansi_highlight(),
                        basic.string, ansi_normal());
                break;

            case SD_BUS_TYPE_OBJECT_PATH:
                fprintf(f, "%sOBJECT_PATH \"%s%s%s\";\n", prefix,
                        ansi_highlight(), basic.string, ansi_normal());
                break;
...
static void dump_capabilities(sd_bus_creds* c, FILE* f, const char* name,
                              bool terse,
                              int (*has)(sd_bus_creds* c, int capability))
{
    ...
    fprintf(f, "%s%s=%s", terse ? "  " : "", name,
            terse ? "" : ansi_highlight());
...
int bus_creds_dump(sd_bus_creds* c, FILE* f, bool terse)
{
    ...
    if (c->mask & SD_BUS_CREDS_PPID)
    {
        if (c->ppid == 0)
            ...
            fprintf(f, "%sPPID=%s" PID_FMT "%s", prefix, color, c->ppid,
                    suffix);
    }
    ...
    if (terse && ((c->mask & (SD_BUS_CREDS_PID | SD_BUS_CREDS_TID |
                              SD_BUS_CREDS_PPID | SD_BUS_CREDS_TTY))))
        fputs("\n", f);
    ...
    if (c->mask & SD_BUS_CREDS_SUPPLEMENTARY_GIDS)
    {
        ...
        for (i = 0; i < c->n_supplementary_gids; i++)
            fprintf(f, "%s" GID_FMT, i > 0 ? " " : "",
                    c->supplementary_gids[i]);
        fprintf(f, "%s", suffix);
    }

    if (terse &&
        ((c->mask & (SD_BUS_CREDS_UID | SD_BUS_CREDS_EUID | SD_BUS_CREDS_SUID |
    ...
    if (terse && (c->mask & (SD_BUS_CREDS_EXE | SD_BUS_CREDS_COMM |
    ...
    if (c->mask & SD_BUS_CREDS_DESCRIPTION)
        fprintf(f, "%sDescription=%s%s%s", prefix, color, c->description,
                suffix);

    if (terse &&
        (c->mask & (SD_BUS_CREDS_SELINUX_CONTEXT | SD_BUS_CREDS_DESCRIPTION)))
        fputs("\n", f);
    ...
    if (terse &&
        ((c->mask & SD_BUS_CREDS_CGROUP) || r != -ENODATA || q != -ENODATA ||
         v != -ENODATA || w != -ENODATA || z != -ENODATA))
        fputs("\n", f);

    r = sd_bus_creds_get_audit_login_uid(c, &audit_loginuid);
    if (r >= 0)
        fprintf(f, "%sAuditLoginUID=%s" UID_FMT "%s", prefix, color,
                audit_loginuid, suffix);
    else if (r != -ENODATA)
        fprintf(f, "%sAuditLoginUID=%sn/a%s", prefix, color, suffix);
    q = sd_bus_creds_get_audit_session_id(c, &audit_sessionid);
    if (q >= 0)
        fprintf(f, "%sAuditSessionID=%s%" PRIu32 "%s", prefix, color,
                audit_sessionid, suffix);
    else if (q != -ENODATA)
    ...
    if (c->mask & SD_BUS_CREDS_UNIQUE_NAME)
        fprintf(f, "%sUniqueName=%s%s%s", prefix, color, c->unique_name,
                suffix);
    ...
    dump_capabilities(c, f, "InheritableCapabilities", terse,
