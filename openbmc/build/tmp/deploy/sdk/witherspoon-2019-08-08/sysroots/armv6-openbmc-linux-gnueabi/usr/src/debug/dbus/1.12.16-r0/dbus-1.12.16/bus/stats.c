                                       ...
                                       DBusMessage* message, DBusError* error)
{
    ...
    if (!_dbus_asv_add_uint32(&arr_iter, "Serial", stats_serial++) ||
        ...
        !_dbus_asv_add_uint32(&arr_iter, "ListMemPoolCachedBytes",
                              in_free_list) ||
        !_dbus_asv_add_uint32(&arr_iter, "ListMemPoolAllocatedBytes",
                              allocated))
    {
...
dbus_bool_t bus_stats_handle_get_connection_stats(
    DBusConnection* caller_connection, BusTransaction* transaction,
    DBusMessage* message, DBusError* error)
{
    ...
    dbus_uint32_t out_messages, out_bytes, out_fds, out_peak_bytes,
        out_peak_fds;
    DBusConnection* stats_connection;

    _DBUS_ASSERT_ERROR_IS_CLEAR(error);

    found = bus_driver_get_conn_helper(caller_connection, message, "statistics",
                                       NULL, &stats_connection, error);
    ...
    _dbus_connection_get_stats(stats_connection, &in_messages, &in_bytes,
                               &in_fds, &in_peak_bytes, &in_peak_fds,
                                         ...
                                         DBusMessage* message, DBusError* error)
{
    ...
    for (i = 0; i < services_len; i++)
    {
        ...
        if (!dbus_message_iter_open_container(&hash_iter, DBUS_TYPE_DICT_ENTRY,
                                              NULL, &entry_iter))
        {
