...
static errcode_t init_module(struct profile_vtable* vtable, void* cbdata,
...
static errcode_t parse_modspec(const char* modspec, char** ret_path,
                               char** ret_residual)
{
    const char* p;
...
static errcode_t init_load_module(const char* modspec, profile_t* ret_profile)
{
    ...
    if (err)
        goto cleanup;
    err =
        krb5int_get_plugin_func(plhandle, "profile_module_init", &fptr, &einfo);
    if (err == ENOENT)
...
errcode_t KRB5_CALLCONV profile_init_flags(const_profile_filespec_t* files,
...
errcode_t KRB5_CALLCONV profile_init(const_profile_filespec_t* files,
                                     profile_t* ret_profile)
{
    return profile_init_flags(files, 0, ret_profile);
}

errcode_t KRB5_CALLCONV profile_init_vtable(struct profile_vtable* vtable,
                                            void* cbdata,
                                            profile_t* ret_profile)
{
...
static errcode_t copy_vtable_profile(profile_t profile,
                                     profile_t* ret_new_profile)
{
    errcode_t err;
    void* cbdata;
...
errcode_t KRB5_CALLCONV profile_copy(profile_t old_profile,
                                     profile_t* new_profile)
{
    size_t size, i;
    ...
    if (files == NULL)
        return ENOMEM;
    for (i = 0, file = old_profile->first_file; i < size;
         i++, file = file->next)
...
errcode_t KRB5_CALLCONV profile_init_path(
    const_profile_filespec_list_t filepath, profile_t* ret_profile)
{
    ...
    for (s = filepath, i = 0; (t = strchr(s, ':')) || (t = s + strlen(s));
         s = t + 1, i++)
    {
...
errcode_t KRB5_CALLCONV profile_is_writable(profile_t profile, int* writable)
{
...
errcode_t KRB5_CALLCONV profile_is_modified(profile_t profile, int* modified)
{
    if (!profile || profile->magic != PROF_MAGIC_PROFILE)
        return PROF_MAGIC_PROFILE;
...
errcode_t KRB5_CALLCONV profile_flush(profile_t profile)
{
    if (!profile || profile->magic != PROF_MAGIC_PROFILE)
        return PROF_MAGIC_PROFILE;
...
errcode_t KRB5_CALLCONV profile_flush_to_file(profile_t profile,
                                              const_profile_filespec_t outfile)
{
    ...
    if (profile->first_file)
        return profile_flush_file_to_file(profile->first_file, outfile);

    return 0;
}

errcode_t KRB5_CALLCONV profile_flush_to_buffer(profile_t profile, char** buf)
{
    if (profile->vt)
        return PROF_UNSUPPORTED;
 ...
 */
errcode_t profile_ser_size(const char* unused, profile_t profile, size_t* sizep)
{
    size_t required;
    prf_file_t pfp;
...
static int unpack_int32(int32_t* intp, unsigned char** bufpp, size_t* remainp)
{
                                  ...
                                  unsigned char** bufpp, size_t* remainp)
{
    ...
    flist = (profile_filespec_t*)malloc(sizeof(profile_filespec_t) *
                                        (size_t)(fcount + 1));
    if (!flist)
        goto cleanup;
    ...
    if (unpack_int32(&tmp, &bp, &remain) || (tmp != PROF_MAGIC_PROFILE))
    {
        retval = EINVAL;
        goto cleanup;
    }
