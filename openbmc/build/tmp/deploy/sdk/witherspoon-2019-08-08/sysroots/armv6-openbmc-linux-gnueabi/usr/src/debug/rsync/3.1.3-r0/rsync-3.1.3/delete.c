...
enum delret delete_item(char* fbuf, uint16 mode, uint16 flags)
{
    ...
    {
        if (make_backups > 0 && !(flags & DEL_FOR_BACKUP) &&
    ...
    else
    {
        ...
        {
            rsyserr(FERROR_XFER, errno, "delete_file: %s(%s) failed", what,
                    fbuf);
            ret = DR_FAILURE;
        }
        else
    ...
    if (ret != DR_SUCCESS && flags & DEL_MAKE_ROOM)
    {
        ...
        switch (flags & DEL_MAKE_ROOM)
        {
            ...
            case DEL_FOR_DIR:
                desc = "directory";
                break;
            case DEL_FOR_SYMLINK:
                desc = "symlink";
                break;
            case DEL_FOR_DEVICE:
                desc = "device file";
                break;
            case DEL_FOR_SPECIAL:
                desc = "special file";
                break;
            default:
        ...
        rprintf(FERROR_XFER, "could not make way for %s %s: %s\n",
                flags & DEL_FOR_BACKUP ? "backup" : "new", desc, fbuf);
    }
    return ret;
}
