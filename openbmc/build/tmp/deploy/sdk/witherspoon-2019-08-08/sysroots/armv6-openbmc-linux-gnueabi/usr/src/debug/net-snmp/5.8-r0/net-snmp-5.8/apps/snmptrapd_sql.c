 ...
 */
static void _parse_queue_fmt(const char* token, char* cptr)
{
 ...
 */
static void _parse_interval_fmt(const char* token, char* cptr)
{
    _sql.queue_interval = atoi(cptr);
    DEBUGMSGTL(
        ("sql:queue", "queue interval now %d seconds\n", _sql.queue_interval));
}
...
{
    register_config_handler("snmptrapd", "sqlMaxQueue", _parse_queue_fmt, NULL,
                            "integer");
    register_config_handler("snmptrapd", "sqlSaveInterval", _parse_interval_fmt,
                            NULL, "seconds");
}

static void netsnmp_sql_disconnected(void)
{
 ...
 */
static void netsnmp_sql_error(const char* message)
{
    ...
    if (_sql.conn != NULL)
    {
        ...
        snmp_log(LOG_ERR, "Error %u (%s): %s\n", err, mysql_sqlstate(_sql.conn),
                 mysql_error(_sql.conn));
#else
        snmp(LOG_ERR, "Error %u: %s\n", mysql_errno(_sql.conn),
             mysql_error(_sql.conn));
#endif
    }
 ...
 */
static void netsnmp_sql_stmt_error(MYSQL_STMT* stmt, const char* message)
{
    u_int err = mysql_errno(_sql.conn);
 ...
 */
static void netsnmp_mysql_cleanup(void)
{
 ...
 */
static int netsnmp_mysql_bind(const char* text, size_t text_size,
 ...
 */
static int netsnmp_mysql_connect(void)
{
    char trap_stmt[] =
        "INSERT INTO notifications "
        "(date_time, host, auth, type, version, request_id, snmpTrapOID, "
        "transport, security_model, v3msgid, v3security_level, v3context_name, "
        "v3context_engine, v3security_name, v3security_engine) "
        "VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
    ...
    if (0 !=
        netsnmp_mysql_bind(vb_stmt, sizeof(vb_stmt), &_sql.vb_stmt, _vbind))
    {
        mysql_stmt_close(_sql.trap_stmt);
        _sql.trap_stmt = NULL;
        goto err;
...
int netsnmp_mysql_init(void)
{
    ...
    {
        DEBUGMSGTL(
            ("sql:init", "mysql not enabled (sqlSaveInterval is <= 0)\n"));
        return 0;
    }
    ...
    _tbind[TBIND_v3_MSGID].is_null = _tbind[TBIND_v3_SECURITY_LEVEL].is_null =
    ...
    /** add handler */
    traph =
        netsnmp_add_global_traphandler(NETSNMPTRAPD_PRE_HANDLER, mysql_handler);
    if (NULL == traph)
    {
 ...
 */
static void _sql_log(sql_buf* sqlb, void* dontcare)
{
    netsnmp_iterator* it;
    sql_vb_buf* sqlvb;
    ...
    snmp_log(LOG_ERR,
             ...
             sqlb->time.year, sqlb->time.month, sqlb->time.day, sqlb->time.hour,
             sqlb->time.minute, sqlb->time.second, sqlb->user, sqlb->type,
             sqlb->version, sqlb->reqid, sqlb->oid, sqlb->transport,
             sqlb->security_model, sqlb->msgid, sqlb->security_level,
             sqlb->context, sqlb->context_engine, sqlb->security_name,
    ...
    if (NULL == it)
    {
        snmp_log(LOG_ERR, "error creating iterator; incomplete trap logged\n");
        return;
    }
 ...
 * to CONTAINER_FOR_EACH.
 */
static void _sql_vb_buf_free(sql_vb_buf* sqlvb, void* dontcare)
{
    if (NULL == sqlvb)
        return;
 ...
 * to CONTAINER_FOR_EACH.
 */
static void _sql_buf_free(sql_buf* sqlb, void* dontcare)
{
    if (NULL == sqlb)
        return;
 ...
 */
static sql_buf* _sql_buf_get(void)
{
    sql_buf* sqlb;
 ...
 */
static int _sql_save_trap_info(sql_buf* sqlb, netsnmp_pdu* pdu,
                               netsnmp_transport* transport)
{
    ...
    buf_host_len_t = 0;
    tmp_size = 0;
    realloc_format_trap((u_char**)&sqlb->host, &tmp_size, &buf_host_len_t, 1,
                        "%B", pdu, transport);
    ...
    tmp_size = 0;
    buf_user_len_t = 0;
    realloc_format_trap((u_char**)&sqlb->user, &tmp_size, &buf_user_len_t, 1,
                        "%u", pdu, transport);
    ...
    if ((SNMP_MP_MODEL_SNMPv3 + 1) == sqlb->version)
    {
        ...
        if (pdu->securityName)
        {
            sqlb->security_name = netsnmp_strdup_and_null(
                (u_char*)pdu->securityName, pdu->securityNameLen);
 ...
 * return 0 on success, anything else is an error
 */
static int _sql_save_varbind_info(sql_buf* sqlb, netsnmp_pdu* pdu)
{
    ...
    while (var)
    {
        ...
        tmp_size = 0;
        buf_val_len_t = 0;
        sprint_realloc_by_type((u_char**)&sqlvb->val, &tmp_size, &buf_val_len_t,
                               1, var, NULL, NULL, NULL);
 ...
 * sql trap handler
 */
int mysql_handler(netsnmp_pdu* pdu, netsnmp_transport* transport,
 ...
 */
static void _sql_save(sql_buf* sqlb, void* dontcare)
{
    netsnmp_iterator* it;
    sql_vb_buf* sqlvb;
    u_long trap_id;
    ...
    for (sqlvb = ITERATOR_FIRST(it); sqlvb; sqlvb = ITERATOR_NEXT(it))
    {
        ...
        if (mysql_stmt_execute(_sql.vb_stmt) != 0)
        {
            netsnmp_sql_stmt_error(
                _sql.vb_stmt, "Could not execute insert statement for varbind");
            _sql_log(sqlb, NULL);
            break;
        }
    }
    ...
    if (_sql.connected)
    {
        ...
        { /* nuts... now what? */
            ...
            CONTAINER_FOR_EACH(_sql.queue,
                               (netsnmp_container_obj_func*)_sql_log, NULL);
        }
    }
