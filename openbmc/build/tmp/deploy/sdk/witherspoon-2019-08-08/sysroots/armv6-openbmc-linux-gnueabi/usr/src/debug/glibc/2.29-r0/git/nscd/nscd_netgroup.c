...
retry:;
    ...
    {
        struct datahead* found = __nscd_cache_search(
            GETNETGRENT, group, group_len, mapped, sizeof netgroup_resp);
        if (found != NULL)
        {
    ...
    if (respdata == NULL)
    {
        sock = __nscd_open_socket(group, group_len, GETNETGRENT, &netgroup_resp,
...
retry:;
    ...
    {
        struct datahead* found = __nscd_cache_search(
            INNETGR, key, key_len, mapped, sizeof innetgroup_resp);
        if (found != NULL)
        {
    ...
    sock = __nscd_open_socket(key, key_len, INNETGR, &innetgroup_resp,
