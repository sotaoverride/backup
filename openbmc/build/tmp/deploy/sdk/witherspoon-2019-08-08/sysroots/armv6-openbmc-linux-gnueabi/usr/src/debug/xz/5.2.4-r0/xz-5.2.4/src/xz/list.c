...
static const char* xz_ver_to_str(uint32_t ver)
{
    ...
    snprintf(buf, sizeof(buf), "%u.%u.%u%s", major, minor, patch, stability);
    return buf;
}
...
static bool parse_block_header(file_pair* pair, const lzma_index_iter* iter,
...
#error IO_BUFFER_SIZE < LZMA_BLOCK_HEADER_SIZE_MAX
    ...
    bhi->flags[0] = block.compressed_size != LZMA_VLI_UNKNOWN ? 'c' : '-';
    bhi->flags[1] = block.uncompressed_size != LZMA_VLI_UNKNOWN ? 'u' : '-';
    bhi->flags[2] = '\0';
    ...
    switch (lzma_block_compressed_size(&block, iter->block.unpadded_size))
    {
        case LZMA_OK:
    ...
    // LZMA2 Block. This decoder bug was fixed in 5.0.2.
    {
        ...
        if (filters[i].id == LZMA_FILTER_LZMA2 &&
            iter->block.uncompressed_size == 0 && xfi->min_version < 50000022U)
...
data_error:
    ...
    message_error("%s: %s", pair->src_name, message_strm(LZMA_DATA_ERROR));
    return true;
}
...
static bool parse_check_value(file_pair* pair, const lzma_index_iter* iter)
{
    ...
    const off_t offset =
        iter->block.compressed_file_offset + iter->block.total_size - size;
    io_buf buf;
    ...
    if (size == 4)
        snprintf(check_value, sizeof(check_value), "%08" PRIx32,
                 conv32le(buf.u32[0]));
    else if (size == 8)
        snprintf(check_value, sizeof(check_value), "%016" PRIx64,
                 conv64le(buf.u64[0]));
    else
...
static bool parse_details(file_pair* pair, const lzma_index_iter* iter,
                          block_header_info* bhi, xz_file_info* xfi)
{
...
static void get_check_names(char buf[CHECKS_STR_SIZE], uint32_t checks,
    ...
    for (size_t i = 0; i <= LZMA_CHECK_ID_MAX; ++i)
    {
        if (checks & (UINT32_C(1) << i))
        {
            my_snprintf(&pos, &left, "%s%s", comma ? sep : "",
                        opt_robot ? check_names[i] : _(check_names[i]));
            comma = true;
        }
    }
...
static bool print_info_basic(const xz_file_info* xfi, file_pair* pair)
{
    ...
    const char* cols[7] = {
        ...
        uint64_to_nicestr(lzma_index_file_size(xfi->idx), NICESTR_B,
                          NICESTR_TIB, false, 2),
        uint64_to_nicestr(lzma_index_uncompressed_size(xfi->idx), NICESTR_B,
                          NICESTR_TIB, false, 3),
    ...
    printf("%*s %*s  %*s  %*s  %*s  %-*s %s\n", tuklib_mbstr_fw(cols[0], 5),
           cols[0], tuklib_mbstr_fw(cols[1], 7), cols[1],
           tuklib_mbstr_fw(cols[2], 11), cols[2], tuklib_mbstr_fw(cols[3], 11),
           cols[3], tuklib_mbstr_fw(cols[4], 5), cols[4],
           tuklib_mbstr_fw(cols[5], 7), cols[5], cols[6]);

    return false;
}

static void print_adv_helper(uint64_t stream_count, uint64_t block_count,
                             uint64_t compressed_size,
                             uint64_t uncompressed_size, uint32_t checks,
                             uint64_t stream_padding)
{
    ...
    printf(_("  Compressed size:    %s\n"),
           uint64_to_nicestr(compressed_size, NICESTR_B, NICESTR_TIB, true, 0));
    printf(
        _("  Uncompressed size:  %s\n"),
        uint64_to_nicestr(uncompressed_size, NICESTR_B, NICESTR_TIB, true, 0));
    printf(_("  Ratio:              %s\n"),
           get_ratio(compressed_size, uncompressed_size));
    printf(_("  Check:              %s\n"), checks_str);
    printf(_("  Stream padding:     %s\n"),
           uint64_to_nicestr(stream_padding, NICESTR_B, NICESTR_TIB, true, 0));
    return;
}
...
static bool print_info_adv(xz_file_info* xfi, file_pair* pair)
{
    ...
    print_adv_helper(
        lzma_index_stream_count(xfi->idx), lzma_index_block_count(xfi->idx),
        lzma_index_file_size(xfi->idx), lzma_index_uncompressed_size(xfi->idx),
        lzma_index_checks(xfi->idx), xfi->stream_padding);
    ...
    while (!lzma_index_iter_next(&iter, LZMA_INDEX_ITER_STREAM))
    {
        ...
        printf("    %*s %*s %*s %*s ", tuklib_mbstr_fw(cols1[0], 6), cols1[0],
        ...
        printf("%*s %*s  %*s  %-*s %*s\n", tuklib_mbstr_fw(cols2[0], 15),
               cols2[0], tuklib_mbstr_fw(cols2[1], 15), cols2[1],
    ...
    if (lzma_index_block_count(xfi->idx) > 0)
    {
        ...
        while (!lzma_index_iter_next(&iter, LZMA_INDEX_ITER_BLOCK))
        {
            ...
            printf("    %*s %*s %*s %*s ", tuklib_mbstr_fw(cols1[0], 6),
                   cols1[0], tuklib_mbstr_fw(cols1[1], 9), cols1[1],
                   tuklib_mbstr_fw(cols1[2], 15), cols1[2],
                   tuklib_mbstr_fw(cols1[3], 15), cols1[3]);
            ...
            const char* cols2[4] = {
                ...
                _(check_names[iter.stream.flags->check])};
            printf("%*s %*s  %*s  %-*s", tuklib_mbstr_fw(cols2[0], 15),
                   cols2[0], tuklib_mbstr_fw(cols2[1], 15), cols2[1],
                   tuklib_mbstr_fw(cols2[2], 5), cols2[2],
                   tuklib_mbstr_fw(cols2[3], detailed ? 11 : 1), cols2[3]);

            if (detailed)
            {
                const lzma_vli compressed_size =
                    iter.block.unpadded_size - bhi.header_size -
                    lzma_check_size(iter.stream.flags->check);
                ...
                const char* cols3[6] = {
                    ...
                    uint64_to_str(round_up_to_mib(bhi.memusage), 2),
                    bhi.filter_chain};
                // Show MiB for memory usage, because it
                // is the only size which is not in bytes.
                printf("%-*s  %*s  %-5s %*s %*s MiB  %s", checkval_width,
                       cols3[0], tuklib_mbstr_fw(cols3[1], 6), cols3[1],
                       cols3[2], tuklib_mbstr_fw(cols3[3], 15), cols3[3],
                       tuklib_mbstr_fw(cols3[4], 7), cols3[4], cols3[5]);
            }
...
static bool print_info_robot(xz_file_info* xfi, file_pair* pair)
{
    ...
    if (message_verbosity_get() >= V_VERBOSE)
    {
        ...
        while (!lzma_index_iter_next(&iter, LZMA_INDEX_ITER_STREAM))
            printf("stream\t%" PRIu64 "\t%" PRIu64 "\t%" PRIu64 "\t%" PRIu64
                   "\t%" PRIu64 "\t%" PRIu64 "\t%s\t%s\t%" PRIu64 "\n",
                   iter.stream.number, iter.stream.block_count,
                   iter.stream.compressed_offset,
                   iter.stream.uncompressed_offset, iter.stream.compressed_size,
                   iter.stream.uncompressed_size,
                   get_ratio(iter.stream.compressed_size,
                             iter.stream.uncompressed_size),
        ...
        while (!lzma_index_iter_next(&iter, LZMA_INDEX_ITER_BLOCK))
        {
            if (message_verbosity_get() >= V_DEBUG &&
                parse_details(pair, &iter, &bhi, xfi))
                return true;

            printf(
                "block\t%" PRIu64 "\t%" PRIu64 "\t%" PRIu64 "\t%" PRIu64
                "\t%" PRIu64 "\t%" PRIu64 "\t%" PRIu64 "\t%s\t%s",
                iter.stream.number, iter.block.number_in_stream,
                iter.block.number_in_file, iter.block.compressed_file_offset,
            ...
            if (message_verbosity_get() >= V_DEBUG)
                printf("\t%s\t%" PRIu32 "\t%s\t%" PRIu64 "\t%" PRIu64 "\t%s",
                       check_value, bhi.header_size, bhi.flags,
                       bhi.compressed_size, bhi.memusage, bhi.filter_chain);
    ...
    if (message_verbosity_get() >= V_DEBUG)
        printf("summary\t%" PRIu64 "\t%s\t%" PRIu32 "\n", xfi->memusage_max,
               xfi->all_have_sizes ? "yes" : "no", xfi->min_version);
...
static void print_totals_basic(void)
{
    ...
    // special handling.
    printf("%5s %7s  %11s  %11s  %5s  %-7s ", uint64_to_str(totals.streams, 0),
           uint64_to_str(totals.blocks, 1),
           uint64_to_nicestr(totals.compressed_size, NICESTR_B, NICESTR_TIB,
                             false, 2),
           uint64_to_nicestr(totals.uncompressed_size, NICESTR_B, NICESTR_TIB,
                             false, 3),
           get_ratio(totals.compressed_size, totals.uncompressed_size), checks);
...
static void print_totals_adv(void)
{
    ...
    print_adv_helper(totals.streams, totals.blocks, totals.compressed_size,
                     totals.uncompressed_size, totals.checks,
...
static void print_totals_robot(void)
{
    ...
    printf("totals\t%" PRIu64 "\t%" PRIu64 "\t%" PRIu64 "\t%" PRIu64
           "\t%s\t%s\t%" PRIu64 "\t%" PRIu64,
           totals.streams, totals.blocks, totals.compressed_size,
           totals.uncompressed_size,
           get_ratio(totals.compressed_size, totals.uncompressed_size), checks,
           totals.stream_padding, totals.files);

    if (message_verbosity_get() >= V_DEBUG)
        printf("\t%" PRIu64 "\t%s\t%" PRIu32, totals.memusage_max,
               totals.all_have_sizes ? "yes" : "no", totals.min_version);
