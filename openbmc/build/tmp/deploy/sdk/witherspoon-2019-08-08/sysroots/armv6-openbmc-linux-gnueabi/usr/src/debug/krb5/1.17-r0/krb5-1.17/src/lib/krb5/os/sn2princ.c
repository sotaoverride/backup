...
static krb5_boolean use_reverse_dns(krb5_context context)
{
    krb5_error_code ret;
    int value;

    ret =
        profile_get_boolean(context->profile, KRB5_CONF_LIBDEFAULTS,
                            KRB5_CONF_RDNS, NULL, DEFAULT_RDNS_LOOKUP, &value);
    if (ret)
...
static const char* find_trailer(const char* hostname)
{
...
krb5_error_code KRB5_CALLCONV krb5_sname_to_principal(krb5_context context,
                                                      const char* hostname,
                                                      const char* sname,
                                                      krb5_int32 type,
