...
bool unit_name_is_valid(const char* n, UnitNameFlags flags)
{
    ...
    assert((flags &
            ~(UNIT_NAME_PLAIN | UNIT_NAME_INSTANCE | UNIT_NAME_TEMPLATE)) == 0);
...
int unit_name_build(const char* prefix, const char* instance,
                    const char* suffix, char** ret)
{
    UnitType type;
...
int unit_name_build_from_type(const char* prefix, const char* instance,
                              UnitType type, char** ret)
{
    const char* ut;
    char* s;
...
int unit_name_from_path_instance(const char* prefix, const char* path,
                                 const char* suffix, char** ret)
{
    _cleanup_free_ char* p = NULL;
    char* s;
 ...
 *
 *  If @allow_globs, globs characters are preserved. Otherwise, they are
 * escaped.
 */
int unit_name_mangle_with_suffix(const char* name, UnitNameMangle flags,
                                 const char* suffix, char** ret)
{
    char* s;
    int r;
    ...
    if (isempty(name)) /* We cannot mangle empty unit names to become valid,
                          sorry. */
        return -EINVAL;
    ...
    if (mangled)
        log_full(flags & UNIT_NAME_MANGLE_WARN ? LOG_NOTICE : LOG_DEBUG,
                 "Invalid unit name \"%s\" was escaped as \"%s\" (maybe you "
    ...
    if ((!(flags & UNIT_NAME_MANGLE_GLOB) || !string_is_glob(s)) &&
...
int slice_build_subslice(const char* slice, const char* name, char** ret)
{
    ...
    else
    {
        ...
        stpcpy(stpcpy(stpcpy(mempcpy(subslice, slice, e - slice), "-"), name),
               ".slice");
    }
