 ...
 *
 * @PART_ENTRY_TYPE: partition type, 0xNN (e.g 0x82) or type UUID (gpt only) or
 type string (mac)
 *
 * @PART_ENTRY_FLAGS: partition flags (e.g. boot_ind) or  attributes (e.g. gpt
 attributes)
 *
...
static const struct blkid_idinfo* idinfos[] = {
    &aix_pt_idinfo,         &sgi_pt_idinfo,
    &sun_pt_idinfo,         &dos_pt_idinfo,
    &gpt_pt_idinfo,         &pmbr_pt_idinfo, /* always after GPT */
    &mac_pt_idinfo,         &ultrix_pt_idinfo,
    &bsd_pt_idinfo,         &unixware_pt_idinfo,
    &solaris_x86_pt_idinfo, &minix_pt_idinfo,
    &atari_pt_idinfo};

/*
 * Driver definition
 */
const struct blkid_chaindrv partitions_drv = {.id = BLKID_CHAIN_PARTS,
                                              .name = "partitions",
                                              .dflt_enabled = FALSE,
                                              .idinfos = idinfos,
...
struct blkid_struct_partition
{
    ...
    unsigned char
        name[128]; /* Partition in UTF8 name (when supported by PT), e.g. Mac */
...
{
    return (blkid_partlist)blkid_probe_get_binary_data(
        pr, &pr->chains[BLKID_CHAIN_PARTS]);
}
...
static void free_parttables(blkid_partlist ls)
{
    ...
    {
        blkid_parttable tab =
            list_entry(ls->l_tabs.next, struct blkid_struct_parttable, t_tabs);
        unref_parttable(tab);
    }
}
...
static blkid_partlist partitions_init_data(struct blkid_chain* chn)
{
    ...
    DBG(LOWPROBE, ul_debug("parts: initialized partitions list (size=%d)",
                           ls->nparts_max));
    return ls;
}
...
static blkid_partition new_partition(blkid_partlist ls, blkid_parttable tab)
{
    ...
    if (ls->nparts + 1 > ls->nparts_max)
    {
         ...
         */
        void* tmp =
            realloc(ls->parts, (ls->nparts_max + 32) *
                                             ...
                                             uint64_t start, uint64_t size)
{
    ...
    DBG(LOWPROBE,
        ul_debug("parts: add partition (start=%" PRIu64 ", size=%" PRIu64 ")",
                 par->start, par->size));
    return par;
}
...
int blkid_is_nested_dimension(blkid_partition par, uint64_t start,
                              uint64_t size)
{
    uint64_t pstart;
    uint64_t psize;
                        ...
                        struct blkid_chain* chn)
{
    ...
    if (id->probefunc)
    {
        ...
        if (rc < 0)
        {
            ...
            DBG(LOWPROBE, ul_debug("%s probefunc failed, rc %d", id->name, rc));
        }
...
static int partitions_probe(blkid_probe pr, struct blkid_chain* chn)
{
    ...
    DBG(LOWPROBE,
        ul_debug("--> starting probing loop [PARTS idx=%d]", chn->idx));
    ...
    for (; i < ARRAY_SIZE(idinfos); i++)
    {
        ...
        if (!chn->binary)
             ...
             * functions.
             */
            blkid_probe_set_value(pr, "PTTYPE", (const unsigned char*)name,
                                  strlen(name) + 1);

        DBG(LOWPROBE,
            ul_debug("<-- leaving probing loop (type=%s) [PARTS idx=%d]", name,
                     chn->idx));
        rc = BLKID_PROBE_OK;
        break;
    }
    ...
    {
        DBG(LOWPROBE,
            ul_debug("<-- leaving probing loop (failed=%d) [PARTS idx=%d]", rc,
                     chn->idx));
    }
                                 ...
                                 const struct blkid_idinfo* id)
{
    ...
    DBG(LOWPROBE, ul_debug("parts: ----> %s subprobe requested)", id->name));
    ...
    {
        DBG(LOWPROBE,
            ul_debug("ERROR: parts: <---- '%s' subprobe: overflow detected.",
                     id->name));
        return -ENOSPC;
    }
    ...
    DBG(LOWPROBE,
        ul_debug("parts: <---- %s subprobe done (rc=%d)", id->name, rc));
...
static int blkid_partitions_probe_partition(blkid_probe pr)
{
    ...
    else
    {
        ...
        if (v)
            ...
            blkid_probe_sprintf_value(pr, "PART_ENTRY_TYPE", "0x%x",
                                      blkid_partition_get_type(par));

        if (blkid_partition_get_flags(par))
            blkid_probe_sprintf_value(pr, "PART_ENTRY_FLAGS", "0x%llx",
                                      blkid_partition_get_flags(par));

        blkid_probe_sprintf_value(pr, "PART_ENTRY_NUMBER", "%d",
                                  blkid_partition_get_partno(par));
        ...
        blkid_probe_sprintf_value(pr, "PART_ENTRY_DISK", "%u:%u", major(disk),
                                  minor(disk));
    }
 ...
 */
int blkid_probe_is_covered_by_pt(blkid_probe pr, uint64_t offset, uint64_t size)
{
    ...
    DBG(LOWPROBE, ul_debug("=> checking if off=%" PRIu64 " size=%" PRIu64
                           " covered by PT",
                           offset, size));
...
blkid_parttable blkid_partlist_get_table(blkid_partlist ls)
{
    ...
    return list_entry(ls->l_tabs.next, struct blkid_struct_parttable, t_tabs);
}

/**
 * blkid_partlist_get_partition:
 * @ls: partitions list
 * @n: partition number in range 0..N, where 'N' is
 * blkid_partlist_numof_partitions().
 *
...
blkid_partition blkid_partlist_get_partition_by_start(blkid_partlist ls,
                                                      uint64_t start)
{
    int i, nparts;
    blkid_partition par;
...
int blkid_partitions_strcpy_ptuuid(blkid_probe pr, char* str)
{
    ...
    if (!blkid_probe_set_value(pr, "PTUUID", (unsigned char*)str,
                               strlen(str) + 1))
        return -ENOMEM;
...
int blkid_partition_set_name(blkid_partition par, const unsigned char* name,
                             size_t len)
{
    if (!par)
        return -1;
...
int blkid_partition_gen_uuid(blkid_partition par)
{
    ...
    snprintf(par->uuid, sizeof(par->uuid), "%.33s-%02x", par->tab->id,
             par->partno);
    return 0;
}
 ...
 *
 * 2) the offset for nested partitions could be relative to parent (e.g.
 * Solaris) _or_ relative to the begin of the whole disk (e.g. bsd).
 *
...
{
    set_string((unsigned char*)par->typestr, sizeof(par->typestr), type, len);
    return 0;
}
