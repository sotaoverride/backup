...
void _gcry_random_progress(const char* what, int printchar, int current,
                           int total)
{
    if (progress_cb)
        ...
        if (!fgets(buffer, sizeof buffer, fp))
        {
            if (!feof(fp))
            {
                ...
                syslog(LOG_USER | LOG_WARNING,
                       "Libgcrypt warning: error reading '%s', line %d", fname,
                       lnr);
#endif /*HAVE_SYSLOG*/
            }
            fclose(fp);
...
#ifdef HAVE_SYSLOG
            syslog(LOG_USER | LOG_WARNING,
                   "Libgcrypt warning: unknown option in '%s', line %d", fname,
                   lnr);
#endif /*HAVE_SYSLOG*/
        }
    }
...
gcry_err_code_t _gcry_random_add_bytes(const void* buf, size_t buflen,
                                       int quality)
{
    if (fips_mode())
...
void _gcry_create_nonce(void* buffer, size_t length)
{
    ...
    {
        _gcry_sha1_hash_buffer(nonce_buffer, nonce_buffer, sizeof nonce_buffer);
