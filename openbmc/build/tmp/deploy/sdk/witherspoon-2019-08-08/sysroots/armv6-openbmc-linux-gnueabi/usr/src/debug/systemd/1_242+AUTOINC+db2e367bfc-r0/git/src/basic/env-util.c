...
static int env_append(char** r, char*** k, char** a)
{
    ...
    /* Expects the following arguments: 'r' shall point to the beginning of an
     * strv we are going to append to, 'k' to a pointer pointing to the NULL
     * entry at the end of the same array. 'a' shall point to another strv.
     *
     * This call adds every entry of 'a' to 'r', either overriding an existing
     * matching entry, or appending to it.
     *
     * This call assumes 'r' has enough pre-allocated space to grow by all of
     * 'a''s items. */
...
int strv_env_replace(char*** l, char* p)
{
    ...
    /* Replace first occurrence of the env var or add a new one in the string
     * list. Drop other occurrences. Edits in-place. Does not copy p.  p must be
     * a valid key=value assignment.
     */
...
char** strv_env_clean_with_callback(char** e,
                                    void (*invalid_callback)(const char* p,
                                                             void* userdata),
                                    void* userdata)
{
    char **p, **q;
    ...
    for (e = format, i = 0; *e && i < n; e++, i++)
        switch (state)
        {
            ...
            case CURLY:
                ...
                else if (flags & REPLACE_ENV_ALLOW_BRACELESS &&
                         strchr(VALID_CHARS_ENV_NAME, *e))
                {
                    k = strnappend(r, word, e - word - 1);
                    if (!k)
                        return NULL;
            ...
            case VARIABLE:
                ...
                else if (*e == ':')
                {
                    if (!(flags & REPLACE_ENV_ALLOW_EXTENDED))
                        /* Treat this as unsupported syntax, i.e. do no
                         * replacement */
                        state = WORD;
                    else
                    {
    ...
    STRV_FOREACH(i, argv)
    {
        ...
        if ((*i)[0] == '$' && !IN_SET((*i)[1], '{', '$'))
        {
            ...
            if (e)
            {
                ...
                r = strv_split_extract(&m, e, WHITESPACE,
                                       EXTRACT_RELAX | EXTRACT_QUOTES);
                if (r < 0)
                {
                    ret[k] = NULL;
                    strv_free(ret);
