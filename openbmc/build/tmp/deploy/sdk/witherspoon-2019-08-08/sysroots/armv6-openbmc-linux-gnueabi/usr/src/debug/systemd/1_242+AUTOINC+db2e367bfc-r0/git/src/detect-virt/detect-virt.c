...
static int help(void)
{
    ...
    printf(
        ...
        "  -r --chroot           Detect whether we are run in a chroot() "
        "environment\n"
        "     --private-users    Only detect whether we are running in a user "
        "namespace\n"
        "  -q --quiet            Don't output anything, just set return value\n"
        "     --list             List all known and detectable types of "
        "virtualization\n"
...
static int run(int argc, char* argv[])
{
    ...
    switch (arg_mode)
    {
        ...
        case ONLY_CHROOT:
            ...
            if (r < 0)
                return log_error_errno(
                    r, "Failed to check for chroot() environment: %m");
            return !r;
        ...
        case ONLY_PRIVATE_USERS:
            ...
            if (r < 0)
                return log_error_errno(
                    r, "Failed to check for user namespace: %m");
            return !r;
        ...
        default:
            ...
            if (r < 0)
                return log_error_errno(
                    r, "Failed to check for virtualization: %m");
            break;
    }
