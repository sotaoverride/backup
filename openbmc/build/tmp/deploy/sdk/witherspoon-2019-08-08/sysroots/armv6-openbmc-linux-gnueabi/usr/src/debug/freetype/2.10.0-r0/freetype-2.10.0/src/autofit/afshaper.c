...
#define COVERAGE(name, NAME, description, tag1, tag2, tag3, tag4)              \
    static const hb_tag_t name##_coverage[] = {HB_TAG(tag1, tag2, tag3, tag4), \
                                               HB_TAG_NONE};
...
FT_Error af_shaper_get_coverage(AF_FaceGlobals globals,
                                AF_StyleClass style_class, FT_UShort* gstyles,
                                FT_Bool default_script)
{
    ...
    hb_tag_t script_tags[] = {HB_TAG_NONE, HB_TAG_NONE, HB_TAG_NONE,
                              HB_TAG_NONE};
    ...
    hb_ot_tags_from_script(script, &script_tags[0], &script_tags[1]);
    ...
    hb_ot_layout_collect_lookups(face, HB_OT_TAG_GSUB, script_tags, NULL,
                                 coverage_tags, gsub_lookups);
    ...
    for (idx = HB_SET_VALUE_INVALID; hb_set_next(gsub_lookups, &idx);)
    {
        ...
        hb_ot_layout_lookup_collect_glyphs(face, HB_OT_TAG_GSUB, idx, NULL,
                                           NULL, NULL, gsub_glyphs);
    }
    ...
    hb_ot_layout_collect_lookups(face, HB_OT_TAG_GPOS, script_tags, NULL,
                                 coverage_tags, gpos_lookups);
    ...
    for (idx = HB_SET_VALUE_INVALID; hb_set_next(gpos_lookups, &idx);)
    {
        ...
        hb_ot_layout_lookup_collect_glyphs(face, HB_OT_TAG_GPOS, idx, NULL,
                                           gpos_glyphs, NULL, NULL);
    }
        ...
        for (; bs->string != AF_BLUE_STRING_MAX; bs++)
        {
            ...
            while (*p)
            {
                ...
                for (idx = HB_SET_VALUE_INVALID;
                     hb_set_next(gsub_lookups, &idx);)
                {
                    hb_codepoint_t gidx = FT_Get_Char_Index(globals->face, ch);

                    if (hb_ot_layout_lookup_would_substitute(face, idx, &gidx,
                                                             1, 1))
                    {
                        found = 1;
                        break;
...
#define COVERAGE(name, NAME, description, tag1, tag2, tag3, tag4)              \
    static const hb_feature_t name##_feature[] = {                             \
        {HB_TAG(tag1, tag2, tag3, tag4), 1, 0, (unsigned int)-1}};
...
const char* af_shaper_get_cluster(const char* p, AF_StyleMetrics metrics,
                                  void* buf_, unsigned int* count)
{
...
FT_ULong af_shaper_get_elem(AF_StyleMetrics metrics, void* buf_,
                            unsigned int idx, FT_Long* advance,
                            FT_Long* y_offset)
{
    hb_buffer_t* buf = (hb_buffer_t*)buf_;
    hb_glyph_info_t* ginfo;
