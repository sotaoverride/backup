...
void init_schedule_container(void)
{
    ...
    if (!schedule_table)
    {
        ...
        DEBUGMSGTL(("disman:schedule:init", "create schedule container(%p)\n",
                    schedule_table));
    }
}
 ...
 */
static void _sched_callback(unsigned int reg, void* magic)
{
    ...
    snmp_set_var_typed_value(&assign, ASN_INTEGER, (u_char*)&entry->schedValue,
    ...
    {
        DEBUGMSGTL(
            ("disman:schedule:callback", "assignment failed (%d)\n", ret));
        entry->schedFailures++;
 ...
 */
static u_char _masks[] = {
    /* 0xff, */ 0x7f, 0x3f, 0x1f, 0x0f, 0x07, 0x03, 0x01, 0x00};
static u_char _bits[] = {0x80, 0x40, 0x20, 0x10, 0x08, 0x04, 0x02, 0x01};

/*
 * Are any of the bits set?
 */
static int _bit_allClear(char* pattern, int len)
{
    int i;
 ...
 */
static int _bit_set(char* pattern, int bit)
{
    int major, minor;
 ...
 */
static int _bit_next(char* pattern, int current, size_t len)
{
    char buf[8];
    int major, minor, i, j;
...
static int _daysPerMonth[] = {31, 28, 31, 30, 31, 30, 31,
                              31, 30, 31, 30, 31, 29};

static u_char _truncate[] = {0xfe, 0xf0, 0xfe, 0xfc, 0xfe, 0xfc, 0xfe,
                             0xfe, 0xfc, 0xfe, 0xfc, 0xfe, 0xf8};
 ...
 */
static int _bit_next_day(char* day_pattern, char weekday_pattern, int day,
                         int month, int year)
{
    char buf[4];
    union
...
void sched_nextTime(struct schedTable_entry* entry)
{
    ...
    switch (entry->schedType)
    {
        case SCHED_TYPE_PERIODIC:
            ...
            {
                entry->schedNextRun =
                    entry->schedLastRun + entry->schedInterval;
            }
            else
            {
        ...
        case SCHED_TYPE_ONESHOT:
            ...
            {
                DEBUGMSGTL(
                    ("disman:schedule:time", "one-shot: expired (%ld) %s",
        ...
        case SCHED_TYPE_CALENDAR:
            ...
            {
                DEBUGMSGTL(
                    ("disman:schedule:time", "calendar: incomplete spec\n"));
                return;
            }
            ...
            if (_bit_set(entry->schedMonth, now_tm.tm_mon))
            {
                     ...
                     _bit_set(entry->schedDay, 31 + rev_day)))
                {
                    ...
                    if (_bit_set(entry->schedHour, now_tm.tm_hour))
                    {
                        ...
                        next_tm.tm_min =
                            _bit_next(entry->schedMinute, now_tm.tm_min, 8);
                    }
                    else
                    {
                        next_tm.tm_min = -1;
                    ...
                    if (next_tm.tm_min == -1)
                    {
                        ...
                        next_tm.tm_hour =
                            _bit_next(entry->schedHour, now_tm.tm_hour, 3);
                    }
                }
                else
                {
                ...
                if (next_tm.tm_hour == -1)
                {
                    ...
                    if (mon == 1 && (now_tm.tm_year % 4 == 0))
                        mon = 12;
                    next_tm.tm_mday =
                        _bit_next_day(entry->schedDay, entry->schedWeekDay,
                                      now_tm.tm_mday, mon, now_tm.tm_year);
                }
            }
            else
            {
            ...
            {
                next_tm.tm_mon =
                    _bit_next(entry->schedMonth, next_tm.tm_mon, 2);
                if (next_tm.tm_mon == -1)
                {
                    next_tm.tm_year++;
                    next_tm.tm_mon = _bit_next(entry->schedMonth, -1, 2);
                }
                /* Handle leap years */
                mon = next_tm.tm_mon;
                if (mon == 1 && (next_tm.tm_year % 4 == 0))
                    mon = 12;
                next_tm.tm_mday =
                    _bit_next_day(entry->schedDay, entry->schedWeekDay, -1, mon,
                                  next_tm.tm_year);
    ...
    entry->schedCallbackID = snmp_alarm_register(entry->schedNextRun - now, 0,
                                                 _sched_callback, entry);
    return;
}
 ...
 */
netsnmp_tdata_row* schedTable_createEntry(const char* schedOwner,
...
void schedTable_removeEntry(netsnmp_tdata_row* row)
{
    ...
    entry = (struct schedTable_entry*)netsnmp_tdata_remove_and_delete_row(
        schedule_table, row);
    if (entry)
    {
