...
static void log_and_status(Manager* m, bool warn, const char* message,
                           const char* reason)
{
    log_full(warn ? LOG_WARNING : LOG_DEBUG, "%s: %s", message, reason);
    if (warn)
        manager_status_printf(m, STATUS_TYPE_EMERGENCY,
                              ANSI_HIGHLIGHT_RED "  !!  " ANSI_NORMAL, "%s: %s",
                              message, reason);
}

void emergency_action(Manager* m, EmergencyAction action,
                      EmergencyActionFlags options, const char* reboot_arg,
                      int exit_status, const char* reason)
{
    ...
    switch (action)
    {
        ...
        case EMERGENCY_ACTION_REBOOT:
            ...
            (void)manager_add_job_by_name_and_warn(
                m, JOB_START, SPECIAL_REBOOT_TARGET, JOB_REPLACE_IRREVERSIBLY,
                NULL, NULL);
            break;
        ...
        case EMERGENCY_ACTION_EXIT:
            ...
            if (MANAGER_IS_USER(m) || detect_container() > 0)
            {
                ...
                (void)manager_add_job_by_name_and_warn(
                    m, JOB_START, SPECIAL_EXIT_TARGET, JOB_REPLACE_IRREVERSIBLY,
                    NULL, NULL);
                break;
            }

            log_notice("Doing \"poweroff\" action instead of an \"exit\" "
                       "emergency action.");
            _fallthrough_;
        ...
        case EMERGENCY_ACTION_POWEROFF:
            ...
            (void)manager_add_job_by_name_and_warn(
                m, JOB_START, SPECIAL_POWEROFF_TARGET, JOB_REPLACE_IRREVERSIBLY,
                NULL, NULL);
            break;
        ...
        case EMERGENCY_ACTION_EXIT_FORCE:
            ...
            log_notice("Doing \"poweroff-force\" action instead of an "
                       "\"exit-force\" emergency action.");
            _fallthrough_;
...
int parse_emergency_action(const char* value, bool system, EmergencyAction* ret)
{
    ...
    if (!system && x != EMERGENCY_ACTION_NONE &&
        x < _EMERGENCY_ACTION_FIRST_USER_ACTION)
        return -EOPNOTSUPP;
