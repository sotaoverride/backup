...
blkid_dev blkid_get_dev(blkid_cache cache, const char* devname, int flags)
{
    ...
    if (dev)
        DBG(DEVNAME, ul_debug("%s requested, found %s in cache", devname,
                              dev->bid_name));
    free(cn);
    return dev;
}
...
static int is_dm_leaf(const char* devname)
{
    ...
    while ((de = readdir(dir)) != NULL)
    {
        if (!strcmp(de->d_name, ".") || !strcmp(de->d_name, "..") ||
            !strcmp(de->d_name, devname) || strncmp(de->d_name, "dm-", 3) ||
 ...
 */
static void probe_one(blkid_cache cache, const char* ptname, dev_t devno,
                      int pri, int only_if_new, int removable)
{
    blkid_dev dev = NULL;
    struct list_head *p, *pnext;
    const char** dir;
...
static void lvm_probe_all(blkid_cache cache, int only_if_new)
{
    ...
    while ((vg_iter = readdir(vg_list)) != NULL)
    {
        ...
        while ((lv_iter = readdir(lv_list)) != NULL)
        {
            ...
            lvm_device = malloc(vg_len + strlen(vg_name) + strlen(lv_name) + 8);
            if (!lvm_device)
            {
                closedir(lv_list);
                goto exit;
            }
            sprintf(lvm_device, "%s/%s/LVs/%s", VG_DIR, vg_name, lv_name);
            dev = lvm_get_devno(lvm_device);
            sprintf(lvm_device, "%s/%s", vg_name, lv_name);
            DBG(DEVNAME, ul_debug("LVM dev %s: devno 0x%04X", lvm_device,
                                  (unsigned int)dev));
            probe_one(cache, lvm_device, dev, BLKID_PRI_LVM, only_if_new, 0);
            free(lvm_device);
        }
        closedir(lv_list);
    }
...
static int evms_probe_all(blkid_cache cache, int only_if_new)
{
    ...
    while (fgets(line, sizeof(line), procpt))
    {
        ...
        DBG(DEVNAME,
            ul_debug("Checking partition %s (%d, %d)", device, ma, mi));

        probe_one(cache, device, makedev(ma, mi), BLKID_PRI_EVMS, only_if_new,
                  0);
        num++;
    }
    fclose(procpt);
...
static void ubi_probe_all(blkid_cache cache, int only_if_new)
{
    const char** dirname;
    ...
    for (dirname = dirlist; *dirname; dirname++)
    {
        ...
        DBG(DEVNAME, ul_debug("probing UBI volumes under %s", *dirname));
        ...
        while ((iter = readdir(dir)) != NULL)
        {
            ...
            if (iter->d_type != DT_UNKNOWN && iter->d_type != DT_CHR &&
                iter->d_type != DT_LNK)
                continue;
#endif
            ...
            if (!S_ISCHR(st.st_mode) || !minor(dev))
                continue;
            DBG(DEVNAME, ul_debug("UBI vol %s/%s: devno 0x%04X", *dirname, name,
                                  (int)dev));
...
static int probe_all(blkid_cache cache, int only_if_new)
{
    ...
    while (fgets(line, sizeof(line), proc))
    {
        ...
        {
            DBG(DEVNAME, ul_debug("partition dev %s, devno 0x%04X", ptname,
                                  (unsigned int)devs[which]));
        ...
        if (lens[last] && strncmp(ptnames[last], ptname, lens[last]))
        {
            DBG(DEVNAME, ul_debug("whole dev %s, devno 0x%04X", ptnames[last],
                                  (unsigned int)devs[last]));
            probe_one(cache, ptnames[last], devs[last], 0, only_if_new, 0);
            lens[last] = 0;
        }
    }
 ...
 * This file usually does not contain removable devices (e.g. CDROMs) and this
 * kind of devices are invisible for libblkid.
 *
