...
{
    uint32_t mvi_mode;
    uint16_t mvi_flags; // there currently is only one flag and kernel has no
                        // flags_mask yet
    uint32_t mvi_mask;
                         ...
                         struct nlattr* xstats)
{
    ...
    if ((err = nla_parse_nested(tb, IFLA_MACVLAN_MAX, data, macvlan_policy)) <
        0)
        goto errout;
    ...
    if (tb[IFLA_MACVLAN_MACADDR_COUNT] && tb[IFLA_MACVLAN_MACADDR_DATA])
    {
        ...
        if (mvi->mvi_maccount > 0)
        {
            ...
            mvi->mvi_macaddr =
                calloc(mvi->mvi_maccount, sizeof(*(mvi->mvi_macaddr)));
    ...
    if (mvi->mvi_mask & MACVLAN_HAS_MACADDR)
    {
        ...
        for (i = 0; i < mvi->mvi_maccount; i++)
        {
            nl_dump(p, "macvlan-sourcemac %s",
                    nl_addr2str(mvi->mvi_macaddr[i], buf, sizeof(buf)));
        }
    }
}
...
static int macvlan_clone(struct rtnl_link* dst, struct rtnl_link* src)
{
    ...
    if (vsrc->mvi_mask & MACVLAN_HAS_MACADDR && vsrc->mvi_maccount > 0)
    {
        vdst->mvi_macaddr =
            calloc(vdst->mvi_maccount, sizeof(*(vdst->mvi_macaddr)));
...
static int macvlan_put_attrs(struct nl_msg* msg, struct rtnl_link* link)
{
    ...
    if (mvi->mvi_mask & MACVLAN_HAS_MACADDR)
    {
        ...
        for (i = 0; i < mvi->mvi_maccount; i++)
        {
            NLA_PUT_ADDR(msg, IFLA_MACVLAN_MACADDR, mvi->mvi_macaddr[i]);
        }
    }
...
{
    return __flags2str(flags, buf, len, macvlan_flags,
                       ARRAY_SIZE(macvlan_flags));
}
