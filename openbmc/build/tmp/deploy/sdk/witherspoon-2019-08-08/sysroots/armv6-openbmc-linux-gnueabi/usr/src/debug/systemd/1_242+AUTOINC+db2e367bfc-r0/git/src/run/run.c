...
static enum {
    ARG_STDIO_NONE, /* The default, as it is for normal services, stdin
                       connected to /dev/null, and stdout+stderr to the journal
                     */
    ARG_STDIO_PTY,  /* Interactive behaviour, requested by --pty: we allocate a
                       pty and connect it to the TTY we are invoked from */
    ARG_STDIO_DIRECT, /* Directly pass our stdin/stdout/stderr to the activated
                         service, useful for usage in shell pipelines, requested
                         by --pipe */
    ARG_STDIO_AUTO,   /* If --pipe and --pty are used together we use --pty when
                         invoked on a TTY, and --pipe otherwise */
...
static int help(void)
{
    ...
    printf(
        ...
        "     --scope                      Run this as scope rather than "
        "service\n"
        "     --unit=UNIT                  Run under the specified unit name\n"
        "  -p --property=NAME=VALUE        Set service or scope unit property\n"
        "     --description=TEXT           Description for unit\n"
        "     --slice=SLICE                Run in the specified slice\n"
        "     --no-block                   Do not wait until operation "
        "finished\n"
        ...
        "  -d --same-dir                   Inherit working directory from "
        "caller\n"
        "  -E --setenv=NAME=VALUE          Set environment\n"
        "  -t --pty                        Run service on pseudo TTY as "
        "STDIN/STDOUT/\n"
        "                                  STDERR\n"
        "  -P --pipe                       Pass STDIN/STDOUT/STDERR directly "
        "to service\n"
        "  -q --quiet                      Suppress information messages "
        "during runtime\n"
        "  -G --collect                    Unload unit after it ran, even when "
        "failed\n"
        ...
        "     --on-boot=SECONDS            Run SECONDS after machine was "
        "booted up\n"
        "     --on-startup=SECONDS         Run SECONDS after systemd "
        "activation\n"
        "     --on-unit-active=SECONDS     Run SECONDS after the last "
        "activation\n"
        "     --on-unit-inactive=SECONDS   Run SECONDS after the last "
        "deactivation\n"
...
static int parse_argv(int argc, char* argv[])
{
    ...
    while ((c = getopt_long(argc, argv, "+hrH:M:E:p:tPqGdS", options, NULL)) >=
           0)
        ...
        switch (c)
        {
            ...
            case ARG_NICE:
                ...
                if (r < 0)
                    return log_error_errno(r, "Failed to parse nice value: %s",
                                           optarg);
            ...
            case 't': /* --pty */
                if (IN_SET(arg_stdio, ARG_STDIO_DIRECT,
                           ARG_STDIO_AUTO)) /* if --pipe is already used,
            ...
            case 'P': /* --pipe */
                if (IN_SET(arg_stdio, ARG_STDIO_PTY,
                           ARG_STDIO_AUTO)) /* If --pty is already used, upgrade
                                               to auto mode */
                    arg_stdio = ARG_STDIO_AUTO;
                else
            ...
            case ARG_TIMER_PROPERTY:
                ...
                arg_with_timer =
                    arg_with_timer ||
                    STARTSWITH_SET(optarg, "OnActiveSec=", "OnBootSec=",
                                   "OnStartupSec=", "OnUnitActiveSec=",
            ...
            case ARG_WORKING_DIRECTORY:
                r = parse_path_argument_and_warn(optarg, true,
                                                 &arg_working_directory);
                if (r < 0)
                    return r;
            ...
            case 'd':
            {
                ...
                if (r < 0)
                    return log_error_errno(
                        r, "Failed to get current working directory: %m");
    ...
    with_trigger =
        !!arg_path_property || !!arg_socket_property || arg_with_timer;

    /* currently, only single trigger (path, socket, timer) unit can be created
     * simultaneously */
    if ((int)!!arg_path_property + (int)!!arg_socket_property +
            (int)arg_with_timer >
        1)
        return log_error_errno(
    ...
    if (arg_shell)
    {
        /* If --shell is imply --pty --pipe --same-dir --service-type=exec
         * --wait --collect, unless otherwise specified. */
    ...
    {
        /* If we both --pty and --pipe are specified we'll automatically pick
         * --pty if we are connected fully to a TTY and pick direct fd passing
         * otherwise. This way, we automatically adapt to usage in a shell
         * pipeline, but we are neatly interactive with tty-level isolation
         * otherwise. */
        arg_stdio = isatty(STDIN_FILENO) && isatty(STDOUT_FILENO) &&
    ...
    if (arg_scope && (arg_remain_after_exit || arg_service_type))
        return log_error_errno(SYNTHETIC_ERRNO(EINVAL),
                               "--remain-after-exit and --service-type= are "
                               "not supported in --scope mode.");
    ...
    if (arg_stdio != ARG_STDIO_NONE && arg_transport == BUS_TRANSPORT_REMOTE)
        return log_error_errno(SYNTHETIC_ERRNO(EINVAL),
                               "--pty/--pipe is only supported when connecting "
                               "to the local system or containers.");
    ...
    if (arg_wait)
    {
        ...
        if (with_trigger)
            return log_error_errno(SYNTHETIC_ERRNO(EINVAL),
                                   "--wait may not be combined with path, "
                                   "socket or timer operations.");
...
static int transient_cgroup_set_properties(sd_bus_message* m)
{
    ...
    if (!isempty(arg_slice))
    {
        ...
        r = unit_name_mangle_with_suffix(
            arg_slice, arg_quiet ? 0 : UNIT_NAME_MANGLE_WARN, ".slice", &slice);
        if (r < 0)
            return log_error_errno(r, "Failed to mangle name '%s': %m",
                                   arg_slice);
...
static int transient_kill_set_properties(sd_bus_message* m)
{
    ...
    if (arg_send_sighup)
    {
        r = sd_bus_message_append(m, "(sv)", "SendSIGHUP", "b",
                                  arg_send_sighup);
        if (r < 0)
            return bus_log_create_error(r);
    }
                                            ...
                                            const char* pty_path)
{
    ...
    {
        r = sd_bus_message_append(m, "(sv)", "RemainAfterExit", "b",
    ...
    if (arg_working_directory)
    {
        r = sd_bus_message_append(m, "(sv)", "WorkingDirectory", "s",
                                  arg_working_directory);
        if (r < 0)
            return bus_log_create_error(r);
    }
    ...
    if (pty_path)
    {
        r = sd_bus_message_append(m, "(sv)(sv)(sv)(sv)", "StandardInput", "s",
                                  "tty", "StandardOutput", "s", "tty",
                                  "StandardError", "s", "tty", "TTYPath", "s",
                                  pty_path);
        if (r < 0)
            return bus_log_create_error(r);
    ...
    else if (arg_stdio == ARG_STDIO_DIRECT)
    {
        r = sd_bus_message_append(
            m, "(sv)(sv)(sv)", "StandardInputFileDescriptor", "h", STDIN_FILENO,
            "StandardOutputFileDescriptor", "h", STDOUT_FILENO,
            "StandardErrorFileDescriptor", "h", STDERR_FILENO);
        if (r < 0)
            return bus_log_create_error(r);

        send_term = isatty(STDIN_FILENO) || isatty(STDOUT_FILENO) ||
                    isatty(STDERR_FILENO);
    }
...
static int transient_scope_set_properties(sd_bus_message* m)
{
    ...
    r = sd_bus_message_append(m, "(sv)", "PIDs", "au", 1,
...
static int make_unit_name(sd_bus* bus, UnitType t, char** ret)
{
    ...
    if (r < 0)
    {
        ...
        if (r < 0)
            return log_error_errno(
                r, "Failed to generate random run unit name: %m");

        if (asprintf(ret, "run-r" SD_ID128_FORMAT_STR ".%s",
    ...
    if (!id)
        return log_error_errno(SYNTHETIC_ERRNO(EINVAL),
                               "Unique name %s has unexpected format.", unique);
...
static void run_context_check_done(RunContext* c)
{
    ...
    if (c->forward &&
        done) /* If the service is gone, it's time to drain the output */
...
static int run_context_update(RunContext* c, const char* path)
{
    ...
    static const struct bus_properties_map map[] = {
        ...
        {"InactiveExitTimestampMonotonic", "t", NULL,
         offsetof(RunContext, inactive_exit_usec)},
        {"InactiveEnterTimestampMonotonic", "t", NULL,
         offsetof(RunContext, inactive_enter_usec)},
    ...
    r = bus_map_all_properties(c->bus, "org.freedesktop.systemd1", path, map,
                               BUS_MAP_STRDUP, &error, NULL, c);
    if (r < 0)
    {
        sd_event_exit(c->event, EXIT_FAILURE);
        return log_error_errno(r, "Failed to query unit state: %s",
...
static int start_transient_service(sd_bus* bus, int* retval)
{
    ...
    if (arg_stdio == ARG_STDIO_PTY)
    {
        ...
        else if (arg_transport == BUS_TRANSPORT_MACHINE)
        {
            ...
            r = sd_bus_call_method(
                system_bus, "org.freedesktop.machine1",
                "/org/freedesktop/machine1", "org.freedesktop.machine1.Manager",
                "OpenMachinePTY", &error, &pty_reply, "s", arg_host);
            if (r < 0)
                return log_error_errno(r, "Failed to get machine PTY: %s",
                                       bus_error_message(&error, -r));
    ...
    if (arg_unit)
    {
        r = unit_name_mangle_with_suffix(arg_unit,
    ...
    if (r < 0)
        return log_error_errno(r, "Failed to start transient service unit: %s",
                               bus_error_message(&error, r));
    ...
    if (arg_wait || arg_stdio != ARG_STDIO_NONE)
    {
        ...
        if (master >= 0)
        {
            assert_se(sigprocmask_many(SIG_BLOCK, NULL, SIGWINCH, SIGTERM,
                                       SIGINT, -1) >= 0);
            ...
            r = pty_forward_new(c.event, master,
                                PTY_FORWARD_IGNORE_INITIAL_VHANGUP, &c.forward);
            if (r < 0)
             ...
             */
            (void)pty_forward_set_priority(c.forward,
        ...
        r = sd_bus_match_signal_async(bus, &c.match, "org.freedesktop.systemd1",
                                      path, "org.freedesktop.DBus.Properties",
                                      "PropertiesChanged",
                                      on_properties_changed, NULL, &c);
        if (r < 0)
            return log_error_errno(
                r, "Failed to request properties changed signal match: %m");
        ...
        if (arg_wait && !arg_quiet)
        {
            ...
            /* Explicitly destroy the PTY forwarder, so that the PTY device is
             * usable again, in its original settings (i.e. proper line breaks),
             * so that we can show the summary in a pretty way. */
            c.forward = pty_forward_free(c.forward);
            ...
            if (c.inactive_enter_usec > 0 &&
                c.inactive_enter_usec != USEC_INFINITY &&
                c.inactive_exit_usec > 0 &&
                c.inactive_exit_usec != USEC_INFINITY &&
                c.inactive_enter_usec > c.inactive_exit_usec)
            {
                char ts[FORMAT_TIMESPAN_MAX];
                log_info("Service runtime: %s",
                         format_timespan(ts, sizeof(ts),
                                         c.inactive_enter_usec -
            ...
            if (c.cpu_usage_nsec != NSEC_INFINITY)
            {
                ...
                log_info(
                    "CPU time consumed: %s",
                    format_timespan(ts, sizeof(ts),
                                    (c.cpu_usage_nsec + NSEC_PER_USEC - 1) /
                                        NSEC_PER_USEC,
                                    USEC_PER_MSEC));
            }
            ...
            if (c.ip_ingress_bytes != UINT64_MAX)
            {
                ...
                log_info(
                    "IP traffic received: %s",
                    format_bytes(bytes, sizeof(bytes), c.ip_ingress_bytes));
            }
            if (c.ip_egress_bytes != UINT64_MAX)
            {
                char bytes[FORMAT_BYTES_MAX];
                log_info("IP traffic sent: %s",
                         format_bytes(bytes, sizeof(bytes), c.ip_egress_bytes));
            }
        }

        /* Try to propagate the service's return value */
        if (c.result && STR_IN_SET(c.result, "success", "exit-code") &&
            c.exit_code == CLD_EXITED)
...
static int acquire_invocation_id(sd_bus* bus, sd_id128_t* ret)
{
    ...
    r = sd_bus_get_property(
        bus, "org.freedesktop.systemd1", "/org/freedesktop/systemd1/unit/self",
        "org.freedesktop.systemd1.Unit", "InvocationID", &error, &reply, "ay");
    if (r < 0)
        return log_error_errno(r,
                               "Failed to request invocation ID for scope: %s",
                               bus_error_message(&error, r));
...
static int start_transient_scope(sd_bus* bus)
{
    ...
    {
        r = unit_name_mangle_with_suffix(
            arg_unit, arg_quiet ? 0 : UNIT_NAME_MANGLE_WARN, ".scope", &scope);
        if (r < 0)
    ...
    r = sd_bus_call(bus, m, 0, &error, &reply);
    if (r < 0)
        return log_error_errno(r, "Failed to start transient scope unit: %s",
                               bus_error_message(&error, -r));
    ...
    r = strv_extendf(&user_env, "INVOCATION_ID=" SD_ID128_FORMAT_STR,
    ...
    if (arg_exec_group)
    {
        ...
        if (r < 0)
            return log_error_errno(r, "Failed to resolve group %s: %m",
                                   arg_exec_group);

        if (setresgid(gid, gid, gid) < 0)
            return log_error_errno(
                errno, "Failed to change GID to " GID_FMT ": %m", gid);
    }
    ...
    if (arg_exec_user)
    {
        ...
        r = get_user_creds(&arg_exec_user, &uid, &gid, &home, &shell,
                           USER_CREDS_CLEAN | USER_CREDS_PREFER_NSS);
        if (r < 0)
            return log_error_errno(r, "Failed to resolve user %s: %m",
                                   arg_exec_user);
        ...
        if (setresuid(uid, uid, uid) < 0)
            return log_error_errno(
                errno, "Failed to change UID to " UID_FMT ": %m", uid);
    }
...
static int start_transient_trigger(sd_bus* bus, const char* suffix)
{
    ...
    if (arg_unit)
    {
        switch (unit_name_to_type(arg_unit))
        {
            ...
            default:
                r = unit_name_mangle_with_suffix(
                    arg_unit, arg_quiet ? 0 : UNIT_NAME_MANGLE_WARN, ".service",
                    &service);
                if (r < 0)
                    return log_error_errno(r, "Failed to mangle unit name: %m");

                r = unit_name_mangle_with_suffix(
                    arg_unit, arg_quiet ? 0 : UNIT_NAME_MANGLE_WARN, suffix,
                    &trigger);
                if (r < 0)
                    return log_error_errno(r, "Failed to mangle unit name: %m");
    ...
    r = sd_bus_call(bus, m, 0, &error, &reply);
    if (r < 0)
        return log_error_errno(r, "Failed to start transient %s unit: %s",
...
static int run(int argc, char* argv[])
{
    ...
    if (!strv_isempty(arg_cmdline) && arg_transport == BUS_TRANSPORT_LOCAL)
    {
        ...
        if (r < 0)
            return log_error_errno(r, "Failed to find executable %s: %m",
                                   arg_cmdline[0]);
    ...
    /* If --wait is used connect via the bus, unconditionally, as ref/unref is
     * not supported via the limited direct connection */
    if (arg_wait || arg_stdio != ARG_STDIO_NONE)
        r = bus_connect_transport(arg_transport, arg_host, arg_user, &bus);
    else
        r = bus_connect_transport_systemd(arg_transport, arg_host, arg_user,
                                          &bus);
    if (r < 0)
