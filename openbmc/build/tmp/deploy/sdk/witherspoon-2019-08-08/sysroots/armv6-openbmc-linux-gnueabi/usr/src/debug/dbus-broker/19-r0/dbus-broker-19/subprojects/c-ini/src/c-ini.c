...
{
    if (!group->domain ||
        group->link_domain.next == &group->domain->list_groups)
        return NULL;
...
{
    if (!group->domain ||
        group->link_domain.prev == &group->domain->list_groups)
        return NULL;
...
_public_ CIniEntry* c_ini_group_find(CIniGroup* group, const char* label,
                                     ssize_t n_label)
{
    CIniBytes bytes;
    CRBNode* iter;
...
static CIniDomain* c_ini_domain_free_internal(CIniDomain* domain)
{
    ...
    c_list_for_each_entry_safe(group, t_group, &domain->list_groups,
                               link_domain) c_ini_group_unlink(group);
...
_public_ CIniGroup* c_ini_domain_find(CIniDomain* domain, const char* label,
                                      ssize_t n_label)
{
    CIniBytes bytes;
    CRBNode* iter;
