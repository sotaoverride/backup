...
static int load_clock_timestamp(uid_t uid, gid_t gid)
{
    ...
    if (fd >= 0)
    {
        ...
        if (geteuid() == 0)
        {
            ...
            if (r < 0)
                log_warning_errno(
                    r, "Failed to chmod or chown %s, ignoring: %m", CLOCK_FILE);
        }
    }
    else
    {
...
settime:
    ...
    if (ct < min)
    {
        ...
        log_info("System clock time unset or jumped backwards, restoring from "
                 "recorded timestamp: %s",
...
static int run(int argc, char* argv[])
{
    ...
    /* Drop privileges, but only if we have been started as root. If we are not
     * running as root we assume all privileges are already dropped. */
    if (uid_current == 0)
    {
    ...
    {
        log_info("The system is configured to read the RTC time in the local "
                 "time zone. "
                 "This mode cannot be fully supported. All system time to RTC "
