...
static int vxlan_put_attrs(struct nl_msg* msg, struct rtnl_link* link)
{
    ...
    if (vxi->ce_mask & VXLAN_ATTR_GROUP6)
        NLA_PUT(msg, IFLA_VXLAN_GROUP6, sizeof(vxi->vxi_group6),
                &vxi->vxi_group6);
    ...
    if (vxi->ce_mask & VXLAN_ATTR_LOCAL6)
        NLA_PUT(msg, IFLA_VXLAN_LOCAL6, sizeof(vxi->vxi_local6),
                &vxi->vxi_local6);
...
#define VXLAN_DIFF(ATTR, EXPR) ATTR_DIFF(attrs, VXLAN_ATTR_##ATTR, a, b, EXPR)
    ...
    diff |=
        VXLAN_DIFF(PORT_RANGE, a->vxi_port_range.low != b->vxi_port_range.low);
    ...
    diff |= VXLAN_DIFF(GROUP6, memcmp(&a->vxi_group6, &b->vxi_group6,
                                      sizeof(a->vxi_group6)) != 0);
    diff |= VXLAN_DIFF(LOCAL6, memcmp(&a->vxi_local6, &b->vxi_local6,
    ...
    diff |= VXLAN_DIFF(COLLECT_METADATA,
...
int rtnl_link_vxlan_get_group(struct rtnl_link* link, struct nl_addr** addr)
{
    ...
    if (vxi->ce_mask & VXLAN_ATTR_GROUP)
        ...
        *addr =
            nl_addr_build(AF_INET6, &vxi->vxi_group6, sizeof(vxi->vxi_group6));
    else
        return -NLE_AGAIN;
...
int rtnl_link_vxlan_get_local(struct rtnl_link* link, struct nl_addr** addr)
{
    ...
    if (vxi->ce_mask & VXLAN_ATTR_LOCAL)
        ...
        *addr =
            nl_addr_build(AF_INET6, &vxi->vxi_local6, sizeof(vxi->vxi_local6));
    else
        return -NLE_AGAIN;
 ...
 * @return 0 on success or a negative error code
 */
int rtnl_link_vxlan_set_flags(struct rtnl_link* link, uint32_t flags,
                              int enable)
{
    struct vxlan_info* vxi = link->l_info;
    ...
    if (flags & ~(RTNL_LINK_VXLAN_F_GBP | RTNL_LINK_VXLAN_F_GPE |
                  RTNL_LINK_VXLAN_F_REMCSUM_NOPARTIAL))
        return -NLE_INVAL;
