...
static void dns_transaction_close_connection(DnsTransaction* t)
{
    ...
    if (t->stream)
    {
        /* Let's detach the stream from our transaction, in case something else
...
DnsTransaction* dns_transaction_free(DnsTransaction* t)
{
    ...
    if (t->scope)
    {
        ...
        if (t->id != 0)
            hashmap_remove(t->scope->manager->dns_transactions,
                           UINT_TO_PTR(t->id));
    }
...
static uint16_t pick_new_id(Manager* m)
{
    ...
    /* Find a fresh, unused transaction id. Note that this loop is bounded
     * because there's a limit on the number of transactions, and it's much
     * lower than the space of IDs. */
...
int dns_transaction_new(DnsTransaction** ret, DnsScope* s, DnsResourceKey* key)
{
    ...
    r = hashmap_ensure_allocated(&s->transactions_by_key,
...
static void dns_transaction_shuffle_id(DnsTransaction* t)
{
    ...
    assert_se(hashmap_remove_and_put(t->scope->manager->dns_transactions,
                                     UINT_TO_PTR(t->id), UINT_TO_PTR(new_id),
                                     t) >= 0);
...
static void dns_transaction_tentative(DnsTransaction* t, DnsPacket* p)
{
    ...
    log_debug("Transaction %" PRIu16
              ...
              af_to_name_short(t->scope->family), strnull(pretty));
    ...
    if (memcmp(&p->sender, &p->destination, FAMILY_ADDRESS_SIZE(p->family)) >=
        0)
    {
        log_debug("Peer has lexicographically larger IP address and thus lost "
                  "in the conflict.");
        return;
    }

    log_debug("We have the lexicographically larger IP address and thus lost "
              "in the conflict.");
...
void dns_transaction_complete(DnsTransaction* t, DnsTransactionState state)
{
    ...
    if (state == DNS_TRANSACTION_DNSSEC_FAILED)
    {
        ...
        log_struct(
            LOG_NOTICE, "MESSAGE_ID=" SD_MESSAGE_DNSSEC_FAILURE_STR,
            LOG_MESSAGE("DNSSEC validation failed for question %s: %s", key_str,
                        dnssec_result_to_string(t->answer_dnssec_result)),
            "DNS_TRANSACTION=%" PRIu16, t->id, "DNS_QUESTION=%s", key_str,
            "DNSSEC_RESULT=%s",
            dnssec_result_to_string(t->answer_dnssec_result), "DNS_SERVER=%s",
            dns_server_string(t->server), "DNS_SERVER_FEATURE_LEVEL=%s",
            dns_server_feature_level_to_string(
                t->server->possible_feature_level));
    }
    ...
    log_debug(
        ...
        t->id, dns_resource_key_to_string(t->key, key_str, sizeof key_str),
        dns_protocol_to_string(t->scope->protocol),
        t->scope->link ? t->scope->link->name : "*",
        af_to_name_short(t->scope->family), st,
        t->answer_source < 0
            ? "none"
            : dns_transaction_source_to_string(t->answer_source),
...
static int dns_transaction_pick_server(DnsTransaction* t)
{
    ...
    /* If we changed the server invalidate the feature level clamping, as the
     * new server might have completely different properties. */
    if (server != t->server)
        t->clamp_feature_level = _DNS_SERVER_FEATURE_LEVEL_INVALID;
    ...
    log_debug("Using DNS server %s for transaction %u.",
              dns_server_string(t->server), t->id);
...
static int dns_transaction_maybe_restart(DnsTransaction* t)
{
    ...
    if (t->current_feature_level <=
        dns_server_possible_feature_level(t->server))
        return 0;

    /* The server's current feature level is lower than when we sent the
       original query. We learnt something from the response or possibly an
       auxiliary DNSSEC response that we didn't know before.  We take that as
       reason to restart the whole transaction. This is a good idea to deal with
       servers that respond rubbish if we include OPT RR or DO bit. One of these
       cases is documented here, for example:
       https://open.nlnetlabs.nl/pipermail/dnssec-trigger/2014-November/000376.html
     */

    log_debug("Server feature level is now lower than when we began our "
              "transaction. Restarting with new ID.");
...
static void on_transaction_stream_error(DnsTransaction* t, int error)
{
    ...
    if (ERRNO_IS_DISCONNECT(error))
    {
        ...
        {
            /* If the LLMNR/TCP connection failed, the host doesn't support
             * LLMNR, and we cannot answer the question on this scope. */
...
static int on_stream_complete(DnsStream* s, int error)
{
    ...
    if (ERRNO_IS_DISCONNECT(error) && s->protocol != DNS_PROTOCOL_LLMNR)
    {
        ...
        if (s->transactions)
        {
            ...
            t = s->transactions;
            dns_server_packet_lost(t->server, IPPROTO_TCP,
...
static int on_stream_packet(DnsStream* s)
{
    ...
    t = hashmap_get(s->manager->dns_transactions,
                    UINT_TO_PTR(DNS_PACKET_ID(p)));
    if (t)
        return dns_transaction_on_stream_packet(t, p);

    /* Ignore incorrect transaction id as an old transaction can have been
     * canceled. */
    log_debug("Received unexpected TCP reply packet with id %" PRIu16
              ", ignoring.",
              DNS_PACKET_ID(p));
    return 0;
}
...
static int dns_transaction_emit_tcp(DnsTransaction* t)
{
    ...
    switch (t->scope->protocol)
    {
        ...
        case DNS_PROTOCOL_DNS:
            ...
            if (!dns_server_dnssec_supported(t->server) &&
                dns_type_is_dnssec(t->key->type))
                return -EOPNOTSUPP;

            r = dns_server_adjust_opt(t->server, t->sent,
                                      t->current_feature_level);
            if (r < 0)
                return r;
            ...
            if (t->server->stream &&
                (DNS_SERVER_FEATURE_LEVEL_IS_TLS(t->current_feature_level) ==
                 t->server->stream->encrypted))
                s = dns_stream_ref(t->server->stream);
            else
                fd = dns_scope_socket_tcp(
                    t->scope, AF_UNSPEC, NULL, t->server,
                    dns_port_for_feature_level(t->current_feature_level), &sa);

            type = DNS_STREAM_LOOKUP;
            break;

        case DNS_PROTOCOL_LLMNR:
            /* When we already received a reply to this (but it was truncated),
             * send to its sender address */
            if (t->received)
                fd = dns_scope_socket_tcp(t->scope, t->received->family,
                                          &t->received->sender, NULL,
                                          t->received->sender_port, &sa);
            else
            {
                ...
                r = dns_name_address(dns_resource_key_name(t->key), &family,
                                     &address);
                if (r < 0)
                    return r;
                if (r == 0)
                ...
                fd = dns_scope_socket_tcp(t->scope, family, &address, NULL,
                                          LLMNR_PORT, &sa);
            }
...
static void dns_transaction_cache_answer(DnsTransaction* t)
{
    ...
    dns_cache_put(&t->scope->cache, t->key, t->answer_rcode, t->answer,
                  t->answer_authenticated, t->answer_nsec_ttl, 0,
                  t->received->family, &t->received->sender);
}
...
static int dns_transaction_dnssec_ready(DnsTransaction* t)
{
    ...
    /* Checks whether the auxiliary DNSSEC transactions of our transaction have
     * completed, or are still ongoing. Returns 0, if we aren't ready for the
     * DNSSEC validation, positive if we are. */
    ...
    SET_FOREACH(dt, t->dnssec_transactions, i)
    {
        ...
        switch (dt->state)
        {
            ...
            case DNS_TRANSACTION_RCODE_FAILURE:
                if (!IN_SET(dt->answer_rcode, DNS_RCODE_NXDOMAIN,
                            DNS_RCODE_SERVFAIL))
                {
                ...
                /* Fall-through: NXDOMAIN/SERVFAIL is good enough for us. This
                 * is because some DNS servers erronously return
                 * NXDOMAIN/SERVFAIL for empty non-terminals (Akamai...) or
                 * missing DS records (Facebook), and we need to handle that
                 * nicely, when asking for parent SOA or similar RRs to make
                 * unsigned proofs. */
            ...
            case DNS_TRANSACTION_DNSSEC_FAILED:
                /* We handle DNSSEC failures different from other errors, as we
                 * care about the DNSSEC validationr result */
                ...
                t->answer_dnssec_result =
                    dt->answer_dnssec_result; /* Copy error code over */
...
static void dns_transaction_process_dnssec(DnsTransaction* t)
{
    ...
    if (r < 0)
        goto fail;
    if (r == 0) /* We aren't ready yet (or one of our auxiliary transactions
                   failed, and we shouldn't validate now */
        return;

    /* See if we learnt things from the additional DNSSEC transactions, that we
     * didn't know before, and better restart the lookup immediately. */
        ...
        t->scope->dnssec_mode == DNSSEC_YES)
    {
        ...
        /*  We are not in automatic downgrade mode, and the server is bad. Let's
         * try a different server, maybe that works. */

        if (t->n_picked_servers < dns_scope_get_n_dns_servers(t->scope))
        {
            /* We tried fewer servers on this transaction than we know, let's
...
static int dns_transaction_has_positive_answer(DnsTransaction* t,
...
static int dns_transaction_fix_rcode(DnsTransaction* t)
{
    ...
    /* Fix up the RCODE to SUCCESS if we get at least one matching RR in a
     * response. Note that this contradicts the DNS RFCs a bit. Specifically,
     * RFC 6604 Section 3 clarifies that the RCODE shall say something about a
     * CNAME/DNAME chain element coming after the last chain element contained
     * in the message, and not the first one included. However, it also
     * indicates that not all DNS servers implement this correctly. Moreover,
     * when using DNSSEC we usually only can prove the first element of a
     * CNAME/DNAME chain anyway, hence let's settle on always processing the
     * RCODE as referring to the immediate look-up we do, i.e. the first element
     * of a CNAME/DNAME chain. This way, we uniformly handle CNAME/DNAME chains,
     * regardless if the DNS server incorrectly implements RCODE, whether DNSSEC
     * is in use, or whether the DNS server only supplied us with an incomplete
     * CNAME/DNAME chain.
     *
     * Or in other words: if we get at least one positive reply in a message we
     * patch NXDOMAIN to become SUCCESS, and then rely on the CNAME chasing
     * logic to figure out that there's actually a CNAME error with a new
     * lookup. */
...
void dns_transaction_process_reply(DnsTransaction* t, DnsPacket* p)
{
    ...
    log_debug("Processing incoming packet on transaction %" PRIu16
              " (rcode=%s).",
    ...
    assert_se(sd_event_now(t->scope->manager->event,
                           clock_boottime_or_monotonic(), &ts) >= 0);
    ...
    switch (t->scope->protocol)
    {
        ...
        case DNS_PROTOCOL_DNS:
            ...
            if (IN_SET(DNS_PACKET_RCODE(p), DNS_RCODE_FORMERR,
                       DNS_RCODE_SERVFAIL, DNS_RCODE_NOTIMP))
            {
                ...
                if (t->current_feature_level <= DNS_SERVER_FEATURE_LEVEL_UDP)
                {
                    ...
                    /* This was already at UDP feature level? If so, it doesn't
                     * make sense to downgrade this transaction anymore, but
                     * let's see if it might make sense to send the request to a
                     * different DNS server instead. If not let's process the
                     * response, and accept the rcode. Note that we don't retry
                     * on TCP, since that's a suitable way to mitigate packet
                     * loss, but is not going to give us better rcodes should we
                     * actually have managed to get them already at UDP level.
                     */

                    if (t->n_picked_servers <
                        dns_scope_get_n_dns_servers(t->scope))
                    {
                        /* We tried fewer servers on this transaction than we
                         * know, let's try another one then */
                        dns_transaction_retry(t, true);
                        return;
                    }
                ...
                switch (t->current_feature_level)
                {
                    ...
                    case DNS_SERVER_FEATURE_LEVEL_TLS_PLAIN + 1:
                        ...
                        t->clamp_feature_level =
                            DNS_SERVER_FEATURE_LEVEL_TLS_PLAIN - 1;
                        break;
                    default:
                ...
                log_debug(
                    "Server returned error %s, retrying transaction with "
                    "reduced feature level %s.",
                    dns_rcode_to_string(DNS_PACKET_RCODE(p)),
            ...
            {
                /* This server refused our request? If so, try again, use a
                 * different server */
                log_debug("Server returned REFUSED, switching servers, and "
                          "retrying.");
            ...
            if (DNS_PACKET_TC(p))
                dns_server_packet_truncated(t->server,
                                            t->current_feature_level);
    ...
    if (t->server)
    {
        /* Report that we successfully received a valid packet with a good rcode
         * after we initially got a bad rcode and subsequently downgraded the
         * protocol */

        if (IN_SET(DNS_PACKET_RCODE(p), DNS_RCODE_SUCCESS,
                   DNS_RCODE_NXDOMAIN) &&
            t->clamp_feature_level != _DNS_SERVER_FEATURE_LEVEL_INVALID)
            dns_server_packet_rcode_downgrade(t->server,
                                              t->clamp_feature_level);
        ...
        dns_server_packet_received(t->server, p->ipproto,
                                   t->current_feature_level, p->size);
    }

    /* See if we know things we didn't know before that indicate we better
               ...
               DNS_PROTOCOL_MDNS))
    {
        ...
        /* When dealing with protocols other than mDNS only consider responses
         * with equivalent query section to the request. For mDNS this check
         * doesn't make sense, because the section 6 of RFC6762 states that
        ...
        /* Maybe the transaction is ready for GC'ing now? If so, free it and
         * return. */
        ...
        if (r > 0)
        {
            /* There are DNSSEC transactions pending now. Update the state
             * accordingly. */
...
static int on_dns_packet(sd_event_source* s, int fd, uint32_t revents,
                         void* userdata)
{
    _cleanup_(dns_packet_unrefp) DnsPacket* p = NULL;
    ...
    if (ERRNO_IS_DISCONNECT(-r))
    {
        ...
        /* UDP connection failure get reported via ICMP and then are possible
         * delivered to us on the next
         * recvmsg(). Treat this like a lost packet. */

        log_debug_errno(r, "Connection failure for DNS UDP packet: %m");
        assert_se(sd_event_now(t->scope->manager->event,
                               clock_boottime_or_monotonic(), &usec) >= 0);
        dns_server_packet_lost(t->server, IPPROTO_UDP,
                               t->current_feature_level);
    ...
    {
        log_debug_errno(
            r, "Received invalid DNS packet as response, ignoring: %m");
        return 0;
    }
    if (r == 0)
    {
...
static int dns_transaction_emit_udp(DnsTransaction* t)
{
    ...
    if (t->scope->protocol == DNS_PROTOCOL_DNS)
    {
        ...
        { /* Server changed, or no connection yet. */
            int fd;
            ...
            r = sd_event_add_io(t->scope->manager->event,
                                &t->dns_udp_event_source, fd, EPOLLIN,
                                on_dns_packet, t);
            if (r < 0)
            {
                safe_close(fd);
...
static int on_transaction_timeout(sd_event_source* s, usec_t usec,
                                  void* userdata)
{
    DnsTransaction* t = userdata;
    ...
    if (!t->initial_jitter_scheduled || t->initial_jitter_elapsed)
    {
        ...
        switch (t->scope->protocol)
        {
            ...
            case DNS_PROTOCOL_DNS:
                assert(t->server);
                dns_server_packet_lost(t->server,
                                       t->stream ? IPPROTO_TCP : IPPROTO_UDP,
                                       t->current_feature_level);
                break;
...
static usec_t transaction_get_resend_timeout(DnsTransaction* t)
{
    ...
    switch (t->scope->protocol)
    {
        ...
        case DNS_PROTOCOL_DNS:
            ...
            /* When we do TCP, grant a much longer timeout, as in this case
             * there's no need for us to quickly resend, as the kernel does that
             * anyway for us, and we really don't want to interrupt it in that
             * needlessly. */
            if (t->stream)
...
static int dns_transaction_prepare(DnsTransaction* t, usec_t ts)
{
    ...
    /* Check the trust anchor. Do so only on classic DNS, since DNSSEC does not
     * apply otherwise. */
    if (t->scope->protocol == DNS_PROTOCOL_DNS)
    {
        r = dns_trust_anchor_lookup_positive(&t->scope->manager->trust_anchor,
                                             t->key, &t->answer);
        if (r < 0)
            return r;
        if (r > 0)
    ...
    if (set_isempty(t->notify_zone_items))
    {
        ...
        r = dns_zone_lookup(&t->scope->zone, t->key,
                            dns_scope_ifindex(t->scope), &t->answer, NULL,
                            NULL);
        if (r < 0)
            return r;
        if (r > 0)
    ...
    if (set_isempty(t->notify_zone_items))
    {
        ...
        r = dns_cache_lookup(&t->scope->cache, t->key, t->clamp_ttl,
                             &t->answer_rcode, &t->answer,
...
static int dns_transaction_make_packet_mdns(DnsTransaction* t)
{
    ...
    /* Discard any previously prepared packet, so we can start over and coalesce
     * again */
    t->sent = dns_packet_unref(t->sent);
    ...
    /*
     * For mDNS, we want to coalesce as many open queries in pending
     * transactions into one single query packet on the wire as possible. To
     * achieve that, we iterate through all pending transactions in our current
     * scope, and see whether their timing contraints allow them to be sent.
     */
    ...
    LIST_FOREACH(transactions_by_scope, other, t->scope->transactions)
    {
         ...
         * One of the 'other' transactions will fire later and take care of the
         * rest.
         */
        if (r == -EMSGSIZE)
            break;
    ...
    SET_FOREACH(tkey, keys, i)
    {
        ...
        r = dns_zone_lookup(&t->scope->zone, tkey, t->scope->link->ifindex,
                            &answer, NULL, &tentative);
        if (r < 0)
            return r;
...
int dns_transaction_go(DnsTransaction* t)
{
    ...
    /* Returns > 0 if the transaction is now pending, returns 0 if could be
     * processed immediately and has finished now. */
    ...
    log_debug("Transaction %" PRIu16 " for <%s> scope %s on %s/%s.", t->id,
              dns_resource_key_to_string(t->key, key_str, sizeof key_str),
              dns_protocol_to_string(t->scope->protocol),
              t->scope->link ? t->scope->link->name : "*",
        ...
        IN_SET(t->scope->protocol, DNS_PROTOCOL_LLMNR, DNS_PROTOCOL_MDNS))
    {
        ...
        r = sd_event_add_time(t->scope->manager->event,
                              ...
                              clock_boottime_or_monotonic(), ts + jitter,
                              accuracy, on_transaction_timeout, t);
        if (r < 0)
            return r;

        (void)sd_event_source_set_description(t->timeout_event_source,
                                              "dns-transaction-timeout");
        ...
        log_debug("Delaying %s transaction for " USEC_FMT "us.",
                  dns_protocol_to_string(t->scope->protocol), jitter);
        return 0;
    }
    ...
    {
        /* On LLMNR, if we cannot connect to a host via TCP when doing reverse
         * lookups. This means we cannot
    ...
    r = sd_event_add_time(t->scope->manager->event, &t->timeout_event_source,
                          clock_boottime_or_monotonic(), ts, 0,
...
static int dns_transaction_add_dnssec_transaction(DnsTransaction* t,
                                                  DnsResourceKey* key,
                                                  DnsTransaction** ret)
{
    DnsTransaction* aux;
    ...
    else
    {
        ...
        if (r > 0)
        {
            char s[DNS_RESOURCE_KEY_STRING_MAX],
                saux[DNS_RESOURCE_KEY_STRING_MAX];

            return log_debug_errno(
                SYNTHETIC_ERRNO(ELOOP),
                "Potential cyclic dependency, refusing to add transaction "
                "%" PRIu16 " (%s) as dependency for %" PRIu16 " (%s).",
                aux->id, dns_resource_key_to_string(t->key, s, sizeof s), t->id,
...
static int dns_transaction_request_dnssec_rr(DnsTransaction* t,
                                             DnsResourceKey* key)
{
    ...
    r = dns_trust_anchor_lookup_positive(&t->scope->manager->trust_anchor, key,
                                         &a);
    if (r < 0)
        return r;
    if (r > 0)
...
static int dns_transaction_negative_trust_anchor_lookup(DnsTransaction* t,
                                                        const char* name)
{
    int r;
    ...
    r = dns_trust_anchor_lookup_negative(&t->scope->manager->trust_anchor,
                                         name);
    if (r != 0)
        return r;
...
static int dns_transaction_has_unsigned_negative_answer(DnsTransaction* t)
{
    ...
    r = dns_transaction_negative_trust_anchor_lookup(
        t, dns_resource_key_name(t->key));
    if (r < 0)
        return r;
    if (r > 0)
        return false;
...
static int dns_transaction_is_primary_response(DnsTransaction* t,
...
static bool dns_transaction_dnssec_supported(DnsTransaction* t)
{
    ...
    /* Checks whether our transaction's DNS server is assumed to be compatible
     * with DNSSEC. Returns false as soon
     * as we changed our mind about a server, and now believe it is incompatible
     * with DNSSEC. */

    if (t->scope->protocol != DNS_PROTOCOL_DNS)
        return false;

    /* If we have picked no server, then we are working from the cache or some
     * other source, and DNSSEC might well be supported, hence return true. */
    if (!t->server)
        return true;

    /* Note that we do not check the feature level actually used for the
     * transaction but instead the feature level the server is known to support
     * currently, as the transaction feature level might be lower than what the
     * server actually supports, since we might have downgraded this
     * transaction's feature level because we got a
...
static bool dns_transaction_dnssec_supported_full(DnsTransaction* t)
{
    ...
    /* Checks whether our transaction our any of the auxiliary transactions
     * couldn't do DNSSEC. */
...
int dns_transaction_request_dnssec_keys(DnsTransaction* t)
{
     ...
     * - For SOA/NS queries with no matching response RR, and no NSEC/NSEC3, the
     * DS RR
     * - For DS queries with no matching response RRs, and no NSEC/NSEC3, the
     * parent's SOA RR
     * - For other queries with no matching response RRs, and no NSEC/NSEC3, the
     * SOA RR
     */
    ...
    if (!dns_transaction_dnssec_supported(t))
        return 0; /* If we can't do DNSSEC anyway there's no point in geting the
                     auxiliary RRs */
    ...
    DNS_ANSWER_FOREACH(rr, t->answer)
    {
        ...
        /* If this RR is in the negative trust anchor, we don't need to validate
         * it. */
        r = dns_transaction_negative_trust_anchor_lookup(
            t, dns_resource_key_name(rr->key));
        if (r < 0)
            return r;
        if (r > 0)
            continue;
        ...
        switch (rr->key->type)
        {
            ...
            case DNS_TYPE_RRSIG:
            {
                ...
                _cleanup_(dns_resource_key_unrefp) DnsResourceKey* dnskey =
                    NULL;
                ...
                {
                    r = dns_name_equal(rr->rrsig.signer,
                ...
                dnskey = dns_resource_key_new(rr->key->class, DNS_TYPE_DNSKEY,
                                              rr->rrsig.signer);
                if (!dnskey)
                    return -ENOMEM;
            ...
            case DNS_TYPE_DNSKEY:
            {
                ...
                ds = dns_resource_key_new(rr->key->class, DNS_TYPE_DS,
                                          dns_resource_key_name(rr->key));
                if (!ds)
                    return -ENOMEM;
            ...
            case DNS_TYPE_NS:
            {
                ...
                if (r == 0)
                {
                    /* Hmm, so this SOA RR doesn't match our original question.
                     * In this case, maybe this is a negative reply, and we need
                     * the a SOA RR's TTL in order to cache a negative entry?
                     * If so, we need to validate it, too. */

                    r = dns_answer_match_key(t->answer, t->key, NULL);
                    if (r < 0)
                        return r;
                    if (r > 0) /* positive reply, we won't need the SOA and
                                * hence don't need to validate it. */
                        continue;

                    /* Only bother with this if the SOA/NS RR we are looking at
                     * is actually a parent of what we are looking for,
                     * otherwise there's no value in it for us. */
                    r = dns_name_endswith(dns_resource_key_name(t->key),
                                          dns_resource_key_name(rr->key));
                    if (r < 0)
                        return r;
            ...
            case DNS_TYPE_DNAME:
            {
                ...
                log_debug(
                    "Requesting parent SOA to validate transaction %" PRIu16
            ...
            default:
            {
                ...
                soa = dns_resource_key_new(rr->key->class, DNS_TYPE_SOA,
                                           dns_resource_key_name(rr->key));
                if (!soa)
                    return -ENOMEM;
    ...
    if (r > 0)
    {
        ...
        /* If this was a SOA or NS request, then check if there's a DS RR for
         * the same domain. Note that this could also be used as indication that
         * we are not at a zone apex, but in real world setups there are too
         * many broken DNS servers (Hello, incapdns.net!) where non-terminal
         * zones return NXDOMAIN even though they have further children. If this
         * was a DS request, then it's signed when the parent zone is signed,
         * hence ask the parent SOA in that case. If this was any other RR then
         * ask for the SOA RR, to see if that is signed. */
        ...
        if (t->key->type == DNS_TYPE_DS)
        {
            ...
            {
                type = DNS_TYPE_SOA;
                log_debug("Requesting parent SOA (→ %s) to validate "
                          "transaction %" PRIu16
...
void dns_transaction_notify(DnsTransaction* t, DnsTransaction* source)
{
    ...
    /* Invoked whenever any of our auxiliary DNSSEC transactions completed its
       work. If the state is still PENDING, we are still in the loop that adds
       further DNSSEC transactions, hence don't check if we are ready yet. If
...
static int dns_transaction_requires_rrsig(DnsTransaction* t,
                                          DnsResourceRecord* rr)
{
    int r;
    ...
    switch (rr->key->type)
    {
        ...
        case DNS_TYPE_NS:
        {
            ...
            SET_FOREACH(dt, t->dnssec_transactions, i)
            {
                ...
                r = dns_name_equal(dns_resource_key_name(dt->key),
                                   dns_resource_key_name(rr->key));
                if (r < 0)
                    return r;
                if (r == 0)
        ...
        case DNS_TYPE_DNAME:
        {
            ...
            /*
             * CNAME/DNAME RRs cannot be located at a zone apex, hence look
             * directly for the parent SOA.
             *
             * DS RRs are signed if the parent is signed, hence also look at the
             * parent SOA
             */
        ...
        default:
        {
            ...
            /* Any other kind of RR (including DNSKEY/NSEC/NSEC3). Let's see if
             * our SOA lookup was authenticated */
...
static int dns_transaction_in_private_tld(DnsTransaction* t,
...
static int dns_transaction_requires_nsec(DnsTransaction* t)
{
    ...
    if (r > 0)
    {
        ...
        log_info("Detected a negative query %s in a private DNS zone, "
                 "permitting unsigned response.",
                 dns_resource_key_to_string(t->key, key_str, sizeof key_str));
        return false;
    }
    ...
    if (t->key->type == DNS_TYPE_DS)
    {
        ...
        /* We got a negative reply for this DS lookup? DS RRs are signed when
         * their parent zone is signed,
        ...
        /* We got a negative reply for this SOA/NS lookup? If so, check if
         * there's a DS RR for this */
        type = DNS_TYPE_DS;
    else
...
static int dns_transaction_dnskey_authenticated(DnsTransaction* t,
                                                DnsResourceRecord* rr)
{
...
static int dns_transaction_known_signed(DnsTransaction* t,
                                        DnsResourceRecord* rr)
{
    assert(t);
    assert(rr);
...
static int dns_transaction_check_revoked_trust_anchors(DnsTransaction* t)
{
    ...
    DNS_ANSWER_FOREACH(rr, t->answer)
    {
        r = dns_trust_anchor_check_revoked(&t->scope->manager->trust_anchor, rr,
                                           t->answer);
        if (r < 0)
            return r;
    }
...
static int dns_transaction_invalidate_revoked_keys(DnsTransaction* t)
{
    ...
    do
    {
        ...
        {
            r = dns_trust_anchor_is_revoked(&t->scope->manager->trust_anchor,
                                            rr);
            if (r < 0)
                return r;
            if (r > 0)
...
static int dns_transaction_copy_validated(DnsTransaction* t)
{
    ...
    /* Copy all validated RRs from the auxiliary DNSSEC transactions into our
     * set of validated RRs */
...
static int dnssec_validate_records(DnsTransaction* t, Phase phase,
                                   bool* have_nsec, DnsAnswer** validated)
{

    DnsResourceRecord* rr;
    int r;

    /* Returns negative on error, 0 if validation failed, 1 to restart
     * validation, 2 when finished. */
    ...
    DNS_ANSWER_FOREACH(rr, t->answer)
    {
        ...
        r = dnssec_verify_rrset_search(t->answer, rr->key, t->validated_keys,
                                       USEC_INFINITY, &result, &rrsig);
        if (r < 0)
            return r;

        log_debug("Looking at %s: %s", strna(dns_resource_record_to_string(rr)),
        ...
        if (result == DNSSEC_VALIDATED)
        {
            ...
            {
                /* If we just validated a DNSKEY RRset, then let's add these
                 * keys to the set of validated keys for this transaction. */

                r = dns_answer_copy_by_key(&t->validated_keys, t->answer,
                                           rr->key, DNS_ANSWER_AUTHENTICATED);
                if (r < 0)
                    return r;

                /* Some of the DNSKEYs we just added might already have been
                 * revoked, remove them again in that case. */
            ...
            /* Exit the loop, we dropped something from the answer, start from
             * the beginning */
            return 1;
        }

        /* If we haven't read all DNSKEYs yet a negative result of the
         * validation is irrelevant, as there might be more DNSKEYs coming.
         * Similar, if we haven't read all NSEC/NSEC3 RRs yet, we cannot do
         * positive wildcard proofs yet, as those require the NSEC/NSEC3 RRs. */
        if (phase != DNSSEC_PHASE_ALL)
            continue;
        ...
        if (result == DNSSEC_VALIDATED_WILDCARD)
        {
            ...
            r = dnssec_test_positive_wildcard(
                *validated, dns_resource_key_name(rr->key), source,
                rrsig->rrsig.signer, &authenticated);

            /* Unless the NSEC proof showed that the key really doesn't exist
             * something is off. */
            if (r == 0)
            ...
            else
            {
                ...
                manager_dnssec_verdict(
                    t->scope->manager,
                    authenticated ? DNSSEC_SECURE : DNSSEC_INSECURE, rr->key);

                /* Exit the loop, we dropped something from the answer, start
                 * from the beginning */
                return 1;
            }
        }
        ...
        if (result == DNSSEC_NO_SIGNATURE)
        {
            ...
            if (r == 0)
            {
                /* Data does not require signing. In that case, just copy it
                 * over, but remember that this is by no means authenticated. */
                r = dns_answer_move_by_key(validated, &t->answer, rr->key, 0);
                if (r < 0)
                    return r;

                manager_dnssec_verdict(t->scope->manager, DNSSEC_INSECURE,
                                       rr->key);
                return 1;
            }
            ...
            if (r > 0)
            {
                /* This is an RR we know has to be signed. If it isn't this
                 * means the server is not attaching RRSIGs, hence complain. */
                ...
                if (t->scope->dnssec_mode == DNSSEC_ALLOW_DOWNGRADE)
                {
                    ...
                    /* Downgrading is OK? If so, just consider the information
                     * unsigned */
            ...
            if (r > 0)
            {
                ...
                /* The data is from a TLD that is proven not to exist, and we
                 * are in downgrade mode, hence ignore the fact that this was
                 * not signed. */

                log_info("Detected RRset %s is in a private DNS zone, "
                         "permitting unsigned RRs.",
        ...
        if (IN_SET(result, DNSSEC_MISSING_KEY, DNSSEC_SIGNATURE_EXPIRED,
                   DNSSEC_UNSUPPORTED_ALGORITHM))
        {
            ...
            if (r == 0)
            {
                /* The DNSKEY transaction was not authenticated, this means
                 * there's no DS for this, which means it's OK if no keys are
        ...
        if (r > 0)
        {
            ...
            if (r == 0)
            {
                if (IN_SET(result, DNSSEC_INVALID, DNSSEC_SIGNATURE_EXPIRED,
                           DNSSEC_NO_SIGNATURE))
                    manager_dnssec_verdict(t->scope->manager, DNSSEC_BOGUS,
                                           rr->key);
                else /* DNSSEC_MISSING_KEY or DNSSEC_UNSUPPORTED_ALGORITHM */
                    manager_dnssec_verdict(t->scope->manager,
                                           DNSSEC_INDETERMINATE, rr->key);

                /* This is a primary response to our question, and it failed
                 * validation. That's fatal. */
...
int dns_transaction_validate_dnssec(DnsTransaction* t)
{
    ...
    if (IN_SET(t->answer_source, DNS_TRANSACTION_ZONE,
    ...
    {
        /* The server does not support DNSSEC, or doesn't augment responses with
         * RRSIGs. */
        t->answer_dnssec_result = DNSSEC_INCOMPATIBLE_SERVER;
    ...
    for (;;)
    {
        ...
        {
            /* OK, we processed all DNSKEYs, and there are NSEC/NSEC3 RRs, look
             * at those now. */
        ...
        if (phase != DNSSEC_PHASE_ALL)
        {
            /* OK, we processed all DNSKEYs and NSEC/NSEC3 RRs, look at all the
             * rest now.
             * Note that in this third phase we start to remove RRs we couldn't
             * validate. */
    ...
    else if (r == 0)
    {
        ...
        r = dnssec_nsec_test(t->answer, t->key, &nr, &authenticated,
                             &t->answer_nsec_ttl);
        if (r < 0)
            return r;
        ...
        switch (nr)
        {
            ...
            case DNSSEC_NSEC_NXDOMAIN:
                ...
                log_debug(
                    "Proved NXDOMAIN via NSEC/NSEC3 for transaction %u (%s)",
                    t->id, key_str);
                t->answer_dnssec_result = DNSSEC_VALIDATED;
                ...
                manager_dnssec_verdict(
                    t->scope->manager,
                    authenticated ? DNSSEC_SECURE : DNSSEC_INSECURE, t->key);
                break;

            case DNSSEC_NSEC_NODATA:
                /* NSEC proves that there's no data here, very good. */
                log_debug(
                    "Proved NODATA via NSEC/NSEC3 for transaction %u (%s)",
                    t->id, key_str);
                t->answer_dnssec_result = DNSSEC_VALIDATED;
                t->answer_rcode = DNS_RCODE_SUCCESS;
                t->answer_authenticated = authenticated;
            ...
            case DNSSEC_NSEC_OPTOUT:
                ...
                log_debug("Data is NSEC3 opt-out via NSEC/NSEC3 for "
                          "transaction %u (%s)",
                          t->id, key_str);
                t->answer_dnssec_result = DNSSEC_UNSIGNED;
                t->answer_authenticated = false;
            ...
            case DNSSEC_NSEC_UNSUPPORTED_ALGORITHM:
                ...
                manager_dnssec_verdict(t->scope->manager, DNSSEC_INDETERMINATE,
                                       t->key);
                break;

            case DNSSEC_NSEC_FOUND:
            case DNSSEC_NSEC_CNAME:
                /* NSEC says it needs to be there, but we couldn't find it?
                 * Bummer! */
...
static const char* const
    dns_transaction_state_table[_DNS_TRANSACTION_STATE_MAX] = {
...
static const char* const
    dns_transaction_source_table[_DNS_TRANSACTION_SOURCE_MAX] = {
