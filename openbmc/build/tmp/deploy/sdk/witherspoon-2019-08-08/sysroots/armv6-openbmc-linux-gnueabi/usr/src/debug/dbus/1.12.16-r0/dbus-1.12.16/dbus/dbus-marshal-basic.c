...
static void pack_2_octets(dbus_uint16_t value, int byte_order,
                          unsigned char* data)
{
...
static void pack_4_octets(dbus_uint32_t value, int byte_order,
                          unsigned char* data)
{
...
static void pack_8_octets(DBusBasicValue value, int byte_order,
                          unsigned char* data)
{
...
static void set_2_octets(DBusString* str, int offset, dbus_uint16_t value,
                         int byte_order)
{
    char* data;
...
static void set_4_octets(DBusString* str, int offset, dbus_uint32_t value,
                         int byte_order)
{
    char* data;
...
static void set_8_octets(DBusString* str, int offset, DBusBasicValue value,
                         int byte_order)
{
    char* data;
 ...
 *
 */
void _dbus_marshal_set_uint32(DBusString* str, int pos, dbus_uint32_t value,
                              int byte_order)
{
 ...
 *
 */
static dbus_bool_t set_string(DBusString* str, int pos, const char* value,
                              int byte_order, int* old_end_pos,
                              int* new_end_pos)
{
    int old_len, new_len;
    DBusString dstr;
    ...
    old_len = _dbus_unpack_uint32(
        byte_order, _dbus_string_get_const_udata_len(str, pos, 4));

    new_len = _dbus_string_get_length(&dstr);

    if (!_dbus_string_replace_len(&dstr, 0, new_len, str, pos + 4, old_len))
        return FALSE;
 ...
 *
 */
static dbus_bool_t set_signature(DBusString* str, int pos, const char* value,
                                 int byte_order, int* old_end_pos,
                                 int* new_end_pos)
{
    int old_len, new_len;
    ...
    if (!_dbus_string_replace_len(&dstr, 0, new_len, str, pos + 1, old_len))
        return FALSE;
 ...
 */
dbus_bool_t _dbus_marshal_set_basic(DBusString* str, int pos, int type,
                                    const void* value, int byte_order,
                                    int* old_end_pos, int* new_end_pos)
{
    ...
    switch (type)
    {
        ...
        case DBUS_TYPE_OBJECT_PATH:
            ...
            return set_string(str, pos, vp->str, byte_order, old_end_pos,
                              new_end_pos);
            break;
        case DBUS_TYPE_SIGNATURE:
            _dbus_assert(vp->str != NULL);
            return set_signature(str, pos, vp->str, byte_order, old_end_pos,
                                 new_end_pos);
            break;
        default:
 ...
 */
dbus_uint32_t _dbus_marshal_read_uint32(const DBusString* str, int pos,
                                        int byte_order, int* new_pos)
{
    pos = _DBUS_ALIGN_VALUE(pos, 4);
 ...
 **/
void _dbus_marshal_read_basic(const DBusString* str, int pos, int type,
                              void* value, int byte_order, int* new_pos)
{
    const char* str_data;
...
static dbus_bool_t marshal_2_octets(DBusString* str, int insert_at,
                                    dbus_uint16_t value, int byte_order,
                                    int* pos_after)
{
    dbus_bool_t retval;
    int orig_len;
...
static dbus_bool_t marshal_4_octets(DBusString* str, int insert_at,
                                    dbus_uint32_t value, int byte_order,
                                    int* pos_after)
{
    dbus_bool_t retval;
    int orig_len;
...
static dbus_bool_t marshal_8_octets(DBusString* str, int insert_at,
                                    DBusBasicValue value, int byte_order,
                                    int* pos_after)
{
    dbus_bool_t retval;
    int orig_len;
...
static dbus_bool_t
    marshal_len_followed_by_bytes(int marshal_as, DBusString* str,
                                  int insert_at, const unsigned char* value,
                                  int data_len, /* doesn't include nul if any */
                                  int byte_order, int* pos_after)
{
    int pos;
    ...
    if (insert_at > _dbus_string_get_length(str))
        _dbus_warn("insert_at = %d string len = %d data_len = %d", insert_at,
                   _dbus_string_get_length(str), data_len);
    ...
    {
        if (!marshal_4_octets(str, pos, data_len, byte_order, &pos))
            goto oom;
    }

    if (!_dbus_string_copy_len(&value_str, 0, value_len, str, pos))
        goto oom;
...
static dbus_bool_t marshal_string(DBusString* str, int insert_at,
                                  const char* value, int byte_order,
                                  int* pos_after)
{
    return marshal_len_followed_by_bytes(MARSHAL_AS_STRING, str, insert_at,
                                         (const unsigned char*)value,
                                         strlen(value), byte_order, pos_after);
}

static dbus_bool_t marshal_signature(DBusString* str, int insert_at,
                                     const char* value, int* pos_after)
{
    return marshal_len_followed_by_bytes(
        MARSHAL_AS_SIGNATURE, str, insert_at, (const unsigned char*)value,
        strlen(value), DBUS_COMPILER_BYTE_ORDER, /* irrelevant */
        pos_after);
}
 ...
 **/
dbus_bool_t _dbus_marshal_write_basic(DBusString* str, int insert_at, int type,
                                      const void* value, int byte_order,
                                      int* pos_after)
{
    const DBusBasicValue* vp;
    ...
    switch (type)
    {
        ...
        case DBUS_TYPE_INT16:
        case DBUS_TYPE_UINT16:
            return marshal_2_octets(str, insert_at, vp->u16, byte_order,
                                    pos_after);
            break;
        case DBUS_TYPE_BOOLEAN:
        ...
        case DBUS_TYPE_UINT32:
        case DBUS_TYPE_UNIX_FD:
            return marshal_4_octets(str, insert_at, vp->u32, byte_order,
                                    pos_after);
            break;
        case DBUS_TYPE_INT64:
        case DBUS_TYPE_UINT64:
        ...
        case DBUS_TYPE_OBJECT_PATH:
            _dbus_assert(vp->str != NULL);
            return marshal_string(str, insert_at, vp->str, byte_order,
                                  pos_after);
            break;
        case DBUS_TYPE_SIGNATURE:
            _dbus_assert(vp->str != NULL);
...
static dbus_bool_t marshal_1_octets_array(DBusString* str, int insert_at,
                                          const unsigned char* value,
                                          int n_elements, int byte_order,
                                          int* pos_after)
{
    int pos;
    DBusString value_str;
    ...
    if (!_dbus_string_copy_len(&value_str, 0, n_elements, str, pos))
        return FALSE;
...
static void swap_array(DBusString* str, int array_start, int n_elements,
                       int byte_order, int alignment)
{
    _dbus_assert(_DBUS_ALIGN_VALUE(array_start, alignment) ==
...
static dbus_bool_t marshal_fixed_multi(DBusString* str, int insert_at,
                                       const DBusBasicValue* value,
                                       int n_elements, int byte_order,
                                       int alignment, int* pos_after)
{
    int old_string_len;
    int array_start;
    DBusString t;
    ...
    if (!_dbus_string_copy(&t, 0, str, array_start))
        goto error;
 ...
 * @returns #TRUE on success
 **/
dbus_bool_t _dbus_marshal_write_fixed_multi(DBusString* str, int insert_at,
                                            ...
                                            int* pos_after)
{
  ...
  _dbus_verbose ("writing %d elements of %s\n",
    ...
    {
        case DBUS_TYPE_BYTE:
            return marshal_1_octets_array(str, insert_at, vp, n_elements,
                                          byte_order, pos_after);
            break;
        case DBUS_TYPE_INT16:
        case DBUS_TYPE_UINT16:
            return marshal_fixed_multi(str, insert_at, vp, n_elements,
                                       byte_order, 2, pos_after);
        case DBUS_TYPE_BOOLEAN:
        case DBUS_TYPE_INT32:
        case DBUS_TYPE_UINT32:
        case DBUS_TYPE_UNIX_FD:
            return marshal_fixed_multi(str, insert_at, vp, n_elements,
                                       byte_order, 4, pos_after);
            break;
        case DBUS_TYPE_INT64:
        case DBUS_TYPE_UINT64:
        case DBUS_TYPE_DOUBLE:
            return marshal_fixed_multi(str, insert_at, vp, n_elements,
                                       byte_order, 8, pos_after);
            break;
 ...
 **/
void _dbus_marshal_skip_basic(const DBusString* str, int type, int byte_order,
                              int* pos)
{
    _dbus_assert(byte_order == DBUS_LITTLE_ENDIAN ||
                 byte_order == DBUS_BIG_ENDIAN);
 ...
 *            updated on return to new position
 */
void _dbus_marshal_skip_array(const DBusString* str, int element_type,
                              int byte_order, int* pos)
{
    ...
    switch (typecode)
    {
        ...
        default:
            _dbus_assert_not_reached(
                "unknown typecode in _dbus_type_get_alignment()");
            return 0;
    }
}
 ...
 */
void _dbus_marshal_read_fixed_multi(const DBusString* str, int pos,
                                    int element_type, void* value,
                                    int n_elements, int byte_order,
                                    int* new_pos)
{
    int array_len;
...
static void swap_test_array(void* array, int len_bytes, int byte_order,
                            int alignment)
{
    DBusString t;
...
#define DEMARSHAL_BASIC(typename, byte_order)                                  \
    do                                                                         \
    {                                                                          \
        _dbus_marshal_read_basic(&str, pos, DBUS_TYPE_##typename,              \
                                 &v_##typename, byte_order, &pos);             \
    } while (0)
    ...
    {                                                                          \
        ...
        {                                                                      \
            _dbus_verbose_bytes_of_string(                                     \
                &str, dump_pos, _dbus_string_get_length(&str) - dump_pos);     \
...
#define MARSHAL_FIXED_ARRAY(typename, byte_order, literal)                     \
    ...
    {                                                                          \
        ...
        if (!_dbus_marshal_write_fixed_multi(                                  \
                &str, next, DBUS_TYPE_##typename, &v_ARRAY_##typename,         \
                _DBUS_N_ELEMENTS(literal), byte_order, NULL))                  \
            _dbus_assert_not_reached("no memory");                             \
    } while (0)
...
#define DEMARSHAL_FIXED_ARRAY(typename, byte_order)                            \
    ...
    {                                                                          \
        ...
        v_UINT32 =                                                             \
            _dbus_marshal_read_uint32(&str, dump_pos, byte_order, &next);      \
        _dbus_marshal_read_fixed_multi(                                        \
            &str, next, DBUS_TYPE_##typename, &v_ARRAY_##typename,             \
            v_UINT32 / alignment, byte_order, NULL);                           \
...
#define DEMARSHAL_FIXED_ARRAY_AND_CHECK(typename, byte_order, literal)         \
    ...
    {                                                                          \
        ...
        {                                                                      \
            ...
            _dbus_verbose_bytes((const unsigned char*)literal,                 \
                                sizeof(literal), 0);                           \
            _dbus_verbose("READ DATA\n");                                      \
            _dbus_verbose_bytes((const unsigned char*)v_ARRAY_##typename,      \
                                sizeof(literal), 0);                           \
...
dbus_bool_t _dbus_marshal_test(void)
{
    ...
    MARSHAL_TEST(INT64, DBUS_LITTLE_ENDIAN,
                 DBUS_INT64_CONSTANT(-0x123456789abc7));

    /* Marshal unsigned integers */
    MARSHAL_TEST(UINT64, DBUS_BIG_ENDIAN,
                 DBUS_UINT64_CONSTANT(0x123456789abc7));
    MARSHAL_TEST(UINT64, DBUS_LITTLE_ENDIAN,
                 DBUS_UINT64_CONSTANT(0x123456789abc7));
  ...
  _dbus_assert (0x123456 ==
    ...
    while (TRUE)
    {
        ...
        _dbus_marshal_set_basic(&str, 0, DBUS_TYPE_STRING, &v_STRING,
                                byte_order, NULL, NULL);

        _dbus_marshal_read_basic(&str, 0, DBUS_TYPE_STRING, &v_STRING,
                                 byte_order, NULL);
