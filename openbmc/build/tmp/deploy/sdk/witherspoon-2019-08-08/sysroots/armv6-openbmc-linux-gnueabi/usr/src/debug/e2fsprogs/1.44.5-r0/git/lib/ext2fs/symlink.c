                         ...
                         const char* name, const char* target)
{
    ...
    if (!fastlink)
    {
        retval = ext2fs_new_block2(
            fs, ext2fs_find_inode_goal(fs, ino, &inode, 0), NULL, &blk);
        if (retval)
            goto cleanup;
    }
    ...
    if (!ino)
    {
        retval = ext2fs_new_inode(fs, parent, LINUX_S_IFLNK | 0755, 0, &ino);
        if (retval)
            goto cleanup;
    }
    ...
    else if (inlinelink)
    {
        ...
        if (retval)
            goto cleanup;
        retval = ext2fs_inline_data_set(fs, ino, &inode, block_buf, target_len);
        if (retval)
        {
            inode.i_flags &= ~EXT4_INLINE_DATA_FL;
            inlinelink = 0;
    ...
    {
        retval = ext2fs_bmap2(fs, ino, &inode, NULL, BMAP_SET, 0, NULL, &blk);
        if (retval)
            goto cleanup;
    ...
    if (name)
    {
        retval = ext2fs_lookup(fs, parent, name, strlen(name), 0, &scratch_ino);
        if (!retval)
        {
