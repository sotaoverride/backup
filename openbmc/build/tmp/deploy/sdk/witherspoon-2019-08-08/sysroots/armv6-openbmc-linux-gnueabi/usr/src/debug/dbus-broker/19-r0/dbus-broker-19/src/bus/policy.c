...
static int policy_xmit_new(PolicyXmit** xmitp, unsigned int type,
                           unsigned int broadcast, const char* path,
                           const char* interface, const char* member,
                           uint64_t min_fds, uint64_t max_fds)
{
...
static PolicyBatchName* policy_batch_name_free(PolicyBatchName* name)
{
    ...
    while ((xmit = c_list_first_entry(&name->recv_unindexed, PolicyXmit,
                                      batch_link)))
        policy_xmit_free(xmit);
    while ((xmit = c_list_first_entry(&name->send_unindexed, PolicyXmit,
                                      batch_link)))
        policy_xmit_free(xmit);
...
static int policy_batch_name_new(PolicyBatchName** namep, PolicyBatch* batch,
                                 const char* name_str)
{
...
void policy_batch_free(_Atomic unsigned long* n_refs, void* userdata)
{
    ...
    c_rbtree_for_each_entry_safe_postorder_unlink(name, t_name,
                                                  &batch->name_tree, batch_node)
        policy_batch_name_free(name);

    free(batch);
}

static PolicyBatchName* policy_batch_find_name(PolicyBatch* batch,
                                               const char* name_str)
{
    return c_rbtree_find_entry(&batch->name_tree, policy_batch_name_compare,
                               name_str, PolicyBatchName, batch_node);
}

static int policy_batch_at_name(PolicyBatch* batch, PolicyBatchName** namep,
                                const char* name_str)
{
    CRBNode *parent, **slot;
    ...
    slot = c_rbtree_find_slot(&batch->name_tree, policy_batch_name_compare,
                              name_str, &parent);
    if (slot)
    {
...
static int policy_batch_add_own(PolicyBatch* batch, const char* name_str,
...
static int policy_batch_add_own_prefix(PolicyBatch* batch, const char* name_str,
                                       PolicyVerdict verdict)
{
    PolicyBatchName* name;
    int r;
...
static int policy_batch_add_send(PolicyBatch* batch, const char* name_str,
                                 PolicyVerdict verdict, unsigned int type,
                                 unsigned int broadcast, const char* path,
                                 const char* interface, const char* member,
                                 uint64_t min_fds, uint64_t max_fds)
{
    ...
    r = policy_xmit_new(&xmit, type, broadcast, path, interface, member,
                        min_fds, max_fds);
    if (r)
        return error_trace(r);
...
static int policy_batch_add_recv(PolicyBatch* batch, const char* name_str,
                                 PolicyVerdict verdict, unsigned int type,
                                 unsigned int broadcast, const char* path,
                                 const char* interface, const char* member,
                                 uint64_t min_fds, uint64_t max_fds)
...
static int policy_registry_node_compare(CRBTree* t, void* k, CRBNode* n)
{
    PolicyRegistryNode* node =
        c_container_of(n, PolicyRegistryNode, registry_node);
...
static int policy_registry_node_new(PolicyRegistryNode** nodep,
                                    uint32_t uidgid_start, uint32_t uidgid_end)
{
...
PolicyRegistry* policy_registry_free(PolicyRegistry* registry)
{
    ...
    c_rbtree_for_each_entry_safe_postorder_unlink(
        node, t_node, &registry->gid_tree, registry_node)
        policy_registry_node_free(node);
    c_rbtree_for_each_entry_safe_postorder_unlink(
        node, t_node, &registry->uid_tree, registry_node)
        policy_registry_node_free(node);
    c_rbtree_for_each_entry_safe_postorder_unlink(
        node, t_node, &registry->uid_range_tree, registry_node)
        policy_registry_node_free(node);
...
static PolicyRegistryNode* policy_registry_find_uid(PolicyRegistry* registry,
                                                    uint32_t uid)
{
    ...
    return c_rbtree_find_entry(&registry->uid_tree,
                               policy_registry_node_compare, &index,
                               PolicyRegistryNode, registry_node);
}

static PolicyRegistryNode* policy_registry_find_gid(PolicyRegistry* registry,
                                                    uint32_t gid)
{
    PolicyRegistryNodeIndex index = {
        .uidgid_start = gid,
...
static int policy_registry_at_uidgid(CRBTree* tree, PolicyRegistryNode** nodep,
                                     uint32_t uidgid_start, uint32_t uidgid_end)
{
    CRBNode *parent, **slot;
    PolicyRegistryNode* node;
    ...
    slot =
        c_rbtree_find_slot(tree, policy_registry_node_compare, &index, &parent);
    if (slot)
    {
...
static int policy_registry_at_uid(PolicyRegistry* registry,
                                  PolicyRegistryNode** nodep, uint32_t uid)
{
    return policy_registry_at_uidgid(&registry->uid_tree, nodep, uid, uid);
}

static int policy_registry_at_uid_range(PolicyRegistry* registry,
                                        PolicyRegistryNode** nodep,
                                        uint32_t uid_start, uint32_t uid_end)
{
    return policy_registry_at_uidgid(&registry->uid_range_tree, nodep,
                                     uid_start, uid_end);
}

static int policy_registry_at_gid(PolicyRegistry* registry,
                                  PolicyRegistryNode** nodep, uint32_t gid)
{
                                        ...
                                        PolicyBatch* batch, CDVar* v)
{
    ...
    while (c_dvar_more(v))
    {
        c_dvar_read(v, "(btbs)", &verdict.verdict, &verdict.priority,
                    &is_prefix, &name_str);
    ...
    while (c_dvar_more(v))
    {
        c_dvar_read(v, "(btssssuutt)", &verdict.verdict, &verdict.priority,
                    &name_str, &path, &interface, &member, &type, &broadcast,
                    &min_fds, &max_fds);

        if (broadcast >= _UTIL_TRISTATE_N)
            return POLICY_E_INVALID;

        r = policy_batch_add_send(batch, name_str, verdict, type, broadcast,
                                  path, interface, member, min_fds, max_fds);
        if (r)
            return error_trace(r);
    }
    ...
    while (c_dvar_more(v))
    {
        ...
        r = policy_batch_add_recv(batch, name_str, verdict, type, broadcast,
                                  path, interface, member, min_fds, max_fds);
        if (r)
            return error_trace(r);
    }
...
int policy_registry_import(PolicyRegistry* registry, CDVar* v)
{
    ...
    while (c_dvar_more(v))
    {
        ...
        if (uid == (uint32_t)-1)
        {
            r = policy_registry_import_batch(registry, registry->default_batch,
                                             v);
            if (r)
                return error_trace(r);
        }
    ...
    while (c_dvar_more(v))
    {
        ...
        {
            r = policy_registry_at_uid_range(registry, &node, uidgid_start,
                                             uidgid_end);
            if (r)
                return error_trace(r);
        }
 ...
 */
int policy_snapshot_new(PolicySnapshot** snapshotp, PolicyRegistry* registry,
                        const char* seclabel, uint32_t uid,
                        const uint32_t* gids, size_t n_gids)
{
    ...
    snapshot =
        calloc(1, sizeof(*snapshot) + n_batches * sizeof(*snapshot->batches));
    if (!snapshot)
        return error_origin(-ENOMEM);
    ...
    *snapshot = (PolicySnapshot)POLICY_SNAPSHOT_NULL;
    ...
    if (node)
        snapshot->batches[snapshot->n_batches++] =
            policy_batch_ref(node->batch);
    else
        snapshot->batches[snapshot->n_batches++] =
            policy_batch_ref(registry->default_batch);
...
int policy_snapshot_check_own(PolicySnapshot* snapshot, const char* name_str)
{
    ...
    for (i = 0; i < snapshot->n_batches; ++i)
    {
         ...
         */
        for (end = name_str;; end = strchrnul(end + 1, '.'))
        {
...
static void policy_snapshot_check_xmit_name(
    PolicyBatch* batch, bool is_send, PolicyVerdict* verdict,
    const char* name_str, const char* interface, const char* member,
    const char* path, unsigned int type, bool broadcast, size_t n_fds)
{
    PolicyBatchName* name;
    PolicyXmit* xmit;
    CList* list;
...
static void policy_snapshot_check_xmit(PolicyBatch* batch, bool is_send,
                                       PolicyVerdict* verdict, NameSet* nameset,
                                       const char* interface,
                                       const char* method, const char* path,
                                       unsigned int type, bool broadcast,
                                       size_t n_fds)
{
     ...
     */
    policy_snapshot_check_xmit_name(batch, is_send, verdict, "", interface,
                                    method, path, type, broadcast, n_fds);
    ...
    if (!nameset)
    {
         ...
         */
        policy_snapshot_check_xmit_name(batch, is_send, verdict,
    ...
    else if (nameset->type == NAME_SET_TYPE_OWNER)
    {
         ...
         */
        c_rbtree_for_each_entry(ownership, &nameset->owner->ownership_tree,
                                owner_node)
            policy_snapshot_check_xmit_name(
                batch, is_send, verdict, ownership->name->name, interface,
                method, path, type, broadcast, n_fds);
    }
    else if (nameset->type == NAME_SET_TYPE_SNAPSHOT)
    {
        ...
        for (i = 0; i < nameset->snapshot->n_names; ++i)
            policy_snapshot_check_xmit_name(
                batch, is_send, verdict, nameset->snapshot->names[i]->name,
                interface, method, path, type, broadcast, n_fds);
    }
...
int policy_snapshot_check_send(PolicySnapshot* snapshot,
                               const char* subject_seclabel, NameSet* subject,
                               const char* interface, const char* method,
                               const char* path, unsigned int type,
                               bool broadcast, size_t n_fds)
{
    PolicyVerdict verdict = POLICY_VERDICT_INIT;
    size_t i;
    int r;

    r = bus_selinux_check_send(snapshot->selinux, snapshot->seclabel,
                               subject_seclabel);
    if (r)
    {
        if (r == SELINUX_E_DENIED)
    ...
    for (i = 0; i < snapshot->n_batches; ++i)
        policy_snapshot_check_xmit(snapshot->batches[i], true, &verdict,
                                   subject, interface, method, path, type,
                                   broadcast, n_fds);
 ...
 */
int policy_snapshot_check_receive(PolicySnapshot* snapshot, NameSet* subject,
                                  const char* interface, const char* method,
                                  const char* path, unsigned int type,
                                  bool broadcast, size_t n_fds)
{
    ...
    for (i = 0; i < snapshot->n_batches; ++i)
        policy_snapshot_check_xmit(snapshot->batches[i], false, &verdict,
                                   subject, interface, method, path, type,
                                   broadcast, n_fds);
