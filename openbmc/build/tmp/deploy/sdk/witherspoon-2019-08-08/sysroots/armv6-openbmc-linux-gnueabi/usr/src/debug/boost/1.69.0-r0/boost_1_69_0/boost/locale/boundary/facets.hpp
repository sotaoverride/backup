...
///
/// \brief This namespae contains all operations required for boundary analysis
/// of text
///
namespace boundary
{
...
/// for a given text.
///
/// It is specialized for 4 types of characters \c char_t, \c wchar_t, \c
/// char16_t and \c char32_t
///
...
class BOOST_LOCALE_DECL boundary_indexing : public std::locale::facet
{
  public:
    ...
    ///
    virtual index_type map(boundary_type t, Char const* begin,
                           Char const* end) const = 0;
    ///
...
class BOOST_LOCALE_DECL boundary_indexing<char16_t> : public std::locale::facet
{
  public:
    ...
    virtual index_type map(boundary_type t, char16_t const* begin,
                           char16_t const* end) const = 0;
    static std::locale::id id;
#if defined(__SUNPRO_CC) && defined(_RWSTD_VER)
    std::locale::id& __get_id(void) const
    {
...
class BOOST_LOCALE_DECL boundary_indexing<char32_t> : public std::locale::facet
{
  public:
    ...
    virtual index_type map(boundary_type t, char32_t const* begin,
                           char32_t const* end) const = 0;
    static std::locale::id id;
#if defined(__SUNPRO_CC) && defined(_RWSTD_VER)
    std::locale::id& __get_id(void) const
    {
