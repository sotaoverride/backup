...
static bool list_match(char* list, const char* item,
...
int login_access(const char* user, const char* from)
{
    ...
    if (NULL != fp)
    {
        ...
        while (!match && (fgets(line, (int)sizeof(line), fp) == line))
        {
            int end;
            lineno++;
            ...
            {
                SYSLOG((LOG_ERR,
                        "%s: line %d: missing newline or line too long", TABLE,
                        lineno));
                continue;
            }
            if (line[0] == '#')
            ...
            {
                SYSLOG(
                    (LOG_ERR, "%s: line %d: bad field count", TABLE, lineno));
                continue;
            }
            if (perm[0] != '+' && perm[0] != '-')
            {
                SYSLOG(
                    (LOG_ERR, "%s: line %d: bad first field", TABLE, lineno));
                continue;
            }
...
static bool netgroup_match(const char* group, const char* machine,
                           const char* user)
{
