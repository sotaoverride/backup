...
libc_hidden_ver(_IO_new_file_finish, _IO_file_finish)
    ...
    FILE* _IO_file_open(FILE* fp, const char* filename, int posix_mode,
                        int prot, int read_write, int is32not64)
{
    int fdesc;
    if (__glibc_unlikely(fp->_flags2 & _IO_FLAGS2_NOTCANCEL))
        fdesc = __open_nocancel(
            filename, posix_mode | (is32not64 ? 0 : O_LARGEFILE), prot);
    else
        fdesc =
            __open(filename, posix_mode | (is32not64 ? 0 : O_LARGEFILE), prot);
    if (fdesc < 0)
        return NULL;
    fp->_fileno = fdesc;
    ...
    if (result != NULL)
    {
        ...
        if (cs != NULL)
        {
            ...
            *((char*)__mempcpy(ccs, cs + 5, endp - (cs + 5))) = '\0';
            ...
            if (__wcsmbs_named_conv(&fcts, ccs[2] == '\0' ? upstr(ccs, cs + 5)
                                                          : ccs) != 0)
            {
            ...
            *cc = __libio_codecvt;
            ...
            cc->__cd_in.__cd.__data[0].__statep =
...
libc_hidden_ver(_IO_new_file_fopen, _IO_file_fopen)
    ...
    FILE* _IO_new_file_attach(FILE* fp, int fd)
{
    if (_IO_file_is_open(fp))
        return NULL;
    fp->_fileno = fd;
...
libc_hidden_ver(_IO_new_file_attach, _IO_file_attach)
    ...
    FILE* _IO_new_file_setbuf(FILE* fp, char* p, ssize_t len)
{
...
libc_hidden_ver(_IO_new_file_setbuf, _IO_file_setbuf)
    ...
    FILE* _IO_file_setbuf_mmap(FILE* fp, char* p, ssize_t len)
{
    FILE* result;
    ...
    static size_t new_do_write(FILE* fp, const char* data, size_t to_do)
{
    ...
    if (fp->_cur_column && count)
        fp->_cur_column =
            _IO_adjust_column(fp->_cur_column - 1, data, count) + 1;
    _IO_setg(fp, fp->_IO_buf_base, fp->_IO_buf_base, fp->_IO_buf_base);
    ...
    if (fp->_flags & (_IO_LINE_BUF | _IO_UNBUFFERED))
    {
       ...
       explicitly.  --drepper */
        ...
        if ((_IO_stdout->_flags &
             (_IO_LINKED | _IO_NO_WRITES | _IO_LINE_BUF)) ==
...
libc_hidden_ver(_IO_new_file_underflow, _IO_file_underflow)
    ...
    static int mmap_remap_check(FILE* fp)
{
    struct stat64 st;
...
#define ROUNDED(x) (((x) + pagesize - 1) & ~(pagesize - 1))
        if (ROUNDED(st.st_size) < ROUNDED(fp->_IO_buf_end - fp->_IO_buf_base))
        {
        ...
        else if (ROUNDED(st.st_size) >
                 ROUNDED(fp->_IO_buf_end - fp->_IO_buf_base))
        {
            /* The file added some pages.  We need to remap it.  */
            void* p;
#if _G_HAVE_MREMAP
            p = __mremap(fp->_IO_buf_base,
                         ROUNDED(fp->_IO_buf_end - fp->_IO_buf_base),
            ...
            (void)__munmap(fp->_IO_buf_base,
                           fp->_IO_buf_end - fp->_IO_buf_base);
            p = __mmap64(NULL, st.st_size, PROT_READ, MAP_SHARED, fp->_fileno,
                         0);
            if (p == MAP_FAILED)
                goto punt;
#endif
       ...
       buffer until EOF.  */
        ...
        if (fp->_offset < fp->_IO_buf_end - fp->_IO_buf_base)
        {
            if (__lseek64(fp->_fileno, fp->_IO_buf_end - fp->_IO_buf_base,
                          SEEK_SET) != fp->_IO_buf_end - fp->_IO_buf_base)
                fp->_flags |= _IO_ERR_SEEN;
            else
                fp->_offset = fp->_IO_buf_end - fp->_IO_buf_base;
        }
    ...
    if ((f->_flags & _IO_CURRENTLY_PUTTING) == 0 || f->_IO_write_base == NULL)
    {
       ...
       alone, so it can continue to correspond to the external position). */
        if (__glibc_unlikely(_IO_in_backup(f)))
        {
            ...
            f->_IO_read_base -=
                MIN(nbackup, f->_IO_read_base - f->_IO_buf_base);
...
libc_hidden_ver(_IO_new_file_sync, _IO_file_sync)
    ...
    static int _IO_file_sync_mmap(FILE* fp)
{
    if (fp->_IO_read_ptr != fp->_IO_read_end)
    {
        if (__lseek64(fp->_fileno, fp->_IO_read_ptr - fp->_IO_buf_base,
                      SEEK_SET) != fp->_IO_read_ptr - fp->_IO_buf_base)
        {
            fp->_flags |= _IO_ERR_SEEN;
            return EOF;
        }
...
off64_t _IO_new_file_seekoff(FILE* fp, off64_t offset, int dir, int mode)
{
    ...
    bool was_writing =
        (fp->_IO_write_ptr > fp->_IO_write_base || _IO_in_put_mode(fp));
        ...
        !_IO_in_backup(fp))
    {
        off64_t start_offset =
            (fp->_offset - (fp->_IO_read_end - fp->_IO_buf_base));
    ...
    else
    {
        count = _IO_SYSREAD(
            fp, fp->_IO_buf_base,
            (must_be_exact ? delta : fp->_IO_buf_end - fp->_IO_buf_base));
        if (count < delta)
        {
...
libc_hidden_ver(_IO_new_file_seekoff, _IO_file_seekoff)
    ...
    off64_t _IO_file_seekoff_mmap(FILE* fp, off64_t offset, int dir, int mode)
{
    off64_t result;
...
static off64_t _IO_file_seekoff_maybe_mmap(FILE* fp, off64_t offset, int dir,
                                           int mode)
{
...
libc_hidden_def(_IO_file_xsgetn)
    ...
    static size_t _IO_file_xsgetn_mmap(FILE* fp, void* data, size_t n)
{
    size_t have;
...
static size_t _IO_file_xsgetn_maybe_mmap(FILE* fp, void* data, size_t n)
{
