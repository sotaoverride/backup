...
template <typename OpenT, typename ExprT, typename CloseT, typename CategoryT,
          typename NestedT, typename LexemeT>
struct confix_parser
    : public parser<
          confix_parser<OpenT, ExprT, CloseT, CategoryT, NestedT, LexemeT>>
{
    typedef confix_parser<OpenT, ExprT, CloseT, CategoryT, NestedT, LexemeT>
        self_t;

    confix_parser(OpenT const& open_, ExprT const& expr_,
                  CloseT const& close_) :
        open(open_),
        expr(expr_), close(close_)
    {
    }
    ...
    {
        return impl::confix_parser_type<CategoryT>::parse(
            NestedT(), LexemeT(), *this, scan, open, expr, close);
    }
...
struct confix_parser_gen
{
    ...
    struct paren_op_result_type
    {
        typedef confix_parser<
            typename as_parser<StartT>::type, typename as_parser<ExprT>::type,
            typename as_parser<EndT>::type,
            typename as_parser<ExprT>::type::parser_category_t, NestedT,
            LexemeT>
            type;
    };
    ...
    typename paren_op_result_type<StartT, ExprT, EndT>::type
        operator()(StartT const& start_, ExprT const& expr_,
                   EndT const& end_) const
    {
        typedef
            typename paren_op_result_type<StartT, ExprT, EndT>::type return_t;

        return return_t(as_parser<StartT>::convert(start_),
    ...
    {
        typedef confix_parser<typename as_parser<StartT>::type,
...
struct comment_parser_gen
{
    ...
    {
        typedef confix_parser<
            typename as_parser<StartT>::type, kleene_star<anychar_parser>,
        ...
        operator()(StartT const& start_) const
    {
        ...
        return return_t(as_parser<StartT>::convert(start_), *anychar_p,
                        eol_p | end_p);
    }
...
struct comment_nest_parser : public parser<comment_nest_parser<OpenT, CloseT>>
{
  ...
  private:
    ...
    {
        return impl::contiguous_parser_parse<
            typename parser_result<ParserT, ScannerT>::type>(p, scan, scan);
    }
...
{
    typedef comment_nest_parser<typename as_parser<OpenT>::type,
    ...
    comment_nest_p(OpenT const& open, CloseT const& close)
{
    ...
    return result_t(as_parser<OpenT>::convert(open),
