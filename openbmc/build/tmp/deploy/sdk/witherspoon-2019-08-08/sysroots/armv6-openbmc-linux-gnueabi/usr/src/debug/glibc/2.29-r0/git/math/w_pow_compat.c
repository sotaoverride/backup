...
#include <math_private.h>
                         ...
                         defined NO_LONG_DOUBLE || defined LONG_DOUBLE_COMPAT)
/* wrapper pow */
double __pow_compat(double x, double y)
{
    ...
    else if (__builtin_expect(z == 0.0, 0) && isfinite(x) && x != 0 &&
             isfinite(y) && _LIB_VERSION != _IEEE_)
        /* pow underflow */
...
weak_alias(__pow_compat, powl)
    ...
    weak_alias(__pow_compat, __pow_compat_alias)
        ...
        LONG_DOUBLE_COMPAT_CHOOSE_libm_powl(
            compat_symbol(libm, __pow_compat_alias, powl,
                          FIRST_VERSION_libm_powl), );
#endif
#endif
