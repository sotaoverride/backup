...
static krb5_error_code tgs_construct_ap_req(krb5_context context,
                                            ...
                                            krb5_data** ap_req_asn1_out)
{
    ...
    *ap_req_asn1_out = NULL;
    ...
    if (ret)
        goto cleanup;
    ret =
        krb5_encrypt_helper(context, &tgt->keyblock, KRB5_KEYUSAGE_TGS_REQ_AUTH,
                            authent_asn1, &authent_enc);
    if (ret)
        goto cleanup;
 ...
 */
krb5_error_code k5_make_tgs_req(
    krb5_context context, struct krb5int_fast_request_state* fast_state,
    krb5_creds* tgt, krb5_flags kdcoptions, krb5_address* const* addrs,
    krb5_pa_data** in_padata, krb5_creds* desired, k5_pacb_fn pacb_fn,
    void* pacb_data, krb5_data* req_asn1_out, krb5_timestamp* timestamp_out,
    krb5_int32* nonce_out, krb5_keyblock** subkey_out)
{
    ...
    *subkey_out = NULL;
    ...
    req.till =
        desired->times.endtime ? desired->times.endtime : tgt->times.endtime;
    ...
    *timestamp_out = time_now;
    ...
    ret = krb5int_fast_prep_req_body(context, fast_state, &req, &req_body_asn1);
    if (ret)
        goto cleanup;

    ret =
        tgs_construct_ap_req(context, req_body_asn1, tgt, subkey, &ap_req_asn1);
    if (ret)
        goto cleanup;
    ...
    padata[0]->contents =
        k5memdup(ap_req_asn1->data, ap_req_asn1->length, &ret);
    if (padata[0] == NULL)
        goto cleanup;
    ...
    for (i = 0; in_padata != NULL && in_padata[i] != NULL; i++)
    {
        ...
        pa->contents =
            k5memdup(in_padata[i]->contents, in_padata[i]->length, &ret);
        if (pa->contents == NULL)
            goto cleanup;
        padata[i + 1] = pa;
    }
