...
static int help(void)
{
    ...
    printf(
        ...
        "     --fsck=no                    Don't run file system check before "
        "mount\n"
        ...
        "  -G --collect                    Unload unit after it stopped, even "
        "when failed\n"
        "\nSee the %s for details.\n",
        program_invocation_short_name,
        streq(program_invocation_short_name, "systemd-umount") ? ""
                                                               : "--umount ",
        link);
...
static int parse_argv(int argc, char* argv[])
{
    ...
    while ((c = getopt_long(argc, argv, "hqH:M:t:o:p:AuG", options, NULL)) >= 0)
        ...
        switch (c)
        {
            ...
            case ARG_FSCK:
                ...
                if (r < 0)
                    return log_error_errno(
                        r, "Failed to parse --fsck= argument: %s", optarg);
            ...
            case ARG_AUTOMOUNT:
                ...
                if (r < 0)
                    return log_error_errno(
                        r, "--automount= expects a valid boolean parameter: %s",
                        optarg);
            ...
            case ARG_TIMEOUT_IDLE:
                ...
                if (r < 0)
                    return log_error_errno(r, "Failed to parse timeout: %s",
                                           optarg);
    ...
    else
    {
        ...
        if (arg_mount_type && (fstype_is_api_vfs(arg_mount_type) ||
        ...
        else if (arg_transport == BUS_TRANSPORT_LOCAL)
        {
            ...
            if (r < 0)
                return log_error_errno(r, "Failed to make path %s absolute: %m",
                                       u);
        }
        else
        {
        ...
        if (argc > optind + 1)
        {
            if (arg_transport == BUS_TRANSPORT_LOCAL)
            {
                r = chase_symlinks(argv[optind + 1], NULL, CHASE_NONEXISTENT,
                                   &arg_mount_where);
                if (r < 0)
                    return log_error_errno(
                        r, "Failed to make path %s absolute: %m",
                        argv[optind + 1]);
            }
            else
            {
        ...
        if (arg_discover && arg_transport != BUS_TRANSPORT_LOCAL)
            return log_error_errno(SYNTHETIC_ERRNO(EOPNOTSUPP),
                                   "Automatic mount location discovery is only "
                                   "supported locally.");
    }
                                         ...
                                         char** properties)
{
    ...
    if (arg_bind_device && is_device_path(arg_mount_what))
    {
        ...
        r = sd_bus_message_append(m, "(sv)(sv)", "After", "as", 1, device_unit,
...
static int transient_mount_set_properties(sd_bus_message* m)
{
    ...
    {
        r = asprintf(&options, "uid=" UID_FMT ",gid=" GID_FMT "%s%s", arg_uid,
                     arg_gid, arg_mount_options ? "," : "",
    ...
    if (options || arg_mount_options)
    {
        ...
        r = sd_bus_message_append(m, "(sv)", "Options", "s",
                                  options ?: arg_mount_options);
        if (r < 0)
            return r;
    }
    else
    ...
    if (arg_fsck)
    {
        ...
        r = unit_name_from_path_instance("systemd-fsck", arg_mount_what,
                                         ".service", &fsck);
        if (r < 0)
            return r;

        r = sd_bus_message_append(m, "(sv)(sv)", "Requires", "as", 1, fsck,
                                  "After", "as", 1, fsck);
        if (r < 0)
            return r;
    }
...
static int transient_automount_set_properties(sd_bus_message* m)
{
    ...
    r = transient_unit_set_properties(m, UNIT_AUTOMOUNT,
                                      arg_automount_property);
    if (r < 0)
        return r;
    ...
    {
        r = sd_bus_message_append(m, "(sv)", "TimeoutIdleUSec", "t",
                                  arg_timeout_idle);
        if (r < 0)
            return r;
    }
...
static int start_transient_mount(sd_bus* bus, char** argv)
{
    ...
    if (r < 0)
        return log_error_errno(r, "Failed to start transient mount unit: %s",
    ...
    if (!arg_quiet)
        log_info("Started unit %s%s%s for mount point: %s%s%s",
                 ansi_highlight(), mount_unit, ansi_normal(), ansi_highlight(),
                 arg_mount_where, ansi_normal());
...
static int find_loop_device(const char* backing_file, char** loop_dev)
{
    ...
    FOREACH_DIRENT(de, d, return -errno)
    {
        ...
        sys = strjoin("/sys/devices/virtual/block/", de->d_name,
                      "/loop/backing_file");
        if (!sys)
            return -ENOMEM;
...
static int stop_mount(sd_bus* bus, const char* where, const char* suffix)
{
    ...
    if (r < 0)
        return log_error_errno(r,
                               "Failed to make %s unit name from path %s: %m",
                               suffix + 1, where);
    ...
    if (r < 0)
    {
        if (streq(suffix, ".automount") &&
            ...
            return 0;
        return log_error_errno(r, "Failed to stop %s unit: %s", suffix + 1,
                               bus_error_message(&error, r));
    }
...
static int stop_mounts(sd_bus* bus, const char* where)
{
    ...
    if (path_equal(where, "/"))
        return log_error_errno(SYNTHETIC_ERRNO(EINVAL),
                               "Refusing to operate on root directory: %s",
                               where);
...
static int umount_by_device(sd_bus* bus, const char* what)
{
    ...
    if (r < 0)
        return log_device_error_errno(d, r,
...
static int action_umount(sd_bus* bus, int argc, char** argv)
{
    ...
    for (i = optind; i < argc; i++)
    {
        ...
        if (r < 0)
        {
            r2 = log_error_errno(r, "Failed to make path %s absolute: %m",
                                 argv[i]);
            continue;
        }

        if (stat(p, &st) < 0)
            return log_error_errno(errno, "Can't stat %s (from %s): %m", p,
                                   argv[i]);
...
static int acquire_mount_where_for_loop_dev(const char* loop_dev)
{
    ...
    if (r < 0)
        ...
        return log_error_errno(SYNTHETIC_ERRNO(EINVAL),
                               "Can't find mount point of %s. It is expected "
                               "that %s is already mounted on a place.",
                               loop_dev, loop_dev);
    else if (r >= 2)
        return log_error_errno(SYNTHETIC_ERRNO(EINVAL),
                               "%s is mounted on %d places. It is expected "
                               "that %s is mounted on a place.",
                               loop_dev, r, loop_dev);
...
static int acquire_removable(sd_device* d)
{
    ...
    if (arg_action != ACTION_DEFAULT && arg_timeout_idle_set &&
        arg_bind_device >= 0)
        return 0;
...
static int discover_loop_backing_file(void)
{
    ...
    if (r < 0 && r != -ENXIO)
        return log_error_errno(errno, "Can't get loop device for %s: %m",
                               arg_mount_what);
    ...
    if (sd_device_get_property_value(d, "ID_FS_USAGE", &v) < 0 ||
        !streq(v, "filesystem"))
    {
        log_device_error(d, "%s does not contain a known file system.",
                         arg_mount_what);
        return -EINVAL;
    }
...
static int list_devices(void)
{
    ...
    static const char* const titles[_COLUMN_MAX] = {
        [COLUMN_NODE] = "NODE",   [COLUMN_PATH] = "PATH",
...
static int run(int argc, char* argv[])
{
    ...
    {
        log_error("Path contains non-normalized components: %s",
                  arg_mount_what);
        return -EINVAL;
    }
    ...
    if (arg_fsck && arg_mount_type && arg_transport == BUS_TRANSPORT_LOCAL)
    {
        ...
        if (r < 0)
            log_warning_errno(r,
                              "Couldn't determine whether fsck for %s exists, "
                              "proceeding anyway.",
                              arg_mount_type);
        else if (r == 0)
        {
    ...
    /* The kernel (properly) refuses mounting file systems with unknown
     * uid=,gid= options, but not for all filesystem types. Let's try to catch
     * the cases where the option would be used if the file system does not
     * support it. It is also possible to autodetect the file system, but that's
     * only possible with disk-based file systems which incidentally seem to be
     * implemented more carefully and reject unknown options, so it's probably
     * OK that we do the check only when the type is specified.
     */
    if (arg_mount_type && !streq(arg_mount_type, "auto") &&
        arg_uid != UID_INVALID && !fstype_can_uid_gid(arg_mount_type))
    {
        log_error(
