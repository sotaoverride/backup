...
const unsigned char tls11downgrade[] = {0x44, 0x4f, 0x57, 0x4e,
                                        0x47, 0x52, 0x44, 0x00};
const unsigned char tls12downgrade[] = {0x44, 0x4f, 0x57, 0x4e,
                                        0x47, 0x52, 0x44, 0x01};
...
const SSL3_ENC_METHOD SSLv3_enc_data = {ssl3_enc,
                                        n_ssl3_mac,
...
long ssl3_ctrl(SSL* s, int cmd, long larg, void* parg)
{
    ...
    switch (cmd)
    {
        ...
        case SSL_CTRL_SET_TMP_DH:
        {
            ...
            if (!ssl_security(s, SSL_SECOP_TMP_DH, EVP_PKEY_security_bits(pkdh),
                              0, pkdh))
            {
        ...
        case SSL_CTRL_SET_TMP_ECDH:
        {
            ...
            return tls1_set_groups(&s->ext.supportedgroups,
                                   &s->ext.supportedgroups_len, &nid, 1);
        }
        break;
#endif /* !OPENSSL_NO_EC */
        case SSL_CTRL_SET_TLSEXT_HOSTNAME:
            /*
             * TODO(OpenSSL1.2)
             * This API is only used for a client to set what SNI it will
             * request from the server, but we currently allow it to be used on
             * servers as well, which is a programming error.  Currently we just
             * clear the field in SSL_do_handshake() for server SSLs, but when
             * we can make ABI-breaking changes, we may want to make use of this
             * API an error on server SSLs.
             */
...
long ssl3_callback_ctrl(SSL* s, int cmd, void (*fp)(void))
{
    ...
    switch (cmd)
    {
        ...
        case SSL_CTRL_SET_TLSEXT_DEBUG_CB:
            s->ext.debug_cb =
                (void (*)(SSL*, int, int, const unsigned char*, int, void*))fp;
            break;
    ...
    switch (cmd)
    {
        ...
        case SSL_CTRL_SET_TMP_ECDH:
        {
            ...
            return tls1_set_groups(&ctx->ext.supportedgroups,
                                   &ctx->ext.supportedgroups_len, &nid, 1);
        }
#endif /* !OPENSSL_NO_EC */
        ...
        case SSL_CTRL_SET_GROUPS:
            return tls1_set_groups(&ctx->ext.supportedgroups,
                                   &ctx->ext.supportedgroups_len, parg, larg);

        case SSL_CTRL_SET_GROUPS_LIST:
            return tls1_set_groups_list(&ctx->ext.supportedgroups,
                                        &ctx->ext.supportedgroups_len, parg);
#endif
        case SSL_CTRL_SET_SIGALGS:
...
long ssl3_ctx_callback_ctrl(SSL_CTX* ctx, int cmd, void (*fp)(void))
{
    switch (cmd)
    {
        ...
        case SSL_CTRL_SET_TLSEXT_TICKET_KEY_CB:
            ctx->ext.ticket_key_cb =
                (int (*)(SSL*, unsigned char*, unsigned char*, EVP_CIPHER_CTX*,
                         HMAC_CTX*, int))fp;
            break;
                                     ...
                                     STACK_OF(SSL_CIPHER) * srvr)
{
    ...
    else if (s->options & SSL_OP_CIPHER_SERVER_PREFERENCE)
    {
        ...
        if (s->options & SSL_OP_PRIORITIZE_CHACHA &&
            sk_SSL_CIPHER_num(clnt) > 0)
        {
            ...
            if (c->algorithm_enc == SSL_CHACHA20POLY1305)
            {
                ...
                if (found)
                {
                    ...
                    if (prio_chacha != NULL)
                    {
                        /* Put all ChaCha20 at the top, starting with the one we
                         * just found */
    ...
    for (i = 0; i < sk_SSL_CIPHER_num(prio); i++)
    {
        ...
        if (!SSL_IS_DTLS(s) &&
            ...
            continue;
        if (SSL_IS_DTLS(s) && (DTLS_VERSION_LT(s->version, c->min_dtls) ||
        ...
        if (ii >= 0)
        {
            ...
            if (!ssl_security(s, SSL_SECOP_CIPHER_SHARED, c->strength_bits, 0,
                              (void*)c))
                continue;
...
int ssl3_get_req_cert_type(SSL* s, WPACKET* pkt)
{
     ...
     */
    if (s->version >= TLS1_VERSION && !(alg_a & SSL_aECDSA) &&
                               ...
                               int free_pms)
{
    ...
    if (alg_k & SSL_PSK)
    {
        ...
        if (!s->method->ssl3_enc->generate_master_secret(
                s, s->session->master_key, pskpms, pskpmslen,
    ...
    {
        if (!s->method->ssl3_enc->generate_master_secret(
                s, s->session->master_key, pms, pmslen,
                &s->session->master_key_length))
        {
            /* SSLfatal() already called */
            goto err;
        ...
        if (SSL_IS_TLS13(s))
        {
            ...
            if (!s->hit)
                rv = tls13_generate_secret(s, ssl_handshake_md(s), NULL, NULL,
                                           0, (unsigned char*)&s->early_secret);
            else
                rv = 1;
