...
FILE* open_wmemstream(wchar_t** bufloc, size_t* sizeloc)
{
    ...
    _IO_wstr_init_static(&new_f->fp._sf._sbf._f, buf, BUFSIZ / sizeof(wchar_t),
                         buf);
...
static int _IO_wmem_sync(FILE* fp)
{
...
static void _IO_wmem_finish(FILE* fp, int dummy)
{
    struct _IO_FILE_wmemstream* mp = (struct _IO_FILE_wmemstream*)fp;

    *mp->bufloc = (wchar_t*)realloc(
        fp->_wide_data->_IO_write_base,
        (fp->_wide_data->_IO_write_ptr - fp->_wide_data->_IO_write_base + 1) *
            sizeof(wchar_t));
    if (*mp->bufloc != NULL)
    {
        size_t len =
