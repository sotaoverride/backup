 ...
 *
 * Please note that the runtime library tends to be plugin-safe. That is, it
 * must be initialized by calling a global initialization function. However,
 * that function checks if the library is already initialized and, if so, does
...
static int iRefCount =
    0; /* our refcount - it MUST exist only once inside a process (not thread)
thus it is perfectly OK to use a static. MUST be initialized to 0! */

/* This is the default instance of the error logger. It simply writes the
 * message to stderr. It is expected that this is replaced by the runtime user
 * very early during startup (at least if the default is unsuitable). However,
 * we provide a default so that we can log errors during the intial phase, most
 * importantly during initialization. -- rgerhards. 2008-04-17
 */
 ...
 * ever poses a problem, we may use proper mutex calls - not considered needed
 * yet. If ppErrObj is provided, it receives a char pointer to the name of the
 * object that caused the problem (if one occured). The caller must never free
 * this pointer. If ppErrObj is NULL, no such information will be provided.
 * pObjIF is the pointer to the "obj" object interface, which may be used to
 * query any other rsyslog objects. rgerhards, 2008-04-16
 */
    ...
    if (iRefCount == 0)
    {
        ...
        if (ret != 0)
        {
            ...
            default_thr_sched_policy =
                0; /* should be default on all platforms */
        }
#if defined(_AIX)
        pthread_attr_setstacksize(&default_thread_attr, 4096 * 512);
#endif

        ret = pthread_attr_setschedpolicy(&default_thread_attr,
                                          default_thr_sched_policy);
        if (ret != 0)
        {
            rs_strerror_r(ret, errstr, sizeof(errstr));
        ...
        ret = pthread_attr_setschedparam(&default_thread_attr,
                                         &default_sched_param);
        if (ret != 0)
        {
            rs_strerror_r(ret, errstr, sizeof(errstr));
        ...
        ret = pthread_attr_setinheritsched(&default_thread_attr,
        ...
        if (ppErrObj != NULL)
            *ppErrObj = "obj";
        CHKiRet(objClassInit(NULL)); /* *THIS* *MUST* always be the first class
                                        initilizer being called! */
        CHKiRet(objGetObjInterface(
            pObjIF)); /* this provides the root pointer for all other queries */

        /* initialize core classes. We must be very careful with the order of
         * events. Some classes use others and if we do not initialize them in
         * the right order, we may end up with an invalid call. The most
         * important thing that can happen is that an error is detected and
         * needs to be logged, wich in turn requires a broader number of classes
         * to be available. The solution is that we take care in the order of
         * calls AND use a class immediately after it is initialized. And, of
         * course, we load those classes first that we use ourselfs... --
 ...
 * NOTE: this is NOT thread safe and must not be called concurrently. If that
 * ever poses a problem, we may use proper mutex calls - not considered needed
 * yet. This function must be provided with the caller's obj object pointer.
 * This is automatically deinitialized by the runtime system. rgerhards,
 * 2008-04-16
 */
rsRetVal rsrtExit(void)
{
    ...
    if (iRefCount == 1)
    {
        ...
        objClassExit(); /* *THIS* *MUST/SHOULD?* always be the first class
                initilizer being called (except debug)! */
    }

    --iRefCount;
    /* TODO we must deinit this pointer! pObjIF = NULL; / * no longer exists for
     * this caller */
