 ...
 * \date   09.03.2009
 *
 * This header contains a wrapper class around a logging record that allows to
 * compose the record message with a streaming expression.
 */
...
struct enable_record_ostream_generic_operator<basic_record_ostream<CharT>, T,
                                              false, R>
 ...
 *
 * This class allows to compose the logging record message by streaming
 * operations. It aggregates the log record and provides the standard output
 * stream interface.
 */
template <typename CharT>
class basic_record_ostream : public basic_formatting_ostream<CharT>
{
  ...
  public:
    ...
    {
        BOOST_ASSERT_MSG(!!rec, "Boost.Log: basic_record_ostream should only "
                                "be attached to a valid record");
        m_record = &rec;
        init_stream();
    }
     ...
     *
     * \return \c true, if stream is valid and ready for formatting, \c false,
     * if the stream is not valid. The latter also applies to the case when the
     * stream is not attached to a log record.
     */
     ...
     *
     * \return \c false, if stream is valid and ready for formatting, \c true,
     * if the stream is not valid. The latter also applies to the case when the
     * stream is not attached to a log record.
     */
    ...
    /*!
     * Flushes internal buffers to complete all pending formatting operations
     * and returns the aggregated log record
     *
    ...
    /*!
     * If the stream is attached to a log record, flushes internal buffers to
     * complete all pending formatting operations. Then reattaches the stream to
     * another log record.
     *
    ...
    //! The function resets the stream into a detached (default initialized)
    //! state
...
typedef basic_record_ostream<char>
    record_ostream; //!< Convenience typedef for narrow-character logging
#endif
#ifdef BOOST_LOG_USE_WCHAR_T
typedef basic_record_ostream<wchar_t>
    wrecord_ostream; //!< Convenience typedef for wide-character logging
#endif
...
// perfect forwarding for the right hand argument because in ths case the
...
inline typename boost::log::aux::enable_record_ostream_generic_operator<
...
struct stream_provider
{
    ...
    BOOST_LOG_API static void
        release_compound(stream_compound* compound) BOOST_NOEXCEPT;
...
class record_pump
{
  ...
  private:
    ...
    class auto_release
    {
      ...
      public:
        explicit auto_release(stream_compound* p) BOOST_NOEXCEPT
            : m_pCompound(p)
        {
        }
        ~auto_release() BOOST_NOEXCEPT
        {
            stream_provider_type::release_compound(m_pCompound);
        }
    };
  ...
  public:
    ...
    ~record_pump() BOOST_NOEXCEPT_IF(false)
    {
        if (m_pLogger)
        {
            ...
            // Only push the record if no exception has been thrown in the
            // streaming expression (if possible)
            if (m_ExceptionCount >= unhandled_exception_count())
                m_pLogger->push_record(
                    boost::move(m_pStreamCompound->stream.get_record()));
        }
    }
...
template <typename LoggerT>
BOOST_FORCEINLINE record_pump<LoggerT> make_record_pump(LoggerT& lg,
                                                        record& rec)
{
...
#define BOOST_LOG_STREAM(logger)                                               \
    BOOST_LOG_STREAM_INTERNAL(                                                 \
        logger, BOOST_LOG_UNIQUE_IDENTIFIER_NAME(_boost_log_record_))

//! The macro writes a record to the log and allows to pass additional named
