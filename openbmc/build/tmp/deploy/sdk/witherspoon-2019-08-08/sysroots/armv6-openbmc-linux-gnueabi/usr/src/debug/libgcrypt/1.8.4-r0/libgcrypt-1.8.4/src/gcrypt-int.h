...
gpg_err_code_t _gcry_cipher_open(gcry_cipher_hd_t* handle, int algo, int mode,
                                 unsigned int flags);
...
gpg_err_code_t _gcry_cipher_encrypt(gcry_cipher_hd_t h, void* out,
                                    size_t outsize, const void* in,
                                    size_t inlen);
gpg_err_code_t _gcry_cipher_decrypt(gcry_cipher_hd_t h, void* out,
                                    size_t outsize, const void* in,
                                    size_t inlen);
...
gpg_err_code_t _gcry_pk_encrypt(gcry_sexp_t* result, gcry_sexp_t data,
                                gcry_sexp_t pkey);
gpg_err_code_t _gcry_pk_decrypt(gcry_sexp_t* result, gcry_sexp_t data,
                                gcry_sexp_t skey);
gpg_err_code_t _gcry_pk_sign(gcry_sexp_t* result, gcry_sexp_t data,
                             gcry_sexp_t skey);
gpg_err_code_t _gcry_pk_verify(gcry_sexp_t sigval, gcry_sexp_t data,
                               gcry_sexp_t pkey);
...
gpg_err_code_t _gcry_pk_algo_info(int algo, int what, void* buffer,
                                  size_t* nbytes);
...
gpg_err_code_t _gcry_pubkey_get_sexp(gcry_sexp_t* r_sexp, int mode,
                                     gcry_ctx_t ctx);
...
gpg_err_code_t _gcry_md_ctl(gcry_md_hd_t hd, int cmd, void* buffer,
                            size_t buflen);
...
gpg_err_code_t _gcry_md_extract(gcry_md_hd_t hd, int algo, void* buffer,
                                size_t length);
void _gcry_md_hash_buffer(int algo, void* digest, const void* buffer,
                          size_t length);
gpg_err_code_t _gcry_md_hash_buffers(int algo, unsigned int flags, void* digest,
...
gpg_err_code_t _gcry_md_setkey(gcry_md_hd_t hd, const void* key, size_t keylen);
...
#define _gcry_md_final(a) _gcry_md_ctl((a), GCRYCTL_FINALIZE, NULL, 0)
...
gpg_err_code_t _gcry_mac_setkey(gcry_mac_hd_t hd, const void* key,
                                size_t keylen);
gpg_err_code_t _gcry_mac_setiv(gcry_mac_hd_t hd, const void* iv, size_t ivlen);
...
gpg_err_code_t _gcry_prime_group_generator(gcry_mpi_t* r_g, gcry_mpi_t prime,
                                           gcry_mpi_t* factors,
                                           gcry_mpi_t start_g);
...
static inline const char* _gcry_strerror(gcry_error_t err)
{
...
static inline const char* _gcry_strsource(gcry_error_t err)
{
...
/* Retrieve the error code for the system error ERR.  This returns
   ...
   this).  */
static inline gcry_err_code_t _gcry_err_code_from_errno(int err)
{
...
static inline int _gcry_err_code_to_errno(gcry_err_code_t code)
{
...
/* Return an error value with the error source SOURCE and the system
   error ERR.  */
static inline gcry_error_t _gcry_err_make_from_errno(gpg_err_source_t source,
                                                     int err)
{
...
static inline gcry_error_t _gcry_error_from_errno(int err)
{
    return gpg_error(gpg_err_code_from_errno(err));
}

gpg_err_code_t _gcry_sexp_new(gcry_sexp_t* retsexp, const void* buffer,
                              size_t length, int autodetect);
gpg_err_code_t _gcry_sexp_create(gcry_sexp_t* retsexp, void* buffer,
                                 size_t length, int autodetect,
                                 void (*freefnc)(void*));
...
gcry_sexp_t _gcry_sexp_find_token(gcry_sexp_t list, const char* tok,
                                  size_t toklen);
...
gpg_err_code_t _gcry_sexp_extract_param(gcry_sexp_t sexp, const char* path,
                                        const char* list, ...)
...
void _gcry_mpi_div(gcry_mpi_t q, gcry_mpi_t r, gcry_mpi_t dividend,
                   gcry_mpi_t divisor, int round);
void _gcry_mpi_mod(gcry_mpi_t r, gcry_mpi_t dividend, gcry_mpi_t divisor);
void _gcry_mpi_powm(gcry_mpi_t w, const gcry_mpi_t b, const gcry_mpi_t e,
                    const gcry_mpi_t m);
...
void _gcry_mpi_point_snatch_get(gcry_mpi_t x, gcry_mpi_t y, gcry_mpi_t z,
                                gcry_mpi_point_t point);
gcry_mpi_point_t _gcry_mpi_point_set(gcry_mpi_point_t point, gcry_mpi_t x,
...
gcry_mpi_point_t _gcry_mpi_ec_get_point(const char* name, gcry_ctx_t ctx,
                                        int copy);
...
void _gcry_mpi_ec_add(gcry_mpi_point_t w, gcry_mpi_point_t u,
                      gcry_mpi_point_t v, mpi_ec_t ctx);
void _gcry_mpi_ec_sub(gcry_mpi_point_t w, gcry_mpi_point_t u,
                      gcry_mpi_point_t v, mpi_ec_t ctx);
...
gcry_mpi_t _gcry_mpi_set_opaque_copy(gcry_mpi_t a, const void* p,
                                     unsigned int nbits);
