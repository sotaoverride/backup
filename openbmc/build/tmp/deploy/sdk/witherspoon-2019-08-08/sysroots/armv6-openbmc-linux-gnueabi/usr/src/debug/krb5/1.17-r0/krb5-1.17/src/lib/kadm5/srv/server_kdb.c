                                ...
                                int from_keyboard)
{
    ...
    master_princ = NULL;
    if ((ret =
             krb5_db_setup_mkey_name(handle->context, handle->params.mkey_name,
     ...
     * valid mkey.
     */
    ret = krb5_db_fetch_mkey(
        handle->context, master_princ, master_keyblock.enctype, from_kbd,
        FALSE /* only prompt once */, handle->params.stash_file,
...
static krb5_error_code create_hist(kadm5_server_handle_t handle)
{
    kadm5_ret_t ret;
    ...
    return kadm5_randkey_principal_3(handle, ent.principal, 0, 1, ks, NULL,
                                     NULL);
}
...
static krb5_error_code kdb_iter_func(krb5_pointer data, krb5_db_entry* kdb)
{
