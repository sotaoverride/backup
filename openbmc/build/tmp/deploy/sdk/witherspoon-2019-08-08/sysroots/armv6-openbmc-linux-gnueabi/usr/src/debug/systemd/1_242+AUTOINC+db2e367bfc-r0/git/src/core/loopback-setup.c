...
static int generic_handler(sd_netlink* rtnl, sd_netlink_message* m,
                           void* userdata)
{
...
static int start_loopback(sd_netlink* rtnl, struct state* s)
{
    ...
    r = sd_netlink_call_async(rtnl, NULL, req, generic_handler, NULL, s,
...
static int add_ipv4_address(sd_netlink* rtnl, struct state* s)
{
    ...
    r = sd_rtnl_message_new_addr(rtnl, &req, RTM_NEWADDR, LOOPBACK_IFINDEX,
                                 AF_INET);
    if (r < 0)
        return r;
    ...
    r = sd_netlink_call_async(rtnl, NULL, req, generic_handler, NULL, s,
                              USEC_INFINITY, "systemd-loopback-ipv4");
    if (r < 0)
        return r;
...
static int add_ipv6_address(sd_netlink* rtnl, struct state* s)
{
    ...
    r = sd_netlink_call_async(rtnl, NULL, req, generic_handler, NULL, s,
                              USEC_INFINITY, "systemd-loopback-ipv6");
    if (r < 0)
        return r;
...
int loopback_setup(void)
{
        ...
        state_4 =
            {
                ...
                .success_message = "Successfully added address 127.0.0.1 to "
                                   "loopback interface",
            },
        state_6 =
            {
    ...
    /* Note that we add the IP addresses here explicitly even though the kernel
     * does that too implicitly when setting up the loopback device. The reason
     * we do this here a second time (and possibly race against the kernel) is
     * that we want to synchronously wait until the IP addresses are set up
     * correctly, see
     *
    ...
    if (r < 0)
        return log_error_errno(
            r, "Failed to enqueue IPv4 loopback address add request: %m");

    r = add_ipv6_address(rtnl, &state_6);
    if (r < 0)
        return log_error_errno(
            r, "Failed to enqueue IPv6 loopback address add request: %m");

    r = start_loopback(rtnl, &state_up);
    if (r < 0)
        return log_error_errno(
            r, "Failed to enqueue loopback interface start request: %m");
    ...
    /* Note that we don't really care whether the addresses could be added or
     * not */
    if (state_up.rcode != 0)
    {
