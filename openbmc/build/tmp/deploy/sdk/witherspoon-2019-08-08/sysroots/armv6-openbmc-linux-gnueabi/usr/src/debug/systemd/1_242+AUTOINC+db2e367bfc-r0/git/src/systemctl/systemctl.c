...
static int get_state_one_unit(sd_bus* bus, const char* name,
...
static int acquire_bus(BusFocus focus, sd_bus** ret)
{
    ...
    if (!busses[focus])
    {
        ...
        if (focus == BUS_MANAGER)
            r = bus_connect_transport_systemd(arg_transport, arg_host, user,
                                              &busses[focus]);
        else
            r = bus_connect_transport(arg_transport, arg_host, user,
                                      &busses[focus]);
        if (r < 0)
            return log_error_errno(r, "Failed to connect to bus: %m");

        (void)sd_bus_set_allow_interactive_authorization(busses[focus],
                                                         arg_ask_password);
    }
...
{
    return arg_all * OUTPUT_SHOW_ALL |
...
{
    unsigned circle_len = 0, id_len, max_id_len, load_len, active_len, sub_len,
             job_len, desc_len, max_desc_len;
    const UnitInfo* u;
    unsigned n_shown = 0;
    int job_count = 0;
    ...
    for (u = unit_infos; u < unit_infos + c; u++)
    {
        max_id_len =
            MAX(max_id_len,
                strlen(u->id) + (u->machine ? strlen(u->machine) + 1 : 0));
        ...
        if (!arg_no_legend && (streq(u->active_state, "failed") ||
    ...
    if (!arg_full && original_stdout_is_tty)
    {
        ...
        id_len = MIN(max_id_len,
                     25u); /* as much as it needs, but at most 25 for now */
        basic_len = circle_len + 1 + id_len + 1 + load_len + 1 + active_len +
                    1 + sub_len + 1;
        ...
        if (basic_len < (unsigned)columns())
        {
            ...
            /* Of the remainder give as much as the ID needs to the ID, and give
             * the rest to the description but not more than it needs. */
            if (extra_len > 0)
            {
    ...
    for (u = unit_infos; u < unit_infos + c; u++)
    {
        ...
        if (!n_shown && !arg_no_legend)
        {
            ...
            printf("%s%-*s %-*s %-*s %-*s ", ansi_underline(), id_len, "UNIT",
                   load_len, "LOAD", active_len, "ACTIVE", sub_len, "SUB");

            if (job_count)
                printf("%-*s ", job_len, "JOB");

            printf("%-*.*s%s\n", desc_len, full ? -1 : (int)desc_len,
                   "DESCRIPTION", ansi_normal());
        }
        ...
        if (STR_IN_SET(u->load_state, "error", "not-found", "bad-setting",
                       "masked") &&
            !arg_plain)
        {
            ...
            circle = true;
            on_loaded = underline ? ansi_highlight_red_underline()
                                  : ansi_highlight_red();
        ...
        else if (streq(u->active_state, "failed") && !arg_plain)
        {
            ...
            off_circle = ansi_normal();
            circle = true;
            on_active = underline ? ansi_highlight_red_underline()
                                  : ansi_highlight_red();
        ...
        if (circle_len > 0)
            printf("%s%s%s ", on_circle,
                   circle ? special_glyph(SPECIAL_GLYPH_BLACK_CIRCLE) : " ",
                   off_circle);

        printf("%s%s%-*s%s %s%-*s%s %s%-*s %-*s%s %-*s", on_underline,
               on_active, id_len, id, off_active, on_loaded, load_len,
               u->load_state, off_loaded, on_active, active_len,
               u->active_state, sub_len, u->sub_state, off_active,
               job_count ? job_len + 1 : 0, u->job_id ? u->job_type : "");

        printf("%-*.*s%s\n", desc_len, full ? -1 : (int)desc_len,
               u->description, off_underline);
    }
    ...
    if (!arg_no_legend)
    {
        ...
        if (arg_all)
            ...
            printf("%s%u loaded units listed.%s Pass --all to see loaded but "
                   "inactive units, too.\n"
                   "To show all installed unit files use 'systemctl "
                   "list-unit-files'.\n",
                   on, n_shown, off);
...
static int get_unit_list(sd_bus* bus, const char* machine, char** patterns,
                         UnitInfo** unit_infos, int c, sd_bus_message** _reply)
{
    ...
    r = sd_bus_message_new_method_call(
        bus, &m, "org.freedesktop.systemd1", "/org/freedesktop/systemd1",
                  ...
                  sd_bus_error_has_name(&error, SD_BUS_ERROR_ACCESS_DENIED)))
    {
        ...
        fallback = true;
        log_debug_errno(r,
                        "Failed to list units: %s Falling back to "
    ...
    if (r < 0)
        return log_error_errno(r, "Failed to list units: %s",
                               bus_error_message(&error, r));
...
static int get_unit_list_recursive(sd_bus* bus, char** patterns,
                                   UnitInfo** _unit_infos, Set** _replies,
                                   char*** _machines)
{
    ...
    if (arg_recursive)
    {
        ...
        STRV_FOREACH(i, machines)
        {
            ...
            {
                log_warning_errno(
                    r, "Failed to connect to container %s, ignoring: %m", *i);
                continue;
            }
...
static int get_triggered_units(sd_bus* bus, const char* path, char*** ret)
{
    ...
    r = sd_bus_get_property_strv(bus, "org.freedesktop.systemd1", path,
                                 ...
                                 &error, ret);
    if (r < 0)
        return log_error_errno(r, "Failed to determine triggers: %s",
                               bus_error_message(&error, r));

    return 0;
}

static int get_listening(sd_bus* bus, const char* unit_path, char*** listening)
{
    ...
    r = sd_bus_get_property(bus, "org.freedesktop.systemd1", unit_path,
                            "org.freedesktop.systemd1.Socket", "Listen", &error,
                            &reply, "a(ss)");
    if (r < 0)
        return log_error_errno(r, "Failed to get list of listening sockets: %s",
                               bus_error_message(&error, r));
...
static int socket_info_compare(const struct socket_info* a,
                               const struct socket_info* b)
{
    int r;
...
static int output_sockets_list(struct socket_info* socket_infos, unsigned cs)
{
    ...
    if (cs)
    {
        ...
        for (s = socket_infos; s < socket_infos + cs; s++)
        {
            ...
            if (arg_show_types)
                printf("%-*s %-*s %-*s", pathlen, path, typelen, s->type,
                       socklen, s->id);
            else
...
static int list_sockets(int argc, char* argv[], void* userdata)
{
    ...
    n = get_unit_list_recursive(bus, strv_skip(argv, 1), &unit_infos, &replies,
                                &machines);
    if (n < 0)
        return n;
...
static int get_next_elapse(sd_bus* bus, const char* path, dual_timestamp* next)
{
    ...
    r = sd_bus_get_property_trivial(
        bus, "org.freedesktop.systemd1", path, "org.freedesktop.systemd1.Timer",
        "NextElapseUSecMonotonic", &error, 't', &t.monotonic);
    if (r < 0)
        return log_error_errno(r, "Failed to get next elapse time: %s",
                               bus_error_message(&error, r));

    r = sd_bus_get_property_trivial(
        bus, "org.freedesktop.systemd1", path, "org.freedesktop.systemd1.Timer",
        "NextElapseUSecRealtime", &error, 't', &t.realtime);
    if (r < 0)
        return log_error_errno(r, "Failed to get next elapse time: %s",
                               bus_error_message(&error, r));
...
static int get_last_trigger(sd_bus* bus, const char* path, usec_t* last)
{
    ...
    r = sd_bus_get_property_trivial(bus, "org.freedesktop.systemd1", path,
                                    ...
                                    "LastTriggerUSec", &error, 't', last);
    if (r < 0)
        return log_error_errno(r, "Failed to get last trigger time: %s",
                               bus_error_message(&error, r));
...
static int timer_info_compare(const struct timer_info* a,
                              const struct timer_info* b)
{
    int r;
...
static int output_timers_list(struct timer_info* timer_infos, unsigned n)
{
    ...
    {
        if (!arg_no_legend)
            printf("%-*s %-*s %-*s %-*s %-*s %s\n", nextlen, "NEXT", leftlen,
                   "LEFT", lastlen, "LAST", passedlen, "PASSED", unitlen,
                   "UNIT", "ACTIVATES");
        ...
        for (t = timer_infos; t < timer_infos + n; t++)
        {
            ...
            printf("%-*s %-*s %-*s %-*s %-*s", nextlen, tstamp1, leftlen, trel1,
                   lastlen, tstamp2, passedlen, trel2, unitlen, unit);
...
static bool output_show_unit_file(const UnitFileList* u, char** states,
                                  char** patterns)
{
    assert(u);
...
static void output_unit_file_list(const UnitFileList* units, unsigned c)
{
    ...
    for (u = units; u < units + c; u++)
    {
        ...
        state_cols =
            MAX(state_cols, strlen(unit_file_state_to_string(u->state)));
    }
    ...
    if (!arg_no_legend && c > 0)
        printf("%s%-*s %-*s%s\n", ansi_underline(), id_cols, "UNIT FILE",
               state_cols, "STATE", ansi_normal());
    ...
    for (u = units; u < units + c; u++)
    {
        ...
        if (IN_SET(u->state, UNIT_FILE_MASKED, UNIT_FILE_MASKED_RUNTIME,
            ...
            on_color = underline ? ansi_highlight_red_underline()
                                 : ansi_highlight_red();
        else if (u->state == UNIT_FILE_ENABLED)
            on_color = underline ? ansi_highlight_green_underline()
        ...
        printf("%s%-*s %s%-*s%s\n", strempty(on_underline), id_cols, e ? e : id,
               strempty(on_color), state_cols,
               unit_file_state_to_string(u->state), strempty(off));
    }
...
static int list_unit_files(int argc, char* argv[], void* userdata)
{
    ...
    if (install_client_side())
    {
        ...
        r = unit_file_get_list(arg_scope, arg_root, h, arg_states,
                               strv_skip(argv, 1));
        if (r < 0)
        {
    ...
    else
    {
        ...
        if (r < 0 && sd_bus_error_has_name(&error, SD_BUS_ERROR_UNKNOWN_METHOD))
        {
            ...
            fallback = true;
            log_debug_errno(r,
                            "Failed to list unit files: %s Falling back to "
                            "ListUnitsFiles method.",
                            bus_error_message(&error, r));
            m = sd_bus_message_unref(m);
            sd_bus_error_free(&error);
        ...
        if (r < 0)
            return log_error_errno(r, "Failed to list unit files: %s",
                                   bus_error_message(&error, r));
        ...
        while ((r = sd_bus_message_read(reply, "(ss)", &path, &state)) > 0)
        {
            ...
            units[c] =
                (struct UnitFileList){path, unit_file_state_from_string(state)};

            if (output_show_unit_file(&units[c], fallback ? arg_states : NULL,
...
static int list_dependencies_print(const char* name, int level,
                                   unsigned branches, bool last)
{
    ...
    if (!arg_plain)
    {
        ...
        for (i = level - 1; i >= 0; i--)
        {
            ...
            printf("%s", special_glyph(branches & (1 << i)
...
static int list_dependencies_get_dependencies(sd_bus* bus, const char* name,
                                              char*** deps)
{
    ...
    static const struct bus_properties_map map[_DEPENDENCY_MAX][6] = {
        [DEPENDENCY_FORWARD] = {{"Requires", "as", NULL,
                                 offsetof(struct DependencyStatusInfo, dep[0])},
                                {"Requisite", "as", NULL,
                                 offsetof(struct DependencyStatusInfo, dep[1])},
                                {"Wants", "as", NULL,
                                 offsetof(struct DependencyStatusInfo, dep[2])},
                                {"ConsistsOf", "as", NULL,
                                 offsetof(struct DependencyStatusInfo, dep[3])},
                                {"BindsTo", "as", NULL,
                                 offsetof(struct DependencyStatusInfo, dep[4])},
                                {}},
        [DEPENDENCY_REVERSE] = {{"RequiredBy", "as", NULL,
                                 offsetof(struct DependencyStatusInfo, dep[0])},
                                {"RequisiteOf", "as", NULL,
                                 offsetof(struct DependencyStatusInfo, dep[1])},
                                {"WantedBy", "as", NULL,
                                ...
                                {"BoundBy", "as", NULL,
                                 offsetof(struct DependencyStatusInfo, dep[4])},
                                {}},
        [DEPENDENCY_AFTER] = {{"After", "as", NULL,
                               offsetof(struct DependencyStatusInfo, dep[0])},
                              {}},
        [DEPENDENCY_BEFORE] = {{"Before", "as", NULL,
                                offsetof(struct DependencyStatusInfo, dep[0])},
                               {}},
    };
    ...
    r = bus_map_all_properties(bus, "org.freedesktop.systemd1", path,
                               map[arg_dependency], BUS_MAP_STRDUP, &error,
                               NULL, &info);
    if (r < 0)
        return log_error_errno(r, "Failed to get properties of %s: %s", name,
                               bus_error_message(&error, r));
...
static int list_dependencies_one(sd_bus* bus, const char* name, int level,
                                 char*** units, unsigned branches)
{
    ...
    STRV_FOREACH(c, deps)
    {
        if (strv_contains(*units, *c))
        {
            if (!arg_plain)
            {
                ...
                r = list_dependencies_print(
                    "...", level + 1, (branches << 1) | (c[1] == NULL ? 0 : 1),
                    1);
                if (r < 0)
                    return r;
            }
        ...
        else
        {
            ...
            printf("%s%s%s ", on, special_glyph(SPECIAL_GLYPH_BLACK_CIRCLE),
                   ansi_normal());
        }
...
static int list_dependencies(int argc, char* argv[], void* userdata)
{
    ...
    if (argv[1])
    {
        r = unit_name_mangle(argv[1], arg_quiet ? 0 : UNIT_NAME_MANGLE_WARN,
                             &unit);
        if (r < 0)
...
static int compare_machine_info(const struct machine_info* a,
                                const struct machine_info* b)
{
    int r;
...
static int get_machine_properties(sd_bus* bus, struct machine_info* mi)
{
    ...
    r = bus_map_all_properties(
        ...
        machine_info_property_map, BUS_MAP_STRDUP, NULL, NULL, mi);
    if (r < 0)
        return r;
...
static int get_machine_list(sd_bus* bus, struct machine_info** _machine_infos,
                            char** patterns)
{
...
static void output_machines_list(struct machine_info* machine_infos, unsigned n)
{
    ...
    unsigned circle_len = 0, namelen = STRLEN("NAME"),
             statelen = STRLEN("STATE"), failedlen = STRLEN("FAILED"),
             jobslen = STRLEN("JOBS");
    ...
    if (!arg_no_legend)
    {
        ...
        printf("%-*s %-*s %-*s %-*s\n", namelen, "NAME", statelen, "STATE",
               failedlen, "FAILED", jobslen, "JOBS");
    }
    ...
    for (m = machine_infos; m < machine_infos + n; m++)
    {
        ...
        if (m->is_host)
            printf("%-*s (host) %s%-*s%s %s%*" PRIu32 "%s %*" PRIu32 "\n",
                   (int)(namelen - (STRLEN(" (host)"))), strna(m->name),
                   on_state, statelen, strna(m->state), off_state, on_failed,
                   failedlen, m->n_failed_units, off_failed, jobslen,
                   m->n_jobs);
        else
            printf("%-*s %s%-*s%s %s%*" PRIu32 "%s %*" PRIu32 "\n", namelen,
                   strna(m->name), on_state, statelen, strna(m->state),
                   off_state, on_failed, failedlen, m->n_failed_units,
                   off_failed, jobslen, m->n_jobs);
    }
...
static int get_default(int argc, char* argv[], void* userdata)
{
    ...
    else
    {
        ...
        r = sd_bus_call_method(bus, "org.freedesktop.systemd1",
                               ...
                               "GetDefaultTarget", &error, &reply, NULL);
        if (r < 0)
...
static int set_default(int argc, char* argv[], void* userdata)
{
    ...
    r = unit_name_mangle_with_suffix(
        argv[1], arg_quiet ? 0 : UNIT_NAME_MANGLE_WARN, ".target", &unit);
    if (r < 0)
        return log_error_errno(r, "Failed to mangle unit name: %m");

    if (install_client_side())
    {
        r = unit_file_set_default(arg_scope, UNIT_FILE_FORCE, arg_root, unit,
                                  &changes, &n_changes);
    ...
    else
    {
        ...
        r = sd_bus_call_method(
            bus, "org.freedesktop.systemd1", "/org/freedesktop/systemd1",
            "org.freedesktop.systemd1.Manager", "SetDefaultTarget", &error,
            &reply, "sb", unit, 1);
        if (r < 0)
            return log_error_errno(r, "Failed to set default target: %s",
                                   bus_error_message(&error, r));
...
static int output_waiting_jobs(sd_bus* bus, uint32_t id, const char* method,
                               const char* prefix)
{
    _cleanup_(sd_bus_error_free) sd_bus_error error = SD_BUS_ERROR_NULL;
    _cleanup_(sd_bus_message_unrefp) sd_bus_message* reply = NULL;
    ...
    r = sd_bus_call_method(
        bus, "org.freedesktop.systemd1", "/org/freedesktop/systemd1",
        "org.freedesktop.systemd1.Manager", method, &error, &reply, "u", id);
    if (r < 0)
        return log_debug_errno(r, "Failed to get waiting jobs for job %" PRIu32,
                               id);
    ...
    while ((r = sd_bus_message_read(reply, "(usssoo)", &other_id, &name, &type,
                                    NULL, NULL, NULL)) > 0)
...
static void output_jobs_list(sd_bus* bus, const struct job_info* jobs,
                             unsigned n, bool skipped)
{
    ...
    for (j = jobs; j < jobs + n; j++)
    {
        ...
        printf("%*u %s%-*s%s %-*s %s%-*s%s\n", id_len, j->id, on, unit_len,
               e ? e : j->name, off, type_len, j->type, on, state_len, j->state,
               off);
...
static int list_jobs(int argc, char* argv[], void* userdata)
{
    ...
    r = sd_bus_call_method(
        bus, "org.freedesktop.systemd1", "/org/freedesktop/systemd1",
        "org.freedesktop.systemd1.Manager", "ListJobs", &error, &reply, NULL);
    if (r < 0)
        return log_error_errno(r, "Failed to list jobs: %s",
                               bus_error_message(&error, r));
...
static int cancel_job(int argc, char* argv[], void* userdata)
{
    ...
    STRV_FOREACH(name, strv_skip(argv, 1))
    {
        ...
        if (q < 0)
            return log_error_errno(q, "Failed to parse job id \"%s\": %m",
                                   *name);

        q = sd_bus_call_method(bus, "org.freedesktop.systemd1",
                               "/org/freedesktop/systemd1",
                               "org.freedesktop.systemd1.Manager", "CancelJob",
                               &error, NULL, "u", id);
        if (q < 0)
        {
            log_error_errno(q, "Failed to cancel job %" PRIu32 ": %s", id,
...
static int need_daemon_reload(sd_bus* bus, const char* unit)
{
    ...
    r = sd_bus_call_method(
        bus, "org.freedesktop.systemd1", "/org/freedesktop/systemd1",
        "org.freedesktop.systemd1.Manager", "GetUnit", NULL, &reply, "s", unit);
    if (r < 0)
        return r;
...
static void warn_unit_file_changed(const char* name)
{
    ...
    log_warning(
        "%sWarning:%s The unit file, source configuration file or drop-ins of "
        "%s changed on disk. Run 'systemctl%s daemon-reload' to reload units.",
        ansi_highlight_red(), ansi_normal(), name,
...
static int unit_find_template_path(const char* unit_name, LookupPaths* lp,
...
static int unit_find_paths(sd_bus* bus, const char* unit_name, LookupPaths* lp,
                           bool force_client_side, char** ret_fragment_path,
                           char*** ret_dropin_paths)
{
    ...
    /**
     * Finds where the unit is defined on disk. Returns 0 if the unit is not
     * found. Returns 1 if it is found, and sets:
     * - the path to the unit in *ret_frament_path, if it exists on disk,
     * - and a strv of existing drop-ins in *ret_dropin_paths, if the arg is not
     * NULL and any dropins were found.
     *
     * Returns -ERFKILL if the unit is masked, and -EKEYREJECTED if the unit
     * file could not be loaded for some reason (the latter only applies if we
     * are going through the service manager)
     */
    ...
    /* Go via the bus to acquire the path, unless we are explicitly told not to,
     * or when the unit name is a template */
    if (!force_client_side && !install_client_side() &&
        !unit_name_is_valid(unit_name, UNIT_NAME_TEMPLATE))
    {
        ...
        r = sd_bus_get_property_string(bus, "org.freedesktop.systemd1", unit,
                                       "org.freedesktop.systemd1.Unit",
                                       "LoadState", &error, &load_state);
        if (r < 0)
            return log_error_errno(r, "Failed to get LoadState: %s",
                                   bus_error_message(&error, r));
        ...
        if (r < 0)
            return log_error_errno(r, "Failed to get FragmentPath: %s",
                                   bus_error_message(&error, r));

        if (ret_dropin_paths)
        {
            r = sd_bus_get_property_strv(bus, "org.freedesktop.systemd1", unit,
                                         "org.freedesktop.systemd1.Unit",
                                         "DropInPaths", &error, &dropins);
            if (r < 0)
                return log_error_errno(r, "Failed to get DropInPaths: %s",
                                       bus_error_message(&error, r));
        }
    }
    else
    {
        ...
        if (r > 0)
        {
            ...
            /* We found the unit file. If we followed symlinks, this name might
             * be different then the unit_name with started with. Look for
             * dropins matching that "final" name. */
            r = set_put(names, basename(path));
        }
        else if (!template)
            /* No unit file, let's look for dropins matching the original name.
             * systemd has fairly complicated rules (based on unit type and
             * provenience), which units are allowed not to have the main unit
             * file. We err on the
             * side of including too many files, and always try to load dropins.
             */
            r = set_put(names, unit_name);
        else
            /* The cases where we allow a unit to exist without the main file
             * are
             * never valid for templates. Don't try to load dropins in this
             * case. */
            goto not_found;
                              ...
                              UnitActiveState* active_state)
{
    ...
    r = sd_bus_get_property_string(bus, "org.freedesktop.systemd1", path,
                                   "org.freedesktop.systemd1.Unit",
                                   "ActiveState", &error, &buf);
    if (r < 0)
        return log_error_errno(r, "Failed to retrieve unit state: %s",
                               bus_error_message(&error, r));

    state = unit_active_state_from_string(buf);
    if (state < 0)
        return log_error_errno(SYNTHETIC_ERRNO(EINVAL),
                               "Invalid unit state '%s' for: %s", buf, name);
...
static int check_triggering_units(sd_bus* bus, const char* name)
{
    ...
    r = sd_bus_get_property_strv(bus, "org.freedesktop.systemd1", path,
                                 ...
                                 &error, &triggered_by);
    if (r < 0)
        return log_error_errno(r, "Failed to get triggered by array of %s: %s",
                               n, bus_error_message(&error, r));
    ...
    STRV_FOREACH(i, triggered_by)
    {
        ...
        {
            log_warning(
                "Warning: Stopping %s, but it can still be activated by:", n);
...
static const struct
{
    ...
    const char* job_type; /* Job type when passing to the generic
    ...
    {"reload-or-try-restart", "ReloadOrTryRestartUnit",
     "reload-or-try-restart"}, /* legacy alias */
                                 ...
                                 sd_bus_error* error)
{
    ...
    /* Called whenever we get a PropertiesChanged signal. Checks if ActiveState
     * changed to inactive/failed.
     *
     * Signal parameters: (s interface, a{sv} changed_properties, as
    ...
    if (!streq(interface,
               "org.freedesktop.systemd1.Unit")) /* ActiveState is on the Unit
                                                    interface */
        return 0;
    ...
    for (;;)
    {
        ...
        else if (streq(s, "Job"))
        {
            ...
            /* There's still a job pending for this unit, let's ignore this for
             * now, and return right-away. */
            if (job_id != 0)
                return 0;
    ...
    {
        log_debug("%s became %s, dropping from --wait tracking", path,
                  active_state);
...
static int wait_context_watch(WaitContext* wait_context, sd_bus* bus,
                              const char* name)
{
    ...
    r = sd_bus_call_method(
        bus, "org.freedesktop.systemd1", "/org/freedesktop/systemd1",
        "org.freedesktop.systemd1.Manager", "RefUnit", &error, NULL, "s", name);
    if (r < 0)
        return log_error_errno(r, "Failed to add reference to unit %s: %s",
    ...
    if (r < 0)
        return log_error_errno(r, "Failed to add unit path %s to set: %m",
                               unit_path);

    r = sd_bus_match_signal_async(bus, &wait_context->match, NULL, unit_path,
                                  "org.freedesktop.DBus.Properties",
                                  "PropertiesChanged", on_properties_changed,
                                  NULL, wait_context);
    if (r < 0)
        return log_error_errno(
            r, "Failed to request match for PropertiesChanged signal: %m");
...
static int start_unit_one(
    ...
    const char* name, const char* mode, sd_bus_error* error, BusWaitForJobs* w,
    WaitContext* wait_context)
{
    ...
    {
        _cleanup_(sd_bus_error_free) sd_bus_error enqueue_error =
            SD_BUS_ERROR_NULL;

        /* Use the new, fancy EnqueueUnitJob() API if the user wants us to print
         * the transaction */
        r = sd_bus_call_method(
            bus, "org.freedesktop.systemd1", "/org/freedesktop/systemd1",
            "org.freedesktop.systemd1.Manager", "EnqueueUnitJob",
            &enqueue_error, &reply, "sss", name, job_type, mode);
        if (r < 0)
        {
            if (!sd_bus_error_has_name(&enqueue_error,
            ...
            /* Hmm, the API is not yet available. Let's use the classic API
             * instead (see below). */
            log_notice("--show-transaction not supported by this service "
                       "manager, proceeding without.");
        }
        else
        {
            const char *u, *jt;
            ...
            for (;;)
            {
                r = sd_bus_message_read(reply, "(uosos)", &id, NULL, &u, NULL,
                                        &jt);
                if (r < 0)
                    return bus_log_parse_error(r);
                if (r == 0)
                ...
                log_info("Enqueued auxiliary job %" PRIu32 " %s/%s.", id, u,
                         jt);
            }
...
fail:
    ...
    log_error_errno(r, "Failed to %s %s: %s", job_type, name,
                    bus_error_message(error, r));
...
static int expand_names(sd_bus* bus, char** names, const char* suffix,
                        char*** ret)
{
    ...
    {
        char* t;
        UnitNameMangle options =
            UNIT_NAME_MANGLE_GLOB | (arg_quiet ? 0 : UNIT_NAME_MANGLE_WARN);
...
static const struct
{
    ...
    [ACTION_HYBRID_SLEEP] = {SPECIAL_HYBRID_SLEEP_TARGET, "hybrid-sleep",
                             "replace-irreversibly"},
    [ACTION_SUSPEND_THEN_HIBERNATE] = {SPECIAL_SUSPEND_THEN_HIBERNATE_TARGET,
...
static int start_unit(int argc, char* argv[], void* userdata)
{
    ...
    _cleanup_free_ char** stopped_units =
        NULL; /* Do not use _cleanup_strv_free_ */
    ...
    if (arg_action == ACTION_SYSTEMCTL)
    {
        ...
        else
        {
            ...
            {
                /* A command in style of "systemctl start <unit1> <unit2> …",
                 * "sysemctl stop <unit1> <unit2> …" and so on */
    ...
    if (arg_wait)
    {
        r = sd_bus_call_method_async(
            bus, NULL, "org.freedesktop.systemd1", "/org/freedesktop/systemd1",
            "org.freedesktop.systemd1.Manager", "Subscribe", NULL, NULL, NULL);
        if (r < 0)
    ...
    STRV_FOREACH(name, names)
    {
        ...
        r = start_unit_one(bus, method, job_type, *name, mode, &error, w,
                           arg_wait ? &wait_context : NULL);
    ...
    if (ret == EXIT_SUCCESS && arg_wait &&
        !set_isempty(wait_context.unit_paths))
    {
...
static int logind_set_wall_message(void)
{
    ...
    log_debug("%s wall message \"%s\".", arg_dry_run ? "Would set" : "Setting",
              m);
    if (arg_dry_run)
        return 0;
    ...
    r = sd_bus_call_method(bus, "org.freedesktop.login1",
                           ...
                           &error, NULL, "sb", m, !arg_no_wall);

    if (r < 0)
        return log_warning_errno(r, "Failed to set wall message, ignoring: %s",
                                 bus_error_message(&error, r));
    return 0;
}
#endif

/* Ask systemd-logind, which might grant access to unprivileged users through
 * polkit */
static int logind_reboot(enum action a)
{
    ...
    static const struct
    {
        ...
        [ACTION_SUSPEND_THEN_HIBERNATE] = {"SuspendThenHibernate",
                                           "suspend system, hibernate later"},
    };
    ...
    r = sd_bus_call_method(bus, "org.freedesktop.login1",
                           "/org/freedesktop/login1",
                           "org.freedesktop.login1.Manager", actions[a].method,
                           &error, NULL, "b", arg_ask_password);
    if (r < 0)
        return log_error_errno(r, "Failed to %s via logind: %s",
...
static int logind_check_inhibitors(enum action a)
{
    ...
    r = sd_bus_call_method(
        ...
        "org.freedesktop.login1.Manager", "ListInhibitors", NULL, &reply, NULL);
    if (r < 0)
    ...
    while ((r = sd_bus_message_read(reply, "(ssssuu)", &what, &who, &why, &mode,
                                    &uid, &pid)) > 0)
    {
        ...
        if (!strv_contains(sv, IN_SET(a, ACTION_HALT, ACTION_POWEROFF,
                                      ACTION_REBOOT, ACTION_KEXEC)
                                   ? "shutdown"
                                   : "sleep"))
            continue;
    ...
    STRV_FOREACH(s, sessions)
    {
        _cleanup_free_ char *type = NULL, *tty = NULL, *seat = NULL,
                            *user = NULL, *service = NULL, *class = NULL;
    ...
    log_error("Please retry operation after closing inhibitors and logging out "
              "other users.\nAlternatively, ignore inhibitors and users with "
              "'systemctl %s -i'.",
              action_table[a].verb);
...
static int prepare_firmware_setup(void)
{
    ...
    r = sd_bus_call_method(bus, "org.freedesktop.login1",
                           ...
                           "SetRebootToFirmwareSetup", &error, NULL, "b", true);
    if (r < 0)
        return log_error_errno(
            r, "Cannot indicate to EFI to boot into setup mode: %s",
            bus_error_message(&error, r));
...
static int prepare_boot_loader_menu(void)
{
    ...
    r = sd_bus_call_method(
        bus, "org.freedesktop.login1", "/org/freedesktop/login1",
        "org.freedesktop.login1.Manager", "SetRebootToBootLoaderMenu", &error,
        NULL, "t", arg_boot_loader_menu);
    if (r < 0)
        return log_error_errno(r,
                               "Cannot indicate to boot loader to enter boot "
                               "loader entry menu: %s",
                               bus_error_message(&error, r));
...
static int prepare_boot_loader_entry(void)
{
    ...
    r = sd_bus_call_method(
        bus, "org.freedesktop.login1", "/org/freedesktop/login1",
        "org.freedesktop.login1.Manager", "SetRebootToBootLoaderEntry", &error,
        NULL, "s", arg_boot_loader_entry);
    if (r < 0)
        return log_error_errno(r, "Cannot set boot into loader entry '%s': %s",
...
static int load_kexec_kernel(void)
{
    ...
    if (!e->kernel)
        return log_error_errno(SYNTHETIC_ERRNO(EOPNOTSUPP),
                               "Boot entry does not refer to Linux kernel, "
                               "which is not supported currently.");
    if (strv_length(e->initrd) > 1)
        return log_error_errno(SYNTHETIC_ERRNO(EOPNOTSUPP),
                               "Boot entry specifies multiple initrds, which "
    ...
    log_full(arg_quiet ? LOG_DEBUG : LOG_INFO,
             ...
             arg_dry_run ? "Would run" : "Running", kernel, options,
             initrd ? " --initrd \"" : NULL, strempty(initrd),
             initrd ? "\"" : "");
    if (arg_dry_run)
        return 0;

    r = safe_fork("(kexec)",
                  FORK_WAIT | FORK_RESET_SIGNALS | FORK_DEATHSIG |
    ...
    if (r == 0)
    {
        const char* const args[] = {KEXEC,   "--load",
                                    kernel,  "--append",
                                    options, initrd ? "--initrd" : NULL,
                                    initrd,  NULL};
...
static int set_exit_code(uint8_t code)
{
    ...
    r = sd_bus_call_method(bus, "org.freedesktop.systemd1",
                           ...
                           &error, NULL, "y", code);
    if (r < 0)
        return log_error_errno(r, "Failed to set exit code: %s",
                               bus_error_message(&error, r));
...
static int start_special(int argc, char* argv[], void* userdata)
{
    ...
    else
    {
        ...
        if (IN_SET(a, ACTION_POWEROFF, ACTION_REBOOT, ACTION_HALT,
                   ...
                   ACTION_SUSPEND_THEN_HIBERNATE))
        {
            ...
            /* On all other errors, try low-level operation. In order to
             * minimize the difference between operation with and without
             * logind, we explicitly enable non-blocking mode for this, as
             * logind's shutdown operations are always non-blocking. */

            arg_no_block = true;
        }
        else if (IN_SET(a, ACTION_EXIT, ACTION_KEXEC))
            /* Since exit/kexec are so close in behaviour to power-off/reboot,
             * let's also make them asynchronous, in order to not confuse the
             * user needlessly with unexpected behaviour. */
            arg_no_block = true;

        r = start_unit(argc, argv, userdata);
    }

    if (termination_action && arg_force < 2 && IN_SET(r, -ENOENT, -ETIMEDOUT))
        log_notice("It is possible to perform action directly, see discussion "
                   "of --force --force in man:systemctl(1).");
...
static int start_system_special(int argc, char* argv[], void* userdata)
{
    ...
    if (arg_scope != UNIT_FILE_SYSTEM)
        return log_error_errno(
            SYNTHETIC_ERRNO(EINVAL), "Bad action for %s mode.",
            arg_scope == UNIT_FILE_GLOBAL ? "--global" : "--user");

    return start_special(argc, argv, userdata);
}

static int check_unit_generic(int code, const UnitActiveState good_states[],
                              int nb_states, char** args)
{
    _cleanup_strv_free_ char** names = NULL;
    UnitActiveState active_state;
    sd_bus* bus;
...
static int check_unit_active(int argc, char* argv[], void* userdata)
{
    ...
    return check_unit_generic(EXIT_PROGRAM_NOT_RUNNING, states,
                              ELEMENTSOF(states), strv_skip(argv, 1));
}
...
static int check_unit_failed(int argc, char* argv[], void* userdata)
{
    ...
    return check_unit_generic(EXIT_PROGRAM_DEAD_AND_PID_EXISTS, states,
                              ELEMENTSOF(states), strv_skip(argv, 1));
}
...
static int kill_unit(int argc, char* argv[], void* userdata)
{
    ...
    STRV_FOREACH(name, names)
    {
        ...
        q = sd_bus_call_method(
            bus, "org.freedesktop.systemd1", "/org/freedesktop/systemd1",
            "org.freedesktop.systemd1.Manager", "KillUnit", &error, NULL, "ssi",
            *name, kill_who ? kill_who : arg_kill_who, arg_signal);
        if (q < 0)
        {
            log_error_errno(q, "Failed to kill unit %s: %s", *name,
                            bus_error_message(&error, q));
            if (r == 0)
                r = q;
        }
...
static void print_status_info(sd_bus* bus, UnitStatusInfo* i, bool* ellipsized)
{
    ...
    printf("%s%s%s %s", active_on, special_glyph(SPECIAL_GLYPH_BLACK_CIRCLE),
    ...
    if (!strv_isempty(i->dropin_paths))
    {
        ...
        STRV_FOREACH(dropin, i->dropin_paths)
        {
            ...
            if (!dir || last)
            {
                ...
                printf("%s\n"
                       "           %s",
                       dir, special_glyph(SPECIAL_GLYPH_TREE_RIGHT));
            }

            last = !(*(dropin + 1) && startswith(*(dropin + 1), dir));

            if (terminal_urlify_path(*dropin, basename(*dropin),
                                     &dropin_formatted) >= 0)
                df = dropin_formatted;
            else
                df = *dropin;
    ...
    if (ss)
        printf("   Active: %s%s (%s)%s", active_on, strna(i->active_state), ss,
               active_off);
    else
    ...
    timestamp = STRPTR_IN_SET(i->active_state, "active", "reloading")
                    ? i->active_enter_timestamp
                    : STRPTR_IN_SET(i->active_state, "inactive", "failed")
                          ? i->inactive_enter_timestamp
                          : STRPTR_IN_SET(i->active_state, "activating")
    ...
    if (endswith(i->id, ".timer"))
    {
        ...
        dual_timestamp nw,
            next = {i->next_elapse_real, i->next_elapse_monotonic};
        usec_t next_elapse;
        ...
        next_rel_time =
            format_timestamp_relative(tstamp1, sizeof tstamp1, next_elapse);
    ...
    if (!i->condition_result && i->condition_timestamp > 0)
    {
        ...
        printf("Condition: start %scondition failed%s at %s%s%s\n",
               ansi_highlight_yellow(), ansi_normal(), s2, s1 ? "; " : "",
               strempty(s1));
        ...
        if (c->tristate < 0)
            printf("           %s %s=%s%s%s was not met\n",
                   --n ? special_glyph(SPECIAL_GLYPH_TREE_BRANCH)
                       : special_glyph(SPECIAL_GLYPH_TREE_RIGHT),
                   c->name, c->trigger ? "|" : "", c->negate ? "!" : "",
                   c->param);
    }
    ...
    if (!i->assert_result && i->assert_timestamp > 0)
    {
        ...
        printf("   Assert: start %sassertion failed%s at %s%s%s\n",
               ansi_highlight_red(), ansi_normal(), s2, s1 ? "; " : "",
               strempty(s1));
    ...
    if (i->accept)
    {
        printf(" Accepted: %u; Connected: %u;", i->n_accepted,
               i->n_connections);
        if (i->n_refused)
    ...
    if (i->main_pid > 0 || i->control_pid > 0)
    {
        ...
        if (i->control_pid > 0)
        {
            ...
            if (i->main_pid > 0)
                ...
                fputs("Cntrl PID: ", stdout); /* if first in column, abbreviated
                                                 so it fits alignment */
    ...
    if (i->ip_ingress_bytes != (uint64_t)-1 &&
        i->ip_egress_bytes != (uint64_t)-1)
    {
    ...
    if (i->memory_current != (uint64_t)-1)
    {
        ...
        if (i->memory_min > 0 || i->memory_low > 0 ||
            i->memory_high != CGROUP_LIMIT_MAX ||
            i->memory_max != CGROUP_LIMIT_MAX ||
            ...
            i->memory_limit != CGROUP_LIMIT_MAX)
        {
            ...
            if (i->memory_min > 0)
            {
                printf("%smin: %s", prefix,
                       format_bytes(buf, sizeof(buf), i->memory_min));
                prefix = " ";
            }
            if (i->memory_low > 0)
            {
                printf("%slow: %s", prefix,
                       format_bytes(buf, sizeof(buf), i->memory_low));
                prefix = " ";
            }
            if (i->memory_high != CGROUP_LIMIT_MAX)
            {
                printf("%shigh: %s", prefix,
                       format_bytes(buf, sizeof(buf), i->memory_high));
                prefix = " ";
            }
            if (i->memory_max != CGROUP_LIMIT_MAX)
            {
                printf("%smax: %s", prefix,
                       format_bytes(buf, sizeof(buf), i->memory_max));
                prefix = " ";
            }
            if (i->memory_swap_max != CGROUP_LIMIT_MAX)
            {
                printf("%sswap max: %s", prefix,
                       format_bytes(buf, sizeof(buf), i->memory_swap_max));
                prefix = " ";
            }
            if (i->memory_limit != CGROUP_LIMIT_MAX)
            {
                printf("%slimit: %s", prefix,
    ...
    if (i->cpu_usage_nsec != (uint64_t)-1)
    {
        ...
        printf("      CPU: %s\n",
               format_timespan(buf, sizeof(buf),
                               i->cpu_usage_nsec / NSEC_PER_USEC,
                               USEC_PER_MSEC));
    }
    ...
    if (i->control_group)
    {
        ...
        r = unit_show_processes(bus, i->id, i->control_group, prefix, c,
        ...
        if (r == -EBADR)
        {
            ...
            /* Fallback for older systemd versions where the GetUnitProcesses()
            ...
            show_cgroup_and_extra(SYSTEMD_CGROUP_CONTROLLER, i->control_group,
                                  prefix, c, extra, k, get_output_flags());
        }
        else if (r < 0)
            log_warning_errno(
                r, "Failed to dump process list for '%s', ignoring: %s", i->id,
                bus_error_message(&error, r));
    }
    ...
    if (i->id && arg_transport == BUS_TRANSPORT_LOCAL)
        show_journal_by_unit(
            ...
            arg_lines, getuid(), get_output_flags() | OUTPUT_BEGIN_NEWLINE,
            SD_JOURNAL_LOCAL_ONLY, arg_scope == UNIT_FILE_SYSTEM, ellipsized);
...
static int map_main_pid(sd_bus* bus, const char* member, sd_bus_message* m,
...
static int map_load_error(sd_bus* bus, const char* member, sd_bus_message* m,
                          sd_bus_error* error, void* userdata)
{
...
static int map_listen(sd_bus* bus, const char* member, sd_bus_message* m,
                      sd_bus_error* error, void* userdata)
{
    const char *type, *path;
    char*** p = userdata;
...
static int map_conditions(sd_bus* bus, const char* member, sd_bus_message* m,
                          sd_bus_error* error, void* userdata)
{
    UnitStatusInfo* i = userdata;
    ...
    while ((r = sd_bus_message_read(m, "(sbbsi)", &cond, &trigger, &negate,
                                    &param, &state)) > 0)
    {
...
static int map_asserts(sd_bus* bus, const char* member, sd_bus_message* m,
                       sd_bus_error* error, void* userdata)
{
    UnitStatusInfo* i = userdata;
    const char *cond, *param;
...
static int map_exec(sd_bus* bus, const char* member, sd_bus_message* m,
                    sd_bus_error* error, void* userdata)
{
...
static int print_property(const char* name, const char* expected_value,
                          sd_bus_message* m, bool value, bool all)
{
    char bus_type;
    const char* contents;
    int r;
    ...
    switch (bus_type)
    {
        ...
        case SD_BUS_TYPE_STRUCT:
            ...
            else if (contents[0] == SD_BUS_TYPE_STRING &&
            ...
            else if (STR_IN_SET(name, "SystemCallFilter",
            ...
            else if (STR_IN_SET(name, "SELinuxContext", "AppArmorProfile",
                                "SmackProcessLabel"))
            {
                int ignore;
                const char* s;
            ...
            else if (endswith(name, "ExitStatus") && streq(contents, "aiai"))
            {
                ...
                r = sd_bus_message_read_array(m, 'i', (const void**)&status,
                                              &sz_status);
                if (r < 0)
                    return bus_log_parse_error(r);

                r = sd_bus_message_read_array(m, 'i', (const void**)&signal,
                                              &sz_signal);
                if (r < 0)
                    return bus_log_parse_error(r);
        ...
        case SD_BUS_TYPE_ARRAY:
            ...
            if (contents[0] == SD_BUS_TYPE_STRUCT_BEGIN &&
                     ...
                     streq(name, "TimersMonotonic"))
            {
                ...
                while ((r = sd_bus_message_read(m, "(stt)", &base, &v,
                                                &next_elapse)) > 0)
                {
                    ...
                    bus_print_property_valuef(
                        ...
                        format_timespan(timespan2, sizeof(timespan2),
                                        next_elapse, 0));
                }
                if (r < 0)
                    return bus_log_parse_error(r);
                     ...
                     streq(name, "TimersCalendar"))
            {
                ...
                while ((r = sd_bus_message_read(m, "(sst)", &base, &spec,
                                                &next_elapse)) > 0)
                {
                     ...
                     startswith(name, "Exec"))
            {
                ...
                while ((r = exec_status_info_deserialize(m, &info)) > 0)
                {
                    ...
                    bus_print_property_valuef(
                        name, expected_value, value,
                        "{ path=%s ; argv[]=%s ; ignore_errors=%s ; "
                        "start_time=[%s] ; stop_time=[%s] ; pid=" PID_FMT
                        " ; code=%s ; status=%i%s%s }",
                        strna(info.path), strna(tt), yes_no(info.ignore),
                        ...
                        strna(format_timestamp(timestamp2, sizeof(timestamp2),
                                               info.exit_timestamp)),
                        info.pid, sigchld_code_to_string(info.code),
                     ...
                     STR_IN_SET(name, "IODeviceWeight", "BlockIODeviceWeight"))
            {
                ...
                return 1;
                     ...
                     (cgroup_io_limit_type_from_string(name) >= 0 ||
                      STR_IN_SET(name, "BlockIOReadBandwidth",
                                 "BlockIOWriteBandwidth")))
            {
                ...
                while ((r = sd_bus_message_read(m, "(st)", &path, &bandwidth)) >
                       0)
                    bus_print_property_valuef(name, expected_value, value,
                                              "%s %" PRIu64, strna(path),
                                              bandwidth);
                     ...
                     streq(name, "IODeviceLatencyTargetUSec"))
            {
                ...
                while ((r = sd_bus_message_read(m, "(st)", &path, &target)) > 0)
                    bus_print_property_valuef(
                        name, expected_value, value, "%s %s", strna(path),
            ...
            else if (STR_IN_SET(name, "IPAddressAllow", "IPAddressDeny"))
            {
                ...
                for (;;)
                {
                    ...
                    if (in_addr_prefix_to_string(family,
                                                 (union in_addr_union*)ap,
                                                 prefixlen, &str) < 0)
                        continue;
            ...
            else if (STR_IN_SET(name, "BindPaths", "BindReadOnlyPaths"))
            {
                ...
                while ((r = sd_bus_message_read(m, "(ssbt)", &source, &dest,
                                                &ignore_enoent, &rbind)) > 0)
                {
                    _cleanup_free_ char* str = NULL;
                    ...
                    if (asprintf(&str, "%s%s%s%s%s", ignore_enoent ? "-" : "",
                                 source, isempty(dest) ? "" : ":",
                                 strempty(dest),
            ...
            else if (streq(name, "TemporaryFileSystem"))
            {
                ...
                while ((r = sd_bus_message_read(m, "(ss)", &target, &option)) >
                       0)
                {
                    _cleanup_free_ char* str = NULL;
...
DEFINE_PRIVATE_STRING_TABLE_LOOKUP_FROM_STRING(systemctl_show_mode,
                                               SystemctlShowMode);

static int show_one(sd_bus* bus, const char* path, const char* unit,
                    SystemctlShowMode show_mode, bool* new_line,
                    bool* ellipsized)
{

    static const struct bus_properties_map
        property_map[] = {{"LoadState", "s", NULL,
                           offsetof(UnitStatusInfo, load_state)},
                          {"ActiveState", "s", NULL,
                           offsetof(UnitStatusInfo, active_state)},
                          {"Documentation", "as", NULL,
                           offsetof(UnitStatusInfo, documentation)},
                          {}},
        status_map[] = {
            ...
            {"UnitFileState", "s", NULL,
             offsetof(UnitStatusInfo, unit_file_state)},
            {"UnitFilePreset", "s", NULL,
             offsetof(UnitStatusInfo, unit_file_preset)},
            {"Description", "s", NULL, offsetof(UnitStatusInfo, description)},
            {"Following", "s", NULL, offsetof(UnitStatusInfo, following)},
            {"Documentation", "as", NULL,
             offsetof(UnitStatusInfo, documentation)},
            {"FragmentPath", "s", NULL,
             offsetof(UnitStatusInfo, fragment_path)},
            {"SourcePath", "s", NULL, offsetof(UnitStatusInfo, source_path)},
            {"ControlGroup", "s", NULL,
             offsetof(UnitStatusInfo, control_group)},
            {"DropInPaths", "as", NULL, offsetof(UnitStatusInfo, dropin_paths)},
            {"LoadError", "(ss)", map_load_error,
             offsetof(UnitStatusInfo, load_error)},
            {"Result", "s", NULL, offsetof(UnitStatusInfo, result)},
            {"InactiveExitTimestamp", "t", NULL,
             offsetof(UnitStatusInfo, inactive_exit_timestamp)},
            {"InactiveExitTimestampMonotonic", "t", NULL,
             offsetof(UnitStatusInfo, inactive_exit_timestamp_monotonic)},
            {"ActiveEnterTimestamp", "t", NULL,
             offsetof(UnitStatusInfo, active_enter_timestamp)},
            {"ActiveExitTimestamp", "t", NULL,
             offsetof(UnitStatusInfo, active_exit_timestamp)},
            {"InactiveEnterTimestamp", "t", NULL,
             offsetof(UnitStatusInfo, inactive_enter_timestamp)},
            {"NeedDaemonReload", "b", NULL,
             offsetof(UnitStatusInfo, need_daemon_reload)},
            ...
            {"ExecMainStartTimestamp", "t", NULL,
             offsetof(UnitStatusInfo, start_timestamp)},
            {"ExecMainExitTimestamp", "t", NULL,
             offsetof(UnitStatusInfo, exit_timestamp)},
            {"ExecMainCode", "i", NULL, offsetof(UnitStatusInfo, exit_code)},
            {"ExecMainStatus", "i", NULL,
             offsetof(UnitStatusInfo, exit_status)},
            {"ConditionTimestamp", "t", NULL,
             offsetof(UnitStatusInfo, condition_timestamp)},
            {"ConditionResult", "b", NULL,
             offsetof(UnitStatusInfo, condition_result)},
            {"Conditions", "a(sbbsi)", map_conditions, 0},
            {"AssertTimestamp", "t", NULL,
             offsetof(UnitStatusInfo, assert_timestamp)},
            {"AssertResult", "b", NULL,
             offsetof(UnitStatusInfo, assert_result)},
            {"Asserts", "a(sbbsi)", map_asserts, 0},
            {"NextElapseUSecRealtime", "t", NULL,
             offsetof(UnitStatusInfo, next_elapse_real)},
            {"NextElapseUSecMonotonic", "t", NULL,
             offsetof(UnitStatusInfo, next_elapse_monotonic)},
            {"NAccepted", "u", NULL, offsetof(UnitStatusInfo, n_accepted)},
            {"NConnections", "u", NULL,
             offsetof(UnitStatusInfo, n_connections)},
            ...
            {"MemoryCurrent", "t", NULL,
             offsetof(UnitStatusInfo, memory_current)},
            {"MemoryMin", "t", NULL, offsetof(UnitStatusInfo, memory_min)},
            {"MemoryLow", "t", NULL, offsetof(UnitStatusInfo, memory_low)},
            {"MemoryHigh", "t", NULL, offsetof(UnitStatusInfo, memory_high)},
            {"MemoryMax", "t", NULL, offsetof(UnitStatusInfo, memory_max)},
            {"MemorySwapMax", "t", NULL,
             offsetof(UnitStatusInfo, memory_swap_max)},
            {"MemoryLimit", "t", NULL, offsetof(UnitStatusInfo, memory_limit)},
            {"CPUUsageNSec", "t", NULL,
             offsetof(UnitStatusInfo, cpu_usage_nsec)},
            {"TasksCurrent", "t", NULL,
             offsetof(UnitStatusInfo, tasks_current)},
            {"TasksMax", "t", NULL, offsetof(UnitStatusInfo, tasks_max)},
            {"IPIngressBytes", "t", NULL,
             offsetof(UnitStatusInfo, ip_ingress_bytes)},
            {"IPEgressBytes", "t", NULL,
             offsetof(UnitStatusInfo, ip_egress_bytes)},
    ...
    r = bus_map_all_properties(
        bus, "org.freedesktop.systemd1", path,
        show_mode == SYSTEMCTL_SHOW_STATUS ? status_map : property_map,
        BUS_MAP_BOOLEAN_AS_BOOL, &error, &reply, &info);
    if (r < 0)
        return log_error_errno(r, "Failed to get properties: %s",
                               bus_error_message(&error, r));

    if (unit && streq_ptr(info.load_state, "not-found") &&
    ...
    *new_line = true;
    ...
    r = bus_message_print_all_properties(reply, print_property, arg_properties,
                                         arg_value, arg_all, &found_properties);
    if (r < 0)
        return bus_log_parse_error(r);
...
static int get_unit_dbus_path_by_pid(sd_bus* bus, uint32_t pid, char** unit)
{
    ...
    r = sd_bus_call_method(bus, "org.freedesktop.systemd1",
                           ...
                           &error, &reply, "u", pid);
    if (r < 0)
        return log_error_errno(r, "Failed to get unit for PID %" PRIu32 ": %s",
...
static int show_all(sd_bus* bus, bool* new_line, bool* ellipsized)
{
    ...
    for (u = unit_infos; u < unit_infos + c; u++)
    {
        ...
        r = show_one(bus, p, u->id, SYSTEMCTL_SHOW_STATUS, new_line,
                     ellipsized);
        if (r < 0)
            return r;
        else if (r > 0 && ret == 0)
...
static int show_system_status(sd_bus* bus)
{
    ...
    r = bus_map_all_properties(
        bus, "org.freedesktop.systemd1", "/org/freedesktop/systemd1",
        machine_info_property_map, BUS_MAP_STRDUP, &error, NULL, &mi);
    if (r < 0)
        return log_error_errno(r, "Failed to read server status: %s",
                               bus_error_message(&error, r));
    ...
    if (IN_SET(arg_transport, BUS_TRANSPORT_LOCAL, BUS_TRANSPORT_MACHINE))
    {
        ...
        show_cgroup(SYSTEMD_CGROUP_CONTROLLER, strempty(mi.control_group),
                    prefix, c, get_output_flags());
    }
...
static int show(int argc, char* argv[], void* userdata)
{
    ...
    if (show_mode < 0)
        return log_error_errno(SYNTHETIC_ERRNO(EINVAL), "Invalid argument.");

    if (show_mode == SYSTEMCTL_SHOW_HELP && argc <= 1)
        return log_error_errno(SYNTHETIC_ERRNO(EINVAL),
                               "This command expects one or more unit names. "
                               "Did you mean --help?");
    ...
    if (show_mode == SYSTEMCTL_SHOW_PROPERTIES && argc <= 1)
        return show_one(bus, "/org/freedesktop/systemd1", NULL, show_mode,
                        &new_line, &ellipsized);
    ...
    else
    {
        ...
        if (!strv_isempty(patterns))
        {
            ...
            STRV_FOREACH(name, names)
            {
                ...
                r = show_one(bus, path, *name, show_mode, &new_line,
                             &ellipsized);
                if (r < 0)
                    return r;
                if (r > 0 && ret == 0)
...
static int cat(int argc, char* argv[], void* userdata)
{
    ...
    STRV_FOREACH(name, names)
    {
        ...
        r = unit_find_paths(bus, *name, &lp, false, &fragment_path,
                            &dropin_paths);
        if (r == -ERFKILL)
        {
        ...
        if (r == -EKEYREJECTED)
        {
            printf("%s# Unit %s could not be loaded.%s\n",
                   ansi_highlight_magenta(), *name, ansi_normal());
            continue;
        }
        if (r < 0)
            return r;
        ...
        if (need_daemon_reload(bus, *name) >
            ...
            fprintf(stderr,
                    "%s# Warning: %s changed on disk, the version systemd has "
                    "loaded is outdated.\n"
                    "%s# This output shows the current version of the unit's "
                    "original fragment and drop-in files.\n"
                    "%s# If fragments or drop-ins were added or removed, they "
                    "are not properly reflected in this output.\n"
                    "%s# Run 'systemctl%s daemon-reload' to reload units.%s\n",
                    ansi_highlight_red(), *name, ansi_highlight_red(),
...
static int set_property(int argc, char* argv[], void* userdata)
{
    ...
    if (t < 0)
        return log_error_errno(SYNTHETIC_ERRNO(EINVAL), "Invalid unit type: %s",
                               n);
    ...
    if (r < 0)
        return log_error_errno(r, "Failed to set unit properties on %s: %s", n,
                               bus_error_message(&error, r));
...
static int daemon_reload(int argc, char* argv[], void* userdata)
{
    ...
    /* Note we use an extra-long timeout here. This is because a reload or
     * reexec means generators are rerun which are timed out after
     * DEFAULT_TIMEOUT_USEC. Let's use twice that time here, so that the
     * generators can have their timeout, and for everything else there's the
     * same time budget in place. */
    ...
    if (r < 0 && arg_action == ACTION_SYSTEMCTL)
        return log_error_errno(r, "Failed to reload daemon: %s",
                               bus_error_message(&error, r));

    /* Note that for the legacy commands (i.e. those with action !=
     * ACTION_SYSTEMCTL) we support fallbacks to the old ways of doing things,
     * hence don't log any error in that case here. */
...
static int trivial_method(int argc, char* argv[], void* userdata)
{
    ...
    method = streq(argv[0], "clear-jobs") || streq(argv[0], "cancel")
                 ? "ClearJobs"
                 : streq(argv[0], "reset-failed")
                       ? "ResetFailed"
                       : streq(argv[0], "halt")
                             ? "Halt"
                             : streq(argv[0], "reboot")
    ...
    r = sd_bus_call_method(
        bus, "org.freedesktop.systemd1", "/org/freedesktop/systemd1",
        "org.freedesktop.systemd1.Manager", method, &error, NULL, NULL);
    if (r < 0 && arg_action == ACTION_SYSTEMCTL)
        return log_error_errno(r, "Failed to execute operation: %s",
                               bus_error_message(&error, r));
...
static int reset_failed(int argc, char* argv[], void* userdata)
{
    ...
    STRV_FOREACH(name, names)
    {
        ...
        q = sd_bus_call_method(bus, "org.freedesktop.systemd1",
                               "/org/freedesktop/systemd1",
                               "org.freedesktop.systemd1.Manager",
                               "ResetFailedUnit", &error, NULL, "s", *name);
        if (q < 0)
        {
            log_error_errno(q, "Failed to reset failed state of unit %s: %s",
...
static int show_environment(int argc, char* argv[], void* userdata)
{
    ...
    r = sd_bus_get_property(bus, "org.freedesktop.systemd1",
                            ...
                            &error, &reply, "as");
    if (r < 0)
        return log_error_errno(r, "Failed to get environment: %s",
                               bus_error_message(&error, r));
    ...
    while ((r = sd_bus_message_read_basic(reply, SD_BUS_TYPE_STRING, &text)) >
           0)
    {
...
static int switch_root(int argc, char* argv[], void* userdata)
{
    ...
    /* Instruct PID1 to exclude us from its killing spree applied during the
     * transition. Otherwise we
     * would exit with a failure status even though the switch to the new root
     * has succeed. */
    assert(saved_argv);
    assert(saved_argv[0]);
    saved_argv[0][0] = '@';
    ...
    if (r < 0)
        log_warning_errno(
            r, "Failed to change disposition of SIGTERM to ignore: %m");
    ...
    r = sd_bus_call_method(bus, "org.freedesktop.systemd1",
                           ...
                           &error, NULL, "ss", root, init);
    if (r < 0)
    {
        (void)default_signals(SIGTERM, -1);

        return log_error_errno(r, "Failed to switch root: %s",
                               bus_error_message(&error, r));
    }
...
static int set_environment(int argc, char* argv[], void* userdata)
{
    ...
    r = sd_bus_call(bus, m, 0, &error, NULL);
    if (r < 0)
        return log_error_errno(r, "Failed to set environment: %s",
                               bus_error_message(&error, r));
...
static int import_environment(int argc, char* argv[], void* userdata)
{
    ...
    r = sd_bus_call(bus, m, 0, &error, NULL);
    if (r < 0)
        return log_error_errno(r, "Failed to import environment: %s",
                               bus_error_message(&error, r));
...
static int enable_sysv_units(const char* verb, char** args)
{
    ...
    /* Processes all SysV units, and reshuffles the array so that afterwards
     * only the native units remain */
    ...
    if (!STR_IN_SET(verb, "enable", "disable", "is-enabled"))
        return 0;
    ...
    while (args[f])
    {
        ...
        /* If we have both a native unit and a SysV script, enable/disable them
         * both (below); for is-enabled, prefer the native unit */
        ...
        {
            if (found_native)
                log_info("Synchronizing state of %s with SysV service script "
                         "with %s.",
                         name, argv[0]);
            else
        ...
        /* Let's copy the verb, since it's still pointing directly into the
         * original argv[] array we
         * got passed, but safe_fork() is likely going to rewrite that for the
         * new child */
        v = strdup(verb);
        if (!v)
            return log_oom();
        ...
        j = wait_for_terminate_and_check("sysv-install", pid,
                                         WAIT_LOG_ABNORMAL);
        if (j < 0)
            return j;
        if (streq(verb, "is-enabled"))
        {
            ...
            return -EBADE; /* We don't warn here, under the assumption the
                              script already showed an explanation */
...
static int mangle_names(char** original_names, char*** mangled_names)
{
    ...
    STRV_FOREACH(name, original_names)
    {
        ...
        {
            r = unit_name_mangle(*name, arg_quiet ? 0 : UNIT_NAME_MANGLE_WARN,
                                 i);
            if (r < 0)
            {
                *i = NULL;
...
static int normalize_filenames(char** names)
{
    ...
    if (!path_is_absolute(*u))
    {
        ...
        if (!strchr(*u, '/'))
            return log_error_errno(SYNTHETIC_ERRNO(EINVAL),
                                   "Link argument does contain at least one "
...
static int normalize_names(char** names, bool warn_if_path)
{
    ...
    if (warn_if_path && was_path)
        log_warning("Warning: Can't execute disable on the unit file path. "
                    "Proceeding with the unit name.");
...
static int unit_exists(LookupPaths* lp, const char* unit)
{
    ...
    r = bus_map_all_properties(bus, "org.freedesktop.systemd1", path,
                               property_map, 0, &error, &m, &info);
    if (r < 0)
        return log_error_errno(r, "Failed to get properties: %s",
                               bus_error_message(&error, r));

    return !streq_ptr(info.load_state, "not-found") ||
           !streq_ptr(info.active_state, "inactive");
}
...
static int enable_unit(int argc, char* argv[], void* userdata)
{
    ...
    /* If the operation was fully executed by the SysV compat, let's finish
     * early */
    ...
    if (install_client_side())
    {
        ...
        {
            r = unit_file_enable(arg_scope, flags, arg_root, names, &changes,
                                 &n_changes);
            carries_install_info = r;
        }
        else if (streq(verb, "disable"))
            r = unit_file_disable(arg_scope, flags, arg_root, names, &changes,
                                  &n_changes);
        else if (streq(verb, "reenable"))
        {
            r = unit_file_reenable(arg_scope, flags, arg_root, names, &changes,
                                   &n_changes);
            carries_install_info = r;
        }
        else if (streq(verb, "link"))
            r = unit_file_link(arg_scope, flags, arg_root, names, &changes,
                               &n_changes);
        else if (streq(verb, "preset"))
        {
            ...
            r = unit_file_mask(arg_scope, flags, arg_root, names, &changes,
                               &n_changes);
        else if (streq(verb, "unmask"))
            r = unit_file_unmask(arg_scope, flags, arg_root, names, &changes,
                                 &n_changes);
        else if (streq(verb, "revert"))
            r = unit_file_revert(arg_scope, arg_root, names, &changes,
                                 &n_changes);
        else
    ...
    else
    {
        ...
        if (STR_IN_SET(verb, "mask", "unmask"))
        {
            ...
            STRV_FOREACH(name, names)
            {
                ...
                if (r < 0)
                    return r;
                if (r == 0)
                    log_notice("Unit %s does not exist, proceeding anyway.",
                               *name);
            }
        }
        ...
        if (send_preset_mode)
        {
            r = sd_bus_message_append(
                m, "s", unit_file_preset_mode_to_string(arg_preset_mode));
            if (r < 0)
                return bus_log_create_error(r);
        }
        ...
        r = sd_bus_call(bus, m, 0, &error, &reply);
        if (r < 0)
            return log_error_errno(r, "Failed to %s unit: %s", verb,
                                   bus_error_message(&error, r));
    ...
    if (carries_install_info == 0 && !ignore_carries_install_info)
        log_notice("The unit files have no installation config (WantedBy=, "
                   "RequiredBy=, Also=,\n"
                   "Alias= settings in the [Install] section, and "
                   "DefaultInstance= for template\n"
                   "units). This means they are not meant to be enabled using "
                   "systemctl.\n"
                   " \n" /* trick: the space is needed so that the line does not
                            get stripped from output */
                   "Possible reasons for having this kind of units are:\n"
                   "%1$s A unit may be statically enabled by being symlinked "
                   "from another unit's\n"
                   "  .wants/ or .requires/ directory.\n"
                   "%1$s A unit's purpose may be to act as a helper for some "
                   "other unit which has\n"
                   "  a requirement dependency on it.\n"
                   "%1$s A unit may be started when needed via activation "
                   "(socket, path, timer,\n"
                   "  D-Bus, udev, scripted systemctl call, ...).\n"
                   "%1$s In case of template units, the unit is meant to be "
                   "enabled with some\n"
...
static int add_dependency(int argc, char* argv[], void* userdata)
{
    ...
    r = unit_name_mangle_with_suffix(
        argv[1], arg_quiet ? 0 : UNIT_NAME_MANGLE_WARN, ".target", &target);
    if (r < 0)
        return log_error_errno(r, "Failed to mangle unit name: %m");
    ...
    if (install_client_side())
    {
        r = unit_file_add_dependency(arg_scope, args_to_flags(), arg_root,
                                     names, target, dep, &changes, &n_changes);
        unit_file_dump_changes(r, "add dependency on", changes, n_changes,
                               arg_quiet);
    ...
    else
    {
        ...
        r = sd_bus_message_append(m, "ssbb", target,
                                  unit_dependency_to_string(dep), arg_runtime,
                                  arg_force);
        if (r < 0)
            return bus_log_create_error(r);
        ...
        r = sd_bus_call(bus, m, 0, &error, &reply);
        if (r < 0)
            return log_error_errno(r, "Failed to add dependency: %s",
                                   bus_error_message(&error, r));
...
static int preset_all(int argc, char* argv[], void* userdata)
{
    ...
    else
    {
        ...
        r = sd_bus_call_method(
            bus, "org.freedesktop.systemd1", "/org/freedesktop/systemd1",
            "org.freedesktop.systemd1.Manager", "PresetAllUnitFiles", &error,
            &reply, "sbb", unit_file_preset_mode_to_string(arg_preset_mode),
            arg_runtime, arg_force);
        if (r < 0)
            return log_error_errno(r, "Failed to preset all units: %s",
                                   bus_error_message(&error, r));
...
static int show_installation_targets(sd_bus* bus, const char* name)
{
    ...
    r = sd_bus_call_method(
        bus, "org.freedesktop.systemd1", "/org/freedesktop/systemd1",
        "org.freedesktop.systemd1.Manager", "GetUnitFileLinks", &error, &reply,
        "sb", name, arg_runtime);
    if (r < 0)
        return log_error_errno(r, "Failed to get unit file links for %s: %s",
                               name, bus_error_message(&error, r));
...
static int unit_is_enabled(int argc, char* argv[], void* userdata)
{
    ...
    if (install_client_side())
    {
        STRV_FOREACH(name, names)
        {
            ...
            if (r < 0)
                return log_error_errno(
                    r, "Failed to get unit file state for %s: %m", *name);

            if (IN_SET(state, UNIT_FILE_ENABLED, UNIT_FILE_ENABLED_RUNTIME,
                       UNIT_FILE_STATIC, UNIT_FILE_INDIRECT,
                       UNIT_FILE_GENERATED))
                enabled = true;
    ...
    else
    {
        ...
        STRV_FOREACH(name, names)
        {
            ...
            r = sd_bus_call_method(
                bus, "org.freedesktop.systemd1", "/org/freedesktop/systemd1",
                "org.freedesktop.systemd1.Manager", "GetUnitFileState", &error,
                &reply, "s", *name);
            if (r < 0)
                return log_error_errno(
                    r, "Failed to get unit file state for %s: %s", *name,
                    bus_error_message(&error, r));
            ...
            if (STR_IN_SET(s, "enabled", "enabled-runtime", "static",
                           "indirect", "generated"))
                enabled = true;
...
static int match_startup_finished(sd_bus_message* m, void* userdata,
                                  sd_bus_error* error)
{
    char** state = userdata;
    int r;
    ...
    r = sd_bus_get_property_string(
        sd_bus_message_get_bus(m), "org.freedesktop.systemd1",
        "/org/freedesktop/systemd1", "org.freedesktop.systemd1.Manager",
...
static int is_system_running(int argc, char* argv[], void* userdata)
{
    ...
    if (running_in_chroot() > 0 ||
        (arg_transport == BUS_TRANSPORT_LOCAL && !sd_booted()))
    {
        if (!arg_quiet)
            puts("offline");
        return EXIT_FAILURE;
    ...
    if (arg_wait)
    {
        ...
        if (r >= 0)
            r = sd_bus_match_signal_async(
                bus, &slot_startup_finished, "org.freedesktop.systemd1",
                "/org/freedesktop/systemd1", "org.freedesktop.systemd1.Manager",
                "StartupFinished", match_startup_finished, NULL, &state);
        if (r < 0)
        {
            log_warning_errno(
                r, "Failed to request match for StartupFinished: %m");
            arg_wait = false;
        }
    }
    ...
    r = sd_bus_get_property_string(
        bus, "org.freedesktop.systemd1", "/org/freedesktop/systemd1",
        "org.freedesktop.systemd1.Manager", "SystemState", &error, &state);
    if (r < 0)
    {
        log_warning_errno(r, "Failed to query system state: %s",
                          bus_error_message(&error, r));
...
static int create_edit_temp_file(const char* new_path,
                                 const char* original_path, char** ret_tmp_fn)
{
    ...
    if (r < 0)
        return log_error_errno(
            r, "Failed to determine temporary filename for \"%s\": %m",
            new_path);

    r = mkdir_parents(new_path, 0755);
    if (r < 0)
        return log_error_errno(r, "Failed to create directories for \"%s\": %m",
                               new_path);
    ...
    if (r == -ENOENT)
    {
        ...
        if (r < 0)
            return log_error_errno(
                r, "Failed to create temporary file \"%s\": %m", t);
    }
    else if (r < 0)
        return log_error_errno(
            r, "Failed to create temporary file for \"%s\": %m", new_path);
...
static int get_file_to_edit(const LookupPaths* paths, const char* name,
                            char** ret_path)
{
    ...
    if (arg_runtime)
    {
        if (access(path, F_OK) >= 0)
            return log_error_errno(SYNTHETIC_ERRNO(EEXIST),
                                   "Refusing to create \"%s\" because it would "
                                   "be overridden by \"%s\" anyway.",
                                   run, path);
...
static int unit_file_create_new(const LookupPaths* paths, const char* unit_name,
                                const char* suffix, char** ret_new_path,
                                char** ret_tmp_path)
{
...
static int unit_file_create_copy(const LookupPaths* paths,
                                 const char* unit_name,
                                 const char* fragment_path, char** ret_new_path,
                                 char** ret_tmp_path)
{
...
static int run_editor(char** paths)
{
    ...
    r = safe_fork("(editor)",
                  FORK_RESET_SIGNALS | FORK_DEATHSIG | FORK_RLIMIT_NOFILE_SAFE |
                      FORK_LOG | FORK_WAIT,
                  NULL);
    if (r < 0)
        return r;
    if (r == 0)
    {
        ...
        /* SYSTEMD_EDITOR takes precedence over EDITOR which takes precedence
         * over VISUAL If neither SYSTEMD_EDITOR nor EDITOR nor VISUAL are
         * present, we try to execute well known editors
         */
        ...
        log_error("Cannot edit unit(s), no editor available. Please set either "
...
static int find_paths_to_edit(sd_bus* bus, char** names, char*** paths)
{
    ...
    STRV_FOREACH(name, names)
    {
        ...
        if (r == -EKEYREJECTED)
        {
            /* If loading of the unit failed server side complete, then the
             * server won't tell us the unit file path. In that case, find the
             * file client side. */
            log_debug_errno(
                r, "Unit '%s' was not loaded correctly, retrying client-side.",
                *name);
        ...
        if (r == 0)
        {
            ...
            if (!arg_force)
            {
                log_info("Run 'systemctl edit%s --force --full %s' to create a "
                         "new unit.",
        ...
        else
        {
            ...
            if (arg_full)
                r = unit_file_create_copy(&lp, unit_name, path, &new_path,
                                          &tmp_path);
            else
                r = unit_file_create_new(&lp, unit_name, ".d/override.conf",
                                         &new_path, &tmp_path);
        }
        if (r < 0)
            return r;
...
static int edit(int argc, char* argv[], void* userdata)
{
    ...
    STRV_FOREACH_PAIR(original, tmp, paths)
    {
        ...
        {
            log_warning("Editing \"%s\" canceled: temporary file is empty.",
                        *original);
            continue;
        }
        ...
        if (r < 0)
        {
            r = log_error_errno(errno, "Failed to rename \"%s\" to \"%s\": %m",
                                *tmp, *original);
            goto end;
        }
    }
...
static int systemctl_help(void)
{
    ...
    printf(
        ...
        "     --state=STATE    List units with particular LOAD or SUB or "
        "ACTIVE state\n"
        "     --failed         Shorcut for --state=failed\n"
        "  -p --property=NAME  Show only properties by this name\n"
        "  -a --all            Show all properties/all units currently in "
        "memory,\n"
        "                      including dead/empty ones. To list all units "
        "installed on\n"
        "                      the system, use the 'list-unit-files' command "
        "instead.\n"
        ...
        "                      queueing a new job\n"
        "  -T --show-transaction\n"
        "                      When enqueuing a unit job, show full "
        "transaction\n"
        "     --show-types     When showing sockets, explicitly show their "
        "type\n"
        "     --value          When showing properties, only print the value\n"
        "  -i --ignore-inhibitors\n"
        "                      When shutting down or sleeping, ignore "
        "inhibitors\n"
        "     --kill-who=WHO   Who to send signal to\n"
        "  -s --signal=SIGNAL  Which signal to send\n"
        "     --now            Start or stop unit in addition to enabling or "
        "disabling it\n"
        "     --dry-run        Only print what would be done\n"
        "  -q --quiet          Suppress output\n"
        "     --wait           For (re)start, wait until service stopped "
        "again\n"
        ...
        "     --preset-mode=   Apply only enable, only disable, or all "
        "presets\n"
        "     --root=PATH      Enable/disable/mask unit files in the specified "
        "root\n"
        "                      directory\n"
        "  -n --lines=INTEGER  Number of journal entries to show\n"
        "  -o --output=STRING  Change journal output mode (short, "
        "short-precise,\n"
        "                             short-iso, short-iso-precise, "
        "short-full,\n"
        ...
        "  list-sockets [PATTERN...]           List socket units currently in "
        "memory,\n"
        "                                      ordered by address\n"
        "  list-timers [PATTERN...]            List timer units currently in "
        "memory,\n"
        "                                      ordered by next elapse\n"
        "  start UNIT...                       Start (activate) one or more "
        "units\n"
        "  stop UNIT...                        Stop (deactivate) one or more "
        "units\n"
        "  reload UNIT...                      Reload one or more units\n"
        "  restart UNIT...                     Start or restart one or more "
        "units\n"
        "  try-restart UNIT...                 Restart one or more units if "
        "active\n"
        "  reload-or-restart UNIT...           Reload one or more units if "
        "possible,\n"
        "                                      otherwise start or restart\n"
        "  try-reload-or-restart UNIT...       If active, reload one or more "
        "units,\n"
        "                                      if supported, otherwise "
        "restart\n"
        "  isolate UNIT                        Start one unit and stop all "
        "others\n"
        "  kill UNIT...                        Send signal to processes of a "
        "unit\n"
        ...
        "specified units\n"
        "  set-property UNIT PROPERTY=VALUE... Sets one or more properties of "
        "a unit\n"
        ...
        "one, or more\n"
        "                                      units\n"
        "  list-dependencies [UNIT]            Recursively show units which "
        "are required\n"
        ...
        "  reenable UNIT...                    Reenable one or more unit "
        "files\n"
        "  preset UNIT...                      Enable/disable one or more unit "
        "files\n"
        "                                      based on preset configuration\n"
        "  preset-all                          Enable/disable all unit files "
        "based on\n"
        "                                      preset configuration\n"
        "  is-enabled UNIT...                  Check whether unit files are "
        "enabled\n"
        "  mask UNIT...                        Mask one or more units\n"
        "  unmask UNIT...                      Unmask one or more units\n"
        "  link PATH...                        Link one or more units files "
        "into\n"
        "                                      the search path\n"
        "  revert UNIT...                      Revert one or more unit files "
        "to vendor\n"
        "                                      version\n"
        "  add-wants TARGET UNIT...            Add 'Wants' dependency for the "
        "target\n"
        "                                      on specified one or more units\n"
        "  add-requires TARGET UNIT...         Add 'Requires' dependency for "
        "the target\n"
        "                                      on specified one or more units\n"
        "  edit UNIT...                        Edit one or more unit files\n"
        "  get-default                         Get the name of the default "
        "target\n"
        "  set-default TARGET                  Set the default target\n\n"
        "%3$sMachine Commands:%4$s\n"
        "  list-machines [PATTERN...]          List local containers and "
        "host\n\n"
        "%3$sJob Commands:%4$s\n"
        "  list-jobs [PATTERN...]              List jobs\n"
        "  cancel [JOB...]                     Cancel all, one, or more "
        "jobs\n\n"
        "%3$sEnvironment Commands:%4$s\n"
        "  show-environment                    Dump environment\n"
        "  set-environment VARIABLE=VALUE...   Set one or more environment "
        "variables\n"
        "  unset-environment VARIABLE...       Unset one or more environment "
        "variables\n"
        ...
        "  is-system-running                   Check whether system is fully "
        "running\n"
        "  default                             Enter system default mode\n"
        "  rescue                              Enter system rescue mode\n"
        "  emergency                           Enter system emergency mode\n"
        "  halt                                Shut down and halt the system\n"
        "  poweroff                            Shut down and power-off the "
        "system\n"
        "  reboot [ARG]                        Shut down and reboot the "
        "system\n"
        "  kexec                               Shut down and reboot the system "
        "with kexec\n"
        "  exit [EXIT_CODE]                    Request user instance or "
        "container exit\n"
        "  switch-root ROOT [INIT]             Change to a different root file "
        "system\n"
        "  suspend                             Suspend the system\n"
        "  hibernate                           Hibernate the system\n"
        "  hybrid-sleep                        Hibernate and suspend the "
        "system\n"
        "  suspend-then-hibernate              Suspend the system, wake after "
        "a period of\n"
        "                                      time and put it into hibernate\n"
        "\nSee the %2$s for details.\n",
        program_invocation_short_name, link, ansi_underline(), ansi_normal());
...
static int halt_help(void)
{
    ...
    printf(
        ...
        arg_action == ACTION_REBOOT
            ? "Reboot"
            : arg_action == ACTION_POWEROFF ? "Power off" : "Halt",
        link);
...
static void help_states(void)
{
    ...
    DUMP_STRING_TABLE(unit_active_state, UnitActiveState,
...
static int help_boot_loader_entry(void)
{
    ...
    r = sd_bus_get_property_strv(
        bus, "org.freedesktop.login1", "/org/freedesktop/login1",
        "org.freedesktop.login1.Manager", "BootLoaderEntries", &error, &l);
    if (r < 0)
        return log_error_errno(r, "Failed to enumerate boot loader entries: %s",
                               bus_error_message(&error, r));
...
static int systemctl_parse_argv(int argc, char* argv[])
{
    ...
    /* we default to allowing interactive authorization only in systemctl (not
     * in the legacy commands) */
    arg_ask_password = true;

    while ((c = getopt_long(argc, argv, "ht:p:alqfs:H:M:n:o:iTr.::", options,
                            NULL)) >= 0)
        ...
        switch (c)
        {
            ...
            case 't':
            {
                ...
                for (p = optarg;;)
                {
                    ...
                    if (r < 0)
                        return log_error_errno(r, "Failed to parse type: %s",
                                               optarg);
                    if (r == 0)
                        break;
            ...
            case 'p':
                /* Make sure that if the empty property list was specified, we
                   won't show any properties. */
                ...
                else
                {
                    ...
                    for (p = optarg;;)
                    {
                        ...
                        if (r < 0)
                            return log_error_errno(
                                r, "Failed to parse property: %s", optarg);
                        if (r == 0)
                            break;
            ...
            case 'o':
                if (streq(optarg, "help"))
                {
                    DUMP_STRING_TABLE(output_mode, OutputMode,
                                      _OUTPUT_MODE_MAX);
                    return 0;
                }
            ...
            case ARG_BOOT_LOADER_MENU:
                ...
                if (r < 0)
                    return log_error_errno(
                        r,
                        "Failed to parse --boot-loader-menu= argument '%s': %m",
                        optarg);
            ...
            case ARG_BOOT_LOADER_ENTRY:
                ...
                if (streq(optarg, "help"))
                { /* Yes, this means, "help" is not a valid boot loader entry
                     name we can deal with */
            ...
            case ARG_STATE:
            {
                ...
                for (p = optarg;;)
                {
                    ...
                    if (r < 0)
                        return log_error_errno(r, "Failed to parse state: %s",
                                               optarg);
                    if (r == 0)
                        break;
            ...
            case ARG_PRESET_MODE:
                if (streq(optarg, "help"))
                {
                    DUMP_STRING_TABLE(unit_file_preset_mode, UnitFilePresetMode,
...
static int halt_parse_argv(int argc, char* argv[])
{
    ...
    {
        r = update_reboot_parameter_and_warn(
            argc == optind + 1 ? argv[optind] : NULL, false);
        if (r < 0)
            return r;
    }
    else if (optind < argc)
...
static int shutdown_parse_argv(int argc, char* argv[])
{
    ...
    while ((c = getopt_long(argc, argv, "HPrhkKat:fFc", options, NULL)) >= 0)
        switch (c)
        {
            ...
            case 'a':
            case 't': /* Note that we also ignore any passed argument to -t, not
                         just the -t itself */
            case 'f':
            case 'F':
                /* Compatibility nops */
...
static int telinit_parse_argv(int argc, char* argv[])
{
    ...
    if (i >= ELEMENTSOF(table))
        return log_error_errno(SYNTHETIC_ERRNO(EINVAL), "Unknown command '%s'.",
                               argv[optind]);
...
static int parse_argv(int argc, char* argv[])
{
    ...
    if (program_invocation_short_name)
    {
        ...
        else if (strstr(program_invocation_short_name, "init"))
        {
            ...
            /* Matches invocations as "init" as well as "telinit", which are
             * synonymous when run as PID != 1 on SysV.
             *
             * On SysV "telinit" was the official command to communicate with
             * PID 1, but "init" would redirect itself to "telinit" if called
             * with PID != 1. We follow the same logic here still, though we add
             * one level of indirection, as we implement "telinit" in
             * "systemctl". Hence, for us if you invoke "init" you get
             * "systemd", but it will execve() "systemctl" immediately with
             * argv[] unmodified if PID is != 1. If you invoke "telinit" you
             * directly get "systemctl". In both cases we shall do the same
             * thing, which is why we do strstr(p_i_s_n, "init") here, as a
            ...
            {
                /* Hmm, so some other init system is running, we need to forward
                 * this request to it. For now we simply guess that it is
                 * Upstart. */
                ...
                return log_error_errno(SYNTHETIC_ERRNO(EIO),
                                       "Couldn't find an alternative telinit "
                                       "implementation to spawn.");
            }
        }
...
_pure_ static int action_to_runlevel(void)
{
    static const char table[_ACTION_MAX] = {
        [ACTION_HALT] = '0',      [ACTION_POWEROFF] = '0',
        [ACTION_REBOOT] = '6',    [ACTION_RUNLEVEL2] = '2',
        [ACTION_RUNLEVEL3] = '3', [ACTION_RUNLEVEL4] = '4',
...
static int systemctl_main(int argc, char* argv[])
{
    static const Verb verbs[] = {
        {"list-units", VERB_ANY, VERB_ANY, VERB_DEFAULT | VERB_ONLINE_ONLY,
         list_units},
        ...
        {"list-machines", VERB_ANY, VERB_ANY,
         VERB_ONLINE_ONLY | VERB_MUST_BE_ROOT, list_machines},
        ...
        {"condstop", 2, VERB_ANY, VERB_ONLINE_ONLY,
        ...
        {"reload-or-try-restart", 2, VERB_ANY, VERB_ONLINE_ONLY,
         start_unit}, /* For compatbility with old systemctl <= 228 */
        {"try-reload-or-restart", 2, VERB_ANY, VERB_ONLINE_ONLY, start_unit},
        {"force-reload", 2, VERB_ANY, VERB_ONLINE_ONLY,
         start_unit}, /* For compatibility with SysV */
        {"condreload", 2, VERB_ANY, VERB_ONLINE_ONLY,
         start_unit}, /* For compatibility with ALTLinux */
        {"condrestart", 2, VERB_ANY, VERB_ONLINE_ONLY,
        ...
        {"check", 2, VERB_ANY, VERB_ONLINE_ONLY,
         check_unit_active}, /* deprecated alias of is-active */
        ...
        {"import-environment", VERB_ANY, VERB_ANY, VERB_ONLINE_ONLY,
         import_environment},
        ...
        {"suspend-then-hibernate", VERB_ANY, 1, VERB_ONLINE_ONLY,
...
static int halt_now(enum action a)
{
    /* The kernel will automatically flush ATA disks and suchlike on reboot(),
     * but the file systems need to be synced explicitly in advance. */
...
static int logind_schedule_shutdown(void)
{
    ...
    r = sd_bus_call_method(bus, "org.freedesktop.login1",
                           ...
                           &error, NULL, "st", action, arg_when);
    if (r < 0)
        return log_warning_errno(r,
                                 "Failed to call ScheduleShutdown in logind, "
                                 "proceeding with immediate shutdown: %s",
                                 bus_error_message(&error, r));
...
#else
    return log_error_errno(SYNTHETIC_ERRNO(ENOSYS),
                           "Cannot schedule shutdown without logind support, "
...
static int halt_main(void)
{
    ...
    {
        if (sd_booted() > 0)
            log_debug("Not writing utmp record, assuming that "
                      "systemd-update-utmp is used.");
        else
        {
            r = utmp_put_shutdown();
            if (r < 0)
...
static int logind_cancel_shutdown(void)
{
    ...
    r = sd_bus_call_method(bus, "org.freedesktop.login1",
                           "/org/freedesktop/login1",
                           "org.freedesktop.login1.Manager",
                           "CancelScheduledShutdown", &error, NULL, NULL);
    if (r < 0)
        return log_warning_errno(
            r, "Failed to talk to logind, shutdown hasn't been cancelled: %s",
            bus_error_message(&error, r));
...
static int run(int argc, char* argv[])
{
    ...
    switch (arg_action)
    {
        ...
        case ACTION_SYSTEMCTL:
            ...
            /* Legacy command aliases set arg_action. They provide some
             * fallbacks, e.g. to tell sysvinit to reboot after you have
             * installed systemd binaries. */
...
finish:
    ...
    /* Note that we return r here, not 0, so that we can implement the LSB-like
     * return codes */
    return r;
}
