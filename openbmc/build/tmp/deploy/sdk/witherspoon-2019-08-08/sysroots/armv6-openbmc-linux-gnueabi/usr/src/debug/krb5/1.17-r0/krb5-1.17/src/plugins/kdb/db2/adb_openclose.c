...
krb5_error_code osa_adb_create_db(char* filename, char* lockfilename, int magic)
{
    int lf;
    DB* db;
    BTREEINFO btinfo;
                                   ...
                                   int magic)
{
    ...
    if (unlink(filename) < 0 || unlink(lockfilename) < 0)
        return errno;
    return OSA_ADB_OK;
}
...
krb5_error_code osa_adb_get_lock(osa_adb_db_t db, int mode)
{
    ...
    {
        (void)krb5_lock_file(db->lock->context, fileno(db->lock->lockfile),
