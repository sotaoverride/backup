...
/* for our current LRU mechanism, we need a monotonically increasing counters.
 * We use it much like a "Lamport logical clock": we do not need the actual
 * time, we just need to know the sequence in which files were accessed. So we
 * use a simple counter to create that sequence. We use an unsigned 64 bit value
 * which is extremely unlike to wrap within the lifetime of a process. If we
 * process 1,000,000 file writes per second, the process could still exist over
 * 500,000 years before a wrap to 0 happens. That should be sufficient (and even
 * than, there would no really bad effect ;)). The variable below is the global
 * counter/clock.
 */
...
{
    pthread_mutex_t
        mutWrite; /* guard against multiple instances writing to single file */
    ...
    uchar*
        pszSizeLimitCmd; /* command to carry out when size limit is reached */
    int iZipLevel;       /* zip mode to use for this selector */
    int iIOBufSize;      /* size of associated io buffer */
    int iFlushInterval;  /* how fast flush buffer on inactivity? */
    short iCloseTimeout; /* after how many *minutes* shall the file be closed if
                            inactive? */
...
typedef struct configSettings_s
{
    ...
    int bCreateDirs; /* auto-create directories for dynaFiles: 0 - no, 1 - yes
                      */
    int bEnableSync; /* enable syncing of files (no dash in front of pathname in
                        conf): 0 - no, 1 - yes */
...
static modConfData_t* loadModConf =
    NULL; /* modConf ptr to use for the current load process */
static modConfData_t* runModConf =
    NULL; /* modConf ptr to use for the current exec process */
...
static struct cnfparamblk modpblk = {
    CNFPARAMBLK_VERSION, sizeof(modpdescr) / sizeof(struct cnfparamdescr),
    modpdescr};
...
static struct cnfparamblk actpblk = {
    CNFPARAMBLK_VERSION, sizeof(actpdescr) / sizeof(struct cnfparamdescr),
    actpdescr};
...
BEGINinitConfVars /* (re)set config variables to default values */
    CODESTARTinitConfVars pszFileDfltTplName =
        NULL;                            /* make sure this can be free'ed! */
iRet = resetConfigVariables(NULL, NULL); /* params are dummies */
ENDinitConfVars

    BEGINisCompatibleWithFeature
        CODESTARTisCompatibleWithFeature if (eFeat ==
                                             sFEATURERepeatedMsgReduction)
            iRet = RS_RET_OK;
ENDisCompatibleWithFeature

    BEGINdbgPrintInstInfo CODESTARTdbgPrintInstInfo if (pData->bDynamicName)
{
    dbgprintf("[dynamic]\n");
}
else
...
ENDdbgPrintInstInfo
     ...
     * This is a module-global parameter, and as such needs special handling. It
     * needs to be coordinated with values set via the v2 config system (rsyslog
     * v6+). What we do is we do not permit this directive after the v2 config
     * system has been used to set the parameter.
     */
    static rsRetVal
 ...
 */
static rsRetVal setDynaFileCacheSize(void __attribute__((unused)) * pVal,
                                     int iNewVal)
{
    DEFiRet;
    ...
    if (iNewVal < 1)
    {
        errno = 0;
        parser_errmsg(
            "DynaFileCacheSize must be greater 0 (%d given), changed to 1.",
            iNewVal);
    ...
    else if (iNewVal > 1000)
    {
        errno = 0;
        parser_errmsg(
            "DynaFileCacheSize maximum is 1,000 (%d given), changed to 1,000.",
            iNewVal);
        iRet = RS_RET_VAL_OUT_OF_RANGE;
        iNewVal = 1000;
    }
 ...
 */
static rsRetVal cflineParseOutchannel(instanceData* pData, uchar* p,
                                      omodStringRequest_t* pOMSR, int iEntry,
                                      int iTplOpts)
{
    DEFiRet;
    size_t i;
    ...
    if (pOch == NULL)
    {
        parser_errmsg("outchannel '%s' not found - ignoring action line",
                      szBuf);
 ...
 */
static rsRetVal dynaFileDelCacheEntry(instanceData* __restrict__ const pData,
                                      const int iEntry, const int bFreeEntry)
{
    ...
    DBGPRINTF("Removing entry %d for file '%s' from dynaCache.\n", iEntry,
              pCache[iEntry]->pName == NULL ? UCHAR_CONSTANT("[OPEN FAILED]")
                                            : pCache[iEntry]->pName);
...
static rsRetVal sigprovPrepare(instanceData* __restrict__ const pData,
                               uchar* __restrict__ const fn)
{
    DEFiRet;
                            ...
                            const uchar* __restrict__ const newFileName)
{
    ...
    if (access((char*)newFileName, F_OK) != 0)
    {
        ...
        /* no matter if we needed to create directories or not, we now try to
         * create the file. -- rgerhards, 2008-12-18 (based on patch from
         * William Tisater)
         */
     ...
     * anomalies in dirname() and basename() [they MODIFY the provided
     * buffer...]
     */
    ...
    /* set the flush interval only if we actually use it - otherwise it will
     * activate async processing, which is a real performance waste if we do not
     * do buffered writes! -- rgerhards, 2009-07-06
     */
    if (pData->bUseAsyncWriter)
        CHKiRet(strm.SetiFlushInterval(pData->pStrm, pData->iFlushInterval));
    if (pData->pszSizeLimitCmd != NULL)
        CHKiRet(strm.SetpszSizeLimitCmd(pData->pStrm,
                                        ustrdup(pData->pszSizeLimitCmd)));
 ...
 */
static rsRetVal prepareDynFile(instanceData* __restrict__ const pData,
                               const uchar* __restrict__ const newFileName)
{
        ...
        !ustrcmp(newFileName, pCache[pData->iCurrElt]->pName))
    {
        ...
        /* LRU needs only a strictly monotonically increasing counter, so such a
         * one could do */
        FINALIZE;
    }
    ...
    iOldest = 0; /* we assume the first element to be the oldest - that will
                    change as we loop */
    ...
    for (i = 0; i < pData->iCurrCacheSize; ++i)
    {
        ...
        { /* got an element, let's see if it matches */
            if (!ustrcmp(newFileName, pCache[i]->pName))
            {
                ...
                pData->iCurrElt = i;
                pCache[i]->clkTickAccessed =
                    getClockFileAccess(); /* update "timestamp" for LRU */
                FINALIZE;
            }
    ...
    /* similarly, we need to set the current pStrm to NULL, because otherwise,
     * if prepareFile() fails, we may end up using an old stream. This bug
     * depends on how exactly prepareFile fails, but it could be triggered in
     * the common case of a failed open() system call. rgerhards, 2010-03-22
     */
    ...
    if (iFirstFree == -1 && (pData->iCurrCacheSize < pData->iDynaFileCacheSize))
    {
        ...
        STATSCOUNTER_SETMAX_NOMUT(pData->ctrMax,
                                  (unsigned)pData->iCurrCacheSize);
    }

    /* Note that the following code sequence does not work with the cache entry
     * itself, but rather with pData->pStrm, the (sole) stream pointer in the
     * non-dynafile case. The cache array is only updated after the open was
     * successful. -- rgerhards, 2010-03-21
     */
    if (iFirstFree == -1)
    {
    ...
    else
    {
        ...
        CHKmalloc(pCache[iFirstFree] = (dynaFileCacheEntry*)calloc(
                      1, sizeof(dynaFileCacheEntry)));
    }

    /* Ok, we finally can open the file */
    localRet = prepareFile(
        pData, newFileName); /* ignore exact error, we check fd below */
...
/* do the actual write process. This function is to be called once we are ready
 * for writing. It will do buffered writes and persist data only when the buffer
 * is full. Note that we must be careful to detect when the file handle changed.
                        ...
                        uchar* __restrict__ const pszBuf, const int lenBuf)
{
    ...
    DBGPRINTF("omfile: write to stream, pData->pStrm %p, lenBuf %d, strt data "
              "%.128s\n",
    ...
    if (pData->pStrm != NULL)
    {
        ...
        if (pData->useSigprov)
        {
            CHKiRet(pData->sigprov.OnRecordWrite(pData->sigprovFileData, pszBuf,
                                                 lenBuf));
        }
    }
                          ...
                          const int iMsg)
{
    ...
    if (pData->bDynamicName)
    {
        ...
        CHKiRet(prepareDynFile(
            pData, actParam(pParam, pData->iNumTpls, iMsg, 1).param));
    }
    else
    { /* "regular", non-dynafile */
        if (pData->pStrm == NULL)
        {
...
BEGINbeginCnfLoad CODESTARTbeginCnfLoad loadModConf = pModConf;
...
ENDbeginCnfLoad
    ...
    BEGINsetModCnf struct cnfparamvals* pvals = NULL;
int i;
CODESTARTsetModCnf pvals = nvlstGetParams(lst, &modpblk, NULL);
if (pvals == NULL)
{
...
finalize_it : if (pvals != NULL) cnfparamvalsDestruct(pvals, &modpblk);
ENDsetModCnf
    ...
    janitorChkDynaFiles(instanceData* __restrict__ const pData)
{
    ...
    for (i = 0; i < pData->iCurrCacheSize; ++i)
    {
        if (pCache[i] == NULL)
            continue;
        DBGPRINTF(
            "omfile janitor: checking dynafile %d:%s, inactive since %d\n", i,
            pCache[i]->pName == NULL ? UCHAR_CONSTANT("[OPEN FAILED]")
                                     : pCache[i]->pName,
            (int)pCache[i]->nInactive);
        if (pCache[i]->nInactive >= pData->iCloseTimeout)
        {
            STATSCOUNTER_INC(pData->ctrCloseTimeouts,
                             pData->mutCtrCloseTimeouts);
            dynaFileDelCacheEntry(pData, i, 1);
            if (pData->iCurrElt == i)
...
BEGINendCnfLoad CODESTARTendCnfLoad loadModConf = NULL; /* done loading */
/* free legacy config vars */
free(pszFileDfltTplName);
pszFileDfltTplName = NULL;
ENDendCnfLoad

    BEGINcheckCnf CODESTARTcheckCnf ENDcheckCnf

        BEGINactivateCnf CODESTARTactivateCnf runModConf = pModConf;
ENDactivateCnf

    BEGINfreeCnf CODESTARTfreeCnf free(pModConf->tplName);
ENDfreeCnf

    BEGINcreateInstance CODESTARTcreateInstance pData->pStrm = NULL;
pthread_mutex_init(&pData->mutWrite, NULL);
ENDcreateInstance

    BEGINcreateWrkrInstance CODESTARTcreateWrkrInstance ENDcreateWrkrInstance

        BEGINfreeInstance CODESTARTfreeInstance free(pData->tplName);
free(pData->fname);
...
ENDfreeInstance
    ...
    BEGINfreeWrkrInstance CODESTARTfreeWrkrInstance ENDfreeWrkrInstance
        ...
        BEGINtryResume CODESTARTtryResume ENDtryResume

            BEGINbeginTransaction CODESTARTbeginTransaction
                /* we have nothing to do to begin a transaction */
                ENDbeginTransaction

                    BEGINcommitTransaction
                        instanceData* __restrict__ const pData =
                            pWrkrData->pData;
unsigned i;
CODESTARTcommitTransaction pthread_mutex_lock(&pData->mutWrite);
...
finalize_it : pthread_mutex_unlock(&pData->mutWrite);
...
static rsRetVal setupInstStatsCtrs(instanceData* __restrict__ const pData)
{
    ...
    snprintf((char*)ctrName, sizeof(ctrName), "dynafile cache %s",
             pData->fname);
...
static void initSigprov(instanceData* __restrict__ const pData,
                        struct nvlst* lst)
{
    uchar szDrvrName[1024];

    if (snprintf((char*)szDrvrName, sizeof(szDrvrName), "lmsig_%s",
                 pData->sigprovName) == sizeof(szDrvrName))
    {
...
static rsRetVal initCryprov(instanceData* __restrict__ const pData,
                            struct nvlst* lst)
{
    uchar szDrvrName[1024];
    DEFiRet;
    ...
    if (snprintf((char*)szDrvrName, sizeof(szDrvrName), "lmcry_%s",
                 pData->cryprovName) == sizeof(szDrvrName))
    {
...
uchar* tplToUse;
int i;
CODESTARTnewActInst DBGPRINTF("newActInst (omfile)\n");
...
for (i = 0; i < actpblk.nParams; ++i)
{
    ...
    else if (!strcmp(actpblk.descr[i].name, "file"))
    {
        ...
        CODE_STD_STRING_REQUESTnewActInst(1) pData->bDynamicName = 0;
    }
    else if (!strcmp(actpblk.descr[i].name, "dynafile"))
    {
        if (pData->fname != NULL)
        {
            parser_errmsg("omfile: both \"file\" and \"dynafile\" set, will "
                          "use dynafile");
        }
        pData->fname = (uchar*)es_str2cstr(pvals[i].val.d.estr, NULL);
        CODE_STD_STRING_REQUESTnewActInst(2) pData->bDynamicName = 1;
    }
...
if (pData->bDynamicName)
{
    /* "filename" is actually a template name, we need this as string 1. So
     * let's add it to the pOMSR. -- rgerhards, 2007-07-27
     */
    CHKiRet(
        OMSRsetEntry(*ppOMSR, 1, ustrdup(pData->fname), OMSR_NO_RQD_TPL_OPTS));
    pData->iNumTpls = 2;
    // TODO: create unified code for this (legacy+v6 system)
    /* we now allocate the cache table */
    CHKmalloc(pData->dynCache = (dynaFileCacheEntry**)calloc(
                  pData->iDynaFileCacheSize, sizeof(dynaFileCacheEntry*)));
    pData->iCurrElt = -1; /* no current element */
}
// TODO: add	pData->iSizeLimit = 0; /* default value, use outchannels to
// configure! */
setupInstStatsCtrs(pData);

if (pData->iCloseTimeout == -1)
{ /* unset? */
...
ENDnewActInst
    ...
    BEGINparseSelectorAct uchar fname[MAXFNAME];
...
switch (*p)
{
    case '$':
         ...
         */
        CHKiRet(
            cflineParseOutchannel(pData, p, *ppOMSR, 0, OMSR_NO_RQD_TPL_OPTS));
    ...
    case '?': /* This is much like a regular file handle, but we need to obtain
         ...
         * let's add it to the pOMSR. -- rgerhards, 2007-07-27
         */
        CHKiRet(OMSRsetEntry(*ppOMSR, 1, ustrdup(pData->fname),
                             OMSR_NO_RQD_TPL_OPTS));
        /* we now allocate the cache table */
        CHKmalloc(pData->dynCache = (dynaFileCacheEntry**)calloc(
                      cs.iDynaFileCacheSize, sizeof(dynaFileCacheEntry*)));
        break;
...
BEGINdoHUP CODESTARTdoHUP pthread_mutex_lock(&pData->mutWrite);
if (pData->bDynamicName)
{
    dynaFileFreeCacheEntries(pData);
}
...
ENDdoHUP
    ...
    BEGINmodExit CODESTARTmodExit objRelease(strm, CORE_COMPONENT);
objRelease(statsobj, CORE_COMPONENT);
DESTROY_ATOMIC_HELPER_MUT(mutClock);
ENDmodExit

    BEGINqueryEtryPt CODESTARTqueryEtryPt CODEqueryEtryPt_STD_OMODTX_QUERIES
        CODEqueryEtryPt_STD_OMOD8_QUERIES CODEqueryEtryPt_STD_CONF2_QUERIES
            CODEqueryEtryPt_STD_CONF2_setModCnf_QUERIES
                CODEqueryEtryPt_STD_CONF2_OMOD_QUERIES CODEqueryEtryPt_doHUP
                    ENDqueryEtryPt

                        BEGINmodInit(File) CODESTARTmodInit* ipIFVersProvided =
                            CURR_MOD_IF_VERSION; /* we only support the current
                                                    interface specification */
CODEmodInit_QueryRegCFSLineHdlr INITLegCnfVars CHKiRet(objUse(strm,
                                                              CORE_COMPONENT));
...
CHKiRet(omsdRegCFSLineHdlr((uchar*)"dynafilecachesize", 0, eCmdHdlrInt,
                           setDynaFileCacheSize, NULL, STD_LOADABLE_MODULE_ID));
CHKiRet(omsdRegCFSLineHdlr((uchar*)"omfileziplevel", 0, eCmdHdlrInt, NULL,
                           &cs.iZipLevel, STD_LOADABLE_MODULE_ID));
CHKiRet(omsdRegCFSLineHdlr((uchar*)"omfileflushinterval", 0, eCmdHdlrInt, NULL,
                           &cs.iFlushInterval, STD_LOADABLE_MODULE_ID));
CHKiRet(omsdRegCFSLineHdlr((uchar*)"omfileasyncwriting", 0, eCmdHdlrBinary,
                           NULL, &cs.bUseAsyncWriter, STD_LOADABLE_MODULE_ID));
CHKiRet(omsdRegCFSLineHdlr((uchar*)"omfileflushontxend", 0, eCmdHdlrBinary,
                           NULL, &cs.bFlushOnTXEnd, STD_LOADABLE_MODULE_ID));
CHKiRet(omsdRegCFSLineHdlr((uchar*)"omfileiobuffersize", 0, eCmdHdlrSize, NULL,
                           &cs.iIOBufSize, STD_LOADABLE_MODULE_ID));
CHKiRet(omsdRegCFSLineHdlr((uchar*)"dirowner", 0, eCmdHdlrUID, NULL, &cs.dirUID,
                           STD_LOADABLE_MODULE_ID));
CHKiRet(omsdRegCFSLineHdlr((uchar*)"dirownernum", 0, eCmdHdlrInt, NULL,
                           &cs.dirUID, STD_LOADABLE_MODULE_ID));
CHKiRet(omsdRegCFSLineHdlr((uchar*)"dirgroup", 0, eCmdHdlrGID, NULL, &cs.dirGID,
                           STD_LOADABLE_MODULE_ID));
CHKiRet(omsdRegCFSLineHdlr((uchar*)"dirgroupnum", 0, eCmdHdlrInt, NULL,
                           &cs.dirGID, STD_LOADABLE_MODULE_ID));
CHKiRet(omsdRegCFSLineHdlr((uchar*)"fileowner", 0, eCmdHdlrUID, NULL,
                           &cs.fileUID, STD_LOADABLE_MODULE_ID));
CHKiRet(omsdRegCFSLineHdlr((uchar*)"fileownernum", 0, eCmdHdlrInt, NULL,
                           &cs.fileUID, STD_LOADABLE_MODULE_ID));
CHKiRet(omsdRegCFSLineHdlr((uchar*)"filegroup", 0, eCmdHdlrGID, NULL,
                           &cs.fileGID, STD_LOADABLE_MODULE_ID));
CHKiRet(omsdRegCFSLineHdlr((uchar*)"filegroupnum", 0, eCmdHdlrInt, NULL,
                           &cs.fileGID, STD_LOADABLE_MODULE_ID));
CHKiRet(omsdRegCFSLineHdlr((uchar*)"dircreatemode", 0, eCmdHdlrFileCreateMode,
                           NULL, &cs.fDirCreateMode, STD_LOADABLE_MODULE_ID));
CHKiRet(omsdRegCFSLineHdlr((uchar*)"filecreatemode", 0, eCmdHdlrFileCreateMode,
                           NULL, &cs.fCreateMode, STD_LOADABLE_MODULE_ID));
CHKiRet(omsdRegCFSLineHdlr((uchar*)"createdirs", 0, eCmdHdlrBinary, NULL,
                           &cs.bCreateDirs, STD_LOADABLE_MODULE_ID));
CHKiRet(omsdRegCFSLineHdlr((uchar*)"failonchownfailure", 0, eCmdHdlrBinary,
                           NULL, &cs.bFailOnChown, STD_LOADABLE_MODULE_ID));
CHKiRet(omsdRegCFSLineHdlr((uchar*)"omfileforcechown", 0, eCmdHdlrGoneAway,
                           NULL, NULL, STD_LOADABLE_MODULE_ID));
CHKiRet(omsdRegCFSLineHdlr((uchar*)"actionfileenablesync", 0, eCmdHdlrBinary,
                           NULL, &cs.bEnableSync, STD_LOADABLE_MODULE_ID));
CHKiRet(omsdRegCFSLineHdlr((uchar*)"actionfiledefaulttemplate", 0,
                           eCmdHdlrGetWord, setLegacyDfltTpl, NULL,
                           STD_LOADABLE_MODULE_ID));
