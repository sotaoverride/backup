...
#define MSG_OUT(f_, ...)                                                       \
    do                                                                         \
    {                                                                          \
        if (verbosity != BT_LOG_NONE)                                          \
        {                                                                      \
            bt_log(LOG_INFO, f_, ##__VA_ARGS__);                               \
        }                                                                      \
    } while (0)
#define MSG_ERR(f_, ...)                                                       \
    do                                                                         \
...
static enum { BT_LOG_NONE = 0, BT_LOG_VERBOSE, BT_LOG_DEBUG } verbosity;
...
__attribute__((format(printf, 2, 3))) static void bt_log(int p, const char* fmt,
                                                         ...)
{
    va_list args;
...
static struct bt_queue* bt_q_get_seq(struct btbridged_context* context,
                                     uint8_t seq)
{
    struct bt_queue* t;
...
static struct bt_queue* bt_q_enqueue(struct btbridged_context* context,
                                     uint8_t* bt_data)
{
    struct bt_queue* n;
    struct bt_queue* bt_q;
...
static struct bt_queue* bt_q_drop(struct btbridged_context* context,
                                  struct bt_queue* element)
{
    struct bt_queue* r;
                               ...
                               sd_bus_error* ret_error)
{
    ...
    if (!context)
    {
        sd_bus_error_set_const(ret_error, "org.openbmc.error",
                               "Internal error");
        r = 0;
        goto out;
    }
    ...
    if (!bt_msg)
    {
        sd_bus_error_set_const(ret_error, "org.openbmc.error",
                               "No matching request");
        MSG_ERR("Failed to find matching request for dbus method with seq: "
                "0x%02x\n",
                seq);
        r = -EINVAL;
        goto out;
...
static int bt_host_write(struct btbridged_context* context,
                         struct bt_queue* bt_msg)
{
    ...
    if (bt_msg == head)
    {
        ...
        r = timerfd_settime(context->fds[TIMER_FD].fd, TFD_TIMER_ABSTIME, &ts,
                            NULL);
        if (r == -1)
    ...
    {
        if (len != data[0] + 1)
            MSG_ERR("Possible short write to %s, desired len: %d, written len: "
                    "%d\n",
                    BT_BMC_PATH, data[0] + 1, len);
        else
            MSG_OUT("Successfully wrote %d of %d bytes to %s\n", len,
                    data[0] + 1, BT_BMC_PATH);
...
static int dispatch_timer(struct btbridged_context* context)
{
    ...
    if (context->fds[TIMER_FD].revents & POLLIN)
    {
        ...
        if (head->call)
        {
            r = sd_bus_message_new_method_errno(head->call, &msg, ETIMEDOUT,
                                                NULL);
            if (r < 0)
            {
                MSG_ERR("Couldn't create response error\n");
...
static int dispatch_bt(struct btbridged_context* context)
{
    ...
    if (context->fds[BT_FD].revents & POLLIN)
    {
        ...
        r = sd_bus_message_append(msg, "yyyy", new->req.seq, new->req.netfn,
                                  new->req.lun, new->req.cmd);
        if (r < 0)
        {
        ...
        MSG_OUT("Sending dbus signal with seq 0x%02x, netfn 0x%02x, lun "
                "0x%02x, cmd 0x%02x\n",
        ...
        /* Note we only actually keep the request data in the queue when
         * debugging */
    ...
    if (context->fds[BT_FD].revents & POLLOUT)
    {
        ...
        if (r < 0)
            MSG_ERR("Problem putting request with seq 0x%02x, netfn 0x%02x, "
                    "lun 0x%02x, cmd 0x%02x, cc 0x%02x\n"
                    "out to %s",
                    bt_msg->rsp.seq, bt_msg->rsp.netfn, bt_msg->rsp.lun,
                    bt_msg->rsp.cmd, bt_msg->rsp.cc, BT_BMC_PATH);
        else
            MSG_OUT("Completed request with seq 0x%02x, netfn 0x%02x, lun "
                    "0x%02x, cmd 0x%02x, cc 0x%02x\n",
                    bt_msg->rsp.seq, bt_msg->rsp.netfn, bt_msg->rsp.lun,
...
static const sd_bus_vtable ipmid_vtable[] = {
    ...
    SD_BUS_METHOD("setAttention", "", "x", &method_send_sms_atn,
                  SD_BUS_VTABLE_UNPRIVILEGED),
    SD_BUS_SIGNAL("ReceivedMessage", "yyyyay", 0), SD_BUS_VTABLE_END};
