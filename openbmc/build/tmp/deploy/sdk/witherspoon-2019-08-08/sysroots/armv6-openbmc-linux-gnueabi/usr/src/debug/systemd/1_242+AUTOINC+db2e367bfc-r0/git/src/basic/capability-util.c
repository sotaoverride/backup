...
unsigned long cap_last_cap(void)
{
    ...
    if (r >= 0)
    {
        ...
        if (r >= 0)
        {
            ...
            if (p >
                63) /* Safety for the future: if one day the kernel learns more
                     * than 64 caps, then we are in trouble (since we, as much
                     * userspace and kernel space store capability masks in
                     * uint64_t types). Let's hence protect ourselves against
                     * that and always cap at 63 for now. */
                p = 63;
...
int capability_bounding_set_drop(uint64_t keep, bool right_now)
{
    ...
    if (fv != CAP_SET)
    {
        ...
        if (cap_set_proc(temp_cap) < 0)
            log_debug_errno(
                errno, "Can't acquire effective CAP_SETPCAP bit, ignoring: %m");

        /* If we didn't manage to acquire the CAP_SETPCAP bit, we continue
         * anyway, after all this just means
    ...
    for (i = 0; i <= cap_last_cap(); i++)
    {
        ...
        if (prctl(PR_CAPBSET_DROP, i) < 0)
        {
            ...
            /* If dropping the capability failed, let's see if we didn't have it
             * in the first place. If so, continue anyway, as dropping a
             * capability we didn't have in the first place doesn't really
             * matter anyway. */
...
int drop_privileges(uid_t uid, gid_t gid, uint64_t keep_capabilities)
{
    ...
    /* Unfortunately we cannot leave privilege dropping to PID 1 here, since we
     * want to run as user but want to keep some capabilities. Since file
     * capabilities have been introduced this cannot be done across exec()
     * anymore, unless our binary has the capability configured in the file
    ...
    /* Ensure we keep the permitted caps across the setresuid(). Note that we do
     * this even if we actually don't want to keep any capabilities, since we
     * want to be able to drop them from the bounding set too, and we can only
     * do that if we have capabilities. */
    ...
    /* Drop all caps from the bounding set (as well as the
     * inheritable/permitted/effective sets), except the ones we want to keep */
...
bool ambient_capabilities_supported(void)
{
    ...
    /* If PR_CAP_AMBIENT returns something valid, or an unexpected error code we
     * assume that ambient caps are available. */
...
int capability_quintet_enforce(const CapabilityQuintet* q)
{
    ...
    if (q->ambient != (uint64_t)-1)
    {
        ...
        /* In order to raise the ambient caps set we first need to raise the
         * matching inheritable + permitted cap */
        for (i = 0; i <= cap_last_cap(); i++)
        {
            ...
            if (cap_get_flag(c, cv, CAP_INHERITABLE, &old_value_inheritable) <
                0)
                return -errno;
            if (cap_get_flag(c, cv, CAP_PERMITTED, &old_value_permitted) < 0)
                return -errno;

            if (old_value_inheritable == CAP_SET &&
                old_value_permitted == CAP_SET)
                continue;
    ...
    if (q->inheritable != (uint64_t)-1 || q->permitted != (uint64_t)-1 ||
        q->effective != (uint64_t)-1)
    {
        bool changed = false;
        unsigned long i;
        ...
        for (i = 0; i <= cap_last_cap(); i++)
        {
            ...
            if (q->inheritable != (uint64_t)-1)
            {
                ...
                {
                    if (errno ==
                        EINVAL) /* If the kernel knows more caps than this
        ...
        if (changed)
        {
            /* In order to change the bounding caps, we need to keep CAP_SETPCAP
             * for a bit longer. Let's add it to our list hence for now. */
            ...
            /* Now, let's enforce the caps for the first time. Note that this is
             * where we acquire caps in any of the sets we currently don't have.
             * We have to do this before dropping the bounding caps below, since
             * at that point we can never acquire new caps in
             * inherited/permitted/effective anymore, but only lose them. */
    ...
    /* If needed, let's now set the caps again, this time in the final version,
     * which differs from what we have already set only in the CAP_SETPCAP bit,
     * which we needed for dropping the bounding bits. This call only undoes
     * bits and doesn't acquire any which means the bounding caps don't
     * matter. */
    if (modified)
        if (cap_set_proc(c) < 0)
            return -errno;
