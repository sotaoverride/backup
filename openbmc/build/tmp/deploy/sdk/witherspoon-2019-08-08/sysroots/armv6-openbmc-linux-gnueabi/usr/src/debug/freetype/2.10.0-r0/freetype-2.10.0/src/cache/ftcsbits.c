...
static FT_Error ftc_sbit_copy_bitmap(FTC_SBit sbit, FT_Bitmap* bitmap,
                                     FT_Memory memory)
{
    FT_Error error;
 ...
 */
static FT_Error ftc_snode_load(FTC_SNode snode, FTC_Manager manager,
                               FT_UInt gindex, FT_ULong* asize)
{
    FT_Error error;
...
#define CHECK_BYTE(d) (temp = (FT_Byte)d, (FT_UInt)temp == (FT_UInt)d)
        ...
        if (!CHECK_BYTE(bitmap->rows) || !CHECK_BYTE(bitmap->width) ||
            !CHECK_CHAR(bitmap->pitch) || !CHECK_CHAR(slot->bitmap_left) ||
            !CHECK_CHAR(slot->bitmap_top) || !CHECK_CHAR(xadvance) ||
            !CHECK_CHAR(yadvance))
        {
...
FTC_SNode_New(FTC_SNode* psnode, FTC_GQuery gquery, FTC_Cache cache)
{
    ...
    if (!FT_NEW(snode))
    {
        ...
        error = ftc_snode_load(snode, cache->manager, gindex, NULL);
        if (error)
        {
