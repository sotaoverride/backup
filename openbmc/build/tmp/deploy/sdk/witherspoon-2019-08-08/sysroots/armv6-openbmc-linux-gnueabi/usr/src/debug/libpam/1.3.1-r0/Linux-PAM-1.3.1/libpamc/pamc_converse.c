...
int pamc_converse(pamc_handle_t pch, pamc_bp_t* prompt_p)
{
    ...
    if ((*prompt_p)->control == PAM_BPC_SELECT)
    {
        ...
        if ((i >= size) ||
            !__pamc_valid_agent_id(i - PAM_BP_MIN_SIZE, rawh + PAM_BP_MIN_SIZE))
        {
            goto pamc_unknown_prompt;
        }
    ...
    do
    {
        int rval = write(pch->current->writer,
                         offset + (const u_int8_t*)(*prompt_p), size - offset);
        if (rval == -1)
        {
            switch (errno)
            {
    ...
    do
    {
        ...
        rval =
            read(pch->current->reader, raw + offset, PAM_BP_MIN_SIZE - offset);
    ...
    D(("agent replied with prompt of size %d and control %u", size, control));
    ...
    while (offset < size)
    {
        ...
        {
            D(("problem reading prompt (%d) with %d to go", size,
               size - offset));
            goto pamc_converse_failure;
        }
    }
