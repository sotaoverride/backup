...
#include "record_locl.h"
    ...
    !(defined(AES_ASM) && (defined(__x86_64) || defined(__x86_64__) ||         \
                     ...
                     size_t* written)
{
    ...
    if (type == SSL3_RT_APPLICATION_DATA &&
        ...
        s->compress == NULL && s->msg_callback == NULL && !SSL_WRITE_ETM(s) &&
            ...
            EVP_CIPH_FLAG_TLS1_1_MULTIBLOCK)
    {
        ...
        { /* allocate jumbo buffer */
            ...
            packlen = EVP_CIPHER_CTX_ctrl(
                s->enc_write_ctx, EVP_CTRL_TLS1_1_MULTIBLOCK_MAX_BUFSIZE,
    ...
    if (maxpipes == 0 || s->enc_write_ctx == NULL ||
        !(EVP_CIPHER_flags(EVP_CIPHER_CTX_cipher(s->enc_write_ctx)) &
    ...
    for (;;)
    {
        ...
        if (tmpwrit == n || (type == SSL3_RT_APPLICATION_DATA &&
...
int do_ssl3_write(SSL* s, int type, const unsigned char* buf, size_t* pipelens,
                  size_t numpipes, int create_empty_fragment, size_t* written)
{
    ...
    else if (prefix_len)
    {
        ...
        if (!WPACKET_init_static_len(&pkt[0], SSL3_BUFFER_get_buf(wb),
            ...
            !WPACKET_allocate_bytes(
                &pkt[0], SSL3_BUFFER_get_offset(wb) + prefix_len, NULL))
        {
            SSLfatal(s, SSL_AD_INTERNAL_ERROR, SSL_F_DO_SSL3_WRITE,
                     ERR_R_INTERNAL_ERROR);
            goto err;
    ...
    for (j = 0; j < numpipes; j++)
    {
        unsigned int version =
            (s->version == TLS1_3_VERSION) ? TLS1_2_VERSION : s->version;
         ...
         */
        if (SSL_get_state(s) == TLS_ST_CW_CLNT_HELLO && !s->renegotiate &&
        ...
        if (!WPACKET_put_bytes_u8(thispkt, rectype) ||
            ...
            (eivlen > 0 && !WPACKET_allocate_bytes(thispkt, eivlen, NULL)) ||
            (maxcomplen > 0 &&
             !WPACKET_reserve_bytes(thispkt, maxcomplen, &compressdata)))
        {
            SSLfatal(s, SSL_AD_INTERNAL_ERROR, SSL_F_DO_SSL3_WRITE,
                     ERR_R_INTERNAL_ERROR);
            goto err;
             ...
             type != SSL3_RT_ALERT))
        {
            ...
            if (rlen < max_send_fragment)
            {
                ...
                {
                    padding = s->record_padding_cb(s, type, rlen,
         ...
         */
        if (!WPACKET_reserve_bytes(thispkt, SSL_RT_MAX_CIPHER_BLOCK_SIZE, NULL)
            /*
    ...
    for (j = 0; j < numpipes; j++)
    {
        ...
        if (!WPACKET_get_length(thispkt, &origlen)
            ...
            (thiswr->length > origlen &&
             !WPACKET_allocate_bytes(thispkt, thiswr->length - origlen, NULL)))
        {
            SSLfatal(s, SSL_AD_INTERNAL_ERROR, SSL_F_DO_SSL3_WRITE,
                     ERR_R_INTERNAL_ERROR);
            goto err;
        ...
        if (!WPACKET_get_length(thispkt, &len) || !WPACKET_close(thispkt))
        {
            SSLfatal(s, SSL_AD_INTERNAL_ERROR, SSL_F_DO_SSL3_WRITE,
                     ERR_R_INTERNAL_ERROR);
            goto err;
        ...
        {
            recordstart =
                WPACKET_get_curr(thispkt) - len - SSL3_RT_HEADER_LENGTH;
            s->msg_callback(1, 0, SSL3_RT_HEADER, recordstart,
                            SSL3_RT_HEADER_LENGTH, s, s->msg_callback_arg);
                       ...
                       size_t* written)
{
    ...
    for (;;)
    {
        ...
        if (s->wbio != NULL)
        {
            ...
            i = BIO_write(
                s->wbio,
                (char*)&(SSL3_BUFFER_get_buf(
                    &wb[currbuf])[SSL3_BUFFER_get_offset(&wb[currbuf])]),
...
start:
    ...
    if (s->rlayer.handshake_fragment_len > 0 &&
        SSL3_RECORD_get_type(rr) != SSL3_RT_HANDSHAKE && SSL_IS_TLS13(s))
    {
    ...
    if (type == SSL3_RECORD_get_type(rr) ||
        (SSL3_RECORD_get_type(rr) == SSL3_RT_CHANGE_CIPHER_SPEC &&
         type == SSL3_RT_HANDSHAKE && recvd_type != NULL && !is_tls13))
    {
        /*
    ...
    /*-
     * s->rlayer.handshake_fragment_len == 4  iff  rr->type ==
     * SSL3_RT_HANDSHAKE; (Possibly rr is 'empty' now, i.e. rr->length may be
     * 0.)
     */
    ...
    switch (SSL3_RECORD_get_type(rr))
    {
        default:
             ...
             * but TLS 1.2 says you MUST send an unexpected message alert. We
             * use the TLS 1.2 behaviour for all protocol versions to prevent
             * issues where no progress is being made and the peer continually
             * sends unrecognised record types, using up resources processing
             * them.
             */
            SSLfatal(s, SSL_AD_UNEXPECTED_MESSAGE, SSL_F_SSL3_READ_BYTES,
        ...
        case SSL3_RT_HANDSHAKE:
             ...
             * SSL3_RT_HANDSHAKE when ossl_statem_get_in_handshake(s) is true,
             * but that should not happen when type != rr->type
             */
            SSLfatal(s, SSL_AD_UNEXPECTED_MESSAGE, SSL_F_SSL3_READ_BYTES,
                     ERR_R_INTERNAL_ERROR);
            return -1;
        case SSL3_RT_APPLICATION_DATA:
            ...
            {
                /*
                 * This can happen after a client sends a CH followed by
                 * early_data, but the server responds with a HelloRetryRequest.
                 * The server reads the next record from the client expecting to
                 * find a plaintext ClientHello but gets a record which appears
                 * to be application data. The trial decrypt "works" because
                 * null decryption was applied. We just skip it and move on to
                 * the next record.
                 */
