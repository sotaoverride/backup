...
static void lzma_attribute((__noreturn__)) help(void)
{
    printf("Usage: %s [OPTION]... [FILE]...\n"
           "Decompress files in the ." TOOL_FORMAT
           " format to standard output.\n"
           "\n"
           "  -d, --decompress   (ignored, only decompression is supported)\n"
           "  -k, --keep         (ignored, files are never deleted)\n"
           "  -c, --stdout       (ignored, output is always written to "
           "standard output)\n"
...
static void lzma_attribute((__noreturn__)) version(void)
{
    ...
    while (true)
    {
        if (strm->avail_in == 0)
        {
            ...
            if (ferror(file))
            {
                ...
                // touch errno.
                my_errorf("%s: Error reading input file: %s", filename,
                          strerror(errno));
                exit(EXIT_FAILURE);
            }
        ...
        if (ret != LZMA_OK)
        {
            if (ret == LZMA_STREAM_END)
            {
                ...
                if (strm->avail_in != 0 || fread(in_buf, 1, 1, file) != 0 ||
                    !feof(file))
                    ret = LZMA_DATA_ERROR;
                else
                    return;
    ...
    else
    {
        ...
        do
        {
            ...
            else
            {
                ...
                if (file == NULL)
                {
                    my_errorf("%s: %s", argv[optind], strerror(errno));
                    exit(EXIT_FAILURE);
                }
