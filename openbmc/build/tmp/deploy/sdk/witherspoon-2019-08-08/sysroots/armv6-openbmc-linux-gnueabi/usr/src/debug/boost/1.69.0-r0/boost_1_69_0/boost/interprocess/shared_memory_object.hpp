...
class shared_memory_object
{
  ...
  public:
    ...
    //! with the access mode "mode". If the file previously exists, it tries to
    //! open it with mode "mode". Otherwise throws an error.
    ...
    //! mode "mode". If the file does not previously exist, it throws an error.
...
inline bool
    shared_memory_object::priv_open_or_create(ipcdetail::create_enum_t type,
                                              const char* filename, mode_t mode,
                                              const permissions& perm)
{
    m_filename = filename;
    std::string shmfile;
    ipcdetail::create_shared_dir_cleaning_old_and_get_filepath(filename,
                                                               shmfile);
    ...
    switch (type)
    {
        case ipcdetail::DoOpen:
            m_handle =
                ipcdetail::open_existing_file(shmfile.c_str(), mode, true);
            break;
        ...
        case ipcdetail::DoOpenOrCreate:
            m_handle = ipcdetail::create_or_open_file(shmfile.c_str(), mode,
                                                      perm, true);
            break;
        default:
        {
                                              ...
                                              const permissions& perm)
{
    ...
    const bool add_leading_slash =
        !shared_memory_object_detail::use_filesystem_based_posix();
#else
    ...
    switch (type)
    {
        ...
        case ipcdetail::DoOpenOrCreate:
        {
            // We need a create/open loop to change permissions correctly using
            // fchmod, since with "O_CREAT" only we don't know if we've created
            // or opened the shm.
            while (1)
            {
                // Try to create shared memory
                m_handle = shm_open(m_filename.c_str(),
                                    oflag | (O_CREAT | O_EXCL), unix_perm);
                ...
                else if (errno == EEXIST)
                {
                    ...
                    //(shm was removed between creation and opening tries), just
                    //retry
