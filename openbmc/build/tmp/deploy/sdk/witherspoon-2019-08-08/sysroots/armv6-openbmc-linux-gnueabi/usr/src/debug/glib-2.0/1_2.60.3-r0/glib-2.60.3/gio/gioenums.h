 ...
 * @G_APP_INFO_CREATE_SUPPORTS_STARTUP_NOTIFICATION: Application supports
 ...
 * @G_DATA_STREAM_BYTE_ORDER_HOST_ENDIAN: Selects endianness based on host
 ...
 * @G_DATA_STREAM_NEWLINE_TYPE_CR_LF: Selects "CR, LF" line ending, common on
 *Microsoft Windows.
 * @G_DATA_STREAM_NEWLINE_TYPE_ANY: Automatically try to handle any line ending
 *type.
 *
 * #GDataStreamNewlineType is used when checking for or setting the line endings
 *for a given file.
 **/
typedef enum
{
 ...
 * @G_FILE_ATTRIBUTE_TYPE_BYTE_STRING: a zero terminated string of non-zero
 *bytes.
...
typedef enum
{
    ...
    G_FILE_ATTRIBUTE_TYPE_BYTE_STRING, /* zero terminated string of non-zero
                                          bytes */
 ...
 * @G_FILE_ATTRIBUTE_INFO_COPY_WITH_FILE: copy the attribute values when the
 *file is copied.
 * @G_FILE_ATTRIBUTE_INFO_COPY_WHEN_MOVED: copy the attribute values when the
 *file is moved.
 *
 ...
 * @G_FILE_ATTRIBUTE_STATUS_ERROR_SETTING: Indicates an error in setting the
 *value.
 *
 ...
 * @G_FILE_COPY_ALL_METADATA: Copy all file metadata instead of just default set
 * used for copy (see #GFileInfo).
 * @G_FILE_COPY_NO_FALLBACK_FOR_MOVE: Don't use copy and delete fallback if
 * native move not supported.
 * @G_FILE_COPY_TARGET_DEFAULT_PERMS: Leaves target file with default perms,
 ...
 * @G_FILESYSTEM_PREVIEW_TYPE_IF_ALWAYS: Only preview files if user has
 *explicitly requested it.
 * @G_FILESYSTEM_PREVIEW_TYPE_IF_LOCAL: Preview files if user has requested
 *preview of "local" files.
 ...
 * @G_FILE_MONITOR_EVENT_CHANGES_DONE_HINT: a hint that this was probably the
 *last change in a set of changes.
 ...
 * @G_IO_ERROR_INVALID_FILENAME: Filename is invalid or contains invalid
 *characters.
 ...
 * @G_IO_ERROR_ADDRESS_IN_USE: The requested address is already in use.
 *Since 2.22
 ...
 * @G_EMBLEM_ORIGIN_LIVEMETADATA: Emblem depicts live metadata, such as
 * "readonly"
 * @G_EMBLEM_ORIGIN_TAG: Emblem comes from a user-defined tag, e.g. set by
...
/**
 * GBusType:
 * @G_BUS_TYPE_STARTER: An alias for the message bus that activated the process,
 * if any.
 ...
 * @G_BUS_NAME_OWNER_FLAGS_ALLOW_REPLACEMENT: Allow another message bus
 * connection to claim the name.
 * @G_BUS_NAME_OWNER_FLAGS_REPLACE: If another message bus connection owns the
 * name and have specified #G_BUS_NAME_OWNER_FLAGS_ALLOW_REPLACEMENT, then take
 * the name from the other connection.
 * @G_BUS_NAME_OWNER_FLAGS_DO_NOT_QUEUE: If another message bus connection owns
 ...
 * @G_DBUS_PROXY_FLAGS_DO_NOT_CONNECT_SIGNALS: Don't connect to signals on the
 * remote object.
 * @G_DBUS_PROXY_FLAGS_DO_NOT_AUTO_START: If the proxy is for a well-known name,
 * do not ask the bus to launch an owner during proxy initialization or a method
 * call. This flag is only meaningful in proxies for well-known names.
 * @G_DBUS_PROXY_FLAGS_GET_INVALIDATED_PROPERTIES: If set, the property value
 * for any __invalidated property__ will be (asynchronously) retrieved upon
 * receiving the
 ...
 * #GDBusProxy::g-properties-changed signal is emitted for the property along
 * with the retrieved value. Since 2.32.
 * @G_DBUS_PROXY_FLAGS_DO_NOT_AUTO_START_AT_CONSTRUCTION: If the proxy is for a
 * well-known name, do not ask the bus to launch an owner during proxy
 * initialization, but allow it to be autostarted by a method call. This flag is
 * only meaningful in proxies for well-known names, and only if
...
/* Remember to update g_dbus_error_quark() in gdbuserror.c if you extend this
 * enumeration */

/**
 * GDBusConnectionFlags:
 * @G_DBUS_CONNECTION_FLAGS_NONE: No flags set.
 * @G_DBUS_CONNECTION_FLAGS_AUTHENTICATION_CLIENT: Perform authentication
 * against server.
 ...
 * method.
 * @G_DBUS_CONNECTION_FLAGS_MESSAGE_BUS_CONNECTION: Pass this flag if connecting
 * to a peer that is a message bus. This means that the Hello() method will be
 * invoked as part of the connection setup.
 * @G_DBUS_CONNECTION_FLAGS_DELAY_MESSAGE_PROCESSING: If set, processing of
 * D-Bus messages is delayed until g_dbus_connection_start_message_processing()
 * is called.
 *
 ...
 * @G_DBUS_MESSAGE_HEADER_FIELD_REPLY_SERIAL: The serial number the message is a
 * reply to.
 * @G_DBUS_MESSAGE_HEADER_FIELD_DESTINATION: The name the message is intended
 * for.
 * @G_DBUS_MESSAGE_HEADER_FIELD_SENDER: Unique name of the sender of the message
 * (filled in by the bus).
 * @G_DBUS_MESSAGE_HEADER_FIELD_SIGNATURE: The signature of the message body.
 * @G_DBUS_MESSAGE_HEADER_FIELD_NUM_UNIX_FDS: The number of UNIX file
 * descriptors that accompany the message.
 *
 ...
 * @G_DBUS_SUBTREE_FLAGS_DISPATCH_TO_UNENUMERATED_NODES: Method calls to objects
 * not in the enumerated range will still be dispatched. This is useful if you
 ...
 * @G_CREDENTIALS_TYPE_LINUX_UCRED: The native credentials type is a struct
 * ucred.
 * @G_CREDENTIALS_TYPE_FREEBSD_CMSGCRED: The native credentials type is a struct
 * cmsgcred.
 ...
 * Added in 2.40.
 * @G_CREDENTIALS_TYPE_NETBSD_UNPCBID: The native credentials type is a struct
 * unpcbid.
 *
 ...
 * @G_DBUS_INTERFACE_SKELETON_FLAGS_HANDLE_METHOD_INVOCATIONS_IN_THREAD: Each
 * method invocation is handled in a thread dedicated to the invocation. This
 * means that the method implementation can use blocking IO without blocking any
 * other part of the process. It also means that the method implementation must
...
/**
 * GPollableReturn:
 * @G_POLLABLE_RETURN_FAILED: Generic error condition for when an operation
 * fails.
