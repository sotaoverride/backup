...
struct BusPolicy
{
    ...
    DBusList* at_console_true_rules;  /**< console user policy rules where
                                         at_console="true"*/
    DBusList* at_console_false_rules; /**< console user policy rules where
                                         at_console="false"*/
};
...
BusPolicy* bus_policy_new(void)
{
    ...
    policy->rules_by_uid =
        _dbus_hash_table_new(DBUS_HASH_UINTPTR, NULL, free_rule_list_func);
...
void bus_policy_unref(BusPolicy* policy)
{
    ...
    if (policy->refcount == 0)
    {
        ...
        _dbus_list_foreach(&policy->at_console_true_rules, free_rule_func,
                           NULL);
        _dbus_list_clear(&policy->at_console_true_rules);

        _dbus_list_foreach(&policy->at_console_false_rules, free_rule_func,
                           NULL);
                                                 ...
                                                 DBusError* error)
{
    ...
    if (!add_list_to_client(&policy->default_rules, client))
        goto nomem;
    ...
    if (_dbus_hash_table_get_n_entries(policy->rules_by_gid) > 0)
    {
        ...
        if (!bus_connection_get_unix_groups(connection, &groups, &n_groups,
                                            error))
            goto failed;
    ...
    if (!add_list_to_client(&policy->mandatory_rules, client))
        goto nomem;
...
static dbus_bool_t list_allows_user(dbus_bool_t def, DBusList** list,
                                    unsigned long uid,
...
dbus_bool_t bus_policy_allow_unix_user(BusPolicy* policy, unsigned long uid)
{
    ...
    allowed = list_allows_user(allowed, &policy->default_rules, uid, group_ids,
                               n_group_ids);

    allowed = list_allows_user(allowed, &policy->mandatory_rules, uid,
...
dbus_bool_t bus_policy_append_user_rule(BusPolicy* policy, dbus_uid_t uid,
                                        BusPolicyRule* rule)
{
    DBusList** list;
...
dbus_bool_t bus_policy_append_group_rule(BusPolicy* policy, dbus_gid_t gid,
                                         BusPolicyRule* rule)
{
    DBusList** list;
...
static dbus_bool_t append_copy_of_policy_list(DBusList** list,
                                              DBusList** to_append)
{
    DBusList* link;
    DBusList* tmp_list;
...
dbus_bool_t bus_policy_merge(BusPolicy* policy, BusPolicy* to_absorb)
{
    ...
    if (!merge_id_hash(policy->rules_by_uid, to_absorb->rules_by_uid))
        return FALSE;

    if (!merge_id_hash(policy->rules_by_gid, to_absorb->rules_by_gid))
        return FALSE;
...
static void remove_rules_by_type_up_to(BusClientPolicy* policy,
                                       BusPolicyRuleType type, DBusList* up_to)
{
    DBusList* link;
...
void bus_client_policy_optimize(BusClientPolicy* policy)
{
    ...
    while (link != NULL)
    {
        ...
        if (remove_preceding)
            remove_rules_by_type_up_to(policy, rule->type, link);
...
dbus_bool_t bus_client_policy_check_can_send(
    BusClientPolicy* policy, BusRegistry* registry, dbus_bool_t requested_reply,
    DBusConnection* receiver, DBusMessage* message, dbus_int32_t* toggles,
    dbus_bool_t* log)
{
    DBusList* link;
    dbus_bool_t allowed;
    ...
    *toggles = 0;
    ...
    while (link != NULL)
    {
        ...
        if (rule->d.send.message_type != DBUS_MESSAGE_TYPE_INVALID)
        {
            ...
            {
                _dbus_verbose(
                    "  (policy) skipping rule for different message type\n");
                continue;
            }
        }
        ...
        if (dbus_message_get_reply_serial(message) != 0)
        {
             ...
             * always allow.
             */
            if (!requested_reply && rule->allow &&
                rule->d.send.requested_reply && !rule->d.send.eavesdrop)
            {
                _dbus_verbose(
                    "  (policy) skipping allow rule since it only applies to "
                    "requested replies and does not allow eavesdropping\n");
                continue;
            }
            ...
            {
                _dbus_verbose("  (policy) skipping deny rule since it only "
                              "applies to unrequested replies\n");
                continue;
            }
        }
        ...
        if (rule->d.send.interface != NULL)
        {
             ...
             * message has no interface we want to skip the rule (and thus not
             * allow); for deny rules, if the message has no interface we want
             * to use the rule (and thus deny).
             */
            ...
            if ((no_interface && rule->allow) ||
                (!no_interface && strcmp(dbus_message_get_interface(message),
                                         rule->d.send.interface) != 0))
            {
                _dbus_verbose(
                    "  (policy) skipping rule for different interface\n");
                continue;
            }
        }
        ...
        if (rule->d.send.member != NULL)
        {
            if (dbus_message_get_member(message) != NULL &&
                strcmp(dbus_message_get_member(message), rule->d.send.member) !=
                    0)
            {
                _dbus_verbose(
                    "  (policy) skipping rule for different member\n");
                continue;
            }
        }
        ...
        if (rule->d.send.error != NULL)
        {
            ...
            {
                _dbus_verbose(
                    "  (policy) skipping rule for different error name\n");
                continue;
            }
        }
        ...
        if (rule->d.send.broadcast != BUS_POLICY_TRISTATE_ANY)
        {
                ...
                dbus_message_get_type(message) == DBUS_MESSAGE_TYPE_SIGNAL)
            {
                ...
                {
                    _dbus_verbose("  (policy) skipping rule because message is "
                                  "a broadcast\n");
                    continue;
                }
            }
            ...
            {
                _dbus_verbose("  (policy) skipping rule because message is not "
                              "a broadcast\n");
                continue;
            }
        }
        ...
        if (rule->d.send.destination != NULL)
        {
            ...
            if (receiver == NULL)
            {
                ...
                {
                    _dbus_verbose("  (policy) skipping rule because message "
                                  "dest is not %s\n",
                                  rule->d.send.destination);
                    continue;
                }
            ...
            else
            {
                ...
                if (service == NULL)
                {
                    _dbus_verbose("  (policy) skipping rule because dest %s "
                                  "doesn't exist\n",
                                  rule->d.send.destination);
                    continue;
                }
...
dbus_bool_t bus_client_policy_check_can_receive(
    ...
    DBusConnection* proposed_recipient, DBusMessage* message,
    dbus_int32_t* toggles)
{
    ...
    eavesdropping = addressed_recipient != proposed_recipient &&
    ...
    *toggles = 0;
    ...
    while (link != NULL)
    {
        ...
        {
            _dbus_verbose("  (policy) skipping allow rule since it doesn't "
        ...
        if (!eavesdropping && !rule->allow && rule->d.receive.eavesdrop)
        {
            _dbus_verbose("  (policy) skipping deny rule since it only applies "
                          "to eavesdropping\n");
            continue;
        }
        ...
        if (dbus_message_get_reply_serial(message) != 0)
        {
             ...
             */
            if (!requested_reply && rule->allow &&
                rule->d.receive.requested_reply && !rule->d.receive.eavesdrop)
            {
                _dbus_verbose(
                    "  (policy) skipping allow rule since it only applies to "
                    "requested replies and does not allow eavesdropping\n");
        ...
        {
            if (dbus_message_get_path(message) != NULL &&
                strcmp(dbus_message_get_path(message), rule->d.receive.path) !=
                    0)
            {
                _dbus_verbose("  (policy) skipping rule for different path\n");
                continue;
        ...
        if (rule->d.receive.origin != NULL)
        {
            ...
            else
            {
                ...
                if (service == NULL)
                {
                    _dbus_verbose("  (policy) skipping rule because origin %s "
                                  "doesn't exist\n",
                                  rule->d.receive.origin);
                    continue;
                }
            ...
            rule->d.receive.max_fds < DBUS_MAXIMUM_MESSAGE_UNIX_FDS)
        {
            ...
            if (n_fds < rule->d.receive.min_fds ||
                n_fds > rule->d.receive.max_fds)
            {
                _dbus_verbose(
                    "  (policy) skipping rule because message has %u fds "
                    "and that is outside range [%u,%u]",
                    n_fds, rule->d.receive.min_fds, rule->d.receive.max_fds);
                continue;
            }
        }
...
static dbus_bool_t bus_rules_check_can_own(DBusList* rules,
