 ...
 * \date   07.04.2008
 *
 * The header contains definition of a formatter parser function, along with
 * facilities to add support for custom formatters.
 */
...
struct formatter_factory
{
    ...
    /*!
     * Type of the map of formatter factory arguments [argument name -> argument
     * value]. This type of maps will be passed to formatter factories on
     * attempt to create a formatter.
     */
     ...
     */
    virtual formatter_type create_formatter(attribute_name const& name,
                                            args_map const& args) = 0;
...
/*!
 * Base class for formatter factories. This class provides default
 * implementation of formatter expressions for types supporting stream output.
 * The factory does not take into account any additional parameters that may be
 * specified.
 */
template <typename CharT, typename AttributeValueT>
class basic_formatter_factory : public formatter_factory<CharT>
{
  private:
  ...
  public:
     ...
     * \param args Formatter arguments
     */
    formatter_type create_formatter(attribute_name const& name,
                                    args_map const& args)
    {
        return formatter_type(expressions::stream
                              << expressions::attr<value_type>(name));
    }
};
 ...
 *
 * The function registers a user-defined formatter factory. The registered
 * factory function will be called when the formatter parser detects the
 * specified attribute name in the formatter string.
 *
...
inline typename boost::enable_if_c<is_base_and_derived<
    formatter_factory<typename FactoryT::char_type>, FactoryT>::value>::type
                               ...
                               shared_ptr<FactoryT> const& factory)
{
    ...
    register_formatter_factory(
        attr_name, boost::static_pointer_cast<factory_base>(factory));
}

/*!
 * \brief The function registers a simple formatter factory
 *
 * The function registers a simple formatter factory. The registered factory
 * will generate formatters that will be equivalent to the
 * <tt>log::expressions::attr</tt> formatter (i.e. that will use the native \c
 * operator<< to format the attribute value). The factory does not use any
 * arguments from the format string, if specified.
 *
 * \pre <tt>!!attr_name</tt>.
 *
 * \param attr_name Attribute name
 ...
 *
 * \b Throws: An <tt>std::exception</tt>-based exception, if a formatter cannot
 * be recognized in the character sequence.
 */
template <typename CharT>
