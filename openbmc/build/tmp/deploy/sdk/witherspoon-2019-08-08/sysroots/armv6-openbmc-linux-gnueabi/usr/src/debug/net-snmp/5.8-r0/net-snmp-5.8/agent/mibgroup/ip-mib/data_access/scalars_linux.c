...
int netsnmp_arch_ip_scalars_ipv6IpForwarding_get(u_long* value)
{
    ...
    {
        DEBUGMSGTL(
            ("access:ipv6IpForwarding", "could not open %s\n", ipfw6_name));
        return -2;
    }
    ...
    {
        DEBUGMSGTL(
            ("access:ipv6IpForwarding", "could not read %s\n", ipfw6_name));
        return -3;
    }
...
int netsnmp_arch_ip_scalars_ipv6IpForwarding_set(u_long value)
{
    ...
    {
        DEBUGMSGTL(
            ("access:ipv6IpForwarding", "could not write %s\n", ipfw6_name));
        return SNMP_ERR_GENERR;
    }
...
int netsnmp_arch_ip_scalars_ipv6IpDefaultHopLimit_get(u_long* value)
{
    ...
    {
        DEBUGMSGTL(
            ("access:ipDefaultHopLimit", "could not open %s\n", iphop6_name));
        return -2;
    }
    ...
    {
        DEBUGMSGTL(
            ("access:ipDefaultHopLimit", "could not read %s\n", iphop6_name));
        return -3;
    }
...
int netsnmp_arch_ip_scalars_ipv6IpDefaultHopLimit_set(u_long value)
{
    ...
    {
        DEBUGMSGTL(
            ("access:ipDefaultHopLimit", "could not write %s\n", iphop6_name));
        return SNMP_ERR_GENERR;
    }
...
void netsnmp_arch_ip_scalars_register_handlers(void)
{
    ...
    netsnmp_register_num_file_instance(
        "ipReasmTimeout", ipReasmTimeout_oid, OID_LENGTH(ipReasmTimeout_oid),
        "/proc/sys/net/ipv4/ipfrag_time", ASN_INTEGER, HANDLER_CAN_RONLY, NULL,
        NULL);
}
