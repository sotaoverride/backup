 ...
 */
static krb5_error_code encrypt_credencpart(krb5_context context,
                                           ...
                                           krb5_enc_data* pencdata)
{
    ...
    retval = k5_encrypt_keyhelper(context, pkey, KRB5_KEYUSAGE_KRB_CRED_ENCPART,
                                  scratch, pencdata);
...
static krb5_error_code
    krb5_mk_ncred_basic(krb5_context context, krb5_creds** ppcreds,
                        krb5_int32 nppcreds, krb5_key key,
                        krb5_replay_data* replaydata, krb5_address* local_addr,
                        krb5_address* remote_addr, krb5_cred* pcred)
{
    ...
    credenc.r_address = 0;
    if (local_addr)
        krb5_copy_addr(context, local_addr, &credenc.s_address);
    if (remote_addr)
        krb5_copy_addr(context, remote_addr, &credenc.r_address);
    ...
    for (i = 0; i < nppcreds; i++)
    {
        ...
        if ((retval =
                 decode_krb5_ticket(&ppcreds[i]->ticket, &pcred->tickets[i])))
            goto cleanup;
                                            ...
                                            krb5_replay_data* outdata)
{
    ...
    if ((pcred->tickets = (krb5_ticket**)calloc((size_t)ncred + 1,
    ...
    if ((retval =
             krb5_mk_ncred_basic(context, ppcreds, ncred, key, &replaydata,
                                 plocal_fulladdr, premote_fulladdr, pcred)))
    {
        goto error;
    }
                                            ...
                                            krb5_replay_data* outdata)
{
    ...
    retval = krb5_mk_ncred(context, auth_context, ppcreds, ppdata, outdata);
