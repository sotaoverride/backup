...
static gpointer freelist_alloc(gsize size, gboolean reuse)
{
    ...
    if (reuse)
    {
        for (free = freelist, prev = &freelist; free != NULL;
             prev = &free->next, free = free->next)
        {
 ...
 */
gpointer _g_atomic_array_copy(GAtomicArray* array, gsize header_size,
...
void _g_atomic_array_update(GAtomicArray* array, gpointer new_data)
{
    ...
    g_assert(old == NULL || G_ATOMIC_ARRAY_DATA_SIZE(old) <=
