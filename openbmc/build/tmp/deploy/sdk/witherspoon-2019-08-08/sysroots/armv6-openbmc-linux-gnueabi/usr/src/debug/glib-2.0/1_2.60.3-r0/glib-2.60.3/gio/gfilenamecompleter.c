...
static void g_filename_completer_finalize(GObject* object)
{
    GFilenameCompleter* completer;
...
static void g_filename_completer_class_init(GFilenameCompleterClass* klass)
{
     ...
     **/
    signals[GOT_COMPLETION_DATA] = g_signal_new(
        I_("got-completion-data"), G_TYPE_FILENAME_COMPLETER, G_SIGNAL_RUN_LAST,
        G_STRUCT_OFFSET(GFilenameCompleterClass, got_completion_data), NULL,
        NULL, g_cclosure_marshal_VOID__VOID, G_TYPE_NONE, 0);
}

static void g_filename_completer_init(GFilenameCompleter* completer)
{
}
...
static char* longest_common_prefix(char* a, char* b)
{
    char* start;
...
static void load_basenames_data_free(LoadBasenamesData* data)
{
    if (data->enumerator)
...
static void got_more_files(GObject* source_object, GAsyncResult* res,
                           gpointer user_data)
{
    ...
    for (l = infos; l != NULL; l = l->next)
    {
        ...
        if (data->should_escape)
            basename = g_uri_escape_string(
                name, G_URI_RESERVED_CHARS_ALLOWED_IN_PATH, TRUE);
        else
    ...
    if (infos)
    {
        ...
        g_file_enumerator_next_files_async(
            data->enumerator, 100, 0, data->cancellable, got_more_files, data);
    }
    else
    {
...
static void got_enum(GObject* source_object, GAsyncResult* res,
                     gpointer user_data)
{
    LoadBasenamesData* data = user_data;
    ...
    data->enumerator =
        g_file_enumerate_children_finish(G_FILE(source_object), res, NULL);
    ...
    g_file_enumerator_next_files_async(data->enumerator, 100, 0,
                                       data->cancellable, got_more_files, data);
}

static void schedule_load_basenames(GFilenameCompleter* completer, GFile* dir,
                                    gboolean should_escape)
{
    ...
    g_file_enumerate_children_async(
        dir, G_FILE_ATTRIBUTE_STANDARD_NAME "," G_FILE_ATTRIBUTE_STANDARD_TYPE,
        0, 0, data->cancellable, got_enum, data);
}
...
static GList* init_completion(GFilenameCompleter* completer,
                              const char* initial_text, char** basename_out)
{
    gboolean should_escape;
    GFile *file, *parent;
    char* basename;
    ...
    *basename_out = NULL;
    ...
    if (should_escape)
    {
        t = basename;
        basename = g_uri_escape_string(
            basename, G_URI_RESERVED_CHARS_ALLOWED_IN_PATH, TRUE);
        g_free(t);
    }
    else
    {
 ...
 *possible completions for @initial_text. This array must be freed by
 *g_strfreev() when finished.
 **/
        ...
        if (g_str_has_prefix(possible_match, prefix))
            g_ptr_array_add(res,
                            g_strconcat(initial_text,
                                        possible_match + strlen(prefix), NULL));
    }
