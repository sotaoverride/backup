...
static OM_uint32 val_acc_sec_ctx_args(
    ...
    gss_cred_id_t verifier_cred_handle, gss_buffer_t input_token_buffer,
...
static int allow_mech_by_default(gss_OID mech)
{
    ...
    if (generic_gss_test_oid_set_member(&minor, GSS_C_MA_DEPRECATED, attrs,
                                        &p) != GSS_S_COMPLETE ||
        p)
        reject = 1;
    else if (generic_gss_test_oid_set_member(&minor, GSS_C_MA_NOT_DFLT_MECH,
                                             attrs, &p) != GSS_S_COMPLETE ||
             p)
        reject = 1;
...
OM_uint32 KRB5_CALLCONV gss_accept_sec_context(minor_status, context_handle,
                                               verifier_cred_handle,
                                               input_token_buffer,
                                               input_chan_bindings, src_name,
                                               mech_type, output_token,
                                               ret_flags, time_rec, d_cred)
...

{
    ...
    status = val_acc_sec_ctx_args(minor_status, context_handle,
    ...
    if (*context_handle == GSS_C_NO_CONTEXT)
    {
        ...
        union_ctx_id =
            (gss_union_ctx_id_t)malloc(sizeof(gss_union_ctx_id_desc));
        if (!union_ctx_id)
    ...
    if (verifier_cred_handle != GSS_C_NO_CREDENTIAL)
    {
        input_cred_handle = gssint_get_mechanism_cred(
            (gss_union_cred_t)verifier_cred_handle, selected_mech);
    ...
    if (mech && mech->gss_accept_sec_context)
    {
        ...
        status = mech->gss_accept_sec_context(
            minor_status, &union_ctx_id->internal_ctx_id, input_cred_handle,
            input_token_buffer, input_chan_bindings,
            src_name ? &internal_name : NULL, &actual_mech, output_token,
            &temp_ret_flags, time_rec, d_cred ? &tmp_d_cred : NULL);
        ...
        if (src_name != NULL)
        {
            if (internal_name != GSS_C_NO_NAME)
            {
                ...
                temp_status = gssint_convert_name_to_union_name(
                    &temp_minor_status, mech, internal_name, &tmp_src_name);
    ...
    (((o1)->length >= (o2)->length) &&                                         \
     (memcmp((o1)->elements, (o2)->elements, (o2)->length) == 0))
            ...
            tmp_d_cred != GSS_C_NO_CREDENTIAL)
        {
            ...
            if (actual_mech != GSS_C_NO_OID && public_mech != GSS_C_NO_OID &&
