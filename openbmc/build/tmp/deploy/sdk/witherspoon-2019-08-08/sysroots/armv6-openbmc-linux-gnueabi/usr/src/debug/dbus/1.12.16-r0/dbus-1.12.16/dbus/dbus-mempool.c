...
struct DBusMemBlock
{
    ...
    unsigned char elements[ELEMENT_PADDING]; /**< the block data, actually
...
struct DBusMemPool
{
    ...
    unsigned int
        zero_elements : 1; /**< whether to zero-init allocated elements */
...
void* _dbus_mem_pool_alloc(DBusMemPool* pool)
{
    ...
    if (_dbus_disable_mem_pools())
    {
        ...
        alloc_size =
            sizeof(DBusMemBlock) - ELEMENT_PADDING + pool->element_size;
...
#endif
    {
        ...
        else
        {
                ...
                pool->blocks->used_so_far == pool->block_size)
            {
                ...
                alloc_size =
                    sizeof(DBusMemBlock) - ELEMENT_PADDING + pool->block_size;
...
void _dbus_mem_pool_get_stats(DBusMemPool* pool, dbus_uint32_t* in_use_p,
...
#define N_ITERATIONS FREE_ARRAY_SIZE * 512
    ...
    while (i < N_ITERATIONS)
    {
        ...
        _dbus_assert(to_free[j] !=
                     NULL); /* in a real app of course this is wrong */
