...
struct tcpLstnPortList_s
{
    ...
    sbool bSuppOctetFram; /**< do we support octect-counted framing? (if
                             no->legay only!)*/
    ratelimit_t* ratelimiter;
    uchar dfltTZ[8]; /**< default TZ if none in timestamp; '\0' =No Default */
    sbool
        bSPFramingFix; /**< support work-around for broken Cisco ASA framing? */
    STATSCOUNTER_DEF(ctrSubmit, mutCtrSubmit)
    tcpLstnPortList_t* pNext; /**< next port or NULL */
};

#define TCPSRV_NO_ADDTL_DELIMITER                                              \
    -1 /* specifies that no additional delimiter is to be used in TCP framing  \
        */
...
struct tcpsrv_s
{
    ...
    uchar*
        pszLstnPortFileName; /**< File in which the dynamic port is written */
    uchar* pszDrvrAuthMode;  /**< auth mode of the stream driver to use */
    uchar* pszDrvrPermitExpiredCerts; /**< current driver setting for handlign
                                         expired certs */
    ...
    sbool bEmitMsgOnClose; /**< emit an informational message when the remote
                              peer closes connection */
    sbool bUsingEPoll; /**< are we in epoll mode (means we do not need to keep
                          track of sessions!) */
    ...
    tcpLstnPortList_t**
        ppLstnPort;  /**< pointer to relevant listen port description */
    ...
    int bDisableLFDelim; /**< if 1, standard LF frame delimiter is disabled
                            (*very dangerous*) */
    ...
    void* pUsr; /**< a user-settable pointer (provides extensibility for
                   "derived classes")*/
    /* callbacks */
    int (*pIsPermittedHost)(struct sockaddr* addr, char* fromHostFQDN,
                            void* pUsrSrv, void* pUsrSess);
    ...
    rsRetVal (*OnMsgReceive)(tcps_sess_t*, uchar* pszMsg,
                             int iLenMsg); /* submit message callback */
};
...
rsRetVal (*SetCBIsPermittedHost)(tcpsrv_t*, int (*)(struct sockaddr* addr,
                                                    char*, void*, void*));
...
rsRetVal (*SetNotificationOnRemoteClose)(tcpsrv_t* pThis,
                                         int bNewVal); /* 2009-10-01 */
...
ENDinterface(tcpsrv)
#define tcpsrvCURR_IF_VERSION                                                  \
    22 /* increment whenever you change the interface structure! */
    /* change for v4:
