...
void fail(int err, boost::system::error_code* ec)
{
    if (ec == 0)
        BOOST_FILESYSTEM_THROW(
            boost::system::system_error(err, boost::system::system_category(),
...
int acquire_crypt_handle(HCRYPTPROV& handle)
{
    ...
    errval = ::GetLastError();
    // Another thread could have attempted to create the keyset at the same
    // time.
                         ...
                         boost::system::error_code* ec)
{
    ...
    if (!errval)
    {
        BOOL gen_ok = ::CryptGenRandom(handle, static_cast<DWORD>(len),
...
path unique_path(const path& model, system::error_code* ec)
{
    ...
    char ran[] = "123456789abcdef"; // init to avoid clang static analyzer
                                    // message see ticket #8954
