...
#define ALIGN(idx)                                                             \
    ...
    {                                                                          \
        if (idx % sizeof(void*))                                               \
            idx += (sizeof(void*) -                                            \
                    idx % sizeof(void*)); /* Align on 32 bit boundary */       \
    } while (0)
...
static char** get_search_domains(void)
{
    ...
    while (!feof(f))
    {
        ...
        /* the resolv.conf manpage also states that 'search' and 'domain' are
           mutually exclusive and that the last one wins. */
                                           ...
                                           int* h_errnop)
{
    ...
    if (buflen < sizeof(char*) + /* alias names */
                     strlen(name) + 1)
    { /* official name */
                                          ...
                                          int* h_errnop)
{
...
#endif
#ifndef NSS_IPV6_ONLY
            r = mdns_query_ipv4(fd, (const ipv4_address_t*)addr, name_callback,
                                &u);
#endif
#if !defined(NSS_IPV6_ONLY) && !defined(NSS_IPV4_ONLY)
        else
#endif
#ifndef NSS_IPV4_ONLY
            r = mdns_query_ipv6(fd, (const ipv6_address_t*)addr, name_callback,
                                &u);
#endif
        if (r < 0)
        {
    ...
    *((char**)buffer) = NULL;
    ...
    if (buflen < strlen(u.data.name[0]) + 1 + /* official names */
                     sizeof(char*) +          /* alias names */
                     address_length +         /* address */
                     sizeof(void*) * 2 +      /* address list */
                     sizeof(void*))
    { /* padding to get the alignment right */
