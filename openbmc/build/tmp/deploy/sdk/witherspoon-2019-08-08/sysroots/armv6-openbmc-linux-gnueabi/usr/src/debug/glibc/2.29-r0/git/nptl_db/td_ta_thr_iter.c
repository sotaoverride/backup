...
static td_err_e iterate_thread_list(td_thragent_t* ta, td_thr_iter_f* callback,
                                    ...
                                    int ti_pri, psaddr_t head, bool fake_empty)
{
    td_err_e err;
    psaddr_t next, ofs;
    void* copy;
    ...
    while (next != head)
    {
       ...
       multiple ps_pdread calls.  */
        ...
        err =
            DB_GET_FIELD_LOCAL(schedpolicy, ta, copy, pthread, schedpolicy, 0);
        if (err != TD_OK)
            break;
                        ...
                        sigset_t* ti_sigmask_p, unsigned int ti_user_flags)
{
    ...
    if (err == TD_OK)
        err = iterate_thread_list(ta, callback, cbdata_p, state, ti_pri, list,
                                  true);
