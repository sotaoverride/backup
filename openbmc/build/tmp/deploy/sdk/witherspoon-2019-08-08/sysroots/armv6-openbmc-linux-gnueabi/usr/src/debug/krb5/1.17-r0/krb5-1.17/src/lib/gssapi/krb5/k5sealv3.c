...
krb5_error_code gss_krb5int_make_seal_token_v3(krb5_context context,
                                               ...
                                               int conf_req_flag, int toktype)
{
    ...
    wrap_with_checksum:
        ...
        err = krb5_k_make_checksum(context, cksumtype, key, key_usage, &plain,
                                   &sum);
        zap(plain.data, plain.length);
        free(plain.data);
        plain.data = 0;
    ...
    int* conf_state, gss_qop_t* qop_state, int toktype)
{
    ...
    if (toktype == KG_TOK_WRAP_MSG)
    {
        ...
        if (ptr[2] & FLAG_WRAP_CONFIDENTIAL)
        {
            ...
            if (plain.data == NULL)
                goto no_mem;
            err = krb5_k_decrypt(context, key, key_usage, 0, &cipher, &plain);
            if (err)
            {
            ...
            if (load_16_be(althdr) != KG2_TOK_WRAP_MSG || althdr[2] != ptr[2] ||
                althdr[3] != ptr[3] || memcmp(althdr + 8, ptr + 8, 8))
            {
                free(plain.data);
                goto defective;
            }
        ...
        else
        {
            ...
            err = krb5_k_verify_checksum(context, key, key_usage, &plain, &sum,
                                         &valid);
            if (err)
                goto error;
            if (!valid)
