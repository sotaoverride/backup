...
#pragma warning(push)
#pragma warning(                                                               \
    disable : 4251) // class 'boost::shared_ptr<T>' needs to have dll-interface
...
class BOOST_PROGRAM_OPTIONS_DECL option_description
{
  public:
    ...
    */
    ...
    /** Returns the canonical name for the option description to enable the user
       to recognised a matching option. 1) For short options ('-', '/'), returns
       the short name prefixed. 2) For long options ('--' / '-') returns the
       first long name prefixed 3) All other cases, returns the first long name
       (if present) or the short name, unprefixed.
    */
