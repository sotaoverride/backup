...
ASN1_SEQUENCE(ASRange) = {ASN1_SIMPLE(ASRange, min, ASN1_INTEGER),
                          ASN1_SIMPLE(ASRange, max,
                                      ASN1_INTEGER)} ASN1_SEQUENCE_END(ASRange)

    ASN1_CHOICE(
        ASIdOrRange) = {ASN1_SIMPLE(ASIdOrRange, u.id, ASN1_INTEGER),
                        ASN1_SIMPLE(ASIdOrRange, u.range,
                                    ASRange)} ASN1_CHOICE_END(ASIdOrRange)

        ASN1_CHOICE(ASIdentifierChoice) =
            {ASN1_SIMPLE(ASIdentifierChoice, u.inherit, ASN1_NULL),
             ASN1_SEQUENCE_OF(ASIdentifierChoice, u.asIdsOrRanges,
                              ASIdOrRange)} ASN1_CHOICE_END(ASIdentifierChoice)

                ASN1_SEQUENCE(ASIdentifiers) =
                    {ASN1_EXP_OPT(ASIdentifiers, asnum, ASIdentifierChoice, 0),
                     ASN1_EXP_OPT(ASIdentifiers, rdi, ASIdentifierChoice,
                                  1)} ASN1_SEQUENCE_END(ASIdentifiers)
     ...
     */
    static int i2r_ASIdentifierChoice(BIO * out, ASIdentifierChoice* choice,
            ...
            for (i = 0; i < sk_ASIdOrRange_num(choice->u.asIdsOrRanges); i++)
            {
                ...
                switch (aor->type)
                {
                    ...
                    case ASIdOrRange_range:
                        if ((s = i2s_ASN1_INTEGER(NULL, aor->u.range->min)) ==
                            NULL)
                            return 0;
                        BIO_printf(out, "%*s%s-", indent + 2, "", s);
                        OPENSSL_free(s);
                        if ((s = i2s_ASN1_INTEGER(NULL, aor->u.range->max)) ==
                            NULL)
                            return 0;
                        BIO_printf(out, "%s\n", s);
                        OPENSSL_free(s);
 ...
 */
static int i2r_ASIdentifiers(const X509V3_EXT_METHOD* method, void* ext,
                             BIO* out, int indent)
{
 ...
 */
static int extract_min_max(ASIdOrRange* aor, ASN1_INTEGER** min,
                           ASN1_INTEGER** max)
{
...
static int ASIdentifierChoice_is_canonical(ASIdentifierChoice* choice)
{
    ...
    for (i = 0; i < sk_ASIdOrRange_num(choice->u.asIdsOrRanges) - 1; i++)
    {
        ...
        if ((bn == NULL && (bn = BN_new()) == NULL) ||
            ASN1_INTEGER_to_BN(a_max, bn) == NULL || !BN_add_word(bn, 1) ||
...
{
    return (asid == NULL || (ASIdentifierChoice_is_canonical(asid->asnum) &&
...
{
    return (asid == NULL || (ASIdentifierChoice_canonize(asid->asnum) &&
...
int X509v3_asid_subset(ASIdentifiers* a, ASIdentifiers* b)
{
    return (
        a == NULL || a == b ||
        (b != NULL && !X509v3_asid_inherits(a) && !X509v3_asid_inherits(b) &&
         asid_contains(b->asnum->u.asIdsOrRanges, a->asnum->u.asIdsOrRanges) &&
         asid_contains(b->rdi->u.asIdsOrRanges, a->rdi->u.asIdsOrRanges)));
}
