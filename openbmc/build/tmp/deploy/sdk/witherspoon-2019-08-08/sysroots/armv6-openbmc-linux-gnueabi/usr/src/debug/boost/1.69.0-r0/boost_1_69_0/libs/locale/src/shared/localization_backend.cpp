...
class localization_backend_manager::impl
{
  ...
  public:
    ...
    {
        for (all_backends_type::const_iterator p = other.all_backends_.begin();
             p != other.all_backends_.end(); ++p)
        {
    ...
    void adopt_backend(std::string const& name,
                       localization_backend* backend_ptr)
    {
                ...
                locale_category_type category = all_categories)
    {
        ...
        if (id == all_backends_.size())
            return;
        for (unsigned flag = 1, i = 0; i < default_backends_.size();
             flag <<= 1, i++)
        {
            if (category & flag)
            {
  ...
  private:
    class actual_backend : public localization_backend
    {
      public:
        actual_backend(
            std::vector<boost::shared_ptr<localization_backend>> const&
                backends,
        ...
        virtual std::locale install(std::locale const& l,
    ...
    typedef std::vector<
        std::pair<std::string, boost::shared_ptr<localization_backend>>>
        all_backends_type;
...
localization_backend_manager::localization_backend_manager(
    localization_backend_manager const& other) :
    pimpl_(new impl(*other.pimpl_))
{
}

localization_backend_manager const& localization_backend_manager::
    operator=(localization_backend_manager const& other)
{
    if (this != &other)
    {
...
void localization_backend_manager::add_backend(
    std::string const& name, std::auto_ptr<localization_backend> backend)
{
    pimpl_->adopt_backend(name, backend.release());
}
#endif
#ifndef BOOST_NO_CXX11_SMART_PTR
std::unique_ptr<localization_backend>
    localization_backend_manager::get_unique_ptr() const
{
    std::unique_ptr<localization_backend> r(pimpl_->create());
    return r;
}
void localization_backend_manager::add_backend(
    std::string const& name, std::unique_ptr<localization_backend> backend)
{
    pimpl_->adopt_backend(name, backend.release());
}
#endif
...
void localization_backend_manager::adopt_backend(std::string const& name,
...
void localization_backend_manager::select(std::string const& backend_name,
                                          locale_category_type category)
{
...
localization_backend_manager
    localization_backend_manager::global(localization_backend_manager const& in)
{
    boost::unique_lock<boost::mutex> lock(localization_backend_manager_mutex());
    localization_backend_manager mgr = localization_backend_manager_global();
