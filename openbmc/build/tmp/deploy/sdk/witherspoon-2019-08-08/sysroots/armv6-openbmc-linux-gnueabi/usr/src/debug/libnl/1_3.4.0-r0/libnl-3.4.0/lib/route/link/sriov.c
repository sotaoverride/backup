...
int rtnl_link_sriov_clone(struct rtnl_link* dst, struct rtnl_link* src)
{
    ...
    nl_list_for_each_entry_safe(s_vf, next, &s_list->vf_list, vf_list)
    {
        ...
        if (s_vf->ce_mask & SRIOV_ATTR_VLAN)
        {
            ...
            err = rtnl_link_vf_vlan_alloc(&dst_vlans, src_vlans->size);
            if (err < 0)
                return err;
...
static void dump_sriov_vlans(nl_vf_vlans_t* vlans, struct nl_dump_params* p)
{
    char buf[64];
    int cur = 0;
    ...
    while (cur < vlans->size)
    {
        ...
        if (vlan_data[cur].vf_vlan_proto)
        {
            ...
            nl_dump(p, " proto %s",
                    rtnl_link_vf_vlanproto2str(prot, buf, sizeof(buf)));
        }
        nl_dump(p, "\n");
        cur++;
    }
    ...
    if (!err)
    {
        if (vf_rate.api == RTNL_LINK_VF_RATE_API_OLD)
            ...
            nl_dump(p,
                    ...
                    "max_rate %u\n",
                    vf_rate.min_tx_rate, vf_rate.max_tx_rate);
    }
                          ...
                          struct nl_dump_params* p)
{
    ...
    nl_dump_line(p,
                 ...
                 res, unit, vf_data->vf_stats[RTNL_LINK_VF_STATS_RX_PACKETS],
...
static int sriov_fill_vfinfo(struct nl_msg* msg, struct rtnl_link_vf* vf_data)
{
    ...
    if (vf_data->ce_mask & SRIOV_ATTR_VLAN)
    {
        ...
        if ((vlan_list->size == 1) && (proto == ETH_P_8021Q))
            ...
            err = sriov_fill_vf_vlan_list(msg, vlan_list, vf_data->vf_index);
    }
    ...
    if (vf_data->ce_mask & SRIOV_ATTR_SPOOFCHK)
    {
        ...
        NLA_PUT(msg, IFLA_VF_SPOOFCHK, sizeof(vf_spoofchk), &vf_spoofchk);
    }
    ...
    if (vf_data->ce_mask & SRIOV_ATTR_LINK_STATE)
    {
        ...
        NLA_PUT(msg, IFLA_VF_LINK_STATE, sizeof(vf_link_state), &vf_link_state);
    }
    ...
    if (vf_data->ce_mask & SRIOV_ATTR_IB_NODE_GUID)
    {
        ...
        NLA_PUT(msg, IFLA_VF_IB_NODE_GUID, sizeof(vf_node_guid), &vf_node_guid);
    }
    ...
    if (vf_data->ce_mask & SRIOV_ATTR_IB_PORT_GUID)
    {
        ...
        NLA_PUT(msg, IFLA_VF_IB_PORT_GUID, sizeof(vf_port_guid), &vf_port_guid);
    }
...
int rtnl_link_sriov_parse_vflist(struct rtnl_link* link, struct nlattr** tb)
{
         ...
         nla = nla_next(nla, &len))
    {
        ...
        if (t[IFLA_VF_MAC])
        {
            ...
            vf_data->vf_lladdr = nl_addr_build(AF_LLC, vf_lladdr->mac, 6);
        ...
        {
            list_len = 0;
            nla_for_each_nested(nla_list, t[IFLA_VF_VLAN_LIST], list_rem)
            {
            ...
            err = rtnl_link_vf_vlan_info(list_len, vf_vlan_info, &vf_vlans);
            if (err < 0)
                return err;
        ...
        if (t[IFLA_VF_STATS])
        {
            err = nla_parse_nested(stb, IFLA_VF_STATS_MAX, t[IFLA_VF_STATS],
                                   sriov_stats_policy);
            if (err < 0)
                return err;

            SET_VF_STAT(link, cur, stb, RTNL_LINK_VF_STATS_RX_PACKETS,
                        IFLA_VF_STATS_RX_PACKETS);
            SET_VF_STAT(link, cur, stb, RTNL_LINK_VF_STATS_TX_PACKETS,
                        IFLA_VF_STATS_TX_PACKETS);
            SET_VF_STAT(link, cur, stb, RTNL_LINK_VF_STATS_RX_BYTES,
                        IFLA_VF_STATS_RX_BYTES);
            SET_VF_STAT(link, cur, stb, RTNL_LINK_VF_STATS_TX_BYTES,
                        IFLA_VF_STATS_TX_BYTES);
            SET_VF_STAT(link, cur, stb, RTNL_LINK_VF_STATS_BROADCAST,
                        IFLA_VF_STATS_BROADCAST);
            SET_VF_STAT(link, cur, stb, RTNL_LINK_VF_STATS_MULTICAST,
...
{
    return __type2str(ls, buf, len, vf_link_states, ARRAY_SIZE(vf_link_states));
}
