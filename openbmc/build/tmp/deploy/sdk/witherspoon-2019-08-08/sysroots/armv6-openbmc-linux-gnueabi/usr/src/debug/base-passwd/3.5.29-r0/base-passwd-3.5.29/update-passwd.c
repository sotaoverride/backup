...
int fputpwent(const struct passwd* passwd, FILE* f)
{
    ...
    if (passwd->pw_name[0] == '+')
        res = fprintf(f, "%s:%s:::%s:%s:%s\n", safestr(passwd->pw_name),
                      safestr(passwd->pw_passwd), safestr(passwd->pw_gecos),
                      safestr(passwd->pw_dir), safestr(passwd->pw_shell));
    else
        res = fprintf(f, "%s:%s:%u:%u:%s:%s:%s\n", safestr(passwd->pw_name),
                      safestr(passwd->pw_passwd), passwd->pw_uid,
                      passwd->pw_gid, safestr(passwd->pw_gecos),
                      safestr(passwd->pw_dir), safestr(passwd->pw_shell));
...
void usage()
{
    printf(
        ...
        "  -v, --verbose             Show details about what we are doing "
        "(recommended)\n"
        ...
        "   system group : %s\n"
        "\n"
        "Report bugs to the Debian bug tracking system, package "
        "\"base-passwd\".\n"
        "\n",
 ...
 * switching entries ("+").
 */
void process_moved_entries(const struct _info* lst, struct _node** passwd,
                           struct _node* master, const char* descr)
{
 ...
 */
void process_new_entries(const struct _info* lst, struct _node** passwd,
                         struct _node* master, const char* descr)
{
    while (master)
    {
 ...
 */
void process_old_entries(const struct _info* lst, struct _node** passwd,
                         struct _node* master, const char* descr)
{
    struct _node* walk = *passwd;
 ...
 */
void process_changed_accounts(struct _node* passwd, struct _node* group,
                              struct _node* master)
{
    for (; passwd; passwd = passwd->next)
    {
        ...
        if (passwd->d.pw.pw_gid != mc->d.pw.pw_gid)
        {
            if (opt_verbose)
            {
                const struct _node* oldentry =
                    find_by_id(group, passwd->d.pw.pw_gid);
                const struct _node* newentry =
                    find_by_id(group, mc->d.pw.pw_gid);
                ...
                printf("Changing gid of %s from %u (%s) to %u (%s)\n",
                       passwd->name, passwd->d.pw.pw_gid, oldname,
                       mc->d.pw.pw_gid, newname);
            }
        ...
        if (!keepgecos(specialusers, passwd->id))
            if ((passwd->d.pw.pw_gecos == NULL) ||
                (strcmp(passwd->d.pw.pw_gecos, mc->d.pw.pw_gecos) != 0))
            {
                if (opt_verbose)
                {
                    const char* oldgecos =
                        passwd->d.pw.pw_gecos ? passwd->d.pw.pw_gecos : "";
                    printf("Changing GECOS of %s from \"%s\" to \"%s\".\n",
                           passwd->name, oldgecos, mc->d.pw.pw_gecos);
                }
                /* We update the pw_gecos entry of passwd so it now points into
                 * the buffer from mc. This is safe for us, since we know we
                 * won't free the data in mc until after we are done.
                 */
        ...
        if (!keephome(specialusers, passwd->id))
            if ((passwd->d.pw.pw_dir == NULL) ||
                (strcmp(passwd->d.pw.pw_dir, mc->d.pw.pw_dir) != 0))
            {
                if (opt_verbose)
                {
                    const char* olddir =
                        passwd->d.pw.pw_dir ? passwd->d.pw.pw_dir : "(none)";
                    printf("Changing home-directory of %s from %s to %s\n",
                           passwd->name, olddir, mc->d.pw.pw_dir);
                }
                /* We update the pw_dir entry of passwd so it now points into
                 * the buffer from mc. This is safe for us, since we know we
                 * won't free the data in mc until after we are done.
                 */
        ...
        if (!keepshell(specialusers, passwd->id))
            if ((passwd->d.pw.pw_shell == NULL) ||
                (strcmp(passwd->d.pw.pw_shell, mc->d.pw.pw_shell) != 0))
            {
                if (opt_verbose)
                {
                    const char* oldshell = passwd->d.pw.pw_shell
                                               ? passwd->d.pw.pw_shell
                                               : "(none)";
                    printf("Changing shell of %s from %s to %s\n", passwd->name,
                           oldshell, mc->d.pw.pw_shell);
                }
                /* We update the pw_shell entry of passwd so it now points into
                 * the buffer from mc. This is safe for us, since we know we
                 * won't free the data in mc until after we are done.
                 */
...
int main(int argc, char** argv)
{
    ...
    while ((optc = getopt_long(argc, argv, "g:p:G:P:S:snvLhV", options,
                               &opt_index)) != -1)
        switch (optc)
        {
            case 'p':
    ...
    process_moved_entries(specialgroups, &system_groups, master_groups,
                          "group");
    ...
    process_moved_entries(specialusers, &system_accounts, master_accounts,
                          "user");
    process_new_entries(specialusers, &system_accounts, master_accounts,
                        "user");
    process_old_entries(specialusers, &system_accounts, master_accounts,
                        "user");
