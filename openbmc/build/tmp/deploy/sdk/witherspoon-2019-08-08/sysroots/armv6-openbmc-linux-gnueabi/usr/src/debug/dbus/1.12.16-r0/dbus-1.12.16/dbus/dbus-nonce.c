...
/* dbus-nonce.c  Nonce handling functions used by nonce-tcp (internal to D-Bus
 * implementation)
 *
 * Copyright (C) 2009 Klaralvdalens Datakonsult AB, a KDAB Group company,
 * info@kdab.net
 *
...
static dbus_bool_t do_check_nonce(DBusSocket fd, const DBusString* nonce,
                                  DBusError* error)
{
    DBusString buffer;
    DBusString p;
    ...
    while (nleft)
    {
        ...
        if (n == -1 && _dbus_get_is_errno_eintr(saved_errno))
            ;
                 ...
                 _dbus_get_is_errno_eagain_or_ewouldblock(saved_errno))
        ...
        {
            if (!_dbus_string_append_len(&buffer,
                                         _dbus_string_get_const_data(&p), n))
            {
                dbus_set_error(error, DBUS_ERROR_NO_MEMORY, NULL);
                _dbus_string_free(&p);
                _dbus_string_free(&buffer);
 ...
 * @param nonce returns the nonce. Must be an initialized string, the nonce will
 * be appended.
 * @param error error object to report possible errors
 * @return FALSE iff reading the nonce fails (error is set then)
 */
dbus_bool_t _dbus_read_nonce(const DBusString* fname, DBusString* nonce,
                             DBusError* error)
{
    FILE* fp;
    char buffer[17];
                                       ...
                                       const DBusNonceFile* noncefile)
{
    ...
    if (_dbus_read_nonce(_dbus_noncefile_get_path(noncefile), &nonce, NULL) !=
        TRUE)
        goto out;
...
static dbus_bool_t generate_and_write_nonce(const DBusString* filename,
                                            DBusError* error)
{
    DBusString nonce;
    dbus_bool_t ret;
 ...
 */
dbus_bool_t _dbus_send_nonce(DBusSocket fd, const DBusString* noncefile,
                             DBusError* error)
{
    ...
    send_result =
        _dbus_write_socket(fd, &nonce, 0, _dbus_string_get_length(&nonce));
    ...
    if (send_result == -1)
    {
        dbus_set_error(error, _dbus_error_from_system_errno(),
                       ...
                       _dbus_socket_printable(fd), _dbus_strerror_from_errno());
        return FALSE;
    }
...
static dbus_bool_t do_noncefile_create(DBusNonceFile* noncefile,
                                       DBusError* error, dbus_bool_t use_subdir)
{
    ...
    if (!_dbus_string_init(&noncefile->dir) || tmp == NULL ||
    ...
    if (use_subdir)
    {
        if (!_dbus_string_append(&noncefile->dir, "/dbus_nonce-") ||
            !_dbus_string_append(&noncefile->dir,
                                 _dbus_string_get_const_data(&randomStr)))
        {
            dbus_set_error(error, DBUS_ERROR_NO_MEMORY, NULL);
            goto on_error;
        }
    ...
    else
    {
        if (!_dbus_string_init(&noncefile->path) ||
            ...
            !_dbus_string_append(&noncefile->path,
                                 _dbus_string_get_const_data(&randomStr)))
        {
            dbus_set_error(error, DBUS_ERROR_NO_MEMORY, NULL);
            goto on_error;
    ...
    if (!generate_and_write_nonce(&noncefile->path, error))
    {
        ...
        if (use_subdir)
            _dbus_delete_directory(
                &noncefile->dir,
                NULL); // we ignore possible errors deleting the dir and return
...
#ifdef DBUS_WIN
/**
 * creates a nonce file in a user-readable location and writes a generated nonce
 * to it
 *
...
/**
 * creates a nonce file in a user-readable location and writes a generated nonce
 * to it. Initializes the noncefile object.
 *
 * @param noncefile returns the nonce file location
 * @param error error details if creating the nonce file fails
 * @return TRUE iff the nonce file was successfully created
