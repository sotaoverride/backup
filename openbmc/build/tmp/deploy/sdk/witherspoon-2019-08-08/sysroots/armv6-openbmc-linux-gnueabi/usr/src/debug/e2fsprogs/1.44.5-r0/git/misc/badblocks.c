...
static void usage(void)
{
    fprintf(
        stderr,
        _("Usage: %s [-b block_size] [-i input_file] [-o output_file] "
          "[-svwnfBX]\n"
...
static char* time_diff_format(struct timeval* tv1, struct timeval* tv2,
                              char* buf)
{
...
#define MILISEC (1000L)
    ...
    if (ext2fs_llseek(dev, (ext2_loff_t)current_block * block_size, SEEK_SET) !=
        (ext2_loff_t)current_block * block_size)
...
static void flush_bufs(void)
{
    ...
    if (retval)
        com_err(program_name, retval, "%s", _("during ext2fs_sync_device"));
}

static unsigned int test_ro(int dev, blk_t last_block, int block_size,
                            blk_t first_block, unsigned int blocks_at_once)
{
    ...
    if (t_flag)
    {
        ...
        pattern_fill(blkbuf + blocks_at_once * block_size, t_patts[0],
                     block_size);
    }
    flush_bufs();
    try
    ...
    while (currently_testing < last_block)
    {
        ...
        if (next_bad)
        {
            if (currently_testing == next_bad)
            {
                ...
                try
                    = next_bad - currently_testing;
        }
        if (currently_testing + try > last_block)
            try
                = last_block - currently_testing;
        ...
        if (t_flag)
        {
            ...
            for (i = 0; i < got; ++i)
                if (memcmp(blkbuf + i * block_size,
                           blkbuf + blocks_at_once * block_size, block_size))
                    bb_count +=
                        bb_output(currently_testing + i, CORRUPTION_ERROR);
        }
        if (got == 0 && try == 1)
        ...
        if (got != try)
        {
            try
                = 1;
            if (recover_block == ~0U)
                recover_block = currently_testing - got + blocks_at_once;
            continue;
        }
...
static unsigned int test_rw(int dev, blk_t last_block, int block_size,
                            blk_t first_block, unsigned int blocks_at_once)
{
    ...
    for (pat_idx = 0; pat_idx < nr_pattern; pat_idx++)
    {
        ...
        while (currently_testing < last_block)
        {
            ...
            if (currently_testing + try > last_block)
                try
                    = last_block - currently_testing;
            got = do_write(dev, buffer, try, block_size, currently_testing);
            if (v_flag > 1)
                print_status();
        ...
        while (currently_testing < last_block)
        {
            ...
            if (currently_testing + try > last_block)
                try
                    = last_block - currently_testing;
            got = do_read(dev, read_buffer, try, block_size, currently_testing);
            if (got == 0 && try == 1)
                bb_count += bb_output(currently_testing++, READ_ERROR);
            currently_testing += got;
            if (got != try)
            ...
            for (i = 0; i < got; i++)
            {
                if (memcmp(read_buffer + i * block_size,
                           buffer + i * block_size, block_size))
                    bb_count +=
                        bb_output(currently_testing + i, CORRUPTION_ERROR);
            }
            if (v_flag > 1)
...
static unsigned int test_nd(int dev, blk_t last_block, int block_size,
                            blk_t first_block, unsigned int blocks_at_once)
{
    ...
    if (setjmp(terminate_env))
    {
        ...
        {
            do_write(dev, save_ptr, test_record[i].num, block_size,
                     test_record[i].block);
    ...
    for (pat_idx = 0; pat_idx < nr_pattern; pat_idx++)
    {
        ...
        while (currently_testing < last_block)
        {
            ...
            if (currently_testing + try > last_block)
                try
                    = last_block - currently_testing;
            got = do_read(dev, save_ptr, try, block_size, currently_testing);
            if (got == 0)
            {
                if (recover_block == ~0U)
                    recover_block = currently_testing + blocks_at_once;
                if (granularity != 1)
                {
                    granularity = 1;
                    continue;
            ...
            /* Write the test data */
            written =
                do_write(dev, test_ptr, got, block_size, currently_testing);
            if (written != got)
                com_err(program_name, errno,
                        _("during test data write, block %lu"),
                        (unsigned long)currently_testing + written);
        ...
        check_for_more:
             ...
             */
            if ((buf_used != granularity) && (currently_testing < last_block))
                continue;
            ...
            while (1)
            {
                if (try == 0)
                {
                    ...
                    try
                        = test_record[used2].num;
                    used2++;
                }

                got =
                    do_read(dev, read_ptr, try, block_size, currently_testing);
                ...
                do_write(dev, save_ptr, got, block_size, currently_testing);
                save_ptr += got * block_size;
...
int main(int argc, char** argv)
{
    ...
    unsigned int (*test_func)(int, blk_t, int, blk_t, unsigned int);
    int open_flag;
    long sysval;
    blk64_t inblk;
...
#define _SC_PAGESIZE _SC_PAGE_SIZE
    ...
    while ((c = getopt(argc, argv, "b:d:e:fi:o:svwnc:p:h:t:BX")) != EOF)
    {
        switch (c)
        {
            ...
            case 'd':
                ...
                break;
            case 'p':
                num_passes = parse_uint(optarg, "number of clean passes");
                break;
            case 'h':
            ...
            case 't':
                if (t_flag + 1 > t_max)
                {
                    ...
                    t_patts_new =
                        realloc(t_patts, sizeof(int) * (t_max + T_INC));
                    if (!t_patts_new)
                    {
    ...
    if (optind > argc - 1)
    {
        errcode = ext2fs_get_device_size2(device_name, block_size, &last_block);
    ...
    {
        com_err(program_name, 0,
                _("invalid starting block (%llu): must be less than %llu"),
    ...
    if (dev == -1)
    {
        com_err(program_name, errno, _("while trying to open %s"), device_name);
        exit(1);
    }
    if (host_device_name)
    {
        ...
        for (;;)
        {
            switch (fscanf(in, "%llu\n", &inblk))
            {
                ...
                default:
                    if (inblk >> 32)
                    {
                        com_err(program_name, EOVERFLOW, "%s",
    ...
    do
    {
        ...
        bb_count =
            test_func(dev, last_block, block_size, first_block, blocks_at_once);
        if (bb_count)
            passes_clean = 0;
        else
            ++passes_clean;
