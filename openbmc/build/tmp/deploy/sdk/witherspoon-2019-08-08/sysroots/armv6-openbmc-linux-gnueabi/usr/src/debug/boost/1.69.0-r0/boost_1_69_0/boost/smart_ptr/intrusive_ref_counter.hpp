...
struct thread_safe_counter
{
    ...
    static unsigned int
        load(boost::detail::atomic_count const& counter) BOOST_SP_NOEXCEPT
    {
        return static_cast<unsigned int>(static_cast<long>(counter));
    }

    static void
        increment(boost::detail::atomic_count& counter) BOOST_SP_NOEXCEPT
    {
        ++counter;
    }

    static unsigned int
        decrement(boost::detail::atomic_count& counter) BOOST_SP_NOEXCEPT
    {
...
void intrusive_ptr_add_ref(
    const intrusive_ref_counter<DerivedT, CounterPolicyT>* p) BOOST_SP_NOEXCEPT;
template <typename DerivedT, typename CounterPolicyT>
...
class intrusive_ref_counter
{
  ...
  public:
    ...
    intrusive_ref_counter&
        operator=(intrusive_ref_counter const&) BOOST_SP_NOEXCEPT
    {
        return *this;
    }
  ...
  protected:
    ...
    friend void intrusive_ptr_add_ref<DerivedT, CounterPolicyT>(
        const intrusive_ref_counter<DerivedT, CounterPolicyT>* p)
        BOOST_SP_NOEXCEPT;
