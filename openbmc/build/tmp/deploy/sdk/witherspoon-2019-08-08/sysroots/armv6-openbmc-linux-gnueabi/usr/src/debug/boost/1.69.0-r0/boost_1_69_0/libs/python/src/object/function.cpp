    ...
    m_nkeyword_values(0)
{
    if (names_and_defaults != 0)
    {
        ...
        for (unsigned i = 0; i < num_keywords; ++i)
        {
            ...
            PyTuple_SET_ITEM(m_arg_names.ptr(), i + keyword_offset,
                             incref(kv.ptr()));
        }
    }
...
PyObject* function::call(PyObject* args, PyObject* keywords) const
{
    ...
    do
    {
            ...
            n_actual <= max_arity)
        {
                ...
                || n_actual < min_arity) // or default keyword values are needed
            {
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        assert(max_arity <=
                               static_cast<std::size_t>(ssize_t_max));
                        inner_args = handle<>(
                        ...
                        // Grab remaining arguments by name from the keyword
                        // dictionary
                             ...
                             arg_pos < max_arity; ++arg_pos)
                        {
                            ...
                            PyTuple_SET_ITEM(inner_args.get(), arg_pos,
                                             incref(value));
                        }
            ...
            PyObject* result =
                inner_args ? f->m_fn(inner_args.get(), keywords) : 0;
                                ...
                                object const& attribute, char const* doc)
{
    ...
    if (attribute.ptr()->ob_type == &function_type)
    {
        ...
        if (PyClass_Check(ns))
            ...
            dict = handle<>(
                PyObject_GetAttrString(ns, const_cast<char*>("__dict__")));

        if (dict == 0)
            throw_error_already_set();

        handle<> existing(
            allow_null(::PyObject_GetItem(dict.get(), name.ptr())));

        if (existing)
        {
            if (existing->ob_type == &function_type)
            {
                new_func->add_overload(handle<function>(
...
struct bind_return
{
    bind_return(PyObject*& result, function const* f, PyObject* args,
                PyObject* keywords) :
        m_result(result),
        m_f(f), m_args(args), m_keywords(keywords)
    {
    }
...
static PyObject* function_call(PyObject* func, PyObject* args, PyObject* kw)
{
    PyObject* result = 0;
    handle_exception(
        bind_return(result, static_cast<function*>(func), args, kw));
    return result;
}
