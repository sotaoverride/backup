...
static const struct
{
    ...
    {FC_DECORATIVE_OBJECT, FcFalse},      {FC_SYMBOL_OBJECT, FcFalse},
    {FC_VARIABLE_OBJECT, FcFalse},
};

#define NUM_FC_BOOL_DEFAULTS                                                   \
    (int)(sizeof FcBoolDefaults / sizeof FcBoolDefaults[0])
...
void FcDefaultSubstitute(FcPattern* pattern)
{
        ...
        FcResultMatch)
    {
        FcRange* r;
        double b, e;
        if (FcPatternObjectGetRange(pattern, FC_SIZE_OBJECT, 0, &r) ==
                FcResultMatch &&
            FcRangeGetDouble(r, &b, &e))
            size = (b + e) * .5;
        else
            size = 12.0L;
    }
