...
int STRFROM(char* dest, size_t size, const char* format, FLOAT f)
{
    ...
    if (size == 0)
    {
        /* When size is zero, nothing is written and dest may be a null pointer.
           This is specified for snprintf in ISO/IEC 9899:2011,
           Section 7.21.6.5, in the second paragraph.  Thus, if size is zero,
           prepare to use the overflow buffer right from the start.  */
    ...
    if (info.spec != 'a' && info.spec != 'A')
            ...
            __printf_fp_l(&sfile.f._sbf._f, _NL_CURRENT_LOCALE, &info, &fpptr);
    else
