...
/*!
  \file
  \brief Provides class \ref pool: a fast memory allocator that guarantees
  proper alignment of all allocated chunks, and which extends and generalizes
  the framework provided by the simple segregated storage solution. Also
  provides two UserAllocator classes which can be used in conjuction with \ref
  pool.
*/
...
//! a <a
//! href="boost_pool/pool/pooling.html#boost_pool.pool.pooling.user_allocator">UserAllocator</a>
//! template parameter.  Uses new and delete.
struct default_user_allocator_new_delete
{
    typedef std::size_t
        size_type; //!< An unsigned integral type that can represent the size of
                   //!< the largest object to be allocated.
    typedef std::ptrdiff_t
        difference_type; //!< A signed integral type that can represent the
                         //!< difference of any two pointers.

    static char* malloc BOOST_PREVENT_MACRO_SUBSTITUTION(const size_type bytes)
    { //! Attempts to allocate n bytes from the system. Returns 0 if
      //! out-of-memory
        return new (std::nothrow) char[bytes];
    }
    static void free BOOST_PREVENT_MACRO_SUBSTITUTION(char* const block)
    { //! Attempts to de-allocate block.
        //! \pre Block must have been previously returned from a call to
...
{ //! PODptr is a class that pretends to be a "pointer" to different class types
    //!  that don't really exist.  It provides member functions to access the
    //!  "data" of the "object" it points to.  Since these "class" types are of
    //!  variable size, and contains some information at the *end* of its memory
    //!  (for alignment reasons),
    //! PODptr must contain the size of this "class" as well as the pointer to
    //! this "object".

    /*! \details A PODptr holds the location and size of a memory block
  allocated from the system. Each memory block is split logically into three
  sections:

    <b>Chunk area</b>. This section may be different sizes. PODptr does not care
  what the size of the chunks is, but it does care (and keep track of) the total
  size of the chunk area.

    <b>Next pointer</b>. This section is always the same size for a given
  SizeType. It holds a pointer to the location of the next memory block in the
  memory block list, or 0 if there is no such block.

    <b>Next size</b>. This section is always the same size for a given SizeType.
  It holds the size of the next memory block in the memory block list.

  The PODptr class just provides cleaner ways of dealing with raw memory blocks.

  A PODptr object is either valid or invalid. An invalid PODptr is analogous to
  a null pointer. The default constructor for PODptr will result in an invalid
  object. Calling the member function invalidate will result in that object
  becoming invalid. The member function valid can be used to test for validity.
  */
  public:
  ...
  public:
    ...
    { //!
        //! \returns next_size.
        return *(
            static_cast<size_type*>(static_cast<void*>((ptr_next_size()))));
    }
    char*& next_ptr() const
...
/*!
  ...
  <b>UserAllocator type</b> - the method that the Pool will use to allocate
  memory from the system.

  There are essentially two ways to use class pool: the client can call \ref
  malloc() and \ref free() to allocate and free single chunks of memory, this is
  the most efficient way to use a pool, but does not allow for the efficient
  allocation of arrays of chunks.  Alternatively, the client may call \ref
  ordered_malloc() and \ref ordered_free(), in which case the free list is
  maintained in an ordered state, and efficient allocation of arrays of chunks
  are possible.  However, this latter option can suffer from poor performance
    ...
    : protected simple_segregated_storage<typename UserAllocator::size_type>
{
  ...
  private:
    BOOST_STATIC_CONSTANT(
        size_type, min_alloc_size = (::boost::integer::static_lcm<
                                     sizeof(void*), sizeof(size_type)>::value));
    BOOST_STATIC_CONSTANT(
        size_type,
        min_align = (::boost::integer::static_lcm<
                     ::boost::alignment_of<void*>::value,
                     ::boost::alignment_of<size_type>::value>::value));

    //! \returns 0 if out-of-memory.
    //! Called if malloc/ordered_malloc needs to resize the free list.
    void* malloc_need_resize(); //! Called if malloc needs to resize the free
                                //! list.
    void* ordered_malloc_need_resize(); //! Called if ordered_malloc needs to
  ...
  protected:
    ...
    { //! \param chunk chunk to check if is from this pool.
        ...
        //! not the total size of that block). \returns true if chunk was
        //! allocated or may be returned. as the result of a future allocation.
        //!
        ...
        // The functor objects guarantee a total order for any pointer
        // [20.3.3/8]
    ...
    { //!  Calculated size of the memory chunks that will be allocated by this
      //!  Pool.
        //! \returns allocated size.
        // For alignment reasons, this used to be defined to be
        // lcm(requested_size, sizeof(void *), sizeof(size_type)), but is now
        // more parsimonious: just rounding up to the minimum required alignment
        // of our housekeeping data when required.  This works provided all
    ...
    { //! Calculated maximum number of memory chunks that can be allocated in a
      //! single call by this Pool.
        size_type partition_size = alloc_size();
        size_type POD_size =
            integer::static_lcm<sizeof(size_type), sizeof(void*)>::value +
            sizeof(size_type);
        size_type max_chunks =
            (std::numeric_limits<size_type>::max() - POD_size) / alloc_size();
  ...
  public:
    ...
    explicit pool(const size_type nrequested_size,
        ...
        start_size(nnext_size), max_size(nmax_size)
    ...
    { //! Number of chunks to request from the system the next time that object
    ...
    { //! Set number of chunks to request from the system the next time that
      //! object needs to allocate system memory. This value should never be set
      //! to 0.
        BOOST_USING_STD_MIN();
        next_size = start_size =
            min BOOST_PREVENT_MACRO_SUBSTITUTION(nnext_size, max_chunks());
    }
    ...
    { //! Set max_size.
        BOOST_USING_STD_MIN();
        max_size =
            min BOOST_PREVENT_MACRO_SUBSTITUTION(nmax_size, max_chunks());
    }
    ...
    { //! Allocates a chunk of memory. Searches in the list of memory blocks
        ...
        //! pool's free list, \returns a free chunk from that block. If a new
        //! memory block cannot be allocated, returns 0. Amortized O(1).
    ...
    void* ordered_malloc()
    { //! Same as malloc, only merges the free lists, to preserve order.
      //! Amortized O(1).
        //! \returns a free chunk from that block.
        //! If a new memory block cannot be allocated, returns 0. Amortized
        //! O(1).
        // Look for a non-empty storage
        if (!store().empty())
            return (store().malloc)();
    ...
    //! Same as malloc, only allocates enough contiguous chunks to cover n *
    //! requested_size bytes. Amortized O(n). \returns a free chunk from that
    //! block. If a new memory block cannot be allocated, returns 0. Amortized
    //! O(1).
    ...
    { //!   Deallocates a chunk of memory. Note that chunk may not be 0. O(1).
        //!
        //! Chunk must have been previously returned by t.malloc() or
        //! t.ordered_malloc(). Assumes that chunk actually refers to a block of
        //! chunks spanning n * partition_sz bytes. deallocates each chunk in
        //! that block. Note that chunk may not be 0. O(n).
        (store().free)(chunk);
    }
    ...
    { //! Same as above, but is order-preserving.
        //!
        //! Note that chunk may not be 0. O(N) with respect to the size of the
        //! free list. chunk must have been previously returned by t.malloc() or
        //! t.ordered_malloc().
    ...
    void free BOOST_PREVENT_MACRO_SUBSTITUTION(void* const chunks,
                                               const size_type n)
    ...
    { //! Assumes that chunk actually refers to a block of chunks spanning n *
      //! partition_sz bytes;
        //! deallocates each chunk in that block.
        //!
        //! Note that chunk may not be 0. Order-preserving. O(N + n) where N is
        //! the size of the free list. chunk must have been previously returned
        //! by t.malloc() or t.ordered_malloc().
    ...
    { //! \returns Returns true if chunk was allocated from u or
        ...
        //! function may not be used to reliably test random pointer values.
...
template <typename UserAllocator>
typename pool<UserAllocator>::size_type const
    pool<UserAllocator>::min_alloc_size;
template <typename UserAllocator>
...
{ //! pool must be ordered. Frees every memory block that doesn't have any
  //! allocated chunks.
    ...
    // This is a current & previous iterator pair over the free memory chunk
    // list
    ...
    while (ptr.valid())
    {
        ...
        // We have to check all the chunks.  If they are *all* free (i.e.,
        // present
        ...
        // post: if the memory block has any chunks, free_p points to one of
        // them otherwise, our assertions above are still valid
        ...
        if (!all_chunks_free)
        {
            ...
            //     free_p points to the first free chunk in some next memory
            //     block, or
            //       0 if there is no such chunk.
            //     prev_free_p points to the last free chunk in this memory
            //     block.
...
{ //! No memory in any of our storages; make a new storage,
    ...
    size_type partition_size = alloc_size();
    size_type POD_size = static_cast<size_type>(
        next_size * partition_size +
        integer::static_lcm<sizeof(size_type), sizeof(void*)>::value +
        sizeof(size_type));
    ...
    if (ptr == 0)
    {
        if (next_size > 4)
        {
            ...
            POD_size = static_cast<size_type>(
                next_size * partition_size +
                integer::static_lcm<sizeof(size_type), sizeof(void*)>::value +
                sizeof(size_type));
    ...
    if (!max_size)
        ...
        set_next_size(min BOOST_PREVENT_MACRO_SUBSTITUTION(
            next_size << 1, max_size * requested_size / partition_size));
...
{ //! No memory in any of our storages; make a new storage,
    ...
    store().add_ordered_block(node.begin(), node.element_size(),
                              partition_size);
...
{ //! Gets address of a chunk n, allocating new memory if not already available.
...
#ifdef BOOST_POOL_INSTRUMENT
    std::cout << "Allocating " << n << " chunks from pool of size "
    ...
    if (ptr == 0)
    {
        ...
        {
            // Try again with just enough memory to do the job, or at least
            // whatever we allocated last time:
            next_size >>= 1;
            next_size =
                max BOOST_PREVENT_MACRO_SUBSTITUTION(next_size, num_chunks);
            POD_size = static_cast<size_type>(
                next_size * partition_size +
                integer::static_lcm<sizeof(size_type), sizeof(void*)>::value +
                sizeof(size_type));
    ...
    if (next_size > num_chunks)
        store().add_ordered_block(
            node.begin() + num_chunks * partition_size,
    ...
    else
    {
        ...
        while (true)
        {
            // if we're about to hit the end, or if we've found where "node"
            // goes.
            if (prev.next_ptr() == 0 ||
                std::greater<void*>()(prev.next_ptr(), node.begin()))
                break;
...
class pool
{
  public:
    ...
    explicit pool(const size_type s, const size_type = 32,
                  const size_type m = 0) :
        chunk_size(s),
        max_alloc_size(m)
    {
    ...
    bool release_memory()
    {
        ...
        for (std::set<void*>::iterator pos = free_list.begin();
             pos != free_list.end(); ++pos)
        {
    ...
    bool purge_memory()
    {
        ...
        free_list.clear();
        for (std::set<void*>::iterator pos = used_list.begin();
             pos != used_list.end(); ++pos)
        {
            (user_allocator::free)(static_cast<char*>(*pos));
        }
        used_list.clear();
