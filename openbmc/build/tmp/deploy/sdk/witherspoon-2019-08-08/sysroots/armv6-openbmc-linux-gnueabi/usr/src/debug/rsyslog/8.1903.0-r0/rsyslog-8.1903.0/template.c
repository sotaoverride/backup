...
static struct cnfparamblk pblkProperty = {CNFPARAMBLK_VERSION,
                                          sizeof(cnfparamdescrProperty) /
...
static struct cnfparamdescr cnfparamdescrConstant[] = {
    ...
    {"format", eCmdHdlrString, 0},
    {"outname", eCmdHdlrString, 0}};
static struct cnfparamblk pblkConstant = {CNFPARAMBLK_VERSION,
                                          sizeof(cnfparamdescrConstant) /
                                              sizeof(struct cnfparamdescr),
                                          cnfparamdescrConstant};

#ifdef FEATURE_REGEXP
DEFobjCurrIf(regexp) static int bFirstRegexpErrmsg =
    1; /**< did we already do a "can't load regexp" error message? */
#endif

/* helper to tplToString and strgen's, extends buffer */
#define ALLOC_INC 128
rsRetVal ExtendBuf(actWrkrIParams_t* __restrict__ const iparam,
                   const size_t iMinSize)
{
    uchar* pNewBuf;
    size_t iNewSize;
    DEFiRet;
                     ...
                     struct syslogTime* const ttNow)
{
    ...
    if (pTpl->bHaveSubtree)
    {
        ...
        if (iLenVal >= (rs_size_t)iparam
                           ->lenBuf) /* we reserve one char for the final \0! */
    ...
    while (pTpe != NULL)
    {
        ...
        else if (pTpe->eEntryType == FIELD)
        {
             ...
             * rgerhards, 2005-09-22: the option values below look somewhat
             * misplaced, but they are handled in this way because of legacy
             * (don't break any existing thing).
             */
        ...
        if (iLenVal > 0)
        { /* may be zero depending on property */
            /* first, make sure buffer fits */
            if (iBuf + iLenVal + extra_space >=
                iparam->lenBuf) /* we reserve one char for the final \0! */
 ...
 */
rsRetVal tplToJSON(struct template* pTpl, smsg_t* pMsg,
                   struct json_object** pjson, struct syslogTime* ttNow)
{
    ...
    for (pTpe = pTpl->pEntryRoot; pTpe != NULL; pTpe = pTpe->pNext)
    {
        if (pTpe->eEntryType == CONSTANT)
        {
            if (pTpe->fieldName == NULL)
                continue;
            jsonf =
                json_object_new_string((char*)pTpe->data.constant.pConstant);
        ...
        else if (pTpe->eEntryType == FIELD)
        {
            ...
            {
                localRet =
                    msgGetJSONPropJSON(pMsg, &pTpe->data.field.msgProp, &jsonf);
                if (localRet == RS_RET_OK)
                {
                    json_object_object_add(json, (char*)pTpe->fieldName,
                                           json_object_get(jsonf));
                }
                else
                {
            ...
            else
            {
                ...
                {
                    jsonf =
                        json_object_new_string_len((char*)pVal, propLen + 1);
                    json_object_object_add(json, (char*)pTpe->fieldName, jsonf);
                }
                if (bMustBeFreed)
                { /* json-c makes its own private copy! */
                    free(pVal);
                }
            }
        }
 ...
 */
rsRetVal doEscape(uchar** pp, rs_size_t* pLen, unsigned short* pbMustBeFreed,
                  int mode)
{
    DEFiRet;
    uchar* p = NULL;
    ...
    while (*p)
    {
        ...
        {
            CHKiRet(
                cstrAppendChar(pStrB, (mode == STDSQL_ESCAPE) ? '\'' : '\\'));
 ...
 */
static rsRetVal do_Constant(unsigned char** pp, struct template* pTpl,
                            int bDoEscapes)
{
    ...
    CHKiRet(
        cstrConvSzStrAndDestruct(&pStrB, &pTpe->data.constant.pConstant, 0));
...
{
    return (pTpe->data.field.options.bCSV || pTpe->data.field.options.bJSON ||
            pTpe->data.field.options.bJSONf || pTpe->data.field.options.bJSONr);
}
        ...
        else if (!strcmp((char*)Buf, "csv"))
        {
            if (hasFormat(pTpe))
            {
                LogError(0, NO_ERRCODE,
                         ...
                         "one option out of (json, jsonf, jsonr, jsonfr, csv) "
                         "- csv ignored");
            }
            else
            {
...
static rsRetVal do_Parameter(uchar** pp, struct template* pTpl)
{
    ...
    CHKiRet(msgPropDescrFill(&pTpe->data.field.msgProp,
                             cstrGetSzStrNoNULL(pStrProp), cstrLen(pStrProp)));
    ...
    if (*p == ':')
    {
        ...
        if (*p == 'R')
        {
            ...
            if (*p == ',')
            {
                ...
                if (p[0] == 'D' && p[1] == 'F' && p[2] == 'L' && p[3] == 'T' &&
                    (p[4] == ',' || p[4] == ':'))
                {
                    pTpe->data.field.nomatchAction =
                        TPL_REGEX_NOMATCH_USE_DFLTSTR;
                    p += 4; /* eat indicator sequence */
                }
                else if (p[0] == 'B' && p[1] == 'L' && p[2] == 'A' &&
                         p[3] == 'N' && p[4] == 'K' &&
                         (p[5] == ',' || p[5] == ':'))
                {
                    pTpe->data.field.nomatchAction =
                        TPL_REGEX_NOMATCH_USE_BLANK;
                    p += 5; /* eat indicator sequence */
                }
                else if (p[0] == 'F' && p[1] == 'I' && p[2] == 'E' &&
                         p[3] == 'L' && p[4] == 'D' &&
                         (p[5] == ',' || p[5] == ':'))
                {
                    pTpe->data.field.nomatchAction =
                ...
                { /* empty, use default */
                    pTpe->data.field.nomatchAction =
                        TPL_REGEX_NOMATCH_USE_DFLTSTR;
                    /* do NOT eat indicator sequence, as this was already eaten
                     * - the comma itself is already part of the next field.
                     */
                }
                else
                {
        ...
        else
        {
                ...
                else if (*p == ',')
                {
                    ...
                    if (!isdigit((int)*p))
                    {
                        ...
                        LogError(0, NO_ERRCODE,
                                 ...
                                 "frompos after \"F,\", property: '%%%s' - "
                                 "using 9 (HT) as field delimiter",
                                 (char*)*pp);
                        pTpe->data.field.field_delim = 9;
                    }
                    else
                    {
                        ...
                        if (iNum < 0 || iNum > 255)
                        {
                            LogError(0, NO_ERRCODE,
                                     ...
                                     "character value %d in template - using 9 "
                                     "(HT) as substitute",
                                     iNum);
                            pTpe->data.field.field_delim = 9;
                        }
            ...
            {
                /* we now have a simple offset in frompos (the previously
                 * "normal" case) */
                iNum = 0;
                while (isdigit((int)*p))
                    iNum = iNum * 10 + *p++ - '0';
    ...
    if (*p == ':')
    {
        ...
        if (pTpe->data.field.has_regex)
        {
            ...
            else
            {
                ...
                if ((iRetLocal = objUse(regexp, LM_REGEXP_FILENAME)) ==
                    RS_RET_OK)
                {
                    int iOptions;
                    iOptions = (pTpe->data.field.typeRegex == TPL_REGEX_ERE)
                                   ? REG_EXTENDED
                                   : 0;
                    int errcode;
                    if ((errcode =
                             regexp.regcomp(&(pTpe->data.field.re),
                                            (char*)regex_char, iOptions) != 0))
                    {
                        ...
                        DBGPRINTF(
                            "Template.c: Error in regular expression: %s\n",
                            errbuff);
                ...
                else
                {
                    ...
                    dbgprintf("error %d trying to load regexp library - this "
                              "may be desired "
                              "and thus OK",
                              iRetLocal);
                    if (bFirstRegexpErrmsg)
                    {
                        /* prevent flood of messages, maybe even an endless
                         * loop! */
    ...
    if (pStrField == NULL)
    {
        if (pTpe->data.field.msgProp.id == PROP_CEE ||
            pTpe->data.field.msgProp.id == PROP_LOCAL_VAR)
        {
            /* in CEE case, we remove "$!"/"$." from the fieldname - it's just
             * our indicator */
 ...
 * returns pointer to new object if it succeeds, NULL otherwise.
 */
struct template* tplAddLine(rsconf_t* conf, const char* pName,
                            uchar** ppRestOfConfLine)
{
    struct template* pTpl;
    unsigned char* p;
    int bDone;
    ...
    switch (*p)
    {
        ...
        case '=':
            *ppRestOfConfLine = p + 1;
            ...
            {
                LogError(0, localRet,
                         "Template '%s': error %d defining template via strgen "
                         "module",
    ...
    while (*p)
    {
        ...
        /* read option word */
        char optBuf[128] = {
            '\0'}; /* buffer for options - should be more than enough... */
        i = 0;
        while ((i < (sizeof(optBuf) - 1)) && *p && *p != '=' && *p != ',' &&
               *p != '\n')
        {
...
static rsRetVal createConstantTpe(struct template* pTpl, struct cnfobj* o)
{
    struct templateEntry* pTpe;
    es_str_t* value =
        NULL; /* init just to keep compiler happy - mandatory parameter */
    int i;
    int is_jsonf = 0;
    ...
    for (i = 0; i < pblkConstant.nParams; ++i)
    {
        ...
        {
            if (!es_strbufcmp(pvals[i].val.d.estr, (uchar*)"jsonf",
                              sizeof("jsonf") - 1))
            {
                is_jsonf = 1;
            }
    ...
    {
        parser_errmsg("constant set to format jsonf, but outname not specified "
                      "- aborting");
        ABORT_FINALIZE(RS_RET_ERR);
    }
    ...
    if (is_jsonf)
    {
        ...
        CHKmalloc(pTpe->data.constant.pConstant =
                      (uchar*)strndup(sz + 2, len_json));
        pTpe->data.constant.iLenConstant =
            ustrlen(pTpe->data.constant.pConstant);
...
static rsRetVal createPropertyTpe(struct template* pTpl, struct cnfobj* o)
{
    ...
    for (i = 0; i < pblkProperty.nParams; ++i)
    {
        ...
        {
            bComplexProcessing = 1;
            if (!es_strbufcmp(pvals[i].val.d.estr, (uchar*)"BRE",
                              sizeof("BRE") - 1))
            {
                re_type = TPL_REGEX_BRE;
            }
            else if (!es_strbufcmp(pvals[i].val.d.estr, (uchar*)"ERE",
                                   sizeof("ERE") - 1))
            {
        ...
        {
            bComplexProcessing = 1;
            if (!es_strbufcmp(pvals[i].val.d.estr, (uchar*)"DFLT",
                              sizeof("DFLT") - 1))
            {
                re_nomatchType = TPL_REGEX_NOMATCH_USE_DFLTSTR;
            }
            else if (!es_strbufcmp(pvals[i].val.d.estr, (uchar*)"BLANK",
                                   sizeof("BLANK") - 1))
            {
                re_nomatchType = TPL_REGEX_NOMATCH_USE_BLANK;
            }
            else if (!es_strbufcmp(pvals[i].val.d.estr, (uchar*)"FIELD",
                                   sizeof("FIELD") - 1))
            {
                re_nomatchType = TPL_REGEX_NOMATCH_USE_WHOLE_FIELD;
            }
            else if (!es_strbufcmp(pvals[i].val.d.estr, (uchar*)"ZERO",
                                   sizeof("ZERO") - 1))
            {
        ...
        {
            bComplexProcessing = 1;
            if (!es_strbufcmp(pvals[i].val.d.estr, (uchar*)"csv",
                              sizeof("csv") - 1))
            {
                formatType = F_CSV;
            }
            else if (!es_strbufcmp(pvals[i].val.d.estr, (uchar*)"json",
                                   sizeof("json") - 1))
            {
                formatType = F_JSON;
            }
            else if (!es_strbufcmp(pvals[i].val.d.estr, (uchar*)"jsonf",
                                   sizeof("jsonf") - 1))
            {
                formatType = F_JSONF;
            }
            else if (!es_strbufcmp(pvals[i].val.d.estr, (uchar*)"jsonr",
                                   sizeof("jsonr") - 1))
            {
                formatType = F_JSONR;
            }
            else if (!es_strbufcmp(pvals[i].val.d.estr, (uchar*)"jsonfr",
                                   sizeof("jsonfr") - 1))
            {
                formatType = F_JSONFR;
            }
        ...
        {
            bComplexProcessing = 1;
            if (!es_strbufcmp(pvals[i].val.d.estr, (uchar*)"escape",
                              sizeof("escape") - 1))
            {
                controlchr = CC_ESCAPE;
            }
            else if (!es_strbufcmp(pvals[i].val.d.estr, (uchar*)"space",
                                   sizeof("space") - 1))
            {
                controlchr = CC_SPACE;
            }
            else if (!es_strbufcmp(pvals[i].val.d.estr, (uchar*)"drop",
                                   sizeof("drop") - 1))
            {
                controlchr = CC_DROP;
            }
            else
            {
                ...
                LogError(0, RS_RET_ERR,
                         "invalid controlcharacter mode '%s' for property",
                         typeStr);
                free(typeStr);
                ABORT_FINALIZE(RS_RET_ERR);
            }
        ...
        {
            bComplexProcessing = 1;
            if (!es_strbufcmp(pvals[i].val.d.estr, (uchar*)"drop",
                              sizeof("drop") - 1))
            {
                secpath = SP_DROP;
            }
            else if (!es_strbufcmp(pvals[i].val.d.estr, (uchar*)"replace",
                                   sizeof("replace") - 1))
            {
                secpath = SP_REPLACE;
            }
            else
            {
                uchar* typeStr = (uchar*)es_str2cstr(pvals[i].val.d.estr, NULL);
                LogError(0, RS_RET_ERR,
                         "invalid securepath mode '%s' for property", typeStr);
                free(typeStr);
                ABORT_FINALIZE(RS_RET_ERR);
            }
        }
        ...
        {
            bComplexProcessing = 1;
            if (!es_strbufcmp(pvals[i].val.d.estr, (uchar*)"lower",
                              sizeof("lower") - 1))
            {
                caseconv = tplCaseConvLower;
            }
            else if (!es_strbufcmp(pvals[i].val.d.estr, (uchar*)"upper",
                                   sizeof("upper") - 1))
            {
            ...
            else
            {
                uchar* typeStr = (uchar*)es_str2cstr(pvals[i].val.d.estr, NULL);
                LogError(0, RS_RET_ERR,
                         "invalid caseconversion type '%s' for property",
                         typeStr);
                free(typeStr);
                ABORT_FINALIZE(RS_RET_ERR);
            }
        ...
        {
            if (!es_strbufcmp(pvals[i].val.d.estr, (uchar*)"mysql",
                              sizeof("mysql") - 1))
            {
                datefmt = tplFmtMySQLDate;
            }
            else if (!es_strbufcmp(pvals[i].val.d.estr, (uchar*)"pgsql",
                                   sizeof("pgsql") - 1))
            {
                datefmt = tplFmtPgSQLDate;
            }
            else if (!es_strbufcmp(pvals[i].val.d.estr, (uchar*)"rfc3164",
                                   sizeof("rfc3164") - 1))
            {
                datefmt = tplFmtRFC3164Date;
            }
            else if (!es_strbufcmp(pvals[i].val.d.estr,
                                   (uchar*)"rfc3164-buggyday",
                                   sizeof("rfc3164-buggyday") - 1))
            {
                datefmt = tplFmtRFC3164BuggyDate;
            }
            else if (!es_strbufcmp(pvals[i].val.d.estr, (uchar*)"rfc3339",
                                   sizeof("rfc3339") - 1))
            {
            ...
            else if (!es_strbufcmp(pvals[i].val.d.estr, (uchar*)"subseconds",
                                   sizeof("subseconds") - 1))
            {
                datefmt = tplFmtSecFrac;
            }
            else if (!es_strbufcmp(pvals[i].val.d.estr, (uchar*)"wdayname",
                                   sizeof("wdayname") - 1))
            {
                datefmt = tplFmtWDayName;
            }
            else if (!es_strbufcmp(pvals[i].val.d.estr, (uchar*)"wday",
                                   sizeof("wday") - 1))
            {
                datefmt = tplFmtWDay;
            }
            else if (!es_strbufcmp(pvals[i].val.d.estr, (uchar*)"year",
                                   sizeof("year") - 1))
            {
                datefmt = tplFmtYear;
            }
            else if (!es_strbufcmp(pvals[i].val.d.estr, (uchar*)"month",
                                   sizeof("month") - 1))
            {
                datefmt = tplFmtMonth;
            }
            else if (!es_strbufcmp(pvals[i].val.d.estr, (uchar*)"day",
                                   sizeof("day") - 1))
            {
                datefmt = tplFmtDay;
            }
            else if (!es_strbufcmp(pvals[i].val.d.estr, (uchar*)"hour",
                                   sizeof("hour") - 1))
            {
                datefmt = tplFmtHour;
            }
            else if (!es_strbufcmp(pvals[i].val.d.estr, (uchar*)"minute",
                                   sizeof("minute") - 1))
            {
                datefmt = tplFmtMinute;
            }
            else if (!es_strbufcmp(pvals[i].val.d.estr, (uchar*)"second",
                                   sizeof("second") - 1))
            {
                datefmt = tplFmtSecond;
            }
            else if (!es_strbufcmp(pvals[i].val.d.estr, (uchar*)"tzoffshour",
                                   sizeof("tzoffshour") - 1))
            {
                datefmt = tplFmtTZOffsHour;
            }
            else if (!es_strbufcmp(pvals[i].val.d.estr, (uchar*)"tzoffsmin",
                                   sizeof("tzoffsmin") - 1))
            {
                datefmt = tplFmtTZOffsMin;
            }
            else if (!es_strbufcmp(pvals[i].val.d.estr,
                                   (uchar*)"tzoffsdirection",
                                   sizeof("tzoffsdirection") - 1))
            {
                datefmt = tplFmtTZOffsDirection;
            }
            else if (!es_strbufcmp(pvals[i].val.d.estr, (uchar*)"ordinal",
                                   sizeof("ordinal") - 1))
            {
                datefmt = tplFmtOrdinal;
            }
            else if (!es_strbufcmp(pvals[i].val.d.estr, (uchar*)"week",
                                   sizeof("week") - 1))
            {
                datefmt = tplFmtWeek;
            }
    ...
    else
    {
        ...
        {
            LogError(0, RS_RET_ERR,
                     "position.to=%d is lower than postion.from=%d\n", topos,
                     frompos);
            ABORT_FINALIZE(RS_RET_ERR);
        }
    }
    ...
    pTpe->eEntryType = FIELD;
    CHKiRet(
        msgPropDescrFill(&pTpe->data.field.msgProp, name, strlen((char*)name)));
    ...
    if (re_expr != NULL)
    {
        ...
        if ((iRetLocal = objUse(regexp, LM_REGEXP_FILENAME)) == RS_RET_OK)
        {
            ...
            iOptions = (pTpe->data.field.typeRegex == TPL_REGEX_ERE)
                           ? REG_EXTENDED
                           : 0;
            if (regexp.regcomp(&(pTpe->data.field.re), (char*)re_expr,
                               iOptions) != 0)
            {
...
rsRetVal ATTR_NONNULL() tplProcessCnf(struct cnfobj* o)
{
    ...
    enum
    {
        ...
        T_STRING; /* init just to keep compiler happy: mandatory parameter */
    int i;
    int o_sql = 0, o_stdsql = 0, o_jsonf = 0, o_json = 0,
        o_casesensitive = 0; /* options */
    int numopts;
    rsRetVal localRet;
    DEFiRet;
    ...
    for (i = 0; i < pblk.nParams; ++i)
    {
        ...
        {
            if (!es_strbufcmp(pvals[i].val.d.estr, (uchar*)"string",
                              sizeof("string") - 1))
            {
                tplType = T_STRING;
            }
            else if (!es_strbufcmp(pvals[i].val.d.estr, (uchar*)"plugin",
                                   sizeof("plugin") - 1))
            {
                tplType = T_PLUGIN;
            }
            else if (!es_strbufcmp(pvals[i].val.d.estr, (uchar*)"list",
                                   sizeof("list") - 1))
            {
                tplType = T_LIST;
            }
            else if (!es_strbufcmp(pvals[i].val.d.estr, (uchar*)"subtree",
                                   sizeof("subtree") - 1))
            {
                tplType = T_SUBTREE;
            }
    ...
    if (name == NULL)
    {
        DBGPRINTF("template/tplProcessConf: logic error name == NULL - pblk "
                  "wrong?\n");
        ABORT_FINALIZE(RS_RET_ERR);
    }
...
struct template* tplFind(rsconf_t* conf, char* pName, int iLenName)
{
    ...
    while (pTpl != NULL &&
           !(pTpl->iLenName == iLenName && !strcmp(pTpl->pszName, pName)))
    {
        pTpl = pTpl->pNext;
    }
    return (pTpl);
...
void tplPrintList(rsconf_t* conf)
{
    ...
    while (pTpl != NULL)
    {
        ...
        while (pTpe != NULL)
        {
            dbgprintf("\tEntry(%lx): type %d, ", (unsigned long)pTpe,
                      pTpe->eEntryType);
            switch (pTpe->eEntryType)
            {
                ...
                case CONSTANT:
                    ...
                    break;
                case FIELD:
                    dbgprintf("(FIELD), value: '%d' ",
                              pTpe->data.field.msgProp.id);
                    if (pTpe->data.field.msgProp.id == PROP_CEE)
                    {
                        dbgprintf("[EE-Property: '%s'] ",
                                  pTpe->data.field.msgProp.name);
                    }
                    else if (pTpe->data.field.msgProp.id == PROP_LOCAL_VAR)
                    {
                        dbgprintf("[Local Var: '%s'] ",
                                  pTpe->data.field.msgProp.name);
                    }
                    switch (pTpe->data.field.eDateFormat)
                    {
                        ...
                        case tplFmtWeek:
                            ...
                            break;
                        default:
                            dbgprintf("[UNKNOWN eDateFormat %d] ",
                                      pTpe->data.field.eDateFormat);
                    }
                    ...
                    {
                        dbgprintf(
                            "[format as JSON field without re-escaping] ");
                    }
                    if (pTpe->data.field.options.bMandatory)
                    {
