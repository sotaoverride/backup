...
static int dt_cmp_subnodes(const struct pdbg_target* a,
                           const struct pdbg_target* b)
{
...
static const char* __dt_path_split(const char* p, const char** namep,
                                   unsigned int* namel, const char** addrp,
                                   unsigned int* addrl)
{
    const char *at, *sl;
                                            ...
                                            const char* name)
{
    ...
    list_for_each(&node->properties, i,
                  list) if (strcmp(i->name, name) == 0) return i;
    return NULL;
}
                                           ...
                                           size_t size)
{
     ...
     * when flattening
     */
    if (strcmp(name, "linux,phandle") == 0 || strcmp(name, "phandle") == 0)
    {
        assert(size == 4);
...
static const struct dt_property*
    dt_require_property(const struct pdbg_target* node, const char* name,
                        int wanted_len)
{
...
static int dt_expand_node(struct pdbg_target* node, const void* fdt,
                          int fdt_node)
{
    const struct fdt_property* prop;
    ...
    if (((err = fdt_check_header(fdt)) != 0) || (fdt_node < 0) ||
        (fdt_node % FDT_TAGSIZE) ||
