...
#include "case_N.h"
        ...
        if (cleanup_got_literal && (cleanup_fname || cleanup_fd_w != -1))
        {
                ...
                handle_partial_dir(cleanup_new_fname, PDIR_CREATE))
            {
                ...
                if (!partial_dir)
                {
                    /* We don't want to leave a partial file with a modern time
                     * or it could be skipped via --update.  Setting the time to
                     * something really old also helps it to stand out as
                     * unfinished in an ls. */
                    tweak_modtime = 1;
...
#include "case_N.h"
        ...
        if ((exit_code && line > 0) || am_daemon ||
            (logfile_name && (am_server || !INFO_GTE(STATS, 1))))
...
#include "case_N.h"
        ...
        if (exit_code && exit_code != RERR_SOCKETIO &&
