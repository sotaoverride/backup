...
static void print_dquot(const char* desc, struct dquot* dq)
{
    ...
    fprintf(stderr, "%u %lld:%lld:%lld %lld:%lld:%lld\n", dq->dq_id,
            (long long)dq->dq_dqb.dqb_curspace,
...
int quota_file_exists(ext2_filsys fs, enum quota_type qtype)
{
    ...
    ret = ext2fs_lookup(fs, EXT2_ROOT_INO, qf_name, strlen(qf_name), 0, &ino);
    if (ret)
        return 0;
...
void quota_set_sb_inum(ext2_filsys fs, ext2_ino_t ino, enum quota_type qtype)
{
    ...
    log_debug("setting quota ino in superblock: ino=%u, type=%d", ino, qtype);
    *inump = ino;
...
errcode_t quota_write_inode(quota_ctx_t qctx, unsigned int qtype_bits)
{
    ...
    if (retval)
    {
        log_debug("Unable to allocate quota handle: %s", error_message(retval));
        goto out;
    }
    ...
    for (qtype = 0; qtype < MAXQUOTAS; qtype++)
    {
        ...
        if (retval)
        {
            log_debug("Cannot finish IO on new quotafile: %s", strerror(errno));
            if (h->qh_qf.e2_file)
...
static inline qid_t get_qid(struct ext2_inode_large* inode,
                            enum quota_type qtype)
{
...
static void quota_dnode_free(dnode_t* node, void* context EXT2FS_ATTR((unused)))
{
...
void quota_release_context(quota_ctx_t* qctx)
{
    ...
    for (qtype = 0; qtype < MAXQUOTAS; qtype++)
    {
        ...
        if (ctx->quota_file[qtype])
        {
            ...
            if (err)
            {
                log_err("Cannot close quotafile: %s", strerror(errno));
...
void quota_data_add(quota_ctx_t qctx, struct ext2_inode_large* inode,
                    ext2_ino_t ino EXT2FS_ATTR((unused)), qsize_t space)
{
    struct dquot* dq;
    dict_t* dict;
    enum quota_type qtype;
    ...
    log_debug("ADD_DATA: Inode: %u, UID/GID: %u/%u, space: %ld", ino,
              inode_uid(*inode), inode_gid(*inode), space);
    for (qtype = 0; qtype < MAXQUOTAS; qtype++)
    {
                       ...
                       ext2_ino_t ino EXT2FS_ATTR((unused)), int adjust)
{
    ...
    log_debug("ADJ_INODE: Inode: %u, UID/GID: %u/%u, adjust: %d", ino,
              inode_uid(*inode), inode_gid(*inode), adjust);
    for (qtype = 0; qtype < MAXQUOTAS; qtype++)
    {
        if (qtype == PRJQUOTA && !project_quota_valid(qctx))
            continue;
...
errcode_t quota_compute_usage(quota_ctx_t qctx)
{
    ...
    while (1)
    {
        ret = ext2fs_get_next_inode_full(scan, &ino, EXT2_INODE(inode),
                                         inode_size);
        if (ret)
        {
        ...
        {
            space = ext2fs_inode_i_blocks(fs, EXT2_INODE(inode)) << 9;
...
int parse_quota_opts(const char* opts, int (*func)(char*))
{
    ...
    if (!buf)
    {
        fprintf(stderr, "Couldn't allocate memory to parse quota options!\n");
        return -ENOMEM;
    }
    strcpy(buf, opts);
