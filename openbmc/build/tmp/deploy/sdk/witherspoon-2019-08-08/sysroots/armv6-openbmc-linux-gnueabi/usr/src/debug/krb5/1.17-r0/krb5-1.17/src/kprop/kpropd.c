...
static krb5_boolean authorized_principal(krb5_context context, krb5_principal p,
...
static krb5_error_code write_pid_file(const char* path)
{
    FILE* fp;
    unsigned long pid;
...
static void signal_wrapper(int sig, sig_handler_fn handler)
{
#ifdef POSIX_SIGNALS
...
static void kill_do_standalone(int sig)
{
    if (fullprop_child > 0)
    {
        if (debug)
...
static void atexit_kill_do_standalone(void)
{
    if (fullprop_child > 0)
...
int main(int argc, char** argv)
{
    ...
    if (standalone)
    {
        ...
        if (pid_file != NULL)
        {
            ...
            if (retval)
            {
                syslog(LOG_ERR, _("Could not write pid file %s: %s"), pid_file,
                       strerror(errno));
                exit(1);
            }
        }
    ...
    else
    {
        ...
        if (devnull == -1)
        {
            syslog(LOG_ERR, _("Could not open /dev/null: %s"), strerror(errno));
            exit(1);
        }
 ...
 * IPv6 if available. */
static int get_wildcard_addr(struct addrinfo** res)
{
        ...
        switch (child_pid)
        {
            ...
            default:
                ...
                if (wait_pid == -1)
                {
                    ...
                    {
                        fprintf(stderr,
                                _("waitpid() failed to wait for doit() "
                                  "(%d %s)\n"),
                ...
                {
                    fprintf(stderr,
                            _("Database load process for full propagation "
                              "completed.\n"));
                }
...
static void doit(int fd)
{
    ...
    if (getnameinfo((const struct sockaddr*)&from, fromlen, host, sizeof(host),
                    NULL, 0, 0) == 0)
    {
    ...
    if (!authorized_principal(kpropd_context, client, etype))
    {
        ...
        syslog(LOG_WARNING,
               _("Rejected connection from unauthorized principal %s"), name);
        free(name);
        exit(1);
    }
    omask = umask(077);
    ...
    {
        com_err(progname, errno, _("while renaming %s to %s"), temp_file_name,
                file);
        exit(1);
    }
    ...
    if (close(fd) < 0)
    {
        com_err(progname, errno, _("while trying to close database file"));
        exit(1);
    }
...
static kdb_fullresync_result_t* full_resync(CLIENT* clnt)
{
    ...
    status = clnt_call(clnt, IPROP_FULL_RESYNC_EXT, (xdrproc_t)xdr_u_int32,
                       &vers, (xdrproc_t)xdr_kdb_fullresync_result_t, &clnt_res,
                       full_resync_timeout);
    if (status == RPC_PROCUNAVAIL)
    {
        status = clnt_call(clnt, IPROP_FULL_RESYNC, (xdrproc_t)xdr_void, &vers,
                           (xdrproc_t)xdr_kdb_fullresync_result_t, &clnt_res,
                           full_resync_timeout);
    }
...
krb5_error_code do_iprop()
{
    ...
    if (retval)
    {
        com_err(progname, retval, _("while canonicalizing principal name"));
...
reinit:
    ...
    retval =
        kadm5_init_with_skey(kpropd_context, iprop_svc_princstr, srvtab,
                             master_svc_princstr, &params, KADM5_STRUCT_VERSION,
                             KADM5_API_VERSION_4, db_args, &server_handle);
    ...
    if (retval)
    {
        ...
        if (retval == KADM5_RPC_ERROR)
        {
            ...
            com_err(progname, retval,
                    _("while attempting to connect"
                      " to master KDC ... retrying"));
            backoff_time = backoff_from_master(&reinit_cnt);
            if (debug)
            {
                fprintf(stderr,
                        _("Sleeping %d seconds to re-initialize "
                          "kadm5 (RPC ERROR)\n"),
                        backoff_time);
            }
            sleep(backoff_time);
        ...
        else
        {
            ...
            {
                com_err(progname, retval, _("while initializing %s interface"),
                        progname);
            ...
            reinit_cnt++;
            com_err(progname, retval,
                    _("while initializing %s interface, retrying"), progname);
            backoff_time = backoff_from_master(&reinit_cnt);
            if (debug)
            {
                fprintf(stderr,
    ...
    for (;;)
    {
        ...
        {
            clnt_perror(handle->clnt, _("iprop_get_updates call failed"));
            if (server_handle)
                kadm5_destroy(server_handle);
            server_handle = NULL;
        ...
        switch (incr_ret->ret)
        {
            ...
            case UPDATE_FULL_RESYNC_NEEDED:
                ...
                switch (full_ret->ret)
                {
                    ...
                    case UPDATE_PERM_DENIED:
                        if (debug)
                            fprintf(stderr,
                                    _("Full resync permission denied\n"));
                    ...
                    case UPDATE_ERROR:
                        if (debug)
                            fprintf(stderr,
                                    _("Full resync error from master\n"));
            ...
            case UPDATE_OK:
                ...
                if (retval)
                {
                    ...
                    {
                        fprintf(stderr,
                                _("ulog_replay failed (%s), updates not "
                                  "registered\n"),
                                msg);
                    }
                    syslog(LOG_ERR,
                           _("ulog_replay failed (%s), updates "
                             "not registered."),
                           msg);
                ...
                {
                    fprintf(stderr,
                            _("Incremental updates: %d updates / "
                              "%lu us\n"),
                            incr_ret->updates.kdb_ulog_t_len, usec);
                }
                break;
            ...
            case UPDATE_ERROR:
                ...
                syslog(LOG_ERR,
                       _("get_updates, error returned from master KDC."));
                goto error;
            ...
            default:
                ...
                if (debug)
                    fprintf(stderr,
                            _("get_updates invalid result from master\n"));
                syslog(LOG_ERR,
                       _("get_updates, invalid return from master KDC."));
                break;
        }
        ...
        else
        {
            ...
            {
                fprintf(stderr,
                        _("Waiting for %d seconds before checking "
                          "for updates again\n"),
                        pollin);
            }
            sleep(pollin);
...
static void kpropd_com_err_proc(const char* whoami, long code, const char* fmt,
                                va_list args)
...
static void parse_args(int argc, char** argv)
{
    ...
    progname = argv[0];
    while ((c = getopt_long(argc, argv, "A:f:F:p:P:r:s:DdSa:tx:", long_options,
                            NULL)) != -1)
    {
        switch (c)
        {
            ...
            case 'x':
                newargs =
                    realloc(db_args, (db_args_size + 2) * sizeof(*db_args));
                if (newargs == NULL)
                {
    ...
    {
        com_err(progname, ENOMEM, _("while allocating filename for temp file"));
        exit(1);
    }
                                  ...
                                  struct sockaddr_storage* my_sin)
{
    ...
    if (retval)
    {
        syslog(LOG_ERR, _("Error in krb5_recvauth: %s"), error_message(retval));
        exit(1);
    }
...
void recv_error(krb5_context context, krb5_data* inbuf)
{
    krb5_error* error;
    krb5_error_code retval;
    ...
    if (retval)
    {
        com_err(progname, retval, _("while decoding error packet from client"));
        exit(1);
    }
    ...
    {
        com_err(progname, (krb5_error_code)error->error + ERROR_TABLE_BASE_krb5,
...
#define WEXITSTATUS(w) (w).w_retcode
    ...
    if (error_ret)
    {
        com_err(progname, 0, _("%s returned a bad exit status (%d)"), kdb_util,
                error_ret);
        exit(1);
    }
    return;
