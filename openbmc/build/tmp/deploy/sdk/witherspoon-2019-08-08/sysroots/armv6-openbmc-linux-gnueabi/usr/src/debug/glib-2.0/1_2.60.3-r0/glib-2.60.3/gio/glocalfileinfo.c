...
#else
#error                                                                         \
    "Neither <sys/xattr.h> nor <attr/xattr.h> is present but extended attribute support is enabled."
...
static char* _g_local_file_info_create_file_id(GLocalFileStat* statbuf)
{
    guint64 ino;
#ifdef G_OS_WIN32
    ...
    return g_strdup_printf("l%" G_GUINT64_FORMAT ":%" G_GUINT64_FORMAT,
                           (guint64)statbuf->st_dev, ino);
}

static char* _g_local_file_info_create_fs_id(GLocalFileStat* statbuf)
{
    return g_strdup_printf("l%" G_GUINT64_FORMAT, (guint64)statbuf->st_dev);
}

#if defined(S_ISLNK) || defined(G_OS_WIN32)

static gchar* read_link(const gchar* full_name)
{
...
#else
        read_size =
            GLIB_PRIVATE_CALL(g_win32_readlink_utf8)(full_name, buffer, size);
#endif
        if (read_size < 0)
        {
            g_free(buffer);
...
static void get_selinux_context(const char* path, GFileInfo* info,
                                ...
                                gboolean follow_symlinks)
{
    ...
    if (!_g_file_attribute_matcher_matches_id(
            attribute_matcher, G_FILE_ATTRIBUTE_ID_SELINUX_CONTEXT))
        return;
    ...
    if (is_selinux_enabled())
    {
        ...
        if (context)
        {
            _g_file_info_set_attribute_string_by_id(
                info, G_FILE_ATTRIBUTE_ID_SELINUX_CONTEXT, context);
            freecon(context);
        }
    }
}
...
static gssize g_getxattr(const char* path, const char* name, void* value,
                         size_t size, gboolean follow_symlinks)
{
#ifdef HAVE_XATTR_NOFOLLOW
    return getxattr(path, name, value, size, 0,
                    follow_symlinks ? 0 : XATTR_NOFOLLOW);
#else
    if (follow_symlinks)
...
static gssize g_listxattr(const char* path, char* namebuf, size_t size,
                          gboolean follow_symlinks)
{
#ifdef HAVE_XATTR_NOFOLLOW
...
static gboolean name_is_valid(const char* str)
{
    while (*str)
    {
        if (!valid_char(*str++))
...
static char* hex_escape_string(const char* str, gboolean* free_return)
{
    int num_invalid, i;
    char *escaped_str, *p;
    unsigned char c;
...
static void escape_xattr(GFileInfo* info,
...
static void get_one_xattr(const char* path, GFileInfo* info,
                          const char* gio_attr, const char* xattr,
                          gboolean follow_symlinks)
{
    char value[64];
    char* value_p;
...
static void get_xattrs(const char* path, gboolean user, GFileInfo* info,
                       GFileAttributeMatcher* matcher, gboolean follow_symlinks)
{
#ifdef HAVE_XATTR
    gboolean all;
    gsize list_size;
    ...
    if (user)
        ...
        all =
            g_file_attribute_matcher_enumerate_namespace(matcher, "xattr-sys");
    ...
    if (all)
    {
        ...
        if (list_res_size == -1 || list_res_size == 0)
            return;
    ...
    retry:
        ...
        while (list_res_size > 0)
        {
                ...
                (!user && !g_str_has_prefix(attr, "user.")))
            {
                ...
                {
                    escaped_attr =
                        hex_escape_string(attr + 5, &free_escaped_attr);
    ...
    {
        while ((attr = g_file_attribute_matcher_enumerate_next(matcher)) !=
               NULL)
        {
            ...
            if (attr2)
            {
                attr2 += 2; /* Skip '::' */
                unescaped_attribute =
                    hex_unescape_string(attr2, NULL, &free_unescaped_attribute);
                if (user)
...
#ifdef HAVE_XATTR
static void get_one_xattr_from_fd(int fd, GFileInfo* info, const char* gio_attr,
                                  const char* xattr)
{
    char value[64];
    char* value_p;
...
static void get_xattrs_from_fd(int fd, gboolean user, GFileInfo* info,
...
#ifdef HAVE_XATTR
static gboolean set_xattr(char* filename, const char* escaped_attribute,
                          const GFileAttributeValue* attr_value, GError** error)
{
...
void _g_local_file_info_get_parent_info(
    const char* dir, GFileAttributeMatcher* attribute_matcher,
    GLocalParentFileInfo* parent_info)
{
    ...
    if (_g_file_attribute_matcher_matches_id(
            attribute_matcher, G_FILE_ATTRIBUTE_ID_ACCESS_CAN_RENAME) ||
        _g_file_attribute_matcher_matches_id(
            attribute_matcher, G_FILE_ATTRIBUTE_ID_ACCESS_CAN_DELETE) ||
        _g_file_attribute_matcher_matches_id(
            attribute_matcher, G_FILE_ATTRIBUTE_ID_ACCESS_CAN_TRASH) ||
        _g_file_attribute_matcher_matches_id(
            attribute_matcher, G_FILE_ATTRIBUTE_ID_UNIX_IS_MOUNTPOINT))
    {
...
{
    if (parent_info->extra_data && parent_info->free_extra_data)
        parent_info->free_extra_data(parent_info->extra_data);
}

static void get_access_rights(GFileAttributeMatcher* attribute_matcher,
                              GFileInfo* info, const gchar* path,
                              GLocalFileStat* statbuf,
                              GLocalParentFileInfo* parent_info)
{
    /* FIXME: Windows: The underlyin _waccess() is mostly pointless */
    if (_g_file_attribute_matcher_matches_id(
            attribute_matcher, G_FILE_ATTRIBUTE_ID_ACCESS_CAN_READ))
        _g_file_info_set_attribute_boolean_by_id(
            info, G_FILE_ATTRIBUTE_ID_ACCESS_CAN_READ,
            g_access(path, R_OK) == 0);

    if (_g_file_attribute_matcher_matches_id(
            attribute_matcher, G_FILE_ATTRIBUTE_ID_ACCESS_CAN_WRITE))
        _g_file_info_set_attribute_boolean_by_id(
            info, G_FILE_ATTRIBUTE_ID_ACCESS_CAN_WRITE,
            g_access(path, W_OK) == 0);

    if (_g_file_attribute_matcher_matches_id(
            attribute_matcher, G_FILE_ATTRIBUTE_ID_ACCESS_CAN_EXECUTE))
        _g_file_info_set_attribute_boolean_by_id(
    ...
    if (parent_info)
    {
        ...
        if (parent_info->writable)
        {
            ...
            if (parent_info->is_sticky)
            {
                ...
                if (uid == statbuf->st_uid || uid == parent_info->owner ||
                    uid == 0)
                    writable = TRUE;
            }
            else
        ...
        if (_g_file_attribute_matcher_matches_id(
                attribute_matcher, G_FILE_ATTRIBUTE_ID_ACCESS_CAN_RENAME))
            _g_file_info_set_attribute_boolean_by_id(
                info, G_FILE_ATTRIBUTE_ID_ACCESS_CAN_RENAME, writable);
        ...
        if (_g_file_attribute_matcher_matches_id(
                attribute_matcher, G_FILE_ATTRIBUTE_ID_ACCESS_CAN_TRASH))
            _g_file_info_set_attribute_boolean_by_id(
                info, G_FILE_ATTRIBUTE_ID_ACCESS_CAN_TRASH,
                writable && parent_info->has_trash_dir);
    }
}

static void set_info_from_stat(GFileInfo* info, GLocalFileStat* statbuf,
                               GFileAttributeMatcher* attribute_matcher)
{
    GFileType file_type;
    ...
    _g_file_info_set_attribute_uint32_by_id(
        info, G_FILE_ATTRIBUTE_ID_UNIX_DEVICE, statbuf->st_dev);
#ifndef G_OS_WIN32
    /* Pointless setting these on Windows even if they exist in the struct */
    _g_file_info_set_attribute_uint64_by_id(
        info, G_FILE_ATTRIBUTE_ID_UNIX_INODE, statbuf->st_ino);
    _g_file_info_set_attribute_uint32_by_id(
        info, G_FILE_ATTRIBUTE_ID_UNIX_NLINK, statbuf->st_nlink);
    _g_file_info_set_attribute_uint32_by_id(info, G_FILE_ATTRIBUTE_ID_UNIX_UID,
                                            statbuf->st_uid);
    _g_file_info_set_attribute_uint32_by_id(info, G_FILE_ATTRIBUTE_ID_UNIX_GID,
                                            statbuf->st_gid);
    _g_file_info_set_attribute_uint32_by_id(info, G_FILE_ATTRIBUTE_ID_UNIX_RDEV,
                                            statbuf->st_rdev);
#endif
    /* FIXME: st_mode is mostly pointless on Windows, too. Set the attribute or
     * not? */
    _g_file_info_set_attribute_uint32_by_id(info, G_FILE_ATTRIBUTE_ID_UNIX_MODE,
                                            statbuf->st_mode);
#if defined(HAVE_STRUCT_STAT_ST_BLKSIZE)
    _g_file_info_set_attribute_uint32_by_id(
        info, G_FILE_ATTRIBUTE_ID_UNIX_BLOCK_SIZE, statbuf->st_blksize);
#endif
#if defined(HAVE_STRUCT_STAT_ST_BLOCKS)
    _g_file_info_set_attribute_uint64_by_id(
        info, G_FILE_ATTRIBUTE_ID_UNIX_BLOCKS, statbuf->st_blocks);
    _g_file_info_set_attribute_uint64_by_id(
        info, G_FILE_ATTRIBUTE_ID_STANDARD_ALLOCATED_SIZE,
    ...
    _g_file_info_set_attribute_uint64_by_id(
        info, G_FILE_ATTRIBUTE_ID_TIME_MODIFIED, statbuf->st_mtime);
#if defined(HAVE_STRUCT_STAT_ST_MTIMENSEC)
    _g_file_info_set_attribute_uint32_by_id(
    ...
    _g_file_info_set_attribute_uint64_by_id(
        info, G_FILE_ATTRIBUTE_ID_TIME_ACCESS, statbuf->st_atime);
#if defined(HAVE_STRUCT_STAT_ST_ATIMENSEC)
    _g_file_info_set_attribute_uint32_by_id(
        info, G_FILE_ATTRIBUTE_ID_TIME_ACCESS_USEC,
     ...
     */
    _g_file_info_set_attribute_uint64_by_id(
        info, G_FILE_ATTRIBUTE_ID_TIME_CHANGED, statbuf->st_ctime);
    ...
    _g_file_info_set_attribute_uint64_by_id(
        info, G_FILE_ATTRIBUTE_ID_TIME_CREATED, statbuf->st_birthtime);
    _g_file_info_set_attribute_uint32_by_id(
        info, G_FILE_ATTRIBUTE_ID_TIME_CREATED_USEC,
        statbuf->st_birthtimensec / 1000);
#elif defined(HAVE_STRUCT_STAT_ST_BIRTHTIM) &&                                 \
    defined(HAVE_STRUCT_STAT_ST_BIRTHTIM_TV_NSEC)
    _g_file_info_set_attribute_uint64_by_id(
        info, G_FILE_ATTRIBUTE_ID_TIME_CREATED, statbuf->st_birthtim.tv_sec);
    _g_file_info_set_attribute_uint32_by_id(
        info, G_FILE_ATTRIBUTE_ID_TIME_CREATED_USEC,
...
#elif defined(HAVE_STRUCT_STAT_ST_BIRTHTIM)
    _g_file_info_set_attribute_uint64_by_id(
        info, G_FILE_ATTRIBUTE_ID_TIME_CREATED, statbuf->st_birthtim);
#elif defined(G_OS_WIN32)
    _g_file_info_set_attribute_uint64_by_id(
        info, G_FILE_ATTRIBUTE_ID_TIME_CREATED, statbuf->st_ctime);
#endif
                                             ...
                                             G_FILE_ATTRIBUTE_ID_ETAG_VALUE))
    {
        ...
        _g_file_info_set_attribute_string_by_id(
            info, G_FILE_ATTRIBUTE_ID_ETAG_VALUE, etag);
        g_free(etag);
    }
                                             ...
                                             G_FILE_ATTRIBUTE_ID_ID_FILE))
    {
        ...
        _g_file_info_set_attribute_string_by_id(
            info, G_FILE_ATTRIBUTE_ID_ID_FILE, id);
        g_free(id);
    }
                                             ...
                                             G_FILE_ATTRIBUTE_ID_ID_FILESYSTEM))
    {
        ...
        _g_file_info_set_attribute_string_by_id(
            info, G_FILE_ATTRIBUTE_ID_ID_FILESYSTEM, id);
        g_free(id);
    }
}
...
static char* make_valid_utf8(const char* name)
{
    GString* string;
...
static char* convert_pwd_string_to_utf8(char* pwd_str)
{
    char* utf8_string;
...
static void uid_data_free(UidData* data)
{
...
static UidData* lookup_uid_data(uid_t uid)
{
    UidData* data;
    char buffer[4096];
    struct passwd pwbuf;
    ...
    if (uid_cache == NULL)
        uid_cache = g_hash_table_new_full(NULL, NULL, NULL,
                                          (GDestroyNotify)uid_data_free);
...
static char* get_realname_from_uid(uid_t uid)
{
    char* res;
    UidData* data;
...
/* called with lock held */
static char* lookup_gid_name(gid_t gid)
{
    char* name;
    ...
    if (gbufp != NULL && gbufp->gr_name != NULL && gbufp->gr_name[0] != 0)
...
static char* get_groupname_from_gid(gid_t gid)
{
    char* res;
    char* name;
...
static char* get_content_type(const char* basename, const char* path,
                              GLocalFileStat* statbuf, gboolean is_symlink,
                              gboolean symlink_broken,
                              GFileQueryInfoFlags flags, gboolean fast)
{
    if (is_symlink &&
    ...
    else if (statbuf != NULL && S_ISREG(statbuf->st_mode) &&
             statbuf->st_size == 0)
    {
    ...
    else
    {
        ...
        content_type =
            g_content_type_guess(basename, NULL, 0, &result_uncertain);
...
/* @stat_buf is the pre-calculated result of stat(path), or %NULL if that
 * failed. */
static void get_thumbnail_attributes(const char* path, GFileInfo* info,
                                     const GLocalFileStat* stat_buf)
{
    ...
    filename = g_build_filename(g_get_user_cache_dir(), "thumbnails", "large",
                                basename, NULL);

    if (g_file_test(filename, G_FILE_TEST_IS_REGULAR))
    {
        _g_file_info_set_attribute_byte_string_by_id(
            info, G_FILE_ATTRIBUTE_ID_THUMBNAIL_PATH, filename);
        _g_file_info_set_attribute_boolean_by_id(
    ...
    {
        g_free(filename);
        filename = g_build_filename(g_get_user_cache_dir(), "thumbnails",
        ...
        else
        {
            g_free(filename);
            filename =
                g_build_filename(g_get_user_cache_dir(), "thumbnails", "fail",
                                 "gnome-thumbnail-factory", basename, NULL);

            if (g_file_test(filename, G_FILE_TEST_IS_REGULAR))
            {
                _g_file_info_set_attribute_boolean_by_id(
                    info, G_FILE_ATTRIBUTE_ID_THUMBNAILING_FAILED, TRUE);
                _g_file_info_set_attribute_boolean_by_id(
                    info, G_FILE_ATTRIBUTE_ID_THUMBNAIL_IS_VALID,
                    thumbnail_verify(filename, uri, stat_buf));
            }
...
#ifdef G_OS_WIN32
static void win32_get_file_user_info(const gchar* filename, gchar** group_name,
                                     gchar** user_name, gchar** real_name)
{
    ...
    if ((GetFileSecurityW(
             wfilename, GROUP_SECURITY_INFORMATION | OWNER_SECURITY_INFORMATION,
             NULL, sd_size, &sd_size) ||
            ...
            psd, sd_size, &sd_size))
    {
        ...
        do
        {
            ...
            if (!LookupAccountSidW(NULL, /* local machine */
                                   psid, name, &name_len, domain,
                                   &domain_len, /* no domain info yet */
                                   &name_use) &&
                (ERROR_INSUFFICIENT_BUFFER != GetLastError()))
                break;
...
static gboolean remove_from_hidden_cache(gpointer user_data)
{
    G_LOCK(hidden_cache);
...
static GHashTable* read_hidden_file(const gchar* dirname)
{
    gchar* contents = NULL;
    gchar* filename;
...
static void maybe_unref_hash_table(gpointer data)
{
    if (data != NULL)
        g_hash_table_unref(data);
}

static gboolean file_is_hidden(const gchar* path, const gchar* basename)
{
    gboolean result;
    gchar* dirname;
    gpointer table;
    ...
    if
        G_UNLIKELY(hidden_cache == NULL)
    hidden_cache = g_hash_table_new_full(g_str_hash, g_str_equal, g_free,
                                         maybe_unref_hash_table);

    if (!g_hash_table_lookup_extended(hidden_cache, dirname, NULL, &table))
    {
        gchar* mydirname;
        GSource* remove_from_cache_source;

        g_hash_table_insert(hidden_cache, mydirname = g_strdup(dirname),
        ...
        g_source_set_callback(remove_from_cache_source,
                              remove_from_hidden_cache, mydirname, NULL);
...
void _g_local_file_info_get_nostat(GFileInfo* info, const char* basename,
                                   const char* path,
                                   GFileAttributeMatcher* attribute_matcher)
{
    ...
    if (_g_file_attribute_matcher_matches_id(
            attribute_matcher, G_FILE_ATTRIBUTE_ID_STANDARD_DISPLAY_NAME))
    {
        ...
        if (strstr(display_name, "\357\277\275") != NULL)
        {
            char* p = display_name;
            display_name =
                g_strconcat(display_name, _(" (invalid encoding)"), NULL);
            g_free(p);
        }
    ...
    if (_g_file_attribute_matcher_matches_id(
            attribute_matcher, G_FILE_ATTRIBUTE_ID_STANDARD_EDIT_NAME))
    {
    ...
    if (_g_file_attribute_matcher_matches_id(
            attribute_matcher, G_FILE_ATTRIBUTE_ID_STANDARD_COPY_NAME))
    {
        char* copy_name = g_filename_to_utf8(basename, -1, NULL, NULL, NULL);
        if (copy_name)
            _g_file_info_set_attribute_string_by_id(
                info, G_FILE_ATTRIBUTE_ID_STANDARD_COPY_NAME, copy_name);
        g_free(copy_name);
    }
}

static const char* get_icon_name(const char* path, gboolean use_symbolic,
                                 gboolean* with_fallbacks_out)
{
    ...
    else if (g_strcmp0(path, g_get_user_special_dir(G_USER_DIRECTORY_MUSIC)) ==
             0)
    {
    ...
    {
        name =
            use_symbolic ? "folder-publicshare-symbolic" : "folder-publicshare";
    }
    else if (g_strcmp0(path,
...
static GIcon* get_icon(const char* path, const char* content_type,
...
GFileInfo* _g_local_file_info_get(const char* basename, const char* path,
                                  GFileAttributeMatcher* attribute_matcher,
                                  ...
                                  GError** error)
{
...
#ifndef G_OS_WIN32
    if (_g_file_attribute_matcher_matches_id(
            attribute_matcher, G_FILE_ATTRIBUTE_ID_STANDARD_IS_HIDDEN))
    {
        if (basename != NULL &&
            (basename[0] == '.' || file_is_hidden(path, basename)))
            g_file_info_set_is_hidden(info, TRUE);
    }
    ...
    if (basename != NULL && basename[strlen(basename) - 1] == '~' &&
        ...
        _g_file_info_set_attribute_boolean_by_id(
            info, G_FILE_ATTRIBUTE_ID_STANDARD_IS_BACKUP, TRUE);
#else
    ...
    if (statbuf.attributes & FILE_ATTRIBUTE_ARCHIVE)
        _g_file_info_set_attribute_boolean_by_id(
            info, G_FILE_ATTRIBUTE_ID_DOS_IS_ARCHIVE, TRUE);

    if (statbuf.attributes & FILE_ATTRIBUTE_SYSTEM)
        _g_file_info_set_attribute_boolean_by_id(
            info, G_FILE_ATTRIBUTE_ID_DOS_IS_SYSTEM, TRUE);

    if (statbuf.reparse_tag == IO_REPARSE_TAG_MOUNT_POINT)
        _g_file_info_set_attribute_boolean_by_id(
            info, G_FILE_ATTRIBUTE_ID_DOS_IS_MOUNTPOINT, TRUE);

    if (statbuf.reparse_tag != 0)
        _g_file_info_set_attribute_uint32_by_id(
            info, G_FILE_ATTRIBUTE_ID_DOS_REPARSE_POINT_TAG,
            statbuf.reparse_tag);
#endif
    ...
    if (is_symlink)
    {
        ...
        if (symlink_target &&
            _g_file_attribute_matcher_matches_id(
                attribute_matcher, G_FILE_ATTRIBUTE_ID_STANDARD_SYMLINK_TARGET))
            g_file_info_set_symlink_target(info, symlink_target);
    }

    if (_g_file_attribute_matcher_matches_id(
            attribute_matcher, G_FILE_ATTRIBUTE_ID_STANDARD_CONTENT_TYPE) ||
        _g_file_attribute_matcher_matches_id(
            attribute_matcher, G_FILE_ATTRIBUTE_ID_STANDARD_ICON) ||
        _g_file_attribute_matcher_matches_id(
            attribute_matcher, G_FILE_ATTRIBUTE_ID_STANDARD_SYMBOLIC_ICON))
    {
        char* content_type =
            get_content_type(basename, path, stat_ok ? &statbuf : NULL,
                             is_symlink, symlink_broken, flags, FALSE);
    ...
    if (_g_file_attribute_matcher_matches_id(
            attribute_matcher, G_FILE_ATTRIBUTE_ID_STANDARD_FAST_CONTENT_TYPE))
    {
        char* content_type =
            get_content_type(basename, path, stat_ok ? &statbuf : NULL,
                             is_symlink, symlink_broken, flags, TRUE);

        if (content_type)
        {
            _g_file_info_set_attribute_string_by_id(
                info, G_FILE_ATTRIBUTE_ID_STANDARD_FAST_CONTENT_TYPE,
                content_type);
            g_free(content_type);
        }
    }
                                             ...
                                             G_FILE_ATTRIBUTE_ID_OWNER_USER))
    {
        ...
        if (name)
            _g_file_info_set_attribute_string_by_id(
                info, G_FILE_ATTRIBUTE_ID_OWNER_USER, name);
        g_free(name);
    }

    if (_g_file_attribute_matcher_matches_id(
            attribute_matcher, G_FILE_ATTRIBUTE_ID_OWNER_USER_REAL))
    {
        char* name = NULL;
#ifdef G_OS_WIN32
        ...
        if (name)
            _g_file_info_set_attribute_string_by_id(
                info, G_FILE_ATTRIBUTE_ID_OWNER_USER_REAL, name);
        g_free(name);
    }
                                             ...
                                             G_FILE_ATTRIBUTE_ID_OWNER_GROUP))
    {
        ...
        if (name)
            _g_file_info_set_attribute_string_by_id(
                info, G_FILE_ATTRIBUTE_ID_OWNER_GROUP, name);
        g_free(name);
    }
    ...
    if (stat_ok && parent_info && parent_info->device != 0 &&
        ...
        (statbuf.st_dev != parent_info->device ||
         statbuf.st_ino == parent_info->inode))
        _g_file_info_set_attribute_boolean_by_id(
            info, G_FILE_ATTRIBUTE_ID_UNIX_IS_MOUNTPOINT, TRUE);

    if (stat_ok)
        get_access_rights(attribute_matcher, info, path, &statbuf, parent_info);

#ifdef HAVE_SELINUX
    get_selinux_context(path, info, attribute_matcher,
                        (flags & G_FILE_QUERY_INFO_NOFOLLOW_SYMLINKS) == 0);
#endif
    get_xattrs(path, TRUE, info, attribute_matcher,
               (flags & G_FILE_QUERY_INFO_NOFOLLOW_SYMLINKS) == 0);
    get_xattrs(path, FALSE, info, attribute_matcher,
               (flags & G_FILE_QUERY_INFO_NOFOLLOW_SYMLINKS) == 0);

    if (_g_file_attribute_matcher_matches_id(
            attribute_matcher, G_FILE_ATTRIBUTE_ID_THUMBNAIL_PATH) ||
        _g_file_attribute_matcher_matches_id(
            attribute_matcher, G_FILE_ATTRIBUTE_ID_THUMBNAIL_IS_VALID) ||
        _g_file_attribute_matcher_matches_id(
            attribute_matcher, G_FILE_ATTRIBUTE_ID_THUMBNAILING_FAILED))
    {
        if (stat_ok)
    ...
    {
        class->local_file_add_info(vfs, path, device, attribute_matcher, info,
                                   NULL, &parent_info->extra_data,
...
GFileInfo* _g_local_file_info_get_from_fd(int fd, const char* attributes,
                                          GError** error)
{
...
#ifdef HAVE_SELINUX
    if (_g_file_attribute_matcher_matches_id(
            matcher, G_FILE_ATTRIBUTE_ID_SELINUX_CONTEXT) &&
        is_selinux_enabled())
    {
        char* context;
...
static gboolean get_uint32(const GFileAttributeValue* value, guint32* val_out,
                           GError** error)
{
...
#ifdef HAVE_UTIMES
static gboolean get_uint64(const GFileAttributeValue* value, guint64* val_out,
                           GError** error)
{
...
static gboolean get_byte_string(const GFileAttributeValue* value,
                                const char** val_out, GError** error)
{
...
#ifdef HAVE_SELINUX
static gboolean get_string(const GFileAttributeValue* value,
                           const char** val_out, GError** error)
{
...
static gboolean set_unix_mode(char* filename, GFileQueryInfoFlags flags,
                              const GFileAttributeValue* value, GError** error)
{
    guint32 val = 0;
    int res = 0;
    ...
    if (flags & G_FILE_QUERY_INFO_NOFOLLOW_SYMLINKS)
    {
        ...
        is_symlink =
            (res == 0 && (statbuf.reparse_tag == IO_REPARSE_TAG_SYMLINK ||
                          statbuf.reparse_tag == IO_REPARSE_TAG_MOUNT_POINT));
#endif
        if (is_symlink)
        {
    ...
    if (res == -1)
    {
        ...
        g_set_error(error, G_IO_ERROR, g_io_error_from_errno(errsv),
                    _("Error setting permissions: %s"), g_strerror(errsv));
        return FALSE;
    }
    return TRUE;
}
...
#ifdef G_OS_UNIX
static gboolean set_unix_uid_gid(char* filename,
                                 const GFileAttributeValue* uid_value,
                                 const GFileAttributeValue* gid_value,
                                 GFileQueryInfoFlags flags, GError** error)
{
    int res;
    guint32 val = 0;
    uid_t uid;
    ...
    if (res == -1)
    {
        ...
        g_set_error(error, G_IO_ERROR, g_io_error_from_errno(errsv),
                    _("Error setting owner: %s"), g_strerror(errsv));
        return FALSE;
    }
    return TRUE;
}
...
#ifdef HAVE_SYMLINK
static gboolean set_symlink(char* filename, const GFileAttributeValue* value,
                            GError** error)
{
    const char* val;
    struct stat statbuf;
    ...
    if (g_lstat(filename, &statbuf))
    {
        ...
        g_set_error(error, G_IO_ERROR, g_io_error_from_errno(errsv),
                    _("Error setting symlink: %s"), g_strerror(errsv));
        return FALSE;
    }

    if (!S_ISLNK(statbuf.st_mode))
    {
        g_set_error_literal(error, G_IO_ERROR, G_IO_ERROR_NOT_SYMBOLIC_LINK,
...
#ifdef HAVE_UTIMES
static int lazy_stat(char* filename, struct stat* statbuf,
...
static gboolean set_mtime_atime(char* filename,
...
#ifdef HAVE_SELINUX
static gboolean set_selinux_context(char* filename,
...
gboolean _g_local_file_info_set_attribute(char* filename, const char* attribute,
                                          ...
                                          GError** error)
{
    ...
    if (class->local_file_set_attributes)
    {
        ...
        if (!class->local_file_set_attributes(vfs, filename, info, flags,
                                              cancellable, error))
        {
            g_object_unref(info);
            return FALSE;
        ...
        if (g_file_info_get_attribute_status(info, attribute) ==
...
gboolean _g_local_file_info_set_attributes(char* filename, GFileInfo* info,
                                           GFileQueryInfoFlags flags,
                                           GCancellable* cancellable,
                                           GError** error)
{
...
#ifdef HAVE_SYMLINK
    value = _g_file_info_get_attribute_value(
     ...
     * Change times as the last thing to avoid it changing due to metadata
     * changes
     */

    mtime =
        _g_file_info_get_attribute_value(info, G_FILE_ATTRIBUTE_TIME_MODIFIED);
    mtime_usec = _g_file_info_get_attribute_value(
        info, G_FILE_ATTRIBUTE_TIME_MODIFIED_USEC);
    atime =
        _g_file_info_get_attribute_value(info, G_FILE_ATTRIBUTE_TIME_ACCESS);
    atime_usec = _g_file_info_get_attribute_value(
        info, G_FILE_ATTRIBUTE_TIME_ACCESS_USEC);

    if (mtime || mtime_usec || atime || atime_usec)
    {
        if (!set_mtime_atime(filename, mtime, mtime_usec, atime, atime_usec,
                             error))
        {
            status = G_FILE_ATTRIBUTE_STATUS_ERROR_SETTING;
            res = FALSE;
    ...
    {
        value = _g_file_info_get_attribute_value(
            info, G_FILE_ATTRIBUTE_SELINUX_CONTEXT);
        if (value)
        {
