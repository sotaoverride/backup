...
BOOST_LOG_ANONYMOUS_NAMESPACE
{
    ...
    //! A special time point type that contains decomposed date and time to
    //! avoid excessive calculations
    struct decomposed_time_point
    {
        struct date_type : public gregorian::greg_year_month_day
        {
        ...
        struct time_duration_type : public date_time::micro_res
        {
            ...
            time_duration_type(hour_type h, min_type m, sec_type s,
                               fractional_seconds_type u) :
                hours(h),
                minutes(m), seconds(s), useconds(u)
            {
            }
        };
    ...
    inline const char* severity_level_to_string(
    ...
    struct message_printer
    {
        ...
        explicit message_printer(boost::log::trivial::severity_level lvl) :
            m_level(lvl)
        {
        }
        ...
        result_type operator()(std::string const& msg) const
        {
            ...
            boost::log::aux::format_thread_id(
                thread_id_buf, sizeof(thread_id_buf),
                boost::log::aux::this_thread::get_id());
#endif

            const decomposed_time_point now =
                date_time::microsec_clock<decomposed_time_point>::local_time();
            ...
            std::printf("[%04u-%02u-%02u %02u:%02u:%02u.%06u] "
                        ...
                        thread_id_buf,
#endif
                        severity_level_to_string(m_level), msg.c_str());
        }
...
void default_sink::consume(record_view const& rec)
{
    ...
    m_message_visitor(
        m_message_name, rec.attribute_values(),
        message_printer(m_severity_extractor(m_severity_name, rec).get()));
}
