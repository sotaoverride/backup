...
static const BIO_METHOD methods_dgramp = {
    BIO_TYPE_DGRAM, "datagram socket",
    /* TODO: Convert to new style write function */
    bwrite_conv, dgram_write,
    /* TODO: Convert to new style read function */
    bread_conv, dgram_read, dgram_puts, NULL, /* dgram_gets,         */
    dgram_ctrl, dgram_new, dgram_free, NULL,  /* dgram_callback_ctrl */
};

#ifndef OPENSSL_NO_SCTP
static const BIO_METHOD methods_dgramp_sctp = {
    BIO_TYPE_DGRAM_SCTP, "datagram sctp socket",
    /* TODO: Convert to new style write function */
    bwrite_conv, dgram_sctp_write,
    /* TODO: Convert to new style write function */
    bread_conv, dgram_sctp_read, dgram_sctp_puts, NULL, /* dgram_gets, */
    dgram_sctp_ctrl, dgram_sctp_new, dgram_sctp_free,
...
static void dgram_adjust_rcv_timeout(BIO* b)
{
    ...
    if (data->next_timeout.tv_sec > 0 || data->next_timeout.tv_usec > 0)
    {
        ...
        sz.i = sizeof(timeout);
        if (getsockopt(b->num, SOL_SOCKET, SO_RCVTIMEO, (void*)&timeout,
                       &sz.i) < 0)
        {
            perror("getsockopt");
        }
        ...
        if (getsockopt(b->num, SOL_SOCKET, SO_RCVTIMEO, &(data->socket_timeout),
                       (void*)&sz) < 0)
        {
            perror("getsockopt");
        }
             ...
             data->socket_timeout.tv_usec >= timeleft.tv_usec))
        {
            ...
            if (setsockopt(b->num, SOL_SOCKET, SO_RCVTIMEO, (void*)&timeout,
                           sizeof(timeout)) < 0)
            {
                perror("setsockopt");
            }
...
static int dgram_write(BIO* b, const char* in, int inl)
{
    ...
    else
    {
        ...
        ret =
            sendto(b->num, in, inl, 0, BIO_ADDR_sockaddr(&data->peer), peerlen);
    }
...
static long dgram_ctrl(BIO* b, int cmd, long num, void* ptr)
{
    ...
    defined(IP_PMTUDISC_DO)
        ...
        case BIO_CTRL_DGRAM_GET_RECV_TIMEOUT:
        {
            ...
            if (getsockopt(b->num, SOL_SOCKET, SO_RCVTIMEO, ptr, (void*)&sz) <
                0)
            {
                perror("getsockopt");
                ret = -1;
...
#ifdef OPENSSL_SYS_WINDOWS
        {
            struct timeval* tv = (struct timeval*)ptr;
            int timeout = tv->tv_sec * 1000 + tv->tv_usec / 1000;
            if (setsockopt(b->num, SOL_SOCKET, SO_SNDTIMEO, (void*)&timeout,
                           sizeof(timeout)) < 0)
            {
                perror("setsockopt");
                ret = -1;
        ...
        case BIO_CTRL_DGRAM_GET_SEND_TIMEOUT:
        {
            ...
            sz.i = sizeof(timeout);
            if (getsockopt(b->num, SOL_SOCKET, SO_SNDTIMEO, (void*)&timeout,
                           &sz.i) < 0)
            {
                perror("getsockopt");
                ret = -1;
...
#else
            sz.i = sizeof(struct timeval);
            if (getsockopt(b->num, SOL_SOCKET, SO_SNDTIMEO, ptr, (void*)&sz) <
                0)
            {
                perror("getsockopt");
                ret = -1;
    ...
    defined(IP_PMTUDISC_PROBE)
                    if ((sockopt_val =
                             num ? IP_PMTUDISC_PROBE : IP_PMTUDISC_DONT),
         ...
         * reasons. When BIO_CTRL_DGRAM_SET_PEEK_MODE was first defined its
         * value was incorrectly clashing with
         * BIO_CTRL_DGRAM_SCTP_SET_IN_HANDSHAKE. The value has been updated to a
         * non-clashing value. However to preserve binary compatiblity we now
         * respond to both the old value and the new one
         */
...
BIO* BIO_new_dgram_sctp(int fd, int close_flag)
{
    ...
    for (p = (unsigned char*)authchunks->gauth_chunks;
         p < (unsigned char*)authchunks + sockopt_len; p += sizeof(uint8_t))
    {
...
void dgram_sctp_handle_auth_free_key_event(BIO* b, union sctp_notification* snp)
{
    ...
    if (authkeyevent->auth_indication == SCTP_AUTH_FREE_KEY)
    {
        ...
        ret = setsockopt(b->num, IPPROTO_SCTP, SCTP_AUTH_DELETE_KEY, &authkeyid,
        ...
        do
        {
            ...
            if (msg.msg_flags & MSG_NOTIFICATION)
            {
                ...
                ret += n;
                 ...
                 (msg.msg_flags & MSG_EOR) && (ret < outl));
...
int BIO_dgram_sctp_notification_cb(
    BIO* b, void (*handle_notifications)(BIO* bio, void* context, void* buf),
    void* context)
{
    bio_dgram_sctp_data* data = (bio_dgram_sctp_data*)b->ptr;
