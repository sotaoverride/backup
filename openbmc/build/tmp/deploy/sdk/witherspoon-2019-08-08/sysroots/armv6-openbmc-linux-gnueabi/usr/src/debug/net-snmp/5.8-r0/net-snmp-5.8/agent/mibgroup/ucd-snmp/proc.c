...
void init_proc(void)
{
     ...
     * information at
     */
    struct variable2 extensible_proc_variables[] = {{MIBINDEX,
                                                     ASN_INTEGER,
    ...
    if ((cptr = skip_white(cptr)))
    {
        ...
        if ((cptr = skip_white(cptr)))
        {
            ...
            if ((cptr = skip_white(cptr)))
            {
                ...
                (*procp)->regexp = pcre_compile(cptr, 0, &pcre_error,
...
u_char* var_extensible_proc(struct variable* vp, oid* name, size_t* length,
        ...
        switch (vp->magic)
        {
            ...
            case ERRORMSG:
                ...
                else if (proc->min && long_ret < proc->min)
                {
                    ...
                    {
                        if (asprintf(&errmsg, "No %s process running",
                                     proc->name) < 0)
                        {
                        }
                    }
...
int fixProcError(int action, u_char* var_val, u_char var_val_type,
...
static struct myproc* get_proc_instance(struct myproc* proc, oid inst)
{
    int i;
...
int sh_count_myprocs(struct myproc* proc)
{
    if (proc == NULL)
        return 0;
...
#ifdef USING_HOST_DATA_ACCESS_SWRUN_MODULE
netsnmp_feature_require(swrun_count_processes_by_name) int sh_count_procs(
    char* procname)
{
...
netsnmp_feature_require(
    swrun_count_processes_by_regex) int sh_count_procs_by_regex(char* procname,
                                                                netsnmp_regex_ptr
                                                                    regexp)
{
...
int sh_count_procs(char* procname)
{
    ...
    {
        if ((pbase = (struct kinfo_proc*)realloc(
                 pbase, nproc + sizeof(struct kinfo_proc))) == 0)
            return -1;
        onproc = nproc;
    ...
    for (pp = pbase, i = 0; i < nproc / sizeof(struct kinfo_proc); pp++, i++)
    {
        ...
        {
            if (PP(pp, p_stat) != SZOMB && !strcmp(PP(pp, p_comm), procname))
                ret++;
        }
    }
    return ret;
...
int sh_count_procs(char* procname)
{
    ...
    while (getprocs(&pinfo, sizeof(pinfo), NULL, 0, &index, 1) == 1)
    {
        ...
        if (sep != NULL)
            *sep = 0;
        if (strcmp(procname, pinfo_name) == 0)
            count++;
    }
...
int sh_count_procs(char* procname)
{
    ...
    while (NULL != (ent = readdir(dir)))
    {
        ...
        while (--len)
            if (!cmdline[len])
                cmdline[len] = ' ';
        if (!strncmp(cmdline, procname, plen))
            total++;
#else
        /* read /proc/XX/status */
        ...
        for (len = 0;; len++)
        {
            if (tmpc[len] && isgraph(tmpc[len]))
                continue;
            tmpc[len] = '\0';
            break;
        }
        DEBUGMSGTL(
            ("proc", "Comparing wanted %s against %s\n", procname, tmpc));
...
static struct nlist proc_nl[] = {{"_nproc"},
#define X_NPROC 0
                                 {"_proc"},
#define X_PROC 1
                                 {"_proc_bitmap"},
...
static struct user* getuser(struct proc* aproc)
{
    static union
    {
        struct user user;
    ...
    if ((aproc->p_sched & SLOAD) == 0)
    {
        ...
        {
            /*
             * warnx("can't read u for pid %d from %s", aproc->p_pid,
             * _PATH_DRUM);
             */
            return (NULL);
        }
    ...
    {
        /*
         * warnx("can't read user pt for pid %d from %s", aproc->p_pid,
         * _PATH_DRUM);
         */
        return (NULL);
    }
...
static int getstruct(off_t loc, char* name, off_t dest, int size)
{
...
#else
netsnmp_feature_require(find_field) int sh_count_procs(char* procname)
{
