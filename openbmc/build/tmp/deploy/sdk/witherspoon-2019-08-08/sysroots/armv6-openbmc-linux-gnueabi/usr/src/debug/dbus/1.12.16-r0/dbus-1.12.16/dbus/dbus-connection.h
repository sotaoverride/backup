...
/** Opaque type representing preallocated resources so a message can be sent
...
/** Opaque type representing a connection to a remote application and associated
...
/**
 * Indicates the status of incoming data on a #DBusConnection. This determines
 * whether dbus_connection_dispatch() needs to be called.
 */
typedef enum
{
    DBUS_DISPATCH_DATA_REMAINS, /**< There is more data to potentially convert
                                   to messages. */
    DBUS_DISPATCH_COMPLETE,     /**< All currently available data has been
                                   processed. */
...
/**
 * Called when a #DBusObjectPathVTable is unregistered (or its connection is
 * freed). Found in #DBusObjectPathVTable.
 */
