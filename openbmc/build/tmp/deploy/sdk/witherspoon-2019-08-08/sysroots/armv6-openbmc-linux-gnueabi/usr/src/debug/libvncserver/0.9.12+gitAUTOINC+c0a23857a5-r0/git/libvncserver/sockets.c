 ...
 * needs to be closed for any reason then rfbCloseClient should be called. In
 * turn, rfbClientConnectionGone will be called by rfbProcessEvents
 * (non-threaded case) or clientInput (threaded case) in main.c.  To make an
 * active connection out, call rfbConnect - note that this does _not_ call
...
static rfbBool rfbNewConnectionFromSock(rfbScreenInfoPtr rfbScreen, int sock)
{
    const int one = 1;
    ...
    if (setsockopt(sock, IPPROTO_TCP, TCP_NODELAY, (char*)&one, sizeof(one)) <
        0)
    {
        rfbLogPerror("rfbCheckFds: setsockopt failed: can't set TCP_NODELAY "
                     "flag, non TCP socket?");
    }
...
#ifdef LIBVNCSERVER_IPv6
    char host[1024];
    if (getnameinfo((struct sockaddr*)&addr, addrlen, host, sizeof(host), NULL,
                    0, NI_NUMERICHOST) != 0)
    {
...
void rfbInitSockets(rfbScreenInfoPtr rfbScreen)
{
    ...
    if (rfbScreen->autoPort && rfbScreen->ipv6port > 0)
    {
        ...
        for (i = 5900; i < 6000; i++)
        {
            if ((rfbScreen->listen6Sock =
                     rfbListenOnTCP6Port(i, rfbScreen->listen6Interface)) >= 0)
            {
        ...
        rfbScreen->maxFd =
            rfbMax((int)rfbScreen->listen6Sock, rfbScreen->maxFd);
    }
#endif
    ...
    if (!rfbScreen->autoPort)
    {
        if (rfbScreen->port > 0)
        {
            ...
            if ((rfbScreen->listenSock =
                     rfbListenOnTCPPort(rfbScreen->port, iface)) < 0)
            {
        ...
        {
            if ((rfbScreen->listen6Sock = rfbListenOnTCP6Port(
                     rfbScreen->ipv6port, rfbScreen->listen6Interface)) < 0)
            {
    ...
    if (rfbScreen->udpPort != 0)
    {
        ...
        if ((rfbScreen->udpSock =
                 rfbListenOnUDPPort(rfbScreen->udpPort, iface)) < 0)
        {
...
int rfbCheckFds(rfbScreenInfoPtr rfbScreen, long usec)
{
    ...
    do
    {
        ...
        if (rfbScreen->listenSock != -1 &&
            FD_ISSET(rfbScreen->listenSock, &fds))
        {
        ...
        if (rfbScreen->listen6Sock != -1 &&
            FD_ISSET(rfbScreen->listen6Sock, &fds))
        {
        ...
        if ((rfbScreen->udpSock != -1) && FD_ISSET(rfbScreen->udpSock, &fds))
        {
            ...
            else
            {
                    ...
                    (memcmp(&addr, &rfbScreen->udpRemoteAddr, addrlen) != 0))
                {
                    ...
                    if (connect(rfbScreen->udpSock, (struct sockaddr*)&addr,
                                addrlen) < 0)
                    {
...
rfbBool rfbProcessNewConnection(rfbScreenInfoPtr rfbScreen)
{
    ...
    if (rfbScreen->listenSock >= 0 &&
        FD_ISSET(rfbScreen->listenSock, &listen_fds))
        chosen_listen_sock = rfbScreen->listenSock;
    if (rfbScreen->listen6Sock >= 0 &&
        FD_ISSET(rfbScreen->listen6Sock, &listen_fds))
...
int rfbConnect(rfbScreenInfoPtr rfbScreen, char* host, int port)
{
    ...
    {
        rfbLogPerror(
            "setsockopt failed: can't set TCP_NODELAY flag, non TCP socket?");
    }
...
int rfbListenOnTCP6Port(int port, const char* iface)
{
    ...
    for (p = servinfo; p != NULL; p = p->ai_next)
    {
...
#ifdef IPV6_V6ONLY
        /* we have separate IPv4 and IPv6 sockets since some OS's do not support
         * dual binding */
        if (setsockopt(sock, IPPROTO_IPV6, IPV6_V6ONLY, (char*)&one,
                       sizeof(one)) < 0)
        {
        ...
        {
            rfbLogPerror(
                "rfbListenOnTCP6Port: error in setsockopt SO_REUSEADDR");
            closesocket(sock);
            freeaddrinfo(servinfo);
            return -1;
        }
