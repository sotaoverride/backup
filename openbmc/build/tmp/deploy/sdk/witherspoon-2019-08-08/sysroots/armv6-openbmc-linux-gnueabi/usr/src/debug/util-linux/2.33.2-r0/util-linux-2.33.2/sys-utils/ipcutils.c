...
int ipc_sem_get_limits(struct ipc_limits* lim)
{
    ...
    if (f)
    {
        rc = fscanf(f, "%d\t%d\t%d\t%d", &lim->semmsl, &lim->semmns,
                    &lim->semopm, &lim->semmni);
        fclose(f);
    }
...
int ipc_shm_get_info(int id, struct shm_data** shmds)
{
    ...
    while (fgets(buf, sizeof(buf), f) != NULL)
    {
        ...
        if (sscanf(buf,
                   ...
                   " %" SCNu64 " %" SCNu64 "\n",
                   &p->shm_perm.key, &p->shm_perm.id, &p->shm_perm.mode,
                   &p->shm_segsz, &p->shm_cprid, &p->shm_lprid, &p->shm_nattch,
                   &p->shm_perm.uid, &p->shm_perm.gid, &p->shm_perm.cuid,
                   &p->shm_perm.cgid, &p->shm_atim, &p->shm_dtim, &p->shm_ctim,
                   &p->shm_rss, &p->shm_swp) < 14)
...
int ipc_sem_get_info(int id, struct sem_data** semds)
{
    ...
    {
        if (fscanf(f,
                   "%d %d  %o %" SCNu64 " %u %u %u %u %" SCNi64 " %" SCNi64
                   "\n",
                   &p->sem_perm.key, &p->sem_perm.id, &p->sem_perm.mode,
                   &p->sem_nsems, &p->sem_perm.uid, &p->sem_perm.gid,
                   &p->sem_perm.cuid, &p->sem_perm.cgid, &p->sem_otime,
                   &p->sem_ctime) != 10)
            continue;
...
int ipc_msg_get_info(int id, struct msg_data** msgds)
{
    ...
    while (feof(f) == 0)
    {
        if (fscanf(f,
                   ...
                   &p->msg_perm.key, &p->msg_perm.id, &p->msg_perm.mode,
                   &p->q_cbytes, &p->q_qnum, &p->q_lspid, &p->q_lrpid,
                   &p->msg_perm.uid, &p->msg_perm.gid, &p->msg_perm.cuid,
                   &p->msg_perm.cgid, &p->q_stime, &p->q_rtime,
                   &p->q_ctime) != 14)
            continue;
                    ...
                    int width)
{
    ...
    switch (unit)
    {
        ...
        case IPC_UNIT_HUMAN:
        {
            ...
            printf(format,
                   (tmp = size_to_human_string(SIZE_SUFFIX_1LETTER, size)));
            free(tmp);
            break;
        }
        default:
