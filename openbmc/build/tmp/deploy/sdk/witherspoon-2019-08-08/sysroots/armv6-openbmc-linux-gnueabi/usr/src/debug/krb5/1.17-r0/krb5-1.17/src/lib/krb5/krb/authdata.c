...
#endif
    LIBDIR "/krb5/plugins/authdata", NULL}; /* should be a list */

/* Internal authdata systems */
static krb5plugin_authdata_client_ftable_v0* authdata_systems[] = {
    &k5_mspac_ad_client_ftable, &k5_s4u2proxy_ad_client_ftable,
    &k5_authind_ad_client_ftable, NULL};
...
static krb5_error_code
    k5_ad_init_modules(krb5_context kcontext, krb5_authdata_context context,
                       ...
                       int* module_count)
{
...
#ifdef DEBUG
        fprintf(stderr,
                "warning: module \"%s\" does not advertise "
                "any AD types\n",
                table->name);
#endif
        return ENOENT;
...
#ifdef DEBUG
        fprintf(stderr,
                "warning: skipping module \"%s\" which "
    ...
    for (j = 0; table->ad_type_list[j] != 0; j++)
    {
        ...
        if (j == 0)
        {
            ...
            code =
                (*table->request_init)(kcontext, context, plugin_context, rcpp);
        ...
        fprintf(stderr, "init module \"%s\", ad_type %d, flags %08x\n",
                context->modules[k].name, context->modules[k].ad_type,
                context->modules[k].flags);
#endif
        k++;
    }
 ...
 */
static krb5_error_code k5_ad_size(krb5_context kcontext,
                                  krb5_authdata_context context,
                                  krb5_flags flags, size_t* sizep)
{
    int i;
    ...
    *sizep += sizeof(krb5_int32); /* count */
    ...
    for (i = 0; i < context->n_modules; i++)
    {
        ...
        code =
            (*module->ftable->size)(kcontext, context, module->plugin_context,
                                    *(module->request_context_pp), &size);
        if (code != 0)
            break;
 ...
 */
static krb5_error_code k5_ad_externalize(krb5_context kcontext,
                                         krb5_authdata_context context,
                                         krb5_flags flags, krb5_octet** buffer,
                                         size_t* lenremain)
{
    int i;
    krb5_error_code code;
    ...
    for (i = 0; i < context->n_modules; i++)
    {
        ...
        code = krb5_ser_pack_bytes((krb5_octet*)module->name, namelen, &bp,
                                   &remain);
        if (code != 0)
            break;

        code = (*module->ftable->externalize)(
            kcontext, context, module->plugin_context,
            *(module->request_context_pp), &bp, &remain);
        if (code != 0)
            break;
...
static struct _krb5_authdata_context_module*
    k5_ad_find_module(krb5_context kcontext, krb5_authdata_context context,
                      krb5_flags flags, const krb5_data* name)
{
    int i;
 ...
 */
static krb5_error_code k5_ad_internalize(krb5_context kcontext,
                                         krb5_authdata_context context,
                                         krb5_flags flags, krb5_octet** buffer,
                                         size_t* lenremain)
{
    ...
    for (i = 0; i < count; i++)
    {
        ...
        code = (*module->ftable->internalize)(
            kcontext, context, module->plugin_context,
            *(module->request_context_pp), &bp, &remain);
        if (code != 0)
            break;
    ...
    krb5_context kcontext, krb5_authdata_context* pcontext)
{
    ...
    *pcontext = NULL;
    ...
    if (PLUGIN_DIR_OPEN(&plugins) == 0 &&
        krb5int_open_plugin_dirs(objdirs, NULL, &plugins, &kcontext->err) ==
            0 &&
        krb5int_get_plugin_dir_data(&plugins, "authdata_client_0", &tables,
                                    &kcontext->err) == 0 &&
        tables != NULL)
    {
                                              ...
                                              krb5_authdata_context context)
{
    ...
    for (i = 0; i < context->n_modules; i++)
    {
        ...
        if (module->client_req_fini != NULL && module->request_context != NULL)
            (*module->client_req_fini)(kcontext, context,
...
static krb5_error_code
    k5_get_kdc_issued_authdata(krb5_context kcontext, const krb5_ap_req* ap_req,
                               ...
                               krb5_authdata*** kdc_issued_authdata)
{
    ...
    *kdc_issued_authdata = NULL;
     ...
     */
    code = krb5_verify_authdata_kdc_issued(
        kcontext, ap_req->ticket->enc_part2->session, authdata[0], kdc_issuer,
        kdc_issued_authdata);
...
static krb5_error_code extract_cammacs(krb5_context kcontext,
                                       krb5_authdata** cammacs,
                                       const krb5_keyblock* key,
                                       krb5_authdata*** ad_out)
{
...
static krb5_error_code get_cammac_authdata(krb5_context kcontext,
                                           const krb5_ap_req* ap_req,
                                           const krb5_keyblock* key,
                                           krb5_authdata*** elems_out)
{
    krb5_error_code ret = 0;
...
krb5_error_code krb5int_authdata_verify(krb5_context kcontext,
                                        krb5_authdata_context context,
                                        krb5_flags usage,
                                        ...
                                        const krb5_ap_req* ap_req)
{
    ...
    for (i = 0; i < context->n_modules; i++)
    {
        ...
        code = (*module->ftable->import_authdata)(
            kcontext, context, module->plugin_context,
            *(module->request_context_pp), authdata, kdc_issued_flag,
            kdc_issuer);
        if (code == 0 && module->ftable->verify != NULL)
        {
            code = (*module->ftable->verify)(
                kcontext, context, module->plugin_context,
                *(module->request_context_pp), auth_context, key, ap_req);
        }
...
static krb5_error_code k5_merge_data_list(krb5_data** dst, krb5_data* src,
                                          unsigned int* len)
{
    unsigned int i;
    krb5_data* d;
...
krb5_error_code KRB5_CALLCONV krb5_authdata_get_attribute_types(
    krb5_context kcontext, krb5_authdata_context context, krb5_data** out_attrs)
{
    int i;
    krb5_error_code code = 0;
    ...
    for (i = 0; i < context->n_modules; i++)
    {
        ...
        if ((*module->ftable->get_attribute_types)(
                kcontext, context, module->plugin_context,
                *(module->request_context_pp), &attrs2))
            continue;
    ...
    int* more)
{
    ...
    *complete = FALSE;
    ...
    for (i = 0; i < context->n_modules; i++)
    {
        ...
        code = (*module->ftable->get_attribute)(
            kcontext, context, module->plugin_context,
            *(module->request_context_pp), attribute, authenticated, complete,
    ...
    const krb5_data* attribute, const krb5_data* value)
{
    ...
    for (i = 0; i < context->n_modules; i++)
    {
        ...
        code = (*module->ftable->set_attribute)(
            kcontext, context, module->plugin_context,
            *(module->request_context_pp), complete, attribute, value);
        if (code == ENOENT)
            code = 0;
        else if (code == 0)
            found++;
    ...
    const krb5_data* attribute)
{
    ...
    for (i = 0; i < context->n_modules; i++)
    {
        ...
        code = (*module->ftable->delete_attribute)(
            kcontext, context, module->plugin_context,
            *(module->request_context_pp), attribute);
        if (code == ENOENT)
            code = 0;
        else if (code == 0)
            found++;
...
krb5_error_code KRB5_CALLCONV krb5_authdata_export_attributes(
    krb5_context kcontext, krb5_authdata_context context, krb5_flags flags,
    krb5_data** attrsp)
{
    krb5_error_code code;
    size_t required = 0;
...
krb5_error_code KRB5_CALLCONV krb5_authdata_export_internal(
    krb5_context kcontext, krb5_authdata_context context,
    krb5_boolean restrict_authenticated, const char* module_name, void** ptr)
{
    krb5_error_code code;
    krb5_data name;
    struct _krb5_authdata_context_module* module;
    ...
    *ptr = NULL;
    ...
    code = (*module->ftable->export_internal)(
        kcontext, context, module->plugin_context,
        *(module->request_context_pp), restrict_authenticated, ptr);
...
krb5_error_code KRB5_CALLCONV krb5_authdata_free_internal(
    krb5_context kcontext, krb5_authdata_context context,
    const char* module_name, void* ptr)
{
    krb5_data name;
    struct _krb5_authdata_context_module* module;
    ...
    (*module->ftable->free_internal)(kcontext, context, module->plugin_context,
                                     *(module->request_context_pp), ptr);

    return 0;
}

static krb5_error_code
    k5_copy_ad_module_data(krb5_context kcontext, krb5_authdata_context context,
                           ...
                           krb5_authdata_context dst)
{
    ...
    if (src_module->ftable->copy == NULL)
    {
        ...
        code = (*src_module->ftable->size)(kcontext, context,
                                           src_module->plugin_context,
                                           src_module->request_context, &size);
        if (code != 0)
            return code;
        ...
        code = (*src_module->ftable->externalize)(
            kcontext, context, src_module->plugin_context,
            *(src_module->request_context_pp), &bp, &remain);
        if (code != 0)
        {
            free(contents);
            return code;
        ...
        code = (*dst_module->ftable->internalize)(
            kcontext, context, dst_module->plugin_context,
            *(dst_module->request_context_pp), &bp, &remain);
        if (code != 0)
        {
            free(contents);
            return code;
    ...
    else
    {
        ...
        code = (*src_module->ftable->copy)(
            kcontext, context, src_module->plugin_context,
            src_module->request_context, dst_module->plugin_context,
 ...
 */
static krb5_error_code krb5_authdata_context_size(krb5_context kcontext,
                                                  krb5_pointer ptr,
                                                  size_t* sizep)
{
    krb5_error_code code;
 ...
 */
static krb5_error_code krb5_authdata_context_externalize(krb5_context kcontext,
                                                         krb5_pointer ptr,
                                                         krb5_octet** buffer,
                                                         size_t* lenremain)
{
 ...
 */
static krb5_error_code krb5_authdata_context_internalize(krb5_context kcontext,
                                                         krb5_pointer* ptr,
                                                         krb5_octet** buffer,
                                                         size_t* lenremain)
{
    ...
    code = k5_ad_internalize(kcontext, context, AD_USAGE_MASK, &bp, &remain);
    if (code != 0)
    {
        krb5_authdata_context_free(kcontext, context);
        return code;
...
static const krb5_ser_entry krb5_authdata_context_ser_entry = {
    ...
    krb5_authdata_context_externalize, krb5_authdata_context_internalize};

/*
 * Register the authdata context serializer.
 */
krb5_error_code krb5_ser_authdata_context_init(krb5_context kcontext)
{
    return krb5_register_serializer(kcontext, &krb5_authdata_context_ser_entry);
}

krb5_error_code krb5int_copy_authdatum(krb5_context context,
...
void KRB5_CALLCONV krb5_free_authdata(krb5_context context, krb5_authdata** val)
{
    krb5_authdata** temp;
