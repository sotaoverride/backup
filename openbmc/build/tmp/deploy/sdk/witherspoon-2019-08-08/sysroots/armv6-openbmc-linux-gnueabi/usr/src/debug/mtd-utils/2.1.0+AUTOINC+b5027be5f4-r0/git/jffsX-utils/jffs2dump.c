...
#define t32_backwards(x)                                                       \
    ({                                                                         \
        uint32_t __b = (x);                                                    \
        (target_endian == __BYTE_ORDER) ? bswap_32(__b) : __b;                 \
    })
...
static void display_help(int error)
{
    printf(
        ...
        " -e, --endianconvert=FNAME    convert image endianness, output to "
        "file fname\n"
        " -r, --recalccrc              recalc name and data crc on endian "
        "conversion\n"
        " -d, --datsize=LEN            size of data chunks, when oob data in "
        "binary image (NAND only)\n"
        " -o, --oobsize=LEN            size of oob data chunk in binary image "
        "(NAND only)\n"
...
static void do_dumpcontent(void)
{
    ...
    while (p < (data + imglen))
    {
        ...
        if (je16_to_cpu(node->u.magic) == 0xFFFF &&
            je16_to_cpu(node->u.nodetype) == 0xFFFF)
        {
            p += 4;
            empty += 4;
            continue;
        ...
        switch (je16_to_cpu(node->u.nodetype))
        {
            ...
            case JFFS2_NODETYPE_INODE:
                printf("%8s Inode      node at 0x%08zx, totlen 0x%08x, #ino  "
                       "%5d, version %5d, isize %8d, csize %8d, dsize %8d, "
                       "offset %8d\n",
                ...
                {
                    printf("Wrong node_crc at  0x%08zx, 0x%08x instead of "
                           "0x%08x\n",
                ...
                crc = mtd_crc32(0, p + sizeof(struct jffs2_raw_inode),
                                je32_to_cpu(node->i.csize));
                if (crc != je32_to_cpu(node->i.data_crc))
                {
                    printf("Wrong data_crc at  0x%08zx, 0x%08x instead of "
                           "0x%08x\n",
            ...
            case JFFS2_NODETYPE_DIRENT:
                ...
                printf("%8s Dirent     node at 0x%08zx, totlen 0x%08x, #pino "
                       "%5d, version %5d, #ino  %8d, nsize %8d, name %s\n",
                       obsolete ? "Obsolete" : "", p - data,
                ...
                crc = mtd_crc32(0, p + sizeof(struct jffs2_raw_dirent),
                                node->d.nsize);
                if (crc != je32_to_cpu(node->d.name_crc))
                {
                    printf("Wrong name_crc at  0x%08zx, 0x%08x instead of "
                           "0x%08x\n",
            ...
            case JFFS2_NODETYPE_XATTR:
                ...
                printf("%8s Xattr      node at 0x%08zx, totlen 0x%08x, xid   "
                       "%5d, version %5d, name_len   %3d, name %s\n",
                       obsolete ? "Obsolete" : "", p - data,
                       je32_to_cpu(node->x.totlen), je32_to_cpu(node->x.xid),
                       je32_to_cpu(node->x.version), node->x.name_len, name);

                crc = mtd_crc32(0, node,
                                sizeof(struct jffs2_raw_xattr) -
                                    sizeof(node->x.node_crc));
                ...
                crc = mtd_crc32(0, p + sizeof(struct jffs2_raw_xattr),
                                node->x.name_len +
                                    je16_to_cpu(node->x.value_len) + 1);
            ...
            case JFFS2_NODETYPE_XREF:
                printf("%8s Xref       node at 0x%08zx, totlen 0x%08x, xid   "
                       "%5d, xseqno  %5d, #ino  %8d\n",
                       obsolete ? "Obsolete" : "", p - data,
                       je32_to_cpu(node->r.totlen), je32_to_cpu(node->r.xid),
                       je32_to_cpu(node->r.xseqno), je32_to_cpu(node->r.ino));
            ...
            case JFFS2_NODETYPE_SUMMARY:
            {
                ...
                printf("%8s Inode Sum  node at 0x%08zx, totlen 0x%08x, sum_num "
                       " %5d, cleanmarker size %5d\n",
                       obsolete ? "Obsolete" : "", p - data,
                ...
                crc = mtd_crc32(0, p + sizeof(struct jffs2_raw_summary),
                                je32_to_cpu(node->s.totlen) -
                ...
                if (verbose)
                {
                    ...
                    for (i = 0; i < je32_to_cpu(node->s.sum_num); i++)
                    {
                            ...
                            ((struct jffs2_sum_unknown_flash*)sp)->nodetype))
                        {
                            case JFFS2_NODETYPE_INODE:
                            {
                                ...
                                printf("%14s #ino  %5d,  version %5d, offset "
                                       "0x%08x, totlen 0x%08x\n",
                            ...
                            case JFFS2_NODETYPE_DIRENT:
                            {
                                ...
                                printf("%14s dirent offset 0x%08x, totlen "
                                       "0x%08x, #pino  %5d,  version %5d, #ino "
                                       " %8d, nsize %8d, name %s \n",
                            ...
                            case JFFS2_NODETYPE_XATTR:
                            {
                                ...
                                spx = sp;
                                printf("%14s Xattr  offset 0x%08x, totlen "
                    ...
                    sm = (struct
                          jffs2_sum_marker*)((char*)p +
                                             je32_to_cpu(node->s.totlen) -
                                             sizeof(struct jffs2_sum_marker));

                    printf("%14s Sum Node Offset  0x%08x, Magic 0x%08x, Padded "
                           "size 0x%08x\n",
                           "", je32_to_cpu(sm->offset), je32_to_cpu(sm->magic),
    ...
    while (p < (data + imglen))
    {
        ...
        switch (je16_to_cpu(node->u.nodetype))
        {
            ...
            case JFFS2_NODETYPE_INODE:
                ...
                newnode.i.hdr_crc = cpu_to_e32(mtd_crc32(
                    0, &newnode, sizeof(struct jffs2_unknown_node) - 4));
                ...
                if (recalccrc)
                {
                    ...
                    newnode.i.data_crc = cpu_to_e32(
                        mtd_crc32(0, p + sizeof(struct jffs2_raw_inode), len));
                }
                else
                    newnode.i.data_crc = cnv_e32(node->i.data_crc);

                newnode.i.node_crc = cpu_to_e32(
                    mtd_crc32(0, &newnode, sizeof(struct jffs2_raw_inode) - 8));

                write_nocheck(fd, &newnode, sizeof(struct jffs2_raw_inode));
                write_nocheck(fd, p + sizeof(struct jffs2_raw_inode),
            ...
            case JFFS2_NODETYPE_DIRENT:
                ...
                newnode.d.hdr_crc = cpu_to_e32(mtd_crc32(
                    0, &newnode, sizeof(struct jffs2_unknown_node) - 4));
                ...
                newnode.d.node_crc = cpu_to_e32(mtd_crc32(
                    0, &newnode, sizeof(struct jffs2_raw_dirent) - 8));
                if (recalccrc)
                    newnode.d.name_crc = cpu_to_e32(mtd_crc32(
                        0, p + sizeof(struct jffs2_raw_dirent), node->d.nsize));
                else
            ...
            case JFFS2_NODETYPE_XATTR:
                ...
                newnode.x.hdr_crc = cpu_to_e32(mtd_crc32(
                    0, &newnode, sizeof(struct jffs2_unknown_node) - 4));
                ...
                if (recalccrc)
                    newnode.x.data_crc = cpu_to_e32(mtd_crc32(
                        0, p + sizeof(struct jffs2_raw_xattr),
                        node->x.name_len + je16_to_cpu(node->x.value_len) + 1));
                else
                    newnode.x.data_crc = cnv_e32(node->x.data_crc);
                newnode.x.node_crc =
                    cpu_to_e32(mtd_crc32(0, &newnode,
                                         sizeof(struct jffs2_raw_xattr) -
                                             sizeof(newnode.x.node_crc)));

                write_nocheck(fd, &newnode, sizeof(struct jffs2_raw_xattr));
                write_nocheck(fd, p + sizeof(struct jffs2_raw_xattr),
                              PAD(je32_to_cpu(node->d.totlen) -
                                  sizeof(struct jffs2_raw_xattr)));
                p += PAD(je32_to_cpu(node->x.totlen));
                break;
            ...
            case JFFS2_NODETYPE_PADDING:
                ...
                newnode.u.hdr_crc = cpu_to_e32(mtd_crc32(
                    0, &newnode, sizeof(struct jffs2_unknown_node) - 4));

                write_nocheck(fd, &newnode, sizeof(struct jffs2_unknown_node));
                len = PAD(je32_to_cpu(node->u.totlen) -
                          sizeof(struct jffs2_unknown_node));
                if (len > 0)
                    write_nocheck(fd, p + sizeof(struct jffs2_unknown_node),
                                  len);
            ...
            case JFFS2_NODETYPE_SUMMARY:
            {
                ...
                newnode.s.hdr_crc = cpu_to_e32(mtd_crc32(
                    0, &newnode, sizeof(struct jffs2_unknown_node) - 4));
                newnode.s.sum_num = cnv_e32(node->s.sum_num);
                newnode.s.cln_mkr = cnv_e32(node->s.cln_mkr);
                newnode.s.padded = cnv_e32(node->s.padded);

                newnode.s.node_crc = cpu_to_e32(mtd_crc32(
                    0, &newnode, sizeof(struct jffs2_raw_summary) - 8));

                // summary header
                p += sizeof(struct jffs2_raw_summary);

                // summary data
                sum_len = je32_to_cpu(node->s.totlen) -
                          sizeof(struct jffs2_raw_summary) -
                          sizeof(struct jffs2_sum_marker);
                ...
                for (i = 0; i < je32_to_cpu(node->s.sum_num); i++)
                {
                    ...
                    switch (je16_to_cpu(fl_ptr->u.nodetype))
                    {
                        ...
                        case JFFS2_NODETYPE_DIRENT:
                            ...
                            p += sizeof(struct jffs2_sum_dirent_flash) +
                                 fl_ptr->d.nsize;
                            counter += sizeof(struct jffs2_sum_dirent_flash) +
                                       fl_ptr->d.nsize;
                            break;
                        ...
                        default:
                            printf("Unknown node in summary information!!! "
                                   "nodetype(%x)\n",
                ...
                newnode.s.sum_crc = cpu_to_e32(mtd_crc32(
                    0, ((char*)node) + sizeof(struct jffs2_raw_summary),
                    je32_to_cpu(node->s.totlen) -
                        sizeof(struct jffs2_raw_summary)));
                ...
                write_nocheck(fd, &node->s.sum,
                              sum_len + sizeof(struct jffs2_sum_marker));
            ...
            default:
                printf("Unknown node type: 0x%04x at 0x%08zx, totlen 0x%08x\n",
                       je16_to_cpu(node->u.nodetype), p - data,
                       je32_to_cpu(node->u.totlen));
                p += PAD(je32_to_cpu(node->u.totlen));
        }
    }
