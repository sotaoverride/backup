...
static inline void literal_matched(lzma_range_encoder* rc,
                                   probability* subcoder, uint32_t match_byte,
                                   uint32_t symbol)
{
...
static inline void literal(lzma_lzma1_encoder* coder, lzma_mf* mf,
                           uint32_t position)
{
    // Locate the literal byte to be encoded and the subcoder.
    const uint8_t cur_byte = mf->buffer[mf->read_pos - mf->read_ahead];
    probability* subcoder = literal_subcoder(
        coder->literal, coder->literal_context_bits, coder->literal_pos_mask,
                                 ...
                                 const uint32_t pos_state)
{
    ...
    for (; i < table_size && i < LEN_LOW_SYMBOLS + LEN_MID_SYMBOLS; ++i)
        prices[i] = b0 + rc_bittree_price(lc->mid[pos_state], LEN_MID_BITS,
                                          i - LEN_LOW_SYMBOLS);

    for (; i < table_size; ++i)
        prices[i] =
            b1 + rc_bittree_price(lc->high, LEN_HIGH_BITS,
                                  i - LEN_LOW_SYMBOLS - LEN_MID_SYMBOLS);

    return;
}

static inline void length(lzma_range_encoder* rc, lzma_length_encoder* lc,
                         ...
                         const uint32_t distance, const uint32_t len)
{
    ...
    rc_bittree(&coder->rc, coder->dist_slot[dist_state], DIST_SLOT_BITS,
               dist_slot);
    ...
    if (dist_slot >= DIST_MODEL_START)
    {
        ...
        else
        {
            ...
            rc_bittree_reverse(&coder->rc, coder->dist_align, ALIGN_BITS,
...
static inline void rep_match(lzma_lzma1_encoder* coder,
                             const uint32_t pos_state, const uint32_t rep,
                             const uint32_t len)
{
    if (rep == 0)
    {
        rc_bit(&coder->rc, &coder->is_rep0[coder->state], 0);
        rc_bit(&coder->rc, &coder->is_rep0_long[coder->state][pos_state],
               len != 1);
    }
    else
    {
...
static void encode_symbol(lzma_lzma1_encoder* coder, lzma_mf* mf, uint32_t back,
    ...
    while (true)
    {
        ...
        if (limit != UINT32_MAX && (mf->read_pos - mf->read_ahead >= limit ||
                                    *out_pos + rc_pending(&coder->rc) >=
                                        LZMA2_CHUNK_MAX - LOOP_INPUT_MAX))
            break;
...
static lzma_ret lzma_encode(void* coder, lzma_mf* restrict mf,
...
static bool is_options_valid(const lzma_options_lzma* options)
{
    ...
    return is_lclppb_valid(options) && options->nice_len >= MATCH_LEN_MIN &&
...
extern lzma_ret lzma_lzma_encoder_reset(lzma_lzma1_encoder* coder,
                                        const lzma_options_lzma* options)
{
    ...
    // Length encoders
    length_encoder_reset(&coder->match_len_encoder, 1U << options->pb,
                         coder->fast_mode);

    length_encoder_reset(&coder->rep_len_encoder, 1U << options->pb,
                         coder->fast_mode);
...
extern lzma_ret lzma_lzma_encoder_create(void** coder_ptr,
                                         ...
                                         lzma_lz_options* lz_options)
{
    ...
    coder->is_initialized =
        options->preset_dict != NULL && options->preset_dict_size > 0;
    coder->is_flushed = false;
...
{
    lz->code = &lzma_encode;
    return lzma_lzma_encoder_create(&lz->coder, allocator, options, lz_options);
}
...
extern bool lzma_lzma_lclppb_encode(const lzma_options_lzma* options,
                                    uint8_t* byte)
{
