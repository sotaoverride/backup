 ...
 * to bypass this restriction we call mount(2) per flag. It's really not a
 * perfect solution, but it's the same like to execute multiple mount(8)
 * commands.
 *
                                               ...
                                               struct libmnt_addmount* ad)
{
    ...
    DBG(CXT, ul_debugobj(cxt, "mount: add additional flag: 0x%08lx",
                         ad->mountflags));
...
static int init_propagation(struct libmnt_context* cxt)
{
    ...
    DBG(CXT,
        ul_debugobj(cxt, "mount: initialize additional propagation mounts"));
    ...
    while (!mnt_optstr_next_option(&opts, &name, &namesz, NULL, NULL))
    {
        ...
        DBG(CXT,
            ul_debugobj(cxt, " adding extra mount(2) call for %s", ent->name));
        rc = mnt_context_append_additional_mount(cxt, ad);
        if (rc)
            return rc;

        DBG(CXT, ul_debugobj(cxt, " removing %s from primary mount(2) call",
                             ent->name));
...
/*
 * add additional mount(2) syscall request to implement "bind,<flags>", the
 * first regular mount(2) is the "bind" operation, the second is
 * "remount,bind,<flags>" call.
 */
...
#define DEF_OPTNAME(n)                                                         \
    {                                                                          \
        .name = n, .namesz = sizeof(n) - 1                                     \
    }
#define DEF_OPTNAME_LAST                                                       \
    {                                                                          \
        .name = NULL                                                           \
    }
                     ...
                     const struct libmnt_optname* names)
{
    ...
    {
        if (p->namesz == namesz && strncmp(name, p->name, namesz) == 0)
            return 1;
    }
...
static int fix_optstr(struct libmnt_context* cxt)
{
    ...
    static const struct libmnt_optname selinux_options[] = {
        DEF_OPTNAME("context"),    DEF_OPTNAME("fscontext"),
        DEF_OPTNAME("defcontext"), DEF_OPTNAME("rootcontext"),
        DEF_OPTNAME("seclabel"),   DEF_OPTNAME_LAST};
#endif
#ifdef HAVE_SMACK
    int sm_rem = 0;
    static const struct libmnt_optname smack_options[] = {
        DEF_OPTNAME("smackfsdef"),       DEF_OPTNAME("smackfsfloor"),
        DEF_OPTNAME("smackfshat"),       DEF_OPTNAME("smackfsroot"),
        DEF_OPTNAME("smackfstransmute"), DEF_OPTNAME_LAST};
#endif
    assert(cxt);
    ...
    if ((cxt->mountflags & MS_BIND) && (cxt->mountflags & MNT_BIND_SETTABLE) &&
    ...
    while (!mnt_optstr_next_option(&next, &name, &namesz, &val, &valsz))
    {
...
#ifdef HAVE_LIBSELINUX
        else if ((se_rem || se_fix) && is_option(name, namesz, selinux_options))
        {
            ...
            if (se_rem)
            {
                ...
                rc = mnt_optstr_remove_option_at(
                    &fs->fs_optstr, name, val ? val + valsz : name + namesz);
            }
            else if (se_fix && val && valsz)
                /* translate selinux contexts */
                rc =
                    mnt_optstr_fix_secontext(&fs->fs_optstr, val, valsz, &next);
        }
#endif
#ifdef HAVE_SMACK
        else if (sm_rem && is_option(name, namesz, smack_options))
        {
            ...
            next = name;
            rc = mnt_optstr_remove_option_at(&fs->fs_optstr, name,
    ...
    DBG(CXT, ul_debugobj(cxt,
                         ...
                         rc, fs->vfs_optstr, fs->fs_optstr, fs->user_optstr,
                         fs->optstr));
...
/*
 * Converts the already evaluated and fixed options to the form that is
 * compatible with /sbin/mount.type helpers.
 */
...
static int evaluate_permissions(struct libmnt_context* cxt)
{
    ...
    else
    {
        ...
        {
            DBG(CXT, ul_debugobj(
                         cxt, "perms: fstab not applied, ignore user mount"));
            return -EPERM;
        }
        ...
        if (cxt->user_mountflags & MNT_MS_USER)
        {
            ...
            if (!mnt_optstr_get_option(cxt->fs->user_optstr, "user", NULL,
                                       &valsz) &&
                valsz)
            {

                DBG(CXT,
                    ul_debugobj(cxt, "perms: user=<name> detected, ignore"));
        ...
        if (u_flags & (MNT_MS_OWNER | MNT_MS_GROUP))
        {
            ...
            { /* Ah... source is TAG */
                ...
                xsrc = mnt_resolve_spec(mnt_context_get_source(cxt), cache);
                srcpath = xsrc;
            }
            if (!srcpath)
            {
            ...
            if (strncmp(srcpath, "/dev/", 5) == 0 && stat(srcpath, &sb) == 0 &&
        ...
        {
            DBG(CXT,
                ul_debugobj(cxt, "permissions evaluation ends with -EPERMS"));
            return -EPERM;
        }
    }
...
static int exec_helper(struct libmnt_context* cxt)
{
    ...
    if (ns_tgt->fd != -1 &&
        asprintf(&namespace, "/proc/%i/fd/%i", getpid(), ns_tgt->fd) == -1)
    {
        free(o);
        return -ENOMEM;
    }
    ...
    switch (pid)
    {
        case 0:
        {
            ...
            if (type && strchr(type, '.') && !endswith(cxt->helper, type))
            {
                args[i++] = "-t"; /* 10 */
                args[i++] = type; /* 11 */
            }
        ...
        default:
        {
            ...
            DBG(CXT, ul_debugobj(cxt, "%s executed [status=%d, rc=%d%s]",
                                 cxt->helper, cxt->helper_status, rc,
                                 rc ? " waitpid failed" : ""));
            break;
        }
...
static int do_mount_additional(struct libmnt_context* cxt, const char* target,
                               unsigned long flags, int* syserr)
{
    struct list_head* p;
    ...
    list_for_each(p, &cxt->addmounts)
    {
        ...
        rc = mount("none", target, NULL, ad->mountflags | (flags & MS_SILENT),
                   NULL);
        if (rc)
        {
            if (syserr)
                *syserr = -errno;
            DBG(CXT, ul_debugobj(cxt, "mount(2) failed [errno=%d %m]", errno));
            return rc;
        }
    }
...
static int do_mount(struct libmnt_context* cxt, const char* try_type)
{
    ...
    if (!src)
    {
        ...
        DBG(CXT,
            ul_debugobj(cxt, "WARNING: source is NULL -- using \"none\"!"));
        src = "none";
    }
    ...
    DBG(CXT, ul_debugobj(cxt,
                         ...
                         mnt_context_is_fake(cxt) ? "(FAKE) " : "", src, target,
                         type, flags, cxt->mountdata ? "yes" : "<none>"));
...
static int do_mount_by_types(struct libmnt_context* cxt, const char* types)
{
    ...
    do
    {
        ...
        if (strcmp(p, "auto") == 0)
        {
            ...
            {
                DBG(CXT,
                    ul_debugobj(cxt, "failed to guess FS type [rc=%d]", rc));
                free(p0);
                free(autotype);
                return rc;
            }
...
int mnt_context_do_mount(struct libmnt_context* cxt)
{
    ...
    if (mnt_context_get_status(cxt) && !mnt_context_is_fake(cxt) &&
        !cxt->helper && mnt_context_mtab_writable(cxt))
    {
         ...
         */
        if ((cxt->mountflags & MS_BIND) && (cxt->mountflags & MS_RDONLY))
        {
 ...
 * @cxt: mount context
 *
 * High-level, mounts the filesystem by mount(2) or
 *fork()+exec(/sbin/mount.type).
 *
 * This is similar to:
 *
 ...
 * different settings, then you have to call mnt_reset_context(). It's NOT
 *enough to call mnt_context_reset_status(). If you want to call this function
 *more than once, the whole context has to be reset.
 *
 * WARNING: non-zero return code does not mean that mount(2) syscall or
 *          mount.type helper wasn't successfully called.
 *
...
again:
     ...
     */
    if ((rc == -EROFS &&
         !mnt_context_syscall_called(cxt)) /* before syscall; rdonly loopdev */
        || mnt_context_get_syscall_errno(cxt) ==
               EROFS /* syscall failed with EROFS */
        || mnt_context_get_syscall_errno(cxt) ==
               EACCES) /* syscall failed with EACCES */
    {
        ...
        if (!(mflags & MS_RDONLY)     /* not yet RDONLY */
            ...
            && !mnt_context_is_rwonly_mount(cxt))
        { /* no explicit read-write */

            assert(!(cxt->flags & MNT_FL_FORCED_RDONLY));
            DBG(CXT,
                ul_debugobj(cxt, "write-protected source, trying RDONLY."));
 ...
 */
int mnt_context_next_mount(struct libmnt_context* cxt, struct libmnt_iter* itr,
                           struct libmnt_fs** fs, int* mntrc, int* ignored)
{
    ...
    if (mnt_fs_is_swaparea(*fs) ||
        ...
        (cxt->fstype_pattern &&
         !mnt_fs_match_fstype(*fs, cxt->fstype_pattern)) ||

        /* ignore filesystems which don't match type patterns */
        (cxt->optstr_pattern &&
         !mnt_fs_match_options(*fs, cxt->optstr_pattern)))
    {
        if (ignored)
            *ignored = 1;
        DBG(CXT, ul_debugobj(
                     ...
                     mnt_fs_get_fstype(*fs), cxt->fstype_pattern,
                     mnt_fs_get_options(*fs), cxt->optstr_pattern));
        return 0;
    }
...
static int is_shared_tree(struct libmnt_context* cxt, const char* dir)
{
    ...
    rc = fs && mnt_fs_is_kernel(fs) &&
         mnt_fs_get_propagation(fs, &mflags) == 0 && (mflags & MS_SHARED);
done:
    free(mnt);
    if (!mnt_context_switch_ns(cxt, ns_old))
        return -MNT_ERR_NAMESPACE;
...
int mnt_context_get_mount_excode(struct libmnt_context* cxt, int rc, char* buf,
                                 size_t bufsz)
{
    int syserr;
    struct stat st;
    ...
    if (!mnt_context_syscall_called(cxt))
    {
        ...
        switch (rc)
        {
            ...
            case -MNT_ERR_AMBIFS:
                if (buf)
                    snprintf(buf, bufsz,
                             _("more filesystems detected on %s; use -t <type> "
                               "or wipefs(8)"),
                             src);
                return MNT_EX_USAGE;
            ...
            case -MNT_ERR_MOUNTOPT:
                if (buf)
                    snprintf(buf, bufsz,
                             errno ? _("failed to parse mount options: %m")
    ...
    else if (mnt_context_get_syscall_errno(cxt) == 0)
    {
        ...
        if (rc == -MNT_ERR_LOCK)
        {
            if (buf)
                snprintf(buf, bufsz,
                         _("filesystem was mounted, but failed to update "
                           "userspace mount table"));
            return MNT_EX_FILEIO;
        }
        else if (rc == -MNT_ERR_NAMESPACE)
        {
            if (buf)
                snprintf(buf, bufsz,
                         _("filesystem was mounted, but failed to switch "
                           "namespace back"));
            return MNT_EX_SYSERR;
        }
        else if (rc < 0)
            return mnt_context_get_generic_excode(
                rc, buf, bufsz,
                _("filesystem was mounted, but any subsequent operation "
                  "failed: %m"));
    ...
    switch (syserr)
    {
        ...
        case EBUSY:
        {
            ...
            if (src && mnt_context_get_mtab(cxt, &tb) == 0)
            {
                ...
                while (mnt_table_next_fs(tb, &itr, &fs) == 0)
                {
                    ...
                    {
                        snprintf(buf, bufsz, _("%s already mounted on %s"), s,
                                 t);
                        break;
                    }
                }
        ...
        case EROFS:
            ...
            if (mflags & MS_RDONLY)
                ...
                snprintf(buf, bufsz,
                         _("%s is write-protected but explicit read-write mode "
                           "requested"),
                         src);
            else if (mflags & MS_REMOUNT)
                snprintf(buf, bufsz,
                         _("cannot remount %s read-write, is write-protected"),
                         src);
        ...
        case EBADMSG:
            ...
            {
                snprintf(buf, bufsz,
                         _("cannot mount; probably corrupted filesystem on %s"),
                         src);
                break;
            }
            /* fallthrough */
