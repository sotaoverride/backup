                     ...
                     const struct blkid_idmag* mag __attribute__((__unused__)))
{
    ...
    uint32_t mags[] = {UFS2_MAGIC,    UFS_MAGIC,     UFS_MAGIC_FEA,
                       UFS_MAGIC_LFN, UFS_MAGIC_SEC, UFS_MAGIC_4GB};
    size_t i;
    uint32_t magic;
    ...
    for (i = 0; i < ARRAY_SIZE(offsets); i++)
    {
        ...
        ufs = (struct ufs_super_block*)blkid_probe_get_buffer(
            pr, offsets[i] * 1024, sizeof(struct ufs_super_block));
        if (!ufs)
    ...
    if (ufs->fs_id[0] || ufs->fs_id[1])
    {
        if (is_be)
            blkid_probe_sprintf_uuid(
                pr, (unsigned char*)&ufs->fs_id, sizeof(ufs->fs_id), "%08x%08x",
                be32_to_cpu(ufs->fs_id[0]), be32_to_cpu(ufs->fs_id[1]));
        else
            blkid_probe_sprintf_uuid(
                pr, (unsigned char*)&ufs->fs_id, sizeof(ufs->fs_id), "%08x%08x",
                le32_to_cpu(ufs->fs_id[0]), le32_to_cpu(ufs->fs_id[1]));
    }

    if (blkid_probe_set_magic(
            pr,
            (offsets[i] * 1024) + offsetof(struct ufs_super_block, fs_magic),
            sizeof(ufs->fs_magic), (unsigned char*)&ufs->fs_magic))
        return 1;
 ...
 */
const struct blkid_idinfo ufs_idinfo = {.name = "ufs",
