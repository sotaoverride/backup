 ...
 */
int mnt_context_find_umount_fs(struct libmnt_context* cxt, const char* tgt,
                               struct libmnt_fs** pfs)
{
     ...
     * (non-writable mtab).
     */
    if (mnt_context_is_nocanonicalize(cxt) && !mnt_context_mtab_writable(cxt) &&
        *tgt == '/')
...
try_loopdev:
    ...
    if (!fs && mnt_context_is_swapmatch(cxt))
    {
        ...
        if (fs)
        {
            struct libmnt_fs* fs1 = mnt_table_find_target(
                mtab, mnt_fs_get_target(fs), MNT_ITER_BACKWARD);
            if (!fs1)
            {
    ...
    if (!fs && !loopdev && mnt_context_is_swapmatch(cxt))
    {
        ...
        if (mnt_stat_mountpoint(tgt, &st) == 0 && S_ISREG(st.st_mode))
        {
            ...
            if (count == 1)
            {
                DBG(CXT, ul_debugobj(cxt, "umount: %s --> %s (retry)", tgt,
                                     loopdev));
                tgt = loopdev;
                goto try_loopdev;
            }
    ...
    DBG(CXT, ul_debugobj(cxt, "umount fs: %s",
                         fs ? mnt_fs_get_target(fs) : "<not found>"));
    return fs ? 0 : 1;
err:
    free(loopdev);
    if (!mnt_context_switch_ns(cxt, ns_old))
...
static int lookup_umount_fs(struct libmnt_context* cxt)
{
     ...
     */
    if (!mnt_context_is_restricted(cxt) && *tgt == '/' &&
        !(cxt->flags & MNT_FL_HELPER) && !mnt_context_mtab_writable(cxt) &&
        !mnt_context_is_force(cxt) && !mnt_context_is_lazy(cxt) &&
        !mnt_context_is_nocanonicalize(cxt) && !mnt_context_is_loopdel(cxt) &&
        ...
        !has_utab_entry(cxt, tgt))
    {
        ...
        /* !mnt_context_mtab_writable(cxt) && has_utab_entry() verified that
         * there is no stuff in utab, so disable all mtab/utab related actions
         */
        ...
        if (type)
        {
            DBG(CXT, ul_debugobj(cxt, "umount: mountinfo unnecessary [type=%s]",
                                 type));
            return 0;
        }
    }
...
static int evaluate_permissions(struct libmnt_context* cxt)
{
    ...
    {
        DBG(CXT, ul_debugobj(cxt, "cannot find %s in mtab and you are not root",
    ...
    if (!fs)
    {
        ...
        if (fs)
        {
            struct libmnt_cache* cache = mnt_context_get_cache(cxt);
            const char* sp =
                mnt_fs_get_srcpath(cxt->fs); /* devname from mtab */
        ...
        {
            DBG(CXT,
                ul_debugobj(cxt, "umount %s: mtab disagrees with fstab", tgt));
            goto eperm;
        }
    }
    ...
    {
        DBG(CXT,
            ul_debugobj(cxt, "umount: promiscuous setting ('users') in fstab"));
        return 0;
    }
    /*
    ...
    if (u_flags & (MNT_MS_USER | MNT_MS_OWNER | MNT_MS_GROUP))
    {
        ...
        DBG(CXT,
            ul_debugobj(cxt, "umount: checking user=<username> from mtab"));
        ...
        if (optstr && !mnt_optstr_get_option(optstr, "user", &mtab_user, &sz) &&
            sz)
...
static int do_umount(struct libmnt_context* cxt)
{
    ...
    DBG(CXT, ul_debugobj(cxt, "umount(2) [target='%s', flags=0x%08x]%s", target,
                         flags, mnt_context_is_fake(cxt) ? " (FAKE)" : ""));
    ...
    if (rc < 0 && cxt->syscall_status == -EBUSY &&
        mnt_context_is_rdonly_umount(cxt) && src)
    {
        ...
        {
            cxt->syscall_status = -errno;
            DBG(CXT, ul_debugobj(cxt, "read-only re-mount(2) failed [errno=%d]",
                                 -cxt->syscall_status));
...
int mnt_context_do_umount(struct libmnt_context* cxt)
{
    ...
    if (mnt_context_get_status(cxt) && !mnt_context_is_fake(cxt))
    {
         ...
         */
        if (mnt_context_is_loopdel(cxt) && !(cxt->mountflags & MS_REMOUNT))
            rc = mnt_context_delete_loopdev(cxt);

        if (!mnt_context_is_nomtab(cxt) && mnt_context_get_status(cxt) &&
            ...
            (cxt->mountflags & MS_REMOUNT))
        {
            ...
            if (!rc && cxt->update && mnt_context_mtab_writable(cxt))
                rc = mnt_update_set_fs(cxt->update, cxt->mountflags, NULL,
                                       cxt->fs);
        }
    }
end:
 ...
 * @cxt: umount context
 *
 * High-level, umounts filesystem by umount(2) or
 *fork()+exec(/sbin/umount.type).
 *
 * This is similar to:
 *
 ...
 */
int mnt_context_next_umount(struct libmnt_context* cxt, struct libmnt_iter* itr,
                            struct libmnt_fs** fs, int* mntrc, int* ignored)
{
    ...
    DBG(CXT, ul_debugobj(cxt,
                         "next-umount: trying %s [fstype: %s, t-pattern: %s, "
                         "options: %s, O-pattern: %s]",
                         tgt, mnt_fs_get_fstype(*fs), cxt->fstype_pattern,
...
int mnt_context_get_umount_excode(struct libmnt_context* cxt, int rc, char* buf,
                                  size_t bufsz)
{
    ...
    else if (mnt_context_get_syscall_errno(cxt) == 0)
    {
        ...
        if (rc == -MNT_ERR_LOCK)
        {
            if (buf)
                snprintf(buf, bufsz,
                         _("filesystem was unmounted, but failed to update "
        ...
        else if (rc == -MNT_ERR_NAMESPACE)
        {
            if (buf)
                snprintf(buf, bufsz,
                         _("filesystem was unmounted, but failed to switch "
                           "namespace back"));
            return MNT_EX_SYSERR;
        }
        else if (rc < 0)
            return mnt_context_get_generic_excode(
                rc, buf, bufsz,
                _("filesystem was unmounted, but any subsequent operation "
                  "failed: %m"));
    ...
    if (buf)
    {
        ...
        switch (syserr)
        {
            ...
            default:
                return mnt_context_get_generic_excode(
                    syserr, buf, bufsz, _("umount(2) system call failed: %m"));
        }
    }
    return MNT_EX_FAIL;
}
