...
#define USE_BUF(channel)                                                       \
    ((channel)->encoding ? (channel)->encoded_read_buf : (channel)->read_buf)
...
guint g_io_add_watch(GIOChannel* channel, GIOCondition condition, GIOFunc func,
                     gpointer user_data)
{
    return g_io_add_watch_full(channel, G_PRIORITY_DEFAULT, condition, func,
                               user_data, NULL);
}
 ...
 **/
void g_io_channel_set_line_term(GIOChannel* channel, const gchar* line_term,
                                gint length)
{
    g_return_if_fail(channel != NULL);
                                 ...
                                 GError** error)
{
    ...
    return (*channel->funcs->io_set_flags)(channel, flags & G_IO_FLAG_SET_MASK,
                                           error);
}
 ...
 **/
GIOStatus g_io_channel_seek_position(GIOChannel* channel, gint64 offset,
                                     GSeekType type, GError** error)
{
    ...
    switch (type)
    {
        case G_SEEK_CUR: /* The user is seeking relative to the head of the
                            buffer */
            if (channel->use_buffer)
            {
                ...
                if (channel->encoded_read_buf)
                {
                    g_assert(channel->encoded_read_buf->len == 0 ||
                             !channel->do_encode);

                    /* If there's anything here, it's because the encoding is
                     * UTF-8, so we can just subtract the buffer length, the
                     * same as for the unencoded data.
                     */
...
GIOStatus g_io_channel_flush(GIOChannel* channel, GError** error)
{
    ...
    do
    {
        ...
        status = channel->funcs->io_write(
            channel, channel->write_buf->str + bytes_written,
            channel->write_buf->len - bytes_written, &this_time, error);
...
void g_io_channel_set_buffered(GIOChannel* channel, gboolean buffered)
{
    ...
    {
        g_warning(
            "Need to have NULL encoding to set the buffering state of the "
            "channel.");
        return;
    }
 ...
 */
GIOStatus g_io_channel_set_encoding(GIOChannel* channel, const gchar* encoding,
                                    GError** error)
{
    ...
    {
        g_warning(
            "Need to set the channel buffered before setting the encoding.");
        ...
        if (err != 0)
        {
            ...
            if (err == EINVAL)
                g_set_error(error, G_CONVERT_ERROR,
                            ...
                            _("Conversion from character set “%s” to “%s” is "
                              "not supported"),
                            from_enc, to_enc);
            else
    ...
    if (channel->encoded_read_buf && channel->encoded_read_buf->len > 0)
    {
        g_assert(!did_encode); /* Encoding UTF-8, NULL doesn't use
                                  encoded_read_buf */
...
static GIOStatus g_io_channel_fill_buffer(GIOChannel* channel, GError** err)
{
    ...
    if (channel->is_seekable && channel->write_buf &&
        channel->write_buf->len > 0)
    {
        status = g_io_channel_flush(channel, err);
        if (status != G_IO_STATUS_NORMAL)
            return status;
    ...
    g_string_set_size(channel->read_buf,
                      channel->read_buf->len + channel->buf_size);
    ...
    reencode:
        ...
        errnum = g_iconv(channel->read_cd, &inbuf, &inbytes_left, &outbuf,
                         &outbytes_left);
        errval = errno;

        g_assert(inbuf + inbytes_left ==
                 channel->read_buf->str + channel->read_buf->len);
        ...
        if (errnum == (gsize)-1)
        {
            switch (errval)
            {
                ...
                default:
                    g_assert(errval !=
                             EBADF); /* The converter should be open */
    ...
    else if (channel->encoding) /* UTF-8 */
    {
        ...
        if (lastchar > channel->read_buf->str)
        {
            ...
            g_string_append_len(channel->encoded_read_buf,
                                channel->read_buf->str, copy_len);
 ...
 * @length: (out) (optional): location to store length of the read data, or
 *%NULL
 * @terminator_pos: (out) (optional): location to store position of line
 *terminator, or %NULL
                                 ...
                                 GError** error)
{
    ...
    status = g_io_channel_read_line_backend(channel, &got_length,
 ...
 *          be overwritten.
 * @terminator_pos: (nullable): location to store position of line terminator,
 *or %NULL
 ...
 * Returns: the status of the operation.
 **/
GIOStatus g_io_channel_read_line_string(GIOChannel* channel, GString* buffer,
                                        gsize* terminator_pos, GError** error)
{
    ...
    status =
        g_io_channel_read_line_backend(channel, &length, terminator_pos, error);
                                                ...
                                                GError** error)
{
    ...
    if (!channel->use_buffer)
    {
        ...
        g_set_error_literal(
            error, G_CONVERT_ERROR, G_CONVERT_ERROR_FAILED,
    ...
    while (TRUE)
    {
        ...
        use_buf =
            USE_BUF(channel); /* The buffer has been created by this point */
        ...
        for (nextchar = use_buf->str + checked_to; nextchar < lastchar;
             channel->encoding ? nextchar = g_utf8_next_char(nextchar)
                               : nextchar++)
        {
            if (channel->line_term)
            {
        ...
        if (status == G_IO_STATUS_EOF)
        {
            ...
            {
                g_set_error_literal(
                    error, G_CONVERT_ERROR, G_CONVERT_ERROR_PARTIAL_INPUT,
 ...
 **/
GIOStatus g_io_channel_read_to_end(GIOChannel* channel, gchar** str_return,
 ...
 * Returns: the status of the operation.
 */
GIOStatus g_io_channel_read_chars(GIOChannel* channel, gchar* buf, gsize count,
                                  gsize* bytes_read, GError** error)
{
    ...
    if (!channel->use_buffer)
    {
        ...
        status =
            channel->funcs->io_read(channel, buf, count, &tmp_bytes, error);
    ...
    /* Don't validate for NULL encoding, binary safe */
    {
        ...
        do
        {
            ...
            nextchar = g_utf8_next_char(nextchar);
            g_assert(nextchar !=
                     prevchar); /* Possible for *prevchar of -1 or -2 */
 ...
 * Returns: a #GIOStatus
 **/
GIOStatus g_io_channel_read_unichar(GIOChannel* channel, gunichar* thechar,
                                    GError** error)
{
    ...
    while (BUF_LEN(channel->encoded_read_buf) == 0 &&
           status == G_IO_STATUS_NORMAL)
        status = g_io_channel_fill_buffer(channel, error);
 ...
 *g_io_channel_write_chars () may only be made on a channel from which data has
 *been read in the cases described in the documentation for
 *g_io_channel_set_encoding ().
 *
 * Returns: the status of the operation.
 **/
GIOStatus g_io_channel_write_chars(GIOChannel* channel, const gchar* buf,
                                   ...
                                   GError** error)
{
    ...
    if (!channel->use_buffer)
    {
        ...
        status =
            channel->funcs->io_write(channel, buf, count, &tmp_bytes, error);
    ...
    while (wrote_bytes < count)
    {
        ...
        if (channel->write_buf->len >= channel->buf_size - MAX_CHAR_SIZE)
        {
            ...
            {
                status = channel->funcs->io_write(
                    channel, channel->write_buf->str + did_write,
                    channel->write_buf->len - did_write, &this_time, error);
                did_write += this_time;
            } while (status == G_IO_STATUS_NORMAL &&
        ...
        space_in_buf =
            MAX(channel->buf_size, channel->write_buf->allocated_len - 1) -
        ...
        reconvert:
            ...
            if (!channel->do_encode) /* UTF-8 encoding */
            {
                ...
                if (!g_utf8_validate_len(from_buf, try_len, &badchar))
                {
                    ...
                    try_char =
                        g_utf8_get_char_validated(badchar, incomplete_len);
                    ...
                    switch (try_char)
                    {
                        case -2:
                            ...
                            break;
                        case -1:
                            g_warning("Invalid UTF-8 passed to "
                                      "g_io_channel_write_chars().");
                            /* FIXME bail here? */
                            errnum = EILSEQ;
                            err = (gsize)-1;
            ...
            else
            {
                ...
                g_string_set_size(channel->write_buf,
                                  channel->write_buf->len + space_in_buf);
                outbuf = channel->write_buf->str + channel->write_buf->len -
                         space_in_buf;
                err = g_iconv(channel->write_cd, (gchar**)&from_buf, &left_len,
                              &outbuf, &space_in_buf);
                errnum = errno;
                g_string_truncate(channel->write_buf,
                                  channel->write_buf->len - space_in_buf);
            }
                ...
                switch (errnum)
                {
                    case EINVAL:
                        ...
                        if (from_buf_old_len == 0)
                        {
                            ...
                            memcpy(channel->partial_write_buf, from_buf,
                                   left_len);
                    ...
                    case EILSEQ:
                        ...
                        if (from_buf_old_len > 0 && from_buf_len == left_len)
                            g_warning(
                                "Illegal sequence due to partial character "
                                "at the end of a previous write.");
                        else
                            wrote_bytes +=
                                from_buf_len - left_len - from_buf_old_len;
                        if (bytes_written)
                            *bytes_written = wrote_bytes;
                        channel->partial_write_buf[0] = '\0';
                        return G_IO_STATUS_ERROR;
 ...
 * Returns: a #GIOStatus
 **/
GIOStatus g_io_channel_write_unichar(GIOChannel* channel, gunichar thechar,
                                     GError** error)
{
    GIOStatus status;
    gchar static_buf[6];
    ...
    status = g_io_channel_write_chars(channel, static_buf, char_len, &wrote_len,
                                      error);
