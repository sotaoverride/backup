                     ...
                     STACK_OF(X509_ALGOR) * mdalgs, const ASN1_ITEM* it)
{
    ...
    if ((flags & SMIME_DETACHED) && data)
    {
        ...
        BIO_printf(bio, "\"; boundary=\"----%s\"%s%s", bound, mime_eol,
                   mime_eol);
        BIO_printf(bio, "This is an S/MIME signed message%s%s", mime_eol,
                   mime_eol);
        ...
        BIO_printf(bio, "%s------%s--%s%s", mime_eol, bound, mime_eol,
                   mime_eol);
        return 1;
    }
    ...
    BIO_printf(bio, "Content-Transfer-Encoding: base64%s%s", mime_eol,
               mime_eol);
