...
static int _pam_parse(const pam_handle_t* pamh, int flags, int argc,
                      const char** argv, const char** maildir,
                      size_t* hashcount)
{
    int ctrl = 0;
...
static int get_folder(pam_handle_t* pamh, int ctrl, const char* path_mail,
                      char** folder_p, size_t hashcount,
                      const struct passwd* pwd)
{
    ...
    if (ctrl & PAM_NEW_MAIL_DIR)
    {
        ...
        { /* support for $HOME delivery */
            ...
            if (!*++path || (*path == '/' && !*++path))
            {
                pam_syslog(pamh, LOG_ERR, "badly formed mail path [%s]",
                           path_mail);
    ...
    hashcount =
        hashcount < strlen(pwd->pw_name) ? hashcount : strlen(pwd->pw_name);
...
static int get_mail_status(pam_handle_t* pamh, int ctrl, const char* folder)
{
    int type = 0;
    struct stat mail_st;
...
static int report_mail(pam_handle_t* pamh, int ctrl, int type,
                       const char* folder)
{
    int retval;
    ...
    else
    {
        if (ctrl & PAM_STANDARD_MAIL)
            ...
            switch (type)
            {
                ...
                case HAVE_NEW_MAIL:
                    retval = pam_info(
                        pamh, _("You have new mail in folder %s."), folder);
                    break;
                case HAVE_OLD_MAIL:
                    retval = pam_info(
                        pamh, _("You have old mail in folder %s."), folder);
                    break;
                case HAVE_MAIL:
                default:
...
static int _do_mail(pam_handle_t* pamh, int flags, int argc, const char** argv,
                    int est)
{
    ...
    if ((est && !(ctrl & PAM_NO_LOGIN)) || (!est && (ctrl & PAM_LOGOUT_TOO)))
    {
