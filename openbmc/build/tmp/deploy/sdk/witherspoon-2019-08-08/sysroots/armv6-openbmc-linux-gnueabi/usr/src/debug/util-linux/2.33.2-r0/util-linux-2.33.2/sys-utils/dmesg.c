...
static const struct dmesg_name facility_names[] = {
    ...
    [FAC_BASE(LOG_SYSLOG)] = {"syslog",
                              N_("messages generated internally by syslogd")},
    ...
    [FAC_BASE(
        LOG_AUTHPRIV)] = {"authpriv",
                          N_("security/authorization messages (private)")},
...
static void __attribute__((__noreturn__)) usage(void)
{
    ...
    fputs(_(" -D, --console-off           disable printing messages to "
            "console\n"),
          out);
    fputs(
        _(" -E, --console-on            enable printing messages to console\n"),
        out);
    fputs(_(" -F, --file <file>           use the file instead of the kernel "
            "log buffer\n"),
          out);
    fputs(_(" -f, --facility <list>       restrict output to defined "
            "facilities\n"),
          out);
    fputs(_(" -H, --human                 human readable output\n"), out);
    fputs(_(" -k, --kernel                display kernel messages\n"), out);
    fputs(_(" -L, --color[=<when>]        colorize messages (auto, always or "
            "never)\n"),
          out);
    fprintf(out, "                               %s\n", USAGE_COLORS_DEFAULT);
    fputs(_(" -l, --level <list>          restrict output to defined levels\n"),
          out);
    fputs(_(" -n, --console-level <level> set level of messages printed to "
            "console\n"),
          out);
    fputs(_(" -P, --nopager               do not pipe output into a pager\n"),
          out);
    fputs(_(" -p, --force-prefix          force timestamp output on each line "
            "of multi-line messages\n"),
          out);
    fputs(_(" -r, --raw                   print the raw message buffer\n"),
          out);
    fputs(_(" -S, --syslog                force to use syslog(2) rather than "
            "/dev/kmsg\n"),
          out);
    fputs(_(" -s, --buffer-size <size>    buffer size to query the kernel ring "
            "buffer\n"),
          out);
    fputs(_(" -u, --userspace             display userspace messages\n"), out);
    fputs(_(" -w, --follow                wait for new messages\n"), out);
    fputs(_(" -x, --decode                decode facility and level to "
            "readable string\n"),
          out);
    fputs(_(" -d, --show-delta            show time delta between printed "
            "messages\n"),
          out);
    fputs(_(" -e, --reltime               show local time and time delta in "
            "readable format\n"),
          out);
    fputs(_(" -T, --ctime                 show human-readable timestamp (may "
            "be inaccurate!)\n"),
          out);
    fputs(_(" -t, --notime                don't show any timestamp with "
            "messages\n"),
          out);
    fputs(
        _("     --time-format <format>  show timestamp using the given "
          "format:\n"
...
/*
 * LEVEL     ::= <number> | <name>
 *  <number> ::= @len is set:  number in range <0..N>, where N <
 * ARRAY_SIZE(level_names)
 *           ::= @len not set: number in range <1..N>, where N <=
 * ARRAY_SIZE(level_names) <name>   ::= case-insensitive text
 *
...
static const char* skip_item(const char* begin, const char* end,
                             const char* sep)
{
    while (begin < end)
    {
                                  ...
                                  struct dmesg_record* rec)
{
    ...
    if (ctl->method != DMESG_METHOD_MMAP && ctl->method != DMESG_METHOD_SYSLOG)
        return -1;
    ...
    for (i = 0; i < rec->next_size; i++)
    {
        ...
        if (*begin == '<')
        {
            if (ctl->fltr_lev || ctl->fltr_fac || ctl->decode || ctl->color)
                begin = parse_faclev(begin + 1, &rec->facility, &rec->level);
            else
                begin = skip_item(begin, end, ">");
        }

        if (*begin == '[' && (*(begin + 1) == ' ' || isdigit(*(begin + 1))))
        {
...
{
    if (ctl->fltr_lev && (rec->facility < 0 || !isset(ctl->levels, rec->level)))
        return 0;

    if (ctl->fltr_fac &&
        (rec->facility < 0 || !isset(ctl->facilities, rec->facility)))
        return 0;
...
static struct tm* record_localtime(struct dmesg_control* ctl,
                                   struct dmesg_record* rec, struct tm* tm)
{
    time_t t = ctl->boot_time.tv_sec + rec->tv.tv_sec;
    return localtime_r(&t, tm);
}

static char* record_ctime(struct dmesg_control* ctl, struct dmesg_record* rec,
...
{
    struct timeval tv = {.tv_sec = ctl->boot_time.tv_sec + rec->tv.tv_sec,
...
static void print_record(struct dmesg_control* ctl, struct dmesg_record* rec)
{
    ...
    if (ctl->raw)
    {
        ctl->indent = snprintf(tsbuf, sizeof(tsbuf), "<%d>[%5ld.%06ld] ",
    ...
    if (ctl->decode && (rec->level > -1) &&
        (rec->level < (int)ARRAY_SIZE(level_names)) && (rec->facility > -1) &&
        (rec->facility < (int)ARRAY_SIZE(facility_names)))
        snprintf(fpbuf, sizeof(fpbuf),
                 "%-6s:%-6s: ", facility_names[rec->facility].name,
    ...
    switch (ctl->time_fmt)
    {
        ...
        case DMESG_TIMEFTM_RELTIME:
            ...
            else
            {
                if (delta < 10)
                    ctl->indent =
                        snprintf(tsbuf, sizeof(tsbuf), "[  %+8.06f] ", delta);
                else
                    ctl->indent =
                        snprintf(tsbuf, sizeof(tsbuf), "[ %+9.06f] ", delta);
            }
            ctl->lasttm = cur;
            break;
        ...
        case DMESG_TIMEFTM_TIME_DELTA:
            ctl->indent =
                snprintf(tsbuf, sizeof(tsbuf), "[%5ld.%06ld <%12.06f>] ",
                         (long)rec->tv.tv_sec, (long)rec->tv.tv_usec,
                         record_count_delta(ctl, rec));
            break;
 ...
 */
static void print_buffer(struct dmesg_control* ctl, const char* buf,
                         size_t size)
{
...
static int parse_kmsg_record(struct dmesg_control* ctl,
                             struct dmesg_record* rec, char* buf, size_t sz)
{
    ...
    if (ctl->fltr_lev || ctl->fltr_fac || ctl->decode || ctl->raw || ctl->color)
...
mesg:
     ...
     */
    rec->mesg_size =
        unhexmangle_to_buffer(rec->mesg, (char*)rec->mesg, p - rec->mesg + 1);
...
int main(int argc, char* argv[])
{
                            ...
                            longopts, NULL)) != -1)
    {
        ...
        switch (c)
        {
            ...
            case 'f':
                ctl.fltr_fac = 1;
                if (string_to_bitarray(optarg, ctl.facilities, parse_facility) <
                    0)
                    return EXIT_FAILURE;
                break;
            case 'H':
            ...
            case 'l':
                ctl.fltr_lev = 1;
                if (string_to_bitarray(optarg, ctl.levels, parse_level) < 0)
                    return EXIT_FAILURE;
                break;
            case 'n':
            ...
            case 's':
                ctl.bufsize =
                    strtou32_or_err(optarg, _("invalid buffer size argument"));
                if (ctl.bufsize < 4096)
                    ctl.bufsize = 4096;
                break;
            case 'T':
    ...
    if (delta)
        switch (ctl.time_fmt)
        {
            ...
            case DMESG_TIMEFTM_TIME:
                ...
                break;
            case DMESG_TIMEFTM_ISO8601:
                warnx(_("--show-delta is ignored when used together with "
                        "iso8601 time format"));
                break;
            default:
    ...
    switch (ctl.action)
    {
        ...
        case SYSLOG_ACTION_READ_CLEAR:
            ...
            if (ctl.raw && ctl.method != DMESG_METHOD_KMSG &&
