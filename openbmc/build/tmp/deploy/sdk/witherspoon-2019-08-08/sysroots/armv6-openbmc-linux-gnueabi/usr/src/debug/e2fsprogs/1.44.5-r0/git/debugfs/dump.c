...
static struct
{
    __u16 lmask;
    mode_t mask;
} mode_table[] = {{LINUX_S_IRUSR, S_IRUSR}, {LINUX_S_IWUSR, S_IWUSR},
                  {LINUX_S_IXUSR, S_IXUSR}, {LINUX_S_IRGRP, S_IRGRP},
                  {LINUX_S_IWGRP, S_IWGRP}, {LINUX_S_IXGRP, S_IXGRP},
                  {LINUX_S_IROTH, S_IROTH}, {LINUX_S_IWOTH, S_IWOTH},
                  {LINUX_S_IXOTH, S_IXOTH}, {0, 0}};
...
static void fix_perms(const char* cmd, const struct ext2_inode* inode, int fd,
                      const char* name)
{
    struct utimbuf ut;
    int i;
...
static void dump_file(const char* cmdname, ext2_ino_t ino, int fd, int preserve,
                      char* outname)
{
    errcode_t retval;
...
void do_dump(int argc, char** argv)
{
    ...
    if (fd < 0)
    {
        com_err(argv[0], errno, "while opening %s for dump_inode", out_fn);
        return;
    }
    ...
    if (close(fd) != 0)
    {
        com_err(argv[0], errno, "while closing %s for dump_inode", out_fn);
        return;
    }
                          ...
                          const char* fullname)
{
    ...
    {
        com_err("rdump", errno, "while creating symlink %s -> %s", buf,
                fullname);
        goto errout;
    }
                        ...
                        const char* name, const char* dumproot)
{
    ...
    else if (LINUX_S_ISREG(inode->i_mode))
    {
        int fd;
        fd =
            open(fullname, O_WRONLY | O_CREAT | O_TRUNC | O_LARGEFILE, S_IRWXU);
        if (fd == -1)
        {
    ...
    else if (LINUX_S_ISDIR(inode->i_mode) && strcmp(name, ".") &&
             strcmp(name, ".."))
    {
        errcode_t retval;
