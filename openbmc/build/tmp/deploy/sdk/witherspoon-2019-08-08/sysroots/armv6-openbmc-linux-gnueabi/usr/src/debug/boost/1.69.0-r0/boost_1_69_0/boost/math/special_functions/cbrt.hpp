...
{
    typedef typename mpl::if_<boost::is_convertible<big_int_type, T>,
                              boost::uintmax_t, unsigned int>::type type;
};
...
T cbrt_imp(T z, const Policy& pol)
{
    ...
    if (!(boost::math::isfinite)(z))
    {
        return policies::raise_domain_error(
            "boost::math::cbrt<%1%>(%1%)",
            "Argument to function must be finite but got %1%.", z, pol);
    }
    ...
    // checking of the boilerplate version as we know in advance that the
    ...
    typedef typename policies::normalise<
        Policy, policies::digits2<new_prec::value>>::type new_policy;
    //
    // Epsilon calculation uses compile time arithmetic when it's available for
    // type T, otherwise uses ldexp to calculate at runtime:
    //
    T eps = (new_prec::value > 3) ? policies::get_epsilon<T, new_policy>()
                                  : ldexp(T(1), -2 - tools::digits<T>() / 3);
    T diff;
    ...
    {
        //
        // Either we're ready to overflow, or we can't tell because
        // numeric_limits isn't available for type T:
        //
        do
        {
            T g2 = guess * guess;
