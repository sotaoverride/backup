...
static dbus_bool_t process_content(ExpatParseContext* context)
{
    ...
    {
        if (!bus_config_parser_content(context->parser, &context->content,
                                       context->error))
        {
            context->failed = TRUE;
            return FALSE;
...
static void expat_StartElementHandler(void* userData, const XML_Char* name,
                                      const XML_Char** atts)
{
    ...
    if (!bus_config_parser_start_element(context->parser, name,
                                         (const char**)names,
                                         (const char**)values, context->error))
    {
        dbus_free(names);
        dbus_free(values);
        context->failed = TRUE;
...
static void expat_CharacterDataHandler(void* userData, const XML_Char* s,
                                       int len)
{
    ExpatParseContext* context = userData;
    if (context->failed)
                                 ...
                                 DBusError* error)
{
    ...
    XML_SetElementHandler(expat, expat_StartElementHandler,
