...
struct BOOST_PYTHON_DECL list_base : object
{
    ...
    ssize_t
        count(object_cref value) const; // return number of occurrences of value

    void extend(
        object_cref sequence); // extend list by appending sequence elements

    long index(
        object_cref value) const; // return index of first occurrence of value
...
class list : public detail::list_base
{
  ...
  public:
    ...
    template <class T>
    explicit list(T const& sequence) : base(object(sequence))
    {
    }
