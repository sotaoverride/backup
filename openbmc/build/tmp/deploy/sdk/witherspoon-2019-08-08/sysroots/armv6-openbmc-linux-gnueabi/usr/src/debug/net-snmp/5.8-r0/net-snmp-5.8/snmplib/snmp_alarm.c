 ...
 */
/** @defgroup snmp_alarm  generic library based alarm timers for various parts
 * of an application
 *  @ingroup library
 *
 *  @{
...
void snmp_alarm_unregister(unsigned int clientreg)
{
    ...
    for (sa_ptr = thealarms; sa_ptr != NULL && sa_ptr->clientreg != clientreg;
         sa_ptr = sa_ptr->next)
    {
    ...
    if (sa_ptr != NULL)
    {
        *prevNext = sa_ptr->next;
        DEBUGMSGTL(
            ("snmp_alarm", "unregistered alarm %d\n", sa_ptr->clientreg));
        /*
...
void set_an_alarm(void)
{
                                             ...
                                             NETSNMP_DS_LIB_ALARM_DONT_USE_SIG))
    {
        ...
        DEBUGMSGTL(("snmp_alarm", "schedule alarm %d in %ld.%03ld seconds\n",
                    nextalarm, (long)delta.tv_sec,
                    (long)(delta.tv_usec / 1000)));
#else /* HAVE_SETITIMER */
#ifdef SIGALRM
        signal(SIGALRM, alarm_handler);
        alarm(delta.tv_sec);
        DEBUGMSGTL(("snmp_alarm", "schedule alarm %d in roughly %ld seconds\n",
