...
static gboolean g_menu_link_hash_iter_get_next(GMenuLinkIter* link_iter,
...
static void g_menu_link_hash_iter_finalize(GObject* object)
{
    GMenuLinkHashIter* iter = (GMenuLinkHashIter*)object;

    g_hash_table_unref(iter->table);

    G_OBJECT_CLASS(g_menu_link_hash_iter_parent_class)->finalize(object);
}

static void g_menu_link_hash_iter_init(GMenuLinkHashIter* iter)
{
}

static void g_menu_link_hash_iter_class_init(GMenuLinkHashIterClass* class)
{
...
G_DEFINE_TYPE(GMenuAttributeHashIter, g_menu_attribute_hash_iter,
...
static void g_menu_attribute_hash_iter_finalize(GObject* object)
{
    GMenuAttributeHashIter* iter = (GMenuAttributeHashIter*)object;

    g_hash_table_unref(iter->table);

    G_OBJECT_CLASS(g_menu_attribute_hash_iter_parent_class)->finalize(object);
}

static void g_menu_attribute_hash_iter_init(GMenuAttributeHashIter* iter)
{
}
                                              ...
                                              gint item_index)
{
    ...
    G_MENU_MODEL_GET_CLASS(model)->get_item_attributes(model, item_index,
                                                       &table);

    if (table)
    {
        GMenuAttributeHashIter* iter =
            g_object_new(g_menu_attribute_hash_iter_get_type(), NULL);
    ...
    {
        g_critical("GMenuModel implementation '%s' doesn't override "
...
static GVariant* g_menu_model_real_get_item_attribute_value(
    GMenuModel* model, gint item_index, const gchar* attribute,
...
static GMenuLinkIter* g_menu_model_real_iterate_item_links(GMenuModel* model,
                                                           gint item_index)
{
    GHashTable* table = NULL;
    GMenuLinkIter* result;

    G_MENU_MODEL_GET_CLASS(model)->get_item_links(model, item_index, &table);

    if (table)
    {
        GMenuLinkHashIter* iter =
            g_object_new(g_menu_link_hash_iter_get_type(), NULL);
        g_hash_table_iter_init(&iter->iter, table);
        iter->table = g_hash_table_ref(table);
...
static GMenuModel* g_menu_model_real_get_item_link(GMenuModel* model,
                                                   gint item_index,
                                                   const gchar* link)
{
    GHashTable* table = NULL;
...
static void g_menu_model_init(GMenuModel* model)
{
}

static void g_menu_model_class_init(GMenuModelClass* class)
{
    class->iterate_item_attributes = g_menu_model_real_iterate_item_attributes;
    class->get_item_attribute_value =
    ...
    g_menu_model_items_changed_signal =
        g_signal_new(I_("items-changed"), G_TYPE_MENU_MODEL, G_SIGNAL_RUN_LAST,
...
{
    return G_MENU_MODEL_GET_CLASS(model)->is_mutable(model);
}
...
{
    return G_MENU_MODEL_GET_CLASS(model)->get_n_items(model);
}
...
GMenuAttributeIter* g_menu_model_iterate_item_attributes(GMenuModel* model,
                                                         gint item_index)
{
    return G_MENU_MODEL_GET_CLASS(model)->iterate_item_attributes(model,
                                                                  item_index);
}
 ...
 */
GVariant*
    g_menu_model_get_item_attribute_value(GMenuModel* model, gint item_index,
                                          const gchar* attribute,
                                          const GVariantType* expected_type)
{
    return G_MENU_MODEL_GET_CLASS(model)->get_item_attribute_value(
 ...
 * Since: 2.32
 */
gboolean g_menu_model_get_item_attribute(GMenuModel* model, gint item_index,
                                         const gchar* attribute,
                                         const gchar* format_string, ...)
{
    ...
    value = g_menu_model_get_item_attribute_value(model, item_index, attribute,
                                                  NULL);
...
GMenuLinkIter* g_menu_model_iterate_item_links(GMenuModel* model,
                                               gint item_index)
{
    return G_MENU_MODEL_GET_CLASS(model)->iterate_item_links(model, item_index);
}
 ...
 * Since: 2.32
 */
GMenuModel* g_menu_model_get_item_link(GMenuModel* model, gint item_index,
                                       const gchar* link)
{
    return G_MENU_MODEL_GET_CLASS(model)->get_item_link(model, item_index,
                                                        link);
}
 ...
 * Since: 2.32
 */
void g_menu_model_items_changed(GMenuModel* model, gint position, gint removed,
                                gint added)
{
    g_signal_emit(model, g_menu_model_items_changed_signal, 0, position,
                  removed, added);
}
...
G_DEFINE_ABSTRACT_TYPE_WITH_PRIVATE(GMenuAttributeIter, g_menu_attribute_iter,
                                    G_TYPE_OBJECT)
                                        ...
                                        GVariant** value)
{
    ...
    iter->priv->valid = G_MENU_ATTRIBUTE_ITER_GET_CLASS(iter)->get_next(
        iter, &name, &iter->priv->value);
...
static void g_menu_attribute_iter_finalize(GObject* object)
{
    ...
    G_OBJECT_CLASS(g_menu_attribute_iter_parent_class)->finalize(object);
}

static void g_menu_attribute_iter_init(GMenuAttributeIter* iter)
{
    iter->priv = g_menu_attribute_iter_get_instance_private(iter);
}

static void g_menu_attribute_iter_class_init(GMenuAttributeIterClass* class)
{
    GObjectClass* object_class = G_OBJECT_CLASS(class);
...
G_DEFINE_ABSTRACT_TYPE_WITH_PRIVATE(GMenuLinkIter, g_menu_link_iter,
                                    G_TYPE_OBJECT)
                                   ...
                                   GMenuModel** value)
{
    ...
    iter->priv->valid = G_MENU_LINK_ITER_GET_CLASS(iter)->get_next(
        iter, &name, &iter->priv->value);
...
static void g_menu_link_iter_finalize(GObject* object)
{
    ...
    G_OBJECT_CLASS(g_menu_link_iter_parent_class)->finalize(object);
}

static void g_menu_link_iter_init(GMenuLinkIter* iter)
{
    iter->priv = g_menu_link_iter_get_instance_private(iter);
}

static void g_menu_link_iter_class_init(GMenuLinkIterClass* class)
{
    GObjectClass* object_class = G_OBJECT_CLASS(class);
