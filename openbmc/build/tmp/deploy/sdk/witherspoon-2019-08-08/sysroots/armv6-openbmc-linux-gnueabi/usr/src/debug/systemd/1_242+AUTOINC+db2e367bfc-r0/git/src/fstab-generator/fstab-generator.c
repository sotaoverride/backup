...
static int add_swap(const char* what, struct mntent* me, MountpointFlags flags)
{
    ...
    {
        log_info("Running in a container, ignoring fstab swap entry for %s.",
                 what);
        return 0;
    }
                         ...
                         const char* filter, const char* variable)
{
    ...
    if (r < 0)
    {
        log_warning("Failed to parse timeout for %s, ignoring: %s", where,
                    timeout);
        return 0;
    }
...
{
    return write_timeout(f, where, opts, "x-systemd.idle-timeout\0",
                         "TimeoutIdleSec");
}

static int write_mount_timeout(FILE* f, const char* where, const char* opts)
{
    return write_timeout(f, where, opts, "x-systemd.mount-timeout\0",
                         "TimeoutSec");
}

static int write_dependency(FILE* f, const char* opts, const char* filter,
                            const char* format)
{
...
{
    return write_dependency(f, opts, "x-systemd.requires",
                            "After=%1$s\nRequires=%1$s\n");
}
...
static int add_mount(const char* dest, const char* what, const char* where,
                     const char* original_where, const char* fstype,
                     const char* opts, int passno, MountpointFlags flags,
                     const char* post, const char* source)
{
    ...
    /* All mounts under /sysroot need to happen later, at initrd-fs.target time.
     * IOW, it's not technically part of the basic initrd filesystem itself, and
     * so shouldn't inherit the default Before=local-fs.target dependency. */
    if (in_initrd() && path_startswith(where, "/sysroot"))
        fprintf(f, "DefaultDependencies=no\n");

    if (STRPTR_IN_SET(fstype, "nfs", "nfs4") && !(flags & AUTOMOUNT) &&
        fstab_test_yes_no_option(opts, "bg\0"
                                       "fg\0"))
    {
        ...
        opts = strjoina("x-systemd.mount-timeout=infinity,retry=10000,", opts,
                        ",fg");
    ...
    if (!(flags & NOAUTO) && !(flags & AUTOMOUNT))
    {
        r = generator_add_symlink(
            dest, post, (flags & NOFAIL) ? "wants" : "requires", name);
        if (r < 0)
            return r;
    }
    ...
    if (flags & AUTOMOUNT)
    {
        ...
        r = fflush_and_check(f);
        if (r < 0)
            return log_error_errno(r, "Failed to write unit file %s: %m",
                                   automount_name);
...
static int parse_fstab(bool initrd)
{
    ...
    while ((me = getmntent(f)))
    {
        ...
        if (is_path(where))
        {
            ...
            /* Follow symlinks here; see
             * 5261ba901845c084de5a8fd06500ed09bfb0bd80 which makes sense for
             * mount units, but causes problems since it historically worked to
             * have symlinks in e.g. /etc/fstab. So we canonicalize here. Note
             * that we use CHASE_NONEXISTENT to handle the case where a symlink
             * refers to another mount target; this works assuming the
             * sub-mountpoint target is the final directory. */
            r = chase_symlinks(where, initrd ? "/sysroot" : NULL,
                               CHASE_PREFIX_ROOT | CHASE_NONEXISTENT,
                               &canonical_where);
            if (r < 0) /* If we can't canonicalize we continue on as if it
                          wasn't a symlink */
                log_debug_errno(
                    r, "Failed to read symlink target for %s, ignoring: %m",
                    where);
            else if (streq(canonical_where,
                           where)) /* If it was fully canonicalized, suppress
                                      the change */
                canonical_where = mfree(canonical_where);
            else
                log_debug("Canonicalized what=%s where=%s to %s", what, where,
                          canonical_where);
        }

        makefs = fstab_test_option(me->mnt_opts, "x-systemd.makefs\0");
        growfs = fstab_test_option(me->mnt_opts, "x-systemd.growfs\0");
        noauto = fstab_test_yes_no_option(me->mnt_opts, "noauto\0"
                                                        "auto\0");
        nofail = fstab_test_yes_no_option(me->mnt_opts, "nofail\0"
                                                        "fail\0");

        log_debug("Found entry what=%s where=%s type=%s makefs=%s growfs=%s "
                  "noauto=%s nofail=%s",
                  what, where, me->mnt_type, yes_no(makefs), yes_no(growfs),
                  yes_no(noauto), yes_no(nofail));

        if (streq(me->mnt_type, "swap"))
            k = add_swap(what, me,
                         makefs * MAKEFS | growfs * GROWFS | noauto * NOAUTO |
                             nofail * NOFAIL);
        else
        {
            bool automount;
            ...
            automount =
                fstab_test_option(me->mnt_opts, "comment=systemd.automount\0"
            ...
            k = add_mount(arg_dest, what, canonical_where ?: where,
                          canonical_where ? where : NULL, me->mnt_type,
                          me->mnt_opts, me->mnt_passno,
                          makefs * MAKEFS | growfs * GROWFS | noauto * NOAUTO |
                              nofail * NOFAIL | automount * AUTOMOUNT,
                          post, fstab_path);
        }
...
static int add_sysroot_mount(void)
{
    ...
    else if (arg_root_rw >= 0 || !fstab_test_option(arg_root_options, "ro\0"
                                                                      "rw\0"))
    ...
    return add_mount(
        arg_dest, what, "/sysroot", NULL, arg_root_fstype, opts,
...
static int add_sysroot_usr_mount(void)
{
    ...
    {
        /* Copy over the root device, in case the /usr mount just differs in a
         * mount option (consider btrfs subvolumes) */
    ...
    if (!arg_usr_options)
        opts = arg_root_rw > 0 ? "rw" : "ro";
    else if (!fstab_test_option(arg_usr_options, "ro\0"
                                                 "rw\0"))
    ...
    return add_mount(arg_dest, what, "/sysroot/usr", NULL, arg_usr_fstype, opts,
                     is_device_path(what) ? 1 : 0, /* passno */
                     0, SPECIAL_INITRD_FS_TARGET, "/proc/cmdline");
}

static int add_volatile_root(void)
{

    /* Let's add in systemd-remount-volatile.service which will remount the root
     * device to tmpfs if this is requested (or as an overlayfs), leaving only
     * /usr from the root mount inside. */

    if (!IN_SET(arg_volatile_mode, VOLATILE_YES, VOLATILE_OVERLAY))
        return 0;

    return generator_add_symlink(
        arg_dest, SPECIAL_INITRD_ROOT_FS_TARGET, "requires",
...
static int add_volatile_var(void)
{
    ...
    /* If requested, mount /var as tmpfs, but do so only if there's nothing else
     * defined for this. */

    return add_mount(arg_dest_late, "tmpfs", "/var", NULL, "tmpfs", "mode=0755",
                     0, 0, SPECIAL_LOCAL_FS_TARGET, "/proc/cmdline");
}
                                   ...
                                   void* data)
{
    ...
    else if (streq(key, "systemd.volatile"))
    {
        ...
        if (value)
        {
            ...
            if (m < 0)
                log_warning("Failed to parse systemd.volatile= argument: %s",
                            value);
            else
                arg_volatile_mode = m;
        }
...
{
    /* If we have a root hash but no root device then Verity is used, and we use
     * the "root" DM device as root. */
...
static int run(const char* dest, const char* dest_early, const char* dest_late)
{
    ...
    /* Always honour root= and usr= in the kernel command line if we are in an
     * initrd */
    if (in_initrd())
    {
        r = add_sysroot_mount();
