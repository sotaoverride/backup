...
void throw_(typename Exception::error_code code, S1 msg, Pos const& pos)
{
    ...
    stream.freeze(false);
    boost::throw_exception(Exception(throwmsg.c_str(), code, pos.get_line(),
                                     pos.get_column(), pos.get_file().c_str()));
}

template <typename Exception, typename Context, typename S1, typename Pos>
void throw_(Context& ctx, typename Exception::error_code code, S1 msg,
            Pos const& pos)
{
    std::strstream stream;
    stream << Exception::severity_text(code) << ": "
    ...
    ctx.get_hooks().throw_exception(
        ctx.derived(), Exception(throwmsg.c_str(), code, pos.get_line(),
                                 pos.get_column(), pos.get_file().c_str()));
}
