...
/// INTERNAL ONLY
///
#define BOOST_PROTO_OFFSETOF(s, m)                                             \
    (BOOST_PROTO_ADDROF((((s*)this)->m)) - BOOST_PROTO_ADDROF(*((s*)this)))
#else
/// INTERNAL ONLY
///
...
#define BOOST_PROTO_DEFINE_FUN_OP_IMPL_(Z, N, DATA, Const)                     \
    BOOST_PP_IF(N, BOOST_PROTO_TEMPLATE_YES_, BOOST_PROTO_TEMPLATE_NO_)        \
    (Z, N) BOOST_PROTO_DISABLE_MSVC_C4714 BOOST_FORCEINLINE                    \
        typename BOOST_PROTO_RESULT_OF<proto_generator(                        \
            typename boost::proto::result_of::BOOST_PP_CAT(funop, N) <         \
                proto_derived_expr Const(),                                    \
            proto_domain BOOST_PP_ENUM_TRAILING_PARAMS_Z(Z, N, const A) >      \
                ::type)>::type const                                           \
        operator()(BOOST_PP_ENUM_BINARY_PARAMS_Z(Z, N, A, const& a)) Const()   \
    {                                                                          \
        typedef boost::proto::result_of::BOOST_PP_CAT(                         \
            funop,                                                             \
            N)<proto_derived_expr Const(),                                     \
               proto_domain BOOST_PP_ENUM_TRAILING_PARAMS_Z(Z, N, const A)>    \
            funop;                                                             \
        return proto_generator()(                                              \
            funop::call(*static_cast<proto_derived_expr Const()*>(this)        \
...
#define BOOST_PROTO_DEFINE_FUN_OP_VARIADIC_IMPL_(Const)                        \
    ...
    BOOST_PROTO_DISABLE_MSVC_C4714 BOOST_FORCEINLINE                           \
        typename BOOST_PROTO_RESULT_OF<proto_generator(                        \
            typename boost::proto::result_of::funop<                           \
                proto_derived_expr Const()(A const&...), proto_derived_expr,   \
                proto_domain>::type)>::type const                              \
    ...
    {                                                                          \
        typedef boost::proto::result_of::funop<                                \
            ...
            proto_domain>                                                      \
            funop;                                                             \
        return proto_generator()(funop::call(                                  \
            *static_cast<proto_derived_expr Const()*>(this), a...));           \
    }                                                                          \
    /**/
...
#define BOOST_PROTO_EXTENDS_CHILD(Z, N, DATA)                                  \
    typedef typename proto_base_expr::BOOST_PP_CAT(proto_child, N)             \
...
#define BOOST_PROTO_BASIC_EXTENDS_(Expr, Derived, Domain)                      \
    ...
    typedef typename proto_base_expr::address_of_hack_type_                    \
    ...
    BOOST_PROTO_DISABLE_MSVC_C4714                                             \
        BOOST_FORCEINLINE static proto_derived_expr const                      \
        make(Expr const& e)                                                    \
    {                                                                          \
        ...
        return that;                                                           \
                                                                               ...
                                                                               \
    BOOST_PROTO_DISABLE_MSVC_C4714 BOOST_FORCEINLINE proto_base_expr&          \
        proto_base()                                                           \
    {                                                                          \
        return this->proto_expr_.proto_base();                                 \
    }                                                                          \
                                                                               \
    BOOST_PROTO_DISABLE_MSVC_C4714 BOOST_FORCEINLINE proto_base_expr const&    \
        proto_base() const                                                     \
    {                                                                          \
        return this->proto_expr_.proto_base();                                 \
    }                                                                          \
    ...
    BOOST_PROTO_DISABLE_MSVC_C4714 BOOST_FORCEINLINE Typename()                \
        BOOST_PROTO_RESULT_OF<Typename() This::proto_generator(                \
            Typename() boost::proto::base_expr<                                \
                Typename() This::proto_domain, boost::proto::tag::assign,      \
                boost::proto::list2<This&, This Const()&>>::type)>::type const \
...
// MSVC 8.0 and higher seem to need copy-assignment operator to be overloaded on
// *both* const and non-const rhs arguments.
...
#define BOOST_PROTO_EXTENDS_ASSIGN_IMPL_(ThisConst, ThatConst)                 \
    ...
    BOOST_PROTO_DISABLE_MSVC_C4714 BOOST_FORCEINLINE                           \
        typename BOOST_PROTO_RESULT_OF<proto_generator(                        \
            typename boost::proto::base_expr<                                  \
                proto_domain, boost::proto::tag::assign,                       \
                boost::proto::list2<                                           \
                    proto_derived_expr ThisConst()&,                           \
                    typename boost::proto::result_of::as_child<                \
                        A ThatConst(), proto_domain>::type>>::type)>::         \
            type const                                                         \
        operator=(A ThatConst() & a) ThisConst()                               \
    {                                                                          \
        typedef typename boost::proto::base_expr<                              \
            proto_domain, boost::proto::tag::assign,                           \
            boost::proto::list2<proto_derived_expr ThisConst()&,               \
                                typename boost::proto::result_of::as_child<    \
                                    A ThatConst(), proto_domain>::type>>::type \
            that_type;                                                         \
...
#define BOOST_PROTO_EXTENDS_SUBSCRIPT_IMPL_(ThisConst, ThatConst)              \
    ...
    BOOST_PROTO_DISABLE_MSVC_C4714 BOOST_FORCEINLINE                           \
        typename BOOST_PROTO_RESULT_OF<proto_generator(                        \
            typename boost::proto::base_expr<                                  \
                proto_domain, boost::proto::tag::subscript,                    \
                boost::proto::list2<                                           \
                    proto_derived_expr ThisConst()&,                           \
                    typename boost::proto::result_of::as_child<                \
                        A ThatConst(), proto_domain>::type>>::type)>::         \
...
#define BOOST_PROTO_EXTENDS_FUNCTION_()                                        \
    ...
    {                                                                          \
        typedef typename BOOST_PROTO_RESULT_OF<proto_generator(                \
            typename boost::proto::result_of::funop<                           \
                Sig, proto_derived_expr, proto_domain>::type)>::type const     \
            type;                                                              \
    };                                                                         \
    /**/
...
#define BOOST_PROTO_EXTENDS_FUNCTION_CONST()                                   \
    BOOST_PROTO_EXTENDS_FUNCTION_()                                            \
    BOOST_PP_REPEAT_FROM_TO(0, BOOST_PROTO_MAX_FUNCTION_CALL_ARITY,            \
...
/// \brief extends\<\> class template for adding behaviors to a Proto expression
/// template
///
...
struct extends
{
    ...
    BOOST_FORCEINLINE
    extends(extends const& that) : proto_expr_(that.proto_expr_)
    {
    }

    BOOST_FORCEINLINE
    extends(Expr const& expr_) : proto_expr_(expr_)
    {
    }
...
#undef BOOST_PROTO_NO_WAVE_OUTPUT
    ...
    {
        using std::size_t;
        return *(This*)((char*)this -
                        BOOST_PROTO_OFFSETOF(This, proto_member_union_start_));
    }
...
#define BOOST_PROTO_EXTENDS_MEMBER_(R, DOMAIN, ELEM)                           \
    boost::proto::exprns_::virtual_member<                                     \
        proto_derived_expr, BOOST_PP_TUPLE_ELEM(2, 0, ELEM), DOMAIN>           \
