...
DEFINE_STRING_TABLE_LOOKUP_WITH_BOOLEAN(show_status, ShowStatus,
                                        SHOW_STATUS_YES);
...
int status_vprintf(const char* status, ShowStatusFlags flags,
                   const char* format, va_list ap)
{
    ...
    /* Before you ask: yes, on purpose we open/close the console for each status
     * line we write individually. This is a good strategy to avoid PID 1
     * getting killed by the kernel's SAK concept (it doesn't fix this entirely,
     * but minimizes the time window the kernel might end up killing PID 1 due
     * to SAK). It also makes things easier for us so that we don't have to
     * recover from hangups and suchlike triggered on the console. */
