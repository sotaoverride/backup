...
#define SETNODEPTR(NP, P)                                                      \
    (*NP) = (node)((((uintptr_t)(*NP)) & (uintptr_t)0x1) | (uintptr_t)(P))
#define LEFT(N) (node)((N)->left_node & ~((uintptr_t)0x1))
#define LEFTPTR(N) (node*)(&(N)->left_node)
#define SETLEFT(N, L)                                                          \
    (N)->left_node = (((N)->left_node & (uintptr_t)0x1) | (uintptr_t)(L))
...
static void check_tree_recurse(node p, int d_sofar, int d_total)
{
    ...
    check_tree_recurse(LEFT(p), d_sofar + (LEFT(p) && !RED(LEFT(p))), d_total);
                                   ...
                                   int p_r, int gp_r, int mode)
{
    ...
    if (mode == 1 || ((rpn) != NULL && (lpn) != NULL && RED(rpn) && RED(lpn)))
    {
...
libc_hidden_def(__tsearch) weak_alias(__tsearch, tsearch)

    /* Find datum in search tree.
       KEY is the key to be located, ROOTP is the address of tree root,
       COMPAR the ordering function.  */
    void* __tfind(const void* key, void* const* vrootp, __compar_fn_t compar)
{
    node root;
    node* rootp = (node*)vrootp;
...
libc_hidden_def(__tfind) weak_alias(__tfind, tfind)
...
libc_hidden_def(__tdelete) weak_alias(__tdelete, tdelete)
...
libc_hidden_def(__twalk) weak_alias(__twalk, twalk)
...
libc_hidden_def(__tdestroy) weak_alias(__tdestroy, tdestroy)
