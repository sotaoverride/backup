...
static int connection_init(Connection* c, DispatchContext* dispatch_ctx,
                           DispatchFn dispatch_fn, User* user, int fd)
{
    ...
    *connection = (Connection)CONNECTION_NULL(*connection);
    ...
    r = dispatch_file_init(&connection->socket_file, dispatch_ctx, dispatch_fn,
                           fd, EPOLLHUP | EPOLLIN | EPOLLOUT,
                           EPOLLIN | EPOLLOUT);
    if (r)
        return error_fold(r);
 ...
 */
int connection_init_server(Connection* c, DispatchContext* dispatch_ctx,
                           DispatchFn dispatch_fn, User* user, const char* guid,
                           int fd)
{
    int r;

    r = connection_init(c, dispatch_ctx, dispatch_fn, user, fd);
    if (r)
 ...
 */
int connection_init_client(Connection* c, DispatchContext* dispatch_ctx,
                           DispatchFn dispatch_fn, User* user, int fd)
{
    int r;
...
static int connection_feed_sasl(Connection* connection, const char* input,
                                size_t n_input)
{
    const char* output;
    size_t n_output;
    ...
    {
        r = sasl_server_dispatch(&connection->sasl_server, input, n_input,
                                 &output, &n_output);
        if (r)
        {
            switch (r)
    ...
    {
        r = sasl_client_dispatch(&connection->sasl_client, input, n_input,
                                 &output, &n_output);
        if (r)
        {
            switch (r)
    ...
    connection->authenticated =
        connection->server ? sasl_server_is_done(&connection->sasl_server)
