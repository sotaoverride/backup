...
static void SECTION ss32(mp_no* x, mp_no* y, int p)
{
    int i;
    double a;
...
/* Compute Multi-Precision cos() function for given p. Receive Multi Precision
   number x and result stored at y.  */
static void SECTION cc32(mp_no* x, mp_no* y, int p)
{
    int i;
    double a;
    mp_no mpt1, x2, gor, sum, mpk = {1, {1.0}};
...
void SECTION __c32(mp_no* x, mp_no* y, mp_no* z, int p)
{
    mp_no u, t, t1, t2, c, s;
    int i;
    __cpy(x, &u, p);
...
double SECTION __sin32(double x, double res, double res1)
{
    int p;
    mp_no a, b, c;
    p = 32;
...
double SECTION __cos32(double x, double res, double res1)
{
    int p;
    mp_no a, b, c;
    p = 32;
...
double SECTION __mpsin(double x, double dx, bool reduce_range)
{
    double y;
    mp_no a, b, c, s;
    int n;
    ...
    switch (n)
    {
        ...
        /* Quadrant not set, so the result must be sin (X + DX), which is also
           in S.  */
        case 0:
        default:
            __mp_dbl(&s, &y, p);
...
/* Compute cos() of double-length number (X + DX) as Multi Precision number and
   return result as double.  If REDUCE_RANGE is true, X is assumed to be the
   original input and DX is ignored.  */
double SECTION __mpcos(double x, double dx, bool reduce_range)
{
    double y;
    mp_no a, b, c, s;
    int n;
...
int SECTION __mpranred(double x, mp_no* y, int p)
{
    number v;
    double t, xn;
    int i, k, n;
