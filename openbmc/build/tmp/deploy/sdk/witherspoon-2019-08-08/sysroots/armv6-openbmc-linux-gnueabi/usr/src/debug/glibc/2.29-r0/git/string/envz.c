...
libc_hidden_def(envz_entry)
    ...
    /* Returns a pointer to the value portion of the entry in ENVZ for NAME, or
       0 if there is none.  */
...
libc_hidden_def(envz_remove)
    ...
    /* Adds an entry for NAME with value VALUE to ENVZ & ENVZ_LEN.  If an entry
       ...
       return NULL, although envz_entry will still return an entry; this is
       handy because when merging with another envz, the null entry can override
       an entry in the other one.  Null entries can be removed with envz_strip
       ().  */
    error_t
