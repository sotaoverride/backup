...
static rfbBool HandleUltraBPP(rfbClient* client, int rx, int ry, int rw, int rh)
{
    rfbZlibHeader hdr;
    int toRead = 0;
    int inflateResult = 0;
    ...
    {
        rfbClientLog("ultra error: rectangle has 0 uncomressed bytes ((%dw * "
                     "%dh) * (%d / 8))\n",
                     rw, rh, BPP);
        return FALSE;
    }
    ...
    inflateResult = lzo1x_decompress_safe((lzo_byte*)client->ultra_buffer,
                                          toRead, (lzo_byte*)client->raw_buffer,
                                          (lzo_uintp)&uncompressedBytes, NULL);

    /* Note that uncompressedBytes will be 0 on output overrun */
    if ((rw * rh * (BPP / 8)) != uncompressedBytes)
        rfbClientLog(
            "Ultra decompressed unexpected amount of data (%d != %d)\n",
            (rw * rh * (BPP / 8)), uncompressedBytes);

    /* Put the uncompressed contents of the update on the screen. */
    if (inflateResult == LZO_E_OK)
    {
        client->GotBitmap(client, (unsigned char*)client->raw_buffer, rx, ry,
                          rw, rh);
    }
    else
    {
...
static rfbBool HandleUltraZipBPP(rfbClient* client, int rx, int ry, int rw,
                                 int rh)
{
    rfbZlibHeader hdr;
    int i = 0;
    ...
    if (uncompressedBytes == 0)
    {
        rfbClientLog("ultrazip error: rectangle has 0 uncomressed bytes (%dy + "
                     "(%dw * 65535)) (%d rectangles)\n",
                     ry, rw, rx);
        return FALSE;
    }
