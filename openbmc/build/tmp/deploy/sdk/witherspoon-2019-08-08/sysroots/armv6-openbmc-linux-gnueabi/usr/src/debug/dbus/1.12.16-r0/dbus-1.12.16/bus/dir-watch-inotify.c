...
/* dir-watch-inotify.c  OS specific directory change notification for message
 * bus
 *
...
static dbus_bool_t _handle_inotify_watch(DBusWatch* passed_watch,
                                         unsigned int flags, void* data)
{
    ...
    {
        _dbus_verbose(
            "Sending SIGHUP signal on reception of %ld inotify event(s)\n",
            (long)ret);
...
static void _set_watched_dirs_internal(DBusList** directories)
{
    ...
    for (i = 0; new_dirs[i]; i++)
    {
        if (new_wds[i] == -1)
        {
            /* FIXME - less lame error handling for failing to add a watch; we
             * may need to sleep. */
            wd = inotify_add_watch(inotify_fd, new_dirs[i],
            ...
            if (!new_dirs[i])
            {
                /* FIXME have less lame handling for OOM, we just silently fail
                 * to watch.  (In reality though, the whole OOM handling in dbus
                 * is stupid but we won't go into that in this comment =) )
                 */
