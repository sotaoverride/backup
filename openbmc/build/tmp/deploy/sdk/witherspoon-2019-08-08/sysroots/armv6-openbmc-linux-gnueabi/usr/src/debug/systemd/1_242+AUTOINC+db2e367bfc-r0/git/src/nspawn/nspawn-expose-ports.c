...
int expose_port_flush(ExposePort* l, union in_addr_union* exposed)
{
    ...
    LIST_FOREACH(ports, p, l)
    {
        r = fw_add_local_dnat(false, af, p->protocol, NULL, NULL, 0, NULL, 0,
                              p->host_port, exposed, p->container_port, NULL);
        if (r < 0)
...
int expose_port_execute(sd_netlink* rtnl, ExposePort* l,
                        union in_addr_union* exposed)
{
    ...
    LIST_FOREACH(ports, p, l)
    {
        ...
        r = fw_add_local_dnat(true, af, p->protocol, NULL, NULL, 0, NULL, 0,
                              p->host_port, &new_exposed, p->container_port,
...
int expose_port_watch_rtnl(sd_event* event, int recv_fd,
                           sd_netlink_message_handler_t handler,
                           union in_addr_union* exposed, sd_netlink** ret)
{
    ...
    if (r < 0)
        return log_error_errno(
            r, "Failed to subscribe to RTM_NEWADDR messages: %m");

    r = sd_netlink_add_match(rtnl, NULL, RTM_DELADDR, handler, NULL, exposed,
                             "nspawn-DELADDR");
    if (r < 0)
        return log_error_errno(
            r, "Failed to subscribe to RTM_DELADDR messages: %m");
