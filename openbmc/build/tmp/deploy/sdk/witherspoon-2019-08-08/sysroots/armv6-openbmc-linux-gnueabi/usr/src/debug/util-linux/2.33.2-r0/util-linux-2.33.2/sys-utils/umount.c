...
static void __attribute__((__noreturn__)) usage(void)
{
    ...
    fputs(_(" -A, --all-targets       unmount all mountpoints for the given "
            "device in the\n"
            "                           current namespace\n"),
          out);
    fputs(_(" -c, --no-canonicalize   don't canonicalize paths\n"), out);
    fputs(_(" -d, --detach-loop       if mounted loop device, also free this "
            "loop device\n"),
          out);
    fputs(_("     --fake              dry run; skip the umount(2) syscall\n"),
          out);
    fputs(_(" -f, --force             force unmount (in case of an unreachable "
            "NFS system)\n"),
          out);
    fputs(_(" -i, --internal-only     don't call the umount.<type> helpers\n"),
          out);
    fputs(_(" -n, --no-mtab           don't write to /etc/mtab\n"), out);
    fputs(_(" -l, --lazy              detach the filesystem now, clean up "
            "things later\n"),
          out);
    fputs(_(" -O, --test-opts <list>  limit the set of filesystems (use with "
            "-a)\n"),
          out);
    fputs(_(" -R, --recursive         recursively unmount a target with all "
            "its children\n"),
          out);
    fputs(_(" -r, --read-only         in case unmounting fails, try to remount "
            "read-only\n"),
          out);
...
static int mk_exit_code(struct libmnt_context* cxt, int rc)
{
    ...
    if (quiet && rc == MNT_EX_FAIL && mnt_context_syscall_called(cxt) &&
...
static int umount_all(struct libmnt_context* cxt)
{
    ...
    while (mnt_context_next_umount(cxt, itr, &fs, &mntrc, &ignored) == 0)
    {
        ...
        else
        {
            ...
            if (xrc == MNT_EX_SUCCESS && mnt_context_is_verbose(cxt))
                             ...
                             struct libmnt_table* tb, struct libmnt_fs* fs)
{
    ...
    for (;;)
    {
        ...
        if (rc < 0)
        {
            warnx(_("failed to get child fs of %s"), mnt_fs_get_target(fs));
            rc = MNT_EX_SOFTWARE;
            goto done;
        }
        else if (rc == 1)
...
static int umount_recursive(struct libmnt_context* cxt, const char* spec)
{
    ...
    else
    {
        rc = MNT_EX_USAGE;
        if (!quiet)
            warnx(access(spec, F_OK) == 0 ? _("%s: not mounted")
                                          : _("%s: not found"),
                  spec);
    }
...
static int umount_alltargets(struct libmnt_context* cxt, const char* spec,
                             int rec)
{
    struct libmnt_fs* fs;
    struct libmnt_table* tb;
...
int main(int argc, char** argv)
{
    ...
    while ((c = getopt_long(argc, argv, "aAcdfhilnqRrO:t:vVN:", longopts,
                            NULL)) != -1)
    {
    ...
    else
    {
        while (argc--)
        {
            ...
            if (mnt_context_is_restricted(cxt) && !mnt_tag_is_valid(path))
