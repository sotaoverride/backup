...
const char* json_tokener_error_desc(enum json_tokener_error jerr)
{
    ...
    if (jerr_int < 0 || jerr_int >= (int)(sizeof(json_tokener_errors) /
                                          sizeof(json_tokener_errors[0])))
        return "Unknown error, "
               "invalid json_tokener_error value passed to "
...
struct json_tokener* json_tokener_new_ex(int depth)
{
    ...
    if (!tok)
        return NULL;
    tok->stack = (struct json_tokener_srec*)calloc(
                                          ...
                                          const char* str, int len)
{
    ...
    redo_char:
        switch (state)
        {
            ...
            case json_tokener_state_inf: /* aka starts with 'i' (or 'I', or
                                            "-i", or "-I") */
            {
                /* If we were guaranteed to have len set, then we could
                 * (usually) handle the entire "Infinity" check in a single
                 * strncmp (strncasecmp), but since len might be -1 (i.e. "read
                 * until \0"), we need to check it a character at a time. Trying
                 * to handle it both ways would make this code considerably more
                ...
                /* Note: tok->st_pos must be 0 when state is set to
                ...
                /* We checked the full length of "Infinity", so create the
                 * object. When handling -Infinity, the number parsing code will
                 * have dropped the "-" into tok->pb for us, so check it now.
                 */
                ...
                current =
                    json_object_new_double(is_negative ? -INFINITY : INFINITY);
                if (current == NULL)
                    goto out;
                saved_state = json_tokener_state_finish;
                state = json_tokener_state_eatws;
                    ...
                    (strncmp(json_null_str, tok->pb->buf, size) == 0))
                {
                    if (tok->st_pos == json_null_str_len)
                    {
                        ...
                        goto redo_char;
                          ...
                          strncasecmp(json_nan_str, tok->pb->buf, size_nan) ==
                              0) ||
            ...
            case json_tokener_state_string:
            {
                ...
                while (1)
                {
                    if (c == tok->quote_char)
                    {
                        printbuf_memappend_fast(tok->pb, case_start,
                                                str - case_start);
                        current = json_object_new_string_len(tok->pb->buf,
                                                             tok->pb->bpos);
                        if (current == NULL)
                            goto out;
                        saved_state = json_tokener_state_finish;
                    ...
                    (strncmp(json_true_str, tok->pb->buf, size1) == 0))
                {
                    if (tok->st_pos == json_true_str_len)
                    {
                        ...
                        goto redo_char;
                          ...
                          strncasecmp(json_false_str, tok->pb->buf, size2) ==
                              0) ||
