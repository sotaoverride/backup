...
int xread_tpm(void* buf, size_t size, struct rng* ent_src)
{
    ...
    while (bytes_read < size)
    {
        ...
        {
            retval = write(ent_src->rng_fd, rng_cmd + r, sizeof(rng_cmd) - r);
            if (retval < 0)
            {
                message(LOG_ERR | LOG_INFO, "Error writing %s\n",
                        ent_src->rng_name);
                retval = -1;
                goto error_out;
            }
...
int init_entropy_source(struct rng* ent_src)
{
    ...
    /* RHEL7: since /dev/hwrng will exist now even if there isn't an rng backing
     * it, check to see if rng_available is empty, and return error if it is.
     */
...
int init_tpm_entropy_source(struct rng* ent_src)
{
    message(LOG_CONS | LOG_INFO,
            "The TPM entropy source only supports TPM1.2 hardware and is "
            "deprecated.  TPM2.0 and later hardware exports entropy via "
            "/dev/hwrng, which "
