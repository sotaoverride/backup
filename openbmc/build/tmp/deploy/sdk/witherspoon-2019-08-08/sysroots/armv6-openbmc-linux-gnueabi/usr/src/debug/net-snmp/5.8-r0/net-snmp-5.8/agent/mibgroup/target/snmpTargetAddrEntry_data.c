...
struct targetAddrTable_struct* snmpTargetAddrTable_create(void)
{
    ...
    newEntry = (struct targetAddrTable_struct*)calloc(
        1, sizeof(struct targetAddrTable_struct));
                                   ...
                                   struct targetAddrTable_struct** listPtr)
{
    ...
    else
    {
        ...
        {
            i = netsnmp_compare_mem(newEntry->nameData, newEntry->nameLen,
                                    curr_struct->nameData,
                                    curr_struct->nameLen);
            if (i == 0)
            { /* Exact match, overwrite with new struct */
            ...
            { /* Found a greater OID, insert struct in front of it. */
...
void snmpTargetAddrTable_remove(struct targetAddrTable_struct* entry)
{
...
struct targetAddrTable_struct*
    search_snmpTargetAddrTable(oid* baseName, size_t baseNameLen, oid* name,
...
void shutdown_snmpTargetAddrEntry_data(void)
{
    ...
    DEBUGMSGTL(
        ("trap:targetAddr:shutdown", "clearing %d object(s)\n", _active));
                                     ...
                                     void* clientarg)
{
    ...
    while (curr_struct != NULL)
    {
             ...
             curr_struct->rowStatus == SNMP_ROW_NOTINSERVICE))
        {
            ...
            *cur++ = ' ';
            cur = read_config_save_octet_string(cur, curr_struct->tAddress,
                                                curr_struct->tAddressLen);
            cur += snprintf(cur, ep - cur, " %i %i \"%s\" %s %i %i",
                            curr_struct->timeout, curr_struct->retryCount,
                            curr_struct->tagListData, curr_struct->paramsData,
                            curr_struct->storageType, curr_struct->rowStatus);
...
static int snmpTargetAddr_addTDomain(struct targetAddrTable_struct* entry,
                                     char* cptr)
{
    size_t len = 128;
    ...
    {
        DEBUGMSGTL(("snmpTargetAddrEntry", "ERROR snmpTargetAddrEntry: tDomain "
                                           "unreadable in config string\n"));
        return (0);
    }
...
static int snmpTargetAddr_addTimeout(struct targetAddrTable_struct* entry,
                                     char* cptr)
{
    if (cptr == NULL)
    {
    ...
    {
        DEBUGMSGTL(
            ("snmpTargetAddrEntry", "ERROR snmpTargeParamsEntry: Timeout is "
                                    "not a digit in config string\n"));
        return (0);
    }
    /*
     * check Timeout >= 0
    ...
    {
        DEBUGMSGTL(
            ("snmpTargetAddrEntry", "ERROR snmpTargeParamsEntry: Timeout out "
...
static int snmpTargetAddr_addRetryCount(struct targetAddrTable_struct* entry,
                                        char* cptr)
{
    if (cptr == NULL)
    {
    ...
    {
        DEBUGMSGTL(
            ("snmpTargetAddrEntry", "ERROR snmpTargeParamsEntry: Retry Count "
                                    "is not a digit in config string\n"));
        return (0);
    }
    /*
     * spec check 0..255
    ...
    else
    {
        ...
        if ((entry->retryCount < 0) || (entry->retryCount > 255))
        {
            DEBUGMSGTL(("snmpTargetAddrEntry",
                        "ERROR snmpTargeParamsEntry: Retry Count is out of "
                        "range in config string\n"));
            return (0);
        }
    }
    return (1);
...
static int snmpTargetAddr_addTagList(struct targetAddrTable_struct* entry,
                                     char* cptr)
{
    if (cptr == NULL)
    {
    ...
    else
    {
        ...
        {
            DEBUGMSGTL(
                ("snmpTargetAddrEntry", "ERROR snmpTargetAddrEntry: tag list "
                                        "out of range in config string\n"));
            return (0);
        }
...
static int snmpTargetAddr_addParams(struct targetAddrTable_struct* entry,
                                    char* cptr)
{
    size_t len;
    if (cptr == NULL)
    ...
    else
    {
        ...
        {
            DEBUGMSGTL(
                ("snmpTargetAddrEntry", "ERROR snmpTargetAddrEntry: params out "
                                        "of range in config string\n"));
            return (0);
        }
...
static int snmpTargetAddr_addStorageType(struct targetAddrTable_struct* entry,
                                         char* cptr)
{
    if (cptr == NULL)
    {
...
static int snmpTargetAddr_addRowStatus(struct targetAddrTable_struct* entry,
                                       char* cptr)
{
    if (cptr == NULL)
    {
...
static void snmpd_parse_config_targetAddr(const char* token, char* char_ptr)
{
    ...
    cptr = read_config_read_octet_string_const(
        cptr, (u_char**)&newEntry->nameData, &bufl);
    if (bufl < 1 || bufl > 32)
    {
        DEBUGMSGTL(("snmpTargetAddrEntry",
    ...
    cptr = read_config_read_octet_string_const(
        cptr, (u_char**)&newEntry->tAddress, &newEntry->tAddressLen);
    ...
    *bptr++ = '\n';
    ...
    {
        bptr += snprintf(bptr, buff + sizeof(buff) - bptr, ".%d",
                         (int)newEntry->tDomain[i]);
    }
    bptr +=
        snprintf(bptr, buff + sizeof(buff) - bptr, " %s %d %d %s %s %d %d\n",
                 newEntry->tAddress, newEntry->timeout, newEntry->retryCount,
                 newEntry->tagListData, newEntry->paramsData,
