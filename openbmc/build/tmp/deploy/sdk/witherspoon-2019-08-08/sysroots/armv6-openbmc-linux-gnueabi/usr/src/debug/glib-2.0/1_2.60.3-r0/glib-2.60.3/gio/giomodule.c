...
static gboolean _g_io_module_scope_contains(GIOModuleScope* scope,
...
static gboolean load_symbols(GIOModule* module)
{
    gchar* name;
    gchar* load_symname;
    ...
    ret = g_module_symbol(module->library, load_symname,
                          (gpointer)&module->load) &&
          g_module_symbol(module->library, unload_symname,
                          (gpointer)&module->unload);

    if (!ret)
    {
        /* Fallback to old names */
        ret = g_module_symbol(module->library, "g_io_module_load",
                              (gpointer)&module->load) &&
              g_module_symbol(module->library, "g_io_module_unload",
                              (gpointer)&module->unload);
    }
 ...
 * Since: 2.30
 **/
void g_io_modules_scan_all_in_directory_with_scope(const char* dirname,
                                                   GIOModuleScope* scope)
{
    const gchar* name;
    char* filename;
    ...
    while ((name = g_dir_read_name(dir)))
    {
        if (is_valid_module_name(name, scope))
        {
            ...
            if (extension_points != NULL && g_stat(path, &statbuf) == 0 &&
                statbuf.st_ctime <= cache_mtime)
            {
                ...
                for (i = 0; extension_points[i] != NULL; i++)
                {
                    ...
                    extension_point->lazy_load_modules = g_list_prepend(
                        extension_point->lazy_load_modules, module);
                }
            }
            else
            {
 ...
 *
 * Returns: (element-type GIOModule) (transfer full): a list of #GIOModules
 *loaded from the directory, All the modules are loaded into memory, if you want
 *to unload them (enabling on-demand loading) you must call
 *      g_type_module_unuse() on all the modules. Free the list
 *      with g_list_free().
 *
 * Since: 2.30
 **/
GList* g_io_modules_load_all_in_directory_with_scope(const char* dirname,
                                                     GIOModuleScope* scope)
{
    const gchar* name;
    GDir* dir;
...
static gpointer try_class(GIOExtension* extension, guint is_supported_offset)
{
    ...
    if (!is_supported_offset ||
        (*G_STRUCT_MEMBER(verify_func, class, is_supported_offset))())
        return class;
...
static void print_help(const char* envvar, GIOExtensionPoint* ep)
{
    ...
    else
    {
        ...
        for (l = g_io_extension_point_get_extensions(ep); l; l = l->next)
        {
            ...
            g_print(" %*s - %d\n", width, g_io_extension_get_name(extension),
                                    ...
                                    guint is_supported_offset)
{
    ...
    if (default_modules)
    {
        ...
        if (g_hash_table_lookup_extended(default_modules, extension_point, &key,
                                         &impl))
        {
    ...
    if (use_this)
    {
        ...
        if (preferred)
        {
            ...
            g_warning("Can't find module '%s' specified in %s", use_this,
                      envvar);
    }
    else
        preferred = NULL;
...
static gpointer try_implementation(const char* extension_point,
                                   GIOExtension* extension,
                                   GIOModuleVerifyFunc verify_func)
{
    ...
    if (g_type_is_a(type, G_TYPE_INITABLE))
    {
        ...
        g_debug("Failed to initialize %s (%s) for %s: %s",
                g_io_extension_get_name(extension), g_type_name(type),
                extension_point, error ? error->message : "");
        g_clear_error(&error);
        return NULL;
    }
    else
                                  ...
                                  GIOModuleVerifyFunc verify_func)
{
    ...
    if (!ep)
    {
        g_debug("%s: Failed to find extension point ‘%s’", G_STRFUNC,
                extension_point);
        g_warn_if_reached();
        g_rec_mutex_unlock(&default_modules_lock);
        return NULL;
    ...
    if (impl != NULL)
    {
        ...
        g_debug("%s: Found default implementation %s (%s) for ‘%s’", G_STRFUNC,
                g_io_extension_get_name(extension), G_OBJECT_TYPE_NAME(impl),
                extension_point);
    }
    else
        g_debug("%s: Failed to find default implementation for ‘%s’", G_STRFUNC,
                extension_point);
...
void* _g_io_win32_get_module(void)
{
    if (!gio_dll)
        GetModuleHandleExA(GET_MODULE_HANDLE_EX_FLAG_FROM_ADDRESS,
                           (const char*)_g_io_win32_get_module, &gio_dll);
    return gio_dll;
}
...
static gchar* get_gio_module_dir(void)
{
    gchar* module_dir;
    ...
    if (module_dir == NULL)
    {
         ...
         */
        module_dir =
            g_build_filename(install_dir, "bin", "gio", "modules", NULL);
#else
        module_dir =
            g_build_filename(install_dir, "lib", "gio", "modules", NULL);
#endif
        g_free(install_dir);
#else
...
static void lazy_load_modules(GIOExtensionPoint* extension_point)
{
    GIOModule* module;
    GList* l;
                                             ...
                                             gint priority)
{
    ...
    {
        g_warning("Tried to implement non-registered extension point %s",
                  extension_point_name);
        return NULL;
    }
    ...
    {
        g_warning("Tried to register an extension of the type %s to extension "
                  "point %s. "
                  "Expected type is %s.",
                  g_type_name(type), extension_point_name,
    ...
    extension_point->extensions = g_list_insert_sorted(
        extension_point->extensions, extension, extension_prio_compare);
