...
static struct nla_policy tc_policy[TCA_MAX + 1] = {
    [TCA_KIND] = {.type = NLA_STRING, .maxlen = TCKINDSIZ},
              ...
              struct nla_policy* policy)
{
    ...
    if (g->ce_mask & TCA_ATTR_OPTS)
        return nla_parse(tb, maxattr, (struct nlattr*)g->tc_opts->d_data,
...
int rtnl_tc_clone(struct nl_object* dstobj, struct nl_object* srcobj)
{
    ...
    dst->tc_xstats = NULL;
    dst->tc_subdata = NULL;
    dst->ce_mask &= ~(TCA_ATTR_OPTS | TCA_ATTR_XSTATS);
...
void rtnl_tc_dump_line(struct nl_object* obj, struct nl_dump_params* p)
{
    ...
    {
        nl_dump(p, "dev %s ",
                rtnl_link_i2name(link_cache, tc->tc_ifindex, buf, sizeof(buf)));
    }
    else
        nl_dump(p, "dev %u ", tc->tc_ifindex);

    nl_dump(p, "id %s ", rtnl_tc_handle2str(tc->tc_handle, buf, sizeof(buf)));
...
void rtnl_tc_dump_stats(struct nl_object* obj, struct nl_dump_params* p)
{
    ...
    nl_dump_line(p, "  stats: %-14s %-10s   %-10s %-10s %-10s %-10s\n", "bytes",
                 "packets", "drops", "overlimits", "qlen", "backlog");

    res = nl_cancel_down_bytes(tc->tc_stats[RTNL_TC_BYTES], &unit);

    nl_dump_line(p, "       %10.2f %3s   %10u   %-10u %-10u %-10u %-10u\n", res,
                 unit, tc->tc_stats[RTNL_TC_PACKETS],
                 tc->tc_stats[RTNL_TC_DROPS], tc->tc_stats[RTNL_TC_OVERLIMITS],
                 tc->tc_stats[RTNL_TC_QLEN], tc->tc_stats[RTNL_TC_BACKLOG]);
...
struct rtnl_tc_ops* rtnl_tc_lookup_ops(enum rtnl_tc_type type, const char* kind)
{
    ...
    nl_list_for_each_entry(ops, &tc_ops_list[type],
                           to_list) if (!strcmp(kind, ops->to_kind)) return ops;
...
void* rtnl_tc_data_check(struct rtnl_tc* tc, struct rtnl_tc_ops* ops, int* err)
{
    ...
    if (tc->tc_ops != ops)
    {
        ...
        snprintf(buf, sizeof(buf),
                 "tc object %p used in %s context but is of type %s", tc,
                 ops->to_kind, tc->tc_ops->to_kind);
        APPBUG(buf);
