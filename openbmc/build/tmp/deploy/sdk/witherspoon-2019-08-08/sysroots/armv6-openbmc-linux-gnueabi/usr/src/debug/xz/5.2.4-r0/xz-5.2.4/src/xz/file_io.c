...
extern void io_init(void)
{
    ...
    if (pipe(user_abort_pipe))
        message_fatal(_("Error creating a pipe: %s"), strerror(errno));
    ...
    for (unsigned i = 0; i < 2; ++i)
    {
        ...
        if (flags == -1 ||
            fcntl(user_abort_pipe[i], F_SETFL, flags | O_NONBLOCK) == -1)
            message_fatal(_("Error creating a pipe: %s"), strerror(errno));
    }
#endif
...
static void io_sandbox_enter(int src_fd)
{
    if (!sandbox_allowed)
    {
    ...
    if (cap_rights_limit(src_fd,
                         cap_rights_init(&rights, CAP_EVENT, CAP_FCNTL,
                                         CAP_LOOKUP, CAP_READ, CAP_SEEK)))
        goto error;

    if (cap_rights_limit(STDOUT_FILENO,
                         cap_rights_init(&rights, CAP_EVENT, CAP_FCNTL,
                                         CAP_FSTAT, CAP_LOOKUP, CAP_WRITE,
                                         CAP_SEEK)))
        goto error;

    if (cap_rights_limit(user_abort_pipe[0],
                         cap_rights_init(&rights, CAP_EVENT)))
        goto error;

    if (cap_rights_limit(user_abort_pipe[1],
    ...
    while (true)
    {
        ...
        if (ret == -1)
        {
            ...
            message_error(_("%s: poll() failed: %s"),
                          is_reading ? pair->src_name : pair->dest_name,
                          strerror(errno));
            return IO_WAIT_ERROR;
        }
...
static void io_unlink(const char* name, const struct stat* known_st)
{
    ...
    // complex fix.
    const int stat_ret = opt_force ? stat(name, &new_st) : lstat(name, &new_st);

    if (stat_ret
#ifdef __VMS
        // st_ino is an array, and we don't want to
        // compare st_dev at all.
        || memcmp(&new_st.st_ino, &known_st->st_ino, sizeof(new_st.st_ino)) != 0
#else
    ...
    if (unlink(name))
        message_error(_("%s: Cannot remove: %s"), name, strerror(errno));
...
static void io_copy_attrs(const file_pair* pair)
{
    ...
    if (fchown(pair->dest_fd, pair->src_st.st_uid, -1) && warn_fchown)
        message_warning(_("%s: Cannot set the file owner: %s"), pair->dest_name,
                        strerror(errno));
    ...
    {
        message_warning(_("%s: Cannot set the file group: %s"), pair->dest_name,
                        strerror(errno));
...
static bool io_open_src_real(file_pair* pair)
{
    ...
    if (pair->src_name == stdin_filename)
    {
        ...
        (void)posix_fadvise(STDIN_FILENO, 0, 0,
                            opt_mode == MODE_LIST ? POSIX_FADV_RANDOM
    ...
    if (pair->src_fd == -1)
    {
        ...
        if (errno == ELOOP && !follow_symlinks)
        {
            ...
            struct stat st;
            if (lstat(pair->src_name, &st) == 0 && S_ISLNK(st.st_mode))
                was_symlink = true;
    ...
    {
        message_warning(_("%s: Is a directory, skipping"), pair->src_name);
        goto error;
    }

    if (reg_files_only && !S_ISREG(pair->src_st.st_mode))
    {
        message_warning(_("%s: Not a regular file, skipping"), pair->src_name);
        goto error;
    }
...
static bool io_open_dest_real(file_pair* pair)
{
    if (opt_stdout || pair->src_fd == STDIN_FILENO)
    {
        ...
        if ((stdout_flags & O_NONBLOCK) == 0 &&
            fcntl(STDOUT_FILENO, F_SETFL, stdout_flags | O_NONBLOCK) != -1)
    ...
    else
    {
        ...
        {
            message_error(_("%s: Cannot remove: %s"), pair->dest_name,
                          strerror(errno));
            free(pair->dest_name);
            return true;
        }
...
static bool io_close_dest(file_pair* pair, bool success)
{
    ...
    {
        message_error(_("%s: Closing the file failed: %s"), pair->dest_name,
                      strerror(errno));
...
extern size_t io_read(file_pair* pair, io_buf* buf_union, size_t size)
{
    ...
    while (left > 0)
    {
        ...
        if (amount == -1)
        {
            ...
            {
                const io_wait_ret ret =
                    io_wait(pair, mytime_get_flush_timeout(), true);
                switch (ret)
                {
                    case IO_WAIT_MORE:
                        continue;
            ...
            message_error(_("%s: Read error: %s"), pair->src_name,
                          strerror(errno));
...
extern bool io_pread(file_pair* pair, io_buf* buf, size_t size, off_t pos)
{
    ...
    {
        message_error(_("%s: Error seeking the file: %s"), pair->src_name,
                      strerror(errno));
        return true;
    }
    ...
    if (amount != size)
    {
        message_error(_("%s: Unexpected end of file"), pair->src_name);
        return true;
    }
...
static bool io_write_buf(file_pair* pair, const uint8_t* buf, size_t size)
{
    ...
    while (size > 0)
    {
        ...
        if (amount == -1)
        {
            ...
            if (errno != EPIPE)
                message_error(_("%s: Write error: %s"), pair->dest_name,
                              strerror(errno));
...
extern bool io_write(file_pair* pair, const io_buf* buf, size_t size)
{
    ...
    if (pair->dest_try_sparse)
    {
        ...
        {
            if (lseek(pair->dest_fd, pair->dest_pending_sparse, SEEK_CUR) == -1)
            {
