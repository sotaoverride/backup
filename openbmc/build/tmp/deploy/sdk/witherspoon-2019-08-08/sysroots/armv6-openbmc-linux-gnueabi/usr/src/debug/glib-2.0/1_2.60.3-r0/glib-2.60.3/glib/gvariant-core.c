...
static void g_variant_ensure_size(GVariant* value)
{
    ...
    if (value->size == (gssize)-1)
    {
        ...
        value->size = g_variant_serialiser_needed_size(
            value->type_info, g_variant_fill_gvs, children, n_children);
    }
}
...
static void g_variant_serialise(GVariant* value, gpointer data)
{
    ...
    g_variant_serialiser_serialise(serialised, g_variant_fill_gvs, children,
                                   n_children);
}
 ...
 */
static GVariant* g_variant_alloc(const GVariantType* type, gboolean serialised,
                                 gboolean trusted)
{
    GVariant* value;
    ...
    value->state = (serialised ? STATE_SERIALISED : 0) |
                   (trusted ? STATE_TRUSTED : 0) | STATE_FLOATING;
    value->size = (gssize)-1;
 ...
 * Since: 2.36
 */
GVariant* g_variant_new_from_bytes(const GVariantType* type, GBytes* bytes,
                                   gboolean trusted)
{
    GVariant* value;
    guint alignment;
     ...
     * with the wrong alignment (which is likely to happen very occasionally,
     * and only cause an abort on some architectures — so is unlikely to be
     * caught in testing). Callers can always actively ensure they use the
     * correct alignment to avoid the performance hit. */
    serialised.type_info = value->type_info;
    ...
    if (!g_variant_serialised_check(serialised))
    {
        ...
        bytes = owned_bytes = g_bytes_new_with_free_func(
            aligned_data, aligned_size, free, aligned_data);
        aligned_data = NULL;
#else
         ...
         */
        bytes = owned_bytes =
            g_bytes_new(g_bytes_get_data(bytes, NULL), g_bytes_get_size(bytes));
#endif
    }
...
GVariant* g_variant_new_from_children(const GVariantType* type,
                                      GVariant** children, gsize n_children,
                                      gboolean trusted)
{
    GVariant* value;
...
void g_variant_unref(GVariant* value)
{
    ...
    {
        if
            G_UNLIKELY(value->state & STATE_LOCKED)
...
GBytes* g_variant_get_data_as_bytes(GVariant* value)
{
    ...
    bytes_data =
        g_bytes_get_data(value->contents.serialised.bytes, &bytes_size);
...
GVariant* g_variant_get_child_value(GVariant* value, gsize index_)
{
...

    {
         ...
         * child. The only situation we expect this to happen in is with a
         * variant, as all other deeply-nested types have a static type, and
         * hence should have been rejected earlier. In the case of a variant
         * whose nesting plus the depth of its child is too great, return a unit
         * variant () instead of the real child. */
...
gboolean g_variant_is_normal_form(GVariant* value)
{
    ...
    {
        GVariantSerialised serialised = {
            value->type_info, (gpointer)value->contents.serialised.data,
            value->size, value->depth};
    ...
    else
    {
        ...
        for (i = 0; i < value->contents.tree.n_children; i++)
            normal &=
                g_variant_is_normal_form(value->contents.tree.children[i]);
