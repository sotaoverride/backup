...
static void do_calc_lpt_geom(struct ubifs_info* c)
{
    ...
    bits =
        UBIFS_LPT_CRC_BITS + UBIFS_LPT_TYPE_BITS + c->lnum_bits * c->lsave_cnt;
    c->lsave_sz = (bits + 7) / 8;
                       ...
                       struct ubifs_pnode* pnode)
{
    ...
    {
        pack_bits(&addr, &pos, pnode->lprops[i].free >> 3, c->space_bits);
        pack_bits(&addr, &pos, pnode->lprops[i].dirty >> 3, c->space_bits);
    ...
    crc =
        crc16(-1, buf + UBIFS_LPT_CRC_BYTES, c->pnode_sz - UBIFS_LPT_CRC_BYTES);
    addr = buf;
    pos = 0;
                       ...
                       struct ubifs_nnode* nnode)
{
    ...
    for (i = 0; i < UBIFS_LPT_FANOUT; i++)
    {
        ...
        pack_bits(&addr, &pos, nnode->nbranch[i].offs, c->lpt_offs_bits);
    }
    crc =
        crc16(-1, buf + UBIFS_LPT_CRC_BYTES, c->nnode_sz - UBIFS_LPT_CRC_BYTES);
    addr = buf;
    pos = 0;
    pack_bits(&addr, &pos, crc, UBIFS_LPT_CRC_BITS);
}
                      ...
                      struct ubifs_lpt_lprops* ltab)
{
    ...
    crc =
        crc16(-1, buf + UBIFS_LPT_CRC_BYTES, c->ltab_sz - UBIFS_LPT_CRC_BYTES);
    addr = buf;
    pos = 0;
    pack_bits(&addr, &pos, crc, UBIFS_LPT_CRC_BITS);
}
...
static void pack_lsave(struct ubifs_info* c, void* buf, int* lsave)
{
    ...
    crc =
        crc16(-1, buf + UBIFS_LPT_CRC_BYTES, c->lsave_sz - UBIFS_LPT_CRC_BYTES);
    addr = buf;
    pos = 0;
    pack_bits(&addr, &pos, crc, UBIFS_LPT_CRC_BITS);
}
...
{
    dbg_msg(3, "LEB %d free %d dirty %d to %d %d", lnum,
            c->ltab[lnum - c->lpt_first].free,
...
int create_lpt(struct ubifs_info* c)
{
    ...
    if (c->big_lpt)
        dbg_msg(1, "LPT lsave is at %d:%d", c->lsave_lnum, c->lsave_offs);
out:
    free(lsave);
    free(buf);
    free(nnode);
