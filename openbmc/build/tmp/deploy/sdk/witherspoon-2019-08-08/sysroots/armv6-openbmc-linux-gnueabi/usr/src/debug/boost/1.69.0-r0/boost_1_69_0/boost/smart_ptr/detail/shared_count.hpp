...
class shared_count
{
  ...
  public:
    ...
    BOOST_CONSTEXPR explicit shared_count(sp_counted_base* pi) :
        pi_(pi) // nothrow
#if defined(BOOST_SP_ENABLE_DEBUG_HOOKS)
        ,
        id_(shared_count_id)
...
#endif
    {
        ...
        typedef
            typename std::allocator_traits<A>::template rebind_alloc<impl_type>
                A2;
...
#endif
    {
        ...
        D2 d2(r.get_deleter());
        pi_ =
            new sp_counted_impl_pd<typename std::unique_ptr<Y, D>::pointer, D2>(
                r.get(), d2);
...
#endif
    {
        ...
        pi_ = new sp_counted_impl_pd<
            typename boost::movelib::unique_ptr<Y, D>::pointer, D2>(r.get(),
                                                                    d2);
    ...
    explicit shared_count(
        weak_count const& r); // throws bad_weak_ptr when r.use_count() == 0
    shared_count(
        weak_count const& r,
