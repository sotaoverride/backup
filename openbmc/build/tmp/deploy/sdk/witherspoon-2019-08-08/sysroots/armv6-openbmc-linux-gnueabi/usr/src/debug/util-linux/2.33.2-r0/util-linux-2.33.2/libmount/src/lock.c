...
struct libmnt_lock* mnt_new_lock(const char* datafile, pid_t id)
{
    ...
    DBG(LOCKS,
        ul_debugobj(ml, "alloc: default linkfile=%s, lockfile=%s", ln, lo));
    return ml;
err:
    free(lo);
    free(ln);
...
int mnt_lock_block_signals(struct libmnt_lock* ml, int enable)
{
    if (!ml)
        return -EINVAL;
    DBG(LOCKS,
        ul_debugobj(ml, "signals: %s", enable ? "BLOCKED" : "UNBLOCKED"));
...
static void unlock_simplelock(struct libmnt_lock* ml)
{
    ...
    {
        DBG(LOCKS,
            ul_debugobj(ml, "%s: unflocking", mnt_lock_get_lockfile(ml)));
 ...
 */
static int mnt_wait_mtab_lock(struct libmnt_lock* ml, struct flock* fl,
                              time_t maxtime)
{
    struct timeval now;
    ...
    DBG(LOCKS,
        ul_debugobj(
            ml, "(%d) waiting for F_SETLKW (now=%lu, maxtime=%lu, diff=%lu)",
            getpid(), (unsigned long)now.tv_sec, (unsigned long)maxtime,
 ...
 *
 * Now we wait for a few thousand microseconds between attempts and we have a
 * global time limit (30s) rather than a limit for the number of attempts. The
 * advantage is that this method also counts time which we spend in
 * fcntl(F_SETLKW) and the number of attempts is not restricted.
...
static int lock_mtab(struct libmnt_lock* ml)
{
    ...
    while (!ml->locked)
    {
        ...
        if (ml->locked)
        {
            ...
            {
                DBG(LOCKS,
                    ul_debugobj(ml, "%s: can't F_SETLK lockfile, errno=%d\n",
                                lockfile, errno));
    ...
    DBG(LOCKS,
        ul_debugobj(ml, "%s: (%d) successfully locked", lockfile, getpid()));
    unlink(linkfile);
    return 0;
 ...
 * uses flock() for private library file /run/mount/utab. The fcntl(2) is used
 * only for backwardly compatible stuff like /etc/mtab.
 *
...
static int test_lock(struct libmnt_test* ts, int argc, char* argv[])
{
    ...
    if (synctime)
    {
        ...
        {
            usecs =
                ((synctime - tv.tv_sec) * 1000000UL) - (1000000UL - tv.tv_usec);
            xusleep(usecs);
        }
    }
