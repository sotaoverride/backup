...
{
#ifdef WIN32SERVICE
    fprintf(stderr, "\nUsage:  snmptrapd [-register] [-quiet] [OPTIONS] "
                    "[LISTENING ADDRESSES]");
    ...
    fprintf(stderr, "  -M DIRLIST\t\tuse DIRLIST as the list of "
                    "locations\n\t\t\t  to look for MIBs\n");
    fprintf(stderr, "  -n\t\t\tuse numeric addresses instead of "
    ...
    fprintf(stderr, "  \t\t\t  Note that some parameters are not relevant when "
                     ...
                     void* transport_data, int transport_data_length)
{
    ...
    if (addr_string != NULL)
    {
        ...
        if (tcpudpaddr != 0)
        {
            ...
            if (hosts_ctl("snmptrapd", STRING_UNKNOWN, sbuf, STRING_UNKNOWN) ==
                0)
            {
    ...
    {
        if (hosts_ctl("snmptrapd", STRING_UNKNOWN, STRING_UNKNOWN,
                      STRING_UNKNOWN) == 0)
        {
...
void parse_config_doNotLogTraps(const char* token, char* cptr)
{
    if (atoi(cptr) > 0)
        SyslogTrap++;
}

void free_config_pidFile(void)
{
    if (pid_file)
        free(pid_file);
    pid_file = NULL;
...
void parse_config_pidFile(const char* token, char* cptr)
{
    free_config_pidFile();
...
#ifdef HAVE_UNISTD_H
void parse_config_agentuser(const char* token, char* cptr)
{
    if (cptr[0] == '#')
    {
        char* ecp;
...
void parse_config_agentgroup(const char* token, char* cptr)
{
    if (cptr[0] == '#')
    {
        char* ecp;
...
void parse_config_doNotFork(const char* token, char* cptr)
{
    if (netsnmp_ds_parse_boolean(cptr) == 1)
        dofork = 0;
}

void parse_config_ignoreAuthFailure(const char* token, char* cptr)
{
    if (netsnmp_ds_parse_boolean(cptr) == 1)
        dropauth = 1;
}

void parse_config_outputOption(const char* token, char* cptr)
{
    char* cp;
...
#endif
{
    ...
    register_config_handler("snmptrapd", "snmpTrapdAddr", parse_trapd_address,
                            free_trapd_address, "string");

    register_config_handler("snmptrapd", "doNotLogTraps",
                            parse_config_doNotLogTraps, NULL,
                            "(1|yes|true|0|no|false)");
#if HAVE_GETPID
    register_config_handler("snmptrapd", "pidFile", parse_config_pidFile, NULL,
                            "string");
#endif
#ifdef HAVE_UNISTD_H
    register_config_handler("snmptrapd", "agentuser", parse_config_agentuser,
                            NULL, "userid");
    register_config_handler("snmptrapd", "agentgroup", parse_config_agentgroup,
                            NULL, "groupid");
#endif

    register_config_handler("snmptrapd", "doNotFork", parse_config_doNotFork,
                            NULL, "(1|yes|true|0|no|false)");

    register_config_handler("snmptrapd", "ignoreAuthFailure",
                            parse_config_ignoreAuthFailure, NULL,
                            "(1|yes|true|0|no|false)");
    ...
    while ((arg = getopt(argc, argv, options)) != EOF)
    {
        switch (arg)
        {
            ...
            case 'S':
                fprintf(stderr, "Warning: -S option has been withdrawn; use "
                                "-Ls <facility> instead\n");
                goto out;
            ...
            case 'o':
                fprintf(stderr, "Warning: -o option has been withdrawn; use "
                                "-Lf <file> instead\n");
                goto out;
            ...
            case 'P':
                fprintf(stderr, "Warning: -P option has been withdrawn; use -f "
                                "-Le instead\n");
                goto out;
        ...
        if (agentx_subagent)
    {
        ...
        {
            netsnmp_ds_set_string(NETSNMP_DS_APPLICATION_ID,
                                  NETSNMP_DS_NOTIF_LOG_CTX, "snmptrapd");
    ...
    init_perl();
    {
        ...
        snprintf(init_file, sizeof(init_file) - 1, "%s/%s", SNMPSHAREPATH,
                 "snmp_perl_trapd.pl");
        netsnmp_ds_set_string(NETSNMP_DS_APPLICATION_ID,
                              NETSNMP_DS_AGENT_PERL_INIT_FILE, init_file);
    }
#endif
    ...
    if (dofork && netsnmp_running)
    {
        ...
        switch (fork())
        {
            ...
            case 0:
                ...
                /*
                 * if we are forked, we don't want to print out to stdout or
                 * stderr
                 */
...
int __cdecl _tmain(int argc, TCHAR* argv[])
{
    ...
    LPCTSTR lpszServiceDisplayName =
        _T("Net-SNMP Trap Handler"); /* Display Name */
    LPCTSTR lpszServiceDescription =
#ifdef IFDESCR
        _T("SNMPv2c / SNMPv3 trap/inform receiver from Net-SNMP. Supports MIB ")
        _T("objects for IP,ICMP,TCP,UDP, and network interface sub-layers.");
#else
    ...
    switch (nRunType)
    {
        case REGISTER_SERVICE:
            ...
            exit(RegisterService(lpszServiceName, lpszServiceDisplayName,
