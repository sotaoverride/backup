...
static void g_list_model_default_init(GListModelInterface* iface)
{
    /**
     ...
     * Since: 2.44
     */
    g_list_model_changed_signal =
        g_signal_new(I_("items-changed"), G_TYPE_LIST_MODEL, G_SIGNAL_RUN_LAST,
 ...
 * Since: 2.44
 */
void g_list_model_items_changed(GListModel* list, guint position, guint removed,
                                guint added)
{
    g_return_if_fail(G_IS_LIST_MODEL(list));

    g_signal_emit(list, g_list_model_changed_signal, 0, position, removed,
                  added);
}
