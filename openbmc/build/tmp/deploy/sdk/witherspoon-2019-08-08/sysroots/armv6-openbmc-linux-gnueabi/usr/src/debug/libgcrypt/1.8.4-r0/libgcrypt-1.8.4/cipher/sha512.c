...
static unsigned int transform(void* context, const unsigned char* data,
                              size_t nblks);
...
#ifdef USE_AVX
    ctx->use_avx =
        (features & HWF_INTEL_AVX) && (features & HWF_INTEL_FAST_SHLD);
#endif
#ifdef USE_AVX2
...
static inline u64 ROTR(u64 x, u64 n)
{
    return ((x >> n) | (x << (64 - n)));
}
...
static inline u64 Sum0(u64 x)
{
    return (ROTR(x, 28) ^ ROTR(x, 34) ^ ROTR(x, 39));
}

static inline u64 Sum1(u64 x)
{
 ...
 */
static unsigned int transform_blk(SHA512_STATE* hd, const unsigned char* data)
{
    u64 a, b, c, d, e, f, g, h;
    u64 w[16];
    int t;
...
void _gcry_sha512_transform_armv7_neon(SHA512_STATE* hd,
                                       const unsigned char* data, const u64 k[],
                                       size_t num_blks);
#endif
...
static void sha512_final(void* context)
{
...
static byte* sha512_read(void* context)
{
                               ...
                               int iovcnt)
{
    ...
    for (; iovcnt > 0; iov++, iovcnt--)
        _gcry_md_block_write(&hd, (const char*)iov[0].data + iov[0].off,
                             iov[0].len);
    sha512_final(&hd);
    memcpy(outbuf, hd.bctx.buf, 64);
}
...
static gpg_err_code_t selftests_sha384(int extended,
...
static gpg_err_code_t selftests_sha512(int extended,
                                       selftest_report_func_t report)
{
    const char* what;
    const char* errtxt;
...
gcry_md_spec_t _gcry_digest_spec_sha512 = {
    GCRY_MD_SHA512,         {0, 1},          "SHA512",    sha512_asn,
    DIM(sha512_asn),        oid_spec_sha512, 64,          sha512_init,
    _gcry_md_block_write,   sha512_final,    sha512_read, NULL,
...
gcry_md_spec_t _gcry_digest_spec_sha384 = {
    GCRY_MD_SHA384,         {0, 1},          "SHA384",    sha384_asn,
    DIM(sha384_asn),        oid_spec_sha384, 48,          sha384_init,
    _gcry_md_block_write,   sha512_final,    sha512_read, NULL,
    sizeof(SHA512_CONTEXT), run_selftests};
