...
#define G_DRIVE_GET_IFACE(obj)                                                 \
    (G_TYPE_INSTANCE_GET_INTERFACE((obj), G_TYPE_DRIVE, GDriveIface))

/**
 * GDriveIface:
 * @g_iface: The parent interface.
 * @changed: Signal emitted when the drive is changed.
 * @disconnected: The removed signal that is emitted when the #GDrive have been
 * disconnected. If the recipient is holding references to the object they
 * should release them so the object can be finalized.
 * @eject_button: Signal emitted when the physical eject button (if any) of a
 ...
 * @is_removable: Returns %TRUE if the #GDrive and/or its media is considered
 * removable by the user. Since 2.50.
 * @is_media_removable: Returns %TRUE if the #GDrive supports removal and
 * insertion of media.
 * @has_media: Returns %TRUE if the #GDrive has media inserted.
 * @is_media_check_automatic: Returns %TRUE if the #GDrive is capabable of
 * automatically detecting media changes.
 * @can_poll_for_media: Returns %TRUE if the #GDrive is capable of manually
 * polling for media change.
 ...
 * @get_start_stop_type: Gets a #GDriveStartStopType with details about
 * starting/stopping the drive. Since 2.22.
 ...
 * @can_start_degraded: Returns %TRUE if a #GDrive can be started degraded.
 * Since 2.22.
 * @start: Starts a #GDrive. Since 2.22.
 * @start_finish: Finishes a start operation. Since 2.22.
 * @stop_button: Signal emitted when the physical stop button (if any) of a
 * drive have been pressed. Since 2.22.
 * @eject_with_operation: Starts ejecting a #GDrive using a #GMountOperation.
 * Since 2.22.
 * @eject_with_operation_finish: Finishes an eject operation using a
 * #GMountOperation. Since 2.22.
 * @get_sort_key: Gets a key used for sorting #GDrive instances or %NULL if no
 * such key exists. Since 2.32.
 * @get_symbolic_icon: Returns a symbolic #GIcon for the given #GDrive.
 * Since 2.34.
 *
...
struct _GDriveIface
{
    ...
    void (*eject_with_operation)(GDrive* drive, GMountUnmountFlags flags,
                                 ...
                                 gpointer user_data);
    gboolean (*eject_with_operation_finish)(GDrive* drive, GAsyncResult* result,
                                            GError** error);
...
void g_drive_eject(GDrive* drive, GMountUnmountFlags flags,
                   GCancellable* cancellable, GAsyncReadyCallback callback,
                   gpointer user_data);

GLIB_DEPRECATED_FOR(g_drive_eject_with_operation_finish)
gboolean g_drive_eject_finish(GDrive* drive, GAsyncResult* result,
                              GError** error);
GLIB_AVAILABLE_IN_ALL
void g_drive_poll_for_media(GDrive* drive, GCancellable* cancellable,
                            GAsyncReadyCallback callback, gpointer user_data);
GLIB_AVAILABLE_IN_ALL
gboolean g_drive_poll_for_media_finish(GDrive* drive, GAsyncResult* result,
                                       GError** error);
GLIB_AVAILABLE_IN_ALL
...
GLIB_AVAILABLE_IN_ALL
void g_drive_start(GDrive* drive, GDriveStartFlags flags,
                   GMountOperation* mount_operation, GCancellable* cancellable,
                   GAsyncReadyCallback callback, gpointer user_data);
GLIB_AVAILABLE_IN_ALL
gboolean g_drive_start_finish(GDrive* drive, GAsyncResult* result,
                              GError** error);

GLIB_AVAILABLE_IN_ALL
gboolean g_drive_can_stop(GDrive* drive);
GLIB_AVAILABLE_IN_ALL
void g_drive_stop(GDrive* drive, GMountUnmountFlags flags,
                  GMountOperation* mount_operation, GCancellable* cancellable,
                  GAsyncReadyCallback callback, gpointer user_data);
GLIB_AVAILABLE_IN_ALL
gboolean g_drive_stop_finish(GDrive* drive, GAsyncResult* result,
                             GError** error);

GLIB_AVAILABLE_IN_ALL
void g_drive_eject_with_operation(GDrive* drive, GMountUnmountFlags flags,
                                  GMountOperation* mount_operation,
                                  GCancellable* cancellable,
                                  GAsyncReadyCallback callback,
                                  gpointer user_data);
