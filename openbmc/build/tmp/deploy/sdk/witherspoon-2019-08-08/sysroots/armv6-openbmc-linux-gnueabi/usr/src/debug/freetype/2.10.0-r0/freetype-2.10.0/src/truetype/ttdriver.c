 ...
 *
 */
static FT_Error tt_property_set(FT_Module module, /* TT_Driver */
...
static FT_Error tt_property_get(FT_Module module, /* TT_Driver */
                                const char* property_name, const void* value)
{
    FT_Error error = FT_Err_Ok;
    TT_Driver driver = (TT_Driver)module;
...
static FT_Error tt_get_advances(FT_Face ttface, FT_UInt start, FT_UInt count,
                                FT_Int32 flags, FT_Fixed* advances)
{
    FT_UInt nn;
    TT_Face face = (TT_Face)ttface;
...
static FT_Error tt_size_select(FT_Size size, FT_ULong strike_index)
{
    ...
    else
    {
        ...
        error = sfnt->load_strike_metrics(ttface, strike_index, size_metrics);
        if (error)
...
static FT_Error tt_size_request(FT_Size size, FT_Size_Request req)
{
    ...
    if (FT_IS_SCALABLE(size->face))
    {
        ...
        if (!error)
        {
            ...
            if (req->type == FT_SIZE_REQUEST_TYPE_SCALES || !resolution)
                resolution = 72;

            ttsize->point_size =
                FT_MulDiv(ttsize->ttmetrics.ppem, 64 * 72, resolution);
        }
#endif
    }
 ...
 *   FreeType error code.  0 means success.
 */
static FT_Error tt_glyph_load(FT_GlyphSlot ttslot, /* TT_GlyphSlot */
                              ...
                              FT_UInt glyph_index, FT_Int32 load_flags)
{
    ...
    size->metrics = (load_flags & FT_LOAD_NO_HINTING) ? &ttsize->metrics
                                                      : &size->hinted_metrics;
...
#ifdef TT_CONFIG_OPTION_GX_VAR_SUPPORT
FT_DEFINE_SERVICEDESCREC6(tt_services,
                          ...
                          &tt_service_truetype_engine, FT_SERVICE_ID_TT_GLYF,
                          &tt_service_truetype_glyf, FT_SERVICE_ID_PROPERTIES,
                          &tt_service_properties)
#else
...
FT_DEFINE_DRIVER(tt_driver_class,
                 ...
                 sizeof(TT_FaceRec), sizeof(TT_SizeRec),
