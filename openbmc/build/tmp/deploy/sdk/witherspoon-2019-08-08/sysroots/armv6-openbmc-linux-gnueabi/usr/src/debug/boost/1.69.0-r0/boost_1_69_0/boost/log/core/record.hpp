 ...
 *
 * The logging record encapsulates all information related to a single logging
 * statement, in particular, attribute values view and the log message string.
 * The record can be updated before pushing for further processing to the
 * logging core.
 */
class record
{
  ...
  private:
    ...
    BOOST_CONSTEXPR explicit record(public_data* impl) BOOST_NOEXCEPT
        : m_impl(impl)
    {
    }
  ...
  public:
    ...
    /*!
     * \return A reference to the set of attribute values attached to this
     * record
     *
     * \pre <tt>!!*this</tt>
     */
     ...
     *
     * \return \c true, if the <tt>*this</tt> identifies a log record, \c false,
     * if the <tt>*this</tt> is not valid
     */
     ...
     *
     * \return \c false, if the <tt>*this</tt> identifies a log record, \c true,
     * if the <tt>*this</tt> is not valid
     */
    ...
    /*!
     * Resets the log record handle. If there are no other handles left, the log
     * record is closed and all resources referenced by the record are released.
     *
     * \post <tt>!*this == true</tt>
     */
     ...
     * thread-specific data. Then the record contents are used to construct a \c
     * record_view which is returned from the function. The record is no longer
     * valid after the call.
     *
     * \pre <tt>!!*this</tt>
     * \post <tt>!*this</tt>
     * \returns The record view that contains all attribute values from the
     * original record.
     */
