...
inline bool is_combining(charT c)
{
    return (c <= static_cast<charT>(0))
               ? false
               : ((c >= static_cast<charT>(
                            (std::numeric_limits<uint_least16_t>::max)()))
                      ? false
                      : is_combining_implementation(
...
#if !defined(__hpux) &&                                                        \
    !defined(__WINSCW__) // can't use WCHAR_MAX/MIN in pp-directives
#ifdef _MSC_VER
template <>
...
struct character_pointer_range
{
    ...
    bool operator==(const character_pointer_range& r) const
    {
        ...
        // in the MS case.
        return ((p2 - p1) == (r.p2 - r.p1)) &&
               BOOST_REGEX_DETAIL_NS::equal(p1, p2, r.p1);
    }
};
template <class charT>
int get_default_class_id(const charT* p1, const charT* p2)
{
    static const charT data[73] = {
        'a', 'l', 'n', 'u', 'm', 'a', 'l', 'p', 'h', 'a', 'b', 'l', 'a',
        'n', 'k', 'c', 'n', 't', 'r', 'l', 'd', 'i', 'g', 'i', 't', 'g',
        'r', 'a', 'p', 'h', 'l', 'o', 'w', 'e', 'r', 'p', 'r', 'i', 'n',
        't', 'p', 'u', 'n', 'c', 't', 's', 'p', 'a', 'c', 'e', 'u', 'n',
        'i', 'c', 'o', 'd', 'e', 'u', 'p', 'p', 'e', 'r', 'v', 'w', 'o',
        'r', 'd', 'x', 'd', 'i', 'g', 'i', 't',
    };
    ...
    static const character_pointer_range<charT>* ranges_end =
    ...
    const character_pointer_range<charT>* p =
...
//
template <>
inline char BOOST_REGEX_CALL global_lower<char>(char c)
{
    return do_global_lower(c);
}
template <>
inline char BOOST_REGEX_CALL global_upper<char>(char c)
{
    return do_global_upper(c);
}
#ifndef BOOST_NO_WREGEX
template <>
inline wchar_t BOOST_REGEX_CALL global_lower<wchar_t>(wchar_t c)
{
    return do_global_lower(c);
}
template <>
inline wchar_t BOOST_REGEX_CALL global_upper<wchar_t>(wchar_t c)
{
    return do_global_upper(c);
}
#endif
#ifdef BOOST_REGEX_HAS_OTHER_WCHAR_T
template <>
inline unsigned short BOOST_REGEX_CALL
    global_lower<unsigned short>(unsigned short c)
{
    return do_global_lower(c);
}
template <>
inline unsigned short BOOST_REGEX_CALL
    global_upper<unsigned short>(unsigned short c)
{
    return do_global_upper(c);
}
#endif
...
boost::intmax_t global_toi(const charT*& p1, const charT* p2, int radix,
                           const traits& t)
{
    (void)t; // warning suppression
    boost::intmax_t limit =
        (std::numeric_limits<boost::intmax_t>::max)() / radix;
...
template <class charT>
inline typename boost::enable_if_c<(sizeof(charT) > 1), const charT*>::type
    get_escape_R_string()
{
#ifdef BOOST_MSVC
#pragma warning(push)
...
template <class charT>
inline typename boost::disable_if_c<(sizeof(charT) > 1), const charT*>::type
    get_escape_R_string()
{
#ifdef BOOST_MSVC
#pragma warning(push)
