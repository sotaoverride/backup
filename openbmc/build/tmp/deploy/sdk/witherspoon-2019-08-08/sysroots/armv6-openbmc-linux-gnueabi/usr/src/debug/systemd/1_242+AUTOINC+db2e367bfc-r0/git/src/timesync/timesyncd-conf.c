    ...
    for (;;)
    {
        ...
        if (r < 0)
            return log_error_errno(
                r, "Failed to parse timesyncd server syntax \"%s\": %m",
                string);
        if (r == 0)
            break;
        ...
        if (r < 0)
            return log_error_errno(r,
                                   "Failed to check validity of NTP server "
                                   "name or address '%s': %m",
                                   word);
        if (r == 0)
        {
...
int manager_parse_config_file(Manager* m)
{
    ...
    r = config_parse_many_nulstr(PKGSYSCONFDIR "/timesyncd.conf",
                                 ...
                                 "Time\0", config_item_perf_lookup,
                                 timesyncd_gperf_lookup, CONFIG_PARSE_WARN, m);
    if (r < 0)
        return r;
    ...
    {
        log_warning("PollIntervalMaxSec= is smaller than PollIntervalMinSec=. "
                    "Using default value.");
        m->poll_interval_max_usec =
            MAX(NTP_POLL_INTERVAL_MAX_USEC, m->poll_interval_min_usec * 32);
    }
