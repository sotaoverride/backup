...
#define REPORT_BUG                                                             \
    "please report occurrence circumstances to gtk-devel-list@gnome.org"
...
/* --- prototypes --- */
static inline guint signal_id_lookup(GQuark quark, GType itype);
...
static inline HandlerList* handler_list_lookup(guint signal_id,
                                               gpointer instance);
static inline Handler* handler_new(guint signal_id, gpointer instance,
                                   gboolean after);
static void handler_insert(guint signal_id, gpointer instance,
                           Handler* handler);
static Handler* handler_lookup(gpointer instance, gulong handler_id,
                               GClosure* closure, guint* signal_id_p);
...
static inline HandlerMatch* handler_match_free1_R(HandlerMatch* node,
                                                  gpointer instance);
static HandlerMatch* handlers_find(gpointer instance, GSignalMatchType mask,
                                   guint signal_id, GQuark detail,
                                   GClosure* closure, gpointer func,
                                   gpointer data, gboolean one_and_only);
static inline void handler_ref(Handler* handler);
static inline void handler_unref_R(guint signal_id, gpointer instance,
                                   Handler* handler);
static gint handler_lists_cmp(gconstpointer node1, gconstpointer node2);
static inline void emission_push(Emission* emission);
static inline void emission_pop(Emission* emission);
static inline Emission* emission_find(guint signal_id, GQuark detail,
                                      gpointer instance);
static gint class_closures_cmp(gconstpointer node1, gconstpointer node2);
static gint signal_key_cmp(gconstpointer node1, gconstpointer node2);
static gboolean signal_emit_unlocked_R(SignalNode* node, GQuark detail,
                                       gpointer instance, GValue* return_value,
                                       const GValue* instance_and_params);
static void add_invalid_closure_notify(Handler* handler, gpointer instance);
static void remove_invalid_closure_notify(Handler* handler, gpointer instance);
static void invalid_closure_notify(gpointer data, GClosure* closure);
...
#define SINGLE_VA_CLOSURE_EMPTY_MAGIC                                          \
    GINT_TO_POINTER(1) /* indicates single_va_closure is valid but empty */
...
static inline SignalNode* LOOKUP_SIGNAL_NODE(guint signal_id)
{
...
static inline guint signal_id_lookup(GQuark quark, GType itype)
{
    ...
    do
    {
        ...
        key.itype = type;
        signal_key = g_bsearch_array_lookup(g_signal_key_bsa,
                                            &g_signal_key_bconfig, &key);
                                               ...
                                               gpointer instance)
{
    ...
    return hlbsa ? g_bsearch_array_lookup(hlbsa, &g_signal_hlbsa_bconfig, &key)
                 : NULL;
}

static guint handler_hash(gconstpointer key)
{
    return (guint)((Handler*)key)->sequential_number;
}

static gboolean handler_equal(gconstpointer a, gconstpointer b)
{
                               ...
                               GClosure* closure, guint* signal_id_p)
{
    ...
    if (hlbsa)
    {
        ...
        {
            HandlerList* hlist =
                g_bsearch_array_get_nth(hlbsa, &g_signal_hlbsa_bconfig, i);
            Handler* handler;

            for (handler = hlist->handlers; handler; handler = handler->next)
                if (closure ? (handler->closure == closure)
                            : (handler->sequential_number == handler_id))
                {
                    if (signal_id_p)
...
static inline HandlerMatch*
    handler_match_prepend(HandlerMatch* list, Handler* handler, guint signal_id)
{
    HandlerMatch* node;
                                   ...
                                   gpointer data, gboolean one_and_only)
{
    ...
    if (mask & G_SIGNAL_MATCH_ID)
    {
        ...
        mask = ~mask;
        for (handler = hlist ? hlist->handlers : NULL; handler;
    ...
    {
        GBSearchArray* hlbsa =
            g_hash_table_lookup(g_handler_list_bsa_ht, instance);
            ...
            for (i = 0; i < hlbsa->n_nodes; i++)
            {
                ...
                for (handler = hlist->handlers; handler;
                    ...
                    if (handler->sequential_number &&
                        ...
                        ((mask & G_SIGNAL_MATCH_FUNC) ||
                         (handler->closure->marshal == node->c_marshaller &&
                          G_REAL_CLOSURE(handler->closure)->meta_marshal ==
                              NULL &&
                          ((GCClosure*)handler->closure)->callback == func)))
                    {
                        mlist = handler_match_prepend(mlist, handler,
                                                      hlist->signal_id);
                        if (one_and_only)
                            return mlist;
                    }
...
static void handler_insert(guint signal_id, gpointer instance, Handler* handler)
{
    HandlerList* hlist;
...
static void node_update_single_va_closure(SignalNode* node)
{
        ...
        (node->emission_hooks == NULL || node->emission_hooks->hooks == NULL))
    {
        ...
        else if (bsa->n_nodes == 1)
        {
            ...
            {
                run_type =
                    node->flags & (G_SIGNAL_RUN_FIRST | G_SIGNAL_RUN_LAST |
                                   G_SIGNAL_RUN_CLEANUP);
                /* Only support *one* of run-first or run-last, not multiple or
                 * cleanup */
...
static inline Emission* emission_find_innermost(gpointer instance)
{
    Emission* emission;
...
void _g_signal_init(void)
{
    ...
    if (!g_n_signal_nodes)
    {
        /* setup handler list binary searchable array hash table (in german,
         * that'd be one word ;) */
        ...
        if (node->itype == itype)
        {
            if (node->destroyed)
                g_warning(G_STRLOC
                          ...
                          node->name, type_debug_name(node->itype));
            else
 ...
 * @instance: (type GObject.Object): the object whose signal handlers you wish
 * to stop.
...
void g_signal_stop_emission(gpointer instance, guint signal_id, GQuark detail)
{
    ...
    {
        g_warning("%s: signal id '%u' does not support detail (%u)", G_STRLOC,
                  signal_id, detail);
        SIGNAL_UNLOCK();
        return;
    }
    if (node && g_type_is_a(G_TYPE_FROM_INSTANCE(instance), node->itype))
    {
        ...
        if (emission)
        {
            if (emission->state == EMISSION_HOOK)
                g_warning(G_STRLOC ": emission of signal \"%s\" for instance "
                                   "'%p' cannot be stopped from emission hook",
                          node->name, instance);
        ...
        g_warning("%s: signal id '%u' is invalid for instance '%p'", G_STRLOC,
                  signal_id, instance);
    SIGNAL_UNLOCK();
}

static void signal_finalize_hook(GHookList* hook_list, GHook* hook)
{
 ...
 */
gulong g_signal_add_emission_hook(guint signal_id, GQuark detail,
                                  ...
                                  GDestroyNotify data_destroy)
{
    ...
    {
        g_warning("%s: signal id '%u' does not support emission hooks "
...
void g_signal_remove_emission_hook(guint signal_id, gulong hook_id)
{
    ...
    else if (!node->emission_hooks ||
             !g_hook_destroy(node->emission_hooks, hook_id))
        g_warning("%s: signal \"%s\" had no hook (%lu) to remove", G_STRLOC,
                  node->name, hook_id);
...
static inline guint signal_parse_name(const gchar* name, GType itype,
                                      GQuark* detail_p, gboolean force_quark)
{
    ...
    else if (colon[1] == ':')
    {
        ...
        if (signal_id && detail_p)
            *detail_p = colon[2] ? (force_quark ? g_quark_from_string
                                                : g_quark_try_string)(colon + 2)
                                 : 0;
    }
    else
        signal_id = 0;
...
gboolean g_signal_parse_name(const gchar* detailed_signal, GType itype,
                             guint* signal_id_p, GQuark* detail_p,
                             gboolean force_detail_quark)
{
    ...
    SIGNAL_LOCK();
    signal_id =
        signal_parse_name(detailed_signal, itype, &detail, force_detail_quark);
    SIGNAL_UNLOCK();
                                    ...
                                    const gchar* detailed_signal)
{
    ...
    if (signal_id)
    {
        ...
        if (detail && !(node->flags & G_SIGNAL_DETAILED))
            g_warning("%s: signal '%s' does not support details", G_STRLOC,
                      detailed_signal);
        else if (!g_type_is_a(itype, node->itype))
            g_warning(
                "%s: signal '%s' is invalid for instance '%p' of type '%s'",
        ...
        else
        {
            ...
            if (emission)
            {
                if (emission->state == EMISSION_HOOK)
                    g_warning(G_STRLOC
                              ": emission of signal \"%s\" for instance '%p' "
                              "cannot be stopped from emission hook",
                              node->name, instance);
                else if (emission->state == EMISSION_RUN)
                    emission->state = EMISSION_STOP;
            }
...
guint g_signal_lookup(const gchar* name, GType itype)
{
    ...
    g_return_val_if_fail(
        G_TYPE_IS_INSTANTIATABLE(itype) || G_TYPE_IS_INTERFACE(itype), 0);
    ...
    if (!signal_id)
    {
        ...
        if (!g_type_name(itype))
            g_warning(G_STRLOC ": unable to lookup signal \"%s\" for invalid "
                               "type id '%" G_GSIZE_FORMAT "'",
                      name, itype);
        else if (!G_TYPE_IS_INSTANTIATABLE(itype))
            g_warning(G_STRLOC ": unable to lookup signal \"%s\" for non "
                               "instantiatable type '%s'",
 ...
 * g_signal_query().
 *
 * Returns: (array length=n_ids) (transfer full): Newly allocated array of
 * signal IDs.
 */
guint* g_signal_list_ids(GType itype, guint* n_ids)
{
    ...
    g_return_val_if_fail(
        G_TYPE_IS_INSTANTIATABLE(itype) || G_TYPE_IS_INTERFACE(itype), NULL);
 ...
 * in their class_init method by doing super_class->signal_handler =
 ...
 */
guint g_signal_new(const gchar* signal_name, GType itype,
                   GSignalFlags signal_flags, guint class_offset,
                   GSignalAccumulator accumulator, gpointer accu_data,
                   GSignalCMarshaller c_marshaller, GType return_type,
                   guint n_params, ...)
{
    va_list args;
    guint signal_id;
    ...
    signal_id = g_signal_new_valist(
        signal_name, itype, signal_flags,
...
guint g_signal_new_class_handler(
    const gchar* signal_name, GType itype, GSignalFlags signal_flags,
    GCallback class_handler, GSignalAccumulator accumulator, gpointer accu_data,
...
static inline ClassClosure* signal_find_class_closure(SignalNode* node,
                                                      GType itype)
{
    GBSearchArray* bsa = node->class_closure_bsa;
    ClassClosure* cc;
...
static inline GClosure* signal_lookup_closure(SignalNode* node,
...
static void signal_add_class_closure(SignalNode* node, GType itype,
                                     GClosure* closure)
{
    ClassClosure key;
    ...
    node->class_closure_bsa = g_bsearch_array_insert(
        node->class_closure_bsa, &g_class_closure_bconfig, &key);
 ...
 * Returns: the signal id
 */
guint g_signal_newv(const gchar* signal_name, GType itype,
                    ...
                    guint n_params, GType* param_types)
{
    ...
    g_strdelimit(name, G_STR_DELIMITERS ":^",
    ...
    if (node && !node->destroyed)
    {
        g_warning(G_STRLOC ": signal \"%s\" already exists in the '%s' %s",
                  name, type_debug_name(node->itype),
                  G_TYPE_IS_INTERFACE(node->itype) ? "interface"
                                                   : "class ancestry");
        g_free(name);
        SIGNAL_UNLOCK();
        return 0;
    ...
    {
        g_warning(G_STRLOC ": signal \"%s\" for type '%s' was previously "
                           "created for type '%s'",
                  name, type_debug_name(itype), type_debug_name(node->itype));
        g_free(name);
        SIGNAL_UNLOCK();
        return 0;
    }
    for (i = 0; i < n_params; i++)
        ...
        {
            g_warning(G_STRLOC ": parameter %d of type '%s' for signal "
                               "\"%s::%s\" is not a value type",
                      i + 1, type_debug_name(param_types[i]),
                      type_debug_name(itype), name);
            g_free(name);
            SIGNAL_UNLOCK();
            return 0;
        }
    if (return_type != G_TYPE_NONE &&
        !G_TYPE_IS_VALUE(return_type & ~G_SIGNAL_TYPE_STATIC_SCOPE))
    {
        g_warning(G_STRLOC ": return value of type '%s' for signal \"%s::%s\" "
                           "is not a value type",
    ...
    {
        g_warning(G_STRLOC ": signal \"%s::%s\" has return type '%s' and is "
                           "only G_SIGNAL_RUN_FIRST",
    ...
    if (!node)
    {
        ...
        key.signal_id = signal_id;
        g_signal_key_bsa = g_bsearch_array_insert(g_signal_key_bsa,
                                                  &g_signal_key_bconfig, &key);
        g_strdelimit(name, "_", '-');
...
#define ADD_CHECK(__type__)                                                    \
    else if (g_type_is_a(param_types[0] & ~G_SIGNAL_TYPE_STATIC_SCOPE,         \
                         G_TYPE_##__type__))                                   \
    {                                                                          \
 ...
 * Since: 2.32
 */
void g_signal_set_va_marshaller(guint signal_id, GType instance_type,
                                GSignalCVaMarshaller va_marshaller)
{
    SignalNode* node;
    ...
    if (node)
    {
        ...
        if (node->class_closure_bsa)
        {
            ClassClosure* cc = g_bsearch_array_get_nth(
                node->class_closure_bsa, &g_class_closure_bconfig, 0);
                          ...
                          guint n_params, va_list args)
{
    ...
    signal_id = g_signal_newv(signal_name, itype, signal_flags, class_closure,
...
static void signal_destroy_R(SignalNode* signal_node)
{
    ...
    /* check current emissions */
    {
        ...
        for (emission = g_emissions; emission; emission = emission->next)
            if (emission->ihint.signal_id == node.signal_id)
                g_critical(G_STRLOC ": signal \"%s\" being destroyed is "
                                    "currently in emission (instance '%p')",
    ...
    if (node.class_closure_bsa)
    {
        ...
        {
            ClassClosure* cc = g_bsearch_array_get_nth(
                node.class_closure_bsa, &g_class_closure_bconfig, i);
 ...
 */
void g_signal_override_class_closure(guint signal_id, GType instance_type,
                                     GClosure* class_closure)
{
    ...
    if (!g_type_is_a(instance_type, node->itype))
        g_warning("%s: type '%s' cannot be overridden for signal id '%u'",
                  G_STRLOC, type_debug_name(instance_type), signal_id);
    else
    {
        ClassClosure* cc = signal_find_class_closure(node, instance_type);

        if (cc && cc->instance_type == instance_type)
            g_warning("%s: type '%s' is already overridden for signal id '%u'",
                      G_STRLOC, type_debug_name(instance_type), signal_id);
        else
                                     ...
                                     GCallback class_handler)
{
    ...
    if (signal_id)
        g_signal_override_class_closure(
            signal_id, instance_type,
            g_cclosure_new(class_handler, NULL, NULL));
    else
        g_warning(
            "%s: signal name '%s' is invalid for type id '%" G_GSIZE_FORMAT "'",
                                    ...
                                    GValue* return_value)
{
    ...
    if (emission)
    {
        ...
        /* we should probably do the same parameter checks as g_signal_emit()
         * here.
         */
        if (emission->chain_type != G_TYPE_NONE)
        {
            ClassClosure* cc =
                signal_find_class_closure(node, emission->chain_type);

            g_assert(cc != NULL); /* closure currently in call stack */

            n_params = node->n_params;
            restore_type = cc->instance_type;
            cc = signal_find_class_closure(node,
                                           g_type_parent(cc->instance_type));
            ...
            g_warning("%s: signal id '%u' cannot be chained from current "
                      "emission stage for instance '%p'",
                      G_STRLOC, node->signal_id, instance);
    }
    else
        g_warning("%s: no signal is currently being emitted for instance '%p'",
                  G_STRLOC, instance);
    ...
    if (closure)
    {
        ...
        SIGNAL_UNLOCK();
        g_closure_invoke(closure, return_value, n_params + 1,
                         instance_and_params, &emission->ihint);
        SIGNAL_LOCK();
...
void g_signal_chain_from_overridden_handler(gpointer instance, ...)
{
    ...
    if (closure)
    {
        ...
        for (i = 0; i < node->n_params; i++)
        {
            ...
            SIGNAL_UNLOCK();
            G_VALUE_COLLECT_INIT(param_values + i, ptype, var_args,
        ...
        {
            g_closure_invoke(closure, NULL, n_params + 1, instance_and_params,
                             &emission->ihint);
        }
        else
        {
            ...
            G_VALUE_LCOPY(&return_value, var_args,
                          static_scope ? G_VALUE_NOCOPY_CONTENTS : 0, &error);
            if (!error)
            {
 ...
 *
 * Returns: (transfer none): the invocation hint of the innermost signal
 * emission.
 */
                                      ...
                                      gboolean after)
{
    ...
    {
        if (detail && !(node->flags & G_SIGNAL_DETAILED))
            g_warning("%s: signal id '%u' does not support detail (%u)",
                      G_STRLOC, signal_id, detail);
 ...
 * Returns: the handler ID (always greater than 0 for successful connections)
 */
gulong g_signal_connect_closure(gpointer instance, const gchar* detailed_signal,
    ...
    if (signal_id)
    {
        ...
        else
        {
            ...
            if (node->c_marshaller && G_CLOSURE_NEEDS_MARSHAL(handler->closure))
            {
                ...
                if (node->va_marshaller)
                    _g_closure_set_va_marshal(handler->closure,
                                              node->va_marshaller);
            }
        }
    }
                             ...
                             GConnectFlags connect_flags)
{
    ...
    if (signal_id)
    {
        ...
        else
        {
            ...
            handler->detail = detail;
            handler->closure =
                g_closure_ref((swapped ? g_cclosure_new_swap : g_cclosure_new)(
 ...
 * @instance: (type GObject.Object): The instance to block the signal handler
 * of.
...
void g_signal_handler_block(gpointer instance, gulong handler_id)
{
    ...
    if (handler)
    {
        ...
        g_warning("%s: instance '%p' has no handler with id '%lu'", G_STRLOC,
                  instance, handler_id);
    SIGNAL_UNLOCK();
}
 ...
 * @instance: (type GObject.Object): The instance to unblock the signal handler
 * of.
...
void g_signal_handler_unblock(gpointer instance, gulong handler_id)
{
    ...
    if (handler)
    {
        if (handler->block_count)
            ...
            g_warning(G_STRLOC
                      ": handler '%lu' of instance '%p' is not blocked",
                      handler_id, instance);
    }
    else
        g_warning("%s: instance '%p' has no handler with id '%lu'", G_STRLOC,
 ...
 * @instance: (type GObject.Object): The instance to remove the signal handler
 * from.
 ...
 * @instance: (type GObject.Object): The instance where a signal handler is
 * sought.
 ...
 * @instance: (type GObject.Object): The instance whose signal handlers are
 * destroyed
 *
 ...
 * @instance: (type GObject.Object): The instance owning the signal handler to
 * be found.
                             ...
                             gpointer func, gpointer data)
{
    ...
    if (mask & G_SIGNAL_MATCH_MASK)
    {
        ...
        SIGNAL_LOCK();
        mlist = handlers_find(instance, mask, signal_id, detail, closure, func,
                              data, TRUE);
        if (mlist)
        {
...
static guint signal_handlers_foreach_matched_R(
    gpointer instance, GSignalMatchType mask, guint signal_id, GQuark detail,
    GClosure* closure, gpointer func, gpointer data,
    void (*callback)(gpointer instance, gulong handler_seq_no))
{
    HandlerMatch* mlist;
    guint n_handlers = 0;
 ...
 */
guint g_signal_handlers_block_matched(gpointer instance, GSignalMatchType mask,
                                      guint signal_id, GQuark detail,
                                      GClosure* closure, gpointer func,
                                      gpointer data)
{
    ...
    if (mask &
        (G_SIGNAL_MATCH_CLOSURE | G_SIGNAL_MATCH_FUNC | G_SIGNAL_MATCH_DATA))
    {
        SIGNAL_LOCK();
        n_handlers = signal_handlers_foreach_matched_R(
 ...
 * @instance: (type GObject.Object): the object whose signal handlers are
 * sought.
 * @signal_id: the signal id.
 * @detail: the detail.
                                      ...
                                      GQuark detail, gboolean may_be_blocked)
{
    ...
    {
        ClassClosure* class_closure =
            signal_find_class_closure(node, G_TYPE_FROM_INSTANCE(instance));
                    ...
                    GQuark detail, GValue* return_value)
{
    ...
    for (i = 0; i < node->n_params; i++)
        if (!G_TYPE_CHECK_VALUE_TYPE(param_values + i,
                                     node->param_types[i] &
                                         ~G_SIGNAL_TYPE_STATIC_SCOPE))
        {
            g_critical("%s: value for '%s' parameter %u for signal \"%s\" is "
                       "of type '%s'",
                       G_STRLOC, type_debug_name(node->param_types[i]), i,
                       node->name, G_VALUE_TYPE_NAME(param_values + i));
            SIGNAL_UNLOCK();
            return;
        }
    if (node->return_type != G_TYPE_NONE)
    {
        if (!return_value)
        {
            g_critical("%s: return value '%s' for signal \"%s\" is (NULL)",
                       G_STRLOC, type_debug_name(node->return_type),
                       node->name);
            SIGNAL_UNLOCK();
            return;
        }
        else if (!node->accumulator &&
                 !G_TYPE_CHECK_VALUE_TYPE(return_value,
                                          node->return_type &
                                              ~G_SIGNAL_TYPE_STATIC_SCOPE))
        {
            g_critical(
                "%s: return value '%s' for signal \"%s\" is of type '%s'",
                G_STRLOC, type_debug_name(node->return_type), node->name,
...
static inline gboolean accumulate(GSignalInvocationHint* ihint,
                                  GValue* return_accu, GValue* handler_return,
                                  SignalAccumulator* accumulator)
{
 ...
 */
void g_signal_emit_valist(gpointer instance, guint signal_id, GQuark detail,
                          va_list var_args)
{
    GValue* instance_and_params;
    GType signal_return_type;
    ...
    if (node->single_va_closure != NULL)
    {
        ...
        for (l = hlist ? hlist->handlers : NULL; fastpath && l != NULL;
             l = l->next)
        {
            if (!l->block_count && (!l->detail || l->detail == detail))
            {
                if (closure != NULL ||
                    !_g_closure_supports_invoke_va(l->closure))
                {
                    fastpath = FALSE;
                    break;
                }
        ...
        /* Don't allow no-recurse emission as we might have to restart, which
       means we will run multiple handlers and thus must ref all arguments */
        ...
        if (fastpath)
        {
            ...
            TRACE(GOBJECT_SIGNAL_EMIT(signal_id, detail, instance,
                                      instance_type));
            ...
            if (closure != NULL)
            {
                ...
                _g_closure_invoke_va(closure, return_accu, instance, var_args,
                                     node->n_params, node->param_types);
                accumulate(&emission.ihint, &emission_return, &accu,
                           accumulator);
            }
            ...
            if (rtype != G_TYPE_NONE)
            {
                ...
                G_VALUE_LCOPY(&emission_return, var_args,
                              static_scope ? G_VALUE_NOCOPY_CONTENTS : 0,
                              &error);
                if (!error)
            ...
            TRACE(GOBJECT_SIGNAL_EMIT_END(signal_id, detail, instance,
                                          instance_type));
...
/**
 * g_signal_emit:
 * @instance: (type GObject.Object): the instance the signal is being emitted
 * on.
 * @signal_id: the signal id
 * @detail: the detail
...
void g_signal_emit_by_name(gpointer instance, const gchar* detailed_signal, ...)
{
    ...
    if (signal_id)
    {
        ...
        g_warning(
            "%s: signal name '%s' is invalid for instance '%p' of type '%s'",
            G_STRLOC, detailed_signal, instance, g_type_name(itype));
}
                                       ...
                                       const GValue* instance_and_params)
{
    ...
    TRACE(GOBJECT_SIGNAL_EMIT(node->signal_id, detail, instance,
...
EMIT_RESTART:
    ...
    if ((node->flags & G_SIGNAL_RUN_FIRST) && class_closure)
    {
        ...
        SIGNAL_UNLOCK();
        g_closure_invoke(class_closure, return_accu, node->n_params + 1,
                         instance_and_params, &emission.ihint);
    ...
    if (node->emission_hooks)
    {
        ...
        while (hook)
        {
            ...
            if (!signal_hook->detail || signal_hook->detail == detail)
            {
                ...
                SIGNAL_UNLOCK();
                need_destroy = !hook_func(&emission.ihint, node->n_params + 1,
    ...
    if (handler_list)
    {
        ...
        do
        {
            ...
            else if (!handler->block_count &&
                     (!handler->detail || handler->detail == detail) &&
                     handler->sequential_number < max_sequential_handler_number)
            {
                SIGNAL_UNLOCK();
                g_closure_invoke(handler->closure, return_accu,
                                 node->n_params + 1, instance_and_params,
                                 &emission.ihint);
                if (!accumulate(&emission.ihint, emission_return, &accu,
                                accumulator) &&
                    emission.state == EMISSION_RUN)
                    emission.state = EMISSION_STOP;
                SIGNAL_LOCK();
...
EMIT_CLEANUP:
    ...
    if ((node->flags & G_SIGNAL_RUN_CLEANUP) && class_closure)
    {
        ...
        g_closure_invoke(
            class_closure, node->return_type != G_TYPE_NONE ? &accu : NULL,
            node->n_params + 1, instance_and_params, &emission.ihint);
        if (need_unset)
            g_value_unset(&accu);
        SIGNAL_LOCK();
        emission.chain_type = G_TYPE_NONE;
    ...
    TRACE(GOBJECT_SIGNAL_EMIT_END(node->signal_id, detail, instance,
                                  G_TYPE_FROM_INSTANCE(instance)));
...
static void add_invalid_closure_notify(Handler* handler, gpointer instance)
{
    g_closure_add_invalidate_notifier(handler->closure, instance,
...
static void remove_invalid_closure_notify(Handler* handler, gpointer instance)
{
    ...
    {
        g_closure_remove_invalidate_notifier(handler->closure, instance,
                                             invalid_closure_notify);
        handler->has_invalid_closure_notify = 0;
    }
}

static void invalid_closure_notify(gpointer instance, GClosure* closure)
{
    Handler* handler;
    guint signal_id;
    ...
    /* See https://bugzilla.gnome.org/show_bug.cgi?id=730296 for discussion
     * about this... */
