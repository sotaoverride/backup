...
{
    fprintf(stderr,
            _("Usage: %s [-c|-l filename] [-b block-size] "
                         ...
                         const char* bad_blocks_file)
{
    ...
    if (!f)
    {
        com_err("read_bad_blocks_file", errno, _("while trying to open %s"),
                bad_blocks_file);
        exit(1);
    }
...
static void test_disk(ext2_filsys fs, badblocks_list* bb_list)
{
    ...
    sprintf(buf, "badblocks -b %d -X %s%s%s %llu", fs->blocksize,
            quiet ? "" : "-s ", (cflag > 1) ? "-w " : "", fs->device_name,
    ...
    if (!f)
    {
        com_err("popen", errno, _("while trying to run '%s'"), buf);
        exit(1);
    }
    retval = ext2fs_read_bb_FILE(fs, f, bb_list, invalid_block);
    pclose(f);
...
static void handle_bad_blocks(ext2_filsys fs, badblocks_list bb_list)
{
    ...
    for (i = fs->super->s_first_data_block; i <= must_be_good; i++)
    {
        ...
        {
            fprintf(stderr,
                    _("Block %d in primary "
                      "superblock/group descriptor area bad.\n"),
                    i);
            fprintf(stderr,
                    _("Blocks %u through %u must be good "
     ...
     */
    group_block = fs->super->s_first_data_block + fs->super->s_blocks_per_group;
    ...
    for (i = 1; i < fs->group_desc_count; i++)
    {
        ...
        {
            if (ext2fs_badblocks_list_test(bb_list, group_block + j))
            {
                if (!group_bad)
                    fprintf(stderr,
                            _("Warning: the backup superblock/group "
                ...
                ext2fs_bg_free_blocks_count_set(
                    fs, group, ext2fs_bg_free_blocks_count(fs, group) + 1);
...
static errcode_t packed_allocate_tables(ext2_filsys fs)
{
    ...
    for (i = 0; i < fs->group_desc_count; i++)
    {
        ...
        retval = ext2fs_get_free_blocks2(
            fs, goal, end, fs->inode_blocks_per_group, fs->block_map, &goal);
        if (retval)
            return retval;
        ext2fs_block_alloc_stats_range(fs, goal, fs->inode_blocks_per_group,
                                       +1);
...
static void write_inode_tables(ext2_filsys fs, int lazy_flag, int itable_zeroed)
{
    ...
    ext2fs_numeric_progress_init(fs, &progress, _("Writing inode tables: "),
                                 fs->group_desc_count);
...
static void create_root_dir(ext2_filsys fs)
{
    ...
    if (retval)
    {
        com_err("ext2fs_mkdir", retval, "%s", _("while creating root dir"));
        exit(1);
    }
...
static void create_lost_and_found(ext2_filsys fs)
{
    ...
    if (retval)
    {
        com_err("ext2fs_mkdir", retval, "%s", _("while creating /lost+found"));
        exit(1);
    }
    ...
    if (retval)
    {
        com_err("ext2_lookup", retval, "%s", _("while looking up /lost+found"));
        exit(1);
    }
...
static void create_journal_dev(ext2_filsys fs)
{
    ...
    retval = ext2fs_create_journal_superblock(
        fs, ext2fs_blocks_count(fs->super), 0, &buf);
    if (retval)
    {
    ...
    ext2fs_numeric_progress_init(fs, &progress, _("Zeroing journal device: "),
...
write_superblock:
    retval = io_channel_write_blk64(fs->io, fs->super->s_first_data_block + 1,
                                    1, buf);
...
static void show_stats(ext2_filsys fs)
{
    ...
    if (!verbose)
    {
        printf(_("Creating filesystem with %llu %dk blocks and "
                 "%u inodes\n"),
               ext2fs_blocks_count(s), fs->blocksize >> 10, s->s_inodes_count);
        goto skip_details;
    }
    ...
    if (ext2fs_has_feature_bigalloc(fs->super))
        printf(_("Cluster size=%u (log=%u)\n"),
               fs->blocksize << fs->cluster_ratio_bits, s->s_log_cluster_size);
    else
        printf(_("Fragment size=%u (log=%u)\n"), EXT2_CLUSTER_SIZE(s),
               s->s_log_cluster_size);
    printf(_("Stride=%u blocks, Stripe width=%u blocks\n"), s->s_raid_stride,
           s->s_raid_stripe_width);
                                ...
                                const char* opts)
{
    ...
    for (token = buf; token && *token; token = next)
    {
        ...
        if (strcmp(token, "desc-size") == 0 || strcmp(token, "desc_size") == 0)
        {
            int desc_size;

            if (!ext2fs_has_feature_64bit(&fs_param))
            {
                fprintf(stderr, _("%s requires '-O 64bit'\n"), token);
                r_usage++;
                continue;
            }
            if (param->s_reserved_gdt_blocks != 0)
            {
                fprintf(stderr, _("'%s' must be before 'resize=%u'\n"), token,
        ...
        else if (strcmp(token, "hash_seed") == 0)
        {
            ...
            if (uuid_parse(arg, (unsigned char*)param->s_hash_seed) != 0)
            {
                fprintf(stderr, _("Invalid hash seed: %s\n"), arg);
                r_usage++;
                continue;
            }
        }
        ...
        else if (strcmp(token, "mmp_update_interval") == 0)
        {
            ...
            if (*p)
            {
                fprintf(stderr, _("Invalid mmp_update_interval: %s\n"), arg);
                r_usage++;
                continue;
            }
        }
        ...
        else if (strcmp(token, "stride") == 0)
        {
            ...
            if (*p)
            {
                fprintf(stderr, _("Invalid stride parameter: %s\n"), arg);
                r_usage++;
                continue;
            }
        }
                 ...
                 strcmp(token, "stripe_width") == 0)
        {
            ...
            if (*p)
            {
                fprintf(stderr, _("Invalid stripe-width parameter: %s\n"), arg);
                r_usage++;
                continue;
            }
        }
        else if (!strcmp(token, "resize"))
        {
            ...
            resize = parse_num_blocks2(arg, param->s_log_block_size);
            ...
            group_desc_count =
                (__u32)ext2fs_div64_ceil(ext2fs_blocks_count(param), bpg);
            desc_blocks = (group_desc_count + gdpb - 1) / gdpb;
            rsv_groups = ext2fs_div64_ceil(resize, bpg);
            rsv_gdb = ext2fs_div_ceil(rsv_groups, gdpb) - desc_blocks;
            ...
            if (rsv_gdb > 0)
            {
                ...
                {
                    fprintf(stderr, "%s",
                            _("On-line resizing not supported with revision 0 "
                              "filesystems\n"));
                    free(buf);
                    exit(1);
                }
        ...
        else if (!strcmp(token, "lazy_journal_init"))
        {
            if (arg)
                journal_flags |=
                    strtoul(arg, &p, 0) ? EXT2_MKJOURNAL_LAZYINIT : 0;
            else
        ...
        else if (!strcmp(token, "root_owner"))
        {
            if (arg)
            {
                ...
                if (*p != ':')
                {
                    fprintf(stderr, _("Invalid root_owner: '%s'\n"), arg);
                    r_usage++;
                    continue;
                }
                p++;
        ...
        else if (!strcmp(token, "quotatype"))
        {
            ...
            if (ret)
            {
                if (errtok)
                {
                    fprintf(stderr, "Failed to parse quota type at %s", errtok);
                    free(errtok);
                }
                else
                    com_err(program_name, ret, "while parsing quota type");
                r_usage++;
                badopt = token;
                continue;
            }
...
static __u32 ok_features[3] = {
    /* Compat */
    EXT3_FEATURE_COMPAT_HAS_JOURNAL | EXT2_FEATURE_COMPAT_RESIZE_INODE |
        EXT2_FEATURE_COMPAT_DIR_INDEX | EXT2_FEATURE_COMPAT_EXT_ATTR |
    ...
    EXT2_FEATURE_INCOMPAT_FILETYPE | EXT3_FEATURE_INCOMPAT_EXTENTS |
        ...
        EXT4_FEATURE_INCOMPAT_FLEX_BG | EXT4_FEATURE_INCOMPAT_EA_INODE |
    ...
    EXT2_FEATURE_RO_COMPAT_LARGE_FILE | EXT4_FEATURE_RO_COMPAT_HUGE_FILE |
        ...
        EXT2_FEATURE_RO_COMPAT_SPARSE_SUPER | EXT4_FEATURE_RO_COMPAT_GDT_CSUM |
        EXT4_FEATURE_RO_COMPAT_BIGALLOC | EXT4_FEATURE_RO_COMPAT_QUOTA |
        EXT4_FEATURE_RO_COMPAT_METADATA_CSUM | EXT4_FEATURE_RO_COMPAT_PROJECT |
                                  ...
                                  const char* subsection)
{
    ...
    if (profile_iterator_create(
            prof, names,
            PROFILE_ITER_LIST_SECTION | PROFILE_ITER_RELATIONS_ONLY, &state))
        return 0;
...
static char** parse_fs_type(const char* fs_type, const char* usage_types,
                            struct ext2_super_block* sb,
                            blk64_t fs_blocks_count, char* progname)
{
    ...
    if (!ext_type)
    {
        profile_get_string(profile, "defaults", "fs_type", 0, "ext2",
                           &profile_type);
                              ...
                              const char* def_val)
{
    ...
    for (i -= 1; i >= 0; i--)
    {
        profile_get_string(profile, "fs_types", types[i], opt, 0, &ret);
        if (ret)
            return ret;
    }
 ...
 */
static int get_device_geometry(const char* file, struct ext2_super_block* param,
...
static void PRS(int argc, char* argv[])
{
    ...
    unsigned long long num_inodes =
        0; /* unsigned long long to catch too-large input */
    errcode_t retval;
...
#define _SC_PAGESIZE _SC_PAGE_SIZE
    ...
    profile_error:
        fprintf(stderr,
                _("Couldn't init profile successfully"
                  " (error: %ld).\n"),
                retval);
        exit(1);
    }
    ...
    while (
        (c = getopt(
             argc, argv,
             "b:cd:e:g:i:jl:m:no:qr:s:t:vC:DE:FG:I:J:KL:M:N:O:R:ST:U:Vz:")) !=
        EOF)
    {
        switch (c)
        {
            case 'b':
                ...
                if (b < EXT2_MIN_BLOCK_SIZE || b > EXT2_MAX_BLOCK_SIZE)
                {
                    com_err(program_name, 0, _("invalid block size - %s"),
                            optarg);
                    exit(1);
                }
                if (blocksize > 4096)
                    fprintf(stderr,
                            _("Warning: blocksize %d not "
            ...
            case 'C':
                ...
                {
                    com_err(program_name, 0, _("invalid cluster size - %s"),
                            optarg);
                    exit(1);
                }
                break;
            ...
            case 'e':
                ...
                {
                    com_err(program_name, 0, _("bad error behavior - %s"),
                            optarg);
                    usage();
                }
                break;
            ...
            case 'i':
                ...
                {
                    com_err(program_name, 0,
                            _("invalid inode ratio %s (min %d/max %d)"), optarg,
                            EXT2_MIN_BLOCK_SIZE, EXT2_MAX_BLOCK_SIZE * 1024);
                    exit(1);
                }
                break;
            case 'I':
                ...
                if (*tmp)
                {
                    com_err(program_name, 0, _("invalid inode size - %s"),
                            optarg);
                    exit(1);
                }
                break;
            ...
            case 'l':
                bad_blocks_filename =
                    realloc(bad_blocks_filename, strlen(optarg) + 1);
            ...
            case 'L':
                ...
                if (strlen(volume_label) > EXT2_LABEL_LEN)
                {
                    ...
                    fprintf(stderr,
                            _("Warning: label too long; will be truncated to "
                              "'%s'\n\n"),
                            volume_label);
                }
                break;
            case 'm':
                ...
                if (*tmp || reserved_ratio > 50 || reserved_ratio < 0)
                {
                    com_err(program_name, 0,
                            _("invalid reserved blocks percent - %s"), optarg);
                    exit(1);
                }
                break;
            case 'M':
            ...
            case 'N':
                ...
                if (*tmp)
                {
                    com_err(program_name, 0, _("bad num inodes - %s"), optarg);
                    exit(1);
                }
                break;
            case 'o':
            ...
            case 'O':
                retval = ext2fs_resize_mem(
                    fs_features_size, fs_features_size + 1 + strlen(optarg),
                    &fs_features);
                if (retval)
                {
            ...
            case 'r':
                ...
                if (*tmp)
                {
                    com_err(program_name, 0, _("bad revision level - %s"),
                            optarg);
                    exit(1);
                }
    ...
    if (blocksize <= 0 && journal_device)
    {
        ...
        retval = ext2fs_open(journal_device, EXT2_FLAG_JOURNAL_DEV_OK, 0, 0,
                             io_ptr, &jfs);
        if (retval)
        {
            com_err(program_name, retval,
    ...
    {
        fs_blocks_count =
            parse_num_blocks2(argv[optind++], fs_param.s_log_block_size);
        if (!fs_blocks_count)
        {
            com_err(program_name, 0, _("invalid blocks '%s' on device '%s'"),
    ...
    profile_get_integer(profile, "options", "sync_kludge", 0, 0, &sync_kludge);
    ...
    if (noaction && fs_blocks_count)
    {
        ...
        retval = ext2fs_get_device_size2(device_name,
                                         EXT2_BLOCK_SIZE(&fs_param), &dev_size);
#else
        retval = ext2fs_get_device_size(device_name, EXT2_BLOCK_SIZE(&fs_param),
                                        &dev_size);
#endif
    ...
    if (!fs_blocks_count)
    {
        ...
        else
        {
            ...
            if (sys_page_size > EXT2_BLOCK_SIZE(&fs_param))
                fs_blocks_count &= ~((blk64_t)(
                    (sys_page_size / EXT2_BLOCK_SIZE(&fs_param)) - 1));
        }
    }
    ...
    if (!fs_type)
        profile_get_string(profile, "devices", device_name, "fs_type", 0,
                           &fs_type);
    if (!usage_types)
        profile_get_string(profile, "devices", device_name, "usage_types", 0,
                           &usage_types);
     ...
     */
    fs_types =
        parse_fs_type(fs_type, usage_types, &fs_param,
                      fs_blocks_count ? fs_blocks_count : dev_size, argv[0]);
    if (!fs_types)
    {
    ...
    if (fs_param.s_rev_level != EXT2_GOOD_OLD_REV)
    {
        ...
        {
            tmp = NULL;
            profile_get_string(profile, "fs_types", *cpp, "features", "", &tmp);
            if (tmp && *tmp)
                edit_feature(tmp, &fs_param.s_feature_compat);
            if (tmp)
                free(tmp);
    ...
    edit_feature(fs_features ? fs_features : tmp, &fs_param.s_feature_compat);
    if (tmp)
        free(tmp);
    ...
    if (for_hurd(creator_os))
    {
        ...
        {
            fprintf(stderr, "%s",
                    _("The HURD does not support the "
                      "filetype feature.\n"));
            exit(1);
        }
    ...
    else
    {
        ...
        else if ((blocksize < psector_size) && (psector_size <= sys_page_size))
        { /* Suboptimal */
            fprintf(stderr,
                    _("Warning: specified blocksize %d is "
                      "less than device physical sectorsize %d\n"),
                    blocksize, psector_size);
        }
    }

    fs_param.s_log_block_size = int_log2(blocksize >> EXT2_MIN_BLOCK_LOG_SIZE);

    /*
     * We now need to do a sanity check of fs_blocks_count for
     * 32-bit vs 64-bit block number support.
     */
    if ((fs_blocks_count > MAX_32_NUM) && ext2fs_has_feature_64bit(&fs_param))
    ...
    {
        reserved_ratio =
            get_double_from_profile(fs_types, "reserved_ratio", 5.0);
        if (reserved_ratio > 50 || reserved_ratio < 0)
        {
            com_err(program_name, 0, _("invalid reserved blocks percent - %lf"),
                    reserved_ratio);
            exit(1);
        }
    }
    ...
    if (ext2fs_has_feature_bigalloc(&fs_param))
    {
        if (!cluster_size)
            cluster_size =
                get_int_from_profile(fs_types, "cluster_size", blocksize * 16);
    ...
    if (inode_ratio == 0)
    {
        inode_ratio = get_int_from_profile(fs_types, "inode_ratio", 8192);
...
#ifdef HAVE_BLKID_PROBE_GET_TOPOLOGY
    retval =
        get_device_geometry(device_name, &fs_param, (unsigned int)psector_size);
    if (retval < 0)
    {
        fprintf(stderr, _("warning: Unable to get device geometry for %s\n"),
                device_name);
    }
    else if (retval)
    {
    ...
    if (blocksize > sys_page_size)
    {
        ...
        {
            com_err(program_name, 0,
                    _("%d-byte blocks too big for system (max %d)"), blocksize,
                    sys_page_size);
            proceed_question(proceed_delay);
        }
        fprintf(stderr,
                _("Warning: %d-byte blocks too big for system "
    ...
    lazy_itable_init =
        get_bool_from_profile(fs_types, "lazy_itable_init", lazy_itable_init);
    discard = get_bool_from_profile(fs_types, "discard", discard);
    journal_flags |= get_bool_from_profile(fs_types, "lazy_journal_init", 0)
    ...
    if (!journal_location_string)
        journal_location_string =
            get_string_from_profile(fs_types, "journal_location", "");
    ...
    packed_meta_blocks =
        get_bool_from_profile(fs_types, "packed_meta_blocks", 0);
    if (packed_meta_blocks)
        journal_location = 0;
    ...
    {
        fprintf(stderr, "%s",
                _("The resize_inode and meta_bg "
    ...
    if (!flex_bg_size && ext2fs_has_feature_flex_bg(&fs_param))
        flex_bg_size = get_uint_from_profile(fs_types, "flex_bg_size", 16);
    if (flex_bg_size)
    {
    ...
    if (inode_size && fs_param.s_rev_level >= EXT2_DYNAMIC_REV)
    {
        ...
        {
            com_err(program_name, 0, _("invalid inode size %d (min %d/max %d)"),
                    inode_size, EXT2_GOOD_OLD_INODE_SIZE, blocksize);
            exit(1);
        }
    ...
    {
        com_err(program_name, 0,
                _("too many inodes (%llu), specify < 2^32 inodes"), num_inodes);
        exit(1);
    }
    /*
    ...
    if ((((unsigned long long)fs_param.s_inodes_count) *
        ...
        ((ext2fs_blocks_count(&fs_param)) * EXT2_BLOCK_SIZE(&fs_param)))
    {
        com_err(program_name, 0,
                _("inode_size (%u) * inodes_count "
                  "(%u) too big for a\n\t"
    ...
    ext2fs_r_blocks_count_set(
        &fs_param, reserved_ratio * ext2fs_blocks_count(&fs_param) / 100.0);
...
static int mke2fs_setup_tdb(const char* name, io_manager* io_ptr)
{
    ...
    if (!tdb_dir)
    {
        profile_get_string(profile, "defaults", "undo_dir", 0,
                           "/var/lib/e2fsprogs", &tdb_dir);
        free_tdb_dir = 1;
    }

    if (!strcmp(tdb_dir, "none") || (tdb_dir[0] == 0) || access(tdb_dir, W_OK))
    {
        if (free_tdb_dir)
            free(tdb_dir);
        return 0;
    ...
    {
        retval = errno;
        com_err(program_name, retval, _("while trying to delete %s"), tdb_file);
        goto errout;
    }
...
errout:
    ...
    free(tdb_file);
err:
    com_err(program_name, retval, "%s", _("while trying to setup undo file\n"));
    return retval;
}
...
static int mke2fs_discard_device(ext2_filsys fs)
{
    ...
    ext2fs_numeric_progress_init(fs, &progress, _("Discarding device blocks: "),
                                 blocks);
    while (cur < blocks)
    {
...
static void fix_cluster_bg_counts(ext2_filsys fs)
{
    ...
    {
        retval = ext2fs_find_first_zero_block_bitmap2(fs->block_map, block,
                                                      last_block, &next);
        if (retval == 0)
            block = next;
        else
        ...
        retval = ext2fs_find_first_set_block_bitmap2(fs->block_map, block,
                                                     last_block, &next);
        if (retval)
            next = last_block + 1;
...
static int create_quota_inodes(ext2_filsys fs)
{
    ...
    if (retval)
    {
        com_err(program_name, retval, _("while initializing quota context"));
        exit(1);
    }
    ...
    if (retval)
    {
        com_err(program_name, retval, _("while writing quota inodes"));
        exit(1);
    }
...
static errcode_t set_error_behavior(ext2_filsys fs)
{
    ...
    {
        com_err(program_name, 0, _("bad error behavior in profile - %s"), arg);
        free(arg);
...
int main(int argc, char* argv[])
{
    ...
    profile_get_boolean(profile, "options", "old_bitmaps", 0, 0, &old_bitmaps);
    if (!old_bitmaps)
    ...
    if (android_sparse_file)
    {
        ...
        sprintf(android_sparse_params, "(%s):%u:%u", device_name,
                fs_param.s_blocks_count, 1024 << fs_param.s_log_block_size);
        retval = ext2fs_initialize(android_sparse_params, flags, &fs_param,
                                   sparse_io_manager, &fs);
    ...
    if (retval)
    {
        com_err(device_name, retval, "%s", _("while setting up superblock"));
        exit(1);
    }
    ...
    if (!quiet && !ext2fs_has_feature_journal_dev(fs->super) &&
        ext2fs_has_feature_metadata_csum(fs->super))
    {
        if (!ext2fs_has_feature_extents(fs->super))
            printf("%s", _("Extents are not enabled.  The file extent "
                           "tree can be checksummed, whereas block maps "
                           "cannot.  Not enabling extents reduces the "
                           "coverage of metadata checksumming.  "
                           "Pass -O extents to rectify.\n"));
        if (!ext2fs_has_feature_64bit(fs->super))
            printf("%s", _("64-bit filesystem support is not enabled.  "
    ...
    if (!journal_device &&
        ((journal_size) || ext2fs_has_feature_journal(&fs_param)))
        journal_blocks = figure_journal_size(journal_size, fs);

    sprintf(opt_string, "tdb_data_size=%d",
            fs->blocksize <= 4096 ? 32768 : fs->blocksize * 8);
    ...
    if (!noaction && discard && dev_size && (io_ptr != undo_io_manager))
    {
        ...
        {
            if (verbose)
                printf("%s", _("Discard succeeded and will return "
    ...
    if (ext2fs_has_feature_csum_seed(fs->super))
        fs->super->s_checksum_seed =
            ext2fs_crc32c_le(~0, fs->super->s_uuid, sizeof(fs->super->s_uuid));
     ...
     */
    hash_alg_str = get_string_from_profile(fs_types, "hash_alg", "half_md4");
    hash_alg = e2p_string2hash(hash_alg_str);
    free(hash_alg_str);
    fs->super->s_def_hash_version =
        (hash_alg >= 0) ? hash_alg : EXT2_HASH_HALF_MD4;

    if (memcmp(fs_param.s_hash_seed, zero_buf, sizeof(fs_param.s_hash_seed)) !=
        0)
    {
...
#define EXT2_DFL_CHECKINTERVAL (86400L * 180L)
    ...
    if (ext2fs_has_feature_encrypt(fs->super))
    {
        ...
        fs->super->s_encrypt_algos[1] = EXT4_ENCRYPTION_MODE_AES_256_CTS;
    }
    ...
    if (ext2fs_has_feature_flex_bg(fs->super) && packed_meta_blocks)
    ...
    else
    {
        ...
        if (start > 0)
            retval =
                ext2fs_zero_blocks2(fs, start, blocks - start, &ret_blk, NULL);
        ...
        if (ext2fs_has_feature_resize_inode(fs->super))
        {
            ...
            if (retval)
            {
                com_err("ext2fs_create_resize_inode", retval, "%s",
    ...
    if (journal_device)
    {
        ...
        if (!check_plausibility(journal_device, CHECK_BLOCK_DEV, NULL) &&
            !force)
            proceed_question(proceed_delay);
        check_mount(journal_device, force, _("journal"));

        retval =
            ext2fs_open(journal_device, EXT2_FLAG_RW | EXT2_FLAG_JOURNAL_DEV_OK,
                        0, fs->blocksize, unix_io_manager, &jfs);
        if (retval)
        {
            com_err(program_name, retval,
                    _("while trying to open journal device %s\n"),
        ...
        if (!quiet)
        {
            printf(_("Adding journal to device %s: "), journal_device);
            fflush(stdout);
        }
    ...
    else if ((journal_size) || ext2fs_has_feature_journal(&fs_param))
    {
        if (super_only)
        {
        ...
        if (!quiet)
        {
            printf(_("Creating journal (%u blocks): "), journal_blocks);
            fflush(stdout);
        }
        retval = ext2fs_add_journal_inode2(fs, journal_blocks, journal_location,
                                           journal_flags);
        if (retval)
        {
            com_err(program_name, retval, "%s",
...
no_journal:
    if (!super_only && ext2fs_has_feature_mmp(fs->super))
    {
