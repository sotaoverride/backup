...
int tests_get_args(int argc, char* argv[], const char* title, const char* desc,
                   const char* opts)
{
    int run_test = 0;
    int display_help = 0;
    ...
    for (i = 1; i < argc; ++i)
    {
        ...
        else if (strcmp(argv[i], "--unlink") == 0 || strcmp(argv[i], "-u") == 0)
            tests_unlink_flag = 1;
        else if (strcmp(argv[i], "--hole") == 0 || strcmp(argv[i], "-o") == 0)
            tests_hole_flag = 1;
        else if (strcmp(argv[i], "--close") == 0 || strcmp(argv[i], "-c") == 0)
            tests_close_flag = 1;
        else if (strcmp(argv[i], "--delete") == 0 || strcmp(argv[i], "-e") == 0)
            tests_delete_flag = 1;
        else
            display_help = 1;
    }
    ...
    else
    {
        ...
        if (display_title || display_description)
            if (argc == 2 ||
                (argc == 3 && display_title && display_description))
                run_test = 0;
    }
    return run_test;
}
...
void tests_check_test_file_system(void)
{
    ...
    {
        fprintf(stderr,
                "Invalid test file system mount directory:"
                " %s\n",
...
unsigned tests_write_fragment_file(unsigned file_number, int fd, off_t offset,
                                   unsigned size)
{
    int i, d;
    uint64_t u;
...
/* Write size bytes to the end of file "fragment_n" where n is the file_number
   ...
   i.e. seed for random numbers */
unsigned tests_append_to_fragment_file(unsigned file_number, unsigned size,
                                       int create)
{
    int fd;
    int flags;
...
/* Write size bytes at offset to the file "fragment_n" where n is the
   file_number and file_number also determines the random data written
   i.e. seed for random numbers */
unsigned tests_overwite_fragment_file(unsigned file_number, off_t offset,
                                      unsigned size)
{
    int fd;
    ...
    CHECK(fd != -1);
    actual_size = tests_write_fragment_file(file_number, fd, offset, size);
    CHECK(close(fd) != -1);
    return actual_size;
}
...
static int tests_get_mount_info(struct mntent* info)
{
    ...
    while (!found)
    {
        ...
        if (entry)
        {
            if (strcmp(entry->mnt_dir, tests_file_system_mount_dir) == 0)
            {
                found = 1;
                *info = *entry;
            }
...
void tests_remount(void)
{
    ...
    if (um)
    {
        ...
        if (!um_rorw)
        {
            ...
            err = mount(source, target, filesystemtype, mountflags, data);
            CHECK(err != -1);
        }
        else
        {
            /* Mount R/O and then re-mount R/W */
            err = mount(source, target, filesystemtype, mountflags | MS_RDONLY,
                        data);
            CHECK(err != -1);
