...
extern uint32_t lzma_mf_find(lzma_mf* mf, uint32_t* count_ptr,
                             lzma_match* matches)
{
    ...
    if (count > 0)
    {
        ...
        for (uint32_t i = 0; i < count; ++i)
        {
            ...
            assert(memcmp(mf_ptr(mf) - 1, mf_ptr(mf) - matches[i].dist - 2,
                          matches[i].len) == 0);
        }
#endif
...
static lzma_match* hc_find_func(const uint32_t len_limit, const uint32_t pos,
                                const uint8_t* const cur, uint32_t cur_match,
                                uint32_t depth, uint32_t* const son,
                                const uint32_t cyclic_pos,
                                const uint32_t cyclic_size, lzma_match* matches,
                                uint32_t len_best)
{
    ...
    while (true)
    {
        ...
        cur_match =
            son[cyclic_pos - delta + (delta > cyclic_pos ? cyclic_size : 0)];
...
#ifdef HAVE_MF_HC3
extern uint32_t lzma_mf_hc3_find(lzma_mf* mf, lzma_match* matches)
{
    header_find(false, 3);
...
extern void lzma_mf_hc3_skip(lzma_mf* mf, uint32_t amount)
{
    do
    {
        if (mf_avail(mf) < 3)
...
#ifdef HAVE_MF_HC4
extern uint32_t lzma_mf_hc4_find(lzma_mf* mf, lzma_match* matches)
{
    header_find(false, 4);
...
extern void lzma_mf_hc4_skip(lzma_mf* mf, uint32_t amount)
{
    do
    {
        if (mf_avail(mf) < 4)
...
static lzma_match* bt_find_func(const uint32_t len_limit, const uint32_t pos,
                                const uint8_t* const cur, uint32_t cur_match,
                                uint32_t depth, uint32_t* const son,
                                const uint32_t cyclic_pos,
                                const uint32_t cyclic_size, lzma_match* matches,
...
static void bt_skip_func(const uint32_t len_limit, const uint32_t pos,
                         const uint8_t* const cur, uint32_t cur_match,
                         uint32_t depth, uint32_t* const son,
                         const uint32_t cyclic_pos, const uint32_t cyclic_size)
{
    uint32_t* ptr0 = son + (cyclic_pos << 1) + 1;
    uint32_t* ptr1 = son + (cyclic_pos << 1);
...
#ifdef HAVE_MF_BT2
extern uint32_t lzma_mf_bt2_find(lzma_mf* mf, lzma_match* matches)
{
    header_find(true, 2);
...
extern void lzma_mf_bt2_skip(lzma_mf* mf, uint32_t amount)
{
    do
    {
        header_skip(true, 2);
...
#ifdef HAVE_MF_BT3
extern uint32_t lzma_mf_bt3_find(lzma_mf* mf, lzma_match* matches)
{
    header_find(true, 3);
...
extern void lzma_mf_bt3_skip(lzma_mf* mf, uint32_t amount)
{
    do
    {
        header_skip(true, 3);
...
#ifdef HAVE_MF_BT4
extern uint32_t lzma_mf_bt4_find(lzma_mf* mf, lzma_match* matches)
{
    header_find(true, 4);
...
extern void lzma_mf_bt4_skip(lzma_mf* mf, uint32_t amount)
{
    do
    {
        header_skip(true, 4);
