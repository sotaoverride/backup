...
static struct colinfo infos[] = {
    ...
    [COL_TARGET] = {"TARGET", 0.30, SCOLS_FL_TREE | SCOLS_FL_NOEXTREMES,
                    N_("mountpoint")},
    [COL_FSTYPE] = {"FSTYPE", 0.10, SCOLS_FL_TRUNC, N_("filesystem type")},
    [COL_OPTIONS] = {"OPTIONS", 0.10, SCOLS_FL_TRUNC, N_("all mount options")},
    [COL_VFS_OPTIONS] = {"VFS-OPTIONS", 0.20, SCOLS_FL_TRUNC,
                         N_("VFS specific mount options")},
    [COL_FS_OPTIONS] = {"FS-OPTIONS", 0.10, SCOLS_FL_TRUNC,
                        N_("FS specific mount options")},
    ...
    [COL_OLD_OPTIONS] = {"OLD-OPTIONS", 0.10, SCOLS_FL_TRUNC,
                         N_("old mount options saved by --poll")},
    [COL_OLD_TARGET] = {"OLD-TARGET", 0.30, 0,
                        N_("old mountpoint saved by --poll")},
    ...
    [COL_USEPERC] = {"USE%", 3, SCOLS_FL_RIGHT,
    ...
    [COL_OPT_FIELDS] = {"OPT-FIELDS", 0.10, SCOLS_FL_TRUNC,
                        N_("optional mount fields")},
    ...
    [COL_PASSNO] = {"PASSNO", 1, SCOLS_FL_RIGHT,
                    N_("pass number on parallel fsck(8) [fstab only]")}};
...
/*
 * Extra functionality for --target <path>. The function
 * mnt_table_find_mountpoint() also checks parents (path elements in reverse
 * order) to get mountpoint.
 *
...
int is_listall_mode(void)
{
    ...
    return (!get_match(COL_SOURCE) && !get_match(COL_TARGET) &&
            !get_match(COL_FSTYPE) && !get_match(COL_OPTIONS) &&
...
static char* get_tag(struct libmnt_fs* fs, const char* tagname,
                     int col
#ifndef HAVE_LIBUDEV
...
static char* get_tabdiff_data(struct libmnt_fs* old_fs,
                              struct libmnt_fs* new_fs, int change, int num)
{
    char* str = NULL;
    ...
    switch (get_column_id(num))
    {
        ...
        case COL_OLD_OPTIONS:
            if (old_fs &&
                (change == MNT_TABDIFF_REMOUNT || change == MNT_TABDIFF_UMOUNT))
                str = xstrdup(mnt_fs_get_options(old_fs));
            break;
        case COL_OLD_TARGET:
            if (old_fs &&
                (change == MNT_TABDIFF_MOVE || change == MNT_TABDIFF_UMOUNT))
...
static struct libscols_line* add_line(struct libscols_table* table,
                                      struct libmnt_fs* fs,
...
static struct libscols_line* add_tabdiff_line(struct libscols_table* table,
...
static int create_treenode(struct libscols_table* table,
...
static struct libmnt_table* parse_tabfiles(char** files, int nfiles,
                                           int tabtype)
{
...
static void cache_set_targets(struct libmnt_cache* tmp)
{
    ...
    path = access(_PATH_PROC_MOUNTINFO, R_OK) == 0 ? _PATH_PROC_MOUNTINFO
                                                   : _PATH_PROC_MOUNTS;
...
static int tab_is_tree(struct libmnt_table* tb)
{
    ...
    rc = (mnt_table_next_fs(tb, itr, &fs) == 0 && mnt_fs_is_kernel(fs) &&
          mnt_fs_get_root(fs));
...
struct libmnt_fs* get_next_fs(struct libmnt_table* tb, struct libmnt_iter* itr)
{
    struct libmnt_fs* fs = NULL;
    ...
    again:
        ...
        if (!fs && !(flags & FL_NOSWAPMATCH) && !get_match(COL_TARGET) &&
...
static int poll_match(struct libmnt_fs* fs)
{
    ...
    if (rc == 0 && !(flags & FL_NOSWAPMATCH) && get_match(COL_SOURCE) &&
...
static int poll_table(struct libmnt_table* tb, const char* tabfile, int timeout,
                      struct libscols_table* table, int direction)
{
    FILE* f = NULL;
    int rc = -1;
...
static int uniq_fs_target_cmp(struct libmnt_table* tb
                              __attribute__((__unused__)),
                              struct libmnt_fs* a, struct libmnt_fs* b)
{
...
static void __attribute__((__noreturn__)) usage(void)
{
    ...
    fprintf(out,
            _(" %1$s [options]\n"
              ...
              " %1$s [options] <device> <mountpoint>\n"
              " %1$s [options] [--source <device>] [--target <path> | "
              "--mountpoint <dir>]\n"),
    ...
    fputs(_(" -k, --kernel           search in kernel table of mounted\n"
          ...
          out);
    fputc('\n', out);
    fputs(_(" -p, --poll[=<list>]    monitor changes in table of mounted "
            "filesystems\n"),
          out);
    fputs(_(" -w, --timeout <num>    upper limit in milliseconds that --poll "
            "will block\n"),
          out);
    fputc('\n', out);

    fputs(_(" -A, --all              disable all built-in filters, print all "
            "filesystems\n"),
          out);
    fputs(_(" -a, --ascii            use ASCII chars for tree formatting\n"),
          out);
    fputs(_(" -b, --bytes            print sizes in bytes rather than in human "
            "readable format\n"),
          out);
    fputs(
        _(" -C, --nocanonicalize   don't canonicalize when comparing paths\n"),
        out);
    fputs(_(" -c, --canonicalize     canonicalize printed paths\n"), out);
    fputs(_(" -D, --df               imitate the output of df(1)\n"), out);
    fputs(_(" -d, --direction <word> direction of search, 'forward' or "
            "'backward'\n"),
          out);
    fputs(_(" -e, --evaluate         convert tags "
            "(LABEL,UUID,PARTUUID,PARTLABEL) \n"
            "                          to device names\n"),
          out);
    fputs(_(" -F, --tab-file <path>  alternative file for -s, -m or -k "
            "options\n"),
          out);
    ...
    fputs(_(" -N, --task <tid>       use alternative namespace "
            "(/proc/<tid>/mountinfo file)\n"),
          out);
    fputs(_(" -n, --noheadings       don't print column headings\n"), out);
    fputs(_(" -O, --options <list>   limit the set of filesystems by mount "
            "options\n"),
          out);
    ...
    fputs(_(" -R, --submounts        print all submounts for the matching "
            "filesystems\n"),
          out);
    fputs(_(" -r, --raw              use raw output format\n"), out);
    fputs(_("     --real             print only real filesystems\n"), out);
    fputs(
        _(" -S, --source <string>  the device to mount (by name, maj:min, \n"
          "                          LABEL=, UUID=, PARTUUID=, PARTLABEL=)\n"),
        out);
    ...
    fputs(
        _(" -t, --types <list>     limit the set of filesystems by FS types\n"),
        out);
    fputs(
        _(" -U, --uniq             ignore filesystems with duplicate target\n"),
        out);
    fputs(_(" -u, --notruncate       don't truncate text in columns\n"), out);
    fputs(_(" -v, --nofsroot         don't print [/dir] for bind or btrfs "
            "mounts\n"),
          out);

    fputc('\n', out);
    fputs(_(" -x, --verify           verify mount table content (default is "
            "fstab)\n"),
          out);
...
int main(int argc, char* argv[])
{
        ...
        switch (c)
        {
            ...
            case 'd':
                if (!strcmp(optarg, "forward"))
                    ...
                    errx(EXIT_FAILURE, _("unknown direction '%s'"), optarg);
                break;
            case 'e':
                flags |= FL_EVALUATE;
                break;
            ...
            case 'p':
                ...
                {
                    nactions =
                        string_to_idarray(optarg, actions, ARRAY_SIZE(actions),
            ...
            case 'N':
                tabtype = TABTYPE_KERNEL;
                tabfiles = append_pid_tabfile(
                    tabfiles, &ntabfiles,
                    strtou32_or_err(optarg, _("invalid TID argument")));
                break;
            case 'v':
                flags |= FL_NOFSROOT;
                break;
            ...
            case 'w':
                timeout =
                    strtos32_or_err(optarg, _("invalid timeout argument"));
                break;
            case 'V':
    ...
    if ((flags & FL_POLL) && ntabfiles > 1)
        errx(EXIT_FAILURE, _("--poll accepts only one file, but more specified "
                             "by --tab-file"));

    if (optind < argc && (get_match(COL_SOURCE) || get_match(COL_TARGET)))
        errx(EXIT_FAILURE,
             _("options --target and --source can't be used together "
    ...
    if (!(flags & FL_SUBMOUNTS) &&
        ((flags & FL_FIRSTONLY) || get_match(COL_TARGET) ||
         get_match(COL_SOURCE) || get_match(COL_MAJMIN)))
        flags &= ~FL_TREE;

    if (!(flags & FL_NOSWAPMATCH) && !get_match(COL_TARGET) &&
        get_match(COL_SOURCE))
    {
        /*
    ...
    if (flags & FL_POLL)
    {
        ...
        rc = poll_table(tb, tabfiles ? *tabfiles : _PATH_PROC_MOUNTINFO,
                        timeout, table, direction);
    }
    ...
    else
    {
        ...
        if (rc != 0 && tabtype == TABTYPE_KERNEL && (flags & FL_NOSWAPMATCH) &&
            !(flags & FL_STRICTTARGET) && get_match(COL_TARGET))
        {
            /*
