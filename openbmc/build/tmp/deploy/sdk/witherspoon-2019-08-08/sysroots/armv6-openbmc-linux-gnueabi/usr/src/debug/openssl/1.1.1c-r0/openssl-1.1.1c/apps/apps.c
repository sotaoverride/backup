...
static int load_pkcs12(BIO* in, const char* desc, pem_password_cb* pem_cb,
                       void* cb_data, EVP_PKEY** pkey, X509** cert,
                       STACK_OF(X509) * *ca)
{
    const char* pass;
                   ...
                   const char* pass, ENGINE* e, const char* key_descrip)
{
    ...
    {
        if (!load_pkcs12(key, key_descrip, (pem_password_cb*)password_callback,
...
static int load_certs_crls(const char* file, int format, const char* pass,
                           ...
                           STACK_OF(X509_CRL) * *pcrls)
{
    ...
    xis = PEM_X509_INFO_read_bio(bio, NULL, (pem_password_cb*)password_callback,
                                 &cb_data);
...
void print_bignum_var(BIO* out, const BIGNUM* in, const char* var, int len,
...
#undef BSIZE
#define BSIZE 256
    BIGNUM* load_serial(const char* serialfile, int create,
                        ASN1_INTEGER** retai)
{
    BIO* in = NULL;
    BIGNUM* ret = NULL;
    ...
    while (*cp)
    {
        ...
        *bp++ = '\0';
        ...
        for (; *cp && *cp != '/'; *bp++ = *cp++)
        {
            ...
            if (*cp == '\\' && *++cp == '\0')
            {
                BIO_printf(bio_err, "%s: escape character at end of string\n",
                           opt_getprog());
                goto err;
            }
        }
        *bp++ = '\0';
        ...
        if (!X509_NAME_add_entry_by_NID(n, nid, chtype, valstr,
                                        strlen((char*)valstr), -1,
                                        ismulti ? -1 : 0))
            goto err;
    }
...
{
    BIO* b =
        BIO_new_fp(stdout, BIO_NOCLOSE | (istext(format) ? BIO_FP_TEXT : 0));
    void* prefix = NULL;
...
{
    BIO* b =
        BIO_new_fp(stderr, BIO_NOCLOSE | (istext(format) ? BIO_FP_TEXT : 0));
#ifdef OPENSSL_SYS_VMS
    if (istext(format))
        b = BIO_push(BIO_new(BIO_f_linebuffer()), b);
#endif
                              ...
                              int quiet)
{
    ...
    if (filename == NULL || strcmp(filename, "-") == 0)
    {
        ...
        if (ret != NULL)
            return ret;
        BIO_printf(bio_err, "Can't open %s, %s\n",
    ...
    else
    {
        ...
        if (ret != NULL)
            return ret;
        BIO_printf(bio_err, "Can't open %s for %s, %s\n", filename,
                   modeverb(mode), strerror(errno));
    }
    ERR_print_errors(bio_err);
    return NULL;
}
