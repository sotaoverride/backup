...
struct basic_ptree<K, D, C>::subs
{
    ...
    typedef multi_index_container<
        value_type, multi_index::indexed_by<
                        multi_index::sequenced<>,
                        multi_index::ordered_non_unique<
                            multi_index::tag<by_name>,
#if defined(BOOST_PROPERTY_TREE_PAIR_BUG)
                            multi_index::member_offset<
                                value_type, const key_type, first_offset>,
#else
    ...
    typedef
        typename base_container::template index<by_name>::type by_name_index;
          ...
          const_iterator, typename subs::base_container::const_iterator>
{
  public:
    ...
    const_iterator(iterator b) : const_iterator::iterator_adaptor_(b.base())
    {
    }
};
template <class K, class D, class C>
    ...
    : public boost::reverse_iterator<iterator>
{
  public:
    ...
    explicit reverse_iterator(iterator b) : boost::reverse_iterator<iterator>(b)
    {
    }
};
template <class K, class D, class C>
...
class basic_ptree<K, D, C>::assoc_iterator
    : public boost::iterator_adaptor<
          assoc_iterator, typename subs::by_name_index::iterator, value_type>
{
    friend class boost::iterator_core_access;
    typedef boost::iterator_adaptor<
        assoc_iterator, typename subs::by_name_index::iterator, value_type>
...
class basic_ptree<K, D, C>::const_assoc_iterator
    : public boost::iterator_adaptor<
          const_assoc_iterator, typename subs::by_name_index::const_iterator>
{
  public:
...
template <class K, class D, class C>
basic_ptree<K, D, C>& basic_ptree<K, D, C>::
    operator=(const basic_ptree<K, D, C>& rhs)
{
...
template <class K, class D, class C>
inline typename basic_ptree<K, D, C>::size_type
...
template <class K, class D, class C>
inline typename basic_ptree<K, D, C>::reverse_iterator
...
template <class K, class D, class C>
inline typename basic_ptree<K, D, C>::const_reverse_iterator
...
template <class K, class D, class C>
inline const typename basic_ptree<K, D, C>::value_type&
...
struct equal_pred
{
    ...
    {
        C c;
        return !c(lhs.first, rhs.first) && !c(rhs.first, lhs.first) &&
...
template <class K, class D, class C>
inline bool basic_ptree<K, D, C>::
    operator==(const basic_ptree<K, D, C>& rhs) const
{
...
template <class K, class D, class C>
inline typename basic_ptree<K, D, C>::assoc_iterator
...
template <class K, class D, class C>
inline typename basic_ptree<K, D, C>::const_assoc_iterator
...
template <class K, class D, class C>
inline std::pair<typename basic_ptree<K, D, C>::assoc_iterator,
    ...
    basic_ptree<K, D, C>::equal_range(const key_type& key)
{
    std::pair<typename subs::by_name_index::iterator,
        ...
        r(subs::assoc(this).equal_range(key));
    return std::pair<assoc_iterator, assoc_iterator>(assoc_iterator(r.first),
                                                     assoc_iterator(r.second));
}

template <class K, class D, class C>
inline std::pair<typename basic_ptree<K, D, C>::const_assoc_iterator,
    ...
    basic_ptree<K, D, C>::to_iterator(const_assoc_iterator ai) const
{
    return const_iterator(
        subs::ch(this).BOOST_NESTED_TEMPLATE project<0>(ai.base()));
}
...
template <class K, class D, class C>
inline const typename basic_ptree<K, D, C>::data_type&
...
{
    return const_cast<self_type*>(this)->get_child(
        path, const_cast<self_type&>(default_value));
}
...
template <class K, class D, class C>
basic_ptree<K, D, C>& basic_ptree<K, D, C>::put_child(const path_type& path,
...
template <class K, class D, class C>
basic_ptree<K, D, C>& basic_ptree<K, D, C>::add_child(const path_type& path,
                                                      const self_type& value)
{
    path_type p(path);
    self_type& parent = force_path(p);
    ...
    basic_ptree<K, D, C>::get_value(Translator tr) const
{
    ...
    BOOST_PROPERTY_TREE_THROW(
        ptree_bad_data(std::string("conversion of data to type \"") +
...
template <class K, class D, class C>
template <class Type, class Translator>
inline Type basic_ptree<K, D, C>::get_value(const Type& default_value,
                                            Translator tr) const
{
...
typename boost::enable_if<detail::is_character<Ch>, std::basic_string<Ch>>::type
    basic_ptree<K, D, C>::get_value(const Ch* default_value,
                                    Translator tr) const
{
...
inline optional<Type>
    basic_ptree<K, D, C>::get_value_optional(Translator tr) const
{
...
template <class K, class D, class C>
template <class Type, class Translator>
inline Type basic_ptree<K, D, C>::get(const path_type& path,
...
template <class Ch, class Translator>
typename boost::enable_if<detail::is_character<Ch>, std::basic_string<Ch>>::type
    basic_ptree<K, D, C>::get(const path_type& path, const Ch* default_value,
                              Translator tr) const
{
...
void basic_ptree<K, D, C>::put_value(const Type& value, Translator tr)
{
    ...
    {
        BOOST_PROPERTY_TREE_THROW(
            ptree_bad_data(std::string("conversion of type \"") +
                               typeid(Type).name() + "\" to data failed",
                           boost::any()));
    }
}
...
basic_ptree<K, D, C>& basic_ptree<K, D, C>::put(const path_type& path,
                                                const Type& value,
                                                Translator tr)
{
...
template <class K, class D, class C>
template <class Type>
inline basic_ptree<K, D, C>& basic_ptree<K, D, C>::put(const path_type& path,
                                                       const Type& value)
{
...
template <class K, class D, class C>
template <class Type, typename Translator>
inline basic_ptree<K, D, C>& basic_ptree<K, D, C>::add(const path_type& path,
                                                       const Type& value,
                                                       Translator tr)
{
...
basic_ptree<K, D, C>& basic_ptree<K, D, C>::force_path(path_type& p)
{
    ...
    // create a new one.
    self_type& child =
        el == not_found() ? push_back(value_type(fragment, self_type()))->second
                          : el->second;
