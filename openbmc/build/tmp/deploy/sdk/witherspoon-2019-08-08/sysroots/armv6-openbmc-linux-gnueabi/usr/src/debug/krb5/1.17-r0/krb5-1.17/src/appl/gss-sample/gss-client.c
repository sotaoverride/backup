 ...
 */
static int connect_to_server(char* host, u_short port)
{
 ...
 * and -1 is returned.
 */
static int client_establish_context(int s, char* service_name,
                                    OM_uint32 gss_flags, int auth_flag,
                                    int v1_format, gss_OID oid, char* username,
                                    char* password, gss_ctx_id_t* gss_context,
                                    OM_uint32* ret_flags)
{
    if (auth_flag)
    {
        ...
        if (username != NULL)
        {
            ...
            maj_stat = gss_import_name(
                &min_stat, &send_tok, (gss_OID)gss_nt_user_name, &gss_username);
        ...
        if (password != NULL)
        {
            ...
            maj_stat = gss_acquire_cred_with_password(
                &min_stat, gss_username, &pwbuf, 0, mechsp, GSS_C_INITIATE,
                &cred, NULL, NULL);
        }
        ...
        *gss_context = GSS_C_NO_CONTEXT;
        ...
        do
        {
            maj_stat = gss_init_sec_context(
                &init_sec_min_stat, cred, gss_context, target_name,
                mechs.elements, gss_flags, 0, NULL, /* channel bindings */
                token_ptr, NULL,                    /* mech type */
                &send_tok, ret_flags, NULL);        /* time_rec */
...
static void read_file(file_name, in_buf) char* file_name;
gss_buffer_t in_buf;
{
    int fd, count;
...
static int call_server(host, port, oid, service_name, gss_flags, auth_flag,
                       ...
                       use_file, mcount, username, password) char* host;
u_short port;
gss_OID oid;
char* service_name;
OM_uint32 gss_flags;
...
char* password;
{
    ...
    if (client_establish_context(s, service_name, gss_flags, auth_flag,
                                 v1_format, oid, username, password, &context,
                                 &ret_flags) < 0)
    {
        (void)closesocket(s);
        return -1;
    ...
    if (auth_flag && verbose)
    {
        ...
        maj_stat =
            gss_display_name(&min_stat, targ_name, &tname, (gss_OID*)NULL);
        if (maj_stat != GSS_S_COMPLETE)
        {
        ...
        maj_stat =
            gss_inquire_names_for_mech(&min_stat, mechanism, &mech_names);
        if (maj_stat != GSS_S_COMPLETE)
        {
        ...
        {
            maj_stat =
                gss_oid_to_str(&min_stat, &mech_names->elements[i], &oid_name);
            if (maj_stat != GSS_S_COMPLETE)
            {
                display_status("converting oid->string", maj_stat, min_stat);
                return -1;
    ...
    for (i = 0; i < (size_t)mcount; i++)
    {
        ...
        if (send_token(s,
                       (v1_format
                            ? 0
                            : (TOKEN_DATA | (wrap_flag ? TOKEN_WRAPPED : 0) |
...
BOOL WaitAndIncrementThreadCounter(void)
{
    for (;;)
    {
...
BOOL DecrementAndSignalThreadCounter(void)
{
    if (WaitForSingleObject(hMutex, INFINITE) == WAIT_OBJECT_0)
    {
        if (thread_count == max_threads)
