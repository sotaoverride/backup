                                   ...
                                   size_t cpusetsize, cpu_set_t* cpuset)
{
    ...
    if (iattr->cpuset != NULL)
    {
       ...
       the user requested.  */
        ...
        /* Copy over the cpuset from the thread attribute object.  Limit the
       copy to the minimum of the source and destination sizes to prevent a
       buffer overrun.  If the destination is larger, fill the remaining space
       with zeroes.  */
        void* p =
            mempcpy(cpuset, iattr->cpuset, MIN(iattr->cpusetsize, cpusetsize));
...
int __pthread_attr_getaffinity_old(const pthread_attr_t* attr,
                                   cpu_set_t* cpuset)
{
