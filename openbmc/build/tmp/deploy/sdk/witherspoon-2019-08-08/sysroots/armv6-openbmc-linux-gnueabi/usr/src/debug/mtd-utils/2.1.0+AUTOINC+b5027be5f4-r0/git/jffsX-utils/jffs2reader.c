                     ...
                     struct jffs2_raw_inode* n)
{
    ...
    if (je32_to_cpu(n->isize) > bsize ||
        (je32_to_cpu(n->offset) + dlen) > bsize)
...
static const mode_t SBIT[] = {0, 0, S_ISUID, 0, 0, S_ISGID, 0, 0, S_ISVTX};

/* The 9 mode bits to test */
static const mode_t MBIT[] = {S_IRUSR, S_IWUSR, S_IXUSR, S_IRGRP, S_IWGRP,
                     ...
                     int recurse, int want_ctime)
{
    ...
    while (d != NULL)
    {
        ...
        mode.v32 = ri->mode.m;
        printf("%s %-4d %-8d %-8d ", mode_string(je32_to_cpu(mode)), 1,
               je16_to_cpu(ri->uid), je16_to_cpu(ri->gid));
    ...
    do
    {
        ...
        if (n < e && je16_to_cpu(n->u.magic) == JFFS2_MAGIC_BITMASK)
        {
            if (je16_to_cpu(n->u.nodetype) == JFFS2_NODETYPE_INODE &&
                ...
                (v = je32_to_cpu(n->i.version)) > vcur)
            {
                /* XXX crc check */
            ...
            n = (union jffs2_node_union*)
                o; /* we're at the end, rewind to the beginning */
    ...
    do
    {
        ...
        if (n < e && je16_to_cpu(n->u.magic) == JFFS2_MAGIC_BITMASK)
        {
            if (je16_to_cpu(n->u.nodetype) == JFFS2_NODETYPE_DIRENT &&
                je32_to_cpu(n->d.pino) == ino &&
                (v = je32_to_cpu(n->d.version)) > vcur)
            {
                /* XXX crc check */
    ...
    do
    {
        ...
        if (n < e && je16_to_cpu(n->u.magic) == JFFS2_MAGIC_BITMASK)
        {
            if (je16_to_cpu(n->u.nodetype) == JFFS2_NODETYPE_DIRENT &&
                ...
                (!pino ||
                 (je32_to_cpu(n->d.pino) == pino && nsize == n->d.nsize &&
                                             ...
                                             int recc)
{
    ...
    while (ino && next != NULL && next[1] != 0 && d)
    {
        ...
        if (DIRENT_INO(dir) == 0 ||
            (next != NULL && !(dir->type == DT_DIR || dir->type == DT_LNK)))
        {
            free(pp);
    ...
    while ((opt = getopt_long(argc, argv, short_opt, long_opt, &c)) > 0)
    {
        switch (opt)
        {
            ...
            case 'V':
                ...
                exit(EXIT_SUCCESS);
            default:
                fprintf(stderr, "Usage: %s <image> [-d|-f] < path >\n",
                        PROGRAM_NAME);
