 ...
 *
 * Returns: (transfer full): A newly allocated #GBytes containing contents of
 * @string; @string itself is freed Since: 2.34
 */
...
GString* g_string_insert_unichar(GString* string, gssize pos, gunichar wc)
{
    ...
    /* If not just an append, move the old stuff */
    if (pos < string->len)
        memmove(string->str + pos + charlen, string->str + pos,
                string->len - pos);
