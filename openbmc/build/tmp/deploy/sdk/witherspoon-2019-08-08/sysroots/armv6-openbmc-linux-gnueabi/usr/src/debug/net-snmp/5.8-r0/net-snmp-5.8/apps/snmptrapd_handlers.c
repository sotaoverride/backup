...
const char* trap1_std_str = "%.4y-%.2m-%.2l %.2h:%.2j:%.2k %B [%b] (via %A "
                            "[%a]): %N\n\t%W Trap (%q) Uptime: %#T\n%v\n";
...
void snmptrapd_parse_traphandle(const char* token, char* line)
{
    char buf[STRINGMAX];
...
static void parse_forward(const char* token, char* line)
{
    char buf[STRINGMAX];
    oid obuf[MAX_OID_LEN];
    size_t olen = MAX_OID_LEN;
...
void snmptrapd_register_configs(void)
{
    register_config_handler(
        "snmptrapd", "traphandle", snmptrapd_parse_traphandle,
        snmptrapd_free_traphandle, "oid|\"default\" program [args ...] ");
    register_config_handler("snmptrapd", "format1", parse_trap1_fmt,
                            free_trap1_fmt, "format");
    register_config_handler("snmptrapd", "format2", parse_trap2_fmt,
                            free_trap2_fmt, "format");
    register_config_handler("snmptrapd", "format", parse_format, NULL,
                            "[print{,1,2}|syslog{,1,2}|execute{,1,2}] format");
    register_config_handler("snmptrapd", "forward", parse_forward, NULL,
                            "OID|\"default\" destination");
}
                                               ...
                                               oid* trapOid, int trapOidLen)
{
     ...
     */
    for (traph2 = netsnmp_specific_traphandlers; traph2; traph2 = traph2->nextt)
    {
        /* XXX - check this! */
        if (snmp_oid_compare(traph2->trapoid, traph2->trapoid_len, trapOid,
                             trapOidLen) <= 0)
            break;
    }
    if (traph2)
...
netsnmp_trapd_handler* netsnmp_get_traphandler(oid* trapOid, int trapOidLen)
{
    ...
    for (traph = netsnmp_specific_traphandlers; traph; traph = traph->nextt)
    {
        ...
        {
            if (snmp_oid_compare(traph->trapoid, traph->trapoid_len, trapOid,
                                 trapOidLen) == 0)
            {
        ...
        else
        {
             ...
             */
            if (snmp_oidsubtree_compare(traph->trapoid, traph->trapoid_len,
...
#define SYSLOG_V1_ENTERPRISE_FORMAT                                            \
    "%a: %W Trap (%q) Uptime: %#T%#v\n" /* XXX - (%q) become (.N) ??? */
#define SYSLOG_V23_NOTIFICATION_FORMAT                                         \
    "%B [%b]: Trap %#v\n" /* XXX - introduces a leading " ," */

/*
 *  Trap handler for logging via syslog
 */
int syslog_handler(netsnmp_pdu* pdu, netsnmp_transport* transport,
                   netsnmp_trapd_handler* handler)
{
    u_char* rbuf = NULL;
    ...
    if (handler && handler->format)
    {
        ...
        {
            free(rbuf);
            return NETSNMPTRAPD_HANDLER_OK; /* A 0-length format string means
                                               don't log */
        }
    ...
    else
    {
        if (pdu->command == SNMP_MSG_TRAP)
        {
            ...
            else if (pdu->trap_type == SNMP_TRAP_ENTERPRISESPECIFIC)
            {
                ...
                trunc = !realloc_format_trap(&rbuf, &r_len, &o_len, 1,
                                             SYSLOG_V1_ENTERPRISE_FORMAT, pdu,
                                             transport);
            }
            else
            {
                ...
                trunc = !realloc_format_trap(&rbuf, &r_len, &o_len, 1,
                                             SYSLOG_V1_STANDARD_FORMAT, pdu,
                                             transport);
            }
        }
        else
        { /* SNMPv2/3 notifications */
            if (syslog_format2)
            {
                DEBUGMSGTL(
                    ("snmptrapd", "syslog_format v1 = '%s'\n", syslog_format2));
 ...
 */
int print_handler(netsnmp_pdu* pdu, netsnmp_transport* transport,
                  netsnmp_trapd_handler* handler)
{
    u_char* rbuf = NULL;
    size_t r_len = 64, o_len = 0;
    ...
    else
    {
        ...
        else
        {
            ...
            else
            {
                DEBUGMSGTL(("snmptrapd", "v2/3 format\n"));
                trunc = !realloc_format_trap(&rbuf, &r_len, &o_len, 1,
                                             PRINT_V23_NOTIFICATION_FORMAT, pdu,
                                             transport);
            }
        }
    }
 ...
 */
int command_handler(netsnmp_pdu* pdu, netsnmp_transport* transport,
                    netsnmp_trapd_handler* handler)
{
#ifndef USING_UTILITIES_EXECUTE_MODULE
    NETSNMP_LOGONCE(
        (LOG_WARNING, "support for run_shell_command not available\n"));
    ...
    if (handler && handler->token && *handler->token)
    {
        ...
        {
            DEBUGMSGTL(("snmptrapd", "format = '%s'\n", handler->format));
            realloc_format_trap(&rbuf, &r_len, &o_len, 1, handler->format,
                                v2_pdu, transport);
        }
        else
        {
            if (pdu->command == SNMP_MSG_TRAP && exec_format1)
            {
                ...
                realloc_format_trap(&rbuf, &r_len, &o_len, 1, exec_format1, pdu,
                                    transport);
            }
            else if (pdu->command != SNMP_MSG_TRAP && exec_format2)
            {
                DEBUGMSGTL(("snmptrapd", "exec v2/3 = '%s'\n", exec_format2));
                realloc_format_trap(&rbuf, &r_len, &o_len, 1, exec_format2, pdu,
                                    transport);
            }
            else
            {
         ...
         */
        run_shell_command(handler->token, (char*)rbuf, NULL,
                          NULL); /* Not interested in output */
        netsnmp_ds_set_boolean(NETSNMP_DS_LIBRARY_ID,
 ...
 */
int axforward_handler(netsnmp_pdu* pdu, netsnmp_transport* transport,
                      netsnmp_trapd_handler* handler)
{
 ...
 */
int forward_handler(netsnmp_pdu* pdu, netsnmp_transport* transport,
                    netsnmp_trapd_handler* handler)
{
    ...
    if (!snmp_send(ss, pdu2) && ss->s_snmp_errno != SNMPERR_SUCCESS)
    {
...
#if defined(USING_NOTIFICATION_LOG_MIB_NOTIFICATION_LOG_MODULE) &&             \
    defined(USING_AGENTX_SUBAGENT_MODULE) &&                                   \
    !defined(NETSNMP_SNMPTRAPD_DISABLE_AGENTX)
/*
 *  "Notification" handler for implementing NOTIFICATION-MIB
 *  		(presumably)
 */
int notification_handler(netsnmp_pdu* pdu, netsnmp_transport* transport,
                         netsnmp_trapd_handler* handler)
{
...
int snmp_input(int op, netsnmp_session* session, int reqid, netsnmp_pdu* pdu,
               void* magic)
{
