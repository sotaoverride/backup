...
void address_label_free(AddressLabel* label)
{
    ...
    if (label->network)
    {
        ...
        if (label->section)
        {
            hashmap_remove(label->network->address_labels_by_section,
                           label->section);
...
static int address_label_new_static(Network* network, const char* filename,
                                    unsigned section_line, AddressLabel** ret)
{
    ...
    *label = (AddressLabel){
    ...
    if (filename)
    {
        ...
        r = hashmap_ensure_allocated(&network->address_labels_by_section,
                                     &network_config_hash_ops);
        if (r < 0)
            return r;

        r = hashmap_put(network->address_labels_by_section, label->section,
                        label);
        if (r < 0)
            return r;
    }
...
static int address_label_handler(sd_netlink* rtnl, sd_netlink_message* m,
                                 Link* link)
{
    int r;
...
int address_label_configure(AddressLabel* label, Link* link,
                            ...
                            bool update)
{
    ...
    r = sd_rtnl_message_new_addrlabel(
        link->manager->rtnl, &req, RTM_NEWADDRLABEL, link->ifindex, AF_INET6);
    if (r < 0)
    ...
    r = sd_netlink_message_append_in6_addr(req, IFA_ADDRESS,
                                           &label->in_addr.in6);
    if (r < 0)
...
int config_parse_address_label_prefix(const char* unit, const char* filename,
                                      ...
                                      void* userdata)
{
    ...
    r = in_addr_prefix_from_string(rvalue, AF_INET6, &n->in_addr,
                                   &n->prefixlen);
    if (r < 0)
    {
...
int config_parse_address_label(const char* unit, const char* filename,
                               unsigned line, const char* section,
                               unsigned section_line, const char* lvalue,
                               int ltype, const char* rvalue, void* data,
                               void* userdata)
