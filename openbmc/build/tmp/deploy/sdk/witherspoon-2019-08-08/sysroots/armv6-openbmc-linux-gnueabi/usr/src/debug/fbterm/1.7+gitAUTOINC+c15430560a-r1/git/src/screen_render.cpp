    ...
    for (u32 i = 0; i < NR_COLORS; i++)
    {
        switch (mBitsPerPixel)
        {
            case 8:
                ...
                break;
            case 15:
                fillColors[i] = ((palette[i].red >> 3) << 10) |
                                ((palette[i].green >> 3) << 5) |
                                (palette[i].blue >> 3);
                fillColors[i] |= fillColors[i] << 16;
                break;
            case 16:
                fillColors[i] = ((palette[i].red >> 3) << 11) |
                                ((palette[i].green >> 2) << 5) |
                                (palette[i].blue >> 3);
                fillColors[i] |= fillColors[i] << 16;
                break;
            case 32:
                fillColors[i] = (palette[i].red << 16) |
...
void Screen::initFillDraw()
{
    if (mBitsPerPixel == 15)
        ...
        bytes_per_pixel = (mBitsPerPixel >> 3);
    ...
    if (getenv("FBTERM_BACKGROUND_IMAGE"))
    {
        ...
        u32 size =
            mBytesPerLine *
            ((mRotateType == Rotate0 || mRotateType == Rotate180) ? mHeight
                                                                  : mWidth);
...
#define drawX(bits, lred, lgreen, lblue, type, fbwrite)                        \
    ...
    {                                                                          \
        ...
        type color;                                                            \
        type* dst =                                                            \
            (type*)(mVMemBase + y * mBytesPerLine + x * bytes_per_pixel);      \
                                                                               \
        for (; w--; pixmap++, dst++)                                           \
        {                                                                      \
            pixel = *pixmap;                                                   \
                                                                               \
            if (!pixel)                                                        \
                fbwrite(dst, fillColors[bc]);                                  \
            else if (pixel == 0xff)                                            \
                fbwrite(dst, fillColors[fc]);                                  \
            else                                                               \
            {                                                                  \
                red = mPalette[bc].red +                                       \
                      (((mPalette[fc].red - mPalette[bc].red) * pixel) >> 8);  \
                green =                                                        \
                    mPalette[bc].green +                                       \
                    (((mPalette[fc].green - mPalette[bc].green) * pixel) >>    \
                     8);                                                       \
                blue =                                                         \
                    mPalette[bc].blue +                                        \
                    (((mPalette[fc].blue - mPalette[bc].blue) * pixel) >> 8);  \
                                                                               \
                color = ((red >> (8 - lred) << (lgreen + lblue)) |             \
                         (green >> (8 - lgreen) << lblue) |                    \
                         (blue >> (8 - lblue)));                               \
                fbwrite(dst, color);                                           \
            }                                                                  \
        }                                                                      \
...
drawX(15, 5, 5, 5, u16, writew) drawX(16, 5, 6, 5, u16, writew)
    ...
    {                                                                          \
        ...
        {                                                                      \
            ...
            {                                                                  \
                color = *bgimg;                                                \
                                                                               \
                redbg = ((color >> (lgreen + lblue)) & ((1 << lred) - 1))      \
                        << (8 - lred);                                         \
                greenbg = ((color >> lblue) & ((1 << lgreen) - 1))             \
                          << (8 - lgreen);                                     \
                bluebg = (color & ((1 << lblue) - 1)) << (8 - lblue);          \
                                                                               \
                red = redbg + (((mPalette[fc].red - redbg) * pixel) >> 8);     \
                green =                                                        \
                    greenbg + (((mPalette[fc].green - greenbg) * pixel) >> 8); \
        ...
        drawXBg(15, 5, 5, 5, u16, writew) drawXBg(16, 5, 6, 5, u16, writew)
