...
static int create_homedir(const struct passwd* pwd, const char* source,
                          const char* dest)
{
    char remark[BUFSIZ];
    DIR* d;
    ...
    for (dent = readdir(d); dent != NULL; dent = readdir(d))
    {
        ...
        {
            pam_syslog(NULL, LOG_DEBUG, "unable to open dest file %s: %m",
                       newdest);
            close(srcfd);
            closedir(d);
       ...
       execute bits */
        ...
        {
            pam_syslog(NULL, LOG_DEBUG, "unable to change perms on copy %s: %m",
                       newdest);
            close(srcfd);
            close(destfd);
            closedir(d);
...
static int make_parent_dirs(char* dir, int make)
{
    int rc = PAM_SUCCESS;
    char* cp = strrchr(dir, '/');
    struct stat st;
