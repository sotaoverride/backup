...
#define _dbus_return_if_fail(condition)                                        \
    ...
    {                                                                          \
        ...
        if (!(condition))                                                      \
        {                                                                      \
            _dbus_warn_return_if_fail(_DBUS_FUNCTION_NAME, #condition,         \
                                      __FILE__, __LINE__);                     \
            return;                                                            \
        }                                                                      \
    } while (0)
...
#ifdef DBUS_DISABLE_CHECKS
/* this is an assert and not an error, but in the typical --disable-checks case
 * (you're trying to really minimize code size), disabling these assertions
 * makes sense.
 */
...
#define _dbus_return_if_error_is_set(error)                                    \
    _dbus_return_if_fail((error) == NULL || !dbus_error_is_set((error)))
#define _dbus_return_val_if_error_is_set(error, val)                           \
    _dbus_return_val_if_fail((error) == NULL || !dbus_error_is_set((error)),   \
                             (val))
...
#define _DBUS_ALIGN_VALUE(this, boundary)                                      \
    ((((uintptr_t)(this)) + (((uintptr_t)(boundary)) - 1)) &                   \
...
#define _DBUS_SET_OOM(error)                                                   \
    dbus_set_error_const((error), DBUS_ERROR_NO_MEMORY, _dbus_no_memory_message)
DBUS_PRIVATE_EXPORT
void _dbus_set_error_valist(DBusError* error, const char* name,
...
void _dbus_set_bad_address(DBusError* error, const char* address_problem_type,
...
{
    dbus_uint32_t
        as_uint32s[DBUS_UUID_LENGTH_WORDS]; /**< guid as four uint32 values */
    char as_bytes[DBUS_UUID_LENGTH_BYTES]; /**< guid as 16 single-byte values */
};

DBUS_PRIVATE_EXPORT _DBUS_GNUC_WARN_UNUSED_RESULT dbus_bool_t
    _dbus_generate_uuid(DBusGUID* uuid, DBusError* error);
DBUS_PRIVATE_EXPORT
