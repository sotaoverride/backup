...
#define ERROR_OUT_WRITEERROR(fp) ERROR_OUT_READERROR(fp)
                                 ...
                                 int* h_errnop, int af, int retry)
{
    ...
    {
        retv =
            read_one_hostent(fp, result, buffer, buflen, errnop, h_errnop, af);
        /* check result */
...
{
    NSS_GETONE(
        NSLCD_ACTION_HOST_BYNAME, WRITE_STRING(fp, name),
...
{
    NSS_GETONE(
        NSLCD_ACTION_HOST_BYADDR, WRITE_ADDRESS(fp, af, len, addr),
        read_hostent(fp, result, buffer, buflen, errnop, h_errnop, af, 0));
}
...
static char* hostent2str(struct hostent* result, char* buffer, size_t buflen)
{
    ...
    if (result->h_addr_list != NULL)
    {
        for (i = 0; result->h_addr_list[i]; i++)
        {
            ...
            snprintf(buffer, buflen - strlen(buffer) - 1, "%s %s",
...
static nss_backend_op_t hosts_ops[] = {
    nss_ldap_destructor, hosts_endhostent,    hosts_sethostent,
    hosts_gethostent,    hosts_gethostbyname, hosts_gethostbyaddr};
