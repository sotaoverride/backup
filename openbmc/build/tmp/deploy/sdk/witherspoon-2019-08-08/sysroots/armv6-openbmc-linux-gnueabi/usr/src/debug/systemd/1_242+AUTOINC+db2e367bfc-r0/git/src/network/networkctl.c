...
static void setup_state_to_color(const char* state, const char** on,
                                 const char** off)
{
    assert(on);
    assert(off);
...
static int decode_link(sd_netlink_message* m, LinkInfo* info, char** patterns)
{
    ...
    info->has_mac_address = sd_netlink_message_read_ether_addr(
                                m, IFLA_ADDRESS, &info->mac_address) >= 0 &&
                            memcmp(&info->mac_address, &ETHER_ADDR_NULL,
                                   sizeof(struct ether_addr)) != 0;

    info->has_mtu =
        sd_netlink_message_read_u32(m, IFLA_MTU, &info->mtu) && info->mtu > 0;
...
static int list_links(int argc, char* argv[], void* userdata)
{
    ...
    for (i = 0; i < c; i++)
    {
        ...
        (void)sd_network_link_get_operational_state(links[i].ifindex,
                                                    &operational_state);
        operational_state_to_color(operational_state, &on_color_operational,
                                   &off_color_operational);

        r = sd_network_link_get_setup_state(links[i].ifindex, &setup_state);
        if (r == -ENODATA) /* If there's no info available about this iface,
                              it's unmanaged by networkd */
        ...
        printf("%3i %-16s %-18s %s%-16s%s %s%-10s%s\n", links[i].ifindex,
               links[i].name, strna(t), on_color_operational,
               strna(operational_state), off_color_operational, on_color_setup,
               strna(setup_state), off_color_setup);
    }
...
static int get_gateway_description(sd_netlink* rtnl, sd_hwdb* hwdb, int ifindex,
                                   int family, union in_addr_union* gateway,
...
static int dump_gateways(sd_netlink* rtnl, sd_hwdb* hwdb, const char* prefix,
                         int ifindex)
{
    ...
    for (i = 0; i < n; i++)
    {
        ...
        r = get_gateway_description(rtnl, hwdb, local[i].ifindex,
                                    local[i].family, &local[i].address,
                                    &description);
        if (r < 0)
...
static int dump_address_labels(sd_netlink* rtnl)
{
    ...
    r = sd_rtnl_message_new_addrlabel(rtnl, &req, RTM_GETADDRLABEL, 0,
                                      AF_INET6);
    if (r < 0)
        return log_error_errno(
            r, "Could not allocate RTM_GETADDRLABEL message: %m");
...
static int next_lldp_neighbor(FILE* f, sd_lldp_neighbor** ret)
{
    ...
    /* each LLDP packet is at most MTU size, but let's allow up to 4KiB just in
     * case */
    if (le64toh(u) >= 4096)
        return -EBADMSG;
...
static int dump_lldp_neighbors(const char* prefix, int ifindex)
{
    ...
    for (;;)
    {
        const char *system_name = NULL, *port_id = NULL,
...
static int link_status_one(sd_netlink* rtnl, sd_hwdb* hwdb,
                           const LinkInfo* info)
{
    ...
    const char *driver = NULL, *path = NULL, *vendor = NULL, *model = NULL,
               *link = NULL;
    const char *on_color_operational, *off_color_operational, *on_color_setup,
        *off_color_setup;
    ...
    (void)sd_network_link_get_operational_state(info->ifindex,
                                                &operational_state);
    operational_state_to_color(operational_state, &on_color_operational,
                               &off_color_operational);

    r = sd_network_link_get_setup_state(info->ifindex, &setup_state);
    if (r == -ENODATA) /* If there's no info available about this iface, it's
    ...
    if (d)
    {
        ...
        if (sd_device_get_property_value(d, "ID_VENDOR_FROM_DATABASE",
                                         &vendor) < 0)
            (void)sd_device_get_property_value(d, "ID_VENDOR", &vendor);

        if (sd_device_get_property_value(d, "ID_MODEL_FROM_DATABASE", &model) <
            0)
    ...
    (void)sd_network_link_get_carrier_bound_to(info->ifindex,
                                               &carrier_bound_to);
    (void)sd_network_link_get_carrier_bound_by(info->ifindex,
                                               &carrier_bound_by);

    printf("%s%s%s %i: %s\n", on_color_operational,
           special_glyph(SPECIAL_GLYPH_BLACK_CIRCLE), off_color_operational,
    ...
    printf("       Link File: %s\n"
           ...
           "            Type: %s\n"
           "           State: %s%s%s (%s%s%s)\n",
           strna(link), strna(network), strna(t), on_color_operational,
           strna(operational_state), off_color_operational, on_color_setup,
           strna(setup_state), off_color_setup);
...
static int system_status(sd_netlink* rtnl, sd_hwdb* hwdb)
{
    ...
    printf("%s%s%s        State: %s%s%s\n", on_color_operational,
           special_glyph(SPECIAL_GLYPH_BLACK_CIRCLE), off_color_operational,
           on_color_operational, strna(operational_state),
           off_color_operational);
...
static int link_lldp_status(int argc, char* argv[], void* userdata)
{
    ...
    if (arg_legend)
        printf("%-16s %-17s %-16s %-11s %-17s %-16s\n", "LINK", "CHASSIS ID",
               "SYSTEM NAME", "CAPS", "PORT ID", "PORT DESCRIPTION");
    ...
    for (i = 0; i < c; i++)
    {
        ...
        {
            log_warning_errno(r,
                              "Failed to open LLDP data for %i, ignoring: %m",
                              links[i].ifindex);
            continue;
        }
        ...
        for (;;)
        {
            ...
            printf("%-16s %-17s %-16s %-11s %-17s %-16s\n", links[i].name,
                   strna(chassis_id), strna(system_name), strna(capabilities),
                   strna(port_id), strna(port_description));
...
static int help(void)
{
    ...
    printf("%s [OPTIONS...]\n\n"
           ...
           "  label                 Show current address label entries in the "
           "kernel\n"
...
static void warn_networkd_missing(void)
{
    ...
    fprintf(stderr, "WARNING: systemd-networkd is not running, output will be "
                    "incomplete.\n\n");
}
