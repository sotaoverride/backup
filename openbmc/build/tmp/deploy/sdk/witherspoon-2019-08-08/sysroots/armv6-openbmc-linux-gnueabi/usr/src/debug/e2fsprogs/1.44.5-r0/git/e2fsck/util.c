...
char* string_copy(e2fsck_t ctx EXT2FS_ATTR((unused)), const char* str, int len)
{
    char* ret;
...
static int read_a_char(void)
{
    ...
    while (1)
    {
        if (e2fsck_global_ctx && (e2fsck_global_ctx->flags & E2F_FLAG_CANCEL))
        {
            return 3;
        }
        r = read(0, &c, 1);
...
void e2fsck_read_bitmaps(e2fsck_t ctx)
{
    ...
    e2fsck_set_bitmap_type(fs, EXT2FS_BMAP64_RBTREE, "fs_bitmaps", &save_type);
    flags = ctx->fs->flags;
    ...
    if (retval)
    {
        com_err(ctx->program_name, retval,
                _("while retrying to read bitmaps for %s"), ctx->device_name);
        fatal_error(ctx, 0);
    }
}
...
static _INLINE_ float timeval_subtract(struct timeval* tv1, struct timeval* tv2)
{
...
#define mbytes(x) (((x) + 1048575) / 1048576)
    if (channel && channel->manager && channel->manager->get_stats)
    {
        ...
        if (delta)
        {
            ...
            bytes_written = delta->bytes_written - track->bytes_written;
        }
        log_out(ctx,
                       ...
                       struct ext2_inode* inode, const char* proc)
{
    ...
    if (retval)
    {
        com_err("ext2fs_read_inode", retval, _("while reading inode %lu in %s"),
                ino, proc);
        fatal_error(ctx, 0);
    }
}
                      ...
                      io_manager manager)
{
    ...
    {
        ret_sb =
            (fs->super->s_blocks_per_group + fs->super->s_first_data_block);
        if (ctx)
        {
         ...
         blocksize *= 2)
    {
        ...
        if (io_channel_read_blk64(io, superblock, -SUPERBLOCK_SIZE, buf))
            continue;
