...
static int netem_msg_parser(struct rtnl_tc* tc, void* data)
{
    ...
    netem->qnm_mask =
        (SCH_NETEM_ATTR_LATENCY | SCH_NETEM_ATTR_LIMIT | SCH_NETEM_ATTR_LOSS |
         SCH_NETEM_ATTR_GAP | SCH_NETEM_ATTR_DUPLICATE | SCH_NETEM_ATTR_JITTER);
    ...
    if (len > 0)
    {
        ...
        err = nla_parse(tb, TCA_NETEM_MAX,
                        (struct nlattr*)(tc->tc_opts->d_data + sizeof(*opts)),
                        len, netem_policy);
        if (err < 0)
        {
            free(netem);
        ...
        if (tb[TCA_NETEM_CORR])
        {
            ...
            netem->qnm_mask |=
                (SCH_NETEM_ATTR_DELAY_CORR | SCH_NETEM_ATTR_LOSS_CORR |
        ...
        if (tb[TCA_NETEM_REORDER])
        {
            ...
            netem->qnm_mask |=
                (SCH_NETEM_ATTR_RO_PROB | SCH_NETEM_ATTR_RO_CORR);
        }
        ...
        if (tb[TCA_NETEM_CORRUPT])
        {
            ...
            netem->qnm_mask |=
                (SCH_NETEM_ATTR_CORRUPT_PROB | SCH_NETEM_ATTR_CORRUPT_CORR);
        }
                               ...
                               struct nl_dump_params* p)
{
    ...
    if (netem)
    {
        ...
        {
            nl_msec2str(nl_ticks2us(netem->qnm_latency) / 1000, buf,
                        sizeof(buf));
            ...
            if (netem->qnm_mask & SCH_NETEM_ATTR_JITTER &&
                netem->qnm_jitter > 0)
            {
                nl_msec2str(nl_ticks2us(netem->qnm_jitter) / 1000, buf,
                            sizeof(buf));
                nl_dump(p, " jitter %s", buf);

                if (netem->qnm_mask & SCH_NETEM_ATTR_DELAY_CORR &&
                    netem->qnm_corr.nmc_delay > 0)
                              ...
                              struct nl_msg* msg)
{
    ...
    if (netem->qnm_dist.dist_data && netem->qnm_dist.dist_size)
    {
        ...
        else
        {
            ...
            int new_msg_len =
                msg->nm_size + netem->qnm_dist.dist_size *
                                      ...
                                      const char* dist_type)
{
    ...
    char* test_path[] = {
        "", "./", "/usr/lib/tc/", "/usr/lib64/tc/", "/usr/local/lib/tc/",
    };
    ...
    {
        snprintf(name, NAME_MAX, "%s%s%s", test_path[i], dist_type,
                 dist_suffix);
        if ((f = fopen(name, "re")))
            break;
    }
