...
{
    fprintf(stderr,
            "Usage: %s [--help] [--system | --session | --bus=ADDRESS | "
            "--peer=ADDRESS] [--dest=NAME] [--type=TYPE] "
            "[--print-reply[=literal]] [--reply-timeout=MSEC] <destination "
...
static void append_arg(DBusMessageIter* iter, int type, const char* value)
{
    ...
    switch (type)
    {
        ...
        case DBUS_TYPE_OBJECT_PATH:
            ret = dbus_message_iter_append_basic(iter, DBUS_TYPE_OBJECT_PATH,
                                                 &value);
            break;
        ...
        case DBUS_TYPE_BOOLEAN:
            ...
            {
                v_BOOLEAN = TRUE;
                ret = dbus_message_iter_append_basic(iter, DBUS_TYPE_BOOLEAN,
                                                     &v_BOOLEAN);
            }
            ...
            else
            {
                fprintf(
                    stderr,
                    "%s: Expected \"true\" or \"false\" instead of \"%s\"\n",
                    appname, value);
                exit(1);
            }
            break;
...
static void append_dict(DBusMessageIter* iter, int keytype, int valtype,
                        const char* value)
{
    const char* val;
    char* dupval = strdup(value);
    ...
    while (val != NULL)
    {
        ...
        handle_oom(dbus_message_iter_open_container(iter, DBUS_TYPE_DICT_ENTRY,
                                                    NULL, &subiter));
...
int main(int argc, char* argv[])
{
    ...
    for (i = 1; i < argc && name == NULL; i++)
    {
                 ...
                 (strstr(arg, "--address=") == arg))
        {
            ...
            {
                fprintf(stderr,
                        "\"--peer=\" and \"--bus=\" require an ADDRESS\n");
                usage(1);
            }
        }
    ...
    {
        fprintf(stderr, "\"--peer\" and \"--bus\" may not be used with "
                        "\"--system\" or \"--session\"\n");
        usage(1);
    }
    ...
    else if ((address != NULL) && is_bus)
    {
        if (!dbus_bus_register(connection, &error))
        {
            fprintf(
                stderr,
                "Failed to register on connection to \"%s\" message bus: %s\n",
    ...
    if (message_type == DBUS_MESSAGE_TYPE_METHOD_CALL)
    {
        ...
        {
            fprintf(stderr,
                    "Must use org.mydomain.Interface.Method notation, no dot "
                    "in \"%s\"\n",
                    name);
            exit(1);
        }
    ...
    else if (message_type == DBUS_MESSAGE_TYPE_SIGNAL)
    {
        ...
        {
            fprintf(stderr,
                    "Must use org.mydomain.Interface.Signal notation, no dot "
                    "in \"%s\"\n",
                    name);
            exit(1);
        }
        ...
        *(c++) = 0;
        ...
        if (container_type == DBUS_TYPE_DICT_ENTRY)
        {
            ...
            *(c++) = 0;
            ...
            sig[4] = '\0';
            handle_oom(dbus_message_iter_open_container(&iter, DBUS_TYPE_ARRAY,
                                                        sig, &container_iter));
        ...
        else if (container_type != DBUS_TYPE_INVALID)
        {
            ...
            sig[0] = type2;
            sig[1] = '\0';
            handle_oom(dbus_message_iter_open_container(&iter, container_type,
                                                        sig, &container_iter));
            target_iter = &container_iter;
        }
        else
