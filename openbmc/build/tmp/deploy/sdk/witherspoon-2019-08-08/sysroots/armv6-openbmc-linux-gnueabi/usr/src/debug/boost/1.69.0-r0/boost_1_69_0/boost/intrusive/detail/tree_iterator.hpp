...
{
  private:
    typedef iiterator<ValueTraits, IsConst, std::bidirectional_iterator_tag>
        types_t;
  ...
  public:
    ...
    typedef
        typename types_t::iterator_type::iterator_category iterator_category;

    BOOST_INTRUSIVE_FORCEINLINE tree_iterator()
    {
    }

    BOOST_INTRUSIVE_FORCEINLINE explicit tree_iterator(
        const node_ptr& nodeptr, const const_value_traits_ptr& traits_ptr) :
    ...
    BOOST_INTRUSIVE_FORCEINLINE tree_iterator&
        operator=(const node_ptr& nodeptr)
    {
        members_.nodeptr_ = nodeptr;
  ...
  public:
    ...
    {
        return members_.nodeptr_ ? &tree_iterator::unspecified_bool_type_func
                                 : 0;
    }
    ...
    BOOST_INTRUSIVE_FORCEINLINE friend bool operator==(const tree_iterator& l,
                                                       const tree_iterator& r)
    {
    ...
    {
        return tree_iterator(node_algorithms::get_header(this->pointed_node()),
                             this->get_value_traits());
    }

    tree_iterator<value_traits, false> unconst() const
    {
        return tree_iterator<value_traits, false>(this->pointed_node(),
                                                  this->get_value_traits());
    }
  ...
  private:
    ...
    iiterator_members<node_ptr, const_value_traits_ptr, stateful_value_traits>
        members_;
};
