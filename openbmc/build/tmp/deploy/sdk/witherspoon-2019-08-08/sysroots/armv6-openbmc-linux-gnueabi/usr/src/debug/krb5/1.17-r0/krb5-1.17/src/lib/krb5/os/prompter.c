                                                  ...
                                                  krb5_prompt prompts[])
{
    ...
    for (i = 0; i < num_prompts; i++)
    {
        ...
        got_int = 0;
        retp = fgets(prompts[i].reply->data, (int)prompts[i].reply->length, fp);
        if (prompts[i].hidden)
            putchar('\n');
        if (retp == NULL)
        {
...
static void catch_signals(osiginfo* osigint)
{
#ifdef POSIX_SIGNALS
    struct sigaction sa;
...
static void restore_signals(osiginfo* osigint)
{
#ifdef POSIX_SIGNALS
...
static krb5_error_code setup_tty(FILE* fp, int hidden, struct termios* saveparm,
                                 osiginfo* osigint)
{
    krb5_error_code ret;
    int fd;
...
static krb5_error_code restore_tty(FILE* fp, struct termios* saveparm,
                                   osiginfo* osigint)
{
    int ret, fd;
...
void k5_set_prompt_types(krb5_context context, krb5_prompt_type* types)
{
