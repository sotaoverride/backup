...
int register_readfd(int fd, void (*func)(int, void*), void* data)
{
    if (external_readfdlen < NUM_EXTERNAL_FDS)
    {
        ...
        external_readfdlen++;
        DEBUGMSGTL(
            ("fd_event_manager:register_readfd", "registered fd %d\n", fd));
...
int register_writefd(int fd, void (*func)(int, void*), void* data)
{
    if (external_writefdlen < NUM_EXTERNAL_FDS)
    {
        ...
        external_writefdlen++;
        DEBUGMSGTL(
            ("fd_event_manager:register_writefd", "registered fd %d\n", fd));
        return FD_REGISTERED_OK;
    }
    else
    {
...
int register_exceptfd(int fd, void (*func)(int, void*), void* data)
{
    if (external_exceptfdlen < NUM_EXTERNAL_FDS)
    {
        ...
        DEBUGMSGTL(
            ("fd_event_manager:register_exceptfd", "registered fd %d\n", fd));
        return FD_REGISTERED_OK;
    }
    else
    {
...
int unregister_readfd(int fd)
{
    ...
    for (i = 0; i < external_readfdlen; i++)
    {
        if (external_readfd[i] == fd)
        {
            ...
            DEBUGMSGTL(("fd_event_manager:unregister_readfd",
                        "unregistered fd %d\n", fd));
...
int unregister_writefd(int fd)
{
    ...
    for (i = 0; i < external_writefdlen; i++)
    {
        if (external_writefd[i] == fd)
        {
            ...
            DEBUGMSGTL(("fd_event_manager:unregister_writefd",
                        "unregistered fd %d\n", fd));
            external_fd_unregistered = 1;
            return FD_UNREGISTERED_OK;
        }
...
int unregister_exceptfd(int fd)
{
    ...
    for (i = 0; i < external_exceptfdlen; i++)
    {
        if (external_exceptfd[i] == fd)
        {
            ...
            DEBUGMSGTL(("fd_event_manager:unregister_exceptfd",
                        "unregistered fd %d\n", fd));
            external_fd_unregistered = 1;
            return FD_UNREGISTERED_OK;
        }
 ...
 */
void netsnmp_external_event_info(int* numfds, fd_set* readfds, fd_set* writefds,
                                 fd_set* exceptfds)
{
    ...
    {
        snmp_log(LOG_ERR, "Use netsnmp_external_event_info2() for processing"
...
void netsnmp_external_event_info2(int* numfds, netsnmp_large_fd_set* readfds,
 ...
 */
void netsnmp_dispatch_external_events(int* count, fd_set* readfds,
                                      fd_set* writefds, fd_set* exceptfds)
{
    ...
    netsnmp_dispatch_external_events2(count, &lreadfds, &lwritefds,
                                      &lexceptfds);
...
{
    int i;
    for (i = 0; *count && (i < external_readfdlen) && !external_fd_unregistered;
         i++)
    {
