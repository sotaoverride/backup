...
BOOST_LOG_ANONYMOUS_NAMESPACE
{
    ...
    BOOST_ALIGNMENT(BOOST_LOG_CPU_CACHE_LINE_SIZE)
    static get_tick_count64_state g_state;

    //! Artifical implementation of GetTickCount64
    uint64_t BOOST_WINAPI_WINAPI_CC get_tick_count64()
    {
        // Note: Even in single-threaded builds we have to implement
        // get_tick_count64 in a thread-safe way because
        //       it can be called in the system thread pool during refreshes
        //       concurrently with user's calls.
        uint64_t old_state = g_state.ticks.load(boost::memory_order_acquire);

        uint32_t new_ticks = boost::winapi::GetTickCount();

        uint32_t old_ticks =
            static_cast<uint32_t>(old_state & UINT64_C(0x00000000ffffffff));
        uint64_t new_state =
            ((old_state & UINT64_C(0xffffffff00000000)) +
             (static_cast<uint64_t>(new_ticks < old_ticks) << 32)) |
            static_cast<uint64_t>(new_ticks);
    ...
    //! The function is called periodically in the system thread pool to make
    //! sure g_state.ticks is timely updated
    void BOOST_WINAPI_NTAPI_CC refresh_get_tick_count64(boost::winapi::PVOID_,
    ...
    {
        boost::winapi::HMODULE_ hKernel32 =
            boost::winapi::GetModuleHandleW(L"kernel32.dll");
        if (hKernel32)
        {
            get_tick_count_t p =
                (get_tick_count_t)boost::winapi::get_proc_address(
                    hKernel32, "GetTickCount64");
            if (p)
            {
                // Use native API
        ...
        // No native API available. Use emulation with periodic refreshes to
        // make sure the GetTickCount wrap arounds are properly counted.
        init_state old_init = uninitialized;
        if (g_state.init.compare_exchange_strong(old_init, in_progress,
                                                 boost::memory_order_acq_rel,
                                                 boost::memory_order_relaxed))
        {
            if (!g_state.wait_event)
                g_state.wait_event =
                    boost::winapi::create_anonymous_event(NULL, false, false);
            if (g_state.wait_event)
            {
                boost::winapi::BOOL_ res =
                    boost::winapi::RegisterWaitForSingleObject(
                        &g_state.wait_handle, g_state.wait_event,
                        &refresh_get_tick_count64, NULL, 0x7fffffff,
                        boost::winapi::WT_EXECUTEINWAITTHREAD_);
                if (res)
                {
                    std::atexit(&cleanup_get_tick_count64);

                    const_cast<get_tick_count_t volatile&>(get_tick_count) =
                        &get_tick_count64;
                    g_state.init.store(initialized,
...
BOOST_LOG_API get_tick_count_t get_tick_count = &get_tick_count_init;
       ...
       (_POSIX_TIMERS + 0) > 0) /* POSIX timers supported */                   \
    || defined(__GNU__) || defined(__OpenBSD__) ||                             \
    defined(                                                                   \
        __CloudABI__) /* GNU Hurd, OpenBSD and Nuxi CloudABI don't support     \
                         POSIX timers fully but do provide clock_gettime() */
    ...
    timestamp get_timestamp_realtime_clock()
    {
        ...
        if (BOOST_UNLIKELY(clock_gettime(CLOCK_REALTIME, &ts) != 0))
        {
            ...
            BOOST_LOG_THROW_DESCR_PARAMS(
                system_error, "Failed to acquire current time", (err));
        }

        return timestamp(static_cast<uint64_t>(ts.tv_sec) *
                             UINT64_C(1000000000) +
                         ts.tv_nsec);
    }
    ...
    timestamp get_timestamp_monotonic_clock()
    {
        ...
        if (BOOST_UNLIKELY(clock_gettime(CLOCK_MONOTONIC, &ts) != 0))
        {
            ...
            if (err == EINVAL)
            {
                ...
                // Fall back to realtime clock, which is not exactly what we
                // need but is better than nothing.
...
BOOST_LOG_API int64_t duration::milliseconds() const
{
    ...
    BOOST_LOG_ONCE_BLOCK()
    {
        ...
        if (err != KERN_SUCCESS)
        {
            BOOST_LOG_THROW_DESCR_PARAMS(system_error,
                                         "Failed to initialize timebase info",
    ...
    {
        return (m_ticks * timebase_info.numer) /
               (INT64_C(1000000) * timebase_info.denom);
    }
}
