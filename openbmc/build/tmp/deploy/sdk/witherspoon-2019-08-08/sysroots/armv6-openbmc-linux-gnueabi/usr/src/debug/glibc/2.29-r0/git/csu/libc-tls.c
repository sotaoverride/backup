...
static void init_slotinfo(void)
{
...
static void init_static_tls(size_t memsz, size_t align)
{
    /* That is the size of the TLS memory for this object.  The initialized
       value of _dl_tls_static_size is provided by dl-open.c to request some
       surplus that permits dynamic loading of modules with IE-model TLS.  */
    GL(dl_tls_static_size) =
        roundup(memsz + GL(dl_tls_static_size), TLS_TCB_ALIGN);
#if TLS_TCB_AT_TP
...
void __libc_setup_tls(void)
{
    ...
    if (_dl_phdr != NULL)
        for (phdr = _dl_phdr; phdr < &_dl_phdr[_dl_phnum]; ++phdr)
            ...
            /* We have to set up the TCB block which also (possibly) contains
               ...
               The initialized value of _dl_tls_static_size is provided by
               dl-open.c to request some surplus that permits dynamic loading of
               modules with IE-model TLS.  */
#if TLS_TCB_AT_TP
    ...
    tlsblock = __sbrk(tcb_offset + memsz + max_align + TLS_PRE_TCB_SIZE +
                      GL(dl_tls_static_size));
...
#error "Either TLS_TCB_AT_TP or TLS_DTV_AT_TP must be defined"
    ...
    /* Align the TLS block.  */
    tlsblock =
        (void*)(((uintptr_t)tlsblock + max_align - 1) & ~(max_align - 1));

    /* Initialize the dtv.  [0] is the length, [1] the generation counter.  */
    _dl_static_dtv[0].counter =
        (sizeof(_dl_static_dtv) / sizeof(_dl_static_dtv[0])) - 2;
