  ...
  public:
    ...
    simple_converter_impl(std::string const& encoding)
    {
        ...
        for (unsigned i = 128; i < 256; i++)
        {
            ...
            try
            {
                std::wstring const tmp =
                    conv::to_utf<wchar_t>(buf, buf + 1, encoding, conv::stop);
                if (tmp.size() == 1)
                {
                    uchar = tmp[0];
                }
...
class simple_codecvt
    : public generic_codecvt<CharType, simple_codecvt<CharType>>
{
  public:
    ...
    static state_type initial_state(
        generic_codecvt_base::initial_convertion_state /* unused */)
    {
        return state_type();
    }
    ...
    utf::code_point from_unicode(state_type&, utf::code_point u, char* begin,
                                 char const* end) const
    {
        return cvt_.from_unicode(u, begin, end);
    }
...
char const* simple_encoding_table[] = {
    ...
    "cp1255",      "cp1256",      "cp1257",      "iso88591",    "iso885913",
    "iso885915",   "iso88592",    "iso88593",    "iso88594",    "iso88595",
    "iso88596",    "iso88597",    "iso88598",    "iso88599",    "koi8r",
    "koi8u",       "usascii",     "windows1250", "windows1251", "windows1252",
...
bool check_is_simple_encoding(std::string const& encoding)
{
    ...
    return std::binary_search<char const**>(simple_encoding_table,
                                            simple_encoding_table +
                                                sizeof(simple_encoding_table) /
                                                    sizeof(char const*),
                                            norm.c_str(), compare_strings);
    return 0;
}
...
class code_converter
    : public generic_codecvt<CharType, code_converter<CharType>>
{
  public:
#ifndef BOOST_NO_CXX11_SMART_PTR
...
#define PTR_TRANS(x) (x)
    ...
    base_converter_ptr initial_state(
    ...
    utf::code_point to_unicode(base_converter_ptr& ptr, char const*& begin,
                               char const* end) const
    {
        if (thread_safe_)
            return cvt_->to_unicode(begin, end);
    ...
    utf::code_point from_unicode(base_converter_ptr& ptr, utf::code_point u,
                                        ...
                                        character_facet_type type)
{
    ...
    switch (type)
    {
        ...
        case char16_t_facet:
            return std::locale(in,
                               new code_converter<char16_t>(PTR_TRANS(cvt)));
#endif
#if defined(BOOST_LOCALE_ENABLE_CHAR32_T) && !defined(BOOST_NO_CHAR32_T_CODECVT)
        case char32_t_facet:
            return std::locale(in,
                               new code_converter<char32_t>(PTR_TRANS(cvt)));
#endif
        default:
            return in;
    }
...
/// Unicode code points,
///
/// Throws invalid_charset_error if the chacater set is not supported or isn't
                                  ...
                                  character_facet_type type)
{
    if (!check_is_simple_encoding(encoding))
        throw boost::locale::conv::invalid_charset_error(
            "Invalid simple encoding " + encoding);
