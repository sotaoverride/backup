    ...
    wint_t _IO_wfile_underflow(FILE* fp)
{
    ...
    if (fp->_IO_read_ptr < fp->_IO_read_end)
    {
        ...
        fp->_wide_data->_IO_read_base = fp->_wide_data->_IO_read_ptr =
            fp->_wide_data->_IO_buf_base;
        status = (*cd->__codecvt_do_in)(
            cd, &fp->_wide_data->_IO_state, fp->_IO_read_ptr, fp->_IO_read_end,
            &read_stop, fp->_wide_data->_IO_read_ptr,
        ...
        fp->_IO_read_end =
            (fp->_IO_buf_base + (fp->_IO_read_end - fp->_IO_read_ptr));
...
again:
    count =
        _IO_SYSREAD(fp, fp->_IO_read_end, fp->_IO_buf_end - fp->_IO_read_end);
    if (count <= 0)
    {
    ...
    status = (*cd->__codecvt_do_in)(
        cd, &fp->_wide_data->_IO_state, from, to, &read_ptr_copy,
        fp->_wide_data->_IO_read_end, fp->_wide_data->_IO_buf_end,
...
libc_hidden_def(_IO_wfile_sync)
    ...
    static int adjust_wide_data(FILE* fp, bool do_convert)
{
    struct _IO_codecvt* cv = fp->_codecvt;
    ...
    {
        fp->_wide_data->_IO_read_end +=
            ((fp->_IO_read_ptr - fp->_IO_read_base) / clen);
        goto done;
    }
    ...
    do
    {
        ...
        fp->_wide_data->_IO_last_state = fp->_wide_data->_IO_state;
        status = (*cv->__codecvt_do_in)(
            cv, &fp->_wide_data->_IO_state, fp->_IO_read_base, fp->_IO_read_ptr,
            &read_stop, fp->_wide_data->_IO_read_base,
            fp->_wide_data->_IO_buf_end, &fp->_wide_data->_IO_read_end);
       ...
       try and return an error.  */
        ...
        if (!unflushed_writes)
        {
            ...
            else
            {
                ...
                nread = (*cv->__codecvt_do_length)(
                    cv, &state, fp->_IO_read_base, fp->_IO_read_end, delta);
        ...
        else
        {
            ...
            else
            {
                ...
                __mbstate_t state = fp->_wide_data->_IO_last_state;
                status =
                    (*cv->__codecvt_do_out)(cv, &state, in, in + delta, &in,
            ...
            /* We don't trust _IO_read_end to represent the current file offset
               ...
               shifted to the end of the file during a flush.  Use the write
               base instead, along with the new offset we got above when we did
               a seek to the end of the file.  */
            if (append_mode)
           ...
           the current position in the internal buffer.  */
            ...
            if (mode != 0 || !was_writing)
            {
                ...
                else
                {
                    ...
                    fp->_wide_data->_IO_state = fp->_wide_data->_IO_last_state;
                    nread = (*cv->__codecvt_do_length)(
                        cv, &fp->_wide_data->_IO_state, fp->_IO_read_base,
                        fp->_IO_read_end, delta);
                    fp->_IO_read_ptr = fp->_IO_read_base + nread;
                    fp->_wide_data->_IO_read_end = fp->_wide_data->_IO_read_ptr;
                    offset -= fp->_IO_read_end - fp->_IO_read_base - nread;
...
libc_hidden_def(_IO_wfile_seekoff)
    ...
    size_t _IO_wfile_xsputn(FILE* f, const void* data, size_t n)
{
