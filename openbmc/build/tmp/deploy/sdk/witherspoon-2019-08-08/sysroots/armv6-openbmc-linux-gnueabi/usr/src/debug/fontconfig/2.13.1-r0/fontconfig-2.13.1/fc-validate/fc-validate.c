...
static const struct option longopts[] = {
    {"index", 1, 0, 'i'},   {"lang", 1, 0, 'l'}, {"verbose", 0, 0, 'v'},
...
static void usage(char* program, int error)
{
...
#if HAVE_GETOPT_LONG
    fprintf(file,
            _("usage: %s [-Vhv] [-i index] [-l LANG] [--index index] [--lang "
              "LANG] [--verbose] [--version] [--help] font-file...\n"),
            program);
#else
    ...
    fprintf(file, "\n");
#if HAVE_GETOPT_LONG
    fprintf(file, _("  -i, --index INDEX    display the INDEX face of each "
                    "font file only\n"));
    fprintf(file,
            _("  -l, --lang=LANG      set LANG instead of current locale\n"));
    fprintf(file, _("  -v, --verbose        show more detailed information\n"));
    fprintf(file,
            _("  -V, --version        display font config version and exit\n"));
    fprintf(file, _("  -h, --help           display this help and exit\n"));
#else
    fprintf(file, _("  -i INDEX   (index)        display the INDEX face of "
                    "each font file only\n"));
    fprintf(
        file,
        _("  -l LANG    (lang)         set LANG instead of current locale\n"));
    fprintf(file,
            _("  -v         (verbose)      show more detailed information\n"));
    fprintf(file, _("  -V         (version)      display font config version "
                    "and exit\n"));
    fprintf(file,
        ...
        do
        {
            ...
            else
            {
                ...
                if (count > 0)
                {
                    ...
                    printf(_("%s:%d Missing %d glyph(s) to satisfy the "
                             "coverage for %s language\n"),
