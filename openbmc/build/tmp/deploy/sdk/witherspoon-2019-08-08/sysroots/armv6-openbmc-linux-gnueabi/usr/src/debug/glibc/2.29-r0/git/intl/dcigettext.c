...
#define PATH_MAX                                                               \
    (__pathconf("/", _PC_PATH_MAX) < 1 ? 1024 : __pathconf("/", _PC_PATH_MAX))
#endif
...
#ifdef IN_LIBGLOCALE
    char* gl_dcigettext(const char* domainname, const char* msgid1,
   ...
   DOMAINNAME and CATEGORY.  */
    ...
    if (domain != NULL)
    {
        ...
        if (retval == NULL)
        {
            ...
            {
#if defined IN_LIBGLOCALE
                retval = _nl_find_msg(domain->successor[cnt], binding, encoding,
                                      msgid1, &retlen);
#else
        ...
        if (retval != NULL)
        {
           ...
           starting at RETVAL, RETLEN bytes.  */
            ...
            if (foundp == NULL)
            {
                ...
                size = offsetof(struct known_translation_t, msgid) + msgid_len +
                       domainname_len + 1;
#ifdef HAVE_PER_THREAD_LOCALE
                ...
                if (newp != NULL)
                {
                    ...
                    new_domainname =
                        (char*)mempcpy(newp->msgid.appended, msgid1, msgid_len);
                    ...
                    foundp = (struct known_translation_t**)tsearch(newp, &root,
                                                                   transcmp);
...
if (!ENABLE_SECURE)
{
    extern void _nl_log_untranslated(const char* logfilename,
                                     const char* domainname, const char* msgid1,
...
#endif
{
    ...
    if (domain->hash_tab != NULL)
    {
        ...
        while (1)
        {
            ...
            if (nstr < nstrings
                    ? W(domain->must_swap, domain->orig_tab[nstr].length) >=
                              len &&
                          (strcmp(msgid,
                                  domain->data +
                                      W(domain->must_swap,
                    ...
                    : domain->orig_sysdep_tab[nstr - nstrings].length > len &&
                          (strcmp(msgid,
                                  domain->orig_sysdep_tab[nstr - nstrings]
                                      .pointer) == 0))
            {
                act = nstr;
                goto found;
        ...
        if (convd == NULL)
        {
...

            {
                ...
                struct converted_domain* new_conversions =
                    (struct
                     converted_domain*)(domain->conversions != NULL
                                            ? realloc(
                                                  domain->conversions,
                                                  (nconversions + 1) *
                                                      sizeof(struct
                                                             converted_domain))
                                            : malloc((nconversions + 1) *
                                                     sizeof(struct
                                                            converted_domain)));
                ...
                /* Find out about the character set the file is encoded with.
                   ...
                   entry does not exist or if this does not contain the
                   'charset=' information, we will assume the charset matches
                   the one the current locale and we don't have to perform any
                   conversion.  */
#ifdef _LIBC
...
#endif
                {
                    ...
                    /* Get the header entry.  This is a recursion, but it
                   doesn't reallocate domain->conversions because we pass
                   encoding = NULL or convert = 0, respectively.  */
                    ...
                    /* Resource problems are fatal.  If we continue onwards we
                       will only attempt to calloc a new conv_tab and fail
                       later.  */
                    ...
                    if (nullentry != NULL)
                    {
                        ...
                        if (charsetstr != NULL)
                        {
                            ...
                            *((char*)mempcpy(charset, charsetstr, len)) = '\0';
                            ...
                            outcharset =
                                norm_add_slashes(outcharset, "TRANSLIT");
...
#else
#if HAVE_ICONV
                            /* When using GNU libc >= 2.2 or GNU libiconv
                           >= 1.5, we want to use transliteration.  */
    ...
    _LIBICONV_VERSION >= 0x0105
        ...
        )
        {
                ...
                if (__builtin_expect(convd->conv_tab == NULL, 0))
            {
                ...
                {
                    convd->conv_tab = (char**)calloc(
                        nstrings + domain->n_sysdep_strings, sizeof(char*));
                    if (convd->conv_tab != NULL)
...
#define INITIAL_BLOCK_SIZE 4080
            ...
            not_translated_yet:
                ...
                while (1)
                {
                    ...
                    res =
                        __gconv(convd->conv, &inbuf, inbuf + resultlen, &outbuf,
                    ...
                    if (iconv(convd->conv, (ICONV_CONST char**)&inptr, &inleft,
                ...
                resize_freemem:
                    ...
                    else
                    {
                        ...
                        if (newmem != NULL)
                        {
                            /* Add the block to the list of blocks we have to
                               free at some point.  */
...
#endif
{
    ...
    /* We use the settings in the following order:
       ...
       value. This is how POSIX specifies it.  The value is a single locale
       name.
