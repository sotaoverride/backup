...
#pragma warning(push)
#pragma warning(                                                               \
    disable : 4275) // non dll-interface class 'std::logic_error' used as base
                    // for dll-interface class 'boost::program_options::error'
#pragma warning(disable : 4251) // class 'std::vector<_Ty>' needs to have
...
*/
class BOOST_PROGRAM_OPTIONS_DECL too_many_positional_options_error
    : public error
{
  public:
    too_many_positional_options_error() :
        error("too many positional options have been specified on the command "
              "line")
    {
    }
};
...
class BOOST_PROGRAM_OPTIONS_DECL error_with_option_name : public error
{
  ...
  protected:
    ...
    virtual void
        substitute_placeholders(const std::string& error_template) const;
...
/** Class thrown when there are several occurrences of an
    ...
    them all. */
class BOOST_PROGRAM_OPTIONS_DECL multiple_occurrences
 ...
 *
 *
 *  It makes no sense to have an option name, when we can't match an option to
 * the parameter
 *
 *  Having this a part of the error_with_option_name hierachy makes error
 * handling a lot easier, even if the name indicates some sort of conceptual
 * dissonance!
 *
 *   */
class BOOST_PROGRAM_OPTIONS_DECL error_with_no_option_name
    : public error_with_option_name
{
  public:
...
class BOOST_PROGRAM_OPTIONS_DECL unknown_option
    : public error_with_no_option_name
{
  public:
    unknown_option(const std::string& original_token = "") :
        error_with_no_option_name("unrecognised option '%canonical_option%'",
                                  original_token)
    {
    }
...
class BOOST_PROGRAM_OPTIONS_DECL ambiguous_option
    : public error_with_no_option_name
{
  public:
...
class BOOST_PROGRAM_OPTIONS_DECL invalid_syntax : public error_with_option_name
{
  public:
    ...
    invalid_syntax(kind_t kind, const std::string& option_name = "",
                   const std::string& original_token = "",
                   int option_style = 0) :
        error_with_option_name(get_template(kind), option_name, original_token,
                               option_style),
        m_kind(kind)
    {
    }
...
class BOOST_PROGRAM_OPTIONS_DECL invalid_config_file_syntax
...
class BOOST_PROGRAM_OPTIONS_DECL invalid_command_line_syntax
    : public invalid_syntax
{
  public:
...
class BOOST_PROGRAM_OPTIONS_DECL validation_error
    : public error_with_option_name
{
  public:
    enum kind_t
