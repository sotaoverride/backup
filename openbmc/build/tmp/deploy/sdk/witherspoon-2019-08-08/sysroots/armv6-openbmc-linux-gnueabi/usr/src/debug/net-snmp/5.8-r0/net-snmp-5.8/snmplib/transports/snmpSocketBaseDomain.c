 ...
 * size as possible)
 */
static int _sock_buffer_maximize(int s, int optname, const char* buftype,
                                 int size)
{
    int curbuf = 0;
     ...
     */
    if ((getsockopt(s, SOL_SOCKET, optname, (void*)&curbuf, &curbuflen) == 0) &&
        (curbuflen == sizeof(int)))
    {
        ...
        {
            mid = (lo + hi) / 2;
            if (setsockopt(s, SOL_SOCKET, optname, (void*)&mid, sizeof(int)) ==
                0)
            {
        ...
        {
            DEBUGMSGTL(
                ("socket:buffer:max", "Maximized %s: %d\n", buftype, curbuf));
        }
    }
    else
    {
...
static const char* _sock_buf_type_get(int optname, int local)
{
 ...
 */
static int _sock_buffer_size_get(int optname, int local, const char** buftype)
{
    int size;
...
int netsnmp_sock_buffer_set(int s, int optname, int local, int size)
{
    ...
    else
    {
        buftype = _sock_buf_type_get(optname, local);
        DEBUGMSGT(
            ("verbose:socket:buffer", "Requested %s is %d\n", buftype, size));
    }
        ...
        (curbuflen == sizeof(int)))
    {
        ...
        DEBUGMSGT(
            ("verbose:socket:buffer", "Original %s is %d\n", buftype, curbuf));
        if (curbuf >= size)
        {
    ...
    if (setsockopt(s, SOL_SOCKET, optname, (void*)&size, sizeof(int)) == 0)
    {
        ...
        DEBUGIF("socket:buffer")
        {
                ...
                (curbuflen == sizeof(int)))
            {
                ...
                DEBUGMSGT(("verbose:socket:buffer", "Now %s is %d\n", buftype,
                           curbuf));
            }
        }
        /*
