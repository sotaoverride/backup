...
struct ib_request* __create_ib_request(const_nis_name name, unsigned int flags)
{
...
static char* get_tablepath(char* name, dir_binding* bptr)
{
    ...
    result = clnt_call(bptr->clnt, NIS_LOOKUP, (xdrproc_t)_xdr_ns_request,
                       (caddr_t)&req, (xdrproc_t)_xdr_nis_result, (caddr_t)&res,
                       RPCTIMEOUT);
...
nis_error __follow_path(char** tablepath, char** tableptr,
                         ...
                         const void* userdata)
{
    ...
    again:
        clnt_status = clnt_call(
            bptr.clnt, NIS_IBLIST, (xdrproc_t)_xdr_ib_request, (caddr_t)ibreq,
            (xdrproc_t)_xdr_nis_result, (caddr_t)res, RPCTIMEOUT);
            ...
            { /* start switch */
                ...
                case NIS_S_SUCCESS:
                        ...
                        if (NIS_RES_OBJECT(res)->LI_data.li_attrs.li_attrs_len)
                            if (ibreq->ibr_srch.ibr_srch_len == 0)
                            {
                                ibreq->ibr_srch.ibr_srch_len =
                                    NIS_RES_OBJECT(res)
                                        ->LI_data.li_attrs.li_attrs_len;
                                ibreq->ibr_srch.ibr_srch_val =
                                    NIS_RES_OBJECT(res)
                                        ->LI_data.li_attrs.li_attrs_val;
                            }
                             ...
                             (FOLLOW_PATH | ALL_RESULTS))
                    {
                        ...
                        {
                            nis_object* objects_val = realloc(
                                NIS_RES_OBJECT(allres),
                ...
                case NIS_CBRESULTS:
                    if (cb != NULL)
                    {
                        ...
                        {
                            enum nis_error err = __follow_path(
                                &tablepath, &tableptr, ibreq, &bptr);
                            if (err != NIS_SUCCESS)
                            {
                                if (err == NIS_NOMEMORY)
                                    NIS_RES_STATUS(res) = err;
...
libnsl_hidden_nolink_def(nis_list, GLIBC_2_1)
    ...
    nis_result* nis_add_entry(const_nis_name name, const nis_object* obj2,
                              unsigned int flags)
{
    ...
    nis_error status = __do_niscall(
        ibreq->ibr_name, NIS_IBADD, (xdrproc_t)_xdr_ib_request, (caddr_t)ibreq,
        (xdrproc_t)_xdr_nis_result, (caddr_t)res, 0, NULL);
...
libnsl_hidden_nolink_def(nis_add_entry, GLIBC_2_1)
    ...
    nis_result* nis_modify_entry(const_nis_name name, const nis_object* obj2,
                                 unsigned int flags)
{
    nis_object obj;
    nis_result* res;
...
libnsl_hidden_nolink_def(nis_modify_entry, GLIBC_2_1)
    ...
    nis_result* nis_remove_entry(const_nis_name name, const nis_object* obj,
                                 unsigned int flags)
{
    nis_result* res;
    ib_request* ibreq;
    ...
    if ((status = __do_niscall(ibreq->ibr_name, NIS_IBREMOVE,
                               (xdrproc_t)_xdr_ib_request, (caddr_t)ibreq,
                               (xdrproc_t)_xdr_nis_result, (caddr_t)res, 0,
                               NULL)) != NIS_SUCCESS)
        NIS_RES_STATUS(res) = status;
