...
static void __attribute__((__noreturn__)) usage(void)
{
    ...
    fprintf(out,
            _(" %1$s [resource-option...] [output-option]\n"
              " %1$s -m|-q|-s -i <id>\n"),
...
static void do_shm(char format, int unit)
{
    ...
    switch (format)
    {
        case LIMITS:
        {
            ...
            /* overflow handling, at least we don't print ridiculous small
             * values */
        ...
        case STATUS:
        {
            ...
            printf(_("segments allocated %d\n"
                     ...
                     "pages swapped   %ld\n"
                     "Swap performance: %ld attempts\t %ld successes\n"),
                   shm_info->used_ids, shm_info->shm_tot, shm_info->shm_rss,
                   shm_info->shm_swp, shm_info->swap_attempts,
        ...
        case CREATOR:
            printf(
                _("------ Shared Memory Segment Creators/Owners --------\n"));
            printf("%-10s %-10s %-10s %-10s %-10s %-10s\n", _("shmid"),
                   _("perms"), _("cuid"), _("cgid"), _("uid"), _("gid"));
            break;
        ...
        case TIME:
            ...
            printf("%-10s %-10s %-20s %-20s %-20s\n", _("shmid"), _("owner"),
                   _("attached"), _("detached"), _("changed"));
            break;
        ...
        default:
            ...
            printf("%-10s %-10s %-10s %-10s %-10s %-10s %-12s\n", _("key"),
                   _("shmid"), _("owner"), _("perms"),
                   unit == IPC_UNIT_HUMAN ? _("size") : _("bytes"), _("nattch"),
                   _("status"));
            break;
    }
        ...
        switch (format)
        {
            case TIME:
                if (pw)
                    ...
                    printf("%-10d %-10u", shmdsp->shm_perm.id,
                           shmdsp->shm_perm.uid);
                /* ctime uses static buffer: use separate calls */
                printf(" %-20.16s", shmdsp->shm_atim
                                        ? xctime(&shmdsp->shm_atim) + 4
                                        : _("Not set"));
                printf(" %-20.16s", shmdsp->shm_dtim
                                        ? xctime(&shmdsp->shm_dtim) + 4
                                        : _("Not set"));
                printf(" %-20.16s\n", shmdsp->shm_ctim
                                          ? xctime(&shmdsp->shm_ctim) + 4
                                          : _("Not set"));
                break;
            case PID:
                if (pw)
        ...
        switch (format)
        {
            case TIME:
                if (pw)
                    ...
                    printf("%-8d %-10u", semdsp->sem_perm.id,
                           semdsp->sem_perm.uid);
                printf("  %-26.24s", semdsp->sem_otime
                                         ? xctime(&semdsp->sem_otime)
                                         : _("Not set"));
                printf(" %-26.24s\n", semdsp->sem_ctime
                                          ? xctime(&semdsp->sem_ctime)
                                          : _("Not set"));
                break;
            case PID:
                break;
            ...
            default:
                ...
                if (pw)
                    ...
                    printf("%-10d %-10u", semdsp->sem_perm.id,
                           semdsp->sem_perm.uid);
...
static void do_msg(char format, int unit)
{
    ...
    switch (format)
    {
        ...
        case TIME:
            printf(
                _("------ Message Queues Send/Recv/Change Times --------\n"));
            printf("%-8s %-10s %-20s %-20s %-20s\n", _("msqid"), _("owner"),
                   _("send"), _("recv"), _("change"));
            break;
        ...
        switch (format)
        {
            case TIME:
                if (pw)
                    ...
                    printf("%-8d %-10u", msgdsp->msg_perm.id,
                           msgdsp->msg_perm.uid);
                printf(" %-20.16s", msgdsp->q_stime
                                        ? xctime(&msgdsp->q_stime) + 4
                                        : _("Not set"));
                printf(" %-20.16s", msgdsp->q_rtime
                                        ? xctime(&msgdsp->q_rtime) + 4
                                        : _("Not set"));
                printf(" %-20.16s\n", msgdsp->q_ctime
                                          ? xctime(&msgdsp->q_ctime) + 4
                                          : _("Not set"));
                break;
            case PID:
                if (pw)
            ...
            default:
                ...
                if (pw)
                    ...
                    printf("%-10d %-10u", msgdsp->msg_perm.id,
                           msgdsp->msg_perm.uid);
...
static void print_shm(int shmid, int unit)
{
    ...
    printf(_("uid=%u\tgid=%u\tcuid=%u\tcgid=%u\n"), shmdata->shm_perm.uid,
           shmdata->shm_perm.gid, shmdata->shm_perm.cuid,
           shmdata->shm_perm.cgid);
    ...
    printf(_("lpid=%u\tcpid=%u\tnattch=%jd\n"), shmdata->shm_lprid,
           shmdata->shm_cprid, shmdata->shm_nattch);
...
static void print_sem(int semid)
{
    ...
    printf(_("uid=%u\t gid=%u\t cuid=%u\t cgid=%u\n"), semdata->sem_perm.uid,
           semdata->sem_perm.gid, semdata->sem_perm.cuid,
           semdata->sem_perm.cgid);
    printf(_("mode=%#o, access_perms=%#o\n"), semdata->sem_perm.mode,
           semdata->sem_perm.mode & 0777);
    printf(_("nsems = %ju\n"), semdata->sem_nsems);
