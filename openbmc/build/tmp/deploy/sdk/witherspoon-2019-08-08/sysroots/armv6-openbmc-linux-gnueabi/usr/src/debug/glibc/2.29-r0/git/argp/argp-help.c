...
static struct uparams uparams = {DUP_ARGS,     DUP_ARGS_NOTE, SHORT_OPT_COL,
                                 LONG_OPT_COL, DOC_OPT_COL,   OPT_DOC_COL,
                                 HEADER_COL,   USAGE_INDENT,  RMARGIN};
...
struct hol_cluster
{
    ...
    /* How to sort this cluster with respect to options and other clusters at
       the same depth (clusters always follow options in the same group).  */
    int group;
    ...
    if (opts)
    {
        ...
        for (o = opts, entry = hol->entries; !oend(o); entry++)
        {
            ...
            entry->group = cur_group =
                o->group ? o->group
                         : ((!o->name && !o->key) ? cur_group + 1 : cur_group);
            ...
            do
            {
                ...
                if (oshort(o) && !find_char(o->key, hol->short_options, so))
                    /* O has a valid short option which hasn't already been
                     * used.*/
                    *so++ = o->key;
                o++;
...
static struct hol_cluster* hol_add_cluster(struct hol* hol, int group,
...
{
    return hol_entry_short_iterate(entry, until_short, entry->argp->argp_domain,
                                   0);
}
    ...
    if (more->num_entries > 0)
    {
           ...
           non-shadowed SHORT_OPTIONS values.  */
        {
            ...
            for (left = more->num_entries; left > 0; e++, left--)
            {
                ...
                for (opts_left = e->num, opt = e->opt; opts_left;
                     opt++, opts_left--)
                {
                    int ch = *more_so;
                    ...
                    /* The next short option in MORE_SO, CH, is from OPT.  */
                    {
                        if (!find_char(ch, short_options,
                            ...
                            /* The short option CH isn't shadowed by HOL's
                               options, so add it to the sum.  */
                            *so++ = ch;
                        more_so++;
                    }
                }
...
/* If the option REAL has an argument, we print it in using the printf
   ...
   optional argument.  */
static void arg(const struct argp_option* real, const char* req_fmt,
                const char* opt_fmt, const char* domain,
                         ...
                         struct pentry_state* pest)
{
    ...
    const char* fstr =
        filter_doc(tstr, ARGP_KEY_HELP_HEADER, argp, pest->state);
...
static void comma(unsigned col, struct pentry_state* pest)
{
    if (pest->first)
    {
        ...
        if (cl && cl->header && *cl->header &&
            (!pe ||
             (pe->cluster != cl && !hol_cluster_is_child(pe->cluster, cl))))
                           ...
                           struct hol_help_state* hhstate)
{
    ...
    __argp_fmtstream_set_wmargin(
        stream, uparams.short_opt_col); /* For truly bizarre cases. */
    for (opt = real, num = entry->num; num > 0; opt++, num--)
...
static int usage_long_opt(const struct argp_option* opt,
                          const struct argp_option* real, const char* domain,
                          void* cookie)
{
    argp_fmtstream_t stream = cookie;
    const char* arg = opt->arg;
...
static void hol_usage(struct hol* hol, argp_fmtstream_t stream)
{
    if (hol->num_entries > 0)
    {
        ...
        for (entry = hol->entries, nentries = hol->num_entries; nentries > 0;
             entry++, nentries--)
...
static int argp_args_usage(const struct argp* argp,
                           const struct argp_state* state, char** levels,
                    ...
                    argp_fmtstream_t stream)
{
    ...
    /* We have to filter the doc strings.  */
    {
        ...
        text = (*argp->help_filter)(post ? ARGP_KEY_HELP_POST_DOC
    ...
    if (child)
        while (child->argp && !(first_only && anything))
            anything |= argp_doc((child++)->argp, state, post,
                                 anything || pre_blank, first_only, stream);
...
/* Output a usage message for ARGP to STREAM.  If called from
   ...
   needed. */
static void _help(const struct argp* argp, const struct argp_state* state,
        ...
        do
        {
            ...
            if (first_pattern)
                __argp_fmtstream_printf(
                    fs, "%s %s", dgettext(argp->argp_domain, "Usage:"), name);
            else
                __argp_fmtstream_printf(
                    fs, "%s %s", dgettext(argp->argp_domain, "  or: "), name);
            ...
            /* Just show where the options go.  */
            {
                if (hol->num_entries > 0)
                    __argp_fmtstream_puts(
                        fs, dgettext(argp->argp_domain, " [OPTION...]"));
            }
            else
...
void __argp_help(const struct argp* argp, FILE* stream, unsigned flags,
                 char* name)
{
...
void __argp_state_help(const struct argp_state* state, FILE* stream,
                       unsigned flags)
{
