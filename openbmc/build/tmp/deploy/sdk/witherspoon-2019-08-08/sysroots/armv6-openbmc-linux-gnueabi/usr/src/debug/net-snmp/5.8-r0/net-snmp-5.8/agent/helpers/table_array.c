...
netsnmp_feature_child_of(table_array_all, mib_helpers)
    ...
    netsnmp_feature_child_of(table_array_register, table_array_all)
        netsnmp_feature_child_of(table_array_find_table_array_handler,
                                 table_array_all)
            netsnmp_feature_child_of(table_array_extract_array_context,
                                     table_array_all)
                netsnmp_feature_child_of(table_array_check_row_status,
                                         table_array_all)
    ...
    static const char* mode_name[] = {"Reserve 1", "Reserve 2", "Action",
                                      "Commit",    "Free",      "Undo"};
 ...
 *  Helps you implement a table when data can be stored locally. The data is
 * stored in a sorted array, using a binary search for lookups.
 *  @ingroup table
 *
                                     ...
                                     int group_rows)
{
     ...
     */
    if ((cb->can_set && ((NULL == cb->duplicate_row) ||
                         (NULL == cb->delete_row) || (NULL == cb->row_copy))))
    {
                                 ...
                                 netsnmp_container* container, int group_rows)
{
    netsnmp_mib_handler* handler = netsnmp_create_handler(
        reginfo->handlerName, netsnmp_table_array_helper_handler);
    if (!handler ||
    ...
    return netsnmp_table_container_register(reginfo, tabreg, cb, container,
                                            group_rows);
}
...
{
    return (netsnmp_container*)netsnmp_request_get_list_data(request,
                                                             TABLE_ARRAY_NAME);
}
...
void build_new_oid(netsnmp_handler_registration* reginfo,
                   netsnmp_table_request_info* tblreq_info, netsnmp_index* row,
 ...
 **********************************************************************
 **********************************************************************/
int process_get_requests(netsnmp_handler_registration* reginfo,
                         netsnmp_agent_request_info* agtreq_info,
...
void group_requests(netsnmp_agent_request_info* agtreq_info,
                    netsnmp_request_info* requests,
                    netsnmp_container* request_group, table_container_data* tad)
{
    ...
    for (current = requests; current; current = current->next)
    {
         ...
         */
        row = g->existing_row =
            (netsnmp_index*)CONTAINER_FIND(tad->table, &index);
        if (!g->existing_row)
        {
...
#ifndef NETSNMP_NO_WRITE_SUPPORT
static void release_netsnmp_request_group(netsnmp_index* g, void* v)
{
...
static void release_netsnmp_request_groups(void* vp)
{
    netsnmp_container* c = (netsnmp_container*)vp;
    CONTAINER_FOR_EACH(
        c, (netsnmp_container_obj_func*)release_netsnmp_request_group, NULL);
    CONTAINER_FREE(c);
}
        ...
        case MODE_SET_RESERVE1: /** -> SET_RESERVE2 || SET_FREE */
            ...
            if (ag->row_created == 0)
            {
                if (context->tad->cb->duplicate_row)
                    ag->undo_info =
                        context->tad->cb->duplicate_row(ag->existing_row);
                else
                    ag->undo_info = NULL;
...
int process_set_requests(netsnmp_agent_request_info* agtreq_info,
                         netsnmp_request_info* requests,
                         table_container_data* tad, char* handler_name)
{
    ...
    if (request_group == NULL)
    {
        ...
        tmp = netsnmp_create_data_list(handler_name, request_group,
                                       ...
                                       netsnmp_request_info* requests)
{
    ...
    {
        DEBUGMSGTL(
            ("table_array", "Mode %d, Got request:\n", agtreq_info->mode));
    }
    else
    {
    ...
    if (MODE_IS_SET(agtreq_info->mode))
    {
         ...
         */
        rc = process_set_requests(agtreq_info, requests, tad,
