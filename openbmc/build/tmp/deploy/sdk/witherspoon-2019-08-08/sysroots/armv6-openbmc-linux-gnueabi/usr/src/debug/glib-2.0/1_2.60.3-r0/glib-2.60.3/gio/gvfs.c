...
static void g_vfs_dispose(GObject* object)
{
...
static GFile* resource_parse_name(GVfs* vfs, const char* parse_name,
                                  gpointer user_data)
{
...
static GFile* resource_get_file_for_uri(GVfs* vfs, const char* uri,
                                        gpointer user_data)
{
    return _g_resource_file_new(uri);
}

static void g_vfs_uri_lookup_func_closure_free(gpointer data)
{
...
{
    GVfsPrivate* priv = g_vfs_get_instance_private(vfs);
    priv->additional_schemes = g_hash_table_new_full(
        g_str_hash, g_str_equal, g_free, g_vfs_uri_lookup_func_closure_free);

    g_vfs_register_uri_scheme(vfs, "resource", resource_get_file_for_uri, NULL,
                              NULL, resource_parse_name, NULL, NULL);
}
...
static GFile* parse_name_internal(GVfs* vfs, const char* parse_name)
{
    GVfsPrivate* priv = g_vfs_get_instance_private(vfs);
    GHashTableIter iter;
    ...
    {
        ret =
            closure->parse_name_func(vfs, parse_name, closure->parse_name_data);
...
static GFile* get_file_for_uri_internal(GVfs* vfs, const char* uri)
{
    GVfsPrivate* priv = g_vfs_get_instance_private(vfs);
    GFile* ret = NULL;
    char* scheme;
...
const gchar* const* g_vfs_get_supported_uri_schemes(GVfs* vfs)
{
    ...
    if (!priv->supported_schemes)
    {
        ...
        while (
            g_hash_table_iter_next(&iter, (gpointer*)&additional_scheme, NULL))
...
GVfs* g_vfs_get_default(void)
{
    ...
    return _g_io_module_get_default(G_VFS_EXTENSION_POINT_NAME, "GIO_USE_VFS",
 ...
 * Since: 2.50
 */
gboolean g_vfs_register_uri_scheme(GVfs* vfs, const char* scheme,
                                   GVfsFileLookupFunc uri_func,
                                   gpointer uri_data,
                                   GDestroyNotify uri_destroy,
                                   GVfsFileLookupFunc parse_name_func,
