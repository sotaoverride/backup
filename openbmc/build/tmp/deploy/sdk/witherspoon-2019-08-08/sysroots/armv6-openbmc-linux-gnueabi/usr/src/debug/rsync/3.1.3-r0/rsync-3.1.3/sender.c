...
static struct sum_struct* receive_sums(int f)
{
    ...
    for (i = 0; i < s->count; i++)
    {
        ...
        if (DEBUG_GTE(DELTASUM, 3))
        {
            rprintf(FINFO, "chunk[%d] len=%d offset=%s sum1=%08x\n", i,
                    s->sums[i].len, big_num(s->sums[i].offset),
                    s->sums[i].sum1);
        }
    }
...
void successful_send(int ndx)
{
    ...
    failed:
        if (errno == ENOENT)
            ...
            rsyserr(FERROR_XFER, errno, "sender failed to %s %s", failed_op,
                    fname);
    }
    else
    {
...
void send_files(int f_in, int f_out)
{
    ...
    while (1)
    {
        ...
        if (!(iflags & ITEM_TRANSFER))
        {
            ...
            write_ndx_and_attrs(f_out, ndx, iflags, fname, file, fnamecmp_type,
                                xname, xlen);
        ...
        if (fd == -1)
        {
            ...
            {
                io_error |= IOERR_GENERAL;
                rsyserr(FERROR_XFER, errno, "send_files failed to open %s",
                        full_fname(fname));
            }
            free_sums(s);
        ...
        if (mbuf)
        {
            ...
            {
                io_error |= IOERR_GENERAL;
                rsyserr(FERROR_XFER, j, "read errors mapping %s",
                        full_fname(fname));
            }
        }
        close(fd);
