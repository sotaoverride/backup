...
static void help(void)
{
    fprintf(
        stderr,
        "Usage: i2cdump [-f] [-y] [-r first-last] [-a] I2CBUS ADDRESS [MODE "
        "[BANK [BANKREG]]]\n"
...
int main(int argc, char* argv[])
{
    ...
    if (argc > flags + 4)
    {
        ...
        if (size == I2C_SMBUS_BLOCK_DATA && (bank < 0 || bank > 0xff))
        {
    ...
    if (range)
    {
        ...
        if (dash == range || *dash != '-' || first < 0 || first > 0xff)
        {
            fprintf(stderr, "Error: Invalid range parameter!\n");
            exit(1);
        }
        last = strtol(++dash, &end, 0);
        if (end == dash || *end != '\0' || last < first || last > 0xff)
        {
            fprintf(stderr, "Error: Invalid range parameter!\n");
            exit(1);
        }
        ...
        switch (size)
        {
            ...
            case I2C_SMBUS_WORD_DATA:
                ...
                /* Fall through */
            default:
                fprintf(stderr, "Error: Range parameter not compatible with "
                                "selected mode!\n");
                exit(1);
        }
    }
    ...
    if (!yes)
    {
        ...
        fprintf(stderr,
                "I will probe file %s, address 0x%x, mode "
                "%s\n",
                filename, address,
                size == I2C_SMBUS_BLOCK_DATA
                    ? "smbus block"
                    : size == I2C_SMBUS_I2C_BLOCK_DATA
                          ? "i2c block"
                          : size == I2C_SMBUS_BYTE
                                ? "byte consecutive read"
                                : size == I2C_SMBUS_DWORD
                                      ? "double word"
                                      : size == I2C_SMBUS_BYTE_DATA ? "byte"
                                                                    : "word");
        if (pec)
        ...
        if (bank)
        {
            if (size == I2C_SMBUS_BLOCK_DATA)
                ...
                fprintf(stderr,
                        "Probing bank %d using bank "
                        "register 0x%02x.\n",
                        bank, bankreg);
        }
        if (range)
        {
            fprintf(stderr, "Probe range limited to 0x%02x-0x%02x.\n", first,
                    last);
        }
    ...
    if (size != I2C_SMBUS_WORD_DATA || even)
    {
        ...
        if (size == I2C_SMBUS_BLOCK_DATA || size == I2C_SMBUS_I2C_BLOCK_DATA)
        {
            ...
            else
            {
                ...
                {
                    i = i2c_smbus_read_i2c_block_data(file, res, 32,
                                                      cblock + res);
                    if (i <= 0)
                    {
                        res = i;
        ...
        for (i = 0; i < 256; i += 16)
        {
            ...
            for (j = 0; j < 16; j++)
            {
                ...
                if (size == I2C_SMBUS_BLOCK_DATA && i + j >= s_length)
                {
                    printf("   ");
                }
                else if (res < 0)
