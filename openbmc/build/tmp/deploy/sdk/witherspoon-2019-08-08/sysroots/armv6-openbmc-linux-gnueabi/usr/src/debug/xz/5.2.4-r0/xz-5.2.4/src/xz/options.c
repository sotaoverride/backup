...
///
static void parse_options(const char* str, const option_map* opts,
                          void (*set)(void* filter_options, unsigned key,
                                      uint64_t value, const char* valuestr),
                          void* filter_options)
{
    if (str == NULL || str[0] == '\0')
        return;
    ...
    while (*name != '\0')
    {
        ...
        if (opts[i].map != NULL)
        {
            ...
            if (opts[i].map[j].name == NULL)
                message_fatal(_("%s: Invalid option value"), value);
        ...
        else
        {
            // value is an integer.
            const uint64_t v =
                str_to_uint64(name, value, opts[i].min, opts[i].max);
...
static void set_delta(void* options, unsigned key, uint64_t value,
...
extern lzma_options_delta* options_delta(const char* str)
{
    static const option_map opts[] = {
        {"dist", NULL, LZMA_DELTA_DIST_MIN, LZMA_DELTA_DIST_MAX},
        {NULL, NULL, 0, 0}};
...
static void set_bcj(void* options, unsigned key, uint64_t value,
                    const char* valuestr lzma_attribute((__unused__)))
{
...
{
    static const option_map opts[] = {{"start", NULL, 0, UINT32_MAX},
                                      {NULL, NULL, 0, 0}};
...
static void set_lzma(void* options, unsigned key, uint64_t value,
                     const char* valuestr)
{
...
extern lzma_options_lzma* options_lzma(const char* str)
{
    static const name_id_map modes[] = {
        {"fast", LZMA_MODE_FAST}, {"normal", LZMA_MODE_NORMAL}, {NULL, 0}};

    static const name_id_map mfs[] = {
        {"hc3", LZMA_MF_HC3}, {"hc4", LZMA_MF_HC4}, {"bt2", LZMA_MF_BT2},
        {"bt3", LZMA_MF_BT3}, {"bt4", LZMA_MF_BT4}, {NULL, 0}};
