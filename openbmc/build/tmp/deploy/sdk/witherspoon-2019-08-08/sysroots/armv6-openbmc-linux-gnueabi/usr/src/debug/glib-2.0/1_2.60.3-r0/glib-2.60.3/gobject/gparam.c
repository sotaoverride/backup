...
#define PSPEC_APPLIES_TO_VALUE(pspec, value)                                   \
    (G_TYPE_CHECK_VALUE_TYPE((value), G_PARAM_SPEC_VALUE_TYPE(pspec)))
...
static void g_param_spec_class_init(GParamSpecClass* class,
                                    gpointer class_data);
static void g_param_spec_init(GParamSpec* pspec, GParamSpecClass* class);
...
static void value_param_copy_value(const GValue* src_value, GValue* dest_value);
...
static gchar* value_param_collect_value(GValue* value, guint n_collect_values,
...
/* --- functions --- */
static inline GParamSpecPrivate* g_param_spec_get_private(GParamSpec* pspec)
{
...
void _g_param_type_init(void)
{
    ...
    type = g_type_register_fundamental(
        G_TYPE_PARAM, g_intern_static_string("GParam"), &param_spec_info,
        &finfo, G_TYPE_FLAG_ABSTRACT);
    g_assert(type == G_TYPE_PARAM);
    g_param_private_offset =
        g_type_add_instance_private(type, sizeof(GParamSpecPrivate));
    g_value_register_transform_func(G_TYPE_PARAM, G_TYPE_PARAM,
...
static void g_param_spec_class_init(GParamSpecClass* class, gpointer class_data)
{
...
static void canonicalize_key(gchar* key)
{
    gchar* p;
...
static gboolean is_canonical(const gchar* key)
{
    const gchar* p;
                               ...
                               GParamFlags flags)
{
    ...
    g_return_val_if_fail(
        G_TYPE_IS_PARAM(param_type) && param_type != G_TYPE_PARAM, NULL);
    g_return_val_if_fail(name != NULL, NULL);
    g_return_val_if_fail((name[0] >= 'A' && name[0] <= 'Z') ||
                             (name[0] >= 'a' && name[0] <= 'z'),
                         NULL);
    g_return_val_if_fail(!(flags & G_PARAM_STATIC_NAME) || is_canonical(name),
                         NULL);
    ...
    if (flags & G_PARAM_STATIC_NAME)
    {
        ...
        if (!is_canonical(pspec->name))
            g_warning(
                "G_PARAM_STATIC_NAME used with non-canonical pspec name: %s",
                pspec->name);
    }
    else
    {
 ...
 */
void g_param_spec_set_qdata_full(GParamSpec* pspec, GQuark quark, gpointer data,
                                 GDestroyNotify destroy)
{
    g_return_if_fail(G_IS_PARAM_SPEC(pspec));
    g_return_if_fail(quark > 0);

    g_datalist_id_set_data_full(&pspec->qdata, quark, data,
...
gboolean g_param_value_defaults(GParamSpec* pspec, GValue* value)
{
    ...
    defaults = G_PARAM_SPEC_GET_CLASS(pspec)->values_cmp(pspec, value,
                                                         &dflt_value) == 0;
 ...
 */
gboolean g_param_value_convert(GParamSpec* pspec, const GValue* src_value,
 ...
 */
gint g_param_values_cmp(GParamSpec* pspec, const GValue* value1,
                        const GValue* value2)
{
    gint cmp;
                                        ...
                                        guint collect_flags)
{
    if (collect_values[0].v_pointer)
    {
        ...
        if (param->g_type_instance.g_class == NULL)
            return g_strconcat(
                "invalid unclassed param spec pointer for value type '",
                G_VALUE_TYPE_NAME(value), "'", NULL);
        else if (!g_value_type_compatible(G_PARAM_SPEC_TYPE(param),
                                          G_VALUE_TYPE(value)))
            return g_strconcat(
                "invalid param spec type '", G_PARAM_SPEC_TYPE_NAME(param),
...
static guint param_spec_pool_hash(gconstpointer key_spec)
{
...
static gboolean param_spec_pool_equals(gconstpointer key_spec_1,
...
GParamSpecPool* g_param_spec_pool_new(gboolean type_prefixing)
{
    ...
    pool->hash_table =
        g_hash_table_new(param_spec_pool_hash, param_spec_pool_equals);
                              ...
                              GType owner_type)
{
    ...
    if (pool && pspec && owner_type > 0 && pspec->owner_type == 0)
    {
        for (p = pspec->name; *p; p++)
        {
            ...
            {
                g_warning(G_STRLOC
                          ": pspec name \"%s\" contains invalid characters",
                          pspec->name);
                return;
            }
        }
...
void g_param_spec_pool_remove(GParamSpecPool* pool, GParamSpec* pspec)
{
    if (pool && pspec)
    {
        ...
        if (g_hash_table_remove(pool->hash_table, pspec))
            ...
            g_warning(G_STRLOC
                      ": attempt to remove unknown pspec '%s' from pool",
                      pspec->name);
        g_mutex_unlock(&pool->mutex);
    }
    else
...
static inline GParamSpec* param_spec_ht_lookup(GHashTable* hash_table,
...
GParamSpec* g_param_spec_pool_lookup(GParamSpecPool* pool,
                                     const gchar* param_name, GType owner_type,
                                     gboolean walk_ancestors)
{
    GParamSpec* pspec;
    gchar* delim;
    ...
    if (pool->type_prefixing && delim[1] == ':')
    {
        ...
        gchar stack_buffer[32],
            *buffer = l < 32 ? stack_buffer : g_new(gchar, l + 1);
        GType type;
        ...
        if (type) /* type==0 isn't a valid type pefix */
        {
            ...
            if ((!walk_ancestors && type != owner_type) ||
                !g_type_is_a(owner_type, type))
            {
                g_mutex_unlock(&pool->mutex);
            ...
            owner_type = type;
            param_name += l + 2;
            pspec = param_spec_ht_lookup(pool->hash_table, param_name,
                                         owner_type, walk_ancestors);
            g_mutex_unlock(&pool->mutex);
...
static void pool_list(gpointer key, gpointer value, gpointer user_data)
{
...
static gint pspec_compare_id(gconstpointer a, gconstpointer b)
{
...
static inline GSList*
    pspec_list_remove_overridden_and_redirected(GSList* plist, GHashTable* ht,
                                                GType owner_type, guint* n_p)
{
    GSList* rlist = NULL;
...
static void pool_depth_list(gpointer key, gpointer value, gpointer user_data)
{
    GParamSpec* pspec = value;
    gpointer* data = user_data;
 ...
 * prerequites.
 */
static void pool_depth_list_for_interface(gpointer key, gpointer value,
                                          gpointer user_data)
{
    GParamSpec* pspec = value;
    gpointer* data = user_data;
 ...
 */
GParamSpec** g_param_spec_pool_list(GParamSpecPool* pool, GType owner_type,
                                    guint* n_pspecs_p)
{
    GParamSpec **pspecs, **p;
    GSList **slists, *node;
    ...
    *n_pspecs_p = 0;
    ...
    for (i = 0; i < d; i++)
        slists[i] = pspec_list_remove_overridden_and_redirected(
            slists[i], pool->hash_table, owner_type, n_pspecs_p);
...
static void param_spec_generic_class_init(gpointer g_class, gpointer class_data)
{
...
static void default_value_set_default(GParamSpec* pspec, GValue* value)
{
    /* value is already zero initialized */
}

static gint default_values_cmp(GParamSpec* pspec, const GValue* value1,
                               const GValue* value2)
{
                                   ...
                                   const GParamSpecTypeInfo* pspec_info)
{
    ...
    cinfo->value_set_default = pspec_info->value_set_default
                                   ? pspec_info->value_set_default
                                   : default_value_set_default;
    cinfo->value_validate = pspec_info->value_validate;
    cinfo->values_cmp =
        pspec_info->values_cmp ? pspec_info->values_cmp : default_values_cmp;
...
GParamSpec* g_value_dup_param(const GValue* value)
{
    ...
    return value->data[0].v_pointer ? g_param_spec_ref(value->data[0].v_pointer)
                                    : NULL;
}
