 ...
 */
DBusUserInfo* _dbus_user_database_lookup(DBusUserDatabase* db, dbus_uid_t uid,
                                         ...
                                         DBusError* error)
{
    ...
    else
    {
        ...
        if (!_dbus_hash_table_insert_string(db->users_by_name, info->username,
                                            info))
        {
...
static dbus_bool_t init_system_db(void)
{
    ...
    if (system_db == NULL)
    {
        ...
        if (!_dbus_user_database_get_uid(system_db, _dbus_getuid(), &info,
                                         &error))
        {
            _dbus_user_database_unref(system_db);
            system_db = NULL;
            ...
            else
            {
                ...
                _dbus_warn("Could not get password database information for "
                           "UID of current process: %s",
                           error.message);
                dbus_error_free(&error);
                return FALSE;
            }
...
DBusUserDatabase* _dbus_user_database_new(void)
{
    ...
    db->users =
        _dbus_hash_table_new(DBUS_HASH_UINTPTR, NULL,
                             (DBusFreeFunction)_dbus_user_info_free_allocated);
    ...
    db->groups =
        _dbus_hash_table_new(DBUS_HASH_UINTPTR, NULL,
                             (DBusFreeFunction)_dbus_group_info_free_allocated);
 ...
 */
dbus_bool_t _dbus_user_database_get_uid(DBusUserDatabase* db, dbus_uid_t uid,
                                        const DBusUserInfo** info,
                                        DBusError* error)
{
