...
class basic_pointerbuf : public BufferT
{
  ...
  public:
    basic_pointerbuf() : base_type()
    {
        this_type::setbuf(0, 0);
    }
    const charT* getnext()
    {
        return this->gptr();
  ...
  protected:
    // VC mistakenly assumes that `setbuf` and other functions are not
    // referenced. Marking those functions with `inline` suppresses the
    // warnings. There must be no harm from marking virtual functions as inline:
    // inline virtual call can be inlined ONLY when the compiler knows the
    // "exact class".
    inline base_type* setbuf(char_type* s, streamsize n);
    inline typename this_type::pos_type
...
typename basic_pointerbuf<charT, BufferT>::pos_type
    basic_pointerbuf<charT, BufferT>::seekoff(off_type off,
...
template <class charT, class BufferT>
typename basic_pointerbuf<charT, BufferT>::pos_type
    basic_pointerbuf<charT, BufferT>::seekpos(pos_type sp,
                                              ::std::ios_base::openmode which)
{
    if (which & ::std::ios_base::out)
        return pos_type(off_type(-1));
