                         ...
                         size_t* outbyteslen, char* mac, size_t* maclen)
{
    ...
    if (!isencrypt)
    {
        ...
        if (NULL ==
            HMAC(digest, key, keylen, inbytes, inbyteslen, calmac, &calmaclen))
        {
            pam_syslog(pamh, LOG_DEBUG, "Failed to verify authentication %d",
                       retval);
            return -1;
        }
        if (!((calmaclen == *maclen) && (memcmp(calmac, mac, calmaclen) == 0)))
        {
            pam_syslog(pamh, LOG_DEBUG,
                       "Authenticated message doesn't match %d, %d", calmaclen,
                       *maclen);
            return -1;
        }
    }
    ...
    if (!retval)
    {
        pam_syslog(pamh, LOG_DEBUG, "EVP_CipherInit_ex failed with %d", retval);
        EVP_CIPHER_CTX_free(ctx);
        return -1;
    }
    if ((retval = EVP_CipherUpdate(ctx, outbytes + outlen, &outEVPlen, inbytes,
                                   inbyteslen)))
    {
        outlen += outEVPlen;
        if ((retval = EVP_CipherFinal(ctx, outbytes + outlen, &outEVPlen)))
        {
            outlen += outEVPlen;
            *outbyteslen = outlen;
        }
        ...
        {
            pam_syslog(pamh, LOG_DEBUG, "EVP_CipherFinal returns with %d",
                       retval);
            EVP_CIPHER_CTX_free(ctx);
            return -1;
        }
    ...
    {
        pam_syslog(pamh, LOG_DEBUG, "EVP_CipherUpdate returns with %d", retval);
        EVP_CIPHER_CTX_free(ctx);
        return -1;
    }
    EVP_CIPHER_CTX_free(ctx);
    ...
    if (isencrypt)
    {
        ...
        if (NULL ==
            HMAC(digest, key, keylen, outbytes, *outbyteslen, mac, maclen))
        {
            pam_syslog(pamh, LOG_DEBUG, "Failed to create authentication %d",
                       retval);
            return -1;
        }
    }
                             ...
                             const char* towhat)
{
    ...
    if (strlen(filename) > (sizeof(tempfilename) - strlen("__XXXXXX") - 1))
    {
    ...
    if (keyfile == NULL)
    {
        pam_syslog(pamh, LOG_DEBUG, "Unable to open key file %s", keyfilename);
        return PAM_AUTHTOK_ERR;
    }
    ...
    { // Create with this settings if file is not present.
    ...
    if (opwfilesize)
    {
        ...
        if (fread(opwfilebuff, 1, opwfilesize, opwfile))
        {
            ...
            HMAC(digest, keybuff, keybuffsize, opwfilebuff + sizeof(*opwmp),
                 opwmp->hashsize, key, &keylen);
            ...
            if (opwmp->datasize != 0)
            {
                ...
                if (encrypt_decrypt_data(
                        ...
                        opwfilebuff + sizeof(*opwmp) + opwmp->hashsize,
                        opwmp->ivsize,
                        opwfilebuff + sizeof(*opwmp) + opwmp->hashsize +
                            opwmp->ivsize,
                        opwmp->datasize + opwmp->padsize, opwptext,
                        &opwptextlen,
                        opwfilebuff + sizeof(*opwmp) + opwmp->hashsize +
                            opwmp->ivsize + opwmp->datasize + opwmp->padsize,
                        &opwmp->macsize) != 0)
                {
                    pam_syslog(pamh, LOG_DEBUG, "Decryption failed");
                    free(pwptext);
                    free(opwptext);
                    free(opwfilebuff);
                    fclose(opwfile);
    ...
    {
        pwptextlen = forwholen + towhatlen + 3 + EVP_CIPHER_block_size(cipher);
        pwptext = malloc(pwptextlen);
        if (pwptext == NULL)
        {
            if (opwfile != NULL)
    ...
    if (!wroteentry)
    {
        ...
        writtensize += snprintf(pwptext + writtensize, pwptextlen - writtensize,
                                "%s:%s\n", forwho, towhat);
    }
    ...
    {
        pam_syslog(pamh, LOG_DEBUG, "Hash genertion failed, bailing out");
        free(pwptext);
        fclose(pwfile);
        err = 1;
        goto done;
    ...
    {
        pam_syslog(pamh, LOG_DEBUG, "IV generation failed, bailing out");
        free(pwptext);
        fclose(pwfile);
        err = 1;
        goto done;
    ...
    if (encrypt_decrypt_data(pamh, 1, cipher, key, keylen, iv,
                             EVP_CIPHER_iv_length(cipher), pwptext, pwptextlen,
                             pwctext, &pwctextlen, mac, &maclen) != 0)
    {
    ...
    if (!err)
    {
        ...
        {
            pam_syslog(pamh, LOG_DEBUG,
                       "password changed for %s in special file", forwho);
        }
        else
        {
            err = 1;
...
int pam_sm_chauthtok(pam_handle_t* pamh, int flags, int argc, const char** argv)
{
    ...
    const char* spec_grp_name = get_option(pamh, "spec_grp_name", argc, argv);
    const char* spec_pass_file = get_option(pamh, "spec_pass_file", argc, argv);
    ...
    if (spec_grp_usr)
    {
        ...
        if (retval = lock_pwdf())
        {
            pam_syslog(pamh, LOG_ERR, "Failed to lock the passwd file");
            return retval;
        }
        retval = update_pass_special_file(pamh, key_file, spec_pass_file, user,
                                          pass_new);
        unlock_pwdf();
        return retval;
    }
