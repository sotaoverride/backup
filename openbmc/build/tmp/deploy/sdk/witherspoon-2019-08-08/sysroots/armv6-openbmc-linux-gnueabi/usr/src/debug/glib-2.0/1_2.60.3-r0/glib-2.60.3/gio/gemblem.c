...
static void g_emblem_get_property(GObject* object, guint prop_id, GValue* value,
                                  GParamSpec* pspec)
{
...
static void g_emblem_set_property(GObject* object, guint prop_id,
...
static void g_emblem_class_init(GEmblemClass* klass)
{
    ...
    g_object_class_install_property(
        gobject_class, PROP_ORIGIN,
        g_param_spec_enum("origin", P_("GEmblem’s origin"),
                          P_("Tells which origin the emblem is derived from"),
                          G_TYPE_EMBLEM_ORIGIN, G_EMBLEM_ORIGIN_UNKNOWN,
                          G_PARAM_CONSTRUCT_ONLY | G_PARAM_READWRITE |
                              G_PARAM_STATIC_STRINGS));

    g_object_class_install_property(
        gobject_class, PROP_ICON,
        g_param_spec_object("icon", P_("The icon of the emblem"),
                            P_("The actual icon of the emblem"), G_TYPE_OBJECT,
                            G_PARAM_CONSTRUCT_ONLY | G_PARAM_READWRITE |
                                G_PARAM_STATIC_STRINGS));
}
...
static gboolean g_emblem_to_tokens(GIcon* icon, GPtrArray* tokens,
                                   gint* out_version)
{
    GEmblem* emblem = G_EMBLEM(icon);
    char* s;
...
static GIcon* g_emblem_from_tokens(gchar** tokens, gint num_tokens,
                                   gint version, GError** error)
{
    ...
    if (version != 0)
    {
        g_set_error(error, G_IO_ERROR, G_IO_ERROR_INVALID_ARGUMENT,
                    _("Can’t handle version %d of GEmblem encoding"), version);
        return NULL;
    }
...
static GVariant* g_emblem_serialize(GIcon* icon)
{
    GEmblem* emblem = G_EMBLEM(icon);
    GVariant* icon_data;
    GEnumValue* origin;
    ...
    origin = g_enum_get_value(g_type_class_peek(G_TYPE_EMBLEM_ORIGIN),
                              emblem->origin);
    result =
        g_variant_new_parsed("('emblem', <(%v, {'origin': <%s>})>)", icon_data,
                             origin ? origin->value_nick : "unknown");
