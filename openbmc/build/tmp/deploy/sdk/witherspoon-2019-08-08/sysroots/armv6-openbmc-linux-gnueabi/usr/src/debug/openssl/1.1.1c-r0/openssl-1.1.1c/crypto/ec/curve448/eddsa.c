...
static c448_error_t oneshot_hash(uint8_t* out, size_t outlen, const uint8_t* in,
                                 size_t inlen)
{
                                       ...
                                       size_t context_len)
{
    ...
    dom[0] =
        (uint8_t)(2 - (prehashed == 0 ? 1 : 0) - (for_prehash == 0 ? 1 : 0));
...
c448_error_t c448_ed448_convert_private_key_to_x448(
    uint8_t x[X448_PRIVATE_BYTES], const uint8_t ed[EDDSA_448_PRIVATE_BYTES])
{
    /* pass the private key through oneshot_hash function */
    /* and keep the first X448_PRIVATE_BYTES bytes */
    return oneshot_hash(x, X448_PRIVATE_BYTES, ed, EDDSA_448_PRIVATE_BYTES);
}

c448_error_t
    c448_ed448_derive_public_key(uint8_t pubkey[EDDSA_448_PUBLIC_BYTES],
...
c448_error_t c448_ed448_sign(uint8_t signature[EDDSA_448_SIGNATURE_BYTES],
                             const uint8_t privkey[EDDSA_448_PRIVATE_BYTES],
...
c448_error_t
    c448_ed448_sign_prehash(uint8_t signature[EDDSA_448_SIGNATURE_BYTES],
                            const uint8_t privkey[EDDSA_448_PRIVATE_BYTES],
                            const uint8_t pubkey[EDDSA_448_PUBLIC_BYTES],
...
c448_error_t
    c448_ed448_verify(const uint8_t signature[EDDSA_448_SIGNATURE_BYTES],
                      const uint8_t pubkey[EDDSA_448_PUBLIC_BYTES],
                      const uint8_t* message, size_t message_len,
                      uint8_t prehashed, const uint8_t* context,
                      uint8_t context_len)
{
    ...
    error = curve448_point_decode_like_eddsa_and_mul_by_ratio(pk_point, pubkey);
