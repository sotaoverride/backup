 ...
 * fdisk_create_disklabel() overwrites this in-memory sector. But it's possible
 * that label driver also uses another buffers, for example GPT reads more
                               ...
                               size_t* nids)
{
    ...
    for (n = 0, i = 0; i < lb->nfields; i++)
    {
        ...
        if ((fdisk_is_details(cxt) &&
            ...
            (id == FDISK_FIELD_SECTORS && fdisk_use_cylinders(cxt)) ||
 ...
 * GPT is composed from two items, PMBR and GPT, n=0 return offset to PMBR and
 * n=1 return offset to GPT. For more details see 'D' expert fdisk command.
 *
 ...
 * function returns 1. Don't forget to use fdisk_reset_labelitem() or
 * fdisk_unref_labelitem().
 *
 * Returns: 0 on success, < 0 on error, 1 on unsupported item, 2 id out of range
 */
int fdisk_get_disklabel_item(struct fdisk_context* cxt, int id,
                             struct fdisk_labelitem* item)
{
    ...
    fdisk_reset_labelitem(item);
    item->id = id;
    DBG(CXT, ul_debugobj(cxt, "asking for disk %s item %d", cxt->label->name,
                         item->id));
 ...
 */
int fdisk_set_partition_type(struct fdisk_context* cxt, size_t partnum,
 ...
 * Returns: 0 on success, otherwise, a corresponding error.
 */
int fdisk_toggle_partition_flag(struct fdisk_context* cxt, size_t partnum,
                                unsigned long flag)
{
    int rc;
    ...
    DBG(CXT, ul_debugobj(cxt, "partition: %zd: toggle: 0x%04lx [rc=%d]",
                         partnum, flag, rc));
    return rc;
}
 ...
 *
 * Returns: 0 on success, 1 reorder unnecessary, otherwise a corresponding
 * error.
 */
...
void fdisk_label_set_disabled(struct fdisk_label* lb, int disabled)
{
    ...
    DBG(LABEL,
        ul_debug("%s label %s", lb->name, disabled ? "DISABLED" : "ENABLED"));
