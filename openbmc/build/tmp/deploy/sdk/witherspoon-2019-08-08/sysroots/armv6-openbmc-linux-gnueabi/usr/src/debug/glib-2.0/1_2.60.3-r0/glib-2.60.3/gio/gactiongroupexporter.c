...
static GVariant* g_action_group_describe_action(GActionGroup* action_group,
                                                const gchar* name)
{
...
static gboolean g_action_group_exporter_dispatch_events(gpointer user_data)
{
    ...
    while (g_hash_table_iter_next(&iter, &key, &value))
    {
         ...
         */
        g_assert(((events & (ACTION_ENABLED_CHANGED_EVENT |
        ...
        if (events & ACTION_ENABLED_CHANGED_EVENT)
        {
            ...
            enabled =
                g_action_group_get_action_enabled(exporter->action_group, name);
        ...
        if (events & ACTION_STATE_CHANGED_EVENT)
        {
            ...
            state =
                g_action_group_get_action_state(exporter->action_group, name);
        ...
        if (events & ACTION_ADDED_EVENT)
        {
            ...
            description =
                g_action_group_describe_action(exporter->action_group, name);
    ...
    g_dbus_connection_emit_signal(
        exporter->connection, NULL, exporter->object_path, "org.gtk.Actions",
        "Changed",
        g_variant_new("(asa{sb}a{sv}a{s(bgav)})", &removes, &enabled_changes,
                      &state_changes, &adds),
        NULL);
...
static void g_action_group_exporter_flush_queue(GActionGroupExporter* exporter)
{
...
static guint g_action_group_exporter_get_events(GActionGroupExporter* exporter,
                                                const gchar* name)
{
    return (gsize)g_hash_table_lookup(exporter->pending_changes, name);
}

static void g_action_group_exporter_set_events(GActionGroupExporter* exporter,
                                               const gchar* name, guint events)
{
    gboolean have_events;
    gboolean is_queued;

    if (events != 0)
        g_hash_table_insert(exporter->pending_changes, g_strdup(name),
    ...
    if (have_events && !is_queued)
    {
        ...
        g_source_set_callback(source, g_action_group_exporter_dispatch_events,
                              exporter, NULL);
        g_source_set_name(source,
                          "[gio] g_action_group_exporter_dispatch_events");
...
static void g_action_group_exporter_action_added(GActionGroup* action_group,
...
static void g_action_group_exporter_action_removed(GActionGroup* action_group,
                                                   const gchar* action_name,
                                                   gpointer user_data)
{
    GActionGroupExporter* exporter = user_data;
...
static void g_action_group_exporter_action_state_changed(
...
static void g_action_group_exporter_action_enabled_changed(
...
static void org_gtk_Actions_method_call(
    ...
    else if (g_str_equal(method_name, "Activate"))
    {
        ...
        if (G_IS_REMOTE_ACTION_GROUP(exporter->action_group))
            g_remote_action_group_activate_action_full(
                G_REMOTE_ACTION_GROUP(exporter->action_group), name, parameter,
                platform_data);
        else
            g_action_group_activate_action(exporter->action_group, name,
                                           parameter);
    ...
    else if (g_str_equal(method_name, "SetState"))
    {
        ...
        if (G_IS_REMOTE_ACTION_GROUP(exporter->action_group))
            g_remote_action_group_change_action_state_full(
                G_REMOTE_ACTION_GROUP(exporter->action_group), name, state,
                platform_data);
        else
            g_action_group_change_action_state(exporter->action_group, name,
                                               state);
...
static void g_action_group_exporter_free(gpointer user_data)
{
    GActionGroupExporter* exporter = user_data;

    g_signal_handlers_disconnect_by_func(
        exporter->action_group, g_action_group_exporter_action_added, exporter);
    g_signal_handlers_disconnect_by_func(
        exporter->action_group, g_action_group_exporter_action_enabled_changed,
        exporter);
    g_signal_handlers_disconnect_by_func(
        exporter->action_group, g_action_group_exporter_action_state_changed,
        exporter);
    g_signal_handlers_disconnect_by_func(exporter->action_group,
                                         g_action_group_exporter_action_removed,
                                         exporter);
...
guint g_dbus_connection_export_action_group(GDBusConnection* connection,
                                            ...
                                            GError** error)
{
    const GDBusInterfaceVTable vtable = {org_gtk_Actions_method_call};
    GActionGroupExporter* exporter;
    guint id;

    if
        G_UNLIKELY(org_gtk_Actions == NULL)
        {
            GError* error = NULL;
            GDBusNodeInfo* info;
            ...
            org_gtk_Actions =
                g_dbus_node_info_lookup_interface(info, "org.gtk.Actions");
    ...
    id = g_dbus_connection_register_object(connection, object_path,
                                           org_gtk_Actions, &vtable, exporter,
                                           g_action_group_exporter_free, error);
    ...
    g_signal_connect(action_group, "action-added",
                     G_CALLBACK(g_action_group_exporter_action_added),
                     exporter);
    g_signal_connect(action_group, "action-removed",
                     G_CALLBACK(g_action_group_exporter_action_removed),
                     exporter);
    g_signal_connect(action_group, "action-state-changed",
                     G_CALLBACK(g_action_group_exporter_action_state_changed),
                     exporter);
    g_signal_connect(action_group, "action-enabled-changed",
                     G_CALLBACK(g_action_group_exporter_action_enabled_changed),
                     exporter);
