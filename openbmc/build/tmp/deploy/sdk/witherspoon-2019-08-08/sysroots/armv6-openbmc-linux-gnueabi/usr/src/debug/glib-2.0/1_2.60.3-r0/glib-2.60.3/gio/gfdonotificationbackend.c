...
#define G_FDO_NOTIFICATION_BACKEND(o)                                          \
    (G_TYPE_CHECK_INSTANCE_CAST((o), G_TYPE_FDO_NOTIFICATION_BACKEND,          \
...
G_DEFINE_TYPE_WITH_CODE(
    GFdoNotificationBackend, g_fdo_notification_backend,
...
static void freedesktop_notification_free(gpointer data)
{
...
static FreedesktopNotification*
    freedesktop_notification_new(GFdoNotificationBackend* backend,
                                 const gchar* id, GNotification* notification)
{
    ...
    n->id = g_strdup(id);
    n->notify_id = 0;
    g_notification_get_default_action(notification, &n->default_action,
                                      &n->default_action_target);

    return n;
}

static FreedesktopNotification* g_fdo_notification_backend_find_notification(
    GFdoNotificationBackend* backend, const gchar* id)
{
    GSList* it;
...
static FreedesktopNotification*
    g_fdo_notification_backend_find_notification_by_notify_id(
        GFdoNotificationBackend* backend, guint32 id)
{
    GSList* it;
...
static void activate_action(GFdoNotificationBackend* backend, const gchar* name,
                            GVariant* parameter)
{
    ...
    if (name)
    {
        if (g_str_has_prefix(name, "app."))
            g_action_group_activate_action(
                G_ACTION_GROUP(g_backend->application), name + 4, parameter);
    }
    else
    {
...
static void notify_signal(GDBusConnection* connection, const gchar* sender_name,
                          ...
                          gpointer user_data)
{
    ...
    if (action)
    {
        ...
        {
            activate_action(backend, n->default_action,
                            n->default_action_target);
        }
        else
        {
 ...
 */
static guchar urgency_from_priority(GNotificationPriority priority)
{
    switch (priority)
    {
...
static void call_notify(GDBusConnection* con, GApplication* app,
                        guint32 replace_id, GNotification* notification,
                        GAsyncReadyCallback callback, gpointer user_data)
{
    ...
    for (i = 0; i < n_buttons; i++)
    {
        ...
        g_variant_builder_add_value(&action_builder,
    ...
    g_variant_builder_add(
        &hints_builder, "{sv}", "desktop-entry",
    ...
    parameters =
        g_variant_new("(susssasa{sv}i)", "", /* app name */
                      replace_id, "",        /* app icon */
                      g_notification_get_title(notification), body ? body : "",
                      &action_builder, &hints_builder, -1); /* expire_timeout */

    g_dbus_connection_call(
        con, "org.freedesktop.Notifications", "/org/freedesktop/Notifications",
        "org.freedesktop.Notifications", "Notify", parameters,
        G_VARIANT_TYPE("(u)"), G_DBUS_CALL_FLAGS_NONE, -1, NULL, callback,
        user_data);
}

static void notification_sent(GObject* source_object, GAsyncResult* result,
                              gpointer user_data)
{
    ...
    val = g_dbus_connection_call_finish(G_DBUS_CONNECTION(source_object),
                                        result, &error);
    if (val)
    {
        ...
        match = g_fdo_notification_backend_find_notification_by_notify_id(
            backend, n->notify_id);
        if (match != NULL)
        {
            backend->notifications =
                g_slist_remove(backend->notifications, match);
    ...
    else
    {
        if (!warning_printed)
        {
            g_warning("unable to send notifications through "
...
static void g_fdo_notification_backend_dispose(GObject* object)
{
    ...
    if (backend->notify_subscription)
    {
        ...
        g_dbus_connection_signal_unsubscribe(session_bus,
                                             backend->notify_subscription);
        backend->notify_subscription = 0;
    }

    if (backend->notifications)
    {
        g_slist_free_full(backend->notifications,
...
static gboolean g_fdo_notification_backend_is_supported(void)
{
...
static void g_fdo_notification_backend_send_notification(
    GNotificationBackend* backend, const gchar* id, GNotification* notification)
{
    ...
    {
        self->notify_subscription = g_dbus_connection_signal_subscribe(
            backend->dbus_connection, "org.freedesktop.Notifications",
    ...
    call_notify(backend->dbus_connection, backend->application, n->notify_id,
                notification, notification_sent, n);
}

static void g_fdo_notification_backend_withdraw_notification(
    GNotificationBackend* backend, const gchar* id)
{
    GFdoNotificationBackend* self = G_FDO_NOTIFICATION_BACKEND(backend);
    FreedesktopNotification* n;
...
static void g_fdo_notification_backend_init(GFdoNotificationBackend* backend)
{
}
    ...
    g_fdo_notification_backend_class_init(GFdoNotificationBackendClass* class)
{
    ...
    GNotificationBackendClass* backend_class =
    ...
    backend_class->send_notification =
        g_fdo_notification_backend_send_notification;
    backend_class->withdraw_notification =
        g_fdo_notification_backend_withdraw_notification;
}
