...
/*
 ...
 aes_ctx cx[1]); aes_rval aes_enc_blk(const unsigned char in_blk[], unsigned
 char out_blk[], const aes_ctx cx[1]);

      aes_rval aes_dec_len(unsigned int blen, aes_ctx cx[1]);
      aes_rval aes_dec_key(const unsigned char in_key[], unsigned int klen,
 aes_ctx cx[1]); aes_rval aes_dec_blk(const unsigned char in_blk[], unsigned
 char out_blk[], const aes_ctx cx[1]);
...
*/
    ...
    defined(CONFIG_SMALL_NO_CRYPTO) /* set tables for the normal encryption    \
                                       round */
...
#if !defined(CONFIG_SMALL) ||                                                  \
    defined(                                                                   \
        CONFIG_SMALL_NO_CRYPTO) /* set tables for the last encryption round */
...
#if !defined(CONFIG_SMALL) ||                                                  \
    defined(CONFIG_SMALL_NO_CRYPTO) /* set tables for the normal decryption    \
                                       round */
...
#if !defined(CONFIG_SMALL) ||                                                  \
    defined(                                                                   \
        CONFIG_SMALL_NO_CRYPTO) /* set tables for the last decryption round */
...
#define FULL 2        /* DO NOT CHANGE */
    ...
    ((BLOCK_SIZE & 3) || BLOCK_SIZE < 16 || BLOCK_SIZE > 32)
...
#define bytes2word(b0, b1, b2, b3)                                             \
    (((uint32_t)(b0) << 24) | ((uint32_t)(b1) << 16) | ((uint32_t)(b2) << 8) | \
     (b3))
#endif
...
#endif
#define bswap_32(x)                                                            \
    ((_lrotl((x), 8) & 0x00ff00ff) | (_lrotl((x), 24) & 0xff00ff00))
#endif
...
(u >> 6)) #define m4  (0x01010101 * BPOLY) #define FFmulX(x) (u = (x) & m1, ((x)
& m2) << 1) ^ ((u - (u >> 7)) & m4)
*/
...
#define no_table(x, box, vf, rf, c)                                            \
    bytes2word(                                                                \
        box[bval(vf(x, 0, c), rf(0, c))], box[bval(vf(x, 1, c), rf(1, c))],    \
        box[bval(vf(x, 2, c), rf(2, c))], box[bval(vf(x, 3, c), rf(3, c))])
...
/* perform forward and inverse column mix operation on four bytes in long word x
 * in */
