...
static struct shobj* load_shobj(const char* name)
{
    ...
    for (ph = map->l_phdr; ph < &map->l_phdr[map->l_phnum]; ++ph)
        if (ph->p_type == PT_LOAD && (ph->p_flags & PF_X))
        {
            ...
            ElfW(Addr) end =
                ((ph->p_vaddr + ph->p_memsz + pagesize - 1) & ~(pagesize - 1));
    ...
    result->expected_size =
        (sizeof(struct gmon_hdr) + 4 + sizeof(struct gmon_hist_hdr) +
         result->kcountsize + 4 + 4 +
         (result->fromssize * sizeof(struct here_cg_arc_record)));
...
#define SCALE_1_TO_1 0x10000L
    ...
    if (result->kcountsize < result->highpc - result->lowpc)
    {
        ...
        if (quot >= SCALE_1_TO_1)
            ...
            result->s_scale =
                ((SCALE_1_TO_1 * 256) / (range / (result->kcountsize / 256)));
        else
            result->s_scale =
                ((SCALE_1_TO_1 * 256) / ((range * 256) / result->kcountsize));
    }
    else
    ...
    if (symtab_entry == NULL && debuglink_entry != NULL)
    {
        ...
        *++cp = '\0';
        ...
        char* workbuf = (char*)alloca(sizeof(usrlibdebug) + (cp - realname) +
        ...
        if (fd2 != -1)
        {
            ...
            if (pread(fd2, &ehdr2, sizeof(ehdr2), 0) != sizeof(ehdr2))
                error(EXIT_FAILURE, errno, _("reading of ELF header failed"));
...
*** The file `%s' is stripped: no detailed analysis possible\n"),
    ...
    else
    {
       ...
       table and the string table.  */
        ...
        result->symbol_map = mmap(NULL, max_offset - min_offset, PROT_READ,
                                  MAP_SHARED | MAP_FILE, symfd, min_offset);
...
static struct profdata* load_profdata(const char* name, struct shobj* shobj)
{
    ...
    result->hist_hdr =
        (struct real_gmon_hist_hdr*)((char*)result->hist + sizeof(uint32_t));
    ...
    result->narcs = *narcsp;
    result->data =
        (struct here_cg_arc_record*)((char*)narcsp + sizeof(uint32_t));
    ...
    if (memcmp(addr, &gmon_hdr, sizeof(struct gmon_hdr)) != 0 ||
        *(uint32_t*)result->hist != GMON_TAG_TIME_HIST ||
        memcmp(result->hist_hdr, &hist_hdr, sizeof(struct gmon_hist_hdr)) !=
            0 ||
        narcsp[-1] != GMON_TAG_CG_ARC)
    {
        error(0, 0, _("`%s' is no correct profile data file for `%s'"), name,
              shobj->name);
        if (do_test)
        {
            if (memcmp(addr, &gmon_hdr, sizeof(struct gmon_hdr)) != 0)
    ...
    result->froms =
        (struct here_fromstruct*)((char*)result->tos + shobj->tossize);
    fromidx = 0;
...
#define obstack_chunk_free free
    ...
    else
    {
       ...
       information contained in the dynamic section of the object.  */
        const ElfW(Sym)* symtab =
            (ElfW(Sym)*)D_PTR(shobj->map, l_info[DT_SYMTAB]);
        const char* strtab = (const char*)D_PTR(shobj->map, l_info[DT_STRTAB]);
       ...
       dynamic symbol table without looking at the section headers.  */
        while ((void*)symtab < (void*)strtab)
        {
                ...
                symtab->st_size != 0)
            {
                ...
                newsym = (struct known_symbol*)obstack_alloc(&shobj->ob_sym,
                                                             sizeof(*newsym));
                if (newsym == NULL)
                    error(EXIT_FAILURE, errno,
                          _("cannot allocate symbol data"));
                ...
                newsym->hidden =
                    (ELFW(ST_VISIBILITY)(symtab->st_other) != STV_DEFAULT);
                newsym->ticks = 0;
                newsym->froms = NULL;
                newsym->tos = NULL;
...
static void add_arcs(struct profdata* profdata)
{
    ...
    for (cnt = 0; cnt < narcs; ++cnt)
    {
        ...
        if (sym_idx != (size_t)-1l)
        {
            ...
            {
                /* We need a new entry.  */
                struct arc_list* newp = (struct arc_list*)obstack_alloc(
                    &ob_list, sizeof(struct arc_list));
        ...
        if (sym_idx != (size_t)-1l)
        {
            ...
            while (runp != NULL &&
                   (runp->idx == (size_t)-1l ||
                    ...
                    data[cnt].self_pc >=
                        (sortsym[runp->idx]->addr + sortsym[runp->idx]->size)))
                runp = runp->next;
...
static void printflat(const void* node, VISIT value, int level)
{
    if (value == leaf || value == postorder)
    {
        ...
        printf("%6.2f%10.2f%9.2f%9" PRIdMAX "%9.2f           %s\n",
               ...
               tick_unit * cumu_ticks, tick_unit * s->ticks, s->calls,
...
static void generate_call_graph(struct profdata* profdata)
{
    ...
    for (cnt = 0; cnt < symidx; ++cnt)
        if (sortsym[cnt]->froms != NULL || sortsym[cnt]->tos != NULL)
        {
            ...
            while (runp != NULL)
            {
                printf("            %8.2f%8.2f%9" PRIdMAX "/%-9" PRIdMAX
                       "   %s",
                       (runp->idx != (size_t)-1l
                            ? sortsym[runp->idx]->ticks * tick_unit
                            : 0.0),
                       0.0, /* FIXME: what's time for the children, recursive */
                       runp->count, sortsym[cnt]->calls,
                       (runp->idx != (size_t)-1l ? sortsym[runp->idx]->name
                                                 : "<UNKNOWN>"));
            ...
            printf("%*s%5.1f%8.2f%8.2f%9" PRIdMAX "         %s [%Zd]\n",
                   (int)(7 - n), " ",
                   total_ticks ? (100.0 * sortsym[cnt]->ticks) / total_ticks
                               : 0,
                   sortsym[cnt]->ticks * tick_unit,
                   0.0, /* FIXME: what's time for the children, recursive */
                   sortsym[cnt]->calls, sortsym[cnt]->name, cnt);
            ...
            while (runp != NULL)
            {
                ...
                if (runp->idx != (size_t)-1l)
                    printf("%-9" PRIdMAX "   %s [%Zd]\n",
                           sortsym[runp->idx]->calls, sortsym[runp->idx]->name,
                           runp->idx);
                else
                    fputs("???         <UNKNOWN>\n\n", stdout);
