                                              ...
                                              int first_arg_type, ...)
{
    ...
    retval = _dbus_message_iter_get_args_valist(iter, error, first_arg_type,
                                                var_args);
    va_end(var_args);
...
static dbus_bool_t check_have_valid_message(DBusMessageLoader* loader)
{
    ...
    if (message == NULL)
    {
        _dbus_warn("didn't load message that was expected to be valid (message "
                   "not popped)");
        goto failed;
    }
...
static dbus_bool_t check_invalid_message(DBusMessageLoader* loader,
                                         DBusValidity expected_validity)
{
    ...
    {
        _dbus_warn("expected message to be corrupted for reason %d and was "
                   "corrupted for %d instead",
...
static dbus_bool_t check_incomplete_message(DBusMessageLoader* loader)
{
    ...
    {
        _dbus_warn("loader corrupted on message that was expected to be valid "
                   "(but incomplete), corruption reason %d",
                   loader->corruption_reason);
        goto failed;
    }
...
static dbus_bool_t check_loader_results(DBusMessageLoader* loader,
                                        DBusValidity expected_validity)
{
                                              ...
                                              DBusValidity expected_validity)
{
    ...
    retval =
        dbus_internal_do_not_use_try_message_data(&data, expected_validity);
                                              ...
                                              DBusValidity expected_validity)
{
    ...
    for (i = 0; i < len; i++)
    {
        ...
        if (!_dbus_string_append_byte(buffer, _dbus_string_get_byte(data, i)))
            goto failed;
...
static dbus_bool_t process_test_subdir(const DBusString* test_base_dir,
                                       const char* subdir,
                                       DBusValidity expected_validity,
...
next:
    while (_dbus_directory_get_next_file(dir, &filename, &error))
    {
        ...
        else
        {
            ...
            {
                printf("SKIP: Could not load %s, message builder language no "
                       "longer supported\n",
                       _dbus_string_get_const_data(&filename));
            }
 ...
 *
 * @param test_data_dir root dir of the test suite data files
 * (top_srcdir/test/data)
...
dbus_bool_t dbus_internal_do_not_use_foreach_message_file(
    const char* test_data_dir, DBusForeachMessageFileFunc func, void* user_data)
{
    DBusString test_directory;
    dbus_bool_t retval;
    ...
    if (!process_test_subdir(&test_directory, "valid-messages", DBUS_VALID,
                             func, user_data))
        goto failed;
...
#define GET_AND_CHECK(iter, typename, literal)                                 \
    ...
    {                                                                          \
        if (dbus_message_iter_get_arg_type(&iter) != DBUS_TYPE_##typename)     \
            _dbus_assert_not_reached(                                          \
                "got wrong argument type from message iter");                  \
...
static void verify_test_message(DBusMessage* message)
{
    ...
    if (!dbus_message_iter_get_args(
            &iter, &error, DBUS_TYPE_INT16, &our_int16, DBUS_TYPE_UINT16,
            &our_uint16, DBUS_TYPE_INT32, &our_int, DBUS_TYPE_UINT32, &our_uint,
            DBUS_TYPE_INT64, &our_int64, DBUS_TYPE_UINT64, &our_uint64,
            DBUS_TYPE_STRING, &our_str, DBUS_TYPE_DOUBLE, &our_double,
            DBUS_TYPE_BOOLEAN, &our_bool, DBUS_TYPE_BYTE, &our_byte_1,
            DBUS_TYPE_BYTE, &our_byte_2, DBUS_TYPE_ARRAY, DBUS_TYPE_UINT32,
            &our_uint32_array, &our_uint32_array_len, DBUS_TYPE_ARRAY,
            DBUS_TYPE_INT32, &our_int32_array, &our_int32_array_len,
            DBUS_TYPE_ARRAY, DBUS_TYPE_UINT64, &our_uint64_array,
            &our_uint64_array_len, DBUS_TYPE_ARRAY, DBUS_TYPE_INT64,
            &our_int64_array, &our_int64_array_len, DBUS_TYPE_ARRAY,
            DBUS_TYPE_DOUBLE, &our_double_array, &our_double_array_len,
            DBUS_TYPE_ARRAY, DBUS_TYPE_BYTE, &our_byte_array,
            &our_byte_array_len, DBUS_TYPE_ARRAY, DBUS_TYPE_BOOLEAN,
            &our_boolean_array, &our_boolean_array_len, DBUS_TYPE_ARRAY,
            DBUS_TYPE_STRING, &our_string_array, &our_string_array_len, 0))
    {
    ...
    if (our_uint32_array_len != 4 || our_uint32_array[0] != 0x12345678 ||
    ...
    if (our_int32_array_len != 4 || our_int32_array[0] != 0x12345678 ||
        our_int32_array[1] != -0x23456781 || our_int32_array[2] != 0x34567812 ||
        our_int32_array[3] != -0x45678123)
        _dbus_assert_not_reached("int array differs");

    if (our_uint64_array_len != 4 || our_uint64_array[0] != 0x12345678 ||
    ...
    if (our_int64_array_len != 4 || our_int64_array[0] != 0x12345678 ||
        our_int64_array[1] != -0x23456781 || our_int64_array[2] != 0x34567812 ||
    ...
    if (our_byte_array[0] != 'a' || our_byte_array[1] != 'b' ||
        our_byte_array[2] != 'c' || our_byte_array[3] != 234)
    ...
    if (our_boolean_array[0] != TRUE || our_boolean_array[1] != FALSE ||
        our_boolean_array[2] != TRUE || our_boolean_array[3] != TRUE ||
...
static void verify_test_message_args_ignored(DBusMessage* message)
{
    ...
    dbus_message_iter_init(message, &iter);
    if (!dbus_message_iter_get_args(&iter, &error, DBUS_TYPE_UINT32, &our_uint,
                                    DBUS_TYPE_INVALID))
    {
        _dbus_warn("error: %s - %s", error.name,
                   (error.message != NULL) ? error.message : "no message");
...
static void verify_test_message_memleak(DBusMessage* message)
{
    ...
    dbus_message_iter_init(message, &iter);
    if (!dbus_message_iter_get_args(
            &iter, &error, DBUS_TYPE_UINT32, &our_uint1, DBUS_TYPE_ARRAY,
            DBUS_TYPE_STRING, &our_string_array1, &our_string_array_len1,
#ifdef HAVE_UNIX_FD_PASSING
            DBUS_TYPE_UNIX_FD, &our_unix_fd1,
#endif
            DBUS_TYPE_UINT32, &our_uint2, DBUS_TYPE_UINT32, &our_uint3,
            DBUS_TYPE_INVALID))
    {
    ...
    if (!dbus_message_iter_get_args(
            ...
            DBUS_TYPE_UNIX_FD, &our_unix_fd1,
#endif
            DBUS_TYPE_UINT32, &our_uint2, DBUS_TYPE_ARRAY, DBUS_TYPE_STRING,
...
dbus_bool_t _dbus_message_test(const char* test_data_dir)
{
    ...
    const char* name1;
    const char* name2;
    const dbus_uint32_t our_uint32_array[] = {0x12345678, 0x23456781,
                                              0x34567812, 0x45678123};
    const dbus_int32_t our_int32_array[] = {0x12345678, -0x23456781, 0x34567812,
                                            -0x45678123};
    const dbus_uint32_t* v_ARRAY_UINT32 = our_uint32_array;
    const dbus_int32_t* v_ARRAY_INT32 = our_int32_array;
    const dbus_uint64_t our_uint64_array[] = {0x12345678, 0x23456781,
                                              0x34567812, 0x45678123};
    const dbus_int64_t our_int64_array[] = {0x12345678, -0x23456781, 0x34567812,
                                            -0x45678123};
    ...
    DBusInitialFDs* initial_fds;
    dbus_bool_t ok;
    char basic_types[] = DBUS_TYPE_BYTE_AS_STRING DBUS_TYPE_BOOLEAN_AS_STRING
        DBUS_TYPE_INT16_AS_STRING DBUS_TYPE_INT32_AS_STRING
            DBUS_TYPE_INT64_AS_STRING DBUS_TYPE_UINT16_AS_STRING
                DBUS_TYPE_UINT32_AS_STRING DBUS_TYPE_UINT64_AS_STRING
                    DBUS_TYPE_DOUBLE_AS_STRING DBUS_TYPE_STRING_AS_STRING;

    initial_fds = _dbus_check_fdleaks_enter();

    message = dbus_message_new_method_call("org.freedesktop.DBus.TestService",
                                           "/org/freedesktop/TestPath",
                                           "Foo.TestInterface", "TestMethod");
    _dbus_assert(dbus_message_has_destination(
        message, "org.freedesktop.DBus.TestService"));
    ...
    if (!dbus_message_set_interface(message, "org.Foo.Bar"))
        _dbus_assert_not_reached("out of memory");
    _dbus_assert(strcmp(dbus_message_get_interface(message), "org.Foo.Bar") ==
                 0);
    ...
    dbus_message_append_args(
        message, DBUS_TYPE_INT16, &v_INT16, DBUS_TYPE_UINT16, &v_UINT16,
        DBUS_TYPE_INT32, &v_INT32, DBUS_TYPE_UINT32, &v_UINT32, DBUS_TYPE_INT64,
        &v_INT64, DBUS_TYPE_UINT64, &v_UINT64, DBUS_TYPE_STRING, &v_STRING,
        DBUS_TYPE_DOUBLE, &v_DOUBLE, DBUS_TYPE_BOOLEAN, &v_BOOLEAN,
        DBUS_TYPE_BYTE, &v_BYTE, DBUS_TYPE_BYTE, &v2_BYTE, DBUS_TYPE_ARRAY,
        DBUS_TYPE_UINT32, &v_ARRAY_UINT32, _DBUS_N_ELEMENTS(our_uint32_array),
        DBUS_TYPE_ARRAY, DBUS_TYPE_INT32, &v_ARRAY_INT32,
        _DBUS_N_ELEMENTS(our_int32_array), DBUS_TYPE_ARRAY, DBUS_TYPE_UINT64,
        &v_ARRAY_UINT64, _DBUS_N_ELEMENTS(our_uint64_array), DBUS_TYPE_ARRAY,
        DBUS_TYPE_INT64, &v_ARRAY_INT64, _DBUS_N_ELEMENTS(our_int64_array),
        DBUS_TYPE_ARRAY, DBUS_TYPE_DOUBLE, &v_ARRAY_DOUBLE,
        _DBUS_N_ELEMENTS(our_double_array), DBUS_TYPE_ARRAY, DBUS_TYPE_BYTE,
        &v_ARRAY_BYTE, _DBUS_N_ELEMENTS(our_byte_array), DBUS_TYPE_ARRAY,
        DBUS_TYPE_BOOLEAN, &v_ARRAY_BOOLEAN,
        _DBUS_N_ELEMENTS(our_boolean_array), DBUS_TYPE_ARRAY, DBUS_TYPE_STRING,
        &v_ARRAY_STRING, _DBUS_N_ELEMENTS(our_string_array),
...
#ifdef HAVE_UNIX_FD_PASSING
    dbus_message_append_args(message, DBUS_TYPE_UNIX_FD, &v_UNIX_FD,
                             DBUS_TYPE_INVALID);
...

    {
        ...
        _dbus_assert(dbus_message_demarshal_bytes_needed(marshalled, len) ==
                     len);
        ...
        _dbus_assert(dbus_message_demarshal_bytes_needed(
                         garbage_header, DBUS_MINIMUM_HEADER_SIZE) == -1);
    }
    ...
    for (i = strlen(basic_types) - 1; i > 0; i--)
    {
        ...
        message = dbus_message_new_method_call("de.ende.test", "/de/ende/test",
                                               "de.ende.Test", "ArtistName");
        _dbus_assert(message != NULL);
        dbus_message_iter_init_append(message, &iter);
        dbus_message_iter_open_container(&iter, DBUS_TYPE_ARRAY, signature,
                                         &array_iter);
        for (some = 0; some < 3; some++)
        {
            if (basic_types[i] == DBUS_TYPE_STRING)
                ...
                dbus_message_iter_append_basic(&array_iter, basic_types[i],
                                               &val);
        }
    ...
    ok = dbus_message_iter_open_container(
        &iter, DBUS_TYPE_ARRAY,
        (DBUS_STRUCT_BEGIN_CHAR_AS_STRING DBUS_TYPE_STRING_AS_STRING
             DBUS_TYPE_STRING_AS_STRING DBUS_STRUCT_END_CHAR_AS_STRING),
        &array_iter);
    _dbus_assert(ok);
    ok = dbus_message_iter_open_container(&array_iter, DBUS_TYPE_STRUCT, NULL,
                                          &struct_iter);
    _dbus_assert(ok);
    s = "peaches";
    ...
    /* signature "uashuash" */
    dbus_message_append_args(
        message, DBUS_TYPE_UINT32, &v_UINT32, DBUS_TYPE_ARRAY, DBUS_TYPE_STRING,
        &v_ARRAY_STRING, _DBUS_N_ELEMENTS(our_string_array),
#ifdef HAVE_UNIX_FD_PASSING
        DBUS_TYPE_UNIX_FD, &v_UNIX_FD,
#endif
        DBUS_TYPE_UINT32, &v1_UINT32, DBUS_TYPE_ARRAY, DBUS_TYPE_STRING,
        &v1_ARRAY_STRING, _DBUS_N_ELEMENTS(our_string_array1),
#ifdef HAVE_UNIX_FD_PASSING
    ...
    /* Load all the sample messages from the message factory */
    {
        ...
        {
            if (!dbus_internal_do_not_use_try_message_data(
                    &mdata.data, mdata.expected_validity))
            {
