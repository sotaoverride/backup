...
static int print_one_thread(struct pdbg_target* thread)
{
    ...
    printf("%c%c%c ", (tstate.active ? 'A' : '.'), c,
           (tstate.quiesced ? 'Q' : '.'));
    return 1;
}
...
static int thread_status_print(void)
{
    ...
    pdbg_for_each_target("thread", core, thread) threads_per_core++;
    ...
    for_each_path_target_class("pib", pib)
    {
        ...
        pdbg_for_each_target("core", pib, core)
        {
            ...
            pdbg_for_each_target("thread", core, thread) count +=
                print_one_thread(thread);
            printf("\n");
        }
    }
...
#define REG_BACKTRACE_FLAG                                                     \
    ("--backtrace", do_backtrace, parse_flag_noarg, false)
...
static int thread_regs_print(struct reg_flags flags)
{
    ...
    for_each_path_target_class("thread", thread)
    {
        ...
        printf("p%d c%d t%d\n", pdbg_target_index(pib), pdbg_target_index(core),
...
OPTCMD_DEFINE_CMD_ONLY_FLAGS(regs, thread_regs_print, reg_flags,
                             (REG_BACKTRACE_FLAG));
