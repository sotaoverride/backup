                ...
                *wildcard_reached_p = TRUE;
                ...
                do
                {
                    ...
                    if (next_wildcard_reached)
                        /* the forthcoming pattern substring up to the next
                         * wildcard has been matched, but a mismatch occoured
                         * for the rest of the pattern, following the next
                         * wildcard. there's no need to advance the current
                         * match position any further if the rest pattern will
                         * not match.
                         */
                        return FALSE;
                } while (*string);
    ...
    switch (pspec->match_type)
    {
        ...
        case G_MATCH_HEAD:
            if (pspec->pattern_length == string_length)
                ...
                return strncmp(pspec->pattern, string, pspec->pattern_length) ==
                       0;
            else
                return TRUE;
        case G_MATCH_TAIL:
            if (pspec->pattern_length)
                return strcmp(pspec->pattern,
                              string +
                                  (string_length - pspec->pattern_length)) == 0;
            else
                return TRUE;
        case G_MATCH_EXACT:
            if (pspec->pattern_length != string_length)
    ...
    *d++ = 0;
    ...
    if (!seen_joker && !more_wildcards)
    {
        if (pspec->pattern[0] == '*')
        {
            ...
            memmove(pspec->pattern, pspec->pattern + 1,
                    --pspec->pattern_length);
