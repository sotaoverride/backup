...
static int read_data(const char* file, void* buf, int buf_len)
{
    ...
    if (tmp1)
    {
        errmsg("file \"%s\" contains too much data (> %d bytes)", file,
               buf_len);
        errno = EINVAL;
        goto out_error;
    }
...
static int read_major(const char* file, int* major, int* minor)
{
    ...
    {
        errno = EINVAL;
        return errmsg("bad major:minor %d:%d in \"%s\"", *major, *minor, file);
    }
 ...
 */
static int dev_get_major(struct libmtd* lib, int mtd_num, int* major,
                         int* minor)
{
...
/**
 * dev_read_pos_ll - read a positive 'long long' value from an MTD device sysfs
 * file.
 * @patt: file pattern to read from
 * @mtd_num: MTD device number
 * @value: the result is stored here
...
static int sysfs_is_supported(struct libmtd* lib)
{
    ...
    while (1)
    {
        ...
        {
            errmsg("invalid entry in %s: \"%s\"", lib->sysfs_mtd,
                   dirent->d_name);
            errno = EINVAL;
            closedir(sysfs_mtd);
            return -1;
        ...
        ret = sscanf(dirent->d_name, MTD_NAME_PATT "%s", &mtd_num, tmp_buf);
        if (ret == 1)
        {
            num = mtd_num;
            break;
...
static inline int mtd_ioctl_error(const struct mtd_dev_info* mtd, int eb,
                                  const char* sreq)
{
    return sys_errmsg("%s ioctl failed for eraseblock %d (mtd%d)", sreq, eb,
                      mtd->mtd_num);
}
...
static int mtd_valid_erase_block(const struct mtd_dev_info* mtd, int eb)
{
    ...
    {
        errmsg("bad eraseblock number %d, mtd%d has %d eraseblocks", eb,
               mtd->mtd_num, mtd->eb_cnt);
        errno = EINVAL;
        return -1;
    }
    return 0;
...
int mtd_erase_multi(libmtd_t desc, const struct mtd_dev_info* mtd, int fd,
                    int eb, int blocks)
{
    int ret;
    struct libmtd* lib = (struct libmtd*)desc;
        ...
        lib->offs64_ioctls == OFFS64_IOCTLS_UNKNOWN)
    {
        ...
        if (errno != ENOTTY || lib->offs64_ioctls != OFFS64_IOCTLS_UNKNOWN)
...
int mtd_torture(libmtd_t desc, const struct mtd_dev_info* mtd, int fd, int eb)
{
    ...
    for (i = 0; i < patt_count; i++)
    {
        ...
        err = mtd_write(desc, mtd, fd, eb, 0, buf, mtd->eb_size, NULL, 0, 0);
        if (err)
            goto out;
        ...
        if (err)
        {
            errmsg("pattern %x checking failed for PEB %d", patterns[i], eb);
            errno = EIO;
            goto out;
        }
    }
             ...
             void* buf, int len)
{
    ...
    {
        errmsg("bad offset %d or length %d, mtd%d eraseblock size is %d", offs,
               len, mtd->mtd_num, mtd->eb_size);
        errno = EINVAL;
        return -1;
    }
    ...
    if (lseek(fd, seek, SEEK_SET) != seek)
        return sys_errmsg("cannot seek mtd%d to offset %lld", mtd->mtd_num,
                          (long long)seek);
    ...
    while (rd < len)
    {
        ...
        if (ret < 0)
            return sys_errmsg(
                "cannot read %d bytes from mtd%d (eraseblock %d, offset %d)",
              ...
              uint8_t mode)
{
    ...
    {
        errmsg("write offset %d is not aligned to mtd%d min. I/O size %d", offs,
               mtd->mtd_num, mtd->subpage_size);
        errno = EINVAL;
        return -1;
    }
    if (len % mtd->subpage_size)
    {
        errmsg("write length %d is not aligned to mtd%d min. I/O size %d", len,
               mtd->mtd_num, mtd->subpage_size);
        errno = EINVAL;
        return -1;
    }
                     ...
                     unsigned int cmd64, unsigned int cmd)
{
    ...
    if (start >= max_offs)
    {
        errmsg("bad page address %" PRIu64
               ", mtd%d has %d eraseblocks (%llu bytes)",
    ...
    {
        errmsg("this system can address only up to address %lu", 0xFFFFFFFFUL);
        errno = EINVAL;
        return -1;
    }
...
int mtd_read_oob(libmtd_t desc, const struct mtd_dev_info* mtd, int fd,
                 uint64_t start, uint64_t length, void* data)
{
    return do_oob_op(desc, mtd, fd, start, length, data, MEMREADOOB64,
                     MEMREADOOB);
}
...
int mtd_write_oob(libmtd_t desc, const struct mtd_dev_info* mtd, int fd,
                  uint64_t start, uint64_t length, void* data)
{
    return do_oob_op(desc, mtd, fd, start, length, data, MEMWRITEOOB64,
                     MEMWRITEOOB);
}
