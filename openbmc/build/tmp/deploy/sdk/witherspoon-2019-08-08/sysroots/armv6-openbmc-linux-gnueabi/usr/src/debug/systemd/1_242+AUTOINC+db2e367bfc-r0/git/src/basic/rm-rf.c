...
int rm_rf_children(int fd, RemoveFlags flags, struct stat* root_dev)
{
    ...
    if (!(flags & REMOVE_PHYSICAL))
    {
        ...
        if (is_physical_fs(&sfs))
        {
            ...
            log_error("Attempted to remove disk file system under \"%s\", and "
                      "we can't allow that.",
                      strna(path));
    ...
    FOREACH_DIRENT_ALL(de, d, return -errno)
    {
        ...
        if (is_dir)
        {
            ...
            if ((flags & REMOVE_SUBVOLUME) && st.st_ino == 256)
            {
                ...
                r = btrfs_subvol_remove_fd(fd, de->d_name,
...
int rm_rf(const char* path, RemoveFlags flags)
{
    ...
    /* For now, don't support dropping subvols when also only dropping
     * directories, since we can't do this race-freely. */
    ...
    if (path_equal_or_files_same(path, "/", AT_SYMLINK_NOFOLLOW))
        return log_error_errno(SYNTHETIC_ERRNO(EPERM),
                               ...
                               "(\"%s\"), and we can't allow that.",
                               path);

    if (FLAGS_SET(flags, REMOVE_SUBVOLUME | REMOVE_ROOT | REMOVE_PHYSICAL))
    {
        /* Try to remove as subvolume first */
        r = btrfs_subvol_remove(path,
                                BTRFS_REMOVE_RECURSIVE | BTRFS_REMOVE_QUOTA);
        if (r >= 0)
            return r;
    ...
    if (fd < 0)
    {
        ...
        if (!(flags & REMOVE_PHYSICAL))
        {
            ...
            if (is_physical_fs(&s))
                return log_error_errno(SYNTHETIC_ERRNO(EPERM),
                                       "Attempted to remove files from a disk "
                                       "file system under \"%s\", refusing.",
                                       path);
        }
