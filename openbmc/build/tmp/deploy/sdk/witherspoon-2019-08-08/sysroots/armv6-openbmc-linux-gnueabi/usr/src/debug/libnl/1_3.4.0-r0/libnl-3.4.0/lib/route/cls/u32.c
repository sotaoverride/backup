...
static struct nla_policy u32_policy[TCA_U32_MAX + 1] = {
    ...
    [TCA_U32_INDEV] = {.type = NLA_STRING, .maxlen = IFNAMSIZ},
...
static int u32_msg_parser(struct rtnl_tc* tc, void* data)
{
    ...
    if (tb[TCA_U32_PCNT])
    {
        ...
        pcnt_size =
            sizeof(struct tc_u32_pcnt) + (sel->nkeys * sizeof(uint64_t));
...
static int u32_clone(void* _dst, void* _src)
{
    ...
    if (src->cu_mark && !(dst->cu_mark = nl_data_clone(src->cu_mark)))
        return -NLE_NOMEM;
                           ...
                           struct rtnl_u32* u)
{
    ...
    if (sel->flags & (TC_U32_OFFSET | TC_U32_VAROFFSET))
    {
        ...
        if (sel->flags & TC_U32_VAROFFSET)
            nl_dump(p, " variable (at %u & 0x%x) >> %u", sel->offoff,
                    ntohs(sel->offmask), sel->offshift);
    }
...
#undef PRINT_FLAG
    ...
    for (i = 0; i < sel->nkeys; i++)
    {
        ...
        if (p->dp_type == NL_DUMP_STATS && (u->cu_mask & U32_ATTR_PCNT))
        {
                             ...
                             struct nl_dump_params* p)
{
    ...
    if (u->cu_mask & U32_ATTR_HASH)
        nl_dump(p, " ht key 0x%x hash 0x%u", TC_U32_USERHTID(u->cu_hash),
                TC_U32_HASH(u->cu_hash));
...
int rtnl_u32_set_hashmask(struct rtnl_cls* cls, uint32_t hashmask,
                          uint32_t offset)
{
    struct rtnl_u32* u;
    struct tc_u32_sel* sel;
...
int rtnl_u32_set_selector(struct rtnl_cls* cls, int offoff, uint32_t offmask,
                          char offshift, uint16_t off, char flags)
{
    struct rtnl_u32* u;
    struct tc_u32_sel* sel;
 ...
 *
 */
int rtnl_u32_add_key(struct rtnl_cls* cls, uint32_t val, uint32_t mask, int off,
                     int offmask)
{
    struct tc_u32_sel* sel;
    struct rtnl_u32* u;
 ...
 *
 */
int rtnl_u32_get_key(struct rtnl_cls* cls, uint8_t index, uint32_t* val,
                     uint32_t* mask, int* off, int* offmask)
{
    struct tc_u32_sel* sel;
    struct rtnl_u32* u;
                           ...
                           int off, int offmask)
{
    ...
    return rtnl_u32_add_key(cls, htonl((uint32_t)val << shift),
                            htonl((uint32_t)mask << shift), off & ~3, offmask);
}
...
int rtnl_u32_add_key_uint32(struct rtnl_cls* cls, uint32_t val, uint32_t mask,
                            int off, int offmask)
{
    return rtnl_u32_add_key(cls, htonl(val), htonl(mask), off & ~3, offmask);
}
                              ...
                              uint8_t bitmask, int off, int offmask)
{
    ...
    for (i = 1; i <= 4; i++)
    {
        if (32 * i - bitmask <= 0)
        {
            if ((err = rtnl_u32_add_key(cls, addr->s6_addr32[i - 1], 0xFFFFFFFF,
                                        off + 4 * (i - 1), offmask)) < 0)
                return err;
        }
        else if (32 * i - bitmask < 32)
        {
            ...
            if ((err =
                     rtnl_u32_add_key(cls, addr->s6_addr32[i - 1], htonl(mask),
                                      off + 4 * (i - 1), offmask)) < 0)
                return err;
        }
