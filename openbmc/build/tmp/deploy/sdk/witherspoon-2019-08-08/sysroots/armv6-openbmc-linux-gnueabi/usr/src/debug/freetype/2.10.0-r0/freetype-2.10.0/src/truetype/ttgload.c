 ...
 */
FT_LOCAL_DEF(void)
TT_Get_VMetrics(TT_Face face, FT_UInt idx, FT_Pos yMax, FT_Short* tsb,
                FT_UShort* ah)
{
...
static FT_Error tt_get_metrics(TT_Loader loader, FT_UInt glyph_index)
{
    ...
    TT_Get_HMetrics(face, glyph_index, &left_bearing, &advance_width);
    TT_Get_VMetrics(face, glyph_index, loader->bbox.yMax, &top_bearing,
                    &advance_height);
...
static void tt_get_metrics_incr_overrides(TT_Loader loader, FT_UInt glyph_index)
{
    TT_Face face = loader->face;
        ...
        face->root.internal->incremental_interface->funcs->get_glyph_metrics)
    {
        ...
        error = face->root.internal->incremental_interface->funcs
                    ->get_glyph_metrics(
...
TT_Access_Glyph_Frame(TT_Loader loader, FT_UInt glyph_index, FT_ULong offset,
                      FT_UInt byte_count)
{
    FT_Error error;
    FT_Stream stream = loader->stream;
...
TT_Load_Simple_Glyph(TT_Loader load)
{
    ...
    if (IS_HINTED(load->load_flags))
    {
        ...
        error = Update_Max(load->exec->memory, &tmp, sizeof(FT_Byte),
                           (void*)&load->exec->glyphIns, n_ins);
...
TT_Load_Composite_Glyph(TT_Loader loader)
{
    ...
    FT_TRACE5(
        ("  %d component%s\n", num_subglyphs, num_subglyphs > 1 ? "s" : ""));
...

    {
        ...
        /*                                                   */
        loader->ins_pos = (FT_ULong)(FT_STREAM_POS() + p - limit);
    }
...
static void tt_prepare_zone(TT_GlyphZone zone, FT_GlyphLoad load,
                            FT_UInt start_point, FT_UInt start_contour)
{
    zone->n_points = (FT_UShort)load->outline.n_points - (FT_UShort)start_point;
    zone->n_contours = load->outline.n_contours - (FT_Short)start_contour;
...
static FT_Error TT_Process_Simple_Glyph(TT_Loader loader)
{
        ...
        FT_IS_VARIATION(FT_FACE(loader->face)))
    {
        ...
        error = TT_Vary_Apply_Glyph_Deltas(loader->face, loader->glyph_index,
                                           outline, (FT_UInt)n_points);
...

    {
        ...
        if (driver->interpreter_version == TT_INTERPRETER_VERSION_38)
        {
             ...
             */
            if (IS_HINTED(loader->load_flags))
                x_scale_factor = sph_test_tweak_x_scaling(
                    face, family, ppem, style, loader->glyph_index);
            /* scale the glyph */
                ...
                x_scale_factor != 1000)
            {
                ...
                if (x_scale_factor != 1000 && ppem > 11)
                    FT_Outline_EmboldenXY(
                        outline, FT_MulFix(1280 * ppem, 1000 - x_scale_factor),
                        0);
                do_scale = TRUE;
            }
        }
                                               ...
                                               FT_UInt num_base_points)
{
    ...
    current.n_points = gloader->base.outline.n_points - (short)num_base_points;

    have_scale =
        FT_BOOL(subglyph->flags &
    ...
    else
    {
        ...
        y = FT_MulFix( y, n );
             ...
             */
            FT_Fixed mac_xscale =
                FT_Hypot(subglyph->transform.xx, subglyph->transform.xy);
            FT_Fixed mac_yscale =
                FT_Hypot(subglyph->transform.yy, subglyph->transform.yx);
        ...
        if (!(loader->load_flags & FT_LOAD_NO_SCALE))
        {
            ...
            if (subglyph->flags & ROUND_XY_TO_GRID)
            {
                ...
                {
                    /*
                     * We round the horizontal offset only if there is hinting
                     * along the x axis; this corresponds to integer advance
                     * width values.
                     *
                     * Theoretically, a glyph's bytecode can toggle ClearType's
                     * `backward compatibility' mode, which would allow
                     * modification of the advance width.  In reality, however,
                     * applications neither allow nor expect modified advance
                     * widths if subpixel rendering is active.
                     *
                     */
                    if (driver->interpreter_version ==
                                           ...
                                           FT_UInt start_contour)
{
    ...
    error =
        FT_GLYPHLOADER_CHECK_POINTS(loader->gloader, outline->n_points + 4, 0);
    if (error)
        return error;
...

    {
        ...
        if (FT_STREAM_SEEK(loader->ins_pos) || FT_READ_USHORT(n_ins))
            return error;
        ...
        if (n_ins > max_ins)
        {
            ...
            error = Update_Max(loader->exec->memory, &tmp, sizeof(FT_Byte),
                               (void*)&loader->exec->glyphIns, n_ins);
    ...
    tt_prepare_zone(&loader->zone, &loader->gloader->base, start_point,
                    start_contour);
...
static void tt_loader_set_pp(TT_Loader loader)
{
    ...
    if (driver->interpreter_version == TT_INTERPRETER_VERSION_38)
    {
        subpixel_hinting = loader->exec ? loader->exec->subpixel_hinting : 0;
        grayscale = loader->exec ? loader->exec->grayscale : 0;
    }
#endif
#ifdef TT_SUPPORT_SUBPIXEL_HINTING_MINIMAL
    if (driver->interpreter_version == TT_INTERPRETER_VERSION_40)
    {
        subpixel_hinting =
            loader->exec ? loader->exec->subpixel_hinting_lean : 0;
        grayscale = loader->exec ? loader->exec->grayscale_cleartype : 0;
    }
#endif
 ...
 *   TT_Loader object.
 */
static FT_Error load_truetype_glyph(TT_Loader loader, FT_UInt glyph_index,
                                    FT_UInt recurse_count, FT_Bool header_only)
{
    FT_Error error = FT_Err_Ok;
    ...
    if (face->root.internal->incremental_interface)
    {
        ...
        FT_ZERO(&inc_stream);
        FT_Stream_OpenMemory(&inc_stream, glyph_data.pointer,
    ...
    if (loader->byte_len > 0)
    {
        ...
        if (!face->glyf_offset && !face->root.internal->incremental_interface)
#else
        if (!face->glyf_offset)
#endif /* FT_CONFIG_OPTION_INCREMENTAL */
        {
    ...
    if (loader->byte_len == 0 || loader->n_contours == 0)
    {
            ...
            FT_IS_VARIATION(FT_FACE(face)))
        {
            ...
            error = TT_Vary_Apply_Glyph_Deltas(
                loader->face, glyph_index, &outline, (FT_UInt)outline.n_points);
            if (error)
                goto Exit;
    ...
    else if (loader->n_contours < 0)
    {
        ...
        if (FT_List_Find(&loader->composites, FT_UINT_TO_POINTER(glyph_index)))
        {
            ...
            FT_IS_VARIATION(FT_FACE(face)))
        {
            ...
            if (FT_SET_ERROR(TT_Vary_Apply_Glyph_Deltas(
                    face, glyph_index, &outline, (FT_UInt)outline.n_points)))
                goto Exit1;
...

        {
            ...
            for (n = 0; n < num_subglyphs; n++)
            {
                ...
                error = load_truetype_glyph(loader, (FT_UInt)subglyph->index,
                ...
                error = TT_Process_Composite_Component(
                    loader, subglyph, start_point, num_base_points);
                if (error)
                    goto Exit;
            }
            ...
            {
                error = TT_Process_Composite_Glyph(loader, start_point,
                                                   start_contour);
                if (error)
                    goto Exit;
            }
...
static FT_Error compute_glyph_metrics(TT_Loader loader, FT_UInt glyph_index)
{
        ...
        !(loader->load_flags & FT_LOAD_COMPUTE_METRICS))
    {
        ...
        widthp = tt_face_get_device_metrics(face, size->metrics->x_ppem,
                                            glyph_index);
        ...
        if (driver->interpreter_version == TT_INTERPRETER_VERSION_38)
        {
            ...
            if (widthp && ((ignore_x_mode && loader->exec->compatible_widths) ||
                           !ignore_x_mode || SPH_OPTION_BITMAP_WIDTHS))
    ...
    /* create some metrics manually                                   */
    {
        ...
        {
            top = (FT_Short)FT_DivFix(loader->pp3.y - bbox.yMax, y_scale);
        ...
        else
        {
            ...
            height =
                (FT_Short)FT_DivFix(SUB_LONG(bbox.yMax, bbox.yMin), y_scale);
...
#ifdef FT_CONFIG_OPTION_INCREMENTAL
        {
            ...
            if (incr && incr->funcs->get_glyph_metrics)
            {
                ...
                error = incr->funcs->get_glyph_metrics(
                    incr->object, glyph_index, TRUE, &incr_metrics);
                if (error)
                    return error;
...
static FT_Error load_sbit_image(TT_Size size, TT_GlyphSlot glyph,
                                FT_UInt glyph_index, FT_Int32 load_flags)
{
    ...
    error = sfnt->load_sbit_image(face, size->strike_index, glyph_index,
                                  (FT_UInt)load_flags, stream, &glyph->bitmap,
                                  &sbit_metrics);
    if (!error)
    {
...
static FT_Error tt_loader_init(TT_Loader loader, TT_Size size,
                               TT_GlyphSlot glyph, FT_Int32 load_flags,
    ...
    if (IS_HINTED(load_flags) && !glyf_table_only)
    {
      ...
      FT_Bool  gray_cleartype;
        ...
        if (driver->interpreter_version == TT_INTERPRETER_VERSION_40)
        {
            ...
            exec->vertical_lcd_lean = FT_BOOL(
                subpixel_hinting_lean && (load_flags & FT_LOAD_TARGET_LCD_V));
        }
        else
        {
        ...
        if (driver->interpreter_version == TT_INTERPRETER_VERSION_38)
        {
            ...
            exec->vertical_lcd = FT_BOOL(FT_LOAD_TARGET_MODE(load_flags) !=
                                         TT_LOAD_VERTICAL_LCD);
            exec->subpixel_positioned = FT_BOOL(
                FT_LOAD_TARGET_MODE(load_flags) != TT_LOAD_SUBPIXEL_POSITIONED);
            exec->gray_cleartype = FT_BOOL(FT_LOAD_TARGET_MODE(load_flags) !=
...
{
    FT_List_Finalize(&loader->composites, NULL, loader->face->root.memory,
                     NULL);
}
...
TT_Load_Glyph(TT_Size size, TT_GlyphSlot glyph, FT_UInt glyph_index,
              FT_Int32 load_flags)
{
    FT_Error error;
    TT_LoaderRec loader;
...
#define IS_DEFAULT_INSTANCE                                                    \
    (!(FT_IS_NAMED_INSTANCE(glyph->face) || FT_IS_VARIATION(glyph->face)))
#else
#define IS_DEFAULT_INSTANCE 1
    ...
    if (size->strike_index != 0xFFFFFFFFUL &&
        (load_flags & FT_LOAD_NO_BITMAP) == 0 && IS_DEFAULT_INSTANCE)
    {
        ...
        if (FT_ERR_EQ(error, Missing_Bitmap))
        {
            ...
            if (!FT_IS_SCALABLE(glyph->face))
            {
                ...
                TT_Get_HMetrics(face, glyph_index, &left_bearing,
                                &advance_width);
                TT_Get_VMetrics(face, glyph_index, 0, &top_bearing,
                                &advance_height);
    ...
    /*                                                              */
    if (!(load_flags & FT_LOAD_NO_SCALE) && size->metrics->y_ppem < 24)
