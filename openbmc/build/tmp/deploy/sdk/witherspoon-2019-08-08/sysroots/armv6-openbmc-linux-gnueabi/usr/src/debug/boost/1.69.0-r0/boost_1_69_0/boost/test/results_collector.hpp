...
///
/// This function is a placeholder where user can set a breakpoint in debugger
/// to catch the very first assertion failure in each test case
// ************************************************************************** //
...
/// an instance of the class. In addition summary conclusion methods are
/// presented to generate simple answer to pass/fail question
...
class BOOST_TEST_DECL test_results
{
  public:
    ...
    typedef BOOST_READONLY_PROPERTY(counter_t,
                                    (results_collector_t)(test_results)(
                                        results_collect_helper)) counter_prop;
    /// Type representing boolean like public property
    typedef BOOST_READONLY_PROPERTY(
        bool,
        (results_collector_t)(test_results)(results_collect_helper)) bool_prop;
    ...
    counter_prop
        p_duration_microseconds; //!< Duration of the test in microseconds
    bool_prop
        p_aborted; //!< Indicates that the test unit execution has been aborted
    bool_prop
        p_skipped; //!< Indicates that the test unit execution has been skipped
    ...
    ///
    /// For test suites, this indicates if the test suite itself has been marked
    /// as skipped, and not if the test suite contains any skipped test.
    bool skipped() const;
    ...
    ///
    /// This methhod return one of the result codes defined in @c
    /// boost/cstdlib.hpp
    /// @returns
    ...
    //!
    //! Only the counters are updated and the @c p_aborted and @c p_skipped are
    //! left unchanged.
...
///
/// This class collects and combines the results of the test unit during the
/// execution of the test tree. The results_collector_t::results() function
/// combines the test results on a subtree of the test tree.
///
