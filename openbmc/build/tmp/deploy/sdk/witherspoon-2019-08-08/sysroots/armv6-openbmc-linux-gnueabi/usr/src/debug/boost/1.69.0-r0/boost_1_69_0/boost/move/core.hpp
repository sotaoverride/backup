...
// Move emulation rv breaks standard aliasing rules so add workarounds for some
// compilers
...
class BOOST_RV_ATTRIBUTE_MAY_ALIAS rv
    : public ::boost::move_detail::if_c<
          ::boost::move_detail::is_class<T>::value, T,
          ::boost::move_detail::nat>::type
{
    rv();
    ~rv() throw();
    rv(rv const&);
...
struct has_move_emulation_disabled
{
    static const bool value =
        !::boost::move_detail::has_move_emulation_enabled_impl<T>::value;
};
...
#define BOOST_RV_REF_BEG ::boost::rv < //
...
#define BOOST_FWD_REF(TYPE) const TYPE& //

#define BOOST_COPY_ASSIGN_REF(TYPE) const ::boost::rv<TYPE>& //

#define BOOST_COPY_ASSIGN_REF_BEG const ::boost::rv < //
...
#define BOOST_CATCH_CONST_RLVALUE(TYPE) const ::boost::rv<TYPE>& //
...
//! This macro marks a type as movable but not copyable, disabling copy
//! construction and assignment. The user will need to write a move
//! constructor/assignment as explained in the documentation to fully write a
...
//! The user will need to write a move constructor/assignment and a copy
//! assignment as explained in the documentation to fully write a copyable and
//! movable class.
#define BOOST_COPYABLE_AND_MOVABLE(TYPE) //
...
//! The C++11 Standard allows implicit move returns when the object to be
//! returned is designated by a lvalue and:
//!   - The criteria for elision of a copy operation are met OR
//!   - The criteria would be met save for the fact that the source object is a
//!   function parameter
//!
//! For C++11 conforming compilers this macros only yields to REF:
//! <code>return BOOST_MOVE_RET(RET_TYPE, REF);</code> -> <code>return
//! REF;</code>
//!
...
//! compilers, a move will be performed even if the C++11 standard does not
//! allow it (e.g. returning a static variable). The user is responsible for
//!using this macro only to return local objects that met C++11 criteria.
...
template <class Ret, class T>
BOOST_MOVE_FORCEINLINE typename ::boost::move_detail::enable_if_c<
    ::boost::move_detail::is_lvalue_reference<Ret>::value, T&>::type
    move_return(T& x) BOOST_NOEXCEPT
{
    return x;
}
...
template <class Ret, class T>
BOOST_MOVE_FORCEINLINE typename ::boost::move_detail::enable_if_c<
    !::boost::move_detail::is_lvalue_reference<Ret>::value, Ret&&>::type
...
//!
//! When implementing the move constructor, in C++03 compilers the moved-from
//! argument must be cast to the base type before calling `::boost::move()` due
//! to rvalue reference limitations.
//!
//! In C++11 compilers the cast from a rvalue reference of a derived type to a
//! rvalue reference of a base type is implicit.
#define BOOST_MOVE_BASE(BASE_TYPE, ARG) ::boost::move((BASE_TYPE&)(ARG))
//
