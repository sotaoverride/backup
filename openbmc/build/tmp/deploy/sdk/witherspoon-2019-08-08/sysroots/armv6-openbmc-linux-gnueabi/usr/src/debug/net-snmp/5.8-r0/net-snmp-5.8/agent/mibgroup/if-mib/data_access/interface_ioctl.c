...
#include "ip-mib/data_access/ipaddress_ioctl.h"
     ...
     */
    static int _ioctl_get(int fd, int which, struct ifreq* ifrq,
                          const char* name)
{
    int ourfd = -1, rc = 0;

    DEBUGMSGTL(
        ("verbose:access:interface:ioctl", "ioctl %d for %s\n", which, name));
 ...
 */
int netsnmp_access_interface_ioctl_physaddr_get(
    int fd, netsnmp_interface_entry* ifentry)
{
    struct ifreq ifrq;
    int rc = 0;
    ...
    else
    {
        ...
        else
        {
            ...
            /*
             * arphrd defines vary greatly. ETHER seems to be the only common
             * one
             */
#ifdef ARPHRD_ETHER
    ...
    *flags &= ~NETSNMP_INTERFACE_FLAGS_HAS_IPV4;
    ...
    for (i = 0; i < interfaces; ++i, ++ifrp)
    {
        ...
        else
        {
            ...
            if (if_index != (int)netsnmp_access_interface_ioctl_ifindex_get(
                                sd, ifrp->ifr_name))
                continue;
        }
