...
static int dupfd_cloexec(int parent_fd)
{
    int fd, errsv;
...
static void child_setup(gpointer user_data)
{
    ...
    if (child_data->needdup_fd_assignments)
    {
        ...
        {
            gint parent_fd =
                g_array_index(child_data->needdup_fd_assignments, int, i);
            gint new_parent_fd;
        ...
        for (i = 0; i < child_data->needdup_fd_assignments->len; i += 2)
        {
            gint parent_fd =
                g_array_index(child_data->needdup_fd_assignments, int, i);
            gint child_fd =
                g_array_index(child_data->needdup_fd_assignments, int, i + 1);
...
static GInputStream* platform_input_stream_from_spawn_fd(gint fd)
{
    if (fd < 0)
        return NULL;
...
static GOutputStream* platform_output_stream_from_spawn_fd(gint fd)
{
    if (fd < 0)
        return NULL;
...
#ifdef G_OS_UNIX
static gint unix_open_file(const char* filename, gint mode, GError** error)
{
    gint my_fd;
...
static void g_subprocess_set_property(GObject* object, guint prop_id,
...
static gboolean g_subprocess_exited(GPid pid, gint status, gpointer user_data)
{
                              ...
                              GError** error)
{
    ...
    else if (self->launcher)
    {
        ...
        {
            child_data.fds[0] = close_fds[0] =
                unix_open_file(self->launcher->stdin_path, O_RDONLY, error);
    ...
    else if (self->launcher)
    {
        ...
        {
            child_data.fds[1] = close_fds[1] = unix_open_file(
                self->launcher->stdout_path, O_CREAT | O_WRONLY, error);
    ...
    else if (self->launcher)
    {
        ...
        {
            child_data.fds[2] = close_fds[2] = unix_open_file(
                self->launcher->stderr_path, O_CREAT | O_WRONLY, error);
    ...
    if (self->launcher)
    {
        ...
        child_data.needdup_fd_assignments =
            self->launcher->needdup_fd_assignments;
    }
#endif
...
#ifdef G_OS_UNIX
    child_data.child_setup_func =
        self->launcher ? self->launcher->child_setup_func : NULL;
    child_data.child_setup_data =
        self->launcher ? self->launcher->child_setup_user_data : NULL;
#endif

    success = g_spawn_async_with_pipes(
        self->launcher ? self->launcher->cwd : NULL, self->argv,
        self->launcher ? self->launcher->envp : NULL, spawn_flags,
#ifdef G_OS_UNIX
        child_setup, &child_data,
#else
        NULL, NULL,
#endif
        &self->pid, pipe_ptrs[0], pipe_ptrs[1], pipe_ptrs[2], error);
...

    {
        ...
        s = g_snprintf(self->identifier, sizeof self->identifier,
                       "%" G_GUINT64_FORMAT, identifier);
...
static void g_subprocess_finalize(GObject* object)
{
    GSubprocess* self = G_SUBPROCESS(object);
...
static void g_subprocess_init(GSubprocess* self)
{
...
static void g_subprocess_class_init(GSubprocessClass* class)
{
    ...
    g_object_class_install_property(
        ...
        g_param_spec_flags("flags", P_("Flags"), P_("Subprocess flags"),
                           G_TYPE_SUBPROCESS_FLAGS, 0,
                           G_PARAM_WRITABLE | G_PARAM_CONSTRUCT_ONLY |
                               G_PARAM_STATIC_STRINGS));
    g_object_class_install_property(
        gobject_class, PROP_ARGV,
 ...
 * Since: 2.40
 */
GSubprocess* g_subprocess_new(GSubprocessFlags flags, GError** error,
                              const gchar* argv0, ...)
{
    GSubprocess* result;
    GPtrArray* args;
 ...
 * @argv: (array zero-terminated=1) (element-type filename): commandline
 ...
 * Since: 2.40
 */
GSubprocess* g_subprocess_newv(const gchar* const* argv, GSubprocessFlags flags,
                               GError** error)
{
    g_return_val_if_fail(argv != NULL && argv[0] != NULL && argv[0][0] != '\0',
                         NULL);

    return g_initable_new(G_TYPE_SUBPROCESS, NULL, error, "argv", argv, "flags",
                          flags, NULL);
}
...
static gboolean slist_remove_if_present(GSList** list, gconstpointer data)
{
    GSList *l, *prev;
...
static void g_subprocess_wait_cancelled(GCancellable* cancellable,
                                        gpointer user_data)
{
 ...
 * Since: 2.40
 */
void g_subprocess_wait_async(GSubprocess* subprocess, GCancellable* cancellable,
                             GAsyncReadyCallback callback, gpointer user_data)
{
    ...
    if (subprocess->pid)
    {
        ...
        subprocess->pending_waits =
            g_slist_prepend(subprocess->pending_waits, task);
        task = NULL;
    }
 ...
 * Since: 2.40
 */
gboolean g_subprocess_wait_finish(GSubprocess* subprocess, GAsyncResult* result,
                                  GError** error)
{
 ...
 * operations.
 */
static void g_subprocess_sync_setup(void)
{
    g_main_context_push_thread_default(g_main_context_new());
}

static void g_subprocess_sync_done(GObject* source_object, GAsyncResult* result,
                                   gpointer user_data)
{
...
static void g_subprocess_sync_complete(GAsyncResult** result)
{
 ...
 * Since: 2.40
 */
gboolean g_subprocess_wait(GSubprocess* subprocess, GCancellable* cancellable,
                           GError** error)
{
    ...
    g_subprocess_wait_async(subprocess, cancellable, g_subprocess_sync_done,
                            &result);
...
static gboolean g_subprocess_actually_send_signal(gpointer user_data)
{
...
static void g_subprocess_dispatch_signal(GSubprocess* subprocess,
                                         gint signalnum)
{
    ...
    g_main_context_invoke_full(GLIB_PRIVATE_CALL(g_get_worker_context)(),
                               ...
                               g_slice_dup(SignalRecord, &signal_record), NULL);
}
...
gboolean g_subprocess_get_successful(GSubprocess* subprocess)
{
    ...
    return WIFEXITED(subprocess->status) &&
           WEXITSTATUS(subprocess->status) == 0;
#else
...
static void g_subprocess_communicate_made_progress(GObject* source_object,
                                                   GAsyncResult* result,
                                                   gpointer user_data)
{
    ...
    if (source == subprocess->stdin_pipe || source == state->stdout_buf ||
        source == state->stderr_buf)
    {
        if (g_output_stream_splice_finish((GOutputStream*)source, result,
                                          &error) == -1)
            goto out;
...
static gboolean g_subprocess_communicate_cancelled(GCancellable* cancellable,
                                                   gpointer user_data)
{
...
static void g_subprocess_communicate_state_free(gpointer data)
{
...
static CommunicateState* g_subprocess_communicate_internal(
    GSubprocess* subprocess, gboolean add_nul, GBytes* stdin_buf,
    GCancellable* cancellable, GAsyncReadyCallback callback, gpointer user_data)
{
    ...
    if (cancellable)
    {
        ...
        g_source_set_callback(state->cancellable_source,
                              ...
                              state, NULL);
        g_source_attach(state->cancellable_source,
    ...
    if (subprocess->stdin_pipe)
    {
        ...
        g_output_stream_splice_async(
            subprocess->stdin_pipe, (GInputStream*)state->stdin_buf,
    ...
    if (subprocess->stdout_pipe)
    {
        ...
        g_output_stream_splice_async(
            (GOutputStream*)state->stdout_buf, subprocess->stdout_pipe,
            G_OUTPUT_STREAM_SPLICE_CLOSE_SOURCE, G_PRIORITY_DEFAULT,
            state->cancellable, g_subprocess_communicate_made_progress,
            g_object_ref(task));
        state->outstanding_ops++;
    }
    ...
    if (subprocess->stderr_pipe)
    {
        state->stderr_buf =
            (GMemoryOutputStream*)g_memory_output_stream_new_resizable();
        g_output_stream_splice_async(
            (GOutputStream*)state->stderr_buf, subprocess->stderr_pipe,
            G_OUTPUT_STREAM_SPLICE_CLOSE_SOURCE, G_PRIORITY_DEFAULT,
            state->cancellable, g_subprocess_communicate_made_progress,
            g_object_ref(task));
        state->outstanding_ops++;
 ...
 * @cancellable: a #GCancellable
 * @stdout_buf: (out) (nullable) (optional) (transfer full): data read from the
 *subprocess stdout
 * @stderr_buf: (out) (nullable) (optional) (transfer full): data read from the
 *subprocess stderr
 ...
 * Since: 2.40
 **/
gboolean g_subprocess_communicate(GSubprocess* subprocess, GBytes* stdin_buf,
                                  GCancellable* cancellable,
                                  ...
                                  GError** error)
{
    ...
    g_return_val_if_fail(G_IS_SUBPROCESS(subprocess), FALSE);
    g_return_val_if_fail(stdin_buf == NULL || (subprocess->flags &
                                               G_SUBPROCESS_FLAGS_STDIN_PIPE),
                         FALSE);
    ...
    g_subprocess_sync_complete(&result);
    success = g_subprocess_communicate_finish(subprocess, result, stdout_buf,
                                              stderr_buf, error);
    g_object_unref(result);
 ...
 */
void g_subprocess_communicate_async(GSubprocess* subprocess, GBytes* stdin_buf,
                                    GCancellable* cancellable,
                                    GAsyncReadyCallback callback,
                                    gpointer user_data)
{
    g_return_if_fail(G_IS_SUBPROCESS(subprocess));
    g_return_if_fail(stdin_buf == NULL ||
                     (subprocess->flags & G_SUBPROCESS_FLAGS_STDIN_PIPE));
 ...
 * @subprocess: Self
 * @result: Result
 * @stdout_buf: (out) (nullable) (optional) (transfer full): Return location for
 * stdout data
 * @stderr_buf: (out) (nullable) (optional) (transfer full): Return location for
 * stderr data
 * @error: Error
 *
                                       ...
                                       GError** error)
{
    ...
    g_subprocess_sync_setup();
    g_subprocess_communicate_internal(subprocess, TRUE, stdin_bytes,
                                      cancellable, g_subprocess_sync_done,
                                      &result);
    g_subprocess_sync_complete(&result);
...
static gboolean communicate_result_validate_utf8(const char* stream_name,
        ...
        if (!g_utf8_validate(*return_location, -1, &end))
        {
            ...
            *return_location = NULL;
            g_set_error(error, G_IO_ERROR, G_IO_ERROR_FAILED,
                        "Invalid UTF-8 in child %s at offset %lu", stream_name,
