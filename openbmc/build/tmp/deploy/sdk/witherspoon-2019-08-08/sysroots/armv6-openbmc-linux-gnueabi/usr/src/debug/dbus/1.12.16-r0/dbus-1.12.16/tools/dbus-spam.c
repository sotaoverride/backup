...
int dbus_test_tool_spam(int argc, char** argv)
{
    ...
    for (i = 2; i < argc; i++)
    {
        ...
        if (payload != NULL &&
            (strstr(arg, "--payload=") == arg || strcmp(arg, "--stdin") == 0 ||
        ...
        else if (strcmp(arg, "--message-stdin") == 0)
        {
            ...
            if (dbus_message_get_type(template) !=
    ...
    VERBOSE(stderr,
            "Will send up to %d messages, with up to %d queued, max %d per "
            "connection\n",
    ...
    while (no_reply ? sent < count : received < count)
    {
         ...
         *  or
         * - When messages_per_conn messages have been sent and no replies are
         * being waited for
         */
        if (connection == NULL ||
            (messages_per_conn > 0 && sent_in_this_conn == messages_per_conn &&
             (no_reply ||
              received - received_before_this_conn == messages_per_conn)))
        {
            if (connection != NULL)
            {
        ...
        while (
            ...
            (queue_len == -1 ||
             sent_in_this_conn <
                 queue_len + received - received_before_this_conn))
        {
            DBusMessage* message;
            ...
            else
            {
                ...
                switch (payload_type)
                {
                    case DBUS_TYPE_STRING:
                        if (random_sizes != NULL)
                        {
                            /* this isn't fair, strictly speaking - the first
                             * few are a bit more likely to be chosen, unless
                             * RAND_MAX is divisible by n_random_sizes - but
                             * it's good enough for traffic-generation */
                            len = random_sizes[rand() % n_random_sizes];
                            payload_buf[len] = '\0';
                        }

                        mem = dbus_message_append_args(
                            message, DBUS_TYPE_STRING, &payload,
                            DBUS_TYPE_INVALID);
                    ...
                    case DBUS_TYPE_ARRAY:
                        ...
                        mem = dbus_message_append_args(
                            message, DBUS_TYPE_ARRAY, DBUS_TYPE_BYTE, &payload,
                            (dbus_uint32_t)len, DBUS_TYPE_INVALID);
                        break;
            ...
            else
            {
                ...
                if (!dbus_connection_send_with_reply(connection, message, &pc,
