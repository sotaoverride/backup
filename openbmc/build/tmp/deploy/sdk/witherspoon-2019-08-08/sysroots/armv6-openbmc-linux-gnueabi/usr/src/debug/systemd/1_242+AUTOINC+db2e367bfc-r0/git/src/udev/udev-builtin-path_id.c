...
static int format_lun_number(sd_device* dev, char** path)
{
    ...
    if (lun < 256)
         ...
         * original lun format */
        path_prepend(path, "lun-0x%04lx%04lx00000000", lun & 0xffff,
                     (lun >> 16) & 0xffff);
...
static sd_device* handle_scsi_fibre_channel(sd_device* parent, char** path)
{
    ...
    if (sd_device_get_parent_with_subsystem_devtype(
            parent, "scsi", "scsi_target", &targetdev) < 0)
        return NULL;
    if (sd_device_get_sysname(targetdev, &sysname) < 0)
        return NULL;
    if (sd_device_new_from_subsystem_sysname(&fcdev, "fc_transport", sysname) <
        0)
        return NULL;
...
static sd_device* handle_scsi_sas_wide_port(sd_device* parent, char** path)
{
    ...
    if (sd_device_get_sysname(target_parent, &sysname) < 0)
        return NULL;
    if (sd_device_new_from_subsystem_sysname(&sasdev, "sas_device", sysname) <
        0)
        return NULL;
...
static sd_device* handle_scsi_sas(sd_device* parent, char** path)
{
    ...
    _cleanup_(sd_device_unrefp) sd_device *target_sasdev = NULL,
                                          *expander_sasdev = NULL,
                                          *port_sasdev = NULL;
    ...
    if (sd_device_get_sysname(target_parent, &sysname) < 0)
        return NULL;
    /* Get sas device */
    if (sd_device_new_from_subsystem_sysname(&target_sasdev, "sas_device",
                                             sysname) < 0)
        return NULL;
    ...
    if (sd_device_get_sysname(port, &sysname) < 0)
        return NULL;
    /* Get port device */
    if (sd_device_new_from_subsystem_sysname(&port_sasdev, "sas_port",
                                             sysname) < 0)
        return NULL;
    ...
    /* Get connected phy */
    if (sd_device_get_sysattr_value(target_sasdev, "phy_identifier", &phy_id) <
        0)
        return NULL;
    ...
    if (sd_device_get_sysname(expander, &sysname) < 0)
        return NULL;
    /* Get expander device */
    if (sd_device_new_from_subsystem_sysname(&expander_sasdev, "sas_device",
                                             sysname) >= 0)
    {
        /* Get expander's address */
        if (sd_device_get_sysattr_value(expander_sasdev, "sas_address",
                                        &sas_address) < 0)
            return NULL;
    }
...
static sd_device* handle_scsi_iscsi(sd_device* parent, char** path)
{
    ...
    if (sd_device_new_from_subsystem_sysname(&sessiondev, "iscsi_session",
                                             sysname) < 0)
        return NULL;
    ...
    if (sd_device_new_from_subsystem_sysname(&conndev, "iscsi_connection",
                                             connname) < 0)
        return NULL;
...
static sd_device* handle_scsi_ata(sd_device* parent, char** path)
{
    ...
    if (sd_device_get_parent_with_subsystem_devtype(parent, "scsi", "scsi_host",
                                                    &targetdev) < 0)
        return NULL;
...
static sd_device* handle_scsi_hyperv(sd_device* parent, char** path,
                                     size_t guid_str_len)
{
    sd_device* hostdev;
    sd_device* vmbusdev;
    ...
    if (strlen(guid_str) < guid_str_len || guid_str[0] != '{' ||
        guid_str[guid_str_len - 1] != '}')
        return NULL;
...
static int builtin_path_id(sd_device* dev, int argc, char* argv[], bool test)
{
    ...
    if (sd_device_get_subsystem(dev, &subsystem) >= 0 &&
        streq(subsystem, "block") && !supported_transport)
        return -ENOENT;
