...
struct variable1 ipaddr_variables[] = {
    {IPADADDR, ASN_IPADDRESS, NETSNMP_OLDAPI_RONLY, var_ipAddrEntry, 1, {1}},
    {IPADIFINDEX, ASN_INTEGER, NETSNMP_OLDAPI_RONLY, var_ipAddrEntry, 1, {2}},
#ifndef sunV3
    {IPADNETMASK, ASN_IPADDRESS, NETSNMP_OLDAPI_RONLY, var_ipAddrEntry, 1, {3}},
#endif
    {IPADBCASTADDR, ASN_INTEGER, NETSNMP_OLDAPI_RONLY, var_ipAddrEntry, 1, {4}},
    {IPADREASMMAX, ASN_INTEGER, NETSNMP_OLDAPI_RONLY, var_ipAddrEntry, 1, {5}}};
...
struct variable1 iproute_variables[] = {
    ...
    {IPROUTENEXTHOP,
     ...
     1,
     {7}},
    {IPROUTETYPE, ASN_INTEGER, NETSNMP_OLDAPI_RWRITE, var_ipRouteEntry, 1, {8}},
    {IPROUTEPROTO, ASN_INTEGER, NETSNMP_OLDAPI_RONLY, var_ipRouteEntry, 1, {9}},
    {IPROUTEAGE, ASN_INTEGER, NETSNMP_OLDAPI_RWRITE, var_ipRouteEntry, 1, {10}},
    {IPROUTEMASK,
     ASN_IPADDRESS,
     NETSNMP_OLDAPI_RWRITE,
     var_ipRouteEntry,
...
struct variable1 ipmedia_variables[] = {
    ...
    {IPMEDIAIFINDEX, ASN_INTEGER, NETSNMP_OLDAPI_RWRITE, var_atEntry, 1, {1}},
    {IPMEDIAPHYSADDRESS,
     ASN_OCTET_STR,
     NETSNMP_OLDAPI_RWRITE,
     var_atEntry,
    ...
    {IPMEDIANETADDRESS,
     ...
     1,
     {3}},
    {IPMEDIATYPE, ASN_INTEGER, NETSNMP_OLDAPI_RWRITE, var_atEntry, 1, {4}}
#else
    {IPMEDIAIFINDEX, ASN_INTEGER, NETSNMP_OLDAPI_RONLY, var_atEntry, 1, {1}},
    {IPMEDIAPHYSADDRESS,
     ASN_OCTET_STR,
     NETSNMP_OLDAPI_RONLY,
     var_atEntry,
    ...
    {IPMEDIANETADDRESS,
     ...
     1,
     {3}},
    {IPMEDIATYPE, ASN_INTEGER, NETSNMP_OLDAPI_RONLY, var_atEntry, 1, {4}}
#endif
#endif
};
...
void init_ip(void)
{
    ...
    reginfo = netsnmp_create_handler_registration(
        "ip", ip_handler, ip_oid, OID_LENGTH(ip_oid), HANDLER_CAN_RONLY);
    ...
    if (rc != SNMPERR_SUCCESS)
         ...
         */
#ifndef hpux11
    netsnmp_inject_handler(reginfo, netsnmp_get_cache_handler(
                                        IP_STATS_CACHE_TIMEOUT, ip_load,
                                        ip_free, ip_oid, OID_LENGTH(ip_oid)));
#endif

    /*
     * register (using the old-style API) to handle the IP tables
     */
    REGISTER_MIB("mibII/ipaddr", ipaddr_variables, variable1,
                 ipaddr_variables_oid);
    REGISTER_MIB("mibII/iproute", iproute_variables, variable1,
                 iproute_variables_oid);
    REGISTER_MIB("mibII/ipmedia", ipmedia_variables, variable1,
...
int ip_handler(netsnmp_mib_handler* handler,
               netsnmp_handler_registration* reginfo,
    ...
    switch (reqinfo->mode)
    {
        case MODE_GET:
            for (request = requests; request; request = request->next)
            {
                ...
                DEBUGMSGOID(
                    ("mibII/ip", requestvb->name, requestvb->name_length));
                DEBUGMSG(("mibII/ip", "\n"));
...
#ifdef HAVE_SYS_TCPIPSTATS_H
                    /*
                     * This actually reads statistics for *all* the groups
                     * together, so we need to isolate the IP-specific bits.
                     */
#define ipstat ipstat.ipstat
                    ...
                    case IPINRECEIVES:
                        ...
                        break;
                    case IPINHDRERRORS:
                        ret_value = ipstat.ips_badsum + ipstat.ips_tooshort +
                    ...
                    case IPOUTNOROUTES:
                        /*
                         * XXX: how to calculate this (counts dropped routes,
                         * not packets)? ipstat.ips_cantforward isn't right, as
                         * it counts packets. ipstat.ips_noroute is also
                         * incorrect.
                         */
                        netsnmp_set_request_error(reqinfo, request,
                                                  SNMP_NOSUCHOBJECT);
...
#undef ipstat
                    ...
                    case IPMEDIATABLE:
                         ...
                         * The relevant table registrations should take
                         * precedence, so skip these three subtrees, regardless
                         * of architecture.
                         */
                        netsnmp_set_request_error(reqinfo, request,
                                                  SNMP_NOSUCHOBJECT);
...
#ifdef hpux11
int ip_load(netsnmp_cache* cache, void* vmagic)
{
    int fd;
    struct nmparms p;
    unsigned int ulen;
    ...
    {
        DEBUGMSGTL(
            ("mibII/ip", "Failed to load IP object %d (hpux11)\n", magic));
        return (-1); /* error */
    }
...
int ip_load(netsnmp_cache* cache, void* vmagic)
{
    ...
    ret_value = getMibstat(MIB_IP, &ipstat, sizeof(mib2_ip_t), GET_FIRST,
                           &Get_everything, NULL);
...
int ip_load(netsnmp_cache* cache, void* vmagic)
{
    ...
    {
        case IPFORWARDING:
            if (!auto_nlist(IP_FORWARDING_SYMBOL, (char*)&ret_value,
                            sizeof(ret_value)))
                return -1;
            else
        ...
        case IPDEFAULTTTL:
            if (!auto_nlist(TCP_TTL_SYMBOL, (char*)&ret_value,
                            sizeof(ret_value)))
                return -1;
            else
                return ret_value;
        ...
        default:
            ret_value =
                sysmp(MP_SAGET, MPSA_TCPIPSTATS, &ipstat, sizeof ipstat);

            if (ret_value < 0)
            {
                DEBUGMSGTL(
                    ("mibII/ip", "Failed to load IP Group (tcpipstats)\n"));
            }
            else
            {
...
void ip_free(netsnmp_cache* cache, void* magic)
{
