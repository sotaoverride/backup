...
static int _pam_audit_writelog(pam_handle_t* pamh, int audit_fd, int type,
                               const char* message, const char* grantors,
                               int retval)
{
    ...
    {
        rc = audit_log_acct_message(
            audit_fd, type, NULL, buf,
            (retval != PAM_USER_UNKNOWN && pamh->user) ? pamh->user : "?", -1,
...
static int _pam_audit_open(pam_handle_t* pamh)
{
    int audit_fd;
    audit_fd = audit_open();
    if (audit_fd < 0)
...
static int _pam_list_grantors(struct handler* hlist, int retval, char** list)
{
    *list = NULL;
...
int _pam_auditlog(pam_handle_t* pamh, int action, int retval, int flags,
                  struct handler* h)
{
    const char* message;
    int type;
...
int _pam_audit_end(pam_handle_t* pamh, int status UNUSED)
{
...
int pam_modutil_audit_write(pam_handle_t* pamh, int type, const char* message,
                            int retval)
{
    int audit_fd;
    int rc;
