...
static GList* srv_records_to_targets(GList* records)
{
...
static GList* g_resolver_real_lookup_service(GResolver* resolver,
                                             const gchar* rrname,
                                             ...
                                             GError** error)
{
    ...
    records = G_RESOLVER_GET_CLASS(resolver)->lookup_records(
        resolver, rrname, G_RESOLVER_RECORD_SRV, cancellable, error);

    return srv_records_to_targets(records);
}

static void g_resolver_real_lookup_service_async(GResolver* resolver,
                                                 const gchar* rrname,
                                                 GCancellable* cancellable,
                                                 GAsyncReadyCallback callback,
                                                 gpointer user_data)
{
    G_RESOLVER_GET_CLASS(resolver)->lookup_records_async(
        resolver, rrname, G_RESOLVER_RECORD_SRV, cancellable, callback,
        user_data);
}

static GList* g_resolver_real_lookup_service_finish(GResolver* resolver,
                                                    GAsyncResult* result,
                                                    GError** error)
{
    GList* records;

    records = G_RESOLVER_GET_CLASS(resolver)->lookup_records_finish(
...
static void g_resolver_class_init(GResolverClass* resolver_class)
{
    ...
    resolver_class->lookup_service_finish =
     ...
     **/
    signals[RELOAD] =
        g_signal_new(I_("reload"), G_TYPE_RESOLVER, G_SIGNAL_RUN_LAST,
                     G_STRUCT_OFFSET(GResolverClass, reload), NULL, NULL,
 ...
 */
static void remove_duplicates(GList* addrs)
{
    GList* l;
    GList* ll;
    GList* lll;
 ...
 */
static gboolean handle_ip_address(const char* hostname, GList** addrs,
                                  GError** error)
{
    GInetAddress* addr;
    ...
    *addrs = NULL;
    ...
    /* Reject IPv6 addresses that have brackets ('[' or ']') and/or port
     * numbers, as no valid addresses should contain these at this point.
     * Non-standard IPv4 addresses would be rejected during the call to
     * getaddrinfo() later.
     */
    if (strrchr(hostname, '[') != NULL || strrchr(hostname, ']') != NULL)
#else
...
static GList* lookup_by_name_real(GResolver* resolver, const gchar* hostname,
                                  ...
                                  GCancellable* cancellable, GError** error)
{
    ...
    if (flags != G_RESOLVER_NAME_LOOKUP_FLAGS_DEFAULT)
    {
        ...
        addrs = G_RESOLVER_GET_CLASS(resolver)->lookup_by_name_with_flags(
            resolver, hostname, flags, cancellable, error);
    }
    else
        addrs = G_RESOLVER_GET_CLASS(resolver)->lookup_by_name(
 ...
 * Since: 2.22
 */
GList* g_resolver_lookup_by_name(GResolver* resolver, const gchar* hostname,
                                 GCancellable* cancellable, GError** error)
{
    return lookup_by_name_real(resolver, hostname,
...
GList* g_resolver_lookup_by_name_with_flags(GResolver* resolver,
                                            ...
                                            GError** error)
{
    return lookup_by_name_real(resolver, hostname, flags, cancellable, error);
}

static void lookup_by_name_async_real(
    ...
    GCancellable* cancellable, GAsyncReadyCallback callback, gpointer user_data)
{
    ...
    g_return_if_fail(!(flags & G_RESOLVER_NAME_LOOKUP_FLAGS_IPV4_ONLY &&
    ...
    if (flags != G_RESOLVER_NAME_LOOKUP_FLAGS_DEFAULT)
    {
        if (G_RESOLVER_GET_CLASS(resolver)->lookup_by_name_with_flags_async ==
            NULL)
        {
            GTask* task;
            ...
            G_RESOLVER_GET_CLASS(resolver)->lookup_by_name_with_flags_async(
                resolver, hostname, flags, cancellable, callback, user_data);
    }
    else
        G_RESOLVER_GET_CLASS(resolver)->lookup_by_name_async(
            resolver, hostname, cancellable, callback, user_data);

    g_free(ascii_hostname);
}

static GList* lookup_by_name_finish_real(GResolver* resolver,
                                         ...
                                         gboolean with_flags)
{
    ...
    if (with_flags)
    {
        ...
        addrs = G_RESOLVER_GET_CLASS(resolver)->lookup_by_name_finish(
            resolver, result, error);
...
void g_resolver_lookup_by_name_with_flags_async(
    ...
    GCancellable* cancellable, GAsyncReadyCallback callback, gpointer user_data)
{
    lookup_by_name_async_real(resolver, hostname, flags, cancellable, callback,
                              user_data);
}
 ...
 * Since: 2.22
 */
void g_resolver_lookup_by_name_async(GResolver* resolver, const gchar* hostname,
                                     GCancellable* cancellable,
                                     GAsyncReadyCallback callback,
                                     gpointer user_data)
{
    lookup_by_name_async_real(resolver, hostname, 0, cancellable, callback,
                              user_data);
}
...
GList* g_resolver_lookup_by_name_finish(GResolver* resolver,
                                        GAsyncResult* result, GError** error)
{
    return lookup_by_name_finish_real(resolver, result, error, FALSE);
}
...
GList* g_resolver_lookup_by_name_with_flags_finish(GResolver* resolver,
                                                   ...
                                                   GError** error)
{
    return lookup_by_name_finish_real(resolver, result, error, TRUE);
}
 ...
 * Since: 2.22
 */
gchar* g_resolver_lookup_by_address(GResolver* resolver, GInetAddress* address,
                                    GCancellable* cancellable, GError** error)
{
    g_return_val_if_fail(G_IS_RESOLVER(resolver), NULL);
    ...
    g_resolver_maybe_reload(resolver);
    return G_RESOLVER_GET_CLASS(resolver)->lookup_by_address(
        resolver, address, cancellable, error);
}
                                        ...
                                        gpointer user_data)
{
    ...
    g_resolver_maybe_reload(resolver);
    G_RESOLVER_GET_CLASS(resolver)->lookup_by_address_async(
                                           ...
                                           GAsyncResult* result, GError** error)
{
    ...
    return G_RESOLVER_GET_CLASS(resolver)->lookup_by_address_finish(
        resolver, result, error);
}

static gchar* g_resolver_get_service_rrname(const char* service,
                                            const char* protocol,
                                            const char* domain)
{
 ...
 * Since: 2.22
 */
GList* g_resolver_lookup_service(GResolver* resolver, const gchar* service,
                                 const gchar* protocol, const gchar* domain,
                                 GCancellable* cancellable, GError** error)
{
    GList* targets;
    gchar* rrname;
    ...
    g_resolver_maybe_reload(resolver);
    targets = G_RESOLVER_GET_CLASS(resolver)->lookup_service(
        resolver, rrname, cancellable, error);
                                     ...
                                     gpointer user_data)
{
    ...
    if (!rrname)
    {
        g_task_report_new_error(resolver, callback, user_data,
                                g_resolver_lookup_service_async, G_IO_ERROR,
                                G_IO_ERROR_FAILED, _("Invalid domain"));
        return;
    }

    g_resolver_maybe_reload(resolver);
    G_RESOLVER_GET_CLASS(resolver)->lookup_service_async(
        resolver, rrname, cancellable, callback, user_data);
                                        ...
                                        GAsyncResult* result, GError** error)
{
    ...
    return G_RESOLVER_GET_CLASS(resolver)->lookup_service_finish(resolver,
                                                                 result, error);
}
 ...
 * Since: 2.34
 */
GList* g_resolver_lookup_records(GResolver* resolver, const gchar* rrname,
 ...
 * Since: 2.34
 */
void g_resolver_lookup_records_async(GResolver* resolver, const gchar* rrname,
                                     GResolverRecordType record_type,
                                     GCancellable* cancellable,
                                     GAsyncReadyCallback callback,
                                     gpointer user_data)
{
    ...
    g_resolver_maybe_reload(resolver);
    G_RESOLVER_GET_CLASS(resolver)->lookup_records_async(
        resolver, rrname, record_type, cancellable, callback, user_data);
}
...
{
    g_return_val_if_fail(G_IS_RESOLVER(resolver), NULL);
    return G_RESOLVER_GET_CLASS(resolver)->lookup_records_finish(resolver,
                                                                 result, error);
}
