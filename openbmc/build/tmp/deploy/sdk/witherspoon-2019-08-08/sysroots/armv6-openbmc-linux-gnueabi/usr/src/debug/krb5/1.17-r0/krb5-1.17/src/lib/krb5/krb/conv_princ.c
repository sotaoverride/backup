...
#define RC(V5NAME, V4NAME)                                                     \
    {                                                                          \
        V5NAME, V4NAME, DO_REALM_CONVERSION, sizeof(V5NAME) - 1                \
    }
/* Realm conversion */
#define R(NAME)                                                                \
    {                                                                          \
        NAME, NAME, DO_REALM_CONVERSION, sizeof(NAME) - 1                      \
    }
/* No Realm conversion */
#define NR(NAME)                                                               \
    {                                                                          \
        NAME, NAME, 0, sizeof(NAME) - 1                                        \
    }

    NR("kadmin"),   RC("rcmd", "host"), R("discuss"),  R("rvdsrv"),
    R("sample"),    R("olc"),           R("pop"),      R("sis"),
    R("rfs"),       R("imap"),          R("ftp"),      R("ecat"),
    R("daemon"),    R("gnats"),         R("moira"),    R("prms"),
    R("mandarin"),  R("register"),      R("changepw"), R("sms"),
    R("afpserver"), R("gdss"),          R("news"),     R("abs"),
    R("nfs"),       R("tftp"),          NR("zephyr"),  R("http"),
    R("khttp"),     R("pgpsigner"),     R("irc"),      R("mandarin-agent"),
    R("write"),     R("palladium"),     {0, 0, 0, 0},
#undef R
#undef RC
#undef NR
};
 ...
 * category. :-)
 */
static char* strnchr(char* s, int c, unsigned int n)
{
    if (n < 1)
        return 0;
                            ...
                            char* name, char* inst, char* realm)
{
    ...
    *name = *inst = '\0';
    ...
    retval = profile_get_string(context->profile, KRB5_CONF_REALMS, tmp_prealm,
                                KRB5_CONF_V4_REALM, 0, &tmp_realm);
    free(tmp_prealm);
    if (retval)
    {
        return retval;
                                                      ...
                                                      krb5_principal* princ)
{
    ...
    /* First, convert the realm, since the v4 realm is not necessarily the same
       as the v5 realm To do that, iterate over all the realms in the config
       file, looking for a matching v4_realm line */
    names2[0] = KRB5_CONF_REALMS;
    names2[1] = NULL;
    retval = profile_iterator_create(
        context->profile, names2,
        PROFILE_ITER_LIST_SECTION | PROFILE_ITER_SECTIONS_ONLY, &iterator);
    while (retval == 0)
    {
