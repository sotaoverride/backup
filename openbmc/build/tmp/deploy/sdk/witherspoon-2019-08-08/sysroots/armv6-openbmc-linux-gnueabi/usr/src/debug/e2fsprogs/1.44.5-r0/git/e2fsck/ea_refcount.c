...
retry:
    ...
    if (create &&
        ((refcount->count == 0) || (ea_key > refcount->list[high].ea_key)))
    {
        if (refcount->count >= refcount->size)
            refcount_collapse(refcount);

        return insert_refcount_el(refcount, ea_key, (unsigned)refcount->count);
    }
    if (refcount->count == 0)
        return 0;
...
errcode_t ea_refcount_validate(ext2_refcount_t refcount, FILE* out)
{
    ...
    for (i = 1; i < refcount->count; i++)
    {
        ...
        {
            fprintf(out, "%s: list[%d].ea_key=%llu, list[%d].ea_key=%llu\n",
...
int bcode_program[] = {BCODE_CREATE,
                       5,
                       BCODE_STORE,
                       3,
                       3,
    ...
    while (1)
    {
        switch (bcode_program[i++])
        {
            ...
            case BCODE_INCR:
                ...
                if (retval)
                    com_err("ea_refcount_increment", retval,
                            "while incrementing ea_key %llu", ea_key);
                else
            ...
            case BCODE_DECR:
                ...
                if (retval)
                    com_err("ea_refcount_decrement", retval,
                            "while decrementing ea_key %llu", ea_key);
                else
            ...
            case BCODE_VALIDATE:
                ...
                if (retval)
                    com_err("ea_refcount_validate", retval, "while validating");
                else
