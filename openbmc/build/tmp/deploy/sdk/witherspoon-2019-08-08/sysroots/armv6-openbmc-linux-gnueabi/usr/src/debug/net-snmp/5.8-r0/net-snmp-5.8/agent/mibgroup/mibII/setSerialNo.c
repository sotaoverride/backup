...
static void setserial_parse_config(const char* token, char* cptr)
{
    ...
    setserialno++;
    DEBUGMSGTL(
        ("snmpSetSerialNo", "Re-setting SnmpSetSerialNo to %d\n", setserialno));
}
static int setserial_store_config(int a, int b, void* c, void* d)
{
...
void init_setSerialNo(void)
{
    ...
    DEBUGMSGTL(("snmpSetSerialNo", "Initalizing SnmpSetSerialNo to %d\n",
                setserialno));
    snmpd_register_config_handler("setserialno", setserial_parse_config, NULL,
                                  "integer");
    ...
    netsnmp_register_watched_spinlock(
        netsnmp_create_handler_registration(
            "snmpSetSerialNo", NULL, set_serial_oid, OID_LENGTH(set_serial_oid),
            HANDLER_CAN_RWRITE),
        &setserialno);
