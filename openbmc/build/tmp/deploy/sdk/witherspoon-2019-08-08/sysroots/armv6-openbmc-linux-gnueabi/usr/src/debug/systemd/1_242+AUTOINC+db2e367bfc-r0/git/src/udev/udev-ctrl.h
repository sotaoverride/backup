...
typedef int (*udev_ctrl_handler_t)(struct udev_ctrl* udev_ctrl,
                                   enum udev_ctrl_msg_type type,
                                   const union udev_ctrl_msg_value* value,
                                   void* userdata);
...
static inline int udev_ctrl_send_set_log_level(struct udev_ctrl* uctrl,
                                               int priority)
{
...
static inline int udev_ctrl_send_set_env(struct udev_ctrl* uctrl,
                                         const char* key)
{
    return udev_ctrl_send(uctrl, UDEV_CTRL_SET_ENV, 0, key);
}

static inline int udev_ctrl_send_set_children_max(struct udev_ctrl* uctrl,
                                                  int count)
{
