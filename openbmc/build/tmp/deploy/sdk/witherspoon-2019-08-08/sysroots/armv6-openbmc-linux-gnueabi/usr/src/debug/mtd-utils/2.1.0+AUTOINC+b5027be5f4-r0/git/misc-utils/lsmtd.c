...
static struct column cols[] = {
    ...
    {"MAJ:MIN", "major:minor device number", COL_DEVNUM, COL_DT_STRING, 0},
    ...
    {"CORRUPTED", "wheather an UBI volume is corrupted", COL_CORRUPTED,
     COL_DT_BOOL, 0},
    {"REGIONS", "number of additional erase regions", COL_REGION, COL_DT_NUMBER,
     0},
    {"BB", "wheather the MTD device may have bad eraseblocks", COL_BB,
     COL_DT_BOOL, 0},
    {"MAX-EC", "current highest erase counter value on UBI devices", COL_MAXEC,
     COL_DT_NUMBER, 0},
    {"FREE", "available bytes on an UBI device or volume", COL_FREE,
     COL_DT_SIZE, 0},
    {"FREE-LEB", "available LEBs on an UBI device or volume", COL_FREE_LEB,
     COL_DT_NUMBER, 0},
    {"BAD-COUNT", "number of bad physical eraseblocks", COL_BAD_COUNT,
     COL_DT_NUMBER, 0},
...
static NORETURN void usage(int status)
{
    ...
    fputs("Usage: " PROGRAM_NAME " [options] [<device> ...]\n\n"
          "List information about memory technology devices.\n\n"
          "Options:\n"
          "  -u, --si-units       Scale sizes by factors of 1000 instead of "
          "1024\n"
          ...
          "  -m, --no-ubi         Do not display information about UBI "
          "devices/volumes\n"
          "\n"
...
static NORETURN void version(int status)
{
    ...
    fputs("Copyright (C) 2017 David Oberhollenzer - sigma star gmbh\n"
...
static void print_json_string(const char* value)
{
    ...
    for (; *value; ++value)
    {
        ...
        else if (isascii(*value) && (iscntrl(*value) || !isprint(*value)))
        {
...
static void print_escaped(const char* value)
{
    while (*value)
    {
        if (iscntrl(*value) || !isprint(*value) || *value == '\\' ||
            *value == '"')
        {
...
static void print_column(struct column* col, const char* value, bool is_first,
                         int level)
{
    bool numeric = false;
    const char* key;
    ...
    for (i = 0; i < num_selected; ++i)
    {
        switch (selected[i]->type)
        {
            case COL_DEVNAME:
                if (flags & FLAG_LIST)
                {
                    sprintf(value, "%subi%d", tree_prefix(true), info->dev_num);
                }
                else
                {
    ...
    for (i = 0; i < num_selected; ++i)
    {
        ...
        switch (selected[i]->type)
        {
            case COL_DEVNAME:
                if (flags & FLAG_LIST)
                {
                    sprintf(buffer, "  %subi%d_%d", tree_prefix(is_last),
                            info->dev_num, info->vol_id);
                }
                else
                {
                    sprintf(buffer, "ubi%d_%d", info->dev_num, info->vol_id);
                }
                break;
            case COL_DEVNUM:
                devno_to_string(buffer, info->major, info->minor);
            ...
            case COL_MINIO:
                ...
                break;
            case COL_FREE:
                size_to_string(buffer, info->rsvd_bytes - info->data_bytes);
                break;
            case COL_FREE_LEB:
...
static void print_list(void)
{
    ...
    if (!(flags & FLAG_NO_HEADING))
    {
        ...
        if (!(flags & FLAG_DRYRUN))
        {
            for (i = 0; i < num_selected; ++i)
            {
                fprintf(stdout, "%-*s ", (int)selected[i]->width,
                        selected[i]->name);
            }
            fputc('\n', stdout);
        }
    ...
    for (i = 0; i < num_mtd_devices; ++i)
    {
        ...
        for (j = 0; j < ubi->info.vol_count; ++j)
        {
            ...
            print_ubi_vol(&mtd_dev[i].info, &ubi->info, ubi->vol_info + j,
                          is_last);
            if (!(flags & FLAG_DRYRUN))
                fputc('\n', stdout);
        }
