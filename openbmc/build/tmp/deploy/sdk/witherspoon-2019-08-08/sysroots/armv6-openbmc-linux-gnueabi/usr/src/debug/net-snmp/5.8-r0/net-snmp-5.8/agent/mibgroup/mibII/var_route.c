 ...
 */
u_char* var_ipRouteEntry(struct variable* vp, oid* name, size_t* length,
                         int exact, size_t* var_len, WriteMethod** write_method)
{
    ...
    *write_method =
        NULL; /* write_rte;  XXX:  SET support not really implemented */
    ...
    switch (vp->magic)
    {
        ...
        case IPROUTEPROTO:
            long_return = (rtp->rtm_flags & RTF_DYNAMIC)
                              ? 10
                              : (rtp->rtm_flags & RTF_STATIC)
                                    ? 2
                                    : (rtp->rtm_flags & RTF_DYNAMIC) ? 4 : 1;
            return (u_char*)&long_return;
        case IPROUTEAGE:
#if NETSNMP_NO_DUMMY_VALUES
            return NULL;
...
RTENTRY** netsnmp_get_routes(size_t* size)
{
    ...
    defined(RTHOST_SYMBOL) && defined(RTNET_SYMBOL)
#define NUM_ROUTE_SYMBOLS 2
static char* route_symbols[] = {RTHOST_SYMBOL, RTNET_SYMBOL};
#endif
#endif
...
static void Route_Scan_Reload(void)
{
    ...
    else
    {
        ...
        if (sysctl(name, sizeof(name) / sizeof(int), all_routes, &size, 0, 0) ==
            -1)
        {
            snmp_log(LOG_ERR, "sysctl(CTL_NET,PF_ROUTE,0,0,NET_RT_DUMP,0)\n");
        }
...
struct sockaddr_in* klgetsa(struct sockaddr_in* dst)
{
    ...
    {
        if (!NETSNMP_KLOOKUP(dst, (char*)&klgetsatmp.sin,
...
#else
            cp = (u_char*)&(((struct sockaddr_in*)&(rthead[RtIndex]->rt_dst))
                                ->sin_addr.s_addr);
#endif
            op = Current + 10;
            *op++ = *cp++;
    ...
    switch (vp->magic)
    {
        case IPROUTEDEST:
            *var_len = sizeof(addr_ret);
            ...
            return (u_char*)&addr_ret;
#else
            return (u_char*)&((struct sockaddr_in*)&rthead[RtIndex]->rt_dst)
                ->sin_addr.s_addr;
#endif
        case IPROUTEIFINDEX:
#ifdef hpux11
        ...
        case IPROUTENEXTHOP:
            *var_len = sizeof(addr_ret);
            ...
            return (u_char*)&addr_ret;
#else
            return (u_char*)&((struct sockaddr_in*)&rthead[RtIndex]->rt_gateway)
                ->sin_addr.s_addr;
#endif /* *bsd */
        case IPROUTETYPE:
#ifdef hpux11
        ...
        case IPROUTEMASK:
            *var_len = sizeof(addr_ret);
            ...
            if (((struct sockaddr_in*)&rthead[RtIndex]->rt_dst)
                    ->sin_addr.s_addr == 0)
                addr_ret = 0; /* Default route */
            else
            {
#ifndef linux
                if (!NETSNMP_KLOOKUP(rthead[RtIndex]->rt_ifp, (char*)&rt_ifnet,
                                     sizeof(rt_ifnet)))
                {
                    DEBUGMSGTL(("mibII/var_route", "klookup failed\n"));
                    return NULL;
                }
                if (!NETSNMP_KLOOKUP(rt_ifnet.if_addrlist, (char*)&rt_ifnetaddr,
                                     sizeof(rt_ifnetaddr)))
                {
                    DEBUGMSGTL(("mibII/var_route", "klookup failed\n"));
                    return NULL;
...
#else  /* linux */
                cp =
                    (u_char*)&(((struct sockaddr_in*)&(rthead[RtIndex]->rt_dst))
                                   ->sin_addr.s_addr);
                return (u_char*)&(
...
static int IP_Cmp_Route(void* addr, void* ep)
{
...
#define IP_ROUTEADDR_OFF 10
    ...
    *write_method =
    ...
    if (*length ==
        IP_ROUTENAME_LENGTH) /* Assume that the input name is the lowest */
        memcpy((char*)lowest, (char*)name, IP_ROUTENAME_LENGTH * sizeof(oid));
    else
    {
        name[IP_ROUTEADDR_OFF] =
            (oid)-1; /* Grhhh: to prevent accidental comparison :-( */
        lowest[0] = 0xff;
    }
         ...
         Nextentry = entry, req_type = GET_NEXT)
    {
        ...
        if (exact)
        {
            if (snmp_oid_compare(current, IP_ROUTENAME_LENGTH, name, *length) ==
                0)
            {
        ...
        else
        {
            if ((snmp_oid_compare(current, IP_ROUTENAME_LENGTH, name, *length) >
                ...
                ((Nextentry.ipRouteDest == (u_long)-2) ||
                 (snmp_oid_compare(current, IP_ROUTENAME_LENGTH, lowest,
                                   IP_ROUTENAME_LENGTH) < 0) ||
                 (snmp_oid_compare(name, IP_ROUTENAME_LENGTH, lowest,
                                   IP_ROUTENAME_LENGTH) == 0)))
            {
    ...
    *var_len = sizeof(long_return);
    ...
    switch (vp->magic)
    {
        ...
        case IPROUTEIFINDEX:
            ...
            Lowentry.ipRouteIfIndex.o_bytes[Lowentry.ipRouteIfIndex.o_length] =
                '\0';
...
void load_rtentries(struct radix_node* pt)
{
    ...
    else
    {
        ...
        if (rt.rt_ifp != 0)
        {
            ...
            netsnmp_feature_require(interface_legacy)
                ...
                Interface_Scan_Init();
            rt.rt_unit = 0;
            while (Interface_Scan_Next((short*)&(rt.rt_unit), temp, NULL,
                                       NULL) != 0)
            {
            ...
            ((rt.rt_flags & RTF_LLINFO) != RTF_LLINFO))
        {
            ...
            {
                rthead = (RTENTRY**)realloc((char*)rthead,
                                            2 * rtallocate * sizeof(RTENTRY*));
        ...
        for (table = 0; table < NUM_ROUTE_SYMBOLS; table++)
    {
        ...
        for (i = 0; i < hashsize; i++)
        {
            ...
            while (m)
            {
                ...
                if (rt->rt_ifp != 0)
                {
                    if (!NETSNMP_KLOOKUP(rt->rt_ifp, (char*)&ifnet,
                                         sizeof(ifnet)))
                    {
                        DEBUGMSGTL(("mibII/var_route", "klookup failed\n"));
                        return;
                ...
                if (rtsize >= rtallocate)
                {
                    rthead = (RTENTRY**)realloc(
                        (char*)rthead, 2 * rtallocate * sizeof(RTENTRY*));
                    memset((char*)&rthead[rtallocate], (0),
                           rtallocate * sizeof(RTENTRY*));
...
#if HAVE_SYS_MBUF_H
netsnmp_feature_require(string_append_int) netsnmp_feature_require(
    interface_legacy) static void Route_Scan_Reload(void)
{
...
static void Route_Scan_Reload(void)
{
    ...
    while (fgets(line, sizeof(line), in))
    {
        ...
        if (rtsize >= rtallocate)
        {
            rthead = (struct rtentry**)realloc(
                (char*)rthead, 2 * rtallocate * sizeof(struct rtentry*));
...
#if NEED_KLGETSA
    register u_long dst1 = ntohl(
        klgetsa((const struct sockaddr_in*)(*r1)->rt_dst)->sin_addr.s_addr);
    register u_long dst2 = ntohl(
        klgetsa((const struct sockaddr_in*)(*r2)->rt_dst)->sin_addr.s_addr);
#else
    register u_long dst1 =
        ntohl(((const struct sockaddr_in*)&((*r1)->rt_dst))->sin_addr.s_addr);
    register u_long dst2 =
        ntohl(((const struct sockaddr_in*)&((*r2)->rt_dst))->sin_addr.s_addr);
#endif /* NEED_KLGETSA */
                         ...
                         int exact, size_t* var_len, WriteMethod** write_method)
{
    ...
    else
    {
        ...
        if ((Time_Of_Last_Reload + 5 <= now.tv_sec) || (pIpRtrTable == NULL))
        {
            ...
            if (status == ERROR_INSUFFICIENT_BUFFER)
            {
                ...
                if (pIpRtrTable != NULL)
                {
                     ...
                     */
                    status =
                        GetIpForwardTable(pIpRtrTable, &dwActualSize, TRUE);
                }
            }
        }
        if (status == NO_ERROR)
        ...
        {
            /*
             * for creation of new row, only ipNetToMediaTable case is
             * considered
             */
            if (*length == 14)
            {
...
static void rtmsg(struct rt_msghdr* rtm)
{
    struct snmprt* rt;
    struct sockaddr* sa;
    ...
    for (bit = 1; ((char*)sa < (char*)rtm + rtm->rtm_msglen) && bit; bit <<= 1)
    {
                         ...
                         int exact, size_t* var_len, WriteMethod** write_method)
{
    ...
    *write_method =
    ...
    else
    {
        ...
        memcpy(Current, vp->name,
               SNMP_MIN(sizeof(Current), (int)(vp->namelen) * sizeof(oid)));
        ...
        memcpy(saveName, name,
               SNMP_MIN(sizeof(saveName), *length * sizeof(oid)));
        saveName[9] = 0;
        saveNameLen = *length;
        saveExact = exact;
        savert = rt;
...
const struct sockaddr* get_address(const void* _ap, int addresses, int wanted)
{
    const struct sockaddr* ap = (const struct sockaddr*)_ap;
    int iindex;
    int bitmask;

    for (iindex = 0, bitmask = 1; iindex < RTAX_MAX; ++iindex, bitmask <<= 1)
    {
        if (bitmask == wanted)
        {
 ...
 * part.
 */
const struct in_addr* get_in_address(const void* ap, int addresses, int wanted)
{
    ...
    {
        DEBUGMSGTL(("snmpd",
                    "unknown socket family %d [AF_INET expected] in "
                    "var_ipRouteEntry.\n",
                    a->sin_family));
    }
    return &a->sin_addr;
