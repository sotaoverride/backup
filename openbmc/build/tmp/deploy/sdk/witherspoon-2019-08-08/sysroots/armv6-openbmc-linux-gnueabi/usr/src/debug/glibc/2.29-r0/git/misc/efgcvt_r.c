...
int __APPEND(FUNC_PREFIX, fcvt_r)(FLOAT_TYPE value, int ndigit, int* decpt,
...
int __APPEND(FUNC_PREFIX, ecvt_r)(FLOAT_TYPE value, int ndigit, int* decpt,
                                  int* sign, char* buf, size_t len)
{
    int exponent = 0;
    ...
    else if (__APPEND(FUNC_PREFIX, fcvt_r)(value, MIN(ndigit, NDIGIT_MAX) - 1,
                                           decpt, sign, buf, len))
        return -1;
...
#define cvt_symbol_1(lib, local, symbol, version)                              \
    libc_hidden_def(local) versioned_symbol(lib, local, symbol, version)
#else
#define cvt_symbol(symbol)                                                     \
    cvt_symbol_1(libc, __APPEND(FUNC_PREFIX, symbol), APPEND(q, symbol),       \
                 GLIBC_2_0);                                                   \
    weak_alias(__APPEND(FUNC_PREFIX, symbol), APPEND(FUNC_PREFIX, symbol))
#define cvt_symbol_1(lib, local, symbol, version)                              \
    libc_hidden_def(local) compat_symbol(lib, local, symbol, version)
#endif
#else
#define cvt_symbol(symbol)                                                     \
    cvt_symbol_1(__APPEND(FUNC_PREFIX, symbol), APPEND(FUNC_PREFIX, symbol))
#define cvt_symbol_1(local, symbol)                                            \
    libc_hidden_def(local) weak_alias(local, symbol)
#endif
cvt_symbol(fcvt_r);
cvt_symbol(ecvt_r);
