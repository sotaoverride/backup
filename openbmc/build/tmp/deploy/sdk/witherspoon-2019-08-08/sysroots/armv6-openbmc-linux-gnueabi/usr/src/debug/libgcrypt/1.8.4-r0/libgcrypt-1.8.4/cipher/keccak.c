...
static unsigned int keccak_extract64(KECCAK_STATE* hd, unsigned int pos,
                                     byte* outbuf, unsigned int outlen)
{
    unsigned int i;
...
static unsigned int keccak_extract32bi(KECCAK_STATE* hd, unsigned int pos,
                                       byte* outbuf, unsigned int outlen)
{
    unsigned int i;
    u32 x0;
...
static inline void keccak_absorb_lane32bi(u32* lane, u32 x0, u32 x1)
{
    u32 t;
...
static unsigned int keccak_absorb_lanes64_armv7_neon(KECCAK_STATE* hd, int pos,
                                                     const byte* lanes,
                                                     unsigned int nlanes,
                                                     int blocklanes)
{
...
static unsigned int keccak_absorb_lanes32bi(KECCAK_STATE* hd, int pos,
                                            const byte* lanes,
                                            unsigned int nlanes, int blocklanes)
{
    unsigned int burn = 0;
...
static inline void keccak_absorb_lane32bi_bmi2(u32* lane, u32 x0, u32 x1)
{
...
static unsigned int keccak_absorb_lanes32bi_bmi2(KECCAK_STATE* hd, int pos,
                                                 const byte* lanes,
                                                 unsigned int nlanes,
                                                 int blocklanes)
{
...
static unsigned int keccak_extract32bi_bmi2(KECCAK_STATE* hd, unsigned int pos,
                                            byte* outbuf, unsigned int outlen)
{
    unsigned int i;
    u32 x0;
...
static void keccak_write(void* context, const void* inbuf_arg, size_t inlen)
{
...
static void sha3_224_init(void* context, unsigned int flags)
{
    keccak_init(GCRY_MD_SHA3_224, context, flags);
}

static void sha3_256_init(void* context, unsigned int flags)
{
    keccak_init(GCRY_MD_SHA3_256, context, flags);
}

static void sha3_384_init(void* context, unsigned int flags)
{
    keccak_init(GCRY_MD_SHA3_384, context, flags);
}

static void sha3_512_init(void* context, unsigned int flags)
{
    keccak_init(GCRY_MD_SHA3_512, context, flags);
}

static void shake128_init(void* context, unsigned int flags)
{
    keccak_init(GCRY_MD_SHAKE128, context, flags);
}

static void shake256_init(void* context, unsigned int flags)
{
 ...
 */
static void keccak_final(void* context)
{
    KECCAK_CONTEXT* ctx = context;
    KECCAK_STATE* hd = &ctx->state;
    const size_t bsize = ctx->blocksize;
...
static byte* keccak_read(void* context)
{
...
static void keccak_extract(void* context, void* out, size_t outlen)
{
    KECCAK_CONTEXT* ctx = context;
    KECCAK_STATE* hd = &ctx->state;
    const size_t bsize = ctx->blocksize;
...
static gpg_err_code_t selftests_keccak(int algo, int extended,
                                       selftest_report_func_t report)
{
    ...
    switch (algo)
    {
        ...
        case GCRY_MD_SHA3_224:
            short_hash = "\xe6\x42\x82\x4c\x3f\x8c\xf2\x4a\xd0\x92\x34\xee\x7d"
                         "\x3c\x76\x6f"
                         "\xc9\xa3\xa5\x16\x8d\x0c\x94\xad\x73\xb4\x6f\xdf";
            long_hash = "\x54\x3e\x68\x68\xe1\x66\x6c\x1a\x64\x36\x30\xdf\x77"
                        "\x36\x7a\xe5"
                        "\xa6\x2a\x85\x07\x0a\x51\xc1\x4c\xbf\x66\x5c\xbc";
            one_million_a_hash =
                "\xd6\x93\x35\xb9\x33\x25\x19\x2e\x51\x6a\x91\x2e\x6d\x19\xa1"
                "\x5c"
        ...
        case GCRY_MD_SHA3_256:
            short_hash = "\x3a\x98\x5d\xa7\x4f\xe2\x25\xb2\x04\x5c\x17\x2d\x6b"
                         "\xd3\x90\xbd"
                         "\x85\x5f\x08\x6e\x3e\x9d\x52\x5b\x46\xbf\xe2\x45\x11"
                         "\x43\x15\x32";
            long_hash = "\x91\x6f\x60\x61\xfe\x87\x97\x41\xca\x64\x69\xb4\x39"
                        "\x71\xdf\xdb"
                        "\x28\xb1\xa3\x2d\xc3\x6c\xb3\x25\x4e\x81\x2b\xe2\x7a"
                        "\xad\x1d\x18";
            one_million_a_hash = "\x5c\x88\x75\xae\x47\x4a\x36\x34\xba\x4f\xd5"
                                 "\x5e\xc8\x5b\xff\xd6"
                                 "\x61\xf3\x2a\xca\x75\xc6\xd6\x99\xd0\xcd\xcb"
                                 "\x6c\x11\x58\x91\xc1";
            hash_len = 32;
            break;

        case GCRY_MD_SHA3_384:
            short_hash = "\xec\x01\x49\x82\x88\x51\x6f\xc9\x26\x45\x9f\x58\xe2"
                         "\xc6\xad\x8d"
                         "\xf9\xb4\x73\xcb\x0f\xc0\x8c\x25\x96\xda\x7c\xf0\xe4"
                         "\x9b\xe4\xb2"
                         "\x98\xd8\x8c\xea\x92\x7a\xc7\xf5\x39\xf1\xed\xf2\x28"
                         "\x37\x6d\x25";
            long_hash = "\x79\x40\x7d\x3b\x59\x16\xb5\x9c\x3e\x30\xb0\x98\x22"
                        "\x97\x47\x91"
                        "\xc3\x13\xfb\x9e\xcc\x84\x9e\x40\x6f\x23\x59\x2d\x04"
                        "\xf6\x25\xdc"
                        "\x8c\x70\x9b\x98\xb4\x3b\x38\x52\xb3\x37\x21\x61\x79"
                        "\xaa\x7f\xc7";
            one_million_a_hash = "\xee\xe9\xe2\x4d\x78\xc1\x85\x53\x37\x98\x34"
                                 "\x51\xdf\x97\xc8\xad"
                                 "\x9e\xed\xf2\x56\xc6\x33\x4f\x8e\x94\x8d\x25"
                                 "\x2d\x5e\x0e\x76\x84"
                                 "\x7a\xa0\x77\x4d\xdb\x90\xa8\x42\x19\x0d\x2c"
                                 "\x55\x8b\x4b\x83\x40";
            hash_len = 48;
            break;

        case GCRY_MD_SHA3_512:
            short_hash = "\xb7\x51\x85\x0b\x1a\x57\x16\x8a\x56\x93\xcd\x92\x4b"
                         "\x6b\x09\x6e"
                         "\x08\xf6\x21\x82\x74\x44\xf7\x0d\x88\x4f\x5d\x02\x40"
                         "\xd2\x71\x2e"
                         "\x10\xe1\x16\xe9\x19\x2a\xf3\xc9\x1a\x7e\xc5\x76\x47"
                         "\xe3\x93\x40"
                         "\x57\x34\x0b\x4c\xf4\x08\xd5\xa5\x65\x92\xf8\x27\x4e"
                         "\xec\x53\xf0";
            long_hash = "\xaf\xeb\xb2\xef\x54\x2e\x65\x79\xc5\x0c\xad\x06\xd2"
                        "\xe5\x78\xf9"
                        "\xf8\xdd\x68\x81\xd7\xdc\x82\x4d\x26\x36\x0f\xee\xbf"
                        "\x18\xa4\xfa"
                        "\x73\xe3\x26\x11\x22\x94\x8e\xfc\xfd\x49\x2e\x74\xe8"
                        "\x2e\x21\x89"
                        "\xed\x0f\xb4\x40\xd1\x87\xf3\x82\x27\x0c\xb4\x55\xf2"
                        "\x1d\xd1\x85";
            one_million_a_hash = "\x3c\x3a\x87\x6d\xa1\x40\x34\xab\x60\x62\x7c"
                                 "\x07\x7b\xb9\x8f\x7e"
                                 "\x12\x0a\x2a\x53\x70\x21\x2d\xff\xb3\x38\x5a"
                                 "\x18\xd4\xf3\x88\x59"
                                 "\xed\x31\x1d\x0a\x9d\x51\x41\xce\x9c\xc5\xc6"
                                 "\x6e\xe6\x89\xb2\x66"
                                 "\xa8\xaa\x18\xac\xe8\x28\x2a\x0e\x0d\xb5\x96"
                                 "\xc9\x0b\x0a\x7b\x87";
            hash_len = 64;
            break;

        case GCRY_MD_SHAKE128:
            short_hash = "\x58\x81\x09\x2d\xd8\x18\xbf\x5c\xf8\xa3\xdd\xb7\x93"
                         "\xfb\xcb\xa7"
                         "\x40\x97\xd5\xc5\x26\xa6\xd3\x5f\x97\xb8\x33\x51\x94"
                         "\x0f\x2c\xc8";
            long_hash = "\x7b\x6d\xf6\xff\x18\x11\x73\xb6\xd7\x89\x8d\x7f\xf6"
                        "\x3f\xb0\x7b"
                        "\x7c\x23\x7d\xaf\x47\x1a\x5a\xe5\x60\x2a\xdb\xcc\xef"
                        "\x9c\xcf\x4b";
            one_million_a_hash = "\x9d\x22\x2c\x79\xc4\xff\x9d\x09\x2c\xf6\xca"
                                 "\x86\x14\x3a\xa4\x11"
                                 "\xe3\x69\x97\x38\x08\xef\x97\x09\x32\x55\x82"
                                 "\x6c\x55\x72\xef\x58";
            hash_len = 32;
            break;

        case GCRY_MD_SHAKE256:
            short_hash = "\x48\x33\x66\x60\x13\x60\xa8\x77\x1c\x68\x63\x08\x0c"
                         "\xc4\x11\x4d"
                         "\x8d\xb4\x45\x30\xf8\xf1\xe1\xee\x4f\x94\xea\x37\xe7"
                         "\x8b\x57\x39";
            long_hash = "\x98\xbe\x04\x51\x6c\x04\xcc\x73\x59\x3f\xef\x3e\xd0"
                        "\x35\x2e\xa9"
                        "\xf6\x44\x39\x42\xd6\x95\x0e\x29\xa3\x72\xa6\x81\xc3"
                        "\xde\xaf\x45";
            one_million_a_hash = "\x35\x78\xa7\xa4\xca\x91\x37\x56\x9c\xdf\x76"
                                 "\xed\x61\x7d\x31\xbb"
                                 "\x99\x4f\xca\x9c\x1b\xbf\x8b\x18\x40\x13\xde"
                                 "\x82\x34\xdf\xd1\x3a";
            hash_len = 32;
            break;
    }
    ...
    what = "short string";
    errtxt =
        _gcry_hash_selftest_check_one(algo, 0, "abc", 3, short_hash, hash_len);
    if (errtxt)
        goto failed;
...
static gpg_err_code_t run_selftests(int algo, int extended,
                                    selftest_report_func_t report)
{
    gpg_err_code_t ec;
...
gcry_md_spec_t _gcry_digest_spec_sha3_224 = {GCRY_MD_SHA3_224,
                                             {0, 1},
                                             "SHA3-224",
                                             sha3_224_asn,
                                             DIM(sha3_224_asn),
                                             ...
                                             run_selftests};
gcry_md_spec_t _gcry_digest_spec_sha3_256 = {GCRY_MD_SHA3_256,
                                             {0, 1},
                                             "SHA3-256",
                                             sha3_256_asn,
                                             DIM(sha3_256_asn),
                                             ...
                                             sizeof(KECCAK_CONTEXT),
                                             run_selftests};
gcry_md_spec_t _gcry_digest_spec_sha3_384 = {GCRY_MD_SHA3_384,
                                             {0, 1},
                                             "SHA3-384",
                                             sha3_384_asn,
                                             DIM(sha3_384_asn),
                                             ...
                                             sizeof(KECCAK_CONTEXT),
                                             run_selftests};
gcry_md_spec_t _gcry_digest_spec_sha3_512 = {GCRY_MD_SHA3_512,
                                             {0, 1},
                                             "SHA3-512",
                                             sha3_512_asn,
                                             DIM(sha3_512_asn),
                                             ...
                                             sizeof(KECCAK_CONTEXT),
                                             run_selftests};
gcry_md_spec_t _gcry_digest_spec_shake128 = {GCRY_MD_SHAKE128,
                                             {0, 1},
                                             "SHAKE128",
                                             shake128_asn,
                                             DIM(shake128_asn),
                                             ...
                                             sizeof(KECCAK_CONTEXT),
                                             run_selftests};
gcry_md_spec_t _gcry_digest_spec_shake256 = {GCRY_MD_SHAKE256,
                                             {0, 1},
                                             "SHAKE256",
                                             shake256_asn,
                                             DIM(shake256_asn),
