...
static errcode_t mark_table_blocks(ext2_filsys fs, ext2fs_block_bitmap bmap);
...
static int is_inode_tb(ext2_filsys fs, unsigned int grp, blk64_t blk)
{
    return blk >= ext2fs_inode_table_loc(fs, grp) &&
           blk < (ext2fs_inode_table_loc(fs, grp) + fs->inode_blocks_per_group);
}
...
static errcode_t resize_group_descriptors(ext2_resize_t rfs, blk64_t new_size)
{
    ...
    if (new_size != ext2fs_blocks_count(rfs->new_fs->super) ||
        ...
        (rfs->flags & RESIZE_DISABLE_64BIT && rfs->flags & RESIZE_ENABLE_64BIT))
    ...
    rfs->new_fs->desc_blocks = ext2fs_div_ceil(
        rfs->old_fs->group_desc_count, EXT2_DESC_PER_BLOCK(rfs->new_fs->super));
...
static errcode_t move_bg_metadata(ext2_resize_t rfs)
{
    ...
    for (i = 0; i < rfs->old_fs->group_desc_count; i++)
    {
        retval = ext2fs_super_and_bgd_loc2(rfs->old_fs, i, &b, &c, &d, NULL);
        if (retval)
            goto out;
        if (b)
        ...
        retval = ext2fs_super_and_bgd_loc2(rfs->new_fs, i, &b, &c, &d, NULL);
        if (retval)
            goto out;
        if (b)
    ...
    for (b = EXT2FS_B2C(rfs->old_fs, rfs->old_fs->super->s_first_data_block);
...
static void fix_uninit_block_bitmaps(ext2_filsys fs)
{
    ...
    for (g = 0; g < fs->group_desc_count; g++)
    {
        ...
        ext2fs_unmark_block_bitmap_range2(fs->block_map, blk, lblk - blk + 1);
...
retry:
    ...
    fs->desc_blocks =
        ext2fs_div_ceil(fs->group_desc_count, EXT2_DESC_PER_BLOCK(fs->super));
    ...
    fs->super->s_inodes_count =
        fs->super->s_inodes_per_group * fs->group_desc_count;
    ...
    if (blk > ext2fs_blocks_count(fs->super))
        ext2fs_free_blocks_count_set(
            fs->super, ext2fs_free_blocks_count(fs->super) -
    ...
    ext2fs_r_blocks_count_set(
        fs->super, (percent * ext2fs_blocks_count(fs->super) / 100.0));

    /*
     * Adjust the bitmaps for size
     */
    retval = ext2fs_resize_inode_bitmap2(
        fs->super->s_inodes_count, fs->super->s_inodes_count, fs->inode_map);
    if (retval)
        goto errout;

    real_end = EXT2_GROUPS_TO_BLOCKS(fs->super, fs->group_desc_count) - 1 +
               fs->super->s_first_data_block;
    retval = ext2fs_resize_block_bitmap2(new_size - 1, real_end, fs->block_map);
    if (retval)
        goto errout;
    ...
    {
        retval =
            ext2fs_resize_block_bitmap2(new_size - 1, real_end, reserve_blocks);
        if (retval)
            goto errout;
    }
    ...
    {
        retval =
            ext2fs_resize_mem(old_fs->desc_blocks * fs->blocksize,
                              fs->desc_blocks * fs->blocksize, &fs->group_desc);
        if (retval)
            goto errout;
        if (fs->desc_blocks > old_fs->desc_blocks)
            memset((char*)fs->group_desc +
                       (old_fs->desc_blocks * fs->blocksize),
                   0, (fs->desc_blocks - old_fs->desc_blocks) * fs->blocksize);
    }
    ...
    if (ext2fs_has_feature_sparse_super2(fs->super))
    {
        ...
        if (last_bg > old_last_bg)
        {
            ...
            if ((old_fs->group_desc_count < 3 && fs->group_desc_count > 2) ||
        ...
        else if (last_bg < old_last_bg)
        {
            ...
            if (last_bg > 1 && old_fs->super->s_backup_bgs[1] == old_last_bg)
                fs->super->s_backup_bgs[1] = last_bg;
        }
    }
    ...
    if (old_fs->group_desc_count > fs->group_desc_count)
    {
         ...
         */
        retval =
            free_gdp_blocks(fs, reserve_blocks, old_fs, fs->group_desc_count);
        goto errout;
    }
    ...
    {
        numblocks =
            (ext2fs_blocks_count(fs->super) - fs->super->s_first_data_block) %
            fs->super->s_blocks_per_group;
        if (!numblocks)
    ...
    for (i = old_fs->group_desc_count; i < fs->group_desc_count; i++)
    {
            ...
            (meta_bg < fs->super->s_first_meta_bg))
        {
            if (has_super)
            {
                for (j = 0; j < old_desc_blocks; j++)
                    ext2fs_block_alloc_stats2(fs, group_block + 1 + j, +1);
        ...
        else
        {
            if (has_super)
                has_super = 1;
            if (((i % meta_bg_size) == 0) || ((i % meta_bg_size) == 1) ||
                ((i % meta_bg_size) == (meta_bg_size - 1)))
                ext2fs_block_alloc_stats2(fs, group_block + has_super, +1);
        }
        ...
        numblocks -= adjblocks;
        ext2fs_free_blocks_count_set(
            fs->super, ext2fs_free_blocks_count(fs->super) - adjblocks);
...
static errcode_t adjust_superblock(ext2_resize_t rfs, blk64_t new_size)
{
    ...
    group_block = ext2fs_group_first_block2(fs, rfs->old_fs->group_desc_count);
    adj = rfs->old_fs->group_desc_count;
    max_group = fs->group_desc_count - adj;
    if (rfs->progress)
    {
        retval = rfs->progress(rfs, E2_RSZ_EXTEND_ITABLE_PASS, 0, max_group);
        if (retval)
            goto errout;
    }
    for (i = rfs->old_fs->group_desc_count; i < fs->group_desc_count; i++)
    {
        ...
        retval = ext2fs_zero_blocks2(fs, ext2fs_inode_table_loc(fs, i),
                                     fs->inode_blocks_per_group, NULL, NULL);
        if (retval)
            goto errout;
        ...
        if (rfs->progress)
        {
            retval = rfs->progress(rfs, E2_RSZ_EXTEND_ITABLE_PASS, i - adj + 1,
                                   max_group);
            if (retval)
                goto errout;
        }
 ...
 */
static void mark_fs_metablock(ext2_resize_t rfs, ext2fs_block_bitmap meta_bmap,
...
static errcode_t blocks_to_move(ext2_resize_t rfs)
{
    ...
    if (new_size < ext2fs_blocks_count(old_fs->super))
    {
        for (g = 0; g < fs->group_desc_count; g++)
        {
            ...
            if ((ext2fs_inode_table_loc(fs, g) + fs->inode_blocks_per_group) >
                new_size)
            {
        ...
        for (i = 0; i < max_groups; i++)
        {
            ...
            for (blk = group_blk + 1 + new_blocks; blk < group_end;)
            {
                if (new_meta_bmap == NULL ||
                    !ext2fs_test_block_bitmap2(new_meta_bmap, blk))
                {
    ...
    for (i = 0; i < max_groups; i++)
    {
            ...
            (meta_bg < fs->super->s_first_meta_bg))
        {
            if (has_super)
            {
                for (blk = group_blk + 1; blk < group_blk + 1 + new_blocks;
                     blk++)
        ...
        else
        {
            ...
            if (((i % meta_bg_size) == 0) || ((i % meta_bg_size) == 1) ||
                ((i % meta_bg_size) == (meta_bg_size - 1)))
                mark_fs_metablock(rfs, meta_bmap, i, group_blk + has_super);
        }
    ...
    for (i = 0; i < max_groups; i++)
    {
        if (ext2fs_inode_table_loc(fs, i) && ext2fs_inode_bitmap_loc(fs, i) &&
            ext2fs_block_bitmap_loc(fs, i))
            continue;

        retval = ext2fs_allocate_group_table(fs, i, rfs->reserve_blocks);
        if (retval)
            goto errout;
...
static blk64_t get_new_block(ext2_resize_t rfs)
{
    ...
    while (1)
    {
        ...
        if (ext2fs_test_block_bitmap2(fs->block_map, rfs->new_blk) ||
            ...
            ((rfs->alloc_state == AVOID_OLD) &&
             (rfs->new_blk < ext2fs_blocks_count(rfs->old_fs->super)) &&
             ext2fs_test_block_bitmap2(rfs->old_fs->block_map, rfs->new_blk)))
        {
            rfs->new_blk++;
            continue;
        }
...
static errcode_t block_mover(ext2_resize_t rfs)
{
    ...
    if (rfs->progress)
    {
        retval = (rfs->progress)(rfs, E2_RSZ_BLOCK_RELOC_PASS, 0, to_move);
        if (retval)
            goto errout;
    }
    while (1)
    {
        ...
        do
        {
            ...
            retval = io_channel_read_blk64(fs->io, old_blk, c, rfs->itable_buf);
            if (retval)
                goto errout;
            retval =
                io_channel_write_blk64(fs->io, new_blk, c, rfs->itable_buf);
            if (retval)
                goto errout;
            size -= c;
            new_blk += c;
            ...
            {
                io_channel_flush(fs->io);
                retval = (rfs->progress)(rfs, E2_RSZ_BLOCK_RELOC_PASS, moved,
                                         to_move);
                if (retval)
                    goto errout;
            }
...
errout:
    if (badblock_list)
    {
        if (!retval && bb_modified)
            retval = ext2fs_update_bb_inode(old_fs, badblock_list);
                         ...
                         int ref_offset EXT2FS_ATTR((unused)), void* priv_data)
{
    ...
    {
        retval =
            ext2fs_add_dir_block2(fs->dblist, pb->ino, block, (int)blockcnt);
        if (retval)
        {
            pb->error = retval;
            ret |= BLOCK_ABORT;
                                   ...
                                   dgrp_t group, void* priv_data)
{
    ...
    {
        io_channel_flush(fs->io);
        retval = (rfs->progress)(rfs, E2_RSZ_INODE_SCAN_PASS, group + 1,
                                 fs->group_desc_count);
        if (retval)
            return retval;
    }
...
#define BLK_IN_CACHE(b, c)                                                     \
    ((b) == (c).blks[0] || (b) == (c).blks[1] || (b) == (c).blks[2] ||         \
     (b) == (c).blks[3])
                                   ...
                                   char* block_buf, ext2_ino_t last_ino)
{
    ...
    while (1)
    {
        ...
        {
            modified =
                fix_ea_ibody_entries(rfs->imap, (struct ext2_inode_large*)inode,
                                     inode_size, last_ino);
            if (modified)
            {
        ...
        if (blk && !BLK_IN_CACHE(blk, blk_cache))
        {
            ...
            modified = fix_ea_block_entries(rfs->imap, block_buf, fs->blocksize,
                                            last_ino);
            if (modified)
            {
...
static errcode_t inode_scan_and_fix(ext2_resize_t rfs)
{
    ...
    if (rfs->progress)
    {
        retval = (rfs->progress)(rfs, E2_RSZ_INODE_SCAN_PASS, 0,
    ...
    while (1)
    {
        ...
        ext2fs_inode_alloc_stats2(rfs->new_fs, new_inode, +1, pb.is_dir);
        /*
        ...
        retval =
            ext2fs_write_inode_full(rfs->old_fs, new_inode, inode, inode_size);
        if (retval)
            goto errout;
        pb.changed = 0;
    ...
    remap_blocks:
        if (pb.changed)
            retval = ext2fs_write_inode_full(rfs->old_fs, new_inode, inode,
                                             inode_size);
        if (retval)
            goto errout;
            ...
            (rfs->bmap || pb.is_dir))
        {
            ...
            retval = ext2fs_block_iterate3(rfs->old_fs, new_inode, 0, block_buf,
                                           process_block, &pb);
            if (retval)
                goto errout;
            if (pb.error)
                 ...
                 (rfs->bmap || pb.is_dir))
        {
            ...
            retval =
                ext2fs_add_dir_block2(rfs->old_fs->dblist, new_inode, 0, 0);
            if (retval)
                goto errout;
        }
    ...
    if (update_ea_inode_refs && ext2fs_has_feature_ea_inode(rfs->new_fs->super))
    {
                                   ...
                                   void* priv_data)
{
    ...
    if (is->rfs->progress && offset == 0)
    {
        ...
        is->err = (is->rfs->progress)(is->rfs, E2_RSZ_INODE_REF_UPD_PASS,
                                      ++is->num, is->max_dirs);
        if (is->err)
            return DIRENT_ABORT;
    }
    ...
    if (is->rfs->flags & RESIZE_DEBUG_INODEMAP)
        printf("Inode translate (dir=%u, name=%.*s, %u->%u)\n", dir,
               ext2fs_dirent_name_len(dirent), dirent->name, dirent->inode,
               new_inode);
#endif
...
static errcode_t inode_ref_fix(ext2_resize_t rfs)
{
    ...
    {
        retval =
            (rfs->progress)(rfs, E2_RSZ_INODE_REF_UPD_PASS, 0, is.max_dirs);
        if (retval)
            goto errout;
    }
    ...
    if (rfs->progress && (is.num < is.max_dirs))
        (rfs->progress)(rfs, E2_RSZ_INODE_REF_UPD_PASS, is.max_dirs,
                        is.max_dirs);
...
static errcode_t move_itables(ext2_resize_t rfs)
{
    ...
    {
        retval =
            ext2fs_allocate_block_bitmap(fs, _("new meta blocks"), &new_bmap);
        if (retval)
            return retval;
    ...
    if (rfs->progress)
    {
        retval = rfs->progress(rfs, E2_RSZ_MOVE_ITABLE_PASS, 0, to_move);
        if (retval)
            goto errout;
    }
    ...
    for (i = 0; i < max_groups; i++)
    {
        ...
        retval = io_channel_read_blk64(
            fs->io, old_blk, fs->inode_blocks_per_group, rfs->itable_buf);
        if (retval)
            goto errout;
        /*
        ...
        retval = io_channel_write_blk64(fs->io, new_blk, num, rfs->itable_buf);
        if (retval)
        {
            io_channel_write_blk64(fs->io, old_blk, num, rfs->itable_buf);
            goto errout;
        }
        if (n > diff)
        {
            retval = io_channel_write_blk64(
                fs->io, old_blk + fs->inode_blocks_per_group, diff,
                (rfs->itable_buf +
                 (fs->inode_blocks_per_group - diff) * fs->blocksize));
            if (retval)
                goto errout;
        }
        ...
        {
            if (new_bmap == NULL || !ext2fs_test_block_bitmap2(new_bmap, blk))
            {
                ext2fs_block_alloc_stats2(fs, blk, -1);
                cluster_freed =
                    EXT2FS_CLUSTER_RATIO(fs) - (blk & EXT2FS_CLUSTER_MASK(fs));
                blk += cluster_freed;
                j += cluster_freed;
                continue;
            }
        ...
        {
            retval =
                rfs->progress(rfs, E2_RSZ_MOVE_ITABLE_PASS, ++moved, to_move);
            if (retval)
                goto errout;
        }
    }
...
static errcode_t clear_sparse_super2_last_group(ext2_resize_t rfs)
{
    ...
    retval = ext2fs_super_and_bgd_loc2(rfs->old_fs, old_last_bg, &sb, &old_desc,
                                       NULL, &num);
    if (retval)
        return retval;
                                                  ...
                                                  ext2fs_block_bitmap meta_bmap)
{
    ...
    retval = ext2fs_super_and_bgd_loc2(rfs->new_fs, last_bg, &sb, &old_desc,
                                       NULL, &num);
    if (retval)
        return retval;
...
static errcode_t fix_resize_inode(ext2_filsys fs)
{
    ...
    retval =
        ext2fs_zero_blocks2(fs, inode.i_block[EXT2_DIND_BLOCK], 1, NULL, NULL);
    if (retval)
        goto errout;
...
static errcode_t ext2fs_calculate_summary_stats(ext2_filsys fs)
{
    ...
    for (blk = B2C(fs->super->s_first_data_block);
         blk < ext2fs_blocks_count(fs->super); blk += EXT2FS_CLUSTER_RATIO(fs))
    {
        if ((uninit && !(EQ_CLSTR(blk, super_blk) ||
    ...
    {
        if (uninit || !ext2fs_fast_test_inode_bitmap2(fs->inode_map, ino))
        {
            group_free++;
            total_inodes_free++;
        }
...
static int calc_group_overhead(ext2_filsys fs, blk64_t grp, int old_desc_blocks)
{
    ...
    ext2fs_super_and_bgd_loc2(fs, grp, &super_blk, &old_desc_blk, &new_desc_blk,
                              0);
    if ((grp == 0) || super_blk)
        overhead++;
    if (old_desc_blk)
...
blk64_t calculate_minimum_resize_size(ext2_filsys fs, int flags)
{
     ...
     */
    inode_count = fs->super->s_inodes_count - fs->super->s_free_inodes_count;
    blks_needed = ext2fs_div_ceil(inode_count, fs->super->s_inodes_per_group) *
                  (blk64_t)EXT2_BLOCKS_PER_GROUP(fs->super);
    groups = ext2fs_div64_ceil(blks_needed, EXT2_BLOCKS_PER_GROUP(fs->super));
#ifdef RESIZE2FS_DEBUG
    ...
    while (blks_needed > data_blocks)
    {
        ...
        extra_grps =
            ext2fs_div64_ceil(remainder, EXT2_BLOCKS_PER_GROUP(fs->super));
     ...
     */
    overhead =
        ext2fs_inode_table_loc(fs, groups - 1) + fs->inode_blocks_per_group;
    ...
    {
        blk64_t safe_margin =
            (ext2fs_blocks_count(fs->super) - blks_needed) / 500;
        unsigned int exts_per_blk =
            (fs->blocksize / sizeof(struct ext3_extent)) - 1;
        blk64_t worst_case = ((data_needed + exts_per_blk - 1) / exts_per_blk);
