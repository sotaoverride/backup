...
#define IP_INOTIFY_DIR_MASK                                                    \
    (IN_MODIFY | IN_ATTRIB | IN_MOVED_FROM | IN_MOVED_TO | IN_DELETE |         \
     IN_CREATE | IN_DELETE_SELF | IN_UNMOUNT | IN_MOVE_SELF | IN_CLOSE_WRITE)
...
static gboolean (*event_callback)(ik_event_t* event, inotify_sub* sub,
                                  gboolean file_event);

gboolean _ip_startup(gboolean (*cb)(ik_event_t* event, inotify_sub* sub,
                                    gboolean file_event))
{
...
static void ip_map_wd_file(gint32 wd, ip_watched_file_t* file)
{
    GList* file_list;
...
static void ip_unmap_wd_file(gint32 wd, ip_watched_file_t* file)
{
...
static ip_watched_file_t* ip_watched_file_new(const gchar* dirname,
...
static void ip_watched_file_free(ip_watched_file_t* file)
{
...
static void ip_watched_file_add_sub(ip_watched_file_t* file, inotify_sub* sub)
{
    file->subs = g_list_prepend(file->subs, sub);
}

static void ip_watched_file_start(ip_watched_file_t* file)
{
    if (file->wd < 0)
    {
        gint err;

        file->wd = _ik_watch(file->path, IP_INOTIFY_FILE_MASK, &err);
...
static void ip_watched_file_stop(ip_watched_file_t* file)
{
    if (file->wd >= 0)
    {
...
static gboolean ip_event_dispatch(GList* dir_list, GList* file_list,
                                  ik_event_t* event)
{
...
static gboolean ip_event_callback(ik_event_t* event)
{
    ...
    /* We can ignore the IGNORED events. Likewise, if the event queue
     * overflowed, there is not much we can do to recover. */
    ...
    {
        dir_list =
            g_hash_table_lookup(wd_dir_hash, GINT_TO_POINTER(event->pair->wd));
        file_list =
            g_hash_table_lookup(wd_file_hash, GINT_TO_POINTER(event->pair->wd));
