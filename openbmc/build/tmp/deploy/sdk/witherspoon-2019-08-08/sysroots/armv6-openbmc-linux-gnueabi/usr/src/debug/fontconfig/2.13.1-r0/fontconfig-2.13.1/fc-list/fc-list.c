...
const struct option longopts[] = {
    {"verbose", 0, 0, 'v'}, {"brief", 0, 0, 'b'},   {"format", 1, 0, 'f'},
    {"quiet", 0, 0, 'q'},   {"version", 0, 0, 'V'}, {"help", 0, 0, 'h'},
    {NULL, 0, 0, 0},
};
#else
#if HAVE_GETOPT
...
static void usage(char* program, int error)
{
...
#if HAVE_GETOPT_LONG
    fprintf(file,
            _("usage: %s [-vbqVh] [-f FORMAT] [--verbose] [--brief] "
              "[--format=FORMAT] [--quiet] [--version] [--help] [pattern] "
              "{element ...} \n"),
            program);
#else
    fprintf(file,
            _("usage: %s [-vbqVh] [-f FORMAT] [pattern] {element ...} \n"),
            program);
#endif
    ...
    fprintf(
        file,
        _("  -v, --verbose        display entire font pattern verbosely\n"));
    fprintf(file,
            _("  -b, --brief          display entire font pattern briefly\n"));
    fprintf(file, _("  -f, --format=FORMAT  use the given output format\n"));
    fprintf(file, _("  -q, --quiet          suppress all normal output, exit 1 "
                    "if no fonts matched\n"));
    fprintf(file,
