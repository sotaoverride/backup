...
class basic_ostringstreambuf : public std::basic_streambuf<CharT, TraitsT>
{
    //! Self type
  ...
  public:
    ...
    struct storage_state
    {
        ...
        BOOST_CONSTEXPR storage_state() BOOST_NOEXCEPT : storage(NULL),
                                                         max_size(0u),
                                                         overflow(false)
        {
        }
  ...
  public:
    ...
    {
        base_type::setp(m_buffer,
                        m_buffer + (sizeof(m_buffer) / sizeof(*m_buffer)));
    }
    //! Constructor
    ...
    storage_state const& get_storage_state() const BOOST_NOEXCEPT
    {
        return m_storage_state;
    }
    void set_storage_state(storage_state const& st) BOOST_NOEXCEPT
    {
        m_storage_state = st;
    }
    ...
    string_type* storage() const BOOST_NOEXCEPT
    {
        return m_storage_state.storage;
    }

    //! Returns the maximum size of the storage
    size_type max_size() const BOOST_NOEXCEPT
    {
        return m_storage_state.max_size;
    }
    ...
    void max_size(size_type size)
    {
        if (m_storage_state.storage)
        {
            const size_type storage_max_size =
                m_storage_state.storage->max_size();
    ...
    //! Makes sure the storage does not exceed the max size limit. Should be
    //! called after the storage is modified externally.
    void ensure_max_size()
    {
        if (m_storage_state.storage &&
            m_storage_state.storage->size() > m_storage_state.max_size)
        {
            const size_type len = length_until_boundary(
                m_storage_state.storage->c_str(),
                m_storage_state.storage->size(), m_storage_state.max_size);
    ...
    bool storage_overflow() const BOOST_NOEXCEPT
    {
        return m_storage_state.overflow;
    }
    //! Sets the overflow flag
    void storage_overflow(bool f) BOOST_NOEXCEPT
    {
        m_storage_state.overflow = f;
    }
    ...
    size_type size_left() const BOOST_NOEXCEPT
    {
        ...
        return size < m_storage_state.max_size ? m_storage_state.max_size - size
                                               : static_cast<size_type>(0u);
    }

    //! Appends a string to the storage and returns the number of written
    //! characters
    size_type append(const char_type* s, size_type n)
    {
        if (!m_storage_state.overflow)
        {
            ...
            {
                // We have to find out where the last character that fits before
                // the limit ends
    ...
    //! Appends the specified number of characters to the storage and returns
    ...
    //! Appends a character to the storage and returns the number of written
    //! characters
    size_type push_back(char_type c)
    {
        if (!m_storage_state.overflow)
        {
            ...
            BOOST_LOG_ASSUME(m_storage_state.max_size <=
                             m_storage_state.storage->max_size());
            if (BOOST_LIKELY(m_storage_state.storage->size() <
  ...
  protected:
    ...
    std::streamsize xsputn(const char_type* s, std::streamsize n)
    {
        ...
        return static_cast<std::streamsize>(
            this->append(s, static_cast<size_type>(n)));
    }

    //! Finds the string length so that it includes only complete characters,
    //! and does not exceed \a max_size
    size_type length_until_boundary(const char_type* s, size_type n,
                                    size_type max_size) const
    {
        BOOST_ASSERT(max_size <= n);
        return length_until_boundary(
            s, n, max_size,
            boost::integral_constant<bool, sizeof(char_type) == 1u>());
    }

  private:
    //! Finds the string length so that it includes only complete characters,
    //! and does not exceed \a max_size
    size_type length_until_boundary(const char_type* s, size_type,
                                    size_type max_size, boost::true_type) const
    {
        ...
        return static_cast<size_type>(
            fac.length(mbs, s, s + max_size, ~static_cast<std::size_t>(0u)));
    }

    //! Finds the string length so that it includes only complete characters,
    //! and does not exceed \a max_size
    static size_type length_until_boundary(const char_type* s, size_type n,
                                           size_type max_size,
                                           boost::false_type)
    {
        // Note: Although it's not required to be true for wchar_t, here we
        // assume that the string has Unicode encoding. Compilers use some
        // version of Unicode for wchar_t on all tested platforms, and
        // std::locale doesn't offer a way to find the character boundary for
        // character types other than char anyway.
