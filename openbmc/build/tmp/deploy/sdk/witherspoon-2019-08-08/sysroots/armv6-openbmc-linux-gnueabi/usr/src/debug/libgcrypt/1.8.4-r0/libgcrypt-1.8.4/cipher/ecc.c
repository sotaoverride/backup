...
static const char* ecc_names[] = {
    "ecc", "ecdsa", "ecdh", "eddsa", "gost", NULL,
};
...
void _gcry_register_pk_ecc_progress(void (*cb)(void*, const char*, int, int,
                                               int),
                                    void* cb_data)
{
    progress_cb = cb;
 ...
 */
static gpg_err_code_t nist_generate_key(ECC_secret_key* sk, elliptic_curve_t* E,
                                        mpi_ec_t ctx, int flags,
                                        ...
                                        gcry_mpi_t* r_y)
{
    ...
    else
    {
        ...
        else /* p - y >= p */
        {
            ...
            if (DBG_CIPHER)
                log_debug(
                    "ecgen didn't need to convert Q to a compliant point\n");
        }
    }
 ...
 */
static int check_secret_key(ECC_secret_key* sk, mpi_ec_t ec, int flags)
{
    int rc = 1;
    mpi_point_struct Q;
    gcry_mpi_t x1, y1;
    ...
    else if (!mpi_cmp_ui(sk->Q.z, 1))
    {
        ...
        {
            if (DBG_CIPHER)
                log_debug("Bad check: There is NO correspondence between 'd' "
                          "and 'Q'!\n");
            goto leave;
        }
    }
...
static gcry_err_code_t ecc_generate(const gcry_sexp_t genparms,
                                    gcry_sexp_t* r_skey)
{
    gpg_err_code_t rc;
    unsigned int nbits;
        ...
        !(flags & PUBKEY_FLAG_NOCOMP))
    {
        ...
        {
            encpk =
                _gcry_mpi_get_buffer_extra(Qx, nbits / 8, -1, &encpklen, NULL);
            if (encpk == NULL)
    ...
    if ((flags & PUBKEY_FLAG_PARAM) && E.name)
        rc = sexp_build(r_skey, NULL,
                        ...
                        " )",
                        curve_info, curve_flags, sk.E.p, sk.E.a, sk.E.b, base,
                        sk.E.n, sk.E.h, public, curve_info, curve_flags, sk.E.p,
                        sk.E.a, sk.E.b, base, sk.E.n, sk.E.h, public, secret);
    else
        rc = sexp_build(r_skey, NULL,
                        "(key-data"
                        " (public-key"
                        ...
                        "  (ecc%S%S(q%m)(d%m)))"
                        " )",
                        curve_info, curve_flags, public, curve_info,
                        curve_flags, public, secret);
    if (rc)
        goto leave;
...
static gcry_err_code_t ecc_check_secret_key(gcry_sexp_t keyparms)
{
    gcry_err_code_t rc;
    gcry_sexp_t l1 = NULL;
    int flags = 0;
    ...
    if ((flags & PUBKEY_FLAG_PARAM))
        rc = sexp_extract_param(keyparms, NULL, "-p?a?b?g?n?h?/q?+d", &sk.E.p,
                                &sk.E.a, &sk.E.b, &mpi_g, &sk.E.n, &sk.E.h,
                                &mpi_q, &sk.d, NULL);
    else
        rc = sexp_extract_param(keyparms, NULL, "/q?+d", &mpi_q, &sk.d, NULL);
    if (rc)
        goto leave;
    ...
    {
        sk.E.model =
            ((flags & PUBKEY_FLAG_EDDSA) ? MPI_EC_EDWARDS : MPI_EC_WEIERSTRASS);
        sk.E.dialect = ((flags & PUBKEY_FLAG_EDDSA) ? ECC_DIALECT_ED25519
    ...
    ec = _gcry_mpi_ec_p_internal_new(sk.E.model, sk.E.dialect, flags, sk.E.p,
                                     sk.E.a, sk.E.b);
...
static gcry_err_code_t ecc_sign(gcry_sexp_t* r_sig, gcry_sexp_t s_data,
                                gcry_sexp_t keyparms)
{
    ...
    if (!curvename)
    {
        sk.E.model = ((ctx.flags & PUBKEY_FLAG_EDDSA) ? MPI_EC_EDWARDS
                                                      : MPI_EC_WEIERSTRASS);
        sk.E.dialect = ((ctx.flags & PUBKEY_FLAG_EDDSA) ? ECC_DIALECT_ED25519
                                                        : ECC_DIALECT_STANDARD);
        if (!sk.E.h)
            sk.E.h = mpi_const(MPI_C_ONE);
    }
    ...
    if ((ctx.flags & PUBKEY_FLAG_EDDSA))
    {
        ...
        rc =
            _gcry_ecc_eddsa_sign(data, &sk, sig_r, sig_s, ctx.hash_algo, mpi_q);
        if (!rc)
            rc = sexp_build(r_sig, NULL, "(sig-val(eddsa(r%M)(s%M)))", sig_r,
                            sig_s);
    }
    else if ((ctx.flags & PUBKEY_FLAG_GOST))
    {
        ...
        if (!rc)
            rc = sexp_build(r_sig, NULL, "(sig-val(gost(r%M)(s%M)))", sig_r,
                            sig_s);
    }
    else
    {
        rc = _gcry_ecc_ecdsa_sign(data, &sk, sig_r, sig_s, ctx.flags,
                                  ctx.hash_algo);
        if (!rc)
            rc = sexp_build(r_sig, NULL, "(sig-val(ecdsa(r%M)(s%M)))", sig_r,
                            sig_s);
    }
...
static gcry_err_code_t ecc_verify(gcry_sexp_t s_sig, gcry_sexp_t s_data,
                                  gcry_sexp_t s_keyparms)
{
     ...
     */
    if ((ctx.flags & PUBKEY_FLAG_PARAM))
        rc = sexp_extract_param(s_keyparms, NULL, "-p?a?b?g?n?h?/q", &pk.E.p,
                                &pk.E.a, &pk.E.b, &mpi_g, &pk.E.n, &pk.E.h,
                                &mpi_q, NULL);
    else
        rc = sexp_extract_param(s_keyparms, NULL, "/q", &mpi_q, NULL);
    if (rc)
        goto leave;
    if (mpi_g)
    {
    ...
    if (!curvename)
    {
        pk.E.model = ((sigflags & PUBKEY_FLAG_EDDSA) ? MPI_EC_EDWARDS
                                                     : MPI_EC_WEIERSTRASS);
        pk.E.dialect = ((sigflags & PUBKEY_FLAG_EDDSA) ? ECC_DIALECT_ED25519
                                                       : ECC_DIALECT_STANDARD);
        if (!pk.E.h)
    ...
    if (!pk.E.p || !pk.E.a || !pk.E.b || !pk.E.G.x || !pk.E.n || !pk.E.h ||
        !mpi_q)
    {
        rc = GPG_ERR_NO_OBJ;
        goto leave;
    ...
    {
        rc = _gcry_ecc_eddsa_verify(data, &pk, sig_r, sig_s, ctx.hash_algo,
                                    mpi_q);
    }
    ...
    else
    {
        ...
        if (mpi_is_opaque(data))
        {
            ...
            rc = _gcry_mpi_scan(&a, GCRYMPI_FMT_USG, abuf, (abits + 7) / 8,
                                NULL);
            if (!rc)
            {
                if (abits > qbits)
 ...
 */
static gcry_err_code_t ecc_encrypt_raw(gcry_sexp_t* r_ciph, gcry_sexp_t s_data,
                                       gcry_sexp_t keyparms)
{
    unsigned int nbits;
    gcry_err_code_t rc;
     ...
     * Extract the key.
     */
    rc = sexp_extract_param(
        keyparms, NULL,
        (flags & PUBKEY_FLAG_DJB_TWEAK) ? "-p?a?b?g?n?h?/q" : "-p?a?b?g?n?h?+q",
        &pk.E.p, &pk.E.a, &pk.E.b, &mpi_g, &pk.E.n, &pk.E.h, &mpi_q, NULL);
    if (rc)
        goto leave;
    if (mpi_g)
    {
    ...
    /* The following is false: assert( mpi_cmp_ui( R.x, 1 )==0 );, so */
    {
        ...
        {
            rawmpi =
                _gcry_mpi_get_buffer_extra(x, nbits / 8, -1, &rawmpilen, NULL);
            if (!rawmpi)
                rc = gpg_err_code_from_syserror();
            else
            {
    ...
    if (!rc)
        rc =
            sexp_build(r_ciph, NULL, "(enc-val(ecdh(s%m)(e%m)))", mpi_s, mpi_e);
 ...
 */
static gcry_err_code_t ecc_decrypt_raw(gcry_sexp_t* r_plain, gcry_sexp_t s_data,
                                       gcry_sexp_t keyparms)
{
    unsigned int nbits;
    gpg_err_code_t rc;
     ...
     * Extract the key.
     */
    rc = sexp_extract_param(keyparms, NULL, "-p?a?b?g?n?h?+d", &sk.E.p, &sk.E.a,
                            &sk.E.b, &mpi_g, &sk.E.n, &sk.E.h, &sk.d, NULL);
    if (rc)
        goto leave;
    if (mpi_g)
    {
...
static gpg_err_code_t compute_keygrip(gcry_md_hd_t md, gcry_sexp_t keyparms)
{
#define N_COMPONENTS 7
    ...
    if ((flags & PUBKEY_FLAG_PARAM))
    {
        if ((flags & PUBKEY_FLAG_DJB_TWEAK))
            rc =
                sexp_extract_param(keyparms, NULL, "p?a?b?g?n?h?/q", &values[0],
                                   &values[1], &values[2], &values[3],
                                   &values[4], &values[5], &values[6], NULL);
        else
            rc = sexp_extract_param(keyparms, NULL, "p?a?b?g?n?h?q", &values[0],
                                    &values[1], &values[2], &values[3],
                                    &values[4], &values[5], &values[6], NULL);
    }
    else
    ...
    if (l1)
    {
        ...
        if (curvename)
        {
            rc = _gcry_ecc_update_curve_param(
                curvename, &model, &dialect, &values[0], &values[1], &values[2],
    ...
    if (!curvename)
    {
        model =
            ((flags & PUBKEY_FLAG_EDDSA) ? MPI_EC_EDWARDS : MPI_EC_WEIERSTRASS);
        dialect = ((flags & PUBKEY_FLAG_EDDSA) ? ECC_DIALECT_ED25519
                                               : ECC_DIALECT_STANDARD);
        if (!values[5])
...
/* This is the worker function for gcry_pubkey_get_sexp for ECC
   ...
   R_SEXP.  */
gpg_err_code_t _gcry_pk_ecc_get_sexp(gcry_sexp_t* r_sexp, int mode, mpi_ec_t ec)
{
    gpg_err_code_t rc;
    gcry_mpi_t mpi_G = NULL;
    gcry_mpi_t mpi_Q = NULL;
    ...
    if (ec->dialect == ECC_DIALECT_ED25519)
    {
        ...
        rc = _gcry_ecc_eddsa_encodepoint(ec->Q, ec, NULL, NULL, 0, &encpk,
                                         &encpklen);
        if (rc)
            goto leave;
    ...
    if (ec->d && (!mode || mode == GCRY_PK_GET_SECKEY))
    {
        ...
        rc = sexp_build(
            r_sexp, NULL,
            "(private-key(ecc(p%m)(a%m)(b%m)(g%m)(n%m)(h%m)(q%m)(d%m)))", ec->p,
            ec->a, ec->b, mpi_G, ec->n, ec->h, mpi_Q, ec->d);
    }
    else if (ec->Q)
    {
...
static const char* selftest_sign(gcry_sexp_t pkey, gcry_sexp_t skey)
{
    ...
    if (!err)
        err = sexp_sscan(&data_bad, NULL, sample_data_bad,
                         strlen(sample_data_bad));
    if (!err)
...
static gpg_err_code_t selftests_ecdsa(selftest_report_func_t report)
{
    const char* what;
    const char* errtxt;
    gcry_error_t err;
