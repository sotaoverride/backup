...
static void g_futex_wait(const volatile gint* address, gint value)
{
    ...
    if
        G_LIKELY(g_atomic_int_get(address) == value)
        {
            WaitAddress* waiter;
 ...
 */
static const volatile gint* g_futex_int_address(const volatile void* address)
{
    ...
    G_STATIC_ASSERT(G_BYTE_ORDER == G_LITTLE_ENDIAN ||
                    (G_BYTE_ORDER == G_BIG_ENDIAN && sizeof(int) == 4 &&
