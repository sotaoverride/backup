...
static int parse_args(pam_handle_t* pamh, struct login_info* loginfo, int argc,
                      const char** argv)
{
    int i;
    ...
    for (i = 0; i < argc; ++i)
    {
        ...
        else if (!strncmp("accessfile=", argv[i], 11))
        {
            ...
            {
                pam_syslog(pamh, LOG_ERR, "failed to open accessfile=[%s]: %m",
                           11 + argv[i]);
                return 0;
            }
        }
...
static int isipaddr(const char* string, int* addr_type,
 ...
 * relevant bits.
 */
static int are_addresses_equal(const char* ipaddr0, const char* ipaddr1,
                               const char* netmask)
{
    ...
    if (memcmp((void*)&addr0, (void*)&addr1, sizeof(struct sockaddr_storage)) ==
        0)
    {
        return (YES);
    }
...
static char* number_to_netmask(long netmask, int addr_type, char* ipaddr_buf,
...
static int login_access(pam_handle_t* pamh, struct login_info* item)
{
    ...
    if (item->debug)
        pam_syslog(pamh, LOG_DEBUG, "login_access: user=%s, from=%s, file=%s",
                   item->user->pw_name, item->from, item->config_file);
    ...
    if ((fp = fopen(item->config_file, "r")) != NULL)
    {
        while (!match && fgets(line, sizeof(line), fp))
        {
            ...
            if (item->debug)
                pam_syslog(pamh, LOG_DEBUG, "line %d: %s : %s : %s", lineno,
                           perm, users, froms);
            match = list_match(pamh, users, NULL, item, user_match);
            if (item->debug)
                pam_syslog(pamh, LOG_DEBUG, "user_match=%d, \"%s\"", match,
                           item->user->pw_name);
            if (match)
            {
                ...
                if (item->debug)
                    pam_syslog(pamh, LOG_DEBUG, "from_match=%d, \"%s\"", match,
                               item->from);
            }
        }
        (void)fclose(fp);
...
#ifdef HAVE_LIBAUDIT
    if (!item->noaudit &&
        (match == YES || (match == ALL && nonall_match == YES)) &&
        line[0] == '-')
    {
        pam_modutil_audit_write(pamh, AUDIT_ANOM_LOGIN_LOCATION, "pam_access",
                                0);
    }
#endif
    if (match == NO)
...
static int list_match(pam_handle_t* pamh, char* list, char* sptr,
                      struct login_info* item, match_func* match_fn)
{
    ...
    if (item->debug && list != NULL)
        pam_syslog(pamh, LOG_DEBUG, "list_match: list=%s, item=%s", list,
                   item->user->pw_name);
    ...
    if (match != NO)
    {
        while ((tok = strtok_r(NULL, item->sep, &sptr)) &&
...
static int netgroup_match(pam_handle_t* pamh, const char* netgroup,
                          const char* machine, const char* user, int debug)
{
    ...
    if (debug == YES)
        pam_syslog(
            ...
            retval, netgroup ? netgroup : "NULL", machine ? machine : "NULL",
...
static int user_match(pam_handle_t* pamh, char* tok, struct login_info* item)
{
    ...
    if (item->debug)
        pam_syslog(pamh, LOG_DEBUG, "user_match: tok=%s, item=%s", tok, string);
    ...
    { /* netgroup */
        ...
        if (tok[1] == '@')
        { /* add hostname to netgroup match */
            if (item->hostname == NULL)
                return NO;
            ++tok;
...
static int group_match(pam_handle_t* pamh, const char* tok, const char* usr,
                       int debug)
{
    char grptok[BUFSIZ];

    if (debug)
        pam_syslog(pamh, LOG_DEBUG, "group_match: grp=%s, user=%s", tok, usr);
...
static int from_match(pam_handle_t* pamh UNUSED, char* tok,
                      struct login_info* item)
{
    ...
    if (item->debug)
        pam_syslog(pamh, LOG_DEBUG, "from_match: tok=%s, item=%s", tok, string);
    ...
    { /* domain: match last fields */
        if ((str_len = strlen(string)) > (tok_len = strlen(tok)) &&
            strcasecmp(tok, string + str_len - tok_len) == 0)
            return (YES);
    }
    else if (item->from_remote_host == 0)
    { /* local: no PAM_RHOSTS */
    ...
    else if (tok[(tok_len = strlen(tok)) - 1] == '.')
    {
        ...
        if (item->gai_rv != 0)
            return NO;
        else if (!item->res && (item->gai_rv = getaddrinfo(string, NULL, &hint,
                                                           &item->res)) != 0)
            return NO;
        else
        {
...
static int string_match(pam_handle_t* pamh, const char* tok, const char* string,
                        int debug)
{

    if (debug)
        pam_syslog(pamh, LOG_DEBUG, "string_match: tok=%s, item=%s", tok,
                   string);
 ...
 */
static int network_netmask_match(pam_handle_t* pamh, const char* tok,
                                 const char* string, struct login_info* item)
{
    char* netmask_ptr;
    ...
    if (item->debug)
        pam_syslog(pamh, LOG_DEBUG, "network_netmask_match: tok=%s, item=%s",
                   tok, string);
    ...
    if ((netmask_ptr = strchr(tok, '/')) != NULL)
    {
        ...
        *netmask_ptr = 0;
        ...
        { /* netmask as integre value */
            ...
            netmask_ptr = number_to_netmask(netmask, addr_type, netmask_string,
                                            MAXHOSTNAMELEN);
        }
    }
    else
            ...
            while (runp != NULL)
            {
                ...
                inet_ntop(
                    ...
                    runp->ai_family == AF_INET
                        ...
                        : (void*)&((struct sockaddr_in6*)runp->ai_addr)
                              ->sin6_addr,
                    buf, sizeof(buf));
                        ...
                        const char** argv)
{
    ...
    if ((from == NULL) || (*from == '\0'))
    {
            ...
            void_from == NULL)
        {
            ...
            {
                if (pam_get_item(pamh, PAM_SERVICE, &void_from) !=
                        PAM_SUCCESS ||
                    void_from == NULL)
                {
                    pam_syslog(
                ...
                if (loginfo.debug)
                    pam_syslog(pamh, LOG_DEBUG,
                               "cannot determine tty or remote hostname, using "
                               "service %s",
                               from);
            }
        }
    ...
    {
        pam_syslog(pamh, LOG_ERR, "access denied for user `%s' from `%s'", user,
                   from);
        return (PAM_PERM_DENIED);
    }
}

int pam_sm_setcred(pam_handle_t* pamh UNUSED, int flags UNUSED, int argc UNUSED,
