...
BusExpireList* bus_expire_list_new(DBusLoop* loop, int expire_after,
                                   BusExpireFunc expire_func, void* data)
{
    BusExpireList* list;
    ...
    list->timeout = _dbus_timeout_new(100, /* irrelevant */
                                      expire_timeout_handler, list, NULL);
...
{
    _dbus_verbose(
        "setting interval on expire list to 0 for immediate recheck\n");

    bus_expire_timeout_set_interval(list->timeout, 0);
}

static int do_expiration_with_monotonic_time(BusExpireList* list, long tv_sec,
                                             long tv_usec)
{
    DBusList* link;
    ...
    while (link != NULL)
    {
        ...
        elapsed = ELAPSED_MILLISECONDS_SINCE(
            item->added_tv_sec, item->added_tv_usec, tv_sec, tv_usec);
...
static void bus_expirelist_expire(BusExpireList* list)
{
    ...
    if (list->items != NULL)
    {
        ...
        next_interval =
            do_expiration_with_monotonic_time(list, tv_sec, tv_usec);
    }
...
static dbus_bool_t test_expire_func(BusExpireList* list, DBusList* link,
                                    void* data)
{
    TestExpireItem* t;
...
#define EXPIRE_AFTER 100
    ...
    time_add_milliseconds(&tv_sec_not_expired, &tv_usec_not_expired,
                          EXPIRE_AFTER - 1);
    ...
    next_interval = do_expiration_with_monotonic_time(list, tv_sec_not_expired,
                                                      tv_usec_not_expired);
    _dbus_assert(item->expire_count == 0);
    _dbus_verbose("next_interval = %d\n", next_interval);
    _dbus_assert(next_interval == 1);

    next_interval = do_expiration_with_monotonic_time(list, tv_sec_expired,
                                                      tv_usec_expired);
    ...
    next_interval =
        do_expiration_with_monotonic_time(list, tv_sec_past, tv_usec_past);
    _dbus_assert(item->expire_count == 1);
    _dbus_verbose("next_interval = %d\n", next_interval);
