...
struct BOOST_PYTHON_DECL registration
{
  ...
  private:
    void operator=(
        registration); // This is not defined, and just keeps MWCW happy.
#endif
};
...
inline registration::registration(type_info target_type, bool is_shared_ptr) :
    target_type(target_type), lvalue_chain(0), rvalue_chain(0),
    m_class_object(0), m_to_python(0), m_to_python_target_type(0),
