...
static int mkfilter_host_byname(const char* name, char* buffer, size_t buflen)
{
    ...
    /* build filter */
    return mysnprintf(buffer, buflen, "(&%s(%s=%s))", host_filter,
                      attmap_host_cn, safename);
}

static int mkfilter_host_byaddr(const char* addrstr, char* buffer,
                                size_t buflen)
{
    char safeaddr[64];
    /* escape attribute */
...
NSLCD_HANDLE(host, byname, NSLCD_ACTION_HOST_BYNAME, char name[BUFLEN_HOSTNAME];
...
NSLCD_HANDLE(
    host, byaddr, NSLCD_ACTION_HOST_BYADDR, int af; char addr[64];
...
NSLCD_HANDLE(host, all, NSLCD_ACTION_HOST_ALL, const char* filter;
             log_setrequest("host(all)");
             , (filter = host_filter, 0), write_host(fp, entry))
