...
class mapped_file_impl
{
  public:
    ...
    BOOST_STATIC_CONSTANT(size_type,
                          max_length = mapped_file_source::max_length);
    mapped_file_impl();
    ~mapped_file_impl();
...
void mapped_file_impl::open_file(param_type p)
{
    ...
    DWORD dwCreationDisposition =
        (p.new_file_size != 0 && !readonly) ? CREATE_ALWAYS : OPEN_EXISTING;
    DWORD dwFlagsandAttributes =
        readonly ? FILE_ATTRIBUTE_READONLY : FILE_ATTRIBUTE_TEMPORARY;
    handle_ = p.path.is_wide()
                  ? ::CreateFileW(p.path.c_wstr(), dwDesiredAccess,
                                  FILE_SHARE_READ, NULL, dwCreationDisposition,
                                  dwFlagsandAttributes, NULL)
                  : ::CreateFileA(p.path.c_str(), dwDesiredAccess,
                                  FILE_SHARE_READ, NULL, dwCreationDisposition,
                                  dwFlagsandAttributes, NULL);
...
void mapped_file_impl::try_map_file(param_type p)
{
    ...
    // Create mapping
    DWORD protect =
        priv ? PAGE_WRITECOPY : readonly ? PAGE_READONLY : PAGE_READWRITE;
    mapped_handle_ = ::CreateFileMappingA(handle_, NULL, protect, 0, 0, NULL);
    if (mapped_handle_ == NULL)
        cleanup_and_throw("failed create mapping");

    // Access data
    DWORD access =
        priv ? FILE_MAP_COPY : readonly ? FILE_MAP_READ : FILE_MAP_WRITE;
    void* data = ::MapViewOfFileEx(
        mapped_handle_, access, (DWORD)(p.offset >> 32),
...
#else
    void* data = ::BOOST_IOSTREAMS_FD_MMAP(
        const_cast<char*>(p.hint), size_,
        readonly ? PROT_READ : (PROT_READ | PROT_WRITE),
        priv ? MAP_PRIVATE : MAP_SHARED, handle_, p.offset);
    if (data == MAP_FAILED)
...
void mapped_file_params_base::normalize()
{
    ...
    if (flags)
    {
        switch (flags)
        {
            ...
            default:
                boost::throw_exception(
                    BOOST_IOSTREAMS_FAILURE("invalid flags"));
        }
    }
    else
    {
        flags = (mode & BOOST_IOS::out) ? mapped_file::readwrite
...
mapped_file_source::mapped_file_source() : pimpl_(new impl_type)
{
}
...
mapped_file::mapped_file(const mapped_file& other) : delegate_(other.delegate_)
{
}
