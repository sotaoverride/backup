...
void Mouse::readyRead(s8* buf, u32 len)
{
    ...
    for (; len--; ev++)
    {
        ...
        if (ev->wdy)
            type = Wheel;
        else if ((ev->type & GPM_MOVE) || (ev->type & GPM_DRAG))
            type = Move;
        else if (ev->type & GPM_DOWN)
            type = (ev->type & GPM_DOUBLE) ? DblClick : Press;
        ...
        if (ev->buttons & GPM_B_RIGHT)
            buttons |= RightButton;
        if (type == Wheel)
            buttons |= ((ev->wdy < 0) ? WheelDown : WheelUp);

        if (ev->modifiers & (1 << KG_SHIFT))
            buttons |= ShiftButton;
        if (ev->modifiers & (1 << KG_CTRL))
            buttons |= ControlButton;
        if (ev->modifiers & ((1 << KG_ALT) | (1 << KG_ALTGR)))
            buttons |= AltButton;
        ...
        if (newy >= maxy)
            newy = maxy - 1;
        if (newx == x && newy == y && !(buttons & MouseButtonMask))
            continue;
