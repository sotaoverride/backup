...
static void selPaintButtons(rfbSelectData* m, rfbBool invertOk,
                            rfbBool invertCancel)
{
    ...
    if (invertOk)
    {
        rfbFillRect(s, m->okBX, m->okY - m->textH, m->okBX + m->buttonWidth,
                    m->okY, colour);
        rfbDrawStringWithClip(
            s, m->font, m->okX + m->xhot, m->okY - 1 + m->yhot, okStr, m->x1,
            m->okY - m->textH, m->x2, m->okY, bcolour, colour);
    }
    else
        rfbDrawString(s, m->font, m->okX + m->xhot, m->okY - 1 + m->yhot, okStr,
                      colour);
    ...
    if (invertCancel)
    {
        ...
        rfbDrawStringWithClip(
            s, m->font, m->cancelX + m->xhot, m->okY - 1 + m->yhot, cancelStr,
            m->x1, m->okY - m->textH, m->x2, m->okY, bcolour, colour);
    }
    else
        rfbDrawString(s, m->font, m->cancelX + m->xhot, m->okY - 1 + m->yhot,
                      cancelStr, colour);
...
static void selPaintLine(rfbSelectData* m, int line, rfbBool invert)
{
    ...
    if (y2 > m->y2)
        y2 = m->y2;
    rfbFillRect(m->screen, m->x1, y1, m->x2, y2,
                invert ? m->colour : m->backColour);
    if (m->displayStart + line < m->listSize)
        rfbDrawStringWithClip(m->screen, m->font, m->x1 + m->xhot,
                              y2 - 1 + m->yhot, m->list[m->displayStart + line],
                              m->x1, y1, m->x2, y2,
    ...
    if (_index < m->displayStart || _index >= m->displayStart + m->pageH)
    {
        ...
        if (delta > -m->pageH && delta < m->pageH)
        {
            if (delta > 0)
            {
                ...
                lineEnd = m->pageH;
                rfbDoCopyRect(m->screen, m->x1, m->y1, m->x2,
                              m->y1 + lineStart * m->textH, 0,
                              -delta * m->textH);
            }
            else
            {
                lineStart = 0;
                lineEnd = -delta;
                rfbDoCopyRect(m->screen, m->x1, m->y1 + lineEnd * m->textH,
                              m->x2, m->y2, 0, -delta * m->textH);
            }
        }
        else
        {
...
static void selKbdAddEvent(rfbBool down, rfbKeySym keySym, rfbClientPtr cl)
{
    if (down)
    {
        if (keySym > ' ' && keySym < 0xff)
        {
            ...
            for (i = m->selected + 1; m->list[i] && tolower(m->list[i][0]) != c;
                 i++)
                ;
            if (!m->list[i])
...
int rfbSelectBox(rfbScreenInfoPtr rfbScreen, rfbFontDataPtr font, char** list,
                 int x1, int y1, int x2, int y2, rfbPixel colour,
                 rfbPixel backColour, int border,
                 SelectionChangedHookPtr selChangedHook)
{
    ...
    for (j = 0; j < y2 - y1; j++)
        memcpy(frameBufferBackup + j * (x2 - x1) * bpp,
               rfbScreen->frameBuffer + j * rfbScreen->paddedWidthInBytes +
                   x1 * bpp,
               (x2 - x1) * bpp);
    ...
    for (j = 0; j < y2 - y1; j++)
        memcpy(rfbScreen->frameBuffer + j * rfbScreen->paddedWidthInBytes +
                   x1 * bpp,
               frameBufferBackup + j * (x2 - x1) * bpp, (x2 - x1) * bpp);
