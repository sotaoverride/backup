...
G_DEFINE_BOXED_TYPE(GUnixMountEntry, g_unix_mount_entry, g_unix_mount_copy,
                    g_unix_mount_free)
...
gboolean g_unix_is_system_fs_type(const char* fs_type)
{
    const char* ignore_fs[] = {
        "adfs",      "afs",        "auto",       "autofs",      "autofs4",
        "cgroup",    "configfs",   "cxfs",       "debugfs",     "devfs",
        "devpts",    "devtmpfs",   "ecryptfs",   "fdescfs",     "fusectl",
        "gfs",       "gfs2",       "gpfs",       "hugetlbfs",   "kernfs",
        "linprocfs", "linsysfs",   "lustre",     "lustre_lite", "mfs",
        "mqueue",    "ncpfs",      "nfsd",       "nullfs",      "ocfs2",
        "overlay",   "proc",       "procfs",     "pstore",      "ptyfs",
        "rootfs",    "rpc_pipefs", "securityfs", "selinuxfs",   "sysfs",
        "tmpfs",     "usbfs",      NULL};
...
{
    const char* ignore_devices[] = {"none",      "sunrpc",  "devpts", "nfsd",
                                    "/dev/loop", "/dev/vn", NULL};
...
static gboolean guess_system_internal(const char* mountpoint, const char* fs,
                                      const char* device, const char* root)
{
     ...
     * ignored (e.g. x-gvfs-show doesn't work for them, trash backend can't
     * handle files on btrfs subvolumes). libmount-based _g_get_unix_mounts()
     * implementation provides a root path. So there is no need to completely
     * ignore those mounts, because e.g. our volume monitors can use the root
     * path to not mengle those mounts with the "regular" mounts (i.e. which
     * points to the root). But because those mounts usually just duplicate
     * other mounts and are completely ignored with mntend-based implementation,
     * let's mark them as system internal. Given the different approches it
     * doesn't mean that all mounts which were ignored will be system internal
     * now, but this should work in most cases. For more info, see
     * g_unix_mount_get_root_path() annotation, comment in mntent-based
     * _g_get_unix_mounts() implementation and the
...
static GUnixMountEntry*
    create_unix_mount_entry(const char* device_path, const char* mount_path,
                            const char* root_path, const char* filesystem_type,
                            const char* options, gboolean is_read_only)
{
    ...
    mount_entry->is_system_internal = guess_system_internal(
        mount_entry->mount_path, mount_entry->filesystem_type,
        mount_entry->device_path, mount_entry->root_path);

    return mount_entry;
}

static GUnixMountPoint*
    create_unix_mount_point(const char* device_path, const char* mount_path,
                            const char* filesystem_type, const char* options,
                            gboolean is_read_only, gboolean is_user_mountable,
                            gboolean is_loopback)
{
...
static GList* _g_get_unix_mounts(void)
{
    ...
    while (mnt_table_next_fs(table, iter, &fs) == 0)
    {
        ...
        if (mount_options)
        {
            mnt_optstr_get_flags(mount_options, &mount_flags,
                                 mnt_get_builtin_optmap(MNT_LINUX_MAP));
            g_free(mount_options);
        }
        is_read_only = (mount_flags & MS_RDONLY) ? TRUE : FALSE;

        mount_entry = create_unix_mount_entry(
            device_path, mnt_fs_get_target(fs), mnt_fs_get_root(fs),
            mnt_fs_get_fstype(fs), mnt_fs_get_options(fs), is_read_only);
...
static const char* get_mtab_read_file(void)
{
#ifdef _PATH_MOUNTED
#ifdef __linux__
    return "/proc/mounts";
...
static GList* _g_get_unix_mounts(void)
{
...
#endif
    {
        ...
        mount_entry = create_unix_mount_entry(device_path, mntent->mnt_dir,
                                              NULL, mntent->mnt_type,
                                              mntent->mnt_opts, is_read_only);

        g_hash_table_insert(mounts_hash, mount_entry->device_path,
...
static const char* get_mtab_monitor_file(void)
{
     ...
     * order to avoid races between g_unix_mounts_get and "mounts-changed"
     * signal: https://bugzilla.gnome.org/show_bug.cgi?id=782814
     */
...
static GList* _g_get_unix_mounts(void)
{
    ...
    while (!getmntent(file, &mntent))
    {
        ...
        mount_entry = create_unix_mount_entry(
            mntent.mnt_special, mntent.mnt_mountp, NULL, mntent.mnt_fstype,
            mntent.mnt_opts, is_read_only);
...
/* mntctl.h (AIX) {{{2 */
#elif defined(HAVE_SYS_MNTCTL_H) && defined(HAVE_SYS_VMOUNT_H) &&              \
    defined(HAVE_SYS_VFS_H)
...
static GList* _g_get_unix_mounts(void)
{
    ...
    while (vmount_number > 0)
    {
        ...
        mount_entry = create_unix_mount_entry(
            vmt2dataptr(vmount_info, VMT_OBJECT),
            vmt2dataptr(vmount_info, VMT_STUB), NULL,
            fs_info == NULL ? "unknown" : fs_info->vfsent_name, NULL,
            is_read_only);

        return_list = g_list_prepend(return_list, mount_entry);

        vmount_info =
            (struct vmount*)((char*)vmount_info + vmount_info->vmt_length);
        vmount_number--;
    }
...
#error statfs juggling failed
        ...
        mount_entry = create_unix_mount_entry(
            mntent[i].f_mntfromname, mntent[i].f_mntonname, NULL,
            mntent[i].f_fstypename, NULL, is_read_only);
...
static GList* _g_get_unix_mount_points(void)
{
    ...
    while (mnt_table_next_fs(table, iter, &fs) == 0)
    {
        ...
        if (mount_options)
        {
            ...
            mnt_optstr_get_flags(mount_options, &mount_flags,
                                 mnt_get_builtin_optmap(MNT_LINUX_MAP));
            mnt_optstr_get_flags(mount_options, &userspace_flags,
            ...
            if ((mount_fstype != NULL &&
                 g_strcmp0("supermount", mount_fstype) == 0) ||
        ...
        mount_point = create_unix_mount_point(
            device_path, mount_path, mount_fstype, mount_options, is_read_only,
            is_user_mountable, is_loopback);
        if (mount_options)
            g_free(mount_options);
...
static GList* _g_get_unix_mount_points(void)
{
...
#endif
    {
        ...
        if ((mntent->mnt_type != NULL &&
             strcmp("supermount", mntent->mnt_type) == 0)
#ifdef HAVE_HASMNTOPT
        ...
        mount_point = create_unix_mount_point(
            device_path, mntent->mnt_dir, mntent->mnt_type, mntent->mnt_opts,
...
static GList* _g_get_unix_mount_points(void)
{
    ...
    while (!getmntent(file, &mntent))
    {
        ...
        mount_point = create_unix_mount_point(
            mntent.mnt_special, mntent.mnt_mountp, mntent.mnt_fstype,
            mntent.mnt_mntopts, is_read_only, is_user_mountable, is_loopback);
...
static GList* _g_get_unix_mount_points(void)
{
    ...
    while (!aix_fs_get(file, &mntent))
    {
        ...
        {
            mount_point = create_unix_mount_point(
                mntent.mnt_special, mntent.mnt_mount, mntent.mnt_fstype,
                mntent.mnt_options, TRUE, TRUE, FALSE);
...
static GList* _g_get_unix_mount_points(void)
{
    ...
    while ((fstab = getfsent()) != NULL)
    {
        ...
        mount_point = create_unix_mount_point(
            fstab->fs_spec, fstab->fs_file, fstab->fs_vfstype, fstab->fs_mntops,
            is_read_only, is_user_mountable, FALSE);
...
static void fstab_file_changed(GFileMonitor* monitor, GFile* file,
                               ...
                               gpointer user_data)
{
    ...
    g_context_specific_group_emit(&mount_monitor_group,
                                  signals[MOUNTPOINTS_CHANGED]);
}

static gboolean mtab_file_changed_cb(gpointer user_data)
{
...
static void mtab_file_changed(GFileMonitor* monitor, GFile* file,
                              GFile* other_file, GFileMonitorEvent event_type,
                              gpointer user_data)
{
    if (event_type != G_FILE_MONITOR_EVENT_CHANGED &&
     ...
     * This should behave equally to GIOChannel based monitoring. See Bug
     * 792235.
     */
...
static gboolean proc_mounts_changed(GIOChannel* channel, GIOCondition cond,
                                    gpointer user_data)
{
    if (cond & G_IO_ERR)
    {
...
static gboolean mount_change_poller(gpointer user_data)
{
...
static void mount_monitor_stop(void)
{
    if (fstab_monitor)
    {
...
static void mount_monitor_start(void)
{
    GFile* file;
    ...
    if (get_fstab_file() != NULL)
    {
        ...
        g_signal_connect(fstab_monitor, "changed",
                         (GCallback)fstab_file_changed, NULL);
    }
    ...
    if (get_mtab_monitor_file() != NULL)
    {
        ...
        if (g_str_has_prefix(mtab_path, "/proc/"))
        {
            ...
            {
                g_warning("Error creating IO channel for %s: %s (%s, %d)",
                          mtab_path, error->message,
            ...
            else
            {
                proc_mounts_watch_source =
                    g_io_create_watch(proc_mounts_channel, G_IO_ERR);
                g_source_set_callback(proc_mounts_watch_source,
                                      (GSourceFunc)proc_mounts_changed, NULL,
                                      NULL);
        ...
        else
        {
            ...
            g_object_unref(file);
            g_signal_connect(mtab_monitor, "changed",
                             (GCallback)mtab_file_changed, NULL);
        }
    }
    else
    {
        ...
        mount_poller_time = (guint64)g_get_monotonic_time();
        g_source_set_callback(proc_mounts_watch_source, mount_change_poller,
                              NULL, NULL);
        g_source_attach(proc_mounts_watch_source,
                        g_main_context_get_thread_default());
        g_source_unref(proc_mounts_watch_source);
...
static void g_unix_mount_monitor_finalize(GObject* object)
{
    ...
    g_context_specific_group_remove(&mount_monitor_group, monitor->context,
                                    monitor, mount_monitor_stop);
...
static void g_unix_mount_monitor_class_init(GUnixMountMonitorClass* klass)
{
     ...
     */
    signals[MOUNTS_CHANGED] = g_signal_new(
        I_("mounts-changed"), G_TYPE_FROM_CLASS(klass), G_SIGNAL_RUN_LAST, 0,
     ...
     */
    signals[MOUNTPOINTS_CHANGED] = g_signal_new(
        I_("mountpoints-changed"), G_TYPE_FROM_CLASS(klass), G_SIGNAL_RUN_LAST,
...
GUnixMountMonitor* g_unix_mount_monitor_get(void)
{
    return g_context_specific_group_get(
        &mount_monitor_group, G_TYPE_UNIX_MOUNT_MONITOR,
        G_STRUCT_OFFSET(GUnixMountMonitor, context), mount_monitor_start);
}
 ...
 * Since: 2.60
 */
const gchar* g_unix_mount_get_root_path(GUnixMountEntry* mount_entry)
{
    g_return_val_if_fail(mount_entry != NULL, NULL);
 ...
 * Since: 2.58
 */
const gchar* g_unix_mount_get_options(GUnixMountEntry* mount_entry)
{
    g_return_val_if_fail(mount_entry != NULL, NULL);
 ...
 * Since: 2.32
 */
const gchar* g_unix_mount_point_get_options(GUnixMountPoint* mount_point)
{
    g_return_val_if_fail(mount_point != NULL, NULL);
...
static GUnixMountType guess_mount_type(const char* mount_path,
...
static GUnixMountType g_unix_mount_guess_type(GUnixMountEntry* mount_entry)
{
    ...
    g_return_val_if_fail(mount_entry->mount_path != NULL,
                         G_UNIX_MOUNT_TYPE_UNKNOWN);
    g_return_val_if_fail(mount_entry->device_path != NULL,
                         G_UNIX_MOUNT_TYPE_UNKNOWN);
    g_return_val_if_fail(mount_entry->filesystem_type != NULL,
                         G_UNIX_MOUNT_TYPE_UNKNOWN);

    return guess_mount_type(mount_entry->mount_path, mount_entry->device_path,
    ...
    g_unix_mount_point_guess_type(GUnixMountPoint* mount_point)
{
    ...
    g_return_val_if_fail(mount_point->mount_path != NULL,
                         G_UNIX_MOUNT_TYPE_UNKNOWN);
    g_return_val_if_fail(mount_point->device_path != NULL,
                         G_UNIX_MOUNT_TYPE_UNKNOWN);
    g_return_val_if_fail(mount_point->filesystem_type != NULL,
                         G_UNIX_MOUNT_TYPE_UNKNOWN);

    return guess_mount_type(mount_point->mount_path, mount_point->device_path,
                                ...
                                gboolean use_symbolic)
{
    ...
    switch (type)
    {
        case G_UNIX_MOUNT_TYPE_HD:
            if (is_mount_point)
                icon_name = use_symbolic ? "drive-removable-media-symbolic"
                                         : "drive-removable-media";
            else
                icon_name =
                    use_symbolic ? "drive-harddisk-symbolic" : "drive-harddisk";
            break;
        ...
        case G_UNIX_MOUNT_TYPE_JAZ:
            if (is_mount_point)
                ...
                icon_name =
                    use_symbolic ? "media-removable-symbolic" : "media-floppy";
            break;
        case G_UNIX_MOUNT_TYPE_CDROM:
            if (is_mount_point)
                icon_name =
                    use_symbolic ? "drive-optical-symbolic" : "drive-optical";
            else
                icon_name =
                    use_symbolic ? "media-optical-symbolic" : "media-optical";
            break;
        case G_UNIX_MOUNT_TYPE_NFS:
            icon_name =
                use_symbolic ? "folder-remote-symbolic" : "folder-remote";
            break;
        case G_UNIX_MOUNT_TYPE_MEMSTICK:
            if (is_mount_point)
                ...
                icon_name =
                    use_symbolic ? "media-removable-symbolic" : "media-flash";
            break;
        case G_UNIX_MOUNT_TYPE_CAMERA:
            if (is_mount_point)
                ...
                icon_name =
                    use_symbolic ? "camera-photo-symbolic" : "camera-photo";
            break;
        case G_UNIX_MOUNT_TYPE_IPOD:
            if (is_mount_point)
                ...
                icon_name = use_symbolic ? "multimedia-player-symbolic"
                                         : "multimedia-player";
            break;
...
GIcon* g_unix_mount_guess_icon(GUnixMountEntry* mount_entry)
{
    return g_themed_icon_new_with_default_fallbacks(
        type_to_icon(g_unix_mount_guess_type(mount_entry), FALSE, FALSE));
}
...
{
    return g_themed_icon_new_with_default_fallbacks(
        type_to_icon(g_unix_mount_guess_type(mount_entry), FALSE, TRUE));
}
...
{
    return g_themed_icon_new_with_default_fallbacks(
        type_to_icon(g_unix_mount_point_guess_type(mount_point), TRUE, FALSE));
}
...
{
    return g_themed_icon_new_with_default_fallbacks(
        type_to_icon(g_unix_mount_point_guess_type(mount_point), TRUE, TRUE));
}
...
gboolean g_unix_mount_guess_should_display(GUnixMountEntry* mount_entry)
{
    ...
    if (mount_path != NULL)
    {
        ...
        /* Hide mounts within a dot path, suppose it was a purpose to hide this
         * mount */
        ...
        else
        {
            ...
            if (strncmp(mount_path, "/run/media/", strlen("/run/media/")) ==
                    0 &&
                strncmp(mount_path + strlen("/run/media/"), user_name,
                        user_name_len) == 0 &&
        ...
        if (!S_ISLNK(statbuf.st_mode))
        {
            ...
            if (f != NULL)
            {
                ...
                while ((entp = getmntent(f)) != NULL)
                {
                    ...
                    {
                        strncpy(real_dev_root, entp->mnt_fsname,
                                sizeof(real_dev_root) - 1);
            ...
            /* no, that didn't work.. next we could scan /dev ... but I
             * digress.. */
        }
        else
        {
