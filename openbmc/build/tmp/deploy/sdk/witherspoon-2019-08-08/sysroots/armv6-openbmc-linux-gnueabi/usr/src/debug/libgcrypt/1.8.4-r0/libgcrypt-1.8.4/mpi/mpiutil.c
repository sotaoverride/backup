...
mpi_ptr_t _gcry_mpi_alloc_limb_space(unsigned int nlimbs, int secure)
{
    mpi_ptr_t p;
    size_t len;
...
void _gcry_mpi_free(gcry_mpi_t a)
{
    ...
    if ((a->flags & ~(1 | 2 | 4 | 16 | GCRYMPI_FLAG_USER1 | GCRYMPI_FLAG_USER2 |
...
static void mpi_set_secure(gcry_mpi_t a)
{
    mpi_ptr_t ap, bp;
...
gcry_mpi_t _gcry_mpi_alloc_like(gcry_mpi_t a)
{
    ...
    {
        int n = (a->sign + 7) / 8;
        void* p = _gcry_is_secure(a->d) ? xtrymalloc_secure(n) : xtrymalloc(n);
        memcpy(p, a->d, n);
        b = mpi_set_opaque(NULL, p, a->sign);
    }
    else if (a)
...
gcry_mpi_t _gcry_mpi_set(gcry_mpi_t w, gcry_mpi_t u)
{
    mpi_ptr_t wp, up;
