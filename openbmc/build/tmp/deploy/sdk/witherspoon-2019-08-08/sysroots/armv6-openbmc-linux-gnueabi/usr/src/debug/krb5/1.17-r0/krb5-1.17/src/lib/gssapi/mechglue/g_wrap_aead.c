...
static OM_uint32 val_wrap_aead_args(OM_uint32* minor_status,
...
static OM_uint32 gssint_wrap_aead_iov_shim(
    gss_mechanism mech, OM_uint32* minor_status, gss_ctx_id_t context_handle,
    int conf_req_flag, gss_qop_t qop_req, gss_buffer_t input_assoc_buffer,
    ...
    gss_buffer_t output_message_buffer)
{
    ...
    status =
        mech->gss_wrap_iov_length(minor_status, context_handle, conf_req_flag,
    ...
    output_message_buffer->value =
        gssalloc_malloc(output_message_buffer->length);
    ...
    memcpy(iov[i].buffer.value, input_payload_buffer->value,
           iov[i].buffer.length);
    i++;
    ...
    status = mech->gss_wrap_iov(minor_status, context_handle, conf_req_flag,
...
OM_uint32 gssint_wrap_aead(gss_mechanism mech, OM_uint32* minor_status,
                           gss_union_ctx_id_t ctx, int conf_req_flag,
                           ...
                           gss_buffer_t output_message_buffer)
{
    ...
    {
        status = mech->gss_wrap_aead(minor_status, ctx->internal_ctx_id,
                                     conf_req_flag, qop_req, input_assoc_buffer,
                                     input_payload_buffer, conf_state,
    ...
    else if (mech->gss_wrap_iov && mech->gss_wrap_iov_length)
    {
        status = gssint_wrap_aead_iov_shim(
            mech, minor_status, ctx->internal_ctx_id, conf_req_flag, qop_req,
...
OM_uint32
    KRB5_CALLCONV gss_wrap_aead(minor_status, context_handle, conf_req_flag,
                                qop_req, input_assoc_buffer,
                                input_payload_buffer, conf_state,
                                output_message_buffer) OM_uint32* minor_status;
gss_ctx_id_t context_handle;
int conf_req_flag;
gss_qop_t qop_req;
gss_buffer_t input_assoc_buffer;
...
gss_buffer_t output_message_buffer;
{
    ...
    status =
        val_wrap_aead_args(minor_status, context_handle, conf_req_flag, qop_req,
                           input_assoc_buffer, input_payload_buffer, conf_state,
                           output_message_buffer);
    if (status != GSS_S_COMPLETE)
        return (status);
    ...
    return gssint_wrap_aead(mech, minor_status, ctx, conf_req_flag, qop_req,
