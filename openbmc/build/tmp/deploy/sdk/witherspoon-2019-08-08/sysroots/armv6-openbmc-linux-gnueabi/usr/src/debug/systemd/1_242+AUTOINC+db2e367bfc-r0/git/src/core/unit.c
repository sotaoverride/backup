...
const UnitVTable* const unit_vtable[_UNIT_TYPE_MAX] = {
    [UNIT_SERVICE] = &service_vtable, [UNIT_SOCKET] = &socket_vtable,
    [UNIT_TARGET] = &target_vtable,   [UNIT_DEVICE] = &device_vtable,
    [UNIT_MOUNT] = &mount_vtable,     [UNIT_AUTOMOUNT] = &automount_vtable,
    [UNIT_SWAP] = &swap_vtable,       [UNIT_TIMER] = &timer_vtable,
    [UNIT_PATH] = &path_vtable,       [UNIT_SLICE] = &slice_vtable,
    [UNIT_SCOPE] = &scope_vtable,
};

static void maybe_warn_about_dependency(Unit* u, const char* other,
...
static void unit_init(Unit* u)
{
    ...
    if (ec)
    {
        ...
        ec->keyring_mode = MANAGER_IS_SYSTEM(u->manager) ? EXEC_KEYRING_SHARED
...
bool unit_may_gc(Unit* u)
{
     ...
     * in unit_gc_sweep(), but using markers to properly collect dependency
     * loops.
     */
    ...
    /* If the unit is inactive and failed and no job is queued for it, then
    ...
    /* But we keep the unit object around for longer when it is referenced or
     * configured to not be gc'ed */
    ...
    if (u->cgroup_path)
    {
        /* If the unit has a cgroup, then check whether there's anything in it.
         * If so, we should stay around. Units with active processes should
         * never be collected. */

        r = cg_is_empty_recursive(SYSTEMD_CGROUP_CONTROLLER, u->cgroup_path);
        if (r < 0)
            log_unit_debug_errno(
                u, r, "Failed to determine whether cgroup %s is empty: %m",
                u->cgroup_path);
        if (r <= 0)
            return false;
    }
...
void unit_submit_to_stop_when_unneeded_queue(Unit* u)
{
    ...
    LIST_PREPEND(stop_when_unneeded_queue, u->manager->stop_when_unneeded_queue,
                 u);
...
static void unit_remove_transient(Unit* u)
{
    ...
    STRV_FOREACH(i, u->dropin_paths)
    {
        ...
        p = dirname_malloc(
            *i); /* Get the drop-in directory from the drop-in file */
        if (!p)
            continue;

        pp = dirname_malloc(
            p); /* Get the config directory from the drop-in directory */
        if (!pp)
            continue;
    ...
    for (;;)
    {
        ...
        else
        {
            ...
            PATH_FOREACH_PREFIX_MORE(s, path)
            {
                ...
                x = hashmap_get2(u->manager->units_requiring_mounts_for, s,
                                 (void**)&y);
                if (!x)
                    continue;
                ...
                if (set_isempty(x))
                {
                    (void)hashmap_remove(u->manager->units_requiring_mounts_for,
                                         y);
                    free(y);
                    set_free(x);
                }
...
void unit_free(Unit* u)
{
    ...
    {
        /* A unit is being dropped from the tree, make sure our parent slice
         * recalculates the member mask */
        unit_invalidate_cgroup_members_masks(UNIT_DEREF(u->slice));

        /* And make sure the parent is realized again, updating cgroup
         * memberships */
    ...
    if (!sd_id128_is_null(u->invocation_id))
        hashmap_remove_value(u->manager->units_by_invocation_id,
                             &u->invocation_id, u);
    ...
    if (u->in_stop_when_unneeded_queue)
        LIST_REMOVE(stop_when_unneeded_queue,
...
static int reserve_dependencies(Unit* u, Unit* other, UnitDependency d)
{
    ...
    n_reserve = hashmap_size(other->dependencies[d]) -
                !!hashmap_get(other->dependencies[d], u);
                               ...
                               UnitDependency d)
{
    ...
    /* Merges all dependencies of type 'd' of the unit 'other' into the deps of
     * the unit 'u' */
    ...
    /* Fix backwards pointers. Let's iterate through all dependendent units of
     * the other unit. */
    HASHMAP_FOREACH_KEY(v, back, other->dependencies[d], i)
    {
        UnitDependency k;

        /* Let's now iterate through the dependencies of that dependencies of
         * the other units, looking for
        ...
        for (k = 0; k < _UNIT_DEPENDENCY_MAX; k++)
        {
            ...
            else
            {
                ...
                /* Let's drop this dependency between "back" and "other", and
                 * let's create it between "back" and "u" instead. Let's merge
                 * the bit masks of the dependency we are moving, and any such
                 * dependency which might already exist */
                ...
                di_merged = (UnitDependencyInfo){
                    ...
                    .destination_mask =
                        di_u.destination_mask | di_other.destination_mask,
                };

                r = hashmap_remove_and_replace(back->dependencies[k], other, u,
                                               di_merged.data);
                if (r < 0)
                    log_warning_errno(
                        r,
                        "Failed to remove/replace: back=%s other=%s u=%s: %m",
                        back->id, other_id, u->id);
                assert(r >= 0);

                /* assert_se(hashmap_remove_and_replace(back->dependencies[k],
                 * other, u, di_merged.data) >= 0); */
            }
        }
    }
    ...
    assert_se(hashmap_complete_move(&u->dependencies[d],
                                    &other->dependencies[d]) == 0);
...
int unit_merge(Unit* u, Unit* other)
{
    ...
    if (!unit_type_may_alias(u->type)) /* Merging only applies to unit names
...
int unit_add_exec_dependencies(Unit* u, ExecContext* c)
{
    ...
    if (c->private_tmp)
    {
        ...
        r = unit_add_dependency_by_name(u, UNIT_AFTER,
                                        SPECIAL_TMPFILES_SETUP_SERVICE, true,
                                        UNIT_DEPENDENCY_FILE);
        if (r < 0)
            return r;
    }

    if (!IN_SET(c->std_output, EXEC_OUTPUT_JOURNAL,
                EXEC_OUTPUT_JOURNAL_AND_CONSOLE, EXEC_OUTPUT_KMSG,
                EXEC_OUTPUT_KMSG_AND_CONSOLE, EXEC_OUTPUT_SYSLOG,
                EXEC_OUTPUT_SYSLOG_AND_CONSOLE) &&
        !IN_SET(c->std_error, EXEC_OUTPUT_JOURNAL,
                EXEC_OUTPUT_JOURNAL_AND_CONSOLE, EXEC_OUTPUT_KMSG,
                EXEC_OUTPUT_KMSG_AND_CONSOLE, EXEC_OUTPUT_SYSLOG,
                EXEC_OUTPUT_SYSLOG_AND_CONSOLE))
        return 0;
...
static void print_unit_dependency_mask(FILE* f, const char* kind,
                                       UnitDependencyMask mask, bool* space)
{
    const struct
    {
...
void unit_dump(Unit* u, FILE* f, const char* prefix)
{
    ...
    Iterator i;
    const char* prefix2;
    char timestamp0[FORMAT_TIMESTAMP_MAX], timestamp1[FORMAT_TIMESTAMP_MAX],
        timestamp2[FORMAT_TIMESTAMP_MAX], timestamp3[FORMAT_TIMESTAMP_MAX],
    ...
    fprintf(f,
            ...
            strna(format_timestamp(timestamp0, sizeof(timestamp0),
                                   u->state_change_timestamp.realtime)),
            prefix,
            ...
            yes_no(unit_need_daemon_reload(u)), prefix, yes_no(u->transient),
            prefix, yes_no(u->perpetual), prefix,
            collect_mode_to_string(u->collect_mode), prefix,
            strna(unit_slice_name(u)), prefix, strna(u->cgroup_path), prefix,
    ...
    if (!sd_id128_is_null(u->invocation_id))
        fprintf(f, "%s\tInvocation ID: " SD_ID128_FORMAT_STR "\n", prefix,
                SD_ID128_FORMAT_VAL(u->invocation_id));
    ...
    for (d = 0; d < _UNIT_DEPENDENCY_MAX; d++)
    {
        ...
        HASHMAP_FOREACH_KEY(di.data, other, u->dependencies[d], i)
        {
            ...
            fprintf(f, "%s\t%s: %s (", prefix, unit_dependency_to_string(d),
                    other->id);

            print_unit_dependency_mask(f, "origin", di.origin_mask, &space);
            print_unit_dependency_mask(f, "destination", di.destination_mask,
                                       &space);
    ...
    for (n = sd_bus_track_first(u->bus_track); n;
         n = sd_bus_track_next(u->bus_track))
...
int unit_load_fragment_and_dropin(Unit* u)
{
     ...
     * target unit needlessly. But we cannot be sure which drops-ins have
     * already been loaded and which not, at least without doing complicated
     * book-keeping, so let's always reread all drop-ins. */
...
static int unit_add_slice_dependencies(Unit* u)
{
    ...
    /* Slice units are implicitly ordered against their parent slices (as this
       relationship is encoded in the name), while all other units are ordered
       based on configuration (as in their case Slice= configures the
       relationship). */
    mask =
        u->type == UNIT_SLICE ? UNIT_DEPENDENCY_IMPLICIT : UNIT_DEPENDENCY_FILE;

    if (UNIT_ISSET(u->slice))
        return unit_add_two_dependencies(u, UNIT_AFTER, UNIT_REQUIRES,
                                         UNIT_DEREF(u->slice), true, mask);

    if (unit_has_name(u, SPECIAL_ROOT_SLICE))
        return 0;

    return unit_add_two_dependencies_by_name(u, UNIT_AFTER, UNIT_REQUIRES,
                                             SPECIAL_ROOT_SLICE, true, mask);
}
...
int unit_load(Unit* u)
{
    ...
    if (u->transient_file)
    {
        /* Finalize transient file: if this is a transient unit file, as soon as
         * we reach unit_load() the setup is complete, hence let's synchronize
         * the unit file we just wrote to disk. */
    ...
    if (u->load_state == UNIT_LOADED)
    {
        ...
        if (u->on_failure_job_mode == JOB_ISOLATE &&
            hashmap_size(u->dependencies[UNIT_ON_FAILURE]) > 1)
        {
            log_unit_error(u, "More than one OnFailure= dependencies specified "
                              "but OnFailureJobMode=isolate set. Refusing.");
            r = -ENOEXEC;
            goto fail;
        }

        if (u->job_running_timeout != USEC_INFINITY &&
            u->job_running_timeout > u->job_timeout)
            log_unit_warning(u, "JobRunningTimeoutSec= is greater than "
                                "JobTimeoutSec=, it has no effect.");

        /* We finished loading, let's ensure our parents recalculate the members
         * mask */
...
fail:
    /* We convert ENOEXEC errors to the UNIT_BAD_SETTING load state here.
     * Configuration parsing code should hence return ENOEXEC to ensure units
     * are placed in this state after loading */

    u->load_state = u->load_state == UNIT_STUB
                        ? UNIT_NOT_FOUND
                        : r == -ENOEXEC ? UNIT_BAD_SETTING : UNIT_ERROR;
    u->load_error = r;
...
_printf_(7, 8) static int log_unit_internal(void* userdata, int level,
                                            int error, const char* file,
                                            ...
                                            const char* format, ...)
{
    ...
    if (u)
        r = log_object_internalv(level, error, file, line, func,
                                 u->manager->unit_log_field, u->id,
                                 u->manager->invocation_log_field,
                                 u->invocation_id_string, format, ap);
    else
...
static bool unit_test_condition(Unit* u)
{
    ...
    u->condition_result = condition_test_list(
        u->conditions, condition_type_to_string, log_unit_internal, u);
...
static bool unit_test_assert(Unit* u)
{
    ...
    u->assert_result = condition_test_list(u->asserts, assert_type_to_string,
                                           log_unit_internal, u);
...
static bool unit_verify_deps(Unit* u)
{
    ...
    /* Checks whether all BindsTo= dependencies of this unit are fulfilled — if
     * they are also combined with After=. We do not check Requires= or
     * Requisite= here as they only should have an effect on the job processing,
     * but do not have any effect afterwards. We don't check BindsTo=
     * dependencies that are not used in
     * conjunction with After= as for them any such check would make things
     * entirely racy. */
    ...
    HASHMAP_FOREACH_KEY(v, other, u->dependencies[UNIT_BINDS_TO], j)
    {
        ...
        {
            log_unit_notice(u, "Bound to unit %s, but unit isn't active.",
                            other->id);
            return false;
        }
    }
 ...
 *         -ESTALE:     This unit has been started before and can't be started a
 * second time -ENOENT:     This is a triggering unit and unit to trigger is not
 * loaded
 */
int unit_start(Unit* u)
{
    ...
    /* If this is already started, then this will succeed. Note that this will
     * even succeed if this unit is not startable by the user. This is relied on
     * to detect when we need to wait for units and when waiting is finished. */
    state = unit_active_state(u);
    ...
    if (UNIT_VTABLE(u)->once_only &&
        dual_timestamp_is_set(&u->inactive_enter_timestamp))
        return -ESTALE;

    /* If the conditions failed, don't do anything at all. If we already are
     * activating this call might still be useful to speed up activation in case
     * there is some hold-off time, but we don't want to recheck the condition
     * in that case. */
    if (state != UNIT_ACTIVATING && !unit_test_condition(u))
    {

        /* Let's also check the start limit here. Normally, the start limit is
         * only checked by the .start() method of the unit type after it did
         * some additional checks verifying everything is in order (so that
         * those other checks can propagate errors properly). However, if a
         * condition check doesn't hold we don't get that far but we should
         * still ensure we are not called in a tight loop without a rate limit
         * check enforced, hence do the check here. Note that ECOMM is generally
         * not a reason for a job to fail, unlike most other errors here, hence
         * the chance is big that any triggering unit for us will trigger us
         * again. Note this condition check is a bit different from the
         * condition check inside the per-unit .start() function, as this one
         * will not change the unit's state in any way (and we shouldn't here,
        ...
        return log_unit_debug_errno(
            u, SYNTHETIC_ERRNO(ECOMM),
            "Starting requested but condition failed. Not starting unit.");
    }

    /* If the asserts failed, fail the entire job */
    if (state != UNIT_ACTIVATING && !unit_test_assert(u))
        return log_unit_notice_errno(u, SYNTHETIC_ERRNO(EPROTO),
                                     "Starting requested but asserts failed.");

    /* Units of types that aren't supported cannot be started. Note that we do
     * this test only after the condition checks, so that we rather return
     * condition check errors (which are usually not considered a true failure)
     * than "not supported" errors (which are considered a failure).
     */
    if (!unit_supported(u))
        return -EOPNOTSUPP;

    /* Let's make sure that the deps really are in order before we start this.
     * Normally the job engine should have taken care of this already, but let's
     * check this here again. After all, our
     * dependencies might not be in effect anymore, due to a reload or due to a
     * failed condition. */
    ...
    if (following)
    {
        log_unit_debug(u, "Redirecting start request from %s to %s.", u->id,
                       following->id);
    ...
    /* We don't suppress calls to ->start() here when we are already starting,
     * to allow this request to be used as a "hurry up" call, for example when
     * the unit is in some "auto restart" state where it waits for a holdoff
     * timer to elapse before it will start again. */
...
bool unit_can_start(Unit* u)
{
    ...
    if (UNIT_VTABLE(u)->once_only &&
        dual_timestamp_is_set(&u->inactive_exit_timestamp))
        return false;
...
int unit_stop(Unit* u)
{
    ...
    if (following)
    {
        log_unit_debug(u, "Redirecting stop request from %s to %s.", u->id,
                       following->id);
...
int unit_reload(Unit* u)
{
    ...
    if (following)
    {
        log_unit_debug(u, "Redirecting reload request from %s to %s.", u->id,
                       following->id);
    ...
    {
        /* Unit doesn't have a reload function, but we need to propagate the
         * reload anyway */
...
bool unit_is_unneeded(Unit* u)
{
    ...
    for (j = 0; j < ELEMENTSOF(deps); j++)
    {
        ...
        /* If a dependent unit has a job queued, is active or transitioning, or
         * is marked for restart, then don't clean this one up. */
...
static void check_unneeded_dependencies(Unit* u)
{
    ...
    /* Add all units this unit depends on to the queue that processes
     * StopWhenUnneeded= behaviour. */
...
static void unit_check_binds_to(Unit* u)
{
    ...
    {
        log_unit_warning(u,
                         "Unit is bound to inactive unit %s, but not stopping "
                         "since we tried this too often recently.",
                         other->id);
        return;
    }

    assert(other);
    log_unit_info(u, "Unit is bound to inactive unit %s. Stopping, too.",
                  other->id);

    /* A unit we need to run is gone. Sniff. Let's stop this. */
    r = manager_add_job(u->manager, JOB_STOP, u, JOB_FAIL, NULL, &error, NULL);
    if (r < 0)
        log_unit_warning_errno(u, r, "Failed to enqueue stop job, ignoring: %s",
...
void unit_start_on_failure(Unit* u)
{
    ...
    HASHMAP_FOREACH_KEY(v, other, u->dependencies[UNIT_ON_FAILURE], i)
    {
        ...
        r = manager_add_job(u->manager, JOB_START, other,
                            u->on_failure_job_mode, NULL, &error, NULL);
        if (r < 0)
            log_unit_warning_errno(
                u, r, "Failed to enqueue OnFailure= job, ignoring: %s",
                bus_error_message(&error, r));
    }
}
...
static int unit_log_resources(Unit* u)
{
    ...
    /* Invoked whenever a unit enters failed or dead state. Logs information
     * about consumed resources if resource accounting was enabled for a unit.
     * It does this in two ways: a friendly human readable string with reduced
    ...
    if (nsec != NSEC_INFINITY)
    {
        ...
        /* Format the CPU time for inclusion in the human language message
         * string */
    ...
    for (m = 0; m < _CGROUP_IP_ACCOUNTING_METRIC_MAX; m++)
    {
        ...
        /* Format the IP accounting data for inclusion in the human language
         * message string, but only for the bytes counters (and not for the
         * packets counters) */
        if (m == CGROUP_IP_INGRESS_BYTES)
        {
            assert(!igress);
            igress = strjoin("received ", format_bytes(buf, sizeof(buf), value),
                             " IP traffic");
            if (!igress)
            {
                r = log_oom();
        ...
        {
            assert(!egress);
            egress = strjoin("sent ", format_bytes(buf, sizeof(buf), value),
                             " IP traffic");
            if (!egress)
            {
                r = log_oom();
    ...
    /* The following four fields we allocate on the stack or are static strings,
     * we hence don't want to free them, and hence don't increase n_iovec for
     * them */
    iovec[n_iovec] = IOVEC_MAKE_STRING(t);
    iovec[n_iovec + 1] =
        IOVEC_MAKE_STRING("MESSAGE_ID=" SD_MESSAGE_UNIT_RESOURCES_STR);
...
static void unit_emit_audit_stop(Unit* u, UnitActiveState state)
{
    ...
    if (u->in_audit)
    {
        ...
        manager_send_unit_audit(u->manager, u, AUDIT_SERVICE_STOP,
                                state == UNIT_INACTIVE);
        u->in_audit = false;
    }
    else
    {
        /* Hmm, if there was no start record written write it now, so that we
         * always have a nice pair */
        manager_send_unit_audit(u->manager, u, AUDIT_SERVICE_START,
                                state == UNIT_INACTIVE);
...
static bool unit_process_job(Job* j, UnitActiveState ns, UnitNotifyFlags flags)
{
    ...
    if (j->state == JOB_WAITING)
        ...
        /* So we reached a different state for this job. Let's see if we can run
         * it now if it failed previously due to EAGAIN. */
        job_add_to_run_queue(j);

    /* Let's check whether the unit's new state constitutes a finished job, or
     * maybe contradicts a running job and hence needs to invalidate jobs. */
            ...
            else if (j->state == JOB_RUNNING && ns != UNIT_ACTIVATING)
            {
                ...
                if (UNIT_IS_INACTIVE_OR_FAILED(ns))
                    job_finish_and_invalidate(
                        j, ns == UNIT_FAILED ? JOB_FAILED : JOB_DONE, true,
                        false);
            }
        ...
        case JOB_TRY_RELOAD:
            ...
            if (j->state == JOB_RUNNING)
            {
                if (ns == UNIT_ACTIVE)
                    job_finish_and_invalidate(
                        j,
                        (flags & UNIT_NOTIFY_RELOAD_FAILURE) ? JOB_FAILED
                                                             : JOB_DONE,
                        true, false);
                 ...
                 UnitNotifyFlags flags)
{
    ...
    /* Note that this is called for all low-level state changes, even if they
     * might map to the same high-level UnitActiveState! That means that ns ==
     * os is an expected behavior here. For example: if a mount point is
     * remounted this function will be called too! */

    m = u->manager;

    /* Let's enqueue the change signal early. In case this unit has a job
     * associated we want that this unit is in the bus queue, so that any job
     * change signal queued will force out the unit change signal first. */
    unit_add_to_dbus_queue(u);
    ...
    /* Make sure the cgroup and state files are always removed when we become
     * inactive */
    if (UNIT_IS_INACTIVE_OR_FAILED(ns))
    {
        unit_prune_cgroup(u);
    ...
    if (!MANAGER_IS_RELOADING(m))
    {
        ...
        /* If this state change happened without being requested by a job, then
         * let's retroactively start or stop dependencies. We skip that step
         * when deserializing, since we don't want to create any
    ...
    if (!MANAGER_IS_RELOADING(m))
    {
        /* Maybe we finished startup and are now ready for being stopped because
         * unneeded? */
        unit_submit_to_stop_when_unneeded_queue(u);

        /* Maybe we finished startup, but something we needed has vanished?
         * Let's die then. (This happens when something BindsTo= to a
         * Type=oneshot unit, as these units go directly from starting to
...
int unit_watch_pid(Unit* u, pid_t pid, bool exclusive)
{
    ...
    /* Caller might be sure that this PID belongs to this unit only. Let's take
     * this opportunity to remove any stalled references to this PID as they can
     * be created easily (when watching a process which is not our direct
     * child). */
    if (exclusive)
    ...
    if (r == -EEXIST)
    {
        ...
        /* OK, the "pid" key is already assigned to a different unit. Let's see
         * if the "-pid" key (which points
        ...
        else
        {
            ...
            r = hashmap_replace(u->manager->watch_pids, PID_TO_PTR(-pid),
                                new_array);
            if (r < 0)
            {
                free(new_array);
...
void unit_unwatch_pid(Unit* u, pid_t pid)
{
    ...
    if (array)
    {
        ...
        if (m == 0)
        {
            ...
            assert(hashmap_remove(u->manager->watch_pids, PID_TO_PTR(-pid)) ==
                   array);
            free(array);
        }
    }
...
int unit_enqueue_rewatch_pids(Unit* u)
{
    ...
    /* Enqueues a low-priority job that will clean up dead PIDs from our list of
     * PIDs to watch and subscribe to new PIDs that might have appeared. We do
     * this in a delayed job because the work might be quite slow, as it
    ...
    if (!u->rewatch_pids_event_source)
    {
        ...
        if (r < 0)
            return log_error_errno(r, "Failed to adjust priority of event "
    ...
    r = sd_event_source_set_enabled(u->rewatch_pids_event_source,
                                    SD_EVENT_ONESHOT);
    if (r < 0)
        return log_error_errno(
            r, "Failed to enable event source for tidying watched PIDs: %m");
...
void unit_dequeue_rewatch_pids(Unit* u)
{
    ...
    if (r < 0)
        log_warning_errno(r, "Failed to disable event source for tidying "
                             "watched PIDs, ignoring: %m");

    u->rewatch_pids_event_source =
        sd_event_source_unref(u->rewatch_pids_event_source);
}
...
bool unit_job_is_applicable(Unit* u, JobType j)
{
    ...
    switch (j)
    {
        ...
        case JOB_START:
        case JOB_NOP:
            /* Note that we don't check unit_can_start() here. That's because
             * .device units and suchlike are not startable by us but may appear
             * due to external events, and it thus makes sense to permit
             * enqueing jobs for it. */
            return true;

        case JOB_STOP:
            /* Similar as above. However, perpetual units can never be stopped
             * (neither explicitly nor due to external events), hence it makes
             * no sense to permit enqueing such a request either. */
            return !u->perpetual;
                                        ...
                                        UnitDependency dependency)
{
    ...
    if (!IN_SET(dependency, UNIT_CONFLICTS, UNIT_CONFLICTED_BY, UNIT_BEFORE,
                UNIT_AFTER, UNIT_ON_FAILURE, UNIT_TRIGGERS, UNIT_TRIGGERED_BY))
        return;

    if (streq_ptr(u->id, other))
        log_unit_warning(u, "Dependency %s=%s dropped",
                         unit_dependency_to_string(dependency), u->id);
    else
        log_unit_warning(u, "Dependency %s=%s dropped, merged into %s",
                         unit_dependency_to_string(dependency), strna(other),
                         u->id);
}

static int unit_add_dependency_hashmap(Hashmap** h, Unit* other,
                        ...
                        bool add_reference, UnitDependencyMask mask)
{
    ...
    {
        log_unit_warning(
            u, "Dependency Before=%s ignored (.device units cannot be delayed)",
            other->id);
        return 0;
    }
    ...
    {
        r = unit_add_dependency_hashmap(other->dependencies + inverse_table[d],
                                        u, 0, mask);
        if (r < 0)
            return r;
    }
    ...
    if (add_reference)
    {
        r = unit_add_dependency_hashmap(u->dependencies + UNIT_REFERENCES,
                                        other, mask, 0);
        if (r < 0)
            return r;

        r = unit_add_dependency_hashmap(
            other->dependencies + UNIT_REFERENCED_BY, u, 0, mask);
        if (r < 0)
            return r;
    }
...
static int resolve_template(Unit* u, const char* name, char** buf,
                            const char** ret)
{
    int r;
...
int unit_add_two_dependencies_by_name(Unit* u, UnitDependency d,
                                      UnitDependency e, const char* name,
                                      bool add_reference,
                                      UnitDependencyMask mask)
{
    _cleanup_free_ char* buf = NULL;
...
int unit_set_default_slice(Unit* u)
{
    ...
    if (u->instance)
    {
        ...
        slice_name = MANAGER_IS_SYSTEM(u->manager) &&
...
static int signal_name_owner_changed(sd_bus_message* message, void* userdata,
                                     sd_bus_error* error)
{
...
int unit_install_bus_match(Unit* u, sd_bus* bus, const char* name)
{
    ...
    return sd_bus_add_match_async(bus, &u->match_bus_slot, match,
                                  signal_name_owner_changed, NULL, u);
}
...
int unit_watch_bus_name(Unit* u, const char* name)
{
    ...
    if (u->manager->api_bus)
    {
         ...
         * Otherwise, just put the name in the list. bus_setup_api() will take
         * care later. */
        r = unit_install_bus_match(u, u->manager->api_bus, name);
        if (r < 0)
            return log_warning_errno(
                r,
                "Failed to subscribe to NameOwnerChanged signal for '%s': %m",
                name);
    }
...
static const char* const
    ip_accounting_metric_field[_CGROUP_IP_ACCOUNTING_METRIC_MAX] = {
...
int unit_serialize(Unit* u, FILE* f, FDSet* fds, bool serialize_jobs)
{
    ...
    (void)serialize_dual_timestamp(f, "state-change-timestamp",
                                   &u->state_change_timestamp);

    (void)serialize_dual_timestamp(f, "inactive-exit-timestamp",
                                   &u->inactive_exit_timestamp);
    (void)serialize_dual_timestamp(f, "active-enter-timestamp",
                                   &u->active_enter_timestamp);
    (void)serialize_dual_timestamp(f, "active-exit-timestamp",
                                   &u->active_exit_timestamp);
    (void)serialize_dual_timestamp(f, "inactive-enter-timestamp",
                                   &u->inactive_enter_timestamp);

    (void)serialize_dual_timestamp(f, "condition-timestamp",
    ...
    (void)serialize_bool(f, "exported-invocation-id",
                         u->exported_invocation_id);
    (void)serialize_bool(f, "exported-log-level-max",
                         u->exported_log_level_max);
    (void)serialize_bool(f, "exported-log-extra-fields",
                         u->exported_log_extra_fields);
    (void)serialize_bool(f, "exported-log-rate-limit-interval",
                         u->exported_log_rate_limit_interval);
    (void)serialize_bool(f, "exported-log-rate-limit-burst",
                         u->exported_log_rate_limit_burst);

    (void)serialize_item_format(f, "cpu-usage-base", "%" PRIu64,
                                u->cpu_usage_base);
    if (u->cpu_usage_last != NSEC_INFINITY)
        (void)serialize_item_format(f, "cpu-usage-last", "%" PRIu64,
                                    u->cpu_usage_last);
    ...
    (void)serialize_cgroup_mask(f, "cgroup-realized-mask",
                                u->cgroup_realized_mask);
    (void)serialize_cgroup_mask(f, "cgroup-enabled-mask",
                                u->cgroup_enabled_mask);
    (void)serialize_cgroup_mask(f, "cgroup-invalidated-mask",
                                u->cgroup_invalidated_mask);
    ...
    if (!sd_id128_is_null(u->invocation_id))
        (void)serialize_item_format(f, "invocation-id", SD_ID128_FORMAT_STR,
                                    SD_ID128_FORMAT_VAL(u->invocation_id));
    ...
    for (m = 0; m < _CGROUP_IP_ACCOUNTING_METRIC_MAX; m++)
    {
        ...
        if (r >= 0)
            (void)serialize_item_format(f, ip_accounting_metric_field[m],
                                        "%" PRIu64, v);
    }
...
int unit_deserialize(Unit* u, FILE* f, FDSet* fds)
{
    ...
    for (;;)
    {
        ...
        if (streq(l, "job"))
        {
            if (v[0] == '\0')
            {
                ...
                log_unit_warning(u,
                                 "Update from too old systemd versions are "
                                 "unsupported, cannot deserialize job: %s",
                                 v);
            continue;
        }
        ...
        else if (streq(l, "condition-result"))
        {
            ...
            if (r < 0)
                log_unit_debug(
                    u, "Failed to parse condition result value %s, ignoring.",
                    v);
            else
        ...
        else if (streq(l, "assert-result"))
        {
            ...
            if (r < 0)
                log_unit_debug(
                    u, "Failed to parse assert result value %s, ignoring.", v);
            else
                u->assert_result = r;
        ...
        else if (streq(l, "transient"))
        {
            ...
            if (r < 0)
                log_unit_debug(
                    u, "Failed to parse transient bool %s, ignoring.", v);
            else
                u->transient = r;
        ...
        else if (streq(l, "in-audit"))
        {
            ...
            r = parse_boolean(v);
            if (r < 0)
                log_unit_debug(u, "Failed to parse in-audit bool %s, ignoring.",
                               v);
            else
                u->in_audit = r;
        ...
        else if (streq(l, "exported-log-extra-fields"))
        {
            ...
            if (r < 0)
                log_unit_debug(u,
                               "Failed to parse exported log extra fields bool "
                               "%s, ignoring.",
                               v);
            else
        ...
        else if (streq(l, "exported-log-rate-limit-interval"))
        {
            ...
            if (r < 0)
                log_unit_debug(u,
                               "Failed to parse exported log rate limit "
                               "interval %s, ignoring.",
                               v);
            else
        ...
        else if (streq(l, "exported-log-rate-limit-burst"))
        {
            ...
            if (r < 0)
                log_unit_debug(u,
                               "Failed to parse exported log rate limit burst "
                               "%s, ignoring.",
                               v);
            else
        ...
        else if (streq(l, "cpu-usage-last"))
        {
            ...
            if (r < 0)
                log_unit_debug(u, "Failed to read CPU usage last %s, ignoring.",
                               v);
        ...
        else if (streq(l, "cgroup"))
        {
            ...
            if (r < 0)
                log_unit_debug_errno(
                    u, r, "Failed to set cgroup path %s, ignoring: %m", v);
        ...
        else if (streq(l, "cgroup-realized"))
        {
            ...
            if (b < 0)
                log_unit_debug(
                    u, "Failed to parse cgroup-realized bool %s, ignoring.", v);
            else
                u->cgroup_realized = b;
        ...
        else if (streq(l, "cgroup-realized-mask"))
        {
            ...
            if (r < 0)
                log_unit_debug(
                    u, "Failed to parse cgroup-realized-mask %s, ignoring.", v);
            continue;
        }
        else if (streq(l, "cgroup-enabled-mask"))
        {
            ...
            if (r < 0)
                log_unit_debug(
                    u, "Failed to parse cgroup-enabled-mask %s, ignoring.", v);
            continue;
        }
        else if (streq(l, "cgroup-invalidated-mask"))
        {
            ...
            if (r < 0)
                log_unit_debug(
                    u, "Failed to parse cgroup-invalidated-mask %s, ignoring.",
                    v);
            continue;
        }
        else if (streq(l, "ref-uid"))
        {
            ...
            if (r < 0)
                log_unit_debug(
                    u, "Failed to parse referenced UID %s, ignoring.", v);
            else
        ...
        else if (streq(l, "ref-gid"))
        {
            ...
            if (r < 0)
                log_unit_debug(
                    u, "Failed to parse referenced GID %s, ignoring.", v);
            else
        ...
        else if (streq(l, "invocation-id"))
        {
            ...
            if (r < 0)
                log_unit_debug(u, "Failed to parse invocation id %s, ignoring.",
                               v);
            else
            {
                ...
                if (r < 0)
                    log_unit_warning_errno(
                        u, r, "Failed to set invocation ID for unit: %m");
            }
        ...
        if (m < _CGROUP_IP_ACCOUNTING_METRIC_MAX)
        {
            ...
            if (r < 0)
                log_unit_debug(
                    u, "Failed to parse IP accounting value %s, ignoring.", v);
            else
        ...
        if (unit_can_serialize(u))
        {
            ...
            if (r < 0)
            {
                log_unit_warning(
                    u,
                    "Failed to deserialize runtime parameter '%s', ignoring.",
                    l);
                continue;
            }
            ...
            if (r < 0)
                log_unit_warning(
                    u, "Failed to deserialize unit parameter '%s', ignoring.",
                    l);
        }
    }

    /* Versions before 228 did not carry a state change timestamp. In this case,
     * take the current time. This is useful, so that timeouts based on this
     * timestamp don't trigger too early, and is in-line with the logic from
    ...
    /* Let's make sure that everything that is deserialized also gets any
     * potential new cgroup settings applied after we are done. For that we
     * invalidate anything already realized, so that we can realize it again. */
...
int unit_add_node_dependency(Unit* u, const char* what, bool wants,
                             UnitDependency dep, UnitDependencyMask mask)
{
    Unit* device;
    ...
    r = unit_add_two_dependencies(
        u, UNIT_AFTER, MANAGER_IS_SYSTEM(u->manager) ? dep : UNIT_WANTS, device,
        true, mask);
    if (r < 0)
        return r;
                                 ...
                                 bool path_masked)
{
    ...
    /* If the source is some virtual kernel file system, then we assume we watch
     * it anyway, and hence pretend we are never out-of-date. */
                     ...
                     pid_t control_pid, sd_bus_error* error)
{
    ...
    if (IN_SET(who, KILL_MAIN, KILL_MAIN_FAIL))
    {
        if (main_pid < 0)
            ...
            return sd_bus_error_set_const(error, BUS_ERROR_NO_SUCH_PROCESS,
    ...
    if (IN_SET(who, KILL_ALL, KILL_ALL_FAIL) && u->cgroup_path)
    {
        ...
        q = cg_kill_recursive(SYSTEMD_CGROUP_CONTROLLER, u->cgroup_path, signo,
                              0, pid_set, NULL, NULL);
...
UnitFileState unit_get_unit_file_state(Unit* u)
{
    ...
    {
        r = unit_file_get_state(u->manager->unit_file_scope, NULL, u->id,
                                &u->unit_file_state);
        if (r < 0)
...
int unit_get_unit_file_preset(Unit* u)
{
    ...
    if (u->unit_file_preset < 0 && u->fragment_path)
        u->unit_file_preset = unit_file_query_preset(
            u->manager->unit_file_scope, NULL, basename(u->fragment_path));
...
void unit_ref_unset(UnitRef* ref)
{
    ...
    /* We are about to drop a reference to the unit, make sure the garbage
     * collection has a look at it as it might be unreferenced now. */
...
static int user_from_unit_name(Unit* u, char** ret)
{
    ...
    static const uint8_t hash_key[] = {0x58, 0x1a, 0xaf, 0xe6, 0x28, 0x58,
                                       0x4e, 0x96, 0xb4, 0x4e, 0xf5, 0x3b,
                                       0x8c, 0x92, 0x07, 0xec};
    ...
    /* If we can't use the unit name as a user name, then let's hash it and use
     * that */
    ...
    if (ec)
    {
        ...
        if (ec->private_devices)
            ec->capability_bounding_set &=
                ~((UINT64_C(1) << CAP_MKNOD) | (UINT64_C(1) << CAP_SYS_RAWIO));
        ...
        if (ec->dynamic_user)
        {
            ...
            /* If the dynamic user option is on, let's make sure that the unit
             * can't leave its UID/GID around in the file system or on IPC
             * objects. Hence enforce a strict sandbox. */
            ...
            /* Make sure this service can neither benefit from SUID/SGID
             * binaries nor create them. */
...
static const char* unit_drop_in_dir(Unit* u, UnitWriteFlags flags)
{
    ...
    if (u->transient) /* Redirect drop-ins for transient units always into the
                         transient directory. */
...
char* unit_escape_setting(const char* s, UnitWriteFlags flags, char** buf)
{
    ...
    /* Escapes the input string as requested. Returns the escaped string. If
     * 'buf' is specified then the allocated return buffer pointer is also
     * written to *buf, except if no escaping was necessary, in which case *buf
     * is set to NULL, and the input pointer is returned as-is. This means the
     * return value always contains a properly escaped version, but *buf when
     * passed only contains a pointer if an allocation was necessary. If *buf is
     * not specified, then the return value always needs to be freed. Callers
     * can use this to optimize memory allocations. */
...
char* unit_concat_strv(char** l, UnitWriteFlags flags)
{
    ...
    /* Takes a list of strings, escapes them, and concatenates them. This may be
     * used to format command lines in a way suitable for ExecStart= stanzas */
...
int unit_write_setting(Unit* u, UnitWriteFlags flags, const char* name,
                       const char* data)
{
    ...
    /* Prefix the section header. If we are writing this out as transient file,
     * then let's suppress this if the previous section header is the same */
    ...
    if (u->transient_file)
    {
        /* When this is a transient unit file in creation, then let's not create
         * a new drop-in but instead write to the transient unit file. */
    ...
    wrapped = strjoina("# This is a drop-in unit file extension, created via "
...
int unit_write_settingf(Unit* u, UnitWriteFlags flags, const char* name,
                        const char* format, ...)
{
    _cleanup_free_ char* p = NULL;
    va_list ap;
...
int unit_make_transient(Unit* u)
{
    ...
    /* Let's open the file we'll write the transient settings into. This file is
     * kept open as long as we are creating the transient, and is closed in
     * unit_load(), as soon as we start loading the file. */
    ...
    fputs("# This is a transient unit file, created programmatically via the "
...
static int log_kill(pid_t pid, int sig, void* userdata)
{
    ...
    /* Don't log about processes marked with brackets, under the assumption that
       these are temporary processes only, like for example systemd's own PAM
       stub process. */
    if (comm && comm[0] == '(')
        return 0;

    log_unit_notice(userdata,
                    "Killing process " PID_FMT " (%s) with signal SIG%s.", pid,
                    strna(comm), signal_to_string(sig));
                      ...
                      pid_t control_pid, bool main_pid_alien)
{
    ...
    /* Kill the processes belonging to this unit, in preparation for shutting
     * the unit down. Returns > 0 if we killed something worth waiting for, 0
     * otherwise. */
    ...
    if (u->cgroup_path && (c->kill_mode == KILL_CONTROL_GROUP ||
                           (c->kill_mode == KILL_MIXED && k == KILL_KILL)))
    {
        ...
        r = cg_kill_recursive(SYSTEMD_CGROUP_CONTROLLER, u->cgroup_path, sig,
                              CGROUP_SIGCONT | CGROUP_IGNORE_SELF, pid_set,
                              log_func, u);
        if (r < 0)
        {
            if (!IN_SET(r, -EAGAIN, -ESRCH, -ENOENT))
                log_unit_warning_errno(
                    u, r, "Failed to kill control group %s, ignoring: %m",
                    u->cgroup_path);
        }
        else if (r > 0)
        {

            /* FIXME: For now, on the legacy hierarchy, we will not wait for the
             * cgroup members to die if we are running in a container or if this
             * is a delegation unit, simply because cgroup notification is
             * unreliable in these cases. It doesn't work at all in containers,
             * and outside of containers it can be confused easily by left-over
             * directories in the cgroup — which however should not exist in
             * non-delegated units. On the unified hierarchy that's different,
            ...
            if (send_sighup)
            {
                ...
                cg_kill_recursive(SYSTEMD_CGROUP_CONTROLLER, u->cgroup_path,
                                  SIGHUP, CGROUP_IGNORE_SELF, pid_set, NULL,
                                  NULL);
            }
        }
    }
...
int unit_require_mounts_for(Unit* u, const char* path, UnitDependencyMask mask)
{
    ...
    /* Registers a unit for requiring a certain path and all its prefixes. We
     * keep a hashtable of these paths in the unit (from the path to the
     * UnitDependencyInfo structure indicating how to the dependency came to
     * be). However, we build a prefix table for all possible prefixes so that
     * new appearing mount units can easily determine which units to make
     * themselves a dependency of. */
    ...
    di = (UnitDependencyInfo){.origin_mask = mask};
    ...
    PATH_FOREACH_PREFIX_MORE(prefix, path)
    {
        ...
        if (!x)
        {
            ...
            r = hashmap_ensure_allocated(
                &u->manager->units_requiring_mounts_for, &path_hash_ops);
            if (r < 0)
                return r;
...
int unit_setup_exec_runtime(Unit* u)
{
    ...
    return exec_runtime_acquire(u->manager, unit_get_exec_context(u), u->id,
                                true, rt);
}
...
void unit_warn_if_dir_nonempty(Unit* u, const char* where)
{
    ...
    log_struct(LOG_NOTICE, "MESSAGE_ID=" SD_MESSAGE_OVERMOUNTING_STR,
               LOG_UNIT_ID(u), LOG_UNIT_INVOCATION_ID(u),
               LOG_UNIT_MESSAGE(
                   u,
                   "Directory %s to mount over is not empty, mounting anyway.",
                   where),
               "WHERE=%s", where);
}
...
int unit_fail_if_noncanonical(Unit* u, const char* where)
{
    ...
    {
        log_unit_debug_errno(
            u, r, "Failed to check %s for symlinks, ignoring: %m", where);
        return 0;
    }
    ...
    /* No need to mention "." or "..", they would already have been rejected by
     * unit_name_from_path() */
    log_struct(
        LOG_ERR, "MESSAGE_ID=" SD_MESSAGE_OVERMOUNTING_STR, LOG_UNIT_ID(u),
        LOG_UNIT_INVOCATION_ID(u),
        LOG_UNIT_MESSAGE(
            u, "Mount path %s is not canonical (contains a symlink).", where),
        "WHERE=%s", where);
...
bool unit_is_pristine(Unit* u)
{
    ...
    return !(!IN_SET(u->load_state, UNIT_NOT_FOUND, UNIT_LOADED) ||
             u->fragment_path || u->source_path ||
             !strv_isempty(u->dropin_paths) || u->job || u->merged_into);
}
...
static void unit_unref_uid_internal(
    Unit* u, uid_t* ref_uid, bool destroy_now,
    void (*_manager_unref_uid)(Manager* m, uid_t uid, bool destroy_now))
{
    ...
    /* Generic implementation of both unit_unref_uid() and unit_unref_gid(),
     * under the assumption that uid_t and gid_t are actually the same time,
     * with the same validity rules.
     *
...
{
    unit_unref_uid_internal(u, (uid_t*)&u->ref_gid, destroy_now,
                            manager_unref_gid);
}

static int unit_ref_uid_internal(Unit* u, uid_t* ref_uid, uid_t uid,
                                 bool clean_ipc,
                                 int (*_manager_ref_uid)(Manager* m, uid_t uid,
                                                         bool clean_ipc))
{
    ...
    /* Generic implementation of both unit_ref_uid() and unit_ref_guid(), under
     * the assumption that uid_t and gid_t are actually the same type, and have
     * the same validity rules.
     *
     * Adds a reference on a specific UID/GID to this unit. Each unit
     * referencing the same UID/GID maintains a reference so that we can destroy
     * the UID/GID's IPC resources as soon as this is requested and the counter
     * drops to zero. */
...
{
    return unit_ref_uid_internal(u, &u->ref_uid, uid, clean_ipc,
                                 manager_ref_uid);
}

int unit_ref_gid(Unit* u, gid_t gid, bool clean_ipc)
{
    return unit_ref_uid_internal(u, (uid_t*)&u->ref_gid, (uid_t)gid, clean_ipc,
                                 manager_ref_gid);
}

static int unit_ref_uid_gid_internal(Unit* u, uid_t uid, gid_t gid,
                                     bool clean_ipc)
{
    int r = 0, q = 0;
    ...
    /* Reference both a UID and a GID in one go. Either references both, or
     * neither. */
...
void unit_notify_user_lookup(Unit* u, uid_t uid, gid_t gid)
{
    ...
    /* This is invoked whenever one of the forked off processes let's us know
     * the UID/GID its user name/group names resolved to. We keep track of which
     * UID/GID is currently assigned in order to be able to destroy its IPC
...
int unit_set_invocation_id(Unit* u, sd_id128_t id)
{
    ...
    /* Set the invocation ID for this unit. If we cannot, this will not roll
     * back, but reset the whole thing. */

    if (sd_id128_equal(u->invocation_id, id))
        return 0;

    if (!sd_id128_is_null(u->invocation_id))
        (void)hashmap_remove_value(u->manager->units_by_invocation_id,
                                   &u->invocation_id, u);
    ...
    r = hashmap_ensure_allocated(&u->manager->units_by_invocation_id,
                                 &id128_hash_ops);
    if (r < 0)
        goto reset;
...
int unit_acquire_invocation_id(Unit* u)
{
    ...
    if (r < 0)
        return log_unit_error_errno(
            u, r, "Failed to generate invocation ID for unit: %m");
...
int unit_set_exec_params(Unit* u, ExecParameters* p)
{
    ...
    SET_FLAG(p->flags, EXEC_PASS_LOG_UNIT | EXEC_CHOWN_DIRECTORIES,
             MANAGER_IS_SYSTEM(u->manager));
...
int unit_fork_helper_process(Unit* u, const char* name, pid_t* ret)
{
    ...
    /* Forks off a helper process and makes sure it is a member of the unit's
     * cgroup. Returns == 0 in the child, and > 0 in the parent. The pid
     * parameter is always filled in with the child's PID. */
    ...
    if (u->cgroup_path)
    {
        r = cg_attach_everywhere(u->manager->cgroup_supported, u->cgroup_path,
                                 0, NULL, NULL);
        if (r < 0)
        {
            log_unit_error_errno(u, r, "Failed to join unit cgroup %s: %m",
                                 u->cgroup_path);
            _exit(EXIT_CGROUP);
        }
    }
...
static void unit_update_dependency_mask(Unit* u, UnitDependency d, Unit* other,
                                        UnitDependencyInfo di)
{
    assert(u);
    assert(d >= 0);
    ...
    if (di.origin_mask == 0 && di.destination_mask == 0)
    {
        ...
        log_unit_debug(u, "%s lost dependency %s=%s", u->id,
...
void unit_remove_dependencies(Unit* u, UnitDependencyMask mask)
{
    ...
    /* Removes all dependencies u has on other units marked for ownership by
     * 'mask'. */
    ...
    for (d = 0; d < _UNIT_DEPENDENCY_MAX; d++)
    {
        ...
        do
        {
            ...
            HASHMAP_FOREACH_KEY(di.data, other, u->dependencies[d], i)
            {
                ...
                /* We updated the dependency from our unit to the other unit
                 * now. But most dependencies imply a reverse dependency. Hence,
                 * let's delete that one too. For that we go through all
                 * dependency types on the other unit and delete all those which
                 * point to us and have the right mask set. */
...
static int unit_export_invocation_id(Unit* u)
{
    ...
    if (r < 0)
        return log_unit_debug_errno(
            u, r, "Failed to create invocation ID symlink %s: %m", p);
...
static int unit_export_log_level_max(Unit* u, const ExecContext* c)
{
    ...
    if (r < 0)
        return log_unit_debug_errno(
            u, r, "Failed to create maximum log level symlink %s: %m", p);
...
static int unit_export_log_extra_fields(Unit* u, const ExecContext* c)
{
    ...
    if (fd < 0)
        return log_unit_debug_errno(
            u, fd, "Failed to create extra fields file %s: %m", p);
    ...
    {
        r = log_unit_debug_errno(u, errno,
...
static int unit_export_log_rate_limit_interval(Unit* u, const ExecContext* c)
{
    ...
    if (r < 0)
        return log_unit_debug_errno(
            u, r, "Failed to create log rate limit interval symlink %s: %m", p);
...
static int unit_export_log_rate_limit_burst(Unit* u, const ExecContext* c)
{
    ...
    if (r < 0)
        return log_unit_debug_errno(
            u, r, "Failed to create log rate limit burst symlink %s: %m", p);
...
void unit_export_state_files(Unit* u)
{
    ...
    /* Exports a couple of unit properties to /run/systemd/units/, so that
     * journald can quickly query this data from there. Ideally, journald would
     * use IPC to query this, like everybody else, but that's hard, as long as
     * the IPC system itself and PID 1 also log to the journal.
     *
     * Note that these files really shouldn't be considered API for anyone else,
     * as use a runtime file system as IPC replacement is not compatible with
     * today's world of file system namespaces. However, this doesn't really
     * apply to communication between the journal and systemd, as we assume that
     * these two daemons live in the same namespace at least.
     *
     * Note that some of the "files" exported here are actually symlinks and not
     * regular files. Symlinks work better for storing small bits of data, in
     * particular as we can write them with two system calls, and read
     * them with one. */
...
static int log_leftover(pid_t pid, int sig, void* userdata)
{
    ...
    if (comm &&
        comm[0] == '(') /* Most likely our own helper process (PAM?), ignore */
        return 0;

    log_unit_warning(userdata,
                     "Found left-over process " PID_FMT
                     " (%s) in control group while starting unit. Ignoring.\n"
                     "This usually indicates unclean termination of a previous "
...
int unit_warn_leftover_processes(Unit* u)
{
    ...
    return cg_kill_recursive(SYSTEMD_CGROUP_CONTROLLER, u->cgroup_path, 0, 0,
                             NULL, log_leftover, u);
}
...
bool unit_needs_console(Unit* u)
{
    ...
    /* If this unit type doesn't implement this call, let's use a generic
...
const char* unit_label_path(Unit* u)
{
    ...
    /* Returns the file system path to use for MAC access decisions, i.e. the
     * file to read the SELinux label off when validating access checks. */

    p = u->source_path ?: u->fragment_path;
    if (!p)
        return NULL;

    /* If a unit is masked, then don't read the SELinux label of /dev/null, as
...
int unit_pid_attachable(Unit* u, pid_t pid, sd_bus_error* error)
{
    ...
    /* Checks whether the specified PID is generally good for attaching, i.e. a
     * valid PID, not our manager itself, and not a kernel thread either */

    /* First, a simple range check */
    if (!pid_is_valid(pid))
        return sd_bus_error_setf(error, SD_BUS_ERROR_INVALID_ARGS,
                                 "Process identifier " PID_FMT " is not valid.",
                                 pid);
    ...
    if (pid == 1 || pid == getpid_cached())
        return sd_bus_error_setf(
            ...
            "Process " PID_FMT " is a manager process, refusing.", pid);

    /* Don't even begin to bother with kernel threads */
    r = is_kernel_thread(pid);
    if (r == -ESRCH)
        return sd_bus_error_setf(error, SD_BUS_ERROR_UNIX_PROCESS_ID_UNKNOWN,
                                 "Process with ID " PID_FMT " does not exist.",
                                 pid);
    if (r < 0)
        return sd_bus_error_set_errnof(
            error, r,
            "Failed to determine whether process " PID_FMT
            " is a kernel thread: %m",
            pid);
    if (r > 0)
        return sd_bus_error_setf(
            error, SD_BUS_ERROR_INVALID_ARGS,
            "Process " PID_FMT " is a kernel thread, refusing.", pid);
...
void unit_log_success(Unit* u)
{
    ...
    log_struct(LOG_INFO, "MESSAGE_ID=" SD_MESSAGE_UNIT_SUCCESS_STR,
               LOG_UNIT_ID(u), LOG_UNIT_INVOCATION_ID(u),
...
void unit_log_failure(Unit* u, const char* result)
{
    ...
    log_struct(LOG_WARNING, "MESSAGE_ID=" SD_MESSAGE_UNIT_FAILURE_RESULT_STR,
               LOG_UNIT_ID(u), LOG_UNIT_INVOCATION_ID(u),
               LOG_UNIT_MESSAGE(u, "Failed with result '%s'.", result),
               "UNIT_RESULT=%s", result);
}

void unit_log_process_exit(Unit* u, int level, const char* kind,
                           const char* command, int code, int status)
{
    ...
    log_struct(level, "MESSAGE_ID=" SD_MESSAGE_UNIT_PROCESS_EXIT_STR,
               LOG_UNIT_MESSAGE(
                   u, "%s exited, code=%s, status=%i/%s", kind,
                   sigchld_code_to_string(code), status,
                   strna(code == CLD_EXITED
                             ? exit_status_to_string(status, EXIT_STATUS_FULL)
                             : signal_to_string(status))),
               "EXIT_CODE=%s", sigchld_code_to_string(code), "EXIT_STATUS=%i",
               status, "COMMAND=%s", strna(command), LOG_UNIT_ID(u),
               LOG_UNIT_INVOCATION_ID(u));
}
...
int unit_exit_status(Unit* u)
{
    ...
    /* Returns the exit status to propagate for the most recent cycle of this
     * unit. Returns a value in the range 0…255 if there's something to
     * propagate. EOPNOTSUPP if the concept does not apply to this unit type,
     * ENODATA if no data is currently known (for example because the unit
     * hasn't deactivated yet) and EBADE if the main
...
int unit_failure_action_exit_status(Unit* u)
{
    ...
    /* Returns the exit status to propagate on failure, or an error if there's
...
int unit_success_action_exit_status(Unit* u)
{
    ...
    /* Returns the exit status to propagate on success, or an error if there's
     * nothing to propagate */
