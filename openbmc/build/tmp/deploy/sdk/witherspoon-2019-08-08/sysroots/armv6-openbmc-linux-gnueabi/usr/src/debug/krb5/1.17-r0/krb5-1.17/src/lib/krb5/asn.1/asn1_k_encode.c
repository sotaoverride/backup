...
static int int32_not_minus1(const void* p)
{
    return *(int32_t*)p != -1;
}

static void init_int32_minus1(void* p)
{
    *(int32_t*)p = -1;
}
...
static int nonempty_data(const void* p)
{
...
DEFCOUNTEDSTRINGTYPE(utf8string, char*, unsigned int, k5_asn1_encode_bytestring,
                     k5_asn1_decode_bytestring, ASN1_UTF8STRING);
...
DEFCNFIELD(princname_1, krb5_principal_data, data, length, 1,
           cseqof_gstring_data);
static const struct atype_info* princname_fields[] = {&k5_atype_princname_0,
 ...
 */
static krb5_error_code encode_seqno(asn1buf* buf, const void* p,
                                    taginfo* rettag)
{
...
static krb5_error_code decode_seqno(const taginfo* t, const uint8_t* asn1,
                                    size_t len, void* p)
{
    krb5_error_code ret;
    intmax_t val;
...
static int check_seqno(const taginfo* t)
{
 ...
 * a krb5_timestamp. */
static krb5_error_code encode_kerberos_time(asn1buf* buf, const void* p,
                                            taginfo* rettag)
{
...
static krb5_error_code decode_kerberos_time(const taginfo* t,
                                            const uint8_t* asn1, size_t len,
                                            void* p)
{
    krb5_error_code ret;
    time_t val;
...
static int check_kerberos_time(const taginfo* t)
{
    return (t->asn1class == UNIVERSAL && t->construction == PRIMITIVE &&
...
const static struct atype_info* address_fields[] = {&k5_atype_address_0,
                                                    &k5_atype_address_1};
...
static int nonempty_enc_data(const void* p)
{
 ...
 * 32-bit integer. */
static krb5_error_code encode_krb5_flags(asn1buf* buf, const void* p,
                                         taginfo* rettag)
{
...
static krb5_error_code decode_krb5_flags(const taginfo* t, const uint8_t* asn1,
                                         size_t len, void* val)
{
    krb5_error_code ret;
    size_t i, blen;
...
static int check_krb5_flags(const taginfo* t)
{
    return (t->asn1class == UNIVERSAL && t->construction == PRIMITIVE &&
...
static const struct atype_info* authdata_elt_fields[] = {&k5_atype_authdata_0,
                                                         &k5_atype_authdata_1};
...
static const struct atype_info* checksum_fields[] = {&k5_atype_checksum_0,
                                                     &k5_atype_checksum_1};
...
static krb5_error_code encode_lr_type(asn1buf* buf, const void* p,
                                      taginfo* rettag)
{
...
static krb5_error_code decode_lr_type(const taginfo* t, const uint8_t* asn1,
                                      size_t len, void* p)
{
    krb5_error_code ret;
    intmax_t val;
...
static int check_lr_type(const taginfo* t)
{
    return (t->asn1class == UNIVERSAL && t->construction == PRIMITIVE &&
            t->tagnum == ASN1_INTEGER);
}
DEFFNTYPE(last_req_type, int32_t, encode_lr_type, decode_lr_type, check_lr_type,
          NULL);
...
static const struct atype_info* lr_fields[] = {&k5_atype_last_req_0,
                                               &k5_atype_last_req_1};
...
static const struct atype_info* pa_data_fields[] = {&k5_atype_pa_data_1,
                                                    &k5_atype_pa_data_2};
...
static int is_enc_kdc_rep_start_set(const void* p)
{
    const krb5_enc_kdc_rep_part* val = p;
    return (val->times.starttime != 0);
}
static void init_enc_kdc_rep_start(void* p)
{
    krb5_enc_kdc_rep_part* val = p;
    val->times.starttime = val->times.authtime;
}
static int is_renewable_flag_set(const void* p)
{
    const krb5_enc_kdc_rep_part* val = p;
...
DEFFIELD(enc_kdc_rep_6_def, krb5_enc_kdc_rep_part, times.starttime, 6,
         kerberos_time);
DEFOPTIONALTYPE(enc_kdc_rep_6, is_enc_kdc_rep_start_set, init_enc_kdc_rep_start,
                enc_kdc_rep_6_def);
DEFFIELD(enc_kdc_rep_7, krb5_enc_kdc_rep_part, times.endtime, 7, kerberos_time);
...
static const struct atype_info* enc_kdc_rep_part_fields[] = {
    ...
    &k5_atype_enc_kdc_rep_9, &k5_atype_enc_kdc_rep_10, &k5_atype_enc_kdc_rep_11,
...
static krb5_error_code encode_kdc_req_body(asn1buf* buf, const void* p,
                                           taginfo* tag_out)
{
...
static void free_kdc_req_body(void* val)
{
...
static krb5_error_code decode_kdc_req_body(const taginfo* t,
                                           const uint8_t* asn1, size_t len,
                                           void* val)
{
    krb5_error_code ret;
...
static int check_kdc_req_body(const taginfo* t)
{
...
static int is_safe_timestamp_set(const void* p)
{
...
static int is_salt_present(const void* p)
{
    const krb5_etype_info_entry* val = p;
    return (val->length != KRB5_ETYPE_NO_SALT);
}
static void init_no_salt(void* p)
{
...
static const struct atype_info* sam_challenge_2_fields[] = {&k5_atype_sch_0,
                                                            &k5_atype_sch_1};
...
DEFFIELD(schb_2, krb5_sam_challenge_2_body, sam_type_name, 2, opt_ostring_data);
...
DEFFIELD(schb_4, krb5_sam_challenge_2_body, sam_challenge_label, 4,
         opt_ostring_data);
DEFFIELD(schb_5, krb5_sam_challenge_2_body, sam_challenge, 5, opt_ostring_data);
...
DEFFIELD(enc_tkt_9, krb5_enc_tkt_part, caddrs, 9, opt_ptr_seqof_host_addresses);
...
static int is_priv_timestamp_set(const void* p)
{
...
static int is_cred_timestamp_set(const void* p)
{
...
static const struct atype_info* error_fields[] = {
    &k5_atype_error_0, &k5_atype_error_1,  &k5_atype_error_2,
    &k5_atype_error_3, &k5_atype_error_4,  &k5_atype_error_5,
    &k5_atype_error_6, &k5_atype_error_7,  &k5_atype_error_8,
    &k5_atype_error_9, &k5_atype_error_10, &k5_atype_error_11,
    &k5_atype_error_12};
...
static const struct atype_info* pa_enc_ts_fields[] = {&k5_atype_pa_enc_ts_0,
...
static int is_s4u_principal_present(const void* p)
{
...
static const struct atype_info* fast_armor_fields[] = {&k5_atype_fast_armor_0,
...
static const struct atype_info* fast_rep_fields[] = {&k5_atype_fast_rep_0};
DEFSEQTYPE(fast_rep, krb5_enc_data, fast_rep_fields);

/* This is a CHOICE type with only one choice (so far) and we're not using a
 * distinguisher/union for it. */
DEFTAGGEDTYPE(pa_fx_fast_reply, CONTEXT_SPECIFIC, CONSTRUCTED, 0, 0, fast_rep);
...
krb5_error_code decode_krb5_enc_kdc_rep_part(const krb5_data* code,
 ...
 */
krb5_error_code decode_krb5_safe_with_body(const krb5_data* code,
                                           krb5_safe** rep_out,
                                           krb5_data** body_out)
{
    krb5_error_code ret;
...
krb5_error_code KRB5_CALLCONV krb5int_get_authdata_containee_types(
    krb5_context context, const krb5_authdata* authdata, unsigned int* num_out,
...
static const struct atype_info* kdf_alg_id_fields[] = {&k5_atype_kdf_alg_id_0};
...
DEFCOUNTEDSTRINGTYPE(s_bitstring, char*, unsigned int, k5_asn1_encode_bitstring,
                     k5_asn1_decode_bitstring, ASN1_BITSTRING);
...
static const struct atype_info* subject_pk_info_fields[] = {&k5_atype_spki_0,
                                                            &k5_atype_spki_1};
...
DEFFIELD(auth_pack_4, krb5_auth_pack, supportedKDFs, 4, opt_ptr_supported_kdfs);
...
DEFFIELD_IMPLICIT(extprinc_0, krb5_external_principal_identifier, subjectName,
                  0, opt_ostring_data);
...
DEFFIELD(dh_key_2, krb5_kdc_dh_key_info, dhKeyExpiration, 2, opt_kerberos_time);
...
static const struct atype_info* typed_data_fields[] = {&k5_atype_typed_data_0,
...
DEFFIELD_IMPLICIT(tokinfo_5, krb5_otp_tokeninfo, token_id, 5, opt_ostring_data);
...
DEFFIELD(kkdcp_message_0, krb5_kkdcp_message, kerb_message, 0, ostring_data);
DEFFIELD(kkdcp_message_1, krb5_kkdcp_message, target_domain, 1,
         opt_gstring_data);
DEFFIELD(kkdcp_message_2, krb5_kkdcp_message, dclocator_hint, 2, opt_int32);
...
DEFSEQTYPE(kkdcp_message, krb5_kkdcp_message, kkdcp_message_fields);
