...
{
    assert(sizeof(lzma_header_magic) + LZMA_STREAM_FLAGS_SIZE + 4 ==
    ...
    const uint32_t crc =
        lzma_crc32(out + sizeof(lzma_header_magic), LZMA_STREAM_FLAGS_SIZE, 0);

    unaligned_write32le(
        out + sizeof(lzma_header_magic) + LZMA_STREAM_FLAGS_SIZE, crc);
    ...
    lzma_stream_footer_encode(const lzma_stream_flags* options, uint8_t* out)
{
    ...
    // Magic
    memcpy(out + 2 * 4 + LZMA_STREAM_FLAGS_SIZE, lzma_footer_magic,
