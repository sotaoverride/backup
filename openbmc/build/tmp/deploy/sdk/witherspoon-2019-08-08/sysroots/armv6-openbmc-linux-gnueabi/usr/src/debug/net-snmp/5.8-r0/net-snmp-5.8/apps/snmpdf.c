...
netsnmp_variable_list* collect(netsnmp_session* ss, netsnmp_pdu* pdu, oid* base,
                               size_t base_length)
{
                   ...
                   unsigned long scale)
{
    if (mem >= scale * scale * scale * scale)
        snprintf(buf, len, "%4.2fP%sB",
                 (float)mem / (scale * scale * scale * scale),
                 scale == 1024 ? "i" : "");
 ...
 */
unsigned long convert_units(unsigned long value, size_t units, size_t divisor)
{
...
int main(int argc, char* argv[])
{
    ...
    if (ucd_mib == 0)
    {
        ...
        while (vlp)
        {
            ...
            if (vlp2->type == SNMP_NOSUCHINSTANCE)
                goto next;
            len = vlp2->val_len;
            if (len >= SPRINT_MAX_LEN)
                len = SPRINT_MAX_LEN - 1;
            ...
            if (human_units)
            {
                ...
                printf("%-18s %10s %10s %10s %4lu%%\n", descr,
                       format_human(size, sizeof size,
                                    units ? convert_units(hssize, units, 1024)
                                          : hssize,
                                    human_units),
                       format_human(used, sizeof used,
                                    units ? convert_units(hsused, units, 1024)
                                          : hsused,
                                    human_units),
                       format_human(
                           avail, sizeof avail,
                           units ? convert_units(hssize - hsused, units, 1024)
                                 : hssize - hsused,
                           human_units),
    ...
    if (count == 0)
    {
        ...
        while (vlp)
        {
            ...
            add(pdu, "UCD-SNMP-MIB:dskPath", &(vlp->name[base_length]),
                vlp->name_length - base_length);
            add(pdu, "UCD-SNMP-MIB:dskTotal", &(vlp->name[base_length]),
                vlp->name_length - base_length);
            add(pdu, "UCD-SNMP-MIB:dskUsed", &(vlp->name[base_length]),
                vlp->name_length - base_length);
