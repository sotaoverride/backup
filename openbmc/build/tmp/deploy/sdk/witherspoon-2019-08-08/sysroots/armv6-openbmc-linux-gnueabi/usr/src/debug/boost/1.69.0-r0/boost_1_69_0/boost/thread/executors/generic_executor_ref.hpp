...
class executor_ref : public executor
{
  ...
  public:
     ...
     *
     * \par Synchronization
     * The completion of all the closures happen before the completion of the
     * executor destructor.
     */
    ~executor_ref()
    {
    ...
    /**
     * \par Effects
     * The specified closure will be scheduled for execution at some point in
     * the future. If invoked closure throws an exception the executor will call
     * std::terminate, as is the case with threads.
     *
     * \par Synchronization
     * Ccompletion of closure on a particular thread happens before destruction
...
class generic_executor_ref
{
  ...
  public:
    ...
    generic_executor_ref(Executor& ex_)
        //: ex(make_shared<executor_ref<Executor> >(ex_)) // todo check why this
        //doesn't works with C++03
        :
    ...
    // generic_executor_ref& operator=(generic_executor_ref const& other)
    // noexcept    {}
    ...
    /**
     * \par Requires
     * \c Closure is a model of Callable(void()) and a model of
     * CopyConstructible/MoveConstructible.
     *
     * \par Effects
     * The specified closure will be scheduled for execution at some point in
     * the future. If invoked closure throws an exception the thread pool will
     * call std::terminate, as is the case with threads.
     *
     * \par Synchronization
