 ...
 * cl->afterEncBuf contains the RRE encoded version.  If the RRE encoded version
 * is larger than the raw data or if it exceeds cl->afterEncBufSize then raw
...
rfbBool rfbSendRectEncodingRRE(rfbClientPtr cl, int x, int y, int w, int h)
{
    ...
    if (cl->beforeEncBufSize < maxRawSize)
    {
        ...
        if (cl->beforeEncBuf == NULL)
            ...
            cl->beforeEncBuf =
                (char*)realloc(cl->beforeEncBuf, cl->beforeEncBufSize);
    }
    ...
    if (cl->afterEncBufSize < maxRawSize)
    {
        ...
        if (cl->afterEncBuf == NULL)
            ...
            cl->afterEncBuf =
                (char*)realloc(cl->afterEncBuf, cl->afterEncBufSize);
    }
    ...
    rfbStatRecordEncodingSent(cl, rfbEncodingRRE,
                              sz_rfbFramebufferUpdateRectHeader +
                                  sz_rfbRREHeader + cl->afterEncBufLen,
                              sz_rfbFramebufferUpdateRectHeader +
                                  w * h * (cl->format.bitsPerPixel / 8));
...
#define DEFINE_SUBRECT_ENCODE(bpp)                                             \
    static int subrectEncode##bpp(rfbClientPtr client, uint##bpp##_t* data,    \
                                  int w, int h)                                \
    {                                                                          \
        uint##bpp##_t cl;                                                      \
        ...
        int newLen;                                                            \
        uint##bpp##_t bg =                                                     \
            (uint##bpp##_t)getBgColour((char*)data, w * h, bpp);               \
                                                                               \
            ...
            {                                                                  \
                ...
                {                                                              \
                    ...
                    subrect.h = Swap16IfLE(theh);                              \
                                                                               \
                    newLen =                                                   \
                        client->afterEncBufLen + (bpp / 8) + sz_rfbRectangle;  \
                    if ((newLen > (w * h * (bpp / 8))) ||                      \
                        (newLen > client->afterEncBufSize))                    \
                        return -1;                                             \
                                                                               \
                    numsubs += 1;                                              \
                    *((uint##bpp##_t*)(client->afterEncBuf +                   \
                                       client->afterEncBufLen)) = cl;          \
                    client->afterEncBufLen += (bpp / 8);                       \
                    memcpy(&client->afterEncBuf[client->afterEncBufLen],       \
                           &subrect, sz_rfbRectangle);                         \
