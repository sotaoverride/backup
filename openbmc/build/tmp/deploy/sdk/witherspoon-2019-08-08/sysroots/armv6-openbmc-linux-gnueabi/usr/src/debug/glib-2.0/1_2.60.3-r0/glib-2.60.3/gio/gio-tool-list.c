...
static const GOptionEntry entries[] = {
    {"attributes", 'a', 0, G_OPTION_ARG_STRING, &attributes,
     N_("The attributes to get"), N_("ATTRIBUTES")},
    {"hidden", 'h', 0, G_OPTION_ARG_NONE, &show_hidden, N_("Show hidden files"),
     NULL},
    {"long", 'l', 0, G_OPTION_ARG_NONE, &show_long,
     N_("Use a long listing format"), NULL},
    {"nofollow-symlinks", 'n', 0, G_OPTION_ARG_NONE, &nofollow_symlinks,
     N_("Don’t follow symbolic links"), NULL},
    {"print-uris", 'u', 0, G_OPTION_ARG_NONE, &print_uris,
     N_("Print full URIs"), NULL},
    {NULL}};

static void show_file_listing(GFileInfo* info, GFile* parent)
{
    ...
    if (show_long)
        g_print("%s\t%" G_GUINT64_FORMAT "\t(%s)", print_uris ? uri : name,
                (guint64)size, type);
    else
    ...
    for (i = 0; attributes[i] != NULL; i++)
    {
        ...
        if (first_attr)
        {
            ...
            g_print(" ");
        val_as_string =
            g_file_info_get_attribute_as_string(info, attributes[i]);
...
static gboolean list(GFile* file)
{
    ...
    enumerator = g_file_enumerate_children(
        file, attributes,
        nofollow_symlinks ? G_FILE_QUERY_INFO_NOFOLLOW_SYMLINKS : 0, NULL,
        &error);
    if (enumerator == NULL)
    {
...
int handle_list(int argc, char* argv[], gboolean do_help)
{
    ...
    g_option_context_set_description(
        ...
        _("gio list is similar to the traditional ls utility, but using GIO\n"
          ...
          "something\n"
          "like smb://server/resource/file.txt as location. File attributes "
          "can\n"
