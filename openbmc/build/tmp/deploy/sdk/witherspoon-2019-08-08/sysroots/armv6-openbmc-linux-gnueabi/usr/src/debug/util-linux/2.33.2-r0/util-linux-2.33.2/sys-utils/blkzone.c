...
static int init_device(struct blkzone_control* ctl, int mode)
{
    ...
    if (blkdev_get_sectors(fd, (unsigned long long*)&ctl->total_sectors))
        err(EXIT_FAILURE, _("%s: blkdev_get_sectors ioctl failed"),
            ctl->devname);
...
static int blkzone_report(struct blkzone_control* ctl)
{
    ...
    if (ctl->offset >= ctl->total_sectors)
        errx(EXIT_FAILURE,
             _("%s: offset is greater than or equal to device size"),
             ctl->devname);

    zonesize = blkdev_chunk_sectors(ctl->devname);
    if (!zonesize)
        errx(EXIT_FAILURE, _("%s: unable to determine zone size"),
             ctl->devname);
    ...
    while (nr_zones && ctl->offset < ctl->total_sectors)
    {
        ...
        if (ioctl(fd, BLKREPORTZONE, zi) == -1)
            err(EXIT_FAILURE, _("%s: BLKREPORTZONE ioctl failed"),
                ctl->devname);
        ...
        for (i = 0; i < zi->nr_zones; i++)
        {
            ...
            printf(_("  start: 0x%09" PRIx64 ", len 0x%06" PRIx64
                   ...
                   start, len, (type == 0x1) ? 0 : wp - start, entry->reset,
                   entry->non_seq, cond,
                   condition_str[cond & (ARRAY_SIZE(condition_str) - 1)], type,
                   type_text[type]);
...
static int blkzone_reset(struct blkzone_control* ctl)
{
    ...
    if (ctl->offset & (zonesize - 1))
        errx(EXIT_FAILURE,
             _("%s: offset %" PRIu64 " is not aligned "
               "to zone size %lu"),
             ctl->devname, ctl->offset, zonesize);

    if (ctl->offset > ctl->total_sectors)
        errx(EXIT_FAILURE, _("%s: offset is greater than device size"),
             ctl->devname);
    ...
    if (ctl->length && (zlen & (zonesize - 1)) &&
        ctl->offset + zlen != ctl->total_sectors)
        errx(EXIT_FAILURE,
             _("%s: number of sectors %" PRIu64 " is not aligned "
               "to zone size %lu"),
    ...
    if (ioctl(fd, BLKRESETZONE, &za) == -1)
        ...
        printf(
            _("%s: successfully reset in range from %" PRIu64 ", to %" PRIu64),
...
static void __attribute__((__noreturn__)) usage(void)
{
    ...
    fputs(_(" -o, --offset <sector>  start sector of zone to act (in 512-byte "
            "sectors)\n"),
          out);
    fputs(_(" -l, --length <sectors> maximum sectors to act (in 512-byte "
            "sectors)\n"),
          out);
...
int main(int argc, char** argv)
{
    ...
    struct blkzone_control ctl = {
        .devname = NULL, .offset = 0, .count = 0, .length = 0};
    ...
    while ((c = getopt_long(argc, argv, "hc:l:o:vV", longopts, NULL)) != -1)
    {
        ...
        switch (c)
        {
            case 'h':
                ...
                break;
            case 'c':
                ctl.count = strtou32_or_err(
                    optarg, _("failed to parse number of zones"));
                break;
            case 'l':
                ctl.length = strtosize_or_err(
                    optarg, _("failed to parse number of sectors"));
                break;
            case 'o':
                ctl.offset =
                    strtosize_or_err(optarg, _("failed to parse zone offset"));
                break;
            case 'v':
                ctl.verbose = 1;
                break;
