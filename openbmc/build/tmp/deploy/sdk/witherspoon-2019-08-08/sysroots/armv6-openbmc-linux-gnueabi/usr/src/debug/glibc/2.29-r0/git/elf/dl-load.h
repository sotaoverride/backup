...
static void __always_inline _dl_postprocess_loadcmd(struct link_map* l,
                                                    ...
                                                    const struct loadcmd* c)
{
    ...
    if (l->l_phdr == 0 && c->mapoff <= header->e_phoff &&
        ((size_t)(c->mapend - c->mapstart + c->mapoff) >=
         header->e_phoff + header->e_phnum * sizeof(ElfW(Phdr))))
        /* Found the program header in this segment.  */
        l->l_phdr =
            (void*)(uintptr_t)(c->mapstart + header->e_phoff - c->mapoff);
}
...
static const char* _dl_map_segments(struct link_map* l, int fd,
                                    ...
                                    size_t nloadcmds, const size_t maplength,
                                    bool has_holes, struct link_map* loader);
...
#define DL_MAP_SEGMENTS_ERROR_MPROTECT N_("cannot change memory protections")
#define DL_MAP_SEGMENTS_ERROR_MAP_ZERO_FILL N_("cannot map zero-fill pages")
