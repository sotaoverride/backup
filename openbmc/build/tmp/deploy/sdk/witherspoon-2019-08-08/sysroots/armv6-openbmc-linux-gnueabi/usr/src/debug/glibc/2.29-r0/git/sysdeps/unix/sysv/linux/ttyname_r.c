...
static int getttyname_r(char* buf, size_t buflen, const struct stat64* mytty,
                        int save, int* dostat);
    ...
    char* buf, size_t buflen, const struct stat64* mytty, int save, int* dostat)
{
    ...
    while ((d = __readdir64(dirstream)) != NULL)
            ...
            strcmp(d->d_name, "stderr"))
        {
            ...
            if (__xstat64(_STAT_VER, buf, &st) == 0 && is_mytty(mytty, &st))
            {
                (void)__closedir(dirstream);
                __set_errno(save);
                return 0;
...
#define UNREACHABLE_LEN strlen("(unreachable)")
        ...
        if (buf[0] == '/' && __xstat64(_STAT_VER, buf, &st1) == 0 &&
            is_mytty(&st, &st1))
            return 0;
