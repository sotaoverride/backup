...
enum
{
    tid_size = sizeof(GetCurrentThreadId())
};
...
enum
{
    headroom_size = sizeof(pthread_t) > sizeof(uintmax_t)
                        ? 0u
                        : (sizeof(uintmax_t) - sizeof(pthread_t)),
...
BOOST_LOG_ANONYMOUS_NAMESPACE
{
    ...
    inline thread::id get_id_impl()
    {
        ...
        // For now we use the hackish cast to get some opaque number that
        // hopefully correlates with system thread identification.
        thread::id::native_type int_id = 0;
        pthread_t pthread_id = pthread_self();
#if BOOST_ENDIAN_BIG_BYTE || BOOST_ENDIAN_BIG_WORD
        std::memcpy(reinterpret_cast<unsigned char*>(&int_id) + headroom_size,
                    &pthread_id, tid_size);
#else
...
BOOST_LOG_ANONYMOUS_NAMESPACE
{
    ...
    struct id_storage
    {
        aligned_storage<sizeof(thread::id),
                        alignment_of<thread::id>::value>::type m_storage;
        bool m_initialized;
    };
...
BOOST_LOG_ANONYMOUS_NAMESPACE
{
    ...
    struct id_storage : lazy_singleton<id_storage>
    {
        struct deleter
        {
    ...
    BOOST_LOG_ONCE_BLOCK()
    {
        if (int err = pthread_key_create(&g_key, &deleter))
        {
            BOOST_LOG_THROW_DESCR_PARAMS(
                system_error,
                "Failed to create a thread-specific storage for thread id",
                (err));
        }
    }
    ...
    operator<<(std::basic_ostream<CharT, TraitsT>& strm, thread::id const& tid)
{
    if (strm.good())
    {
        CharT buf[tid_size * 2 + 3]; // 2 chars per byte + 3 chars for the
                                     // leading 0x and terminating zero
        format_id<tid_size>(buf, sizeof(buf) / sizeof(*buf), tid.native_id(),
                            (strm.flags() & std::ios_base::uppercase) != 0);
...
template BOOST_LOG_API std::basic_ostream<char, std::char_traits<char>>&
    operator<<(std::basic_ostream<char, std::char_traits<char>>& strm,
               thread::id const& tid);
#endif // defined(BOOST_LOG_USE_CHAR)

#if defined(BOOST_LOG_USE_WCHAR_T)
template BOOST_LOG_API std::basic_ostream<wchar_t, std::char_traits<wchar_t>>&
    operator<<(std::basic_ostream<wchar_t, std::char_traits<wchar_t>>& strm,
               thread::id const& tid);
