...
class BOOST_LOG_NO_VTABLE basic_sink_frontend : public sink
{
    //! Base type
  ...
  public:
     ...
     *
     * \param cross_thread The flag indicates whether the sink passes log
     * records between different threads
     */
    ...
    void set_filter(FunT const& filter)
    {
        BOOST_LOG_EXPR_IF_MT(
            boost::log::aux::exclusive_lock_guard<mutex_type> lock(m_Mutex);)
        m_Filter = filter;
    }
    /*!
    ...
    /*!
     * The method returns \c true if no filter is set or the attribute values
     * pass the filter
     *
    ...
    {
        BOOST_LOG_EXPR_IF_MT(
            boost::log::aux::shared_lock_guard<mutex_type> lock(m_Mutex);)
        try
        {
  ...
  protected:
                     ...
                     BackendT& backend)
    {
        ...
        {
            BOOST_LOG_EXPR_IF_MT(
                boost::log::aux::exclusive_lock_guard<BackendMutexT> lock(
                    backend_mutex);)
            backend.consume(rec);
        }
#if !defined(BOOST_LOG_NO_THREADS)
    ...
    //! Attempts to feeds log record to the backend, does not block if \a
    //! backend_mutex is locked
    template <typename BackendMutexT, typename BackendT>
                         ...
                         BackendT& backend)
    {
        ...
        {
            boost::log::aux::shared_lock_guard<mutex_type> frontend_lock(
                this->frontend_mutex());
        ...
        boost::log::aux::exclusive_auto_unlocker<BackendMutexT> unlocker(
            backend_mutex);
#endif
    ...
    : public basic_sink_frontend
{
  ...
  private:
    struct formatting_context
    {
        class cleanup_guard
        {
          ...
          public:
            explicit cleanup_guard(formatting_context& ctx) BOOST_NOEXCEPT
                : m_context(ctx)
            {
            }
            ...
            ~cleanup_guard()
            {
                ...
                m_context.m_FormattingStream.rdbuf()->max_size(
                    m_context.m_FormattedRecord.max_size());
        ...
        {
            m_FormattingStream.exceptions(std::ios_base::badbit |
        ...
        formatting_context(unsigned int version, std::locale const& loc,
            ...
            m_Version(version),
            m_FormattingStream(m_FormattedRecord), m_Formatter(formatter)
        {
            m_FormattingStream.exceptions(std::ios_base::badbit |
                                          std::ios_base::failbit);
  ...
  public:
    ...
    {
#if !defined(BOOST_LOG_NO_THREADS)
        boost::log::aux::exclusive_lock_guard<mutex_type> lock(
            this->frontend_mutex());
  ...
  protected:
                     ...
                     BackendT& backend)
    {
        ...
        {
            {
                boost::log::aux::shared_lock_guard<mutex_type> lock(
                    this->frontend_mutex());
                context =
                    new formatting_context(m_Version, m_Locale, m_Formatter);
            }
...
template <typename BackendT,
          bool RequiresFormattingV =
              has_requirement<typename BackendT::frontend_requirements,
                              formatted_records>::value>
