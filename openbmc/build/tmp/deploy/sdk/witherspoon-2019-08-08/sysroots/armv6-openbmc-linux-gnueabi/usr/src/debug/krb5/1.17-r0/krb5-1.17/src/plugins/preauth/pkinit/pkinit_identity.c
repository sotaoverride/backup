...
static krb5_error_code copy_list(char*** dst, char** src)
{
    int i;
    char** newlist;
...
#ifndef WITHOUT_PKCS11
static krb5_error_code parse_pkcs11_options(krb5_context context,
...
static krb5_error_code parse_fs_options(krb5_context context,
                                        pkinit_identity_opts* idopts,
                                        const char* residual)
{
...
static krb5_error_code parse_pkcs12_options(krb5_context context,
                                            pkinit_identity_opts* idopts,
                                            const char* residual)
{
    krb5_error_code retval = ENOMEM;
...
static krb5_error_code process_option_identity(
    ...
    pkinit_identity_crypto_context id_cryptoctx, const char* value)
{
    const char* residual;
    int idtype;
...
static krb5_error_code process_option_ca_crl(
    krb5_context context, pkinit_plg_crypto_context plg_cryptoctx,
    pkinit_req_crypto_context req_cryptoctx, pkinit_identity_opts* idopts,
    pkinit_identity_crypto_context id_cryptoctx, const char* value, int catype)
{
    char* residual;
    unsigned int typelen;
    int idtype;
...
krb5_error_code pkinit_identity_initialize(
    krb5_context context, pkinit_plg_crypto_context plg_cryptoctx,
    pkinit_req_crypto_context req_cryptoctx, pkinit_identity_opts* idopts,
    pkinit_identity_crypto_context id_cryptoctx, krb5_clpreauth_callbacks cb,
    krb5_clpreauth_rock rock, krb5_principal princ)
{
    krb5_error_code retval = EINVAL;
    int i;

    pkiDebug("%s: %p %p %p\n", __FUNCTION__, context, idopts, id_cryptoctx);
    if (!(princ && krb5_principal_compare_any_realm(
                       context, princ, krb5_anonymous_principal())))
    {
        ...
        {
            retval =
                process_option_identity(context, plg_cryptoctx, req_cryptoctx,
                                        idopts, id_cryptoctx, idopts->identity);
        }
        else if (idopts->identity_alt != NULL)
        {
            for (i = 0; retval != 0 && idopts->identity_alt[i] != NULL; i++)
            {
                retval = process_option_identity(
                    context, plg_cryptoctx, req_cryptoctx, idopts, id_cryptoctx,
...
krb5_error_code pkinit_identity_prompt(
    ...
    pkinit_req_crypto_context req_cryptoctx, pkinit_identity_opts* idopts,
    pkinit_identity_crypto_context id_cryptoctx, krb5_clpreauth_callbacks cb,
    krb5_clpreauth_rock rock, int do_matching, krb5_principal princ)
{
    krb5_error_code retval = EINVAL;
    ...
    for (i = 0; idopts->anchors != NULL && idopts->anchors[i] != NULL; i++)
    {
        retval = process_option_ca_crl(context, plg_cryptoctx, req_cryptoctx,
                                       idopts, id_cryptoctx, idopts->anchors[i],
                                       CATYPE_ANCHORS);
        if (retval)
            goto errout;
    }
    for (i = 0;
         idopts->intermediates != NULL && idopts->intermediates[i] != NULL; i++)
    {
        retval = process_option_ca_crl(
            context, plg_cryptoctx, req_cryptoctx, idopts, id_cryptoctx,
            idopts->intermediates[i], CATYPE_INTERMEDIATES);
        if (retval)
            goto errout;
    }
    ...
    {
        retval =
            process_option_ca_crl(context, plg_cryptoctx, req_cryptoctx, idopts,
                                  id_cryptoctx, idopts->crls[i], CATYPE_CRLS);
        if (retval)
            goto errout;
    }
 ...
 */
krb5_error_code pkinit_set_deferred_id(pkinit_deferred_id** identities,
                                       const char* identity,
 ...
 */
const char* pkinit_find_deferred_id(pkinit_deferred_id* identities,
                                    const char* identity)
{
    int i;
 ...
 */
unsigned long pkinit_get_deferred_id_flags(pkinit_deferred_id* identities,
                                           const char* identity)
{
    int i;
