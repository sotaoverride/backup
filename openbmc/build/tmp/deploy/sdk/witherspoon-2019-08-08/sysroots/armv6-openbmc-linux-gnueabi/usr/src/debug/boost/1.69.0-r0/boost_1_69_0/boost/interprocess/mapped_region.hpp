...
#if defined(sun) || defined(__sun) || defined(__osf__) || defined(__osf) ||    \
    defined(_hpux) || defined(hpux) || defined(_AIX)
...
// Solaris declares madvise only in some configurations but defines MADV_XXX, a
// bit confusing. Predeclare it here to avoid any compilation error
...
class mapped_region
{
  ...
  public:
    ...
    //!"dwDesiredAccess" to "MapViewOfFileEx". If "default_map_options" is
    //!passed it's initialized to zero. "map_options" is XORed with
    //! FILE_MAP_[COPY|READ|WRITE].
    //!
    //! In UNIX systems and POSIX mappings "map_options" is an int value passed
    //! as "flags" to "mmap". If "default_map_options" is specified it's
    //! initialized to MAP_NOSYNC if that option exists and to zero otherwise.
    //! "map_options" XORed with MAP_PRIVATE or MAP_SHARED.
    //!
    //! In UNIX systems and XSI mappings "map_options" is an int value passed as
    //! "shmflg" to "shmat". If "default_map_options" is specified it's
    //! initialized to zero. "map_options" is XORed with SHM_RDONLY if needed.
    //!
    //! The OS could allocate more pages than size/page_size(), but
    //! get_address() will always return the address passed in this function (if
    //! not null) and get_size() will return the specified size.
    ...
    //! Move constructor. *this will be constructed taking ownership of
    //! "other"'s region and "other" will be left in default constructor state.
    mapped_region(BOOST_RV_REF(mapped_region) other)
#if defined(BOOST_INTERPROCESS_WINDOWS)
        :
        m_base(0),
        m_size(0), m_page_offset(0), m_mode(read_only),
        m_file_or_mapping_hnd(ipcdetail::invalid_file())
#else
        :
        m_base(0),
        m_size(0), m_page_offset(0), m_mode(read_only), m_is_xsi(false)
#endif
    {
        this->swap(other);
    }
    ...
    //! If 'async' is true, the function will return before flushing operation
    //! is completed If 'async' is false, function will return once data has been
    //! written into the underlying device (i.e., in mapped files OS cached
    //! information is written to disk). Never throws. Returns false if operation
    //! could not be performed.
    bool flush(std::size_t mapping_offset = 0, std::size_t numbytes = 0,
               bool async = true);

    //! Shrinks current mapped region. If after shrinking there is no longer
    //! need for a previously mapped memory page, accessing that page can trigger
    //! a segmentation fault. Depending on the OS, this operation might fail (XSI
    //! shared memory), it can decommit storage and free a portion of the virtual
    //! address space (e.g.POSIX) or this function can release some physical
    //! memory wihout freeing any virtual address space(Windows). Returns true on
    //! success. Never throws.
    bool shrink_by(std::size_t bytes, bool from_back = true);

    //! This enum specifies region usage behaviors that an application can
    //! specify to the mapped region implementation.
    enum advice_types
    {
        //! Specifies that the application has no advice to give on its behavior
        //! with respect to
        //! the region. It is the default characteristic if no advice is given
        //! for a range of memory.
        advice_normal,
        //! Specifies that the application expects to access the region
        //! sequentially from
        //! lower addresses to higher addresses. The implementation can lower
        //! the priority of
        //! preceding pages within the region once a page have been accessed.
        advice_sequential,
        //! Specifies that the application expects to access the region in a
        //! random order,
        //! and prefetching is likely not advantageous.
        advice_random,
        //! Specifies that the application expects to access the region in the
        //! near future.
        ...
        //! The implementation can unload pages within the range to save system
        //! resources.
        advice_dontneed
    };

    //! Advises the implementation on the expected behavior of the application
    //! with respect to the data in the region. The implementation may use this
    //! information to optimize handling of the region data. This function has no
    //! effect on the semantics of access to memory in the region, although it
    //! may affect the performance of access. If the advise type is not known to
    //! the implementation, the function returns false. True otherwise.
  ...
  private:
    ...
    bool priv_flush_param_check(std::size_t mapping_offset, void*& addr,
                                std::size_t& numbytes) const;
    bool priv_shrink_param_check(std::size_t bytes, bool from_back,
                                 void*& shrink_page_start,
                                 std::size_t& shrink_page_bytes);
    static void priv_size_from_mapping_size(offset_t mapping_size,
                                            offset_t offset,
                                            offset_t page_offset,
                                            std::size_t& size);
    static offset_t priv_page_offset_addr_fixup(offset_t page_offset,
                                                const void*& addr);
...
#if defined(BOOST_INTERPROCESS_WINDOWS) &&                                     \
    !defined(BOOST_INTERPROCESS_FORCE_GENERIC_EMULATION)
    template <int Dummy>
...
inline bool
    mapped_region::priv_shrink_param_check(std::size_t bytes, bool from_back,
                                           void*& shrink_page_start,
                                           std::size_t& shrink_page_bytes)
{
    // Check some errors
    ...
    else
    {
        ...
        if (from_back)
        {
            const std::size_t new_pages =
                (m_size + m_page_offset - bytes - 1) / page_size + 1;
            shrink_page_start = static_cast<char*>(this->priv_map_address()) +
                                new_pages * page_size;
...
inline void mapped_region::priv_size_from_mapping_size(offset_t mapping_size,
                                                       offset_t offset,
                                                       offset_t page_offset,
                                                       std::size_t& size)
{
...
inline offset_t mapped_region::priv_page_offset_addr_fixup(offset_t offset,
                                                           const void*& address)
{
    ...
    //(always less than a page in std::size_t, thus, representable by
    //std::size_t)
...
template <class MemoryMappable>
inline mapped_region::mapped_region(const MemoryMappable& mapping, mode_t mode,
                                    offset_t offset, std::size_t size,
                                    const void* address,
                                    map_options_t map_options) :
    m_base(0),
    m_size(0), m_page_offset(0), m_mode(mode),
    m_file_or_mapping_hnd(ipcdetail::invalid_file())
{
    mapping_handle_t mhandle = mapping.get_mapping_handle();
    {
        ...
        // For "mapviewoffile"
        unsigned long map_access =
            map_options == default_map_options ? 0 : map_options;
        ...
        // For file mapping (including emulated shared memory through temporary
        // files), the device is a file handle so we need to obtain file's size
        // and call create_file_mapping to obtain the mapping handle. For files
        // we don't need the file mapping after mapping the memory, as the file
        // is there so we'll program the handle close
        ...
        {
            // For windows_shared_memory the device handle is already a mapping
            // handle and we need to maintain it
        ...
        const offset_t page_offset =
            priv_page_offset_addr_fixup(offset, address);

        // Obtain mapping size if user provides 0 size
        if (size == 0)
        {
            offset_t mapping_size;
            if (!winapi::get_file_mapping_size(native_mapping_handle,
                                               mapping_size))
            {
                error_info err = winapi::get_last_error();
                throw interprocess_exception(err);
            ...
            // This can throw
            priv_size_from_mapping_size(mapping_size, offset, page_offset,
                                        size);
        }

        // Map with new offsets and size
        void* base = winapi::map_view_of_file_ex(
            native_mapping_handle, map_access, offset - page_offset,
    ...
    //
    // For mapped files, we duplicate the file handle to be able to
    // FlushFileBuffers
    if (!winapi::duplicate_current_process_handle(mhandle.handle,
...
inline bool mapped_region::flush(std::size_t mapping_offset,
                                 std::size_t numbytes, bool async)
{
    void* addr;
...
inline bool mapped_region::shrink_by(std::size_t bytes, bool from_back)
{
    ...
    if (!this->priv_shrink_param_check(bytes, from_back, shrink_page_start,
                                       shrink_page_bytes))
    {
        return false;
    }
    ...
    {
        // In Windows, we can't decommit the storage or release the virtual
        // address space, the best we can do is try to remove some memory from
        // the process working set. With a bit of luck we can free some physical
        // memory.
        unsigned long old_protect_ignored;
        bool b_ret =
            winapi::virtual_unlock(shrink_page_start, shrink_page_bytes) ||
        ...
        b_ret = b_ret && winapi::virtual_protect(
                             shrink_page_start, shrink_page_bytes,
                             winapi::page_noaccess, old_protect_ignored);
        return b_ret;
    }
    else
    {
...
inline mapped_region::mapped_region(const MemoryMappable& mapping, mode_t mode,
                                    ...
                                    map_options_t map_options) :
    m_base(0),
    m_size(0), m_page_offset(0), m_mode(mode), m_is_xsi(false)
{
...
// Some systems dont' support XSI shared memory
    ...
    if (map_hnd.is_xsi)
    {
        ...
        // Attach memory
        // Some old shmat implementation take the address as a non-const void
        // pointer so uncast it to make code portable.
...
#define BOOST_INTERPROCESS_MAP_NOSYNC 0
    ...
    // Create new mapping
    int prot = 0;
    int flags = map_options == default_map_options
...
#undef BOOST_INTERPROCESS_MAP_NOSYNC
    ...
    void* base =
        mmap(const_cast<void*>(address),
             static_cast<std::size_t>(page_offset + size), prot, flags,
             mapping.get_mapping_handle().handle, offset - page_offset);
...
inline bool mapped_region::shrink_by(std::size_t bytes, bool from_back)
{
    void* shrink_page_start = 0;
    std::size_t shrink_page_bytes = 0;
    if (m_is_xsi || !this->priv_shrink_param_check(
                        bytes, from_back, shrink_page_start, shrink_page_bytes))
    {
        return false;
    }
    else if (shrink_page_bytes)
                                 ...
                                 std::size_t numbytes, bool async)
{
    void* addr;
    if (m_is_xsi ||
        !this->priv_flush_param_check(mapping_offset, addr, numbytes))
    {
        return false;
    }
    // Flush it all
...
inline bool mapped_region::advise(advice_types advice)
{
    ...
    defined(BOOST_INTERPROCESS_MADV_DONTNEED_HAS_NONDESTRUCTIVE_SEMANTICS)
...
#if defined(POSIX_MADV_NORMAL)
        case mode_padv:
            return 0 == posix_madvise(this->priv_map_address(),
                                      this->priv_map_size(), unix_advice);
#endif
...
#if defined(BOOST_INTERPROCESS_WINDOWS)
    ::boost::adl_move_swap(this->m_file_or_mapping_hnd,
                           other.m_file_or_mapping_hnd);
#else
...
template <int Dummy>
inline void mapped_region::destroy_syncs_in_range(const void* addr,
                                                  std::size_t size)
{
    ipcdetail::sync_handles& handles = ipcdetail::windows_intermodule_singleton<
        ipcdetail::sync_handles>::get();
