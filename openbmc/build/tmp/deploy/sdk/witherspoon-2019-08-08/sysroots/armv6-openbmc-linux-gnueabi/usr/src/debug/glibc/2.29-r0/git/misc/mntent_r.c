...
libc_hidden_def(__setmntent) weak_alias(__setmntent, setmntent)

    /* Close a stream opened with `setmntent'.  */
    int __endmntent(FILE* stream)
{
    if (stream) /* SunOS 4.x allows for NULL stream */
        fclose(stream);
    return 1; /* SunOS 4.x says to always return 1 */
}
libc_hidden_def(__endmntent) weak_alias(__endmntent, endmntent)
...
/* Read one mount table entry from STREAM.  Returns a pointer to storage
   ...
   check).  */
struct mntent* __getmntent_r(FILE* stream, struct mntent* mp, char* buffer,
                             int bufsiz)
{
    char* cp;
    char* head;
...
libc_hidden_def(__getmntent_r) weak_alias(__getmntent_r, getmntent_r)
    ...
    int __addmntent(FILE* stream, const struct mntent* mnt)
{
    ...
    return (fprintf(stream, "%s %s %s %s %d %d\n", mntcopy.mnt_fsname,
                    mntcopy.mnt_dir, mntcopy.mnt_type, mntcopy.mnt_opts,
                    mntcopy.mnt_freq, mntcopy.mnt_passno) < 0 ||
            fflush(stream) != 0);
}
...
libc_hidden_def(__hasmntopt) weak_alias(__hasmntopt, hasmntopt)
