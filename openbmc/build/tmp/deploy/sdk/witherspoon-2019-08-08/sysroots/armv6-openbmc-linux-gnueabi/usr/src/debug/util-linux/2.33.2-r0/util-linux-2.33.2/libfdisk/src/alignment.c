 ...
 * SECTION: alignment
 * @title: Alignment
 * @short_description: functions to align partitions and work with disk topology
 * and geometry
 *
                               ...
                               int direction)
{
    ...
    if (lba != res)
        DBG(CXT,
            ul_debugobj(cxt, "LBA %12ju aligned-%s %12ju [grain=%lus]",
                        (uintmax_t)lba,
                        direction == FDISK_ALIGN_UP
                            ? "up  "
                            : direction == FDISK_ALIGN_DOWN ? "down" : "near",
                        (uintmax_t)res, cxt->grain / cxt->sector_size));
    else
                                        ...
                                        fdisk_sector_t stop)
{
    ...
    DBG(CXT, ul_debugobj(cxt, "%ju in range <%ju..%ju> aligned to %ju",
                         (uintmax_t)lba, (uintmax_t)start, (uintmax_t)stop,
                         (uintmax_t)res));
    return res;
}
...
static void recount_geometry(struct fdisk_context* cxt)
{
    ...
    cxt->geom.cylinders =
        cxt->total_sectors / (cxt->geom.heads * cxt->geom.sectors);
}
 ...
 *
 * The difference between fdisk_override_geometry() and
 * fdisk_save_user_geometry() is that saved user geometry is persistent setting
 * and it's applied always when device is assigned to the context or device
                            ...
                            unsigned int heads, unsigned int sectors)
{
    ...
    DBG(CXT, ul_debugobj(
                 cxt, "override C/H/S: %u/%u/%u", (unsigned)cxt->geom.cylinders,
                 (unsigned)cxt->geom.heads, (unsigned)cxt->geom.sectors));
...
{
    return (cxt->user_pyh_sector || cxt->user_log_sector || cxt->user_grain ||
            fdisk_has_user_device_geometry(cxt));
}

int fdisk_has_user_device_geometry(struct fdisk_context* cxt)
{
    return (cxt->user_geom.heads || cxt->user_geom.sectors ||
...
int fdisk_apply_user_device_properties(struct fdisk_context* cxt)
{
    ...
    if (cxt->user_log_sector)
    {
        ...
        cxt->sector_size = cxt->min_io_size = cxt->io_size =
            cxt->user_log_sector;

        if (cxt->sector_size != old_secsz)
        {
            cxt->total_sectors =
                (old_total * (old_secsz / 512)) / (cxt->sector_size >> 9);
            DBG(CXT, ul_debugobj(cxt, "new total sectors: %ju",
    ...
    if (cxt->user_grain)
    {
        ...
        cxt->grain =
            cxt->user_grain < granularity ? granularity : cxt->user_grain;
    ...
    DBG(CXT,
        ul_debugobj(cxt, "new C/H/S: %u/%u/%u", (unsigned)cxt->geom.cylinders,
                    (unsigned)cxt->geom.heads, (unsigned)cxt->geom.sectors));
...
int fdisk_discover_geometry(struct fdisk_context* cxt)
{
    ...
    DBG(CXT, ul_debugobj(cxt, "total sectors: %ju (ioctl=%ju)",
                         (uintmax_t)cxt->total_sectors, (uintmax_t)nsects));
    ...
    DBG(CXT, ul_debugobj(
                 cxt, "result: C/H/S: %u/%u/%u", (unsigned)cxt->geom.cylinders,
                 (unsigned)cxt->geom.heads, (unsigned)cxt->geom.sectors));
    return 0;
}
...
static int has_topology(struct fdisk_context* cxt)
{
     ...
     */
    if (cxt && (cxt->optimal_io_size || cxt->alignment_offset ||
...
fdisk_sector_t fdisk_cround(struct fdisk_context* cxt, fdisk_sector_t num)
{
    return fdisk_use_cylinders(cxt)
               ? (num / fdisk_get_units_per_sector(cxt)) + 1
               : num;
}
