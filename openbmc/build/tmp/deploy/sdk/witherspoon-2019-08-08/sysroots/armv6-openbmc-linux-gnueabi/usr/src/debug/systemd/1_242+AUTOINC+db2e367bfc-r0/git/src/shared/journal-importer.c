...
static int fill_fixed_size(JournalImporter* imp, void** data, size_t size)
{
    ...
    assert(imp);
    assert(IN_SET(imp->state, IMPORTER_STATE_DATA_START, IMPORTER_STATE_DATA,
...
static int process_special_field(JournalImporter* imp, char* line)
{
    ...
    if (value)
    {
        ...
        if (r < 0)
            return log_warning_errno(
                r, "Failed to parse __REALTIME_TIMESTAMP '%s': %m",
                cellescape(buf, sizeof buf, value));
        else if (!VALID_REALTIME(x))
        {
            log_warning("__REALTIME_TIMESTAMP out of range, ignoring: %" PRIu64,
                        x);
            return -ERANGE;
        }
    ...
    if (value)
    {
        ...
        if (r < 0)
            return log_warning_errno(
                r, "Failed to parse __MONOTONIC_TIMESTAMP '%s': %m",
                cellescape(buf, sizeof buf, value));
        else if (!VALID_MONOTONIC(x))
        {
            log_warning(
                "__MONOTONIC_TIMESTAMP out of range, ignoring: %" PRIu64, x);
            return -ERANGE;
        }
    ...
    if (value)
    {
        log_notice("Unknown dunder line __%s, ignoring.",
                   cellescape(buf, sizeof buf, value));
        return 1;
    }
    ...
    switch (imp->state)
    {
        case IMPORTER_STATE_LINE:
        {
            ...
            */
            ...
            log_trace("Received: %.*s (%s)", (int)n, line,
                      sep ? "text" : "binary");
        ...
        case IMPORTER_STATE_DATA_START:
            ...
            imp->state = imp->data_size > 0 ? IMPORTER_STATE_DATA
                                            : IMPORTER_STATE_DATA_FINISH;
        ...
        case IMPORTER_STATE_DATA:
        {
            ...
            r = iovw_put(&imp->iovw, field + sizeof(uint64_t),
                         imp->field_len + imp->data_size);
            if (r < 0)
                return r;
...
int journal_importer_push_data(JournalImporter* imp, const char* data,
                               size_t size)
{
    assert(imp);
...
void journal_importer_drop_iovw(JournalImporter* imp)
{
    ...
    /* This function drops processed data that along with the iovw that points
     * at it */
    ...
    if (target < imp->size)
    {
        ...
        {
            log_debug("Reallocated buffer from %zu to %zu bytes", imp->size,
                      target);
            imp->buf = tmp;
            imp->size = target;
        }
