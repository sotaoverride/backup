...
typedef enum
{
    ...
    STREAMTYPE_NAMED_PIPE =
        3 /**< file is a named pipe (so far, tested for output only) */
} strmType_t;

typedef enum
{ /* when extending, do NOT change existing modes! */
...
#define STREAM_ASYNC_NUMBUFS                                                   \
    2 /* must be a power of 2 -- TODO: make configurable */
...
typedef struct strm_s
{
    ...
    unsigned int iCurrFNum; /* current file number (NOT descriptor, but the
    ...
    unsigned int
        iMaxFiles; /* maximum number of files if a circular mode is in use */
    int iFileNumDigits;   /* min number of digits to use in file number (only in
                             circular mode) */
    sbool bDeleteOnClose; /* set to 1 to auto-delete on close -- be careful with
                             that setting! */
    int64 iCurrOffs;      /* current offset */
    int64* pUsrWCntr; /* NULL or a user-provided counter that receives the nbr
    of bytes written since the last CntrSet() */
    sbool bPrevWasNL; /* used for readLine() when reading multi-line messages */
    /* dynamic properties, valid only during file open, not to be persistet */
    sbool bDisabled; /* should file no longer be written to? (currently set only
                        if omfile file size limit fails) */
    ...
    int fdDir; /* the directory's descriptor, in case bSync is requested (-1 if
                  closed) */
    ...
    char* pIOBuf_truncation; /* iobuffer used during trucation detection block
                                re-reads */
    size_t iBufPtrMax;       /* current max Ptr in Buffer (if partial read!) */
    size_t iBufPtr;          /* pointer into current buffer */
    int iUngetC;     /* char set via UngetChar() call or -1 if none set */
    sbool bInRecord; /* if 1, indicates that we are currently writing a not-yet
                        complete record */
    int iZipLevel;   /* zip level (0..9). If 0, zip is completely disabled */
    Bytef* pZipBuf;
    /* support for async flush procesing */
    sbool bAsyncWrite;  /* do asynchronous writes (always if a flush interval is
                           given) */
    sbool bStopWriter;  /* shall writer thread terminate? */
    sbool bDoTimedWait; /* instruct writer thread to do a times wait to support
                           flush timeouts */
    sbool bzInitDone;   /* did we do an init of zstrm already? */
    sbool bFlushNow;    /* shall we flush with the next async write? */
    sbool bVeryReliableZip; /* shall we write interim headers to create a very
                               reliable ZIP file? */
    ...
    unsigned short iEnq;   /* this MUST be unsigned as we use module arithmetic
                              (else invalid indexing happens!) */
    unsigned short iDeq;   /* this MUST be unsigned as we use module arithmetic
                              (else invalid indexing happens!) */
    ...
    /* support for omfile size-limiting commands, special counters, NOT
     * persisted! */
    ...
    cstr_t*
        prevLineSegment; /* for ReadLine, previous, unprocessed part of file */
    cstr_t* prevMsgSegment; /* for ReadMultiLine, previous, yet unprocessed part
                               of msg */
    int64 strtOffs;         /* start offset in file for current line/msg */
    int fileNotFoundError;  /* boolean; if set, report file not found errors,
                               else silently ignore */
    int noRepeatedErrorOutput; /* if a file is missing the Error is only given
                                  once */
    int ignoringMsg;
} strm_t;
...
/* v6 added */
rsRetVal (*ReadLine)(strm_t* pThis, cstr_t** ppCStr, uint8_t mode,
...
ENDinterface(strm)
#define strmCURR_IF_VERSION                                                    \
    13 /* increment whenever you change the interface structure! */
/* V10, 2013-09-10: added new parameter bEscapeLF, changed mode to uint8_t
 * (rgerhards) */
/* V11, 2015-12-03: added new parameter bReopenOnTruncate */
/* V12, 2015-12-11: added new parameter trimLineOverBytes, changed mode to
 * uint32_t */
...
rsRetVal strmMultiFileSeek(strm_t* pThis, unsigned int fileNum, off64_t offs,
                           off64_t* bytesDel);
rsRetVal strmReadMultiLine(strm_t* pThis, cstr_t** ppCStr, regex_t* start_preg,
                           regex_t* end_preg, sbool bEscapeLF,
                           sbool discardTruncatedMsg, sbool msgDiscardingError,
                           int64* const strtOffs);
