...
//! A simple trait that allows to use charset-specific Qi parsers in a generic
//! way
template <typename EncodingT>
struct encoding_specific;

#define BOOST_LOG_CHARSET_PARSERS                                              \
    ((char_type, char_))((string_type, string))((alnum_type, alnum))(          \
        (alpha_type, alpha))((blank_type, blank))((cntrl_type, cntrl))(        \
        (digit_type, digit))((graph_type, graph))((print_type, print))(        \
        (punct_type, punct))((space_type, space))((xdigit_type, xdigit))(      \
        (no_case_type, no_case))((lower_type, lower))((upper_type, upper))(    \
        (lowernum_type, lowernum))((uppernum_type, uppernum))

#define BOOST_LOG_DECLARE_CHARSET_PARSER(r, charset, parser)                   \
    typedef spirit::charset::BOOST_PP_TUPLE_ELEM(2, 0, parser)                 \
        BOOST_PP_TUPLE_ELEM(2, 0, parser);                                     \
    BOOST_LOG_API static BOOST_PP_TUPLE_ELEM(2, 0, parser)                     \
        const& BOOST_PP_TUPLE_ELEM(2, 1, parser);

#define BOOST_LOG_DECLARE_CHARSET_PARSERS(charset)                             \
    BOOST_PP_SEQ_FOR_EACH(BOOST_LOG_DECLARE_CHARSET_PARSER, charset,           \
                          BOOST_LOG_CHARSET_PARSERS)
