...
/* Maximal clock adjustment in seconds per day.
   (adjtime() glibc call has 2145 seconds limit on i386, so it is good enough
   for us as well, 43219 is a maximal safe value preventing exact_adjustment
   overflow.) */
...
static int hw_clock_is_utc(const struct hwclock_control* ctl,
                        ...
                        struct adjtime* adjtime_p)
{
    ...
    sscanf(line1, "%lf %ld %lf", &adjtime_p->drift_factor,
           &adjtime_p->last_adj_time, &adjtime_p->not_adjusted);
 ...
 * return *valid_p == true.
 */
static int mktime_tz(const struct hwclock_control* ctl, struct tm tm,
                     time_t* systime_p)
{
    int valid;
    ...
    else
    {
        ...
        if (ctl->verbose)
            printf(_("Hw clock time : %4d/%.2d/%.2d %.2d:%.2d:%.2d = "
                   ...
                   tm.tm_year + 1900, tm.tm_mon + 1, tm.tm_mday, tm.tm_hour,
                   tm.tm_min, tm.tm_sec, (long)*systime_p);
    }
    return valid;
}
 ...
 * clock.
 */
static int read_hardware_clock(const struct hwclock_control* ctl, int* valid_p,
                               time_t* systime_p)
{
    struct tm tm;
    int err;
    ...
    if (ctl->verbose)
        printf(
            _("Time read from Hardware Clock: %4d/%.2d/%.2d %02d:%02d:%02d\n"),
 ...
 */
static void set_hardware_clock(const struct hwclock_control* ctl,
...
static double get_hardware_delay(const struct hwclock_control* ctl)
{
 ...
 */
static void set_hardware_clock_exact(const struct hwclock_control* ctl,
                                     ...
                                     const struct timeval refsystime)
{
    ...
    while (1)
    {
        ...
        if (ticksize < 0)
        {
            if (ctl->verbose)
                printf(_("time jumped backward %.6f seconds "
                       ...
                       ticksize, nowsystime.tv_sec, nowsystime.tv_usec);
        ...
        else if (deltavstarget < 0)
        {
            ...
            DBG(DELTA_VS_TARGET,
                ul_debug("%ld.%06ld < %ld.%06ld (%.6f)", nowsystime.tv_sec,
                         nowsystime.tv_usec, targetsystime.tv_sec,
    ...
    newhwtime = sethwtime + (int)(time_diff(nowsystime, refsystime) -
                                  delay /* don't count this */
                                  + 0.5 /* for rounding */);
    if (ctl->verbose)
        printf(_("%ld.%06ld is close enough to %ld.%06ld (%.6f < %.6f)\n"
                 "Set RTC to %ld (%ld + %d; refsystime = %ld.%06ld)\n"),
               nowsystime.tv_sec, nowsystime.tv_usec, targetsystime.tv_sec,
               targetsystime.tv_usec, deltavstarget, target_time_tolerance_secs,
               newhwtime, sethwtime, (int)(newhwtime - sethwtime),
               refsystime.tv_sec, refsystime.tv_usec);

    set_hardware_clock(ctl, newhwtime);
}

static int display_time(struct timeval hwctime)
{
    char buf[ISO_BUFSIZ];
 ...
 */
static int set_system_clock(const struct hwclock_control* ctl,
 ...
 */
static void adjust_drift_factor(const struct hwclock_control* ctl,
                                struct adjtime* adjtime_p,
        ...
        else
        {
            if (ctl->verbose)
                printf(_("Clock drifted %f seconds in the past "
                       ...
                       time_diff(nowtime, last_calib), adjtime_p->drift_factor,
                       factor_adjust);
        }
 ...
 *
 */
static void calculate_adjustment(const struct hwclock_control* ctl,
                                 const double factor, const time_t last_time,
                                 ...
                                 const time_t systime, struct timeval* tdrift_p)
{
    ...
    tdrift_p->tv_usec = (exact_adjustment - (double)tdrift_p->tv_sec) * 1E6;
    if (ctl->verbose)
    {
                        ...
                        const struct adjtime* adjtime)
{
    ...
    xasprintf(&content, "%f %ld %f\n%ld\n%s\n", adjtime->drift_factor,
              adjtime->last_adj_time, adjtime->not_adjusted,
              adjtime->last_calib_time,
              (adjtime->local_utc == LOCAL) ? "LOCAL" : "UTC");

    if (ctl->verbose)
    {
        printf(_("New %s data:\n%s"), ctl->adj_file_name, content);
    }
 ...
 *
 */
static void do_adjustment(const struct hwclock_control* ctl,
                          struct adjtime* adjtime_p,
                          const struct timeval hclocktime,
                          const struct timeval read_time)
{
    ...
    {
        if (ctl->verbose)
            printf(_("Not setting clock because drift factor %f is far too "
                     "high.\n"),
                   adjtime_p->drift_factor);
    }
    else
    {
        set_hardware_clock_exact(
            ctl, hclocktime.tv_sec,
            time_inc(read_time, -(hclocktime.tv_usec / 1E6)));
...
static int manipulate_clock(const struct hwclock_control* ctl,
                            ...
                            struct adjtime* adjtime)
{
    ...
    if (ctl->predict)
    {
        ...
        calculate_adjustment(ctl, adjtime->drift_factor, adjtime->last_adj_time,
                             adjtime->not_adjusted, hclocktime.tv_sec, &tdrift);
        hclocktime = time_inc(hclocktime,
                              (double)-(tdrift.tv_sec + tdrift.tv_usec / 1E6));
        if (ctl->verbose)
        {
    ...
    else if (ctl->systohc)
    {
        ...
        if (!ctl->noadjfile)
            adjust_drift_factor(ctl, adjtime, nowtime, hclocktime);
    }
    else if (ctl->hctosys)
    {
...
static void manipulate_epoch(const struct hwclock_control* ctl)
{
    if (ctl->getepoch)
    {
        unsigned long epoch;
...
static void __attribute__((__noreturn__)) usage(void)
{
    ...
    puts(
        _("     --systz          send timescale configurations to the kernel"));
    puts(_(" -a, --adjust         adjust the RTC to account for systematic "
           "drift"));
#if defined(__linux__) && defined(__alpha__)
    puts(_("     --getepoch       display the RTC epoch"));
    puts(_("     --setepoch       set the RTC epoch according to --epoch"));
#endif
    puts(_("     --predict        predict the drifted RTC time according to "
           "--date"));
    fputs(USAGE_OPTIONS, stdout);
    puts(_(" -u, --utc            the RTC timescale is UTC"));
    puts(_(" -l, --localtime      the RTC timescale is Local"));
#ifdef __linux__
    printf(_(" -f, --rtc <file>     use an alternate file to %1$s\n"),
           _PATH_RTC_DEV);
#endif
    printf(_("     --directisa      use the ISA bus instead of %1$s access\n"),
           _PATH_RTC_DEV);
...
int main(int argc, char** argv)
{
    ...
    static const ul_excl_t excl[] = {
        ...
        {'a', 'r', 's', 'w', OPT_GET, OPT_GETEPOCH, OPT_PREDICT, OPT_SET,
    ...
    while ((c = getopt_long(argc, argv, "hvVDd:alrsuwf:", longopts, NULL)) !=
           -1)
    {
        ...
        switch (c)
        {
            ...
            case OPT_DELAY:
                ctl.rtc_delay =
                    strtod_or_err(optarg, "invalid --delay argument");
                break;
            case OPT_ADJFILE:
