...
static int _unregister_notification_cb(int major, int minor, void* serverarg,
                                       void* clientarg);
...
static int _checkFilter(const char* paramName, netsnmp_pdu* pdu)
{
    /*
    ...
    DEBUGMSGTL(
        ("send_notifications", "checking filters for '%s'...\n", paramName));
    ...
    */
    ...
    profileName =
        get_FilterProfileName(paramName, strlen(paramName), &profileNameLen);
   ...
   the matching entries.
    ...
    {
        trap_var = find_varbind_in_list(pdu->variables, snmptrap_oid,
                                        snmptrap_oid_len);
    }
    ...
    else
    {
        ...
        if ((netsnmp_build_trap_oid(pdu, enterprise, &enterprise_len) !=
            ...
            !snmp_varlist_add_variable(
                &trap_var, snmptrap_oid, snmptrap_oid_len, ASN_OBJECT_ID,
   ...
   included, and the notification may be sent to this management target.
        ...
        vp = netsnmp_view_get(head, profileName, var->name, var->name_length,
                              VACM_MODE_FIND);
...
int notifyTable_register_notifications(int major, int minor, void* serverarg,
                                       void* clientarg)
{
    ...
    pptr->paramNameData =
        netsnmp_memdup_nt(ptr->paramsData, ptr->paramsLen, &pptr->paramNameLen);
    ...
    if (ss->version == SNMP_VERSION_3)
    {
        ...
        pptr->secNameData = netsnmp_memdup_nt(
            ss->securityName, ss->securityNameLen, &pptr->secNameLen);
    ...
    else
    {
        ...
        {
            pptr->secNameData = netsnmp_memdup_nt(
                ss->community, ss->community_len, &pptr->secNameLen);
            if (pptr->secNameData == NULL)
                goto bail;
        }
    }
    ...
    ++_active;
    nptr->snmpNotifyName =
        netsnmp_memdup_nt(name, nameLen, &nptr->snmpNotifyNameLen);
    /** selects target addr */
    nptr->snmpNotifyTag =
        netsnmp_memdup_nt(tag, tagLen, &nptr->snmpNotifyTagLen);
    if (!nptr->snmpNotifyName || !nptr->snmpNotifyTag)
        goto bail;
    nptr->snmpNotifyType =
        confirm ? SNMPNOTIFYTYPE_INFORM : SNMPNOTIFYTYPE_TRAP;
    ...
    if (NULL != notifyProfile)
    {
        ...
        profile = snmpNotifyFilterProfileTable_create(
            ptr->paramsData, ptr->paramsLen, notifyProfile, notifyProfileLen);
        if (NULL == profile)
        {
 ...
 */
int notifyTable_unregister_all_notifications(int major, int minor,
...
void shutdown_snmpNotifyTable_data(void)
{
    ...
    snmp_unregister_callback(
        ...
        notifyTable_unregister_all_notifications, NULL, FALSE);
    ...
    if (_active != 0)
    {
        ...
        snmp_log(LOG_WARNING, "notify count %d, not 0, after shutdown.\n",
                 _active);
    }
...
int snmpNotifyTable_add(struct snmpNotifyTable_data* thedata)
{
    ...
    snmp_varlist_add_variable(&vars, NULL, 0, ASN_PRIV_IMPLIED_OCTET_STR,
                              (u_char*)thedata->snmpNotifyName,
                              thedata->snmpNotifyNameLen); /* snmpNotifyName */

    if (header_complex_maybe_add_data(&snmpNotifyTableStorage, vars, thedata,
                                      1) != NULL)
    {
    ...
    snmpNotifyTable_extract(struct snmpNotifyTable_data* thedata)
{
    ...
    return header_complex_extract_entry(
        (struct header_complex_index**)&snmpNotifyTableStorage, hptr);
}
...
struct snmpNotifyTable_data* get_notifyTable2(const char* name, size_t nameLen)
{
    struct header_complex_index* hptr;
                                                  ...
                                                  WriteMethod** write_method)
{
    struct snmpNotifyTable_data* result =
        header_complex((struct header_complex_index*)snmpNotifyTableStorage, vp,
                       name, len, exact, var_len, write_method);
    return result;
}

void snmpNotifyTable_unregister_notification(const char* name,
                          ...
                          void* clientarg)
{
         ...
         hcindex = hcindex->next)
    {
            ...
            (StorageTmp->snmpNotifyStorageType == ST_PERMANENT))
        {
            ...
            cptr = read_config_store_data(
                ASN_INTEGER, cptr, &StorageTmp->snmpNotifyStorageType, &tmpint);
            cptr = read_config_store_data(
                ASN_INTEGER, cptr, &StorageTmp->snmpNotifyRowStatus, &tmpint);
