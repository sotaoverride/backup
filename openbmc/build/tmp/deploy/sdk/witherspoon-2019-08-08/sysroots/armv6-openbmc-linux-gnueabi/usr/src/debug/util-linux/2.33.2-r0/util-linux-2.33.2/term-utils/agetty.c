...
int main(int argc, char** argv)
{
    ...
    { /* regular (auto)login */
        ...
        else
        {
            ...
            while ((username = get_logname(&issue, &options, &termios,
                                           &chardata)) == NULL)
...
static void termio_init(struct options* op, struct termios* tp)
{
    ...
    struct termios lock;
    int i =
        (plymouth_command(MAGIC_PING) == 0) ? PLYMOUTH_TERMIOS_FLAGS_DELAY : 0;
    if (i)
...
static FILE* issuedir_next_file(int dd, struct dirent** namelist, int nfiles,
                                int* n)
{
    while (*n < nfiles)
    {
                            ...
                            struct termios* tp)
{
    ...
    if (dirname)
    {
        ...
        if (dd >= 0)
            nfiles =
                scandirat(dd, ".", &namelist, issuedir_filter, versionsort);
        if (nfiles <= 0)
            dirname = NULL;
    }
    i = 0;
