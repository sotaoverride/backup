...
static void exec_dbus_daemon(const char* dbus_daemon, int bus_address_pipe[2],
...
#define MAX_FD_LEN 64
    ...
    execlp(dbus_daemon, dbus_daemon, "--nofork", "--print-address",
...
int main(int argc, char** argv)
{
    ...
    while (i < argc)
    {
        ...
        if (requires_arg)
        {
            ...
            else
            {
                /* shouldn't happen */
                fprintf(stderr,
                        "%s: internal error: %s not fully implemented\n", me,
                        prev_arg);
                return 127;
            }
    ...
    switch (
        read_line(bus_address_pipe[PIPE_READ_END], bus_address, MAX_ADDR_LEN))
    {
        case READ_STATUS_OK:
            break;
