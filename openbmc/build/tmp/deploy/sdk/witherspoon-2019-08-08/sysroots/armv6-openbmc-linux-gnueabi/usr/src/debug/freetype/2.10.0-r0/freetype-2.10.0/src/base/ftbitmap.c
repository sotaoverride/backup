                  ...
                  FT_Bitmap* target, FT_Int alignment)
{
    ...
    switch (source->pixel_mode)
    {
        ...
        case FT_PIXEL_MODE_BGRA:
        {
            ...
            if (FT_QREALLOC(target->buffer, old_size,
                            target->rows * (FT_UInt)target_pitch))
                return error;
    ...
    switch (source->pixel_mode)
    {
        case FT_PIXEL_MODE_MONO:
        {
            ...
            for (i = source->rows; i > 0; i--)
            {
                ...
                {
                    FT_Int val =
                        ss[0]; /* avoid a byte->int cast on each line */
...
FT_EXPORT_DEF(FT_Error)
FT_Bitmap_Blend(FT_Library library, const FT_Bitmap* source_,
                const FT_Vector source_offset_, FT_Bitmap* target,
                FT_Vector* atarget_offset, FT_Color color)
{
    ...
    if (!(target->pixel_mode == FT_PIXEL_MODE_NONE ||
          (target->pixel_mode == FT_PIXEL_MODE_BGRA && target->buffer)))
        return FT_THROW(Invalid_Argument);
    ...
    FT_TRACE5(("FT_Bitmap_Blend:\n"
               ...
               source_llx / 64, source_lly / 64, source_urx / 64,
               source_ury / 64, source_->width, source_->rows));

    if (target->width && target->rows)
        FT_TRACE5(("  target bitmap: (%d, %d) -- (%d, %d); %d x %d\n",
                   target_llx / 64, target_lly / 64, target_urx / 64,
                   target_ury / 64, target->width, target->rows));
    else
        FT_TRACE5(("  target bitmap: empty\n"));

    FT_TRACE5(("  final bitmap: (%d, %d) -- (%d, %d); %d x %d\n",
               final_llx / 64, final_lly / 64, final_urx / 64, final_ury / 64,
    ...
    else if (target->width != final_width || target->rows != final_rows)
    {
        ...
        else
        {
            ...
            unsigned char* q =
                buffer + (final_rows - y - target->rows) * new_pitch + x * 4;
