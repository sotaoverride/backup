...
static int generate_path(char** var, char** filenames)
{
    ...
    /* First, prepend our directories. Second, if some path was specified, use
     * that, and otherwise use the defaults. Any duplicates will be filtered out
     * in path-lookup.c. Treat explicit empty path to mean that nothing should
     * be appended.
     */
...
static int verify_socket(Unit* u)
{
    ...
    if (r < 0)
        return log_unit_error_errno(
            u, r, "Socket cannot be started, failed to create instance: %m");
    ...
    if (UNIT_ISSET(SOCKET(u)->service))
    {
        ...
        {
            log_unit_error(u, "Service %s not loaded, %s cannot be started.",
...
static int verify_executable(Unit* u, ExecCommand* exec)
{
    ...
    if (access(exec->path, X_OK) < 0)
        return log_unit_error_errno(
            u, errno, "Command %s is not executable: %m", exec->path);
    ...
    STRV_FOREACH(p, u->documentation)
    {
        ...
        if (check_man && startswith(*p, "man:"))
        {
            ...
            if (k != 0)
            {
                ...
                {
                    log_unit_error(u, "Command 'man %s' failed with code %d",
                                   *p + 4, k);
                    k = -ENOEXEC;
                }
                if (r == 0)
...
static int verify_unit(Unit* u, bool check_man)
{
    ...
    r = manager_add_job(u->manager, JOB_START, u, JOB_REPLACE, NULL, &err,
                        NULL);
    if (r < 0)
        log_unit_error_errno(u, r, "Failed to create %s/start: %s", u->id,
                 ...
                 bool run_generators)
{
    const ManagerTestRunFlags flags =
        MANAGER_TEST_RUN_BASIC | MANAGER_TEST_RUN_ENV_GENERATORS |
    ...
    STRV_FOREACH(filename, filenames)
    {
        ...
        k = manager_load_startable_unit_or_warn(m, NULL, prepared,
                                                &units[count]);
        if (k < 0 && r == 0)
            r = k;
        else
