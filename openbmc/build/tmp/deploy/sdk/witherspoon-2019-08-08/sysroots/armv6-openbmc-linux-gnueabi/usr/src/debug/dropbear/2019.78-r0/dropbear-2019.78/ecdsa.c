...
ecc_key* buf_get_ecdsa_pub_key(buffer* buf)
{
    ...
    if (memcmp(&key_ident[strlen("ecdsa-sha2-")], identifier, identifier_len) !=
        0)
    {
    ...
    {
        if (memcmp(identifier, (char*)(*curve)->name,
                   strlen((char*)(*curve)->name)) == 0)
        {
            break;
        }
    }
...
void buf_put_ecdsa_sign(buffer* buf, const ecc_key* key, const buffer* data_buf)
{
    ...
    for (;;)
    {
        ...
        if (ecc_make_key_ex(NULL, dropbear_ltc_prng, &R_key, key->dp) !=
            CRYPT_OK)
        {
            goto out;
        }
...
static int buf_get_ecdsa_verify_params(buffer* buf, void* r, void* s)
{
...
int buf_ecdsa_verify(buffer* buf, const ecc_key* key, const buffer* data_buf)
{
    ...
    if (ltc_init_multi(&r, &s, &v, &w, &u1, &u2, &p, &e, &m, NULL) !=
            CRYPT_OK ||
        !mG || !mQ)
    {
        dropbear_exit("ECC error");
    ...
    if (ltc_mp.compare_d(r, 0) == LTC_MP_EQ ||
        ...
        ltc_mp.compare(r, p) != LTC_MP_LT || ltc_mp.compare(s, p) != LTC_MP_LT)
    {
        goto out;
    }
