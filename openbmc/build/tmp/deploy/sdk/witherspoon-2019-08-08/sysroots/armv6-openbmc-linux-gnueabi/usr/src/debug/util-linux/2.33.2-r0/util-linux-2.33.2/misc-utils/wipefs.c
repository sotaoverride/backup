...
struct wipe_desc
{
    ...
    unsigned int on_disk : 1, is_parttable : 1;
};
...
struct wipe_control
{
    ...
    unsigned int noact : 1, all : 1, quiet : 1, backup : 1, force : 1, json : 1,
        no_headings : 1, parsable : 1;
};
...
static blkid_probe new_probe(const char* devname, int mode)
{
    blkid_probe pr = NULL;
    ...
    blkid_probe_set_superblocks_flags(
        pr, BLKID_SUBLKS_MAGIC |       /* return magic string and offset */
    ...
    blkid_probe_set_partitions_flags(pr,
                                     BLKID_PARTS_MAGIC | BLKID_PARTS_FORCE_GPT);
    return pr;
error:
    blkid_free_probe(pr);
                         ...
                         struct wipe_desc* w)
{
    ...
    printf(P_("%s: %zd byte was erased at offset 0x%08jx (%s): ",
              "%s: %zd bytes were erased at offset 0x%08jx (%s): ", w->len),
...
static int do_wipe(struct wipe_control* ctl)
{
    ...
    while (blkid_do_probe(pr) == 0)
    {
        ...
        if (!ctl->force && wp->is_parttable && !blkid_probe_is_wholedisk(pr))
        {
            warnx(_("%s: ignoring nested \"%s\" partition table "
                    "on non-whole disk device"),
                  ctl->devname, wp->type);
            need_force = 1;
            goto done;
        }
    ...
    for (w = ctl->offsets; w; w = w->next)
    {
        if (!w->on_disk && !ctl->quiet)
            warnx(_("%s: offset 0x%jx not found"), ctl->devname,
                  (uintmax_t)w->offset);
    }
...
static void __attribute__((__noreturn__)) usage(void)
{
    ...
    puts(
        _(" -n, --no-act        do everything except the actual write() call"));
    puts(_(" -o, --offset <num>  offset to erase, in bytes"));
    puts(_(" -O, --output <list> COLUMNS to display (see below)"));
    puts(_(" -p, --parsable      print out in parsable instead of printable "
           "format"));
    puts(_(" -q, --quiet         suppress output messages"));
    puts(_(" -t, --types <list>  limit the set of filesystem, RAIDs or "
           "partition tables"));
...
int main(int argc, char** argv)
{
    ...
    while ((c = getopt_long(argc, argv, "abfhiJnO:o:pqt:V", longopts, NULL)) !=
           -1)
    {
