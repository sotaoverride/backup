                             ...
                             const unsigned char* from, int flen, int num)
{
    ...
    for (i = 2; i < num; i++)
    {
        ...
        zero_index =
            constant_time_select_int(~found_zero_byte & equals0, i, zero_index);
    ...
    err =
        constant_time_select_int(mask | good, err, RSA_R_SSLV3_ROLLBACK_ATTACK);
    mask = ~good;
     ...
     */
    tlen = constant_time_select_int(constant_time_lt(num - 11, tlen), num - 11,
                                    tlen);
