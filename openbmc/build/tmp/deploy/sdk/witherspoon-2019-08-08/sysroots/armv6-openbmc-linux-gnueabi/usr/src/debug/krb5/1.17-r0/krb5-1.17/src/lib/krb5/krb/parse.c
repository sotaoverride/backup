 ...
 */
static krb5_error_code allocate_princ(krb5_context context, const char* name,
                                      krb5_boolean enterprise,
                                      krb5_principal* princ_out,
                                      krb5_boolean* has_realm_out)
{
    krb5_error_code ret;
    const char* p;
 ...
 */
static void parse_name_into_princ(const char* name, krb5_boolean enterprise,
                          ...
                          krb5_principal* principal_out)
{
    ...
    *principal_out = NULL;
    ...
    if (!has_realm)
    {
        ...
        {
            ret = KRB5_PARSE_MALFORMED;
            k5_setmsg(context, ret, _("Principal %s is missing required realm"),
                      name);
            goto cleanup;
        }
    ...
    princ->type =
        (enterprise) ? KRB5_NT_ENTERPRISE_PRINCIPAL : KRB5_NT_PRINCIPAL;
