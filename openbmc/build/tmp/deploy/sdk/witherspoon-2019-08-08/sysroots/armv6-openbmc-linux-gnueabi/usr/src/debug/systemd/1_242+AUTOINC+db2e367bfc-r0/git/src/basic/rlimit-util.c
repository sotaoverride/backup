...
int setrlimit_closest_all(const struct rlimit* const* rlim, int* which_failed)
{
    ...
    /* On failure returns the limit's index that failed in *which_failed, but
     * only if non-NULL */
...
static int rlimit_parse_nice(const char* val, rlim_t* ret)
{
    ...
    /* So, Linux is weird. The range for RLIMIT_NICE is 40..1, mapping to the
     * nice levels -20..19. However, the RLIMIT_NICE limit defaults to 0 by the
     * kernel, i.e. a value that maps to nice level 20, which of course is bogus
     * and does not exist. In order to permit parsing the RLIMIT_NICE of 0 here
     * we hence implement a slight asymmetry: when parsing as positive nice
     * level we permit 0..19. When parsing as negative nice level, we permit
     * -20..0. But when parsing as raw resource limit value then we also allow
     * the special value 0.
     *
...
static int (*const rlimit_parse_table[_RLIMIT_MAX])(const char* val,
                                                    rlim_t* ret) = {
...
int rlimit_nofile_bump(int limit)
{
    ...
    /* Bumps the (soft) RLIMIT_NOFILE resource limit as close as possible to the
     * specified limit. If a negative limit is specified, bumps it to the
     * maximum the kernel and the hard resource limit allows. This call should
     * be used by all our programs that might need a lot of fds, and that know
     * how to deal with high fd numbers
...
int rlimit_nofile_safe(void)
{
    ...
    /* Resets RLIMIT_NOFILE's soft limit FD_SETSIZE (i.e. 1024), for
     * compatibility with software still using select() */
    ...
    if (setrlimit(RLIMIT_NOFILE, &rl) < 0)
        return log_debug_errno(
            errno,
            "Failed to lower RLIMIT_NOFILE's soft limit to " RLIM_FMT ": %m",
            rl.rlim_cur);
