...
#define DISPATCH_FILE_NULL(_x)                                                 \
    {                                                                          \
        .ready_link = C_LIST_INIT((_x).ready_link), .fd = -1,                  \
    }

int dispatch_file_init(DispatchFile* file, DispatchContext* ctx, DispatchFn fn,
                       int fd, uint32_t mask, uint32_t events);
...
#define DISPATCH_CONTEXT_NULL(_x)                                              \
    {                                                                          \
        .ready_list = C_LIST_INIT((_x).ready_list), .epoll_fd = -1,            \
    }
