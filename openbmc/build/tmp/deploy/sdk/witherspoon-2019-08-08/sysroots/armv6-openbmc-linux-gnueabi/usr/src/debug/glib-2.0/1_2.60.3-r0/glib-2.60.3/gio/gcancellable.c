...
static void g_cancellable_class_init(GCancellableClass* klass)
{
    ...
    signals[CANCELLED] = g_signal_new(
        ...
        G_STRUCT_OFFSET(GCancellableClass, cancelled), NULL, NULL,
...
void g_cancellable_cancel(GCancellable* cancellable)
{
    ...
    if (cancellable == NULL || cancellable->priv->cancelled)
        return;
 ...
 * Since: 2.22
 */
gulong g_cancellable_connect(GCancellable* cancellable, GCallback callback,
                             gpointer data, GDestroyNotify data_destroy_func)
{
    ...
    {
        id = g_signal_connect_data(cancellable, "cancelled", callback, data,
                                   (GClosureNotify)data_destroy_func, 0);
 ...
 */
static void cancellable_source_cancelled(GCancellable* cancellable,
                                         gpointer user_data)
{
...
static gboolean cancellable_source_dispatch(GSource* source,
...
static gboolean cancellable_source_closure_callback(GCancellable* cancellable,
                                                    gpointer data)
{
...
GSource* g_cancellable_source_new(GCancellable* cancellable)
{
    ...
    source =
        g_source_new(&cancellable_source_funcs, sizeof(GCancellableSource));
    ...
    if (cancellable)
    {
        ...
        cancellable_source->cancelled_handler =
            g_signal_connect(cancellable, "cancelled",
                             G_CALLBACK(cancellable_source_cancelled), source);
        if (g_cancellable_is_cancelled(cancellable))
            g_source_set_ready_time(source, 0);
    }
