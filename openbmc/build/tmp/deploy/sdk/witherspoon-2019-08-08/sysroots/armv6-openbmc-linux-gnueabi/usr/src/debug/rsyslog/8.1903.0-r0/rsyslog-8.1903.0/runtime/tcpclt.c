    ...
    CreateSocket(struct addrinfo* addrDest)
{
    ...
    while (r != NULL)
    {
        ...
        if (fd != -1)
        {
             ...
             * coupled via a memory buffer. Now, I think, we can afford the
             * extra wait time. Thus, we enable blocking mode for TCP if we
             * compile with pthreads. -- rgerhards, 2005-10-25 And now, we
             * always run on multiple threads... -- rgerhards, 2007-12-20
             */
 ...
 */
static rsRetVal TCPSendBldFrame(tcpclt_t* pThis, char** pmsg, size_t* plen,
                                int* pbMustBeFreed)
{
    DEFiRet;
    ...
    len = *plen;
    bIsCompressed =
        *msg == 'z'; /* cache this, so that we can modify the message buffer */
    /* select framing for this record. If we have a compressed record, we always
     * need to use octet counting because the data potentially contains all
     * control characters including LF.
     */
    framingToUse =
        bIsCompressed ? TCP_FRAMING_OCTET_COUNTING : pThis->tcp_framing;
    ...
    else
    {
        ...
        if ((buf = malloc(len + iLenBuf)) == NULL)
        {
             ...
             * that, there is nothing left to do. We can not sent the message
             * (as in case of the other framing, because the message is
             * incomplete. We could, however, send two chunks (header and text
             * separate), but that would cause a lot of complexity in the code.
             * So we think it is appropriate enough to just make sure we do not
             * crash in this very unlikely case. For this, it is justified just
             * to loose the message. Rgerhards, 2006-12-07
             */
...
static int Send(tcpclt_t* pThis, void* pData, char* msg, size_t len)
{
    ...
    int retry = 0;
    int bMsgMustBeFreed =
        0; /* must msg be freed at end of function? 0 - no, 1 - yes */
    ...
    if (pThis->iRebindInterval > 0 &&
        ++pThis->iNumMsgs == pThis->iRebindInterval)
    {
    ...
    while (!bDone)
    { /* loop is broken when send succeeds or error occurs */
            ...
            iRet == RS_RET_PREVIOUS_COMMITTED)
        {
             ...
             * message was succesfully received (it's not always the case, but
             * its at least our best shot at it -- rgerhards, 2008-03-12 As of
             * 2008-06-09, we have implemented an algorithm which detects
             * connection loss quite good in some (common) scenarios. Thus, the
             * probability of message duplication due to the code below has
             * increased. We so now have a config setting, default off, that
             * enables the user to request retransmits. However, if not
             * requested, we do NOT need to do all the stuff needed for it.
             */
            if (pThis->bResendLastOnRecon == 1)
            {
                if (pThis->prevMsg != NULL)
                    free(pThis->prevMsg);
                /* if we can not alloc a new buffer, we silently ignore it. The
                 * worst that happens is that we lose our message recovery
                 * buffer - anything else would be worse, so don't try anything
        ...
        else
        {
            ...
            { /* OK, one retry */
                ++retry;
                CHKiRet(pThis->prepRetryFunc(pData)); /* try to recover */
                /* now try to send our stored previous message (which most
                 * probably didn't make it. Note that if bResendLastOnRecon is
                 * 0, prevMsg will never become non-NULL, so the check below
                 * covers all cases.
                 */
                if (pThis->prevMsg != NULL)
                {
                    CHKiRet(pThis->initFunc(pData));
                    CHKiRet(pThis->sendFunc(pData, pThis->prevMsg,
                                            pThis->lenPrevMsg));
                }
            }
            else
...
/* set functions */
static rsRetVal SetResendLastOnRecon(tcpclt_t* pThis, int bResendLastOnRecon)
{
    DEFiRet;
...
static rsRetVal SetSendFrame(tcpclt_t* pThis,
                             rsRetVal (*pCB)(void*, char*, size_t))
{
    DEFiRet;
    pThis->sendFunc = pCB;
    RETiRet;
...
static rsRetVal SetRebindInterval(tcpclt_t* pThis, int iRebindInterval)
{
    DEFiRet;
 ...
 */
BEGINobjConstruct(
    tcpclt) /* be sure to specify the object type also in END macro! */
...
BEGINobjDestruct(tcpclt) /* be sure to specify the object type also in END and
                            CODESTART macros! */
...
ENDobjDestruct(tcpclt)
    ...
    BEGINobjQueryInterface(tcpclt)
        CODESTARTobjQueryInterface(tcpclt) if (pIf->ifVersion !=
                                               tcpcltCURR_IF_VERSION)
...
finalize_it
    : ENDobjQueryInterface(tcpclt)
       ...
       */
      BEGINObjClassExit(
          tcpclt, OBJ_IS_LOADABLE_MODULE) /* CHANGE class also in END MACRO! */
       ...
       */
      BEGINObjClassInit(tcpclt, 1,
                        OBJ_IS_LOADABLE_MODULE) /* class, version - CHANGE class
                                                   also in END MACRO! */
      /* request objects we use */

      /* set our own handlers */
      OBJSetMethodHandler(objMethod_CONSTRUCTION_FINALIZER,
                          tcpcltConstructFinalize);
...
CHKiRet(
    tcpcltClassInit(pModInfo)); /* must be done after tcps_sess, as we use it */
ENDmodInit
