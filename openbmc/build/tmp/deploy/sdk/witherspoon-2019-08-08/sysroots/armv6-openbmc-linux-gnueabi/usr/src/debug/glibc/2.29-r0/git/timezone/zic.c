...
static struct lookup const leap_line_codes[] = {{"Leap", LC_LEAP}, {NULL, 0}};
...
static struct lookup const wday_names[] = {
    {"Sunday", TM_SUNDAY},     {"Monday", TM_MONDAY},
...
static struct lookup const end_years[] = {{"minimum", YR_MINIMUM},
                                          {"maximum", YR_MAXIMUM},
                                          {"only", YR_ONLY},
                                          {NULL, 0}};

static struct lookup const leap_types[] = {
    {"Rolling", true}, {"Stationary", false}, {NULL, 0}};
...
static ATTRIBUTE_PURE size_t align_to(size_t size, size_t alignment)
{
...
static char* ATTRIBUTE_MALLOC ecpyalloc(char const* str)
{
    return memcheck(strdup(str));
}

static void* growalloc(void* ptr, size_t itemsize, ptrdiff_t nitems,
    ...
    verror(const char* const string, va_list args)
{
    ...
    */
    ...
    if (rfilename != NULL)
        fprintf(stderr, _(" (rule from \"%s\", line %" PRIdMAX ")"), rfilename,
                rlinenum);
    fprintf(stderr, "\n");
}
...
{
    char const* e =
        (ferror(stream) ? _("I/O error")
                        : fclose(stream) != 0 ? strerror(errno) : NULL);
    if (e)
    {
        fprintf(stderr, "%s: %s%s%s%s%s\n", progname, dir ? dir : "",
                dir ? "/" : "", name ? name : "", name ? ": " : "", e);
        exit(EXIT_FAILURE);
    }
}
                           ...
                           char const* component_end)
{
    ...
    if (0 < component_len && component_len <= 2 && component[0] == '.' &&
        component_end[-1] == '.')
    {
        int len = component_len;
        error(_("file name '%s' contains '%.*s' component"), name, len,
              component);
        return false;
    }
    if (noise)
...
static void dolink(char const* fromfield, char const* tofield, bool staysymlink)
{
    ...
    */
    if (itsdir(fromfield))
    {
        fprintf(stderr, _("%s: link from %s/%s failed: %s\n"), progname,
                directory, fromfield, strerror(EPERM));
        exit(EXIT_FAILURE);
    }
    if (staysymlink)
...
static zic_t gethms(char const* string, char const* errstring)
{
    ...
    if (*string == '-')
    {
        ...
        sign = 1;
    switch (sscanf(string, "%" SCNdZIC "%c%d%c%d%c%1d%*[0]%c%*[0123456789]%c",
    ...
    if (noise &&
        (hh > HOURSPERDAY || (hh == HOURSPERDAY && (mm != 0 || ss != 0))))
        warning(
            _("values over 24 hours not handled by pre-2007 versions of zic"));
    return oadd(sign * hh * SECSPERHOUR, sign * (mm * SECSPERMIN + ss));
}

static zic_t getstdoff(char* field, bool* isdst)
{
    int dst = -1;
    zic_t stdoff;
...
static bool inzsub(char** fields, int nfields, bool iscont)
{
    ...
    if (hasuntil)
    {
        ...
        if (iscont && nzones > 0 && z.z_untiltime > min_time &&
            z.z_untiltime < max_time &&
            zones[nzones - 1].z_untiltime > min_time &&
            zones[nzones - 1].z_untiltime < max_time &&
            zones[nzones - 1].z_untiltime >= z.z_untiltime)
        {
            error(_("Zone continuation line end time is not after end time of "
                    "previous line"));
            return false;
        }
    }
...
static void inleap(char** fields, int nfields)
{
    ...
    cp = fields[LP_DAY];
    if (sscanf(cp, "%d%c", &day, &xs) != 1 || day <= 0 ||
        day > len_months[isleap(year)][month])
    {
                    ...
                    const char* timep)
{
    ...
    */
    ...
    else
    {
        ...
        warning(_("year type \"%s\" is obsolete; use \"-\" instead"), typep);
...
static void swaptypes(int i, int j)
{
    {
        zic_t t = gmtoffs[i];
        gmtoffs[i] = gmtoffs[j];
                      ...
                      char version, int defaulttype)
{
    ...
    zic_t* ats =
        emalloc(align_to(size_product(nats, sizeof *ats + 1), _Alignof(zic_t)));
        ...
        for (; fromi < timecnt; ++fromi)
        {
            if (toi != 0 &&
                ((attypes[fromi].at + gmtoffs[attypes[toi - 1].type]) <=
                 (attypes[toi - 1].at +
                  gmtoffs[toi == 1 ? 0 : attypes[toi - 2].type])))
            {
    ...
    */
    ...
    if (!fp)
    {
        ...
        {
            fprintf(stderr, _("%s: Can't create %s/%s: %s\n"), progname,
                    directory, name, strerror(fopen_errno));
            exit(EXIT_FAILURE);
        }
    }
        ...
        */
        {
            ...
            {
                isdsts[mrudst] = -1;
                type = addtype(gmtoffs[mrudst], &chars[abbrinds[mrudst]], true,
                               ttisstds[mrudst], ttisgmts[mrudst]);
                isdsts[mrudst] = 1;
                omittype[type] = false;
            }
            ...
            {
                isdsts[mrustd] = -1;
                type = addtype(gmtoffs[mrustd], &chars[abbrinds[mrustd]], false,
                               ttisstds[mrustd], ttisgmts[mrustd]);
                isdsts[mrustd] = 0;
                omittype[type] = false;
            }
        }
        ...
        for (i = old0; i < typecnt; i++)
            if (!omittype[i])
                typemap[i == old0 ? defaulttype : i == defaulttype ? old0 : i] =
                    thistypecnt++;
...
#undef DO
        for (i = thistimei; i < thistimelim; ++i)
            if (pass == 1)
                ...
                */
                puttzcode(((ats[i] < INT32_MIN) ? INT32_MIN : ats[i]), fp);
            else
...
static int stringrule(char* result, const struct rule* const rp,
                      const zic_t dstoff, const zic_t gmtoff)
{
...
static int stringzone(char* result, struct zone const* zpfirst,
                      ptrdiff_t zonecount)
{
    ...
    if (dstrp == NULL)
        return compat;
    len += doabbr(result + len, zp, dstrp->r_abbrvar, dstrp->r_isdst,
                  dstrp->r_stdoff, true);
    if (dstrp->r_stdoff != SECSPERMIN * MINSPERHOUR)
    {
        offsetlen =
            stringoffset(result + len, -(zp->z_gmtoff + dstrp->r_stdoff));
        if (!offsetlen)
        {
            result[0] = '\0';
            return -1;
...
static void outzone(const struct zone* zpfirst, ptrdiff_t zonecount)
{
    ...
    */
    ...
    {
        if (!*envvar)
            warning("%s %s", _("no POSIX environment variable for zone"),
                    zpfirst->z_name);
    ...
    for (i = 0; i < zonecount; ++i)
    {
        ...
        *startbuf = '\0';
        ...
        if (zp->z_nrules == 0)
        {
            ...
            type = addtype(oadd(zp->z_gmtoff, stdoff), startbuf, zp->z_isdst,
                ...
                */
                for (j = 0; j < zp->z_nrules; ++j)
                {
                    ...
                    if (rp->r_todo)
                    {
                        ...
                        rp->r_todo =
                            (rp->r_temp < y2038_boundary || year <= max_year0);
                    }
                }
                for (;;)
                {
                    ...
                    */
                    ...
                    if (usestart)
                    {
                        if (ktime < starttime)
                        {
                            ...
                            doabbr(startbuf, zp, rp->r_abbrvar, rp->r_isdst,
                                   rp->r_stdoff, false);
                            continue;
                        }
                        if (*startbuf == '\0' &&
                    ...
                    eats(zp->z_filename, zp->z_linenum, rp->r_filename,
                         rp->r_linenum);
                    doabbr(ab, zp, rp->r_abbrvar, rp->r_isdst, rp->r_stdoff,
                           false);
                    offset = oadd(zp->z_gmtoff, rp->r_stdoff);
                    type = addtype(offset, ab, rp->r_isdst, rp->r_todisstd,
                                   rp->r_todisgmt);
                    if (defaulttype < 0 && !rp->r_isdst)
                        defaulttype = type;
                    if (rp->r_hiyear == ZIC_MAX &&
                        !(0 <= lastatmax && ktime < attypes[lastatmax].at))
                        lastatmax = timecnt;
                    addtt(ktime, type);
                }
            }
        if (usestart)
        {
            if (*startbuf == '\0' && zp->z_format != NULL &&
                strchr(zp->z_format, '%') == NULL &&
                strchr(zp->z_format, '/') == NULL)
                strcpy(startbuf, zp->z_format);
            eat(zp->z_filename, zp->z_linenum);
            if (*startbuf == '\0')
                error(_("can't determine time zone abbreviation to use just "
                        "after until time"));
            else
            {
                bool isdst = startoff != zp->z_gmtoff;
                type = addtype(startoff, startbuf, isdst, startttisstd,
                               startttisgmt);
...
static int addtype(zic_t gmtoff, char const* abbr, bool isdst, bool ttisstd,
                   bool ttisgmt)
{
    register int i, j;
    ...
    */
    for (i = 0; i < typecnt; ++i)
    {
        if (gmtoff == gmtoffs[i] && isdst == isdsts[i] &&
            strcmp(abbr, &chars[abbrinds[i]]) == 0 && ttisstd == ttisstds[i] &&
            ttisgmt == ttisgmts[i])
            return i;
    }
    /*
...
static bool yearistype(zic_t year, const char* type)
{
    ...
    if (type == NULL || *type == '\0')
        return true;
    buf = emalloc(1 + 4 * strlen(yitcommand) + 2 + INT_STRLEN_MAXIMUM(zic_t) +
                  2 + 4 * strlen(type) + 2);
...
static ATTRIBUTE_PURE bool ciprefix(char const* abbr, char const* word)
{
    do
        if (!*abbr)
            return true;
...
static const struct lookup* byword(const char* word, const struct lookup* table)
{
    ...
    {
        if (word[4] == '-')
            warning(_("\"%s\" is undocumented; use \"last%s\" instead"), word,
                    word + 5);
        else
        {
            word += 4;
...
static ATTRIBUTE_PURE zic_t oadd(zic_t t1, zic_t t2)
{
...
static ATTRIBUTE_PURE zic_t tadd(zic_t t1, zic_t t2)
{
    if (t1 < 0)
    {
        if (t2 < min_time - t1)
...
static void newabbr(const char* string)
{
    ...
    if (strcmp(string, GRANDPARENTED) != 0)
    {
        ...
        cp = string;
        mp = NULL;
        while (is_alpha(*cp) || ('0' <= *cp && *cp <= '9') || *cp == '-' ||
               *cp == '+')
            ++cp;
    ...
    while (cp && ((cp = strchr(cp, '/')) || !ancestors))
    {
        ...
        */
        if (mkdir(name, MKDIR_UMASK) != 0)
        {
            ...
            {
                error(_("%s: Can't create directory %s: %s"), progname, name,
                      strerror(err));
                exit(EXIT_FAILURE);
            }
        }
