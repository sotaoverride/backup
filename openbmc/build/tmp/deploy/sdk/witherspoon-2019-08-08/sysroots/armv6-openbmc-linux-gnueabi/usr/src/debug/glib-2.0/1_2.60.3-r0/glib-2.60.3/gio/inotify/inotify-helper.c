...
static gboolean ih_event_callback(ik_event_t* event, inotify_sub* sub,
                                  gboolean file_event);
    ...
    if (event->mask & IN_MOVE)
    {
        ...
        if (event->pair && event->pair->wd == event->wd)
        {
            ...
            interesting = g_file_monitor_source_handle_event(
                sub->user_data, G_FILE_MONITOR_EVENT_RENAMED, event->name,
                event->pair->name, NULL, event->timestamp);
        }
        else
        {
            GFile* other;
            ...
            if (event->pair)
            {
                ...
                fullpath =
                    _ih_fullpath_from_event(event->pair, parent_dir, NULL);
             ...
             * event->mask above, it should have converted to a
             * #GFileMonitorEvent properly. If not, the assumption we have made
             * about event->mask only ever having a single bit set (apart from
             * IN_ISDIR) is false. The kernel documentation is lacking here. */
            g_assert(event_flags != -1);
            interesting = g_file_monitor_source_handle_event(
                sub->user_data, event_flags, event->name, NULL, other,
                event->timestamp);
        ...
        interesting = g_file_monitor_source_handle_event(
            sub->user_data, event_flags, event->name, NULL, NULL,
            event->timestamp);
    else
        interesting = FALSE;
    ...
    if (event->mask & IN_CREATE)
    {
        ...
        if (s != 0 || !S_ISREG(buf.st_mode) || buf.st_nlink != 1)
            g_file_monitor_source_handle_event(
                sub->user_data, G_FILE_MONITOR_EVENT_CHANGES_DONE_HINT,
...
static void ih_not_missing_callback(inotify_sub* sub)
{
    ...
    g_file_monitor_source_handle_event(sub->user_data,
