...
static int decrypt_key_data(krb5_context context, int n_key_data,
                            krb5_key_data* key_data, krb5_keyblock** keyblocks,
                            int* n_keys);
...
static krb5_boolean ks_tuple_present(int n_ks_tuple,
...
static kadm5_ret_t get_policy(kadm5_server_handle_t handle, const char* name,
                              kadm5_policy_ent_t policy_out,
                              krb5_boolean* have_pol_out)
{
    kadm5_ret_t ret;
 ...
 */
static kadm5_ret_t apply_keysalt_policy(kadm5_server_handle_t handle,
                                        const char* policy, int n_ks_tuple,
                                        krb5_key_salt_tuple* ks_tuple,
                                        int* new_n_kstp,
                                        krb5_key_salt_tuple** new_kstp)
{
    kadm5_ret_t ret;
 ...
 */
static void check_1_6_dummy(kadm5_principal_ent_t entry, long mask,
...
kadm5_ret_t kadm5_create_principal(void* server_handle,
                                   ...
                                   char* password)
{
    return kadm5_create_principal_3(server_handle, entry, mask, 0, NULL,
                                    password);
}
                                     ...
                                     char* password)
{
    ...
    else if (password)
    {
        ret = krb5_dbe_cpw(handle->context, act_mkey, new_ks_tuple,
                           ...
                           (mask & KADM5_KVNO) ? entry->kvno : 1, FALSE, kdb);
    }
    else
    {
        /* Null password means create with random key (new in 1.8). */
        ret = krb5_dbe_crk(handle->context, &master_keyblock, new_ks_tuple,
                           new_n_ks_tuple, FALSE, kdb);
    }
    if (ret)
        goto cleanup;
    ...
    /* In all cases key and the principal data is set, let the database provider
     * know */
                                   ...
                                   krb5_principal principal)
{
    ...
    if (ret == 0)
        (void)k5_kadm5_hook_remove(handle->context, handle->hook_handles,
                                   ...
                                   kadm5_principal_ent_t entry, long mask)
{
    ...
    if (mask & KADM5_TL_DATA)
    {
        ...
        /* may have to change the version number of the API. Updates the list
         * with the given tl_data rather than over-writting */
...
kadm5_ret_t kadm5_rename_principal(void* server_handle, krb5_principal source,
...
kadm5_ret_t kadm5_get_principal(void* server_handle, krb5_principal principal,
                                kadm5_principal_ent_t entry, long in_mask)
{
    krb5_db_entry* kdb;
    osa_princ_ent_rec adb;
    ...
    if ((mask & KADM5_POLICY) && adb.policy &&
        (adb.aux_attributes & KADM5_POLICY))
    {
    ...
    {
        ret = krb5_dbe_lookup_mod_princ_data(
            handle->context, kdb, &(entry->mod_date), &(entry->mod_name));
        if (ret)
        {
            goto done;
        }
    ...
    if (mask & KADM5_KEY_DATA)
    {
        ...
        {
            entry->key_data =
                k5calloc(entry->n_key_data, sizeof(krb5_key_data), &ret);
        ...
        for (i = 0; i < entry->n_key_data; i++)
            ret = krb5_copy_key_data_contents(
                handle->context, &kdb->key_data[i], &entry->key_data[i]);
        if (ret)
            goto done;
    }
 ...
 */
static kadm5_ret_t
    check_pw_reuse(krb5_context context, krb5_keyblock* hist_keyblocks,
 ...
 * set to n_key_data.
 */
static int create_history_entry(krb5_context context, krb5_keyblock* hist_key,
                                int n_key_data, krb5_key_data* key_data,
                                osa_pw_hist_ent* hist_out)
{
    int i;
    krb5_error_code ret = 0;
    krb5_keyblock key;
    ...
    for (i = 0; i < n_key_data; i++)
    {
        if (key_data[i].key_data_kvno < kvno)
            break;
        ret =
            krb5_dbe_decrypt_key_data(context, NULL, &key_data[i], &key, &salt);
        if (ret)
            goto cleanup;
...
static kadm5_ret_t add_to_history(krb5_context context, krb5_kvno hist_kvno,
                                  osa_princ_ent_t adb, kadm5_policy_ent_t pol,
                                  osa_pw_hist_ent* pw)
{
    ...
    if (nkeys + 1 < nhist)
    {
        ...
        {
            adb->old_keys =
                (osa_pw_hist_ent*)malloc((nkeys + 1) * sizeof(osa_pw_hist_ent));
        }
        else
        {
            adb->old_keys = (osa_pw_hist_ent*)realloc(
                adb->old_keys, (nkeys + 1) * sizeof(osa_pw_hist_ent));
        }
        if (adb->old_keys == NULL)
            return (ENOMEM);
...
#ifdef USE_PASSWORD_SERVER
static krb5_boolean kadm5_use_password_server(void)
{
...
static kadm5_ret_t kadm5_launch_task(krb5_context context,
                                     ...
                                     const char* buffer)
{
    ...
    if (!ret)
    {
        ...
        else
        {
            ...
            if (buffer)
            {
                if (krb5_net_write(context, data_pipe[1], buffer,
                                   strlen(buffer)) < 0 ||
...
{
    return kadm5_chpass_principal_3(server_handle, principal, FALSE, 0, NULL,
                                    password);
}
                                     ...
                                     char* password)
{
    ...
    if ((ret =
             passwd_check(handle, password, have_pol ? &pol : NULL, principal)))
        goto done;
    ...
    ret = krb5_dbe_cpw(handle->context, act_mkey, new_ks_tuple, new_n_ks_tuple,
                       password, 0 /* increment kvno */, keepold, kdb);
    if (ret)
        goto done;
    ...
    if ((adb.aux_attributes & KADM5_POLICY))
    {
        ...
        ret = check_pw_reuse(handle->context, hist_keyblocks, kdb->n_key_data,
                             kdb->key_data, 1, &hist);
        if (ret)
            goto done;
...
{
    return kadm5_randkey_principal_3(server_handle, principal, FALSE, 0, NULL,
                                     keyblocks, n_keys);
}
                                     ...
                                     krb5_keyblock* keyblock)
{
    ...
    for (i = 0; i < tmp_key_data.key_data_ver; i++)
    {
        ...
        {
            memset(tmp_key_data.key_data_contents[i], 0,
                   tmp_key_data.key_data_length[i]);
...
kadm5_ret_t kadm5_setkey_principal(void* server_handle,
                                   ...
                                   krb5_keyblock* keyblocks, int n_keys)
{
    return kadm5_setkey_principal_3(server_handle, principal, FALSE, 0, NULL,
                                    keyblocks, n_keys);
}
                                     ...
                                     krb5_keyblock* keyblocks, int n_keys)
{
    ...
    ret = kadm5_setkey_principal_4(server_handle, principal, keepold, key_data,
                                   n_keys);
    free(key_data);
    return ret;
}
...
static kadm5_ret_t make_ks_from_key_data(krb5_context context,
                                         kadm5_key_data* key_data,
                                         int n_key_data,
                                         krb5_key_salt_tuple** out)
{
    int i;
...
kadm5_ret_t kadm5_setkey_principal_4(void* server_handle,
                                     ...
                                     kadm5_key_data* key_data, int n_key_data)
{
    krb5_db_entry* kdb;
    osa_princ_ent_rec adb;
    krb5_timestamp now;
    ...
    for (i = 0; i < n_key_data; i++)
    {
        ...
        {
            ret =
                krb5_c_enctype_compare(handle->context, key_data[i].key.enctype,
                                       key_data[j].key.enctype, &similar);
            if (ret)
                goto done;
            if (similar)
            {
 ...
 */
kadm5_ret_t kadm5_get_principal_keys(void* server_handle /* IN */,
                                     ...
                                     int* n_key_data_out /* OUT */)
{
    ...
    for (i = 0, nkeys = 0; i < kdb->n_key_data; i++)
    {
        ...
        ret = krb5_dbe_decrypt_key_data(handle->context, NULL,
                                        &kdb->key_data[i], &key_data[nkeys].key,
                                        &key_data[nkeys].salt);
        if (ret)
            goto done;
        nkeys++;
 ...
 * returned.
 */
kadm5_ret_t kadm5_decrypt_key(void* server_handle, kadm5_principal_ent_t entry,
                              krb5_int32 ktype, krb5_int32 stype,
                              krb5_int32 kvno, krb5_keyblock* keyblock,
                              krb5_keysalt* keysalt, int* kvnop)
{
    ...
    if ((ret = krb5_dbe_find_enctype(handle->context, &dbent, ktype, stype,
                                     kvno, &key_data)))
        return ret;
...
kadm5_ret_t kadm5_purgekeys(void* server_handle, krb5_principal principal,
                            int keepkvno)
{
    kadm5_server_handle_t handle = server_handle;
    kadm5_ret_t ret;
...
kadm5_ret_t kadm5_get_strings(void* server_handle, krb5_principal principal,
...
kadm5_ret_t kadm5_set_string(void* server_handle, krb5_principal principal,
