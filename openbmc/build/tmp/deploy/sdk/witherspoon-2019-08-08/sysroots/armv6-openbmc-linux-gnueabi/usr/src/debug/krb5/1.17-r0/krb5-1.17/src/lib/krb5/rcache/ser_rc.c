 ...
 */
static krb5_error_code krb5_rcache_size(krb5_context, krb5_pointer, size_t*);

static krb5_error_code krb5_rcache_externalize(krb5_context, krb5_pointer,
                                               krb5_octet**, size_t*);

static krb5_error_code krb5_rcache_internalize(krb5_context, krb5_pointer*,
                                               krb5_octet**, size_t*);
 ...
 */
static krb5_error_code krb5_rcache_size(krb5_context kcontext, krb5_pointer arg,
                                        size_t* sizep)
{
 ...
 */
static krb5_error_code krb5_rcache_externalize(krb5_context kcontext,
                                               krb5_pointer arg,
                                               krb5_octet** buffer,
                                               size_t* lenremain)
{
    ...
    {
        kret = ENOMEM;
        if (!krb5_rcache_size(kcontext, arg, &required) && (required <= remain))
        {
            /* Our identifier */
            ...
            if (rcname)
            {
                ...
                (void)krb5_ser_pack_int32((krb5_int32)strlen(rcname), &bp,
                                          &remain);

                /* Put the name */
                (void)krb5_ser_pack_bytes((krb5_octet*)rcname, strlen(rcname),
                                          &bp, &remain);
 ...
 */
static krb5_error_code krb5_rcache_internalize(krb5_context kcontext,
                                               krb5_pointer* argp,
                                               krb5_octet** buffer,
                                               size_t* lenremain)
{
    ...
    if (!rcname)
        return ENOMEM;
    kret =
        krb5_ser_unpack_bytes((krb5_octet*)rcname, (size_t)ibuf, &bp, &remain);
    if (kret)
        goto cleanup;
    rcname[ibuf] = '\0';
 ...
 */
krb5_error_code KRB5_CALLCONV krb5_ser_rcache_init(krb5_context kcontext)
{
