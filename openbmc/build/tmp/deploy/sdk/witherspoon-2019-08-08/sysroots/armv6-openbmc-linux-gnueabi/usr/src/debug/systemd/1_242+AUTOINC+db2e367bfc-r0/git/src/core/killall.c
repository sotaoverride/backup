...
static bool ignore_proc(pid_t pid, bool warn_rootfs)
{
    ...
    /* Try to read the first character of the command line. If the cmdline is
     * empty (which might be the case for kernel threads but potentially also
     * other stuff), this line won't do anything, but we don't care much, as
    ...
    if (warn_rootfs && pid_from_same_root_fs(pid) == 0)
    {
        ...
        log_notice("Process " PID_FMT
                   ...
                   "running from the root file system, and thus likely to "
                   "block re-mounting of the "
                   "root file system to read-only. Please consider moving it "
                   "into an initrd file "
                   "system instead.",
                   pid, strna(comm));
    }
...
static int killall(int sig, Set* pids, bool send_sighup)
{
    ...
    FOREACH_DIRENT_ALL(d, dir, break)
    {
        ...
        if (sig == SIGKILL)
        {
            ...
            log_notice("Sending SIGKILL to PID " PID_FMT " (%s).", pid,
                       strna(s));
        }
