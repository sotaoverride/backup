 ...
 */
#define GPIOD_LINE_BULK_INITIALIZER                                            \
    {                                                                          \
        {NULL}, 0                                                              \
    }
...
int gpiod_line_request_bulk_rising_edge_events_flags(
    struct gpiod_line_bulk* bulk, const char* consumer, int flags) GPIOD_API;
...
#define gpiod_foreach_chip(iter, chip)                                         \
    for ((chip) = gpiod_chip_iter_next(iter); (chip);                          \
...
#define gpiod_foreach_chip_noclose(iter, chip)                                 \
    for ((chip) = gpiod_chip_iter_next_noclose(iter); (chip);                  \
...
#define gpiod_foreach_line(iter, line)                                         \
    for ((line) = gpiod_line_iter_next(iter); (line);                          \
