...
void profile_free_node(struct profile_node* relation);
...
errcode_t profile_delete_node_relation(struct profile_node* section,
                                       const char* name);

errcode_t profile_find_node_name(struct profile_node* section, void** state,
                                 char** ret_name);
...
errcode_t profile_open_file(const_profile_filespec_t file, prf_file_t* ret_prof,
                            char** ret_modspec);

#define profile_update_file(P, M) profile_update_file_data((P)->data, M)
errcode_t profile_update_file_data(prf_data_t profile, char** ret_modspec);
#define profile_update_file_locked(P, M)                                       \
    profile_update_file_data_locked((P)->data, M)
errcode_t profile_update_file_data_locked(prf_data_t data, char** ret_modspec);

#define profile_flush_file(P)                                                  \
    (((P) && (P)->magic == PROF_MAGIC_FILE)                                    \
         ? profile_flush_file_data((P)->data)                                  \
         : PROF_MAGIC_FILE)
errcode_t profile_flush_file_data(prf_data_t data);

#define profile_flush_file_to_file(P, F)                                       \
    (((P) && (P)->magic == PROF_MAGIC_FILE)                                    \
         ? profile_flush_file_data_to_file((P)->data, (F))                     \
         : PROF_MAGIC_FILE)
...
void profile_free_file(prf_file_t profile);

errcode_t profile_close_file(prf_file_t profile);
...
errcode_t profile_ser_size(const char*, profile_t, size_t*);

errcode_t profile_ser_externalize(const char*, profile_t, unsigned char**,
                                  size_t*);

errcode_t profile_ser_internalize(const char*, profile_t*, unsigned char**,
                                  size_t*);
