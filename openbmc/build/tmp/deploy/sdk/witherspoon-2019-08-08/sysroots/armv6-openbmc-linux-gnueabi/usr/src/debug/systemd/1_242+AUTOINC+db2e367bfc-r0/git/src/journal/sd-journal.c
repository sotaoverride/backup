...
static void init_location(Location* l, LocationType type, JournalFile* f,
                          Object* o)
{
    assert(l);
...
_public_ int sd_journal_add_match(sd_journal* j, const void* data, size_t size)
{
    ...
    LIST_FOREACH(matches, l3, j->level2->matches)
    {
        ...
        LIST_FOREACH(matches, l4, l3->matches)
        {
             ...
             * this addition */
            if (l4->le_hash == le_hash && l4->size == size &&
...
_pure_ static int compare_with_location(JournalFile* f, Location* l)
{
    ...
    if (l->monotonic_set && sd_id128_equal(f->current_boot_id, l->boot_id) &&
        l->realtime_set && f->current_realtime == l->realtime &&
...
static int next_for_match(sd_journal* j, Match* m, JournalFile* f,
                          uint64_t after_offset, direction_t direction,
                          Object** ret, uint64_t* offset)
{
    ...
    else if (m->type == MATCH_OR_TERM)
    {
        ...
        LIST_FOREACH(matches, i, m->matches)
        {
            ...
            {
                if (np == 0 ||
                    (direction == DIRECTION_DOWN ? cp < np : cp > np))
                    np = cp;
            }
        }
    ...
    else if (m->type == MATCH_AND_TERM)
    {
        ...
        r = next_for_match(j, m->matches, f, after_offset, direction, NULL,
                           &np);
        if (r <= 0)
            return r;

        assert(direction == DIRECTION_DOWN ? np >= after_offset
                                           : np <= after_offset);
...
static int find_location_for_match(sd_journal* j, Match* m, JournalFile* f,
                                   ...
                                   uint64_t* offset)
{
    ...
    if (m->type == MATCH_DISCRETE)
    {
        ...
        if (j->current_location.type == LOCATION_HEAD)
            return journal_file_next_entry_for_data(
                f, NULL, 0, dp, DIRECTION_DOWN, ret, offset);
        if (j->current_location.type == LOCATION_TAIL)
            return journal_file_next_entry_for_data(f, NULL, 0, dp,
                                                    DIRECTION_UP, ret, offset);
        ...
        if (j->current_location.monotonic_set)
        {
            r = journal_file_move_to_entry_by_monotonic_for_data(
                f, dp, j->current_location.boot_id,
                j->current_location.monotonic, direction, ret, offset);
            if (r != -ENOENT)
                return r;
        }
        ...
        return journal_file_next_entry_for_data(f, NULL, 0, dp, direction, ret,
                                                offset);
    }
    else if (m->type == MATCH_OR_TERM)
    {
        ...
        LIST_FOREACH(matches, i, m->matches)
        {
            ...
            {
                if (np == 0 ||
                    (direction == DIRECTION_DOWN ? np > cp : np < cp))
                    np = cp;
            }
        }
...
static int find_location_with_matches(sd_journal* j, JournalFile* f,
                                      direction_t direction, Object** ret,
                                      uint64_t* offset)
{
    ...
    if (!j->level0)
    {
        ...
        if (j->current_location.monotonic_set)
        {
            r = journal_file_move_to_entry_by_monotonic(
                f, j->current_location.boot_id, j->current_location.monotonic,
...
static int next_with_matches(sd_journal* j, JournalFile* f,
                             direction_t direction, Object** ret,
                             uint64_t* offset)
{
     ...
     * pointer by one. */
    if (!j->level0)
        return journal_file_next_entry(f, f->current_offset, direction, ret,
                                       offset);
...
static int next_beyond_location(sd_journal* j, JournalFile* f,
...
static int real_journal_next(sd_journal* j, direction_t direction)
{
    ...
    for (i = 0; i < n_files; i++)
    {
        ...
        if (r < 0)
        {
            log_debug_errno(r, "Can't iterate through %s, ignoring: %m",
                            f->path);
...
_public_ int sd_journal_get_cursor(sd_journal* j, char** cursor)
{
    ...
    r = journal_file_move_to_object(j->current_file, OBJECT_ENTRY,
    ...
    if (asprintf(
            ...
            sid, le64toh(o->entry.seqnum), bid, le64toh(o->entry.monotonic),
            le64toh(o->entry.realtime), le64toh(o->entry.xor_hash)) < 0)
        return -ENOMEM;
...
_public_ int sd_journal_seek_cursor(sd_journal* j, const char* cursor)
{
    ...
    bool seqnum_id_set = false, seqnum_set = false, boot_id_set = false,
         monotonic_set = false, realtime_set = false, xor_hash_set = false;
    ...
    if ((!seqnum_set || !seqnum_id_set) && (!monotonic_set || !boot_id_set) &&
        !realtime_set)
        return -EINVAL;
...
_public_ int sd_journal_test_cursor(sd_journal* j, const char* cursor)
{
    ...
    for (;;)
    {
        ...
        r = extract_first_word(&cursor, &item, ";",
...
_public_ int sd_journal_seek_monotonic_usec(sd_journal* j, sd_id128_t boot_id,
                                            uint64_t usec)
{
    assert_return(j, -EINVAL);
    assert_return(!journal_pid_changed(j), -ECHILD);
...
static bool file_has_type_prefix(const char* prefix, const char* filename)
{
    ...
    return STR_IN_SET(filename, full, tilded) || startswith(filename, atted);
}
...
static int add_any_file(sd_journal* j, int fd, const char* path)
{
    ...
    {
        if (j->toplevel_fd >= 0)
            /* If there's a top-level fd defined make the path relative,
             * explicitly, since otherwise
             * openat() ignores the first argument. */

            fd = openat(j->toplevel_fd, skip_slash(path),
                        O_RDONLY | O_CLOEXEC | O_NONBLOCK);
        else
            fd = open(path, O_RDONLY | O_CLOEXEC | O_NONBLOCK);
        if (fd < 0)
        {
            r = log_debug_errno(errno, "Failed to open journal file %s: %m",
                                path);
            goto finish;
        }
        ...
        {
            r = log_debug_errno(
                errno, "Failed to turn off O_NONBLOCK for %s: %m", path);
            goto finish;
        }
    }
    ...
    {
        log_debug_errno(
            r, "Refusing to open '%s', as it is not a regular file.", path);
        goto finish;
    }
    ...
    if (f)
    {
            ...
            f->last_stat.st_ino == st.st_ino)
        {
             ...
             * device/inode numbers, it's hence really the same. Mark this file
             * as seen in this generation. This is used to GC old files in
             * process_q_overflow() to detect journal files that are still there
             * and discern them from those which are gone. */
        ...
        /* So we tracked a file under this name, but it has a different
         * inode/device. In that case, it got replaced (probably due to
         * rotation?), let's drop it hence from our list. */
        remove_file_real(j, f);
        f = NULL;
    }
    ...
    r = journal_file_open(fd, path, O_RDONLY, 0, false, 0, false, NULL, j->mmap,
                          NULL, NULL, &f);
    if (r < 0)
    {
    ...
    if (r < 0)
    {
        f->close_fd = false; /* make sure journal_file_close() doesn't close the
                                caller's fd (or our own). We'll let the caller
                                do that, or ourselves */
...
static int add_file_by_name(sd_journal* j, const char* prefix,
                            const char* filename)
{
...
static void remove_file_by_name(sd_journal* j, const char* prefix,
                                const char* filename)
{
...
static int directory_open(sd_journal* j, const char* path, DIR** ret)
{
    ...
    if (j->toplevel_fd < 0)
        ...
        /* Open the specified directory relative to the toplevel fd. Enforce
         * that the path specified is relative, by dropping the initial slash */
...
static int add_directory(sd_journal* j, const char* prefix,
                         const char* dirname);
...
fail:
    log_debug_errno(errno, "Failed to enumerate directory %s, ignoring: %m",
                    m->path);
}
...
static void directory_watch(sd_journal* j, Directory* m, int fd, uint32_t mask)
{
    ...
    if (m->wd < 0)
    {
        log_debug_errno(errno,
                        "Failed to watch journal directory '%s', ignoring: %m",
                        m->path);
        return;
    }
    ...
    if (r == -EEXIST)
        log_debug_errno(r,
                        "Directory '%s' already being watched under a "
                        "different path, ignoring: %m",
                        m->path);
    if (r < 0)
    {
        log_debug_errno(r,
                        "Failed to add watch for journal directory '%s' to "
                        "hashmap, ignoring: %m",
                        m->path);
...
static int add_directory(sd_journal* j, const char* prefix, const char* dirname)
{
    ...
    /* Adds a journal file directory to watch. If the directory is already
     * tracked this updates the inotify watch and reenumerates directory
     * contents */
    ...
    /* We consider everything local that is in a directory for the local machine
     * ID, or that is stored in /run */
    if ((j->flags & SD_JOURNAL_LOCAL_ONLY) &&
        !((dirname && dirname_is_machine_id(dirname) > 0) ||
          path_has_prefix(j, path, "/run")))
        return 0;
    ...
    directory_watch(j, m, dirfd(d),
                    IN_CREATE | IN_MOVED_TO | IN_MODIFY | IN_ATTRIB |
...
static int add_root_directory(sd_journal* j, const char* p, bool missing_ok)
{
    ...
    /* Adds a root directory to our set of directories to use. If the root
     * directory is already in the set, we update the inotify logic, and
     * renumerate the directory entries. This call may hence be called to
     * initially populate the set, as well as to update it later. */
    ...
    else
    {
        ...
        /* If there's no path specified, then we use the top-level fd itself. We
         * duplicate the fd here, since opendir() will take possession of the
         * fd, and close it, which we don't want. */
...
static int add_current_paths(sd_journal* j)
{
    ...
    /* Simply adds all directories for files we have open as directories. We
     * don't expect errors here, so we treat them as fatal. */
...
#define OPEN_CONTAINER_ALLOWED_FLAGS (SD_JOURNAL_LOCAL_ONLY | SD_JOURNAL_SYSTEM)
                                       ...
                                       int flags)
{
    ...
    /* This is pretty much deprecated, people should use machined's
     * OpenMachineRootDirectory() call instead in combination with
    ...
    r = parse_env_file(NULL, p, "ROOT", &root, "CLASS", &class);
    if (r == -ENOENT)
        return -EHOSTDOWN;
    if (r < 0)
        return r;
...
#define OPEN_DIRECTORY_ALLOWED_FLAGS                                           \
    (SD_JOURNAL_OS_ROOT | SD_JOURNAL_SYSTEM | SD_JOURNAL_CURRENT_USER)

_public_ int sd_journal_open_directory(sd_journal** ret, const char* path,
                                       int flags)
{
    _cleanup_(sd_journal_closep) sd_journal* j = NULL;
    int r;
...
_public_ int sd_journal_open_files(sd_journal** ret, const char** paths,
                                   int flags)
{
    _cleanup_(sd_journal_closep) sd_journal* j = NULL;
    const char** path;
...
_public_ int sd_journal_open_files_fd(sd_journal** ret, int fds[],
...
fail:
    /* If we fail, make sure we don't take possession of the files we managed to
     * make use of successfully, and they remain open */
...
_public_ void sd_journal_close(sd_journal* j)
{
    ...
    if (j->mmap)
    {
        log_debug("mmap cache statistics: %u hit, %u miss",
                  mmap_cache_get_hit(j->mmap), mmap_cache_get_missed(j->mmap));
...
_public_ int sd_journal_get_monotonic_usec(sd_journal* j, uint64_t* ret,
...
_public_ int sd_journal_get_data(sd_journal* j, const char* field,
    ...
    for (i = 0; i < n; i++)
    {
        ...
        if (compression)
        {
            ...
            r = decompress_startswith(
                compression, o->data.payload, l, &f->compress_buffer,
                &f->compress_buffer_size, field, field_length, '=');
            if (r < 0)
                log_debug_errno(r,
...
static int return_data(sd_journal* j, JournalFile* f, Object* o,
                       const void** data, size_t* size)
{
    size_t t;
    uint64_t l;
    ...
    if (compression)
    {
        ...
        r = decompress_blob(compression, o->data.payload, l,
                            &f->compress_buffer, &f->compress_buffer_size,
                            &rsize, j->data_threshold);
        if (r < 0)
            return r;
...
_public_ int sd_journal_enumerate_data(sd_journal* j, const void** data,
                                       size_t* size)
{
    JournalFile* f;
    uint64_t p, n;
...
static void process_q_overflow(sd_journal* j)
{
    ...
    /* When the inotify queue overruns we need to enumerate and re-validate all
     * journal files to bring our list back in sync with what's on disk. For
     * this we pick a new generation counter value. It'll be assigned to all
     * journal files we encounter. All journal files and all directories that
     * don't carry it after reenumeration are subject for unloading. */
    ...
    ORDERED_HASHMAP_FOREACH(f, j->files, i)
    {
        ...
        log_debug("File '%s' hasn't been seen in this enumeration, removing.",
                  f->path);
        remove_file_real(j, f);
    }
    ...
    HASHMAP_FOREACH(m, j->directories_by_path, i)
    {
        ...
        log_debug(
            "Directory '%s' hasn't been seen in this enumeration, removing.",
            f->path);
...
_public_ int sd_journal_enumerate_unique(sd_journal* j, const void** data,
                                         size_t* l)
{
    size_t k;
    ...
    for (;;)
    {
        ...
        {
            r = journal_file_find_field_object(j->unique_file, j->unique_field,
                                               k, &o, NULL);
            if (r < 0)
                return r;
        ...
        {
            r = journal_file_move_to_object(j->unique_file, OBJECT_DATA,
                                            j->unique_offset, &o);
            if (r < 0)
                return r;
        ...
        {
            j->unique_file =
                ordered_hashmap_next(j->files, j->unique_file->path);
            if (!j->unique_file)
                return 0;
        ...
        r = journal_file_move_to_object(j->unique_file, OBJECT_UNUSED,
                                        j->unique_offset, &o);
        if (r < 0)
            return r;
    ...
    for (;;)
    {
        ...
        if (j->fields_offset == 0)
        {
            ...
            for (;;)
            {
                ...
                j->fields_offset =
                    le64toh(f->field_hash_table[j->fields_hash_table_index]
                                .head_hash_offset);
        ...
        {
            /* We are already positioned at a field. If so, let's figure out the
             * next field from it */

            r = journal_file_move_to_object(f, OBJECT_FIELD, j->fields_offset,
                                            &o);
            if (r < 0)
                return r;
        ...
        /* We use OBJECT_UNUSED here, so that the iterator below doesn't remove
         * our mmap window */
        r = journal_file_move_to_object(f, OBJECT_UNUSED, j->fields_offset, &o);
        if (r < 0)
            return r;

        /* Because we used OBJECT_UNUSED above, we need to do our type check
         * manually */
        if (o->object.type != OBJECT_FIELD)
            return log_debug_errno(
                SYNTHETIC_ERRNO(EBADMSG),
                "%s:offset " OFSfmt ": object has type %i, expected %i",
                f->path, j->fields_offset, o->object.type, OBJECT_FIELD);
        ...
        if (!GREEDY_REALLOC(j->fields_buffer, j->fields_buffer_allocated,
                            sz + 1))
            return -ENOMEM;
