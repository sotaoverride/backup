...
static enum fsi_result fsi_d_poll_wait(uint8_t slave_id, uint64_t* resp,
                                       int len)
{
    int i;
    uint64_t seq;
...
void fsi_destroy(struct pdbg_target* target)
{
    ...
    /* Crank things - this is needed to use this tool for kicking off system
     * boot  */
    write_gpio(FSI_CLK, 1);
    write_gpio(FSI_DAT, 1); /* Data standby state */
...
int bmcfsi_probe(struct pdbg_target* target)
{
    ...
    if (!gpio_reg)
    {
        ...
        assert(
            !(pdbg_target_u32_property(target, "clock_delay", &clock_delay)));

        /* We only have to do this init once per backend */
        gpio_reg = mmap(NULL, getpagesize(), PROT_READ | PROT_WRITE, MAP_SHARED,
                        mem_fd, GPIO_BASE);
...
__attribute__((constructor)) static void register_bmcfsi(void)
{
