...
static dbus_bool_t socket_set_epoll_add(DBusSocketSet* set, DBusPollable fd,
...
static void socket_set_epoll_enable(DBusSocketSet* set, DBusPollable fd,
                                    unsigned int flags)
{
    DBusSocketSetEpoll* self = socket_set_epoll_cast(set);
    struct epoll_event event;
...
static int socket_set_epoll_poll(DBusSocketSet* set, DBusSocketEvent* revents,
                                 int max_events, int timeout_ms)
{
    ...
    n_ready = epoll_wait(self->epfd, events,
                         MIN(_DBUS_N_ELEMENTS(events), max_events), timeout_ms);
...
DBusSocketSetClass _dbus_socket_set_epoll_class = {
    socket_set_epoll_free,   socket_set_epoll_add,     socket_set_epoll_remove,
    socket_set_epoll_enable, socket_set_epoll_disable, socket_set_epoll_poll};
