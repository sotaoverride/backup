...
static void _pam_dump_env(pam_handle_t* pamh)
{
    ...
    D(("environment entries used = %d [of %d allocated]", pamh->env->requested,
       pamh->env->entries));

    for (i = 0; i < pamh->env->requested; ++i)
    {
        _pam_output_debug(">%-3d [%9p]:[%s]", i, pamh->env->list[i],
                          pamh->env->list[i]);
    }
...
int pam_putenv(pam_handle_t* pamh, const char* name_value)
{
    ...
    { /* (re)setting */
        ...
        { /* new variable */
            ...
            if (pamh->env->entries <= pamh->env->requested)
            {
                ...
                /* get some new space */
                tmp = calloc(pamh->env->entries + PAM_ENV_CHUNK, sizeof(char*));
                if (tmp == NULL)
                {
    ...
    if (item < 0)
    {
        pam_syslog(pamh, LOG_ERR, "pam_putenv: delete non-existent entry; %s",
                   name_value);
        return PAM_BAD_ITEM;
    }
    ...
    _pam_drop(pamh->env->list[item]);
    --(pamh->env->requested);
    D(("mmove: item[%d]+%d -> item[%d]", item + 1,
       (pamh->env->requested - item), item));
