...
#define SECSPER400YEARS                                                        \
    (SECSPERNYEAR * (intmax_t)(300 + 3) + SECSPERLYEAR * (intmax_t)(100 - 3))
...
enum
{
    SECSPER400YEARS_FITS = SECSPERLYEAR <= INTMAX_MAX / 400
};
...
enum
{
    atime_shift = CHAR_BIT * sizeof(time_t) - 2
};
...
static timezone_t tzalloc(char const* val)
{
    ...
    if (!env)
    {
        ...
        while (*e++)
            continue;
        env =
            xmalloc(sumsize(sizeof *environ, (e - environ) * sizeof *environ));
        to = 1;
...
static void usage(FILE* const stream, const int status)
{
    fprintf(stream,
            _("%s: usage: %s OPTIONS TIMEZONE ...\n"
              "Options include:\n"
              "  -c [L,]U   Start at year L (default -500), end before year U "
              "(default 2500)\n"
              "  -t [L,]U   Start at time L, end before time U (in seconds "
              "since 1970)\n"
...
arg_processing_done:;
    ...
    if (iflag | vflag | Vflag)
    {
        ...
        if (cuttimes != NULL)
        {
            ...
            {
                fprintf(stderr, _("%s: wild -t argument %s\n"), progname,
                        cuttimes);
                return EXIT_FAILURE;
            }
        }
...
static time_t hunt(timezone_t tz, char* name, time_t lot, time_t hit)
{
    ...
    for (;;)
    {
        ...
        if (lotm_ok & tm_ok
                ? (delta(&tm, &lotm) == t - lot &&
                   tm.tm_isdst == lotm.tm_isdst && strcmp(abbr(&tm), ab) == 0)
                : lotm_ok == tm_ok)
        {
            lot = t;
            if (tm_ok)
...
{
    return (newp->tm_year < oldp->tm_year ? -delta_nonneg(oldp, newp)
...
static int format_local_time(char* buf, size_t size, struct tm const* tm)
{
    ...
    return (ss ? my_snprintf(buf, size, "%02d:%02d:%02d", hh, mm, ss)
               : mm ? my_snprintf(buf, size, "%02d:%02d", hh, mm)
...
static int format_utc_offset(char* buf, size_t size, struct tm const* tm,
                             time_t t)
{
    long off = gmtoff(tm, &t, NULL);
    char sign =
        ((off < 0 ||
          (off == 0 && (*abbr(tm) == '-' || strcmp(abbr(tm), "zzz") == 0)))
             ? '-'
             : '+');
    long hh;
    int mm, ss;
    ...
    return (ss || 100 <= hh
                ...
                : mm ? my_snprintf(buf, size, "%c%02ld%02d", sign, hh, mm)
...
static bool istrftime(char* buf, size_t size, char const* time_fmt,
                      struct tm const* tm, time_t t, char const* ab,
                 ...
                 (*p == '%' && (p[1] == 'f' || p[1] == 'L' || p[1] == 'Q')))
        {
            ...
            switch (*p)
            {
                ...
                case 'Q':
                {
                    ...
                    if (show_abbr)
                    {
                        ...
                        *b++ = '\t', s--;
                        for (abp = ab; is_alpha(*abp); abp++)
                            continue;
                        len = (!*abp && *ab ? my_snprintf(b, s, "%s", ab)
                    ...
                    formatted_len =
                        (tm->tm_isdst ? my_snprintf(b, s, &"\t\t%d"[show_abbr],
                                                    tm -> tm_isdst)
                                      : 0);
                }
                break;
...
static void showtrans(char const* time_fmt, struct tm const* tm, time_t t,
                      char const* ab, char const* zone_name)
{
    if (!tm)
    {
        printf(tformat(), t);
...
static void dumptime(register const struct tm* timeptr)
{
    ...
    */
    ...
    printf("%s %s%3d %.2d:%.2d:%.2d ", wn, mn, timeptr->tm_mday,
           timeptr->tm_hour, timeptr->tm_min, timeptr->tm_sec);
#define DIVISOR 10
    trail = timeptr->tm_year % DIVISOR + TM_YEAR_BASE % DIVISOR;
    lead =
        timeptr->tm_year / DIVISOR + TM_YEAR_BASE / DIVISOR + trail / DIVISOR;
    trail %= DIVISOR;
    if (trail < 0 && lead > 0)
    {
        trail += DIVISOR;
