               ...
               message is stateful.  */
                while (1)
                {
                    ...
                    if (inlen == 0)
                    {
                        ...
                        assert(
                            wbuf[(wbufsize - outlen) / sizeof(wchar_t) - 1] ==
                            L'\0');
                        break;
                    }
                      ...
                      const char* header_name)
{
    ...
    array1 = (uint32_t*)alloca(best_size * best_depth * sizeof(uint32_t) * 3);
    memset(array1, '\0', best_size * best_depth * sizeof(uint32_t) * 3);
    array2 = (uint32_t*)alloca(best_size * best_depth * sizeof(uint32_t) * 3);
    ...
    while (set_run != NULL)
    {
        ...
        {
            size_t idx =
                (((message_run->number * set_run->number) % best_size) * 3);
...
static int open_conversion(const char* codeset, iconv_t* cd_towcp,
                           iconv_t* cd_tombp, wchar_t* escape_charp)
{
    char buf[2];
    char* bufptr;
