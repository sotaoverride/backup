 ...
 */
#define CFGLNSIZ                                                               \
    64 * 1024 /* the maximum size of a configuraton file line, after           \
                 re-combination */
#include "config.h"
...
/* static data */
DEFobjStaticHelpers DEFobjCurrIf(module) DEFobjCurrIf(net) DEFobjCurrIf(ruleset)

    int bConfStrictScoping =
        0; /* force strict scoping during config processing? */
...
static rsRetVal doModLoad(uchar** pp, __attribute__((unused)) void* pVal)
{
     ...
     * $ModLoad MySQL forward compatibility statement. This needs to be
     * supported for legacy format.
     */
...
static rsRetVal doNameLine(uchar** pp, void* pVal)
{
    ...
    switch (eDir)
    {
        ...
        default: /* we do this to avoid compiler warning - not all
                  ...
                  */
            dbgprintf(
                "INTERNAL ERROR: doNameLine() called with invalid eDir %d.\n",
                eDir);
            break;
    }
...
static rsRetVal cfsysline(uchar* p)
{
    ...
    if (*p && *p != '#')
    { /* we have a non-whitespace, so let's complain */
 ...
 */
rsRetVal cflineParseTemplateName(uchar** pp, omodStringRequest_t* pOMSR,
                                 int iEntry, int iTplOpts, uchar* dfltTplName)
{
    uchar* p;
    uchar* tplName = NULL;
    cstr_t* pStrB = NULL;
 ...
 */
rsRetVal cflineParseFileName(uchar* p, uchar* pFileName,
                             omodStringRequest_t* pOMSR, int iEntry,
                             int iTplOpts, uchar* pszTpl)
{
...
rsRetVal DecodePRIFilter(uchar* pline, uchar pmask[])
{
    ...
    for (p = pline; *p && *p != '\t' && *p != ' ';)
    {
        ...
        for (bp = buf;
             *q && !strchr("\t ,;", *q) && bp < buf + sizeof(buf) - 1;)
            *bp++ = *q++;
        *bp = '\0';
        ...
        if (pri < 0)
        {
            snprintf((char*)xbuf, sizeof(xbuf), "unknown priority name \"%s\"",
                     buf);
        ...
        {
            for (bp = buf;
                 *p && !strchr("\t ,;.", *p) && bp < buf + sizeof(buf) - 1;)
                *bp++ = *p++;
            *bp = '\0';
            if (*buf == '*')
            {
            ...
            else
            {
                ...
                if (i < 0)
                {
                    ...
                    snprintf((char*)xbuf, sizeof(xbuf),
...
rsRetVal cflineDoAction(rsconf_t* conf, uchar** p, action_t** ppAction)
{
    ...
    /* Note: clang static analyzer reports that node maybe == NULL. However,
     * this is not possible, because we have the built-in output modules which
     * are always present. Anyhow, we guard this by an assert. -- rgerhards,
     * 2010-12-16
     */
    assert(node != NULL);
    while (node != NULL)
    {
        ...
        if (iRet == RS_RET_OK)
        {
            if ((iRet = addAction(&pAction, pMod, pModData, pOMSR, NULL,
                                  NULL)) == RS_RET_OK)
            {
...
finalize_it : ENDobjQueryInterface(conf)
...
ENDObjClassExit(conf)
     ...
     * rgerhards, 2008-02-29
     */
    BEGINAbstractObjClassInit(conf, 1,
                              OBJ_IS_CORE_MODULE) /* class, version - CHANGE
 ...
 * far better methods. We will remove the related code soon. -- rgerhards,
 * 2012-01-09
 */
