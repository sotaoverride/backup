...
void list_disk_identifier(struct fdisk_context* cxt)
{
    ...
    if (fdisk_has_label(cxt))
        fdisk_info(cxt, _("Disklabel type: %s"), fdisk_label_get_name(lb));
...
void list_disk_geometry(struct fdisk_context* cxt)
{
    ...
    char* strsz =
        size_to_human_string(SIZE_SUFFIX_SPACE | SIZE_SUFFIX_3LETTER, bytes);

    color_scheme_enable("header", UL_COLOR_BOLD);
    fdisk_info(cxt, _("Disk %s: %s, %ju bytes, %ju sectors"),
               fdisk_get_devname(cxt), strsz, bytes,
    ...
    if (lb && (fdisk_label_require_geometry(lb) || fdisk_use_cylinders(cxt)))
        fdisk_info(cxt,
                   ...
                   fdisk_get_geom_heads(cxt), fdisk_get_geom_sectors(cxt),
    ...
    fdisk_info(cxt, _("Sector size (logical/physical): %lu bytes / %lu bytes"),
               fdisk_get_sector_size(cxt), fdisk_get_physector_size(cxt));
    fdisk_info(cxt, _("I/O size (minimum/optimal): %lu bytes / %lu bytes"),
               fdisk_get_minimal_iosize(cxt), fdisk_get_optimal_iosize(cxt));
...
void list_disklabel(struct fdisk_context* cxt)
{
    ...
    for (i = 0; i < nids; i++)
    {
        ...
        co = scols_table_new_column(out, _(fdisk_field_get_name(field)),
...
void list_freespace(struct fdisk_context* cxt)
{
    ...
    static const char* colnames[] = {N_("Start"), N_("End"), N_("Sectors"),
                                     N_("Size")};
    static const int colids[] = {FDISK_FIELD_START, FDISK_FIELD_END,
                                 FDISK_FIELD_SECTORS, FDISK_FIELD_SIZE};
    ...
    {
        struct libscols_column* co =
            scols_table_new_column(out, _(colnames[i]), 5, SCOLS_FL_RIGHT);
    ...
    fdisk_info(cxt, _("Unpartitioned space %s: %s, %ju bytes, %ju sectors"),
               fdisk_get_devname(cxt), strsz, bytes, sumsize);
    color_disable();
    free(strsz);
...
static int fieldname_to_id(const char* name, size_t namesz)
{
    ...
    if (!fl)
    {
        warnx(_("%s unknown column: %s"), fdisk_label_get_name(fields_label),
              buf);
        return -1;
    }
