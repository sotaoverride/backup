...
#define DEFINE_CATEGORY(category, name, items, postload)                       \
    static struct cat_item category##_desc[] = {NO_PAREN items};
...
int main(int argc, char* argv[])
{
    ...
    if (do_all != 0)
    {
        if (setlocale(LC_COLLATE, "") == NULL)
            error(0, errno, gettext("Cannot set LC_COLLATE to default locale"));
        write_locales();
        exit(EXIT_SUCCESS);
    }
...
static int select_dirs(const struct dirent* dirent)
{
    ...
    if (strcmp(dirent->d_name, ".") != 0 && strcmp(dirent->d_name, "..") != 0)
    {
        ...
        {
            struct stat64 st;
            char buf[sizeof(COMPLOCALEDIR) + strlen(dirent->d_name) + 1];

            stpcpy(stpcpy(stpcpy(buf, COMPLOCALEDIR), "/"), dirent->d_name);
...
#define PUT(name)                                                              \
    tsearch(name, &all_data, (int (*)(const void*, const void*))strcoll)
#define GET(name)                                                              \
    tfind(name, &all_data, (int (*)(const void*, const void*))strcoll)
    ...
    ndirents = scandir(COMPLOCALEDIR, &dirents, select_dirs, alphasort);
    for (cnt = 0; cnt < ndirents; ++cnt)
    {
        /* Test whether at least the LC_CTYPE data is there.  Some
       directories only contain translations.  */
        char buf[sizeof(COMPLOCALEDIR) + strlen(dirents[cnt]->d_name) +
                 sizeof "/LC_IDENTIFICATION"];
        char* enddir;
        struct stat64 st;

        stpcpy(enddir = stpcpy(stpcpy(stpcpy(buf, COMPLOCALEDIR), "/"),
                               dirents[cnt]->d_name),
               "/LC_IDENTIFICATION");
            ...
            if (verbose && GET(dirents[cnt]->d_name) == NULL)
            {
               ...
               information.  */
                ...
                printf("locale: %-15.15s directory: %.*s\n%s\n",
                       dirents[cnt]->d_name, (int)(enddir - buf), buf, linebuf);

                fd = open64(buf, O_RDONLY);
                if (fd != -1)
                {
                    void* mapped =
                        mmap64(NULL, st.st_size, PROT_READ, MAP_SHARED, fd, 0);
    ...
    for (size_t cat_no = 0; cat_no < NCATEGORIES; ++cat_no)
        if (cat_no != LC_ALL)
        {
            ...
            if (lcall[0] != '\0' || val == NULL)
                print_assignment(
                    name,
                    lcall[0] != '\0' ? lcall : lang[0] != '\0' ? lang : "POSIX",
                    true);
            else
    ...
    for (size_t cat_no = 0; cat_no < NCATEGORIES; ++cat_no)
        if (cat_no != LC_ALL)
        {
            ...
            /* Print the whole category.  */
            {
                ...
                for (size_t item_no = 0; item_no < category[cat_no].number;
                     ++item_no)
