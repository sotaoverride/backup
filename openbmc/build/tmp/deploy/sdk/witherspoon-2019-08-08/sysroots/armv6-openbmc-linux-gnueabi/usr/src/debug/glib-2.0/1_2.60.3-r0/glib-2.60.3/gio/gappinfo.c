                                                 ...
                                                 GError** error)
{
    ...
    if (iface->set_as_default_for_extension)
        return (*iface->set_as_default_for_extension)(appinfo, extension,
                                                      error);
 ...
 **/
gboolean g_app_info_launch(GAppInfo* appinfo, GList* files,
 ...
 * Returns: %TRUE on successful launch, %FALSE otherwise.
 **/
gboolean g_app_info_launch_uris(GAppInfo* appinfo, GList* uris,
 ...
 * Since: 2.60
 **/
void g_app_info_launch_uris_async(GAppInfo* appinfo, GList* uris,
                                  ...
                                  gpointer user_data)
{
    ...
    (*iface->launch_uris_async)(appinfo, uris, context, cancellable, callback,
                                user_data);
}
 ...
 * Since: 2.60
 */
gboolean g_app_info_launch_uris_finish(GAppInfo* appinfo, GAsyncResult* result,
                                       GError** error)
{
    GAppInfoIface* iface;
                                           ...
                                           GError** error)
{
    ...
    if (!res && glib_should_use_portal())
    {
        ...
        if (launch_context && launch_context->priv->envp)
            parent_window = g_environ_getenv(launch_context->priv->envp,
                                             "PARENT_WINDOW_ID");
...
static void launch_default_for_uri_data_free(LaunchDefaultForUriData* data)
{
    g_free(data->uri);
...
#ifdef G_OS_UNIX
static void launch_default_for_uri_portal_open_uri_cb(GObject* object,
                                                      GAsyncResult* result,
                                                      gpointer user_data)
{
...
static void launch_default_for_uri_portal_open_uri(GTask* task, GError* error)
{
#ifdef G_OS_UNIX
    ...
    if (glib_should_use_portal())
    {
        ...
        if (data->context && data->context->priv->envp)
            parent_window =
                g_environ_getenv(data->context->priv->envp, "PARENT_WINDOW_ID");

        g_openuri_portal_open_uri_async(
            data->uri, parent_window, cancellable,
            launch_default_for_uri_portal_open_uri_cb, g_steal_pointer(&task));
        return;
    }
#endif
...
static void launch_default_for_uri_launch_uris_cb(GObject* object,
                                                  GAsyncResult* result,
                                                  gpointer user_data)
{
    ...
    if (g_app_info_launch_uris_finish(app_info, result, &error))
    {
        ...
        launch_default_for_uri_portal_open_uri(g_steal_pointer(&task),
                                               g_steal_pointer(&error));
}

static void launch_default_for_uri_launch_uris(GTask* task, GAppInfo* app_info)
{
    GCancellable* cancellable = g_task_get_cancellable(task);
    GList l;
    LaunchDefaultForUriData* data = g_task_get_task_data(task);
    ...
    l.next = l.prev = NULL;
    g_app_info_launch_uris_async(app_info, &l, data->context, cancellable,
                                 launch_default_for_uri_launch_uris_cb,
                                 g_steal_pointer(&task));
    g_object_unref(app_info);
}

static void launch_default_for_uri_default_handler_cb(GObject* object,
                                                      GAsyncResult* result,
                                                      gpointer user_data)
{
                                             ...
                                             gpointer user_data)
{
    ...
    if (!app_info)
    {
        ...
        g_file_query_default_handler_async(
            ...
            launch_default_for_uri_default_handler_cb, g_steal_pointer(&task));
        g_object_unref(file);
    }
    else
        launch_default_for_uri_launch_uris(g_steal_pointer(&task),
...
G_DEFINE_TYPE_WITH_PRIVATE(GAppLaunchContext, g_app_launch_context,
                           G_TYPE_OBJECT)
...
static void g_app_launch_context_finalize(GObject* object)
{
...
static void g_app_launch_context_class_init(GAppLaunchContextClass* klass)
{
    ...
    signals[LAUNCH_FAILED] =
        g_signal_new(I_("launch-failed"), G_OBJECT_CLASS_TYPE(object_class),
                     G_SIGNAL_RUN_LAST,
     ...
     * Since: 2.36
     */
    signals[LAUNCHED] = g_signal_new(
        I_("launched"), G_OBJECT_CLASS_TYPE(object_class), G_SIGNAL_RUN_LAST,
        G_STRUCT_OFFSET(GAppLaunchContextClass, launched), NULL, NULL, NULL,
        G_TYPE_NONE, 2, G_TYPE_APP_INFO, G_TYPE_VARIANT);
}

static void g_app_launch_context_init(GAppLaunchContext* context)
{
                                   ...
                                   const char* variable)
{
    ...
    context->priv->envp = g_environ_unsetenv(context->priv->envp, variable);
}
...
static void g_app_info_monitor_finalize(GObject* object)
{
    GAppInfoMonitor* monitor = G_APP_INFO_MONITOR(object);

    g_context_specific_group_remove(&g_app_info_monitor_group, monitor->context,
                                    monitor, NULL);

    G_OBJECT_CLASS(g_app_info_monitor_parent_class)->finalize(object);
}

static void g_app_info_monitor_init(GAppInfoMonitor* monitor)
{
}

static void g_app_info_monitor_class_init(GAppInfoMonitorClass* class)
{
     ...
     *
     * Signal emitted when the app info database for changes (ie: newly
     *installed or removed applications).
     **/
    g_app_info_monitor_changed_signal = g_signal_new(
        I_("changed"), G_TYPE_APP_INFO_MONITOR, G_SIGNAL_RUN_FIRST, 0, NULL,
...
GAppInfoMonitor* g_app_info_monitor_get(void)
{
    return g_context_specific_group_get(
        &g_app_info_monitor_group, G_TYPE_APP_INFO_MONITOR,
        G_STRUCT_OFFSET(GAppInfoMonitor, context), NULL);
}

void g_app_info_monitor_fire(void)
{
    g_context_specific_group_emit(&g_app_info_monitor_group,
