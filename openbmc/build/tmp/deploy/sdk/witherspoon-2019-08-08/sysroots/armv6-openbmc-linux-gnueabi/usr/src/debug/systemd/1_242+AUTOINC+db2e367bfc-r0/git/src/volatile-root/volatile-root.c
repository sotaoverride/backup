...
static int make_volatile(const char* path)
{
    ...
    if (r < 0)
        return log_error_errno(
            r, "Couldn't generate volatile sysroot directory: %m");

    r = mount_verbose(LOG_ERR, "tmpfs", "/run/systemd/volatile-sysroot",
                      "tmpfs", MS_STRICTATIME, "mode=755");
    if (r < 0)
        goto finish_rmdir;
    ...
    r = mount_verbose(LOG_ERR, old_usr, "/run/systemd/volatile-sysroot/usr",
                      NULL, MS_BIND | MS_REC, NULL);
    if (r < 0)
        goto finish_umount;

    r = bind_remount_recursive("/run/systemd/volatile-sysroot/usr", MS_RDONLY,
                               MS_RDONLY, NULL);
    if (r < 0)
    {
    ...
    if (mount(NULL, "/", NULL, MS_SLAVE | MS_REC, NULL) < 0)
        log_warning_errno(
            errno, "Failed to remount %s MS_SLAVE|MS_REC, ignoring: %m", path);

    r = mount_verbose(LOG_ERR, "/run/systemd/volatile-sysroot", path, NULL,
                      MS_MOVE, NULL);
...
static int make_overlay(const char* path)
{
    ...
    if (r < 0)
        return log_error_errno(r,
                               "Couldn't create overlay sysroot directory: %m");

    r = mount_verbose(LOG_ERR, "tmpfs", "/run/systemd/overlay-sysroot", "tmpfs",
                      MS_STRICTATIME, "mode=755");
    if (r < 0)
        goto finish;
    ...
    {
        r = log_error_errno(
            errno, "Failed to create /run/systemd/overlay-sysroot/upper: %m");
        goto finish;
    }
    ...
    {
        r = log_error_errno(
            errno, "Failed to create /run/systemd/overlay-sysroot/work: %m");
        goto finish;
    }
    ...
    options = strjoina("lowerdir=", escaped_path,
                       ",upperdir=/run/systemd/overlay-sysroot/upper,workdir=/"
                       "run/systemd/overlay-sysroot/work");
...
static int run(int argc, char* argv[])
{
    ...
    if (r < 0)
        return log_error_errno(
            r, "Failed to determine volatile mode from kernel command line.");
    if (r == 0 && argc >= 2)
    {
        /* The kernel command line always wins. However if nothing was set
         * there, the argument passed here wins instead. */
        m = volatile_mode_from_string(argv[1]);
        if (m < 0)
            return log_error_errno(SYNTHETIC_ERRNO(EINVAL),
                                   "Couldn't parse volatile mode: %s", argv[1]);
    }
    ...
    if (r < 0)
        return log_error_errno(
            r, "Couldn't determine whether %s is a mount point: %m", path);
    if (r == 0)
        return log_error_errno(SYNTHETIC_ERRNO(EINVAL),
    ...
    if (r < 0)
        return log_error_errno(
            r, "Couldn't determine whether %s is a temporary file system: %m",
            path);
    if (r > 0)
    {
    ...
    /* We are about to replace the root directory with something else. Later
     * code might want to know what we replaced here, hence let's save that
     * information as a symlink we can later use. (This is particularly relevant
     * for the overlayfs case where we'll fully obstruct the view onto the
     * underlying device, hence querying the backing device node from the file
     * system directly is no longer possible. */
    r = get_block_device_harder(path, &devt);
    if (r < 0)
        return log_error_errno(
            r, "Failed to determine device major/minor of %s: %m", path);
    else if (r > 0)
    {
        ...
        if (symlink(dn, "/run/systemd/volatile-root") < 0)
            log_warning_errno(
                errno,
                "Failed to create symlink /run/systemd/volatile-root: %m");
    }
