...
static void register_foreach(const struct sysORTable* data, void* dummy)
{
...
{
    DEBUGMSGTL(("mibII/sysORTable/register_cb", "register_cb(%d, %d, %p, %p)\n",
...
static int unregister_cb(int major, int minor, void* serv, void* client)
{
    ...
    while ((value = (sysORTable_entry*)ITERATOR_NEXT(it)) &&
           value->data != serv)
        ;
    ITERATOR_RELEASE(it);
    if (value)
...
static int sysORTable_handler(netsnmp_mib_handler* handler,
                              ...
                              netsnmp_request_info* requests)
{
    ...
    DEBUGMSGTL(
        ("mibII/sysORTable/sysORTable_handler", "sysORTable_handler called\n"));

    if (reqinfo->mode != MODE_GET)
    {
        snmp_log(LOG_ERR, "Got unexpected operation for sysORTable\n");
    ...
    while (request)
    {
        ...
        {
            snmp_log(LOG_ERR, "could not extract table info for sysORTable\n");
            snmp_set_var_typed_value(request->requestvb, SNMP_ERR_GENERR, NULL,
                                     0);
        }
...
void init_sysORTable(void)
{
    ...
    sysORLastChange_reg = netsnmp_create_handler_registration(
        "mibII/sysORLastChange", NULL, sysORLastChange_oid,
        OID_LENGTH(sysORLastChange_oid), HANDLER_CAN_RONLY);
    netsnmp_init_watcher_info(&sysORLastChange_winfo, &sysORLastChange,
                              sizeof(u_long), ASN_TIMETICKS,
                              WATCHER_FIXED_SIZE);
    netsnmp_register_watched_scalar(sysORLastChange_reg,
                                    &sysORLastChange_winfo);

    sysORTable_reg = netsnmp_create_handler_registration(
        "mibII/sysORTable", sysORTable_handler, sysORTable_oid,
        OID_LENGTH(sysORTable_oid), HANDLER_CAN_RONLY);
