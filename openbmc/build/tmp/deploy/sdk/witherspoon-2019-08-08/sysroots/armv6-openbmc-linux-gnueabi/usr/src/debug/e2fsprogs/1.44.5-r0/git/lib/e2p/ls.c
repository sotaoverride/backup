...
static const char* interval_string(unsigned int secs)
{
    ...
    if (secs >= WEEK_INT)
    {
        num = secs / WEEK_INT;
        secs -= num * WEEK_INT;
        sprintf(tmp, "%s%d week%s", buf[0] ? ", " : "", num,
                (num > 1) ? "s" : "");
        strcat(buf, tmp);
    }
    if (secs >= DAY_INT)
    {
        num = secs / DAY_INT;
        secs -= num * DAY_INT;
        sprintf(tmp, "%s%d day%s", buf[0] ? ", " : "", num,
                (num > 1) ? "s" : "");
        strcat(buf, tmp);
    }
    if (secs > 0)
    {
        ...
        min = secs / MINUTE_INT;
        secs -= min * MINUTE_INT;
        sprintf(tmp, "%s%d:%02d:%02d", buf[0] ? ", " : "", hr, min, secs);
        strcat(buf, tmp);
    }
    return buf;
}
...
void list_super2(struct ext2_super_block* sb, FILE* f)
{
    ...
    inode_blocks_per_group = (((sb->s_inodes_per_group * EXT2_INODE_SIZE(sb)) +
...
#define POW2(x) ((__u64)1 << (x))
    ...
    if (ext2fs_has_feature_dir_index(sb) || sb->s_def_hash_version)
