...
int krb5_db_get_key_data_kvno(context, count, data) krb5_context context;
int count;
krb5_key_data* data;
{
    int i, kvno;
...
static krb5_error_code preserve_one_old_key(krb5_context context,
                                            krb5_keyblock* mkey,
                                            krb5_db_entry* dbent,
                                            krb5_key_data* old_kd,
 ...
 */
static krb5_error_code preserve_old_keys(krb5_context context,
                                         krb5_keyblock* mkey,
                                         krb5_db_entry* dbent, int kvno,
                                         int n_key_data,
...
static krb5_error_code add_key_rnd(context, master_key, ks_tuple,
                                   ks_tuple_count, db_entry,
                                   kvno) krb5_context context;
...
int kvno;
{
    ...
    for (i = 0; i < ks_tuple_count; i++)
    {
        ...
        {
            if ((retval =
                     krb5_c_enctype_compare(context, ks_tuple[i].ks_enctype,
                                            ks_tuple[j].ks_enctype, &similar)))
                return (retval);
        ...
        /* make new key */
        if ((retval =
                 krb5_c_make_random_key(context, ks_tuple[i].ks_enctype, &key)))
            return retval;
...
static krb5_error_code make_random_salt(krb5_context context,
 ...
 */
static krb5_error_code add_key_pwd(context, master_key, ks_tuple,
                                   ks_tuple_count, passwd, db_entry,
                                   kvno) krb5_context context;
krb5_keyblock* master_key;
krb5_key_salt_tuple* ks_tuple;
int ks_tuple_count;
...
int kvno;
{
    ...
    for (i = 0; i < ks_tuple_count; i++)
    {
        ...
        for (j = 0; j < i; j++)
        {
            ...
            if (similar && (ks_tuple[j].ks_salttype == ks_tuple[i].ks_salttype))
                break;
        }
        ...
        switch (key_salt.type = ks_tuple[i].ks_salttype)
        {
            ...
            {
                krb5_data* saltdata;
                if ((retval = krb5_copy_data(
                         context, krb5_princ_realm(context, db_entry->princ),
                         &saltdata)))
                    return (retval);
            ...
            case KRB5_KDB_SALTTYPE_NOREALM:
                if ((retval = krb5_principal2salt_norealm(
                         context, db_entry->princ, &key_salt.data)))
                    return (retval);
                break;
            ...
            case KRB5_KDB_SALTTYPE_AFS3:
                retval = krb5int_copy_data_contents(
                    context, &db_entry->princ->realm, &key_salt.data);
                if (retval)
                    return retval;
        ...
        retval = krb5_c_string_to_key_with_params(
            context, ks_tuple[i].ks_enctype, &pwd, &key_salt.data, s2k_params,
            &key);
        if (retval)
        {
        ...
        retval = krb5_dbe_encrypt_key_data(context, master_key, &key,
                                           (const krb5_keysalt*)&key_salt, kvno,
                                           kd_slot);
...
static krb5_error_code rekey(krb5_context context, krb5_keyblock* mkey,
                             krb5_key_salt_tuple* ks_tuple, int ks_tuple_count,
                             const char* password, int new_kvno,
 ...
 */
krb5_error_code krb5_dbe_crk(krb5_context context, krb5_keyblock* mkey,
                             krb5_key_salt_tuple* ks_tuple, int ks_tuple_count,
 ...
 */
krb5_error_code krb5_dbe_ark(krb5_context context, krb5_keyblock* mkey,
                             krb5_key_salt_tuple* ks_tuple, int ks_tuple_count,
                             krb5_db_entry* dbent)
{
    return rekey(context, mkey, ks_tuple, ks_tuple_count, NULL, 0,
 ...
 * As a side effect all old keys are nuked if keepold is false.
 */
krb5_error_code krb5_dbe_def_cpw(krb5_context context, krb5_keyblock* mkey,
                                 krb5_key_salt_tuple* ks_tuple,
 ...
 * As a side effect all old keys older than the max kvno are nuked.
 */
krb5_error_code krb5_dbe_apw(krb5_context context, krb5_keyblock* mkey,
                             krb5_key_salt_tuple* ks_tuple, int ks_tuple_count,
                             char* password, krb5_db_entry* dbent)
{
