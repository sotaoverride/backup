...
DEFobjStaticHelpers DEFobjCurrIf(statsobj)
...
#define DYNSTATS_HASHTABLE_SIZE_OVERPROVISIONING 1.25
    ...
    static struct cnfparamdescr modpdescr[] = {
        ...
        {DYNSTATS_PARAM_UNUSED_METRIC_LIFE, eCmdHdlrPositiveInt,
         0} /* in minutes */
};
...
static void /* assumes exclusive access to bucket */
    dynstats_destroyCountersIn(dynstats_bucket_t* b, htable* table,
                               dynstats_ctr_t* ctrs)
{
    dynstats_ctr_t* ctr;
    int ctrs_purged = 0;
...
static rsRetVal dynstats_addBucketMetrics(dynstats_buckets_t* bkts,
                                          dynstats_bucket_t* b,
                                          const uchar* name)
{
    ...
    CHKmalloc(metric_name_buff =
                  malloc((name_len + DYNSTATS_MAX_BUCKET_NS_METRIC_LENGTH + 1) *
                         sizeof(uchar)));
    ...
    *metric_suffix = DYNSTATS_METRIC_NAME_SEPARATOR;
    ...
    ustrncpy(metric_suffix, suffix_litteral,
             DYNSTATS_MAX_BUCKET_NS_METRIC_LENGTH);
    STATSCOUNTER_INIT(b->ctrOpsOverflow, b->mutCtrOpsOverflow);
    CHKiRet(statsobj.AddManagedCounter(
        bkts->global_stats, metric_name_buff, ctrType_IntCtr,
    ...
    CHKiRet(statsobj.AddManagedCounter(bkts->global_stats, metric_name_buff,
    ...
    dynstats_rebuildSurvivorTable(dynstats_bucket_t* b)
{
    ...
    htab_sz = (size_t)(
        DYNSTATS_HASHTABLE_SIZE_OVERPROVISIONING * b->maxCardinality + 1);
    if (b->table == NULL)
    {
        CHKmalloc(survivor_table =
                      create_hashtable(htab_sz, hash_from_string,
                                       key_equals_string, no_op_free));
    }
    CHKmalloc(new_table = create_hashtable(htab_sz, hash_from_string,
                                           key_equals_string, no_op_free));
    statsobj.UnlinkAllCounters(b->stats);
...
static void dynstats_resetIfExpired(dynstats_bucket_t* b)
{
    ...
    if (timeout == 0)
    {
        LogMsg(0, RS_RET_TIMED_OUT, LOG_INFO,
               "dynstats: bucket '%s' is being reset", b->name);
        dynstats_resetBucket(b);
    }
}

static void dynstats_readCallback(statsobj_t __attribute__((unused)) * ignore,
                                  void* b)
{
    dynstats_buckets_t* bkts;
    bkts = &loadConf->dynstats_buckets;
...
static rsRetVal dynstats_newBucket(const uchar* name, uint8_t resettable,
                                   ...
                                   uint32_t unusedMetricLife)
{
    ...
    if (bkts->initialized)
    {
        ...
        pthread_rwlockattr_setkind_np(
            &bucket_lock_attr, PTHREAD_RWLOCK_PREFER_WRITER_NONRECURSIVE_NP);
#endif
...
rsRetVal dynstats_processCnf(struct cnfobj* o)
{
    ...
    for (i = 0; i < modpblk.nParams; ++i)
    {
        ...
        else if (!strcmp(modpblk.descr[i].name, DYNSTATS_PARAM_MAX_CARDINALITY))
        {
            maxCardinality = (uint32_t)pvals[i].val.d.n;
                         ...
                         DYNSTATS_PARAM_UNUSED_METRIC_LIFE))
        {
    ...
    if (name != NULL)
    {
        CHKiRet(dynstats_newBucket(name, resettable, maxCardinality,
                                   unusedMetricLife));
    }
...
rsRetVal dynstats_initCnf(dynstats_buckets_t* bkts)
{
    ...
    CHKiRet(statsobj.SetReportingNamespace(bkts->global_stats,
                                           UCHAR_CONSTANT("values")));
...
static rsRetVal dynstats_createCtr(dynstats_bucket_t* b, const uchar* metric,
                                   dynstats_ctr_t** ctr)
{
    DEFiRet;
...
#endif
static rsRetVal dynstats_addNewCtr(dynstats_bucket_t* b, const uchar* metric,
                                   uint8_t doInitialIncrement)
{
    ...
    if ((unsigned)ATOMIC_FETCH_32BIT_unsigned(
            &b->metricCount, &b->mutMetricCount) >= b->maxCardinality)
    {
    ...
    else
    {
        ...
        if (copy_of_key != NULL)
        {
            survivor_ctr = (dynstats_ctr_t*)hashtable_search(b->survivor_table,
                                                             ctr->metric);
            ...
            if ((created =
                     hashtable_insert(b->table, copy_of_key, effective_ctr)))
            {
...
finalize_it:
    if (iRet != RS_RET_OK)
    {
        ...
        {
            /* NOTE: this is not tested (because it requires very strong
            orchestration to guarantee contended lock for testing) */
