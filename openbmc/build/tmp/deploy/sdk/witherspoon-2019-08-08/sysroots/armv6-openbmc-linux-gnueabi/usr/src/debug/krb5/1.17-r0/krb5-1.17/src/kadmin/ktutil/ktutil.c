...
char* argv[];
{
    ...
    sci_idx = ss_create_invocation("ktutil", "5.0", (char*)NULL, &ktutil_cmds,
                                   &retval);
    if (retval)
    {
...
void ktutil_clear_list(argc, argv) int argc;
char* argv[];
{
    krb5_error_code retval;
...
void ktutil_read_v5(argc, argv) int argc;
char* argv[];
{
    krb5_error_code retval;
...
void ktutil_read_v4(argc, argv) int argc;
char* argv[];
{
    krb5_error_code retval;
...
void ktutil_write_v5(argc, argv) int argc;
char* argv[];
{
    krb5_error_code retval;
...
void ktutil_write_v4(argc, argv) int argc;
char* argv[];
{
    fprintf(stderr, _("%s: writing srvtabs is no longer supported\n"), argv[0]);
}

void ktutil_add_entry(argc, argv) int argc;
char* argv[];
{
    krb5_error_code retval;
    char* princ = NULL;
    ...
    {
        fprintf(stderr,
                _("usage: %s (-key | -password) -p principal "
...
void ktutil_delete_entry(argc, argv) int argc;
char* argv[];
{
    krb5_error_code retval;
...
void ktutil_list(argc, argv) int argc;
char* argv[];
{
    krb5_error_code retval;
    krb5_kt_list lp;
    ...
    for (i = 1, lp = ktlist; lp; i++, lp = lp->next)
    {
        ...
        if (show_time)
        {
            ...
            if (!krb5_timestamp_to_sfstring(
                    (krb5_timestamp)lp->entry->timestamp, fmtbuf,
                    sizeof(fmtbuf), &fill))
                printf("%s ", fmtbuf);
        }
        printf("%40s", pname);
