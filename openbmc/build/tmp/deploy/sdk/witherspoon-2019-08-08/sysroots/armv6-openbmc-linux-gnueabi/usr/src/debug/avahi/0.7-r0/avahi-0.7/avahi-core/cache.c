...
static void remove_entry(AvahiCache* c, AvahiCacheEntry* e)
{
    ...
    avahi_multicast_lookup_engine_notify(c->server->multicast_lookup_engine,
                                         c->interface, e->record,
...
void* avahi_cache_walk(AvahiCache* c, AvahiKey* pattern,
                       AvahiCacheWalkCallback cb, void* userdata)
{
    void* ret;
...
static void* lookup_record_callback(AvahiCache* c, AvahiKey* pattern,
...
static void elapse_func(AvahiTimeEvent* t, void* userdata)
{
    ...
    switch (e->state)
    {
        ...
        case AVAHI_CACHE_REPLACE_FINAL:
            ...
            e = NULL;
            /*         avahi_log_debug("Removing entry from cache due to
             * expiration (%s)", txt); */
            break;
    ...
    if (e)
    {
        ...
        if (avahi_querier_shall_refresh_cache(e->cache->interface,
                                              e->record->key))
            avahi_interface_post_query(e->cache->interface, e->record->key, 0,
                                       NULL);
...
static void update_time_event(AvahiCache* c, AvahiCacheEntry* e)
{
    ...
    if (e->time_event)
        ...
        e->time_event = avahi_time_event_new(c->server->time_event_queue,
                                             &e->expiry, elapse_func, e);
}
...
static void next_expiry(AvahiCache* c, AvahiCacheEntry* e, unsigned percent)
{
    ...
    usec = left + (AvahiUsec)((double)(right - left) * c->last_rand /
                              (RAND_MAX + 1.0));

    e->expiry = e->timestamp;
    avahi_timeval_add(&e->expiry, usec);

    /*     g_message("wake up in +%lu seconds", e->expiry.tv_sec -
     * e->timestamp.tv_sec); */
...
void avahi_cache_update(AvahiCache* c, AvahiRecord* r, int cache_flush,
                        const AvahiAddress* a)
{
    ...
    else
    {
        ...
        else
        {
            ...
            /*             avahi_log_debug("cache: couldn't find matching cache
             * entry for %s", txt);   */
            ...
            avahi_multicast_lookup_engine_notify(
                c->server->multicast_lookup_engine, c->interface, e->record,
                AVAHI_BROWSER_NEW);
        }
...
static void* start_poof_callback(AvahiCache* c, AvahiKey* pattern,
                                 AvahiCacheEntry* e, void* userdata)
{
...
void avahi_cache_stop_poof(AvahiCache* c, AvahiRecord* record,
                           const AvahiAddress* a)
{
    AvahiCacheEntry* e;
