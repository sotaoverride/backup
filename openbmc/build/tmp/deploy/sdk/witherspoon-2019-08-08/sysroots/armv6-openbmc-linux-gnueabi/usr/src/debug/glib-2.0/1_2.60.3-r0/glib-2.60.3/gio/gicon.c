...
static gboolean g_icon_to_string_tokenized(GIcon* icon, GString* s)
{
    ...
    for (i = 0; i < tokens->len; i++)
    {
        ...
        g_string_append_c(s, ' ');
        /* We really only need to escape spaces here, so allow lots of otherwise
         * reserved chars */
...
gchar* g_icon_to_string(GIcon* icon)
{
    ...
    else if (G_IS_THEMED_ICON(icon))
    {
        ...
        g_object_get(G_OBJECT(icon), "names", &names, "use-default-fallbacks",
                     &use_default_fallbacks, NULL);
        /* Themed icon initialized with a single name and no fallbacks. */
        if (names != NULL && names[0] != NULL &&
            names[0][0] != '.' && /* Allowing icons starting with dot would
                                     break G_ICON_SERIALIZATION_MAGIC0 */
            g_utf8_validate(names[0], -1,
                            NULL) && /* Only return utf8 strings */
            names[1] == NULL &&
...
static GIcon* g_icon_new_from_tokens(char** tokens, GError** error)
{
    ...
    if (num_tokens < 1)
    {
        g_set_error(error, G_IO_ERROR, G_IO_ERROR_INVALID_ARGUMENT,
                    _("Wrong number of tokens (%d)"), num_tokens);
        goto out;
    }
    ...
    {
        g_set_error(error, G_IO_ERROR, G_IO_ERROR_INVALID_ARGUMENT,
                    _("No type for class name %s"), tokens[0]);
        goto out;
    }
    ...
    {
        g_set_error(error, G_IO_ERROR, G_IO_ERROR_INVALID_ARGUMENT,
                    _("Type %s is not classed"), tokens[0]);
        goto out;
    }
    ...
    if (version_str)
    {
        ...
        {
            g_set_error(error, G_IO_ERROR, G_IO_ERROR_INVALID_ARGUMENT,
                        _("Malformed version number: %s"), version_str);
            goto out;
        }
    }
    ...
    {
        g_set_error(error, G_IO_ERROR, G_IO_ERROR_INVALID_ARGUMENT,
                    _("Type %s does not implement from_tokens() on the GIcon "
                      "interface"),
                    tokens[0]);
        goto out;
    }
...
static GIcon* g_icon_new_for_string_simple(const gchar* str)
{
    gchar* scheme;
    GIcon* icon;
...
GIcon* g_icon_new_for_string(const gchar* str, GError** error)
{
    ...
    if (g_str_has_prefix(str, G_ICON_SERIALIZATION_MAGIC0))
    {
        ...
        tokens =
            g_strsplit(str + sizeof(G_ICON_SERIALIZATION_MAGIC0) - 1, " ", 0);
...
static GEmblem* g_icon_deserialize_emblem(GVariant* value)
{
    ...
    if (emblem_icon != NULL)
    {
        ...
        if (g_variant_lookup(emblem_metadata, "origin", "&s", &origin_nick))
        {
            ...
            if (origin_value)
                emblem =
                    g_emblem_new_with_origin(emblem_icon, origin_value->value);
...
static GIcon* g_icon_deserialize_emblemed(GVariant* value)
{
...
GIcon* g_icon_deserialize(GVariant* value)
{
    ...
    else if (g_str_equal(tag, "themed") &&
    ...
    else if (g_str_equal(tag, "emblem") &&
             g_variant_is_of_type(val, G_VARIANT_TYPE("(va{sv})")))
    {
        GEmblem* emblem;
    ...
    else if (g_str_equal(tag, "emblemed") &&
             g_variant_is_of_type(val, G_VARIANT_TYPE("(va(va{sv}))")))
    {
...
GVariant* g_icon_serialize(GIcon* icon)
{
    ...
    if (!iface->serialize)
    {
        g_critical("g_icon_serialize() on icon type '%s' is not implemented",
    ...
    if (result)
    {
        ...
        {
            g_critical("g_icon_serialize() on icon type '%s' returned GVariant "
                       "of type '%s' but it must return "
                       "one with type '(sv)'",
                       G_OBJECT_TYPE_NAME(icon),
