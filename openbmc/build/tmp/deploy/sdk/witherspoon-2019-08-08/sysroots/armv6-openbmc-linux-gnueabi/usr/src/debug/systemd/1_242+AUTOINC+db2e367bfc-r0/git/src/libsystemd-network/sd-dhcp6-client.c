...
#define DHCP6_CLIENT_DONT_DESTROY(client)                                      \
    _cleanup_(sd_dhcp6_client_unrefp)                                          \
        _unused_ sd_dhcp6_client* _dont_destroy_##client =                     \
...
int sd_dhcp6_client_set_local_address(sd_dhcp6_client* client,
                                      const struct in6_addr* local_address)
{

    assert_return(client, -EINVAL);
    assert_return(local_address, -EINVAL);
    assert_return(in_addr_is_link_local(
                      AF_INET6, (const union in_addr_union*)local_address) > 0,
                  -EINVAL);
 ...
 */
static int dhcp6_client_set_duid_internal(sd_dhcp6_client* client,
                                          uint16_t duid_type, const void* duid,
                                          size_t duid_len, usec_t llt_time)
{
    int r;
    ...
    if (duid != NULL)
    {
        ...
        if (r < 0)
        {
            ...
            log_dhcp6_client(client,
                             "Setting DUID of type %u with unexpected content",
                             duid_type);
        }
        ...
        switch (duid_type)
        {
            case DUID_TYPE_LLT:
                ...
                r = dhcp_identifier_set_duid_llt(
                    &client->duid, llt_time, client->mac_addr,
                    client->mac_addr_len, client->arp_type, &client->duid_len);
                if (r < 0)
                    return r;
                break;
            case DUID_TYPE_EN:
                r = dhcp_identifier_set_duid_en(&client->duid,
                                                &client->duid_len);
                if (r < 0)
                    return r;
                break;
            case DUID_TYPE_LL:
                ...
                r = dhcp_identifier_set_duid_ll(
                    &client->duid, client->mac_addr, client->mac_addr_len,
                    client->arp_type, &client->duid_len);
                if (r < 0)
                    return r;
                break;
            case DUID_TYPE_UUID:
                r = dhcp_identifier_set_duid_uuid(&client->duid,
                                                  &client->duid_len);
                if (r < 0)
                    return r;
                break;
...
int sd_dhcp6_client_set_duid_llt(sd_dhcp6_client* client, usec_t llt_time)
{
    return dhcp6_client_set_duid_internal(client, DUID_TYPE_LLT, NULL, 0,
                                          llt_time);
}
...
int sd_dhcp6_client_set_fqdn(sd_dhcp6_client* client, const char* fqdn)
{
...
static int client_send_message(sd_dhcp6_client* client, usec_t time_now)
{
    ...
    switch (client->state)
    {
        ...
        case DHCP6_STATE_SOLICITATION:
            ...
            r = dhcp6_option_append(&opt, &optlen, SD_DHCP6_OPTION_RAPID_COMMIT,
                                    0, NULL);
            if (r < 0)
                return r;
                                 ...
                                 void* userdata)
{
    ...
    log_dhcp6_client(client, "Next retransmission in %s",
                     format_timespan(time_string, FORMAT_TIMESPAN_MAX,
                                     client->retransmit_time, USEC_PER_SEC));
    ...
    if (max_retransmit_duration &&
        event_source_is_enabled(client->timeout_resend_expire) <= 0)
    {
        ...
        r = event_reset_time(
            client->event, &client->timeout_resend_expire,
            clock_boottime_or_monotonic(), time_now + max_retransmit_duration,
            USEC_PER_SEC, client_timeout_resend_expire, client,
...
static int client_parse_message(sd_dhcp6_client* client, DHCP6Message* message,
                                size_t len, sd_dhcp6_lease* lease)
{
    ...
    while (pos < len)
    {
        ...
        switch (optcode)
        {
            case SD_DHCP6_OPTION_CLIENTID:
                ...
                {
                    log_dhcp6_client(
                        client, "%s contains multiple clientids",
                        dhcp6_message_type_to_string(message->type));
                    return -EINVAL;
                }
            ...
            case SD_DHCP6_OPTION_SERVERID:
                ...
                {
                    log_dhcp6_client(
                        client, "%s contains multiple serverids",
                        dhcp6_message_type_to_string(message->type));
                    return -EINVAL;
                }
            ...
            case SD_DHCP6_OPTION_STATUS_CODE:
                status = dhcp6_option_parse_status(
                    option, optlen + sizeof(DHCP6Option));
                if (status < 0)
                    return status;
            ...
            case SD_DHCP6_OPTION_IA_NA:
                ...
                {
                    log_dhcp6_client(
                        client, "Information request ignoring IA NA option");
                ...
                {
                    log_dhcp6_client(
                        client, "%s has wrong IAID for IA NA",
                        dhcp6_message_type_to_string(message->type));
                    return -EINVAL;
                }
            ...
            case SD_DHCP6_OPTION_IA_PD:
                ...
                {
                    log_dhcp6_client(
                        client, "Information request ignoring IA PD option");
                ...
                {
                    log_dhcp6_client(
                        client, "%s has wrong IAID for IA PD",
                        dhcp6_message_type_to_string(message->type));
                    return -EINVAL;
                }
                                  ...
                                  void* userdata)
{
    ...
    if (client->transaction_id !=
        (message->transaction_id & htobe32(0x00ffffff)))
        return 0;
...
static int client_start(sd_dhcp6_client* client, enum DHCP6State state)
{
    ...
    if (!client->receive_message)
    {
        r = sd_event_add_io(client->event, &client->receive_message, client->fd,
                            EPOLLIN, client_receive_message, client);
        if (r < 0)
            goto error;
    ...
    switch (state)
    {
        ...
        case DHCP6_STATE_BOUND:
            ...
            r = event_reset_time(
                client->event, &client->timeout_t1,
                clock_boottime_or_monotonic(), time_now + timeout,
                10 * USEC_PER_SEC, client_timeout_t1, client,
            ...
            r = event_reset_time(
                client->event, &client->timeout_t2,
                clock_boottime_or_monotonic(), time_now + timeout,
                10 * USEC_PER_SEC, client_timeout_t2, client,
...
int sd_dhcp6_client_start(sd_dhcp6_client* client)
{
    ...
    assert_return(
        in_addr_is_link_local(
            AF_INET6, (const union in_addr_union*)&client->local_address) > 0,
        -EINVAL);
    ...
    if (client->fd < 0)
    {
        r = dhcp6_network_bind_udp_socket(client->ifindex,
                                          &client->local_address);
        if (r < 0)
        {
            _cleanup_free_ char* p = NULL;

            (void)in_addr_to_string(
                AF_INET6, (const union in_addr_union*)&client->local_address,
                &p);
            return log_dhcp6_client_errno(
                client, r, "Failed to bind to UDP socket at address %s: %m",
                strna(p));
        }
...
DEFINE_TRIVIAL_REF_UNREF_FUNC(sd_dhcp6_client, sd_dhcp6_client,
                              dhcp6_client_free);
