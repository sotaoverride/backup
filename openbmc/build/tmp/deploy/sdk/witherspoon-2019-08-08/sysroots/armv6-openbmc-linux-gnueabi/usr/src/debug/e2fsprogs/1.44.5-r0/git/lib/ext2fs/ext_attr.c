                                      ...
                                      void* data, __u32* hash)
{
    *hash = ext2fs_ext_attr_hash_entry(entry, data);
    ...
    if (entry->e_value_inum)
    {
        ...
        retval = read_ea_inode_hash(fs, entry->e_value_inum, &ea_inode_hash);
        if (retval)
            return retval;
                                  ...
                                  struct ext2_ext_attr_entry* end)
{
    ...
    while (here < end && !EXT2_EXT_IS_LAST_ENTRY(here))
    {
        ...
        hash = (hash << BLOCK_HASH_SHIFT) ^
               (hash >> (8 * sizeof(hash) - BLOCK_HASH_SHIFT)) ^ here->e_hash;
                                 ...
                                 ext2_ino_t inum)
{
    ...
    retval = ext2fs_ext_attr_block_csum_set(
        fs, inum, block, (struct ext2_ext_attr_header*)write_buf);
    if (retval)
        return retval;
...
errcode_t ext2fs_adjust_ea_refcount2(ext2_filsys fs, blk64_t blk,
                                     ...
                                     __u32* newcount)
{
    return ext2fs_adjust_ea_refcount3(fs, blk, block_buf, adjust, newcount, 0);
}

errcode_t ext2fs_adjust_ea_refcount(ext2_filsys fs, blk_t blk, char* block_buf,
                                         ...
                                         struct ext2_inode_large* inode)
{
    ...
    else
    {
        ...
        err = ext2fs_iblk_add_blocks(fs, (struct ext2_inode*)inode, 1);
        if (err)
            goto out;
    }
 ...
 */
static errcode_t convert_posix_acl_to_disk_buffer(const void* value,
                                                  size_t size, void* out_buf,
                                                  size_t* size_out)
{
    const posix_acl_xattr_header* header = (const posix_acl_xattr_header*)value;
    const posix_acl_xattr_entry *end,
        *entry = (const posix_acl_xattr_entry*)(header + 1);
...
static errcode_t convert_disk_buffer_to_posix_acl(const void* value,
                                                  size_t size, void** out_buf,
                                                  size_t* size_out)
{
    ...
    if ((!value) || (size < sizeof(ext4_acl_header)) ||
        (ext_acl->a_version != ext2fs_cpu_to_le32(EXT4_ACL_VERSION)))
        return EINVAL;
                           ...
                           unsigned int value_offset_correction, int write_hash)
{
    ...
    for (x = attrs; x < attrs + count; x++)
    {
        ...
        value_size =
            ((x->value_len + EXT2_EXT_ATTR_PAD - 1) / EXT2_EXT_ATTR_PAD) *
            EXT2_EXT_ATTR_PAD;
        ...
        else
        {
            ...
            e->e_value_offs =
                end - (char*)entries_start + value_offset_correction;
            memcpy(end, x->value, e->e_value_size);
        }

        if (write_hash || x->ea_ino)
        {
            err = ext2fs_ext_attr_hash_entry2(fs, e, x->ea_ino ? 0 : end,
                                              &e->e_hash);
            if (err)
                return err;
        }
...
errcode_t ext2fs_xattrs_write(struct ext2_xattr_handle* handle)
{
    ...
    err =
        ext2fs_read_inode_full(fs, handle->ino, EXT2_INODE(inode), inode_size);
    if (err)
        goto out;

    /* If extra_isize isn't set, we need to set it now */
    if (inode->i_extra_isize == 0 && inode_size > EXT2_GOOD_OLD_INODE_SIZE)
    {
        char* p = (char*)inode;
    ...
    if (inode->i_extra_isize < sizeof(inode->i_extra_isize) ||
        inode_size <=
            EXT2_GOOD_OLD_INODE_SIZE + inode->i_extra_isize + sizeof(__u32))
        goto write_ea_block;

    /* Write the inode EA */
    ea_inode_magic = EXT2_EXT_ATTR_MAGIC;
    memcpy(((char*)inode) + EXT2_GOOD_OLD_INODE_SIZE + inode->i_extra_isize,
           &ea_inode_magic, sizeof(__u32));
    storage_size = inode_size - EXT2_GOOD_OLD_INODE_SIZE -
                   inode->i_extra_isize - sizeof(__u32);
    start = ((char*)inode) + EXT2_GOOD_OLD_INODE_SIZE + inode->i_extra_isize +
            sizeof(__u32);

    err = write_xattrs_to_buffer(fs, handle->attrs, handle->ibody_count, start,
                                 storage_size, 0, 0);
    if (err)
        goto out;
write_ea_block:
...
skip_ea_block:
    ...
    /* Write the inode */
    err =
        ext2fs_write_inode_full(fs, handle->ino, EXT2_INODE(inode), inode_size);
    if (err)
        goto out2;
                                         ...
                                         char* value_start)
{
    ...
    errcode_t err;
    unsigned int values_size = storage_size + ((char*)entries - value_start);
           ...
           !EXT2_EXT_IS_LAST_ENTRY(entry))
    {
        ...
        err = ext2fs_get_memzero(entry->e_name_len + prefix_len + 1, &x->name);
        if (err)
            return err;
        if (prefix)
            memcpy(x->name, prefix, prefix_len);
        if (entry->e_name_len)
            memcpy(x->name + prefix_len, (char*)entry + sizeof(*entry),
                   entry->e_name_len);
        ...
        if (entry->e_value_inum == 0)
        {
            ...
            if (entry->e_value_size > 0 &&
                value_start + entry->e_value_offs < (char*)end + sizeof(__u32))
                return EXT2_ET_EA_BAD_VALUE_OFFSET;
        ...
        else
        {
            ...
            else if (ext2fs_file_get_size(ea_file) != entry->e_value_size)
                err = EXT2_ET_EA_BAD_VALUE_SIZE;
            else
                err =
                    ext2fs_file_read(ea_file, x->value, entry->e_value_size, 0);
        ...
        if (entry->e_hash != 0)
        {
            ...
            err = ext2fs_ext_attr_hash_entry2(handle->fs, entry, data, &hash);
            if (err)
                return err;
            if (entry->e_hash != hash)
            {
                 ...
                 */
                err =
                    ext2fs_read_inode(handle->fs, entry->e_value_inum, &child);
                if (err)
                    return err;
...
errcode_t ext2fs_xattrs_read(struct ext2_xattr_handle* handle)
{
    ...
    memcpy(&ea_inode_magic,
           ((char*)inode) + EXT2_GOOD_OLD_INODE_SIZE + inode->i_extra_isize,
           sizeof(__u32));
...
read_ea_block:
    ...
    if (blk != 0)
    {
        ...
        err = ext2fs_read_ext_attr3(handle->fs, blk, block_buf, handle->ino);
        if (err)
            goto out3;
        ...
        /* Read EAs */
        storage_size =
            handle->fs->blocksize - sizeof(struct ext2_ext_attr_header);
        start = block_buf + sizeof(struct ext2_ext_attr_header);
        err = read_xattrs_from_buffer(handle, inode,
                                      (struct ext2_ext_attr_entry*)start,
                                      ...
                                      size_t* size)
{
    ...
    else
    {
        ...
        *size = EXT2_EXT_ATTR_SIZE(minoff - EXT2_EXT_ATTR_LEN(strlen("data")) -
...
static errcode_t xattr_array_update(struct ext2_xattr_handle* h,
                                    const char* name, const void* value,
                                    size_t value_len, int ibody_free,
                                    int block_free, int old_idx, int in_inode)
{
    ...
    if (old_idx >= 0 && old_idx < h->ibody_count)
    {
        ...
        if (!h->attrs[old_idx].ea_ino)
            ibody_free += EXT2_EXT_ATTR_SIZE(h->attrs[old_idx].value_len);
    }
    ...
    if (needed <= ibody_free)
    {
        ...
        ret = xattr_update_entry(h->fs, &h->attrs[old_idx], name, value,
                                 value_len, in_inode);
        if (ret)
            return ret;
        if (h->ibody_count <= old_idx)
        {
            ...
            tmp = h->attrs[old_idx];
            memmove(h->attrs + h->ibody_count + 1, h->attrs + h->ibody_count,
    ...
    if (h->ibody_count <= old_idx)
    {
        ...
        if (!h->attrs[old_idx].ea_ino)
            block_free += EXT2_EXT_ATTR_SIZE(h->attrs[old_idx].value_len);
    }
...
add_new:
    ...
    ret = xattr_update_entry(h->fs, &h->attrs[h->count], name, value, value_len,
                             in_inode);
    if (ret)
        return ret;
                           ...
                           const void* value, size_t value_len)
{
    ...
    {
        ret = convert_posix_acl_to_disk_buffer(value, value_len, new_value,
                                               &value_len);
        if (ret)
            goto out;
    }
    ...
    if (ret)
        goto out;
    ret = ext2fs_read_inode_full(fs, h->ino, (struct ext2_inode*)inode,
                                 inode_size);
    if (ret)
        goto out;
    ...
    if (strcmp(name, "system.data") == 0)
    {
        ...
        ret = xattr_array_update(h, name, new_value, value_len, ibody_free,
                                 0 /* block_free */, old_idx, 0 /* in_inode */);
        if (ret)
            goto out;
        goto write_out;
    }
    ...
    block_free -=
        space_used(h->attrs + h->ibody_count, h->count - h->ibody_count);
...
errcode_t ext2fs_xattr_remove(struct ext2_xattr_handle* handle, const char* key)
{
    struct ext2_xattr* x;
                             ...
                             struct ext2_xattr_handle** handle)
{
    ...
    h->capacity = 4;
    err =
        ext2fs_get_arrayzero(h->capacity, sizeof(struct ext2_xattr), &h->attrs);
    if (err)
    {
        ext2fs_free_mem(&h);
        return err;
