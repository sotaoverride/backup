...
BOOST_INTRUSIVE_HAS_STATIC_MEMBER_FUNC_SIGNATURE(
    has_member_function_callable_with_pointer_to, pointer_to)
BOOST_INTRUSIVE_HAS_STATIC_MEMBER_FUNC_SIGNATURE(
    has_member_function_callable_with_dynamic_cast_from, dynamic_cast_from)
BOOST_INTRUSIVE_HAS_STATIC_MEMBER_FUNC_SIGNATURE(
    has_member_function_callable_with_static_cast_from, static_cast_from)
BOOST_INTRUSIVE_HAS_STATIC_MEMBER_FUNC_SIGNATURE(
    has_member_function_callable_with_const_cast_from, const_cast_from)
#else
BOOST_INTRUSIVE_HAS_MEMBER_FUNC_CALLED_IGNORE_SIGNATURE(
    has_member_function_callable_with_pointer_to, pointer_to)
BOOST_INTRUSIVE_HAS_MEMBER_FUNC_CALLED_IGNORE_SIGNATURE(
    has_member_function_callable_with_dynamic_cast_from, dynamic_cast_from)
BOOST_INTRUSIVE_HAS_MEMBER_FUNC_CALLED_IGNORE_SIGNATURE(
...
//! pointer_traits is the implementation of C++11 std::pointer_traits class with
//! some extensions like castings.
//!
//! pointer_traits supplies a uniform interface to certain attributes of
//! pointer-like types.
//!
//! <b>Note</b>: When defining a custom family of pointers or references to be
//! used with BI library, make sure the public static conversion functions
//! accessed through the `pointer_traits` interface (`*_cast_from` and
//! `pointer_to`) can properly convert between const and nonconst referred
//! member types <b>without the use of implicit constructor calls</b>. It is
//! suggested these conversions be implemented as function templates, where the
//! template argument is the type of the object being converted from.
...
struct pointer_traits
{
    ...
    //! zero or more type arguments ; otherwise , the specialization is
    //! ill-formed.
    ...
    //! Ptr::rebind<U> if such a type exists; otherwise, SomePointer<U, Args> if
    //! Ptr is a class template instantiation of the form SomePointer<T, Args>,
    //! where Args is zero or more type arguments ; otherwise, the instantiation
    //! of rebind is ill-formed.
    //!
    //! For portable code for C++03 and C++11, <pre>typename
    //! rebind_pointer<U>::type</pre> shall be used instead of rebind<U> to
    //! obtain a pointer to U.
    template <class U>
    using rebind = unspecified;

    //! Ptr::reference if such a type exists (non-standard extension);
    ...
    typedef BOOST_INTRUSIVE_OBTAIN_TYPE_WITH_DEFAULT(
        boost::intrusive::detail::, Ptr, difference_type,
    ...
    //! <b>Remark</b>: If element_type is (possibly cv-qualified) void, r type
    //! is unspecified; otherwise,
    //!   it is element_type &.
    //!
    //! <b>Returns</b>: A dereferenceable pointer to r obtained by calling
    //! Ptr::pointer_to(reference).
    //!   Non-standard extension: If such function does not exist, returns
    //!   pointer(addressof(r));
    //!
    //! <b>Note</b>: For non-conforming compilers only the existence of a member
    //! function called
    //!   <code>pointer_to</code> is checked.
    static pointer pointer_to(reference r)
    {
        // Non-standard extension, it does not require Ptr::pointer_to. If not
        // present tries to converts &r to pointer.
    ...
    //!
    //! <b>Returns</b>: A dereferenceable pointer to r obtained by calling the
    //! static template function
    //!   Ptr::static_cast_from(UPpr/const UPpr &).
    //!   If such function does not exist, returns
    //!   pointer_to(static_cast<element_type&>(*uptr))
    //!
    //! <b>Note</b>: For non-conforming compilers only the existence of a member
    //! function called
    ...
    static pointer static_cast_from(const UPtr& uptr)
    {
        ...
        return pointer_traits::priv_static_cast_from(
    ...
    //!   If such function does not exist, returns
    //!   pointer_to(const_cast<element_type&>(*uptr))
    //!
    //! <b>Note</b>: For non-conforming compilers only the existence of a member
    //! function called
    ...
    static pointer const_cast_from(const UPtr& uptr)
    {
        ...
        const bool value =
            ...
            boost::intrusive::detail::
                has_member_function_callable_with_const_cast_from<
                    pointer, pointer (*)(UPtr)>::value;
        return pointer_traits::priv_const_cast_from(
            boost::intrusive::detail::bool_<value>(), uptr);
    }
    ...
    //!   If such function does not exist, returns
    //!   pointer_to(*dynamic_cast<element_type*>(&*uptr))
    //!
    //! <b>Note</b>: For non-conforming compilers only the existence of a member
    //! function called
    ...
    static pointer dynamic_cast_from(const UPtr& uptr)
    {
        ...
        const bool value =
            ...
            boost::intrusive::detail::
                has_member_function_callable_with_dynamic_cast_from<
                    pointer, pointer (*)(UPtr)>::value;
        return pointer_traits::priv_dynamic_cast_from(
            boost::intrusive::detail::bool_<value>(), uptr);
    }
  ...
  private:
    ...
    static pointer priv_pointer_to(boost::intrusive::detail::false_,
                                   reference r)
    {
    ...
    template <class UPtr>
    static pointer priv_static_cast_from(boost::intrusive::detail::true_,
                                         const UPtr& uptr)
    {
    ...
    template <class UPtr>
    static pointer priv_static_cast_from(boost::intrusive::detail::false_,
                                         const UPtr& uptr)
    {
        return uptr ? pointer_to(
                          *static_cast<element_type*>(to_raw_pointer(uptr)))
                    : pointer();
    }

    // priv_const_cast_from
    template <class UPtr>
    static pointer priv_const_cast_from(boost::intrusive::detail::true_,
                                        const UPtr& uptr)
    {
    ...
    template <class UPtr>
    static pointer priv_const_cast_from(boost::intrusive::detail::false_,
                                        const UPtr& uptr)
    {
    ...
    template <class UPtr>
    static pointer priv_dynamic_cast_from(boost::intrusive::detail::true_,
                                          const UPtr& uptr)
    {
    ...
    template <class UPtr>
    static pointer priv_dynamic_cast_from(boost::intrusive::detail::false_,
                                          const UPtr& uptr)
    {
        return uptr ? pointer_to(dynamic_cast<element_type&>(*uptr))
                    : pointer();
    }
    ///@endcond
};
...
struct pointer_traits<T*>
{
    ...
    using rebind = U*;
#else
    typedef typename boost::intrusive::detail::unvoid_ref<element_type>::type
        reference;
#if !defined(BOOST_NO_CXX11_TEMPLATE_ALIASES)
    template <class U>
    using rebind = U*;
