 ...
 *
 * Module begun 2008-10-09 by Rainer Gerhards (based on previous code from
 * syslogd.c)
 *
...
DEFobjStaticHelpers DEFobjCurrIf(glbl) DEFobjCurrIf(datetime)
 ...
 * it to NULL.
 */
static rsRetVal InitParserList(parserList_t** pListRoot)
{
    *pListRoot = NULL;
    return RS_RET_OK;
}

/* destruct a parser list. The list elements are destroyed, but the parser
 * objects themselves are not modified. (That is done at a late stage during
 * rsyslogd shutdown and need not be considered here.)
 */
 ...
 * I prefer to keep it very simple and with simple data structures.
 * Unfortunately, we need to preserve the order, but I don't like to add a tail
 * pointer as that would require a container object. So I do the extra work to
 * skip to the tail when adding elements... rgerhards, 2009-11-03
 */
...
static rsRetVal FindParser(parser_t** ppParser, uchar* pName)
{
    parserList_t* pThis;
    DEFiRet;
...
BEGINobjConstruct(
    parser) /* be sure to specify the object type also in END macro! */
    ENDobjConstruct(parser)

    /* ConstructionFinalizer. The most important chore is to add the parser
     * object to our global list of available parsers. rgerhards, 2009-11-03
     */
 ...
 */
rsRetVal parserConstructViaModAndName(modInfo_t* __restrict__ pMod,
                                      uchar* const __restrict__ pName,
                                      void* pInst)
{
    rsRetVal localRet;
...
BEGINobjDestruct(parser) /* be sure to specify the object type also in END and
                            CODESTART macros! */
    ...
    static rsRetVal uncompressMessage(smsg_t* pMsg)
{
    ...
    { /* compressed data present? (do NOT change order if conditions!) */
        /* we have compressed data, so let's deflate it. We support a maximum
         * message size of iMaxLine. If it is larger, an error message is logged
         * and the message is dropped. We do NOT try to decompress larger
         * messages as such might be used for denial of service. It might happen
         * to later builds that such functionality be added as an optional,
         * operator-configurable feature.
         */
        int ret;
        iLenDefBuf = glbl.GetMaxLine();
        CHKmalloc(deflateBuf = malloc(iLenDefBuf + 1));
        ret = uncompress((uchar*)deflateBuf, &iLenDefBuf, (uchar*)pszMsg + 1,
                         lenMsg - 1);
        DBGPRINTF("Compressed message uncompressed with status %d, length: new "
                  "%ld, old %d.\n",
                  ret, (long)iLenDefBuf, (int)(lenMsg - 1));
        /* Now check if the uncompression worked. If not, there is not much we
         * can do. In that case, we log an error message but ignore the message
         * itself. Storing the compressed text is dangerous, as it contains
         * control characters. So we do not do this. If someone would like to
         * have a copy, this code here could be modified to do a hex-dump of the
         * buffer in question. We do not include this functionality right now.
...
static rsRetVal SanitizeMsg(smsg_t* pMsg)
{
     ...
     */
    if (glbl.GetParserDropTrailingLFOnReception() && lenMsg > 0 &&
        pszMsg[lenMsg - 1] == '\n')
    {
        DBGPRINTF(
            "dropped LF at very end of message (DropTrailingLF is set)\n");
        lenMsg--;
        pszMsg[lenMsg] = '\0';
        bUpdatedLen = RSTRUE;
    }
     ...
     * need below (but it then still will work perfectly well!). -- rgerhards,
     * 2009-11-27
     */
    int bNeedSanitize = 0;
    for (iSrc = 0; iSrc < lenMsg; iSrc++)
    {
        if (pszMsg[iSrc] < 32)
        {
            ...
            else if (pszMsg[iSrc] == '\0' ||
                     glbl.GetParserEscapeControlCharactersOnReceive())
            {
                bNeedSanitize = 1;
        ...
        else if (pszMsg[iSrc] > 127 &&
                 glbl.GetParserEscape8BitCharactersOnReceive())
        {
            bNeedSanitize = 1;
            break;
        }
    ...
    /* now copy over the message and sanitize it. Note that up to iSrc-1 there
     * was obviously no need to sanitize, so we can go over that quickly...
     */
    ...
    iDst = iSrc;
    while (iSrc < lenMsg && iDst < maxDest - 3)
    { /* leave some space if last char must be escaped */
        if ((pszMsg[iSrc] < 32) &&
            (pszMsg[iSrc] != '\t' || glbl.GetParserEscapeControlCharacterTab()))
        {
                ...
                if (glbl.GetParserEscapeControlCharactersCStyle())
                {
                    ...
                    switch (pszMsg[iSrc])
                    {
                        ...
                        case '\b':
                            pDst[iDst++] = 'b';
                            break;
                        case '\x1b': /* equivalent to '\e' which is not accepted
                                        by XLC */
                            pDst[iDst++] = 'e';
                            break;
                        case '\f':
...
static rsRetVal ParsePRI(smsg_t* pMsg)
{
    ...
    else
    {
        ...
        msgSetPRI(pMsg, pri);
        MsgSetAfterPRIOffs(pMsg,
                           (pri == LOG_PRI_INVLD) ? 0 : msg - pMsg->pszRawMsg);
    }
    RETiRet;
}
...
static rsRetVal ParseMsg(smsg_t* pMsg)
{
    ...
    /* we take the risk to print a non-sanitized string, because this is the
     * best we can get (and that functionality is too important for debugging to
     * drop it...).
     */
    DBGPRINTF("msg parser: flags %x, from '%s', msg '%.60s'\n", pMsg->msgFlags,
              (pMsg->msgFlags & NEEDS_DNSRESOL)
                  ? UCHAR_CONSTANT("~NOTRESOLVED~")
                  : getRcvFrom(pMsg),
              pMsg->pszRawMsg);

    /* we now need to go through our list of parsers and see which one is
     * capable of parsing the message. Note that the first parser that requires
     * message sanitization will cause it to happen. After that, access to the
     * unsanitized message is no loger possible.
     */
    ...
    /* We need to log a warning message and drop the message if we did not find
     * a parser. Note that we log at most the first 1000 message, as this may
     * very well be a problem that causes a message generation loop. We do not
     * synchronize that counter, it doesn't matter if we log a handful messages
     * more than we should...
     */
    if (localRet != RS_RET_OK)
    {
        if (++iErrMsgRateLimiter < 1000)
        {
            LogError(0, localRet,
                     ...
                     "any parser, message is being discarded (start of raw "
                     "msg: '%.60s')",
                     pMsg->pszRawMsg);
        }
        DBGPRINTF("No parser could process the message (state %d), we need to "
                  "discard it.\n",
                  localRet);
...
BEGINobjQueryInterface(parser)
    CODESTARTobjQueryInterface(parser) if (pIf->ifVersion !=
                                           parserCURR_IF_VERSION)
...
finalize_it : ENDobjQueryInterface(parser)

              /* This destroys the master parserlist and all of its parser
               * entries. MUST only be done when the module is shut down. Parser
               * modules are NOT unloaded, rsyslog does that at a later stage
               * for all dynamically loaded modules.
               */
              static void destroyMasterParserList(void)
{
    parserList_t* pParsLst;
    parserList_t* pParsLstDel;
