...
{
    printf("Usage: %s [OPTIONS] <chip name/number> <offset1>=<value1> "
           "<offset2>=<value2> ...\n",
           get_progname());
    ...
    printf("  -s, --sec=SEC:\tspecify the number of seconds to wait (only "
           "valid for --mode=time)\n");
    printf("  -u, --usec=USEC:\tspecify the number of microseconds to wait "
           "(only valid for --mode=time)\n");
    printf("  -b, --background:\tafter setting values: detach from the "
    ...
    printf("\n");
    printf("Note: the state of a GPIO line controlled over the character "
           "device reverts to default\n");
    printf("when the last process referencing the file descriptor representing "
           "the device file exits.\n");
    printf("This means that it's wrong to run gpioset, have it exit and expect "
           "the line to continue\n");
    printf("being driven high or low. It may happen if given pin is floating "
...
int main(int argc, char** argv)
{
    ...
    for (;;)
    {
        ...
        switch (optc)
        {
            ...
            case 'u':
                ...
                if (*end != '\0')
                    die("invalid time value in microseconds: %s", optarg);
                break;
            case 'b':
    ...
    if (mode->id != MODE_SIGNAL && mode->id != MODE_TIME && cbdata.daemonize)
    ...
    rv = gpiod_ctxless_set_value_multiple(device, offsets, values, num_lines,
                                          active_low, "gpioset", mode->callback,
                                          &cbdata);
    if (rv < 0)
