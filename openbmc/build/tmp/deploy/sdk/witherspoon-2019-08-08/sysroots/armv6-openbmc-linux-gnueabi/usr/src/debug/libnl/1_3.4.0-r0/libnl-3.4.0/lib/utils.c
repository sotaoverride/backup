...
char* nl_size2str(const size_t size, char* buf, const size_t len)
{
    ...
    for (i = 0; i < ARRAY_SIZE(size_units); i++)
    {
        ...
        {
            snprintf(buf, len, "%.2g%s", (double)size / size_units[i].limit,
                     size_units[i].unit);
            return buf;
        }
    }
...
static void get_psched_settings(void)
{
    ...
    else
    {
        ...
        if ((fd = fopen(name, "re")))
        {
            ...
            if (fscanf(fd, "%08x %08x %08x %08x", &ns_per_usec, &ns_per_tick,
                       &nom, &denom) != 4)
            {
                NL_DBG(
                    1,
            ...
            ticks_per_usec = (double)ns_per_usec / (double)ns_per_tick;
...
#undef _SPLIT
    ...
    for (i = 0; i < ARRAY_SIZE(split) && len; i++)
    {
        ...
        if (split[i] == 0)
            continue;
        l = snprintf(buf, len, "%s%" PRIu64 "%s", (buf == buf_orig) ? "" : " ",
                     split[i], units[i]);
        buf += l;
        len -= l;
    }
...
{
    return __type2str(family, buf, size, nlfamilies, ARRAY_SIZE(nlfamilies));
}
...
{
    return __type2str(eproto, buf, len, ether_protos, ARRAY_SIZE(ether_protos));
}
        ...
        for (i = 0; i < params->dp_prefix; i++)
        {
            if (params->dp_fd)
                ...
                strncat(params->dp_buf, " ",
                        params->dp_buflen - strlen(params->dp_buf) - 1);
        }
    }
...
char* __type2str(int type, char* buf, size_t len, const struct trans_tbl* tbl,
                 size_t tbl_len)
{
    size_t i;
    for (i = 0; i < tbl_len; i++)
...
char* __flags2str(int flags, char* buf, size_t len, const struct trans_tbl* tbl,
                  size_t tbl_len)
{
    size_t i;
    int tmp = flags;
...
#define _NL_ASSERT(expr)                                                       \
    (0 * sizeof(struct { unsigned int x : ((!!(expr)) ? 1 : -1); }))
#define _NL_SETV(i, r, v)                                                      \
    (_NL_ASSERT((v) == 0 || (i)*8 + (r) == (v)-1) + ((v) == 0 ? 0 : (1 << (r))))
#define _NL_SET(i, v0, v1, v2, v3, v4, v5, v6, v7)                             \
    [(i)] = (_NL_SETV((i), 0, (v0)) | _NL_SETV((i), 4, (v4)) |                 \
             _NL_SETV((i), 1, (v1)) | _NL_SETV((i), 5, (v5)) |                 \
             _NL_SETV((i), 2, (v2)) | _NL_SETV((i), 6, (v6)) |                 \
             _NL_SETV((i), 3, (v3)) | _NL_SETV((i), 7, (v7)))
        _NL_SET(0, NL_CAPABILITY_ROUTE_BUILD_MSG_SET_SCOPE,
        ...
        _NL_SET(1, NL_CAPABILITY_LINK_BUILD_CHANGE_REQUEST_SET_CHANGE,
        ...
        _NL_SET(
            2, NL_CAPABILITY_XFRM_SA_KEY_SIZE, NL_CAPABILITY_RTNL_ADDR_PEER_FIX,
            NL_CAPABILITY_VERSION_3_2_28, NL_CAPABILITY_RTNL_ADDR_PEER_ID_FIX,
            NL_CAPABILITY_NL_ADDR_FILL_SOCKADDR, NL_CAPABILITY_XFRM_SEC_CTX_LEN,
            NL_CAPABILITY_LINK_BUILD_ADD_REQUEST_SET_CHANGE,
            NL_CAPABILITY_NL_RECVMSGS_PEEK_BY_DEFAULT),
        _NL_SET(3, NL_CAPABILITY_VERSION_3_2_29,
                NL_CAPABILITY_XFRM_SP_SEC_CTX_LEN, NL_CAPABILITY_VERSION_3_3_0,
                NL_CAPABILITY_VERSION_3_4_0, 0, 0, 0, 0),
    /* IMPORTANT: these capability numbers are intended to be universal and
     * stable for libnl3. Don't allocate new numbers on your own that differ
     * from upstream libnl3.
     *
     * Instead register a capability number upstream too. We will take patches
     * for that. We especially take patches to register a capability number that
     * is only implemented in your fork of libnl3.
     *
     * If you really don't want that, use capabilities in the range 0x7000 to
     * 0x7FFF. (NL_CAPABILITY_IS_USER_RESERVED). Upstream libnl3 will not
     * register conflicting capabilities in that range.
     *
     * Obviously, only backport capability numbers to libnl versions that
     * actually implement that capability as well. */
#undef _NL_SET
#undef _NL_SETV
#undef _NL_ASSERT
    };
