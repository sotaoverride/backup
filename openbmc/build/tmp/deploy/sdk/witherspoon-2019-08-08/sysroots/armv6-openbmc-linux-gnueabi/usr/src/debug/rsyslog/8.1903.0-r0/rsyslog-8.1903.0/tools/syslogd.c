...
/* rgerhards, 2005-10-24: crunch_list is called only during option processing.
 * So it is never called once rsyslogd is running. This code contains some
 * exits, but they are considered safe because they only happen during startup.
 * Anyhow, when we review the code here, we might want to reconsider the
 * exit()s. Note: this stems back to sysklogd, so we cannot put it under
 * ASL 2.0. But we may want to check if the code inside the BSD sources is
 * exactly the same (remember that sysklogd forked the BSD sources). If so, the
 * BSD license applies and permits us to move to ASL 2.0 (but we need to check
 * the fine details). Probably it is best just to rewrite this code.
 */
...
char** syslogd_crunch_list(char* list)
{
    ...
    if ((result[count] = (char*)malloc(strlen(p) + 1)) == NULL)
    {
        printf("Sorry, can't get enough memory, exiting.\n");
        exit(0); /* safe exit, because only called during startup */
    }
...
#else
            /* TODO: we need to implement something for HP UX! -- rgerhards,
             * 2008-03-04 */
            /* actually, HP UX should have setsid, so the code directly above
             * should trigger. So the actual question is why it doesn't do
             * that...
             */
#endif
            close(i);
