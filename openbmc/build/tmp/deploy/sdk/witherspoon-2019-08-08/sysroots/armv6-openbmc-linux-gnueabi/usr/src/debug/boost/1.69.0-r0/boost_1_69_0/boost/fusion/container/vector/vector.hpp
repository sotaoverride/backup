...
template <typename This, typename T, typename T_, std::size_t Size, bool IsSeq>
...
struct can_convert_impl<This, Sequence, T, 1, true>
    : integral_constant<
          bool,
          !is_convertible<
              Sequence, typename fusion::extension::value_at_impl<vector_tag>::
                            template apply<This, mpl::int_<0>>::type>::value>
{
};

template <typename This, typename T, typename T_, std::size_t Size>
struct can_convert
    : can_convert_impl<This, T, T_, Size, traits::is_sequence<T_>::value>
{
};
...
struct is_longer_sequence_impl<Sequence, true, Size>
    : integral_constant<bool,
                        (fusion::result_of::size<Sequence>::value >= Size)>
{
};
...
struct forward_at_c
    : fusion::result_of::deref<typename fusion::result_of::advance_c<
          typename fusion::result_of::begin<
              typename remove_reference<Sequence>::type>::type,
          N>::type>
{
};
} // namespace result_of
...
BOOST_CONSTEXPR BOOST_FUSION_GPU_ENABLED inline
    typename result_of::forward_at_c<Sequence, N>::type
    forward_at_c(Sequence&& seq)
{
    typedef typename result_of::forward_at_c<Sequence, N>::type result;
...
struct store
{
    BOOST_CONSTEXPR BOOST_FUSION_GPU_ENABLED store() :
        elem() // value-initialized explicitly
    {
    }

    BOOST_CONSTEXPR BOOST_FUSION_GPU_ENABLED store(store const& rhs) :
        elem(rhs.elem)
    {
    }

    BOOST_CXX14_CONSTEXPR BOOST_FUSION_GPU_ENABLED store&
    ...
    BOOST_CONSTEXPR BOOST_FUSION_GPU_ENABLED store(store&& rhs) :
        elem(static_cast<T&&>(rhs.elem))
    {
    }

    BOOST_CXX14_CONSTEXPR BOOST_FUSION_GPU_ENABLED store& operator=(store&& rhs)
    {
        elem = static_cast<T&&>(rhs.elem);
        return *this;
    }

    template <typename U, typename = typename boost::disable_if<is_base_of<
                              store, typename remove_reference<U>::type>>::type>
    BOOST_CONSTEXPR BOOST_FUSION_GPU_ENABLED store(U&& rhs) :
...
static inline BOOST_FUSION_GPU_ENABLED store<J, U> store_at_impl(store<J, U>*);
      ...
      sequence_base<vector_data<detail::index_sequence<I...>, T...>>
{
    ...
    BOOST_CONSTEXPR
        BOOST_FUSION_GPU_ENABLED BOOST_DEFAULTED_FUNCTION(vector_data(), {})

            template <
                typename Sequence,
                typename Sequence_ = typename remove_reference<Sequence>::type,
                typename = typename boost::enable_if<can_convert<
                    vector_data, Sequence, Sequence_, sizeof...(I)>>::type>
    ...
    template <typename... U>
    BOOST_CONSTEXPR BOOST_FUSION_GPU_ENABLED explicit vector_data(each_elem,
                                                                  U&&... var) :
    ...
    BOOST_CXX14_CONSTEXPR BOOST_FUSION_GPU_ENABLED void
  ...
  private:
    ...
    using store_at =
        decltype(store_at_impl<J>(static_cast<vector_data*>(nullptr)));
          ...
          typename detail::make_index_sequence<sizeof...(T)>::type, T...>
{
    ...
    BOOST_CONSTEXPR BOOST_FUSION_GPU_ENABLED BOOST_DEFAULTED_FUNCTION(vector(),
                                                                      {})
        ...
        template <typename... U,
                  typename = typename boost::enable_if_c<
                      (sizeof...(U) >= 1 &&
                       ...
                       !fusion::detail::and_<is_base_of<
                           vector, typename remove_reference<U>::type>...>::
                           value)>::type>
    ...
    template <typename Sequence,
              typename =
                  typename boost::enable_if_c<vector_detail::is_longer_sequence<
                      typename remove_reference<Sequence>::type,
                      sizeof...(T)>::value>::type>
    BOOST_CONSTEXPR BOOST_FUSION_GPU_ENABLED vector(Sequence&& seq) :
        base(vector_detail::each_elem(), std::forward<Sequence>(seq))
    {
    }

    template <typename Sequence>
    BOOST_CXX14_CONSTEXPR BOOST_FUSION_GPU_ENABLED vector&
