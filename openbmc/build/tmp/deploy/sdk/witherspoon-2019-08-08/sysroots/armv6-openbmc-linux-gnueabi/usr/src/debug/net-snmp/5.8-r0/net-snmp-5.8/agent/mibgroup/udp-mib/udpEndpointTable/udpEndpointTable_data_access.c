...
{
    DEBUGMSGTL(
        ("verbose:udpEndpointTable:udpEndpointTable_init_data", "called\n"));
...
{
    DEBUGMSGTL(("verbose:udpEndpointTable:udpEndpointTable_container_shutdown",
                "called\n"));
 ...
 *
 */
static u_long _address_type_from_len(int addrlen)
{
    switch (addrlen)
    {
        case 4:
...
int udpEndpointTable_container_load(netsnmp_container* container)
{
         ...
         ep = (netsnmp_udp_endpoint_entry*)ITERATOR_NEXT(ep_it))
    {
        ...
        /*
         * TODO:352:M: |   |-> set indexes in new udpEndpointTable rowreq
         * context.
         */
        ...
        if (MFD_SUCCESS != udpEndpointTable_indexes_set(
                               rowreq_ctx, udpEndpointLocalAddressType,
                               (char*)ep->loc_addr, ep->loc_addr_len,
                               ep->loc_port, udpEndpointRemoteAddressType,
                               (char*)ep->rmt_addr, ep->rmt_addr_len,
                               ep->rmt_port, ep->instance, ep->pid))
        {
...
{
    DEBUGMSGTL(
        ("verbose:udpEndpointTable:udpEndpointTable_row_prep", "called\n"));
