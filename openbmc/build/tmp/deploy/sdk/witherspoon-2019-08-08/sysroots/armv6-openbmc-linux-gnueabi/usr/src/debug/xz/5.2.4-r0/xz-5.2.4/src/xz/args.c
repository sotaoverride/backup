...
static void parse_memlimit(const char* name, const char* name_percentage,
                           char* str, bool set_compress, bool set_decompress)
{
    ...
    hardware_memlimit_set(value, set_compress, set_decompress, is_percentage);
    return;
}
...
static void parse_block_list(char* str)
{
    ...
    if (count > SIZE_MAX / sizeof(uint64_t) - 1)
        message_fatal(_("%s: Too many arguments to --block-list"), str);
    ...
    for (size_t i = 0; i < count; ++i)
    {
        ...
        {
            opt_block_list[i] = str_to_uint64("block-list", str, 0, UINT64_MAX);
...
static void parse_real(args_info* args, int argc, char** argv)
{
    ...
    static const char short_opts[] = "cC:defF:hHlkM:qQrS:tT:vVz0123456789";
    ...
    while ((c = getopt_long(argc, argv, short_opts, long_opts, NULL)) != -1)
    {
        switch (c)
        {
            ...
            case OPT_MEM_COMPRESS:
                parse_memlimit("memlimit-compress", "memlimit-compress%",
                               optarg, true, false);
                break;

            // --memlimit-decompress
            case OPT_MEM_DECOMPRESS:
                parse_memlimit("memlimit-decompress", "memlimit-decompress%",
                               optarg, false, true);
                break;

            // --memlimit
            case 'M':
                parse_memlimit("memlimit", "memlimit%", optarg, true, true);
                break;
            ...
            case 'T':
                ...
                hardware_threads_set(
                    str_to_uint64("threads", optarg, 0, 16384));
                break;
            ...
            case OPT_X86:
                coder_add_filter(LZMA_FILTER_X86, options_bcj(optarg));
                break;

            case OPT_POWERPC:
                coder_add_filter(LZMA_FILTER_POWERPC, options_bcj(optarg));
                break;

            case OPT_IA64:
                coder_add_filter(LZMA_FILTER_IA64, options_bcj(optarg));
                break;

            case OPT_ARM:
                coder_add_filter(LZMA_FILTER_ARM, options_bcj(optarg));
                break;

            case OPT_ARMTHUMB:
                coder_add_filter(LZMA_FILTER_ARMTHUMB, options_bcj(optarg));
                break;

            case OPT_SPARC:
                coder_add_filter(LZMA_FILTER_SPARC, options_bcj(optarg));
                break;
            ...
            case OPT_BLOCK_SIZE:
                opt_block_size =
                    str_to_uint64("block-size", optarg, 0, LZMA_VLI_MAX);
                break;
            ...
            case OPT_FILES0:
                ...
                else
                {
                    ...
                    args->files_file =
                        fopen(optarg, c == OPT_FILES ? "r" : "rb");
                    if (args->files_file == NULL)
                        message_fatal("%s: %s", optarg, strerror(errno));
                }
            ...
            case OPT_FLUSH_TIMEOUT:
                opt_flush_timeout =
                    str_to_uint64("flush-timeout", optarg, 0, UINT64_MAX);
                break;
...
static void parse_environment(args_info* args, char* argv0, const char* varname)
{
    ...
    for (size_t i = 0; env[i] != '\0'; ++i)
    {
        ...
        else if (prev_was_space)
        {
            ...
            if (++argc == my_min(INT_MAX, SIZE_MAX / sizeof(char*)))
