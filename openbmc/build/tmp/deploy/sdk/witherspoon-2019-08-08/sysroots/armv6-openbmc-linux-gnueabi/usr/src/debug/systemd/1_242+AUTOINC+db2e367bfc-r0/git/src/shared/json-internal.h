...
{
    /* Encodes a simple value. On x86-64 this structure is 16 bytes wide (as
     * long double is 128bit). */
    bool boolean;
    long double real;
    intmax_t integer;
    uintmax_t unsig;
...
/* Let's protect us against accidental structure size changes on our most
 * relevant arch */
#ifdef __x86_64__
...
/* We use fake JsonVariant objects for some special values, in order to avoid
 * memory allocations for them. Note that effectively this means that there are
 * multiple ways to encode the same objects: via these magic values or as
 * properly allocated JsonVariant. We convert between both on-the-fly as
 * necessary. */
...
/* This is only safe as long as we don't define more than 4K magic pointers,
 * i.e. the page size of the simplest architectures we support. That's because
 * we rely on the fact that malloc() will never allocate from the first memory
...
int json_tokenize(const char** p, char** ret_string, JsonValue* ret_value,
                  unsigned* ret_line, unsigned* ret_column, void** state,
