...
static int ssl_srp_server_param_cb(SSL* s, int* ad, void* arg)
{
    ...
    if (SSL_set_srp_server_param(s, p->user->N, p->user->g, p->user->s,
                                 p->user->v, p->user->info) < 0)
    {
...
static const BIO_METHOD* BIO_f_ebcdic_filter()
{
    ...
    {
        methods_ebcdic =
            BIO_meth_new(BIO_TYPE_EBCDIC_FILTER, "EBCDIC/ASCII filter");
        if (methods_ebcdic == NULL ||
                                        ...
                                        OCSP_RESPONSE** resp)
{
    ...
    *resp =
        process_responder(req, host, path, port, use_ssl, NULL, srctx->timeout);
    if (*resp == NULL)
    {
...
const OPTIONS s_server_options[] = {
    ...
    {"accept", OPT_ACCEPT, 's',
     "TCP/IP optional host and port to listen on for connections (default is "
     "*:" PORT ")"},
#ifdef AF_UNIX
    ...
    {"key", OPT_KEY, 's', "Private Key if not in -cert; default is " TEST_CERT},
    ...
    {"nextprotoneg", OPT_NEXTPROTONEG, 's',
     "Set the advertised protocols for the NPN extension (comma-separated "
     "list)"},
#endif
...
#endif
    {"alpn", OPT_ALPN, 's',
     "Set the advertised protocols for the ALPN extension (comma-separated "
     "list)"},
    ...
    {"num_tickets", OPT_S_NUM_TICKETS, 'n',
     ...
     "issue"},
    {"anti_replay", OPT_ANTI_REPLAY, '-',
     "Switch on anti-replay protection (default)"},
    {"no_anti_replay", OPT_NO_ANTI_REPLAY, '-',
...
int s_server_main(int argc, char* argv[])
{
    ...
    const char *CApath = NULL, *CAfile = NULL, *chCApath = NULL,
               *chCAfile = NULL;
    ...
    const char *s_cert_file = TEST_CERT, *s_key_file = NULL,
               *s_chain_file = NULL;
    ...
    while ((o = opt_next()) != OPT_EOF)
    {
        ...
        switch (o)
        {
            ...
            case OPT_PORT:
                ...
                OPENSSL_free(host);
                host = NULL;
                if (BIO_parse_hostserv(opt_arg(), NULL, &port,
            ...
            case OPT_ACCEPT:
                ...
                OPENSSL_free(host);
                host = NULL;
                if (BIO_parse_hostserv(opt_arg(), &host, &port,
                                       BIO_PARSE_PRIO_SERV) < 1)
                {
                    BIO_printf(bio_err,
    ...
    {
        BIO_printf(bio_err, "Can't use -early_data in combination with -www, "
                            "-WWW, -HTTP, or -rev\n");
        goto end;
    }
    ...
    if (nocert == 0)
    {
        ...
        s_cert =
            load_cert(s_cert_file, s_cert_format, "server certificate file");
    ...
    if (s_dcert_file != NULL)
    {
        ...
        s_dkey = load_key(s_dkey_file, s_dkey_format, 0, dpass, engine,
    ...
    if (max_pipelines > 0 && !SSL_CTX_set_max_pipelines(ctx, max_pipelines))
    {
    ...
    if (ctx2 != NULL)
    {
        ...
        if (!ctx_set_verify_locations(ctx2, CAfile, CApath, noCAfile, noCApath))
        {
            ERR_print_errors(bio_err);
            goto end;
        }
    ...
    if (ctx2 != NULL)
    {
        ...
        if (!SSL_CTX_set_session_id_context(
                ctx2, (void*)&s_server_session_id_context,
                sizeof(s_server_session_id_context)))
        {
            BIO_printf(bio_err, "error setting session id context\n");
            ERR_print_errors(bio_err);
    ...
    if (srp_verifier_file != NULL)
    {
        ...
        if ((ret = SRP_VBASE_init(srp_callback_parm.vb, srp_verifier_file)) !=
            SRP_NO_ERROR)
        {
            BIO_printf(bio_err,
...
static int sv_body(int s, int stype, int prot, unsigned char* context)
{
    ...
    for (;;)
    {
        ...
        read_from_sslcon =
            SSL_has_pending(con) || (async && SSL_waiting_for_async(con));
        ...
        if (read_from_terminal)
        {
            ...
            for (;;)
            {
                ...
                while (SSL_get_error(con, k) == SSL_ERROR_WANT_X509_LOOKUP)
                {
                    ...
                    srp_callback_parm.user = SRP_VBASE_get1_by_user(
                        srp_callback_parm.vb, srp_callback_parm.login);
                ...
                switch (SSL_get_error(con, k))
                {
                    ...
                    case SSL_ERROR_WANT_ASYNC_JOB:
                        /*
                         * This shouldn't ever happen in s_server. Treat as an
                         * error
                         */
...
static int is_retryable(SSL* con, int i)
{
    ...
    return (err != SSL_ERROR_SSL) && (err != SSL_ERROR_SYSCALL) &&
                               ...
                               unsigned int* id_len)
{
    ...
    do
    {
        ...
        memcpy(id, session_id_prefix,
               (strlen(session_id_prefix) < *id_len) ? strlen(session_id_prefix)
                                                     : *id_len);
