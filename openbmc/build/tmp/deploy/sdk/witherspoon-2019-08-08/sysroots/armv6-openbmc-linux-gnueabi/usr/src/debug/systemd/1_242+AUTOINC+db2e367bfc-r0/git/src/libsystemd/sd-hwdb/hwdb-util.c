...
static const char* const conf_file_dirs[] = {"/etc/udev/hwdb.d",
                                             UDEVLIBEXECDIR "/hwdb.d", NULL};
...
static int trie_children_cmp(const struct trie_child_entry* a,
                             const struct trie_child_entry* b)
{
    return CMP(a->c, b->c);
}

static int node_add_child(struct trie* trie, struct trie_node* node,
                          struct trie_node* node_child, uint8_t c)
{
    struct trie_child_entry* child;

    /* extend array, add new entry, sort for bisection */
    child = reallocarray(node->children, node->children_count + 1,
...
static struct trie_node* node_lookup(const struct trie_node* node, uint8_t c)
{
    ...
    search.c = c;
    child = typesafe_bsearch(&search, node->children, node->children_count,
                             trie_children_cmp);
    if (child)
        return child->child;
    return NULL;
...
static int trie_values_cmp(const struct trie_value_entry* a,
                           const struct trie_value_entry* b, struct trie* trie)
{
...
static int trie_node_add_value(struct trie* trie, struct trie_node* node,
                               ...
                               const char* filename, uint16_t file_priority,
                               uint32_t line_number, bool compat)
{
    ssize_t k, v, fn = 0;
    ...
    if (node->values_count)
    {
        ...
        val = typesafe_bsearch_r(&search, node->values, node->values_count,
                                 trie_values_cmp, trie);
        if (val)
        {
            /* At this point we have 2 identical properties on the same
             * match-string. Since we process files in order, we just replace
             * the previous value. */
            val->value_off = v;
            val->filename_off = fn;
    ...
    /* extend array, add new entry, sort for bisection */
    val = reallocarray(node->values, node->values_count + 1,
...
static int trie_insert(struct trie* trie, struct trie_node* node,
                       const char* search, const char* key, const char* value,
                       const char* filename, uint16_t file_priority,
                       uint32_t line_number, bool compat)
{
    size_t i = 0;
    ...
    for (;;)
    {
        ...
        c = search[i];
        if (c == '\0')
            return trie_node_add_value(trie, node, key, value, filename,
                                       file_priority, line_number, compat);
        ...
        if (!child)
        {
            ...
            *new_child = (struct trie_node){
            ...
            return trie_node_add_value(trie, child, key, value, filename,
                                       file_priority, line_number, compat);
        }
                                ...
                                bool compat)
{
    ...
    if (node->children_count)
    {
        fwrite(children, sizeof(struct trie_child_entry_f),
               node->children_count, trie->f);
...
static int trie_store(struct trie* trie, const char* filename, bool compat)
{
    ...
    struct trie_header_f h = {
        ...
        .value_entry_size =
            htole64(compat ? sizeof(struct trie_value_entry_f)
                           : sizeof(struct trie_value_entry2_f)),
    };
    int r;
    ...
    log_debug("value pointers:   %8" PRIu64 " bytes (%8" PRIu64 ")",
              t.values_count * (compat ? sizeof(struct trie_value_entry_f)
                                       : sizeof(struct trie_value_entry2_f)),
              t.values_count);
                       ...
                       uint32_t line_number, bool compat)
{
    ...
    if (isempty(line + 1) || isempty(value))
        return log_syntax(NULL, LOG_WARNING, filename, line_number, EINVAL,
                          ...
                          isempty(line + 1) ? "key" : "value", line, value);

    STRV_FOREACH(entry, match_list)
    trie_insert(trie, trie->root, *entry, line, value, filename, file_priority,
                line_number, compat);
...
static int import_file(struct trie* trie, const char* filename,
                       uint16_t file_priority, bool compat)
{
    enum
    {
        HW_NONE,
    ...
    for (;;)
    {
        ...
        switch (state)
        {
            case HW_NONE:
                ...
                if (line[0] == ' ')
                {
                    log_syntax(NULL, LOG_WARNING, filename, line_number, EINVAL,
                               "Match expected but got indented property "
                               "\"%s\", ignoring line",
                               line);
                    r = -EINVAL;
                    break;
            ...
            case HW_MATCH:
                ...
                {
                    log_syntax(NULL, LOG_WARNING, filename, line_number, EINVAL,
                               "Property expected, ignoring record with no "
                               "properties");
                    r = -EINVAL;
                    state = HW_NONE;
            ...
            case HW_DATA:
                ...
                {
                    log_syntax(NULL, LOG_WARNING, filename, line_number, EINVAL,
                               "Property or empty line expected, got \"%s\", "
                               "ignoring record",
                               line);
                    r = -EINVAL;
                    state = HW_NONE;
...
int hwdb_update(const char* root, const char* hwdb_bin_dir, bool strict,
                bool compat)
{
    ...
    /* The argument 'compat' controls the format version of database. If false,
     * then hwdb.bin will be created with additional information such that
     * priority, line number, and filename of database source. If true, then
     * hwdb.bin will be created without the information. systemd-hwdb command
     * should set the argument false, and 'udevadm hwdb' command should set it
     * true. */
    ...
    log_debug("strings incoming: %8zu bytes (%8zu)", trie->strings->in_len,
              trie->strings->in_count);
    log_debug("strings dedup'ed: %8zu bytes (%8zu)", trie->strings->dedup_len,
              trie->strings->dedup_count);

    hwdb_bin =
        path_join(root, hwdb_bin_dir ?: default_hwdb_bin_dir, "hwdb.bin");
    if (!hwdb_bin)
        return -ENOMEM;
    ...
    if (err < 0)
        return log_error_errno(err, "Failed to write database %s: %m",
                               hwdb_bin);
