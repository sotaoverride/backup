...
static void system_parse_config_sysdescr(const char* token, char* cptr)
{
...
{
    system_parse_config_string(token, cptr, "sysName", sysName, sizeof(sysName),
                               &sysNameSet);
}

static void system_parse_config_sysServices(const char* token, char* cptr)
{
    sysServices = atoi(cptr);
    sysServicesConfiged = 1;
}

static void system_parse_config_sysObjectID(const char* token, char* cptr)
{
...
static int system_store(int a, int b, void* c, void* d)
{
...
static int handle_sysServices(netsnmp_mib_handler* handler,
...
static int handle_sysUpTime(netsnmp_mib_handler* handler,
                            netsnmp_handler_registration* reginfo,
                            netsnmp_agent_request_info* reqinfo,
                            netsnmp_request_info* requests)
{
...
void init_system_mib(void)
{
    ...
    uname(&utsName);
    snprintf(version_descr, sizeof(version_descr), "%s %s %s %s %s",
    ...
    }
    {
        ...
        netsnmp_register_watched_scalar(
            netsnmp_create_handler_registration(
                "mibII/sysObjectID", NULL, sysObjectID_oid,
                OID_LENGTH(sysObjectID_oid), HANDLER_CAN_RONLY),
            netsnmp_init_watcher_info6(&sysObjectID_winfo, sysObjectID, 0,
                                       ASN_OBJECT_ID,
    ...
    }
    {
        ...
        netsnmp_register_scalar(netsnmp_create_handler_registration(
            "mibII/sysUpTime", handle_sysUpTime, sysUpTime_oid,
            OID_LENGTH(sysUpTime_oid), HANDLER_CAN_RONLY));
    }
    {
        ...
        netsnmp_register_watched_scalar(
            ...
            netsnmp_init_watcher_info(&sysContact_winfo, sysContact,
                                      SYS_STRING_LEN - 1, ASN_OCTET_STR,
    ...
    }
    {
        ...
        netsnmp_register_watched_scalar(
            ...
            netsnmp_init_watcher_info(&sysName_winfo, sysName,
                                      SYS_STRING_LEN - 1, ASN_OCTET_STR,
                                      WATCHER_MAX_SIZE | WATCHER_SIZE_STRLEN));
#else  /* !NETSNMP_NO_WRITE_SUPPORT */
        netsnmp_register_watched_scalar(
    ...
    }
    {
        ...
        netsnmp_register_watched_scalar(
            netsnmp_create_update_handler_registration(
                ...
                OID_LENGTH(sysLocation_oid), HANDLER_CAN_RWRITE,
                &sysLocationSet),
            netsnmp_init_watcher_info(&sysLocation_winfo, sysLocation,
                                      SYS_STRING_LEN - 1, ASN_OCTET_STR,
                                      WATCHER_MAX_SIZE | WATCHER_SIZE_STRLEN));
#else  /* !NETSNMP_NO_WRITE_SUPPORT */
        netsnmp_register_watched_scalar(
            netsnmp_create_update_handler_registration(
                "mibII/sysLocation", sysLocation_oid,
                OID_LENGTH(sysLocation_oid), HANDLER_CAN_RONLY,
                &sysLocationSet),
            netsnmp_init_watcher_info(&sysLocation_winfo, sysLocation,
                                      SYS_STRING_LEN - 1, ASN_OCTET_STR,
                                      WATCHER_MAX_SIZE | WATCHER_SIZE_STRLEN));
    ...
    snmpd_register_config_handler("syscontact", system_parse_config_syscon,
                                  NULL, "contact-name");
    snmpd_register_config_handler("sysname", system_parse_config_sysname, NULL,
                                  "node-name");
    ...
    snmpd_register_config_handler("psyscontact", system_parse_config_syscon,
                                  NULL, NULL);
    snmpd_register_config_handler("psysname", system_parse_config_sysname, NULL,
                                  NULL);
    snmpd_register_config_handler(
        "sysservices", system_parse_config_sysServices, NULL, "NUMBER");
    snmpd_register_config_handler("sysobjectid",
                                  system_parse_config_sysObjectID, NULL, "OID");
...
static void windowsOSVersionString(char stringbuf[], size_t stringbuflen)
{
    ...
    dwMajorVersion =
        RegReadDword(HKEY_LOCAL_MACHINE, wcv, "CurrentMajorVersionNumber");
    dwMinorVersion =
        RegReadDword(HKEY_LOCAL_MACHINE, wcv, "CurrentMinorVersionNumber");
    if (!RegReadString(HKEY_LOCAL_MACHINE, wcv, "CurrentBuildNumber", build,
                       &buildSz))
        RegReadString(HKEY_LOCAL_MACHINE, wcv, "CurrentBuild", build, &buildSz);
    ...
    snprintf(stringbuf, stringbuflen, "Windows %s %d.%d.%s %s %s", hostname,
             (int)dwMajorVersion, (int)dwMinorVersion, build, windowsVersion,
             identifier);
}
