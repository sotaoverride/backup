...
static errcode_t add_link(ext2_filsys fs, ext2_ino_t parent_ino, ext2_ino_t ino,
                          const char* name)
{
    ...
    retval =
        ext2fs_link(fs, parent_ino, name, ino, ext2_file_type(inode.i_mode));
    if (retval == EXT2_ET_DIR_NO_SPACE)
    {
        retval = ext2fs_expand_dir(fs, parent_ino);
        if (retval)
        {
            com_err(__func__, retval, _("while expanding directory"));
            return retval;
        }
    ...
    for (i = 0; i < size; i += strlen(&list[i]) + 1)
    {
        ...
        if (value_size == -1)
        {
            ...
            com_err(__func__, retval,
                    _("while reading attribute \"%s\" of \"%s\""), name,
                    filename);
            break;
        }
        ...
        {
            com_err(__func__, retval,
                    _("while writing attribute \"%s\" to inode %u"), name, ino);
            break;
        }
    }
out:
                            ...
                            unsigned int st_mode, unsigned int st_rdev)
{
    ...
    if (retval)
    {
        com_err(__func__, retval, _("while allocating inode \"%s\""), name);
        return retval;
    }
                              ...
                              char* target, ext2_ino_t root)
{
    ...
    if (retval)
        com_err("ext2fs_symlink", retval, _("while creating symlink \"%s\""),
                name);
    return retval;
}
                            ...
                            ext2_ino_t root)
{
    ...
    if (cp)
    {
        *cp = 0;
        ...
        if (retval)
        {
            com_err(name, retval, _("while looking up \"%s\""), name);
            return retval;
        }
        name = cp + 1;
    }
    ...
    if (retval)
        com_err("ext2fs_mkdir", retval, _("while creating directory \"%s\""),
                name);
    return retval;
}
                                 ...
                                 char* zerobuf)
{
    ...
    for (off = start; off < end; off += COPY_FILE_BUFLEN)
    {
        ...
        for (bpos = 0, ptr = buf; bpos < got; bpos += fs->blocksize)
        {
            ...
            err = ext2fs_file_llseek(e2_file, off + bpos, EXT2_SEEK_SET, NULL);
            if (err)
                goto fail;
            while (blen > 0)
            {
                err = ext2fs_file_write(e2_file, ptr, blen, &written);
                if (err)
                    goto fail;
                if (written == 0)
                {
                                ...
                                ext2_file_t e2_file, char* buf, char* zerobuf)
{
    ...
    while (data < statbuf->st_size)
    {
        ...
        hole_blk =
            (hole + (off_t)(fs->blocksize - 1)) & ~(off_t)(fs->blocksize - 1);
        err =
            copy_file_chunk(fs, fd, e2_file, data_blk, hole_blk, buf, zerobuf);
        if (err)
            return err;
...
#define EXTENT_MAX_COUNT 512
    ...
    do
    {
        ...
        {
            err =
                copy_file_chunk(fs, fd, e2_file, ext->fe_logical,
                                ext->fe_logical + ext->fe_length, buf, zerobuf);
            if (err)
                goto out;
        }
                           ...
                           ext2_ino_t ino)
{
    ...
    err = copy_file_chunk(fs, fd, e2_file, 0, statbuf->st_size, buf, zerobuf);
out:
...
static int is_hardlink(struct hdlinks_s* hdlinks, dev_t dev, ino_t ino)
{
    ...
    {
        if (hdlinks->hdl[i].src_dev == dev && hdlinks->hdl[i].src_ino == ino)
            return i;
    }
    return -1;
}
                            ...
                            const char* dest, ext2_ino_t root)
{
    ...
    {
        retval = errno;
        com_err(__func__, retval, _("while opening \"%s\" to copy"), src);
        return retval;
    }
...
static errcode_t path_append(struct file_info* target, const char* file)
{
    ...
    target->path_len += sprintf(target->path + target->path_len, "/%s", file);
    return 0;
}
                               ...
                               struct fs_ops_callbacks* fs_callbacks)
{
    ...
    {
        retval = errno;
        com_err(__func__, retval, _("while opening directory \"%s\""),
                source_dir);
        return retval;
    }
    ...
    while ((dent = readdir(dh)))
    {
        ...
        {
            retval = errno;
            com_err(__func__, retval, _("while lstat \"%s\""), dent->d_name);
            goto out;
        }
        name = dent->d_name;
        ...
        if (!S_ISDIR(st.st_mode) && !S_ISLNK(st.st_mode) && st.st_nlink > 1)
        {
            ...
            if (hdlink >= 0)
            {
                retval = add_link(fs, parent_ino, hdlinks->hdl[hdlink].dst_ino,
                                  name);
                if (retval)
                {
        ...
        if (retval)
        {
            com_err(__func__, retval, "while appending %s", name);
            goto out;
        }

        if (fs_callbacks && fs_callbacks->create_new_inode)
        {
            retval = fs_callbacks->create_new_inode(
                fs, target->path, name, parent_ino, root, st.st_mode & S_IFMT);
            if (retval)
                goto out;
        }
        ...
        switch (st.st_mode & S_IFMT)
        {
...
#ifndef _WIN32
            case S_IFSOCK:
                retval = do_mknod_internal(fs, parent_ino, name, st.st_mode,
                                           st.st_rdev);
                if (retval)
                {
                    com_err(__func__, retval,
            ...
            case S_IFLNK:
                ...
                if (ln_target == NULL)
                {
                    com_err(__func__, retval, _("malloc failed"));
                    goto out;
                }
                read_cnt = readlink(name, ln_target, st.st_size + 1);
                if (read_cnt == -1)
                {
                    retval = errno;
                    com_err(__func__, retval,
                            _("while trying to read link \"%s\""), name);
                    free(ln_target);
                    goto out;
                }
                ...
                retval =
                    do_symlink_internal(fs, parent_ino, name, ln_target, root);
                free(ln_target);
                if (retval)
                {
                    com_err(__func__, retval, _("while writing symlink\"%s\""),
                            name);
                    goto out;
                }
                break;
#endif
            case S_IFREG:
                retval = do_write_internal(fs, parent_ino, name, name, root);
                if (retval)
                {
                    com_err(__func__, retval, _("while writing file \"%s\""),
                            name);
                    goto out;
                }
                break;
            case S_IFDIR:
                ...
                if (parent_ino == EXT2_ROOT_INO &&
                    ...
                    goto find_lnf;
                retval = do_mkdir_internal(fs, parent_ino, name, root);
                if (retval)
                {
            ...
            find_lnf:
                ...
                retval = __populate_fs(fs, ino, name, root, hdlinks, target,
                                       fs_callbacks);
                if (retval)
                    goto out;
                if (chdir(".."))
                {
                    retval = errno;
                    com_err(__func__, retval, _("while changing directory"));
                    goto out;
                }
                break;
            default:
                com_err(__func__, 0, _("ignoring entry \"%s\""), name);
        }
        ...
        if (retval)
        {
            com_err(__func__, retval, _("while setting inode for \"%s\""),
                    name);
            goto out;
        }
        ...
        if (retval)
        {
            com_err(__func__, retval, _("while setting xattrs for \"%s\""),
                    name);
            goto out;
        }
        ...
        if (save_inode)
        {
            ...
            {
                void* p = realloc(hdlinks->hdl, (hdlinks->size + HDLINK_CNT) *
                                                    sizeof(struct hdlink_s));
                if (p == NULL)
                {
                    retval = EXT2_ET_NO_MEMORY;
                    com_err(name, retval, _("while saving inode data"));
                    goto out;
                }
                hdlinks->hdl = p;
