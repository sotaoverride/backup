                         ...
                         json_c_visit_userfunc* userfunc, void* userarg)
{
    ...
    switch (userret)
    {
        ...
        default:
            fprintf(
                stderr,
                "ERROR: invalid return value from json_c_visit userfunc: %d\n",
                userret);
            return JSON_C_VISIT_RETURN_ERROR;
    }
    ...
    switch (json_object_get_type(jso))
    {
        ...
        case json_type_object:
        {
            ...
            {
                userret =
                    _json_c_visit(child, jso, key, NULL, userfunc, userarg);
                ...
                {
                    fprintf(stderr,
                            "INTERNAL ERROR: _json_c_visit returned %d\n",
                            userret);
                    return JSON_C_VISIT_RETURN_ERROR;
                }
            }
        ...
        case json_type_array:
        {
            ...
            for (ii = 0; ii < array_len; ii++)
            {
                ...
                userret =
                    _json_c_visit(child, jso, NULL, &ii, userfunc, userarg);
                if (userret == JSON_C_VISIT_RETURN_POP)
                    break;
                if (userret == JSON_C_VISIT_RETURN_STOP ||
                    userret == JSON_C_VISIT_RETURN_ERROR)
        ...
        default:
            fprintf(stderr,
                    "INTERNAL ERROR: _json_c_visit found object of unknown "
                    "type: %d\n",
    ...
    userret = userfunc(jso, JSON_C_VISIT_SECOND, parent_jso, jso_key, jso_index,
                       userarg);
    switch (userret)
    {
        case JSON_C_VISIT_RETURN_SKIP:
