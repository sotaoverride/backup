...
class class_property
{
  protected:
    ...
    typedef typename add_pointer<typename add_const<PropertyType>::type>::type
        address_res_t;
...
inline std::basic_ostream<CharT1, Tr>&
    operator<<(std::basic_ostream<CharT1, Tr>& os,
               class_property<PropertyType> const& p)
...
#define DEFINE_PROPERTY_FREE_BINARY_OPERATOR(op)                               \
    template <class PropertyType>                                              \
    inline bool operator op(PropertyType const& lhs,                           \
                            class_property<PropertyType> const& rhs)           \
    {                                                                          \
        return lhs op rhs.get();                                               \
    }                                                                          \
    template <class PropertyType>                                              \
    inline bool operator op(class_property<PropertyType> const& lhs,           \
...
class readonly_property : public class_property<PropertyType>
{
  ...
  public:
    ...
    {
        return boost::addressof(base_prop::value);
    }
};
...
#define BOOST_READONLY_PROPERTY(property_type, friends)                        \
    boost::unit_test::readwrite_property<property_type>
...
#define BOOST_READONLY_PROPERTY(property_type, friends)                        \
    ...
    {                                                                          \
        ...
        BOOST_PP_SEQ_FOR_EACH(BOOST_READONLY_PROPERTY_DECLARE_FRIEND, ' ',     \
                              friends)                                         \
      ...
      public:                                                                  \
        ...
        explicit BOOST_JOIN(readonly_property,                                 \
                            __LINE__)(write_param_t init_v) :                  \
            base_prop(init_v)                                                  \
        {                                                                      \
        }                                                                      \
    } /**/
...
class readwrite_property : public class_property<PropertyType>
{
  ...
  public:
    ...
    explicit readwrite_property(write_param_t init_value) :
        base_prop(init_value)
    {
    }

    // access methods
    void set(write_param_t v)
    {
        base_prop::value = v;
    }
    arrow_res_t operator->()
    {
        return boost::addressof(base_prop::value);
    }
    const_arrow_res_t operator->() const
    {
        return boost::addressof(base_prop::value);
    }
