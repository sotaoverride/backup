...
static netsnmp_transport* find_transport_from_callback_num(int num)
{
...
static void callback_debug_pdu(const char* ourstring, netsnmp_pdu* pdu)
{
    netsnmp_variable_list* vb;
    int i = 1;
    DEBUGMSGTL((ourstring, "PDU: command = %d, errstat = %ld, errindex = %ld\n",
...
void callback_push_queue(int num, netsnmp_callback_pass* item)
{
...
char* netsnmp_callback_fmtaddr(netsnmp_transport* t, const void* data, int len)
{
    char* buf;
...
int netsnmp_callback_recv(netsnmp_transport* t, void* buf, int size,
...
int netsnmp_callback_send(netsnmp_transport* t, const void* buf, int size,
                          void** opaque, int* olength)
{
    int from, rc = -1;
...
int netsnmp_callback_hook_parse(netsnmp_session* sp, netsnmp_pdu* pdu,
                                u_char* packetptr, size_t len)
{
    if (SNMP_MSG_RESPONSE == pdu->command || SNMP_MSG_REPORT == pdu->command)
...
int netsnmp_callback_hook_build(netsnmp_session* sp, netsnmp_pdu* pdu,
    ...
    switch (pdu->version)
    {
        ...
        case SNMP_VERSION_2c:
            ...
            if (pdu->community_len == 0)
            {
                ...
                memmove(pdu->community, sp->community, sp->community_len);
        ...
        case SNMP_VERSION_3:
            if (pdu->securityNameLen == 0)
            {
                ...
                memmove(pdu->securityName, sp->securityName,
                        sp->securityNameLen);
...
{
    netsnmp_pdu* pdu;
    netsnmp_callback_pass* cp = callback_pop_queue(
        ((netsnmp_callback_info*)transport->data)->callback_num);
    if (!cp)
        return NULL;
    pdu = cp->pdu;
...
netsnmp_session* netsnmp_callback_open(
    int attach_to,
    int (*return_func)(int op, netsnmp_session* session, int reqid,
    ...
    int (*fpost_parse)(netsnmp_session*, netsnmp_pdu*, int))
{
    ...
    callback_ss = snmp_add_full(
        &callback_sess, callback_tr, fpre_parse, netsnmp_callback_hook_parse,
        fpost_parse, netsnmp_callback_hook_build, NULL,
        netsnmp_callback_check_packet, netsnmp_callback_create_pdu);
    if (callback_ss)
