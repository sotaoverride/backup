 ...
 */
#elif !defined(OPENSSL_SYS_VMS) && !defined(OPENSSL_SYS_MSDOS) &&              \
...
static int read_string(UI* ui, UI_STRING* uis)
{
    ...
    switch (UI_get_string_type(uis))
    {
        case UIT_BOOLEAN:
            ...
            fflush(tty_out);
            return read_string_inner(
                ui, uis, UI_get_input_flags(uis) & UI_INPUT_FLAG_ECHO, 0);
        case UIT_PROMPT:
            fputs(UI_get0_output_string(uis), tty_out);
            fflush(tty_out);
            return read_string_inner(
                ui, uis, UI_get_input_flags(uis) & UI_INPUT_FLAG_ECHO, 1);
        case UIT_VERIFY:
            fprintf(tty_out, "Verifying - %s", UI_get0_output_string(uis));
            fflush(tty_out);
            if ((ok = read_string_inner(
                     ui, uis, UI_get_input_flags(uis) & UI_INPUT_FLAG_ECHO,
                     1)) <= 0)
                return ok;
            if (strcmp(UI_get0_result_string(uis), UI_get0_test_string(uis)) !=
                0)
            {
...
static int read_string_inner(UI* ui, UI_STRING* uis, int echo, int strip_nl)
{
    ...
    if (is_a_tty)
    {
        ...
        if (GetEnvironmentVariableW(L"OPENSSL_WIN32_UTF8", NULL, 0) != 0)
        {
            ...
            if (ReadConsoleW(GetStdHandle(STD_INPUT_HANDLE), wresult, maxsize,
                             &numread, NULL))
            {
                ...
                wresult[numread] = '\0';
                if (WideCharToMultiByte(CP_UTF8, 0, wresult, -1, result,
                                        sizeof(result), NULL, 0) > 0)
                    p = result;
...
#endif
            if (ReadConsoleA(GetStdHandle(STD_INPUT_HANDLE), result, maxsize,
                             &numread, NULL))
        {
            if (numread >= 2 && result[numread - 2] == '\r' &&
                result[numread - 1] == '\n')
            {
                result[numread - 2] = '\n';
                numread--;
...
#define DEV_TTY "/dev/tty"
    ...
    status = sys$qiow(0, channel, IO$_SENSEMODE, &iosb, 0, 0, tty_orig, 12, 0,
                      0, 0, 0);
...
static int noecho_console(UI* ui)
{
    ...
    if (is_a_tty)
    {
        ...
        status = sys$qiow(0, channel, IO$_SETMODE, &iosb, 0, 0, tty_new, 12, 0,
                          0, 0, 0);
        if ((status != SS$_NORMAL) || (iosb.iosb$w_value != SS$_NORMAL))
        {
            char tmp_num[2][12];

            BIO_snprintf(tmp_num[0], sizeof(tmp_num[0]) - 1, "%%X%08X", status);
