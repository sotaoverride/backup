...
void init_mteScalars(void)
{
    ...
    netsnmp_register_scalar_group(
        netsnmp_create_handler_registration(
            "mteResource", handle_mteResourceGroup, mteResource_oid,
            OID_LENGTH(mteResource_oid), HANDLER_CAN_RONLY),
        MTE_RESOURCE_SAMPLE_MINFREQ, MTE_RESOURCE_SAMPLE_LACKS);

    netsnmp_register_scalar(netsnmp_create_handler_registration(
        "mteTriggerFailures", handle_mteTriggerFailures, mteTriggerFail_oid,
        OID_LENGTH(mteTriggerFail_oid), HANDLER_CAN_RONLY));
}

int handle_mteResourceGroup(netsnmp_mib_handler* handler,
                            ...
                            netsnmp_request_info* requests)
{
    ...
    switch (reqinfo->mode)
    {
        ...
        default:
            /*
             * Although mteResourceSampleMinimum and
             * mteResourceSampleInstanceMaximum are defined with MAX-ACCESS
             * read-write, this version hardcodes these values, so doesn't need
             * to implement write access.
             */
            snmp_log(LOG_ERR, "unknown mode (%d) in handle_mteResourceGroup\n",
                     reqinfo->mode);
            return SNMP_ERR_GENERR;
    }
...
int handle_mteTriggerFailures(netsnmp_mib_handler* handler,
                              netsnmp_handler_registration* reginfo,
                              netsnmp_agent_request_info* reqinfo,
                              netsnmp_request_info* requests)
{
