...
static void idxtostr(struct chmem_desc* desc, uint64_t idx, char* buf,
                     size_t bufsz)
{
    uint64_t start, end;
...
static int chmem_size(struct chmem_desc* desc, int enable, int zone_id)
{
    ...
    for (; i >= 0 && i < desc->ndirs && size; i += enable ? 1 : -1)
    {
        ...
        if (ul_path_readf_buffer(desc->sysmem, line, sizeof(line), "%s/state",
                                 name) > 0 &&
        ...
        if (desc->have_zones)
        {
            ul_path_readf_buffer(desc->sysmem, line, sizeof(line),
                                 "%s/valid_zones", name);
            if (zone_id >= 0)
            {
...
int main(int argc, char** argv)
{
    ...
    return rc == 0 ? EXIT_SUCCESS : rc < 0 ? EXIT_FAILURE : CHMEM_EXIT_SOMEOK;
}
