...
static int on_connected(sd_bus_message* message, void* userdata,
                        sd_bus_error* ret_error)
{
    Manager* m = userdata;
    ...
    /* Did we get a timezone or transient hostname from DHCP while D-Bus wasn't
     * up yet? */
...
int manager_connect_bus(Manager* m)
{
    ...
    r = sd_bus_add_object_vtable(m->bus, NULL, "/org/freedesktop/network1",
    ...
    r = sd_bus_add_fallback_vtable(
        m->bus, NULL, "/org/freedesktop/network1/link",
        "org.freedesktop.network1.Link", link_vtable, link_object_find, m);
    if (r < 0)
    ...
    r = sd_bus_add_fallback_vtable(m->bus, NULL,
                                   ...
                                   "org.freedesktop.network1.Network",
                                   network_vtable, network_object_find, m);
    if (r < 0)
        return log_error_errno(r, "Failed to add network object vtable: %m");

    r = sd_bus_add_node_enumerator(m->bus, NULL,
                                   "/org/freedesktop/network1/network",
                                   network_node_enumerator, m);
    if (r < 0)
        return log_error_errno(r, "Failed to add network enumerator: %m");

    r = sd_bus_request_name_async(m->bus, NULL, "org.freedesktop.network1", 0,
                                  NULL, NULL);
    if (r < 0)
    ...
    r = sd_bus_match_signal_async(m->bus, NULL, "org.freedesktop.DBus.Local",
                                  NULL, "org.freedesktop.DBus.Local",
                                  "Connected", on_connected, NULL, m);
    if (r < 0)
        return log_error_errno(
            r, "Failed to request match on Connected signal: %m");
...
static int manager_udev_process_link(sd_device_monitor* monitor,
                                     sd_device* device, void* userdata)
{
    ...
    if (r < 0)
    {
        log_device_debug_errno(
            device, r, "Failed to get udev action, ignoring device: %m");
        return 0;
    }

    if (!IN_SET(action, DEVICE_ACTION_ADD, DEVICE_ACTION_CHANGE,
                DEVICE_ACTION_MOVE))
    {
        log_device_debug(device, "Ignoring udev %s event for device.",
    ...
    if (r < 0)
    {
        log_device_debug_errno(device, r,
                               "Ignoring udev ADD event for device without "
                               "ifindex or with invalid ifindex: %m");
        return 0;
    }
    ...
    if (r < 0)
    {
        log_device_error_errno(device, r,
                               "Failed to determine the device is renamed or "
                               "not, ignoring '%s' uevent: %m",
                               device_action_to_string(action));
        return 0;
    }
    if (r > 0)
    {
        log_device_debug(device, "Interface is under renaming, wait for the "
                                 "interface to be renamed: %m");
        return 0;
    }
    ...
    if (r < 0)
    {
        if (r != -ENODEV)
            log_debug_errno(
                r, "Failed to get link from ifindex %i, ignoring: %m", ifindex);
        return 0;
    }
...
static int manager_connect_udev(Manager* m)
{
    ...
    r = sd_device_monitor_filter_add_match_subsystem_devtype(m->device_monitor,
                                                             "net", NULL);
    if (r < 0)
    ...
    r = sd_device_monitor_start(m->device_monitor, manager_udev_process_link,
                                m);
    if (r < 0)
...
int manager_rtnl_process_route(sd_netlink* rtnl, sd_netlink_message* message,
                               void* userdata)
{
    Manager* m = userdata;
    Link* link = NULL;
    ...
    {
        log_warning("rtnl: received unexpected message type when processing "
                    "route, ignoring");
        return 0;
    }
    ...
    {
        log_warning_errno(
            r, "rtnl: could not get ifindex from route, ignoring: %m");
        return 0;
    }
    else if (ifindex <= 0)
    {
        log_warning(
            "rtnl: received route message with invalid ifindex, ignoring: %d",
            ifindex);
        return 0;
    }
    else
    {
        ...
        if (r < 0 || !link)
        {
            ...
            if (!m->enumerating)
                log_warning(
                    "rtnl: received route for nonexistent link (%d), ignoring",
                    ifindex);
            return 0;
        }
    }
    ...
    {
        log_link_warning(
            link, "rtnl: received address with invalid family, ignoring");
        return 0;
    }
    ...
    switch (family)
    {
        case AF_INET:
            ...
            if (r < 0 && r != -ENODATA)
            {
                log_link_warning_errno(link, r,
                                       "rtnl: received route without valid "
                                       "destination, ignoring: %m");
                return 0;
            }
            ...
            r = sd_netlink_message_read_in_addr(message, RTA_PREFSRC,
                                                &prefsrc.in);
            if (r < 0 && r != -ENODATA)
            {
                log_link_warning_errno(link, r,
                                       "rtnl: received route with invalid "
                                       "preferred source, ignoring: %m");
                return 0;
            }
        ...
        case AF_INET6:
            ...
            r = sd_netlink_message_read_in6_addr(message, RTA_PREFSRC,
                                                 &prefsrc.in6);
            if (r < 0 && r != -ENODATA)
            {
                log_link_warning_errno(link, r,
    ...
    {
        log_link_warning_errno(link, r,
                               "rtnl: received route with invalid destination "
                               "prefixlen, ignoring: %m");
        return 0;
    }
    ...
    {
        log_link_warning_errno(
            link, r, "rtnl: received route with invalid scope, ignoring: %m");
        return 0;
    }
    ...
    {
        log_link_warning_errno(
            link, r, "rtnl: received route with invalid tos, ignoring: %m");
        return 0;
    }
    ...
    {
        log_link_warning_errno(
            link, r, "rtnl: received route with invalid type, ignoring: %m");
        return 0;
    }
    ...
    {
        log_link_warning_errno(
            link, r, "rtnl: received route with invalid table, ignoring: %m");
        return 0;
    }
    ...
    (void)route_get(link, family, &dst, dst_prefixlen, tos, priority, table,
                    &route);

    if (DEBUG_LOGGING)
    {
        _cleanup_free_ char *buf_dst = NULL, *buf_dst_prefixlen = NULL,
                            *buf_src = NULL, *buf_gw = NULL,
                            *buf_prefsrc = NULL;
        ...
        log_link_debug(
            link, "%s route: dst: %s%s, src: %s, gw: %s, prefsrc: %s",
            type == RTM_DELROUTE ? "Removing" : route ? "Updating" : "Adding",
            strna(buf_dst), strempty(buf_dst_prefixlen), strna(buf_src),
            strna(buf_gw), strna(buf_prefsrc));
    }
    ...
    switch (type)
    {
        case RTM_NEWROUTE:
            if (!route)
            {
                ...
                r = route_add_foreign(link, family, &dst, dst_prefixlen, tos,
            ...
            route_update(route, &src, src_prefixlen, &gw, &prefsrc, scope,
                         protocol, rt_type);
                                 ...
                                 void* userdata)
{
    ...
    {
        log_warning_errno(
            r, "rtnl: could not get ifindex from address, ignoring: %m");
        return 0;
    }
    else if (ifindex <= 0)
    {
        log_warning(
            "rtnl: received address message with invalid ifindex, ignoring: %d",
            ifindex);
        return 0;
    }
    else
    {
        ...
        if (r < 0 || !link)
        {
            ...
            if (!m->enumerating)
                log_warning("rtnl: received address for nonexistent link (%d), "
                            "ignoring",
                            ifindex);
            return 0;
        }
    ...
    {
        log_link_warning_errno(
            link, r, "rtnl: received address with invalid scope, ignoring: %m");
        return 0;
    }
    ...
    {
        log_link_warning_errno(
            link, r, "rtnl: received address with invalid flags, ignoring: %m");
        return 0;
    }
    ...
    switch (family)
    {
        case AF_INET:
            ...
            {
                log_link_warning_errno(link, r,
                                       "rtnl: received address without valid "
                                       "address, ignoring: %m");
                return 0;
            }
        ...
        case AF_INET6:
            r = sd_netlink_message_read_in6_addr(message, IFA_ADDRESS,
                                                 &in_addr.in6);
            if (r < 0)
            {
                log_link_warning_errno(link, r,
    ...
    {
        log_link_warning_errno(
            link, r, "rtnl: cannot get IFA_CACHEINFO attribute, ignoring: %m");
        return 0;
    }
    else if (r >= 0)
    {
        if (cinfo.ifa_valid != CACHE_INFO_INFINITY_LIFE_TIME)
            valid_str =
                format_timespan(valid_buf, FORMAT_TIMESPAN_MAX,
    ...
    switch (type)
    {
        case RTM_NEWADDR:
            if (address)
                log_link_debug(link, "Updating address: %s/%u (valid %s%s)",
                               buf, prefixlen, valid_str ? "for " : "forever",
                               strempty(valid_str));
            else
            {
                /* An address appeared that we did not request */
                r = address_add_foreign(link, family, &in_addr, prefixlen,
                                        &address);
                if (r < 0)
                {
                    log_link_warning_errno(
                        link, r, "Failed to add address %s/%u, ignoring: %m",
                        buf, prefixlen);
                    return 0;
                }
                else
                    log_link_debug(link, "Adding address: %s/%u (valid %s%s)",
                                   buf, prefixlen,
            ...
            {
                log_link_warning_errno(
                    link, r, "Failed to update address %s/%u, ignoring: %m",
                    buf, prefixlen);
                return 0;
            }
        ...
        case RTM_DELADDR:
            ...
            if (address)
            {
                log_link_debug(link, "Removing address: %s/%u (valid %s%s)",
                               buf, prefixlen, valid_str ? "for " : "forever",
                               strempty(valid_str));
                ...
                log_link_warning(link,
                                 "Removing non-existent address: %s/%u (valid "
                                 "%s%s), ignoring",
                                 buf, prefixlen, valid_str ? "for " : "forever",
                                 strempty(valid_str));
...
static int manager_rtnl_process_link(sd_netlink* rtnl,
                                     ...
                                     void* userdata)
{
    ...
    {
        log_warning_errno(
            r, "rtnl: Could not get ifindex from link, ignoring: %m");
        return 0;
    }
    else if (ifindex <= 0)
    {
        log_warning(
            "rtnl: received link message with invalid ifindex %d, ignoring",
            ifindex);
        return 0;
    }
    ...
    {
        log_warning_errno(
            r, "rtnl: Received link message without ifname, ignoring: %m");
        return 0;
    }
    ...
    switch (type)
    {
        case RTM_NEWLINK:
            ...
            if (netdev)
            {
                ...
                {
                    log_warning_errno(
                        r, "Could not set ifindex on netdev, ignoring: %m");
                    return 0;
                }
            }
...
int manager_rtnl_process_rule(sd_netlink* rtnl, sd_netlink_message* message,
                              void* userdata)
{
    ...
    {
        log_warning("rtnl: received unexpected message type '%u' when "
                    "processing rule, ignoring",
                    type);
        return 0;
    }
    ...
    {
        log_debug("rtnl: received address with invalid family %u, ignoring",
                  family);
        return 0;
    }
    ...
    switch (family)
    {
        case AF_INET:
            ...
            {
                log_warning_errno(
                    r, "rtnl: could not get FRA_SRC attribute, ignoring: %m");
                return 0;
            }
            else if (r >= 0)
            {
                r = sd_rtnl_message_routing_policy_rule_get_rtm_src_prefixlen(
                    message, &from_prefixlen);
                if (r < 0)
                {
                    log_warning_errno(r, "rtnl: failed to retrieve rule from "
                                         "prefix length, ignoring: %m");
                    return 0;
                }
            }
            ...
            {
                log_warning_errno(
                    r, "rtnl: could not get FRA_DST attribute, ignoring: %m");
                return 0;
            }
            else if (r >= 0)
            {
                r = sd_rtnl_message_routing_policy_rule_get_rtm_dst_prefixlen(
                    message, &to_prefixlen);
                if (r < 0)
                {
                    log_warning_errno(r, "rtnl: failed to retrieve rule to "
                                         "prefix length, ignoring: %m");
                    return 0;
                }
            }
    ...
    {
        log_warning_errno(
            r, "rtnl: could not get FRA_FWMARK attribute, ignoring: %m");
        return 0;
    }
    ...
    {
        log_warning_errno(
            r, "rtnl: could not get FRA_TABLE attribute, ignoring: %m");
        return 0;
    }
    ...
    {
        log_warning_errno(
            r, "rtnl: could not get FRA_IIFNAME attribute, ignoring: %m");
        return 0;
    }
    ...
    {
        log_warning_errno(
            r, "rtnl: could not get FRA_OIFNAME attribute, ignoring: %m");
        return 0;
    }
    ...
    {
        log_warning_errno(
            r, "rtnl: could not get FRA_IP_PROTO attribute, ignoring: %m");
        return 0;
    }

    r = sd_netlink_message_read(message, FRA_SPORT_RANGE, sizeof(sport),
                                (void*)&sport);
    if (r < 0 && r != -ENODATA)
    {
        log_warning_errno(
            r, "rtnl: could not get FRA_SPORT_RANGE attribute, ignoring: %m");
        return 0;
    }

    r = sd_netlink_message_read(message, FRA_DPORT_RANGE, sizeof(dport),
                                (void*)&dport);
    if (r < 0 && r != -ENODATA)
    {
        log_warning_errno(
            r, "rtnl: could not get FRA_DPORT_RANGE attribute, ignoring: %m");
        return 0;
    }

    (void)routing_policy_rule_get(m, family, &from, from_prefixlen, &to,
                                  to_prefixlen, tos, fwmark, table, iif, oif,
                                  protocol, &sport, &dport, &rule);
        ...
        case RTM_NEWRULE:
            if (!rule)
            {
                r = routing_policy_rule_add_foreign(
                    m, family, &from, from_prefixlen, &to, to_prefixlen, tos,
                    fwmark, table, iif, oif, protocol, &sport, &dport, &rule);
                if (r < 0)
                {
...
static int manager_connect_rtnl(Manager* m)
{
    ...
    r = sd_netlink_add_match(m->rtnl, NULL, RTM_NEWLINK,
                             &manager_rtnl_process_link, NULL, m,
                             "network-rtnl_process_link");
    if (r < 0)
        return r;

    r = sd_netlink_add_match(m->rtnl, NULL, RTM_DELLINK,
                             &manager_rtnl_process_link, NULL, m,
                             "network-rtnl_process_link");
    if (r < 0)
        return r;
    ...
    r = sd_netlink_add_match(m->rtnl, NULL, RTM_NEWADDR,
                             &manager_rtnl_process_address, NULL, m,
                             "network-rtnl_process_address");
    if (r < 0)
        return r;

    r = sd_netlink_add_match(m->rtnl, NULL, RTM_DELADDR,
                             &manager_rtnl_process_address, NULL, m,
                             "network-rtnl_process_address");
    if (r < 0)
        return r;

    r = sd_netlink_add_match(m->rtnl, NULL, RTM_NEWROUTE,
                             &manager_rtnl_process_route, NULL, m,
                             "network-rtnl_process_route");
    if (r < 0)
        return r;

    r = sd_netlink_add_match(m->rtnl, NULL, RTM_DELROUTE,
                             &manager_rtnl_process_route, NULL, m,
                             "network-rtnl_process_route");
    if (r < 0)
        return r;

    r = sd_netlink_add_match(m->rtnl, NULL, RTM_NEWRULE,
                             &manager_rtnl_process_rule, NULL, m,
                             "network-rtnl_process_rule");
    if (r < 0)
        return r;

    r = sd_netlink_add_match(m->rtnl, NULL, RTM_DELRULE,
                             &manager_rtnl_process_rule, NULL, m,
                             "network-rtnl_process_rule");
    if (r < 0)
        return r;
...
static int ordered_set_put_in_addr_data(OrderedSet* s,
...
static int ordered_set_put_in_addr_datav(OrderedSet* s,
                                         const struct in_addr_data* addresses,
                                         unsigned n)
{
    int r, c = 0;
    unsigned i;
...
static int ordered_set_put_in4_addr(OrderedSet* s,
...
static int
    ordered_set_put_in4_addrv(OrderedSet* s, const struct in_addr* addresses,
                              size_t n,
...
{
    _cleanup_ordered_set_free_free_ OrderedSet *dns = NULL, *ntp = NULL,
                                               *search_domains = NULL,
                                               *route_domains = NULL;
    Link* link;
    Iterator i;
    ...
    HASHMAP_FOREACH(link, m->links, i)
    {
        ...
        r = ordered_set_put_in_addr_datav(dns, link->network->dns,
                                          link->network->n_dns);
        if (r < 0)
            return r;
        ...
        r = ordered_set_put_string_set(search_domains,
                                       link->network->search_domains);
        if (r < 0)
            return r;

        r = ordered_set_put_string_set(route_domains,
        ...
        if (link->network->dhcp_use_domains != DHCP_USE_DOMAINS_NO)
        {
            ...
            OrderedSet* target_domains =
                (link->network->dhcp_use_domains == DHCP_USE_DOMAINS_YES)
                    ? search_domains
                    : route_domains;
...
fail:
    ...
    return log_error_errno(r, "Failed to save network state to %s: %m",
                           m->state_file);
}
...
static int dhcp6_route_add_handler(sd_netlink* nl, sd_netlink_message* m,
                                   Link* link)
{
    int r;
...
static void dhcp6_prefixes_hash_func(const struct in6_addr* addr,
...
static int dhcp6_prefixes_compare_func(const struct in6_addr* a,
                                       const struct in6_addr* b)
{
    return memcmp(a, b, sizeof(*a));
}

DEFINE_PRIVATE_HASH_OPS(dhcp6_prefixes_hash_ops, struct in6_addr,
                        dhcp6_prefixes_hash_func, dhcp6_prefixes_compare_func);
...
int manager_dhcp6_prefix_add(Manager* m, struct in6_addr* addr, Link* link)
{
    ...
    r = route_add(link, AF_INET6, (union in_addr_union*)addr, 64, 0, 0, 0,
                  &route);
    if (r < 0)
        return r;
...
static int dhcp6_route_remove_handler(sd_netlink* nl, sd_netlink_message* m,
                                      Link* link)
{
    int r;
...
void manager_free(Manager* m)
{
    ...
    m->links_requesting_uuid =
        set_free_with_destructor(m->links_requesting_uuid, link_unref);
...
int manager_address_pool_acquire(Manager* m, int family, unsigned prefixlen,
...
Link* manager_find_uplink(Manager* m, Link* exclude)
{
    ...
    for (i = 0; i < n; i++)
    {
        ...
        {
            log_debug(
                "Weird, found a gateway for a link we don't know. Ignoring.");
            continue;
        }
...
static int set_hostname_handler(sd_bus_message* m, void* userdata,
                                sd_bus_error* ret_error)
{
    ...
    if (e)
        log_warning_errno(sd_bus_error_get_errno(e),
                          "Could not set hostname: %s", e->message);
...
int manager_set_hostname(Manager* m, const char* hostname)
{
    ...
    r = sd_bus_call_method_async(
        m->bus, NULL, "org.freedesktop.hostname1", "/org/freedesktop/hostname1",
        "org.freedesktop.hostname1", "SetHostname", set_hostname_handler, m,
        "sb", hostname, false);
...
static int set_timezone_handler(sd_bus_message* m, void* userdata,
                                sd_bus_error* ret_error)
{
    Manager* manager = userdata;
    const sd_bus_error* e;
    ...
    if (e)
        log_warning_errno(sd_bus_error_get_errno(e),
                          "Could not set timezone: %s", e->message);
...
int manager_set_timezone(Manager* m, const char* tz)
{
    ...
    r = sd_bus_call_method_async(m->bus, NULL, "org.freedesktop.timedate1",
                                 "/org/freedesktop/timedate1",
                                 "org.freedesktop.timedate1", "SetTimezone",
                                 set_timezone_handler, m, "sb", tz, false);
    if (r < 0)
...
int manager_request_product_uuid(Manager* m, Link* link)
{
    ...
    {
        log_debug(
            "Not connected to system bus, requesting product UUID later.");
        return 0;
    }

    r = sd_bus_call_method_async(m->bus, NULL, "org.freedesktop.hostname1",
                                 "/org/freedesktop/hostname1",
                                 "org.freedesktop.hostname1", "GetProductUUID",
                                 get_product_uuid_handler, m, "b", false);
    if (r < 0)
...
int manager_sysctl_ipv6_enabled(Manager* manager)
{
    ...
    if (r < 0)
        return log_warning_errno(
            r, "Failed to read net.ipv6.conf.all.disable_ipv6 sysctl property: "
               "%m");
