...
static int v2_scan_dquots(struct quota_handle* h,
                          int (*process_dquot)(struct dquot* dquot, void* data),
                          void* data);
                                     ...
                                     struct util_dqinfo* m)
{
    ...
    d->dqi_free_blk = ext2fs_cpu_to_le32(m->u.v2_mdqi.dqi_qtree.dqi_free_blk);
    d->dqi_free_entry =
...
static int v2_init_io(struct quota_handle* h)
{
    ...
    if (h->e2fs_read(&h->qh_qf, V2_DQINFOOFF, &ddqinfo, sizeof(ddqinfo)) !=
        sizeof(ddqinfo))
        return -1;
