...
static gint g_tree_node_pre_order(GTreeNode* node, GTraverseFunc traverse_func,
                                  gpointer data);
static gint g_tree_node_in_order(GTreeNode* node, GTraverseFunc traverse_func,
                                 gpointer data);
static gint g_tree_node_post_order(GTreeNode* node, GTraverseFunc traverse_func,
                                   gpointer data);
static gpointer g_tree_node_search(GTreeNode* node, GCompareFunc search_func,
                                   gconstpointer data);
...
GTree* g_tree_new(GCompareFunc key_compare_func)
{
    ...
    return g_tree_new_full((GCompareDataFunc)key_compare_func, NULL, NULL,
                           NULL);
}
                     ...
                     GTraverseType traverse_type, gpointer user_data)
{
    ...
    switch (traverse_type)
    {
        ...
        case G_LEVEL_ORDER:
            g_warning("g_tree_traverse(): traverse type G_LEVEL_ORDER isn't "
                      "implemented.");
            break;
    }
}
