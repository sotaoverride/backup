...
static char* shift_buf(char* mem, int from)
{
    char* start = mem;
...
static void trim_spaces(char* buf, char* from)
{
    while (from > buf)
    {
        --from;
...
static int read_field(const pam_handle_t* pamh, int fd, char** buf, int* from,
                      int* state)
{
    char* to;
    char* src;
...
static int logic_field(const pam_handle_t* pamh, const void* me, const char* x,
                       int rule,
...
static int is_same(const pam_handle_t* pamh UNUSED, const void* A,
                   const char* b, int len, int rule UNUSED)
{
    int i;
    const char* a;
...
static struct day
{
    const char* d;
    int bit;
} const days[11] = {{"su", 01},   {"mo", 02},   {"tu", 04},   {"we", 010},
                    {"th", 020},  {"fr", 040},  {"sa", 0100}, {"wk", 076},
                    {"wd", 0101}, {"al", 0177}, {NULL, 0}};
...
static int check_time(const pam_handle_t* pamh, const void* AT,
...
static int mkgrplist(pam_handle_t* pamh, char* buf, gid_t** list, int len)
{
    ...
    while ((l = find_member(buf, &at)))
    {
        ...
        if (len >= blks)
        {
            ...
            tmp = (gid_t*)realloc((*list), sizeof(gid_t) * (blks += GROUP_BLK));
            if (tmp != NULL)
            {
                (*list) = tmp;
            }
                         ...
                         const char* tty, const char* user)
{
    ...
    do
    {
        ...
        {
            pam_syslog(pamh, LOG_ERR, "%s: malformed rule #%d", PAM_GROUP_CONF,
                       count);
            continue;
        }
...
#else
            pam_syslog(pamh, LOG_ERR,
                       "pam_group does not have netgroup support");
#endif
        ...
        if (state == STATE_FIELD)
        {
            pam_syslog(pamh, LOG_ERR, "%s: poorly terminated rule #%d",
                       PAM_GROUP_CONF, count);
            continue;
        }
...
int pam_sm_setcred(pam_handle_t* pamh, int flags, int argc UNUSED,
