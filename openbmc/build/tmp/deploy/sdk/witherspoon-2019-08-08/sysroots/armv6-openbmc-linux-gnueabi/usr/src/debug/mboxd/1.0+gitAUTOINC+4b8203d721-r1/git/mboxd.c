...
static int poll_loop(struct mbox_context* context)
{
    ...
    while (1)
    {
        ...
        if (context->fds[SIG_FD].revents & POLLIN)
        { /* Signal */
            struct signalfd_siginfo info = {0};

            rc = read(context->fds[SIG_FD].fd, (void*)&info, sizeof(info));
            if (rc != sizeof(info))
            {
                              ...
                              NULL)) != -1)
    {
        switch (opt)
        {
            ...
            case 'b':
                ...
                break;
            case 'n':
                context->windows.num = strtol(argv[optind], &endptr, 10);
            ...
            case 'w':
                ...
                if (optarg == endptr || (*endptr != '\0' && *endptr != 'M'))
                {
            ...
            case 't':
                context->blktracefd =
                    open(argv[optind], O_CREAT | O_TRUNC | O_WRONLY, 0666);
