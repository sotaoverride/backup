...
class pool_data_t : public block_slist_base<>
{
  ...
  public:
    ...
    void replenish(memory_resource& mr, std::size_t pool_block,
                   std::size_t max_blocks_per_chunk)
    {
        // Limit max value
        std::size_t blocks_per_chunk = boost::container::dtl::min_value(
            max_blocks_per_chunk, next_blocks_per_chunk);
        // Avoid overflow
        blocks_per_chunk = boost::container::dtl::min_value(
            blocks_per_chunk, std::size_t(-1) / pool_block);

        // Minimum block size is at least max_align, so all pools allocate sizes
        // that are multiple of max_align, meaning that all blocks are
        // max_align-aligned.
        char* p = static_cast<char*>(
            block_slist_base_t::allocate(blocks_per_chunk * pool_block, mr));

        // Create header types. This is no-throw
        for (std::size_t i = 0, max = blocks_per_chunk; i != max; ++i)
        {
            slist_node* const pv =
                ::new (p, boost_container_new_t()) slist_node();
            slist_algo::link_after(&free_slist, pv);
            p += pool_block;
        }

        // Update next block per chunk
        next_blocks_per_chunk = max_blocks_per_chunk / 2u < blocks_per_chunk
...
BOOST_STATIC_ASSERT(pool_options_default_max_blocks_per_chunk <=
                    (std::size_t(-1) / 2u + 1u));
// Sanity checks
BOOST_STATIC_ASSERT(bi::detail::static_is_pow2<
                    pool_options_default_max_blocks_per_chunk>::value);
BOOST_STATIC_ASSERT(bi::detail::static_is_pow2<
                    pool_options_minimum_largest_required_pool_block>::value);

// unsynchronized_pool_resource

void pool_resource::priv_limit_option(std::size_t& val, std::size_t min,
                                      std::size_t max) // static
{
    if (!val)
    {
        val = max;
...
{
    // For allocations equal or less than
    // pool_options_minimum_largest_required_pool_block the smallest pool is used
    block_size = boost::container::dtl::max_value(
        block_size, pool_options_minimum_largest_required_pool_block);
...
void pool_resource::priv_fix_options()
{
    ...
    priv_limit_option(m_options.largest_required_pool_block,
                      pool_options_minimum_largest_required_pool_block,
                      pool_options_default_largest_required_pool_block);
    m_options.largest_required_pool_block =
        bi::detail::ceil_pow2(m_options.largest_required_pool_block);
}

void pool_resource::priv_init_pools()
{
    const std::size_t num_pools =
        priv_pool_index(m_options.largest_required_pool_block) + 1u;
    ...
    {
        ::new (&m_pool_data[i], boost_container_new_t())
            pool_data_t(pool_options_minimum_max_blocks_per_chunk);
    }
...
pool_resource::pool_resource(const pool_options& opts,
...
pool_resource::pool_resource() BOOST_NOEXCEPT
    : m_options(),
      m_upstream(*get_default_resource()),
      m_oversized_list(),
      m_pool_data(),
      m_pool_count()
{
...
pool_resource::~pool_resource() // virtual
{
    ...
    if (m_pool_data)
    {
        m_upstream.deallocate((void*)m_pool_data,
                              sizeof(pool_data_t) * m_pool_count);
    }
}
...
void* pool_resource::do_allocate(std::size_t bytes,
                                 std::size_t alignment) // virtual
{
    if (!m_pool_data)
    {
    ...
    else
    {
        ...
        if (!p)
        {
            pool.replenish(m_upstream, priv_pool_block(pool_idx),
...
void pool_resource::do_deallocate(void* p, std::size_t bytes,
                                  std::size_t alignment) // virtual
{
    (void)alignment; // alignment ignored here, max_align is used by pools
    if (bytes > m_options.largest_required_pool_block)
...
bool pool_resource::do_is_equal(const memory_resource& other) const
    BOOST_NOEXCEPT // virtual
{
...
std::size_t
    pool_resource::pool_next_blocks_per_chunk(std::size_t pool_idx) const
{
