...
G_DEFINE_TYPE_WITH_CODE(
    GCharsetConverter, g_charset_converter, G_TYPE_OBJECT,
    G_IMPLEMENT_INTERFACE(G_TYPE_CONVERTER, g_charset_converter_iface_init);
...
static void g_charset_converter_set_property(GObject* object, guint prop_id,
                                             const GValue* value,
                                             GParamSpec* pspec)
{
    GCharsetConverter* conv;
...
static void g_charset_converter_get_property(GObject* object, guint prop_id,
...
static void g_charset_converter_class_init(GCharsetConverterClass* klass)
{
    ...
    g_object_class_install_property(
        gobject_class, PROP_TO_CHARSET,
        g_param_spec_string("to-charset", P_("To Charset"),
                            P_("The character encoding to convert to"), NULL,
                            G_PARAM_READWRITE | G_PARAM_CONSTRUCT_ONLY |
                                G_PARAM_STATIC_STRINGS));
    g_object_class_install_property(
        gobject_class, PROP_FROM_CHARSET,
        g_param_spec_string("from-charset", P_("From Charset"),
                            P_("The character encoding to convert from"), NULL,
                            G_PARAM_READWRITE | G_PARAM_CONSTRUCT_ONLY |
                                G_PARAM_STATIC_STRINGS));
    g_object_class_install_property(
        gobject_class, PROP_USE_FALLBACK,
        g_param_spec_boolean(
            "use-fallback", P_("Fallback enabled"),
            P_("Use fallback (of form \\<hexval>) for invalid bytes"), FALSE,
                                           ...
                                           GError** error)
{
    ...
    conv = g_initable_new(G_TYPE_CHARSET_CONVERTER, NULL, error, "to-charset",
                          to_charset, "from-charset", from_charset, NULL);
...
static GConverterResult g_charset_converter_convert(
    GConverter* converter, const void* inbuf, gsize inbuf_size, void* outbuf,
    gsize outbuf_size, GConverterFlags flags, gsize* bytes_read,
    gsize* bytes_written, GError** error)
{
    ...
    if (inbuf_size == 0)
    {
        if (flags & G_CONVERTER_INPUT_AT_END || flags & G_CONVERTER_FLUSH)
        {
            reset = TRUE;
        }
        else
    ...
    if (reset)
        ...
        res = g_iconv(conv->iconv, NULL, &in_left, &outbufp, &out_left);
    else
        res = g_iconv(conv->iconv, &inbufp, &in_left, &outbufp, &out_left);
    ...
    *bytes_written = outbufp - (char*)outbuf;
    ...
    if (res == (gsize)-1 && *bytes_read == 0)
    {
        ...
        switch (errsv)
        {
            case EINVAL:
                ...
                g_set_error_literal(
                    error, G_IO_ERROR, G_IO_ERROR_PARTIAL_INPUT,
                    _("Incomplete multibyte sequence in input"));
                break;
    ...
    ok:
        ...
        if (reset && (flags & G_CONVERTER_INPUT_AT_END))
            ret = G_CONVERTER_FINISHED;
        else if (reset && (flags & G_CONVERTER_FLUSH))
...
static gboolean g_charset_converter_initable_init(GInitable* initable,
    ...
    if (conv->iconv == (GIConv)-1)
    {
        if (errsv == EINVAL)
            g_set_error(error, G_IO_ERROR, G_IO_ERROR_NOT_SUPPORTED,
                        _("Conversion from character set “%s” to “%s” is not "
                          "supported"),
                        conv->from, conv->to);
        else
            g_set_error(error, G_IO_ERROR, G_IO_ERROR_FAILED,
