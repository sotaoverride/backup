...
FT_Matrix_Multiply(const FT_Matrix* a, FT_Matrix* b)
{
    ...
    xx = ADD_LONG(FT_MulFix(a->xx, b->xx), FT_MulFix(a->xy, b->yx));
    xy = ADD_LONG(FT_MulFix(a->xx, b->xy), FT_MulFix(a->xy, b->yy));
    yx = ADD_LONG(FT_MulFix(a->yx, b->xx), FT_MulFix(a->yy, b->yx));
    yy = ADD_LONG(FT_MulFix(a->yx, b->xy), FT_MulFix(a->yy, b->yy));
...
FT_Matrix_Invert(FT_Matrix* matrix)
{
    ...
    delta =
        FT_MulFix(matrix->xx, matrix->yy) - FT_MulFix(matrix->xy, matrix->yx);
...
FT_Matrix_Multiply_Scaled(const FT_Matrix* a, FT_Matrix* b, FT_Long scaling)
{
    ...
    xx = ADD_LONG(FT_MulDiv(a->xx, b->xx, val), FT_MulDiv(a->xy, b->yx, val));
    xy = ADD_LONG(FT_MulDiv(a->xx, b->xy, val), FT_MulDiv(a->xy, b->yy, val));
    yx = ADD_LONG(FT_MulDiv(a->yx, b->xx, val), FT_MulDiv(a->yy, b->yx, val));
    yy = ADD_LONG(FT_MulDiv(a->yx, b->xy, val), FT_MulDiv(a->yy, b->yy, val));
...
ft_corner_orientation(FT_Pos in_x, FT_Pos in_y, FT_Pos out_x, FT_Pos out_y)
{
    ...
    FT_Int64 delta = SUB_INT64(MUL_INT64(in_x, out_y), MUL_INT64(in_y, out_x));
