...
struct threadsafe_queue_impl
{
    ...
    virtual bool try_pop(node_base*& node_to_free,
                         node_base*& node_with_value) = 0;
};

//! Thread-safe queue node type
template <typename T>
struct threadsafe_queue_node : public threadsafe_queue_impl::node_base
{
    ...
    explicit threadsafe_queue_node(T const& val)
    {
        new (storage.address()) T(val);
    }
    T& value() BOOST_NOEXCEPT
    {
        return *static_cast<T*>(storage.address());
    }
    void destroy() BOOST_NOEXCEPT
    {
        static_cast<T*>(storage.address())->~T();
    }
    ...
    BOOST_DELETED_FUNCTION(
        threadsafe_queue_node& operator=(threadsafe_queue_node const&))
};
 ...
 * in PODC96 by Maged M. Michael and Michael L. Scott. Pseudocode is available
 * here:
 * http://www.cs.rochester.edu/research/synchronization/pseudocode/queues.html
 *
 * The implementation provides thread-safe \c push and \c try_pop operations, as
 * well as a thread-unsafe \c empty operation. The queue imposes the following
 * requirements on the element type:
 *
 * \li Default constructible, the default constructor must not throw.
 * \li Copy constructible.
 * \li Movable (i.e. there should be an efficient move assignment for this
 * type).
 *
...
class threadsafe_queue
    : private boost::log::aux::rebind_alloc<AllocatorT,
                                            threadsafe_queue_node<T>>::type
{
  private:
    typedef threadsafe_queue_node<T> node;

  public:
    typedef typename boost::log::aux::rebind_alloc<AllocatorT, node>::type
        allocator_type;
  ...
  private:
    ...
    struct auto_deallocate
    {
        auto_deallocate(allocator_type* alloc, node* dealloc,
                        node* destr) BOOST_NOEXCEPT : m_pAllocator(alloc),
  ...
  public:
    ...
    /*!
     * Checks if the queue is empty. Not thread-safe, the returned result may
     * not be actual.
     */
    bool unsafe_empty() const
    {
        return m_pImpl->unsafe_empty();
    }

    /*!
     * Puts a new element to the end of the queue. Thread-safe, can be called
     * concurrently by several threads, and concurrently with the \c pop
     * operation.
     */
    ...
    /*!
     * Attempts to pop an element from the beginning of the queue. Thread-safe,
     * can be called concurrently with the \c push operation. Should not be
    ...
    bool try_pop(reference value)
    {
        ...
        if (m_pImpl->try_pop(dealloc, destr))
        {
            ...
            auto_deallocate guard(static_cast<allocator_type*>(this),
                                  static_cast<node*>(dealloc), p);
