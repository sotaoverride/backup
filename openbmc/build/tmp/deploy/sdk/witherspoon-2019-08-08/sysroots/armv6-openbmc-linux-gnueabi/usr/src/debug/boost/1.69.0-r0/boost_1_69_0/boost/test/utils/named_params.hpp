...
struct param_type
    : mpl::if_<typename is_same<typename NP::id, typename Keyword::id>::type,
               typename remove_cv<typename NP::data_type>::type, DefaultType>
{
};
...
/// has_param<Params,Keyword>::value is true if Params has parameter
...
inline void report_access_to_invalid_parameter(bool v)
{
...
struct nil
{
    ...
    {
        nfp_detail::report_access_to_invalid_parameter(true);
        static typename remove_reference<T>::type* v = 0;
        return *v;
    }
...
struct named_parameter_base
{
    ...
    named_parameter_combine<NP, Derived> operator,(NP const& np) const
    {
        return named_parameter_combine<NP, Derived>(
            np, *static_cast<Derived const*>(this));
    }
};
      ...
      nfp_detail::named_parameter_base<named_parameter_combine<NP, Rest>>
{
    ...
    // Constructor
    named_parameter_combine(NP const& np, Rest const& r) : Rest(r), m_param(np)
    {
    }
    ...
    : nfp_detail::named_parameter_base<named_parameter<T, unique_id, RefType>>
{
    ...
    // Constructor
    explicit named_parameter(ref_type v) : m_value(v), m_erased(false)
    {
    }
    named_parameter(named_parameter const& np) :
        m_value(np.m_value), m_erased(np.m_erased)
    {
    }
    ...
    {
        return m_erased ? nil::inst().template any_cast<ref_type>() : m_value;
    }
...
struct keyword
{
    ...
    {
        return named_parameter<T const, unique_id>(t);
    }

    template <typename T>
    named_parameter<T, unique_id> operator=(T& t) const
    {
    ...
    {
        return named_parameter<char const*, unique_id, char const*>(t);
    }
};
...
struct typed_keyword<bool, unique_id, required>
    : keyword<unique_id, required>, named_parameter<bool, unique_id, bool>
{
    typedef unique_id id;
    ...
    {
        return named_parameter<bool, unique_id, bool>(false);
    }
};
...
template <typename T, typename Params, typename Keyword>
inline T opt_get(Params const& p, Keyword k, T default_val)
{
