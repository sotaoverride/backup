...
void FbShell::drawChars(CharAttr attr, u16 x, u16 y, u16 w, u16 num, u16* chars,
                        bool* dws)
{
...
{
    if (manager->activeShell() != this || mCursor.x >= w() || mCursor.y >= h())
        return;
    mCursor.showed ^= true;
    ...
    if (shape == CurDefault)
    {
        ...
        if (!inited)
        {
            ...
            if (!default_shape)
                default_shape = CurUnderline;
            else
                default_shape = CurBlock;
        }
    ...
    switch (shape)
    {
        ...
        case CurUnderline:
            screen->fillRect(FW(mCursor.x), FH(mCursor.y + 1) - 1, FW(1), 1,
                             mCursor.showed ? mCursor.attr.fcolor
                                            : mCursor.attr.bcolor);
            if (mImProxy)
            {
                Rectangle rect = {FW(mCursor.x), FH(mCursor.y + 1) - 1, FW(1),
                                  1};
                mImProxy->redrawImWin(rect);
            }
            break;
        ...
        default:
        {
            ...
            u16 x = mCursor.x;
            if (mCursor.attr.type == CharAttr::DoubleRight)
                x--;
            ...
            drawChars(attr, x, mCursor.y, dw ? FW(2) : FW(1), 1, &mCursor.code,
                      &dw);
            break;
        }
    }
...
void FbShell::mouseInput(u16 x, u16 y, s32 type, s32 buttons)
{
    if (type == Move)
    {
        ...
        if (attr.type == CharAttr::DoubleRight)
            x--;
...
void FbShell::expose(u16 x, u16 y, u16 w, u16 h)
{
    ...
    if (mode(CursorVisible) && mCursor.y >= y && mCursor.y < (y + h) &&
        mCursor.x >= x && mCursor.x < (x + w))
    {
        mCursor.showed = false;
        updateCursor();
    }
...
{
    if (mImProxy)
        mImProxy->changeCursorPos(mCursor.x, mCursor.y);
}

void FbShell::reportMode()
{
    if (mImProxy)
        mImProxy->changeTermMode(mode(CRWithLF), mode(ApplicKeypad),
                                 mode(CursorKeyEscO));
}
