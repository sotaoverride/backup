...
netsnmp_feature_child_of(software_running, libnetsnmpmibs)
    ...
    netsnmp_feature_child_of(swrun_max_processes, software_running)
        netsnmp_feature_child_of(swrun_count_processes_by_name,
                                 software_running)
            netsnmp_feature_child_of(swrun_count_processes_by_regex,
                                     software_running)
 ...
 */
static void _swrun_entry_release(netsnmp_swrun_entry* entry, void* unused);
...
int swrun_count_processes_by_regex(char* name, struct real_pcre* regexp)
{
    ...
    while ((entry = (netsnmp_swrun_entry*)ITERATOR_NEXT(it)) != NULL)
    {
        ...
        sprintf(fullCommand, "%s %s", entry->hrSWRunPath,
                entry->hrSWRunParameters);
        found = pcre_exec(regexp, NULL, fullCommand, strlen(fullCommand), 0, 0,
                          found_ndx, 30);
        if (found > 0)
        {
            i++;
...
netsnmp_cache* netsnmp_swrun_cache(void)
{
    ...
    if (!swrun_cache)
    {
        swrun_cache = netsnmp_cache_create(
            30, /* timeout in seconds */
            _cache_load, _cache_free, hrSWRunTable_oid, hrSWRunTable_oid_len);
        if (swrun_cache)
 ...
 */
netsnmp_container*
    netsnmp_swrun_container_load(netsnmp_container* user_container,
                                 u_int load_flags)
{
    ...
    if (0 != rc)
    {
        ...
        {
            DEBUGMSGTL(
                ("swrun:container:load", " discarding partial results\n"));
...
void netsnmp_swrun_container_free(netsnmp_container* container,
                                  u_int free_flags)
{
...
void netsnmp_swrun_container_free_items(netsnmp_container* container)
{
    ...
    {
        snmp_log(LOG_ERR,
                 "invalid container for netsnmp_swrun_container_free_items\n");
        return;
    }
    ...
    CONTAINER_CLEAR(container,
                    (netsnmp_container_obj_func*)_swrun_entry_release, NULL);
}
...
/**
 */
netsnmp_swrun_entry* netsnmp_swrun_entry_create(int32_t index)
{
    netsnmp_swrun_entry* entry = SNMP_MALLOC_TYPEDEF(netsnmp_swrun_entry);
...
/**
 */
NETSNMP_INLINE void netsnmp_swrun_entry_free(netsnmp_swrun_entry* entry)
{
    if (NULL == entry)
        return;
...
/**
 */
static void _swrun_entry_release(netsnmp_swrun_entry* entry, void* context)
{
