...
WRAP_K(krb5_db2_destroy,
       (krb5_context kcontext, char* conf_section, char** db_args),
       (kcontext, conf_section, db_args));
WRAP_K(krb5_db2_get_age, (krb5_context ctx, char* s, time_t* t), (ctx, s, t));

WRAP_K(krb5_db2_lock, (krb5_context context, int in_mode), (context, in_mode));
WRAP_K(krb5_db2_unlock, (krb5_context ctx), (ctx));

WRAP_K(krb5_db2_get_principal,
       (krb5_context ctx, krb5_const_principal p, unsigned int f,
        krb5_db_entry** d),
       (ctx, p, f, d));
WRAP_K(krb5_db2_put_principal,
       (krb5_context ctx, krb5_db_entry* d, char** db_args), (ctx, d, db_args));
WRAP_K(krb5_db2_delete_principal,
       (krb5_context context, krb5_const_principal searchfor),
       (context, searchfor));

WRAP_K(krb5_db2_iterate,
       (krb5_context ctx, char* s,
        krb5_error_code (*f)(krb5_pointer, krb5_db_entry*), krb5_pointer p,
        krb5_flags flags),
       (ctx, s, f, p, flags));

WRAP_K(krb5_db2_create_policy, (krb5_context context, osa_policy_ent_t entry),
       (context, entry));
WRAP_K(krb5_db2_get_policy,
       (krb5_context kcontext, char* name, osa_policy_ent_t* policy),
       (kcontext, name, policy));
WRAP_K(krb5_db2_put_policy, (krb5_context kcontext, osa_policy_ent_t policy),
       (kcontext, policy));
WRAP_K(krb5_db2_iter_policy,
       (krb5_context kcontext, char* match_entry, osa_adb_iter_policy_func func,
        void* data),
       (kcontext, match_entry, func, data));
WRAP_K(krb5_db2_delete_policy, (krb5_context kcontext, char* policy),
       (kcontext, policy));
...
WRAP_VOID(krb5_db2_audit_as_req,
          (krb5_context kcontext, krb5_kdc_req* request,
           const krb5_address* local_addr, const krb5_address* remote_addr,
           krb5_db_entry* client, krb5_db_entry* server,
           krb5_timestamp authtime, krb5_error_code error_code),
          (kcontext, request, local_addr, remote_addr, client, server, authtime,
           error_code));

static krb5_error_code hack_init(void)
{
    krb5_error_code c;
...
static krb5_error_code hack_cleanup(void)
{
