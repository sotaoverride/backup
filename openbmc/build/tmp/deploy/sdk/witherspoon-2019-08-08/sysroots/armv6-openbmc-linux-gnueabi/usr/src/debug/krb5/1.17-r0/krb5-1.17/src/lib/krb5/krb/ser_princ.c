 ...
 */
static krb5_error_code krb5_principal_size(krb5_context, krb5_pointer, size_t*);
static krb5_error_code krb5_principal_externalize(krb5_context, krb5_pointer,
                                                  krb5_octet**, size_t*);
static krb5_error_code krb5_principal_internalize(krb5_context, krb5_pointer*,
                                                  krb5_octet**, size_t*);
 ...
 *                                the krb5_principal.
 */
static krb5_error_code krb5_principal_size(krb5_context kcontext,
 ...
 */
static krb5_error_code krb5_principal_externalize(krb5_context kcontext,
                                                  krb5_pointer arg,
                                                  krb5_octet** buffer,
                                                  size_t* lenremain)
{
    ...
    if ((principal = (krb5_principal)arg))
    {
            ...
            (required <= remain))
        {
            if (!(kret = krb5_unparse_name(kcontext, principal, &fname)))
            {
                ...
                (void)krb5_ser_pack_int32((krb5_int32)strlen(fname), &bp,
                                          &remain);
                (void)krb5_ser_pack_bytes((krb5_octet*)fname, strlen(fname),
                                          &bp, &remain);
                (void)krb5_ser_pack_int32(KV5M_PRINCIPAL, &bp, &remain);
                *buffer = bp;
                *lenremain = remain;
 ...
 */
static krb5_error_code krb5_principal_internalize(krb5_context kcontext,
                                                  krb5_pointer* argp,
                                                  krb5_octet** buffer,
                                                  size_t* lenremain)
{
    ...
    *argp = NULL;
    ...
    kret =
        krb5_ser_unpack_bytes((krb5_octet*)tmpname, (size_t)ibuf, &bp, &remain);
    if (kret)
        goto cleanup;
    tmpname[ibuf] = '\0';
 ...
 */
krb5_error_code krb5_ser_principal_init(krb5_context kcontext)
{
