 ...
 * @get_parameter_type: the virtual function pointer for
 ...
 * @activate: the virtual function pointer for g_action_activate().  Note that
 * #GAction does not have an 'activate' signal but that implementations of it
 * may have one.
 *
...
void g_action_default_init(GActionInterface* iface)
{
     ...
     **/
    g_object_interface_install_property(
        iface, g_param_spec_boolean("enabled", P_("Enabled"),
     ...
     **/
    g_object_interface_install_property(
        iface, g_param_spec_variant("state", P_("State"),
...
void g_action_change_state(GAction* action, GVariant* value)
{
    ...
    G_ACTION_GET_IFACE(action)->change_state(action, value);
                                      ...
                                      GVariant** target_value, GError** error)
{
    ...
    switch (target[0])
    {
        ...
        case ':':
            ...
            *target_value =
                g_variant_ref_sink(g_variant_new_string(target + 2));
            break;
        ...
        case '(':
        {
            ...
            *target_value = g_variant_parse(
                NULL, target + 1, target + target_len - 1, NULL, error);
...
bad_fmt:
    if (error)
    {
        if (*error == NULL)
            g_set_error(
                error, G_VARIANT_PARSE_ERROR, G_VARIANT_PARSE_ERROR_FAILED,
                "Detailed action name '%s' has invalid format", detailed_name);
        else
            g_prefix_error(error,
                           "Detailed action name '%s' has invalid format: ",
                           detailed_name);
    }
