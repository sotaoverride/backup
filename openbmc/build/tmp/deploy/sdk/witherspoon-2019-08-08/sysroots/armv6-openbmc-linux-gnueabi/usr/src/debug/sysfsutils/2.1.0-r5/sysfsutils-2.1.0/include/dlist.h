...
void dlist_move(struct dlist* source, struct dlist* dest,
                struct dl_node* target, int direction);
void* dlist_insert(Dlist*, void*, int);

void* dlist_insert_sorted(struct dlist* list, void* new_elem,
...
int _dlist_merge(struct dlist* listsource, struct dlist* listdest,
                 unsigned int passcount, int (*compare)(void*, void*));

void* dlist_find_custom(struct dlist* list, void* target,
                        int (*comp)(void*, void*));
...
void* _dlist_insert_dlnode(struct dlist* list, struct dl_node* new_node,
                           int direction);
...
#define dlist_for_each(list)                                                   \
    for (dlist_start(list), dlist_next(list); (list)->marker != (list)->head;  \
         dlist_next(list))
...
#define dlist_for_each_rev(list)                                               \
    for (dlist_end(list), dlist_prev(list); (list)->marker != (list)->head;    \
         dlist_prev(list))
...
#define dlist_for_each_data(list, data_iterator, datatype)                     \
    for (dlist_start(list), (data_iterator) = (datatype*)dlist_next(list);     \
         ...
         (data_iterator) = (datatype*)dlist_next(list))
...
#define dlist_for_each_data_rev(list, data_iterator, datatype)                 \
    for (dlist_end(list), (data_iterator) = (datatype*)dlist_prev(list);       \
         (list)->marker != (list)->head;                                       \
         (data_iterator) = (datatype*)dlist_prev(list))
...
#define dlist_for_each_data_nomark(list, iterator, data_iterator, datatype)    \
    for ((iterator) = (list)->head->next,                                      \
        (data_iterator) = (datatype*)(iterator)->data;                         \
         (iterator) != (list)->head; (iterator) = (iterator)->next,            \
        (data_iterator) = (datatype*)(iterator))
 ...
 * datatype:  actual type of the contents in the dl_node->data
 */
#define dlist_for_each_data_nomark_rev(list, iterator, data_iterator,          \
                                       datatype)                               \
    for ((iterator) = (list)->head->prev,                                      \
        (data_iterator) = (datatype*)(iterator)->data;                         \
         (iterator) != (list)->head; (iterator) = (iterator)->prev,            \
        (data_iterator) = (datatype*)(iterator))
