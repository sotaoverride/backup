...
class special_values_parser
{
  public:
    ...
    {
        sv_strings(string_type(nadt_string), string_type(neg_inf_string),
                   string_type(pos_inf_string),
                   string_type(min_date_time_string),
                   string_type(max_date_time_string));
    }

    //! Creates a special_values_parser using a user defined set of element
    //! strings
                          ...
                          typename collection_type::iterator end)
    {
        ...
        m_sv_strings =
            parse_tree_type(phrases, static_cast<int>(not_a_date_time));
    }
    ...
    //! The parser is expensive to create, and not thread-safe so it cannot be
    //! static therefore given a string, determine if it is likely to be a
    //! special value. A negative response is a definite no, whereas a positive
    //! is only likely and match() should be called and return value checked.
    ...
    //!                  mr.current_match is set to the corresponding
    //!                  special_value or -1
