...
#define mkdir(path, perms)                                                     \
    mkdir(path) /* Omit the perms argument to match POSIX signature */
#else           /* MSVC and other windows compilers */
#define mkdir(path, perms)                                                     \
    _mkdir(path) /* Omit the perms argument to match POSIX signature */
...
static const int tight2turbo_qual[10] = {15, 29, 41, 42, 62,
                                         77, 79, 86, 92, 100};

static const int tight2turbo_subsamp[10] = {1, 1, 1, 2, 2, 2, 0, 0, 0, 0};
#endif
...
rfbClientPtr rfbReverseConnection(rfbScreenInfoPtr rfbScreen, char* host,
                                  int port)
{
    int sock;
    rfbClientPtr cl;
...
static rfbClientPtr rfbNewTCPOrUDPClient(rfbScreenInfoPtr rfbScreen, int sock,
                                         rfbBool isUDP)
{
    ...
    else
    {
        ...
        if (setsockopt(sock, IPPROTO_TCP, TCP_NODELAY, (char*)&one,
                       sizeof(one)) < 0)
        {
            rfbLogPerror("setsockopt failed: can't set TCP_NODELAY flag, non "
                         "TCP socket?");
        }
...
#ifdef LIBVNCSERVER_WITH_WEBSOCKETS
        /*
         * Wait a few ms for the client to send WebSockets connection (TLS/SSL
         * or plain)
         */
...
static void rfbProcessClientProtocolVersion(rfbClientPtr cl)
{
    ...
    if (major_ != rfbProtocolMajorVersion)
    {
        rfbErr("RFB protocol version mismatch - server %d.%d, client %d.%d",
               cl->screen->protocolMajorVersion,
               cl->screen->protocolMinorVersion, major_, minor_);
        rfbCloseClient(cl);
        return;
    }
    ...
    rfbLog("Protocol version sent %d.%d, using %d.%d\n", major_, minor_,
           rfbProtocolMajorVersion, cl->protocolMinorVersion);
 ...
 */
static rfbBool rectSwapIfLEAndClip(uint16_t* x, uint16_t* y, uint16_t* w,
                                   uint16_t* h, rfbClientPtr cl)
{
    int x1 = Swap16IfLE(*x);
    int y1 = Swap16IfLE(*y);
    int w1 = Swap16IfLE(*w);
...
rfbBool rfbSendKeyboardLedState(rfbClientPtr cl)
{
    ...
    rfbStatRecordEncodingSent(cl, rfbEncodingKeyboardLedState,
                              sz_rfbFramebufferUpdateRectHeader,
                              sz_rfbFramebufferUpdateRectHeader);
...
rfbBool rfbSendSupportedMessages(rfbClientPtr cl)
{
    ...
    rfbStatRecordEncodingSent(
        cl, rfbEncodingSupportedMessages,
...
rfbBool rfbSendSupportedEncodings(rfbClientPtr cl)
{
    ...
    rfbStatRecordEncodingSent(
        cl, rfbEncodingSupportedEncodings,
...
rfbBool rfbSendServerIdentity(rfbClientPtr cl)
{
    ...
    if (cl->ublen + sz_rfbFramebufferUpdateRectHeader + (strlen(buffer) + 1) >
        UPDATE_BUF_SIZE)
    {
        if (!rfbSendUpdateBuf(cl))
            return FALSE;
...
rfbBool rfbSendTextChatMessage(rfbClientPtr cl, uint32_t length, char* buffer)
{
    ...
    rfbStatRecordMessageSent(cl, rfbTextChat, sz_rfbTextChatMsg + bytesToSend,
                             sz_rfbTextChatMsg + bytesToSend);
...
rfbBool rfbSendFileTransferMessage(rfbClientPtr cl, uint8_t contentType,
                                   ...
                                   uint32_t length, const char* buffer)
{
    ...
    ft.pad = 0; /* UltraVNC did not Swap16LE(ft.contentParam) (Looks like it
                   might be BigEndian) */
    ...
    */
    ...
    rfbStatRecordMessageSent(cl, rfbFileTransfer,
                             sz_rfbFileTransferMsg + length,
                             sz_rfbFileTransferMsg + length);
...
rfbBool rfbFilenameTranslate2UNIX(rfbClientPtr cl, /* in */ char* path,
                                  /* out */ char* unixPath,
                                  size_t unixPathMaxLen)
{
     ...
     * undesirable side effects Instead check if destination buffer is big
     * enough
     */
...
rfbBool rfbSendDirContent(rfbClientPtr cl, int length, char* buffer)
{
    ...
    if (dirp == NULL)
#endif
        return rfbSendFileTransferMessage(cl, rfbDirPacket, rfbADirectory, 0, 0,
                                          NULL);
...
#endif
    {
        /* get stats */
#ifdef WIN32
        snprintf(retfilename, sizeof(retfilename), "%s/%s", path,
        ...
        if (retval == 0)
        {
            ...
            win32filename.ftCreationTime.dwLowDateTime =
                winFindData.ftCreationTime.dwLowDateTime;
            win32filename.ftCreationTime.dwHighDateTime =
                winFindData.ftCreationTime.dwHighDateTime;
            win32filename.ftLastAccessTime.dwLowDateTime =
                winFindData.ftLastAccessTime.dwLowDateTime;
            win32filename.ftLastAccessTime.dwHighDateTime =
                winFindData.ftLastAccessTime.dwHighDateTime;
            win32filename.ftLastWriteTime.dwLowDateTime =
                winFindData.ftLastWriteTime.dwLowDateTime;
            win32filename.ftLastWriteTime.dwHighDateTime =
                winFindData.ftLastWriteTime.dwHighDateTime;
            ...
            strcpy((char*)win32filename.cAlternateFileName,
                   winFindData.cAlternateFileName);
#else
            win32filename.dwFileAttributes =
                Swap32IfBE(RFB_FILE_ATTRIBUTE_NORMAL);
            if (S_ISDIR(statbuf.st_mode))
                win32filename.dwFileAttributes =
                    Swap32IfBE(RFB_FILE_ATTRIBUTE_DIRECTORY);
            win32filename.ftCreationTime.dwLowDateTime =
                Swap32IfBE(statbuf.st_ctime); /* Intel Order */
            win32filename.ftCreationTime.dwHighDateTime = 0;
            win32filename.ftLastAccessTime.dwLowDateTime =
                Swap32IfBE(statbuf.st_atime); /* Intel Order */
            win32filename.ftLastAccessTime.dwHighDateTime = 0;
            win32filename.ftLastWriteTime.dwLowDateTime =
                Swap32IfBE(statbuf.st_mtime); /* Intel Order */
            win32filename.ftLastWriteTime.dwHighDateTime = 0;
            win32filename.nFileSizeLow =
                Swap32IfBE(statbuf.st_size); /* Intel Order */
            win32filename.nFileSizeHigh = 0;
            win32filename.dwReserved0 = 0;
            win32filename.dwReserved1 = 0;

            /* If this had the full path, we would need to translate to DOS
             * format ("C:\") */
            /* rfbFilenameTranslate2DOS(cl, retfilename,
             * win32filename.cFileName); */
            strcpy((char*)win32filename.cFileName, direntp->d_name);
#endif

            /* Do not show hidden files (but show how to move up the tree) */
            if ((strcmp((char*)win32filename.cFileName, "..") == 0) ||
                (win32filename.cFileName[0] != '.'))
            {
                nOptLen = sizeof(RFB_FIND_DATA) - MAX_PATH - 14 +
                          strlen((char*)win32filename.cFileName);
                /*
                rfbLog("rfbProcessFileTransfer() rfbDirContentRequest:
                rfbRDirContent: Sending \"%s\"\n", (char
                *)win32filename.cFileName);
                */
                if (rfbSendFileTransferMessage(cl, rfbDirPacket, rfbADirectory,
                                               0, nOptLen,
                                               (char*)&win32filename) == FALSE)
                {
#ifdef WIN32
...
char* rfbProcessFileTransferReadBuffer(rfbClientPtr cl, uint32_t length)
{
    ...
    /*
       ...
       length equals 0XFFFFFFFF, i.e. SIZE_MAX for 32-bit systems. On 64-bit
       systems, a length of 0XFFFFFFFF will safely be allocated since this check
       will never trigger and malloc() can digest length+1 without problems as
       length is a uint32_t. We also later pass length to rfbReadExact() that
       expects a signed int type and that might wrap on platforms with a 32-bit
       int type if length is bigger than 0X7FFFFFFF.
    */
    ...
    if ((cl->fileTransfer.fd != -1) && (cl->fileTransfer.sending == 1))
    {
        ...
        if (n > 0)
        {
            ...
            switch (bytesRead)
            {
                case 0:
                    /*
                    rfbLog("rfbSendFileTransferChunk(): End-Of-File
                    Encountered\n");
                    */
                    retval = rfbSendFileTransferMessage(cl, rfbEndOfFile, 0, 0,
                                                        0, NULL);
                ...
                case -1:
                    ...
                    retval = rfbSendFileTransferMessage(
                        cl, rfbAbortFileTransfer, 0, 0, 0, NULL);
                    close(cl->fileTransfer.fd);
                    cl->fileTransfer.fd = -1;
                    cl->fileTransfer.sending = 0;
                    cl->fileTransfer.receiving = 0;
                    ...
                    */
                    if (!cl->fileTransfer.compressionEnabled)
                        return rfbSendFileTransferMessage(
                            cl, rfbFilePacket, 0, 0, bytesRead, (char*)readBuf);
                    else
                    {
#ifdef LIBVNCSERVER_HAVE_LIBZ
                        nRetC = compress(compBuf, &nMaxCompSize, readBuf,
                                         bytesRead);
                        /*
                        ...
                        */
                        ...
                        if ((nRetC == 0) && (nMaxCompSize < bytesRead))
                            ...
                            return rfbSendFileTransferMessage(cl, rfbFilePacket,
                                                              0, 0, bytesRead,
                                                              (char*)readBuf);
#else
...
rfbBool rfbProcessFileTransfer(rfbClientPtr cl, uint8_t contentType,
                               uint8_t contentParam, uint32_t size,
                               uint32_t length)
{
    ...
    */
    ...
    switch (contentType)
    {
        case rfbDirContentRequest:
            switch (contentParam)
            {
                case rfbRDrivesList: /* Client requests the List of Local Drives
                    ...
                    */
                    /* Format when filled :
                     * "C:\<NULL>D:\<NULL>....Z:\<NULL><NULL>
                     *
                     * We replace the "\" char following the drive letter and
                     * ":" with a char corresponding to the type of drive We
                     * obtain something like
                     * "C:l<NULL>D:c<NULL>....Z:n\<NULL><NULL>" Isn't it ugly ?
                     * DRIVE_FIXED = 'l'     (local?)
                    ...
                    /* in unix, there are no 'drives'  (We could list mount
                     * points though) We fake the root as a "C:" for the
                     * Winblows users
                     */
                    filename2[0] = 'C';
                    filename2[1] = ':';
                    ...
                    filename2[4] = 0;
                    retval = rfbSendFileTransferMessage(
                        cl, rfbDirPacket, rfbADrivesList, 0, 5, filename2);
                    if (buffer != NULL)
                        free(buffer);
                    return retval;
                    break;
                case rfbRDirContent: /* Client requests the content of a
                                        directory */
                    /*
                    rfbLog("rfbProcessFileTransfer() rfbDirContentRequest:
                    rfbRDirContent\n");
                    */
                    if ((buffer = rfbProcessFileTransferReadBuffer(
                             cl, length)) == NULL)
                        return FALSE;
            ...
            */
            ...
            if (DB)
                rfbLog(
                    ...
                    buffer, filename1,
                    (cl->fileTransfer.fd == -1 ? "Failed" : "Success"),
                    cl->fileTransfer.fd);
            ...
            if (cl->fileTransfer.fd != -1)
            {
                ...
                else
                {
                    ...
                    strftime(timespec, sizeof(timespec), "%m/%d/%Y %H:%M",
                             gmtime(&statbuf.st_ctime));
                    buffer = realloc(buffer, length + strlen(timespec) +
                                                 2); /* comma, and Null term */
                    if (buffer == NULL)
                    {
                        rfbLog(
                            "rfbProcessFileTransfer() rfbFileTransferRequest: "
            ...
            /*
            rfbLog("rfbProcessFileTransfer()
            rfbFileTransferRequest(\"%s\"->\"%s\")%s\n", buffer, filename1,
            (size==1?" <Compression Enabled>":""));
            */

            /* File Size in bytes, 0xFFFFFFFF (-1) means error */
            retval = rfbSendFileTransferMessage(
                cl, rfbFileHeader, 0,
                (cl->fileTransfer.fd == -1 ? -1 : statbuf.st_size), length,
                buffer);
            ...
            cl->fileTransfer.receiving = 0;
            cl->fileTransfer.sending =
                0; /* set when we receive a rfbFileHeader: */
        ...
        case rfbFileHeader:
            /* Destination file (viewer side) is ready for reception (size > 0)
             * or not (size = -1) */
            if (size == -1)
            {
                rfbLog("rfbProcessFileTransfer() rfbFileHeader (error, "
                       "aborting)\n");
                close(cl->fileTransfer.fd);
                cl->fileTransfer.fd = -1;
                return TRUE;
            ...
            /*
            rfbLog("rfbProcessFileTransfer() rfbFileHeader (%d bytes of a
            file)\n", size);
            */
            ...
            */
            ...
            if (p != NULL)
            {
                *p = '\0';
                ...
                szFileTime[sizeof(szFileTime) - 1] =
                    '\x00'; /* ensure NULL terminating byte is present, even if
                               copy overflowed */
            }
            else
                szFileTime[0] = 0;
            ...
            cl->fileTransfer.fd =
                open(filename1, O_CREAT | O_WRONLY | O_TRUNC, 0744);
            if (DB)
                rfbLog("rfbProcessFileTransfer() "
                       "rfbFileTransferOffer(\"%s\"->\"%s\") %s %s fd=%d\n",
                       buffer, filename1,
                       (cl->fileTransfer.fd == -1 ? "Failed" : "Success"),
                       (cl->fileTransfer.fd == -1 ? strerror(errno) : ""),
                       cl->fileTransfer.fd);
            /*
             */
            ...
            /* File Size in bytes, 0xFFFFFFFF (-1) means error */
            retval = rfbSendFileTransferMessage(
                cl, rfbFileAcceptHeader, 0,
                (cl->fileTransfer.fd == -1 ? -1 : 0), length, buffer);
            if (cl->fileTransfer.fd == -1)
            {
                free(buffer);
                return retval;
...
#ifdef LIBVNCSERVER_HAVE_LIBZ
                    /* compressed packet */
                    nRet = uncompress(compBuff, &nRawBytes,
                                      (const unsigned char*)buffer, length);
                    if (nRet == Z_OK)
                        retval = write(cl->fileTransfer.fd, (char*)compBuff,
                                       nRawBytes);
                    else
                        retval = -1;
#else
        ...
        case rfbAbortFileTransfer:
            ...
            else
            {
                ...
                if (contentParam == 0)
                {
                    rfbLog("rfbProcessFileTransfer() File Transfer Permission "
                           "DENIED! (Client Version <=RC18)\n");
                    /* Old method for FileTransfer handshake perimssion (<=RC18)
                     * (Deny it)*/
                    return rfbSendFileTransferMessage(cl, rfbAbortFileTransfer,
                                                      0, -1, 0, "");
                }
                ...
                if (cl->screen->getFileTransferPermission != NULL)
                {
                    ...
                    {
                        rfbLog("rfbProcessFileTransfer() File Transfer "
                               "Permission Granted!\n");
                        return rfbSendFileTransferMessage(
                            cl, rfbFileTransferAccess, 0, 1, 0,
                            ""); /* Permit */
                    }
                    else
                    {
                        rfbLog("rfbProcessFileTransfer() File Transfer "
                               "Permission DENIED!\n");
                        return rfbSendFileTransferMessage(
                            cl, rfbFileTransferAccess, 0, -1, 0, ""); /* Deny */
                    }
                }
                else
                {
        ...
        case rfbCommand:
            ...
            */
            ...
            switch (contentParam)
            {
                case rfbCDirCreate: /* Client requests the creation of a
                                       directory */
                    if (!rfbFilenameTranslate2UNIX(cl, buffer, filename1,
                                                   sizeof(filename1)))
                        goto fail;
                    ...
                    /*
                     */
                    retval = rfbSendFileTransferMessage(cl, rfbCommandReturn,
                                                        rfbADirCreate, retval,
                                                        length, buffer);
                    if (buffer != NULL)
                        free(buffer);
                    return retval;
                case rfbCFileDelete: /* Client requests the deletion of a file
                    ...
                    if (stat(filename1, &statbuf) == 0)
                    {
                        ...
                        retval = -1;
                    retval = rfbSendFileTransferMessage(cl, rfbCommandReturn,
                                                        rfbAFileDelete, retval,
                                                        length, buffer);
                    if (buffer != NULL)
                        free(buffer);
                    return retval;
                case rfbCFileRename: /* Client requests the Renaming of a
                                        file/directory */
                    p = strrchr(buffer, '*');
                    if (p != NULL)
                    {
                        ...
                        *p = '\0';
                        ...
                        if (DB)
                            rfbLog("rfbProcessFileTransfer() rfbCommand: "
                                   "rfbCFileRename(\"%s\"->\"%s\" -->> "
                                   "\"%s\"->\"%s\") %s\n",
                                   buffer, filename1, p + 1, filename2,
                                   (retval == -1 ? "Failed" : "Success"));
                        /*
                         */
                        ...
                        *p = '*';
                        retval = rfbSendFileTransferMessage(
                            cl, rfbCommandReturn, rfbAFileRename, retval,
                            length, buffer);
                        if (buffer != NULL)
                            free(buffer);
                        return retval;
...
static void rfbProcessClientNormalMessage(rfbClientPtr cl)
{
    ...
    switch (msg.type)
    {
        ...
        case rfbSetPixelFormat:
            ...
            rfbStatRecordMessageRcvd(cl, msg.type, sz_rfbSetPixelFormatMsg,
        ...
        /* NOTE: Some clients send us a set of encodings (ie: PointerPos)
         * designed to enable/disable features... We may want to look into
         * this... Example: case rfbEncodingXCursor:
        ...
        case rfbSetEncodings:
        {
            ...
            rfbStatRecordMessageRcvd(
                cl, msg.type, sz_rfbSetEncodingsMsg + (msg.se.nEncodings * 4),
                sz_rfbSetEncodingsMsg + (msg.se.nEncodings * 4));

            /*
             * UltraVNC Client has the ability to adapt to changing network
             * environments So, let's give it a change to tell us what it wants
             * now!
             */
            if (cl->preferredEncoding != -1)
                lastPreferredEncoding = cl->preferredEncoding;

            /* Reset all flags to defaults (allows us to switch between
             * PointerPos and Server Drawn Cursors) */
            cl->preferredEncoding = -1;
            cl->useCopyRect = FALSE;
            cl->useNewFBSize = FALSE;
            ...
            for (i = 0; i < msg.se.nEncodings; i++)
            {
                ...
                switch (enc)
                {
...
#endif
                        /* The first supported encoding is the 'preferred'
                         * encoding */
                        if (cl->preferredEncoding == -1)
                    ...
                    case rfbEncodingXCursor:
                        ...
                        {
                            rfbLog("Enabling X-style cursor updates for client "
                                   "%s\n",
                                   cl->host);
                    ...
                    case rfbEncodingPointerPos:
                        ...
                        {
                            rfbLog("Enabling cursor position updates for "
                                   "client %s\n",
                                   cl->host);
                    ...
                    case rfbEncodingLastRect:
                        ...
                        {
                            rfbLog("Enabling LastRect protocol extension for "
                                   "client "
                                   "%s\n",
                                   cl->host);
                    ...
                    case rfbEncodingNewFBSize:
                        if (!cl->useNewFBSize)
                        {
                            rfbLog("Enabling NewFBSize protocol extension for "
                                   "client "
                                   "%s\n",
                                   cl->host);
                    ...
                    case rfbEncodingKeyboardLedState:
                        ...
                        {
                            rfbLog("Enabling KeyboardLedState protocol "
                    ...
                    case rfbEncodingSupportedMessages:
                        ...
                        {
                            rfbLog("Enabling SupportedMessages protocol "
                                   "extension for client "
                                   "%s\n",
                                   cl->host);
                    ...
                    case rfbEncodingSupportedEncodings:
                        ...
                        {
                            rfbLog("Enabling SupportedEncodings protocol "
                                   "extension for client "
                                   "%s\n",
                                   cl->host);
                    ...
                    case rfbEncodingServerIdentity:
                        ...
                        {
                            rfbLog("Enabling ServerIdentity protocol extension "
                                   "for client "
                                   "%s\n",
                                   cl->host);
                    ...
                    default:
                                 ...
                                 enc <= (uint32_t)rfbEncodingQualityLevel9)
                        {
                            ...
                            rfbLog(
                                "Using image quality level %d for client %s\n",
                                cl->tightQualityLevel, cl->host);
#ifdef LIBVNCSERVER_HAVE_LIBJPEG
                            cl->turboQualityLevel =
                                tight2turbo_qual[enc & 0x0F];
                            cl->turboSubsampLevel =
                                tight2turbo_subsamp[enc & 0x0F];
                            rfbLog("Using JPEG subsampling %d, Q%d for client "
                                   "%s\n",
                                   cl->turboSubsampLevel, cl->turboQualityLevel,
                                   cl->host);
                        }
                        else if (enc >= (uint32_t)rfbEncodingFineQualityLevel0 +
                                            1 &&
                                 enc <=
                                     (uint32_t)rfbEncodingFineQualityLevel100)
                        {
                            cl->turboQualityLevel = enc & 0xFF;
                            rfbLog(
                                "Using fine quality level %d for client %s\n",
...
#endif
                        {
                            ...
                            for (e = cl->extensions; e;)
                            {
                                ...
                                if (e->extension->enablePseudoEncoding &&
                                    e->extension->enablePseudoEncoding(
                                        cl, &e->data, (int)enc))
                            ...
                            if (e == NULL)
                            {
                                ...
                                for (e = rfbGetExtensionIterator(); e;)
                                {
                                    ...
                                    while (encs && *encs != 0)
                                    {
                                        ...
                                        {
                                            void* data = NULL;
                                            if (!e->enablePseudoEncoding(
                                                    cl, &data, (int)enc))
                                            {
                                                rfbLog("Installed extension "
                                                       "pretends to handle "
                                                       "pseudo encoding 0x%x, "
                                                       "but does not!\n",
                                                       (int)enc);
                                            }
                                            else
                                ...
                                if (!handled)
                                    rfbLog("rfbProcessClientNormalMessage: "
                                           "ignoring unsupported encoding type "
                                           "%s\n",
                                           encodingName(enc, encBuf,
                                                        sizeof(encBuf)));
                            }
                        }
                }
            ...
            if (cl->preferredEncoding == -1)
            {
                if (lastPreferredEncoding == -1)
                {
                    ...
                    rfbLog("Defaulting to %s encoding for client %s\n",
                           encodingName(cl->preferredEncoding, encBuf,
                                        sizeof(encBuf)),
                           cl->host);
                }
                else
            ...
            else
            {
                ...
                else
                {
                    rfbLog("Switching from %s to %s Encoding for client %s\n",
                           encodingName(lastPreferredEncoding, encBuf2,
                                        sizeof(encBuf2)),
                           encodingName(cl->preferredEncoding, encBuf,
                                        sizeof(encBuf)),
                           cl->host);
        ...
        case rfbFramebufferUpdateRequest:
        {
             ...
             */
            if (!rectSwapIfLEAndClip(&msg.fur.x, &msg.fur.y, &msg.fur.w,
                                     &msg.fur.h, cl))
            {
            ...
            tmpRegion =
                sraRgnCreateRect(msg.fur.x, msg.fur.y, msg.fur.x + msg.fur.w,
                                 msg.fur.y + msg.fur.h);
        ...
        case rfbKeyEvent:
            ...
            rfbStatRecordMessageRcvd(cl, msg.type, sz_rfbKeyEventMsg,
                                     sz_rfbKeyEventMsg);

            if (!cl->viewOnly)
            {
                cl->screen->kbdAddEvent(msg.ke.down,
                                        (rfbKeySym)Swap32IfLE(msg.ke.key), cl);
            }
        ...
        case rfbPointerEvent:
            ...
            rfbStatRecordMessageRcvd(cl, msg.type, sz_rfbPointerEventMsg,
            ...
            if (!cl->viewOnly)
            {
                    ...
                    cl->screen->deferPtrUpdateTime == 0)
                {
                    cl->screen->ptrAddEvent(msg.pe.buttonMask,
                                            ScaleX(cl->scaledScreen, cl->screen,
                                                   Swap16IfLE(msg.pe.x)),
                                            ScaleY(cl->scaledScreen, cl->screen,
                                                   Swap16IfLE(msg.pe.y)),
                                            cl);
                ...
                {
                    cl->lastPtrX = ScaleX(cl->scaledScreen, cl->screen,
                                          Swap16IfLE(msg.pe.x));
                    cl->lastPtrY = ScaleY(cl->scaledScreen, cl->screen,
                                          Swap16IfLE(msg.pe.y));
                    cl->lastPtrButtons = msg.pe.buttonMask;
                }
            }
        ...
        case rfbFileTransfer:
            ...
            /* record statistics in rfbProcessFileTransfer as length is filled
             * with garbage when it is not valid */
            rfbProcessFileTransfer(cl, msg.ft.contentType, msg.ft.contentParam,
                                   msg.ft.size, msg.ft.length);
            return;

        case rfbSetSW:
            if ((n = rfbReadExact(cl, ((char*)&msg) + 1, sz_rfbSetSWMsg - 1)) <=
                0)
            {
                if (n != 0)
                    rfbLogPerror("rfbProcessClientNormalMessage: read");
            ...
            rfbStatRecordMessageRcvd(cl, msg.type, sz_rfbSetSWMsg,
                                     sz_rfbSetSWMsg);
            /* msg.sw.status is not initialized in the ultraVNC viewer and
             * contains random numbers (why???) */
        ...
        case rfbSetServerInput:
            ...
            rfbStatRecordMessageRcvd(cl, msg.type, sz_rfbSetServerInputMsg,
                                     sz_rfbSetServerInputMsg);

            /* msg.sim.pad is not initialized in the ultraVNC viewer and
             * contains random numbers (why???) */
        ...
        case rfbTextChat:
            ...
            switch (msg.tc.length)
            {
                ...
                case rfbTextChatFinished:
                    ...
                    str = NULL;
                    rfbStatRecordMessageRcvd(cl, msg.type, sz_rfbTextChatMsg,
                                             sz_rfbTextChatMsg);
                    break;
                default:
                    if ((msg.tc.length > 0) && (msg.tc.length < rfbTextMaxSize))
                    {
                        ...
                        if (str == NULL)
                        {
                            rfbLog("Unable to malloc %d bytes for a TextChat "
                                   "Message\n",
                                   msg.tc.length);
                            rfbCloseClient(cl);
                            return;
                        ...
                        rfbStatRecordMessageRcvd(
                            cl, msg.type, sz_rfbTextChatMsg + msg.tc.length,
                            sz_rfbTextChatMsg + msg.tc.length);
                    }
                    else
                    {
                        /* This should never happen */
                        rfbLog("client sent us a Text Message that is too big "
                               "%d>%d\n",
            ...
            /* Note: length can be commands: rfbTextChatOpen, rfbTextChatClose,
             * and rfbTextChatFinished at which point, the str is NULL (as it is
             * not sent)
             */
        ...
        case rfbClientCutText:
             ...
             * to rfbReadExact()'s int argument, to rfbStatRecordMessageRcvd()'s
             * int argument increased of sz_rfbClientCutTextMsg, and to
             * setXCutText()'s int argument. Here we impose a limit of 1 MB so
             * that the value fits into all of the types to prevent from
             * misinterpretation and thus from accessing uninitialized memory
             * (CVE-2018-7225) and also to prevent from a denial-of-service by
             * allocating too much memory in the server. */
            ...
            {
                rfbLogPerror(
                    "rfbProcessClientNormalMessage: not enough memory");
                rfbCloseClient(cl);
                return;
            }
            ...
            rfbStatRecordMessageRcvd(cl, msg.type,
                                     sz_rfbClientCutTextMsg + msg.cct.length,
                                     sz_rfbClientCutTextMsg + msg.cct.length);
            if (!cl->viewOnly)
            {
        ...
        case rfbXvp:
            ...
            if ((n = rfbReadExact(cl, ((char*)&msg) + 1, sz_rfbXvpMsg - 1)) <=
                0)
            {
                if (n != 0)
                    rfbLogPerror("rfbProcessClientNormalMessage: read");
            ...
            else
            {
                ...
                if (cl->screen->xvpHook &&
                    !cl->screen->xvpHook(cl, msg.xvp.version, msg.xvp.code))
        ...
        default:
        {
            ...
            for (e = cl->extensions; e;)
            {
                ...
                {
                    rfbStatRecordMessageRcvd(
                        cl, msg.type, 0, 0); /* Extension should handle this */
                    return;
                }
                e = next;
            }
                                 ...
                                 sraRegionPtr givenUpdateRegion)
{
    ...
    if (cl->useNewFBSize && cl->newFBSizePending)
    {
        ...
        if (!rfbSendNewFBSize(cl, cl->scaledScreen->width,
    ...
    if (cl->preferredEncoding == rfbEncodingCoRRE)
    {
        ...
        for (i = sraRgnGetIterator(updateRegion); sraRgnIteratorNext(i, &rect);)
        {
            ...
            if (cl->screen != cl->scaledScreen)
                rfbScaledCorrection(cl->screen, cl->scaledScreen, &x, &y, &w,
                                    &h, "rfbSendFramebufferUpdate");
    ...
    if (nUpdateRegionRects != 0xFFFF)
    {
        ...
        fu->nRects = Swap16IfLE(
            (uint16_t)(sraRgnCountRects(updateCopyRegion) + nUpdateRegionRects +
                       !!sendCursorShape + !!sendCursorPos +
                       !!sendKeyboardLedState + !!sendSupportedMessages +
                       !!sendSupportedEncodings + !!sendServerIdentity));
    }
    else
    {
        fu->nRects = 0xFFFF;
    ...
    (defined(LIBVNCSERVER_HAVE_LIBZ) || defined(LIBVNCSERVER_HAVE_LIBPNG))
            case rfbEncodingTight:
...
rfbBool rfbSendLastRectMarker(rfbClientPtr cl)
{
    ...
    rfbStatRecordEncodingSent(cl, rfbEncodingLastRect,
                              sz_rfbFramebufferUpdateRectHeader,
                              sz_rfbFramebufferUpdateRectHeader);
...
rfbBool rfbSendNewFBSize(rfbClientPtr cl, int w, int h)
{
    ...
    if (cl->PalmVNC == TRUE)
        rfbLog("Sending rfbEncodingNewFBSize in response to a PalmVNC style "
               "framebuffer resize (%dx%d)\n",
               w, h);
    else
    ...
    rfbStatRecordEncodingSent(cl, rfbEncodingNewFBSize,
                              sz_rfbFramebufferUpdateRectHeader,
                              sz_rfbFramebufferUpdateRectHeader);
...
rfbBool rfbSendSetColourMapEntries(rfbClientPtr cl, int firstColour,
                                   int nColours)
{
    ...
    for (i = 0; i < nColours; i++)
    {
        if (i < (int)cm->count)
        {
            ...
            else
            {
                ...
                rgb[i * 3 + 1] =
                    Swap16IfLE((unsigned short)cm->data.bytes[i * 3 + 1]);
                rgb[i * 3 + 2] =
                    Swap16IfLE((unsigned short)cm->data.bytes[i * 3 + 2]);
            }
        }
    }
...
void rfbSendServerCutText(rfbScreenInfoPtr rfbScreen, char* str, int len)
{
    ...
    while ((cl = rfbClientIteratorNext(iterator)) != NULL)
    {
        ...
        UNLOCK(cl->sendMutex);
        rfbStatRecordMessageSent(cl, rfbServerCutText,
                                 sz_rfbServerCutTextMsg + len,
                                 sz_rfbServerCutTextMsg + len);
    }
    rfbReleaseClientIterator(iterator);
}
...
void rfbProcessUDPInput(rfbScreenInfoPtr rfbScreen)
{
    ...
    switch (msg.type)
    {
        ...
        case rfbPointerEvent:
            ...
            cl->screen->ptrAddEvent(msg.pe.buttonMask, Swap16IfLE(msg.pe.x),
                                    Swap16IfLE(msg.pe.y), cl);
            break;
