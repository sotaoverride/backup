...
static const char sample_public_key_2048[] =
    ...
    " (dsa"
    "  (p "
    "#9DB6FB5951B66BB6FE1E140F1D2CE5502374161FD6538DF1648218642F0B5C48C8F7A41AA"
    "DFA187324B87674FA1822B00F1ECF8136943D7C55757264E5A1A44FFE012E9936E00C1D3E9"
    "310B01C7D179805D3058B2A9F4BB6F9716BFE6117C6B5B3CC4D9BE341104AD4A80AD6C94E0"
    "05F4B993E14F091EB51743BF33050C38DE235567E1B34C3D6A5C0CEAA1A0F368213C3D1984"
    "3D0B4B09DCB9FC72D39C8DE41F1BF14D4BB4563CA28371621CAD3324B6A2D392145BEBFAC7"
    "48805236F5CA2FE92B871CD8F9C36D3292B5509CA8CAA77A2ADFC7BFD77DDA6F71125A7456"
    "FEA153E433256A2261C6A06ED3693797E7995FAD5AABBCFBE3EDA2741E375404AE25B#)"
    "  (q #F2C3119374CE76C9356990B465374A17F23F9ED35089BD969F61C6DDE9998C1F#)"
    "  (g "
    "#5C7FF6B06F8F143FE8288433493E4769C4D988ACE5BE25A0E24809670716C613D7B0CEE69"
    "32F8FAA7C44D2CB24523DA53FBE4F6EC3595892D1AA58C4328A06C46A15662E7EAA703A1DE"
    "CF8BBB2D05DBE2EB956C142A338661D10461C0D135472085057F3494309FFA73C611F78B32"
    "ADBB5740C361C9F35BE90997DB2014E2EF5AA61782F52ABEB8BD6432C4DD097BC5423B285D"
    "AFB60DC364E8161F4A2A35ACA3A10B1C4D203CC76A470A33AFDCBDD92959859ABD8B56E172"
    "5252D78EAC66E71BA9AE3F1DD2487199874393CD4D832186800654760E1E34C09E4D155179"
    "F9EC0DC4473F996BDCE6EED1CABED8B6F116F7AD9CF505DF0F998E34AB27514B0FFE7#)"
    "  (y "
    "#667098C654426C78D7F8201EAC6C203EF030D43605032C2F1FA937E5237DBD949F34A0A25"
    "64FE126DC8B715C5141802CE0979C8246463C40E6B6BDAA2513FA611728716C2E4FD53BC95"
    "B89E69949D96512E873B9C8F8DFD499CC312882561ADECB31F658E934C0C197F2C4D96B05C"
    "BAD67381E7B768891E4DA3843D24D94CDFB5126E9B8BF21E8358EE0E0A30EF13FD6A664C0D"
    "CE3731F7FB49A4845A4FD8254687972A2D382599C9BAC4E0ED799819307891303255813497"
    "6410B89D2C171D123AC35FD977219597AA7D15C1A9A428E59194F75C721EBCBCFAE44696A4"
    "99AFA74E04299F132026601638CB87AB79190D4A0986315DA8EEC6561C938996BEADF#)))";
...
static gpg_err_code_t generate(DSA_secret_key* sk, unsigned int nbits,
                               unsigned int qbits, int transient_key,
                               dsa_domain_t* domain, gcry_mpi_t** ret_factors);
...
void _gcry_register_pk_dsa_progress(void (*cb)(void*, const char*, int, int,
                                               int),
                                    void* cb_data)
{
    progress_cb = cb;
...
static gpg_err_code_t generate_fips186(DSA_secret_key* sk, unsigned int nbits,
                                       unsigned int qbits,
                                       gcry_sexp_t deriveparms,
                                       int use_fips186_2, dsa_domain_t* domain,
                                       int* r_counter, void** r_seed,
                                       size_t* r_seedlen, gcry_mpi_t* r_h)
{
    gpg_err_code_t ec;
    struct
    {
    ...
    else
    {
        ...
        if (deriveparms)
        {
            ...
            if (initial_seed.sexp)
                initial_seed.seed =
                    sexp_nth_data(initial_seed.sexp, 1, &initial_seed.seedlen);
        }

        if (use_fips186_2)
            ec = _gcry_generate_fips186_2_prime(
                nbits, qbits, initial_seed.seed, initial_seed.seedlen, &prime_q,
                &prime_p, r_counter, r_seed, r_seedlen);
        else
            ec = _gcry_generate_fips186_3_prime(nbits, qbits, NULL, 0, &prime_q,
                                                &prime_p, r_counter, r_seed,
                                                r_seedlen, NULL);
...
again:
    ...
    if ((flags & PUBKEY_FLAG_RFC6979) && hashalgo)
    {
        ...
        rc = _gcry_dsa_gen_rfc6979_k(&k, skey->q, skey->x, abuf,
                                     (abits + 7) / 8, hashalgo, extraloops);
        if (rc)
            goto leave;
    }
    else
...
static gcry_err_code_t dsa_generate(const gcry_sexp_t genparms,
                                    gcry_sexp_t* r_skey)
{
    gpg_err_code_t rc;
    unsigned int nbits;
        ...
        (flags & PUBKEY_FLAG_USE_FIPS186_2) || fips_mode())
    {
        ...
        rc = generate_fips186(&sk, nbits, qbits, deriveparms,
                              !!(flags & PUBKEY_FLAG_USE_FIPS186_2), &domain,
        ...
        if (!rc && h_value)
        {
            ...
            rc = sexp_build(&seedinfo, NULL,
                            "(seed-values(counter %d)(seed %b)(h %m))", counter,
                            (int)seedlen, seed, h_value);
            xfree(seed);
    ...
    {
        rc = generate(&sk, nbits, qbits, !!(flags & PUBKEY_FLAG_TRANSIENT_KEY),
                      &domain, &factors);
    }
    ...
    if (!rc)
        rc = sexp_build(r_skey, NULL,
                        ...
                        " %S)",
                        sk.p, sk.q, sk.g, sk.y, sk.p, sk.q, sk.g, sk.y, sk.x,
                        misc_info);
...
static gcry_err_code_t dsa_check_secret_key(gcry_sexp_t keyparms)
{
    gcry_err_code_t rc;
    DSA_secret_key sk = {NULL, NULL, NULL, NULL, NULL};

    rc = _gcry_sexp_extract_param(keyparms, NULL, "pqgyx", &sk.p, &sk.q, &sk.g,
                                  &sk.y, &sk.x, NULL);
    if (rc)
        goto leave;
...
static gcry_err_code_t dsa_sign(gcry_sexp_t* r_sig, gcry_sexp_t s_data,
...
static gcry_err_code_t dsa_verify(gcry_sexp_t s_sig, gcry_sexp_t s_data,
                                  gcry_sexp_t s_keyparms)
{
    ...
    /* Extract the key.  */
    rc = _gcry_sexp_extract_param(s_keyparms, NULL, "pqgy", &pk.p, &pk.q, &pk.g,
                                  &pk.y, NULL);
    if (rc)
        goto leave;
    if (DBG_CIPHER)
...
static gpg_err_code_t selftests_dsa_2048(selftest_report_func_t report)
{
    const char* what;
    const char* errtxt;
    gcry_error_t err;
    ...
    what = "convert";
    err = sexp_sscan(&skey, NULL, sample_secret_key_2048,
                     strlen(sample_secret_key_2048));
    if (!err)
        err = sexp_sscan(&pkey, NULL, sample_public_key_2048,
...
gcry_pk_spec_t _gcry_pubkey_spec_dsa = {GCRY_PK_DSA,
                                        {0, 1},
                                        GCRY_PK_USAGE_SIGN,
                                        "DSA",
                                        dsa_names,
