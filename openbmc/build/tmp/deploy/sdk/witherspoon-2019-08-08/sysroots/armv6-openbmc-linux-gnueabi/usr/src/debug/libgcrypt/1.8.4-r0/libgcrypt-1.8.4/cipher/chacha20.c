...
typedef unsigned int (*chacha20_blocks_t)(u32* state, const byte* src,
                                          byte* dst, size_t bytes) ASM_FUNC_ABI;
...
#ifndef USE_SSE2
ASM_FUNC_ABI static unsigned int chacha20_blocks(u32* state, const byte* src,
                                                 byte* dst, size_t bytes)
{
    ...
    do
    {
        ...
        {
#define DO_OUT(idx)                                                            \
    buf_put_le32(dst + (idx)*4,                                                \
                 (pad[idx] + inp[idx]) ^ buf_get_le32(src + (idx)*4))
            /* Last round. */
            QROUND(pad[0], pad[5], pad[10], pad[15]);
            QOUT(0, 5, 10, 15);
            QROUND(pad[1], pad[6], pad[11], pad[12]);
...
static unsigned int chacha20_core(u32* dst, struct CHACHA20_context_s* ctx)
{
    return ctx->blocks(ctx->input, NULL, (byte*)dst, CHACHA20_BLOCK_SIZE) +
           ASM_EXTRA_STACK;
}

static void chacha20_keysetup(CHACHA20_context_t* ctx, const byte* key,
                              unsigned int keylen)
{
...
static void chacha20_ivsetup(CHACHA20_context_t* ctx, const byte* iv,
                             size_t ivlen)
{
...
static gcry_err_code_t chacha20_do_setkey(CHACHA20_context_t* ctx,
...
static gcry_err_code_t chacha20_setkey(void* context, const byte* key,
                                       unsigned int keylen)
{
...
static void chacha20_do_encrypt_stream(CHACHA20_context_t* ctx, byte* outbuf,
...
static void chacha20_encrypt_stream(void* context, byte* outbuf,
                                    const byte* inbuf, size_t length)
{
    CHACHA20_context_t* ctx = (CHACHA20_context_t*)context;
...
static const char* selftest(void)
{
    ...
    static byte key_1[] = {0xc4, 0x6e, 0xc1, 0xb1, 0x8c, 0xe8, 0xa8, 0x78,
                           0x72, 0x5a, 0x37, 0xe7, 0x80, 0xdf, 0xb7, 0x35,
                           0x1f, 0x68, 0xed, 0x2e, 0x19, 0x4c, 0x79, 0xfb,
                           0xc6, 0xae, 0xbe, 0xe1, 0xa6, 0x67, 0x97, 0x5d};
    static const byte nonce_1[] = {0x1a, 0xda, 0x31, 0xd5,
                                   0xcf, 0x68, 0x82, 0x21};
    ...
    static const byte ciphertext_1[127] = {
        0xf6, 0x3a, 0x89, 0xb7, 0x5c, 0x22, 0x71, 0xf9, 0x36, 0x88, 0x16, 0x54,
        0x2b, 0xa5, 0x2f, 0x06, 0xed, 0x49, 0x24, 0x17, 0x92, 0x30, 0x2b, 0x00,
        0xb5, 0xe8, 0xf8, 0x0a, 0xe9, 0xa4, 0x73, 0xaf, 0xc2, 0x5b, 0x21, 0x8f,
        0x51, 0x9a, 0xf0, 0xfd, 0xd4, 0x06, 0x36, 0x2e, 0x8d, 0x69, 0xde, 0x7f,
        0x54, 0xc6, 0x04, 0xa6, 0xe0, 0x0f, 0x35, 0x3f, 0x11, 0x0f, 0x77, 0x1b,
        0xdc, 0xa8, 0xab, 0x92, 0xe5, 0xfb, 0xc3, 0x4e, 0x60, 0xa1, 0xd9, 0xa9,
        0xdb, 0x17, 0x34, 0x5b, 0x0a, 0x40, 0x27, 0x36, 0x85, 0x3b, 0xf9, 0x10,
        0xb0, 0x60, 0xbd, 0xf1, 0xf8, 0x97, 0xb6, 0x29, 0x0f, 0x01, 0xd1, 0x38,
        0xae, 0x2c, 0x4c, 0x90, 0x22, 0x5b, 0xa9, 0xea, 0x14, 0xd5, 0x18, 0xf5,
        0x59, 0x29, 0xde, 0xa0, 0x98, 0xca, 0x7a, 0x6c, 0xcf, 0xe6, 0x12, 0x27,
    ...
    chacha20_encrypt_stream(ctx, buf + (sizeof buf) - 1, buf + (sizeof buf) - 1,
                            1);
    for (i = 0; i < sizeof buf; i++)
        if (buf[i] != (byte)i)
