...
struct gzip_params : zlib_params
{
    ...
    gzip_params(int level = gzip::default_compression,
                ...
                std::string file_name_ = "", std::string comment_ = "",
                std::time_t mtime_ = 0) :
...
class gzip_error : public BOOST_IOSTREAMS_FAILURE
{
  public:
    explicit gzip_error(int error) :
        BOOST_IOSTREAMS_FAILURE("gzip error"), error_(error),
        zlib_error_code_(zlib::okay)
    {
    }
    explicit gzip_error(const zlib_error& e) :
        BOOST_IOSTREAMS_FAILURE("gzip error"), error_(gzip::zlib_error),
        zlib_error_code_(e.error())
    {
    }
    int error() const
    {
  ...
  public:
    ...
    std::streamsize write(Sink& snk, const char_type* s, std::streamsize n)
    {
        if (!(flags_ & f_header_done))
        {
            ...
            offset_ +=
                boost::iostreams::write_if(snk, header_.data() + offset_, amt);
...
class basic_gzip_decompressor : basic_zlib_decompressor<Alloc>
{
  ...
  public:
    ...
    std::streamsize read(Source& src, char_type* s, std::streamsize n)
    {
        ...
        return result != 0 || state_ != s_done ? result : -1;
    }
  ...
  private:
    ...
    struct peekable_source
    {
        ...
        // Returns the sequence of characters that have been put back but not
        // re-read.
    ...
    offset_(0), flags_(0)
{
    ...
    int extra_flags =
        (p.level == zlib::best_compression ? gzip::extra_flags::best_compression
                                           : 0) +
        (p.level == zlib::best_speed ? gzip::extra_flags::best_speed : 0);
...
template <typename Alloc>
std::streamsize basic_gzip_compressor<Alloc>::read_string(char* s,
                                                          std::streamsize n,
                                                          std::string& str)
{
    std::streamsize avail = static_cast<std::streamsize>(str.size() - offset_);
    std::streamsize amt = (std::min)(avail, n);
    std::copy(str.data() + offset_, str.data() + offset_ + amt, s);
    offset_ += amt;
    if (!(flags_ & f_header_done) &&
