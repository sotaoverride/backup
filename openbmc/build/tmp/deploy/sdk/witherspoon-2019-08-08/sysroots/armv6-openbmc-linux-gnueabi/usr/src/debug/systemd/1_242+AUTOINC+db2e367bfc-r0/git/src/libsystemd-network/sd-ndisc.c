...
#define NDISC_TIMEOUT_NO_RA_USEC                                               \
    (NDISC_ROUTER_SOLICITATION_INTERVAL * NDISC_MAX_ROUTER_SOLICITATIONS)
...
static void ndisc_callback(sd_ndisc* ndisc, sd_ndisc_event event,
                           sd_ndisc_router* rt)
{
    assert(ndisc);
    ...
    log_ndisc("Invoking callback for '%s' event.",
              ndisc_event_to_string(event));
    ndisc->callback(ndisc, event, rt, ndisc->userdata);
}

_public_ int sd_ndisc_set_callback(sd_ndisc* nd, sd_ndisc_callback_t callback,
                                   void* userdata)
{
...
_public_ int sd_ndisc_attach_event(sd_ndisc* nd, sd_event* event,
                                   int64_t priority)
{
    int r;
...
static int ndisc_handle_datagram(sd_ndisc* nd, sd_ndisc_router* rt)
{
    ...
    log_ndisc("Received Router Advertisement: flags %s preference %s lifetime "
              "%" PRIu16 " sec",
              rt->flags & ND_RA_FLAG_MANAGED
                  ? "MANAGED"
                  : rt->flags & ND_RA_FLAG_OTHER ? "OTHER" : "none",
              rt->preference == SD_NDISC_PREFERENCE_HIGH
                  ? "high"
                  : rt->preference == SD_NDISC_PREFERENCE_LOW ? "low"
                                                              : "medium",
              rt->lifetime);
...
static int ndisc_recv(sd_event_source* s, int fd, uint32_t revents,
                      void* userdata)
{
        ...
        switch (r)
        {
            case -EADDRNOTAVAIL:
                (void)in_addr_to_string(
                    AF_INET6, (union in_addr_union*)&rt->address, &addr);
                log_ndisc(
                    "Received RA from non-link-local address %s. Ignoring",
                    addr);
                break;
...
static int ndisc_timeout(sd_event_source* s, uint64_t usec, void* userdata)
{
    ...
    assert_se(
        sd_event_now(nd->event, clock_boottime_or_monotonic(), &time_now) >= 0);
    ...
    else
    {
        if (nd->retransmit_time > NDISC_MAX_ROUTER_SOLICITATION_INTERVAL / 2)
            ...
            nd->retransmit_time +=
                ndisc_timeout_compute_random(nd->retransmit_time);
    }

    r = event_reset_time(
        nd->event, &nd->timeout_event_source, clock_boottime_or_monotonic(),
        time_now + nd->retransmit_time, 10 * USEC_PER_MSEC, ndisc_timeout, nd,
...
static int ndisc_timeout_no_ra(sd_event_source* s, uint64_t usec,
                               void* userdata)
{
    sd_ndisc* nd = userdata;
...
_public_ int sd_ndisc_start(sd_ndisc* nd)
{
    ...
    r = sd_event_add_io(nd->event, &nd->recv_event_source, nd->fd, EPOLLIN,
                        ndisc_recv, nd);
    if (r < 0)
        goto fail;
    ...
    (void)sd_event_source_set_description(nd->recv_event_source,
                                          "ndisc-receive-message");

    r = event_reset_time(nd->event, &nd->timeout_event_source,
                         clock_boottime_or_monotonic(), 0, 0, ndisc_timeout, nd,
