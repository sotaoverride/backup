 ...
 * #GApplication attempts to acquire the bus name of your application (which
 * happens in g_application_register()).  Unfortunately, this means that you
 * cannot use g_application_get_is_remote() to decide if you want to register
 * object paths.
 *
 ...
 * @name_lost: invoked when another instance is taking over the name.
 * Since: 2.60
 *
...
G_DEFINE_TYPE_WITH_CODE(
    GApplication, g_application, G_TYPE_OBJECT,
...
static void g_application_exported_actions_iface_init(
    GRemoteActionGroupInterface* iface);
G_DEFINE_TYPE_WITH_CODE(
    GApplicationExportedActions, g_application_exported_actions,
    G_TYPE_SIMPLE_ACTION_GROUP,
    G_IMPLEMENT_INTERFACE(G_TYPE_REMOTE_ACTION_GROUP,
                          g_application_exported_actions_iface_init))

static void g_application_exported_actions_activate_action_full(
    GRemoteActionGroup* remote, const gchar* action_name, GVariant* parameter,
    GVariant* platform_data)
{
    GApplicationExportedActions* exported =
        (GApplicationExportedActions*)remote;

    G_APPLICATION_GET_CLASS(exported->application)
        ->before_emit(exported->application, platform_data);

    g_action_group_activate_action(G_ACTION_GROUP(exported), action_name,
                                   parameter);
...
static void g_application_exported_actions_change_action_state_full(
    ...
    GVariant* platform_data)
{
    ...
    g_action_group_change_action_state(G_ACTION_GROUP(exported), action_name,
                                       value);
...
static void g_application_exported_actions_iface_init(
    GRemoteActionGroupInterface* iface)
{
    iface->activate_action_full =
        g_application_exported_actions_activate_action_full;
    iface->change_action_state_full =
        g_application_exported_actions_change_action_state_full;
}

static void g_application_exported_actions_class_init(
    GApplicationExportedActionsClass* class)
{
}
...
static void free_option_entry(gpointer data)
{
...
static void g_application_pack_option_entries(GApplication* application,
                                              GVariantDict* dict)
{
    GHashTableIter iter;
    gpointer item;
...
static GVariantDict* g_application_parse_command_line(GApplication* application,
                                                      gchar*** arguments,
                                                      GError** error)
{
    ...
    gapplication_group =
        g_option_group_new("gapplication", _("GApplication options"),
                           _("Show GApplication options"), NULL, NULL);
     ...
     */
    if (application->priv->main_options == NULL &&
        (application->priv->flags & G_APPLICATION_HANDLES_COMMAND_LINE))
    {
    ...
    while (application->priv->option_groups)
    {
        ...
        application->priv->option_groups = g_slist_delete_link(
            application->priv->option_groups, application->priv->option_groups);
    }
     ...
     */
    if ((application->priv->flags &
         (G_APPLICATION_IS_SERVICE | G_APPLICATION_IS_LAUNCHER)) == 0)
    {
        GOptionEntry entries[] = {{"gapplication-service", '\0', 0,
                                   G_OPTION_ARG_NONE, &become_service,
                                   N_("Enter GApplication service mode (use "
    ...
    {
        GOptionEntry entries[] = {{"gapplication-app-id", '\0', 0,
                                   G_OPTION_ARG_STRING, &app_id,
    ...
    {
        GOptionEntry entries[] = {{"gapplication-replace", '\0', 0,
...
static void add_packed_option(GApplication* application, GOptionEntry* entry)
{
    switch (entry->arg)
    {
        case G_OPTION_ARG_NONE:
    ...
    if (!application->priv->packed_options)
        application->priv->packed_options = g_hash_table_new_full(
            g_str_hash, g_str_equal, g_free, free_option_entry);
                                           ...
                                           const GOptionEntry* entries)
{
    ...
    {
        application->priv->main_options =
            g_option_group_new(NULL, NULL, NULL, NULL, NULL);
        g_option_group_set_translation_domain(application->priv->main_options,
                                              NULL);
    }
...
void g_application_add_main_option(GApplication* application,
                                   ...
                                   GOptionFlags flags, GOptionArg arg,
                                   const char* description,
                                   const char* arg_description)
{
    gchar* dup_string;
    GOptionEntry my_entry[2] = {
        {NULL, short_name, flags, arg, NULL, NULL, NULL}, {NULL}};
    ...
    application->priv->option_strings =
        g_slist_prepend(application->priv->option_strings, dup_string);
                                    ...
                                    GOptionGroup* group)
{
    ...
    application->priv->option_groups =
        g_slist_prepend(application->priv->option_groups, group);
}
 ...
 *   in the first line of `--help` output, after the usage summary `programname
 * [OPTION...]`.
 *
...
/* vfunc defaults {{{1 */
static void g_application_real_before_emit(GApplication* application,
                                           GVariant* platform_data)
{
}

static void g_application_real_after_emit(GApplication* application,
                                          GVariant* platform_data)
{
}
...
static void g_application_real_shutdown(GApplication* application)
{
...
static void g_application_real_activate(GApplication* application)
{
    if (!g_signal_has_handler_pending(
            application, g_application_signals[SIGNAL_ACTIVATE], 0, TRUE) &&
        G_APPLICATION_GET_CLASS(application)->activate ==
...
static void g_application_real_open(GApplication* application, GFile** files,
                                    gint n_files, const gchar* hint)
{
    if (!g_signal_has_handler_pending(
            application, g_application_signals[SIGNAL_OPEN], 0, TRUE) &&
...
static int g_application_real_command_line(GApplication* application,
                                           GApplicationCommandLine* cmdline)
{
    if (!g_signal_has_handler_pending(
            application, g_application_signals[SIGNAL_COMMAND_LINE], 0, TRUE) &&
        G_APPLICATION_GET_CLASS(application)->command_line ==
            g_application_real_command_line)
    {
        ...
        g_warning(
            "Your application claims to support custom command line "
...
static gint g_application_real_handle_local_options(GApplication* application,
                                                    GVariantDict* options)
{
    return -1;
}

static GVariant* get_platform_data(GApplication* application, GVariant* options)
{
...
static void g_application_call_command_line(GApplication* application,
                                            ...
                                            gint* exit_status)
{
    if (application->priv->is_remote)
    {
        ...
        *exit_status = g_application_impl_command_line(
            application->priv->impl, arguments, platform_data);
    }
    else
    {
        GApplicationCommandLine* cmdline;
        ...
        cmdline = g_object_new(G_TYPE_APPLICATION_COMMAND_LINE, "arguments", v,
                               "options", options, NULL);
        g_signal_emit(application, g_application_signals[SIGNAL_COMMAND_LINE],
                      0, cmdline, exit_status);
        g_object_unref(cmdline);
    }
}

static gboolean g_application_real_local_command_line(GApplication* application,
                                                      gchar*** arguments,
                                                      int* exit_status)
{
    GError* error = NULL;
    ...
    g_signal_emit(application,
                  g_application_signals[SIGNAL_HANDLE_LOCAL_OPTIONS], 0,
                  options, exit_status);
    ...
    {
        g_application_call_command_line(application, (const gchar**)*arguments,
    ...
    else
    {
        ...
        else
        {
            ...
            else
            {
                ...
                for (i = 0; i < n_files; i++)
                    files[i] =
                        g_file_new_for_commandline_arg((*arguments)[i + 1]);
...
static void g_application_real_add_platform_data(GApplication* application,
                                                 GVariantBuilder* builder)
{
}

static gboolean g_application_real_dbus_register(GApplication* application,
...
static void g_application_real_dbus_unregister(GApplication* application,
                                               GDBusConnection* connection,
                                               const gchar* object_path)
{
}

static gboolean g_application_real_name_lost(GApplication* application)
{
...
static void g_application_set_property(GObject* object, guint prop_id,
...
static void g_application_get_property(GObject* object, guint prop_id,
                                       GValue* value, GParamSpec* pspec)
{
    ...
    switch (prop_id)
    {
        ...
        case PROP_FLAGS:
            g_value_set_flags(value, g_application_get_flags(application));
            break;

        case PROP_RESOURCE_BASE_PATH:
            g_value_set_string(
                value, g_application_get_resource_base_path(application));
            break;
...
static void g_application_constructed(GObject* object)
{
    ...
    if (application->priv->id != NULL)
    {
        ...
        application->priv->resource_path =
            g_strconcat("/", application->priv->id, NULL);
...
static void g_application_dispose(GObject* object)
{
    GApplication* application = G_APPLICATION(object);

    if (application->priv->impl != NULL &&
        G_APPLICATION_GET_CLASS(application)->dbus_unregister !=
            g_application_real_dbus_unregister)
    {
        ...
        if (!warned)
        {
            g_warning("Your application did not unregister from D-Bus before "
                      "destruction. "
...
static void g_application_finalize(GObject* object)
{
    ...
    g_slist_free_full(application->priv->option_groups,
                      (GDestroyNotify)g_option_group_unref);
    if (application->priv->main_options)
...
static void g_application_init(GApplication* application)
{
    ...
    g_signal_connect_swapped(
        application->priv->actions, "action-enabled-changed",
...
static gboolean g_application_handle_local_options_accumulator(
...
static void g_application_class_init(GApplicationClass* class)
{
    ...
    g_object_class_install_property(
        object_class, PROP_APPLICATION_ID,
        g_param_spec_string(
            "application-id", P_("Application identifier"),
            P_("The unique identifier for the application"), NULL,
            G_PARAM_READWRITE | G_PARAM_CONSTRUCT | G_PARAM_STATIC_STRINGS));

    g_object_class_install_property(
        object_class, PROP_FLAGS,
        g_param_spec_flags(
            "flags", P_("Application flags"),
            P_("Flags specifying the behaviour of the application"),
            G_TYPE_APPLICATION_FLAGS, G_APPLICATION_FLAGS_NONE,
            G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS));

    g_object_class_install_property(
        object_class, PROP_RESOURCE_BASE_PATH,
        g_param_spec_string("resource-base-path", P_("Resource base path"),
                            P_("The base resource path for the application"),
                            NULL, G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS));

    g_object_class_install_property(
        object_class, PROP_IS_REGISTERED,
        g_param_spec_boolean("is-registered", P_("Is registered"),
                             P_("If g_application_register() has been called"),
                             FALSE, G_PARAM_READABLE | G_PARAM_STATIC_STRINGS));

    g_object_class_install_property(
        object_class, PROP_IS_REMOTE,
        g_param_spec_boolean("is-remote", P_("Is remote"),
                             P_("If this application instance is remote"),
                             FALSE, G_PARAM_READABLE | G_PARAM_STATIC_STRINGS));

    g_object_class_install_property(
        object_class, PROP_INACTIVITY_TIMEOUT,
        g_param_spec_uint("inactivity-timeout", P_("Inactivity timeout"),
                          P_("Time (ms) to stay alive after becoming idle"), 0,
                          G_MAXUINT, 0,
                          G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS));

    g_object_class_install_property(
        object_class, PROP_ACTION_GROUP,
        g_param_spec_object(
            "action-group", P_("Action group"),
     ...
     */
    g_object_class_install_property(
        object_class, PROP_IS_BUSY,
        g_param_spec_boolean("is-busy", P_("Is busy"),
    ...
    g_application_signals[SIGNAL_STARTUP] =
        g_signal_new(I_("startup"), G_TYPE_APPLICATION, G_SIGNAL_RUN_FIRST,
                     G_STRUCT_OFFSET(GApplicationClass, startup), NULL, NULL,
    ...
    g_application_signals[SIGNAL_SHUTDOWN] =
        g_signal_new(I_("shutdown"), G_TYPE_APPLICATION, G_SIGNAL_RUN_LAST,
                     G_STRUCT_OFFSET(GApplicationClass, shutdown), NULL, NULL,
                     g_cclosure_marshal_VOID__VOID, G_TYPE_NONE, 0);
    ...
    g_application_signals[SIGNAL_ACTIVATE] =
        g_signal_new(I_("activate"), G_TYPE_APPLICATION, G_SIGNAL_RUN_LAST,
                     G_STRUCT_OFFSET(GApplicationClass, activate), NULL, NULL,
                     g_cclosure_marshal_VOID__VOID, G_TYPE_NONE, 0);
    ...
    g_application_signals[SIGNAL_OPEN] =
        g_signal_new(I_("open"), G_TYPE_APPLICATION, G_SIGNAL_RUN_LAST,
                     G_STRUCT_OFFSET(GApplicationClass, open), NULL, NULL, NULL,
    ...
    g_application_signals[SIGNAL_COMMAND_LINE] =
        g_signal_new(I_("command-line"), G_TYPE_APPLICATION, G_SIGNAL_RUN_LAST,
                     ...
                     g_signal_accumulator_first_wins, NULL, NULL, G_TYPE_INT, 1,
     ...
     * Since: 2.40
     **/
    g_application_signals[SIGNAL_HANDLE_LOCAL_OPTIONS] = g_signal_new(
        I_("handle-local-options"), G_TYPE_APPLICATION, G_SIGNAL_RUN_LAST,
        G_STRUCT_OFFSET(GApplicationClass, handle_local_options),
        g_application_handle_local_options_accumulator, NULL, NULL, G_TYPE_INT,
     ...
     * Since: 2.60
     */
    g_application_signals[SIGNAL_NAME_LOST] = g_signal_new(
        I_("name-lost"), G_TYPE_APPLICATION, G_SIGNAL_RUN_LAST,
                                ...
                                GApplicationFlags flags)
{
    ...
    return g_object_new(G_TYPE_APPLICATION, "application-id", application_id,
                        "flags", flags, NULL);
}
                                ...
                                GCancellable* cancellable, GError** error)
{
    ...
    if (!application->priv->is_registered)
    {
        ...
        application->priv->impl = g_application_impl_register(
            application, application->priv->id, application->priv->flags,
            application->priv->actions, &application->priv->remote_actions,
            cancellable, error);
        ...
        application->priv->is_remote =
            application->priv->remote_actions != NULL;
        ...
        if (!application->priv->is_remote)
        {
            g_signal_emit(application, g_application_signals[SIGNAL_STARTUP],
                          0);
                        ...
                        const gchar* hint)
{
    ...
    if (application->priv->is_remote)
        g_application_impl_open(application->priv->impl, files, n_files, hint,
                                get_platform_data(application, NULL));

    else
        g_signal_emit(application, g_application_signals[SIGNAL_OPEN], 0, files,
                      n_files, hint);
}
...
int g_application_run(GApplication* application, int argc, char** argv)
{
...
#ifdef G_OS_WIN32
    {
         ...
         * pulls in the whole command line that is used to call the program.
         * This is fine in cases where the program is a .exe program, but in the
         * cases where the program is a called via a script, such as PyGObject's
         * gtk-demo.py, which is normally called using 'python gtk-demo.py' on
         * Windows, the program name (argv[0]) returned by
         * g_win32_get_command_line() will not be the argv[0] that
         * ->local_command_line() would expect, causing the program to fail with
        ...
        if (new_argc > argc)
        {
            ...
            memmove(&arguments[0], &arguments[new_argc - argc],
    ...
    if (application->priv->flags & G_APPLICATION_IS_SERVICE &&
        application->priv->is_registered && !application->priv->use_count &&
        !application->priv->inactivity_timeout_id)
    {
        application->priv->inactivity_timeout_id =
    ...
    while (application->priv->use_count ||
           application->priv->inactivity_timeout_id)
    {
...
static gchar** g_application_list_actions(GActionGroup* action_group)
{
    ...
    if (application->priv->remote_actions != NULL)
        return g_action_group_list_actions(
            G_ACTION_GROUP(application->priv->remote_actions));
...
static gboolean g_application_query_action(
    GActionGroup* group, const gchar* action_name, gboolean* enabled,
    const GVariantType** parameter_type, const GVariantType** state_type,
    GVariant** state_hint, GVariant** state)
{
    ...
    if (application->priv->remote_actions != NULL)
        return g_action_group_query_action(
            G_ACTION_GROUP(application->priv->remote_actions), action_name,
            enabled, parameter_type, state_type, state_hint, state);

    if (application->priv->actions != NULL)
        return g_action_group_query_action(application->priv->actions,
                                           action_name, enabled, parameter_type,
                                           state_type, state_hint, state);

    return FALSE;
}

static void g_application_change_action_state(GActionGroup* action_group,
                                              ...
                                              GVariant* value)
{
    ...
    if (application->priv->remote_actions)
        g_remote_action_group_change_action_state_full(
            application->priv->remote_actions, action_name, value,
            get_platform_data(application, NULL));
...
static void g_application_activate_action(GActionGroup* action_group,
                                          const gchar* action_name,
                                          GVariant* parameter)
{
    GApplication* application = G_APPLICATION(action_group);
    ...
    if (application->priv->remote_actions)
        ...
        g_action_group_activate_action(application->priv->actions, action_name,
                                       parameter);
}

static GAction* g_application_lookup_action(GActionMap* action_map,
                                            const gchar* action_name)
{
    ...
    return g_action_map_lookup_action(G_ACTION_MAP(application->priv->actions),
                                      action_name);
}

static void g_application_add_action(GActionMap* action_map, GAction* action)
{
    GApplication* application = G_APPLICATION(action_map);
...
static void g_application_remove_action(GActionMap* action_map,
                                        const gchar* action_name)
{
    GApplication* application = G_APPLICATION(action_map);
    ...
    g_action_map_remove_action(G_ACTION_MAP(application->priv->actions),
                               action_name);
}
 ...
 * Since: 2.40
 */
void g_application_send_notification(GApplication* application, const gchar* id,
                                     GNotification* notification)
{
    ...
    if (application->priv->notifications == NULL)
        application->priv->notifications =
    ...
    g_notification_backend_send_notification(application->priv->notifications,
                                             id, notification);
                                         ...
                                         const gchar* id)
{
    ...
    g_notification_backend_withdraw_notification(
        application->priv->notifications, id);
}
...
static void g_application_busy_binding_destroy(gpointer data, GClosure* closure)
{
...
static void g_application_notify_busy_binding(GObject* object,
                                              GParamSpec* pspec,
                                              gpointer user_data)
{
                                      ...
                                      gpointer object, const gchar* property)
{
    ...
    if (g_signal_handler_find(object,
                              G_SIGNAL_MATCH_ID | G_SIGNAL_MATCH_DETAIL |
                                  G_SIGNAL_MATCH_FUNC,
                              notify_id, property_quark, NULL,
                              g_application_notify_busy_binding, NULL) > 0)
    {
        g_critical(
            "%s: '%s' is already bound to the busy state of the application",
            G_STRFUNC, property);
        return;
    }
    ...
    closure = g_cclosure_new(G_CALLBACK(g_application_notify_busy_binding),
                             binding, g_application_busy_binding_destroy);
    g_signal_connect_closure_by_id(object, notify_id, property_quark, closure,
                                   FALSE);
                                        ...
                                        gpointer object, const gchar* property)
{
    ...
    handler_id = g_signal_handler_find(
        object, G_SIGNAL_MATCH_ID | G_SIGNAL_MATCH_DETAIL | G_SIGNAL_MATCH_FUNC,
        notify_id, property_quark, NULL, g_application_notify_busy_binding,
        NULL);
    if (handler_id == 0)
    {
        g_critical("%s: '%s' is not bound to the busy state of the application",
                   G_STRFUNC, property);
        return;
    }
