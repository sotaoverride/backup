...
BOOST_CONTAINER_DECL int dlmalloc_multialloc_nodes(size_t n_elements,
                                                   size_t elem_size,
                                                   size_t contiguous_elements,
                                                   boost_cont_memchain* pchain)
{
    return boost_cont_multialloc_nodes(n_elements, elem_size,
                                       contiguous_elements, pchain);
}

BOOST_CONTAINER_DECL int dlmalloc_multialloc_arrays(size_t n_elements,
                                                    const size_t* sizes,
                                                    size_t sizeof_element,
                                                    size_t contiguous_elements,
                                                    boost_cont_memchain* pchain)
{
    return boost_cont_multialloc_arrays(n_elements, sizes, sizeof_element,
                                        contiguous_elements, pchain);
}
...
BOOST_CONTAINER_DECL int dlmalloc_mallopt(int parameter_number,
                                          int parameter_value)
{
    return boost_cont_mallopt(parameter_number, parameter_value);
}

BOOST_CONTAINER_DECL int dlmalloc_grow(void* oldmem, size_t minbytes,
                                       size_t maxbytes, size_t* received)
{
    return boost_cont_grow(oldmem, minbytes, maxbytes, received);
}

BOOST_CONTAINER_DECL int dlmalloc_shrink(void* oldmem, size_t minbytes,
                                         size_t maxbytes, size_t* received,
                                         int do_commit)
{
...
BOOST_CONTAINER_DECL void* dlmalloc_alloc(size_t minbytes,
...
{
    return boost_cont_allocation_command(command, sizeof_object, limit_objects,
                                         preferred_objects, received_objects,
                                         reuse_ptr);
}
