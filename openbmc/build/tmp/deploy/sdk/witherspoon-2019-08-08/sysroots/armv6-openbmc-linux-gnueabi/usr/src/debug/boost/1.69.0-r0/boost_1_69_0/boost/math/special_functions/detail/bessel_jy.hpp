...
int temme_jy(T v, T x, T* Y, T* Y1, const Policy& pol)
{
    ...
    a = log(x / 2);
    sigma = -a * v;
    d = abs(sigma) < tools::epsilon<T>() ? T(1) : sinh(sigma) / sigma;
    e = abs(v) < tools::epsilon<T>() ? T(v * pi<T>() * pi<T>() / 2)
                                     : T(2 * spv2 * spv2 / v);

    T g1 = (v == 0) ? T(-euler<T>())
                    : T((gp - gm) / ((1 + gp) * (1 + gm) * 2 * v));
    T g2 = (2 + gp + gm) / ((1 + gp) * (1 + gm) * 2);
    T vspv = (fabs(v) < tools::epsilon<T>()) ? T(1 / constants::pi<T>())
                                             : T(v / spv);
    ...
    policies::check_series_iterations<T>(
        "boost::math::bessel_jy<%1%>(%1%,%1%) in temme_jy", k, pol);
    *Y = -sum;
    *Y1 = -2 * sum1 / x;
...
int CF1_jy(T v, T x, T* fv, int* sign, const Policy& pol)
{
    ...
    policies::check_series_iterations<T>(
        "boost::math::bessel_jy<%1%>(%1%,%1%) in CF1_jy", k / 100, pol);
    *fv = -f;
...
int CF2_jy(T v, T x, T* p, T* q, const Policy& pol)
{
    ...
    Dr = br;
    Di = bi;
    if (fabs(Cr) + fabs(Ci) < tiny)
    {
        Cr = tiny;
    }
    if (fabs(Dr) + fabs(Di) < tiny)
    {
        Dr = tiny;
    }
    temp = Dr * Dr + Di * Di;
    ...
    policies::check_series_iterations<T>(
        "boost::math::bessel_jy<%1%>(%1%,%1%) in CF2_jy", k, pol);
    *p = fr;
    *q = fi;
...
int bessel_jy(T v, T x, T* J, T* Y, int kind, const Policy& pol)
{
    ...
    {
        *J = *Y = policies::raise_evaluation_error<T>(
            function,
            "Order of Bessel function is too large to evaluate: got %1%", v,
            pol);
        return 1;
    }
    n = iround(v, pol);
    ...
    if (x == 0)
    {
        if (v == 0)
            ...
            *J = policies::raise_domain_error<T>(
                function, "Value of Bessel J_v(x) is complex-infinity at %1%",
                x, pol); // complex infinity
        else
            *J = std::numeric_limits<T>::quiet_NaN(); // any value will do, not
                                                      // using J.

        if ((kind & need_y) == 0)
            *Y = std::numeric_limits<T>::quiet_NaN(); // any value will do, not
                                                      // using Y.
        else if (v == 0)
            *Y = -policies::raise_overflow_error<T>(function, 0, pol);
        else
            *Y = policies::raise_domain_error<T>(
                function, "Value of Bessel Y_v(x) is complex-infinity at %1%",
                x, pol); // complex infinity
        return 1;
    }
    ...
    if (((kind & need_y) == 0) && ((x < 1) || (v > x * x / 4) || (x < 5)))
    {
        ...
        Yv = std::numeric_limits<T>::quiet_NaN();
             ...
             (log(policies::get_epsilon<T, Policy>() / 2) >
              v * log((x / 2) * (x / 2) / v)))
    {
        ...
        {
            // Only calculate if we need it, and if the reflection formula will
            // actually use it:
    ...
    else if (asymptotic_bessel_large_x_limit(v, x))
    {
        if (kind & need_y)
        {
            ...
            Yv = std::numeric_limits<T>::quiet_NaN(); // any value will do,
                                                      // we're not using it.
        if (kind & need_j)
        {
            ...
            Jv = std::numeric_limits<T>::quiet_NaN(); // any value will do,
                                                      // we're not using it.
    }
    else if ((x > 8) && hankel_PQ(v, x, &p, &q, pol))
    {
        ...
        // of large values, with horrendous resulting accuracy.  It is fast
        // though when it works....
        //
        // Normally we calculate sin/cos(chi) where:
        //
        // chi = x - fmod(T(v / 2 + 0.25f), T(2)) *
        ...
        T chi = boost::math::constants::root_two<T>() /
                (boost::math::constants::root_pi<T>() *
                 sqrt(x)); // sqrt(2 / (boost::math::constants::pi<T>() * x));
        Yv = chi * (p * sc + q * cc);
        Jv = chi * (p * cc - q * sc);
    }
    ...
    else // x in (2, \infty)
    {
        ...
        if (v < max_factorial<T>::value)
        {
            ...
            // can also call CF1_jy() to get fu, not much difference in
            // precision
            fu = prev / current;
        }
        else
        ...
        // and set to a suitably small value.  The only test case we've been
        // able to find for this, is when v = 8.5 and x = 4*PI.
        //
        if (gamma == 0)
        {
    ...
    {
        if ((sp != 0) &&
            (tools::max_value<T>() * fabs(Yv_scale) < fabs(sp * Yv)))
            *J = org_kind & need_j
                     ? T(-sign(sp) * sign(Yv) * sign(Yv_scale) *
                         policies::raise_overflow_error<T>(function, 0, pol))
                     : T(0);
        else
            *J = cp * Jv -
                 (sp == 0 ? T(0)
                          : T((sp * Yv) / Yv_scale)); // reflection formula
        if ((cp != 0) &&
            (tools::max_value<T>() * fabs(Yv_scale) < fabs(cp * Yv)))
            *Y = org_kind & need_y
                     ? T(-sign(cp) * sign(Yv) * sign(Yv_scale) *
                         policies::raise_overflow_error<T>(function, 0, pol))
                     : T(0);
        else
            *Y = (sp != 0 ? sp * Jv : T(0)) +
                 (cp == 0 ? T(0) : T((cp * Yv) / Yv_scale));
    }
    else
    {
        *J = Jv;
