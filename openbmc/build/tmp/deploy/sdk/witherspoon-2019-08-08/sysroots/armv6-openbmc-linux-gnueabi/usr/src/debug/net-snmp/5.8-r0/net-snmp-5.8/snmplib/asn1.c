...
#define CHECK_OVERFLOW_U(x, y)                                                 \
    ...
    {                                                                          \
        ...
        {                                                                      \
            x &= 0xffffffff;                                                   \
            DEBUGMSG(                                                          \
                ("asn", "truncating unsigned value to 32 bits (%d)\n", y));    \
        }                                                                      \
    } while (0)
#endif
 ...
 */
static void _asn_size_err(const char* str, size_t wrongsize, size_t rightsize)
{
    char ebuf[128];

    snprintf(ebuf, sizeof(ebuf), "%s size %lu: s/b %lu", str,
 ...
 */
static void _asn_type_err(const char* str, int wrongtype)
{
    char ebuf[128];
 ...
 */
static void _asn_length_err(const char* str, size_t wrongsize, size_t rightsize)
{
    char ebuf[128];

    snprintf(ebuf, sizeof(ebuf), "%s length %lu too large: exceeds %lu", str,
             (unsigned long)wrongsize, (unsigned long)rightsize);
    ebuf[sizeof(ebuf) - 1] = 0;
    ERROR_MSG(ebuf);
}
 ...
 * @param rightsize  expected length
 */
static void _asn_short_err(const char* str, size_t wrongsize, size_t rightsize)
{
    char ebuf[128];
 ...
 *
 */
u_char* asn_parse_nlength(u_char* pkt, size_t pkt_len, u_long* data_len)
{
    int len_len;
 ...
 */
static int _asn_build_header_check(const char* str, const u_char* data,
 ...
 * @return 0 on success 1 on error
 */
static int _asn_realloc_build_header_check(const char* str, u_char** pkt,
...
static int _asn_bitstring_check(const char* str, size_t asn_length,
                                u_char datum)
{
    char ebuf[128];
    ...
    if (asn_length < 1)
    {
        snprintf(ebuf, sizeof(ebuf), "%s: length %d too small", str,
                 (int)asn_length);
        ebuf[sizeof(ebuf) - 1] = 0;
        ERROR_MSG(ebuf);
        return 1;
 ...
 *   of this object (i.e. the start of the next object) Returns NULL on any
 * error
 */
u_char* asn_parse_int(u_char* data, size_t* datalength, u_char* type,
 ...
 * error
 */
u_char* asn_parse_unsigned_int(u_char* data, size_t* datalength, u_char* type,
 ...
 *          Returns NULL on any error.
 */
u_char* asn_build_int(u_char* data, size_t* datalength, u_char type,
                      const long* intp, size_t intsize)
{
    /*
     * ASN.1 integer ::= 0x02 asnlength byte {byte}*
     */
 ...
 *          Returns NULL on any error.
 */
u_char* asn_build_unsigned_int(u_char* data, size_t* datalength, u_char type,
                               const u_long* intp, size_t intsize)
{
    /*
     * ASN.1 integer ::= 0x02 asnlength byte {byte}*
     */
...
u_char* asn_parse_string(u_char* data, size_t* datalength, u_char* type,
                         u_char* str, size_t* strlength)
{
    ...
    *datalength -= asn_length + (bufp - data);
    ...
    DEBUGIF("dumpv_recv")
    {
        ...
        if (sprint_realloc_asciistring(&buf, &l, &ol, 1, str, asn_length))
        {
...
u_char* asn_build_string(u_char* data, size_t* datalength, u_char type,
                         const u_char* str, size_t strlength)
{
    ...
    *datalength -= strlength;
    ...
    DEBUGIF("dumpv_send")
    {
        ...
        if (sprint_realloc_asciistring(&buf, &l, &ol, 1, str, strlength))
        {
 ...
 *
 */
u_char* asn_parse_sequence(u_char* data, size_t* datalength, u_char* type,
...
{ /* error message prefix */
    ...
    {
        char ebuf[128];
        snprintf(ebuf, sizeof(ebuf), "%s header type %02X: s/b %02X", estr,
 ...
 *          Returns NULL on any error.
 */
u_char* asn_build_header(u_char* data, size_t* datalength, u_char type,
                         size_t length)
{
    char ebuf[128];
    ...
    if (*datalength < 1)
    {
        snprintf(ebuf, sizeof(ebuf), "bad header length < 1 :%lu, %lu",
 ...
 *         Returns NULL on any error.
 */
u_char* asn_build_sequence(u_char* data, size_t* datalength, u_char type,
                           size_t length)
{
    ...
    if (*datalength < 4)
    {
        snprintf(ebuf, sizeof(ebuf), "%s: length %d < 4: PUNT", errpre,
                 (int)*datalength);
        ebuf[sizeof(ebuf) - 1] = 0;
        ERROR_MSG(ebuf);
        return NULL;
...
u_char* asn_parse_length(u_char* data, u_long* length)
{
    ...
    if (lengthbyte & ASN_LONG_LEN)
    {
        ...
        if (lengthbyte == 0)
        {
            snprintf(ebuf, sizeof(ebuf), "%s: indefinite length not supported",
                     errpre);
            ebuf[sizeof(ebuf) - 1] = 0;
            ERROR_MSG(ebuf);
            return NULL;
        ...
        *length = 0; /* protect against short lengths */
        ...
        if ((long)*length < 0)
        {
            snprintf(ebuf, sizeof(ebuf), "%s: negative data length %ld\n",
                     errpre, (long)*length);
            ebuf[sizeof(ebuf) - 1] = 0;
            ERROR_MSG(ebuf);
            return NULL;
...
u_char* asn_build_length(u_char* data, size_t* datalength, size_t length)
{
    ...
    if (length < 0x80)
    {
        if (*datalength < 1)
        {
            snprintf(ebuf, sizeof(ebuf), "%s: bad length < 1 :%lu, %lu", errpre,
                     (unsigned long)*datalength, (unsigned long)length);
            ebuf[sizeof(ebuf) - 1] = 0;
            ERROR_MSG(ebuf);
            return NULL;
    ...
    else if (length <= 0xFF)
    {
        if (*datalength < 2)
        {
            snprintf(ebuf, sizeof(ebuf), "%s: bad length < 2 :%lu, %lu", errpre,
                     (unsigned long)*datalength, (unsigned long)length);
            ebuf[sizeof(ebuf) - 1] = 0;
            ERROR_MSG(ebuf);
            return NULL;
    ...
    { /* 0xFF < length <= 0xFFFF */
        if (*datalength < 3)
        {
            snprintf(ebuf, sizeof(ebuf), "%s: bad length < 3 :%lu, %lu", errpre,
                     (unsigned long)*datalength, (unsigned long)length);
            ebuf[sizeof(ebuf) - 1] = 0;
            ERROR_MSG(ebuf);
            return NULL;
 ...
 *
 */
u_char* asn_parse_objid(u_char* data, size_t* datalength, u_char* type,
 ...
 *           Returns NULL on any error.
 */
u_char* asn_build_objid(u_char* data, size_t* datalength, u_char type,
                        oid* objid, size_t objidlength)
{
    /*
     * ASN.1 objid ::= 0x06 asnlength subidentifier {subidentifier}*
 ...
 *         Returns NULL on any error.
 */
u_char* asn_parse_bitstring(u_char* data, size_t* datalength, u_char* type,
                            u_char* str, size_t* strlength)
{
    /*
 ...
 *         Returns NULL on any error.
 */
u_char* asn_build_bitstring(u_char* data, size_t* datalength, u_char type,
                            const u_char* str, size_t strlength)
{
    /*
    ...
    if (_asn_bitstring_check(errpre, strlength, (u_char)((str) ? *str : 0)))
        return NULL;
 ...
 *          Returns NULL on any error.
 */
u_char* asn_parse_unsigned_int64(u_char* data, size_t* datalength, u_char* type,
 ...
 *          Returns NULL on any error.
 */
u_char* asn_build_unsigned_int64(u_char* data, size_t* datalength, u_char type,
                                 const struct counter64* cp, size_t countersize)
{
    ...
    if (type == ASN_OPAQUE_COUNTER64)
    {
        ...
        if (_asn_build_header_check("build counter u64", data, *datalength,
                                    intsize + 3))
            return NULL;
        ...
        if (type == ASN_OPAQUE_U64)
    {
         ...
         */
        data = asn_build_header(data, datalength, ASN_OPAQUE, intsize + 3);
        if (_asn_build_header_check("build opaque u64", data, *datalength,
                                    intsize + 3))
            return NULL;
...
u_char* asn_parse_signed_int64(u_char* data, size_t* datalength, u_char* type,
                               struct counter64* cp, size_t countersize)
{
    ...
    *type = *bufp++;
    ...
    {
        snprintf(ebuf, sizeof(ebuf),
                 "%s: wrong type: %d, len %d, buf bytes (%02X,%02X)", errpre,
                 *type, (int)asn_length, *bufp, *(bufp + 1));
        ebuf[sizeof(ebuf) - 1] = 0;
        ERROR_MSG(ebuf);
        return NULL;
    }
 ...
 *          Returns NULL on any error.
 */
u_char* asn_build_signed_int64(u_char* data, size_t* datalength, u_char type,
                               const struct counter64* cp, size_t countersize)
{
    /*
     * ASN.1 integer ::= 0x02 asnlength byte {byte}*
...
/**
 * @internal
 * asn_parse_float - pulls a single precision floating-point out of an opaque
 * type.
 *
 *  On entry, datalength is input as the number of valid bytes following
 *   "data".  On exit, it is returned as the number of valid bytes
 ...
 *          Returns NULL on any error.
 */
u_char* asn_parse_float(u_char* data, size_t* datalength, u_char* type,
                        float* floatp, size_t floatsize)
{
    ...
    *type = *bufp++;
     ...
     */
    if ((*type == ASN_OPAQUE) && (asn_length == ASN_OPAQUE_FLOAT_BER_LEN) &&
...
/**
 * @internal
 * asn_build_float - builds an ASN object containing a single precision
 floating-point
 ...
 */
u_char* asn_build_float(u_char* data, size_t* datalength, u_char type,
                        const float* floatp, size_t floatsize)
{
    union
    {
        float floatVal;
    ...
    if (_asn_build_header_check("build float", data, *datalength,
                                (floatsize + 3)))
        return NULL;
 ...
 *          Returns NULL on any error.
 */
u_char* asn_parse_double(u_char* data, size_t* datalength, u_char* type,
                         double* doublep, size_t doublesize)
{
    ...
    *type = *bufp++;
    ...
    if ((*type == ASN_OPAQUE) && (asn_length == ASN_OPAQUE_DOUBLE_BER_LEN) &&
 ...
 *          Returns NULL on any error.
 */
u_char* asn_build_double(u_char* data, size_t* datalength, u_char type,
                         const double* doublep, size_t doublesize)
{
    long tmp;
    union
    {
    ...
    if (_asn_build_header_check("build double", data, *datalength,
                                doublesize + 3))
        return NULL;
...
int asn_realloc(u_char** pkt, size_t* pkt_len)
{
    if (pkt != NULL && pkt_len != NULL)
    {
        ...
        DEBUGMSGTL(("asn_realloc", " old_pkt %8p, old_pkt_len %lu\n", *pkt,
                    (unsigned long)old_pkt_len));

        if (snmp_realloc(pkt, pkt_len))
        {
            DEBUGMSGTL(("asn_realloc", " new_pkt %8p, new_pkt_len %lu\n", *pkt,
                        (unsigned long)*pkt_len));
            DEBUGMSGTL(("asn_realloc", " memmove(%8p + %08x, %8p, %08x)\n",
                        *pkt, (unsigned)(*pkt_len - old_pkt_len), *pkt,
                        (unsigned)old_pkt_len));
 ...
 */
int asn_realloc_rbuild_length(u_char** pkt, size_t* pkt_len, size_t* offset,
                              int r, size_t length)
{
    static const char* errpre = "build length";
    char ebuf[128];
    ...
    if (length <= 0x7f)
    {
        ...
        {
            snprintf(ebuf, sizeof(ebuf), "%s: bad length < 1 :%ld, %lu", errpre,
    ...
    else
    {
        while (length > 0xff)
        {
            if (((*pkt_len - *offset) < 1) && !(r && asn_realloc(pkt, pkt_len)))
            {
                snprintf(ebuf, sizeof(ebuf), "%s: bad length < 1 :%ld, %lu",
                         errpre, (long)(*pkt_len - *offset),
 ...
 * @return 1 on success, 0 on error
 */
int asn_realloc_rbuild_header(u_char** pkt, size_t* pkt_len, size_t* offset,
                              int r, u_char type, size_t length)
{
    char ebuf[128];
    ...
    if (asn_realloc_rbuild_length(pkt, pkt_len, offset, r, length))
    {
        if (((*pkt_len - *offset) < 1) && !(r && asn_realloc(pkt, pkt_len)))
        {
            snprintf(ebuf, sizeof(ebuf), "bad header length < 1 :%ld, %lu",
                     (long)(*pkt_len - *offset), (unsigned long)length);
            ebuf[sizeof(ebuf) - 1] = 0;
            ERROR_MSG(ebuf);
            return 0;
 ...
 * @return 1 on success, 0 on error
 */
int asn_realloc_rbuild_int(u_char** pkt, size_t* pkt_len, size_t* offset, int r,
...
int asn_realloc_rbuild_string(u_char** pkt, size_t* pkt_len, size_t* offset,
                              int r, u_char type, const u_char* str,
                              size_t strlength)
{
        ...
        else
        {
            ...
            DEBUGIF("dumpv_send")
            {
                ...
                else
                {
                    ...
                    if (sprint_realloc_asciistring(&buf, &l, &ol, 1, str,
                                                   strlength))
                    {
                        DEBUGMSG(("dumpv_send", "  String:\t%s\n", buf));
                    }
...
int asn_realloc_rbuild_sequence(u_char** pkt, size_t* pkt_len, size_t* offset,
                                int r, u_char type, size_t length)
{
...
int asn_realloc_rbuild_objid(u_char** pkt, size_t* pkt_len, size_t* offset,
                             int r, u_char type, const oid* objid,
                             size_t objidlength)
{
    /*
     * ASN.1 objid ::= 0x06 asnlength subidentifier {subidentifier}*
...
int asn_realloc_rbuild_null(u_char** pkt, size_t* pkt_len, size_t* offset,
                            int r, u_char type)
{
    /*
     * ASN.1 null ::= 0x05 0x00
...
int asn_realloc_rbuild_bitstring(u_char** pkt, size_t* pkt_len, size_t* offset,
                                 int r, u_char type, const u_char* str,
                                 size_t strlength)
{
    /*
            ...
            DEBUGIF("dumpv_send")
            {
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        {
                            DEBUGMSG(
                                ("dumpv_send", "  Bitstring:\t[TRUNCATED]\n"));
                        }
                        else
                        {
                            DEBUGMSG(("dumpv_send",
    ...
    *(*pkt + *pkt_len - (++*offset)) = (u_char)low;
    ...
    if (type == ASN_OPAQUE_COUNTER64)
    {
        ...
        *(*pkt + *pkt_len - (++*offset)) = ASN_OPAQUE_TAG1;
         ...
         * Put the tag and length for the Opaque wrapper.
         */
        if (asn_realloc_rbuild_header(pkt, pkt_len, offset, r, ASN_OPAQUE,
                                      intsize + 3))
        {
            if (_asn_realloc_build_header_check("build counter u64", pkt,
                                                pkt_len, intsize + 3))
            {
                return 0;
            }
...
int asn_realloc_rbuild_float(u_char** pkt, size_t* pkt_len, size_t* offset,
                             int r, u_char type, const float* floatp,
                             size_t floatsize)
{
    size_t start_offset = *offset;
    union
...
int asn_realloc_rbuild_double(u_char** pkt, size_t* pkt_len, size_t* offset,
                              int r, u_char type, const double* doublep,
                              size_t doublesize)
{
    size_t start_offset = *offset;
    long tmp;
