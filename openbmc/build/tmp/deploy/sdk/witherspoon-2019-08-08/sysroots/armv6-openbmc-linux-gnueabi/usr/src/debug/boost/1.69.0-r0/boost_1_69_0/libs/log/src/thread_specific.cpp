...
thread_specific_base::thread_specific_base()
{
    ...
    {
        BOOST_LOG_THROW_DESCR_PARAMS(system_error, "TLS capacity depleted",
...
BOOST_LOG_ANONYMOUS_NAMESPACE
{
    ...
    template <typename KeyT, bool IsStoreableV = sizeof(KeyT) <= sizeof(void*),
              bool IsIntegralV = boost::is_integral<KeyT>::value>
    struct pthread_key_traits
    {
        ...
        {
            const int res = pthread_setspecific(
                *static_cast<pthread_key_type*>(stg), value);
            if (BOOST_UNLIKELY(res != 0))
            {
                BOOST_LOG_THROW_DESCR_PARAMS(system_error,
                                             "Failed to set TLS value", (res));
            }
        }
    ...
    struct pthread_key_traits<KeyT, true, true>
    {
        ...
        typedef typename mpl::if_c<boost::is_signed<pthread_key_type>::value,
                                   intptr_t, uintptr_t>::type intptr_type;
#else
        typedef
            typename mpl::if_c<boost::is_signed<pthread_key_type>::value,
                               std::ptrdiff_t, std::size_t>::type intptr_type;
#endif
        ...
        {
            const int res =
                pthread_setspecific((pthread_key_type)(intptr_type)stg, value);
            if (BOOST_UNLIKELY(res != 0))
            {
                BOOST_LOG_THROW_DESCR_PARAMS(system_error,
                                             "Failed to set TLS value", (res));
