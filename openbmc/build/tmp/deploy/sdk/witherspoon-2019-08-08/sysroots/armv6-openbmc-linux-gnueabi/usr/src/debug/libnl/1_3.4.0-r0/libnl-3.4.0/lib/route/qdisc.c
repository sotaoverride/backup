...
static int qdisc_request_update(struct nl_cache* c, struct nl_sock* sk)
{
    ...
    return nl_send_simple(sk, RTM_GETQDISC, NLM_F_DUMP, &tchdr, sizeof(tchdr));
}
 ...
 */
struct rtnl_qdisc* rtnl_qdisc_get_by_parent(struct nl_cache* cache, int ifindex,
                                            uint32_t parent)
{
    struct rtnl_qdisc* q;
...
int rtnl_qdisc_build_change_request(struct rtnl_qdisc* qdisc,
                                    ...
                                    struct nl_msg** result)
{
    return rtnl_qdisc_build_update_request(qdisc, new, NLM_F_REPLACE, result);
}
