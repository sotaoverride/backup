...
static kadm5_ret_t validate_allowed_keysalts(const char* allowed_keysalts)
{
    ...
    ret = krb5_string_to_keysalts(allowed_keysalts, ",", NULL, 0, &ks_tuple,
                                  &n_ks_tuple);
    free(ks_tuple);
    if (ret == EINVAL)
        return KADM5_BAD_KEYSALTS;
...
kadm5_ret_t kadm5_create_policy(void* server_handle, kadm5_policy_ent_t entry,
                                long mask)
{
...
static kadm5_ret_t copy_tl_data(krb5_int16 n_tl_data, krb5_tl_data* tl_data,
                                krb5_tl_data** out)
{
    kadm5_ret_t ret;
...
kadm5_ret_t kadm5_modify_policy(void* server_handle, kadm5_policy_ent_t entry,
                                long mask)
{
    kadm5_server_handle_t handle = server_handle;
    krb5_tl_data* tl;
...
kadm5_ret_t kadm5_get_policy(void* server_handle, kadm5_policy_t name,
