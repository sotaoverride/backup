...
int krb5int_pthread_loaded(void)
{
    ...
    if (x != -1)
        return x;
    if (&pthread_getspecific == 0 || &pthread_setspecific == 0 ||
        &pthread_key_create == 0 || &pthread_key_delete == 0 ||
        &pthread_once == 0 || &pthread_mutex_lock == 0 ||
        &pthread_mutex_unlock == 0 || &pthread_mutex_destroy == 0 ||
        &pthread_mutex_init == 0 || &pthread_self == 0 ||
        &pthread_equal == 0
...
int KRB5_CALLCONV krb5int_mutex_alloc(k5_mutex_t** m)
{
    k5_mutex_t* ptr;
    int err;
...
void KRB5_CALLCONV krb5int_mutex_free(k5_mutex_t* m)
{
...
void KRB5_CALLCONV krb5int_mutex_lock(k5_mutex_t* m)
{
    k5_mutex_lock(m);
}
void KRB5_CALLCONV krb5int_mutex_unlock(k5_mutex_t* m)
{
    k5_mutex_unlock(m);
}
