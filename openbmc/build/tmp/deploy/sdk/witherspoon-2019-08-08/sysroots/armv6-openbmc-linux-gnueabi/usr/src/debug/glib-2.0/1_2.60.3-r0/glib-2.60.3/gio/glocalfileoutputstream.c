...
static gssize g_local_file_output_stream_write(GOutputStream* stream,
                                               ...
                                               GError** error);
#ifdef G_OS_UNIX
static gboolean g_local_file_output_stream_writev(
...
static GFileInfo* g_local_file_output_stream_query_info(
    GFileOutputStream* stream, const char* attributes,
    ...
    g_local_file_output_stream_class_init(GLocalFileOutputStreamClass* klass)
{
    ...
    GFileOutputStreamClass* file_stream_class =
    ...
    while (1)
    {
        ...
        if (res == -1)
        {
            ...
            g_set_error(error, G_IO_ERROR, g_io_error_from_errno(errsv),
                        _("Error writing to file: %s"), g_strerror(errsv));
        }
...
#define G_OUTPUT_VECTOR_IS_IOVEC                                               \
    (sizeof(struct iovec) == sizeof(GOutputVector) &&                          \
     sizeof((struct iovec*)0)->iov_base ==                                     \
         sizeof((GOutputVector*)0)->buffer &&                                  \
...
gboolean _g_local_file_output_stream_really_close(GLocalFileOutputStream* file,
                                                  GCancellable* cancellable,
                                                  GError** error)
{
    ...
    if (!g_close(file->priv->fd, NULL))
    {
        ...
        g_set_error(error, G_IO_ERROR, g_io_error_from_errno(errsv),
                    _("Error closing file: %s"), g_strerror(errsv));
        return FALSE;
    }
    ...
    if (file->priv->tmp_filename)
    {
        ...
        if (file->priv->backup_filename)
        {
            ...
            if (g_unlink(file->priv->backup_filename) != 0 && errno != ENOENT)
            {
                ...
                g_set_error(error, G_IO_ERROR, G_IO_ERROR_CANT_CREATE_BACKUP,
                            _("Error removing old backup link: %s"),
                            g_strerror(errsv));
                goto err_out;
            }

            if (link(file->priv->original_filename,
                     file->priv->backup_filename) != 0)
            {
                /*  link failed or is not supported, try rename  */
                if (g_rename(file->priv->original_filename,
                             file->priv->backup_filename) != 0)
                {
                    int errsv = errno;
                    ...
                    g_set_error(
                        ...
                        _("Error creating backup copy: %s"), g_strerror(errsv));
                    goto err_out;
                }
            }
#else
        ...
        /* tmp -> original */
        if (g_rename(file->priv->tmp_filename, file->priv->original_filename) !=
            0)
        {
            int errsv = errno;
                                                 ...
                                                 GError** error)
{
    ...
    if (file->priv->do_close)
        return _g_local_file_output_stream_really_close(file, cancellable,
                                                        error);
    return TRUE;
}
                                                ...
                                                GError** error)
{
    ...
    if (pos == (off_t)-1)
    {
        ...
        g_set_error(error, G_IO_ERROR, g_io_error_from_errno(errsv),
                    _("Error seeking in file: %s"), g_strerror(errsv));
        return FALSE;
    }
...
restart:
    ...
    if (res == -1)
    {
        ...
        g_set_error(error, G_IO_ERROR, g_io_error_from_errno(errsv),
                    _("Error truncating file: %s"), g_strerror(errsv));
        return FALSE;
    }
...
static void set_error_from_open_errno(const char* filename, GError** error)
{
    int errsv = errno;
...
static GFileOutputStream* output_stream_open(const char* filename,
                                             gint open_flags, guint mode,
                                             GCancellable* cancellable,
                                             GError** error)
{
    GLocalFileOutputStream* stream;
...
static gint mode_from_flags_or_info(GFileCreateFlags flags,
...
GFileOutputStream* _g_local_file_output_stream_create(
    const char* filename, gboolean readable, GFileCreateFlags flags,
    GFileInfo* reference_info, GCancellable* cancellable, GError** error)
{
    int mode;
    int open_flags;
                                                      ...
                                                      GError** error)
{
    ...
    return output_stream_open(filename,
                              O_CREAT | O_APPEND | O_WRONLY | O_BINARY, mode,
                              cancellable, error);
}
...
static gboolean copy_file_data(gint sfd, gint dfd, GError** error)
{
    ...
    do
    {
        ...
        if (bytes_read == -1)
        {
            ...
            g_set_error(error, G_IO_ERROR, g_io_error_from_errno(errsv),
                        _("Error reading from file: %s"), g_strerror(errsv));
            ret = FALSE;
            break;
        }
...
static int handle_overwrite_open(const char* filename, gboolean readable,
                                 const char* etag, gboolean create_backup,
                                 char** temp_filename, GFileCreateFlags flags,
                                 GFileInfo* reference_info,
                                 GCancellable* cancellable, GError** error)
{
    int fd = -1;
    ...
    if (etag != NULL)
    {
        ...
        {
            g_set_error_literal(error, G_IO_ERROR, G_IO_ERROR_WRONG_ETAG,
        ...
        (!(original_stat.st_nlink > 1) && !is_symlink))
    {
        ...
        tmpfd = g_mkstemp_full(tmp_filename,
                               (readable ? O_RDWR : O_WRONLY) | O_BINARY, mode);
        if (tmpfd == -1)
        {
            g_free(tmp_filename);
                ...
                0))
        {
            ...
            if (tres != 0 || original_stat.st_uid != tmp_statbuf.st_uid ||
...
fallback_strategy:
    ...
    if (create_backup)
    {
        ...
        bfd = g_open(backup_filename, O_WRONLY | O_CREAT | O_EXCL | O_BINARY,
        ...
        {
            if (fchmod(bfd, (original_stat.st_mode & 0707) |
    ...
    if (flags & G_FILE_CREATE_REPLACE_DESTINATION)
    {
        ...
        if (g_unlink(filename) != 0)
        {
            ...
            g_set_error(error, G_IO_ERROR, g_io_error_from_errno(errsv),
                        _("Error removing old file: %s"), g_strerror(errsv));
            goto err_out2;
        }
...
GFileOutputStream* _g_local_file_output_stream_replace(
    const char* filename, gboolean readable, const char* etag,
    gboolean create_backup, GFileCreateFlags flags, GFileInfo* reference_info,
    GCancellable* cancellable, GError** error)
{
    GLocalFileOutputStream* stream;
    int mode;
    ...
    if (fd == -1 && errno == EEXIST)
    {
        ...
        fd = handle_overwrite_open(filename, readable, etag, create_backup,
                                   &temp_file, flags, reference_info,
                                   cancellable, error);
        if (fd == -1)
            return NULL;

        /* If the final destination exists, we want to sync the newly written
         * file to ensure the data is on disk when we rename over the
         * destination. otherwise if we get a system crash we can lose both the
         * new and the old file on some filesystems. (I.E. those that don't
