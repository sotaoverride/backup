...
OM_uint32 KRB5_CALLCONV gss_display_status(minor_status, status_value,
                                           status_type, req_mech_type,
...

{
    ...
    if (minor_status == NULL || message_context == NULL ||
    ...
    }
    {
        ...
        if (m_oid.length == 0)
        {
            ...
            status =
                g_display_com_err_status(minor_status, m_status, status_string);
    ...
    if (mech && mech->gss_display_status)
    {
        ...
        r = mech->gss_display_status(minor_status, status_value, status_type,
 ...
 */
static OM_uint32 displayMajor(status, msgCtxt, outStr) OM_uint32 status;
OM_uint32* msgCtxt;
gss_buffer_t outStr;
{
             ...
             (oneVal = GSS_SUPPLEMENTARY_INFO(status)))
    {
        ...
        if (!haveErr)
            ...
            *msgCtxt =
                (OM_uint32)((GSS_SUPPLEMENTARY_INFO(oneVal) ^ mask) << 1);
        else
            *msgCtxt = 0;
    }
