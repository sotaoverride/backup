...
#define HID0_TRACE_BITS                                                        \
    (HID0_ONE_PER_GROUP | HID0_DO_SINGLE | HID0_SINGLE_DECODE |                \
     HID0_EN_INST_TRACE | HID0_TRACE_EN)
...
#define HTM_STATUS_ERROR_MASK                                                  \
    (HTM_STATUS_CRESP_OV | PPC_BITMASK(4, 7) | PPC_BITMASK(17, 19))
...
static int configure_nhtm(struct htm* htm, bool wrap)
{
    ...
    val = wrap ? HTM_MODE_WRAP : 0;
    if (HTM_ERR(pib_write(&htm->target, HTM_COLLECTION_MODE,
                          HTM_MODE_ENABLE | NHTM_MODE_CRESP_PRECISE | val |
                              0xFFFF000000)))
        return -1;

    /* Stop on core xstop */
    if (HTM_ERR(
            pib_write(&htm->target, HTM_TRIGGER_CONTROL, HTM_CTRL_XSTOP_STOP)))
        return -1;
    ;
     ...
     */
    if (HTM_ERR(
            pib_write(&htm->target, NHTM_FILTER_CONTROL,
...
static int configure_memory(struct htm* htm)
{
    ...
    if (size < 16777216UL)
    {
        PR_ERROR("memtrace size must be atleast 16MB. Currently:%" PRIx64 "\n",
                 size);
        return -1;
    }
    if (size > 274877906944UL)
    {
        PR_ERROR("memtrace size must be smaller than 256GB. Currently:%" PRIx64
                 "\n",
                 size);
        return -1;
    }
...
__attribute__((constructor)) static void register_htm(void)
{
