...
gchar* g_credentials_to_string(GCredentials* credentials)
{
    ...
    if (credentials->native.pid != -1)
        g_string_append_printf(ret, "pid=%" G_GINT64_FORMAT ",",
                               (gint64)credentials->native.pid);
    if (credentials->native.uid != -1)
        g_string_append_printf(ret, "uid=%" G_GINT64_FORMAT ",",
                               (gint64)credentials->native.uid);
    if (credentials->native.gid != -1)
        g_string_append_printf(ret, "gid=%" G_GINT64_FORMAT ",",
                                    ...
                                    GError** error)
{
    ...
    if (credentials->native.uid == other_credentials->native.uid)
        ret = TRUE;
#elif G_CREDENTIALS_USE_SOLARIS_UCRED
    if (ucred_geteuid(credentials->native) ==
        ucred_geteuid(other_credentials->native))
        ret = TRUE;
#else
...
static gboolean credentials_native_type_check(GCredentialsType requested_type,
                                              const char* op)
{
    ...
    g_warning("g_credentials_%s_native: Trying to %s credentials of type %s "
              ...
              op, op, requested ? requested->value_name : "(unknown)",
