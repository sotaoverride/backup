...
class token_data
{
  public:
    ...
    token_data(token_id id_, string_type const& value_,
        ...
        id(id_),
        value(value_), pos(pos_), refcnt(1)
    {
    }
...
inline void* token_data<StringTypeT, PositionT>::operator new(std::size_t size)
{
    ...
    typedef boost::singleton_pool<token_data_tag,
                                  sizeof(token_data<StringTypeT, PositionT>)>
        pool_type;
...
template <typename StringTypeT, typename PositionT>
inline void token_data<StringTypeT, PositionT>::
...
inline std::ostream& operator<<(std::ostream& stream,
                                lex_token<PositionT> const& object)
{
    object.print(stream);
    return stream;
}
