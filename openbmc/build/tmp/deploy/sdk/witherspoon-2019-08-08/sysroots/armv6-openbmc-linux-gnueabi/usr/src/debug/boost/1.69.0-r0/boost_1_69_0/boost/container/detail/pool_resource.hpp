...
static const std::size_t pool_options_minimum_largest_required_pool_block =
    memory_resource::max_align > 2 * sizeof(void*) ? memory_resource::max_align
                                                   : 2 * sizeof(void*);
...
class pool_resource
{
    ...
    static void priv_limit_option(std::size_t& val, std::size_t min,
                                  std::size_t max);
  ...
  public:
    ...
    //! allocation of `bytes`.
    //!   from the pool specified by `pool_index`. Returns `pool_count()` if
    //!   `bytes` is bigger than `options().largest_required_pool_block` (no
    //!   pool will be used to serve this).
    //!
    //! <b>Note</b>: Non-standard extension.
