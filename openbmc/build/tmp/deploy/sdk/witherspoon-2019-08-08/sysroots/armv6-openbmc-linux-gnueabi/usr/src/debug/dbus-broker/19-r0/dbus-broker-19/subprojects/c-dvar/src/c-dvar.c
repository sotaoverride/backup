 ...
 */
_public_ void c_dvar_get_root_types(CDVar* var, const CDVarType** typesp,
                                    size_t* n_typesp)
{
    if (typesp)
 ...
 */
_public_ void c_dvar_get_parent_types(CDVar* var, const CDVarType** typesp,
                                      size_t* n_typesp)
{
    if (typesp)
...
int c_dvar_next_varg(CDVar* var, char c)
{
    ...
    switch (c)
    {
        ...
        case '{':
             ...
             */
            if (_unlikely_(var->current >=
                           var->levels + C_DVAR_TYPE_DEPTH_MAX - 1))
                return -ENOTRECOVERABLE;
