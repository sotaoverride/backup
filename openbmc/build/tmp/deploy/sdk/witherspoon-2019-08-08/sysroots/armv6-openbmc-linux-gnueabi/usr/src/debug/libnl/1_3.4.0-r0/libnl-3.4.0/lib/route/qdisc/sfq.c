...
static int sfq_msg_parser(struct rtnl_tc* tc, void* data)
{
    ...
    sfq->qs_mask =
        (SCH_SFQ_ATTR_QUANTUM | SCH_SFQ_ATTR_PERTURB | SCH_SFQ_ATTR_LIMIT |
         SCH_SFQ_ATTR_DIVISOR | SCH_SFQ_ATTR_FLOWS);
                          ...
                          struct nl_dump_params* p)
{
    ...
    if (sfq)
        nl_dump(p, " quantum %u perturb %us", sfq->qs_quantum, sfq->qs_perturb);
}
                             ...
                             struct nl_dump_params* p)
{
    ...
    if (sfq)
        nl_dump(p, "limit %u divisor %u", sfq->qs_limit, sfq->qs_divisor);
}
