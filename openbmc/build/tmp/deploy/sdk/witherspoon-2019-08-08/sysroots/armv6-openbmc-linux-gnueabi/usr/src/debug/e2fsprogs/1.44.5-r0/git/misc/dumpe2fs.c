...
static void print_free(unsigned long group, char* bitmap, unsigned long num,
...
static void print_bg_opt(int bg_flags, int mask, const char* str, int* first)
{
    if (bg_flags & mask)
    {
        if (*first)
...
static void print_bg_opts(ext2_filsys fs, dgrp_t i)
{
    ...
    print_bg_opt(bg_flags, EXT2_BG_INODE_UNINIT, "INODE_UNINIT", &first);
    print_bg_opt(bg_flags, EXT2_BG_BLOCK_UNINIT, "BLOCK_UNINIT", &first);
    print_bg_opt(bg_flags, EXT2_BG_INODE_ZEROED, "ITABLE_ZEROED", &first);
    if (!first)
        fputc(']', stdout);
    fputc('\n', stdout);
}
...
static void list_desc(ext2_filsys fs, int grp_only)
{
    ...
    inode_blocks_per_group =
        ((fs->super->s_inodes_per_group * EXT2_INODE_SIZE(fs->super)) +
    ...
    for (i = 0; i < fs->group_desc_count; i++)
    {
        ...
        if (grp_only)
        {
            ...
            if (old_desc_blk)
            {
                print_range(old_desc_blk, old_desc_blk + old_desc_blocks - 1);
                printf(":");
            }
        ...
        print_bg_rel_offset(fs, ext2fs_block_bitmap_loc(fs, i), 0, first_block,
                            last_block);
        if (ext2fs_has_feature_metadata_csum(fs->super))
            printf(_(", csum 0x%08x"), ext2fs_block_bitmap_checksum(fs, i));
        ...
        print_bg_rel_offset(fs, ext2fs_inode_bitmap_loc(fs, i), 0, first_block,
                            last_block);
        if (ext2fs_has_feature_metadata_csum(fs->super))
            printf(_(", csum 0x%08x"), ext2fs_inode_bitmap_checksum(fs, i));
        fputs(_("\n  Inode table at "), stdout);
        print_range(ext2fs_inode_table_loc(fs, i),
                    ext2fs_inode_table_loc(fs, i) + inode_blocks_per_group - 1);
        print_bg_rel_offset(fs, ext2fs_inode_table_loc(fs, i), 1, first_block,
                            last_block);
        ...
        if (block_bitmap)
        {
            ...
            retval = ext2fs_get_block_bitmap_range2(
                fs->block_map, blk_itr, block_nbytes << 3, block_bitmap);
            if (retval)
                com_err("list_desc", retval, "while reading block bitmap");
            else
                print_free(i, block_bitmap, fs->super->s_clusters_per_group,
        ...
        if (inode_bitmap)
        {
            ...
            retval = ext2fs_get_inode_bitmap_range2(
                fs->inode_map, ino_itr, inode_nbytes << 3, inode_bitmap);
            if (retval)
...
static void print_inline_journal_information(ext2_filsys fs)
{
    ...
    if (retval)
    {
        com_err(program_name, retval, "%s", _("while opening journal inode"));
        exit(1);
    }
...
static void print_journal_information(ext2_filsys fs)
{
    ...
    if ((retval = io_channel_read_blk64(
             fs->io, fs->super->s_first_data_block + 1, -1024, buf)))
    {
        com_err(program_name, retval, "%s",
...
static int check_mmp(ext2_filsys fs)
{
    ...
    if (retval)
    {
        com_err(program_name, retval, _("while trying to open %s"),
                fs->device_name);
        if (retval == EXT2_ET_MMP_FAILED || retval == EXT2_ET_MMP_FSCK_ON ||
            ...
            retval == EXT2_ET_MMP_UNKNOWN_SEQ)
        {
            if (fs->mmp_buf)
            {
                ...
                fprintf(stderr, "%s: MMP last updated by '%s' on %s",
                        program_name, mmp->mmp_nodename, ctime(&mmp_time));
            }
            retval = 1;
        }
        else
...
static void print_mmp_block(ext2_filsys fs)
{
    ...
    if (fs->mmp_buf == NULL)
    {
        ...
        if (retval)
        {
            com_err(program_name, retval, _("failed to alloc MMP buffer\n"));
            return;
        }
    }
    ...
    if (retval)
    {
        com_err(program_name, retval, _("reading MMP block %llu from '%s'\n"),
                                ...
                                int* blocksize)
{
    ...
    for (token = buf; token && *token; token = next)
    {
        ...
        if (strcmp(token, "superblock") == 0 || strcmp(token, "sb") == 0)
        {
            if (!arg)
            {
                do_usage++;
            ...
            *superblock = strtoul(arg, &p, 0);
            if (*p)
            {
                fprintf(stderr, _("Invalid superblock parameter: %s\n"), arg);
                do_usage++;
                continue;
            }
        }
        else if (strcmp(token, "blocksize") == 0 || strcmp(token, "bs") == 0)
        {
            if (!arg)
            {
                do_usage++;
            ...
            *blocksize = strtoul(arg, &p, 0);
            if (*p)
            {
                fprintf(stderr, _("Invalid blocksize parameter: %s\n"), arg);
                do_usage++;
                continue;
            }
        }
    ...
    while ((c = getopt(argc, argv, "bfghimxVo:")) != EOF)
    {
        switch (c)
        {
            ...
            case 'm':
                ...
                break;
            case 'o':
                parse_extended_opts(optarg, &use_superblock, &use_blocksize);
                break;
            case 'V':
...
try_open_again:
    if (use_superblock && !use_blocksize)
    {
        for (use_blocksize = EXT2_MIN_BLOCK_SIZE;
             use_blocksize <= EXT2_MAX_BLOCK_SIZE; use_blocksize *= 2)
        {
    ...
    {
        retval = ext2fs_open(device_name, flags, use_superblock, use_blocksize,
                             unix_io_manager, &fs);
    }
    ...
    if (mmp_check)
    {
        if (ext2fs_has_feature_mmp(fs->super) && fs->super->s_mmp_block != 0)
        {
            if (mmp_info)
            {
                print_mmp_block(fs);
