...
static const char doc[] =
    PROGRAM_NAME " version " VERSION " - a tool to write data to UBI volumes.";
...
static const char usage[] =
    "Usage: " PROGRAM_NAME
    " <UBI volume node file name> [-t] [-s <size>] [-h] [-V] [--truncate]\n"
    "\t\t\t[--size=<size>] [--help] [--version] <image file>\n\n"
    "Example 1: " PROGRAM_NAME
    " /dev/ubi0_1 fs.img - write file \"fs.img\" to UBI volume /dev/ubi0_1\n"
    "Example 2: " PROGRAM_NAME
    " /dev/ubi0_1 -t - wipe out UBI volume /dev/ubi0_1";
...
static int parse_opt(int argc, char* const argv[])
{
    ...
    if (args.img && args.truncate)
        return errmsg(
            "You can't truncate and specify an image (use -h for help)");
...
static int ubi_write(int fd, const void* buf, int len)
{
    ...
    while (len)
    {
        ...
        if (ret < 0)
        {
            ...
            return sys_errmsg("cannot write %d bytes to volume \"%s\"", len,
                              args.node);
        }

        if (ret == 0)
            return errmsg("cannot write %d bytes to volume \"%s\"", len,
                          args.node);
...
static int update_volume(libubi_t libubi, struct ubi_vol_info* vol_info)
{
    ...
    while (bytes)
    {
        ...
        if (ret <= 0)
        {
            ...
            {
                sys_errmsg("cannot read %d bytes from \"%s\"", to_copy,
                           args.img);
                goto out_close;
            }
        }
...
int main(int argc, char* const argv[])
{
    ...
    if (err)
    {
        sys_errmsg("cannot get information about UBI volume \"%s\"", args.node);
        goto out_libubi;
    }
