...
int event_reset_time(sd_event* e, sd_event_source** s, clockid_t clock,
                     uint64_t usec, uint64_t accuracy,
                     sd_event_time_handler_t callback, void* userdata,
                     int64_t priority, const char* description,
                     bool force_reset)
{
    ...
    if (*s)
    {
        if (!force_reset)
        {
            ...
            if (r < 0)
                return log_debug_errno(
                    r,
                    "sd-event: Failed to query whether event source \"%s\" is "
                    "enabled or not: %m",
        ...
        if (c != clock)
            return log_debug_errno(
                ...
                "sd-event: Current clock id %i of event source \"%s\" is "
                "different from specified one %i.",
                (int)c, strna((*s)->description ?: description), (int)clock);

        r = sd_event_source_set_time(*s, usec);
        if (r < 0)
            return log_debug_errno(
                r, "sd-event: Failed to set time for event source \"%s\": %m",
                strna((*s)->description ?: description));
        ...
        /* callback function is not updated, as we do not have
         * sd_event_source_set_time_callback(). */
        ...
        if (r < 0)
            return log_debug_errno(
                r, "sd-event: Failed to enable event source \"%s\": %m",
                strna((*s)->description ?: description));
    }
    else
    {
        ...
        if (r < 0)
            return log_debug_errno(
                r, "sd-event: Failed to create timer event \"%s\": %m",
                strna(description));
    ...
    if (r < 0)
        return log_debug_errno(
            r, "sd-event: Failed to set priority for event source \"%s\": %m",
            strna((*s)->description ?: description));
    ...
    if (description)
    {
        ...
        if (r < 0)
            return log_debug_errno(r,
                                   "sd-event: Failed to set description for "
                                   "event source \"%s\": %m",
                                   description);
    }
