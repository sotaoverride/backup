...
#else
    : detail::components_bcc<typename remove_cv<T>::type, T, ClassTypeTransform>
#endif
{
...
template <typename Components, typename IfTagged, typename ThenTag,
          typename DefaultBase = components_non_func_base>
struct retagged_if
    : mpl::if_<detail::represents_impl<Components, IfTagged>,
...
template <typename T, typename L>
struct components_impl
    : detail::retagged_if<detail::components_impl<T*, L>, pointer_tag,
                          /* --> */ function_tag>
{
};
template <typename T, typename L>
...
{
    typedef typename mpl::remove<
        typename Base::types,
        typename detail::class_transform<a_class, L>::type>::type types;
};
...
struct components_impl<T*, L>
    : mpl::if_<
          ...
          components_non_func_base>::type
{
};

template <typename T, typename L>
struct components_impl<T a_class::*, L> : components_non_func_base
{
};
#else
template <typename T, typename L>
struct components_impl<T*, L> : components_non_func_base
{
};
#endif
...
struct member_obj_ptr_components : member_object_pointer_base
{
    ...
    typedef mpl::vector2<typename detail::member_obj_ptr_result<T, C>::type,
                         typename detail::class_transform<C, L>::type>
        types;
};
...
template <typename T, class C, typename L>
struct components_impl<T C::*, L> : member_obj_ptr_components<T, C, L>
{
};
...
template <typename T>
struct encode_cv<T const*>
{
    typedef char (&type)[2];
    BOOST_STATIC_CONSTANT(std::size_t, value = 2);
};
template <typename T>
struct encode_cv<T volatile*>
{
    typedef char (&type)[3];
    BOOST_STATIC_CONSTANT(std::size_t, value = 3);
};
template <typename T>
struct encode_cv<T const volatile*>
{
    typedef char (&type)[4];
...
struct encode_mfp_cv
{
    BOOST_STATIC_CONSTANT(std::size_t,
                          value = sizeof(detail::mfp_cv_tester((T)0L)));
};

// Associate bits with the CV codes above.
template <std::size_t>
struct cv_tag_mfp_impl;
...
template <>
struct cv_tag_mfp_impl<1> : non_cv
{
};
template <>
struct cv_tag_mfp_impl<2> : const_non_volatile
{
};
template <>
struct cv_tag_mfp_impl<3> : volatile_non_const
{
};
template <>
struct cv_tag_mfp_impl<4> : cv_qualified
{
};
...
template <typename T, std::size_t CV>
struct decode_cv;

template <typename T>
struct decode_cv<T, 1> : mpl::identity<T*>
{
};
template <typename T>
struct decode_cv<T, 2> : mpl::identity<T const*>
{
};
template <typename T>
struct decode_cv<T, 3> : mpl::identity<T volatile*>
{
};
template <typename T>
struct decode_cv<T, 4> : mpl::identity<T const volatile*>
{
};
...
template <typename T, typename L>
struct bcc_class_transform_impl<T*, L> : class_transform<T, L>
{
};
...
struct bcc_class_transform
    : bcc_class_transform_impl<
          typename decode_cv<T, ::boost::function_types::detail::encode_mfp_cv<
                                    D>::value>::type,
          L>
{
};
...
struct mfp_components<Base, T C::*, OrigT, L>
{
  private:
    ...
    typedef typename mpl::advance<
        ...
        typename mpl::if_<
            ...
            mpl::integral_c<int, 2>, mpl::integral_c<int, 1>>::type>::type from;
  ...
  public:
    typedef typename mpl::reverse_copy<
        types_view, mpl::front_inserter<mpl::vector0<>>>::type types;

    typedef typename function_types::tag<Base, detail::cv_tag_mfp<OrigT>>::bits
        bits;
...
struct components_bcc
    : mpl::if_<
          ...
          detail::components_impl<T, L>>::type
{
};
