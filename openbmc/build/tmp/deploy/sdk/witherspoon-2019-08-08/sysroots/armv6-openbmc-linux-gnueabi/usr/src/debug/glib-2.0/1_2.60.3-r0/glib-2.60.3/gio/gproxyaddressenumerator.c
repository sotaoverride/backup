...
static void save_userinfo(GProxyAddressEnumeratorPrivate* priv,
                          const gchar* proxy)
{
    gchar* userinfo;
    ...
    if (_g_uri_parse_authority(proxy, NULL, NULL, &userinfo, NULL))
    {
        if (userinfo)
        {
            ...
            if (split[0] != NULL)
            {
                ...
                if (split[1] != NULL)
                    priv->proxy_password =
                        g_uri_unescape_string(split[1], NULL);
            }
...
static void next_enumerator(GProxyAddressEnumeratorPrivate* priv)
{
    ...
    while (priv->addr_enum == NULL && *priv->next_proxy)
    {
        ...
        if (strcmp("direct", priv->proxy_type) == 0)
        {
            if (priv->connectable)
                ...
                connectable =
                    g_network_address_new(priv->dest_hostname, priv->dest_port);
        }
        else
        {
            GError* error = NULL;
            ...
            connectable =
                g_network_address_parse_uri(priv->proxy_uri, 0, &error);

            if (error)
            {
                g_warning("Invalid proxy URI '%s': %s", priv->proxy_uri,
                          error->message);
                g_error_free(error);
            }
                                    ...
                                    GCancellable* cancellable, GError** error)
{
    ...
    {
        priv->proxies = g_proxy_resolver_lookup(
            priv->proxy_resolver, priv->dest_uri, cancellable, error);
    ...
    while (result == NULL && (*priv->next_proxy || priv->addr_enum))
    {
        ...
        if (!priv->supports_hostname)
        {
            ...
            if (!priv->dest_ips)
            {
                ...
                priv->dest_ips = g_resolver_lookup_by_name(
                    resolver, priv->dest_hostname, cancellable,
                    first_error ? NULL : &first_error);
        ...
        result = g_object_new(
            G_TYPE_PROXY_ADDRESS, "address", inetaddr, "port", port, "protocol",
            priv->proxy_type, "destination-protocol", dest_protocol,
            "destination-hostname", dest_hostname, "destination-port",
            priv->dest_port, "username", priv->proxy_username, "password",
            priv->proxy_password, "uri", priv->proxy_uri, NULL);
...
static void complete_async(GTask* task)
{
...
static void address_enumerate_cb(GObject* object, GAsyncResult* result,
                                 gpointer user_data);

static void next_proxy(GTask* task)
{
    GProxyAddressEnumeratorPrivate* priv = g_task_get_task_data(task);
...
static void dest_hostname_lookup_cb(GObject* object, GAsyncResult* result,
                                    gpointer user_data)
{
    ...
    priv->dest_ips = g_resolver_lookup_by_name_finish(
        G_RESOLVER(object), result, &priv->last_error);
    if (priv->dest_ips)
        return_result(task);
    else
    {
                                 ...
                                 gpointer user_data)
{
    ...
    g_clear_error(&priv->last_error);
    priv->proxy_address = g_socket_address_enumerator_next_finish(
        priv->addr_enum, result, &priv->last_error);
    if (priv->proxy_address)
    {
        if (!priv->supports_hostname && !priv->dest_ips)
        {
            GResolver* resolver;
            resolver = g_resolver_get_default();
            g_resolver_lookup_by_name_async(resolver, priv->dest_hostname,
                                            g_task_get_cancellable(task),
                                            dest_hostname_lookup_cb, task);
            g_object_unref(resolver);
            return;
        }
...
static void proxy_lookup_cb(GObject* object, GAsyncResult* result,
                            gpointer user_data)
{
    GTask* task = user_data;
    GProxyAddressEnumeratorPrivate* priv = g_task_get_task_data(task);
...
static void g_proxy_address_enumerator_next_async(
    ...
    GAsyncReadyCallback callback, gpointer user_data)
{
    ...
    if (priv->proxies == NULL)
    {
        g_proxy_resolver_lookup_async(priv->proxy_resolver, priv->dest_uri,
                                      cancellable, proxy_lookup_cb, task);
        return;
    }
    ...
    if (priv->addr_enum)
    {
        ...
        {
            g_socket_address_enumerator_next_async(priv->addr_enum, cancellable,
                                                   address_enumerate_cb, task);
            return;
        }
    }
...
static void g_proxy_address_enumerator_constructed(GObject* object)
{
    ...
    if (priv->dest_uri)
    {
        conn = g_network_address_parse_uri(priv->dest_uri, priv->default_port,
                                           NULL);
        if (conn)
        {
            g_object_get(conn, "hostname", &priv->dest_hostname, "port", &port,
                         NULL);
            priv->dest_port = port;
...
static void g_proxy_address_enumerator_get_property(GObject* object,
                                                    guint property_id,
                                                    GValue* value,
                                                    GParamSpec* pspec)
{
...
static void g_proxy_address_enumerator_set_property(GObject* object,
                                                    guint property_id,
                                                    const GValue* value,
                                                    GParamSpec* pspec)
{
...
static void g_proxy_address_enumerator_finalize(GObject* object)
{
    GProxyAddressEnumeratorPrivate* priv = GET_PRIVATE(object);
...
static void g_proxy_address_enumerator_init(GProxyAddressEnumerator* self)
{
    self->priv = g_proxy_address_enumerator_get_instance_private(self);
}

static void g_proxy_address_enumerator_class_init(
    GProxyAddressEnumeratorClass* proxy_enumerator_class)
{
    ...
    g_object_class_install_property(
        ...
        g_param_spec_string(
            "uri", P_("URI"),
            P_("The destination URI, use none:// for generic socket"), NULL,
     ...
     */
    g_object_class_install_property(
        object_class, PROP_DEFAULT_PORT,
        g_param_spec_uint(
            "default-port", P_("Default port"),
            P_("The default port to use if uri does not specify one"), 0, 65535,
            0,
            G_PARAM_READWRITE | G_PARAM_CONSTRUCT_ONLY |
                G_PARAM_STATIC_STRINGS));

    g_object_class_install_property(
        object_class, PROP_CONNECTABLE,
        g_param_spec_object("connectable", P_("Connectable"),
