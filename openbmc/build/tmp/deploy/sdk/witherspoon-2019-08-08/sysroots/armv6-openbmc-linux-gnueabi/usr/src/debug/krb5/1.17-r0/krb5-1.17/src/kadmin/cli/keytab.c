...
void kadmin_keytab_add(int argc, char** argv)
{
    ...
    while (argc)
    {
        ...
        else if (strcmp(*argv, "-e") == 0)
        {
            ...
            if (retval)
            {
                com_err("ktadd", retval, _("while parsing keysalts %s"), *argv);
...
static krb5_error_code fetch_new_keys(void* lhandle, krb5_principal princ,
                                      krb5_boolean keepold, int n_ks_tuple,
                                      ...
                                      int* nkeys_out)
{
    ...
    *nkeys_out = 0;
    ...
    code = randkey_princ(lhandle, princ, keepold, n_ks_tuple, ks_tuple, &keys,
                         &nkeys);
    if (code)
        goto cleanup;
                             ...
                             char* princ_str, char* kvno_str)
{
    ...
    if (code != 0)
    {
        ...
        else if (code == KRB5_KT_NOTFOUND)
        {
            ...
            {
                fprintf(stderr,
                        _("%s: No entry for principal %s exists in "
                          "keytab %s\n"),
            ...
            {
                fprintf(stderr,
                        _("%s: No entry for principal %s with kvno %d "
                          "exists in keytab %s\n"),
    ...
    while ((code = krb5_kt_next_entry(context, keytab, &entry, &cursor)) == 0)
    {
        if (krb5_principal_compare(context, princ, entry.principal) &&
            ((mode == ALL) || (mode == SPEC && entry.vno == kvno) ||
    ...
    {
        fprintf(stderr,
                _("%s: There is only one entry for principal %s in "
                  "keytab %s\n"),
                whoami, princ_str, keytab_str);
    }
