...
/* Note: unused areas in the sd_bus_vtable[] array must be initalized to 0. The
 * stucture contains an embedded union, and the compiler is NOT required to
 * initalize the unused areas of the union when the rest of the structure is
 * initalized. Normally the array is defined as read-only data, in which case
 * the linker places it in the BSS section, which is always fully initalized, so
 * this is not a concern. But if the array is created on the stack or on the
 * heap, care must be taken to initalize the unused areas, for examply by
...
#define SD_BUS_METHOD_WITH_NAMES_OFFSET(_member, _signature, _in_names,        \
                                        _result, _out_names, _handler,         \
                                        _offset, _flags)                       \
    {                                                                          \
...
#define SD_BUS_METHOD_WITH_OFFSET(_member, _signature, _result, _handler,      \
                                  _offset, _flags)                             \
    SD_BUS_METHOD_WITH_NAMES_OFFSET(_member, _signature, "", _result, "",      \
                                    _handler, _offset, _flags)
#define SD_BUS_METHOD_WITH_NAMES(_member, _signature, _in_names, _result,      \
                                 _out_names, _handler, _flags)                 \
    SD_BUS_METHOD_WITH_NAMES_OFFSET(_member, _signature, _in_names, _result,   \
                                    _out_names, _handler, 0, _flags)
...
#define SD_BUS_WRITABLE_PROPERTY(_member, _signature, _get, _set, _offset,     \
                                 _flags)                                       \
    {                                                                          \
