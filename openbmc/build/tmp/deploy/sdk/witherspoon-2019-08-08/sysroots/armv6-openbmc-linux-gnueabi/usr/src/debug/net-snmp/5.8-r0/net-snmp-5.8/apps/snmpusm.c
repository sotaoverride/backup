...
static const char* successNotes[CMD_NUM] = {
    "SNMPv3 Key(s) successfully changed.", "User successfully created.",
    "User successfully deleted.",          "User successfully cloned.",
    "User successfully activated.",        "User successfully deactivated.",
...
#if defined(HAVE_OPENSSL_DH_H) && defined(HAVE_LIBCRYPTO)
    usmDHUserOwnAuthKeyChange[MAX_OID_LEN] = {1, 3, 6, 1, 3, 101,
                                              1, 1, 2, 1, 2},
    usmDHUserOwnPrivKeyChange[MAX_OID_LEN] = {1, 3, 6, 1, 3, 101,
                                              1, 1, 2, 1, 4},
#endif /* HAVE_OPENSSL_DH_H && HAVE_LIBCRYPTO */
...
void usage(void)
{
    ...
    fprintf(stderr, "  [options] [-Ca] [-Cx]   passwd     OLD-PASSPHRASE "
                    "NEW-PASSPHRASE [USER]\n");
    fprintf(stderr, "  [options] (-Ca|-Cx) -Ck passwd     OLD-KEY-OR-PASS "
                    "NEW-KEY-OR-PASS [USER]\n");
    fprintf(stderr, "\nsnmpusm options:\n");
    fprintf(
        stderr,
 ...
 */
void setup_oid(oid* it, size_t* len, u_char* id, size_t idlen, const char* user)
{
    int i, itIndex = *len;
...
#if defined(HAVE_OPENSSL_DH_H) && defined(HAVE_LIBCRYPTO)
int get_USM_DH_key(netsnmp_variable_list* vars, netsnmp_variable_list* dhvar,
                   size_t outkey_len, netsnmp_pdu* pdu, const char* keyname,
                   oid* keyoid, size_t keyoid_len)
{
    ...
    if (DH_compute_key(key, other_pub, dh))
    {
        ...
        for (kp = key + key_len - outkey_len; kp - key < (int)key_len; kp++)
        {
    ...
    snmp_pdu_add_variable(pdu, keyoid, keyoid_len, ASN_OCTET_STR, dhkeychange,
                          2 * vars->val_len);
            ...
            while (*optarg)
            {
                switch (*optarg++)
                {
                    ...
                    case 'p':
                        ...
                        {
                            fprintf(stderr,
                                    "Bad -Cp option: no argument given\n");
                            exit(1);
                        }
                        optind++;
                        break;
                    ...
                    case 'E':
                    {
                        ...
                        if (optind < argc)
                        {
                            if (argv[optind])
                            {
                                ...
                                if (ebuf == NULL)
                                {
                                    fprintf(stderr, "malloc failure processing "
                                                    "-CE option.\n");
                                    exit(1);
                                }
                                if (!snmp_hex_to_binary(&ebuf, &ebuf_len,
                                                        &usmUserEngineIDLen, 1,
                                                        argv[optind]))
                                {
                                    fprintf(stderr, "Bad usmUserEngineID value "
                                                    "after -CE option.\n");
                                    free(ebuf);
                                    exit(1);
                                }
                                ...
                                DEBUGMSGTL(
                                    ("snmpusm", "usmUserEngineID set to: "));
                                DEBUGMSGHEX(("snmpusm", usmUserEngineID,
                                             usmUserEngineIDLen));
                                DEBUGMSG(("snmpusm", "\n"));
                            }
                        }
                        ...
                        {
                            fprintf(stderr,
                                    "Bad -CE option: no argument given\n");
                            exit(1);
                        }
                        optind++;
                        break;
...
int main(int argc, char* argv[])
{
    ...
    size_t oldKu_len = SNMP_MAXBUF_SMALL, newKu_len = SNMP_MAXBUF_SMALL,
           oldkul_len = SNMP_MAXBUF_SMALL, oldkulpriv_len = SNMP_MAXBUF_SMALL,
           newkulpriv_len = SNMP_MAXBUF_SMALL, newkul_len = SNMP_MAXBUF_SMALL,
           keychange_len = SNMP_MAXBUF_SMALL,
           keychangepriv_len = SNMP_MAXBUF_SMALL;

    char *newpass = NULL, *oldpass = NULL;
    u_char oldKu[SNMP_MAXBUF_SMALL], newKu[SNMP_MAXBUF_SMALL],
        oldkul[SNMP_MAXBUF_SMALL], oldkulpriv[SNMP_MAXBUF_SMALL],
        newkulpriv[SNMP_MAXBUF_SMALL], newkul[SNMP_MAXBUF_SMALL],
        keychange[SNMP_MAXBUF_SMALL], keychangepriv[SNMP_MAXBUF_SMALL];
    ...
    if (strcmp(argv[arg], CMD_PASSWD_NAME) == 0)
    {
        ...
        {
            fprintf(stderr,
                    "New passphrase must be greater than %d characters in "
                    "length.\n",
                    USM_LENGTH_P_MIN);
            goto close_session;
        }
        ...
        {
            fprintf(stderr,
                    "Old passphrase must be greater than %d characters in "
                    "length.\n",
                    USM_LENGTH_P_MIN);
            goto close_session;
        }

        DEBUGMSGTL(("9:usm:passwd",
                    "oldpass len %" NETSNMP_PRIz "d, newpass len %" NETSNMP_PRIz
                    "d\n",
        ...
        {
            session.securityName = strdup(netsnmp_ds_get_string(
        ...
        if (session.securityAuthProto == NULL)
        {
            ...
            session.securityAuthProto = snmp_duplicate_objid(
                usmHMACMD5AuthProtocol, session.securityAuthProtoLen);
#else
            session.securityAuthProtoLen =
                sizeof(usmHMACSHA1AuthProtocol) / sizeof(oid);
            session.securityAuthProto = snmp_duplicate_objid(
                usmHMACSHA1AuthProtocol, session.securityAuthProtoLen);
#endif
        }
        ...
        if (uselocalizedkey && (strncmp(oldpass, "0x", 2) == 0))
        {
            ...
            if (!snmp_hex_to_binary((u_char**)(&buf), &buf_len, &oldkul_len, 0,
                                    oldpass))
            {
                snmp_perror(argv[0]);
                fprintf(stderr,
                        "generating the old Kul from localized key failed\n");
                goto close_session;
            }
        ...
        else
        {
            ...
            rval = generate_Ku(session.securityAuthProto,
                               session.securityAuthProtoLen, (u_char*)oldpass,
                               strlen(oldpass), oldKu, &oldKu_len);
            ...
            rval = generate_kul(session.securityAuthProto,
                                session.securityAuthProtoLen, usmUserEngineID,
                                usmUserEngineIDLen, oldKu, oldKu_len, oldkul,
                                &oldkul_len);
        ...
        if (uselocalizedkey && (strncmp(newpass, "0x", 2) == 0))
        {
            ...
            if (!snmp_hex_to_binary((u_char**)(&buf), &buf_len, &newkul_len, 0,
                                    newpass))
            {
                snmp_perror(argv[0]);
                fprintf(stderr,
                        "generating the new Kul from localized key failed\n");
                goto close_session;
            }
        ...
        {
            rval = generate_Ku(session.securityAuthProto,
                               session.securityAuthProtoLen, (u_char*)newpass,
                               strlen(newpass), newKu, &newKu_len);
            ...
            rval = generate_kul(session.securityAuthProto,
                                session.securityAuthProtoLen, usmUserEngineID,
                                usmUserEngineIDLen, newKu, newKu_len, newkul,
                                &newkul_len);
        ...
        if (doprivkey)
        {
            ...
            {
                snmp_log(LOG_ERR, "no encryption type specified, which I need "
                                  "in order to know to change the key\n");
                goto close_session;
            }
            ...
            {
                rval = netsnmp_extend_kul(
                    properlength, session.securityAuthProto,
                    session.securityAuthProtoLen, privtype, usmUserEngineID,
                    usmUserEngineIDLen, &okp, &oldkulpriv_len,
                    sizeof(oldkulpriv));
                rval = netsnmp_extend_kul(
                    properlength, session.securityAuthProto,
                    session.securityAuthProtoLen, privtype, usmUserEngineID,
                    usmUserEngineIDLen, &nkp, &newkulpriv_len,
                    sizeof(newkulpriv));
            }
        }
        ...
        if (doauthkey)
        {
            rval = encode_keychange(
                session.securityAuthProto, session.securityAuthProtoLen, oldkul,
                oldkul_len, newkul, newkul_len, keychange, &keychange_len);
        ...
        if (doprivkey)
        {
            DEBUGMSGTL(("9:usm:passwd:encode",
                        "proper len %" NETSNMP_PRIz "d, old_len %" NETSNMP_PRIz
                        "d, new_len %" NETSNMP_PRIz "d\n",
                        oldkulpriv_len, oldkulpriv_len, newkulpriv_len));
            rval = encode_keychange(session.securityAuthProto,
                                    session.securityAuthProtoLen, oldkulpriv,
                                    oldkulpriv_len, newkulpriv, newkulpriv_len,
                                    keychangepriv, &keychangepriv_len);

            DEBUGMSGTL(("9:usm:passwd:encode",
                        "keychange len %" NETSNMP_PRIz "d\n",
                        keychangepriv_len));
            if (rval != SNMPERR_SUCCESS)
            {
                snmp_perror(argv[0]);
        ...
        if (doauthkey)
        {
            setup_oid(authKeyChange, &name_length, usmUserEngineID,
                      usmUserEngineIDLen, session.securityName);
            snmp_pdu_add_variable(pdu, authKeyChange, name_length,
                                  ASN_OCTET_STR, keychange, keychange_len);
        }
        if (doprivkey)
        {
            setup_oid(privKeyChange, &name_length2, usmUserEngineID,
                      usmUserEngineIDLen, session.securityName);
            snmp_pdu_add_variable(pdu, privKeyChange, name_length2,
                                  ASN_OCTET_STR, keychangepriv,
                                  keychangepriv_len);
        }
    }
    else if (strcmp(argv[arg], CMD_CREATE_NAME) == 0)
    {
        ...
        if (++arg < argc)
        {
             ...
             */
            setup_oid(usmUserStatus, &name_length, usmUserEngineID,
                      usmUserEngineIDLen, argv[arg - 1]);
            if (docreateandwait)
            {
            ...
            snmp_pdu_add_variable(pdu, usmUserStatus, name_length, ASN_INTEGER,
                                  (u_char*)&longvar, sizeof(longvar));

            name_length = USM_OID_LEN;
            setup_oid(usmUserCloneFrom, &name_length, usmUserEngineID,
                      usmUserEngineIDLen, argv[arg - 1]);
            setup_oid(usmUserSecurityName, &name_length2, usmUserEngineID,
                      usmUserEngineIDLen, argv[arg]);
    ...
    else if (strcmp(argv[arg], CMD_CLONEFROM_NAME) == 0)
    {
        ...
        setup_oid(usmUserSecurityName, &name_length2, usmUserEngineID,
                  usmUserEngineIDLen, argv[arg]);
        snmp_pdu_add_variable(pdu, usmUserCloneFrom, name_length, ASN_OBJECT_ID,
    ...
    else if (strcmp(argv[arg], CMD_CHANGEKEY_NAME) == 0)
    {
        ...
        if (doauthkey)
        {
            setup_oid(dhauthKeyChange, &name_length, usmUserEngineID,
                      usmUserEngineIDLen, session.securityName);
        ...
        if (doprivkey)
        {
            setup_oid(dhprivKeyChange, &name_length2, usmUserEngineID,
                      usmUserEngineIDLen, session.securityName);
            ...
            dhresponse->variables->type != ASN_OCTET_STR)
        {
                ...
                dhresponse->variables->type != ASN_OCTET_STR)
            {
                ...
                fprintf(stderr, "  (maybe it doesn't support the "
        ...
        if (doauthkey)
        {
            if (get_USM_DH_key(vars, dhvar,
                               ...
                               pdu, "auth", dhauthKeyChange,
                               name_length) != SNMPERR_SUCCESS)
                goto begone;
        ...
        if (doprivkey)
        {
            ...
            if (get_USM_DH_key(vars, dhvar, dhprivKeyLen, pdu, "priv",
                               dhprivKeyChange,
                               name_length2) != SNMPERR_SUCCESS)
                goto begone;
            vars = vars->next_variable;
        }
    ...
    {
        name_length = USM_OID_LEN;
        setup_oid(usmUserPublic, &name_length, usmUserEngineID,
                  usmUserEngineIDLen, session.securityName);
        snmp_pdu_add_variable(pdu, usmUserPublic, name_length, ASN_OCTET_STR,
                              usmUserPublic_val, strlen(usmUserPublic_val));
    }
