...
netsnmp_feature_require(set_agent_uptime)
    ...
    static int agentx_synch_input(int op, netsnmp_session* session, int reqid,
...
static int agentx_synch_response(netsnmp_session* ss, netsnmp_pdu* pdu,
...
int agentx_open_session(netsnmp_session* ss)
{
...
int agentx_close_session(netsnmp_session* ss, int why)
{
    netsnmp_pdu *pdu, *response;
...
int agentx_register(netsnmp_session* ss, oid start[], size_t startlen,
                    ...
                    int timeout, u_char flags, const char* contextName)
{
    ...
    DEBUGMSGOIDRANGE(
        ("agentx/subagent", start, startlen, range_subid, range_ubound));
...
int agentx_unregister(netsnmp_session* ss, oid start[], size_t startlen,
                      int priority, int range_subid, oid range_ubound,
...
netsnmp_variable_list* agentx_register_index(netsnmp_session* ss,
...
int agentx_unregister_index(netsnmp_session* ss, netsnmp_variable_list* varbind)
{
    netsnmp_pdu *pdu, *response;
    netsnmp_variable_list* varbind2;
...
int agentx_add_agentcaps(netsnmp_session* ss, const oid* agent_cap,
                         size_t agent_caplen, const char* descr)
{
    netsnmp_pdu *pdu, *response;
...
int agentx_remove_agentcaps(netsnmp_session* ss, const oid* agent_cap,
                            size_t agent_caplen)
{
    netsnmp_pdu *pdu, *response;
...
int agentx_send_ping(netsnmp_session* ss)
{
    netsnmp_pdu *pdu, *response;
