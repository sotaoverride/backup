...
/* note: v4, v5 are already used by more recent versions, so we need to skip
 * them! */
/* added v6, 2009-11-16 as part of varmojfekoj's "unlimited select()" patch
 * Note that it must be always present, otherwise the interface would have
 * different versions depending on compile settings, what is not acceptable. Use
 * this property with care, it is only truly available if UNLIMITED_SELECT is
 * enabled (I did not yet further investigate the details, because that code
 * hopefully can be removed at some later stage).
 */
