...
static krb5_error_code process_chpw_request(krb5_context context,
                                            ...
                                            krb5_data* req, krb5_data* rep)
{
    ...
    *rep = empty_data();
    ...
    ret = krb5_rd_req(context, &auth_context, &ap_req, changepw, keytab, NULL,
                      &ticket);
    ...
    if (getnameinfo(ss2sa(&ss), salen, addrbuf, sizeof(addrbuf), NULL, 0,
    ...
    if (vno == RFC3244_VERSION)
    {
        ...
        krb5_klog_syslog(LOG_NOTICE,
                         _("setpw request from %s by %.*s%s for "
                           "%.*s%s: %s"),
                         addrbuf, (int)clen, clientstr, cdots, (int)tlen,
                         targetp, tdots, errmsg ? errmsg : "success");
    }
    else
    {
...
chpwfail:
    ...
    *ptr++ = numresult & 0xff;
    ...
    if (ap_rep.length)
    {
        ret = krb5_auth_con_setaddrs(context, auth_context, local_addr->address,
                                     NULL);
        if (ret)
        {
            numresult = KRB5_KPASSWD_HARDERROR;
    ...
    if (cipher.length == 0)
    {
        ...
        ret = krb5_build_principal(context, &krberror.server, strlen(realm),
              ...
              verto_ctx* vctx, loop_respond_fn respond, void* arg)
{
    ...
    ret = process_chpw_request(server_handle->context, handle,
                               server_handle->params.realm, kt, local_addr,
