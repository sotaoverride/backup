...
static int netsnmp_internal_asc2bin(char* p)
{
    char *r, *q = p;
    char c;
    int n = 0;
...
static int netsnmp_internal_bin2asc(char* p, size_t n)
{
    int i, flag = 0;
...
unsigned char* netsnmp_internal_pass_parse(char* buf, char* buf2,
                                           size_t* var_len, struct variable* vp)
{
    ...
    else if (!strncasecmp(buf, "ipaddress", 9))
    {
        ...
        addr_ret = (objid[0] << (8 * 3)) + (objid[1] << (8 * 2)) +
                   (objid[2] << 8) + objid[3];
...
void netsnmp_internal_pass_set_format(char* buf, const u_char* var_val,
    ...
    switch (var_val_type)
    {
        ...
        case ASN_IPADDRESS:
            ...
            sprintf(buf, "ipaddress %d.%d.%d.%d\n",
                    ...
                    (int)((utmp & 0xff00) >> (8)), (int)((utmp & 0xff)));
            break;
        case ASN_OCTET_STR:
            ...
            break;
        case ASN_OBJECT_ID:
            sprint_mib_oid(buf2, (const oid*)var_val,
