...
static int scan_mpi(gcry_mpi_t retval, const char* string)
{
    gpg_error_t err;
    gcry_mpi_t val;
...
static void print_mpi(gcry_mpi_t a)
{
    gpg_error_t err;
    char* buf;
    void* bufaddr = &buf;
...
static void do_mulm(void)
{
    ...
    mpi_mulm(stack[stackidx - 3], stack[stackidx - 3], stack[stackidx - 2],
             stack[stackidx - 1]);
    stackidx -= 2;
}
...
static void do_div(void)
{
    ...
    mpi_fdiv(stack[stackidx - 2], NULL, stack[stackidx - 2],
             stack[stackidx - 1]);
    stackidx--;
}
...
static void do_rem(void)
{
    ...
    mpi_mod(stack[stackidx - 2], stack[stackidx - 2], stack[stackidx - 1]);
    stackidx--;
}

static void do_powm(void)
{
    gcry_mpi_t a;
    if (stackidx < 3)
    {
...
static void do_primecheck(void)
{
    gpg_error_t err;
...
int main(int argc, char** argv)
{
    ...
    while (argc && last_argc != argc)
    {
        ...
        else if (!strcmp(*argv, "--version") || !strcmp(*argv, "--help"))
        {
