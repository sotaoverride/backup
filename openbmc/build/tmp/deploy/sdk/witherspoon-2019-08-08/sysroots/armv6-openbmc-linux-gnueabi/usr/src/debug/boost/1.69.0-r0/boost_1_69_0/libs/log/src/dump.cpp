...
#include <ostream>
#if defined(_MSC_VER) &&                                                       \
    (defined(BOOST_LOG_USE_SSSE3) || defined(BOOST_LOG_USE_AVX2))
...
void dump_data_generic(const void* data, std::size_t size,
                       std::basic_ostream<CharT>& strm)
{
    ...
    const char* const char_table =
        g_hex_char_table[(strm.flags() & std::ios_base::uppercase) != 0];
...
#if !defined(BOOST_NO_CXX11_CHAR16_T)
BOOST_LOG_API dump_data_char16_t* dump_data_char16 =
    &dump_data_generic<char16_t>;
#endif
#if !defined(BOOST_NO_CXX11_CHAR32_T)
BOOST_LOG_API dump_data_char32_t* dump_data_char32 =
    &dump_data_generic<char32_t>;
#endif
    ...
    struct function_pointer_initializer
    {
        function_pointer_initializer()
        {
            ...
            if (max_cpuid_function >= 1)
            {
                ...
                {
                    // To check for AVX2 availability we also need to verify
                    // that OS supports it Check that OSXSAVE is supported by
                    // CPU
                    if (ecx & (1u << 27))
                    {
                        ...
                        // MSVC does not have an intrinsic for xgetbv, we have
                        // to query OS
                        boost::winapi::HMODULE_ hKernel32 =
                            boost::winapi::GetModuleHandleW(L"kernel32.dll");
                        if (hKernel32)
                        {
                            typedef uint64_t(BOOST_WINAPI_WINAPI_CC *
                                             get_enabled_extended_features_t)(
                                uint64_t);
                            get_enabled_extended_features_t
                                get_enabled_extended_features =
                                    (get_enabled_extended_features_t)
                            ...
                            {
                                // XSTATE_MASK_LEGACY_SSE | XSTATE_MASK_GSSE ==
                                // 6
                                mmstate =
      ...
      private:
        ...
        {
#if defined(__GNUC__)
#if (defined(__i386__) || defined(__VXWORKS__)) &&                             \
    (defined(__PIC__) || defined(__PIE__)) &&                                  \
    !(defined(__clang__) || (defined(BOOST_GCC) && BOOST_GCC >= 50100))
            // Unless the compiler can do it automatically, we have to backup
            // ebx in 32-bit PIC/PIE code because it is reserved by the ABI. For
            // VxWorks ebx is reserved on 64-bit as well.
#if defined(__x86_64__)
            uint64_t rbx = ebx;
            __asm__ __volatile__("xchgq %%rbx, %0\n\t"
                                 "cpuid\n\t"
                                 "xchgq %%rbx, %0\n\t"
