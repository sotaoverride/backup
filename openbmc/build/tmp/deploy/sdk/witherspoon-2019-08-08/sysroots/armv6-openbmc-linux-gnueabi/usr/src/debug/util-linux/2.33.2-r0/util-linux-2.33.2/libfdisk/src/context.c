 ...
 *      any change to the object has no effect to in-memory (or on-disk) label
 * data
 */
...
struct fdisk_label* fdisk_get_label(struct fdisk_context* cxt, const char* name)
{
    ...
    for (i = 0; i < cxt->nlabels; i++)
        if (cxt->labels[i] && strcasecmp(cxt->labels[i]->name, name) == 0)
            return cxt->labels[i];
...
int __fdisk_switch_label(struct fdisk_context* cxt, struct fdisk_label* lb)
{
    ...
    if (lb->disabled)
    {
        DBG(CXT,
            ul_debugobj(cxt,
                        "*** attempt to switch to disabled label %s -- ignore!",
                        lb->name));
        return -EINVAL;
    }
    cxt->label = lb;
                        ...
                        int readonly)
{
    ...
    DBG(CXT, ul_debugobj(cxt, "initialized for %s [%s]", fname,
...
int fdisk_reread_partition_table(struct fdisk_context* cxt)
{
    ...
    if (i)
    {
        ...
        fdisk_info(cxt, _("The kernel still uses the old table. The "
...
#ifdef __linux__
static inline int add_to_partitions_array(struct fdisk_partition*** ary,
...
int fdisk_reread_changes(struct fdisk_context* cxt, struct fdisk_table* org)
{
    ...
    for (i = 0; i < nrems; i++)
    {
        pa = rem[i];
        DBG(PART,
            ul_debugobj(pa, "#%zu calling BLKPG_DEL_PARTITION", pa->partno));
        if (partx_del_partition(cxt->dev_fd, pa->partno + 1) != 0)
        {
            fdisk_warn(cxt, _("Failed to remove partition %zu from system"),
                       pa->partno + 1);
            err++;
        }
    }
    for (i = 0; i < nupds; i++)
    {
        pa = upd[i];
        DBG(PART,
            ul_debugobj(pa, "#%zu calling BLKPG_RESIZE_PARTITION", pa->partno));
        if (partx_resize_partition(cxt->dev_fd, pa->partno + 1, pa->start,
                                   pa->size) != 0)
        {
            fdisk_warn(
                cxt,
                _("Failed to update system information about partition %zu"),
                pa->partno + 1);
            err++;
        }
    }
    for (i = 0; i < nadds; i++)
    {
        pa = add[i];
        DBG(PART,
            ul_debugobj(pa, "#%zu calling BLKPG_ADD_PARTITION", pa->partno));
        if (partx_add_partition(cxt->dev_fd, pa->partno + 1, pa->start,
                                pa->size) != 0)
        {
            fdisk_warn(cxt, _("Failed to add partition %zu to system"),
                       pa->partno + 1);
            err++;
        }
    }
    if (err)
        fdisk_info(cxt, _("The kernel still uses the old partitions. The new "
...
int fdisk_device_is_used(struct fdisk_context* cxt)
{
    ...
    errno = ENOSYS;
#endif
    DBG(CXT, ul_debugobj(cxt, "device used: %s [errno=%d]",
                         rc ? "TRUE" : "FALSE", errno));
    return rc;
}
 ...
 * Since: 2.30
 *
 * Returns: 1 if open file descriptor is regular file rather than a block
 * device.
 */
...
void fdisk_unref_context(struct fdisk_context* cxt)
{
    ...
    {
        DBG(CXT,
            ul_debugobj(cxt, "freeing context %p for %s", cxt, cxt->dev_path));

        reset_context(
            cxt); /* this is sensitive to parent<->child relationship! */
 ...
 *
 * Returns: number of "units" per sector, default is 1 if display unit is
 * sector.
 */
...
const char* fdisk_get_devmodel(struct fdisk_context* cxt)
{
    ...
    {
        struct path_cxt* pc =
            ul_new_sysfs_path(fdisk_get_devno(cxt), NULL, NULL);
