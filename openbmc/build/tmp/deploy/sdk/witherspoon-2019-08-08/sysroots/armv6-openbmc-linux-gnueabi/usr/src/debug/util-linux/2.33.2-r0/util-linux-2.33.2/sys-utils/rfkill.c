...
/*
 * NFC supported by kernel since v3.10 (year 2013); FM and another types are
 * from year 2009 (2.6.33) or older.
 */
...
static char* rfkill_actions[] = {[ACT_LIST] = "list",
                                 [ACT_HELP] = "help",
                                 [ACT_EVENT] = "event",
                                 [ACT_BLOCK] = "block",
                                 [ACT_UNBLOCK] = "unblock"};
...
static const struct colinfo infos[] = {
    ...
    [COL_TYPE] = {"TYPE", 0, 0,
                  N_("device type name that can be used as identifier")},
...
struct control
{
    ...
    unsigned int json : 1, no_headings : 1, raw : 1;
};
...
static int rfkill_read_event(int fd, struct rfkill_event* event)
{
    ...
    {
        warnx(_("wrong size of rfkill event: %zu < %d"), len,
...
static int rfkill_event(void)
{
    ...
    while (1)
    {
        ...
        strtimeval_iso(&tv, ISO_TIMESTAMP_COMMA, date_buf, sizeof(date_buf));
...
static void fill_table_row(struct libscols_table* tb,
                           struct rfkill_event* event)
{
...
static void rfkill_list_init(struct control* ctrl)
{
    ...
    for (i = 0; i < (size_t)ncolumns; i++)
    {
        ...
        cl =
            scols_table_new_column(ctrl->tb, col->name, col->whint, col->flags);
        if (!cl)
...
{
    struct rfkill_id id;
    struct rfkill_event event = {.op = RFKILL_OP_CHANGE_ALL, .soft = block, 0};
    ssize_t len;
    int fd;
    char* message = NULL;
    ...
    else
    {
        ...
        syslog(LOG_NOTICE, "%s set for %s", block ? "block" : "unblock",
               message);
        closelog();
    }
    free(message);
...
int main(int argc, char** argv)
{
    ...
    static const struct option longopts[] = {
        ...
        {NULL, 0, NULL, 0}};
    static const ul_excl_t excl[] = {{'J', 'r'}, {0}};
