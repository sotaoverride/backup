...
static gcry_err_code_t do_twofish_setkey(TWOFISH_context* ctx, const byte* key,
...
static gcry_err_code_t twofish_setkey(void* context, const byte* key,
                                      unsigned int keylen)
{
...
static inline void twofish_amd64_ctr_enc(const TWOFISH_context* c, byte* out,
                                         const byte* in, byte* ctr)
{
#ifdef HAVE_COMPATIBLE_GCC_WIN64_PLATFORM_AS
...
static inline void twofish_amd64_cbc_dec(const TWOFISH_context* c, byte* out,
                                         const byte* in, byte* iv)
{
#ifdef HAVE_COMPATIBLE_GCC_WIN64_PLATFORM_AS
...
static inline void twofish_amd64_cfb_dec(const TWOFISH_context* c, byte* out,
                                         const byte* in, byte* iv)
{
#ifdef HAVE_COMPATIBLE_GCC_WIN64_PLATFORM_AS
...
static inline void twofish_amd64_ocb_enc(const TWOFISH_context* ctx, byte* out,
                                         const byte* in, byte* offset,
                                         byte* checksum, const u64 Ls[3])
{
#ifdef HAVE_COMPATIBLE_GCC_WIN64_PLATFORM_AS
...
static inline void twofish_amd64_ocb_dec(const TWOFISH_context* ctx, byte* out,
                                         const byte* in, byte* offset,
                                         byte* checksum, const u64 Ls[3])
{
#ifdef HAVE_COMPATIBLE_GCC_WIN64_PLATFORM_AS
...
/* Assembly implementations of Twofish. */
extern void _gcry_twofish_arm_encrypt_block(const TWOFISH_context* c, byte* out,
                                            const byte* in);

extern void _gcry_twofish_arm_decrypt_block(const TWOFISH_context* c, byte* out,
                                            const byte* in);
...
static unsigned int twofish_encrypt(void* context, byte* out, const byte* in)
{
    TWOFISH_context* ctx = context;
...
static void do_twofish_encrypt(const TWOFISH_context* ctx, byte* out,
                               const byte* in)
{
...
static unsigned int twofish_decrypt(void* context, byte* out, const byte* in)
{
    TWOFISH_context* ctx = context;
...
static void do_twofish_decrypt(const TWOFISH_context* ctx, byte* out,
                               const byte* in)
{
    /* The four 32-bit chunks of the text. */
    u32 a, b, c, d;
...
static const char* selftest(void)
{
    ...
    static byte plaintext[16] = {0xD4, 0x91, 0xDB, 0x16, 0xE7, 0xB1,
                                 0xC3, 0x9E, 0x86, 0xCB, 0x08, 0x6B,
                                 0x78, 0x9F, 0x54, 0x19};
    static byte key[16] = {0x9F, 0x58, 0x9F, 0x5C, 0xF6, 0x12, 0x2C, 0x32,
                           0xB6, 0xBF, 0xEC, 0x2F, 0x2A, 0xE8, 0xC3, 0x5A};
    static const byte ciphertext[16] = {0x01, 0x9F, 0x98, 0x09, 0xDE, 0x17,
                                        0x11, 0x85, 0x8F, 0xAA, 0xC3, 0xA3,
                                        0xBA, 0x20, 0xFB, 0xC3};
    static byte plaintext_256[16] = {0x90, 0xAF, 0xE9, 0x1B, 0xB2, 0x88,
                                     0x54, 0x4F, 0x2C, 0x32, 0xDC, 0x23,
                                     0x9B, 0x26, 0x35, 0xE6};
    static byte key_256[32] = {0xD4, 0x3B, 0xB7, 0x55, 0x6E, 0xA3, 0x2E, 0x46,
                               0xF2, 0xA2, 0x82, 0xB7, 0xD4, 0x5B, 0x4E, 0x0D,
                               0x57, 0xFF, 0x73, 0x9D, 0x4D, 0xC9, 0x2C, 0x1B,
                               0xD7, 0xFC, 0x01, 0x70, 0x0C, 0xC8, 0x21, 0x6F};
    static const byte ciphertext_256[16] = {0x6C, 0xB4, 0x56, 0x1C, 0x40, 0xBF,
                                            0x0A, 0x97, 0x05, 0x93, 0x1C, 0xB6,
                                            0xD4, 0x08, 0xE7, 0xFA};
...
int main()
{
    ...
    /* Test buffer. */
    byte buffer[4][16] = {{0x00, 0x11, 0x22, 0x33, 0x44, 0x55, 0x66, 0x77, 0x88,
                           0x99, 0xAA, 0xBB, 0xCC, 0xDD, 0xEE, 0xFF},
                          {0x0F, 0x1E, 0x2D, 0x3C, 0x4B, 0x5A, 0x69, 0x78, 0x87,
                           0x96, 0xA5, 0xB4, 0xC3, 0xD2, 0xE1, 0xF0},
                          {0x01, 0x23, 0x45, 0x67, 0x89, 0xAB, 0xCD, 0xEF, 0xFE,
                           0xDC, 0xBA, 0x98, 0x76, 0x54, 0x32, 0x10},
                          {0x01, 0x23, 0x45, 0x67, 0x76, 0x54, 0x32, 0x10, 0x89,
                           0xAB, 0xCD, 0xEF, 0xFE, 0xDC, 0xBA, 0x98}};

    /* Expected outputs for the million-operation test */
    static const byte test_encrypt[4][16] = {
        {0xC8, 0x23, 0xB8, 0xB7, 0x6B, 0xFE, 0x91, 0x13, 0x2F, 0xA7, 0x5E, 0xE6,
         0x94, 0x77, 0x6F, 0x6B},
        {0x90, 0x36, 0xD8, 0x29, 0xD5, 0x96, 0xC2, 0x8E, 0xE4, 0xFF, 0x76, 0xBC,
         0xE5, 0x77, 0x88, 0x27},
        {0xB8, 0x78, 0x69, 0xAF, 0x42, 0x8B, 0x48, 0x64, 0xF7, 0xE9, 0xF3, 0x9C,
         0x42, 0x18, 0x7B, 0x73},
        {0x7A, 0x88, 0xFB, 0xEB, 0x90, 0xA4, 0xB4, 0xA8, 0x43, 0xA3, 0x1D, 0xF1,
         0x26, 0xC4, 0x53, 0x57}};
    static const byte test_decrypt[4][16] = {
        ...
        {0x01, 0x23, 0x45, 0x67, 0x89, 0xAB, 0xCD, 0xEF, 0xFE, 0xDC, 0xBA, 0x98,
         0x76, 0x54, 0x32, 0x10},
        {0x01, 0x23, 0x45, 0x67, 0x76, 0x54, 0x32, 0x10, 0x89, 0xAB, 0xCD, 0xEF,
         0xFE, 0xDC, 0xBA, 0x98}};
...
gcry_cipher_spec_t _gcry_cipher_spec_twofish = {GCRY_CIPHER_TWOFISH,
                                                {0, 0},
                                                "TWOFISH",
                                                NULL,
                                                NULL,
...
gcry_cipher_spec_t _gcry_cipher_spec_twofish128 = {GCRY_CIPHER_TWOFISH128,
                                                   {0, 0},
                                                   "TWOFISH128",
                                                   NULL,
                                                   NULL,
