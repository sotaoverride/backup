...
struct color_traits<two_bit_color_type>
{
    ...
    {
        return two_bit_green;
    }
    static two_bit_color_type black()
    {
        return two_bit_black;
    }
};
...
struct two_bit_color_map
{
    ...
    explicit two_bit_color_map(std::size_t n,
        ...
        data(new unsigned char[(n + elements_per_char - 1) / elements_per_char])
    {
        // Fill to white
        std::fill(data.get(),
                  data.get() + (n + elements_per_char - 1) / elements_per_char,
                  0);
    }
};
                              ...
                              typename property_traits<IndexMap>::key_type key)
{
    BOOST_STATIC_CONSTANT(int,
                          elements_per_char =
                              two_bit_color_map<IndexMap>::elements_per_char);
