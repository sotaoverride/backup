...
#define BUS_SERVER_DATA(server)                                                \
    (dbus_server_get_data((server), server_data_slot))
                                    ...
                                    DBusConnection* new_connection, void* data)
{
    ...
    dbus_connection_set_max_received_unix_fds(
        new_connection, context->limits.max_incoming_unix_fds);

    dbus_connection_set_max_message_unix_fds(
        new_connection, context->limits.max_message_unix_fds);
...
static dbus_bool_t setup_server(BusContext* context, DBusServer* server,
                                char** auth_mechanisms, DBusError* error)
{
    BusServerData* bd;

    bd = dbus_new0(BusServerData, 1);
    if (bd == NULL ||
        !dbus_server_set_data(server, server_data_slot, bd, free_server_data))
    {
        dbus_free(bd);
        BUS_SET_OOM(error);
        return FALSE;
    ...
    if (!dbus_server_set_watch_functions(server, add_server_watch,
                                         remove_server_watch,
                                         toggle_server_watch, server, NULL))
    {
        BUS_SET_OOM(error);
        return FALSE;
    }

    if (!dbus_server_set_timeout_functions(server, add_server_timeout,
                                           remove_server_timeout, NULL, server,
                                           NULL))
    {
        BUS_SET_OOM(error);
        return FALSE;
 ...
 */
static dbus_bool_t process_config_first_time_only(BusContext* context,
                                                  ...
                                                  DBusError* error)
{
        ...
        if (_dbus_stat(&u, &stbuf, NULL))
        {
            ...
            else
            {
                ...
                dbus_set_error(error, DBUS_ERROR_FAILED,
                               "The pid file \"%s\" exists, if the message bus "
                               "is not running, remove this file",
                               pidfile);
                goto failed;
#ifdef DBUS_CYGWIN
            }
        ...
        while (link != NULL)
        {
            ...
            if (!_dbus_auth_is_supported_mechanism(&name))
            {
                ...
                dbus_set_error(
                    ...
                    "Unsupported auth mechanism \"%s\" in bus config file "
                    "detected. Supported mechanisms are \"%s\".",
    ...
    if (address)
    {
        ...
        server =
            dbus_server_listen(_dbus_string_get_const_data(address), error);
        if (server == NULL)
        {
 ...
 */
static dbus_bool_t process_config_every_time(BusContext* context,
                                             BusConfigParser* parser,
                                             ...
                                             DBusError* error)
{
    ...
    {
        if (!bus_activation_reload(context->activation, &full_address, dirs,
                                   error))
            goto failed;
    }
    else
    {
        context->activation =
            bus_activation_new(context, &full_address, dirs, error);
    }
...
static void raise_file_descriptor_limit(BusContext* context)
{
    ...
    if (context->initial_fd_limit == NULL)
    {
        bus_context_log(context, DBUS_SYSTEM_LOG_WARNING, "%s: %s", error.name,
                        error.message);
...
static dbus_bool_t process_config_postinit(BusContext* context,
                                           BusConfigParser* parser,
                                           DBusError* error)
{
    ...
    service_context_table =
        bus_config_parser_steal_service_context_table(parser);
...
BusContext* bus_context_new(const DBusString* config_file,
                            BusContextFlags flags, DBusPipe* print_addr_pipe,
                            DBusPipe* print_pid_pipe, const DBusString* address,
                            DBusError* error)
{
    BusContext* context;
    BusConfigParser* parser;
...
{
    if (server == NULL || !dbus_server_get_is_connected(server))
        return;

    if (!dbus_server_set_watch_functions(server, NULL, NULL, NULL, context,
                                         NULL))
        _dbus_assert_not_reached("setting watch functions to NULL failed");

    if (!dbus_server_set_timeout_functions(server, NULL, NULL, NULL, context,
                                           NULL))
...
void bus_context_log(BusContext* context, DBusSystemLogSeverity severity,
                     const char* msg, ...)
{
    va_list args;
...
void bus_context_log_literal(BusContext* context,
                             DBusSystemLogSeverity severity, const char* msg)
{
...
void bus_context_log_and_set_error(BusContext* context,
                                   ...
                                   DBusError* error, const char* name,
                                   const char* msg, ...)
{
 ...
 */
static void complain_about_message(BusContext* context, const char* error_name,
                                   const char* complaint, int matched_rules,
                                   DBusMessage* message, DBusConnection* sender,
                                   DBusConnection* proposed_recipient,
                                   dbus_bool_t requested_reply, dbus_bool_t log,
                                   DBusError* error)
{
    DBusError stack_error = DBUS_ERROR_INIT;
    ...
    dbus_set_error(
        ...
        complaint, matched_rules,
        dbus_message_type_to_string(dbus_message_get_type(message)),
        sender_name, sender_loginfo,
...
dbus_bool_t bus_context_check_security_policy(
    BusContext* context, BusTransaction* transaction, DBusConnection* sender,
    DBusConnection* addressed_recipient, DBusConnection* proposed_recipient,
    DBusMessage* message, BusActivationEntry* activation_entry,
    DBusError* error)
{
    const char *src, *dest;
    ...
    _dbus_assert(type == DBUS_MESSAGE_TYPE_SIGNAL ||
                 addressed_recipient != NULL || activation_entry != NULL ||
    ...
    switch (type)
    {
        ...
        default:
            _dbus_verbose(
                "security check disallowing message of unknown type %d\n",
                type);
    ...
    if (sender != NULL)
    {
        if (bus_connection_is_active(sender))
        {
            ...
            if (dbus_message_get_reply_serial(message) != 0)
            {
                if (proposed_recipient != NULL /* not to the bus driver */ &&
                    addressed_recipient ==
                        proposed_recipient /* not eavesdropping */)
                {
                    DBusError error2;

                    dbus_error_init(&error2);
                    requested_reply = bus_connections_check_reply(
                        bus_connection_get_connections(sender), transaction,
                        sender, addressed_recipient, message, &error2);
        ...
        if (!bus_selinux_allows_send(
                sender, proposed_recipient,
                dbus_message_type_to_string(dbus_message_get_type(message)),
                dbus_message_get_interface(message),
                dbus_message_get_member(message),
                dbus_message_get_error_name(message),
                dest ? dest : DBUS_SERVICE_DBUS, activation_entry, error))
        {
            if (error != NULL && !dbus_error_is_set(error))
            {
                ...
                complain_about_message(
                    context, DBUS_ERROR_ACCESS_DENIED,
                    "An SELinux policy prevents this sender from sending this "
                    "message to this recipient",
                    0, message, sender, proposed_recipient, FALSE, FALSE,
                    error);
                _dbus_verbose(
        ...
        if (!bus_apparmor_allows_send(
                sender, proposed_recipient, requested_reply,
                bus_context_get_type(context), dbus_message_get_type(message),
                dbus_message_get_path(message),
                dbus_message_get_interface(message),
                dbus_message_get_member(message),
                dbus_message_get_error_name(message),
                dest ? dest : DBUS_SERVICE_DBUS, src ? src : DBUS_SERVICE_DBUS,
        ...
        if (!bus_connection_is_active(sender))
        {
             ...
             */
            if (proposed_recipient == NULL &&
                dbus_message_is_method_call(message, DBUS_INTERFACE_DBUS,
                                            "Hello"))
            {
            ...
            else
            {
                ...
                dbus_set_error(error, DBUS_ERROR_ACCESS_DENIED,
                               "Client tried to send a message other than %s "
    ...
    if (proposed_recipient != NULL)
    {
        ...
        {
            _dbus_verbose("security check using NULL recipient policy for "
                          "message from bus\n");
        ...
        else
        {
            _dbus_assert_not_reached(
                "a message was somehow sent to an inactive recipient from a "
                "source other than the message bus");
            recipient_policy = NULL;
        }
    }
    else
    ...
    _dbus_assert((proposed_recipient != NULL && recipient_policy != NULL) ||
                 (proposed_recipient != NULL && sender == NULL &&
                  recipient_policy == NULL) ||
                 (proposed_recipient == NULL && recipient_policy == NULL));

    log = FALSE;
    if (sender_policy && !bus_client_policy_check_can_send(
                             sender_policy, context->registry, requested_reply,
                             proposed_recipient, message, &toggles, &log))
    {
        complain_about_message(
            context, DBUS_ERROR_ACCESS_DENIED, "Rejected send message", toggles,
            message, sender, proposed_recipient, requested_reply,
            (addressed_recipient == proposed_recipient), error);
        _dbus_verbose(
            "security policy disallowing message due to sender policy\n");
        return FALSE;
    }
    ...
    if (log)
    {
        ...
        complain_about_message(context, DBUS_ERROR_ACCESS_DENIED,
                               "Would reject message", toggles, message, sender,
                               proposed_recipient, requested_reply, TRUE, NULL);
    }

    if (recipient_policy &&
        !bus_client_policy_check_can_receive(
            recipient_policy, context->registry, requested_reply, sender,
            addressed_recipient, proposed_recipient, message, &toggles))
    {
        complain_about_message(
            context, DBUS_ERROR_ACCESS_DENIED, "Rejected receive message",
    ...
    if (proposed_recipient &&
        ((dbus_connection_get_outgoing_size(proposed_recipient) >
         ...
         (dbus_connection_get_outgoing_unix_fds(proposed_recipient) >
          context->limits.max_outgoing_unix_fds)))
    {
        complain_about_message(context, DBUS_ERROR_LIMITS_EXCEEDED,
                               "Rejected: destination has a full message queue",
                               0, message, sender, proposed_recipient,
                               requested_reply, TRUE, error);
        _dbus_verbose(
            "security policy disallowing message due to full message queue\n");
        return FALSE;
    }
     ...
     */
    if (type == DBUS_MESSAGE_TYPE_METHOD_CALL && sender &&
        addressed_recipient &&
        ...
        !bus_connections_expect_reply(bus_connection_get_connections(sender),
                                      transaction, sender, addressed_recipient,
                                      message, error))
    {
        _dbus_verbose("Failed to record reply expectation or problem with the "
