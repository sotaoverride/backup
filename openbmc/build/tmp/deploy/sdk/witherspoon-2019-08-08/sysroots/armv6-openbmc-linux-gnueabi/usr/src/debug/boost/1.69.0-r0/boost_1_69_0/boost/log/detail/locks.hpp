...
// Forward declaration of Boost.Thread locks. Specified here to avoid including
// Boost.Thread, which would bring in many dependent headers, including a great
// deal of Boost.DateTime.
template <typename>
class lock_guard;
template <typename>
...
//! An auxiliary pseudo-lock to express no locking requirements in logger
//! features
...
class no_lock
{
  public:
    /*!
     * Constructs the pseudo-lock. The mutex is not affected during the
     * construction.
     */
...
struct is_exclusively_lockable
{
    ...
    template <typename T>
    static true_type check_lockable(T*, void (T::*)() = &T::lock,
                                    void (T::*)() = &T::unlock);
    static false_type check_lockable(void*);

    enum value_t
    {
        value = sizeof(check_lockable((MutexT*)NULL)) == sizeof(true_type)
    };
};
...
struct is_shared_lockable
{
    ...
    template <typename T>
    static true_type check_shared_lockable(T*, void (T::*)() = &T::lock_shared,
                                           void (T::*)() = &T::unlock_shared);
    static false_type check_shared_lockable(void*);

    enum value_t
    {
        value =
            sizeof(check_shared_lockable((MutexT*)NULL)) == sizeof(true_type)
    };
};
...
struct exclusive_auto_unlocker
{
    ...
    BOOST_DELETED_FUNCTION(
        exclusive_auto_unlocker(exclusive_auto_unlocker const&))
    BOOST_DELETED_FUNCTION(
        exclusive_auto_unlocker& operator=(exclusive_auto_unlocker const&))

  protected:
    MutexT& m_Mutex;
};

//! An analogue to the minimalistic \c lock_guard template. Defined here to
//! avoid including Boost.Thread.
template <typename MutexT>
...
//! An analogue to the minimalistic \c lock_guard template that locks \c
//! shared_mutex with shared ownership.
template <typename MutexT>
...
{
  public:
    multiple_unique_lock2(MutexT1& m1, MutexT2& m2) : m_p1(&m1), m_p2(&m2)
    {
