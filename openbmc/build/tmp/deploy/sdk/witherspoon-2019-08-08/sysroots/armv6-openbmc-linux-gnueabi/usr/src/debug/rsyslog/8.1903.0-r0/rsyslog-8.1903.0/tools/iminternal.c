...
rsRetVal iminternalAddMsg(smsg_t* pMsg)
{
    ...
    {
        dbgprintf(
            "iminternalAddMsg: timedlock for mutex failed with %d, msg %s\n", r,
            getMSG(pMsg));
...
finalize_it:
    ...
    if (iRet != RS_RET_OK)
    {
        dbgprintf("iminternalAddMsg() error %d - can not otherwise report this "
                  "error, message lost\n",
                  iRet);
        if (pThis != NULL)
...
rsRetVal iminternalRemoveMsg(smsg_t** ppMsg)
{
    ...
    *ppMsg = pThis->pMsg;
    ...
    {
        dbgprintf(
            "Root element of iminternal linked list could not be destroyed - "
            "there is "
            "nothing we can do against it, we ignore it for now. Things may go "
            "wild "
