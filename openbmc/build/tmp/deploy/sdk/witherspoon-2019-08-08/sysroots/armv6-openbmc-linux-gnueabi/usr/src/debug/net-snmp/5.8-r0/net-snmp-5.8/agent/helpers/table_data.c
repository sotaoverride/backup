    ...
    netsnmp_feature_child_of(table_data, table_data_all)
        netsnmp_feature_child_of(register_read_only_table_data, table_data_all)
            netsnmp_feature_child_of(
                ...
                table_data_all) netsnmp_feature_child_of(insert_table_row,
                                                         table_data_all)
                netsnmp_feature_child_of(table_data_delete_table,
                                         table_data_all)
                    ...
                    netsnmp_feature_child_of(table_data_extras, table_data_all)
                        ...
                        netsnmp_feature_child_of(table_data_create_table,
                                                 table_data_extras)
                            netsnmp_feature_child_of(table_data_create_row,
                                                     table_data_extras)
                                netsnmp_feature_child_of(table_data_copy_row,
                                                         table_data_extras)
                                    netsnmp_feature_child_of(
                                        ...
                                        netsnmp_feature_child_of(
                                            ...
                                            netsnmp_feature_child_of(
                                                ...
                                                table_data_extras)
                                                netsnmp_feature_child_of(
                                                    table_data_row_operations,
                                                    table_data_extras)
                                                    netsnmp_feature_child_of(
                                                        table_data_row_first,
                                                        table_data_extras)
                               ...
                               netsnmp_table_row* row)
{
    ...
    if (rc > 0)
    {
        for (nextrow = table->first_row, prevrow = NULL; nextrow != NULL;
...
netsnmp_table_data* netsnmp_table_data_create_table(const char* name,
                                                    long flags)
{
...
int netsnmp_table_data_copy_row(netsnmp_table_row* old_row,
...
void* netsnmp_table_data_remove_delete_row(netsnmp_table_data* table,
                                           netsnmp_table_row* row)
{
...
netsnmp_mib_handler* netsnmp_get_table_data_handler(netsnmp_table_data* table)
{
    ...
    ret = netsnmp_create_handler(TABLE_DATA_NAME,
...
int netsnmp_register_read_only_table_data(
    netsnmp_handler_registration* reginfo, netsnmp_table_data* table,
    netsnmp_table_registration_info* table_info)
{
                                      ...
                                      netsnmp_request_info* requests)
{
    ...
    for (request = requests; request; request = request->next)
    {
        ...
        switch (reqinfo->mode)
        {
            ...
            case MODE_SET_RESERVE1:
                ...
                netsnmp_request_add_list_data(
                    request,
                    netsnmp_create_data_list(TABLE_DATA_TABLE, table, NULL));
        }
        ...
        switch (reqinfo->mode)
        {
            ...
            case MODE_GETBULK: /* XXXWWW */
                ...
                regresult =
                    snmp_oid_compare(request->requestvb->name,
                                     SNMP_MIN(request->requestvb->name_length,
                                              reginfo->rootoid_len),
                                     reginfo->rootoid, reginfo->rootoid_len);
                if (regresult == 0 &&
                ...
                else
                {
                    ...
                    for (row = table->first_row; row; row = row->next)
                    {
                        ...
                        result = snmp_oid_compare(
                            row->index_oid, row->index_oid_len,
                            request->requestvb->name + 2 + reginfo->rootoid_len,
                            request->requestvb->name_length - 2 -
                                reginfo->rootoid_len);
                        if (result == 0)
                        {
                            /*
                ...
                { /* no decent result found.  Give up. It's beyond us. */
                    request->processed = 1;
                }
                break;
            ...
            case MODE_GET:
                ...
                else if (NULL == (row = netsnmp_table_data_get_from_oid(
                                      table,
            ...
            case MODE_SET_RESERVE1:
                valid_request = 1;
                if (NULL !=
                    (row = netsnmp_table_data_get_from_oid(
                         table,
        ...
        (reqinfo->mode == MODE_GETNEXT || reqinfo->mode == MODE_GETBULK))
    {
        ...
        result = netsnmp_call_next_handler(handler, reginfo, reqinfo, requests);
                              ...
                              netsnmp_table_row* row)
{
    ...
    build_oid_noalloc(this_oid, MAX_OID_LEN, &this_oid_len, base_oid, 2,
                      this_index);
    ...
    for (; req; req = req->next)
    {
        ...
        build_oid_noalloc(that_oid, MAX_OID_LEN, &that_oid_len, base_oid, 2,
                          that_index);
         ...
         */
        if (snmp_oid_compare(this_oid, this_oid_len, that_oid, that_oid_len) ==
            0)
        {
            netsnmp_request_add_list_data(
                req, netsnmp_create_data_list(TABLE_DATA_ROW, row, NULL));
        }
    }
}
                                    ...
                                    size_t result_data_len)
{
    ...
    snmp_set_var_typed_value(request->requestvb, type, result_data,
                             result_data_len);
                                                   ...
                                                   size_t searchfor_len)
{
    ...
    {
        if (row->index_oid &&
            snmp_oid_compare(searchfor, searchfor_len, row->index_oid,
                             row->index_oid_len) == 0)
            return row;
    }
    return NULL;
...
netsnmp_table_row* netsnmp_table_data_row_get(netsnmp_table_data* table,
                                              netsnmp_table_row* row)
{
    if (!table || !row)
        return NULL;
...
netsnmp_table_row* netsnmp_table_data_row_next(netsnmp_table_data* table,
                                               netsnmp_table_row* row)
{
    return netsnmp_table_data_get_next_row(table, row);
}

netsnmp_table_row* netsnmp_table_data_row_get_byoid(netsnmp_table_data* table,
                                                    oid* instance, size_t len)
{
    return netsnmp_table_data_get_from_oid(table, instance, len);
}

netsnmp_table_row* netsnmp_table_data_row_next_byoid(netsnmp_table_data* table,
                                                     oid* instance, size_t len)
{
    netsnmp_table_row* row;
    ...
    for (row = table->first_row; row; row = row->next)
    {
        if (snmp_oid_compare(row->index_oid, row->index_oid_len, instance,
                             len) > 0)
            return row;
    }
    return NULL;
...
{
    netsnmp_table_row* row = netsnmp_table_data_get_first_row(table);
    return (row ? row->data : NULL);
}

void* netsnmp_table_data_entry_get(netsnmp_table_data* table,
                                   netsnmp_table_row* row)
{
    return (row ? row->data : NULL);
}

void* netsnmp_table_data_entry_next(netsnmp_table_data* table,
                                    netsnmp_table_row* row)
{
    row = netsnmp_table_data_row_next(table, row);
    return (row ? row->data : NULL);
}

void* netsnmp_table_data_entry_get_byidx(netsnmp_table_data* table,
                                         netsnmp_variable_list* indexes)
{
    netsnmp_table_row* row = netsnmp_table_data_row_get_byidx(table, indexes);
    return (row ? row->data : NULL);
}

void* netsnmp_table_data_entry_next_byidx(netsnmp_table_data* table,
                                          netsnmp_variable_list* indexes)
{
    netsnmp_table_row* row = netsnmp_table_data_row_next_byidx(table, indexes);
    return (row ? row->data : NULL);
}

void* netsnmp_table_data_entry_get_byoid(netsnmp_table_data* table,
                                         oid* instance, size_t len)
{
    netsnmp_table_row* row =
...
void* netsnmp_table_data_entry_next_byoid(netsnmp_table_data* table,
                                          oid* instance, size_t len)
{
    netsnmp_table_row* row =
