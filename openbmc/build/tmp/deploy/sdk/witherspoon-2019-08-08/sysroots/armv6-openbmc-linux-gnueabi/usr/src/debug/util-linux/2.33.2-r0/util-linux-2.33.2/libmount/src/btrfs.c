...
uint64_t btrfs_get_default_subvol_id(const char* path)
{
    ...
    {
        DBG(BTRFS,
            ul_debug("opendir() failed for \"%s\" [errno=%d %m]", path, errno));
        return UINT64_MAX;
    }
    fd = dirfd(dirstream);
    if (fd < 0)
    {
        DBG(BTRFS, ul_debug("dirfd(opendir()) failed for \"%s\" [errno=%d %m]",
                            path, errno));
        goto out;
    }
    ...
    {
        DBG(BTRFS,
            ul_debug("ioctl() failed for \"%s\" [errno=%d %m]", path, errno));
        goto out;
    }
    ...
    DBG(BTRFS,
        ul_debug("found %d root tree dir object id items", sk->nr_items));
    ...
    if (sh->type == BTRFS_DIR_ITEM_KEY)
    {
        ...
        if (!strncmp("default", name, name_len))
        {
            ...
            DBG(BTRFS,
                ul_debug("\"default\" id is %llu", (unsigned long long)found));
        }
        else
        {
