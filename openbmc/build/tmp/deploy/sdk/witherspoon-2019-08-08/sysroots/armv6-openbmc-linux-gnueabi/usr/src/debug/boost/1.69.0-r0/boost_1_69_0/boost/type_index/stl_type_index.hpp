...
/// for std::type_index.
///
/// It is used in situations when RTTI is enabled or typeid() method is
/// available. When typeid() is disabled or
/// BOOST_TYPE_INDEX_FORCE_NO_RTTI_COMPATIBILITY macro is defined
/// boost::typeindex::ctti is usually used instead of
...
#error                                                                         \
    "File boost/type_index/stl_type_index.ipp is not usable when typeid() is not available."
#endif
...
/// This class is a wrapper around std::type_info, that workarounds issues and
/// provides much more rich interface. \b For \b description \b of \b functions
/// \b see type_index_facade.
///
/// This class requires typeid() to work. For cases when RTTI is disabled see
/// ctti_type_index.
...
inline const stl_type_index::type_info_t&
    stl_type_index::type_info() const BOOST_NOEXCEPT
{
    return *data_;
}
...
inline std::string stl_type_index::pretty_name() const
{
    ...
    static BOOST_CONSTEXPR_OR_CONST std::string::size_type cvr_saver_name_len =
        sizeof(cvr_saver_name) - 1;

    // In case of MSVC demangle() is a no-op, and name() already returns
    // demangled name. In case of GCC and Clang (on non-Windows systems) name()
    // returns mangled name and demangle() undecorates it.
    ...
    if (!begin)
    {
        boost::throw_exception(
            std::runtime_error("Type name demangling failed"));
    }
    ...
    if (len > cvr_saver_name_len)
    {
        ...
        if (b)
        {
            ...
            // Trim leading spaces
            while (*b == ' ')
            { // the string is zero terminated, we won't exceed the buffer size
                ++b;
            }
...
inline bool
    stl_type_index::equal(const stl_type_index& rhs) const BOOST_NOEXCEPT
{
...
inline bool
    stl_type_index::before(const stl_type_index& rhs) const BOOST_NOEXCEPT
{
#ifdef BOOST_TYPE_INDEX_CLASSINFO_COMPARE_BY_NAMES
    return raw_name() != rhs.raw_name() &&
           std::strcmp(raw_name(), rhs.raw_name()) < 0;
#else
...
inline stl_type_index stl_type_index::type_id() BOOST_NOEXCEPT
{
    ...
    typedef BOOST_DEDUCED_TYPENAME boost::remove_cv<no_ref_t>::type
        no_cvr_prefinal_t;

#if (defined(__EDG_VERSION__) && __EDG_VERSION__ < 245) ||                     \
    (defined(__sgi) && defined(_COMPILER_VERSION) && _COMPILER_VERSION <= 744)

    // Old EDG-based compilers seem to mistakenly distinguish 'integral' from
    // 'signed integral' in typeid() expressions. Full template specialization
    // for 'integral' fixes that issue:
    typedef BOOST_DEDUCED_TYPENAME boost::conditional<
        boost::is_signed<no_cvr_prefinal_t>::value,
        boost::make_signed<no_cvr_prefinal_t>,
        boost::type_identity<no_cvr_prefinal_t>>::type no_cvr_prefinal_lazy_t;
...
{
    typedef BOOST_DEDUCED_TYPENAME boost::conditional<
        boost::is_reference<T>::value || boost::is_const<T>::value ||
            boost::is_volatile<T>::value,
        detail::cvr_saver<T>, T>::type type;
...
inline stl_type_index
    stl_type_index::type_id_runtime(const T& value) BOOST_NOEXCEPT
{
#ifdef BOOST_NO_RTTI
