...
T ellint_pi_imp(T v, T phi, T k, T vc, const Policy& pol)
{
    ...
    if (abs(k) > 1)
    {
        return policies::raise_domain_error<T>(
            function, "Got k = %1%, function requires |k| <= 1", k, pol);
    }
    ...
    if (v == 1)
    {
        ...
        T m = k * k;
        result =
            sqrt(1 - m * sphi * sphi) * tan(phi) - ellint_e_imp(phi, k, pol);
        result /= 1 - m;
    ...
    if ((phi > constants::half_pi<T>()) || (phi < 0))
    {
        ...
        if (fabs(phi) > 1 / tools::epsilon<T>())
        {
            if (v > 1)
                return policies::raise_domain_error<T>(
                    function,
                    "Got v = %1%, but this is only supported for 0 <= phi <= "
                    "pi/2",
                    v, pol);
            //
            ...
            //
            result = 2 * fabs(phi) * ellint_pi_imp(v, k, vc, pol) /
                     constants::pi<T>();
        }
        else
        {
            T rphi = boost::math::tools::fmod_workaround(
                T(fabs(phi)), T(constants::half_pi<T>()));
            T m = boost::math::round((fabs(phi) - rphi) /
                                     constants::half_pi<T>());
            int sign = 1;
            if ((m != 0) && (k >= 1))
            {
                return policies::raise_domain_error<T>(
                    function,
                    "Got k=1 and phi=%1% but the result is complex in that "
                    "domain",
                    phi, pol);
            }
    ...
    if (k == 0)
    {
        ...
        else
        {
            ...
            T vcr = sqrt(-vc);
            T arg = vcr * tan(phi);
            return (boost::math::log1p(arg, pol) -
                    boost::math::log1p(-arg, pol)) /
                   (2 * vcr);
        }
    }
    if (v < 0)
    {
        ...
        //
        // V = (k2 - n)/(1 - n)
        // Assuming[(k2 >= 0 && k2 <= 1) && n < 0, FullSimplify[Sqrt[(1 - V)*(1
        // - k2 / V)] / Sqrt[((1 - n)*(1 - k2 / n))]]] Result: ((-1 + k2) n) /
        // ((-1 + n) (-k2 + n))
        //
        // Assuming[(k2 >= 0 && k2 <= 1) && n < 0, FullSimplify[k2 /
        // (Sqrt[-n*(k2 - n) / (1 - n)] * Sqrt[(1 - n)*(1 - k2 / n)])]] Result :
        // k2 / (k2 - n)
        //
        // Assuming[(k2 >= 0 && k2 <= 1) && n < 0, FullSimplify[Sqrt[1 / ((1 -
        // n)*(1 - k2 / n))]]] Result : Sqrt[n / ((k2 - n) (-1 + n))]
        //
        T k2 = k * k;
        T N = (k2 - v) / (1 - v);
        T Nm1 = (1 - k2) / (1 - v);
        ...
        else
        {
            result += atan((p2 / 2) * sin(2 * phi) / delta) *
                      sqrt(fabs(1 / (k2 - v))) * sqrt(fabs(v / (v - 1)));
        }
        return result;
    }
    if (k == 1)
    {
        ...
        result =
            sqrt(v) * atanh(sqrt(v) * sin(phi)) - log(1 / cos(phi) + tan(phi));
        result /= v - 1;
        return result;
    }
   ...
   if(v > 1)
   {
    ...
    if (v * t < 0.5)
        ...
        p = x + vc * t;
    result = sphi * (ellint_rf_imp(x, y, z, pol) +
                     v * t * ellint_rj_imp(x, y, z, p, pol) / 3);
...
T ellint_pi_imp(T v, T k, T vc, const Policy& pol)
{
    ...
    if (vc <= 0)
    {
        // Result is complex:
        return policies::raise_domain_error<T>(
            function, "Got v = %1%, function requires v < 1", v, pol);
    }

    if (v == 0)
    {
        return (k == 0) ? boost::math::constants::pi<T>() / 2
                        : ellint_k_imp(k, pol);
    }
    ...
    T p = vc;
    T value =
        ellint_rf_imp(x, y, z, pol) + v * ellint_rj_imp(x, y, z, p, pol) / 3;
...
inline typename tools::promote_args<T1, T2>::type
    ellint_3(T1 k, T2 v, const Policy& pol, const mpl::true_&)
{
    typedef typename tools::promote_args<T1, T2>::type result_type;
    typedef typename policies::evaluation<result_type, Policy>::type value_type;
    return policies::checked_narrowing_cast<result_type, Policy>(
        detail::ellint_pi_imp(static_cast<value_type>(v),
                              static_cast<value_type>(k),
                              static_cast<value_type>(1 - v), pol),
        "boost::math::ellint_3<%1%>(%1%,%1%)");
}
    ...
    ellint_3(T1 k, T2 v, T3 phi, const Policy& pol)
{
    ...
    return policies::checked_narrowing_cast<result_type, Policy>(
        detail::ellint_pi_imp(
            static_cast<value_type>(v), static_cast<value_type>(phi),
            static_cast<value_type>(k), static_cast<value_type>(1 - v), pol),
        "boost::math::ellint_3<%1%>(%1%,%1%,%1%)");
}
