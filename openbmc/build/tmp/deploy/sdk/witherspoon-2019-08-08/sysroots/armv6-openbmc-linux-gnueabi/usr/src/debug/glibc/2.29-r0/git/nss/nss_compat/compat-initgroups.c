...
static enum nss_status (*nss_initgroups_dyn)(const char*, gid_t, long int*,
                                             long int*, gid_t**, long int,
                                             int*);
static enum nss_status (*nss_getgrnam_r)(const char* name, struct group* grp,
...
static enum nss_status internal_endgrent(ent_t* ent)
{
    if (ent->stream != NULL)
    {
        fclose(ent->stream);
...
static void add_group(long int* start, long int* size, gid_t** groupsp,
                      long int limit, gid_t gid)
{
...
static enum nss_status getgrent_next_nss(ent_t* ent, char* buffer,
                                         size_t buflen, const char* user,
                                         gid_t group, long int* start,
                                         ...
                                         long int limit, int* errnop)
{
    ...
    if (!ent->skip_initgroups_dyn)
    {
       ...
       in a limited number of groups.  */
        if (nss_initgroups_dyn(user, group, &mystart, &mysize, &mygroups, limit,
...
static enum nss_status internal_getgrent_r(ent_t* ent, char* buffer,
                                           size_t buflen, const char* user,
                                           gid_t group, long int* start,
                                           long int* size, gid_t** groupsp,
                                           long int limit, int* errnop)
{
    ...
    if (!ent->files)
        return getgrent_next_nss(ent, buffer, buflen, user, group, start, size,
    ...
    while (1)
    {
        ...
        if (grpbuf.gr_name[0] == '+' && grpbuf.gr_name[1] != '\0' &&
            grpbuf.gr_name[1] != '@')
        {
            if (in_blacklist(&grpbuf.gr_name[1], strlen(&grpbuf.gr_name[1]),
                             ent))
                continue;
            ...
            else if (nss_getgrnam_r(&grpbuf.gr_name[1], &grpbuf, buffer, buflen,
                                    errnop) != NSS_STATUS_SUCCESS)
                continue;

            check_and_add_group(user, group, start, size, groupsp, limit,
                                &grpbuf);
        ...
        if (grpbuf.gr_name[0] == '+' && grpbuf.gr_name[1] == '\0')
        {
            ...
            return getgrent_next_nss(ent, buffer, buflen, user, group, start,
                                     size, groupsp, limit, errnop);
        }
    }
...
enum nss_status _nss_compat_initgroups_dyn(const char* user, gid_t group,
                                           ...
                                           int* errnop)
{
    ...
    do
    {
        while ((status = internal_getgrent_r(
                    &intern, tmpbuf.data, tmpbuf.length, user, group, start,
                    size, groupsp, limit, errnop)) == NSS_STATUS_TRYAGAIN &&
               *errnop == ERANGE)
