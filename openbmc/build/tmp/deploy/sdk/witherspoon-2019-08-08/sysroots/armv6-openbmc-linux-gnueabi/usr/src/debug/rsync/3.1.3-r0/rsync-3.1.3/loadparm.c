...
#define DEFAULT_DONT_COMPRESS                                                  \
    "*.gz *.zip *.z *.rpm *.deb *.iso *.bz2"                                   \
...
#define SECTION_PTR(s, p)                                                      \
    (((char*)(s)) + (ptrdiff_t)(((char*)(p)) - (char*)&Vars.l))
...
static struct parm_struct parm_table[] = {
    ...
    {"ignore nonreadable", P_BOOL, P_LOCAL, &Vars.l.ignore_nonreadable, NULL,
     0},
    ...
    {"syslog facility", P_ENUM, P_LOCAL, &Vars.l.syslog_facility,
     enum_facilities, 0},
...
#define FN_GLOBAL_STRING(fn_name, ptr)                                         \
    char* fn_name(void)                                                        \
    {                                                                          \
        return expand_vars(*(char**)(ptr) ? *(char**)(ptr) : "");              \
    }
...
#define FN_LOCAL_STRING(fn_name, val)                                          \
    char* fn_name(int i)                                                       \
    {                                                                          \
        return expand_vars(LP_SNUM_OK(i) && iSECTION(i).val                    \
                               ? iSECTION(i).val                               \
                               : Vars.l.val ? Vars.l.val : "");                \
    }
#define FN_LOCAL_BOOL(fn_name, val)                                            \
    BOOL fn_name(int i)                                                        \
    {                                                                          \
        return LP_SNUM_OK(i) ? iSECTION(i).val : Vars.l.val;                   \
    }
...
{
    if (strwicmp(parmvalue, "yes") == 0 || strwicmp(parmvalue, "true") == 0 ||
        strwicmp(parmvalue, "1") == 0)
        *pb = True;
    else if (strwicmp(parmvalue, "no") == 0 ||
             strwicmp(parmvalue, "False") == 0 || strwicmp(parmvalue, "0") == 0)
        *pb = False;
    else
    {
