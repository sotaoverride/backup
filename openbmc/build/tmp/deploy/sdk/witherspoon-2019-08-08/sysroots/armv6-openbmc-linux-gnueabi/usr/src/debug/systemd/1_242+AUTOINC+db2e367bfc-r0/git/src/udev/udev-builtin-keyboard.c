...
static const struct key_name* keyboard_lookup_key(const char* str,
                                                  GPERF_LEN_TYPE len);
#include "keyboard-keys-from-name.h"

static int install_force_release(sd_device* dev, const unsigned* release,
                                 unsigned release_count)
{
    ...
    if (r < 0)
        return log_device_error_errno(
            atkbd, r, "Failed to get force-release attribute: %m");
    ...
    log_device_debug(atkbd, "keyboard: updating force-release list with '%s'",
                     codes);
    r = sd_device_set_sysattr_value(atkbd, "force_release", codes);
    if (r < 0)
        return log_device_error_errno(
            atkbd, r, "Failed to set force-release attribute: %m");

    return 0;
}

static int map_keycode(sd_device* dev, int fd, int scancode,
                       const char* keycode)
{
    struct
    {
    ...
    else
    {
        ...
        if (endptr[0] != '\0')
            return log_device_error_errno(dev, SYNTHETIC_ERRNO(EINVAL),
                                          "Failed to parse key identifier '%s'",
                                          keycode);
    }
    ...
    log_device_debug(
        dev, "keyboard: mapping scan code %d (0x%x) to key code %d (0x%x)",
        map.scan, map.scan, map.key, map.key);

    if (ioctl(fd, EVIOCSKEYCODE, &map) < 0)
        return log_device_error_errno(dev, errno,
                                      "Failed to call EVIOCSKEYCODE with scan "
                                      "code 0x%x, and key code %d: %m",
                                      map.scan, map.key);
...
static int override_abs(sd_device* dev, int fd, unsigned evcode,
                        const char* value)
{
    ...
    if (!next)
        return log_device_error(dev, "Failed to parse EV_ABS override '%s'",
                                value);

    log_device_debug(dev,
                     "keyboard: %x overridden with %" PRIi32 "/%" PRIi32
                     "/%" PRIi32 "/%" PRIi32 "/%" PRIi32,
                     evcode, absinfo.minimum, absinfo.maximum,
                     absinfo.resolution, absinfo.fuzz, absinfo.flat);
...
static int set_trackpoint_sensitivity(sd_device* dev, const char* value)
{
    ...
    if (r < 0)
        return log_device_error_errno(
            dev, r, "Failed to parse POINTINGSTICK_SENSITIVITY '%s': %m",
            value);
    else if (val_i < 0 || val_i > 255)
        return log_device_error_errno(
            dev, SYNTHETIC_ERRNO(ERANGE),
    ...
    if (r < 0)
        return log_device_error_errno(
            dev, r, "Failed to write 'sensitivity' attribute: %m");
...
static int builtin_keyboard(sd_device* dev, int argc, char* argv[], bool test)
{
    ...
    FOREACH_DEVICE_PROPERTY(dev, key, value)
    {
        ...
        if (startswith(key, "KEYBOARD_KEY_"))
        {
            ...
            {
                log_device_warning(
                    dev, "Failed to parse scan code from \"%s\", ignoring",
                    key);
                continue;
            }
            ...
            if (fd < 0)
            {
                ...
                if (fd < 0)
                    return log_device_error_errno(
                        dev, errno, "Failed to open device '%s': %m", node);
            }
        ...
        else if (startswith(key, "EVDEV_ABS_"))
        {
            ...
            {
                log_device_warning(
                    dev, "Failed to parse EV_ABS code from \"%s\", ignoring",
                    key);
                continue;
            }
            ...
            if (has_abs == -1)
            {
                ...
                if (!has_abs)
                    log_device_warning(dev, "EVDEV_ABS override set but no "
                                            "EV_ABS present on device");
            }
