...
#define ARRAY_DESC(name, offset, obj)                                          \
    DB_DEFINE_DESC(name, 8 * sizeof(obj)[0], sizeof(obj) / sizeof(obj)[0],     \
                   offset);
...
#define DB_STRUCT_FIELD(type, field)                                           \
    DESC(_thread_db_##type##_##field, offsetof(type, field), ((type*)0)->field)
#define DB_STRUCT_ARRAY_FIELD(type, field)                                     \
    ARRAY_DESC(_thread_db_##type##_##field, offsetof(type, field),             \
               ((type*)0)->field)
...
#define REGISTER_THREAD_AREA(bits, regofs, scale)                              \
    DB_DEFINE_DESC(_thread_db_register##bits##_thread_area, bits, (scale),     \
                   (regofs));
