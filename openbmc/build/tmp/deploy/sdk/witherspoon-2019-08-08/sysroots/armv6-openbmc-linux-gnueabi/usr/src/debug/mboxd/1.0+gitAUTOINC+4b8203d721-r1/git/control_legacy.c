...
static const sd_bus_vtable control_legacy_vtable[] = {
    ...
    SD_BUS_METHOD("cmd", "yay", "yay", &method_cmd, SD_BUS_VTABLE_UNPRIVILEGED),
    SD_BUS_VTABLE_END};
...
int control_legacy_init(struct mbox_context* context)
{
    ...
    rc = sd_bus_add_object_vtable(context->bus, NULL, MBOX_DBUS_LEGACY_OBJECT,
                                  MBOX_DBUS_LEGACY_NAME, control_legacy_vtable,
                                  context);
    if (rc < 0)
    {
