...
netsnmp_feature_provide(table_container)
    netsnmp_feature_child_of(table_container, table_container_all)
        netsnmp_feature_child_of(table_container_replace_row,
                                 table_container_all)
            netsnmp_feature_child_of(table_container_extract,
                                     table_container_all)
                netsnmp_feature_child_of(table_container_management,
                                         table_container_all)
                    netsnmp_feature_child_of(table_container_row_remove,
                                             table_container_all)
                        netsnmp_feature_child_of(table_container_row_insert,
                                                 table_container_all)
                            netsnmp_feature_child_of(table_container_all,
                                                     mib_helpers)
...
static int _container_table_handler(netsnmp_mib_handler* handler,
                                    netsnmp_handler_registration* reginfo,
                                    netsnmp_agent_request_info* agtreq_info,
                                    netsnmp_request_info* requests);

static void* _find_next_row(netsnmp_container* c,
                            netsnmp_table_request_info* tblreq, void* key);
...
netsnmp_index* netsnmp_tcontainer_remove_row(container_table_data* table,
                                             netsnmp_index* row)
{
    if (!table || !table->table || !row)
        return NULL;
...
int netsnmp_tcontainer_replace_row(container_table_data* table,
...
static void netsnmp_container_table_data_free(container_table_data* tad)
{
    if (--tad->refcnt == 0)
        free(tad);
}
                                        ...
                                        char key_type)
{
    ...
    handler =
        netsnmp_create_handler("table_container", _container_table_handler);
    if ((NULL == tad) || (NULL == handler))
    {
        if (tad)
            free(tad); /* SNMP_FREE wasted on locals */
        if (handler)
            free(handler); /* SNMP_FREE wasted on locals */
        snmp_log(LOG_ERR,
...
int netsnmp_container_table_register(netsnmp_handler_registration* reginfo,
...
int netsnmp_container_table_unregister(netsnmp_handler_registration* reginfo)
{
    ...
    tad = (container_table_data*)netsnmp_find_handler_data_by_name(
        reginfo, "table_container");
    if (tad)
    {
...
{
    return (netsnmp_container*)netsnmp_request_get_list_data(
...
{
    /*
     * NOTE: this function must match in table_container.c and
     * table_container.h. if you change one, change them both!
     */
    ...
    for (; req; req = req->next)
    {
            ...
            0)
        {
            netsnmp_request_add_list_data(
                req, netsnmp_create_data_list(TABLE_CONTAINER_ROW, row, NULL));
        }
    }
}
 ...
 **********************************************************************
 **********************************************************************/
NETSNMP_STATIC_INLINE void _set_key(container_table_data* tad,
                                        ...
                                        container_table_data* tad)
{
    ...
    else
    {
        ...
        if (NULL == row)
        {
            ...
            if ((agtreq_info->mode != MODE_SET_RESERVE1) || /* get */
                (reginfo->modes & HANDLER_CAN_NOT_CREATE))
            { /* no create */
    ...
    if (SNMP_ENDOFMIBVIEW != request->requestvb->type)
    {
        ...
        netsnmp_request_add_list_data(
            request, netsnmp_create_data_list(TABLE_CONTAINER_CONTAINER,
                                              tad->table, NULL));
    }
}
                                    ...
                                    netsnmp_request_info* requests)
{
    ...
    )
    {
         ...
         */
        for (curr_request = requests; curr_request;
    ...
    if ((oldmode == MODE_GETNEXT) && (handler->next))
    {
        ...
        if (need_processing > 0)
        {
            ...
            {
                DEBUGMSGTL(
                    ("table_container", "next handler returned %d\n", rc));
            }
