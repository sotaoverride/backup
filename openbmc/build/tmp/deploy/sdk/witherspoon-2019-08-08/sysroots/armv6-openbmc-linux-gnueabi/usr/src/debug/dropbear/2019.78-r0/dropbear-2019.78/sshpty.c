...
int pty_allocate(int* ptyfd, int* ttyfd, char* namebuf, int namebuflen)
{
    ...
    if (i < 0)
    {
        dropbear_log(LOG_WARNING, "pty_allocate: openpty: %.100s",
                     strerror(errno));
        return 0;
    }
    name = ttyname(*ttyfd);
    ...
    if (slave == NULL)
    {
        dropbear_log(LOG_WARNING, "pty_allocate: _getpty: %.100s",
                     strerror(errno));
        return 0;
    }
    ...
    *ttyfd = open(namebuf, O_RDWR | O_NOCTTY);
    if (*ttyfd < 0)
    {
        dropbear_log(LOG_WARNING, "pty_allocate error: ttyftd open error");
        close(*ptyfd);
        return 0;
    }
    return 1;
    ...
    if (ptm < 0)
    {
        dropbear_log(LOG_WARNING, "pty_allocate: /dev/ptmx: %.100s",
                     strerror(errno));
        return 0;
    }
    if (grantpt(ptm) < 0)
    ...
    *ttyfd = open(namebuf, O_RDWR | O_NOCTTY);
    if (*ttyfd < 0)
    {
        dropbear_log(LOG_ERR, "error opening pts %.100s: %.100s", namebuf,
                     strerror(errno));
        close(*ptyfd);
        return 0;
    }
    ...
    {
        dropbear_log(LOG_WARNING, "ioctl I_PUSH ldterm: %.100s",
                     strerror(errno));
    }
#ifndef __hpux
    if (ioctl(*ttyfd, I_PUSH, "ttcompat") < 0)
    {
        dropbear_log(LOG_WARNING, "ioctl I_PUSH ttcompat: %.100s",
                     strerror(errno));
    }
#endif
#endif
    ...
    *ptyfd = open("/dev/ptc", O_RDWR | O_NOCTTY);
    if (*ptyfd < 0)
    {
        dropbear_log(LOG_ERR, "Could not open /dev/ptc: %.100s",
                     strerror(errno));
        return 0;
    }
    name = ttyname(*ptyfd);
    ...
    *ttyfd = open(name, O_RDWR | O_NOCTTY);
    if (*ttyfd < 0)
    {
        dropbear_log(LOG_ERR, "Could not open pty slave side %.100s: %.100s",
                     name, strerror(errno));
        close(*ptyfd);
        return 0;
    }
    ...
    for (i = 0; i < num_ptys; i++)
    {
        ...
        snprintf(namebuf, namebuflen, "/dev/tty%c%c", ptymajors[i / num_minors],
                 ptyminors[i % num_minors]);
        ...
        *ttyfd = open(namebuf, O_RDWR | O_NOCTTY);
        if (*ttyfd < 0)
        {
            dropbear_log(LOG_ERR, "pty_allocate: %.100s: %.100s", namebuf,
                         strerror(errno));
            close(*ptyfd);
            return 0;
        }
        ...
        {
            dropbear_log(LOG_WARNING, "ptyallocate: tty modes failed: %.100s",
                         strerror(errno));
        }
        else
        {
...
void pty_release(const char* tty_name)
{
    ...
    {
        dropbear_log(LOG_ERR, "chown %.100s 0 0 failed: %.100s", tty_name,
                     strerror(errno));
    }
    if (chmod(tty_name, (mode_t)0666) < 0 && (errno != ENOENT))
    {
        dropbear_log(LOG_ERR, "chmod %.100s 0666 failed: %.100s", tty_name,
                     strerror(errno));
    }
}
...
void pty_make_controlling_tty(int* ttyfd, const char* tty_name)
{
    ...
    if (setpgrp(0, 0) < 0)
    {
        dropbear_log(LOG_ERR, error("SETPGRP %s", strerror(errno)));
    }
#endif /* HAVE_NEWS4 */
#ifdef USE_VHANGUP
...
void pty_setowner(struct passwd* pw, const char* tty_name)
{
    ...
    if (stat(tty_name, &st))
    {
        dropbear_exit("pty_setowner: stat(%.101s) failed: %.100s", tty_name,
                      strerror(errno));
    }
    ...
    if (st.st_uid != pw->pw_uid || st.st_gid != gid)
    {
        if (chown(tty_name, pw->pw_uid, gid) < 0)
        {
            ...
            {
                dropbear_log(LOG_ERR, "chown(%.100s, %u, %u) failed: %.100s",
                             tty_name, (unsigned int)pw->pw_uid,
                             (unsigned int)gid, strerror(errno));
            }
            else
            {
                dropbear_exit("chown(%.100s, %u, %u) failed: %.100s", tty_name,
    ...
    if ((st.st_mode & (S_IRWXU | S_IRWXG | S_IRWXO)) != mode)
    {
        if (chmod(tty_name, mode) < 0)
        {
            ...
            {
                dropbear_log(LOG_ERR, "chmod(%.100s, 0%o) failed: %.100s",
                             tty_name, mode, strerror(errno));
            }
            else
            {
                dropbear_exit("chmod(%.100s, 0%o) failed: %.100s", tty_name,
                              mode, strerror(errno));
            }
        }
    }
