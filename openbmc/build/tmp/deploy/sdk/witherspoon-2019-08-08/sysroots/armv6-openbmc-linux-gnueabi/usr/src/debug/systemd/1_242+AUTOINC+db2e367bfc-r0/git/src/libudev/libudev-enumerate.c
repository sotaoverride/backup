...
static struct udev_enumerate*
    udev_enumerate_free(struct udev_enumerate* udev_enumerate)
{
 ...
 * Returns: #NULL
 **/
DEFINE_PUBLIC_TRIVIAL_REF_UNREF_FUNC(struct udev_enumerate, udev_enumerate,
                                     udev_enumerate_free);
    ...
    udev_enumerate_get_list_entry(struct udev_enumerate* udev_enumerate)
{
    ...
    if (!udev_enumerate->devices_uptodate)
    {
        ...
        FOREACH_DEVICE_AND_SUBSYSTEM(udev_enumerate->enumerator, device)
        {
            ...
            if (!udev_list_entry_add(&udev_enumerate->devices_list, syspath,
                                     NULL))
                return_with_errno(NULL, ENOMEM);
        }
                                       ...
                                       const char* subsystem)
{
    ...
    return sd_device_enumerator_add_match_subsystem(udev_enumerate->enumerator,
                                                    subsystem, true);
}
                                     ...
                                     const char* sysattr, const char* value)
{
    ...
    return sd_device_enumerator_add_match_sysattr(udev_enumerate->enumerator,
                                                  sysattr, value, true);
}
                                      ...
                                      const char* property, const char* value)
{
    ...
    return sd_device_enumerator_add_match_property(udev_enumerate->enumerator,
                                                   property, value);
}
 ...
 * Returns: 0 on success, otherwise a negative error value.
 */
_public_ int udev_enumerate_add_match_tag(struct udev_enumerate* udev_enumerate,
                                          const char* tag)
{
    assert_return(udev_enumerate, -EINVAL);
                                    ...
                                    struct udev_device* parent)
{
    ...
    return sd_device_enumerator_add_match_parent(udev_enumerate->enumerator,
                                                 parent->device);
}
 ...
 * Returns: 0 on success, otherwise a negative error value.
 */
_public_ int udev_enumerate_add_match_is_initialized(
    struct udev_enumerate* udev_enumerate)
{
    assert_return(udev_enumerate, -EINVAL);

    return device_enumerator_add_match_is_initialized(
                                     ...
                                     const char* sysname)
{
    ...
    return sd_device_enumerator_add_match_sysname(udev_enumerate->enumerator,
                                                  sysname);
}
 ...
 *
 * Add a device to the list of devices, to retrieve it back sorted in dependency
 * order.
 *
 * Returns: 0 on success, otherwise a negative error value.
 */
