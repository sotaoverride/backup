...
struct BusConnections
{
    ...
    BusContext* context;
    DBusHashTable*
        completed_by_user; /**< Number of completed connections for each UID */
    DBusTimeout*
        expire_timeout; /**< Timeout for expiring incomplete connections. */
...
typedef struct
{
    ...
    int n_match_rules;
    char* name;
    DBusList* transaction_messages; /**< Stuff we need to send as part of a
                                       transaction */
...
#define BUS_CONNECTION_DATA(connection)                                        \
    (dbus_connection_get_data((connection), connection_data_slot))
...
static dbus_bool_t adjust_connections_for_uid(BusConnections* connections,
                                              dbus_uid_t uid, int adjustment)
{
    int current_count;
    ...
    else
    {
        ...
        retval = _dbus_hash_table_insert_uintptr(
            connections->completed_by_user, uid,
            _DBUS_INT_TO_POINTER(current_count));
...
void bus_connection_disconnected(DBusConnection* connection)
{
    ...
    retry:
        ...
        while ((transaction = bus_transaction_new(d->connections->context)) ==
               NULL)
        ...
        if (!bus_service_remove_owner(service, connection, transaction, &error))
        {
            ...
            else
            {
                ...
                _dbus_assert_not_reached("Removing service owner failed for "
                                         "non-memory-related reason");
            }
        }
    ...
    /* no more watching */
    if (!dbus_connection_set_watch_functions(connection, NULL, NULL, NULL,
                                             connection, NULL))
        _dbus_assert_not_reached("setting watch functions to NULL failed");

    if (!dbus_connection_set_timeout_functions(connection, NULL, NULL, NULL,
                                               connection, NULL))
    ...
    if (d->link_in_connection_list != NULL)
    {
        if (d->name != NULL)
        {
            ...
            _dbus_list_remove_link(&d->connections->completed,
            ...
            if (dbus_connection_get_unix_user(connection, &uid))
            {
                if (!adjust_connections_for_uid(d->connections, uid, -1))
                    _dbus_assert_not_reached(
                        "adjusting downward should never fail");
            }
        }
        else
        {
            _dbus_list_remove_link(&d->connections->incomplete,
                                   d->link_in_connection_list);
            d->link_in_connection_list = NULL;
...
static dbus_bool_t allow_unix_user_function(DBusConnection* connection,
                                            unsigned long uid, void* data)
{
    BusConnectionData* d;
...
static void free_connection_data(void* data)
{
    ...
    if (d->oom_preallocated)
        dbus_connection_free_preallocated_send(d->connection,
                                               d->oom_preallocated);
...
BusConnections* bus_connections_new(BusContext* context)
{
    ...
    connections->completed_by_user =
        _dbus_hash_table_new(DBUS_HASH_UINTPTR, NULL, NULL);
    if (connections->completed_by_user == NULL)
        goto failed_2;

    connections->expire_timeout =
        _dbus_timeout_new(100, /* irrelevant */
    ...
    connections->pending_replies = bus_expire_list_new(
        bus_context_get_loop(context), bus_context_get_reply_timeout(context),
...
/* Used for logging */
static dbus_bool_t cache_peer_loginfo_string(BusConnectionData* d,
                                             DBusConnection* connection)
{
    ...
    if (dbus_connection_get_windows_user(connection, &windows_sid))
    {
        ...
        did_append =
            _dbus_string_append_printf(&loginfo_buf, "sid=\"%s\"", windows_sid);
    ...
    if (_dbus_connection_get_linux_security_label(connection, &security_label))
    {
        ...
        did_append = _dbus_string_append_printf(&loginfo_buf, "label=\"%s\"",
                                                security_label);
                                             ...
                                             DBusConnection* connection)
{
    ...
    if (!dbus_connection_set_data(connection, connection_data_slot, d,
    ...
    d->apparmor_confinement =
        bus_apparmor_init_connection_confinement(connection, &error);
    if (dbus_error_is_set(&error))
    {
        bus_context_log(connections->context, DBUS_SYSTEM_LOG_WARNING,
                        "Unable to set up new connection: %s", error.message);
    ...
    if (!dbus_connection_set_watch_functions(
            connection, add_connection_watch, remove_connection_watch,
            toggle_connection_watch, connection, NULL))
        goto oom;

    if (!dbus_connection_set_timeout_functions(
            connection, add_connection_timeout, remove_connection_timeout, NULL,
            connection, NULL))
        goto oom;
     ...
     */
    dbus_connection_set_unix_user_function(connection, allow_unix_user_function,
                                           NULL, NULL);
    ...
    {
        if (!_dbus_loop_queue_dispatch(
                bus_context_get_loop(connections->context), connection))
        {
            bus_dispatch_remove_connection(connection);
            goto oom;
        }
    ...
    d->pending_unix_fds_timeout =
        _dbus_timeout_new(100, /* irrelevant */
                          pending_unix_fds_timeout_cb, connection, NULL);
    ...
    _dbus_connection_set_pending_fds_function(
        connection, (DBusPendingFdsChangeFunction)check_pending_fds_cb,
        connection);

    _dbus_list_append_link(&connections->incomplete,
                           d->link_in_connection_list);
...
error:
    if (d != NULL)
    {
        ...
        dbus_connection_set_dispatch_status_function(connection, NULL, NULL,
                                                     NULL);
        ...
        if (!dbus_connection_set_data(connection, connection_data_slot, NULL,
                                      NULL))
...
void bus_connections_expire_incomplete(BusConnections* connections)
{
    ...
    if (connections->incomplete != NULL)
    {
        ...
        {
            DBusList* next =
                _dbus_list_get_next_link(&connections->incomplete, link);
            DBusConnection* connection;
            BusConnectionData* d;
            double elapsed;
            ...
            elapsed = ELAPSED_MILLISECONDS_SINCE(
                d->connection_tv_sec, d->connection_tv_usec, tv_sec, tv_usec);
            ...
            if (elapsed >= (double)auth_timeout)
            {
                 ...
                 * and so on. */
                bus_context_log(
                    connections->context, DBUS_SYSTEM_LOG_WARNING,
            ...
            {
                /* We can end the loop, since the connections are in
                 * oldest-first order */
                next_interval = ((double)auth_timeout) - elapsed;
                _dbus_verbose(
                    "Connection %p authentication expires in %d milliseconds\n",
...
static dbus_bool_t foreach_active(BusConnections* connections,
                                  ...
                                  void* data)
{
    ...
    while (link != NULL)
    {
        ...
        DBusList* next =
            _dbus_list_get_next_link(&connections->completed, link);
...
static dbus_bool_t foreach_inactive(BusConnections* connections,
                                    BusConnectionForeachFunction function,
                                    void* data)
{
    DBusList* link;
...
void bus_connections_foreach(BusConnections* connections,
                             BusConnectionForeachFunction function, void* data)
{
                                   ...
                                   DBusMessage* in_reply_to)
{
    ...
    if (!dbus_message_set_reply_serial(d->oom_message,
        ...
        _dbus_assert_not_reached(
            "Failed to set reply serial for preallocated oom message");
...
dbus_bool_t bus_connection_complete(DBusConnection* connection,
                                    const DBusString* name, DBusError* error)
{
    BusConnectionData* d;
    unsigned long uid;
                                         ...
                                         int* limit_out, DBusError* error)
{
    ...
    if (connections->n_completed >= limit)
    {
        ...
        dbus_set_error(
            error, DBUS_ERROR_LIMITS_EXCEEDED,
    ...
    if (dbus_connection_get_unix_user(requesting_completion, &uid))
    {
        ...
        if (get_connections_for_uid(connections, uid) >= limit)
        {
            ...
            dbus_set_error(error, DBUS_ERROR_LIMITS_EXCEEDED,
                           "The maximum number of active connections for UID "
                           "%lu has been reached",
                           uid);
            return FALSE;
        }
...
static void bus_pending_reply_free(BusPendingReply* pending)
{
    _dbus_verbose(
        ...
        pending->will_send_reply, pending->will_get_reply,
        pending->reply_serial);

    dbus_free(pending);
}

static dbus_bool_t bus_pending_reply_send_no_reply(BusConnections* connections,
                                                   BusTransaction* transaction,
                                                   BusPendingReply* pending)
{
    ...
    if (!dbus_message_set_reply_serial(message, pending->reply_serial))
        goto out;

    if (!dbus_message_set_error_name(message, DBUS_ERROR_NO_REPLY))
        goto out;
                                             ...
                                             DBusList* link, void* data)
{
    ...
    _dbus_verbose(
        ...
        pending, pending->will_send_reply, pending->will_get_reply,
        pending->reply_serial);
                                                ...
                                                DBusConnection* connection)
{
    ...
    while (link != NULL)
    {
        ...
        next =
            bus_expire_list_get_next_link(connections->pending_replies, link);
        pending = link->data;
        ...
        else if (pending->will_send_reply == connection)
        {
             ...
             */
            _dbus_verbose("Will expire pending reply %p, replier %p receiver "
                          "%p serial %u\n",
                          pending, pending->will_send_reply,
                          pending->will_get_reply, pending->reply_serial);
                                         ...
                                         DBusError* error)
{
    ...
    if (dbus_message_get_no_reply(reply_to_this))
        return TRUE; /* we won't allow a reply, since client doesn't care for
                        one. */
    ...
    while (link != NULL)
    {
            ...
            pending->will_send_reply == will_send_reply)
        {
            dbus_set_error(error, DBUS_ERROR_ACCESS_DENIED,
                           "Message has the same reply serial as a "
                           "currently-outstanding existing method call");
            return FALSE;
        }
    ...
    if (count >= limit)
    {
        bus_context_log(connections->context, DBUS_SYSTEM_LOG_WARNING,
                        ...
                        bus_connection_get_loginfo(will_get_reply), limit);

        dbus_set_error(error, DBUS_ERROR_LIMITS_EXCEEDED,
                       "The maximum number of pending replies per connection "
                       "has been reached");
        return FALSE;
    }
    ...
    if (!bus_transaction_add_cancel_hook(transaction, cancel_pending_reply,
                                         cprd, cancel_pending_reply_data_free))
    {
        BUS_SET_OOM(error);
        bus_expire_list_remove(connections->pending_replies,
                               &pending->expire_item);
        dbus_free(cprd);
        bus_pending_reply_free(pending);
        return FALSE;
...
static void check_pending_reply_data_free(void* data)
{
    ...
    if (d->link != NULL)
    {
        ...
        _dbus_assert(!bus_expire_list_contains_item(
            d->connections->pending_replies, &pending->expire_item));
                                        ...
                                        DBusMessage* reply, DBusError* error)
{
    ...
    if (!bus_transaction_add_cancel_hook(transaction,
                                         cancel_check_pending_reply, cprd,
    ...
    _dbus_assert(!bus_expire_list_contains_item(connections->pending_replies,
                                                link->data));
...
static void message_to_send_free(DBusConnection* connection,
                                 MessageToSend* to_send)
{
    ...
    if (to_send->preallocated)
        dbus_connection_free_preallocated_send(connection,
                                               to_send->preallocated);
                                    ...
                                    DBusMessage* message)
{
    ...
    if (!bus_matchmaker_get_recipients(
            mm, connections, sender, addressed_recipient, message, &recipients))
        goto out;
...
dbus_bool_t bus_transaction_capture_error_reply(
    BusTransaction* transaction, DBusConnection* addressed_recipient,
    const DBusError* error, DBusMessage* in_reply_to)
{
    BusConnections* connections;
    DBusMessage* reply;
    dbus_bool_t ret = FALSE;
    ...
    ret =
        bus_transaction_capture(transaction, NULL, addressed_recipient, reply);
                                             ...
                                             DBusMessage* message)
{
    ...
    if (!bus_context_check_security_policy(
            bus_transaction_get_context(transaction), transaction, NULL,
            connection, connection, message, NULL, &error))
    {
                                 ...
                                 DBusMessage* message)
{
    ...
    _dbus_verbose(
        ...
        dbus_message_get_type(message) == DBUS_MESSAGE_TYPE_ERROR
            ? "error"
            : dbus_message_get_reply_serial(message) != 0 ? "reply" : "message",
        dbus_message_get_interface(message)
            ? dbus_message_get_interface(message)
            : "(unset)",
    ...
    while (link != NULL)
    {
        ...
        DBusList* next =
            _dbus_list_get_next_link(&d->transaction_messages, link);
...
static void connection_cancel_transaction(DBusConnection* connection,
                                          BusTransaction* transaction)
{
    DBusList* link;
    BusConnectionData* d;
...
static void connection_execute_transaction(DBusConnection* connection,
                                           BusTransaction* transaction)
{
    DBusList* link;
    BusConnectionData* d;
    ...
    while (link != NULL)
    {
        MessageToSend* m = link->data;
        DBusList* prev =
            _dbus_list_get_prev_link(&d->transaction_messages, link);
        ...
        if (m->transaction == transaction)
        {
            ...
            dbus_connection_send_preallocated(connection, m->preallocated,
                                              m->message, NULL);
...
static void bus_connection_remove_transactions(DBusConnection* connection)
{
    ...
    {
        /* only has an effect for the first MessageToSend listing this
         * transaction */
...
dbus_bool_t bus_transaction_add_cancel_hook(
    BusTransaction* transaction, BusTransactionCancelFunction cancel_function,
...
static dbus_bool_t bcd_add_monitor_rules(BusConnectionData* d,
                                         DBusConnection* connection,
                                         DBusList** rules)
{
    BusMatchmaker* mm = d->connections->monitor_matchmaker;
...
static void bcd_drop_monitor_rules(BusConnectionData* d,
                                   DBusConnection* connection)
{
    BusMatchmaker* mm = d->connections->monitor_matchmaker;
...
dbus_bool_t bus_connection_be_monitor(DBusConnection* connection,
                                      BusTransaction* transaction,
                                      DBusList** rules, DBusError* error)
{
    BusConnectionData* d;
    DBusList* link;
    DBusList* tmp;
    ...
    for (iter = _dbus_list_get_first_link(&tmp); iter != NULL;
