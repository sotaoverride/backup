...
const char* _gcry_selftest_helper_cbc(const char* cipher,
                                      ...
                                      const int context_size)
{
    ...
    memsize =
        ctx_aligned_size + (blocksize * 2) + (blocksize * nblocks * 3) + 16;
    ...
    if (memcmp(plaintext2, plaintext, blocksize))
    {
        ...
        syslog(LOG_USER | LOG_WARNING,
               "Libgcrypt warning: "
               "%s-CBC-%d test failed (plaintext mismatch)",
               cipher, blocksize * 8);
#else
    ...
    if (memcmp(iv2, iv, blocksize))
    {
        ...
        syslog(LOG_USER | LOG_WARNING,
               "Libgcrypt warning: "
               "%s-CBC-%d test failed (IV mismatch)",
               cipher, blocksize * 8);
#endif
        return "selftest for CBC failed - see syslog for details";
    }
...
/* Run the self-tests for <block cipher>-CFB-<block size>, tests bulk CFB
   decryption.  Returns NULL on success. */
const char* _gcry_selftest_helper_cfb(const char* cipher,
                                      gcry_cipher_setkey_t setkey_func,
                                      gcry_cipher_encrypt_t encrypt_one,
                                      ...
                                      const int context_size)
{
    ...
    if (memcmp(plaintext2, plaintext, blocksize))
    {
        ...
        syslog(LOG_USER | LOG_WARNING,
               "Libgcrypt warning: "
               "%s-CFB-%d test failed (plaintext mismatch)",
               cipher, blocksize * 8);
#else
        (void)cipher; /* Not used.  */
#endif
    ...
    if (memcmp(iv2, iv, blocksize))
    {
        ...
        syslog(LOG_USER | LOG_WARNING,
               "Libgcrypt warning: "
               "%s-CFB-%d test failed (IV mismatch)",
               cipher, blocksize * 8);
#endif
        return "selftest for CFB failed - see syslog for details";
    }
    ...
    if (memcmp(iv2, iv, blocksize))
    {
        ...
        syslog(LOG_USER | LOG_WARNING,
               "Libgcrypt warning: "
               "%s-CFB-%d test failed (IV mismatch, parallel path)",
               cipher, blocksize * 8);
#endif
        return "selftest for CFB failed - see syslog for details";
    }
...
const char* _gcry_selftest_helper_ctr(const char* cipher,
                                      gcry_cipher_setkey_t setkey_func,
                                      gcry_cipher_encrypt_t encrypt_one,
...
{
    int i, j, offs, diff;
    unsigned char *ctx, *plaintext, *plaintext2, *ciphertext, *ciphertext2, *iv,
        *iv2, *mem;
    unsigned int ctx_aligned_size, memsize;
    ...
    memsize =
        ctx_aligned_size + (blocksize * 2) + (blocksize * nblocks * 4) + 16;
    ...
    if (memcmp(plaintext2, plaintext, blocksize))
    {
        ...
        syslog(LOG_USER | LOG_WARNING,
               "Libgcrypt warning: "
               "%s-CTR-%d test failed (plaintext mismatch)",
               cipher, blocksize * 8);
#else
        (void)cipher; /* Not used.  */
#endif
    ...
    if (memcmp(iv2, iv, blocksize))
    {
        ...
        syslog(LOG_USER | LOG_WARNING,
               "Libgcrypt warning: "
               "%s-CTR-%d test failed (IV mismatch)",
               cipher, blocksize * 8);
#endif
        return "selftest for CTR failed - see syslog for details";
    }
    ...
    if (memcmp(ciphertext2, ciphertext, blocksize * nblocks))
    {
        ...
        syslog(LOG_USER | LOG_WARNING,
               "Libgcrypt warning: "
               "%s-CTR-%d test failed (ciphertext mismatch, bulk)",
               cipher, blocksize * 8);
#endif
        return "selftest for CTR failed - see syslog for details";
    }
    if (memcmp(iv2, iv, blocksize))
    {
        ...
        syslog(LOG_USER | LOG_WARNING,
               "Libgcrypt warning: "
               "%s-CTR-%d test failed (IV mismatch, bulk)",
               cipher, blocksize * 8);
#endif
        return "selftest for CTR failed - see syslog for details";
    }
    ...
    for (diff = 0; diff < nblocks; diff++)
    {
        ...
        if (memcmp(plaintext2, plaintext, blocksize * nblocks))
        {
            ...
            syslog(LOG_USER | LOG_WARNING,
                   "Libgcrypt warning: "
                   "%s-CTR-%d test failed (plaintext mismatch, diff: %d)",
        ...
        if (memcmp(iv2, iv, blocksize))
        {
            ...
            syslog(LOG_USER | LOG_WARNING,
                   "Libgcrypt warning: "
                   "%s-CTR-%d test failed (IV mismatch, diff: %d)",
                   cipher, blocksize * 8, diff);
#endif
            return "selftest for CTR failed - see syslog for details";
        }
