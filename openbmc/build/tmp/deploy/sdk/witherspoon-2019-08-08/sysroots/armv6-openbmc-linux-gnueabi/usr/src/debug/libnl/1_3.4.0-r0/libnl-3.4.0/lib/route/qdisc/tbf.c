...
static int tbf_msg_parser(struct rtnl_tc* tc, void* data)
{
    ...
    if (tb[TCA_TBF_PARMS])
    {
        ...
        bufsize =
            rtnl_tc_calc_bufsize(nl_ticks2us(opts.mtu), opts.peakrate.rate);
                          ...
                          struct nl_dump_params* p)
{
    ...
    nl_dump(p, " rate %.2f%s/s (%.0f%s) limit %.2f%s", r, ru, rbit, rubit, lim,
            limu);
}
                             ...
                             struct nl_dump_params* p)
{
    ...
    if (1)
    {
        ...
        double cl = nl_cancel_down_bytes(1 << tbf->qt_rate.rs_cell_log, &cu);
    ...
    if (tbf->qt_mask & TBF_ATTR_PEAKRATE)
    {
        ...
        cl = nl_cancel_down_bits(1 << tbf->qt_peakrate.rs_cell_log, &clu);
...
int rtnl_qdisc_tbf_set_limit_by_latency(struct rtnl_qdisc* qdisc, int latency)
{
    ...
    {
        limit2 =
            calc_limit(&tbf->qt_peakrate, latency, tbf->qt_peakrate_bucket);
