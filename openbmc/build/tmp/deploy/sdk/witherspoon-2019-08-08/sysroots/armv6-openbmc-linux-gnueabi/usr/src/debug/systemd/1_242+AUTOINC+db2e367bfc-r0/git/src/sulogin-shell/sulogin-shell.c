...
static int start_default_target(sd_bus* bus)
{
    ...
    r = sd_bus_call_method(bus, "org.freedesktop.systemd1",
                           ...
                           &error, NULL, "ss", "default.target", "isolate");

    if (r < 0)
        return log_error_errno(r, "Failed to start default target: %s",
                               bus_error_message(&error, r));
...
{
    printf("You are in %s mode. After logging in, type \"journalctl -xb\" to "
           "view\n"
...
{
    const char* sulogin_cmdline[] = {SULOGIN, NULL, /* --force */
                                     NULL};
