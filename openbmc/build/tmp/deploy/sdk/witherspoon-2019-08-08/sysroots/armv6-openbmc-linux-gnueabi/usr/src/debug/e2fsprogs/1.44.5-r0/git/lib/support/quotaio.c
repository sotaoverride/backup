...
const char* quota_get_qf_name(enum quota_type type, int fmt, char* buf)
{
    if (!buf)
        return NULL;
    snprintf(buf, QUOTA_NAME_LEN, "%s.%s", basenames[fmt], extensions[type]);
...
static ext2_off64_t compute_inode_size(ext2_filsys fs, ext2_ino_t ino)
{
    ...
    ext2fs_block_iterate3(fs, ino, BLOCK_FLAG_READ_ONLY, NULL,
                          compute_num_blocks_proc, &num_blocks);
                          ...
                          int flags)
{
    ...
    if (h->qh_ops->check_file && (h->qh_ops->check_file(h, qtype, fmt) == 0))
    {
...
static errcode_t quota_inode_init_new(ext2_filsys fs, ext2_ino_t ino)
{
    ...
    inode.i_atime = inode.i_mtime = inode.i_ctime = fs->now ? fs->now : time(0);
                            ...
                            enum quota_type qtype, int fmt)
{
    ...
    {
        err = ext2fs_new_inode(fs, EXT2_ROOT_INO, LINUX_S_IFREG | 0600, 0,
                               &qf_inum);
        if (err)
            return err;
