...
static krb5_error_code
    make_seal_token_v1_iov(krb5_context context, krb5_gss_ctx_id_rec* ctx,
                           int conf_req_flag, int* conf_state,
                           gss_iov_buffer_desc* iov, int iov_count, int toktype)
{
    ...
    if (toktype == KG_TOK_WRAP_MSG)
    {
        ...
        {
            /* DCE will pad the actual data itself; padding buffer optional and
             * will be zeroed */
            gss_padlen = 0;
    ...
    g_make_token_header(ctx->mech_used, 14 + ctx->cksum_size + tmsglen, &ptr,
                        toktype);
    ...
    code =
        krb5_c_checksum_length(context, md5cksum.checksum_type, &k5_trailerlen);
    if (code != 0)
        goto cleanup;
    ...
    code = kg_make_checksum_iov_v1(
        context, md5cksum.checksum_type, ctx->cksum_size, ctx->seq, ctx->enc,
        sign_usage, iov, iov_count, toktype, &md5cksum);
    if (code != 0)
        goto cleanup;
    ...
    if (conf_req_flag)
    {
        if (ctx->sealalg == SEAL_ALG_MICROSOFT_RC4)
        {
            ...
            code = kg_arcfour_docrypt_iov(context, enc_key, 0, bigend_seqnum, 4,
                                          iov, iov_count);
        ...
        else
        {
            code = kg_encrypt_iov(context, ctx->proto,
                                  ((ctx->gss_flags & GSS_C_DCE_STYLE) != 0),
                                  0 /*EC*/, 0 /*RRC*/, ctx->enc, KG_USAGE_SEAL,
                                  NULL, iov, iov_count);
        }
        if (code != 0)
            goto cleanup;
                      ...
                      gss_iov_buffer_desc* iov, int iov_count, int toktype)
{
    ...
    switch (ctx->proto)
    {
        case 0:
            ...
            break;
        case 1:
            code = gss_krb5int_make_seal_token_v3_iov(context, ctx,
                             ...
                             int toktype)
{
    ...
    dce_or_mic =
        ((ctx->gss_flags & GSS_C_DCE_STYLE) != 0 || toktype == KG_TOK_MIC_MSG);

    /* For CFX, EC is used instead of padding, and is placed in header or
     * trailer */
    padding = kg_locate_iov(iov, iov_count, GSS_IOV_BUFFER_TYPE_PADDING);
    if (padding == NULL)
    {
    ...
    if (ctx->proto == 1)
    {
        ...
        if (conf_req_flag)
        {
            ...
            gss_trailerlen =
                16 /* E(Header) */ + k5_trailerlen; /* Kerb-Trailer */

            code = krb5_c_padding_length(context, enctype,
                                         data_length - assoc_data_length +
                                             16 /* E(Header) */,
                                         &k5_padlen);
            if (code != 0)
            {
    ...
    else if (!dce_or_mic)
    {
        ...
        if (k5_padlen == 1)
            ...
            gss_padlen =
                k5_padlen - ((data_length - assoc_data_length) % k5_padlen);
    }
    ...
    gss_qop_t qop_req, int* conf_state, gss_iov_buffer_desc* iov, int iov_count)
{
    ...
    major_status =
        kg_seal_iov(minor_status, context_handle, conf_req_flag, qop_req,
                    conf_state, iov, iov_count, KG_TOK_WRAP_MSG);
    ...
    gss_qop_t qop_req, int* conf_state, gss_iov_buffer_desc* iov, int iov_count)
{
    ...
    major_status =
        kg_seal_iov_length(minor_status, context_handle, conf_req_flag, qop_req,
                           conf_state, iov, iov_count, KG_TOK_WRAP_MSG);
    return major_status;
}
                                             ...
                                             int iov_count)
{
    ...
    major_status = kg_seal_iov(minor_status, context_handle, FALSE, qop_req,
                               NULL, iov, iov_count, KG_TOK_MIC_MSG);

    return major_status;
}

OM_uint32 KRB5_CALLCONV krb5_gss_get_mic_iov_length(OM_uint32* minor_status,
                                                    gss_ctx_id_t context_handle,
                                                    gss_qop_t qop_req,
                                                    gss_iov_buffer_desc* iov,
                                                    int iov_count)
{
    ...
    major_status =
        kg_seal_iov_length(minor_status, context_handle, FALSE, qop_req, NULL,
