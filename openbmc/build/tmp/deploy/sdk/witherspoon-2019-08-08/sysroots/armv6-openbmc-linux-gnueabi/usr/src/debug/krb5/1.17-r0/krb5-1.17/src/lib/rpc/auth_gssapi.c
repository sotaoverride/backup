...
static struct auth_ops auth_gssapi_ops = {
    auth_gssapi_nextverf, auth_gssapi_marshall, auth_gssapi_validate,
    auth_gssapi_refresh,  auth_gssapi_destroy,  auth_gssapi_wrap,
    auth_gssapi_unwrap,
};
...
AUTH* auth_gssapi_create_default(CLIENT* clnt, char* service_name)
{
    ...
    gssstat = gss_import_name(&minor_stat, &input_name, gss_nt_service_name,
                              &target_name);
    if (gssstat != GSS_S_COMPLETE)
    {
        AUTH_GSSAPI_DISPLAY_STATUS(("parsing name", gssstat, minor_stat));
        rpc_createerr.cf_stat = RPC_SYSTEMERROR;
        rpc_createerr.cf_error.re_errno = ENOMEM;
        return NULL;
    }

    auth = auth_gssapi_create(clnt, &gssstat, &minor_stat, GSS_C_NO_CREDENTIAL,
                              target_name, GSS_C_NULL_OID,
 ...
 * Effects: See design document, section XXX.
 */
AUTH* auth_gssapi_create(CLIENT* clnt, OM_uint32* gssstat,
                         OM_uint32* minor_stat,
                         gss_cred_id_t claimant_cred_handle,
                         gss_name_t target_name, gss_OID mech_type,
                         OM_uint32 req_flags, OM_uint32 time_req,
                         gss_OID* actual_mech_type, OM_uint32* ret_flags,
                         OM_uint32* time_rec)
{
    AUTH *auth, *save_auth;
...
try_new_version:
     ...
     * works.
     */
    if (call_arg.version < 4 &&
        (mech_type == gss_mech_krb5 || mech_type == GSS_C_NULL_OID))
...
next_token:
    *gssstat = gss_init_sec_context(
        minor_stat, claimant_cred_handle, &AUTH_PRIVATE(auth)->context,
        target_name, mech_type, req_flags, time_req, bindp, input_token,
        actual_mech_type, &call_arg.token, ret_flags, time_rec);

    if (*gssstat != GSS_S_COMPLETE && *gssstat != GSS_S_CONTINUE_NEEDED)
    {
        AUTH_GSSAPI_DISPLAY_STATUS(
            ("initializing context", *gssstat, *minor_stat));
        goto cleanup;
    }
    ...
    if (call_arg.token.length != 0)
    {
        ...
        memset(&call_res, 0, sizeof(call_res));
        callstat =
            clnt_call(clnt, init_func, xdr_authgssapi_init_arg, &call_arg,
                      xdr_authgssapi_init_res, &call_res, timeout);
    ...
    if (*gssstat == GSS_S_COMPLETE)
    {
        ...
        else
        {
            ...
            *gssstat = gss_unseal(minor_stat, AUTH_PRIVATE(auth)->context,
                                  &call_res.signed_isn, &isn_buf, NULL, NULL);

            if (*gssstat != GSS_S_COMPLETE)
            {
                AUTH_GSSAPI_DISPLAY_STATUS(
                    ("unsealing isn", *gssstat, *minor_stat));
                goto cleanup;
            }
            ...
            AUTH_PRIVATE(auth)->seq_num =
                (uint32_t)ntohl(*((uint32_t*)isn_buf.value));
            *gssstat = gss_release_buffer(minor_stat, &isn_buf);
            if (*gssstat != GSS_S_COMPLETE)
            {
                AUTH_GSSAPI_DISPLAY_STATUS(
                    ("releasing unsealed isn", *gssstat, *minor_stat));
                goto cleanup;
            }
    ...
    marshall_new_creds(auth, FALSE, &AUTH_PRIVATE(auth)->client_handle);
 ...
 * Modifies: auth
 */
static bool_t marshall_new_creds(AUTH* auth, bool_t auth_msg,
                                 gss_buffer_t client_handle)
{
    ...
    xdrmem_create(&xdrs, (caddr_t)AUTH_PRIVATE(auth)->cred_buf, MAX_AUTH_BYTES,
                  XDR_ENCODE);
 ...
 * them.
 */
static bool_t auth_gssapi_marshall(AUTH* auth, XDR* xdrs)
{
    OM_uint32 minor_stat;
 ...
 * Effects: See design document, section XXX.
 */
static bool_t auth_gssapi_validate(AUTH* auth, struct opaque_auth* verf)
{
 ...
 */
static bool_t auth_gssapi_refresh(AUTH* auth, struct rpc_msg* msg)
{
...
skip_call:
    ...
    gssstat =
        gss_delete_sec_context(&minor_stat, &AUTH_PRIVATE(auth)->context, NULL);
    if (gssstat != GSS_S_COMPLETE)
    ...
    if (AUTH_PRIVATE(auth)->def_cred)
    {
        ...
        if (gssstat != GSS_S_COMPLETE)
            AUTH_GSSAPI_DISPLAY_STATUS(
                ("deleting default credential", gssstat, minor_stat));
    }
 ...
 */
static bool_t auth_gssapi_wrap(AUTH* auth, XDR* out_xdrs, bool_t (*xdr_func)(),
                               caddr_t xdr_ptr)
{
    OM_uint32 gssstat, minor_stat;
    ...
    if (!AUTH_PRIVATE(auth)->established)
    {
        ...
        return (*xdr_func)(out_xdrs, xdr_ptr);
                 ...
                 &gssstat, &minor_stat, AUTH_PRIVATE(auth)->context,
                 AUTH_PRIVATE(auth)->seq_num + 1, out_xdrs, xdr_func, xdr_ptr))
    {
        if (gssstat != GSS_S_COMPLETE)
            AUTH_GSSAPI_DISPLAY_STATUS(
 ...
 * Effects: See design doc, section XXX.
 */
static bool_t auth_gssapi_unwrap(AUTH* auth, XDR* in_xdrs, bool_t (*xdr_func)(),
                                 caddr_t xdr_ptr)
{
    OM_uint32 gssstat, minor_stat;
    ...
    if (!AUTH_PRIVATE(auth)->established)
    {
        ...
        return (*xdr_func)(in_xdrs, xdr_ptr);
                 ...
                 &gssstat, &minor_stat, AUTH_PRIVATE(auth)->context,
                 AUTH_PRIVATE(auth)->seq_num, in_xdrs, xdr_func, xdr_ptr))
    {
        if (gssstat != GSS_S_COMPLETE)
            AUTH_GSSAPI_DISPLAY_STATUS(
