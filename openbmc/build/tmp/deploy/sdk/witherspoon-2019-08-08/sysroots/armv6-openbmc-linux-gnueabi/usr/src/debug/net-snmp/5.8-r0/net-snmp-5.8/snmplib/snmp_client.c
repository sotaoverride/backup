 ...
 */
netsnmp_variable_list* snmp_add_null_var(netsnmp_pdu* pdu, const oid* name,
                                         size_t name_length)
{
                            ...
                            netsnmp_pdu* pdu, void* magic)
{
    ...
    {
        DEBUGMSGTL(("snmp_synch",
                    "Unexpected response (ReqID: %d,%d - Cmd %d)\n", reqid,
                    state->reqid, pdu->command));
        return 0;
    }
    ...
    if (op == NETSNMP_CALLBACK_OP_RECEIVED_MESSAGE && pdu)
    {
        ...
        else
        {
            ...
            snprintf(msg_buf, sizeof(msg_buf),
                     "Expected RESPONSE-PDU but got %s-PDU",
...
int snmp_clone_var(netsnmp_variable_list* var, netsnmp_variable_list* newvar)
{
    ...
    if (var->val.string)
    {
        ...
        { /* fix the pointer to new local store */
            newvar->val.string = newvar->buf;
            /*
 ...
 */
static netsnmp_pdu* _clone_pdu_header(netsnmp_pdu* pdu)
{
    netsnmp_pdu* newpdu;
    ...
    if (pdu->securityStateRef && pdu->command == SNMP_MSG_TRAP2)
    {
        ...
        ret = usm_clone_usmStateReference(
 ...
 * Returns 0 if failure.
 */
static netsnmp_pdu* _clone_pdu(netsnmp_pdu* pdu, int drop_err)
{
    netsnmp_pdu* newpdu;
    newpdu = _clone_pdu_header(pdu);
    newpdu = _copy_pdu_vars(pdu, newpdu, drop_err, 0,
                            10000); /* skip none, copy all */
...
netsnmp_pdu* snmp_fix_pdu(netsnmp_pdu* pdu, int command)
{
    ...
    if ((pdu->command != SNMP_MSG_RESPONSE) ||
        (pdu->errstat == SNMP_ERR_NOERROR) || (NULL == pdu->variables) ||
        (pdu->errindex > (int)snmp_varbind_len(pdu)) || (pdu->errindex <= 0))
    {
...
int snmp_set_var_objid(netsnmp_variable_list* vp, const oid* objid,
                       size_t name_length)
{
...
int snmp_set_var_typed_integer(netsnmp_variable_list* newvar, u_char type,
                               long val)
{
    newvar->type = type;
...
int snmp_set_var_value(netsnmp_variable_list* vars, const void* value,
                       size_t len)
{
    int largeval = 1;
    ...
    switch (vars->type)
    {
        ...
        case ASN_UINTEGER:
            if (vars->val_len == sizeof(int))
            {
                ...
                {
                    const int* val_int = (const int*)value;
                    *(vars->val.integer) = (long)*val_int;
                }
                else
                {
                    const u_int* val_uint = (const u_int*)value;
            ...
            {
                const u_long* val_ulong = (const u_long*)value;
            ...
            {
                const uintmax_t* val_uintmax_t = (const uintmax_t*)value;
            ...
            else if (vars->val_len == sizeof(short))
            {
                if (ASN_INTEGER == vars->type)
                {
                    const short* val_short = (const short*)value;
                    *(vars->val.integer) = (long)*val_short;
                }
                else
                {
                    const u_short* val_ushort = (const u_short*)value;
            ...
            else if (vars->val_len == sizeof(char))
            {
                if (ASN_INTEGER == vars->type)
                {
                    const char* val_char = (const char*)value;
                    *(vars->val.integer) = (long)*val_char;
                }
                else
                {
                    const u_char* val_uchar = (const u_char*)value;
...
int snmp_synch_response_cb(netsnmp_session* ss, netsnmp_pdu* pdu,
                           netsnmp_pdu** response, snmp_callback pcb)
{
    ...
    while (state->waiting)
    {
        ...
        if (ss->flags & SNMP_FLAGS_RESP_CALLBACK)
        {
            ...
            cb(); /* Used to invoke
                     'netsnmp_check_outstanding_agent_requests();' on internal
                     AgentX queries.  */
        }
    }
    *response = state->pdu;
...
const char* snmp_errstring(int errstat)
{
    const char* const error_string[19] = {
        ...
        "(readOnly) The two parties used do not have access to use the "
        "specified SNMP PDU.",
        "(genError) A general failure occured",
        "noAccess",
        "wrongType (The set datatype does not match the data type the agent "
        "expects)",
        "wrongLength (The set value has an illegal length from what the agent "
        "expects)",
        "wrongEncoding",
        ...
        "inconsistentValue (The set value is illegal or unsupported in some "
        "way)",
        "resourceUnavailable (This is likely a out-of-memory failure within "
        "the agent)",
        "commitFailed",
        "undoFailed",
...
netsnmp_session* netsnmp_query_get_default_session(void)
{
    ...
    if (!_def_query_session && !warning_logged)
    {
        ...
        {
            snmp_log(LOG_WARNING, "iquerySecName has not been configured - "
                                  "internal queries will fail\n");
        }
        else
        {
            snmp_log(LOG_WARNING, "default session is not available - internal "
                                  "queries will fail\n");
        }
        warning_logged = 1;
    }
 ...
 */
static int _query(netsnmp_variable_list* list, int request,
...
retry:
    ...
    if (ret == SNMP_ERR_NOERROR)
    {
...
#ifndef NETSNMP_NO_WRITE_SUPPORT
            if (request != SNMP_MSG_SET && response->errindex != 0)
            {
        ...
        {
            for (vb1 = response->variables, vb2 = list; vb1;
...
int netsnmp_query_walk(netsnmp_variable_list* list, netsnmp_session* session)
{
    ...
    while (ret == SNMP_ERR_NOERROR &&
           snmp_oidtree_compare(list->name, list->name_length, vb->name,
                                vb->name_length) == 0)
    {

        if (vb->type == SNMP_ENDOFMIBVIEW || vb->type == SNMP_NOSUCHOBJECT ||
...
int netsnmp_state_machine_run(netsnmp_state_machine_input* input)
{
    ...
    while (current)
    {
        ...
        {
            DEBUGMSGT(
                ("state_machine:run:result", "ABORT from %s\n", current->name));
            break;
        }
        else
...
static netsnmp_variable_list* _get_vb_num(netsnmp_variable_list* vars,
                                          int index)
{
 ...
 * cleanup
 */
static int _row_status_state_cleanup(netsnmp_state_machine_input* input,
                                     netsnmp_state_machine_step* step)
{
    ...
    DEBUGMSGT(("row_create:called",
               "_row_status_state_cleanup, last run step was %s rc %d\n",
 ...
 */
static int _row_status_state_activate(netsnmp_state_machine_input* input,
                                      netsnmp_state_machine_step* step)
{
    rowcreate_state* ctx;
                                           ...
                                           netsnmp_state_machine_step* step)
{
    ...
    {
        for (last = vars, var = last->next_variable; var;
 ...
 */
static int _row_status_state_single_value_createAndWait(
    netsnmp_state_machine_input* input, netsnmp_state_machine_step* step)
{
    rowcreate_state* ctx;
 ...
 */
static int _row_status_state_all_values_createAndWait(
    netsnmp_state_machine_input* input, netsnmp_state_machine_step* step)
{
    rowcreate_state* ctx;
 ...
 *
 */
int netsnmp_row_create(netsnmp_session* sess, netsnmp_variable_list* vars,
                       int row_status_index)
{
    netsnmp_state_machine_step rc_cleanup = {"row_create_cleanup",
                                             0,
                                             _row_status_state_cleanup,
                                             0,
                                             NULL,
                                             NULL,
                                             0,
                                             NULL};
    netsnmp_state_machine_step rc_activate = {"row_create_activate",
                                              0,
    ...
    netsnmp_state_machine_step rc_sv_cols = {
        "row_create_single_value_cols",
        0,
        _row_status_state_single_value_cols,
        0,
        &rc_activate,
        NULL,
        0,
    ...
    netsnmp_state_machine_step rc_mv_cols = {
        "row_create_multiple_values_cols",
        0,
        _row_status_state_multiple_values_cols,
        0,
        &rc_activate,
        &rc_sv_cols,
        0,
    ...
    netsnmp_state_machine_step rc_sv_caw = {
        "row_create_single_value_createAndWait",
        0,
        _row_status_state_single_value_createAndWait,
        0,
        &rc_mv_cols,
        NULL,
        0,
    ...
    netsnmp_state_machine_step rc_av_caw = {
        "row_create_all_values_createAndWait",
        0,
        _row_status_state_all_values_createAndWait,
        0,
        &rc_activate,
        &rc_sv_caw,
        0,
    ...
    netsnmp_state_machine_step rc_av_cag = {
        ...
        0,
        _row_status_state_all_values_createAndGo,
        0,
        NULL,
        &rc_av_caw,
        0,
    ...
    netsnmp_state_machine_input sm_input = {"row_create_machine", 0, &rc_av_cag,
                                            &rc_cleanup};
