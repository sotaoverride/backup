...
char* netsnmp_udp_fmtaddr(netsnmp_transport* t, const void* data, int len)
{
    ...
    (defined(HAVE_IP_RECVDSTADDR) && defined(HAVE_IP_SENDSRCADDR))
...
static netsnmp_transport* netsnmp_udp_transport_base(netsnmp_transport* t)
{
    if (NULL == t)
    {
        return NULL;
...
int netsnmp_udp_com2SecEntry_create(com2SecEntry** entryp,
                                    const char* community, const char* secName,
                                    const char* contextName,
                                    struct in_addr* network,
                                    struct in_addr* mask, int negate)
{
    ...
    DEBUGIF("netsnmp_udp_parse_security")
    {
        ...
        DEBUGMSGTL(("netsnmp_udp_parse_security", "<\"%s\", %s/%s> => \"%s\"\n",
                    community, inet_ntop(AF_INET, network, buf1, sizeof(buf1)),
                    inet_ntop(AF_INET, mask, buf2, sizeof(buf2)), secName));
    }
...
void netsnmp_udp_agent_config_tokens_register(void)
{
    ...
    register_app_config_handler(
        "com2sec", netsnmp_udp_parse_security, netsnmp_udp_com2SecList_free,
...
#if !defined(NETSNMP_DISABLE_SNMPV1) || !defined(NETSNMP_DISABLE_SNMPV2C)
int netsnmp_udp_getSecName(void* opaque, int olength, const char* community,
                           ...
                           const char** contextName)
{
    ...
    {
        DEBUGMSGTL(
            ("netsnmp_udp_getSecName", "no IPv4 source address in PDU?\n"));
        return 1;
    }
    ...
    for (c = com2SecList; c != NULL; c = c->next)
    {
            ...
            ((from->sin_addr.s_addr & c->mask) == c->network))
        {
            ...
            {
                /*
                 * If we matched a negative entry, then we are done - claim that
                 * we matched nothing.
                 */
...
netsnmp_transport* netsnmp_udp_create_tstring(const char* str, int local,
                                              const char* default_target)
{
...
netsnmp_transport* netsnmp_udp_create_ostring(const void* o, size_t o_len,
                                              int local)
{
