...
#include <boost/mpl/if.hpp>
    ...
    (BOOST_MPL_CFG_GCC != 0) || BOOST_WORKAROUND(__IBMCPP__, <= 600)
...
// agurt, 10/nov/06: use enums for Borland (which cannot cope with static
// constants) and GCC (which issues "unused variable" warnings when static
// constants are used at a function scope)
#if BOOST_WORKAROUND(__BORLANDC__, BOOST_TESTED_AT(0x610)) ||                  \
...
template <bool (*)(failed, failed), long x, long y>
struct assert_relation
{
};
#define BOOST_MPL_AUX_ASSERT_RELATION(x, y, r) assert_relation<r, x, y>
#else
template <BOOST_MPL_AUX_NTTP_DECL(long, x), BOOST_MPL_AUX_NTTP_DECL(long, y),
          bool (*)(failed, failed)>
struct assert_relation
{
};
...
struct eval_assert
{
    ...
    typedef typename ::boost::mpl::if_c<
        p_type::value, AUX778076_ASSERT_ARG(assert<false>),
        failed*********** * P::************>::type type;
};
...
struct eval_assert_not
{
    ...
    typedef typename ::boost::mpl::if_c<
        !p_type::value, AUX778076_ASSERT_ARG(assert<false>),
        failed************ ::boost::mpl::not_<P>::************>::type type;
};
...
failed************(Pred::************ assert_arg(
    void (*)(Pred), typename assert_arg_pred<Pred>::type));

template <typename Pred>
failed************(boost::mpl::not_<Pred>::************ assert_not_arg(
...
struct assert_arg_type
    : assert_arg_type_impl<BOOST_MPL_AUX_VALUE_WKND(
                               BOOST_MPL_AUX_NESTED_TYPE_WKND(Pred))::value,
                           Pred>
{
};

template <typename Pred>
typename assert_arg_type<Pred>::type assert_arg(void (*)(Pred), int);
...
typename assert_arg_type_impl<false,
                              BOOST_MPL_AUX_ASSERT_RELATION(x, y, r)>::type
...
#define BOOST_MPL_ASSERT(pred)                                                 \
    BOOST_MPL_AUX_ASSERT_CONSTANT(                                             \
        std::size_t,                                                           \
        BOOST_PP_CAT(mpl_assertion_in_line_, BOOST_MPL_AUX_PP_COUNTER()) =     \
...
#define BOOST_MPL_ASSERT_NOT(pred)                                             \
    BOOST_MPL_AUX_ASSERT_CONSTANT(                                             \
        ...
        BOOST_PP_CAT(mpl_assertion_in_line_, BOOST_MPL_AUX_PP_COUNTER()) =     \
            sizeof(boost::mpl::assertion_failed<false>(                        \
                boost::mpl::make_assert_arg<                                   \
                    typename boost::mpl::eval_assert_not<                      \
                        void pred>::type>())))                                 \
    /**/
...
#define BOOST_MPL_ASSERT_NOT(pred)                                             \
    ...
    {                                                                          \
        BOOST_PP_CAT(mpl_assertion_in_line_, BOOST_MPL_AUX_PP_COUNTER()) =     \
            sizeof(boost::mpl::assertion<false>::failed(                       \
...
// agurt, 9/nov/06: 'enum' below is a workaround for gcc 4.0.4/4.1.1 bugs #29522
// and #29518
#define BOOST_MPL_ASSERT_RELATION_IMPL(counter, x, rel, y)                     \
    enum                                                                       \
    {                                                                          \
        BOOST_PP_CAT(mpl_assert_rel_value, counter) = (x rel y)                \
    };                                                                         \
    BOOST_MPL_AUX_ASSERT_CONSTANT(                                             \
        std::size_t,                                                           \
        BOOST_PP_CAT(mpl_assertion_in_line_, counter) =                        \
            sizeof(boost::mpl::assertion_failed<BOOST_PP_CAT(                  \
                       mpl_assert_rel_value, counter)>(                        \
                (boost::mpl::failed *                                          \
                 ***********(boost::mpl::assert_relation<                      \
                             boost::mpl::assert_::relations(                   \
                                 sizeof(boost::mpl::assert_::arg rel           \
                                            boost::mpl::assert_::arg)),        \
                             x, y>::************))0)))                         \
/**/
#else
#define BOOST_MPL_ASSERT_RELATION_IMPL(counter, x, rel, y)                     \
    BOOST_MPL_AUX_ASSERT_CONSTANT(                                             \
        std::size_t,                                                           \
        BOOST_PP_CAT(mpl_assert_rel, counter) =                                \
            sizeof(boost::mpl::assert_::arg rel boost::mpl::assert_::arg));    \
    BOOST_MPL_AUX_ASSERT_CONSTANT(                                             \
        bool, BOOST_PP_CAT(mpl_assert_rel_value, counter) = (x rel y));        \
    BOOST_MPL_AUX_ASSERT_CONSTANT(                                             \
        std::size_t,                                                           \
        BOOST_PP_CAT(mpl_assertion_in_line_, counter) =                        \
            sizeof(boost::mpl::assertion_failed<BOOST_PP_CAT(                  \
                       mpl_assert_rel_value, counter)>(                        \
                boost::mpl::assert_rel_arg(                                    \
                    boost::mpl::assert_relation<                               \
                        boost::mpl::assert_::relations(                        \
                            BOOST_PP_CAT(mpl_assert_rel, counter)),            \
                        x, y>()))))                                            \
...
#define BOOST_MPL_ASSERT_RELATION(x, rel, y)                                   \
    BOOST_MPL_AUX_ASSERT_CONSTANT(                                             \
        std::size_t,                                                           \
        BOOST_PP_CAT(mpl_assertion_in_line_, BOOST_MPL_AUX_PP_COUNTER()) =     \
            sizeof(boost::mpl::assertion_failed<(x rel y)>(                    \
                boost::mpl::assert_rel_arg(                                    \
                    boost::mpl::BOOST_MPL_AUX_ASSERT_RELATION(                 \
    ...
    BOOST_MPL_AUX_ASSERT_CONSTANT(                                             \
        ...
        BOOST_PP_CAT(mpl_assertion_in_line_, BOOST_MPL_AUX_PP_COUNTER()) =     \
            sizeof(boost::mpl::assertion_failed<(x rel y)>(                    \
                (boost::mpl::failed *                                          \
                 ***********(boost::mpl::BOOST_MPL_AUX_ASSERT_RELATION(        \
                     x, y, (&boost::mpl::operator rel))::************))0)))    \
/**/
#endif

#endif

// BOOST_MPL_ASSERT_MSG( (pred<x,...>::value), USER_PROVIDED_MESSAGE,
// (types<x,...>) )
...
#define BOOST_MPL_ASSERT_MSG_IMPL(counter, c, msg, types_)                     \
    ...
    BOOST_MPL_AUX_ASSERT_CONSTANT(                                             \
        std::size_t,                                                           \
        BOOST_PP_CAT(mpl_assertion_in_line_, counter) =                        \
            sizeof(boost::mpl::assertion<(c)>::failed(                         \
...
#define BOOST_MPL_ASSERT_MSG_IMPL(counter, c, msg, types_)                     \
    ...
    BOOST_MPL_AUX_ASSERT_CONSTANT(                                             \
        std::size_t,                                                           \
        BOOST_PP_CAT(mpl_assertion_in_line_, counter) =                        \
            sizeof(boost::mpl::assertion_failed<(c)>(                          \
                BOOST_PP_CAT(mpl_assert_arg, counter)::assert_arg())))         \
/**/
#endif
