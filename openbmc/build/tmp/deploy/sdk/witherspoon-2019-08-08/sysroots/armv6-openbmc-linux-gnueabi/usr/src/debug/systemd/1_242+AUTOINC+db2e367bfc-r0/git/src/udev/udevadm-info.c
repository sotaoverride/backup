...
static int print_device_chain(sd_device* device)
{
    ...
    for (child = device; sd_device_get_parent(child, &parent) >= 0;
         child = parent)
    {
...
static int stat_device(const char* name, bool export, const char* prefix)
{
    ...
    if (export)
    {
        ...
        printf("%sMAJOR=%u\n"
               "%sMINOR=%u\n",
               prefix, major(statbuf.st_dev), prefix, minor(statbuf.st_dev));
    }
    else
...
{
    _cleanup_closedir_ DIR *dir1 = NULL, *dir2 = NULL, *dir3 = NULL,
                           *dir4 = NULL, *dir5 = NULL;
...
static int help(void)
{
    printf("%s info [OPTIONS] [DEVPATH|FILE]\n\n"
           ...
           "       all                      All values\n"
           "  -p --path=SYSPATH           sysfs device path used for query or "
           "attribute walk\n"
           "  -n --name=NAME              Node or symlink name used for query "
           "or attribute walk\n"
           "  -r --root                   Prepend dev directory to path names\n"
           "  -a --attribute-walk         Print all key matches walking along "
           "the chain\n"
           "                              of parent devices\n"
           "  -d --device-id-of-file=FILE Print major:minor of device "
           "containing this file\n"
           "  -x --export                 Export key/value pairs\n"
           "  -P --export-prefix          Export the key name with a prefix\n"
           "  -e --export-db              Export the content of the udev "
           "database\n"
...
int info_main(int argc, char* argv[], void* userdata)
{
    ...
    while ((c = getopt_long(argc, argv, "aced:n:p:q:rxP:Vh", options, NULL)) >=
           0)
        switch (c)
        {
            case 'n':
            case 'p':
            {
                ...
                path = path_join(
                    path_startswith(optarg, prefix) ? NULL : prefix, optarg);
                if (!path)
                    return log_oom();
    ...
    if (action == ACTION_DEVICE_ID_FILE)
    {
        if (argv[optind])
            return log_error_errno(SYNTHETIC_ERRNO(EINVAL),
                                   "Positional arguments are not allowed with "
                                   "-d/--device-id-of-file.");
        assert(name);
    ...
    STRV_FOREACH(p, devices)
    {
        ...
        if (r == -EINVAL)
            return log_error_errno(r,
                                   "Bad argument \"%s\", expected an absolute "
                                   "path in /dev/ or /sys or a unit name: %m",
                                   *p);
        if (r < 0)
