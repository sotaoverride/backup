...
#define BOOST_GRAPH_MEMBER_OR_VOID(name)                                       \
    ...
    template <typename T>                                                      \
    struct BOOST_JOIN(get_member_, name)                                       \
    {                                                                          \
        typedef typename T::name type;                                         \
    };                                                                         \
    template <typename T>                                                      \
    struct BOOST_JOIN(get_opt_member_, name) :                                 \
        boost::mpl::eval_if_c<BOOST_JOIN(has_, name) < T>::value,              \
        BOOST_JOIN(get_member_, name)<T>, boost::mpl::identity<void>>          \
    {                                                                          \
    };
...
namespace detail
{
inline bool is_directed(directed_tag)
{
    return true;
}
inline bool is_directed(undirected_tag)
{
    return false;
}
} // namespace detail
...
struct is_directed_tag : mpl::bool_<is_convertible<Tag, directed_tag>::value>
{
};
} // namespace graph_detail

template <typename Graph>
struct is_directed_graph : graph_detail::is_directed_tag<
...
namespace detail
{
inline bool allows_parallel(allow_parallel_edge_tag)
{
    return true;
}
inline bool allows_parallel(disallow_parallel_edge_tag)
{
    return false;
}
} // namespace detail
...
template <typename G>
struct get_graph_property_type
{
    typedef typename G::graph_property_type type;
};
template <typename G>
struct get_edge_property_type
{
    typedef typename G::edge_property_type type;
};
template <typename G>
struct get_vertex_property_type
{
...
template <typename Graph>
struct has_edge_property : mpl::not_<typename detail::is_no_property<
...
T source(pair<T, T> p, const G&)
{
    return p.first;
}

template <class T, class G>
T target(pair<T, T> p, const G&)
{
    return p.second;
}
