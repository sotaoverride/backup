 ...
 * Since: 2.32
 */
GBytes* g_bytes_new_with_free_func(gconstpointer data, gsize size,
...
GBytes* g_bytes_new_from_bytes(GBytes* bytes, gsize offset, gsize length)
{
    ...
    g_return_val_if_fail(base + length <= (gchar*)bytes->data + bytes->size,
                         NULL);

    return g_bytes_new_with_free_func(
        base, length, (GDestroyNotify)g_bytes_unref, g_bytes_ref(bytes));
}
 ...
 * (nullable): a pointer to the byte data, or %NULL
 *
 * Since: 2.32
 */
...
guint g_bytes_hash(gconstpointer bytes)
{
    ...
    for (p = (signed char*)a->data, e = (signed char*)a->data + a->size; p != e;
         p++)
        h = (h << 5) + h + *p;
...
static gpointer try_steal_and_unref(GBytes* bytes, GDestroyNotify free_func,
                                    gsize* size)
{
    gpointer result;
 ...
 *
 * Returns: (transfer full): a new mutable #GByteArray containing the same byte
 * data
 *
 * Since: 2.32
 */
