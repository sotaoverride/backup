...
/* Pick a good default that is likely to fit into AF_UNIX and AF_INET SOCK_DGRAM
 * datagrams, and even leaves some room for a bit of additional metadata. */
#define DEFAULT_LINE_MAX (48 * 1024)

#define DEFERRED_CLOSES_MAX (4096)

static int determine_path_usage(Server* s, const char* path, uint64_t* ret_used,
                                uint64_t* ret_free)
{
    ...
    *ret_used = 0;
    FOREACH_DIRENT_ALL(de, d, break)
    {
        ...
        {
            log_debug_errno(errno, "Failed to stat %s/%s, ignoring: %m", path,
                            de->d_name);
            continue;
        }
...
static int cache_space_refresh(Server* s, JournalStorage* storage)
{
    ...
    space->limit =
        MIN(MAX(vfs_used + avail, metrics->min_use), metrics->max_use);
...
static void patch_min_use(JournalStorage* storage)
{
    ...
    storage->metrics.min_use =
        MAX(storage->metrics.min_use, storage->space.vfs_used);
}
...
void server_space_usage_message(Server* s, JournalStorage* storage)
{
    ...
    server_driver_message(
        s, 0, "MESSAGE_ID=" SD_MESSAGE_JOURNAL_USAGE_STR,
        LOG_MESSAGE("%s (%s) is %s, max %s, %s free.", storage->name,
                    storage->path, fb1, fb5, fb6),
        "JOURNAL_NAME=%s", storage->name, "JOURNAL_PATH=%s", storage->path,
        "CURRENT_USE=%" PRIu64, storage->space.vfs_used,
        "CURRENT_USE_PRETTY=%s", fb1, "MAX_USE=%" PRIu64, metrics->max_use,
        "MAX_USE_PRETTY=%s", fb2, "DISK_KEEP_FREE=%" PRIu64, metrics->keep_free,
        "DISK_KEEP_FREE_PRETTY=%s", fb3, "DISK_AVAILABLE=%" PRIu64,
        storage->space.vfs_available, "DISK_AVAILABLE_PRETTY=%s", fb4,
        "LIMIT=%" PRIu64, storage->space.limit, "LIMIT_PRETTY=%s", fb5,
        "AVAILABLE=%" PRIu64, storage->space.available, "AVAILABLE_PRETTY=%s",
        fb6, NULL);
}
...
static bool uid_for_system_journal(uid_t uid)
{
    ...
    /* Returns true if the specified UID shall get its data stored in the system
     * journal*/
...
static int open_journal(Server* s, bool reliably, const char* fname, int flags,
                        bool seal, JournalMetrics* metrics, JournalFile** ret)
{
    ...
    if (reliably)
        r = journal_file_open_reliably(fname, flags, 0640, s->compress.enabled,
                                       s->compress.threshold_bytes, seal,
                                       metrics, s->mmap, s->deferred_closes,
                                       NULL, &f);
    else
        r = journal_file_open(-1, fname, flags, 0640, s->compress.enabled,
                              s->compress.threshold_bytes, seal, metrics,
                              s->mmap, s->deferred_closes, NULL, &f);

    if (r < 0)
        return r;

    r = journal_file_enable_post_change_timer(f, s->event,
...
static int system_journal_open(Server* s, bool flush_requested)
{
        ...
        (flush_requested || flushed_flag_is_set()))
    {
        ...
        r = open_journal(s, true, fn, O_RDWR | O_CREAT, s->seal,
                         &s->system_storage.metrics, &s->system_journal);
        if (r >= 0)
        {
    ...
    if (!s->runtime_journal && (s->storage != STORAGE_NONE))
    {
        ...
        if (s->system_journal)
        {
            ...
            r = open_journal(s, false, fn, O_RDWR, false,
                             &s->runtime_storage.metrics, &s->runtime_journal);
            if (r < 0)
            {
                if (r != -ENOENT)
        ...
        else
        {
            ...
            r = open_journal(s, true, fn, O_RDWR | O_CREAT, false,
                             &s->runtime_storage.metrics, &s->runtime_journal);
            if (r < 0)
...
static JournalFile* find_journal(Server* s, uid_t uid)
{
    ...
    r = open_journal(s, true, p, O_RDWR | O_CREAT, s->seal,
                     &s->system_storage.metrics, &f);
    if (r < 0)
        return s->system_journal;
...
static int do_rotate(Server* s, JournalFile** f, const char* name, bool seal,
                     uint32_t uid)
{
    ...
    r = journal_file_rotate(f, s->compress.enabled, s->compress.threshold_bytes,
                            seal, s->deferred_closes);
    if (r < 0)
    {
        if (*f)
            return log_error_errno(r, "Failed to rotate %s: %m", (*f)->path);
        else
            return log_error_errno(r, "Failed to create new %s journal: %m",
                                   name);
    }
...
static void server_vacuum_deferred_closes(Server* s)
{
    ...
    /* Make some room in the deferred closes list, so that it doesn't grow
     * without bounds */
    if (set_size(s->deferred_closes) < DEFERRED_CLOSES_MAX)
        return;

    /* Let's first remove all journal files that might already have completed
     * closing */
...
static int open_user_journal_directory(Server* s, DIR** ret_dir,
                                       char** ret_path)
{
    ...
    if (asprintf(&path, "/var/log/journal/" SD_ID128_FORMAT_STR "/",
    ...
    if (!dir)
        return log_error_errno(
            errno, "Failed to open user journal directory '%s': %m", path);
...
void server_rotate(Server* s)
{
    ...
    /* First, rotate the system journal (either in its runtime flavour or in its
     * runtime flavour) */
     ...
     * (But do so only if we actually have access to /var, i.e. are not in the
     * log-to-runtime-journal mode). */
    if (!s->runtime_journal && open_user_journal_directory(s, &d, &path) >= 0)
    {
        ...
        FOREACH_DIRENT(de, d,
                       log_warning_errno(
                           errno, "Failed to enumerate %s, ignoring: %m", path))
        {
            ...
            {
                log_debug_errno(
                    r, "Failed to parse UID from file name '%s', ignoring: %m",
                    de->d_name);
                continue;
            }

            /* Already rotated in the above loop? i.e. is it an open user
             * journal? */
            ...
            fd =
                openat(dirfd(d), de->d_name,
                       O_RDWR | O_CLOEXEC | O_NOCTTY | O_NOFOLLOW | O_NONBLOCK);
            if (fd < 0)
            {
                log_full_errno(
                    IN_SET(errno, ELOOP, ENOENT) ? LOG_DEBUG : LOG_WARNING,
                    errno, "Failed to open journal file '%s' for rotation: %m",
                    full);
                continue;
            }
            ...
            r = journal_file_open(fd, full, O_RDWR, 0640, s->compress.enabled,
                                  s->compress.threshold_bytes, s->seal,
                                  &s->system_storage.metrics, s->mmap,
                                  s->deferred_closes, NULL, &f);
            if (r < 0)
            {
                log_warning_errno(
                    r,
                    "Failed to read journal file %s for rotation, trying to "
                    "move it out of the way: %m",
                    full);

                r = journal_file_dispose(dirfd(d), de->d_name);
                if (r < 0)
                    log_warning_errno(
                        r, "Failed to move %s out of the way, ignoring: %m",
                        full);
                else
            ...
            if (r < 0)
                log_debug_errno(
                    r, "Failed to archive journal file '%s', ignoring: %m",
                    full);
...
static void do_vacuum(Server* s, JournalStorage* storage, bool verbose)
{
    ...
    r = journal_directory_vacuum(
        storage->path, storage->space.limit, storage->metrics.n_max_files,
        s->max_retention_usec, &s->oldest_file_usec, verbose);
    if (r < 0 && r != -ENOENT)
        log_warning_errno(r, "Failed to vacuum %s, ignoring: %m",
                          storage->path);
...
static bool shall_try_append_again(JournalFile* f, int r)
{
    switch (r)
    {
        ...
        case -ETXTBSY: /* Journal file is from the future */
            log_warning("%s: Journal file is from the future, rotating.",
                        f->path);
            return true;
...
static void write_to_journal(Server* s, uid_t uid, struct iovec* iovec,
                             size_t n, int priority)
{
    ...
    /* Get the closest, linearized time we have for this log event from the
     * event loop. (Note that we do not use the source time, and not even the
     * time the event was originally seen, but instead simply the time we
     * started processing it, as we want strictly linear ordering in what we
     * write out.) */
    ...
    {
        /* When the time jumps backwards, let's immediately rotate. Of course,
         * this should not happen during regular operation. However, when it
         * does happen, then we should make sure that we start fresh files to
         * ensure that the entries in the journal files are strictly ordered by
    ...
    else
    {
        ...
        {
            log_debug("%s: Journal header limits reached or header "
    ...
    r = journal_file_append_entry(f, &ts, NULL, iovec, n, &s->seqnum, NULL,
                                  NULL);
    if (r >= 0)
    {
    ...
    {
        log_error_errno(
            r, "Failed to write entry (%zu items, %zu bytes), ignoring: %m", n,
    ...
    if (r < 0)
        log_error_errno(r,
                        "Failed to write entry (%zu items, %zu bytes) despite "
                        "vacuuming, ignoring: %m",
...
static void dispatch_message_real(Server* s, struct iovec* iovec, size_t n,
                                  size_t m, const ClientContext* c,
                                  const struct timeval* tv, int priority,
                                  pid_t object_pid)
{

    char source_time[sizeof("_SOURCE_REALTIME_TIMESTAMP=") +
    ...
    if (c)
    {
        IOVEC_ADD_NUMERIC_FIELD(iovec, n, c->pid, pid_t, pid_is_valid, PID_FMT,
                                "_PID");
        IOVEC_ADD_NUMERIC_FIELD(iovec, n, c->uid, uid_t, uid_is_valid, UID_FMT,
                                "_UID");
        IOVEC_ADD_NUMERIC_FIELD(iovec, n, c->gid, gid_t, gid_is_valid, GID_FMT,
                                "_GID");

        IOVEC_ADD_STRING_FIELD(
            iovec, n, c->comm,
        ...
        if (c->cmdline)
            /* At most _SC_ARG_MAX (2MB usually), which is too much to put on
             * stack. Let's use a heap allocation for this one. */
            cmdline1 =
                set_iovec_string_field(iovec, &n, "_CMDLINE=", c->cmdline);

        IOVEC_ADD_STRING_FIELD(
            iovec, n, c->capeff,
            "_CAP_EFFECTIVE"); /* Read from /proc/.../status */
        IOVEC_ADD_SIZED_FIELD(iovec, n, c->label, c->label_size,
                              "_SELINUX_CONTEXT");
        IOVEC_ADD_NUMERIC_FIELD(iovec, n, c->auditid, uint32_t,
                                audit_session_is_valid, "%" PRIu32,
                                "_AUDIT_SESSION");
        IOVEC_ADD_NUMERIC_FIELD(iovec, n, c->loginuid, uid_t, uid_is_valid,
                                UID_FMT, "_AUDIT_LOGINUID");

        IOVEC_ADD_STRING_FIELD(iovec, n, c->cgroup,
                               "_SYSTEMD_CGROUP"); /* A path */
        IOVEC_ADD_STRING_FIELD(iovec, n, c->session, "_SYSTEMD_SESSION");
        IOVEC_ADD_NUMERIC_FIELD(iovec, n, c->owner_uid, uid_t, uid_is_valid,
                                UID_FMT, "_SYSTEMD_OWNER_UID");
        IOVEC_ADD_STRING_FIELD(
            iovec, n, c->unit,
        ...
        IOVEC_ADD_ID128_FIELD(iovec, n, c->invocation_id,
                              "_SYSTEMD_INVOCATION_ID");

        if (c->extra_fields_n_iovec > 0)
        {
            memcpy(iovec + n, c->extra_fields_iovec,
    ...
    if (pid_is_valid(object_pid) &&
        client_context_get(s, object_pid, NULL, NULL, 0, NULL, &o) >= 0)
    {

        IOVEC_ADD_NUMERIC_FIELD(iovec, n, o->pid, pid_t, pid_is_valid, PID_FMT,
                                "OBJECT_PID");
        IOVEC_ADD_NUMERIC_FIELD(iovec, n, o->uid, uid_t, uid_is_valid, UID_FMT,
                                "OBJECT_UID");
        IOVEC_ADD_NUMERIC_FIELD(iovec, n, o->gid, gid_t, gid_is_valid, GID_FMT,
                                "OBJECT_GID");

        /* See above for size limits, only ->cmdline may be large, so use a heap
         * allocation for it. */
        IOVEC_ADD_STRING_FIELD(iovec, n, o->comm, "OBJECT_COMM");
        IOVEC_ADD_STRING_FIELD(iovec, n, o->exe, "OBJECT_EXE");
        if (o->cmdline)
            cmdline2 = set_iovec_string_field(iovec, &n,
                                              "OBJECT_CMDLINE=", o->cmdline);

        IOVEC_ADD_STRING_FIELD(iovec, n, o->capeff, "OBJECT_CAP_EFFECTIVE");
        IOVEC_ADD_SIZED_FIELD(iovec, n, o->label, o->label_size,
                              "OBJECT_SELINUX_CONTEXT");
        IOVEC_ADD_NUMERIC_FIELD(iovec, n, o->auditid, uint32_t,
                                audit_session_is_valid, "%" PRIu32,
                                "OBJECT_AUDIT_SESSION");
        IOVEC_ADD_NUMERIC_FIELD(iovec, n, o->loginuid, uid_t, uid_is_valid,
                                UID_FMT, "OBJECT_AUDIT_LOGINUID");
        ...
        IOVEC_ADD_NUMERIC_FIELD(iovec, n, o->owner_uid, uid_t, uid_is_valid,
                                UID_FMT, "OBJECT_SYSTEMD_OWNER_UID");
        IOVEC_ADD_STRING_FIELD(iovec, n, o->unit, "OBJECT_SYSTEMD_UNIT");
        IOVEC_ADD_STRING_FIELD(iovec, n, o->user_unit,
                               "OBJECT_SYSTEMD_USER_UNIT");
        IOVEC_ADD_STRING_FIELD(iovec, n, o->slice, "OBJECT_SYSTEMD_SLICE");
        IOVEC_ADD_STRING_FIELD(iovec, n, o->user_slice,
                               "OBJECT_SYSTEMD_USER_SLICE");

        IOVEC_ADD_ID128_FIELD(iovec, n, o->invocation_id,
    ...
    if (tv)
    {
        sprintf(source_time, "_SOURCE_REALTIME_TIMESTAMP=" USEC_FMT,
                timeval_load(tv));
    ...
    if (s->split_mode == SPLIT_UID && c && uid_is_valid(c->uid))
        ...
        journal_uid = c->uid;
    else if (s->split_mode == SPLIT_LOGIN && c && c->uid > 0 &&
             uid_is_valid(c->owner_uid))
...
void server_driver_message(Server* s, pid_t object_pid, const char* message_id,
                           const char* format, ...)
{
    ...
    m = N_IOVEC_META_FIELDS + 5 + N_IOVEC_PAYLOAD_FIELDS +
        client_context_extra_fields_n_iovec(s->my_context) +
    ...
    if (r >= 0)
        dispatch_message_real(s, iovec, n, m, s->my_context, NULL, LOG_INFO,
                              object_pid);
...
void server_dispatch_message(Server* s, struct iovec* iovec, size_t n, size_t m,
                             ClientContext* c, const struct timeval* tv,
                             int priority, pid_t object_pid)
{
    ...
    if (c && c->unit)
    {
        ...
        rl = journal_rate_limit_test(
            s->rate_limit, c->unit, c->log_rate_limit_interval,
            c->log_rate_limit_burst, priority & LOG_PRIMASK, available);
        if (rl == 0)
            return;
        ...
        if (rl > 1)
            server_driver_message(
                s, c->pid, "MESSAGE_ID=" SD_MESSAGE_JOURNAL_DROPPED_STR,
                LOG_MESSAGE("Suppressed %i messages from %s", rl - 1, c->unit),
                "N_DROPPED=%i", rl - 1, NULL);
    }
...
int server_flush_to_var(Server* s, bool require_flag_file)
{
    ...
    SD_JOURNAL_FOREACH(j)
    {
        ...
        if (!s->system_journal)
        {
            log_notice("Didn't flush runtime journal since rotation of system "
                       "journal wasn't successful.");
            r = -EIO;
            goto finish;
        }
                            ...
                            void* userdata)
{
    ...
    /* Try to get the right size, if we can. (Not all sockets support SIOCINQ,
     * hence we just try, but don't rely on it.) */
    (void)ioctl(fd, SIOCINQ, &v);

    /* Fix it up, if it is too small. We use the same fixed value as auditd
     * here. Awful! */
    m = PAGE_ALIGN(MAX3((size_t)v + 1, (size_t)LINE_MAX,
    ...
    {
        if (n > 0 && n_fds == 0)
            server_process_syslog_message(s, s->buffer, n, ucred, tv, label,
                                          label_len);
        else if (n_fds > 0)
    ...
    else
    {
        ...
        if (n > 0 && n_fds == 0)
            server_process_audit_message(s, s->buffer, n, ucred, &sa,
                                         msghdr.msg_namelen);
        else if (n_fds > 0)
...
static int dispatch_sigusr1(sd_event_source* es,
                            const struct signalfd_siginfo* si, void* userdata)
{
    ...
    log_info("Received request to flush runtime journal from PID " PID_FMT,
             si->ssi_pid);
    ...
    if (r < 0)
        log_warning_errno(
            r, "Failed to touch /run/systemd/journal/flushed, ignoring: %m");

    server_space_usage_message(s, NULL);
    return 0;
}

static int dispatch_sigusr2(sd_event_source* es,
                            const struct signalfd_siginfo* si, void* userdata)
{
    Server* s = userdata;
    int r;
    ...
    log_info("Received request to rotate journal from PID " PID_FMT,
             si->ssi_pid);
    server_rotate(s);
    ...
    r = write_timestamp_file_atomic("/run/systemd/journal/rotated",
                                    now(CLOCK_MONOTONIC));
    if (r < 0)
        log_warning_errno(
            r, "Failed to write /run/systemd/journal/rotated, ignoring: %m");

    return 0;
}

static int dispatch_sigterm(sd_event_source* es,
                            const struct signalfd_siginfo* si, void* userdata)
{
    Server* s = userdata;
...
static int dispatch_sigrtmin1(sd_event_source* es,
                              const struct signalfd_siginfo* si, void* userdata)
{
    Server* s = userdata;
    int r;
    ...
    r = write_timestamp_file_atomic("/run/systemd/journal/synced",
                                    now(CLOCK_MONOTONIC));
    if (r < 0)
        log_warning_errno(
            r, "Failed to write /run/systemd/journal/synced, ignoring: %m");
...
static int setup_signals(Server* s)
{
    ...
    assert_se(sigprocmask_many(SIG_SETMASK, NULL, SIGINT, SIGTERM, SIGUSR1,
                               SIGUSR2, SIGRTMIN + 1, -1) >= 0);
    ...
    r = sd_event_add_signal(s->event, &s->sigrtmin1_event_source, SIGRTMIN + 1,
                            dispatch_sigrtmin1, s);
    if (r < 0)
        return r;

    r = sd_event_source_set_priority(s->sigrtmin1_event_source,
                                     SD_EVENT_PRIORITY_NORMAL + 15);
    if (r < 0)
        return r;
                                   ...
                                   void* data)
{
    ...
    else if (proc_cmdline_key_streq(key, "systemd.journald.max_level_console"))
    {
        ...
        if (r < 0)
            log_warning(
                "Failed to parse max level console value \"%s\". Ignoring.",
                value);
        else
    ...
    else if (proc_cmdline_key_streq(key, "systemd.journald.max_level_store"))
    {
        ...
        if (r < 0)
            log_warning(
                "Failed to parse max level store value \"%s\". Ignoring.",
                value);
        else
            s->max_level_store = r;
    }
    else if (proc_cmdline_key_streq(key, "systemd.journald.max_level_syslog"))
    {
        ...
        if (r < 0)
            log_warning(
                "Failed to parse max level syslog value \"%s\". Ignoring.",
                value);
        else
    ...
    else if (proc_cmdline_key_streq(key, "systemd.journald.max_level_kmsg"))
    {
        ...
        if (r < 0)
            log_warning(
                "Failed to parse max level kmsg value \"%s\". Ignoring.",
                value);
        else
            s->max_level_kmsg = r;
    }
    else if (proc_cmdline_key_streq(key, "systemd.journald.max_level_wall"))
    {
        ...
        if (r < 0)
            log_warning(
                "Failed to parse max level wall value \"%s\". Ignoring.",
                value);
        else
            s->max_level_wall = r;
    }
        ...
        log_warning(
            "Unknown journald kernel command line option \"%s\". Ignoring.",
            key);
...
static int server_parse_config_file(Server* s)
{
    ...
    return config_parse_many_nulstr(
        ...
        CONF_PATHS_NULSTR("systemd/journald.conf.d"), "Journal\0",
        config_item_perf_lookup, journald_gperf_lookup, CONFIG_PARSE_WARN, s);
}
...
static int dispatch_hostname_change(sd_event_source* es, int fd,
...
static int server_open_hostname(Server* s)
{
    ...
    r = sd_event_add_io(s->event, &s->hostname_event_source, s->hostname_fd, 0,
                        dispatch_hostname_change, s);
    if (r < 0)
    {
        ...
        return log_error_errno(
            r, "Failed to register hostname fd in event loop: %m");
    }
    ...
    if (r < 0)
        return log_error_errno(
            r, "Failed to adjust priority of host name event source: %m");

    return 0;
}

static int dispatch_notify_event(sd_event_source* es, int fd, uint32_t revents,
                                 void* userdata)
{
    Server* s = userdata;
    int r;
    ...
    if (!s->sent_notify_ready)
    {
        ...
        if (l < 0)
        {
            ...
            return log_error_errno(
                errno, "Failed to send READY=1 notification message: %m");
        }
    ...
    else if (s->send_watchdog)
    {
        ...
        if (l < 0)
        {
            ...
            return log_error_errno(
                errno, "Failed to send WATCHDOG=1 notification message: %m");
        }
...
static int server_connect_notify(Server* s)
{
    ...
    */
    ...
    if (salen < 0)
        return log_error_errno(
            salen, "NOTIFY_SOCKET set to invalid value '%s': %m", e);
...
int server_init(Server* s)
{
    ...
    zero(*s);
    s->syslog_fd = s->native_fd = s->stdout_fd = s->dev_kmsg_fd = s->audit_fd =
        s->hostname_fd = s->notify_fd = -1;
    ...
    r = proc_cmdline_parse(parse_proc_cmdline_item, s,
    ...
    {
        log_debug("Setting both rate limit interval and burst from " USEC_FMT
                  ",%u to 0,0",
    ...
    for (fd = SD_LISTEN_FDS_START; fd < SD_LISTEN_FDS_START + n; fd++)
    {
        ...
        if (sd_is_socket_unix(fd, SOCK_DGRAM, -1, "/run/systemd/journal/socket",
                              0) > 0)
        {
                                   ...
                                   "/run/systemd/journal/stdout", 0) > 0)
        {
            ...
            s->stdout_fd = fd;
                 ...
                 sd_is_socket_unix(fd, SOCK_DGRAM, -1,
                                   "/run/systemd/journal/dev-log", 0) > 0)
        {
    ...
    no_sockets = s->native_fd < 0 && s->stdout_fd < 0 && s->syslog_fd < 0 &&
                 s->audit_fd < 0;
...
int config_parse_line_max(const char* unit, const char* filename, unsigned line,
                          ...
                          void* data, void* userdata)
{
    ...
    else
    {
        ...
        if (v < 79)
        {
            /* Why specify 79 here as minimum line length? Simply, because the
             * most common traditional terminal size is 80ch, and it might make
             * sense to break one character before the natural
             * line break would occur on that. */
            log_syntax(unit, LOG_WARNING, filename, line, 0,
                       "LineMax= too small, clamping to 79: %s", rvalue);
            *sz = 79;
        }
        else if (v > (uint64_t)(SSIZE_MAX - 1))
        {
            /* So, why specify SSIZE_MAX-1 here? Because that's one below the
             * largest size value read() can return, and we need one extra byte
             * for the trailing NUL byte. Of course IRL such large memory
             * allocations will fail anyway, hence this limit is mostly
...
int config_parse_compress(const char* unit, const char* filename, unsigned line,
                          const char* section, unsigned section_line,
                          const char* lvalue, int ltype, const char* rvalue,
                          void* data, void* userdata)
{
    ...
    {
        log_syntax(unit, LOG_WARNING, filename, line, 0,
                   "Compress= ambiguously specified as 1, enabling compression "
