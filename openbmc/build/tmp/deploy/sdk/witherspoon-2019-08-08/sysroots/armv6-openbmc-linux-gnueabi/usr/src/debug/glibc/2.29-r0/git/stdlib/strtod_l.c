...
static FLOAT overflow_value(int negative)
{
    __set_errno(ERANGE);
    FLOAT result =
        math_narrow_eval((negative ? -MAX_VALUE : MAX_VALUE) * MAX_VALUE);
    return result;
}
...
static FLOAT underflow_value(int negative)
{
    __set_errno(ERANGE);
    FLOAT result =
        math_narrow_eval((negative ? -MIN_VALUE : MIN_VALUE) * MIN_VALUE);
    return result;
}

/* Return a floating point number of the needed type according to the given
   multi-precision number after possible rounding.  */
static FLOAT round_and_return(mp_limb_t* retval, intmax_t exponent,
                              int negative, mp_limb_t round_limb,
    ...
    if (exponent < MIN_EXP - 1)
    {
        ...
        else if (shift >= BITS_PER_MP_LIMB)
        {
            ...
            if ((shift % BITS_PER_MP_LIMB) != 0)
                ...
                for (i = 0; i < RETURN_LIMB_SIZE - (shift / BITS_PER_MP_LIMB);
                     i++)
        ...
        else if (shift > 0)
        {
            if (TININESS_AFTER_ROUNDING && shift == 1)
            {
               ...
               a subnormal exponent.  */
                ...
                if (round_away(
                        negative, (retval[0] & 1) != 0,
                        (round_limb & (((mp_limb_t)1) << round_bit)) != 0,
                        (more_bits ||
                         ((round_limb & ((((mp_limb_t)1) << round_bit) - 1)) !=
                          0)),
                        mode))
                {
                    mp_limb_t cy =
                        __mpn_add_1(retval_normal, retval, RETURN_LIMB_SIZE, 1);
...
#define DENORM_EXP (MIN_EXP - 2)
    ...
    bool more_bits_nonzero =
        (more_bits || (round_limb & ((((mp_limb_t)1) << round_bit) - 1)) != 0);
    if (round_away(negative, (retval[0] & 1) != 0, half_bit, more_bits_nonzero,
                   mode))
    {
              ...
              (((mp_limb_t)1) << (MANT_DIG % BITS_PER_MP_LIMB))) != 0))
        {
            ...
            retval[RETURN_LIMB_SIZE - 1] |=
                ((mp_limb_t)1) << ((MANT_DIG - 1) % BITS_PER_MP_LIMB);
                 ...
                 (retval[RETURN_LIMB_SIZE - 1] &
                  (((mp_limb_t)1) << ((MANT_DIG - 1) % BITS_PER_MP_LIMB))) != 0)
...
static const STRING_TYPE* str_to_mpn(const STRING_TYPE* str, int digcnt,
                                     mp_limb_t* n, mp_size_t* nsize,
                                     intmax_t* exponent
#ifndef USE_WIDE_CHAR
                                     ,
                                     const char* decimal, size_t decimal_len,
...
#define decimal_len 1
    ...
    else if (c < L_('0') || c > L_('9'))
    {
        ...
        if (lowc == L_('i') && STRNCASECMP(cp, L_("inf"), 3) == 0)
        {
            /* Return +/- infinity.  */
            if (endptr != NULL)
                *endptr = (STRING_TYPE*)(cp + (STRNCASECMP(cp + 3, L_("inity"),
                                                           5) == 0
                                                   ? 8
                                                   : 3));
    ...
    if ((base == 16 && lowc == L_('p')) || (base != 16 && lowc == L_('e')))
    {
        ...
        if (c >= L_('0') && c <= L_('9'))
        {
            ...
            if (base == 16)
            {
                ...
                {
                    assert(int_no <=
                           (uintmax_t)(INTMAX_MAX + MIN_EXP - MANT_DIG) / 4);
...
number_parsed:
    ...
    if (lead_zero)
    {
        ...
        assert(lead_zero <= (base == 16 ? (uintmax_t)INTMAX_MAX / 4
                                        : (uintmax_t)INTMAX_MAX));
        assert(lead_zero <=
               (base == 16 ? ((uintmax_t)exponent - (uintmax_t)INTMAX_MIN) / 4
    ...
    {
        intmax_t incr =
            (exponent < 0 ? MAX(-(intmax_t)int_no, exponent)
    ...
    if (int_no > 0)
    {
            ...
            do
            {
                if ((exponent & expbit) != 0)
                {
                    ...
                    if (numsize >= ttab->arraysize - _FPIO_CONST_OFFSET)
                        ...
                        cy = __mpn_mul(
                            pdest, &__tens[ttab->arrayoff + _FPIO_CONST_OFFSET],
                            size, psrc, numsize);
                    numsize += size;
                    if (cy == 0)
                        --numsize;
       ...
       to determine the result.  */
        if (bits > MANT_DIG)
        {
            ...
            const mp_size_t round_idx =
                least_bit == 0 ? least_idx - 1 : least_idx;
            const mp_size_t round_bit =
                least_bit == 0 ? BITS_PER_MP_LIMB - 1 : least_bit - 1;
            ...
            else
            {
                for (i = least_idx; i < numsize - 1; ++i)
                    retval[i - least_idx] =
                        (num[i] >> least_bit) |
                        (num[i + 1] << (BITS_PER_MP_LIMB - least_bit));
            ...
            return round_and_return(retval, bits - 1, negative, num[round_idx],
                                    round_bit,
        ...
        else if (dig_no == int_no)
        {
            ...
            {
                (void)__mpn_lshift(&retval[RETURN_LIMB_SIZE - numsize], num,
                                   numsize, target_bit - is_bit);
                /* FIXME: the following loop can be avoided if we assume a
               maximal MANT_DIG value.  */
                MPN_ZERO(retval, RETURN_LIMB_SIZE - numsize);
            }
            else
            {
                ...
                cy = __mpn_rshift(&retval[RETURN_LIMB_SIZE - numsize], num,
                                  numsize, is_bit - target_bit);
    ...
    /* We have to compute at least some of the fractional digits.  */
    {
        ...
        if (cnt > 0)
        {
            /* Don't call `mpn_shift' with a count of zero since the
               specification does not allow this.  */
    ...
    bits += BITS_PER_MP_LIMB
                ...
                while (bits <= MANT_DIG)
                {
                    ...
                    if (n1 == d1)
                    {
                        /* QUOT should be either 111..111 or 111..110.  We need
                           special treatment of this rare case as normal
                           division would give overflow.  */
                        quot = ~(mp_limb_t)0;
            ...
            default:
            {
                    ...
                    else
                    {
                        ...
                        else
                        {
                            ...
                            if (used >= BITS_PER_MP_LIMB)
                            {
                                int i;
                                (void)__mpn_lshift(
                                    &retval[used / BITS_PER_MP_LIMB], retval,
                                    (RETURN_LIMB_SIZE -
                                     used / BITS_PER_MP_LIMB),
                                    used % BITS_PER_MP_LIMB);
                                for (i = used / BITS_PER_MP_LIMB - 1; i >= 0;
                                     --i)
                                    retval[i] = 0;
                            }
                            else if (used > 0)
                ...
                while (bits <= MANT_DIG)
                {
                    ...
                    /* Possible optimization: We already have (q * n0) and (1 *
                   n1) after the calculation of QUOT.  Taking advantage of this,
                   we could make this loop make two iterations less.  */
