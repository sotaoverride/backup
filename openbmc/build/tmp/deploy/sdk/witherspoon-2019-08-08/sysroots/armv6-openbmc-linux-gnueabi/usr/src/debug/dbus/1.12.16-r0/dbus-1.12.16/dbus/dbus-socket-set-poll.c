...
static dbus_bool_t socket_set_poll_add(DBusSocketSet* set, DBusPollable fd,
                                       unsigned int flags, dbus_bool_t enabled)
{
    DBusSocketSetPoll* self = socket_set_poll_cast(set);
    ...
    {
        DBusPollFD* new_fds =
            dbus_realloc(self->fds, sizeof(DBusPollFD) * (self->n_allocated +
                                                          REALLOC_INCREMENT));
    ...
    _dbus_verbose("before adding fd %" DBUS_POLLABLE_FORMAT
                  " to %p, %d en/%d res/%d alloc\n",
                  _dbus_pollable_printable(fd), self, self->n_fds,
                  self->n_reserved, self->n_allocated);
...
static void socket_set_poll_enable(DBusSocketSet* set, DBusPollable fd,
                                   unsigned int flags)
{
    DBusSocketSetPoll* self = socket_set_poll_cast(set);
    int i;
...
static void socket_set_poll_remove(DBusSocketSet* set, DBusPollable fd)
{
    ...
    _dbus_verbose("after removing fd %" DBUS_POLLABLE_FORMAT
    ...
    if (self->n_reserved + MINIMUM_SIZE < self->n_allocated / 2)
    {
        ...
        DBusPollFD* new_fds = dbus_realloc(
            self->fds, sizeof(DBusPollFD) * (self->n_reserved + MINIMUM_SIZE));

        _dbus_verbose("before deflating %p, %d en/%d res/%d alloc\n", self,
                      self->n_fds, self->n_reserved, self->n_allocated);
...
static int socket_set_poll_poll(DBusSocketSet* set, DBusSocketEvent* revents,
                                int max_events, int timeout_ms)
{
    ...
    for (i = 0; i < self->n_fds; i++)
    {
        if (self->fds[i].revents != 0)
        {
            ...
            revents[n_events].flags =
                watch_flags_from_poll_revents(self->fds[i].revents);
...
DBusSocketSetClass _dbus_socket_set_poll_class = {
    socket_set_poll_free,   socket_set_poll_add,     socket_set_poll_remove,
    socket_set_poll_enable, socket_set_poll_disable, socket_set_poll_poll};
