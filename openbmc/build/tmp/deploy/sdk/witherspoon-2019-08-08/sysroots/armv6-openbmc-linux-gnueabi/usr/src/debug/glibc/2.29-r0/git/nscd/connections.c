...
const char* const serv2str[LASTREQ] = {[GETPWBYNAME] = "GETPWBYNAME",
...
struct database_dyn dbs[lastdb] = {
    ...
    [hstdb] = {.lock = RWLOCK_INITIALIZER,
               ...
               .ro_fd = -1,
               .mmap_used = false},
    [servdb] = {.lock = RWLOCK_INITIALIZER,
                .prune_lock = PTHREAD_MUTEX_INITIALIZER,
                .prune_run_lock = PTHREAD_MUTEX_INITIALIZER,
                .enabled = 0,
                .check_file = 1,
                ...
                .ro_fd = -1,
                .mmap_used = false},
    [netgrdb] = {.lock = RWLOCK_INITIALIZER,
                 .prune_lock = PTHREAD_MUTEX_INITIALIZER,
                 .prune_run_lock = PTHREAD_MUTEX_INITIALIZER,
                 .enabled = 0,
                 .check_file = 1,
...
} const reqinfo[LASTREQ] = {[GETPWBYNAME] = {true, &dbs[pwddb]},
...
static int verify_persistent_db(void* mem, struct database_pers_head* readhead,
                                int dbnr)
{
    ...
    if (head->version != DB_VERSION ||
       ...
       This should cover daylight saving time changes.  */
        || head->timestamp > now + 60 * 60 + 60 || (head->gc_cycle & 1) ||
        head->module == 0 || (size_t)head->module > INT32_MAX / sizeof(ref_t) ||
        (size_t)head->data_size > INT32_MAX - head->module * sizeof(ref_t) ||
        head->first_free < 0 || head->first_free > head->data_size ||
        (head->first_free & BLOCK_ALIGN_M1) != 0 || head->maxnentries < 0 ||
        head->maxnsearched < 0)
        return 0;
    ...
    const char* data =
        (char*)&head->array[roundup(head->module, ALIGN / sizeof(ref_t))];
    ...
    for (nscd_ssize_t cnt = 0; cnt < head->module; ++cnt)
    {
        ...
        while (work != ENDREF)
        {
            ...
            if (here->type >= LASTREQ || reqinfo[here->type].db != &dbs[dbnr])
                goto fail;
            ...
            /* Now the data.  */
            if (here->packet < 0 || here->packet > head->first_free ||
        ...
        if (dbs[cnt].enabled)
        {
            ...
            if (dbs[cnt].persistent)
            {
                ...
                if (fd != -1)
                {
                    ...
                    ssize_t n =
                        TEMP_FAILURE_RETRY(read(fd, &head, sizeof(head)));
                    ...
                    fail_db:
                        dbg_log(
                            _("invalid persistent database file \"%s\": %s"),
                    ...
                    else
                    {
                        ...
                        dbs[cnt].data = (char*)&dbs[cnt].head->array[roundup(
                            dbs[cnt].head->module, ALIGN / sizeof(ref_t))];
                        dbs[cnt].mmap_used = true;

                        if (dbs[cnt].suggested_module > head.module)
                            dbg_log(_("suggested size of table for database %s "
                                      "larger than the persistent database's "
                                      "table"),
                                    dbnames[cnt]);
...
cannot create read-only descriptor for \"%s\"; no mmap"),
            ...
            if (dbs[cnt].head == NULL)
            {
                ...
                size_t total =
                    (sizeof(head) +
                     roundup(dbs[cnt].suggested_module * sizeof(ref_t), ALIGN) +
                     (dbs[cnt].suggested_module * DEFAULT_DATASIZE_PER_BUCKET));
                ...
                if (dbs[cnt].persistent)
                {
                    ...
                    if (fd != -1 && dbs[cnt].shared)
                        ro_fd =
                            open(dbs[cnt].db_filename, O_RDONLY | O_CLOEXEC);
                }
                else
                {
                ...
                if (fd == -1)
                {
                    if (errno == EEXIST)
                    {
                        dbg_log(_("database for %s corrupted or simultaneously "
                                  "used; remove %s manually if necessary and "
                                  "restart"),
                    ...
                    if (dbs[cnt].persistent)
                        dbg_log(
                            _("cannot create %s; no persistent database used"),
                            dbs[cnt].db_filename);
                    else
...
cannot create read-only descriptor for \"%s\"; no mmap"),
            ...
            if (dbs[cnt].head == NULL)
            {
                ...
                dbs[cnt].head->data_size =
                    (DEFAULT_DATASIZE_PER_BUCKET * dbs[cnt].head->module);
    ...
    if (dbs[hstdb].enabled)
    {
        ...
        nl_status_fd = socket(
            AF_NETLINK, SOCK_RAW | SOCK_CLOEXEC | SOCK_NONBLOCK, NETLINK_ROUTE);
        if (nl_status_fd != -1)
        {
            ...
            snl.nl_groups =
                (RTMGRP_IPV4_IFADDR | RTMGRP_TC | RTMGRP_IPV4_MROUTE |
                 RTMGRP_IPV4_ROUTE | RTMGRP_IPV4_RULE | RTMGRP_IPV6_IFADDR |
                 RTMGRP_IPV6_MROUTE | RTMGRP_IPV6_ROUTE | RTMGRP_IPV6_IFINFO |
                 RTMGRP_IPV6_PREFIX);
...
#define TRACED_DIR_MASK                                                        \
    (IN_DELETE_SELF | IN_CREATE | IN_MOVED_TO | IN_MOVE_SELF)
void install_watches(struct traced_file* finfo)
{
    ...
    if (finfo->inotify_descr[TRACED_FILE] < 0)
        finfo->inotify_descr[TRACED_FILE] =
            inotify_add_watch(inotify_fd, finfo->fname, TRACED_FILE_MASK);
    if (finfo->inotify_descr[TRACED_FILE] < 0)
    {
    ...
    dbg_log(_("monitoring file `%s` (%d)"), finfo->fname,
            finfo->inotify_descr[TRACED_FILE]);
    ...
    if (finfo->inotify_descr[TRACED_DIR] < 0)
        finfo->inotify_descr[TRACED_DIR] =
            inotify_add_watch(inotify_fd, finfo->dname, TRACED_DIR_MASK);
    if (finfo->inotify_descr[TRACED_DIR] < 0)
    {
    ...
    dbg_log(_("monitoring directory `%s` (%d)"), finfo->dname,
            finfo->inotify_descr[TRACED_DIR]);
}
#endif
...
void register_traced_file(size_t dbidx, struct traced_file* finfo)
{
    ...
    if (__glibc_unlikely(debug_level > 0))
        dbg_log(_("monitoring file %s for database %s"), finfo->fname,
                dbnames[dbidx]);
...
/* Handle new request.  */
static void handle_request(int fd, request_header* req, void* key, uid_t uid,
                           pid_t pid)
{
    ...
    if (__builtin_expect(reqinfo[req->type].data_request, true))
    {
        if (__builtin_expect(debug_level, 0) > 0)
        {
            if (req->type == GETHOSTBYADDR || req->type == GETHOSTBYADDRv6)
            {
                ...
                dbg_log(
                    "\t%s (%s)", serv2str[req->type],
                    inet_ntop(req->type == GETHOSTBYADDR ? AF_INET : AF_INET6,
                              key, buf, sizeof(buf)));
            }
            else
        ...
        if (__glibc_unlikely(!db->enabled))
        {
            ...
            if (TEMP_FAILURE_RETRY(send(
                    fd, db->disabled_iov->iov_base, db->disabled_iov->iov_len,
                    MSG_NOSIGNAL)) != (ssize_t)db->disabled_iov->iov_len &&
...
static char* read_cmdline(size_t* size)
{
    ...
    while (1)
    {
        ...
        ssize_t n =
            TEMP_FAILURE_RETRY(read(fd, buffer + current, limit - current));
        if (n == -1)
        {
            int e = errno;
            free(buffer);
...
cannot change to old working directory: %s; disabling paranoia mode"),
    ...
    dbg_log(_("re-exec failed: %s; disabling paranoia mode"), strerror(errno));
...
static void* __attribute__((__noreturn__)) nscd_run_prune(void* p)
{
...
/* This is the main loop.  It is replicated in different threads but
   ...
   incoming connection.  */
static void* __attribute__((__noreturn__)) nscd_run_worker(void* p)
{
    char buf[256];
    ...
    while (1)
    {
       ...
       request (i.e., a terribly large key).  We limit the size to 1kb.  */
        ...
        else
        {
            ...
            if (__builtin_expect(TEMP_FAILURE_RETRY(read(
                                     fd, keybuf, req.key_len)) != req.key_len,
                                 0))
            {
    ...
    for (size_t dbcnt = 0; dbcnt < lastdb; ++dbcnt)
    {
        ...
        while (finfo != NULL)
        {
            ...
            if (finfo->inotify_descr[TRACED_DIR] == inev->i.wd &&
                ((inev->i.mask & IN_DELETE_SELF) ||
                 (inev->i.mask & IN_MOVE_SELF) || (inev->i.mask & IN_IGNORED)))
            {
                bool moved = (inev->i.mask & IN_MOVE_SELF) != 0;
               ...
               files.  */
                ...
                {
                    dbg_log(_("monitored parent directory `%s` was %s, "
                              "removing watch on `%s`"),
                            finfo->dname, moved ? "moved" : "deleted",
                            finfo->fname);
                    if (inotify_rm_watch(inotify_fd,
                                         finfo->inotify_descr[TRACED_FILE]) < 0)
                        dbg_log(_("failed to remove file watch `%s`: %s"),
            ...
            if (finfo->inotify_descr[TRACED_DIR] == inev->i.wd &&
                ((inev->i.mask & IN_MOVED_TO) || (inev->i.mask & IN_CREATE)) &&
                strcmp(inev->i.name, finfo->sfname) == 0)
            {
               ...
               into the directory.  */
                int ret;
                dbg_log(
                    _("monitored file `%s` was %s, adding watch"), finfo->fname,
                ...
                ret = inotify_add_watch(inotify_fd, finfo->fname,
                                        TRACED_FILE_MASK);
                if (ret < 0)
...
int handle_inotify_events(void)
{
    ...
    while (1)
    {
        ...
        ssize_t nb =
            TEMP_FAILURE_RETRY(read(inotify_fd, &inev.buf, sizeof(inev)));
...
static void __attribute__((__noreturn__)) main_loop_poll(void)
{
    struct pollfd* conns = (struct pollfd*)xmalloc(nconns * sizeof(conns[0]));
...
#define MAIN_THREAD_TIMEOUT                                                    \
    (MAX_ACCEPT_TIMEOUT * 1000 -                                               \
     ((MAX_ACCEPT_TIMEOUT - MIN_ACCEPT_TIMEOUT) * 1000 * nused) /              \
         (2 * nconns))
        ...
        if (n > 0)
        {
            if (conns[0].revents != 0)
            {
                ...
                int fd = TEMP_FAILURE_RETRY(
                    accept4(sock, NULL, NULL, SOCK_NONBLOCK));
                ...
                if (conns[1].revents != 0)
                {
                    ...
                    if (ret == -1)
                    {
                       ...
                       data.  Better disable inotify.  */
                        dbg_log(_("disabled inotify-based monitoring after "
                                  "read error %d"),
                                errno);
                        conns[1].fd = -1;
                        firstfree = 1;
            ...
            if (idx_nl_status_fd != 0 && conns[idx_nl_status_fd].revents != 0)
            {
                ...
                while (TEMP_FAILURE_RETRY(
                           read(nl_status_fd, buf, sizeof(buf))) != -1)
                    ;
        ...
        for (int cnt = 0; cnt < n; ++cnt)
            ...
            else if (revs[cnt].data.fd == inotify_fd)
            {
                ...
                if (ret == -1)
                {
                    /* Something went wrong when reading the inotify
                       data.  Better disable inotify.  */
                    dbg_log(_("disabled inotify-based monitoring after read "
                              "error %d"),
                            errno);
...
static void begin_drop_privileges(void)
{
    ...
    if (pwd == NULL)
    {
        ...
        do_exit(EXIT_FAILURE, 0, _("Failed to run nscd as user '%s'"),
                server_user);
    }
    ...
    if (getgrouplist(server_user, server_gid, NULL, &server_ngroups) == 0)
    {
        ...
        dbg_log(_("Failed to run nscd as user '%s'"), server_user);
        do_exit(EXIT_FAILURE, errno, _("initial getgrouplist failed"));
    }
