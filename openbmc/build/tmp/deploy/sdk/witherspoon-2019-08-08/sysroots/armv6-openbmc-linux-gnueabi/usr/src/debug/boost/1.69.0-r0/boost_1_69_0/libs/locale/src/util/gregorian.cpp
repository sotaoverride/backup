...
std::time_t internal_timegm(std::tm const* t)
{
    ...
    std::time_t result = seconds_in_day * days_since_epoch + 3600 * t->tm_hour +
...
// sun - AR AS AZ BW CA CN FO GE GL GU HK IL IN JM JP KG KR LA MH MN MO MP MT NZ
...
int first_day_of_week(char const* terr)
{
    static char const* const sat[] = {
        "AE", "AF", "BH", "DJ", "DZ", "EG", "ER", "ET", "IQ", "IR", "JO", "KE",
        "KW", "LY", "MA", "OM", "QA", "SA", "SD", "SO", "SY", "TN", "YE"};
    static char const* const sunday[] = {
        "AR", "AS", "AZ", "BW", "CA", "CN", "FO", "GE", "GL", "GU", "HK", "IL",
    ...
    if (std::binary_search<char const* const*>(
        ...
        return 6; // sat
    if (std::binary_search<char const* const*>(
            sunday, sunday + sizeof(sunday) / (sizeof(sunday[0])), terr,
            comparator))
        return 0; // sun
    // default
    return 1; // mon
...
class gregorian_calendar : public abstract_calendar
{
  public:
    ...
    virtual void set_value(period::marks::period_mark p, int value)
    {
        ...
        switch (p)
        {
            case era: ///< Era i.e. AC, BC in Gregorian and Julian calendar,
                      ///< range [0,1]
                return;
            case year:          ///< Year, it is calendar specific
            case extended_year: ///< Extended year for Gregorian/Julian
            ...
            case day_of_week:  ///< Day of week, starting from Sunday, [1..7]
                ...
                // fall throght
            case day_of_week_local: ///< Local day of week, for example in
                                    ///< France Monday is 1, in US Sunday is 1,
                                    ///< [1..7]
                normalize();
                tm_updated_.tm_mday +=
                    (value - 1) -
                    (tm_updated_.tm_wday - first_day_of_week_ + 7) % 7;
                break;
            case day_of_week_in_month: ///< Original number of the day of the
                                       ///< week in month. (1st sunday, 2nd
                                       ///< sunday etc)
            case week_of_year:  ///< The week number in the year, 4 is the
                                ///< minimal number of days to be in month
            ...
            break;
            case period::marks::first_day_of_week: ///< For example Sunday in
                                                   ///< US, Monday in France
            default:
                return;
        }
        ...
        if (!normalized_)
        {
            ...
            if (is_local_)
            {
                ...
                if (point == static_cast<std::time_t>(-1))
                {
                    ...
                    // windows does not handle negative time_t, under other
                    // plaforms it may be actually valid value in  1969-12-31
                    // 23:59:59 so we check that a filed was updated - does not
            ...
            else
            {
                ...
                if (point < 0 || (revert_point = gmtime(&point)) == 0)
                    throw date_time_error("boost::locale::gregorian_calendar "
        ...
        switch (p)
        {
            ...
            case extended_year:
                switch (v)
                {
                    ...
                    case actual_minimum:
#ifdef BOOST_WINDOWS
                        return 1970; // Unix epoch windows can't handle negative
                                     // time_t
#else
            ...
            case day:
                switch (v)
                {
                    ...
                    case least_maximum:
                        return 28;
                    case actual_maximum:
                        return days_in_month(tm_.tm_year + 1900,
                                             tm_.tm_mon + 1);
                    case current:
                        return tm_.tm_mday;
                };
                ...
                switch (v)
                {
                    ...
                    case actual_maximum:
                    {
                        ...
                        int dow_of_end_of_year =
                            (end_of_year_days - tm_.tm_yday + tm_.tm_wday) % 7;
                        return get_week_number(end_of_year_days,
                                               dow_of_end_of_year);
                    }
                    case current:
                    {
                ...
                switch (v)
                {
                    ...
                    case actual_maximum:
                    {
                        int end_of_month_days =
                            days_in_month(tm_.tm_year + 1900, tm_.tm_mon + 1);
                        int dow_of_end_of_month =
                            (end_of_month_days - tm_.tm_mday + tm_.tm_wday) % 7;
                        return get_week_number(end_of_month_days,
                                               dow_of_end_of_month);
                    }
                    case current:
                    {
            ...
            case day_of_week_in_month: ///< Original number of the day of the
                ...
                switch (v)
                {
                    ...
                    case actual_maximum:
                        ...
                        {
                            // only in february in non leap year is 28 days, the
                            // rest conver more then 4 weeks
                            return 4;
                        }
                        return 5;
                    case current:
        ...
        switch (u)
        {
            case move:
            {
                ...
                switch (p)
                {
                    ...
                    case day:
                    case day_of_year:
                    case day_of_week: ///< Day of week, starting from Sunday,
                                      ///< [1..7]
                    case day_of_week_local: ///< Local day of week, for example
                                            ///< in France Monday is 1, in US
                                            ///< Sunday is 1, [1..7]
                    ...
                    case second:
                        ...
                        break;
                    case week_of_year:  ///< The week number in the year
                    case week_of_month: ///< The week number withing current
                                        ///< month
                    case day_of_week_in_month: ///< Original number of the day
            ...
            { // roll
                ...
                if (max_diff > 0)
                {
                    ...
                    value = (value - cur_min + difference + addon) % max_diff +
                            cur_min;
                    set_value(p, value);
                    normalize();
                }
    ...
    int get_diff(period::marks::period_mark p, int diff,
                 gregorian_calendar const* other) const
    {
        if (diff == 0)
            return 0;
    ...
    /// units
    ///
    virtual int difference(abstract_calendar const* other_cal,
                           period::marks::period_mark p) const
    {
        hold_ptr<gregorian_calendar> keeper;
        gregorian_calendar const* other =
            dynamic_cast<gregorian_calendar const*>(other_cal);
        if (!other)
        {
            keeper.reset(clone());
    ...
    {
        gregorian_calendar const* gcal =
            dynamic_cast<gregorian_calendar const*>(other);
        if (!gcal)
            return false;
...
#else
        std::tm tmp_tm;
        t = is_local_ ? localtime_r(&real_point, &tmp_tm)
                      : gmtime_r(&real_point, &tmp_tm);
#endif
        if (!t)
        {
            throw date_time_error(
...
class gregorian_facet : public calendar_facet
{
  public:
    gregorian_facet(std::string const& terr, size_t refs = 0) :
        calendar_facet(refs), terr_(terr)
    {
    }
...
std::locale install_gregorian_calendar(std::locale const& in,
