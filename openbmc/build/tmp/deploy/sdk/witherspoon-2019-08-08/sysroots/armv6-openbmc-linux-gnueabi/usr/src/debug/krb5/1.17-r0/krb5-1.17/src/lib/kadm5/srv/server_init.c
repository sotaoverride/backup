...
kadm5_ret_t kadm5_init(krb5_context context, char* client_name, char* pass,
                       char* service_name, kadm5_config_params* params_in,
                       ...
                       char** db_args, void** server_handle)
{
    ...
    ret =
        kadm5_get_config_params(handle->context, 1, params_in, &handle->params);
    if (ret)
    {
        free_db_args(handle);
        free(handle);
...
#define REQUIRED_PARAMS                                                        \
    (KADM5_CONFIG_REALM | KADM5_CONFIG_DBNAME | KADM5_CONFIG_ENCTYPE |         \
     KADM5_CONFIG_FLAGS | KADM5_CONFIG_MAX_LIFE | KADM5_CONFIG_MAX_RLIFE |     \
     KADM5_CONFIG_EXPIRATION | KADM5_CONFIG_ENCTYPES)

#define IPROP_REQUIRED_PARAMS                                                  \
    (KADM5_CONFIG_IPROP_ENABLED | KADM5_CONFIG_IPROP_LOGFILE |                 \
    ...
    if ((handle->params.mask & KADM5_CONFIG_IPROP_ENABLED) ==
            KADM5_CONFIG_IPROP_ENABLED &&
        handle->params.iprop_enabled)
    {
        if ((handle->params.mask & IPROP_REQUIRED_PARAMS) !=
            IPROP_REQUIRED_PARAMS)
        {
            kadm5_free_config_params(handle->context, &handle->params);
            free_db_args(handle);
            free(handle);
