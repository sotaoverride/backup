...
struct cons : sequence_base<cons<Car, Cdr>>
{
    ...
    BOOST_CONSTEXPR BOOST_FUSION_GPU_ENABLED cons() : car(), cdr()
    {
    }

    BOOST_CONSTEXPR BOOST_FUSION_GPU_ENABLED explicit cons(
    ...
    BOOST_CONSTEXPR BOOST_FUSION_GPU_ENABLED
        cons(typename detail::call_param<Car>::type in_car,
    ...
    BOOST_CONSTEXPR BOOST_FUSION_GPU_ENABLED cons(cons<Car2, Cdr2> const& rhs) :
    ...
    // Workaround for `array used as initializer` compile error on gcc 4.6 w/
    // c++0x.
    template <typename = void>
#endif
    BOOST_CONSTEXPR BOOST_FUSION_GPU_ENABLED cons(cons const& rhs) :
        car(rhs.car), cdr(rhs.cdr)
    {
    }
    ...
    BOOST_FUSION_GPU_ENABLED cons(
        ...
        typename boost::enable_if<
            mpl::and_<traits::is_sequence<Sequence>,
                      mpl::not_<is_base_of<cons, Sequence>>,
                      mpl::not_<is_convertible<Sequence, Car>>> // use copy to
                                                                // car instead
            ,
            detail::enabler_>::type = detail::enabler) :
    ...
    template <typename Car2, typename Cdr2>
    BOOST_CXX14_CONSTEXPR BOOST_FUSION_GPU_ENABLED cons&
    ...
    template <typename Sequence>
    BOOST_CXX14_CONSTEXPR BOOST_FUSION_GPU_ENABLED typename boost::enable_if<
        mpl::and_<traits::is_sequence<Sequence>,
    ...
    BOOST_CXX14_CONSTEXPR BOOST_FUSION_GPU_ENABLED void
        assign_from_iter(Iterator const& iter)
    {
        car = *iter;
