...
static int ssl_dane_dup(SSL* to, SSL* from)
{
    ...
    for (i = 0; i < num; ++i)
    {
        ...
        if (SSL_dane_tlsa_add(to, t->usage, t->selector, t->mtype, t->data,
                              t->dlen) <= 0)
            return 0;
    }
    return 1;
    ...
    if (mtype == DANETLS_MATCHING_FULL)
    {
        ...
        switch (selector)
        {
            case DANETLS_SELECTOR_CERT:
                ...
                /*
                 * For usage DANE-TA(2), we support authentication via "2 0 0"
                 * TLSA records that contain full certificates of trust-anchors
                 * that are not present in the wire chain.  For usage
                 * PKIX-TA(0), we augment the chain with untrusted Full(0)
                 * certificates from DNS, in case they are missing from the
                 * chain.
                 */
                if ((dane->certs == NULL &&
            ...
            case DANETLS_SELECTOR_SPKI:
                ...
                /*
                 * For usage DANE-TA(2), we support authentication via "2 1 0"
                 * TLSA records that contain full bare keys of trust-anchors
                 * that are not present in the wire chain.
                 */
...
static int ssl_check_allowed_versions(int min_version, int max_version)
{
    ...
    if (min_version == DTLS1_BAD_VER || min_version >> 8 == DTLS1_VERSION_MAJOR)
        minisdtls = 1;
    if (max_version == DTLS1_BAD_VER || max_version >> 8 == DTLS1_VERSION_MAJOR)
        maxisdtls = 1;
...
SSL* SSL_new(SSL_CTX* ctx)
{
    ...
    if (ctx->ext.supportedgroups)
    {
        s->ext.supportedgroups = OPENSSL_memdup(
            ...
            ctx->ext.supportedgroups_len * sizeof(*ctx->ext.supportedgroups));
...
int SSL_write_early_data(SSL* s, const void* buf, size_t num, size_t* written)
{
    ...
    switch (s->early_data_state)
    {
        ...
        case SSL_EARLY_DATA_WRITE_RETRY:
            ...
            /*
             * We disable partial write for early data because we don't keep
             * track of how many bytes we've written between the SSL_write_ex()
             * call and the flush if the flush needs to be retried)
             */
...
char* SSL_get_shared_ciphers(const SSL* s, char* buf, int size)
{
    ...
    if (!s->server || s->session == NULL || s->session->ciphers == NULL ||
        size < 2)
        return NULL;
...
{
    if (s->session &&
        (!s->ext.hostname ? s->session->ext.hostname : s->ext.hostname))
                               ...
                               int use_context)
{
    ...
    return s->method->ssl3_enc->export_keying_material(
...
SSL_CTX* SSL_CTX_new(const SSL_METHOD* meth)
{
    ...
    if (!ssl_create_cipher_list(ret->method, ret->tls13_ciphersuites,
    ...
    if ((RAND_bytes(ret->ext.tick_key_name, sizeof(ret->ext.tick_key_name)) <=
         0) ||
...
void ssl_update_cache(SSL* s, int mode)
{
    ...
    if ((i & mode) != 0 && (!s->hit || SSL_IS_TLS13(s)))
    {
         ...
         * normally don't do this because by default it's a full stateless
         * ticket with only a dummy session id so there is no reason to cache
         * it, unless:
                                   ...
                                   void* arg)
{
     ...
     */
    if (callback != NULL &&
        SSL_CTX_has_client_custom_ext(s->ctx,
...
static int nss_keylog_int(const char* prefix, SSL* ssl,
                                    ...
                                    size_t premaster_len)
{
    ...
    return nss_keylog_int("RSA", ssl, encrypted_premaster, 8, premaster,
                          premaster_len);
}

int ssl_log_secret(SSL* ssl, const char* label, const uint8_t* secret,
                   size_t secret_len)
{
    return nss_keylog_int(label, ssl, ssl->s3->client_random, SSL3_RANDOM_SIZE,
                          secret, secret_len);
}
...
int ssl_cache_cipherlist(SSL* s, PACKET* cipher_suites, int sslv2format)
{
    ...
    if (sslv2format)
    {
        ...
        for (s->s3->tmp.ciphers_rawlen = 0; PACKET_remaining(&sslv2ciphers) > 0;
             raw += TLS_CIPHER_LEN)
        {
            if (!PACKET_get_1(&sslv2ciphers, &leadbyte) ||
                (leadbyte == 0 &&
                 !PACKET_copy_bytes(&sslv2ciphers, raw, TLS_CIPHER_LEN)) ||
                (leadbyte != 0 &&
...
int SSL_verify_client_post_handshake(SSL* ssl)
{
    ...
    {
        case SSL_PHA_NONE:
            SSLerr(SSL_F_SSL_VERIFY_CLIENT_POST_HANDSHAKE,
