 ...
 *         }
 *
 *       if (!cake_decorate (cake, decoration->frosting, decoration->message,
 * &error))
 *         {
 *           g_object_unref (cake);
 ...
 *         }
 *
 *       cake = _baker_get_cached_cake (self, radius, flavor, frosting,
 * message); if (cake != NULL)
 *         {
 ...
 *       task = g_task_new (self, cancellable, callback, user_data);
 *       g_task_set_task_data (task, cake_data, (GDestroyNotify)
 * cake_data_free); g_task_run_in_thread (task, bake_cake_thread);
 *       g_object_unref (task);
 *     }
 *
 *     Cake *
 ...
 *       task = g_task_new (self, cancellable, callback, user_data);
 *       g_task_set_task_data (task, cake_data, (GDestroyNotify)
 * cake_data_free); g_task_set_return_on_cancel (task, TRUE);
...
struct _GTask
{
    ...
    gpointer source_tag;
    gchar*
        name; /* (owned); may only be modified before the #GTask is threaded */
 ...
 * Since: 2.36
 */
GTask* g_task_new(gpointer source_object, GCancellable* cancellable,
                  GAsyncReadyCallback callback, gpointer callback_data)
{
    ...
    TRACE(GIO_TASK_NEW(task, source_object, cancellable, callback,
                       callback_data));
 ...
 * Since: 2.36
 */
void g_task_report_error(gpointer source_object, GAsyncReadyCallback callback,
                             ...
                             GQuark domain, gint code, const char* format, ...)
{
    ...
    g_task_report_error(source_object, callback, callback_data, source_tag,
                        error);
}
...
static void g_task_return_now(GTask* task)
{
    ...
    {
        task->callback(task->source_object, G_ASYNC_RESULT(task),
                       task->callback_data);
    }
...
static void g_task_thread_complete(GTask* task)
{
    ...
    if (task->cancellable)
        g_signal_handlers_disconnect_by_func(task->cancellable,
                                             task_thread_cancelled, task);
...
static void g_task_thread_setup(void)
{
    ...
    else if (tasks_running > G_TASK_POOL_SIZE &&
             task_wait_time < G_TASK_WAIT_TIME_MAX)
        task_wait_time *= G_TASK_WAIT_TIME_MULTIPLIER;

    if (tasks_running >= G_TASK_POOL_SIZE)
        g_source_set_ready_time(task_pool_manager,
...
static void task_thread_cancelled_disconnect_notify(gpointer task,
                                                    GClosure* closure)
{
    g_object_unref(task);
}
...
static void g_task_start_task_thread(GTask* task, GTaskThreadFunc task_func)
{
    ...
    if (task->cancellable)
    {
        if (task->return_on_cancel && g_cancellable_set_error_if_cancelled(
         ...
         * GCancellable, but is necessary to avoid a race on finalising the
         * GTask between task_thread_cancelled() (in one thread) and
        ...
        g_signal_connect_data(
            task->cancellable, "cancelled", G_CALLBACK(task_thread_cancelled),
            g_object_ref(task), task_thread_cancelled_disconnect_notify, 0);
    }
...
void g_task_run_in_thread_sync(GTask* task, GTaskThreadFunc task_func)
{
    ...
    TRACE(GIO_TASK_BEFORE_RETURN(task, task->source_object, NULL /* callback */,
                                 NULL /* callback data */));
 ...
 * Since: 2.36
 */
void g_task_return_new_error(GTask* task, GQuark domain, gint code,
                             const char* format, ...)
{
    GError* error;
    va_list args;
...
static gint g_task_compare_priority(gconstpointer a, gconstpointer b,
                                    gpointer user_data)
{
    const GTask* ta = a;
    const GTask* tb = b;
    ...
    a_cancelled =
        (ta->check_cancellable && g_cancellable_is_cancelled(ta->cancellable));
    b_cancelled =
        (tb->check_cancellable && g_cancellable_is_cancelled(tb->cancellable));
...
static gboolean trivial_source_dispatch(GSource* source, GSourceFunc callback,
                                        gpointer user_data)
{
    return callback(user_data);
}

GSourceFuncs trivial_source_funcs = {NULL, /* prepare */
                                     NULL, /* check */
                                     trivial_source_dispatch, NULL};
...
static void g_task_thread_pool_init(void)
{
    ...
    g_source_set_callback(task_pool_manager, task_pool_manager_timeout, NULL,
                          NULL);
    g_source_set_ready_time(task_pool_manager, -1);
...
static void g_task_get_property(GObject* object, guint prop_id, GValue* value,
                                GParamSpec* pspec)
{
    GTask* task = G_TASK(object);
...
static void g_task_class_init(GTaskClass* klass)
{
     ...
     * g_task_return_error() or one of the other return functions have been
     * called on the task.
     *
     * This property is guaranteed to change from %FALSE to %TRUE exactly once.
     *
     * The #GObject::notify signal for this change is emitted in the same main
     * context as the task’s callback, immediately after that callback is
     * invoked.
     *
     * Since: 2.44
     */
    g_object_class_install_property(
        gobject_class, PROP_COMPLETED,
        g_param_spec_boolean("completed", P_("Task completed"),
                             P_("Whether the task has completed yet"), FALSE,
