...
static void nfnl_queue_dump(struct nl_object* a, struct nl_dump_params* p)
{
    ...
    if (queue->ce_mask & QUEUE_ATTR_COPY_MODE)
        nl_dump(
            p, "copy_mode=%s ",
            nfnl_queue_copy_mode2str(queue->queue_copy_mode, buf, sizeof(buf)));
...
char* nfnl_queue_copy_mode2str(enum nfnl_queue_copy_mode copy_mode, char* buf,
                               size_t len)
{
    return __type2str(copy_mode, buf, len, copy_modes, ARRAY_SIZE(copy_modes));
}
...
enum nfnl_queue_copy_mode
    nfnl_queue_get_copy_mode(const struct nfnl_queue* queue)
{
