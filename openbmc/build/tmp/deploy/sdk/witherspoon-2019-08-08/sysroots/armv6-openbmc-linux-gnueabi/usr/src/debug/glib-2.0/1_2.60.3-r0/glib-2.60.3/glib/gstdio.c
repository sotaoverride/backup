 ...
 */
static gint64 _g_win32_filetime_to_unix_time(FILETIME* ft)
{
    gint64 result;
    /* 1 unit of FILETIME is 100ns */
    const gint64 hundreds_of_usec_per_sec = 10000000;
    /* The difference between January 1, 1601 UTC (FILETIME epoch) and UNIX
     * epoch in hundreds of nanoseconds.
     */
...
static int _g_win32_fill_statbuf_from_handle_info(
    const wchar_t* filename, const wchar_t* filename_target,
    BY_HANDLE_FILE_INFORMATION* handle_info, struct __stat64* statbuf)
{
     ...
     */
    if (filename_target != NULL && filename_target[0] != L'\0' &&
        filename_target[1] == L':')
        drive_letter_w = filename_target[0];
    else if (filename[0] != L'\0' && filename[1] == L':')
        drive_letter_w = filename[0];

    if (drive_letter_w > 0 && iswalpha(drive_letter_w) &&
        iswascii(drive_letter_w) && wctomb(drive_letter, drive_letter_w) == 1)
    ...
    if ((handle_info->dwFileAttributes & FILE_ATTRIBUTE_DIRECTORY) ==
    ...
    if ((handle_info->dwFileAttributes & FILE_ATTRIBUTE_READONLY) !=
    ...
    if (!S_ISDIR(statbuf->st_mode))
    {
        ...
        if ((dot != NULL &&
             (wcsicmp(dot, L".exe") == 0 || wcsicmp(dot, L".com") == 0 ||
              wcsicmp(dot, L".bat") == 0 || wcsicmp(dot, L".cmd") == 0)))
            statbuf->st_mode |= S_IXUSR | S_IXGRP | S_IXOTH;
    }

    statbuf->st_nlink = handle_info->nNumberOfLinks;
    statbuf->st_uid = statbuf->st_gid = 0;
    statbuf->st_size = (((guint64)handle_info->nFileSizeHigh) << 32) |
                       handle_info->nFileSizeLow;
    statbuf->st_ctime =
        _g_win32_filetime_to_unix_time(&handle_info->ftCreationTime);
    statbuf->st_mtime =
        _g_win32_filetime_to_unix_time(&handle_info->ftLastWriteTime);
    statbuf->st_atime =
        _g_win32_filetime_to_unix_time(&handle_info->ftLastAccessTime);
                                                   ...
                                                   gboolean for_symlink)
{
    ...
    if (fd < 0)
    {
        ...
        is_symlink = (immediate_attributes & FILE_ATTRIBUTE_REPARSE_POINT) ==
                     FILE_ATTRIBUTE_REPARSE_POINT;
        is_directory = (immediate_attributes & FILE_ATTRIBUTE_DIRECTORY) ==
                       FILE_ATTRIBUTE_DIRECTORY;
        ...
        file_handle =
            CreateFileW(filename, FILE_READ_ATTRIBUTES, FILE_SHARE_READ, NULL,
    ...
    succeeded_so_far = GetFileInformationByHandle(file_handle, &handle_info);
    error_code = GetLastError();

    if (succeeded_so_far)
    {
        succeeded_so_far = GetFileInformationByHandleEx(
            file_handle, FileStandardInfo, &std_info, sizeof(std_info));
        error_code = GetLastError();
    }
    ...
    if (fd < 0)
    {
        ...
        {
            HANDLE tmp = FindFirstFileW(filename, &finddata);
...
#undef SANE_LIMIT
            ...
            if (new_len > 0)
            {
                 ...
                 * and ask for a bit more space than is needed, and allocate
                 * even more.
                 */
                filename_target_len = new_len + 3;
                filename_target =
                    g_malloc((filename_target_len + 1) * sizeof(wchar_t));

                new_len = GetFinalPathNameByHandleW(
                    file_handle, filename_target, filename_target_len,
                ...
                /* GetFinalPathNameByHandle() is documented to return extended
                 * paths, strip the extended prefix, if it is followed by a
                 * drive letter and a colon. Otherwise keep it (the path could
                 * be
                ...
                else if (new_len > 0)
                {
                    ...
                    _g_win32_strip_extended_ntobjm_prefix(filename_target,
                                                          &len);
                    new_len = len;
                }
            }
     ...
     */
    if (fd < 0)
        result = _g_win32_fill_statbuf_from_handle_info(
            filename, filename_target, &handle_info, &statbuf);
    else
    ...
    buf->file_index = (((guint64)handle_info.nFileIndexHigh) << 32) |
                      handle_info.nFileIndexLow;
    ...
    buf->st_size =
        (((guint64)handle_info.nFileSizeHigh) << 32) | handle_info.nFileSizeLow;
...
static int _g_win32_stat_utf8(const gchar* filename, GWin32PrivateStat* buf,
                              gboolean for_symlink)
{
    wchar_t* wfilename;
    int result;
    ...
    result = _g_win32_stat_utf16_no_trailing_slashes(wfilename, -1, buf,
                                                     for_symlink);
...
static int _g_win32_readlink_utf16_raw(const gunichar2* filename,
                                       gunichar2* buf, gsize buf_size)
{
    DWORD returned_bytes;
    BYTE returned_data[MAXIMUM_REPARSE_DATA_BUFFER_SIZE]; /* This is 16k, by the
                                                             way */
    HANDLE h;
    DWORD attributes;
     ...
     */
    h = CreateFileW(filename, FILE_READ_EA,
    ...
    if (!DeviceIoControl(h, FSCTL_GET_REPARSE_POINT, NULL, 0, returned_data,
                         MAXIMUM_REPARSE_DATA_BUFFER_SIZE, &returned_bytes,
                         NULL))
    {
        error_code = GetLastError();
        errno = w32_error_to_errno(error_code);
    ...
    if (rep_buf->ReparseTag == IO_REPARSE_TAG_SYMLINK)
    {
        ...
        memcpy(buf,
               &((BYTE*)rep_buf->SymbolicLinkReparseBuffer.PathBuffer)
                   [rep_buf->SymbolicLinkReparseBuffer.SubstituteNameOffset],
               to_copy);
    }
    else if (rep_buf->ReparseTag == IO_REPARSE_TAG_MOUNT_POINT)
    {
        ...
        memcpy(buf,
               &((BYTE*)rep_buf->MountPointReparseBuffer.PathBuffer)
...
static int _g_win32_readlink_utf16(const gunichar2* filename, gunichar2* buf,
                                   gsize buf_size)
{
...
static gchar* _g_win32_get_mode_alias(const gchar* mode)
{
    gchar* alias;
...
int g_win32_readlink_utf8(const gchar* filename, gchar* buf, gsize buf_size)
{
    ...
    if (result > 0)
    {
        glong tmp_len;
        gchar* tmp =
            g_utf16_to_utf8((const gunichar2*)buf, result / sizeof(gunichar2),
                            NULL, &tmp_len, NULL);
