 ...
 * slab allocator and magazine extensions as outlined in:
...
#define P2ALIGNMENT                                                            \
    (2 * sizeof(gsize)) /* fits 2 pointers (assumed to be 2 *                  \
                           GLIB_SIZEOF_SIZE_T below) */
#define ALIGN(size, base) ((base) * (gsize)(((size) + (base)-1) / (base)))
#define NATIVE_MALLOC_PADDING                                                  \
    P2ALIGNMENT /* per-page padding left for native malloc(3) see [1] */
#define SLAB_INFO_SIZE P2ALIGN(sizeof(SlabInfo) + NATIVE_MALLOC_PADDING)
#define MAX_MAGAZINE_SIZE                                                      \
    (256) /* see [3] and allocator_get_magazine_threshold() for this */
#define MIN_MAGAZINE_SIZE (4)
#define MAX_STAMP_COUNTER (7) /* distributes the load of gettimeofday() */
#define MAX_SLAB_CHUNK_SIZE(al)                                                \
    (((al)->max_page_size - SLAB_INFO_SIZE) /                                  \
     8) /* we want at last 8 chunks per page, see [4] */
#define MAX_SLAB_INDEX(al) (SLAB_INDEX(al, MAX_SLAB_CHUNK_SIZE(al)) + 1)
#define SLAB_INDEX(al, asize)                                                  \
    ((asize) / P2ALIGNMENT - 1) /* asize must be P2ALIGNMENT aligned */
...
static void g_slice_init_nomessage(void)
{
...
#else
    sys_page_size =
        sysconf(_SC_PAGESIZE); /* = sysconf (_SC_PAGE_SIZE); = getpagesize(); */
#endif
    ...
    {
        allocator->contention_counters =
            g_new0(guint, MAX_SLAB_INDEX(allocator));
    ...
    allocator->max_slab_chunk_size_for_magazine_cache =
...
static inline guint allocator_categorize(gsize aligned_chunk_size)
{
    ...
    if (G_LIKELY(aligned_chunk_size &&
                 aligned_chunk_size <=
                     allocator->max_slab_chunk_size_for_magazine_cache))
        return 1; /* use magazine cache */

    if (!allocator->config.always_malloc && aligned_chunk_size &&
...
static inline void g_mutex_lock_a(GMutex* mutex, guint* contention_counter)
{
    ...
    if (contention)
    {
        ...
        if (allocator->mutex_counter >= 1) /* quickly adapt to contention */
        {
            allocator->mutex_counter = 0;
            *contention_counter =
                MIN(*contention_counter + 1, MAX_MAGAZINE_SIZE);
        }
    }
    else /* !contention */
    {
...
static inline ThreadMemory* thread_memory_from_self(void)
{
    ...
    if (G_UNLIKELY(!tmem))
    {
        ...
        if
            G_UNLIKELY(sys_page_size == 0)
        g_slice_init_nomessage();
        g_mutex_unlock(&init_mutex);

        n_magazines = MAX_SLAB_INDEX(allocator);
        tmem = g_private_set_alloc0(&private_thread_memory,
                                    sizeof(ThreadMemory) +
                                        sizeof(Magazine) * 2 * n_magazines);
                                                     ...
                                                     guint ix)
{
     ...
     * which is required by the implementation. also, for moderately sized
     * chunks (say >= 64 bytes), magazine sizes shouldn't be much smaller then
     * the number of chunks available per page/2 to avoid excessive traffic in
     * the magazine cache for small to medium sized structures. the upper bound
     * of the magazine size is effectively provided by MAX_MAGAZINE_SIZE. for
     * larger chunks, this number is scaled down so that the content of a single
     * magazine doesn't exceed ca. 16KB.
     */
    gsize chunk_size = SLAB_CHUNK_SIZE(allocator, ix);
    guint threshold = MAX(MIN_MAGAZINE_SIZE, allocator->max_page_size /
                                                 MAX(5 * chunk_size, 5 * 32));
...
static inline void magazine_cache_update_stamp(void)
{
    if (allocator->stamp_counter >= MAX_STAMP_COUNTER)
    {
        ...
        allocator->last_stamp =
            tv.tv_sec * 1000 + tv.tv_usec / 1000; /* milli seconds */
...
static void
    magazine_cache_push_magazine(guint ix, ChunkLink* magazine_chunks,
...
static inline gboolean thread_memory_magazine1_is_empty(ThreadMemory* tmem,
                                                        guint ix)
{
    return tmem->magazine1[ix].chunks == NULL;
}

static inline gboolean thread_memory_magazine2_is_full(ThreadMemory* tmem,
                                                       guint ix)
{
...
static inline gpointer thread_memory_magazine1_alloc(ThreadMemory* tmem,
                                                     guint ix)
{
    Magazine* mag = &tmem->magazine1[ix];
    ChunkLink* chunk = magazine_chain_pop_head(&mag->chunks);
...
static inline void thread_memory_magazine2_free(ThreadMemory* tmem, guint ix,
                                                gpointer mem)
{
    Magazine* mag = &tmem->magazine2[ix];
    ChunkLink* chunk = mem;
 ...
 * Since: 2.10
 */
void g_slice_free_chain_with_offset(gsize mem_size, gpointer mem_chain,
                                    gsize next_offset)
{
...
static void allocator_slab_stack_push(Allocator* allocator, guint ix,
                                      SlabInfo* sinfo)
{
...
static void allocator_add_slab(Allocator* allocator, guint ix, gsize chunk_size)
{
    ...
    page_size = allocator_aligned_page_size(
        allocator, SLAB_BPAGE_SIZE(allocator, chunk_size));
    /* allocate 1 page for the chunks and the slab */
    aligned_memory =
        allocator_memalign(page_size, page_size - NATIVE_MALLOC_PADDING);
    errsv = errno;
    mem = aligned_memory;
    ...
    if (!mem)
    {
        ...
        mem_error("failed to allocate %u bytes (alignment: %u): %s\n",
                  (guint)(page_size - NATIVE_MALLOC_PADDING), (guint)page_size,
                  syserr);
    }
    /* mask page address */
...
static void slab_allocator_free_chunk(gsize chunk_size, gpointer mem)
{
    ...
    gboolean was_empty;
    guint ix = SLAB_INDEX(allocator, chunk_size);
    gsize page_size = allocator_aligned_page_size(
        allocator, SLAB_BPAGE_SIZE(allocator, chunk_size));
 ...
 * <stdlib.h> define HAVE_MEMALIGN                 1 // if free(memalign(3))
 * works, <malloc.h> define HAVE_VALLOC                   1 // if
 * free(valloc(3)) works, <stdlib.h> or <malloc.h> if none is provided, we
 * implement malloc(3)-based alloc-only page alignment
 */
...
static gpointer allocator_memalign(gsize alignment, gsize memsize)
{
    ...
    if (!compat_valloc_trash)
    {
        ...
        if (mem)
        {
            ...
            while (--i >= 0)
                g_trash_stack_push(&compat_valloc_trash,
                                   amem + i * sys_page_size);
...
static void mem_error(const char* format, ...)
{
    const char* pname;
    va_list args;
    /* at least, put out "MEMORY-ERROR", in case we segfault during the rest of
     * the function */
    fputs("\n***MEMORY-ERROR***: ", stderr);
    pname = g_get_prgname();
...
static int smc_notify_free(void* pointer, size_t size)
{
    ...
    {
        g_fprintf(stderr,
                  "GSlice: MemChecker: attempt to release non-allocated block: "
                  "%p size=%" G_GSIZE_FORMAT "\n",
                  pointer, size);
        return 0;
    }
    ...
    {
        g_fprintf(stderr,
                  "GSlice: MemChecker: attempt to release block with invalid "
                  "size: %p size=%" G_GSIZE_FORMAT
...
#define SMC_TRUNK_COUNT                                                        \
    (4093 /* 16381 */)         /* prime, to distribute trunk collisions (big,  \
                                  allocated just once) */
#define SMC_BRANCH_COUNT (511) /* prime, to distribute branch collisions */
#define SMC_TRUNK_EXTENT                                                       \
    (SMC_BRANCH_COUNT * 2039) /* key address space per trunk, should           \
                                 distribute uniformly across BRANCH_COUNT */
#define SMC_TRUNK_HASH(k)                                                      \
    ((k / SMC_TRUNK_EXTENT) %                                                  \
     SMC_TRUNK_COUNT) /* generate new trunk hash per megabyte (roughly) */
...
static inline SmcEntry* smc_tree_branch_grow_L(SmcBranch* branch,
                                               unsigned int index)
{
...
static inline SmcEntry* smc_tree_branch_lookup_nearest_L(SmcBranch* branch,
                                                         SmcKType key)
{
    ...
    return cmp > 0 ? check + 1
                   : check; /* return insertion position for inexact match */
}
...
static void smc_tree_insert(SmcKType key, SmcVType value)
{
    ...
    {
        smc_tree_root[ix0] =
            calloc(SMC_BRANCH_COUNT, sizeof(smc_tree_root[0][0]));
        if (!smc_tree_root[ix0])
            smc_tree_abort(errno);
    }
    entry = smc_tree_branch_lookup_nearest_L(&smc_tree_root[ix0][ix1], key);
    if (!entry || /* need create */
        entry >= smc_tree_root[ix0][ix1].entries +
                     smc_tree_root[ix0][ix1].n_entries || /* need append */
        entry->key != key)                                /* need insert */
        entry = smc_tree_branch_grow_L(&smc_tree_root[ix0][ix1],
                                       entry - smc_tree_root[ix0][ix1].entries);
    entry->key = key;
    entry->value = value;
...
static gboolean smc_tree_lookup(SmcKType key, SmcVType* value_p)
{
    ...
    *value_p = 0;
    ...
    if (smc_tree_root && smc_tree_root[ix0])
    {
        ...
        if (entry &&
            entry < smc_tree_root[ix0][ix1].entries +
                        smc_tree_root[ix0][ix1].n_entries &&
            entry->key == key)
        {
            found_one = TRUE;
            *value_p = entry->value;
...
static gboolean smc_tree_remove(SmcKType key)
{
    ...
    {
        SmcEntry* entry =
            smc_tree_branch_lookup_nearest_L(&smc_tree_root[ix0][ix1], key);
        if (entry &&
            entry < smc_tree_root[ix0][ix1].entries +
                        smc_tree_root[ix0][ix1].n_entries &&
            entry->key == key)
        {
            ...
            memmove(entry, entry + 1,
                    (smc_tree_root[ix0][ix1].n_entries - i) * sizeof(entry[0]));
...
void g_slice_debug_tree_statistics(void)
{
    ...
    if (smc_tree_root)
    {
        unsigned int i, j, t = 0, o = 0, b = 0, su = 0, ex = 0,
                           en = 4294967295u;
        double tf, bf;
        ...
        g_fprintf(stderr,
                  "GSlice: MemChecker: %f entries per branch, %u minimum, %u "
                  "maximum\n",
                  su / bf, en, ex);
    }
    else
     ...
     * $ cat /proc/8887/statm # total-program-size resident-set-size
     * shared-pages text/code data/stack library dirty-pages 114017 103714 2354
     * 344 0 108676 0 $ cat /proc/8887/status Name:   beast-0.7.1 VmSize: 456068
     * kB VmLck:         0 kB VmRSS:    414856 kB VmData:   434620 kB VmStk: 84
     * kB VmExe:      1376 kB VmLib:     13036 kB VmPTE:       456 kB Threads: 3
