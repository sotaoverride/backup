...
#define RESOLVE_DONT_DESTROY(resolve)                                          \
    _cleanup_(sd_resolve_unrefp)                                               \
        _unused_ sd_resolve* _dont_destroy_##resolve = sd_resolve_ref(resolve)

static void query_assign_errno(sd_resolve_query* q, int ret, int error,
                               int h_error)
{
    assert(q);
...
static void* serialize_addrinfo(void* p, const struct addrinfo* ai,
                                size_t* length, size_t maxlength)
{
    ...
    memcpy((uint8_t*)p + sizeof(AddrInfoSerialization), ai->ai_addr,
           ai->ai_addrlen);
...
static int send_addrinfo_reply(int out_fd, unsigned id, int ret,
                               struct addrinfo* ai, int _errno, int _h_errno)
{
    ...
    if (ret == 0 && ai)
    {
        ...
        for (k = ai; k; k = k->ai_next)
        {
            p = serialize_addrinfo(p, k, &resp.header.length,
                                   (uint8_t*)&buffer + BUFSIZE - (uint8_t*)p);
            if (!p)
            {
                freeaddrinfo(ai);
                return -ENOBUFS;
...
static int send_nameinfo_reply(int out_fd, unsigned id, int ret,
                               const char* host, const char* serv, int _errno,
                               int _h_errno)
{
...
static int handle_request(int out_fd, const Packet* packet, size_t length)
{
    ...
    switch (req->type)
    {
        ...
        case REQUEST_ADDRINFO:
        {
            ...
            assert_return(length == sizeof(AddrInfoRequest) + ai_req->node_len +
                                        ai_req->service_len,
                          -EBADMSG);
            ...
            node = ai_req->node_len
                       ? (const char*)ai_req + sizeof(AddrInfoRequest)
                       : NULL;
            service = ai_req->service_len
                          ? (const char*)ai_req + sizeof(AddrInfoRequest) +
                                ai_req->node_len
                          : NULL;

            ret = getaddrinfo(node, service,
                              ai_req->hints_valid ? &hints : NULL, &result);
        ...
        case REQUEST_NAMEINFO:
        {
            ...
            assert_return(length ==
                              sizeof(NameInfoRequest) + ni_req->sockaddr_len,
                          -EBADMSG);
            assert_return(ni_req->sockaddr_len <= sizeof(sa), -EBADMSG);

            memcpy(&sa, (const uint8_t*)ni_req + sizeof(NameInfoRequest),
                   ni_req->sockaddr_len);

            ret = getnameinfo(
                &sa.sa, ni_req->sockaddr_len, ni_req->gethost ? hostbuf : NULL,
                ni_req->gethost ? sizeof(hostbuf) : 0,
                ni_req->getserv ? servbuf : NULL,
                ni_req->getserv ? sizeof(servbuf) : 0, ni_req->flags);

            return send_nameinfo_reply(
                out_fd, req->id, ret,
                ret == 0 && ni_req->gethost ? hostbuf : NULL,
                ret == 0 && ni_req->getserv ? servbuf : NULL, errno, h_errno);
        }
...
static void* thread_worker(void* p)
{
    ...
    while (!resolve->dead)
    {
        ...
        if (handle_request(resolve->fds[RESPONSE_SEND_FD], &buf.packet,
                           (size_t)length) < 0)
            break;
    }
...
static int start_threads(sd_resolve* resolve, unsigned extra)
{
    ...
    /* No signals in forked off threads please. We set the mask before forking,
     * so that the threads never exist with a different mask than a fully
     * blocked one */
    ...
    {
        r = pthread_create(&resolve->workers[resolve->n_valid_workers], NULL,
                           thread_worker, resolve);
        if (r > 0)
        {
            r = -r;
            goto finish;
...
_public_ int sd_resolve_new(sd_resolve** ret)
{
    ...
    if (socketpair(PF_UNIX, SOCK_DGRAM | SOCK_CLOEXEC, 0,
                   resolve->fds + REQUEST_RECV_FD) < 0)
        return -errno;

    if (socketpair(PF_UNIX, SOCK_DGRAM | SOCK_CLOEXEC, 0,
                   resolve->fds + RESPONSE_RECV_FD) < 0)
        return -errno;
...
static sd_resolve* resolve_free(sd_resolve* resolve)
{
    ...
    if (resolve->fds[REQUEST_SEND_FD] >= 0)
    {
        ...
        for (i = 0; i < resolve->n_valid_workers; i++)
            (void)send(resolve->fds[REQUEST_SEND_FD], &req, req.length,
                       MSG_NOSIGNAL);
    }
...
static int unserialize_addrinfo(const void** p, size_t* length,
                                struct addrinfo** ret_ai)
{
    ...
    *ai = (struct addrinfo){
    ...
    if (s.ai_addrlen > 0)
    {
        ai->ai_addr = memdup((const uint8_t*)*p + sizeof(AddrInfoSerialization),
                             s.ai_addrlen);
        if (!ai->ai_addr)
        {
            free(ai);
    ...
    {
        ai->ai_canonname = memdup(
            (const uint8_t*)*p + sizeof(AddrInfoSerialization) + s.ai_addrlen,
            s.canonname_len);
        if (!ai->ai_canonname)
        {
            free(ai->ai_addr);
                           ...
                           size_t length)
{
    ...
    switch (resp->type)
    {
        ...
        case RESPONSE_ADDRINFO:
        {
            ...
            query_assign_errno(q, ai_resp->ret, ai_resp->_errno,
                               ai_resp->_h_errno);
        ...
        case RESPONSE_NAMEINFO:
        {
            ...
            if (ni_resp->hostlen > DNS_HOSTNAME_MAX ||
                ...
                sizeof(NameInfoResponse) + ni_resp->hostlen + ni_resp->servlen >
                    length)
                query_assign_errno(q, EAI_SYSTEM, EIO, 0);
            else
            {
                query_assign_errno(q, ni_resp->ret, ni_resp->_errno,
                                   ni_resp->_h_errno);

                if (ni_resp->hostlen > 0)
                {
                    q->host =
                        strndup((const char*)ni_resp + sizeof(NameInfoResponse),
                                ni_resp->hostlen - 1);
                    if (!q->host)
                ...
                if (ni_resp->servlen > 0)
                {
                    q->serv =
                        strndup((const char*)ni_resp +
                                    sizeof(NameInfoResponse) + ni_resp->hostlen,
                                ni_resp->servlen - 1);
                    if (!q->serv)
                        query_assign_errno(q, EAI_MEMORY, ENOMEM, 0);
                }
...
_public_ int sd_resolve_wait(sd_resolve* resolve, uint64_t timeout_usec)
{
    ...
    do
    {
        r = fd_wait_for_event(resolve->fds[RESPONSE_RECV_FD], POLLIN,
                              timeout_usec);
    } while (r == -EINTR);
...
static int alloc_query(sd_resolve* resolve, bool floating,
                       sd_resolve_query** _q)
{
    sd_resolve_query* q;
    int r;
    ...
    q = resolve->query_array[resolve->current_id % QUERIES_MAX] =
...
int resolve_getaddrinfo_with_destroy_callback(
    sd_resolve* resolve, sd_resolve_query** ret_query, const char* node,
                                    ...
                                    void* userdata)
{
    ...
    return resolve_getaddrinfo_with_destroy_callback(
        resolve, ret_query, node, service, hints, callback, NULL, userdata);
}
...
int resolve_getnameinfo_with_destroy_callback(
    sd_resolve* resolve, sd_resolve_query** ret_query,
...
_public_ int sd_resolve_getnameinfo(sd_resolve* resolve,
                                    ...
                                    int flags, uint64_t get,
                                    sd_resolve_getnameinfo_handler_t callback,
                                    void* userdata)
{

    return resolve_getnameinfo_with_destroy_callback(
        resolve, ret_query, sa, salen, flags, get, callback, NULL, userdata);
}
...
DEFINE_PUBLIC_TRIVIAL_REF_UNREF_FUNC(sd_resolve_query, sd_resolve_query,
                                     resolve_query_free);
...
_public_ void* sd_resolve_query_set_userdata(sd_resolve_query* q,
                                             void* userdata)
{
    void* ret;
...
_public_ int sd_resolve_query_get_destroy_callback(
    sd_resolve_query* q, sd_resolve_destroy_t* destroy_callback)
{
    assert_return(q, -EINVAL);
...
_public_ int
    sd_resolve_query_set_destroy_callback(sd_resolve_query* q,
                                          sd_resolve_destroy_t destroy_callback)
{
    assert_return(q, -EINVAL);
...
_public_ int sd_resolve_attach_event(sd_resolve* resolve, sd_event* event,
                                     int64_t priority)
{
    int r;
    ...
    r = sd_event_add_io(resolve->event, &resolve->event_source,
                        resolve->fds[RESPONSE_RECV_FD], POLLIN, io_callback,
                        resolve);
    if (r < 0)
        goto fail;
