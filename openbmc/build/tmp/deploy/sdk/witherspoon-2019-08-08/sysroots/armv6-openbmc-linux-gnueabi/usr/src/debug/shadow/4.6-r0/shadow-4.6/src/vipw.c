...
#define MSG_WARN_EDIT_OTHER_FILE                                               \
    _("You have modified %s.\n"                                                \
...
static void usage(int status)
{
    ...
    (void)fputs(_("  -R, --root CHROOT_DIR         directory to chroot into\n"),
                usageout);
    (void)fputs(
        _("  -s, --shadow                  edit shadow or gshadow database\n"),
        usageout);
#ifdef WITH_TCB
    (void)fputs(_("  -u, --user                    which user's tcb shadow "
                  "file to edit\n"),
                usageout);
#endif /* WITH_TCB */
    (void)fputs(_("\n"), usageout);
...
static int create_backup_file(FILE* fp, const char* backup, struct stat* sb)
{
    ...
    if ((utime(backup, &ub) != 0) || (chmod(backup, sb->st_mode) != 0) ||
 ...
 *
 */
static void vipwedit(const char* file, int (*file_lock)(void),
                     int (*file_unlock)(void))
{
    ...
    else if (0 == pid)
    {
        ...
        snprintf(buf, strlen(editor) + strlen(fileedit) + 2, "%s %s", editor,
                 fileedit);
        status = system(buf);
        if (-1 == status)
        {
            fprintf(stderr, _("%s: %s: %s\n"), Prog, editor, strerror(errno));
            exit(1);
        }
...
int main(int argc, char** argv)
{
    ...
    if (do_vipw)
    {
        if (editshadow)
        {
            ...
            printf(MSG_WARN_EDIT_OTHER_FILE, spw_dbname(), pw_dbname(), "vipw");
        }
        else
        {
            ...
            if (spw_file_present())
            {
                printf(MSG_WARN_EDIT_OTHER_FILE, pw_dbname(), spw_dbname(),
                       "vipw -s");
            }
        }
    }
    else
    {
        ...
        if (editshadow)
        {
            ...
            printf(MSG_WARN_EDIT_OTHER_FILE, sgr_dbname(), gr_dbname(), "vigr");
        }
        else
        {
#endif /* SHADOWGRP */
            ...
            if (sgr_file_present())
            {
                printf(MSG_WARN_EDIT_OTHER_FILE, gr_dbname(), sgr_dbname(),
                       "vigr -s");
            }
        }
#endif /* SHADOWGRP */
