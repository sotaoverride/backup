...
static void help(void)
{
    printf(
        ...
        "allocate in the broker\n"
        "     --max-fds FDS              Maximum number of file descriptors "
        "each user may allocate in the broker\n"
        "     --max-matches MATCHES      Maximum number of match rules each "
        "user may allocate in the broker\n"
        "     --max-objects OBJECTS      Maximum total number of names, peers, "
        "pending replies, etc each user may allocate in the broker\n",
...
static int parse_argv(int argc, char* argv[])
{
    ...
    while ((c = getopt_long(argc, argv, "h", options, NULL)) >= 0)
    {
        switch (c)
        {
            ...
            case ARG_CONTROLLER:
            {
                ...
                {
                    fprintf(stderr,
                            "%s: invalid controller file-descriptor -- '%s'\n",
            ...
            case ARG_MAX_MATCHES:
                ...
                {
                    fprintf(stderr,
                            "%s: invalid max number of matches -- '%s'\n",
                            program_invocation_name, optarg);
                    return MAIN_FAILED;
                }
            ...
            case ARG_MAX_OBJECTS:
                ...
                {
                    fprintf(stderr,
                            "%s: invalid max number of objects -- '%s'\n",
                            program_invocation_name, optarg);
                    return MAIN_FAILED;
                }
    ...
    if (main_arg_log >= 0)
    {
        ...
        {
            fprintf(stderr,
                    "%s: socket type of log file-descriptor not supported -- "
                    "'%d'\n",
                    program_invocation_name, main_arg_log);
            return MAIN_FAILED;
        }
    ...
    /* verify controller-fd is STREAM */
    {
        ...
        if (r < 0)
        {
            ...
            fprintf(stderr,
                    "%s: controller file-descriptor not a socket -- '%d'\n",
        ...
        {
            fprintf(stderr,
                    "%s: socket type of controller file-descriptor not "
                    "supported -- '%d'\n",
                    program_invocation_name, main_arg_controller);
            return MAIN_FAILED;
        }
...
static int run(void)
{
    ...
    r = broker_new(&broker, main_arg_machine_id, main_arg_log,
                   main_arg_controller, main_arg_max_bytes, main_arg_max_fds,
                   main_arg_max_matches, main_arg_max_objects);
    if (!r)
        r = broker_run(broker);
