...
// Some Android NDKs (Google NDK and older Crystax.NET NDK versions) don't
// define SYS_futex
...
BOOST_LOG_API void futex_based_event::wait()
{
    if (m_state.exchange(0, boost::memory_order_acq_rel) == 0)
    {
        while (true)
        {
            if (::syscall(BOOST_LOG_SYS_FUTEX, &m_state.storage(),
            ...
            else if (BOOST_UNLIKELY(err != EINTR))
            {
                BOOST_LOG_THROW_DESCR_PARAMS(
                    system_error, "Failed to block on the futex", (err));
            }
        }
...
BOOST_LOG_API void futex_based_event::set_signalled()
{
    ...
    {
        if (BOOST_UNLIKELY(::syscall(BOOST_LOG_SYS_FUTEX, &m_state.storage(),
                                     BOOST_LOG_FUTEX_WAKE, 1, NULL, NULL,
                                     0) < 0))
        {
            const int err = errno;
            BOOST_LOG_THROW_DESCR_PARAMS(
                system_error, "Failed to wake threads blocked on the futex",
                (err));
        }
    }
}
...
BOOST_LOG_API void sem_based_event::wait()
{
    ...
    while (true)
    {
        if (sem_wait(&m_semaphore) != 0)
        {
            ...
            {
                BOOST_LOG_THROW_DESCR_PARAMS(
                    system_error, "Failed to block on the semaphore", (err));
            }
        }
        else
            break;
...
BOOST_LOG_API void sem_based_event::set_signalled()
{
    if (!m_state.test_and_set(boost::memory_order_release))
    {
        if (BOOST_UNLIKELY(sem_post(&m_semaphore) != 0))
        {
            const int err = errno;
            BOOST_LOG_THROW_DESCR_PARAMS(
                system_error, "Failed to wake the blocked thread", (err));
        }
    }
}
...
BOOST_LOG_API winapi_based_event::winapi_based_event() :
    m_state(0), m_event(CreateEventA(NULL, false, false, NULL))
{
...
{
    // On Windows we assume that memory view is always actual (Intel x86 and
    // x86_64 arch)
    if (const_cast<volatile boost::uint32_t&>(m_state) == 0)
    {
        if (BOOST_UNLIKELY(WaitForSingleObject(m_event, INFINITE) != 0))
        {
            const DWORD err = GetLastError();
            BOOST_LOG_THROW_DESCR_PARAMS(
                system_error, "Failed to block on Windows event", (err));
        }
    }
...
{
    if (BOOST_INTERLOCKED_COMPARE_EXCHANGE(reinterpret_cast<long*>(&m_state), 1,
                                           0) == 0)
    {
