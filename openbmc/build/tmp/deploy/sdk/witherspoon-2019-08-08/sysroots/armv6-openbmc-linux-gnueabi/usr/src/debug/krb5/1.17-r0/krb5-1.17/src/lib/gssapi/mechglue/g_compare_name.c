...
static OM_uint32 val_comp_name_args(OM_uint32* minor_status, gss_name_t name1,
...
OM_uint32 KRB5_CALLCONV gss_compare_name(minor_status, name1, name2, name_equal)
    ...
    *name_equal = 0; /* Default to *not* equal.... */
    ...
    if (union_name1->mech_type && union_name2->mech_type)
    {
        ...
        if (!mech->gss_compare_name)
            return (GSS_S_UNAVAILABLE);
        major_status =
            mech->gss_compare_name(minor_status, union_name1->mech_name,
    ...
    if (!union_name1->mech_type && !union_name2->mech_type)
    {
        ...
        if ((union_name1->name_type != GSS_C_NULL_OID &&
             union_name2->name_type != GSS_C_NULL_OID) &&
            !g_OID_equal(union_name1->name_type, union_name2->name_type))
            return (GSS_S_COMPLETE);
        /*
    ...
    major_status = gssint_import_internal_name(
        minor_status, union_name1->mech_type, union_name2, &internal_name);
    if (major_status != GSS_S_COMPLETE)
    ...
    if (!mech->gss_compare_name)
        return (GSS_S_UNAVAILABLE);
    major_status = mech->gss_compare_name(minor_status, union_name1->mech_name,
