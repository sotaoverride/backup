...
typedef struct MapField
{
    ...
    int (*map)(const char* field, const char** p, struct iovec** iov,
               size_t* n_iov_allocated, size_t* n_iov);
} MapField;

static int map_simple_field(const char* field, const char** p,
                            struct iovec** iov, size_t* n_iov_allocated,
                            size_t* n_iov)
{
...
static int map_string_field_internal(const char* field, const char** p,
                                     struct iovec** iov,
                                     size_t* n_iov_allocated, size_t* n_iov,
...
static int map_string_field(const char* field, const char** p,
                            struct iovec** iov, size_t* n_iov_allocated,
                            size_t* n_iov)
{
    return map_string_field_internal(field, p, iov, n_iov_allocated, n_iov,
                                     false);
}

static int map_string_field_printable(const char* field, const char** p,
                                      struct iovec** iov,
                                      size_t* n_iov_allocated, size_t* n_iov)
{
    return map_string_field_internal(field, p, iov, n_iov_allocated, n_iov,
...
static int map_generic_field(const char* prefix, const char** p,
                             struct iovec** iov, size_t* n_iov_allocated,
                             size_t* n_iov)
{
    const char *e, *f;
...
static int map_all_fields(const char* p, const MapField map_fields[],
                          const char* prefix, bool handle_msg,
                          struct iovec** iov, size_t* n_iov_allocated,
                          size_t* n_iov)
{
    ...
    for (;;)
    {
        ...
        if (handle_msg)
        {
            ...
            if (v)
            {
                ...
                e = endswith(v, "'");
                if (!e)
                    return 0; /* don't continue splitting up if the final
                                 quotation mark is missing */

                c = strndupa(v, e - v);
                return map_all_fields(c, map_fields_userspace, "AUDIT_FIELD_",
                                      false, iov, n_iov_allocated, n_iov);
            }
        }
...
void process_audit_string(Server* s, int type, const char* data, size_t size)
{
    ...
    k = 0;
    if (sscanf(p, "(%" PRIu64 ".%" PRIu64 ":%" PRIu64 "):%n", &seconds, &msec,
               &id, &k) != 3 ||
        k == 0)
        return;
    ...
    map_all_fields(p, map_fields_kernel, "_AUDIT_FIELD_", true, &iov,
                   &n_iov_allocated, &n_iov);
    ...
    server_dispatch_message(s, iov, n_iov, n_iov_allocated, NULL, NULL,
                            LOG_NOTICE, 0);
...
void server_process_audit_message(Server* s, const void* buffer,
                                  size_t buffer_size, const struct ucred* ucred,
                                  ...
                                  socklen_t salen)
{
    ...
    if (!sa || salen != sizeof(struct sockaddr_nl) ||
        sa->nl.nl_family != AF_NETLINK || sa->nl.nl_pid != 0)
    {
    ...
    /* Except AUDIT_USER, all messsages below AUDIT_FIRST_USER_MSG are control
     * messages, let's ignore those */
    if (nl->nlmsg_type < AUDIT_FIRST_USER_MSG && nl->nlmsg_type != AUDIT_USER)
        return;

    process_audit_string(s, nl->nlmsg_type, NLMSG_DATA(nl),
                         nl->nlmsg_len - ALIGN(sizeof(struct nlmsghdr)));
}
...
int server_open_audit(Server* s)
{
    ...
    if (s->audit_fd < 0)
    {
        ...
        s->audit_fd = socket(
            AF_NETLINK, SOCK_RAW | SOCK_CLOEXEC | SOCK_NONBLOCK, NETLINK_AUDIT);
        if (s->audit_fd < 0)
        {
            if (IN_SET(errno, EAFNOSUPPORT, EPROTONOSUPPORT))
                ...
                log_warning_errno(
                    errno, "Failed to create audit socket, ignoring: %m");
        ...
        {
            log_warning_errno(errno, "Failed to join audit multicast group. "
                                     "The kernel is probably too old or "
                                     "multicast reading is not supported. "
                                     "Ignoring: %m");
