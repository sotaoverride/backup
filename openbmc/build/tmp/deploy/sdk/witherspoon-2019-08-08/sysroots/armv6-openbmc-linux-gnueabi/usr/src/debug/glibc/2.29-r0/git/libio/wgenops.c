...
libc_hidden_def(_IO_least_wmarker)
     ...
     */
    void _IO_switch_to_main_wget_area(FILE* fp)
{
    wchar_t* tmp;
...
libc_hidden_def(_IO_switch_to_main_wget_area)
    ...
    void _IO_switch_to_wbackup_area(FILE* fp)
{
    wchar_t* tmp;
    fp->_flags |= _IO_IN_BACKUP;
    /* Swap _IO_read_end and _IO_save_end. */
...
libc_hidden_def(_IO_wsetb)
    ...
    wint_t _IO_wdefault_pbackfail(FILE* fp, wint_t c)
{
    if (fp->_wide_data->_IO_read_ptr > fp->_wide_data->_IO_read_base &&
        !_IO_in_backup(fp) && (wint_t)fp->_IO_read_ptr[-1] == c)
        --fp->_IO_read_ptr;
    else
    {
        ...
        if (!_IO_in_backup(fp))
        {
            ...
            else if (!_IO_have_wbackup(fp))
            {
                ...
                int backup_size = 128;
                wchar_t* bbuf = (wchar_t*)malloc(backup_size * sizeof(wchar_t));
                if (bbuf == NULL)
                    return WEOF;
                fp->_wide_data->_IO_save_base = bbuf;
                fp->_wide_data->_IO_save_end =
                    (fp->_wide_data->_IO_save_base + backup_size);
...
libc_hidden_def(_IO_wdefault_pbackfail)
    ...
    void _IO_wdefault_finish(FILE* fp, int dummy)
{
    struct _IO_marker* mark;
...
libc_hidden_def(_IO_wdefault_uflow)
    ...
    wint_t __woverflow(FILE* f, wint_t wch)
{
    if (f->_mode == 0)
        _IO_fwide(f, 1);
...
libc_hidden_def(__wunderflow)
    ...
    size_t _IO_wdefault_xsputn(FILE* f, const void* data, size_t n)
{
...
libc_hidden_def(_IO_wdoallocbuf)
    ...
    int _IO_wdefault_doallocate(FILE* fp)
{
    wchar_t* buf;
...
libc_hidden_def(_IO_switch_to_wget_mode)
    ...
    void _IO_free_wbackup_area(FILE* fp)
{
    if (_IO_in_backup(fp))
...
libc_hidden_def(_IO_free_wbackup_area)
    ...
    static int save_for_wbackup(FILE* fp, wchar_t* end_p)
{
    /* Append [_IO_read_base..end_p] to backup area. */
    ssize_t least_mark = _IO_least_wmarker(fp, end_p);
    /* needed_size is how much space we need in the backup area. */
    size_t needed_size = ((end_p - fp->_wide_data->_IO_read_base) - least_mark);
    /* FIXME: Dubious arithmetic if pointers are NULL */
    size_t current_Bsize =
    ...
    if (needed_size > current_Bsize)
    {
        wchar_t* new_buffer;
        avail = 100;
        new_buffer = (wchar_t*)malloc((avail + needed_size) * sizeof(wchar_t));
        if (new_buffer == NULL)
            return EOF; /* FIXME */
        if (least_mark < 0)
        {
        ...
        else
        {
            __wmemcpy(new_buffer + avail,
                      fp->_wide_data->_IO_read_base + least_mark, needed_size);
        }
        free(fp->_wide_data->_IO_save_base);
    ...
    else
    {
        ...
        if (least_mark < 0)
        {
            __wmemmove(fp->_wide_data->_IO_save_base + avail,
                       fp->_wide_data->_IO_save_end + least_mark, -least_mark);
...
int _IO_seekwmark(FILE* fp, struct _IO_marker* mark, int delta)
{
    ...
    if (mark->_pos >= 0)
    {
        if (_IO_in_backup(fp))
            _IO_switch_to_main_wget_area(fp);
        fp->_wide_data->_IO_read_ptr =
            (fp->_wide_data->_IO_read_base + mark->_pos);
    }
    else
    {
        if (!_IO_in_backup(fp))
            _IO_switch_to_wbackup_area(fp);
        fp->_wide_data->_IO_read_ptr =
            fp->_wide_data->_IO_read_end + mark->_pos;
    }
    return 0;
}
