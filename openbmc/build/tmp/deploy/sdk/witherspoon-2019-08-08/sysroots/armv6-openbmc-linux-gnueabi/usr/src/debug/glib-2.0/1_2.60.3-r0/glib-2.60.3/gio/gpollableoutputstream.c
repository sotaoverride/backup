...
static gssize g_pollable_output_stream_default_write_nonblocking(
    ...
    GError** error);
static GPollableReturn g_pollable_output_stream_default_writev_nonblocking(
    ...
    g_pollable_output_stream_default_init(GPollableOutputStreamInterface* iface)
{
    ...
    iface->writev_nonblocking =
        g_pollable_output_stream_default_writev_nonblocking;
}
                                                ...
                                                GCancellable* cancellable)
{
    ...
    return G_POLLABLE_OUTPUT_STREAM_GET_INTERFACE(stream)->create_source(
        stream, cancellable);
}
    ...
    GError** error)
{
    ...
    return G_OUTPUT_STREAM_GET_CLASS(stream)->write_fn(
        G_OUTPUT_STREAM(stream), buffer, count, NULL, error);
}
    ...
    gsize n_vectors, gsize* bytes_written, GError** error)
{
    ...
    for (i = 0; i < n_vectors; i++)
    {
        ...
        res = iface->write_nonblocking(stream, vectors[i].buffer,
                                       vectors[i].size, &err);
        if (res == -1)
        {
            if (bytes_written)
                                                  ...
                                                  GError** error)
{
    ...
    res = G_POLLABLE_OUTPUT_STREAM_GET_INTERFACE(stream)->write_nonblocking(
    ...
    GError** error)
{
    ...
    g_return_val_if_fail(G_IS_POLLABLE_OUTPUT_STREAM(stream),
                         G_POLLABLE_RETURN_FAILED);
    g_return_val_if_fail(vectors != NULL || n_vectors == 0,
                         G_POLLABLE_RETURN_FAILED);
    ...
    g_return_val_if_fail(iface->writev_nonblocking != NULL,
                         G_POLLABLE_RETURN_FAILED);

    if (cancellable)
        g_cancellable_push_current(cancellable);

    res = iface->writev_nonblocking(stream, vectors, n_vectors, &_bytes_written,
                                    error);
