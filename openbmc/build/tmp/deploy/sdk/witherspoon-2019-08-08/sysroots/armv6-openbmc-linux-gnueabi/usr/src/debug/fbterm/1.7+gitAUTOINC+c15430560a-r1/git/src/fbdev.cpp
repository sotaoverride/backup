...
FbDev* FbDev::initFbDev()
{
    ...
    switch (vinfo.bits_per_pixel)
    {
        case 8:
            ...
            {
                fprintf(stderr,
                        "only support pseudo-color visual with 8bpp depth!\n");
                return 0;
            }
            break;
        ...
        case 32:
            ...
            {
                fprintf(stderr, "only support true-color/direct-color visual "
                                "with 15/16/32bpp depth!\n");
                return 0;
            }
            break;

        default:
            fprintf(stderr, "only support frame buffer device with 8/15/16/32 "
                            "color depth!\n");
            return 0;
    }
...
FbDev::FbDev()
{
    ...
    mVMemBase = (u8*)mmap(0, finfo.smem_len, PROT_READ | PROT_WRITE, MAP_SHARED,
                          fbdev_fd, 0);
