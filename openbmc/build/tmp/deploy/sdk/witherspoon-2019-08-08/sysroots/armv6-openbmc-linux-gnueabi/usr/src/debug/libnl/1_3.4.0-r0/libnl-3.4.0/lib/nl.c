 ...
 *       reset it to `unspecified` by calling `nl_socket_set_local_port(sk,
 * 0)`). This capability is indicated by
 *       `%NL_CAPABILITY_NL_CONNECT_RETRY_GENERATE_PORT_ON_ADDRINUSE`.
 *
 * @note nl_connect() creates and sets the file descriptor. You can setup the
 * file descriptor yourself by creating and binding it, and then calling
...
int nl_connect(struct nl_sock* sk, int protocol)
{
    ...
    if (_nl_socket_is_local_port_unspecified(sk))
    {
        ...
        while (1)
        {
            if (ntries++ > 5)
            {
                /* try only a few times. We hit this only if many ports are
                 * already in use but allocated *outside*
                 * libnl/generate_local_port(). */
    ...
    {
        err =
            bind(sk->s_fd, (struct sockaddr*)&sk->s_local, sizeof(sk->s_local));
        if (err != 0)
        {
            errsv = errno;
 ...
 * @lowlevel
 */
int nl_send_iovec(struct nl_sock* sk, struct nl_msg* msg, struct iovec* iov,
                  unsigned iovlen)
{
...
struct pickup_param
{
    int (*parser)(struct nl_cache_ops*, struct sockaddr_nl*, struct nlmsghdr*,
...
static int __pickup_answer_syserr(struct sockaddr_nl* nla,
...
/**
 * Pickup netlink answer, parse is and return object with preserving system
 * error
 * @arg sk              Netlink socket
 * @arg parser          Parser function to parse answer
 * @arg result          Result pointer to return parsed object
...
int nl_pickup_keep_syserr(struct nl_sock* sk,
                          int (*parser)(struct nl_cache_ops*,
                                        struct sockaddr_nl*, struct nlmsghdr*,
                                        struct nl_parser_param*),
                          struct nl_object** result, int* syserror)
{
    struct nl_cb* cb;
    int err;
