...
static FT_Error tt_face_load_bdf_props(TT_Face face, FT_Stream stream)
{
    TT_BDF bdf = &face->bdf;
    FT_ULong length;
    FT_Error error;
    ...
    if (error || length < 8 || FT_FRAME_EXTRACT(length, bdf->table))
    {
...

    {
        ...
        if (version != 0x0001 || strings < 8 ||
            (strings - 8) / 4 < num_strikes || strings + 1 > length)
        {
            goto BadTable;
        }
...
tt_face_find_bdf_prop(TT_Face face, const char* property_name,
