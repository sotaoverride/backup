...
/* Note that we use the new /run prefix here (instead of /var/run) since we
 * require them to be aliases and that way we become independent of /var being
 * mounted */
...
struct reply_callback
{
    ...
    usec_t timeout_usec; /* this is a relative timeout until we reach the
                            BUS_HELLO state, and an absolute one right after */
    uint64_t cookie;
    unsigned prioq_idx;
};
...
struct sd_bus_slot
{
    ...
    /* Slots can be "floating" or not. If they are not floating (the usual case)
     * then they reference the bus object they are associated with. This means
     * the bus object stays allocated at least as long as there is a slot around
     * associated with it. If it is floating, then the slot's lifecycle is bound
     * to the lifecycle of the bus: it will be disconnected from the bus when
     * the bus is destroyed, and it keeping the slot reffed hence won't mean the
     * bus stays reffed too. Internally this means the reference direction is
     * reversed: floating slots objects are referenced by the bus object, and
     * not vice versa. */
    bool floating : 1;
...
enum bus_state
{
    ...
    BUS_AUTHENTICATING, /* we are currently in the "SASL" authorization phase of
                           dbus */
...
struct sd_bus
{
    ...
    /* zero means use value specified by $SYSTEMD_BUS_TIMEOUT= environment
     * variable or built-in default */
    usec_t method_call_timeout;
};

/* For method calls we timeout at 25s, like in the D-Bus reference
 * implementation */
#define BUS_DEFAULT_TIMEOUT ((usec_t)(25 * USEC_PER_SEC))

/* For the authentication phase we grant 90s, to provide extra room during boot,
 * when RNGs and such are not filled up
 ...
 * limit. We enforce here one anyway, since truly unbounded strings are a
 * security problem. The limit we pick is relatively large however,
