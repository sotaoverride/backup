...
///
/// This file defines interfaces that are responsible for results reporting.
/// Interface is presented in a form of free standing function implemented in
/// namespace result_reporter
// ***************************************************************************
...
///
/// This is abstract interface for the report formatter used by results reporter
/// routines. You can define a custom formatter by implementing this interface
/// and setting the formatter using set_format function. This is usually done
...
class BOOST_TEST_DECL format
{
  public:
    ...
    virtual void test_unit_report_start(test_unit const&,
                                        std::ostream& ostr) = 0;
    virtual void test_unit_report_finish(test_unit const&,
                                         std::ostream& ostr) = 0;

    virtual void do_confirmation_report(test_unit const&,
                                        std::ostream& ostr) = 0;
};
...
/// - confirmation report (boost::unit_test::CONFIRMATION_REPORT). This report
/// level only produces short confirmation
///   message about test module pass/fail status
/// - short report (boost::unit_test::SHORT_REPORT). This report level produces
/// short summary report for failed/passed
///   assertions and test units.
/// - detailed report (boost::unit_test::DETAILED_REPORT). This report level
/// produces detailed report per test unit for
///   passed/failed assertions and uncaught exceptions
/// - no report (boost::unit_test::NO_REPORT). This report level produces no
/// results report. This is used for test modules
...
/// By default std::cerr is used. Use this function to set a different stream.
/// The framework refers to the stream by reference, so you need to make sure
/// the stream object lifetime exceeds the testing main scope.
...
/// The framework takes ownership of the pointer passed as an argument. So this
/// should be a pointer to a heap allocated object
/// @param[in] f pointer to heap allocated instance of custom report formatter
/// class
...
BOOST_TEST_DECL void make_report(report_level l = INV_REPORT_LEVEL,
