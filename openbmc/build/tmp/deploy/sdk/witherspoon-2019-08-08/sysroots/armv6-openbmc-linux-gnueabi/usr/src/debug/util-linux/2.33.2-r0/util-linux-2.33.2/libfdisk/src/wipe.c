...
static struct fdisk_wipe* fdisk_get_wipe_area(struct fdisk_context* cxt,
...
void fdisk_free_wipe_areas(struct fdisk_context* cxt)
{
    while (!list_empty(&cxt->wipes))
    {
        struct fdisk_wipe* wp =
            list_entry(cxt->wipes.next, struct fdisk_wipe, wipes);
                        ...
                        uint64_t size, int enable)
{
    ...
    DBG(WIPE, ul_debugobj(wp, "enable [start=%ju, size=%ju]", (uintmax_t)start,
                          (uintmax_t)size));
...
int fdisk_do_wipe(struct fdisk_context* cxt)
{
    ...
    list_for_each(p, &cxt->wipes)
    {
        ...
        DBG(WIPE,
            ul_debugobj(wp, "initialize libblkid prober [start=%ju, size=%ju]",
                        (uintmax_t)start, (uintmax_t)size));
        ...
        {
            DBG(WIPE,
                ul_debugobj(wp, "blkid_probe_set_device() failed [rc=%d]", rc));
            return rc;
        }
