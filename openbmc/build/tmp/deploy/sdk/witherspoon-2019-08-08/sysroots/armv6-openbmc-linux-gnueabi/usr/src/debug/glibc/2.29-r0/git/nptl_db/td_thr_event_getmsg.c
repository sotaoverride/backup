...
td_err_e td_thr_event_getmsg(const td_thrhandle_t* th, td_event_msg_t* msg)
{
    ...
    err = DB_GET_FIELD_LOCAL(eventnum, th->th_ta_p, copy, td_eventbuf_t,
                             eventnum, 0);
    if (err != TD_OK)
        return err;
    ...
    err = DB_GET_FIELD_LOCAL(eventdata, th->th_ta_p, copy, td_eventbuf_t,
                             eventdata, 0);
    if (err != TD_OK)
        return err;
    ...
    {
        psaddr_t next;
        err = DB_GET_FIELD(next, th->th_ta_p, th->th_unique, pthread, nextevent,
                           0);
        if (err != TD_OK)
            return err;
        ...
        if (thp == th->th_unique)
        {
            ...
            err = _td_store_value(th->th_ta_p,
                                  th->th_ta_p->ta_var___nptl_last_event, -1, 0,
                                  prevp, next);
            if (err != TD_OK)
                return err;
        ...
        err = DB_GET_FIELD_ADDRESS(prevp, th->th_ta_p, thp, pthread, nextevent,
                                   0);
        assert(err == TD_OK); /* We used this field before.  */
        thp = next;
    }
