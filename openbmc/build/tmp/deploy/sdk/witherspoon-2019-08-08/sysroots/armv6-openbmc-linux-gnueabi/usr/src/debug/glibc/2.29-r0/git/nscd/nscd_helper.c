...
#endif
        ssize_t wres = TEMP_FAILURE_RETRY(
            __send(sock, reqdata, real_sizeof_reqdata, MSG_NOSIGNAL));
...
struct mapped_database* __nscd_get_mapping(request_type type, const char* key,
...
#define MSG_CMSG_CLOEXEC 0
    ...
    if (__glibc_likely(mapping != MAP_FAILED))
    {
        ...
        size_t size =
            (sizeof(*head) + roundup(head->module * sizeof(ref_t), ALIGN) +
             head->data_size);
...
/* Don't return const struct datahead *, as eventhough the record
   ...
   garbage collection.  */
struct datahead* __nscd_cache_search(request_type type, const char* key,
                                     size_t keylen,
    ...
    while (work != ENDREF && work + MINIMUM_HASHENTRY_SIZE <= datasize)
    {
       ...
       application.  */
             ...
             datasize))
        {
            ...
            if (dh->usable && here_packet + dh->allocsize <= datasize &&
                       ...
                       void* response, size_t responselen)
{
    ...
    if (sock >= 0)
    {
        ...
        {
            ssize_t nbytes =
                TEMP_FAILURE_RETRY(__read(sock, response, responselen));
