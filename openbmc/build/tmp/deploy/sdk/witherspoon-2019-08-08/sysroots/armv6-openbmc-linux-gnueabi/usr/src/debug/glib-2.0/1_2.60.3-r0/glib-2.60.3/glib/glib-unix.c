...
{
    g_return_val_if_fail(signum == SIGHUP || signum == SIGINT ||
                             signum == SIGTERM || signum == SIGUSR1 ||
                             signum == SIGUSR2 || signum == SIGWINCH,
                         NULL);
 ...
 * Since: 2.30
 */
guint g_unix_signal_add_full(int priority, int signum, GSourceFunc handler,
...
{
    return g_unix_signal_add_full(G_PRIORITY_DEFAULT, signum, handler,
                                  user_data, NULL);
}
...
static gboolean g_unix_fd_source_dispatch(GSource* source, GSourceFunc callback,
                                          gpointer user_data)
{
    ...
    return (*func)(fd_source->fd,
                   g_source_query_unix_fd(source, fd_source->tag), user_data);
}

GSourceFuncs g_unix_fd_source_funcs = {NULL, NULL, g_unix_fd_source_dispatch,
                                       NULL};
 ...
 * Since: 2.36
 **/
guint g_unix_fd_add_full(gint priority, gint fd, GIOCondition condition,
 ...
 * Since: 2.36
 **/
guint g_unix_fd_add(gint fd, GIOCondition condition, GUnixFDSourceFunc function,
                    gpointer user_data)
{
    return g_unix_fd_add_full(G_PRIORITY_DEFAULT, fd, condition, function,
                              user_data, NULL);
}
