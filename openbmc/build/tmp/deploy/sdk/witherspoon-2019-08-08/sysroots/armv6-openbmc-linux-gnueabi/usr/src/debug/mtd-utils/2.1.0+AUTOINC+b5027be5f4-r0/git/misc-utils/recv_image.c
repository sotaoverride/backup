...
int main(int argc, char** argv)
{
    ...
    unsigned long fec_time = 0, flash_time = 0, crc_time = 0, rflash_time = 0,
                  erase_time = 0, net_time = 0;
    ...
    for (runp = ai; runp; runp = runp->ai_next)
    {
        ...
        if (runp->ai_family == AF_INET &&
            IN_MULTICAST(
                ntohl(((struct sockaddr_in*)runp->ai_addr)->sin_addr.s_addr)))
        {
            struct ip_mreq rq;
            rq.imr_multiaddr = ((struct sockaddr_in*)runp->ai_addr)->sin_addr;
            rq.imr_interface.s_addr = INADDR_ANY;
            if (setsockopt(sock, IPPROTO_IP, IP_ADD_MEMBERSHIP, &rq,
                           sizeof(rq)))
            {
                ...
                continue;
                 ...
                 ((struct sockaddr_in6*)runp->ai_addr)->sin6_addr.s6_addr[0] ==
                     0xff)
        {
            struct ipv6_mreq rq;
            rq.ipv6mr_multiaddr =
                ((struct sockaddr_in6*)runp->ai_addr)->sin6_addr;
            rq.ipv6mr_interface = 0;
            if (setsockopt(sock, IPPROTO_IPV6, IPV6_ADD_MEMBERSHIP, &rq,
                           sizeof(rq)))
            {
    ...
    while (1)
    {
        ...
        if (!eraseblocks)
        {
            ...
            {
                eraseblocks[i].pkt_indices =
                    malloc(sizeof(int) * pkts_per_block);
        ...
        {
            if (eraseblocks[block_nr].pkt_indices[i] ==
                ntohs(thispkt.hdr.pkt_nr))
            {
                //				printf("Discarding duplicate packet at %08x pkt
                //%d\n", 				       block_nr * meminfo.erasesize,
                //eraseblocks[block_nr].pkt_indices[i]);
                duplicates++;
                break;
            }
        }
    ...
    pkt_again:
        ...
        {
            uint32_t pkts_sent =
                ntohl(thispkt.hdr.pkt_sequence) - start_seq + 1;
            long time_msec;
            ...
            printf("\rReceived %d/%d (%d%%) in %lds @%ldKiB/s, %d lost (%d%%), "
                   "%d dup/xs    ",
                   ...
                   (pkts_sent - total_pkts - duplicates - ignored_pkts) * 100 /
                       pkts_sent,
        ...
        else
        {
            ...
            if (wrotelen < WBUF_SIZE ||
                (block_nr == 5 && eraseblocks[block_nr].nr_pkts == 5 && !faked))
            {
                faked = 1;
                if (wrotelen < 0)
                    perror("\npacket write");
                ...
                if (!file_mode)
                {
                    ...
                    printf(
                        "Will erase at %08x len %08x (bad write was at %08x)\n",
            ...
            memcpy(eraseblocks[block_nr].wbuf, &thispkt.data[fits],
                   PKT_SIZE - fits);
    ...
    for (block_nr = 0; block_nr < nr_blocks; block_nr++)
    {
        ...
        rwlen = pread(flfd, eb_buf, meminfo.erasesize,
                      eraseblocks[block_nr].flash_offset);
        ...
        if (rwlen < 0)
        {
            ...
            /* Argh. Perhaps we could go back and try again, but if the flash is
               going to fail to read back what we write to it, and the whole
               point in this program is to write to it, what's the point? */
            fprintf(
                stderr,
                "Packets we wrote to flash seem to be unreadable. Aborting\n");
            exit(1);
        }
        ...
        gettimeofday(&start, NULL);
        if (fec_decode(fec, src_pkts, eraseblocks[block_nr].pkt_indices,
                       PKT_SIZE))
        {
        ...
        /* Paranoia */
        gettimeofday(&start, NULL);
        if (mtd_crc32(-1, decode_buf, meminfo.erasesize) !=
        ...
        if (!file_mode)
        {
            ...
            {
                perror("MEMERASE");
                /* This block has dirty data on it. If the erase failed, we're
                 * screwed */
                fprintf(
                    stderr,
                    "Erase to clean FEC data from flash failed. Aborting\n");
                exit(1);
            }
            gettimeofday(&now, NULL);
            ...
            printf("\r");
    write_again:
        rwlen = pwrite(flfd, decode_buf, meminfo.erasesize,
                       eraseblocks[block_nr].flash_offset);
