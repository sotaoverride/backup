...
inline std::ostream& commandline_pretty_print(std::ostream& ostr,
...
{
    parameter_cla_id(cstring prefix, cstring tag, cstring value_separator,
                     bool negatable) :
        ...
        m_negatable(negatable)
    {
        ...
        BOOST_TEST_I_ASSRT(algorithm::all_of(m_prefix.begin(), m_prefix.end(),
                                             valid_prefix_char),
                           invalid_cla_id()
                               << "Parameter " << m_tag
        ...
        BOOST_TEST_I_ASSRT(
            algorithm::all_of(m_tag.begin(), m_tag.end(), valid_name_char),
            invalid_cla_id()
                << "Parameter " << m_tag << " has invalid characters in name.");

        BOOST_TEST_I_ASSRT(
            algorithm::all_of(m_value_separator.begin(),
                              m_value_separator.end(), valid_separator_char),
...
class basic_param
{
  ...
  protected:
    ...
    basic_param(cstring name, bool is_optional, bool is_repeatable,
                Modifiers const& m) :
        ...
        p_optional(is_optional), p_repeatable(is_repeatable),
  ...
  public:
    ...
    /// Access methods
    param_cla_ids const& cla_ids() const
    {
        return m_cla_ids;
    }
    ...
    virtual void produce_argument(cstring token, bool negative_form,
                                  arguments_store& store) const = 0;
  ...
  protected:
    void add_cla_id_impl(cstring prefix, cstring tag, cstring value_separator,
                         bool negatable, bool validate_value_separator)
    {
        BOOST_TEST_I_ASSRT(!tag.is_empty(),
                           invalid_cla_id()
                               << "Parameter can't have an empty name.");
        ...
        // We trim value separator from all the spaces, so token end will
        // indicate separator
        value_separator.trim();
        BOOST_TEST_I_ASSRT(
            !validate_value_separator || !value_separator.is_empty() ||
                !p_has_optional_value,
            invalid_cla_id() << "Parameter " << tag
                             << " with optional value attribute can't use "
                                "space as value separator.");

        m_cla_ids.push_back(
            parameter_cla_id(prefix, tag, value_separator, negatable));
    }

  private:
    /// interface for usage/help customization
    virtual void cla_name_help(std::ostream& ostr, cstring cla_tag,
                               cstring /*negation_prefix_*/,
...
class parameter : public basic_param
{
...
#endif
        :
        basic_param(name, a != runtime::REQUIRED_PARAM,
                    a == runtime::REPEATABLE_PARAM, m),
        m_arg_factory(m)
    {
        BOOST_TEST_I_ASSRT(
            !m.has(default_value) || a == runtime::OPTIONAL_PARAM,
            invalid_param_spec()
                << "Parameter " << name
                << " is not optional and can't have default_value.");

        BOOST_TEST_I_ASSRT(
            !m.has(optional_value) || !this->p_repeatable,
            invalid_param_spec()
                << "Parameter " << name
  ...
  private:
    ...
    virtual void produce_argument(cstring token, bool,
                                  arguments_store& store) const
    {
    ...
    // Data members
    typedef argument_factory<ValueType, is_enum, a == runtime::REPEATABLE_PARAM>
        factory_t;
...
#endif
        :
        basic_param(name, true, false,
                    nfp::opt_append(nfp::opt_append(m, optional_value = true),
                                    default_value = false)),
        m_arg_factory(nfp::opt_append(nfp::opt_append(m, optional_value = true),
                                      default_value = false))
    {
    }
                                  ...
                                  arguments_store& store) const
    {
        ...
        else
        {
            BOOST_TEST_I_ASSRT(
                !negative_form,
                format_error(p_name)
                    << "Can't set value to negative form of the argument.");
...
class enum_parameter : public parameter<EnumType, a, true>
{
  ...
  public:
        ...
        base(name, m)
    {
        ...
        typename std::vector<std::pair<cstring, EnumType>>::const_iterator it =
            values.begin();
#endif
        while (it != values.end())
        {
...
class parameters_store
{
  ...
  public:
    ...
    void add(basic_param const& in)
    {
        ...
        BOOST_TEST_I_ASSRT(
            m_parameters.insert(std::make_pair(cstring(p->p_name), p)).second,
