...
void listenForIncomingConnections(rfbClient* client)
{
    ...
    listenSocket =
        ListenAtTcpPortAndAddress(client->listenPort, client->listenAddress);
...
#ifdef LIBVNCSERVER_IPv6 /* only try that if we're IPv6-capable, otherwise we  \
                            may try to bind to the same port which would make  \
                            all that listening fail */
    /* only do IPv6 listen of listen6Port is set */
    if (client->listen6Port > 0)
    {
        listen6Socket = ListenAtTcpPortAndAddress(client->listen6Port,
                                                  client->listen6Address);
    ...
    while (TRUE)
    {
        ...
        r = select(rfbMax(listenSocket, listen6Socket) + 1, &fds, NULL, NULL,
                   NULL);
...
int listenForIncomingConnectionsNoFork(rfbClient* client, int timeout)
{
    ...
    {
        client->listenSock = ListenAtTcpPortAndAddress(client->listenPort,
        ...
        rfbClientLog("%s -listennofork: Listening on port %d\n",
                     client->programName, client->listenPort);
        rfbClientLog(
            "%s -listennofork: Command line errors are not reported until "
            "a connection comes in.\n",
            client->programName);
    }
    ...
    {
        client->listen6Sock = ListenAtTcpPortAndAddress(client->listen6Port,
                                                        client->listen6Address);
    ...
    if (timeout < 0)
        r = select(rfbMax(client->listenSock, client->listen6Sock) + 1, &fds,
                   NULL, NULL, NULL);
    else
        r = select(rfbMax(client->listenSock, client->listen6Sock) + 1, &fds,
                   NULL, NULL, &to);
