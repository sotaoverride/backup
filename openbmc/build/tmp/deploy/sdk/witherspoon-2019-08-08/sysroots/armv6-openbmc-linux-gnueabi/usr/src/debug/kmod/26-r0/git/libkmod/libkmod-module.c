...
static int kmod_module_new(struct kmod_ctx* ctx, const char* key,
                           const char* name, size_t namelen, const char* alias,
                           size_t aliaslen, struct kmod_module** mod)
{
...
static const struct kmod_list*
    module_get_dependencies_noref(const struct kmod_module* mod)
{
    if (!mod->init.dep)
    {
        /* lazy init */
 ...
 */
KMOD_EXPORT struct kmod_module*
 ...
 * @mod: kmod module
 * @flags: flags to pass to Linux kernel when removing the module. The only
 * valid flag is KMOD_REMOVE_FORCE: force remove module regardless if it's still
 * in use by a kernel subsystem or other process; KMOD_REMOVE_NOWAIT is always
 * enforced, causing us to pass O_NONBLOCK to delete_module(2).
 *
...
static struct kmod_list* lookup_softdep(struct kmod_ctx* ctx,
 ...
 * states are KMOD_MODULE_BUILTIN: module is builtin; KMOD_MODULE_LIVE: module
 * is live in kernel; KMOD_MODULE_COMING: module is being loaded;
...
KMOD_EXPORT int kmod_module_get_initstate(const struct kmod_module* mod)
{
    ...
    pathlen =
        snprintf(path, sizeof(path), "/sys/module/%s/initstate", mod->name);
...
static struct kmod_module_info* kmod_module_info_new(const char* key,
                                                     size_t keylen,
                                                     const char* value,
                                                     size_t valuelen)
{
...
static struct kmod_list* kmod_module_info_append(struct kmod_list** list,
                                     ...
                                     struct kmod_list** list)
{
    ...
    if (kmod_module_signature_info(mod->file, &sig_info))
    {
        ...
        n = kmod_module_info_append(list, "sig_hashalgo",
                                    strlen("sig_hashalgo"), sig_info.hash_algo,
 ...
 *
 * Returns: the crc of this kmod module version if available, otherwise default
 * to 0.
 */
...
static struct kmod_module_symbol* kmod_module_symbols_new(uint64_t crc,
                                                          const char* symbol)
{
 ...
 *
 * Returns: the crc of this kmod module symbol if available, otherwise default
 * to 0.
 */
...
static struct kmod_module_dependency_symbol*
 ...
 */
KMOD_EXPORT const char*
    kmod_module_dependency_symbol_get_symbol(const struct kmod_list* entry)
{
    struct kmod_module_dependency_symbol* dependency_symbol;
 ...
 *
 * Returns: the crc of this kmod module dependency_symbol if available,
 * otherwise default to 0.
 */
KMOD_EXPORT uint64_t
 ...
 */
KMOD_EXPORT int
    kmod_module_dependency_symbol_get_bind(const struct kmod_list* entry)
{
    struct kmod_module_dependency_symbol* dependency_symbol;
 ...
 */
KMOD_EXPORT void
    kmod_module_dependency_symbols_free_list(struct kmod_list* list)
{
    while (list)
    {
