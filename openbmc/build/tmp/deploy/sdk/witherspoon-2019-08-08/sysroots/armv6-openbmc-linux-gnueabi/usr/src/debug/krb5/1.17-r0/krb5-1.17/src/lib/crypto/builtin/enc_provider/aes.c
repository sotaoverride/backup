...
static krb5_boolean aesni_supported_by_cpu()
{
...
static inline krb5_boolean aesni_supported(krb5_key key)
{
    return CACHE(key)->aesni;
}

static void aesni_expand_enc_key(krb5_key key)
{
...
static void aesni_expand_dec_key(krb5_key key)
{
    struct aes_key_info_cache* cache = CACHE(key);
...
static inline void aesni_enc(krb5_key key, unsigned char* data, size_t nblocks,
                             unsigned char* iv)
{
    struct aes_key_info_cache* cache = CACHE(key);
    struct aes_data d;
...
static inline void aesni_dec(krb5_key key, unsigned char* data, size_t nblocks,
                             unsigned char* iv)
{
    struct aes_key_info_cache* cache = CACHE(key);
    struct aes_data d;
...
/* out = out ^ in */
static inline void xorblock(const unsigned char* in, unsigned char* out)
{
    size_t q;
...
static inline krb5_error_code init_key_cache(krb5_key key)
{
    if (key->cache != NULL)
        return 0;
...
static inline void expand_enc_key(krb5_key key)
{
...
static inline void expand_dec_key(krb5_key key)
{
...
static inline void cbc_enc(krb5_key key, unsigned char* data, size_t nblocks,
                           unsigned char* iv)
{
    if (aesni_supported(key))
    {
...
static inline void cbc_dec(krb5_key key, unsigned char* data, size_t nblocks,
                           unsigned char* iv)
{
...
static krb5_error_code aes_init_state(const krb5_keyblock* key,
...
static void aes_key_cleanup(krb5_key key)
{
    zapfree(key->cache, sizeof(struct aes_key_info_cache));
}

const struct krb5_enc_provider krb5int_enc_aes128 = {16,
                                                     16,
                                                     16,
                                                     krb5int_aes_encrypt,
                                                     krb5int_aes_decrypt,
...
const struct krb5_enc_provider krb5int_enc_aes256 = {16,
                                                     32,
                                                     32,
                                                     krb5int_aes_encrypt,
                                                     krb5int_aes_decrypt,
