...
int FcStat(const FcChar8* file, struct stat* statb)
{
     ...
     * is on disk.
     */
    rc = GetFullPathName((LPCSTR)file, sizeof(full_path_name), full_path_name,
                         &basename);
    if (rc == 0 || rc > sizeof(full_path_name))
        return -1;

    rc =
        GetLongPathName(full_path_name, full_path_name, sizeof(full_path_name));
    ...
    statb->st_atime =
        (*(INT64*)&wfad.ftLastAccessTime) / 10000000 - EPOCH_OFFSET;
    statb->st_mtime =
        (*(INT64*)&wfad.ftLastWriteTime) / 10000000 - EPOCH_OFFSET;
...
static void Adler32Init(struct Adler32* ctx)
{
    ctx->a = 1;
    ctx->b = 0;
}

static void Adler32Update(struct Adler32* ctx, const char* data, int data_len)
{
    while (data_len--)
    {
...
static int Adler32Finish(struct Adler32* ctx)
{
...
static FcBool FcDirChecksumScandirFilter(const struct dirent* entry)
{
    return entry->d_type != DT_DIR;
}
#endif

static int FcDirChecksumScandirSorter(const struct dirent** lhs,
                                      const struct dirent** rhs)
{
    return strcmp((*lhs)->d_name, (*rhs)->d_name);
}

static void free_dirent(struct dirent** p)
{
    struct dirent** x;
...
int FcScandir(const char* dirp, struct dirent*** namelist,
              ...
              int (*compar)(const struct dirent**, const struct dirent**))
{
    ...
    *dlist = NULL;
    while ((dent = readdir(d)))
    {
        ...
        {
            size_t dentlen =
                FcPtrToOffset(dent, dent->d_name) + strlen(dent->d_name) + 1;
            ...
            {
                lsize += 128;
                dlp = (struct dirent**)realloc(dlist,
                                               sizeof(struct dirent*) * lsize);
                if (!dlp)
                {
                    free(p);
                    free_dirent(dlist);
    ...
    qsort(dlist, n, sizeof(struct dirent*),
...
static int FcDirChecksum(const FcChar8* dir, time_t* checksum)
{
    struct Adler32 ctx;
    struct dirent** files;
    int n;
...
static int FcFStatFs(int fd, FcStatFS* statb)
{
    const char* p = NULL;
    int ret = -1;
    FcBool flag = FcFalse;

#if defined(HAVE_FSTATVFS) && (defined(HAVE_STRUCT_STATVFS_F_BASETYPE) ||      \
    ...
    (defined(HAVE_STRUCT_STATFS_F_FLAGS) ||                                    \
     defined(HAVE_STRUCT_STATFS_F_FSTYPENAME) || defined(__linux__))
    struct statfs buf;
...
#error "BUG: No way to figure out with fstatfs()"
    ...
    if (p)
    {
        if (!flag && strcmp(p, "nfs") == 0)
            statb->is_remote_fs = FcTrue;
        if (strcmp(p, "msdosfs") == 0 || strcmp(p, "pcfs") == 0)
            statb->is_mtime_broken = FcTrue;
    }
...
FcBool FcIsFsMtimeBroken(const FcChar8* dir)
{
