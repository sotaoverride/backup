...
extern void
    _gcry_serpent_neon_ocb_enc(serpent_context_t* ctx, unsigned char* out,
                               const unsigned char* in, unsigned char* offset,
                               unsigned char* checksum, const void* Ls[8]);

extern void
    _gcry_serpent_neon_ocb_dec(serpent_context_t* ctx, unsigned char* out,
                               const unsigned char* in, unsigned char* offset,
                               unsigned char* checksum, const void* Ls[8]);
...
#define SBOX(which, array0, array1)                                            \
    SBOX##which(array0[0], array0[1], array0[2], array0[3], array1[0],         \
...
static void serpent_key_prepare(const byte* key, unsigned int key_length,
...
static void serpent_subkeys_generate(serpent_key_t key,
                                     serpent_subkeys_t subkeys)
{
    u32 w[8]; /* The `prekey'.  */
    u32 ws[4];
    u32 wt[4];
...
#define EXPAND_KEY4(wo, r)                                                     \
    wo[0] = w[(r + 0) % 8] =                                                   \
        rol(w[(r + 0) % 8] ^ w[(r + 3) % 8] ^ w[(r + 5) % 8] ^                 \
                w[(r + 7) % 8] ^ PHI ^ (r + 0),                                \
            11);                                                               \
    wo[1] = w[(r + 1) % 8] =                                                   \
        rol(w[(r + 1) % 8] ^ w[(r + 4) % 8] ^ w[(r + 6) % 8] ^                 \
                w[(r + 0) % 8] ^ PHI ^ (r + 1),                                \
            11);                                                               \
    wo[2] = w[(r + 2) % 8] =                                                   \
        rol(w[(r + 2) % 8] ^ w[(r + 5) % 8] ^ w[(r + 7) % 8] ^                 \
                w[(r + 1) % 8] ^ PHI ^ (r + 2),                                \
            11);                                                               \
    wo[3] = w[(r + 3) % 8] =                                                   \
        rol(w[(r + 3) % 8] ^ w[(r + 6) % 8] ^ w[(r + 0) % 8] ^                 \
                w[(r + 2) % 8] ^ PHI ^ (r + 3),                                \
            11);
...
#define EXPAND_KEY(r)                                                          \
    ...
    SBOX(2, wt, subkeys[9]);
    EXPAND_KEY(40);
    SBOX(1, ws, subkeys[10]);
    SBOX(0, wt, subkeys[11]);
    EXPAND_KEY(48);
    SBOX(7, ws, subkeys[12]);
    SBOX(6, wt, subkeys[13]);
    EXPAND_KEY(56);
    SBOX(5, ws, subkeys[14]);
    SBOX(4, wt, subkeys[15]);
    EXPAND_KEY(64);
    SBOX(3, ws, subkeys[16]);
    SBOX(2, wt, subkeys[17]);
    EXPAND_KEY(72);
    SBOX(1, ws, subkeys[18]);
    SBOX(0, wt, subkeys[19]);
    EXPAND_KEY(80);
    SBOX(7, ws, subkeys[20]);
    SBOX(6, wt, subkeys[21]);
    EXPAND_KEY(88);
    SBOX(5, ws, subkeys[22]);
    SBOX(4, wt, subkeys[23]);
    EXPAND_KEY(96);
    SBOX(3, ws, subkeys[24]);
    SBOX(2, wt, subkeys[25]);
    EXPAND_KEY(104);
    SBOX(1, ws, subkeys[26]);
    SBOX(0, wt, subkeys[27]);
    EXPAND_KEY(112);
    SBOX(7, ws, subkeys[28]);
    SBOX(6, wt, subkeys[29]);
    EXPAND_KEY(120);
    SBOX(5, ws, subkeys[30]);
    SBOX(4, wt, subkeys[31]);
    EXPAND_KEY4(ws, 128);
    SBOX(3, ws, subkeys[32]);
...
static void serpent_setkey_internal(serpent_context_t* context, const byte* key,
...
static gcry_err_code_t serpent_setkey(void* ctx, const byte* key,
                                      unsigned int key_length)
{
...
static void serpent_encrypt_internal(serpent_context_t* context,
...
static void serpent_decrypt_internal(serpent_context_t* context,
                                     const byte* input, byte* output)
{
    serpent_block_t b, b_next;
    int round = ROUNDS;
...
static unsigned int serpent_encrypt(void* ctx, byte* buffer_out,
...
static unsigned int serpent_decrypt(void* ctx, byte* buffer_out,
                                    const byte* buffer_in)
{
    serpent_context_t* context = ctx;
...
void _gcry_serpent_ctr_enc(void* context, unsigned char* ctr, void* outbuf_arg,
...
void _gcry_serpent_cbc_dec(void* context, unsigned char* iv, void* outbuf_arg,
                           const void* inbuf_arg, size_t nblocks)
{
    serpent_context_t* ctx = context;
    unsigned char* outbuf = outbuf_arg;
...
/* Bulk decryption of complete blocks in CFB mode.  This function is only
   intended for the bulk encryption feature of cipher.c. */
void _gcry_serpent_cfb_dec(void* context, unsigned char* iv, void* outbuf_arg,
                           const void* inbuf_arg, size_t nblocks)
{
    serpent_context_t* ctx = context;
    unsigned char* outbuf = outbuf_arg;
...
static const char* selftest_ctr_128(void)
{
    ...
    return _gcry_selftest_helper_ctr("SERPENT", &serpent_setkey,
                                     &serpent_encrypt, &_gcry_serpent_ctr_enc,
...
static const char* selftest_cbc_128(void)
{
    ...
    return _gcry_selftest_helper_cbc("SERPENT", &serpent_setkey,
                                     &serpent_encrypt, &_gcry_serpent_cbc_dec,
                                     nblocks, blocksize, context_size);
}
...
static const char* selftest_cfb_128(void)
{
    ...
    return _gcry_selftest_helper_cfb("SERPENT", &serpent_setkey,
                                     &serpent_encrypt, &_gcry_serpent_cfb_dec,
                                     nblocks, blocksize, context_size);
}
...
static const char* serpent128_aliases[] = {"SERPENT", "SERPENT-128", NULL};
static const char* serpent192_aliases[] = {"SERPENT-192", NULL};
static const char* serpent256_aliases[] = {"SERPENT-256", NULL};

gcry_cipher_spec_t _gcry_cipher_spec_serpent128 = {GCRY_CIPHER_SERPENT128,
                                                   {0, 0},
                                                   "SERPENT128",
                                                   serpent128_aliases,
                                                   serpent128_oids,
...
gcry_cipher_spec_t _gcry_cipher_spec_serpent192 = {GCRY_CIPHER_SERPENT192,
                                                   {0, 0},
                                                   "SERPENT192",
                                                   serpent192_aliases,
                                                   serpent192_oids,
...
gcry_cipher_spec_t _gcry_cipher_spec_serpent256 = {GCRY_CIPHER_SERPENT256,
                                                   {0, 0},
                                                   "SERPENT256",
                                                   serpent256_aliases,
                                                   serpent256_oids,
