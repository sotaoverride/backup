...
size_t char16_strlen(const char16_t* s); /* returns the number of 16bit words in
                                            the string (not bytes!) */
...
static inline char32_t utf16_surrogate_pair_to_unichar(char16_t lead,
                                                       char16_t trail)
{
    return ((((char32_t)lead - 0xd800U) << 10) + ((char32_t)trail - 0xdc00U) +
            0x10000U);
}
