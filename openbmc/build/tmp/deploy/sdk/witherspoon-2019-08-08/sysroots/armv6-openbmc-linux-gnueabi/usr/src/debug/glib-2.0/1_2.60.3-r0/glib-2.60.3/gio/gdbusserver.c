...
/**
 * GDBusServerClass:
 * @new_connection: Signal class handler for the #GDBusServer::new-connection
 * signal.
 *
...
static void g_dbus_server_finalize(GObject* object)
{
    ...
    if (server->run_signal_handler_id > 0)
        g_signal_handler_disconnect(server->listener,
...
static void g_dbus_server_get_property(GObject* object, guint prop_id,
...
static void g_dbus_server_set_property(GObject* object, guint prop_id,
...
static void g_dbus_server_class_init(GDBusServerClass* klass)
{
    ...
    g_object_class_install_property(
        ...
        g_param_spec_flags("flags", P_("Flags"), P_("Flags for the server"),
                           G_TYPE_DBUS_SERVER_FLAGS, G_DBUS_SERVER_FLAGS_NONE,
    ...
    g_object_class_install_property(
        ...
        g_param_spec_string("guid", P_("GUID"), P_("The guid of the server"),
                            NULL,
                            G_PARAM_READABLE | G_PARAM_WRITABLE |
                                G_PARAM_CONSTRUCT_ONLY | G_PARAM_STATIC_NAME |
                                G_PARAM_STATIC_BLURB | G_PARAM_STATIC_NICK));
    ...
    g_object_class_install_property(
        ...
        g_param_spec_string("address", P_("Address"),
                            P_("The address to listen on"), NULL,
                            G_PARAM_READABLE | G_PARAM_WRITABLE |
                                G_PARAM_CONSTRUCT_ONLY | G_PARAM_STATIC_NAME |
                                G_PARAM_STATIC_BLURB | G_PARAM_STATIC_NICK));
     ...
     */
    g_object_class_install_property(
        gobject_class, PROP_CLIENT_ADDRESS,
        g_param_spec_string("client-address", P_("Client Address"),
                            P_("The address clients can use"), NULL,
    ...
    g_object_class_install_property(
        ...
        g_param_spec_boolean("active", P_("Active"),
     ...
     * Since: 2.26
     */
    _signals[NEW_CONNECTION_SIGNAL] = g_signal_new(
        I_("new-connection"), G_TYPE_DBUS_SERVER, G_SIGNAL_RUN_LAST,
        G_STRUCT_OFFSET(GDBusServerClass, new_connection),
        g_signal_accumulator_true_handled, NULL, /* accu_data */
        NULL, G_TYPE_BOOLEAN, 1, G_TYPE_DBUS_CONNECTION);
}
...
static gboolean on_run(GSocketService* service,
...
GDBusServer* g_dbus_server_new_sync(const gchar* address,
                                    GDBusServerFlags flags, const gchar* guid,
                                    ...
                                    GCancellable* cancellable, GError** error)
{
    ...
    server = g_initable_new(G_TYPE_DBUS_SERVER, cancellable, error, "address",
                            address, "flags", flags, "guid", guid,
...
/* note that address_entry has already been validated => exactly one of path,
 * tmpdir or abstract keys are set */
static gboolean try_unix(GDBusServer* server, const gchar* address_entry,
    ...
    retry:
        ...
        if (g_unix_socket_address_abstract_names_supported())
            address = g_unix_socket_address_new_with_type(
                s->str, -1, G_UNIX_SOCKET_ADDRESS_ABSTRACT);
        else
        ...
        if (!g_socket_listener_add_address(
                server->listener, address, G_SOCKET_TYPE_STREAM,
                G_SOCKET_PROTOCOL_DEFAULT, NULL, /* source_object */
                NULL,                            /* effective_address */
                &local_error))
        {
            if (local_error->domain == G_IO_ERROR &&
                local_error->code == G_IO_ERROR_ADDRESS_IN_USE)
            {
...
out:
    ...
    if (address != NULL)
    {
        ...
        if (ret)
        {
            ...
            switch (g_unix_socket_address_get_address_type(
                G_UNIX_SOCKET_ADDRESS(address)))
            {
                ...
                case G_UNIX_SOCKET_ADDRESS_PATH:
                    server->client_address = g_strdup_printf(
                        "unix:path=%s", g_unix_socket_address_get_path(
                                            G_UNIX_SOCKET_ADDRESS(address)));
                    break;
 ...
 *  both host and port (guaranteed to be a number in [0, 65535]) are set (family
 * is optional)
 */
static gboolean try_tcp(GDBusServer* server, const gchar* address_entry,
                        GHashTable* key_value_pairs, gboolean do_nonce,
                        GError** error)
{
    gboolean ret;
    const gchar* host;
    ...
    for (l = resolved_addresses; l != NULL; l = l->next)
    {
        ...
        if (!g_socket_listener_add_address(
                server->listener, socket_address, G_SOCKET_TYPE_STREAM,
                G_SOCKET_PROTOCOL_TCP, NULL, /* GObject *source_object */
    ...
    if (do_nonce)
    {
        ...
        fd = g_file_open_tmp("gdbus-nonce-file-XXXXXX", &server->nonce_file,
                             error);
        if (fd == -1)
        {
    ...
    again:
        ...
        while (bytes_remaining > 0)
        {
            ...
            if (ret == -1)
            {
                ...
                g_set_error(error, G_IO_ERROR, g_io_error_from_errno(errsv),
                            ...
                            server->nonce_file, g_strerror(errsv));
                goto out;
            }
            bytes_written += ret;
            bytes_remaining -= ret;
        ...
        server->client_address =
            g_strdup_printf("nonce-tcp:host=%s,port=%d,noncefile=%s", host,
                            port_num, file_escaped);
        g_free(file_escaped);
    }
    else
    {
        server->client_address =
            g_strdup_printf("tcp:host=%s,port=%d", host, port_num);
    }
    server->is_using_listener = TRUE;
    ret = TRUE;
...
static gboolean emit_new_connection_in_idle(gpointer user_data)
{
    ...
    claimed = FALSE;
    g_signal_emit(data->server, _signals[NEW_CONNECTION_SIGNAL], 0,
                  data->connection, &claimed);
                       ...
                       GObject* source_object, gpointer user_data)
{
    ...
    if (server->nonce != NULL)
    {
        ...
        if (!g_input_stream_read_all(
                ...
                buf, 16, &bytes_read, NULL, /* GCancellable */
                NULL))                      /* GError */
            goto out;
    ...
    connection = g_dbus_connection_new_sync(
        G_IO_STREAM(socket_connection), server->guid, connection_flags,
        server->authentication_observer, NULL, /* GCancellable */
        NULL);                                 /* GError */
    if (connection == NULL)
        goto out;
    ...
    else
    {
        ...
        g_source_set_callback(idle_source, emit_new_connection_in_idle, data,
                              ...
                              GError** error)
{
    ...
    {
        g_set_error(&last_error, G_IO_ERROR, G_IO_ERROR_INVALID_ARGUMENT,
    ...
    for (n = 0; addr_array != NULL && addr_array[n] != NULL; n++)
    {
        ...
        this_error = NULL;
        if (g_dbus_is_supported_address(address_entry, &this_error) &&
            _g_dbus_address_parse_entry(address_entry, &transport_name,
                                        &key_value_pairs, &this_error))
        {
            ...
            if (FALSE)
            {
                ...
                ret = try_unix(server, address_entry, key_value_pairs,
                               &this_error);
#endif
            else if (g_strcmp0(transport_name, "tcp") == 0)
                ret = try_tcp(server, address_entry, key_value_pairs, FALSE,
                              &this_error);
            else if (g_strcmp0(transport_name, "nonce-tcp") == 0)
                ret = try_tcp(server, address_entry, key_value_pairs, TRUE,
                              &this_error);
            else
...
out:
    ...
    if (ret)
    {
        ...
        g_assert(server->is_using_listener);
        server->run_signal_handler_id =
            g_signal_connect(G_SOCKET_SERVICE(server->listener), "run",
                             G_CALLBACK(on_run), server);
    }
    else
    {
