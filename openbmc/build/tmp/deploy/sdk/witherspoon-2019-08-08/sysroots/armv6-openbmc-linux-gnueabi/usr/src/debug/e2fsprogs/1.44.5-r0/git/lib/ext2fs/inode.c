                                 ...
                                 ext2_inode_scan* ret_scan)
{
    ...
    scan->inode_buffer_blocks =
        buffer_blocks ? buffer_blocks : EXT2_INODE_SCAN_DEFAULT_BUFFER_BLOCKS;
    scan->current_block = ext2fs_inode_table_loc(scan->fs, scan->current_group);
    ...
    if (ext2fs_has_group_desc_csum(fs))
    {
        ...
        if (scan->inodes_left > unused)
            ...
            scan->inodes_left = 0;
        scan->blocks_left =
            (scan->inodes_left + (fs->blocksize / scan->inode_size - 1)) *
...
static errcode_t get_next_blockgroup(ext2_inode_scan scan)
{
    ...
    scan->current_block = ext2fs_inode_table_loc(scan->fs, scan->current_group);
    scan->current_inode =
        scan->current_group * EXT2_INODES_PER_GROUP(fs->super);
...
errcode_t ext2fs_inode_scan_goto_blockgroup(ext2_inode_scan scan, int group)
{
...
#ifdef WORDS_BIGENDIAN
        ext2fs_swap_inode_full(scan->fs, (struct ext2_inode_large*)inode,
                               (struct ext2_inode_large*)p, 0,
                               EXT2_INODE_SIZE(scan->fs->super));
#else
...
static errcode_t get_next_blocks(ext2_inode_scan scan)
{
    ...
    {
        memset(scan->inode_buffer, 0, (size_t)num_blocks * scan->fs->blocksize);
    }
    else
    {
        retval = io_channel_read_blk64(scan->fs->io, scan->current_block,
                                       (int)num_blocks, scan->inode_buffer);
        if (retval)
                                     ...
                                     struct ext2_inode* inode, int bufsize)
{
    ...
    force_new_group:
        if (scan->done_group)
        {
            retval = (scan->done_group)(scan->fs, scan, scan->current_group,
    ...
    if ((scan->scan_flags & EXT2_SF_DO_LAZY) &&
        (ext2fs_bg_flags_test(scan->fs, scan->current_group,
    ...
    iblk = scan->current_inode % EXT2_INODES_PER_GROUP(scan->fs->super) /
           EXT2_INODES_PER_BLOCK(scan->fs->super) % scan->inode_buffer_blocks;
    if (extra_bytes)
    {
        ...
        if (!(iblock_status[iblk] & IBLOCK_STATUS_CSUMS_OK) &&
            ...
            !ext2fs_inode_csum_verify(
                scan->fs, scan->current_inode + 1,
                (struct ext2_inode_large*)scan->temp_buffer))
            retval = EXT2_ET_INODE_CSUM_INVALID;

#ifdef WORDS_BIGENDIAN
        memset(iptr, 0, length);
        ext2fs_swap_inode_full(scan->fs, (struct ext2_inode_large*)iptr,
                               (struct ext2_inode_large*)scan->temp_buffer, 0,
                               length);
#else
    ...
    else
    {
        ...
        memset(iptr, 0, length);
        ext2fs_swap_inode_full(scan->fs, (struct ext2_inode_large*)iptr,
                               (struct ext2_inode_large*)scan->ptr, 0, length);
#else
                                 ...
                                 struct ext2_inode* inode, int bufsize)
{
    ...
    if (fs->flags & EXT2_FLAG_IMAGE_FILE)
    {
        ...
        block_nr =
            ext2fs_le32_to_cpu(fs->image_header->offset_inode) / fs->blocksize;
        block_nr += (ino - 1) / inodes_per_block;
        offset = ((ino - 1) % inodes_per_block) * EXT2_INODE_SIZE(fs->super);
        io = fs->image_io;
    }
    else
    {
    ...
    while (length)
    {
        ...
        {
            retval = io_channel_read_blk64(io, block_nr, 1, fs->icache->buffer);
            if (retval)
                return retval;
            fs->icache->buffer_blk = block_nr;
        }

        memcpy(ptr, ((char*)fs->icache->buffer) + (unsigned)offset, clen);
    ...
    ext2fs_swap_inode_full(fs, (struct ext2_inode_large*)iptr,
                           (struct ext2_inode_large*)iptr, 0, length);
#endif
...
errcode_t ext2fs_read_inode(ext2_filsys fs, ext2_ino_t ino,
                            struct ext2_inode* inode)
{
    return ext2fs_read_inode_full(fs, ino, inode, sizeof(struct ext2_inode));
}
                                  ...
                                  struct ext2_inode* inode, int bufsize)
{
    ...
    if (bufsize < length)
    {
        ...
        retval = ext2fs_read_inode_full(fs, ino, (struct ext2_inode*)w_inode,
                                        length);
    ...
    while (length)
    {
        ...
        {
            retval =
                io_channel_read_blk64(fs->io, block_nr, 1, fs->icache->buffer);
            if (retval)
                goto errout;
            fs->icache->buffer_blk = block_nr;
        }

        memcpy((char*)fs->icache->buffer + (unsigned)offset, ptr, clen);

        retval =
            io_channel_write_blk64(fs->io, block_nr, 1, fs->icache->buffer);
        if (retval)
            goto errout;
...
errcode_t ext2fs_write_inode(ext2_filsys fs, ext2_ino_t ino,
                             struct ext2_inode* inode)
{
    return ext2fs_write_inode_full(fs, ino, inode, sizeof(struct ext2_inode));
}
