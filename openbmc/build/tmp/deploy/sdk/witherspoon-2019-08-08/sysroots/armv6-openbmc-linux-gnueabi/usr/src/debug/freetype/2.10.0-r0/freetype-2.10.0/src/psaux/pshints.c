...
static CF2_Int cf2_getWindingMomentum(CF2_Fixed x1, CF2_Fixed y1, CF2_Fixed x2,
                                      CF2_Fixed y2)
{
 ...
 *
 */
static void cf2_hint_init(CF2_Hint hint, const CF2_ArrStack stemHintArray,
                          size_t indexStemHint, const CF2_Font font,
                          CF2_Fixed hintOrigin, CF2_Fixed scale, FT_Bool bottom)
{
    CF2_Fixed width;
    ...
    stemHint = (const CF2_StemHintRec*)cf2_arrstack_getPointer(stemHintArray,
                                                               indexStemHint);
...
FT_LOCAL_DEF(void)
cf2_hintmap_init(CF2_HintMap hintmap, CF2_Font font, CF2_HintMap initialMap,
                 CF2_ArrStack hintMoves, CF2_Fixed scale)
{
    FT_ZERO(hintmap);
...
static void cf2_hintmap_dump(CF2_HintMap hintmap)
{
    ...
    for (i = 0; i < hintmap->count; i++)
    {
        ...
        FT_TRACE6(("  %3d    %7.2f  %7.2f  %5d  %s%s%s%s\n", hint->index,
                   hint->csCoord / 65536.0, hint->dsCoord / (hint->scale * 1.0),
                   hint->scale, (cf2_hint_isPair(hint) ? "p" : "g"),
...
static CF2_Fixed cf2_hintmap_map(CF2_HintMap hintmap, CF2_Fixed csCoord)
{
    ...
    else
    {
        ...
        if (i == 0 && csCoord < hintmap->edge[0].csCoord)
        {
            ...
            return ADD_INT32(
                FT_MulFix(SUB_INT32(csCoord, hintmap->edge[0].csCoord),
                          hintmap->scale),
        ...
        else
        {
             ...
             */
            return ADD_INT32(
                FT_MulFix(SUB_INT32(csCoord, hintmap->edge[i].csCoord),
...
static void cf2_hintmap_adjustHints(CF2_HintMap hintmap)
{
    ...
    for (i = 0; i < hintmap->count; i++)
    {
        ...
        if (!cf2_hint_isLocked(&hintmap->edge[i]))
        {
            ...
            CF2_Fixed downMoveUp =
                (fracDown == 0) ? 0 : cf2_intToFixed(1) - fracDown;
            CF2_Fixed upMoveUp = (fracUp == 0) ? 0 : cf2_intToFixed(1) - fracUp;
          ...
          upMinCounter = 0;
            ...
            if (j >= hintmap->count - 1 ||
                hintmap->edge[j + 1].dsCoord >=
                    ADD_INT32(hintmap->edge[j].dsCoord, moveUp + upMinCounter))
            {
                /* there is room to move up; is there also room to move down? */
                if (i == 0 || hintmap->edge[i - 1].dsCoord <=
                                  ADD_INT32(hintmap->edge[i].dsCoord,
                                            moveDown - downMinCounter))
                {
                    /* move smaller absolute amount */
                    move =
                        (-moveDown < moveUp) ? moveDown : moveUp; /* optimum */
                }
                else
                    move = moveUp;
            }
            else
            {
                ...
                {
                    /* no room to move either way without overlapping or
                     * reducing */
            ...
            /* move the edge(s) */
            hintmap->edge[i].dsCoord =
                ADD_INT32(hintmap->edge[i].dsCoord, move);
            if (isPair)
                hintmap->edge[j].dsCoord =
                    ADD_INT32(hintmap->edge[j].dsCoord, move);
        }
    ...
    {
        CF2_HintMove hintMove =
            (CF2_HintMove)cf2_arrstack_getPointer(hintmap->hintMoves, i - 1);
                      ...
                      hintMove->moveUp + CF2_MIN_COUNTER))
        {
            ...
            hintmap->edge[j].dsCoord =
                ADD_INT32(hintmap->edge[j].dsCoord, hintMove->moveUp);
...
static void cf2_hintmap_insertHint(CF2_HintMap hintmap, CF2_Hint bottomHintEdge,
        ...
        !cf2_hint_isLocked(firstHintEdge))
    {
        if (isPair)
        {
            ...
            CF2_Fixed midpoint = cf2_hintmap_map(
                ...
                ADD_INT32(secondHintEdge->csCoord, firstHintEdge->csCoord) / 2);
            CF2_Fixed halfWidth = FT_MulFix(
                SUB_INT32(secondHintEdge->csCoord, firstHintEdge->csCoord) / 2,
                hintmap->scale);
 ...
 */
FT_LOCAL_DEF(void)
cf2_hintmap_build(CF2_HintMap hintmap, CF2_ArrStack hStemHintArray,
                  CF2_ArrStack vStemHintArray, CF2_HintMask hintMask,
                  CF2_Fixed hintOrigin, FT_Bool initialMap)
{
    ...
    if (!initialMap && !cf2_hintmap_isValid(hintmap->initialHintMap))
    {
        ...
        cf2_hintmap_build(hintmap->initialHintMap, hStemHintArray,
                          vStemHintArray, &tempHintMask, hintOrigin, TRUE);
    }

    if (!cf2_hintmask_isValid(hintMask))
    {
        /* without a hint mask, assume all hints are active */
        cf2_hintmask_setAll(hintMask, cf2_arrstack_size(hStemHintArray) +
    ...
    if (font->blues.doEmBoxHints)
    {
        ...
        /* ghost bottom */
        cf2_hintmap_insertHint(hintmap, &font->blues.emBoxBottomEdge, &dummy);
        /* ghost top */
        cf2_hintmap_insertHint(hintmap, &dummy, &font->blues.emBoxTopEdge);
    }
    ...
    for (i = 0, maskByte = 0x80; i < bitCount; i++)
    {
        if (maskByte & *maskPtr)
        {
            ...
            cf2_hint_init(&bottomHintEdge, hStemHintArray, i, font, hintOrigin,
                          hintmap->scale, TRUE /* bottom */);
            cf2_hint_init(&topHintEdge, hStemHintArray, i, font, hintOrigin,
                          hintmap->scale, FALSE /* top */);

            if (cf2_hint_isLocked(&bottomHintEdge) ||
                cf2_hint_isLocked(&topHintEdge) ||
                cf2_blues_capture(&font->blues, &bottomHintEdge, &topHintEdge))
            {
    ...
    if (initialMap)
    {
        ...
        if (hintmap->count == 0 || hintmap->edge[0].csCoord > 0 ||
            hintmap->edge[hintmap->count - 1].csCoord < 0)
        {
            ...
            edge.flags = CF2_GhostBottom | CF2_Locked | CF2_Synthetic;
    ...
    else
    {
        ...
        for (i = 0, maskByte = 0x80; i < bitCount; i++)
        {
            if (maskByte & *maskPtr)
            {
                ...
                cf2_hint_init(&bottomHintEdge, hStemHintArray, i, font,
                              hintOrigin, hintmap->scale, TRUE /* bottom */);
                cf2_hint_init(&topHintEdge, hStemHintArray, i, font, hintOrigin,
                              hintmap->scale, FALSE /* top */);
    ...
    if (!initialMap)
    {
        for (i = 0; i < hintmap->count; i++)
        {
            if (!cf2_hint_isSynthetic(&hintmap->edge[i]))
            {
                ...
                CF2_StemHint stemhint = (CF2_StemHint)cf2_arrstack_getPointer(
                    hStemHintArray, hintmap->edge[i].index);
...
FT_LOCAL_DEF(void)
cf2_glyphpath_init(CF2_GlyphPath glyphpath, CF2_Font font,
                   CF2_OutlineCallbacks callbacks, CF2_Fixed scaleY,
                   /* CF2_Fixed  hShift, */
                   CF2_ArrStack hStemHintArray, CF2_ArrStack vStemHintArray,
                   CF2_HintMask hintMask, CF2_Fixed hintOriginY,
                   ...
                   const FT_Vector* fractionalTranslation)
{
    ...
    cf2_arrstack_init(&glyphpath->hintMoves, font->memory, &font->error,
                      sizeof(CF2_HintMoveRec));

    cf2_hintmap_init(&glyphpath->initialHintMap, font,
                     &glyphpath->initialHintMap, &glyphpath->hintMoves, scaleY);
    cf2_hintmap_init(&glyphpath->firstHintMap, font, &glyphpath->initialHintMap,
                     &glyphpath->hintMoves, scaleY);
    cf2_hintmap_init(&glyphpath->hintMap, font, &glyphpath->initialHintMap,
                     &glyphpath->hintMoves, scaleY);
    ...
    glyphpath->miterLimit = 2 * FT_MAX(cf2_fixedAbs(glyphpath->xOffset),
...
static void cf2_glyphpath_hintPoint(CF2_GlyphPath glyphpath,
                                    CF2_HintMap hintmap, FT_Vector* ppt,
                                    CF2_Fixed x, CF2_Fixed y)
{
    ...
    ppt->x =
        ADD_INT32(FT_MulFix(glyphpath->font->outerTransform.a, pt.x),
                  ADD_INT32(FT_MulFix(glyphpath->font->outerTransform.c, pt.y),
                            glyphpath->fractionalTranslation.x));
    ppt->y =
        ADD_INT32(FT_MulFix(glyphpath->font->outerTransform.b, pt.x),
                  ADD_INT32(FT_MulFix(glyphpath->font->outerTransform.d, pt.y),
 ...
 *
 */
static FT_Bool cf2_glyphpath_computeIntersection(
    CF2_GlyphPath glyphpath, const FT_Vector* u1, const FT_Vector* u2,
    const FT_Vector* v1, const FT_Vector* v2, FT_Vector* intersection)
{
    /*
...
#define CF2_CS_SCALE(x) (((x) + 0x10) >> 5)
    ...
    intersection->x = ADD_INT32(u1->x, FT_MulFix(s, SUB_INT32(u2->x, u1->x)));
    intersection->y = ADD_INT32(u1->y, FT_MulFix(s, SUB_INT32(u2->y, u1->y)));
    ...
    if (u1->x == u2->x && cf2_fixedAbs(SUB_INT32(intersection->x, u1->x)) <
                              glyphpath->snapThreshold)
        intersection->x = u1->x;
    if (u1->y == u2->y && cf2_fixedAbs(SUB_INT32(intersection->y, u1->y)) <
                              glyphpath->snapThreshold)
        intersection->y = u1->y;

    if (v1->x == v2->x && cf2_fixedAbs(SUB_INT32(intersection->x, v1->x)) <
                              glyphpath->snapThreshold)
        intersection->x = v1->x;
    if (v1->y == v2->y && cf2_fixedAbs(SUB_INT32(intersection->y, v1->y)) <
                              glyphpath->snapThreshold)
        intersection->y = v1->y;
...
static void cf2_glyphpath_pushPrevElem(CF2_GlyphPath glyphpath,
                                       CF2_HintMap hintmap, FT_Vector* nextP0,
                                       FT_Vector nextP1, FT_Bool close)
{
    ...
    if (prevP1->x != nextP0->x || prevP1->y != nextP0->y)
    {
        ...
        useIntersection = cf2_glyphpath_computeIntersection(
            glyphpath, prevP0, prevP1, nextP0, &nextP1, &intersection);
        if (useIntersection)
        {
    ...
    switch (glyphpath->prevElemOp)
    {
        case CF2_PathOpLineTo:
            ...
            if (close)
            {
                ...
                cf2_glyphpath_hintPoint(glyphpath, &glyphpath->firstHintMap,
                                        &params.pt1, glyphpath->prevElemP1.x,
                                        glyphpath->prevElemP1.y);
            }
            else
            {
                cf2_glyphpath_hintPoint(glyphpath, hintmap, &params.pt1,
        ...
        case CF2_PathOpCubeTo:
            ...
            cf2_glyphpath_hintPoint(glyphpath, hintmap, &params.pt1,
                                    glyphpath->prevElemP1.x,
                                    glyphpath->prevElemP1.y);
            cf2_glyphpath_hintPoint(glyphpath, hintmap, &params.pt2,
                                    glyphpath->prevElemP2.x,
                                    glyphpath->prevElemP2.y);
            cf2_glyphpath_hintPoint(glyphpath, hintmap, &params.pt3,
    ...
    if (!useIntersection || close)
    {
        ...
        if (close)
        {
            ...
            /* hint zone                                                      */
            cf2_glyphpath_hintPoint(glyphpath, &glyphpath->firstHintMap,
                                    &params.pt1, nextP0->x, nextP0->y);
        }
        else
        {
            cf2_glyphpath_hintPoint(glyphpath, hintmap, &params.pt1, nextP0->x,
                                    nextP0->y);
        }
...
static void cf2_glyphpath_pushMove(CF2_GlyphPath glyphpath, FT_Vector start)
{
    ...
    if (!cf2_hintmap_isValid(&glyphpath->hintMap))
    {
        ...
        cf2_glyphpath_moveTo(glyphpath, glyphpath->start.x, glyphpath->start.y);
    }

    cf2_glyphpath_hintPoint(glyphpath, &glyphpath->hintMap, &params.pt1,
                            start.x, start.y);
 ...
 */
static void cf2_glyphpath_computeOffset(CF2_GlyphPath glyphpath, CF2_Fixed x1,
                                        CF2_Fixed y1, CF2_Fixed x2,
                                        CF2_Fixed y2, CF2_Fixed* x,
                                        CF2_Fixed* y)
{
    ...
    *x = *y = 0;
    ...
    if (dx >= 0)
    {
        if (dy >= 0)
        {
            ...
            {
                /* +x +y */
                *x = FT_MulFix(cf2_doubleToFixed(0.7), glyphpath->xOffset);
                *y =
                    FT_MulFix(cf2_doubleToFixed(1.0 - 0.7), glyphpath->yOffset);
            }
        }
        else
        {
...
cf2_glyphpath_moveTo(CF2_GlyphPath glyphpath, CF2_Fixed x, CF2_Fixed y)
{
    ...
    if (!cf2_hintmap_isValid(&glyphpath->hintMap) ||
        ...
        cf2_hintmap_build(&glyphpath->hintMap, glyphpath->hStemHintArray,
                          glyphpath->vStemHintArray, glyphpath->hintMask,
                          glyphpath->hintOriginY, FALSE);
...
cf2_glyphpath_lineTo(CF2_GlyphPath glyphpath, CF2_Fixed x, CF2_Fixed y)
{
    ...
    newHintMap =
        cf2_hintmask_isNew(glyphpath->hintMask) && !glyphpath->pathIsClosing;
    ...
    cf2_glyphpath_computeOffset(glyphpath, glyphpath->currentCS.x,
                                glyphpath->currentCS.y, x, y, &xOffset,
                                &yOffset);
    ...
    if (glyphpath->elemIsQueued)
    {
        ...
        cf2_glyphpath_pushPrevElem(glyphpath, &glyphpath->hintMap, &P0, P1,
                                   FALSE);
    }
...
FT_LOCAL_DEF(void)
cf2_glyphpath_curveTo(CF2_GlyphPath glyphpath, CF2_Fixed x1, CF2_Fixed y1,
                      CF2_Fixed x2, CF2_Fixed y2, CF2_Fixed x3, CF2_Fixed y3)
{
    ...
    cf2_glyphpath_computeOffset(glyphpath, glyphpath->currentCS.x,
                                glyphpath->currentCS.y, x1, y1, &xOffset1,
                                &yOffset1);
    cf2_glyphpath_computeOffset(glyphpath, x2, y2, x3, y3, &xOffset3,
                                &yOffset3);
...
cf2_glyphpath_closeOpenPath(CF2_GlyphPath glyphpath)
{
    if (glyphpath->pathIsOpen)
    {
        ...
        cf2_glyphpath_lineTo(glyphpath, glyphpath->start.x, glyphpath->start.y);

        /* empty the final element from the queue and close the path */
        if (glyphpath->elemIsQueued)
            cf2_glyphpath_pushPrevElem(glyphpath, &glyphpath->hintMap,
                                       &glyphpath->offsetStart0,
                                       glyphpath->offsetStart1, TRUE);
