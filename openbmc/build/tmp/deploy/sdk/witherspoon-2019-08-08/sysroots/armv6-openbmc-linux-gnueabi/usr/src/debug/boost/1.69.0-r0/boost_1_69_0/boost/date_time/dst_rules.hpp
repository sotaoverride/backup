...
class dst_calculator
{
  public:
     ...
     *  @param dst_end_offset Time offset within day given in dst for dst
     * boundary
     *  @param dst_length lenght of dst adjusment
     *  @retval The time is either ambiguous, invalid, in dst, or not in dst
     */
    static time_is_dst_result local_is_dst(
        const date_type& current_day, const time_duration_type& time_of_day,
        const date_type& dst_start_day,
        const time_duration_type& dst_start_offset,
        const date_type& dst_end_day, const time_duration_type& dst_end_offset,
        const time_duration_type& dst_length_minutes)
    {
        ...
        return local_is_dst(current_day, time_of_day, dst_start_day,
                            start_minutes, dst_end_day, end_minutes,
                            length_minutes);
    }
    ...
    static time_is_dst_result local_is_dst(
        ...
        const date_type& dst_end_day, unsigned int dst_end_offset_minutes,
        long dst_length_minutes)
    {
        ...
        { // southern hemisphere dst is at begining /end of year
            if ((current_day < dst_start_day) && (current_day > dst_end_day))
            {
                return is_not_in_dst;
            }
        ...
        {
            return process_local_dst_start_day(
                time_of_day, dst_start_offset_minutes, dst_length_minutes);
        }

        if (current_day == dst_end_day)
        {
            return process_local_dst_end_day(
                time_of_day, dst_end_offset_minutes, dst_length_minutes);
        }
 ...
 * dst_start_offset_minutes - number of minutes from start of day to transition
 * to dst -- 120 (or 2:00 am) is typical for the U.S. and E.U.
 * dst_start_offset_minutes - number of minutes from start of day to transition
 * off of dst -- 180 (or 3:00 am) is typical for E.U. dst_length_minutes -
 * number of minutes that dst shifts clock
 */
...
class dst_calc_engine
{
  public:
                                           ...
                                           const time_duration_type& td)
    {
        ...
        return dstcalc::local_is_dst(
            d, td, dst_start, dst_traits::dst_start_offset_minutes(), dst_end,
    ...
    {
        year_type y = d.year();
        return ((d == local_dst_start_day(y)) || (d == local_dst_end_day(y)));
    }

    //! The time of day for the dst transition (eg: typically 01:00:00 or
    //! 02:00:00)
...
class us_dst_rules
{
  public:
                                           ...
                                           const time_duration_type& td)
    {
        ...
        date_type dst_start = local_dst_start_day(y);
        date_type dst_end = local_dst_end_day(y);
        return dstcalc::local_is_dst(d, td, dst_start, dst_start_offset_minutes,
