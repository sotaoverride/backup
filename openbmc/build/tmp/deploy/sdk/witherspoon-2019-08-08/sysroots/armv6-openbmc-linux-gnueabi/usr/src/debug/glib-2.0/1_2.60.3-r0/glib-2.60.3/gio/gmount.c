 ...
 * callback should then call g_mount_unmount_with_operation_finish() with the
 *#GMount and the #GAsyncResult data to see if the operation was completed
 * successfully.  If an @error is present when
 *g_mount_unmount_with_operation_finish() is called, then it will be filled with
...
static void g_mount_default_init(GMountInterface* iface)
{
     ...
     **/
    g_signal_new(I_("changed"), G_TYPE_MOUNT, G_SIGNAL_RUN_LAST,
                 G_STRUCT_OFFSET(GMountIface, changed), NULL, NULL,
     ...
     * finalized.
     **/
    g_signal_new(I_("unmounted"), G_TYPE_MOUNT, G_SIGNAL_RUN_LAST,
                 G_STRUCT_OFFSET(GMountIface, unmounted), NULL, NULL,
                 g_cclosure_marshal_VOID__VOID, G_TYPE_NONE, 0);
    /**
     * GMount::pre-unmount:
     * @mount: the object on which the signal is emitted
     ...
     * Since: 2.22
     **/
    g_signal_new(I_("pre-unmount"), G_TYPE_MOUNT, G_SIGNAL_RUN_LAST,
                 G_STRUCT_OFFSET(GMountIface, pre_unmount), NULL, NULL,
                 g_cclosure_marshal_VOID__VOID, G_TYPE_NONE, 0);
}
                     ...
                     gpointer user_data)
{
    ...
    if (iface->unmount == NULL)
    {
        g_task_report_new_error(mount, callback, user_data,
                                g_mount_unmount_with_operation, G_IO_ERROR,
                   ...
                   gpointer user_data)
{
    ...
    {
        g_task_report_new_error(mount, callback, user_data,
                                g_mount_eject_with_operation, G_IO_ERROR,
                                G_IO_ERROR_NOT_SUPPORTED,
                                /* Translators: This is an error
                                 * message for mount objects that
                                    ...
                                    gpointer user_data)
{
    ...
    if (iface->unmount_with_operation != NULL)
        (*iface->unmount_with_operation)(mount, flags, mount_operation,
                                  ...
                                  gpointer user_data)
{
    ...
    if (iface->eject_with_operation != NULL)
        (*iface->eject_with_operation)(mount, flags, mount_operation,
                                       cancellable, callback, user_data);
    else
        (*iface->eject)(mount, flags, cancellable, callback, user_data);
}
                     ...
                     gpointer user_data)
{
    ...
    {
        g_task_report_new_error(mount, callback, user_data, g_mount_remount,
                                G_IO_ERROR, G_IO_ERROR_NOT_SUPPORTED,
                                /* Translators: This is an error
                                 * message for mount objects that
    ...
    (*iface->remount)(mount, flags, mount_operation, cancellable, callback,
                      user_data);
}
                                ...
                                gpointer user_data)
{
    ...
    {
        g_task_report_new_error(
            mount, callback, user_data, g_mount_guess_content_type, G_IO_ERROR,
            G_IO_ERROR_NOT_SUPPORTED,
            /* Translators: This is an error
             * message for mount objects that
    ...
    (*iface->guess_content_type)(mount, force_rescan, cancellable, callback,
                                 user_data);
}
 ...
 * guessing.
 *
 * Returns: (transfer full) (element-type utf8): a %NULL-terminated array of
 *content types or %NULL on error. Caller should free this array with
 ...
 * Since: 2.18
 */
char** g_mount_guess_content_type_sync(GMount* mount, gboolean force_rescan,
                                       GCancellable* cancellable,
                                       GError** error)
{
    GMountIface* iface;
    ...
    return (*iface->guess_content_type_sync)(mount, force_rescan, cancellable,
                                             error);
}
...
static GMountPrivate* get_private(GMount* mount)
{
  ...
  private
    = g_object_get_data(G_OBJECT(mount), "g-mount-private");
    if (G_LIKELY(private != NULL))
        goto out;

  private
    = g_new0(GMountPrivate, 1);
    g_object_set_data_full(G_OBJECT(mount), "g-mount-private", private,
 ...
 *
 * Returns: (nullable): Sorting key for @mount or %NULL if no such key is
 * available.
 *
 * Since: 2.32
 */
