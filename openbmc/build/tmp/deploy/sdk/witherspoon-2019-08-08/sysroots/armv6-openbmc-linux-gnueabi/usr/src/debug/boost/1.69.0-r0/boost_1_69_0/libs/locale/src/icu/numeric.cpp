...
namespace details
{
template <typename V, int n = std::numeric_limits<V>::digits,
          bool integer = std::numeric_limits<V>::is_integer>
struct cast_traits;
...
// to POSIX formatting
template <typename V, bool Sig = std::numeric_limits<V>::is_signed,
...
class num_base
{
  protected:
    ...
    static bool use_parent(std::ios_base& ios, ValueType v)
    {
        ...
        if (flg == flags::number &&
            (ios.flags() & std::ios_base::basefield) != std::ios_base::dec)
        {
            return true;
        }
        return false;
...
class num_format : public std::num_put<CharType>, protected num_base
{
  public:
    ...
    num_format(cdata const& d, size_t refs = 0) :
        std::num_put<CharType>(refs), loc_(d.locale), enc_(d.encoding)
    {
    }
  ...
  private:
                          ...
                          ValueType val) const
    {
        ...
        string_type const& str =
            formatter->format(static_cast<cast_type>(val), code_points);
...
class num_parse : public std::num_get<CharType>, protected num_base
{
  public:
    num_parse(cdata const& d, size_t refs = 0) :
        std::num_get<CharType>(refs), loc_(d.locale), enc_(d.encoding)
    {
    }
  ...
  private:
                          ...
                          std::ios_base::iostate& err, ValueType& val) const
    {
        ...
        CharType c;
        while (in != end && (((c = *in) <= 32 && (c > 0)) ||
                             c == 127)) // Assuming that ASCII is a subset
            ++in;
        ...
        if ((parsed_chars = formatter->parse(tmp, value)) == 0 ||
            !valid<ValueType>(value))
        {
...
std::locale create_formatting(std::locale const& in, cdata const& cd,
...
std::locale create_parsing(std::locale const& in, cdata const& cd,
                           character_facet_type type)
{
    switch (type)
    {
