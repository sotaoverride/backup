...
static int files_add(Hashmap* h, Set* masked, const char* suffix,
                     const char* root, unsigned flags, const char* path)
{
    ...
    if (!dir)
    {
        ...
        return log_debug_errno(errno, "Failed to open directory '%s': %m",
                               dirpath);
    }
    ...
    FOREACH_DIRENT(de, dir, return -errno)
    {
        ...
        if ((flags & CONF_FILES_FILTER_MASKED) &&
            set_contains(masked, de->d_name))
        {
            log_debug("File '%s/%s' is masked by previous entry.", dirpath,
                      de->d_name);
            continue;
        }

        /* Read file metadata if we shall validate the check for file masks, for
         * node types or whether the node is marked executable. */
        if (flags & (CONF_FILES_FILTER_MASKED | CONF_FILES_REGULAR |
                     CONF_FILES_DIRECTORY | CONF_FILES_EXECUTABLE))
        ...
        if (flags & CONF_FILES_EXECUTABLE)
            /* As requested: check if the file is marked exectuable. Note that
             * we don't check access(X_OK) here, as we care about whether the
             * file is marked executable at all, and not whether it is
             * executable for us, because if so, such errors are stuff we should
             * log about. */
...
static int conf_files_list_strv_internal(char*** strv, const char* suffix,
                                         const char* root, unsigned flags,
                                         char** dirs)
{
    ...
    STRV_FOREACH(p, dirs)
    {
        ...
        if (r == -ENOMEM)
            return r;
        if (r < 0)
            log_debug_errno(r, "Failed to search for files in %s, ignoring: %m",
                            *p);
    }
...
int conf_files_insert(char*** strv, const char* root, char** dirs,
                      const char* path)
{
    ...
    for (i = 0; i < n; i++)
    {
        ...
        if (c == 0)
        {
            ...
            /* Oh, there already is an entry with a matching name (the last
             * component). */
...
int conf_files_list_strv(char*** strv, const char* suffix, const char* root,
                         unsigned flags, const char* const* dirs)
{
...
int conf_files_list_nulstr(char*** strv, const char* suffix, const char* root,
                           unsigned flags, const char* dirs)
{
...
int conf_files_list_with_replacement(const char* root, char** config_dirs,
                                     const char* replacement, char*** files,
                                     char** replace_file)
{
    ...
    r = conf_files_list_strv(&f, ".conf", root, 0,
