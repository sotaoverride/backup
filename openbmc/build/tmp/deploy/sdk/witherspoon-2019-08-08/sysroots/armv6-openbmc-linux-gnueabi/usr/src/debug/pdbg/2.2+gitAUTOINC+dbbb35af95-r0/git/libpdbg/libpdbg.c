...
struct pdbg_target* __pdbg_next_target(const char* class,
...
retry:
    ...
    if (last)
        next =
            list_entry(last->class_link.next, struct pdbg_target, class_link);
    else if (!(next = list_top(&target_class->targets, struct pdbg_target,
                               class_link)))
        return NULL;
...
struct pdbg_target* pdbg_target_parent(const char* class,
...
struct pdbg_target* pdbg_target_require_parent(const char* class,
                                               struct pdbg_target* target)
{
...
/* Searched up the tree for the first target of the right class and returns its
 * index */
