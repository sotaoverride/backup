...
typedef void (*gensalt_fn)(unsigned long count, const uint8_t* rbytes,
...
static const struct hashfn hash_algorithms[] = {HASH_ALGORITHM_TABLE_ENTRIES};

#if INCLUDE_descrypt || INCLUDE_bigcrypt
static int is_des_salt_char(char c)
{
...
static const struct hashfn* get_hashfn(const char* setting)
{
...
static void do_crypt(const char* phrase, const char* setting,
                     struct crypt_data* data)
{
    ...
    h->crypt(phrase, phr_size, setting, set_size, (unsigned char*)data->output,
             sizeof data->output, cint->alg_specific,
             sizeof cint->alg_specific);
                       ...
                       int output_size)
{
    ...
    h->gensalt(count, (const unsigned char*)rbytes, (size_t)nrbytes,
