...
struct DBusKeyring
{
    ...
    DBusCredentials*
        credentials; /**< Credentials containing user the keyring is for */
};
...
static dbus_bool_t add_new_key(DBusKey** keys_p, int* n_keys_p,
                               DBusError* error)
{
    DBusKey* new;
    DBusString bytes;
 ...
 */
static dbus_bool_t _dbus_keyring_reload(DBusKeyring* keyring,
                                        dbus_bool_t add_new, DBusError* error)
{
    DBusString contents;
    DBusString line;
    dbus_bool_t retval;
    ...
    {
        _dbus_warn("Secret keyring file contains non-ASCII! Ignoring existing "
                   "contents");
    ...
    while (_dbus_string_pop_line(&contents, &line))
    {
        ...
        {
            _dbus_verbose("dropping/ignoring %ld-seconds old key with "
                          "timestamp %ld as current time is %ld\n",
    ...
    if (add_new)
    {
        ...
        while (i < n_keys)
        {
            if (!_dbus_string_append_int(&contents, keys[i].id))
                goto nomem;

            if (!_dbus_string_append_byte(&contents, ' '))
                goto nomem;

            if (!_dbus_string_append_int(&contents, keys[i].creation_time))
                goto nomem;

            if (!_dbus_string_append_byte(&contents, ' '))
                goto nomem;

            if (!_dbus_string_hex_encode(&keys[i].secret, 0, &contents,
                                         _dbus_string_get_length(&contents)))
                goto nomem;
        ...
        if (!_dbus_string_save_to_file(&contents, &keyring->filename, FALSE,
                                       error))
            goto out;
    }
...
out:
          ...
          (retval == FALSE && (error == NULL || error->name != NULL))))
    {
        ...
        _dbus_warn("returning %d but error pointer %p name %s", retval, error,
                                               ...
                                               DBusError* error)
{
    ...
    if (!_dbus_string_copy(&ringdir, 0, &keyring->directory, 0))
        goto failed;

    /* Create keyring->filename based on keyring dir and context */
    if (!_dbus_string_copy(&keyring->directory, 0, &keyring->filename, 0))
        goto failed;

    if (!_dbus_concat_dir_and_file(&keyring->filename, context))
        goto failed;

    /* Create lockfile name */
    if (!_dbus_string_copy(&keyring->filename, 0, &keyring->filename_lock, 0))
        goto failed;
...
int _dbus_keyring_get_best_key(DBusKeyring* keyring, DBusError* error)
{
     ...
     */
    if (!_dbus_keyring_reload(keyring, TRUE, error))
        return -1;
    ...
    {
        dbus_set_error_const(error, DBUS_ERROR_FAILED,
                             "No recent-enough key found in keyring, and "
                             "unable to create a new key");
        return -1;
    }
}
 ...
 */
dbus_bool_t _dbus_keyring_get_hex_key(DBusKeyring* keyring, int key_id,
                                      DBusString* hex_key)
{
    DBusKey* key;
    ...
    return _dbus_string_hex_encode(&key->secret, 0, hex_key,
...
dbus_bool_t _dbus_keyring_test(void)
{
    ...
    while (i < ring1->n_keys)
    {
        ...
        {
            fprintf(stderr, "Keyrings 1 and 2 have different secrets for same "
                            "ID/timestamp\n");
            goto failure;
        }
