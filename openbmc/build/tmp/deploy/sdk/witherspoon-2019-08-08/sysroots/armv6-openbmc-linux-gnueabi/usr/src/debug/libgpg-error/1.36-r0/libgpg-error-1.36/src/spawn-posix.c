...
static void out_of_core(int line)
{
    _gpgrt_log_fatal("malloc failed at line %d: %s\n", line,
                     _gpg_strerror(_gpg_err_code_from_syserror()));
    /*NOTREACHED*/
}
...
static void close_all_fds(int first, int* except)
{
...
static void do_exec(const char* pgmname, const char* argv[], int fd_in,
                    int fd_out, int fd_err, int* except, void (*preexec)(void))
{
    char** arg_list;
    int i, j;
    int fds[3];
...
static gpg_err_code_t do_create_pipe(int filedes[2])
{
    gpg_error_t err = 0;
...
/* Helper for _gpgrt_make_pipe.  */
static gpg_err_code_t do_create_pipe_and_estream(int filedes[2],
...
{
    if (r_fp && direction)
        return do_create_pipe_and_estream(filedes, r_fp, (direction > 0),
                                          nonblock);
    else
                                    ...
                                    pid_t* pid)
{
    ...
    *pid = fork();
    ...
    if (!*pid)
    {
        ...
        _gpgrt_fclose(outfp);
        _gpgrt_fclose(errfp);
        do_exec(pgmname, argv, inpipe[0], outpipe[1], errpipe[1], except,
                preexec);
        /*NOTREACHED*/
    }
...
static gpg_err_code_t store_result(pid_t pid, int exitcode)
{
...
static int get_result(pid_t pid, int* r_exitcode)
{
    struct terminated_child *c, **prevp;

    for (prevp = &terminated_children, c = terminated_children; c;
                                     ...
                                     size_t count, int hang, int* r_exitcodes)
{
    ...
    while (left > 0)
    {
        ...
        _gpgrt_pre_syscall();
        while ((pid = waitpid(-1, &status, hang ? 0 : WNOHANG)) ==
                   (pid_t)(-1) &&
               errno == EINTR)
            ;
        _gpgrt_post_syscall();
