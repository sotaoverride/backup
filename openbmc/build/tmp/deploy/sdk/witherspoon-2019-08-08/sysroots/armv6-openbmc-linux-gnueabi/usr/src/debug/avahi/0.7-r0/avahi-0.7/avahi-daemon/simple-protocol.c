...
static void client_work(AvahiWatch* watch, int fd, AvahiWatchEvent events,
                        void* userdata);
...
static void client_new(Server* s, int fd)
{
    ...
    c->watch =
        s->poll_api->watch_new(s->poll_api, fd, AVAHI_WATCH_IN, client_work, c);
...
static void host_name_resolver_callback(
    AVAHI_GCC_UNUSED AvahiSHostNameResolver* r, AvahiIfIndex iface,
    AvahiProtocol protocol, AvahiResolverEvent event, const char* hostname,
    const AvahiAddress* a, AVAHI_GCC_UNUSED AvahiLookupResultFlags flags,
    void* userdata)
{
    ...
    if (event == AVAHI_RESOLVER_FAILURE)
        client_output_printf(c, "%+i %s\n", avahi_server_errno(avahi_server),
...
static void address_resolver_callback(
    AVAHI_GCC_UNUSED AvahiSAddressResolver* r, AvahiIfIndex iface,
    AvahiProtocol protocol, AvahiResolverEvent event,
    AVAHI_GCC_UNUSED const AvahiAddress* a, const char* hostname,
...
static void dns_server_browser_callback(
    AVAHI_GCC_UNUSED AvahiSDNSServerBrowser* b, AvahiIfIndex interface,
    AvahiProtocol protocol, AvahiBrowserEvent event,
    AVAHI_GCC_UNUSED const char* host_name, const AvahiAddress* a,
    uint16_t port, AVAHI_GCC_UNUSED AvahiLookupResultFlags flags,
    void* userdata)
{
...
static void handle_line(Client* c, const char* s)
{
    ...
    {
        client_output_printf(c, "+ Available commands are:\n"
    ...
    else if (strcmp(cmd, "RESOLVE-HOSTNAME-IPV4") == 0 && n_args == 2)
    {
        ...
        if (!(c->host_name_resolver = avahi_s_host_name_resolver_new(
                  avahi_server, AVAHI_IF_UNSPEC, AVAHI_PROTO_UNSPEC, arg,
                  c->afquery = AVAHI_PROTO_INET, AVAHI_LOOKUP_USE_MULTICAST,
                  host_name_resolver_callback, c)))
            goto fail;
    ...
    else if (strcmp(cmd, "RESOLVE-HOSTNAME-IPV6") == 0 && n_args == 2)
    {
        ...
        if (!(c->host_name_resolver = avahi_s_host_name_resolver_new(
                  avahi_server, AVAHI_IF_UNSPEC, AVAHI_PROTO_UNSPEC, arg,
                  c->afquery = AVAHI_PROTO_INET6, AVAHI_LOOKUP_USE_MULTICAST,
                  host_name_resolver_callback, c)))
            goto fail;
    ...
    else if (strcmp(cmd, "RESOLVE-HOSTNAME") == 0 && n_args == 2)
    {
        ...
        if (!(c->host_name_resolver = avahi_s_host_name_resolver_new(
                  avahi_server, AVAHI_IF_UNSPEC, AVAHI_PROTO_UNSPEC, arg,
                  c->afquery = AVAHI_PROTO_UNSPEC, AVAHI_LOOKUP_USE_MULTICAST,
                  host_name_resolver_callback, c)))
            goto fail;
    ...
    else if (strcmp(cmd, "RESOLVE-ADDRESS") == 0 && n_args == 2)
    {
        ...
        else
        {
            ...
            if (!(c->address_resolver = avahi_s_address_resolver_new(
                      avahi_server, AVAHI_IF_UNSPEC, AVAHI_PROTO_UNSPEC, &addr,
                      AVAHI_LOOKUP_USE_MULTICAST, address_resolver_callback,
                      c)))
                goto fail;
        }
    ...
    else if (strcmp(cmd, "BROWSE-DNS-SERVERS-IPV4") == 0 && n_args == 1)
    {
        ...
        if (!(c->dns_server_browser = avahi_s_dns_server_browser_new(
                  avahi_server, AVAHI_IF_UNSPEC, AVAHI_PROTO_UNSPEC, NULL,
                  AVAHI_DNS_SERVER_RESOLVE, c->afquery = AVAHI_PROTO_INET,
                  AVAHI_LOOKUP_USE_MULTICAST, dns_server_browser_callback, c)))
            goto fail;
    ...
    else if (strcmp(cmd, "BROWSE-DNS-SERVERS-IPV6") == 0 && n_args == 1)
    {
        ...
        if (!(c->dns_server_browser = avahi_s_dns_server_browser_new(
                  avahi_server, AVAHI_IF_UNSPEC, AVAHI_PROTO_UNSPEC, NULL,
                  AVAHI_DNS_SERVER_RESOLVE, c->afquery = AVAHI_PROTO_INET6,
                  AVAHI_LOOKUP_USE_MULTICAST, dns_server_browser_callback, c)))
            goto fail;
        client_output_printf(c, "+ Browsing ...\n");
    ...
    else if (strcmp(cmd, "BROWSE-DNS-SERVERS") == 0 && n_args == 1)
    {
        ...
        if (!(c->dns_server_browser = avahi_s_dns_server_browser_new(
                  avahi_server, AVAHI_IF_UNSPEC, AVAHI_PROTO_UNSPEC, NULL,
                  AVAHI_DNS_SERVER_RESOLVE, c->afquery = AVAHI_PROTO_UNSPEC,
                  AVAHI_LOOKUP_USE_MULTICAST, dns_server_browser_callback, c)))
            goto fail;
        client_output_printf(c, "+ Browsing ...\n");
...
static void client_work(AvahiWatch* watch, AVAHI_GCC_UNUSED int fd,
                        AvahiWatchEvent events, void* userdata)
{
    ...
    if ((events & AVAHI_WATCH_IN) && c->inbuf_length < sizeof(c->inbuf))
    {
        ...
        if ((r = read(c->fd, c->inbuf + c->inbuf_length,
                      sizeof(c->inbuf) - c->inbuf_length)) <= 0)
        {
            if (r < 0)
    ...
    c->server->poll_api->watch_update(
        watch, (c->outbuf_length > 0 ? AVAHI_WATCH_OUT : 0) |
...
int simple_protocol_setup(const AvahiPoll* poll_api)
{
    ...
    {
        avahi_log_warn("Failed to acquire systemd file descriptors: %s",
                       strerror(-n));
        goto fail;
    }
    ...
    if (n == 1)
    {
        ...
        if ((r = sd_is_socket(SD_LISTEN_FDS_START, AF_LOCAL, SOCK_STREAM, 1)) <
            0)
        {
            avahi_log_warn(
                "Passed systemd file descriptor is of wrong type: %s",
                strerror(-r));
            goto fail;
        }
    ...
    else
    {
        ...
        {
            avahi_log_warn("socket(AF_LOCAL, SOCK_STREAM, 0): %s",
                           strerror(errno));
            goto fail;
        }
...
void simple_protocol_restart_queries(void)
{
    ...
    for (c = server->clients; c; c = c->clients_next)
        ...
        {
            avahi_s_dns_server_browser_free(c->dns_server_browser);
            c->dns_server_browser = avahi_s_dns_server_browser_new(
                avahi_server, AVAHI_IF_UNSPEC, AVAHI_PROTO_UNSPEC, NULL,
                AVAHI_DNS_SERVER_RESOLVE, c->afquery,
                AVAHI_LOOKUP_USE_MULTICAST, dns_server_browser_callback, c);
        }
}
