...
static void button_free(gpointer data)
{
    Button* button = data;
...
static void g_notification_dispose(GObject* object)
{
...
static void g_notification_finalize(GObject* object)
{
    GNotification* notification = G_NOTIFICATION(object);
...
static void g_notification_class_init(GNotificationClass* klass)
{
...
static void g_notification_init(GNotification* notification)
{
...
{
    g_return_val_if_fail(G_IS_NOTIFICATION(notification),
...
void g_notification_set_urgent(GNotification* notification, gboolean urgent)
{
    ...
    notification->priority = urgent ? G_NOTIFICATION_PRIORITY_URGENT
                                    : G_NOTIFICATION_PRIORITY_NORMAL;
}
 ...
 * Since: 2.40
 */
void g_notification_add_button(GNotification* notification, const gchar* label,
                               const gchar* detailed_action)
{
    ...
    if (!g_action_parse_detailed_name(detailed_action, &action, &target,
                                      &error))
    {
...
/**
 * g_notification_add_button_with_target_value: (rename-to
                                                 ...
                                                 GVariant* target)
{
    ...
    if (!g_str_has_prefix(action, "app."))
    {
        g_warning("%s: action '%s' does not start with 'app.'."
                  "This is unlikely to work properly.",
                  G_STRFUNC, action);
    }
 ...
 * g_notification_get_n_buttons().
 */
void g_notification_get_button(GNotification* notification, gint index,
                               gchar** label, gchar** action, GVariant** target)
{
    Button* button;
                                       ...
                                       const gchar* detailed_action)
{
    ...
    g_notification_set_default_action_and_target_value(notification, action,
                                                       target);
...
static GVariant* g_notification_serialize_button(Button* button)
{
    ...
    g_variant_builder_add(&builder, "{sv}", "action",
...
static GVariant* g_notification_get_priority_nick(GNotification* notification)
{
    ...
    value =
        g_enum_get_value(enum_class, g_notification_get_priority(notification));
    g_assert(value != NULL);
...
GVariant* g_notification_serialize(GNotification* notification)
{
    ...
    g_variant_builder_add(&builder, "{sv}", "priority",
                          g_notification_get_priority_nick(notification));

    if (notification->default_action)
    {
        g_variant_builder_add(
            &builder, "{sv}", "default-action",
    ...
    if (notification->buttons->len > 0)
    {
        ...
        for (i = 0; i < notification->buttons->len; i++)
        {
            ...
            g_variant_builder_add(&actions_builder, "@a{sv}",
                                  g_notification_serialize_button(button));
        }
