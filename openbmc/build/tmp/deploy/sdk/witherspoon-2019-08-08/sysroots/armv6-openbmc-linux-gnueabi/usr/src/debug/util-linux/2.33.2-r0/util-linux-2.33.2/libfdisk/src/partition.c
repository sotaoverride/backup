 ...
 * @b: partition
 *
 * Compares partitions according to start offset, See
...
{
    int no_a = FDISK_IS_UNDEF(a->start), no_b = FDISK_IS_UNDEF(b->start);
 ...
 * value when size is undefined (for example after
 * fdisk_partition_unset_size()). Always use fdisk_partition_has_size() to be
 * sure that you work with valid numbers.
 *
 ...
 * value when partno is undefined (for example after
 * fdisk_partition_unset_partno()). Always use fdisk_partition_has_partno() to
 * be sure that you work with valid numbers.
 *
 ...
 * @b: partition
 *
 * Compares partitions according to partition number See
 * fdisk_table_sort_partitions().
 *
 * Return: 0 if the same, <0 if @b greater, >0 if @a greater.
 */
 ...
 *
 * If @pa specified and partno-follow-default (see
 * fdisk_partition_partno_follow_default()) enabled then returns next expected
 * partno or -ERANGE on error.
 *
 ...
 * no free partition available. If fdisk dialogs are disabled then returns
 * -EINVAL.
 *
...
static int probe_partition_content(struct fdisk_context* cxt,
                                   struct fdisk_partition* pa)
{
    int rc = 1; /* nothing */

    DBG(PART, ul_debugobj(pa, "start probe #%zu partition [cxt %p] >>>",
                          pa->partno, cxt));
    ...
    DBG(PART, ul_debugobj(pa, "<<< end probe #%zu partition[cxt %p, rc=%d]",
                          pa->partno, cxt, rc));
    return rc;
}
...
int fdisk_partition_to_string(struct fdisk_partition* pa,
                              struct fdisk_context* cxt, int id, char** data)
{
    char* p = NULL;
    int rc = 0;
    uint64_t x;
    ...
    switch (id)
    {
                ...
                switch (cxt->sizeunit)
                {
                    ...
                    case FDISK_SIZEUNIT_HUMAN:
                        if (fdisk_is_details(cxt))
                            rc = pa->size_post ? asprintf(&p, "%" PRIu64 "%c",
                                                          sz, pa->size_post)
        ...
        case FDISK_FIELD_CYLINDERS:
        {
            ...
            if (sz)
                ...
                rc = asprintf(
                    &p, "%ju",
                    (uintmax_t)(sz / (cxt->geom.heads * cxt->geom.sectors)) +
                        1);
            break;
        }
        case FDISK_FIELD_SECTORS:
            rc = asprintf(&p, "%ju",
                          fdisk_partition_has_size(pa) ? (uintmax_t)pa->size
                                                       : 0);
            break;
...
static struct fdisk_partition* resize_get_by_offset(struct fdisk_table* tb,
 ...
 * partition.
 */
static int resize_get_last_possible(struct fdisk_table* tb,
                                    struct fdisk_partition* cur,
                                    fdisk_sector_t start, fdisk_sector_t* maxsz)
{
    ...
    *maxsz = 0;
    DBG(TAB, ul_debugobj(tb, "checking last possible for start=%ju",
                         (uintmax_t)start));
    ...
    while (fdisk_table_next_partition(tb, &itr, &pa) == 0)
    {
        ...
        DBG(TAB,
            ul_debugobj(tb,
                        " checking entry %p [partno=%zu start=%ju, end=%ju, "
                        "size=%ju%s%s%s]",
                        pa, fdisk_partition_get_partno(pa),
            ...
            pa->parent_partno == cur->partno)
        {
            DBG(TAB,
                ul_debugobj(
                    tb, "  ignore (nested child of the current partition)"));
            continue;
        }

        /* The current is nested, free space has to be nested within the same
         * parent */
        if (fdisk_partition_is_nested(cur) &&
            pa->parent_partno != cur->parent_partno)
        {
        ...
        else
        {
            last = pa;
            *maxsz = pa->size - (start - pa->start);
            DBG(TAB, ul_debugobj(tb, "  new max=%ju (last updated)",
                                 (uintmax_t)*maxsz));
        }
    }
    ...
    if (last)
        ...
        DBG(PART, ul_debugobj(cur, "resize: nothing usable after %ju",
                              (uintmax_t)start));
...
/*
 * Uses template @tpl to recount start and size change of the partition @res.
 * The
 * @tpl->size and @tpl->start are interpreted as relative to the current
 * setting.
 */
static int recount_resize(struct fdisk_context* cxt, size_t partno,
                          ...
                          struct fdisk_partition* tpl)
{
    ...
    DBG(PART,
        ul_debugobj(tpl, "resize partition partno=%zu in table:", partno));
    ...
    if (tpl->movestart && fdisk_partition_has_start(tpl))
    {
        ...
        DBG(PART,
            ul_debugobj(tpl, "resize: moving start %s relative, new start: %ju",
                        tpl->movestart == FDISK_MOVE_DOWN ? "DOWN" : "UP",
                        (uintmax_t)start));
    ...
    else if (fdisk_partition_has_start(tpl))
    {
        start = fdisk_partition_get_start(tpl);
        DBG(PART,
            ul_debugobj(tpl, "resize: moving start to absolute offset: %ju",
                        (uintmax_t)start));
    }

    /* 2) verify that start is within the current partition or any freespace
     * area */
    if (!FDISK_IS_UNDEF(start))
    {
        ...
        else if (!area && start >= cxt->first_lba &&
                 start < cxt->first_lba + (cxt->grain / cxt->sector_size))
            DBG(PART, ul_debugobj(
                          tpl, "resize: start points before first partition"));
        else
        {
    ...
    DBG(PART,
        ul_debugobj(tpl, "<<< resize: SUCCESS: start %ju->%ju; size %ju->%ju",
                        ...
                        struct fdisk_partition* pa)
{
    ...
    if (pa->resize || fdisk_partition_has_start(pa) ||
    ...
    DBG(CXT, ul_debugobj(
                 cxt, "setting partition %zu %p (start=%ju, end=%ju, size=%ju)",
                 partno, xpa, (uintmax_t)fdisk_partition_get_start(xpa),
 ...
 * @enable: 0 or 1
 *
 * Enable/disable filesystems/RAIDs wiping in area defined by partition start
 * and size.
 *
 ...
 * Returns: 0 on success, <0 on error.
 */
int fdisk_add_partition(struct fdisk_context* cxt, struct fdisk_partition* pa,
                        size_t* partno)
{
    int rc;
    ...
    if (pa)
    {
        ...
        if (fdisk_partition_has_start(pa))
            DBG(CXT, ul_debugobj(cxt, "     start: %ju",
                                 (uintmax_t)fdisk_partition_get_start(pa)));
        if (fdisk_partition_has_end(pa))
            DBG(CXT, ul_debugobj(cxt, "       end: %ju",
                                 (uintmax_t)fdisk_partition_get_end(pa)));
        if (fdisk_partition_has_size(pa))
            DBG(CXT, ul_debugobj(cxt, "      size: %ju",
                                 (uintmax_t)fdisk_partition_get_size(pa)));
