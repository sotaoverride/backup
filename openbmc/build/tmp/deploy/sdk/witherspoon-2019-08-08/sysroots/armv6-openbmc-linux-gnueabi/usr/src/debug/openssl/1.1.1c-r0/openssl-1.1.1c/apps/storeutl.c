...
const OPTIONS storeutl_options[] = {
    ...
    {"issuer", OPT_CRITERION_ISSUER, 's',
     "Search by issuer and serial, issuer name"},
    {"serial", OPT_CRITERION_SERIAL, 's',
     "Search by issuer and serial, serial number"},
    {"fingerprint", OPT_CRITERION_FINGERPRINT, 's',
...
int storeutl_main(int argc, char* argv[])
{
        ...
        switch (o)
        {
            ...
            case OPT_CRITERION_FINGERPRINT:
                ...
                }
                {
                    ...
                    if ((fingerprint =
                             OPENSSL_hexstr2buf(opt_arg(), &tmplen)) == NULL)
                    {
                        BIO_printf(bio_err,
    ...
    if (criterion != 0)
    {
        switch (criterion)
        {
            ...
            case OSSL_STORE_SEARCH_BY_ISSUER_SERIAL:
                ...
                if ((search = OSSL_STORE_SEARCH_by_issuer_serial(
                         issuer, serial)) == NULL)
                {
                    ERR_print_errors(bio_err);
                    goto end;
            ...
            case OSSL_STORE_SEARCH_BY_KEY_FINGERPRINT:
                if ((search = OSSL_STORE_SEARCH_by_key_fingerprint(
                         digest, fingerprint, fingerprintlen)) == NULL)
                {
                    ERR_print_errors(bio_err);
                    goto end;
                }
    ...
    ret = process(argv[0], get_ui_method(), &pw_cb_data, expected, criterion,
                  search, text, noout, recursive, 0, out, prog);
                   ...
                   const char* prog)
{
    ...
    if ((store_ctx = OSSL_STORE_open(uri, uimeth, uidata, NULL, NULL)) == NULL)
    {
    ...
    if (criterion != 0)
    {
        ...
        {
            BIO_printf(bio_err,
                       "%s: the store scheme doesn't support the given search "
                       "criteria.\n",
                       prog);
            goto end2;
        }
        ...
        switch (type)
        {
            case OSSL_STORE_INFO_NAME:
                if (recursive)
                {
                    ...
                    ret += process(suburi, uimeth, uidata, expected, criterion,
                                   search, text, noout, recursive, indent + 2,
                                   out, prog);
                }
                break;
            case OSSL_STORE_INFO_PARAMS:
                if (text)
                    EVP_PKEY_print_params(
                        out, OSSL_STORE_INFO_get0_PARAMS(info), 0, NULL);
                if (!noout)
