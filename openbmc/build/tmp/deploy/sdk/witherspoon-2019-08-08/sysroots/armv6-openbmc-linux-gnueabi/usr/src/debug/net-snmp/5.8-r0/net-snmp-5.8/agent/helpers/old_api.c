...
struct variable* netsnmp_duplicate_variable(const struct variable* var)
{
    ...
    if (var)
    {
        const int varsize = offsetof(struct variable, name) +
                            var->namelen * sizeof(var->name[0]);
        var2 = malloc(varsize);
        if (var2)
 ...
 */
int netsnmp_register_old_api(const char* moduleName, const struct variable* var,
                             ...
                             const char* context, int timeout, int flags)
{
    ...
    for (i = 0; i < numvars; i++)
    {
        ...
        vp = netsnmp_duplicate_variable(
            (const struct variable*)((const char*)var + varsize * i));
        ...
        memcpy(reginfo->rootoid + mibloclen, vp->name,
...
int netsnmp_register_mib_table_row(const char* moduleName,
                                   const struct variable* var, size_t varsize,
                                   size_t numvars, oid* mibloc,
                                   size_t mibloclen, int priority,
                                   int var_subid, netsnmp_session* ss,
                                   const char* context, int timeout, int flags)
{
    ...
    for (i = 0; i < numvars; i++)
    {
        ...
        if (r->handler == NULL || r->handlerName == NULL || r->rootoid == NULL)
        {
        ...
        DEBUGMSGOID(
            ("netsnmp_register_mib_table_row", r->rootoid, r->rootoid_len));
        ...
        if ((rc = netsnmp_register_handler_nocallback(r)) != MIB_REGISTERED_OK)
        {
            snmp_log(LOG_ERR, "mib table row registration failed\n");
            DEBUGMSGTL(
                ("netsnmp_register_mib_table_row", "register failed %d\n", rc));
        ...
        switch (reqinfo->mode)
        {
            ...
            case MODE_SET_RESERVE1:
                ...
                if (vp && vp->findVar)
                {
                    ...
                    access = (*(vp->findVar))(cvp, tmp_name, &tmp_len, exact,
                                              &len, &write_method);
...
#endif /* !NETSNMP_NO_WRITE_SUPPORT */
                        snmp_set_var_typed_value(requests->requestvb, cvp->type,
                                                 access, len);
                }
                else
                {
                ...
                netsnmp_request_add_list_data(
                    requests, netsnmp_create_data_list(OLD_API_NAME, cacheptr,
                                                       &free_wrapper));
                /*
            ...
            default:
                 ...
                 */
                cacheptr =
                    (netsnmp_old_api_cache*)netsnmp_request_get_list_data(
                        requests, OLD_API_NAME);
