...
#define MAX_TZYEAR                                                             \
    2999 /* And it's not likely ever to go away, but                           \
...
again:
    ...
    if (ref_count == 1)
    {
        if (tz->name != NULL)
        {
            ...
            if
                G_UNLIKELY(g_atomic_int_get(&tz->ref_count) != 1)
                {
                    G_UNLOCK(time_zones);
                    goto again;
                }
        ...
        if (tz->t_info != NULL)
        {
            ...
            {
                TransitionInfo* info =
                    &g_array_index(tz->t_info, TransitionInfo, idx);
                g_free(info->abbrev);
            }
        ...
        G_UNLIKELY(!g_atomic_int_compare_and_exchange(&tz->ref_count, ref_count,
                                                      ref_count - 1))
    goto again;
}
...
static void zone_for_constant_offset(GTimeZone* gtz, const gchar* name)
{
    gint32 offset;
    TransitionInfo info;
...
#ifdef G_OS_UNIX
static GBytes* zone_info_unix(const gchar* identifier, gchar** out_identifier)
{
    gchar* filename;
    ...
    else
    {
        ...
        {
            gboolean not_a_symlink = g_error_matches(
                read_link_err, G_FILE_ERROR, G_FILE_ERROR_INVAL);
            ...
            if (not_a_symlink &&
                (g_file_get_contents("/var/db/zoneinfo", &resolved_identifier,
                                     NULL, NULL) ||
                 g_file_get_contents("/etc/timezone", &resolved_identifier,
                                     NULL, NULL)))
        ...
        else
        {
            ...
            canonical_path =
                g_canonicalize_filename(resolved_identifier, "/etc");
        ...
        if (prefix_len > 0)
            memmove(resolved_identifier, resolved_identifier + prefix_len,
                    strlen(resolved_identifier) - prefix_len +
                        1 /* nul terminator */);
    ...
    if (file != NULL)
    {
        zoneinfo = g_bytes_new_with_free_func(
            ...
            (GDestroyNotify)g_mapped_file_unref, g_mapped_file_ref(file));
...
static void init_zone_from_iana_info(GTimeZone* gtz, GBytes* zoneinfo,
                                     gchar* identifier /* (transfer full) */)
{
    ...
    if (header->tzh_version == '2')
    {
        ...
        header =
            (const struct tzhead*)(((const gchar*)(header + 1)) +
    ...
    gtz->t_info =
        g_array_sized_new(FALSE, TRUE, sizeof(TransitionInfo), type_count);
    gtz->transitions =
        g_array_sized_new(FALSE, TRUE, sizeof(Transition), time_count);
...
static void copy_windows_systemtime(SYSTEMTIME* s_time, TimeZoneDate* tzdate)
{
...
static void rule_from_windows_time_zone_info(TimeZoneRule* rule,
...
static gchar* windows_default_tzname(void)
{
    const gchar* subkey =
        "SYSTEM\\CurrentControlSet\\Control\\TimeZoneInformation";
    HKEY key;
    gchar* key_name = NULL;
    if (RegOpenKeyExA(HKEY_LOCAL_MACHINE, subkey, 0, KEY_QUERY_VALUE, &key) ==
        ERROR_SUCCESS)
    {
        DWORD size = 0;
        if (RegQueryValueExA(key, "TimeZoneKeyName", NULL, NULL, NULL, &size) ==
            ERROR_SUCCESS)
        {
...
static void register_tzi_to_tzi(RegTZI* reg, TIME_ZONE_INFORMATION* tzi)
{
...
static gint rules_from_windows_time_zone(const gchar* identifier,
                                         ...
                                         TimeZoneRule** rules)
{
    ...
    *rules = NULL;
    ...
    if (RegQueryValueExA(key, "Std", NULL, NULL, (LPBYTE) & (tzi.StandardName),
                         &size) != ERROR_SUCCESS)
        goto failed;

    size = sizeof tzi.DaylightName;

    if (RegQueryValueExA(key, "Dlt", NULL, NULL, (LPBYTE) & (tzi.DaylightName),
                         &size) != ERROR_SUCCESS)
        goto failed;

    RegCloseKey(key);
    if (RegOpenKeyExA(HKEY_LOCAL_MACHINE, subkey_dynamic, 0, KEY_QUERY_VALUE,
                      &key) == ERROR_SUCCESS)
    {
        DWORD first, last;
        int year, i;
        ...
        size = sizeof first;
        if (RegQueryValueExA(key, "FirstEntry", NULL, NULL, (LPBYTE)&first,
                             &size) != ERROR_SUCCESS)
            goto failed;

        size = sizeof last;
        if (RegQueryValueExA(key, "LastEntry", NULL, NULL, (LPBYTE)&last,
                             &size) != ERROR_SUCCESS)
            goto failed;
        ...
        *rules = g_new0(TimeZoneRule, rules_num);
        ...
        for (year = first, i = 0; year <= last; year++)
        {
            ...
            size = sizeof regtzi;
            if (RegQueryValueExA(key, s, NULL, NULL, (LPBYTE)&regtzi, &size) !=
                ERROR_SUCCESS)
            {
                g_free(*rules);
                *rules = NULL;
            ...
            if (year > first &&
                memcmp(&regtzi_prev, &regtzi, sizeof regtzi) == 0)
                continue;
            else
    ...
    else if (RegOpenKeyExA(HKEY_LOCAL_MACHINE, subkey, 0, KEY_QUERY_VALUE,
                           &key) == ERROR_SUCCESS)
    {
        size = sizeof regtzi;
        if (RegQueryValueExA(key, "TZI", NULL, NULL, (LPBYTE)&regtzi, &size) ==
            ERROR_SUCCESS)
        {
            rules_num = 2;
    ...
    if (*rules)
    {
        ...
        if ((*rules)[rules_num - 2].start_year < MAX_TZYEAR)
            ...
            (*rules)[rules_num - 1].start_year =
                (*rules)[rules_num - 2].start_year + 1;
...
static void find_relative_date(TimeZoneDate* buffer)
{
    ...
    if (buffer->mon == 13 || buffer->mon == 14) /* Julian Date */
    {
        ...
        if (wday >= 59 && buffer->mon == 13 &&
            g_date_is_leap_year(buffer->year))
...
static gint64 boundary_for_year(TimeZoneDate* boundary, gint year,
                                gint32 offset)
{
    TimeZoneDate buffer;
    GDate date;
...
static void fill_transition_info_from_rule(TransitionInfo* info,
                                           TimeZoneRule* rule, gboolean is_dst)
{
    ...
    if (name)
        ...
        info->abbrev = g_strdup_printf("%+03d%02d", (int)offset / 3600,
                                       (int)abs(offset / 60) % 60);
}

static void init_zone_from_rules(GTimeZone* gtz, TimeZoneRule* rules,
                                 gint rules_num,
                                 gchar* identifier /* (transfer full) */)
{
    ...
    gtz->t_info =
        g_array_sized_new(FALSE, TRUE, sizeof(TransitionInfo), type_count);
    gtz->transitions =
        g_array_sized_new(FALSE, TRUE, sizeof(Transition), trans_count);
    ...
    for (ri = 0; ri < rules_num - 1; ri++)
    {
            ...
            rules[ri].dlt_start.mon == 0 && rules[ri].dlt_end.mon == 0)
        {
                 ...
                 rules[ri - 1].dlt_start.mon > rules[ri - 1].dlt_end.mon))
            {
                ...
                gint64 std_time =
                    boundary_for_year(&rules[ri].dlt_end, year, last_offset);
        ...
        else
        {
            ...
            {
                gint32 dlt_offset =
                    (dlt_first ? last_offset : rules[ri].dlt_offset);
                gint32 std_offset =
                    (dlt_first ? rules[ri].std_offset : last_offset);
                /* NB: boundary_for_year returns 0 if mon == 0 */
                gint64 std_time =
                    boundary_for_year(&rules[ri].dlt_end, year, dlt_offset);
                gint64 dlt_time =
                    boundary_for_year(&rules[ri].dlt_start, year, std_offset);
                Transition std_trans = {std_time, info_index};
                Transition dlt_trans = {dlt_time, info_index + 1};
                last_offset =
                    (dlt_first ? rules[ri].dlt_offset : rules[ri].std_offset);
                if (dlt_first)
                {
         ...
         rules[ri - 1].dlt_start.mon > rules[ri - 1].dlt_end.mon))
    {
        ...
        g_array_append_val(gtz->t_info, info);
        trans.time =
            boundary_for_year(&rules[ri - 1].dlt_end, year, last_offset);
 ...
 *  - ss is 00 to 59
 */
static gboolean parse_mwd_boundary(gchar** pos, TimeZoneDate* boundary)
{
    gint month, week, day;
...
static gboolean parse_julian_boundary(gchar** pos, TimeZoneDate* boundary,
...
static gboolean parse_tz_boundary(const gchar* identifier,
...
static gint create_ruleset_from_rule(TimeZoneRule** rules, TimeZoneRule* rule)
{
    *rules = g_new0(TimeZoneRule, 2);
...
static gboolean parse_offset(gchar** pos, gint32* target)
{
    gchar* buffer;
...
static gboolean parse_identifier_boundary(gchar** pos, TimeZoneDate* target)
{
    gchar* buffer;
    gchar* target_pos = *pos;
    gboolean ret;
...
static gboolean set_tz_name(gchar** pos, gchar* buffer, guint size)
{
    gchar* name_pos = *pos;
    guint len;
...
static gboolean parse_identifier_boundaries(gchar** pos, TimeZoneRule* tzr)
{
    if (*(*pos)++ != ',')
        return FALSE;
 ...
 */
static gint rules_from_identifier(const gchar* identifier,
                                  gchar** out_identifier, TimeZoneRule** rules)
{
    gchar* pos;
    TimeZoneRule tzr;
 ...
 * [The GNU C Library
 *manual](http://www.gnu.org/s/libc/manual/html_node/TZ-Variable.html) for an
 *explanation of the possible values of the `TZ` environment variable. See
...
GTimeZone* g_time_zone_new(const gchar* identifier)
{
    ...
    if (tz->t_info == NULL && (rules_num = rules_from_identifier(
                                   identifier, &resolved_identifier, &rules)))
    {
        init_zone_from_rules(tz, rules, rules_num,
                             g_steal_pointer(&resolved_identifier));
        g_free(rules);
    }
...
#elif defined(G_OS_WIN32)
        if ((rules_num = rules_from_windows_time_zone(
                 identifier, &resolved_identifier, &rules)))
        {
            init_zone_from_rules(tz, rules, rules_num,
                                 g_steal_pointer(&resolved_identifier));
...
GTimeZone* g_time_zone_new_offset(gint32 seconds)
{
    ...
    identifier = g_strdup_printf("%c%02u:%02u:%02u", (seconds >= 0) ? '+' : '-',
                                 (ABS(seconds) / 60) / 60,
                                 (ABS(seconds) / 60) % 60, ABS(seconds) % 60);
 ...
 */
inline static const TransitionInfo* interval_info(GTimeZone* tz, guint interval)
{
    guint index;
...
inline static gint64 interval_start(GTimeZone* tz, guint interval)
{
...
inline static gint64 interval_end(GTimeZone* tz, guint interval)
{
...
inline static gint32 interval_offset(GTimeZone* tz, guint interval)
{
    g_return_val_if_fail(tz->t_info != NULL, 0);
    return interval_info(tz, interval)->gmt_offset;
}

inline static gboolean interval_isdst(GTimeZone* tz, guint interval)
{
    g_return_val_if_fail(tz->t_info != NULL, 0);
    return interval_info(tz, interval)->is_dst;
}

inline static gchar* interval_abbrev(GTimeZone* tz, guint interval)
{
    g_return_val_if_fail(tz->t_info != NULL, 0);
    return interval_info(tz, interval)->abbrev;
}

inline static gint64 interval_local_start(GTimeZone* tz, guint interval)
{
    if (interval)
...
inline static gint64 interval_local_end(GTimeZone* tz, guint interval)
{
    if (tz->transitions && interval < tz->transitions->len)
...
static gboolean interval_valid(GTimeZone* tz, guint interval)
{
    if (tz->transitions == NULL)
        return interval == 0;
