...
struct zlib_params
{
    ...
    zlib_params(int level_ = zlib::default_compression,
        ...
        level(level_),
        method(method_), window_bits(window_bits_), mem_level(mem_level_),
        strategy(strategy_), noheader(noheader_), calculate_crc(calculate_crc_)
    {
    }
    int level;
    int method;
...
#else
    typedef typename std::allocator_traits<Alloc>::template rebind_alloc<char>
        type;
#endif
#else
...
struct zlib_allocator : private Base
{
  ...
  public:
    BOOST_STATIC_CONSTANT(
        bool, custom = (!is_same<std::allocator<char>, Base>::value));
...
class BOOST_IOSTREAMS_DECL zlib_base
{
  ...
  protected:
    ...
    template <typename Alloc>
    void init(const zlib_params& p, bool compress,
              zlib_allocator<Alloc>& zalloc)
    {
        bool custom = zlib_allocator<Alloc>::custom;
        do_init(p, compress, custom ? zlib_allocator<Alloc>::allocate : 0,
  ...
  private:
    void do_init(const zlib_params& p, bool compress, zlib::xalloc_func,
...
class zlib_decompressor_impl : public zlib_base, public zlib_allocator<Alloc>
{
  public:
    ...
    bool filter(const char*& begin_in, const char* end_in, char*& begin_out,
    ...
    : symmetric_filter<detail::zlib_compressor_impl<Alloc>, Alloc>
{
  ...
  public:
    ...
    basic_zlib_compressor(
        const zlib_params& = zlib::default_compression,
...
void* zlib_allocator<Alloc, Base>::allocate(void* self, zlib::uint items,
                                            zlib::uint size)
{
...
template <typename Alloc>
bool zlib_compressor_impl<Alloc>::filter(const char*& src_begin,
                                         const char* src_end, char*& dest_begin,
                                         char* dest_end, bool flush)
{
...
template <typename Alloc>
bool zlib_decompressor_impl<Alloc>::filter(const char*& src_begin,
                                           const char* src_end,
