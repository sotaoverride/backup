...
enum
{
    pid_size = sizeof(GetCurrentProcessId())
};
    ...
    operator<<(std::basic_ostream<CharT, TraitsT>& strm, process::id const& pid)
{
    if (strm.good())
    {
        CharT buf[pid_size * 2 + 3]; // 2 chars per byte + 3 chars for the
                                     // leading 0x and terminating zero
        format_id<pid_size>(buf, sizeof(buf) / sizeof(*buf), pid.native_id(),
