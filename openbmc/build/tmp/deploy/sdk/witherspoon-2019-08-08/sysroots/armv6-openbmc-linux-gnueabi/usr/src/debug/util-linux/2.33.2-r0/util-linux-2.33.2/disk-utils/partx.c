...
static struct colinfo infos[] = {
    ...
    [COL_START] = {"START", 0.30, SCOLS_FL_RIGHT,
                   N_("start of the partition in sectors")},
    [COL_END] = {"END", 0.30, SCOLS_FL_RIGHT,
...
static void assoc_loopdev(const char* fname)
{
    ...
    if (rc)
        err(EXIT_FAILURE, _("%s: failed to find unused loop device"), fname);
...
static int get_max_partno(const char* disk, dev_t devno)
{
    ...
    snprintf(path, sizeof(path), _PATH_SYS_DEVBLOCK "/%d:%d/", major(devno),
             minor(devno));
...
static void del_parts_warnx(const char* device, int first, int last)
{
    if (first == last)
        ...
        warnx(_("%s: error deleting partitions %d-%d"), device, first, last);
}

static int del_parts(int fd, const char* device, dev_t devno, int lower,
                     int upper)
{
...
static void add_parts_warnx(const char* device, int first, int last)
{
    if (first == last)
        ...
        warnx(_("%s: error adding partitions %d-%d"), device, first, last);
}

static int add_parts(int fd, const char* device, blkid_partlist ls, int lower,
                     int upper)
{
...
static void upd_parts_warnx(const char* device, int first, int last)
{
    if (first == last)
        ...
        warnx(_("%s: error updating partitions %d-%d"), device, first, last);
}

static int upd_parts(int fd, const char* device, dev_t devno, blkid_partlist ls,
                     int lower, int upper)
{
...
static int list_parts(blkid_partlist ls, int lower, int upper)
{
    ...
    for (i = 0; i < nparts; i++)
    {
        ...
        printf(P_("#%2d: %9ju-%9ju (%9ju sector, %6ju MB)\n",
               ...
               n, start, start + size - 1, size, (size << 9) / 1000000);
    }
    return 0;
}
        ...
        switch (get_column_id(i))
        {
            ...
            case COL_SIZE:
                if (partx_flags & FL_BYTES)
                    xasprintf(&str, "%ju",
                              (uintmax_t)blkid_partition_get_size(par) << 9);
                else
            ...
            case COL_TYPE:
                if (blkid_partition_get_type_string(par))
                    ...
                    xasprintf(&str, "0x%x", blkid_partition_get_type(par));
                break;
            case COL_FLAGS:
...
static blkid_partlist get_partlist(blkid_probe pr, const char* device,
                                   char* type)
{
    blkid_partlist ls;
    blkid_parttable tab;
    ...
    if (type)
    {
        ...
        if (blkid_probe_filter_partitions_type(pr, BLKID_FLTR_ONLYIN, name))
        {
...
static void __attribute__((__noreturn__)) usage(void)
{
    ...
    fprintf(out, _(" %s [-a|-d|-s|-u] [--nr <n:m> | <partition>] <disk>\n"),
            program_invocation_short_name);

    fputs(USAGE_SEPARATOR, out);
    fputs(
        _("Tell the kernel about the presence and numbering of partitions.\n"),
        out);
    ...
    fputs(_(" -a, --add            add specified partitions or all of them\n"),
          out);
    fputs(
        _(" -d, --delete         delete specified partitions or all of them\n"),
        out);
    fputs(
        _(" -u, --update         update specified partitions or all of them\n"),
        out);
    ...
    fputs(_(" -n, --nr <n:m>       specify the range of partitions (e.g. --nr "
            "2:4)\n"),
          out);
...
int main(int argc, char** argv)
{
    ...
    while ((c = getopt_long(argc, argv, "abdglrsuvn:t:o:PS:hV", long_opts,
                            NULL)) != -1)
    {
    ...
    if (what == ACT_SHOW && outarg &&
        string_add_to_idarray(outarg, columns, ARRAY_SIZE(columns), &ncolumns,
                              column_name_to_id) < 0)
        return EXIT_FAILURE;
    ...
    else if (optind == argc - 1)
    {
        ...
        if (blkid_devno_to_wholedisk(part_devno, NULL, 0, &disk_devno) == 0 &&
    ...
    if (what == ACT_ADD || what == ACT_DELETE)
    {
        ...
        if (S_ISREG(x.st_mode))
        {
            ...
            if (!loopmod_supports_partscan())
                errx(EXIT_FAILURE,
                     _("%s: partitioned loop devices unsupported"), wholedisk);
    ...
    else
    {
        ...
        if (!pr || blkid_probe_set_device(pr, fd, 0, 0))
            warnx(_("%s: failed to initialize blkid prober"), wholedisk);
        else
        {
            if (sector_size)
