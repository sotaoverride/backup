...
static int process_input(FILE* input, const char* inname, int to_lowercase,
                         int be_quiet);
    ...
    if (do_undo)
    {
        ...
        if (fd == -1)
            error(EXIT_FAILURE, errno,
                  gettext("cannot open database file `%s'"), input_name);
                         ...
                         int be_quiet)
{
    ...
    while (!feof_unlocked(input))
    {
        ...
        *cp++ = '\0';
        ...
        /* Store the data.  */
        struct valstrentry* nentry =
            xmalloc(sizeof(struct valstrentry) + datalen);
        ...
        struct valstrentry** fdata =
            tsearch(nentry, &valstrtree, valstr_compare);
        if (fdata == NULL)
        ...
        struct dbentry** found =
            tsearch(newp, &last_database->entries, dbentry_compare);
        if (found == NULL)
            error(EXIT_FAILURE, errno, gettext("cannot create search tree"));
...
#define TEST_RANGE 30
            size_t nhashentries_min = next_prime(
                db->nentries < TEST_RANGE ? db->nentries
            ...
            void add_key(const void* nodep, const VISIT which, const int depth)
            {
                ...
                db->hashtable[hidx] =
                    ((db->extra_string ? valstrlen : 0) + dbe->validx);
...
static int write_output(int fd)
{
    ...
    uint64_t file_offset =
        (sizeof(struct nss_db_header) + (ndatabases * sizeof(header->dbs[0])));
...
static int print_database(int fd)
{
    ...
    const struct nss_db_header* header =
        mmap(NULL, st.st_size, PROT_READ, MAP_PRIVATE | MAP_POPULATE, fd, 0);
    ...
    for (unsigned int dbidx = 0; dbidx < header->ndbs; ++dbidx)
    {
        ...
        for (uint32_t hidx = 0; hidx < header->dbs[dbidx].hashsize; ++hidx)
            if (hashtab[hidx] != ~((stridx_t)0))
                printf("%c%s %s\n", header->dbs[dbidx].id,
                       keystrtab + stridxtab[hidx], valstrtab + hashtab[hidx]);
    }
