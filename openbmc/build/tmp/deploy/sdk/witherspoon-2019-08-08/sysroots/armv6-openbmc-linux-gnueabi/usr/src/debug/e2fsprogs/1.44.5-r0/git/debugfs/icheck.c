...
static int icheck_proc(ext2_filsys fs EXT2FS_ATTR((unused)), blk64_t* block_nr,
...
void do_icheck(int argc, char** argv)
{
    ...
    if (!bw.barray)
    {
        com_err("icheck", ENOMEM, "while allocating inode info array");
        return;
    }
    ...
    while (ino)
    {
        ...
        retval = ext2fs_block_iterate3(current_fs, ino, BLOCK_FLAG_READ_ONLY,
                                       block_buf, icheck_proc, &bw);
        if (retval)
        {
            com_err("icheck", retval, "while calling ext2fs_block_iterate");
            goto next;
        }
    ...
    next:
        ...
        if (retval)
        {
            com_err("icheck", retval, "while doing inode scan");
            goto error_out;
        }
    }
