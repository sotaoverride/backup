...
class BOOST_SYMBOL_VISIBLE date_facet : public std::locale::facet
{
  public:
    ...
    explicit date_facet(::size_t a_ref = 0) :
        ...
        m_format(default_date_format), m_month_format(short_month_format),
    ...
    explicit date_facet(const char_type* format_str,
        ...
        m_format(format_str), m_month_format(short_month_format),
        m_weekday_format(short_weekday_format), m_month_short_names(short_names)
    {
    }
    ...
    explicit date_facet(
        ...
        special_values_formatter_type sv_formatter =
            special_values_formatter_type(),
        ...
        m_format(format_str), m_month_format(short_month_format),
        m_weekday_format(short_weekday_format),
        m_period_formatter(per_formatter), m_date_gen_formatter(dg_formatter),
    ...
    void date_gen_phrase_strings(
        const input_collection_type& new_strings,
                ...
                const date_type& d) const
    {
        ...
        // The following line of code required the date to support a to_tm
        // function
                ...
                const duration_type& dd) const
    {
        if (dd.is_special())
        {
            return do_put_special(next, a_ios, fill_char,
                                  dd.get_rep().as_special());
        }

        typedef std::num_put<CharT, OutItrT> num_put;
        if (std::has_facet<num_put>(a_ios.getloc()))
        {
            return std::use_facet<num_put>(a_ios.getloc())
                .put(next, a_ios, fill_char, dd.get_rep().as_number());
        }
        else
        {
    ...
    {
        return m_date_gen_formatter.put_partial_date(next, a_ios, fill_char, pd,
                                                     *this);
    }
    ...
    {
        return m_date_gen_formatter.put_nth_kday(next, a_ios, fill_char, nkd,
                                                 *this);
    }
    ...
    {
        return m_date_gen_formatter.put_first_kday(next, a_ios, fill_char, fkd,
                                                   *this);
    }
    ...
    {
        return m_date_gen_formatter.put_last_kday(next, a_ios, fill_char, lkd,
                                                  *this);
    }
    ...
    {
        return m_date_gen_formatter.put_kday_before(next, a_ios, fill_char, fkb,
                                                    *this);
    }
    ...
    {
        return m_date_gen_formatter.put_kday_after(next, a_ios, fill_char, fka,
                                                   *this);
    }

  protected:
    virtual OutItrT
        do_put_special(OutItrT next, std::ios_base& /*a_ios*/,
                       char_type /*fill_char*/,
    ...
    virtual OutItrT do_put_tm(OutItrT next, std::ios_base& a_ios,
                              char_type fill_char, const tm& tm_value,
...
class BOOST_SYMBOL_VISIBLE date_input_facet : public std::locale::facet
{
  public:
    ...
    explicit date_input_facet(::size_t a_ref = 0) :
        std::locale::facet(a_ref), m_format(default_date_format),
    ...
    explicit date_input_facet(const string_type& format_str,
        ...
        m_format(format_str), m_month_format(short_month_format),
        m_weekday_format(short_weekday_format),
        m_year_format(four_digit_year_format), m_parser(date_parser),
        m_date_gen_parser(date_gen_parser), m_period_parser(per_parser),
    ...
    void date_gen_element_strings(
        const string_type& first, const string_type& second,
        const string_type& third, const string_type& fourth,
        const string_type& fifth, const string_type& last,
        const string_type& before, const string_type& after,
        const string_type& of)

    {
        m_date_gen_parser.element_strings(first, second, third, fourth, fifth,
                                          last, before, after, of);
    }
               ...
               duration_type& dd) const
    {
        ...
        {
            from = std::use_facet<num_get>(a_ios.getloc())
                       .get(from, to, a_ios, err, val);
        }
        else
        {
        ...
        {
            typedef typename special_values_parser_type::match_results
                match_results;
            match_results mr;
            if (c == '-' || c == '+')
            ...
            if (mr.current_match == match_results::PARSE_ERROR)
            {
                boost::throw_exception(std::ios_base::failure(
                    "Parse failed. No match found for '" + mr.cache + "'"));
                BOOST_DATE_TIME_UNREACHABLE_EXPRESSION(
                    return from); // should never reach
            }
    ...
    InItrT get(InItrT& from, InItrT& to, std::ios_base& a_ios,
               period_type& p) const
    {
        p = m_period_parser.get_period(from, to, a_ios, p,
                                       duration_type::unit(), *this);
        return from;
    }
    InItrT get(InItrT& from, InItrT& to, std::ios_base& a_ios,
...
const typename date_input_facet<date_type, CharT, OutItrT>::char_type
    date_input_facet<date_type, CharT, OutItrT>::long_weekday_format[3] = {'%',
                                                                           'A'};

template <class date_type, class CharT, class OutItrT>
const typename date_input_facet<date_type, CharT, OutItrT>::char_type
    date_input_facet<date_type, CharT, OutItrT>::short_weekday_format[3] = {
        '%', 'a'};

template <class date_type, class CharT, class OutItrT>
const typename date_input_facet<date_type, CharT, OutItrT>::char_type
    date_input_facet<date_type, CharT, OutItrT>::long_month_format[3] = {'%',
                                                                         'B'};

template <class date_type, class CharT, class OutItrT>
const typename date_input_facet<date_type, CharT, OutItrT>::char_type
    date_input_facet<date_type, CharT, OutItrT>::short_month_format[3] = {'%',
                                                                          'b'};

template <class date_type, class CharT, class OutItrT>
const typename date_input_facet<date_type, CharT, OutItrT>::char_type
    date_input_facet<date_type, CharT, OutItrT>::four_digit_year_format[3] = {
        '%', 'Y'};

template <class date_type, class CharT, class OutItrT>
const typename date_input_facet<date_type, CharT, OutItrT>::char_type
    date_input_facet<date_type, CharT, OutItrT>::two_digit_year_format[3] = {
        '%', 'y'};
...
const typename date_input_facet<date_type, CharT, OutItrT>::char_type
    date_input_facet<date_type, CharT,
                     OutItrT>::iso_format_extended_specifier[9] = {
        '%', 'Y', '-', '%', 'm', '-', '%', 'd'};
