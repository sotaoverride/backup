                                      ...
                                      const DBusString* address)
{
    ...
    _dbus_counter_set_notify(
        transport->live_messages, transport->max_live_messages_size,
        transport->max_live_messages_unix_fds, live_messages_notify, transport);
 ...
 * @returns a new transport, or #NULL on failure.
 */
static DBusTransport* _dbus_transport_new_for_autolaunch(const char* scope,
                                                         DBusError* error)
{
    DBusString address;
...
static DBusTransportOpenResult _dbus_transport_open_autolaunch(
    DBusAddressEntry* entry, DBusTransport** transport_p, DBusError* error)
{
    const char* method;
...
static const struct
{
    DBusTransportOpenResult (*func)(DBusAddressEntry* entry,
                                    ...
                                    DBusError* error);
} open_funcs[] = {{_dbus_transport_open_socket},
...
out:
    ...
    if (transport == NULL)
    {
        if (!dbus_error_is_set(&tmp_error))
            _dbus_set_bad_address(&tmp_error, NULL, NULL,
                                  "Unknown address type (examples of valid "
...
static dbus_bool_t auth_via_windows_user_function(DBusTransport* transport)
{
    ...
    windows_sid =
        _dbus_strdup(_dbus_credentials_get_windows_sid(auth_identity));
    ...
    allow =
        (*windows_user_function)(connection, windows_sid, windows_user_data);
...
static dbus_bool_t auth_via_default_rules(DBusTransport* transport)
{
    ...
    {
        if (_dbus_credentials_include(our_identity,
...
dbus_bool_t _dbus_transport_try_to_authenticate(DBusTransport* transport)
{
    ...
    else
    {
        ...
        maybe_authenticated = (!(transport->send_credentials_pending ||
        ...
        if (maybe_authenticated && !transport->is_server)
        {
            ...
            {
                _dbus_verbose("Client expected GUID '%s' and we got '%s' from "
                              "the server\n",
        ...
        /* If we're the server, see if we want to allow this identity to
         * proceed.
         */
        if (maybe_authenticated && transport->is_server)
        {
            ...
            if (transport->unix_user_function != NULL &&
                _dbus_credentials_include(auth_identity,
                                         ...
                                         unsigned int condition)
{
    ...
    {
        _dbus_warn_check_failed("Tried to handle an invalidated watch; this "
                                "watch should have been removed");
        return TRUE;
    }
 ...
 */
void _dbus_transport_do_iteration(DBusTransport* transport, unsigned int flags,
                                  int timeout_milliseconds)
{
    ...
    if ((flags & (DBUS_ITERATION_DO_WRITING | DBUS_ITERATION_DO_READING)) == 0)
...
static dbus_bool_t recover_unused_bytes(DBusTransport* transport)
{
    ...
    else
    {
        ...
        succeeded = TRUE;
        if (!_dbus_string_copy(bytes, 0, buffer,
...
{
    if (_dbus_counter_get_size_value(transport->live_messages) >=
    ...
    if (!transport->unused_bytes_recovered && !recover_unused_bytes(transport))
        return DBUS_DISPATCH_NEED_MEMORY;
...
dbus_bool_t _dbus_transport_queue_messages(DBusTransport* transport)
{
  ...
  _dbus_verbose ("enter\n");
    ...
    /* Queue any messages */
    while ((status = _dbus_transport_get_dispatch_status(transport)) ==
...
void _dbus_transport_set_unix_user_function(
    DBusTransport* transport, DBusAllowUnixUserFunction function, void* data,
                                                     ...
                                                     char** label_p)
{
    ...
    *label_p = NULL;
                                  ...
                                  DBUS_CREDENTIAL_LINUX_SECURITY_LABEL))
    {
        ...
        *label_p = _dbus_strdup(
            _dbus_credentials_get_linux_security_label(auth_identity));
 ...
 * @returns #TRUE if user is available; the returned value may still be #NULL if
 * no memory to copy it
 */
...
void _dbus_transport_set_windows_user_function(
    DBusTransport* transport, DBusAllowWindowsUserFunction function, void* data,
    DBusFreeFunction free_data_function, void** old_data,
    DBusFreeFunction* old_free_data_function)
{
...
void _dbus_transport_set_pending_fds_function(DBusTransport* transport,
                                              ...
                                              void* data)
{
    _dbus_message_loader_set_pending_fds_function(transport->loader, callback,
                                                  data);
}
                               ...
                               dbus_uint32_t* peak_queue_fds)
{
    ...
    if (peak_queue_bytes != NULL)
        *peak_queue_bytes =
            _dbus_counter_get_peak_size_value(transport->live_messages);

    if (peak_queue_fds != NULL)
        *peak_queue_fds =
            _dbus_counter_get_peak_unix_fd_value(transport->live_messages);
}
