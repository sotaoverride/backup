...
class enable_shared_from_this
{
  protected:
    ...
    BOOST_CONSTEXPR enable_shared_from_this(enable_shared_from_this const&)
        BOOST_SP_NOEXCEPT
    {
    }

    enable_shared_from_this&
        operator=(enable_shared_from_this const&) BOOST_SP_NOEXCEPT
    {
        return *this;
    }

    ~enable_shared_from_this()
        BOOST_SP_NOEXCEPT // ~weak_ptr<T> newer throws, so this call also must
                          // not throw
    {
    }
  ...
  public: // actually private, but avoids compiler template friendship issues
    ...
    void _internal_accept_owner(shared_ptr<X> const* ppx,
                                Y* py) const BOOST_SP_NOEXCEPT
    {
