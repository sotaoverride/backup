...
{
    begin(const_string fn, std::size_t ln) : m_file_name(fn), m_line_num(ln)
    {
    }
...
class BOOST_TEST_DECL entry_value_collector
{
  public:
    ...
    entry_value_collector(entry_value_collector const& rhs) : m_last(true)
    {
        rhs.m_last = false;
    }
    ~entry_value_collector();
...
///
/// The Boost.Test framework allows for having several formatters/loggers at the
/// same time, each of which having their own log level and output stream.
///
/// This class serves the purpose of
/// - exposing an interface to the test framework (as a
/// boost::unit_test::test_observer)
/// - exposing an interface to the testing tools
/// - managing several loggers
///
/// @note Accesses to the functions exposed by this class are made through the
/// singleton
///   @c boost::unit_test::unit_test_log.
///
/// Users/developers willing to implement their own formatter need to:
/// - implement a boost::unit_test::unit_test_log_formatter that will output the
/// desired format
/// - register the formatter during a eg. global fixture using the method @c
/// set_formatter (though the framework singleton).
///
/// @warning this observer has a higher priority than the @ref
/// boost::unit_test::results_collector_t. This means that the various @ref
/// boost::unit_test::test_results associated to each test unit may not be
/// available at the time the @c test_unit_start, @c test_unit_finish ... are
/// called.
///
/// @see
/// - boost::unit_test::test_observer
...
class BOOST_TEST_DECL unit_test_log_t : public test_observer
{
  public:
    ...
    //!
    //! This will override the log sink/stream of all loggers, whether enabled
    //! or not.
    ...
    //!
    //! Adding a logger means that the specified logger is enabled. The log
    //! level is managed by the formatter itself and specifies what events are
    ...
    //!
    //! @returns the logger/formatter instance, or @c
    //! (unit_test_log_formatter*)0 if the format is not found.
    //! @par Since Boost 1.62
    unit_test_log_formatter* get_formatter(output_format);

    //! Sets the logger instance
    //!
    //! The specified logger becomes the unique active one. The custom log
    //! formatter has the format @c OF_CUSTOM_LOGGER. If such a format exists
    //! already, its formatter gets replaced by the one given in argument.
    //!
    //! The log level and output stream of the new formatter are taken from the
    //! currently active logger. In case several loggers are active, the order
    //! of priority is CUSTOM, HRF, XML, and JUNIT. If
    //! (unit_test_log_formatter*)0 is given as argument, the custom logger (if
    //! any) is removed.
    //!
    //! @note The ownership of the pointer is transfered to the Boost.Test
    //! framework. This call is equivalent to
    //! - a call to @c add_formatter
    //! - a call to @c set_format(OF_CUSTOM_LOGGER)
    //! - a configuration of the newly added logger with a previously configured
    //! stream and log level.
    void set_formatter(unit_test_log_formatter*);

    //! Adds a custom log formatter to the set of formatters
    //!
    //! The specified logger is added with the format @c OF_CUSTOM_LOGGER, such
    //! that it can be futher selected or its stream/log level can be specified.
    ...
    //! If (unit_test_log_formatter*)0 is given as argument, the custom logger
    //! (if any) is removed and no other action is performed.
    //!
    //! @note The ownership of the pointer is transfered to the Boost.Test
    //! framework.
    //! @par Since Boost 1.62
    ...
    void set_checkpoint(const_string file, std::size_t line_num,
    ...
    ut_detail::entry_value_collector
        operator()(log_level); // initiate entry collection
...
#define BOOST_TEST_LOG_ENTRY(ll)                                               \
    (::boost::unit_test::unit_test_log << ::boost::unit_test::log::begin(      \
         BOOST_TEST_L(__FILE__), __LINE__))(ll) /**/
...
#define BOOST_TEST_FRAMEWORK_MESSAGE(M)                                        \
    (::boost::unit_test::unit_test_log << ::boost::unit_test::log::begin(      \
         "boost.test framework", __LINE__))(::boost::unit_test::log_messages)  \
...
#define BOOST_TEST_PASSPOINT()                                                 \
    ::boost::unit_test::unit_test_log.set_checkpoint(                          \
        BOOST_TEST_L(__FILE__), static_cast<std::size_t>(__LINE__)) /**/
