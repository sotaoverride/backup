                            ...
                            struct ext2_inode_large* inode)
{
    ...
    if (EXT2_INODE_SIZE(fs->super) - EXT2_GOOD_OLD_INODE_SIZE - extra_size > 0)
        eamagic =
            *(__u32*)(((char*)inode) + EXT2_GOOD_OLD_INODE_SIZE + extra_size);
    /*
        ...
        (LINUX_S_ISREG(inode->i_mode) || LINUX_S_ISDIR(inode->i_mode)))
    {
        if (fix_problem(ctx, PR_4_ZERO_LEN_INODE, &pctx))
        {
            ...
            ext2fs_inode_alloc_stats2(fs, i, -1, LINUX_S_ISDIR(inode->i_mode));
...
void e2fsck_pass4(e2fsck_t ctx)
{
     ...
     */
    if (ctx->readahead_kb && (fs->block_map == NULL || fs->inode_map == NULL))
    ...
    for (i = 1; i <= fs->super->s_inodes_count && i > 0; i++)
    {
        ...
        {
            e2fsck_read_inode_full(ctx, i, EXT2_INODE(inode), inode_size,
                                   "pass4");
            pctx.ino = i;
            pctx.inode = EXT2_INODE(inode);
            ...
            {
                pctx.num = link_count;
                fix_problem(ctx, PR_4_INCONSISTENT_COUNT, &pctx);
            }
            pctx.num = link_counted;
            /* i_link_count was previously exceeded, but no longer
             * is, fix this but don't consider it an error */
            if ((isdir && link_counted > 1 &&
                 (inode->i_flags & EXT2_INDEX_FL) && link_count == 1 &&
                 !(ctx->options & E2F_OPT_NO)) ||
                fix_problem(ctx, PR_4_BAD_REF_COUNT, &pctx))
            {
                inode->i_links_count = link_counted;
                e2fsck_write_inode_full(ctx, i, EXT2_INODE(inode), inode_size,
                                        "pass4");
            }
        }
    }
