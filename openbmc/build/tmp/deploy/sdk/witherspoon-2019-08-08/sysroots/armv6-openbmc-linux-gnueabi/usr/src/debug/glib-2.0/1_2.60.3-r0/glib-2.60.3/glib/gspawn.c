 ...
 *              g_spawn_check_exit_status (status, NULL) ? "normally" :
 * "abnormally");
 *
...
static gint g_execute(const gchar* file, gchar** argv, gchar** envp,
                      gboolean search_path, gboolean search_path_from_envp);

static gboolean fork_exec_with_pipes(
    gboolean intermediate_child, const gchar* working_directory, gchar** argv,
    gchar** envp, gboolean close_descriptors, gboolean search_path,
    gboolean search_path_from_envp, gboolean stdout_to_null,
    gboolean stderr_to_null, gboolean child_inherits_stdin,
    gboolean file_and_argv_zero, gboolean cloexec_pipes,
    GSpawnChildSetupFunc child_setup, gpointer user_data, GPid* child_pid,
    gint* standard_input, gint* standard_output, gint* standard_error,
    GError** error);

static gboolean fork_exec_with_fds(
    gboolean intermediate_child, const gchar* working_directory, gchar** argv,
    gchar** envp, gboolean close_descriptors, gboolean search_path,
    gboolean search_path_from_envp, gboolean stdout_to_null,
    gboolean stderr_to_null, gboolean child_inherits_stdin,
    gboolean file_and_argv_zero, gboolean cloexec_pipes,
    GSpawnChildSetupFunc child_setup, gpointer user_data, GPid* child_pid,
    gint* child_close_fds, gint stdin_fd, gint stdout_fd, gint stderr_fd,
    GError** error);
 ...
 * @child_setup: (scope async) (nullable): function to run in the child just
 *before exec()
 * @user_data: (closure): user data for @child_setup
 * @child_pid: (out) (optional): return location for child process reference, or
 *%NULL
                       ...
                       GPid* child_pid, GError** error)
{
    ...
    return g_spawn_async_with_pipes(working_directory, argv, envp, flags,
                                    child_setup, user_data, child_pid, NULL,
                                    NULL, NULL, error);
}
 ...
 */
static void close_and_invalidate(gint* fd)
{
    if (*fd < 0)
        return;
    else
 ...
 * (optional): return location for child output, or %NULL
 * @standard_error: (out) (array zero-terminated=1) (element-type guint8)
 * (optional): return location for child error messages, or %NULL
 * @exit_status: (out) (optional): return location for child exit status, as
 * returned by waitpid(), or %NULL
                      ...
                      gint* exit_status, GError** error)
{
    ...
    g_return_val_if_fail(
        standard_error == NULL || !(flags & G_SPAWN_STDERR_TO_DEV_NULL), FALSE);
    ...
    if (!fork_exec_with_pipes(FALSE, working_directory, argv, envp,
                              ...
                              (flags & G_SPAWN_CLOEXEC_PIPES) != 0, child_setup,
                              user_data, &pid, NULL,
                              standard_output ? &outpipe : NULL,
                              standard_error ? &errpipe : NULL, error))
        return FALSE;
    ...
    while (!failed && (outpipe >= 0 || errpipe >= 0))
    {
        ...
        ret = select(MAX(outpipe, errpipe) + 1, &fds, NULL, NULL,
                     NULL /* no timeout */);
        ...
        if (ret < 0)
        {
            ...
            g_set_error(error, G_SPAWN_ERROR, G_SPAWN_ERROR_READ,
                        _("Unexpected error in select() reading data from a "
                          "child process (%s)"),
                        g_strerror(errsv));
...
again:
    ...
    if (ret < 0)
    {
        ...
        else if (errno == ECHILD)
        {
            if (exit_status)
            {
                g_warning("In call to g_spawn_sync(), exit status of a child "
 ...
 * @standard_input: (out) (optional): return location for file descriptor to
 * write to child's stdin, or %NULL
 * @standard_output: (out) (optional): return location for file descriptor to
 * read child's stdout, or %NULL
 * @standard_error: (out) (optional): return location for file descriptor to
 * read child's stderr, or %NULL
 * @error: return location for error
 *
 ...
 * 6. The program is of a recognised binary format, or has a shebang. Otherwise,
 * GLib will have to execute the program through the shell, which is not done
 * using the optimized codepath.
 *
 * If you are writing a GTK+ application, and the program you are spawning is a
 * graphical application too, then to ensure that the spawned program opens its
 * windows on the right screen, you may want to use #GdkAppLaunchContext,
                                  ...
                                  gint* standard_error, GError** error)
{
    ...
    return fork_exec_with_pipes(
        !(flags & G_SPAWN_DO_NOT_REAP_CHILD), working_directory, argv, envp,
        !(flags & G_SPAWN_LEAVE_DESCRIPTORS_OPEN),
        (flags & G_SPAWN_SEARCH_PATH) != 0,
        (flags & G_SPAWN_SEARCH_PATH_FROM_ENVP) != 0,
        (flags & G_SPAWN_STDOUT_TO_DEV_NULL) != 0,
        ...
        (flags & G_SPAWN_CHILD_INHERITS_STDIN) != 0,
        (flags & G_SPAWN_FILE_AND_ARGV_ZERO) != 0,
        (flags & G_SPAWN_CLOEXEC_PIPES) != 0, child_setup, user_data, child_pid,
 ...
 * @working_directory: (type filename) (nullable): child's current working
 * directory, or %NULL to inherit parent's, in the GLib file name encoding
 * @argv: (array zero-terminated=1): child's argument vector, in the GLib file
 * name encoding
 * @envp: (array zero-terminated=1) (nullable): child's environment, or %NULL to
...
{
    g_return_val_if_fail(argv != NULL, FALSE);
    g_return_val_if_fail(stdout_fd < 0 || !(flags & G_SPAWN_STDOUT_TO_DEV_NULL),
                         FALSE);
    g_return_val_if_fail(stderr_fd < 0 || !(flags & G_SPAWN_STDERR_TO_DEV_NULL),
                         FALSE);
    /* can't inherit stdin if we have an input pipe. */
    g_return_val_if_fail(
        stdin_fd < 0 || !(flags & G_SPAWN_CHILD_INHERITS_STDIN), FALSE);

    return fork_exec_with_fds(
        !(flags & G_SPAWN_DO_NOT_REAP_CHILD), working_directory, argv, envp,
        !(flags & G_SPAWN_LEAVE_DESCRIPTORS_OPEN),
        (flags & G_SPAWN_SEARCH_PATH) != 0,
        (flags & G_SPAWN_SEARCH_PATH_FROM_ENVP) != 0,
        ...
        (flags & G_SPAWN_FILE_AND_ARGV_ZERO) != 0,
        (flags & G_SPAWN_CLOEXEC_PIPES) != 0, child_setup, user_data, child_pid,
        NULL, stdin_fd, stdout_fd, stderr_fd, error);
}
 ...
 * @standard_output: (out) (array zero-terminated=1) (element-type guint8)
 *(optional): return location for child output
 * @standard_error: (out) (array zero-terminated=1) (element-type guint8)
 *(optional): return location for child errors
 * @exit_status: (out) (optional): return location for child exit status, as
 *returned by waitpid()
...
gboolean g_spawn_command_line_async(const gchar* command_line, GError** error)
{
    ...
    retval = g_spawn_async(NULL, argv, NULL, G_SPAWN_SEARCH_PATH, NULL, NULL,
                           NULL, error);
    g_strfreev(argv);
...
G_GNUC_NORETURN
static void write_err_and_exit(gint fd, gint msg)
{
    gint en = errno;
...
static gint filename_to_fd(const char* p)
{
    char c;
    int fd = 0;
...
static gint sane_open(const char* path, gint mode)
{
    gint ret;
...
static void do_exec(gint child_err_report_fd, gint stdin_fd, gint stdout_fd,
                    gint stderr_fd, const gchar* working_directory,
                    gchar** argv, gchar** envp, gboolean close_descriptors,
                    gboolean search_path, gboolean search_path_from_envp,
                    gboolean stdout_to_null, gboolean stderr_to_null,
                    gboolean child_inherits_stdin, gboolean file_and_argv_zero,
                    GSpawnChildSetupFunc child_setup, gpointer user_data)
{
...
static gboolean read_ints(int fd, gint* buf, gint n_ints_in_buf,
                          gint* n_ints_read, GError** error)
{
    gsize bytes = 0;
...
static gboolean do_posix_spawn(gchar** argv, gchar** envp, gboolean search_path,
                               gboolean stdout_to_null, gboolean stderr_to_null,
                               gboolean child_inherits_stdin,
                               gboolean file_and_argv_zero, GPid* child_pid,
                               gint* child_close_fds, gint stdin_fd,
                               gint stdout_fd, gint stderr_fd)
{
    ...
    if (stdin_fd >= 0)
    {
        ...
        if (!g_slist_find(child_close, GINT_TO_POINTER(stdin_fd)))
            child_close =
                g_slist_prepend(child_close, GINT_TO_POINTER(stdin_fd));
    }
    else if (!child_inherits_stdin)
    {
        /* Keep process from blocking on a read of stdin */
    ...
    if (stdout_fd >= 0)
    {
        ...
        if (!g_slist_find(child_close, GINT_TO_POINTER(stdout_fd)))
            child_close =
                g_slist_prepend(child_close, GINT_TO_POINTER(stdout_fd));
    }
    else if (stdout_to_null)
    {
    ...
    if (stderr_fd >= 0)
    {
        ...
        if (!g_slist_find(child_close, GINT_TO_POINTER(stderr_fd)))
            child_close =
                g_slist_prepend(child_close, GINT_TO_POINTER(stderr_fd));
    }
    else if (stderr_to_null)
    {
        gint write_null = sane_open("/dev/null", O_WRONLY | O_CLOEXEC);
    ...
    GError** error)
{
...
#ifdef POSIX_SPAWN_AVAILABLE
    if (!intermediate_child && working_directory == NULL &&
        !close_descriptors && !search_path_from_envp && child_setup == NULL)
    {
        g_debug("Launching with posix_spawn");
        status = do_posix_spawn(argv, envp, search_path, stdout_to_null,
                                stderr_to_null, child_inherits_stdin,
                                file_and_argv_zero, child_pid, child_close_fds,
        ...
        {
            g_set_error(error, G_SPAWN_ERROR, G_SPAWN_ERROR_FAILED,
                        _("Failed to spawn child process “%s” (%s)"), argv[0],
                        g_strerror(status));
            return FALSE;
        }
    ...
    if (intermediate_child &&
        !g_unix_open_pipe(child_pid_report_pipe, pipe_flags, error))
    ...
    if (pid < 0)
    {
        ...
        g_set_error(error, G_SPAWN_ERROR, G_SPAWN_ERROR_FORK,
    ...
    else if (pid == 0)
    {
        ...
        if (intermediate_child)
        {
            ...
            else if (grandchild_pid == 0)
            {
                ...
                do_exec(child_err_report_pipe[1], stdin_fd, stdout_fd,
                        stderr_fd, working_directory, argv, envp,
                        close_descriptors, search_path, search_path_from_envp,
                        stdout_to_null, stderr_to_null, child_inherits_stdin,
                        file_and_argv_zero, child_setup, user_data);
            }
            else
            {
                write_all(child_pid_report_pipe[1], &grandchild_pid,
        ...
        else
        {
            ...
            do_exec(child_err_report_pipe[1], stdin_fd, stdout_fd, stderr_fd,
                    working_directory, argv, envp, close_descriptors,
                    search_path, search_path_from_envp, stdout_to_null,
                    stderr_to_null, child_inherits_stdin, file_and_argv_zero,
                    child_setup, user_data);
        }
    }
    else
        ...
        if (n_ints >= 2)
        {
            ...
            switch (buf[0])
            {
                ...
                case CHILD_DUP2_FAILED:
                    g_set_error(error, G_SPAWN_ERROR, G_SPAWN_ERROR_FAILED,
                                _("Failed to redirect output or input of child "
                                  "process (%s)"),
                                g_strerror(buf[1]));
    ...
    GError** error)
{
    ...
    ret = fork_exec_with_fds(
        intermediate_child, working_directory, argv, envp, close_descriptors,
        search_path, search_path_from_envp, stdout_to_null, stderr_to_null,
        child_inherits_stdin, file_and_argv_zero, pipe_flags, child_setup,
        user_data, child_pid, child_close_fds, stdin_pipe[0], stdout_pipe[1],
        stderr_pipe[1], error);
    if (!ret)
        goto cleanup_and_fail;
...
static void script_execute(const gchar* file, gchar** argv, gchar** envp)
{
    /* Count the arguments.  */
    int argc = 0;
    while (argv[argc])
