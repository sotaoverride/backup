...
static int ask_callback(struct fdisk_context* cxt __attribute__((__unused__)),
                        struct fdisk_ask* ask, void* data)
{
...
static void sfdisk_init(struct sfdisk* sf)
{
    ...
    if (sf->label_nested)
    {
        struct fdisk_context* x =
            fdisk_new_nested_context(sf->cxt, sf->label_nested);
        if (!x)
...
static struct fdisk_partition* get_partition(struct fdisk_context* cxt,
                                             size_t partno)
{
...
static void backup_sectors(struct sfdisk* sf, const char* tpl, const char* name,
                           const char* devname, uint64_t offset, size_t size)
{
    char* fname;
    int fd, devfd;
    ...
    fdisk_info(sf->cxt, _("%12s (offset %5ju, size %5ju): %s"), name,
               (uintmax_t)offset, (uintmax_t)size, fname);
    close(fd);
    free(fname);
    return;
fail:
...
static char* mk_backup_filename_tpl(const char* filename, const char* devname,
                                    const char* suffix)
{
    char* tpl = NULL;
...
static void backup_partition_table(struct sfdisk* sf, const char* devname)
{
    ...
    while (fdisk_locate_disklabel(sf->cxt, i++, &name, &offset, &size) == 0 &&
           size)
...
static int move_partition_data(struct sfdisk* sf, size_t partno,
                               struct fdisk_partition* orig_pa)
{
    ...
    if (!pa)
        warnx(_(
            "failed to read new partition from device; ignoring --move-data"));
    else if (!fdisk_partition_has_size(pa))
        warnx(
            _("failed to get size of the new partition; ignoring --move-data"));
    else if (!fdisk_partition_has_start(pa))
        warnx(_(
            "failed to get start of the new partition; ignoring --move-data"));
    else if (!fdisk_partition_has_size(orig_pa))
        warnx(
            _("failed to get size of the old partition; ignoring --move-data"));
    else if (!fdisk_partition_has_start(orig_pa))
        warnx(_(
            "failed to get start of the old partition; ignoring --move-data"));
    else if (fdisk_partition_get_start(pa) ==
             fdisk_partition_get_start(orig_pa))
        warnx(_(
            "start of the partition has not been moved; ignoring --move-data"));
    else if (fdisk_partition_get_size(orig_pa) < fdisk_partition_get_size(pa))
        warnx(
            _("new partition is smaller than original; ignoring --move-data"));
    else
        ok = 1;
    if (!ok)
        return -EINVAL;
        ...
        (from >= to && to + nsectors >= from))
    {
        ...
        backward = from < to;
        DBG(MISC,
            ul_debug(" copy order: %s", backward ? "backward" : "forward"));
    ...
    {
        int yes = 0;
        fdisk_ask_yesno(sf->cxt, _("Do you want to move partition data?"),
                        &yes);
        if (!yes)
        {
...
static int print_geom(struct sfdisk* sf, const char* devname)
{
    ...
    fdisk_info(sf->cxt, "%s: %ju cylinders, %ju heads, %ju sectors/track",
               devname, (uintmax_t)fdisk_get_geom_cylinders(sf->cxt),
...
static int command_activate(struct sfdisk* sf, int argc, char** argv)
{
    ...
    for (i = 0; i < nparts; i++)
    {
        ...
        if (listonly)
        {
            if (!fdisk_partition_is_bootable(pa))
                continue;
            if (fdisk_partition_to_string(pa, sf->cxt, FDISK_FIELD_DEVICE,
                                          &data) == 0)
            {
                printf("%s\n", data);
                free(data);
    ...
    for (i = 1; i < argc; i++)
    {
        ...
        if (rc)
            errx(EXIT_FAILURE,
                 _("%s: partition %d: failed to toggle bootable flag"), devname,
                 i + 1);
    }
...
static int command_delete(struct sfdisk* sf, int argc, char** argv)
{
    ...
    if (argc == 1)
    {
        ...
        for (i = 0; i < nparts; i++)
        {
            if (fdisk_is_partition_used(sf->cxt, i) &&
                ...
                errx(EXIT_FAILURE, _("%s: partition %zu: failed to delete"),
                     devname, i + 1);
        }
        /* delete specified */
    }
    else
    {
        ...
        {
            size_t n =
                strtou32_or_err(argv[i], _("failed to parse partition number"));
...
static void assign_device_partition(struct sfdisk* sf, const char* devname,
                                    size_t partno, int rdonly)
{
    int rc;
    size_t n;
    ...
    if (!fdisk_is_partition_used(sf->cxt, partno - 1))
        errx(EXIT_FAILURE, _("%s: partition %zu: partition is unused"), devname,
             partno);
}
...
static int command_parttype(struct sfdisk* sf, int argc, char** argv)
{
    ...
    if (!typestr)
    {
        ...
        if (!t)
            errx(EXIT_FAILURE,
                 _("%s: partition %zu: failed to get partition type"), devname,
                 partno);
...
static int command_partuuid(struct sfdisk* sf, int argc, char** argv)
{
    ...
    if (!uuid)
    {
        ...
        if (!str)
            errx(EXIT_FAILURE,
                 _("%s: partition %zu: failed to get partition UUID"), devname,
                 partno);
        printf("%s\n", str);
        fdisk_unref_partition(pa);
        fdisk_deassign_device(sf->cxt, 1);
...
static int command_partlabel(struct sfdisk* sf, int argc, char** argv)
{
    ...
    if (!name)
    {
        ...
        if (!str)
            errx(EXIT_FAILURE,
                 _("%s: partition %zu: failed to get partition name"), devname,
                 partno);
        printf("%s\n", str);
        fdisk_unref_partition(pa);
        fdisk_deassign_device(sf->cxt, 1);
...
static void command_fdisk_help(void)
{
    ...
    fputs(_("   quit     show new situation and wait for user's feedback "
            "before write\n"),
          stdout);
    ...
    fputc('\n', stdout);
    fputs(_("   <type>   The partition type.  Default is a Linux data "
            "partition.\n"),
          stdout);
...
static int loop_control_commands(struct sfdisk* sf, struct fdisk_script* dp,
                                 char* buf)
{
...
#ifdef HAVE_LIBREADLINE
static char* sfdisk_fgets(struct fdisk_script* dp, char* buf, size_t bufsz,
                          FILE* f)
{
...
static void follow_wipe_mode(struct sfdisk* sf)
{
    ...
    if (dowipe)
    {
        if (!fdisk_is_ptcollision(sf->cxt))
        {
            fdisk_info(
                sf->cxt,
                _("The old %s signature will be removed by a write command."),
    ...
    {
        fdisk_warnx(sf->cxt,
                    _("The old %s signature may remain on the device. "
...
static int wipe_partition(struct sfdisk* sf, size_t partno)
{
    ...
    fdisk_warnx(sf->cxt, _("Partition #%zu contains a %s signature."),
                partno + 1, fstype);

    if (sf->pwipemode == WIPEMODE_AUTO && isatty(STDIN_FILENO))
        fdisk_ask_yesno(sf->cxt, _("Do you want to remove the signature?"),
                        &yes);
...
static int command_fdisk(struct sfdisk* sf, int argc, char** argv)
{
    ...
    if (!sf->quiet && sf->interactive)
    {
        ...
        color_disable();
        fdisk_info(sf->cxt, _("Changes will remain in memory only, until you "
    ...
    {
        if (!sf->quiet)
            fputs(_("Checking that no-one is using this disk right now ..."),
                  stdout);
        if (fdisk_device_is_used(sf->cxt))
        {
            ...
            fdisk_warnx(
                sf->cxt,
                _("This disk is currently in use - repartitioning is probably "
                  "a bad idea.\n"
                  "Umount all file systems, and swapoff all swap partitions on "
                  "this disk.\n"
    ...
    do
    {
        ...
        if (created && partno < 0 &&
            next_partno == fdisk_get_npartitions(sf->cxt) && !has_container(sf))
        {
        ...
        if (rc < 0)
        {
            DBG(PARSE,
                ul_debug(
                    "script parsing failed, trying sfdisk specific commands"));
            buf[sizeof(buf) - 1] = '\0';
        ...
        if (nparts)
        {
            size_t cur_partno;
            struct fdisk_partition* pa =
                fdisk_table_get_partition(tb, nparts - 1);
            ...
            { /* create a new disklabel */
                ...
                created = !rc;
                if (rc)
                    fdisk_warnx(sf->cxt, _("Failed to apply script headers, "
            ...
            { /* add partition */
                ...
                {
                    errno = -rc;
                    fdisk_warn(sf->cxt, _("Failed to add #%d partition"),
                               next_partno + 1);
                }
            }
    ...
    if ((rc == SFDISK_DONE_EOF || rc == SFDISK_DONE_WRITE) && created == 0 &&
        ...
        fdisk_table_get_nents(tb) == 0 && fdisk_script_get_header(dp, "label"))
    {
...
static void __attribute__((__noreturn__)) usage(void)
{
    ...
    fputs(_(" -A, --activate <dev> [<part> ...] list or set bootable (P)MBR "
            "partitions\n"),
          out);
    fputs(_(" -d, --dump <dev>                  dump partition table (usable "
            "for later input)\n"),
          out);
    fputs(_(" -J, --json <dev>                  dump partition table in JSON "
            "format\n"),
          out);
    fputs(_(" -g, --show-geometry [<dev> ...]   list geometry of all or "
            "specified devices\n"),
          out);
    fputs(_(" -l, --list [<dev> ...]            list partitions of each "
            "device\n"),
          out);
    fputs(_(" -F, --list-free [<dev> ...]       list unpartitioned free areas "
            "of each device\n"),
          out);
    fputs(_(" -r, --reorder <dev>               fix partitions order (by start "
            "offset)\n"),
          out);
    fputs(_(" -s, --show-size [<dev> ...]       list sizes of all or specified "
            "devices\n"),
          out);
    fputs(_(" -T, --list-types                  print the recognized types "
            "(see -X)\n"),
          out);
    fputs(_(" -V, --verify [<dev> ...]          test whether partitions seem "
            "correct\n"),
          out);
    fputs(_("     --delete <dev> [<part> ...]   delete all or specified "
            "partitions\n"),
          out);

    fputs(USAGE_SEPARATOR, out);
    fputs(_(" --part-label <dev> <part> [<str>] print or change partition "
            "label\n"),
          out);
    fputs(_(" --part-type <dev> <part> [<type>] print or change partition "
            "type\n"),
          out);
    fputs(_(" --part-uuid <dev> <part> [<uuid>] print or change partition "
            "uuid\n"),
          out);
    fputs(_(" --part-attrs <dev> <part> [<str>] print or change partition "
            "attributes\n"),
          out);
    ...
    fputs(_(" <type>                    partition type, GUID for GPT, hex for "
            "MBR\n"),
          out);

    fputs(USAGE_OPTIONS, out);
    fputs(_(" -a, --append              append partitions to existing "
            "partition table\n"),
          out);
    fputs(_(" -b, --backup              backup partition table sectors (see "
            "-O)\n"),
          out);
    fputs(_("     --bytes               print SIZE in bytes rather than in "
            "human readable format\n"),
          out);
    fputs(_("     --move-data[=<typescript>] move partition data after "
            "relocation (requires -N)\n"),
          out);
    fputs(_(" -f, --force               disable all consistency checking\n"),
          out);
    fputs(_("     --color[=<when>]      colorize output (auto, always or "
            "never)\n"),
          out);
    fprintf(out, "                             %s\n", USAGE_COLORS_DEFAULT);
    fputs(_(" -N, --partno <num>        specify partition number\n"), out);
    fputs(
        _(" -n, --no-act              do everything except write to device\n"),
        out);
    fputs(_("     --no-reread           do not check whether the device is in "
            "use\n"),
          out);
    ...
    fputs(_(" -w, --wipe <mode>         wipe signatures (auto, always or "
            "never)\n"),
          out);
    fputs(_(" -W, --wipe-partitions <mode>  wipe signatures from new "
            "partitions (auto, always or never)\n"),
          out);
    fputs(_(" -X, --label <name>        specify label type (dos, gpt, ...)\n"),
          out);
    fputs(
        _(" -Y, --label-nested <name> specify nested label type (dos, bsd)\n"),
        out);
    fputs(USAGE_SEPARATOR, out);
    fputs(
        _(" -G, --show-pt-geometry    deprecated, alias to --show-geometry\n"),
        out);
    fputs(_(" -L, --Linux               deprecated, only for backward "
            "compatibility\n"),
          out);
    fputs(_(" -u, --unit S              deprecated, only sector unit is "
            "supported\n"),
          out);
                            ...
                            longopts, &longidx)) != -1)
    {
        ...
        switch (c)
        {
            ...
            case 'f':
                ...
                break;
            case 'G':
                warnx(_("--show-pt-geometry is no more implemented. Using "
                        "--show-geometry."));
                /* fallthrough */
            case 'g':
