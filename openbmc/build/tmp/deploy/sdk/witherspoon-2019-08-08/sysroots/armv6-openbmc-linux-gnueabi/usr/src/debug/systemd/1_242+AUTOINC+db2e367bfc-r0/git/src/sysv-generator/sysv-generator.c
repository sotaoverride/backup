...
static const struct
{
    ...
    /* We ignore the SysV runlevels for shutdown here, as SysV services get
     * default dependencies anyway, and that
...
static int sysv_translate_facility(SysvStub* s, unsigned line, const char* name,
                                   char** ret)
{
    ...
    if (*name == '$')
    {
        ...
        if (r < 0)
            return log_error_errno(
                r, "[%s:%u] Could not build name for facility %s: %m", s->path,
                line, name);
...
static int handle_provides(SysvStub* s, unsigned line, const char* full_text,
                           const char* text)
{
    int r;
    ...
    for (;;)
    {
        ...
        r = extract_first_word(&text, &word, NULL,
                               EXTRACT_QUOTES | EXTRACT_RELAX);
        if (r < 0)
            return log_error_errno(
                r, "[%s:%u] Failed to parse word from provides string: %m",
                s->path, line);
        if (r == 0)
            break;
        ...
        switch (unit_name_to_type(m))
        {
            ...
            case UNIT_SERVICE:
                ...
                if (r < 0)
                    log_warning_errno(r,
                                      "[%s:%u] Failed to add LSB Provides name "
                                      "%s, ignoring: %m",
                                      s->path, line, m);
                break;
...
static int handle_dependencies(SysvStub* s, unsigned line,
                               const char* full_text, const char* text)
{
    int r;
    ...
    for (;;)
    {
        ...
        {
            /* the network-online target is special, as it needs to be actively
             * pulled in */
...
static int load_sysv(SysvStub* s)
{
    ...
    _cleanup_free_ char *short_description = NULL, *long_description = NULL,
                        *chkconfig_description = NULL;
    char* description;
    ...
    for (;;)
    {
        ...
        if (r < 0)
            return log_error_errno(
                r, "Failed to read configuration file '%s': %m", s->path);
        if (r == 0)
            break;
        ...
        if (IN_SET(state, LSB_DESCRIPTION, LSB) &&
            streq(t, "### END INIT INFO"))
        {
            state = NORMAL;
            continue;
        }
        ...
        if (state == NORMAL)
        {
            ...
            else if (startswith_no_case(t, "pidfile:"))
            {
                ...
                {
                    log_error("[%s:%u] PID file not absolute. Ignoring.",
                              s->path, line);
                    continue;
                }
...
static int acquire_search_path(const char* def, const char* envvar, char*** ret)
{
    ...
    if (e)
    {
        ...
        if (r < 0)
            return log_error_errno(
                r, "Failed to make $%s search path absolute: %m", envvar);
    }
...
static int enumerate_sysv(const LookupPaths* lp, Hashmap* all_services)
{
    ...
    r = acquire_search_path(SYSTEM_SYSVINIT_PATH, "SYSTEMD_SYSVINIT_PATH",
                            &sysvinit_path);
    if (r < 0)
        return r;
    ...
    STRV_FOREACH(path, sysvinit_path)
    {
        ...
        {
            if (errno != ENOENT)
                log_warning_errno(errno, "Opening %s failed, ignoring: %m",
                                  *path);
            continue;
        }
                ...
                errno, "Failed to enumerate directory %s, ignoring: %m", *path))
        {
            ...
            {
                log_warning_errno(errno, "stat() failed on %s/%s, ignoring: %m",
                                  *path, de->d_name);
                continue;
            }
            ...
            {
                log_debug_errno(
                    r, "Failed to detect whether %s exists, skipping: %m",
                    name);
                continue;
            }
            else if (r != 0)
...
static int set_dependencies_from_rcnd(const LookupPaths* lp,
                                      Hashmap* all_services)
{
    ...
    r = acquire_search_path(SYSTEM_SYSVRCND_PATH, "SYSTEMD_SYSVRCND_PATH",
                            &sysvrcnd_path);
    if (r < 0)
        return r;
    ...
    STRV_FOREACH(p, sysvrcnd_path)
    {
        for (i = 0; i < ELEMENTSOF(rcnd_table); i++)
        {
            ...
            FOREACH_DIRENT(de, d,
                           log_error_errno(
                               errno,
                               "Failed to enumerate directory %s, ignoring: %m",
                               path))
            {
...
static int run(const char* dest, const char* dest_early, const char* dest_late)
{
    ...
    r = lookup_paths_init(&lp, UNIT_FILE_SYSTEM, LOOKUP_PATHS_EXCLUDE_GENERATED,
                          NULL);
    if (r < 0)
