                     ...
                     const struct blkid_idmag* mag __attribute__((__unused__)))
{
    ...
    for (i = 0; i < ARRAY_SIZE(hdrs); i++)
    {
        ...
        ddf = (struct ddf_header*)blkid_probe_get_buffer(
            pr, off, sizeof(struct ddf_header));
        if (!ddf)
    ...
    if (lba > 0)
    {
        ...
        buf = blkid_probe_get_buffer(pr, lba << 9, sizeof(ddf->signature));
        if (!buf)
            return errno ? -errno : 1;
    ...
    *(version + sizeof(ddf->ddf_rev)) = '\0';
    ...
    if (blkid_probe_set_version(pr, version) != 0)
        return 1;
    if (blkid_probe_set_magic(pr, off, sizeof(ddf->signature),
                              (unsigned char*)&ddf->signature))
        return 1;
    return 0;
}

const struct blkid_idinfo ddfraid_idinfo = {.name = "ddf_raid_member",
