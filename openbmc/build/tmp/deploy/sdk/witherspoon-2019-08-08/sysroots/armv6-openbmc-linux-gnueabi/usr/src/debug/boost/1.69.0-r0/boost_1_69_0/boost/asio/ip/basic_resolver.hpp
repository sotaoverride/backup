...
#define BOOST_ASIO_SVC_T boost::asio::detail::resolver_service<InternetProtocol>
#endif
 ...
 */
template <typename InternetProtocol BOOST_ASIO_SVC_TPARAM_DEF1(
    = resolver_service<InternetProtocol>)>
class basic_resolver : BOOST_ASIO_SVC_ACCESS basic_io_object<BOOST_ASIO_SVC_T>,
                       public resolver_base
{
  public:
     ...
     * a numeric address string. If an empty string and the passive flag has
     ...
     * endpoint entries.
     *
     * @param protocol A protocol object, normally representing either the IPv4
     * or IPv6 version of an internet protocol.
     *
     * @param host A string identifying a location. May be a descriptive name or
     * a numeric address string. If an empty string and the passive flag has
     * been specified, the resolved endpoints are suitable for local service
    ...
    async_resolve(const query& q, BOOST_ASIO_MOVE_ARG(ResolveHandler) handler)
    {
        // If you get an error on the following line it means that your handler
        // does not meet the documented type requirements for a ResolveHandler.
        BOOST_ASIO_RESOLVE_HANDLER_CHECK(ResolveHandler, handler, results_type)
        type_check;

#if defined(BOOST_ASIO_ENABLE_OLD_SERVICES)
        return this->get_service().async_resolve(
            this->get_implementation(), q,
            BOOST_ASIO_MOVE_CAST(ResolveHandler)(handler));
#else  // defined(BOOST_ASIO_ENABLE_OLD_SERVICES)
        boost::asio::async_completion<
            ResolveHandler, void(boost::system::error_code, results_type)>
            init(handler);

        this->get_service().async_resolve(this->get_implementation(), q,
                  ...
                  BOOST_ASIO_MOVE_ARG(ResolveHandler) handler)
    {
        ...
        this->get_service().async_resolve(this->get_implementation(), e,
                                          init.completion_handler);
