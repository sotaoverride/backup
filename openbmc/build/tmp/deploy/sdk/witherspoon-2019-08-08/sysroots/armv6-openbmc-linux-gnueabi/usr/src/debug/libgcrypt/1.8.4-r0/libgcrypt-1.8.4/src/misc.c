...
void _gcry_set_log_handler(void (*f)(void*, int, const char*, va_list),
                           void* opaque)
{
    log_handler = f;
...
void _gcry_log(int level, const char* fmt, ...)
{
    va_list arg_ptr;
...
{
    _gcry_log(GCRY_LOG_BUG, ("... this is a bug (%s:%d:%s)\n"), file, line,
              func);
...
void _gcry_assert_failed(const char* expr, const char* file, int line,
                         const char* func)
{
    _gcry_log(GCRY_LOG_BUG, ("Assertion `%s' failed (%s:%d:%s)\n"), expr, file,
              line, func);
    abort(); /* Never called, but it makes the compiler happy. */
}
#else
...
void _gcry_assert_failed(const char* expr, const char* file, int line)
{
    _gcry_log(GCRY_LOG_BUG, ("Assertion `%s' failed (%s:%d)\n"), expr, file,
              line);
    abort(); /* Never called, but it makes the compiler happy. */
}
#endif
...
static void do_printhex(const char* text, const char* text2, const void* buffer,
                        size_t length)
{
    int wrap = 0;
    int cnt = 0;
    ...
    if (length && buffer)
    {
        ...
        for (; length--; p++)
        {
            ...
            if (wrap && ++cnt == 32 && length)
            {
                cnt = 0;
                log_printf(" \\\n");
                log_debug("%*s %*s", (int)strlen(text), "", (int)strlen(text2),
                          "");
            }
        }
    }
...
static int count_closing_parens(const char* p)
{
    int count = 0;
