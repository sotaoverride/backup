 ...
 * Free a kt_list
 */
krb5_error_code ktutil_free_kt_list(context, list) krb5_context context;
krb5_kt_list list;
{
 ...
 */
krb5_error_code ktutil_delete(context, list, idx) krb5_context context;
krb5_kt_list* list;
int idx;
{
    krb5_kt_list lp, prev;
 ...
 * one first.
 */
krb5_error_code ktutil_add(context, list, princ_str, fetch, kvno, enctype_str,
                           use_pass, salt_str) krb5_context context;
krb5_kt_list* list;
char* princ_str;
int fetch;
krb5_kvno kvno;
...
char* salt_str;
{
    ...
    if (use_pass)
    {
        ...
        retval = get_etype_info(context, princ, fetch, salt_str, &enctype,
                                &salt, &params);
        if (retval)
            goto cleanup;
 ...
 */
krb5_error_code ktutil_read_keytab(context, name, list) krb5_context context;
char* name;
krb5_kt_list* list;
{
 ...
 */
krb5_error_code ktutil_write_keytab(context, list, name) krb5_context context;
krb5_kt_list list;
char* name;
{
    krb5_kt_list lp;
 ...
 * if needed.
 */
krb5_error_code ktutil_read_srvtab(context, name, list) krb5_context context;
char* name;
krb5_kt_list* list;
{
    char* ktname;
