                    ...
                    if (*run_nlspath == '%')
                    {
                        ...
                        switch (*run_nlspath++)
                        {
                            ...
                            case 'L':
                                ...
                                break;
                            case 'l':
                                /* Use language element of locale category
                                 * value.  */
                                tmp = env_var;
                                do
                                {
                                ...
                                break;
                            case 't':
                                /* Use territory element of locale category
                                 * value.  */
                                tmp = env_var;
                                do
                                    ++tmp;
                                ...
                                break;
                            case 'c':
                                /* Use code set element of locale category
                                 * value.  */
                                tmp = env_var;
                                do
                                    ++tmp;
...
#define MAP_FILE 0
#endif
    catalog->file_ptr = (struct catalog_obj*)__mmap(NULL, st.st_size, PROT_READ,
...
#endif /* _POSIX_MAPPED_FILES */
    {
       ...
       implemented.  Try to load the file.  */
        ...
        do
        {
            size_t now = __read_nocancel(
                fd, (((char*)catalog->file_ptr) + (st.st_size - todo)), todo);
...
#define SWAP(x) (swapping ? SWAPU32(x) : (x))
    ...
    catalog->name_ptr =
        &catalog->file_ptr
             ->name_ptr[catalog->plane_size * catalog->plane_depth * 3];
#else
#error Cannot handle __BYTE_ORDER byte order
    ...
    catalog->strings =
        (const char*)&catalog->file_ptr
            ->name_ptr[catalog->plane_size * catalog->plane_depth * 3 * 2];
    ...
    max_offset =
        (st.st_size - sizeof(struct catalog_obj) + 2 * tab_size + max_offset);
    while (*lastp != '\0')
    {
        if (--max_offset == 0)
            goto invalid_file;
