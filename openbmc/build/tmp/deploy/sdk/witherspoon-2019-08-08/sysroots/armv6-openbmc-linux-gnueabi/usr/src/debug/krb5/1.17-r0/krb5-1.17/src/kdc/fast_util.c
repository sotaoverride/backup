...
static krb5_error_code armor_ap_request(struct kdc_request_state* state,
                                        krb5_fast_armor* armor)
{
    ...
    if (retval == 0)
        retval = krb5_auth_con_setflags(kdc_context, authcontext,
                                        0); /*disable replay cache*/
    retval = krb5_rd_req(kdc_context, &authcontext, &armor->armor_value,
                         NULL /*server*/, kdc_active_realm->realm_keytab, NULL,
                         &ticket);
    if (retval != 0)
    {
    ...
    if (retval == 0)
    {
        if (!krb5_principal_compare_any_realm(kdc_context, tgs_server,
                                              ticket->server))
        {
    ...
    if (retval == 0)
        retval = krb5_c_fx_cf2_simple(kdc_context, subkey, "subkeyarmor",
...
static krb5_error_code encrypt_fast_reply(struct kdc_request_state* state,
                                          ...
                                          krb5_data** fx_fast_reply)
{
    ...
    if (retval == 0)
        retval = krb5_encrypt_helper(kdc_context, state->armor_key,
                                     KRB5_KEYUSAGE_FAST_REP, encoded_response,
                                     &encrypted_reply);
    if (encoded_response)
    ...
    if (retval == 0)
    {
        retval = encode_krb5_pa_fx_fast_reply(&encrypted_reply, fx_fast_reply);
                  ...
                  struct kdc_request_state* state, krb5_data** inner_body_out)
{
    ...
    fast_padata =
        krb5int_find_pa_data(kdc_context, request->padata, KRB5_PADATA_FX_FAST);
    if (fast_padata != NULL)
    {
        ...
        {
            if (tgs_subkey)
                retval = krb5_c_fx_cf2_simple(kdc_context, tgs_subkey,
                                              "subkeyarmor", tgs_session,
                                              "ticketarmor", &state->armor_key);
            else
            {
                retval = KRB5KDC_ERR_PREAUTH_FAILED;
                k5_setmsg(kdc_context, retval,
        ...
        if (retval == 0)
        {
            ...
            if (plaintext.data == NULL)
                retval = ENOMEM;
            retval = krb5_c_decrypt(kdc_context, state->armor_key,
                                    KRB5_KEYUSAGE_FAST_ENC, NULL,
                                    &fast_armored_req->enc_part, &plaintext);
            if (retval == 0)
                retval = decode_krb5_fast_req(&plaintext, &fast_req);
            if (retval == 0 && inner_body_out != NULL)
            {
                retval = fetch_asn1_field((unsigned char*)plaintext.data, 1, 2,
                                          &scratch);
                if (retval == 0)
                {
                    retval = krb5_copy_data(kdc_context, &scratch, &inner_body);
                }
            }
            if (plaintext.data)
                free(plaintext.data);
        ...
        if (retval == 0)
            retval = krb5_c_verify_checksum(
                kdc_context, state->armor_key, KRB5_KEYUSAGE_FAST_REQ_CHKSUM,
                checksummed_data, cksum, &cksum_valid);
        ...
        if (retval == 0)
        {
            if ((fast_req->fast_options & UNSUPPORTED_CRITICAL_FAST_OPTIONS) !=
                0)
                                                ...
                                                krb5_enctype enctype)
{
    ...
    if (retval == 0)
        retval =
            krb5_us_timeofday(kdc_context, &finish.timestamp, &finish.usec);
    if (retval == 0)
        retval = encode_krb5_ticket(rep->ticket, &encoded_ticket);
    if (retval == 0)
        retval = krb5int_c_mandatory_cksumtype(
            kdc_context, state->armor_key->enctype, &cksumtype);
    if (retval == 0)
        retval = krb5_c_make_checksum(kdc_context, cksumtype, state->armor_key,
                                          ...
                                          krb5_keyblock** out_key)
{
    ...
    if (state->armor_key)
        retval = krb5_c_fx_cf2_simple(kdc_context, state->strengthen_key,
                                      "strengthenkey", existing_key, "replykey",
                                      out_key);
    else
...
static krb5_error_code make_padata(krb5_preauthtype pa_type,
 ...
 * kvno used.
 */
static krb5_error_code get_cookie_key(krb5_context context, krb5_db_entry* tgt,
                                      krb5_kvno kvno,
...
static krb5_boolean is_relevant(krb5_pa_data* const* cpadata,
                                krb5_pa_data* const* rpadata)
{
