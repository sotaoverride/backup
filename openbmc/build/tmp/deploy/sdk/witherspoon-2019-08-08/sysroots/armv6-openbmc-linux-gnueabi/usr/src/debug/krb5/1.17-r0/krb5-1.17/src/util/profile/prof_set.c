 ...
 */
errcode_t KRB5_CALLCONV profile_update_relation(profile_t profile,
                                                const char** names,
                                                ...
                                                const char* new_value)
{
    ...
    {
        state = 0;
        retval = profile_find_node(section, *cpp, 0, 1, &state, &section);
        if (retval)
        {
 ...
 * TYT - 2/25/99
 */
errcode_t KRB5_CALLCONV profile_clear_relation(profile_t profile,
                                               const char** names)
{
    errcode_t retval;
    struct profile_node *section, *node;
    ...
    if (profile->vt)
    {
        if (!profile->vt->update_relation)
            return PROF_UNSUPPORTED;
        return profile->vt->update_relation(profile->cbdata, names, NULL, NULL);
    }
 ...
 * ADL - 2/23/99, rewritten TYT 2/25/99
 */
errcode_t KRB5_CALLCONV profile_rename_section(profile_t profile,
                                               const char** names,
                                               const char* new_name)
{
    errcode_t retval;
 ...
 * ADL - 2/23/99, rewritten TYT 2/25/99
 */
errcode_t KRB5_CALLCONV profile_add_relation(profile_t profile,
                                             const char** names,
                                             const char* new_value)
{
    errcode_t retval;
