...
static int FcCharSetFindLeafForward(const FcCharSet* fcs, int start,
                                    FcChar16 num)
{
...
static FcCharLeaf* FcCharSetFindLeaf(const FcCharSet* fcs, FcChar32 ucs4)
{
...
static FcBool FcCharSetPutLeaf(FcCharSet* fcs, FcChar32 ucs4, FcCharLeaf* leaf,
                               int pos)
{
    ...
    if (FC_IS_ZERO_OR_POWER_OF_TWO(fcs->num))
    {
        ...
        else
        {
            ...
            if (!numbers)
            {
                ...
                leaves =
                    realloc(new_leaves, (alloced / 2) * sizeof(*new_leaves));
    ...
    memmove(leaves + pos + 1, leaves + pos, (fcs->num - pos) * sizeof(*leaves));
...
FcCharLeaf* FcCharSetFindLeafCreate(FcCharSet* fcs, FcChar32 ucs4)
{
    int pos;
    FcCharLeaf* leaf;
...
static FcBool FcCharSetInsertLeaf(FcCharSet* fcs, FcChar32 ucs4,
                                  FcCharLeaf* leaf)
{
    int pos;
    ...
    if (pos >= 0)
    {
        ...
        FcCharSetLeaves(fcs)[pos] = FcPtrToOffset(FcCharSetLeaves(fcs), leaf);
        return FcTrue;
    }
    pos = -pos - 1;
...
static void FcCharSetIterSet(const FcCharSet* fcs, FcCharSetIter* iter)
{
...
static void FcCharSetIterNext(const FcCharSet* fcs, FcCharSetIter* iter)
{
    int pos = iter->pos + 1;
    if (pos >= fcs->num)
    {
...
static void FcCharSetIterStart(const FcCharSet* fcs, FcCharSetIter* iter)
{
    iter->ucs4 = 0;
    iter->pos = 0;
...
static FcBool FcCharSetAddLeaf(FcCharSet* fcs, FcChar32 ucs4, FcCharLeaf* leaf)
{
...
static FcCharSet* FcCharSetOperate(const FcCharSet* a, const FcCharSet* b,
                                   FcBool (*overlap)(FcCharLeaf* result,
                                                     const FcCharLeaf* al,
                                                     const FcCharLeaf* bl),
                                   FcBool aonly, FcBool bonly)
{
    FcCharSet* fcs;
    FcCharSetIter ai, bi;
...
static FcBool FcCharSetIntersectLeaf(FcCharLeaf* result, const FcCharLeaf* al,
                                     const FcCharLeaf* bl)
{
    int i;
...
static FcBool FcCharSetUnionLeaf(FcCharLeaf* result, const FcCharLeaf* al,
                                 const FcCharLeaf* bl)
{
    int i;
...
static FcBool FcCharSetSubtractLeaf(FcCharLeaf* result, const FcCharLeaf* al,
                                    const FcCharLeaf* bl)
{
    int i;
    FcBool nonempty = FcFalse;
...
static FcBool FcNameParseRange(FcChar8** string, FcChar32* pfirst,
                               FcChar32* plast)
{
    char* s = (char*)*string;
    char* t;
...
static void FcNameUnparseUnicode(FcStrBuf* buf, FcChar32 u)
{
...
FcBool FcNameUnparseCharSet(FcStrBuf* buf, const FcCharSet* c)
{
    FcCharSetIter ci;
    FcChar32 first, last;
    int i;
    ...
    for (FcCharSetIterStart(c, &ci); ci.leaf; FcCharSetIterNext(c, &ci))
    {
        for (i = 0; i < 256 / 32; i++)
        {
...
static FcCharLeafEnt* FcCharLeafEntCreate(FcCharSetFreezer* freezer)
{
    if (!freezer->leaf_remain)
    {
        ...
        newBlocks = realloc(freezer->leaf_blocks,
                            freezer->leaf_block_count * sizeof(FcCharLeafEnt*));
        if (!newBlocks)
            return 0;
        freezer->leaf_blocks = newBlocks;
        freezer->current_block =
            freezer->leaf_blocks[freezer->leaf_block_count - 1] =
                malloc(FC_CHAR_LEAF_BLOCK * sizeof(FcCharLeafEnt));
...
static FcChar32 FcCharLeafHash(FcCharLeaf* leaf)
{
    FcChar32 hash = 0;
    int i;
...
static FcCharLeaf* FcCharSetFreezeLeaf(FcCharSetFreezer* freezer,
                                       FcCharLeaf* leaf)
{
    FcChar32 hash = FcCharLeafHash(leaf);
    FcCharLeafEnt** bucket =
        &freezer->leaf_hash_table[hash % FC_CHAR_LEAF_HASH_SIZE];
    FcCharLeafEnt* ent;
...
static FcChar32 FcCharSetHash(FcCharSet* fcs)
{
    FcChar32 hash = 0;
    int i;
    ...
    for (i = 0; i < fcs->num; i++)
        hash = ((hash << 1) | (hash >> 31)) ^
               FcCharLeafHash(FcCharSetLeaf(fcs, i));
    /* hash in numbers */
    for (i = 0; i < fcs->num; i++)
...
static FcBool FcCharSetFreezeOrig(FcCharSetFreezer* freezer,
                                  const FcCharSet* orig,
                                  const FcCharSet* frozen)
{
    FcCharSetOrigEnt** bucket =
        &freezer->orig_hash_table[((uintptr_t)orig) % FC_CHAR_SET_HASH_SIZE];
...
static FcCharSet* FcCharSetFreezeBase(FcCharSetFreezer* freezer, FcCharSet* fcs)
{
    FcChar32 hash = FcCharSetHash(fcs);
    FcCharSetEnt** bucket =
        &freezer->set_hash_table[hash % FC_CHAR_SET_HASH_SIZE];
    FcCharSetEnt* ent;
    int size;
    int i;
    ...
    for (ent = *bucket; ent; ent = ent->next)
    {
        if (ent->hash == hash && ent->set.num == fcs->num &&
            !memcmp(FcCharSetNumbers(&ent->set), FcCharSetNumbers(fcs),
    ...
    size = (sizeof(FcCharSetEnt) + fcs->num * sizeof(FcCharLeaf*) +
            fcs->num * sizeof(FcChar16));
    ent = malloc(size);
    if (!ent)
        return 0;
    ...
    if (fcs->num)
    {
        ...
        ent->set.numbers_offset =
            (ent->set.leaves_offset + fcs->num * sizeof(intptr_t));

        ent_leaves = FcCharSetLeaves(&ent->set);
        for (i = 0; i < fcs->num; i++)
            ent_leaves[i] = FcPtrToOffset(ent_leaves, FcCharSetLeaf(fcs, i));
        memcpy(FcCharSetNumbers(&ent->set), FcCharSetNumbers(fcs),
               fcs->num * sizeof(FcChar16));
    }
    else
    {
...
static const FcCharSet* FcCharSetFindFrozen(FcCharSetFreezer* freezer,
                                            const FcCharSet* orig)
{
    FcCharSetOrigEnt** bucket =
        &freezer->orig_hash_table[((uintptr_t)orig) % FC_CHAR_SET_HASH_SIZE];
...
const FcCharSet* FcCharSetFreeze(FcCharSetFreezer* freezer,
                                 const FcCharSet* fcs)
{
    FcCharSet* b;
    const FcCharSet* n = 0;
...
void FcCharSetFreezerDestroy(FcCharSetFreezer* freezer)
{
    int i;
    ...
    if (FcDebug() & FC_DBG_CACHE)
    {
        printf("\ncharsets %d -> %d leaves %d -> %d\n", freezer->charsets_seen,
               freezer->charsets_allocated, freezer->leaves_seen,
               freezer->leaves_allocated);
    }
...
FcCharSet* FcCharSetSerialize(FcSerialize* serialize, const FcCharSet* cs)
{
    ...
    if (cs->num)
    {
        ...
        cs_serialized->leaves_offset =
            FcPtrToOffset(cs_serialized, leaves_serialized);
        ...
        cs_serialized->numbers_offset =
            FcPtrToOffset(cs_serialized, numbers_serialized);
        ...
        for (i = 0; i < cs->num; i++)
        {
            ...
            *leaf_serialized = *leaf;
            leaves_serialized[i] =
                FcPtrToOffset(leaves_serialized, leaf_serialized);
