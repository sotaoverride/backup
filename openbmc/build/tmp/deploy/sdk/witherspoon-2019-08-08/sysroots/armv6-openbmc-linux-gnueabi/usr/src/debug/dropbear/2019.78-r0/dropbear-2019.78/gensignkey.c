...
static int buf_writefile(buffer* buf, const char* filename)
{
    ...
    if (fd < 0)
    {
        dropbear_log(LOG_ERR, "Couldn't create new file %s: %s", filename,
                     strerror(errno));
        goto out;
    }
    ...
    while (buf->pos != buf->len)
    {
        ...
        if (len <= 0)
        {
            dropbear_log(LOG_ERR, "Failed writing file %s: %s", filename,
                         strerror(errno));
            goto out;
        }
        buf_incrpos(buf, len);
...
out:
    if (fd >= 0)
    {
        if (fsync(fd) != 0)
        {
            dropbear_log(LOG_ERR, "fsync of %s failed: %s", filename,
                         strerror(errno));
        }
        m_close(fd);
    }
                     ...
                     int skip_exist)
{
    ...
    if (link(fn_temp, filename) < 0)
    {
        ...
        - we probably just lost a race with another connection to generate the
        key */
