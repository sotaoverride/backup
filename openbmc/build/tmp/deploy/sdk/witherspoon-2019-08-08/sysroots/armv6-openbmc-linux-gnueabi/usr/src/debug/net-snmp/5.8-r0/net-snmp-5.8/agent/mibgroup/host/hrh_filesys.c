/*
 *  Host Resources MIB - File System device group implementation (HAL rewrite) -
 * hrh_filesys.c
 *
 */
...
int header_hrhfilesys(struct variable*, oid*, size_t*, int, size_t*,
                      WriteMethod**);
...
struct variable4 hrfsys_variables[] = {
    ...
    {HRFSYS_ACCESS,
     ...
     2,
     {1, 5}},
    {HRFSYS_BOOT, ASN_INTEGER, NETSNMP_OLDAPI_RONLY, var_hrhfilesys, 2, {1, 6}},
    {HRFSYS_STOREIDX,
     ASN_INTEGER,
     NETSNMP_OLDAPI_RONLY,
     var_hrhfilesys,
...
int header_hrhfilesys(struct variable* vp, oid* name, size_t* length, int exact,
...
#define HRFSYS_ENTRY_NAME_LENGTH 11
    ...
    for (;;)
    {
        ...
        if ((!exact && (result < 0)) && (LowIndex == -1 || fsys_idx < LowIndex))
        {
            LowIndex = fsys_idx;
...
u_char* var_hrhfilesys(struct variable* vp, oid* name, size_t* length,
                       int exact, size_t* var_len, WriteMethod** write_method)
{
    ...
    switch (vp->magic)
    {
        ...
        default:
            DEBUGMSGTL(
                ("snmpd", "unknown sub-id %d in var_hrhfilesys\n", vp->magic));
    }
    return NULL;
}
