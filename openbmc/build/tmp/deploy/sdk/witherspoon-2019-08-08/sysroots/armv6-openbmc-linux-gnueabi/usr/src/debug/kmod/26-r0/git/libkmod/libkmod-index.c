...
static int add_value(struct index_value** values, const char* value,
                     unsigned len, unsigned int priority)
{
...
static char* index_search__node(struct index_node_f* node, const char* key,
                                int i)
{
    char* value;
    ...
    while (node)
    {
        ...
        if (key[i] == '\0')
        {
            value = node->values != NULL ? strdup(node->values[0].value) : NULL;
...
static void index_searchwild__all(struct index_node_f* node, int j,
                                  struct strbuf* buf, const char* subkey,
                                  struct index_value** out)
{
    int pushed = 0;
    int ch;
...
static void index_searchwild__node(struct index_node_f* node,
                                   struct strbuf* buf, const char* key, int i,
                                   struct index_value** out)
{
    struct index_node_f* child;
    int j;
                                                ...
                                                uint32_t offset)
{
    ...
    node =
        malloc(sizeof(struct index_mm_node) + sizeof(uint32_t) * child_count +
...
static struct index_mm_node*
    index_mm_readchild(const struct index_mm_node* parent, int ch)
{
    if (parent->first <= ch && ch <= parent->last)
    {
                                  ...
                                  int i)
{
    ...
    while (node)
    {
        ...
        if (key[i] == '\0')
        {
            value = node->values.len > 0 ? strdup(node->values.values[0].value)
                                         : NULL;
