...
int register_sec_mod(int secmod, const char* modname,
    ...
    if (result != SE_OK)
    {
        switch (result)
        {
            ...
            case SE_ALREADY_THERE:
                ...
                {
                    snmp_log(LOG_ERR,
                             "snmp_secmod: two security modules %s and %s "
                             "registered with the same security number\n",
                             modname, othername);
                }
                break;

            default:
                snmp_log(LOG_ERR, "snmp_secmod: unknown error trying to "
...
static int set_default_secmod(int major, int minor, void* serverarg,
                              void* clientarg)
{
