...
static inline void g_module_set_error_unduped(gchar* error)
{
...
#define SUPPORT_OR_RETURN(rv)                                                  \
    {                                                                          \
        g_module_set_error("dynamic modules are "                              \
...
static gchar* parse_libtool_archive(const gchar* libtool_name)
{
    ...
    if (fd < 0)
    {
        ...
        g_module_set_error_unduped(g_strdup_printf(
            "failed to open libtool archive \"%s\"", display_libtool_name));
    ...
    while (!g_scanner_eof(scanner))
    {
            ...
            token == TOKEN_LIBDIR)
        {
            if (g_scanner_get_next_token(scanner) != '=' ||
                g_scanner_get_next_token(scanner) != (token == TOKEN_INSTALLED
                                                          ? G_TOKEN_IDENTIFIER
                                                          : G_TOKEN_STRING))
            {
                gchar* display_libtool_name =
                    g_filename_display_name(libtool_name);
                g_module_set_error_unduped(
                    g_strdup_printf("unable to parse libtool archive \"%s\"",
...
static inline gboolean str_check_suffix(const gchar* string,
                                        const gchar* suffix)
{
...
static void _g_module_debug_init(void)
{
 ...
 * so it always fails to create main_module if file_name is NULL */
    ...
    else
    {
        ...
        g_module_set_error_unduped(
            g_strdup_printf("unable to access file \"%s\"", display_file_name));
    ...
    if (handle)
    {
        ...
        if (check_failed)
        {
            ...
            error = g_strconcat("GModule (", file_name, ") ",
                                "initialization check failed: ", check_failed,
                                NULL);
...
GModule* g_module_open_utf8(const gchar* file_name, GModuleFlags flags)
{
