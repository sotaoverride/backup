 ...
 */
#define MAX_COOKIE_SIZE                                                        \
    (2 + 2 + 2 + 2 + 2 + 1 + 4 + 2 + EVP_MAX_MD_SIZE + 1 + SSL_COOKIE_LENGTH + \
 ...
 */
#define MAX_HRR_SIZE                                                           \
    (SSL3_HM_HEADER_LENGTH + 2 + SSL3_RANDOM_SIZE + 1 +                        \
     SSL_MAX_SSL_SESSION_ID_LENGTH + 2 + 1 + 2 + 6 + 4 + MAX_COOKIE_SIZE)
                               ...
                               X509* x, size_t chainidx)
{
    ...
    if (!PACKET_get_1(pkt, &ilen) || !PACKET_get_bytes(pkt, &data, ilen))
    {
    ...
    if (!s->hit || SSL_IS_TLS13(s))
    {
        if (PACKET_remaining(&hostname) > TLSEXT_MAXLEN_host_name)
        {
            SSLfatal(s, SSL_AD_UNRECOGNIZED_NAME,
                     SSL_F_TLS_PARSE_CTOS_SERVER_NAME, SSL_R_BAD_EXTENSION);
            return 0;
        }
                       ...
                       size_t chainidx)
{
    ...
    {
        SSLfatal(s, SSL_AD_DECODE_ERROR, SSL_F_TLS_PARSE_CTOS_SRP,
                 SSL_R_BAD_EXTENSION);
        return 0;
    }
                                  ...
                                  X509* x, size_t chainidx)
{
    if (s->ext.session_ticket_cb &&
        !s->ext.session_ticket_cb(s, PACKET_data(pkt), PACKET_remaining(pkt),
                                  s->ext.session_ticket_cb_arg))
    {
        SSLfatal(s, SSL_AD_INTERNAL_ERROR, SSL_F_TLS_PARSE_CTOS_SESSION_TICKET,
                 ERR_R_INTERNAL_ERROR);
        return 0;
    }
                                 ...
                                 X509* x, size_t chainidx)
{
    ...
    {
        SSLfatal(s, SSL_AD_DECODE_ERROR, SSL_F_TLS_PARSE_CTOS_SIG_ALGS_CERT,
                 SSL_R_BAD_EXTENSION);
        return 0;
    }
                            ...
                            size_t chainidx)
{
    ...
    if (!PACKET_as_length_prefixed_2(pkt, &supported_sig_algs) ||
        PACKET_remaining(&supported_sig_algs) == 0)
    {
        SSLfatal(s, SSL_AD_DECODE_ERROR, SSL_F_TLS_PARSE_CTOS_SIG_ALGS,
                 SSL_R_BAD_EXTENSION);
        return 0;
    }
                                  ...
                                  X509* x, size_t chainidx)
{
    ...
    {
        SSLfatal(s, SSL_AD_DECODE_ERROR, SSL_F_TLS_PARSE_CTOS_STATUS_REQUEST,
                 SSL_R_BAD_EXTENSION);
        return 0;
    }
                        ...
                        size_t chainidx)
{
    ...
    if (!PACKET_memdup(&save_protocol_list, &s->s3->alpn_proposed,
                            ...
                            size_t chainidx)
{
    ...
    if (!PACKET_forward(pkt, mki_len) || PACKET_remaining(pkt))
    {
        SSLfatal(s, SSL_AD_DECODE_ERROR, SSL_F_TLS_PARSE_CTOS_USE_SRTP,
                             ...
                             size_t chainidx)
{
    ...
    while (PACKET_remaining(&key_share_list) > 0)
    {
         ...
         */
        if (s->s3->group_id != 0 && (group_id != s->s3->group_id ||
                          ...
                          size_t chainidx)
{
    ...
    pkey = EVP_PKEY_new_raw_private_key(
        EVP_PKEY_HMAC, NULL, s->session_ctx->ext.cookie_hmac_key,
    ...
    if (!PACKET_get_1(&cookie, &key_share) || !PACKET_get_net_4(&cookie, &tm) ||
    ...
    if (!WPACKET_put_bytes_u16(&hrrpkt, TLSEXT_TYPE_supported_versions) ||
        !WPACKET_start_sub_packet_u16(&hrrpkt) ||
        !WPACKET_put_bytes_u16(&hrrpkt, s->version) || !WPACKET_close(&hrrpkt))
    {
        WPACKET_cleanup(&hrrpkt);
        SSLfatal(s, SSL_AD_INTERNAL_ERROR, SSL_F_TLS_PARSE_CTOS_COOKIE,
                 ERR_R_INTERNAL_ERROR);
    ...
    if (!create_synthetic_message_hash(s, PACKET_data(&chhash),
                                       PACKET_remaining(&chhash), hrr, hrrlen))
    {
                                    ...
                                    X509* x, size_t chainidx)
{
    ...
    {
        SSLfatal(s, SSL_AD_DECODE_ERROR, SSL_F_TLS_PARSE_CTOS_SUPPORTED_GROUPS,
                 SSL_R_BAD_EXTENSION);
        return 0;
    }
                       ...
                       size_t chainidx)
{
    ...
    {
        SSLfatal(s, SSL_AD_DECODE_ERROR, SSL_F_TLS_PARSE_CTOS_EMS,
                 SSL_R_BAD_EXTENSION);
        return 0;
    }
                              ...
                              X509* x, size_t chainidx)
{
    if (PACKET_remaining(pkt) != 0)
    {
        SSLfatal(s, SSL_AD_DECODE_ERROR, SSL_F_TLS_PARSE_CTOS_EARLY_DATA,
                 SSL_R_BAD_EXTENSION);
        return 0;
    }
    ...
    {
        SSLfatal(s, SSL_AD_ILLEGAL_PARAMETER, SSL_F_TLS_PARSE_CTOS_EARLY_DATA,
                 SSL_R_BAD_EXTENSION);
        return 0;
    }
                       ...
                       size_t chainidx)
{
    ...
    for (id = 0; PACKET_remaining(&identities) != 0; id++)
    {
        ...
        if (s->psk_find_session_cb != NULL &&
            !s->psk_find_session_cb(s, PACKET_data(&identity), idlen, &sess))
        {
        ...
        if (sess == NULL && s->psk_server_callback != NULL &&
            idlen <= PSK_MAX_IDENTITY_LEN)
        {
            ...
            pskdatalen =
                s->psk_server_callback(s, pskid, pskdata, sizeof(pskdata));
            OPENSSL_free(pskid);
            ...
            else if (pskdatalen > 0)
            {
                ...
                if (sess == NULL ||
                    !SSL_SESSION_set1_master_key(sess, pskdata, pskdatalen) ||
        ...
        else
        {
             ...
             * rounding errors.
             */
            if (id == 0 && sess->timeout >= (long)agesec &&
...
EXT_RETURN tls_construct_stoc_server_name(SSL* s, WPACKET* pkt,
                                          ...
                                          size_t chainidx)
{
    if (s->hit || s->servername_done != 1 || s->ext.hostname == NULL)
        return EXT_RETURN_NOT_SENT;
                                            ...
                                            size_t chainidx)
{
    ...
    if (!WPACKET_put_bytes_u16(pkt, TLSEXT_TYPE_ec_point_formats) ||
        !WPACKET_start_sub_packet_u16(pkt) ||
        !WPACKET_sub_memcpy_u8(pkt, plist, plistlen) || !WPACKET_close(pkt))
    {
        SSLfatal(s, SSL_AD_INTERNAL_ERROR,
                                   ...
                                   X509* x, size_t chainidx)
{
    ...
    if (!WPACKET_put_bytes_u16(
            pkt, TLSEXT_TYPE_application_layer_protocol_negotiation) ||
        !WPACKET_start_sub_packet_u16(pkt) ||
        !WPACKET_start_sub_packet_u16(pkt) ||
        ...
        !WPACKET_close(pkt) || !WPACKET_close(pkt))
    {
        SSLfatal(s, SSL_AD_INTERNAL_ERROR, SSL_F_TLS_CONSTRUCT_STOC_ALPN,
                 ERR_R_INTERNAL_ERROR);
        return EXT_RETURN_FAIL;
    }
                                       ...
                                       size_t chainidx)
{
    ...
    if (!WPACKET_put_bytes_u16(pkt, TLSEXT_TYPE_use_srtp) ||
        !WPACKET_start_sub_packet_u16(pkt) || !WPACKET_put_bytes_u16(pkt, 2) ||
                                                 ...
                                                 size_t chainidx)
{
    ...
    if (!WPACKET_put_bytes_u16(pkt, TLSEXT_TYPE_supported_versions) ||
        !WPACKET_start_sub_packet_u16(pkt) ||
        !WPACKET_put_bytes_u16(pkt, s->version) || !WPACKET_close(pkt))
    {
        SSLfatal(s, SSL_AD_INTERNAL_ERROR,
                 SSL_F_TLS_CONSTRUCT_STOC_SUPPORTED_VERSIONS,
                 ERR_R_INTERNAL_ERROR);
                                        ...
                                        size_t chainidx)
{
    ...
    if (s->hello_retry_request == SSL_HRR_PENDING)
    {
        ...
        if (!WPACKET_put_bytes_u16(pkt, TLSEXT_TYPE_key_share) ||
            !WPACKET_start_sub_packet_u16(pkt) ||
            !WPACKET_put_bytes_u16(pkt, s->s3->group_id) || !WPACKET_close(pkt))
        {
            SSLfatal(s, SSL_AD_INTERNAL_ERROR,
                                     ...
                                     X509* x, size_t chainidx)
{
    ...
    if (!WPACKET_put_bytes_u16(pkt, TLSEXT_TYPE_cookie) ||
        ...
        !WPACKET_put_bytes_u16(pkt, s->s3->group_id) ||
        !s->method->put_cipher_by_char(s->s3->tmp.new_cipher, pkt, &ciphlen)
    ...
    if (!WPACKET_allocate_bytes(pkt, hashlen, &hashval2) ||
        !ossl_assert(hashval1 == hashval2) || !WPACKET_close(pkt) ||
        !WPACKET_start_sub_packet_u8(pkt) ||
    ...
    if (!WPACKET_allocate_bytes(pkt, appcookielen, &appcookie2) ||
        !ossl_assert(appcookie1 == appcookie2) || !WPACKET_close(pkt) ||
    ...
    if (EVP_DigestSignInit(hctx, NULL, EVP_sha256(), NULL, pkey) <= 0 ||
        EVP_DigestSign(hctx, hmac, &hmaclen, cookie, totcookielen) <= 0)
    {
        SSLfatal(s, SSL_AD_INTERNAL_ERROR, SSL_F_TLS_CONSTRUCT_STOC_COOKIE,
                 ERR_R_INTERNAL_ERROR);
        goto err;
    ...
    if (!WPACKET_allocate_bytes(pkt, hmaclen, &hmac2) ||
        ...
        !ossl_assert(cookie == hmac - totcookielen) || !WPACKET_close(pkt) ||
        !WPACKET_close(pkt))
    {
        SSLfatal(s, SSL_AD_INTERNAL_ERROR, SSL_F_TLS_CONSTRUCT_STOC_COOKIE,
                 ERR_R_INTERNAL_ERROR);
                                         ...
                                         size_t chainidx)
{
    ...
    if (!WPACKET_put_bytes_u16(pkt, TLSEXT_TYPE_early_data) ||
        !WPACKET_start_sub_packet_u16(pkt) || !WPACKET_close(pkt))
    {
                                  ...
                                  X509* x, size_t chainidx)
{
    ...
    if (!WPACKET_put_bytes_u16(pkt, TLSEXT_TYPE_psk) ||
        ...
        !WPACKET_close(pkt))
    {
        SSLfatal(s, SSL_AD_INTERNAL_ERROR, SSL_F_TLS_CONSTRUCT_STOC_PSK,
                 ERR_R_INTERNAL_ERROR);
        return EXT_RETURN_FAIL;
    }
