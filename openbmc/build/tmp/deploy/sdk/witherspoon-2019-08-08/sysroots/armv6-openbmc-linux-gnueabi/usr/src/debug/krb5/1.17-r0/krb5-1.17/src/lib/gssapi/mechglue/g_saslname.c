...
static OM_uint32 oidToSaslName(OM_uint32* minor, const gss_OID mech,
                               char sasl_name[OID_SASL_NAME_LENGTH + 1])
{
    ...
    *minor =
        krb5_k_make_checksum_iov(NULL, CKSUMTYPE_NIST_SHA, NULL, 0, iov, 3);
    if (*minor != 0)
...
static OM_uint32 oidToSaslNameAlloc(OM_uint32* minor, const gss_OID mech,
    ...
    gss_buffer_t mech_description)
{
    ...
    *minor_status = 0;
    ...
    if (status == GSS_S_UNAVAILABLE)
    {
        if (sasl_mech_name != GSS_C_NO_BUFFER)
            status =
                oidToSaslNameAlloc(minor_status, desired_mech, sasl_mech_name);
        else
    ...
    gss_OID* mech_type)
{
    ...
    *minor_status = 0;
    ...
    for (i = 0, status = GSS_S_BAD_MECH; i < mechSet->count; i++)
    {
        ...
        {
            status = mech->gss_inquire_mech_for_saslname(
        ...
        if (status == GSS_S_BAD_MECH &&
            ...
            oidToSaslName(&tmpMinor, &mechSet->elements[i], mappedName) ==
                GSS_S_COMPLETE &&
            memcmp(sasl_mech_name->value, mappedName, OID_SASL_NAME_LENGTH) ==
                0)
        {
            if (mech_type != NULL)
