...
static size_t is_dst(const char* input, const char* ref)
{
    ...
    if (strncmp(input, ref, rlen) != 0 || (is_curly && input[rlen] != '}') ||
        ((input[rlen] >= 'A' && input[rlen] <= 'Z') ||
         (input[rlen] >= 'a' && input[rlen] <= 'z') ||
         (input[rlen] >= '0' && input[rlen] <= '9') || (input[rlen] == '_')))
        return 0;
        ...
        if (__glibc_unlikely(*input == '$'))
        {
            ...
            if ((len = is_dst(input, "ORIGIN")) != 0)
            {
               ...
               [$ORIGIN/../$LIB/somedir].  */
                ...
                check_for_trusted =
                    (__libc_enable_secure && l->l_type == lt_executable);
            }
...
static struct r_search_path_elem**
    fillin_rpath(char* rpath, struct r_search_path_elem** result,
                 const char* sep, const char* what, const char* where,
                 struct link_map* l)
{
    char* cp;
    size_t nelems = 0;
    ...
    while ((cp = __strsep(&rpath, sep)) != NULL)
    {
        ...
        else
        {
            ...
            dirp = (struct r_search_path_elem*)malloc(
                sizeof(*dirp) + ncapstr * sizeof(enum r_dir_status) +
                where_len + len + 1);
            if (dirp == NULL)
...
static bool decompose_rpath(struct r_search_path_struct* sps, const char* rpath,
                            struct link_map* l, const char* what)
{
    ...
    result =
        (struct r_search_path_elem**)malloc((nelems + 1 + 1) * sizeof(*result));
    if (result == NULL)
    {
        free(copy);
...
static bool cache_rpath(struct link_map* l, struct r_search_path_struct* sp,
                        int tag, const char* what)
{
    if (sp->dirs == (void*)-1)
        return false;
    ...
    return decompose_rpath(
        sp,
        (const char*)(D_PTR(l, l_info[DT_STRTAB]) + l->l_info[tag]->d_un.d_val),
        l, what);
}

void _dl_init_paths(const char* llp)
{
    size_t idx;
    const char* strp;
    ...
    aelem = rtld_search_dirs.dirs = (struct r_search_path_elem**)malloc(
        (nsystem_dirs_len + 1) * sizeof(struct r_search_path_elem*));
    ...
    *aelem = NULL;
    ...
    if (llp != NULL && *llp != '\0')
    {
       ...
       elements it has.  */
        ...
        env_path_list.dirs = (struct r_search_path_elem**)malloc(
            (nllp + 1) * sizeof(struct r_search_path_elem*));
        ...
        (void)fillin_rpath(llp_tmp, env_path_list.dirs, ":;", "LD_LIBRARY_PATH",
                           NULL, l);
...
#define MAP_ANON 0
    ...
    if (r->r_state == RT_CONSISTENT)
    {
        ...
        if ((mode & __RTLD_AUDIT) == 0 && __glibc_unlikely(GLRO(dl_naudit) > 0))
        {
...

    {
        ...
        for (ph = phdr; ph < &phdr[l->l_phnum]; ++ph)
            switch (ph->p_type)
            {
                    /* These entries tell us where to find things once the
                       file's segments are mapped in.  We record the addresses
                       it says
                       verbatim, and later correct for the run-time load
                       address.  */
                case PT_DYNAMIC:
                    if (ph->p_filesz)
                    {
                ...
                case PT_LOAD:
                     ...
                     */
                    if (__glibc_unlikely(
                            (ph->p_align & (GLRO(dl_pagesize) - 1)) != 0))
                    {
                        errstring =
                    ...
                    if (__glibc_unlikely(((ph->p_vaddr - ph->p_offset) &
                                          (ph->p_align - 1)) != 0))
                    {
                        errstring = N_("ELF load command address/offset not "
                                       "properly aligned");
                        goto call_lose;
                    }
                ...
                case PT_TLS:
                    ...
                    if (__glibc_likely(l->l_type == lt_library)
                        /* If GL(dl_tls_dtv_slotinfo_list) == NULL, then rtld.c
                       did not set up TLS data structures, so don't use them
                       now.  */
...
#else
                    assert(false &&
                           "TLS not initialized in static application");
#endif
                    break;
    ...
    if (__glibc_unlikely(l->l_flags_1 & DF_1_NOOPEN) && (mode & __RTLD_DLOPEN))
    {
    ...
    if (l->l_phdr == NULL)
    {
       ...
       temporary place.  */
        ElfW(Phdr)* newp =
            (ElfW(Phdr)*)malloc(header->e_phnum * sizeof(ElfW(Phdr)));
        if (newp == NULL)
        {
    ...
    if (__glibc_unlikely((stack_flags & ~GL(dl_stack_flags)) & PF_X))
    {
       ...
       the mprotect calls.  */
        ...
        if ((mode & (__RTLD_DLOPEN | __RTLD_AUDIT)) == __RTLD_DLOPEN)
        {
            ...
            const uintptr_t relro_end =
                ((m->l_addr + m->l_relro_addr + m->l_relro_size) &
                 -GLRO(dl_pagesize));
...
static int open_verify(const char* name, int fd, struct filebuf* fbp,
                       struct link_map* loader, int whatcode, int mode,
...
#define VALID_ELF_ABIVERSION(osabi, ver) (ver == 0)
    ...
    if (fd != -1)
    {
       ...
       we can use.  */
        ...
        {
            errval = errno;
            errstring = (errval == 0 ? N_("file too short")
        ...
        if (__glibc_unlikely(
                !VALID_ELF_HEADER(ehdr->e_ident, expected, EI_ABIVERSION) ||
                       ...
                       EI_NIDENT - EI_PAD) != 0))
        {
            ...
            if (*magp !=
                ...
                ((ELFMAG0 << (EI_MAG0 * 8)) | (ELFMAG1 << (EI_MAG1 * 8)) |
                 (ELFMAG2 << (EI_MAG2 * 8)) | (ELFMAG3 << (EI_MAG3 * 8)))
#else
                ((ELFMAG0 << (EI_MAG3 * 8)) | (ELFMAG1 << (EI_MAG2 * 8)) |
                 (ELFMAG2 << (EI_MAG1 * 8)) | (ELFMAG3 << (EI_MAG0 * 8)))
#endif
            )
        ...
        if (__glibc_unlikely(elf_machine_reject_phdr_p(
                phdr, ehdr->e_phnum, fbp->buf, fbp->len, loader, fd)))
            goto close_and_out;
        ...
        for (ph = phdr; ph < &phdr[ehdr->e_phnum]; ++ph)
            if (ph->p_type == PT_NOTE && ph->p_filesz >= 32 && ph->p_align >= 4)
            {
                ...
                {
                    ElfW(Addr) note_size =
                        ELF_NOTE_NEXT_OFFSET(abi_note[0], abi_note[1], align);
                ...
                osversion = (abi_note[5] & 0xff) * 65536 +
                            (abi_note[6] & 0xff) * 256 + (abi_note[7] & 0xff);
    ...
    do
    {
       ...
       now if it hasn't happened now.  */
        ...
        for (cnt = 0; fd == -1 && cnt < ncapstr; ++cnt)
        {
            ...
            buflen = ((char*)__mempcpy(
                          __mempcpy(edp, capstr[cnt].str, capstr[cnt].len),
                          name, namelen) -
                      buf);
...
struct link_map* _dl_map_object(struct link_map* loader, const char* name,
                                int type, int trace_mode, int mode, Lmid_t nsid)
{
    ...
    if (strchr(name, '/') == NULL)
    {
       ...
       RPATHs.  */
        if (loader == NULL || loader->l_info[DT_RUNPATH] == NULL)
        {
            ...
            if (fd == -1 && !did_main_map && main_map != NULL &&
        ...
        if (fd == -1 && env_path_list.dirs != (void*)-1)
            fd = open_path(name, namelen, mode, &env_path_list, &realname, &fb,
        ...
        if (fd == -1)
        {
            ...
            {
                fd = open_verify(
                    realname, fd, &fb, loader ?: GL(dl_ns)[nsid]._ns_loaded,
            ...
            __glibc_likely(GLRO(dl_inhibit_cache) == 0))
        {
            ...
            if (cached != NULL)
            {
               ...
               use a cache entry from any of these directories.  */
                ...
                {
                    fd = open_verify(
                        cached, -1, &fb, loader ?: GL(dl_ns)[nsid]._ns_loaded,
                        LA_SER_CONFIG, mode, &found_other_class, false);
        ...
        if (fd == -1 &&
            ...
            fd = open_path(name, namelen, mode, &rtld_search_dirs, &realname,
                           &fb, l, LA_SER_DEFAULT, &found_other_class);
    ...
    else
    {
        ...
        realname = (loader ? expand_dynamic_string_token(loader, name)
                           : __strdup(name));
        if (realname == NULL)
            fd = -1;
        else
    ...
    if (__glibc_unlikely(fd == -1))
    {
            ...
            __glibc_likely((GLRO(dl_debug_mask) & DL_DEBUG_PRELINK) == 0))
        {
            ...
            if ((name_copy = __strdup(name)) == NULL ||
                (l = _dl_new_object(name_copy, name, type, loader, mode,
                                    nsid)) == NULL)
            {
                free(name_copy);
...
static void add_path(struct add_path_state* p,
                     const struct r_search_path_struct* sps, unsigned int flags)
{
    if (sps->dirs != (void*)-1)
    {
        struct r_search_path_elem** dirs = sps->dirs;
            ...
            else
            {
                ...
                if (r->dirnamelen < 2)
                    *p->allocptr++ = r->dirnamelen ? '/' : '.';
                    ...
                    p->allocptr =
                        __mempcpy(p->allocptr, r->dirname, r->dirnamelen - 1);
                *p->allocptr++ = '\0';
                sp->dls_flags = flags;
            }
        } while (*dirs != NULL);
...
void _dl_rtld_di_serinfo(struct link_map* loader, Dl_serinfo* si, bool counting)
{
    ...
    struct add_path_state p = {.counting = counting,
                               .idx = 0,
                               .si = si,
                               .allocptr =
