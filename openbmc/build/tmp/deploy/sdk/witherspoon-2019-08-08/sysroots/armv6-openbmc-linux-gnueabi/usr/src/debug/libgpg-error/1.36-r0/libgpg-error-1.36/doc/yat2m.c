...
static const char* const standard_sections[] = {
    "NAME",           "SYNOPSIS", "DESCRIPTION", "RETURN VALUE", "EXIT STATUS",
    "ERROR HANDLING", "ERRORS",   "COMMANDS",    "OPTIONS",      "USAGE",
    "EXAMPLES",       "FILES",    "ENVIRONMENT", "DIAGNOSTICS",  "SECURITY",
    "CONFORMING TO",  "ASSUAN",   "NOTES",       "BUGS",         "AUTHOR",
    "SEE ALSO",       NULL};
...
static char* ascii_strupr(char* string)
{
    char* p;
...
const char* isodatestring(void)
{
    ...
    {
        tp = gmtime(&atime);
        sprintf(buffer, "%04d-%02d-%02d", 1900 + tp->tm_year, tp->tm_mon + 1,
                tp->tm_mday);
    }
    return buffer;
}
...
/* Add NAME to the list of predefined macros which are global for all
   files.  */
static void add_predefined_macro(const char* name)
{
    macro_t m;
...
/* Create or update a macro with name MACRONAME and set its values TO
   ...
   to this function.  */
static void set_macro(const char* macroname, char* macrovalue)
{
    macro_t m;
...
static void set_variable(char* nameandvalue)
{
    macro_t m;
    const char* value;
    char* p;
...
static int macro_set_p(const char* name)
{
    macro_t m;
...
static void evaluate_conditions(const char* fname, int lnr)
{
    int i;
...
static void push_condition(const char* name, int isset, const char* fname,
                           int lnr)
{
    condition_t cond;
    int manverb = 0;
...
/* Remove the last condition from the stack.  ISSET is used for error
   reporting.  */
static void pop_condition(int isset, const char* fname, int lnr)
{
    if (!condition_stack_idx)
    {
        err("%s:%d: unbalanced \"@end %s\"", fname, lnr,
            isset ? "isset" : "isclear");
        return;
    }
    condition_stack_idx--;
...
static section_buffer_t get_section_buffer(const char* name)
{
    int i;
    ...
    else
    {
        ...
        {
            thepage.sections = xrealloc(
                thepage.sections, ((old_n + new_n) * sizeof *thepage.sections));
...
static void add_content(const char* sectname, char* line, int verbatim)
{
    section_buffer_t sect;
    line_buffer_t lb;
...
static void start_page(char* name)
{
    if (verbose)
...
static int write_th(FILE* fp)
{
    ...
    *p++ = 0;
    fprintf(fp, ".TH %s %s %s \"%s\" \"%s\"\n", name, p, isodatestring(),
            opt_release, opt_source);
    free(name);
    return 0;
}
...
static size_t proc_texi_cmd(FILE* fp, const char* command, const char* rest,
                            size_t len, int* table_level, int* eol_action)
{
    static struct
    {
        ...
        switch (cmdtbl[i].what)
        {
            ...
            case 8:
                ...
                else
                {
                    ...
                    if (*s != '}')
                    {
                        err("closing brace for command '%s' not found",
                            command);
                        return len;
                    }
                    else
                    {
                        ...
                        if (m)
                            ...
                            inf("texinfo variable '%.*s' is not set", (int)rlen,
                                rest + 1);
                    }
                }
                break;
    ...
    else /* macro */
    {
        ...
        if (m)
        {
            proc_texi_buffer(fp, m->value, strlen(m->value), table_level,
                             eol_action);
...
static void parse_texi_line(FILE* fp, const char* line, int* table_level)
{
    int eol_action = 0;
...
static void write_content(FILE* fp, line_buffer_t lines)
{
    line_buffer_t line;
    int table_level = 0;
...
static int is_standard_section(const char* name)
{
    int i;
    const char* s;
...
static void parse_file(const char* fname, FILE* fp, char** section_name,
                       int in_pause)
{
    char* line;
    int lnr = 0;
    ...
    while (fgets(line, LINESIZE, fp))
    {
        ...
        if (*line == '@')
        {
            if (skip_to_end && n == 4 && !memcmp(line, "@end", 4) &&
                (line[4] == ' ' || line[4] == '\t' || !line[4]))
            {
                skip_to_end = 0;
            }
                     ...
                     (line[8] == ' ' || line[8] == '\t' || !line[8]))
            {
                ...
                {
                    free(incname);
                    incname = xmalloc(strlen(opt_include) + 1 + strlen(p) + 1);
                ...
                if (!incfp)
                    err("can't open include file '%s': %s", incname,
                        strerror(errno));
                else
                {
...
static void top_parse_file(const char* fname, FILE* fp)
{
