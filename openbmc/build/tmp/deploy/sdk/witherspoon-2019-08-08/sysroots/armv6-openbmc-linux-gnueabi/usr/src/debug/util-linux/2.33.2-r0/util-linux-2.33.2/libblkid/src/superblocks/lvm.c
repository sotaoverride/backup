...
static int probe_lvm2(blkid_probe pr, const struct blkid_idmag* mag)
{
    ...
    blkid_probe_sprintf_uuid(pr, label->pv_uuid, sizeof(label->pv_uuid), "%s",
                             uuid);
...
const struct blkid_idinfo lvm2_idinfo = {
    ...
    .magics = {{.magic = "LVM2 001", .len = 8, .sboff = 0x218},
