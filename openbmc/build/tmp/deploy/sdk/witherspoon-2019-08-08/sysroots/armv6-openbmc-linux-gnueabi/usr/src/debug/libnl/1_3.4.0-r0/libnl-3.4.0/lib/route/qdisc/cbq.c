...
static const struct trans_tbl ovl_strategies[] = {
    __ADD(TC_CBQ_OVL_CLASSIC, classic),   __ADD(TC_CBQ_OVL_DELAY, delay),
    __ADD(TC_CBQ_OVL_LOWPRIO, lowprio),   __ADD(TC_CBQ_OVL_DROP, drop),
...
static int cbq_msg_parser(struct rtnl_tc* tc, void* data)
{
    ...
    nla_memcpy(&cbq->cbq_ovl, tb[TCA_CBQ_OVL_STRATEGY], sizeof(cbq->cbq_ovl));
    nla_memcpy(&cbq->cbq_police, tb[TCA_CBQ_POLICE], sizeof(cbq->cbq_police));
                          ...
                          struct nl_dump_params* p)
{
    ...
    nl_dump(p, " rate %.2f%s/s (%.0f%s) prio %u", r, ru, rbit, rubit,
            cbq->cbq_wrr.priority);
}
                             ...
                             struct nl_dump_params* p)
{
    ...
    nl_dump(p, "avgpkt %u mpu %u cell %u allot %u weight %.0f%s\n",
            cbq->cbq_lss.avpkt, cbq->cbq_rate.mpu, 1 << cbq->cbq_rate.cell_log,
    ...
    nl_dump_line(p,
                 ...
                 nl_ticks2us(cbq->cbq_lss.offtime >> el), cbq->cbq_lss.level,
                           ...
                           struct nl_dump_params* p)
{
    ...
    nl_dump_line(p, "         %10u %10u %10u %10u\n", x->borrows,
                 x->overactions, x->avgidle, x->undertime);
}
