...
/*!Memory chain: A container holding memory portions allocated by
   boost_cont_multialloc_nodes and boost_cont_multialloc_arrays functions.*/
...
/*!Advances the iterator one position so that it points to the next element in
 * the memory chain*/
#define BOOST_CONTAINER_MEMIT_NEXT(IT)                                         \
    (IT.node_ptr = IT.node_ptr->next_node_ptr)
...
/*!Initializer for an iterator pointing to the position before the first
 * element*/
#define BOOST_CONTAINER_MEMCHAIN_BEFORE_BEGIN_IT(PMEMCHAIN)                    \
    {                                                                          \
        &((PMEMCHAIN)->root_node)                                              \
    }

/*!Initializer for an iterator pointing to the first element*/
#define BOOST_CONTAINER_MEMCHAIN_BEGIN_IT(PMEMCHAIN)                           \
    {                                                                          \
        (PMEMCHAIN)->root_node.next_node_ptr                                   \
    }

/*!Initializer for an iterator pointing to the last element*/
#define BOOST_CONTAINER_MEMCHAIN_LAST_IT(PMEMCHAIN)                            \
    {                                                                          \
        (PMEMCHAIN)->last_node_ptr                                             \
    }

/*!Initializer for an iterator pointing to one past the last element (end
 * iterator)*/
#define BOOST_CONTAINER_MEMCHAIN_END_IT(PMEMCHAIN)                             \
    {                                                                          \
        (boost_cont_memchain_node*)0                                           \
    }
...
#define BOOST_CONTAINER_MEMCHAIN_FIRSTMEM(PMEMCHAIN)                           \
    ((void*)((PMEMCHAIN)->root_node.next_node_ptr))

/*!The address of the last memory portion hold by the memory chain*/
#define BOOST_CONTAINER_MEMCHAIN_LASTMEM(PMEMCHAIN)                            \
    ((void*)((PMEMCHAIN)->last_node_ptr))
...
/*!Default initializes a memory chain. Postconditions: begin iterator is end
   iterator, the number of portions is zero.*/
#define BOOST_CONTAINER_MEMCHAIN_INIT(PMEMCHAIN)                               \
    ((PMEMCHAIN)->root_node.next_node_ptr = 0,                                 \
     (PMEMCHAIN)->last_node_ptr = &((PMEMCHAIN)->root_node),                   \
     (PMEMCHAIN)->num_mem = 0) /**/
...
#define BOOST_CONTAINER_MEMCHAIN_PUSH_BACK(PMEMCHAIN, MEM)                     \
    ...
    {                                                                          \
        ...
        boost_cont_memchain_node* ____tmp_mem____ =                            \
            (boost_cont_memchain_node*)(MEM);                                  \
...
#define BOOST_CONTAINER_MEMCHAIN_PUSH_FRONT(PMEMCHAIN, MEM)                    \
    ...
    {                                                                          \
        ...
        boost_cont_memchain_node* ____old_first____ =                          \
            ____root____->next_node_ptr;                                       \
        ____tmp_mem____->next_node_ptr = ____old_first____;                    \
        ____root____->next_node_ptr = ____tmp_mem____;                         \
        ++____chain____->num_mem;                                              \
    } while (0) /**/

/*!Erases the memory portion after the portion pointed by BEFORE_IT from the
 * memory chain*/
/*!Precondition: BEFORE_IT must be a valid iterator of the memory chain and it
 * can't be the end iterator*/
#define BOOST_CONTAINER_MEMCHAIN_ERASE_AFTER(PMEMCHAIN, BEFORE_IT)             \
    ...
    {                                                                          \
        ...
        boost_cont_memchain_node* ____erase_node____ =                         \
            ____prev_node____->next_node_ptr;                                  \
...
#define BOOST_CONTAINER_MEMCHAIN_POP_FRONT(PMEMCHAIN)                          \
    ...
    {                                                                          \
        boost_cont_memchain* ____chain____ = (PMEMCHAIN);                      \
        boost_cont_memchain_node* ____prev_node____ =                          \
            &____chain____->root_node;                                         \
        boost_cont_memchain_node* ____erase_node____ =                         \
            ____prev_node____->next_node_ptr;                                  \
        if (____chain____->last_node_ptr == ____erase_node____)                \
        {                                                                      \
...
/*!Joins two memory chains inserting the portions of the second chain at the
 * back of the first chain*/
/*
#define BOOST_CONTAINER_MEMCHAIN_SPLICE_BACK(PMEMCHAIN, PMEMCHAIN2)\
   do{\
      ...
      if(!____chain2____->root_node.next_node_ptr){\
         ...
         ____chain____->last_node_ptr->next_node_ptr =
____chain2____->first_mem;\
         ____chain____->last_node_ptr = ____chain2____->last_node_ptr;\
         ____chain____->num_mem += ____chain2____->num_mem;\
      }\
   }while(0)\*/
...
/*!Joins two memory chains inserting the portions of the second chain at the
 * back of the first chain*/
#define BOOST_CONTAINER_MEMCHAIN_INCORPORATE_AFTER(PMEMCHAIN, BEFORE_IT,       \
                                                   FIRST, BEFORELAST, NUM)     \
    do                                                                         \
    {                                                                          \
        boost_cont_memchain* ____chain____ = (PMEMCHAIN);                      \
        ...
        boost_cont_memchain_node* ____first____ =                              \
            (boost_cont_memchain_node*)(FIRST);                                \
        boost_cont_memchain_node* ____blast____ =                              \
            (boost_cont_memchain_node*)(BEFORELAST);                           \
...
/*!Indicates the all elements allocated by boost_cont_multialloc_nodes or
   boost_cont_multialloc_arrays must be contiguous.*/
#define DL_MULTIALLOC_ALL_CONTIGUOUS ((size_t)(-1))

/*!Indicates the number of contiguous elements allocated by
   boost_cont_multialloc_nodes or boost_cont_multialloc_arrays should be
   selected by those functions.*/
...
enum
{
    ...
    BOOST_CONTAINER_EXPAND_BOTH =
        BOOST_CONTAINER_EXPAND_FWD | BOOST_CONTAINER_EXPAND_BWD,
    BOOST_CONTAINER_EXPAND_OR_NEW =
        BOOST_CONTAINER_ALLOCATE_NEW | BOOST_CONTAINER_EXPAND_BOTH
};
...
enum
{
    BOOST_CONTAINER_ALLOCATION_PAYLOAD = sizeof(size_t) * 2
};
#endif
...
boost_cont_command_ret_t boost_cont_allocation_command(
    allocation_type command, size_t sizeof_object, size_t limit_objects,
    size_t preferred_objects, size_t* received_objects, void* reuse_ptr);
