...
void neighbor_free(Neighbor* neighbor)
{
    ...
    if (neighbor->network)
    {
        ...
        if (neighbor->section)
        {
            hashmap_remove(neighbor->network->neighbors_by_section,
                           neighbor->section);
...
static int neighbor_new_static(Network* network, const char* filename,
                               unsigned section_line, Neighbor** ret)
{
    ...
    *neighbor = (Neighbor){
    ...
    if (filename)
    {
        ...
        r = hashmap_ensure_allocated(&network->neighbors_by_section,
                                     &network_config_hash_ops);
        if (r < 0)
            return r;

        r = hashmap_put(network->neighbors_by_section, neighbor->section,
                        neighbor);
        if (r < 0)
            return r;
    }
...
int neighbor_configure(Neighbor* neighbor, Link* link,
                       link_netlink_message_handler_t callback)
{
    ...
    r = sd_netlink_message_set_flags(req, NLM_F_REQUEST | NLM_F_CREATE |
                                              NLM_F_REPLACE);
    if (r < 0)
    ...
    r = netlink_message_append_in_addr_union(req, NDA_DST, neighbor->family,
                                             &neighbor->in_addr);
    if (r < 0)
        return log_error_errno(r, "Could not append NDA_DST attribute: %m");

    r = netlink_call_async(link->manager->rtnl, NULL, req,
                           callback ?: neighbor_handler,
...
int config_parse_neighbor_address(const char* unit, const char* filename,
                                  ...
                                  void* userdata)
{
    ...
    if (r < 0)
    {
        log_syntax(unit, LOG_ERR, filename, line, r,
                   "Neighbor Address is invalid, ignoring assignment: %s",
                   rvalue);
        return 0;
    }
...
int config_parse_neighbor_hwaddr(const char* unit, const char* filename,
                                 unsigned line, const char* section,
                                 unsigned section_line, const char* lvalue,
                                 int ltype, const char* rvalue, void* data,
                                 void* userdata)
{
    ...
    {
        log_syntax(unit, LOG_ERR, filename, line, r,
                   "Neighbor MACAddress is invalid, ignoring assignment: %s",
                   rvalue);
        return 0;
    }
