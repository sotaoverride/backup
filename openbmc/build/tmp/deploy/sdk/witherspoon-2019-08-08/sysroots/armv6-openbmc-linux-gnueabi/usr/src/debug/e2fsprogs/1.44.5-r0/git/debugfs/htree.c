    ...
    while (offset < fs->blocksize)
    {
        ...
        if (errcode)
        {
            com_err("htree_dump_leaf_inode", errcode,
                    "while getting rec_len for block %lu", (unsigned long)blk);
            return;
        }
        thislen = ext2fs_dirent_name_len(dirent);
        if (((offset + rec_len) > fs->blocksize) || (rec_len < 8) ||
            ((rec_len % 4) != 0) || ((unsigned)thislen + 8 > rec_len))
        {
        ...
        name[thislen] = '\0';
        errcode = ext2fs_dirhash(hash_alg, name, thislen,
                                 fs->super->s_hash_seed, &hash, &minor_hash);
        if (errcode)
            com_err("htree_dump_leaf_node", errcode, "while calculating hash");
        if ((offset == fs->blocksize - csum_size) && (dirent->inode == 0) &&
            ...
            (dirent->name_len == EXT2_DIR_NAME_LEN_CSUM))
        {
            ...
            snprintf(tmp, EXT2_NAME_LEN + 64, "leaf block checksum: 0x%08x  ",
                     t->det_checksum);
        }
        else
        {
...
static void htree_dump_int_node(ext2_filsys fs, ext2_ino_t ino,
                                struct ext2_inode* inode,
                                struct ext2_dx_root_info* rootnode,
                                struct ext2_dx_entry* ent, char* buf, int level)
{
    ...
    for (i = 0; i < count; i++)
    {
        ...
        fprintf(pager, "Entry #%d: Hash 0x%08x%s, block %u\n", i, hash,
                (hash & 1) ? " (**)" : "", ext2fs_le32_to_cpu(ent[i].block));
    }
    ...
    for (i = 0; i < count; i++)
    {
        ...
        fprintf(pager, "Entry #%d: Hash 0x%08x, block %u\n", i, i ? hashval : 0,
                block);
        if (level)
            htree_dump_int_block(fs, ino, inode, rootnode, block, buf,
                                 level - 1);
        else
            htree_dump_leaf_node(fs, ino, inode, rootnode, block, buf);
    }
                                 ...
                                 blk64_t blk, char* buf, int level)
{
    ...
    if (errcode)
    {
        com_err("htree_dump_int_block", errcode, "while 	reading block %llu\n",
                blk);
        goto errout;
    }

    htree_dump_int_node(fs, ino, inode, rootnode,
                        (struct ext2_dx_entry*)(buf + 8), cbuf, level);
errout:
    free(cbuf);
}
...
void do_htree_dump(int argc, char* argv[])
{
    ...
    if (errcode)
    {
        com_err("do_htree_block", errcode, "while mapping logical block 0\n");
        goto errout;
    }

    errcode = io_channel_read_blk64(current_fs->io, blk, 1, buf);
    if (errcode)
    {
    ...
    htree_dump_int_node(current_fs, ino, &inode, rootnode, ent,
                        buf + current_fs->blocksize, rootnode->indirect_levels);
                            ...
                            void* priv_data)
{
    ...
    if (errcode)
    {
        com_err("search_dir_block", errcode, "while reading block %lu",
    ...
    while (offset < fs->blocksize)
    {
        ...
        if (dirent->inode && p->len == ext2fs_dirent_name_len(dirent) &&
            strncmp(p->search_name, dirent->name, p->len) == 0)
        {
            printf("Entry found at logical block %lld, "
                   "phys %llu, offset %u\n",
                   (long long)blockcnt, *blocknr, offset);
            printf("offset %u\n", offset);
            return BLOCK_ABORT;
        }
        offset += rec_len;
