...
BOOST_LOG_ANONYMOUS_NAMESPACE
{
    ...
    //! Sequence shuffling algorithm. Very similar to std::random_shuffle, used
    //! for forward portability with compilers that removed it from the standard
    //! library (C++17).
...
struct record_view::private_data : public public_data
{
  ...
  private:
    ...
    //! The flag indicates that the record has to be detached from the current
    //! thread
    bool m_detach_from_thread_needed;

  private:
    //! Initializing constructor
    private_data(BOOST_RV_REF(attribute_value_set) values,
                 uint32_t capacity) BOOST_NOEXCEPT
  ...
  public:
    ...
    static private_data* create(BOOST_RV_REF(attribute_value_set) values,
                                uint32_t capacity)
    {
        private_data* p =
            reinterpret_cast<private_data*>(stateless_allocator().allocate(
                sizeof(private_data) +
                boost::log::aux::alignment_gap_between<private_data,
                                                       sink_ptr>::value +
    ...
    uint32_t accepting_sink_count() const BOOST_NOEXCEPT
    {
        return m_accepting_sink_count;
    }

    //! Returns the flag indicating whether it is needed to detach the record
    //! from the current thread
    bool is_detach_from_thread_needed() const BOOST_NOEXCEPT
    {
        return m_detach_from_thread_needed;
    }
  ...
  private:
    ...
    sink_ptr* begin() BOOST_NOEXCEPT
    {
        return reinterpret_cast<sink_ptr*>(
            reinterpret_cast<char*>(this) + sizeof(private_data) +
            boost::log::aux::alignment_gap_between<private_data,
                                                   sink_ptr>::value);
    }
};
...
//! Destructor
BOOST_LOG_API void
    record_view::public_data::destroy(const public_data* p) BOOST_NOEXCEPT
{
    const_cast<private_data*>(static_cast<const private_data*>(p))->destroy();
}

//! The function ensures that the log record does not depend on any
//! thread-specific data.
BOOST_LOG_API record_view record::lock()
{
    BOOST_ASSERT(m_impl != NULL);

    record_view::private_data* const impl =
        static_cast<record_view::private_data*>(m_impl);
    if (impl->is_detach_from_thread_needed())
    {
        attribute_value_set::const_iterator it = impl->m_attribute_values
                                                     .begin(),
                                            end =
                                                impl->m_attribute_values.end();
        for (; it != end; ++it)
        {
            // Yep, a bit hackish. I'll need a better backdoor to do it
            // gracefully.
            const_cast<attribute_value_set::mapped_type&>(it->second)
                .detach_from_thread();
        }
    }
...
struct core::implementation
    : public log::aux::lazy_singleton<implementation, core_ptr>
{
  public:
    //! Base type of singleton holder
    typedef log::aux::lazy_singleton<implementation, core_ptr> base_type;

#if !defined(BOOST_LOG_NO_THREADS)
    //! Read lock type
    typedef log::aux::shared_lock_guard<log::aux::light_rw_mutex>
        scoped_read_lock;
    //! Write lock type
    typedef log::aux::exclusive_lock_guard<log::aux::light_rw_mutex>
        scoped_write_lock;
#endif
      ...
      private:
        ...
        static uint32_t get_random_seed()
        {
            ...
            seed += static_cast<uint32_t>(
                log::aux::this_thread::get_id().native_id());
#endif
            return seed;
        }
    };
  ...
  public:
    ...
    BOOST_FORCEINLINE record open_record(BOOST_FWD_REF(SourceAttributesT)
                                             source_attributes)
    {
        ...
        if (m_enabled)
            try
            {
                ...
                // Lock the core to be safe against any attribute or sink set
                // modifications
                ...
                if (m_enabled)
                {
                    ...
                    attribute_value_set attr_values(
                        boost::forward<SourceAttributesT>(source_attributes),
                        tsd->m_thread_attributes, m_global_attributes);
                    if (m_filter(attr_values))
                    {
                        // The global filter passed, trying the sinks
                        attribute_value_set* values = &attr_values;

                        // apply_sink_filter will invoke the exception handler
                        // if it has to
                        ...
                        if (!m_sinks.empty())
                        {
                            uint32_t remaining_capacity =
                                static_cast<uint32_t>(m_sinks.size());
                            sink_list::iterator it = m_sinks.begin(),
                                                end = m_sinks.end();
                            for (; it != end; ++it, --remaining_capacity)
                            {
                                apply_sink_filter(*it, rec_impl, values,
                                                  remaining_capacity);
                            }
                        }
                        else
                        {
                            ...
                            apply_sink_filter(m_default_sink, rec_impl, values,
                                              1);
                        }
            ...
            catch (...)
            {
                ...
                {
                    // Lock the core to be safe against any attribute or sink
                    // set modifications
                    BOOST_LOG_EXPR_IF_MT(scoped_read_lock lock(m_mutex);)
  ...
  private:
    ...
    //! Invokes sink-specific filter and adds the sink to the record if the
    //! filter passes the log record
    void apply_sink_filter(shared_ptr<sinks::sink> const& sink,
                           record_view::private_data*& rec_impl,
                           attribute_value_set*& attr_values,
                           uint32_t remaining_capacity)
    {
        try
        {
            ...
            {
                // If at least one sink accepts the record, it's time to create
                // it
                record_view::private_data* impl = rec_impl;
                if (!impl)
                {
                    rec_impl = impl = record_view::private_data::create(
                        boost::move(*attr_values), remaining_capacity);
...
core::core() : m_impl(new implementation())
{
}
...
//! The method enables or disables logging and returns the previous state of
//! logging flag
BOOST_LOG_API bool core::set_logging_enabled(bool enabled)
{
    BOOST_LOG_EXPR_IF_MT(
        implementation::scoped_write_lock lock(m_impl->m_mutex);)
...
{
    // Should have a read barrier here, but for performance reasons it is
    // omitted. The function should be used as a quick check and doesn't need to
    // be reliable.
...
BOOST_LOG_API std::pair<attribute_set::iterator, bool>
    core::add_global_attribute(attribute_name const& name,
...
//! The method returns the complete set of currently registered global
//! attributes
BOOST_LOG_API attribute_set core::get_global_attributes() const
{
    BOOST_LOG_EXPR_IF_MT(
        implementation::scoped_read_lock lock(m_impl->m_mutex);)
...
BOOST_LOG_API std::pair<attribute_set::iterator, bool>
    core::add_thread_attribute(attribute_name const& name,
                               attribute const& attr)
{
...
//! The method returns the complete set of currently registered thread-specific
//! attributes
...
//! The method replaces the complete set of currently registered thread-specific
//! attributes with the provided set
...
BOOST_LOG_API void
    core::set_exception_handler(exception_handler_type const& handler)
{
    BOOST_LOG_EXPR_IF_MT(
        implementation::scoped_write_lock lock(m_impl->m_mutex);)
...
BOOST_LOG_API void core::flush()
{
    ...
    BOOST_LOG_EXPR_IF_MT(
        implementation::scoped_write_lock lock(m_impl->m_mutex);)
    implementation::sink_list::iterator it = m_impl->m_sinks.begin(),
                                        end = m_impl->m_sinks.end();
    for (; it != end; ++it)
    {
        try
        {
...
BOOST_LOG_API record
    core::open_record(attribute_value_set const& source_attributes)
{
    return m_impl->open_record(source_attributes);
}
...
//! The method attempts to open a new record to be written.
BOOST_LOG_API record
    core::open_record_move(attribute_value_set& source_attributes)
{
...
BOOST_LOG_API void core::push_record_move(record& rec)
{
    try
    {
        ...
        record_view::private_data* data =
            static_cast<record_view::private_data*>(rec_view.m_impl.get());
        ...
        record_view::private_data::sink_list weak_sinks =
            data->get_accepting_sinks();
        record_view::private_data::sink_list::iterator weak_it =
                                                           weak_sinks.begin(),
                                                       weak_end =
                                                           weak_sinks.end();
            ...
            try
            {
                ...
                if (begin != end)
                {
                    if (all_locked)
                    {
                        ...
                        if (!shuffled)
                        {
                            implementation::thread_data* tsd =
                                m_impl->get_thread_data();
