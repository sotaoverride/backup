...
class basic_iarchive_impl
{
    ...
    class cobject_id
    {
      public:
        ...
        cobject_id(const basic_iserializer& bis_) :
            bis_ptr(&bis_), bpis_ptr(NULL), file_version(0),
            tracking_level(track_never), initialized(false)
        {
        }
        cobject_id(const cobject_id& rhs) :
            bis_ptr(rhs.bis_ptr), bpis_ptr(rhs.bpis_ptr),
            file_version(rhs.file_version), tracking_level(rhs.tracking_level),
            initialized(rhs.initialized)
        {
        }
    };
    ...
    const basic_pointer_iserializer*
        load_pointer(basic_iarchive& ar, void*& t,
...
inline void
    basic_iarchive_impl::reset_object_address(void const* const new_address,
    ...
    for (; i < m_moveable_objects.end; ++i)
    {
        ...
        if (this_address > old_address)
        {
            ...
            object_id_vector[i].address = reinterpret_cast<void*>(
                reinterpret_cast<std::size_t>(new_address) +
                member_displacement);
        }
        else
        {
                                             ...
                                             const basic_iserializer& bis)
{
    ...
    serialization::state_saver<bool> ss_is_pointer(
        m_moveable_objects.is_pointer);
    ...
    m_moveable_objects.start = this_id =
        object_id_type(object_id_vector.size());
        ...
        const boost::serialization::extended_type_info& type_))
{
    ...
    // save state
    serialization::state_saver<object_id_type> w_start(
        m_moveable_objects.start);
    ...
    else
    {
        ...
        serialization::state_saver<object_id_type> w_end(
            m_moveable_objects.end);
...
BOOST_ARCHIVE_DECL void
    basic_iarchive::reset_object_address(const void* new_address,
...
BOOST_ARCHIVE_DECL void
    basic_iarchive::load_object(void* t, const basic_iserializer& bis)
{
