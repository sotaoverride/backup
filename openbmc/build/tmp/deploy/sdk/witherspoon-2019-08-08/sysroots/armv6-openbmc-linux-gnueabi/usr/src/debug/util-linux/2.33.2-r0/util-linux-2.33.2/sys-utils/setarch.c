...
static void __attribute__((__noreturn__)) usage(int archwrapper)
{
    ...
    fputs(_(" -F, --fdpic-funcptrs     makes function pointers point to "
            "descriptors\n"),
          stdout);
    fputs(_(" -I, --short-inode        turns on SHORT_INODE\n"), stdout);
    fputs(_(" -L, --addr-compat-layout changes the way virtual memory is "
            "allocated\n"),
          stdout);
    fputs(_(" -R, --addr-no-randomize  disables randomization of the virtual "
            "address space\n"),
          stdout);
    ...
    fputs(_(" -3, --3gb                limits the used address space to a "
            "maximum of 3 GB\n"),
          stdout);
    fputs(_("     --4gb                ignored (for backward compatibility "
            "only)\n"),
          stdout);
    fputs(_("     --uname-2.6          turns on UNAME26\n"), stdout);
    fputs(
        _(" -v, --verbose            say what options are being switched on\n"),
        stdout);

    if (!archwrapper)
        fputs(_("     --list               list settable architectures, and "
                "exit\n"),
              stdout);
...
static struct arch_domain* get_arch_domain(struct arch_domain* doms,
                                           const char* pers)
{
    struct arch_domain* d;
...
static void verify_arch_domain(struct arch_domain* dom, const char* wanted)
{
    ...
    if (strcmp(un.machine, dom->result_arch))
    {
        if (strcmp(dom->result_arch, "i386") ||
            (strcmp(un.machine, "i486") && strcmp(un.machine, "i586") &&
             strcmp(un.machine, "i686") && strcmp(un.machine, "athlon")))
            errx(EXIT_FAILURE, _("Kernel cannot set architecture to %s"),
                 wanted);
    }
}
...
int main(int argc, char* argv[])
{
    ...
    while ((c = getopt_long(argc, argv, "+hVv3BFILRSTXZ", longopts, NULL)) !=
           -1)
    {
        switch (c)
        {
