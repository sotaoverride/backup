...
static void ct_dump_tuples(struct nfnl_ct* ct, struct nl_dump_params* p)
{
    ...
    if (orig_src && orig_dst && reply_src && reply_dst &&
        ...
        !nl_addr_cmp(orig_src, reply_dst) && !nl_addr_cmp(orig_dst, reply_src))
        sync = 1;
...
static void ct_dump_line(struct nl_object* a, struct nl_dump_params* p)
{
    ...
    if (nfnl_ct_test_tcp_state(ct))
        nl_dump(
            p, "%s ",
            nfnl_ct_tcp_state2str(nfnl_ct_get_tcp_state(ct), buf, sizeof(buf)));
...
static void ct_dump_details(struct nl_object* a, struct nl_dump_params* p)
{
    ...
    nl_dump_line(p, "family %s ", nl_af2str(ct->ct_family, buf, sizeof(buf)));
...
static void ct_dump_stats(struct nl_object* a, struct nl_dump_params* p)
{
    ...
    if (!nfnl_ct_test_bytes(ct, 0) || !nfnl_ct_test_packets(ct, 0) ||
        !nfnl_ct_test_bytes(ct, 1) || !nfnl_ct_test_packets(ct, 1))
    {
        nl_dump_line(p, "    Statistics are not available.\n");
        nl_dump_line(
            p, "    Please set sysctl net.netfilter.nf_conntrack_acct=1\n");
...
#define CT_DIFF_ADDR(ATTR, FIELD)                                              \
    ...
    if (flags & LOOSE_COMPARISON)
        diff |=
            CT_DIFF(STATUS, (a->ct_status ^ b->ct_status) & b->ct_status_mask);
    else
...
{
    return __flags2str(flags, buf, len, status_flags, ARRAY_SIZE(status_flags));
}
...
static int ct_set_addr(struct nfnl_ct* ct, struct nl_addr* addr, int attr,
