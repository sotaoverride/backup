...
typedef void dump_data_char_t(const void* data, std::size_t size,
                              std::basic_ostream<char>& strm);
extern BOOST_LOG_API dump_data_char_t* dump_data_char;
BOOST_FORCEINLINE void dump_data(const void* data, std::size_t size,
                                 std::basic_ostream<char>& strm)
{
    (dump_data_char)(data, size, strm);
}

typedef void dump_data_wchar_t(const void* data, std::size_t size,
...
typedef void dump_data_char16_t(const void* data, std::size_t size,
...
typedef void dump_data_char32_t(const void* data, std::size_t size,
...
struct enable_dump : public enable_dump_size_based<sizeof(T), R>
{
};
...
class dump_manip
{
  ...
  public:
    ...
    dump_manip(dump_manip const& that) BOOST_NOEXCEPT : m_data(that.m_data),
                                                        m_size(that.m_size)
    {
    }
...
/*!
 * \brief Manipulator for printing binary representation of the data with a size
 * limit
 */
class bounded_dump_manip : public dump_manip
{
  ...
  public:
    bounded_dump_manip(const void* data, std::size_t size,
                       std::size_t max_size) BOOST_NOEXCEPT
        : dump_manip(data, size),
          m_max_size(max_size)
    {
    }
    bounded_dump_manip(bounded_dump_manip const& that) BOOST_NOEXCEPT
        : dump_manip(static_cast<dump_manip const&>(that)),
               ...
               bounded_dump_manip const& manip)
{
    if (strm.good())
    {
        const std::size_t size = manip.get_size(),
                          max_size = manip.get_max_size();
        if (max_size >= size)
        {
...
/*!
 * \brief Creates a stream manipulator that will output contents of a memory
 * region in hexadecimal form \param data The pointer to the beginning of the
 * region \param size The size of the region, in bytes \return The manipulator
 * that is to be put to a stream
 */
template <typename T>
inline typename aux::enable_dump<T, dump_manip>::type
...
/*!
 * \brief Creates a stream manipulator that will dump elements of an array in
 * hexadecimal form \param data The pointer to the beginning of the array \param
 * count The size of the region, in number of \c T elements \return The
 * manipulator that is to be put to a stream
 */
template <typename T>
 ...
 * \brief Creates a stream manipulator that will output contents of a memory
 * region in hexadecimal form \param data The pointer to the beginning of the
 * region \param size The size of the region, in bytes \param max_size The
 * maximum number of bytes of the region to output \return The manipulator that
 * is to be put to a stream
 */
template <typename T>
inline typename aux::enable_dump<T, bounded_dump_manip>::type
 ...
 * \brief Creates a stream manipulator that will dump elements of an array in
 * hexadecimal form \param data The pointer to the beginning of the array \param
 * count The size of the region, in number of \c T elements \param max_count The
 * maximum number of elements to output \return The manipulator that is to be
 * put to a stream
 */
template <typename T>
inline bounded_dump_manip dump_elements(T* data, std::size_t count,
                                        std::size_t max_count) BOOST_NOEXCEPT
{
    return bounded_dump_manip((const void*)data, count * sizeof(T),
                              max_count * sizeof(T));
}
