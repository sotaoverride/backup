...
// glibc < 2.3.4 declares those as macros if compiled with optimization turned
// on
#ifdef gettext
#undef gettext
#undef ngettext
...
///
/// This module provides message translation functionality, i.e. allow your
/// application to speak native language
///
/// @{
///
...
class message_format : public base_message_format<CharType>
{
  public:
    ...
    ///
    /// This function returns a pointer to the string for a message defined by a
    /// \a context and identification string \a id. Both create a single key for
    /// message lookup in a domain defined by \a domain_id.
    ///
    /// If \a context is NULL it is not considered to be a part of the key
    ///
    /// If a translated string is found, it is returned, otherwise NULL is
    /// returned
    ///
    ///
    virtual char_type const* get(int domain_id, char_type const* context,
                                 char_type const* id) const = 0;
    ///
    /// This function returns a pointer to the string for a plural message
    /// defined by a \a context and identification string \a single_id.
    ///
    /// If \a context is NULL it is not considered to be a part of the key
    ///
    /// Both create a single key for message lookup in
    /// a domain defined \a domain_id. \a n is used to pick the correct
    /// translation string for a specific number.
    ///
    /// If a translated string is found, it is returned, otherwise NULL is
    /// returned
    ///
    ///
    virtual char_type const* get(int domain_id, char_type const* context,
                                 char_type const* single_id, int n) const = 0;

    ///
    /// Convert a string that defines \a domain to the integer id used by \a get
    /// functions
    ///
    virtual int domain(std::string const& domain) const = 0;

    ///
    /// Convert the string \a msg to target locale's encoding. If \a msg is
    /// already in target encoding it would be returned otherwise the converted
    /// string is stored in temporary \a buffer and buffer.c_str() is returned.
    ///
    /// Note: for char_type that is char16_t, char32_t and wchar_t it is no-op,
    /// returns msg
    ///
    virtual char_type const* convert(char_type const* msg,
...
{
    static CharType const* cast(CharType const* msg,
...
///
/// \brief This class represents a message that can be converted to a specific
/// locale message
///
/// It holds the original ASCII string that is queried in the dictionary when
/// converting to the output string. The created string may be UTF-8, UTF-16,
/// UTF-32 or other 8-bit encoded string according to the target character type
/// and locale encoding.
///
template <typename CharType>
class basic_message
{
  public:
    typedef CharType
        char_type; ///< The character this message object is used with
    typedef std::basic_string<char_type>
        string_type; ///< The string type this object can be used with
    typedef message_format<char_type>
        facet_type; ///< The type of the facet the messages are fetched with
    ...
    ///
    /// Create a simple message from 0 terminated string. The string should
    /// exist until the message is destroyed. Generally useful with static
    /// constant strings
    ///
    explicit basic_message(char_type const* id) :
        n_(0), c_id_(id), c_context_(0), c_plural_(0)
    {
    }

    ///
    /// Create a simple plural form message from 0 terminated strings. The
    /// strings should exist until the message is destroyed. Generally useful
    /// with static constant strings.
    ///
    /// \a n is the number, \a single and \a plural are singular and plural
    /// forms of the message
    ///
    explicit basic_message(char_type const* single, char_type const* plural,
                           int n) :
        n_(n),
        c_id_(single), c_context_(0), c_plural_(plural)
    {
    }
    ...
    /// until the message is destroyed. Generally useful with static constant
    /// strings
    ///
    ...
    ///
    /// Create a simple plural form message from 0 terminated strings, with
    /// context. The strings should exist until the message is destroyed.
    /// Generally useful with static constant strings.
    ///
    /// \a n is the number, \a single and \a plural are singular and plural
    /// forms of the message
    ///
    explicit basic_message(char_type const* context, char_type const* single,
                           char_type const* plural, int n) :
        n_(n),
    ...
    explicit basic_message(string_type const& id) :
        n_(0), c_id_(0), c_context_(0), c_plural_(0), id_(id)
    {
    }
    ...
    ///
    /// \a n is the number, \a single and \a plural are single and plural forms
    /// of the message
    ///
    explicit basic_message(string_type const& single, string_type const& plural,
                           int number) :
        n_(number),
        c_id_(0), c_context_(0), c_plural_(0), id_(single), plural_(plural)
    {
    }
    ...
    explicit basic_message(string_type const& context,
                           string_type const& single, string_type const& plural,
                           int number) :
        n_(number),
        c_id_(0), c_context_(0), c_plural_(0), id_(single), context_(context),
        plural_(plural)
    {
    }
    ...
    basic_message(basic_message const& other) :
        n_(other.n_), c_id_(other.c_id_), c_context_(other.c_context_),
        c_plural_(other.c_plural_), id_(other.id_), context_(other.context_),
    ...
    ///
    /// Translate message to a string in the default global locale, using
    /// default domain
    ///
    ...
    ///
    /// Translate message to a string in the locale \a locale, using default
    /// domain
    ///
    ...
    ///
    /// Translate message to a string using locale \a locale and message domain
    /// \a domain_id
    ///
    string_type str(std::locale const& locale,
                    std::string const& domain_id) const
    {
        int id = 0;
    ...
    ///
    /// Translate message to a string using the default locale and message
    /// domain  \a domain_id
    ///
    ...
    ///
    /// Translate message to a string using locale \a loc and message domain
    /// index  \a id
    ///
    ...
    ///
    /// Translate message and write to stream \a out, using imbued locale and
    /// domain set to the stream
    ///
  ...
  private:
    ...
    char_type const* write(std::locale const& loc, int domain_id,
                           string_type& buffer) const
    {
        ...
        if (!translated)
        {
            ...
            {
                translated =
                    details::string_cast_traits<char_type>::cast(msg, buffer);
            }
        }
        return translated;
    }
...
///
/// \brief Translate a plural message form, \a single and \a plural are not
/// copied
///
template <typename CharType>
inline basic_message<CharType> translate(CharType const* single,
                                         CharType const* plural, int n)
{
    return basic_message<CharType>(single, plural, n);
}
///
/// \brief Translate a plural message from in constext, \a context, \a single
/// and \a plural are not copied
///
template <typename CharType>
inline basic_message<CharType> translate(CharType const* context,
...
inline basic_message<CharType>
    translate(std::basic_string<CharType> const& context,
              std::basic_string<CharType> const& msg)
{
    return basic_message<CharType>(context, msg);
}
///
/// \brief Translate a plural message form in constext, \a context, \a single
/// and \a plural are copied
///
template <typename CharType>
inline basic_message<CharType>
    translate(std::basic_string<CharType> const& context,
              std::basic_string<CharType> const& single,
              std::basic_string<CharType> const& plural, int n)
{
    return basic_message<CharType>(context, single, plural, n);
}
...
template <typename CharType>
inline basic_message<CharType>
    translate(std::basic_string<CharType> const& single,
              std::basic_string<CharType> const& plural, int n)
{
    return basic_message<CharType>(single, plural, n);
}
...
///
template <typename CharType>
std::basic_string<CharType> ngettext(CharType const* s, CharType const* p,
                                     int n,
                                     std::locale const& loc = std::locale())
{
...
///
template <typename CharType>
std::basic_string<CharType> dgettext(char const* domain, CharType const* id,
                                     std::locale const& loc = std::locale())
{
...
///
template <typename CharType>
std::basic_string<CharType> dngettext(char const* domain, CharType const* s,
                                      CharType const* p, int n,
                                      std::locale const& loc = std::locale())
{
...
std::basic_string<CharType>
    npgettext(CharType const* context, CharType const* s, CharType const* p,
              int n, std::locale const& loc = std::locale())
{
    return basic_message<CharType>(context, s, p, n).str(loc);
}
///
/// Translate message \a id according to locale \a loc in domain \a domain in
/// context \a context
///
template <typename CharType>
std::basic_string<CharType>
    dpgettext(char const* domain, CharType const* context, CharType const* id,
              std::locale const& loc = std::locale())
{
    return basic_message<CharType>(context, id).str(loc, domain);
}
///
/// Translate plural form according to locale \a loc in domain \a domain in
/// context \a context
///
template <typename CharType>
std::basic_string<CharType>
    dnpgettext(char const* domain, CharType const* context, CharType const* s,
               CharType const* p, int n, std::locale const& loc = std::locale())
{
...
template <>
struct BOOST_LOCALE_DECL base_message_format<wchar_t>
...
template <>
struct BOOST_LOCALE_DECL base_message_format<char16_t>
    : public std::locale::facet
{
    base_message_format(size_t refs = 0) : std::locale::facet(refs)
    {
...
template <>
struct BOOST_LOCALE_DECL base_message_format<char32_t>
    : public std::locale::facet
{
    base_message_format(size_t refs = 0) : std::locale::facet(refs)
    {
...
template <typename CharType>
std::basic_ostream<CharType>& operator<<(std::basic_ostream<CharType>& out,
                                         set_domain const& dom)
{
    int id = std::use_facet<message_format<CharType>>(out.getloc())
...
///
/// \note The returned object throws std::bad_cast if the I/O stream does not
/// have \ref message_format facet installed
///
inline
