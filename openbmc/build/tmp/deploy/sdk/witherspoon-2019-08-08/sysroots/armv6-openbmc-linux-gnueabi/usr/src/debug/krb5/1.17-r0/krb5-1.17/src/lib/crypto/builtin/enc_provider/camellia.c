                           ...
                           unsigned char* iv)
{
    ...
    {
        xorblock(iv, data);
        if (camellia_enc_blk(data, data, &CACHE(key)->enc_ctx) != camellia_good)
            abort();
                           ...
                           unsigned char* iv)
{
    ...
    for (; nblocks > 0; nblocks--, data -= BLOCK_SIZE)
    {
        if (camellia_dec_blk(data, data, &CACHE(key)->dec_ctx) != camellia_good)
            abort();
...
static krb5_error_code krb5int_camellia_encrypt(krb5_key key,
...
static krb5_error_code krb5int_camellia_decrypt(krb5_key key,
                                                const krb5_data* ivec,
                                                krb5_crypto_iov* data,
                                                size_t num_data)
{
...
static krb5_error_code camellia_init_state(const krb5_keyblock* key,
                                           krb5_keyusage usage,
                                           krb5_data* state)
{
    state->length = 16;
...
static void camellia_key_cleanup(krb5_key key)
{
