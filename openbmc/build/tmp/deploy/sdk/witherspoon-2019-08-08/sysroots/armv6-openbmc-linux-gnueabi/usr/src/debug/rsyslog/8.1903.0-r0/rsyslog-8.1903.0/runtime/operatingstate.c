...
void osf_open(void)
{
    ...
    fd_osf =
        open(fn_osf, O_WRONLY | O_CREAT | O_APPEND | O_LARGEFILE | O_CLOEXEC,
...
void ATTR_NONNULL() osf_write(const char* const tag, const char* const line)
{
    ...
    DBGPRINTF("osf: %s %s: ", tag,
              line); /* ensure everything is inside the debug log */
    ...
    len = snprintf(buf, sizeof(buf) - 1,
                   "%d%2.2d%2.2d-%2.2d%2.2d%2.2d: %-5.5s %s\n",
                   tm.tm_year + 1900, tm.tm_mon, tm.tm_mday, tm.tm_hour,
                   tm.tm_min, tm.tm_sec, tag, line);
    if (len > sizeof(buf) - 1)
    {
    ...
    {
        LogError(errno, RS_RET_IO_ERROR,
                 "error writing operating state file - line lost");
    }
}
