...
FT_DEFINE_TT_CMAP(tt_cmap0_class_rec,
                  ...
                  0, (TT_CMap_ValidateFunc)tt_cmap0_validate, /* validate */
...
FT_DEFINE_TT_CMAP(tt_cmap2_class_rec,
                  ...
                  2, (TT_CMap_ValidateFunc)tt_cmap2_validate, /* validate */
...
static FT_Int tt_cmap4_set_range(TT_CMap4 cmap, FT_UInt range_index)
{
    ...
    while (range_index < num_ranges)
    {
        ...
        /* we have to catch it                        */
        if (range_index >= num_ranges - 1 && cmap->cur_start == 0xFFFFU &&
    ...
    for (;;)
    {
        ...
        if (charcode <= end)
        {
            ...
            else
            {
                ...
                {
                    FT_UInt gindex =
                        (FT_UInt)((FT_Int)charcode + delta) & 0xFFFFU;

                    if (gindex >= (FT_UInt)face->root.num_glyphs)
                    {
                        /* we have an invalid glyph index; if there is an
                         * overflow, */
                        /* we can adjust `charcode', otherwise the whole segment
                         * is */
                        /* invalid */
                        gindex = 0;
...
static FT_UInt tt_cmap4_char_map_linear(TT_CMap cmap, FT_UInt32* pcharcode,
                                        FT_Bool next)
{
    ...
    Again:
        if (charcode <= end)
        {
            ...
            /* some fonts have an incorrect last segment; */
            /* we have to catch it                        */
            if (i >= num_segs - 1 && start == 0xFFFFU && end == 0xFFFFU)
            {
...
FT_DEFINE_TT_CMAP(tt_cmap4_class_rec,
                  ...
                  4, (TT_CMap_ValidateFunc)tt_cmap4_validate, /* validate */
...
FT_DEFINE_TT_CMAP(tt_cmap6_class_rec,
                  ...
                  6, (TT_CMap_ValidateFunc)tt_cmap6_validate, /* validate */
...
FT_DEFINE_TT_CMAP(tt_cmap8_class_rec,
                  ...
                  8, (TT_CMap_ValidateFunc)tt_cmap8_validate, /* validate */
...
FT_DEFINE_TT_CMAP(tt_cmap10_class_rec,
                  ...
                  10, (TT_CMap_ValidateFunc)tt_cmap10_validate, /* validate */
...
FT_CALLBACK_DEF(FT_Error)
tt_cmap12_init(TT_CMap12 cmap, FT_Byte* table)
{
    cmap->cmap.data = table;
...
tt_cmap12_validate(FT_Byte* table, FT_Validator valid)
{
    ...
    if (length > (FT_ULong)(valid->limit - table) ||
        ...
        length < 16 || (length - 16) / 12 < num_groups)
        FT_INVALID_TOO_SHORT;
...
static FT_UInt tt_cmap12_char_map_binary(TT_CMap cmap, FT_UInt32* pchar_code,
                                         FT_Bool next)
{
    FT_UInt gindex = 0;
...
FT_DEFINE_TT_CMAP(tt_cmap12_class_rec,
                  ...
                  12, (TT_CMap_ValidateFunc)tt_cmap12_validate, /* validate */
...
FT_CALLBACK_DEF(FT_Error)
tt_cmap13_init(TT_CMap13 cmap, FT_Byte* table)
{
    cmap->cmap.data = table;
...
FT_CALLBACK_DEF(FT_Error)
tt_cmap13_validate(FT_Byte* table, FT_Validator valid)
{
    FT_Byte* p;
    FT_ULong length;
    FT_ULong num_groups;
...
/* cmap->cur_group should be set up properly by caller         */
/*                                                             */
static void tt_cmap13_next(TT_CMap13 cmap)
{
    FT_Face face = cmap->cmap.cmap.charmap.face;
    FT_Byte* p;
...
static FT_UInt tt_cmap13_char_map_binary(TT_CMap cmap, FT_UInt32* pchar_code,
                                         FT_Bool next)
{
    FT_UInt gindex = 0;
    FT_Byte* p = cmap->data + 12;
...
FT_CALLBACK_DEF(FT_UInt)
tt_cmap13_char_index(TT_CMap cmap, FT_UInt32 char_code)
{
    return tt_cmap13_char_map_binary(cmap, &char_code, 0);
}

FT_CALLBACK_DEF(FT_UInt32)
tt_cmap13_char_next(TT_CMap cmap, FT_UInt32* pchar_code)
{
    TT_CMap13 cmap13 = (TT_CMap13)cmap;
    FT_UInt gindex;
...
FT_CALLBACK_DEF(FT_Error)
tt_cmap13_get_info(TT_CMap cmap, TT_CMapInfo* cmap_info)
{
    FT_Byte* p = cmap->data + 8;
...
FT_DEFINE_TT_CMAP(tt_cmap13_class_rec,
                  ...
                  13, (TT_CMap_ValidateFunc)tt_cmap13_validate, /* validate */
...
static FT_Error tt_cmap14_ensure(TT_CMap14 cmap, FT_UInt32 num_results,
                                 FT_Memory memory)
{
...
FT_CALLBACK_DEF(FT_Error)
tt_cmap14_init(TT_CMap14 cmap, FT_Byte* table)
{
    cmap->cmap.data = table;
...
FT_CALLBACK_DEF(FT_Error)
tt_cmap14_validate(FT_Byte* table, FT_Validator valid)
{
    FT_Byte* p;
    FT_ULong length;
    FT_ULong num_selectors;
    ...
    if (length > (FT_ULong)(valid->limit - table) ||
        ...
        length < 10 || (length - 10) / 11 < num_selectors)
        FT_INVALID_TOO_SHORT;
...
FT_CALLBACK_DEF(FT_UInt)
tt_cmap14_char_index(TT_CMap cmap, FT_UInt32 char_code)
{
    FT_UNUSED(cmap);
    FT_UNUSED(char_code);
...
FT_CALLBACK_DEF(FT_UInt32)
tt_cmap14_char_next(TT_CMap cmap, FT_UInt32* pchar_code)
{
    FT_UNUSED(cmap);
...
FT_CALLBACK_DEF(FT_Error)
tt_cmap14_get_info(TT_CMap cmap, TT_CMapInfo* cmap_info)
{
    FT_UNUSED(cmap);
...
static FT_UInt tt_cmap14_char_map_def_binary(FT_Byte* base, FT_UInt32 char_code)
{
...
static FT_UInt tt_cmap14_char_map_nondef_binary(FT_Byte* base,
                                                FT_UInt32 char_code)
{
...
static FT_Byte* tt_cmap14_find_variant(FT_Byte* base, FT_UInt32 variantCode)
{
...
FT_CALLBACK_DEF(FT_UInt)
tt_cmap14_char_var_index(TT_CMap cmap, TT_CMap ucmap, FT_UInt32 charcode,
...
tt_cmap14_char_var_isdefault(TT_CMap cmap, FT_UInt32 charcode,
                             FT_UInt32 variantSelector)
{
    FT_Byte* p = tt_cmap14_find_variant(cmap->data + 6, variantSelector);
    FT_ULong defOff;
    ...
    if (nondefOff != 0 &&
        tt_cmap14_char_map_nondef_binary(cmap->data + nondefOff, charcode) != 0)
        return 0;
...
FT_CALLBACK_DEF(FT_UInt32*)
tt_cmap14_char_variants(TT_CMap cmap, FT_Memory memory, FT_UInt32 charCode)
{
    TT_CMap14 cmap14 = (TT_CMap14)cmap;
    FT_UInt32 count = cmap14->num_selectors;
    FT_Byte* p = cmap->data + 10;
    ...
    for (q = cmap14->results; count > 0; count--)
    {
        ...
        if ((defOff != 0 &&
             tt_cmap14_char_map_def_binary(cmap->data + defOff, charCode)) ||
            (nondefOff != 0 && tt_cmap14_char_map_nondef_binary(
                                   cmap->data + nondefOff, charCode) != 0))
        {
            q[0] = varSel;
            q++;
        }
...
static FT_UInt tt_cmap14_def_char_count(FT_Byte* p)
{
...
static FT_UInt32* tt_cmap14_get_def_chars(TT_CMap cmap, FT_Byte* p,
                                          FT_Memory memory)
{
    TT_CMap14 cmap14 = (TT_CMap14)cmap;
    FT_UInt32 numRanges;
...
static FT_UInt32* tt_cmap14_get_nondef_chars(TT_CMap cmap, FT_Byte* p,
                                             FT_Memory memory)
{
    TT_CMap14 cmap14 = (TT_CMap14)cmap;
...
FT_DEFINE_TT_CMAP(tt_cmap14_class_rec,
                  ...
                  14, (TT_CMap_ValidateFunc)tt_cmap14_validate, /* validate */
...
tt_get_glyph_name(TT_Face face, FT_UInt idx)
{
    FT_String* PSname;
...
FT_CALLBACK_DEF(FT_Error)
tt_cmap_unicode_init(PS_Unicodes unicodes, FT_Pointer pointer)
{
    ...
    return psnames->unicodes_init(memory, unicodes, face->root.num_glyphs,
                                  (PS_GetGlyphNameFunc)&tt_get_glyph_name,
                                  (PS_FreeGlyphNameFunc)NULL, (FT_Pointer)face);
}
...
FT_CALLBACK_DEF(FT_UInt)
tt_cmap_unicode_char_index(PS_Unicodes unicodes, FT_UInt32 char_code)
{
    TT_Face face = (TT_Face)FT_CMAP_FACE(unicodes);
    FT_Service_PsCMaps psnames = (FT_Service_PsCMaps)face->psnames;
...
FT_CALLBACK_DEF(FT_UInt32)
tt_cmap_unicode_char_next(PS_Unicodes unicodes, FT_UInt32* pchar_code)
{
    TT_Face face = (TT_Face)FT_CMAP_FACE(unicodes);
    FT_Service_PsCMaps psnames = (FT_Service_PsCMaps)face->psnames;
...
FT_DEFINE_TT_CMAP(
    ...
    ~0U, (TT_CMap_ValidateFunc)NULL, /* validate      */
            ...
            for (; *pclazz; pclazz++)
            {
                ...
                if (clazz->format == format)
                {
                    ...
                    if (!valid.validator.error)
                    {
                        ...
                        /* selector cmap somewhere special.  But it would have
                         * to be */
                        /* in the public FT_FaceRec, and we can't change that.
                         */

                        if (!FT_CMap_New((FT_CMap_Class)clazz, cmap, &charmap,
                                         &ttcmap))
                        {
