 ...
 * allows unprivileged users (normally anyone other than root, or those not in
 * the "disk" group) to locate devices by label/id.  The standard location of
 * the cache file can be overridden by the environment variable BLKID_FILE.
 *
...
int blkid_get_cache(blkid_cache* ret_cache, const char* filename)
{
    ...
    DBG(CACHE, ul_debugobj(cache, "alloc (from %s)",
...
void blkid_put_cache(blkid_cache cache)
{
    ...
    {
        blkid_dev dev =
            list_entry(cache->bic_devs.next, struct blkid_struct_dev, bid_devs);
        blkid_free_dev(dev);
    }
    ...
    {
        blkid_tag tag =
            list_entry(cache->bic_tags.next, struct blkid_struct_tag, bit_tags);

        while (!list_empty(&tag->bit_names))
        {
            blkid_tag bad = list_entry(tag->bit_names.next,
                                       struct blkid_struct_tag, bit_names);
...
void blkid_gc_cache(blkid_cache cache)
{
    ...
    list_for_each_safe(p, pnext, &cache->bic_devs)
    {
        ...
        {
            DBG(CACHE,
                ul_debugobj(cache, "freeing non-exiting %s", dev->bid_name));
            blkid_free_dev(dev);
