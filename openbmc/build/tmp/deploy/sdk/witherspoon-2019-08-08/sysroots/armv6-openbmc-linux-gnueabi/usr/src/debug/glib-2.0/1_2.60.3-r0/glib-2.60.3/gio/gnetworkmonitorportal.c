...
static void
    g_network_monitor_portal_iface_init(GNetworkMonitorInterface* iface);
...
G_DEFINE_TYPE_WITH_CODE(
    ...
    g_io_extension_point_implement(G_NETWORK_MONITOR_EXTENSION_POINT_NAME,
                                   g_define_type_id, "portal", 40))

static void g_network_monitor_portal_init(GNetworkMonitorPortal* nm)
{
    nm->priv = g_network_monitor_portal_get_instance_private(nm);
}

static void g_network_monitor_portal_get_property(GObject* object,
...
static gboolean is_valid_connectivity(guint32 value)
{
...
static void got_available(GObject* source, GAsyncResult* res, gpointer data)
{
...
static void got_metered(GObject* source, GAsyncResult* res, gpointer data)
{
    GDBusProxy* proxy = G_DBUS_PROXY(source);
    GNetworkMonitorPortal* nm = G_NETWORK_MONITOR_PORTAL(data);
    GError* error = NULL;
...
static void got_connectivity(GObject* source, GAsyncResult* res, gpointer data)
{
    GDBusProxy* proxy = G_DBUS_PROXY(source);
    GNetworkMonitorPortal* nm = G_NETWORK_MONITOR_PORTAL(data);
    GError* error = NULL;
...
static void got_status(GObject* source, GAsyncResult* res, gpointer data)
{
    GDBusProxy* proxy = G_DBUS_PROXY(source);
    GNetworkMonitorPortal* nm = G_NETWORK_MONITOR_PORTAL(data);
    GError* error = NULL;
    ...
    if (ret == NULL)
    {
        if (g_error_matches(error, G_DBUS_ERROR, G_DBUS_ERROR_UNKNOWN_METHOD))
        {
            ...
            g_dbus_proxy_call(proxy, "GetConnectivity", NULL, 0, -1, NULL,
                              got_connectivity, nm);
            g_dbus_proxy_call(proxy, "GetMetered", NULL, 0, -1, NULL,
                              got_metered, nm);
            g_dbus_proxy_call(proxy, "GetAvailable", NULL, 0, -1, NULL,
                              got_available, nm);
        }
        else
            g_warning("%s", error->message);
...
static void update_properties(GDBusProxy* proxy, GNetworkMonitorPortal* nm)
{
    /* Try version 3 first */
    g_dbus_proxy_call(proxy, "GetStatus", NULL, 0, -1, NULL, got_status, nm);
}

static void proxy_signal(GDBusProxy* proxy, const char* sender,
...
static void proxy_properties_changed(GDBusProxy* proxy, GVariant* changed,
...
static gboolean g_network_monitor_portal_initable_init(
    GInitable* initable, GCancellable* cancellable, GError** error)
{
    ...
    proxy = g_dbus_proxy_new_for_bus_sync(
        G_BUS_TYPE_SESSION, G_DBUS_PROXY_FLAGS_DO_NOT_LOAD_PROPERTIES, NULL,
        "org.freedesktop.portal.Desktop", "/org/freedesktop/portal/desktop",
        "org.freedesktop.portal.NetworkMonitor", cancellable, error);
    if (!proxy)
        return FALSE;
    ...
    g_signal_connect(proxy, "g-properties-changed",
                     G_CALLBACK(proxy_properties_changed), nm);
...
static void g_network_monitor_portal_finalize(GObject* object)
{
    GNetworkMonitorPortal* nm = G_NETWORK_MONITOR_PORTAL(object);
    ...
    g_network_monitor_portal_class_init(GNetworkMonitorPortalClass* class)
{
    ...
    g_object_class_override_property(gobject_class, PROP_NETWORK_AVAILABLE,
                                     "network-available");
    g_object_class_override_property(gobject_class, PROP_NETWORK_METERED,
                                     "network-metered");
    g_object_class_override_property(gobject_class, PROP_CONNECTIVITY,
                                     "connectivity");
}

static gboolean g_network_monitor_portal_can_reach(
    GNetworkMonitor* monitor, GSocketConnectable* connectable,
    GCancellable* cancellable, GError** error)
{
    ...
    ret = g_dbus_proxy_call_sync(
        nm->priv->proxy, "CanReach",
        g_variant_new("(su)", g_network_address_get_hostname(address),
...
static void can_reach_done(GObject* source, GAsyncResult* result, gpointer data)
{
    GTask* task = data;
    GNetworkMonitorPortal* nm =
        G_NETWORK_MONITOR_PORTAL(g_task_get_source_object(task));
    GError* error = NULL;
    GVariant* ret;
    gboolean reachable;
...
static void g_network_monitor_portal_can_reach_async(
    GNetworkMonitor* monitor, GSocketConnectable* connectable,
    GCancellable* cancellable, GAsyncReadyCallback callback, gpointer data)
{
    ...
    g_dbus_proxy_call(
        ...
        g_variant_new("(su)", g_network_address_get_hostname(address),
                      g_network_address_get_port(address)),
        G_DBUS_CALL_FLAGS_NONE, -1, cancellable, can_reach_done, task);
}

static gboolean g_network_monitor_portal_can_reach_finish(
