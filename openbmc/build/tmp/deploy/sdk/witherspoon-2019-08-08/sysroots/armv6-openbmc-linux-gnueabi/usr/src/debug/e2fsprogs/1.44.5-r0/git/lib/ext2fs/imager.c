        ...
        while (left)
        {
            ...
            while (c)
            {
                ...
                if (check_zero_block(cp, fs->blocksize))
                {
                    ...
                    left--;
                    cp += fs->blocksize;
                    r = ext2fs_llseek(fd, fs->blocksize, SEEK_CUR);
                    if (r < 0)
                    {
                        retval = errno;
                        goto errout;
...
errcode_t ext2fs_image_bitmap_write(ext2_filsys fs, int fd, int flags)
{
    ...
    while (cnt > 0)
    {
        ...
        retval = ext2fs_get_generic_bmap_range(bmap, itr, size << 3, buf);
        if (retval)
            return retval;
...
errcode_t ext2fs_image_bitmap_read(ext2_filsys fs, int fd, int flags)
{
    ...
    while (cnt > 0)
    {
        ...
        retval = ext2fs_set_generic_bmap_range(bmap, itr, size << 3, buf);
        if (retval)
            return retval;
