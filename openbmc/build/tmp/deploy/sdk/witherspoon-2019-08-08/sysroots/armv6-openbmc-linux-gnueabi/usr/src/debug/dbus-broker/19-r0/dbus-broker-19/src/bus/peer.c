    ...
    for (;;)
    {
        ...
        else if (r)
        {
            ...
            if (r == CONNECTION_E_QUOTA)
            {
                ...
                r = log_commitf(
                    peer->bus->log,
                    "Peer :1.%llu is being disconnected as it does not have "
                    "the resources to perform an operation.",
                    peer->id);
                if (r)
                    return error_fold(r);
            ...
            else if (r == CONNECTION_E_UNEXPECTED_FDS)
            {
                ...
                r = log_commitf(
                    peer->bus->log,
                    "Peer :1.%llu is being disconnected as it attempted to "
                    "pass file descriptors without negotiating support for it.",
                    peer->id);
                if (r)
                    return error_fold(r);
        ...
        if (r)
        {
            ...
            if (r == MESSAGE_E_INVALID_HEADER)
            {
                ...
                r = log_commitf(peer->bus->log,
                                "Peer :1.%llu is being disconnected as it sent "
                                "a message with an invalid header.",
                                peer->id);
                if (r)
                    return error_fold(r);
            ...
            else if (r == MESSAGE_E_INVALID_BODY)
            {
                ...
                r = log_commitf(peer->bus->log,
                                "Peer :1.%llu is being disconnected as it sent "
                                "a message with an invalid body.",
                                peer->id);
                if (r)
                    return error_fold(r);
            ...
            else if (r == MESSAGE_E_MISSING_FDS)
            {
                ...
                r = log_commitf(
                    peer->bus->log,
                    "Peer :1.%llu is being disconnected as it passed fewer "
                    "file descriptors than its header declared.",
                    peer->id);
                if (r)
                    return error_fold(r);
        ...
        if (r)
        {
            ...
            if (r == DRIVER_E_PEER_NOT_REGISTERED)
            {
                ...
                r = log_commitf(peer->bus->log,
                                "Peer :1.%llu is being disconnected as it sent "
                                "a message before calling Hello().",
                                peer->id);
                if (r)
                    return error_fold(r);
            ...
            else if (r == DRIVER_E_MONITOR_READ_ONLY)
            {
                ...
                r = log_commitf(peer->bus->log,
                                "Monitor :1.%llu is being disconnected as it "
...
int peer_dispatch(DispatchFile* file)
{
    ...
    {
        r = peer_dispatch_connection(peer,
    ...
    if (r)
    {
        ...
        else if (r == PEER_E_QUOTA || r == PEER_E_PROTOCOL_VIOLATION)
        {
 ...
 * peer_new() - XXX
 */
int peer_new_with_fd(Peer** peerp, Bus* bus, PolicyRegistry* policy,
                     const char guid[], DispatchContext* dispatcher, int fd)
{
    ...
    r = sockopt_get_peergroups(fd, bus->log, ucred.uid, ucred.gid, &gids,
                               &n_gids);
    if (r)
        return error_trace(r);
    ...
    *peer = (Peer)PEER_INIT(*peer);
    ...
    r = user_charge(user, &peer->charges[0], NULL, USER_SLOT_BYTES,
                    sizeof(Peer));
    ...
    r = policy_snapshot_new(&peer->policy, policy, peer->seclabel, ucred.uid,
                            peer->gids, peer->n_gids);
    if (r)
        return error_fold(r);

    r = policy_snapshot_check_connect(peer->policy);
    if (r)
        return (r == POLICY_E_ACCESS_DENIED) ? PEER_E_CONNECTION_REFUSED
                                             : error_fold(r);

    r = connection_init_server(&peer->connection, dispatcher, peer_dispatch,
                               peer->user, guid, fd);
    if (r < 0)
        return error_fold(r);

    peer->id = bus->peers.ids++;
    slot = c_rbtree_find_slot(&bus->peers.peer_tree, peer_compare, &peer->id,
                              &parent);
...
int peer_request_name(Peer* peer, const char* name, uint32_t flags,
                      NameChange* change)
{
    int r;
    ...
    r = name_registry_request_name(&peer->bus->names, &peer->owned_names,
                                   peer->user, name, flags, change);
    if (r == NAME_E_QUOTA)
        return PEER_E_QUOTA;
...
int peer_release_name(Peer* peer, const char* name, NameChange* change)
{
    ...
    r = name_registry_release_name(&peer->bus->names, &peer->owned_names, name,
                                   change);
...
void peer_release_name_ownership(Peer* peer, NameOwnership* ownership,
                                 NameChange* change)
{
...
static int peer_link_match(Peer* peer, MatchRule* rule, bool monitor)
{
    ...
    else if (strcmp(rule->keys.sender, "org.freedesktop.DBus") == 0)
    {
        ...
        {
            /*
             * This rule is a subscription to NameOwnerChanged signals on a
             * specific name, link it on the name or peer that may trigger it.
             */
            ...
            switch (addr.type)
            {
                case ADDRESS_TYPE_ID:
                {
                    ...
                    {
                        r = match_rule_link(
                            rule, &owner->name_owner_changed_matches, monitor);
                        if (r)
                            return error_fold(r);
                    }
                    else if (addr.id >= peer->bus->peers.ids)
                    {
                         ...
                         * a bug in a client.
                         */
                        r = match_rule_link(rule, &peer->bus->sender_matches,
                                            monitor);
                        if (r)
                            return error_fold(r);
                    }
                ...
                case ADDRESS_TYPE_OTHER:
                {
                    ...
                    r = name_registry_ref_name(&peer->bus->names, &name,
                                               rule->keys.filter.args[0]);
                    if (r)
                        return error_fold(r);

                    r = match_rule_link(rule, &name->name_owner_changed_matches,
                                        monitor);
                    if (r)
                        return error_fold(r);
                    name_ref(
                        name); /* this reference must be explicitly released */
                    break;
                }
                default:
        ...
        {
            /*
             * This should be a wildcard match on all NameOwnerChanged signals,
             * we also install other (unexpected) matches here, they will always
             * be false negatives but for the sake of simplicity we do not
             * attempt to optimize them away.
             */
        ...
        switch (addr.type)
        {
            case ADDRESS_TYPE_ID:
            {
                ...
                else if (addr.id >= peer->bus->peers.ids)
                {
                     ...
                     * match.
                     */
                    r = match_rule_link(rule, &peer->bus->wildcard_matches,
                                        monitor);
                    if (r)
                        return error_fold(r);
                }
...
int peer_add_match(Peer* peer, const char* rule_string)
{
    ...
    r = match_owner_ref_rule(&peer->owned_matches, &rule, peer->user,
                             rule_string);
    if (r)
    {
        if (r == MATCH_E_QUOTA)
...
static Name* peer_match_rule_to_name(MatchRule* rule)
{
    ...
    if (strcmp(rule->keys.sender, "org.freedesktop.DBus") == 0)
    {
        if (rule->keys.filter.member &&
            ...
            return c_container_of(rule->registry, Name,
                                  name_owner_changed_matches);
    }
...
int peer_queue_unicast(PolicySnapshot* sender_policy, NameSet* sender_names,
                       ReplyOwner* sender_replies, User* sender_user,
                       uint64_t sender_id, Peer* receiver, Message* message)
{
    ...
    r = policy_snapshot_check_receive(
        ...
        message->header->type, false, message->metadata.fields.unix_fds);
    if (r)
    {
        if (r == POLICY_E_ACCESS_DENIED)
        {
            log_append_here(receiver->bus->log, LOG_WARNING, 0);
            bus_log_append_policy_receive(receiver->bus, receiver->id,
                                          sender_id, sender_names,
                                          &receiver_names, message);
            r = log_commitf(receiver->bus->log,
                            "A security policy denied %s to receive %s "
                            "%s:%s.%s from :1.%llu.",
                            ...
                            message->metadata.fields.member, sender_id);
            if (r)
                return error_fold(r);
    ...
    r = policy_snapshot_check_send(
        sender_policy, receiver->seclabel, &receiver_names,
        message->metadata.fields.interface, message->metadata.fields.member,
        message->metadata.fields.path, message->header->type, false,
...
int peer_queue_reply(Peer* sender, const char* destination,
                     uint32_t reply_serial, Message* message)
{
    _c_cleanup_(reply_slot_freep) ReplySlot* slot = NULL;
    Peer* receiver;
    Address addr;
    ...
    if (r)
    {
        if (r == CONNECTION_E_QUOTA || r == CONNECTION_E_UNEXPECTED_FDS)
        {
            ...
            log_append_here(receiver->bus->log, LOG_WARNING, 0);
            bus_log_append_transaction(receiver->bus, sender->id, receiver->id,
            ...
            if (r == CONNECTION_E_QUOTA)
                r = log_commitf(
                    receiver->bus->log,
                    "Peer :1.%llu is being disconnected as it does not have "
                    "the resources to receive a reply it requested.",
                    receiver->id);
            else
                r = log_commitf(
                    receiver->bus->log,
                    "Peer :1.%llu is being disconnected as it does not support "
                    "receiving file descriptors it requested.",
                    receiver->id);
...
void peer_registry_flush(PeerRegistry* registry)
{
    ...
    c_rbtree_for_each_entry_safe_postorder_unlink(
        peer, safe, &registry->peer_tree, registry_node)
    {
...
Peer* peer_registry_find_peer(PeerRegistry* registry, uint64_t id)
{
    ...
    peer = c_rbtree_find_entry(&registry->peer_tree, peer_compare, &id, Peer,
                               registry_node);
