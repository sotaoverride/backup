                          ...
                          int replace_bad_blocks)
{
     ...
     */
    retval =
        ext2fs_block_iterate(fs, EXT2_BAD_INO, 0, 0, check_bb_inode_blocks, 0);
    if (retval)
    {
    ...
    else
    {
        ...
        if (!f)
        {
            com_err("read_bad_blocks_file", errno, _("while trying popen '%s'"),
                    buf);
            goto fatal;
        }
    }
...
static int check_bb_inode_blocks(ext2_filsys fs, blk_t* block_nr,
