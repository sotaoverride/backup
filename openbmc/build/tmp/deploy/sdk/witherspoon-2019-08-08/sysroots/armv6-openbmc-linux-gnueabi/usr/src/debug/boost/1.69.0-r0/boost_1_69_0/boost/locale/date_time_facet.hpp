...
{
///
/// \brief This namespace holds a enum of various period types like era, year,
/// month, etc..
///
namespace marks
{
...
enum period_mark
{
    ...
    year,    ///< Year, it is calendar specific, for example 2011 in Gregorian
             ///< calendar.
    extended_year, ///< Extended year for Gregorian/Julian calendars, where 1 BC
                   ///< == 0, 2 BC == -1.
    month,       ///< The month of year, calendar specific, in Gregorian [0..11]
    day,         ///< The day of month, calendar specific, in Gregorian [1..31]
    day_of_year, ///< The number of day in year, starting from 1, in Gregorian
                 ///< [1..366]
    day_of_week, ///< Day of week, Sunday=1, Monday=2,..., Saturday=7.
                 ///< Note that updating this value respects local day of week,
                 ///< so for example, If first day of week is Monday and the
                 ///< current day is Tuesday then setting the value to Sunday
                 ///< (1) would forward the date by 5 days forward and not
                 ///< backward by two days as it could be expected if the
                 ///< numbers were taken as is.
    day_of_week_in_month, ///< Original number of the day of the week in month.
                          ///< For example 1st Sunday, 2nd Sunday, etc. in
                          ///< Gregorian [1..5]
    day_of_week_local, ///< Local day of week, for example in France Monday is
                       ///< 1, in US Sunday is 1, [1..7]
    ...
    first_day_of_week, ///< First day of week, constant, for example Sunday in
                       ///< US = 1, Monday in France = 2
};
...
///
/// It can be created from either marks::period_mark type or by using shortcuts
/// in period namespace - calling functions like period::year(), period::hour()
/// and so on.
///
/// Basically it represents the same object as enum marks::period_mark but
/// allows to provide save operator overloading that would not collide with
/// casing of enum to numeric values.
///
class period_type
{
  public:
...
///
/// This class defines generic calendar class, it is used by date_time and
/// calendar objects internally. It is less useful for end users, but it is
/// build for localization backend implementation
///
...
class abstract_calendar
{
  public:
    ...
    typedef enum
    {
        absolute_minimum, ///< Absolute possible minimum for the value, for
                          ///< example for day is 1
        ...
        least_maximum, ///< The last maximal value for this period, For example
                       ///< for Gregorian calendar day it is 28
        actual_maximum,   ///< Actual maximum, for it can be 28, 29, 30, 31 for
                          ///< day according to current month
    ...
    typedef enum
    {
        move, ///< Change the value up or down effecting others for example
              ///< 1990-12-31 + 1 day = 1991-01-01
        roll, ///< Change the value up or down not effecting others for example
              ///< 1990-12-31 + 1 day = 1990-12-01
    } update_type;
    ...
    ///
    /// After call of set_value you may want to call normalize() function to
    /// make sure vall periods are updated, if you set sereral fields that are
    /// part of single date/time representation you should call set_value
    /// several times and then call normalize().
    ///
    ...
    ///
    /// Recalculate all periods after setting them, should be called after use
    /// of set_value() function.
    ///
    ...
    ///
    /// Get option for calendar, currently only check if it is Gregorian
    /// calendar
    ///
    virtual int get_option(calendar_option_type opt) const = 0;

    ///
    /// Adjust period's \a p value by \a difference items using a update_type \a
    /// u. Note: not all values are adjustable
    ///
    virtual void adjust_value(period::marks::period_mark p, update_type u,
                              int difference) = 0;

    ///
    /// Calculate the difference between this calendar  and \a other in \a p
    /// units
    ///
    virtual int difference(abstract_calendar const* other,
                           period::marks::period_mark p) const = 0;
