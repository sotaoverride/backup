...
int mnt_reset_context(struct libmnt_context* cxt)
{
    ...
    while (!list_empty(&cxt->addmounts))
    {
        struct libmnt_addmount* ad =
            list_entry(cxt->addmounts.next, struct libmnt_addmount, mounts);
...
static int context_init_paths(struct libmnt_context* cxt, int writable)
{
    ...
    if (!cxt->mtab_path)
    {
        ...
        DBG(CXT,
            ul_debugobj(cxt, "mtab path initialized to: %s", cxt->mtab_path));
    }
#endif
    if (!cxt->utab_path)
    {
        ...
        DBG(CXT,
            ul_debugobj(cxt, "utab path initialized to: %s", cxt->utab_path));
    }
 ...
 *
 * @MNT_OMODE_FORCE: always read mtab/fstab (although source and target are
 * defined)
 *
 ...
 *
 * @MNT_OMODE_AUTO: default mode (MNT_OMODE_PREPEND | MNT_OMODE_FSTAB |
 * MNT_OMODE_MTAB)
 *
 * @MNT_OMODE_USER: default for non-root users (MNT_OMODE_REPLACE |
 ...
 *
 * Enable/disable paths canonicalization and tags evaluation. The libmount
 * context canonicalizes paths when searching in fstab and when preparing source
 * and target paths for mount(2) syscall.
 *
 * This function has an effect on the private (within context) fstab instance
 * only (see mnt_context_set_fstab()). If you want to use an external fstab then
 * you need to manage your private struct libmnt_cache (see
 ...
 * @enable: TRUE or FALSE
 *
 * Enable/disable the loop delete (destroy) after umount (see umount(8), option
 * -d)
 *
 * Returns: 0 on success, negative number in case of error.
 */
 ...
 * Note that the FS is modified by
 * mnt_context_set_{source,target,options,fstype} functions.
 *
 ...
 * @tb: fstab
 *
 * The mount context reads /etc/fstab to the private struct libmnt_table by
 * default. This function allows to overwrite the private fstab with an external
 * instance.
 *
...
int mnt_context_get_mtab(struct libmnt_context* cxt, struct libmnt_table** tb)
{
    ...
    if (!cxt->mtab)
    {
        ...
        if (cxt->table_fltrcb)
            mnt_table_set_parser_fltrcb(cxt->mtab, cxt->table_fltrcb,
        ...
        if (cxt->utab)
            ...
            rc = __mnt_table_parse_mtab(cxt->mtab, cxt->mtab_path, cxt->utab);
        else
...
int mnt_context_set_tabfilter(struct libmnt_context* cxt,
                              int (*fltr)(struct libmnt_fs*, void*), void* data)
{
    if (!cxt)
        return -EINVAL;
 ...
 * Returns: 0 on success, negative number in case of error.
 */
int mnt_context_get_table(struct libmnt_context* cxt, const char* filename,
...
struct libmnt_lock* mnt_context_get_lock(struct libmnt_context* cxt)
{
    ...
    if (!cxt->lock)
    {
        cxt->lock = mnt_new_lock(mnt_context_get_writable_tabpath(cxt), 0);
        if (cxt->lock)
...
int mnt_context_set_mflags(struct libmnt_context* cxt, unsigned long flags)
{
    ...
    if ((cxt->flags & MNT_FL_MOUNTOPTS_FIXED) && cxt->fs)
         ...
         */
        return mnt_optstr_apply_flags(&cxt->fs->vfs_optstr, cxt->mountflags,
                                ...
                                unsigned long* flags)
{
    ...
    *flags = 0;
    if (!(cxt->flags & MNT_FL_MOUNTFLAGS_MERGED) && cxt->fs)
    {
        ...
        if (o)
            rc = mnt_optstr_get_flags(
                o, flags, mnt_get_builtin_optmap(MNT_USERSPACE_MAP));
    }
    if (!rc)
...
int mnt_context_prepare_srcpath(struct libmnt_context* cxt)
{
    ...
    {
        DBG(CXT,
            ul_debugobj(cxt, "REMOUNT/BIND/MOVE/pseudo FS source: %s", path));
        goto end;
    }
    ...
    DBG(CXT,
        ul_debugobj(cxt, "final srcpath '%s'", mnt_fs_get_source(cxt->fs)));
...
static int mkdir_target(const char* tgt, struct libmnt_fs* fs)
{
    ...
    if (mnt_optstr_get_option(fs->user_optstr, "X-mount.mkdir", &mstr,
                              &mstr_sz) != 0 &&
        mnt_optstr_get_option(fs->user_optstr, "x-mount.mkdir", &mstr,
                              &mstr_sz) != 0) /* obsolete */
        return 0;
...
int mnt_context_prepare_target(struct libmnt_context* cxt)
{
    ...
    /* mkdir target */
    if (cxt->action == MNT_ACT_MOUNT && !mnt_context_is_restricted(cxt) &&
    ...
    if (rc)
        ...
        DBG(CXT,
            ul_debugobj(cxt, "final target '%s'", mnt_fs_get_target(cxt->fs)));
    return 0;
}
...
int mnt_context_guess_fstype(struct libmnt_context* cxt)
{
    ...
    DBG(CXT, ul_debugobj(cxt, "FS type: %s [rc=%d]", mnt_fs_get_fstype(cxt->fs),
                         rc));
    return rc;
none:
                               ...
                               const char* type)
{
    ...
    if (mnt_context_is_nohelpers(cxt) || !type || !strcmp(type, "none") ||
    ...
    while (path)
    {
        ...
        rc = snprintf(helper, sizeof(helper), "%s/%s.%s", path, name, type);
...
int mnt_context_prepare_update(struct libmnt_context* cxt)
{
    ...
    {
        DBG(CXT, ul_debugobj(cxt, "skip update: syscall failed [status=%d]",
                             cxt->syscall_status));
        return 0;
    }
...
int mnt_context_update_tabs(struct libmnt_context* cxt)
{
    ...
    if (cxt->syscall_status != 0 && !(mnt_context_helper_executed(cxt) &&
                                      mnt_context_get_helper_status(cxt) == 0))
    {

        DBG(CXT,
            ul_debugobj(cxt, "don't update: syscall/helper failed/not called"));
        goto end;
    }
    ...
    if ((cxt->mountflags & MS_RDONLY) != (fl & MS_RDONLY))
         ...
         */
        mnt_update_force_rdonly(cxt->update, cxt->mountflags & MS_RDONLY);
...
int mnt_context_apply_fstab(struct libmnt_context* cxt)
{
    ...
    if (!src && tgt && !(cxt->optsmode & MNT_OMODE_FSTAB) &&
        !(cxt->optsmode & MNT_OMODE_MTAB))
    {
    ...
    {
        DBG(CXT, ul_debugobj(cxt, "trying to apply fstab (src=%s, target=%s)",
                             src, tgt));
    ...
    {
        DBG(CXT, ul_debugobj(cxt, "trying to apply mtab (src=%s, target=%s)",
                             src, tgt));
        if (tgt)
    ...
    if (rc)
    {
        if (!mnt_context_is_restricted(cxt) && tgt && !src && isremount)
        {
            DBG(CXT,
                ul_debugobj(
                    cxt, "only target; ignore missing mtab entry on remount"));
            return 0;
        }

        DBG(CXT,
            ul_debugobj(cxt, "failed to find entry in fstab/mtab [rc=%d]: %m",
                        rc));
...
int mnt_context_propagation_only(struct libmnt_context* cxt)
{
    ...
    return !list_empty(&cxt->addmounts) &&
           (cxt->mountflags == 0 || cxt->mountflags == MS_SILENT) && cxt->fs &&
 ...
 */
int mnt_context_get_excode(struct libmnt_context* cxt, int rc, char* buf,
                           size_t bufsz)
{
    if (buf)
    {
                            ...
                            int flags __attribute__((__unused__)))
{
    ...
    DBG(CXT,
        ul_debugobj(cxt, "initialized for [u]mount.<type> helper [rc=%d]", rc));
    return rc;
}
 ...
 * @arg: getopt() optarg
 *
 * This function applies the [u]mount.type command line option (for example
 * parsed by getopt or getopt_long) to @cxt. All unknown options are ignored and
 * then 1 is returned.
 *
 ...
 */
int mnt_context_is_fs_mounted(struct libmnt_context* cxt, struct libmnt_fs* fs,
                              int* mounted)
{
...
int mnt_context_wait_for_children(struct libmnt_context* cxt, int* nchildren,
                                  int* nerrs)
{
    int i;
    ...
    for (i = 0; i < cxt->nchildren; i++)
    {
        ...
        do
        {
            DBG(CXT, ul_debugobj(cxt, "waiting for child (%d/%d): %d", i + 1,
                                 cxt->nchildren, pid));
            errno = 0;
...
int mnt_context_set_target_ns(struct libmnt_context* cxt, const char* path)
{
    ...
    if (setns(tmp, CLONE_NEWNS) || setns(cxt->ns_orig.fd, CLONE_NEWNS))
    {
        errsv = errno;
                                        ...
                                        struct libmnt_ns* ns)
{
    ...
    DBG(CXT, ul_debugobj(cxt, "Switching to %s namespace",
                         ns == mnt_context_get_target_ns(cxt)
                             ? "target"
                             : ns == mnt_context_get_origin_ns(cxt) ? "original"
                                                                    : "other"));
