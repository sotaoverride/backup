...
static int certify_cert(X509** xret, const char* infile, EVP_PKEY* pkey,
                        ...
                        const char* ext_sect, CONF* conf, int verbose,
...
static int do_body(X509** xret, EVP_PKEY* pkey, X509* x509, const EVP_MD* dgst,
                   ...
                   const char* subj, unsigned long chtype, int multirdn,
                   int email_dn, const char* startdate, const char* enddate,
                   long days, int batch, int verbose, X509_REQ* req,
...
end_of_options:
    ...
    if ((outdir == NULL) && (req))
    {
        ...
        if (outdir == NULL)
        {
            BIO_printf(bio_err, "there needs to be defined a directory for new "
                                "certificate to be placed in\n");
            goto end;
        }
    ...
    if (req)
    {
        ...
        {
            BIO_printf(bio_err, "start date is invalid, it should be "
        ...
        if (ss_cert_file != NULL)
        {
            ...
            j = certify_cert(&x, ss_cert_file, pkey, x509, dgst, sigopts,
                             ...
                             extensions, conf, verbose, certopt, get_nameopt(),
                             default_op, ext_copy);
            if (j < 0)
                goto end;
            if (j > 0)
        ...
        if (sk_X509_num(cert_sk) > 0)
        {
            ...
            {
                BIO_printf(bio_err,
                           "\n%d out of %d certificate requests certified, "
                           "commit? [y/n]",
                           total_done, total);
    ...
    if (gencrl)
    {
        ...
        {
            if (!NCONF_get_number(conf, section, ENV_DEFAULT_CRL_DAYS,
                                  &crldays))
                crldays = 0;
            if (!NCONF_get_number(conf, section, ENV_DEFAULT_CRL_HOURS,
                                  &crlhours))
                crlhours = 0;
            ERR_clear_error();
        }
        ...
        tmptm = ASN1_TIME_new();
        if (tmptm == NULL || X509_gmtime_adj(tmptm, 0) == NULL ||
...
static int certify_cert(X509** xret, const char* infile, EVP_PKEY* pkey,
                        ...
                        int multirdn, int email_dn, const char* startdate,
                        const char* enddate, long days, int batch,
                        const char* ext_sect, CONF* lconf, int verbose,
                        unsigned long certopt, unsigned long nameopt,
                        int default_op, int ext_copy)
{
    X509* req = NULL;
...
static int do_body(X509** xret, EVP_PKEY* pkey, X509* x509, const EVP_MD* dgst,
                   ...
                   int email_dn, const char* startdate, const char* enddate,
                   long days, int batch, int verbose, X509_REQ* req,
                   const char* ext_sect, CONF* lconf, unsigned long certopt,
                   unsigned long nameopt, int default_op, int ext_copy,
                   int selfsign)
{
    ...
    for (i = 0; i < X509_NAME_entry_count(name); i++)
    {
        ...
        if (str->type != V_ASN1_BMPSTRING && str->type != V_ASN1_UTF8STRING)
        {
            ...
            {
                BIO_printf(bio_err, "\nThe string contains characters that are "
                                    "illegal for the ASN.1 type\n");
                goto end;
            }
        }
                         ...
                         unsigned long nameopt, int default_op, int ext_copy)
{
    ...
    for (i = 0;; i++)
    {
        ...
        if (!X509_NAME_add_entry_by_NID(n, nid, chtype, (unsigned char*)buf, -1,
                                        -1, 0))
            goto end;
    }
    if (spki == NULL)
...
static int make_revoked(X509_REVOKED* rev, const char* str)
{
    ...
    if (rev && comp_time)
    {
        if (!X509_REVOKED_add1_ext_i2d(rev, NID_invalidity_date, comp_time, 0,
                                       0))
            goto end;
    }
    if (rev && hold)
    {
        if (!X509_REVOKED_add1_ext_i2d(rev, NID_hold_instruction_code, hold, 0,
                                       0))
            goto end;
    }
