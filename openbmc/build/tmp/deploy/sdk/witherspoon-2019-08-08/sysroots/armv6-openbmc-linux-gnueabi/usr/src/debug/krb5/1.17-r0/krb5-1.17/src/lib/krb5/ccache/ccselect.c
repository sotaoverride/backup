...
static void free_handles(krb5_context context,
                         struct ccselect_module_handle** handles)
{
...
static krb5_error_code load_modules(krb5_context context)
{
    krb5_error_code ret;
    ...
    for (mod = modules; *mod != NULL; mod++)
    {
        ...
        if (ret != 0)
        { /* Failed initialization is non-fatal. */
                                             ...
                                             krb5_principal* princ_out)
{
    ...
    *princ_out = NULL;
    ...
    {
        ret =
            krb5_get_fallback_host_realm(context, &server->data[1], &fbrealms);
        if (ret)
            goto cleanup;
