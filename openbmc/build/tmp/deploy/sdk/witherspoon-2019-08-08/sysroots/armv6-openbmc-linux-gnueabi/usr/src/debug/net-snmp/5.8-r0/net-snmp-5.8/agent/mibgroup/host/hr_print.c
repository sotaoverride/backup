...
struct variable4 hrprint_variables[] = {
    {HRPRINT_STATUS, ASN_INTEGER, NETSNMP_OLDAPI_RONLY, var_hrprint, 2, {1, 1}},
    {HRPRINT_ERROR,
     ASN_OCTET_STR,
 ...
 *
 */
int header_hrprint(struct variable* vp, oid* name, size_t* length, int exact,
...
u_char* var_hrprint(struct variable* vp, oid* name, size_t* length, int exact,
                    size_t* var_len, WriteMethod** write_method)
{
    int print_idx;
    ...
    if ((p = run_lpstat(&fd)) != NULL)
    {
        ...
        while (fgets(buf, sizeof buf, p))
        {
            ...
            if (HRP_nbrnames == HRP_maxnames)
            {
                char** tmp;
                tmp =
                    (char**)calloc(HRP_maxnames + HRP_MAX_INCR, sizeof(char*));
                if (!tmp)
                    goto finish;
                if (HRP_name)
                {
