...
struct variable4 hrnet_variables[] = {
    {HRNET_IFINDEX, ASN_INTEGER, NETSNMP_OLDAPI_RONLY, var_hrnet, 2, {1, 1}}};
...
int header_hrnet(struct variable* vp, oid* name, size_t* length, int exact,
...
u_char* var_hrnet(struct variable* vp, oid* name, size_t* length, int exact,
                  size_t* var_len, WriteMethod** write_method)
{
    int net_idx;
    ...
    switch (vp->magic)
    {
        ...
        default:
            DEBUGMSGTL(
                ("snmpd", "unknown sub-id %d in var_hrnet\n", vp->magic));
    }
    return NULL;
}
...
void Save_HR_Network_Info(void)
{
    ...
    HRN_savedErrors =
        HRN_ifnet.if_entry.ifInErrors + HRN_ifnet.if_entry.ifOutErrors;
#elif defined(__APPLE__) /* or darwin? */
    HRN_savedFlags = HRN_ifnet.ifm_flags;
    HRN_savedErrors =
        HRN_ifnet.ifm_data.ifi_ierrors + HRN_ifnet.ifm_data.ifi_oerrors;
#elif !defined(WIN32)
...
const char* describe_networkIF(int idx)
{
