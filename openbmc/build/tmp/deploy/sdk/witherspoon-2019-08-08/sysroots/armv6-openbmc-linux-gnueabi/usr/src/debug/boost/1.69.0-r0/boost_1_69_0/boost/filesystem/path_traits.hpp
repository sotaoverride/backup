...
//    partial:  Not all source characters converted; one or more additional
//    source
//              characters are needed to produce the final target character, or
//              the size of the target intermediate buffer was too small to hold
//              the result.
//    error:    A character in the source could not be converted to the target
//    encoding. noconv:   The source and target characters have the same type
//    and encoding, so no
...
//  is_pathable type trait; allows disabling over-agressive class path member
//  templates
...
// ---------------------------------------------------------------//
//
//   A from_end argument of 0 is less efficient than a known end, so use only if
//   needed
...
inline void convert(const wchar_t* from, const wchar_t* from_end,
                    std::wstring& to, const codecvt_type&)
{
    BOOST_ASSERT(from);
    BOOST_ASSERT(from_end);
    to.append(from, from_end);
...
inline void convert(const wchar_t* from, std::wstring& to, const codecvt_type&)
{
    BOOST_ASSERT(from);
    to += from;
}
    ...
    dispatch(const Container& c, U& to)
{
    if (c.size())
    {
        std::basic_string<typename Container::value_type> seq(c.begin(),
                                                              c.end());
