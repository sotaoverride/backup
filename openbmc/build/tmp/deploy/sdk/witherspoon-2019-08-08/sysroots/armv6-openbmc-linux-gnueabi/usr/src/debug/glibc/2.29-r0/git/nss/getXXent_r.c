...
libc_hidden_proto(DB_LOOKUP_FCT)
    ...
    void SETFUNC_NAME(STAYOPEN)
{
    int save;

    __libc_lock_lock(lock);
    __nss_setent(SETFUNC_NAME_STRING, DB_LOOKUP_FCT, &nip, &startp, &last_nip,
                 STAYOPEN_VAR, STAYOPEN_TMPVAR, NEED__RES);
...
int INTERNAL(REENTRANT_GETNAME)(LOOKUP_TYPE* resbuf, char* buffer,
                                size_t buflen,
                                LOOKUP_TYPE** result H_ERRNO_PARM)
{
    ...
    __libc_lock_lock(lock);
    status =
        __nss_getent_r(GETFUNC_NAME_STRING, SETFUNC_NAME_STRING, DB_LOOKUP_FCT,
                       &nip, &startp, &last_nip, STAYOPEN_TMPVAR, NEED__RES,
                       resbuf, buffer, buflen, (void**)result, H_ERRNO_VAR_P);
    save = errno;
    __libc_lock_unlock(lock);
    __set_errno(save);
    return status;
...
{
    int ret =
        INTERNAL(REENTRANT_GETNAME)(resbuf, buffer, buflen, result H_ERRNO_VAR);
...
do_default_symbol_version(NEW(REENTRANT_GETNAME), REENTRANT_GETNAME,
                          GLIBC_2_1_2);
#endif

nss_interface_function(SETFUNC_NAME) nss_interface_function(ENDFUNC_NAME)
