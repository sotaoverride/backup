...
void network_apply_anonymize_if_set(Network* network)
{
    ...
    network->dhcp_vendor_class_identifier =
        mfree(network->dhcp_vendor_class_identifier);
    /* RFC7844 section 3.6.:
...
static int network_resolve_netdev_one(Network* network, const char* name,
                                      NetDevKind kind, NetDev** ret_netdev)
{
    ...
    if (r < 0)
        return log_error_errno(
            r, "%s: %s NetDev could not be found, ignoring assignment.",
            network->filename, name);

    if (netdev->kind != kind &&
        !(kind == _NETDEV_KIND_TUNNEL &&
          IN_SET(netdev->kind, NETDEV_KIND_IPIP, NETDEV_KIND_SIT,
                 NETDEV_KIND_GRE, NETDEV_KIND_GRETAP, NETDEV_KIND_IP6GRE,
                 NETDEV_KIND_IP6GRETAP, NETDEV_KIND_VTI, NETDEV_KIND_VTI6,
                 NETDEV_KIND_IP6TNL, NETDEV_KIND_ERSPAN)))
...
static int network_resolve_stacked_netdevs(Network* network)
{
    ...
    HASHMAP_FOREACH_KEY(kind, name, network->stacked_netdev_names, i)
    {
        ...
        r = network_resolve_netdev_one(network, name, PTR_TO_INT(kind),
                                       &netdev);
        if (r <= 0)
            continue;

        r = hashmap_ensure_allocated(&network->stacked_netdevs,
                                     &string_hash_ops);
        if (r < 0)
            return log_oom();
        ...
        if (r < 0)
            return log_error_errno(
                r, "%s: Failed to add NetDev '%s' to network: %m",
...
int network_verify(Network* network)
{
    ...
    if (set_isempty(network->match_mac) && strv_isempty(network->match_path) &&
        strv_isempty(network->match_driver) &&
        strv_isempty(network->match_type) &&
        strv_isempty(network->match_name) && !network->conditions)
        log_warning(
            "%s: No valid settings found in the [Match] section. "
    ...
    if (!condition_test_list(network->conditions, NULL, NULL, NULL))
        return log_debug_errno(SYNTHETIC_ERRNO(EINVAL),
                               "%s: Conditions in the file do not match the "
                               "system environment, skipping.",
                               network->filename);

    (void)network_resolve_netdev_one(network, network->bond_name,
                                     NETDEV_KIND_BOND, &network->bond);
    (void)network_resolve_netdev_one(network, network->bridge_name,
                                     NETDEV_KIND_BRIDGE, &network->bridge);
    (void)network_resolve_netdev_one(network, network->vrf_name,
                                     NETDEV_KIND_VRF, &network->vrf);
    ...
    if (network->bond)
    {
        ...
        {
            log_warning("%s: Cannot enable IPv6AcceptRA= when Bond= is "
                        "specified, disabling IPv6AcceptRA=.",
                        network->filename);
        ...
        {
            log_warning("%s: Cannot enable LinkLocalAddressing= when Bond= is "
                        "specified, disabling LinkLocalAddressing=.",
                        network->filename);
            network->link_local = ADDRESS_FAMILY_NO;
        }
        if (network->dhcp != ADDRESS_FAMILY_NO)
        {
            log_warning("%s: Cannot enable DHCP= when Bond= is specified, "
                        "disabling DHCP=.",
                        network->filename);
        ...
        {
            log_warning("%s: Cannot enable DHCPServer= when Bond= is "
                        "specified, disabling DHCPServer=.",
                        network->filename);
            network->dhcp_server = false;
        }
        if (network->n_static_addresses > 0)
        {
            log_warning("%s: Cannot set addresses when Bond= is specified, "
                        "ignoring addresses.",
                        network->filename);
        ...
        {
            log_warning("%s: Cannot set routes when Bond= is specified, "
                        "ignoring routes.",
                        network->filename);
    ...
    if (network->link_local < 0)
        network->link_local =
            network->bridge ? ADDRESS_FAMILY_NO : ADDRESS_FAMILY_IPV6;
    ...
    if (network->mtu < mtu)
    {
        if (network->mtu_is_set)
            log_notice("%s: Bumping MTUBytes= from %" PRIu32 " to %" PRIu32
    ...
    {
        log_warning("%s: MTUBytes= in [Link] section and UseMTU= in [DHCP] "
                    "section are set. "
                    "Disabling UseMTU=.",
                    network->filename);
        network->dhcp_use_mtu = false;
    ...
    LIST_FOREACH_SAFE(addresses, address, address_next,
    ...
    LIST_FOREACH_SAFE(static_fdb_entries, fdb, fdb_next,
                      network->static_fdb_entries)
...
int network_load_one(Manager* manager, const char* filename)
{
    ...
    *network = (Network){
        ...
        /* If LinkLocalAddressing= is not set, then set to ADDRESS_FAMILY_IPV6
           later. */
    ...
    if (r < 0)
        log_warning_errno(r, "%s: Failed to add IPv4LL route, ignoring: %m",
                          network->filename);
...
int network_get(Manager* manager, sd_device* device, const char* ifname,
                const struct ether_addr* address, Network** ret)
{
    ...
    LIST_FOREACH(networks, network, manager->networks)
    {
        if (net_match_config(network->match_mac, network->match_path,
                             ...
                             network->match_name, address, path, driver,
                             devtype, ifname))
        {
            if (network->match_name && device)
            {
                ...
                if (sd_device_get_sysattr_value(device, "name_assign_type",
                                                &attr) >= 0)
                    (void)safe_atou8(attr, &name_assign_type);

                if (name_assign_type == NET_NAME_ENUM)
                    log_warning("%s: found matching network '%s', based on "
                                "potentially unpredictable ifname",
...
int network_apply(Network* network, Link* link)
{
    ...
    if (network->n_dns > 0 || !strv_isempty(network->ntp) ||
...
int config_parse_stacked_netdev(const char* unit, const char* filename,
                                ...
                                void* userdata)
{
    ...
    assert(rvalue);
    assert(data);
    assert(IN_SET(kind, NETDEV_KIND_VLAN, NETDEV_KIND_MACVLAN,
                  NETDEV_KIND_MACVTAP, NETDEV_KIND_IPVLAN, NETDEV_KIND_VXLAN,
                  NETDEV_KIND_L2TP, _NETDEV_KIND_TUNNEL));

    if (!ifname_valid(rvalue))
    {
        log_syntax(unit, LOG_ERR, filename, line, 0,
                   "Invalid netdev name in %s=, ignoring assignment: %s",
                   lvalue, rvalue);
        return 0;
    }
    ...
    if (r < 0)
        log_syntax(unit, LOG_ERR, filename, line, r,
                   "Cannot add NetDev '%s' to network, ignoring assignment: %m",
                   name);
    else if (r == 0)
...
int config_parse_domains(const char* unit, const char* filename, unsigned line,
                         ...
                         void* data, void* userdata)
{
    ...
    for (;;)
    {
        ...
        {
            log_syntax(unit, LOG_ERR, filename, line, r,
                       "Failed to extract search or route domain, ignoring: %s",
                       rvalue);
            break;
        }
        if (r == 0)
        ...
        {
            /* If the root domain appears as is, or the special token "*" is
             * found, we'll consider this as routing domain, unconditionally. */
            is_route = true;
            domain = "."; /* make sure we don't allow empty strings, thus write
                           * the root domain as "." */
        }
        else
        {
            ...
            {
                log_syntax(unit, LOG_ERR, filename, line, r,
                           "'%s' is not a valid domain name, ignoring.",
                           domain);
                continue;
            }
            ...
            {
                log_syntax(unit, LOG_ERR, filename, line, 0,
                           "'localhost' domain may not be configured as search "
                           "or route domain, ignoring assignment: %s",
                           domain);
                continue;
            }
        }
                      ...
                      void* data, void* userdata)
{
    ...
    if (s < 0)
    {
        ...
        log_syntax(unit, LOG_WARNING, filename, line, 0,
                   "DHCP=%s is deprecated, please use DHCP=%s instead.", rvalue,
...
DEFINE_CONFIG_PARSE_ENUM(config_parse_dhcp_client_identifier,
                         dhcp_client_identifier, DHCPClientIdentifier,
                         "Failed to parse client identifier type");

int config_parse_ipv6token(const char* unit, const char* filename,
                           unsigned line, const char* section,
                           ...
                           const char* rvalue, void* data, void* userdata)
{
    ...
    {
        log_syntax(unit, LOG_ERR, filename, line, 0,
                   "IPv6 token cannot be the ANY address, ignoring: %s",
                   rvalue);
        return 0;
    }
    ...
    {
        log_syntax(unit, LOG_ERR, filename, line, 0,
                   "IPv6 token cannot be longer than 64 bits, ignoring: %s",
                   rvalue);
        return 0;
    }
...
static const char* const
    ipv6_privacy_extensions_table[_IPV6_PRIVACY_EXTENSIONS_MAX] = {
...
int config_parse_ipv6_privacy_extensions(const char* unit, const char* filename,
                                         unsigned line, const char* section,
...
int config_parse_hostname(const char* unit, const char* filename, unsigned line,
                          const char* section, unsigned section_line,
                          const char* lvalue, int ltype, const char* rvalue,
                          void* data, void* userdata)
{
    ...
    r = config_parse_string(unit, filename, line, section, section_line, lvalue,
                            ltype, rvalue, &hn, userdata);
    if (r < 0)
        return r;
    ...
    {
        log_syntax(unit, LOG_ERR, filename, line, r,
                   "Failed to check validity of hostname '%s', ignoring "
                   "assignment: %m",
                   rvalue);
        return 0;
    }
    if (r == 0)
    {
        log_syntax(
            ...
            "Hostname is not a valid DNS domain name, ignoring assignment: %s",
            rvalue);
        return 0;
    }
...
int config_parse_timezone(const char* unit, const char* filename, unsigned line,
                          const char* section, unsigned section_line,
                          const char* lvalue, int ltype, const char* rvalue,
                          void* data, void* userdata)
{
...
int config_parse_dhcp_server_dns(const char* unit, const char* filename,
                                 unsigned line, const char* section,
                                 unsigned section_line, const char* lvalue,
                                 int ltype, const char* rvalue, void* data,
                                 void* userdata)
{
    ...
    for (;;)
    {
        ...
        m = reallocarray(n->dhcp_server_dns, n->n_dhcp_server_dns + 1,
...
int config_parse_radv_dns(const char* unit, const char* filename, unsigned line,
                          const char* section, unsigned section_line,
                          const char* lvalue, int ltype, const char* rvalue,
                          void* data, void* userdata)
{
    ...
    for (;;)
    {
        ...
        if (in_addr_from_string(AF_INET6, w, &a) >= 0)
        {
            ...
            m = reallocarray(n->router_dns, n->n_router_dns + 1,
...
int config_parse_radv_search_domains(const char* unit, const char* filename,
                                     unsigned line, const char* section,
                                     unsigned section_line, const char* lvalue,
                                     int ltype, const char* rvalue, void* data,
                                     void* userdata)
{
    ...
    for (;;)
    {
        ...
        {
            log_syntax(unit, LOG_ERR, filename, line, r,
                       "Failed to apply IDNA to domain name '%s', ignoring: %m",
                       w);
            continue;
        }
        else if (r == 0)
        ...
        r = ordered_set_ensure_allocated(&n->router_search_domains,
                                         &string_hash_ops);
        if (r < 0)
            return r;
...
int config_parse_dhcp_server_ntp(const char* unit, const char* filename,
                                 unsigned line, const char* section,
                                 unsigned section_line, const char* lvalue,
                                 int ltype, const char* rvalue, void* data,
                                 void* userdata)
{
    ...
    for (;;)
    {
        ...
        m = reallocarray(n->dhcp_server_ntp, n->n_dhcp_server_ntp + 1,
                         sizeof(struct in_addr));
        if (!m)
            return log_oom();
    ...
    void* data, void* userdata)
{
    ...
    if (isempty(rvalue))
    {
        n->dnssec_negative_trust_anchors =
            set_free_free(n->dnssec_negative_trust_anchors);
        return 0;
    }
    ...
    for (;;)
    {
        ...
        if (r < 0)
        {
            log_syntax(
                ...
                "Failed to extract negative trust anchor domain, ignoring: %s",
                rvalue);
            break;
        }
        if (r == 0)
        ...
        r = set_ensure_allocated(&n->dnssec_negative_trust_anchors,
                                 &dns_name_hash_ops);
        if (r < 0)
            return log_oom();
...
int config_parse_ntp(const char* unit, const char* filename, unsigned line,
                     const char* section, unsigned section_line,
                     const char* lvalue, int ltype, const char* rvalue,
                     void* data, void* userdata)
{
    ...
    for (;;)
    {
        ...
        {
            log_syntax(unit, LOG_ERR, filename, line, r,
                       "Failed to extract NTP server name, ignoring: %s",
                       rvalue);
            break;
        }
        if (r == 0)
        ...
        {
            log_syntax(unit, LOG_ERR, filename, line, r,
                       "%s is not a valid domain name or IP address, ignoring.",
                       w);
            continue;
        }
        ...
        {
            log_syntax(unit, LOG_WARNING, filename, line, 0,
                       "More than %u NTP servers specified, ignoring \"%s\" "
...
int config_parse_dhcp_user_class(const char* unit, const char* filename,
                                 unsigned line, const char* section,
                                 unsigned section_line, const char* lvalue,
                                 int ltype, const char* rvalue, void* data,
                                 void* userdata)
{
    ...
    for (;;)
    {
        ...
        {
            log_syntax(unit, LOG_ERR, filename, line, r,
                       "Failed to split user classes option, ignoring: %s",
                       rvalue);
            break;
        }
        if (r == 0)
...
int config_parse_section_route_table(const char* unit, const char* filename,
                                     unsigned line, const char* section,
                                     unsigned section_line, const char* lvalue,
                                     int ltype, const char* rvalue, void* data,
                                     void* userdata)
{
    ...
    {
        log_syntax(unit, LOG_ERR, filename, line, r,
                   "Failed to parse RouteTable=%s, ignoring assignment: %m",
                   rvalue);
        return 0;
    }
...
DEFINE_CONFIG_PARSE_ENUM(config_parse_dhcp_use_domains, dhcp_use_domains,
                         DHCPUseDomains,
...
DEFINE_STRING_TABLE_LOOKUP_WITH_BOOLEAN(dhcp_use_domains, DHCPUseDomains,
...
int config_parse_required_for_online(const char* unit, const char* filename,
                                     unsigned line, const char* section,
                                     unsigned section_line, const char* lvalue,
                                     int ltype, const char* rvalue, void* data,
                                     void* userdata)
