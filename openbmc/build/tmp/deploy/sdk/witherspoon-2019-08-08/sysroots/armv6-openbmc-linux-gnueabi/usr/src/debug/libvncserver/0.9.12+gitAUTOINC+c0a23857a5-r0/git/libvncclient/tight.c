...
static void FilterPaletteBPP(rfbClient* client, int srcx, int srcy,
                             int numRows);
static void FilterGradientBPP(rfbClient* client, int srcx, int srcy,
                              int numRows);

#if BPP != 8
static rfbBool DecompressJpegRectBPP(rfbClient* client, int x, int y, int w,
                                     int h);
#endif
...
static rfbBool HandleTightBPP(rfbClient* client, int rx, int ry, int rw, int rh)
{
    ...
    if (comp_ctl == rfbTightFill)
    {
            ...
            client->format.greenMax == 0xFF && client->format.blueMax == 0xFF)
        {
            if (!ReadFromRFBServer(client, client->buffer, 3))
                return FALSE;
            fill_colour = RGB24_TO_PIXEL32(client->buffer[0], client->buffer[1],
                                           client->buffer[2]);
        }
        else
        {
            if (!ReadFromRFBServer(client, (char*)&fill_colour,
                                   sizeof(fill_colour)))
                return FALSE;
        }
#else
    ...
    while (compressedLen > 0)
    {
        ...
        do
        {
            ...
            if (extraBytes > 0)
                memcpy(client->buffer, &client->buffer[numRows * rowSize],
                       extraBytes);
...
static void FilterCopyBPP(rfbClient* client, int srcx, int srcy, int numRows)
{
    ...
    if (client->cutZeros)
    {
        for (y = 0; y < numRows; y++)
        {
            ...
            {
                dst[y * client->width + x] = RGB24_TO_PIXEL32(
                    client->buffer[(y * client->rectWidth + x) * 3],
...
static void FilterGradient24(rfbClient* client, int srcx, int srcy, int numRows)
{
    CARDBPP* dst =
        (CARDBPP*)&client->frameBuffer[(srcy * client->width + srcx) * BPP / 8];
    int x, y, c;
    ...
    for (y = 0; y < numRows; y++)
    {
        ...
        for (c = 0; c < 3; c++)
        {
            pix[c] = client->tightPrevRow[c] +
                     client->buffer[y * client->rectWidth * 3 + c];
            thisRow[c] = pix[c];
        }
        ...
        for (x = 1; x < client->rectWidth; x++)
        {
            for (c = 0; c < 3; c++)
            {
                ...
                pix[c] = (uint8_t)est[c] +
                         client->buffer[(y * client->rectWidth + x) * 3 + c];
                thisRow[x * 3 + c] = pix[c];
            }
                              ...
                              int numRows)
{
    ...
    for (y = 0; y < numRows; y++)
    {
        ...
        {
            pix[c] = (uint16_t)(
                ((src[y * client->rectWidth] >> shift[c]) + thatRow[c]) &
                max[c]);
            thisRow[c] = pix[c];
        }
        ...
        for (x = 1; x < client->rectWidth; x++)
        {
            for (c = 0; c < 3; c++)
            {
                ...
                pix[c] = (uint16_t)(
                    ((src[y * client->rectWidth + x] >> shift[c]) + est[c]) &
                    max[c]);
                thisRow[x * 3 + c] = pix[c];
            }
            dst[y * client->width + x] =
                RGB_TO_PIXEL(BPP, pix[0], pix[1], pix[2]);
        }
...
static int InitFilterPaletteBPP(rfbClient* client, int rw, int rh)
{
    ...
    if (client->format.depth == 24 && client->format.redMax == 0xFF &&
        client->format.greenMax == 0xFF && client->format.blueMax == 0xFF)
    {
        if (!ReadFromRFBServer(client, (char*)&client->tightPalette,
                               client->rectColors * 3))
            return 0;
...
static void FilterPaletteBPP(rfbClient* client, int srcx, int srcy, int numRows)
{
    ...
    else
    {
        for (y = 0; y < numRows; y++)
            for (x = 0; x < client->rectWidth; x++)
                dst[y * client->width + x] =
                    palette[(int)src[y * client->rectWidth + x]];
    }
}
                                     ...
                                     int h)
{
    ...
    if (client->GotJpeg != NULL)
        return client->GotJpeg(client, compressedData, compressedLen, x, y, w,
                               h);
    ...
    if (tjDecompress(client->tjhnd, compressedData,
                     (unsigned long)compressedLen, dst, w, pitch, h, pixelSize,
                     flags) == -1)
    {
        rfbClientLog("TurboJPEG error: %s\n", tjGetErrorStr());
        free(compressedData);
