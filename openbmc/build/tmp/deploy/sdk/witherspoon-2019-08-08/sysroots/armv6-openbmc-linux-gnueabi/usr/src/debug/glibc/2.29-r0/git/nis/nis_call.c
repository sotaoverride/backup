...
libnsl_hidden_nolink_def(__nisbind_destroy, GLIBC_2_1)
    ...
    nis_error __nisbind_next(dir_binding* bind)
{
    if (bind->clnt != NULL)
    {
        if (bind->use_auth)
...
libnsl_hidden_nolink_def(__nisbind_connect, GLIBC_2_1)
    ...
    nis_error __nisbind_create(dir_binding* dbp, const nis_server* serv_val,
            ...
            switch (prog)
            {
                case NIS_IBLIST:
                    if ((((nis_result*)resp)->status == NIS_CBRESULTS) &&
                        (cb != NULL))
                    {
                        __nis_do_callback(dbp, &((nis_result*)resp)->cookie,
                                          cb);
                        break;
                    }
...
static directory_obj* rec_dirsearch(const_nis_name name, directory_obj* dir,
                                    nis_error* status)
{
    fd_result* fd_res;
    XDR xdrs;
...
__libc_lock_define_initialized(static, nis_server_cache_lock)
    ...
    static directory_obj* nis_server_cache_search(const_nis_name name,
                                                  int search_parent,
                                                  unsigned int* server_used,
                                                  unsigned int* current_ep,
                                                  struct timeval* now)
...
nis_error __nisfind_server(const_nis_name name, int search_parent,
                           directory_obj** dir, dir_binding* dbp,
                           unsigned int flags)
{
    ...
    *dir = readColdStartFile();
    ...
    if (result == NIS_SUCCESS)
    {
        ...
        if (result == NIS_SUCCESS)
        {
            ...
            if ((flags & NO_CACHE) == 0)
                nis_server_cache_add(name, search_parent, obj, server_used,
                                     current_ep, &now);
        }
        else
        {
...
libnsl_hidden_nolink_def(__prepare_niscall, GLIBC_PRIVATE)
    ...
    nis_error
    __do_niscall(const_nis_name name, u_long prog, xdrproc_t xargs, caddr_t req,
                 xdrproc_t xres, caddr_t resp, unsigned int flags, nis_cb* cb)
{
    dir_binding bptr;
