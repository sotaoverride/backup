...
static void g_threaded_socket_service_func(gpointer _data, gpointer user_data)
{
    ...
    g_signal_emit(threaded, g_threaded_socket_service_run_signal, 0,
                  data->connection, data->source_object, &result);
...
static void g_threaded_socket_service_constructed(GObject* object)
{
    ...
    service->priv->thread_pool =
        g_thread_pool_new(g_threaded_socket_service_func, service,
                          service->priv->max_threads, FALSE, NULL);
}
...
static void g_threaded_socket_service_get_property(GObject* object,
...
static void g_threaded_socket_service_set_property(GObject* object,
                                                   guint prop_id,
                                                   const GValue* value,
                                                   GParamSpec* pspec)
{
    ...
    g_threaded_socket_service_class_init(GThreadedSocketServiceClass* class)
{
     ...
     */
    g_threaded_socket_service_run_signal = g_signal_new(
        I_("run"), G_TYPE_FROM_CLASS(class), G_SIGNAL_RUN_LAST,
    ...
    g_object_class_install_property(
        gobject_class, PROP_MAX_THREADS,
        g_param_spec_int(
            "max-threads", P_("Max threads"),
...
{
    return g_object_new(G_TYPE_THREADED_SOCKET_SERVICE, "max-threads",
                        max_threads, NULL);
}
