 ...
 */
static guint explode_locale(const gchar* locale, gchar** language,
                            gchar** territory, gchar** codeset,
                            gchar** modifier)
{
    ...
    at_pos =
        strchr(dot_pos ? dot_pos : (uscore_pos ? uscore_pos : locale), '@');
...
static void append_locale_variants(GPtrArray* array, const gchar* locale)
{
    ...
    for (j = 0; j <= mask; ++j)
    {
        ...
        if ((i & ~mask) == 0)
        {
            gchar* val = g_strconcat(
                language, (i & COMPONENT_TERRITORY) ? territory : "",
 ...
 * newly allocated array of newly allocated strings with the locale variants.
 ...
 * of strings owned by GLib that must not be modified or freed.
 *
 * Since: 2.6
 */
 ...
 *
 * Returns: (array zero-terminated=1) (transfer none): a %NULL-terminated array
 * of strings owned by the thread g_get_language_names_with_category was called
 * from. It must not be modified or freed. It must be copied if planned to be
...
{
    static GPrivate cache_private =
        G_PRIVATE_INIT((void (*)(gpointer))g_hash_table_unref);
    ...
    name_cache =
        (GLanguageNamesCache*)g_hash_table_lookup(cache, category_name);
