...
struct saved_position : public saved_state
{
    ...
    saved_position(const re_syntax_base* ps, BidiIterator pos, int i) :
        saved_state(i), pstate(ps), position(pos){};
};
...
{
    saved_state** stack;
    save_state_init(saved_state** base, saved_state** end) : stack(base)
    {
        *base = static_cast<saved_state*>(get_mem_block());
        *end = reinterpret_cast<saved_state*>(reinterpret_cast<char*>(*base) +
...
struct saved_single_repeat : public saved_state
{
    ...
    saved_single_repeat(std::size_t c, const re_repeat* r, BidiIterator lp,
                        int arg_id) :
        saved_state(arg_id),
        count(c), rep(r), last_position(lp)
    {
    }
};
...
struct saved_recursion : public saved_state
{
    saved_recursion(int idx, const re_syntax_base* p, Results* pr,
                    Results* pr2) :
        saved_state(14),
        recursion_id(idx), preturn_address(p), internal_results(*pr),
        prior_results(*pr2)
    {
    }
    int recursion_id;
...
bool perl_matcher<BidiIterator, Allocator, traits>::match_all_states()
{
    static matcher_proc_type const s_match_vtable[34] = {
        ...
        // Although this next line *should* be evaluated at compile time, in
        // practice some compilers (VC++) emit run-time initialisation which
        // breaks thread safety, so use a dispatch function instead:
        //(::boost::is_random_access_iterator<BidiIterator>::value ?
        //&perl_matcher<BidiIterator, Allocator, traits>::match_dot_repeat_fast
        //: &perl_matcher<BidiIterator, Allocator,
        //traits>::match_dot_repeat_slow),
        &perl_matcher<BidiIterator, Allocator,
                      traits>::match_dot_repeat_dispatch,
...
void perl_matcher<BidiIterator, Allocator, traits>::extend_stack()
{
    if (used_block_count)
    {
        ...
        backup_state = reinterpret_cast<saved_state*>(
            reinterpret_cast<char*>(stack_base) + BOOST_REGEX_BLOCKSIZE);
        saved_extra_block* block =
            static_cast<saved_extra_block*>(backup_state);
        --block;
...
inline void perl_matcher<BidiIterator, Allocator, traits>::push_matched_paren(
    int index, const sub_match<BidiIterator>& sub)
{
    // BOOST_ASSERT(index);
...
template <class BidiIterator, class Allocator, class traits>
inline void
    perl_matcher<BidiIterator, Allocator, traits>::push_case_change(bool c)
{
    // BOOST_ASSERT(index);
...
template <class BidiIterator, class Allocator, class traits>
inline void
    perl_matcher<BidiIterator, Allocator, traits>::push_recursion_stopper()
{
...
template <class BidiIterator, class Allocator, class traits>
inline void perl_matcher<BidiIterator, Allocator, traits>::push_assertion(
    const re_syntax_base* ps, bool positive)
{
    saved_assertion<BidiIterator>* pmp =
        static_cast<saved_assertion<BidiIterator>*>(m_backup_state);
    --pmp;
    if (pmp < m_stack_base)
    {
        extend_stack();
...
template <class BidiIterator, class Allocator, class traits>
inline void perl_matcher<BidiIterator, Allocator, traits>::push_alt(
    const re_syntax_base* ps)
{
    saved_position<BidiIterator>* pmp =
        static_cast<saved_position<BidiIterator>*>(m_backup_state);
    --pmp;
    if (pmp < m_stack_base)
    {
        extend_stack();
...
template <class BidiIterator, class Allocator, class traits>
inline void
    perl_matcher<BidiIterator, Allocator, traits>::push_non_greedy_repeat(
        const re_syntax_base* ps)
{
    saved_position<BidiIterator>* pmp =
        static_cast<saved_position<BidiIterator>*>(m_backup_state);
    ...
    (void)new (pmp) saved_position<BidiIterator>(
        ps, position, saved_state_non_greedy_long_repeat);
    m_backup_state = pmp;
}

template <class BidiIterator, class Allocator, class traits>
inline void perl_matcher<BidiIterator, Allocator, traits>::push_repeater_count(
    int i, repeater_count<BidiIterator>** s)
{
    saved_repeater<BidiIterator>* pmp =
        static_cast<saved_repeater<BidiIterator>*>(m_backup_state);
    --pmp;
    if (pmp < m_stack_base)
    {
        extend_stack();
    ...
    (void)new (pmp) saved_repeater<BidiIterator>(
        i, s, position,
        this->recursion_stack.size() ? this->recursion_stack.back().idx
                                     : (INT_MIN + 3));
    m_backup_state = pmp;
}
...
inline void perl_matcher<BidiIterator, Allocator, traits>::push_single_repeat(
    std::size_t c, const re_repeat* r, BidiIterator last_position, int state_id)
{
    saved_single_repeat<BidiIterator>* pmp =
        static_cast<saved_single_repeat<BidiIterator>*>(m_backup_state);
    --pmp;
    if (pmp < m_stack_base)
    {
        extend_stack();
    ...
    (void)new (pmp)
        saved_single_repeat<BidiIterator>(c, r, last_position, state_id);
    m_backup_state = pmp;
}

template <class BidiIterator, class Allocator, class traits>
inline void perl_matcher<BidiIterator, Allocator, traits>::push_recursion(
    int idx, const re_syntax_base* p, results_type* presults,
    results_type* presults2)
{
    saved_recursion<results_type>* pmp =
        static_cast<saved_recursion<results_type>*>(m_backup_state);
    --pmp;
    if (pmp < m_stack_base)
    {
        extend_stack();
...
bool perl_matcher<BidiIterator, Allocator, traits>::match_startmark()
{
    ...
    switch (index)
    {
        ...
        case -2:
        {
            ...
            const re_syntax_base* next_pstate =
                static_cast<const re_jump*>(pstate->next.p)->alt.p->next.p;
        ...
        case -3:
        {
            ...
            try
            {
                ...
                if (!r && !m_independent)
                {
                    // Must be unwinding from a COMMIT/SKIP/PRUNE and the
                    // independent sub failed, need to unwind everything else:
                    while (unwind(false))
                        ;
                    return false;
                }
            ...
            if (r && (m_match_flags & match_extra))
            {
                ...
                {
                    typedef
                        typename sub_match<BidiIterator>::capture_sequence_type
                            seq;
        ...
        case -4:
        {
            ...
            else
            {
                ...
                bool negated =
                    static_cast<const re_brace*>(pstate)->index == -2;
...
#pragma option push -w-8008 -w-8066 -w-8004
    ...
    bool greedy =
        (rep->greedy) &&
        (!(m_match_flags & regex_constants::match_any) || m_independent);
    if (greedy)
    {
...
bool perl_matcher<BidiIterator, Allocator, traits>::match_dot_repeat_slow()
{
    ...
    if (greedy)
    {
        ...
        if (count - rep->min)
            push_single_repeat(count, rep, position,
    ...
    else
    {
        ...
        pstate = rep->alt.p;
        return (position == last) ? (rep->can_be_null & mask_skip)
                                  : can_start(*position, rep->_map, mask_skip);
    }
}
...
bool perl_matcher<BidiIterator, Allocator, traits>::match_dot_repeat_fast()
{
    ...
    if ((static_cast<const re_dot*>(pstate->next.p)->mask & match_any_mask) ==
        0)
        return match_dot_repeat_slow();
    ...
    bool greedy =
        (rep->greedy) &&
        (!(m_match_flags & regex_constants::match_any) || m_independent);
    std::size_t count = static_cast<std::size_t>((std::min)(
        static_cast<std::size_t>(
            ::boost::BOOST_REGEX_DETAIL_NS::distance(position, last)),
        greedy ? rep->max : rep->min));
    if (rep->min > count)
    {
        position = last;
...
#pragma option push -w-8008 -w-8066 -w-8004
    ...
    const char_type what = *reinterpret_cast<const char_type*>(
        static_cast<const re_literal*>(rep->next.p) + 1);
    std::size_t count = 0;
    //
    ...
    if (::boost::is_random_access_iterator<BidiIterator>::value)
    {
        ...
        // Move end forward by "desired", preferably without using distance or
        // advance if we can as these can be slow for some iterator types.
        std::size_t len =
            (desired == (std::numeric_limits<std::size_t>::max)())
                ? 0u
                : ::boost::BOOST_REGEX_DETAIL_NS::distance(position, last);
        if (desired >= len)
            end = last;
        ...
        while ((position != end) &&
               (traits_inst.translate(*position, icase) == what))
        {
            ++position;
        }
        count = (unsigned)::boost::BOOST_REGEX_DETAIL_NS::distance(origin,
                                                                   position);
    }
    else
    {
    ...
    if (::boost::is_random_access_iterator<BidiIterator>::value)
    {
        ...
        while ((position != end) &&
               map[static_cast<unsigned char>(
                   traits_inst.translate(*position, icase))])
        {
            ++position;
        }
        count = (unsigned)::boost::BOOST_REGEX_DETAIL_NS::distance(origin,
...
#pragma option push -w-8008 -w-8066 -w-8004
    ...
    const re_repeat* rep = static_cast<const re_repeat*>(pstate);
    const re_set_long<m_type>* set =
        static_cast<const re_set_long<m_type>*>(pstate->next.p);
    std::size_t count = 0;
    //
    // start by working out how much we can skip:
    //
    ...
    if (::boost::is_random_access_iterator<BidiIterator>::value)
    {
        ...
        while ((position != end) &&
               (position !=
                re_is_set_member(position, last, set, re.get_data(), icase)))
        {
            ++position;
        }
        count = (unsigned)::boost::BOOST_REGEX_DETAIL_NS::distance(origin,
...
bool perl_matcher<BidiIterator, Allocator, traits>::match_recursion()
{
    ...
    //
    for (typename std::vector<recursion_info<results_type>>::reverse_iterator
             i = recursion_stack.rbegin();
         i != recursion_stack.rend(); ++i)
    {
        if (i->idx == static_cast<const re_brace*>(
                          static_cast<const re_jump*>(pstate)->alt.p)
                          ->index)
        {
    ...
    {
        push_repeater_count(-(2 + static_cast<const re_brace*>(pstate)->index),
                            &next_count);
    }
        ...
        if (!recursion_stack.empty())
        {
            if (index == recursion_stack.back().idx)
            {
                ...
                *m_presult = recursion_stack.back().results;
                push_recursion(recursion_stack.back().idx,
...
bool perl_matcher<BidiIterator, Allocator, traits>::match_match()
{
    if (!recursion_stack.empty())
    {
        ...
        pstate = recursion_stack.back().preturn_address;
        push_recursion(recursion_stack.back().idx,
                       recursion_stack.back().preturn_address, m_presult,
    ...
    if ((m_match_flags & match_all) && (position != last))
        return false;
    if ((m_match_flags & regex_constants::match_not_initial_null) &&
        (position == search_base))
        return false;
...
bool perl_matcher<BidiIterator, Allocator, traits>::match_commit()
{
    ...
    switch (static_cast<const re_commit*>(pstate)->action)
    {
        ...
        case commit_skip:
            ...
            {
                restart = position;
                // Have to decrement restart since it will get incremented again
                // later:
                --restart;
            }
            break;
...
template <class BidiIterator, class Allocator, class traits>
bool perl_matcher<BidiIterator, Allocator, traits>::skip_until_paren(
    int index, bool have_match)
{
    while (pstate)
    {
        if (pstate->type == syntax_element_endmark)
        {
            ...
            {
                // Unenclosed closing ), occurs when (*ACCEPT) is inside some
                // other parenthesis which may or may not have other side
                ...
                {
                    unwind(true);
                    // unwind may leave pstate NULL if we've unwound a forward
                    // lookahead, in which case just move to the next state and
                    // keep looking...
                    if (!pstate)
                        pstate = sp->next.p;
                }
...
bool perl_matcher<BidiIterator, Allocator, traits>::unwind(bool have_match)
{
    static unwind_proc_type const s_unwind_table[19] = {
        ...
        &perl_matcher<BidiIterator, Allocator,
                      traits>::unwind_recursion_stopper,
        ...
        &perl_matcher<BidiIterator, Allocator,
                      traits>::unwind_greedy_single_repeat,
        ...
        &perl_matcher<BidiIterator, Allocator,
                      traits>::unwind_non_greedy_repeat,
...
template <class BidiIterator, class Allocator, class traits>
bool perl_matcher<BidiIterator, Allocator, traits>::unwind_paren(
    bool have_match)
{
    saved_matched_paren<BidiIterator>* pmp =
        static_cast<saved_matched_paren<BidiIterator>*>(m_backup_state);
...
template <class BidiIterator, class Allocator, class traits>
bool perl_matcher<BidiIterator, Allocator, traits>::unwind_recursion_stopper(
    bool)
{
...
template <class BidiIterator, class Allocator, class traits>
bool perl_matcher<BidiIterator, Allocator, traits>::unwind_repeater_counter(
    bool)
{
    saved_repeater<BidiIterator>* pmp =
        static_cast<saved_repeater<BidiIterator>*>(m_backup_state);
...
template <class BidiIterator, class Allocator, class traits>
inline void
    perl_matcher<BidiIterator, Allocator, traits>::destroy_single_repeat()
{
    saved_single_repeat<BidiIterator>* p =
        static_cast<saved_single_repeat<BidiIterator>*>(m_backup_state);
...
template <class BidiIterator, class Allocator, class traits>
bool perl_matcher<BidiIterator, Allocator, traits>::unwind_greedy_single_repeat(
    bool r)
{
    saved_single_repeat<BidiIterator>* pmp =
        static_cast<saved_single_repeat<BidiIterator>*>(m_backup_state);
...
template <class BidiIterator, class Allocator, class traits>
bool perl_matcher<BidiIterator, Allocator, traits>::unwind_slow_dot_repeat(
    bool r)
{
    saved_single_repeat<BidiIterator>* pmp =
        static_cast<saved_single_repeat<BidiIterator>*>(m_backup_state);
    ...
    if (position != last)
    {
        ...
        do
        {
            ...
            ++state_count;
            pstate = rep->next.p;
        } while ((count < rep->max) && (position != last) &&
                 !can_start(*position, rep->_map, mask_skip));
    }
    if (position == last)
    {
...
template <class BidiIterator, class Allocator, class traits>
bool perl_matcher<BidiIterator, Allocator, traits>::unwind_fast_dot_repeat(
    bool r)
{
    saved_single_repeat<BidiIterator>* pmp =
        static_cast<saved_single_repeat<BidiIterator>*>(m_backup_state);
...
bool perl_matcher<BidiIterator, Allocator, traits>::unwind_char_repeat(bool r)
{
    ...
    pstate = rep->next.p;
    const char_type what = *reinterpret_cast<const char_type*>(
        static_cast<const re_literal*>(pstate) + 1);
    position = pmp->last_position;
...
template <class BidiIterator, class Allocator, class traits>
bool perl_matcher<BidiIterator, Allocator, traits>::unwind_short_set_repeat(
    bool r)
{
    saved_single_repeat<BidiIterator>* pmp =
        static_cast<saved_single_repeat<BidiIterator>*>(m_backup_state);
    ...
    if (position != last)
    {
        ...
        do
        {
            if (!map[static_cast<unsigned char>(
                    traits_inst.translate(*position, icase))])
            {
                // failed repeat match, discard this state and look for another:
                destroy_single_repeat();
...
template <class BidiIterator, class Allocator, class traits>
bool perl_matcher<BidiIterator, Allocator, traits>::unwind_long_set_repeat(
    bool r)
{
    typedef typename traits::char_class_type m_type;
    saved_single_repeat<BidiIterator>* pmp =
    ...
    pstate = rep->next.p;
    const re_set_long<m_type>* set =
        static_cast<const re_set_long<m_type>*>(pstate);
    position = pmp->last_position;
...
template <class BidiIterator, class Allocator, class traits>
bool perl_matcher<BidiIterator, Allocator, traits>::unwind_non_greedy_repeat(
    bool r)
{
    saved_position<BidiIterator>* pmp =
        static_cast<saved_position<BidiIterator>*>(m_backup_state);
...
bool perl_matcher<BidiIterator, Allocator, traits>::unwind_commit(bool b)
{
    ...
    // This prevents us from stopping when we exit from an independent
    // sub-expression:
    m_independent = false;
    return false;
}
...
bool perl_matcher<BidiIterator, Allocator, traits>::unwind_then(bool b)
{
    ...
    // since *all* the other alternatives must fail once we've reached a THEN
    // clause:
...
/*
template <class BidiIterator, class Allocator, class traits>
bool perl_matcher<BidiIterator, Allocator, traits>::unwind_parenthesis_pop(bool
r)
{
   saved_state* pmp = static_cast<saved_state*>(m_backup_state);
   if(!r)
...
template <class BidiIterator, class Allocator, class traits>
bool perl_matcher<BidiIterator, Allocator, traits>::unwind_parenthesis_push(bool
r)
{
   saved_position<BidiIterator>* pmp =
static_cast<saved_position<BidiIterator>*>(m_backup_state); if(!r)
   {
...
template <class BidiIterator, class Allocator, class traits>
inline void perl_matcher<BidiIterator, Allocator,
traits>::push_parenthesis_push(BidiIterator p)
{
   saved_position<BidiIterator>* pmp =
static_cast<saved_position<BidiIterator>*>(m_backup_state);
   --pmp;
