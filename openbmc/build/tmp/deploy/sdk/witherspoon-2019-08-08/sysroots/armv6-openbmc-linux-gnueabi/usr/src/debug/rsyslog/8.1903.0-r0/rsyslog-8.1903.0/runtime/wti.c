...
/* advise all workers to start by interrupting them. That should unblock all
 * srSleep() calls.
 */
 ...
 * Note that when waiting for the thread to terminate, we do a busy wait,
 * checking progress every 10ms. It is very unlikely that we will ever cancel a
 * thread and, if so, it will only happen at the end of the rsyslog run. So
 * doing this kind of non-optimal wait is considered preferable over using
 * condition variables. rgerhards, 2008-02-26
 */
    ...
    wtiCancelThrd(wti_t* pThis, const uchar* const cancelobj)
{
    ...
    if (wtiGetState(pThis) != WRKTHRD_STOPPED)
    {
        ...
        /* we first try the cooperative "cancel" interface */
        pthread_kill(pThis->thrdID, SIGTTIN);
        DBGPRINTF("sent SIGTTIN to worker thread %p, giving it a chance to "
                  "terminate\n",
                  (void*)pThis->thrdID);
        srSleep(0, 50000);
        wtiJoinThrd(pThis);
    ...
    if (wtiGetState(pThis) != WRKTHRD_STOPPED)
    {
        LogMsg(0, RS_RET_ERR, LOG_WARNING, "%s: need to do hard cancellation",
               cancelobj);
        if (dbgTimeoutToStderr)
        {
        ...
        pthread_kill(pThis->thrdID, SIGTTIN);
        DBGPRINTF(
            "cooperative worker termination failed, using cancellation...\n");
        DBGOPRINT((obj_t*)pThis, "canceling worker thread\n");
        pthread_cancel(pThis->thrdID);
        /* now wait until the thread terminates... */
        while (wtiGetState(pThis) != WRKTHRD_STOPPED &&
               wtiGetState(pThis) != WRKTHRD_WAIT_JOIN)
        {
...
rsRetVal wtiNewIParam(wti_t* const pWti, action_t* const pAction,
                      actWrkrIParams_t** piparams)
{
    ...
    {
        /* we need to extend */
        newMax = (wrkrInfo->p.tx.maxIParams == 0)
                     ? CONF_IPARAMS_BUFSIZE
                     : 2 * wrkrInfo->p.tx.maxIParams;
        CHKmalloc(iparams = realloc(wrkrInfo->p.tx.iparams,
                                    sizeof(actWrkrIParams_t) *
                                        pAction->iNumTpls * newMax));
        memset(iparams + (wrkrInfo->p.tx.currIParam * pAction->iNumTpls), 0,
               sizeof(actWrkrIParams_t) * pAction->iNumTpls *
                   (newMax - wrkrInfo->p.tx.maxIParams));
        wrkrInfo->p.tx.iparams = iparams;
        wrkrInfo->p.tx.maxIParams = newMax;
    }
    *piparams =
        wrkrInfo->p.tx.iparams + wrkrInfo->p.tx.currIParam * pAction->iNumTpls;
...
/* Destructor */
BEGINobjDestruct(wti) /* be sure to specify the object type also in END and
                         CODESTART macros! */
    CODESTARTobjDestruct(wti) if (wtiGetState(pThis) != WRKTHRD_STOPPED)
{
...
ENDobjDestruct(wti)
     ...
     */
    BEGINobjConstruct(
        wti) /* be sure to specify the object type also in END macro! */
    ...
    rsRetVal wtiConstructFinalize(wti_t* pThis)
{
    ...
    DBGPRINTF("%s: finalizing construction of worker instance data (for %d "
              "actions)\n",
    ...
    {
        dbgprintf(
            "wtiConstructFinalize: pWtp not set, this may be intentional\n");
        FINALIZE;
    }

    /* we now alloc the array for user pointers. We obtain the max from the
     * queue itself. */
 ...
 * also be used for other cases. This function waits until the queue is
 * non-empty or a timeout occurs. The timeout must be passed in as absolute
 * value.
    ...
    wtiWaitNonEmpty(wti_t* const pThis, const struct timespec timeout)
{
    ...
    if (d_pthread_cond_timedwait(&pThis->pcondBusy, pWtp->pmutUsr, &timeout) !=
        0)
    {
        r = 0;
    }
...
static void ATTR_NONNULL()
    doIdleProcessing(wti_t* const pThis, wtp_t* const pWtp,
                     int* const pbInactivityTOOccured)
{
    struct timespec t;
...
{
    wtp_t* __restrict__ const pWtp =
        pThis->pWtp; /* our worker thread pool -- shortcut */
    ...
    while (1)
    { /* loop will be broken below */
        if (pWtp->pfRateLimiter != NULL)
        { /* call rate-limiter, if defined */
        ...
        if (wrkrInfo->actWrkrData != NULL)
        {
            ...
            if (pAction->isTransactional)
            {
                ...
                for (j = 0; j < wrkrInfo->p.tx.maxIParams; ++j)
                {
                    ...
                    {
                        free(actParam(wrkrInfo->p.tx.iparams, pAction->iNumTpls,
                                      j, k)
                                 .param);
                    }
                }
...
ENDObjClassExit(wti)
     ...
     */
    BEGINObjClassInit(wti, 1,
                      OBJ_IS_CORE_MODULE) /* one is the object version (most
                                             important for persisting) */
    int r;
