...
G_DEFINE_TYPE_WITH_CODE(
    GDBusObjectManagerClient, g_dbus_object_manager_client, G_TYPE_OBJECT,
...
static void process_get_all_result(GDBusObjectManagerClient* manager,
                                   GVariant* value, const gchar* name_owner);

static void g_dbus_object_manager_client_finalize(GObject* object)
{
    ...
    if (manager->priv->control_proxy != NULL)
    {
        g_signal_handlers_disconnect_by_func(
            manager->priv->control_proxy, on_control_proxy_g_signal, manager);
    ...
    if (manager->priv->get_proxy_type_destroy_notify != NULL)
        manager->priv->get_proxy_type_destroy_notify(
            manager->priv->get_proxy_type_user_data);

    g_mutex_clear(&manager->priv->lock);

    if (G_OBJECT_CLASS(g_dbus_object_manager_client_parent_class)->finalize !=
        NULL)
        G_OBJECT_CLASS(g_dbus_object_manager_client_parent_class)
            ->finalize(object);
}

static void g_dbus_object_manager_client_get_property(GObject* _object,
                                                      guint prop_id,
                                                      GValue* value,
                                                      GParamSpec* pspec)
{
    ...
    switch (prop_id)
    {
        case PROP_CONNECTION:
            g_value_set_object(
                value, g_dbus_object_manager_client_get_connection(manager));
            break;

        case PROP_OBJECT_PATH:
            g_value_set_string(value, g_dbus_object_manager_get_object_path(
        ...
        case PROP_NAME_OWNER:
            g_value_take_string(
                value, g_dbus_object_manager_client_get_name_owner(manager));
            break;
...
static void g_dbus_object_manager_client_set_property(GObject* _object,
                                                      guint prop_id,
                                                      const GValue* value,
                                                      GParamSpec* pspec)
{
...
static void g_dbus_object_manager_client_class_init(
    GDBusObjectManagerClientClass* klass)
{
    ...
    g_object_class_install_property(
        ...
        g_param_spec_object("connection", "Connection", "The connection to use",
     ...
     */
    g_object_class_install_property(
        gobject_class, PROP_BUS_TYPE,
        g_param_spec_enum(
            "bus-type", "Bus Type", "The bus to connect to, if any",
    ...
    g_object_class_install_property(
        ...
        g_param_spec_flags("flags", "Flags", "Flags for the proxy manager",
     ...
     */
    g_object_class_install_property(
        gobject_class, PROP_OBJECT_PATH,
        g_param_spec_string("object-path", "Object Path",
                            "The object path of the control object", NULL,
                            G_PARAM_READABLE | G_PARAM_WRITABLE |
                                G_PARAM_CONSTRUCT_ONLY |
                                G_PARAM_STATIC_STRINGS));
    ...
    g_object_class_install_property(
        ...
        g_param_spec_string(
            "name", "Name", "Name that the manager is for", NULL,
     ...
     */
    g_object_class_install_property(
        gobject_class, PROP_NAME_OWNER,
        g_param_spec_string("name-owner", "Name Owner",
                            "The owner of the name we are watching", NULL,
     ...
     */
    g_object_class_install_property(
        gobject_class, PROP_GET_PROXY_TYPE_FUNC,
        g_param_spec_pointer(
            "get-proxy-type-func", "GDBusProxyTypeFunc Function Pointer",
     ...
     *
     * The #gpointer user_data to pass to
     * #GDBusObjectManagerClient:get-proxy-type-func.
     *
     * Since: 2.30
     */
    g_object_class_install_property(
        gobject_class, PROP_GET_PROXY_TYPE_USER_DATA,
        g_param_spec_pointer(
            "get-proxy-type-user-data", "GDBusProxyTypeFunc User Data",
     ...
     *
     * A #GDestroyNotify for the #gpointer user_data in
     * #GDBusObjectManagerClient:get-proxy-type-user-data.
     *
     * Since: 2.30
     */
    g_object_class_install_property(
        gobject_class, PROP_GET_PROXY_TYPE_DESTROY_NOTIFY,
     ...
     * @object_proxy: The #GDBusObjectProxy on which an interface is emitting a
     * D-Bus signal.
     * @interface_proxy: The #GDBusProxy that is emitting a D-Bus signal.
     * @sender_name: The sender of the signal or NULL if the connection is not a
     * bus connection.
     ...
     * Since: 2.30
     */
    signals[INTERFACE_PROXY_SIGNAL_SIGNAL] = g_signal_new(
        I_("interface-proxy-signal"), G_TYPE_DBUS_OBJECT_MANAGER_CLIENT,
        G_SIGNAL_RUN_LAST,
        G_STRUCT_OFFSET(GDBusObjectManagerClientClass, interface_proxy_signal),
        NULL, NULL, NULL, G_TYPE_NONE, 5, G_TYPE_DBUS_OBJECT_PROXY,
        G_TYPE_DBUS_PROXY, G_TYPE_STRING, G_TYPE_STRING, G_TYPE_VARIANT);

    /**
     * GDBusObjectManagerClient::interface-proxy-properties-changed:
     * @manager: The #GDBusObjectManagerClient emitting the signal.
     * @object_proxy: The #GDBusObjectProxy on which an interface has properties
     * that are changing.
     ...
     * (type: `a{sv}`).
     * @invalidated_properties: (array zero-terminated=1) (element-type utf8): A
     * %NULL terminated array of properties that were invalidated.
     *
    ...
    signals[INTERFACE_PROXY_PROPERTIES_CHANGED_SIGNAL] =
        g_signal_new(I_("interface-proxy-properties-changed"),
                     G_TYPE_DBUS_OBJECT_MANAGER_CLIENT, G_SIGNAL_RUN_LAST,
                     G_STRUCT_OFFSET(GDBusObjectManagerClientClass,
                                     interface_proxy_properties_changed),
                     NULL, NULL, NULL, G_TYPE_NONE, 4, G_TYPE_DBUS_OBJECT_PROXY,
                     G_TYPE_DBUS_PROXY, G_TYPE_VARIANT, G_TYPE_STRV);
}

static void g_dbus_object_manager_client_init(GDBusObjectManagerClient* manager)
{
    manager->priv = g_dbus_object_manager_client_get_instance_private(manager);
    g_mutex_init(&manager->priv->lock);
    manager->priv->map_object_path_to_object_proxy = g_hash_table_new_full(
 ...
 * @flags: Zero or more flags from the #GDBusObjectManagerClientFlags
 * enumeration.
 * @name: (nullable): The owner of the control object (unique or well-known
 * name), or %NULL when not using a message bus connection.
 * @object_path: The object path of the control object.
 * @get_proxy_type_func: (nullable): A #GDBusProxyTypeFunc function or %NULL to
 * always construct #GDBusProxy proxies.
 * @get_proxy_type_user_data: User data to pass to @get_proxy_type_func.
 * @get_proxy_type_destroy_notify: (nullable): Free function for
 * @get_proxy_type_user_data or %NULL.
...
GDBusObjectManager* g_dbus_object_manager_client_new_sync(
    GDBusConnection* connection, GDBusObjectManagerClientFlags flags,
    const gchar* name, const gchar* object_path,
    GDBusProxyTypeFunc get_proxy_type_func, gpointer get_proxy_type_user_data,
    GDestroyNotify get_proxy_type_destroy_notify, GCancellable* cancellable,
    GError** error)
{
    GInitable* initable;
    ...
    initable = g_initable_new(
        G_TYPE_DBUS_OBJECT_MANAGER_CLIENT, cancellable, error, "connection",
        connection, "flags", flags, "name", name, "object-path", object_path,
        "get-proxy-type-func", get_proxy_type_func, "get-proxy-type-user-data",
        get_proxy_type_user_data, "get-proxy-type-destroy-notify",
        get_proxy_type_destroy_notify, NULL);
    if (initable != NULL)
    ...
    GAsyncReadyCallback callback, gpointer user_data)
{
    ...
    g_async_initable_new_async(
        G_TYPE_DBUS_OBJECT_MANAGER_CLIENT, G_PRIORITY_DEFAULT, cancellable,
        callback, user_data, "connection", connection, "flags", flags, "name",
        name, "object-path", object_path, "get-proxy-type-func",
        get_proxy_type_func, "get-proxy-type-user-data",
        get_proxy_type_user_data, "get-proxy-type-destroy-notify",
        get_proxy_type_destroy_notify, NULL);
}
 ...
 * g_dbus_object_manager_client_new_for_bus() for the asynchronous version.
 *
 * Returns: (transfer full) (type GDBusObjectManagerClient): A
 *   #GDBusObjectManagerClient object or %NULL if @error is set. Free
 *   with g_object_unref().
...
GDBusObjectManager* g_dbus_object_manager_client_new_for_bus_sync(
    GBusType bus_type, GDBusObjectManagerClientFlags flags, const gchar* name,
    const gchar* object_path, GDBusProxyTypeFunc get_proxy_type_func,
    gpointer get_proxy_type_user_data,
    GDestroyNotify get_proxy_type_destroy_notify, GCancellable* cancellable,
    GError** error)
{
    ...
    initable = g_initable_new(
        G_TYPE_DBUS_OBJECT_MANAGER_CLIENT, cancellable, error, "bus-type",
        bus_type, "flags", flags, "name", name, "object-path", object_path,
        "get-proxy-type-func", get_proxy_type_func, "get-proxy-type-user-data",
        get_proxy_type_user_data, "get-proxy-type-destroy-notify",
        get_proxy_type_destroy_notify, NULL);
    if (initable != NULL)
 ...
 * of the thread you are calling this method from. You can
 * then call g_dbus_object_manager_client_new_for_bus_finish() to get the
 * result. See g_dbus_object_manager_client_new_for_bus_sync() for the
 * synchronous version.
 *
 * Since: 2.30
 */
    ...
    GAsyncReadyCallback callback, gpointer user_data)
{
    ...
    g_async_initable_new_async(
        G_TYPE_DBUS_OBJECT_MANAGER_CLIENT, G_PRIORITY_DEFAULT, cancellable,
        callback, user_data, "bus-type", bus_type, "flags", flags, "name", name,
        "object-path", object_path, "get-proxy-type-func", get_proxy_type_func,
        "get-proxy-type-user-data", get_proxy_type_user_data,
        "get-proxy-type-destroy-notify", get_proxy_type_destroy_notify, NULL);
}
    ...
    g_dbus_object_manager_client_get_flags(GDBusObjectManagerClient* manager)
{
    ...
    g_return_val_if_fail(G_IS_DBUS_OBJECT_MANAGER_CLIENT(manager),
                         G_DBUS_OBJECT_MANAGER_CLIENT_FLAGS_NONE);
    g_mutex_lock(&manager->priv->lock);
                      ...
                      gpointer user_data)
{
    ...
    object_proxy = g_hash_table_lookup(
        manager->priv->map_object_path_to_object_proxy, object_path);
    ...
    if (g_strcmp0(interface_name, "org.freedesktop.DBus.Properties") == 0)
    {
        if (g_strcmp0(signal_name, "PropertiesChanged") == 0)
        {
            ...
            g_variant_get(parameters, "(&s@a{sv}^a&s)", &interface_name,
                          &changed_properties, &invalidated_properties);

            interface = g_dbus_object_get_interface(G_DBUS_OBJECT(object_proxy),
                                                    interface_name);
            if (interface != NULL)
            {
                ...
                while (g_variant_iter_next(&property_iter, "{&sv}",
                                           &property_name, &property_value))
                {
                    g_dbus_proxy_set_cached_property(
                        G_DBUS_PROXY(interface), property_name, property_value);
                ...
                g_signal_emit_by_name(interface, "g-properties-changed",
                                      changed_properties,
                                      invalidated_properties);
                g_signal_emit(
                    manager, signals[INTERFACE_PROXY_PROPERTIES_CHANGED_SIGNAL],
                    0, object_proxy, interface, changed_properties,
                    invalidated_properties);
    ...
    else
    {
        ...
        if (interface != NULL)
        {
            g_signal_emit_by_name(interface, "g-signal", sender_name,
                                  signal_name, parameters);
            g_signal_emit(manager, signals[INTERFACE_PROXY_SIGNAL_SIGNAL], 0,
                          object_proxy, interface, sender_name, signal_name,
                          parameters);
            g_object_unref(interface);
        }
    }
...
static void subscribe_signals(GDBusObjectManagerClient* manager,
                              const gchar* name_owner)
{
    ...
    if (name_owner != NULL)
    {
        /* Only add path_namespace if it's non-'/'. This removes a no-op key
         * from the match rule, and also works around a D-Bus bug where
        ...
        if (g_str_equal(manager->priv->object_path, "/"))
        {
            manager->priv->match_rule =
                g_strdup_printf("type='signal',sender='%s'", name_owner);
        }
        else
        {
            manager->priv->match_rule =
        ...
        ret = g_dbus_connection_call_sync(
            manager->priv->connection, "org.freedesktop.DBus",
            "/org/freedesktop/DBus", "org.freedesktop.DBus", "AddMatch",
            g_variant_new("(s)", manager->priv->match_rule),
            NULL,                       /* reply_type */
            G_DBUS_CALL_FLAGS_NONE, -1, /* default timeout */
    ...
    if (error == NULL)
    {
        ...
        manager->priv->signal_subscription_id =
            g_dbus_connection_signal_subscribe(
                manager->priv->connection, name_owner, NULL, /* interface */
                NULL,                                        /* member */
                NULL, /* path - TODO: really want wilcard support here */
                NULL, /* arg0 */
                G_DBUS_SIGNAL_FLAGS_NONE | G_DBUS_SIGNAL_FLAGS_NO_MATCH_RULE,
                signal_cb, manager, NULL); /* user_data_free_func */
    }
    else
    {
        /* TODO: we could report this to the user
        g_warning ("Message bus daemon does not support path_namespace: %s (%s
        %d)", error->message, g_quark_to_string (error->domain), error->code);
        */
        ...
        manager->priv->signal_subscription_id =
            g_dbus_connection_signal_subscribe(
                ...
                NULL, /* path - TODO: really want wilcard support here */
                NULL, /* arg0 */
                G_DBUS_SIGNAL_FLAGS_NONE, signal_cb, manager,
...
static void maybe_unsubscribe_signals(GDBusObjectManagerClient* manager)
{
    ...
    if (manager->priv->signal_subscription_id > 0)
    {
        g_dbus_connection_signal_unsubscribe(
            manager->priv->connection, manager->priv->signal_subscription_id);
...
static void on_notify_g_name_owner(GObject* object, GParamSpec* pspec,
                                   gpointer user_data)
{
    GDBusObjectManagerClient* manager = G_DBUS_OBJECT_MANAGER_CLIENT(user_data);
    ...
    if (g_strcmp0(old_name_owner, new_name_owner) != 0)
    {
        ...
        proxies = g_hash_table_get_values(
    ...
    if (new_name_owner != NULL)
    {
        ...
        /* TODO: do this async! */
        subscribe_signals(manager, new_name_owner);
        error = NULL;
        value =
            g_dbus_proxy_call_sync(manager->priv->control_proxy,
                                   "GetManagedObjects", NULL, /* parameters */
                                   G_DBUS_CALL_FLAGS_NONE, -1, NULL, &error);
        if (value == NULL)
        {
            maybe_unsubscribe_signals(manager);
            g_warning("Error calling GetManagedObjects() when name owner %s "
                      "for name %s came back: %s",
                      new_name_owner, manager->priv->name, error->message);
            g_error_free(error);
        }
        else
        {
        ...
        /* do the :name-owner notify *AFTER* emitting ::object-proxy-added
         * signals - this way the user knows that the signals were emitted
         * because the name owner came back
         */
        g_mutex_lock(&manager->priv->lock);
                              ...
                              GError** error)
{
    ...
    if (manager->priv->bus_type != G_BUS_TYPE_NONE)
    {
        ...
        manager->priv->connection =
            g_bus_get_sync(manager->priv->bus_type, cancellable, error);
        if (manager->priv->connection == NULL)
            goto out;
    }

    proxy_flags = G_DBUS_PROXY_FLAGS_DO_NOT_LOAD_PROPERTIES;
    if (manager->priv->flags &
        G_DBUS_OBJECT_MANAGER_CLIENT_FLAGS_DO_NOT_AUTO_START)
        proxy_flags |= G_DBUS_PROXY_FLAGS_DO_NOT_AUTO_START;

    manager->priv->control_proxy = g_dbus_proxy_new_sync(
        manager->priv->connection, proxy_flags, NULL, /* GDBusInterfaceInfo* */
        manager->priv->name, manager->priv->object_path,
        "org.freedesktop.DBus.ObjectManager", cancellable, error);
    if (manager->priv->control_proxy == NULL)
        goto out;

    g_signal_connect(G_OBJECT(manager->priv->control_proxy),
                     "notify::g-name-owner", G_CALLBACK(on_notify_g_name_owner),
                     manager);

    g_signal_connect(manager->priv->control_proxy, "g-signal",
                     G_CALLBACK(on_control_proxy_g_signal), manager);

    manager->priv->name_owner =
        g_dbus_proxy_get_name_owner(manager->priv->control_proxy);
    ...
    else
    {
        ...
        subscribe_signals(manager, manager->priv->name_owner);
        value = g_dbus_proxy_call_sync(
            manager->priv->control_proxy, "GetManagedObjects",
            NULL, /* parameters */
            G_DBUS_CALL_FLAGS_NONE, -1, cancellable, error);
        if (value == NULL)
        {
            maybe_unsubscribe_signals(manager);
            g_warn_if_fail(g_signal_handlers_disconnect_by_func(
                               manager->priv->control_proxy,
                               on_control_proxy_g_signal, manager) == 1);
            g_object_unref(manager->priv->control_proxy);
...
static void add_interfaces(GDBusObjectManagerClient* manager,
                           ...
                           const gchar* name_owner)
{
    ...
    op = g_hash_table_lookup(manager->priv->map_object_path_to_object_proxy,
                             object_path);
    if (op == NULL)
    {
        ...
        {
            object_proxy_type = manager->priv->get_proxy_type_func(
                manager, object_path, NULL,
                manager->priv->get_proxy_type_user_data);
            g_warn_if_fail(
                g_type_is_a(object_proxy_type, G_TYPE_DBUS_OBJECT_PROXY));
        }
        else
        {
            object_proxy_type = G_TYPE_DBUS_OBJECT_PROXY;
        }
        op = g_object_new(object_proxy_type, "g-connection",
                          manager->priv->connection, "g-object-path",
                          object_path, NULL);
        added = TRUE;
    }
    g_object_ref(op);
        ...
        g_variant_iter_next(&iter, "{&s@a{sv}}", &interface_name, &properties))
    {
        ...
        if (manager->priv->get_proxy_type_func != NULL)
        {
            interface_proxy_type = manager->priv->get_proxy_type_func(
                manager, object_path, interface_name,
                manager->priv->get_proxy_type_user_data);
            g_warn_if_fail(
                g_type_is_a(interface_proxy_type, G_TYPE_DBUS_PROXY));
        }
        else
        {
        ...
        /* this is fine - there is no blocking IO because we pass
         * DO_NOT_LOAD_PROPERTIES and DO_NOT_CONNECT_SIGNALS and use a unique
         * name
         */
        error = NULL;
        interface_proxy = g_initable_new(
            interface_proxy_type, NULL, /* GCancellable */
            &error, "g-connection", manager->priv->connection, "g-flags",
            G_DBUS_PROXY_FLAGS_DO_NOT_LOAD_PROPERTIES |
                G_DBUS_PROXY_FLAGS_DO_NOT_CONNECT_SIGNALS,
            "g-name", name_owner, "g-object-path", object_path,
            "g-interface-name", interface_name, NULL);
        if (interface_proxy == NULL)
        {
            g_warning(
                "%s: Error constructing proxy for path %s and interface %s: %s",
                G_STRLOC, object_path, interface_name, error->message);
            g_error_free(error);
        }
        else
        {
            ...
            while (g_variant_iter_next(&property_iter, "{&sv}", &property_name,
                                       &property_value))
            {
                g_dbus_proxy_set_cached_property(interface_proxy, property_name,
                                                 property_value);
                g_variant_unref(property_value);
            }
            ...
            if (!added)
                interface_added_signals = g_list_append(
                    interface_added_signals, g_object_ref(interface_proxy));
...
static void remove_interfaces(GDBusObjectManagerClient* manager,
                              const gchar* object_path,
                              const gchar* const* interface_names)
{
    ...
    if (op == NULL)
    {
        g_warning("%s: Processing InterfaceRemoved signal for path %s but no "
                  "object proxy exists",
                  G_STRLOC, object_path);
        g_mutex_unlock(&manager->priv->lock);
        goto out;
    }
    ...
    else
    {
        ...
        {
            GDBusInterface* interface;
            interface = g_dbus_object_get_interface(G_DBUS_OBJECT(op),
                                                    interface_names[n]);
            _g_dbus_object_proxy_remove_interface(op, interface_names[n]);
            if (interface != NULL)
            {
                g_signal_emit_by_name(manager, "interface-removed", op,
                                      interface);
                g_object_unref(interface);
            }
        }
                                      ...
                                      GVariant* parameters, gpointer user_data)
{
    ...
    // g_debug ("yay, g_signal %s: %s\n", signal_name, g_variant_print
    // (parameters, TRUE));

    if (g_strcmp0(signal_name, "InterfacesAdded") == 0)
    {
        GVariant* ifaces_and_properties;
        g_variant_get(parameters, "(&o@a{sa{sv}})", &object_path,
                      &ifaces_and_properties);
        add_interfaces(manager, object_path, ifaces_and_properties,
                       manager->priv->name_owner);
        g_variant_unref(ifaces_and_properties);
    }
    ...
    {
        const gchar** ifaces;
        g_variant_get(parameters, "(&o^a&s)", &object_path, &ifaces);
                                            ...
                                            const gchar* object_path)
{
    ...
    g_mutex_lock(&manager->priv->lock);
    ret = g_hash_table_lookup(manager->priv->map_object_path_to_object_proxy,
                              object_path);
    if (ret != NULL)
        g_object_ref(ret);
    g_mutex_unlock(&manager->priv->lock);
    ...
    g_dbus_object_manager_client_get_objects(GDBusObjectManager* _manager)
{
    ...
    g_mutex_lock(&manager->priv->lock);
    ret =
        g_hash_table_get_values(manager->priv->map_object_path_to_object_proxy);
