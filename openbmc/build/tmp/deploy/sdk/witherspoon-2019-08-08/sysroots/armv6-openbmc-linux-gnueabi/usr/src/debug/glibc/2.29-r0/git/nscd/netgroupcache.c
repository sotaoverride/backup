...
static const netgroup_response_header disabled = {
    .version = NSCD_VERSION, .found = -1, .nresults = 0, .result_len = 0};

/* This is the struct describing how to write this record.  */
const struct iovec netgroup_iov_disabled = {.iov_base = (void*)&disabled,
                                            .iov_len = sizeof(disabled)};

/* This is the standard reply in case we haven't found the dataset.  */
static const netgroup_response_header notfound = {
    .version = NSCD_VERSION, .found = 0, .nresults = 0, .result_len = 0};
...
static bool do_notfound(struct database_dyn* db, int fd, request_header* req,
                        const char* key, struct dataset** datasetp,
                        ssize_t* totalp, time_t* timeoutp, char** key_copy)
{
...
static time_t addgetnetgrentX(struct database_dyn* db, int fd,
                              request_header* req, const char* key, uid_t uid,
                              struct hashentry* he, struct datahead* dh,
                              struct dataset** resultp, void** tofreep)
{
    ...
    *tofreep = buffer;
    ...
    while (data.needed_groups != NULL)
    {
        ...
        while (!no_more)
        {
            ...
            if (status == NSS_STATUS_SUCCESS)
            {
                ...
                if (getfct.f != NULL)
                    while (1)
                    {
                        int e;
                        status =
                            getfct.f(&data, buffer + buffilled,
                        ...
                        if (status == NSS_STATUS_SUCCESS)
                        {
                            if (data.type == triple_val)
                            {
                                ...
                                if (nhost == NULL || nuser == NULL ||
                                    ndomain == NULL || nhost > nuser ||
                                    nuser > ndomain)
                                {
                                    ...
                                    size_t needed =
                                        hostlen + userlen + domainlen;

                                    if (buflen - req->key_len - bufused <
                                        needed)
                                    {
                                        buflen += MAX(buflen, 2 * needed);
                                        /* Save offset in the old buffer.  We
                                           don't bother with the NULL check here
                                           since we'll do that later anyway.  */
                                        size_t nhostdiff = nhost - buffer;
                                        size_t nuserdiff = nuser - buffer;
                                        size_t ndomaindiff = ndomain - buffer;

                                        char* newbuf = xrealloc(buffer, buflen);
                                        /* Fix up the triplet pointers into the
                                           new buffer.  */
                                        nhost =
                                            (nhost ? newbuf + nhostdiff : NULL);
                                        nuser =
                                            (nuser ? newbuf + nuserdiff : NULL);
                                        ndomain =
                                            (ndomain ? newbuf + ndomaindiff
                                                     : NULL);
                                        buffer = newbuf;
                                    }
                            ...
                            else
                            {
                                ...
                                if (runp == NULL)
                                {
                                    ...
                                    struct name_list* newg =
                                        alloca(sizeof(*newg) + namelen);
                        ...
                        else if (status == NSS_STATUS_TRYAGAIN && e == ERANGE)
                        {
                            ...
                            /* This was either the last one for this group or
                           the group was empty or the NSS module had an internal
            ...
            no_more =
                __nss_next2(&nip, "setnetgrent", NULL, &setfct.ptr, status, 0);
        }
    }
    ...
    timeout =
        datahead_init_pos(&dataset->head, total + req->key_len,
                          total - offsetof(struct dataset, resp),
                          he == NULL ? 0 : dh->nreloads + 1, db->postimeout);
    ...
    if (cacheable)
    {
        ...
        (void)cache_add(req->type, key_copy, req->key_len, &dataset->head, true,
                        db, uid, he == NULL);
                          ...
                          struct datahead* dh)
{
    ...
    if (__glibc_unlikely(debug_level > 0))
    {
        if (he == NULL)
            ...
            dbg_log(_("Reloading \"%s (%s,%s,%s)\" in netgroup cache!"), group,
                    host ?: "", user ?: "", domain ?: "");
    }

    struct dataset* result =
        (struct dataset*)cache_search(GETNETGRENT, group, group_len, db, uid);
    time_t timeout;
    void* tofree;
    if (result != NULL)
    {
    ...
    {
        request_header req_get = {.type = GETNETGRENT, .key_len = group_len};
    ...
    }* dataset = (struct indataset*)mempool_alloc(
        db, sizeof(*dataset) + req->key_len, 1);
...
static time_t addgetnetgrentX_ignore(struct database_dyn* db, int fd,
                                     request_header* req, const char* key,
                                     uid_t uid, struct hashentry* he,
                                     struct datahead* dh)
{
    struct dataset* ignore;
    void* tofree;
    time_t timeout =
        addgetnetgrentX(db, fd, req, key, uid, he, dh, &ignore, &tofree);
    free(tofree);
    return timeout;
}
...
time_t readdgetnetgrent(struct database_dyn* db, struct hashentry* he,
                        struct datahead* dh)
{
    request_header req = {.type = GETNETGRENT, .key_len = he->len};
    return addgetnetgrentX_ignore(db, -1, &req, db->data + he->key, he->owner,
                                  he, dh);
}

void addinnetgr(struct database_dyn* db, int fd, request_header* req, void* key,
                uid_t uid)
{
...
time_t readdinnetgr(struct database_dyn* db, struct hashentry* he,
                    struct datahead* dh)
{
    request_header req = {.type = INNETGR, .key_len = he->len};
