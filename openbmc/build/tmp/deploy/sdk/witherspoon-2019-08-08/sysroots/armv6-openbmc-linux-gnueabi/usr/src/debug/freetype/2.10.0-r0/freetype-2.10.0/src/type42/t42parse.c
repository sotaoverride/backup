...
#define T1CODE T1_FIELD_LOCATION_FONT_INFO
    ...
    T1_FIELD_STRING("version", version, 0) T1_FIELD_STRING(
        "Notice", notice, 0) T1_FIELD_STRING("FullName", full_name, 0)
        T1_FIELD_STRING("FamilyName", family_name, 0) T1_FIELD_STRING(
            "Weight", weight, 0) T1_FIELD_NUM("ItalicAngle", italic_angle, 0)
            T1_FIELD_BOOL("isFixedPitch", is_fixed_pitch, 0) T1_FIELD_NUM(
                "UnderlinePosition", underline_position,
                0) T1_FIELD_NUM("UnderlineThickness", underline_thickness, 0)
...
#define T1CODE T1_FIELD_LOCATION_FONT_DICT
                    ...
                    T1_FIELD_KEY("FontName", font_name,
                                 0) T1_FIELD_NUM("PaintType", paint_type, 0)
                        T1_FIELD_NUM("FontType", font_type, 0) T1_FIELD_FIXED(
...
#define T1CODE T1_FIELD_LOCATION_BBOX
                            ...
                            T1_FIELD_BBOX("FontBBox", xMin, 0)
                                ...
                                T1_FIELD_CALLBACK(
                                    ...
                                    0) T1_FIELD_CALLBACK("Encoding",
                                                         t42_parse_encoding, 0)
                                    T1_FIELD_CALLBACK(
                                        "CharStrings", t42_parse_charstrings,
                                        0) T1_FIELD_CALLBACK("sfnts",
                                                             t42_parse_sfnts, 0)

                                        {0, T1_FIELD_LOCATION_CID_INFO,
                                         T1_FIELD_TYPE_NONE, 0, 0, 0, 0, 0, 0}};
                ...
                PSAux_Service psaux)
{
    ...
    if (FT_STREAM_SEEK(0L) || FT_FRAME_ENTER(17))
        goto Exit;
...
static void t42_parse_encoding(T42_Face face, T42_Loader loader)
{
    ...
    if (ft_isdigit(*cur) || *cur == '[')
    {
        ...
        if (FT_NEW_ARRAY(encode->char_index, count) ||
            ...
            FT_SET_ERROR(
                psaux->ps_table_funcs->init(char_table, count, memory)))
        {
        ...
        while (parser->root.cursor < limit)
        {
            ...
            {
                if (cur[1] == 'e' && cur[2] == 'f' && t42_is_space(cur[3]))
                {
            ...
            switch (status)
            {
                case BEFORE_START:
                    ...
                    else
                    {
                        ...
                        {
                            FT_ERROR(("t42_parse_sfnts: invalid data in sfnts "
                                      "array\n"));
                            error = FT_THROW(Invalid_File_Format);
                            goto Fail;
                        }
...
static void t42_parse_charstrings(T42_Face face, T42_Loader loader)
{
    ...
    for (;;)
    {
        ...
        if (*cur == 'e' && cur + 3 < limit && cur[1] == 'n' && cur[2] == 'd' &&
            t42_is_space(cur[3]))
            break;
        if (*cur == '>')
            break;
...
static FT_Error t42_load_keyword(T42_Face face, T42_Loader loader,
                                 T1_Field field)
{
    FT_Error error;
    void* dummy_object;
    ...
    if (field->type == T1_FIELD_TYPE_INTEGER_ARRAY ||
        ...
        error = T1_Load_Field(&loader->parser, field, objects, max_objects, 0);
...
t42_parse_dict(T42_Face face, T42_Loader loader, FT_Byte* base, FT_Long size)
{
    ...
    FT_Byte* limit;
    FT_Int n_keywords =
        (FT_Int)(sizeof(t42_keywords) / sizeof(t42_keywords[0]));
            ...
            if (len > 0 && len < 22 && parser->root.cursor < limit)
            {
                ...
                for (i = 0; i < n_keywords; i++)
                {
                        ...
                        ft_memcmp(cur, name, len) == 0)
                    {
                        ...
                        parser->root.error =
                            t42_load_keyword(face, loader, keyword);
                        if (parser->root.error)
