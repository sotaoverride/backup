...
static void dlmod_delete_module(struct dlmod* dlm)
{
    struct dlmod** pdlmod;
...
static void dlmod_load_module(struct dlmod* dlm)
{
...
static void dlmod_unload_module(struct dlmod* dlm)
{
...
static struct dlmod* dlmod_get_by_index(int iindex)
{
    struct dlmod* dlmod;
...
static void dlmod_parse_config(const char* token, char* cptr)
{
...
static int header_dlmod(struct variable* vp, oid* name, size_t* length,
...
static u_char* var_dlmod(struct variable* vp, oid* name, size_t* length,
                         int exact, size_t* var_len, WriteMethod** write_method)
{
    ...
    *var_len = sizeof(int); /* assume an integer and change later if not */
    ...
    switch (vp->magic)
    {
        ...
        default:
            DEBUGMSGTL(
                ("dlmod", "unknown sub-id %d in var_dlmod\n", vp->magic));
    }
    return NULL;
}

static int write_dlmodName(int action, u_char* var_val, u_char var_val_type,
...
static int write_dlmodPath(int action, u_char* var_val, u_char var_val_type,
                           size_t var_val_len, u_char* statP, oid* name,
                           size_t name_len)
{
    static struct dlmod* dlm;
...
static int write_dlmodStatus(int action, u_char* var_val, u_char var_val_type,
                             size_t var_val_len, u_char* statP, oid* name,
                             size_t name_len)
{
    /*
...
static struct dlmod* header_dlmodEntry(struct variable* vp, oid* name,
...
#define DLMODENTRY_NAME_LENGTH 12
    ...
    *write_method = NULL;
    ...
    {
        if (dlmod_index == dlmod_next_index && exact &&
            vp->magic == DLMODSTATUS)
...
static u_char* var_dlmodEntry(struct variable* vp, oid* name, size_t* length,
                              ...
                              WriteMethod** write_method)
{
    ...
    *var_len = sizeof(int); /* assume an integer and change later
    ...
    switch (vp->magic)
    {
        ...
        default:
            DEBUGMSGTL(
                ("dlmod", "unknown sub-id %d in var_dlmodEntry\n", vp->magic));
    }
    return NULL;
}
...
static struct variable4 dlmod_variables[] = {
    {DLMODNEXTINDEX, ASN_INTEGER, NETSNMP_OLDAPI_RONLY, var_dlmod, 1, {1}},
    {DLMODNAME,
     ASN_OCTET_STR,
...
void init_dlmod(void)
{
    ...
    snmpd_register_config_handler("dlmod", dlmod_parse_config,
                                  dlmod_free_config, "module-name module-path");
