...
struct menu
{
    ...
    int (*callback)(struct fdisk_context**, const struct menu*,
...
#define MENU_CXT_EMPTY                                                         \
    {                                                                          \
        0, 0                                                                   \
    }
#define DECLARE_MENU_CB(x)                                                     \
    static int x(struct fdisk_context**, const struct menu*,                   \
                 const struct menu_entry*)
...
#define MENU_SEP(t)                                                            \
    {                                                                          \
        .title = t, .key = '-', .normal = 1                                    \
    }
#define MENU_XSEP(t)                                                           \
    {                                                                          \
        .title = t, .key = '-', .expert = 1                                    \
    }
#define MENU_BSEP(t)                                                           \
    {                                                                          \
        .title = t, .key = '-', .expert = 1, .normal = 1                       \
    }

/* entry */
#define MENU_ENT(k, t)                                                         \
    {                                                                          \
        .title = t, .key = k, .normal = 1                                      \
    }
#define MENU_ENT_E(k, t, l)                                                    \
    {                                                                          \
        .title = t, .key = k, .normal = 1, .exclude = l                        \
    }
#define MENU_ENT_L(k, t, l)                                                    \
    {                                                                          \
        .title = t, .key = k, .normal = 1, .label = l                          \
    }

#define MENU_XENT(k, t)                                                        \
    {                                                                          \
        .title = t, .key = k, .expert = 1                                      \
    }
#define MENU_XENT_H(k, t)                                                      \
    {                                                                          \
        .title = t, .key = k, .expert = 1, .hidden = 1                         \
    }

#define MENU_BENT(k, t)                                                        \
    {                                                                          \
        .title = t, .key = k, .expert = 1, .normal = 1                         \
    }
#define MENU_BENT_E(k, t, l)                                                   \
    {                                                                          \
        .title = t, .key = k, .expert = 1, .normal = 1, .exclude = l           \
    }

#define MENU_ENT_NEST(k, t, l, p)                                              \
    {                                                                          \
        .title = t, .key = k, .normal = 1, .label = l, .parent = p             \
    }
#define MENU_XENT_NEST(k, t, l, p)                                             \
    {                                                                          \
        .title = t, .key = k, .expert = 1, .label = l, .parent = p             \
    }
#define MENU_BENT_NEST(k, t, l, p)                                             \
    {                                                                          \
        .title = t, .key = k, .expert = 1, .normal = 1, .label = l,            \
        .parent = p                                                            \
    }
...
static const struct menu menu_generic = {
    ...
    .entries = {
        ...
        MENU_XENT('d',
                  N_("print the raw data of the first sector from the device")),
        MENU_XENT('D',
                  N_("print the raw data of the disklabel from the device")),
        ...
        MENU_ENT_E('x', N_("extra functionality (experts only)"),
                   FDISK_DISKLABEL_BSD),
        ...
        MENU_ENT_E('w', N_("write table to disk and exit"),
                   FDISK_DISKLABEL_BSD),
        ...
        MENU_ENT_NEST('r', N_("return from BSD to DOS"), FDISK_DISKLABEL_BSD,
                      FDISK_DISKLABEL_DOS),
...
static const struct menu menu_geo = {
    ...
    .entries = {MENU_XSEP(N_("Geometry (for the current label)")),
...
static const struct menu menu_dos = {
    ...
    .entries = {MENU_BSEP(N_("DOS (MBR)")),
                ...
                MENU_BENT_NEST('M',
                               N_("return from protective/hybrid MBR to GPT"),
...
static const struct menu* menus[] = {
    &menu_gpt, &menu_sun, &menu_sgi,     &menu_dos,
    &menu_bsd, &menu_geo, &menu_generic, &menu_createlabel,
};

static const struct menu_entry* next_menu_entry(struct fdisk_context* cxt,
...
static const struct menu_entry* get_fdisk_menu_entry(struct fdisk_context* cxt,
                                                     int key,
...
static int menu_detect_collisions(struct fdisk_context* cxt)
{
    ...
    while ((e = next_menu_entry(cxt, &mc)))
    {
        ...
        if (r != e)
        {
            DBG(MENU, ul_debug("warning: duplicate key '%c'", e->key));
...
static int print_fdisk_menu(struct fdisk_context* cxt)
{
    ...
    if (fdisk_get_parent(cxt))
    {
        ...
        fdisk_info(cxt,
                   ...
                   fdisk_label_get_name(l), fdisk_label_get_name(p));
    }
...
int process_fdisk_menu(struct fdisk_context** cxt0)
{
    ...
    if (rc == -ECANCELED)
    {
        ...
        {
            rc = get_user_reply(_("\nDo you really want to quit? "), buf,
                                sizeof(buf));
            if (rc || !rpmatch(buf))
                return 0;
        }
    ...
    DBG(MENU, ul_debug("selected: key=%c, entry='%s'", key, ent->title));
...
static int ask_for_wipe(struct fdisk_context* cxt, size_t partno)
{
    ...
    fdisk_warnx(cxt, _("Partition #%zu contains a %s signature."), partno + 1,
                fstype);
                           ...
                           const struct menu_entry* ent)
{
    ...
    switch (ent->key)
    {
        ...
        case 'u':
            fdisk_set_unit(cxt,
                           fdisk_use_cylinders(cxt) ? "sectors" : "cylinders");
            if (fdisk_use_cylinders(cxt))
                fdisk_info(cxt, _("Changing display/entry units to cylinders "
                                  "(DEPRECATED!)."));
            else
        ...
        switch (ent->key)
        {
            ...
            case 'l':
                rc = fdisk_ask_number(cxt, 1, fdisk_get_npartitions(cxt),
                                      ~(uint32_t)0, _("New maximum entries"),
                                      &length);
                if (rc)
                    return rc;
        ...
        switch (ent->key)
        {
            case 'u':
                ...
                else
                {
                    char* str = NULL;
                    rc = fdisk_ask_string(
                        cxt, _("New UUID (in 8-4-4-4-12 format)"), &str);
                    if (!rc)
                       ...
                       const struct menu_entry* ent)
{
    ...
    switch (ent->key)
    {
        case 'c':
            ...
            rc = fdisk_ask_number(cxt, mi, fdisk_get_geom_cylinders(cxt), ma,
        ...
        case 'h':
        {
            ...
            rc = fdisk_ask_number(cxt, i, fdisk_get_geom_heads(cxt), a,
                                  _("Number of heads"), &h);
            break;
        }
        case 's':
            fdisk_label_get_geomrange_sectors(lb, &mi, &ma);
            rc = fdisk_ask_number(cxt, mi, fdisk_get_geom_sectors(cxt), ma,
