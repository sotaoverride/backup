...
static int specifier_machine_id_safe(char specifier, const void* data,
                                     const void* userdata, char** ret);
static int specifier_directory(char specifier, const void* data,
                               const void* userdata, char** ret);
...
static int log_unresolvable_specifier(const char* filename, unsigned line)
{
    ...
    log_full(notified ? LOG_DEBUG : LOG_NOTICE,
             "[%s:%u] Failed to resolve specifier: %s, skipping", filename,
             line,
             arg_user ? "Required $XDG_... variable not defined"
                      : "uninitialized /etc detected");
...
static int user_config_paths(char*** ret)
{
    ...
    _cleanup_free_ char *persistent_config = NULL, *runtime_config = NULL,
                        *data_home = NULL;
...
{
    return IN_SET(t, WRITE_FILE, IGNORE_PATH, IGNORE_DIRECTORY_PATH,
                  REMOVE_PATH, RECURSIVE_REMOVE_PATH, EMPTY_DIRECTORY,
                  ADJUST_MODE, RELABEL_PATH, RECURSIVE_RELABEL_PATH, SET_XATTR,
                  RECURSIVE_SET_XATTR, SET_ACL, RECURSIVE_SET_ACL,
                  SET_ATTRIBUTE, RECURSIVE_SET_ATTRIBUTE);
}

static bool takes_ownership(ItemType t)
{
    return IN_SET(t, CREATE_FILE, TRUNCATE_FILE, CREATE_DIRECTORY,
                  EMPTY_DIRECTORY, TRUNCATE_DIRECTORY, CREATE_SUBVOLUME,
                  CREATE_SUBVOLUME_INHERIT_QUOTA, CREATE_SUBVOLUME_NEW_QUOTA,
                  CREATE_FIFO, CREATE_SYMLINK, CREATE_CHAR_DEVICE,
                  CREATE_BLOCK_DEVICE, COPY_FILES, WRITE_FILE, IGNORE_PATH,
...
static int load_unix_sockets(void)
{
    ...
    /* We maintain a cache of the sockets we found in /proc/net/unix to speed
     * things up a little. */
    ...
    if (r < 0)
        return log_warning_errno(
            r, "Failed to skip /proc/net/unix header line: %m");
    if (r == 0)
        return log_warning_errno(
            SYNTHETIC_ERRNO(EIO),
    ...
    for (;;)
    {
        ...
        if (r < 0)
            return log_warning_errno(
                r, "Failed to read /proc/net/unix line, ignoring: %m");
        if (r == 0) /* EOF */
            break;
        ...
        if (r < 0)
            return log_warning_errno(
                r, "Failed to add AF_UNIX socket to set, ignoring: %m");
...
static int dir_cleanup(Item* i, const char* p, DIR* d, const struct stat* ds,
                       usec_t cutoff, dev_t rootdev, bool mountpoint,
                       int maxdepth, bool keep_this_level)
{
    ...
    FOREACH_DIRENT_ALL(dent, d, break)
    {
        ...
        if (s.st_dev != rootdev)
        {
            log_debug("Ignoring \"%s/%s\": different filesystem.", p,
                      dent->d_name);
            continue;
        }
        ...
        {
            log_debug(
                "Ignoring \"%s/%s\": different mount of the same filesystem.",
                p, dent->d_name);
            continue;
        }
        ...
        if (S_ISDIR(s.st_mode))
        {
            ...
            if (mountpoint && streq(dent->d_name, "lost+found") &&
                s.st_uid == 0)
            {
            ...
            else
            {
                ...
                if (!sub_dir)
                {
                    if (errno != ENOENT)
                        r = log_warning_errno(
                            errno,
                            "Opening directory \"%s\" failed, ignoring: %m",
                            sub_path);
                ...
                if (flock(dirfd(sub_dir), LOCK_EX | LOCK_NB) < 0)
                {
                    log_debug_errno(errno,
                                    "Couldn't acquire shared BSD lock on "
                                    "directory \"%s\", skipping: %m",
                                    p);
                    continue;
                }

                q = dir_cleanup(i, sub_path, sub_dir, &s, cutoff, rootdev,
                                false, maxdepth - 1, false);
                if (q < 0)
                    r = q;
            }

            /* Note: if you are wondering why we don't support the sticky bit
             * for excluding directories from cleaning like we do it for other
             * file system objects: well, the sticky bit already has a meaning
             * for directories, so we don't want to overload that. */
            ...
            if (age >= cutoff)
            {
                ...
                log_debug("Directory \"%s\": modify time %s is too new.",
                          sub_path, format_timestamp_us(a, sizeof(a), age));
                continue;
            }
            ...
            if (unlinkat(dirfd(d), dent->d_name, AT_REMOVEDIR) < 0)
                if (!IN_SET(errno, ENOENT, ENOTEMPTY))
                    r = log_warning_errno(
                        errno,
                        "Failed to remove directory \"%s\", ignoring: %m",
                        sub_path);
        }
        else
        {
            /* Skip files for which the sticky bit is set. These are semantics
             * we define, and are unknown elsewhere. See XDG_RUNTIME_DIR
            ...
            if (mountpoint && S_ISREG(s.st_mode))
                if (s.st_uid == 0 && STR_IN_SET(dent->d_name, ".journal",
            ...
            if (age >= cutoff)
            {
                char a[FORMAT_TIMESTAMP_MAX];
                /* Follows spelling in stat(1). */
                log_debug("File \"%s\": modify time %s is too new.", sub_path,
            ...
            {
                char a[FORMAT_TIMESTAMP_MAX];
                log_debug("File \"%s\": access time %s is too new.", sub_path,
                          format_timestamp_us(a, sizeof(a), age));
                continue;
            }
            ...
            {
                char a[FORMAT_TIMESTAMP_MAX];
                log_debug("File \"%s\": change time %s is too new.", sub_path,
                          format_timestamp_us(a, sizeof(a), age));
                continue;
            }
            ...
            if (unlinkat(dirfd(d), dent->d_name, 0) < 0)
                if (errno != ENOENT)
                    r = log_warning_errno(
                        errno, "Failed to remove \"%s\", ignoring: %m",
                        sub_path);
...
finish:
    if (deleted)
    {
        ...
        log_debug("Restoring access and modification time on \"%s\": %s, %s", p,
                  format_timestamp_us(a, sizeof(a), age1),
                  format_timestamp_us(b, sizeof(b), age2));

        /* Restore original directory timestamps */
        if (futimens(dirfd(d), (struct timespec[]){ds->st_atim, ds->st_mtim}) <
            0)
            log_warning_errno(
                errno, "Failed to revert timestamps of '%s', ignoring: %m", p);
    }
...
static bool dangerous_hardlinks(void)
{
    ...
    /* Check whether the fs.protected_hardlinks sysctl is on. If we can't
     * determine it we assume its off, as that's what the upstream default is.
     */
...
static int fd_set_perms(Item* i, int fd, const char* path,
                        const struct stat* st)
{
    ...
    if (hardlink_vulnerable(st))
        return log_error_errno(
            ...
            "Refusing to set permissions on hardlinked file %s while the "
            "fs.protected_hardlinks sysctl is turned off.",
            path);
        ...
        else
        {
            ...
            if (i->mask_perms)
            {
                ...
                if (!S_ISDIR(st->st_mode))
                    m &= ~07000; /* remove sticky/sgid/suid bit, unless
                                    directory */
            }

            if (m == (st->st_mode & 07777))
                log_debug("\"%s\" has correct mode %o already.", path,
                          st->st_mode);
            else
            {
                log_debug("Changing \"%s\" to mode %o.", path, m);
                if (fchmod_opath(fd, m) < 0)
                    return log_error_errno(errno, "fchmod() of %s failed: %m",
                                           path);
            }
        }
    }
    ...
    {
        log_debug("Changing \"%s\" to owner " UID_FMT ":" GID_FMT, path,
                  i->uid_set ? i->uid : UID_INVALID,
                  i->gid_set ? i->gid : GID_INVALID);

        if (fchownat(fd, "", i->uid_set ? i->uid : UID_INVALID,
                     i->gid_set ? i->gid : GID_INVALID, AT_EMPTY_PATH) < 0)
...
static int path_open_parent_safe(const char* path)
{
    ...
    fd = chase_symlinks(dn, arg_root, CHASE_OPEN | CHASE_SAFE | CHASE_WARN,
                        NULL);
    if (fd < 0 && fd != -ENOLINK)
...
static int path_open_safe(const char* path)
{
    ...
    fd = chase_symlinks(path, arg_root,
                        CHASE_OPEN | CHASE_SAFE | CHASE_WARN | CHASE_NOFOLLOW,
                        NULL);
    if (fd < 0 && fd != -ENOLINK)
        return log_error_errno(fd, "Failed to validate path %s: %m", path);
...
static int parse_xattrs_from_arg(Item* i)
{
    ...
    for (;;)
    {
        ...
        if (r < 0)
            log_warning_errno(
                r, "Failed to parse extended attribute '%s', ignoring: %m", p);
        if (r <= 0)
            break;
        ...
        {
            log_warning_errno(
                r, "Failed to parse extended attribute, ignoring: %s", xattr);
            continue;
        }

        if (isempty(name) || isempty(value))
        {
            log_warning("Malformed extended attribute found, ignoring: %s",
                        xattr);
            continue;
        }
...
static int fd_set_xattrs(Item* i, int fd, const char* path,
                         const struct stat* st)
{
    ...
    {
        log_debug("Setting extended attribute '%s=%s' on %s.", *name, *value,
                  path);
        if (setxattr(procfs_path, *name, *value, strlen(*value), 0) < 0)
            return log_error_errno(
                errno, "Setting extended attribute %s=%s on %s failed: %m",
                *name, *value, path);
    }
    return 0;
}
...
static int parse_acls_from_arg(Item* item)
{
    ...
    if (r < 0)
        log_warning_errno(r, "Failed to parse ACL \"%s\": %m. Ignoring",
                          item->argument);
#else
    log_warning_errno(SYNTHETIC_ERRNO(ENOSYS),
...
#if HAVE_ACL
static int path_set_acl(const char* path, const char* pretty, acl_type_t type,
                        acl_t acl, bool modify)
{
    ...
    log_debug("Setting %s ACL %s on %s.",
              type == ACL_TYPE_ACCESS ? "access" : "default", strna(t), pretty);

    r = acl_set_file(path, type, dup);
    if (r < 0)
        /* Return positive to indicate we already warned */
        return -log_error_errno(errno, "Setting %s ACL \"%s\" on %s failed: %m",
...
static int fd_set_acls(Item* item, int fd, const char* path,
                       const struct stat* st)
{
    int r = 0;
#if HAVE_ACL
    ...
    if (hardlink_vulnerable(st))
        return log_error_errno(
            SYNTHETIC_ERRNO(EPERM),
            "Refusing to set ACLs on hardlinked file %s while the "
            "fs.protected_hardlinks sysctl is turned off.",
            path);
    ...
    if (item->acl_access)
        r = path_set_acl(procfs_path, path, ACL_TYPE_ACCESS, item->acl_access,
                         item->force);

    /* set only default acls to folders */
    if (r == 0 && item->acl_default && S_ISDIR(st->st_mode))
        r = path_set_acl(procfs_path, path, ACL_TYPE_DEFAULT, item->acl_default,
                         item->force);
...
static int parse_attribute_from_arg(Item* item)
{
    ...
    for (; p && *p; p++)
    {
        ...
        if (i >= ELEMENTSOF(attributes))
            return log_error_errno(SYNTHETIC_ERRNO(EINVAL),
                                   "Unknown file attribute '%c' on '%s'.", *p,
                                   item->path);
...
static int fd_set_attribute(Item* item, int fd, const char* path,
                            const struct stat* st)
{
    ...
    if (!S_ISREG(st->st_mode) && !S_ISDIR(st->st_mode))
        return log_error_errno(
            SYNTHETIC_ERRNO(EINVAL),
            "Setting file flags is only supported on regular files and "
            "directories, cannot set on '%s'.",
            path);
    ...
    if (r < 0)
        log_full_errno(
            IN_SET(r, -ENOTTY, -EOPNOTSUPP) ? LOG_DEBUG : LOG_WARNING, r,
...
static int write_one_file(Item* i, const char* path)
{
    ...
    /* Follows symlinks */
    fd = openat(dir_fd, bn, O_NONBLOCK | O_CLOEXEC | O_WRONLY | O_NOCTTY,
                i->mode);
    if (fd < 0)
    {
        if (errno == ENOENT)
...
static int create_file(Item* i, const char* path)
{
    ...
    if (fd < 0)
    {
        ...
        if (fd < 0)
            return log_error_errno(errno, "Failed to re-open file %s: %m",
                                   path);
    ...
    else
    {
        ...
        if (i->argument)
        {
            ...
            r = loop_write(fd, i->argument, strlen(i->argument), false);
            if (r < 0)
                return log_error_errno(r, "Failed to write file \"%s\": %m",
                                       path);
        }
    }
...
static int truncate_file(Item* i, const char* path)
{
    ...
    RUN_WITH_UMASK(0000)
    {
        mac_selinux_create_file_prepare(path, S_IFREG);
        fd = openat(dir_fd, bn,
                    O_CREAT | O_NOFOLLOW | O_NONBLOCK | O_CLOEXEC | O_WRONLY |
                        O_NOCTTY,
                    i->mode);
        mac_selinux_create_file_clear();
    }
    ...
    {
        if (errno != EROFS)
            return log_error_errno(errno, "Failed to open/create file %s: %m",
                                   path);
        ...
        if (fd < 0)
        {
            if (errno == ENOENT)
            {
                log_error("Cannot create file %s on a read-only file system.",
                          path);
                return -EROFS;
            }
...
static int copy_files(Item* i)
{
    ...
    r = copy_tree_at(
        AT_FDCWD, i->argument, dfd, bn, i->uid_set ? i->uid : UID_INVALID,
        i->gid_set ? i->gid : GID_INVALID, COPY_REFLINK | COPY_MERGE_EMPTY);
    if (r < 0)
    {
        struct stat a, b;
        ...
        if (r != -EEXIST)
            return log_error_errno(r, "Failed to copy files to %s: %m",
                                   i->path);
        ...
        {
            log_debug("Can't copy to %s, file exists already and is of "
                      "different type",
                      i->path);
            return 0;
        }
...
static int create_directory_or_subvolume(const char* path, mode_t mode,
                                         bool subvol, CreationMode* creation)
{
    ...
    if (r < 0)
    {
        ...
        if (!IN_SET(r, -EEXIST, -EROFS))
            return log_error_errno(
                r, "Failed to create directory or subvolume \"%s\": %m", path);

        k = is_dir_fd(pfd);
        if (k == -ENOENT && r == -EROFS)
            return log_error_errno(r,
                                   "%s does not exist and cannot be created as "
    ...
    log_debug("%s directory \"%s\".", creation_mode_verb_to_string(*creation),
              path);

    r = openat(pfd, basename(path), O_NOCTTY | O_CLOEXEC | O_DIRECTORY);
    if (r < 0)
        return log_error_errno(errno, "Failed to open directory '%s': %m",
                               basename(path));
...
static int create_subvolume(Item* i, const char* path)
{
    ...
    assert(i);
    assert(IN_SET(i->type, CREATE_SUBVOLUME, CREATE_SUBVOLUME_NEW_QUOTA,
    ...
    if (creation == CREATION_NORMAL &&
        IN_SET(i->type, CREATE_SUBVOLUME_NEW_QUOTA,
               CREATE_SUBVOLUME_INHERIT_QUOTA))
    {
        r = btrfs_subvol_auto_qgroup_fd(fd, 0,
                                        i->type == CREATE_SUBVOLUME_NEW_QUOTA);
        if (r == -ENOTTY)
            log_debug_errno(r,
                            "Couldn't adjust quota for subvolume \"%s\" "
                            "(unsupported fs or dir not a subvolume): %m",
                            i->path);
        else if (r == -EROFS)
            log_debug_errno(
                r,
                "Couldn't adjust quota for subvolume \"%s\" (fs is read-only).",
                i->path);
        else if (r == -ENOPROTOOPT)
            log_debug_errno(r,
                            "Couldn't adjust quota for subvolume \"%s\" (quota "
                            "support is disabled).",
                            i->path);
        else if (r < 0)
            q = log_error_errno(
                r, "Failed to adjust quota for subvolume \"%s\": %m", i->path);
        else if (r > 0)
            log_debug("Adjusted quota for subvolume \"%s\".", i->path);
        else if (r == 0)
            log_debug(
                "Quota for subvolume \"%s\" already in place, no change made.",
                i->path);
    }
...
static int create_device(Item* i, mode_t file_type)
{
    ...
    if (r < 0)
    {
        ...
        if (errno == EPERM)
        {
            log_debug("We lack permissions, possibly because of cgroup "
                      "configuration; "
                      "skipping creation of device node %s.",
                      i->path);
            return 0;
        }

        if (errno != EEXIST)
            return log_error_errno(errno, "Failed to create device node %s: %m",
                                   i->path);
        ...
        if ((st.st_mode & S_IFMT) != file_type)
        {
            ...
            if (i->force)
            {
                ...
                RUN_WITH_UMASK(0000)
                {
                    ...
                    r = mknod_atomic(i->path, i->mode | file_type,
                                     i->major_minor);
                    mac_selinux_create_file_clear();
                }
    ...
    log_debug("%s %s device node \"%s\" %u:%u.",
              ...
              i->type == CREATE_BLOCK_DEVICE ? "block" : "char", i->path,
              major(i->mode), minor(i->mode));
    ...
    if (r < 0)
    {
        ...
        if (!S_ISFIFO(st.st_mode))
        {
            ...
            if (i->force)
            {
                ...
                if (r < 0)
                    return log_error_errno(r, "Failed to create fifo %s: %m",
                                           path);
                creation = CREATION_FORCE;
            }
            else
...
typedef int (*fdaction_t)(Item* i, int fd, const char* path,
                          const struct stat* st);
...
static int item_do(Item* i, int fd, const char* path, fdaction_t action)
{
    ...
    if (S_ISDIR(st.st_mode))
    {
        ...
        FOREACH_DIRENT_ALL(de, d, q = -errno; goto finish)
        {
            ...
            if (de_fd < 0)
                q = log_error_errno(errno, "Failed to open() file '%s': %m",
                                    de->d_name);
            else
            {
    ...
    switch (i->type)
    {
        ...
        case CREATE_SYMLINK:
        {
            ...
            if (r < 0)
            {
                ...
                if (errno != EEXIST)
                    return log_error_errno(errno, "symlink(%s, %s) failed: %m",
                                           i->argument, i->path);
                ...
                if (r < 0 || !streq(i->argument, x))
                {
                    ...
                    {
                        log_debug("\"%s\" is not a symlink or does not point "
                                  "to the correct path.",
                                  i->path);
                        return 0;
                    }
        ...
        case CREATE_CHAR_DEVICE:
            ...
            {
                /* In a container we lack CAP_MKNOD. We shouldn't attempt to
                 * create the device node in that case to avoid noise, and we
                 * don't support virtualized devices in containers anyway. */

                log_debug(
                    "We lack CAP_MKNOD, skipping creation of device node %s.",
                    i->path);
                return 0;
            }
            ...
            r = create_device(i, i->type == CREATE_BLOCK_DEVICE ? S_IFBLK
                                                                : S_IFCHR);
            if (r < 0)
                return r;
...
static int remove_item_instance(Item* i, const char* instance)
{
    ...
    switch (i->type)
    {
        ...
        case RECURSIVE_REMOVE_PATH:
            /* FIXME: we probably should use dir_cleanup() here instead of
             * rm_rf() so that 'x' is honoured. */
            log_debug("rm -rf \"%s\"", instance);
            r = rm_rf(instance,
                      REMOVE_ROOT | REMOVE_SUBVOLUME | REMOVE_PHYSICAL);
            if (r < 0 && r != -ENOENT)
...
static int clean_item_instance(Item* i, const char* instance)
{
    ...
    log_debug("Cleanup threshold for %s \"%s\" is %s",
              mountpoint ? "mount point" : "directory", instance,
...
static int process_item(Item* i, OperationMask operation)
{
    ...
    if (r < 0)
        log_debug_errno(
            r, "Failed to determine whether '%s' is below autofs, ignoring: %m",
            i->path);
...
static int process_item_array(ItemArray* array, OperationMask operation)
{
    ...
    if ((operation & (OPERATION_REMOVE | OPERATION_CLEAN)) &&
        !set_isempty(array->children))
    {
        Iterator i;
        ItemArray* c;
        ...
        SET_FOREACH(c, array->children, i)
        {
            ...
            k = process_item_array(c, operation &
                                          (OPERATION_REMOVE | OPERATION_CLEAN));
            if (k < 0 && r == 0)
                r = k;
        }
    }
...
static bool item_compatible(Item* a, Item* b)
{
    ...
    if (takes_ownership(a->type) && takes_ownership(b->type))
        ...
        return streq_ptr(a->argument, b->argument) &&
               ...
               a->uid_set == b->uid_set && a->uid == b->uid &&

               a->gid_set == b->gid_set && a->gid == b->gid &&

               a->mode_set == b->mode_set && a->mode == b->mode &&

               a->age_set == b->age_set && a->age == b->age &&
...
static bool should_include_path(const char* path)
{
    ...
    {
        log_debug("Entry \"%s\" matches exclude prefix \"%s\", skipping.", path,
                  *prefix);
        return false;
    }
    ...
    log_debug("Entry \"%s\" does not match any include prefix, skipping.",
              path);
    return false;
}
...
static int specifier_expansion_from_arg(Item* i)
{
    ...
    switch (i->type)
    {
        ...
        case WRITE_FILE:
            ...
            if (r < 0)
                return log_error_errno(
                    r, "Failed to unescape parameter to write: %s",
                    i->argument);
...
static int patch_var_run(const char* fname, unsigned line, char** path)
{
     ...
     * Why bother? tmpfiles merges lines in some cases and detects conflicts in
     * others. If files/directories are specified through two equivalent lines
     * this is problematic as neither case will be detected. Ideally we'd detect
     * these cases by resolving symlinks early, but that's precisely not what we
     * can do here as this code very likely is running very early on, at a time
     * where the paths in question are not available yet, or even more
     * importantly, our own tmpfiles rules might create the paths that are
     * intermediary to the listed paths. We can't really cover the generic case,
     * but the least we can do is cover the specific case of /var/run vs. /run,
     * as /var/run is a legacy name for /run only, and we explicitly document
     * that and require that on systemd systems the former is a symlink to the
     * latter. Moreover files below this path are by far the primary usecase for
     * tmpfiles.d/. */

    k = path_startswith(*path, "/var/run/");
    if (isempty(k)) /* Don't complain about other paths than /var/run, and not
                       about /var/run itself either. */
        return 0;
    ...
    /* Also log about this briefly. We do so at LOG_NOTICE level, as we fixed up
     * the situation automatically, hence there's no immediate need for action
     * by the user. However, in the interest of making things less confusing
     * to the user, let's still inform the user that these snippets should
     * really be updated. */

    log_notice("[%s:%u] Line references path below legacy directory /var/run/, "
               "updating %s → %s; please update the tmpfiles.d/ drop-in file "
               "accordingly.",
               fname, line, *path, n);
                      ...
                      bool* invalid_config)
{
    ...
    r = extract_many_words(&buffer, NULL, EXTRACT_QUOTES, &action, &path, &mode,
                           &user, &group, &age, NULL);
    if (r < 0)
    {
        if (IN_SET(r, -EINVAL, -EBADSLT))
            ...
            *invalid_config = true;
        return log_error_errno(r, "[%s:%u] Failed to parse line: %m", fname,
                               line);
    }
    else if (r < 2)
    {
    ...
    for (pos = 1; action[pos]; pos++)
    {
        ...
        {
            *invalid_config = true;
            log_error("[%s:%u] Unknown modifiers in command '%s'", fname, line,
                      action);
            return -EINVAL;
        }
    }
    ...
    switch (i.type)
    {
        ...
        case RECURSIVE_RELABEL_PATH:
            if (i.argument)
                log_warning(
                    "[%s:%u] %c lines don't take argument fields, ignoring.",
                    fname, line, i.type);
        ...
        case CREATE_BLOCK_DEVICE:
            ...
            {
                *invalid_config = true;
                log_error("[%s:%u] Device file requires argument.", fname,
                          line);
                return -EBADMSG;
            }
            ...
            if (r < 0)
            {
                *invalid_config = true;
                log_error_errno(
                    r, "[%s:%u] Can't parse device file major/minor '%s'.",
                    fname, line, i.argument);
                return -EBADMSG;
            }
        ...
        case RECURSIVE_SET_XATTR:
            ...
            {
                *invalid_config = true;
                log_error("[%s:%u] Set extended attribute requires argument.",
                          fname, line);
                return -EBADMSG;
            }
        ...
        case RECURSIVE_SET_ATTRIBUTE:
            ...
            {
                *invalid_config = true;
                log_error("[%s:%u] Set file attribute requires argument.",
                          fname, line);
                return -EBADMSG;
            }
    ...
    if (r < 0)
    {
        if (IN_SET(r, -EINVAL, -EBADSLT))
            *invalid_config = true;
        return log_error_errno(
            r, "[%s:%u] Failed to substitute specifiers in argument: %m", fname,
            line);
    }
    ...
    if (!empty_or_dash(user))
    {
        ...
        {
            *invalid_config = true;
            return log_error_errno(r, "[%s:%u] Unknown user '%s'.", fname, line,
                                   user);
        }
    ...
    if (!empty_or_dash(mode))
    {
        ...
        i.mode = IN_SET(i.type, CREATE_DIRECTORY, TRUNCATE_DIRECTORY,
                        CREATE_SUBVOLUME, CREATE_SUBVOLUME_INHERIT_QUOTA,
    ...
    if (!GREEDY_REALLOC(existing->items, existing->allocated,
                        existing->n_items + 1))
        return log_oom();
...
static int cat_config(char** config_dirs, char** args)
{
    ...
    r = conf_files_list_with_replacement(arg_root, config_dirs, arg_replace,
                                         &files, NULL);
    if (r < 0)
        return r;
...
static int help(void)
{
    ...
    printf(
        ...
        "     --prefix=PATH          Only apply rules with the specified "
        "prefix\n"
...
static int parse_argv(int argc, char* argv[])
{
        ...
        switch (c)
        {
            ...
            case ARG_REPLACE:
                if (!path_is_absolute(optarg) || !endswith(optarg, ".conf"))
                    return log_error_errno(SYNTHETIC_ERRNO(EINVAL),
                                           "The argument to --replace= must an "
    ...
    if (arg_operation == 0 && !arg_cat_config)
        return log_error_errno(SYNTHETIC_ERRNO(EINVAL),
                               "You need to specify at least one of --clean, "
                               "--create or --remove.");
    ...
    if (arg_replace && optind >= argc)
        return log_error_errno(SYNTHETIC_ERRNO(EINVAL),
                               "When --replace= is given, some configuration "
                               "items must be specified");

    return 1;
}

static int read_config_file(char** config_dirs, const char* fn,
                            bool ignore_enoent, bool* invalid_config)
{
    ...
    {
        r = search_and_fopen(fn, "re", arg_root, (const char**)config_dirs,
                             &_f);
        if (r < 0)
        {
    ...
    for (;;)
    {
        ...
        {
            if (invalid_line)
                /* Allow reporting with a special code if the caller requested
                 * this */
                *invalid_config = true;
            else if (r == 0)
    ...
    ORDERED_HASHMAP_FOREACH(i, globs, iterator)
    {
        ...
        {
            if (!IN_SET(j->type, CREATE_DIRECTORY, TRUNCATE_DIRECTORY,
                        CREATE_SUBVOLUME, CREATE_SUBVOLUME_INHERIT_QUOTA,
                        CREATE_SUBVOLUME_NEW_QUOTA))
                continue;
            ...
            if ((!candidate_item && path_startswith(i->path, j->path)) ||
                (candidate_item &&
                 path_startswith(j->path, candidate_item->path) &&
...
static int parse_arguments(char** config_dirs, char** args,
                           bool* invalid_config)
{
    char** arg;
    int r;
...
static int read_config_files(char** config_dirs, char** args,
                             bool* invalid_config)
{
    _cleanup_strv_free_ char** files = NULL;
    ...
    if (p && path_equal(*f, p))
    {
         ...
         * read_config_file() has some debug output, so no need to print
         * anything. */
...
static int link_parent(ItemArray* a)
{
    ...
    /* Finds the closest "parent" item array for the specified item array. Then
     * registers the specified item array as child of it, and fills the parent
     * in, linking them both ways. This allows us to later create parents
...
DEFINE_PRIVATE_HASH_OPS_WITH_VALUE_DESTRUCTOR(item_array_hash_ops, char,
                                              string_hash_func,
                                              string_compare_func, ItemArray,
                                              item_array_free);
...
static int run(int argc, char* argv[])
{
    ...
    if (arg_user)
    {
        ...
        if (r < 0)
            return log_error_errno(
                r, "Failed to initialize configuration directory list: %m");
    }
    else
    {
    ...
    if (DEBUG_LOGGING)
    {
        ...
        if (t)
            log_debug("Looking for configuration files in (higher priority "
                      "first):\n\t%s",
                      t);
    }
    ...
    /* If multiple operations are requested, let's first run the remove/clean
     * operations, and only then the create
    ...
    for (phase = 0; phase < _PHASE_MAX; phase++)
    {
        ...
        /* The non-globbing ones usually create things, hence we apply them
         * first */
        ORDERED_HASHMAP_FOREACH(a, items, iterator)
        {
