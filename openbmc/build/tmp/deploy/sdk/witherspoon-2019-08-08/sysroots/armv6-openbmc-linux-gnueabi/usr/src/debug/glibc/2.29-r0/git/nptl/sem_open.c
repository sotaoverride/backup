...
int attribute_hidden __sem_search(const void* a, const void* b)
{
...
static sem_t* check_add_mapping(const char* name, size_t namelen, int fd,
                                sem_t* existing)
{
    ...
    if (__fxstat64(_STAT_VER, fd, &st) == 0)
    {
        ...
        struct inuse_sem** foundp =
            __tfind(fake, &__sem_mappings, __sem_search);
        if (foundp != NULL)
        {
...
sem_t* sem_open(const char* name, int oflag, ...)
{
    ...
    {
    try_again:
        fd = __libc_open(shm_name, (oflag & ~(O_CREAT | O_ACCMODE)) |
                                       O_NOFOLLOW | O_RDWR);
