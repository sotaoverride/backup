...
static void g_unix_volume_finalize(GObject* object)
{
    GUnixVolume* volume;
...
static void g_unix_volume_class_init(GUnixVolumeClass* klass)
{
...
static void g_unix_volume_init(GUnixVolume* unix_volume)
{
}
                                ...
                                GUnixMountPoint* mountpoint)
{
    ...
    if (!(g_unix_mount_point_is_user_mountable(mountpoint) ||
          g_str_has_prefix(g_unix_mount_point_get_device_path(mountpoint),
                           "/vol/")) ||
    ...
    volume->volume_monitor =
        volume_monitor != NULL ? g_object_ref(volume_monitor) : NULL;
    volume->mount_path =
...
void _g_unix_volume_unset_mount(GUnixVolume* volume, GUnixMount* mount)
{
    if (volume->mount == mount)
    {
        ...
        g_signal_emit_by_name(volume, "changed");
        if (volume->volume_monitor != NULL)
            g_signal_emit_by_name(volume->volume_monitor, "volume-changed",
                                  volume);
    }
}

static GIcon* g_unix_volume_get_icon(GVolume* volume)
{
    GUnixVolume* unix_volume = G_UNIX_VOLUME(volume);
    return g_object_ref(unix_volume->icon);
}

static GIcon* g_unix_volume_get_symbolic_icon(GVolume* volume)
{
    GUnixVolume* unix_volume = G_UNIX_VOLUME(volume);
    return g_object_ref(unix_volume->symbolic_icon);
}

static char* g_unix_volume_get_name(GVolume* volume)
{
    GUnixVolume* unix_volume = G_UNIX_VOLUME(volume);
    return g_strdup(unix_volume->name);
}

static char* g_unix_volume_get_uuid(GVolume* volume)
{
    return NULL;
}

static gboolean g_unix_volume_can_mount(GVolume* volume)
{
    return TRUE;
}

static gboolean g_unix_volume_can_eject(GVolume* volume)
{
    GUnixVolume* unix_volume = G_UNIX_VOLUME(volume);
    return unix_volume->can_eject;
}

static gboolean g_unix_volume_should_automount(GVolume* volume)
{
...
static GDrive* g_unix_volume_get_drive(GVolume* volume)
{
    return NULL;
}

static GMount* g_unix_volume_get_mount(GVolume* volume)
{
    GUnixVolume* unix_volume = G_UNIX_VOLUME(volume);
...
static void eject_mount_done(GObject* source, GAsyncResult* result,
                             gpointer user_data)
{
    ...
    if (!g_subprocess_communicate_utf8_finish(subprocess, result, NULL,
                                              &stderr_str, &error))
    {
    ...
    else /* successful communication */
    {
        ...
        else
        {
            ...
            _g_unix_volume_monitor_update(
                G_UNIX_VOLUME_MONITOR(unix_volume->volume_monitor));
...
static void eject_mount_do(GVolume* volume, GCancellable* cancellable,
                           ...
                           const gchar* const* argv)
{
    ...
    g_subprocess_communicate_utf8_async(
        subprocess, NULL, g_task_get_cancellable(task), eject_mount_done, task);
}

static void g_unix_volume_mount(GVolume* volume, GMountMountFlags flags,
...
static gboolean g_unix_volume_mount_finish(GVolume* volume,
...
static void g_unix_volume_eject(GVolume* volume, GMountUnmountFlags flags,
                                GCancellable* cancellable,
                                GAsyncReadyCallback callback,
                                gpointer user_data)
{
...
static gboolean g_unix_volume_eject_finish(GVolume* volume,
                                           GAsyncResult* result, GError** error)
{
    g_return_val_if_fail(g_task_is_valid(result, volume), FALSE);
...
static gchar* g_unix_volume_get_identifier(GVolume* volume, const gchar* kind)
{
    GUnixVolume* unix_volume = G_UNIX_VOLUME(volume);
...
static gchar** g_unix_volume_enumerate_identifiers(GVolume* volume)
{
    GUnixVolume* unix_volume = G_UNIX_VOLUME(volume);
    gchar** res;
