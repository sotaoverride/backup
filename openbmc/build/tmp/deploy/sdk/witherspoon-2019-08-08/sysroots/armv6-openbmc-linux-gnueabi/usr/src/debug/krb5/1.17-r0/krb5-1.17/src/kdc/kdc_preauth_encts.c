...
/*
 * Copyright (C) 1995, 2003, 2007, 2011 by the Massachusetts Institute of
...
static void enc_ts_verify(krb5_context context, krb5_data* req_pkt,
                          ...
                          krb5_kdcpreauth_verify_respond_fn respond, void* arg)
{
    ...
    while (1)
    {
        if ((retval = krb5_dbe_search_enctype(context, rock->client, &start,
                                              enc_data->enctype, -1, 0,
                                              &client_key)))
            goto cleanup;

        if ((retval = krb5_dbe_decrypt_key_data(context, NULL, client_key, &key,
                                                NULL)))
            goto cleanup;
...
static krb5_preauthtype enc_ts_types[] = {KRB5_PADATA_ENC_TIMESTAMP, 0};

krb5_error_code kdcpreauth_encrypted_timestamp_initvt(krb5_context context,
