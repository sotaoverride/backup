...
void init_hw_fsys(void)
{
    ...
    {
        DEBUGMSGTL(("fsys",
                    "Reloading Hardware FileSystems automatically (%d)\n",
                    _fsysAutoUpdate));
        snmp_alarm_register(_fsysAutoUpdate, SA_REPEAT, _fsys_update_stats,
                            NULL);
    }
    else
    {
 ...
 */
netsnmp_container* netsnmp_fsys_get_container(void)
{
    return _fsys_container;
}
 ...
 */
netsnmp_fsys_info* netsnmp_fsys_by_path(char* path, int create_type)
{
    netsnmp_fsys_info* sp;
...
netsnmp_fsys_info* _fsys_create_entry(void)
{
    netsnmp_fsys_info* sp;
    ...
    DEBUGMSGTL(
        ("fsys:new", "Create filesystem entry (index = %d)\n", _fsys_idx));
 ...
 */
unsigned long long _fsys_to_K(unsigned long long size, unsigned long long units)
{
    int factor = 1;
...
unsigned long long netsnmp_fsys_size_ull(netsnmp_fsys_info* f)
{
    if (!f)
    {
        return 0;
...
unsigned long long netsnmp_fsys_used_ull(netsnmp_fsys_info* f)
{
    if (!f)
    {
        return 0;
...
unsigned long long netsnmp_fsys_avail_ull(netsnmp_fsys_info* f)
{
    if (!f)
    {
        return 0;
...
void netsnmp_fsys_calculate32(netsnmp_fsys_info* f)
{
    ...
    DEBUGMSGTL(("fsys",
                "Results of 32-bit conversion: size %" PRIu64 " -> %lu;"
                " units %" PRIu64 " -> %lu; avail %" PRIu64 " -> %lu;"
                " used %" PRIu64 " -> %lu\n",
                (uint64_t)f->size, f->size_32, (uint64_t)f->units, f->units_32,
                (uint64_t)f->avail, f->avail_32, (uint64_t)f->used,
                f->used_32));
}
