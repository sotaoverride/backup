...
#define DNS_SERVER_FEATURE_LEVEL_IS_TLS(x)                                     \
    IN_SET(x, DNS_SERVER_FEATURE_LEVEL_TLS_PLAIN,                              \
           DNS_SERVER_FEATURE_LEVEL_TLS_DO)
...
struct DnsServer
{
    ...
    /* Whether we already warned about downgrading to non-DNSSEC mode for this
     * server */
...
int dns_server_new(Manager* m, DnsServer** ret, DnsServerType type, Link* link,
                   int family, const union in_addr_union* address, int ifindex);
