...
/* static data */
DEFobjStaticHelpers DEFobjCurrIf(glbl) DEFobjCurrIf(prop)
...
struct AllowedSenders* pAllowedSenders_UDP =
    NULL; /* the roots of the allowed sender */
struct AllowedSenders* pAllowedSenders_TCP =
    NULL; /* lists. If NULL, all senders are ok! */
static struct AllowedSenders* pLastAllowedSenders_UDP =
    NULL; /* and now the pointers to the last */
static struct AllowedSenders* pLastAllowedSenders_TCP =
    NULL; /* element in the respective list */
#ifdef USE_GSSAPI
struct AllowedSenders* pAllowedSenders_GSS = NULL;
static struct AllowedSenders* pLastAllowedSenders_GSS = NULL;
#endif

int ACLAddHostnameOnFail =
    0; /* add hostname to acl when DNS resolving has failed */
int ACLDontResolve =
    0; /* add hostname to acl instead of resolving it to IP(s) */
 ...
 */
static rsRetVal setAllowRoot(struct AllowedSenders** ppAllowRoot,
                             uchar* pszType)
{
    DEFiRet;
    ...
    else
    {
        dbgprintf("program error: invalid allowed sender ID '%s', denying...\n",
                  pszType);
        ABORT_FINALIZE(
            RS_RET_CODE_ERR); /* everything is invalid for an invalid type */
    }
 ...
 */
static rsRetVal AddPermittedPeerWildcard(permittedPeers_t* pPeer, uchar* pszStr,
                                         size_t lenStr)
{
    ...
    if (lenStr == 0)
    { /* empty domain components are permitted */
...
/* add a permitted peer. PermittedPeers is an interim solution until we can
 * provide access control via enhanced RainerScript methods. Note: the provided
 * string is handed over to this function, caller must no longer access it. --
...
static rsRetVal AddPermittedPeer(permittedPeers_t** ppRootPeer, uchar* pszID)
{
    ...
    CHKmalloc(
        pNew = calloc(
            1, sizeof(permittedPeers_t))); /* we use calloc() for consistency
                                              with "real" objects */
                                           ...
                                           int* const pbIsMatching)
{
    ...
    while (*pC != '\0')
    {
        if (pWildcard == NULL)
        {
            /* we have more domain components than we have wildcards --> no
             * match */
            *pbIsMatching = 0;
            FINALIZE;
        }
 ...
 */
static int mygetnameinfo(const struct sockaddr* sa, socklen_t salen, char* host,
                                      ...
                                      uint8_t iSignificantBits)
{
    ...
    if ((pEntry = (struct AllowedSenders*)calloc(
             1, sizeof(struct AllowedSenders))) == NULL)
    {
                                 ...
                                 uint8_t iSignificantBits)
{
    ...
    if (!F_ISSET(iAllow->flags, ADDR_NAME))
    {
        ...
        switch (iAllow->addr.NetAddr->sa_family)
        {
            case AF_INET:
                ...
                {
                    LogError(0, NO_ERRCODE,
                             "Invalid number of bits (%d) in IPv4 address - "
                             "adjusted to 32",
                             (int)iSignificantBits);
                    iSignificantBits = 32;
                }

                MaskIP4(&(SIN(iAllow->addr.NetAddr)->sin_addr),
                        iSignificantBits);
                break;
            case AF_INET6:
                ...
                {
                    LogError(0, NO_ERRCODE,
                             "Invalid number of bits (%d) in IPv6 address - "
                             "adjusted to 128",
                             iSignificantBits);
                    iSignificantBits = 128;
                }

                MaskIP6(&(SIN6(iAllow->addr.NetAddr)->sin6_addr),
                        iSignificantBits);
                break;
            default:
                /* rgerhards, 2007-07-16: We have an internal program error in
                 * this case. However, there is not much we can do against it
                 * right now. Of course, we could abort, but that would probably
                 * cause more harm than good. So we continue to run. We simply
                 * do not add this line - the worst thing that happens is that
                 * one host will not be allowed to log.
                 */
                LogError(0, NO_ERRCODE,
                         "Internal error caused AllowedSender to be ignored, "
                         "AF = %d",
    ...
    else
    {
        ...
        if (!strchr(iAllow->addr.HostWildcard, '*') &&
            !strchr(iAllow->addr.HostWildcard, '?') && ACLDontResolve == 0)
        {
            ...
            {
                LogError(0, NO_ERRCODE, "DNS error: Can't resolve \"%s\"",
                ...
                if (ACLAddHostnameOnFail)
                {
                    LogError(0, NO_ERRCODE,
                             ...
                             "entry.",
                             iAllow->addr.HostWildcard);
                    iRet = AddAllowedSenderEntry(ppRoot, ppLast, iAllow,
                                                 iSignificantBits);
                    FINALIZE;
                }
                else
            ...
            for (; res != NULL; res = res->ai_next)
            {
                switch (res->ai_family)
                {
                    case AF_INET: /* add IPv4 */
                        iSignificantBits = 32;
                        allowIP.flags = 0;
                        if ((allowIP.addr.NetAddr = malloc(res->ai_addrlen)) ==
                            NULL)
                        {
                            ABORT_FINALIZE(RS_RET_OUT_OF_MEMORY);
                        }
                        memcpy(allowIP.addr.NetAddr, res->ai_addr,
                               res->ai_addrlen);

                        if ((iRet = AddAllowedSenderEntry(
                                 ppRoot, ppLast, &allowIP, iSignificantBits)) !=
                            RS_RET_OK)
                        {
                        ...
                        break;
                    case AF_INET6: /* IPv6 - but need to check if it is a
                                      v6-mapped IPv4 */
                        if (IN6_IS_ADDR_V4MAPPED(
                                &SIN6(res->ai_addr)->sin6_addr))
                        {
                            /* extract & add IPv4 */
                            ...
                            SIN(allowIP.addr.NetAddr)->sin_len =
                                sizeof(struct sockaddr_in);
#endif
                            SIN(allowIP.addr.NetAddr)->sin_port = 0;
                            memcpy(
                                &(SIN(allowIP.addr.NetAddr)->sin_addr.s_addr),
                                &(SIN6(res->ai_addr)->sin6_addr.s6_addr32[3]),
                                sizeof(in_addr_t));

                            if ((iRet = AddAllowedSenderEntry(
                                     ppRoot, ppLast, &allowIP,
                        ...
                        else
                        {
                            ...
                            iSignificantBits = 128;
                            allowIP.flags = 0;
                            if ((allowIP.addr.NetAddr =
                                     malloc(res->ai_addrlen)) == NULL)
                            {
                                ABORT_FINALIZE(RS_RET_OUT_OF_MEMORY);
                            }
                            memcpy(allowIP.addr.NetAddr, res->ai_addr,
        ...
        else
        {
             ...
             */
            iRet =
                AddAllowedSenderEntry(ppRoot, ppLast, iAllow, iSignificantBits);
        }
    }
...
static rsRetVal addAllowedSenderLine(char* pName, uchar** ppRestOfConfLine)
{
    ...
    if ((iRet = rsParsConstructFromSz(&pPars, (uchar*)*ppRestOfConfLine) !=
                RS_RET_OK))
    {
        LogError(0, iRet,
                 "Error %d constructing parser object - ignoring allowed "
                 "sender list",
                 iRet);
        return (iRet);
    }
 ...
 * Returns 0 if they do not match, 1 if they match and 2 if a DNS name would
 * have been required. contributed 2007-07-16 by mildew@gmail.com
 */
static int MaskCmp(struct NetAddr* pAllow, uint8_t bits, struct sockaddr* pFrom,
                   const char* pszFromHost, int bChkDNS)
{
    assert(pAllow != NULL);
    assert(pFrom != NULL);
    ...
    if (F_ISSET(pAllow->flags, ADDR_NAME))
    {
        ...
        dbgprintf("MaskCmp: host=\"%s\"; pattern=\"%s\"\n", pszFromHost,
                  pAllow->addr.HostWildcard);

#if !defined(FNM_CASEFOLD)
        /* TODO: I don't know if that then works, seen on HP UX, what I have not
         * in lab... ;) */
        return (fnmatch(pAllow->addr.HostWildcard, pszFromHost, FNM_NOESCAPE) ==
                0);
#else
    ...
    { /* We need to compare an IP address */
        switch (pFrom->sa_family)
        {
            case AF_INET:
                if (AF_INET == pAllow->addr.NetAddr->sa_family)
                    return ((SIN(pFrom)->sin_addr.s_addr &
            ...
            case AF_INET6:
                switch (pAllow->addr.NetAddr->sa_family)
                {
                    case AF_INET6:
                    {
                        ...
                        memcpy(&ip, &(SIN6(pFrom))->sin6_addr,
                               sizeof(struct in6_addr));
                        memcpy(&net, &(SIN6(pAllow->addr.NetAddr))->sin6_addr,
                               sizeof(struct in6_addr));

                        i = bits / 32;
                        if (bits % 32)
                            ip.s6_addr32[i++] &=
                                htonl(0xffffffff << (32 - (bits % 32)));
                        for (; i < (sizeof ip.s6_addr32) / 4; i++)
                            ip.s6_addr32[i] = 0;

                        return (
                            memcmp(ip.s6_addr, net.s6_addr,
                                   sizeof ip.s6_addr) == 0 &&
                            (SIN6(pAllow->addr.NetAddr)->sin6_scope_id != 0
                                 ? SIN6(pFrom)->sin6_scope_id ==
                                       SIN6(pAllow->addr.NetAddr)->sin6_scope_id
                                 : 1));
                    }
                    case AF_INET:
                    {
                        ...
                        struct in_addr* net =
                            &(SIN(pAllow->addr.NetAddr))->sin_addr;

                        if ((ip6->s6_addr32[3] &
                             (u_int32_t)htonl((0xffffffff << (32 - bits)))) ==
                                net->s_addr &&
 ...
 * rgerhards, 2005-09-26
 */
static int isAllowedSender2(uchar* pszType, struct sockaddr* pFrom,
                            const char* pszFromHost, int bChkDNS)
{
    ...
    if (setAllowRoot(&pAllowRoot, pszType) != RS_RET_OK)
        return 0; /* if something went wrong, we deny access - that's the better
                     choice... */
    ...
    {
        ret = MaskCmp(&(pAllow->allowedSender), pAllow->SignificantBits, pFrom,
                      pszFromHost, bChkDNS);
        if (ret == 1)
            return 1;
        else if (ret == 2)
...
static int isAllowedSender(uchar* pszType, struct sockaddr* pFrom,
 ...
 */
static rsRetVal cvthname(struct sockaddr_storage* f, prop_t** localName,
                         prop_t** fqdn, prop_t** ip)
{
    DEFiRet;
    assert(f != NULL);
...
static rsRetVal getLocalHostname(uchar** ppName)
{
    DEFiRet;
    char hnbuf[8192];
    uchar* fqdn = NULL;
    ...
    if (!empty_hostname && dot == NULL)
    {
        ...
        if (error != 0 && error != EAI_NONAME && error != EAI_AGAIN &&
            error != EAI_FAIL)
        {
...
static rsRetVal ATTR_NONNULL(1, 2)
    create_single_udp_socket(int* const s, /* socket */
                             ...
                             const uchar* const hostname, const int bIsServer,
                             const int rcvbuf, const int sndbuf,
                             const int ipfreebind, const char* const device)
{
    const int on = 1;
    int sockflags;
    int actrcvbuf;
    ...
    *s = socket(r->ai_family, r->ai_socktype, r->ai_protocol);
    ...
    {
        int ion = 1;
        if (setsockopt(*s, IPPROTO_IPV6, IPV6_V6ONLY, (char*)&ion,
                       sizeof(ion)) < 0)
        {
            LogError(errno, RS_RET_ERR,
                     "error creating UDP socket - setsockopt");
            ABORT_FINALIZE(RS_RET_ERR);
        }
    }
#endif
    ...
    if (device)
    {
        ...
        if (setsockopt(*s, SOL_SOCKET, SO_BINDTODEVICE, device,
                       strlen(device) + 1) < 0)
#endif
        {
            LogError(errno, RS_RET_ERR,
                     "create UDP socket bound to device failed");
            ABORT_FINALIZE(RS_RET_ERR);
        }
    }
    ...
    {
        LogError(errno, RS_RET_ERR,
                 "create UDP socket failed to set REUSEADDR");
        ABORT_FINALIZE(RS_RET_ERR);
    }
    ...
    {
        if (setsockopt(*s, SOL_SOCKET, SO_BSDCOMPAT, (char*)&on, sizeof(on)) <
            0)
        {
            LogError(errno, RS_RET_ERR,
                     "create UDP socket failed to set BSDCOMPAT");
            ABORT_FINALIZE(RS_RET_ERR);
        }
    }
#endif
    ...
    {
        DBGPRINTF(
            "net.c: trying to set server socket %d to non-blocking mode\n", *s);
        ...
        if (sockflags == -1)
        {
            LogError(errno, RS_RET_ERR, "net.c: socket %d fcntl(O_NONBLOCK)",
                     *s);
            ABORT_FINALIZE(RS_RET_ERR);
        }
    }
    ...
    if (sndbuf != 0)
    {
        ...
        if (setsockopt(*s, SOL_SOCKET, SO_SNDBUFFORCE, &sndbuf,
                       sizeof(sndbuf)) < 0)
#endif
        {
            /* if we fail, try to do it the regular way. Experiments show that
             * at least some platforms do not return an error here, but silently
             * set it to the max permitted value. So we do our error check a bit
             * differently by querying the size below.
             */
            if (setsockopt(*s, SOL_SOCKET, SO_SNDBUF, &sndbuf,
                           sizeof(sndbuf)) != 0)
            {
                /* keep Coverity happy */
        ...
        if (getsockopt(*s, SOL_SOCKET, SO_SNDBUF, &actsndbuf, &optlen) == 0)
        {
            LogMsg(0, NO_ERRCODE, LOG_INFO,
                   "socket %d, actual os socket sndbuf size is %d", *s,
                   actsndbuf);
        ...
        {
            DBGPRINTF(
                "could not obtain os socket rcvbuf size for socket %d: %s\n",
    ...
    if (rcvbuf != 0)
    {
        ...
        if (setsockopt(*s, SOL_SOCKET, SO_RCVBUFFORCE, &rcvbuf,
                       sizeof(rcvbuf)) < 0)
#endif
        {
            /* if we fail, try to do it the regular way. Experiments show that
             ...
             * differently by querying the size below.
             */
            if (setsockopt(*s, SOL_SOCKET, SO_RCVBUF, &rcvbuf,
                           sizeof(rcvbuf)) != 0)
            {
                /* keep Coverity happy */
                DBGPRINTF("setsockopt in %s:%d failed - this is expected and "
        ...
        if (getsockopt(*s, SOL_SOCKET, SO_RCVBUF, &actrcvbuf, &optlen) == 0)
        {
            ...
            if (rcvbuf != 0 && actrcvbuf / 2 != rcvbuf)
            {
                LogError(errno, NO_ERRCODE,
                         "cannot set os socket rcvbuf size %d for socket %d, "
                         "value now is %d",
    ...
    if (bIsServer)
    {
        ...
        )
        {
            ...
            {
                if (setsockopt(*s, IPPROTO_IP, IP_FREEBIND, &on, sizeof(on)) <
                    0)
                {
 ...
 * similar for sndbuf.
 */
static int* create_udp_socket(uchar* hostname, uchar* pszPort,
                              const int bIsServer, const int rcvbuf,
                              const int sndbuf, const int ipfreebind,
                              char* device)
{
    ...
    assert(!((pszPort == NULL) &&
             (hostname == NULL))); /* one of them must be non-NULL */
    ...
    if (error)
    {
        ...
        LogError(0, NO_ERRCODE,
                 "UDP message reception disabled due to error logged in last "
                 "message.\n");
        return NULL;
    }
 ...
 * length of the sockets must be provided as third parameter. This is necessary
 * to compare non IPv4/v6 hosts, in which case we do a simple memory compare of
 * the address structure (in that case, the same host may not reliably be
 * detected). Note that we need to do the comparison not on the full structure,
 * because it contains things like the port, which we do not need to look at
 * when thinking about hostnames. So we look at the relevant fields, what means
 * a somewhat more complicated processing. Also note that we use a non-standard
 * calling interface, as this is much more natural and it looks extremely
 * unlikely that we get an exception of any kind here. What we return is mimiced
 * after memcmp(), and as such useful for building binary trees (the order
 * relation may be a bit arbritrary, but at least it is consistent). rgerhards,
 * 2009-09-03
 */
static int CmpHost(struct sockaddr_storage* s1, struct sockaddr_storage* s2,
                   size_t socklen)
{
    int ret;
    ...
    {
        if (((struct sockaddr_in*)s1)->sin_addr.s_addr ==
            ((struct sockaddr_in*)s2)->sin_addr.s_addr)
        {
            ret = 0;
        }
        else if (((struct sockaddr_in*)s1)->sin_addr.s_addr <
                 ((struct sockaddr_in*)s2)->sin_addr.s_addr)
        {
            ret = -1;
        }
...
/* check if restrictions (ALCs) exists. The goal of this function is to disable
 * the somewhat time-consuming ACL checks if no restrictions are defined (the
 * usual case). This also permits to gain some speedup by using firewall-based
 * ACLs instead of rsyslog ACLs (the recommended method. rgerhards, 2009-11-16
 */
...
finalize_it:
    ...
    {
        *bHasRestrictions =
            1; /* in this case it is better to check individually */
...
PRAGMA_IGNORE_Wcast_align /* TODO: how can we fix these warnings? */
    /* Problem with the warnings: they seem to stem back from the way the API is
       structured */
    static rsRetVal
...
finalize_it
    : ENDobjQueryInterface(net)
       ...
       */
      BEGINObjClassExit(
          net, OBJ_IS_LOADABLE_MODULE) /* CHANGE class also in END MACRO! */
...
ENDObjClassInit(net)
    ...
    /* --------------- here now comes the plumbing that makes as a library
       module --------------- */

    BEGINmodExit CODESTARTmodExit netClassExit();
ENDmodExit

    BEGINqueryEtryPt CODESTARTqueryEtryPt CODEqueryEtryPt_STD_LIB_QUERIES
        ENDqueryEtryPt

    BEGINmodInit() CODESTARTmodInit* ipIFVersProvided =
        CURR_MOD_IF_VERSION; /* we only support the current interface
                                specification */

/* Initialize all classes that are in our module - this includes ourselfs */
CHKiRet(
    netClassInit(pModInfo)); /* must be done after tcps_sess, as we use it */
ENDmodInit
    /* vi:set ai:
     */
