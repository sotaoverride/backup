...
#define ALL_STDIN_FLAGS                                                        \
    (G_SUBPROCESS_FLAGS_STDIN_PIPE | G_SUBPROCESS_FLAGS_STDIN_INHERIT)
#define ALL_STDOUT_FLAGS                                                       \
    (G_SUBPROCESS_FLAGS_STDOUT_PIPE | G_SUBPROCESS_FLAGS_STDOUT_SILENCE)
#define ALL_STDERR_FLAGS                                                       \
    (G_SUBPROCESS_FLAGS_STDERR_PIPE | G_SUBPROCESS_FLAGS_STDERR_SILENCE |      \
...
static gboolean verify_disposition(const gchar* stream_name,
                                   GSubprocessFlags filtered_flags, gint fd,
                                   const gchar* filename)
{
    ...
    if (n_bits + (fd >= 0) + (filename != NULL) > 1)
    {
        ...
        if (fd >= 0)
            g_string_append_printf(err, " g_subprocess_launcher_take_%s_fd()",
                                   stream_name);

        if (filename)
            g_string_append_printf(
                err, " g_subprocess_launcher_set_%s_file_path()", stream_name);

        g_critical("You may specify at most one disposition for the %s stream, "
                   "but you specified:%s.",
                   stream_name, err->str);
...
static gboolean verify_flags(GSubprocessFlags flags)
{
...
static void g_subprocess_launcher_set_property(GObject* object, guint prop_id,
                                               const GValue* value,
                                               GParamSpec* pspec)
{
...
static void g_subprocess_launcher_finalize(GObject* object)
{
...
static void g_subprocess_launcher_init(GSubprocessLauncher* self)
{
...
static void g_subprocess_launcher_class_init(GSubprocessLauncherClass* class)
{
...
GSubprocessLauncher* g_subprocess_launcher_new(GSubprocessFlags flags)
{
    ...
    return g_object_new(G_TYPE_SUBPROCESS_LAUNCHER, "flags", flags, NULL);
}
                                     ...
                                     GSubprocessFlags flags)
{
    ...
    if (verify_disposition("stdin", flags & ALL_STDIN_FLAGS, stdin_fd,
                           stdin_path) &&
        verify_disposition("stdout", flags & ALL_STDOUT_FLAGS, stdout_fd,
                           stdout_path) &&
        verify_disposition("stderr", flags & ALL_STDERR_FLAGS, stderr_fd,
                           stderr_path))
        self->flags = flags;
}

#ifdef G_OS_UNIX
static void assign_fd(gint* fd_ptr, gint fd)
{
    gint flags;
...
void g_subprocess_launcher_set_stdin_file_path(GSubprocessLauncher* self,
                                               const gchar* path)
{
    if (verify_disposition("stdin", self->flags & ALL_STDIN_FLAGS,
                           self->stdin_fd, path))
    {
        g_free(self->stdin_path);
...
{
    if (verify_disposition("stdin", self->flags & ALL_STDIN_FLAGS, fd,
                           self->stdin_path))
...
void g_subprocess_launcher_set_stdout_file_path(GSubprocessLauncher* self,
                                                const gchar* path)
{
    if (verify_disposition("stdout", self->flags & ALL_STDOUT_FLAGS,
                           self->stdout_fd, path))
    {
        g_free(self->stdout_path);
...
{
    if (verify_disposition("stdout", self->flags & ALL_STDOUT_FLAGS, fd,
                           self->stdout_path))
...
void g_subprocess_launcher_set_stderr_file_path(GSubprocessLauncher* self,
                                                const gchar* path)
{
    if (verify_disposition("stderr", self->flags & ALL_STDERR_FLAGS,
                           self->stderr_fd, path))
    {
        g_free(self->stderr_path);
...
{
    if (verify_disposition("stderr", self->flags & ALL_STDERR_FLAGS, fd,
                           self->stderr_path))
 ...
 */
void g_subprocess_launcher_take_fd(GSubprocessLauncher* self, gint source_fd,
                                   gint target_fd)
{
 ...
 *will be set)
 **/
GSubprocess* g_subprocess_launcher_spawn(GSubprocessLauncher* launcher,
                                         ...
                                         ...)
{
    ...
    result = g_subprocess_launcher_spawnv(
 ...
 * @self: a #GSubprocessLauncher
 * @argv: (array zero-terminated=1) (element-type filename): Command line
 *arguments
 * @error: Error
 *
                                          ...
                                          GError** error)
{
    ...
    subprocess = g_object_new(G_TYPE_SUBPROCESS, "argv", argv, "flags",
                              launcher->flags, NULL);
