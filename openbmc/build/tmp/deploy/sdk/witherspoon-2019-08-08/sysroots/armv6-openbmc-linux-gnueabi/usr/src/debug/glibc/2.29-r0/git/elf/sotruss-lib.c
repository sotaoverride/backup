...
/* Check whether a file name is on the colon-separated list of file
   names.  */
static unsigned int match_file(const char* list, const char* name,
                               size_t name_len, unsigned int mask)
{
    if (list[0] == '\0')
        return 0;
...
unsigned int la_objopen(struct link_map* map, Lmid_t lmid, uintptr_t* cookie)
{
    ...
    for (struct libname_list* l = map->l_libname; l != NULL; l = l->next)
    {
        ...
        if (fromlist != NULL)
            result |=
                match_file(fromlist, l->name, strlen(l->name), LA_FLG_BINDFROM);

        if (tolist != NULL)
            result |=
                match_file(tolist, l->name, strlen(l->name), LA_FLG_BINDTO);
    }
    ...
    *cookie = (uintptr_t)print_name;
    ...
    if (fromlist == NULL)
        ...
        result |=
            (match_file(fromlist, full_name, full_name_len, LA_FLG_BINDFROM) |
             match_file(fromlist, base_name, base_name_len, LA_FLG_BINDFROM));
...
static void print_enter(uintptr_t* refcook, uintptr_t* defcook,
                        const char* symname, unsigned long int reg1,
                        unsigned long int reg2, unsigned long int reg3,
                        unsigned int flags)
{
...
Elf64_Addr la_x86_64_gnu_pltenter(Elf64_Sym* sym __attribute__((unused)),
                                  ...
                                  const char* symname, long int* framesizep)
{
    print_enter(refcook, defcook, symname, regs->lr_rdi, regs->lr_rsi,
                regs->lr_rdx, *flags);
...
static void print_exit(uintptr_t* refcook, uintptr_t* defcook,
                       const char* symname, unsigned long int reg)
{
    char buf[3 * sizeof(pid_t) + 3];
    buf[0] = '\0';
    if (print_pid)
    ...
    fprintf(out_file, "%s%15s -> %-15s:%s%s - 0x%lx\n", buf, (char*)*refcook,
            (char*)*defcook, " ", symname, reg);
}

#ifdef __i386__
unsigned int la_i86_gnu_pltexit(Elf32_Sym* sym, unsigned int ndx,
                                uintptr_t* refcook, uintptr_t* defcook,
                                const struct La_i86_regs* inregs,
                                struct La_i86_retval* outregs,
                                const char* symname)
{
...
unsigned int la_x86_64_gnu_pltexit(Elf64_Sym* sym, unsigned int ndx,
                                   uintptr_t* refcook, uintptr_t* defcook,
                                   const struct La_x86_64_regs* inregs,
                                   struct La_x86_64_retval* outregs,
                                   const char* symname)
{
...
#elif defined __sparc__ && !defined __arch64__
unsigned int la_sparc32_gnu_pltexit(Elf32_Sym* sym, unsigned int ndx,
                                    uintptr_t* refcook, uintptr_t* defcook,
                                    const struct La_sparc32_regs* inregs,
                                    struct La_sparc32_retval* outregs,
                                    const char* symname)
{
...
#elif defined __sparc__ && defined __arch64__
unsigned int la_sparc64_gnu_pltexit(Elf64_Sym* sym, unsigned int ndx,
                                    uintptr_t* refcook, uintptr_t* defcook,
                                    const struct La_sparc64_regs* inregs,
                                    struct La_sparc64_retval* outregs,
                                    const char* symname)
{
    print_exit(refcook, defcook, symname, outregs->lrv_reg[0]);
