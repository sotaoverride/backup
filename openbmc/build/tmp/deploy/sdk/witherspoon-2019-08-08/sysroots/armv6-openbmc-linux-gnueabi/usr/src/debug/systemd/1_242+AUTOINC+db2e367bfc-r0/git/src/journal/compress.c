...
#if HAVE_LZ4
DEFINE_TRIVIAL_CLEANUP_FUNC(LZ4F_compressionContext_t,
                            LZ4F_freeCompressionContext);
DEFINE_TRIVIAL_CLEANUP_FUNC(LZ4F_decompressionContext_t,
                     ...
                     size_t dst_alloc_size, size_t* dst_size)
{
    ...
    ret =
        lzma_stream_buffer_encode((lzma_filter*)filters, LZMA_CHECK_NONE, NULL,
                    ...
                    size_t dst_max)
{
    if (compression == OBJECT_COMPRESSED_XZ)
        return decompress_blob_xz(src, src_size, dst, dst_alloc_size, dst_size,
                                  dst_max);
    else if (compression == OBJECT_COMPRESSED_LZ4)
        return decompress_blob_lz4(src, src_size, dst, dst_alloc_size, dst_size,
                                   dst_max);
    else
        return -EBADMSG;
}
...
int decompress_startswith_xz(const void* src, uint64_t src_size, void** buffer,
                             size_t* buffer_size, const void* prefix,
...
int decompress_startswith(int compression, const void* src, uint64_t src_size,
                          ...
                          const void* prefix, size_t prefix_len, uint8_t extra)
{
    if (compression == OBJECT_COMPRESSED_XZ)
        return decompress_startswith_xz(src, src_size, buffer, buffer_size,
                                        prefix, prefix_len, extra);
    else if (compression == OBJECT_COMPRESSED_LZ4)
        return decompress_startswith_lz4(src, src_size, buffer, buffer_size,
                                         prefix, prefix_len, extra);
    else
        return -EBADMSG;
}
...
int compress_stream_xz(int fdf, int fdt, uint64_t max_bytes)
{
    ...
    for (;;)
    {
        ...
        if (s.avail_out == 0 || ret == LZMA_STREAM_END)
        {
            ...
            if (ret == LZMA_STREAM_END)
            {
                log_debug("XZ compression finished (%" PRIu64 " -> %" PRIu64
                          " bytes, %.1f%%)",
...
int compress_stream_lz4(int fdf, int fdt, uint64_t max_bytes)
{
...
#if HAVE_LZ4
    LZ4F_errorCode_t c;
    _cleanup_(LZ4F_freeCompressionContextp) LZ4F_compressionContext_t ctx =
        NULL;
    ...
    while (total_in < (size_t)st.st_size)
    {
        ...
        {
            log_debug("Compressed stream longer than %" PRIu64 " bytes",
                      max_bytes);
            return -EFBIG;
        }
    ...
    log_debug("LZ4 compression finished (%zu -> %zu bytes, %.1f%%)", total_in,
...
int decompress_stream_xz(int fdf, int fdt, uint64_t max_bytes)
{
    ...
    for (;;)
    {
        ...
        if (s.avail_out == 0 || ret == LZMA_STREAM_END)
        {
            ...
            if (ret == LZMA_STREAM_END)
            {
                log_debug("XZ decompression finished (%" PRIu64 " -> %" PRIu64
                          " bytes, %.1f%%)",
                          s.total_in, s.total_out,
                          (double)s.total_out / s.total_in * 100);
...
int decompress_stream_lz4(int in, int out, uint64_t max_bytes)
{
#if HAVE_LZ4
    size_t c;
    _cleanup_(LZ4F_freeDecompressionContextp) LZ4F_decompressionContext_t ctx =
        NULL;
    _cleanup_free_ char* buf = NULL;
    char* src;
    struct stat st;
    ...
    while (total_in < (size_t)st.st_size)
    {
        ...
        if (max_bytes != (uint64_t)-1 && total_out > (size_t)max_bytes)
        {
            log_debug("Decompressed stream longer than %" PRIu64 " bytes",
                      max_bytes);
            r = -EFBIG;
            goto cleanup;
        }
    ...
    log_debug("LZ4 decompression finished (%zu -> %zu bytes, %.1f%%)", total_in,
              total_out,
...
int decompress_stream(const char* filename, int fdf, int fdt,
                      uint64_t max_bytes)
{
