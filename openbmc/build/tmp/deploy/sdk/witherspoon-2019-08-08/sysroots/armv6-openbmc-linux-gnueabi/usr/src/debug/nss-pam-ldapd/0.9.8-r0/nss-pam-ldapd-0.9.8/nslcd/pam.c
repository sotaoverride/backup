...
static void search_var_add(DICT* dict, const char* name, const char* value)
{
    ...
    {
        log_log(LOG_CRIT,
                "search_var_add(): malloc() failed to allocate memory");
        return;
    }
                             ...
                             const char* rhost, const char* tty)
{
    ...
    {
        log_log(LOG_CRIT,
                "search_vars_new(): dict_new() failed to allocate memory");
        return NULL;
    }
                       ...
                       const char* filter)
{
    ...
    for (i = 0; (base = nslcd_cfg->bases[i]) != NULL; i++)
    {
        ...
        if (search == NULL)
        {
            log_log(LOG_ERR, "%s \"%s\" failed: %s", option, filter,
                    ldap_err2string(rc));
            return rc;
        }
        /* try to get an entry */
                    ...
                    int* authzrc, char* authzmsg, size_t authzmsgsz)
{
    ...
    if (rc == LDAP_SUCCESS)
    {
        ...
        else if (strcasecmp(nslcd_cfg->pam_authc_search, "NONE") != 0)
        {
            ...
            dict =
                search_vars_new(userdn, username, service, ruser, rhost, tty);
            if (dict == NULL)
            {
                myldap_session_close(session);
                return LDAP_LOCAL_ERROR;
            }
            res = expr_parse(nslcd_cfg->pam_authc_search, filter,
                             sizeof(filter), search_var_get, (void*)dict);
            if (res == NULL)
            {
...
/* ensure that both userdn and username are filled in from the entry,
   returns an LDAP result code */
static MYLDAP_ENTRY* validate_user(MYLDAP_SESSION* session, char* username,
                                   int* rcp)
{
    int rc;
                            ...
                            size_t username_len)
{
    ...
    if (value == NULL)
    {
        ...
        {
            log_log(LOG_WARNING, "%s: %s: missing", myldap_get_dn(entry),
                    attmap_passwd_uid);
            return;
        }
        value = values[0];
    ...
    {
        log_log(LOG_INFO, "username changed from \"%s\" to \"%s\"", username,
                value);
...
static int check_shadow(MYLDAP_SESSION* session, const char* username,
                        char* authzmsg, size_t authzmsgsz, int check_maxdays,
                        int check_mindays)
{
    MYLDAP_ENTRY* entry = NULL;
    ...
    if ((expiredate != -1) && (today >= expiredate))
    {
        ...
        log_log(LOG_WARNING, "%s: %s: %s", myldap_get_dn(entry),
                attmap_shadow_shadowExpire, authzmsg);
    ...
    if (check_maxdays)
    {
        ...
        if (lastchangedate == 0)
        {
            ...
            log_log(LOG_WARNING, "%s: %s: %s", myldap_get_dn(entry),
                    attmap_shadow_shadowLastChange, authzmsg);
        ...
        else if (maxdays != -1)
        {
            ...
            if (daysleft == 0)
                ...
                mysnprintf(authzmsg, authzmsgsz - 1,
                           "password expired %ld days ago", -daysleft);
            /* check inactdays */
            if ((daysleft <= 0) && (inactdays != -1))
            {
                inactleft = lastchangedate + maxdays + inactdays - today;
                if (inactleft == 0)
                    mysnprintf(authzmsg + strlen(authzmsg),
                               authzmsgsz - strlen(authzmsg) - 1,
                               ", account will be locked today");
                else if (inactleft > 0)
                    mysnprintf(authzmsg + strlen(authzmsg),
                               authzmsgsz - strlen(authzmsg) - 1,
                               ", account will be locked in %ld days",
                               inactleft);
                else
                {
                    mysnprintf(authzmsg + strlen(authzmsg),
            ...
            if (daysleft <= 0)
            {
                ...
                log_log(LOG_WARNING, "%s: %s: %s", myldap_get_dn(entry),
                        attmap_shadow_shadowMax, authzmsg);
                return NSLCD_PAM_NEW_AUTHTOK_REQD;
            }
            /* check warndays */
            if ((warndays > 0) && (daysleft <= warndays))
            {
                ...
                log_log(LOG_WARNING, "%s: %s: %s", myldap_get_dn(entry),
                        attmap_shadow_shadowWarning, authzmsg);
            }
        }
    }
    if (check_mindays)
    {
        ...
        {
            mysnprintf(authzmsg, authzmsgsz - 1,
                       "password cannot be changed for another %ld days",
                       daysleft);
            log_log(LOG_WARNING, "%s: %s: %s", myldap_get_dn(entry),
                    attmap_shadow_shadowMin, authzmsg);
...
int nslcd_pam_authc(TFILE* fp, MYLDAP_SESSION* session, uid_t calleruid)
{
    ...
    log_log(LOG_DEBUG, "nslcd_pam_authc(\"%s\",\"%s\",\"%s\")", username,
            service, *password ? "***" : "");
    ...
    if (*username == '\0')
    {
            ...
            (nslcd_cfg->rootpwmodpw != NULL))
        {
            if (strlen(nslcd_cfg->rootpwmodpw) >= sizeof(password))
            {
                log_log(
                    LOG_ERR,
                    "nslcd_pam_authc(): rootpwmodpw will not fit in password");
                memset(password, 0, sizeof(password));
                return -1;
            }
    ...
    if ((*username != '\0') && (authzrc == NSLCD_PAM_SUCCESS))
        authzrc =
            check_shadow(session, username, authzmsg, sizeof(authzmsg), 1, 0);
    /* write response */
                            ...
                            const char* tty)
{
    ...
    for (i = 0; (i < NSS_LDAP_CONFIG_MAX_AUTHZ_SEARCHES) &&
                (nslcd_cfg->pam_authz_searches[i] != NULL);
         i++)
    {
        if (dict == NULL)
        {
        ...
        /* build the search filter */
        res = expr_parse(nslcd_cfg->pam_authz_searches[i], filter,
                         sizeof(filter), search_var_get, (void*)dict);
        if (res == NULL)
        {
            search_vars_free(dict);
...
int nslcd_pam_sess_c(TFILE* fp, MYLDAP_SESSION UNUSED(*session))
{
    ...
    log_log(LOG_DEBUG, "nslcd_pam_sess_c(\"%s\",\"%s\",%s)", username, service,
            sessionid);
    /* write the response header */
    WRITE_INT32(fp, NSLCD_VERSION);
...
static int update_lastchange(MYLDAP_SESSION* session, const char* userdn)
{
    ...
    if (rc != LDAP_SUCCESS)
        log_log(LOG_WARNING, "%s: %s: modification failed: %s", userdn, attr,
                ldap_err2string(rc));
    else
...
static int try_pwmod(MYLDAP_SESSION* oldsession, const char* binddn,
                     const char* userdn, const char* oldpassword,
                     const char* newpassword, char* authzmsg,
                     size_t authzmsg_len)
{
    MYLDAP_SESSION* session;
    ...
    if (rc == LDAP_SUCCESS)
    {
        /* if doing password modification as admin, don't pass old password
         * along */
        if ((nslcd_cfg->rootpwmoddn != NULL) &&
        ...
        else
        {
            ...
            if ((myldap_error_message(session, rc, buffer, sizeof(buffer)) ==
                 LDAP_SUCCESS) &&
                (buffer[0] != '\0'))
                mysnprintf(authzmsg, authzmsg_len - 1,
                           "password change failed: %s", buffer);
        }
    }
    /* close the session */
    myldap_session_close(session);
...
int nslcd_pam_pwmod(TFILE* fp, MYLDAP_SESSION* session, uid_t calleruid)
{
    ...
    if (asroot)
    {
            ...
            (nslcd_cfg->rootpwmodpw != NULL))
        {
            ...
            {
                log_log(LOG_ERR, "nslcd_pam_pwmod(): rootpwmodpw will not fit "
                                 "in oldpassword");
                memset(oldpassword, 0, sizeof(oldpassword));
                memset(newpassword, 0, sizeof(newpassword));
                return -1;
    ...
    rc = try_pwmod(session, binddn, myldap_get_dn(entry), oldpassword,
                   newpassword, authzmsg, sizeof(authzmsg));
    if (rc != LDAP_SUCCESS)
    {
        if (authzmsg[0] == '\0')
            mysnprintf(authzmsg, sizeof(authzmsg) - 1,
                       "password change failed: %s", ldap_err2string(rc));
        WRITE_INT32(fp, NSLCD_RESULT_BEGIN);
        WRITE_INT32(fp, NSLCD_PAM_PERM_DENIED);
        WRITE_STRING(fp, authzmsg);
        WRITE_INT32(fp, NSLCD_RESULT_END);
