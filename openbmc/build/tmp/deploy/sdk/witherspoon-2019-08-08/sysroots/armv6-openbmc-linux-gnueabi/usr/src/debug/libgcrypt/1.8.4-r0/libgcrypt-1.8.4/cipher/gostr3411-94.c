...
static void gost3411_init(void* context, unsigned int flags)
{
...
static void gost3411_cp_init(void* context, unsigned int flags)
{
    GOSTR3411_CONTEXT* hd = context;
...
static void do_p(u32* p, u32* u, u32* v)
{
    int k;
    u32 t[8];
    ...
    {
        p[k + 0] =
            ((t[0] >> (8 * k)) & 0xff) << 0 | ((t[2] >> (8 * k)) & 0xff) << 8 |
            ((t[4] >> (8 * k)) & 0xff) << 16 | ((t[6] >> (8 * k)) & 0xff) << 24;
        p[k + 4] =
            ((t[1] >> (8 * k)) & 0xff) << 0 | ((t[3] >> (8 * k)) & 0xff) << 8 |
            ((t[5] >> (8 * k)) & 0xff) << 16 | ((t[7] >> (8 * k)) & 0xff) << 24;
    }
}

static void do_a(u32* u)
{
    u32 t[2];
    int i;
    memcpy(t, u, 2 * 4);
...
#define do_chi_doublestep(e, i)                                                \
    e[i] ^= (e[i] >> 16) ^ (e[(i + 1) % 8] << 16) ^ e[(i + 1) % 8] ^           \
            (e[(i + 1) % 8] >> 16) ^ (e[(i + 2) % 8] << 16) ^ e[(i + 6) % 8] ^ \
            (e[(i + 7) % 8] >> 16);                                            \
    e[i] ^= (e[i] << 16);

static void do_chi_submix12(u32* e, u32* x)
{
    e[6] ^= x[0];
    e[7] ^= x[1];
    e[0] ^= x[2];
...
static void do_chi_submix13(u32* e, u32* x)
{
    e[6] ^= (x[0] << 16) | (x[7] >> 16);
    e[7] ^= (x[1] << 16) | (x[0] >> 16);
    e[0] ^= (x[2] << 16) | (x[1] >> 16);
...
static void do_add(u32* s, u32* a)
{
    u32 carry = 0;
    int i;
...
static unsigned int do_hash_step(GOSTR3411_CONTEXT* hd, u32* h, u32* m)
{
    u32 u[8], v[8];
    u32 s[8];
    u32 k[8];
    ...
    for (i = 0; i < 4; i++)
    {
        ...
        burn = _gcry_gost_enc_data(&hd->hd, k, &s[2 * i], &s[2 * i + 1],
                                   h[2 * i], h[2 * i + 1], hd->cryptopro);
    ...
    return /* burn_stack */ 4 * sizeof(void*) /* func call (ret addr + args) */
           ...
           max(burn /* _gcry_gost_enc_one */,
               sizeof(void*) * 2 /* do_a2 call */ + 16 +
                   sizeof(int) /* do_a2 stack */);
}
...
static void gost3411_final(void* context)
{
    GOSTR3411_CONTEXT* hd = context;
    size_t padlen = 0;
    u32 l[8];
...
static byte* gost3411_read(void* context)
{
    GOSTR3411_CONTEXT* hd = context;
...
gcry_md_spec_t _gcry_digest_spec_gost3411_94 = {GCRY_MD_GOSTR3411_94,
                                                {0, 0},
                                                "GOSTR3411_94",
                                                NULL,
                                                0,
...
gcry_md_spec_t _gcry_digest_spec_gost3411_cp = {GCRY_MD_GOSTR3411_CP,
                                                {0, 0},
                                                "GOSTR3411_CP",
                                                asn,
                                                DIM(asn),
