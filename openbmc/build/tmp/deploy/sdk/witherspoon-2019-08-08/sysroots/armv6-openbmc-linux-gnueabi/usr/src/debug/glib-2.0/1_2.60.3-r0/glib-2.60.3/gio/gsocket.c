...
#ifdef G_OS_WIN32
/* For Windows XP runtime compatibility, but use the system's if_nametoindex()
 * if available */
 ...
 * g_socket_condition_check(), or g_socket_condition_wait(). You can also use
 * g_socket_create_source() and attach it to a #GMainContext to get callbacks
 * when I/O is possible. Note that all sockets are always set to non blocking
 * mode in the system, and blocking mode is emulated in GSocket.
 *
...
static gint g_socket_datagram_based_receive_messages(
    GDatagramBased* self, GInputMessage* messages, guint num_messages,
    gint flags, gint64 timeout_us, GCancellable* cancellable, GError** error);
static gint g_socket_datagram_based_send_messages(
    GDatagramBased* self, GOutputMessage* messages, guint num_messages,
    gint flags, gint64 timeout_us, GCancellable* cancellable, GError** error);
static GSource* g_socket_datagram_based_create_source(
    GDatagramBased* self, GIOCondition condition, GCancellable* cancellable);
static GIOCondition
    g_socket_datagram_based_condition_check(GDatagramBased* datagram_based,
                                            GIOCondition condition);
static gboolean g_socket_datagram_based_condition_wait(
    GDatagramBased* datagram_based, GIOCondition condition, gint64 timeout_us,
    GCancellable* cancellable, GError** error);

static GSocketAddress* cache_recv_address(GSocket* socket,
                                          struct sockaddr* native,
                                          int native_len);

static gssize g_socket_receive_message_with_timeout(
    GSocket* socket, GSocketAddress** address, GInputVector* vectors,
    gint num_vectors, GSocketControlMessage*** messages, gint* num_messages,
    gint* flags, gint64 timeout_us, GCancellable* cancellable, GError** error);
static gint g_socket_receive_messages_with_timeout(
    GSocket* socket, GInputMessage* messages, guint num_messages, gint flags,
    gint64 timeout_us, GCancellable* cancellable, GError** error);
static gint g_socket_send_messages_with_timeout(
    GSocket* socket, GOutputMessage* messages, guint num_messages, gint flags,
    gint64 timeout_us, GCancellable* cancellable, GError** error);
...
_G_DEFINE_TYPE_EXTENDED_WITH_PRELUDE(
    GSocket, g_socket, G_TYPE_OBJECT, 0,
    /* Need a prelude for https://bugzilla.gnome.org/show_bug.cgi?id=674885 */
    g_type_ensure(G_TYPE_SOCKET_FAMILY);
    g_type_ensure(G_TYPE_SOCKET_TYPE); g_type_ensure(G_TYPE_SOCKET_PROTOCOL);
    ...
    G_ADD_PRIVATE(GSocket)
        G_IMPLEMENT_INTERFACE(G_TYPE_INITABLE, g_socket_initable_iface_init);
...
#define win32_unset_event_mask(_socket, _mask)                                 \
    _win32_unset_event_mask(_socket, _mask)
...
#define recv(sockfd, buf, len, flags) recv(sockfd, (gpointer)buf, len, flags)
#endif
...
static gboolean check_timeout(GSocket* socket, GError** error)
{
...
static void g_socket_details_from_fd(GSocket* socket)
{
    ...
    if (addrlen > 0)
    {
        g_assert(G_STRUCT_OFFSET(struct sockaddr, sa_family) +
                     sizeof address.storage.ss_family <=
                 addrlen);
...
err:
    g_set_error(&socket->priv->construct_error, G_IO_ERROR,
                ...
                _("creating GSocket from fd: %s"), socket_strerror(errsv));
}

/* Wrapper around socket() that is shared with gnetworkmonitornetlink.c */
gint g_socket(gint domain, gint type, gint protocol, GError** error)
{
    int fd, errsv;
...
static gint g_socket_create_socket(GSocketFamily family, GSocketType type,
                                   int protocol, GError** error)
{
    gint native_type;
    ...
    if (family <= 0)
    {
        g_set_error(error, G_IO_ERROR, G_IO_ERROR_INVALID_ARGUMENT,
                    ...
                    _("Unknown family was specified"));
        return -1;
    }
    ...
    if (protocol == -1)
    {
        g_set_error(error, G_IO_ERROR, G_IO_ERROR_INVALID_ARGUMENT,
                    _("Unable to create socket: %s"),
                    _("Unknown protocol was specified"));
        return -1;
    }
...
static void g_socket_constructed(GObject* object)
{
    ...
    if (socket->priv->fd >= 0)
        ...
        socket->priv->fd = g_socket_create_socket(
            socket->priv->family, socket->priv->type, socket->priv->protocol,
    ...
    if (socket->priv->fd != -1)
    {
        ...
        if (ioctlsocket(socket->priv->fd, FIONBIO, &arg) == SOCKET_ERROR)
        {
            ...
            g_warning("Error setting socket status flags: %s",
...
static void g_socket_get_property(GObject* object, guint prop_id, GValue* value,
                                  GParamSpec* pspec)
{
    GSocket* socket = G_SOCKET(object);
...
static void g_socket_set_property(GObject* object, guint prop_id,
...
static void g_socket_finalize(GObject* object)
{
    ...
    if (socket->priv->fd != -1 && !socket->priv->closed)
...
static void g_socket_class_init(GSocketClass* klass)
{
    ...
    g_object_class_install_property(
        ...
        g_param_spec_enum("family", P_("Socket family"),
                          P_("The sockets address family"),
                          G_TYPE_SOCKET_FAMILY, G_SOCKET_FAMILY_INVALID,
                          G_PARAM_CONSTRUCT_ONLY | G_PARAM_READWRITE |
                              G_PARAM_STATIC_STRINGS));

    g_object_class_install_property(
        gobject_class, PROP_TYPE,
        g_param_spec_enum("type", P_("Socket type"), P_("The sockets type"),
                          G_TYPE_SOCKET_TYPE, G_SOCKET_TYPE_STREAM,
                          G_PARAM_CONSTRUCT_ONLY | G_PARAM_READWRITE |
                              G_PARAM_STATIC_STRINGS));
    ...
    g_object_class_install_property(
        ...
        g_param_spec_int("fd", P_("File descriptor"),
                         P_("The sockets file descriptor"), G_MININT, G_MAXINT,
                         -1,
                         G_PARAM_CONSTRUCT_ONLY | G_PARAM_READWRITE |
                             G_PARAM_STATIC_STRINGS));

    g_object_class_install_property(
        gobject_class, PROP_BLOCKING,
        g_param_spec_boolean(
            "blocking", P_("blocking"),
            P_("Whether or not I/O on this socket is blocking"), TRUE,
            G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS));

    g_object_class_install_property(
        gobject_class, PROP_LISTEN_BACKLOG,
        g_param_spec_int("listen-backlog", P_("Listen backlog"),
                         P_("Outstanding connections in the listen queue"), 0,
                         SOMAXCONN, 10,
                         G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS));

    g_object_class_install_property(
        gobject_class, PROP_KEEPALIVE,
        g_param_spec_boolean(
            "keepalive", P_("Keep connection alive"),
            P_("Keep connection alive by sending periodic pings"), FALSE,
            G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS));

    g_object_class_install_property(
        gobject_class, PROP_LOCAL_ADDRESS,
        g_param_spec_object("local-address", P_("Local address"),
                            P_("The local address the socket is bound to"),
                            G_TYPE_SOCKET_ADDRESS,
                            G_PARAM_READABLE | G_PARAM_STATIC_STRINGS));

    g_object_class_install_property(
        gobject_class, PROP_REMOTE_ADDRESS,
        g_param_spec_object("remote-address", P_("Remote address"),
    ...
    g_object_class_install_property(
        ...
        g_param_spec_uint("timeout", P_("Timeout"),
                          P_("The timeout in seconds on socket I/O"), 0,
                          G_MAXUINT, 0,
                          G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS));
     ...
     */
    g_object_class_install_property(
        gobject_class, PROP_BROADCAST,
        g_param_spec_boolean(
            "broadcast", P_("Broadcast"),
            P_("Whether to allow sending to broadcast addresses"), FALSE,
            G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS));
     ...
     */
    g_object_class_install_property(
        gobject_class, PROP_TTL,
        g_param_spec_uint(
            "ttl", P_("TTL"), P_("Time-to-live of outgoing unicast packets"), 0,
     ...
     */
    g_object_class_install_property(
        gobject_class, PROP_MULTICAST_LOOPBACK,
        g_param_spec_boolean("multicast-loopback", P_("Multicast loopback"),
                             P_("Whether outgoing multicast packets loop back "
                                "to the local host"),
     ...
     */
    g_object_class_install_property(
        gobject_class, PROP_MULTICAST_TTL,
        g_param_spec_uint("multicast-ttl", P_("Multicast TTL"),
                          P_("Time-to-live of outgoing multicast packets"), 0,
                          G_MAXUINT, 1,
                          G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS));
}
...
static gboolean check_datagram_based(GDatagramBased* self, GError** error)
{
    ...
    gint flags, gint64 timeout_us, GCancellable* cancellable, GError** error)
{
    ...
    return g_socket_receive_messages_with_timeout(
        G_SOCKET(self), messages, num_messages, flags, timeout_us, cancellable,
        error);
}
 ...
 * Since: 2.22
 */
GSocket* g_socket_new(GSocketFamily family, GSocketType type,
                      GSocketProtocol protocol, GError** error)
{
    return G_SOCKET(g_initable_new(G_TYPE_SOCKET, NULL, error, "family", family,
...
void g_socket_set_keepalive(GSocket* socket, gboolean keepalive)
{
    ...
    if (!g_socket_set_option(socket, SOL_SOCKET, SO_KEEPALIVE, keepalive,
                             &error))
    {
...
guint g_socket_get_ttl(GSocket* socket)
{
    ...
    {
        g_socket_get_option(socket, IPPROTO_IP, IP_TTL, &value, &error);
    }
    else if (socket->priv->family == G_SOCKET_FAMILY_IPV6)
    {
        g_socket_get_option(socket, IPPROTO_IPV6, IPV6_UNICAST_HOPS, &value,
                            &error);
    }
    else
...
void g_socket_set_ttl(GSocket* socket, guint ttl)
{
    ...
    if (socket->priv->family == G_SOCKET_FAMILY_IPV4)
    {
        g_socket_set_option(socket, IPPROTO_IP, IP_TTL, ttl, &error);
    }
    else if (socket->priv->family == G_SOCKET_FAMILY_IPV6)
    {
        g_socket_set_option(socket, IPPROTO_IP, IP_TTL, ttl, NULL);
        g_socket_set_option(socket, IPPROTO_IPV6, IPV6_UNICAST_HOPS, ttl,
                            &error);
    }
    else
        g_return_if_reached();
...
gboolean g_socket_get_broadcast(GSocket* socket)
{
    ...
    if (!g_socket_get_option(socket, SOL_SOCKET, SO_BROADCAST, &value, &error))
    {
...
void g_socket_set_broadcast(GSocket* socket, gboolean broadcast)
{
    ...
    if (!g_socket_set_option(socket, SOL_SOCKET, SO_BROADCAST, broadcast,
                             &error))
    {
...
gboolean g_socket_get_multicast_loopback(GSocket* socket)
{
    ...
    if (socket->priv->family == G_SOCKET_FAMILY_IPV4)
    {
        g_socket_get_option(socket, IPPROTO_IP, IP_MULTICAST_LOOP, &value,
                            &error);
    }
    else if (socket->priv->family == G_SOCKET_FAMILY_IPV6)
    {
        g_socket_get_option(socket, IPPROTO_IPV6, IPV6_MULTICAST_LOOP, &value,
                            &error);
    }
    else
...
void g_socket_set_multicast_loopback(GSocket* socket, gboolean loopback)
{
    ...
    if (socket->priv->family == G_SOCKET_FAMILY_IPV4)
    {
        g_socket_set_option(socket, IPPROTO_IP, IP_MULTICAST_LOOP, loopback,
                            &error);
    }
    else if (socket->priv->family == G_SOCKET_FAMILY_IPV6)
    {
        g_socket_set_option(socket, IPPROTO_IP, IP_MULTICAST_LOOP, loopback,
                            NULL);
        g_socket_set_option(socket, IPPROTO_IPV6, IPV6_MULTICAST_LOOP, loopback,
                            &error);
    }
    else
        g_return_if_reached();
...
guint g_socket_get_multicast_ttl(GSocket* socket)
{
    ...
    if (socket->priv->family == G_SOCKET_FAMILY_IPV4)
    {
        g_socket_get_option(socket, IPPROTO_IP, IP_MULTICAST_TTL, &value,
                            &error);
    }
    else if (socket->priv->family == G_SOCKET_FAMILY_IPV6)
    {
        g_socket_get_option(socket, IPPROTO_IPV6, IPV6_MULTICAST_HOPS, &value,
                            &error);
    }
    else
        g_return_val_if_reached(FALSE);
...
void g_socket_set_multicast_ttl(GSocket* socket, guint ttl)
{
    ...
    if (socket->priv->family == G_SOCKET_FAMILY_IPV4)
    {
        g_socket_set_option(socket, IPPROTO_IP, IP_MULTICAST_TTL, ttl, &error);
    }
    else if (socket->priv->family == G_SOCKET_FAMILY_IPV6)
    {
        g_socket_set_option(socket, IPPROTO_IP, IP_MULTICAST_TTL, ttl, NULL);
        g_socket_set_option(socket, IPPROTO_IPV6, IPV6_MULTICAST_HOPS, ttl,
                            &error);
    }
    else
        g_return_if_reached();
...
GSocketAddress* g_socket_get_local_address(GSocket* socket, GError** error)
{
    ...
    if (getsockname(socket->priv->fd, &buffer.sa, &len) < 0)
    {
        int errsv = get_socket_errno();
        g_set_error(error, G_IO_ERROR, socket_io_error_from_errno(errsv),
                    _("could not get local address: %s"),
                    socket_strerror(errsv));
        return NULL;
    }
...
GSocketAddress* g_socket_get_remote_address(GSocket* socket, GError** error)
{
    ...
    if (!socket->priv->remote_address)
    {
        if (getpeername(socket->priv->fd, &buffer.sa, &len) < 0)
        {
            int errsv = get_socket_errno();
            g_set_error(error, G_IO_ERROR, socket_io_error_from_errno(errsv),
                        _("could not get remote address: %s"),
                        socket_strerror(errsv));
            return NULL;
        }

        socket->priv->remote_address =
            g_socket_address_new_from_native(&buffer.storage, len);
    }
...
gboolean g_socket_bind(GSocket* socket, GSocketAddress* address,
                       gboolean reuse_address, GError** error)
{
    union
    {
        struct sockaddr_storage storage;
    ...
    g_return_val_if_fail(G_IS_SOCKET(socket) && G_IS_SOCKET_ADDRESS(address),
                         FALSE);
...
static gboolean g_socket_multicast_group_operation(
    GSocket* socket, GInetAddress* group, gboolean source_specific,
    const gchar* iface, gboolean join_group, GError** error)
{
    ...
    if (g_inet_address_get_family(group) == G_SOCKET_FAMILY_IPV4)
    {
        ...
        if (source_specific)
        {
            ...
            optname = join_group ? IP_ADD_SOURCE_MEMBERSHIP
                                 : IP_DROP_SOURCE_MEMBERSHIP;
#else
            g_set_error(error, G_IO_ERROR, G_IO_ERROR_NOT_SUPPORTED,
                        join_group ? _("Error joining multicast group: %s")
        ...
        result = setsockopt(socket->priv->fd, IPPROTO_IP, optname, &mc_req,
                            sizeof(mc_req));
    }
    else if (g_inet_address_get_family(group) == G_SOCKET_FAMILY_IPV6)
    {
        ...
        memcpy(&mc_req_ipv6.ipv6mr_multiaddr, native_addr,
 ...
 * Since: 2.32
 */
gboolean g_socket_join_multicast_group(GSocket* socket, GInetAddress* group,
 ...
 * Since: 2.32
 */
gboolean g_socket_leave_multicast_group(GSocket* socket, GInetAddress* group,
                                        gboolean source_specific,
                                        const gchar* iface, GError** error)
{
    return g_socket_multicast_group_operation(socket, group, source_specific,
...
static gboolean g_socket_multicast_group_operation_ssm(
    GSocket* socket, GInetAddress* group, GInetAddress* source_specific,
    const gchar* iface, gboolean join_group, GError** error)
{
    gint result;
...
#undef S_ADDR_FIELD
        ...
        case G_SOCKET_FAMILY_IPV6:
        {
            ...
            res =
                g_socket_address_to_native(saddr_group, &mc_req_src.gsr_group,
                                           sizeof(mc_req_src.gsr_group), error);
            g_object_unref(saddr_group);
            if (!res)
                return FALSE;

            saddr_source_specific =
                g_inet_socket_address_new(source_specific, 0);
            res = g_socket_address_to_native(
                saddr_source_specific, &mc_req_src.gsr_source,
                sizeof(mc_req_src.gsr_source), error);
 ...
 * Since: 2.56
 */
gboolean g_socket_join_multicast_group_ssm(GSocket* socket, GInetAddress* group,
                                           GInetAddress* source_specific,
                                           const gchar* iface, GError** error)
{
    return g_socket_multicast_group_operation_ssm(
        socket, group, source_specific, iface, TRUE, error);
}
...
{
    return g_socket_multicast_group_operation_ssm(
        socket, group, source_specific, iface, FALSE, error);
}
...
gboolean g_socket_speaks_ipv4(GSocket* socket)
{
    switch (socket->priv->family)
    {
...
#if defined(IPPROTO_IPV6) && defined(IPV6_V6ONLY)
        {
            ...
            if (!g_socket_get_option(socket, IPPROTO_IPV6, IPV6_V6ONLY,
                                     &v6_only, NULL))
                return FALSE;
 ...
 * Since: 2.22
 */
GSocket* g_socket_accept(GSocket* socket, GCancellable* cancellable,
                         GError** error)
{
    GSocket* new_socket;
    gint ret;
    ...
    while (TRUE)
    {
        if ((ret = accept(socket->priv->fd, NULL, 0)) < 0)
        {
...
#endif
            {
                ...
                {
                    if (!g_socket_condition_wait(socket, G_IO_IN, cancellable,
                                                 error))
                        return NULL;
 ...
 * Since: 2.22
 */
gboolean g_socket_connect(GSocket* socket, GSocketAddress* address,
                          GCancellable* cancellable, GError** error)
{
    union
    {
    ...
    if (!g_socket_address_to_native(address, &buffer.storage, sizeof buffer,
                                    error))
        return FALSE;
    ...
    while (1)
    {
                    ...
                    g_socket_address_get_native_size(address)) < 0)
        {
...
#endif
            {
                ...
                if (socket->priv->blocking)
                {
                    if (g_socket_condition_wait(socket, G_IO_OUT, cancellable,
                                                error))
                    {
                        if (g_socket_check_connect_result(socket, error))
                            break;
 ...
 */
static gboolean block_on_timeout(GSocket* socket, GIOCondition condition,
                                 gint64 timeout_us, gint64 start_time,
                                 GCancellable* cancellable, GError** error)
{
...
static gssize g_socket_receive_with_timeout(GSocket* socket, guint8* buffer,
                                            gsize size, gint64 timeout_us,
                                            GCancellable* cancellable,
                                            GError** error)
{
    gssize ret;
 ...
 * Since: 2.22
 */
gssize g_socket_receive(GSocket* socket, gchar* buffer, gsize size,
                        GCancellable* cancellable, GError** error)
{
 ...
 * Since: 2.26
 */
gssize g_socket_receive_with_blocking(GSocket* socket, gchar* buffer,
                                      gsize size, gboolean blocking,
                                      GCancellable* cancellable, GError** error)
{
    return g_socket_receive_with_timeout(socket, (guint8*)buffer, size,
                             ...
                             GCancellable* cancellable, GError** error)
{
    ...
    return g_socket_receive_message(socket, address, &v, 1, NULL, 0, NULL,
                                    cancellable, error);
}
...
static gssize g_socket_send_with_timeout(GSocket* socket, const guint8* buffer,
                                         gsize size, gint64 timeout_us,
                                         GCancellable* cancellable,
                                         GError** error)
{
    ...
    while (1)
    {
        if ((ret = send(socket->priv->fd, (const char*)buffer, size,
 ...
 * Since: 2.22
 */
gssize g_socket_send(GSocket* socket, const gchar* buffer, gsize size,
                     GCancellable* cancellable, GError** error)
{
    return g_socket_send_with_blocking(
        socket, buffer, size, socket->priv->blocking, cancellable, error);
}
 ...
 * Since: 2.26
 */
gssize g_socket_send_with_blocking(GSocket* socket, const gchar* buffer,
                                   gsize size, gboolean blocking,
                                   GCancellable* cancellable, GError** error)
{
                        ...
                        GCancellable* cancellable, GError** error)
{
    ...
    return g_socket_send_message(socket, address, &v, 1, NULL, 0, 0,
                                 cancellable, error);
}
 ...
 * Since: 2.22
 */
gboolean g_socket_shutdown(GSocket* socket, gboolean shutdown_read,
...
gboolean g_socket_close(GSocket* socket, GError** error)
{
    ...
    while (1)
    {
        ...
        if (res == -1)
        {
            ...
            g_set_error(error, G_IO_ERROR, socket_io_error_from_errno(errsv),
                        _("Error closing socket: %s"), socket_strerror(errsv));
            return FALSE;
        }
        break;
    }
...
static gboolean broken_dispatch(GSource* source, GSourceFunc callback,
                                gpointer user_data)
{
    return TRUE;
}

static GSourceFuncs broken_funcs = {NULL, NULL, broken_dispatch, NULL};
...
{
    g_mutex_lock(&socket->priv->win32_source_lock);
    g_assert(g_list_find(socket->priv->requested_conditions, condition) ==
             NULL);
...
static GIOCondition update_condition_unlocked(GSocket* socket)
{
    WSANETWORKEVENTS events;
    GIOCondition condition;

    if (WSAEnumNetworkEvents(socket->priv->fd, socket->priv->event, &events) ==
        0)
    {
...
static gboolean socket_source_prepare(GSource* source, gint* timeout)
{
    ...
    *timeout = -1;
    ...
    return (update_condition(socket_source->socket) &
            socket_source->condition) != 0;
#else
    return g_socket_is_closed(socket_source->socket) &&
           socket_source->fd_tag != NULL;
#endif
}

#ifdef G_OS_WIN32
static gboolean socket_source_check_win32(GSource* source)
{
    int timeout;
...
static gboolean socket_source_dispatch(GSource* source, GSourceFunc callback,
                                       gpointer user_data)
{
...
static gboolean socket_source_closure_callback(GSocket* socket,
                                               GIOCondition condition,
                                               gpointer data)
{
...
static GSource* socket_source_new(GSocket* socket, GIOCondition condition,
                                  GCancellable* cancellable)
{
    GSource* source;
...
#else
    socket_source->fd_tag =
        g_source_add_unix_fd(source, socket->priv->fd, condition);
#endif
 ...
 * Since: 2.22
 */
GSource* g_socket_create_source(GSocket* socket, GIOCondition condition,
                                GCancellable* cancellable)
{
    g_return_val_if_fail(G_IS_SOCKET(socket) && (cancellable == NULL ||
 ...
 * Since: 2.22
 */
gboolean g_socket_condition_wait(GSocket* socket, GIOCondition condition,
                                 GCancellable* cancellable, GError** error)
{
    g_return_val_if_fail(G_IS_SOCKET(socket), FALSE);

    return g_socket_condition_timed_wait(socket, condition, -1, cancellable,
                                         error);
}
 ...
 * Since: 2.32
 */
gboolean g_socket_condition_timed_wait(GSocket* socket, GIOCondition condition,
                                       gint64 timeout_us,
                                       GCancellable* cancellable,
                                       GError** error)
{
...
#ifdef G_OS_WIN32
    {
        ...
        while ((condition & current_condition) == 0)
        {
            if (!socket->priv->waiting)
            {
                ...
                res = WSAWaitForMultipleEvents(num_events, events, FALSE,
                                               timeout_ms, FALSE);
            ...
            else
            {
                ...
                {
                    if (!g_cond_wait_until(&socket->priv->win32_source_cond,
                                           &socket->priv->win32_source_lock,
                                           timeout_ms))
                    {
                ...
                {
                    g_cond_wait(&socket->priv->win32_source_cond,
                                &socket->priv->win32_source_lock);
                    res = socket->priv->waiting_result;
                }
            }
...
#define output_message_to_msghdr(message, prev_message, msg, prev_msg, error)  \
    ...
    {                                                                          \
        ...
        _msg->msg_flags = 0;                                                   \
                                                                               \
        /* name */                                                             \
        if (_prev_message != NULL &&                                           \
            _prev_message->address == _message->address)                       \
        {                                                                      \
        ...
        {                                                                      \
            _msg->msg_namelen =                                                \
                g_socket_address_get_native_size(_message->address);           \
        ...
        {                                                                      \
            ...
            if (sizeof *_msg->msg_iov == sizeof *_message->vectors &&          \
                sizeof _msg->msg_iov->iov_base ==                              \
                    sizeof _message->vectors->buffer &&                        \
                G_STRUCT_OFFSET(struct iovec, iov_base) ==                     \
                    G_STRUCT_OFFSET(GOutputVector, buffer) &&                  \
                sizeof _msg->msg_iov->iov_len ==                               \
                    sizeof _message->vectors->size &&                          \
            ...
            {                                                                  \
                ...
                {                                                              \
                    _msg->msg_iov[i].iov_base =                                \
                        (void*)_message->vectors[i].buffer;                    \
        ...
        {                                                                      \
            ...
            for (i = 0; i < _message->num_control_messages; i++)               \
                _msg->msg_controllen +=                                        \
                    CMSG_SPACE(g_socket_control_message_get_size(              \
            ...
            {                                                                  \
                ...
                cmsg->cmsg_type = g_socket_control_message_get_msg_type(       \
                    _message->control_messages[i]);                            \
                cmsg->cmsg_len = CMSG_LEN(g_socket_control_message_get_size(   \
                    _message->control_messages[i]));                           \
                g_socket_control_message_serialize(                            \
                    _message->control_messages[i], CMSG_DATA(cmsg));           \
...
static void input_message_from_msghdr(const struct msghdr* msg,
                                      GInputMessage* message, GSocket* socket)
{
    /* decode address */
    if (message->address != NULL)
    {
        *message->address =
            cache_recv_address(socket, msg->msg_name, msg->msg_namelen);
    }
    ...
    /* decode control messages */
    {
        ...
        if (message->num_control_messages)
            *message->num_control_messages =
                my_messages != NULL ? my_messages->len : 0;
        ...
        if (message->control_messages)
        {
            ...
            else
            {
                ...
                *message->control_messages =
                    (GSocketControlMessage**)g_ptr_array_free(my_messages,
                                                              FALSE);
            }
        }
        else
 ...
 * Since: 2.22
 */
gssize g_socket_send_message(GSocket* socket, GSocketAddress* address,
                             GOutputVector* vectors, gint num_vectors,
                             ...
                             GCancellable* cancellable, GError** error)
{
    ...
    res = g_socket_send_message_with_timeout(
        socket, address, vectors, num_vectors, messages, num_messages, flags,
        socket->priv->blocking ? -1 : 0, &bytes_written, cancellable, error);

    if (res == G_POLLABLE_RETURN_WOULD_BLOCK)
    {
#ifndef G_OS_WIN32
        socket_set_error_lazy(error, EWOULDBLOCK,
                              _("Error sending message: %s"));
#else
        socket_set_error_lazy(error, WSAEWOULDBLOCK,
                              _("Error sending message: %s"));
#endif
    }
 ...
 * @flags: an int containing #GSocketMsgFlags flags
 * @timeout_us: the maximum time (in microseconds) to wait, or -1
 * @bytes_written: (out) (optional): location to store the number of bytes that
 * were written to the socket
 * @cancellable: (nullable): a %GCancellable or %NULL
 * @error: #GError for error reporting, or %NULL to ignore.
 *
 ...
 * Since: 2.60
 */
GPollableReturn g_socket_send_message_with_timeout(
    GSocket* socket, GSocketAddress* address, const GOutputVector* vectors,
    gint num_vectors, GSocketControlMessage** messages, gint num_messages,
    gint flags, gint64 timeout_us, gsize* bytes_written,
    GCancellable* cancellable, GError** error)
{
    ...
    g_return_val_if_fail(address == NULL || G_IS_SOCKET_ADDRESS(address),
                         G_POLLABLE_RETURN_FAILED);
    g_return_val_if_fail(num_vectors == 0 || vectors != NULL,
                         G_POLLABLE_RETURN_FAILED);
    g_return_val_if_fail(num_messages == 0 || messages != NULL,
                         G_POLLABLE_RETURN_FAILED);
    ...
    if (num_vectors == -1)
    {
        for (num_vectors = 0; vectors[num_vectors].buffer != NULL;
             num_vectors++)
            ;
    }
    ...
    {
        for (num_messages = 0;
             messages != NULL && messages[num_messages] != NULL; num_messages++)
            ;
    }
...
#ifndef G_OS_WIN32
    {
        ...
        output_message_to_msghdr(&output_message, NULL, &msg, NULL,
                                 &child_error);
        ...
        while (1)
        {
            result = sendmsg(socket->priv->fd, &msg,
...
#else
    {
        ...
        while (1)
        {
            if (address)
                result = WSASendTo(
                    socket->priv->fd, bufs, num_vectors, &bytes_sent, flags,
                    (const struct sockaddr*)&addr, addrlen, NULL, NULL);
            else
                result = WSASend(socket->priv->fd, bufs, num_vectors,
...
gint g_socket_send_messages(GSocket* socket, GOutputMessage* messages,
                            guint num_messages, gint flags,
                            GCancellable* cancellable, GError** error)
{
    return g_socket_send_messages_with_timeout(
        socket, messages, num_messages, flags, socket->priv->blocking ? -1 : 0,
        cancellable, error);
}
...
#define MAX_NUM_MESSAGES 1024
        ...
        for (i = 0; i < num_messages; ++i)
        {
            ...
            output_message_to_msghdr(
                msg, (i > 0) ? &messages[i - 1] : NULL, msg_hdr,
                (i > 0) ? &msgvec[i - 1].msg_hdr : NULL, &child_error);
        ...
        for (num_sent = 0; num_sent < num_messages;)
        {
            ...
            ret = sendmmsg(socket->priv->fd, msgvec + num_sent,
                           num_messages - num_sent,
                           flags | G_SOCKET_DEFAULT_SEND_FLAGS);
            ...
            if (ret < 0)
            {
                ...
                if (timeout_us != 0 &&
                    (errsv == EWOULDBLOCK || errsv == EAGAIN))
                {
                    if (!block_on_timeout(socket, G_IO_OUT, timeout_us,
                                          start_time, cancellable, error))
                    {
...
#else
    {
        ...
        for (i = 0; i < num_messages; ++i)
        {
            ...
            pollable_result = g_socket_send_message_with_timeout(
                socket, msg->address, msg->vectors, msg->num_vectors,
                msg->control_messages, msg->num_control_messages, flags,
                wait_timeout, &bytes_written, cancellable, &msg_error);

            if (pollable_result == G_POLLABLE_RETURN_WOULD_BLOCK)
            {
#ifndef G_OS_WIN32
                socket_set_error_lazy(&msg_error, EWOULDBLOCK,
                                      _("Error sending message: %s"));
#else
                socket_set_error_lazy(&msg_error, WSAEWOULDBLOCK,
                                      _("Error sending message: %s"));
#endif
            }

            result =
                pollable_result == G_POLLABLE_RETURN_OK ? bytes_written : -1;
    ...
    cache_recv_address(GSocket* socket, struct sockaddr* native, int native_len)
{
    ...
    socket->priv->recv_addr_cache[oldest_index].native =
        g_memdup(native, native_len);
    ...
    gint* flags, gint64 timeout_us, GCancellable* cancellable, GError** error)
{
...
#else
    {
        ...
        while (1)
        {
            ...
            if (address)
                result = WSARecvFrom(
                    socket->priv->fd, bufs, num_vectors, &bytes_received,
                    &win_flags, (struct sockaddr*)&addr, &addrlen, NULL, NULL);
            else
                result = WSARecv(socket->priv->fd, bufs, num_vectors,
                                 &bytes_received, &win_flags, NULL, NULL);
            if (result != 0)
            {
                int errsv = get_socket_errno();
        ...
        {
            *address =
                cache_recv_address(socket, (struct sockaddr*)&addr, addrlen);
        }
 ...
 * Since: 2.48
 */
gint g_socket_receive_messages(GSocket* socket, GInputMessage* messages,
                               guint num_messages, gint flags,
                               GCancellable* cancellable, GError** error)
{
    if (!check_socket(socket, error) || !check_timeout(socket, error))
        return -1;
...
#define MAX_NUM_MESSAGES 1024
        ...
        for (num_received = 0; num_received < num_messages;)
        {
             ...
             */
            ret = recvmmsg(socket->priv->fd, msgvec + num_received,
...
#else
    {
        ...
        for (i = 0; i < num_messages; i++)
        {
            ...
            len = g_socket_receive_message_with_timeout(
                socket, msg->address, msg->vectors, msg->num_vectors,
                msg->control_messages, (gint*)msg->num_control_messages,
                &msg->flags, wait_timeout, cancellable, &msg_error);
            ...
            if (i != 0 &&
                (g_error_matches(msg_error, G_IO_ERROR,
 ...
 * Since: 2.22
 */
gssize g_socket_receive_message(GSocket* socket, GSocketAddress** address,
                                GInputVector* vectors, gint num_vectors,
                                GSocketControlMessage*** messages,
                                gint* num_messages, gint* flags,
                                GCancellable* cancellable, GError** error)
{
    return g_socket_receive_message_with_timeout(
        socket, address, vectors, num_vectors, messages, num_messages, flags,
        socket->priv->blocking ? -1 : 0, cancellable, error);
}
 ...
 *
 * Returns: (transfer full): %NULL if @error is set, otherwise a #GCredentials
 * object that must be freed with g_object_unref().
 *
 * Since: 2.26
 */
GCredentials* g_socket_get_credentials(GSocket* socket, GError** error)
{
...
#ifdef SO_PEERCRED
    {
        ...
        if (getsockopt(socket->priv->fd, SOL_SOCKET, SO_PEERCRED,
                       native_creds_buf, &optlen) == 0)
        {
            ret = g_credentials_new();
            g_credentials_set_native(ret, G_CREDENTIALS_NATIVE_TYPE,
                                     native_creds_buf);
        }
    }
#elif G_CREDENTIALS_USE_NETBSD_UNPCBID
    {
        ...
        if (getsockopt(socket->priv->fd, 0, LOCAL_PEEREID, &cred, &optlen) == 0)
        {
            ret = g_credentials_new();
            g_credentials_set_native(ret, G_CREDENTIALS_NATIVE_TYPE, &cred);
        }
    }
#elif G_CREDENTIALS_USE_SOLARIS_UCRED
    {
        ...
        {
            ret = g_credentials_new();
            g_credentials_set_native(ret, G_CREDENTIALS_TYPE_SOLARIS_UCRED,
                                     ucred);
            ucred_free(ucred);
        }
    }
#else
#error                                                                         \
    "G_CREDENTIALS_SOCKET_GET_CREDENTIALS_SUPPORTED is set but this is no code for this platform"
#endif
 ...
 * Since: 2.36
 */
gboolean g_socket_get_option(GSocket* socket, gint level, gint optname,
 ...
 * Since: 2.36
 */
gboolean g_socket_set_option(GSocket* socket, gint level, gint optname,
                             gint value, GError** error)
{
    gint errsv;
    ...
    g_set_error_literal(error, G_IO_ERROR, socket_io_error_from_errno(errsv),
                        socket_strerror(errsv));
#ifndef G_OS_WIN32
    errno = errsv;
#endif
