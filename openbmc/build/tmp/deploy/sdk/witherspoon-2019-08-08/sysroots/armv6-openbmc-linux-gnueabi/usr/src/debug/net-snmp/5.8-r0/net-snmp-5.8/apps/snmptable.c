                ...
                switch (*optarg++)
                {
                    ...
                    case 'c':
                        ...
                        else
                        {
                            usage();
                            fprintf(stderr,
                                    "Bad -Cc option: no argument given\n");
                            exit(1);
                        }
                        optind++;
                        break;
...
void get_table_entries(netsnmp_session* ss)
{
    ...
    while (running && ((max_width && !column_width) || (entries < max_getbulk)))
    {
        /*
            ...
            if (response->errstat == SNMP_ERR_NOERROR)
            {
                ...
                if (entries >= allocated)
                {
                    if (allocated == 0)
                    {
                        allocated = 10;
                        data =
                            (char**)malloc(allocated * fields * sizeof(char*));
                     ...
                     vars = vars->next_variable)
                {
                    ...
                    if ((vars->name_length < name_length) ||
                        ...
                        memcmp(name, vars->name, name_length * sizeof(oid)) !=
                            0 ||
                    ...
                    if (!have_current_index)
                    {
                        ...
                        out_len = 0;
                        if (!sprint_realloc_objid((u_char**)&buf, &buf_len,
                                                  &out_len, 1, vars->name,
                                                  vars->name_length))
                        {
                            break;
                ...
                while (vars)
                {
                    ...
                    if (row == entries)
                    {
                        ...
                        if (entries >= allocated)
                        {
                            ...
                            {
                                allocated = 10;
                                data = (char**)malloc(allocated * fields *
                                                      sizeof(char*));
                                memset(data, 0,
                                       allocated * fields * sizeof(char*));
                                indices =
                                    (char**)malloc(allocated * sizeof(char*));
                            }
                            else
                            {
                                allocated += 10;
                                data = (char**)realloc(
                                    data, allocated * fields * sizeof(char*));
                                memset(data + entries * fields, 0,
