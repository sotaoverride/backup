...
static void generic_write(int fd, void* buf, int blocksize, blk64_t block)
{
    ...
    if (!buf)
    {
        ...
        if (err)
        {
            com_err(program_name, err, "%s", _("while allocating buffer"));
            exit(1);
        }
    }
    if (nop_flag)
    ...
    if (count != blocksize)
    {
        ...
        if (block)
            com_err(program_name, err, _("error writing block %llu"), block);
        else
            com_err(program_name, err, "%s", _("error in generic_write()"));
...
static void write_image_file(ext2_filsys fs, int fd)
{
    ...
    if (retval)
    {
        com_err(program_name, retval, "%s", _("while writing superblock"));
        exit(1);
    }
    ...
    if (retval)
    {
        com_err(program_name, retval, "%s", _("while writing inode table"));
        exit(1);
    }
    ...
    if (retval)
    {
        com_err(program_name, retval, "%s", _("while writing block bitmap"));
        exit(1);
    }
    ...
    if (retval)
    {
        com_err(program_name, retval, "%s", _("while writing inode bitmap"));
        exit(1);
    }
...
static ext2fs_block_bitmap
    scramble_block_map; /* Directory blocks to be scrambled */
...
static void mark_table_blocks(ext2_filsys fs)
{
    ...
    for (j = 0; j < fs->desc_blocks; j++)
    {
        ext2fs_mark_block_bitmap2(
            meta_block_map, ext2fs_descriptor_block_loc2(fs, first_block, j));
    }
    ...
    for (i = 0; i < fs->group_desc_count; i++)
    {
            ...
            ext2fs_inode_table_loc(fs, i))
        {
            ...
            for (j = 0, b = ext2fs_inode_table_loc(fs, i); j < end; j++, b++)
            {
...
#define calc_percent(a, b)                                                     \
    ((int)((100.0 * (((float)(a)) / ((float)(b)))) + 0.5))
...
static void output_meta_data_blocks(ext2_filsys fs, int fd, int flags)
{
    ...
    if (retval)
    {
        com_err(program_name, retval, "%s", _("while allocating buffer"));
        exit(1);
    }
...
more_blocks:
    ...
    for (blk = start; blk < end; blk++)
    {
        ...
        if (show_progress && last_update != time(NULL))
        {
            ...
            {
                time_t est =
                    (duration * meta_blocks_count / total_written) - duration;
                char buff[30];
                strftime(buff, 30, "%T", gmtime(&est));
                bscount +=
                    fprintf(stderr, _(" %s remaining at %.2f MB/s"), buff,
            ...
            ext2fs_test_block_bitmap2(meta_block_map, blk))
        {
            ...
            if (retval)
            {
                com_err(program_name, retval, _("error reading block %llu"),
                        blk);
            }
            total_written++;
    ...
    if (sparse)
    {
        ...
        if (distance)
            offset =
                seek_set(fd, fs->blocksize * ext2fs_blocks_count(fs->super) +
                                 dest_offset);
        else
...
static void init_l1_table(struct ext2_qcow2_image* image)
{
    ...
    if (ret)
    {
        com_err(program_name, ret, "%s", _("while allocating l1 table"));
        exit(1);
    }
...
static void init_l2_cache(struct ext2_qcow2_image* image)
{
    ...
    ret = ext2fs_get_arrayzero(1, sizeof(struct ext2_qcow2_l2_cache), &cache);
    if (ret)
        goto alloc_err;
    ...
    {
        ret =
            ext2fs_get_arrayzero(1, sizeof(struct ext2_qcow2_l2_table), &table);
        if (ret)
            goto alloc_err;

        ret = ext2fs_get_arrayzero(image->l2_size, sizeof(__u64), &table->data);
        if (ret)
            goto alloc_err;
...
again:
    ...
    {
        fprintf(stderr, "%s",
                _("Warning: There are still tables in "
...
static int init_refcount(struct ext2_qcow2_image* img, blk64_t table_offset)
{
    ...
    ret = ext2fs_get_arrayzero(ref->refcount_table_clusters, img->cluster_size,
                               &ref->refcount_table);
    if (ret)
        return ret;
...
static void flush_l2_cache(struct ext2_qcow2_image* image)
{
    ...
    while (cache->free < cache->count)
    {
        ...
        generic_write(fd, (char*)table->data, image->cluster_size, NO_BLK);
...
static int add_l2_item(struct ext2_qcow2_image* img, blk64_t blk, blk64_t data,
                       blk64_t next)
{
    struct ext2_qcow2_l2_cache* cache = img->l2_cache;
...
static int update_refcount(int fd, struct ext2_qcow2_image* img, blk64_t offset,
                           blk64_t rfblk_pos)
{
    struct ext2_qcow2_refcount* ref;
    __u32 table_index;
...
static int sync_refcount(int fd, struct ext2_qcow2_image* img)
{
    ...
    seek_set(fd, ref->refcount_block_offset);
    generic_write(fd, (char*)ref->refcount_block, img->cluster_size, NO_BLK);
    return 0;
}
...
static void output_qcow2_meta_data_blocks(ext2_filsys fs, int fd)
{
    ...
    header_size =
        align_offset(sizeof(struct ext2_qcow2_hdr), img->cluster_size);
    ...
    for (blk = 0; blk < ext2fs_blocks_count(fs->super); blk++)
    {
            ...
            ext2fs_test_block_bitmap2(meta_block_map, blk))
        {
            ...
            if (add_l2_item(img, blk, offset, offset + img->cluster_size))
            {
                offset += img->cluster_size;
...
static void write_raw_image_file(ext2_filsys fs, int fd, int type, int flags)
{
    ...
    if (retval)
    {
        com_err(program_name, retval, "%s", _("while allocating block bitmap"));
        exit(1);
    }
    ...
    if (retval)
    {
        com_err(program_name, retval, "%s", _("while opening inode scan"));
        exit(1);
    }
    ...
    if (retval)
    {
        com_err(program_name, 0, "%s", _("Can't allocate block buffer"));
        exit(1);
    }
    ...
    while (1)
    {
        ...
        if (retval)
        {
            com_err(program_name, retval, "%s", _("while getting next inode"));
            exit(1);
        }
        if (ino == 0)
            break;
                ...
                inode.i_block[EXT2_TIND_BLOCK] || all_data)
            {
                retval =
                    ext2fs_block_iterate3(fs, ino, BLOCK_FLAG_READ_ONLY,
                                          block_buf, process_file_block, &pb);
                if (retval)
                {
                    com_err(program_name, retval,
                            _("while iterating over inode %u"), ino);
...
static void install_image(char* device, char* image_fn, int type)
{
    ...
    ext2_filsys fs;
    int open_flag =
        EXT2_FLAG_IMAGE_FILE | EXT2_FLAG_64BITS | EXT2_FLAG_IGNORE_CSUM_ERRORS;
    int fd = 0;
    io_manager io_ptr;
    io_channel io;
    ...
    retval = ext2fs_open(image_fn, open_flag, 0, 0, io_ptr, &fs);
    if (retval)
    {
        com_err(program_name, retval, _("while trying to open %s"), image_fn);
        exit(1);
    }
    ...
    if (retval)
    {
        com_err(image_fn, 0, "%s", _("while restoring the image table"));
        exit(1);
    }
...
int main(int argc, char** argv)
{
    ...
    if (optind == argc - 1 && (source_offset || dest_offset))
        move_mode = 1;
    ...
    {
        com_err(program_name, 0, "%s", _("Move mode requires all data mode."));
        exit(1);
    }
    ...
    if (img_type && !ignore_rw_mount && (mount_flags & EXT2_MF_MOUNTED) &&
