...
static void delete_file(e2fsck_t ctx, ext2_ino_t ino, struct dup_inode* dp,
                        char* block_buf);
static errcode_t clone_file(e2fsck_t ctx, ext2_ino_t ino, struct dup_inode* dp,
                            char* block_buf);
                     ...
                     struct ext2_inode_large* inode)
{
    ...
    {
        db = (struct dup_cluster*)e2fsck_allocate_memory(
            ctx, sizeof(struct dup_cluster), "duplicate cluster header");
        db->num_bad = 0;
        db->inode_list = 0;
        dict_alloc_insert(&clstr_dict, INT_TO_VOIDPTR(cluster), db);
    }
    ino_el = (struct inode_el*)e2fsck_allocate_memory(
        ctx, sizeof(struct inode_el), "inode element");
    ino_el->inode = ino;
    ...
    else
    {
        di = (struct dup_inode*)e2fsck_allocate_memory(
            ctx, sizeof(struct dup_inode), "duplicate inode header");
    ...
    cluster_el = (struct cluster_el*)e2fsck_allocate_memory(
        ctx, sizeof(struct cluster_el), "cluster element");
 ...
 */
static void inode_dnode_free(dnode_t* node, void* context EXT2FS_ATTR((unused)))
{
    struct dup_inode* di;
...
void e2fsck_pass1_dupblocks(e2fsck_t ctx, char* block_buf)
{
    ...
    pctx.errcode = e2fsck_allocate_inode_bitmap(
        fs, _("multiply claimed inode map"), EXT2FS_BMAP64_RBTREE,
        "inode_dup_map", &inode_dup_map);
    if (pctx.errcode)
    {
        ...
        (ctx->options & E2F_OPT_UNSHARE_BLOCKS))
    {
        ...
        int result = ext2fs_find_first_set_block_bitmap2(
            ctx->block_dup_map, ctx->fs->super->s_first_data_block,
            ext2fs_blocks_count(ctx->fs->super) - 1, &next);
...
static void pass1b(e2fsck_t ctx, char* block_buf)
{
    ...
    while (1)
    {
        ...
        pctx.errcode = ext2fs_get_next_inode_full(
            scan, &ino, EXT2_INODE(&inode), sizeof(inode));
        ...
        if (ext2fs_inode_has_valid_blocks2(fs, EXT2_INODE(&inode)) ||
            (ino == EXT2_BAD_INO))
            pctx.errcode =
                ext2fs_block_iterate3(fs, ino, BLOCK_FLAG_READ_ONLY, block_buf,
            ...
            ext2fs_file_acl_block(fs, EXT2_INODE(&inode)))
        {
            ...
            process_pass1b_block(fs, &blk, BLOCK_COUNT_EXTATTR, 0, 0, &pb);
        ...
        if (pb.dup_blocks)
        {
            if (ino != EXT2_BAD_INO)
            {
                op = pctx.blk == pctx.blk2 ? PR_1B_DUP_BLOCK : PR_1B_DUP_RANGE;
                fix_problem(ctx, op, pb.pctx);
            }
            end_problem_latch(ctx, PR_LATCH_DBLOCK);
            if (ino >= EXT2_FIRST_INODE(fs->super) || ino == EXT2_ROOT_INO)
                dup_inode_count++;
        }
        if (pctx.errcode)
                                ...
                                void* priv_data)
{
    ...
    if (p->ino != EXT2_BAD_INO)
    {
        if (p->last_blk + 1 != *block_nr)
        {
            if (p->last_blk)
            {
                op = p->pctx->blk == p->pctx->blk2 ? PR_1B_DUP_BLOCK
                                                   : PR_1B_DUP_RANGE;
...
static void pass1c(e2fsck_t ctx, char* block_buf)
{
    ...
    ext2fs_dblist_dir_iterate(fs->dblist, 0, block_buf, search_dirent_proc,
                              &sd);
}
...
static void pass1d(e2fsck_t ctx, char* block_buf)
{
    ...
    shared = (ext2_ino_t*)e2fsck_allocate_memory(
        ctx, sizeof(ext2_ino_t) * dict_count(&ino_dict), "Shared inode list");
    for (n = dict_first(&ino_dict); n; n = dict_next(&ino_dict, n))
    {
        ...
        {
            m = dict_lookup(&clstr_dict, INT_TO_VOIDPTR(s->cluster));
            if (!m)
...
{
    p->num_bad--;
    if (p->num_bad <= 0 || (p->num_bad == 1 && !check_if_fs_block(ctx, block)))
    {
...
static int delete_file_block(ext2_filsys fs, blk64_t* block_nr,
                        ...
                        char* block_buf)
{
    ...
    quota_data_sub(ctx->qctx, &dp->inode, ino, pb.dup_blocks * fs->blocksize);
    quota_data_inodes(ctx->qctx, &dp->inode, ino, -1);

    /* Inode may have changed by block_iterate, so reread it */
    e2fsck_read_inode_full(ctx, ino, EXT2_INODE(&dp->inode), sizeof(dp->inode),
                           "delete_file");
        ...
        ext2fs_has_feature_xattr(fs->super))
    {
        ...
        count = 1;
        pctx.errcode = ext2fs_adjust_ea_refcount3(fs, file_acl_block, block_buf,
                                                  -1, &count, ino);
         ...
         */
        if ((count == 0) ||
            ext2fs_test_block_bitmap2(ctx->block_dup_map, file_acl_block))
        {
            delete_file_block(fs, &file_acl_block, BLOCK_COUNT_EXTATTR, 0, 0,
                              &pb);
                            ...
                            void* priv_data)
{
    ...
    if (ext2fs_test_block_bitmap2(ctx->block_dup_map, *block_nr))
    {
        n = dict_lookup(&clstr_dict, INT_TO_VOIDPTR(EXT2FS_B2C(fs, *block_nr)));
        if (!n)
        {
        ...
        if (retval == 0 && new_block != 0 &&
            EXT2FS_B2C(ctx->fs, new_block) != EXT2FS_B2C(ctx->fs, *block_nr))
    ...
    got_block:
        ...
        {
            retval =
                ext2fs_set_dir_block2(fs->dblist, cs->dir, new_block, blockcnt);
            if (retval)
            {
                cs->errcode = retval;
                return BLOCK_ABORT;
                            ...
                            char* block_buf)
{
    ...
    if (ext2fs_inode_has_valid_blocks2(fs, EXT2_INODE(&dp->inode)))
        pctx.errcode =
            ext2fs_block_iterate3(fs, ino, 0, block_buf, clone_file_block, &cs);
    deferred_dec_badcount(&cs);
    ...
    new_blk = blk;
    if (blk && (clone_file_block(fs, &new_blk, BLOCK_COUNT_EXTATTR, 0, 0,
                                 &cs) == BLOCK_CHANGED))
    {
         ...
         */
        n = dict_lookup(&clstr_dict, INT_TO_VOIDPTR(EXT2FS_B2C(fs, blk)));
        if (!n)
        {
        ...
        for (ino_el = dc->inode_list; ino_el; ino_el = ino_el->next)
        {
            ...
            di = (struct dup_inode*)dnode_get(n);
            if (ext2fs_file_acl_block(fs, EXT2_INODE(&di->inode)) == blk)
            {
                ext2fs_file_acl_block_set(
                    fs, EXT2_INODE(&di->inode),
                    ext2fs_file_acl_block(fs, EXT2_INODE(&dp->inode)));
    ...
    for (i = 0; i < fs->group_desc_count; i++)
    {
        ...
        if (ext2fs_bg_has_super(fs, i))
        {
            if (cluster >= EXT2FS_B2C(fs, first_block) &&
                (cluster <= EXT2FS_B2C(fs, first_block + fs->desc_blocks)))
                return 1;
        }

        /* Check the inode table */
        if ((ext2fs_inode_table_loc(fs, i)) &&
            (cluster >= EXT2FS_B2C(fs, ext2fs_inode_table_loc(fs, i))) &&
            (cluster <= EXT2FS_B2C(fs, ext2fs_inode_table_loc(fs, i) +
                                           fs->inode_blocks_per_group - 1)))
            return 1;

        /* Check the bitmap blocks */
        if ((cluster == EXT2FS_B2C(fs, ext2fs_block_bitmap_loc(fs, i))) ||
            (cluster == EXT2FS_B2C(fs, ext2fs_inode_bitmap_loc(fs, i))))
            return 1;
