...
void __backtrace_symbols_fd(void* const* array, int size, int fd)
{
    ...
    for (cnt = 0; cnt < size; ++cnt)
    {
        ...
        if (_dl_addr(array[cnt], &info, &map, NULL) && info.dli_fname != NULL &&
            info.dli_fname[0] != '\0')
        {
            /* Name of the file.  */
            ...
            if (info.dli_sname != NULL || map->l_addr != 0)
            {
                ...
                if (info.dli_sname != NULL)
                {
                    ...
                    /* We have no symbol, so describe it as relative to the
                       file. The load bias is more useful to the user than the
                       load address.  The use of these addresses is to calculate
                       an address in the ELF file, so its prelinked bias is not
                ...
                iov[last].iov_len =
                    (&buf2[WORD_WIDTH] - (char*)iov[last].iov_base);
                ++last;
        ...
        iov[last].iov_base =
            _itoa_word((unsigned long int)array[cnt], &buf[WORD_WIDTH], 16, 0);
