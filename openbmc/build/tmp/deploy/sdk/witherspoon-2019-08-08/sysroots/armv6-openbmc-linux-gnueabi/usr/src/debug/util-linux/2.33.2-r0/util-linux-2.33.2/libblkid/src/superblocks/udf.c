...
#define udf_cid_to_enc(cid)                                                    \
    ((cid) == 8 ? BLKID_ENC_LATIN1 : (cid) == 16 ? BLKID_ENC_UTF16BE : -1)
...
#define UDF_LVIDIU_OFFSET(vd)                                                  \
    (sizeof((vd).tag) + sizeof((vd).type.logical_vol_integ) +                  \
     2 * 4 * le32_to_cpu((vd).type.logical_vol_integ.num_partitions))
#define UDF_LVIDIU_LENGTH(vd)                                                  \
    (le32_to_cpu((vd).type.logical_vol_integ.imp_use_length))

static inline int gen_uuid_from_volset_id(unsigned char uuid[17],
                                          struct dstring128* volset_id)
{
    int enc;
    size_t i;
    ...
    if (nonhexpos < 8)
    {
        snprintf((char*)uuid, 17, "%02x%02x%02x%02x%02x%02x%02x%02x", buf[0],
    ...
    else if (nonhexpos < 16)
    {
        ...
        snprintf((char*)uuid + 8, 9, "%02x%02x%02x%02x", buf[8], buf[9],
                 buf[10], buf[11]);
    }
    else
    {
                     ...
                     const struct blkid_idmag* mag __attribute__((__unused__)))
{
    ...
    for (i = 0; i < ARRAY_SIZE(pbs); i++)
    {
        ...
        for (b = 0; b < 64; b++)
        {
            vsd = (struct volume_structure_descriptor*)blkid_probe_get_buffer(
                pr, UDF_VSD_OFFSET + b * vsd_len, sizeof(*vsd));
            if (!vsd)
    ...
    anchor:
        ...
        vd = (struct volume_descriptor*)blkid_probe_get_buffer(pr, 256 * pbs[i],
                                                               sizeof(*vd));
        if (!vd)
            return errno ? -errno : 1;
...
real_blksz:
    ...
    for (b = 0; b < count; b++)
    {
        vd = (struct volume_descriptor*)blkid_probe_get_buffer(
            pr, (uint64_t)(loc + b) * bs, sizeof(*vd));
        if (!vd)
            return errno ? -errno : 1;
        type = le16_to_cpu(vd->tag.id);
        if (type == 0)
        ...
        if (type == TAG_ID_PVD)
        {
            if (!have_volid)
            {
                ...
                if (enc != -1)
                    have_volid = !blkid_probe_set_utf8_id_label(
                        pr, "VOLUME_ID", vd->type.primary.ident.c, clen, enc);
            }
            if (!have_uuid)
            {
                 ...
                 *
                 * The first 16 characters of this field should be set to a
                 * unique value. The remainder of the field may be set to any
                 * allowed value. Specifically, software generating volumes
                 * conforming to this specification shall not set this field to
                 * a fixed or trivial value. Duplicate disks which are intended
                 * to be identical may contain the same value in this field.
                 *
                 * NOTE: The intended purpose of this is to guarantee Volume
                 * Sets with unique identifiers. The first 8 characters of the
                 * unique part should come from a CS0 hexadecimal representation
                 * of a 32-bit time value. The remaining 8 characters are free
                 * for implementation use.
                 * =================================================================================
                 *
                 * Implementation in libblkid:
                 * The first 16 (Unicode) characters of VolumeSetIdentifier are
                 * encoded to UTF-8 and then first 16 UTF-8 bytes are used to
                 * generate UUID. If all 16 bytes are hexadecimal digits then
                 * their lowercase variants are used as UUID. If one of the
                 * first 8 bytes (time value) is not hexadecimal digit then
                 * first 8 bytes are encoded to their hexadecimal
                 * representations, resulting in 16 characters and set as UUID.
                 * If all first 8 bytes (time value) are hexadecimal digits but
                 * some remaining not then lowercase variant of the first 8
                 * bytes are used as first part of UUID and next 4 bytes encoded
                 * in hexadecimal representations (resulting in 8 characters)
                 * are used as second part of UUID string.
                 */
                unsigned char uuid[17];
                if (gen_uuid_from_volset_id(uuid,
                                            &vd->type.primary.volset_id) == 0)
                    have_uuid =
            ...
            if (!have_volsetid)
            {
                ...
                if (enc != -1)
                    have_volsetid = !blkid_probe_set_utf8_id_label(
                        pr, "VOLUME_SET_ID", vd->type.primary.volset_id.c, clen,
                        enc);
            }
        }
        else if (type == TAG_ID_LVD)
        {
            ...
            {
                uint32_t num_partition_maps =
                    le32_to_cpu(vd->type.logical.num_partition_maps);
                /* ECMA-167 3/10.6.12: If num_partition_maps is 0, then no LVID
                 * is specified */
            ...
            if (!udf_rev)
            {
                /* UDF-2.60: 2.1.5.3: UDF revision field shall indicate revision
                 * of UDF document We use this field as fallback value for
                 * ID_FS_VERSION when LVIDIU is missing */
                if (strncmp(vd->type.logical.domain_id, "*OSTA UDF Compliant",
                            sizeof(vd->type.logical.domain_id)) == 0)
            ...
            if (!have_logvolid || !have_label)
            {
                 ...
                 * should contain an identifier that aids in the identification
                 * of the logical volume. Specifically, software generating
                 ...
                 *
                 * Implementation in libblkid:
                 * The LogicalVolumeIdentifier field is used for LABEL. MS
                 * Windows read Volume Label also from LogicalVolumeIdentifier.
                 * Grub2 read LABEL also from this field. Program newfs_udf
                 * (from UDFclient) when formatting disk set this field from
                 * user option Disc Name.
                 */
                ...
                {
                    if (!have_label)
                        have_label = !blkid_probe_set_utf8label(
                            pr, vd->type.logical.logvol_id.c, clen, enc);
                    if (!have_logvolid)
                        have_logvolid = !blkid_probe_set_utf8_id_label(
                            pr, "LOGICAL_VOLUME_ID",
                            vd->type.logical.logvol_id.c, clen, enc);
                }
            }
        }
        if (have_volid && have_uuid && have_volsetid && have_logvolid &&
            have_label && lvid_len && lvid_loc)
            break;
    }
    ...
    {
        vd = (struct volume_descriptor*)blkid_probe_get_buffer(
            pr, (uint64_t)lvid_loc * bs, sizeof(*vd));
        if (!vd)
            return errno ? -errno : 1;
        type = le16_to_cpu(vd->tag.id);
        if (type == TAG_ID_LVID && le32_to_cpu(vd->tag.location) == lvid_loc &&
            UDF_LVIDIU_LENGTH(*vd) >= sizeof(*lvidiu))
        {
            /* ECMA-167 3/8.8.2: There is stored sequence of LVIDs and valid is
             * just last one. So correctly we should jump to next_lvid_location
             * and read next LVID until we find last one. This could be time
             * consuming process and could lead to scanning lot of disk blocks.
             * Because we use LVID only for UDF version, in the worst case we
             * would report only wrong ID_FS_VERSION. */
            uint16_t lvidiu_udf_rev;
            lvidiu = (struct logical_vol_integ_descriptor_imp_use*)
                blkid_probe_get_buffer(
                    pr, (uint64_t)lvid_loc * bs + UDF_LVIDIU_OFFSET(*vd),
                    sizeof(*lvidiu));
            if (!lvidiu)
                return errno ? -errno : 1;
    ...
    if (udf_rev)
         ...
         */
        blkid_probe_sprintf_version(pr, "%x.%02x", (unsigned int)(udf_rev >> 8),
                                    (unsigned int)(udf_rev & 0xFF));
...
const struct blkid_idinfo udf_idinfo = {
    ...
    .magics = {{.magic = "BEA01", .len = 5, .kboff = 32, .sboff = 1},
