...
static int genl_message_new(sd_netlink* nl, sd_genl_family family,
                            uint16_t nlmsg_type, uint8_t cmd,
                            sd_netlink_message** ret)
{
    ...
    r = type_system_get_type(&genl_family_type_system_root, &genl_cmd_type,
                             family);
    if (r < 0)
        return r;
...
static int lookup_id(sd_netlink* nl, sd_genl_family family, uint16_t* id)
{
    ...
    r = sd_netlink_message_append_string(req, CTRL_ATTR_FAMILY_NAME,
                                         genl_families[family].name);
    if (r < 0)
        return r;
