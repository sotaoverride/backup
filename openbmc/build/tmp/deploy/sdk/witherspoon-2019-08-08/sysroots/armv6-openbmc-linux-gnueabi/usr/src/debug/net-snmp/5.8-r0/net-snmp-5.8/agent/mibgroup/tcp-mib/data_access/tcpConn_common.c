...
/**
 */
netsnmp_container* netsnmp_access_tcpconn_container_init(u_int flags)
{
                                          ...
                                          u_int load_flags)
{
    ...
    if (0 != rc)
    {
        netsnmp_access_tcpconn_container_free(
            container, NETSNMP_ACCESS_TCPCONN_FREE_NOFLAGS);
        container = NULL;
    }
                                           ...
                                           u_int free_flags)
{
    ...
    if (!(free_flags & NETSNMP_ACCESS_TCPCONN_FREE_DONT_CLEAR))
    {
        ...
        CONTAINER_CLEAR(
            container,
            (netsnmp_container_obj_func*)_access_tcpconn_entry_release, NULL);
    }
...
/**
 */
netsnmp_tcpconn_entry* netsnmp_access_tcpconn_entry_create(void)
{
    netsnmp_tcpconn_entry* entry = SNMP_MALLOC_TYPEDEF(netsnmp_tcpconn_entry);
    int rc = 0;
 ...
 * @retval -1 : error
 */
int netsnmp_access_tcpconn_entry_set(netsnmp_tcpconn_entry* entry)
{
