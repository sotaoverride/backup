...
static inline void /* keep this function in sync with gvaluecollector.h and
                      gboxed.c */
...
{
    /* g_return_val_if_fail (G_TYPE_IS_VALUE (g_type), NULL);	be more elaborate
     * below */
    g_return_val_if_fail(value != NULL, NULL);
    /* g_return_val_if_fail (G_VALUE_TYPE (value) == 0, NULL);	be more
     * elaborate below */
    ...
    if (G_TYPE_IS_VALUE(g_type) && G_VALUE_TYPE(value) == 0)
    {
        ...
        g_warning("%s: cannot initialize GValue with type '%s', the value has "
                  "already been initialized as '%s'",
                  G_STRLOC, g_type_name(g_type),
                  g_type_name(G_VALUE_TYPE(value)));
    else /* !G_TYPE_IS_VALUE (g_type) */
        g_warning("%s: cannot initialize GValue with type '%s', %s", G_STRLOC,
                  g_type_name(g_type),
                  g_type_value_table_peek(g_type)
                      ? "this type is abstract with regards to GValue use, use "
                        "a more specific (derived) type"
...
void g_value_copy(const GValue* src_value, GValue* dest_value)
{
    ...
    g_return_if_fail(g_value_type_compatible(G_VALUE_TYPE(src_value),
...
void g_value_set_instance(GValue* value, gpointer instance)
{
    ...
    if (instance)
    {
        ...
        g_return_if_fail(g_value_type_compatible(G_TYPE_FROM_INSTANCE(instance),
                                                 G_VALUE_TYPE(value)));
    }
...
static GValueTransform transform_func_lookup(GType src_type, GType dest_type)
{
    ...
    do
    {
        ...
        do
        {
            ...
            e = g_bsearch_array_lookup(transform_array, &transform_bconfig,
                                       &entry);
            if (e)
            {
                /* need to check that there hasn't been a change in value
                 * handling */
                if (g_type_value_table_peek(entry.dest_type) ==
                        g_type_value_table_peek(dest_type) &&
                    g_type_value_table_peek(entry.src_type) ==
                                     ...
                                     GValueTransform transform_func)
{
  ...
  if (g_bsearch_array_lookup (transform_array, &transform_bconfig, &entry))
    ...
    transform_array =
        g_bsearch_array_replace(transform_array, &transform_bconfig, &entry);
}
...
gboolean g_value_transform(const GValue* src_value, GValue* dest_value)
{
    ...
    else
    {
        GValueTransform transform =
            transform_func_lookup(G_VALUE_TYPE(src_value), dest_type);
