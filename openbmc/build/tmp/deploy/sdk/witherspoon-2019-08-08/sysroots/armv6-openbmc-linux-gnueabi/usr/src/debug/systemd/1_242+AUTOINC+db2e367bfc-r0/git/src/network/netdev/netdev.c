                             ...
                             void* userdata)
{
    ...
    {
        log_syntax(unit, LOG_ERR, filename, line, 0,
                   "Specified netdev kind is different from the previous value "
                   "'%s', ignoring assignment: %s",
...
static NetDev* netdev_free(NetDev* netdev)
{
    ...
    /* Invoke the per-kind done() destructor, but only if the state field is
     * initialized. We conditionalize that because we parse .netdev files twice:
     * once to determine the kind (with a short, minimal NetDev structure
     * allocation, with no room for per-kind fields), and once to read the
     * kind's properties (with a full, comprehensive NetDev structure allocation
     * with enough space for whatever the specific kind needs). Now, in the
     * first case we shouldn't try to destruct the per-kind NetDev fields on
     * destruction, in the second case we should. We use the state field to
     * discern the two cases: it's _NETDEV_STATE_INVALID on the first "raw"
     * call. */
    if (netdev->state != _NETDEV_STATE_INVALID && NETDEV_VTABLE(netdev) &&
...
static int netdev_enslave_ready(NetDev* netdev, Link* link,
                                link_netlink_message_handler_t callback)
{
    ...
    if (link->flags & IFF_UP && netdev->kind == NETDEV_KIND_BOND)
    {
        log_netdev_debug(netdev,
                         "Link '%s' was up when attempting to enslave it. "
                         "Bringing link down.",
                         link->ifname);
        r = link_down(link);
        if (r < 0)
    ...
    r = sd_rtnl_message_new_link(netdev->manager->rtnl, &req, RTM_SETLINK,
                                 link->ifindex);
    if (r < 0)
        return log_netdev_error_errno(
            netdev, r, "Could not allocate RTM_SETLINK message: %m");

    r = sd_netlink_message_append_u32(req, IFLA_MASTER, netdev->ifindex);
    if (r < 0)
        return log_netdev_error_errno(
            netdev, r, "Could not append IFLA_MASTER attribute: %m");
...
static int netdev_create_handler(sd_netlink* rtnl, sd_netlink_message* m,
                                 NetDev* netdev)
{
    int r;
...
static int netdev_enslave(NetDev* netdev, Link* link,
                          link_netlink_message_handler_t callback)
{
    int r;
    ...
    else if (IN_SET(netdev->state, NETDEV_STATE_LINGER, NETDEV_STATE_FAILED))
    {
        ...
        r = rtnl_message_new_synthetic_error(netdev->manager->rtnl, -ENODEV, 0,
                                             &m);
        if (r >= 0)
...
int netdev_set_ifindex(NetDev* netdev, sd_netlink_message* message)
{
    ...
    {
        log_netdev_error(
            netdev, "Cannot set ifindex from unexpected rtnl message type.");
        return -EINVAL;
    }
...
#define HASH_KEY                                                               \
    SD_ID128_MAKE(52, e1, 45, bd, 00, 6f, 29, 96, 21, c6, 30, 6d, 83, 71, 04,  \
                  48)
                         ...
                         link_netlink_message_handler_t callback)
{
    ...
    else
    {
        ...
        if (netdev->mac)
        {
            r = sd_netlink_message_append_ether_addr(m, IFLA_ADDRESS,
                                                     netdev->mac);
            if (r < 0)
                return log_netdev_error_errno(
        ...
        {
            r = netlink_call_async(netdev->manager->rtnl, NULL, m,
...
/* the callback must be called, possibly after a timeout, as otherwise the Link
 * will hang */
int netdev_join(NetDev* netdev, Link* link,
                link_netlink_message_handler_t callback)
{
    int r;
...
int netdev_load_one(Manager* manager, const char* filename)
{
    ...
    *netdev_raw = (NetDev){
        ...
        .state = _NETDEV_STATE_INVALID, /* an invalid state means done() of the
                                           implementation won't be called on
                                           destruction */
    };

    dropin_dirname = strjoina(basename(filename), ".d");
    r = config_parse_many(filename, NETWORK_DIRS, dropin_dirname,
                          "Match\0NetDev\0", config_item_perf_lookup,
    ...
    {
        log_debug("%s: Conditions in the file do not match the system "
    ...
    {
        log_warning("NetDev without Name= configured in %s. Ignoring",
                    filename);
        return 0;
    }
    ...
    netdev->state =
        NETDEV_STATE_LOADING; /* we initialize the state here for the first
                                 time, so that done() will be called on
                                 destruction */
    ...
    if (r == -EEXIST)
    {
        ...
        /* Clear ifname before netdev_free() is called. Otherwise, the NetDev
         * object 'n' is removed from the hashmap 'manager->netdevs'. */
    ...
    if (IN_SET(netdev_get_create_type(netdev), NETDEV_CREATE_MASTER,
