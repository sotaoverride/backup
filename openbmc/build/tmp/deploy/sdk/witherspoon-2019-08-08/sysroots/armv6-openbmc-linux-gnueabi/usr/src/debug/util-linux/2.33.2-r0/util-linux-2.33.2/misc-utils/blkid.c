...
struct blkid_control
{
    ...
    uintmax_t size;
    char* show[128];
    unsigned int eval : 1, gc : 1, lookup : 1, lowprobe : 1,
        lowprobe_superblocks : 1, lowprobe_topology : 1, raw_chars : 1;
};

static void print_version(FILE* out)
{
    fprintf(out, _("%s from %s  (libblkid %s, %s)\n"),
            program_invocation_short_name, PACKAGE_STRING, LIBBLKID_VERSION,
            LIBBLKID_DATE);
}
...
static void __attribute__((__noreturn__)) usage(void)
{
    ...
    fprintf(out,
            _(" %s [--cache-file <file>] [-ghlLv] [--output <format>] "
              "[--match-tag <tag>] \n"
              "       [--match-token <token>] [<dev> ...]\n\n"),
            program_invocation_short_name);
    fprintf(
        out,
        _(" %s -p [--match-tag <tag>] [--offset <offset>] [--size <size>] \n"
          "       [--output <format>] <dev> ...\n\n"),
        program_invocation_short_name);
    fprintf(out,
            _(" %s -i [--match-tag <tag>] [--output <format>] <dev> ...\n"),
            program_invocation_short_name);
    fputs(USAGE_OPTIONS, out);
    fputs(_(" -c, --cache-file <file>    read from <file> instead of reading "
            "from the default\n"
            "                              cache file (-c /dev/null means no "
            "cache)\n"),
          out);
    fputs(
        _(" -d, --no-encoding          don't encode non-printing characters\n"),
        out);
    fputs(_(" -g, --garbage-collect      garbage collect the blkid cache\n"),
          out);
    fputs(_(" -o, --output <format>      output format; can be one of:\n"
            "                              value, device, export or full; "
            "(default: full)\n"),
          out);
    fputs(_(" -k, --list-filesystems     list all known filesystems/RAIDs and "
            "exit\n"),
          out);
    fputs(_(" -s, --match-tag <tag>      show specified tag(s) (default show "
            "all tags)\n"),
          out);
    fputs(_(" -t, --match-token <token>  find device with a specific token "
            "(NAME=value pair)\n"),
          out);
    fputs(_(" -l, --list-one             look up only first device with token "
            "specified by -t\n"),
          out);
    fputs(_(" -L, --label <label>        convert LABEL to device name\n"), out);
    fputs(_(" -U, --uuid <uuid>          convert UUID to device name\n"), out);
    fputs(_(" <dev>                      specify device(s) to probe (default: "
            "all devices)\n"),
          out);
    fputs("\n", out);
    fputs(_("Low-level probing options:\n"), out);
    fputs(_(" -p, --probe                low-level superblocks probing (bypass "
            "cache)\n"),
          out);
    fputs(
        _(" -i, --info                 gather information about I/O limits\n"),
        out);
    fputs(_(" -S, --size <size>          overwrite device size\n"), out);
    fputs(_(" -O, --offset <offset>      probe at the given offset\n"), out);
    fputs(_(" -u, --usages <list>        filter by \"usage\" (e.g. -u "
            "filesystem,raid)\n"),
          out);
    fputs(_(" -n, --match-types <list>   filter by filesystem type (e.g. -n "
            "vfat,ext3)\n"),
          out);
...
static int pretty_print_word(const char* str, int max_len, int left_len,
                             int overflow_nl)
{
...
static void pretty_print_dev(blkid_dev dev)
{
    ...
    if (dev == NULL)
    {
        pretty_print_line("device", "fs_type", "label", "mount point", "UUID");
...
static void print_udev_format(const char* name, const char* value)
{
    ...
    *safe = *enc = '\0';
    ...
    else if (!strcmp(name, "UUID") || !strncmp(name, "LABEL", 5) ||
    ...
    else if (namelen >= 15 && (!strcmp(name + (namelen - 12), "_SECTOR_SIZE") ||
...
static void print_tags(const struct blkid_control* ctl, blkid_dev dev)
{
    ...
    {
        if (!(ctl->output &
              (OUTPUT_VALUE_ONLY | OUTPUT_UDEV_LIST | OUTPUT_EXPORT_LIST)))
            printf("\n");
        first = 0;
    }
}
                           ...
                           struct blkid_control* ctl)
{
    ...
    if (nvals >= 1 && !(ctl->output & (OUTPUT_VALUE_ONLY | OUTPUT_UDEV_LIST |
                                       OUTPUT_EXPORT_LIST)))
        printf("\n");
done:
    if (rc == -2)
...
err:
    *flag = 0;
    warnx(_("unknown keyword in -u <list> argument: '%s'"), word ? word : list);
...
int main(int argc, char** argv)
{
    ...
    while ((c = getopt_long(argc, argv, "c:dghilL:n:ko:O:ps:S:t:u:U:w:Vv",
                            longopts, NULL)) != -1)
    {
        ...
        switch (c)
        {
            ...
            case 'o':
                if (!strcmp(optarg, "value"))
                    ...
                    errx(BLKID_EXIT_OTHER, _("unsupported output format %s"),
                         optarg);
                break;
            case 'O':
                ctl.offset =
                    strtosize_or_err(optarg, _("invalid offset argument"));
                break;
            case 'p':
            ...
            case 't':
                ...
                if (blkid_parse_tag_string(optarg, &search_type, &search_value))
                {
    ...
    {
        if (ctl.lowprobe)
            errx(BLKID_EXIT_OTHER, _("The low-level probing mode does not "
    ...
    if (ctl.lowprobe)
    {
        ...
        if (!numdev)
            errx(BLKID_EXIT_OTHER, _("The low-level probing mode "
                                     "requires a device"));
    ...
    else if (ctl.lookup)
    {
        ...
        if (!search_type)
            errx(BLKID_EXIT_OTHER, _("The lookup option requires a "
                                     "search type specified using -t"));
        /* Load any additional devices not in the cache */
        for (i = 0; i < numdev; i++)
            blkid_get_dev(cache, devices[i], BLKID_DEV_NORMAL);

        if ((dev = blkid_find_dev_with_tag(cache, search_type, search_value)))
        {
            print_tags(&ctl, dev);
            err = 0;
        }
    ...
    else if (!numdev)
    {
        ...
        for (i = 0; i < numdev; i++)
        {
            blkid_dev dev = blkid_get_dev(cache, devices[i], BLKID_DEV_NORMAL);

            if (dev)
            {
                if (search_type &&
                    !blkid_dev_has_tag(dev, search_type, search_value))
                    continue;
                print_tags(&ctl, dev);
                err = 0;
            }
