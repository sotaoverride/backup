...
int vmstat_handler(netsnmp_mib_handler* handler,
                   ...
                   netsnmp_request_info* requests)
{
    ...
    switch (reqinfo->mode)
    {
        case MODE_GET:
            obj =
                requests->requestvb->name[requests->requestvb->name_length - 2];
            ...
            switch (obj)
            {
                 ...
                 *
                 *   This is actually a change of behaviour for some
                 * architectures, but: a)  It ensures consistency across all
                 * systems a)  It matches the definition of the MIB objects
                 *
                ...
                case CPUUSER:
                    ...
                    {
                        value =
                            (info->user_ticks - info->history[0].user_hist) *
                            100;
                        if (info->total_ticks - info->history[0].total_hist)
                            value /= (info->total_ticks -
                                      info->history[0].total_hist);
                        else
                ...
                case CPUSYSTEM:
                    if (info->history && info->history[0].total_hist)
                    {
                        /* or sys2_ticks ??? */
                        value =
                            (info->sys_ticks - info->history[0].sys_hist) * 100;
                        if (info->total_ticks - info->history[0].total_hist)
                            value /= (info->total_ticks -
                                      info->history[0].total_hist);
                        else
                ...
                case CPUIDLE:
                    ...
                    {
                        value =
                            (info->idle_ticks - info->history[0].idle_hist) *
                            100;
                        if (info->total_ticks - info->history[0].total_hist)
                            value /= (info->total_ticks -
                                      info->history[0].total_hist);
                ...
                case SYSINTERRUPTS:
                    ...
                    {
                        value =
                            (info->nInterrupts - info->history[0].intr_hist) /
                            60;
                        snmp_set_var_typed_integer(requests->requestvb,
                                                   ASN_INTEGER,
                                                   value & 0x7fffffff);
                ...
                case SYSCONTEXT:
                    ...
                    {
                        value =
                            (info->nCtxSwitches - info->history[0].ctx_hist) /
                            60;
                        snmp_set_var_typed_integer(requests->requestvb,
                                                   ASN_INTEGER,
                                                   value & 0x7fffffff);
                ...
                case SWAPIN:
                    ...
                    {
                        value =
                            (info->swapIn - info->history[0].swpi_hist) / 60;
                        /* ??? value *= PAGE_SIZE;  */
                        snmp_set_var_typed_integer(requests->requestvb,
                                                   ASN_INTEGER,
                                                   value & 0x7fffffff);
                ...
                case SWAPOUT:
                    ...
                    {
                        value =
                            (info->swapOut - info->history[0].swpo_hist) / 60;
                        /* ??? value *= PAGE_SIZE;  */
                        snmp_set_var_typed_integer(requests->requestvb,
                                                   ASN_INTEGER,
                                                   value & 0x7fffffff);
                ...
                case IOSENT:
                    ...
                    {
                        value =
                            (info->pageOut - info->history[0].pageo_hist) / 60;
                        snmp_set_var_typed_integer(requests->requestvb,
                                                   ASN_INTEGER,
                                                   value & 0x7fffffff);
                    }
                ...
                case IORECEIVE:
                    ...
                    {
                        value =
                            (info->pageIn - info->history[0].pagei_hist) / 60;
                        snmp_set_var_typed_integer(requests->requestvb,
                                                   ASN_INTEGER,
                                                   value & 0x7fffffff);
                    }
                ...
                default:
                    /*
                       XXX - The systemStats group is "holely", so walking it
                       would trigger this message repeatedly.  We really need a
                       form of the table column registration mechanism, that
                       would work with scalar groups. snmp_log(LOG_ERR, "unknown
                       object (%d) in vmstat_handler\n", (int)obj);
                     */
                    break;
            }
            break;
