...
typedef enum
{
    ...
    AVAHI_CLIENT_CONNECTING =
        101 /**< We're still connecting. This state is only entered when
               AVAHI_CLIENT_NO_FAIL has been passed to avahi_client_new() and
...
/** Get the last error number. See avahi_strerror() for converting this error
...
/** \cond fulldocs */
/** Return the local service cookie. returns AVAHI_SERVICE_COOKIE_INVALID on
 * failure. */
