...
static int dlerror_run(void (*operate)(void*), void* args)
{
    ...
    int result =
        (_dl_catch_error(&objname, &last_errstring, &malloced, operate, args)
             ?: last_errstring != NULL);
...
static void do_dlopen(void* ptr)
{
    ...
    args->map =
        GLRO(dl_open)(args->name, args->mode, args->caller_dlopen,
                      __LM_ID_CALLER, __libc_argc, __libc_argv, __environ);
}
...
static void do_dlvsym(void* ptr)
{
    ...
    args->dlsym.ref = NULL;
    args->dlsym.loadbase = GLRO(dl_lookup_symbol_x)(
        args->dlsym.name, args->dlsym.map, &args->dlsym.ref,
        args->dlsym.map->l_local_scope, &args->version, 0, 0, NULL);
}
    ...
    void* __libc_dlvsym(void* map, const char* name, const char* version)
{
    ...
    return (
        dlerror_run(do_dlvsym, &args)
            ? NULL
            : (void*)(DL_SYMBOL_ADDRESS(args.dlsym.loadbase, args.dlsym.ref)));
}
