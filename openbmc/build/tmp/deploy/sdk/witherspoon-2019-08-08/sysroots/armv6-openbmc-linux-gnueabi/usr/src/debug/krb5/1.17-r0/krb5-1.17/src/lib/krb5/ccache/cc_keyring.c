...
static const char* KRB5_CALLCONV krcc_get_name(krb5_context context,
                                               krb5_ccache id);

static krb5_error_code KRB5_CALLCONV krcc_start_seq_get(krb5_context,
                                                        krb5_ccache id,
                                                        krb5_cc_cursor* cursor);

static krb5_error_code KRB5_CALLCONV krcc_next_cred(krb5_context context,
                                                    krb5_ccache id,
                                                    krb5_cc_cursor* cursor,
                                                    krb5_creds* creds);

static krb5_error_code KRB5_CALLCONV krcc_end_seq_get(krb5_context context,
                                                      krb5_ccache id,
                                                      krb5_cc_cursor* cursor);
...
static krb5_error_code
    make_krcc_data(const char* anchor_name, const char* collection_name,
                   ...
                   key_serial_t collection_id, krcc_data** datapp);
...
static key_serial_t get_persistent_fallback(uid_t uid)
{
...
static key_serial_t get_persistent_real(uid_t uid)
{
    key_serial_t key;
 ...
 */
static key_serial_t session_write_anchor()
{
    key_serial_t s, u;
 ...
 */
static krb5_error_code find_or_create_keyring(key_serial_t parent,
 ...
 * subsidiary name. */
static krb5_error_code parse_residual(const char* residual,
                                      char** anchor_name_out,
                                      char** collection_name_out,
                                      char** subsidiary_name_out)
{
    krb5_error_code ret;
 ...
 * collection name.
 */
static krb5_boolean is_legacy_cache_name(const char* residual)
{
...
static krb5_error_code get_default(krb5_context context, char** anchor_name_out,
                                   char** collection_name_out,
                                   char** subsidiary_name_out)
{
    const char* defname;
...
static krb5_error_code make_subsidiary_residual(const char* anchor_name,
...
static krb5_error_code get_collection(const char* anchor_name,
                                      const char* collection_name,
...
static krb5_error_code set_primary_name(krb5_context context,
                                        ...
                                        const char* subsidiary_name)
{
    ...
    key = add_key(KRCC_KEY_TYPE_USER, KRCC_COLLECTION_PRIMARY, payload, plen,
                  collection_id);
    free(payload);
    return (key == -1) ? errno : 0;
}

static krb5_error_code parse_index(krb5_context context, int32_t* version,
                                   char** primary, const unsigned char* payload,
                                   size_t psize)
{
    krb5_error_code ret;
    uint32_t len;
 ...
 */
static krb5_error_code get_primary_name(krb5_context context,
                                        ...
                                        char** subsidiary_out)
{
    ...
    *subsidiary_out = NULL;
    ...
    if (primary_id == -1)
    {
        ...
        subsidiary_name =
            strdup((*collection_name == '\0') ? "tkt" : collection_name);
        if (subsidiary_name == NULL)
        {
            ret = ENOMEM;
            goto cleanup;
 ...
 */
static krb5_error_code unique_keyring(krb5_context context,
                                      key_serial_t collection_id,
                                      char** subsidiary_out,
                                      key_serial_t* cache_id_out)
{
    key_serial_t key;
    krb5_error_code ret;
    ...
    *cache_id_out = 0;
    ...
    while (tries-- > 0)
    {
        ...
        key =
            keyctl_search(collection_id, KRCC_KEY_TYPE_KEYRING, uniquename, 0);
        if (key < 0)
        {
...
static krb5_error_code add_cred_key(const char* name, const void* payload,
                                    size_t plen, key_serial_t cache_id,
...
static void update_keyring_expiration(krb5_context context, krb5_ccache id)
{
...
static krb5_error_code KRB5_CALLCONV krcc_initialize(krb5_context context,
                                                     krb5_ccache id,
                                                     krb5_principal princ)
{
...
static krb5_error_code KRB5_CALLCONV krcc_close(krb5_context context,
                                                krb5_ccache id)
{
    krcc_data* data = id->data;
 ...
 * mutex locked. */
static krb5_error_code clear_cache_keyring(krb5_context context, krb5_ccache id)
{
    krcc_data* data = id->data;
    int res;
...
static krb5_error_code KRB5_CALLCONV krcc_destroy(krb5_context context,
                                                  krb5_ccache id)
{
               ...
               krb5_ccache* cache_out)
{
    ...
    pkey =
        keyctl_search(cache_id, KRCC_KEY_TYPE_USER, KRCC_SPEC_PRINC_KEYNAME, 0);
    if (pkey < 0)
        pkey = 0;
...
static krb5_error_code KRB5_CALLCONV krcc_resolve(krb5_context context,
                                                  krb5_ccache* id,
                                                  const char* residual)
{
    krb5_error_code ret;
    ...
    cache_id =
        keyctl_search(collection_id, KRCC_KEY_TYPE_KEYRING, subsidiary_name, 0);
    if (cache_id < 0)
        cache_id = 0;
...
static krb5_error_code KRB5_CALLCONV krcc_start_seq_get(krb5_context context,
                                                        krb5_ccache id,
                                                        krb5_cc_cursor* cursor)
{
    ...
    krcursor->offsets_id =
        keyctl_search(data->cache_id, KRCC_KEY_TYPE_USER, KRCC_TIME_OFFSETS, 0);
...
static krb5_error_code KRB5_CALLCONV krcc_generate_new(krb5_context context,
                                                       krb5_ccache* id_out)
{
    krb5_ccache id = NULL;
    krb5_error_code ret;
    ...
    *id_out = NULL;
    ...
    ret =
        get_default(context, &anchor_name, &collection_name, &subsidiary_name);
    if (ret)
        return ret;
    if (anchor_name == NULL)
    {
    ...
    if (ret)
        goto cleanup;
    ret =
        unique_keyring(context, collection_id, &new_subsidiary_name, &cache_id);
    if (ret)
        goto cleanup;
...
static krb5_error_code KRB5_CALLCONV krcc_get_principal(
    krb5_context context, krb5_ccache id, krb5_principal* princ_out)
{
    krcc_data* data = id->data;
    krb5_error_code ret;
    void* payload = NULL;
...
static krb5_error_code KRB5_CALLCONV krcc_retrieve(krb5_context context,
                                                   krb5_ccache id,
                                                   krb5_flags whichfields,
                                                   krb5_creds* mcreds,
                                                   krb5_creds* creds)
{
    return k5_cc_retrieve_cred_default(context, id, whichfields, mcreds, creds);
}

/* Non-functional stub for removing a cred from the cache keyring. */
static krb5_error_code KRB5_CALLCONV krcc_remove_cred(krb5_context context,
                                                      krb5_ccache cache,
                                                      krb5_flags flags,
                                                      krb5_creds* creds)
{
...
static krb5_error_code KRB5_CALLCONV krcc_set_flags(krb5_context context,
                                                    krb5_ccache id,
                                                    krb5_flags flags)
{
    return 0;
...
static krb5_error_code KRB5_CALLCONV krcc_get_flags(krb5_context context,
                                                    krb5_ccache id,
                                                    krb5_flags* flags_out)
{
    *flags_out = 0;
    return 0;
}
...
static krb5_error_code KRB5_CALLCONV krcc_store(krb5_context context,
                                                krb5_ccache id,
                                                krb5_creds* creds)
{
    krb5_error_code ret;
    ...
    {
        (void)keyctl_set_timeout(cred_key, ts_delta(creds->times.endtime, now));
    }
...
static krb5_error_code KRB5_CALLCONV krcc_last_change_time(
...
static krb5_error_code KRB5_CALLCONV krcc_lock(krb5_context context,
                                               krb5_ccache id)
{
    krcc_data* data = id->data;
...
static krb5_error_code KRB5_CALLCONV krcc_unlock(krb5_context context,
                                                 krb5_ccache id)
{
    krcc_data* data = id->data;
    ...
    krcc_ptcursor_new(krb5_context context, krb5_cc_ptcursor* cursor_out)
{
    ...
    *cursor_out = NULL;
    ...
    {
        ret = get_primary_name(context, ptd->anchor_name, ptd->collection_name,
                               ptd->collection_id, &ptd->primary_name);
        if (ret)
            goto error;
...
static krb5_error_code KRB5_CALLCONV krcc_ptcursor_next(krb5_context context,
                                                        krb5_cc_ptcursor cursor,
                                                        krb5_ccache* cache_out)
{
    krb5_error_code ret;
...
static krb5_error_code KRB5_CALLCONV krcc_switch_to(krb5_context context,
                                                    krb5_ccache cache)
{
 ...
 */
const krb5_cc_ops krb5_krcc_ops = {
    0,    "KEYRING", NULL, NULL, NULL, NULL, NULL, NULL, NULL,
    NULL, NULL,      NULL, NULL, NULL, NULL, NULL, NULL, /* added after 1.4
                                                            release */
