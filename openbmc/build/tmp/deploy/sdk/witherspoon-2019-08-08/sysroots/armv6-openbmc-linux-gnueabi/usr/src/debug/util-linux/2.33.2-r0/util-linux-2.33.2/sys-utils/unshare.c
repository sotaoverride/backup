...
} namespace_files[] = {{.type = CLONE_NEWUSER, .name = "ns/user"},
...
static void __attribute__((__noreturn__)) usage(void)
{
    ...
    fputs(
        _(" -u, --uts[=<file>]        unshare UTS namespace (hostname etc)\n"),
        out);
    ...
    fputs(_(" -f, --fork                fork before launching <program>\n"),
          out);
    fputs(_(" -r, --map-root-user       map current user to root (implies "
            "--user)\n"),
          out);
    fputs(USAGE_SEPARATOR, out);
    fputs(_(" --kill-child[=<signame>]  when dying, kill the forked child "
            "(implies --fork)\n"
            "                             defaults to SIGKILL\n"),
          out);
    fputs(_(" --mount-proc[=<dir>]      mount proc filesystem first (implies "
            "--mount)\n"),
          out);
    fputs(_(" --propagation slave|shared|private|unchanged\n"
            ...
            "namespace\n"),
          out);
    fputs(_(" --setgroups allow|deny    control the setgroups syscall in user "
            "namespaces\n"),
          out);
...
int main(int argc, char* argv[])
{
            ...
            case OPT_KILLCHILD:
                ...
                if (optarg)
                {
                    if ((kill_child_signo = signame_to_signum(optarg)) < 0)
                        errx(EXIT_FAILURE, _("unknown signal: %s"), optarg);
                }
                else
                {
    ...
    if (procmnt &&
        (mount("none", procmnt, NULL, MS_PRIVATE | MS_REC, NULL) != 0 ||
         mount("proc", procmnt, "proc", MS_NOSUID | MS_NOEXEC | MS_NODEV,
               NULL) != 0))
