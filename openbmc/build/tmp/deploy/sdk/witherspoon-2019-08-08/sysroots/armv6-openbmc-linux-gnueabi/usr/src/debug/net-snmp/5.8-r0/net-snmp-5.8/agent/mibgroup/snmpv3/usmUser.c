...
{
    register_mib_context("snmpv3/usmUser", (struct variable*)usmUser_variables,
                         sizeof(struct variable4),
                         sizeof(usmUser_variables) / sizeof(struct variable4),
                         usmUser_variables_oid,
                         sizeof(usmUser_variables_oid) / sizeof(oid),
                         DEFAULT_MIB_PRIORITY, 0, 0, NULL, contextName, -1, 0);
}
 ...
 */
int usm_parse_oid(oid* oidIndex, size_t oidLen, unsigned char** engineID,
                  size_t* engineIDLen, unsigned char** name, size_t* nameLen)
{
    int nameL;
    int engineIDL;
    int i;
    ...
    {
        DEBUGMSGTL(
            ("usmUser", "parse_oid: null oid or zero length oid passed in\n"));
        return 1;
    }
 ...
 */
u_char* var_usmUser(struct variable* vp, oid* name, size_t* length, int exact,
                    size_t* var_len, WriteMethod** write_method)
{
    ...
    *var_len = sizeof(long_ret);
    ...
    if (vp->magic != USMUSERSPINLOCK)
    {
        ...
        {
            for (nptr = usm_get_userList(), uptr = NULL; nptr != NULL;
                 nptr = nptr->next)
            {
    ...
    switch (vp->magic)
    {
        ...
        case USMUSERCLONEFROM:
            *write_method = write_usmUserCloneFrom;
            ...
            {
                objid[0] =
                    0; /* "When this object is read, the ZeroDotZero OID */
        ...
        default:
            DEBUGMSGTL(
                ("snmpd", "unknown sub-id %d in var_usmUser\n", vp->magic));
 ...
 */
int write_usmUserSpinLock(int action, u_char* var_val, u_char var_val_type,
 ...
 */
int write_usmUserCloneFrom(int action, u_char* var_val, u_char var_val_type,
                           size_t var_val_len, u_char* statP, oid* name,
                           size_t name_len)
{
    ...
    if (action == RESERVE1)
    {
        ...
        {
            DEBUGMSGTL(
                ("usmUser", "write to usmUserCloneFrom not ASN_OBJECT_ID\n"));
            return SNMP_ERR_WRONGTYPE;
        }
    ...
    else if (action == RESERVE2)
    {
        ...
        uptr->cloneFrom =
            snmp_duplicate_objid((oid*)var_val, var_val_len / sizeof(oid));
 ...
 *	SNMP_ERR_WRONGTYPE
 */
int write_usmUserAuthProtocol(int action, u_char* var_val, u_char var_val_type,
                              size_t var_val_len, u_char* statP, oid* name,
                              size_t name_len)
{
    static oid* optr;
    ...
    if (action == RESERVE1)
    {
        ...
        {
            DEBUGMSGTL(
                ("usmUser", "write to usmUserAuthProtocol: bad length\n"));
            return SNMP_ERR_WRONGLENGTH;
        }
    }
    else if (action == RESERVE2)
    {
            ...
            uptr->userStatus == RS_NOTINSERVICE)
        {
                                 ...
                                 sizeof(usmNoAuthProtocol) / sizeof(oid)) == 0)
            {
                ...
                resetOnFail = 1;
                uptr->authProtocol = snmp_duplicate_objid(
 ...
 * XXX:  should handle action=UNDO's.
 */
int write_usmUserAuthKeyChange(int action, u_char* var_val, u_char var_val_type,
                               size_t var_val_len, u_char* statP, oid* name,
                               size_t name_len)
{
    struct usmUser* uptr;
    ...
    else if (action == ACTION)
    {
        ...
        if (decode_keychange(uptr->authProtocol, uptr->authProtocolLen,
                             uptr->authKey, uptr->authKeyLen, var_val,
                             var_val_len, buf, &buflen) != SNMPERR_SUCCESS)
        {
...
int write_usmUserPrivProtocol(int action, u_char* var_val, u_char var_val_type,
                              size_t var_val_len, u_char* statP, oid* name,
                              size_t name_len)
{
    static oid* optr;
    ...
    if (action == RESERVE1)
    {
        ...
        {
            DEBUGMSGTL(
                ("usmUser", "write to usmUserPrivProtocol: bad length\n"));
            return SNMP_ERR_WRONGLENGTH;
        }
    }
    else if (action == RESERVE2)
    {
            ...
            uptr->userStatus == RS_NOTINSERVICE)
        {
                                 ...
                                 sizeof(usmNoPrivProtocol) / sizeof(oid)) == 0)
            {
                ...
                uptr->privProtocol = snmp_duplicate_objid(
                    (oid*)var_val, var_val_len / sizeof(oid));
 ...
 *
 */
int write_usmUserPrivKeyChange(int action, u_char* var_val, u_char var_val_type,
                               size_t var_val_len, u_char* statP, oid* name,
                               size_t name_len)
{
    struct usmUser* uptr;
    ...
    else if (action == RESERVE2)
    {
        ...
        else
        {
            ...
            alen =
                sc_get_properlength(uptr->authProtocol, uptr->authProtocolLen);
    ...
    else if (action == ACTION)
    {
        ...
        alen = sc_get_properlength(uptr->authProtocol, uptr->authProtocolLen);
        DEBUGMSGTL(("usmUser", "plen %d, alen %d\n", plen, alen));
        if (snmp_oid_compare(uptr->privProtocol, uptr->privProtocolLen,
                             usmNoPrivProtocol,
                             sizeof(usmNoPrivProtocol) / sizeof(oid)) == 0)
        ...
        if (decode_keychange(uptr->authProtocol, uptr->authProtocolLen,
                             uptr->privKey, uptr->privKeyLen, var_val,
                             var_val_len, buf, &buflen) != SNMPERR_SUCCESS)
        {
            DEBUGMSGTL(("usmUser", "%s: ... failed\n", fname));
            return SNMP_ERR_GENERR;
...
int write_usmUserPublic(int action, u_char* var_val, u_char var_val_type,
                        size_t var_val_len, u_char* statP, oid* name,
                        size_t name_len)
{
...
int write_usmUserStorageType(int action, u_char* var_val, u_char var_val_type,
                             size_t var_val_len, u_char* statP, oid* name,
                             size_t name_len)
{
    ...
    if (action == RESERVE1)
    {
        ...
        {
            DEBUGMSGTL(
                ("usmUser", "write to usmUserStorageType not ASN_INTEGER\n"));
            return SNMP_ERR_WRONGTYPE;
        }
        if (var_val_len != sizeof(long))
        {
            DEBUGMSGTL(
                ("usmUser", "write to usmUserStorageType: bad length\n"));
            return SNMP_ERR_WRONGLENGTH;
        }
        ...
        else
        {
                 ...
                 uptr->userStatus == RS_NOTINSERVICE))
            {
                return SNMP_ERR_WRONGVALUE;
                     ...
                     (snmp_oid_compare(uptr->privProtocol,
                                       uptr->privProtocolLen, usmNoPrivProtocol,
                                       sizeof(usmNoPrivProtocol) /
                                           sizeof(oid)) != 0 ||
                      snmp_oid_compare(uptr->authProtocol,
                                       uptr->authProtocolLen, usmNoAuthProtocol,
 ...
 *	SNMP_ERR_WRONGTYPE
 */
int write_usmUserStatus(int action, u_char* var_val, u_char var_val_type,
                        size_t var_val_len, u_char* statP, oid* name,
                        size_t name_len)
{
    /*
    ...
    if (action == RESERVE1)
    {
        ...
        {
            DEBUGMSGTL(
                ("usmUser", "can't parse the OID for engineID or name\n"));
            return SNMP_ERR_INCONSISTENTNAME;
        }
