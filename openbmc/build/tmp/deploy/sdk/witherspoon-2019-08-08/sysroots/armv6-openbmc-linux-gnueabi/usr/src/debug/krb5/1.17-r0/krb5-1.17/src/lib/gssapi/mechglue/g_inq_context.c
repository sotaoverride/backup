...
static OM_uint32 val_inq_ctx_args(OM_uint32* minor_status,
    ...
    OM_uint32* ctx_flags, int* locally_initiated, int* opened)
{
    ...
    status = val_inq_ctx_args(minor_status, context_handle, src_name, targ_name,
    ...
    status = mech->gss_inquire_context(minor_status, ctx->internal_ctx_id,
                                       (src_name ? &localSourceName : NULL),
                                       (targ_name ? &localTargName : NULL),
                                       lifetime_rec, &actual_mech, ctx_flags,
                                       locally_initiated, opened);
    ...
    if (src_name)
    {
        if (localSourceName)
        {
            status = gssint_convert_name_to_union_name(
                minor_status, mech, localSourceName, src_name);
    ...
    if (targ_name)
    {
        ...
        {
            status = gssint_convert_name_to_union_name(
                minor_status, mech, localTargName, targ_name);
