...
FcValueListPtr FcValueListPrepend(FcValueListPtr vallist, FcValue value,
...
FcValueListPtr FcValueListAppend(FcValueListPtr vallist, FcValue value,
                                 FcValueBinding binding)
{
    ...
    if (vallist)
    {
        for (last = vallist; FcValueListNext(last);
             last = FcValueListNext(last))
            ;
...
static FcChar32 FcValueHash(const FcValue* v)
{
    switch (v->type)
    {
        ...
        case FcTypeFTFace:
            return FcStringHash(
                       (const FcChar8*)((FT_Face)v->u.f)->family_name) ^
...
static void* FcPatternGetCacheObject(FcPattern* p)
{
     ...
     * because the pattern itself may be a cache allocation if we rewrote the
     * path, so the p may not be in the cached region. */
    return FcPatternEltValues(&FcPatternElts(p)[0]);
}

FcPattern* FcPatternCacheRewriteFile(const FcPattern* p, FcCache* cache,
                                     const FcChar8* relocated_font_file)
{
     ...
     * cache */
    data = FcCacheAllocate(cache, sizeof(FcPattern) +
                                      p->num * sizeof(FcPatternElt) +
                                      sizeof(FcValueList) + new_path_len + 1);
...
FcPatternElt* FcPatternObjectInsertElt(FcPattern* p, FcObject object)
{
    ...
    if (i < 0)
    {
        ...
        if (FcPatternObjectCount(p) + 1 >= p->size)
        {
            ...
            if (p->size)
            {
                ...
                if (!e) /* maybe it was mmapped */
                {
                    ...
                    if (e)
                        memcpy(e, e0,
                               FcPatternObjectCount(p) * sizeof(FcPatternElt));
                }
            }
            else
        ...
        /* move elts up */
        memmove(e + i + 1, e + i,
                sizeof(FcPatternElt) * (FcPatternObjectCount(p) - i));
...
FcChar32 FcPatternHash(const FcPattern* p)
{
    ...
    for (i = 0; i < FcPatternObjectCount(p); i++)
    {
        h = (((h << 1) | (h >> 31)) ^ pe[i].object ^
             FcValueListHash(FcPatternEltValues(&pe[i])));
    }
    return h;
}

FcBool FcPatternEqualSubset(const FcPattern* pai, const FcPattern* pbi,
                            const FcObjectSet* os)
{
    ...
    for (i = 0; i < os->nobject; i++)
    {
        ...
        if (ea)
        {
            if (!eb)
                return FcFalse;
            if (!FcValueListEqual(FcPatternEltValues(ea),
...
FcBool FcPatternObjectListAdd(FcPattern* p, FcObject object,
...
FcBool FcPatternObjectAddWithBinding(FcPattern* p, FcObject object,
                                     FcValue value, FcValueBinding binding,
                                     FcBool append)
{
    FcPatternElt* e;
...
FcBool FcPatternObjectAdd(FcPattern* p, FcObject object, FcValue value,
                          FcBool append)
{
    return FcPatternObjectAddWithBinding(p, object, value, FcValueBindingStrong,
                                         append);
}
...
FcBool FcPatternAdd(FcPattern* p, const char* object, FcValue value,
                    FcBool append)
{
    return FcPatternObjectAddWithBinding(p, FcObjectFromName(object), value,
                                         FcValueBindingStrong, append);
}
...
FcResult FcPatternObjectGetWithBinding(const FcPattern* p, FcObject object,
...
FcResult FcPatternObjectGet(const FcPattern* p, FcObject object, int id,
                            FcValue* v)
{
    return FcPatternObjectGetWithBinding(p, object, id, v, NULL);
}

FcResult FcPatternGetWithBinding(const FcPattern* p, const char* object, int id,
...
FcResult FcPatternGet(const FcPattern* p, const char* object, int id,
                      FcValue* v)
{
    return FcPatternObjectGetWithBinding(p, FcObjectFromName(object), id, v,
                                         NULL);
}

FcResult FcPatternObjectGetInteger(const FcPattern* p, FcObject object, int id,
                                   int* i)
{
    FcValue v;
    FcResult r;
...
FcResult FcPatternGetInteger(const FcPattern* p, const char* object, int id,
                             int* i)
{
    return FcPatternObjectGetInteger(p, FcObjectFromName(object), id, i);
}

FcResult FcPatternObjectGetDouble(const FcPattern* p, FcObject object, int id,
                                  double* d)
{
    FcValue v;
    FcResult r;
...
FcResult FcPatternGetDouble(const FcPattern* p, const char* object, int id,
                            double* d)
{
    return FcPatternObjectGetDouble(p, FcObjectFromName(object), id, d);
}

FcResult FcPatternObjectGetString(const FcPattern* p, FcObject object, int id,
                                  FcChar8** s)
{
    FcValue v;
    FcResult r;
...
FcResult FcPatternObjectGetBool(const FcPattern* p, FcObject object, int id,
                                FcBool* b)
{
    FcValue v;
    FcResult r;
...
FcResult FcPatternGetCharSet(const FcPattern* p, const char* object, int id,
                             FcCharSet** c)
{
    FcValue v;
    FcResult r;
...
FcResult FcPatternGetFTFace(const FcPattern* p, const char* object, int id,
                            FT_Face* f)
{
    FcValue v;
    FcResult r;
...
FcResult FcPatternGetLangSet(const FcPattern* p, const char* object, int id,
                             FcLangSet** ls)
{
    FcValue v;
    FcResult r;
...
FcResult FcPatternObjectGetRange(const FcPattern* p, FcObject object, int id,
                                 FcRange** r)
{
    FcValue v;
    FcResult res;
    ...
    do
    {
        for (l = FcPatternIterGetValues(orig, &iter); l; l = FcValueListNext(l))
        {
            if (!FcPatternObjectAddWithBinding(
                    new, FcPatternIterGetObjectId(orig, &iter),
                    FcValueCanonicalize(&l->value), l->binding, FcTrue))
                goto bail1;
        }
...
FcBool FcPatternAppend(FcPattern* p, FcPattern* s)
{
    ...
    do
    {
        ...
        {
            if (!FcPatternObjectAddWithBinding(
                    p, FcPatternIterGetObjectId(s, &iter),
                    FcValueCanonicalize(&v->value), v->binding, FcTrue))
                return FcFalse;
        }
...
FcPattern* FcPatternFilter(FcPattern* p, const FcObjectSet* os)
{
    ...
    for (i = 0; i < os->nobject; i++)
    {
        ...
        if (e)
        {
            ...
            {
                if (!FcPatternObjectAddWithBinding(
                        ret, e->object, FcValueCanonicalize(&v->value),
                        v->binding, FcTrue))
                    goto bail0;
            }
        }
...
{
    iter->elt =
        FcPatternObjectCount(pat) > 0 && iter->pos < FcPatternObjectCount(pat)
            ? &FcPatternElts(pat)[iter->pos]
            : NULL;
}
...
FcBool FcPatternFindObjectIter(const FcPattern* pat, FcPatternIter* iter,
                               FcObject object)
{
    FcPatternPrivateIter* priv = (FcPatternPrivateIter*)iter;
...
FcResult FcPatternIterGetValue(const FcPattern* pat, FcPatternIter* iter,
                               int id, FcValue* v, FcValueBinding* b)
{
    FcValueListPtr l;
...
FcBool FcPatternSerializeAlloc(FcSerialize* serialize, const FcPattern* pat)
{
    ...
    if (!FcSerializeAlloc(serialize, pat, sizeof(FcPattern)))
        return FcFalse;
    if (!FcSerializeAlloc(serialize, elts,
                          FcPatternObjectCount(pat) * sizeof(FcPatternElt)))
        return FcFalse;
...
FcPattern* FcPatternSerialize(FcSerialize* serialize, const FcPattern* pat)
{
    ...
    *pat_serialized = *pat;
    ...
    pat_serialized->elts_offset =
        FcPtrToOffset(pat_serialized, elts_serialized);

    for (i = 0; i < FcPatternObjectCount(pat); i++)
    {
        values_serialized =
            FcValueListSerialize(serialize, FcPatternEltValues(elts + i));
        if (!values_serialized)
            return NULL;
        elts_serialized[i].object = elts[i].object;
        elts_serialized[i].values = FcPtrToEncodedOffset(
            &elts_serialized[i], values_serialized, FcValueList);
    }
...
FcValueList* FcValueListSerialize(FcSerialize* serialize, const FcValueList* vl)
{
    ...
    while (vl)
    {
        ...
        if (prev_serialized)
            prev_serialized->next = FcPtrToEncodedOffset(
                prev_serialized, vl_serialized, FcValueList);
        else
        ...
        switch ((int)vl->value.type)
        {
            ...
            case FcTypeString:
                ...
                if (!s_serialized)
                    return NULL;
                vl_serialized->value.u.s = FcPtrToEncodedOffset(
                    &vl_serialized->value, s_serialized, FcChar8);
                break;
            case FcTypeBool:
            ...
            case FcTypeCharSet:
                ...
                if (!c_serialized)
                    return NULL;
                vl_serialized->value.u.c = FcPtrToEncodedOffset(
                    &vl_serialized->value, c_serialized, FcCharSet);
                break;
            case FcTypeFTFace:
                /* can't happen */
                break;
            case FcTypeLangSet:
                ...
                if (!l_serialized)
                    return NULL;
                vl_serialized->value.u.l = FcPtrToEncodedOffset(
                    &vl_serialized->value, l_serialized, FcLangSet);
                break;
            case FcTypeRange:
                r_serialized = FcRangeSerialize(serialize, vl->value.u.r);
                if (!r_serialized)
                    return NULL;
                vl_serialized->value.u.r = FcPtrToEncodedOffset(
                    &vl_serialized->value, r_serialized, FcRange);
                break;
            default:
                break;
        }
