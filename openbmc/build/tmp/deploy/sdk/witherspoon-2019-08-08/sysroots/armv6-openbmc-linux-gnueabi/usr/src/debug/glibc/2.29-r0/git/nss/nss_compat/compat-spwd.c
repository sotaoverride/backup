...
{
    if (__nss_database_lookup("shadow_compat", "passwd_compat", "nis", &ni) >=
        0)
    {
...
static void copy_spwd_changes(struct spwd* dest, struct spwd* src, char* buffer,
                              size_t buflen)
{
    if (src->sp_pwdp != NULL && strlen(src->sp_pwdp))
    {
        ...
        else if (dest->sp_pwdp && strlen(dest->sp_pwdp) >= strlen(src->sp_pwdp))
...
static enum nss_status internal_endspent(ent_t* ent)
{
    if (ent->stream != NULL)
    {
        fclose(ent->stream);
...
static enum nss_status getspent_next_nss_netgr(const char* name,
                                               struct spwd* result, ent_t* ent,
                                               char* group, char* buffer,
                                               size_t buflen, int* errnop)
{
...
static enum nss_status getspnam_plususer(const char* name, struct spwd* result,
                                         ...
                                         size_t buflen, int* errnop)
{
    ...
    enum nss_status status =
        nss_getspnam_r(name, result, buffer, buflen, errnop);
                                          ...
                                          int* errnop)
{
    ...
    while (1)
    {
            ...
            result->sp_namp[2] != '\0')
        {
            ...
            status = getspent_next_nss_netgr(
                NULL, result, ent, &result->sp_namp[2], buffer, buflen, errnop);
            ...
            result->sp_namp[1] != '@')
        {
            ...
            status = getspnam_plususer(&result->sp_namp[1], result, ent, buffer,
                                       buflen, errnop);
                                           ...
                                           int* errnop)
{
    if (ent->netgroup)
    {
        ...
        status = getspent_next_nss_netgr(NULL, pw, ent, NULL, buffer, buflen,
                                         errnop);
...
enum nss_status _nss_compat_getspent_r(struct spwd* pwd, char* buffer,
                                       size_t buflen, int* errnop)
{
...
static enum nss_status internal_getspnam_r(const char* name,
                                           struct spwd* result, ent_t* ent,
                                           char* buffer, size_t buflen,
                                           int* errnop)
{
    ...
    while (1)
    {
            ...
            result->sp_namp[2] != '\0')
        {
            ...
            if (innetgr(&result->sp_namp[2], NULL, name, NULL))
            {
                status = getspnam_plususer(name, result, ent, buffer, buflen,
                                           errnop);
        ...
        if (result->sp_namp[0] == '+' && result->sp_namp[1] == '\0')
        {
            ...
            status =
                getspnam_plususer(name, result, ent, buffer, buflen, errnop);
