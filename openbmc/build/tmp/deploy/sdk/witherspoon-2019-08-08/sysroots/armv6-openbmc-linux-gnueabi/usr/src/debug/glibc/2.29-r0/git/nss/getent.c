...
static struct argp argp = {args_options, parse_option, args_doc,
                           doc,          NULL,         more_help};
...
static void print_aliases(struct aliasent* alias)
{
    ...
    for (i = 0; i < alias->alias_members_len; ++i)
        printf("%s%s", alias->alias_members[i],
...
static int ahosts_keys_int(int af, int xflags, int number, char* key[])
{
    ...
    hint.ai_flags =
        (AI_V4MAPPED | AI_ADDRCONFIG | AI_CANONNAME | idn_flags | xflags);
    hint.ai_family = af;
    ...
    for (i = 0; i < number; ++i)
    {
        ...
        else
        {
            ...
            while (runp != NULL)
            {
                ...
                {
                    snprintf(sockbuf, sizeof(sockbuf), "%d", runp->ai_socktype);
                    sockstr = sockbuf;
                }
                ...
                printf("%-15s %-6s %s\n",
                       inet_ntop(
                           ...
                           runp->ai_family == AF_INET
                               ? (void*)&((struct sockaddr_in*)runp->ai_addr)
                                     ->sin_addr
                               : (void*)&((struct sockaddr_in6*)runp->ai_addr)
                                     ->sin6_addr,
                           buf, sizeof(buf)),
                       sockstr, runp->ai_canonname ?: "");
...
static int netgroup_keys(int number, char* key[])
{
    ...
    if (number == 4)
    {
        ...
        printf("%-21s (%s,%s,%s) = %d\n", key[0], host ?: "", user ?: "",
               domain ?: "", innetgr(key[0], host, user, domain));
    }
    else if (number == 1)
    {
...
static int initgroups_keys(int number, char* key[])
{
    ...
    for (int i = 0; i < number; ++i)
    {
        ...
        int n;
        while ((n = getgrouplist(key[i], -1, gid_list_begin(&list), &no)) ==
                   -1 &&
...
static void print_rpc(struct rpcent* rpc)
{
    ...
    printf("%-15s %d%s", rpc->r_name, rpc->r_number,
           rpc->r_aliases[0] ? " " : "");
...
static int services_keys(int number, char* key[])
{
    ...
    for (i = 0; i < number; ++i)
    {
        ...
        if (isdigit(key[i][0]) && *endptr == '\0' && 0 <= port && port <= 65535)
...
#define D(name) {#name, name##_keys},
    D(ahosts) D(ahostsv4) D(ahostsv6) D(aliases) D(ethers) D(group) D(gshadow)
        D(hosts) D(initgroups) D(netgroup) D(networks) D(passwd) D(protocols)
#if HAVE_SUNRPC
            D(rpc)
#endif
                D(services) D(shadow)
...
int main(int argc, char* argv[])
{
    ...
    for (int i = 0; databases[i].name; ++i)
        if (argv[remaining][0] == databases[i].name[0] &&
            ...
            return databases[i].func(argc - remaining - 1,
                                     &argv[remaining + 1]);
