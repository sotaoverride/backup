...
static void byteswap_body_helper(DBusTypeReader* reader,
                                 dbus_bool_t walk_reader_to_end,
                                 int old_byte_order, int new_byte_order,
                                 unsigned char* p, unsigned char** new_p)
{
    int current_type;
           ...
           DBUS_TYPE_INVALID)
    {
        switch (current_type)
        {
            ...
            case DBUS_TYPE_UINT16:
            {
                ...
                *((dbus_uint16_t*)p) =
                    DBUS_UINT16_SWAP_LE_BE(*((dbus_uint16_t*)p));
                p += 2;
            }
            break;
            ...
            case DBUS_TYPE_UINT32:
            {
                ...
                *((dbus_uint32_t*)p) =
                    DBUS_UINT32_SWAP_LE_BE(*((dbus_uint32_t*)p));
                p += 4;
            }
            break;
            ...
            case DBUS_TYPE_DOUBLE:
            {
                ...
                *((dbus_uint64_t*)p) =
                    DBUS_UINT64_SWAP_LE_BE(*((dbus_uint64_t*)p));
                p += 8;
            }
            break;
            ...
            case DBUS_TYPE_OBJECT_PATH:
            {
                ...
                *((dbus_uint32_t*)p) =
                ...
                if (current_type == DBUS_TYPE_ARRAY)
                {
                    ...
                    _dbus_assert((array_len / alignment) <
                    ...
                    {
                        if (alignment > 1)
                            _dbus_swap_array(p, array_len / alignment,
                                             alignment);
                        p += array_len;
                    }
                    else
                    {
                        ...
                        while (p < array_end)
                        {
                            byteswap_body_helper(&sub, FALSE, old_byte_order,
                                                 new_byte_order, p, &p);
                        }
                    }
                }
            ...
            case DBUS_TYPE_VARIANT:
            {
                ...
                contained_alignment = _dbus_type_get_alignment(
                    _dbus_first_type_in_signature(&sig, 0));
            ...
            case DBUS_TYPE_DICT_ENTRY:
            {
                ...
                byteswap_body_helper(&sub, TRUE, old_byte_order, new_byte_order,
                                     p, &p);
            }
            break;
            ...
            case DBUS_TYPE_UNIX_FD:
                ...
                _dbus_assert_not_reached(
                    "attempted to byteswap unix fds which makes no sense");
                break;
 ...
 */
void _dbus_marshal_byteswap(const DBusString* signature, int signature_start,
                            int old_byte_order, int new_byte_order,
                            DBusString* value_str, int value_pos)
{
    ...
    byteswap_body_helper(&reader, TRUE, old_byte_order, new_byte_order,
