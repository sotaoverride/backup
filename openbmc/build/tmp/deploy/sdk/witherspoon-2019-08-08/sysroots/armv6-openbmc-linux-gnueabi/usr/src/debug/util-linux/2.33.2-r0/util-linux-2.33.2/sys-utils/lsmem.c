...
struct lsmem
{
    ...
    unsigned int have_nodes : 1, raw : 1, export : 1, json : 1, noheadings : 1,
        summary : 1, list_all : 1, bytes : 1, want_summary : 1, want_table : 1,
        split_by_node : 1, split_by_state : 1, split_by_removable : 1,
        split_by_zones : 1, have_zones : 1;
};
...
static char* zone_names[] = {
    ...
    [ZONE_NORMAL] = "Normal",   [ZONE_HIGHMEM] = "Highmem",
    [ZONE_MOVABLE] = "Movable", [ZONE_DEVICE] = "Device",
    [ZONE_NONE] = "None", /* block contains more than one zone, can't be
                             offlined */
    ...
    for (i = 0; i < ncolumns; i++)
    {
        ...
        switch (get_column_id(i))
        {
            case COL_RANGE:
            {
                ...
                xasprintf(&str, "0x%016" PRIx64 "-0x%016" PRIx64, start,
                          start + size - 1);
                break;
            }
            case COL_SIZE:
            ...
            case COL_STATE:
                str =
                    xstrdup(blk->state == MEMORY_STATE_ONLINE
                                ? _("online")
            ...
            case COL_BLOCK:
                if (blk->count == 1)
                    ...
                    xasprintf(&str, "%" PRId64 "-%" PRId64, blk->index,
                              blk->index + blk->count - 1);
                break;
            case COL_NODE:
                if (lsmem->have_nodes)
            ...
            case COL_ZONES:
                if (lsmem->have_zones)
                {
                    ...
                    {
                        zone_id = blk->zones[j];
                        if (strlen(valid_zones) + strlen(zone_names[zone_id]) >
                            BUFSIZ - 2)
                            break;
...
static void read_info(struct lsmem* lsmem)
{
    ...
    if (ul_path_read_buffer(lsmem->sysmem, buf, sizeof(buf),
                            "block_size_bytes") <= 0)
...
static void __attribute__((__noreturn__)) usage(void)
{
    ...
    fputs(
        _(" -s, --sysroot <dir>  use the specified directory as system root\n"),
        out);
    fputs(_("     --summary[=when] print summary information (never,always or "
            "only)\n"),
          out);
...
{
    struct lsmem _lsmem = {.want_table = 1, .want_summary = 1},
                 *lsmem = &_lsmem;
    ...
    while ((c = getopt_long(argc, argv, "abhJno:PrS:s:V", longopts, NULL)) !=
           -1)
    {
        ...
        switch (c)
        {
            ...
            case 'o':
                ...
                break;
            case OPT_OUTPUT_ALL:
                for (ncolumns = 0; (size_t)ncolumns < ARRAY_SIZE(coldescs);
                     ncolumns++)
    ...
    if (lsmem->want_table + lsmem->want_summary == 0)
        errx(EXIT_FAILURE, _("options --{raw,json,pairs} and --summary=only "
    ...
    if (!lsmem->sysmem)
        err(EXIT_FAILURE, _("failed to initialize %s handler"),
            _PATH_SYS_MEMORY);
    ...
    for (i = 0; i < ncolumns; i++)
    {
        ...
        cl = scols_table_new_column(lsmem->table, ci->name, ci->whint,
                                    ci->flags);
        if (!cl)
