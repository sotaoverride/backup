...
G_DEFINE_TYPE_WITH_CODE(
    GSimpleActionGroup, g_simple_action_group, G_TYPE_OBJECT,
...
static gboolean g_simple_action_group_query_action(
...
static void g_simple_action_group_change_state(GActionGroup* group,
...
static void g_simple_action_group_activate(GActionGroup* group,
                                           const gchar* action_name,
                                           GVariant* parameter)
{
    GSimpleActionGroup* simple = G_SIMPLE_ACTION_GROUP(group);
...
static void action_enabled_notify(GAction* action, GParamSpec* pspec,
                                  gpointer user_data)
{
    g_action_group_action_enabled_changed(user_data, g_action_get_name(action),
                                          g_action_get_enabled(action));
}

static void action_state_notify(GAction* action, GParamSpec* pspec,
                                gpointer user_data)
{
    GVariant* value;

    value = g_action_get_state(action);
    g_action_group_action_state_changed(user_data, g_action_get_name(action),
                                        value);
    g_variant_unref(value);
}

static void g_simple_action_group_disconnect(gpointer key, gpointer value,
                                             gpointer user_data)
{
...
static GAction* g_simple_action_group_lookup_action(GActionMap* action_map,
                                                    const gchar* action_name)
{
...
static void g_simple_action_group_add_action(GActionMap* action_map,
                                             GAction* action)
{
    GSimpleActionGroup* simple = G_SIMPLE_ACTION_GROUP(action_map);
    const gchar* action_name;
    ...
    if (old_action != action)
    {
        ...
        g_hash_table_insert(simple->priv->table, g_strdup(action_name),
                            g_object_ref(action));
...
static void g_simple_action_group_remove_action(GActionMap* action_map,
                                                const gchar* action_name)
{
    GSimpleActionGroup* simple = G_SIMPLE_ACTION_GROUP(action_map);
    GAction* action;
...
static void g_simple_action_group_finalize(GObject* object)
{
    ...
    g_hash_table_foreach(simple->priv->table, g_simple_action_group_disconnect,
                         simple);
...
{
    g_action_map_add_action_entries(G_ACTION_MAP(simple), entries, n_entries,
                                    user_data);
}
