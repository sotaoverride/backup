                                   ...
                                   size_t nbytes)
{
    ...
    for (p = buffer; nbytes; p++, nbytes--)
    {
        ...
        if (idx > 2)
        {
            ...
            tmp[1] =
                bintoasc[(((*radbuf << 4) & 060) | ((radbuf[1] >> 4) & 017)) &
                         077];
            tmp[2] =
                bintoasc[(((radbuf[1] << 2) & 074) | ((radbuf[2] >> 6) & 03)) &
                         077];
...
gpg_err_code_t _gpgrt_b64enc_finish(gpgrt_b64state_t state)
{
    ...
    if (quad_count && !(state->flags & B64ENC_NO_LINEFEEDS) &&
        _gpgrt_fputs("\n", state->stream) == EOF)
        goto write_error;
    ...
    if ((state->flags & B64ENC_USE_PGPCRC))
    {
        ...
        tmp[0] = bintoasc[(*radbuf >> 2) & 077];
        tmp[1] =
            bintoasc[(((*radbuf << 4) & 060) | ((radbuf[1] >> 4) & 017)) & 077];
        tmp[2] = bintoasc[(((radbuf[1] << 2) & 074) | ((radbuf[2] >> 6) & 03)) &
                          077];
        tmp[3] = bintoasc[radbuf[2] & 077];
        for (idx = 0; idx < 4; idx++)
            _gpgrt_fputc(tmp[idx], state->stream);
