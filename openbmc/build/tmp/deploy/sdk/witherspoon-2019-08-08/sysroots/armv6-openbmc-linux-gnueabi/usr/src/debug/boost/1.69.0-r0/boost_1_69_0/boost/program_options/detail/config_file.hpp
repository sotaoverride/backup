...
#ifdef BOOST_MSVC
#pragma warning(push)
#pragma warning(disable : 4251) // class XYZ needs to have dll-interface to be
                                // used by clients of class XYZ
#endif
    ...
    : public eof_iterator<common_config_file_iterator, option>
{
  public:
    ...
    common_config_file_iterator(const std::set<std::string>& allowed_options,
...
basic_config_file_iterator<charT>::basic_config_file_iterator(
    std::basic_istream<charT>& is, const std::set<std::string>& allowed_options,
