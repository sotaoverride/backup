...
{
    NSS_GETONE(NSLCD_ACTION_SERVICE_BYNAME, WRITE_STRING(fp, name);
                                       ...
                                       size_t buflen, int* errnop)
{
    ...
    NSS_GETONE(NSLCD_ACTION_SERVICE_BYNUMBER, tmpint32 = ntohs(port);
...
{
    int res, i;
    res = snprintf(buffer, buflen, "%s %d/%s", result->s_name,
                   ntohs(result->s_port), result->s_proto);
...
static nss_status_t services_getservbyname(nss_backend_t UNUSED(*be),
                                           void* args)
{
...
static nss_status_t services_getservbyport(nss_backend_t UNUSED(*be),
                                           void* args)
{
...
static nss_backend_op_t services_ops[] = {
    nss_ldap_destructor, services_endservent,    services_setservent,
    services_getservent, services_getservbyname, services_getservbyport};
