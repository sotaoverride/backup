...
/* dbus-server-protected.h Used by subclasses of DBusServer object (internal to
...
struct DBusServer
{
    ...
    dbus_bool_t published_address; /**< flag which indicates that server has
                                      published its bus address. */
    ...
    unsigned int
        have_server_lock : 1; /**< Does someone have the server mutex locked */
#endif
};
...
typedef enum
{
    DBUS_SERVER_LISTEN_NOT_HANDLED,     /**< we aren't in charge of this address
                                           type */
    DBUS_SERVER_LISTEN_OK,              /**< we set up the listen */
    DBUS_SERVER_LISTEN_BAD_ADDRESS,     /**< malformed address */
    DBUS_SERVER_LISTEN_DID_NOT_CONNECT, /**< well-formed address but failed to
                                           set it up */
...
DBusServerListenResult _dbus_server_listen_platform_specific(
    DBusAddressEntry* entry, DBusServer** server_p, DBusError* error);
