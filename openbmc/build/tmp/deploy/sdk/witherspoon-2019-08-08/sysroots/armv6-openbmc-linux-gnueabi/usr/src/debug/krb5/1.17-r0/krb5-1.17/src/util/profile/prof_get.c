...
static errcode_t get_values_vt(profile_t profile, const char* const* names,
                               char*** ret_values)
{
    errcode_t retval;
    char **vtvalues, **val;
...
errcode_t KRB5_CALLCONV profile_get_values(profile_t profile,
                                           ...
                                           char*** ret_values)
{
    ...
    *ret_values = NULL;
    ...
    if ((retval = profile_node_iterator_create(
             profile, names, PROFILE_ITER_RELATIONS_ONLY, &state)))
        return retval;
 ...
 * result. */
static errcode_t get_value_vt(profile_t profile, const char* const* names,
                              char** ret_value)
{
    errcode_t retval;
    char** vtvalues;
...
errcode_t KRB5_CALLCONV profile_get_string(profile_t profile, const char* name,
                                           const char* subname,
...
static errcode_t parse_int(const char* value, int* ret_int)
{
    char* end_value;
    long ret_long;
...
errcode_t KRB5_CALLCONV profile_get_integer(profile_t profile, const char* name,
                                            const char* subname,
...
errcode_t KRB5_CALLCONV profile_get_boolean(profile_t profile, const char* name,
                                            const char* subname,
                                            const char* subsubname, int def_val,
                                            int* ret_boolean)
{
 ...
 */
errcode_t KRB5_CALLCONV profile_get_subsection_names(profile_t profile,
                                                     const char** names,
                                                     char*** ret_names)
{
    errcode_t retval;
    ...
    if ((retval = profile_iterator_create(
             profile, names,
             PROFILE_ITER_LIST_SECTION | PROFILE_ITER_SECTIONS_ONLY, &state)))
        return retval;
 ...
 * under the specified section name.
 */
errcode_t KRB5_CALLCONV profile_get_relation_names(profile_t profile,
                                                   const char** names,
                                                   char*** ret_names)
{
    errcode_t retval;
...
errcode_t KRB5_CALLCONV profile_iterator_create(profile_t profile,
                                                const char* const* names,
    ...
    *ret_iter = NULL;
    ...
    if (profile->vt)
    {
        if (!profile->vt->iterator_create)
            ...
            retval = profile->vt->iterator_create(profile->cbdata, names, flags,
                                                  &iter->idata);
    }
    else
    {
        retval =
            profile_node_iterator_create(profile, names, flags, &iter->idata);
    }
    if (retval)
    {
        free(iter);
...
static errcode_t set_results(const char* name, const char* value,
...
errcode_t KRB5_CALLCONV profile_iterator(void** iter_p, char** ret_name,
                                         char** ret_value)
{
    char *name, *value;
    errcode_t retval;
    ...
    {
        retval =
            profile->vt->iterator(profile->cbdata, iter->idata, &name, &value);
        if (retval)
            return retval;
        if (name == NULL)
        {
