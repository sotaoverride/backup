...
regex_error::regex_error(const std::string& s, regex_constants::error_type err,
                         std::ptrdiff_t pos) :
    std::runtime_error(s),
    m_error_code(err), m_position(pos)
{
}
    ...
    verify_options(boost::regex::flag_type /*ef*/, match_flag_type mf)
{
    ...
    {
        std::logic_error msg("Usage Error: Can't mix regular expression "
                             "captures with POSIX matching rules");
        throw_exception(msg);
    }
#endif
}
...
BOOST_REGEX_DECL void BOOST_REGEX_CALL reset_stack_guard_page()
{
    ...
    if (!VirtualFree(mi.AllocationBase,
                     (LPBYTE)page - (LPBYTE)mi.AllocationBase, MEM_DECOMMIT) ||
