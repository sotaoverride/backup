  ...
  private:
    ...
    void Init(size_type size, size_type capacity)
    {
        ...
        else
        {
            ...
            pData_ =
                static_cast<Data*>(malloc(sizeof(Data) + capacity * sizeof(E)));
            if (!pData_)
                boost::throw_exception(std::bad_alloc());
...
class AllocatorStringStorage : public A
{
    ...
    void Init(size_type size, size_type cap)
    {
        ...
        {
            pData_ =
                const_cast<Data*>(&SimpleStringStorage<E, A>::emptyString_);
        }
        else
        {
        ...
        else
        {
            ...
            else
            {
                flex_string_details::pod_move(
                    s, s + sz, buf_ + maxSmallString - buf_[maxSmallString]);
...
class CowString
{
  ...
  private:
    ...
    void MakeUnique() const
    {
        ...
        new (buf_)
            Storage(*new (p) Storage(Data()), flex_string_details::Shallow());
        *Data().begin() = 1;
    }
...
class flex_string : private Storage
{
  ...
  private:
    ...
    bool IsAliasedRange(Iterator beginIterator, Iterator endIterator)
    {
        if (!empty() && beginIterator != endIterator)
        {
            ...
            const std::less_equal<pointer> less_equal =
                std::less_equal<pointer>();
            if (less_equal(myBegin, rangeBegin) &&
                less_equal(rangeBegin, myEnd))
                return true;
        }
        return false;
    }
  ...
  public:
    ...
    // Replaces at most n1 chars of *this, starting with pos1 with the content
    // of str
    ...
    // occurrences of c consolidated with Replaces at most n1 chars of *this,
    // starting with pos, with at most n2 chars of str. str must have at least
    // n2 chars.
  ...
  public:
    ...
    std::ptrdiff_t compare(size_type pos1, size_type n1, const value_type* s,
                           size_type n2) const
    {
    ...
    std::ptrdiff_t compare(size_type pos1, size_type n1, const flex_string& str,
                 ...
                 const unsigned int file_version)
{
    boost::serialization::stl::load_collection<
        ...
        boost::serialization::stl::reserve_imp<
            boost::wave::util::flex_string<E, T, A, S>>>(ar, t);
}
...
inline void serialize(Archive& ar,
                      boost::wave::util::flex_string<E, T, A, S>& t,
                      const unsigned int file_version)
{
