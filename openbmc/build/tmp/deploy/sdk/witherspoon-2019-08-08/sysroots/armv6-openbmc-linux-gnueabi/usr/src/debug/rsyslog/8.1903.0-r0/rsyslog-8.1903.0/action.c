...
/* AIXPORT : cs renamed to legacy_cs as clashes with libpthreads variable in
 * complete file*/
#ifdef _AIX
#define cs legacy_cs
#endif
...
#define NO_TIME_PROVIDED 0 /* indicate we do not provide any cached time */
    ...
    /* forward definitions */
    static rsRetVal
    processBatchMain(void* pVoid, batch_t* pBatch, wti_t* const pWti);
static rsRetVal doSubmitToActionQ(action_t* const pAction, wti_t* const pWti,
                                  smsg_t*);
static rsRetVal doSubmitToActionQComplex(action_t* const pAction,
                                         wti_t* const pWti, smsg_t*);
static rsRetVal doSubmitToActionQNotAllMark(action_t* const pAction,
                                            wti_t* const pWti, smsg_t*);

/* object static data (once for all instances) */
DEFobjCurrIf(obj) DEFobjCurrIf(datetime) DEFobjCurrIf(module)
    DEFobjCurrIf(statsobj) DEFobjCurrIf(ruleset)

        typedef struct configSettings_s
{
    int bActExecWhenPrevSusp;    /* execute action only when previous one was
                                    suspended? */
    int bActionWriteAllMarkMsgs; /* should all mark messages be unconditionally
                                    written? */
    int iActExecOnceInterval;    /* execute action once every nn seconds */
    int iActExecEveryNthOccur;   /* execute action every n-th occurence
                                    (0,1=always) */
    time_t iActExecEveryNthOccurTO; /* timeout for n-occurence setting (in
                                       seconds, 0=never) */
    int glbliActionResumeInterval;
    int glbliActionResumeRetryCount; /* how often should suspended actions be
                                        retried? */
    ...
    int iActionQueueNumWorkers; /* number of worker threads for the mm queue
                                   above */
    ...
    int iActionQWrkMinMsgs; /* minimum messages per worker needed to start a new
                               one */
    ...
    int iActionQueueDeqtWinFromHr; /* hour begin of time frame when queue is to
                                      be dequeued */
    int iActionQueueDeqtWinToHr; /* hour begin of time frame when queue is to be
                                    dequeued */
} configSettings_t;

static configSettings_t cs; /* our current config settings */

/* the counter below counts actions created. It is used to obtain unique IDs for
 * the action. They should not be relied on for any long-term activity (e.g.
 * disk queue names!), but they are nice to have during one instance of an
 * rsyslogd run. For example, I use them to name actions when there is no better
 * name available.
 */
int iActionNbr = 0;
...
static struct cnfparamdescr cnfparamdescr[] = {
    ...
    {"action.writeallmarkmessages", eCmdHdlrBinary,
     0}, /* legacy: actionwriteallmarkmessages */
    {"action.execonlyeverynthtime", eCmdHdlrInt,
     0}, /* legacy: actionexeconlyeverynthtime */
    {"action.execonlyeverynthtimetimeout", eCmdHdlrInt,
     0}, /* legacy: actionexeconlyeverynthtimetimeout */
    {"action.execonlyonceeveryinterval", eCmdHdlrInt,
     0}, /* legacy: actionexeconlyonceeveryinterval */
    {"action.execonlywhenpreviousissuspended", eCmdHdlrBinary, 0},
    /* legacy: actionexeconlywhenpreviousissuspended */
    {"action.repeatedmsgcontainsoriginalmsg", eCmdHdlrBinary,
     0}, /* legacy: repeatedmsgcontainsoriginalmsg */
    {"action.resumeretrycount", eCmdHdlrInt,
     0}, /* legacy: actionresumeretrycount */
...
static struct cnfparamblk pblk = {
    CNFPARAMBLK_VERSION, sizeof(cnfparamdescr) / sizeof(struct cnfparamdescr),
    cnfparamdescr};
...
static time_t getActNow(action_t* const pThis)
{
    ...
    {
        pThis->tActNow =
            datetime.GetTime(NULL); /* good time call - the only one done */
...
static rsRetVal actionResetQueueParams(void)
{
    ...
    cs.ActionQueType =
        QUEUETYPE_DIRECT;             /* type of the main message queue above */
    ...
    cs.iActionQWrkMinMsgs =
        -1; /* minimum messages per worker needed to start a new one */
    cs.bActionQSaveOnShutdown =
        1; /* save queue on shutdown (when DA enabled)? */
    ...
    cs.glbliActionResumeRetryCount =
        0; /* I guess it is smart to reset this one, too */
...
rsRetVal actionConstruct(action_t** ppThis)
{
    ...
    pThis->bCopyMsg = 0;
    pThis->tLastOccur =
        datetime.GetTime(NULL); /* done once per action on startup only */
 ...
 */
rsRetVal actionConstructFinalize(action_t* __restrict__ const pThis,
                                 struct nvlst* lst)
{
    DEFiRet;
    ...
    CHKiRet(statsobj.AddCounter(pThis->statsobj,
                                UCHAR_CONSTANT("suspended.duration"),
     ...
     * This is only possible if some features, which require strict sequence,
     * are not used. Thankfully, that is usually the case. The benefit of
     * firehose mode is much faster processing (and simpler code) -- rgerhards,
     * 2010-06-08
     */
     ...
     */
    CHKiRet(qqueueConstruct(&pThis->pQueue, cs.ActionQueType, 1,
                            cs.iActionQueueSize, processBatchMain));
...
#define setQPROPstr(func, directive, data)                                     \
    CHKiRet_Hdlr(                                                              \
        func(pThis->pQueue, data, (data == NULL) ? 0 : strlen((char*)data)))   \
    {                                                                          \
        LogError(0, NO_ERRCODE, "Invalid " #directive ", \
				error %d. Ignored, running with default setting",              \
                 iRet);                                                        \
    }
        setQPROP(qqueueSetsizeOnDiskMax, "$ActionQueueMaxDiskSpace",
                 cs.iActionQueMaxDiskSpace);
        setQPROP(qqueueSetiDeqBatchSize, "$ActionQueueDequeueBatchSize",
                 cs.iActionQueueDeqBatchSize);
        setQPROP(qqueueSetMaxFileSize, "$ActionQueueFileSize",
                 cs.iActionQueMaxFileSize);
        setQPROPstr(qqueueSetFilePrefix, "$ActionQueueFileName",
                    cs.pszActionQFName);
        setQPROP(qqueueSetiPersistUpdCnt, "$ActionQueueCheckpointInterval",
                 cs.iActionQPersistUpdCnt);
        setQPROP(qqueueSetbSyncQueueFiles, "$ActionQueueSyncQueueFiles",
                 cs.bActionQSyncQeueFiles);
        setQPROP(qqueueSettoQShutdown, "$ActionQueueTimeoutShutdown",
                 cs.iActionQtoQShutdown);
        setQPROP(qqueueSettoActShutdown, "$ActionQueueTimeoutActionCompletion",
                 cs.iActionQtoActShutdown);
        setQPROP(qqueueSettoWrkShutdown,
                 "$ActionQueueWorkerTimeoutThreadShutdown",
                 cs.iActionQtoWrkShutdown);
        setQPROP(qqueueSettoEnq, "$ActionQueueTimeoutEnqueue",
                 cs.iActionQtoEnq);
        setQPROP(qqueueSetiHighWtrMrk, "$ActionQueueHighWaterMark",
                 cs.iActionQHighWtrMark);
        setQPROP(qqueueSetiLowWtrMrk, "$ActionQueueLowWaterMark",
                 cs.iActionQLowWtrMark);
        setQPROP(qqueueSetiDiscardMrk, "$ActionQueueDiscardMark",
                 cs.iActionQDiscardMark);
        setQPROP(qqueueSetiDiscardSeverity, "$ActionQueueDiscardSeverity",
                 cs.iActionQDiscardSeverity);
        setQPROP(qqueueSetiMinMsgsPerWrkr,
                 "$ActionQueueWorkerThreadMinimumMessages",
                 cs.iActionQWrkMinMsgs);
        setQPROP(qqueueSetiNumWorkerThreads, "$ActionQueueWorkerThreads",
                 cs.iActionQueueNumWorkers);
        setQPROP(qqueueSetbSaveOnShutdown, "$ActionQueueSaveOnShutdown",
                 cs.bActionQSaveOnShutdown);
        setQPROP(qqueueSetiDeqSlowdown, "$ActionQueueDequeueSlowdown",
                 cs.iActionQueueDeqSlowdown);
        setQPROP(qqueueSetiDeqtWinFromHr, "$ActionQueueDequeueTimeBegin",
                 cs.iActionQueueDeqtWinFromHr);
        setQPROP(qqueueSetiDeqtWinToHr, "$ActionQueueDequeueTimeEnd",
                 cs.iActionQueueDeqtWinToHr);
    }
    else
    {
...
static rsRetVal getReturnCode(action_t* const pThis, wti_t* const pWti)
{
    ...
    switch (getActionState(pWti, pThis))
    {
        ...
        case ACT_STATE_ITX:
            ...
            {
                pWti->actWrkrInfo[pThis->iActionNbr].bHadAutoCommit =
                    0; /* auto-reset */
 ...
 * rgerhards, 2007-08-02
 */
static void actionSetState(action_t* const pThis, wti_t* const pWti,
                           uint8_t newState)
{
 ...
 * rgerhards, 2007-08-02
 */
static void actionSuspend(action_t* const pThis, wti_t* const pWti)
{
    time_t ttNow;
    int suspendDuration;
    char timebuf[32];
    ...
    suspendDuration =
        pThis->iResumeInterval * (getActionNbrResRtry(pWti, pThis) / 10 + 1);
    if (pThis->iResumeIntervalMax > 0 &&
        ...
        (pThis->bReportSuspension && getActionNbrResRtry(pWti, pThis) == 0))
    {
        ...
        LogMsg(0, RS_RET_SUSPENDED, LOG_WARNING,
               "action '%s' suspended (module '%s'), next retry is %s, retry "
               "nbr %d. "
 ...
 * Note that we do the full retry processing here, doing the configured number
 * of iterations. -- rgerhards, 2009-05-07 We need to guard against module which
 * always return RS_RET_OK from their tryResume() entry point. This is invalid,
 * but has harsh consequences: it will cause the rsyslog engine to go into a
 * tight loop. That obviously is not acceptable. As such, we track the count of
 * iterations that a tryResume returning RS_RET_OK is immediately followed by an
 * unsuccessful call to doAction(). If that happens more than 10 times, we
 * assume the return acutally is a RS_RET_SUSPENDED. In order to go through the
 * various resumption stages, we do this for every 10 requests. This magic
 * number 10 may not be the most appropriate, but it should be thought of a "if
 * nothing else helps" kind of facility: in the first place, the module should
 * return a proper indication of its inability to recover. -- rgerhards,
 * 2010-04-26.
 */
static rsRetVal actionDoRetry(action_t* const pThis, wti_t* const pWti)
{
    int iRetries;
    int iSleepPeriod;
    int bTreatOKasSusp;
           ...
           getActionState(pWti, pThis) == ACT_STATE_RTRY)
    {
        ...
        else if (iRet == RS_RET_SUSPENDED || bTreatOKasSusp)
        {
            ...
            if ((pThis->iResumeRetryCount != -1 &&
                 iRetries >= pThis->iResumeRetryCount))
            {
...
static rsRetVal actionCheckAndCreateWrkrInstance(action_t* const pThis,
                                                 const wti_t* const pWti)
{
    int locked = 0;
    DEFiRet;
    if (pWti->actWrkrInfo[pThis->iActionNbr].actWrkrData == NULL)
    {
        ...
        CHKiRet(pThis->pMod->mod.om.createWrkrInstance(
            &(pWti->actWrkrInfo[pThis->iActionNbr].actWrkrData),
            pThis->pModData));
...
static rsRetVal actionTryResume(action_t* const pThis, wti_t* const pWti)
{
    ...
    if (getActionState(pWti, pThis) == ACT_STATE_SUSP)
    {
         ...
         * point where a resumption is actually tried, because the action
         * timestamp is always in the past. So we can not avoid doing a fresh
         * time() call here. -- rgerhards, 2009-03-18
         */
                  ...
                  getActionState(pWti, pThis) == ACT_STATE_SUSP))
    {
        if (ttNow == NO_TIME_PROVIDED) /* use cached result if we have it */
            datetime.GetTime(&ttNow);
        dbgprintf("actionTryResume: action[%s] state: %s, next retry (if "
                  "applicable): %u [now %u]\n",
 ...
 * rgerhards, 2009-05-07
 */
static rsRetVal actionPrepare(action_t* __restrict__ const pThis,
                              wti_t* __restrict__ const pWti)
{
    ...
    if (getActionState(pWti, pThis) == ACT_STATE_RDY)
    {
        iRet = pThis->pMod->mod.om.beginTransaction(
            pWti->actWrkrInfo[pThis->iActionNbr].actWrkrData);
        switch (iRet)
        {
            case RS_RET_OK:
        ...
        for (i = 0; i < pAction->iNumTpls; ++i)
        {
            switch (pAction->peParamPassing[i])
            {
                ...
                case ACT_JSON_PASSING:
                    ...
                    break;
                default:
                    dbgprintf("software bug/error: unknown "
                              "pAction->peParamPassing[%d] %d in "
                              "prepareDoActionParams\n",
...
PRAGMA_IGNORE_Wcast_align void
    releaseDoActionParams(action_t* __restrict__ const pAction,
                          wti_t* __restrict__ const pWti, int action_destruct)
{
    int j;
    actWrkrInfo_t* __restrict__ pWrkrInfo;
    ...
    for (j = 0; j < pAction->iNumTpls; ++j)
    {
        ...
        else
        {
            switch (pAction->peParamPassing[j])
            {
                ...
                case ACT_JSON_PASSING:
                    json_object_put(
                        (struct json_object*)pWrkrInfo->p.nontx.actParams[j]
                            .param);
                    pWrkrInfo->p.nontx.actParams[j].param = NULL;
                    break;
                case ACT_STRING_PASSING:
 ...
 */
static void actionSetActionWorked(action_t* __restrict__ const pThis,
                                  wti_t* __restrict__ const pWti)
{
    setActionResumeInRow(pWti, pThis, 0);
}
                                       ...
                                       const rsRetVal ret)
{
    ...
    switch (ret)
    {
        ...
        case RS_RET_SUSPENDED:
            ...
            break;
        default
            : /* error happened - if it hits us here, we assume the message
               * cannot be processed but an retry makes no sense. Usually, this
               * should be return code RS_RET_DATAFAIL. -- rgerhards, 2017-10-06
               */
            LogError(0, ret,
                     "action '%s' (module '%s') "
                                   ...
                                   wti_t* __restrict__ const pWti)
{
    ...
    iRet = pThis->pMod->mod.om.doAction(
        param, pWti->actWrkrInfo[pThis->iActionNbr].actWrkrData);
...
static rsRetVal
    actionCallCommitTransaction(action_t* const pThis, wti_t* const pWti,
                                actWrkrIParams_t* __restrict__ const iparams,
                                const int nparams)
{
    DEFiRet;
    ...
    iRet = pThis->pMod->mod.om.commitTransaction(
        pWti->actWrkrInfo[pThis->iActionNbr].actWrkrData, iparams, nparams);
 ...
 * rgerhards, 2008-01-28
 */
static rsRetVal actionProcessMessage(action_t* const pThis, void* actParams,
                                     wti_t* const pWti)
{
    DEFiRet;
    ...
    if (pThis->pMod->mod.om.SetShutdownImmdtPtr != NULL)
        pThis->pMod->mod.om.SetShutdownImmdtPtr(pThis->pModData,
                                                pWti->pbShutdownImmediate);
                              ...
                              const int nparams)
{
    ...
    {
        DBGPRINTF("doTransaction: have commitTransaction IF, using that, "
                  "pWrkrInfo %p\n",
                  wrkrInfo);
        CHKiRet(actionCallCommitTransaction(pThis, pWti, iparams, nparams));
    }
    else
    { /* note: this branch is for compatibility with old TX modules */
        DBGPRINTF("doTransaction: action '%s', currIParam %d\n", pThis->pszName,
                  wrkrInfo->p.tx.currIParam);
        for (i = 0; i < nparams; ++i)
        {
            /* Note: we provide the message's base iparam -
             * actionProcessMessage() uses this as *base* address.
             */
            iRet = actionProcessMessage(
                pThis, &actParam(iparams, pThis->iNumTpls, i, 0), pWti);
            DBGPRINTF(
                "doTransaction: action %d, processing msg %d, result %d\n",
            ...
            else if (iRet != RS_RET_DEFER_COMMIT &&
                     iRet != RS_RET_PREVIOUS_COMMITTED && iRet != RS_RET_OK)
            {
...
static rsRetVal ATTR_NONNULL()
    actionTryCommit(action_t* __restrict__ const pThis,
                    wti_t* __restrict__ const pWti,
                    actWrkrIParams_t* __restrict__ const iparams,
                    const int nparams)
{
    ...
    if (getActionState(pWti, pThis) == ACT_STATE_ITX)
    {
        iRet = pThis->pMod->mod.om.endTransaction(
            pWti->actWrkrInfo[pThis->iActionNbr].actWrkrData);
        switch (iRet)
        {
            case RS_RET_OK:
            ...
            case RS_RET_DISABLE_ACTION:
                ...
                break;
            case RS_RET_DEFER_COMMIT:
                DBGPRINTF("output plugin error: endTransaction() returns "
                          "RS_RET_DEFER_COMMIT "
                          "- ignored\n");
                actionCommitted(pThis, pWti);
                break;
            case RS_RET_PREVIOUS_COMMITTED:
                ...
                actionCommitted(pThis, pWti);
                break;
            default: /* permanent failure of this message - no sense in
                      * retrying. This is not yet handled (but easy TODO)
                      */
                         ...
                         const int nparams)
{
    ...
    DBGPRINTF("action %d commit failed, writing %u messages (%d tpls) to error "
              "file\n",
    ...
    {
        pThis->fdErrFile =
            open(pThis->pszErrFile,
                 O_WRONLY | O_CREAT | O_APPEND | O_LARGEFILE | O_CLOEXEC,
                 S_IRUSR | S_IWUSR | S_IRGRP | S_IWGRP);
        if (pThis->fdErrFile == -1)
        {
            LogError(errno, RS_RET_ERR,
                     "action %s: error opening error file %s", pThis->pszName,
                     pThis->pszErrFile);
            goto done;
        }
    }
    ...
    for (int i = 0; i < nparams; ++i)
    {
        if ((etry = fjson_object_new_object()) == NULL)
            goto done;
        fjson_object_object_add(etry, "action",
                                fjson_object_new_string((char*)pThis->pszName));
        ...
        for (int j = 0; j < pThis->iNumTpls; ++j)
        {
            ...
            fjson_object_object_add(
                etry, tplname,
                fjson_object_new_string(
                    (char*)actParam(iparams, 1, i, j).param));
        }
...
static rsRetVal actionTryRemoveHardErrorsFromBatch(
    action_t* __restrict__ const pThis, wti_t* __restrict__ const pWti,
    actWrkrIParams_t* const new_iparams, unsigned* new_nMsgs)
{
    ...
    *new_nMsgs = 0;
    ...
    {
        setActionResumeInRow(
            pWti, pThis,
            0); // make sure we do not trigger OK-as-SUSPEND handling
        memcpy(&oneParamSet,
 ...
 */
static rsRetVal ATTR_NONNULL() actionCommit(action_t* __restrict__ const pThis,
                                            wti_t* __restrict__ const pWti)
{
    actWrkrInfo_t* const wrkrInfo = &(pWti->actWrkrInfo[pThis->iActionNbr]);
    ...
    int needfree_iparams =
        0; // work-around for clang static analyzer false positive
    DEFiRet;
     ...
     */
    iRet = actionTryCommit(pThis, pWti, wrkrInfo->p.tx.iparams,
                           wrkrInfo->p.tx.currIParam);
     ...
     * are done. If it is a multi-message batch, we need to sort out the
    ...
    {
        DBGPRINTF("actionCommit[%s]: somewhat unhappy, full batch of %d msgs "
                  "returned "
     ...
     */
    DBGPRINTF(
        "actionCommit[%s]: unhappy, we still have %d uncommited messages.\n",
        pThis->pszName, nMsgs);
    int bDone = 0;
    do
    {
        ...
        else if (iRet == RS_RET_OK || iRet == RS_RET_SUSPENDED ||
                 iRet == RS_RET_ACTION_FAILED)
        {
            bDone = 1;
        }
                               ...
                               struct syslogTime* ttNow)
{
    ...
    iRet = actionProcessMessage(
        pAction, pWti->actWrkrInfo[pAction->iActionNbr].p.nontx.actParams,
        pWti);
 ...
 */
static rsRetVal processBatchMain(void* __restrict__ const pVoid,
                                 batch_t* __restrict__ const pBatch,
                                 wti_t* __restrict__ const pWti)
{
    action_t* __restrict__ const pAction =
        (action_t * __restrict__ const) pVoid;
    int i;
    struct syslogTime ttNow;
    DEFiRet;
    ...
    for (i = 0; i < batchNumMsgs(pBatch) && !*pWti->pbShutdownImmediate; ++i)
    {
        if (batchIsValidElem(pBatch, i))
        {
            ...
            DBGPRINTF("processBatchMain: i %d, processMsgMain iRet %d\n", i,
                      localRet);
            if (localRet == RS_RET_OK || localRet == RS_RET_DEFER_COMMIT ||
 ...
 * rgerhards, 2008-01-28
 */
static rsRetVal setActionQueType(void __attribute__((unused)) * pVal,
                                 uchar* pszType)
{
    DEFiRet;
    ...
    {
        LogError(0, RS_RET_INVALID_PARAMS, "unknown actionqueue parameter: %s",
                 (char*)pszType);
...
/* This submits the message to the action queue in case we do NOT need to handle
 * repeat message processing. That case permits us to gain lots of freedom
 * during processing and thus speed. This is also utilized to submit messages in
 * more complex cases once the complex logic has been applied ;) rgerhards,
 * 2010-06-08
 */
static rsRetVal doSubmitToActionQ(action_t* const pAction, wti_t* const pWti,
                                  smsg_t* pMsg)
{
    ...
    { /* in this case, we do single submits to the queue.
 ...
 */
rsRetVal actionWriteToAction(action_t* const pAction, smsg_t* pMsg,
                             wti_t* const pWti)
{
    DEFiRet;

    /* first, we check if the action should actually be called. The
     * action-specific $ActionExecOnlyEveryNthTime permits us to execute an
     * action only every Nth time. So we need to check if we need to drop the
     * (otherwise perfectly executable) action for this reason. Note that in
     * case we need to drop it, we return RS_RET_OK as the action was properly
     * "passed to execution" from the upper layer's point of view. -- rgerhards,
     * 2008-08-07.
     */
    if (pAction->iExecEveryNthOccur > 1)
    {
        ...
        if (pAction->iExecEveryNthOccurTO > 0 &&
            (getActNow(pAction) - pAction->tLastOccur) >
                pAction->iExecEveryNthOccurTO)
        {
            DBGPRINTF("n-th occurence handling timed out (%d sec), restarting "
                      "from 0\n",
        ...
        {
            ++pAction->iNbrNoExec;
            DBGPRINTF("action %p passed %d times to execution - less than "
                      "configured - discarding\n",
                      pAction, pAction->iNbrNoExec);
            FINALIZE;
        }
        else
        {
            pAction->iNbrNoExec = 0; /* we execute the action now, so the number
                                        of no execs is down to */
        }
    }
    ...
    /* now check if we need to drop the message because otherwise the action
     * would be too frequently called. -- rgerhards, 2008-04-08 Note that the
     * check for "pAction->iSecsExecOnceInterval > 0" is not necessary from a
     * purely logical point of view. However, if safes us to check the system
     * time in (those common) cases where ExecOnceInterval is not used. --
     * rgerhards, 2008-09-16
     */
    if (pAction->iSecsExecOnceInterval > 0 &&
        pAction->iSecsExecOnceInterval + pAction->tLastExec >
            getActNow(pAction))
    {
        /* in this case we need to discard the message - its not yet time to
         * exec the action */
        DBGPRINTF("action not yet ready again to be executed, onceInterval %d, "
                  "tCurr %d, tNext %d\n",
    ...
    /* we use reception time, not dequeue time - this is considered more
     * appropriate and also faster ;) rgerhards, 2008-09-17 */
...
/* Call configured action, most complex case with all features supported (and
 * thus slow). rgerhards, 2010-06-08
 */

PRAGMA_DIAGNOSTIC_PUSH;
PRAGMA_IGNORE_Wempty_body;
static rsRetVal doSubmitToActionQComplex(action_t* const pAction,
                                         wti_t* const pWti, smsg_t* pMsg)
{
    DEFiRet;
    ...
    pAction->tActNow =
        -1; /* we do not yet know our current time (clear prev. value) */
...
/* This function "activates" the action after privileges have been dropped.
 * Currently, this means that the queues are started. rgerhards, 2011-05-02
 */
 ...
 * bWriteAllMarkMessage == RSFALSE only. Note that we use a non-blocking CAS
 * loop for the synchronization. Here, we just modify the filter condition to be
 * false when a mark message must not be written. However, in this case we must
 * save the previous filter as we may need it in the next action (potential
 * future optimization: check if this is the last action TODO). rgerhards,
 * 2010-06-08
 */
static rsRetVal doSubmitToActionQNotAllMark(action_t* const pAction,
                                            wti_t* const pWti,
                                            smsg_t* const pMsg)
{
    ...
    /* TODO: think about the whole logic. If messages come in out of order,
     * things tend to become a bit unreliable. On the other hand, this only
     * happens if we have very high traffic, in which this use case here is not
     * really affected (as the MarkInterval is pretty corase).
     */
    ...
    do
    {
        ...
        if (pMsg->msgFlags & MARK)
        {
            if ((pMsg->ttGenTime - lastAct) < MarkInterval / 2)
            {
                doProcess = 0;
                DBGPRINTF(
                    "action was recently called, ignoring mark message\n");
                break; /* do not update timestamp for non-written mark messages
                        */
            }
        }
    } while (ATOMIC_CAS_time_t(&pAction->f_time, lastAct, pMsg->ttGenTime,
                               &pAction->mutCAS) == 0);
...
/* apply all params from param block to action. This supports the v6 config
 * system. Defaults must have been set appropriately during action construct!
 * rgerhards, 2011-08-01
 */
static rsRetVal actionApplyCnfParam(action_t* const pAction,
    ...
    for (i = 0; i < pblk.nParams; ++i)
    {
        ...
        else if (!strcmp(pblk.descr[i].name, "action.execonlyeverynthtime"))
        {
            pAction->iExecEveryNthOccur = pvals[i].val.d.n;
                         ...
                         "action.execonlyeverynthtimetimeout"))
        {
 ...
 */
rsRetVal addAction(action_t** ppAction, modInfo_t* pMod, void* pModData,
                   ...
                   struct nvlst* const lst)
{
    ...
    /* check if we can obtain the template pointers - TODO: move to separate
     * function? */
    pAction->iNumTpls = OMSRgetEntryCount(pOMSR);
    assert(pAction->iNumTpls >=
           0); /* only debug check because this "can not happen" */
    /* please note: iNumTpls may validly be zero. This is the case if the module
     * does not request any templates. This sounds unlikely, but an actual
     * example is the discard action, which does not require a string. --
     * rgerhards, 2007-07-30
     */
    if (pAction->iNumTpls > 0)
    {
        /* we first need to create the template arrays */
        CHKmalloc(pAction->ppTpl = (struct template**)calloc(
                      pAction->iNumTpls, sizeof(struct template*)));
        CHKmalloc(pAction->peParamPassing = (paramPassing_t*)calloc(
                      pAction->iNumTpls, sizeof(paramPassing_t)));
    }
...
rsRetVal actionNewInst(struct nvlst* lst, action_t** ppAction)
{
    ...
    cnfModName = (uchar*)es_str2cstr(
        paramvals[cnfparamGetIdx(&pblk, ("type"))].val.d.estr, NULL);
    if ((pMod = module.FindWithCnfName(loadConf, cnfModName, eMOD_OUT)) == NULL)
    {
        LogError(0, RS_RET_MOD_UNKNOWN, "module name '%s' is unknown",
                 cnfModName);
...
rsRetVal actionClassInit(void)
{
    ...
    CHKiRet(objGetObjInterface(
        &obj)); /* this provides the root pointer for all other queries */
    CHKiRet(objUse(datetime, CORE_COMPONENT));
    CHKiRet(objUse(module, CORE_COMPONENT));
    CHKiRet(objUse(statsobj, CORE_COMPONENT));
    CHKiRet(objUse(ruleset, CORE_COMPONENT));

    CHKiRet(regCfSysLineHdlr((uchar*)"actionname", 0, eCmdHdlrGetWord, NULL,
                             &cs.pszActionName, NULL));
    CHKiRet(regCfSysLineHdlr((uchar*)"actionqueuefilename", 0, eCmdHdlrGetWord,
                             NULL, &cs.pszActionQFName, NULL));
    CHKiRet(regCfSysLineHdlr((uchar*)"actionqueuesize", 0, eCmdHdlrInt, NULL,
                             &cs.iActionQueueSize, NULL));
    CHKiRet(regCfSysLineHdlr((uchar*)"actionwriteallmarkmessages", 0,
                             eCmdHdlrBinary, NULL, &cs.bActionWriteAllMarkMsgs,
                             NULL));
    CHKiRet(regCfSysLineHdlr((uchar*)"actionqueuedequeuebatchsize", 0,
                             eCmdHdlrInt, NULL, &cs.iActionQueueDeqBatchSize,
                             NULL));
    CHKiRet(regCfSysLineHdlr((uchar*)"actionqueuemaxdiskspace", 0, eCmdHdlrSize,
                             NULL, &cs.iActionQueMaxDiskSpace, NULL));
    CHKiRet(regCfSysLineHdlr((uchar*)"actionqueuehighwatermark", 0, eCmdHdlrInt,
                             NULL, &cs.iActionQHighWtrMark, NULL));
    CHKiRet(regCfSysLineHdlr((uchar*)"actionqueuelowwatermark", 0, eCmdHdlrInt,
                             NULL, &cs.iActionQLowWtrMark, NULL));
    CHKiRet(regCfSysLineHdlr((uchar*)"actionqueuediscardmark", 0, eCmdHdlrInt,
                             NULL, &cs.iActionQDiscardMark, NULL));
    CHKiRet(regCfSysLineHdlr((uchar*)"actionqueuediscardseverity", 0,
                             eCmdHdlrInt, NULL, &cs.iActionQDiscardSeverity,
                             NULL));
    CHKiRet(regCfSysLineHdlr((uchar*)"actionqueuecheckpointinterval", 0,
                             eCmdHdlrInt, NULL, &cs.iActionQPersistUpdCnt,
                             NULL));
    CHKiRet(regCfSysLineHdlr((uchar*)"actionqueuesyncqueuefiles", 0,
                             eCmdHdlrBinary, NULL, &cs.bActionQSyncQeueFiles,
                             NULL));
    CHKiRet(regCfSysLineHdlr((uchar*)"actionqueuetype", 0, eCmdHdlrGetWord,
                             setActionQueType, NULL, NULL));
    CHKiRet(regCfSysLineHdlr((uchar*)"actionqueueworkerthreads", 0, eCmdHdlrInt,
                             NULL, &cs.iActionQueueNumWorkers, NULL));
    CHKiRet(regCfSysLineHdlr((uchar*)"actionqueuetimeoutshutdown", 0,
                             eCmdHdlrInt, NULL, &cs.iActionQtoQShutdown, NULL));
    CHKiRet(regCfSysLineHdlr((uchar*)"actionqueuetimeoutactioncompletion", 0,
                             eCmdHdlrInt, NULL, &cs.iActionQtoActShutdown,
                             NULL));
    CHKiRet(regCfSysLineHdlr((uchar*)"actionqueuetimeoutenqueue", 0,
                             eCmdHdlrInt, NULL, &cs.iActionQtoEnq, NULL));
    CHKiRet(regCfSysLineHdlr((uchar*)"actionqueueworkertimeoutthreadshutdown",
                             0, eCmdHdlrInt, NULL, &cs.iActionQtoWrkShutdown,
                             NULL));
    CHKiRet(regCfSysLineHdlr((uchar*)"actionqueueworkerthreadminimummessages",
                             0, eCmdHdlrInt, NULL, &cs.iActionQWrkMinMsgs,
                             NULL));
    CHKiRet(regCfSysLineHdlr((uchar*)"actionqueuemaxfilesize", 0, eCmdHdlrSize,
                             NULL, &cs.iActionQueMaxFileSize, NULL));
    CHKiRet(regCfSysLineHdlr((uchar*)"actionqueuesaveonshutdown", 0,
                             eCmdHdlrBinary, NULL, &cs.bActionQSaveOnShutdown,
                             NULL));
    CHKiRet(regCfSysLineHdlr((uchar*)"actionqueuedequeueslowdown", 0,
                             eCmdHdlrInt, NULL, &cs.iActionQueueDeqSlowdown,
                             NULL));
    CHKiRet(regCfSysLineHdlr((uchar*)"actionqueuedequeuetimebegin", 0,
                             eCmdHdlrInt, NULL, &cs.iActionQueueDeqtWinFromHr,
                             NULL));
    CHKiRet(regCfSysLineHdlr((uchar*)"actionqueuedequeuetimeend", 0,
                             eCmdHdlrInt, NULL, &cs.iActionQueueDeqtWinToHr,
                             NULL));
    CHKiRet(regCfSysLineHdlr((uchar*)"actionexeconlyeverynthtime", 0,
                             eCmdHdlrInt, NULL, &cs.iActExecEveryNthOccur,
                             NULL));
    CHKiRet(regCfSysLineHdlr((uchar*)"actionexeconlyeverynthtimetimeout", 0,
                             eCmdHdlrInt, NULL, &cs.iActExecEveryNthOccurTO,
                             NULL));
    CHKiRet(regCfSysLineHdlr((uchar*)"actionexeconlyonceeveryinterval", 0,
                             eCmdHdlrInt, NULL, &cs.iActExecOnceInterval,
                             NULL));
    CHKiRet(regCfSysLineHdlr((uchar*)"repeatedmsgcontainsoriginalmsg", 0,
                             eCmdHdlrBinary, NULL, &cs.bActionRepMsgHasMsg,
                             NULL));
    CHKiRet(regCfSysLineHdlr((uchar*)"actionexeconlywhenpreviousissuspended", 0,
                             eCmdHdlrBinary, NULL, &cs.bActExecWhenPrevSusp,
                             NULL));
    CHKiRet(regCfSysLineHdlr((uchar*)"actionresumeretrycount", 0, eCmdHdlrInt,
                             NULL, &cs.glbliActionResumeRetryCount, NULL));
    CHKiRet(regCfSysLineHdlr((uchar*)"resetconfigvariables", 1,
                             eCmdHdlrCustomHandler, resetConfigVariables, NULL,
                             NULL));
