...
_public_ int sd_bus_emit_signal(sd_bus* bus, const char* path,
...
_public_ int sd_bus_call_method_async(sd_bus* bus, sd_bus_slot** slot,
                                      ...
                                      void* userdata, const char* types, ...)
{
    ...
    r = sd_bus_message_new_method_call(bus, &m, destination, path, interface,
                                       member);
    if (r < 0)
        return r;
...
_public_ int sd_bus_call_method(sd_bus* bus, const char* destination,
...
_public_ int sd_bus_reply_method_error(sd_bus_message* call,
...
_public_ int sd_bus_reply_method_errorf(sd_bus_message* call, const char* name,
                                        const char* format, ...)
{
...
_public_ int sd_bus_reply_method_errno(sd_bus_message* call, int error,
...
_public_ int sd_bus_reply_method_errnof(sd_bus_message* call, int error,
                                        const char* format, ...)
{
...
_public_ int sd_bus_get_property(sd_bus* bus, const char* destination,
                                 const char* path, const char* interface,
                                 const char* member, sd_bus_error* error,
                                 sd_bus_message** reply, const char* type)
{
    ...
    bus_assert_return(bus, -EINVAL, error);
    bus_assert_return(isempty(interface) || interface_name_is_valid(interface),
                      -EINVAL, error);
    ...
    r = sd_bus_call_method(bus, destination, path,
                           ...
                           &rep, "ss", strempty(interface), member);
    if (r < 0)
        return r;
...
_public_ int sd_bus_get_property_trivial(sd_bus* bus, const char* destination,
                                         const char* path,
                                         const char* interface,
                                         const char* member,
                                         sd_bus_error* error, char type,
                                         void* ptr)
{
    ...
    r = sd_bus_call_method(bus, destination, path,
                           "org.freedesktop.DBus.Properties", "Get", error,
                           &reply, "ss", strempty(interface), member);
    if (r < 0)
        return r;
...
_public_ int sd_bus_get_property_string(sd_bus* bus, const char* destination,
                                        const char* path, const char* interface,
                                        const char* member, sd_bus_error* error,
                                        char** ret)
{
...
_public_ int sd_bus_get_property_strv(sd_bus* bus, const char* destination,
                                      const char* path, const char* interface,
                                      const char* member, sd_bus_error* error,
                                      char*** ret)
{
...
_public_ int sd_bus_set_property(sd_bus* bus, const char* destination,
                                 const char* path, const char* interface,
                                 const char* member, sd_bus_error* error,
                                 const char* type, ...)
{
...
_public_ int sd_bus_query_sender_creds(sd_bus_message* call, uint64_t mask,
                                       sd_bus_creds** creds)
{
    sd_bus_creds* c;
...
_public_ int sd_bus_query_sender_privilege(sd_bus_message* call, int capability)
{
    ...
    else
    {
        r = sd_bus_query_sender_creds(
            call, SD_BUS_CREDS_UID | SD_BUS_CREDS_EUID, &creds);
        if (r < 0)
            return r;
    }
...
#define make_expression(sender, path, interface, member)                       \
    strjoina("type='signal'", sender ? ",sender='" : "", sender ?: "",         \
             sender ? "'" : "", path ? ",path='" : "", path ?: "",             \
             path ? "'" : "", interface ? ",interface='" : "",                 \
             interface ?: "", interface ? "'" : "", member ? ",member='" : "", \
             member ?: "", member ? "'" : "")

_public_ int sd_bus_match_signal(sd_bus* bus, sd_bus_slot** ret,
...
_public_ int sd_bus_match_signal_async(
    sd_bus* bus, sd_bus_slot** ret, const char* sender, const char* path,
    const char* interface, const char* member,
    sd_bus_message_handler_t callback,
    sd_bus_message_handler_t install_callback, void* userdata)
{
    ...
    return sd_bus_add_match_async(bus, ret, expression, callback,
