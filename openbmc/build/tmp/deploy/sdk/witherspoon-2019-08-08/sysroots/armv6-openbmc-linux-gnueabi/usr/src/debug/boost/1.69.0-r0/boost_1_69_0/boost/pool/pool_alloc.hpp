...
<tr><td><tt>PoolAlloc::allocate()</tt></td><td><tt>T
*</tt></td><td><tt>PoolAlloc::allocate(1)</tt></tr>
<tr><td><tt>PoolAlloc::deallocate(p)</tt></td><td>void</tt></td><td><tt>PoolAlloc::deallocate(p,
1)</tt></tr>
</table>

The typedef user_allocator publishes the value of the UserAllocator template
parameter.

<b>Notes</b>

If the allocation functions run out of memory, they will throw
<tt>std::bad_alloc</tt>.

The underlying Pool type used by the allocators is accessible through the
Singleton Pool Interface. The identifying tag used for pool_allocator is
pool_allocator_tag, and the tag used for fast_pool_allocator is
fast_pool_allocator_tag. All template parameters of the allocators (including
implementation-specific ones) determine the type of the underlying Pool, with
the exception of the first parameter T, whose size is used instead.
...
<tt>sizeof(int) == sizeof(void *)</tt>, <tt>pool_allocator<int></tt> and
<tt>pool_allocator<void *></tt> will both allocate/deallocate from/to the same
pool.
...
/*!  \brief A C++ Standard Library conforming allocator, based on an underlying
  pool.
  ...
  <b>UserAllocator</B>. Defines the method that the underlying Pool will use to
  allocate memory from the system. See <a
  href="boost_pool/pool/pooling.html#boost_pool.pool.pooling.user_allocator">User
  Allocators</a> for details.

  <b>Mutex</b> Allows the user to determine the type of synchronization to be
  used on the underlying singleton_pool.

  <b>NextSize</b> The value of this parameter is passed to the underlying
  singleton_pool when it is created.
...
class pool_allocator
{
  public:
    ...
    typedef UserAllocator
        user_allocator;  //!< allocator that defines the method that the
                         //!< underlying Pool will use to allocate memory from
                         //!< the system.
    typedef Mutex mutex; //!< typedef mutex publishes the value of the template
                         //!< parameter Mutex.
    BOOST_STATIC_CONSTANT(
        unsigned, next_size = NextSize); //!< next_size publishes the values of
                                         //!< the template parameter NextSize.
    ...
    struct rebind
    { //
        typedef pool_allocator<U, UserAllocator, Mutex, NextSize, MaxSize>
            other;
    };

  public:
    pool_allocator()
    { /*! Results in default construction of the underlying singleton_pool IFF
       an instance of this allocator is constructed during global initialization
       ( required to ensure construction of singleton_pool IFF an instance of
       this allocator is constructed during global initialization. See ticket
    ...
    template <typename U>
    pool_allocator(
        const pool_allocator<U, UserAllocator, Mutex, NextSize, MaxSize>&)
    { /*! Results in the default construction of the underlying singleton_pool,
         this is required to ensure construction of singleton_pool IFF an
    ...
    static pointer allocate(const size_type n)
    {
        ...
        std::cout << "Allocating " << n << " * " << sizeof(T)
                  ...
                  << debug_info<true>::allocated << std::endl;
#endif
...
Specialization of pool_allocator for type void: required by the standard to make
this a conforming allocator type.
*/
...
//! Simple tag type used by fast_pool_allocator as a template parameter to the
//! underlying singleton_pool.
struct fast_pool_allocator_tag
{
};

/*! \brief A C++ Standard Library conforming allocator geared towards allocating
 single chunks.

 While class template <tt>pool_allocator</tt> is a more general-purpose solution
 geared towards efficiently servicing requests for any number of contiguous
 chunks, <tt>fast_pool_allocator</tt> is also a general-purpose solution, but is
 geared towards efficiently servicing requests for one chunk at a time; it will
 work for contiguous chunks, but not as well as <tt>pool_allocator</tt>.

 If you are seriously concerned about performance,
 use <tt>fast_pool_allocator</tt> when dealing with containers such as
 <tt>std::list</tt>, and use <tt>pool_allocator</tt> when dealing with
 containers such as <tt>std::vector</tt>.
 ...
 <b>Mutex</b> Allows the user to determine the type of synchronization to be
 used on the underlying <tt>singleton_pool</tt>.

 <b>NextSize</b> The value of this parameter is passed to the underlying Pool
 when it is created.
...
class fast_pool_allocator
{
  public:
    ...
    struct rebind
    {
        typedef fast_pool_allocator<U, UserAllocator, Mutex, NextSize, MaxSize>
            other;
    };
  ...
  public:
    fast_pool_allocator()
    {
        ...
        //! initialization. See ticket #2359 for a complete explanation
        //! at http://svn.boost.org/trac/boost/ticket/2359 .
        singleton_pool<fast_pool_allocator_tag, sizeof(T), UserAllocator, Mutex,
                       NextSize, MaxSize>::is_from(0);
    }
    ...
    { //! Allocate memory.
        const pointer ret = static_cast<pointer>(
            (singleton_pool<fast_pool_allocator_tag, sizeof(T), UserAllocator,
                            Mutex, NextSize, MaxSize>::malloc)());
        if (ret == 0)
            boost::throw_exception(std::bad_alloc());
        return ret;
    }
    ...
    { //! Deallocate memory.
        ...
        if (n == 1)
            (singleton_pool<fast_pool_allocator_tag, sizeof(T), UserAllocator,
                            Mutex, NextSize, MaxSize>::free)(ptr);
        else
            (singleton_pool<fast_pool_allocator_tag, sizeof(T), UserAllocator,
                            Mutex, NextSize, MaxSize>::free)(ptr, n);
    }
...
Specialization of fast_pool_allocator<void> required to make the allocator
standard-conforming.
*/
template <typename UserAllocator, typename Mutex, unsigned NextSize,
          unsigned MaxSize>
