 ...
 *     is_big = TRUE;
 *   #endif
 *     return is_big ? *(((gpointer *) a) + index) : GUINT_TO_POINTER (*(((guint
 * *) a) + index));
 *
 * ...instead of this...
 *
...
static inline gpointer
    g_hash_table_realloc_key_or_value_array(gpointer a, guint size,
                                            G_GNUC_UNUSED gboolean is_big)
{
...
static inline gpointer g_hash_table_fetch_key_or_value(gpointer a, guint index,
                                                       gboolean is_big)
{
#ifndef USE_SMALL_ARRAYS
    is_big = TRUE;
#endif
    return is_big ? *(((gpointer*)a) + index)
                  : GUINT_TO_POINTER(*(((guint*)a) + index));
}

static inline void g_hash_table_assign_key_or_value(gpointer a, guint index,
                                                    gboolean is_big, gpointer v)
{
#ifndef USE_SMALL_ARRAYS
    is_big = TRUE;
#endif
...
static inline gpointer g_hash_table_evict_key_or_value(gpointer a, guint index,
                                                       gboolean is_big,
                                                       gpointer v)
{
#ifndef USE_SMALL_ARRAYS
...
static inline guint g_hash_table_hash_to_index(GHashTable* hash_table,
                                               guint hash)
{
                                             ...
                                             guint* hash_return)
{
     ...
     * table is empty prior to removing the last reference using
     * g_hash_table_unref(). */
 ...
 */
static void g_hash_table_remove_node(GHashTable* hash_table, gint i,
                                     gboolean notify)
{
    gpointer key;
    gpointer value;

    key = g_hash_table_fetch_key_or_value(hash_table->keys, i,
                                          hash_table->have_big_keys);
    value = g_hash_table_fetch_key_or_value(hash_table->values, i,
    ...
    /* Be GC friendly */
    g_hash_table_assign_key_or_value(hash_table->keys, i,
                                     hash_table->have_big_keys, NULL);
    g_hash_table_assign_key_or_value(hash_table->values, i,
                                     hash_table->have_big_values, NULL);
 ...
 */
static void g_hash_table_setup_storage(GHashTable* hash_table)
{
    gboolean small;
    ...
    hash_table->keys = g_hash_table_realloc_key_or_value_array(
        NULL, hash_table->size, hash_table->have_big_keys);
...
static void g_hash_table_remove_all_nodes(GHashTable* hash_table,
                                          gboolean notify, gboolean destruction)
{
    int i;
    gpointer key;
    gpointer value;
...
#ifdef USE_SMALL_ARRAYS
            memset(hash_table->keys, 0,
                   hash_table->size * (hash_table->have_big_keys
                                           ? BIG_ENTRY_SIZE
                                           : SMALL_ENTRY_SIZE));
            memset(hash_table->values, 0,
                   hash_table->size * (hash_table->have_big_values
                                           ? BIG_ENTRY_SIZE
                                           : SMALL_ENTRY_SIZE));
#else
    ...
    for (i = 0; i < old_size; i++)
    {
        ...
        {
            key =
                g_hash_table_fetch_key_or_value(old_keys, i, old_have_big_keys);
            value = g_hash_table_fetch_key_or_value(old_values, i,
                                                    old_have_big_values);

            old_hashes[i] = UNUSED_HASH_VALUE;

            g_hash_table_assign_key_or_value(old_keys, i, old_have_big_keys,
                                             NULL);
            g_hash_table_assign_key_or_value(old_values, i, old_have_big_values,
                                             NULL);
...
static void realloc_arrays(GHashTable* hash_table, gboolean is_a_set)
{
    hash_table->hashes = g_renew(guint, hash_table->hashes, hash_table->size);
    hash_table->keys = g_hash_table_realloc_key_or_value_array(
        hash_table->keys, hash_table->size, hash_table->have_big_keys);

    if (is_a_set)
        hash_table->values = hash_table->keys;
    else
        hash_table->values = g_hash_table_realloc_key_or_value_array(
...
static inline gboolean get_status_bit(const guint32* bitmap, guint index)
{
    return (bitmap[index / 32] >> (index % 32)) & 1;
}

static inline void set_status_bit(guint32* bitmap, guint index)
{
...
#define DEFINE_RESIZE_FUNC(fname)                                              \
    static void fname(GHashTable* hash_table, guint old_size,                  \
...
#define ASSIGN_KEYVAL(ht, index, key, value)                                   \
    G_STMT_START                                                               \
    {                                                                          \
        g_hash_table_assign_key_or_value((ht)->keys, (index),                  \
                                         (ht)->have_big_keys, (key));          \
        g_hash_table_assign_key_or_value((ht)->values, (index),                \
                                         (ht)->have_big_values, (value));      \
    }                                                                          \
    G_STMT_END
...
#define EVICT_KEYVAL(ht, index, key, value, outkey, outvalue)                  \
    G_STMT_START                                                               \
    {                                                                          \
        (outkey) = g_hash_table_evict_key_or_value(                            \
            (ht)->keys, (index), (ht)->have_big_keys, (key));                  \
        (outvalue) = g_hash_table_evict_key_or_value(                          \
            (ht)->values, (index), (ht)->have_big_values, (value));            \
    }                                                                          \
    G_STMT_END
...
static void g_hash_table_resize(GHashTable* hash_table)
{
    ...
    if (hash_table->size > old_size)
    {
        ...
        memset(&hash_table->hashes[old_size], 0,
               (hash_table->size - old_size) * sizeof(guint));

        reallocated_buckets_bitmap =
            g_new0(guint32, (hash_table->size + 31) / 32);
    }
    else
    {
...
static inline gboolean entry_is_big(gpointer v)
{
    return (((guintptr)v) >> ((BIG_ENTRY_SIZE - SMALL_ENTRY_SIZE) * 8)) != 0;
}

static inline gboolean g_hash_table_maybe_make_big_keys_or_values(gpointer* a_p,
                                                                  gpointer v,
                                                                  gint ht_size)
{
    if (entry_is_big(v))
...
static inline void g_hash_table_ensure_keyval_fits(GHashTable* hash_table,
                                                   gpointer key, gpointer value)
{
    ...
    if (is_a_set)
    {
        ...
        {
            if (key != value)
                hash_table->values = g_memdup(
                    hash_table->keys, sizeof(gpointer) * hash_table->size);
        ...
        else
        {
            if (key != value)
            {
                hash_table->values = g_memdup(hash_table->keys,
    ...
    if (!hash_table->have_big_keys)
    {
        hash_table->have_big_keys = g_hash_table_maybe_make_big_keys_or_values(
            &hash_table->keys, key, hash_table->size);
    ...
    {
        hash_table->have_big_values =
            g_hash_table_maybe_make_big_keys_or_values(&hash_table->values,
    ...
    if (is_a_set && key != value)
        hash_table->values =
            g_memdup(hash_table->keys, sizeof(gpointer) * hash_table->size);
 ...
 * the key used when removing the entry from the #GHashTable, or %NULL if you
 * don't want to supply such a function.
 * @value_destroy_func: (nullable): a function to free the memory allocated for
 * the value used when removing the entry from the #GHashTable, or %NULL if you
 * don't want to supply such a function.
 *
                                ...
                                gpointer* value)
{
    ...
    if (key != NULL)
        *key = g_hash_table_fetch_key_or_value(ri->hash_table->keys, position,
                                               ri->hash_table->have_big_keys);
    if (value != NULL)
        *value = g_hash_table_fetch_key_or_value(
            ri->hash_table->values, position, ri->hash_table->have_big_values);
 ...
 */
static gboolean g_hash_table_insert_node(GHashTable* hash_table,
                                         guint node_index, guint key_hash,
                                         gpointer new_key, gpointer new_value,
                                         ...
                                         gboolean reusing_key)
{
    ...
    if (already_exists)
    {
         ...
         * arrays are shared.
         */
        value_to_free = g_hash_table_fetch_key_or_value(
            hash_table->values, node_index, hash_table->have_big_values);

        if (keep_new_key)
        {
            key_to_free = g_hash_table_fetch_key_or_value(
                hash_table->keys, node_index, hash_table->have_big_keys);
            key_to_keep = new_key;
        }
        else
        {
            key_to_free = new_key;
            key_to_keep = g_hash_table_fetch_key_or_value(
                hash_table->keys, node_index, hash_table->have_big_keys);
        }
    }
    else
    ...
    g_hash_table_assign_key_or_value(hash_table->keys, node_index,
                                     hash_table->have_big_keys, key_to_keep);

    /* Step 3: Actually do the write */
    g_hash_table_assign_key_or_value(hash_table->values, node_index,
                                     hash_table->have_big_values, new_value);
...
void g_hash_table_iter_replace(GHashTableIter* iter, gpointer value)
{
    ...
    key = g_hash_table_fetch_key_or_value(ri->hash_table->keys, ri->position,
                                          ri->hash_table->have_big_keys);

    g_hash_table_insert_node(ri->hash_table, ri->position, node_hash, key,
                             value, TRUE, TRUE);
...
gpointer g_hash_table_lookup(GHashTable* hash_table, gconstpointer key)
{
    ...
    return HASH_IS_REAL(hash_table->hashes[node_index])
               ? g_hash_table_fetch_key_or_value(hash_table->values, node_index,
                                                 hash_table->have_big_values)
               : NULL;
}
                                      ...
                                      gpointer* orig_key, gpointer* value)
{
    ...
    if (orig_key)
        *orig_key = g_hash_table_fetch_key_or_value(
            hash_table->keys, node_index, hash_table->have_big_keys);

    if (value)
        *value = g_hash_table_fetch_key_or_value(hash_table->values, node_index,
                                                 hash_table->have_big_values);
...
static gboolean g_hash_table_remove_internal(GHashTable* hash_table,
                                             gconstpointer key, gboolean notify)
{
    guint node_index;
    guint node_hash;
                                     ...
                                     gpointer* stolen_value)
{
    ...
    if (stolen_key != NULL)
    {
        *stolen_key = g_hash_table_fetch_key_or_value(
            hash_table->keys, node_index, hash_table->have_big_keys);
        g_hash_table_assign_key_or_value(hash_table->keys, node_index,
                                         hash_table->have_big_keys, NULL);
    }
    ...
    if (stolen_value != NULL)
    {
        *stolen_value = g_hash_table_fetch_key_or_value(
            hash_table->values, node_index, hash_table->have_big_values);
        g_hash_table_assign_key_or_value(hash_table->values, node_index,
                                         hash_table->have_big_values, NULL);
    }
                                                  ...
                                                  gboolean notify)
{
    ...
    for (i = 0; i < hash_table->size; i++)
    {
        ...
        gpointer node_value = g_hash_table_fetch_key_or_value(
                                  ...
                                  gpointer user_data)
{
    ...
    return g_hash_table_foreach_remove_or_steal(hash_table, func, user_data,
                                                TRUE);
}
    ...
    for (i = 0; i < hash_table->size; i++)
    {
        if (HASH_IS_REAL(hash_table->hashes[i]))
            retval = g_list_prepend(
                retval, g_hash_table_fetch_key_or_value(
                            hash_table->keys, i, hash_table->have_big_keys));
    }
...
gpointer* g_hash_table_get_keys_as_array(GHashTable* hash_table, guint* length)
{
    ...
    {
        if (HASH_IS_REAL(hash_table->hashes[i]))
            result[j++] = g_hash_table_fetch_key_or_value(
                hash_table->keys, i, hash_table->have_big_keys);
    }
...
GList* g_hash_table_get_values(GHashTable* hash_table)
{
    ...
    {
        if (HASH_IS_REAL(hash_table->hashes[i]))
            retval = g_list_prepend(retval, g_hash_table_fetch_key_or_value(
                                                hash_table->values, i,
                                                hash_table->have_big_values));
    }
