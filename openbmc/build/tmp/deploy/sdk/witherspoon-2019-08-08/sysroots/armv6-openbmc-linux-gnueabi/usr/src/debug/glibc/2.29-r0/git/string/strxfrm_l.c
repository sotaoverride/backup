...
{
    int32_t tmp =
        findidx(l_data->table, l_data->indirect, l_data->extra, us, -1);
    *rule_idx = tmp >> 24;
    ...
    for (pass = 0; pass < l_data->nrules; ++pass)
    {
        ...
        if (position == 0)
        {
            while (*cur != L('\0'))
            {
                ...
                size_t len =
                    find_idx(&cur, &weight_idx, &rule_idx, l_data, pass);
        ...
        else
        {
                ...
                if ((rule & sort_forward) != 0)
                {
                    ...
                    if (backw_start != NULL)
                    {
                        for (size_t p = backw_len; p > 0; p--)
                        {
                            ...
                            len = find_idx(&backw_cur, &weight_idx, &rule_idx,
                                           l_data, pass);
            ...
            if (backw_start != NULL)
            {
                for (size_t p = backw_len; p > 0; p--)
                {
                    ...
                    /* To prevent a warning init the used vars.  */
                    len = find_idx(&backw_cur, &weight_idx, &rule_idx, l_data,
                                   pass);
...
static size_t do_xfrm_cached(STRING_TYPE* dest, size_t n,
                             const locale_data_t* l_data, size_t idxmax,
                             int32_t* idxarr, const unsigned char* rulearr)
{
...
size_t STRXFRM(STRING_TYPE* dest, const STRING_TYPE* src, size_t n, locale_t l)
{
    ...
    /* Get the locale data.  */
    l_data.rulesets =
        (unsigned char*)current->values[_NL_ITEM_INDEX(_NL_COLLATE_RULESETS)]
            .string;
    l_data.table =
        (int32_t*)current
            ->values[_NL_ITEM_INDEX(CONCAT(_NL_COLLATE_TABLE, SUFFIX))]
            .string;
    l_data.weights =
        (USTRING_TYPE*)current
            ->values[_NL_ITEM_INDEX(CONCAT(_NL_COLLATE_WEIGHT, SUFFIX))]
            .string;
    l_data.extra =
        (USTRING_TYPE*)current
            ->values[_NL_ITEM_INDEX(CONCAT(_NL_COLLATE_EXTRA, SUFFIX))]
            .string;
    l_data.indirect =
        (int32_t*)current
            ->values[_NL_ITEM_INDEX(CONCAT(_NL_COLLATE_INDIRECT, SUFFIX))]
            .string;
    ...
    {
        int32_t tmp =
            findidx(l_data.table, l_data.indirect, l_data.extra, &cur, -1);
