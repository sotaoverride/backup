 ...
 *
 * Returns: a pointer to a new struct libscols_column instance, NULL in case of
 * an ENOMEM error.
 */
...
int scols_column_set_flags(struct libscols_column* cl, int flags)
{
    ...
    DBG(COL,
        ul_debugobj(cl, "setting flags from 0%x to 0%x", cl->flags, flags));
    cl->flags = flags;
    return 0;
}
 ...
 *
 * Returns: JSON type used for formatting or a negative value in case of an
 * error.
 *
 * Since: 2.33
 */
...
char* scols_wrapnl_nextchunk(const struct libscols_column* cl
                             ...
                             char* data, void* userdata __attribute__((unused)))
{
...
{
    return (cl->flags & SCOLS_FL_WRAP) && cl->wrap_chunksize &&
                   cl->wrap_nextchunk
               ? 1
               : 0;
}
