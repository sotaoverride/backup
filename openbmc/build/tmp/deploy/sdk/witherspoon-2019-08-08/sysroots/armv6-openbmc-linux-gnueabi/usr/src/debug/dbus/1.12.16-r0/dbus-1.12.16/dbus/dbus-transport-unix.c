 ...
 * @returns a new transport, or #NULL on failure.
 */
static DBusTransport* _dbus_transport_new_for_exec(const char* path,
                                                   char* const argv[],
                                                   DBusError* error)
{
    DBusSocket fd = DBUS_SOCKET_INIT;
    ...
    if (argv)
    {
        for (i = 0; argv[i]; i++)
        {
            ...
            success =
                _dbus_string_append_printf(&address, ",argv%u=%s", i, escaped);
            dbus_free(escaped);
    ...
    DBusAddressEntry* entry, DBusTransport** transport_p, DBusError* error)
{
    ...
    if (strcmp(method, "unix") == 0)
    {
        ...
        if (tmpdir != NULL)
        {
            _dbus_set_bad_address(
                error, NULL, NULL,
                "cannot use the \"tmpdir\" option for an address to connect "
        ...
        if (path != NULL && abstract != NULL)
        {
            _dbus_set_bad_address(error, NULL, NULL,
                                  "can't specify both \"path\" and "
                                  "\"abstract\" options in an address");
            return DBUS_TRANSPORT_OPEN_BAD_ADDRESS;
        }

        if (path)
            *transport_p =
                _dbus_transport_new_for_domain_socket(path, FALSE, error);
        else
            *transport_p =
                _dbus_transport_new_for_domain_socket(abstract, TRUE, error);
        if (*transport_p == NULL)
        {
            _DBUS_ASSERT_ERROR_IS_SET(error);
    ...
    else if (strcmp(method, "unixexec") == 0)
    {
        ...
        for (i = 1;; i++)
        {
            char t[4 + 20 + 1]; /* "argv" plus space for a formatted base 10
                                   64bit integer, plus NUL */
    ...
    else if (strcmp(method, "launchd") == 0)
    {
        ...
        const char* launchd_env_var =
            dbus_address_entry_get_value(entry, "env");
        ...
        valid_socket =
            _dbus_lookup_launchd_socket(&socket_path, launchd_env_var, error);
        ...
        *transport_p =
            _dbus_transport_new_for_domain_socket(launchd_socket, FALSE, error);
