...
static void g_buffered_input_stream_set_property(GObject* object, guint prop_id,
                                                 const GValue* value,
                                                 GParamSpec* pspec);

static void g_buffered_input_stream_get_property(GObject* object, guint prop_id,
                                                 GValue* value,
                                                 GParamSpec* pspec);
static void g_buffered_input_stream_finalize(GObject* object);

static gssize g_buffered_input_stream_skip(GInputStream* stream, gsize count,
...
static gssize g_buffered_input_stream_skip_finish(GInputStream* stream,
                                                  GAsyncResult* result,
                                                  GError** error);
static gssize g_buffered_input_stream_read(GInputStream* stream, void* buffer,
                                           gsize count,
                                           GCancellable* cancellable,
                                           GError** error);
static gssize g_buffered_input_stream_real_fill(GBufferedInputStream* stream,
                                                ...
                                                GCancellable* cancellable,
                                                GError** error);
static void g_buffered_input_stream_real_fill_async(
    GBufferedInputStream* stream, gssize count, int io_priority,
    GCancellable* cancellable, GAsyncReadyCallback callback,
    gpointer user_data);
static gssize g_buffered_input_stream_real_fill_finish(
...
G_DEFINE_TYPE_WITH_CODE(
    GBufferedInputStream, g_buffered_input_stream, G_TYPE_FILTER_INPUT_STREAM,
...
static void g_buffered_input_stream_class_init(GBufferedInputStreamClass* klass)
{
    ...
    g_object_class_install_property(
        object_class, PROP_BUFSIZE,
        g_param_spec_uint("buffer-size", P_("Buffer Size"),
                          P_("The size of the backend buffer"), 1, G_MAXUINT,
                          DEFAULT_BUFFER_SIZE,
...
GInputStream* g_buffered_input_stream_new(GInputStream* base_stream)
{
    ...
    stream = g_object_new(G_TYPE_BUFFERED_INPUT_STREAM, "base-stream",
                          base_stream, NULL);
                                                ...
                                                gsize size)
{
    ...
    stream = g_object_new(G_TYPE_BUFFERED_INPUT_STREAM, "base-stream",
                          base_stream, "buffer-size", (guint)size, NULL);
 ...
 *     or -1 on error.
 */
gssize g_buffered_input_stream_fill(GBufferedInputStream* stream, gssize count,
                                        ...
                                        gpointer user_data)
{
    ...
    if (count < -1)
    {
        g_task_report_new_error(
            stream, callback, user_data, g_buffered_input_stream_fill_async,
    ...
    if (!g_input_stream_set_pending(G_INPUT_STREAM(stream), &error))
    {
        g_task_report_error(stream, callback, user_data,
                            g_buffered_input_stream_fill_async, error);
        return;
    }
 ...
 */
gsize g_buffered_input_stream_peek(GBufferedInputStream* stream, void* buffer,
                                                ...
                                                GError** error)
{
    ...
    nread = g_input_stream_read(base_stream, priv->buffer + priv->end, count,
                                cancellable, error);
                                           ...
                                           GError** error)
{
    ...
    if (count > priv->len)
    {
        ...
        nread = g_input_stream_read(base_stream, (char*)buffer + bytes_read,
...
static gboolean g_buffered_input_stream_can_seek(GSeekable* seekable)
{
    ...
    base_stream = G_FILTER_INPUT_STREAM(seekable)->base_stream;
    return G_IS_SEEKABLE(base_stream) &&
...
static void fill_async_callback(GObject* source_object, GAsyncResult* result,
                                gpointer user_data)
{
    GError* error;
    gssize res;
    ...
    error = NULL;
    res = g_input_stream_read_finish(G_INPUT_STREAM(source_object), result,
                                     &error);
    if (res == -1)
    ...
    GCancellable* cancellable, GAsyncReadyCallback callback, gpointer user_data)
{
    ...
    base_stream = G_FILTER_INPUT_STREAM(stream)->base_stream;
    g_input_stream_read_async(base_stream, priv->buffer + priv->end, count,
                              io_priority, cancellable, fill_async_callback,
                              task);
}
...
static void large_skip_callback(GObject* source_object, GAsyncResult* result,
                                gpointer user_data)
{
    ...
    error = NULL;
    nread = g_input_stream_skip_finish(G_INPUT_STREAM(source_object), result,
                                       &error);
...
static void skip_fill_buffer_callback(GObject* source_object,
                                               ...
                                               gpointer user_data)
{
    ...
    if (count > priv->len)
    {
        ...
        g_input_stream_skip_async(base_stream, count, io_priority, cancellable,
                                  large_skip_callback, task);
    }
    else
    {
        class = G_BUFFERED_INPUT_STREAM_GET_CLASS(stream);
