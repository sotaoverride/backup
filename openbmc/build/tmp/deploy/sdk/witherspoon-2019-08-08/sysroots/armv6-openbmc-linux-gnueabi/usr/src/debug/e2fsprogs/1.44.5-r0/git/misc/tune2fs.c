...
static const char* fsck_explain =
    N_("\nThis operation requires a freshly checked filesystem.\n");

static const char* please_fsck =
...
static __u32 ok_features[3] = {
    ...
    EXT2_FEATURE_RO_COMPAT_LARGE_FILE | EXT4_FEATURE_RO_COMPAT_HUGE_FILE |
        ...
        EXT4_FEATURE_RO_COMPAT_GDT_CSUM | EXT2_FEATURE_RO_COMPAT_SPARSE_SUPER |
...
static __u32 clear_ok_features[3] = {
    ...
    /* Incompat */
    EXT2_FEATURE_INCOMPAT_FILETYPE | EXT4_FEATURE_INCOMPAT_FLEX_BG |
        EXT4_FEATURE_INCOMPAT_MMP | EXT4_FEATURE_INCOMPAT_64BIT |
        EXT4_FEATURE_INCOMPAT_CSUM_SEED,
    /* R/O compat */
    EXT2_FEATURE_RO_COMPAT_LARGE_FILE | EXT4_FEATURE_RO_COMPAT_HUGE_FILE |
        EXT4_FEATURE_RO_COMPAT_DIR_NLINK | EXT4_FEATURE_RO_COMPAT_EXTRA_ISIZE |
        EXT4_FEATURE_RO_COMPAT_GDT_CSUM | EXT4_FEATURE_RO_COMPAT_QUOTA |
        EXT4_FEATURE_RO_COMPAT_PROJECT | EXT4_FEATURE_RO_COMPAT_METADATA_CSUM |
...
static int get_journal_sb(ext2_filsys jfs, char buf[SUPERBLOCK_SIZE])
{
    ...
    if ((retval = io_channel_read_blk64(jfs->io,
                                        ext2fs_journal_sb_start(jfs->blocksize),
                                        -SUPERBLOCK_SIZE, buf)))
    {
...
static int remove_journal_device(ext2_filsys fs)
{
    ...
    retval = ext2fs_open(journal_path, EXT2_FLAG_RW | EXT2_FLAG_JOURNAL_DEV_OK,
                         0, fs->blocksize, io_ptr, &jfs);
    if (retval)
    {
        com_err(program_name, retval, "%s",
    ...
    retval = io_channel_write_blk64(
        jfs->io, ext2fs_journal_sb_start(fs->blocksize), -SUPERBLOCK_SIZE, buf);
    if (retval)
    {
        com_err(program_name, retval, "while writing journal superblock.");
        goto no_valid_journal;
    }
...
static errcode_t remove_journal_inode(ext2_filsys fs)
{
    ...
    if (retval)
    {
        com_err(program_name, retval, "%s", _("while reading journal inode"));
        return retval;
    }
    if (ino == EXT2_JOURNAL_INO)
    {
        ...
        if (retval)
        {
            com_err(program_name, retval, "%s", _("while reading bitmaps"));
            return retval;
        }
        retval = ext2fs_block_iterate3(fs, ino, BLOCK_FLAG_READ_ONLY, NULL,
    ...
    if (retval)
    {
        com_err(program_name, retval, "%s", _("while writing journal inode"));
        return retval;
    }
...
static void convert_64bit(ext2_filsys fs, int direction)
{
     ...
     * user now.
     */
    if (!fsck_requested && ((fs->super->s_state & EXT2_ERROR_FS) ||
                            !(fs->super->s_state & EXT2_VALID_FS) ||
                            fs->super->s_lastcheck < fs->super->s_mtime))
                                 ...
                                 struct ext2_inode* inode)
{
    ...
    do
    {
        ...
        {
            errcode = ext2fs_extent_get(handle, EXT2_EXTENT_LAST_SIB, &extent);
            if (errcode)
                break;
        }
...
static int rewrite_dir_block(ext2_filsys fs, blk64_t* blocknr,
                             e2_blkcnt_t blockcnt EXT2FS_ATTR((unused)),
                             blk64_t ref_block EXT2FS_ATTR((unused)),
                             int ref_offset EXT2FS_ATTR((unused)),
                             void* priv_data)
{
    ...
    ctx->errcode = ext2fs_read_dir_block4(fs, *blocknr, ctx->buf, 0, ctx->dir);
    if (ctx->errcode)
        return BLOCK_ABORT;

    /* if htree node... */
    if (ctx->is_htree)
        ext2fs_get_dx_countlimit(fs, (struct ext2_dir_entry*)ctx->buf, &dcl,
                                 &dcl_offset);
    if (dcl)
    {
        if (!ext2fs_has_feature_metadata_csum(fs->super))
        {
            ...
            int max_entries =
                (fs->blocksize - dcl_offset) / sizeof(struct ext2_dx_entry);
        ...
        else
        {
             ...
             */
            int max_entries =
                (fs->blocksize - (dcl_offset + sizeof(struct ext2_dx_tail))) /
    ...
    else
    {
        ...
        if (!ext2fs_has_feature_metadata_csum(fs->super))
        {
             ...
             */
            ctx->errcode = ext2fs_get_rec_len(fs, penultimate_de, &rec_len);
            if (!rec_len)
                ctx->errcode = EXT2_ET_DIR_CORRUPTED;
            if (ctx->errcode)
                return BLOCK_ABORT;
            ext2fs_set_rec_len(fs, rec_len + sizeof(struct ext2_dir_entry_tail),
                               penultimate_de);
            changed = 1;
        }
        else
        {
             ...
             * the checksum.
             */
            if (!last_de->inode && rec_len == csum_size)
            {
...
out:
    ...
    ctx->errcode = ext2fs_write_dir_block4(fs, *blocknr, ctx->buf, 0, ctx->dir);
    if (ctx->errcode)
        return BLOCK_ABORT;
                                   ...
                                   struct ext2_inode* inode)
{
    ...
    ctx.dir = dir;
    ctx.errcode = 0;
    retval = ext2fs_block_iterate3(fs, dir,
                                 ...
                                 struct ext2_inode* inode)
{
    ...
    retval = ext2fs_file_read(file, ctx->ea_buf, inode->i_size, NULL);
    if (retval)
    ...
    hash = ext2fs_crc32c_le(ctx->fs->csum_seed, (unsigned char*)ctx->ea_buf,
                            inode->i_size);
                                     ...
                                     struct ext2_ext_attr_entry* end)
{
    ...
    while (entry < end && !EXT2_EXT_IS_LAST_ENTRY(entry))
    {
        ...
        {
            retval =
                ext2fs_ext_attr_hash_entry2(fs, entry, NULL, &entry->e_hash);
            if (retval)
                              ...
                              struct ext2_inode* inode)
{
    ...
    if (ctx->inode_size != EXT2_GOOD_OLD_INODE_SIZE)
        update_inline_xattr_hashes(ctx, (struct ext2_inode_large*)inode);
    ...
    retval = ext2fs_read_ext_attr3(ctx->fs, file_acl_block, ctx->ea_buf, ino);
    if (retval)
        fatal_err(retval, "while rewriting extended attribute");

    update_block_xattr_hashes(ctx, ctx->ea_buf);
    retval = ext2fs_write_ext_attr3(ctx->fs, file_acl_block, ctx->ea_buf, ino);
    if (retval)
        fatal_err(retval, "while rewriting extended attribute");
}
...
static void rewrite_inodes(ext2_filsys fs)
{
    ...
    for (; pass <= 2; pass++)
    {
        ...
        {
            retval =
                ext2fs_get_next_inode_full(scan, &ino, inode, ctx.inode_size);
            if (retval)
...
static errcode_t disable_uninit_bg(ext2_filsys fs, __u32 csum_feature_flag)
{
    ...
    if (retval)
    {
        com_err("disable_uninit_bg", retval, "while reading bitmaps");
        request_fsck_afterwards(fs);
        return retval;
    }
    ext2fs_mark_ib_dirty(fs);
    ...
    if (csum_feature_flag == EXT4_FEATURE_RO_COMPAT_GDT_CSUM)
    {
        ...
        if (retval)
        {
            com_err("disable_uninit_bg", retval, "while zeroing unused inodes");
            request_fsck_afterwards(fs);
            return retval;
        }
    }
...
static void try_confirm_csum_seed_support(void)
{
...
#define FEATURE_ON(type, mask)                                                 \
    (!(old_features[(type)] & (mask)) &&                                       \
     ((&sb->s_feature_compat)[(type)] & (mask)))
#define FEATURE_OFF(type, mask)                                                \
    ((old_features[(type)] & (mask)) &&                                        \
     !((&sb->s_feature_compat)[(type)] & (mask)))
#define FEATURE_CHANGED(type, mask)                                            \
    ((mask) & (old_features[(type)] ^ (&sb->s_feature_compat)[(type)]))
    ...
    if (e2p_edit_feature2(features, &sb->s_feature_compat, ok_features,
                          clear_ok_features, &type_err, &mask_err))
    {
        if (!mask_err)
            ...
            fprintf(
                ...
                _("Clearing filesystem feature '%s' "
                  "not supported.\n"),
                e2p_feature2string(type_err & E2P_FEATURE_TYPE_MASK, mask_err));
        else
            fprintf(stderr,
    ...
    if (FEATURE_OFF(E2P_FEATURE_COMPAT, EXT3_FEATURE_COMPAT_HAS_JOURNAL))
    {
        ...
        if (ext2fs_has_feature_journal_needs_recovery(sb) && f_flag < 2)
        {
    ...
    if (FEATURE_ON(E2P_FEATURE_INCOMPAT, EXT4_FEATURE_INCOMPAT_MMP))
    {
        ...
        if ((mount_flags & EXT2_MF_MOUNTED) || (mount_flags & EXT2_MF_READONLY))
        {
    ...
    if (FEATURE_OFF(E2P_FEATURE_RO_INCOMPAT, EXT4_FEATURE_RO_COMPAT_HUGE_FILE))
    {
        if ((mount_flags & EXT2_MF_MOUNTED) &&
            !(mount_flags & EXT2_MF_READONLY))
        {
    ...
    {
        check_fsck_needed(fs, _("Enabling checksums could take some time."));
        if (mount_flags & EXT2_MF_MOUNTED)
        {
                    ...
                    EXT4_FEATURE_RO_COMPAT_METADATA_CSUM))
    {
        ...
        check_fsck_needed(fs, _("Disabling checksums could take some time."));
        if (mount_flags & EXT2_MF_MOUNTED)
        {
        ...
        {
            err = disable_uninit_bg(fs, EXT4_FEATURE_RO_COMPAT_METADATA_CSUM);
            if (err)
                return 1;
        }
        else
    ...
    if (FEATURE_ON(E2P_FEATURE_RO_INCOMPAT, EXT4_FEATURE_RO_COMPAT_GDT_CSUM))
    {
     ...
     */
    if (FEATURE_ON(E2P_FEATURE_INCOMPAT, EXT4_FEATURE_INCOMPAT_64BIT))
    {
        if (mount_flags & EXT2_MF_MOUNTED)
        {
    ...
    if (FEATURE_OFF(E2P_FEATURE_INCOMPAT, EXT4_FEATURE_INCOMPAT_64BIT))
    {
        if (mount_flags & EXT2_MF_MOUNTED)
        {
    ...
    if (FEATURE_ON(E2P_FEATURE_RO_INCOMPAT, EXT4_FEATURE_RO_COMPAT_QUOTA))
    {
        /*
    ...
    if (FEATURE_ON(E2P_FEATURE_RO_INCOMPAT, EXT4_FEATURE_RO_COMPAT_PROJECT))
    {
    ...
    if (FEATURE_OFF(E2P_FEATURE_RO_INCOMPAT, EXT4_FEATURE_RO_COMPAT_QUOTA))
    {
        /*
         * Set the Q_flag here and handle the quota options in the code
         * below.
    ...
    if (FEATURE_ON(E2P_FEATURE_INCOMPAT, EXT4_FEATURE_INCOMPAT_CSUM_SEED))
    {
    ...
    if (FEATURE_OFF(E2P_FEATURE_INCOMPAT, EXT4_FEATURE_INCOMPAT_CSUM_SEED))
    {
        __le32 uuid_seed;
        ...
        if (fs->super->s_checksum_seed != uuid_seed)
        {
            if (mount_flags & (EXT2_MF_BUSY | EXT2_MF_MOUNTED))
            {
                fputs(_("UUID has changed since enabling "
                        ...
                        "\nto safely rewrite all metadata to match the new "
                        "UUID.\n"),
                      stderr);
                return 1;
            }
    ...
    if (FEATURE_CHANGED(E2P_FEATURE_RO_INCOMPAT,
        ...
        FEATURE_CHANGED(E2P_FEATURE_COMPAT, EXT2_FEATURE_COMPAT_RESIZE_INODE) ||
        FEATURE_OFF(E2P_FEATURE_RO_INCOMPAT, EXT2_FEATURE_RO_COMPAT_LARGE_FILE))
        request_fsck_afterwards(fs);
...
static int add_journal(ext2_filsys fs)
{
    ...
    if (journal_device)
    {
        ...
        printf(_("Creating journal on device %s: "), journal_device);
        fflush(stdout);
...
static void handle_quota_options(ext2_filsys fs)
{
    ...
    for (qtype = 0; qtype < MAXQUOTAS; qtype++)
    {
            ...
            *quota_sb_inump(fs->super, qtype) == 0)
        {
            ...
            {
                retval = quota_update_limits(qctx, qf_ino, qtype);
                if (retval)
                {
                    com_err(program_name, retval,
                            _("while updating quota limits (%d)"), qtype);
                    exit(1);
                }
            }
            ...
            {
                com_err(program_name, retval,
                        _("while writing quota file (%d)"), qtype);
                exit(1);
            }
            ...
            if (qtype == PRJQUOTA && !ext2fs_has_feature_project(fs->super))
            {
        ...
        else if (quota_enable[qtype] == QOPT_DISABLE)
        {
            ...
            {
                com_err(program_name, retval,
                        _("while removing quota file (%d)"), qtype);
                exit(1);
            }
            if (qtype == PRJQUOTA)
            {
...
static void parse_e2label_options(int argc, char** argv)
{
    ...
    if (!device_name)
    {
        com_err("e2label", 0, _("Unable to resolve '%s'"), argv[1]);
        exit(1);
    }
...
#else
    sscanf(str, "%4d%2d%2d%2d%2d%2d", &ts.tm_year, &ts.tm_mon, &ts.tm_mday,
           &ts.tm_hour, &ts.tm_min, &ts.tm_sec);
    ts.tm_year -= 1900;
    ts.tm_mon -= 1;
    if (ts.tm_year < 0 || ts.tm_mon < 0 || ts.tm_mon > 11 || ts.tm_mday < 0 ||
        ts.tm_mday > 31 || ts.tm_hour > 23 || ts.tm_min > 59 || ts.tm_sec > 61)
        ts.tm_mday = 0;
#endif
    if (ts.tm_mday == 0)
    {
        com_err(program_name, 0, _("Couldn't parse date/time specifier: %s"),
                str);
        usage();
    }
    ts.tm_isdst = -1;
...
static void parse_tune2fs_options(int argc, char** argv)
{
    ...
    while ((c = getopt(argc, argv, optstring)) != EOF)
        switch (c)
        {
            case 'c':
                ...
                {
                    com_err(program_name, 0, _("bad mounts count - %s"),
                            optarg);
                    usage();
                }
            ...
            case 'g':
                ...
                if (*tmp)
                {
                    com_err(program_name, 0, _("bad gid/group name - %s"),
                            optarg);
                    usage();
                }
                g_flag = 1;
            ...
            case 'i':
                ...
                if (*tmp)
                {
                    com_err(program_name, 0, _("bad interval - %s"), optarg);
                    usage();
                }
                i_flag = 1;
                open_flag = EXT2_FLAG_RW;
            ...
            case 'm':
                ...
                {
                    com_err(program_name, 0, _("bad reserved block ratio - %s"),
                            optarg);
                    usage();
                }
                m_flag = 1;
            ...
            case 'r':
                ...
                if (*tmp)
                {
                    com_err(program_name, 0,
                            _("bad reserved blocks count - %s"), optarg);
                    usage();
                }
                r_flag = 1;
                open_flag = EXT2_FLAG_RW;
            ...
            case 'u':
                ...
                if (*tmp)
                {
                    com_err(program_name, 0, _("bad uid/user name - %s"),
                            optarg);
                    usage();
                }
                u_flag = 1;
            ...
            case 'I':
                ...
                if (*tmp)
                {
                    com_err(program_name, 0, _("bad inode size - %s"), optarg);
                    usage();
                }
    ...
    if (!device_name)
    {
        com_err(program_name, 0, _("Unable to resolve '%s'"), argv[optind]);
        exit(1);
    }
}
...
void do_findfs(int argc, char** argv)
{
    ...
    if (!dev)
    {
        com_err("findfs", 0, _("Unable to resolve '%s'"), argv[1]);
        exit(1);
    }
    puts(dev);
    exit(0);
...
static int parse_extended_opts(ext2_filsys fs, const char* opts)
{
    ...
    for (token = buf; token && *token; token = next)
    {
        ...
        if (strcmp(token, "clear-mmp") == 0 || strcmp(token, "clear_mmp") == 0)
        {
            clear_mmp = 1;
        }
        else if (strcmp(token, "mmp_update_interval") == 0)
        {
            ...
            {
                fprintf(stderr, _("mmp_update_interval too big: %lu\n"), intv);
                r_usage++;
                continue;
            }
        ...
        else if (strcmp(token, "stride") == 0)
        {
            ...
            if (*p)
            {
                fprintf(stderr, _("Invalid RAID stride: %s\n"), arg);
                r_usage++;
                continue;
            }
            stride_set = 1;
                 ...
                 strcmp(token, "stripe_width") == 0)
        {
            ...
            if (*p)
            {
                fprintf(stderr, _("Invalid RAID stripe-width: %s\n"), arg);
                r_usage++;
                continue;
            }
            stripe_width_set = 1;
        ...
        else if (!strcmp(token, "mount_opts"))
        {
            ...
            {
                fprintf(stderr, "Extended mount options too long\n");
                continue;
            }
                            ...
                            ext2fs_block_bitmap bmap)
{
    ...
    for (i = 0; i < fs->group_desc_count; i++)
    {
        start_blk = ext2fs_inode_table_loc(fs, i) + fs->inode_blocks_per_group;

        end_blk = ext2fs_inode_table_loc(fs, i) + new_ino_blks_per_grp;
...
static int inode_scan_and_fix(ext2_filsys fs, ext2fs_block_bitmap bmap)
{
    ...
    while (1)
    {
        ...
        if (ext2fs_file_acl_block(fs, &inode) &&
            ext2fs_test_block_bitmap2(bmap, ext2fs_file_acl_block(fs, &inode)))
        {
            blk = translate_block(ext2fs_file_acl_block(fs, &inode));
            if (!blk)
                continue;
        ...
        retval =
            ext2fs_block_iterate3(fs, ino, 0, block_buf, process_block, bmap);
        if (retval)
            goto err_out;
    }
...
static int expand_inode_table(ext2_filsys fs, unsigned long new_ino_size)
{
    ...
    new_ino_blks_per_grp = ext2fs_div_ceil(
        EXT2_INODES_PER_GROUP(fs->super) * new_ino_size, fs->blocksize);
    ...
    for (i = 0; i < fs->group_desc_count; i++)
    {
        ...
        retval = io_channel_read_blk64(fs->io, blk, fs->inode_blocks_per_group,
                                       old_itable);
        if (retval)
            goto err_out;
        ...
        for (j = 0; j < EXT2_INODES_PER_GROUP(fs->super); j++)
        {
            ...
            memset(new_itable + old_ino_size, 0, new_ino_size - old_ino_size);
        ...
        retval = io_channel_write_blk64(fs->io, blk, new_ino_blks_per_grp,
                                        new_itable);
        if (retval)
            goto err_out;
    }
...
static int resize_inode(ext2_filsys fs, unsigned long new_size)
{
    ...
    new_ino_blks_per_grp = ext2fs_div_ceil(
        EXT2_INODES_PER_GROUP(fs->super) * new_size, fs->blocksize);
    ...
    retval = ext2fs_allocate_block_bitmap(fs, _("blocks to be moved"), &bmap);
    if (retval)
    {
...
static int fs_update_journal_user(struct ext2_super_block* sb,
                                  __u8 old_uuid[UUID_SIZE])
{
    ...
    retval = ext2fs_open2(journal_path, io_options,
                          EXT2_FLAG_JOURNAL_DEV_OK | EXT2_FLAG_RW, 0, 0,
                          unix_io_manager, &jfs);
    if (retval)
    {
...
retry_open:
    ...
    retval =
        ext2fs_open2(device_name, io_options, open_flag, 0, 0, io_ptr, &fs);
    if (retval)
    {
        com_err(program_name, retval, _("while trying to open %s"),
                device_name);
        if (retval == EXT2_ET_MMP_FSCK_ON || retval == EXT2_ET_MMP_UNKNOWN_SEQ)
    ...
    if (I_flag)
    {
        ...
        check_fsck_needed(fs, _("Resizing inodes could take some time."));
        /*
    ...
    {
        fprintf(stderr,
                _("Warning: The journal is dirty. You may wish to replay the "
                  "journal like:\n\n"
                  "\te2fsck -E journal_only %s\n\n"
                  "then rerun this command.  Otherwise, any changes made may "
                  "be overwritten\n"
                  "by journal recovery.\n"),
                device_name);
    }
    ...
    if (i_flag)
    {
        ...
        {
            com_err(program_name, 0,
                    _("interval between checks is too big (%lu)"), interval);
            rc = 1;
            goto closefs;
        }
    ...
    if (m_flag)
    {
        ext2fs_r_blocks_count_set(sb, reserved_ratio * ext2fs_blocks_count(sb) /
                                          100.0);
        ext2fs_mark_super_dirty(fs);
    ...
    if (s_flag == 1)
    {
        ...
        else
        {
            ...
            ext2fs_mark_super_dirty(fs);
            printf(_("\nSparse superblock flag set.  %s"), _(please_fsck));
        }
    }
    if (s_flag == 0)
    {
    ...
    if (L_flag)
    {
        ...
        strncpy(sb->s_volume_name, new_label, sizeof(sb->s_volume_name));
        ext2fs_mark_super_dirty(fs);
    }
    if (M_flag)
    {
    ...
    if (U_flag)
    {
        ...
        {
            check_fsck_needed(fs, _("Setting the UUID on this "
        ...
        if (!(rc = get_journal_sb(fs, buf)))
        {
            ...
            if ((rc = io_channel_write_blk64(
                     fs->io, ext2fs_journal_sb_start(fs->blocksize),
                     -SUPERBLOCK_SIZE, buf)))
                goto closefs;
        }
