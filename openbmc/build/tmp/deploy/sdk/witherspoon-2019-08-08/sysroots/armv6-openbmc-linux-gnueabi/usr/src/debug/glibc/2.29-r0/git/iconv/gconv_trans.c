                          ...
                          unsigned char** outbufstart, size_t* irreversible)
{
    ...
    if (winbuf + 1 > winbufend)
        return (winbuf == winbufend ? __GCONV_EMPTY_INPUT
       ...
       position in the input buffer.  */
        ...
        if (cnt > 0 && from_tbl[idx + cnt] == L'\0')
        {
            ...
            do
            {
                ...
                outptr = *outbufstart;
                res =
                    DL_CALL_FCT(fct, (step, step_data, &toinptr,
...
no_rules:
    ...
    {
        const uint32_t* default_missing = (const uint32_t*)_NL_CURRENT(
            LC_CTYPE, _NL_CTYPE_TRANSLIT_DEFAULT_MISSING);
        const unsigned char* toinptr = (const unsigned char*)default_missing;
        uint32_t len =
            _NL_CURRENT_WORD(LC_CTYPE, _NL_CTYPE_TRANSLIT_DEFAULT_MISSING_LEN);
        unsigned char* outptr;
        int res;
        ...
        outptr = *outbufstart;
        res = DL_CALL_FCT(fct, (step, step_data, &toinptr,
