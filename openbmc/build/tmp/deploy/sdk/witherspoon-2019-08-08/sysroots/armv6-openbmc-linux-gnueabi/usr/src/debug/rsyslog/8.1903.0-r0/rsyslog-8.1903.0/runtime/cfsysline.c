 ...
 * like \t etc.
 */
static rsRetVal doGetChar(uchar** pp, rsRetVal (*pSetHdlr)(void*, uid_t),
                          void* pVal)
{
    DEFiRet;
    ...
    /* if we are not at a '\0', we have our new char - no validity checks
     * here... */
    if (**pp == '\0')
    {
 ...
 */
static rsRetVal doCustomHdlr(uchar** pp, rsRetVal (*pSetHdlr)(uchar**, void*),
                             void* pVal)
{
    DEFiRet;
 ...
 */
static rsRetVal doGetSize(uchar** pp, rsRetVal (*pSetHdlr)(void*, int64),
                          void* pVal)
{
    DEFiRet;
    int64 i;
 ...
 * rgerhards, 2007-07-31
 */
static rsRetVal doGetInt(uchar** pp, rsRetVal (*pSetHdlr)(void*, uid_t),
                         void* pVal)
{
    uchar* p;
    DEFiRet;
 ...
 */
static rsRetVal doFileCreateMode(uchar** pp, rsRetVal (*pSetHdlr)(void*, uid_t),
                                 void* pVal)
{
    uchar* p;
    DEFiRet;
...
static int doParseOnOffOption(uchar** pp)
{
    ...
    {
        LogError(0, NO_ERRCODE, "Option value must be on or off, but is '%s'",
                 (char*)pOptStart);
        return -1;
    }
}
 ...
 */
static rsRetVal doGetGID(uchar** pp, rsRetVal (*pSetHdlr)(void*, uid_t),
                         void* pVal)
{
    ...
    if (pgBuf == NULL)
    {
        if (err != 0)
        {
            LogError(err, RS_RET_NOT_FOUND,
                     "Query for group '%s' resulted in an error", szName);
        }
        else
        {
            LogError(0, RS_RET_NOT_FOUND,
                     "ID for group '%s' could not be found", szName);
        }
        iRet = RS_RET_NOT_FOUND;
    }
    else
 ...
 * rgerhards, 2007-07-17
 */
static rsRetVal doGetUID(uchar** pp, rsRetVal (*pSetHdlr)(void*, uid_t),
                         void* pVal)
{
    ...
    {
        LogError(0, RS_RET_NOT_FOUND,
                 "ID for user '%s' could not be found or error", (char*)szName);
        iRet = RS_RET_NOT_FOUND;
    }
    else
    {
 ...
 * rgerhards, 2007-07-15
 */
static rsRetVal doBinaryOptionLine(uchar** pp, rsRetVal (*pSetHdlr)(void*, int),
                                   void* pVal)
{
    int iOption;
    DEFiRet;
 ...
 */
static rsRetVal doGetWord(uchar** pp, rsRetVal (*pSetHdlr)(void*, uchar*),
                          void* pVal)
{
    DEFiRet;
    cstr_t* pStrB = NULL;
 ...
 */
static rsRetVal doSyslogName(uchar** pp, rsRetVal (*pSetHdlr)(void*, int),
 ...
 * rgerhards, 2008-02-14
 */
static rsRetVal doFacility(uchar** pp, rsRetVal (*pSetHdlr)(void*, int),
                           void* pVal)
{
    DEFiRet;
 ...
 * rgerhards, 2008-02-14
 */
static rsRetVal doSeverity(uchar** pp, rsRetVal (*pSetHdlr)(void*, int),
                           void* pVal)
{
    DEFiRet;
 ...
 */
static rsRetVal cslchSetEntry(cslCmdHdlr_t* pThis, ecslCmdHdrlType eType,
                              rsRetVal (*pHdlr)(), void* pData, int* permitted)
{
    assert(pThis != NULL);
...
static rsRetVal cslchCallHdlr(cslCmdHdlr_t* pThis, uchar** ppConfLine)
{
    ...
    switch (pThis->eType)
    {
        ...
        default:
            dbgprintf("error: command handler type %d not implemented in "
                      "legacy system\n",
                      pThis->eType);
...
static rsRetVal cslcConstruct(cslCmd_t** ppThis, int bChainingPermitted)
{
    ...
    CHKiRet(llInit(&pThis->llCmdHdlrs, cslchDestruct, cslchKeyDestruct,
                   cslchKeyCompare));
 ...
 */
static rsRetVal cslcAddHdlr(cslCmd_t* pThis, ecslCmdHdrlType eType,
                            rsRetVal (*pHdlr)(), void* pData,
 ...
 */
rsRetVal regCfSysLineHdlr2(const uchar* pCmdName, int bChainingPermitted,
                           ...
                           void* pData, void* pOwnerCookie, int* permitted)
{
    ...
    if (iRet == RS_RET_NOT_FOUND)
    {
        ...
        CHKiRet_Hdlr(
            cslcAddHdlr(pThis, eType, pHdlr, pData, pOwnerCookie, permitted))
        {
            cslcDestruct(pThis);
            FINALIZE;
        }
...
rsRetVal regCfSysLineHdlr(const uchar* pCmdName, int bChainingPermitted,
                          ecslCmdHdrlType eType, rsRetVal (*pHdlr)(),
                          void* pData, void* pOwnerCookie)
{
    DEFiRet;
    iRet = regCfSysLineHdlr2(pCmdName, bChainingPermitted, eType, pHdlr, pData,
                             pOwnerCookie, NULL);
    RETiRet;
}
...
/* helper function for unregCfSysLineHdlrs4Owner(). This is used to see if there
 * is a handler of this owner inside the element and, if so, remove it. Please
 * note that it keeps track of a pointer to the last linked list entry, as this
 * is needed to remove an entry from the list. rgerhards, 2007-11-21
 */
 ...
 * most importantly used before unloading a loadable module providing some
 * handlers. The full list of handlers is searched. If the to-be removed handler
 * was the only handler for a directive name, the directive header, too, is
 * deleted. rgerhards, 2007-11-21
 */
...
rsRetVal processCfSysLineCommand(uchar* pCmdName, uchar** p)
{
    ...
    uchar* pHdlrP;  /* the handler's private p (else we could only call one
                       handler) */
    ...
    bWasOnceOK = 0;
    while ((iRetLL = llGetNextElt(&pCmd->llCmdHdlrs, &llCookieCmdHdlr,
                                  (void*)&pCmdHdlr)) == RS_RET_OK)
    {
        ...
        if (pCmdHdlr->permitted != NULL && !*(pCmdHdlr->permitted))
        {
            LogError(0, RS_RET_PARAM_NOT_PERMITTED,
                     ...
                     "permitted - did you already set it via a RainerScript "
                     "command (v6+ config)?",
                     pCmdName);
...
void dbgPrintCfSysLineHandlers(void)
{
    ...
    {
        llGetKey(llCookieCmd,
                 (void*)&pKey); /* TODO: using the cookie is NOT clean! */
        dbgprintf("\tCommand '%s':\n", pKey);
        llCookieCmdHdlr = NULL;
        while (llGetNextElt(&pCmd->llCmdHdlrs, &llCookieCmdHdlr,
                            (void*)&pCmdHdlr) == RS_RET_OK)
        {
            dbgprintf("\t\ttype : %d\n", pCmdHdlr->eType);
