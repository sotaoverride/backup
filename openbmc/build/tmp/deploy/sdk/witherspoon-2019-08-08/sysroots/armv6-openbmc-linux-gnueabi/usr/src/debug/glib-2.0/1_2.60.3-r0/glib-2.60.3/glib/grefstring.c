 ...
 *   if (!parse_person_details (data, &full_name, &address, &city, &state,
 * &zip_code)) return NULL;
 *
 ...
 * into it.
 *
 * Returns: (transfer full) (not nullable): the newly created reference counted
 * string
 *
 * Since: 2.58
 */
 ...
 * contents
 */
static gboolean interned_str_equal(gconstpointer v1, gconstpointer v2)
{
    const char* str1 = v1;
    const char* str2 = v2;
...
static void remove_if_interned(gpointer data)
{
    char* str = data;
