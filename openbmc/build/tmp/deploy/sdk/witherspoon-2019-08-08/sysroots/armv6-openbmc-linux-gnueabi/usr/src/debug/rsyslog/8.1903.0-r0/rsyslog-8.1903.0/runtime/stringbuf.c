...
static rsRetVal rsCStrConstructFromszStrv(cstr_t** const ppThis,
 ...
 */
static rsRetVal rsCStrExtendBuf(cstr_t* const __restrict__ pThis,
                                const size_t iMinNeeded)
{
    uchar* pNewBuf;
    size_t iNewSize;
    DEFiRet;
    ...
    if (iMinNeeded > RS_STRINGBUF_ALLOC_INCREMENT)
    {
         ...
         */
        iNewSize = (iMinNeeded / RS_STRINGBUF_ALLOC_INCREMENT + 1) *
                   RS_STRINGBUF_ALLOC_INCREMENT;
    }
    else
    {
    ...
    /* DEV debugging only: dbgprintf("extending string buffer, old %d, new
 ...
 */
rsRetVal rsCStrAppendStrWithLen(cstr_t* const pThis, const uchar* const psz,
                                const size_t iStrLen)
{
    DEFiRet;
...
void cstrTrimTrailingWhiteSpace(cstr_t* const __restrict__ pThis)
{
    ...
    {
        pThis->iStrLen = i;
        pThis->pBuf[pThis->iStrLen] = '\0';
            /* we always have this space */ // TODO: can we remove this?
    }
 ...
 */
int rsCStrCStrCmp(cstr_t* const __restrict__ pCS1,
                  cstr_t* const __restrict__ pCS2)
{
 ...
 * rgerhards: 2009-03-04: ERE support added, via parameter iType: 0 - BRE, 1 -
 * ERE Arnaud Cornet/rgerhards: 2009-04-02: performance improvement by caching
 * compiled regex If a caller does not need the cached version, it must still
 * provide memory for it and must call rsCStrRegexDestruct() afterwards.
 */
    ...
    if (objUse(regexp, LM_REGEXP_FILENAME) == RS_RET_OK)
    {
        if (*cache == NULL)
        {
            *cache = calloc(sizeof(regex_t), 1);
            int errcode;
            if ((errcode = regexp.regcomp(
                     *cache, (char*)rsCStrGetSzStrNoNULL(pCS1),
 ...
 * rgerhards 2005-09-26
 */
int rsCStrOffsetSzStrCmp(cstr_t* pCS1, size_t iOffset, uchar* psz,
                         size_t iLenSz)
{
    rsCHECKVALIDOBJECT(pCS1, OIDrsCStr);
    ...
    if ((pCS1->iStrLen - iOffset) == iLenSz)
    {
        ...
        { /* we now have two non-empty strings of equal
