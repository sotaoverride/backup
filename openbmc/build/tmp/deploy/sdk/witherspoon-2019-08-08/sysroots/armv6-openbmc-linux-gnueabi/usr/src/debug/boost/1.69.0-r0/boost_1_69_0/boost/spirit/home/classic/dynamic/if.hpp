          ...
          parser<if_else_parser<ParsableTrueT, ParsableFalseT, CondT>>>
{
    ...
    if_else_parser(ParsableTrueT const& p_true, ParsableFalseT const& p_false,
                   CondT const& cond_) :
        eval_t(cond_as_parser_t::convert(cond_)),
        base_t(as_parser_true_t::convert(p_true),
    ...
    {
        typedef
            typename parser_result<parser_true_t, ScannerT>::type then_result_t;
        typedef typename parser_result<parser_false_t, ScannerT>::type
            else_result_t;
        ...
        if (length >= 0)
        {
            ...
            if (then_result)
            {
                ...
                return scan.create_match(std::size_t(length), nil_t(), save,
                                         scan.first);
            }
        }
        else
...
struct if_else_parser_gen
{
    ...
    if_else_parser<ParsableTrueT, ParsableFalseT, CondT>
...
struct if_parser : public condition_evaluator<typename as_parser<CondT>::type>,
                   public unary<typename as_parser<ParsableT>::type,
                                parser<if_parser<ParsableT, CondT>>>
{
    ...
    if_parser(ParsableT const& p, CondT const& cond_) :
        eval_t(cond_as_parser_t::convert(cond_)),
        base_t(as_parser_t::convert(p)), else_p(p, cond_)
    {
    }
