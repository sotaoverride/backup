...
static int logmatch_update_filename(const char* pattern, char* currentFilename)
{
    time_t t;
    struct tm* tmp;
...
static void updateLogmatch(int iindex)
{
    ...
    if (logmatchTable[iindex].virgin)
    {
        ...
        if ((perfile = fopen(perfilename, "r")))
        {
            ...
            if (fscanf(perfile, "%lu %lu %lu %255s", &pos, &ccounter, &counter,
                       lastFilename))
            {
                ...
                if (logmatch_update_filename(
                        logmatchTable[iindex].filenamePattern, lastFilename) ==
                    0)
                {
                             ...
                             fopen(logmatchTable[iindex].filename, "r")))
                    {
                        ...
                        if (!fseek(logmatchTable[iindex].logfile, pos,
                                   SEEK_SET))
                        {
    ...
    if (stat(logmatchTable[iindex].filename, &sb) == 0)
    {
                 ...
                 fopen(logmatchTable[iindex].filename, "r")))
        {
            ...
            result = fseek(logmatchTable[iindex].logfile,
            ...
            while (fgets(inbuf, sizeof(inbuf), logmatchTable[iindex].logfile))
            {

                matchResultCode = regexec(&(logmatchTable[iindex].regexBuffer),
...
static void updateLogmatch_Scheduled(unsigned int registrationNumber,
...
static void logmatch_parse_config(const char* token, char* cptr)
{
    ...
    if (logmatchCount < MAXLOGMATCH)
    {
        ...
        sscanf(cptr, "%255s%c%255s%c %d %255c\n",
               logmatchTable[logmatchCount].name, &space_name,
               logmatchTable[logmatchCount].filenamePattern, &space_path,
        ...
        {
            snmp_log(LOG_ERR,
                     "logmatch_parse_config: the name scanned "
        ...
        else if (space_path != ' ')
        {
            snmp_log(
                LOG_ERR,
                "logmatch_parse_config: the file name "
        ...
        logmatchTable[logmatchCount].myRegexError = regcomp(
            &logmatchTable[logmatchCount].regexBuffer,
            logmatchTable[logmatchCount].regEx, REG_EXTENDED | REG_NOSUB);
        ...
        if (logmatchTable[logmatchCount].myRegexError)
        {
            ...
            snmp_log(LOG_ERR,
                     "Could not process the logmatch regex - %s,"
...
static void logmatch_free_config(void)
{
    int i;
...
static u_char* var_logmatch_table(struct variable* vp, oid* name,
...
{
    struct variable2 logmatch_info[] = {{LOGMATCH_INFO, ASN_INTEGER,
                                         NETSNMP_OLDAPI_RONLY,
                                         var_logmatch_table, 0}};

    struct variable2 logmatch_table[] = {{LOGMATCH_INDEX,
                                          ASN_INTEGER,
                                          NETSNMP_OLDAPI_RONLY,
                                          var_logmatch_table,
                                          1,
