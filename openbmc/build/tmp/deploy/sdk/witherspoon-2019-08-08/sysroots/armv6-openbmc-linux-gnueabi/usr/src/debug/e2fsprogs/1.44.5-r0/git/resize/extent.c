...
errcode_t ext2fs_create_extent_table(ext2_extent* ret_extent, __u64 size)
{
    ...
    retval = ext2fs_get_array(sizeof(struct ext2_extent_entry), extent->size,
                              &extent->list);
    if (retval)
    {
    ...
    memset(extent->list, 0, sizeof(struct ext2_extent_entry) * extent->size);
    *ret_extent = extent;
    return 0;
}
 ...
 */
errcode_t ext2fs_add_extent_entry(ext2_extent extent, __u64 old_loc,
                                  __u64 new_loc)
{
    ...
    if (extent->num >= extent->size)
    {
        ...
        retval = ext2fs_resize_mem(
            ...
            sizeof(struct ext2_extent_entry) * newsize, &extent->list);
        if (retval)
            return retval;
        extent->size = newsize;
    }
...
__u64 ext2fs_extent_translate(ext2_extent extent, __u64 old_loc)
{
    ...
    if (!extent->sorted)
    {
        qsort(extent->list, extent->num, sizeof(struct ext2_extent_entry),
              extent_cmp);
        extent->sorted = 1;
    }
    low = 0;
    ...
    while (low <= high)
    {
        ...
        else
        {
            ...
            {
                range = ((float)(old_loc - lowval)) / (highval - lowval);
                if (range > 0.9)
                    range = 0.9;
                if (range < 0.1)
                    range = 0.1;
