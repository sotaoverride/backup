...
struct queuecnf_s
{
    ...
    int iMainMsgQDiscardSeverity; /* by default, discard nothing to prevent
                                     unintentional loss */
    int iMainMsgQueueNumWorkers;  /* number of worker threads for the mm queue
                                     above */
    ...
    int iMainMsgQWrkMinMsgs;    /* minimum messages per worker needed to start a
                                   new one */
    int iMainMsgQDeqSlowdown;   /* dequeue slowdown (simple rate limiting) */
    int64
        iMainMsgQueMaxDiskSpace; /* max disk space allocated 0 ==> unlimited */
    int64 iMainMsgQueDeqBatchSize; /* dequeue batch size */
    int bMainMsgQSaveOnShutdown; /* save queue on shutdown (when DA enabled)? */
    int iMainMsgQueueDeqtWinFromHr; /* hour begin of time frame when queue is to
                                       be dequeued */
    int iMainMsgQueueDeqtWinToHr;   /* hour begin of time frame when queue is to
                                       be dequeued */
};
...
struct globals_s
{
    ...
    uchar* pszConfDAGFile; /* name of config DAG file, non-NULL means generate
                              one */
...
struct outchannels_s
{
    ...
    struct outchannel*
        ochLast; /* points to the last element of the outchannel list */
};
...
struct rulesets_s
{
    ...
    ruleset_t* pDflt; /* current default ruleset, e.g. for binding to actions
                         which have no other */
};
...
#define rsconfCURR_IF_VERSION                                                  \
    0 /* increment whenever you change the interface above! */
...
extern rsconf_t* loadConf; /* the config currently being loaded (no concurrent
