...
#define set_segment_flags(S, v)                                                \
    (((v) != IS_MMAPPED_BIT)                                                   \
         ? (ABORT, (v))                                                        \
         : (((S)->exec_offset = mmap_exec_offset((S)->base, (S)->size)),       \
            (mmap_exec_offset((S)->base + (S)->exec_offset, (S)->size) !=      \
             (S)->exec_offset)                                                 \
                ? (ABORT, (v))                                                 \
                : (mmap_exec_offset((S)->base, (S)->size) = 0),                \
            (v)))
...
static mchunkptr mmap_resize(mstate m, mchunkptr oldp, size_t nb)
{
    ...
    else
    {
        ...
        char* cp =
            (char*)CALL_MREMAP((char*)oldp - offset, oldmmsize, newmmsize, 1);
        if (cp != CMFAIL)
        {
            ...
            { /* misaligned */
                ...
                */
                char* br = (char*)mem2chunk((size_t)(
                    ((size_t)(mem + alignment - SIZE_T_ONE)) & -alignment));
