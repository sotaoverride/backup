 ...
 */
static const char* abbrevs[] = {N_("aextended attribute"),
...
static void print_time(FILE* f, time_t t)
{
    ...
    if (do_gmt == -1)
    {
        ...
        if (!time_str)
            time_str = "";
        do_gmt = !strcmp(time_str, "GMT") || !strcmp(time_str, "GMT0");
    }
#endif
    ...
    switch (ch)
    {
        ...
        case 't':
            if (LINUX_S_ISREG(inode->i_mode))
                ...
                fprintf(f, _("unknown file type with mode 0%o"), inode->i_mode);
            break;
        default:
        no_inode:
            fprintf(f, "%%I%c", ch);
...
static _INLINE_ void expand_percent_expression(FILE* f, ext2_filsys fs, char ch,
                                               int width, int* first,
                                               struct problem_context* ctx)
{
