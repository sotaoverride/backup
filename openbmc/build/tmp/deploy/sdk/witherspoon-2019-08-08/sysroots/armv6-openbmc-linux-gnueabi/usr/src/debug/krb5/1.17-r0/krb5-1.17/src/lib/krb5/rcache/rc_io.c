...
#define GETDIR                                                                 \
    (dir = getdir(), dirlen = strlen(dir) + sizeof(PATH_SEPARATOR) - 1)
 ...
 * incorrect modes.
 */
static krb5_error_code krb5_rc_io_mkstemp(krb5_context context,
                                          krb5_rc_iostuff* d, char* dir)
{
...
#endif
    if (asprintf(&d->fn, "%s%skrb5_RCXXXXXX", dir, PATH_SEPARATOR) < 0)
    {
        d->fn = NULL;
    ...
    if (retval)
    {
        k5_setmsg(context, retval, _("Cannot fstat replay cache file %s: %s"),
                  d->fn, strerror(errno));
    ...
    if (stbuf.st_mode & 077)
    {
        k5_setmsg(context, retval,
                  _("Insecure mkstemp() file mode for replay cache file %s; "
                    "try running this program with umask 077"),
                  d->fn);
        return KRB5_RC_IO_UNKNOWN;
    }
#endif
...
static krb5_error_code rc_map_errno(krb5_context context, int e, const char* fn,
    ...
    switch (e)
    {
        ...
        case EEXIST:
            k5_setmsg(context, KRB5_RC_IO_PERM,
                      _("Cannot %s replay cache file %s: %s"), operation, fn,
                      strerror(e));
            return KRB5_RC_IO_PERM;

        default:
            k5_setmsg(context, KRB5_RC_IO_UNKNOWN,
                      _("Cannot %s replay cache: %s"), operation, strerror(e));
            return KRB5_RC_IO_UNKNOWN;
    }
}

krb5_error_code krb5_rc_io_creat(krb5_context context, krb5_rc_iostuff* d,
                                 char** fn)
{
    ...
    set_cloexec_fd(d->fd);
    retval =
        krb5_rc_io_write(context, d, (krb5_pointer)&rc_vno, sizeof(rc_vno));
    if (retval)
        goto cleanup;
...
static krb5_error_code krb5_rc_io_open_internal(krb5_context context,
                                                ...
                                                char* full_pathname)
{
    ...
    {
        retval = KRB5_RC_IO_PERM;
        k5_setmsg(context, retval, _("rcache not owned by %d"), (int)geteuid());
        goto cleanup;
    }
#endif
    set_cloexec_fd(d->fd);
    ...
    do_not_unlink = 0;
    retval = krb5_rc_io_read(context, d, (krb5_pointer)&rc_vno, sizeof(rc_vno));
    if (retval)
        goto cleanup;
...
krb5_error_code krb5_rc_io_open(krb5_context context, krb5_rc_iostuff* d,
                                char* fn)
{
    return krb5_rc_io_open_internal(context, d, fn, NULL);
}

krb5_error_code krb5_rc_io_move(krb5_context context, krb5_rc_iostuff* new1,
                                krb5_rc_iostuff* old)
{
...
krb5_error_code krb5_rc_io_write(krb5_context context, krb5_rc_iostuff* d,
                                 krb5_pointer buf, unsigned int num)
{
...
krb5_error_code krb5_rc_io_sync(krb5_context context, krb5_rc_iostuff* d)
{
#if defined(_WIN32)
#ifndef fsync
...
krb5_error_code krb5_rc_io_read(krb5_context context, krb5_rc_iostuff* d,
                                krb5_pointer buf, unsigned int num)
{
    int count;
...
krb5_error_code krb5_rc_io_close(krb5_context context, krb5_rc_iostuff* d)
{
    if (d->fn != NULL)
    {
        free(d->fn);
...
krb5_error_code krb5_rc_io_destroy(krb5_context context, krb5_rc_iostuff* d)
{
    if (unlink(d->fn) == -1)
        switch (errno)
        {
...
krb5_error_code krb5_rc_io_mark(krb5_context context, krb5_rc_iostuff* d)
{
    d->mark = lseek(d->fd, (off_t)0, SEEK_CUR); /* can't fail */
    return 0;
}

krb5_error_code krb5_rc_io_unmark(krb5_context context, krb5_rc_iostuff* d)
{
    (void)lseek(d->fd, d->mark, SEEK_SET); /* if it fails, tough luck */
    return 0;
}

long krb5_rc_io_size(krb5_context context, krb5_rc_iostuff* d)
{
    struct stat statb;
