...
BOOST_INTRUSIVE_OPTION_TYPE(header_holder_type, HeaderHolder, HeaderHolder,
                            header_holder_type)
...
BOOST_INTRUSIVE_OPTION_TYPE(value_traits, ValueTraits, ValueTraits,
                            proto_value_traits)

//#define BOOST_INTRUSIVE_COMMA ,
//#define BOOST_INTRUSIVE_LESS <
//#define BOOST_INTRUSIVE_MORE >
// BOOST_INTRUSIVE_OPTION_TYPE (member_hook, Parent BOOST_INTRUSIVE_COMMA class
// MemberHook BOOST_INTRUSIVE_COMMA MemberHook Parent::* PtrToMember , mhtraits
// BOOST_INTRUSIVE_LESS Parent BOOST_INTRUSIVE_COMMA MemberHook
// BOOST_INTRUSIVE_COMMA PtrToMember BOOST_INTRUSIVE_MORE , proto_value_traits)
// template< class Parent , class MemberHook , MemberHook Parent::* PtrToMember>
// struct member_hook {
//   template<class Base> struct pack : Base {
//      typedef mhtraits < Parent , MemberHook , PtrToMember >
//      proto_value_traits;
//   };
//};
//
...
struct member_hook
{
    ...
    //      //Since we control the layout between the hook and the node, and
    //      there is
    //      //always single inheritance, the offset of the node is exactly the
    //      offset of
    //      //the hook. Since the node type is shared between all member hooks,
    //      this saves
...
BOOST_INTRUSIVE_OPTION_TYPE(function_hook, Functor, fhtraits<Functor>,
                            proto_value_traits)
...
//! default one
BOOST_INTRUSIVE_OPTION_TYPE(void_pointer, VoidPointer, VoidPointer,
                            void_pointer)
...
//! If true, slist also stores a pointer to the last element of the singly
//! linked list. This allows O(1) swap and splice_after(iterator, slist &) for
//! circular slists and makes possible new functions like push_back(reference)
//! and back().
...
//! instead of using the default bucket traits, a user defined holder will be
//! defined
BOOST_INTRUSIVE_OPTION_TYPE(bucket_traits, BucketTraits, BucketTraits,
                            bucket_traits)
...
//! with the same key.
BOOST_INTRUSIVE_OPTION_CONSTANT(optimize_multikey, bool, Enabled,
                                optimize_multikey)

//! This option setter specifies if the bucket array will be always power of
//! two. This allows using masks instead of the default modulo operation to
//! determine the bucket number from the hash value, leading to better
//! performance. In debug mode, if power of two buckets mode is activated, the
//! bucket length will be checked with assertions.
BOOST_INTRUSIVE_OPTION_CONSTANT(power_2_buckets, bool, Enabled, power_2_buckets)

//! This option setter specifies if the container will cache a pointer to the
//! first non-empty bucket so that begin() is always constant-time. This is
//! specially helpful when we can have containers with a few elements but with
//! big bucket arrays (that is, hashtables with low load factors).
...
//! out across each hash table insertion operation, as opposed to be incurred
//! all at once. Therefore linear hashing is well suited for interactive
//! applications or real-time appplications where the worst-case insertion time
//! of non-incremental hash containers (rehashing the whole bucket array) is not
//!admisible.
