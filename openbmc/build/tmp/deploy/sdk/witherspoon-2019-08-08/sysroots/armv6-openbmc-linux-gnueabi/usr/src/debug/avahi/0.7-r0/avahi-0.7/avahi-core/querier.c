...
static void querier_elapse_callback(AVAHI_GCC_UNUSED AvahiTimeEvent* e,
                                    void* userdata)
{
...
void avahi_querier_add(AvahiInterface* i, AvahiKey* key,
                       struct timeval* ret_ctime)
{
    ...
    q->time_event =
        avahi_time_event_new(i->monitor->server->time_event_queue,
                             avahi_elapse_time(&tv, q->sec_delay * 1000, 0),
                             querier_elapse_callback, q);
...
static void remove_querier_callback(AvahiInterfaceMonitor* m, AvahiInterface* i,
                                    void* userdata)
{
    assert(m);
    assert(i);
...
void avahi_querier_remove_for_all(AvahiServer* s, AvahiIfIndex idx,
                                  AvahiProtocol protocol, AvahiKey* key)
{
    assert(s);
    assert(key);

    avahi_interface_monitor_walk(s->monitor, idx, protocol,
                                 remove_querier_callback, key);
}
...
static void add_querier_callback(AvahiInterfaceMonitor* m, AvahiInterface* i,
                                 void* userdata)
{
    ...
    if (i->announcing)
    {
        ...
        if (cbdata->ret_ctime &&
            avahi_timeval_compare(&tv, cbdata->ret_ctime) > 0)
            *cbdata->ret_ctime = tv;
    }
}

void avahi_querier_add_for_all(AvahiServer* s, AvahiIfIndex idx,
                               AvahiProtocol protocol, AvahiKey* key,
                               struct timeval* ret_ctime)
{
