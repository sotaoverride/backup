...
#define OBJ_NUM_IDS                                                            \
    100 /* TODO change to a linked list?  info: 16 were currently in use       \
           2008-02-29 */
...
/* static data */
DEFobjCurrIf(
    obj) /* we define our own interface, as this is expected by some macros! */
    DEFobjCurrIf(var) DEFobjCurrIf(module)
        DEFobjCurrIf(strm) static objInfo_t* arrObjInfo
            [OBJ_NUM_IDS]; /* array with object information pointers */
pthread_mutex_t
    mutObjGlobalOp; /* mutex to guard global operations of the object system */
 ...
 * object shall receive a reference. A constructor and destructor MUST be
 * provided for all objects, thus they are in the parameter list. pszID is the
 * identifying object name and must point to constant pool memory. It is never
 * freed.
 */
static rsRetVal InfoConstruct(objInfo_t** ppThis, uchar* pszID, int iObjVers,
                              ...
                              modInfo_t* pModInfo)
{
    ...
    pThis->pszName = ustrdup(pszID); /* it's OK if we have NULL ptr, GetName()
                                        will deal with that! */
...
static rsRetVal InfoSetMethod(objInfo_t* pThis, objMethod_t objMethod,
                              rsRetVal (*pHandler)(void*))
{
                                   ...
                                   uchar* pszRecType)
{
    ...
    ISOBJ_assert(pObj);
    assert(!strcmp((char*)pszRecType, "Obj") ||
           !strcmp((char*)pszRecType, "OPB"));

    /* object cookie and serializer version (so far always 1) */
    CHKiRet(strm.WriteChar(pStrm, COOKIE_OBJLINE));
    CHKiRet(strm.Write(pStrm, (uchar*)pszRecType,
                       3)); /* record types are always 3 octets */
...
/* append a property
 */
static rsRetVal SerializeProp(strm_t* pStrm, uchar* pszPropName,
                              propType_t propType, void* pUsr)
{
    DEFiRet;
    uchar* pszBuf = NULL;
    size_t lenBuf = 0;
    ...
    /* TODO: use the stream functions for data conversion here - should be
     * quicker */
    ...
    switch (propType)
    {
        ...
        case PROPTYPE_SHORT:
            CHKiRet(
                srUtilItoA((char*)szBuf, sizeof(szBuf), (long)*((short*)pUsr)));
            pszBuf = szBuf;
        ...
        case PROPTYPE_INT:
            CHKiRet(
                srUtilItoA((char*)szBuf, sizeof(szBuf), (long)*((int*)pUsr)));
            pszBuf = szBuf;
            lenBuf = ustrlen(szBuf);
            vType = VARTYPE_NUMBER;
            break;
        ...
        case PROPTYPE_SYSLOGTIME:
            lenBuf = snprintf(
                (char*)szBuf, sizeof(szBuf),
                "%d:%d:%d:%d:%d:%d:%d:%d:%d:%c:%d:%d",
                ((syslogTime_t*)pUsr)->timeType, ((syslogTime_t*)pUsr)->year,
                ((syslogTime_t*)pUsr)->month, ((syslogTime_t*)pUsr)->day,
                ((syslogTime_t*)pUsr)->hour, ((syslogTime_t*)pUsr)->minute,
 ...
 */
static rsRetVal objDeserializeHeader(uchar* pszRecType, cstr_t** ppstrID,
                                     int* poVers, strm_t* pStrm)
{
    DEFiRet;
    number_t oVers;
    uchar c;
...
/* Deserialize a single property. Pointer must be positioned at begin of line.
 * Whole line up until the \n is read.
 */
rsRetVal objDeserializeProperty(var_t* pProp, strm_t* pStrm)
{
    ...
    {
        /* oops, we've read one char that does not belong to use - unget it
         * first */
...
finalize_it:
    ...
    if (Debug && iRet != RS_RET_OK && iRet != RS_RET_NO_PROPLINE)
    {
        ...
        dbgprintf(
            "error %d deserializing property name, offset %lld, step %d\n",
            iRet, offs, step);
...
finalize_it:
    dbgprintf("deserializer has possibly been able to re-sync and recover, "
              "state %d\n",
              iRet);
    RETiRet;
}
 ...
 * rgerhards, 2008-01-11
 */
static rsRetVal objDeserializeProperties(obj_t* pObj,
                                         rsRetVal (*objSetProperty)(),
                                         strm_t* pStrm)
{
    DEFiRet;
    var_t* pVar = NULL;
 ...
 * Params: Pointer to object Pointer (pObj) (like a obj_t**, but can not do that
 * due to compiler warning) expected object ID (to check against), a fixup
 * function that can modify the object before it is finalized and a user pointer
 * that is to be passed to that function in addition to the object. The fixup
 * function pointer may be NULL, in which case none is called. The caller must
 * destruct the created object. rgerhards, 2008-01-07
 */
static rsRetVal Deserialize(void* ppObj, uchar* pszTypeExpected, strm_t* pStrm,
                            rsRetVal (*fFixup)(obj_t*, void*), void* pUsr)
{
    DEFiRet;
    rsRetVal iRetLocal;
    obj_t* pObj = NULL;
    int oVers = 0; /* keep compiler happy, but it is totally useless but takes
                      up some execution time... */
    cstr_t* pstrID = NULL;
    objInfo_t* pObjInfo;
     ...
     * from objDeserializeTryRecover(), what will cause us to ultimately give
     * up. rgerhards, 2008-07-08
     */
    do
    {
    ...
    CHKiRet(objDeserializeProperties(
        pObj, pObjInfo->objMethods[objMethod_SETPROPERTY], pStrm));
...
/* De-Serialize an object, with known constructur and destructor. Params like
 * Deserialize(). Note: this is for the queue subsystem, and optimized for its
 * use. rgerhards, 2012-11-03
 */
rsRetVal objDeserializeWithMethods(void* ppObj, uchar* pszTypeExpected,
                                   int lenTypeExpected, strm_t* pStrm,
                                   rsRetVal (*fFixup)(obj_t*, void*),
                                   void* pUsr, rsRetVal (*objConstruct)(),
                                   rsRetVal (*objConstructFinalize)(),
...
/* De-Serialize an object property bag. As a property bag contains only partial
 * properties, it is not instanciable. Thus, the caller must provide a pointer
 * of an already-instanciated object of the correct type. Params: Pointer to
 * object (pObj) Pointer to be passed to the function The caller must destruct
 * the created object. rgerhards, 2008-01-07
 */
static rsRetVal DeserializePropBag(obj_t* pObj, strm_t* pStrm)
{
    ...
    do
    {
        ...
        if (iRetLocal != RS_RET_OK)
        {
            dbgprintf("objDeserializePropBag error %d during header - trying "
                      "to recover\n",
                      iRetLocal);
            CHKiRet(objDeserializeTryRecover(pStrm));
        }
 ...
 * fail, else we run into real big problems. So it must make sure that at least
 * someting is returned. rgerhards, 2008-01-30
 */
uchar* ATTR_NONNULL() objGetName(obj_t* const pThis)
{
    ...
    {
        snprintf((char*)szName, sizeof(szName), "%s %p", objGetClassName(pThis),
                 pThis);
 ...
 * rgerhards, 2008-02-29
 */
static rsRetVal FindObjInfo(const char* const __restrict__ strOID,
                            objInfo_t** ppInfo)
{
    DEFiRet;
    int bFound;
    ...
    while (!bFound && i < OBJ_NUM_IDS)
    {
        if (arrObjInfo[i] != NULL &&
            !strcmp(strOID, (const char*)arrObjInfo[i]->pszID))
        {
            bFound = 1;
            break;
        }
...
/* register a classes' info pointer, so that we can reference it later, if
 * needed to (e.g. for de-serialization support). rgerhards, 2008-01-07 In this
 * function, we look for a free space in the object table. While we do so, we
 * also detect if the same object has already been registered, which is not
 * valid. rgerhards, 2008-02-29
 */
...
finalize_it:
    if (iRet != RS_RET_OK)
    {
        LogError(0, NO_ERRCODE,
                 "registering object '%s' failed with error code %d",
                 pszObjName, iRet);
    }
...
/* deregister a classes' info pointer, usually called because the class is
 * unloaded. After deregistration, the class can no longer be accessed, except
 * if it is reloaded. rgerhards, 2008-03-10
 */
static rsRetVal UnregisterObj(uchar* pszObjName)
{
    ...
    /* DEV debug only: dbgprintf("object '%s' successfully unregistered with
     * index %d\n", pszObjName, i); */
...
/* This function shall be called by anyone who would like to use an object. It
 * will try to locate the object, load it into memory if not already present and
 * return a pointer to the objects interface. rgerhards, 2008-02-29
 */
static rsRetVal UseObj(const char* srcFile, uchar* pObjName, uchar* pObjFile,
                       interface_t* pIf)
{
    DEFiRet;
    objInfo_t* pObjInfo;
    ...
    {
        ABORT_FINALIZE(
            RS_RET_LOAD_ERROR); /* we had a load error and can not continue */
    }

    /* we must be careful that we do not enter in infinite loop if an error
     * occurs during loading a module. ModLoad emits an error message in such
     * cases and that potentially can trigger the same code here. So we
     * initially set the module state to "load error" and set it to "fully
     * initialized" when the load succeeded. It's a bit hackish, but looks like
     * a good solution. -- rgerhards, 2008-03-07
     */
    pIf->ifIsLoaded = 2;
...
/* This function shall be called when a caller is done with an object. Its
 * primary purpose is to keep the reference count correct, which is highly
 * important for modules residing in loadable modules. rgerhards, 2008-03-10
 */
static rsRetVal ReleaseObj(const char* srcFile, uchar* pObjName,
                           uchar* pObjFile, interface_t* pIf)
{
    DEFiRet;
    objInfo_t* pObjInfo;
    ...
    if (pObjFile == NULL)
        FINALIZE; /* if it is not a lodable module, we do not need to do
                     anything... */
...
finalize_it : ENDobjQueryInterface(obj)

              /* This function returns a pointer to our own interface. It is
               * used as the hook that every object (including dynamically
               * loaded ones) can use to obtain a pointer to our interface which
               * than can be used to obtain pointers to any other interface in
               * the system. This function must be externally visible because of
               * its special nature. rgerhards, 2008-02-29 [nice - will have
               * that date the next time in 4 years ;)]
               */
