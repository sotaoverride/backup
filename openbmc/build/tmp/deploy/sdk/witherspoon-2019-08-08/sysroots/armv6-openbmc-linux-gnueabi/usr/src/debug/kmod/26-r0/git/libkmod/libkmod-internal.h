...
static _always_inline_
    _printf_format_(2, 3) void kmod_log_null(struct kmod_ctx* ctx,
                                             const char* format, ...)
{
}
...
void kmod_log(const struct kmod_ctx* ctx, int priority, const char* file,
...
struct kmod_list* kmod_list_append(struct kmod_list* list,
                                   const void* data) _must_check_
    __attribute__((nonnull(2)));
struct kmod_list* kmod_list_prepend(struct kmod_list* list,
                                    const void* data) _must_check_
    __attribute__((nonnull(2)));
...
struct kmod_list* kmod_list_append_list(struct kmod_list* list1,
                                        struct kmod_list* list2) _must_check_;

#undef kmod_list_foreach
#define kmod_list_foreach(list_entry, first_entry)                             \
    for (list_entry = ((first_entry) == NULL) ? NULL : (first_entry);          \
         list_entry != NULL;                                                   \
         list_entry = (list_entry->node.next == &((first_entry)->node))        \
                          ? NULL                                               \
                          : container_of(list_entry->node.next,                \
                                         struct kmod_list, node))

#undef kmod_list_foreach_reverse
#define kmod_list_foreach_reverse(list_entry, first_entry)                     \
    for (list_entry =                                                          \
             (((first_entry) == NULL) ? NULL                                   \
                                      : container_of(first_entry->node.prev,   \
                                                     struct kmod_list, node)); \
         list_entry != NULL;                                                   \
         list_entry = ((list_entry == first_entry)                             \
                           ? NULL                                              \
                           : container_of(list_entry->node.prev,               \
                                          struct kmod_list, node)))
...
void kmod_pool_add_module(struct kmod_ctx* ctx, struct kmod_module* mod,
                          const char* key) __attribute__((nonnull(1, 2, 3)));
...
int kmod_config_new(struct kmod_ctx* ctx, struct kmod_config** config,
...
const char* kmod_softdep_get_name(const struct kmod_list* l)
    __attribute__((nonnull(1)));
const char* const* kmod_softdep_get_pre(const struct kmod_list* l,
                                        unsigned int* count)
    __attribute__((nonnull(1, 2)));
const char* const* kmod_softdep_get_post(const struct kmod_list* l,
                                         unsigned int* count);
...
struct kmod_file* kmod_file_open(const struct kmod_ctx* ctx,
                                 const char* filename) _must_check_
    __attribute__((nonnull(1, 2)));
...
off_t kmod_file_get_size(const struct kmod_file* file) _must_check_
    __attribute__((nonnull(1)));
bool kmod_file_get_direct(const struct kmod_file* file) _must_check_
    __attribute__((nonnull(1)));
...
const void* kmod_elf_get_memory(const struct kmod_elf* elf) _must_check_
    __attribute__((nonnull(1)));
int kmod_elf_get_strings(const struct kmod_elf* elf, const char* section,
                         char*** array) _must_check_
    __attribute__((nonnull(1, 2, 3)));
int kmod_elf_get_modversions(const struct kmod_elf* elf,
                             struct kmod_modversion** array) _must_check_
    __attribute__((nonnull(1, 2)));
int kmod_elf_get_symbols(const struct kmod_elf* elf,
                         struct kmod_modversion** array) _must_check_
    __attribute__((nonnull(1, 2)));
int kmod_elf_get_dependency_symbols(const struct kmod_elf* elf,
                                    struct kmod_modversion** array) _must_check_
    __attribute__((nonnull(1, 2)));
int kmod_elf_strip_section(struct kmod_elf* elf,
                           const char* section) _must_check_
    __attribute__((nonnull(1, 2)));
int kmod_elf_strip_vermagic(struct kmod_elf* elf) _must_check_
    __attribute__((nonnull(1)));
...
int kmod_elf_get_section(const struct kmod_elf* elf, const char* section,
                         const void** buf, uint64_t* buf_size) _must_check_
