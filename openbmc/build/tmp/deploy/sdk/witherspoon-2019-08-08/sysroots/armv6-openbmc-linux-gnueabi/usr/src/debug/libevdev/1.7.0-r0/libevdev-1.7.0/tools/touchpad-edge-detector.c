...
static int print_current_values(const struct dimensions* d)
{
    static int progress;
    char status = 0;
    ...
    printf("\rTouchpad sends:	x [%d..%d], y [%d..%d] %c", d->left, d->right,
           d->top, d->bottom, status);
    return 0;
}

static int handle_event(struct dimensions* d, const struct input_event* ev)
{
    if (ev->type == EV_SYN)
    {
...
static inline void pid_vid_matchstr(struct libevdev* dev, char* match,
                                    size_t sz)
{
    snprintf(match, sz, "input:b%04Xv%04Xp%04X", libevdev_get_id_bustype(dev),
             libevdev_get_id_vendor(dev), libevdev_get_id_product(dev));
}

static inline void dmi_matchstr(struct libevdev* dev, char* match, size_t sz)
{
...
static void print_udev_override_rule(struct libevdev* dev,
                                     ...
                                     const struct size* size)
{
    ...
    {
        int width = x->maximum - x->minimum, height = y->maximum - y->minimum;
    ...
    printf("# <Laptop model description goes here>\n"
           ...
           " EVDEV_ABS_00=%d:%d:%d\n"
           " EVDEV_ABS_01=%d:%d:%d\n",
           match, dim->left, dim->right, xres, dim->top, dim->bottom, yres);
    if (libevdev_has_event_code(dev, EV_ABS, ABS_MT_POSITION_X))
        printf(" EVDEV_ABS_35=%d:%d:%d\n"
               " EVDEV_ABS_36=%d:%d:%d\n",
               dim->left, dim->right, xres, dim->top, dim->bottom, yres);
}
...
int main(int argc, char** argv)
{
    ...
    if (sscanf(argv[1], "%dx%d", &size.w, &size.h) != 2 || size.w <= 0 ||
        size.h <= 0)
        return usage();
