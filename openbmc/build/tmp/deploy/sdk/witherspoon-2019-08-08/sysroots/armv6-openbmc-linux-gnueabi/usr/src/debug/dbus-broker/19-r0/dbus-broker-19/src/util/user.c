...
static int user_usage_new(UserUsage** usagep, User* user, uid_t uid)
{
    ...
    usage = calloc(1, sizeof(*usage) +
                          user->registry->n_slots * sizeof(*usage->slots));
    if (!usage)
...
static int user_charge_check(unsigned int remaining, unsigned int users,
                             unsigned int share, unsigned int charge)
{
    unsigned int limit;
    ...
    if (!__builtin_add_overflow(share, charge, &limit) &&
        ...
        !__builtin_sub_overflow(limit, share, &limit) && limit <= remaining)
        return 0;
...
{
    c_rbtree_add(&user->registry->user_tree, parent, slot,
                 &user->registry_node);
}
...
static int user_ref_usage(User* user, UserUsage** usagep, User* actor)
{
    ...
    slot = c_rbtree_find_slot(&user->usage_tree, user_usage_compare,
                              &actor->uid, &parent);
    if (slot)
    {
...
static int user_charge_commit_log(Log* log, User* user, UserCharge* charge,
                                  User* actor, size_t slot, unsigned int amount)
{
    int r;
    ...
    r = log_commitf(log, "UID %u exceeded its '%s' quota on UID %u.",
                    actor->uid, user_slot_to_string(slot), user->uid);
    if (r)
        return error_fold(r);
 ...
 */
int user_charge(User* user, UserCharge* charge, User* actor, size_t slot,
                unsigned int amount)
{
...
quota:
    ...
    r = user_charge_commit_log(user->registry->log, user, charge, actor, slot,
                               amount);
    if (r)
        return error_trace(r);
 ...
 */
int user_registry_init(UserRegistry* registry, Log* log, size_t n_slots,
...
int user_registry_ref_user(UserRegistry* registry, User** userp, uid_t uid)
{
    ...
    slot =
        c_rbtree_find_slot(&registry->user_tree, user_compare, &uid, &parent);
    if (slot)
    {
