...
netsnmp_feature_provide(row_merge)
     ...
     * row index that they refer to, and passes all requests for a given row to
     * the lower handlers. This is useful for handlers that don't want to
     * process multiple rows at the same time, but are happy to iterate through
     * the request list for a single row.
     *  @{
     */
    ...
    netsnmp_mib_handler* netsnmp_get_row_merge_handler(int prefix_len)
{
    ...
    ret = netsnmp_create_handler("row_merge", netsnmp_row_merge_helper_handler);
    if (ret)
    {
...
static void _rm_status_free(void* mem)
{
                                 ...
                                 int create_missing)
{
    ...
    rm_status =
        (netsnmp_row_merge_status*)netsnmp_agent_get_list_data(reqinfo, buf);
    if ((NULL == rm_status) && create_missing)
    {
        ...
        data_list = netsnmp_create_data_list(buf, rm_status, _rm_status_free);
        if (NULL == data_list)
        {
            free(rm_status);
            return NULL;
                                     ...
                                     netsnmp_request_info* requests)
{
    ...
    if (0 == rm_status->count)
    {
         ...
         */
        rm_status->saved_requests = (netsnmp_request_info**)calloc(
            count + 1, sizeof(netsnmp_request_info*));
    ...
    for (i = 0; i < count; i++)
    {
        ...
        if (0 == rm_status->count)
            rm_status->rows++;
        DEBUGMSGTL(
            ("helper:row_merge", " row %d oid[%d]: ", rm_status->rows, i));
        ...
        for (j = i + 1; j < count; j++)
        {
            ...
            DEBUGMSGOID(("helper:row_merge", saved_requests[j]->requestvb->name,
