...
static enum nss_status getgrent_next_nss(struct group* result, ent_t* ent,
                                         char* buffer, size_t buflen,
                                         int* errnop)
{
    if (!nss_getgrent_r)
...
static enum nss_status getgrnam_plusgroup(const char* name,
                                          struct group* result, ent_t* ent,
                                          char* buffer, size_t buflen,
                                          int* errnop)
{
    if (!nss_getgrnam_r)
    ...
    enum nss_status status =
        nss_getgrnam_r(name, result, buffer, buflen, errnop);
...
enum nss_status _nss_compat_getgrent_r(struct group* grp, char* buffer,
                                       size_t buflen, int* errnop)
{
...
static enum nss_status internal_getgrnam_r(const char* name,
                                           struct group* result, ent_t* ent,
                                           char* buffer, size_t buflen,
                                           int* errnop)
{
    ...
    while (1)
    {
        ...
        if (result->gr_name[0] == '+' && result->gr_name[1] != '\0')
        {
            if (strcmp(name, &result->gr_name[1]) == 0)
            {
                ...
                status = getgrnam_plusgroup(name, result, ent, buffer, buflen,
                                            errnop);
        ...
        if (result->gr_name[0] == '+' && result->gr_name[1] == '\0')
        {
            ...
            status =
                getgrnam_plusgroup(name, result, ent, buffer, buflen, errnop);
            if (status == NSS_STATUS_RETURN)
                /* We couldn't parse the entry */
                continue;
            else
...
static enum nss_status internal_getgrgid_r(gid_t gid, struct group* result,
                                           ...
                                           size_t buflen, int* errnop)
{
    ...
    while (1)
    {
        ...
        if (result->gr_name[0] == '+' && result->gr_name[1] == '\0')
        {
            ...
            enum nss_status status =
                nss_getgrgid_r(gid, result, buffer, buflen, errnop);
