...
void bus_slot_disconnect(sd_bus_slot* slot, bool unref)
{
    ...
    switch (slot->type)
    {
        ...
        case BUS_MATCH_CALLBACK:
            ...
            if (slot->match_added)
                (void)bus_remove_match_internal(
                    slot->bus, slot->match_callback.match_string);
            ...
            slot->match_callback.match_string =
                mfree(slot->match_callback.match_string);
        ...
        case BUS_NODE_CALLBACK:
            ...
            {
                LIST_REMOVE(callbacks, slot->node_callback.node->callbacks,
                            &slot->node_callback);
        ...
        case BUS_NODE_VTABLE:
                ...
                slot->node_vtable.vtable)
            {
                ...
                for (v = slot->node_vtable.vtable;
                     ...
                     v = bus_vtable_next(slot->node_vtable.vtable, v))
                {
                    ...
                    switch (v->type)
                    {
                        ...
                        case _SD_BUS_VTABLE_WRITABLE_PROPERTY:
                        {
                            ...
                            x = hashmap_remove(slot->bus->vtable_properties,
                                               &key);
                            break;
                        }
                    }
...
_public_ int sd_bus_slot_set_destroy_callback(sd_bus_slot* slot,
...
_public_ int sd_bus_slot_get_destroy_callback(sd_bus_slot* slot,
                                              sd_bus_destroy_t* callback)
{
    assert_return(slot, -EINVAL);
...
_public_ int sd_bus_slot_set_floating(sd_bus_slot* slot, int b)
{
    ...
    /* When a slot is "floating" then the bus references the slot. Otherwise the
     * slot references the bus. Hence,
     * when we move from one to the other, let's increase one reference and
     * decrease the other. */
...
_public_ int sd_bus_slot_set_description(sd_bus_slot* slot,
...
_public_ int sd_bus_slot_get_description(sd_bus_slot* slot,
                                         const char** description)
{
    assert_return(slot, -EINVAL);
