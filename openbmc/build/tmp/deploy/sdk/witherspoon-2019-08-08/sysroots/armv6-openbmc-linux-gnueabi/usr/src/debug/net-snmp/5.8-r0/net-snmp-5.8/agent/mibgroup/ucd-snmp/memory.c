...
void init_memory(void)
{
    ...
    netsnmp_register_scalar_group(
        netsnmp_create_handler_registration("memory", handle_memory, memory_oid,
                                            OID_LENGTH(memory_oid),
                                            HANDLER_CAN_RONLY),
        1, 26);
    netsnmp_register_scalar(netsnmp_create_handler_registration(
        "memSwapError", handle_memory, memSwapError_oid,
        OID_LENGTH(memSwapError_oid), HANDLER_CAN_RONLY));
    netsnmp_register_scalar(netsnmp_create_handler_registration(
        "memSwapErrMsg", handle_memory, memSwapErrMsg_oid,
        OID_LENGTH(memSwapErrMsg_oid), HANDLER_CAN_RONLY));
...
int handle_memory(netsnmp_mib_handler* handler,
        ...
        case MODE_GET:
            ...
            switch (requests->requestvb->name[reginfo->rootoid_len - 2])
            {
                ...
                case MEMORY_SWAP_TOTAL:
                    mem_info =
                        netsnmp_memory_get_byIdx(NETSNMP_MEM_TYPE_SWAP, 0);
                    if (!mem_info)
                        goto NOSUCH;
                ...
                case MEMORY_REAL_TOTAL:
                    mem_info =
                        netsnmp_memory_get_byIdx(NETSNMP_MEM_TYPE_PHYSMEM, 0);
                    if (!mem_info)
                        goto NOSUCH;
                ...
                case MEMORY_STXT_TOTAL:
                    mem_info =
                        netsnmp_memory_get_byIdx(NETSNMP_MEM_TYPE_STEXT, 0);
                    if (!mem_info)
                        goto NOSUCH;
                    val = mem_info->size;
                    val *= (mem_info->units / 1024);
                ...
                case MEMORY_RTXT_TOTAL:
                    mem_info =
                        netsnmp_memory_get_byIdx(NETSNMP_MEM_TYPE_RTEXT, 0);
                    if (!mem_info)
                        goto NOSUCH;
                    val = mem_info->size;
                    val *= (mem_info->units / 1024);
                ...
                case MEMORY_FREE:
                    mem_info =
                        netsnmp_memory_get_byIdx(NETSNMP_MEM_TYPE_VIRTMEM, 0);
                    if (!mem_info)
                        goto NOSUCH;
                ...
                case MEMORY_SHARED:
                    mem_info =
                        netsnmp_memory_get_byIdx(NETSNMP_MEM_TYPE_SHARED, 0);
                    if (!mem_info)
                        goto NOSUCH;
                ...
                case MEMORY_BUFFER:
                    mem_info =
                        netsnmp_memory_get_byIdx(NETSNMP_MEM_TYPE_MBUF, 0);
                ...
                case MEMORY_CACHED:
                    mem_info =
                        netsnmp_memory_get_byIdx(NETSNMP_MEM_TYPE_CACHED, 0);
                    if (!mem_info || mem_info->size == -1)
                        goto NOSUCH;
                ...
                case MEMORY_SWAP_ERROR:
                    ...
                    if (!mem_info)
                        goto NOSUCH;
                    val = ((mem_info->units / 1024) * mem_info->free >
                           minimum_swap)
                              ? 0
                              : 1;
                    break;
                case MEMORY_SWAP_ERRMSG:
                    ...
                    if (!mem_info)
                        goto NOSUCH;
                    if ((mem_info->units / 1024) * mem_info->free >
                        minimum_swap)
                        buf[0] = 0;
                    else
                        sprintf(buf, "Running out of swap space (%ld)",
                                mem_info->free);
                    snmp_set_var_typed_value(requests->requestvb, ASN_OCTET_STR,
                                             (u_char*)buf, strlen(buf));
                    return SNMP_ERR_NOERROR;
