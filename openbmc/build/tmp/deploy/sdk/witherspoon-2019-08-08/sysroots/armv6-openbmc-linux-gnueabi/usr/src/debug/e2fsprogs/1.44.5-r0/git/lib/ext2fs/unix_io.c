...
#define IS_ALIGNED(n, align) ((((uintptr_t)n) & ((uintptr_t)((align)-1))) == 0)
                              ...
                              unsigned long long block, int count, void* bufv)
{
    ...
    if ((channel->align == 0) ||
        (IS_ALIGNED(buf, channel->align) && IS_ALIGNED(size, channel->align)))
    {
...
error_out:
    if (channel->write_error)
        retval = (channel->write_error)(channel, block, count, buf, size,
                                        actual, retval);
    return retval;
}
...
static errcode_t alloc_cache(io_channel channel, struct unix_private_data* data)
{
    errcode_t retval;
                                            ...
                                            struct unix_cache** eldest)
{
    ...
    for (i = 0, cache = data->cache; i < CACHE_SIZE; i++, cache++)
    {
        ...
        if (!oldest_cache || (cache->access_time < oldest_cache->access_time))
            oldest_cache = cache;
    }
    if (eldest)
...
static errcode_t unix_open_channel(const char* name, int fd, int flags,
                                   io_channel* channel, io_manager io_mgr)
{
    io_channel io = NULL;
    ...
    {
        int zeroes = 0;
        if (ioctl(data->dev, BLKDISCARDZEROES, &zeroes) == 0 && zeroes)
            io->flags |= CHANNEL_FLAGS_DISCARD_ZEROES;
    }
#endif
...
#undef RLIM_INFINITY
     ...
     ((defined(__sparc__) || defined(__mips__)) && (SIZEOF_LONG == 4)))
...
#define RLIM_INFINITY (~0UL)
     ...
     *
     */
    if ((flags & IO_FLAG_RW) && (uname(&ut) == 0) &&
        ...
        (ext2fsP_is_disk_device(st.st_mode)))
    {
        ...
        if (((unsigned long)rlim.rlim_cur) < ((unsigned long)rlim.rlim_max))
        {
...
static errcode_t unixfd_open(const char* str_fd, int flags, io_channel* channel)
{
    int fd;
    int fd_flags;
                                 ...
                                 int count, void* buf)
{
    ...
    while (count > 0)
    {
        ...
        if (count == 1)
        {
            ...
            if ((retval = raw_read_blk(channel, data, block, 1, cache->buf)))
            {
                cache->in_use = 0;
                return retval;
            }
                                      ...
                                      unsigned long long count)
{
    ...
    return posix_fadvise(
        data->dev, (ext2_loff_t)block * channel->block_size + data->offset,
        (ext2_loff_t)count * channel->block_size, POSIX_FADV_WILLNEED);
#else
                              ...
                              unsigned long long count)
{
    ...
    else
    {
        ...
        {
            ret = ftruncate(data->dev, (block + count) * channel->block_size +
                                           data->offset);
            if (ret)
                goto err;
        }
    ...
    (defined(FALLOC_FL_ZERO_RANGE) ||                                          \
     (defined(FALLOC_FL_PUNCH_HOLE) && defined(FALLOC_FL_KEEP_SIZE)))
        ...
        ret = fallocate(data->dev, FALLOC_FL_ZERO_RANGE,
                        (off_t)(block)*channel->block_size + data->offset,
                        (off_t)(count)*channel->block_size);
#endif
#else
