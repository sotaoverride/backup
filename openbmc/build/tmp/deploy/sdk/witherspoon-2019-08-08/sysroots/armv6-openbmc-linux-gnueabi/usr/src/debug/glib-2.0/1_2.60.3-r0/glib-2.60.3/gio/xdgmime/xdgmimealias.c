...
static int alias_entry_cmp(const void* v1, const void* v2)
{
...
const char* _xdg_mime_alias_list_lookup(XdgAliasList* list, const char* alias)
{
    ...
    if (list->n_aliases > 0)
    {
        ...
        entry = bsearch(&key, list->aliases, list->n_aliases, sizeof(XdgAlias),
                        alias_entry_cmp);
        if (entry)
...
void _xdg_mime_alias_read_from_file(XdgAliasList* list, const char* file_name)
{
    ...
    if (list->n_aliases > 1)
        qsort(list->aliases, list->n_aliases, sizeof(XdgAlias),
              alias_entry_cmp);
}
