...
typedef void (*AvahiWatchCallback)(AvahiWatch* w, int fd, AvahiWatchEvent event,
                                   void* userdata);
...
struct AvahiPoll
{
    ...
    /** Update the events to wait for. It is safe to call this function from an
     * AvahiWatchCallback */
    void (*watch_update)(AvahiWatch* w, AvahiWatchEvent event);

    /** Return the events that happened. It is safe to call this function from
     * an AvahiWatchCallback  */
    AvahiWatchEvent (*watch_get_events)(AvahiWatch* w);

    /** Free a watch. It is safe to call this function from an
     * AvahiWatchCallback */
    ...
    AvahiTimeout* (*timeout_new)(const AvahiPoll* api, const struct timeval* tv,
                                 AvahiTimeoutCallback callback, void* userdata);

    /** Update the absolute expiration time for a timeout, If tv is
     * NULL, the timeout is disabled. It is safe to call this function from an
     * AvahiTimeoutCallback */
    void (*timeout_update)(AvahiTimeout*, const struct timeval* tv);

    /** Free a timeout. It is safe to call this function from an
     * AvahiTimeoutCallback */
