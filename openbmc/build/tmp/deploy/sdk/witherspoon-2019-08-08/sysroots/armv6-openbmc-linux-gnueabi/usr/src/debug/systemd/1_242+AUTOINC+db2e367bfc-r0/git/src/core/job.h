...
/* Be careful when changing the job types! Adjust job_merging_table[]
 * accordingly! */
enum JobType
{
    JOB_START, /* if a unit does not support being started, we'll just wait
...
enum JobMode
{
    ...
    JOB_IGNORE_DEPENDENCIES, /* Ignore both requirement and ordering
                                dependencies */
...
enum JobResult
{
    JOB_DONE,        /* Job completed successfully (or skipped due to a failed
                        ConditionXYZ=) */
    JOB_CANCELED,    /* Job canceled by a conflicting job installation or by
    ...
    JOB_UNSUPPORTED, /* Couldn't start a unit, because the unit type is not
