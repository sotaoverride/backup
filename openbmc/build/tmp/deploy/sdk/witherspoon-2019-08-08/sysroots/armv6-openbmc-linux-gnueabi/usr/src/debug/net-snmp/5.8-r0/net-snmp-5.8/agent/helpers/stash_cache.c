 ...
 */
netsnmp_mib_handler*
    netsnmp_get_timed_bare_stash_cache_handler(int timeout, oid* rootoid,
                                               size_t rootoid_len)
{
    ...
    cinfo =
        netsnmp_cache_create(timeout, _netsnmp_stash_cache_load,
    ...
    handler->next =
        netsnmp_create_handler("stash_cache", netsnmp_stash_cache_helper);
    if (!handler->next)
    {
...
netsnmp_mib_handler* netsnmp_get_timed_stash_cache_handler(int timeout,
                                                           ...
                                                           size_t rootoid_len)
{
    netsnmp_mib_handler* handler = netsnmp_get_timed_bare_stash_cache_handler(
        timeout, rootoid, rootoid_len);
    if (handler && handler->next)
    {
        handler->next->next = netsnmp_get_stash_to_next_handler();
    }
...
{
    return (netsnmp_oid_stash_node**)netsnmp_agent_get_list_data(
        reqinfo, STASH_CACHE_NAME);
}

/** @internal Implements the stash_cache handler */
int netsnmp_stash_cache_helper(netsnmp_mib_handler* handler,
                               ...
                               netsnmp_request_info* requests)
{
    ...
    switch (reqinfo->mode)
    {
        ...
        case MODE_GET:
            ...
            {
                cdata = (netsnmp_variable_list*)netsnmp_oid_stash_get_data(
                    cinfo->cache, requests->requestvb->name,
                    requests->requestvb->name_length);
                if (cdata && cdata->val.string && cdata->val_len)
                {
                    DEBUGMSGTL(
                        ("helper:stash_cache", "Found cached GET varbind\n"));
                    DEBUGMSGOID(("helper:stash_cache", cdata->name,
                                 cdata->name_length));
        ...
        case MODE_GETNEXT:
            ...
            for (request = requests; request; request = request->next)
            {
                cnode = netsnmp_oid_stash_getnext_node(
                    cinfo->cache, requests->requestvb->name,
                    requests->requestvb->name_length);
                if (cnode && cnode->thedata)
                {
                    ...
                    {
                        DEBUGMSGTL(("helper:stash_cache",
                                    "Found cached GETNEXT varbind\n"));
                        DEBUGMSGOID(("helper:stash_cache", cdata->name,
                                     cdata->name_length));
                        DEBUGMSG(("helper:stash_cache", "\n"));
                        snmp_set_var_typed_value(request->requestvb,
                                                 cdata->type, cdata->val.string,
                                                 cdata->val_len);
...
int _netsnmp_stash_cache_load(netsnmp_cache* cache, void* magic)
{
    ...
    netsnmp_agent_add_list_data(
        reqinfo,
        netsnmp_create_data_list(STASH_CACHE_NAME, &cinfo->cache, NULL));
...
{
    netsnmp_stash_cache_info* cinfo = (netsnmp_stash_cache_info*)magic;
    netsnmp_oid_stash_free(&cinfo->cache, (NetSNMPStashFreeNode*)snmp_free_var);
    return;
}
