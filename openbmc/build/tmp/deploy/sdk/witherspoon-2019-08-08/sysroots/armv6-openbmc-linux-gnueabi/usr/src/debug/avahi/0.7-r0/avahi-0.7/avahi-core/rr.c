...
AvahiRecord* avahi_record_new_full(const char* name, uint16_t class,
...
char* avahi_record_to_string(const AvahiRecord* r)
{
    ...
    {
        case AVAHI_DNS_TYPE_A:
            inet_ntop(AF_INET, &r->data.a.address.address, t = buf,
                      sizeof(buf));
            break;

        case AVAHI_DNS_TYPE_AAAA:
            inet_ntop(AF_INET6, &r->data.aaaa.address.address, t = buf,
                      sizeof(buf));
            break;
        ...
        case AVAHI_DNS_TYPE_HINFO:
            ...
            snprintf(t = buf, sizeof(buf), "\"%s\" \"%s\"", r->data.hinfo.cpu,
                     r->data.hinfo.os);
            break;

        case AVAHI_DNS_TYPE_SRV:

            snprintf(t = buf, sizeof(buf), "%u %u %u %s", r->data.srv.priority,
                     r->data.srv.weight, r->data.srv.port, r->data.srv.name);
...
int avahi_key_equal(const AvahiKey* a, const AvahiKey* b)
{
    ...
    return avahi_domain_equal(a->name, b->name) && a->type == b->type &&
           a->clazz == b->clazz;
}
...
int avahi_key_pattern_match(const AvahiKey* pattern, const AvahiKey* k)
{
    ...
    return avahi_domain_equal(pattern->name, k->name) &&
           ...
           (pattern->clazz == k->clazz ||
            pattern->clazz == AVAHI_DNS_CLASS_ANY);
}
...
static int rdata_equal(const AvahiRecord* a, const AvahiRecord* b)
{
    ...
    {
        case AVAHI_DNS_TYPE_SRV:
            return a->data.srv.priority == b->data.srv.priority &&
        ...
        case AVAHI_DNS_TYPE_HINFO:
            return !strcmp(a->data.hinfo.cpu, b->data.hinfo.cpu) &&
                   !strcmp(a->data.hinfo.os, b->data.hinfo.os);

        case AVAHI_DNS_TYPE_TXT:
            return avahi_string_list_equal(a->data.txt.string_list,
                                           b->data.txt.string_list);

        case AVAHI_DNS_TYPE_A:
            return memcmp(&a->data.a.address, &b->data.a.address,
                          sizeof(AvahiIPv4Address)) == 0;

        case AVAHI_DNS_TYPE_AAAA:
            return memcmp(&a->data.aaaa.address, &b->data.aaaa.address,
                          sizeof(AvahiIPv6Address)) == 0;

        default:
            return a->data.generic.size == b->data.generic.size &&
                   (a->data.generic.size == 0 ||
                    memcmp(a->data.generic.data, b->data.generic.data,
                           a->data.generic.size) == 0);
    }
}
...
AvahiRecord* avahi_record_copy(AvahiRecord* r)
{
    ...
    switch (r->key->type)
    {
        ...
        case AVAHI_DNS_TYPE_TXT:
            copy->data.txt.string_list =
                avahi_string_list_copy(r->data.txt.string_list);
            break;
        ...
        default:
            if (!(copy->data.generic.data =
                      avahi_memdup(r->data.generic.data, r->data.generic.size)))
                goto fail;
...
static int lexicographical_memcmp(const void* a, size_t al, const void* b,
                                  size_t bl)
{
    size_t c;
    int ret;
...
int avahi_record_lexicographical_compare(AvahiRecord* a, AvahiRecord* b)
{
    ...
    switch (a->key->type)
    {
        ...
        case AVAHI_DNS_TYPE_SRV:
        {
            if ((r = uint16_cmp(a->data.srv.priority, b->data.srv.priority)) ==
                    0 &&
        ...
        case AVAHI_DNS_TYPE_TXT:
        {
            ...
            asize =
                avahi_string_list_serialize(a->data.txt.string_list, NULL, 0);
            bsize =
                avahi_string_list_serialize(b->data.txt.string_list, NULL, 0);
        ...
        default:
            return lexicographical_memcmp(
                a->data.generic.data, a->data.generic.size,
...
int avahi_record_is_valid(AvahiRecord* r)
{
    ...
    switch (r->key->type)
    {
        ...
        case AVAHI_DNS_TYPE_HINFO:
            return strlen(r->data.hinfo.os) <= 255 &&
