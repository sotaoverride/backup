...
netsnmp_feature_child_of(tools_all, libnetsnmp)
    ...
    netsnmp_feature_child_of(memory_wrappers, tools_all)
        netsnmp_feature_child_of(valgrind, tools_all) netsnmp_feature_child_of(
            string_time_to_secs,
            tools_all) netsnmp_feature_child_of(netsnmp_check_definedness,
                                                valgrind)
                ...
                netsnmp_feature_child_of(timeval_tticks, netsnmp_unused)
                    ...
                    netsnmp_feature_child_of(memory_strdup, memory_wrappers)
                        netsnmp_feature_child_of(memory_calloc, memory_wrappers)
                            netsnmp_feature_child_of(memory_malloc,
                                                     memory_wrappers)
                                netsnmp_feature_child_of(memory_realloc,
                                                         memory_wrappers)
                                    netsnmp_feature_child_of(memory_free,
                                                             memory_wrappers)
 ...
 */
u_int netsnmp_binary_to_hex(u_char** dest, size_t* dest_len, int allow_realloc,
 ...
 *
 * @param buf     address of a pointer (pointer to pointer) for the output
 * buffer. If allow_realloc is set, the buffer may be grown via snmp_realloc to
 * accomodate the data.
 *
 ...
 * @retval 0  error
 */
int netsnmp_hex_to_binary(u_char** buf, size_t* buf_len, size_t* offset,
 ...
 *	 size
 */
void dump_chunk(const char* debugtoken, const char* title, const u_char* buf,
                int size)
{
...
char* dump_snmpEngineID(const u_char* estring, size_t* estring_len)
{
#define eb(b) (*(esp + b) & 0xff)
    ...
    s = buf;
    s += sprintf(s, "enterprise %d, ",
                 ((*(esp + 0) & 0x7f) << 24) | ((*(esp + 1) & 0xff) << 16) |
    ...
    switch ((int)*esp++)
    {
        ...
        case 2: /* IPv6 address. */
            ...
            s += sprintf(s,
                         ...
                         eb(8), eb(9), eb(10), eb(11), eb(12), eb(13), eb(14),
                         eb(15));
        ...
        case 3: /* MAC address. */
            ...
            s += sprintf(s, "%02X:%02X:%02X:%02X:%02X:%02X", eb(0), eb(1),
                         eb(2), eb(3), eb(4), eb(5));
        ...
        case 4: /* Text. */
            ...
            s += sprintf(s, "\"%.*s\"", (int)(sizeof(buf) - strlen(buf) - 3),
                         esp);
            goto dump_snmpEngineID_quit;
            break;
        ...
        /*NOTREACHED*/ case 5: /* Octets. */
            ...
            snprint_hexstring(s, (SNMP_MAXBUF - (s - buf)), esp, remaining_len);
            s = strchr(buf, '\0');
            s -= 1;
            goto dump_snmpEngineID_quit;
            break;

            /*NOTREACHED*/ dump_snmpEngineID_violation : case 0
                : /* Violation of RESERVED,
...
long atime_diff(const_marker_t first, const_marker_t second)
{
    ...
    NETSNMP_TIMERSUB((const struct timeval*)second,
                     (const struct timeval*)first, &diff);
 ...
 * \deprecated Don't use in new code.
 */
u_long uatime_hdiff(const_marker_t first, const_marker_t second)
{
    struct timeval diff;
...
char* netsnmp_getenv(const char* name)
{
    ...
    if (temp)
    {
        getenv_worked = 1;
        DEBUGMSGTL(
            ("read_config", "netsnmp_getenv will return from ENV: %s\n", temp));
    }
    ...
    if (temp == NULL)
    {
        ...
        if (RegOpenKeyExA(HKEY_CURRENT_USER, "SOFTWARE\\Net-SNMP", 0,
                          KEY_QUERY_VALUE, &hKey) == ERROR_SUCCESS)
        {

            if (RegQueryValueExA(hKey, name, NULL, &key_value_type,
                                 ...
                                 &key_value_size) == ERROR_SUCCESS)
            {
                ...
                /* Allocate memory needed +1 to allow RegQueryValueExA to NULL
                 * terminate the string data in registry is missing one (which
                 * is unlikely).
                 */
                key_value =
                    malloc((sizeof(char) * key_value_size) + sizeof(char));
            ...
            if (temp)
                DEBUGMSGTL(("read_config",
                            "netsnmp_getenv will return from HKCU: %s\n",
                            temp));
        }
    }
    ...
    if (temp == NULL)
    {
        if (RegOpenKeyExA(HKEY_LOCAL_MACHINE, "SOFTWARE\\Net-SNMP", 0,
                          KEY_QUERY_VALUE, &hKey) == ERROR_SUCCESS)
        {
            ...
            if (temp)
                DEBUGMSGTL(("read_config",
                            "netsnmp_getenv will return from HKLM: %s\n",
                            temp));
        }
    }
...
int netsnmp_string_time_to_secs(const char* time_string)
{
    ...
    switch (time_string[strlen(time_string) - 1])
    {
        ...
        default:
            snmp_log(LOG_ERR,
                     "time string %s contains an invalid suffix letter\n",
                     time_string);
            return -1;
    }
