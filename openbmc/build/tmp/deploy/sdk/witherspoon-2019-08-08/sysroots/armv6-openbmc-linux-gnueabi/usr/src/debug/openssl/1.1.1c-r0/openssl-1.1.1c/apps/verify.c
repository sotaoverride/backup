...
int verify_main(int argc, char** argv)
{
    ...
    {
        BIO_printf(bio_err, "%s: Cannot use -trusted with -CAfile or -CApath\n",
                   prog);
        goto end;
    }
    ...
    if (i > 0 && X509_STORE_CTX_get_error(csc) == X509_V_OK)
    {
        ...
        if (show_chain)
        {
            ...
            for (j = 0; j < sk_X509_num(chain); j++)
            {
                ...
                printf("depth=%d: ", j);
                X509_NAME_print_ex_fp(stdout, X509_get_subject_name(cert), 0,
                                      get_nameopt());
                if (j < num_untrusted)
                    printf(" (untrusted)");
                printf("\n");
...
static int cb(int ok, X509_STORE_CTX* ctx)
{
    ...
    if (!ok)
    {
        ...
        {
            X509_NAME_print_ex(bio_err, X509_get_subject_name(current_cert), 0,
                               get_nameopt());
            BIO_printf(bio_err, "\n");
        }
