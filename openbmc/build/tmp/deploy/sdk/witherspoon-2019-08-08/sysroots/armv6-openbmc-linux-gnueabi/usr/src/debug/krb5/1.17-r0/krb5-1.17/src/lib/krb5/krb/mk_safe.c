                       ...
                       krb5_cksumtype sumtype, krb5_data* outbuf)
{
    ...
    if ((retval = krb5_k_make_checksum(context, sumtype, key,
                                       KRB5_KEYUSAGE_KRB_SAFE_CKSUM, scratch1,
                                       &safe_checksum)))
 ...
 * mandatory checksum. */
static krb5_cksumtype safe_cksumtype(krb5_context context,
                                     ...
                                     krb5_enctype enctype)
{
    ...
    retval =
        krb5_c_keyed_checksum_types(context, enctype, &nsumtypes, &sumtypes);
    if (retval != 0)
        return 0;
