...
//
// Bug - when ICU tries to find a file that is equivalent to /etc/localtime it
// finds /usr/share/zoneinfo/localtime that is just a symbolic link to
// /etc/localtime.
//
// It started in 4.0 and was fixed in version 4.6, also the fix was backported
// to the 4.4 branch so it should be available from 4.4.3... So we test if the
...
//
    ...
    (U_ICU_VERSION_MINOR_NUM * 100 + U_ICU_VERSION_PATCHLEVEL_NUM) <= 402
...
std::string find_file_in(std::string const& ref, size_t size,
                         std::string const& dir)
{
    ...
    while ((name = d.next()) != 0)
    {
        ...
        if (file_name == "." || file_name == ".." ||
            file_name == "posixrules" || file_name == "localtime")
        {
            continue;
        }
        struct stat st;
