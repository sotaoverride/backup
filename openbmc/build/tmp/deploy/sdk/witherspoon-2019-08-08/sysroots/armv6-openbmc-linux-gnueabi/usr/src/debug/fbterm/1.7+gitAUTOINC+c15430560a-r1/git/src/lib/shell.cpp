...
void Shell::mouseInput(u16 x, u16 y, s32 type, s32 buttons)
{
    ...
    if (btn && rtype != Wheel &&
        (rtype == MouseNone || (modifies & ShiftButton)))
    {
    ...
    {
        if (modifies & ShiftButton)
            val |= 4;
        if (modifies & AltButton)
            val |= 8;
        if (modifies & ControlButton)
            val |= 16;
    }
    ...
    {
        s8 buf[8];
        snprintf(buf, sizeof(buf), "\e[M%c%c%c", ' ' + val, ' ' + x + 1,
                 ' ' + y + 1);
        sendBack(buf);
    }
}
...
void Shell::middleTextSelect(u16 x, u16 y)
{
    ...
    if (dir_sel == dir_new_sel)
    {
        ...
        if (dir_sel)
        {
            if (attr.type == CharAttr::DoubleLeft)
                pos++;
            pos++;
        }
        else
...
void Shell::endTextSelect()
{
    ...
    for (u16 y = sy; y <= ey; y++)
    {
        ...
        for (; x <= end; x++)
        {
            ...
            if (charAttr(x, y).type == CharAttr::DoubleLeft)
                x++;
        }
    }
