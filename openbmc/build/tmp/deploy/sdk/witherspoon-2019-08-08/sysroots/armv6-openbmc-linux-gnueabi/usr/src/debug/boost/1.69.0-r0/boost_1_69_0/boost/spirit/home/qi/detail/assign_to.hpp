...
struct assign_to_attribute_from_iterators
{
    ...
    // If Attribute is a container with value_type==iterator_range<T> just push
    // the iterator_range into it
    static void call(Iterator const& first, Iterator const& last,
...
struct assign_to_attribute_from_iterators<reference_wrapper<Attribute>,
                                          Iterator>
{
    static void call(Iterator const& first, Iterator const& last,
...
inline void assign_to(Iterator const& first, Iterator const& last,
                      Attribute& attr)
{
    assign_to_attribute_from_iterators<Attribute, Iterator>::call(first, last,
                                                                  attr);
}
...
struct assign_to_attribute_from_value
{
    ...
    typedef typename mpl::eval_if<is_one_element_sequence,
...
{
    static void call(Attribute const& val, Attribute& attr)
    {
        attr = val;
    }
};
...
struct assign_to_attribute_from_value<
    Attribute, reference_wrapper<T>,
    typename disable_if<is_same<Attribute, reference_wrapper<T>>>::type>
{
    static void call(reference_wrapper<T> const& val, Attribute& attr)
    {
...
template <typename Attribute, typename T>
struct assign_to_attribute_from_value<
    Attribute, boost::optional<T>,
    typename disable_if<is_same<Attribute, boost::optional<T>>>::type>
{
    static void call(boost::optional<T> const& val, Attribute& attr)
    {
        assign_to(val.get(), attr);
    }
};
...
struct assign_to_attribute_from_value<
    fusion::extension::adt_attribute_proxy<Attribute, N, Const>, T>
{
    static void
        call(T const& val,
             typename fusion::extension::adt_attribute_proxy<Attribute, N,
                                                             Const>& attr)
    {
        attr = val;
    }
...
struct is_same_size_sequence : mpl::bool_<fusion::result_of::size<A>::value ==
...
struct assign_to_container_from_value
{
    ...
    template <typename T_>
    static void append_to_container_not_string(T_ const& val, Attribute& attr,
                                               mpl::true_)
    {
        traits::push_back(attr, val);
    }
    ...
    {
        typedef
            typename traits::container_iterator<T_ const>::type iterator_type;
...
inline void assign_to(T const& val, Attribute& attr)
{
    typedef typename mpl::and_<
        traits::is_container<Attribute>, traits::not_is_variant<Attribute>,
