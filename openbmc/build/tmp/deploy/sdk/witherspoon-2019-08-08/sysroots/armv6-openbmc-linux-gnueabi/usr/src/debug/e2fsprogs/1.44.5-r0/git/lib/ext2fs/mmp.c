...
errcode_t ext2fs_mmp_read(ext2_filsys fs, blk64_t mmp_blk, void* buf)
{
    ...
    if (fs->mmp_cmp == NULL)
    {
        ...
        retval = ext2fs_get_memalign(fs->blocksize, align, &fs->mmp_cmp);
        if (retval)
            return retval;
    }

    if ((blk64_t)ext2fs_llseek(fs->mmp_fd, mmp_blk * fs->blocksize, SEEK_SET) !=
...
errcode_t ext2fs_mmp_write(ext2_filsys fs, blk64_t mmp_blk, void* buf)
{
    ...
    retval = io_channel_write_blk64(fs->io, mmp_blk,
...
#endif
    strncpy(mmp_s->mmp_bdevname, fs->device_name, sizeof(mmp_s->mmp_bdevname));
...
errcode_t ext2fs_mmp_stop(ext2_filsys fs)
{
    ...
    if (!ext2fs_has_feature_mmp(fs->super) || !(fs->flags & EXT2_FLAG_RW) ||
