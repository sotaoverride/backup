...
netsnmp_feature_child_of(data_list_all, libnetsnmp)
    ...
    /** @defgroup data_list generic linked-list data handling with a string as a
     * key.
     * @ingroup library
     * @{
     */
 ...
 * @return a newly created data_list node which can be given to the
 * netsnmp_add_list_data function.
 */
 ...
 * @param type the name of the application to save the data as.  If left NULL
 * the default application name that was registered during the init_snmp call
 * will be used (recommended).
 * @param token the unique token identifier string to use as the first word in
 * the persistent file line.
 * @param data_list_save_ptr a function pointer which will be called to save the
 * rest of the data to a buffer.
 * @param data_list_read_ptr a function pointer which can read the remainder of
 * a saved line and return the application specific void * pointer.
 * @param data_list_free_ptr a function pointer which will be passed to the data
 * node for freeing it in the future when/if the list/node is cleaned up or
 * destroyed.
 */
void netsnmp_register_save_list(netsnmp_data_list** datalist, const char* type,
                                const char* token,
                                ...
                                Netsnmp_Free_List_Data* data_list_free_ptr)
{
    ...
    {
        snmp_log(LOG_ERR,
                 "couldn't malloc a netsnmp_data_list_saveinfo typedef");
        return;
    }
    ...
    {
        /** @todo netsnmp_register_save_list should handle the same token name
         * being saved from different types? */
                                   ...
                                   void* clientarg)
{
    ...
    {
        snmp_log(LOG_WARNING,
                 "netsnmp_save_all_data_callback called with no passed data");
...
int netsnmp_save_all_data(netsnmp_data_list* head, const char* type,
                          const char* token,
                          Netsnmp_Save_List_Data* data_list_save_ptr)
{
    ...
    for (; head; head = head->next)
    {
        if (head->name)
        {
            ...
            *cp++ = ' ';
            ...
            if (!(data_list_save_ptr)(cp, sizeof(buf) - strlen(buf),
                                      head->data))
            {
...
void netsnmp_read_data_callback(const char* token, char* line)
{
    ...
    if (!info)
    {
        snmp_log(LOG_WARNING, "netsnmp_read_data_callback called without "
                              "previously registered subparser");
        return;
    }

    /* read in the token */
    line = read_config_read_data(ASN_OCTET_STR, line, &dataname, &dataname_len);
    ...
    netsnmp_add_list_data(
        info->datalist,
        netsnmp_create_data_list(dataname, data, info->data_list_free_ptr));
