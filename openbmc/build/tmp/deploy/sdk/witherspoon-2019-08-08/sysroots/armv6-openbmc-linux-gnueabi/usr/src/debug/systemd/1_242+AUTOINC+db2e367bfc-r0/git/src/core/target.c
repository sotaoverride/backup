...
static const UnitActiveState state_translation_table[_TARGET_STATE_MAX] = {
    [TARGET_DEAD] = UNIT_INACTIVE, [TARGET_ACTIVE] = UNIT_ACTIVE};
...
static int target_add_default_dependencies(Target* t)
{
    ...
    /* Imply ordering for requirement dependencies on target units. Note that
     * when the user created a contradicting ordering manually we won't add
     * anything in here to make sure we don't create a loop. */
...
static void target_dump(Unit* u, FILE* f, const char* prefix)
{
    ...
    fprintf(f, "%sTarget State: %s\n", prefix,
            target_state_to_string(t->state));
}
