...
class BOOST_FILESYSTEM_DECL path
{
  public:
    ...
    //  0x1B 0x24 0x42 [switch to X0208-1983] 0x24 0x2F [U+304F Kiragana letter
    //  KU]
    //                                             ^^^^
    ...
    //  Supported source arguments: half-open iterator range, container,
    //  c-array, and single pointer to null terminated string.

    //  All source arguments except pointers to null terminated byte strings
    //  support multi-byte character strings which may have embedded nulls.
    //  Embedded null support is required for some Asian languages on Windows.

    //  "const codecvt_type& cvt=codecvt()" default arguments are not used
    //  because this limits the impact of locale("") initialization failures on
    //  POSIX systems to programs that actually depend on locale(""). It further
    //  ensures that exceptions thrown as a result of such failues occur after
    //  main() has started, so can be caught.
    ...
    path(InputIterator begin, InputIterator end)
    {
        if (begin != end)
        {
            ...
            std::basic_string<
                ...
                seq(begin, end);
            path_traits::convert(seq.c_str(), seq.c_str() + seq.size(),
                                 m_pathname);
        }
    }
    ...
    typename boost::enable_if<
        path_traits::is_pathable<typename boost::decay<Source>::type>,
        path&>::type
    ...
    path&
        operator=(const value_type* ptr) // required in case ptr overlaps *this
    {
        m_pathname = ptr;
        return *this;
    }
    ...
    path& assign(const value_type* ptr,
                 const codecvt_type&) // required in case ptr overlaps *this
    {
        m_pathname = ptr;
        return *this;
    }
    ...
    void swap(path& rhs) BOOST_NOEXCEPT
    {
    ...
    //  paths, return values from observers are formatted as file names unless
    //  there is a trailing separator, in which case returns are formatted as
    //  directory paths. POSIX and Windows make no such distinction.

    //  Implementations are permitted to return const values or const
    //  references.

    //  The string or path returned by an observer are specified as being
    //  formatted as "native" or "generic".
    //
    //  For POSIX, these are all the same format; slashes and backslashes are as
    //  input and are not modified.
    //
    //  For Windows,   native:    as input; slashes and backslashes are not
    //  modified;
    //                            this is the format of the internally stored
    //                            string.
    //                 generic:   backslashes are converted to slashes

    //  -----  native format observers  -----

    const string_type& native() const BOOST_NOEXCEPT
    {
        return m_pathname;
    }
    const value_type* c_str() const BOOST_NOEXCEPT
    {
        return m_pathname.c_str();
    }
    string_type::size_type size() const BOOST_NOEXCEPT
    {
    ...
    const std::string string() const
    {
        std::string tmp;
        if (!m_pathname.empty())
            path_traits::convert(&*m_pathname.begin(),
                                 &*m_pathname.begin() + m_pathname.size(), tmp);
        return tmp;
    }
    ...
    const std::wstring& wstring(const codecvt_type&) const
    {
        return m_pathname;
    }
    ...
    //  Experimental generic function returning generic formatted path (i.e.
    //  separators are forward slashes). Motivation: simpler than a family of
    //  generic_*string functions.
...
#else // BOOST_POSIX_API
    //  On POSIX-like systems, the generic format is the same as the native
    //  format
    const std::string& generic_string() const
    {
        return m_pathname;
...
#error both BOOST_FILESYSTEM_DEPRECATED and BOOST_FILESYSTEM_NO_DEPRECATED are defined
...
#if defined(BOOST_FILESYSTEM_DEPRECATED)
    //  deprecated functions with enough signature or semantic changes that they
    //  are not supplied by default
    const std::string file_string() const
    {
        return string();
    }
    const std::string directory_string() const
    {
        return string();
    }
    ...
    const string_type external_file_string() const
    {
        return native();
    }
    const string_type external_directory_string() const
    {
        return native();
    }
...
#pragma warning(disable : 4251) // disable warning: class
                                // 'std::basic_string<_Elem,_Traits,_Ax>'
#endif                          // needs to have dll-interface...
                                /*
                                      m_pathname has the type, encoding, and format required by the native
                                      operating system. Thus for POSIX and Windows there is no conversion
                                   for                             passing m_pathname.c_str() to the O/S API or when obtaining a path
                                   from the                             O/S API. POSIX encoding is unspecified other than for dot and
                                   slash                             characters; POSIX just treats paths as a sequence of bytes. Windows
                                      encoding is UCS-2 or UTF-16 depending on the version.
                                */
    string_type m_pathname; // Windows: as input; backslashes NOT converted to
                            // slashes, slashes NOT converted to backslashes
#if defined(_MSC_VER)
#pragma warning(pop) // restore warning settings.
#endif

    string_type::size_type m_append_separator_if_needed();
    //  Returns: If separator is to be appended, m_pathname.size() before
    //  append. Otherwise 0. Note: An append is never performed if size()==0, so
    //  a returned 0 is unambiguous.
...
class path::iterator
    : public boost::iterator_facade<path::iterator, path const,
                                    boost::bidirectional_traversal_tag>
{
  private:
    ...
    void increment()
    {
        m_path_iterator_increment(*this);
    }
    void decrement()
    {
        m_path_iterator_decrement(*this);
    }
...
class path::reverse_iterator
    : public boost::iterator_facade<path::reverse_iterator, path const,
                                    boost::bidirectional_traversal_tag>
{
  public:
  ...
  private:
    ...
    bool equal(const reverse_iterator& rhs) const
    {
...
inline bool operator==(const path& lhs, const path::string_type& rhs)
{
    return lhs.compare(rhs) == 0;
}
inline bool operator==(const path::string_type& lhs, const path& rhs)
{
...
inline bool operator==(const path::value_type* lhs, const path& rhs)
{
    return rhs.compare(lhs) == 0;
}
...
{
    return os << boost::io::quoted(p.template string<std::basic_string<Char>>(),
                                   static_cast<Char>('&'));
}
...
namespace detail
{
//  For POSIX, is_directory_separator() and is_element_separator() are identical
//  since a forward slash is the only valid directory separator and also the
//  only valid element separator. For Windows, forward slash and back slash are
//  the possible directory separators, but colon (example: "c:foo") is also an
//  element separator.
...
{
    // implicit dot is tricky, so actually call filename(); see path::filename()
    // example in reference.html
    path p(filename());
...
{
    return size() >= 2 && m_pathname[size() - 1] == dot &&
           m_pathname[size() - 2] == dot &&
           (m_pathname.size() == 2 ||
            detail::is_element_separator(m_pathname[size() - 3]));
    // use detail::is_element_separator() rather than
    // detail::is_directory_separator to deal with "c:.." edge case on Windows
    // when ':' acts as a separator
}
...
template <>
inline std::string
    path::generic_string<std::string>(const codecvt_type& cvt) const
{
...
template <>
inline std::wstring
    path::generic_string<std::wstring>(const codecvt_type& cvt) const
{
