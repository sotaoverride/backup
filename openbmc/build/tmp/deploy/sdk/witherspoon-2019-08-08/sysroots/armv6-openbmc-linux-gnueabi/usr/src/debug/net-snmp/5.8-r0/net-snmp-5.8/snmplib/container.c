...
netsnmp_feature_child_of(container_all, libnetsnmp)
    ...
    netsnmp_feature_child_of(
        container_factories,
        container_all) netsnmp_feature_child_of(container_types, container_all)
            ...
            netsnmp_feature_child_of(container_dup, container_all)
                netsnmp_feature_child_of(container_free_all, container_all)
                    netsnmp_feature_child_of(subcontainer_find, container_all)
                        ...
                        netsnmp_feature_child_of(container_ncompare_cstring,
                                                 container_compare)
                            netsnmp_feature_child_of(container_compare_mem,
                                                     container_compare)
                                netsnmp_feature_child_of(container_compare_long,
                                                         container_compare)
                                    netsnmp_feature_child_of(
                                        container_compare_ulong,
                                        container_compare)
                                        netsnmp_feature_child_of(
...
/*------------------------------------------------------------------
 */
static void _factory_free(void* dat, void* context)
{
    ...
    if (data->name != NULL)
    {
        DEBUGMSGTL(("container", "  _factory_free_list() called for %s\n",
                    data->name));
        free(NETSNMP_REMOVE_CONST(
            void*,
            data->name)); /* SNMP_FREE wasted on object about to be freed */
    }
...
#ifndef NETSNMP_FEATURE_REMOVE_CONTAINER_LINKED_LIST
    netsnmp_container_register("linked_list", netsnmp_container_get_factory(
...
void netsnmp_container_free_list(void)
{
     ...
     */
    CONTAINER_FOR_EACH(containers, ((netsnmp_container_obj_func*)_factory_free),
                       NULL);
                                            ...
                                            netsnmp_container_compare* c)
{
    ...
    {
        DEBUGMSGT(
            ("container_registry", "replacing previous container factory\n"));
        ct->factory = f;
    }
    else
    {
...
netsnmp_factory* netsnmp_container_find_factory(const char* type_list)
{
...
/*------------------------------------------------------------------
 */
static container_type* netsnmp_container_get_ct(const char* type)
{
    container_type ct;
...
static container_type* netsnmp_container_find_ct(const char* type_list)
{
...
/*------------------------------------------------------------------
 */
netsnmp_container* netsnmp_container_find(const char* type)
{
    container_type* ct = netsnmp_container_find_ct(type);
    netsnmp_container* c =
        ct ? (netsnmp_container*)(ct->factory->produce()) : NULL;
...
int CONTAINER_INSERT_BEFORE(netsnmp_container* x, size_t pos, void* k)
{
    ...
    if (rc < 0)
        snmp_log(LOG_ERR,
                 "error on container '%s' insert_before %" NETSNMP_PRIz
                 "d (%d)\n",
...
int CONTAINER_REMOVE_AT(netsnmp_container* x, size_t pos, void** k)
{
    ...
    {
        snmp_log(LOG_ERR,
                 "error on container '%s' remove_at %" NETSNMP_PRIz "d (%d)\n",
                 x->container_name ? x->container_name : "", pos, rc);
        return rc;
    }
...
int CONTAINER_GET_AT(netsnmp_container* x, size_t pos, void** k)
{
    ...
    if (rc < 0)
        snmp_log(LOG_ERR,
                 "error on container '%s' get_at %" NETSNMP_PRIz "d (%d)\n",
                 x->container_name ? x->container_name : "", pos, rc);
 ...
 */
netsnmp_container* SUBCONTAINER_FIND(netsnmp_container* x, const char* name)
{
    if ((NULL == x) || (NULL == name))
        return NULL;
...
/*------------------------------------------------------------------
 */
void netsnmp_init_container(netsnmp_container* c, netsnmp_container_rc* init,
...
int netsnmp_container_data_dup(netsnmp_container* dup, netsnmp_container* c)
{
    if (!dup || !c)
        return -1;
...
int netsnmp_compare_netsnmp_index(const void* lhs, const void* rhs)
{
    ...
    rc = snmp_oid_compare(
        ((const netsnmp_index*)lhs)->oids, ((const netsnmp_index*)lhs)->len,
        ((const netsnmp_index*)rhs)->oids, ((const netsnmp_index*)rhs)->len);
...
int netsnmp_compare_mem(const char* lhs, size_t lhs_len, const char* rhs,
                        size_t rhs_len)
{
