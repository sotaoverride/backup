...
static void warn_bitmap(ext2fs_generic_bitmap bitmap, int code, __u64 arg)
{
#ifndef OMIT_COM_ERR
    if (bitmap->description)
        com_err(0, bitmap->base_error_code + code, "#%llu for %s", arg,
                bitmap->description);
    else
                                    ...
                                    ext2fs_generic_bitmap* ret)
{
    ...
    if (gettimeofday(&bitmap->stats.created, (struct timezone*)NULL) == -1)
    {
        perror("gettimeofday");
...
static void ext2fs_print_bmap_statistics(ext2fs_generic_bitmap bitmap)
{
    ...
    if (stats->test_count)
    {
        test_seq_perc = ((float)stats->test_seq / stats->test_count) * 100;
        test_back_perc = ((float)stats->test_back / stats->test_count) * 100;
    }

    if (stats->mark_count)
    {
        mark_seq_perc = ((float)stats->mark_seq / stats->mark_count) * 100;
        mark_back_perc = ((float)stats->mark_back / stats->mark_count) * 100;
    }
#endif
    ...
    inuse = (double)now.tv_sec + (((double)now.tv_usec) * 0.000001);
    ...
    fprintf(stderr,
            ...
            stats->mark_seq, mark_seq_perc, stats->test_back, test_back_perc);
    fprintf(stderr,
...
#endif
#ifdef ENABLE_BMAP_STATS
    if (gettimeofday(&new_bmap->stats.created, (struct timezone*)NULL) == -1)
    {
        perror("gettimeofday");
                                     ...
                                     __u64 new_real_end)
{
    ...
    if (EXT2FS_IS_32_BITMAP(bmap))
        return ext2fs_resize_generic_bitmap(bmap->magic, new_end, new_real_end,
                                            bmap);
                                        ...
                                        errcode_t neq, __u64 end, __u64* oend)
{
    ...
    if (EXT2FS_IS_32_BITMAP(bitmap))
    {
        ...
        retval = ext2fs_fudge_generic_bitmap_end(bitmap, bitmap->magic, neq,
                                                 end, &tmp_oend);
        if (oend)
            *oend = tmp_oend;
        return retval;
...
int ext2fs_mark_generic_bmap(ext2fs_generic_bitmap bitmap, __u64 arg)
{
    if (!bitmap)
        return 0;
    ...
    if (EXT2FS_IS_32_BITMAP(bitmap))
    {
        ...
        {
            ext2fs_warn_bitmap2(bitmap, EXT2FS_MARK_ERROR, 0xffffffff);
            return 0;
        }
...
int ext2fs_unmark_generic_bmap(ext2fs_generic_bitmap bitmap, __u64 arg)
{
    if (!bitmap)
        return 0;
    ...
    if (EXT2FS_IS_32_BITMAP(bitmap))
    {
        if (arg & ~0xffffffffULL)
        {
            ext2fs_warn_bitmap2(bitmap, EXT2FS_UNMARK_ERROR, 0xffffffff);
            return 0;
        }
...
int ext2fs_test_generic_bmap(ext2fs_generic_bitmap bitmap, __u64 arg)
{
    if (!bitmap)
        return 0;
    ...
    if (EXT2FS_IS_32_BITMAP(bitmap))
    {
        if (arg & ~0xffffffffULL)
        {
            ext2fs_warn_bitmap2(bitmap, EXT2FS_TEST_ERROR, 0xffffffff);
            return 0;
        }
                                        ...
                                        unsigned int num, void* in)
{
    ...
    if (EXT2FS_IS_32_BITMAP(bmap))
    {
        ...
        {
            ext2fs_warn_bitmap2(bmap, EXT2FS_UNMARK_ERROR, 0xffffffff);
            return EINVAL;
        }
        return ext2fs_set_generic_bitmap_range(bmap, bmap->magic, start, num,
                                               in);
    }
                                        ...
                                        unsigned int num, void* out)
{
    ...
    if (EXT2FS_IS_32_BITMAP(bmap))
    {
        ...
        return ext2fs_get_generic_bitmap_range(bmap, bmap->magic, start, num,
                                               out);
    }
                                      ...
                                      ext2fs_generic_bitmap bm2)
{
    ...
    if ((bm1->start != bm2->start) || (bm1->end != bm2->end))
        return neq;
...
int ext2fs_test_block_bitmap_range2(ext2fs_block_bitmap bmap, blk64_t block,
                                    unsigned int num)
{
    __u64 end = block + num;
    ...
    if (num == 1)
        return !ext2fs_test_generic_bmap((ext2fs_generic_bitmap)bmap, block);
    ...
    {
        ext2fs_warn_bitmap(EXT2_ET_BAD_BLOCK_TEST, block, bmap->description);
        return EINVAL;
    }
...
void ext2fs_mark_block_bitmap_range2(ext2fs_block_bitmap bmap, blk64_t block,
                                     unsigned int num)
{
    __u64 end = block + num;
    ...
    if (EXT2FS_IS_32_BITMAP(bmap))
    {
        ...
        ext2fs_mark_block_bitmap_range((ext2fs_generic_bitmap)bmap, block, num);
    }
    ...
    if ((block < bmap->start) || (block + num - 1 > bmap->end))
    {
        ext2fs_warn_bitmap(EXT2_ET_BAD_BLOCK_MARK, block, bmap->description);
        return;
    }
...
void ext2fs_unmark_block_bitmap_range2(ext2fs_block_bitmap bmap, blk64_t block,
                                       unsigned int num)
{
    __u64 end = block + num;
    ...
    if (EXT2FS_IS_32_BITMAP(bmap))
    {
        ...
        ext2fs_unmark_block_bitmap_range((ext2fs_generic_bitmap)bmap, block,
                                         num);
    }
    ...
    if ((block < bmap->start) || (block + num - 1 > bmap->end))
    {
        ext2fs_warn_bitmap(EXT2_ET_BAD_BLOCK_UNMARK, block, bmap->description);
        return;
    }
...
void ext2fs_warn_bitmap32(ext2fs_generic_bitmap bitmap, const char* func)
{
    ...
    if (bitmap && bitmap->description)
        ...
        com_err(0, EXT2_ET_MAGIC_GENERIC_BITMAP, "called %s with 64-bit bitmap",
                func);
#endif
}
                                           ...
                                           ext2fs_block_bitmap* bitmap)
{
    ...
    retval =
        ext2fs_allocate_block_bitmap(fs, "converted cluster bitmap", &cmap);
    if (retval)
        return retval;
                                              ...
                                              __u64* out)
{
    ...
    if (EXT2FS_IS_32_BITMAP(bitmap))
    {
        ...
        retval =
            ext2fs_find_first_zero_generic_bitmap(bitmap, start, end, &blk);
        if (retval == 0)
            *out = blk;
        return retval;
    }
    ...
    {
        retval =
            bitmap->bitmap_ops->find_first_zero(bitmap, cstart, cend, &cout);
        if (retval)
            return retval;
    found:
                                             ...
                                             __u64 start, __u64 end, __u64* out)
{
    ...
    if (EXT2FS_IS_32_BITMAP(bitmap))
    {
        ...
        retval = ext2fs_find_first_set_generic_bitmap(bitmap, start, end, &blk);
        if (retval == 0)
            *out = blk;
        return retval;
    }
    ...
    {
        retval =
            bitmap->bitmap_ops->find_first_set(bitmap, cstart, cend, &cout);
        if (retval)
            return retval;
    found:
        cout <<= bitmap->cluster_bits;
