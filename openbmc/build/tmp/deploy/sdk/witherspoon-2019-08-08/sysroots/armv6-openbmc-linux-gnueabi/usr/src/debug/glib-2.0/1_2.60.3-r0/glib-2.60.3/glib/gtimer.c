...
static time_t mktime_utc(struct tm* tm)
{
...
#ifndef HAVE_TIMEGM
    static const gint days_before[] = {0,   31,  59,  90,  120, 151,
    ...
    retval =
        ((((retval * 24) + tm->tm_hour) * 60) + tm->tm_min) * 60 + tm->tm_sec;
#else
    retval = timegm(tm);
#endif /* !HAVE_TIMEGM */
...
gboolean g_time_val_from_iso8601(const gchar* iso_date, GTimeVal* time_)
{
    ...
    else if (*iso_date == '+' || *iso_date == '-')
    {
        ...
        time_->tv_sec =
            mktime_utc(&tm) + (time_t)(60 * (gint64)(60 * hour + min) * sign);
    }
    else
    {
...
gchar* g_time_val_to_iso8601(GTimeVal* time_)
{
    ...
    g_return_val_if_fail(time_->tv_usec >= 0 && time_->tv_usec < G_USEC_PER_SEC,
                         NULL);
    ...
    else
    {
        ...
        retval = g_strdup_printf(
            "%4d-%02d-%02dT%02d:%02d:%02dZ", tm->tm_year + 1900, tm->tm_mon + 1,
