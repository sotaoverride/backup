...
static void socket_event(AvahiWatch* w, int fd,
                         AVAHI_GCC_UNUSED AvahiWatchEvent event, void* userdata)
{
...
AvahiNetlink* avahi_netlink_new(const AvahiPoll* poll_api, uint32_t groups,
                                void (*cb)(AvahiNetlink* nl, struct nlmsghdr* n,
                                           void* userdata),
                                void* userdata)
{
    int fd = -1;
    ...
    if (!(nl->watch = poll_api->watch_new(poll_api, fd, AVAHI_WATCH_IN,
                                          socket_event, nl)))
    {
