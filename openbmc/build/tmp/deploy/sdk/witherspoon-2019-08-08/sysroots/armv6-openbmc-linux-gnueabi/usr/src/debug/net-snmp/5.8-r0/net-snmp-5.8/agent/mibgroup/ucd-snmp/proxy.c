...
void proxy_parse_config(const char* token, char* line)
{
    ...
    listpp = &proxies;
    while (*listpp &&
           snmp_oid_compare(newp->name, newp->name_len, (*listpp)->name,
                            (*listpp)->name_len) > 0)
    {
        listpp = &((*listpp)->next);
    }
    ...
    *listpp = newp;
    ...
    reg = netsnmp_create_handler_registration(
        "proxy", proxy_handler, newp->name, newp->name_len, HANDLER_CAN_RWRITE);
...
void proxy_free_config(void)
{
    ...
    while (proxies)
    {
        ...
        unregister_mib_context(rm->name, rm->name_len, DEFAULT_MIB_PRIORITY, 0,
                               0, rm->context);
 ...
 */
int proxy_fill_in_session(netsnmp_mib_handler* handler,
 ...
 */
void proxy_free_filled_in_session_args(netsnmp_session* session,
                                       void** configured)
{
...
int proxy_handler(netsnmp_mib_handler* handler,
                  ...
                  netsnmp_request_info* requests)
{
    ...
    while (request)
    {
        ...
        if (sp->base_len && reqinfo->mode == MODE_GETNEXT &&
            (snmp_oid_compare(ourname, ourlength, sp->base, sp->base_len) < 0))
        {
        ...
        snmp_pdu_add_variable(pdu, ourname, ourlength, request->requestvb->type,
    ...
    snmp_async_send(sp->sess, pdu, proxy_got_response,
                    netsnmp_create_delegated_cache(handler, reginfo, reqinfo,
                                                   requests, (void*)sp));
...
int proxy_got_response(int operation, netsnmp_session* sess, int reqid,
                       netsnmp_pdu* pdu, void* cb_data)
{
    ...
    switch (operation)
    {
        case NETSNMP_CALLBACK_OP_TIMED_OUT:
             ...
             */
            DEBUGMSGTL(
                ("proxy", "got timed out... requests = %8p\n", requests));
            ...
            if (cache->reqinfo->mode != MODE_GETNEXT)
            {
                ...
                netsnmp_set_request_error(
                    cache->reqinfo, requests, /* XXXWWW: should be index = 0 */
                    SNMP_ERR_GENERR);
            }
        ...
        case NETSNMP_CALLBACK_OP_RECEIVED_MESSAGE:
            ...
            if (pdu->errstat != SNMP_ERR_NOERROR)
            {
                 ...
                 * it passes that error back to the comman initiator. What it
                 * should do is ignore the error and move on to the next tree.
                 * To accomplish that, all we need to do is clear the delegated
                 * flag. Not sure if any other error codes need the same
                 * treatment. Left as an exercise to the reader...
                 */
                DEBUGMSGTL(
                    ("proxy", "got error response (%ld)\n", pdu->errstat));
                if ((cache->reqinfo->mode == MODE_GETNEXT) &&
                ...
                else if (cache->reqinfo->mode == MODE_SET_ACTION)
                {
                     ...
                     */
                    DEBUGMSGTL(("proxy", "got SET error %s, index %ld\n",
                ...
                for (var = vars, request = requests; request && var;
                     request = request->next, var = var->next_variable)
                {
                                          ...
                                          sp->base_len) != 0))
                    {
                        ...
                        continue;
                             ...
                             (var->name_length < sp->name_len ||
                              snmp_oid_compare(var->name, sp->name_len,
                    ...
                    else
                    {
                        ...
                        if (sp->base_len)
                        {
                             ...
                             * XXX: oid size maxed?
                             */
                            memcpy(myname, sp->name,
                                   sizeof(oid) * sp->name_len);
                            myname_len =
                            ...
                            if (myname_len > MAX_OID_LEN)
                            {
                                ...
                                netsnmp_set_request_error(
                                    cache->reqinfo, requests, SNMP_ERR_GENERR);
                                if (pdu)
                                    snmp_free_pdu(pdu);
                                netsnmp_free_delegated_cache(cache);
                                return 1;
