...
class BOOST_SYMBOL_VISIBLE timer::impl : public attribute::impl
{
  private:
#if !defined(BOOST_LOG_NO_THREADS)
    ...
    attribute_value get_value()
    {
        uint64_t duration;
        {
            BOOST_LOG_EXPR_IF_MT(
                log::aux::exclusive_lock_guard<mutex_type> lock(m_Mutex);)
            ...
            duration =
                m_Duration + static_cast<uint64_t>(counts * m_FrequencyFactor);
            m_Duration = duration;
        }
...
#include <boost/log/detail/footer.hpp>
      ...
      // !defined(BOOST_LOG_NO_QUERY_PERFORMANCE_COUNTER)
