...
krb5_key_salt_tuple def_kslist = {ENCTYPE_DES_CBC_CRC,
...
struct realm_info
{
    ...
    krb5_key_salt_tuple* kslist;
            ...
            KRB5_KDB_DEF_FLAGS, (krb5_keyblock*)NULL, 1,
            &def_kslist};
...
static krb5_error_code add_principal(krb5_context, krb5_principal, enum ap_op,
                                     struct realm_info*);
...
krb5_data tgt_princ_entries[] = {{0, KRB5_TGS_NAME_SIZE, KRB5_TGS_NAME},
                                 {0, 0, 0}};
...
void kdb5_create(argc, argv) int argc;
char* argv[];
{
    int optchar;
    ...
    if ((retval = krb5_db_setup_mkey_name(util_context, global_params.mkey_name,
                                          global_params.realm, &mkey_fullname,
                                          &master_princ)))
    {
    ...
    krb5_princ_set_realm_data(util_context, &db_create_princ,
                              global_params.realm);
    krb5_princ_set_realm_length(util_context, &db_create_princ,
                                strlen(global_params.realm));
    krb5_princ_set_realm_data(util_context, &tgt_princ, global_params.realm);
    krb5_princ_set_realm_length(util_context, &tgt_princ,
                                strlen(global_params.realm));
    ...
    if (!mkey_password)
    {
        ...
        retval = krb5_read_password(util_context, KRB5_KDC_MKEY_1,
                                    KRB5_KDC_MKEY_2, pw_str, &pw_size);
        if (retval)
        {
    ...
    retval = krb5_c_string_to_key(util_context, master_keyblock.enctype, &pwd,
                                  &master_salt, &master_keyblock);
    if (retval)
    {
        com_err(progname, retval,
    ...
    {
        com_err(progname, retval, _("while initializing random key generator"));
        exit_status++;
        return;
    }
    ...
    if ((retval =
             add_principal(util_context, master_princ, MASTER_KEY, &rblock)) ||
    ...
    retval = krb5_db_store_master_key(util_context, global_params.stash_file,
                                      master_princ, mkey_kvno, &master_keyblock,
                                      mkey_password);
    if (retval)
    {
...
static krb5_error_code tgt_keysalt_iterate(ksent,
                                           ptr) krb5_key_salt_tuple* ksent;
krb5_pointer ptr;
{
    ...
    if (!(kret = krb5_dbe_create_key_data(iargs->ctx, iargs->dbentp)))
    {
        ...
        if (!(kret = krb5_c_make_random_key(context, ksent->ks_enctype, &key)))
        {
...
static krb5_error_code add_principal(context, princ, op,
                                     pblock) krb5_context context;
...
struct realm_info* pblock;
{
    ...
    if ((retval = krb5_dbe_update_mod_princ_data(context, entry, now,
                                                 &db_create_princ)))
        goto error_out;
    ...
    switch (op)
    {
        case MASTER_KEY:
            if ((entry->key_data =
                     (krb5_key_data*)malloc(sizeof(krb5_key_data))) == NULL)
                goto error_out;
            ...
            if ((retval = krb5_dbe_encrypt_key_data(
                     context, pblock->key, &master_keyblock, NULL, mkey_kvno,
                     entry->key_data)))
                return retval;
            /*
        ...
        case TGT_KEY:
             ...
             */
            if ((retval = krb5_keysalt_iterate(pblock->kslist, pblock->nkslist,
                                               1, tgt_keysalt_iterate,
                                               (krb5_pointer)&iargs)))
                return retval;
            break;
        case NULL_KEY:
    ...
    entry->mask =
        (KADM5_KEY_DATA | KADM5_PRINCIPAL | KADM5_ATTRIBUTES | KADM5_MAX_LIFE |
         KADM5_MAX_RLIFE | KADM5_TL_DATA | KADM5_PRINC_EXPIRE_TIME);
