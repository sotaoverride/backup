...
class BOOST_SYMBOL_VISIBLE error_category
{
  ...
  protected:
    ...
    explicit BOOST_SYSTEM_CONSTEXPR
        error_category(boost::ulong_long_type id) BOOST_NOEXCEPT : id_(id)
    {
    }
  ...
  public:
    ...
    virtual bool
        equivalent(int code,
                   const error_condition& condition) const BOOST_NOEXCEPT;
    virtual bool equivalent(const error_code& code,
                            int condition) const BOOST_NOEXCEPT;

    virtual std::string message(int ev) const = 0;
    virtual char const* message(int ev, char* buffer,
                                std::size_t len) const BOOST_NOEXCEPT;

    virtual bool failed(int ev) const BOOST_NOEXCEPT;

    BOOST_SYSTEM_CONSTEXPR bool
        operator==(const error_category& rhs) const BOOST_NOEXCEPT
    {
...
class BOOST_SYMBOL_VISIBLE generic_error_category : public error_category
{
  public:
    ...
    BOOST_SYSTEM_CONSTEXPR generic_error_category() BOOST_NOEXCEPT
        : error_category((boost::ulong_long_type(0xB2AB117A) << 32) +
                         0x257EDF0D)
    {
    }
    ...
    char const* message(int ev, char* buffer,
                        std::size_t len) const BOOST_NOEXCEPT;
};
...
class BOOST_SYMBOL_VISIBLE system_error_category : public error_category
{
  public:
    BOOST_SYSTEM_CONSTEXPR system_error_category() BOOST_NOEXCEPT
        : error_category((boost::ulong_long_type(0x8FAFD21E) << 32) +
                         0x25C5E09B)
    {
    }
...
struct cat_holder
{
    BOOST_SYSTEM_REQUIRE_CONST_INIT static constexpr system_error_category
        system_category_instance{};
    BOOST_SYSTEM_REQUIRE_CONST_INIT static constexpr generic_error_category
        generic_category_instance{};
};

template <class T>
BOOST_SYSTEM_REQUIRE_CONST_INIT constexpr system_error_category
    cat_holder<T>::system_category_instance;
template <class T>
BOOST_SYSTEM_REQUIRE_CONST_INIT constexpr generic_error_category
    cat_holder<T>::generic_category_instance;
...
BOOST_SYSTEM_CONSTEXPR inline bool failed_impl(int ev,
  ...
  public:
    ...
    BOOST_SYSTEM_CONSTEXPR error_condition(
        ...
        typename detail::enable_if<
            is_error_condition_enum<ErrorConditionEnum>::value>::type* = 0)
        BOOST_NOEXCEPT
    {
    ...
    BOOST_SYSTEM_CONSTEXPR void assign(int val,
                                       const error_category& cat) BOOST_NOEXCEPT
    {
        val_ = val;
        failed_ = detail::failed_impl(val, cat);
        cat_ = &cat;
    ...
    BOOST_SYSTEM_CONSTEXPR typename detail::enable_if<
    ...
    BOOST_SYSTEM_CONSTEXPR explicit
        operator bool() const BOOST_NOEXCEPT // true if error
    {
        return failed_;
    }
    ...
    BOOST_SYSTEM_CONSTEXPR
        operator unspecified_bool_type() const BOOST_NOEXCEPT // true if error
    {
        return failed_ ? unspecified_bool_true : 0;
    }

    BOOST_SYSTEM_CONSTEXPR bool operator!()
        const BOOST_NOEXCEPT // true if no error
    {
        return !failed_;
    }
    ...
    BOOST_SYSTEM_CONSTEXPR inline friend bool
        operator==(const error_condition& lhs,
                   const error_condition& rhs) BOOST_NOEXCEPT
    {
                  ...
                  const error_condition& rhs) BOOST_NOEXCEPT
    {
        return *lhs.cat_ < *rhs.cat_ ||
               (*lhs.cat_ == *rhs.cat_ && lhs.val_ < rhs.val_);
    }
...
class error_code
{
  ...
  public:
    ...
    BOOST_SYSTEM_CONSTEXPR error_code(int val,
                                      const error_category& cat) BOOST_NOEXCEPT
        : val_(val),
          failed_(detail::failed_impl(val, cat)),
          cat_(&cat)
    ...
    BOOST_SYSTEM_CONSTEXPR error_code(
        ErrorCodeEnum e,
        typename detail::enable_if<
            is_error_code_enum<ErrorCodeEnum>::value>::type* = 0) BOOST_NOEXCEPT
    {
    ...
    BOOST_SYSTEM_CONSTEXPR
        typename detail::enable_if<is_error_code_enum<ErrorCodeEnum>::value,
                                   error_code>::type&
    ...
    BOOST_SYSTEM_CONSTEXPR inline friend bool
        operator==(const error_code& lhs, const error_code& rhs) BOOST_NOEXCEPT
    {
        return lhs.val_ == rhs.val_ && *lhs.cat_ == *rhs.cat_;
    }
...
BOOST_SYSTEM_CONSTEXPR inline bool
    operator!=(const error_code& lhs, const error_code& rhs) BOOST_NOEXCEPT
{
    return !(lhs == rhs);
}
...
inline bool operator==(const error_code& code,
                       const error_condition& condition) BOOST_NOEXCEPT
{
...
inline bool operator==(const error_condition& condition,
                       const error_code& code) BOOST_NOEXCEPT
{
    return code.category().equivalent(code.value(), condition) ||
           condition.category().equivalent(code, condition.value());
}
...
inline bool operator!=(const error_condition& lhs,
                       const error_code& rhs) BOOST_NOEXCEPT
{
    return !(lhs == rhs);
}
...
inline std::size_t hash_value(error_code const& ec)
{
    ...
    boost::ulong_long_type hv =
        (boost::ulong_long_type(0xCBF29CE4) << 32) + 0x84222325;
    boost::ulong_long_type const prime =
        (boost::ulong_long_type(0x00000100) << 32) + 0x000001B3;
...
// explicit conversion:
BOOST_SYSTEM_CONSTEXPR inline error_code
    make_error_code(errc_t e) BOOST_NOEXCEPT
{
    return error_code(e, generic_category());
}

// implicit conversion:
BOOST_SYSTEM_CONSTEXPR inline error_condition
    make_error_condition(errc_t e) BOOST_NOEXCEPT
{
...
inline error_condition
    error_category::default_error_condition(int ev) const BOOST_NOEXCEPT
{
    return error_condition(ev, *this);
}

inline bool error_category::equivalent(
    int code, const error_condition& condition) const BOOST_NOEXCEPT
{
...
inline char const* error_category::message(int ev, char* buffer,
                                           std::size_t len) const BOOST_NOEXCEPT
{
    if (len == 0)
    {
...
inline std::string
    boost::system::detail::generic_error_category::message(int ev) const
{
    return generic_error_category_message(ev);
}

inline char const* boost::system::detail::generic_error_category::message(
    int ev, char* buffer, std::size_t len) const BOOST_NOEXCEPT
{
...
inline std::string
    boost::system::detail::system_error_category::message(int ev) const
{
    return system_category_message_win32(ev);
}

inline char const* boost::system::detail::system_error_category::message(
    int ev, char* buffer, std::size_t len) const BOOST_NOEXCEPT
{
