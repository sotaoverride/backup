...
gcry_error_t gcry_sexp_new(gcry_sexp_t* retsexp, const void* buffer,
...
{
    return gpg_error(
        _gcry_sexp_create(retsexp, buffer, length, autodetect, freefnc));
}
...
gcry_error_t gcry_mpi_print(enum gcry_mpi_format format, unsigned char* buffer,
                            size_t buflen, size_t* nwritten, const gcry_mpi_t a)
{
...
gcry_error_t gcry_cipher_open(gcry_cipher_hd_t* handle, int algo, int mode,
                              unsigned int flags)
{
...
gcry_error_t gcry_cipher_setiv(gcry_cipher_hd_t hd, const void* iv,
                               size_t ivlen)
{
    if (!fips_is_operational())
        return gpg_error(fips_not_operational());
...
int gcry_mac_map_name(const char* string)
{
...
void gcry_mac_close(gcry_mac_hd_t hd)
{
    _gcry_mac_close(hd);
}
...
gcry_error_t gcry_mac_write(gcry_mac_hd_t hd, const void* buf, size_t buflen)
{
    if (!fips_is_operational())
        return gpg_error(fips_not_operational());
...
gcry_error_t gcry_mac_read(gcry_mac_hd_t hd, void* outbuf, size_t* outlen)
{
    if (!fips_is_operational())
        return gpg_error(fips_not_operational());
...
gcry_error_t gcry_mac_verify(gcry_mac_hd_t hd, const void* buf, size_t buflen)
{
    if (!fips_is_operational())
        return gpg_error(fips_not_operational());
...
gcry_error_t gcry_pk_encrypt(gcry_sexp_t* result, gcry_sexp_t data,
                             gcry_sexp_t pkey)
{
    if (!fips_is_operational())
    {
...
gcry_error_t gcry_pk_decrypt(gcry_sexp_t* result, gcry_sexp_t data,
                             gcry_sexp_t skey)
{
    if (!fips_is_operational())
    {
...
gcry_error_t gcry_md_extract(gcry_md_hd_t hd, int algo, void* buffer,
                             size_t length)
{
                            ...
                            size_t keysize, void* keybuffer)
{
    return gpg_error(_gcry_kdf_derive(passphrase, passphraselen, algo, hashalgo,
                                      salt, saltlen, iterations, keysize,
                                      keybuffer));
}
...
gcry_error_t gcry_prime_generate(gcry_mpi_t* prime, unsigned int prime_bits,
...
gcry_error_t gcry_prime_group_generator(gcry_mpi_t* r_g, gcry_mpi_t prime,
