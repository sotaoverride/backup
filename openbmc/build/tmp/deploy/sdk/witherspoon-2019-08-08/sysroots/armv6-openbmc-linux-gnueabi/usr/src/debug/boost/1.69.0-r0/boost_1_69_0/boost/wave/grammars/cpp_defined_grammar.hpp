...
#if !defined(                                                                  \
    CPP_DEFINED_GRAMMAR_HPP_F48287B2_DC67_40A8_B4A1_800EFBD67869_INCLUDED)
...
{
    defined_grammar(ContainerT& result_seq_) : result_seq(result_seq_)
    {
        ...
        definition(defined_grammar const& self)
        {
            ...
            defined_op // parens not required, see C++ standard 16.1.1
                = ch_p(T_IDENTIFIER) // token contains 'defined'
                  >> ((ch_p(T_LEFTPAREN) >> identifier >> ch_p(T_RIGHTPAREN)) |
                      identifier);

            identifier =
                ch_p(T_IDENTIFIER)[spirit_append_actor(self.result_seq)] |
                pattern_p(
                    KeywordTokenType,
                    TokenTypeMask |
                        PPTokenFlag)[spirit_append_actor(self.result_seq)] |
                pattern_p(
            ...
            BOOST_SPIRIT_DEBUG_TRACE_RULE(defined_op,
                                          TRACE_CPP_DEFINED_GRAMMAR);
            BOOST_SPIRIT_DEBUG_TRACE_RULE(identifier,
                                          TRACE_CPP_DEFINED_GRAMMAR);
        }
...
BOOST_WAVE_DEFINED_GRAMMAR_GEN_INLINE boost::spirit::classic::parse_info<
