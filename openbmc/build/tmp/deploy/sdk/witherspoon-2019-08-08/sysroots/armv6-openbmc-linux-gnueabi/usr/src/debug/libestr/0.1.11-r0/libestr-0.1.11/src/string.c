...
es_str_t* es_newStr(es_size_t lenhint)
{
    ...
    if (sizeof(es_str_t) + lenhint < lenhint)
    { /* overflow? */
        s = NULL;
        goto done;
    }
...
int es_strbufcmp(es_str_t* s, const unsigned char* buf, es_size_t lenBuf)
{
    int r;
    es_size_t i;
    unsigned char* c;
 ...
 */
int es_strcasebufcmp(es_str_t* s, const unsigned char* buf, es_size_t lenBuf)
{
    int r;
    es_size_t i;
    unsigned char* c;
...
int es_addBuf(es_str_t** ps1, const char* buf, es_size_t lenBuf)
{
    int r;
    es_size_t newlen;
    es_str_t* s1 = *ps1;
    ...
    {
        /* we need to extend */
        if ((r = es_extendBuf(ps1, newlen - s1->lenBuf)) != 0)
            goto done;
        s1 = *ps1;
    }
...
char* es_str2cstr(es_str_t* s, const char* nulEsc)
{
    ...
    if (nbrNUL == 0)
    {
        ...
        if ((cstr = malloc(s->lenStr + 1)) == NULL)
            goto done;
        if (s->lenStr > 0)
...
static inline long long es_str2num_dec(es_str_t* s, unsigned i, int* bSuccess)
{
    long long num;
    unsigned char* c;
...
static inline long long es_str2num_oct(es_str_t* s, int* bSuccess)
{
    long long num;
    unsigned char* c;
    unsigned i;
...
static inline long long es_str2num_hex(es_str_t* s, int* bSuccess)
{
    long long num;
    unsigned char* c;
    unsigned i;
 ...
 */
static inline void doUnescape(unsigned char* c, es_size_t lenStr,
                              es_size_t* iSrc, es_size_t iDst)
{
    if (c[*iSrc] == '\\')
    {
        if (++(*iSrc) == lenStr)
        ...
        switch (c[*iSrc])
        {
            ...
            case 'x':
                ...
                if ((*iSrc) + 2 == lenStr || !isxdigit(c[(*iSrc) + 1]) ||
                    !isxdigit(c[(*iSrc) + 2]))
                {
