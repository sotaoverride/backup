...
int config_parse_ip_address_access(const char* unit, const char* filename,
                                   ...
                                   void* userdata)
{
    ...
    for (;;)
    {
        ...
        else if (streq(word, "link-local"))
        {
            ...
            a->family = AF_INET6;
            a->address.in6 =
                (struct in6_addr){.s6_addr32[0] = htobe32(0xfe800000)};
            a->prefixlen = 64;
        }
        else if (streq(word, "multicast"))
        {
            ...
            a->family = AF_INET6;
            a->address.in6 =
                (struct in6_addr){.s6_addr32[0] = htobe32(0xff000000)};
            a->prefixlen = 8;
        }
        else
        {
            r = in_addr_prefix_from_string_auto(word, &a->family, &a->address,
                                                &a->prefixlen);
            if (r < 0)
            {
                log_syntax(unit, LOG_WARNING, filename, line, r,
    ...
    *list = ip_address_access_reduce(*list);
    ...
    if (*list)
    {
        ...
        if (r == BPF_FIREWALL_UNSUPPORTED)
        {
            ...
            log_full(
                warned ? LOG_DEBUG : LOG_WARNING,
                "File %s:%u configures an IP firewall (%s=%s), but the local "
                "system does not support BPF/cgroup based firewalling.\n"
                "Proceeding WITHOUT firewalling in effect! (This warning is "
                "only shown for the first loaded unit using IP firewalling.)",
                filename, line, lvalue, rvalue);
...
IPAddressAccessItem* ip_address_access_reduce(IPAddressAccessItem* first)
{
    ...
    /* Drops all entries from the list that are covered by another entry in
     * full, thus removing all redundant entries. */
    ...
    LIST_FOREACH_SAFE(items, a, tmp, first)
    {
        ...
        LIST_FOREACH(items, b, first)
        {
            ...
            r = in_addr_prefix_covers(b->family, &b->address, b->prefixlen,
                                      &a->address);
            if (r > 0)
            {
