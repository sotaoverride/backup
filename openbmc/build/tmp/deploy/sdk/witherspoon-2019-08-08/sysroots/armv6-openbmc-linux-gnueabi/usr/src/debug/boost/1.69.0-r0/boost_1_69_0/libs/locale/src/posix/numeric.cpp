...
class num_format : public util::base_num_format<CharType>
{
  public:
    ...
    num_format(boost::shared_ptr<locale_t> lc, size_t refs = 0) :
        util::base_num_format<CharType>(refs), lc_(lc)
    {
    }
  ...
  protected:
                                         ...
                                         long double val) const
    {
        ...
        errno = 0;
        ssize_t n =
            strfmon_l(buf, sizeof(buf), *lc_, format, static_cast<double>(val));
        if (n >= 0)
            return write_it(out, buf, n);

        for (std::vector<char> tmp(sizeof(buf) * 2); tmp.size() <= 4098;
             tmp.resize(tmp.size() * 2))
        {
            n = strfmon_l(&tmp.front(), tmp.size(), *lc_, format,
    ...
    std::ostreambuf_iterator<char> write_it(std::ostreambuf_iterator<char> out,
                                            char const* ptr, size_t n) const
    {
    ...
    std::ostreambuf_iterator<wchar_t>
        write_it(std::ostreambuf_iterator<wchar_t> out, char const* ptr,
                 size_t n) const
    {
        std::wstring tmp =
            conv::to_utf<wchar_t>(ptr, ptr + n, nl_langinfo_l(CODESET, *lc_));
...
{
    static std::string ftime(char const* format, const struct tm* t,
                             locale_t lc)
    {
        char buf[16];
...
{
    static std::wstring ftime(wchar_t const* format, const struct tm* t,
                              locale_t lc)
    {
...
class time_put_posix : public std::time_put<CharType>
{
  public:
    time_put_posix(boost::shared_ptr<locale_t> lc, size_t refs = 0) :
        std::time_put<CharType>(refs), lc_(lc)
    {
    }
    ...
    virtual iter_type do_put(iter_type out, std::ios_base& /*ios*/,
                             ...
                             char modifier) const
    {
        char_type fmt[4] = {'%', modifier != 0 ? modifier : format,
                            modifier == 0 ? '\0' : format};
...
class ctype_posix<wchar_t> : public std::ctype<wchar_t>
{
  public:
    ...
    wchar_t const* do_is(wchar_t const* begin, wchar_t const* end,
                         mask* m) const
    {
        while (begin != end)
        {
    ...
    wchar_t const* do_scan_is(mask m, wchar_t const* begin,
    ...
    wchar_t const* do_scan_not(mask m, wchar_t const* begin,
                               wchar_t const* end) const
    {
        while (begin != end)
            if (!do_is(m, *begin))
...
template <typename CharType>
std::locale create_formatting_impl(std::locale const& in,
...
template <typename CharType>
std::locale create_parsing_impl(std::locale const& in,
                                boost::shared_ptr<locale_t> lc)
{
    std::locale tmp = std::locale(in, new num_punct_posix<CharType>(*lc));
    tmp = std::locale(tmp, new ctype_posix<CharType>(lc));
