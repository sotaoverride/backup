...
static void g_poll_file_monitor_finalize(GObject* object)
{
...
static void g_poll_file_monitor_class_init(GPollFileMonitorClass* klass)
{
...
static void g_poll_file_monitor_init(GPollFileMonitor* poll_monitor)
{
}

static int calc_event_type(GFileInfo* last, GFileInfo* new)
{
...
static void got_new_info(GObject* source_object, GAsyncResult* res,
                         gpointer user_data)
{
...
static gboolean poll_file_timeout(gpointer data)
{
    ...
    g_file_query_info_async(
        poll_monitor->file,
        G_FILE_ATTRIBUTE_ETAG_VALUE "," G_FILE_ATTRIBUTE_STANDARD_SIZE, 0, 0,
        NULL, got_new_info, g_object_ref(poll_monitor));
...
static void schedule_poll_timeout(GPollFileMonitor* poll_monitor)
{
    ...
    g_source_set_callback(poll_monitor->timeout, poll_file_timeout,
                          poll_monitor, NULL);
    g_source_attach(poll_monitor->timeout, g_main_context_get_thread_default());
}

static void got_initial_info(GObject* source_object, GAsyncResult* res,
                             gpointer user_data)
{
    GPollFileMonitor* poll_monitor = user_data;
    GFileInfo* info;
...
GFileMonitor* _g_poll_file_monitor_new(GFile* file)
{
    ...
    g_file_query_info_async(
        file, G_FILE_ATTRIBUTE_ETAG_VALUE "," G_FILE_ATTRIBUTE_STANDARD_SIZE, 0,
        0, NULL, got_initial_info, g_object_ref(poll_monitor));
