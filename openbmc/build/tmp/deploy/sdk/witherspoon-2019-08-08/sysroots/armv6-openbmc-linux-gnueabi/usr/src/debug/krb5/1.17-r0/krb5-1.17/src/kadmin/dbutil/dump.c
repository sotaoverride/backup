...
typedef krb5_error_code (*dump_func)(krb5_context context, krb5_db_entry* entry,
 ...
 */
krb5_error_code master_key_convert(krb5_context context,
                                   krb5_db_entry* db_entry)
{
    ...
    if (is_mkey)
    {
        return add_new_mkey(context, db_entry, &new_master_keyblock, new_mkvno);
    }
    ...
    for (i = 0; i < db_entry->n_key_data; i++)
    {
        ...
        retval =
            krb5_dbe_encrypt_key_data(context, &new_master_keyblock, key_ptr,
                                      &keysalt, kvno, &new_key_data);
        if (retval)
            return retval;
...
static FILE* create_ofile(char* ofile, char** tmpname)
{
    int fd = -1;
    FILE* f;
...
static void finish_ofile(char* ofile, char** tmpname)
{
...
static krb5_boolean prep_ok_file(krb5_context context, char* file_name,
                                 int* fd_out)
{
 ...
 * Update the "ok" file.
 */
static void update_ok_file(krb5_context context, int fd)
{
    write(fd, "", 1);
...
static int name_matches(char* name, struct dump_args* args)
{
#if HAVE_REGCOMP
    regex_t reg;
    regmatch_t rmatch;
...
static void dump_octets_or_minus1(FILE* fp, unsigned char* data, size_t len)
{
    if (len > 0)
    {
        for (; len > 0; len--)
 ...
 * support policies.
 */
static void dump_tl_data(FILE* ofile, krb5_tl_data* tlp,
                         krb5_boolean filter_kadm)
{
 ...
 * is false. */
static krb5_error_code k5beta7_common(krb5_context context,
                                      ...
                                      krb5_boolean omit_nra, krb5_boolean kadm)
{
    ...
    {
        fprintf(stderr,
                _("%s: tagged data list inconsistency for %s "
                  "(counted %d, stored %d)\n"),
                progname, name, counter + skip, (int)entry->n_tl_data);
        return EINVAL;
    }
    ...
    fprintf(fp, "%d\t%d\t%d\t%u\t%u\t%u\t%u\t%d", entry->attributes,
            ...
            (unsigned int)entry->expiration, (unsigned int)entry->pw_expiration,
...
static krb5_error_code dump_k5beta7_princ(krb5_context context,
                                          krb5_db_entry* entry,
                                          const char* name, FILE* fp,
                                          krb5_boolean verbose,
                                          krb5_boolean omit_nra)
...
static krb5_error_code dump_k5beta7_princ_withpolicy(krb5_context context,
                                                     krb5_db_entry* entry,
                                                     const char* name, FILE* fp,
                                                     krb5_boolean verbose,
                                                     krb5_boolean omit_nra)
...
static void dump_r1_11_policy(void* data, osa_policy_ent_t entry)
{
    struct dump_args* arg = data;
...
static void print_key_data(FILE* f, krb5_key_data* kd)
{
    int c;
...
static krb5_error_code dump_ov_princ(krb5_context context, krb5_db_entry* entry,
                                     const char* name, FILE* fp,
                                     krb5_boolean verbose,
                                     krb5_boolean omit_nra)
{
    ...
    for (x = 0; x < adb.old_key_len; x++)
    {
        ...
        for (y = 0; y < adb.old_keys[x].n_key_data; y++)
        {
            ...
            {
                fprintf(stderr,
                        _("Warning!  Multiple DES-CBC-CRC keys for "
        ...
        {
            fprintf(stderr,
                    _("Warning!  No DES-CBC-CRC key for principal %s, "
...
static krb5_error_code dump_iterator(void* ptr, krb5_db_entry* entry)
{
    krb5_error_code ret;
...
static inline void load_err(const char* fname, int lineno, const char* msg)
{
...
static int read_string(FILE* f, char* buf, int len, int* lp)
{
    int c, i;
...
static int read_octet_string(FILE* f, unsigned char* buf, int len)
{
    int c, i;
...
static void read_record_end(FILE* f, const char* fn, int lineno)
{
    int ch;
...
static int read_octets_or_minus1(FILE* fp, size_t len, unsigned char** out)
{
    int ival;
    unsigned char* buf;
 ...
 * failure. */
static int process_tl_data(const char* fname, FILE* filep, int lineno,
                           krb5_tl_data* tl_data)
{
    krb5_tl_data* tl;
    int nread, i1;
...
static int process_k5beta7_princ(krb5_context context, const char* fname,
                                 ...
                                 int* linenop)
{
    ...
    nread = fscanf(filep, "%d\t%d\t%d\t%u\t%u\t%d\t%d\t%d\t", &t1, &t2, &t3,
                   &u1, &u2, &u3, &u4, &u5);
    if (nread != 8)
    {
    ...
    for (i = 0; i < dbentry->n_key_data; i++)
    {
        ...
        for (j = 0; j < t1; j++)
        {
            ...
            if (nread != 2 || t4 < 0)
            {
                load_err(fname, *linenop, _("cannot read key type and length"));
                goto fail;
            }
...
static int process_k5beta7_policy(krb5_context context, const char* fname,
                                  FILE* filep, krb5_boolean verbose,
                                  int* linenop)
{
...
static int process_r1_8_policy(krb5_context context, const char* fname,
                               FILE* filep, krb5_boolean verbose, int* linenop)
{
    osa_policy_ent_rec rec;
    char namebuf[1024];
    unsigned int refcnt;
    ...
    nread = fscanf(filep, "%1023s\t%u\t%u\t%u\t%u\t%u\t%u\t%u\t%u\t%u",
                   ...
                   &rec.pw_min_length, &rec.pw_min_classes, &rec.pw_history_num,
                   &refcnt, &rec.pw_max_fail, &rec.pw_failcnt_interval,
...
static int process_r1_11_policy(krb5_context context, const char* fname,
                                FILE* filep, krb5_boolean verbose, int* linenop)
{
    osa_policy_ent_rec rec;
    ...
    if (c != '\n')
    {
        ...
        nread = fscanf(filep,
                       "%u\t%u\t%u\t" K5CONST_WIDTH_SCANF_STR(
                           KRB5_KDB_MAX_ALLOWED_KS_LEN) "\t%hd",
...
static int process_tagged(krb5_context context, const char* fname, FILE* filep,
...
static int process_k5beta7_record(krb5_context context, const char* fname,
                                  FILE* filep, krb5_boolean verbose,
                                  int* linenop)
{
...
static int process_ov_record(krb5_context context, const char* fname,
                             FILE* filep, krb5_boolean verbose, int* linenop)
{
    return process_tagged(context, fname, filep, verbose, linenop,
...
static int process_r1_8_record(krb5_context context, const char* fname,
                               FILE* filep, krb5_boolean verbose, int* linenop)
{
    return process_tagged(context, fname, filep, verbose, linenop,
...
static int process_r1_11_record(krb5_context context, const char* fname,
                                FILE* filep, krb5_boolean verbose, int* linenop)
{
    return process_tagged(context, fname, filep, verbose, linenop,
...
dump_version ov_version = {
    "OpenV*Secure V1.0", "OpenV*Secure V1.0\t", 1, 0, 0, dump_ov_princ,
    dump_k5beta7_policy, process_ov_record};
...
static int parse_iprop_header(char* buf, dump_version** dv, kdb_last_t* last)
{
    char head[128];
    int nread;
    uint32_t u[4];
 ...
 * in the ulog. */
static krb5_boolean current_dump_sno_in_ulog(krb5_context context,
                                             const char* ifile)
{
 ...
 *              [filename [principals...]]
 */
void dump_db(int argc, char** argv)
{
    FILE* f;
    ...
    if (mkey_convert)
    {
        ...
        if (new_mkey_file)
        {
            ...
            retval = krb5_db_fetch_mkey(
                util_context, master_princ, new_master_keyblock.enctype, FALSE,
                FALSE, new_mkey_file, &kt_kvno, NULL, &new_master_keyblock);
            if (retval)
            {
        ...
        else
        {
            ...
            retval = krb5_db_fetch_mkey(util_context, master_princ,
                                        new_master_keyblock.enctype, TRUE, TRUE,
                                        NULL, NULL, NULL, &new_master_keyblock);
            if (retval)
            {
                com_err(progname, retval, _("while reading new master key"));
                exit(1);
...
static int restore_dump(krb5_context context, char* dumpfile, FILE* f,
...
void load_db(int argc, char** argv)
{
    ...
    else
    {
        ...
        else if (strncmp(buf, ov_version.header, strlen(ov_version.header)) ==
                 0)
        {
            load = &ov_version;
        }
    ...
    {
        fprintf(stderr,
                _("%s: dump version %s can only be loaded with the "
                  "-update flag\n"),
                progname, load->name);
        goto error;
    }
    ...
    if (restore_dump(util_context, dumpfile ? dumpfile : _("standard input"), f,
                     verbose, load))
    {
...
cleanup:
    ...
    if (exit_status && temp_db_created)
    {
        ...
        if (ret != 0 && ret != KRB5_PLUGIN_OP_NOTSUPP)
        {
            com_err(progname, ret, _("while deleting bad database %s"), dbname);
        }
    }
