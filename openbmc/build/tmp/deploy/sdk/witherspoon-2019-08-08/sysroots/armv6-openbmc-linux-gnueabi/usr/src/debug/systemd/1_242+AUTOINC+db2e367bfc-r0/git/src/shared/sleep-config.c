...
int parse_sleep_config(const char* verb, bool* ret_allow, char*** ret_modes,
                       char*** ret_states, usec_t* ret_delay)
{
    int allow_suspend = -1, allow_hibernate = -1, allow_s2h = -1,
    ...
    const ConfigTableItem items[] = {
        ...
        {"Sleep", "AllowHibernation", config_parse_tristate, 0,
         &allow_hibernate},
        {"Sleep", "AllowSuspendThenHibernate", config_parse_tristate, 0,
         &allow_s2h},
        {"Sleep", "AllowHybridSleep", config_parse_tristate, 0,
         &allow_hybrid_sleep},
    ...
    (void)config_parse_many_nulstr(
        PKGSYSCONFDIR "/sleep.conf", CONF_PATHS_NULSTR("systemd/sleep.conf.d"),
        "Sleep\0", config_item_table_lookup, items, CONFIG_PARSE_WARN, NULL);
...
int can_sleep_disk(char** types)
{
    ...
    STRV_FOREACH(type, types)
    {
        ...
        FOREACH_WORD_SEPARATOR(word, l, p, WHITESPACE, state)
        {
            ...
            if (l == k + 2 && word[0] == '[' &&
                memcmp(word + 1, *type, l - 2) == 0 && word[l - 1] == ']')
                return true;
        }
    }
...
int find_hibernate_location(char** device, char** type, size_t* size,
                            size_t* used)
{
    ...
    for (i = 1;; i++)
    {
        ...
        else if (streq(type_field, "partition"))
        {
            ...
            {
                log_debug("Ignoring compressed RAM swap device '%s'.",
                          dev_field);
                continue;
            }
        }
...
static bool enough_swap_for_hibernation(void)
{
    ...
    {
        log_debug_errno(
            r, "Failed to retrieve Active(anon) from /proc/meminfo: %m");
        return false;
    }
    ...
    {
        log_debug_errno(
            r, "Failed to parse Active(anon) from /proc/meminfo: %s: %m",
            active);
        return false;
    }

    r = act <= (size - used) * HIBERNATION_SWAP_THRESHOLD;
    log_debug("%s swap for hibernation, Active(anon)=%llu kB, size=%zu kB, "
              "used=%zu kB, threshold=%.2g%%",
              r ? "Enough" : "Not enough", act, size, used,
...
int read_fiemap(int fd, struct fiemap** ret)
{
    ...
    const size_t n_extra =
        DIV_ROUND_UP(sizeof(struct fiemap), sizeof(struct fiemap_extent));
    ...
    while (fiemap_start < fiemap_length)
    {
        *fiemap = (struct fiemap){
        ...
        /* Resize fiemap to allow us to read in the extents, result fiemap has
         * to hold all
         * the extents for the whole file. Add space for the initial struct
         * fiemap. */
        if (!greedy_realloc0((void**)&fiemap, &fiemap_allocated,
                             n_extra + fiemap->fm_mapped_extents,
        ...
        if (!greedy_realloc((void**)&result_fiemap, &result_fiemap_allocated,
                            n_extra + result_extents +
                                fiemap->fm_mapped_extents,
                            sizeof(struct fiemap_extent)))
            return -ENOMEM;

        memcpy(result_fiemap->fm_extents + result_extents, fiemap->fm_extents,
...
static bool can_s2h(void)
{
    ...
    FOREACH_STRING(p, "suspend", "hibernate")
    {
        ...
        if (r < 0)
            return log_debug_errno(r, "Failed to check if %s is possible: %m",
                                   p);
    }
