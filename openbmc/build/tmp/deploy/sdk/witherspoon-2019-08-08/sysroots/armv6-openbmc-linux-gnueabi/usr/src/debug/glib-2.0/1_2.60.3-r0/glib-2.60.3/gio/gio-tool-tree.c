...
static void do_tree(GFile* f, int level, guint64 pattern)
{
    ...
    if (info != NULL)
    {
        if (g_file_info_get_attribute_uint32(
                info, G_FILE_ATTRIBUTE_STANDARD_TYPE) == G_FILE_TYPE_MOUNTABLE)
        {
            /* don't process mountables; we avoid these by getting the
             * target_uri below */
            g_object_unref(info);
            return;
        }
    ...
    if (enumerator != NULL)
    {
        ...
        for (l = info_list; l != NULL; l = l->next)
        {
            ...
            type = g_file_info_get_attribute_uint32(
                info, G_FILE_ATTRIBUTE_STANDARD_TYPE);
            if (name != NULL)
            {
...
int handle_tree(int argc, char* argv[], gboolean do_help)
{
    ...
    g_option_context_set_summary(
        context, _("List contents of directories in a tree-like format."));
