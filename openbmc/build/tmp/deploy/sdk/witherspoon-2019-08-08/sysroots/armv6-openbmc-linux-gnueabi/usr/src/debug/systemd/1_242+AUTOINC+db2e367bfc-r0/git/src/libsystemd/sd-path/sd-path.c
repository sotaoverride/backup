...
static int from_environment(const char* envname, const char* fallback,
                            const char** ret)
{
    assert(ret);
...
static int from_home_dir(const char* envname, const char* suffix, char** buffer,
                         const char** ret)
{
...
static int from_user_dir(const char* field, char** buffer, const char** ret)
{
    ...
    for (;;)
    {
        ...
        *e = 0;
        ...
        /* Three syntaxes permitted: relative to $HOME, $HOME itself, and
         * absolute path */
...
static int get_path(uint64_t type, char** buffer, const char** ret)
{
    ...
    switch (type)
    {
        ...
        case SD_PATH_USER_LIBRARY_ARCH:
            return from_home_dir(NULL, ".local/lib/" LIB_ARCH_TUPLE, buffer,
                                 ret);
...
_public_ int sd_path_home(uint64_t type, const char* suffix, char** path)
{
    ...
    if (IN_SET(type, SD_PATH_SEARCH_BINARIES, SD_PATH_SEARCH_BINARIES_DEFAULT,
...
static int search_from_environment(char*** list, const char* env_home,
...
static int get_search(uint64_t type, char*** list)
{
    ...
    switch (type)
    {
        ...
        case SD_PATH_SEARCH_BINARIES:
            return search_from_environment(list, NULL, ".local/bin", "PATH",
                                           true, ARRAY_SBIN_BIN("/usr/local/"),
                                           ARRAY_SBIN_BIN("/usr/"),
#if HAVE_SPLIT_USR
                                           ARRAY_SBIN_BIN("/"),
#endif
        ...
        case SD_PATH_SEARCH_LIBRARY_PRIVATE:
            return search_from_environment(list, NULL, ".local/lib", NULL,
                                           false, "/usr/local/lib", "/usr/lib",
#if HAVE_SPLIT_USR
                                           "/lib",
#endif
                                           NULL);
        ...
        case SD_PATH_SEARCH_LIBRARY_ARCH:
            return search_from_environment(list, NULL,
                                           ".local/lib/" LIB_ARCH_TUPLE,
                                           "LD_LIBRARY_PATH", true, LIBDIR,
#if HAVE_SPLIT_USR
                                           ROOTLIBDIR,
#endif
                                           NULL);
        ...
        case SD_PATH_SEARCH_SHARED:
            return search_from_environment(
                list, "XDG_DATA_HOME", ".local/share", "XDG_DATA_DIRS", false,
                "/usr/local/share", "/usr/share", NULL);

        case SD_PATH_SEARCH_CONFIGURATION_FACTORY:
            return search_from_environment(list, NULL, NULL, NULL, false,
                                           "/usr/local/share/factory/etc",
                                           "/usr/share/factory/etc", NULL);

        case SD_PATH_SEARCH_STATE_FACTORY:
            return search_from_environment(list, NULL, NULL, NULL, false,
                                           "/usr/local/share/factory/var",
                                           "/usr/share/factory/var", NULL);

        case SD_PATH_SEARCH_CONFIGURATION:
            return search_from_environment(list, "XDG_CONFIG_HOME", ".config",
                                           "XDG_CONFIG_DIRS", false, "/etc",
                                           NULL);
