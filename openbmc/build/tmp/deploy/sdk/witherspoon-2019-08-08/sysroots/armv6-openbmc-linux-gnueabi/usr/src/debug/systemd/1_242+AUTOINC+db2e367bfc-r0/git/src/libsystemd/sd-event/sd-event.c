...
static const char* const
    event_source_type_table[_SOURCE_EVENT_SOURCE_TYPE_MAX] = {
        [SOURCE_IO] = "io",
...
struct sd_event
{
    ...
    /* A list of inode structures that still have an fd open, that we need to
     * close before the next loop iteration */
    LIST_HEAD(struct inode_data, inode_data_to_close);

    /* A list of inotify objects that already have events buffered which aren't
     * processed yet */
...
_public_ int sd_event_new(sd_event** ret)
{
    ...
    *e = (sd_event){
    ...
    {
        log_debug(
            "Event loop profiling enabled. Logarithmic histogram of event loop "
            "iterations in the range 2^0 ... 2^63 us will be logged every 5s.");
...
static void source_io_unregister(sd_event_source* s)
{
    ...
    if (r < 0)
        log_debug_errno(
            errno, "Failed to remove source %s (type %s) from epoll: %m",
...
static int event_make_signal_data(sd_event* e, int sig,
...
static void event_unmask_signal_data(sd_event* e, struct signal_data* d,
                                     int sig)
{
    assert(e);
    assert(d);
...
static void event_gc_signal_data(sd_event* e, const int64_t* priority, int sig)
{
     ...
     * 2) the one that belongs to the priority of the event source of the signal
     * (if there is one) 3) the 0 priority (to cover the SIGCHLD case)
     *
    ...
    switch (s->type)
    {
        ...
        case SOURCE_INOTIFY:
        {
            ...
            if (inode_data)
            {
                ...
                LIST_REMOVE(inotify.by_inode_data, inode_data->event_sources,
                            s);
                ...
                /* Note that we don't reduce the inotify mask for the watch
                 * descriptor here if the inode is continued to being watched.
                 * That's because inotify doesn't really have an API for that:
                 * we can only change watch masks with access to the original
                 * inode either by fd or by path. But paths aren't stable, and
                 * keeping an O_PATH fd open all the time would mean wasting an
                 * fd continuously and keeping the mount busy which we can't
                 * really do. We could reconstruct the original inode from
                 * /proc/self/fdinfo/$INOTIFY_FD (as all watch descriptors are
                 * listed there), but given the need for open_by_handle_at()
                 * which is privileged and not universally available this would
                 * be quite an incomplete solution. Hence we go the other way,
                 * leave the mask set, even if it is not minimized now, and
                 * ignore all events we aren't interested in anymore after
                 * reception. Yes, this sucks, but … Linux … */
...
static sd_event_source* source_new(sd_event* e, bool floating,
...
_public_ int sd_event_add_io(sd_event* e, sd_event_source** ret, int fd,
                             ...
                             void* userdata)
{
    ...
    assert_return(!(events & ~(EPOLLIN | EPOLLOUT | EPOLLRDHUP | EPOLLPRI |
...
static int event_setup_timer_fd(sd_event* e, struct clock_data* d,
                                clockid_t clock)
{
...
_public_ int sd_event_add_time(sd_event* e, sd_event_source** ret,
                               ...
                               sd_event_time_handler_t callback, void* userdata)
{
    ...
    if (!clock_supported(
            clock)) /* Checks whether the kernel supports the clock */
        return -EOPNOTSUPP;

    type = clock_to_event_source_type(
        clock); /* checks whether sd-event supports this clock */
    if (type < 0)
        return -EOPNOTSUPP;
...
static int signal_exit_callback(sd_event_source* s,
...
_public_ int sd_event_add_defer(sd_event* e, sd_event_source** ret,
...
_public_ int sd_event_add_post(sd_event* e, sd_event_source** ret,
                               sd_event_handler_t callback, void* userdata)
{
...
_public_ int sd_event_add_exit(sd_event* e, sd_event_source** ret,
                               sd_event_handler_t callback, void* userdata)
{
...
static void event_free_inotify_data(sd_event* e, struct inotify_data* d)
{
    ...
    if (d->fd >= 0)
    {
        if (epoll_ctl(e->epoll_fd, EPOLL_CTL_DEL, d->fd, NULL) < 0)
            log_debug_errno(
                errno, "Failed to remove inotify fd from epoll, ignoring: %m");
...
static int event_make_inotify_data(sd_event* e, int64_t priority,
                                   struct inotify_data** ret)
{
    ...
    *d = (struct inotify_data){
    ...
    if (epoll_ctl(e->epoll_fd, EPOLL_CTL_ADD, d->fd, &ev) < 0)
    {
        r = -errno;
        d->fd = safe_close(
            d->fd); /* let's close this ourselves, as event_free_inotify_data()
                     * would otherwise remove the fd from the epoll first, which
                     * we don't want as we couldn't add it in the first place.
                     */
...
static int inode_data_compare(const struct inode_data* x,
                              const struct inode_data* y)
{
    int r;
...
static void inode_data_hash_func(const struct inode_data* d,
...
DEFINE_PRIVATE_HASH_OPS(inode_data_hash_ops, struct inode_data,
                        inode_data_hash_func, inode_data_compare);
...
static void event_free_inode_data(sd_event* e, struct inode_data* d)
{
    ...
    if (d->inotify_data)
    {
        ...
        if (d->wd >= 0)
        {
            ...
            {
                /* So here's a problem. At the time this runs the watch
                 * descriptor might already be invalidated, because an
                 * IN_IGNORED event might be queued right the moment we enter
                 * the syscall. Hence, whenever we get EINVAL, ignore it
                 * entirely, since it's a very likely case to happen. */

                if (inotify_rm_watch(d->inotify_data->fd, d->wd) < 0 &&
                    errno != EINVAL)
                    log_debug_errno(errno,
                                    "Failed to remove watch descriptor %i from "
                                    "inotify, ignoring: %m",
                                    d->wd);
            }

            assert_se(hashmap_remove(d->inotify_data->wd, INT_TO_PTR(d->wd)) ==
                      d);
        }
...
static int event_make_inode_data(sd_event* e, struct inotify_data* inotify_data,
                                 dev_t dev, ino_t ino, struct inode_data** ret)
{
...
static uint32_t inode_data_determine_mask(struct inode_data* d)
{
    ...
    /* Combines the watch masks of all event sources watching this inode. We
     * generally just OR them together, but the IN_EXCL_UNLINK flag is ANDed
     * instead.
     *
     * Note that we add all sources to the mask here, regardless whether
     * enabled, disabled or oneshot. That's because we cannot change the mask
     * anymore after the event source was created once, since the kernel has no
     * API for that. Hence we need to subscribe to the maximum mask we ever
     * might be interested in, and suppress events we don't care for
     * client-side. */
    ...
    return (combined &
            ~(IN_ONESHOT | IN_DONT_FOLLOW | IN_ONLYDIR | IN_EXCL_UNLINK)) |
...
static int inode_data_realize_watch(sd_event* e, struct inode_data* d)
{
    ...
    else if (d->wd != wd)
    {
        ...
        log_debug("Weird, the watch descriptor we already knew for this inode "
                  "changed?");
...
_public_ int sd_event_add_inotify(sd_event* e, sd_event_source** ret,
                                  ...
                                  void* userdata)
{
    ...
    /* Refuse IN_MASK_ADD since we coalesce watches on the same inode, and hence
     * really don't want to merge masks. Or in other words, this whole code
     * exists only to manage IN_MASK_ADD type operations for you, hence
     * the user can't use them for us. */
    if (mask & IN_MASK_ADD)
        return -EINVAL;

    fd = open(path, O_PATH | O_CLOEXEC | (mask & IN_ONLYDIR ? O_DIRECTORY : 0) |
    ...
    /* Allocate an inotify object for this priority, and an inode object within
     * it */
    r = event_make_inotify_data(e, SD_EVENT_PRIORITY_NORMAL, &inotify_data);
    if (r < 0)
        return r;

    r = event_make_inode_data(e, inotify_data, st.st_dev, st.st_ino,
                              &inode_data);
    if (r < 0)
    {
        event_free_inotify_data(e, inotify_data);
    ...
    /* Keep the O_PATH fd around until the first iteration of the loop, so that
     * we can still change the priority of the event source, until then, for
     * which we need the original inode. */
    if (inode_data->fd < 0)
    {
...
DEFINE_PUBLIC_TRIVIAL_REF_UNREF_FUNC(sd_event_source, sd_event_source,
                                     event_source_free);

_public_ int sd_event_source_set_description(sd_event_source* s,
...
_public_ int sd_event_source_get_description(sd_event_source* s,
                                             const char** description)
{
    assert_return(s, -EINVAL);
...
_public_ int sd_event_source_get_io_revents(sd_event_source* s,
                                            uint32_t* revents)
{
    assert_return(s, -EINVAL);
...
_public_ int sd_event_source_set_priority(sd_event_source* s, int64_t priority)
{
    ...
    if (s->type == SOURCE_INOTIFY)
    {
        ...
        /* We need the original fd to change the priority. If we don't have it
         * we can't change the priority, anymore. Note that we close any fds
         * when entering the next event loop iteration, i.e. for inotify
         * events we allow priority changes only until the first following
         * iteration. */
        ...
        r = event_make_inode_data(s->event, new_inotify_data,
                                  old_inode_data->dev, old_inode_data->ino,
                                  &new_inode_data);
        if (r < 0)
            goto fail;
        rm_inode = r > 0;
        ...
        {
            /* Duplicate the fd for the new inode object if we don't have any
             * yet */
            ...
            LIST_PREPEND(to_close, s->event->inode_data_to_close,
                         new_inode_data);
        }
        ...
        {
            /* Move it back */
            LIST_REMOVE(inotify.by_inode_data, new_inode_data->event_sources,
                        s);
            LIST_PREPEND(inotify.by_inode_data, old_inode_data->event_sources,
                         s);
...
_public_ int sd_event_source_set_enabled(sd_event_source* s, int m)
{
    ...
    assert_return(s, -EINVAL);
    assert_return(IN_SET(m, SD_EVENT_OFF, SD_EVENT_ON, SD_EVENT_ONESHOT),
                  -EINVAL);
    assert_return(!event_pid_changed(s->event), -ECHILD);
...
_public_ int sd_event_source_get_time_accuracy(sd_event_source* s,
                                               uint64_t* usec)
{
    assert_return(s, -EINVAL);
    assert_return(usec, -EINVAL);
...
_public_ int sd_event_source_set_time_accuracy(sd_event_source* s,
                                               uint64_t usec)
{
    struct clock_data* d;
    int r;
...
_public_ int sd_event_source_get_time_clock(sd_event_source* s,
                                            clockid_t* clock)
{
    assert_return(s, -EINVAL);
...
_public_ int sd_event_source_get_inotify_mask(sd_event_source* s,
                                              uint32_t* mask)
{
    assert_return(s, -EINVAL);
...
_public_ int sd_event_source_set_prepare(sd_event_source* s,
...
static int process_child(sd_event* e)
{
    ...
    */
    ...
    HASHMAP_FOREACH(s, e->child_sources, i)
    {
        ...
        {
            bool zombie = IN_SET(s->child.siginfo.si_code, CLD_EXITED,
...
static int event_inotify_data_read(sd_event* e, struct inotify_data* d,
                                   uint32_t revents)
{
    ssize_t n;
    ...
    /* If there's already an event source pending for this priority, don't read
     * another */
    if (d->n_pending > 0)
        return 0;
...
static void event_inotify_data_drop(sd_event* e, struct inotify_data* d,
                                    size_t sz)
{
    assert(e);
    assert(d);
    ...
    /* Move the rest to the buffer to the front, in order to get things properly
     * aligned again */
...
static int event_inotify_data_process(sd_event* e, struct inotify_data* d)
{
    ...
    while (d->buffer_filled > 0)
    {
        ...
        if (d->buffer.ev.mask & IN_Q_OVERFLOW)
        {
            ...
            /* The queue overran, let's pass this event to all event sources
             * connected to this inotify object */

            HASHMAP_FOREACH(inode_data, d->inodes, i)
            {
                sd_event_source* s;

                LIST_FOREACH(inotify.by_inode_data, s,
                             inode_data->event_sources)
                {
        ...
        else
        {
            ...
            /* Find the inode object for this watch descriptor. If IN_IGNORED is
             * set we also remove it from our watch descriptor table. */
            if (d->buffer.ev.mask & IN_IGNORED)
            {
                ...
                /* The watch descriptor was removed by the kernel, let's drop it
                 * here too */
                inode_data->wd = -1;
            }
            else
            ...
            /* Trigger all event sources that are interested in these events.
             * Also trigger all event sources if IN_IGNORED or IN_UNMOUNT is
             * set. */
        ...
        /* Something pending now? If so, let's finish, otherwise let's read
         * more. */
        if (d->n_pending > 0)
            return 1;
    }
...
static int source_dispatch(sd_event_source* s)
{
    ...
    /* Save the event source type, here, so that we still know it after the
     * event callback which might invalidate the event. */
    saved_type = s->type;
    ...
    switch (s->type)
    {
        ...
        case SOURCE_CHILD:
        {
            ...
            zombie = IN_SET(s->child.siginfo.si_code, CLD_EXITED, CLD_KILLED,
                            CLD_DUMPED);
            ...
            if (zombie)
                (void)waitid(P_PID, s->child.pid, &s->child.siginfo,
                             WNOHANG | WEXITED);
        ...
        case SOURCE_INOTIFY:
        {
            ...
            /* When no event is pending anymore on this inotify object, then
             * let's drop the event from the buffer. */
            if (d->n_pending == 0)
                event_inotify_data_drop(e, d, sz);
    ...
    if (r < 0)
        log_debug_errno(
            r, "Event source %s (type %s) returned error, disabling: %m",
...
static int event_prepare(sd_event* e)
{
    ...
    for (;;)
    {
        ...
        if (r < 0)
            log_debug_errno(r,
                            "Prepare callback of event source %s (type %s) "
                            "returned error, disabling: %m",
                            strna(s->description),
...
static void event_close_inode_data_fds(sd_event* e)
{
    ...
    /* Close the fds pointing to the inodes to watch now. We need to close them
     * as they might otherwise pin filesystems. But we can't close them
     * right-away as we need them as long as the user still wants to make
     * adjustments to the even source, such as changing the priority (which
     * requires us to remove and readd a watch for the inode). Hence, let's
     * close them when entering the first iteration after they were added, as a
     * compromise. */
...
_public_ int sd_event_wait(sd_event* e, uint64_t timeout)
{
    ...
    /* If we still have inotify data buffered, then query the other fds, but
     * don't wait on it */
    ...
    m = epoll_wait(e->epoll_fd, ev_queue, ev_queue_max,
                   timeout == (uint64_t)-1
                       ? -1
                       : (int)DIV_ROUND_UP(timeout, USEC_PER_MSEC));
    if (m < 0)
    {
        if (errno == EINTR)
        {
        ...
        else
        {
            ...
            switch (*t)
            {
                ...
                case WAKEUP_SIGNAL_DATA:
                    r = process_signal(e, ev_queue[i].data.ptr,
                                       ev_queue[i].events);
                    break;

                case WAKEUP_INOTIFY_DATA:
                    r = event_inotify_data_read(e, ev_queue[i].data.ptr,
                                                ev_queue[i].events);
                    break;
...
_public_ int sd_event_now(sd_event* e, clockid_t clock, uint64_t* usec)
{
    ...
    /* Generate a clean error in case CLOCK_BOOTTIME is not available. Note that
     * don't use clock_supported() here, for a reason: there are systems where
     * CLOCK_BOOTTIME is supported, but CLOCK_BOOTTIME_ALARM is not, but for
...
_public_ int sd_event_source_set_destroy_callback(sd_event_source* s,
...
_public_ int sd_event_source_get_destroy_callback(sd_event_source* s,
                                                  sd_event_destroy_t* ret)
{
    assert_return(s, -EINVAL);
