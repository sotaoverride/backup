...
netsnmp_feature_require(iquery) netsnmp_feature_require(table_tdata)
    ...
    netsnmp_feature_require(iquery_pdu_session) netsnmp_feature_require(
        check_vb_type_and_max_size) netsnmp_feature_require(check_vb_oid)
...
void init_mteTriggerTable(void)
{
    ...
    reg = netsnmp_create_handler_registration(
        "mteTriggerTable", mteTriggerTable_handler, mteTriggerTable_oid,
        mteTriggerTable_oid_len, HANDLER_CAN_RWRITE);
#else  /* !NETSNMP_NO_WRITE_SUPPORT */
    reg = netsnmp_create_handler_registration(
        "mteTriggerTable", mteTriggerTable_handler, mteTriggerTable_oid,
        mteTriggerTable_oid_len, HANDLER_CAN_RONLY);
#endif /* !NETSNMP_NO_WRITE_SUPPORT */
...
int mteTriggerTable_handler(netsnmp_mib_handler* handler,
                            ...
                            netsnmp_request_info* requests)
{
    ...
    DEBUGMSGTL(
        ("disman:event:mib", "Trigger Table handler (%d)\n", reqinfo->mode));
                    ...
                    case COLUMN_MTETRIGGERENTRYSTATUS:
                        switch (*request->requestvb->val.integer)
                        {
                            ...
                            case RS_CREATEANDWAIT:
                                ...
                                row = mteTrigger_createEntry(mteOwner, mteTName,
                                                             0);
                                if (!row)
                                {
