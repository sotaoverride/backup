 ...
 * @annotations: (array zero-terminated=1): A pointer to a %NULL-terminated
 * array of pointers to #GDBusAnnotationInfo structures or %NULL if there are no
 * annotations.
 *
 ...
 * @in_args: (array zero-terminated=1): A pointer to a %NULL-terminated array of
 * pointers to #GDBusArgInfo structures or %NULL if there are no in arguments.
 * @out_args: (array zero-terminated=1): A pointer to a %NULL-terminated array
 * of pointers to #GDBusArgInfo structures or %NULL if there are no out
 * arguments.
 * @annotations: (array zero-terminated=1): A pointer to a %NULL-terminated
 * array of pointers to #GDBusAnnotationInfo structures or %NULL if there are no
 * annotations.
 ...
 * @args: (array zero-terminated=1): A pointer to a %NULL-terminated array of
 * pointers to #GDBusArgInfo structures or %NULL if there are no arguments.
 * @annotations: (array zero-terminated=1): A pointer to a %NULL-terminated
 * array of pointers to #GDBusAnnotationInfo structures or %NULL if there are no
 * annotations.
 *
 ...
 * @methods: (array zero-terminated=1): A pointer to a %NULL-terminated array of
 * pointers to #GDBusMethodInfo structures or %NULL if there are no methods.
 * @signals: (array zero-terminated=1): A pointer to a %NULL-terminated array of
 * pointers to #GDBusSignalInfo structures or %NULL if there are no signals.
 * @properties: (array zero-terminated=1): A pointer to a %NULL-terminated array
 * of pointers to #GDBusPropertyInfo structures or %NULL if there are no
 * properties.
 * @annotations: (array zero-terminated=1): A pointer to a %NULL-terminated
 * array of pointers to #GDBusAnnotationInfo structures or %NULL if there are no
 * annotations.
 ...
 * GDBusNodeInfo:
 * @ref_count: The reference count or -1 if statically allocated.
 * @path: The path of the node or %NULL if omitted. Note that this may be a
 * relative path. See the D-Bus specification for more details.
 * @interfaces: (array zero-terminated=1): A pointer to a %NULL-terminated array
 * of pointers to #GDBusInterfaceInfo structures or %NULL if there are no
 * interfaces.
 * @nodes: (array zero-terminated=1): A pointer to a %NULL-terminated array of
 * pointers to #GDBusNodeInfo structures or %NULL if there are no nodes.
 * @annotations: (array zero-terminated=1): A pointer to a %NULL-terminated
 * array of pointers to #GDBusAnnotationInfo structures or %NULL if there are no
 * annotations.
 *
