                               ...
                               va_list args, unsigned int mode_flags)
{
    ...
    assert(
        ...
        (new_f.ofile.file.file._IO_write_base + obstack_object_size(obstack)));
    obstack_blank_fast(obstack, room);

    new_f.ofile.obstack = obstack;

    result =
        __vfprintf_internal(&new_f.ofile.file.file, format, args, mode_flags);
...
ldbl_weak_alias(__obstack_vprintf, obstack_vprintf)
    ...
    int __obstack_printf(struct obstack* obstack, const char* format, ...)
{
    int result;
    va_list ap;
    va_start(ap, format);
