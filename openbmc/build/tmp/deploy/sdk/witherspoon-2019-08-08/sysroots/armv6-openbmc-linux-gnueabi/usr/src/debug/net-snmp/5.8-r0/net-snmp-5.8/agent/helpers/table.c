    ...
    netsnmp_feature_child_of(table_all, mib_helpers)
        ...
        netsnmp_feature_child_of(table_build_result, table_all)
            netsnmp_feature_child_of(table_get_or_create_row_stash, table_all)
                netsnmp_feature_child_of(registration_owns_table_info,
                                         table_all)
                    netsnmp_feature_child_of(table_sparse, table_all)

                        static void table_helper_cleanup(
                            netsnmp_agent_request_info* reqinfo,
                            netsnmp_request_info* request, int status);
static void table_data_free_func(void* data);
static int sparse_table_helper_handler(netsnmp_mib_handler* handler,
...
{
    return (netsnmp_table_request_info*)netsnmp_request_get_list_data(
        request, TABLE_HANDLER_NAME);
}
...
{
    return (netsnmp_table_registration_info*)netsnmp_find_handler_data_by_name(
        reginfo, TABLE_HANDLER_NAME);
}
                         ...
                         netsnmp_request_info* requests)
{
    ...
    DEBUGIF("helper:table:req")
    {
        ...
        DEBUGMSGOID(
            ("helper:table:req", reginfo->rootoid, reginfo->rootoid_len));
    ...
    else
    {
        ...
        for (request = requests; request; request = request->next)
        {
            ...
            if ((reqinfo->mode == MODE_GET) && (var->type != ASN_NULL))
            {
                 ...
                 */
                DEBUGMSGTL(
                    ("helper:table", "  GET var type is not ASN_NULL\n"));
            ...
            if (reqinfo->mode == MODE_SET_RESERVE1)
            {
                DEBUGIF("helper:table:set")
                {
                    ...
                    DEBUGMSGOID(
                        ("helper:table:set", var->name, var->name_length));
                    out_len = 0;
                    if (sprint_realloc_by_type(&buf, &buf_len, &out_len, 1, var,
                                               NULL, NULL, NULL))
                    {
            ...
            if (NULL == tbl_req_info)
            {
                ...
                {
                    table_helper_cleanup(reqinfo, request, SNMP_ERR_GENERR);
                    continue;
                }
                ...
                netsnmp_request_add_list_data(
                    request, netsnmp_create_data_list(TABLE_HANDLER_NAME,
                                                      (void*)tbl_req_info,
            ...
            {
                DEBUGMSGTL(
                    ("helper:table", "  using existing tbl_req_info\n "));
            }
            ...
            if (var->name_length > oid_column_pos)
            {
                ...
                {
                    tbl_req_info->colnum = netsnmp_closest_column(
                        var->name[oid_column_pos], tbl_info->valid_columns);
                    DEBUGMSGTL(("helper:table:col",
                                "    closest column is %d\n",
                                tbl_req_info->colnum));
                    /*
                    ...
                    if (tbl_req_info->colnum != var->name[oid_column_pos])
                    {
                         ...
                         */
                        var->name_length =
                            oid_column_pos + 1; /* pos is 0 based */
                    }
                }
                /*
                 * var->name_length may have changed - check again
                 */
                if ((int)var->name_length <= oid_index_pos)
                { /* pos is 0 based */
                    DEBUGMSGTL(
            ...
            {
                /*
                 * oid is NOT long enough to contain column or index info, so
                 * start at the minimum column. Set index oid len to 0 because
                 * we don't have any index info in the OID.
                 */
            ...
            /*
             * set up tmp_len to be the number of OIDs we have beyond the
             * column; these should be the index(s) for the table. If the
             * index_oid_len is 0, set tmp_len to -1 so that when we try to
             * parse the index below, we just zero fill everything.
             */
                 ...
                 ++tmp_idx, vb = vb->next_variable)
            {
                ...
                if (parse_one_oid_index(&tmp_name, &parsed_oid_len, vb, 1) !=
                    SNMPERR_SUCCESS)
                {
                    incomplete = 1;
            ...
            DEBUGIF("helper:table:results")
            {
                ...
                DEBUGMSGTL(("helper:table:results", "  column: %d, indexes: %d",
                            tbl_req_info->colnum,
                            tbl_req_info->number_indexes));
                ...
                {
                    out_len = 0;
                    if (sprint_realloc_by_type(&buf, &buf_len, &out_len, 1, vb,
                                               NULL, NULL, NULL))
                    {
                        DEBUGMSG(("helper:table:results",
                                  "   index: type=%d(%02x), value=%s", vb->type,
                                  vb->type, buf));
                    }
                    else
                    {
        ...
        (NULL != handler->next))
    {
            ...
            !(handler->flags & MIB_HANDLER_CUSTOM1))
        {
            snmp_log(LOG_WARNING,
                     "handler (%s) registered after sparse table "
        ...
        for (request = requests; request; request = request->next)
        {
                ...
                request->requestvb->type == SNMP_ENDOFMIBVIEW)
            {
                ...
                if (0 != table_info->colnum)
                {
                    ...
                    snmp_set_var_objid(request->requestvb, coloid,
...
int netsnmp_update_variable_list_from_index(netsnmp_table_request_info* tri)
{
    ...
    return parse_oid_indexes(tri->index_oid, tri->index_oid_len, tri->indexes);
}
 ...
 * returns 0 if not.
 */
int netsnmp_check_getnext_reply(netsnmp_request_info* request, oid* prefix,
                                size_t prefix_len,
                                ...
                                netsnmp_variable_list** outvar)
{
    ...
    build_oid_noalloc(myname, MAX_OID_LEN, &myname_len, prefix, prefix_len,
                      newvar);
    /*
     * is the build of the new indexes less than our current result
     */
    if ((!(*outvar) ||
         snmp_oid_compare(myname + prefix_len, myname_len - prefix_len,
                          (*outvar)->name + prefix_len,
                          (*outvar)->name_length - prefix_len) < 0))
    {
        /*
         * and greater than the requested oid
         */
        if (snmp_oid_compare(myname, myname_len, request->requestvb->name,
 ...
 * internal routines
 */
void table_data_free_func(void* data)
{
...
static void _row_stash_data_list_free(void* ptr)
{
                                          ...
                                          const u_char* storage_name)
{
    ...
    stashp = (netsnmp_oid_stash_node**)netsnmp_agent_get_list_data(
        reqinfo, (const char*)storage_name);
    ...
    if (!stashp)
    {
        ...
        netsnmp_agent_add_list_data(
            reqinfo, netsnmp_create_data_list((const char*)storage_name, stashp,
