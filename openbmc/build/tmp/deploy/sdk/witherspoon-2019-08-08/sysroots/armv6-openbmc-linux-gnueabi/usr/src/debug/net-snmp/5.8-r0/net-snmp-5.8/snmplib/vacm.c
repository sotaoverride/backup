 ...
 */
void vacm_save_view(struct vacm_viewEntry* view, const char* token,
                    const char* type)
{
    char line[4096];
    char* cptr;
    ...
    cptr = read_config_save_octet_string(cptr, (u_char*)view->viewName + 1,
                                         view->viewName[0]);
    *cptr++ = ' ';
    cptr = read_config_save_objid(cptr, view->viewSubtree + 1,
...
void vacm_parse_config_view(const char* token, const char* line)
{
    ...
    line = read_config_read_objid_const(line, (oid**)&viewSubtree,
                                        &view.viewSubtreeLen);

    vptr = vacm_createViewEntry(view.viewName, view.viewSubtree,
                                view.viewSubtreeLen);
    if (!vptr)
    {
        return;
    ...
    line = read_config_read_octet_string(line, &viewMask, &vptr->viewMaskLen);
}

/*
 * vacm_save_access(): saves an access entry to the persistent cache
 */
void vacm_save_access(struct vacm_accessEntry* access_entry, const char* token,
                      const char* type)
{
    char line[4096];
    char* cptr;
    ...
    snprintf(line, sizeof(line), "%s%s %d %d %d %d %d ", token, "Access",
             access_entry->status, access_entry->storageType,
             access_entry->securityModel, access_entry->securityLevel,
             access_entry->contextMatch);
    line[sizeof(line) - 1] = 0;
    cptr = &line[strlen(line)]; /* the NULL */
    ...
    *cptr++ = ' ';
    cptr = read_config_save_octet_string(
        cptr, (u_char*)access_entry->contextPrefix + 1,
        access_entry->contextPrefix[0] + 1);

    *cptr++ = ' ';
    cptr = read_config_save_octet_string(
        cptr, (u_char*)access_entry->views[VACM_VIEW_READ],
        strlen(access_entry->views[VACM_VIEW_READ]) + 1);
    *cptr++ = ' ';
    cptr = read_config_save_octet_string(
        cptr, (u_char*)access_entry->views[VACM_VIEW_WRITE],
        strlen(access_entry->views[VACM_VIEW_WRITE]) + 1);
    *cptr++ = ' ';
    cptr = read_config_save_octet_string(
        cptr, (u_char*)access_entry->views[VACM_VIEW_NOTIFY],
                           ...
                           const char* token, const char* type, int authtype)
{
    ...
    memset(line, 0, sizeof(line));
    snprintf(line, sizeof(line), "%s%s %d %d %d %d %d ", token, "AuthAccess",
             access_entry->status, access_entry->storageType,
             access_entry->securityModel, access_entry->securityLevel,
             access_entry->contextMatch);
    line[sizeof(line) - 1] = 0;
    ...
    *cptr++ = ' ';
    cptr = read_config_save_octet_string(
        cptr, (u_char*)access_entry->views[authtype],
        strlen(access_entry->views[authtype]) + 1);

    read_config_store(type, line);
}

char* _vacm_parse_config_access_common(struct vacm_accessEntry** aptr,
                                       const char* line)
{
    ...
    *aptr = vacm_getAccessEntry(access.groupName, access.contextPrefix,
                                access.securityModel, access.securityLevel);
    if (!*aptr)
        *aptr =
            vacm_createAccessEntry(access.groupName, access.contextPrefix,
                                   access.securityModel, access.securityLevel);
    if (!*aptr)
        return NULL;
...
void vacm_parse_config_access(const char* token, const char* line)
{
    struct vacm_accessEntry* aptr;
...
void vacm_parse_config_auth_access(const char* token, const char* line)
{
    struct vacm_accessEntry* aptr;
    int authtype;
    char* view;
 ...
 */
void vacm_save_group(struct vacm_groupEntry* group_entry, const char* token,
                     const char* type)
{
    char line[4096];
    char* cptr;
    ...
    snprintf(line, sizeof(line), "%s%s %d %d %d ", token, "Group",
             group_entry->status, group_entry->storageType,
             group_entry->securityModel);
    line[sizeof(line) - 1] = 0;
    cptr = &line[strlen(line)]; /* the NULL */
...
void vacm_parse_config_group(const char* token, const char* line)
{
...
struct vacm_viewEntry* netsnmp_view_get(struct vacm_viewEntry* head,
                                        const char* viewName, oid* viewSubtree,
                                        size_t viewSubtreeLen, int mode)
{
    ...
    for (vp = head; vp; vp = vp->next)
    {
            ...
            viewSubtreeLen >= (vp->viewSubtreeLen - 1))
        {
            ...
            for (oidpos = 0; found && oidpos < vp->viewSubtreeLen - 1; oidpos++)
            {
                if (mode == VACM_MODE_IGNORE_MASK ||
                    (VIEW_MASK(vp, maskpos, mask) != 0))
                {
 ...
 */
int netsnmp_view_exists(struct vacm_viewEntry* head, const char* viewName)
{
 ...
 *                         portions.
 */
int netsnmp_view_subtree_check(struct vacm_viewEntry* head,
                               const char* viewName, oid* viewSubtree,
    ...
    for (vp = head; vp; vp = vp->next)
    {
        if (!memcmp(view, vp->viewName, glen + 1))
        {
            ...
            if (viewSubtreeLen >= (vp->viewSubtreeLen - 1))
            {
                 ...
                 * check the mask
                 */
                for (oidpos = 0; found && oidpos < vp->viewSubtreeLen - 1;
                     oidpos++)
                {
            ...
            else
            {
                 ...
                 */
                for (oidpos = 0; found && oidpos < viewSubtreeLen; oidpos++)
                {
                    if (VIEW_MASK(vp, maskpos, mask) != 0)
                    {
                        if (viewSubtree[oidpos] != vp->viewSubtree[oidpos + 1])
...
struct vacm_viewEntry* netsnmp_view_create(struct vacm_viewEntry** head,
                                           const char* viewName,
                                           oid* viewSubtree,
                                           size_t viewSubtreeLen)
{
...
void netsnmp_view_destroy(struct vacm_viewEntry** head, const char* viewName,
...
void netsnmp_view_clear(struct vacm_viewEntry** head)
{
    struct vacm_viewEntry* vp;
    while ((vp = (*head)))
    {
...
struct vacm_groupEntry* vacm_getGroupEntry(int securityModel,
...
struct vacm_groupEntry* vacm_createGroupEntry(int securityModel,
                                              const char* securityName)
{
    ...
    if (gp == NULL)
        return NULL;
    gp->reserved =
        (struct vacm_groupEntry*)calloc(1, sizeof(struct vacm_groupEntry));
    if (gp->reserved == NULL)
    {
        free(gp);
        return NULL;
    ...
    while (lg)
    {
        if (lg->securityModel > securityModel)
            break;
        if (lg->securityModel == securityModel &&
            (cmp = memcmp(lg->securityName, gp->securityName, glen + 1)) > 0)
            break;
        /*
...
void vacm_destroyGroupEntry(int securityModel, const char* securityName)
{
...
struct vacm_accessEntry* _vacm_choose_best(struct vacm_accessEntry* current,
...
struct vacm_accessEntry* vacm_getAccessEntry(const char* groupName,
...
struct vacm_accessEntry* vacm_createAccessEntry(const char* groupName,
                                                const char* contextPrefix,
                                                int securityModel,
                                                int securityLevel)
{
    ...
    if (vp == NULL)
        return NULL;
    vp->reserved =
        (struct vacm_accessEntry*)calloc(1, sizeof(struct vacm_accessEntry));
    if (vp->reserved == NULL)
    {
        free(vp);
        return NULL;
...
void vacm_destroyAccessEntry(const char* groupName, const char* contextPrefix,
 ...
 */
struct vacm_viewEntry* vacm_getViewEntry(const char* viewName, oid* viewSubtree,
                                         size_t viewSubtreeLen, int mode)
{
...
int vacm_checkSubtree(const char* viewName, oid* viewSubtree,
                      size_t viewSubtreeLen)
{
...
struct vacm_viewEntry* vacm_createViewEntry(const char* viewName,
                                            oid* viewSubtree,
                                            size_t viewSubtreeLen)
{
