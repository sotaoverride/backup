...
#define LRZSZ_ATTRIB_REGPARM(n) __attribute__((__regparm__(n)))
#endif
#endif /* __GNUC__ */
...
#define READLINE_PF(timeout)                                                   \
    (--readline_left >= 0 ? (*readline_ptr++ & 0377)                           \
                          : readline_internal(timeout))

int readline_internal __P((unsigned int timeout));
void readline_purge __P((void));
void readline_setup __P((int fd, size_t readnum, size_t buffer_size))
    LRZSZ_ATTRIB_SECTION(lrzsz_rare);
...
#endif
#define VPRINTF(level, format_args)                                            \
    do                                                                         \
    {                                                                          \
        if ((Verbose) >= (level))                                              \
            vstringf format_args;                                              \
    } while (0)
