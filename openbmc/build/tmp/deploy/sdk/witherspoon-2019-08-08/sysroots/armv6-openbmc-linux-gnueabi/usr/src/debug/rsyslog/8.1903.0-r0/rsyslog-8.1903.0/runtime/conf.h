...
/* definitions used for doNameLine to differentiate between different command
 * types (with otherwise identical code). This is a left-over from the previous
 * config system. It stays, because it is still useful. So do not wonder why it
 * looks somewhat strange (at least its name). -- rgerhards, 2007-08-01
 */
enum eDirective
{
    DIR_TEMPLATE = 0,
...
extern int
    bConfStrictScoping; /* force strict scoping during config processing? */
...
ENDinterface(conf)
#define confCURR_IF_VERSION                                                    \
    6 /* increment whenever you change the interface structure! */
    /* in Version 3, entry point "ReInitConf()" was removed, as we do not longer
     * need to support restart-type HUP -- rgerhards, 2009-07-15
     */
