...
void xdump(const void* data, size_t length, const char* prefix)
{
 ...
 * u_char *var_val_type      OUT - type of variable (int or octet string) (one
 * byte) int *var_val_len          OUT - length of variable u_char **var_val OUT
 * - pointer to ASN1 encoded value of variable int *listlength          IN/OUT -
 * number of valid bytes left in var_op_list
 */

u_char* snmp_parse_var_op(u_char* data, oid* var_name, size_t* var_name_len,
                          ...
                          u_char** var_val, size_t* listlength)
{
    ...
    data = asn_parse_objid(data, &var_op_len, &var_op_type, var_name,
                           var_name_len);
    DEBUGINDENTLESS();
    if (data == NULL)
    {
...
u_char* snmp_build_var_op(u_char* data, oid* var_name, size_t* var_name_len,
                          u_char var_val_type, size_t var_val_len,
                          u_char* var_val, size_t* listlength)
{
    ...
    *listlength -= headerLen;
    ...
    data =
        asn_build_objid(data, listlength,
    ...
    {
        case ASN_INTEGER:
            data = asn_build_int(data, listlength, var_val_type, (long*)var_val,
                                 var_val_len);
            break;
        case ASN_GAUGE:
        case ASN_COUNTER:
        ...
        case ASN_OPAQUE:
        case ASN_NSAP:
            data = asn_build_string(data, listlength, var_val_type, var_val,
                                    var_val_len);
            break;
        case ASN_OBJECT_ID:
        ...
        case ASN_NULL:
            ...
            break;
        case ASN_BIT_STR:
            data = asn_build_bitstring(data, listlength, var_val_type, var_val,
                                       var_val_len);
            break;
        ...
        case ASN_OPAQUE_I64:
            data =
                asn_build_signed_int64(data, listlength, var_val_type,
...
int snmp_realloc_rbuild_var_op(u_char** pkt, size_t* pkt_len, size_t* offset,
                               int allow_realloc, const oid* var_name,
                               size_t* var_name_len, u_char var_val_type,
                               u_char* var_val, size_t var_val_len)
{
    ...
    switch (var_val_type)
    {
...
#endif
        case ASN_COUNTER64:
            rc = asn_realloc_rbuild_unsigned_int64(
                pkt, pkt_len, offset, allow_realloc, var_val_type,
                (struct counter64*)var_val, var_val_len);
            break;
        ...
        case ASN_OPAQUE_I64:
            rc = asn_realloc_rbuild_signed_int64(
                pkt, pkt_len, offset, allow_realloc, var_val_type,
                (struct counter64*)var_val, var_val_len);
            break;
#endif /* NETSNMP_WITH_OPAQUE_SPECIAL_TYPES */
        default:
        {
            char error_buf[64];
            snprintf(error_buf, sizeof(error_buf),
                     "wrong type in snmp_realloc_rbuild_var_op: %d",
                     var_val_type);
            ERROR_MSG(error_buf);
            rc = 0;
        }
    ...
    rc = asn_realloc_rbuild_objid(
        pkt, pkt_len, offset, allow_realloc,
        (u_char)(ASN_UNIVERSAL | ASN_PRIMITIVE | ASN_OBJECT_ID), var_name,
        *var_name_len);
    DEBUGINDENTLESS();
    if (rc == 0)
    {
