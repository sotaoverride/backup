...
static void cf2_computeDarkening(CF2_Fixed emRatio, CF2_Fixed ppem,
                                 CF2_Fixed stemWidth, CF2_Fixed* darkenAmount,
                                 CF2_Fixed boldenAmount, FT_Bool stemDarkened,
                                 FT_Int* darkenParams)
{
    /*
    ...
    *darkenAmount = 0;
    ...
    if (stemDarkened)
    {
        ...
        logBase2 =
            FT_MSB((FT_UInt32)stemWidthPer1000) + FT_MSB((FT_UInt32)ppem);
        ...
        else if (scaledStem < cf2_intToFixed(x2))
        {
            FT_Int xdelta = x2 - x1;
            FT_Int ydelta = y2 - y1;
            FT_Int x = stemWidthPer1000 - FT_DivFix(cf2_intToFixed(x1), ppem);
        ...
        Try_x3:
        {
            FT_Int xdelta = x3 - x2;
            FT_Int ydelta = y3 - y2;
            FT_Int x = stemWidthPer1000 - FT_DivFix(cf2_intToFixed(x2), ppem);
        ...
        Try_x4:
        {
            FT_Int xdelta = x4 - x3;
            FT_Int ydelta = y4 - y3;
            FT_Int x = stemWidthPer1000 - FT_DivFix(cf2_intToFixed(x3), ppem);
...
static void cf2_font_setup(CF2_Font font, const CF2_Matrix* transform)
{
    ...
    if (!font->isT1)
    {
        ...
        if (hasVariations)
        {
                                            ...
                                            lenNormalizedV, normalizedV))
            {
                ...
                cffload->load_private_dict(decoder->cff, subFont,
                                           lenNormalizedV, normalizedV);
                needExtraSetup = TRUE;
            }
#endif
    ...
    if (ft_memcmp(transform, &font->currentTransform, 4 * sizeof(CF2_Fixed)) !=
        0)
    {
        ...
        font->currentTransform.tx = font->currentTransform.ty =
            cf2_intToFixed(0);

        /* TODO: FreeType transform is simple scalar; for now, use identity */
        /*       for outer                                                  */
        font->innerTransform = *transform;
        font->outerTransform.a = font->outerTransform.d = cf2_intToFixed(1);
        font->outerTransform.b = font->outerTransform.c = cf2_intToFixed(0);
    ...
    {
        font->stemDarkened = FT_BOOL(font->renderingFlags & CF2_FlagsDarkened);
    ...
    if (needExtraSetup)
    {
        ...
        ppem =
            FT_MAX(cf2_intToFixed(4), font->ppem); /* use minimum ppem of 4 */
      ...
      emRatio = cf2_fixedFracMul( cf2_intToFixed( 1000 ), fontMatrix->a );
        ...
        if (boldenX > 0)
        {
            ...
            boldenX =
                FT_MAX(boldenX, FT_DivFix(cf2_intToFixed(unitsPerEm), ppem));
            ...
            /* synthetic bold font.                                          */
            cf2_computeDarkening(emRatio, ppem, font->stdVW, &font->darkenX,
                                 boldenX, FALSE, font->darkenParams);
        }
        else
            cf2_computeDarkening(emRatio, ppem, font->stdVW, &font->darkenX, 0,
                                 font->stemDarkened, font->darkenParams);
      ...
      emRatio = cf2_fixedFracMul( cf2_intToFixed( 1000 ), fontMatrix->d );
        ...
        cf2_computeDarkening(emRatio, ppem, font->stdHW, &font->darkenY,
                             boldenY, font->stemDarkened, font->darkenParams);
...
cf2_getGlyphOutline(CF2_Font font, CF2_Buffer charstring,
                    const CF2_Matrix* transform, CF2_F16Dot16* glyphWidth)
{
    ...
    while (1)
    {
        ...
        cf2_interpT2CharString(font, charstring,
                               (CF2_OutlineCallbacks)&font->outline,
                               &translation, FALSE, 0, 0, &advWidth);
