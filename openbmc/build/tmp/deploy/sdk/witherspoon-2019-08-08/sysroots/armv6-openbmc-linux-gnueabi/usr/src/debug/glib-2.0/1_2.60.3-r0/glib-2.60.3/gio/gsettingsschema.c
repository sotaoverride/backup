 ...
 * Since: 2.32
 **/
G_DEFINE_BOXED_TYPE(GSettingsSchemaSource, g_settings_schema_source,
 ...
 * Since: 2.32
 **/
G_DEFINE_BOXED_TYPE(GSettingsSchema, g_settings_schema, g_settings_schema_ref,
...
void g_settings_schema_source_unref(GSettingsSchemaSource* source)
{
    if (g_atomic_int_dec_and_test(&source->ref_count))
    {
        if (source == schema_sources)
            g_error("g_settings_schema_source_unref() called too many times on "
...
static void try_prepend_dir(const gchar* directory)
{
    GSettingsSchemaSource* source;

    source = g_settings_schema_source_new_from_directory(
        directory, schema_sources, TRUE, NULL);
...
static void try_prepend_data_dir(const gchar* directory)
{
                                                 ...
                                                 gboolean recursive)
{
    ...
    {
        schema->extends =
            g_settings_schema_source_lookup(source, extends, TRUE);
        if (schema->extends == NULL)
            g_warning(
                "Schema '%s' extends schema '%s' but we could not find it",
                schema_id, extends);
    }
...
static const gchar* get_attribute_value(GSList* list)
{
    GSList* node;
...
static void pop_attribute_value(GSList** list)
{
    gchar* top;
...
static void push_attribute_value(GSList** list, const gchar* value)
{
...
static gchar* normalise_whitespace(const gchar* orig)
{
                        ...
                        gpointer user_data, GError** error)
{
    ...
    if (info->string)
    {
        ...
        if (source_table && schema_id && key_name)
        {
            ...
            {
                schema_table = g_hash_table_new_full(g_str_hash, g_str_equal,
                                                     g_free, g_free);
                g_hash_table_insert(source_table, g_strdup(schema_id),
                                    schema_table);
            }
...
static void parse_into_text_tables(const gchar* directory,
                                   GHashTable* summaries,
                                   GHashTable* descriptions)
{
    ...
    while ((basename = g_dir_read_name(dir)))
    {
        ...
        if (g_file_get_contents(filename, &contents, &size, NULL))
        {
            ...
            context = g_markup_parse_context_new(
                &parser, G_MARKUP_TREAT_CDATA_AS_TEXT, &info, NULL);
    ...
    g_settings_schema_source_get_text_tables(GSettingsSchemaSource* source)
{
    if (g_once_init_enter(&source->text_tables))
    {
        ...
        if (source->directory)
            parse_into_text_tables(source->directory, text_tables[0],
                                   text_tables[1]);
                                           ...
                                           gchar*** relocatable)
{
    ...
    {
        *non_relocatable =
            (gchar**)g_hash_table_get_keys_as_array(single, NULL);
                                          ...
                                          const gchar* key)
{
    ...
    if
        G_UNLIKELY(value == NULL ||
                   !g_variant_is_of_type(value, G_VARIANT_TYPE_TUPLE))
        ...
        while (g_hash_table_iter_next(&iter, &name, NULL))
            if (g_str_has_suffix(name, "/"))
            {
                ...
                for (source = schema->source; source; source = source->parent)
                    if ((child_table = gvdb_table_get_table(
                             source->table,
                             g_variant_get_string(child_schema, NULL))))
                        break;
                ...
                if (gvdb_table_has_value(child_table, ".path"))
                {
                    ...
                    same =
                        g_str_equal(expected, g_variant_get_string(path, NULL));
        ...
        switch (code)
        {
            ...
            case 'c':
                ...
                key->strinfo = g_variant_get_fixed_array(
                    data, &key->strinfo_length, sizeof(guint32));
                break;
...
GVariant* g_settings_schema_key_get_translated_default(GSettingsSchemaKey* key)
{
    ...
    if (value == NULL)
    {
        g_warning("Failed to parse translated string '%s' for "
                  "key '%s' in schema '%s': %s",
                  translated, key->name, g_settings_schema_get_id(key->schema),
                  error->message);
...
GVariant* g_settings_schema_key_get_per_desktop_default(GSettingsSchemaKey* key)
{
    ...
    for (i = 0; value == NULL && current_desktops[i] != NULL; i++)
        value = g_variant_lookup_value(key->desktop_overrides,
                                       current_desktops[i], NULL);

    return value;
}

gint g_settings_schema_key_to_enum(GSettingsSchemaKey* key, GVariant* value)
{
    gboolean it_worked;
    guint result;

    it_worked =
        strinfo_enum_from_string(key->strinfo, key->strinfo_length,
                                 g_variant_get_string(value, NULL), &result);

    /* 'value' can only come from the backend after being filtered for validity,
     * from the translation after being filtered for validity, or from the
     * schema itself (which the schema compiler checks for validity).  If this
     * assertion fails then it's really a bug in GSettings or the schema
     * compiler...
     */
    g_assert(it_worked);
...
GVariant* g_settings_schema_key_from_enum(GSettingsSchemaKey* key, gint value)
{
    const gchar* string;
...
guint g_settings_schema_key_to_flags(GSettingsSchemaKey* key, GVariant* value)
{
    GVariantIter iter;
    const gchar* flag;
    guint result;
...
/* Returns a new floating #GVariant. */
GVariant* g_settings_schema_key_from_flags(GSettingsSchemaKey* key, guint value)
{
    GVariantBuilder builder;
    gint i;
...
G_DEFINE_BOXED_TYPE(GSettingsSchemaKey, g_settings_schema_key,
                    g_settings_schema_key_ref, g_settings_schema_key_unref)
