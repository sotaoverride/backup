...
void read_power_gpios(GpioConfigs* gpios, const cJSON* gpio_configs)
{
    ...
    const cJSON* power_config =
        cJSON_GetObjectItem(gpio_configs, "power_config");
    ...
    const cJSON* power_up_outs =
        cJSON_GetObjectItem(power_config, "power_up_outs");
    ...
    const cJSON* pci_reset_outs =
        cJSON_GetObjectItem(power_config, "pci_reset_outs");

    gpios->power_gpio.num_pci_reset_outs = cJSON_GetArraySize(pci_reset_outs);
    ...
    if (gpios->power_gpio.num_pci_reset_outs != 0)
    {
        ...
        cJSON_ArrayForEach(pci_reset_out, pci_reset_outs)
        {
            ...
            const cJSON* polarity =
                cJSON_GetObjectItem(pci_reset_out, "polarity");
            g_assert(polarity != NULL);
