...
/* static data */
DEFobjStaticHelpers DEFobjCurrIf(glbl) DEFobjCurrIf(netstrm) DEFobjCurrIf(prop)
...
BEGINobjConstruct(
    tcps_sess)       /* be sure to specify the object type also in END macro! */
...
BEGINobjDestruct(tcps_sess) /* be sure to specify the object type also in END
                               and CODESTART macros! */
    CODESTARTobjDestruct(tcps_sess) if (pThis->pStrm !=
                                        NULL) netstrm.Destruct(&pThis->pStrm);
...
ENDobjDestruct(tcps_sess)
    ...
    BEGINobjDebugPrint(tcps_sess) /* be sure to specify the object type also in
                                     END and CODESTART macros! */
    CODESTARTobjDebugPrint(tcps_sess) ENDobjDebugPrint(tcps_sess)

    /* set property functions */
    /* set the hostname. Note that the caller *hands over* the string. That is,
     * the caller no longer controls it once SetHost() has received it. Most
     * importantly, the caller must not free it. -- rgerhards, 2008-04-24
     */
...
/* set the remote host's IP. Note that the caller *hands over* the property.
 * That is, the caller no longer controls it once SetHostIP() has received it.
 * Most importantly, the caller must not destruct it. -- rgerhards, 2008-05-16
 */
...
static rsRetVal SetOnMsgReceive(tcps_sess_t* pThis,
                                       ...
                                       multi_submit_t* pMultiSub)
{
    ...
    STATSCOUNTER_INC(pThis->pLstnInfo->ctrSubmit,
                     pThis->pLstnInfo->mutCtrSubmit);
...
static rsRetVal PrepareClose(tcps_sess_t* pThis)
{
    ...
    { /* here, we have traditional framing. Missing LF at the end
       ...
       * this case.
       */
        DBGPRINTF("Extra data at end of stream in legacy syslog/tcp message - "
                  "processing\n");
 ...
 * data inside a state machine. The actual data received is passed in
 * byte-by-byte from DataRcvd, and this function here compiles messages from
 * them and submits the end result to the queue. Introducing this function fixes
 * a long-term bug ;) rgerhards, 2008-03-14
 */
static rsRetVal processDataRcvd(tcps_sess_t* pThis, char c,
                                struct syslogTime* stTime,
                                ...
                                unsigned* const __restrict__ pnMsgs)
{
    ...
    if (pThis->inputState == eInOctetCnt)
    {
        ...
        { /* done with the octet count, so this must be the SP terminator */
            ...
            if (c != ' ')
            {
                LogError(
                    ...
                    "ASCII value %d.",
                    pThis->pSrv->pszInputName, propPeerName, propPeerIP, c);
            }
            if (pThis->iOctetsRemain < 1)
            {
                ...
                LogError(
                    ...
                    pThis->pSrv->pszInputName, propPeerName, propPeerIP,
                    pThis->iOctetsRemain);
                pThis->eFraming = TCP_FRAMING_OCTET_STUFFING;
            }
            else if (pThis->iOctetsRemain > iMaxLine)
            {
                /* while we can not do anything against it, we can at least log
                 * an indication that something went wrong) -- rgerhards,
                 * 2008-03-14
                 */
                LogError(
                    0, NO_ERRCODE,
    ...
    else
    {
        ...
        {
            /* emergency, we now need to flush, no matter if we are at end of
             * message or not... */
            DBGPRINTF("error: message received is larger than max msg size, we "
                      "split it\n");
        ...
        else
        {
             ...
             * for BOTH framing modes! If we have a message that is larger than
             * the max msg size, we truncate it. This is the best we can do in
             * light of what the engine supports. -- rgerhards, 2008-03-14
             */
            if (pThis->iMsg < iMaxLine)
            {
                *(pThis->pMsg + pThis->iMsg++) = c;
...
BEGINobjQueryInterface(tcps_sess)
    CODESTARTobjQueryInterface(tcps_sess) if (pIf->ifVersion !=
                                              tcps_sessCURR_IF_VERSION)
...
finalize_it : ENDobjQueryInterface(tcps_sess)
...
ENDObjClassExit(tcps_sess)
     ...
     * rgerhards, 2008-02-29
     */
    BEGINObjClassInit(tcps_sess, 1,
                      OBJ_IS_CORE_MODULE) /* class, version - CHANGE class also
                                             in END MACRO! */
