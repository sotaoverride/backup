...
/* An externally visible state. We don't actually maintain this as state
 * variable, but derive it from various fields when requested */
 ...
 *
 * 3. The realtime timestamp of TIMESTAMP_KERNEL will be unset if the system
 * does not have RTC.
 *
 * 4. TIMESTAMP_FIRMWARE and TIMESTAMP_LOADER will be unset if the system does
 * not have RTC, or systemd is built without EFI support.
 *
 * 5. The monotonic timestamps of TIMESTAMP_FIRMWARE and TIMESTAMP_LOADER are
 * stored as negative of the actual value.
 *
...
typedef enum ManagerTestRunFlags
{
    ...
    MANAGER_TEST_FULL = MANAGER_TEST_RUN_BASIC |
...
struct Manager
{
    ...
    /* This maps PIDs we care about to units that are interested in. We allow
     * multiple units to he interested in the same PID and multiple PIDs to be
     * relevant to the same unit. Since in most cases only a single unit will be
     * interested in the same PID we use a somewhat special encoding here: the
     * first unit interested in a PID is stored directly in the hashmap, keyed
     * by the PID unmodified. If there are other units interested too they'll be
     * stored in a NULL-terminated array, and keyed by the negative PID. This is
     * safe as pid_t is signed and negative PIDs are not used for regular
     * processes but process groups, which we don't care about in this
     * context, but this allows us to use the negative range for our own
     * purposes. */
    ...
    char** transient_environment; /* The environment, as determined from config
                                     files, kernel cmdline and environment
                                     generators */
    char** client_environment;    /* Environment variables created by clients
                                     through the bus API */
    ...
    /* Notifications from cgroups, when the unified hierarchy is used is done
     * via inotify. */
    int cgroup_inotify_fd;
    sd_event_source* cgroup_inotify_event_source;
    Hashmap* cgroup_inotify_wd_unit;

    /* A defer event for handling cgroup empty events and processing them after
     * SIGCHLD in all cases. */
    ...
    /* Keep track of all UIDs and GIDs any of our services currently use. This
     * is useful for the RemoveIPC= logic. */
    Hashmap* uid_refs;
    Hashmap* gid_refs;
    ...
    /* When the user hits C-A-D more than 7 times per 2s, do something
     * immediately... */
    ...
    /* Used in the SIGCHLD and sd_notify() message invocation logic to avoid
...
#define MANAGER_IS_FINISHED(m)                                                 \
    (dual_timestamp_is_set((m)->timestamps + MANAGER_TIMESTAMP_FINISH))

/* The objective is set to OK as soon as we enter the main loop, and set
 * otherwise as soon as we are done with it */
...
int manager_add_job(Manager* m, JobType type, Unit* unit, JobMode mode,
                    Set* affected_jobs, sd_bus_error* e, Job** _ret);
