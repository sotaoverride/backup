...
static const struct stats_descr ipv4_snmp_stats[] = {
    ...
    {"Ip:", "ReasmOKs", &cached_ip_mib, offsetof(struct ip_mib, ipReasmOKs)},
    {"Ip:", "ReasmFails", &cached_ip_mib,
     offsetof(struct ip_mib, ipReasmFails)},
    {"Ip:", "FragOKs", &cached_ip_mib, offsetof(struct ip_mib, ipFragOKs)},
    {"Ip:", "FragFails", &cached_ip_mib, offsetof(struct ip_mib, ipFragFails)},
    ...
    {"Tcp:", "RtoMin", &cached_tcp_mib, offsetof(struct tcp_mib, tcpRtoMin)},
    {"Tcp:", "RtoMax", &cached_tcp_mib, offsetof(struct tcp_mib, tcpRtoMax)},
    {"Tcp:", "MaxConn", &cached_tcp_mib, offsetof(struct tcp_mib, tcpMaxConn)},
    ...
    {"Tcp:", "InSegs", &cached_tcp_mib, offsetof(struct tcp_mib, tcpInSegs)},
    {"Tcp:", "OutSegs", &cached_tcp_mib, offsetof(struct tcp_mib, tcpOutSegs)},
    {"Tcp:", "RetransSegs", &cached_tcp_mib,
     offsetof(struct tcp_mib, tcpRetransSegs)},
    {"Tcp:", "InErrs", &cached_tcp_mib, offsetof(struct tcp_mib, tcpInErrs),
     offsetof(struct tcp_mib, tcpInErrsValid)},
    {"Tcp:", "OutRsts", &cached_tcp_mib, offsetof(struct tcp_mib, tcpOutRsts),
     offsetof(struct tcp_mib, tcpOutRstsValid)},

    {"Udp:", "InDatagrams", &cached_udp_mib,
     offsetof(struct udp_mib, udpInDatagrams)},
    {"Udp:", "NoPorts", &cached_udp_mib, offsetof(struct udp_mib, udpNoPorts)},
...
netsnmp_feature_child_of(linux_ip6_stat_all, libnetsnmpmibs)
    ...
    netsnmp_feature_child_of(linux_read_ip6_stat, linux_ip6_stat_all)
        ...
        static int decode_icmp_msg(char* line, char* data,
                                   struct icmp4_msg_mib* msg)
{
...
static int linux_read_mibII_stats(void)
{
    FILE* in;
    char line[1024], data[1024], *linepos, *datapos, *pfx, *hdr, *v;
    const struct stats_descr* d;
    const struct stats_descr* const end =
        ipv4_snmp_stats + sizeof(ipv4_snmp_stats) / sizeof(ipv4_snmp_stats[0]);
    int ret = 0;
...
int linux_read_ip6_stat(struct ip6_mib* ip6stat)
{
...
#ifdef NETSNMP_ENABLE_IPV6
    DEBUGMSGTL(
        ("mibII/kernel_linux/ip6stats", "Reading /proc/net/snmp6 stats\n"));
    ...
    while (NULL != fgets(line, sizeof(line), in))
    {
        ...
        else if (0 == strncmp(line + 3, "Out", 3))
        { /* Out */
        ...
        else if (0 == strncmp(line + 3, "Reasm", 5))
        { /* Reasm */
        ...
        else if (0 == strncmp(line + 3, "Frag", 4))
        { /* Frag */
        ...
        if (!match)
            DEBUGMSGTL(("mibII/kernel_linux/ip6stats", "%s is an unknown tag\n",
                        line));
    }
...
int linux_read_icmp_msg_stat(struct icmp_mib* icmpstat,
                             struct icmp4_msg_mib* icmpmsgstat, int* flag)
{
    int ret;
...
static int linux_read_icmp6_parse(struct icmp6_mib* icmp6stat,
                                  ...
                                  int* support)
{
...
#ifdef NETSNMP_ENABLE_IPV6
    DEBUGMSGTL(
        ("mibII/kernel_linux/icmp6stats", "Reading /proc/net/snmp6 stats\n"));
    if (NULL == (in = fopen("/proc/net/snmp6", "r")))
    {
    ...
    while (NULL != fgets(line, sizeof(line), in))
    {
        ...
        else if (0 == strncmp(line + 5, "Out", 3))
        { /* Out */
    ...
    memcpy((char*)icmp6stat, (char*)&cached_icmp6_mib, sizeof(*icmp6stat));
    return 0;
}

int linux_read_icmp6_msg_stat(struct icmp6_mib* icmp6stat,
                              struct icmp6_msg_mib* icmp6msgstat, int* support)
{
...
#ifdef NETSNMP_ENABLE_IPV6
    DEBUGMSGTL(
        ("mibII/kernel_linux/udp6stats", "Reading /proc/net/snmp6 stats\n"));
    if (NULL == (in = fopen("/proc/net/snmp6", "r")))
    {
