...
static void info_scsi_cmd_err(const char* cmd, int err)
{
    ...
    log_debug("%s failed with SK=%Xh/ASC=%02Xh/ACQ=%02Xh", cmd, SK(err),
              ASC(err), ASCQ(err));
}
...
static int scsi_cmd_run(struct scsi_cmd* cmd, int fd, unsigned char* buf,
                        size_t bufsize)
{
    int ret = 0;
...
static int cd_profiles(int fd)
{
    ...
    if ((err != 0))
    {
        ...
        {
            log_debug("drive is pre-MMC2 and does not support 46h get "
    ...
    len =
        features[0] << 24 | features[1] << 16 | features[2] << 8 | features[3];
    ...
    for (i = 8; i + 4 < len; i += (4 + features[i + 3]))
    {
        ...
        switch (feature)
        {
            case 0x00:
                log_debug(
                    "GET CONFIGURATION: feature 'profiles', with %i entries",
                    features[i + 3] / 4);
                feature_profiles(&features[i] + 4,
                                 MIN(features[i + 3], len - i - 4));
                break;
            default:
                log_debug("GET CONFIGURATION: feature 0x%04x <ignored>, with "
                          "0x%02x bytes",
...
static int cd_media_info(int fd)
{
    struct scsi_cmd sc;
    unsigned char header[32];
    static const char* const media_status[] = {"blank", "appendable",
                                               "complete", "other"};
    int err;
    ...
    if ((cd_media_dvd_rw_ro || cd_media_dvd_plus_rw ||
         cd_media_dvd_plus_rw_dl || cd_media_dvd_ram) &&
        (header[2] & 3) > 1)
    {
...
static int cd_media_toc(int fd)
{
    ...
    log_debug("READ TOC: len: %d, start track: %d, end track: %d", len,
              header[2], header[3]);
    if (len > sizeof(toc))
        return -1;
    if (len < 2)
    ...
    for (p = toc + 4, i = 4; i < len - 8 && num_tracks > 0;
         i += 8, p += 8, --num_tracks)
    {
        unsigned block;
        ...
        log_debug("track=%u info=0x%x(%s) start_block=%u", p[2], p[1] & 0x0f,
                  is_data_track ? "data" : "audio", block);
    ...
    len = header[4 + 4] << 24 | header[4 + 5] << 16 | header[4 + 6] << 8 |
          header[4 + 7];
...
int main(int argc, char* argv[])
{
        ...
        switch (option)
        {
            ...
            case 'h':
                printf("Usage: cdrom_id [options] <device>\n"
                       "  -l,--lock-media    lock the media (to enable eject "
                       "request events)\n"
    ...
    /* at this point we are guaranteed to have media in the drive - find out
     * more about it */
