...
extern int INTERNAL(REENTRANT_NAME)(
    ADD_PARAMS, LOOKUP_TYPE* resbuf, char* buffer, size_t buflen,
    LOOKUP_TYPE** result H_ERRNO_PARM) attribute_hidden;
...
LOOKUP_TYPE* FUNCTION_NAME(ADD_PARAMS)
{
    ...
    if (buffer != NULL)
    {
        if (__nss_hostname_digits_dots_context(res_ctx, name, &resbuf, &buffer,
                                               &buffer_size, 0, &result, NULL,
    ...
    while (
        buffer != NULL &&
        (INTERNAL(REENTRANT_NAME)(ADD_VARIABLES, &resbuf, buffer, buffer_size,
                                  &result H_ERRNO_VAR) == ERANGE)
#ifdef NEED_H_ERRNO
