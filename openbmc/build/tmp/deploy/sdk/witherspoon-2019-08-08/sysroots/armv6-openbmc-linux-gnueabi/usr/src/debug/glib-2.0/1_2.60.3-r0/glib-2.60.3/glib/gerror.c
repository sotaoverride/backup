 ...
 * g_assert ((contents == NULL && err != NULL) || (contents != NULL && err ==
 * NULL)); if (err != NULL)
 *   {
 ...
 * Since: 2.22
 */
GError* g_error_new_valist(GQuark domain, gint code, const gchar* format,
                           va_list args)
{
    GError* error;
 ...
 * Since: 2.18
 */
void g_set_error_literal(GError** err, GQuark domain, gint code,
                         const gchar* message)
{
    if (err == NULL)
        return;
...
G_GNUC_PRINTF(2, 0)
static void g_error_add_prefix(gchar** string, const gchar* format, va_list ap)
{
    gchar* oldstring;
    gchar* prefix;
