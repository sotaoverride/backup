...
static void af_cjk_metrics_init_blues(AF_CJKMetrics metrics, FT_Face face)
{
    ...
    for (; bs->string != AF_BLUE_STRING_MAX; bs++)
    {
        ...
        if (num_flats == 0)
        {
            *blue_ref = *blue_shoot = fills[num_fills / 2];
        }
        else if (num_fills == 0)
        {
...
static void af_cjk_metrics_scale_dim(AF_CJKMetrics metrics, AF_Scaler scaler,
                                     AF_Dimension dim)
{
    FT_Fixed scale;
    FT_Pos delta;
        ...
        if (dist <= 48 && dist >= -48)
        {
          ...
          delta2 = 32 + ( ( ( delta2 - 32 ) + 16 ) & ~31 );
            ...
            FT_TRACE5((">> active cjk blue zone %c%d[%ld/%ld]:\n"
                       "     ref:   cur=%.2f fit=%.2f\n"
                       "     shoot: cur=%.2f fit=%.2f\n",
                       (dim == AF_DIMENSION_HORZ) ? 'H' : 'V', nn,
                       blue->ref.org, blue->shoot.org, blue->ref.cur / 64.0,
                       blue->ref.fit / 64.0, blue->shoot.cur / 64.0,
                       blue->shoot.fit / 64.0));
...
FT_LOCAL_DEF(void)
af_cjk_get_standard_widths(AF_CJKMetrics metrics, FT_Pos* stdHW, FT_Pos* stdVW)
{
    if (stdHW)
...
static void af_cjk_hints_link_segments(AF_GlyphHints hints, AF_Dimension dim)
{
    AF_AxisHints axis = &hints->axis[dim];
    AF_Segment segments = axis->segments;
    AF_Segment segment_limit = segments + axis->num_segments;
    ...
    dist_threshold =
        (dim == AF_DIMENSION_HORZ) ? hints->x_scale : hints->y_scale;
...

    {
        ...
        for (seg1 = segments; seg1 < segment_limit; seg1++)
        {
            ...
            for (seg2 = segments; seg2 < segment_limit; seg2++)
            {
                ...
                if (seg2->score <= seg1->score ||
                    seg1->score * 4 <= seg2->score)
                    continue;
                                           ...
                                           AF_Dimension dim)
{
    ...
    for (seg = segments; seg < segment_limit; seg++)
    {
        ...
        if (!found)
        {
            ...
            error = af_axis_hints_new_edge(
                axis, seg->pos, (AF_Direction)seg->dir, 0, memory, &edge);
            if (error)
                goto Exit;
...
static void af_cjk_hints_compute_blue_edges(AF_GlyphHints hints,
...
static FT_Pos af_cjk_snap_width(AF_Width widths, FT_UInt count, FT_Pos width)
{
    FT_UInt n;
    FT_Pos best = 64 + 32 + 2;
...
static FT_Pos af_cjk_compute_stem_width(AF_GlyphHints hints, AF_Dimension dim,
                                        FT_Pos width, FT_UInt base_flags,
                                        FT_UInt stem_flags)
{
...
static void af_cjk_align_linked_edge(AF_GlyphHints hints, AF_Dimension dim,
                                     AF_Edge base_edge, AF_Edge stem_edge)
{
    FT_Pos dist = stem_edge->opos - base_edge->opos;

    FT_Pos fitted_width = af_cjk_compute_stem_width(
        hints, dim, dist, base_edge->flags, stem_edge->flags);
    ...
    FT_TRACE5(("  CJKLINK: edge %d @%d (opos=%.2f) linked to %.2f,"
               ...
               stem_edge->opos / 64.0, stem_edge->pos / 64.0, dist / 64.0,
               fitted_width / 64.0));
}
...
static void af_cjk_align_serif_edge(AF_GlyphHints hints, AF_Edge base,
                                    AF_Edge serif)
{
    FT_UNUSED(hints);
...
static FT_Pos af_hint_normal_stem(AF_GlyphHints hints, AF_Edge edge,
                                  AF_Edge edge2, FT_Pos anchor,
                                  AF_Dimension dim)
{
    ...
    if (!AF_LATIN_HINTS_DO_STEM_ADJUST(hints))
    {
        if ((edge->flags & AF_EDGE_ROUND) && (edge2->flags & AF_EDGE_ROUND))
        {
    ...
    cur_len = af_cjk_compute_stem_width(hints, dim, org_len, edge->flags,
                                        edge2->flags);
    ...
    if (threshold < 64)
    {
        if (d_off1 >= threshold || u_off1 >= threshold || d_off2 >= threshold ||
            u_off2 >= threshold)
            goto Exit;
    }
...
static void af_cjk_hint_edges(AF_GlyphHints hints, AF_Dimension dim)
{
    AF_AxisHints axis = &hints->axis[dim];
    AF_Edge edges = axis->edges;
    AF_Edge edge_limit = edges + axis->num_edges;
    ...
    for (edge = edges; edge < edge_limit; edge++)
    {
         ...
         */
        if (has_last_stem &&
            (edge->pos < last_stem_pos + 64 || edge2->pos < last_stem_pos + 64))
        {
            skipped++;
            continue;
        }
        ...
        if ( fixedpitch )
        {
          ...
          right->flags       |= AF_EDGE_DONE;
            ...
            delta =
                af_hint_normal_stem(hints, edge, edge2, 0, AF_DIMENSION_HORZ);
        }
        else
    ...
    if (dim == AF_DIMENSION_HORZ && (n_edges == 6 || n_edges == 12))
    {
        ...
        if (edge1->link == edge1 + 1 && edge2->link == edge2 + 1 &&
        ...
        if (before >= edges || after < edge_limit)
        {
            ...
            else
            {
                if (after->fpos == before->fpos)
                    ...
                    edge->pos =
                        before->pos + FT_MulDiv(edge->fpos - before->fpos,
...
static void af_cjk_align_edge_points(AF_GlyphHints hints, AF_Dimension dim)
{
    AF_AxisHints axis = &hints->axis[dim];
    AF_Edge edges = axis->edges;
    AF_Edge edge_limit = edges + axis->num_edges;
    ...
    snapping = FT_BOOL(
        (dim == AF_DIMENSION_HORZ && AF_LATIN_HINTS_DO_HORZ_SNAP(hints)) ||
...
FT_LOCAL_DEF(FT_Error)
af_cjk_hints_apply(FT_UInt glyph_index, AF_GlyphHints hints,
                   FT_Outline* outline, AF_CJKMetrics metrics)
{
    FT_Error error;
    int dim;
            ...
            (dim == AF_DIMENSION_VERT && AF_HINTS_DO_VERTICAL(hints)))
        {
                ...
                AF_HINTS_DO_WARP(hints))
            {
                ...
                af_warper_compute(&warper, hints, (AF_Dimension)dim, &scale,
                                  &delta);
                af_glyph_hints_scale_dim(hints, (AF_Dimension)dim, scale,
                                         delta);
                continue;
            }
