                                   ...
                                   unsigned char* to, RSA* rsa, int padding)
{
    ...
    if (BN_ucmp(f, rsa->n) >= 0)
    {
        RSAerr(RSA_F_RSA_OSSL_PUBLIC_DECRYPT, RSA_R_DATA_TOO_LARGE_FOR_MODULUS);
        goto err;
    }
...
static int rsa_ossl_mod_exp(BIGNUM* r0, const BIGNUM* I, RSA* rsa, BN_CTX* ctx)
{
    ...
    if (rsa->flags & RSA_FLAG_CACHE_PRIVATE)
    {
        ...
        if (!(BN_with_flags(factor, rsa->p, BN_FLG_CONSTTIME),
              BN_MONT_CTX_set_locked(&rsa->_method_mod_p, rsa->lock, factor,
                                     ctx)) ||
            !(BN_with_flags(factor, rsa->q, BN_FLG_CONSTTIME),
              BN_MONT_CTX_set_locked(&rsa->_method_mod_q, rsa->lock, factor,
                                     ctx)))
        {
            BN_free(factor);
            goto err;
    ...
    if (smooth)
    {
        ...
        if (/* m1 = I moq q */
            ...
            /* r1 = r1 * iqmp mod p */
            || !bn_to_mont_fixed_top(r1, r1, rsa->_method_mod_p, ctx) ||
            !bn_mul_mont_fixed_top(r1, r1, rsa->iqmp, rsa->_method_mod_p, ctx)
            /* r0 = r1 * q + m1 */
