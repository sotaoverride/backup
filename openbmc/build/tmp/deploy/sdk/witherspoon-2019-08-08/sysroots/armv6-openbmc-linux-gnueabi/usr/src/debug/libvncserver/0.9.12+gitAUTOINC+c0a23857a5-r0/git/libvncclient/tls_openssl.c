...
static int ssl_errno(SSL* ssl, int ret)
{
    switch (SSL_get_error(ssl, ret))
    {
        ...
        case SSL_ERROR_SYSCALL:
            ...
            return EINTR;
        case SSL_ERROR_SSL:
            // d(printf ("ssl_errno: SSL_ERROR_SSL  <-- very useful
            // error...riiiiight\n"));
            return EINTR;
        default:
...
static rfbBool InitializeTLS(void)
{
    int i;
...
static rfbBool load_crls_from_file(char* file, SSL_CTX* ssl_ctx)
{
    X509_STORE* st;
    X509_CRL* crl;
    int i;
...
static SSL* open_ssl_connection(rfbClient* client, int sockfd, rfbBool anonTLS,
                                rfbCredential* cred)
{
    ...
    if (!anonTLS)
    {
        if (cred->x509Credential.x509CACertFile)
        {
            if (!SSL_CTX_load_verify_locations(
                    ssl_ctx, cred->x509Credential.x509CACertFile, NULL))
            {
        ...
        {
            if (!load_crls_from_file(cred->x509Credential.x509CACrlFile,
                                     ssl_ctx))
            {
            ...
            if (verify_crls == rfbX509CrlVerifyNone)
                verify_crls = rfbX509CrlVerifyAll;
        }
            ...
            cred->x509Credential.x509ClientKeyFile)
        {
            if (SSL_CTX_use_certificate_chain_file(
                    ssl_ctx, cred->x509Credential.x509ClientCertFile) != 1)
            {
            ...
            if (SSL_CTX_use_PrivateKey_file(
                    ssl_ctx, cred->x509Credential.x509ClientKeyFile,
                    SSL_FILETYPE_PEM) != 1)
            {
        ...
        if (!X509_VERIFY_PARAM_set1_host(param, client->serverHost,
...
static rfbBool InitializeTLSSession(rfbClient* client, rfbBool anonTLS,
                                    rfbCredential* cred)
{
    ...
    client->tlsSession =
        open_ssl_connection(client, client->sock, anonTLS, cred);
...
static rfbBool HandshakeTLS(rfbClient* client)
{
    int timeout = 15;
    int ret;
...
static rfbBool ReadVeNCryptSecurityType(rfbClient* client, uint32_t* result)
{
    uint8_t count = 0;
    uint8_t loop = 0;
    uint8_t flag = 0;
    ...
    {
        if (!ReadFromRFBServer(client, (char*)&tAuth[loop], 4))
            return FALSE;
        t = rfbClientSwap32IfLE(tAuth[loop]);
        rfbClientLog("%d) Received security type %d\n", loop, t);
        if (flag)
            continue;
        if (t == rfbVeNCryptTLSNone || t == rfbVeNCryptTLSVNC ||
            t == rfbVeNCryptTLSPlain ||
#ifdef LIBVNCSERVER_HAVE_SASL
            t == rfbVeNCryptTLSSASL || t == rfbVeNCryptX509SASL ||
#endif /*LIBVNCSERVER_HAVE_SASL */
            t == rfbVeNCryptX509None || t == rfbVeNCryptX509VNC ||
            t == rfbVeNCryptX509Plain)
        {
            ...
            /* send back 4 bytes (in original byte order!) indicating which
             * security type to use */
            if (!WriteToRFBServer(client, (char*)&tAuth[loop], 4))
                return FALSE;
        }
        tAuth[loop] = t;
    }
...
{
    if (!InitializeTLS() || !InitializeTLSSession(client, TRUE, NULL))
        return FALSE;
...
static void FreeX509Credential(rfbCredential* cred)
{
    if (cred->x509Credential.x509CACertFile)
...
rfbBool HandleVeNCryptAuth(rfbClient* client)
{
    ...
    if (!ReadVeNCryptSecurityType(client, &authScheme))
        return FALSE;
    ...
    if (!InitializeTLSSession(client, anonTLS, cred))
        result = FALSE;
