...
static void exp_dump_tuples(struct nfnl_exp* exp, struct nl_dump_params* p)
{
    ...
    for (i = NFNL_EXP_TUPLE_EXPECT; i < NFNL_EXP_TUPLE_MAX; i++)
    {
        ...
        {
            nl_dump(p, "%s ",
                    nl_ip_proto2str(nfnl_exp_get_l4protonum(exp, i), buf,
                                    sizeof(buf)));
        }
...
static void exp_dump_details(struct nl_object* a, struct nl_dump_params* p)
{
    ...
    nl_dump_line(p, "family %s ", nl_af2str(exp->exp_family, buf, sizeof(buf)));

    if (nfnl_exp_test_timeout(exp))
    {
        uint64_t timeout_ms = nfnl_exp_get_timeout(exp) * 1000UL;
        nl_dump(p, "timeout %s ", nl_msec2str(timeout_ms, buf, sizeof(buf)));
    }
...
static int exp_cmp_l4proto_ports(union nfnl_exp_protodata* a,
                                 union nfnl_exp_protodata* b)
{
    // Must return 0 for match, 1 for mismatch
    int d = 0;
    d = ((a->port.src != b->port.src) || (a->port.dst != b->port.dst));

    return d;
}

static int exp_cmp_l4proto_icmp(union nfnl_exp_protodata* a,
                                union nfnl_exp_protodata* b)
{
    // Must return 0 for match, 1 for mismatch
    int d = 0;
    d = ((a->icmp.code != b->icmp.code) || (a->icmp.type != b->icmp.type) ||
         (a->icmp.id != b->icmp.id));
...
#define EXP_DIFF_STRING(ATTR, FIELD)                                           \
    EXP_DIFF(ATTR, (strcmp(a->FIELD, b->FIELD) != 0))
...
#define EXP_DIFF_L4PROTO_ICMP(ATTR, FIELD)                                     \
    ...
    diff |= EXP_DIFF_L4PROTO_PORTS(EXPECT_L4PROTO_PORTS,
                                   exp_expect.proto.l4protodata);
    diff |= EXP_DIFF_L4PROTO_ICMP(EXPECT_L4PROTO_ICMP,
                                  exp_expect.proto.l4protodata);
    ...
    diff |= EXP_DIFF_L4PROTO_PORTS(MASTER_L4PROTO_PORTS,
                                   exp_master.proto.l4protodata);
    diff |= EXP_DIFF_L4PROTO_ICMP(MASTER_L4PROTO_ICMP,
                                  exp_master.proto.l4protodata);
    ...
    diff |=
        EXP_DIFF_L4PROTO_PORTS(MASK_L4PROTO_PORTS, exp_mask.proto.l4protodata);
    diff |=
        EXP_DIFF_L4PROTO_ICMP(MASK_L4PROTO_ICMP, exp_mask.proto.l4protodata);
    ...
    diff |=
        EXP_DIFF_L4PROTO_PORTS(NAT_L4PROTO_PORTS, exp_nat.proto.l4protodata);
...
{
    return __flags2str(flags, buf, len, flag_table, ARRAY_SIZE(flag_table));
}
...
#define EXP_GET_TUPLE(e, t)                                                    \
    (t == NFNL_EXP_TUPLE_MASTER)                                               \
        ...
        : (t == NFNL_EXP_TUPLE_MASK)                                           \
              ? &(e->exp_mask)                                                 \
              : (t == NFNL_EXP_TUPLE_NAT) ? &(e->exp_nat) : &(exp->exp_expect)
...
static int exp_set_addr(struct nfnl_exp* exp, struct nl_addr* addr, int attr,
...
void nfnl_exp_set_l4protonum(struct nfnl_exp* exp, int tuple,
                             uint8_t l4protonum)
{
    struct nfnl_exp_dir* dir = EXP_GET_TUPLE(exp, tuple);
...
void nfnl_exp_set_ports(struct nfnl_exp* exp, int tuple, uint16_t srcport,
                        uint16_t dstport)
{
    struct nfnl_exp_dir* dir = EXP_GET_TUPLE(exp, tuple);
...
void nfnl_exp_set_icmp(struct nfnl_exp* exp, int tuple, uint16_t id,
