...
static void
    g_socket_address_connectable_iface_init(GSocketConnectableIface* iface);
static GSocketAddressEnumerator*
    g_socket_address_connectable_enumerate(GSocketConnectable* connectable);
static GSocketAddressEnumerator* g_socket_address_connectable_proxy_enumerate(
    GSocketConnectable* connectable);

G_DEFINE_ABSTRACT_TYPE_WITH_CODE(
    GSocketAddress, g_socket_address, G_TYPE_OBJECT,
...
static void g_socket_address_get_property(GObject* object, guint prop_id,
...
static void g_socket_address_class_init(GSocketAddressClass* klass)
{
    ...
    g_object_class_install_property(
        ...
        g_param_spec_enum("family", P_("Address family"),
                          P_("The family of the socket address"),
                          G_TYPE_SOCKET_FAMILY, G_SOCKET_FAMILY_INVALID,
                          G_PARAM_READABLE | G_PARAM_STATIC_STRINGS));
}

static void g_socket_address_connectable_iface_init(
    GSocketConnectableIface* connectable_iface)
{
    ...
    connectable_iface->proxy_enumerate =
        g_socket_address_connectable_proxy_enumerate;
...
GSocketAddress* g_socket_address_new_from_native(gpointer native, gsize len)
{
    ...
    if (family == AF_INET)
    {
        ...
        iaddr =
            g_inet_address_new_from_bytes((guint8*)&(addr->sin_addr), AF_INET);
    ...
    if (family == AF_INET6)
    {
        ...
        if (IN6_IS_ADDR_V4MAPPED(&(addr->sin6_addr)))
        {
            ...
            memcpy(&(sin_addr.sin_addr.s_addr), addr->sin6_addr.s6_addr + 12,
                   4);
            iaddr = g_inet_address_new_from_bytes((guint8*)&(sin_addr.sin_addr),
                                                  AF_INET);
        }
        else
        {
            iaddr = g_inet_address_new_from_bytes((guint8*)&(addr->sin6_addr),
                                                  AF_INET6);
        }

        sockaddr = g_object_new(G_TYPE_INET_SOCKET_ADDRESS, "address", iaddr,
                                "port", g_ntohs(addr->sin6_port), "flowinfo",
                                addr->sin6_flowinfo, "scope_id",
                                addr->sin6_scope_id, NULL);
        g_object_unref(iaddr);
        return sockaddr;
    }
    ...
    if (family == AF_UNIX)
    {
        ...
        if (path_len == 0)
        {
            return g_unix_socket_address_new_with_type(
                "", 0, G_UNIX_SOCKET_ADDRESS_ANONYMOUS);
        }
...
#define G_TYPE_SOCKET_ADDRESS_ADDRESS_ENUMERATOR                               \
    (_g_socket_address_address_enumerator_get_type())
#define G_SOCKET_ADDRESS_ADDRESS_ENUMERATOR(obj)                               \
    (G_TYPE_CHECK_INSTANCE_CAST((obj),                                         \
                                G_TYPE_SOCKET_ADDRESS_ADDRESS_ENUMERATOR,      \
...
G_DEFINE_TYPE(GSocketAddressAddressEnumerator,
              _g_socket_address_address_enumerator,
...
static void g_socket_address_address_enumerator_finalize(GObject* object)
{
    ...
    G_OBJECT_CLASS(_g_socket_address_address_enumerator_parent_class)
        ->finalize(object);
}

static GSocketAddress* g_socket_address_address_enumerator_next(
...
static void _g_socket_address_address_enumerator_init(
    GSocketAddressAddressEnumerator* enumerator)
{
}

static void _g_socket_address_address_enumerator_class_init(
    GSocketAddressAddressEnumeratorClass* sockaddrenum_class)
{
    ...
    g_socket_address_connectable_enumerate(GSocketConnectable* connectable)
{
    ...
    sockaddr_enum =
        g_object_new(G_TYPE_SOCKET_ADDRESS_ADDRESS_ENUMERATOR, NULL);
    ...
    GSocketConnectable* connectable)
{
        ...
        !G_IS_PROXY_ADDRESS(connectable))
    {
        ...
        addr_enum = g_object_new(G_TYPE_PROXY_ADDRESS_ENUMERATOR, "connectable",
