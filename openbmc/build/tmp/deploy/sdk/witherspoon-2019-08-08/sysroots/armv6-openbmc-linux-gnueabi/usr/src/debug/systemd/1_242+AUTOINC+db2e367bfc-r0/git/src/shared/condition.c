...
typedef enum
{
    /* Listed in order of checking. Note that some comparators are prefixes of
     * others, hence the longest should be listed first. */
...
static OrderOperator parse_order(const char** s)
{
    ...
    static const char* const prefix[_ORDER_MAX] = {
        [ORDER_LOWER_OR_EQUAL] = "<=", [ORDER_GREATER_OR_EQUAL] = ">=",
        [ORDER_LOWER] = "<",           [ORDER_GREATER] = ">",
        [ORDER_EQUAL] = "=",           [ORDER_UNEQUAL] = "!=",
    };
...
static int condition_test_kernel_version(Condition* c)
{
    ...
    if (order < 0)
        return fnmatch(skip_leading_chars(c->parameter, NULL), u.release, 0) ==
               0;
...
static int condition_test_memory(Condition* c)
{
    ...
    order = parse_order(&p);
    if (order < 0)
        order = ORDER_GREATER_OR_EQUAL; /* default to >= check, if nothing is
                                           specified. */
...
static int condition_test_virtualization(Condition* c)
{
    ...
    return v != VIRTUALIZATION_NONE &&
           streq(c->parameter, virtualization_to_string(v));
}
...
static int condition_test_needs_update(Condition* c)
{
    ...
    if (usr.st_mtim.tv_nsec > 0 && other.st_mtim.tv_nsec == 0)
    {
        ...
        {
            log_error_errno(
                r, "Failed to parse timestamp file '%s', using mtime: %m", p);
            return true;
        }
        else if (r == 0)
        {
        ...
        {
            log_error_errno(r,
                            "Failed to parse timestamp value '%s' in file "
                            "'%s', using mtime: %m",
                            timestamp_str, p);
            return true;
        }
...
static int condition_test_file_not_empty(Condition* c)
{
    ...
    return (stat(c->parameter, &st) >= 0 && S_ISREG(st.st_mode) &&
            st.st_size > 0);
}
...
bool condition_test_list(Condition* first,
                         ...
                         condition_test_logger_t logger, void* userdata)
{
    Condition* c;
    int triggered = -1;
        ...
        if (logger)
        {
            if (r < 0)
                logger(userdata, LOG_WARNING, r, __FILE__, __LINE__, __func__,
                       "Couldn't determine result for %s=%s%s%s, assuming "
                       "failed: %m",
                       to_string(c->type), c->trigger ? "|" : "",
                       c->negate ? "!" : "", c->parameter);
            else
                logger(userdata, LOG_DEBUG, 0, __FILE__, __LINE__, __func__,
                       "%s=%s%s%s %s.", to_string(c->type),
                       c->trigger ? "|" : "", c->negate ? "!" : "",
                       c->parameter, condition_result_to_string(c->result));
        }
