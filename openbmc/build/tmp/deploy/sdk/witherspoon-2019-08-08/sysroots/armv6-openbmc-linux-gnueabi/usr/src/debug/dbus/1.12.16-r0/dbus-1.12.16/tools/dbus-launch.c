...
static void save_machine_uuid(const char* uuid_arg)
{
    ...
    {
        fprintf(stderr,
                "machine ID '%s' looks like it's the wrong length, should be "
                "32 hex digits",
                uuid_arg);
        exit(1);
    }
...
static ReadStatus read_line(int fd, char* buf, size_t maxlen)
{
    ...
    if (retval == READ_STATUS_EOF && bytes > 0)
        retval = READ_STATUS_OK;
...
static void print_variables(const char* bus_address, pid_t bus_pid,
                            long bus_wid, int c_shell_syntax,
...
static void kill_bus_when_session_ends(void)
{
    ...
    if (tty_fd < 0 && x_fd < 0)
    {
        fprintf(stderr, "No terminal on standard input and no X display; "
        ...
        if (tty_fd >= 0)
        {
            if (FD_ISSET(tty_fd, &read_set))
            {
                ...
                else if (bytes_read < 0 && errno != EINTR)
                {
                     ...
                     */
                    fprintf(stderr,
                            "dbus-launch: error reading from stdin: %s\n",
                            strerror(errno));
                    kill_bus_and_exit(0);
                }
            }
...
_DBUS_GNUC_NORETURN static void
    babysit(int exit_with_session, pid_t child_pid,
            int read_bus_pid_fd) /* read pid from here */
{
    ...
    verbose("babysitting, exit_with_session = %d, child_pid = %ld, "
            "read_bus_pid_fd = %d\n",
    ...
    if (ret > 0)
    {
        ...
        if (do_waitpid(child_pid) < 0)
        {
            /* shouldn't happen */
            fprintf(stderr,
                    "Failed waitpid() waiting for bus daemon's parent\n");
            exit(1);
        }
    ...
    switch (read_pid(read_bus_pid_fd, &bus_pid_to_kill))
    {
        ...
        case READ_STATUS_ERROR:
            fprintf(stderr,
                    "Error in dbus-launch reading PID from bus daemon: %s\n",
                    strerror(errno));
            exit(1);
            break;
        default:
...
static void pass_info(const char* runprog, const char* bus_address,
                      ...
                      int bourne_shell_syntax, int binary_syntax, int argc,
                      char** argv, int remaining_args) _DBUS_GNUC_NORETURN;

static void pass_info(const char* runprog, const char* bus_address,
                      pid_t bus_pid, long bus_wid, int c_shell_syntax,
                      int bourne_shell_syntax, int binary_syntax, int argc,
                      char** argv, int remaining_args)
{
    char* envvar = NULL;
    char** args = NULL;
...
int main(int argc, char** argv)
{
    ...
    if (autolaunch)
    {
        ...
        {
            fprintf(stderr,
                    "Machine UUID not provided as arg to --autolaunch\n");
            exit(1);
        }
        ...
        if (address != NULL)
        {
            verbose("dbus-daemon is already running. Returning existing "
                    "parameters.\n");
    ...
    {
#ifndef DBUS_BUILD_X11
        fprintf(stderr, "Session lifetime based on X11 requested, but X11 "
        ...
        {
            fprintf(stderr,
                    "Session lifetime based on X11 requested, but machine UUID "
                    "unavailable: %s.\n",
                    error.message);
...
#define MAX_FD_LEN 64
        ...
        sprintf(write_pid_fd_as_string, "%d",
                bus_pid_to_launcher_pipe[WRITE_END]);

        sprintf(write_address_fd_as_string, "%d",
...
#ifdef DBUS_ENABLE_EMBEDDED_TESTS
        {
            ...
            if (test_daemon != NULL)
            {
                ...
                execl(test_daemon, test_daemon,
                      close_stderr ? "--syslog-only" : "--syslog", "--fork",
                      "--print-pid", write_pid_fd_as_string, "--print-address",
        ...
        execl(DBUS_DAEMONDIR "/dbus-daemon", DBUS_DAEMONDIR "/dbus-daemon",
              close_stderr ? "--syslog-only" : "--syslog", "--fork",
              "--print-pid", write_pid_fd_as_string, "--print-address",
              write_address_fd_as_string,
              config_file ? "--config-file" : "--session",
        ...
        fprintf(stderr,
                "Failed to execute message bus daemon %s: %s.  Will try again "
                "without full path.\n",
...
#define MAX_PID_LEN 64
        ...
        if (do_waitpid(ret) < 0)
        {
            fprintf(
                stderr,
                "Failed to waitpid() for babysitter intermediate process: %s\n",
                strerror(errno));
            exit(1);
        }
        ...
        switch (read_line(bus_address_to_launcher_pipe[READ_END], bus_address,
                          MAX_ADDR_LEN))
        {
            case READ_STATUS_OK:
                break;
            ...
            case READ_STATUS_ERROR:
                fprintf(stderr,
                        "Error in dbus-launch reading address from bus daemon: "
                        "%s\n",
                        strerror(errno));
                exit(1);
                break;
            ...
            if (autolaunch)
            {
                if (ret2 == 0)
                {
                    ...
                    if (x11_get_address(&address, &bus_pid, &wid) &&
                        address != NULL)
                    {
                        verbose("dbus-daemon is already running. Returning "
                                "existing parameters.\n");
                        /* Kill the old bus */
                        kill_bus();
                        pass_info(runprog, address, bus_pid, wid,
                                  c_shell_syntax, bourne_shell_syntax,
                                  binary_syntax, argc, argv, remaining_args);
                    }
                }
                if (ret2 < 0)
                {
