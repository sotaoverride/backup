...
static void gvdb_item_free(gpointer data)
{
    GvdbItem* item = data;
...
GHashTable* gvdb_hash_table_new(GHashTable* parent, const gchar* name_in_parent)
{
    GHashTable* table;

    table =
        g_hash_table_new_full(g_str_hash, g_str_equal, g_free, gvdb_item_free);
...
GvdbItem* gvdb_hash_table_insert(GHashTable* table, const gchar* key)
{
    GvdbItem* item;
...
void gvdb_hash_table_insert_string(GHashTable* table, const gchar* key,
                                   const gchar* value)
{
    GvdbItem* item;
...
void gvdb_item_set_value(GvdbItem* item, GVariant* value)
{
...
void gvdb_item_set_hash_table(GvdbItem* item, GHashTable* table)
{
    g_return_if_fail(!item->value && !item->table && !item->child);
...
void gvdb_item_set_parent(GvdbItem* item, GvdbItem* parent)
{
    GvdbItem** node;
...
static HashTable* hash_table_new(gint n_buckets)
{
    HashTable* table;
...
static void hash_table_insert(gpointer key, gpointer value, gpointer data)
{
...
static guint32_le item_to_index(GvdbItem* item)
{
    if (item != NULL)
...
static gpointer file_builder_allocate(FileBuilder* fb, guint alignment,
                                      gsize size, struct gvdb_pointer* pointer)
{
    FileChunk* chunk;
...
static void file_builder_add_value(FileBuilder* fb, GVariant* value,
...
static void file_builder_allocate_for_hash(
    FileBuilder* fb, gsize n_buckets, gsize n_items, guint bloom_shift,
    gsize n_bloom_words, guint32_le** bloom_filter, guint32_le** hash_buckets,
    struct gvdb_hash_item** hash_items, struct gvdb_pointer* pointer)
{
    ...
    size = sizeof bloom_hdr + sizeof table_hdr +
           n_bloom_words * sizeof(guint32_le) + n_buckets * sizeof(guint32_le) +
...
#define chunk(s) (size -= (s), data += (s), data - (s))
    ...
    *hash_items =
        (struct gvdb_hash_item*)chunk(n_items * sizeof(struct gvdb_hash_item));
    g_assert(size == 0);
#undef chunk
                                  ...
                                  struct gvdb_pointer* pointer)
{
    ...
    for (bucket = 0; bucket < mytable->n_buckets; bucket++)
    {
        ...
        for (item = mytable->buckets[bucket]; item; item = item->next)
        {
            ...
            file_builder_add_string(fb, basename, &entry->key_start,
                                    &entry->key_size);
...
static FileBuilder* file_builder_new(gboolean byteswap)
{
    FileBuilder* builder;
...
static GString* file_builder_serialise(FileBuilder* fb,
...
gboolean gvdb_table_write_contents(GHashTable* table, const gchar* filename,
                                   gboolean byteswap, GError** error)
{
    struct gvdb_pointer root;
    gboolean status;
    FileBuilder* fb;
