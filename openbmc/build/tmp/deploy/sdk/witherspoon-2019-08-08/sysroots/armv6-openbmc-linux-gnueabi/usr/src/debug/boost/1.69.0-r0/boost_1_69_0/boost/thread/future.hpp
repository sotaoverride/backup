...
struct shared_state_base : enable_shared_from_this<shared_state_base>
{
    ...
    // This type should be only included conditionally if interruptions are
    // allowed, but is included to maintain the same layout.
    ...
    // This declaration should be only included conditionally, but is included
    // to maintain the same layout.
    ...
    shared_state_base() :
        done(false), is_valid_(true), is_deferred_(false),
        is_constructed(false), policy_(launch::none), continuations(), ex_()
    {
    }

    shared_state_base(exceptional_ptr const& ex) :
        exception(ex.ptr_), done(true), is_valid_(true), is_deferred_(false),
        is_constructed(false), policy_(launch::none), continuations(), ex_()
    {
    }
    ...
    notify_when_ready_handle
        notify_when_ready(boost::condition_variable_any& cv)
    {
...
#define BOOST_THREAD_DO_CONTINUATION                                           \
    ...
    void wait_internal(boost::unique_lock<boost::mutex>& lk,
                       bool rethrow = true)
    {
        do_callback(lk);
        if (is_deferred_)
        ...
        waiters.wait(
            lk, boost::bind(&shared_state_base::is_done, boost::ref(*this)));
...
#endif
    void
        mark_exceptional_finish_internal(boost::exception_ptr const& e,
                                         boost::unique_lock<boost::mutex>& lock)
    {
        exception = e;
        mark_finished_internal(lock);
...
struct shared_state : detail::shared_state_base
{
    ...
    typedef
        typename conditional<boost::is_fundamental<T>::value, T, T const&>::type
    ...
    void mark_finished_with_result_internal(
        source_reference_type result_, boost::unique_lock<boost::mutex>& lock)
    {
#if defined BOOST_THREAD_FUTURE_USES_OPTIONAL
        result = result_;
#else
    ...
    void mark_finished_with_result_internal(
        rvalue_source_type result_, boost::unique_lock<boost::mutex>& lock)
    {
#if defined BOOST_THREAD_FUTURE_USES_OPTIONAL
    ...
    void mark_finished_with_result(rvalue_source_type result_)
    {
        ...
        mark_finished_with_result_internal(
            static_cast<rvalue_source_type>(result_), lock);
#endif
    }
    ...
    virtual shared_future_get_result_type
        get_sh(boost::unique_lock<boost::mutex>& lk)
    {
    ...
    void set_value_at_thread_exit(rvalue_source_type result_)
    {
        ...
        //#else
        //                result.reset(new
        //                T(static_cast<rvalue_source_type>(result_)));
        //#endif
        //#endif
        //              this->is_constructed = true;
        detail::make_ready_at_thread_exit(shared_from_this());
...
struct shared_state<void> : detail::shared_state_base
{
    ...
    shared_state(exceptional_ptr const& ex) : detail::shared_state_base(ex)
    {
    }
...
struct future_async_shared_state : future_async_shared_state_base<Rp>
{
    ...
    static void run(shared_ptr<future_async_shared_state> that,
...
struct future_async_shared_state<void, Fp>
  ...
  private:
    ...
    struct all_futures_lock
    {
            ...
            locks(new boost::unique_lock<boost::mutex>[count])
        {
            ...
            {
                locks[i] =
                    BOOST_THREAD_MAKE_RV_REF(boost::unique_lock<boost::mutex>(
                        futures[i].future_->mutex));
            }
        }
    ...
    void add(F& f)
    {
        if (f.future_)
        {
            registered_waiter waiter(
                f.future_, f.future_->notify_when_ready(cv), future_count);
            try
            {
...
typename boost::enable_if<is_future_type<F1>,
                          typename detail::future_waiter::count_type>::type
...
class basic_future : public base_future
{
  ...
  public:
    ...
    static // BOOST_CONSTEXPR
        future_ptr
        make_exceptional_future_ptr(exceptional_ptr const& ex)
    {
    ...
    basic_future(future_ptr a_future) : future_(a_future)
    {
    }
  ...
  public:
    ...
    basic_future& operator=(BOOST_THREAD_RV_REF(basic_future)
    ...
    exception_ptr get_exception_ptr()
    {
        return future_ ? future_->get_exception_ptr() : exception_ptr();
    }
    ...
    typedef detail::shared_state_base::notify_when_ready_handle
        notify_when_ready_handle;
    ...
    namespace detail
{
    ...
    BOOST_THREAD_FUTURE<Rp> make_future_async_shared_state(
        BOOST_THREAD_FWD_REF(Fp) f);

    template <class Rp, class Fp>
    BOOST_THREAD_FUTURE<Rp> make_future_deferred_shared_state(
        BOOST_THREAD_FWD_REF(Fp) f);
    ...
    BOOST_THREAD_FUTURE<Rp> make_future_executor_continuation_shared_state(
        Ex & ex, boost::unique_lock<boost::mutex> & lock,
        BOOST_THREAD_RV_REF(F) f, BOOST_THREAD_FWD_REF(Fp) c);

    template <typename Ex, typename F, typename Rp, typename Fp>
    BOOST_THREAD_FUTURE<Rp>
        make_shared_future_executor_continuation_shared_state(
            Ex & ex, boost::unique_lock<boost::mutex> & lock, F f,
            BOOST_THREAD_FWD_REF(Fp) c);

    template <class Rp, class Fp, class Executor>
    BOOST_THREAD_FUTURE<Rp> make_future_executor_shared_state(
        Executor & ex, BOOST_THREAD_FWD_REF(Fp) f);
#endif
#endif
...
BOOST_THREAD_FUTURE<
    csbl::tuple<typename decay<T0>::type, typename decay<T>::type...>>
...
class BOOST_THREAD_FUTURE : public detail::basic_future<R>
{
  private:
    ...
    friend BOOST_THREAD_FUTURE<Rp>
        detail::make_future_async_continuation_shared_state(
            boost::unique_lock<boost::mutex>& lock, BOOST_THREAD_RV_REF(F) f,
            BOOST_THREAD_FWD_REF(Fp) c);

    template <class F, class Rp, class Fp>
    friend BOOST_THREAD_FUTURE<Rp>
        detail::make_future_sync_continuation_shared_state(
            boost::unique_lock<boost::mutex>& lock, BOOST_THREAD_RV_REF(F) f,
            BOOST_THREAD_FWD_REF(Fp) c);

    template <class F, class Rp, class Fp>
    friend BOOST_THREAD_FUTURE<Rp>
        detail::make_future_deferred_continuation_shared_state(
            boost::unique_lock<boost::mutex>& lock, BOOST_THREAD_RV_REF(F) f,
            BOOST_THREAD_FWD_REF(Fp) c);

    template <typename F, typename Rp, typename Fp>
    friend BOOST_THREAD_FUTURE<Rp>
        detail::make_shared_future_deferred_continuation_shared_state(
            boost::unique_lock<boost::mutex>& lock, F f,
            BOOST_THREAD_FWD_REF(Fp) c);

    template <typename F, typename Rp, typename Fp>
    friend BOOST_THREAD_FUTURE<Rp>
        detail::make_shared_future_async_continuation_shared_state(
            boost::unique_lock<boost::mutex>& lock, F f,
            BOOST_THREAD_FWD_REF(Fp) c);

    template <typename F, typename Rp, typename Fp>
    friend BOOST_THREAD_FUTURE<Rp>
        detail::make_shared_future_sync_continuation_shared_state(
            boost::unique_lock<boost::mutex>& lock, F f,
            BOOST_THREAD_FWD_REF(Fp) c);
    ...
    template <typename Ex, typename F, typename Rp, typename Fp>
    friend BOOST_THREAD_FUTURE<Rp>
        detail::make_future_executor_continuation_shared_state(
            Ex& ex, boost::unique_lock<boost::mutex>& lock,
            BOOST_THREAD_RV_REF(F) f, BOOST_THREAD_FWD_REF(Fp) c);

    template <typename Ex, typename F, typename Rp, typename Fp>
    friend BOOST_THREAD_FUTURE<Rp>
        detail::make_shared_future_executor_continuation_shared_state(
            Ex& ex, boost::unique_lock<boost::mutex>& lock, F f,
            BOOST_THREAD_FWD_REF(Fp) c);

    template <class Rp, class Fp, class Executor>
    friend BOOST_THREAD_FUTURE<Rp>
        detail::make_future_executor_shared_state(Executor& ex,
                                                  BOOST_THREAD_FWD_REF(Fp) f);
#endif
#endif
#if defined BOOST_THREAD_PROVIDES_FUTURE_UNWRAP
    ...
    template <class F, class Rp>
    friend BOOST_THREAD_FUTURE<Rp> detail::make_future_unwrap_shared_state(
        boost::unique_lock<boost::mutex>& lock, BOOST_THREAD_RV_REF(F) f);
#endif
#if defined(BOOST_THREAD_PROVIDES_FUTURE_WHEN_ALL_WHEN_ANY)
    template <typename InputIterator>
    ...
    friend BOOST_THREAD_FUTURE<
        csbl::tuple<typename decay<T0>::type, typename decay<T>::type...>>
        when_all(BOOST_THREAD_FWD_REF(T0) f,
    ...
    friend BOOST_THREAD_FUTURE<
        csbl::tuple<typename decay<T0>::type, typename decay<T>::type...>>
        when_any(BOOST_THREAD_FWD_REF(T0) f,
                 BOOST_THREAD_FWD_REF(T)... futures);
#endif
#endif // BOOST_THREAD_PROVIDES_FUTURE_WHEN_ALL_WHEN_ANY
    ...
    BOOST_THREAD_FUTURE(future_ptr a_future) : base_type(a_future)
    {
    }
  ...
  public:
    ...
    // BOOST_CONSTEXPR
    BOOST_THREAD_FUTURE(exceptional_ptr const& ex) : base_type(ex)
    {
    }
    ...
    BOOST_NOEXCEPT : base_type(boost::move(
                         static_cast<base_type&>(BOOST_THREAD_RV(other))))
    {
    }
#if defined BOOST_THREAD_PROVIDES_FUTURE_UNWRAP
    inline explicit BOOST_THREAD_FUTURE(BOOST_THREAD_RV_REF(
    ...
    BOOST_THREAD_FUTURE& operator=(BOOST_THREAD_RV_REF(BOOST_THREAD_FUTURE)
                                       other) BOOST_NOEXCEPT
    {
        this->base_type::operator=(
            boost::move(static_cast<base_type&>(BOOST_THREAD_RV(other))));
        return *this;
    }
    ...
    // todo this function must be private and friendship provided to the
    // internal users.
    void set_async()
    {
    ...
    inline BOOST_THREAD_FUTURE<
        typename boost::result_of<F(BOOST_THREAD_FUTURE)>::type>
...
BOOST_THREAD_FUTURE<T> BOOST_THREAD_DCL_MOVABLE_END
    ...
    : public detail::basic_future<BOOST_THREAD_FUTURE<R2>>
{
...
class shared_future : public detail::basic_future<R>
{
    ...
    friend class packaged_task<R>;
#endif
    shared_future(future_ptr a_future) : base_type(a_future)
    {
    }
  ...
  public:
    ...
    shared_future(shared_future const& other) : base_type(other.future_)
    {
    }
    ...
    // BOOST_CONSTEXPR
    shared_future(exceptional_ptr const& ex) : base_type(ex)
    {
    }
    ~shared_future()
    {
    ...
    shared_future(BOOST_THREAD_RV_REF(BOOST_THREAD_FUTURE<R>)
                      other) BOOST_NOEXCEPT
        : base_type(
              boost::move(static_cast<base_type&>(BOOST_THREAD_RV(other))))
    {
    ...
    shared_future& operator=(BOOST_THREAD_RV_REF(shared_future)
                                 other) BOOST_NOEXCEPT
    {
        base_type::operator=(
            boost::move(static_cast<base_type&>(BOOST_THREAD_RV(other))));
    ...
    shared_future& operator=(BOOST_THREAD_RV_REF(BOOST_THREAD_FUTURE<R>)
                                 other) BOOST_NOEXCEPT
    {
        base_type::operator=(
            boost::move(static_cast<base_type&>(BOOST_THREAD_RV(other))));
    ...
    typename boost::disable_if<
        is_void<R2>,
        typename detail::shared_state<R>::shared_future_get_result_type>::type
    ...
    template <typename F>
    inline BOOST_THREAD_FUTURE<
        typename boost::result_of<F(shared_future)>::type>
    ...
    class promise
{
    ...
    typedef typename detail::shared_state<R>::source_reference_type
        source_reference_type;
    typedef
        typename detail::shared_state<R>::rvalue_source_type rvalue_source_type;
    typedef typename detail::shared_state<R>::move_dest_type move_dest_type;
    typedef typename detail::shared_state<R>::shared_future_get_result_type
...
#include <boost/thread/detail/atomic_undef_macros.hpp>
        ...
        {
            future_ptr blank;
            atomic_compare_exchange(&future_, &blank,
                                    future_ptr(new detail::shared_state<R>));
        }
#include <boost/thread/detail/atomic_redef_macros.hpp>
  ...
  public:
    ...
    {
        typedef
            typename Allocator::template rebind<detail::shared_state<R>>::other
                A2;
        A2 a2(a);
        typedef thread_detail::allocator_destructor<A2> D;

        future_ = future_ptr(::new (a2.allocate(1)) detail::shared_state<R>(),
                             D(a2, 1));
    ...
    typename boost::enable_if_c<
        is_copy_constructible<TR>::value && is_same<R, TR>::value, void>::type
    ...
    void set_value(rvalue_source_type r)
    {
        ...
        future_->mark_finished_with_result_internal(
            static_cast<rvalue_source_type>(r), lock);
#endif
    }
...
class promise<void>
{
    ...
    void lazy_init()
    {
        ...
        if (!atomic_load(&future_))
        {
            future_ptr blank;
            atomic_compare_exchange(&future_, &blank,
                                    future_ptr(new detail::shared_state<void>));
        }
#endif
    }
  ...
  public:
    ...
    promise(boost::allocator_arg_t, Allocator a)
    {
        typedef typename Allocator::template rebind<
            detail::shared_state<void>>::other A2;
        A2 a2(a);
        typedef thread_detail::allocator_destructor<A2> D;

        future_ = future_ptr(
            ::new (a2.allocate(1)) detail::shared_state<void>(), D(a2, 1));
        future_obtained = false;
    }
#endif
    promise() :
    ...
    promise(BOOST_THREAD_RV_REF(promise) rhs) BOOST_NOEXCEPT
        : future_(BOOST_THREAD_RV(rhs).future_),
          future_obtained(BOOST_THREAD_RV(rhs).future_obtained)
    {
        // we need to release the future as shared_ptr doesn't implements move
        // semantics
        BOOST_THREAD_RV(rhs).future_.reset();
        BOOST_THREAD_RV(rhs).future_obtained = false;
    }
    ...
    namespace detail
{
        ...
        detail::shared_state<R>
    {
        ...
        void reset()
        {
            // todo The packaged_task::reset must be as if an assignemnt froma
            // new packaged_task with the same function the reset function is an
            // optimization that avoids reallocating a new task.
            started = false;
            this->validate();
        }
#if defined BOOST_THREAD_PROVIDES_SIGNATURE_PACKAGED_TASK &&                   \
        ...
        void owner_destroyed()
        {
            ...
            if (!started)
            {
                ...
                this->mark_exceptional_finish_internal(
                    boost::copy_exception(boost::broken_promise()), lk);
            }
        }
    };
...
#endif
    {
      ...
      public:
        F f;
        task_shared_state(F const& f_) : f(f_)
        {
        }
        task_shared_state(BOOST_THREAD_RV_REF(F) f_) : f(boost::move(f_))
        {
        }
    ...
    defined(BOOST_THREAD_PROVIDES_VARIADIC_THREAD)
      ...
      public:
        CallableType f;
        task_shared_state(CallableType f_) : f(f_)
        {
        }
...
{
    typedef boost::shared_ptr<detail::task_base_shared_state<R(ArgTypes...)>>
        task_ptr;
...
class packaged_task
{
  ...
  public:
    ...
    explicit packaged_task(R (*f)(), BOOST_THREAD_FWD_REF(ArgTypes)... args)
    {
        ...
        typedef detail::task_shared_state<FR, R(ArgTypes...)>
    ...
    explicit packaged_task(
        ...
        typename boost::disable_if<
            is_same<typename decay<F>::type, packaged_task>, dummy*>::type = 0)
    {
...
#if defined BOOST_THREAD_PROVIDES_SIGNATURE_PACKAGED_TASK
#if defined(BOOST_THREAD_PROVIDES_VARIADIC_THREAD)
        typedef detail::task_shared_state<F, R(ArgTypes...)>
            task_shared_state_type;
#else
...
#endif
        typedef
            typename Allocator::template rebind<task_shared_state_type>::other
                A2;
        A2 a2(a);
        typedef thread_detail::allocator_destructor<A2> D;

        task = task_ptr(::new (a2.allocate(1)) task_shared_state_type(f),
                        D(a2, 1));
        future_obtained = false;
    }
                  ...
                  BOOST_THREAD_FWD_REF(F) f)
    {
        ...
        task = task_ptr(::new (a2.allocate(1))
                            task_shared_state_type(boost::forward<F>(f)),
                        D(a2, 1));
        future_obtained = false;
    }
    ...
    packaged_task(boost::allocator_arg_t, Allocator a, BOOST_THREAD_RV_REF(F) f)
    {
        ...
        task = task_ptr(::new (a2.allocate(1))
                            task_shared_state_type(boost::move(f)),
                        D(a2, 1));
        future_obtained = false;
    }
    ...
    packaged_task& operator=(BOOST_THREAD_RV_REF(packaged_task)
                                 other) BOOST_NOEXCEPT
    {
...
#else
        packaged_task temp(
            static_cast<BOOST_THREAD_RV_REF(packaged_task)>(other));
#endif
        swap(temp);
        return *this;
    }
    ...
    namespace detail
{
    ...
    BOOST_THREAD_FUTURE<Rp> make_future_async_shared_state(
        BOOST_THREAD_FWD_REF(Fp) f)
    {
        shared_ptr<future_async_shared_state<Rp, Fp>> h(
            new future_async_shared_state<Rp, Fp>());
...
BOOST_THREAD_FUTURE<R> async(launch policy,
                             R (*f)(BOOST_THREAD_FWD_REF(ArgTypes)...),
                             BOOST_THREAD_FWD_REF(ArgTypes)... args)
{
    typedef R (*F)(BOOST_THREAD_FWD_REF(ArgTypes)...);
    typedef detail::invoker<typename decay<F>::type,
    ...
    if (underlying_cast<int>(policy) & int(launch::async))
    {
        return BOOST_THREAD_MAKE_RV_REF(
            boost::detail::make_future_async_shared_state<Rp>(BF(
                f,
    ...
    {
        return BOOST_THREAD_MAKE_RV_REF(
            boost::detail::make_future_deferred_shared_state<Rp>(BF(
                f,
                thread_detail::decay_copy(boost::forward<ArgTypes>(args))...)));
    }
    else
...
BOOST_THREAD_FUTURE<typename boost::result_of<
    typename decay<F>::type(typename decay<ArgTypes>::type...)>::type>
...
struct shared_state_nullary_task
{
  ...
  public:
    ...
    shared_state_nullary_task(
        shared_state_nullary_task const& x) // BOOST_NOEXCEPT
        :
        that(x.that),
        f_(x.f_)
    {
    ...
    shared_state_nullary_task&
        operator=(BOOST_THREAD_COPY_ASSIGN_REF(shared_state_nullary_task)
                      x) // BOOST_NOEXCEPT
    {
        if (this != &x)
        {
    ...
    // move
    shared_state_nullary_task(BOOST_THREAD_RV_REF(shared_state_nullary_task)
                                  x) // BOOST_NOEXCEPT
        :
        that(x.that),
        f_(boost::move(x.f_))
    ...
    shared_state_nullary_task&
        operator=(BOOST_THREAD_RV_REF(shared_state_nullary_task)
                      x) // BOOST_NOEXCEPT
    {
        if (this != &x)
        {
    ...
    void operator()()
    {
        shared_ptr<shared_state<Rp>> that_ =
            static_pointer_cast<shared_state<Rp>>(that);
        try
        {
...
struct shared_state_nullary_task<void, Fp>
{
  ...
  public:
    ...
    void operator()()
    {
        shared_ptr<shared_state<void>> that_ =
            static_pointer_cast<shared_state<void>>(that);
        try
        {
            f_();
...
detail::shared_state_nullary_task<R, F> BOOST_THREAD_DCL_MOVABLE_END
    namespace detail
{
      ...
      public:
        ...
        void init(Executor& ex, BOOST_THREAD_FWD_REF(Fp) f)
        {
            ...
            this->set_executor_policy(
                executor_ptr_type(new executor_ref<Executor>(ex)));
            shared_state_nullary_task<Rp, Cont> t(this->shared_from_this(),
                                                  boost::forward<Fp>(f));
    ...
    BOOST_THREAD_FUTURE<Rp> make_future_executor_shared_state(
        Executor & ex, BOOST_THREAD_FWD_REF(Fp) f)
    {
        shared_ptr<future_executor_shared_state<Rp>> h(
            new future_executor_shared_state<Rp>());
...
BOOST_THREAD_FUTURE<R> async(Executor& ex,
                             R (*f)(BOOST_THREAD_FWD_REF(ArgTypes)...),
                             BOOST_THREAD_FWD_REF(ArgTypes)... args)
{
    typedef R (*F)(BOOST_THREAD_FWD_REF(ArgTypes)...);
    ...
    return BOOST_THREAD_MAKE_RV_REF(
        boost::detail::make_future_executor_shared_state<Rp>(
            ex, BF(f, thread_detail::decay_copy(
...
BOOST_THREAD_FUTURE<R> async(Executor& ex, R (*f)())
{
    ...
    return BOOST_THREAD_MAKE_RV_REF(
        boost::detail::make_future_executor_shared_state<Rp>(ex, BF(f)));
}

template <class Executor, class R, class A1>
BOOST_THREAD_FUTURE<R> async(Executor& ex, R (*f)(BOOST_THREAD_FWD_REF(A1)),
                             BOOST_THREAD_FWD_REF(A1) a1)
{
    ...
    return BOOST_THREAD_MAKE_RV_REF(
        boost::detail::make_future_executor_shared_state<Rp>(
            ex, BF(f, thread_detail::decay_copy(boost::forward<A1>(a1)))));
}
#endif // defined BOOST_THREAD_RVALUE_REFERENCES_DONT_MATCH_FUNCTION_PTR
    ...
    async(Executor& ex, BOOST_THREAD_FWD_REF(F) f)
{
    ...
    return boost::detail::make_future_executor_shared_state<Rp>(
        ex, BF(thread_detail::decay_copy(boost::forward<F>(f))));
}

template <class Executor, class F, class A1>
BOOST_THREAD_FUTURE<typename boost::result_of<
    typename decay<F>::type(typename decay<A1>::type)>::type>
    async(Executor& ex, BOOST_THREAD_FWD_REF(F) f, BOOST_THREAD_FWD_REF(A1) a1)
{
    typedef detail::invoker<typename decay<F>::type, typename decay<A1>::type>
        BF;
    typedef typename BF::result_type Rp;
...
BOOST_THREAD_FUTURE<typename boost::result_of<typename decay<F>::type(
    typename decay<A1>::type, typename decay<A2>::type)>::type>
    async(Executor& ex, BOOST_THREAD_FWD_REF(F) f, BOOST_THREAD_FWD_REF(A1) a1,
          BOOST_THREAD_FWD_REF(A2) a2)
{
    typedef detail::invoker<typename decay<F>::type, typename decay<A1>::type,
                            typename decay<A2>::type>
        BF;
    typedef typename BF::result_type Rp;
...
#if defined(BOOST_THREAD_PROVIDES_VARIADIC_THREAD)
template <class R, class... ArgTypes>
BOOST_THREAD_FUTURE<R> async(R (*f)(BOOST_THREAD_FWD_REF(ArgTypes)...),
                             BOOST_THREAD_FWD_REF(ArgTypes)... args)
{
    return BOOST_THREAD_MAKE_RV_REF(
        async(launch(launch::any), f, boost::forward<ArgTypes>(args)...));
}
#else
template <class R>
...
{
    return BOOST_THREAD_MAKE_RV_REF(async(launch(launch::any),
                                          boost::forward<F>(f),
                                          boost::forward<ArgTypes>(args)...));
}
#else
...
{
    return BOOST_THREAD_MAKE_RV_REF(
        async(launch(launch::any), boost::forward<F>(f)));
}
#endif
...
////////////////////////////////
template <typename T>
BOOST_THREAD_FUTURE<typename decay<T>::type> make_future(BOOST_THREAD_FWD_REF(T)
                                                             value)
{
...
{
    typedef
        typename detail::deduced_type_impl<typename decay<T>::type>::type type;
};
...
template <class T>
#endif
BOOST_THREAD_FUTURE<typename detail::deduced_type<T>::type>
...
BOOST_THREAD_FUTURE<T>
    make_ready_future(BOOST_THREAD_FWD_REF(typename remove_reference<T>::type)
                          x)
{
    promise<T> p;
...
////////////////////////////////
template <typename T>
shared_future<typename decay<T>::type>
...
struct continuation_shared_state : ShSt
{
  ...
  public:
    continuation_shared_state(BOOST_THREAD_RV_REF(F) f,
                              BOOST_THREAD_FWD_REF(Fp) c) :
        parent(boost::move(f)),
    ...
    void call(boost::unique_lock<boost::mutex>& lck)
    {
        try
        {
            ...
            // neither continuation nor parent are protected by the lock -
            // call() must only be called once, and no one else must modify it.
    ...
    {
        continuation_shared_state* that =
            static_cast<continuation_shared_state*>(that_.get());
        that->call();
    }
        ...
        {
            {
                relocker relock(lck);
                // neither continuation nor parent are protected by the lock -
                // call() must only be called once, and no one else must modify
                // it.
                this->continuation(boost::move(this->parent));
    ...
    : continuation_shared_state<F, Rp, Fp, future_async_shared_state_base<Rp>>
{
    typedef continuation_shared_state<F, Rp, Fp,
                                      future_async_shared_state_base<Rp>>
        base_type;

  public:
    future_async_continuation_shared_state(BOOST_THREAD_RV_REF(F) f,
                                           BOOST_THREAD_FWD_REF(Fp) c) :
    ...
    void launch_continuation()
    {
        ...
        boost::lock_guard<boost::mutex> lk(this->mutex);
        this->thr_ = boost::thread(&future_async_continuation_shared_state::run,
...
struct future_sync_continuation_shared_state
    : continuation_shared_state<F, Rp, Fp, shared_state<Rp>>
{
    typedef continuation_shared_state<F, Rp, Fp, shared_state<Rp>> base_type;

  public:
    future_sync_continuation_shared_state(BOOST_THREAD_RV_REF(F) f,
                                          BOOST_THREAD_FWD_REF(Fp) c) :
        base_type(boost::move(f), boost::forward<Fp>(c))
    {
    }
...
struct run_it
{
    ...
    // move
    run_it(BOOST_THREAD_RV_REF(run_it) x) BOOST_NOEXCEPT : that_(x.that_)
    {
        x.that_.reset();
    }
    ...
    namespace detail
{
    ...
    struct future_executor_continuation_shared_state
        : continuation_shared_state<F, Rp, Fp>
    {
        typedef continuation_shared_state<F, Rp, Fp> base_type;

      public:
        future_executor_continuation_shared_state(BOOST_THREAD_RV_REF(F) f,
                                                  BOOST_THREAD_FWD_REF(Fp) c) :
            base_type(boost::move(f), boost::forward<Fp>(c))
        {
        }
        ...
        {
            this->set_executor_policy(
                executor_ptr_type(new executor_ref<Ex>(ex)), lk);
                                    ...
                                    future_async_shared_state_base<Rp>>
    {
      ...
      public:
        shared_future_async_continuation_shared_state(F f,
                                                      BOOST_THREAD_FWD_REF(Fp)
                                                          c) :
            base_type(boost::move(f), boost::forward<Fp>(c))
        {
        }
    ...
    struct shared_future_sync_continuation_shared_state
        : continuation_shared_state<F, Rp, Fp, shared_state<Rp>>
    {
        typedef continuation_shared_state<F, Rp, Fp, shared_state<Rp>>
            base_type;

      public:
        shared_future_sync_continuation_shared_state(F f,
                                                     BOOST_THREAD_FWD_REF(Fp)
                                                         c) :
            base_type(boost::move(f), boost::forward<Fp>(c))
        {
        ...
        : continuation_shared_state<F, Rp, Fp>
    {
      ...
      public:
        future_deferred_continuation_shared_state(BOOST_THREAD_RV_REF(F) f,
                                                  BOOST_THREAD_FWD_REF(Fp) c) :
            base_type(boost::move(f), boost::forward<Fp>(c))
        {
            this->set_deferred();
        ...
        BOOST_THREAD_FWD_REF(Fp) c)
    {
        ...
        shared_ptr<future_deferred_continuation_shared_state<F, Rp, Cont>> h(
            new future_deferred_continuation_shared_state<F, Rp, Cont>(
        ...
        BOOST_THREAD_FWD_REF(Fp) c)
    {
        ...
        shared_ptr<future_async_continuation_shared_state<F, Rp, Cont>> h(
            new future_async_continuation_shared_state<F, Rp, Cont>(
                boost::move(f), boost::forward<Fp>(c)));
        h->init(lock);
        ...
        BOOST_THREAD_FWD_REF(Fp) c)
    {
        ...
        shared_ptr<future_sync_continuation_shared_state<F, Rp, Cont>> h(
            new future_sync_continuation_shared_state<F, Rp, Cont>(
                boost::move(f), boost::forward<Fp>(c)));
        h->init(lock);
        ...
        BOOST_THREAD_RV_REF(F) f, BOOST_THREAD_FWD_REF(Fp) c)
    {
        ...
        shared_ptr<future_executor_continuation_shared_state<F, Rp, Cont>> h(
            new future_executor_continuation_shared_state<F, Rp, Cont>(
                boost::move(f), boost::forward<Fp>(c)));
        h->init(lock, ex);
            ...
            BOOST_THREAD_FWD_REF(Fp) c)
    {
        typedef typename decay<Fp>::type Cont;
        shared_ptr<
            shared_future_deferred_continuation_shared_state<F, Rp, Cont>>
        ...
        BOOST_THREAD_FWD_REF(Fp) c)
    {
        ...
        shared_ptr<shared_future_async_continuation_shared_state<F, Rp, Cont>>
            h(new shared_future_async_continuation_shared_state<F, Rp, Cont>(
                f, boost::forward<Fp>(c)));
        h->init(lock);
        ...
        BOOST_THREAD_FWD_REF(Fp) c)
    {
        ...
        shared_ptr<shared_future_sync_continuation_shared_state<F, Rp, Cont>> h(
            new shared_future_sync_continuation_shared_state<F, Rp, Cont>(
                f, boost::forward<Fp>(c)));
        h->init(lock);
            ...
            BOOST_THREAD_FWD_REF(Fp) c)
    {
        typedef typename decay<Fp>::type Cont;
        shared_ptr<
            shared_future_executor_continuation_shared_state<F, Rp, Cont>>
            h(new shared_future_executor_continuation_shared_state<F, Rp, Cont>(
                f, boost::forward<Fp>(c)));
        h->init(lock, ex);
...
////////////////////////////////
// template<typename F>
// auto future<R>::then(launch policy, F&& func) ->
// BOOST_THREAD_FUTURE<decltype(func(*this))>;
////////////////////////////////
template <typename R>
template <typename F>
inline BOOST_THREAD_FUTURE<
...
{
    typedef
        typename boost::result_of<F(BOOST_THREAD_FUTURE<R>)>::type future_type;
    BOOST_THREAD_ASSERT_PRECONDITION(this->future_.get() != 0,
    ...
    if (underlying_cast<int>(policy) & int(launch::async))
    {
        return BOOST_THREAD_MAKE_RV_REF(
            (boost::detail::make_future_async_continuation_shared_state<
                BOOST_THREAD_FUTURE<R>, future_type>(lock, boost::move(*this),
    ...
    else if (underlying_cast<int>(policy) & int(launch::executor))
    {
        ...
        return BOOST_THREAD_MAKE_RV_REF(
            (boost::detail::make_future_executor_continuation_shared_state<
                ...
                ex, lock, boost::move(*this), boost::forward<F>(func))));
#endif
    }
...
////////////////////////////////
// template<typename Ex, typename F>
// auto future<future<R2> >::then(Ex&, F&& func) ->
// BOOST_THREAD_FUTURE<decltype(func(*this))>;
////////////////////////////////
template <typename R>
template <typename Ex, typename F>
...
////////////////////////////////
// template<typename F>
// auto future<future<R2> >::then(F&& func) ->
// BOOST_THREAD_FUTURE<decltype(func(*this))>;
////////////////////////////////
template <typename R>
template <typename F>
...
////////////////////////////////
// template<typename F>
// auto future<future<R2> >::then(launch, F&& func) ->
// BOOST_THREAD_FUTURE<decltype(func(*this))>;
////////////////////////////////
template <typename R2>
template <typename F>
inline BOOST_THREAD_FUTURE<typename boost::result_of<
    F(BOOST_THREAD_FUTURE<BOOST_THREAD_FUTURE<R2>>)>::type>
    BOOST_THREAD_FUTURE<BOOST_THREAD_FUTURE<R2>>::then(launch policy,
                                                       BOOST_THREAD_FWD_REF(F)
                                                           func)
{
    typedef BOOST_THREAD_FUTURE<R2> R;
    typedef
...
inline BOOST_THREAD_FUTURE<typename boost::result_of<
    F(BOOST_THREAD_FUTURE<BOOST_THREAD_FUTURE<R2>>)>::type>
    BOOST_THREAD_FUTURE<BOOST_THREAD_FUTURE<R2>>::then(Ex& ex,
                                                       BOOST_THREAD_FWD_REF(F)
                                                           func)
{
    typedef BOOST_THREAD_FUTURE<R2> R;
...
inline BOOST_THREAD_FUTURE<typename boost::result_of<
    F(BOOST_THREAD_FUTURE<BOOST_THREAD_FUTURE<R2>>)>::type>
    BOOST_THREAD_FUTURE<BOOST_THREAD_FUTURE<R2>>::then(BOOST_THREAD_FWD_REF(F)
                                                           func)
{
...
////////////////////////////////
// template<typename F>
// auto shared_future<R>::then(launch policy, F&& func) ->
// BOOST_THREAD_FUTURE<decltype(func(*this))>;
////////////////////////////////
template <typename R>
template <typename F>
    ...
    if (underlying_cast<int>(policy) & int(launch::async))
    {
        return BOOST_THREAD_MAKE_RV_REF(
            (boost::detail::make_shared_future_async_continuation_shared_state<
                shared_future<R>, future_type>(lock, *this,
                                               boost::forward<F>(func))));
    }
    else if (underlying_cast<int>(policy) & int(launch::deferred))
    {
        ...
        return BOOST_THREAD_MAKE_RV_REF(
            (boost::detail::
                 make_shared_future_executor_continuation_shared_state<
                     ...
                     ex, lock, *this, boost::forward<F>(func))));
#endif
    }
    else if (underlying_cast<int>(policy) & int(launch::inherit))
    {
...
////////////////////////////////
// template<typename Ex, typename F>
// auto shared_future<R>::then(Ex&, F&& func) ->
// BOOST_THREAD_FUTURE<decltype(func(*this))>;
////////////////////////////////
template <typename R>
template <typename Ex, typename F>
    ...
    shared_future<R>::then(Ex& ex, BOOST_THREAD_FWD_REF(F) func) const
{
    ...
    return BOOST_THREAD_MAKE_RV_REF(
        (boost::detail::make_shared_future_executor_continuation_shared_state<
            Ex, shared_future<R>, future_type>(ex, lock, *this,
                                               boost::forward<F>(func))));
}
#endif
...
////////////////////////////////
// template<typename F>
// auto shared_future<R>::then(F&& func) ->
// BOOST_THREAD_FUTURE<decltype(func(*this))>;
////////////////////////////////
template <typename R>
template <typename F>
...
struct mfallbacker_to
{
    ...
    mfallbacker_to(BOOST_THREAD_RV_REF(T) v) : value_(boost::move(v))
    {
    }
...
struct cfallbacker_to
{
    T value_;
    typedef T result_type;
    cfallbacker_to(T const& v) : value_(v)
    {
    }
    ...
    void launch_continuation()
    {
        ...
        if (!unwrapped.valid())
        {
            ...
            else
            {
                ...
                {
                    lk.unlock();
                    boost::unique_lock<boost::mutex> lk2(
                        unwrapped.future_->mutex);
                    unwrapped.future_->set_continuation_ptr(
                        this->shared_from_this(), lk2);
                }
                else
                {
                    this->mark_exceptional_finish_internal(
                        boost::copy_exception(future_uninitialized()), lk);
                }
            }
        }
        else
        {
            ...
            {
                this->mark_exceptional_finish_internal(
                    unwrapped.get_exception_ptr(), lk);
            }
            else
            {
...
{
    shared_ptr<future_unwrap_shared_state<F, Rp>> h(
        new future_unwrap_shared_state<F, Rp>(boost::move(f)));
...
inline BOOST_THREAD_FUTURE<R>::BOOST_THREAD_FUTURE(
    BOOST_THREAD_RV_REF(BOOST_THREAD_FUTURE<BOOST_THREAD_FUTURE<R>>) other) :
...
BOOST_THREAD_FUTURE<R2> BOOST_THREAD_FUTURE<BOOST_THREAD_FUTURE<R2>>::unwrap()
{
    ...
    return boost::detail::make_future_unwrap_shared_state<
        BOOST_THREAD_FUTURE<BOOST_THREAD_FUTURE<R2>>, R2>(lock,
                                                          boost::move(*this));
}
#endif
...
struct future_when_all_vector_shared_state
    : future_async_shared_state_base<csbl::vector<F>>
{
    ...
    bool run_deferred()
    {
        ...
        bool res = false;
        for (typename csbl::vector<F>::iterator it = vec_.begin();
             it != vec_.end(); ++it)
        {
...
#ifdef BOOST_THREAD_FUTURE_BLOCKING
        this->thr_ = boost::thread(&future_when_all_vector_shared_state::run,
  ...
  public:
    template <typename InputIterator>
    future_when_all_vector_shared_state(input_iterator_tag, InputIterator first,
                                        InputIterator last) :
    ...
    future_when_all_vector_shared_state(vector_tag,
                                        BOOST_THREAD_RV_REF(csbl::vector<F>)
                                            v) :
        vec_(boost::move(v))
    {
    }
...
#if !defined(BOOST_NO_CXX11_VARIADIC_TEMPLATES)
    template <typename T0, typename... T>
    future_when_all_vector_shared_state(values_tag, BOOST_THREAD_FWD_REF(T0) f,
                                        BOOST_THREAD_FWD_REF(T)... futures)
    {
    ...
    : future_async_shared_state_base<csbl::vector<F>>
{
  ...
  public:
    template <typename InputIterator>
    future_when_any_vector_shared_state(input_iterator_tag, InputIterator first,
                                        InputIterator last) :
        vec_(std::make_move_iterator(first), std::make_move_iterator(last))
    {
    }
...
#if !defined(BOOST_NO_CXX11_VARIADIC_TEMPLATES)
    template <typename T0, typename... T>
    future_when_any_vector_shared_state(values_tag, BOOST_THREAD_FWD_REF(T0) f,
                                        BOOST_THREAD_FWD_REF(T)... futures)
    {
        vec_.push_back(boost::forward<T0>(f));
        typename alias_t<char[]>::type{
...
struct accumulate_run_if_is_deferred
{
    ...
    {
        return (!csbl::get<i - 1>(t).run_if_is_deferred()) ||
               accumulate_run_if_is_deferred<Tuple, i - 1>()(t);
    }
};
...
struct future_when_all_tuple_shared_state
    : future_async_shared_state_base<Tuple>
{
    Tuple tup_;
    ...
    static void run(shared_ptr<boost::detail::shared_state_base> that_)
    {
        ...
        try
        {
            // TODO make use of apply(that->tup_,
            // boost::detail::wait_for_all_fctor());
  ...
  public:
    ...
    future_when_all_tuple_shared_state(values_tag, BOOST_THREAD_FWD_REF(F) f,
                                       BOOST_THREAD_FWD_REF(Fs)... futures) :
        tup_(boost::csbl::make_tuple(boost::forward<F>(f),
                                     boost::forward<Fs>(futures)...))
    {
    }
    ...
    : future_async_shared_state_base<Tuple>
{
  ...
  public:
    template <typename F, typename... Fs>
    future_when_any_tuple_shared_state(values_tag, BOOST_THREAD_FWD_REF(F) f,
                                       BOOST_THREAD_FWD_REF(Fs)... futures) :
        tup_(boost::csbl::make_tuple(boost::forward<F>(f),
                                     boost::forward<Fs>(futures)...))
    {
    ...
    when_all(InputIterator first, InputIterator last)
{
    ...
    typedef detail::future_when_all_vector_shared_state<value_type>
        factory_type;

    if (first == last)
        return make_ready_future(container_type());
    shared_ptr<factory_type> h(
        new factory_type(detail::input_iterator_tag_value, first, last));
    h->init();
...
BOOST_THREAD_FUTURE<
    ...
    when_all(BOOST_THREAD_FWD_REF(T0) f, BOOST_THREAD_FWD_REF(T)... futures)
{
    typedef csbl::tuple<typename decay<T0>::type, typename decay<T>::type...>
        container_type;
    typedef detail::future_when_all_tuple_shared_state<
        container_type, typename decay<T0>::type, typename decay<T>::type...>
        factory_type;

    shared_ptr<factory_type> h(new factory_type(detail::values_tag_value,
                                                boost::forward<T0>(f),
    ...
    when_any(InputIterator first, InputIterator last)
{
    typedef typename InputIterator::value_type value_type;
    typedef csbl::vector<value_type> container_type;
    typedef detail::future_when_any_vector_shared_state<value_type>
        factory_type;
    ...
    when_any(BOOST_THREAD_FWD_REF(T0) f, BOOST_THREAD_FWD_REF(T)... futures)
{
    typedef csbl::tuple<typename decay<T0>::type, typename decay<T>::type...>
        container_type;
    typedef detail::future_when_any_tuple_shared_state<
        container_type, typename decay<T0>::type, typename decay<T>::type...>
        factory_type;
