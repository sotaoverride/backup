 ...
 *
 * This is a simple and low-level API to working with mount options that are
 * stored in a string.
 */
#include <ctype.h>
...
/*
 * Parses the first option from @optstr. The @optstr pointer is set to the
 * beginning of the next option.
 *
...
static int __mnt_optstr_append_option(char** optstr, const char* name,
                                      size_t nsz, const char* value, size_t vsz)
{
    char* p;
    size_t sz, osz;
 ...
 * @value: value
 *
 * Returns: 0 on success or <0 in case of error. After an error the @optstr
 ...
 * should be unmodified.
 */
int mnt_optstr_prepend_option(char** optstr, const char* name,
                              const char* value)
{
    int rc = 0;
    char* tmp;
    ...
    *optstr = tmp;
    ...
    DBG(OPTIONS,
        ul_debug("failed to prepend '%s[=%s]' to '%s'", name, value, *optstr));
    return rc;
}
 ...
 * @value: returns a pointer to the beginning of the value (e.g. name=VALUE) or
 * NULL
 ...
 * of error.
 */
int mnt_optstr_get_option(const char* optstr, const char* name, char** value,
                          size_t* valsz)
{
    struct libmnt_optloc ol;
    int rc;
    ...
    while (!mnt_optstr_next_option(&str, &name, &namesz, &val, &valsz))
    {
        ...
        if (ent && m && m == maps[0] && vfs)
        {
            if (ignore_vfs && (ent->mask & ignore_vfs))
                continue;
            rc = __mnt_optstr_append_option(vfs, name, namesz, val, valsz);
        }
        else if (ent && m && m == maps[1] && user)
        {
            if (ignore_user && (ent->mask & ignore_user))
                continue;
            rc = __mnt_optstr_append_option(user, name, namesz, val, valsz);
        }
        else if (!m && fs)
            rc = __mnt_optstr_append_option(fs, name, namesz, val, valsz);
        if (rc)
        {
            if (vfs)
            {
                           ...
                           const struct libmnt_optmap* map)
{
    ...
    if (map == mnt_get_builtin_optmap(MNT_LINUX_MAP))
    {
        ...
        if (next && (!strncmp(next, "rw", 2) || !strncmp(next, "ro", 2)) &&
            (*(next + 2) == '\0' || *(next + 2) == ','))
        {
    ...
    if (next && *next)
    {
        ...
        while (!mnt_optstr_next_option(&next, &name, &namesz, &val, &valsz))
        {
            ...
            if (mnt_optmap_get_entry(maps, 1, name, namesz, &ent))
            {
                ...
                if (ent->id == MS_RDONLY || (ent->mask & MNT_INVERT) ||
    ...
    if (fl && fl != MS_REC)
    {
        ...
        {
            if ((ent->mask & MNT_INVERT) || ent->id == 0 ||
                (fl & ent->id) != (unsigned long)ent->id)
                continue;
...
#else
int mnt_optstr_fix_secontext(char** optstr, char* value, size_t valsz,
                             char** next)
{
    int rc = 0;
    ...
    DBG(CXT, ul_debug("SELinux context '%s' translated to '%s'", p,
                      rc == -1 ? "FAILED" : (char*)raw));
...
static int set_uint_value(char** optstr, unsigned int num, char* begin,
                          char* end, char** next)
{
    char buf[40];
 ...
 *
 * The "no" prefix interpretation could be disabled by the "+" prefix, for
 * example
...
int mnt_match_options(const char* optstr, const char* pattern)
{
     ...
     */
    while (match &&
           !mnt_optstr_next_option(&pat, &name, &namesz, &patval, &patvalsz))
    {
        char* val;
        size_t sz;
        int no = 0, rc;
...
static int test_flags(struct libmnt_test* ts, int argc, char* argv[])
{
    ...
    rc = mnt_optstr_get_flags(optstr, &fl,
                              mnt_get_builtin_optmap(MNT_LINUX_MAP));
    if (rc)
        return rc;
