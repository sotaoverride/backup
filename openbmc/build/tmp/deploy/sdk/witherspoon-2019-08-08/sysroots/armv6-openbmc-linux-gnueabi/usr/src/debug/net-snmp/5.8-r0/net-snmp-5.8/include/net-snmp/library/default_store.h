...
#define NETSNMP_DS_LIB_PRINT_NUMERIC_OIDS 9 /* print only numeric enum values  \
                                             */
#define NETSNMP_DS_LIB_DONT_BREAKDOWN_OIDS                                     \
    10 /* dont print oid indexes specially */
...
#define NETSNMP_DS_LIB_DONT_CHECK_RANGE                                        \
    16 /* don't check values for ranges on send */
#define NETSNMP_DS_LIB_NO_TOKEN_WARNINGS                                       \
    17 /* no warn about unknown config tokens */
...
#define NETSNMP_DS_LIB_REVERSE_ENCODE 20 /* encode packets from back to front  \
                                          */
#define NETSNMP_DS_LIB_PRINT_BARE_VALUE                                        \
    21 /* just print value (not OID = value) */
#define NETSNMP_DS_LIB_EXTENDED_INDEX                                          \
    22 /* print extended index format [x1][x2] */
#define NETSNMP_DS_LIB_PRINT_HEX_TEXT                                          \
    23 /* print ASCII text along with hex strings */
#define NETSNMP_DS_LIB_PRINT_UCD_STYLE_OID                                     \
    24 /* print OID's using the UCD-style prefix suppression */
#define NETSNMP_DS_LIB_READ_UCD_STYLE_OID                                      \
    25 /* require top-level OIDs to be prefixed with a dot */
#define NETSNMP_DS_LIB_HAVE_READ_PREMIB_CONFIG                                 \
    26 /* have the pre-mib parsing config tokens been processed */
#define NETSNMP_DS_LIB_HAVE_READ_CONFIG                                        \
    27 /* have the config tokens been processed */
...
#define NETSNMP_DS_LIB_16BIT_IDS                                               \
    31 /* restrict requestIDs, etc to 16-bit values */
#define NETSNMP_DS_LIB_DONT_PERSIST_STATE                                      \
    32 /* don't load config and don't load/save persistent file */
#define NETSNMP_DS_LIB_2DIGIT_HEX_OUTPUT                                       \
    33 /* print a leading 0 on hex values <= 'f' */
#define NETSNMP_DS_LIB_IGNORE_NO_COMMUNITY                                     \
    34 /* don't complain if no community is specified in the command arguments \
        */
#define NETSNMP_DS_LIB_DISABLE_PERSISTENT_LOAD                                 \
    35 /* don't load persistent file */
#define NETSNMP_DS_LIB_DISABLE_PERSISTENT_SAVE                                 \
    36 /* don't save persistent file */
#define NETSNMP_DS_LIB_APPEND_LOGFILES                                         \
    37 /* append, don't overwrite, log files */
#define NETSNMP_DS_LIB_NO_DISCOVERY                                            \
    38 /* don't support RFC5343 contextEngineID discovery */
#define NETSNMP_DS_LIB_TSM_USE_PREFIX                                          \
    39 /* TSM's simple security name mapping */
...
#define NETSNMP_DS_LIB_CLIENT_ADDR_USES_PORT                                   \
    42 /* NETSNMP_DS_LIB_CLIENT_ADDR includes address and also port */
...
#define NETSNMP_RUNTIME_PROTOCOL_SKIP_V1(pc_ver)                               \
    ((pc_ver == SNMP_VERSION_1) &&                                             \
     netsnmp_ds_get_boolean(NETSNMP_DS_LIBRARY_ID, NETSNMP_DS_LIB_DISABLE_V1))
...
#define NETSNMP_RUNTIME_PROTOCOL_CHECK_V1V2(pc_ver, pc_target)                 \
    do                                                                         \
    {                                                                          \
...
#define NETSNMP_RUNTIME_PROTOCOL_CHECK_V1V2(ver, gt)                           \
    do                                                                         \
    {                                                                          \
        ;                                                                      \
    } while (0)
...
#define NETSNMP_RUNTIME_PROTOCOL_SKIP_V3(pc_ver)                               \
    ((pc_ver == SNMP_VERSION_3) &&                                             \
     netsnmp_ds_get_boolean(NETSNMP_DS_LIBRARY_ID, NETSNMP_DS_LIB_DISABLE_V3))

#define NETSNMP_RUNTIME_PROTOCOL_CHECK_V3(pc_ver, pc_target)                   \
    do                                                                         \
    {                                                                          \
...
#define NETSNMP_RUNTIME_PROTOCOL_CHECK(pc_ver, pc_target)                      \
    do                                                                         \
    {                                                                          \
...
#define NETSNMP_RUNTIME_PROTOCOL_CHECK(ver, gt)                                \
    do                                                                         \
    {                                                                          \
        ;                                                                      \
    } while (0)
...
#define NETSNMP_RUNTIME_PROTOCOL_CHECK_V3(ver, gt)                             \
    do                                                                         \
    {                                                                          \
        ;                                                                      \
    } while (0)
