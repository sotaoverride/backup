...
static void setup_tdb(e2fsck_t ctx, ext2_ino_t num_dirs)
{
    ...
    profile_get_uint(ctx->profile, "scratch_files", "numdirs_threshold", 0, 0,
                     &threshold);
    profile_get_boolean(ctx->profile, "scratch_files", "dirinfo", 0, 1,
                        &enable);
...
static void setup_db(e2fsck_t ctx)
{
    ...
    db = (struct dir_info_db*)e2fsck_allocate_memory(
        ctx, sizeof(struct dir_info_db), "directory map db");
    db->count = db->size = 0;
    db->array = 0;
    ...
    db->size = num_dirs + 10;
    db->array = (struct dir_info*)e2fsck_allocate_memory(
        ctx, db->size * sizeof(struct dir_info), "directory map");
}
...
static struct dir_info* e2fsck_get_dir_info(e2fsck_t ctx, ext2_ino_t ino)
{
    ...
    {
#ifdef DIRINFO_DEBUG
        printf("(%d,%d,%d)\n", ino, ctx->dir_info->array[low].dotdot,
    ...
    {
#ifdef DIRINFO_DEBUG
        printf("(%d,%d,%d)\n", ino, ctx->dir_info->array[high].dotdot,
    ...
    while (low < high)
    {
        ...
        {
#ifdef DIRINFO_DEBUG
            printf("(%d,%d,%d)\n", ino, ctx->dir_info->array[mid].dotdot,
 ...
 */
int e2fsck_dir_info_set_parent(e2fsck_t ctx, ext2_ino_t ino, ext2_ino_t parent)
{
    struct dir_info* p;
 ...
 * dirinfo structure has already been created.
 */
int e2fsck_dir_info_set_dotdot(e2fsck_t ctx, ext2_ino_t ino, ext2_ino_t dotdot)
{
    struct dir_info* p;
 ...
 * dirinfo structure has already been created.
 */
int e2fsck_dir_info_get_parent(e2fsck_t ctx, ext2_ino_t ino, ext2_ino_t* parent)
{
    struct dir_info* p;
 ...
 * dirinfo structure has already been created.
 */
int e2fsck_dir_info_get_dotdot(e2fsck_t ctx, ext2_ino_t ino, ext2_ino_t* dotdot)
{
    struct dir_info* p;
