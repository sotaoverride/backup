...
int bus_init(Bus* bus, Log* log, const char* machine_id, unsigned int max_bytes,
             unsigned int max_fds, unsigned int max_matches,
...
void bus_get_monitor_destinations(Bus* bus, CList* destinations, Peer* sender,
                                  MessageMetadata* metadata)
{
    ...
    if (sender)
    {
        ...
        c_rbtree_for_each_entry(ownership, &sender->owned_names.ownership_tree,
                                owner_node)
        {
            if (!name_ownership_is_primary(ownership))
                continue;

            match_registry_get_monitors(&ownership->name->sender_matches,
                                        destinations, metadata);
        }

        match_registry_get_monitors(&sender->sender_matches, destinations,
                                    metadata);
    }
    else
    {
        /* sent from the driver */
        match_registry_get_monitors(&bus->sender_matches, destinations,
                                    metadata);
    }
}

void bus_get_broadcast_destinations(Bus* bus, CList* destinations,
                                    MatchRegistry* matches, Peer* sender,
                                    MessageMetadata* metadata)
{
    match_registry_get_subscribers(&bus->wildcard_matches, destinations,
                                   metadata);
    ...
    if (sender)
    {
        ...
        c_rbtree_for_each_entry(ownership, &owner->ownership_tree, owner_node)
        {
            ...
            match_registry_get_subscribers(&ownership->name->sender_matches,
                                           destinations, metadata);
        }
    }
    else
    {
        /* sent from the driver */
        match_registry_get_subscribers(&bus->sender_matches, destinations,
                                       metadata);
    }
}

void bus_log_append_sender(Bus* bus, uint64_t sender_id, NameSet* sender_names,
                           const char* sender_label)
{
    ...
    {
        log_appendf(log, "DBUS_BROKER_SENDER_UNIQUE_NAME=:1.%llu\n", sender_id);
    }
    ...
    if (sender_names)
    {
        if (sender_names->type == NAME_SET_TYPE_OWNER)
        {
            ...
            c_rbtree_for_each_entry(
                ownership, &sender_names->owner->ownership_tree, owner_node)
                log_appendf(log, "DBUS_BROKER_SENDER_WELL_KNOWN_NAME_%zu=%s\n",
...
void bus_log_append_receiver(Bus* bus, uint64_t receiver_id,
                             ...
                             const char* receiver_label)
{
    ...
    {
        log_appendf(log, "DBUS_BROKER_RECEIVER_UNIQUE_NAME=:1.%llu\n",
                    receiver_id);
    }
    ...
    if (receiver_names)
    {
        if (receiver_names->type == NAME_SET_TYPE_OWNER)
        {
            ...
            c_rbtree_for_each_entry(
                ownership, &receiver_names->owner->ownership_tree, owner_node)
                log_appendf(log,
        ...
        else if (receiver_names->type == NAME_SET_TYPE_SNAPSHOT)
        {
            for (size_t i = 0; i < receiver_names->snapshot->n_names; ++i)
                log_appendf(log,
                            "DBUS_BROKER_RECEIVER_WELL_KNOWN_NAME_%zu=%s\n", i,
                            receiver_names->snapshot->names[i]->name);
        }
    }
}

void bus_log_append_transaction(Bus* bus, uint64_t sender_id,
                                uint64_t receiver_id, NameSet* sender_names,
                                NameSet* receiver_names,
                                const char* sender_label,
                                const char* receiver_label, Message* message)
{
...
void bus_log_append_policy_send(Bus* bus, int policy_type, uint64_t sender_id,
                                uint64_t receiver_id, NameSet* sender_names,
                                NameSet* receiver_names,
                                const char* sender_label,
                                const char* receiver_label, Message* message)
{
    ...
    bus_log_append_transaction(bus, sender_id, receiver_id, sender_names,
                               receiver_names, sender_label, receiver_label,
                               message);
}

void bus_log_append_policy_receive(Bus* bus, uint64_t receiver_id,
                                   uint64_t sender_id, NameSet* sender_names,
                                   NameSet* receiver_names, Message* message)
{
    log_appendf(bus->log, "DBUS_BROKER_POLICY_TYPE=internal\n");
    log_appendf(bus->log, "DBUS_BROKER_TRANSMIT_ACTION=receive\n");
    bus_log_append_transaction(bus, sender_id, receiver_id, sender_names,
                               receiver_names, NULL, NULL, message);
}
