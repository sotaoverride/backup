...
static lzma_ret lzma2_decode(void* coder_ptr, lzma_dict* restrict dict,
    ...
    while (*in_pos < in_size || coder->sequence == SEQ_LZMA)
        switch (coder->sequence)
        {
            ...
            case SEQ_LZMA:
            {
                ...
                const lzma_ret ret = coder->lzma.code(coder->lzma.coder, dict,
                                                      in, in_pos, in_size);
                                   ...
                                   const void* opt, lzma_lz_options* lz_options)
{
    ...
    return lzma_lzma_decoder_create(&coder->lzma, allocator, options,
                                    lz_options);
}
...
extern lzma_ret lzma_lzma2_props_decode(void** options,
                                        const lzma_allocator* allocator,
