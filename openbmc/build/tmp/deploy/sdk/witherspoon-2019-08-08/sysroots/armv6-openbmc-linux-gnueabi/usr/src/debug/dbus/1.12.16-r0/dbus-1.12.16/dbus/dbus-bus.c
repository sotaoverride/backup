...
typedef struct
{
    ...
    unsigned int is_well_known : 1; /**< Is one of the well-known connections in
                                       our global array */
} BusData;
    ...
    while (i < N_BUS_TYPES)
    {
        if (bus_connections[i] != NULL)
            _dbus_warn_check_failed(
                "dbus_shutdown() called but connections were still live. This "
                "probably means the application did not drop all its "
...
static dbus_bool_t init_session_address(void)
{
    ...
    if (bus_connection_addresses[DBUS_BUS_SESSION] == NULL)
    {
         ...
         * method. On MacOS, this involves asking launchd.  On Windows (not
         * specified yet) we might do a COM lookup. Ignore errors - if we
         * failed, fall back to autolaunch. */
        retval = _dbus_lookup_session_address(&supported, &addr, &error);
        if (supported && retval)
        {
            retval = _dbus_string_steal_data(
                &addr, &bus_connection_addresses[DBUS_BUS_SESSION]);
        }
...
static dbus_bool_t init_connections_unlocked(void)
{
    if (!initialized)
    {
        ...
        if (!_dbus_register_shutdown_func(addresses_shutdown_func, NULL))
            return FALSE;
 ...
 */
void _dbus_bus_notify_shared_connection_disconnected_unlocked(
...
static DBusConnection* internal_bus_get(DBusBusType type, dbus_bool_t private,
                                        DBusError* error)
{
    const char* address;
    DBusConnection* connection;
 ...
 *
 * This function calls
 * dbus_connection_set_exit_on_disconnect() on the new connection, so the
 * application will exit if the connection closes. You can undo this by calling
 * dbus_connection_set_exit_on_disconnect() yourself after you get the
 * connection.
 *
...
dbus_bool_t dbus_bus_register(DBusConnection* connection, DBusError* error)
{
    ...
    if (reply == NULL)
        ...
        goto out;
    else if (!dbus_message_get_args(reply, error, DBUS_TYPE_STRING, &name,
                                    DBUS_TYPE_INVALID))
        goto out;
                                     ...
                                     const char* name, DBusError* error)
{
    ...
    _dbus_return_val_if_fail(_dbus_check_is_valid_bus_name(name),
                             DBUS_UID_UNSET);
    ...
    if (!dbus_message_get_args(reply, error, DBUS_TYPE_UINT32, &uid,
                               DBUS_TYPE_INVALID))
    {
        _DBUS_ASSERT_ERROR_IS_SET(error);
        dbus_message_unref(reply);
...
char* dbus_bus_get_id(DBusConnection* connection, DBusError* error)
{
    ...
    v_STRING = NULL;
    if (!dbus_message_get_args(reply, error, DBUS_TYPE_STRING, &v_STRING,
                               DBUS_TYPE_INVALID))
    {
        _DBUS_ASSERT_ERROR_IS_SET(error);
        dbus_message_unref(reply);
 ...
 * name can take it away from you by specifying
                          ...
                          unsigned int flags, DBusError* error)
{
    ...
    if (!dbus_message_get_args(reply, error, DBUS_TYPE_UINT32, &result,
                               DBUS_TYPE_INVALID))
    {
        _DBUS_ASSERT_ERROR_IS_SET(error);
        dbus_message_unref(reply);
                          ...
                          DBusError* error)
{
    ...
    message = dbus_message_new_method_call(DBUS_SERVICE_DBUS, DBUS_PATH_DBUS,
                                           DBUS_INTERFACE_DBUS, "ReleaseName");
                                    ...
                                    const char* name, DBusError* error)
{
    ...
    if (!dbus_message_get_args(reply, error, DBUS_TYPE_BOOLEAN, &exists,
                               DBUS_TYPE_INVALID))
    {
        _DBUS_ASSERT_ERROR_IS_SET(error);
        dbus_message_unref(reply);
                                           ...
                                           DBusError* error)
{
    ...
    if (result != NULL && !dbus_message_get_args(reply, error, DBUS_TYPE_UINT32,
...
static void send_no_return_values(DBusConnection* connection, DBusMessage* msg,
                                  DBusError* error)
{
    if (error)
    {
