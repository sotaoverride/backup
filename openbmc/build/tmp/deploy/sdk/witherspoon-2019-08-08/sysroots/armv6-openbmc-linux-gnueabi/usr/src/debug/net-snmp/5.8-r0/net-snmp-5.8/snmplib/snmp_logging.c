                ...
                netsnmp_feature_child_of(logging_syslog, logging_outputs)
                    netsnmp_feature_child_of(logging_external, logging_all)
                        ...
                        netsnmp_feature_child_of(enable_stderrlog, logging_all)
                            ...
                            netsnmp_feature_child_of(logging_enable_calllog,
                                                     netsnmp_unused)
                                netsnmp_feature_child_of(
                                    logging_enable_loghandler, netsnmp_unused)
...
netsnmp_log_handler* netsnmp_register_stdio_loghandler(int is_stdout,
                                                       int priority,
                                                       int priority_max,
                                                       const char* tok);
#endif
#ifndef NETSNMP_FEATURE_REMOVE_LOGGING_FILE
netsnmp_log_handler* netsnmp_register_filelog_handler(const char* logfilename,
                                                      int priority,
                                                      int priority_max,
                                                      int dont_zero_log);
#endif
...
void init_snmp_logging(void)
{
    ...
    register_prenetsnmp_mib_handler("snmp", "logOption", parse_config_logOption,
                                    NULL, "string");
}
 ...
 */
static int decode_priority(char** optarg, int* pri_max)
{
...
#ifndef NETSNMP_FEATURE_REMOVE_LOGGING_SYSLOG
static int decode_facility(char* optarg)
{
    if (optarg == NULL)
        return -1;
...
int snmp_log_options(char* optarg, int argc, char* const* argv)
{
    ...
    switch (*cp)
    {
        ...
        case 'E':
            ...
            /* Fallthrough */
        case 'e':
            logh = netsnmp_register_stdio_loghandler(0, priority, pri_max,
                                                     "stderr");
            break;
        ...
        case 'O':
            ...
            /* Fallthrough */
        case 'o':
            logh = netsnmp_register_stdio_loghandler(1, priority, pri_max,
                                                     "stdout");
            break;
        ...
        case 'f':
            ...
            DEBUGMSGTL(("logging:options", "%d-%d: '%s'\n", priority, pri_max,
                        optarg));
            logh =
                netsnmp_register_filelog_handler(optarg, priority, pri_max, -1);
            break;
#endif /* NETSNMP_FEATURE_REMOVE_LOGGING_FILE */
        ...
        case 'n':
            ...
            while (NULL != logh_head)
                netsnmp_remove_loghandler(logh_head);
            logh =
                netsnmp_register_loghandler(NETSNMP_LOGHANDLER_NONE, priority);
            if (logh)
            {
                logh->pri_max = pri_max;
            }
...
static char* sprintf_stamp(time_t* now, char* sbuf)
{
    time_t Now;
    struct tm* tm;
    ...
    tm = localtime(now);
    sprintf(sbuf, "%.4d-%.2d-%.2d %.2d:%.2d:%.2d ", tm->tm_year + 1900,
...
void snmp_enable_syslog_ident(const char* ident, const int facility)
{
    ...
    if (eventlog_h == NULL)
    {
         ...
         */
        fprintf(stderr,
                "Could not open event log for %s. "
                "Last error: 0x%x\n",
                ident, GetLastError());
        enable = 0;
    }
    ...
    {
        logh =
            netsnmp_register_loghandler(NETSNMP_LOGHANDLER_SYSLOG, LOG_DEBUG);
        if (logh)
        {
            logh->magic = (void*)eventlog_h;
...
{
    netsnmp_log_handler* logh =
        netsnmp_register_loghandler(NETSNMP_LOGHANDLER_FILE, priority);
    if (NULL == logh)
        return NULL;
    logh->pri_max = priority_max;
...
void snmp_enable_filelog(const char* logfilename, int dont_zero_log)
{
    ...
    if (logfilename)
    {
        ...
        if (!logh)
            logh = netsnmp_register_filelog_handler(logfilename, LOG_DEBUG, 0,
                                                    dont_zero_log);
        else
            netsnmp_enable_filelog(logh, dont_zero_log);
    }
...
/* used in the perl modules and
 * ip-mib/ipv4InterfaceTable/ipv4InterfaceTable_subagent.c */
void snmp_enable_stderrlog(void)
{
    ...
    {
        logh =
            netsnmp_register_loghandler(NETSNMP_LOGHANDLER_STDERR, LOG_DEBUG);
        if (logh)
...
#ifndef NETSNMP_FEATURE_REMOVE_LOGGING_ENABLE_CALLLOG
void snmp_enable_calllog(void) /* XXX - or take a callback routine ??? */
{
    netsnmp_log_handler* logh;
    int found = 0;
...
void snmp_log_string(int priority, const char* str)
{
    ...
    static netsnmp_log_handler lh = {
        1, 0, 0, 0, "stderr", log_handler_stdouterr, 0, NULL, NULL, NULL};
#endif /* NETSNMP_FEATURE_REMOVE_LOGGING_STDIO */
    netsnmp_log_handler* logh;
