...
int pkcs8_main(int argc, char** argv)
{
  ...
  private
        ...
        else
        {
            ...
            if (cipher)
            {
                ...
                if (scrypt_N && scrypt_r && scrypt_p)
                    pbe = PKCS5_pbe2_set_scrypt(cipher, NULL, 0, NULL, scrypt_N,
                                                scrypt_r, scrypt_p);
                else
#endif
                    pbe =
                        PKCS5_pbe2_set_iv(cipher, iter, NULL, 0, NULL, pbe_nid);
            }
            else
            {
    ...
    {
        if (traditional)
            PEM_write_bio_PrivateKey_traditional(out, pkey, NULL, NULL, 0, NULL,
                                                 passout);
        else
