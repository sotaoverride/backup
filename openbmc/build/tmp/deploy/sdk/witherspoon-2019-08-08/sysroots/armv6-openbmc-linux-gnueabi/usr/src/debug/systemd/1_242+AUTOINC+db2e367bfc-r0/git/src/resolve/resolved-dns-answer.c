                   ...
                   DnsAnswerFlags flags)
{
    ...
    for (i = 0; i < a->n_rrs; i++)
    {
        ...
        /* There's already an RR of the same RRset in place! Let's see if the
         * TTLs more or less match. We don't really care if they match
         * precisely, but we do care whether one is 0 and
...
int dns_answer_contains_zone_nsec3(DnsAnswer* answer, const char* zone)
{
    ...
    /* Checks whether the specified answer contains at least one NSEC3 RR in the
     * specified zone */
...
int dns_answer_find_soa(DnsAnswer* a, const DnsResourceKey* key,
                        DnsResourceRecord** ret, DnsAnswerFlags* flags)
{
...
int dns_answer_find_cname_or_dname(DnsAnswer* a, const DnsResourceKey* key,
...
int dns_answer_copy_by_key(DnsAnswer** a, DnsAnswer* source,
                           const DnsResourceKey* key, DnsAnswerFlags or_flags)
{
...
int dns_answer_move_by_key(DnsAnswer** to, DnsAnswer** from,
                           const DnsResourceKey* key, DnsAnswerFlags or_flags)
{
    int r;
        ...
        if (a->items[i].rr->key->class == DNS_CLASS_IN &&
            ((a->items[i].rr->key->type == DNS_TYPE_A &&
              in_addr_is_link_local(
                  AF_INET, (union in_addr_union*)&a->items[i].rr->a.in_addr) !=
                  prefer_link_local) ||
             (a->items[i].rr->key->type == DNS_TYPE_AAAA &&
              in_addr_is_link_local(
                  AF_INET6,
                  (union in_addr_union*)&a->items[i].rr->aaaa.in6_addr) !=
                  prefer_link_local)))
            /* Order address records that are not preferred to the end of the
             * array */
            items[end--] = a->items[i];
        else
...
int dns_answer_reserve(DnsAnswer** a, size_t n_free)
{
    ...
    if (*a)
    {
        ...
        n = realloc(*a,
                    offsetof(DnsAnswer, items) + sizeof(DnsAnswerItem) * ns);
        if (!n)
            return -ENOMEM;
...
void dns_answer_dump(DnsAnswer* answer, FILE* f)
{
    ...
    DNS_ANSWER_FOREACH_FULL(rr, ifindex, flags, answer)
    {
        ...
        if (ifindex != 0 ||
            flags & (DNS_ANSWER_AUTHENTICATED | DNS_ANSWER_CACHEABLE |
...
int dns_answer_has_dname_for_cname(DnsAnswer* a, DnsResourceRecord* cname)
{
    ...
    /* Checks whether the answer contains a DNAME record that indicates that the
     * specified CNAME record is synthesized from it */
    ...
    DNS_ANSWER_FOREACH(rr, a)
    {
        ...
        r = dns_name_change_suffix(cname->cname.name, rr->dname.name,
                                   dns_resource_key_name(rr->key), &n);
        if (r < 0)
            return r;
        if (r == 0)
            continue;
