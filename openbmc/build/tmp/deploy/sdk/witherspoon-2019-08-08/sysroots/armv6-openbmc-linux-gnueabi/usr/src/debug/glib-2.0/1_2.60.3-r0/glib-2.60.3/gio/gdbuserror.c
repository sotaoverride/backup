 ...
 *
 * Applications can associate a #GError error domain with a set of D-Bus errors
 * in order to automatically map from D-Bus errors to #GError and back. This is
 * typically done in the function returning the #GQuark for the error domain:
 * |[<!-- language="C" -->
 * // foo-bar-error.h:
 *
 ...
 *   {FOO_BAR_ERROR_SOME_THIRD_ERROR,
 * "org.project.Foo.Bar.Error.SomeThirdError"},
 * };
 *
 ...
 * g_dbus_error_register_error_domain() in advance (e.g. by invoking the
...
{
    G_STATIC_ASSERT(G_N_ELEMENTS(g_dbus_error_entries) - 1 ==
                    G_DBUS_ERROR_PROPERTY_READ_ONLY);
    static volatile gsize quark_volatile = 0;
    g_dbus_error_register_error_domain("g-dbus-error-quark", &quark_volatile,
 ...
 * @entries: (array length=num_entries): A pointer to @num_entries
                                        ...
                                        guint num_entries)
{
    ...
    if (g_once_init_enter(quark_volatile))
    {
        ...
        {
            g_warn_if_fail(g_dbus_error_register_error(
                quark, entries[n].error_code, entries[n].dbus_error_name));
        }
        g_once_init_leave(quark_volatile, quark);
    }
}

static gboolean _g_dbus_error_decode_gerror(const gchar* dbus_name,
...
static gboolean quark_code_pair_equal_func(const QuarkCodePair* a,
                                           const QuarkCodePair* b)
{
    return (a->error_domain == b->error_domain) &&
 ...
 * Since: 2.26
 */
gboolean g_dbus_error_register_error(GQuark error_domain, gint error_code,
                                     const gchar* dbus_error_name)
{
    ...
    if (quark_code_pair_to_re == NULL)
    {
        ...
        quark_code_pair_to_re =
            g_hash_table_new((GHashFunc)quark_code_pair_hash_func,
 ...
 * Since: 2.26
 */
gboolean g_dbus_error_unregister_error(GQuark error_domain, gint error_code,
                                       const gchar* dbus_error_name)
{
    gboolean ret;
    RegisteredError* re;
    ...
    if (re == NULL)
    {
        ...
        pair.error_domain = error_domain;
        pair.error_code = error_code;
        g_warn_if_fail(g_hash_table_lookup(quark_code_pair_to_re, &pair) ==
                       NULL); /* check invariant */
        goto out;
    }

    ret = TRUE;

    g_warn_if_fail(g_hash_table_lookup(quark_code_pair_to_re, &(re->pair)) ==
                   re); /* check invariant */

    g_warn_if_fail(g_hash_table_remove(quark_code_pair_to_re, &(re->pair)));
    g_warn_if_fail(
        g_hash_table_remove(dbus_error_name_to_re, re->dbus_error_name));

    /* destroy hashes if empty */
    hash_size = g_hash_table_size(dbus_error_name_to_re);
    if (hash_size == 0)
    {
        g_warn_if_fail(g_hash_table_size(quark_code_pair_to_re) ==
                       0); /* check invariant */
 ...
 * If so, use g_dbus_error_get_remote_error() to get the name of the error.
 *
...
gchar* g_dbus_error_get_remote_error(const GError* error)
{
    ...
    /* Ensure that e.g. G_DBUS_ERROR is registered using
     * g_dbus_error_register_error() */
    _g_dbus_initialize();
                                        ...
                                        const gchar* dbus_error_message)
{
    ...
    if (re != NULL)
    {
        error = g_error_new(re->pair.error_domain, re->pair.error_code,
                            "GDBus.Error:%s: %s", dbus_error_name,
                            dbus_error_message);
    }
    else
    {
        ...
        if (_g_dbus_error_decode_gerror(dbus_error_name, &error_domain,
                                        &error_code))
        {
            error = g_error_new(error_domain, error_code, "GDBus.Error:%s: %s",
                                dbus_error_name, dbus_error_message);
        }
        else
        {
            error = g_error_new(G_IO_ERROR, G_IO_ERROR_DBUS_ERROR,
                                "GDBus.Error:%s: %s", dbus_error_name,
                                dbus_error_message);
        }
    }
 ...
 * @dbus_error_name: D-Bus error name.
 * @dbus_error_message: D-Bus error message.
 * @format: (nullable): printf()-style format to prepend to @dbus_error_message
 * or %NULL.
 ...
 * Since: 2.26
 */
void g_dbus_error_set_dbus_error(GError** error, const gchar* dbus_error_name,
                                 const gchar* dbus_error_message,
                                 const gchar* format, ...)
{
    ...
    if (format == NULL)
    {
        *error = g_dbus_error_new_for_dbus_error(dbus_error_name,
                                                 dbus_error_message);
    }
    else
    {
