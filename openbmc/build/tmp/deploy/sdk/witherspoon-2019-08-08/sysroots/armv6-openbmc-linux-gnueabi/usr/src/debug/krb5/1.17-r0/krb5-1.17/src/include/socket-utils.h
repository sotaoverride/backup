 ...
 * Internet socket. */
static inline void sa_setport(struct sockaddr* sa, uint16_t port)
{
...
static inline uint16_t sa_getport(struct sockaddr* sa)
{
    if (sa->sa_family == AF_INET)
...
static inline int sa_is_inet(struct sockaddr* sa)
{
    return sa->sa_family == AF_INET || sa->sa_family == AF_INET6;
}

/* Return true if sa is an IPv4 or IPv6 wildcard address. */
static inline int sa_is_wildcard(struct sockaddr* sa)
{
 ...
 * neither. */
static inline socklen_t sa_socklen(struct sockaddr* sa)
{
    if (sa->sa_family == AF_INET6)
