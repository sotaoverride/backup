...
#define GSS_ERROR_STR(value, array, select, min, max, num)                     \
    (((select(value) < (min)) || (select(value) > (max)))                      \
         ? NULL                                                                \
         : _((array)[num(value)]))
...
#define GSS_ROUTINE_ERROR_STR(x)                                               \
    GSS_ERROR_STR((x), routine_error_string, GSS_ROUTINE_ERROR,                \
                  GSS_S_BAD_MECH, GSS_S_FAILURE, GSS_ROUTINE_ERROR_FIELD)
...
#define GSS_SINFO_STR(x)                                                       \
    ((((1 << (x)) < GSS_S_CONTINUE_NEEDED) ||                                  \
      ((1 << (x)) > GSS_S_UNSEQ_TOKEN))                                        \
         ? /**/ NULL                                                           \
         : sinfo_string[(x)])
...
static int display_unknown(const char* kind, OM_uint32 value,
                           gss_buffer_t buffer)
{
    char* str;
...
static OM_uint32 display_calling(OM_uint32* minor_status, OM_uint32 code,
...
static OM_uint32 display_routine(OM_uint32* minor_status, OM_uint32 code,
                                 gss_buffer_t status_string)
{
    const char* str;
...
static OM_uint32 display_bit(OM_uint32* minor_status, OM_uint32 code,
                             gss_buffer_t status_string)
{
    const char* str;
