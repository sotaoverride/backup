...
static gcry_err_code_t do_vsexp_sscan(gcry_sexp_t* retsexp, size_t* erroff,
                                      const char* buffer, size_t length,
                                      int argflag, void** arg_list,
                                      va_list arg_ptr);

static gcry_err_code_t do_sexp_sscan(gcry_sexp_t* retsexp, size_t* erroff,
                                     const char* buffer, size_t length,
                                     int argflag, void** arg_list, ...);
...
#undef whitespacep
static GPG_ERR_INLINE int whitespacep(const char* p)
{
    switch (*p)
    {
        case ' ':
...
gcry_err_code_t _gcry_sexp_create(gcry_sexp_t* retsexp, void* buffer,
...
gcry_err_code_t _gcry_sexp_new(gcry_sexp_t* retsexp, const void* buffer,
                               size_t length, int autodetect)
{
 ...
 */
gcry_sexp_t _gcry_sexp_find_token(const gcry_sexp_t list, const char* tok,
                                  size_t toklen)
{
    const byte* p;
    DATALEN n;
...
static const char* do_sexp_nth_data(const gcry_sexp_t list, int number,
                                    size_t* datalen)
{
    const byte* p;
    DATALEN n;
...
void* _gcry_sexp_nth_buffer(const gcry_sexp_t list, int number, size_t* rlength)
{
    const char* s;
    size_t n;
    char* buf;
...
static GPG_ERR_INLINE int hextonibble(int s)
{
    if (s >= '0' && s <= '9')
        return s - '0';
    else if (s >= 'A' && s <= 'F')
...
static size_t unquote_string(const char* string, size_t length,
                             unsigned char* buf)
{
    int esc = 0;
 ...
 */
static gpg_err_code_t do_vsexp_sscan(gcry_sexp_t* retsexp, size_t* erroff,
                                     const char* buffer, size_t length,
                                     int argflag, void** arg_list,
                                     va_list arg_ptr)
{
    ...
    for (p = buffer, n = length; n; p++, n--)
    {
        ...
        else if (percent)
        {
            ...
            else if (*p == 'b')
            {
                ...
                MAKE_SPACE(alen);
                if (alen && !_gcry_is_secure(c.sexp->d) &&
                    _gcry_is_secure(astr))
                {
                    ...
                    newsexp =
                        xtrymalloc_secure(sizeof *newsexp + c.allocated - 1);
                    if (!newsexp)
                    {
                        err = gpg_err_code_from_errno(errno);
                        goto leave;
...
static gpg_err_code_t do_sexp_sscan(gcry_sexp_t* retsexp, size_t* erroff,
                                    const char* buffer, size_t length,
                                    int argflag, void** arg_list, ...)
{
    gcry_err_code_t rc;
    ...
    rc = do_vsexp_sscan(retsexp, erroff, buffer, length, argflag, arg_list,
                        arg_ptr);
    va_end(arg_ptr);
                                ...
                                const char* format, ...)
{
    ...
    rc = do_vsexp_sscan(retsexp, erroff, format, strlen(format), 1, NULL,
                        arg_ptr);
    va_end(arg_ptr);
...
{
    return do_vsexp_sscan(retsexp, erroff, format, strlen(format), 1, NULL,
                          arg_ptr);
}

/* Like gcry_sexp_build, but uses an array instead of variable
   function arguments.  */
gcry_err_code_t _gcry_sexp_build_array(gcry_sexp_t* retsexp, size_t* erroff,
                                       const char* format, void** arg_list)
{
    return do_sexp_sscan(retsexp, erroff, format, strlen(format), 1, arg_list);
}

gcry_err_code_t _gcry_sexp_sscan(gcry_sexp_t* retsexp, size_t* erroff,
                                 const char* buffer, size_t length)
{
...
/* Figure out a suitable encoding for BUFFER of LENGTH.
   Returns: 0 = Binary
            ...
            2 = Token possible
*/
static int suitable_encoding(const unsigned char* buffer, size_t length)
{
    ...
    for (s = buffer; length; s++, length--)
    {
        if ((*s < 0x20 || (*s >= 0x7f && *s <= 0xa0)) &&
            !strchr("\b\t\v\n\f\r\"\'\\", *s))
            return 0; /*binary*/
        if (maybe_token && !alphap(s) && !digitp(s) &&
            !strchr(TOKEN_SPECIALS, *s))
            maybe_token = 0;
    }
    s = buffer;
 ...
 */
size_t _gcry_sexp_sprint(const gcry_sexp_t list, int mode, void* buffer,
                         size_t maxlength)
{
