...
static lzma_ret lzma2_encode(void* coder_ptr, lzma_mf* restrict mf,
    ...
    while (*out_pos < out_size)
        switch (coder->sequence)
        {
            ...
            case SEQ_LZMA_ENCODE:
            {
                ...
                // could accept.
                const uint32_t left =
                    LZMA2_UNCOMPRESSED_MAX - coder->uncompressed_size;
                uint32_t limit;
                ...
                else
                {
                    ...
                    limit = mf->read_pos - mf->read_ahead + left -
                            mf->match_len_max;
                }
                ...
                const lzma_ret ret = lzma_lzma_encode(
                    coder->lzma, mf, coder->buf + LZMA2_HEADER_MAX,
                    &coder->compressed_size, LZMA2_CHUNK_MAX, limit);

                coder->uncompressed_size +=
                    mf->read_pos - mf->read_ahead - read_start;
            ...
            case SEQ_LZMA_COPY:
                ...
                // output buffer.
                lzma_bufcpy(coder->buf, &coder->buf_pos, coder->compressed_size,
            ...
            case SEQ_UNCOMPRESSED_HEADER:
                ...
                lzma_bufcpy(coder->buf, &coder->buf_pos,
                            LZMA2_HEADER_UNCOMPRESSED, out, out_pos, out_size);
                                             ...
                                             const lzma_filter* filter)
{
        ...
        coder->opt_cur.pb != opt->pb)
    {
        ...
        if (opt->lc > LZMA_LCLP_MAX || opt->lp > LZMA_LCLP_MAX ||
            opt->lc + opt->lp > LZMA_LCLP_MAX || opt->pb > LZMA_PB_MAX)
                                   ...
                                   lzma_lz_options* lz_options)
{
    ...
    coder->need_properties = true;
    coder->need_state_reset = false;
    coder->need_dictionary_reset = coder->opt_cur.preset_dict == NULL ||
...
extern lzma_ret lzma_lzma2_encoder_init(lzma_next_coder* next,
                                        const lzma_allocator* allocator,
