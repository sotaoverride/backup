    ...
    while ((c = getopt_long(argc, argv, "-", options, NULL)) >= 0)
        switch (c)
        {
            ...
            case ARG_LOG_LEVEL:
                ...
                if (r < 0)
                    log_error_errno(
                        r, "Failed to parse log level %s, ignoring: %m",
                        optarg);
            ...
            case ARG_LOG_TARGET:
                ...
                if (r < 0)
                    log_error_errno(
                        r, "Failed to parse log target %s, ignoring: %m",
                        optarg);
            ...
            case ARG_LOG_COLOR:
                ...
                if (optarg)
                {
                    ...
                    if (r < 0)
                        log_error_errno(r,
                                        "Failed to parse log color setting %s, "
                                        "ignoring: %m",
                                        optarg);
                }
                else
            ...
            case ARG_LOG_LOCATION:
                if (optarg)
                {
                    ...
                    if (r < 0)
                        log_error_errno(r,
                                        "Failed to parse log location setting "
                                        "%s, ignoring: %m",
                                        optarg);
                }
                else
            ...
            case ARG_EXIT_CODE:
                ...
                if (r < 0)
                    log_error_errno(
                        r, "Failed to parse exit code %s, ignoring: %m",
                        optarg);
            ...
            case ARG_TIMEOUT:
                ...
                if (r < 0)
                    log_error_errno(
                        r, "Failed to parse shutdown timeout %s, ignoring: %m",
                        optarg);
...
static int switch_root_initramfs(void)
{
    ...
    if (mount(NULL, "/run/initramfs", NULL, MS_PRIVATE, NULL) < 0)
        return log_error_errno(
            errno, "Failed to make /run/initramfs private mount: %m");

    /* switch_root with MS_BIND, because there might still be processes lurking
     * around, which have open file descriptors. /run/initramfs/shutdown will
     * take care of these. Also do not detach the old root, because
     * /run/initramfs/shutdown needs to access it.
     */
...
static int sync_making_progress(unsigned long long* prev_dirty)
{
    ...
    for (;;)
    {
        ...
        if (!first_word(line, "NFS_Unstable:") &&
            !first_word(line, "Writeback:") && !first_word(line, "Dirty:"))
            continue;
...
static void sync_with_progress(void)
{
    ...
    /* Due to the possiblity of the sync operation hanging, we fork a child
     * process and monitor the progress. If the timeout lapses, the assumption
     * is that that particular sync stalled. */
    ...
    log_error("Syncing filesystems and block devices - timed out, issuing "
              "SIGKILL to PID " PID_FMT ".",
              pid);
...
int main(int argc, char* argv[])
{
    ...
    /* The log target defaults to console, but the original systemd process will
     * pass its log target in through a command line argument, which will
     * override this default. Also, ensure we'll never log to the journal or
     * syslog, as these logging daemons are either already dead or will die very
     * soon. */
    ...
    /* Synchronize everything that is not written to disk yet at this point
     * already. This is a good idea so that slow IO is processed here already
     * and the final process killing spree is not impacted by processes
     * desperately trying to sync IO to disk within their timeout. Do not remove
     * this sync, data corruption will result. */
    if (!in_container)
        sync_with_progress();
    ...
    for (;;)
    {
        ...
        if (!need_umount && !need_swapoff && !need_loop_detach &&
            !need_dm_detach)
        {
            log_info("All filesystems, swaps, loop devices and DM devices "
                     "detached.");
            /* Yay, done */
            break;
        }
    ...
    arguments[2] = NULL;
    (void)execute_directories(dirs, DEFAULT_TIMEOUT_USEC, NULL, NULL, arguments,
     ...
     * but the file systems need to be sync'ed explicitly in advance. So let's
     * do this here, but not needlessly slow down containers. Note that we
     * sync'ed things already once above, but we did some more work since then
     * which might have caused IO, hence let's do it once more. Do not remove
     * this sync, data corruption will result. */
    if (!in_container)
        sync_with_progress();
    ...
    if (streq(arg_verb, "exit"))
    {
        ...
        cmd = RB_POWER_OFF; /* We cannot exit() on the host, fallback on another
                               method. */
    }
    ...
    switch (cmd)
    {
        ...
        case LINUX_REBOOT_CMD_KEXEC:
            ...
            if (!in_container)
            {
                ...
                /* If we are still running, then the kexec can't have worked,
                 * let's fall through */
            }
