...
struct DBusTypeReader
{
    ...
    dbus_uint32_t finished : 1; /**< marks we're at end iterator for cases
                                 ...
                                 */
    dbus_uint32_t
        array_len_offset : 3;    /**< bytes back from start_pos that len ends */
...
struct DBusTypeWriter
{
    ...
    dbus_uint32_t container_type : 8; /**< what are we inside? (e.g. struct,
                                         variant, array) */

    dbus_uint32_t
        type_pos_is_expectation : 1; /**< type_pos can be either an insertion
                                        point for or an expected next type */

    dbus_uint32_t enabled : 1; /**< whether to write values */

    DBusString*
        type_str; /**< where to write typecodes (or read type expectations) */
    ...
    union
    {
        struct
        {
            ...
            int element_type_pos; /**< position of array element type in
                                     type_str */
        } array;
    } u; /**< class-specific data */
};
