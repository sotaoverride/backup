...
assert_cc(offsetof(struct boot_option, title) ==
          offsetof(struct boot_option__packed, title));
...
static int read_flag(const char* varname)
{
    ...
    if (!is_efi_boot()) /* If this is not an EFI boot, assume the queried flags
                           are zero */
        return 0;
...
int efi_reboot_to_firmware_supported(void)
{
    ...
    r = efi_get_variable(EFI_VENDOR_GLOBAL, "OsIndicationsSupported", NULL, &v,
                         &s);
...
static int get_os_indications(uint64_t* os_indication)
{
    ...
    if (r == -ENOENT)
    {
        /* Some firmware implementations that do support OsIndications and
         * report that with OsIndicationsSupported will remove the OsIndications
         * variable when it is unset. Let's pretend it's 0 then, to hide this
         * implementation detail. Note that this call will return -ENOENT then
         * only if the support for OsIndications is missing entirely, as
         * determined by efi_reboot_to_firmware_supported() above. */
        *os_indication = 0;
        return 0;
    }
    else if (r < 0)
...
int efi_set_reboot_to_firmware(bool value)
{
    ...
    if (b != b_new)
        return efi_set_variable(EFI_VENDOR_GLOBAL, "OsIndications", &b_new,
                                sizeof(uint64_t));

    return 0;
}

int efi_get_variable(sd_id128_t vendor, const char* name, uint32_t* attribute,
...
int efi_set_variable(sd_id128_t vendor, const char* name, const void* value,
                     size_t size)
{
    ...
    /* Newer efivarfs protects variables that are not in a whitelist with
     * FS_IMMUTABLE_FL by default, to protect them for accidental removal and
     * modification. We are not changing these variables accidentally however,
     * hence let's unset the bit first. */

    r = chattr_path(p, 0, FS_IMMUTABLE_FL, &saved_flags);
    if (r < 0 && r != -ENOENT)
        log_debug_errno(
            r, "Failed to drop FS_IMMUTABLE_FL flag from '%s', ignoring: %m",
            p);
...
finish:
    if (saved_flags_valid)
    {
        ...
        if (q < 0)
            log_debug_errno(
                q, "Failed to restore FS_IMMUTABLE_FL on '%s', ignoring: %m",
                p);
    }
...
int efi_set_variable_string(sd_id128_t vendor, const char* name, const char* v)
{
    ...
    return efi_set_variable(vendor, name, u16,
                            (char16_strlen(u16) + 1) * sizeof(char16_t));
}
...
static ssize_t utf16_size(const uint16_t* s, size_t buf_len_bytes)
{
    ...
    /* Returns the size of the string in bytes without the terminating two zero
     * bytes */
...
int efi_get_boot_option(uint16_t id, char** title, sd_id128_t* part_uuid,
                        char** path, bool* active)
{
    ...
    title_size =
        utf16_size(header->title, l - offsetof(struct boot_option, title));
    if (title_size < 0)
        return title_size;
    ...
    if (header->path_len > 0)
    {
        ...
        while (dnext < header->path_len)
        {
            ...
            /* Type 0x7F – End of Hardware Device Path, Sub-Type 0xFF – End
             * Entire Device Path */
...
{
    struct guid uuid = {
        .u1 = id.bytes[0] << 24 | id.bytes[1] << 16 | id.bytes[2] << 8 |
              id.bytes[3],
...
int efi_add_boot_option(uint16_t id, const char* title, uint32_t part,
                        uint64_t pstart, uint64_t psize, sd_id128_t part_uuid,
                        const char* path)
{
    ...
    option->path_len = offsetof(struct device_path, drive) +
    ...
    devicep->length =
        offsetof(struct device_path, drive) + sizeof(struct drive_path);
...
int efi_set_boot_order(uint16_t* order, size_t n)
{
    ...
    return efi_set_variable(EFI_VENDOR_GLOBAL, "BootOrder", order,
                            n * sizeof(uint16_t));
}
...
int efi_get_boot_options(uint16_t** options)
{
    ...
    FOREACH_DIRENT(de, dir, return -errno)
    {
        ...
        if (strcmp(de->d_name + 8, "-8be4df61-93ca-11d2-aa0d-00e098032b8c") !=
            0)
            continue;
...
int efi_loader_get_device_part_uuid(sd_id128_t* u)
{
    ...
    if (sscanf(p,
               ...
               &parsed[5], &parsed[6], &parsed[7], &parsed[8], &parsed[9],
               &parsed[10], &parsed[11], &parsed[12], &parsed[13], &parsed[14],
               &parsed[15]) != 16)
        return -EIO;
...
int efi_loader_get_entries(char*** ret)
{
    ...
    r = efi_get_variable(EFI_VENDOR_LOADER, "LoaderEntries", NULL,
                         (void**)&entries, &size);
    if (r < 0)
        return r;

    /* The variable contains a series of individually NUL terminated UTF-16
     * strings. */
    ...
    for (i = 0, start = 0;; i++)
    {
        ...
        /* Are we in the middle of a string? (i.e. not at the end of the
         * variable, nor at a NUL terminator?) If so, let's go to the next
         * entry. */
        if (!end && entries[i] != 0)
            continue;
        ...
        decoded =
            utf16_to_utf8(entries + start, (i - start) * sizeof(char16_t));
        if (!decoded)
            return -ENOMEM;
...
int efi_loader_get_features(uint64_t* ret)
{
    ...
    if (r == -ENOENT)
    {
        ...
        /* The new (v240+) LoaderFeatures variable is not supported, let's see
         * if it's systemd-boot at all */
        ...
        else if (first_word(info, "systemd-boot"))
        {
            ...
            /* An older systemd-boot version. Let's hardcode the feature set,
             * since it was pretty static in all its versions. */
