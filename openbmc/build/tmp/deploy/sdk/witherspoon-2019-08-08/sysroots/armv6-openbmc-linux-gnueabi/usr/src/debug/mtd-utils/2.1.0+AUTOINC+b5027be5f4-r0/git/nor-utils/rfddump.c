...
{
    printf(
        "Usage: %s [OPTIONS] MTD-device filename\n"
...
static int build_block_map(struct rfd* rfd, int fd, int block)
{
    ...
    for (i = 0; i < rfd->data_sectors; i++)
    {
        ...
        {
            fprintf(stderr,
                    "%s: warning: more than one entry "
                    "for sector %d\n",
                    rfd->mtd_filename, entry);
            continue;
        }

        rfd->sector_map[entry] =
            rfd->block_size * block + (i + rfd->header_sectors) * SECTOR_SIZE;
        sectors++;
    }
...
int main(int argc, char* argv[])
{
    ...
    else
    {
        ...
        if (st.st_size % SECTOR_SIZE)
            fprintf(stderr,
                    "%s: warning: not a multiple of sectors (512 bytes)\n",
                    rfd.mtd_filename);
    ...
    rfd.header_sectors =
        ((HEADER_MAP_OFFSET + sectors_per_block) * sizeof(uint16_t) +
         SECTOR_SIZE - 1) /
        SECTOR_SIZE;
    rfd.data_sectors = sectors_per_block - rfd.header_sectors;
    cylinders =
        ((rfd.block_count - 1) * rfd.data_sectors - 1) / SECTORS_PER_TRACK;
    rfd.sector_count = cylinders * SECTORS_PER_TRACK;
    rfd.header_size = (HEADER_MAP_OFFSET + rfd.data_sectors) * sizeof(uint16_t);
