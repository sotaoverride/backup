...
static int probe_drbdmanage(blkid_probe pr, const struct blkid_idmag* mag
                            __attribute__((__unused__)))
{
    ...
    hdr = (struct drbdmanage_hdr*)blkid_probe_get_buffer(pr, 0, sizeof(*hdr));
    if (!hdr)
    ...
    if (blkid_probe_set_id_label(pr, "UUID", hdr->uuid, sizeof(hdr->uuid)))
        return errno ? -errno : 1;

    prs = (struct drbdmanage_pers*)blkid_probe_get_buffer(pr, 0x1000,
                                                          sizeof(*prs));
    if (!prs)
        return errno ? -errno : 1;

    if (memcmp(prs->magic, persistence_magic, sizeof(prs->magic)) == 0 &&
        blkid_probe_sprintf_version(pr, "%d", be32_to_cpu(prs->version_le)) !=
            0)
        return errno ? -errno : 1;
...
const struct blkid_idinfo drbdmanage_idinfo = {
    ...
    .minsz = 64 * 1024,
    .magics = {{.magic = "$DRBDmgr=q", .len = 10, .sboff = 0}, {NULL}},
};
