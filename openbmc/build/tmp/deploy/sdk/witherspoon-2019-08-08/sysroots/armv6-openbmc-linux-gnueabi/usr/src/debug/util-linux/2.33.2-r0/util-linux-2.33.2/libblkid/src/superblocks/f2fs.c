...
{                                              /* According to version 1.1 */
    ...
    /* 0x20 */ uint32_t
        checksum_offset;               /* checksum offset inside super block */
...
static int probe_f2fs(blkid_probe pr, const struct blkid_idmag* mag)
{
    ...
    if (*((unsigned char*)sb->volume_name))
        blkid_probe_set_utf8label(pr, (unsigned char*)sb->volume_name,
                                  sizeof(sb->volume_name), BLKID_ENC_UTF16LE);
...
const struct blkid_idinfo f2fs_idinfo = {.name = "f2fs",
                                         .usage = BLKID_USAGE_FILESYSTEM,
                                         .probefunc = probe_f2fs,
                                         .magics = {{.magic = F2FS_MAGIC,
                                                     .len = 4,
