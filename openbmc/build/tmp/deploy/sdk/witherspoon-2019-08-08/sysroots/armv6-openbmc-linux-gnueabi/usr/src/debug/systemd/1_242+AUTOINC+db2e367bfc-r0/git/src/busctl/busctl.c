...
static int list_bus_names(int argc, char** argv, void* userdata)
{
    ...
    r = sd_bus_list_names(bus, (arg_acquired || arg_unique) ? &acquired : NULL,
    ...
    if (arg_legend)
    {
        printf("%-*s %*s %-*s %-*s %-*s %-*s %-*s %-*s", (int)max_i, "NAME", 10,
               "PID", 15, "PROCESS", 16, "USER", 13, "CONNECTION", 25, "UNIT",
               10, "SESSION", 19, "DESCRIPTION");
    ...
    STRV_FOREACH(i, merged)
    {
        ...
        r = sd_bus_get_name_creds(
            bus, *i,
            (arg_augment_creds ? SD_BUS_CREDS_AUGMENT : 0) | SD_BUS_CREDS_EUID |
...
static void print_subtree(const char* prefix, const char* path, char** l)
{
    ...
    for (;;)
    {
        ...
        printf("%s%s%s\n", prefix,
               special_glyph(has_more ? SPECIAL_GLYPH_TREE_BRANCH
...
static int find_nodes(sd_bus* bus, const char* service, const char* path,
                      Set* paths, bool many)
{
                    ...
                    bool many)
{
    ...
    for (;;)
    {
        ...
        if (set_contains(done, p) || set_contains(failed, p))
            continue;
...
static int on_method(const char* interface, const char* name,
                     const char* signature, const char* result, uint64_t flags,
                     void* userdata)
{
    _cleanup_(member_freep) Member* m;
    Set* members = userdata;
...
static int on_signal(const char* interface, const char* name,
                     const char* signature, uint64_t flags, void* userdata)
{
    _cleanup_(member_freep) Member* m;
    Set* members = userdata;
    int r;
...
static int on_property(const char* interface, const char* name,
                       const char* signature, bool writable, uint64_t flags,
                       void* userdata)
{
    _cleanup_(member_freep) Member* m;
    Set* members = userdata;
...
DEFINE_PRIVATE_HASH_OPS(member_hash_ops, Member, member_hash_func,
                        member_compare_func);
...
static int introspect(int argc, char** argv, void* userdata)
{
    ...
    if (r < 0)
        return log_error_errno(
            r, "Failed to introspect object %s of service %s: %s", argv[2],
    ...
    SET_FOREACH(m, members, i)
    {
        ...
        for (;;)
        {
            ...
            z = set_get(members, &((Member){.type = "property",
    ...
    {
        printf("%-*s %-*s %-*s %-*s %s\n", (int)name_width, "NAME",
               (int)type_width, "TYPE", (int)signature_width, "SIGNATURE",
               (int)result_width, "RESULT/VALUE", "FLAGS");
    }
    ...
    for (j = 0; j < k; j++)
    {
        ...
        printf("%s%s%-*s%s %-*s %-*s %-*s%s%s%s%s%s%s\n",
               ...
               empty_to_dash(
                   streq_ptr(m->type, "interface") ? m->interface : m->name),
               is_interface ? ansi_normal() : "", (int)type_width,
               empty_to_dash(m->type), (int)signature_width,
               empty_to_dash(m->signature), (int)result_width, rv,
               (m->flags & SD_BUS_VTABLE_DEPRECATED)
                   ? " deprecated"
                   : (m->flags || m->writable ? "" : " -"),
               (m->flags & SD_BUS_VTABLE_METHOD_NO_REPLY) ? " no-reply" : "",
               (m->flags & SD_BUS_VTABLE_PROPERTY_CONST) ? " const" : "",
               (m->flags & SD_BUS_VTABLE_PROPERTY_EMITS_CHANGE)
                   ? " emits-change"
                   : "",
               (m->flags & SD_BUS_VTABLE_PROPERTY_EMITS_INVALIDATION)
                   ? " emits-invalidation"
                   : "",
               m->writable ? " writable" : "");
    }
...
static int message_json(sd_bus_message* m, FILE* f)
{
    ...
    r = json_build(
        ...
        JSON_BUILD_OBJECT(
            JSON_BUILD_PAIR(
                "type",
                JSON_BUILD_STRING(bus_message_type_to_string(m->header->type))),
            ...
            JSON_BUILD_PAIR_CONDITION(m->priority != 0, "priority",
                                      JSON_BUILD_INTEGER(m->priority)),
            JSON_BUILD_PAIR("cookie",
                            JSON_BUILD_INTEGER(BUS_MESSAGE_COOKIE(m))),
            JSON_BUILD_PAIR_CONDITION(m->reply_cookie != 0, "reply_cookie",
                                      JSON_BUILD_INTEGER(m->reply_cookie)),
            JSON_BUILD_PAIR_CONDITION(m->sender, "sender",
                                      JSON_BUILD_STRING(m->sender)),
            JSON_BUILD_PAIR_CONDITION(m->destination, "destination",
                                      JSON_BUILD_STRING(m->destination)),
            JSON_BUILD_PAIR_CONDITION(m->path, "path",
                                      JSON_BUILD_STRING(m->path)),
            JSON_BUILD_PAIR_CONDITION(m->interface, "interface",
                                      JSON_BUILD_STRING(m->interface)),
            JSON_BUILD_PAIR_CONDITION(m->member, "member",
                                      JSON_BUILD_STRING(m->member)),
            JSON_BUILD_PAIR_CONDITION(m->monotonic != 0, "monotonic",
                                      JSON_BUILD_INTEGER(m->monotonic)),
            JSON_BUILD_PAIR_CONDITION(m->realtime != 0, "realtime",
                                      JSON_BUILD_INTEGER(m->realtime)),
            JSON_BUILD_PAIR_CONDITION(m->seqnum != 0, "seqnum",
                                      JSON_BUILD_INTEGER(m->seqnum)),
            JSON_BUILD_PAIR_CONDITION(m->error.name, "error_name",
                                      JSON_BUILD_STRING(m->error.name)),
...
static int monitor(int argc, char** argv,
                   int (*dump)(sd_bus_message* m, FILE* f))
{
    _cleanup_(sd_bus_flush_close_unrefp) sd_bus* bus = NULL;
    ...
    r = sd_bus_message_new_method_call(
        bus, &message, "org.freedesktop.DBus", "/org/freedesktop/DBus",
    ...
    for (;;)
    {
        ...
        if (!is_monitor)
        {
            ...
            if (sd_bus_message_is_signal(m, "org.freedesktop.DBus",
                                         "NameLost") <= 0)
                continue;
...
static int verb_monitor(int argc, char** argv, void* userdata)
{
    return monitor(argc, argv,
                   arg_json != JSON_OFF ? message_json : message_dump);
}
...
static int verb_capture(int argc, char** argv, void* userdata)
{
    ...
    if (isatty(fileno(stdout)) > 0)
        return log_error_errno(SYNTHETIC_ERRNO(EINVAL),
                               "Refusing to write message data to console, "
    ...
    else
    {
        ...
        if (r >= 0)
            printf("BusID=%s" SD_ID128_FORMAT_STR "%s\n", ansi_highlight(),
                   SD_ID128_FORMAT_VAL(bus_id), ansi_normal());
...
static int message_append_cmdline(sd_bus_message* m, const char* signature,
                                  char*** x)
{
    char** p;
    int r;
    ...
    for (;;)
    {
        ...
        switch (t)
        {
            ...
            case SD_BUS_TYPE_BOOLEAN:
                ...
                if (r < 0)
                    return log_error_errno(
                        r, "Failed to parse '%s' as boolean: %m", v);
            ...
            case SD_BUS_TYPE_BYTE:
            {
                ...
                if (r < 0)
                    return log_error_errno(r,
                                           "Failed to parse '%s' as byte "
                                           "(unsigned 8bit integer): %m",
                                           v);
            ...
            case SD_BUS_TYPE_INT16:
            {
                ...
                if (r < 0)
                    return log_error_errno(
                        r, "Failed to parse '%s' as signed 16bit integer: %m",
                        v);
            ...
            case SD_BUS_TYPE_UINT16:
            {
                ...
                if (r < 0)
                    return log_error_errno(
                        r, "Failed to parse '%s' as unsigned 16bit integer: %m",
                        v);
            ...
            case SD_BUS_TYPE_INT32:
            {
                ...
                if (r < 0)
                    return log_error_errno(
                        r, "Failed to parse '%s' as signed 32bit integer: %m",
                        v);
            ...
            case SD_BUS_TYPE_UINT32:
            {
                ...
                if (r < 0)
                    return log_error_errno(
                        r, "Failed to parse '%s' as unsigned 32bit integer: %m",
                        v);
            ...
            case SD_BUS_TYPE_INT64:
            {
                ...
                if (r < 0)
                    return log_error_errno(
                        r, "Failed to parse '%s' as signed 64bit integer: %m",
                        v);
            ...
            case SD_BUS_TYPE_UINT64:
            {
                ...
                if (r < 0)
                    return log_error_errno(
                        r, "Failed to parse '%s' as unsigned 64bit integer: %m",
                        v);
            ...
            case SD_BUS_TYPE_DOUBLE:
            {
                ...
                if (r < 0)
                    return log_error_errno(r,
                                           "Failed to parse '%s' as double "
                                           "precision floating point: %m",
                                           v);
            ...
            case SD_BUS_TYPE_ARRAY:
            {
                ...
                if (r < 0)
                    return log_error_errno(
                        r, "Failed to parse '%s' number of array entries: %m",
                        v);
            ...
            case SD_BUS_TYPE_DICT_ENTRY_BEGIN:
            {
                ...
                if (r < 0)
                    return log_error_errno(
                        r, "Invalid struct/dict entry signature: %m");
...
static int json_transform_variant(sd_bus_message* m, const char* contents,
                                  JsonVariant** ret)
{
...
static int json_transform_message(sd_bus_message* m, JsonVariant** ret)
{
    ...
    r = json_build(
        ret, JSON_BUILD_OBJECT(JSON_BUILD_PAIR("type", JSON_BUILD_STRING(type)),
...
static int call(int argc, char** argv, void* userdata)
{
    ...
    r = sd_bus_message_new_method_call(bus, &m, argv[1], argv[2], argv[3],
                                       argv[4]);
    if (r < 0)
        return bus_log_create_error(r);
    ...
    r = sd_bus_message_set_allow_interactive_authorization(
...
static int help(void)
{
    ...
    printf("%s [OPTIONS...] {COMMAND} ...\n\n"
           ...
           "     --list                Don't show tree, but simple object path "
           "list\n"
           "  -q --quiet               Don't show method call reply\n"
           "     --verbose             Show result values in long format\n"
           "     --json=MODE           Output as JSON\n"
           "  -j                       Same as --json=pretty on tty, "
           "--json=short otherwise\n"
           "     --expect-reply=BOOL   Expect a method call reply\n"
           "     --auto-start=BOOL     Auto-start destination service\n"
           "     --allow-interactive-authorization=BOOL\n"
           "                           Allow interactive authorization for "
           "operation\n"
           "     --timeout=SECS        Maximum time to wait for method call "
           "completion\n"
           "     --augment-creds=BOOL  Extend credential data with data read "
           "from /proc/$PID\n"
           "     --watch-bind=BOOL     Wait for bus AF_UNIX socket to be bound "
           "in the file\n"
           "                           system\n"
           "     --destination=SERVICE Destination service of a signal\n"
           "\nCommands:\n"
           "  list                     List bus names\n"
           "  status [SERVICE]         Show bus service, process or bus owner "
           "credentials\n"
...
static int parse_argv(int argc, char* argv[])
{
    ...
    static const struct option options[] = {
        ...
        {"allow-interactive-authorization", required_argument, NULL,
         ARG_ALLOW_INTERACTIVE_AUTHORIZATION},
    ...
    while ((c = getopt_long(argc, argv, "hH:M:qj", options, NULL)) >= 0)
        ...
        switch (c)
        {
            ...
            case ARG_SIZE:
            {
                ...
                if (r < 0)
                    return log_error_errno(r, "Failed to parse size '%s': %m",
                                           optarg);
            ...
            case ARG_EXPECT_REPLY:
                ...
                if (r < 0)
                    return log_error_errno(
                        r, "Failed to parse --expect-reply= parameter '%s': %m",
                        optarg);
            ...
            case ARG_AUTO_START:
                ...
                if (r < 0)
                    return log_error_errno(
                        r, "Failed to parse --auto-start= parameter '%s': %m",
                        optarg);
            ...
            case ARG_ALLOW_INTERACTIVE_AUTHORIZATION:
                ...
                if (r < 0)
                    return log_error_errno(
                        r,
                        "Failed to parse --allow-interactive-authorization= "
                        "parameter '%s': %m",
                        optarg);
            ...
            case ARG_TIMEOUT:
                ...
                if (r < 0)
                    return log_error_errno(
                        r, "Failed to parse --timeout= parameter '%s': %m",
                        optarg);
            ...
            case ARG_AUGMENT_CREDS:
                ...
                if (r < 0)
                    return log_error_errno(
                        r,
                        "Failed to parse --augment-creds= parameter '%s': %m",
                        optarg);
            ...
            case ARG_WATCH_BIND:
                ...
                if (r < 0)
                    return log_error_errno(
                        r, "Failed to parse --watch-bind= parameter '%s': %m",
                        optarg);
