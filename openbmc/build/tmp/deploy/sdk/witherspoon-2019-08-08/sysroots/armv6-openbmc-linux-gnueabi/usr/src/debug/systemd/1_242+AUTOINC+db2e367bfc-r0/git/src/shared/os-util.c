...
int path_is_os_tree(const char* path)
{
    ...
    /* Does the path exist at all? If not, generate an error immediately. This
     * is useful so that a missing root dir always results in -ENOENT, and we
     * can properly distuingish the case where the whole root doesn't exist from
...
int open_os_release(const char* root, char** ret_path, int* ret_fd)
{
    ...
    {
        k = chase_symlinks(p, root,
                           CHASE_PREFIX_ROOT | (ret_fd ? CHASE_OPEN : 0),
                           (ret_path ? &q : NULL));
        if (k != -ENOENT)
            break;
    }
