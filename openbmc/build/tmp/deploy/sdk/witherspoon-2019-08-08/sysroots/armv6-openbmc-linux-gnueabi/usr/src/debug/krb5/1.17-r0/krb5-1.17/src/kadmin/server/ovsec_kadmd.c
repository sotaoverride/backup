 ...
 */
static void fail_to_start(krb5_error_code code, const char* msg)
{
    const char* errmsg;
...
static krb5_error_code setup_loop(kadm5_config_params* params, int proponly,
                                  verto_ctx** ctx_out)
{
    krb5_error_code ret;
    verto_ctx* ctx;
    ...
    *ctx_out = ctx = loop_init(VERTO_EV_TYPE_SIGNAL);
    ...
    {
        ret =
            loop_add_udp_address(params->kpasswd_port, params->kpasswd_listen);
        if (ret)
            return ret;
        ret =
            loop_add_tcp_address(params->kpasswd_port, params->kpasswd_listen);
        if (ret)
            return ret;
        ret = loop_add_rpc_service(params->kadmind_port, params->kadmind_listen,
...
static krb5_error_code setup_kdb_keytab()
{
    krb5_error_code ret;
...
/* Return "name@realm". */
static char* build_princ_name(char* name, char* realm)
{
    char* fullname;
...
static void log_badverf(gss_name_t client_name, gss_name_t server_name,
    ...
    else
    {
        krb5_klog_syslog(LOG_NOTICE,
                         _("WARNING! Forged/garbled request: %d, claimed "
                           "client = %.*s%s, server = %.*s%s, addr = %s"),
                         proc, (int)clen, (char*)client.value, cdots, (int)slen,
                         (char*)server.value, sdots, a);
    }
...
static void log_miscerr(struct svc_req* rqst, struct rpc_msg* msg, char* error,
                        char* data)
{
...
static void log_badauth_display_status_1(char* m, OM_uint32 code, int type)
{
...
{
    krb5_klog_syslog(LOG_NOTICE,
                     _("Authentication attempt failed: %s, "
...
int main(int argc, char* argv[])
{
    ...
    while (argc)
    {
        if (strcmp(*argv, "-x") == 0)
        {
            ...
            {
                fprintf(stderr,
                        _("%s: cannot initialize. Not enough "
                          "memory\n"),
                        progname);
                exit(1);
            }
    ...
    if (params.iprop_enabled == TRUE)
    {
        ...
        if (nofork)
        {
            fprintf(stderr, _("%s: create IPROP svc (PROG=%d, VERS=%d)\n"),
