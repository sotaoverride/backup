...
static void g_zlib_compressor_set_gzheader(GZlibCompressor* compressor)
{
    ...
    compressor->gzheader.time = (uLong)g_file_info_get_attribute_uint64(
        compressor->file_info, G_FILE_ATTRIBUTE_TIME_MODIFIED);
...
static void g_zlib_compressor_set_property(GObject* object, guint prop_id,
                                           const GValue* value,
                                           GParamSpec* pspec)
{
    GZlibCompressor* compressor;
    ...
    switch (prop_id)
    {
        ...
        case PROP_FILE_INFO:
            g_zlib_compressor_set_file_info(compressor,
...
static void g_zlib_compressor_get_property(GObject* object, guint prop_id,
...
static void g_zlib_compressor_constructed(GObject* object)
{
    ...
    {
        /* + 16 for gzip */
        res = deflateInit2(&compressor->zstream, compressor->level, Z_DEFLATED,
...
static void g_zlib_compressor_class_init(GZlibCompressorClass* klass)
{
    ...
    g_object_class_install_property(
        ...
        g_param_spec_enum("format", P_("compression format"),
    ...
    g_object_class_install_property(
        gobject_class, PROP_LEVEL,
        g_param_spec_int(
            "level", P_("compression level"),
            P_("The level of compression from 0 (no compression) to 9 (most "
     ...
     */
    g_object_class_install_property(
        gobject_class, PROP_FILE_INFO,
        g_param_spec_object("file-info", P_("file info"), P_("File info"),
                            G_TYPE_FILE_INFO,
...
GZlibCompressor* g_zlib_compressor_new(GZlibCompressorFormat format, int level)
{
    ...
    compressor = g_object_new(G_TYPE_ZLIB_COMPRESSOR, "format", format, "level",
                              level, NULL);
...
static GConverterResult
    g_zlib_compressor_convert(GConverter* converter, const void* inbuf,
