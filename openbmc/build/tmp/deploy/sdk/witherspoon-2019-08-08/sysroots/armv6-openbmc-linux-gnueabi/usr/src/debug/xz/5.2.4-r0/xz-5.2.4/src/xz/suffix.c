...
///             false is returned.
static bool has_sfn_suffix(const char* str, size_t len)
{
        ...
        !is_dir_sep(str[len - 2]))
    {
        ...
        // *.??-
        if (len >= 5 && !is_dir_sep(str[len - 3]) && str[len - 4] == '.')
...
///             is returned.
static size_t test_suffix(const char* suffix, const char* src_name,
                          size_t src_len)
{
    ...
    if (src_len <= suffix_len || is_dir_sep(src_name[src_len - suffix_len - 1]))
        return 0;
...
static char* uncompressed_name(const char* src_name, const size_t src_len)
{
    ...
    else
    {
        ...
        {
            new_len = test_suffix(suffixes[i].compressed, src_name, src_len);
            if (new_len != 0)
            {
...
{
    message_warning(_("%s: File already has `%s' suffix, skipping"), src_name,
                    suffix);
    return;
}
...
static char* compressed_name(const char* src_name, size_t src_len)
{
    ...
    const char* suffix = custom_suffix != NULL ? custom_suffix : suffixes[0];
    ...
    if (!_use_lfn(src_name))
    {
        ...
        if (sufsep == NULL || sufsep[1] == '\0' || has_dir_sep(sufsep))
        {
            ...
            //
            // Avoid double dots:
            if (sufsep != NULL && sufsep[1] == '\0' && suffix[0] == '.')
                --src_len;
        }
        else if (custom_suffix == NULL && strcasecmp(sufsep, ".tar") == 0)
        {
        ...
        else
        {
            ...
            else
            {
                ...
                if (strlen(sufsep) > 4 - suffix_len)
                    src_len = sufsep - src_name + 4 - suffix_len;
            }
        }
    }
#endif
...
extern char* suffix_get_dest_name(const char* src_name)
{
    ...
    return opt_mode == MODE_COMPRESS ? compressed_name(src_name, src_len)
