...
static void* _dl_tls_symaddr(struct link_map* map, const ElfW(Sym) * ref)
{
    ...
    tls_index tmp = {.ti_module = map->l_tls_modid, .ti_offset = ref->st_value};
                    ...
                    struct r_found_version* vers, int flags)
{
       ...
       array can change.  */
        if (RTLD_SINGLE_THREAD_P)
            result = GLRO(dl_lookup_symbol_x)(
                name, match, &ref, match->l_scope, vers, 0,
    ...
    else if (handle == RTLD_NEXT)
    {
        ...
        {
            if (match == NULL || caller < match->l_map_start ||
    ...
    if (ref != NULL)
    {
       ...
       tell us whether further auditing is wanted.  */
        if (__glibc_unlikely(GLRO(dl_naudit) > 0))
        {
            ...
            unsigned int ndx =
                (ref - (ElfW(Sym)*)D_PTR(result, l_info[DT_SYMTAB]));
                ...
                for (unsigned int cnt = 0; cnt < GLRO(dl_naudit); ++cnt)
                {
                          ...
                          0)))
                    {
                        ...
                        uintptr_t new_value = afct->symbind(
                            &sym, ndx, &match->l_audit[cnt].cookie,
