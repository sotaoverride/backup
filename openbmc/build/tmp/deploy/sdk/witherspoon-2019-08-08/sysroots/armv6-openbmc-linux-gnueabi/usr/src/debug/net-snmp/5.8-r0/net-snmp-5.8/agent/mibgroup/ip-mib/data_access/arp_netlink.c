...
netsnmp_arp_access*
    netsnmp_access_arp_create(u_int init_flags,
                              ...
                              NetsnmpAccessArpGC* gc_hook, int* cache_timeout,
                              int* cache_flags, char* cache_expired)
{
...
int netsnmp_access_arp_load(netsnmp_arp_access* access)
{
    ...
    if (fd == 0)
    {
        ...
        {
            snmp_log_perror(
                "netsnmp_access_arp_load: netlink socket create error");
            return -1;
        }
        ...
        if (register_readfd(fd, netsnmp_access_arp_read_netlink, access) != 0)
        {
            snmp_log(
                LOG_ERR,
                "netsnmp_access_arp_load: error registering netlink socket\n");
            return -1;
        }
    }
    ...
    for (h = (struct nlmsghdr*)buf; NLMSG_OK(h, len); h = NLMSG_NEXT(h, len))
    {
        ...
        else
        {
            if (r < 0)
            {
                ...
                DEBUGMSGTL(
                    ("access:netlink:arp", "filling entry info failed\n"));
            }
...
static int fillup_entry_info(netsnmp_arp_entry* entry, struct nlmsghdr* nlmp)
{
    struct ndmsg* rtmp;
    ...
    switch (nlmp->nlmsg_type)
    {
        ...
        case RTM_GETNEIGH:
            return 0;
        default:
            DEBUGMSGTL(("access:netlink:arp", "Wrong Netlink message type %d\n",
                        nlmp->nlmsg_type));
            return -1;
    }
    ...
    rta =
        ((struct rtattr*)(((char*)(rtmp)) + NLMSG_ALIGN(sizeof(struct ndmsg))));
    ...
    switch (rtmp->ndm_state)
    {
        ...
        case NUD_NONE:
            ...
            break;
        default:
            snmp_log(LOG_ERR, "Unrecognized ARP entry state %d",
                     rtmp->ndm_state);
            break;
    }
