...
static int c_dvar_read_data(CDVar* var, int alignment, const void** datap,
                            size_t n_data)
{
    size_t i, align;
    ...
    while ((c = *format++))
    {
        ...
        switch (c)
        {
            case '[':
                ...
                r = c_dvar_read_data(var, (var->current->i_type + 1)->alignment,
                                     NULL, 0);
                if (r)
                    goto error;
            ...
            case 'g':
                ...
                if (u8 || (c == 's' && !c_dvar_is_string(str, u32)) ||
...
static int c_dvar_ff(CDVar* var)
{
    ...
    {
        if (var->current->n_type &&
            (var->current->container != 'a' || c_dvar_more(var)))
        {
 ...
 */
_public_ void c_dvar_begin_read(CDVar* var, bool big_endian,
