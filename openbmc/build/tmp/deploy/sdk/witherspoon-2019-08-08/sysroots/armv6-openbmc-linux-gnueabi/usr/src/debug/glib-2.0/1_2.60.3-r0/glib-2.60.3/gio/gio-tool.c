...
char* attribute_flags_to_string(GFileAttributeInfoFlags flags)
{
    ...
    struct
    {
        ...
        {G_FILE_ATTRIBUTE_INFO_COPY_WITH_FILE, N_("Copy with file")},
...
gboolean file_is_dir(GFile* file)
{
    ...
    info =
        g_file_query_info(file, G_FILE_ATTRIBUTE_STANDARD_TYPE, 0, NULL, NULL);
...
static int handle_version(int argc, char* argv[], gboolean do_help)
{
    if (do_help || argc > 1)
    {
        if (!do_help)
