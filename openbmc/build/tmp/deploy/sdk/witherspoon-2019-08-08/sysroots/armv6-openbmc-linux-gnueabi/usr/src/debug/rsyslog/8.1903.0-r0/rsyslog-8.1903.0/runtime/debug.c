...
DEFobjCurrIf(obj) int Debug =
    DEBUG_OFF;        /* debug flag  - read-only after startup */
...
char* pszAltDbgFileName =
    NULL;        /* if set, debug output is *also* sent to here */
int altdbg = -1; /* and the handle for alternate debug output */
int stddbg = 1;  /* the handle for regular debug output, set to stdout if not
                    forking, -1 otherwise */
static uint64_t dummy_errcount =
    0; /* just to avoid some static analyzer complaints */
...
static void ATTR_NONNULL()
    dbgGetThrdName(char* const pszBuf, const size_t lenBuf,
                   const pthread_t thrdID)
{
    assert(pszBuf != NULL);
 ...
 */
static void DBGL_UNUSED dbgprint(obj_t* pObj, char* pszMsg,
                                 const char* pszFileName, const size_t lenMsg)
{
    uchar* pszObjName = NULL;
    static pthread_t ptLastThrdID = 0;
    static int bWasNL = 0;
    char pszThrdName[64]; /* 64 is to be on the safe side, anything over 20 is
                             bad... */
     ...
     * is worth fixing it, giving the limited appliability. -- rgerhards,
     * 2005-10-25 I have decided that it is not worth fixing it - especially as
     * it works pretty well. -- rgerhards, 2007-06-15
     */
...
#endif
            lenWriteBuf = snprintf(
                pszWriteBuf + offsWriteBuf, sizeof(pszWriteBuf) - offsWriteBuf,
                "%4.4ld.%9.9ld:", (long)(t.tv_sec % 10000), t.tv_nsec);
            offsWriteBuf += lenWriteBuf;
        }

        lenWriteBuf =
            snprintf(pszWriteBuf + offsWriteBuf,
                     sizeof(pszWriteBuf) - offsWriteBuf, "%s: ", pszThrdName);
        offsWriteBuf += lenWriteBuf;
        /* print object name header if we have an object */
        if (pszObjName != NULL)
        {
            lenWriteBuf = snprintf(pszWriteBuf + offsWriteBuf,
                                   sizeof(pszWriteBuf) - offsWriteBuf,
                                   "%s: ", pszObjName);
            offsWriteBuf += lenWriteBuf;
        }
        lenWriteBuf =
            snprintf(pszWriteBuf + offsWriteBuf,
                     sizeof(pszWriteBuf) - offsWriteBuf, "%s: ", pszFileName);
        offsWriteBuf += lenWriteBuf;
    }
...
static int DBGL_UNUSED checkDbgFile(const char* srcname)
{
    ...
    if (glblDbgWhitelist)
    {
        if (bsearch(srcname, glblDbgFiles, glblDbgFilesNum, sizeof(char*),
                    bs_arrcmp_glblDbgFiles) == NULL)
        {
            return 0;
        }
        else
 ...
 * into a single function as we have variable arguments and I don't know how to
 * call from one vararg function into another. I don't dig in this, it is OK for
 * the time being. -- rgerhards, 2008-01-29
 */
#ifndef DEBUGLESS
        ...
        while (dbgGetRTOptNamVal(&pszOpts, &optname))
        {
            if (!strcasecmp((char*)optname, "help"))
            {
                fprintf(stderr,
                        ...
                        "additional logfile: export "
                        "RSYSLOG_DEBUGFILE=\"/path/to/file\"\n"
                        ...
                        "Nostdout\n"
                        "OutputTidToStderr\n"
                        "DebugOnDemand - enables debugging on USR1, but does "
                        "not turn on output\n"
                        "\nSee debug.html in your doc set or "
                        "https://www.rsyslog.com for details\n");
                exit(1);
            }
            ...
            else if (!strcasecmp((char*)optname, "debugondemand"))
            {
                ...
                debugging_on = 1;
                dbgprintf(
                    "Note: debug on demand turned on via configuraton file, "
            ...
            {
                fprintf(stderr,
                        "rsyslogd " VERSION " error: invalid debug option '%s' "
                        "- ignored\n",
                        optname);
            }
        }
...
void dbgSetDebugFile(uchar* fn)
{
    ...
    if ((altdbg = open((char*)fn,
                       O_WRONLY | O_CREAT | O_TRUNC | O_NOCTTY | O_CLOEXEC,
                       S_IRUSR | S_IWUSR)) == -1)
    {
        fprintf(
            stderr,
            "alternate debug file could not be opened, ignoring. Error: %s\n",
            strerror(errno));
    }
}
...
{
    rsRetVal iRet; /* do not use DEFiRet, as this makes calls into the debug
                      system! */

    (void)pthread_key_create(&keyThrdName, dbgThrdNameDestruct);

    /* while we try not to use any of the real rsyslog code (to avoid infinite
     * loops), we need to have the ability to query object names. Thus, we need
     * to obtain a pointer to the object interface. -- rgerhards, 2008-02-29
     */
    ...
    dbgTimeoutToStderr =
        (dbgto2stderr != NULL && !strcmp(dbgto2stderr, "on")) ? 1 : 0;
    ...
    if (pszAltDbgFileName != NULL)
    {
        ...
        {
            fprintf(stderr,
                    "alternate debug file could not be opened, ignoring. "
                    "Error: %s\n",
                    strerror(errno));
        }
    }
