...
int csum_len_for_type(int cst, BOOL flist_csum)
{
    switch (cst)
    {
        ...
        case CSUM_MD4_ARCHAIC:
            /* The oldest checksum code is rather weird: the file-list code only
             * sent 2-byte checksums, but all other checksums were full MD4
             * length. */
...
void file_checksum(const char* fname, const STRUCT_STAT* st_p, char* sum)
{
    ...
    switch (checksum_type)
    {
        case CSUM_MD5:
            ...
            for (i = 0; i + CSUM_CHUNK <= len; i += CSUM_CHUNK)
            {
                md5_update(&m, (uchar*)map_ptr(buf, i, CSUM_CHUNK), CSUM_CHUNK);
            }
        ...
        case CSUM_MD4_ARCHAIC:
            ...
            if (remainder > 0 || checksum_type > CSUM_MD4_BUSTED)
                mdfour_update(&m, (uchar*)map_ptr(buf, i, remainder),
                              remainder);
