...
static void read_sockets(const char* dir, SocketEntry*** entries_p,
                         int* n_entries_p)
{
    DIR* dirh;
    struct dirent* dent;
...
static SocketStatus open_socket(SocketEntry* se)
{
    ...
    {
        fprintf(stderr,
                "Warning: giving up on socket %s after several retries; unable "
                "to determine socket's status\n",
                se->name);
    ...
    {
        fprintf(stderr,
                "Warning: failed to open a socket to use for connecting: %s\n",
                strerror(errno));
    ...
    else
    {
        switch (errno)
        {
            ...
            default:
                fprintf(
                    stderr,
                    "Warning: unexpected error connecting to socket %s: %s\n",
                    se->name, strerror(errno));
                return SOCKET_FAILED_TO_HANDLE;
        }
    }
...
static int handle_sockets(SocketEntry** entries, int n_entries)
{
    ...
    while (i < n_entries)
    {
        ...
        if (se->fd >= 0)
        {
            fprintf(
                stderr,
                "Internal error, socket has fd  kept open while status = %d\n",
                se->status);
            exit(1);
        }
        ...
        switch (status)
        {
            ...
            default:
                fprintf(stderr,
                        "Bad status from open_socket(), should not happen\n");
                exit(1);
                break;
        }
...
static void clean_dir(const char* dir)
{
    ...
    {
        fprintf(stderr, "Unable to determine state of some sockets, retrying "
                        "in 2 seconds\n");
        sleep(2);
    }
...
int main(int argc, char** argv)
{
    ...
    while (i < argc)
    {
        ...
        else
        {
            if (dirname != NULL)
            {
                fprintf(stderr, "dbus-cleanup-sockets only supports a single "
                                "directory name\n");
                exit(1);
            }
    ...
    printf("Cleaned up %d sockets in %s; %d sockets are still in use; %d in "
           "unknown state\n",
           cleaned_count, dirname, alive_count, unhandled_count);
#else
    printf("This system does not support UNIX domain sockets, so "
           "dbus-cleanup-sockets does nothing\n");
#endif
