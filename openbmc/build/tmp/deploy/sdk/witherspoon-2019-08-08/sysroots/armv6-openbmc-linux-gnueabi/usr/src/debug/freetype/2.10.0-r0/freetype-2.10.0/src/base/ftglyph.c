...
ft_outline_glyph_init(FT_Glyph outline_glyph, FT_GlyphSlot slot)
{
    ...
    error = FT_Outline_New(library, (FT_UInt)source->n_points,
                           source->n_contours, &glyph->outline);
    if (error)
        goto Exit;
...
ft_outline_glyph_copy(FT_Glyph outline_source, FT_Glyph outline_target)
{
    ...
    error = FT_Outline_New(library, (FT_UInt)source->outline.n_points,
...
FT_CALLBACK_DEF(void)
ft_outline_glyph_transform(FT_Glyph outline_glyph, const FT_Matrix* matrix,
...
FT_DEFINE_GLYPH(
    ...
    sizeof(FT_OutlineGlyphRec), FT_GLYPH_FORMAT_OUTLINE,
...
static FT_Error ft_new_glyph(FT_Library library, const FT_Glyph_Class* clazz,
                             FT_Glyph* aglyph)
{
...
FT_Get_Glyph(FT_GlyphSlot slot, FT_Glyph* aglyph)
{
    ...
    if (slot->advance.x >= 0x8000L * 64 || slot->advance.x <= -0x8000L * 64)
    {
        FT_ERROR(("FT_Get_Glyph: advance width too large\n"));
        error = FT_THROW(Invalid_Argument);
        goto Exit2;
    }
    if (slot->advance.y >= 0x8000L * 64 || slot->advance.y <= -0x8000L * 64)
    {
