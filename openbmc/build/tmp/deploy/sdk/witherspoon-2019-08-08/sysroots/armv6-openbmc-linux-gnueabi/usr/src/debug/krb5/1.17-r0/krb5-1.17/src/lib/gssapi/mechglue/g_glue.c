 ...
 */
int gssint_get_der_length(unsigned char** buf, unsigned int buf_len,
                          unsigned int* bytes)
{
 ...
 */
int gssint_put_der_length(unsigned int length, unsigned char** buf,
...
OM_uint32 gssint_get_mech_type_oid(OID, token) gss_OID OID;
gss_buffer_t token;
{
...
OM_uint32 gssint_get_mech_type(OID, token) gss_OID OID;
gss_buffer_t token;
{
    /* Check for interoperability exceptions */
    if (token->length >= sizeof(NTLMSSP_SIGNATURE) &&
        memcmp(token->value, NTLMSSP_SIGNATURE, sizeof(NTLMSSP_SIGNATURE)) == 0)
    {
        *OID = gss_ntlm_mechanism_oid_desc;
    }
    else if (token->length != 0 && ((char*)token->value)[0] == 0x6E)
    {
...
static OM_uint32 import_internal_attributes(OM_uint32* minor,
                                            gss_mechanism dmech,
                                            ...
                                            gss_name_t dname)
{
    ...
    major =
        smech->gss_inquire_name(minor, sname->mech_name, NULL, NULL, &attrs);
    ...
    for (i = 0; i < attrs->count; i++)
    {
        ...
        while (more != 0)
        {
            ...
            major = smech->gss_get_name_attribute(
                minor, sname->mech_name, &attrs->elements[i], &authenticated,
...
OM_uint32 gssint_import_internal_name(minor_status, mech_type, union_name,
                                      internal_name) OM_uint32* minor_status;
gss_OID mech_type;
...
gss_name_t* internal_name;
{
    ...
    {
        status = mech->gss_duplicate_name(minor_status, union_name->mech_name,
                                          internal_name);
    ...
    if (mech->gssspi_import_name_by_mech)
    {
        ...
        status = mech->gssspi_import_name_by_mech(
            minor_status, public_mech, union_name->external_name,
    ...
    if (status == GSS_S_COMPLETE)
    {
        ...
        (void)import_internal_attributes(&tmpMinor, mech, union_name,
                                         *internal_name);
    }
    else
    {
...
OM_uint32 gssint_export_internal_name(minor_status, mech_type, internal_name,
                                      name_buf) OM_uint32* minor_status;
...
gss_buffer_t name_buf;
{
    ...
    {
        status = mech->gss_export_name(minor_status, internal_name, name_buf);
        if (status != GSS_S_COMPLETE)
            map_error(minor_status, mech);
        return status;
    }
     ...
     */
    if ((status = mech->gss_display_name(minor_status, internal_name, &dispName,
                                         &nameOid)) != GSS_S_COMPLETE)
    {
        map_error(minor_status, mech);
        return (status);
    }
    ...
    name_buf->length = tokIdLen + mechOidLenLen + mechOidTagLen +
                       mechOidDERLen + mech_type->length + nameLenLen +
                       dispName.length;
...
OM_uint32 gssint_display_internal_name(minor_status, mech_type, internal_name,
                                       external_name,
                                       name_type) OM_uint32* minor_status;
gss_OID mech_type;
gss_name_t internal_name;
...
gss_OID* name_type;
{
    ...
    if (mech)
    {
        if (mech->gss_display_name)
        {
            status = mech->gss_display_name(minor_status, internal_name,
...
OM_uint32 gssint_release_internal_name(minor_status, mech_type,
                                       internal_name) OM_uint32* minor_status;
gss_OID mech_type;
gss_name_t* internal_name;
{
    ...
    if (mech)
    {
        ...
        {
            status = mech->gss_release_name(minor_status, internal_name);
            if (status != GSS_S_COMPLETE)
                map_error(minor_status, mech);
        }
        else
...
OM_uint32
    gssint_delete_internal_sec_context(minor_status, mech_type, internal_ctx,
                                       output_token) OM_uint32* minor_status;
gss_OID mech_type;
...
gss_buffer_t output_token;
{
    ...
    if (mech)
    {
        if (mech->gss_delete_sec_context)
            status = mech->gss_delete_sec_context(minor_status, internal_ctx,
                                                  output_token);
        else
            status = GSS_S_UNAVAILABLE;
 ...
 */
OM_uint32
    gssint_convert_name_to_union_name(minor_status, mech, internal_name,
                                      external_name) OM_uint32* minor_status;
gss_mechanism mech;
gss_name_t internal_name;
gss_name_t* external_name;
{
    ...
    major_status = mech->gss_display_name(minor_status, internal_name,
 ...
 */
gss_cred_id_t gssint_get_mechanism_cred(union_cred,
                                        mech_type) gss_union_cred_t union_cred;
gss_OID mech_type;
{
    int i;
 ...
 */
OM_uint32 gssint_create_copy_buffer(srcBuf, destBuf,
                                    addNullChar) const gss_buffer_t srcBuf;
