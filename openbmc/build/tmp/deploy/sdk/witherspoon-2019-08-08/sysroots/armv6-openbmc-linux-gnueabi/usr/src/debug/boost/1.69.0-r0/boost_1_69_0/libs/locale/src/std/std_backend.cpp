...
class std_localization_backend : public localization_backend
{
  public:
    ...
    std_localization_backend(std_localization_backend const& other) :
        localization_backend(), paths_(other.paths_), domains_(other.domains_),
        locale_id_(other.locale_id_), invalid_(true),
        ...
        if (!data_.utf8)
        {
            if (loadable(lid))
            {
                ...
                utf_mode_ = utf8_none;
                     ...
                     win_codepage == conv::impl::encoding_to_windows_codepage(
                                         data_.encoding.c_str()))
            {
                name_ = win_name;
                utf_mode_ = utf8_none;
            }
    ...
    std::pair<std::string, int> to_windows_name(std::string const& l)
    {
        ...
        if (GetLocaleInfoA(lcid, LOCALE_SENGLANGUAGE, win_lang,
                           sizeof(win_lang)) == 0)
            return res;
        std::string lc_name = win_lang;
        if (GetLocaleInfoA(lcid, LOCALE_SENGCOUNTRY, win_country,
                           sizeof(win_country)) != 0)
        {
            lc_name += "_";
            lc_name += win_country;
        }
        ...
        if (GetLocaleInfoA(lcid, LOCALE_IDEFAULTANSICODEPAGE, win_codepage,
                           sizeof(win_codepage)) != 0)
                                ...
                                character_facet_type type = nochar_facet)
    {
        ...
        switch (category)
        {
            ...
            case message_facet:
            {
                ...
                std::copy(domains_.begin(), domains_.end(),
                          std::back_inserter<
                ...
                switch (type)
                {
                    case char_facet:
                        return std::locale(
                            base,
                            gnu_gettext::create_messages_facet<char>(minf));
                    case wchar_t_facet:
                        return std::locale(
                            base,
                            gnu_gettext::create_messages_facet<wchar_t>(minf));
#ifdef BOOST_LOCALE_ENABLE_CHAR16_T
                    case char16_t_facet:
                        return std::locale(
                            base,
                            gnu_gettext::create_messages_facet<char16_t>(minf));
#endif
#ifdef BOOST_LOCALE_ENABLE_CHAR32_T
                    case char32_t_facet:
                        return std::locale(
                            base,
                            gnu_gettext::create_messages_facet<char32_t>(minf));
#endif
                    default:
                        return base;
                }
