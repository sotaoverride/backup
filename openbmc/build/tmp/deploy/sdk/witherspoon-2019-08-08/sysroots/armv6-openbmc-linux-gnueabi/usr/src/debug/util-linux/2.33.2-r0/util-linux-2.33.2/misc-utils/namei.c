...
static void free_namei(struct namei* nm)
{
    while (nm)
    {
...
static void readlink_to_namei(struct namei* nm, const char* path)
{
    char sym[PATH_MAX];
    ssize_t sz;
    int isrel = 0;
...
static struct stat* dotdot_stat(const char* dirname, struct stat* st)
{
    char* path;
    size_t len;
...
static struct namei* new_namei(struct namei* parent, const char* path,
                               const char* fname, int lev)
{
    struct namei* nm;
...
static struct namei* add_namei(struct namei* parent, const char* orgpath,
                               int start, struct namei** last)
{
...
static int follow_symlinks(struct namei* nm)
{
    int symcount = 0;
...
static int print_namei(struct namei* nm, char* path)
{
    int i;
    ...
    for (; nm; nm = nm->next)
    {
        ...
        if (flags & NAMEI_OWNERS)
        {
            printf(" %-*s", ucache->width, get_id(ucache, nm->st.st_uid)->name);
            printf(" %-*s", gcache->width, get_id(gcache, nm->st.st_gid)->name);
        }
        ...
        if (S_ISLNK(nm->st.st_mode))
            printf(" %s -> %s\n", nm->name, nm->abslink + nm->relstart);
        else
            printf(" %s\n", nm->name);
    }
    return 0;
...
static void __attribute__((__noreturn__)) usage(void)
{
    ...
    fprintf(out, _(" %s [options] <pathname>...\n"), p);
