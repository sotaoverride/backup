        ...
        netsnmp_feature_child_of(mib_snprint, mib_strings_all)
            netsnmp_feature_child_of(mib_snprint_description, mib_strings_all)
                netsnmp_feature_child_of(mib_snprint_variable, mib_strings_all)
                    netsnmp_feature_child_of(mib_string_conversions,
                                             mib_strings_all)
                        netsnmp_feature_child_of(print_mib, mib_strings_all)
                            netsnmp_feature_child_of(snprint_objid,
                                                     mib_strings_all)
                                netsnmp_feature_child_of(snprint_value,
                                                         mib_strings_all)

                                    netsnmp_feature_child_of(mib_to_asn_type,
                                                             mib_api)
...
static struct tree* _get_realloc_symbol(const oid* objid, size_t objidlen,
                                        struct tree* subtree, u_char** buf,
                                        size_t* buf_len, size_t* out_len,
                                        int allow_realloc, int* buf_overflow,
                                        struct index_list* in_dices,
                                        size_t* end_of_known);

static int print_tree_node(u_char** buf, size_t* buf_len, size_t* out_len,
                           int allow_realloc, struct tree* tp, int width);
...
static char* uptimeString(u_long timeticks, char* buf, size_t buflen)
{
    ...
    if (netsnmp_ds_get_boolean(NETSNMP_DS_LIBRARY_ID,
        ...
        snprintf(buf, buflen, "%d:%d:%02d:%02d.%02d", days, hours, minutes,
                 seconds, centisecs);
    else
    {
        if (days == 0)
        {
            snprintf(buf, buflen, "%d:%02d:%02d.%02d", hours, minutes, seconds,
                     centisecs);
        }
        else if (days == 1)
        {
            snprintf(buf, buflen, "%d day, %d:%02d:%02d.%02d", days, hours,
                     minutes, seconds, centisecs);
        }
        else
        {
            snprintf(buf, buflen, "%d days, %d:%02d:%02d.%02d", days, hours,
                     minutes, seconds, centisecs);
        }
    }
    return buf;
 ...
 */
int _sprint_hexstring_line(u_char** buf, size_t* buf_len, size_t* out_len,
                           int allow_realloc, const u_char* cp, size_t line_len)
{
    ...
    for (; lenleft >= 8; lenleft -= 8)
    {
        sprintf((char*)(*buf + *out_len),
                "%02X %02X %02X %02X %02X %02X %02X %02X ", cp[0], cp[1], cp[2],
                             ...
                             int allow_realloc, const u_char* cp, size_t len)
{
    ...
    {
        if (!_sprint_hexstring_line(buf, buf_len, out_len, allow_realloc, cp,
                                    line_len))
            return 0;
        *(*buf + (*out_len)++) = '\n';
        *(*buf + *out_len) = 0;
 ...
 *         small when not allowed to realloc.)
 */
int sprint_realloc_asciistring(u_char** buf, size_t* buf_len, size_t* out_len,
                               int allow_realloc, const u_char* cp, size_t len)
{
    int i;
 ...
 *         small when not allowed to realloc.)
 */
int sprint_realloc_octet_string(u_char** buf, size_t* buf_len, size_t* out_len,
                                int allow_realloc,
                                ...
                                const char* units)
{
    ...
    if (var->type != ASN_OCTET_STR)
    {
        ...
        return sprint_realloc_by_type(buf, buf_len, out_len, allow_realloc, var,
                                      NULL, NULL, NULL);
    }
...
#define HEX2DIGIT_NEED_INIT 3
                ...
                switch (code)
                {
                    case 'x':
                        if (HEX2DIGIT_NEED_INIT == hex2digit)
                            hex2digit = netsnmp_ds_get_boolean(
                                NETSNMP_DS_LIBRARY_ID,
                                NETSNMP_DS_LIB_2DIGIT_HEX_OUTPUT);
                        /*
                         * if value is < 16, it will be a single hex digit. If
                         * the width is 1 (we are outputting a byte at a time),
                         * pat it to 2 digits if
                         * NETSNMP_DS_LIB_2DIGIT_HEX_OUTPUT is set or all of the
                         * following are true:
                    ...
                    case 'a':
                        /* A string hint gives the max size - we may not need
                         * this much */
                        ...
                        else if (!sprint_realloc_asciistring(
                                     buf, buf_len, out_len, allow_realloc, cp,
                                     cnt))
                        {
                            return 0;
                        }
                        ...
                        *out_len = saved_out_len;
                        if (snmp_cstrcat(buf, buf_len, out_len, allow_realloc,
                            ...
                            snmp_cstrcat(buf, buf_len, out_len, allow_realloc,
                                         ") "))
                        {
                            return sprint_realloc_octet_string(
                                buf, buf_len, out_len, allow_realloc, var,
                                enums, NULL, NULL);
                        }
                        else
                        {
    ...
    output_format = netsnmp_ds_get_int(NETSNMP_DS_LIBRARY_ID,
 ...
 *         small when not allowed to realloc.)
 */
int sprint_realloc_float(u_char** buf, size_t* buf_len, size_t* out_len,
                         int allow_realloc, const netsnmp_variable_list* var,
                         const struct enum_list* enums, const char* hint,
                         const char* units)
{
 ...
 *         small when not allowed to realloc.)
 */
int sprint_realloc_double(u_char** buf, size_t* buf_len, size_t* out_len,
                          int allow_realloc, const netsnmp_variable_list* var,
                          const struct enum_list* enums, const char* hint,
                          const char* units)
{
 ...
 *         small when not allowed to realloc.)
 */
int sprint_realloc_counter64(u_char** buf, size_t* buf_len, size_t* out_len,
                             int allow_realloc,
                             const netsnmp_variable_list* var,
                             const struct enum_list* enums, const char* hint,
                             const char* units)
...
#ifdef NETSNMP_WITH_OPAQUE_SPECIAL_TYPES
        && var->type != ASN_OPAQUE_COUNTER64 && var->type != ASN_OPAQUE_I64 &&
 ...
 *         small when not allowed to realloc.)
 */
int sprint_realloc_opaque(u_char** buf, size_t* buf_len, size_t* out_len,
                          int allow_realloc, const netsnmp_variable_list* var,
                          const struct enum_list* enums, const char* hint,
                          const char* units)
{
...
#ifdef NETSNMP_WITH_OPAQUE_SPECIAL_TYPES
        && var->type != ASN_OPAQUE_COUNTER64 && var->type != ASN_OPAQUE_U64 &&
        var->type != ASN_OPAQUE_I64 && var->type != ASN_OPAQUE_FLOAT &&
    ...
    switch (var->type)
    {
        ...
        case ASN_OPAQUE_U64:
        case ASN_OPAQUE_I64:
            return sprint_realloc_counter64(
                buf, buf_len, out_len, allow_realloc, var, enums, hint, units);
            break;
                                     ...
                                     const char* hint, const char* units)
{
    ...
    netsnmp_sprint_realloc_objid_tree(buf, buf_len, out_len, allow_realloc,
                                      &buf_overflow, (oid*)(var->val.objid),
 ...
 *         small when not allowed to realloc.)
 */
int sprint_realloc_timeticks(u_char** buf, size_t* buf_len, size_t* out_len,
                             int allow_realloc,
                             const netsnmp_variable_list* var,
                             const struct enum_list* enums, const char* hint,
                             const char* units)
...
int sprint_realloc_hinted_integer(u_char** buf, size_t* buf_len,
                                  size_t* out_len, int allow_realloc, long val,
                                  const char decimaltype, const char* hint,
                                  const char* units)
{
 ...
 *         small when not allowed to realloc.)
 */
int sprint_realloc_integer(u_char** buf, size_t* buf_len, size_t* out_len,
                           int allow_realloc, const netsnmp_variable_list* var,
                           const struct enum_list* enums, const char* hint,
                           const char* units)
{
                               ...
                               NETSNMP_DS_LIB_PRINT_NUMERIC_ENUM))
    {
        if (hint)
        {
            if (!(sprint_realloc_hinted_integer(
                    buf, buf_len, out_len, allow_realloc, *var->val.integer,
                    'd', hint, units)))
            {
                return 0;
            }
 ...
 *         small when not allowed to realloc.)
 */
int sprint_realloc_uinteger(u_char** buf, size_t* buf_len, size_t* out_len,
                            int allow_realloc, const netsnmp_variable_list* var,
                            const struct enum_list* enums, const char* hint,
                            const char* units)
{
 ...
 *         small when not allowed to realloc.)
 */
int sprint_realloc_gauge(u_char** buf, size_t* buf_len, size_t* out_len,
                         int allow_realloc, const netsnmp_variable_list* var,
                         const struct enum_list* enums, const char* hint,
                         const char* units)
{
    ...
    if (hint)
    {
        if (!sprint_realloc_hinted_integer(buf, buf_len, out_len, allow_realloc,
                                           *var->val.integer, 'u', hint, units))
        {
            return 0;
        }
    }
 ...
 *         small when not allowed to realloc.)
 */
int sprint_realloc_counter(u_char** buf, size_t* buf_len, size_t* out_len,
                           int allow_realloc, const netsnmp_variable_list* var,
                           const struct enum_list* enums, const char* hint,
                           const char* units)
{
 ...
 *         small when not allowed to realloc.)
 */
int sprint_realloc_ipaddress(u_char** buf, size_t* buf_len, size_t* out_len,
                             int allow_realloc,
                             const netsnmp_variable_list* var,
                             const struct enum_list* enums, const char* hint,
                             const char* units)
 ...
 *         small when not allowed to realloc.)
 */
int sprint_realloc_null(u_char** buf, size_t* buf_len, size_t* out_len,
                        int allow_realloc, const netsnmp_variable_list* var,
                        const struct enum_list* enums, const char* hint,
                        const char* units)
{
 ...
 *         small when not allowed to realloc.)
 */
int sprint_realloc_bitstring(u_char** buf, size_t* buf_len, size_t* out_len,
                             int allow_realloc,
                             const netsnmp_variable_list* var,
                             const struct enum_list* enums, const char* hint,
                             const char* units)
...
int sprint_realloc_nsapaddress(u_char** buf, size_t* buf_len, size_t* out_len,
                               int allow_realloc,
                               const netsnmp_variable_list* var,
                               const struct enum_list* enums, const char* hint,
                               const char* units)
...
/**
 * Fallback routine for a bad type, prints "Variable has bad type" into a
 * buffer.
 *
 * If allow_realloc is true the buffer will be (re)allocated to fit in the
 * needed size. (Note: *buf may change due to this.)
 ...
 *         small when not allowed to realloc.)
 */
int sprint_realloc_badtype(u_char** buf, size_t* buf_len, size_t* out_len,
                           int allow_realloc, const netsnmp_variable_list* var,
                           const struct enum_list* enums, const char* hint,
                           const char* units)
{
...
/**
 * Universal print routine, prints a variable into a buffer according to the
 * variable type.
 *
 * If allow_realloc is true the buffer will be (re)allocated to fit in the
 * needed size. (Note: *buf may change due to this.)
 ...
 *         small when not allowed to realloc.)
 */
int sprint_realloc_by_type(u_char** buf, size_t* buf_len, size_t* out_len,
                           int allow_realloc, const netsnmp_variable_list* var,
                           const struct enum_list* enums, const char* hint,
                           const char* units)
{
    ...
    switch (var->type)
    {
        ...
        case ASN_OCTET_STR:
            return sprint_realloc_octet_string(
                buf, buf_len, out_len, allow_realloc, var, enums, hint, units);
        case ASN_BIT_STR:
            return sprint_realloc_bitstring(
                buf, buf_len, out_len, allow_realloc, var, enums, hint, units);
        case ASN_OPAQUE:
            return sprint_realloc_opaque(buf, buf_len, out_len, allow_realloc,
                                         var, enums, hint, units);
        case ASN_OBJECT_ID:
            return sprint_realloc_object_identifier(
                buf, buf_len, out_len, allow_realloc, var, enums, hint, units);
        case ASN_TIMETICKS:
            return sprint_realloc_timeticks(
                buf, buf_len, out_len, allow_realloc, var, enums, hint, units);
        case ASN_GAUGE:
        ...
        case ASN_COUNTER:
            return sprint_realloc_counter(buf, buf_len, out_len, allow_realloc,
                                          var, enums, hint, units);
        case ASN_IPADDRESS:
            return sprint_realloc_ipaddress(
                buf, buf_len, out_len, allow_realloc, var, enums, hint, units);
        case ASN_NULL:
 ...
 *
 * Example:
 * If the original format string is "%f", and the
 * NETSNMP_DS_LIB_OUTPUT_PRECISION is "5.2", the returned format string will be
 * "%5.2f".
 *
 ...
 *
 * @return The address of of the new allocated format string (which must be
 * freed if no longer used), or NULL if any error (malloc).
 */
char* make_printf_format_string(const char* printf_format_default)
{
    ...
    printf_precision = netsnmp_ds_get_string(NETSNMP_DS_LIBRARY_ID,
    ...
    printf_format_string = (char*)malloc(strlen(printf_format_default) +
                                         strlen(printf_precision) + 1);
    if (!printf_format_string)
    {
...
static void handle_mibdirs_conf(const char* token, char* line)
{
    ...
    if (confmibdir)
    {
        if ((*line == '+') || (*line == '-'))
        {
            ...
            {
                DEBUGMSGTL(
                    ("read_config:initmib", "mibdir conf malloc failed"));
                return;
            }
            if (*line++ == '+')
...
static void handle_print_numeric(const char* token, char* line)
{
    ...
    if (value && ((strcasecmp(value, "yes") == 0) ||
                  (strcasecmp(value, "true") == 0) || (*value == '1')))
    {
        ...
        switch (*options++)
        {
            ...
            case 'p':
                ...
                netsnmp_ds_set_string(NETSNMP_DS_LIBRARY_ID,
                                      NETSNMP_DS_LIB_OUTPUT_PRECISION, options);
                return NULL; /* -Op... is a standalone option, so we're done
                                here */
            case 'q':
                netsnmp_ds_toggle_boolean(NETSNMP_DS_LIBRARY_ID,
                                          NETSNMP_DS_LIB_QUICK_PRINT);
...
void snmp_out_toggle_options_usage(const char* lead, FILE* outf)
{
    ...
    fprintf(outf,
            "%sp PRECISION:  display floating point values with specified "
            "PRECISION (printf format string)\n",
            lead);
        ...
        switch (*cp)
        {
            ...
            case 's':
                ...
                if (!*(++cp))
                    cp = argv[optind++];
                netsnmp_ds_set_string(NETSNMP_DS_LIBRARY_ID,
                                      NETSNMP_DS_LIB_OIDSUFFIX, cp);
                return NULL; /* -Is... is a standalone option, so we're done
                                here */
...
{
#ifndef NETSNMP_DISABLE_MIB_LOADING
    register_prenetsnmp_mib_handler("snmp", "mibdirs", handle_mibdirs_conf,
                                    NULL, "[mib-dirs|+mib-dirs|-mib-dirs]");
    register_prenetsnmp_mib_handler("snmp", "mibs", handle_mibs_conf, NULL,
    ...
    netsnmp_ds_register_premib(
        ASN_BOOLEAN, "snmp", "strictCommentTerm", /* Backward compatibility */
        NETSNMP_DS_LIBRARY_ID, NETSNMP_DS_LIB_MIB_COMMENT_TERM);
...
void netsnmp_set_mib_directory(const char* dir)
{
    ...
    olddir =
        netsnmp_ds_get_string(NETSNMP_DS_LIBRARY_ID, NETSNMP_DS_LIB_MIBDIRS);
    if (olddir)
    {
        if ((*dir == '+') || (*dir == '-'))
        {
...
char* netsnmp_get_mib_directory(void)
{
    ...
    if (dir == NULL)
    {
        ...
        {
            DEBUGMSGTL(("get_mib_directory",
                        "mib directories set by environment (but added)\n"));
            netsnmp_set_mib_directory(NETSNMP_DEFAULT_MIBDIRS);
        ...
        {
            DEBUGMSGTL(
                ("get_mib_directory", "mib directories set by environment\n"));
            netsnmp_set_mib_directory(dir);
        }
        dir = netsnmp_ds_get_string(NETSNMP_DS_LIBRARY_ID,
...
void netsnmp_init_mib(void)
{
    ...
    if (env_var && ((*env_var == '+') || (*env_var == '-')))
    {
        ...
        {
            if (*env_var == '+')
                sprintf(entry, "%s%c%s", NETSNMP_DEFAULT_MIBS,
    ...
    if (env_var != NULL)
    {
        if ((*env_var == '+') || (*env_var == '-'))
        {
            ...
            entry = (char*)malloc(strlen(NETSNMP_DEFAULT_MIBFILES) +
                                  strlen(env_var) + 2);
            if (!entry)
            {
            ...
            {
                if (*env_var++ == '+')
                    sprintf(entry, "%s%c%s", NETSNMP_DEFAULT_MIBFILES,
    ...
    DEBUGMSGTL(
        ("init_mib", "Seen PREFIX: Looking in '%s' for prefix ...\n", Prefix));
...
int _mibindex_add(const char* dirname, int i)
{
    ...
    if (i >= _mibindex_max)
    {
        ...
        _mibindex_max = i + 10;
        _mibindexes =
            realloc(_mibindexes, _mibindex_max * sizeof(_mibindexes[0]));
...
{ /* number of subid's in "output" */
    ...
    *out_len = 0;
#ifndef NETSNMP_DISABLE_MIB_LOADING
    if ((ret = _add_strings_to_oid(root, name, output, out_len, max_out_len)) <=
        0)
#else
    if ((ret = _add_strings_to_oid(NULL, name, output, out_len, max_out_len)) <=
        0)
#endif /* NETSNMP_DISABLE_MIB_LOADING */
    {
        if (ret == 0)
 ...
 *
 */
void netsnmp_sprint_realloc_objid(u_char** buf, size_t* buf_len,
                                  size_t* out_len, int allow_realloc,
                                  ...
                                  size_t objidlen)
{
    ...
    _oid_finish_printing(objid, objidlen, &tbuf, &tbuf_len, &tout_len,
...
void netsnmp_sprint_realloc_objid_tree(u_char** buf, size_t* buf_len,
                                       size_t* out_len, int allow_realloc,
                                       int* buf_overflow, const oid* objid,
                                       size_t objidlen)
{
...
#else
struct tree* netsnmp_sprint_realloc_objid_tree(
    u_char** buf, size_t* buf_len, size_t* out_len, int allow_realloc,
    int* buf_overflow, const oid* objid, size_t objidlen)
{
    u_char *tbuf = NULL, *cp = NULL;
    ...
    subtree = _get_realloc_symbol(objid, objidlen, subtree, &tbuf, &tbuf_len,
                                  &tout_len, allow_realloc, &tbuf_overflow,
                                  NULL, &midpoint_offset);
    ...
    switch (output_format)
    {
        ...
        case NETSNMP_OID_OUTPUT_MODULE:
            ...
            if ((NETSNMP_OID_OUTPUT_MODULE == output_format) && cp > tbuf)
            {
                char modbuf[256] = {0},
                     *mod = module_name(subtree->modid, modbuf);

                /*
                 * Don't add the module ID if it's just numeric (i.e. we
                 * couldn't look it up properly.
                 */
...
int sprint_realloc_objid(u_char** buf, size_t* buf_len, size_t* out_len,
                         int allow_realloc, const oid* objid, size_t objidlen)
{
    int buf_overflow = 0;
...
int snprint_objid(char* buf, size_t buf_len, const oid* objid, size_t objidlen)
{
    ...
    if (sprint_realloc_objid((u_char**)&buf, &buf_len, &out_len, 0, objid,
                             objidlen))
    {
        return (int)out_len;
    }
...
int sprint_realloc_variable(u_char** buf, size_t* buf_len, size_t* out_len,
                            int allow_realloc, const oid* objid,
                            size_t objidlen,
                            const netsnmp_variable_list* variable)
{
    ...
    {
        return snmp_strcat(buf, buf_len, out_len, allow_realloc,
                           (const u_char*)"No Such Object available on this "
                                          "agent at this OID");
    }
    ...
    else if (variable->type == SNMP_ENDOFMIBVIEW)
    {
        return snmp_strcat(
            buf, buf_len, out_len, allow_realloc,
            (const u_char*)"No more variables left in this MIB View (It is "
...
int snprint_variable(char* buf, size_t buf_len, const oid* objid,
                     size_t objidlen, const netsnmp_variable_list* variable)
{
    size_t out_len = 0;

    if (sprint_realloc_variable((u_char**)&buf, &buf_len, &out_len, 0, objid,
                                objidlen, variable))
    {
        return (int)out_len;
    }
 ...
 * @param variable  The variable to print.
 */
void fprint_variable(FILE* f, const oid* objid, size_t objidlen,
                     const netsnmp_variable_list* variable)
{
    u_char* buf = NULL;
    size_t buf_len = 256, out_len = 0;
    ...
    {
        if (sprint_realloc_variable(&buf, &buf_len, &out_len, 1, objid,
                                    objidlen, variable))
        {
            fprintf(f, "%s\n", buf);
        }
...
int sprint_realloc_value(u_char** buf, size_t* buf_len, size_t* out_len,
                         int allow_realloc, const oid* objid, size_t objidlen,
                         const netsnmp_variable_list* variable)
{
    if (variable->type == SNMP_NOSUCHOBJECT)
    ...
    else
    {
        ...
        if (subtree)
        {
            if (subtree->printomat)
            {
                return (*subtree->printomat)(
                    buf, buf_len, out_len, allow_realloc, variable,
                    subtree->enums, subtree->hint, units);
            }
            else
            {
...
int snprint_value(char* buf, size_t buf_len, const oid* objid, size_t objidlen,
                  const netsnmp_variable_list* variable)
{
    size_t out_len = 0;

    if (sprint_realloc_value((u_char**)&buf, &buf_len, &out_len, 0, objid,
                             objidlen, variable))
    {
        return (int)out_len;
    }
...
void fprint_value(FILE* f, const oid* objid, size_t objidlen,
                  const netsnmp_variable_list* variable)
{
    u_char* buf = NULL;
    size_t buf_len = 256, out_len = 0;
    ...
    {
        if (sprint_realloc_value(&buf, &buf_len, &out_len, 1, objid, objidlen,
                                 variable))
        {
            fprintf(f, "%s\n", buf);
        }
...
int build_oid_segment(netsnmp_variable_list* var)
{
    ...
    switch (var->type)
    {
        ...
        default:
            DEBUGMSGTL(
                ("build_oid_segment", "invalid asn type: %d\n", var->type));
            return SNMPERR_GENERR;
    }
...
int build_oid_noalloc(oid* in, size_t in_len, size_t* out_len, oid* prefix,
              ...
              netsnmp_variable_list* indexes)
{
     ...
     */
    if (build_oid_noalloc(tmpout, sizeof(tmpout), out_len, prefix, prefix_len,
                          indexes) != SNMPERR_SUCCESS)
        return SNMPERR_GENERR;
    ...
    else
    {
        switch (var->type)
        {
            ...
            case ASN_IPADDRESS:
                ...
                for (i = 0; i < 4 && i < *oidLen; ++i)
                {
                    if (oidIndex[i] > 255)
                    {
                        DEBUGMSGTL(("parse_oid_indexes",
                                    "illegal oid in index: %" NETSNMP_PRIo
                                    "d\n",
                                    oidIndex[0]));
                ...
                uitmp =
                    htonl(uitmp); /* put it in proper order for byte copies */
            ...
            case ASN_PRIV_IMPLIED_OBJECT_ID:
                if (var->type == ASN_PRIV_IMPLIED_OBJECT_ID)
                {
                    ...
                    {
                        DEBUGMSGTL(
                            ("parse_oid_indexes:fix", "fixed len oid\n"));
                        uitmp = var->val_len;
                    }
                }
                else
                ...
                DEBUGMSGOID(("parse_oid_indexes", var->val.objid,
                             var->val_len / sizeof(oid)));
            ...
            case ASN_PRIV_IMPLIED_OCTET_STR:
                if (var->type == ASN_PRIV_IMPLIED_OCTET_STR)
                {
                    ...
                    {
                        DEBUGMSGTL(
                            ("parse_oid_indexes:fix", "fixed len str\n"));
                        uitmp = var->val_len;
                    }
                }
                else
            ...
            default:
                DEBUGMSGTL(
                    ("parse_oid_indexes", "invalid asn type: %d\n", var->type));
                return SNMPERR_GENERR;
        }
    }
    (*oidStart) = oidIndex;
...
int dump_realloc_oid_to_inetaddress(const int addr_type, const oid* objid,
                                    size_t objidlen, u_char** buf,
                                    size_t* buf_len, size_t* out_len,
    ...
    *p++ = quotechar;
    ...
    switch (addr_type)
    {
        ...
        case IPV6Z:
            ...
            for (i = 0; i < 16; i++)
            {
                len = snprintf(p, end - p, "%s%02" NETSNMP_PRIo "x",
                               i ? ":" : "", objid[i]);
                p += len;
                if (p >= end)
                    return 2;
...
int dump_realloc_oid_to_string(const oid* objid, size_t objidlen, u_char** buf,
                               size_t* buf_len, size_t* out_len,
                               int allow_realloc, char quotechar)
{
    if (buf)
...
void _oid_finish_printing(const oid* objid, size_t objidlen, u_char** buf,
                          size_t* buf_len, size_t* out_len, int allow_realloc,
                          int* buf_overflow)
{
    char intbuf[64];
    if (*buf != NULL && *(*buf + *out_len - 1) != '.')
    {
        if (!*buf_overflow && !snmp_strcat(buf, buf_len, out_len, allow_realloc,
                                           (const u_char*)"."))
        {
            *buf_overflow = 1;
        }
...
#ifndef NETSNMP_DISABLE_MIB_LOADING
static void _get_realloc_symbol_octet_string(size_t numids, const oid* objid,
                                             ...
                                             int* buf_overflow, struct tree* tp)
{
    ...
    if (!*buf_overflow)
    {
        if (!sprint_realloc_octet_string(buf, buf_len, out_len, allow_realloc,
                                         &var, NULL, tp->hint, NULL))
        {
            *buf_overflow = 1;
        }
    }
                                        ...
                                        size_t* end_of_known)
{
    ...
    int extended_index = netsnmp_ds_get_boolean(NETSNMP_DS_LIBRARY_ID,
                                                NETSNMP_DS_LIB_EXTENDED_INDEX);
    int output_format = netsnmp_ds_get_int(NETSNMP_DS_LIBRARY_ID,
                                           NETSNMP_DS_LIB_OID_OUTPUT_FORMAT);
    char intbuf[64];
    ...
    for (; subtree; subtree = subtree->next_peer)
    {
        if (*objid == subtree->subid)
        {
            ...
            if (objidlen > 1)
            {
                ...
                return_tree = _get_realloc_symbol(
                    objid + 1, objidlen - 1, subtree->child_list, buf, buf_len,
                    out_len, allow_realloc, buf_overflow, in_dices,
                    end_of_known);
            }
        ...
        switch (tp->type)
        {
            case TYPE_OCTETSTR:
                if (extended_index && tp->hint)
                {
                    ...
                    if (numids > objidlen)
                        goto finish_it;
                    _get_realloc_symbol_octet_string(
                        numids, objid, buf, buf_len, out_len, allow_realloc,
                        buf_overflow, tp);
                }
                else if (in_dices->isimplied)
                {
                    ...
                    if (!*buf_overflow)
                    {
                        if (!dump_realloc_oid_to_string(objid, numids, buf,
                                                        buf_len, out_len,
                                                        allow_realloc, '\''))
                        {
                            *buf_overflow = 1;
                ...
                else
                {
                    ...
                    else
                    {
                        if (!*buf_overflow)
                        {
                            ...
                            /* Try handling the InetAddress in the OID, in case
                             * of failure, use the normal_handling.
                             */
                            if (tp->next_peer && tp->tc_index != -1 &&
                                next_peer->tc_index != -1 &&
                                strcmp(get_tc_descriptor(tp->tc_index),
                                       "InetAddress") == 0 &&
                                       ...
                                       "InetAddressType") == 0)
                            {
                                ...
                                ret = dump_realloc_oid_to_inetaddress(
                                    addr_type, objid + 1, numids - 1, buf,
...
finish_it:
    _oid_finish_printing(objid, objidlen, buf, buf_len, out_len, allow_realloc,
                         buf_overflow);
    return NULL;
}
...
void fprint_description(FILE* f, oid* objid, size_t objidlen, int width)
{
    ...
    {
        if (!sprint_realloc_description(&buf, &buf_len, &out_len, 1, objid,
                                        objidlen, width))
        {
            fprintf(f, "%s [TRUNCATED]\n", buf);
        }
...
int snprint_description(char* buf, size_t buf_len, oid* objid, size_t objidlen,
                        int width)
{
    size_t out_len = 0;

    if (sprint_realloc_description((u_char**)&buf, &buf_len, &out_len, 0, objid,
                                   objidlen, width))
    {
        return (int)out_len;
    }
...
int sprint_realloc_description(u_char** buf, size_t* buf_len, size_t* out_len,
                               int allow_realloc, oid* objid, size_t objidlen,
                               int width)
{
    ...
    while (objidlen > 1)
    {
        for (; subtree; subtree = subtree->next_peer)
        {
            if (*objid == subtree->subid)
            {
                while (subtree->next_peer &&
                       subtree->next_peer->subid == *objid)
                    subtree = subtree->next_peer;
                if (strncmp(subtree->label, ANON, ANON_LEN))
                {
                    snprintf(tmpbuf, sizeof(tmpbuf), " %s(%lu)", subtree->label,
                             subtree->subid);
                    tmpbuf[sizeof(tmpbuf) - 1] = 0;
                }
                else
                           ...
                           int allow_realloc, struct tree* tp, int width)
{
    ...
    if (tp)
    {
        ...
        { /* ranges without type ? */
            sprintf(str, "?0 with %s %s ?", tp->ranges ? "Range" : "",
                    tp->enums ? "Enum" : "");
            cp = str;
        }
        ...
        if (tp->indexes)
        {
            ...
            while (ip)
            {
                ...
                snprintf(str, sizeof(str), "%s%s",
                         ip->isimplied ? "IMPLIED " : "", ip->ilabel);
                str[sizeof(str) - 1] = 0;
                len = strlen(str);
                if (pos + len + 2 > width)
                {
...
int get_module_node(const char* fname, const char* module, oid* objid,
                    size_t* objidlen)
{
    int modid, rc = 0;
    struct tree* tp;
...
static int
#ifndef NETSNMP_DISABLE_MIB_LOADING
    _add_strings_to_oid(struct tree* tp, char* cp, oid* objid, size_t* objidlen,
                        size_t maxlen)
#else
    _add_strings_to_oid(void* tp, char* cp, oid* objid, size_t* objidlen,
                        size_t maxlen)
#endif /* NETSNMP_DISABLE_MIB_LOADING */
{
    oid subid;
            ...
            case TYPE_OCTETSTR:
                ...
                if (*cp == '"' || *cp == '\'')
                {
                    ...
                    else
                    {
                        ...
                        if (do_hint && tp->hint)
                        {
                            parsed_value = parse_octet_hint(
                                tp->hint, cp, &new_val, &new_val_len);
...
#ifndef NETSNMP_DISABLE_MIB_LOADING
    if (in_dices)
        snprintf(buf, sizeof(buf), "Index out of range: %s (%s)", fcp,
                 in_dices->ilabel);
    else if (tp)
        snprintf(buf, sizeof(buf), "Sub-id not found: %s -> %s", tp->label,
                 fcp);
    else
#endif /* NETSNMP_DISABLE_MIB_LOADING */
...
main(int argc, char* argv[])
{
    ...
    for (argc--; argc; argc--, argv++)
    {
        ...
        printf("read_objid(%s) = %d\n", argv[1],
               read_objid(argv[1], objid, &objidlen));
...
oid* snmp_parse_oid(const char* argv, oid* root, size_t* rootlen)
{
    ...
    suffix =
        netsnmp_ds_get_string(NETSNMP_DS_LIBRARY_ID, NETSNMP_DS_LIB_OIDSUFFIX);
    prefix =
        netsnmp_ds_get_string(NETSNMP_DS_LIBRARY_ID, NETSNMP_DS_LIB_OIDPREFIX);
...
static int parse_hints_add_result_octet(struct parse_hints* ph,
                                        unsigned char octet)
{
            ...
            case 'o':
                base += 8; /* fall through */
                {
                    ...
                    for (i = 0; i < ph->length; i++)
                    {
                        ...
                        if (!parse_hints_add_result_octet(
                                ph, (u_char)(number >> shift)))
                        {
                            return 0; /* failed */
                        }
                    }
...
{
    size_t out_len = 0;
    if (sprint_realloc_by_type((u_char**)&buf, &buf_len, &out_len, 0, var,
                               enums, hint, units))
        return (int)out_len;
    else
        return -1;
...
{
    size_t out_len = 0;
    if (sprint_realloc_hexstring((u_char**)&buf, &buf_len, &out_len, 0, cp,
                                 len))
        return (int)out_len;
    else
        return -1;
...
{
    size_t out_len = 0;
    if (sprint_realloc_asciistring((u_char**)&buf, &buf_len, &out_len, 0, cp,
                                   len))
        return (int)out_len;
    else
        return -1;
...
{
    size_t out_len = 0;
    if (sprint_realloc_octet_string((u_char**)&buf, &buf_len, &out_len, 0, var,
                                    enums, hint, units))
        return (int)out_len;
    else
        return -1;
...
int snprint_opaque(char* buf, size_t buf_len, const netsnmp_variable_list* var,
                   const struct enum_list* enums, const char* hint,
                   const char* units)
{
    size_t out_len = 0;
    if (sprint_realloc_opaque((u_char**)&buf, &buf_len, &out_len, 0, var, enums,
                              hint, units))
        return (int)out_len;
    else
        return -1;
...
{
    size_t out_len = 0;
    if (sprint_realloc_timeticks((u_char**)&buf, &buf_len, &out_len, 0, var,
                                 enums, hint, units))
        return (int)out_len;
    else
        return -1;
...
int snprint_hinted_integer(char* buf, size_t buf_len, long val,
                           const char* hint, const char* units)
{
    size_t out_len = 0;
...
int snprint_integer(char* buf, size_t buf_len, const netsnmp_variable_list* var,
                    const struct enum_list* enums, const char* hint,
                    const char* units)
{
    size_t out_len = 0;
    if (sprint_realloc_integer((u_char**)&buf, &buf_len, &out_len, 0, var,
                               enums, hint, units))
        return (int)out_len;
    else
        return -1;
...
{
    size_t out_len = 0;
    if (sprint_realloc_uinteger((u_char**)&buf, &buf_len, &out_len, 0, var,
                                enums, hint, units))
        return (int)out_len;
    else
        return -1;
...
int snprint_gauge(char* buf, size_t buf_len, const netsnmp_variable_list* var,
                  const struct enum_list* enums, const char* hint,
                  const char* units)
{
    size_t out_len = 0;
    if (sprint_realloc_gauge((u_char**)&buf, &buf_len, &out_len, 0, var, enums,
                             hint, units))
        return (int)out_len;
    else
        return -1;
...
int snprint_counter(char* buf, size_t buf_len, const netsnmp_variable_list* var,
                    const struct enum_list* enums, const char* hint,
                    const char* units)
{
    size_t out_len = 0;
    if (sprint_realloc_counter((u_char**)&buf, &buf_len, &out_len, 0, var,
                               enums, hint, units))
        return (int)out_len;
    else
        return -1;
...
{
    size_t out_len = 0;
    if (sprint_realloc_ipaddress((u_char**)&buf, &buf_len, &out_len, 0, var,
                                 enums, hint, units))
        return (int)out_len;
    else
        return -1;
...
int snprint_null(char* buf, size_t buf_len, const netsnmp_variable_list* var,
                 const struct enum_list* enums, const char* hint,
                 const char* units)
{
    size_t out_len = 0;
    if (sprint_realloc_null((u_char**)&buf, &buf_len, &out_len, 0, var, enums,
                            hint, units))
        return (int)out_len;
    else
        return -1;
...
{
    size_t out_len = 0;
    if (sprint_realloc_bitstring((u_char**)&buf, &buf_len, &out_len, 0, var,
                                 enums, hint, units))
        return (int)out_len;
    else
        return -1;
...
{
    size_t out_len = 0;
    if (sprint_realloc_nsapaddress((u_char**)&buf, &buf_len, &out_len, 0, var,
                                   enums, hint, units))
        return (int)out_len;
    else
        return -1;
...
{
    size_t out_len = 0;
    if (sprint_realloc_counter64((u_char**)&buf, &buf_len, &out_len, 0, var,
                                 enums, hint, units))
        return (int)out_len;
    else
        return -1;
...
int snprint_badtype(char* buf, size_t buf_len, const netsnmp_variable_list* var,
                    const struct enum_list* enums, const char* hint,
                    const char* units)
{
    size_t out_len = 0;
    if (sprint_realloc_badtype((u_char**)&buf, &buf_len, &out_len, 0, var,
                               enums, hint, units))
        return (int)out_len;
    else
        return -1;
...
#ifdef NETSNMP_WITH_OPAQUE_SPECIAL_TYPES
int snprint_float(char* buf, size_t buf_len, const netsnmp_variable_list* var,
                  const struct enum_list* enums, const char* hint,
                  const char* units)
{
    size_t out_len = 0;
    if (sprint_realloc_float((u_char**)&buf, &buf_len, &out_len, 0, var, enums,
                             hint, units))
        return (int)out_len;
    else
        return -1;
...
int snprint_double(char* buf, size_t buf_len, const netsnmp_variable_list* var,
                   const struct enum_list* enums, const char* hint,
                   const char* units)
{
    size_t out_len = 0;
    if (sprint_realloc_double((u_char**)&buf, &buf_len, &out_len, 0, var, enums,
                              hint, units))
        return (int)out_len;
    else
        return -1;
