...
#define FREE_P(descr) ((descr)->tid <= 0)
    ...
    static void stack_list_del(list_t* elem)
{
...
static void stack_list_add(list_t* elem, list_t* list)
{
...
static void free_stacks(size_t limit)
{
...
void __nptl_stacks_freeres(void)
{
    free_stacks(0);
}
...
static int change_stack_perm(struct pthread* pd
...
)
{
    ...
    void* stack = (pd->stackblock +
                   (((((pd->stackblock_size - pd->guardsize) / 2) & pagemask) +
                     pd->guardsize) &
                    pagemask));
...
static inline int setup_stack_prot(char* mem, size_t size, char* guard,
                                   size_t guardsize, const int prot)
{
...
static inline void __always_inline advise_stack_range(void* mem, size_t size,
                                                      uintptr_t pd,
                                                      size_t guardsize)
{
...
#elif TLS_DTV_AT_TP
        adj =
            ((uintptr_t)stackaddr - __static_tls_size) & __static_tls_align_m1;
        assert(size > adj);
#endif
...
#elif TLS_DTV_AT_TP
        pd =
            (struct pthread*)(((uintptr_t)stackaddr - __static_tls_size - adj) -
                              TLS_PRE_TCB_SIZE);
#endif
    ...
    else
    {
       ...
       eventually the thread descriptor.  */
...
#elif TLS_DTV_AT_TP
            pd =
                (struct pthread*)((((uintptr_t)mem + size - __static_tls_size) &
                                  ...
                                  0))
        {
            ...
            char* oldguard =
                mem + (((size - pd->guardsize) / 2) & ~pagesize_m1);
...
attribute_hidden struct pthread* __find_thread_by_id(pid_t tid)
{
    struct pthread* result = NULL;
...
#ifdef SIGSETXID
static void setxid_mark_thread(struct xid_command* cmdp, struct pthread* t)
{
    int ch;
...
/* Check for consistency across set*id system call results.  The abort
   ...
   code.  */
void attribute_hidden __nptl_setxid_error(struct xid_command* cmdp, int error)
{
    do
    {
...
int attribute_hidden __nptl_setxid(struct xid_command* cmdp)
{
    int signalled;
    int result;
    lll_lock(stack_cache_lock, LLL_PRIVATE);
    ...
    INTERNAL_SYSCALL_DECL(err);
    result = INTERNAL_SYSCALL_NCS(cmdp->syscall_no, err, 3, cmdp->id[0],
                                  cmdp->id[1], cmdp->id[2]);
    int error = 0;
...
void attribute_hidden __pthread_init_static_tls(struct link_map* map)
{
    lll_lock(stack_cache_lock, LLL_PRIVATE);
...
void attribute_hidden __wait_lookup_done(void)
{
    lll_lock(stack_cache_lock, LLL_PRIVATE);
