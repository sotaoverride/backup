...
/* static data */
DEFobjStaticHelpers DEFobjCurrIf(glbl) DEFobjCurrIf(netstrm)
    ...
    static rsRetVal loadDrvr(netstrms_t* pThis)
{
    ...
    if (snprintf((char*)szDrvrName, sizeof(szDrvrName), "lmnsd_%s",
     ...
     */
    CHKiRet(
        obj.UseObj(__FILE__, szDrvrName + 2, szDrvrName, (void*)&pThis->Drvr));
...
BEGINobjConstruct(
    netstrms) /* be sure to specify the object type also in END macro! */
    ENDobjConstruct(netstrms)

    /* destructor for the netstrms object */
    BEGINobjDestruct(netstrms) /* be sure to specify the object type also in END
    ...
    if (pThis->pDrvrName != NULL)
{
    obj.ReleaseObj(__FILE__, pThis->pDrvrName + 2, pThis->pDrvrName,
                   (void*)&pThis->Drvr);
    free(pThis->pDrvrName);
}
...
static rsRetVal SetDrvrPermPeers(netstrms_t* pThis,
...
BEGINobjQueryInterface(netstrms)
    CODESTARTobjQueryInterface(netstrms) if (pIf->ifVersion !=
                                             netstrmsCURR_IF_VERSION)
...
finalize_it : ENDobjQueryInterface(netstrms)
...
ENDObjClassExit(netstrms)
     ...
     */
    BEGINAbstractObjClassInit(netstrms, 1,
...
ENDObjClassInit(netstrms)
    ...
    BEGINmodExit CODESTARTmodExit nsselClassExit();
nspollClassExit();
netstrmsClassExit();
netstrmClassExit(); /* we use this object, so we must exit it after we are
                       finished */
ENDmodExit
