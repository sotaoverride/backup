...
static int mtd_dev_init(struct backend* backend, void* data)
{
    ...
    priv->flash_bmap = calloc(backend->flash_size >> backend->erase_size_shift,
...
int flash_validate(struct mbox_context* context, uint32_t offset, uint32_t size,
                   bool ro)
{
...
static int mtd_erase(struct backend* backend, uint32_t offset, uint32_t count)
{
    ...
    while (count)
    {
        if (!mtd_is_erased(backend, offset))
        { /* Need to erase */
            if (!erase_info.length)
            { /* Start of not-erased run */
                erase_info.start = offset;
            }
            erase_info.length += erase_size;
        }
        else if (erase_info.length)
        { /* Already erased|end of run? */
 ...
 *		error code
 */
static int mtd_reset(struct backend* backend, void* buf __attribute__((unused)),
