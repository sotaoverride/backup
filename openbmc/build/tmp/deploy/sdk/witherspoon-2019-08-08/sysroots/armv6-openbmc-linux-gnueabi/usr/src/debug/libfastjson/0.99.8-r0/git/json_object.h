 ...
 * identical to the fjson_object_dump_ext() function that you can use the
 * specify how to format the string for which the size is calculated
 ...
 * @returns number of bytes written (the sum of all return values of calls to
 * func)
 */
extern size_t fjson_object_dump(struct fjson_object* obj, fjson_write_fn* func,
                                void* ptr);
 ...
 * @returns number of bytes written
 */
extern size_t fjson_object_write_ext(struct fjson_object* obj, int flags,
                                     FILE* fp);
 ...
 * @param obj the fjson_object instance
 * @param flags formatting options, see FJSON_TO_STRING_PRETTY and other
 * constants
 * @returns a string in JSON format
 */
 ...
 */
extern void fjson_object_object_add_ex(struct fjson_object* obj,
                                       const char* key,
                                       struct fjson_object* val,
                                       const unsigned opts);
 ...
 */
THIS_FUNCTION_IS_DEPRECATED(extern struct fjson_object* fjson_object_object_get(
    struct fjson_object* obj, const char* key));
 ...
 */
extern void fjson_object_array_sort(struct fjson_object* jso,
 ...
 */
extern struct fjson_object*
    fjson_object_array_bsearch(const struct fjson_object* key,
...
/** Insert or replace an element at a specified index in an array (a
 * fjson_object of type fjson_type_array)
 *
 * The reference count will *not* be incremented. This is to make adding
 * fields to objects in code more compact. If you want to retain a reference
 * to an added object you must wrap the passed object with fjson_object_get
...
#define json_object_to_json_string_ext(a, b)                                   \
    fjson_object_to_json_string_ext((a), (b))
