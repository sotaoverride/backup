...
struct BusActivation
{
    ...
    char* server_address;
    BusContext* context;
    int n_pending_activations; /**< This is in fact the number of
                                * BusPendingActivationEntry, i.e. number of
                                * pending activation requests, not pending
                                * activations per se
                                */
    ...
    bus_pending_activation_unref(BusPendingActivation* pending_activation)
{
    ...
    if (pending_activation->babysitter)
    {
        if (!_dbus_babysitter_set_watch_functions(
                pending_activation->babysitter, NULL, NULL, NULL,
...
static dbus_bool_t update_desktop_file_entry(BusActivation* activation,
                                             BusServiceDirectory* s_dir,
                                             DBusString* filename,
                                             ...
                                             DBusError* error)
{
    ...
    if (!bus_desktop_file_get_string(desktop_file, DBUS_SERVICE_SECTION,
                                     DBUS_SERVICE_NAME, &name, error))
        goto out;

    if (!bus_desktop_file_get_string(desktop_file, DBUS_SERVICE_SECTION,
                                     DBUS_SERVICE_EXEC, &exec_tmp, error))
        goto out;
    ...
    if (!bus_desktop_file_get_string(desktop_file, DBUS_SERVICE_SECTION,
                                     DBUS_SERVICE_USER, &user, &tmp_error))
    {
    ...
    entry = _dbus_hash_table_lookup_string(
        s_dir->entries, _dbus_string_get_const_data(filename));
    ...
    if (entry == NULL) /* New file */
    {
        ...
        else if (s_dir->flags & BUS_SERVICE_DIR_FLAGS_STRICT_NAMING)
        {
            bus_context_log_and_set_error(
                activation->context, DBUS_SYSTEM_LOG_WARNING, error,
                DBUS_ERROR_FAILED,
        ...
        if (!_dbus_hash_table_insert_string(activation->entries, entry->name,
        ...
        if (!_dbus_hash_table_insert_string(s_dir->entries, entry->filename,
                                            bus_activation_entry_ref(entry)))
        {
    ...
    else /* Just update the entry */
    {
        ...
        if (_dbus_hash_table_lookup_string(activation->entries, name))
        {
            _dbus_verbose("The new service name \"%s\" of service file \"%s\" "
                          "is already in cache, ignoring\n",
                          name, _dbus_string_get_const_data(&file_path));
            dbus_set_error(error, DBUS_ERROR_FAILED,
                           "The new service name \"%s\" of service file \"%s\" "
                           "is already in cache, ignoring\n",
                           name, _dbus_string_get_const_data(&file_path));
            goto out;
        }
...
static dbus_bool_t check_service_file(BusActivation* activation,
                                      BusActivationEntry* entry,
 ...
 */
static dbus_bool_t update_directory(BusActivation* activation,
                                    BusServiceDirectory* s_dir,
                                    DBusError* error)
{
    DBusDirIter* iter;
    ...
    while (_dbus_directory_get_next_file(iter, &filename, &tmp_error))
    {
         ...
         *       Handle these both better
         */
        if (!update_desktop_file_entry(activation, s_dir, &filename,
                                       desktop_file, &tmp_error))
        {
            bus_desktop_file_free(desktop_file);
            desktop_file = NULL;
...
static dbus_bool_t populate_environment(BusActivation* activation)
{
    ...
    retval =
        _dbus_hash_table_from_array(activation->environment, environment, '=');
...
dbus_bool_t bus_activation_reload(BusActivation* activation,
                                  ...
                                  DBusList** directories, DBusError* error)
{
    DBusList* link;
    char* dir;
    ...
    activation->entries = _dbus_hash_table_new(
        DBUS_HASH_STRING, NULL, (DBusFreeFunction)bus_activation_entry_unref);
    if (activation->entries == NULL)
    {
        BUS_SET_OOM(error);
        goto failed;
    ...
    while (link != NULL)
    {
        ...
        s_dir->entries =
            _dbus_hash_table_new(DBUS_HASH_STRING, NULL,
                                  ...
                                  DBusList** directories, DBusError* error)
{
    ...
    activation->pending_activations = _dbus_hash_table_new(
        DBUS_HASH_STRING, NULL, (DBusFreeFunction)bus_pending_activation_unref);
    ...
    activation->environment =
        _dbus_hash_table_new(DBUS_HASH_STRING, (DBusFreeFunction)dbus_free,
...
static dbus_bool_t add_bus_environment(BusActivation* activation,
                                       DBusError* error)
{
    const char* type;

    if (!bus_activation_set_environment_variable(
            activation, "DBUS_STARTER_ADDRESS", activation->server_address,
            error))
        return FALSE;
    ...
    {
        if (!bus_activation_set_environment_variable(
                activation, "DBUS_STARTER_BUS_TYPE", type, error))
            return FALSE;

        if (strcmp(type, "session") == 0)
        {
            if (!bus_activation_set_environment_variable(
                    activation, "DBUS_SESSION_BUS_ADDRESS",
                    activation->server_address, error))
                return FALSE;
        }
        else if (strcmp(type, "system") == 0)
        {
            if (!bus_activation_set_environment_variable(
                    activation, "DBUS_SYSTEM_BUS_ADDRESS",
                    activation->server_address, error))
                return FALSE;
        }
    }
...
static void restore_pending(void* data)
{
    ...
    _dbus_hash_table_insert_string_preallocated(
        d->pending_activation->activation->pending_activations, d->hash_entry,
                                       ...
                                       BusPendingActivation* pending_activation)
{
    ...
    _dbus_verbose(
        "Saved pending activation to be restored if the transaction fails\n");
                                           ...
                                           DBusError* error)
{
    ...
    pending_activation = _dbus_hash_table_lookup_string(
        activation->pending_activations, service_name);

    if (!pending_activation)
        return TRUE;

    bus_context_log(activation->context, DBUS_SYSTEM_LOG_INFO,
                    "Successfully activated service '%s'", service_name);
    ...
    while (link != NULL)
    {
        BusPendingActivationEntry* entry = link->data;
        DBusList* next =
            _dbus_list_get_next_link(&pending_activation->entries, link);
            ...
            dbus_connection_get_is_connected(entry->connection))
        {
            ...
            if (!entry->auto_activation)
            {
                ...
                message =
                    dbus_message_new_method_return(entry->activation_message);
                if (!message)
                {
                    BUS_SET_OOM(error);
                    goto error;
                ...
                if (!dbus_message_append_args(message, DBUS_TYPE_UINT32,
                ...
                if (!bus_transaction_send_from_driver(
...
dbus_bool_t bus_activation_send_pending_auto_activation_messages(
    BusActivation* activation, BusService* service, BusTransaction* transaction)
{
    BusPendingActivation* pending_activation;
    DBusList* link;
    ...
    /* Check if it's a pending activation */
    pending_activation = _dbus_hash_table_lookup_string(
        activation->pending_activations, bus_service_get_name(service));
    ...
    while (link != NULL)
    {
             ...
             dbus_connection_get_is_connected(entry->connection)))
        {
            ...
            if (!bus_dispatch_matches(transaction, entry->connection,
                                      addressed_recipient,
                                      entry->activation_message, &error))
            {
                ...
                if (!bus_transaction_send_error_reply(
                        transaction, entry->connection, &error,
    ...
    {
        _dbus_verbose("Could not add cancel hook to transaction to revert "
                      "removing pending activation\n");
        goto error;
    }
 ...
 */
static void pending_activation_failed(BusPendingActivation* pending_activation,
                                      const DBusError* how)
{
    ...
    _dbus_hash_table_remove_string(
        pending_activation->activation->pending_activations,
    ...
    switch (exit_code)
    {
        ...
        case BUS_SPAWN_EXIT_CODE_FILE_INVALID:
            dbus_set_error(error, DBUS_ERROR_SPAWN_PERMISSIONS_INVALID,
                           "The service file is incorrect or does not have all "
                           "required attributes");
            break;
...
static void pending_activation_finished_cb(DBusBabysitter* babysitter,
                                           void* data)
{
    ...
    /* There are two major cases here; are we the system bus or the session?
     * Here this is distinguished by whether or not we use a setuid helper
     * launcher.  With the launch helper, some process exit codes are
     * meaningful, processed by handle_servicehelper_exit_error.
     *
     * In both cases though, just ignore when a process exits with status 0;
     * it's possible for a program to (misguidedly) "daemonize", and that
     * appears to us as an exit.  This closes a race condition between this code
     * and the child process claiming the bus name.
     */
    uses_servicehelper = bus_context_get_servicehelper(
    ...
    if (_dbus_babysitter_get_child_exited(babysitter))
    {
        ...
        if (activation_failed)
        {
            bus_context_log(pending_activation->activation->context,
                            ...
                            pending_activation->service_name, error.message);

            /* Destroy all pending activations with the same exec */
            _dbus_hash_iter_init(
                pending_activation->activation->pending_activations, &iter);
...
static dbus_bool_t add_babysitter_watch(DBusWatch* watch, void* data)
{
    ...
    return _dbus_loop_add_watch(
        bus_context_get_loop(pending_activation->activation->context), watch);
}
                                      ...
                                      BusPendingActivation* pending_activation)
{
    ...
    _dbus_verbose(
        "Saved pending activation to be canceled if the transaction fails\n");

    return TRUE;
}

static dbus_bool_t update_service_cache(BusActivation* activation,
                                        DBusError* error)
{
    DBusList* iter;
...
static BusActivationEntry* activation_find_entry(BusActivation* activation,
                                                 const char* service_name,
                                                 DBusError* error)
{
    BusActivationEntry* entry;
    ...
    if (!entry)
    {
        ...
        entry =
            _dbus_hash_table_lookup_string(activation->entries, service_name);
    }
    else
    {
        BusActivationEntry* updated_entry;
                                                    ...
                                                    DBusError* error)
{
    ...
    if (!_dbus_hash_table_insert_string(activation->environment, hash_key,
                                        hash_value))
        goto out;
...
dbus_bool_t bus_activation_activate_service(
    BusActivation* activation, DBusConnection* connection,
    BusTransaction* transaction, dbus_bool_t auto_activation,
    DBusMessage* activation_message, const char* service_name, DBusError* error)
{
    ...
    if (auto_activation && entry != NULL &&
        !bus_context_check_security_policy(activation->context, transaction,
                                           connection, /* sender */
                                           NULL,       /* addressed recipient */
                                           NULL,       /* proposed recipient */
                                           activation_message, entry, error))
    {
        _DBUS_ASSERT_ERROR_IS_SET(error);
     ...
     * not call us if the service is already active.
     */
    if (!auto_activation)
    {
        /* Check if the service is active */
        _dbus_string_init_const(&service_str, service_name);
        if (bus_registry_lookup(bus_context_get_registry(activation->context),
                                &service_str) != NULL)
        {
            dbus_uint32_t result;
            ...
            {
                _dbus_verbose(
                    "No memory to create reply to activate message\n");
                BUS_SET_OOM(error);
                return FALSE;
            }
            ...
            if (!dbus_message_append_args(message, DBUS_TYPE_UINT32, &result,
                                          DBUS_TYPE_INVALID))
            {
                _dbus_verbose(
                    "No memory to set args of reply to activate message\n");
                BUS_SET_OOM(error);
                dbus_message_unref(message);
                return FALSE;
            }

            retval = bus_transaction_send_from_driver(transaction, connection,
                                                      message);
            dbus_message_unref(message);
            if (!retval)
            {
    ...
    else
    {
        ...
        {
            _dbus_verbose(
                "Failed to copy service name for pending activation\n");
        ...
        {
            pending_activation->systemd_service =
                _dbus_strdup(entry->systemd_service);
            if (!pending_activation->systemd_service)
            {
                _dbus_verbose(
                    "Failed to copy systemd service for pending activation\n");
                BUS_SET_OOM(error);
                bus_pending_activation_unref(pending_activation);
                bus_pending_activation_entry_free(pending_activation_entry);
                return FALSE;
        ...
        pending_activation->timeout = _dbus_timeout_new(
            ...
            pending_activation_timed_out, pending_activation, NULL);
        if (!pending_activation->timeout)
        {
        ...
        {
            _dbus_verbose(
                "Failed to add entry to just-created pending activation\n");
    ...
    {
        _dbus_verbose(
            "Failed to add pending activation cancel hook to transaction\n");
        BUS_SET_OOM(error);
    ...
    if (bus_context_get_systemd_activation(activation->context))
    {
        ...
        if (entry->systemd_service)
        {
            ...
            message = dbus_message_new_signal(
                DBUS_PATH_DBUS, "org.freedesktop.systemd1.Activator",
                "ActivationRequest");
            if (!message)
            {
            ...
            _dbus_string_init_const(&service_string,
                                    "org.freedesktop.systemd1");
            ...
            if (!bus_transaction_capture(activation_transaction, NULL, systemd,
                                         message))
            {
                dbus_message_unref(message);
                BUS_SET_OOM(error);
            ...
            {
                bus_context_log(activation->context, DBUS_SYSTEM_LOG_INFO,
                                "Activating via systemd: service name='%s' "
                                "unit='%s' requested by '%s' (%s)",
                                service_name, entry->systemd_service,
            ...
            else
            {
                bus_context_log(activation->context, DBUS_SYSTEM_LOG_INFO,
                                ...
                                "name='%s' unit='%s' requested by '%s' (%s)",
                                service_name, entry->systemd_service,
                                bus_connection_get_name(connection),
                                bus_connection_get_loginfo(connection));
                /* systemd is not around, let's "activate" it. */
                retval = bus_activation_activate_service(
                    activation, NULL, activation_transaction, TRUE, message,
            ...
            {
                bus_context_log(activation->context, DBUS_SYSTEM_LOG_INFO,
                                "Failed to activate via systemd: service "
                                "name='%s' unit='%s'",
                                service_name, entry->systemd_service);
                _DBUS_ASSERT_ERROR_IS_SET(error);
    ...
    {
        /* the bus does not use a helper, so we can append arguments with the
         * exec line */
    ...
    if (!_dbus_shell_parse_argv(_dbus_string_get_const_data(&command), &argc,
                                &argv, error))
    {
    ...
    if (servicehelper != NULL)
        bus_context_log(activation->context, DBUS_SYSTEM_LOG_INFO,
                        ...
                        "(using servicehelper)",
                        service_name, bus_connection_get_name(connection),
                        bus_connection_get_loginfo(connection));
    else
        bus_context_log(activation->context, DBUS_SYSTEM_LOG_INFO,
                        "Activating service name='%s' requested by '%s' (%s)",
    ...
    if (!_dbus_spawn_async_with_babysitter(&pending_activation->babysitter,
                                           service_name, argv, envp, flags,
                                           child_setup, activation, &tmp_error))
    {
        _dbus_verbose("Failed to spawn child\n");
        bus_context_log(activation->context, DBUS_SYSTEM_LOG_INFO,
                        "Failed to activate service %s: %s", service_name,
                        tmp_error.message);
        _DBUS_ASSERT_ERROR_IS_SET(&tmp_error);
        dbus_move_error(&tmp_error, error);
        dbus_free_string_array(argv);
    ...
    if (!_dbus_babysitter_set_watch_functions(
            pending_activation->babysitter, add_babysitter_watch,
            remove_babysitter_watch, toggle_babysitter_watch,
...
dbus_bool_t bus_activation_list_services(BusActivation* activation,
                                         char*** listp, int* array_len)
{
    int i, j, len;
    char** retval;
    DBusHashIter iter;
                                            ...
                                            DBusMessage* message)
{
    ...
    if (dbus_message_get_args(message, &error, DBUS_TYPE_STRING, &unit,
                              DBUS_TYPE_STRING, &code, DBUS_TYPE_STRING, &str,
                              DBUS_TYPE_INVALID))
    ...
    if (unit)
    {
        ...
        bus_context_log(activation->context, DBUS_SYSTEM_LOG_INFO,
                        "Activation via systemd failed for unit '%s': %s", unit,
                        str);
...
static dbus_bool_t test_create_service_file(DBusString* dir,
                                            const char* filename,
...
static dbus_bool_t test_remove_service_file(DBusString* dir,
                                            const char* filename)
{
    DBusString file_name, full_path;
    dbus_bool_t ret_val;
...
static dbus_bool_t test_remove_directory(DBusString* dir)
{
    ...
    {
        if (!test_remove_service_file(dir,
                                      _dbus_string_get_const_data(&filename)))
        {
            ret_val = FALSE;
            _dbus_directory_close(iter);
...
static dbus_bool_t do_test(const char* description, dbus_bool_t oom_test,
                           CheckData* data)
{
    dbus_bool_t err;
...
static dbus_bool_t do_service_reload_test(const DBusString* test_data_dir,
                                          DBusString* dir, dbus_bool_t oom_test)
{
    BusActivation* activation;
    ...
    if (!test_create_service_file(dir, SERVICE_FILE_2, SERVICE_NAME_2,
                                  "exec-2"))
        return FALSE;
    ...
    _dbus_sleep_milliseconds(
        1000); /* Sleep a second to make sure the mtime is updated */

    if (!test_create_service_file(dir, SERVICE_FILE_1, SERVICE_NAME_3,
                                  "exec-3"))
        return FALSE;
