...
bool arg_ifindex_permissive =
    false; /* If true, don't generate an error if the specified interface index
              doesn't exist */
...
int ifname_mangle(const char* s)
{
    ...
    if (parse_ifindex(iface, &ifi) < 0)
    {
        ...
        if (ifi <= 0)
        {
            ...
            {
                log_debug(
                    "Interface '%s' not found, but -f specified, ignoring.",
                    iface);
                return 0; /* done */
            }
...
static void print_source(uint64_t flags, usec_t rtt)
{
    ...
    if (flags != 0)
        printf(" protocol%s%s%s%s%s", flags & SD_RESOLVED_DNS ? " DNS" : "",
    ...
    printf(" in %s.%s\n"
           ...
           rtt_str, ansi_normal(), ansi_grey(),
           yes_no(flags & SD_RESOLVED_AUTHENTICATED), ansi_normal());
}
...
static void print_ifindex_comment(int printed_so_far, int ifindex)
{
    ...
    if (!if_indextoname(ifindex, ifname))
        log_warning_errno(
            errno,
            "Failed to resolve interface name for index %i, ignoring: %m",
            ifindex);
    else
        printf("%*s%s-- link: %s%s",
               60 > printed_so_far ? 60 - printed_so_far : 0,
               " ", /* Align comment to the 60th column */
...
static int resolve_host(sd_bus* bus, const char* name)
{
    ...
    log_debug("Resolving %s (family %s, interface %s).", name,
              af_to_name(arg_family) ?: "*",
              isempty(arg_ifname) ? "*" : arg_ifname);

    r = sd_bus_message_new_method_call(
        bus, &req, "org.freedesktop.resolve1", "/org/freedesktop/resolve1",
        "org.freedesktop.resolve1.Manager", "ResolveHostname");
    if (r < 0)
        return bus_log_create_error(r);

    r = sd_bus_message_append(req, "isit", arg_ifindex, name, arg_family,
                              arg_flags);
    if (r < 0)
        return bus_log_create_error(r);
    ...
    if (r < 0)
        return log_error_errno(r, "%s: resolve call failed: %s", name,
    ...
    while ((r = sd_bus_message_enter_container(reply, 'r', "iiay")) > 0)
    {
        ...
        {
            log_debug("%s: skipping entry with family %d (%s)", name, family,
        ...
        {
            log_error("%s: systemd-resolved returned address of invalid size "
                      "%zu for family %s",
        ...
        if (r < 0)
            return log_error_errno(r, "Failed to print address for %s: %m",
                                   name);

        k = printf("%*s%s %s%s%s", (int)strlen(name), c == 0 ? name : "",
                   c == 0 ? ":" : " ", ansi_highlight(), pretty, ansi_normal());
...
static int resolve_address(sd_bus* bus, int family,
                           const union in_addr_union* address, int ifindex)
{
    _cleanup_(sd_bus_message_unrefp) sd_bus_message *req = NULL, *reply = NULL;
    _cleanup_(sd_bus_error_free) sd_bus_error error = SD_BUS_ERROR_NULL;
    _cleanup_free_ char* pretty = NULL;
    ...
    r = sd_bus_message_append_array(req, 'y', address,
    ...
    r = sd_bus_call(bus, req, SD_RESOLVED_QUERY_TIMEOUT_USEC, &error, &reply);
    if (r < 0)
        return log_error_errno(r, "%s: resolve call failed: %s", pretty,
                               bus_error_message(&error, r));
    ...
    while ((r = sd_bus_message_enter_container(reply, 'r', "is")) > 0)
    {
        ...
        k = printf("%*s%s %s%s%s", (int)strlen(pretty), c == 0 ? pretty : "",
                   c == 0 ? ":" : " ", ansi_highlight(), n, ansi_normal());
...
static int resolve_record(sd_bus* bus, const char* name, uint16_t class,
                          uint16_t type, bool warn_missing)
{
    _cleanup_(sd_bus_message_unrefp) sd_bus_message *req = NULL, *reply = NULL;
    _cleanup_(sd_bus_error_free) sd_bus_error error = SD_BUS_ERROR_NULL;
    unsigned n = 0;
    ...
    log_debug("Resolving %s %s %s (interface %s).", name,
    ...
    r = sd_bus_message_append(req, "isqqt", arg_ifindex, name, class, type,
                              arg_flags);
    if (r < 0)
        return bus_log_create_error(r);
    ...
    if ((flags & SD_RESOLVED_AUTHENTICATED) == 0 && needs_authentication)
    {
        ...
        fprintf(stderr,
                "\n%s"
                "WARNING: The resources shown contain cryptographic key data "
                "which could not be\n"
                "         authenticated. It is not suitable to authenticate "
                "any communication.\n"
                "         This is usually indication that DNSSEC "
                "authentication was not enabled\n"
                "         or is not available for the selected protocol or DNS "
                "servers.%s\n",
...
static int resolve_rfc4501(sd_bus* bus, const char* name)
{
    ...
    if (p[0] == '/')
    {
        ...
        if (e != p + 2)
            log_warning("DNS authority specification not supported; ignoring "
                        "specified authority.");
...
invalid:
    return log_error_errno(SYNTHETIC_ERRNO(EINVAL), "Invalid DNS URI: %s",
                           name);
}
...
static int resolve_service(sd_bus* bus, const char* name, const char* type,
                           const char* domain)
{
    ...
    if (name)
        log_debug("Resolving service \"%s\" of type %s in %s (family %s, "
                  "interface %s).",
                  name, type, domain, af_to_name(arg_family) ?: "*",
                  isempty(arg_ifname) ? "*" : arg_ifname);
    else if (type)
        log_debug("Resolving service type %s of %s (family %s, interface %s).",
                  type, domain, af_to_name(arg_family) ?: "*",
                  isempty(arg_ifname) ? "*" : arg_ifname);
    else
        log_debug("Resolving service type %s (family %s, interface %s).",
                  domain, af_to_name(arg_family) ?: "*",
                  isempty(arg_ifname) ? "*" : arg_ifname);
    ...
    r = sd_bus_message_append(req, "isssit", arg_ifindex, name, type, domain,
                              arg_family, arg_flags);
    if (r < 0)
        return bus_log_create_error(r);
    ...
    r = sd_bus_call(bus, req, SD_RESOLVED_QUERY_TIMEOUT_USEC, &error, &reply);
    if (r < 0)
        return log_error_errno(r, "Resolve call failed: %s",
                               bus_error_message(&error, r));
    ...
    indent = (name ? strlen(name) + 1 : 0) + (type ? strlen(type) + 1 : 0) +
             strlen(domain) + 2;
    ...
    while ((r = sd_bus_message_enter_container(reply, 'r', "qqqsa(iiay)s")) > 0)
    {
        ...
        r = sd_bus_message_read(reply, "qqqs", &priority, &weight, &port,
                                &hostname);
        if (r < 0)
            return bus_log_parse_error(r);

        if (name)
            printf("%*s%s", (int)strlen(name), c == 0 ? name : "",
                   c == 0 ? "/" : " ");
        if (type)
            printf("%*s%s", (int)strlen(type), c == 0 ? type : "",
                   c == 0 ? "/" : " ");

        printf("%*s%s %s:%u [priority=%u, weight=%u]\n", (int)strlen(domain),
               c == 0 ? domain : "", c == 0 ? ":" : " ", hostname, port,
               priority, weight);
        ...
        while ((r = sd_bus_message_enter_container(reply, 'r', "iiay")) > 0)
        {
            ...
            if (!IN_SET(family, AF_INET, AF_INET6))
            {
                log_debug("%s: skipping entry with family %d (%s)", name,
                          family, af_to_name(family) ?: "unknown");
                continue;
            }

            if (sz != FAMILY_ADDRESS_SIZE(family))
            {
                log_error("%s: systemd-resolved returned address of invalid "
                          "size %zu for family %s",
                          name, sz, af_to_name(family) ?: "unknown");
                return -EINVAL;
            }
    ...
    while ((r = sd_bus_message_read_array(reply, 'y', (const void**)&p, &sz)) >
           0)
    {
    ...
    r = sd_bus_message_read(reply, "ssst", &canonical_name, &canonical_type,
                            &canonical_domain, &flags);
    if (r < 0)
        return bus_log_parse_error(r);
...
static int resolve_openpgp(sd_bus* bus, const char* address)
{
    ...
    r = resolve_record(bus, full, arg_class ?: DNS_CLASS_IN,
                       arg_type ?: DNS_TYPE_OPENPGPKEY, false);

    if (IN_SET(r, -ENXIO, -ESRCH))
    { /* NXDOMAIN or NODATA? */
        hashed = mfree(hashed);
        ...
        return resolve_record(bus, full, arg_class ?: DNS_CLASS_IN,
...
static int resolve_tlsa(sd_bus* bus, const char* family, const char* address)
{
    ...
    r = asprintf(&full, "_%u._%s.%s", port_num, family, address);
    if (r < 0)
        return log_oom();
...
static int show_statistics(int argc, char** argv, void* userdata)
{
    ...
    sd_bus* bus = userdata;
    uint64_t n_current_transactions, n_total_transactions, cache_size,
        n_cache_hit, n_cache_miss, n_dnssec_secure, n_dnssec_insecure,
    ...
    r = sd_bus_get_property_trivial(
        ...
        "org.freedesktop.resolve1.Manager", "DNSSECSupported", &error, 'b',
        &dnssec_supported);
    if (r < 0)
        return log_error_errno(r, "Failed to get DNSSEC supported state: %s",
                               bus_error_message(&error, r));

    printf("DNSSEC supported by current servers: %s%s%s\n\n", ansi_highlight(),
           yes_no(dnssec_supported), ansi_normal());

    r = sd_bus_get_property(bus, "org.freedesktop.resolve1",
                            "/org/freedesktop/resolve1",
                            "org.freedesktop.resolve1.Manager",
                            "TransactionStatistics", &error, &reply, "(tt)");
    if (r < 0)
        return log_error_errno(r, "Failed to get transaction statistics: %s",
                               bus_error_message(&error, r));

    r = sd_bus_message_read(reply, "(tt)", &n_current_transactions,
    ...
    printf("%sTransactions%s\n"
           ...
           ansi_highlight(), ansi_normal(), n_current_transactions,
           n_total_transactions);
    ...
    r = sd_bus_get_property(bus, "org.freedesktop.resolve1",
                            "/org/freedesktop/resolve1",
                            "org.freedesktop.resolve1.Manager",
                            "CacheStatistics", &error, &reply, "(ttt)");
    if (r < 0)
        return log_error_errno(r, "Failed to get cache statistics: %s",
                               bus_error_message(&error, r));

    r = sd_bus_message_read(reply, "(ttt)", &cache_size, &n_cache_hit,
                            &n_cache_miss);
    if (r < 0)
        return bus_log_parse_error(r);
    ...
    r = sd_bus_get_property(bus, "org.freedesktop.resolve1",
                            "/org/freedesktop/resolve1",
                            "org.freedesktop.resolve1.Manager",
                            "DNSSECStatistics", &error, &reply, "(tttt)");
    if (r < 0)
        return log_error_errno(r, "Failed to get DNSSEC statistics: %s",
                               bus_error_message(&error, r));

    r = sd_bus_message_read(reply, "(tttt)", &n_dnssec_secure,
                            &n_dnssec_insecure, &n_dnssec_bogus,
    ...
    printf("\n%sDNSSEC Verdicts%s\n"
           ...
           ansi_highlight(), ansi_normal(), n_dnssec_secure, n_dnssec_insecure,
           n_dnssec_bogus, n_dnssec_indeterminate);
...
static int reset_statistics(int argc, char** argv, void* userdata)
{
    ...
    r = sd_bus_call_method(bus, "org.freedesktop.resolve1",
                           "/org/freedesktop/resolve1",
                           "org.freedesktop.resolve1.Manager",
                           "ResetStatistics", &error, NULL, NULL);
    if (r < 0)
        return log_error_errno(r, "Failed to reset statistics: %s",
                               bus_error_message(&error, r));
...
static int flush_caches(int argc, char** argv, void* userdata)
{
    ...
    r = sd_bus_call_method(
        bus, "org.freedesktop.resolve1", "/org/freedesktop/resolve1",
        "org.freedesktop.resolve1.Manager", "FlushCaches", &error, NULL, NULL);
    if (r < 0)
        return log_error_errno(r, "Failed to flush caches: %s",
                               bus_error_message(&error, r));
...
static int reset_server_features(int argc, char** argv, void* userdata)
{
    ...
    r = sd_bus_call_method(bus, "org.freedesktop.resolve1",
                           "/org/freedesktop/resolve1",
                           "org.freedesktop.resolve1.Manager",
                           "ResetServerFeatures", &error, NULL, NULL);
    if (r < 0)
        return log_error_errno(r, "Failed to reset server features: %s",
                               bus_error_message(&error, r));
...
static int map_link_dns_servers(sd_bus* bus, const char* member,
                                sd_bus_message* m, sd_bus_error* error,
                                void* userdata)
{
    char*** l = userdata;
    int r;
...
static int map_link_current_dns_server(sd_bus* bus, const char* member,
                                       sd_bus_message* m, sd_bus_error* error,
                                       void* userdata)
{
    assert(m);
...
static int map_link_domains(sd_bus* bus, const char* member, sd_bus_message* m,
...
static int status_print_strv_ifindex(int ifindex, const char* ifname, char** p)
{
    ...
    printf("%sLink %i (%s)%s:", ansi_highlight(), ifindex, ifname,
           ansi_normal());
...
static int status_ifindex(sd_bus* bus, int ifindex, const char* name,
                          StatusMode mode, bool* empty_line)
{
    static const struct bus_properties_map property_map[] = {
        {"ScopesMask", "t", NULL, offsetof(struct link_info, scopes_mask)},
        {"DNS", "a(iay)", map_link_dns_servers,
         offsetof(struct link_info, dns)},
        {"CurrentDNSServer", "(iay)", map_link_current_dns_server,
         offsetof(struct link_info, current_dns)},
        {"Domains", "a(sb)", map_link_domains,
         offsetof(struct link_info, domains)},
        ...
        {"DNSSECNegativeTrustAnchors", "as", NULL,
         offsetof(struct link_info, ntas)},
        {"DNSSECSupported", "b", NULL,
         offsetof(struct link_info, dnssec_supported)},
        {}};
    _cleanup_(sd_bus_error_free) sd_bus_error error = SD_BUS_ERROR_NULL;
    ...
    if (!name)
    {
        if (!if_indextoname(ifindex, ifname))
            return log_error_errno(
                errno, "Failed to resolve interface name for %i: %m", ifindex);
    ...
    r = bus_map_all_properties(bus, "org.freedesktop.resolve1", p, property_map,
                               BUS_MAP_BOOLEAN_AS_BOOL, &error, &m, &link_info);
    if (r < 0)
        return log_error_errno(r, "Failed to get link data for %i: %s", ifindex,
                               bus_error_message(&error, r));
    ...
    {
        printf("%sLink %i (%s)%s: %s\n", ansi_highlight(), ifindex, name,
               ansi_normal(), yes_no(link_info.default_route));
    ...
    {
        printf("%sLink %i (%s)%s: %s\n", ansi_highlight(), ifindex, name,
               ansi_normal(), strna(link_info.llmnr));
    ...
    {
        printf("%sLink %i (%s)%s: %s\n", ansi_highlight(), ifindex, name,
               ansi_normal(), strna(link_info.mdns));
    ...
    {
        printf("%sLink %i (%s)%s: %s\n", ansi_highlight(), ifindex, name,
               ansi_normal(), strna(link_info.dns_over_tls));
    ...
    {
        printf("%sLink %i (%s)%s: %s\n", ansi_highlight(), ifindex, name,
               ansi_normal(), strna(link_info.dnssec));
    ...
    printf("DefaultRoute setting: %s\n"
           ...
           "      DNSSEC setting: %s\n"
           "    DNSSEC supported: %s\n",
           yes_no(link_info.default_route), strna(link_info.llmnr),
           strna(link_info.mdns), strna(link_info.dns_over_tls),
           strna(link_info.dnssec), yes_no(link_info.dnssec_supported));
...
static int map_global_dns_servers(sd_bus* bus, const char* member,
                                  sd_bus_message* m, sd_bus_error* error,
                                  void* userdata)
{
    char*** l = userdata;
...
static int map_global_current_dns_server(sd_bus* bus, const char* member,
                                         sd_bus_message* m, sd_bus_error* error,
                                         void* userdata)
{
    assert(m);
...
static int map_global_domains(sd_bus* bus, const char* member,
                              sd_bus_message* m, sd_bus_error* error,
                              void* userdata)
{
    char*** l = userdata;
...
{
    static const struct bus_properties_map property_map[] = {
        {"DNS", "a(iiay)", map_global_dns_servers,
         offsetof(struct global_info, dns)},
        {"FallbackDNS", "a(iiay)", map_global_dns_servers,
         offsetof(struct global_info, fallback_dns)},
        {"CurrentDNSServer", "(iiay)", map_global_current_dns_server,
         offsetof(struct global_info, current_dns)},
        {"Domains", "a(isb)", map_global_domains,
         offsetof(struct global_info, domains)},
        {"DNSSECNegativeTrustAnchors", "as", NULL,
         offsetof(struct global_info, ntas)},
        ...
        {"DNSSECSupported", "b", NULL,
         offsetof(struct global_info, dnssec_supported)},
        {}};
    _cleanup_(sd_bus_error_free) sd_bus_error error = SD_BUS_ERROR_NULL;
    _cleanup_(sd_bus_message_unrefp) sd_bus_message* m = NULL;
    ...
    r = bus_map_all_properties(
        bus, "org.freedesktop.resolve1", "/org/freedesktop/resolve1",
        property_map, BUS_MAP_BOOLEAN_AS_BOOL, &error, &m, &global_info);
    if (r < 0)
        return log_error_errno(r, "Failed to get global data: %s",
                               bus_error_message(&error, r));
    ...
    printf("       LLMNR setting: %s\n"
           ...
           "      DNSSEC setting: %s\n"
           "    DNSSEC supported: %s\n",
           strna(global_info.llmnr), strna(global_info.mdns),
           strna(global_info.dns_over_tls), strna(global_info.dnssec),
...
static int log_interface_is_managed(int r, int ifindex)
{
    ...
    return log_error_errno(
        r,
        "The specified interface %s is managed by systemd-networkd. Operation "
        "refused.\n"
        "Please configure DNS settings for systemd-networkd managed interfaces "
        "directly in their .network files.",
...
static int verb_dns(int argc, char** argv, void* userdata)
{
    ...
    if (!strv_equal(argv + 2, STRV_MAKE("")))
    {
        STRV_FOREACH(p, argv + 2)
        {
            ...
            if (r < 0)
                return log_error_errno(
                    r, "Failed to parse DNS server address: %s", *p);
            ...
            r = sd_bus_message_append_array(req, 'y', &data.address,
                                            FAMILY_ADDRESS_SIZE(data.family));
            if (r < 0)
                return bus_log_create_error(r);
    ...
    if (r < 0)
    {
        ...
        return log_error_errno(r, "Failed to set DNS configuration: %s",
                               bus_error_message(&error, r));
    }
...
static int verb_domain(int argc, char** argv, void* userdata)
{
    ...
    if (!strv_equal(argv + 2, STRV_MAKE("")))
    {
        STRV_FOREACH(p, argv + 2)
        {
            ...
            if (r < 0)
                return log_error_errno(
                    r, "Failed to validate specified domain %s: %m", n);
            if (r == 0)
            {
    ...
    if (r < 0)
    {
        ...
        return log_error_errno(r, "Failed to set domain configuration: %s",
                               bus_error_message(&error, r));
    }
...
static int verb_default_route(int argc, char** argv, void* userdata)
{
    ...
    if (b < 0)
        return log_error_errno(b, "Failed to parse boolean argument: %s",
                               argv[2]);

    r = sd_bus_call_method(
        bus, "org.freedesktop.resolve1", "/org/freedesktop/resolve1",
        "org.freedesktop.resolve1.Manager", "SetLinkDefaultRoute", &error, NULL,
        "ib", arg_ifindex, b);
    if (r < 0)
    {
        if (sd_bus_error_has_name(&error, BUS_ERROR_LINK_BUSY))
        ...
        return log_error_errno(r,
                               "Failed to set default route configuration: %s",
                               bus_error_message(&error, r));
    }
...
static int verb_llmnr(int argc, char** argv, void* userdata)
{
    ...
    r = sd_bus_call_method(bus, "org.freedesktop.resolve1",
                           ...
                           &error, NULL, "is", arg_ifindex, argv[2]);
    if (r < 0)
    {
        if (sd_bus_error_has_name(&error, BUS_ERROR_LINK_BUSY))
            return log_interface_is_managed(r, arg_ifindex);
        ...
        return log_error_errno(r, "Failed to set LLMNR configuration: %s",
                               bus_error_message(&error, r));
    }
...
static int verb_mdns(int argc, char** argv, void* userdata)
{
    ...
    r = sd_bus_call_method(
        bus, "org.freedesktop.resolve1", "/org/freedesktop/resolve1",
        "org.freedesktop.resolve1.Manager", "SetLinkMulticastDNS", &error, NULL,
    ...
    if (r < 0)
    {
        ...
        return log_error_errno(r,
                               "Failed to set MulticastDNS configuration: %s",
                               bus_error_message(&error, r));
    }
...
static int verb_dns_over_tls(int argc, char** argv, void* userdata)
{
    ...
    r = sd_bus_call_method(
        bus, "org.freedesktop.resolve1", "/org/freedesktop/resolve1",
        "org.freedesktop.resolve1.Manager", "SetLinkDNSOverTLS", &error, NULL,
        "is", arg_ifindex, argv[2]);
    if (r < 0)
    {
        if (sd_bus_error_has_name(&error, BUS_ERROR_LINK_BUSY))
        ...
        return log_error_errno(r, "Failed to set DNSOverTLS configuration: %s",
                               bus_error_message(&error, r));
    }
...
static int verb_dnssec(int argc, char** argv, void* userdata)
{
    ...
    if (r < 0)
    {
        ...
        return log_error_errno(r, "Failed to set DNSSEC configuration: %s",
                               bus_error_message(&error, r));
    }
...
static int verb_nta(int argc, char** argv, void* userdata)
{
    ...
    /* If only argument is the empty string, then call
     * SetLinkDNSSECNegativeTrustAnchors()
     * with an empty list, which will clear the list of domains for an
     * interface. */
    ...
    if (!clear)
        STRV_FOREACH(p, argv + 2)
        {
            ...
            if (r < 0)
                return log_error_errno(
                    r, "Failed to validate specified domain %s: %m", *p);
            if (r == 0)
            {
    ...
    r = sd_bus_message_new_method_call(bus, &req, "org.freedesktop.resolve1",
                                       "/org/freedesktop/resolve1",
                                       "org.freedesktop.resolve1.Manager",
                                       "SetLinkDNSSECNegativeTrustAnchors");
    if (r < 0)
    ...
    if (r < 0)
    {
        ...
        return log_error_errno(r, "Failed to set DNSSEC NTA configuration: %s",
                               bus_error_message(&error, r));
    }
...
static int verb_revert_link(int argc, char** argv, void* userdata)
{
    ...
    if (r < 0)
    {
        ...
        return log_error_errno(r,
                               "Failed to revert interface configuration: %s",
                               bus_error_message(&error, r));
    }
...
static int compat_help(void)
{
    ...
    printf(
        ...
        "\n"
        "Resolve domain names, IPv4 and IPv6 addresses, DNS records, and "
        "services.\n\n"
        ...
        "     --service-address=BOOL Resolve address for services (default: "
        "yes)\n"
        ...
        "     --search=BOOL          Use search domains for single-label "
        "names\n"
        "                                                              "
        "(default: yes)\n"
        "     --raw[=payload|packet] Dump the answer as binary data\n"
        "     --legend=BOOL          Print headers and additional info "
        "(default: yes)\n"
...
static int native_help(void)
{
    ...
    printf(
        ...
        "\n"
        "Commands:\n"
        "  query HOSTNAME|ADDRESS...    Resolve domain names, IPv4 and IPv6 "
        "addresses\n"
        ...
        "  reset-statistics             Reset resolver statistics\n"
        "  flush-caches                 Flush all local DNS caches\n"
        "  reset-server-features        Forget learnt DNS server feature "
        "levels\n"
        "  dns [LINK [SERVER...]]       Get/set per-interface DNS server "
        "address\n"
        "  domain [LINK [DOMAIN...]]    Get/set per-interface search domain\n"
        "  default-route [LINK [BOOL]]  Get/set per-interface default route "
        "flag\n"
        "  llmnr [LINK [MODE]]          Get/set per-interface LLMNR mode\n"
        "  mdns [LINK [MODE]]           Get/set per-interface MulticastDNS "
        "mode\n"
        "  dnsovertls [LINK [MODE]]     Get/set per-interface DNS-over-TLS "
        "mode\n"
...
static int compat_parse_argv(int argc, char* argv[])
{
    ...
    while ((c = getopt_long(argc, argv, "h46i:t:c:p:", options, NULL)) >= 0)
        switch (c)
        {
            ...
            case ARG_LEGEND:
                ...
                if (r < 0)
                    return log_error_errno(
                        r, "Failed to parse --legend= argument");
            ...
            case ARG_CNAME:
                ...
                if (r < 0)
                    return log_error_errno(
                        r, "Failed to parse --cname= argument.");
            ...
            case ARG_SERVICE_ADDRESS:
                ...
                if (r < 0)
                    return log_error_errno(
                        r, "Failed to parse --service-address= argument.");
            ...
            case ARG_SERVICE_TXT:
                ...
                if (r < 0)
                    return log_error_errno(
                        r, "Failed to parse --service-txt= argument.");
            ...
            case ARG_SEARCH:
                ...
                if (r < 0)
                    return log_error_errno(
                        r, "Failed to parse --search argument.");
    ...
    if (IN_SET(arg_mode, MODE_SET_LINK, MODE_REVERT_LINK))
    {
        ...
        if (arg_ifindex <= 0)
            return log_error_errno(
                SYNTHETIC_ERRNO(EINVAL),
                "--set-dns=, --set-domain=, --set-llmnr=, --set-mdns=, "
                "--set-dnsovertls=, --set-dnssec=, --set-nta= and --revert "
                "require --interface=.");
    }
...
static int translate(const char* verb, const char* single_arg, size_t num_args,
                     char** args, sd_bus* bus)
{
    char **fake, **p;
    size_t num, i;
...
static int compat_main(int argc, char* argv[], sd_bus* bus)
{
    ...
    switch (arg_mode)
    {
        ...
        case MODE_RESOLVE_TLSA:
            return translate("tlsa", arg_service_family, argc - optind,
                             argv + optind, bus);
        ...
        case MODE_SET_LINK:
            ...
            if (arg_set_dns)
            {
                r = translate("dns", arg_ifname, strv_length(arg_set_dns),
                              arg_set_dns, bus);
                if (r < 0)
                    return r;
            }
            ...
            if (arg_set_domain)
            {
                r = translate("domain", arg_ifname, strv_length(arg_set_domain),
                              arg_set_domain, bus);
                if (r < 0)
                    return r;
            }
            ...
            if (arg_set_nta)
            {
                r = translate("nta", arg_ifname, strv_length(arg_set_nta),
                              arg_set_nta, bus);
                if (r < 0)
                    return r;
            }
            ...
            {
                r = translate("dnsovertls", arg_ifname, 1,
                              (char**)&arg_set_dns_over_tls, bus);
                if (r < 0)
                    return r;
            }
