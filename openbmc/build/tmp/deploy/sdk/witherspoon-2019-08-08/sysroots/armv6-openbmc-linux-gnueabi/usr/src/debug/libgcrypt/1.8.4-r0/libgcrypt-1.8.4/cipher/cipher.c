...
static gcry_cipher_spec_t* spec_from_algo(int algo)
{
    int idx;
...
static gcry_cipher_spec_t* spec_from_name(const char* name)
{
    gcry_cipher_spec_t* spec;
    int idx;
    const char** aliases;
...
static gcry_cipher_spec_t* spec_from_oid(const char* oid)
{
    gcry_cipher_spec_t* spec;
...
static gcry_cipher_spec_t* search_oid(const char* oid,
...
/* Map STRING to the cipher algorithm identifier.  Returns the
   ...
   return value of 0. */
int _gcry_cipher_map_name(const char* string)
{
    gcry_cipher_spec_t* spec;
...
int _gcry_cipher_mode_from_oid(const char* string)
{
    gcry_cipher_spec_t* spec;
    gcry_cipher_oid_spec_t oid_spec;
...
const char* _gcry_cipher_algo_name(int algorithm)
{
    gcry_cipher_spec_t* spec;
...
/* Flag the cipher algorithm with the identifier ALGORITHM as
   ...
   called early. */
static void disable_cipher_algo(int algo)
{
...
/* Return 0 if the cipher algorithm with identifier ALGORITHM is
   ...
   available.  */
static gcry_err_code_t check_cipher_algo(int algorithm)
{
    gcry_cipher_spec_t* spec;
...
static unsigned int cipher_get_keylen(int algorithm)
{
    gcry_cipher_spec_t* spec;
    unsigned len = 0;
...
static unsigned int cipher_get_blocksize(int algorithm)
{
    gcry_cipher_spec_t* spec;
    unsigned len = 0;
 ...
 */
gcry_err_code_t _gcry_cipher_open(gcry_cipher_hd_t* handle, int algo, int mode,
                                  unsigned int flags)
{
    gcry_err_code_t rc;
...
gcry_err_code_t _gcry_cipher_open_internal(gcry_cipher_hd_t* handle, int algo,
                                           int mode, unsigned int flags)
{
    ...
    /* check flags */
    if ((!err) && ((flags & ~(0 | GCRY_CIPHER_SECURE | GCRY_CIPHER_ENABLE_SYNC |
    ...
    if (!err)
    {
        size_t size = (sizeof(*h) + 2 * spec->contextsize -
...
static gcry_err_code_t cipher_setkey(gcry_cipher_hd_t c, byte* key,
                                     size_t keylen)
{
    gcry_err_code_t rc;
    ...
    if (!rc)
    {
        ...
        memcpy((void*)((char*)&c->context.c + c->spec->contextsize),
               (void*)&c->context.c, c->spec->contextsize);
        c->marks.key = 1;
...
static gcry_err_code_t cipher_setiv(gcry_cipher_hd_t c, const byte* iv,
                                    size_t ivlen)
{
...
static void cipher_reset(gcry_cipher_hd_t c)
{
    unsigned int marks_key;

    marks_key = c->marks.key;

    memcpy(&c->context.c, (char*)&c->context.c + c->spec->contextsize,
           c->spec->contextsize);
...
static gcry_err_code_t do_ecb_crypt(gcry_cipher_hd_t c, unsigned char* outbuf,
                                    size_t outbuflen,
...
static gcry_err_code_t do_ecb_encrypt(gcry_cipher_hd_t c, unsigned char* outbuf,
                                      size_t outbuflen,
...
static gcry_err_code_t do_ecb_decrypt(gcry_cipher_hd_t c, unsigned char* outbuf,
                                      size_t outbuflen,
                                      const unsigned char* inbuf,
                                      size_t inbuflen)
{
 ...
 */
static gcry_err_code_t cipher_encrypt(gcry_cipher_hd_t c, byte* outbuf,
                                      size_t outbuflen, const byte* inbuf,
                                      size_t inbuflen)
{
    gcry_err_code_t rc;
    ...
    switch (c->mode)
    {
        ...
        case GCRY_CIPHER_MODE_CBC:
            rc =
                _gcry_cipher_cbc_encrypt(c, outbuf, outbuflen, inbuf, inbuflen);
            break;

        case GCRY_CIPHER_MODE_CFB:
            rc =
                _gcry_cipher_cfb_encrypt(c, outbuf, outbuflen, inbuf, inbuflen);
            break;
        ...
        case GCRY_CIPHER_MODE_OFB:
            rc =
                _gcry_cipher_ofb_encrypt(c, outbuf, outbuflen, inbuf, inbuflen);
            break;

        case GCRY_CIPHER_MODE_CTR:
            rc =
                _gcry_cipher_ctr_encrypt(c, outbuf, outbuflen, inbuf, inbuflen);
            break;

        case GCRY_CIPHER_MODE_AESWRAP:
            rc = _gcry_cipher_aeswrap_encrypt(c, outbuf, outbuflen, inbuf,
                                              inbuflen);
            break;

        case GCRY_CIPHER_MODE_CCM:
            rc =
                _gcry_cipher_ccm_encrypt(c, outbuf, outbuflen, inbuf, inbuflen);
            break;
        ...
        case GCRY_CIPHER_MODE_GCM:
            rc =
                _gcry_cipher_gcm_encrypt(c, outbuf, outbuflen, inbuf, inbuflen);
            break;

        case GCRY_CIPHER_MODE_POLY1305:
            rc = _gcry_cipher_poly1305_encrypt(c, outbuf, outbuflen, inbuf,
                                               inbuflen);
            break;

        case GCRY_CIPHER_MODE_OCB:
            rc =
                _gcry_cipher_ocb_encrypt(c, outbuf, outbuflen, inbuf, inbuflen);
            break;

        case GCRY_CIPHER_MODE_XTS:
            rc = _gcry_cipher_xts_crypt(c, outbuf, outbuflen, inbuf, inbuflen,
                                        1);
            break;

        case GCRY_CIPHER_MODE_STREAM:
            c->spec->stencrypt(&c->context.c, outbuf, (byte*)/*arggg*/ inbuf,
                               inbuflen);
            rc = 0;
            break;
 ...
 * been requested.
 */
gcry_err_code_t _gcry_cipher_encrypt(gcry_cipher_hd_t h, void* out,
 ...
 */
static gcry_err_code_t cipher_decrypt(gcry_cipher_hd_t c, byte* outbuf,
                                      size_t outbuflen, const byte* inbuf,
                                      size_t inbuflen)
{
    gcry_err_code_t rc;
    ...
    switch (c->mode)
    {
        ...
        case GCRY_CIPHER_MODE_CBC:
            rc =
                _gcry_cipher_cbc_decrypt(c, outbuf, outbuflen, inbuf, inbuflen);
            break;

        case GCRY_CIPHER_MODE_CFB:
            rc =
                _gcry_cipher_cfb_decrypt(c, outbuf, outbuflen, inbuf, inbuflen);
            break;
        ...
        case GCRY_CIPHER_MODE_AESWRAP:
            rc = _gcry_cipher_aeswrap_decrypt(c, outbuf, outbuflen, inbuf,
                                              inbuflen);
            break;

        case GCRY_CIPHER_MODE_CCM:
            rc =
                _gcry_cipher_ccm_decrypt(c, outbuf, outbuflen, inbuf, inbuflen);
            break;
        ...
        case GCRY_CIPHER_MODE_GCM:
            rc =
                _gcry_cipher_gcm_decrypt(c, outbuf, outbuflen, inbuf, inbuflen);
            break;

        case GCRY_CIPHER_MODE_POLY1305:
            rc = _gcry_cipher_poly1305_decrypt(c, outbuf, outbuflen, inbuf,
                                               inbuflen);
            break;

        case GCRY_CIPHER_MODE_OCB:
            rc =
                _gcry_cipher_ocb_decrypt(c, outbuf, outbuflen, inbuf, inbuflen);
            break;
        ...
        case GCRY_CIPHER_MODE_STREAM:
            c->spec->stdecrypt(&c->context.c, outbuf, (byte*)/*arggg*/ inbuf,
                               inbuflen);
            rc = 0;
            break;
...
gcry_err_code_t _gcry_cipher_decrypt(gcry_cipher_hd_t h, void* out,
                                     size_t outsize, const void* in,
                                     size_t inlen)
{
    if (!in) /* Caller requested in-place encryption. */
 ...
 */
static void cipher_sync(gcry_cipher_hd_t c)
{
    if ((c->flags & GCRY_CIPHER_ENABLE_SYNC) && c->unused)
    {
        memmove(c->u_iv.iv + c->unused, c->u_iv.iv,
                c->spec->blocksize - c->unused);
        memcpy(c->u_iv.iv, c->lastiv + c->spec->blocksize - c->unused,
               c->unused);
        c->unused = 0;
    }
}

gcry_err_code_t _gcry_cipher_setkey(gcry_cipher_hd_t hd, const void* key,
                                    size_t keylen)
{
    return cipher_setkey(hd, (void*)key, keylen);
}

gcry_err_code_t _gcry_cipher_setiv(gcry_cipher_hd_t hd, const void* iv,
                                   size_t ivlen)
{
...
/* Set counter for CTR mode.  (CTR,CTRLEN) must denote a buffer of
   ...
   block. */
gpg_err_code_t _gcry_cipher_setctr(gcry_cipher_hd_t hd, const void* ctr,
                                   size_t ctrlen)
{
...
gpg_err_code_t _gcry_cipher_getctr(gcry_cipher_hd_t hd, void* ctr,
                                   size_t ctrlen)
{
    if (ctr && ctrlen == hd->spec->blocksize)
...
gcry_err_code_t _gcry_cipher_authenticate(gcry_cipher_hd_t hd, const void* abuf,
                                          size_t abuflen)
{
    gcry_err_code_t rc;
...
gcry_err_code_t _gcry_cipher_gettag(gcry_cipher_hd_t hd, void* outtag,
                                    size_t taglen)
{
    gcry_err_code_t rc;
...
gcry_err_code_t _gcry_cipher_checktag(gcry_cipher_hd_t hd, const void* intag,
                                      size_t taglen)
{
    gcry_err_code_t rc;
...
gcry_err_code_t _gcry_cipher_ctl(gcry_cipher_hd_t h, int cmd, void* buffer,
                                 size_t buflen)
{
    gcry_err_code_t rc = 0;
    ...
    switch (cmd)
    {
        ...
        case PRIV_CIPHERCTL_DISABLE_WEAK_KEY: /* (private)  */
            if (h->spec->set_extra_info)
                rc = h->spec->set_extra_info(&h->context.c,
                                             CIPHER_INFO_NO_WEAK_KEY, NULL, 0);
            else
        ...
        case GCRYCTL_SET_SBOX:
            if (h->spec->set_extra_info)
                rc = h->spec->set_extra_info(&h->context.c, GCRYCTL_SET_SBOX,
                                             buffer, buflen);
            else
                rc = GPG_ERR_NOT_SUPPORTED;
            break;
 ...
 */
gcry_err_code_t _gcry_cipher_info(gcry_cipher_hd_t h, int cmd, void* buffer,
                                  size_t* nbytes)
{
    gcry_err_code_t rc = 0;
 ...
 */
gcry_err_code_t _gcry_cipher_algo_info(int algo, int what, void* buffer,
                                       size_t* nbytes)
{
    gcry_err_code_t rc = 0;
    unsigned int ui;
