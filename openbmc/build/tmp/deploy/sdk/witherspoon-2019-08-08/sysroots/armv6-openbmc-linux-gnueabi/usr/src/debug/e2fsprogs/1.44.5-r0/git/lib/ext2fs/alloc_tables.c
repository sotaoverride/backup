                                      ...
                                      ext2fs_block_bitmap bmap)
{
    ...
    {
        retval = ext2fs_get_free_blocks2(fs, group_blk, last_blk, 1, bmap,
                                         &start_blk);
        if (retval)
            return retval;
        start_blk += fs->inode_blocks_per_group;
        start_blk += ((fs->stride * group) % (last_blk - start_blk + 1));
    ...
    if (flexbg_size)
    {
        ...
        start_blk = flexbg_offset(fs, group, prev_block, bmap, rem_grps, 1);
        last_blk = ext2fs_group_last_block2(fs, last_grp);
    }
    ...
    {
        retval =
            ext2fs_get_free_blocks2(fs, start_blk, last_blk, 1, bmap, &new_blk);
        ...
        if (flexbg_size)
        {
            ...
            ext2fs_bg_free_blocks_count_set(
                fs, gr, ext2fs_bg_free_blocks_count(fs, gr) - 1);
    ...
    if (flexbg_size)
    {
        ...
        if (group % flexbg_size)
            ...
            prev_block = ext2fs_block_bitmap_loc(fs, group) + table_offset;
        /* FIXME: Take backup group descriptor blocks into account
         * if the flexbg allocations will grow to overlap them... */
        start_blk = flexbg_offset(fs, group, prev_block, bmap, rem_grps, 1);
        last_blk = ext2fs_group_last_block2(fs, last_grp);
    ...
    if (flexbg_size)
    {
        ...
        if (group % flexbg_size)
            ...
            prev_block = ext2fs_inode_bitmap_loc(fs, group) + table_offset;
    ...
    {
        retval =
            ext2fs_get_free_blocks2(fs, group_blk, last_blk,
                                    fs->inode_blocks_per_group, bmap, &new_blk);
        if (retval)
            return retval;
